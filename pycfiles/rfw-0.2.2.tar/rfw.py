...
# [GCC 8.4.0]
# Embedded file name: /home/sk/seckiss/rfw/rfw/rfw.py
# Compiled at: 2014-03-26 12:32:11
...
def create_requesthandlers(rfwconf, cmd_queue, expiry_queue):
    ...
    return (
     LocalRequestHandler, OutwardRequestHandler)
...
def main():
    ...
    cmd_queue = Queue()
    rfwthreads.CommandProcessor(cmd_queue, rfwconf.whitelist(), expiry_queue, rfwconf.default_expire()).start()
    ...
    if rfwconf.is_outward_server():
        server_address = (
         rfwconf.outward_server_ip(), int(rfwconf.outward_server_port()))
        httpd = SSLServer(server_address, OutwardHandlerClass, rfwconf.outward_server_certfile(), rfwconf.outward_server_keyfile())
        rfwthreads.ServerRunner(httpd).start()
    if rfwconf.is_local_server():
        server_address = (
         '127.0.0.1', int(rfwconf.local_server_port()))
        httpd = PlainServer(server_address, LocalHandlerClass)
        rfwthreads.ServerRunner(httpd).start()
