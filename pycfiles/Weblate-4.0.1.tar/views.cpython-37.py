...
# [GCC 8.4.0]
# Embedded file name: /home/nijel/weblate/weblate/weblate/fonts/views.py
# Compiled at: 2020-03-12 04:44:13
# Size of source mod 2**32: 6285 bytes
    ...
    def post(self, request, **kwargs):
        if not request.user.has_perm('project.edit', self.project):
            ...
            if request.FILES:
                ...
                form = self._group_form = FontGroupForm((request.POST),
...
class FontGroupDetailView(ProjectViewMixin, DetailView):
    ...
    def post(self, request, **kwargs):
        ...
        if 'override' in request.POST:
            try:
                self.object.fontoverride_set.filter(pk=(int(request.POST['override']))).delete()
                return redirect(self.object)
