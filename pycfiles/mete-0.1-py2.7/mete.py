...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mete/mete.py
...
import os.path, sys, cPickle, numpy as np, matplotlib.pyplot as plt, mpmath as mp
    ...
    for i, S in enumerate(Svals):
        ...
        if (
         S, N) in beta_dict:
            ...
            exp_neg_beta = bisect(y, BOUNDS[0] + DIST_FROM_BOUND, min((sys.float_info[0] / S) ** (1 / N), 2), xtol=1.490116e-08)
            betas.append(-1 * log(exp_neg_beta))
        elif version == 'untruncated':
            y = lambda x: 1 / log(1 / (1 - x)) * x / (1 - x) - N / S
            exp_neg_beta = bisect(y, BOUNDS[0] + DIST_FROM_BOUND, BOUNDS[1] - DIST_FROM_BOUND)
            betas.append(-1 * log(exp_neg_beta))
        elif version == 'approx':
...
def get_mete_sad(S0, N0, beta=None, bin_edges=None):
    ...
    if bin_edges != None:
        ...
        for edge in range(0, len(bin_edges) - 1):
            bin_probability = sum(pmf[((N >= bin_edges[edge]) & (N < bin_edges[(edge + 1)]))])
...
def get_lambda_spatialdistrib(A, A0, n0):
    ...
    if A < A0 / 2:
        BOUNDS = [
         0, 1]
        DIST_FROM_BOUND = 1e-15
        exp_neg_lambda = bisect(y, BOUNDS[0] + DIST_FROM_BOUND, BOUNDS[1] - DIST_FROM_BOUND)
    elif A == A0 / 2:
        exp_neg_lambda = 1
    else:
...
def get_spatialdistrib_dict(A, A0, n0, lambda_list=[
 0, {}]):
    """Solve for lambda_Pi from Harte 2011 equ. 7.50 and 7.51
...
def upscale_sar(A, S, N, Amax):
    ...
    def equations_for_S_2A(x, S_A, N_A):
        ...
        out = [
         x[1] / x[0] - 2 * N_A * (1 - x[0]) / (x[0] - x[0] ** (2 * N_A + 1)) * (1 - x[0] ** (2 * N_A) / (2 * N_A + 1)) - S_A]
...
def chi_heap(i, j, n0, chi_dict={}):
    ...
    if n0 == 1:
        ...
        if key not in chi_dict:
            if j == 1:
                chi_dict[key] = (n0 + 1) ** (-1) * sum(map(lambda m: get_lambda_heap(i - 1, m) * get_lambda_heap(i - 1, n0 - m), range(1, n0)))
            else:
                i -= 1
                j -= 1
...
def chi_bisect(i, j, n0, psi, chi_dict={}):
    ...
    if n0 == 1:
        ...
        if key not in chi_dict:
            if j == 1:
                chi_dict[key] = sum(map(lambda m: single_prob(m, n0, psi) * get_lambda_bisect(i - 1, m, psi) * get_lambda_bisect(i - 1, n0 - m, psi), range(1, n0)))
            else:
                i -= 1
                j -= 1
                chi_dict[key] = sum(map(lambda m: single_prob(m, n0, psi) * chi_bisect(i, j, m, psi, chi_dict), range(2, n0 + 1)))
        out = chi_dict[key]
    return out
...
def sim_spatial_whole(S, N, bisec, transect=False, abu=None, beta=None):
    ...
    while bisec_num < bisec:
        ...
        for cell in abu_prev:
            ...
            if transect:
                cell_new_1 = [
                 x_prev * 2 - 1, y_prev, abu_new_cell[0]]
                ...
                cell_new_1 = [
                 x_prev, y_prev * 2 - 1, abu_new_cell[0]]
...
def community_energy_pdf(epsilon, S0, N0, E0):
    ...
    return S0 / N0 * (exp_neg_gamma / (1 - exp_neg_gamma) ** 2 - exp_neg_gamma ** N0 / (1 - exp_neg_gamma) * (N0 + exp_neg_gamma / (1 - exp_neg_gamma)))
