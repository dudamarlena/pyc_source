...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mete_distributions/mete_distributions.py
...
trunc_logser = trunc_logser_gen(a=1, name='trunc_logser', longname='Upper truncated logseries', shapes='upper_bound', extradoc='Truncated logseries\n                                \n                                Upper truncated logseries distribution\n                                ')
...
class psi_epsilon:
    ...
    def __init__(self, S0, N0, E0):
        ...
        self.norm_factor = self.lambda2 / ((exp(-self.beta) - exp(-self.beta * (N0 + 1))) / (1 - exp(-self.beta)) - (exp(-self.sigma) - exp(-self.sigma * (N0 + 1))) / (1 - exp(-self.sigma)))

    def pdf(self, x):
        exp_neg_gamma = exp(-(self.beta + (x - 1) * self.lambda2))
        return self.norm_factor * exp_neg_gamma * (1 - (self.N0 + 1) * exp_neg_gamma ** self.N0 + self.N0 * exp_neg_gamma ** (self.N0 + 1)) / (1 - exp_neg_gamma) ** 2
...
class theta_epsilon:
    ...
    def pdf(self, x, n):
        pdf = self.lambda2 * n * exp(-(self.lambda1 + self.lambda2 * x) * n) / (exp(-self.beta * n) - exp(-self.sigma * n))
        return pdf
