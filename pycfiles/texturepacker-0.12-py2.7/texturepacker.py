...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.4-x86_64/egg/texturepacker.py
# Compiled at: 2011-06-11 06:05:23
"""
texture.py
...
"""
import sys, os, weakref, re
from zipfile import ZipFile, ZIP_DEFLATED
from StringIO import StringIO
from base64 import b64decode
from datetime import datetime
import Image, httplib2, fnmatch, json, yaml
...
MINECRAFT_DIR_PATH_FUNCS = {'windows': lambda : os.path.expandvars('%appdata%\\.minecraft'), 
...
def minecraft_dir_path():
    ...
    func = MINECRAFT_DIR_PATH_FUNCS.get(sys.platform) or MINECRAFT_DIR_PATH_FUNCS['default']
    return func()
...
def resolve_generic_url(base_url, url_ref):
    u"""Resolve a relative reference per RFC 3986
...
class PackBase(object):
    ...
    def get_last_modified(self):
        ...
        """
        return max(self.get_resource(n).get_last_modified() for n in self.get_resource_names())
    ...
    def is_modified_since(self, then):
        ...
        """
        return any(self.get_resource(n).is_modified_since(then) for n in self.get_resource_names())
...
class RecipePack(PackBase):
    ...
    def __init__(self, label, desc, atlas=None):
        ...
        self.desc = desc
        self.resources = {}
        self.add_resource(TextResource('pack.txt', ('{label}\n{desc}').format(label=label, desc=desc)))
...
class SourcePack(PackBase):
    ...
    def __init__(self, zip_data, atlas):
        ...
        if isinstance(zip_data, basestring) and os.path.isdir(zip_data):
    ...
    def get_resource_last_modified(self, name):
        u"""Helper function to get last-modified of a resource.
...
class MapBase(object):
    ...
    def get_alts_list(self):
        ...
        for cell_name in self.names:
            ...
            if names:
                ngroups.setdefault(group_name, {})[cell_name] = [
                 cell_name] + names
...
class NotInMap(Exception):
    ...
    def __init__(self, name, map):
        super(NotInMap, self).__init__(('{0!r} not found in map (must be one of {1})').format(name, (', ').join(map.names)))
        self.name = name
        self.map = map
...
class GridMap(MapBase):
    ...
    def get_css_background_dimens(self):
        return (
         self.nx * self.cell_wd, self.ny * self.cell_ht)
...
class CompositeResource(ImagingResourceBase):
    ...
    def get_last_modified(self):
        return max([self.res.get_last_modified()] + [ x.get_last_modified() for x, _, _ in self.replacements ])
...
class UnknownPack(Exception):
    ...
    def __init__(self, pack_spec, exc=None):
        ...
        super(UnknownPack, self).__init__(('{0!r}: specified pack is not in mixer{1}').format(pack_spec, extra))
...
class MissingParameter(Exception):
    ...
    def __init__(self, param_spec, exc=None):
        extra = ': ' + unicode(exc) if exc else ''
        super(MissingParameter, self).__init__(('{0!r}: required parameter is not in mixer{1}').format(param_spec, extra))
        self.inner_exception = exc
...
class Mixer(object):
            ...
            if isinstance(file_spec, basestring):
                ...
                if 'replace' in file_spec:
                    ...
                    for spec in specs:
                        ...
                        src2_res = src2_pack.get_resource(spec.get('source', res_name))
                        src2_map = self.get_map(src2_pack.atlas, spec.get('map', src2_res.name), base)
                        cells = spec['cells']
    ...
    def _load_raw_pack(self, atlas, pack_spec, base):
        ...
        except CouldNotLoad as e:
            raise UnknownPack(pack_spec, e)
        except DudeItsADirectory as e:
    ...
    def get_atlas(self, atlas_spec, base, atlas=None):
        ...
        if atlas_spec:
            ...
            atlas_base = base if not hasattr(atlas_spec, 'items') else ({'file': resolve_file_path(atlas_spec['file'], base)} if 'file' in atlas_spec else self.loader.get_url(atlas_spec, base))
