...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mimseq/tRNAmap.py
# Compiled at: 2020-03-18 11:21:30
...
def mainAlign(sampleData, experiment_name, genome_index_path, genome_index_name, snp_index_path, snp_index_name, out_dir, threads, snp_tolerance, keep_temp, mismatches, map_round):
    if map_round == 2:
        log.info('\n+------------------+ \t\t\t\n| Realigning reads |\t\t\t\n+------------------+')
    else:
        if map_round == 1:
    ...
    coverageData.close()
    return (
     unique_bam_list, coverageData.name)


def remap(fq, genome_index_path, genome_index_name, snp_index_path, snp_index_name, threads, out_dir, snp_tolerance, keep_temp, mismatches):
    ...
    if not keep_temp:
        ...
        if snp_tolerance:
            map_cmd = 'gsnap -D ' + genome_index_path + ' -d ' + genome_index_name + ' -V ' + snp_index_path + ' -v ' + snp_index_name + ' -t ' + str(threads) + ' --split-output ' + out_dir + output_prefix + ' --format=sam --genome-unk-mismatch=0 --md-lowercase-snp  --ignore-trim-in-filtering 1 --force-single-end ' + mismatch_string + nomap_fastq + ' ' + multi_fastq + ' &>> ' + out_dir + 'remap_align.log'
        else:
            map_cmd = 'gsnap -D ' + genome_index_path + ' -d ' + genome_index_name + ' -t ' + str(threads) + ' --split-output ' + out_dir + output_prefix + ' --format=sam --genome-unk-mismatch=0 --md-lowercase-snp --ignore-trim-in-filtering 1 --force-single-end ' + mismatch_string + ' ' + nomap_fastq + ' ' + multi_fastq + ' &>> ' + out_dir + 'remap_align.log'
    ...
    with open(out_dir + 'mapping_stats.txt', 'a') as (stats_out):
        ...
        for file in align_pathlist:
            if re.search('mult', file.name) and not re.search('bam', file.name):
                ...
                if re.search('uniq', file.name):
                    if not re.search('bam', file.name):
                        ...
                        if re.search('nomapping', file.name):
                            if not re.search('bam', file.name):
...
def mapReads(fq, genome_index_path, genome_index_name, snp_index_path, snp_index_name, threads, out_dir, snp_tolerance, keep_temp, mismatches, remap):
    zipped = ''
    if re.search('.gz', fq):
        zipped = '--gunzip'
    ...
    if not mismatches == None:
        mismatch_list = [
         '--max-mismatches', str(mismatches)]
    else:
        if mismatches == None:
            mismatch_list = ''
        if snp_tolerance:
            map_cmd = [
             'gsnap', zipped, '-D', genome_index_path, '-d', genome_index_name, '-V', snp_index_path, '-v',
            ...
            map_cmd = [
             'gsnap', zipped, '-D', genome_index_path, '-d', genome_index_name, '-t', str(threads),
    ...
    cmd = [
     'samtools', 'view', '-c', out_dir + output_prefix + '.unpaired_transloc']
    ...
    with open(out_dir + 'mapping_stats.txt', 'a') as (stats_out):
        ...
        for file in align_pathlist:
            if re.search('mult', file.name) and not re.search('bam', file.name):
                cmd = [
                 'samtools', 'view', '-@', str(threads), '-F', '0x904', '-c', out_dir + file.name]
                multi_count = int(subprocess.check_output(cmd))
                if keep_temp or remap:
                    cmd = [
                     'samtools', 'view', '-@', str(threads), '-bh', '-o', out_dir + file.name + '.bam', out_dir + file.name]
                ...
                if re.search('uniq', file.name):
                    if not re.search('bam', file.name):
                        cmd = [
                         'samtools', 'view', '-@', str(threads), '-c', out_dir + file.name]
