...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/ZPTKit/zptwareweb.py
...
import sys, os, templatetools, emailtemplate

class ZPTComponent(object):
    __module__ = __name__

    def __init__(self):
        self.template_pool = templatetools.TemplatePool(templatetools.FilePageTemplate)
        self.email_template_pool = templatetools.TemplatePool(emailtemplate.EmailTemplate)
    ...
    def awake_event(self, servlet, next_method, call_setup=None):
        ...
        servlet.context = AttrDict(servlet=servlet, request=servlet.fields)
    ...
    def template_paths(self, servlet):
        ...
        if config.get('template_directory'):
            base_dirs.append(config['template_directory'])
        if not base_dirs:
            base_dirs = [
             'templates/']
        return [ os.path.join(config['root_path'], dir) for dir in base_dirs ]
    ...
    def write_template(self, servlet):
        ...
        if view == '<DEFAULT>':
            ...
            servlet_base = servlet.config['servlet_directory']
            assert module_path.startswith(servlet_base), 'Module path (%r) is not under publish_dir path (%r)' % (module_path, base_path)
            servlet_path = os.path.splitext(module_path[len(servlet_base) + 1:])[0]
            assert not servlet_path.startswith('/'), 'Servlet path should not start with /: %r' % servlet_path
        ...
        servlet.write(template(context=servlet.context, **servlet.options))
        return
    ...
    def template(self, servlet, base_filename, pool=None):
        ...
        """
        if pool is None:
            pool = self.get_pool(tuple(self.template_paths(servlet)), self._tmpl_pools, templatetools.FilePageTemplate)
        template = pool.get_template(base_filename)
        return template

    def send_mail(self, servlet, filename, to_address, from_address, **options):
        pool = self.get_pool(tuple(self.template_paths(servlet)), self._email_pool, emailtemplate.EmailTemplate)
        ...
        return emailtemplate.send_mail(template, to_address=to_address, from_address=from_address, **options)
