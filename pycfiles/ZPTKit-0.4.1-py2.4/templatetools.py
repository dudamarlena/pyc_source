...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/ZPTKit/templatetools.py
...
class TemplatePool(object):
    ...
    def get_template(self, filename, refresh=True, dir=None):
        ...
        if dir is None:
            for pt_dir in self.here_dirs:
                ...
                raise KeyError('No template %s found (looked in %s)' % (filename, (', ').join(self.here_dirs)))
        ...
        for pt_dir in dirs:
            if os.path.exists(os.path.join(pt_dir, filename)):
                if os.path.isdir(os.path.join(pt_dir, filename)):
                    return HereSpace(self.get_template_or_dir, (
                     [
                      os.path.join(pt_dir, filename)],))
                else:
                    return self.get_template(filename, dir=pt_dir)
        else:
            raise KeyError('No template or dir %s found (looked in %s)' % (filename, (', ').join(dirs)))
    ...
    def make_template(self, filename, *args):
        ...
        if tmpl._v_errors:
            raise PTRuntimeError('Template %s has errors:\n%s\n%s' % (filename, tmpl._v_errors[0], tmpl._v_errors[1]))
        return tmpl

    def __repr__(self):
        return '<TemplatePool %s here_dirs=%s pt_class=%s>' % (hex(abs(id(self)))[2:], self.here_dirs, self.pt_class)
...
class HereSpace(object):
    ...
    def __repr__(self):
        return '<%s %s, factory=%s %s>' % (self.__class__.__name__, hex(abs(id(self)))[2:], self.factory, self.factory_args)
...
class FilePageTemplate(PageTemplate):
    ...
    def __call__(self, context=None, *args, **kw):
        ...
        extra_context = {'options': kw, 'here': self.here, 'test': test}
        extra_context.update(self.other_vars)
        context.update(extra_context)
        return self.pt_render(extra_context=context)

    def __repr__(self):
        return '<%s %s %s>' % (self.__class__.__name__, hex(abs(id(self)))[2:], self.filename)
...
class EggTemplateSpace(object):
    ...
    def __init__(self, egg, spec, factory):
        self.egg = egg
        self.spec = spec
        self.egg_map = self.egg.get_entry_map('zptkit.template_dir')
        if not self.egg_map:
            raise KeyError('The Egg %s (from %s) has no zptkit.template_dir entry points' % (self.egg, self.egg.location))
        self.factory = factory

    def __getitem__(self, key):
        if key in self.egg_map:
            return EggEntryPoint(self.factory, self.egg, key, self.egg_map[key].load())
        elif 'main' in self.egg_map:
            return EggEntryPoint(self.factory, self.egg, 'main', self.egg_map['main'].load())[key]
        else:
            raise KeyError("The Egg %s doesn't have an entry point %r or a main entry point" % (self.egg, key))
...
class EggEntryPoint(object):
    ...
    def __init__(self, factory, egg, entry_point_name, resource_name):
        self.factory = factory
        self.egg = egg
    ...
    def __getitem__(self, key):
        ...
        if not self.egg.has_resource(new):
            raise KeyError('Resource %s in %s#%s does not exist' % (new, self.egg, self.entry_point_name))
        if self.egg.resource_isdir(new):
            return self.__class__(self.factory, self.egg, self.entry_point_name, new)
        fn = self.egg.get_resource_filename(_resource_manager, new)
        return self.factory(fn)

    def __repr__(self):
        return '<%s for %s#%s (%s)>' % (self.__class__.__name__, self.egg, self.entry_point_name, self.resource_name)
