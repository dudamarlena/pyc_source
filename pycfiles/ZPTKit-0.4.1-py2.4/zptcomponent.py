...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/ZPTKit/zptcomponent.py
...
import templatetools, emailtemplate, ExceptionFormatter
from shadowdict import ShadowDict

class ZPTServletComponent(ServletComponent):
    __module__ = __name__
    _servletMethods = [
     'writeTemplate', 'context', 'options', 'template', 'sendMail', 'renderTemplate', 'add_template_path']

    def __init__(self, template_paths, smtp_server='localhost', use_exception_formatter=True, default_encoding='UTF-8'):
        ...
        self.template_pool = self.get_pool(template_paths, self._tmpl_pools, templatetools.FilePageTemplate)
        self.email_pool = self.get_pool(template_paths, self._email_pools, emailtemplate.EmailTemplate)
    ...
    def awakeEvent(self, trans):
        ...
        self._context = {'servlet': self.servlet(), 'request': trans.request().fields()}
        self._options = {}
        self._setupDone = 1
    ...
    def writeTemplate(self):
        ...
        if view == 'writeContent':
            ...
            assert module_path.startswith(context_path), 'Module path (%r) is not under Context path (%r)' % (module_path, context_path)
        ...
        if self._debugging:
            ...
            for (name, value) in items:
                ...
                write('<tr><td style="text-align: right; padding-right: 1em">%s</td><td style="border-top: 1px solid #ddd">%s</td></tr>' % (cgi.escape(name), cgi.escape(value)))
    ...
    def renderTemplate(self, template):
        ...
        try:
            ...
            if 'paste.config' in self.servlet().request().environ():
                raise
            (t, v, tb) = sys.exc_info()
            result = ExceptionFormatter.format_exception(t, v, tb, as_html=True)
            header = result[0]
            result = result[1:]
            return ('\n').join(result)
    ...
    def sendMail(self, filename, to_address, from_address, **options):
