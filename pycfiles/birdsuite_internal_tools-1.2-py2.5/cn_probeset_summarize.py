...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/birdsuite/cn_probeset_summarize.py
# Compiled at: 2009-05-07 14:12:01
...
import optparse, sys, numpy, medpolish
...
MISSING_VALUE_LABEL = float(-9999)
...
class RangeCNP(object):
    ...
    def getOutputFields(self):
        return [
         self.strCNPId, str(self.iChr), str(self.iStartPosn), str(self.iEndPosn)]
...
def loadRangeCNPDefinitions(strCnpDefsFile):
    ...
    if lstFields[0] != 'cnp_id':
        raise Exception('Header missing from CNP definition file ' + strCnpDefsFile + ".  Expected header line 'cnp_id chr start end'.")
    for strLine in fIn:
...
def loadSmartCNPDefinitions(strSmartCnpDefsFile):
    ...
    lstFields = strHeader.split()
    if lstFields[0] != 'cnp_id':
        raise Exception('Header missing from CNP definition file ' + strSmartCnpDefsFile + ".  Expected header line 'cnp_id chr pos'.")
    dctCNPs = {}
    for strLine in fIn:
...
def loadBISmartCNPDefinitions(strCnpDefsFile):
    ...
    lstFields = strHeader.split()
    if lstFields[0] != 'cnp_id':
        raise Exception('Header missing from CNP definition file ' + strCnpDefsFile + ".  Expected header line 'cnp_id probeset_id'.")
    dctCNPs = {}
    for strLine in fIn:
...
class LocusCNVAccumulator(object):
    ...
    def accumulate(self, lstFields):
        while len(self._lstForIteration) > 0 and (self._lstForIteration[0][0].iChr < lstFields[SUMMARY_CHR_FIELD_INDEX] or self._lstForIteration[0][0].iChr == lstFields[SUMMARY_CHR_FIELD_INDEX] and self._lstForIteration[0][0].iEndPosn < lstFields[SUMMARY_POSN_FIELD_INDEX]):
...
lstSTANDARD_HEADERS = [
 'cnp_id', 'chr', 'start_position', 'end_position']
...
def sloshHeader(fOut, fIn, retainAll=True, bNoLocus=False):
    ...
    for strLine in fIn:
        if strLine.startswith('probeset_id'):
            ...
            lstOutHeaders = lstFirstHeaders + lstInHeaders[SUMMARY_FIRST_INTENSITY_FIELD_INDEX:]
...
def extractIntensitiesFromSummaryFields(lstFields):
    """Returns list of floats given string fields of a probe set summary line."""
    return [ float(strIntensity) for strIntensity in lstFields[SUMMARY_FIRST_INTENSITY_FIELD_INDEX:] ]
...
def median_polish(lstRows, bVerbose=False):
    ...
    if bVerbose:
        ...
        print >> sys.stderr, '\noverall', overall, '\nrowEffects', rowEffects, '\ncolumnEffects', columnEffects, '\x08residuals', residuals
...
def sumProbes(lstRows, bVerbose=False):
    ...
    for row in matIntensities:
        idx = fk_utils.indices(row, MISSING_VALUE_LABEL)
        if len(idx) > 0:
            row[idx] = 0
...
dctSUMMARIZATION_ALGORITHMS = {'medpolish': median_polish, 
...
def findMediansOfSelectProbes(lstProbeSummariesFiles, stProbes):
    ...
    for strLine in fIn:
        ...
        lstMatrix.append(extractIntensitiesFromSummaryFields(lstFields))

    if len(stProbes) != len(lstMatrix):
        raise Exception('Not all normalization probes found in summary file ' + str(lstProbeSummariesFiles) + '; ' + str(stProbes - stFoundProbes))
    fIn.close()
...
def main(argv=None):
    ...
    parser.add_option('--smart-cnps', dest='smartCnpDefsFile', help='Tab-separated input file with the following columns:\nCNP ID, chromosome, genomic position.  Multiple lines for the same CNP ID are allowed.\nAt least one of --cnps and --smart-cnps must be used.')
    parser.add_option('--bi-smart-cnps', dest='biSmartCnpDefsFile', help='Build-independent smart probe file.  Tab-separated input file with the following columns:\nCNP ID, probeset_id.  Multiple lines for the same CNP ID are allowed.\nAt least one of --cnps and --smart-cnps must be used.')
    parser.add_option('-o', '--output', dest='output', help='Tab-separated output file in tabular format, with one row for each CNP in cnpRangeFile\nand one column for each sample in the input probeSummaryFile.  The value in each cell of the table\nis the result of median polishing all the intensities for the CNP (i.e. all the probe sets in the CNP,\n                      and all the samples.  Default: stdout.')
    parser.add_option('-a', '--algorithm', dest='summarizationAlgorithm', choices=dctSUMMARIZATION_ALGORITHMS.keys(), default=strDEFAULT_SUMMARIZATION_ALGORITHM, help='Which summarization algorithm to use.  One of %s  Default: %s.' % (
     str(dctSUMMARIZATION_ALGORITHMS.keys()), strDEFAULT_SUMMARIZATION_ALGORITHM))
    parser.add_option('--ignore-snps', dest='ignoreSNPs', default=False, action='store_true', help='Skip SNP intensities and just summarize CNP intensities.')
    parser.add_option('--normalization-probes', dest='normalizationProbes', help='Optional list of probes for normalizing each sample.  For each sample,\n                      find the median of these probes and divide the output intensity by this value.')
    parser.add_option('--no_locus', action='store_true', default=False, help='Do not emit CNP locus information (chromosome, start and stop) in output.')
    parser.add_option('-v', '--verbose', dest='verbose', action='store_true', default=False, help='Enable debugging output')
    parser.add_option('-m', '--missing_value_label', dest='strMissingValueLabel', default='-9999', help='Label of data that is missing from the platform.  \n                      Illumina products do not always have data available for every probe/individual combination.\n                      Default is -9999')
    ...
    if dctOptions.strMissingValueLabel is not None:
        globals()['MISSING_VALUE_LABEL'] = float(dctOptions.strMissingValueLabel)
        print >> sys.stderr, 'Missing Value Label changed to ' + str(MISSING_VALUE_LABEL) + '\n'
    ...
    if dctOptions.normalizationProbes is not None:
        ...
        if len(lstProbes) == 0:
            ...
            lstMedians = findMediansOfSelectProbes(lstProbeSummariesFiles, set(lstProbes))
    else:
        lstMedians = None
    fIn = InputFileMergeWithPushBack(lstProbeSummariesFiles)
    ...
    for (i, strLine) in enumerate(fIn):
        lstFields = strLine.split()
        if dctOptions.ignoreSNPs and lstFields[SUMMARY_PROBESET_TYPE_FIELD_INDEX] in ('A',
                                                                                      'B',
                                                                                      'S'):
            continue
    ...
    for (cnp, lstRows) in lstCNVsAndAccumulatedIntensities:
        ...
        lstRows = [ extractIntensitiesFromSummaryFields(lstFields) for lstFields in lstRows ]
        if dctOptions.biSmartCnpDefsFile is not None:
        ...
        if len(lstRows) == 0:
            ...
            print >> fOut, ('\t').join(lstCNPOutputFields + [ '%.4f' % f for f in lstNormalizedCNPSummary ])
