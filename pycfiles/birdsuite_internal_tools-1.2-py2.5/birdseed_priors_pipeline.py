...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/birdsuitepriors/birdseed/birdseed_priors_pipeline.py
...
@author: nemesh
"""
import optparse, sys, mpgutils.utils as utils, mpgutils.filterMatrix as filterMatrix, mpgutils.fk_utils as fk_utils
...
class EnvironmentSpec(object):
    ...
    def __init__(self, intensityFile, genotypeFile, hapmapGenderFile, specialSNPsFile, tempDir, outputFile):
...
def reformat(envSpec):
    sumz_reformat.writeSumZ(envSpec.intensityFile(), envSpec.filteredGenotypeFile(), envSpec.hapmapGenderFile(), envSpec.sumZFile())
...
def summarizeSNPs(envSpec):
    ...
    dctArguments = {'inputThetasFile': envSpec.sumZFile(), 'specialSNPsFile': envSpec.specialSNPsFile(), 
...
def autoseeds(envSpec):
    ...
    dctArguments = {'sourceFile': envSpec.tempDir() + '/auto_snps.priors', 'outFileDir': envSpec.tempDir()}
    RscriptToPython.callRscript(lstLibraries, methodName, dctArguments, captureOutput=False, bVerbose=True)
...
def mitoseeds(envSpec):
    ...
    dctArguments = {'sourceFile': envSpec.tempDir() + '/mito_snps.priors', 'outFileDir': envSpec.tempDir()}
    RscriptToPython.callRscript(lstLibraries, methodName, dctArguments, captureOutput=False, bVerbose=True)
...
def xchrseeds(envSpec):
    ...
    dctArguments = {'sourceFile': envSpec.tempDir() + '/xchr_snps.priors', 'outFileDir': envSpec.tempDir()}
    RscriptToPython.callRscript(lstLibraries, methodName, dctArguments, captureOutput=False, bVerbose=True)
...
def ychrseeds(envSpec):
    ...
    dctArguments = {'sourceFile': envSpec.tempDir() + '/ychr_snps.priors', 'outFileDir': envSpec.tempDir()}
    RscriptToPython.callRscript(lstLibraries, methodName, dctArguments, captureOutput=False, bVerbose=True)
...
def main(argv=None):
    ...
    parser.add_option('-o', '--output', dest='outputFile', default=None, help='Where to write output.  Default: stdout')
    ...
    parser.add_option('--genotype_file', dest='genotypeFile', default=None, help='Hapmap genotype file.  Each row is a SNP, each column an individual.  \n                      SNPS should match the SNPs in the intensity file.  There must be a SNP here for each SNP\n                      in the intensity file, and the same set of samples. File is tab separated.\n                      \n                      Example:\n                      \n                      probeset_id     NA06985 NA06991\n                      AFFX-SNP_10000979       1       1')
    parser.add_option('--genderFile', dest='hapmapGenderFile', default=None, help='Hapmap gender file.  Contains two columns, the sample ID, and a 1 for males, 2 for females.  The file is tab seperated.\n                      \n                      Example:\n                      \n                      sampleID        gender\n                      \n                      NA12003         1\n                      \n                      NA12004         2\n')
    parser.add_option('--specialSNPsFile', dest='specialSNPsFile', default=None, help="A file that details SNPs that aren't copy number 2 in both males and females.  This covers\n                      the X, Y, PAR, and mitochondria SNPs. File is tab separated.\n                      \n                      Example:\n                      \n                      probeset_id     chr     copy_male       copy_female\n                      SNP_A-8574947   MT      1       1\n                      SNP_A-4261602   PAR     2       2\n                      SNP_A-8524777   X       1       2\n                      SNP_A-8290281    Y    1    0")
    ...
    lstRequiredOptions = [
     'outputFile', 'intensityFile', 'genotypeFile', 'hapmapGenderFile', 'specialSNPsFile', 'tempDir']
    if not utils.validateRequiredOptions(dctOptions, lstRequiredOptions):
        parser.print_help()
        return 1
    envSpec = EnvironmentSpec(dctOptions.intensityFile, dctOptions.genotypeFile, dctOptions.hapmapGenderFile, dctOptions.specialSNPsFile, dctOptions.tempDir, dctOptions.outputFile)
