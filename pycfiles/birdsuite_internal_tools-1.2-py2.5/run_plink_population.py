...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/birdsuite/run_plink_population.py
...
import optparse, re, glob, os.path, make_ped_and_map_file, merge_pedfiles, plink_compile, plink_summarize
...
def main(argv=None):
    ...
    parser.add_option('--map3', dest='map3Path', default='/humgen/cnp04/sandbox/data/TEST_DATA_SET/pre_plink_artifacts/500b.map3', help='tab-delimited map3 file with 3 columns: chromosome, SNP, position.\nDefault:/humgen/cnp04/sandbox/data/TEST_DATA_SET/pre_plink_artifacts/500b.map3')
    parser.add_option('--platemap', dest='plateMapPath', default=None, help='tab-delimited file with 3 columns: container, well, participant ID, used to determine a participant ID for plates.\nEach of these files should look like the plate name with a filetype of .platemap.  EG: hurts.platemap')
    parser.add_option('--pedigree', dest='pedigreePath', default=None, help='tab-delimited file with 5 columns: Family ID, Individual ID, Father ID, Mother ID, Gender (1=M, 2=F)\nEach of these files should look like the platename with a filetype of .pedigree.  EG: hurts.pedigree')
    parser.add_option('--calls', dest='callsSuffix', default=None, help="The suffix of the tab-delimited files with 1 line for each SNP, 1 column for each genotype call, \nplus 1st column is SNP name.  Header line has CEL file name \nfor each column of genotypes.  Genotypes are: 0=AA, 1=AB, 2=BB.\nFor each directory given, a calls file will be searched for that looks like *<suffix>\nEG: birdseed_aspen_q1c86p1_calls_snp_list_sample_snp_filtered.txt \nwould have the suffix 'calls_snp_list_sample_snp_filtered.txt'")
    parser.add_option('--confidences', dest='confsSuffix', default=None, help='The suffix of the tab-delimited file identical to calls file, \nexcept that each value in the matrix is a confidence of the\ncorresponding genotype call in the calls file.\nFor each directory given, a confidence file will be searched for that looks like *<suffix>\nEG: birdseed_aspen_q1c86p1_confs_snp_list_sample_snp_filtered.txt would have the suffix confs_snp_list_sample_snp_filtered.txt')
    parser.add_option('--threshold', dest='threshold', type='float', default='0.1', help='If a genotype has confidence > this value, it is considered a no-call.  Defaults to 0.1')
    parser.add_option('--output-dir', dest='outputdir', default=None, help='A directory where the output of these operations will be written')
    parser.add_option('--which-re', dest='reIndex', type='int', help='Index of regular expression to use in conjunction with platemap.  Default: 0', default=0)
    parser.add_option('--exclude-mendel-error-snp', dest='excludeMESNP', default=1, help='Let plink filter Mendel errors on a SNP by SNP basis.  This excludes all SNPs with an error rate higher than the number given.\n                      This may be combined with the family based Mendel error exclusion parameter.  In that case, both filters will be run concurrently.\n                      This option is set to 1 by default.')
    parser.add_option('--exclude-mendel-error-family', dest='excludeMEF', default=1, help='Let plink filter Mendel errors on a family by family basis.  \nThis excludes all families with an error rate higher than the number given. This may be combined with the \nSNP based Mendel error exclusion parameter.  In that case, both filters will be run concurrently.\nThis option is set to 1 by default.')
    parser.add_option('--skip-summary', dest='skipSummary', default=False, help='If you do not wish to generate summary data, set this to true')
    parser.add_option('--outputFileNameRoot', dest='rootName', default='plink', help='What is the root name for the files comming out of plink?  \nThis is equivilent to the --out option in plink.')
