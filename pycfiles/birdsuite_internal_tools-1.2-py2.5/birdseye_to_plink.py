...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/birdsuite/birdseye_to_plink.py
...
def main():
    ...
    parser.add_option('-o', '--outputname', dest='outputname', help='define the file name root for the output files')
...
def ReadPlateFile(BirdsEyeCallFile, cnvFile, options):
    ...
    for strLine in BirdsEyeCallFile:
        ...
        if dctFamMap.has_key(ID):
            FID = dctFamMap[ID]
        elif not dctFamMissing.has_key(ID):
