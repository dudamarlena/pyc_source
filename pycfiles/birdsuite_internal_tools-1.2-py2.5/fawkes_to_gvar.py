...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/plink_pipeline/fawkes_to_gvar.py
# Compiled at: 2009-05-22 09:59:53
        ...
        for calls_line in CallsFile:
            ...
            for index in range(1, len(calls_fields)):
                ...
                if dctFamMap.has_key(CelList[index]):
                    ...
                    if iA == -1:
                        if iB != -1:
                            print 'Strange call (' + strCall + ') for SNP ' + strSNP
                            sys.exit(2)
                    if iA + iB != 2:
            ...
            if skipped_indiv > 0 and total_indiv > 0:
                if skipped_indiv / total_indiv > 0.8:
                    print 'Most of the individuals are not present in pedigree file (> 80%), You may need to provide a celMap'
                    print 'Please keep a record that :' + CallsPaths[file_number] + ' did not pass filtering'
