...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/birdsuite/post_birdseye.py
# Compiled at: 2009-04-09 16:13:12
...
import bisect, copy, fileinput, math, optparse, os, sys, cn_create_exclude_list, cnv_definition_collection, pre_birdseye
    ...
    def __init__(self, strCNVDefinitionsFile, strCallsFile, strConfsFile, fConfidenceThreshold, fOutputScore):
        ...
        for (iSampleNum, lstCNVTuples) in enumerate(lstCNVsBySample):
            ...
            for tupCNV in lstCNVTuples:
                ...
                tupToInsert = [
                 cnvDef.iStartPosn, cnvDef.iEndPosn, fConf, iCall]
...
def main(argv=None):
    ...
    parser.add_option('--input_dir', default='.', help='Parent directory of the birdseye subdirectories.  Default: current directory')
    ...
    parser.add_option('--birdseye_canary_overlap_threshold', type='int', default='2', help='If a birdseye call completely encloses a canary call, has the same copy number\nas the canary call, and is not longer (in number of CN and SNP probes spanned) by at least this amount,\nthen discard the birdseye call.  Default: %default')
            ...
            if lstCanary[iCOPY_NUMBER_INDEX] == lstBirdseye[iCOPY_NUMBER_INDEX]:
                if lstBirdseye[iSTART_INDEX] <= lstCanary[iSTART_INDEX]:
                    if lstBirdseye[iEND_INDEX] >= lstCanary[iEND_INDEX]:
                        pass
                    elif probeCounter.countProbesInRange(lstBirdseye[iCHROMOSOME_INDEX], lstBirdseye[iSTART_INDEX], lstBirdseye[iEND_INDEX]) - probeCounter.countProbesInRange(lstCanary[iCHROMOSOME_INDEX], lstCanary[iSTART_INDEX], lstCanary[iEND_INDEX]) < dctOptions.birdseye_canary_overlap_threshold:
                        birdseyeIterator.advance()
                    canaryIterator.advance()
                    continue
