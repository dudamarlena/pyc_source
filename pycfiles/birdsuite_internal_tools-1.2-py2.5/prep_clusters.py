...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/canarypriors/prep_clusters.py
# Compiled at: 2009-05-28 10:36:10
from __future__ import division
import sys, mpgutils.fk_utils as fk, mpgutils.utils as utils, optparse
min_variance = 0.0001
...
def processCNVs(intensities_header, genotypes_header, dctIntensities, dctGenotypes, outFile):
    ...
    for cnv in cnvNames:
        ...
        for genotype in unique_genotypes:
            ...
            relevant_intensities = fk.arbslice(dctIntensities[cnv], relevant_intensities_indices)
        ...
        for genotype in unique_genotypes:
            out.write(' ' + float_format(len(cluster_intensities[genotype]) / total_genotypes))
...
def main(argv=None):
    ...
    parser.add_option('-i', '--intensities', default=None, dest='intensities', help='intensities file, with the same samples/regions as the genotypes file.')
    parser.add_option('-g', '--genotypes', default=None, dest='genotypes', help='genotypes file, with the same samples/regions as the intensities file.')
    parser.add_option('-o', '--output', default='.', dest='output', help='Where to write the cluster file.')
