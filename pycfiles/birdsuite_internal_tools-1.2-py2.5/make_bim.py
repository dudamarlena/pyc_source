...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/birdsuite/make_bim.py
...
def main(argv=None):
    ...
    parser.add_option('-a', '--annotations', help='(Required) Affymetrix annotation file.')
    parser.add_option('-c', '--calls', help='(Required) Input calls file in traditional apt-probeset-genotype format or Larry Bird format.\nThis file is used only to get the order of SNPs.')
    parser.add_option('-o', '--output', help='(Required) Output calls file in tped (transposed ped) format')
    parser.add_option('--affy_names', default=False, action='store_true', help='Output Affymetrix SNP names instead of using RS numbers when available.  Default: use RS number when available.')
    parser.add_option('--affy_strand', default=False, action='store_true', help='Write alleles as they appear in annotation file, rather than converting to positive strand.  Default: convert alleles to positive strand')
    parser.add_option('-v', '--verbose', default=False, action='store_true', help='Write informational message to stderr, e.g. that locus or allele information could not be found.')
    parser.add_option('--include_affx_snps', action='store_true', default=False, help='Include AFFX-SNP SNPs in the output.  Default: exclude these SNPs.')
