# uncompyle6 version 3.7.4
# Python bytecode 2.5 (62131)
# Decompiled from: Python 3.6.9 (default, Apr 18 2020, 01:56:04) 
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/plink_pipeline/birdsuite_to_plink.py
# Compiled at: 2009-02-18 20:33:47
from plink_pipeline import create_map, birdseye_to_cnv, canary_to_gvar, fawkes_to_gvar, fawkes_to_diploid, fawkes_merge, make_tped, make_tfam, fam_to_gender
import optparse, sys, os, getopt, re
from mpgutils import utils

def main(argv=None):
    if not argv:
        argv = sys.argv
    lstRequiredOptions = ['metadir', 'plateroot', 'familyfile']
    parser = optparse.OptionParser(usage=__doc__)
    parser.add_option('-p', '--plateroot', help='(Required) Contains folders with larry-bird style files')
    parser.add_option('-f', '--familyfile', help='(Required) PLINK formatted .fam file')
    parser.add_option('-x', '--metadir', help='(Required) Meta directory holding CNV data')
    parser.add_option('-d', '--outputdir', help='Define the directory for output files')
    parser.add_option('-m', '--celmap', dest='celmap', help='Cel Map file')
    parser.add_option('-t', '--threshold', default=0.1, help='define a cutoff for confidence data')
    parser.add_option('-n', '--outputname', default='Output', help='Define the file name root for the output files')
    parser.add_option('-c', '--chip', default='6', help="Chip Type, Use either '5' or '6'")
    parser.add_option('-b', '--build', default='18', help="Human Genome Build, Use either '17' or '18'")
    parser.add_option('-g', '--buildgender', default=False, action='store_true', help='Build Gender Files')
    parser.add_option('-a', '--noallelemap', default=False, action='store_true', help='Do not convert Probes to Alleles')
    parser.add_option('-v', '--verifyfam', default=False, action='store_true', help='Check to make sure the Fam file is not missing phenotypes')
    parser.add_option('-r', '--norscoding', default=False, action='store_true', help='Do not convert SNP id to RS id')
    parser.add_option('-1', '--firstStep', default=False, action='store_true', help='Complete the first step, please define each step that needs to be completed')
    parser.add_option('-2', '--secondStep', default=False, action='store_true', help='Complete the second step, please define each step that needs to be completed')
    parser.add_option('-3', '--thirdStep', default=False, action='store_true', help='Complete the third step, please define each step that needs to be completed')
    parser.add_option('-4', '--fourthStep', default=False, action='store_true', help='Complete the fourth step, please define each step that needs to be completed')
    (dctOptions, lstArgs) = parser.parse_args(argv)
    if not utils.validateRequiredOptions(dctOptions, lstRequiredOptions):
        parser.print_help()
        return 1
    lstValidChips = [
     '5', '6']
    if dctOptions.chip not in lstValidChips:
        print 'You entered a wrong chip type'
        sys.exit(1)
    lstValidBuilds = [
     '17', '18']
    if dctOptions.chip not in lstValidChips:
        print 'You entered a wrong chip type'
        sys.exit(1)
    if not dctOptions.outputdir:
        setattr(dctOptions, 'outputdir', sys.path[0])
    if not (dctOptions.firstStep or dctOptions.secondStep or dctOptions.thirdStep or dctOptions.fourthStep):
        setattr(dctOptions, 'firstStep', True)
        setattr(dctOptions, 'secondStep', True)
        setattr(dctOptions, 'thirdStep', True)
        setattr(dctOptions, 'fourthStep', True)
    lstOptionsToCheck = [
     'familyfile', 'plateroot', 'metadir', 'outputdir']
    if dctOptions.celmap:
        lstOptionsToCheck.append('celmap')
    utils.validatePathArgs(dctOptions, lstOptionsToCheck, True)
    print '\n    =========================================================\n    =             BIRDSUITE -> PLINK PIPELINE               =\n    ========================================================='
    if dctOptions.verifyfam:
        if not utils.checkFamFile(dctOptions.familyfile, 0.6):
            print 'Family file does not meet the required threshold, please ensure it is not missing Phenotypes'
            sys.exit(1)
    if dctOptions.buildgender:
        possible_args = [
         'plateroot', 'outputdir', 'familyfile', 'outputname', 'celmap']
        current_args = MakeArguments(possible_args, dctOptions)
        print '\n-----FAM->GENDER-----'
        fam_to_gender.main(current_args)
    if dctOptions.firstStep:
        print '\nRunning First Step'
        possible_args = [
         'metadir', 'outputdir', 'outputname', 'chip', 'build']
        current_args = MakeArguments(possible_args, dctOptions)
        print '\n-----CREATE-MAP-----'
        create_map.main(current_args)
        possible_args = [
         'plateroot', 'outputdir', 'familyfile', 'celmap', 'outputname']
        current_args = MakeArguments(possible_args, dctOptions)
        print '\n-----BIRDSEYE->CNV-----'
        birdseye_to_cnv.main(current_args)
    if dctOptions.secondStep:
        print '\nRunning Second Step'
        possible_args = [
         'plateroot', 'outputdir', 'familyfile', 'threshold', 'celmap', 'outputname']
        current_args = MakeArguments(possible_args, dctOptions)
        print '\n-----CANARY->GVAR-----'
        canary_to_gvar.main(current_args)
        possible_args = [
         'plateroot', 'outputdir', 'familyfile', 'threshold', 'celmap', 'outputname', 'metadir', 'noallelemap', 'norscoding']
        current_args = MakeArguments(possible_args, dctOptions)
        print '\n-----FAWKES->GVAR-----'
        fawkes_to_gvar.main(current_args)
    if dctOptions.thirdStep:
        print '\nRunning Third Step'
        possible_args = [
         'plateroot', 'metadir', 'chip']
        current_args = MakeArguments(possible_args, dctOptions)
        print '\n-----FAWKES->DIPLOID-----'
        fawkes_to_diploid.main(current_args)
        possible_args = [
         'plateroot', 'outputname']
        current_args = MakeArguments(possible_args, dctOptions)
        print '\n-----FAWKES-MERGE-----'
        fawkes_merge.main(current_args)
    if dctOptions.fourthStep:
        print '\nRunning Fourth Step'
        possible_args = [
         'plateroot', 'metadir', 'chip', 'build', 'outputdir', 'outputname', 'threshold']
        current_args = MakeArguments(possible_args, dctOptions)
        print '\n-----MAKE-TPED-----'
        make_tped.main(current_args)
        possible_args = [
         'plateroot', 'familyfile', 'outputdir', 'outputname', 'celmap']
        current_args = MakeArguments(possible_args, dctOptions)
        print '\n-----MAKE-TFAM-----'
        make_tfam.main(current_args)


def MakeArguments(possible_args, dctOptions):
    current_args = []
    for key in possible_args:
        item = getattr(dctOptions, key)
        if not item == None:
            if item == True:
                current_args.append('--' + key)
            elif item == False:
                continue
            else:
                current_args.append('--' + key)
                current_args.append(item)

    return current_args


if __name__ == '__main__':
    main()