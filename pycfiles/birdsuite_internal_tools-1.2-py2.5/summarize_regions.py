...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/hapmap3/summarize_regions.py
# Compiled at: 2009-03-03 11:24:34
...
def main(argv=None):
    ...
    parser.add_option('-o', '--output', dest='output', help='(Required) Where to write output file.')
    parser.add_option('--algorithm', dest='algorithm', default='medpolish-exp', help='Which algorithm to summarize with. Default: medpolish-exp')
    parser.add_option('--plateFile', dest='plateFile', help='(Required) PlateFile is a list of plates to process, has no header and 1 column.')
    parser.add_option('--dataDir', dest='dataDir', help='(Required) The directory where the exchange format files are located. \n                      Example file: hapmap3_intensities_1M_p47576_chr10.dat')
    parser.add_option('--regions', dest='regions', help='(Required) A file describing the regions to summarize.  \n                      This has 4 columns (with header)\n                      cnp_id, chr, start, end.\n                      It follows the CNV map file format described by cn_probeset_summarize')
    parser.add_option('--normalizationProbes', dest='normalizationProbes', default=None, help='The normalization probes file.  See: cn_probeset_summarize.')
    parser.add_option('--tempDir', dest='tempDir', help='(Required) Where to put intermediate temp files.')
    parser.add_option('--startAtSummarization', dest='sumFlag', action='store_true', default=False, help="Should the process skip annotation and locus summarization, and start with CNV probeset summarization?\n                      You can do this if you've run the entire pipeline on a data set once, and wish to process with a different map/algorithm.")
    parser.add_option('-m', '--missing_value_label', dest='missingValueLabel', default='-1', help='Encode this value in the canonical missing value label (-1)')
    ...
    parser.add_option('-v', '--verbose', dest='bVerbose', default=False, action='store_true', help='Run in verbose mode')
    lstRequiredOptions = [
     'plateFile', 'output', 'dataDir', 'regions', 'tempDir']
    ...
    if dctOptions.sumFlag == False:
        toAnnotatedSummary(dctOptions.dataDir, dctOptions.tempDir, lstPlates, dctOptions.missingValueLabel, dctOptions.intensityMultiplier, bVerbose=bVerbose)
