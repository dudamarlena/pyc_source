...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/birdsuite/plink_summarize_by_population.py
...
def main(argv=None):
    ...
    parser.add_option('--populationFile', help='tab-delimited file with 2 columns: popultion and sample')
    parser.add_option('--pedigreeFile', help='The pedigree file for the populations')
    parser.add_option('--workingDirectory', help='The directory where the input bed/bim/fam and output files will go.')
    parser.add_option('--rootBedBimFamName', default='plink', help='The name of the bed/bim/fam files, eg: "plink" for plink.bed')
    parser.add_option('--plink-path', dest='strPlinkPath', default=strDEFAULT_PLINK_PATH, help='plink program.  Default: find plink on path.  \n                      Hint: put "use plink" in your startup script.')
    (dctOptions, lstArgs) = parser.parse_args(argv)
    argCount = len(lstArgs) - 1
    lstRequiredOptions = [
     'populationFile', 'pedigreeFile', 'workingDirectory', 'rootBedBimFamName']
