...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/birdsuitepriors/canary/canary_priors_pipeline.py
# Compiled at: 2009-10-27 21:09:56
"""
Created on Aug 27, 2009

@author: nemesh
"""
import optparse, sys, mpgutils.utils as utils, birdsuite.cn_annotate_allele_summary, birdsuite.cn_locus_summarize, birdsuite.cn_probeset_summarize, birdsuitepriors.canary.prep_clusters
...
def locus_summarize(envSpec):
    ...
    if envSpec.force_cn_summarization():
        t = [
         '--force_cn_summarization', envSpec.force_cn_summarization()]
        argv.extend(t)
...
def fill_clusters(envSpec):
    ...
    dctArguments = {'sourceFile': envSpec.prepClusterOut(), 'outputFile': envSpec.outputFile()}
    RscriptToPython.callRscript(lstLibraries, methodName, dctArguments, captureOutput=False, bVerbose=True)
...
def main(argv=None):
    ...
    lstRequiredOptions = [
     'intensityFile', 'genotypeFile', 'probe_locus', 'tempDir', 'basename', 'outputFile', 'cnpDefsFile']
    (dctOptions, lstArgs) = parser.parse_args(argv)
    if not utils.validateRequiredOptions(dctOptions, lstRequiredOptions):
        parser.print_help()
        return 1
    envSpec = EnvironmentSpec(dctOptions.intensityFile, dctOptions.genotypeFile, dctOptions.probe_locus, dctOptions.tempDir, dctOptions.basename, dctOptions.outputFile, dctOptions.force_cn_summarization, dctOptions.cnpDefsFile, dctOptions.strMissingValueLabel)
