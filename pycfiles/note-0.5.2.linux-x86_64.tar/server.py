...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/note/server.py
# Compiled at: 2015-01-25 19:44:51
import zmq, sys, json
...
class Note_Server(object):
    ...
    def Handle_Search(self, msg):
        ...
        reply = {'status': 'OK', 'type': 'search', 
           'object': {'received search': msg['object']['searchTerm'], 
                      'results': results}}
    ...
    def Handle_Note(self, msg):
        ...
        if 'ID' in msg['object']:
            ...
            self.db.addItem('note', {'note': note_text, 'tags': note_tags}, note_id)
        else:
            note_id = self.db.addItem('note', {'note': note_text, 'tags': note_tags})
        reply = {'status': 'OK', 'type': 'Note', 
           'object': {'received note': msg['object']['note'], 
    ...
    def Get_By_Label(self, msg):
        ...
        if item is None:
            reply = {'status': 'ERROR', 'type': 'Get', 'object': {'msg': 'Item does not exist', 'ID': ID}}
        else:
            reply = {'status': 'OK', 'type': 'Get', 
               'object': item}
        return reply
    ...
    def Handle_Delete(self, msg):
        ...
        ID = msg['object']['id']
        try:
            reply = {'status': 'OK', 'type': 'Delete', 'object': self.db.deleteItem(ID)}
        except ValueError:
            e_msg = ('Object with ID {0} does not exist').format(ID)
            reply = {'status': 'ERROR', 'type': 'Delete', 
               'object': {'msg': e_msg}}

        return json.dumps(reply)

    def Handle_ERROR(self, msg):
        reply = {'status': 'ERROR', 'object': {'msg': 'unknown command'}, 'type': 'ERROR MSG'}
    ...
    def Handle_Place(self, msg):
        ...
        if 'ID' in msg['object']:
            note_id = msg['object']['ID']
            self.db.addItem('place', {'note': note, 'address': address, 
               'place': place, 
               'tags': tags}, note_id)
        else:
            note_id = self.db.addItem('place', {'note': note, 'address': address, 
               'place': place, 
               'tags': tags})
        reply = {'status': 'OK', 'type': 'Place', 
           'object': {'received note': msg['object']['note'], 
                      'received tags': msg['object']['tags'], 
                      'ID': note_id}}
        return json.dumps(reply)
    ...
    def Handle_Todo(self, msg):
        ...
        if 'ID' in msg['object']:
            note_id = msg['object']['ID']
            self.db.addItem('todo', {'todo': todo, 'done': done, 
               'date': date, 
               'tags': tags}, note_id)
        else:
            note_id = self.db.addItem('todo', {'todo': todo, 'done': done, 
               'date': date, 
               'tags': tags})
        reply = {'status': 'OK', 'type': 'Todo', 
    ...
    def Handle_Label(self, msg):
        ...
        if 'name' not in obj or 'id' not in obj:
            r_msg = {'status': 'ERROR', 'type': 'Label', 'object': {'msg': 'improper request'}}
            ...
            if r_val is None:
                r_msg = {'status': 'ERROR', 'type': 'Label', 'object': {'msg': 'label already exists'}}
            else:
                r_msg = {'status': 'OK', 'type': 'Label', 'object': r_val}
            return json.dumps(r_msg)
    ...
    def Handle_Delete_Label(self, msg):
        ...
        try:
            ...
            r_msg = {'status': 'ERROR', 'type': 'Delete_Label', 
               'object': {'msg': 'improper request'}}
            return json.dumps(r_msg)

        r_val = {'status': 'OK', 'type': 'Delete', 
