...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/note/web.py
# Compiled at: 2015-01-06 22:49:29
__author__ = 'Devin Kelly'
__todo__ = '\n'
import time, json, os, sys, markdown
...
def NewNote():
    ...
    if request.method == 'GET':
        ...
        s = render_template('noteAdded.html', p=pages, note=noteText, tags=tags)
    elif request.method == 'POST' and request.form['api'] == 'true':
        noteText = request.form['note']
...
def NewContact():
    ...
    if request.method == 'GET':
        ...
        contactInfo = {'NAME': name, 'WORK PHONE': work, 
        ...
        tags = request.form['tags'].split(',')
        contact = {'contactText': contactText, 'tags': tags, 
           'timestamp': time.time()}
...
def NewTodo():
    ...
    if request.method == 'GET':
        ...
        todoItem = {'todoText': todoText, 'done': done, 
        ...
        db.addItem('todos', {'todoText': todoText, 'done': done, 
           'date': dateStr})
        s = json.dumps(todo)
    return s