...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/note/mongo_driver.py
# Compiled at: 2015-01-24 20:28:03
from db_api import dbBaseClass
from util import scrubID
import pymongo, time, os, sys, subprocess as SP
...
class mongoDB(dbBaseClass):
    ...
    def getNewID(self):
        ...
        if not unusedIDs:
            ...
            ID = min(unusedIDs)
            unusedIDs.remove(ID)
            idCollection.update({'unusedIDs': {'$exists': True}}, {'$set': {'unusedIDs': unusedIDs}})
        return int(ID)
    ...
    def getDone(self, done):
        """
            ...
            :rval: list
        """
        doneItems = self.noteDB['todo'].find({'done': done}).sort('date', pymongo.DESCENDING)
    ...
    def makeBackupFile(self, dstPath, fileName):
        ...
        with open(os.devnull) as (devnull):
            ...
            SP.call(['zip',
             ...
             os.path.join(dstPath, self.dbName)], stdout=devnull, stderr=devnull)
    ...
    def get_data_collections(self):
        ...
        collections.remove('IDs')
        collections.remove('label')
        return collections