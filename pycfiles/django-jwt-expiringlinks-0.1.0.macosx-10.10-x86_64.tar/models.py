...
# [GCC 8.4.0]
# Embedded file name: /Users/hugo/.virtualenvs/django-jwt/lib/python2.7/site-packages/django_jwt/models.py
# Compiled at: 2015-12-28 14:25:41
"""django_jwt models."""
...
import jwt
from jwt.exceptions import InvalidAudienceError, ExpiredSignatureError, ImmatureSignatureError, MissingRequiredClaimError
from django_jwt.exceptions import MaxUseError, TargetUrlError
REGISTERED_CLAIMS = ('iss', 'aud', 'exp', 'nbf', 'iat', 'jti')
DEFAULT_DECODE_OPTIONS = {'verify_signature': True, 
   'verify_exp': False, 
   'verify_nbf': False, 
   'verify_iat': False, 
   'verify_aud': False, 
...
class RequestToken(models.Model):
    ...
    """
    user = models.ForeignKey(User, blank=True, null=True, help_text='Intended recipient of the JWT.')
    target_url = models.CharField(max_length=200, help_text='The target endpoint.')
    expiration_time = models.DateTimeField(blank=True, null=True, help_text='Token will expire at this time (raises ExpiredSignatureError).')
    not_before_time = models.DateTimeField(blank=True, null=True, help_text='Token cannot be used before this time (raises ImmatureSignatureError).')
    data = models.TextField(max_length=1000, help_text='Custom data (JSON) added to the default payload.', blank=True, default='{}')
    issued_at = models.DateTimeField(blank=True, null=True, help_text='Time the token was created (set in the initial save).')
    max_uses = models.IntegerField(default=1, help_text='The maximum number of times the token can be used.')
    used_to_date = models.IntegerField(default=0, help_text='Number times the token has been used to date (raises MaxUseError).')
    ...
    def encode(self):
        ...
        """
        assert self.id is not None, 'RequestToken missing `id` - ensure that the token is saved before calling the `encode` method.'
    ...
    def _validate_request_user(self, request):
        ...
        """
        assert hasattr(request, 'user'), 'Request is missing user property. Please ensure that the Django authentication middleware is installed.'
        if self.user is None:
            return
        else:
    ...
    def log(self, request, response):
        ...
        rtu = RequestTokenLog(token=self, user=user, user_agent=meta.get('HTTP_USER_AGENT', 'unknown'), client_ip=client_ip, status_code=response.status_code).save()
...
class RequestTokenLog(models.Model):
    ...
    token = models.ForeignKey(RequestToken, help_text='The RequestToken that was used.', db_index=True)
    user = models.ForeignKey(User, blank=True, null=True, help_text='The user who made the request (None if anonymous).')
    user_agent = models.TextField(blank=True, help_text='User-agent of client used to make the request.')
    client_ip = models.CharField(max_length=15, help_text='Client IP of device used to make the request.')
    status_code = models.IntegerField(blank=True, null=True, help_text='Response status code associated with this use of the token.')
