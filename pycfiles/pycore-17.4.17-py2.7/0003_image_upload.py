...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/tests/0003_image_upload.py
...
def test_upload():
    ...
    image = api.image_create('upload image', 'image description', 10, 'transient', disk_controllers[0], 'private', format='qcow2')
...
def test_wait_created():
    for i in xrange(240):
        ...
        if 'progress' in img.data and img.data['progress'] == 1.0:
            return
        if img.state == 'failed':
