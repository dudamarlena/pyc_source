...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/estimationtools/workloadchart.py
# Compiled at: 2011-09-05 07:25:07
from datetime import timedelta
from estimationtools.utils import parse_options, execute_query, get_estimation_suffix, get_closed_states, get_serverside_charts, EstimationToolsBase
...
class WorkloadChart(EstimationToolsBase, WikiMacroBase):
    ...
    def expand_macro(self, formatter, name, content):
        ...
        chart_args = unicode_urlencode({'chs': '%sx%s' % (options['width'], options['height']), 'chf': 'bg,s,00000000', 
        ...
        if self.serverside_charts:
            return tag.image(src='%s?data=%s' % (req.href.estimationtools('chart'),
             unicode_quote(chart_args)), alt='Workload Chart (server)')
        else:
            return tag.image(src='http://chart.apis.google.com/chart?%s' % chart_args, alt='Workload Chart (client)')
            return