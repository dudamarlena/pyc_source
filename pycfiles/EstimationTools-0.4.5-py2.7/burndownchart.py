...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/estimationtools/burndownchart.py
# Compiled at: 2011-09-05 07:25:07
from decimal import Decimal, InvalidOperation, ROUND_HALF_UP
from datetime import timedelta
from genshi.builder import tag
from estimationtools.utils import parse_options, execute_query, get_estimation_field, get_closed_states, from_timestamp, get_serverside_charts, EstimationToolsBase
from trac.core import TracError
from trac.util.html import Markup
from trac.util.text import unicode_quote, unicode_urlencode
from trac.wiki.macros import WikiMacroBase
import copy
DEFAULT_OPTIONS = {'width': '800', 'height': '200', 'color': 'ff9900', 'expected': '0', 'bgcolor': 'ffffff00', 
   'wecolor': 'ccccccaa', 'colorexpected': 'ffddaa', 'weekends': 'true', 'gridlines': '0'}
...
class BurndownChart(EstimationToolsBase, WikiMacroBase):
    ...
    def expand_macro(self, formatter, name, content):
        ...
        bottomaxis = '0:|' + ('|').join([ str(date.day) for date in dates ]) + '|1:|%s/%s|%s/%s' % (dates[0].month, dates[0].year, dates[(-1)].month, dates[(-1)].year)
        ...
        chart_args = unicode_urlencode({'chs': '%sx%s' % (options['width'], options['height']), 'chf': 'c,s,%s|bg,s,00000000' % options['bgcolor'], 
        ...
        if self.serverside_charts:
            return tag.image(src='%s?data=%s' % (req.href.estimationtools('chart'),
             unicode_quote(chart_args)), alt='Burndown Chart (server)')
        else:
            return tag.image(src='http://chart.apis.google.com/chart?%s' % chart_args, alt='Burndown Chart (client)')
            return
    ...
    def _calculate_timetable(self, options, query_args, req):
        ...
        for t in tickets:
            ...
            history_cursor.execute("SELECT DISTINCT c.field as field, c.time AS time, c.oldvalue as oldvalue, c.newvalue as newvalue FROM ticket t, ticket_change c WHERE t.id = %s and c.ticket = t.id and (c.field=%s or c.field='status')ORDER BY c.time ASC", [
    ...
    def _scale_data(self, timetable, options):
        ...
        ydata = [ str(self._round(timetable[d] * Decimal(100) / maxhours)) for d in dates ]
        xdata = [ str(self._round(x * Decimal(100) / (len(dates) - 1))) for x in range(len(dates))
                ]
