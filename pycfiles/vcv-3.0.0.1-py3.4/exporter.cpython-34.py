...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/object_detection/exporter.py
# Compiled at: 2018-06-15 01:39:54
# Size of source mod 2**32: 15091 bytes
"""Functions to export object detection inference graph."""
import logging, os, tensorflow as tf
...
def freeze_graph_with_def_protos(input_graph_def, input_saver_def, input_checkpoint, output_node_names, restore_op_name, filename_tensor_name, clear_devices, initializer_nodes, optimize_graph=False, variable_names_blacklist=''):
    ...
    with tf.Graph().as_default():
        ...
        with session.Session(config=config) as (sess):
            ...
            output_graph_def = graph_util.convert_variables_to_constants(sess, input_graph_def, output_node_names.split(','), variable_names_blacklist=variable_names_blacklist)
...
def _tf_example_input_placeholder():
  ...
  """
    ...
    return (batch_tf_example_placeholder,
     tf.map_fn(decode, elems=batch_tf_example_placeholder, dtype=tf.uint8, parallel_iterations=32, back_prop=False))
...
def _encoded_image_string_tensor_input_placeholder():
  ...
  Returns:
    a tuple of placeholder and input nodes that output decoded images.
  """
    batch_image_str_placeholder = tf.placeholder(dtype=tf.string, shape=[
     None], name='encoded_image_string_tensor')
    ...
    return (batch_image_str_placeholder,
     tf.map_fn(decode, elems=batch_image_str_placeholder, dtype=tf.uint8, parallel_iterations=32, back_prop=False))
...
def export_inference_graph(input_type, pipeline_config, trained_checkpoint_prefix, output_directory, optimize_graph=False, output_collection_name='inference_op'):
  ...
  """
    ...
    _export_inference_graph(input_type, detection_model, pipeline_config.eval_config.use_moving_averages, trained_checkpoint_prefix, output_directory, optimize_graph, output_collection_name)