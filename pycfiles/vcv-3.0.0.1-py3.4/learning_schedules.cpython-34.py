...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/utils/learning_schedules.py
# Compiled at: 2018-06-15 01:29:01
# Size of source mod 2**32: 4588 bytes
...
def exponential_decay_with_burnin(global_step, learning_rate_base, learning_rate_decay_steps, learning_rate_decay_factor, burnin_learning_rate=0.0, burnin_steps=0):
  ...
  """
    ...
    post_burnin_learning_rate = tf.train.exponential_decay(learning_rate_base, global_step, learning_rate_decay_steps, learning_rate_decay_factor, staircase=True)
    return tf.cond(tf.less(global_step, burnin_steps), lambda : tf.convert_to_tensor(burnin_learning_rate), lambda : post_burnin_learning_rate)
...
def manual_stepping(global_step, boundaries, rates):
  ...
  """
    ...
    unreached_boundaries = tf.reshape(tf.where(tf.greater(step_boundaries, global_step)), [-1])
