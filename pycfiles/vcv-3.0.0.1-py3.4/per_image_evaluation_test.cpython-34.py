...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/utils/per_image_evaluation_test.py
# Compiled at: 2018-06-15 01:29:01
# Size of source mod 2**32: 10549 bytes
...
class SingleClassTpFpNoDifficultBoxesTest(tf.test.TestCase):
    ...
    def setUp(self):
        ...
        nms_iou_threshold = 1.0
        nms_max_output_boxes = 10000
        self.eval1 = per_image_evaluation.PerImageEvaluation(num_groundtruth_classes, matching_iou_threshold1, nms_iou_threshold, nms_max_output_boxes)
        self.eval2 = per_image_evaluation.PerImageEvaluation(num_groundtruth_classes, matching_iou_threshold2, nms_iou_threshold, nms_max_output_boxes)
        self.detected_boxes = np.array([[0, 0, 1, 1], [0, 0, 2, 2], [0, 0, 3, 3]], dtype=float)
        self.detected_scores = np.array([0.6, 0.8, 0.5], dtype=float)
    ...
    def test_no_true_positives(self):
        ...
        scores, tp_fp_labels = self.eval1._compute_tp_fp_for_single_class(self.detected_boxes, self.detected_scores, groundtruth_boxes, groundtruth_groundtruth_is_difficult_list)
        expected_scores = np.array([0.8, 0.6, 0.5], dtype=float)
    ...
    def test_one_true_positives_with_very_small_iou_threshold(self):
        groundtruth_boxes = np.array([[0, 0, 1, 1]], dtype=float)
        groundtruth_groundtruth_is_difficult_list = np.zeros(1, dtype=bool)
        scores, tp_fp_labels = self.eval2._compute_tp_fp_for_single_class(self.detected_boxes, self.detected_scores, groundtruth_boxes, groundtruth_groundtruth_is_difficult_list)
        expected_scores = np.array([0.8, 0.6, 0.5], dtype=float)
...
class MultiClassesTpFpTest(tf.test.TestCase):
    ...
    def test_tp_fp(self):
        ...
        detected_boxes = np.array([[0, 0, 1, 1], [10, 10, 5, 5], [0, 0, 2, 2],
         [
          5, 10, 10, 5], [10, 5, 5, 10], [0, 0, 3, 3]], dtype=float)
        ...
        scores, tp_fp_labels, _ = eval1.compute_object_detection_metrics(detected_boxes, detected_scores, detected_class_labels, groundtruth_boxes, groundtruth_class_labels, groundtruth_groundtruth_is_difficult_list)
