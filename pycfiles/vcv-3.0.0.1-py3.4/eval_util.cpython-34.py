...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/object_detection/eval_util.py
# Compiled at: 2018-06-15 01:39:54
# Size of source mod 2**32: 24890 bytes
"""Common functions for repeatedly evaluating a checkpoint.
"""
import copy, logging, os, time, numpy as np, tensorflow as tf
...
def evaluate_detection_results_pascal_voc(result_lists, categories, label_id_offset=0, iou_thres=0.5, corloc_summary=False):
  ...
  """
    ...
    evaluator = object_detection_evaluation.ObjectDetectionEvaluation(num_classes, matching_iou_threshold=iou_thres)
    difficult_lists = None
    if 'difficult' in result_lists:
        if result_lists['difficult']:
            difficult_lists = result_lists['difficult']
    for idx, image_id in enumerate(image_ids):
        difficult = None
        if difficult_lists is not None:
            if difficult_lists[idx].size:
                difficult = difficult_lists[idx].astype(np.bool)
        evaluator.add_single_ground_truth_image_info(image_id, result_lists['groundtruth_boxes'][idx], result_lists['groundtruth_classes'][idx] - label_id_offset, difficult)
        evaluator.add_single_detected_image_info(image_id, result_lists['detection_boxes'][idx], result_lists['detection_scores'][idx], result_lists['detection_classes'][idx] - label_id_offset)

    per_class_ap, mean_ap, _, _, per_class_corloc, mean_corloc = evaluator.evaluate()
    metrics = {'Precision/mAP@{}IOU'.format(iou_thres): mean_ap}
    category_index = label_map_util.create_category_index(categories)
    for idx in range(per_class_ap.size):
        if idx in category_index:
            display_name = 'PerformanceByCategory/mAP@{}IOU/{}'.format(iou_thres, category_index[idx]['name'])
    ...
    if corloc_summary:
        ...
        for idx in range(per_class_corloc.size):
            if idx in category_index:
                display_name = 'PerformanceByCategory/CorLoc@{}IOU/{}'.format(iou_thres, category_index[idx]['name'])
...
def visualize_detection_results(result_dict, tag, global_step, categories, summary_dir='', export_dir='', agnostic_mode=False, show_groundtruth=False, min_score_thresh=0.5, max_num_predictions=20):
  ...
  """
    if not set([
     'original_image', 'detection_boxes', 'detection_scores',
     'detection_classes']).issubset(set(result_dict.keys())):
        raise ValueError('result_dict does not contain all expected keys.')
    if show_groundtruth:
        if 'groundtruth_boxes' not in result_dict:
    ...
    if show_groundtruth:
        ...
        vis_utils.visualize_boxes_and_labels_on_image_array(image, groundtruth_boxes, None, None, category_index, keypoints=groundtruth_keypoints, use_normalized_coordinates=False, max_boxes_to_draw=None)
    vis_utils.visualize_boxes_and_labels_on_image_array(image, detection_boxes, detection_classes, detection_scores, category_index, instance_masks=detection_masks, keypoints=detection_keypoints, use_normalized_coordinates=False, max_boxes_to_draw=max_num_predictions, min_score_thresh=min_score_thresh, agnostic_mode=agnostic_mode)
    if export_dir:
        export_path = os.path.join(export_dir, 'export-{}.png'.format(tag))
        vis_utils.save_image_array_as_png(image, export_path)
    summary = tf.Summary(value=[
     tf.Summary.Value(tag=tag, image=tf.Summary.Image(encoded_image_string=vis_utils.encode_image_array_as_png_str(image)))])
    summary_writer = tf.summary.FileWriter(summary_dir)
    summary_writer.add_summary(summary, global_step)
    summary_writer.close()
            ...
            try:
                for batch in range(int(num_batches)):
                    ...
                    if not batch_processor:
                        ...
                        result_dict = batch_processor(tensor_dict, sess, batch, counters, update_op)
...
def repeated_checkpoint_run(tensor_dict, update_op, summary_dir, aggregated_result_processor=None, batch_processor=None, checkpoint_dirs=None, variables_to_restore=None, restore_fn=None, num_batches=1, eval_interval_secs=120, max_number_of_evaluations=None, master='', save_graph=False, save_graph_dir='', metric_names_to_values=None, keys_to_exclude_from_results=()):
  ...
  """
    ...
    while 1:
        ...
        if not model_path:
            logging.info('No model found in %s. Will try again in %d seconds', checkpoint_dirs[0], eval_interval_secs)
        else:
            if model_path == last_evaluated_model_path:
                ...
                run_checkpoint_once(tensor_dict, update_op, summary_dir, aggregated_result_processor, batch_processor, checkpoint_dirs, variables_to_restore, restore_fn, num_batches, master, save_graph, save_graph_dir, metric_names_to_values, keys_to_exclude_from_results)
