...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/object_detection/exporter_test.py
# Compiled at: 2018-06-15 01:39:54
# Size of source mod 2**32: 25563 bytes
"""Tests for object_detection.export_inference_graph."""
import os, numpy as np, six, tensorflow as tf
...
class FakeModel(model.DetectionModel):
    ...
    def postprocess(self, prediction_dict):
        with tf.control_dependencies(prediction_dict.values()):
            postprocessed_tensors = {'detection_boxes': tf.constant([
                                 [[0.0, 0.0, 0.5, 0.5],
                                  [
                                   0.5, 0.5, 0.8, 0.8]],
                                 [
...
class ExportInferenceGraphTest(tf.test.TestCase):
    ...
    def _save_checkpoint_from_mock_model(self, checkpoint_path, use_moving_averages):
        ...
        with g.as_default():
            ...
            preprocessed_inputs = mock_model.preprocess(tf.placeholder(tf.float32, shape=[None, None, None, 3]))
    ...
    def _create_tf_example(self, image_array):
        ...
        example = tf.train.Example(features=tf.train.Features(feature={'image/encoded': _bytes_feature(encoded_image), 
         'image/format': _bytes_feature('jpg'), 
         'image/source_id': _bytes_feature('image_id')})).SerializeToString()
        return example
    ...
    def test_export_and_run_inference_with_image_tensor(self):
        ...
        with self.test_session(graph=inference_graph) as (sess):
            ...
            boxes_np, scores_np, classes_np, masks_np, num_detections_np = sess.run([
             boxes, scores, classes, masks, num_detections], feed_dict={image_tensor: np.ones((2, 4, 4, 3)).astype(np.uint8)})
    ...
    def test_export_and_run_inference_with_encoded_image_string_tensor(self):
        ...
        with self.test_session(graph=inference_graph) as (sess):
            ...
            for image_str in [jpg_image_str, png_image_str]:
                ...
                boxes_np, scores_np, classes_np, masks_np, num_detections_np = sess.run([
                 boxes, scores, classes, masks, num_detections], feed_dict={image_str_tensor: image_str_batch_np})
                self.assertAllClose(boxes_np, [
                 [[0.0, 0.0, 0.5, 0.5],
                  [
                   0.5, 0.5, 0.8, 0.8]],
    ...
    def test_raise_runtime_error_on_images_with_different_sizes(self):
        ...
        with self.test_session(graph=inference_graph) as (sess):
            ...
            masks = inference_graph.get_tensor_by_name('detection_masks:0')
            num_detections = inference_graph.get_tensor_by_name('num_detections:0')
            with self.assertRaisesRegexp(tf.errors.InvalidArgumentError, '^TensorArray has inconsistent shapes.'):
    ...
    def test_export_and_run_inference_with_tf_example(self):
        ...
        with self.test_session(graph=inference_graph) as (sess):
            ...
            num_detections = inference_graph.get_tensor_by_name('num_detections:0')
            boxes_np, scores_np, classes_np, masks_np, num_detections_np = sess.run([
             boxes, scores, classes, masks, num_detections], feed_dict={tf_example: tf_example_np})
            self.assertAllClose(boxes_np, [
             [[0.0, 0.0, 0.5, 0.5],
              [
               0.5, 0.5, 0.8, 0.8]],
