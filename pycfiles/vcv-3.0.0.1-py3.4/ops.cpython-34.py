...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/utils/ops.py
# Compiled at: 2018-06-15 01:29:01
# Size of source mod 2**32: 27525 bytes
"""A module for helper tensorflow ops."""
import math, six, tensorflow as tf
...
def normalized_to_image_coordinates(normalized_boxes, image_shape, parallel_iterations=32):
  ...
  """
    ...
    absolute_boxes = tf.map_fn(_to_absolute_coordinates, elems=normalized_boxes, dtype=tf.float32, parallel_iterations=parallel_iterations, back_prop=True)
...
def pad_to_multiple(tensor, multiple):
  ...
  """
    ...
    if tensor_height is None:
        ...
        padded_tensor_height = tf.to_int32(tf.ceil(tf.to_float(tensor_height) / tf.to_float(multiple))) * multiple
    else:
        padded_tensor_height = int(math.ceil(float(tensor_height) / multiple) * multiple)
    if tensor_width is None:
        tensor_width = tf.shape(tensor)[2]
        padded_tensor_width = tf.to_int32(tf.ceil(tf.to_float(tensor_width) / tf.to_float(multiple))) * multiple
    else:
...
def padded_one_hot_encoding(indices, depth, left_pad):
  ...
  """
    ...
    def one_hot_and_pad():
        one_hot = tf.cast(tf.one_hot(tf.cast(indices, tf.int64), depth, on_value=1, off_value=0), tf.float32)
...
def normalize_to_target(inputs, target_norm_value, dim, epsilon=1e-07, trainable=True, scope='NormalizeToTarget', summarize=True):
  ...
  """
    with tf.variable_scope(scope, 'NormalizeToTarget', [inputs]):
        ...
        if not (isinstance(target_norm_value, float) or isinstance(target_norm_value, list) and len(target_norm_value) == depth and all([isinstance(val, float) for val in target_norm_value])):
        ...
        target_norm = tf.contrib.framework.model_variable(name='weights', dtype=tf.float32, initializer=tf.constant(initial_norm, dtype=tf.float32), trainable=trainable)
        if summarize:
...
def position_sensitive_crop_regions(image, boxes, box_ind, crop_size, num_spatial_bins, global_pool, extrapolation_value=None):
  ...
  """
    ...
    for split, box in zip(image_splits, position_sensitive_boxes):
        crop = tf.image.crop_and_resize(split, box, box_ind, bin_crop_size, extrapolation_value=extrapolation_value)
        image_crops.append(crop)

    if global_pool:
        position_sensitive_features = tf.add_n(image_crops) / len(image_crops)
        position_sensitive_features = tf.reduce_mean(position_sensitive_features, [1, 2], keep_dims=True)
    else:
        ...
        position_sensitive_features = tf.squeeze(tf.batch_to_space_nd(position_sensitive_features, block_shape=[
         1] + num_spatial_bins, crops=tf.zeros((3, 2), dtype=tf.int32)), squeeze_dims=[
         0])
    if block_size >= 2:
        position_sensitive_features = tf.depth_to_space(position_sensitive_features, block_size=block_size)
...
def reframe_box_masks_to_image_masks(box_masks, boxes, image_height, image_width):
  ...
  """
    ...
    image_masks = tf.image.crop_and_resize(image=box_masks, boxes=reverse_boxes, box_ind=tf.range(num_boxes), crop_size=[
     image_height, image_width], extrapolation_value=0.0)
