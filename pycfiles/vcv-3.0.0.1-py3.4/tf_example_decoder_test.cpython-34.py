...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/object_detection/data_decoders/tf_example_decoder_test.py
# Compiled at: 2018-06-15 01:39:54
# Size of source mod 2**32: 12324 bytes
...
class TfExampleDecoderTest(tf.test.TestCase):
    ...
    def testDecodeJpegImage(self):
        ...
        example = tf.train.Example(features=tf.train.Features(feature={'image/encoded': self._BytesFeature(encoded_jpeg), 
         ...
         'image/source_id': self._BytesFeature('image_id')})).SerializeToString()
        example_decoder = tf_example_decoder.TfExampleDecoder()
        tensor_dict = example_decoder.decode(tf.convert_to_tensor(example))
        self.assertAllEqual(tensor_dict[fields.InputDataFields.image].get_shape().as_list(), [
         None, None, 3])
    ...
    def testDecodeImageKeyAndFilename(self):
        ...
        example = tf.train.Example(features=tf.train.Features(feature={'image/encoded': self._BytesFeature(encoded_jpeg), 
         ...
         'image/filename': self._BytesFeature('filename')})).SerializeToString()
        example_decoder = tf_example_decoder.TfExampleDecoder()
        tensor_dict = example_decoder.decode(tf.convert_to_tensor(example))
        with self.test_session() as (sess):
            tensor_dict = sess.run(tensor_dict)
    ...
    def testDecodePngImage(self):
        ...
        example = tf.train.Example(features=tf.train.Features(feature={'image/encoded': self._BytesFeature(encoded_png), 
    ...
    def testDecodeBoundingBox(self):
        ...
        example = tf.train.Example(features=tf.train.Features(feature={'image/encoded': self._BytesFeature(encoded_jpeg), 
         ...
         'image/object/bbox/xmax': self._FloatFeature(bbox_xmaxs)})).SerializeToString()
        example_decoder = tf_example_decoder.TfExampleDecoder()
        tensor_dict = example_decoder.decode(tf.convert_to_tensor(example))
        self.assertAllEqual(tensor_dict[fields.InputDataFields.groundtruth_boxes].get_shape().as_list(), [
         None, 4])
        with self.test_session() as (sess):
            tensor_dict = sess.run(tensor_dict)
    ...
    def testDecodeObjectLabel(self):
        ...
        example = tf.train.Example(features=tf.train.Features(feature={'image/encoded': self._BytesFeature(encoded_jpeg), 
         'image/format': self._BytesFeature('jpeg'), 
         'image/object/class/label': self._Int64Feature(bbox_classes)})).SerializeToString()
        example_decoder = tf_example_decoder.TfExampleDecoder()
        tensor_dict = example_decoder.decode(tf.convert_to_tensor(example))
        self.assertAllEqual(tensor_dict[fields.InputDataFields.groundtruth_classes].get_shape().as_list(), [
         None])
        with self.test_session() as (sess):
            tensor_dict = sess.run(tensor_dict)
    ...
    def testDecodeObjectArea(self):
        ...
        example = tf.train.Example(features=tf.train.Features(feature={'image/encoded': self._BytesFeature(encoded_jpeg), 
         'image/format': self._BytesFeature('jpeg'), 
         'image/object/area': self._FloatFeature(object_area)})).SerializeToString()
        example_decoder = tf_example_decoder.TfExampleDecoder()
        tensor_dict = example_decoder.decode(tf.convert_to_tensor(example))
        self.assertAllEqual(tensor_dict[fields.InputDataFields.groundtruth_area].get_shape().as_list(), [
         None])
        with self.test_session() as (sess):
            tensor_dict = sess.run(tensor_dict)
    ...
    def testDecodeObjectIsCrowd(self):
        ...
        example = tf.train.Example(features=tf.train.Features(feature={'image/encoded': self._BytesFeature(encoded_jpeg), 
         'image/format': self._BytesFeature('jpeg'), 
         'image/object/is_crowd': self._Int64Feature(object_is_crowd)})).SerializeToString()
        example_decoder = tf_example_decoder.TfExampleDecoder()
        tensor_dict = example_decoder.decode(tf.convert_to_tensor(example))
        self.assertAllEqual(tensor_dict[fields.InputDataFields.groundtruth_is_crowd].get_shape().as_list(), [
         None])
        with self.test_session() as (sess):
            tensor_dict = sess.run(tensor_dict)
    ...
    def testDecodeObjectDifficult(self):
        ...
        example = tf.train.Example(features=tf.train.Features(feature={'image/encoded': self._BytesFeature(encoded_jpeg), 
         'image/format': self._BytesFeature('jpeg'), 
         'image/object/difficult': self._Int64Feature(object_difficult)})).SerializeToString()
        example_decoder = tf_example_decoder.TfExampleDecoder()
        tensor_dict = example_decoder.decode(tf.convert_to_tensor(example))
        self.assertAllEqual(tensor_dict[fields.InputDataFields.groundtruth_difficult].get_shape().as_list(), [
         None])
        with self.test_session() as (sess):
            tensor_dict = sess.run(tensor_dict)
    ...
    def testDecodeInstanceSegmentation(self):
        ...
        image_tensor = np.random.randint(255, size=(image_height,
         ...
         3)).astype(np.uint8)
        encoded_jpeg = self._EncodeImage(image_tensor)
        instance_segmentation = np.random.randint(2, size=(num_instances,
         image_height,
         image_width)).astype(np.int64)
        instance_segmentation_classes = np.random.randint(100, size=num_instances).astype(np.int64)
        example = tf.train.Example(features=tf.train.Features(feature={'image/encoded': self._BytesFeature(encoded_jpeg), 
         'image/format': self._BytesFeature('jpeg'), 
         'image/height': self._Int64Feature([image_height]), 
         'image/width': self._Int64Feature([image_width]), 
         'image/segmentation/object': self._Int64Feature(instance_segmentation.flatten()), 
         'image/segmentation/object/class': self._Int64Feature(instance_segmentation_classes)})).SerializeToString()
        example_decoder = tf_example_decoder.TfExampleDecoder()
        tensor_dict = example_decoder.decode(tf.convert_to_tensor(example))
        self.assertAllEqual(tensor_dict[fields.InputDataFields.groundtruth_instance_masks].get_shape().as_list(), [
         None, None, None])
        self.assertAllEqual(tensor_dict[fields.InputDataFields.groundtruth_instance_classes].get_shape().as_list(), [
         None])
        with self.test_session() as (sess):
            tensor_dict = sess.run(tensor_dict)
        self.assertAllEqual(instance_segmentation.astype(np.bool), tensor_dict[fields.InputDataFields.groundtruth_instance_masks])
        self.assertAllEqual(instance_segmentation_classes, tensor_dict[fields.InputDataFields.groundtruth_instance_classes])
