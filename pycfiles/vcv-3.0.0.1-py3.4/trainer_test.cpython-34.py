...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/object_detection/trainer_test.py
# Compiled at: 2018-06-15 01:39:54
# Size of source mod 2**32: 6550 bytes
...
def get_input_function():
    ...
    return {fields.InputDataFields.image: image, 
...
class FakeDetectionModel(model.DetectionModel):
    __doc__ = 'A simple (and poor) DetectionModel for use in test.'

    def __init__(self):
        super(FakeDetectionModel, self).__init__(num_classes=NUMBER_OF_CLASSES)
        self._classification_loss = losses.WeightedSigmoidClassificationLoss(anchorwise_output=True)
        self._localization_loss = losses.WeightedSmoothL1LocalizationLoss(anchorwise_output=True)
    ...
    """
        ...
        return {'class_predictions_with_background': tf.reshape(class_prediction, [-1, 1, self._num_classes]), 
    ...
    """
        ...
        weights = tf.constant(1.0, dtype=tf.float32, shape=[
         len(self.groundtruth_lists(fields.BoxListFields.boxes)), 1])
        location_losses = self._localization_loss(prediction_dict['box_encodings'], batch_reg_targets, weights=weights)
        cls_losses = self._classification_loss(prediction_dict['class_predictions_with_background'], batch_cls_targets, weights=weights)
        loss_dict = {'localization_loss': tf.reduce_sum(location_losses), 
