...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/object_detection/core/target_assigner.py
...
class TargetAssigner(object):
    ...
    """
        ...
        return (
         cls_targets, cls_weights, reg_targets, reg_weights, match)
  ...
  """
    if reference == 'Multibox' and stage == 'proposal':
        ...
        if reference == 'FasterRCNN' and stage == 'proposal':
            ...
            matcher = argmax_matcher.ArgMaxMatcher(matched_threshold=0.7, unmatched_threshold=0.3, force_match_for_each_row=True)
            box_coder = faster_rcnn_box_coder.FasterRcnnBoxCoder(scale_factors=[
             10.0, 10.0, 5.0, 5.0])
        else:
            if reference == 'FasterRCNN' and stage == 'detection':
                ...
                if reference == 'FastRCNN':
                    similarity_calc = sim_calc.IouSimilarity()
                    matcher = argmax_matcher.ArgMaxMatcher(matched_threshold=0.5, unmatched_threshold=0.1, force_match_for_each_row=False, negatives_lower_than_unmatched=False)
                    box_coder = faster_rcnn_box_coder.FasterRcnnBoxCoder()
                else:
                    raise ValueError('No valid combination of reference and stage.')
    return TargetAssigner(similarity_calc, matcher, box_coder, positive_class_weight=positive_class_weight, negative_class_weight=negative_class_weight, unmatched_cls_target=unmatched_cls_target)
...
def batch_assign_targets(target_assigner, anchors_batch, gt_box_batch, gt_class_targets_batch):
  ...
  """
    ...
    reg_weights_list = []
    match_list = []
    for anchors, gt_boxes, gt_class_targets in zip(anchors_batch, gt_box_batch, gt_class_targets_batch):
        cls_targets, cls_weights, reg_targets, reg_weights, match = target_assigner.assign(anchors, gt_boxes, gt_class_targets)
