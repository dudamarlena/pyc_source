...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/object_detection/builders/model_builder.py
...
FASTER_RCNN_FEATURE_EXTRACTOR_CLASS_MAP = {'faster_rcnn_resnet50': frcnn_resnet_v1.FasterRCNNResnet50FeatureExtractor, 
...
def _build_ssd_model(ssd_config, is_training):
  ...
  """
    ...
    feature_extractor = _build_ssd_feature_extractor(ssd_config.feature_extractor, is_training)
    box_coder = box_coder_builder.build(ssd_config.box_coder)
    matcher = matcher_builder.build(ssd_config.matcher)
    region_similarity_calculator = sim_calc.build(ssd_config.similarity_calculator)
    ssd_box_predictor = box_predictor_builder.build(hyperparams_builder.build, ssd_config.box_predictor, is_training, num_classes)
...
def _build_faster_rcnn_model(frcnn_config, is_training):
  ...
  """
    ...
    feature_extractor = _build_faster_rcnn_feature_extractor(frcnn_config.feature_extractor, is_training)
    ...
    first_stage_box_predictor_arg_scope = hyperparams_builder.build(frcnn_config.first_stage_box_predictor_conv_hyperparams, is_training)
    first_stage_box_predictor_kernel_size = frcnn_config.first_stage_box_predictor_kernel_size
    first_stage_box_predictor_depth = frcnn_config.first_stage_box_predictor_depth
    first_stage_minibatch_size = frcnn_config.first_stage_minibatch_size
    first_stage_positive_balance_fraction = frcnn_config.first_stage_positive_balance_fraction
    ...
    if isinstance(second_stage_box_predictor, box_predictor.RfcnBoxPredictor):
        ...
        return faster_rcnn_meta_arch.FasterRCNNMetaArch(initial_crop_size=initial_crop_size, maxpool_kernel_size=maxpool_kernel_size, maxpool_stride=maxpool_stride, second_stage_mask_rcnn_box_predictor=second_stage_box_predictor, **common_kwargs)