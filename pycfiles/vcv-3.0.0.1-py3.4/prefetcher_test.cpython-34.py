...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/object_detection/core/prefetcher_test.py
    ...
    def test_prefetch_tensors_with_fully_defined_shapes(self):
        with self.test_session() as (sess):
            ...
            image = tf.random_normal([batch_size, image_size,
             image_size, 3], dtype=tf.float32, name='images')
            label = tf.random_uniform([batch_size, 1], 0, 10, dtype=tf.int32, name='labels')
            prefetch_queue = prefetcher.prefetch(tensor_dict={'counter': counter,  'image': image, 
             'label': label}, capacity=100)
    ...
    def test_prefetch_tensors_with_partially_defined_shapes(self):
        with self.test_session() as (sess):
            ...
            image = tf.random_normal([batch_size,
             ...
             tf.Variable(image_size), 3], dtype=tf.float32, name='image')
