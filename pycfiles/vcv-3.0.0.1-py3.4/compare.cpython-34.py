...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/util/protobuf/compare.py
...
def assertProtoEqual(self, a, b, check_initialized=True, normalize_numbers=False, msg=None):
  ...
  """
    ...
    self.assertMultiLineEqual(text_format.MessageToString(a, descriptor_pool=pool), text_format.MessageToString(b, descriptor_pool=pool), msg=msg)
    ...
    for desc, values in pb.ListFields():
        ...
        if desc.type in (descriptor.FieldDescriptor.TYPE_INT64,
         ...
         descriptor.FieldDescriptor.TYPE_SINT64):
            ...
            if desc.type in (descriptor.FieldDescriptor.TYPE_INT32,
        ...
        if desc.type == descriptor.FieldDescriptor.TYPE_MESSAGE or desc.type == descriptor.FieldDescriptor.TYPE_GROUP:
            if desc.type == descriptor.FieldDescriptor.TYPE_MESSAGE and desc.message_type.has_options and desc.message_type.GetOptions().map_entry:
                if desc.message_type.fields_by_number[2].type == descriptor.FieldDescriptor.TYPE_MESSAGE:
