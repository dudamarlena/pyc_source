...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/util/tf_should_use.py
...
import collections, functools, itertools, traceback, types, six
from backports import weakref
from tensorflow.python.platform import tf_logging
from tensorflow.python.util import tf_decorator

class _RefInfoField(collections.namedtuple('_RefInfoField', ('type_', 'repr_', 'creation_stack', 'object_used'))):
    pass
...
def _add_should_use_warning(x, fatal_error=False):
  ...
  """
    if x is None:
        return x
    if hasattr(x, '_tf_ref_id'):
        return x

    def override_method(method):

        def fn(self, *args, **kwargs):
            _REF_INFO[self._tf_ref_id] = _REF_INFO[self._tf_ref_id]._replace(object_used=True)
            return method(self, *args, **kwargs)

        return fn

    class TFShouldUseWarningWrapper:
        __doc__ = 'Wrapper for objects that keeps track of their use.'
        ...
        def __init__(self, true_self):
            ...
            _REF_INFO[self._tf_ref_id] = _RefInfoField(type_=type(x), repr_=repr(x), creation_stack=stack, object_used=False)
    ...
    for name in dir(TFShouldUseWarningWrapper):
        ...
        if name in ('__init__', '__getattribute__', '__del__', 'mark_used'):
            continue
        setattr(TFShouldUseWarningWrapper, name, functools.wraps(method)(override_method(method)))

    wrapped = TFShouldUseWarningWrapper(x)
    wrapped.__doc__ = x.__doc__
    ref_id = wrapped._tf_ref_id
...
def should_use_result(fn):
  ...
  """
    ...
    return tf_decorator.make_decorator(fn, wrapped, 'should_use_result', (fn.__doc__ or '') + '\n\n  **NOTE** The output of this function should be used.  If it is not, a warning will be logged.  To mark the output as used, call its .mark_used() method.')
