...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/object_detection/core/box_predictor.py
# Compiled at: 2018-06-15 01:39:54
# Size of source mod 2**32: 24381 bytes
    ...
    """
        ...
        with slim.arg_scope(self._conv_hyperparams):
            ...
            location_feature_map = slim.conv2d(net, location_feature_map_depth, [
             1, 1], activation_fn=None, scope='refined_locations')
...
class MaskRCNNBoxPredictor(BoxPredictor):
    ...
    """
        ...
        if self._predict_instance_masks or self._predict_keypoints:
            if self._conv_hyperparams is None:
    ...
    """
        ...
        if self._predict_instance_masks:
            with slim.arg_scope(self._conv_hyperparams):
                upsampled_features = slim.conv2d_transpose(image_features, num_outputs=self._mask_prediction_conv_depth, kernel_size=[
                 2, 2], stride=2)
                mask_predictions = slim.conv2d(upsampled_features, num_outputs=self.num_classes, activation_fn=None, kernel_size=[
                 1, 1])
                instance_masks = tf.expand_dims(tf.transpose(mask_predictions, perm=[
...
class ConvolutionalBoxPredictor(BoxPredictor):
    ...
    """
        ...
        with slim.arg_scope(self._conv_hyperparams):
            with slim.arg_scope([slim.dropout], is_training=self._is_training):
                ...
                with slim.arg_scope([slim.conv2d], activation_fn=None, normalizer_fn=None, normalizer_params=None):
                    box_encodings = slim.conv2d(net, num_predictions_per_location * self._box_code_size, [
                     self._kernel_size, self._kernel_size], scope='BoxEncodingPredictor')
                    if self._use_dropout:
                        net = slim.dropout(net, keep_prob=self._dropout_keep_prob)
                    class_predictions_with_background = slim.conv2d(net, num_predictions_per_location * num_class_slots, [
        ...
        box_encodings = tf.reshape(box_encodings, tf.stack([combined_feature_map_shape[0],
         combined_feature_map_shape[1] * combined_feature_map_shape[2] * num_predictions_per_location,
         1, self._box_code_size]))
        class_predictions_with_background = tf.reshape(class_predictions_with_background, tf.stack([combined_feature_map_shape[0],
         combined_feature_map_shape[1] * combined_feature_map_shape[2] * num_predictions_per_location,
         num_class_slots]))
        return {BOX_ENCODINGS: box_encodings,  CLASS_PREDICTIONS_WITH_BACKGROUND: class_predictions_with_background}