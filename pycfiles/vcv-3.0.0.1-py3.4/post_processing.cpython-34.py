...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/object_detection/core/post_processing.py
# Compiled at: 2018-06-15 01:39:54
# Size of source mod 2**32: 14887 bytes
  ...
  """
    ...
    with tf.name_scope(scope, 'MultiClassNonMaxSuppression'):
        ...
        for class_idx, boxes_idx in zip(range(num_classes), boxes_ids):
            ...
            boxlist_filtered = box_list_ops.filter_greater_than(boxlist_and_class_scores, score_thresh)
            if clip_window is not None:
                boxlist_filtered = box_list_ops.clip_to_window(boxlist_filtered, clip_window)
                if change_coordinate_frame:
                    boxlist_filtered = box_list_ops.change_coordinate_frame(boxlist_filtered, clip_window)
                max_selection_size = tf.minimum(max_size_per_class, boxlist_filtered.num_boxes())
                selected_indices = tf.image.non_max_suppression(boxlist_filtered.get(), boxlist_filtered.get_field(fields.BoxListFields.scores), max_selection_size, iou_threshold=iou_thresh)
  ...
  """
    ...
    with tf.name_scope(scope, 'BatchMultiClassNonMaxSuppression'):
        ...
        def single_image_nms_fn(args):
            ...
            per_image_boxes, per_image_scores, per_image_masks, per_image_num_valid_boxes = args
            ...
            nmsed_boxlist = multiclass_non_max_suppression(per_image_boxes, per_image_scores, score_thresh, iou_thresh, max_size_per_class, max_total_size, masks=per_image_masks, clip_window=clip_window, change_coordinate_frame=change_coordinate_frame)
            padded_boxlist = box_list_ops.pad_or_clip_box_list(nmsed_boxlist, max_total_size)
        ...
        batch_nmsed_boxes, batch_nmsed_scores, batch_nmsed_classes, batch_nmsed_masks, batch_num_detections = tf.map_fn(single_image_nms_fn, elems=[
         boxes, scores, masks, num_valid_boxes], dtype=[
         tf.float32, tf.float32, tf.float32, tf.float32, tf.int32], parallel_iterations=parallel_iterations)
