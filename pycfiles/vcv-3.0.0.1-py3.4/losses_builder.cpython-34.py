...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/object_detection/builders/losses_builder.py
...
def _build_localization_loss(loss_config):
  ...
  """
    ...
    if loss_type == 'weighted_l2':
        ...
        return losses.WeightedL2LocalizationLoss(anchorwise_output=config.anchorwise_output)
    if loss_type == 'weighted_smooth_l1':
        config = loss_config.weighted_smooth_l1
        return losses.WeightedSmoothL1LocalizationLoss(anchorwise_output=config.anchorwise_output)
...
def _build_classification_loss(loss_config):
  ...
  """
    ...
    if loss_type == 'weighted_sigmoid':
        ...
        return losses.WeightedSigmoidClassificationLoss(anchorwise_output=config.anchorwise_output)
    if loss_type == 'weighted_softmax':
        config = loss_config.weighted_softmax
        return losses.WeightedSoftmaxClassificationLoss(anchorwise_output=config.anchorwise_output)
    if loss_type == 'bootstrapped_sigmoid':
        config = loss_config.bootstrapped_sigmoid
        return losses.BootstrappedSigmoidClassificationLoss(alpha=config.alpha, bootstrap_type='hard' if config.hard_bootstrap else 'soft', anchorwise_output=config.anchorwise_output)
    raise ValueError('Empty loss config.')