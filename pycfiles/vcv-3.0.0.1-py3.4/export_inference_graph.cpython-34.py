...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/object_detection/export_inference_graph.py
...
def main(_):
    ...
    assert FLAGS.trained_checkpoint_prefix, '`trained_checkpoint_prefix` is missing'
    assert FLAGS.output_directory, '`output_directory` is missing'
    pipeline_config = pipeline_pb2.TrainEvalPipelineConfig()
    with tf.gfile.GFile(FLAGS.pipeline_config_path, 'r') as (f):
        text_format.Merge(f.read(), pipeline_config)
    exporter.export_inference_graph(FLAGS.input_type, pipeline_config, FLAGS.trained_checkpoint_prefix, FLAGS.output_directory)
