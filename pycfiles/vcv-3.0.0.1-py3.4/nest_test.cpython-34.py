...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/util/nest_test.py
# Compiled at: 2018-06-15 01:22:48
# Size of source mod 2**32: 14870 bytes
...
class NestTest(test.TestCase):
    ...
    def testFlattenAndPack(self):
        ...
        self.assertEqual(nest.pack_sequence_as(structure, flat), (('a', 'b'), 'c',
                                                                  ('d', 'e', ('f', 'g'), 'h')))
    ...
    def testFlattenDictItems(self):
        ...
        with self.assertRaisesRegexp(ValueError, 'Key had [0-9]* elements, but value had [0-9]* elements'):
    ...
    def testAssertSameStructure(self):
        ...
        with self.assertRaisesRegexp(ValueError, "don't have the same number of elements"):
        ...
        with self.assertRaisesRegexp(ValueError, "don't have the same nested structure"):
            nest.assert_same_structure(structure1, structure_different_nesting)
        named_type_0 = collections.namedtuple('named_0', ('a', 'b'))
        named_type_1 = collections.namedtuple('named_1', ('a', 'b'))
        self.assertRaises(TypeError, nest.assert_same_structure, (0, 1), named_type_0('a', 'b'))
        nest.assert_same_structure(named_type_0(3, 4), named_type_0('a', 'b'))
        self.assertRaises(TypeError, nest.assert_same_structure, named_type_0(3, 4), named_type_1(3, 4))
        with self.assertRaisesRegexp(ValueError, "don't have the same nested structure"):
            nest.assert_same_structure(named_type_0(3, 4), named_type_0([3], 4))
        with self.assertRaisesRegexp(ValueError, "don't have the same nested structure"):
            nest.assert_same_structure([[3], 4], [3, [4]])
        structure1_list = [[[1, 2], 3], 4, [5, 6]]
        with self.assertRaisesRegexp(TypeError, "don't have the same sequence type"):
    ...
    def testMapStructure(self):
        ...
        self.assertAllEqual([
         2, 3, 4, 5, 6, 7], nest.flatten(structure1_plus1))
        structure1_plus_structure2 = nest.map_structure(lambda x, y: x + y, structure1, structure2)
        self.assertEqual((((8, 10), 12), 14, (16, 18)), structure1_plus_structure2)
    ...
    def testAssertShallowStructure(self):
        inp_ab = ['a', 'b']
        inp_abc = ['a', 'b', 'c']
        expected_message = "The two structures don't have the same sequence length. Input structure has length 2, while shallow structure has length 3."
        with self.assertRaisesRegexp(ValueError, expected_message):
            nest.assert_shallow_structure(inp_abc, inp_ab)
        inp_ab1 = [(1, 1), (2, 2)]
        inp_ab2 = [[1, 1], [2, 2]]
        expected_message = "The two structures don't have the same sequence type. Input structure has type <(type|class) 'tuple'>, while shallow structure has type <(type|class) 'list'>."
