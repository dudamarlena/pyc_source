...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/object_detection/core/box_predictor_test.py
...
class MaskRCNNBoxPredictorTest(tf.test.TestCase):
    ...
    def test_get_boxes_with_five_classes(self):
        ...
        mask_box_predictor = box_predictor.MaskRCNNBoxPredictor(is_training=False, num_classes=5, fc_hyperparams=self._build_arg_scope_with_hyperparams(), use_dropout=False, dropout_keep_prob=0.5, box_code_size=4)
        box_predictions = mask_box_predictor.predict(image_features, num_predictions_per_location=1, scope='BoxPredictor')
        box_encodings = box_predictions[box_predictor.BOX_ENCODINGS]
        class_predictions_with_background = box_predictions[box_predictor.CLASS_PREDICTIONS_WITH_BACKGROUND]
        init_op = tf.global_variables_initializer()
        with self.test_session() as (sess):
            sess.run(init_op)
            box_encodings_shape, class_predictions_with_background_shape = sess.run([
    ...
    def test_value_error_on_predict_keypoints(self):
        with self.assertRaises(ValueError):
            box_predictor.MaskRCNNBoxPredictor(is_training=False, num_classes=5, fc_hyperparams=self._build_arg_scope_with_hyperparams(), use_dropout=False, dropout_keep_prob=0.5, box_code_size=4, predict_keypoints=True)
...
class RfcnBoxPredictorTest(tf.test.TestCase):
    ...
    def test_get_correct_box_encoding_and_class_prediction_shapes(self):
        ...
        rfcn_box_predictor = box_predictor.RfcnBoxPredictor(is_training=False, num_classes=2, conv_hyperparams=self._build_arg_scope_with_conv_hyperparams(), num_spatial_bins=[
         3, 3], depth=4, crop_size=[
         12, 12], box_code_size=4)
...
class ConvolutionalBoxPredictorTest(tf.test.TestCase):
    ...
    def test_get_boxes_for_five_aspect_ratios_per_location(self):
        image_features = tf.random_uniform([4, 8, 8, 64], dtype=tf.float32)
        conv_box_predictor = box_predictor.ConvolutionalBoxPredictor(is_training=False, num_classes=0, conv_hyperparams=self._build_arg_scope_with_conv_hyperparams(), min_depth=0, max_depth=32, num_layers_before_predictor=1, use_dropout=True, dropout_keep_prob=0.8, kernel_size=1, box_code_size=4)
        box_predictions = conv_box_predictor.predict(image_features, num_predictions_per_location=5, scope='BoxPredictor')
        box_encodings = box_predictions[box_predictor.BOX_ENCODINGS]
        objectness_predictions = box_predictions[box_predictor.CLASS_PREDICTIONS_WITH_BACKGROUND]
        init_op = tf.global_variables_initializer()
        with self.test_session() as (sess):
            sess.run(init_op)
    ...
    def test_get_boxes_for_one_aspect_ratio_per_location(self):
        image_features = tf.random_uniform([4, 8, 8, 64], dtype=tf.float32)
        conv_box_predictor = box_predictor.ConvolutionalBoxPredictor(is_training=False, num_classes=0, conv_hyperparams=self._build_arg_scope_with_conv_hyperparams(), min_depth=0, max_depth=32, num_layers_before_predictor=1, use_dropout=True, dropout_keep_prob=0.8, kernel_size=1, box_code_size=4)
        box_predictions = conv_box_predictor.predict(image_features, num_predictions_per_location=1, scope='BoxPredictor')
        box_encodings = box_predictions[box_predictor.BOX_ENCODINGS]
        objectness_predictions = box_predictions[box_predictor.CLASS_PREDICTIONS_WITH_BACKGROUND]
        init_op = tf.global_variables_initializer()
        with self.test_session() as (sess):
    ...
    def test_get_boxes_for_five_aspect_ratios_per_location_fully_convolutional(self):
        ...
        with self.test_session() as (sess):
            sess.run(init_op)
            box_encodings_shape, objectness_predictions_shape = sess.run([
             tf.shape(box_encodings), tf.shape(objectness_predictions)], feed_dict={image_features: np.random.rand(4, resolution, resolution, 64)})
