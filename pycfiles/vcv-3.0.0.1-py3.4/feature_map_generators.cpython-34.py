...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/object_detection/models/feature_map_generators.py
...
def multi_resolution_feature_maps(feature_map_layout, depth_multiplier, min_depth, insert_1x1_conv, image_features):
  ...
  """
    ...
    for index, (from_layer, layer_depth) in enumerate(zip(feature_map_layout['from_layer'], feature_map_layout['layer_depth'])):
        if from_layer:
            ...
            stride = 2
            layer_name = '{}_2_Conv2d_{}_3x3_s2_{}'.format(base_from_layer, index, depth_fn(layer_depth))
            if use_depthwise:
                feature_map = slim.separable_conv2d(ops.pad_to_multiple(intermediate_layer, stride), None, [3, 3], depth_multiplier=1, padding='SAME', stride=stride, scope=layer_name + '_depthwise')
                feature_map = slim.conv2d(feature_map, depth_fn(layer_depth), [1, 1], padding='SAME', stride=1, scope=layer_name)
            else:
                feature_map = slim.conv2d(ops.pad_to_multiple(intermediate_layer, stride), depth_fn(layer_depth), [3, 3], padding='SAME', stride=stride, scope=layer_name)
            if index > 0:
                if feature_map_strides and feature_map_strides[(index - 1)] > 0:
                    feature_map_strides[index] = stride * feature_map_strides[(index - 1)]
