...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/object_detection/anchor_generators/multiple_grid_anchor_generator.py
# Compiled at: 2018-06-15 01:39:54
# Size of source mod 2**32: 12824 bytes
...
class MultipleGridAnchorGenerator(anchor_generator.AnchorGenerator):
    ...
    """
        ...
        if clip_window is not None:
            if clip_window.get_shape().as_list() != [4]:
    ...
    """
        ...
        if not anchor_strides:
            anchor_strides = [(tf.to_float(im_height) / tf.to_float(pair[0]), tf.to_float(im_width) / tf.to_float(pair[1])) for pair in feature_map_shape_list]
        ...
        for grid_size, scales, aspect_ratios, stride, offset in zip(feature_map_shape_list, self._scales, self._aspect_ratios, anchor_strides, anchor_offsets):
            anchor_grid_list.append(grid_anchor_generator.tile_anchors(grid_height=grid_size[0], grid_width=grid_size[1], scales=scales, aspect_ratios=aspect_ratios, base_anchor_size=base_anchor_size, anchor_stride=stride, anchor_offset=offset))
        ...
        if self._clip_window is not None:
            clip_window = tf.multiply(tf.to_float([im_height, im_width, im_height, im_width]), self._clip_window)
            concatenated_anchors = box_list_ops.clip_to_window(concatenated_anchors, clip_window, filter_nonoverlapping=False)
            concatenated_anchors.set(tf.reshape(concatenated_anchors.get(), [num_anchors, 4]))
...
def create_ssd_anchors(num_layers=6, min_scale=0.2, max_scale=0.95, aspect_ratios=(1.0, 2.0, 3.0, 0.5, 0.3333333333333333), base_anchor_size=None, reduce_boxes_in_lowest_layer=True):
