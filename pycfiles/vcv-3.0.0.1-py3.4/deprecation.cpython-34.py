...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/util/deprecation.py
# Compiled at: 2018-06-15 01:22:48
# Size of source mod 2**32: 15749 bytes
"""Tensor utility functions."""
from __future__ import absolute_import
from __future__ import division
from __future__ import print_function
import collections, functools, re
...
def _add_deprecated_function_notice_to_docstring(doc, date, instructions):
    ...
    return decorator_utils.add_notice_to_docstring(doc, instructions, 'DEPRECATED FUNCTION', '(deprecated)', [
...
def _add_deprecated_arg_notice_to_docstring(doc, date, instructions):
    ...
    return decorator_utils.add_notice_to_docstring(doc, instructions, 'DEPRECATED FUNCTION ARGUMENTS', '(deprecated arguments)', [
     'SOME ARGUMENTS ARE DEPRECATED. They will be removed %s.' % ('in a future version' if date is None else 'after %s' % date),
     'Instructions for updating:'])


def _validate_deprecation_args(date, instructions):
    if date is not None:
        if not re.match('20\\d\\d-[01]\\d-[0123]\\d', date):
    ...
    def deprecated_wrapper(func):
        ...
        def new_func(*args, **kwargs):
            ...
            if _PRINT_DEPRECATION_WARNINGS:
                logging.warning('From %s: %s (from %s) is deprecated and will be removed %s.\nInstructions for updating:\n%s', _call_location(), decorator_utils.get_qualified_name(func), func.__module__, 'in a future version' if date is None else 'after %s' % date, instructions)
            return func(*args, **kwargs)

        return tf_decorator.make_decorator(func, new_func, 'deprecated', _add_deprecated_function_notice_to_docstring(func.__doc__, date, instructions))
...
def deprecated_args(date, instructions, *deprecated_arg_names_or_tuples):
  ...
  """
    ...
    def deprecated_wrapper(func):
        ...
        if len(deprecated_positions) + is_varargs_deprecated + is_kwargs_deprecated != len(deprecated_arg_names_or_tuples):
            ...
            raise ValueError('The following deprecated arguments are not present in the function signature: %s. Found next arguments: %s.' % (
             missing_args, known_args))
      ...
      """
        ...
        def new_func(*args, **kwargs):
            ...
            if _PRINT_DEPRECATION_WARNINGS:
                ...
                if is_varargs_deprecated:
                    if len(args) > len(arg_spec.args):
                        invalid_args.append(arg_spec.varargs)
                if is_kwargs_deprecated:
                    if kwargs:
                ...
                for arg_name in invalid_args:
                    logging.warning('From %s: calling %s (from %s) with %s is deprecated and will be removed %s.\nInstructions for updating:\n%s', _call_location(), decorator_utils.get_qualified_name(func), func.__module__, arg_name, 'in a future version' if date is None else 'after %s' % date, instructions)

            return func(*args, **kwargs)

        return tf_decorator.make_decorator(func, new_func, 'deprecated', _add_deprecated_arg_notice_to_docstring(func.__doc__, date, instructions))
