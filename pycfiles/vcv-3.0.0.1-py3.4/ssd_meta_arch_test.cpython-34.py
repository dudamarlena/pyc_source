...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/object_detection/meta_architectures/ssd_meta_arch_test.py
...
import functools, numpy as np, tensorflow as tf
...
class FakeSSDFeatureExtractor(ssd_meta_arch.SSDFeatureExtractor):
    ...
    def __init__(self):
        super(FakeSSDFeatureExtractor, self).__init__(depth_multiplier=0, min_depth=0, conv_hyperparams=None)
    ...
    def extract_features(self, preprocessed_inputs):
        with tf.variable_scope('mock_model'):
            features = slim.conv2d(inputs=preprocessed_inputs, num_outputs=32, kernel_size=[
             1, 1], scope='layer1')
            return [features]


class MockAnchorGenerator2x2(anchor_generator.AnchorGenerator):
    __doc__ = 'Sets up a simple 2x2 anchor grid on the unit square.'
    ...
    def _generate(self, feature_map_shape_list):
        return box_list.BoxList(tf.constant([[0, 0, 0.5, 0.5],
         [
          0, 0.5, 0.5, 1],
         [
          0.5, 0, 1, 0.5],
         [
          0.5, 0.5, 1, 1]], tf.float32))
...
class SsdMetaArchTest(tf.test.TestCase):
    ...
    def test_predict_results_have_correct_keys_and_shapes(self):
        ...
        for input_shape in input_shapes:
            ...
            with self.test_session(graph=tf_graph) as (sess):
                sess.run(init_op)
                prediction_out = sess.run(prediction_dict, feed_dict={preprocessed_input_placeholder: np.random.uniform(size=(
                                                  batch_size, 2, 2, 3))})
    ...
    def test_postprocess_results_are_correct(self):
        ...
        for input_shape in input_shapes:
            ...
            with self.test_session(graph=tf_graph) as (sess):
                sess.run(init_op)
                detections_out = sess.run(detections, feed_dict={preprocessed_input_placeholder: np.random.uniform(size=(
                                                  batch_size, 2, 2, 3))})
    ...
    def test_loss_results_are_correct(self):
        batch_size = 2
        preprocessed_input = tf.random_uniform((batch_size, 2, 2, 3), dtype=tf.float32)
        ...
        self._model.provide_groundtruth(groundtruth_boxes_list, groundtruth_classes_list)
        ...
        expected_classification_loss = batch_size * self._num_anchors * (self._num_classes + 1) * np.log(2.0)
        init_op = tf.global_variables_initializer()
        with self.test_session() as (sess):
            sess.run(init_op)
            losses_out = sess.run(loss_dict)
            self.assertAllClose(losses_out['localization_loss'], expected_localization_loss)
            self.assertAllClose(losses_out['classification_loss'], expected_classification_loss)
