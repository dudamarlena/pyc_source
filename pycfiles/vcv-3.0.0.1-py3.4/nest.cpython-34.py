...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/util/nest.py
# Compiled at: 2018-06-15 01:22:48
# Size of source mod 2**32: 19252 bytes
...
import collections as _collections, six as _six
...
def _sequence_like(instance, args):
  ...
  """
    if isinstance(instance, tuple) and hasattr(instance, '_fields') and isinstance(instance._fields, _collections.Sequence) and all(isinstance(f, _six.string_types) for f in instance._fields):
...
def is_sequence(seq):
  ...
  """
    return isinstance(seq, _collections.Sequence) and not isinstance(seq, _six.string_types)
...
def _recursive_assert_same_structure(nest1, nest2, check_types):
    ...
    if is_sequence_nest1 != is_sequence(nest2):
        raise ValueError("The two structures don't have the same nested structure. First structure: %s, second structure: %s." % (
         nest1, nest2))
    if is_sequence_nest1:
        ...
        if check_types:
            if type_nest1 != type_nest2:
                raise TypeError("The two structures don't have the same sequence type. First structure has type %s, while second structure has type %s." % (
...
def assert_same_structure(nest1, nest2, check_types=True):
  ...
  """
    ...
    if len_nest1 != len_nest2:
        raise ValueError("The two structures don't have the same number of elements. First structure: %s, second structure: %s." % (
         nest1, nest2))
    ...
    for i, v in _six.iteritems(dictionary):
        if not is_sequence(i):
            if i in flat_dictionary:
                raise ValueError('Could not flatten dictionary: key %s is not unique.' % i)
            flat_dictionary[i] = v
        else:
            flat_i = flatten(i)
            flat_v = flatten(v)
            if len(flat_i) != len(flat_v):
                raise ValueError('Could not flatten dictionary. Key had %d elements, but value had %d elements. Key: %s, value: %s.' % (
                 len(flat_i), len(flat_v), flat_i, flat_v))
            for new_i, new_v in zip(flat_i, flat_v):
                if new_i in flat_dictionary:
                    raise ValueError('Could not flatten dictionary: key %s is not unique.' % new_i)
...
def pack_sequence_as(structure, flat_sequence):
  ...
  """
    ...
    if len(flat_structure) != len(flat_sequence):
        raise ValueError('Could not pack sequence. Structure had %d elements, but flat_sequence had %d elements.  Structure: %s, flat_sequence: %s.' % (
    ...
    if is_sequence(shallow_tree):
        ...
        if check_types:
            if not isinstance(input_tree, type(shallow_tree)):
                raise TypeError("The two structures don't have the same sequence type. Input structure has type %s, while shallow structure has type %s." % (
                 type(input_tree), type(shallow_tree)))
        if len(input_tree) != len(shallow_tree):
            raise ValueError("The two structures don't have the same sequence length. Input structure has length %s, while shallow structure has length %s." % (
             len(input_tree), len(shallow_tree)))
        for shallow_branch, input_branch in zip(shallow_tree, input_tree):
            assert_shallow_structure(shallow_branch, input_branch, check_types=check_types)
...
def map_structure_up_to(shallow_tree, func, *inputs):
  ...
  """
    ...
    all_flattened_up_to = [flatten_up_to(shallow_tree, input_tree) for input_tree in inputs]
