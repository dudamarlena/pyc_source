...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/object_detection/meta_architectures/faster_rcnn_meta_arch.py
# Compiled at: 2018-06-15 01:39:54
# Size of source mod 2**32: 73790 bytes
...
class FasterRCNNMetaArch(model.DetectionModel):
    ...
    """
        ...
        self._proposal_target_assigner = target_assigner.create_target_assigner('FasterRCNN', 'proposal')
        self._detector_target_assigner = target_assigner.create_target_assigner('FasterRCNN', 'detection', unmatched_cls_target=unmatched_cls_target)
        ...
        self._first_stage_box_predictor_arg_scope = first_stage_box_predictor_arg_scope
        ...
        self._first_stage_sampler = sampler.BalancedPositiveNegativeSampler(positive_fraction=first_stage_positive_balance_fraction)
        self._first_stage_box_predictor = box_predictor.ConvolutionalBoxPredictor(self._is_training, num_classes=1, conv_hyperparams=self._first_stage_box_predictor_arg_scope, min_depth=0, max_depth=0, num_layers_before_predictor=0, use_dropout=False, dropout_keep_prob=1.0, kernel_size=1, box_code_size=self._box_coder.code_size)
        ...
        self._first_stage_localization_loss = losses.WeightedSmoothL1LocalizationLoss(anchorwise_output=True)
        self._first_stage_objectness_loss = losses.WeightedSoftmaxClassificationLoss(anchorwise_output=True)
        ...
        self._second_stage_sampler = sampler.BalancedPositiveNegativeSampler(positive_fraction=second_stage_balance_fraction)
        ...
        self._second_stage_classification_loss = losses.WeightedSoftmaxClassificationLoss(anchorwise_output=True)
    ...
    """
        ...
        if not self._first_stage_only:
            prediction_dict.update(self._predict_second_stage(rpn_box_encodings, rpn_objectness_predictions_with_background, rpn_features_to_crop, anchors, image_shape))
    ...
    """
        ...
        prediction_dict = {'refined_box_encodings': refined_box_encodings, 
    ...
    """
        ...
        with slim.arg_scope(self._first_stage_box_predictor_arg_scope):
            ...
            rpn_box_predictor_features = slim.conv2d(rpn_features_to_crop, self._first_stage_box_predictor_depth, kernel_size=[
             kernel_size, kernel_size], rate=self._first_stage_atrous_rate, activation_fn=tf.nn.relu6)
        return (
    ...
    """
        ...
        box_predictions = self._first_stage_box_predictor.predict(rpn_box_predictor_features, num_anchors_per_location[0], scope=self.first_stage_box_predictor_scope)
    ...
    """
        ...
        def _batch_gather_kept_indices(predictions_tensor):
            return tf.map_fn(partial(tf.gather, indices=keep_indices), elems=predictions_tensor, dtype=tf.float32, parallel_iterations=self._parallel_iterations, back_prop=True)
    ...
    """
        with tf.name_scope('FirstStagePostprocessor'):
            ...
            if self._first_stage_only:
                proposal_boxes, proposal_scores, num_proposals = self._postprocess_rpn(prediction_dict['rpn_box_encodings'], prediction_dict['rpn_objectness_predictions_with_background'], prediction_dict['anchors'], image_shape)
                return {'detection_boxes': proposal_boxes, 
                 'detection_scores': proposal_scores, 
                 'num_detections': num_proposals}
        with tf.name_scope('SecondStagePostprocessor'):
            mask_predictions = prediction_dict.get(box_predictor.MASK_PREDICTIONS)
            detections_dict = self._postprocess_box_classifier(prediction_dict['refined_box_encodings'], prediction_dict['class_predictions_with_background'], prediction_dict['proposal_boxes'], prediction_dict['num_proposals'], image_shape, mask_predictions=mask_predictions)
    ...
    """
        ...
        if self._is_training:
            ...
            if not self._hard_example_miner:
                groundtruth_boxlists, groundtruth_classes_with_background_list = self._format_groundtruth_data(image_shape)
                proposal_boxes, proposal_scores, num_proposals = self._unpad_proposals_and_sample_box_classifier_batch(proposal_boxes, proposal_scores, num_proposals, groundtruth_boxlists, groundtruth_classes_with_background_list)
        proposal_boxes_reshaped = tf.reshape(proposal_boxes, [-1, 4])
        normalized_proposal_boxes_reshaped = box_list_ops.to_normalized_coordinates(box_list.BoxList(proposal_boxes_reshaped), image_shape[1], image_shape[2], check_range=False).get()
    ...
    def _unpad_proposals_and_sample_box_classifier_batch(self, proposal_boxes, proposal_scores, num_proposals, groundtruth_boxlists, groundtruth_classes_with_background_list):
    ...
    """
        ...
        for single_image_proposal_boxes, single_image_proposal_scores, single_image_num_proposals, single_image_groundtruth_boxlist, single_image_groundtruth_classes_with_background in zip(tf.unstack(proposal_boxes), tf.unstack(proposal_scores), tf.unstack(num_proposals), groundtruth_boxlists, groundtruth_classes_with_background_list):
            ...
            sliced_static_shape = tf.TensorShape([tf.Dimension(None),
             static_shape.dims[(-1)]])
            single_image_proposal_boxes = tf.slice(single_image_proposal_boxes, [
             0, 0], [
            ...
            sampled_boxlist = self._sample_box_classifier_minibatch(single_image_boxlist, single_image_groundtruth_boxlist, single_image_groundtruth_classes_with_background)
    ...
    """
        ...
        groundtruth_classes_with_background_list = [tf.to_float(tf.pad(one_hot_encoding, [[0, 0], [1, 0]], mode='CONSTANT')) for one_hot_encoding in self.groundtruth_lists(fields.BoxListFields.classes)]
    ...
    """
        ...
        sampled_indices = self._second_stage_sampler.subsample(tf.cast(cls_weights, tf.bool), self._second_stage_batch_size, positive_indicator)
    ...
    """
        ...
        cropped_regions = tf.image.crop_and_resize(features_to_crop, self._flatten_first_two_dimensions(proposal_boxes_normalized), get_box_inds(proposal_boxes_normalized), (
         self._initial_crop_size, self._initial_crop_size))
        return slim.max_pool2d(cropped_regions, [
         self._maxpool_kernel_size, self._maxpool_kernel_size], stride=self._maxpool_stride)
    ...
    """
        ...
        class_predictions_with_background_batch = self._second_stage_score_conversion_fn(class_predictions_with_background_batch)
        class_predictions_batch = tf.reshape(tf.slice(class_predictions_with_background_batch, [
         0, 0, 1], [-1, -1, -1]), [
        ...
        nmsed_boxes, nmsed_scores, nmsed_classes, nmsed_masks, num_detections = self._second_stage_nms_fn(refined_decoded_boxes_batch, class_predictions_batch, clip_window=clip_window, change_coordinate_frame=True, num_valid_boxes=num_proposals, masks=mask_predictions_batch)
        detections = {'detection_boxes': nmsed_boxes,  'detection_scores': nmsed_scores, 
         'detection_classes': nmsed_classes, 
         'num_detections': tf.to_float(num_detections)}
        if nmsed_masks is not None:
            detections['detection_masks'] = nmsed_masks
        if mask_predictions is not None:
            detections['detection_masks'] = tf.to_float(tf.greater_equal(detections['detection_masks'], mask_threshold))
        return detections
    ...
    """
        with tf.name_scope(scope, 'Loss', prediction_dict.values()):
            ...
            loss_dict = self._loss_rpn(prediction_dict['rpn_box_encodings'], prediction_dict['rpn_objectness_predictions_with_background'], prediction_dict['anchors'], groundtruth_boxlists, groundtruth_classes_with_background_list)
            if not self._first_stage_only:
                loss_dict.update(self._loss_box_classifier(prediction_dict['refined_box_encodings'], prediction_dict['class_predictions_with_background'], prediction_dict['proposal_boxes'], prediction_dict['num_proposals'], groundtruth_boxlists, groundtruth_classes_with_background_list))
        return loss_dict
    ...
    """
        with tf.name_scope('RPNLoss'):
            ...
            batch_sampled_indices = tf.to_float(tf.map_fn(_minibatch_subsample_fn, [
             batch_cls_targets, batch_cls_weights], dtype=tf.bool, parallel_iterations=self._parallel_iterations, back_prop=True))
            ...
            localization_losses = self._first_stage_localization_loss(rpn_box_encodings, batch_reg_targets, weights=sampled_reg_indices)
            objectness_losses = self._first_stage_objectness_loss(rpn_objectness_predictions_with_background, batch_one_hot_targets, weights=batch_sampled_indices)
            ...
            loss_dict = {'first_stage_localization_loss': self._first_stage_loc_loss_weight * localization_loss, 
             'first_stage_objectness_loss': self._first_stage_obj_loss_weight * objectness_loss}
        return loss_dict
    ...
    """
        with tf.name_scope('BoxClassifierLoss'):
            ...
            num_proposals_or_one = tf.to_float(tf.expand_dims(tf.maximum(num_proposals, tf.ones_like(num_proposals)), 1))
            normalizer = tf.tile(num_proposals_or_one, [
             1, self.max_num_proposals]) * batch_size
            batch_cls_targets_with_background, batch_cls_weights, batch_reg_targets, batch_reg_weights, _ = target_assigner.batch_assign_targets(self._detector_target_assigner, proposal_boxlists, groundtruth_boxlists, groundtruth_classes_with_background_list)
            ...
            refined_box_encodings_masked_by_class_targets = tf.boolean_mask(refined_box_encodings_with_background, tf.greater(flat_cls_targets_with_background, 0))
            reshaped_refined_box_encodings = tf.reshape(refined_box_encodings_masked_by_class_targets, [
             batch_size, -1, 4])
            second_stage_loc_losses = self._second_stage_localization_loss(reshaped_refined_box_encodings, batch_reg_targets, weights=batch_reg_weights) / normalizer
            second_stage_cls_losses = self._second_stage_classification_loss(class_predictions_with_background, batch_cls_targets_with_background, weights=batch_cls_weights) / normalizer
            ...
            loss_dict = {'second_stage_localization_loss': self._second_stage_loc_loss_weight * second_stage_loc_loss, 
             'second_stage_classification_loss': self._second_stage_cls_loss_weight * second_stage_cls_loss}
        return loss_dict
    ...
    """
        for proposal_boxlist, single_image_loc_loss, single_image_cls_loss, single_image_num_proposals in zip(proposal_boxlists, tf.unstack(second_stage_loc_losses), tf.unstack(second_stage_cls_losses), tf.unstack(num_proposals)):
            ...
            single_image_cls_loss = tf.slice(single_image_cls_loss, [
             0], [single_image_num_proposals])
            return self._hard_example_miner(location_losses=tf.expand_dims(single_image_loc_loss, 0), cls_losses=tf.expand_dims(single_image_cls_loss, 0), decoded_boxlist_list=[
             proposal_boxlist])
    ...
    """
        ...
        feature_extractor_variables = tf.contrib.framework.filter_variables(variables_to_restore, include_patterns=[
