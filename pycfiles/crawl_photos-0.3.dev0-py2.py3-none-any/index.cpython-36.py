...
# Embedded file name: C:\Users\xh\Documents\github-xh\crawl_image\crawl_image\index.py
# Compiled at: 2019-06-13 02:23:59
...
import time, os, threading, chardet, datetime
...
def run_multi_by_url(url_list, img_save_path=IMG_SAVE_PATH, count_thread=10, do_last_url_file_name=False):
    ...
    for i in range(count_thread):
        ...
        if i == last:
            ...
            threads.append(threading.Thread(target=thread_for_run_multi_by_url, args=(
             url_list[start:start + count_url_per_thread], folder, start, do_last_url_file_name)))
...
class ImgCrawl:
    ...
    def run_multi(self, folder, host, img_list):
        ...
        for index, img in enumerate(img_list):
            threads.append(threading.Thread(target=run, args=(folder, index, host, img, img_suffix(img.get('src'), self.img_crawl_model_.default_img_suffix))))
