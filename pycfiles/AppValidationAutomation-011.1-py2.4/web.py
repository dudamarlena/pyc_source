...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/App/Validation/Automation/web.py
# Compiled at: 2012-05-02 10:31:22
import sys, re, logging, mechanize
...
class Web:
    ...
    def validate_url(self, url):
        ...
        if 'COMMON.USER' in self.config and 'COMMON.PASSWORD' in self.config:
            if self.config['COMMON.USER'] and self.config['COMMON.PASSWORD']:
                ...
                if 'COMMON.USER' in self.config and 'COMMON.PASSWORD' in self.config:
                    if self.config['COMMON.USER'] and self.config['COMMON.PASSWORD']:
                        ...
                        if ret_value is True:
                            ...
                            match = re.search('(strWebSrvrName="(?P<webs>.+?)";)', html_content)
                            web_server = match.group('webs')
                            match = re.search('(strAppSrvrName="(?P<apps>.+?)";)', html_content)
                            app_server = match.group('apps')
                            match = re.search('(strKSAppSrvrName="(?P<a_apps>.+?)";)', html_content)
    ...
    def __login(self):
        ...
        if 'COMMON.SITE' in self.config and 'COMMON.ZONE' in self.config:
            try:
                ...
                mechanize.Item(zone1, {'contents': 'lee10', 'value': 'lee10'})
                br.form['zone'] = [
                 'lee10']
        ...
        if re.search('Password\\s+has\\s+expired', html_content):
            self.data['WEB_MSG'] = 'Password has Expired'
            self.data['BROWSER_STATE'] = br
            return False
        elif re.search('Authentication\\s+Failure', html_content):
            self.data['WEB_MSG'] = 'Authentication Failure!'
            return False
        elif re.search('launchMenu\\((.*)\\)\\;', html_content):
            string = re.search('launchMenu\\((.*)\\)\\;', html_content)
            request_string = string.group(1)
            (web_server, web_server_ip, web_port, app_server, user, menu_tokens, auto_tokens, site, zone, slif_flag) = re.split('\\W+,\\W+', request_string)
            web_server = re.sub("^\\'", '', web_server)
            slif_flag = re.sub("\\'$", '', slif_flag)
