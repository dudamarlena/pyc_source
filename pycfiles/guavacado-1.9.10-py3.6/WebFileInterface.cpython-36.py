...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/guavacado/WebFileInterface.py
# Compiled at: 2019-06-30 15:40:23
...
class WebFileInterface(object):
    ...
    def get_dir_page(self, path):
        if path[(-1)] != '/':
            ...
            data = '<html><head><title>Index of {path}</title></head><body><h1>Index of {path}</h1><table><tr><th valign="top"></th><th>Name</th><th>Last modified</th><th>Size</th></tr><tr><th colspan="5"><hr></th></tr><tr><td valign="top"></td><td><a href="..">Parent Directory</a></td><td>&nbsp;</td><td align="right">  - </td></tr>'.format(path=('/' + path))
            for filename in os.listdir(path):
                ...
                size_str = str(size)
                if size > 1024:
                    size_str = str(size / 1024) + 'K'
                if size > 1048576:
                    size_str = str(size / 1048576) + 'M'
                if size > 1073741824:
                    size_str = str(size / 1073741824) + 'G'
                if size > 1099511627776:
                    size_str = str(size / 1099511627776) + 'T'
                data = data + '<tr><td valign="top"></td><td><a href="{filename}">{filename}</a></td><td align="right">{lastmodified}</td><td align="right">{size}</td></tr>'.format(filename=filename,
                  lastmodified=(datetime.fromtimestamp(filestat.st_mtime)),
                  size=size_str)

            data = data + '<tr><th colspan="5"><hr></th></tr></table><address>{address}</address></body></html>'.format(address=(self.get_address_string()))
            return data
