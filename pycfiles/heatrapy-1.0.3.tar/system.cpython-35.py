...
# [GCC 8.4.0]
# Embedded file name: /home/daniel/Dropbox/Programming/heatrapy/heatrapy/objects/system.py
# Compiled at: 2019-02-21 06:54:10
...
class system_objects:
    __doc__ = 'system_objects class\n\n    This class creates a system of thermal objects, establishes contact between\n    them and computes the respective thermal processes.\n\n    '

    def __init__(self, number_objects=2, materials=('Cu', 'Cu'), objects_length=(10, 10), amb_temperature=293, dx=0.01, dt=0.1, file_name='data', initial_state=False, boundaries=((2, 0), (3, 0))):
        ...
        for i in range(number_objects):
            if i not in [l[0] for l in boundaries] or (i, 0) in boundaries:
                ...
                heat_save = True
            self.objects.append(object(amb_temperature, materials=(
             materials[i],), borders=(
             1, objects_length[i] + 1), materials_order=(0, ), dx=dx, dt=dt, file_name=file_name + '_' + str(i) + '.txt', boundaries=(0,
                                                                                                                                      0), Q=[], Q0=[], initial_state=initial_state, heat_save=heat_save))
        ...
        for i in boundaries:
            if i[1] != 0:
                for j in range(len(self.objects[i[0]].temperature)):
                    self.objects[i[0]].temperature[j] = [
                     i[1], i[1]]
                ...
                if cond1 or (object_number, 0) in self.boundaries:
                    for i in range(1, obj.num_points - 1):
                        if obj.state[i] is True:
                            ...
                            obj.rho[i] = obj.materials[ind].rhoa(obj.temperature[i][0])
                            obj.Cp[i] = obj.materials[ind].cpa(obj.temperature[i][0])
                            obj.k[i] = obj.materials[ind].ka(obj.temperature[i][0])
                        if obj.state[i] is False:
                            ind = obj.materials_index[i]
                            obj.rho[i] = obj.materials[ind].rho0(obj.temperature[i][0])
                            obj.Cp[i] = obj.materials[ind].cp0(obj.temperature[i][0])
                            obj.k[i] = obj.materials[ind].k0(obj.temperature[i][0])
...
class single_object(object):
    ...
    def __init__(self, amb_temperature, materials=('Cu', ), borders=(1, 11), materials_order=(0, ), dx=0.01, dt=0.1, file_name='data.txt', boundaries=(0, 0), Q=[], Q0=[], heat_points=(1, -2), initial_state=False, h_left=50000.0, h_right=50000.0):
        ...
        for i in range(len(materials)):
            tadi = os.path.dirname(os.path.realpath(__file__)) + '/../database/' + materials[i] + '/' + 'tadi.txt'
            tadd = os.path.dirname(os.path.realpath(__file__)) + '/../database/' + materials[i] + '/' + 'tadd.txt'
            cpa = os.path.dirname(os.path.realpath(__file__)) + '/../database/' + materials[i] + '/' + 'cpa.txt'
            cp0 = os.path.dirname(os.path.realpath(__file__)) + '/../database/' + materials[i] + '/' + 'cp0.txt'
            k0 = os.path.dirname(os.path.realpath(__file__)) + '/../database/' + materials[i] + '/' + 'k0.txt'
            ka = os.path.dirname(os.path.realpath(__file__)) + '/../database/' + materials[i] + '/' + 'ka.txt'
            rho0 = os.path.dirname(os.path.realpath(__file__)) + '/../database/' + materials[i] + '/' + 'rho0.txt'
            rhoa = os.path.dirname(os.path.realpath(__file__)) + '/../database/' + materials[i] + '/' + 'rhoa.txt'
        ...
        line = line + ',heat[' + str(self.heat_points[0]) + '](W)' + ',heat[' + str(self.heat_points[1]) + '](J)\n'
    ...
    def compute(self, timeInterval, write_interval, solver='explicit_k(x)', modeTemp=False, numFlag=0.5, modeTempPoint=1):
        """Computes the thermal process
        ...
        for j in range(nt):
            ...
            self.heatLeft = -self.dt * self.h_left * (self.temperature[self.heat_points[0]][1] - self.boundaries[0]) + self.heatLeft
            self.heatRight = self.dt * self.h_right * (self.temperature[self.heat_points[1]][1] - self.boundaries[1]) + self.heatRight
            nw = nw + 1
            if modeTemp is True:
