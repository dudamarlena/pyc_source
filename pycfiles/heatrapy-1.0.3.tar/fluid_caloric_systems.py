...
# [GCC 8.4.0]
# Embedded file name: heatrapy/systems/fluid_caloric_systems.py
# Compiled at: 2018-04-19 10:40:52
from __future__ import unicode_literals
from .. import objects
import time, numpy as np
from .. import fields

def fluid_active_regenerator(file_name, amb_temperature=298, fluid_length=160, MCM_length=50, right_reservoir_length=20, left_reservoir_length=20, MCM_material=((0.0, 'Gd'), ), fluid_material=b'water', left_reservoir_material=b'Cu', right_reservoir_material=b'Cu', freq=0.17, dt=0.1, dx=0.001, stop_criteria=1e-06, solver=b'implicit_k(x)', min_cycle_number=1, max_cycle_number=5000, cycle_points=25, note=None, boundaries=(
 (2, 298), (3, 0)), mode=b'heat_pump', version=None, leftHEXpositions=10, rightHEXpositions=10, starting_field=b'applied', temperature_sensor=(3, -3), field_removal_steps=1, field_applied_steps=1, field_removal_mode=b'accelerated_left', field_applied_mode=b'accelerated_right', applied_static_field_time_ratio=(0.0, 0.0), removed_static_field_time_ratio=(0.0, 0.0), h_mcm_fluid=1, h_leftreservoir_fluid=1, h_rightreservoir_fluid=1, mcm_discontinuity=b'default', type_study=b'no_load', stroke=0.02, mod_freq=b'default'):
    ...
    cond24 = starting_field == b'applied' or starting_field == b'removal'
    ...
    cond35 = type_study == b'no_load' or type_study == b'fixed_temperature_span'
    ...
    if starting_field == b'removal':
        ...
        initial_state = False
    cycle_number = 0
    AMR = objects.system_objects(number_objects=4, materials=(
     fluid_material,
     MCM_material[0][1],
     left_reservoir_material,
     right_reservoir_material), objects_length=(
     fluid_length, MCM_length,
     left_reservoir_length,
     right_reservoir_length), amb_temperature=amb_temperature, dx=dx, dt=dt, file_name=file_name, initial_state=initial_state, boundaries=boundaries)
    k = 1
    ...
    if mcm_discontinuity != b'default':
        from .. import mats
        import os
        tadi = os.path.dirname(os.path.realpath(__file__)) + b'/../database/vacuum/tadi.txt'
        tadd = os.path.dirname(os.path.realpath(__file__)) + b'/../database/vacuum/tadd.txt'
        cpa = os.path.dirname(os.path.realpath(__file__)) + b'/../database/vacuum/cpa.txt'
        cp0 = os.path.dirname(os.path.realpath(__file__)) + b'/../database/vacuum/cp0.txt'
        k0 = os.path.dirname(os.path.realpath(__file__)) + b'/../database/vacuum/k0.txt'
        ka = os.path.dirname(os.path.realpath(__file__)) + b'/../database/vacuum/ka.txt'
        rho0 = os.path.dirname(os.path.realpath(__file__)) + b'/../database/vacuum/rho0.txt'
        rhoa = os.path.dirname(os.path.realpath(__file__)) + b'/../database/vacuum/rhoa.txt'
        AMR.objects[1].materials.append(mats.calmatpro(tadi, tadd, cpa, cp0, k0, ka, rho0, rhoa))
        j = 1
        for i in range(1, len(AMR.objects[1].temperature) - 1):
            i_cond1 = j * len(AMR.objects[1].temperature) / (mcm_discontinuity[0] + 1) + int(mcm_discontinuity[1] / (2 * dx))
            i_cond2 = j * len(AMR.objects[1].temperature) / (mcm_discontinuity[0] + 1) - int(mcm_discontinuity[1] / (2 * dx))
    ...
    time_step = 1 / (2.0 * freq) / steps
    velocity = 2 * freq * stroke
    if int(time_step / dt) == 0:
        print b'dt or frequency too low'
    ...
    print b'Module: fluid_active_regenerator'
    if note is not None:
        print b''
        print b'Note:', note
    print b''
    print b'Mode:', mode
    print b'Fluid:', fluid_material + b' (' + str(fluid_length * dx) + b')'
    string = b' (' + str(MCM_length * dx) + b')'
    print b'MCM material:', MCM_material, string
    string = b' (' + str(left_reservoir_length * dx) + b')'
    print b'Left reservoir:', left_reservoir_material + string
    string = b' (' + str(right_reservoir_length * dx) + b')'
    print b'Right reservoir:', right_reservoir_material + string
    print b'Distance between MCM and left HEX:', leftHEXpositions * dx
    print b'Distance between MCM and right HEX:', rightHEXpositions * dx
    print b'dx (m):', dx
    print b'dt (s):', dt
    print b'Frequency (Hz):', freq
    if mcm_discontinuity == b'default':
        print b'Discontinuity: None'
    else:
        print b'Discontinuity:', mcm_discontinuity
    print b'MCM - fluid heat transfer coefficient:', h_mcm_fluid
    val = h_leftreservoir_fluid
    print b'Left reservoir - fluid heat transfer coefficient:', val
    val = h_rightreservoir_fluid
    print b'Right reservoir - fluid heat transfer coefficient:', val
    val = applied_static_field_time_ratio
    print b'Applied field static time ratios:', val
    val = removed_static_field_time_ratio
    print b'Removed field static time ratios:', val
    ...
    if type_study == b'no_load':
        ...
        if starting_field == b'applied':
            ...
            while condition:
                ...
                j_cond = 0
                j_inc = 1
                com = [b'constant_left', b'accelerated_left',
                 b'decelerated_left']
                ...
                for n in range(steps):
                    contacts = set()
                    init_pos = fluid_length / 2 - (left_reservoir_length + leftHEXpositions + MCM_length + rightHEXpositions + right_reservoir_length) / 2 - steps / 2
                    for i in range(left_reservoir_length):
                        contacts.add(((0, i + n + init_pos), (2, i + 1),
                         h_leftreservoir_fluid))

                    if mcm_discontinuity == b'default':
                        init_pos = left_reservoir_length + leftHEXpositions + fluid_length / 2 - (left_reservoir_length + leftHEXpositions + MCM_length + rightHEXpositions + right_reservoir_length) / 2 - steps / 2
                        ...
                        init_pos = left_reservoir_length + leftHEXpositions + fluid_length / 2 - (left_reservoir_length + leftHEXpositions + MCM_length + rightHEXpositions + right_reservoir_length) / 2 - steps / 2
                        for i in range(MCM_length):
                            cond1 = p * len(AMR.objects[1].temperature) / (mcm_discontinuity[0] + 1) + int(mcm_discontinuity[1] / (2 * dx))
                            cond2 = p * len(AMR.objects[1].temperature) / (mcm_discontinuity[0] + 1) - int(mcm_discontinuity[1] / (2 * dx))
                        ...
                        init_pos = left_reservoir_length + leftHEXpositions + rightHEXpositions + MCM_length + fluid_length / 2 - (left_reservoir_length + leftHEXpositions + MCM_length + rightHEXpositions + right_reservoir_length) / 2 - steps / 2
                        for i in range(right_reservoir_length):
                            contacts.add(((0, i + init_pos + n), (3, i + 1),
                             h_rightreservoir_fluid))

                    AMR.contacts = contacts
                    cond1 = 1 / (2.0 * freq) - applied_static_field_time_ratio[1] / freq
                    if n * time_step < applied_static_field_time_ratio[0] / freq:
                        AMR.compute(time_step, write_interval, solver=solver)
                        ...
                        if j == j_cond:
                            ...
                            time_interval = fields.operating_mode(md, ap1, ap2, ms, freq, j)
                        while cond:
                            if time_interval - time_passed > time_step:
                                val = field_applied_steps
                                if time_passed == 0.0 and j < val:
                                    v1 = field_applied_mode
                                    v2 = applied_static_field_time_ratio[0]
                                    v3 = applied_static_field_time_ratio[1]
                                    v4 = field_applied_steps
                                    val = fields.operating_mode(v1, v2, v3, v4, freq, j)
                                    time_interval = val
                                    first = j * MCM_length / field_applied_steps + 1
                                    second = (j + 1) * MCM_length / field_applied_steps + 1
                                    AMR.objects[1].activate(first, second)
                                    j = j + j_inc
                                    delta_t = time_step - previous_time
                                else:
                                    delta_t = time_step
                                AMR.compute(delta_t, write_interval, solver=solver)
                                time_passed = time_passed + delta_t
                                cond = False
                                flag = True
                            else:
                                val = field_applied_steps
                                if time_passed == 0.0 and j < val and flag:
                                    v1 = field_applied_mode
                                    v2 = applied_static_field_time_ratio[0]
                                    v3 = applied_static_field_time_ratio[1]
                                    v4 = field_applied_steps
                                ...
                                AMR.compute(delta_t, write_interval, solver=solver)
                                com = [
                                 b'constant_left', b'accelerated_left',
                                 b'decelerated_left']
                ...
                for n in range(steps):
                    ...
                    AMR.contacts = contacts
                    cond1 = 1 / (2.0 * freq) - removed_static_field_time_ratio[1] / freq
                        ...
                        while cond:
                            if time_interval - time_passed > time_step:
                                ...
                                if time_passed == 0.0 and j < val:
                                    ...
                                    val = fields.operating_mode(v1, v2, v3, v4, freq, j)
                                    time_interval = val
                                    first = j * MCM_length / field_removal_steps + 1
                                    second = (j + 1) * MCM_length / field_removal_steps + 1
                                ...
                                if time_passed == 0.0 and j < ds and flag:
                                    v1 = field_removal_mode
                                    v2 = removed_static_field_time_ratio[0]
                                    v3 = removed_static_field_time_ratio[1]
                                    v4 = field_removal_steps
                ...
                if value1 == value2:
                    ...
                    vl1 = left_reservoir_length / 2
                    vl2 = right_reservoir_length / 2
                    cond2 = abs(abs(AMR.objects[2].temperature[vl1][0] - AMR.objects[3].temperature[vl2][0]) - abs(value1 - value2)) / abs(value1 - value2)
                    cond2 = cond2 > stop_criteria
                    cond3 = cycle_number < max_cycle_number - 1
                    condition = (cond1 or cond2) and cond3
                cycle_number = cycle_number + 1

        elif starting_field == b'removal':
            condition = True
            while condition:
                vl1 = left_reservoir_length / 2
                val = AMR.objects[2].temperature[vl1][0]
                ...
                for n in range(steps):
                    ...
                    if mcm_discontinuity == b'default':
                        ...
                        init_pos = left_reservoir_length + leftHEXpositions + fluid_length / 2 - (left_reservoir_length + leftHEXpositions + MCM_length + rightHEXpositions + right_reservoir_length) / 2 + steps / 2
                        for i in range(MCM_length):
                            val1 = p * len(AMR.objects[1].temperature) / (mcm_discontinuity[0] + 1) + int(mcm_discontinuity[1] / (2 * dx))
                            val2 = p * len(AMR.objects[1].temperature) / (mcm_discontinuity[0] + 1) - int(mcm_discontinuity[1] / (2 * dx))
                    ...
                    AMR.contacts = contacts
                    val = 1 / (2.0 * freq) - removed_static_field_time_ratio[1] / freq
                    if n * time_step < removed_static_field_time_ratio[0] / freq:
                        AMR.compute(time_step, write_interval, solver=solver)
            ...
            print b'incorrect starting_field'
    if type_study == b'fixed_temperature_span':
        if starting_field == b'applied':
            ...
            print b'incorrect starting_field'
    cond1 = starting_field == b'applied'
    cond2 = starting_field == b'removal'
    if cond1 or cond2:
        endTime = time.time()
        ...
        print b'------------------------------------------------------'
        print b''
        print b'Number of cycles:', cycle_number
        if mode == b'refrigerator':
            if type_study == b'no_load':
                v1 = left_reservoir_length / 2
                v2 = right_reservoir_length / 2
                val = abs(AMR.objects[2].temperature[v1][0] - AMR.objects[3].temperature[v2][0])
                temperature_span = val
                error = abs(abs(AMR.objects[2].temperature[vl1][0] - AMR.objects[3].temperature[vl2][0]) - abs(value1 - value2)) / abs(value1 - value2)
                print b'Final cycle error:', error
            ...
            if type_study == b'fixed_temperature_span':
                ...
                print b'Final cycle error:', error
                print b'Cooling power (W/m2):', cooling_power
                print b'Heating power (W/m2):', heating_power
                print b'Working power (W/m2)', working_power
                print b'COP:', COP
        if mode == b'heat_pump':
            if type_study == b'no_load':
                v1 = left_reservoir_length / 2
        ...
        print b'Final time (s):', AMR.objects[0].time_passed
        ...
        print b'simulation not complete'
    return