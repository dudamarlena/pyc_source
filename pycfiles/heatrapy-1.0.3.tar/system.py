...
# [GCC 8.4.0]
# Embedded file name: heatrapy/objects/system.py
# Compiled at: 2018-04-20 06:41:44
...
class system_objects:
    ...
    def __init__(self, number_objects=2, materials=('Cu', 'Cu'), objects_length=(10, 10), amb_temperature=293, dx=0.01, dt=0.1, file_name=b'data', initial_state=False, boundaries=(
 (2, 0), (3, 0))):
        ...
        for i in range(number_objects):
            if i not in [ l[0] for l in boundaries ] or (i, 0) in boundaries:
                ...
                heat_save = True
            self.objects.append(object.object(amb_temperature, materials=(
             materials[i],), borders=(
             1, objects_length[i] + 1), materials_order=(0, ), dx=dx, dt=dt, file_name=file_name + b'_' + str(i) + b'.txt', boundaries=(0,
    ...
    def compute(self, timeInterval, write_interval, solver=b'implicit_k'):
...
class single_object(object.object):
    ...
    def __init__(self, amb_temperature, materials=('Cu', ), borders=(1, 11), materials_order=(0, ), dx=0.01, dt=0.1, file_name=b'data.txt', boundaries=(0, 0), Q=[], Q0=[], heat_points=(1, -2), initial_state=False, h_left=50000.0, h_right=50000.0):
        """Initializes the object.
        ...
        line = line + b',heat[' + str(self.heat_points[0]) + b'](W)' + b',heat[' + str(self.heat_points[1]) + b'](J)\n'
    ...
    def compute(self, timeInterval, write_interval, solver=b'explicit_k(x)', modeTemp=False, numFlag=0.5, modeTempPoint=1):
        """Computes the thermal process
        ...
        for j in range(nt):
            ...
            if nw == write_interval or j == 0 or j == nt - 1:
                line = b'%f,' % self.time_passed
                for i in self.temperature:
                    new_line = b'%f,' % i[1]
                    line = line + new_line

                new_line = b'%f,' % self.heatLeft
                line = line + new_line
                new_line = b'%f' % self.heatRight
                line = line + new_line + b'\n'
                f = open(self.file_name, b'a')
                f.write(line)
                f.close()
            if nw == write_interval:
