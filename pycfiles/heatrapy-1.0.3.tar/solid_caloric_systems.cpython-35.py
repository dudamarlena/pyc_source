...
# [GCC 8.4.0]
# Embedded file name: /home/daniel/Dropbox/Programming/heatrapy/heatrapy/systems/solid_caloric_systems.py
# Compiled at: 2019-02-18 08:50:12
...
def solid_active_regenerator(file_name, amb_temperature=293, left_thermalswitch_length=2, right_thermalswitch_length=2, MCM_length=20, right_reservoir_length=3, left_reservoir_length=3, MCM_material=((0.002, 'Gd'), ), left_thermalswitch_material='idealTS_hot', right_thermalswitch_material='idealTS_cold', left_reservoir_material='Cu', right_reservoir_material='Cu', freq=0.1, dt=0.01, dx=0.002, stop_criteria=0.005, solver='implicit_k(x)', min_cycle_number=30, max_cycle_number=31, field_removal_steps=3, field_applied_steps=1, field_removal_mode='accelerated_right', field_applied_mode='accelerated_left', cycle_points=25, boundaries=(293, 293), note=None, temperature_sensor='default', heat_points='default', mode='refrigerator', version=None, resting_time_hot='default', resting_time_cold='default', starting_field='applied', type_study='fixed_temperature_span', h_left=50000.0, h_right=50000.0, mod_freq='default'):
    ...
    print('')
    print('Mode:', mode)
    print('System:', left_reservoir_material + '/' + left_thermalswitch_material + '/MCM material/' + right_thermalswitch_material + '/' + right_reservoir_material)
    print('Dimensions (m):', str(dx * left_reservoir_length) + '/' + str(dx * left_thermalswitch_length) + '/' + str(dx * MCM_length) + '/' + str(dx * right_thermalswitch_length) + '/' + str(dx * right_reservoir_length))
    print('Number of points:', str(left_reservoir_length) + '/' + str(left_thermalswitch_length) + '/' + str(MCM_length) + '/' + str(right_thermalswitch_length) + '/' + str(right_reservoir_length))
    ...
    while (abs((value1 - value2) / value2) > stop_criteria or i < min_cycle_number or abs((value1 - value2) / value2) > stop_criteria2) and i < max_cycle_number:
        ...
        if starting_field == 'applied':
            com = [
             'constant_left', 'accelerated_left', 'decelerated_left']
    ...
    for o in range(1, len(lines)):
        ...
        final_temperature_right = (time_now - time_before) * float(lines[o].split(',')[sensor]) + final_temperature_right
    ...
    for o in range(1, len(lines)):
        ...
        final_temperature_left = (time_now - time_before) * float(lines[o].split(',')[sensor]) + final_temperature_left
        time_accumulated = time_accumulated + (time_now - time_before)
