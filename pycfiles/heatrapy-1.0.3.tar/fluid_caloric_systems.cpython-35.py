...
# [GCC 8.4.0]
# Embedded file name: /home/daniel/Dropbox/Programming/heatrapy/heatrapy/systems/fluid_caloric_systems.py
# Compiled at: 2019-02-21 08:27:45
# Size of source mod 2**32: 97517 bytes
...
def fluid_active_regenerator(file_name, amb_temperature=298, fluid_length=160, MCM_length=50, right_reservoir_length=20, left_reservoir_length=20, MCM_material=((0.0, 'Gd'), ), fluid_material='water', left_reservoir_material='Cu', right_reservoir_material='Cu', freq=0.17, dt=0.1, dx=0.001, stop_criteria=1e-06, solver='implicit_k(x)', min_cycle_number=1, max_cycle_number=5000, cycle_points=25, note=None, boundaries=((2, 298), (3, 0)), mode='heat_pump', version=None, leftHEXpositions=10, rightHEXpositions=10, starting_field='applied', temperature_sensor=(3, -3), field_removal_steps=1, field_applied_steps=1, field_removal_mode='accelerated_left', field_applied_mode='accelerated_right', applied_static_field_time_ratio=(0.0, 0.0), removed_static_field_time_ratio=(0.0, 0.0), h_mcm_fluid=1, h_leftreservoir_fluid=1, h_rightreservoir_fluid=1, mcm_discontinuity='default', type_study='no_load', stroke=0.02, mod_freq='default'):
    ...
    for i in range(len(MCM_material)):
        from .. import mats
        import os
        tadi = os.path.dirname(os.path.realpath(__file__)) + '/../database/' + MCM_material[i][1] + '/' + 'tadi.txt'
        tadd = os.path.dirname(os.path.realpath(__file__)) + '/../database/' + MCM_material[i][1] + '/' + 'tadd.txt'
        cpa = os.path.dirname(os.path.realpath(__file__)) + '/../database/' + MCM_material[i][1] + '/' + 'cpa.txt'
        cp0 = os.path.dirname(os.path.realpath(__file__)) + '/../database/' + MCM_material[i][1] + '/' + 'cp0.txt'
        k0 = os.path.dirname(os.path.realpath(__file__)) + '/../database/' + MCM_material[i][1] + '/' + 'k0.txt'
        ka = os.path.dirname(os.path.realpath(__file__)) + '/../database/' + MCM_material[i][1] + '/' + 'ka.txt'
        rho0 = os.path.dirname(os.path.realpath(__file__)) + '/../database/' + MCM_material[i][1] + '/' + 'rho0.txt'
        rhoa = os.path.dirname(os.path.realpath(__file__)) + '/../database/' + MCM_material[i][1] + '/' + 'rhoa.txt'
    ...
    if mcm_discontinuity != 'default':
        from .. import mats
        import os
        tadi = os.path.dirname(os.path.realpath(__file__)) + '/../database/vacuum/tadi.txt'
        tadd = os.path.dirname(os.path.realpath(__file__)) + '/../database/vacuum/tadd.txt'
        cpa = os.path.dirname(os.path.realpath(__file__)) + '/../database/vacuum/cpa.txt'
        cp0 = os.path.dirname(os.path.realpath(__file__)) + '/../database/vacuum/cp0.txt'
        k0 = os.path.dirname(os.path.realpath(__file__)) + '/../database/vacuum/k0.txt'
        ka = os.path.dirname(os.path.realpath(__file__)) + '/../database/vacuum/ka.txt'
        rho0 = os.path.dirname(os.path.realpath(__file__)) + '/../database/vacuum/rho0.txt'
        rhoa = os.path.dirname(os.path.realpath(__file__)) + '/../database/vacuum/rhoa.txt'
        AMR.objects[1].materials.append(mats.calmatpro(tadi, tadd, cpa, cp0, k0, ka, rho0, rhoa))
        j = 1
                        ...
                        if n * time_step < applied_static_field_time_ratio[0] / freq:
                            ...
                            while cond:
                                if time_interval - time_passed > time_step:
                                    ...
                                    AMR.compute(delta_t, write_interval, solver=solver)
                                    com = [
                                     'constant_left', 'accelerated_left',
                                     'decelerated_left']
