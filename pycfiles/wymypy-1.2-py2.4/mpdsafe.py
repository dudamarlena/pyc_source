...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/wymypy/libs/mpdsafe.py
# Compiled at: 2007-02-03 04:21:39
import mpdclient, os
    ...
    def display(self, s, format='<b>%(artist)s</b> - %(title)s'):
        ...
        if self.__dispTags:
            ...
            if format.find('(artist)') > 0 and artist.strip() != '':
                atLessOne = True
            elif format.find('(title)') > 0 and title.strip() != '':
                atLessOne = True
            elif format.find('(album)') > 0 and album.strip() != '':
                atLessOne = True
            elif format.find('(track)') > 0 and track.strip() != '':
                atLessOne = True
            if atLessOne:
