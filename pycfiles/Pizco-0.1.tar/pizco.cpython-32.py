...
# [GCC 8.4.0]
# Embedded file name: /Users/grecco/Documents/code/pizco/pizco/pizco.py
# Compiled at: 2012-11-20 03:09:51
"""
    pyzco
    ~~~~~
...
"""
import os, sys, uuid, hmac, json, pickle, weakref, hashlib, inspect, logging, threading, subprocess
...
if not DEFAULT_LAUNCHER:
    ...
    if sw('linux'):
        DEFAULT_LAUNCHER = 'xterm -e "{0[python]} {0[pizco]} {0[rep_endpoint]} {0[pub_endpoint]} -p {0[cwd]} {0[verbose]} '
    else:
        if sw('win32'):
            DEFAULT_LAUNCHER = 'cmd.exe /k "{0[python]} {0[pizco]} {0[rep_endpoint]} {0[pub_endpoint]} -p {0[cwd]} {0[verbose]} '
        elif sw('darwin'):
            DEFAULT_LAUNCHER = 'osascript -e \'tell application "Terminal"\'  -e \'do script "\\"{0[python]}\\" \\"{0[pizco]}\\" {0[rep_endpoint]} {0[pub_endpoint]} -p \\"{0[cwd]}\\" {0[verbose]}"\'  -e \'end tell\' '
...
class Protocol(object):
    ...
    def _format(self, key, serializer, sender, topic='', content='', msgid=None, just_header=False):
        ...
        parts = [
         (self.HEADER + '+' + sender + '+' + topic).encode('utf-8'),
...
class Server(Agent):
    ...
    def on_request(self, sender, topic, content, msgid):
        ...
        try:
            ...
            if isinstance(ret, futures.Future):
                ret.add_done_callback(lambda fut: self.publish('__future__', {'msgid': msgid,  'result': fut.result() if not fut.exception() else None, 
    ...
    def return_as_remote(self, attr):
        ...
        """
        return hasattr(attr, '__get__') or hasattr(attr, '__getitem__') or hasattr(attr, '__setitem__') or callable(attr) or hasattr(attr, 'connect') and hasattr(attr, 'disconnect') and hasattr(attr, 'emit')
    ...
    def inspect(self):
        ...
        """
        remotes = set([name for name, value in inspect.getmembers(self.served_object) if not name.startswith('_') and self.return_as_remote(value)])
        objects = set([name for name, value in inspect.getmembers(self.served_object) if not name.startswith('_') and self.force_as_object(value)])
...
class ProxyAgent(Agent):
    ...
    def __init__(self, remote_rep_endpoint):
        ...
        self._futures = {}
        self.subscribe(self.remote_rep_endpoint, '__future__', self.on_future_completed, self.remote_pub_endpoint)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-g', '--gui', action='store_true', help='Open a small window to display the server status.')
    parser.add_argument('-v', '--verbose', action='store_true', help='Print debug information to the console.')
    parser.add_argument('-p', '--path', type=str, help='Append this path to sys.path')
