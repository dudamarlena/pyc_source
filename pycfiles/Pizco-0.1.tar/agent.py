...
# Compiled at: 2012-11-08 06:38:06
"""
    pyzr
    ~~~~
...
"""
import os, sys, uuid, pickle, logging, threading, subprocess
from collections import defaultdict, namedtuple
import zmq
from zmq.eventloop import zmqstream, ioloop
logger = logging.getLogger(__name__)
logger.addHandler(logging.NullHandler())
PSMessage = namedtuple('PSMessage', 'action options')
...
class Agent(object):
    ...
    def _start(self, rep, pub, sub, in_callback=False):
        ...
        if not self.loop.running() and not in_callback:
            with self._lock:
                threading.Thread(target=self.loop.start, name=('ioloop-{}').format(id(self.loop))).start()
    ...
    def on_request(self, sender, msgid, content):
        ...
        if content == 'stop':
            cb = ioloop.DelayedCallback(self.stop, 10, self.loop)
            cb.start()
            return 'stopping'
        return content
...
class Server(Agent):
    ...
    def on_request(self, sender, msgid, content):
        ...
        Messages between proxy and server are handled using the PSMessage named tuple
        containing two attributes: `action` and `options`.
    ...
    @classmethod
    def serve_in_process(cls, served_cls, args, kwargs, rep_endpoint, pub_endpoint='tcp://127.0.0.1:0'):
        cwd = os.path.split(os.path.dirname(__file__))[0]
        subprocess.Popen(DEFAULT_LAUNCHER + [sys.executable, __file__, '-v', rep_endpoint, pub_endpoint], cwd=cwd)
        import time
        time.sleep(1)
        proxy = Proxy(rep_endpoint)
        proxy._proxy_agent.instantiate(served_cls, args, kwargs)
...
class ProxyAgent(Agent):
    ...
    def request_server(self, action, options):
        """Sends a request to the associated server using the PSMessage protocol.
        ...
        try:
            ret_action, ret_options = content
        except:
