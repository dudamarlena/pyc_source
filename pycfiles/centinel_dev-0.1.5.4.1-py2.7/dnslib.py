...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/centinel/primitives/dnslib.py
...
def lookup_domain(domain, nameservers=[], rtype='A', timeout=2):
    ...
    dns_exp = DNSQuery(domains=[domain], nameservers=nameservers, rtype=rtype, timeout=timeout)
    return dns_exp.lookup_domain(domain)


def lookup_domains(domains, nameservers=[], rtype='A', timeout=10):
    dns_exp = DNSQuery(domains=domains, nameservers=nameservers, rtype=rtype, timeout=timeout)
...
class DNSQuery:
    ...
    def __init__(self, domains=[], nameservers=[], rtype='A', timeout=10, max_threads=100):
