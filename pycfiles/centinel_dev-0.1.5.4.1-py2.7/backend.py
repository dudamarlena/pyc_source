...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/centinel/backend.py
# Compiled at: 2015-10-20 23:16:22
...
def get_meta(config, ip=''):
    url = '%s/%s/%s' % (config['server']['server_url'],
     'meta', ip)
    try:
        req = requests.get(url, proxies=config['proxy']['proxy'], verify=config['server']['verify'])
        req.raise_for_status()
        return req.json()
    except Exception as exp:
