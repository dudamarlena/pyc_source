...
# Embedded file name: build/bdist.linux-i686/egg/plemp/gui.py
# Compiled at: 2010-10-21 11:19:43
import os, gtk, gobject, dbus, dbus.service
...
class GUI(object):
    ...
    def waitForAuthentication(self, url):
        d = gtk.MessageDialog(self.window, gtk.DIALOG_DESTROY_WITH_PARENT, gtk.MESSAGE_INFO, gtk.BUTTONS_OK, 'You need to allow plemp to access your account at Flickr. Click the link below to do the authorization.\n\nClick OK when done authorizing.')
    ...
    def authorizationError(self, f):
        d = gtk.MessageDialog(self.window, gtk.DIALOG_DESTROY_WITH_PARENT, gtk.MESSAGE_ERROR, gtk.BUTTONS_OK, 'Authorization failed. Please retry.')
        d.run()
        d.destroy()
        reactor.stop()
    ...
    def finished(self, r):
        ...
        if errors:
            d = gtk.MessageDialog(self.window, gtk.DIALOG_DESTROY_WITH_PARENT, gtk.MESSAGE_ERROR, gtk.BUTTONS_OK, 'The following file(s) were not accepted by Flickr:\n\n' + ('\n').join([ '- %s' % f for f in errors ]))
            d.run()
            d.destroy()
        reactor.stop()
