...
class Agent(object):
    __doc__ = 'Monitor agent that will be constantly verifying if the URL is healthy\n    and updating the component.\n    '
...
class Decorator(object):
    __doc__ = "Defines the actions a user can configure to be executed when there's an incident."
...
class UpdateStatusDecorator(Decorator):
    __doc__ = 'Updates the component status when an incident happens.'
...
class CreateIncidentDecorator(Decorator):
    __doc__ = 'Creates an incident entry on cachet when an incident happens.'
...
class PushMetricsDecorator(Decorator):
    __doc__ = 'Updates the URL latency metric.'
