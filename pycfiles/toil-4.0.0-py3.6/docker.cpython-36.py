...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil/lib/docker.py
...
import docker, base64, requests, logging, os, sys
...
def dockerCheckOutput(*args, **kwargs):
    raise RuntimeError('dockerCheckOutput() using subprocess.check_output() has been removed, please switch to apiDockerCall().')


def dockerCall(*args, **kwargs):
    raise RuntimeError('dockerCall() using subprocess.check_output() has been removed, please switch to apiDockerCall().')


def subprocessDockerCall(*args, **kwargs):
    raise RuntimeError('subprocessDockerCall() has been removed, please switch to apiDockerCall().')


def apiDockerCall(job, image, parameters=None, deferParam=None, volumes=None, working_dir=None, containerName=None, entrypoint=None, detach=False, log_config=None, auto_remove=None, remove=False, user=None, environment=None, stdout=None, stderr=False, streamfile=None, timeout=31536000, **kwargs):
    """
    ...
    if parameters is None:
        ...
        if len(parameters) > 0:
            if type(parameters[0]) is list:
                if entrypoint is None:
                    entrypoint = [
                     '/bin/bash', '-c']
                chain_params = [' '.join(quote(arg) for arg in command) for command in parameters]
            ...
            if len(parameters) > 0 and type(parameters) is list:
    ...
    try:
        if detach is False:
            if stdout is None:
                stdout = True
            out = (client.containers.run)(image=image, command=command, 
             working_dir=working_dir, 
            ...
            if stdout or stderr:
                if log_config is None:
                    logger.warning('stdout or stderr specified, but log_config is not set.  Defaulting to "journald".')
...
def dockerKill(container_name, gentleKill=False, timeout=31536000):
    """
    ...
    try:
        ...
        logger.debug('Attempted to stop container, but container != exist: ', container_name)
    except requests.exceptions.HTTPError as e:
        logger.debug('Attempted to stop container, but server gave an error: ', container_name)
        raise create_api_error_from_http_exception(e)
...
def containerIsRunning(container_name, timeout=31536000):
    """
    ...
    try:
        ...
        logger.debug('Server error attempting to call container: ', container_name)
        raise create_api_error_from_http_exception(e)
...
def getContainerName(job):
    ...
    return '--'.join([str(job),
     base64.b64encode(os.urandom(9), b'-_').decode('utf-8')]).replace("'", '').replace('"', '').replace('_', '')