...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil/utils/toilStats.py
# Compiled at: 2020-04-03 17:41:24
# Size of source mod 2**32: 24259 bytes
"""
...
def initializeOptions(parser):
    parser.add_argument('jobStore', type=str, help=('The location of the job store used by the workflow for which statistics should be reported. ' + jobStoreLocatorHelp))
    parser.add_argument('--outputFile', dest='outputFile', default=None, help='File in which to write results')
    parser.add_argument('--raw', action='store_true', default=False, help='output the raw json data.')
    parser.add_argument('--pretty', '--human', action='store_true', default=False, help='if not raw, prettify the numbers to be human readable.')
    parser.add_argument('--categories', help='comma separated list from [time, clock, wait, memory]')
    parser.add_argument('--sortCategory', default='time', help='how to sort Job list. may be from [alpha, time, clock, wait, memory, count]. default=%(default)s')
    parser.add_argument('--sortField', default='med', help='how to sort Job list. may be from [min, med, ave, max, total]. default=%(default)s')
    parser.add_argument('--sortReverse', '--reverseSort', default=False, action='store_true',
    ...
    if options.jobStore == None:
        ...
        if options.sortCategory is not None:
            if options.sortCategory not in defaultCategories:
                if options.sortCategory not in extraSort:
...
def prettyTime(t, field=None):
    ...
    if t < 7200:
        m = floor(old_div(t, 60.0))
        s = t % 60
        return padStr('%dm%ds' % (m, s), field)
    if t < 90000:
        h = floor(t / 60.0 / 60.0)
        m = floor(old_div(t - h * 60.0 * 60.0, 60.0))
        s = t % 60
        ...
        if t < 604800:
            ...
            m = floor(old_div(t - d * 24.0 * 60.0 * 60.0 - h * 60.0 * 60.0, 60.0))
            s = t % 60
        ...
        h = floor((t - w * 7.0 * 24.0 * 60.0 * 60.0 - d * 24.0 * 60.0 * 60.0) / 60.0 / 60.0)
        m = floor(old_div(t - w * 7.0 * 24.0 * 60.0 * 60.0 - d * 24.0 * 60.0 * 60.0 - h * 60.0 * 60.0, 60.0))
        s = t % 60
...
def sortJobs(jobTypes, options):
    ...
    if options.sortCategory == 'time' or options.sortCategory == 'clock' or options.sortCategory == 'wait' or options.sortCategory == 'memory':
        return sorted(jobTypes,
          key=(lambda tag: getattr(tag, '%s_%s' % (
         sortField, options.sortCategory))),
          reverse=(options.sortReverse))
    if options.sortCategory == 'alpha':
        return sorted(jobTypes,
          key=(lambda tag: tag.name), reverse=(options.sortReverse))
    if options.sortCategory == 'count':
        ...
        if category in options.categories:
            for field in ('min', 'med', 'ave', 'max', 'total'):
                ...
                if category in ('time', 'clock', 'wait'):
                    s = reportTime(t, options, field=(cw.getWidth(category, field))).strip()
                else:
                    s = reportMemory(t, options, field=(cw.getWidth(category, field)),
                      isBytes=True).strip()
...
def buildElement(element, items, itemName):
    ...
    if len(itemTimes) == 0:
        ...
        itemWaits.append(0)
        itemMemory.append(0)
    element[itemName] = Expando(total_number=(float(len(items))),
...
def processData(config, stats):
    ...
    collatedStatsTag = Expando(total_run_time=(stats.total_time), total_clock=(stats.total_clock),
    ...
    createSummary(buildElement(collatedStatsTag, jobs, 'jobs'), getattr(stats, 'workers', []), 'worker', fn4)
    jobNames = set()
    for job in jobs:
