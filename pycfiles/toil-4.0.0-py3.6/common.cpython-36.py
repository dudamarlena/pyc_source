...
# Size of source mod 2**32: 66912 bytes
...
def _addOptions(addGroupFn, config):
    ...
    if not config.cwl:
        ...
        addOptionFn('--maxServiceJobs', dest='maxServiceJobs', default=None, help=('The maximum number of service jobs that can be run concurrently, excluding service jobs running on preemptable nodes. default=%s' % config.maxServiceJobs))
        addOptionFn('--maxPreemptableServiceJobs', dest='maxPreemptableServiceJobs', default=None, help=('The maximum number of service jobs that can run concurrently on preemptable nodes. default=%s' % config.maxPreemptableServiceJobs))
        addOptionFn('--deadlockWait', dest='deadlockWait', default=None, help=('The minimum number of seconds to observe the cluster stuck running only the same service jobs before throwing a deadlock exception. default=%s' % config.deadlockWait))
        addOptionFn('--statePollingWait', dest='statePollingWait', default=1, help='Time, in seconds, to wait before doing a scheduler query for job state. Return cached results if within the waiting period.')
    else:
