...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil/resource.py
# Compiled at: 2020-04-03 17:41:24
# Size of source mod 2**32: 23677 bytes
from __future__ import absolute_import
from future import standard_library
standard_library.install_aliases()
from builtins import map
import errno, hashlib, importlib, json, logging, os, shutil, sys
...
class Resource(namedtuple('Resource', ('name', 'pathHash', 'url', 'contentHash'))):
    ...
    def create(cls, jobStore, leaderPath):
        ...
        return cls(name=(os.path.basename(leaderPath)), pathHash=pathHash,
          url=jobStore.getSharedPublicUrl(sharedFileName=pathHash),
          contentHash=(contentHash.hexdigest()))

    def refresh(self, jobStore):
        return type(self)(name=(self.name), pathHash=(self.pathHash),
...
class DirectoryResource(Resource):
    ...
    def _load(cls, path):
        ...
        if os.path.isfile(initfile):
            ...
            rootDir = path
        skipdirList = [
         '/tmp', '/var', '/etc', '/bin', '/sbin', '/home', '/dev', '/sys', '/usr', '/run']
        if path not in skipdirList:
            ...
            log.critical("Couldn't package the directory at %s for hot deployment. Would recommend to create a                 subdirectory (ie %s/MYDIR_HERE/)" % (path, path))
            sys.exit(1)
        bytesIO.seek(0)
        return bytesIO
...
class ModuleDescriptor(namedtuple('ModuleDescriptor', ('dirPath', 'name', 'fromVirtualEnv'))):
    ...
    def _check_conflict(cls, dirPath, name):
        ...
        try:
            ...
            try:
                ...
                raise ResourceException("The user module '%s' collides with module '%s from '%s'." % (
    ...
    def _getResourceClass(self):
        ...
        if self.fromVirtualEnv:
            ...
            if os.path.isdir(self._resourcePath):
                ...
                if os.path.isfile(self._resourcePath):
                    subcls = FileResource
                else:
                    if os.path.exists(self._resourcePath):
    ...
    def _runningOnWorker(self):
        try:
            ...
            try:
                ...
                workerModuleFiles = concat(('worker' + ext for ext in self.moduleExtensions), '_toil_worker')
    ...
    def _resourcePath(self):
        ...
        if self.fromVirtualEnv:
            ...
            if initName:
                raise ResourceException('Toil does not support loading a user script from a package directory. You may want to remove %s from %s or invoke the user script as a module via \'PYTHONPATH="%s" %s -m %s.%s\'.' % tuple(concat(initName, self.dirPath, exactPython, os.path.split(self.dirPath), self.name)))
            return self.dirPath
