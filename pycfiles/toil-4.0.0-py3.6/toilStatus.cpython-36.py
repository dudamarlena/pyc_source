...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil/utils/toilStatus.py
# Compiled at: 2020-04-03 17:41:24
# Size of source mod 2**32: 15474 bytes
...
class ToilStatus:
    __doc__ = 'Tool for reporting on job status.'
...
def main():
    ...
    parser.add_argument('jobStore', type=str, help=('The location of a job store that holds the information about the workflow whose status is to be reported on.' + jobStoreLocatorHelp))
    parser.add_argument('--failIfNotComplete', action='store_true', help='Return exit value of 1 if toil jobs not all completed. default=%(default)s',
      default=False)
    parser.add_argument('--noAggStats', dest='stats', action='store_false', help='Do not print overall, aggregate status of workflow.',
      default=True)
    parser.add_argument('--printDot', action='store_true', help='Print dot formatted description of the graph. If using --jobs will restrict to subgraph including only those jobs. default=%(default)s',
      default=False)
    parser.add_argument('--jobs', nargs='+', help='Restrict reporting to the following jobs (allows subsetting of the report).',
      default=None)
    parser.add_argument('--printPerJobStats', action='store_true', help='Print info about each job. default=%(default)s',
      default=False)
    parser.add_argument('--printLogs', action='store_true', help='Print the log files of jobs (if they exist). default=%(default)s',
      default=False)
    parser.add_argument('--printChildren', action='store_true', help='Print children of each job. default=%(default)s',
      default=False)
    ...
    try:
        ...
        if options.stats:
            print('Of the %i jobs considered, there are %i jobs with children, %i jobs ready to run, %i zombie jobs, %i jobs with services, %i services, and %i jobs with log files currently in %s.' % (
        ...
        if len(status.jobsToReport) > 0:
            if options.failIfNotComplete:
                exit(1)