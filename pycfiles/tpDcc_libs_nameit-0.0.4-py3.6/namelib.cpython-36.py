...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/nameit/core/namelib.py
# Compiled at: 2020-04-11 22:26:06
# Size of source mod 2**32: 37703 bytes
"""
Library module related with naming convention for the complete rigging toolkit
"""
from __future__ import print_function, division, absolute_import, unicode_literals
import os, re, copy, json, yaml, lucidity, traceback
...
class Serializable(object):
    ...
    def from_data(cls, data, skip_check=False):
        if skip_check or data.get('_Serializable_classname') != cls.__name__:
            return
        else:
...
class Token(Serializable, object):
    ...
    def _get_default(self):
        ...
        if not items:
            ...
            if items:
                if python.is_number(self.default):
                    if self.default >= 0:
...
class Rule(Serializable, object):
    ...
    def fields(self):
        ...
        """
        return re.findall('\\{([^}]+)\\}', re.sub('_', '_', self.expression))
                    ...
                    if k == field:
                        if v is None:
                            if self.auto_fix:
                                ...
                                logger.warning('Missing field: "{}" when generating new name (None will be used instead)!'.format(k))
                        valid_values[k] = v
...
class NameLib(object):
    DEFAULT_DATA = {'rules':[],  'tokens':[],  'template_tokens':[],  'templates':[]}
    _active_rule = ''
    _templates = list()
    ...
    def parse_field_from_string(self, string_to_parse, field_name):
        ...
        if len(rule_fields) != len(string_split):
            logger.warning('Given string "{}" is not a valid name generated with current nomenclature rule: {}'.format(string_to_parse, active_rule.name()))
            return
        found_index = -1
    ...
    def init_naming_data(self):
        ...
        if not self.has_valid_naming_file():
            ...
            if not self.has_valid_naming_file():
                logger.warning('Impossible to initialize naming data because naming file: "{}" does not exists!'.format(self._naming_file))
                return
    ...
    def get_naming_data(self):
        ...
        return {'active_rule':self._active_rule, 
         'rules':rules, 
         'tokens':tokens, 
    ...
    def load_naming_data(self):
        ...
        """
        if not self.has_valid_naming_file():
            logger.warning('Impossible to read naming file because naming file: "{}" does not exists!'.format(self._naming_file))
            return
        try:
            if self._parser_format == 'yaml':
            ...
            logger.error('Impossible to read naming file "{}": {} | {}'.format(self._naming_file, exc, traceback.format_exc()))
    ...
    def save_naming_data(self, data):
        ...
        """
        if not self.has_valid_naming_file():
            logger.warning('Impossible to save naming file because naming file: "{}" does not exists!'.format(self._naming_file))
            return
        try:
            if self._parser_format == 'yaml':
                yamlio.write_to_file(data, self._naming_file)
    ...
    def load_session(self, repo=None):
        ...
        if self.has_valid_naming_file():
            ...
            naming_data or logger.warning('No naming data found!')
            return
        else:
