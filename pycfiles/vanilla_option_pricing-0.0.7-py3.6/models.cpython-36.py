...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\vanilla_option_pricing\models.py
# Compiled at: 2018-09-15 10:51:21
...
class LogMeanRevertingToGeneralisedWienerProcess(PossiblePricingModel):
    ...
    def variance(self, t):
        ...
        """
        first_term = (self.p_0[(0, 0)] - 2 * self.p_0[(1, 0)] + self.p_0[(1, 1)] - (self.s_x ** 2 + self.s_y ** 2) / (2 * self.l)) * np.exp(-2 * self.l * t)
        second_term = 2 * (self.p_0[(1, 0)] - self.p_0[(1, 1)] + self.s_y ** 2 / self.l) * np.exp(-self.l * t)
        third_term = self.s_y ** 2 * t + (self.s_x ** 2 - 3 * self.s_y ** 2) / (2 * self.l) + self.p_0[(1,
                                                                                                        1)]
...
class NumericalLogMeanRevertingToGeneralisedWienerProcess(PossiblePricingModel):
    ...
    def __init__(self, p_0: np.matrix, l: float, s_x: float, s_y: float):
        ...
        self.s_x = s_x
        self.s_y = s_y
        self.numerical_model = NumericalModel(self._NumericalLogMeanRevertingToGeneralisedWienerProcess__get_A_matrix(), self._NumericalLogMeanRevertingToGeneralisedWienerProcess__get_B_matrix(), self.p_0)
    ...
    def parameters(self, value):
        ...
        self.s_x = value[1]
        self.s_y = value[2]
        self.numerical_model.parameters = [self._NumericalLogMeanRevertingToGeneralisedWienerProcess__get_A_matrix(), self._NumericalLogMeanRevertingToGeneralisedWienerProcess__get_B_matrix()]
...
class NumericalModel:
    ...
    def variance(self, t):
        ...
        F = la.expm(np.bmat([
         ...
         [
          np.zeros_like(self.A), -np.transpose(self.A)]]) * t)
