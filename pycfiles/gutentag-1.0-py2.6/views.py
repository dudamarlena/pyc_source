...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\tagbase\gutentag\views.py
# Compiled at: 2009-08-05 21:22:14
...
import subprocess, blast, log, os, glob, thread
...
def process_cloud_tag(tag_type):
    ...
    emptylist = []
    if tag_type == 'user':
        gtagObj = Genetag.objects.exclude(Q(tags__tag_type='cdd') | Q(tags__tag_type='go') | Q(tags__tag_type='gene_name') | Q(tags__tag_type='organism'))
        if gtagObj.count() == 0:
            return emptylist
        for gtag in gtagObj:
...
def main(request):
    ...
    return render_to_response('gutentag/main.html', {'gnamelist': d['gname'], 'cddlist': d['cdd'], 
       'genelist': gobj, 'orgn': d['organism'], 'cddCloud': cddCloud, 
       'gnameCloud': gnameCloud, 'userCloud': userCloud, 'tag_orgn': tag_orgn, 
       'blastdb': settings.BLASTDB, 'dbname': settings.DBNAME})


def getUser():
    userList = []
    tagObj = Tags.objects.exclude(Q(tag_type='cdd') | Q(tag_type='go') | Q(tag_type='gene_name') | Q(tag_type='organism'))
...
def add(request):
    tag_orgn = org_tag()
    userList = getUser()
    return render_to_response('gutentag/add.html', {'tag_orgn': tag_orgn, 'userList': userList, 
       'blastdb': settings.BLASTDB, 'dbname': settings.DBNAME})


def retrievecdd(data):
    cdd_qs = Seq.objects.filter(gene__genetag__tags__tag_type='cdd', gene__genetag__tags__tag_name__iexact=data)
    cdd_qs = set(cdd_qs)
    return cdd_qs


def retrieveGname(data):
    gname_qs = Seq.objects.filter(gene__genetag__tags__tag_type='gene_name', gene__genetag__tags__tag_name__iexact=data)
    gname_qs = set(gname_qs)
    return gname_qs


def retrieveSearch(data):
    s_qs = Seq.objects.filter(gene__descript__icontains=data) | Seq.objects.filter(gene__genetag__tags__tag_name__icontains=data) | Seq.objects.filter(gene__genetag__tags__tag_details__icontains=data)
    s_qs = set(s_qs)
    return s_qs


def retrieveUser(data):
    user_qs1 = Seq.objects.exclude(Q(gene__genetag__tags__tag_type='cdd') | Q(gene__genetag__tags__tag_type='go') | Q(gene__genetag__tags__tag_type='gene_name') | Q(gene__genetag__tags__tag_type='organism'))
    user_qs = user_qs1.filter(Q(gene__genetag__tags__tag_name__iexact=data) | Q(gene__genetag__tags__tag_details__iexact=data) | Q(gene__genetag__tags__tag_type__iexact=data) | Q(gene__genetag__tags__tag_acc__iexact=data))
    user_qs = set(user_qs)
    return user_qs
...
def retrieveOrg(data):
    org_qs = Seq.objects.filter(gene__genetag__tags__tag_type='organism', gene__genetag__tags__tag_name__iexact=data)
    org_qs = set(org_qs)
    return org_qs
...
def tagCloudSearch(request, tag_name, tag_type):
    ...
    return render_to_response('gutentag/display.html', {'cstr': cstr, 'new_queryset': gs, 
       'count': count})
...
def mainSearch(request):
    ...
    return render_to_response('gutentag/display.html', {'cstr': cstr, 'new_queryset': final_gs, 
       'count': count})
...
def go(request, go_id):
    ...
    return render_to_response('gutentag/gobrowser.html', {'go_list': go_list, 'dbname': settings.DBNAME, 
       'blastdb': settings.BLASTDB})
...
def load_new_seq(request, org, seq_type, taglist, usertag):
    ...
    if usertag != 'empty':
        ...
        utag = Tags.objects.get(tag_type=tagtok[0], tag_name=tagtok[1], tag_acc=tagtok[2], tag_details=tagtok[3])
    if taglist != []:
        tag = Tags.objects.create(tag_type=taglist[0][0], tag_name=taglist[0][1], tag_acc=taglist[0][2], tag_details=taglist[0][3])
    ...
    for line in blast:
        ...
        if 'Expect = ' in line and eYes == 0:
            ...
            if e <= evalue:
                ...
                try:
                    tag = Tags.objects.get(tag_type='cdd', tag_name=cddname, tag_acc=cddacc, tag_details=cdddet)
                except:
                    tag = Tags.objects.create(tag_type='cdd', tag_name=cddname, tag_acc=cddacc, tag_details=cdddet)
                else:
...
def upload(request):
    ...
    if request.FILES == {} and seq == '':
        return render_to_response('gutentag/add.html', {'tag_orgn': tag_orgn, 'userList': userList, 
           'dbname': settings.DBNAME, 'blastdb': settings.BLASTDB, 'error_message': 'ERROR...No sequence data'})
    else:
        if seq != '':
        ...
        if org == 'empty' and neworg == '':
            return render_to_response('gutentag/add.html', {'tag_orgn': tag_orgn, 'userList': userList, 
               'dbname': settings.DBNAME, 'blastdb': settings.BLASTDB, 'error_message': 'ERROR...Sequence/s must be associated with an organism'})
        if org == 'empty':
        ...
        if addtag:
            ...
            if newtagtype == newtagname:
                return render_to_response('gutentag/add.html', {'tag_orgn': tag_orgn, 'userList': userList, 
                   'dbname': settings.DBNAME, 'blastdb': settings.BLASTDB, 'error_message': 'ERROR...Tag name and tag type cannot be identical'})
        ...
        if doannotations:
            ...
            if checkerror == 'error dup':
                return render_to_response('gutentag/add.html', {'tag_orgn': tag_orgn, 'userList': userList, 
                   'dbname': settings.DBNAME, 'blastdb': settings.BLASTDB, 'error_message': 'ERROR...Sequence name and organism already present in database'})
...
def makeBlastInput(key, seqtype):
    ...
    blastin_location = settings.BLASTDB + '/%sblastdb' % key
