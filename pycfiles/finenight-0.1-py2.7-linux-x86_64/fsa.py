...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/finenight/fsa.py
# Compiled at: 2015-02-10 12:16:37
import copy, pdb
from error import *
...
def __rename__(fsa, nameGenerator):
    ...
    while keysToRename != []:
        ...
        if keysToRename == []:
            keysToRename = filter(lambda s: not newKeys.has_key(s), fsa.states.keys())
...
def rename(nfa, nameGenerator=None):
    ...
    newNfa.states = dict(map(lambda s: (newKeys[s],
     newNfa.states[s]), newNfa.states.keys()))
    for state in newNfa.states.values():
        state.epsilon = map(lambda s: newKeys[s], state.epsilon)
        for key in state.transitions.keys():
            state.transitions[key] = map(lambda s: newKeys[s], state.transitions[key])
...
def binaryOperationRenaming(lhs, rhs, renameStates, nameGenerator):
    ...
    if renameStates is True:
        ...
        raise ConstructionError('States names collides when trying ' + 'to apply the operation to the FSAs without renaming ' + 'states names')
    else:
...
class Nfa():
        ...
        if self.states:
            ...
            for state in self.states.values():
                for epsilon in state.epsilon:
                    f.write('"' + str(state.name) + '" -> "' + epsilon + '" [fontname="Symbol", label="e"];\n')
                ...
                for symbol in state.transitions.keys():
                    ...
                    for transition in transitions:
                        f.write('"' + str(state.name) + '" -> "' + str(transition) + '" [ label = "' + str(symbol) + '" ];\n')
    ...
    def __init__(self, states, alphabet, startState, finalStates):
        ...
        for state in self.states.values():
            for key in state.transitions.keys():
                invalidStates = filter(lambda s: s not in self.states.keys(), state.transitions[key])
                if invalidStates != []:
                    raise StateError('state ' + str(state.name) + ' has unknown states: ' + str(invalidStates))

            missingInputs = filter(lambda i: i not in state.transitions.keys(), self.alphabet)
            for input in missingInputs:
                state.transitions[input] = []

            invalidInputs = filter(lambda i: i not in self.alphabet, state.transitions.keys())
            if len(invalidInputs) > 0:
                output = ''
                for input in invalidInputs:
                    output += input + ', '

                raise ConstructionError('symbol ' + output[:-2] + 'in ' + str(state.name) + ' state is/are not part of the alphabet')
    ...
    def transition(self, states, input):
        ...
        acceptedInputStates = []
        for state in states:
            acceptedInputStates += map(lambda s: self.states[s], self.states[state.name].transitions[input])
    ...
    def eClose(self, states):
        ...
        while states != []:
            ...
            epsilonStates = []
            for state in states:
                epsilonStates += filter(lambda s: s not in eClosedStates and s not in states, map(lambda s: self.states[s], state.epsilon))
    ...
    def concatenate(self, other, renameStates=True, nameGenerator=None):
        ...
        new = Nfa(lhs.states.values() + rhs.states.values(), lhs.alphabet + filter(lambda s: s not in lhs.alphabet, rhs.alphabet), lhs.startState, rhs.finalStates)
        return new
    ...
    def union(self, other, renameStates=True, nameGenerator=None):
        ...
        lhs = Dfa(lhs.states.values(), newAlphabet, lhs.startState, lhs.finalStates)
        rhs = Dfa(rhs.states.values(), newAlphabet, rhs.startState, rhs.finalStates)
    ...
    def plus(self):
        ...
        """
        newStates = map(lambda s: copy.deepcopy(self.states[s]), filter(lambda s: self.startState not in self.states[s].epsilon, self.finalStates))
        ...
        return Nfa(newStates.values(), self.alphabet, self.startState, self.finalStates)
    ...
    def complement(self):
        ...
        """
        newDfa = self.determinize()
        newDfa.finalStates = filter(lambda s: s not in newDfa.finalStates, newDfa.states.keys())
        return newDfa
    ...
    def difference(self, other):
        ...
        lhs = self
        rhs = other.determinize()
        rhs = rhs.createFrom(alphabet=rhs.alphabet + filter(lambda s: s not in rhs.alphabet, lhs.alphabet))
        rhs = rhs.complement()
    ...
    def determinize(self, rename=True):
        ...
        while statesToDiscover != []:
            ...
            statesForAlphabet = dict(map(lambda s: (s, self.transition(currentStates, s)), self.alphabet))
            stateDict = {}
        ...
        dfa = Dfa(statesDict.values(), self.alphabet, str([self.startState]), finalStates)
        return dfa
        ...
        for state in self.states.values():
            for targetStates in state.transitions.values():
                if len(targetStates) > 1:
                    raise StateError('You cannot have 2 possible states ' + 'for the same symbol in a DFA')
            ...
            for symbol in self.states[stateName].transitions.keys():
                if self.states[stateName].transitions[symbol] == []:
                    if missingTransitionsStateName not in self.states.keys():
                        newState = State(missingTransitionsStateName, dict(map(lambda s: (s, missingTransitionsStateName), self.alphabet)))
