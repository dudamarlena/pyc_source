...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/finenight/fsc.py
# Compiled at: 2015-02-12 13:35:48
import possibleStates, copy, types, pdb
...
class Position:
    ...
    def __eq__(lhs, rhs):
        return lhs.i == rhs.i and lhs.e == rhs.e and lhs.isTransposition == rhs.isTransposition
    ...
    if it:
        if jt:
            if it and jt and f > e and i == j:
                return True
        elif n == f and f > e and i == j:
            return True
    elif jt:
        if f > e and abs(j - (i - 1)) <= f - e:
            return True
    elif e < f and abs(j - i) <= f - e:
        return True
    return False
...
def isLikeStates(state, lowerStates):
    ...
    while i < len(lowerStates) and isLike == False:
        ...
        for index in range(len(state)):
            state[index] = (
             state[index][0] - difference, state[index][1])
...
class ErrorTolerantRecognizer:
    ...
    def recognize(self, word, transitions, n):
        states = [
         (
          '', self.fsa.startState, [(0, 0)], 0)]
        results = innerLoop(states, word, n, transitions, self.fsa.states, self.fsa_finalStates)
        return results
