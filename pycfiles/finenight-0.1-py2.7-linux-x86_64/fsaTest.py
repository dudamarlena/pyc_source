...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/finenight/fsaTest.py
...
class FsaTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.fsas = [
         [
          self.dfa, self.nfa], [self.dfa2, self.nfa2]]
        self.recognizableTapes = [
         (
          [
           'baa!', 'baaa!', 'baaaa!'], {'nfa': self.nfa, 'dfa': self.dfa}),
         (
          [
    ...
    def testTransition(self):
        ...
        for s in tests:
            ...
            expectedResult.sort()
            errorMsg = 'Transition(' + str(states) + ', ' + str(s[1]) + ') == ' + str(nfa.transition(states, s[1])) + ' != ' + str(s[2])
            self.assert_(result == expectedResult, msg=errorMsg)

    def testNfaKleenee(self):
        nfa = self.nfa.concatenate(self.nfa2.kleenee())
        tapes = self.concatenationTapes + self.recognizableTapes[0][0]
        for tape in tapes:
            errorMsg = 'The tape "' + tape + '" was not accepted for the nfa:\n' + str(nfa) + 'concatened from nfa:\n ' + str(self.nfa) + 'and the kleenee of:\n' + str(self.nfa2)
    ...
    def testRenamingOfStates(self):
        nfa = rename(self.nfa)
        errorMsg = "The renamed FSA doesn't recognize anymore the same langage that the original FSA was recognizing.\noriginal:\n" + str(self.nfa) + 'renamed:\n' + str(nfa)
    ...
    def testEqualityOfStatesWhenNfaIsDuplicated(self):
        nfaCopy = copy.deepcopy(self.nfa)
        errorMsg = "The start state of a copy of a nfa seem to not be equal to the original nfa's start state.\noriginal:\n" + str(self.nfa.startState) + 'current:\n' + str(nfaCopy.startState)
        self.assert_(self.nfa.startState == nfaCopy.startState, msg=errorMsg)

    def testEqualityOfStates(self):
        state1 = State('q1', {'a': 'q2'}, epsilon=['q2'])
        state2 = State('q2', {'a': 'q2'})
        self.assert_(state1 != state2, msg='The state q1 should not be equal to q2')
    ...
    def testAddDfaState(self):
        ...
        try:
            ...
            msg = 'The current dfa accepted a list of choices for a specific letter:' + self.dfa
            self.fail(msg)
        ...
        for testingTapes in self.alphabetErrorTapes:
            for tape in testingTapes[0]:
                try:
                    ...
                    msg = 'The tape "' + tape + '" was accepted ' + '(but supposed to raise an alphabet error) for the ' + type + ':\n' + str(testingTapes[1][type])
                    self.fail(msg)
    ...
    def tryNonRecognizableTapes(self, type):
        for testingTapes in self.nonRecognizableTapes:
            for tape in testingTapes[0]:
                errorMsg = 'The tape "' + tape + '" was accepted  for the ' + type + ':\n' + str(testingTapes[1][type])
    ...
    def tryRecognizableTapes(self, type):
        for testingTapes in self.recognizableTapes:
            for tape in testingTapes[0]:
                errorMsg = 'The tape "' + tape + '" was not accepted  for the ' + type + ':\n' + str(testingTapes[1][type])
    ...
    def testComplement(self):
        ...
        self.assert_(complementDfa.recognize('a') == False, msg='The complement operation over a DFA should not recognize ' + 'a string recognized by the original DFA')
        self.assert_(complementDfa.recognize('aa'), msg='The complement operation over a DFA should recognize ' + 'any string not recognized by the original DFA')
        complementNfa = self.nfa.complement()
        self.assert_(complementNfa.recognize('baaa!') == False, msg='The complement operation over a NFA should not recognize ' + 'a string recognized by the original NFA')
        self.assert_(complementNfa.recognize('aaabb'), msg='The complement operation over a NFA should recognize ' + 'any string not recognized by the original NFA')
    ...
    def testPowerSet(self):
        self.assert_(powerSet(['q1', 'q2', 'q3']) == [[],
         ...
         [
          'q2', 'q3'],
         [
          'q1', 'q2', 'q3']], msg='The power set function return an invalid power set')
    ...
    def testDeterminize(self):
        ...
        self.assert_(dfa.recognize('baaa!'), msg="The determinized NFA doesn't recognize the same language")
        self.assert_(dfa.recognize('baab!') == False, msg="The determinized NFA doesn't recognize the same language")
    ...
    def testStateValidity(self):
        ...
        try:
            ...
            msg = "The current DFA accepted a transition's state " + 'that is not part of the DFA'
            self.fail(msg)
