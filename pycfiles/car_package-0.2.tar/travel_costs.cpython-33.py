...
# Embedded file name: build/bdist.linux-x86_64/egg/cary_travelcommand/travel_costs.py
# Compiled at: 2015-08-04 11:35:41
# Size of source mod 2**32: 6477 bytes
...
class QPXCostEstimator(RouteCostEstimator):
    ...
    def last_formatted_response(self):
        ...
        return dict(found=True, estimator=self.name, fares=option_prices(self.last_response), supplemental_data=self.last_response.copy())


class FakeQPXCostEstimator(QPXCostEstimator):

    def __init__(self, api_key, single_request_filename, roundtrip_request_filename, single_response_filename, roundtrip_response_filename):
    ...
    def roundtrip_estimate(self, start, end, date_out, date_back):
        ...
        if outslice['origin'] == start and outslice['destination'] == end and backslice['origin'] == end and backslice['destination'] == start:
...
def make_test_estimator():
    ...
    return FakeQPXCostEstimator('nokey', fakeFile('sample_single_request.json'), fakeFile('sample_roundtrip_request.json'), fakeFile('sample_single_response.json'), fakeFile('sample_roundtrip_response.json'))