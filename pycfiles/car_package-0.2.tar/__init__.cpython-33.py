...
# Embedded file name: build/bdist.linux-x86_64/egg/cary_travelcommand/__init__.py
# Compiled at: 2015-08-06 07:48:38
...
class TravelCommand(CaryCommand):
    ...
    def required_attachments(self):
        return [
         'trip as lines of text in body of message']
...
def trip_travel_cost(costs):
    ...
    def leg_travel_cost(leg):
        ...
        if travel_cost['found']:
            return mean_filter(travel_cost['fares'])
        return 0
...
def trip_total_cost(costs):
    return trip_travel_cost(costs) + trip_lodging_cost(costs) + trip_mie_cost(costs)
...
class TravelAction(CaryAction):
    ...
    def execute_action(self):
        self.estimators = self.config['cost_estimators'] if 'cost_estimators' in self.config else []
        self.environment = Environment(loader=FileSystemLoader(self.template_path), undefined=DebugUndefined)
        ...
        self.pd = PerdiemDatabase(self.config['LOCSTRING_FILENAME'], self.config['DB_FILENAME'])
        ...
        self._perdiem_text_plain = self.environment.get_template('plaintext_template.txt').render(costs=self.costs)
        self._perdiem_text_html = self.environment.get_template('html_template.html').render(costs=self.costs)
