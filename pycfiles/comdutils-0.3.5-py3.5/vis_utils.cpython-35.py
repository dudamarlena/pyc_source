...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/comdutils/vis_utils.py
# Compiled at: 2019-10-23 13:54:17
...
def give_frame(image, company_name, project_name):
    ...
    cv2.putText(image, project_name, (5, int(5 + space / 2)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,
                                                                                              100,
                                                                                              255), 2, cv2.LINE_AA)
    cv2.putText(image, company_name, (5, int(5 + space + space / 2)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,
                                                                                                      100,
                                                                                                      255), 2, cv2.LINE_AA)
    return image
...
def put_vertical_textsoverrect(image, rects, text_list):
    ...
    for idx, i in enumerate(rects):
        ...
        for idx2, j in enumerate(text_list[idx]):
            cv2.putText(image, j, (i[0] + 5, int(i[1] + space * idx2 + space / 2)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,
                                                                                                                    255,
                                                                                                                    0), 2, cv2.LINE_AA)
