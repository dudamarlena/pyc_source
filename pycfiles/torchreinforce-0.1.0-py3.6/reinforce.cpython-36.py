...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/torchreinforce/reinforce.py
# Compiled at: 2019-01-18 12:14:20
...
class ReinforceModule(torch.nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        if issubclass(self.distribution, torch.distributions.Distribution):
            if not issubclass(self.distribution, ReinforceDistribution):
    ...
    def _check_used(self):
        if len(self.history) != 0:
            if len(list(filter(lambda x: x.used, self.history))) != 0:
