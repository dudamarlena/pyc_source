...
# [GCC 8.4.0]
# Embedded file name: /home/mukulhase/Dev/github/webwhatsapp-scripts/webwhatsapi/objects/chat.py
...
class UserChat(Chat):
    ...
    def __repr__(self):
        ...
        return '<User chat - {name}: {id}>'.format(name=safe_name,
          id=(self.id))
...
class GroupChat(Chat):
    ...
    def __repr__(self):
        safe_name = safe_str(self.name)
        return '<Group chat - {name}: {id}, {participants} participants>'.format(name=safe_name,
          id=(self.id),
