...
# [GCC 8.4.0]
# Embedded file name: /home/mukulhase/Dev/github/webwhatsapp-scripts/webwhatsapi/__init__.py
# Compiled at: 2018-05-27 10:21:46
# Size of source mod 2**32: 21953 bytes
"""
WebWhatsAPI module
...
class WhatsAPIDriver(object):
    ...
    _CLASSES = {'unreadBadge':'icon-meta', 
    ...
    def set_local_storage(self, data):
        self.driver.execute_script(''.join(["window.localStorage.setItem('{}', '{}');".format(k, v) for k, v in data.items()]))
    ...
    def __init__(self, client='firefox', username='API', proxy=None, command_executor=None, loadstyles=False, profile=None, headless=False, autoconnect=True, logger=None, extra_params=None, chrome_options=None):
                ...
                if self.client == 'chrome':
                    ...
                    if client == 'remote':
                        ...
                        self.driver = (webdriver.Remote)(command_executor=command_executor, 
                         desired_capabilities=capabilities, **extra_params)
                    else:
    ...
    def wait_for_login(self, timeout=90):
        ...
        WebDriverWait(self.driver, timeout).until(EC.visibility_of_element_located((By.CSS_SELECTOR, self._SELECTORS['mainPage'])))
    ...
    def get_unread_messages_in_chat(self, id, include_me=False, include_notifications=False):
        ...
        :rtype: list
        """
        messages = self.wapi_functions.getUnreadMessagesInChat(id, include_me, include_notifications)
    ...
    def get_chat_from_phone_number(self, number, createIfNotFound=False):
        ...
        for chat in self.get_all_chats():
            if not not isinstance(chat, UserChat):
                if number not in chat.id:
                    continue
            return chat
    ...
    def download_media(self, media_msg):
        ...
        derivative = HKDFv3().deriveSecrets(media_key, binascii.unhexlify(media_msg.crypt_keys[media_msg.type]), 112)
