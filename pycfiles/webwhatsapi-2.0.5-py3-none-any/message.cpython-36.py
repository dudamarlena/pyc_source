...
# [GCC 8.4.0]
# Embedded file name: /home/mukulhase/Dev/github/webwhatsapp-scripts/webwhatsapi/objects/message.py
# Compiled at: 2018-05-27 06:05:33
# Size of source mod 2**32: 7188 bytes
import mimetypes
...
class Message(WhatsappObject):
    ...
    def __repr__(self):
        return '<Message - {type} from {sender} at {timestamp}: {content}>'.format(type=(self.type),
...
class MediaMessage(Message):
    ...
    def __repr__(self):
        return '<MediaMessage - {type} from {sender} at {timestamp} ({filename})>'.format(type=(self.type),
          sender=(safe_str(self.sender.get_safe_name())),
          timestamp=(self.timestamp),
...
class MMSMessage(MediaMessage):
    ...
    def __repr__(self):
        return '<MMSMessage - {type} from {sender} at {timestamp}>'.format(type=(self.type),
          sender=(safe_str(self.sender.get_safe_name())),
          timestamp=(self.timestamp))
...
class VCardMessage(Message):
    ...
    def __repr__(self):
        return '<VCardMessage - {type} from {sender} at {timestamp} ({contacts})>'.format(type=(self.type),
          sender=(safe_str(self.sender.get_safe_name())),
          timestamp=(self.timestamp),
...
class GeoMessage(Message):
    ...
    def __repr__(self):
        return '<GeoMessage - {type} from {sender} at {timestamp} ({lat}, {lng})>'.format(type=(self.type),
          sender=(safe_str(self.sender.get_safe_name())),
          timestamp=(self.timestamp),
          lat=(self.latitude),
          lng=(self.longitude))
...
class NotificationMessage(Message):
    ...
    def __repr__(self):
        readable = {'call_log':{'miss': 'Missed Call'}, 
         'e2e_notification':{'encrypt': 'Messages now Encrypted'}, 
         'gp2':{'invite':'Joined an invite link', 
        ...
        return '<NotificationMessage - {type} {recip} {sender} at {timestamp}>'.format(type=(readable[self.type][self.subtype]),
          sender=sender,
          timestamp=(self.timestamp),
...
class MessageGroup(object):
    ...
    def __repr__(self):
        ...
        return '<MessageGroup - {num} {messages} in {chat}>'.format(num=(len(self.messages)),
