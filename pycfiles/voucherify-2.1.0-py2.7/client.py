...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/voucherify/client.py
# Compiled at: 2019-07-18 09:15:08
...
class VoucherifyRequest(object):
    ...
    def __init__(self, application_id, client_secret_key, api_endpoint=None):
        ...
        self.headers = {'X-App-Id': application_id, 
    ...
    def request(self, path, method='GET', **kwargs):
        try:
            url = self.url + path
            response = requests.request(method=method, url=url, headers=self.headers, timeout=self.timeout, **kwargs)
...
class Vouchers(VoucherifyRequest):
    ...
    def create(self, voucher):
        ...
        path = '/vouchers/' + quote(code)
        return self.request(path, data=json.dumps(voucher), method='POST')

    def update(self, voucher_update):
        path = '/vouchers/' + quote(voucher_update.get('code'))
        return self.request(path, data=json.dumps(voucher_update), method='PUT')
...
class Redemptions(VoucherifyRequest):
    ...
    def redeem(self, code, tracking_id=None):
        ...
        return self.request(path, method='POST', data=json.dumps(context))
...
class Distributions(VoucherifyRequest):
    ...
    def publish(self, params):
        ...
        return self.request(path, method='POST', data=json.dumps(params))
...
class Customers(VoucherifyRequest):
    ...
    def create(self, customer):
        path = '/customers/'
        return self.request(path, data=json.dumps(customer), method='POST')
    ...
    def update(self, customer):
        ...
        return self.request(path, data=json.dumps(customer), method='PUT')
