...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/qt/widgets/library/treewidget.py
# Compiled at: 2020-05-13 19:31:15
# Size of source mod 2**32: 17326 bytes
"""
...
class LibraryTreeWidget(mixin.LibraryViewWidgetMixin, QTreeWidget):
    ...
    def _remove_duplicates(self, labels):
        ...
        s = set()
        sadd = s.add
        return [x for x in labels if x.strip() if not (x in s or sadd(x))]
    ...
    def column_settings(self):
        ...
        for column in range(self.columnCount()):
            ...
            width = self.columnWidth(column)
            column_settings[label] = {'index':column, 
             'hidden':hidden, 
             'width':width}
