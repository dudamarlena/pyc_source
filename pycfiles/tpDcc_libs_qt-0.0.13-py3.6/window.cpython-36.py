...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/qt/widgets/library/window.py
# Compiled at: 2020-05-13 19:31:15
# Size of source mod 2**32: 68631 bytes
"""
Module that contains main window widget used in libraries
"""
from __future__ import print_function, division, absolute_import
import os, re, time, operator
...
class LibraryWindow(window.BaseWindow, object):
    ...
    def set_settings(self, settings):
        ...
        viewer_settings = dict(self.settings().get('{}/viewerWidget'.format(library_name), {'spacing':2, 
         'padding':6,  'zoomAmount':80,  'textVisible':True}))
        if viewer_settings:
    ...
    def set_trash_folder_visible(self, flag):
        ...
        if flag:
            query = {'name':'trash_query',  'filters':list()}
        else:
            query = {'name':'trash_query',  'filters':[
              ('path', 'not_contains', 'Trash')]}
    ...
    def show_hello_dialog(self):
        ...
        dialog = messagebox.create_message_box(None,
          ('Welcome {}'.format(self.name())), text, header_pixmap=(self._library_icon_path))
