...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/qt/core/config.py
...
class ConfigurationAttribute(dict, object):
    ...
    def from_nested_dict(data):
        ...
        if not isinstance(data, dict):
            ...
            return ConfigurationAttribute({key:ConfigurationAttribute.from_nested_dict(data[key]) for key in data})
...
class YAMLConfiguration(object):
        ...
        if not self._parsed_data:
            ...
            if attr_section:
                if attr_name:
                    ...
                    if not attr_section:
                        LOGGER.warning('Configuration "{}" has no attribute "{}" in section "{}"'.format(self._config_name, attr_name, orig_section))
                        return default
                    else:
                        attr_value = attr_section.get(attr_name, None)
                        if attr_value is None:
                            LOGGER.warning('Configuration "{}" has no attribute "{}" in section "{}"'.format(self._config_name, attr_name, attr_section))
                            return default
                        return attr_value
    ...
    def _get_config_data(self, config_name, config_dict):
        ...
        if not config_name:
            ...
            all_config_paths = self._manager.get_config_paths(module_config_name=module_config_name,
            ...
            if not valid_config_paths:
                raise RuntimeError('Impossible to load configuration "{}" because it does not exists in any of the configuration folders: {}'.format(config_name, ''.join(all_config_paths)))
            root_config_path = valid_config_paths[(-1)]
            config_data = metayaml.read(valid_config_paths, config_dict) or OrderedDict()
            if config_data is None:
                raise RuntimeError('Project Configuration File {} is empty! {}'.format(self, root_config_path))
            if 'config' in config_data:
                if 'path' in config_data['config']:
                    raise RuntimeError('Project Configuration File for {} Project cannot contains config section with path attribute! {}'.format(self, root_config_path))
                    return
            if 'config' in config_data:
