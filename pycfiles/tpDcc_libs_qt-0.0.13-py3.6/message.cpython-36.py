...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/qt/widgets/message.py
# Compiled at: 2020-05-13 19:31:15
# Size of source mod 2**32: 13431 bytes
"""
...
class BaseMessage(base.BaseWidget, object):
    ...
    def _set_type(self, value):
        ...
        if value in [MessageTypes.INFO, MessageTypes.SUCCESS, MessageTypes.WARNING, MessageTypes.ERROR]:
            ...
            raise ValueError('Given button type: "{}" is not supported. Supported types are: info, success, warning, error'.format(value))
        if current_them:
            self._icon_label.image = tp.ResourcesMgr().pixmap((self._type),
              color=(getattr(current_them, '{}_color'.format(self._type))))
        else:
            self._icon_label.image = tp.ResourcesMgr().pixmap(self._type)
...
class PopupMessage(base.BaseWidget, object):
    ...
    def ui(self):
        ...
        if current_theme:
            icon_label.image = tp.ResourcesMgr().pixmap(current_type,
              color=(getattr(current_theme, '{}_color'.format(current_type))))
    ...
    def info(cls, text, parent, duration=None, closable=None):
        ...
        popup_message_inst = cls(text,
          theme_type=(MessageTypes.INFO), duration=duration, closable=closable, parent=parent)
    ...
    def success(cls, text, parent, duration=None, closable=None):
        ...
        """
        popup_message_inst = cls(text,
          theme_type=(MessageTypes.SUCCESS), duration=duration, closable=closable, parent=parent)
        popup_message_inst.show()
        return popup_message_inst
    ...
    def warning(cls, text, parent, duration=None, closable=None):
        ...
        """
        popup_message_inst = cls(text,
          theme_type=(MessageTypes.WARNING), duration=duration, closable=closable, parent=parent)
        popup_message_inst.show()
        return popup_message_inst
    ...
    def error(cls, text, parent, duration=None, closable=None):
        ...
        """
        popup_message_inst = cls(text,
          theme_type=(MessageTypes.ERROR), duration=duration, closable=closable, parent=parent)
        popup_message_inst.show()
        return popup_message_inst
    ...
    def loading(cls, text, parent, duration=None, closable=None):
        ...
        """
        popup_message_inst = cls(text,
          theme_type=(MessageTypes.LOADING), duration=duration, closable=closable, parent=parent)
        popup_message_inst.show()
        return popup_message_inst
