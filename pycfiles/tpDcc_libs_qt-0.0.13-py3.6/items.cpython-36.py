...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/qt/widgets/library/items.py
# Compiled at: 2020-05-13 19:31:15
# Size of source mod 2**32: 77590 bytes
"""
Module that contains library item widget implementation
"""
from __future__ import print_function, division, absolute_import
import os, math, shutil, tempfile, traceback
...
class LibraryItem(QTreeWidgetItem, object):
    ...
    def icon(self, column):
        ...
        if not ic:
            if column == self.DEFAULT_THUMBNAIL_COLUMN:
    ...
    def save(self, path=None, *args, **kwargs):
        ...
        if path:
            if not path.endswith(self.Extension):
                ...
                valid_save or qt.logger.warning('Item {} not saved!'.format(path))
                if self.library_window():
    ...
    def show_rename_dialog(self, parent=None):
        ...
        name, btn = messagebox.MessageBox.input(parent,
          'Rename item', 'Rename the current item to:', input_text=(self.name()))
    ...
    def show_already_existing_dialog(self):
        ...
        if not self.library_window():
            ...
            try:
                ...
                btn = self.library_window().show_question_dialog('Item already exists', 'Would you like to move the existing item "{}" to trash?'.format(self.name()), buttons)
            finally:
    ...
    def thumbnail_icon(self):
        ...
        if not self._thumbnail_icon:
            if self.ENABLE_THUMBNAIL_THREAD:
...
class BaseItem(LibraryItem, object):
    ...
    def save(self, path=None, *args, **kwargs):
        ...
        :return:
        """
        valid_save = (super(BaseItem, self).save)(args, path=path, **kwargs)
        if not valid_save:
