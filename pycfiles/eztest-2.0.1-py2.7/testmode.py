...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\eztest\testmode.py
# Compiled at: 2018-06-30 02:52:25
"""Test mode:
...
"""
import copy, datetime, os, threading, time, traceback, zipfile, socket
from . import utility
...
class NormalTest(object):
    ...
    def case_finished(self, case):
        ...
        if self._file:
            ...
            try:
                self._socket.sendto(pickle.dumps(dict(repeat_index=case.repeat_index, id=case.id, description=case.description, status=case.status, expected=case.expected, received=case.received, output_messages=case.output_messages, start_time=case.start_datetime, end_time=case.end_datetime, time_taken=case.get_time_taken())), self.report_server)
            except Exception:
                pass
            ...
            try:
                if not self.no_report:
                    if self.report_server:
                        ...
                        report_file = os.path.join(self.report_folder, 'report_%s.csv' % datetime.datetime.now().strftime('%Y%m%d%H%M%S%f'))
                        f = open(report_file, 'w')
                        f.write('"Repeat Index","Id","Description","Status","Expected","Received","Output","Starts DateTime","Ends DateTime","E2E Taken","Log Path"')
