...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\eztest\calc_report.py
# Compiled at: 2018-06-29 17:55:14
...
STATUS_PATTERN = re.compile('^"\\d+","(.+?)",".+?","(Pass|Fail)"')
TIME_PATTERN = re.compile('"(\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}\\.\\d{6})","(\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}\\.\\d{6})","([\\d\\\\.]+)"')
...
def add_to_group_summary(summary, case_id, start_time, time_taken=None, is_case_pass=None):
    ...
    if time_taken is None:
        if key not in summary:
            summary[key] = {ID: case_id, START_TIME: start_time}
    elif key not in summary:
        summary[key] = {ID: case_id, START_TIME: start_time, AVERAGE: time_taken, 
           TOTAL_COUNT: 1, 
...
def add_to_case_summary(summary, case_id, time_taken, is_case_pass):
    ...
    if case_id not in summary:
        summary[case_id] = {AVERAGE: time_taken, TOTAL_COUNT: 1, FAIL_COUNT: 0 if is_case_pass else 1, 
           MIN_TIME: time_taken, 
           MAX_TIME: time_taken}
    else:
...
def analyze_case(case_id, is_pass, start_date, end_date, time_taken, case_summary, start_times, group_summary, group_gap):
...
def output_summary(case_summary, group_summary, group_gap):
    ...
    for case_id, value in case_summary.items():
        sb.append_line(('{},{},{},{},{},{},{}').format(case_id, value[FAIL_COUNT], value[TOTAL_COUNT], ('{:.4f}%').format(value[FAIL_COUNT] / (1 if value[TOTAL_COUNT] == 0 else value[TOTAL_COUNT]) * 100), value[MIN_TIME], value[MAX_TIME], value[AVERAGE]))
        index = 1
        for key, group_value in group_summary.items():
            if key.startswith(case_id):
                groups.append(('{},{},{},{},{},{},{},{},{},{}').format(case_id, index, utility.date2str(group_value[START_TIME], '%Y-%m-%d %H:%M:%S'), utility.date2str(group_value[START_TIME] + group_gap, '%Y-%m-%d %H:%M:%S'), group_value.get(FAIL_COUNT, 0), group_value.get(TOTAL_COUNT, 0), ('{:.4f}%').format(group_value[FAIL_COUNT] / (1 if group_value[TOTAL_COUNT] == 0 else group_value[TOTAL_COUNT]) * 100 if FAIL_COUNT in group_value else 0), group_value.get(MIN_TIME, ''), group_value.get(MAX_TIME, ''), group_value.get(AVERAGE, '')))
                index += 1
            ...
            with open(file_path, 'r') as (f):
                ...
                while True:
                    ...
                    if time_match:
                        ...
                        analyze_case(case_id, is_pass, start_date, end_date, time_taken, case_summary, start_times, group_summary, group_gap)
