...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\eztest\mail.py
...
class Mail:
    __slots__ = [
     'server', 'port', 'timeout', 'key_file', 'cert_file', 'is_ssl',
    ...
    def send(self, subject, body=None, attachments=None):
        ...
        if self.is_ssl:
            s = smtplib.SMTP_SSL(host=self.server, port=int(self.port or 0), timeout=int(self.timeout), keyfile=self.key_file, certfile=self.cert_file)
        else:
        ...
        s.sendmail(from_addr=self.from_addr, to_addrs=self.to_addrs if isinstance(self.to_addrs, list) else self.to_addrs.split(','), msg=msg.as_string())
        s.quit()