...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\eztest\utility.py
# Compiled at: 2018-06-30 03:46:57
...
def str2date(date_time_str, date_format=None):
    ...
    if not date_time_str:
        ...
        g = re.search('(\\d+):(\\d+)(:(\\d+)((\\.|,)(\\d+))?)?( (AM|PM))? ?([a-zA-Z]+|[+-]\\d{4}|[+-]\\d+:\\d+)?$', date_time_str)
        if g:
...
def total_seconds(start_date, ends_date, date_format=None):
    ...
    return (timedelta.microseconds + (timedelta.seconds + timedelta.days * 24 * 3600) * 1000000.0) / 1000000.0
...
def _verify_list(result_value, expect_value, key_in_list=None, count_in_list=True, ignore_key_list=None):
    ...
    for index, expect_item in enumerate(expect_value):
        ...
        elif not expect_item in result_value:
            raise AssertionError('result does not contain: %s' % expect_item)
    ...
    if isinstance(expect, list):
        ...
        for k in expect:
            ...
            if expect_value is None:
                ...
                elif not result_value == expect_value:
                    raise AssertionError('%s=%s in result does not match the expected %s=%s' % (k, result_value, k, expect_value))
