...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\eztest\report.py
# Compiled at: 2018-07-05 20:50:11
"""Report server and report handler."""
import datetime, importlib, os, re, socket, sys, traceback
...
class ReportBaseHandler(object):
    ...
    def write(self, case_result):
        ...
        """
        calc_report.analyze_case(case_id=case_result['id'], is_pass=case_result['status'], start_date=case_result['start_time'], end_date=case_result['end_time'], time_taken=case_result['time_taken'], case_summary=self.case_summary, start_times=self.start_times, group_summary=self.group_summary, group_gap=self.group_gap)
    ...
    def dump(self):
        ...
        if self.group_summary:
            return calc_report.output_summary(case_summary=self.case_summary, group_summary=self.group_summary, group_gap=self.group_gap)
        else:
...
class ReportFileHandler(ReportBaseHandler):
    ...
    def format(cls, case_result):
        ...
        """
        output_messages = ('\n').join(utility.csv_format(message) for message in case_result.get('output_messages', []))
