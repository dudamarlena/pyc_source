...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\eztest\testcase.py
# Compiled at: 2018-06-21 22:15:50
"""Test case class."""
import datetime, os, re, sys, traceback, uuid
...
class BaseCase(object):
    ...
    def log(self, message, to_console=False, level=INFO, no_format=False):
        ...
        if message is None:
            ...
            msg = (no_format or ('{}\t{}\t{}\t{}').format)(datetime.datetime.now().strftime('%Y-%d-%m %H:%M:%S.%f'), level, self.id, message) if 1 else str(message)
            if to_console:
                print msg
    ...
    def generate_log(self):
        ...
        self.log_path = os.path.join(self.log_folder, re.sub('[^\\w_\\\\.-]', '', filename))
