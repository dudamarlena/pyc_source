...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\eztest\__init__.py
# Compiled at: 2018-07-05 20:22:51
...
"""
import argparse, datetime, importlib, inspect, os, re, socket, sys, traceback, psutil
        ...
        for match_part in match_parts:
            if match_part.find('*') >= 0:
                ...
                if re.match(match_pattern, name, flags=re.IGNORECASE):
                    is_matched = True
                    break
            elif utility.compare_str(match_part, name, True) == 0:
                is_matched = True
                break
...
def dump(args):
    ...
    try:
        try:
            ...
            s.sendto('dump', _get_report_server(args.report_server))
...
def test(args):
    ...
    results = _load_cases(args.target, case_matches=args.cases, ignore_match_parts=args.not_cases, class_matches=args.classes, ignore_class_matches=args.not_classes)
...
def _parser_args(args=None):
    ...
    case_group.add_argument('--target', '-t', required=True, help='Folder or file path, or a module, a __init__.py file is required under that folder/module.')
    case_group.add_argument('--classes', '-cl', nargs='+', help='Class names to be tested. It will be considered if target is file.')
    case_group.add_argument('--not-classes', '-ncl', nargs='+', help='Class names to be ignored. It will be considered if target is file.')
    case_group.add_argument('--cases', '-c', nargs='+', help='Case names to be tested. It can be whole case name or part of them(e.g.: "*a", "a*", "*a*").')
    case_group.add_argument('--not-cases', '-nc', nargs='+', help='Case names to be ignored. It can be whole case name or part of them(e.g.: "*a", "a*", "*a*").')
    test_group = test_parser.add_argument_group('Test Mode Group', 'Define arguments of test mode related.')
    test_group.add_argument('--mode', '-m', default='normal', choices=[
     '0', '1', '2', '3', '4', 'normal', 'continuous', 'simultaneous', 'concurrency', 'frequent'], help="(a)0 or normal: Run selected cases only once. \n(b)1 or continuous: Run cases [repeat] times with [interval] seconds' sleeping. \n(c)2 or simultaneous: Start [stress] threads and run cases in each thread, sleep [interval] seconds after all cases are finished, and then start testing again with [repeat] times. \n(d)3 or concurrency: Start [stress] threads and each thread will continuously run cases with [interval] seconds' sleeping. \n(e)4 or frequent: Start [stress] threads per [interval] seconds. And only can have [limit] available threads running.")
    test_group.add_argument('--stress', '-s', type=int, default=1, help='Start [stress] threads in each round of testing. Default value is 1.')
    test_group.add_argument('--repeat', '-r', type=int, default=1, help='Repeat [repeat] times of testing. Default value is 1')
    test_group.add_argument('--interval', '-i', type=int, default=0, help='Sleep [interval] seconds after one round of testing. Default value is 0.')
    test_group.add_argument('--limit', '-l', type=int, default=0, help='Only can have [limit] count of running threads. No limitation if this is less than or equals to [stress].')
    test_group.add_argument('--starts', '-st', type=_to_datetime, help='Testing will be started at [starts]. It is datetime string(e.g.: "2014-01-02 03:04:05").')
    test_group.add_argument('--duration', '-d', type=int, help="Testing will continue with [duration] minutes. Will be ignored if 'ends' is provided.")
    test_group.add_argument('--ends', '-et', type=_to_datetime, help='Testing will be stopped at [ends]. It is datetime string(e.g.: "2014-01-02 03:04:05").')
    log_group = test_parser.add_argument_group('Report/Log Group', 'Define arguments of report or log related.')
    log_group.add_argument('--report-folder', '-rf', help='Report and log files will be saved under [report-folder].')
    log_group.add_argument('--report-server', '-rs', help='Report server. The format is "host_name:port_number" or "host_name" with default port number 8765.')
    log_group.add_argument('--noreport', '-nr', action='store_true', help='No report file will be generated if [noreport] is clarified.')
    log_group.add_argument('--nolog', '-nl', action='store_true', help='No log file will be generated if [nolog] is clarified.')
    log_group.add_argument('--mail-config', '-mc', help='Mail configuration file which contains mail server information etc. \nIt should be INI format file(http://en.wikipedia.org/wiki/INI_file). \nWill send report by mail only if mail-config is provided and report file is generated. \nSection is "SMTP" and properties can be "server", "from_mail", "to_mails", "cc_mails", "bcc_mails", "username", "password", "need_authentication" and "subject". \n"server", "from_mail" and "to_mails" are mandatory. \n"to_mails", "cc_mails" and "bcc_mails" can be multiple values separated by comma. \n"need_authentication" is boolean, "username" and "password" are required if "need_authentication" is True.')
    ...
    calc_parser.add_argument('--path', '-p', required=True, nargs='+', help='Report folders or files to be calculated.')
    calc_parser.add_argument('--group-minutes', '-gm', type=int, default=60, help='Calculate by grouping case results with [group-minutes] minutes. Default is 60 minutes.')
    ...
    port_argument.add_argument('--handler', '-hl', help='Custom handler. The format is: "file_path:handler_class_name", or "module_name:handler_class_name".')
    ...
    dump_parser.add_argument('--report-server', '-rs', default='localhost:8765', help='Report server. The format is "host_name:port_number" or "host_name" with default port number 8765.')
