...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.5/site-packages/doapfiend_ebuild.py
# Compiled at: 2008-06-09 04:14:17
"""
...
def get_license(license_url):
    ...
    known_licenses = {'http://usefulinc.com/doap/licenses/artistic': 'Artistic', 
...
def make_ebuild(doap_xml):
    ...
    if hasattr(doap, 'license') and len(doap.license) > 0:
        ...
        my_license = ''
    if hasattr(doap, 'shortdesc') and len(doap.shortdesc) > 0:
        description = doap.shortdesc[0].strip()
    elif hasattr(doap, 'description') and len(doap.description) > 0:
        description = doap.description[0].strip()
    else:
        description = ''
    my_vars = dict(description=description, homepage=doap.homepage.resUri, license=my_license, rdepend='', depend='', s='', use='', slot='0', keywords='~x86')
...
class EbuildPlugin(Plugin):
    ...
    def add_options(self, parser, output, search):
        ...
        output.add_option('--%s' % self.name, action='store_true', dest=self.enable_opt, help='Create Gentoo ebuild from DOAP')
