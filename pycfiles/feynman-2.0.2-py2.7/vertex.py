...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/feynman/core/vertex.py
# Compiled at: 2018-05-30 16:56:31
from copy import deepcopy
import numpy as np, matplotlib as mpl, matplotlib.pyplot as plt, matplotlib.patches as mpa, matplotlib.text as mpt
...
class Vertex(Drawable):
    ...
    **kwargs:
    ...
    def __init__(self, xy=(0, 0), **kwargs):
        dx = np.array(kwargs.pop('dx', 0.0))
        dy = np.array(kwargs.pop('dy', 0.0))
        dxy = np.array(kwargs.pop('dxy', (0.0, 0.0)))
        angle = np.array(kwargs.pop('angle', 0.0))
        radius = np.array(kwargs.pop('radius', 0.0))
        cxy = np.complex(*xy) + np.complex(*dxy) + np.complex(dx, dy) + radius * np.e ** (complex(0.0, 1.0) * tau * angle)
        self.xy = np.array([cxy.real, cxy.imag])
        self.style = dict(marker='o', linestyle='', markersize=10, color='k', zorder=20)
    ...
    def text(self, s, x=-0.025, y=+0.025, **kwargs):
        """
        ...
        **kwargs:
        ...
        """
        default = dict(verticalalignment='center', horizontalalignment='center', fontsize=14)
