...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/feynman/core/lines.py
# Compiled at: 2018-05-31 10:41:26
from copy import copy, deepcopy
import warnings, numpy as np
from numpy import sin, cos, sqrt
from numpy.linalg import norm
import matplotlib as mpl, matplotlib.pyplot as plt, matplotlib.patches as mpa, matplotlib.text as mpt
...
class Line(Drawable):
    ...
    **line_kwargs: 
    ...
    _shape_allowed_values = sum([], _shape_linear_aliases + _shape_elliptic_aliases + _shape_circular_aliases)
    _flavour = 'simple'
    _flavour_simple_aliases = ('simple', )
    _flavour_wiggly_aliases = ('wiggly', 'w', 'wiggle', 'wiggles')
    _flavour_loopy_aliases = ('loopy', 'l', 'loop', 'loops')
    _flavour_allowed_values = sum([], _flavour_simple_aliases + _flavour_wiggly_aliases + _flavour_loopy_aliases)
    _stroke = 'single'
    _stroke_single_aliases = ('single', 's')
    _stroke_double_aliases = ('double', 'd')
    _stroke_dashed_aliases = ('dashed', '--')
    _stroke_dotted_aliases = ('dotted', ':')
    _stroke_allowed_values = sum([], _stroke_single_aliases + _stroke_double_aliases + _stroke_dashed_aliases + _stroke_dotted_aliases)
    ...
    def set_default(self, **overwrite):
        ...
        default = dict(stroke='single', flavour='simple', shape='linear', style='', arrow=True, npoints=400, ellipse_spread=0.5, ellipse_excentricity=1.2, ellipse_position='up', circle_radius=0.15, circle_angle=0.25, circle_excentricity=1.0, amplitude=0.025, xamp=0.025, yamp=0.05, nwiggles=5, nloops=14, phase=0, double_center_color='w')
    ...
    def set_shape_dependent_defaults(self, kwargs):
        ...
        if self.shape == 'circular':
            if self.flavour == 'simple':
                self.default.update(circle_radius=0.1)
            elif self.flavour == 'wiggly':
                self.default.update(nwiggles=7.25)
                self.default.update(phase=0.75)
                self.default.update(circle_radius=0.15)
            elif self.flavour == 'loopy':
                pass
        elif self.shape == 'elliptic' and 'ellipse_position' in kwargs:
            if kwargs['ellipse_position'] in ('down', -1):
    ...
    def set_line_kwargs_defaults(self, kwargs):
        ...
        self.line_kwargs = dict(marker='', color='k', linestyle='-', linewidth=3, zorder=10, solid_capstyle='butt')
        valid_keys = list()
    ...
    def set_attributes(self, kwargs):
        ...
        for key in ('arrow', 'ellipse_spread', 'ellipse_excentricity', 'ellipse_position',
                    'circle_radius', 'circle_excentricity', 'circle_angle', 'amplitude',
                    'xamp', 'yamp', 'nwiggles', 'nloops', 'phase', 'double_center_color'):
    ...
    def _set_tangent_numeric(self):
        ...
        norms = np.array(list(map(lambda n: max(np.linalg.norm(n), 1e-08), v)))
        self.tangent = vectors.dot(v, 1.0 / norms)
    ...
    def _set_normal(self):
        ...
        R = vectors.rotation_matrix(0.25)
    ...
    def set_flavour(self, flavour):
        ...
        if flavour in self._flavour_simple_aliases:
            ...
            return
        if flavour in self._flavour_wiggly_aliases:
            self._set_xy = self._set_xy_wiggly
            self._flavour = 'wiggly'
            return
        if flavour in self._flavour_loopy_aliases:
    ...
    def add_arrow(self, *args, **kwargs):
        ...
        arrow_kwargs.setdefault('t', 0.5)
        arrow_kwargs.setdefault('width', 0.03)
        arrow_kwargs.setdefault('length', 0.09)
        ...
        if style == 'normal':
            ...
            raise ValueError('Wrong value for style.\n Allowed values : ' + str(self._arrow_style_allowed_values))

    def _get_normal_arrow(self, t=0.5, width=0.03, length=0.09, direction=1.0, **kwargs):
        """
        ...
        **kwargs :
        """
        for key, val in dict(color='k', zorder=12).items():
            kwargs.setdefault(key, val)

        center = self.path_point(t)
        if direction > 0.0:
            tip = center + 0.5 * length * self.tangent_point(t)
            back = tip - 0.5 * length * self.tangent_point(t)
        else:
            tip = center - 0.5 * length * self.tangent_point(t)
            back = tip + 0.5 * length * self.tangent_point(t)
        c1 = back + 0.5 * width * self.normal_point(t)
        c2 = back - 0.5 * width * self.normal_point(t)
        arrow = mpa.Polygon([tip, c1, c2], **kwargs)
        return arrow

    def _get_fancy_arrow(self, t=0.5, width=0.03, length=0.09, fancyness=0.09, direction=1.0, t_shift_dir=0.025, **kwargs):
        """
        Get a fancy arrow.
        ...
        if direction > 0.0:
            ...
            tip = center + 0.5 * length * tangent
            back = tip - 0.5 * length * tangent
            c1 = back + 0.5 * width * normal
            c2 = back - 0.5 * width * normal
    ...
    def text(self, s, t=0.5, y=-0.06, **kwargs):
        """
    ...
    def get_double_main_lines(self):
        ...
        style['linewidth'] = 0.5 * self.line_kwargs.get('linewidth', 2)
        lines.append(mpl.lines.Line2D(x, y, **style))
        return lines
    ...
    def _set_elliptic_linepath(self):
        ...
        ro = self.rstart + self.dr / 2 + vectors.rotate(self.dr, -0.25) * b * sin(beta) / norm(self.dr)
    ...
    def _set_circular_linepath(self):
        ...
        theta = tau * self.t
        circle = r * np.array([-cos(theta), -sin(theta) / c])
    ...
    def normal_point(self, t):
        return self.get_normal_point(t)
    ...
    def _set_xy_loopy(self):
        ...
        dx -= dx[0]
        dxy = self.xamp * vectors.dot(self.tangent, dx) + self.yamp * vectors.dot(self.normal, dy)
        self.xy = self.linepath + dxy
