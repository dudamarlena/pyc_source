...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/htconsole/web/index.py
# Compiled at: 2006-05-01 04:17:37
import simplejson, time, os, sys
...
class index(SitePage):
    ...
    def setup(self):
        if self.context is None:
            self.__class__.context = EvalContext(self.make_callback, extrabuiltins.ExtraBuiltins(self))
        self.title = 'Console'
        return

    def make_callback(self, callback_id):
        return self.app_url(_action__='callback', _id__=callback_id, __='true')

    def action_default(self):
        self.namespace = []
        for (name, value) in sorted(self.context.namespace.items()):
            if name.startswith('_'):
                continue
            self.namespace.append((name, self.context.html_repr(value)))

        self.saved_console = self.session.get('saved_console', '')
    ...
    def action_callback(self):
        ...
        if '__body__' in fields:
            ...
            fields.update(args)
        result = self.context.do_callback(id, **dict(fields))
        self.write(result)
        self.template = None
        return
    ...
    def action_run(self):
        ...
        lines = ['>>> %s' % lines[0]] + [ '... %s' % l for l in lines[1:] ]
