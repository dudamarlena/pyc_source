...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/htconsole/htmlrepr.py
# Compiled at: 2006-05-01 04:17:37
import dispatch, types, inspect, os, textwrap, itertools
...
def html_repr_object(obj, context, verbosity, interactive):
    ...
    if concrete_repr is object.__repr__:
        ...
        return '\n        <code class="py-repr">&lt;<span\n         title="%s">%s</span> object\n         %s&gt;</code>' % (class_name, obj.__class__.__name__, small_id)
    return '<code class="py-repr">%s</code>' % html_escape(repr(obj))


@html_repr.when('isinstance(obj, (list, tuple))')
def html_repr_list(obj, context, type, verbosity, interactive):
    if isinstance(obj, list):
        sep = '[]'
    else:
        sep = '()'
    content = ('').join([ '&nbsp; &nbsp; %s,<br>\n' % html_repr(item, context, verbosity - 1) for item in obj ])
    return '%s: %s<br>\n%s%s' % (type(obj).__name__, sep[0], content, sep[1])


@html_repr.when('isinstance(obj, types.FunctionType)')
def html_repr_func(obj, context, verbosity, interactive):
    args = inspect.formatargspec(*inspect.getargspec(obj))[1:-1]
    try:
...
def html_repr_module(obj, context, verbosity, interactive):
    ...
    if obj_dir == stdlib_dir:
        return 'Module <code class="py-module-name">%s</code> from <a href="%s" target="_blank">standard library</a>' % (obj.__name__, py_doc_base % obj.__name__)
    return 'Module <code class="py-module-name">%s</code> in <code class="py-module-file">%s</code>' % (obj.__name__, obj.__file__)
...
def html_repr_string(obj, context, verbosity, interactive):
    ...
    callback = context.get_js_callback(obj, _html_repr_string_long, insert_into=expand_id)
    return '<code id="%s">' % expand_id + r[:40] + '<a href="#" onclick="return %s" title="click to expand to full width (%i characters)">...</a>' % (html_escape(callback), len(r)) + r[-5:] + '</code>'
...
def html_repr_class(obj, context, verbosity, interactive):
    ...
    attrs = ('<br>\n').join([ '<code class="py-name">%s</code> = %s' % (html_escape(name), html_repr(value, context, verbosity - 1)) for (name, value) in attrs ])
    methods = sorted(methods.items())
    methods = ('<br>\n').join([ html_repr(value, context, verbosity - 1) for (name, value) in methods ])
    if not methods and not attrs and not doc:
        extra = '<code>pass</code>'
    else:
        extra = ''
    return '<code>class <b>%(name)s</b>%(bases)s:</code><blockquote class="py-class-body">\n%(doc)s %(attrs)s %(methods)s %(extra)s\n</blockquote>\n' % dict(name=cls_name, bases=bases, doc=doc or '', extra=extra, attrs=attrs, methods=methods)