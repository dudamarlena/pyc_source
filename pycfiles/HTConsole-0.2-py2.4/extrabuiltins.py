...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/htconsole/extrabuiltins.py
...
class ExtraBuiltins(object):
    ...
    def _show_load(self):
        save_dir = self.servlet.config['save_dir']
        files = [ fn for fn in os.listdir(save_dir) if not fn.startswith('.') ]
        options = []
        for file in files:
            file = html_escape(file)
            options.append('<option value="%s">%s</option>' % (file, file))

        id = self.context.get_id()
        html = '<div>load from: <select id="%(id)s">%(options)s</select><button onclick="return sendCommand(\'load(&quot;\'+$(\'%(id)s\').value+\'&quot;)\')">load</button><button onclick="return sendCommand(\'show_file(&quot;\'+$(\'%(id)s\').value+\'&quot;)\')">show</button></div>' % {'id': id, 'options': ('\n').join(options)}
    ...
    def show_file(self, filename):
        ...
        """
        js = "window.open('./?_action_=display&filename=%s', '_blank')" % html_escape(filename)
        self.result = {'result_javascript': js}
    ...
    def dir(self, obj=None):
        if obj is None:
            names = [ n for n in sorted(self.context.namespace.keys()) if not n.startswith('_') ]
        ...
        rows = []
        rows.append('<tr><th colspan="2">%s</th></tr>' % title)
        for name in names:
            try:
                value = getter(name)
            except Exception, e:
            ...
            rows.append('<tr><td class="py-label"><span title="%s">%s</span></td><td class="py-repr">%s</td></tr>' % (html_escape(h[:10]), html_escape(name), html))

        table = '<table class="py-dir">%s</table>' % ('').join(rows)
