...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/htconsole/evalcontext.py
# Compiled at: 2006-05-01 04:17:37
from cStringIO import StringIO
import traceback, threading, pdb, sys, re, weakref
from itertools import count
import time, simplejson, textwrap
...
class EvalContext(object):
    ...
    def snapshot(self):
        ...
        """
        return dict(((name, id(ob)) for (name, ob) in self.namespace.items() if name != '__builtins__'))
    ...
    def get_js_callback(self, obj, callback, weak=False, insert_into=None, js_args=None):
        """
        ...
        """
        js_args = js_args or {}
        uri = self.get_http_callback(obj, callback, weak=weak)
        js_args = [
         str(uri), insert_into, js_args]
    ...
    def do_callback(self, pair_id, *args, **kw):
        ...
        if pair_id not in self.callbacks:
            raise ObjectGone('The object by the id %s no longer exists; has the server been restarted?' % pair_id)
        ref = self.callbacks[pair_id]
        if isinstance(ref, weakref.ReferenceType):
            ref = ref()
        if ref is None:
            raise ObjectGone('The object by the id %s has been garbage collected and is not available' % pair_id)
        (obj, callback) = ref
    ...
    def objects_changed(self, snapshot):
        ...
        was taken
        """
        return dict(((name, ob) for (name, ob) in self.namespace.items() if id(ob) != snapshot.get(name) if name != '__builtins__'))

    def objects_removed(self, snapshot):
        """
        Returns a list of names removed since the snapshot
        """
        return [ name for name in snapshot.keys() if name not in self.namespace ]

    _func_re = re.compile('^def\\s+([a-z_][a-z0-9_]*).*[)]:[ \\t]*(?:[\\n\\r])?', re.I | re.S)
    ...
    def print_altrepr(self, value):
        if value is None:
            return
        self.stdout.write_html(self.html_repr(value, interactive=True))
        return
    ...
    def exec_expr_response(self, s):
        ...
        try:
            ...
            response = {'result_html': html, 'changed': {}}
...
def html_escape_quote(v):
    v = html_escape(v)
    v = v.replace('\n', '<br>\n')
    v = re.sub('  +', lambda m: '&nbsp;' * (len(m.group(0)) - 1) + ' ', v)
    return v