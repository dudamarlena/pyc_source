...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/shortfuse_test/concurrency.py
# Compiled at: 2019-03-06 13:38:37
# Size of source mod 2**32: 4075 bytes
import os, random
from multiprocessing import Pool
from integration import retry_assertion
...
def create_delete_file_test(root_path):
    ...
    for iteration in range(0, 10000):
        ...
        results.append(pool.apply_async(create_delete_file, (
         file_path,)))
...
def create_file_test(root_path):
    ...
    results = []
    file_paths = [os.path.join(root_path, 'create_delete_file_%s' % index) for index in range(0, 10)]
    file_contents = [build_random_string(204800) for _ in range(0, 10)]
    root_stats = os.stat(root_path)
    for iteration in range(0, 10000):
        file_path = file_paths[random.randint(0, len(file_paths) - 1)]
...
def create_delete_dir_test(root_path):
    ...
    for iteration in range(0, 10000):
        ...
        results.append(pool.apply_async(create_delete_dir, (
         file_path,)))
