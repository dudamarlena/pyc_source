...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/shortfuse/operations.py
# Compiled at: 2019-03-06 13:38:27
# Size of source mod 2**32: 17126 bytes
import logging, os
...
class Shortfuse(LoggingMixIn, Operations):
    ...
    def chmod(self, path, mode):
        ...
        """
        self.root.get_child(path).get_attributes().set_permissions(mode)
        return 0
    ...
    def chown(self, path, uid, gid):
        ...
        Returns:
            int: 0 if the operation succeeded, negative otherwise
        """
        self.root.get_child(path).get_attributes().set_uid(uid).set_gid(gid)
        return 0
    ...
    def flush(self, path, file_info):
        ...
        return node.get_descriptor(file_info.fh).flush()
    ...
    def fsync(self, path, datasync, file_info):
        ...
        if not isinstance(node, FileNode):
            raise FuseOSError(EISDIR)
        return node.get_descriptor(file_info.fh).sync(datasync == 0)
    ...
    def fsyncdir(self, path, datasync, fh):
        ...
        if not isinstance(node, DirectoryNode):
            raise FuseOSError(ENOTDIR)
        return node.get_descriptor(fh).sync(datasync == 0)
    ...
    def getattr(self, path, file_info=None):
        ...
        return child.get_attributes().get_all()
    ...
    def getxattr(self, path, name, position=0):
        ...
        """
        return self.root.get_child(path).get_extra_attributes().get_attribute(name)
    ...
    def listxattr(self, path):
        ...
        """
        return self.root.get_child(path).get_extra_attributes().get_attribute_names()
    ...
    def read(self, path, size, offset, file_info):
        ...
        if not isinstance(node, FileNode):
            raise FuseOSError(EISDIR)
        return node.get_descriptor(file_info.fh).read(size, offset)
    ...
    def removexattr(self, path, name):
        ...
        Returns:
           int: 0 if the operation succeeded, negative otherwise
        """
        self.root.get_child(path).get_extra_attributes().remove_attribute(name)
        return 0
    ...
    def rename(self, old, new):
        ...
        """
        return self.root.get_child(old).rename(new)
    ...
    def setxattr(self, path, name, value, options, position=0):
        ...
        Returns:
            int: 0 if the operation succeeded, negative otherwise
        """
        self.root.get_child(path).get_extra_attributes().set_attribute(name, value)
        return 0
    ...
    def statfs(self, path):
        ...
        """
        return self.root.get_child(path).get_fs_attributes().get_all()
    ...
    def truncate(self, path, length, file_info=None):
        ...
        node.get_descriptor(fh).truncate(length)
    ...
    def utimens(self, path, times=None):
        ...
        Returns:
            int: 0 if the operation succeeded, negative otherwise
        """
        self.root.get_child(path).get_attributes().set_access_time(times[0]).set_modified_time(times[0])
        return 0
    ...
    def write(self, path, data, offset, file_info):
        ...
        if not isinstance(node, FileNode):
            raise FuseOSError(EISDIR)
        return node.get_descriptor(file_info.fh).write(data, offset)