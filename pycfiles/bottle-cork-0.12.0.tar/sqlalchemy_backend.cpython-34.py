...
# [GCC 8.4.0]
# Embedded file name: /home/fede/newhome/projects/bottle-cork/tests/cork/sqlalchemy_backend.py
# Compiled at: 2015-11-07 14:08:33
# Size of source mod 2**32: 6889 bytes
"""
...
try:
    from sqlalchemy import create_engine, delete, select, Column, ForeignKey, Integer, MetaData, String, Table, Unicode
...
class SqlTable(base_backend.Table):
    __doc__ = 'Provides dictionary-like access to an SQL table.'
    ...
    def _row_to_value(self, row):
        ...
        row_value = SqlRowProxy(self, row_key, ((k, row[k]) for k in row.keys() if k != self._key_col.name))
...
class SqlSingleValueTable(SqlTable):
    ...
    def _row_to_value(self, row):
        return (
         row[self._key_col], row[self._col_name])
...
class SqlAlchemyBackend(base_backend.Backend):
    ...
    def __init__(self, db_full_url, users_tname='users', roles_tname='roles', pending_reg_tname='register', initialize=False, **kwargs):
        ...
        self._users = Table(users_tname, self._metadata, Column('username', Unicode(128), primary_key=True), Column('role', ForeignKey(roles_tname + '.role')), Column('hash', String(256), nullable=False), Column('email_addr', String(128)), Column('desc', String(128)), Column('creation_date', String(128), nullable=False), Column('last_login', String(128), nullable=False))
        self._roles = Table(roles_tname, self._metadata, Column('role', String(128), primary_key=True), Column('level', Integer, nullable=False))
        self._pending_reg = Table(pending_reg_tname, self._metadata, Column('code', String(128), primary_key=True), Column('username', Unicode(128), nullable=False), Column('role', ForeignKey(roles_tname + '.role')), Column('hash', String(256), nullable=False), Column('email_addr', String(128)), Column('desc', String(128)), Column('creation_date', String(128), nullable=False))
