...
# Embedded file name: build/bdist.macosx-10.3-i386/egg/aha/modelcontroller/crudcontroller.py
# Compiled at: 2010-10-22 05:16:45
__doc__ = ' crudcontroller.py - Classes to handle CRUD form for a model.\n\n$Id: crudcontroller.py 652 2010-08-23 01:58:52Z ats $\n'
...
class CRUDHandlerMetaClass(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        new_class = super(CRUDHandlerMetaClass, cls).__new__(cls, name, bases, attrs)
        return new_class
...
class CRUDHandlerBase(object):
    ...
    def __call__(self, controller):
        ...
        controller.form['session_key'] = HiddenField(name='session_key', default=key)
    ...
    def do_validate(self, state, controller):
        ...
        if state == FC.INITIAL:
            ...
            controller.form.values = dict([ (x, controller.request.get(x)) for x in controller.request.arguments()
                                          ])
...
class ModelCRUDController(MakoTemplateController, CRUDControllerMixIn):
    ...
    def index(self):
        ...
        self.page_title = self.LISTPAGE_TITLE or 'List of %s' % self.MODEL.__class__.__name__
        self.path = self.BASEPATH
        self.objects = self.get_index_object(start, end)
        self.prev = 'Prev'
        self.next = 'Next'
        if start:
            self.prev = '<a href = "%s%s">前</a>' % (
             self.path, max(start - self.PAGE_SIZE, 0))
        self.next = '<a href = "%s%s">次</a>' % (self.path, start + self.PAGE_SIZE)
