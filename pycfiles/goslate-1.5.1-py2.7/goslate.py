...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/goslate.py
# Compiled at: 2016-01-04 11:39:16
"""Goslate: Free Google Translate API
"""
from __future__ import print_function
from __future__ import unicode_literals
import sys, os, json, itertools, functools, time, socket, random, re
try:
...
__author__ = b'ZHUO Qiang'
__email__ = b'zhuo.qiang@gmail.com'
__copyright__ = b'2013, http://zhuoqiang.me'
__license__ = b'MIT'
__date__ = b'2013-05-11'
__version_info__ = (1, 5, 1)
__version__ = (b'.').join(str(i) for i in __version_info__)
__home__ = b'https://bitbucket.org/zhuoqiang/goslate'
__download__ = b'https://pypi.python.org/pypi/goslate'
try:
    unicode
except NameError:
    unicode = str
...
def _is_sequence(arg):
    return not isinstance(arg, unicode) and not isinstance(arg, bytes) and (hasattr(arg, b'__getitem__') or hasattr(arg, b'__iter__'))
...
class Goslate(object):
    ...
    def __init__(self, writing=WRITING_NATIVE, opener=None, retry_times=4, executor=_g_executor, timeout=4, service_urls=('http://translate.google.com', ), debug=False):
        self._DEBUG = debug
    ...
    def _open_url(self, url):
        if len(url) > self._MAX_LENGTH_PER_QUERY + 100:
            raise Error(b'input too large')
        request = Request(url, headers={b'User-Agent': b'Mozilla/4.0'})
        exception = None
        for i in range(self._RETRY_TIMES):
            try:
                response = self._opener.open(request, timeout=self._TIMEOUT)
                response_content = response.read().decode(b'utf-8')
                if self._DEBUG:
                    print((b'GET Response body:{}').format(response_content))
                return response_content
            except socket.error as e:
                if self._DEBUG:
                    import threading
                    print(threading.currentThread(), e)
                if b'Connection reset by peer' not in str(e):
                    raise e
                exception = e
                time.sleep(0.0001)
    ...
    def _basic_translate(self, text, target_language, source_language):
        if not target_language:
            raise Error(b'invalid target language')
        if not text.strip():
            return (tuple(b'' for i in range(len(self._writing))), unicode(target_language))
        GOOGLE_TRASLATE_URL = urljoin(random.choice(self._service_urls), b'/translate_a/single')
        GOOGLE_TRASLATE_PARAMETERS = {b'client': b'a', 
           b'sl': source_language, 
           b'tl': target_language, 
           b'ie': b'UTF-8', 
           b'oe': b'UTF-8', 
           b'dt': b't', 
           b'q': text}
        url = (b'?').join((GOOGLE_TRASLATE_URL, urlencode(GOOGLE_TRASLATE_PARAMETERS)))
        if b'translit' in self._writing:
            url += b'&dt=rm'
        response_content = self._open_url(url)
        raw_data = json.loads(_empty_comma.subn(b'', response_content)[0].replace(b'\xa0', b' ').replace(b'[,', b'[1,'))
        data = {b'src': raw_data[(-1)][0][0]}
        if raw_data[0][(-1)][0] == 1:
            data[b'translit'] = raw_data[0][(-1)][1]
            data[b'trans'] = (b'').join(i[0] for i in raw_data[0][:-1])
        else:
            data[b'translit'] = b''
            data[b'trans'] = (b'').join(i[0] for i in raw_data[0])
        translation = tuple(data[part] for part in self._writing)
        detected_source_language = data[b'src']
    ...
    def get_languages(self):
        ...
        GOOGLE_TRASLATOR_URL = b'http://translate.google.com/translate_a/l'
        GOOGLE_TRASLATOR_PARAMETERS = {b'client': b't'}
        url = (b'?').join((GOOGLE_TRASLATOR_URL, urlencode(GOOGLE_TRASLATOR_PARAMETERS)))
        response_content = self._open_url(url)
        data = json.loads(response_content)
        languages = data[b'sl']
        languages.update(data[b'tl'])
        if b'auto' in languages:
            del languages[b'auto']
        if b'zh' not in languages:
            languages[b'zh'] = b'Chinese'
        self._languages = languages
        return self._languages

    _SEPERATORS = [ quote_plus(i.encode(b'utf-8')) for i in b'.!?,;。，？！:："“”’‘#$%&()（）*×+/<=>@＃￥[\\]…［］^`{|}｛｝～~\n\r\t '
                  ]
    ...
    def _translate_single_text(self, text, target_language, source_lauguage):
        ...
        return tuple((b'').join(i[n] for i in results) for n in range(len(self._writing)))

    def translate(self, text, target_language, source_language=b'auto'):
        u"""Translate text from source language to target language
        ...
        if not source_language:
            source_language = b'auto'
        if target_language.lower() == b'zh':
            target_language = b'zh-CN'
        if source_language.lower() == b'zh':
            source_language = b'zh-CN'
        if not _is_sequence(text):
            if isinstance(text, unicode):
                text = text.encode(b'utf-8')
            return _unwrapper_single_element(self._translate_single_text(text, target_language, source_language))
        JOINT = b'⛿'
        UTF8_JOINT = (b'\n%s\n' % JOINT).encode(b'utf-8')
        ...
        return (_unwrapper_single_element(i) for i in itertools.chain.from_iterable(self._execute(make_task(i) for i in join_texts(text))))

    def _detect_language(self, text):
        if _is_bytes(text):
            text = text.decode(b'utf-8')
        return self._basic_translate(text[:50].encode(b'utf-8'), b'en', b'auto')[1]
    ...
    def lookup_dictionary(self, text, target_language, source_language=b'auto', examples=False, etymology=False, pronunciation=False, related_words=False, synonyms=False, antonyms=False, output_language=None):
        ...
        parameters = [
         ...
         ('dt', 'bd')]
        if output_language:
            parameters.append((b'hl', output_language))
        if examples:
        ...
        url = (b'?').join((GOOGLE_TRASLATE_URL, urlencode(parameters)))
        response_content = self._open_url(url)
        raw_data = json.loads(_empty_comma.subn(b'', response_content)[0].replace(b'\xa0', b' ').replace(b'[,', b'[1,'))
        return raw_data
...
def _main(argv):
    import optparse
    usage = b'usage: %prog [options] <file1 file2 ...>\n<stdin> will be used as input source if no file specified.'
    parser = optparse.OptionParser(usage=usage, version=b'%%prog %s @ Copyright %s' % (__version__, __copyright__))
    parser.add_option(b'-t', b'--target-language', metavar=b'zh-CN', help=b'specify target language to translate the source text into')
    parser.add_option(b'-s', b'--source-language', default=b'auto', metavar=b'en', help=b'specify source language, if not provide it will identify the source language automatically')
    parser.add_option(b'-i', b'--input-encoding', default=sys.getfilesystemencoding(), metavar=b'utf-8', help=b'specify input encoding, default to current console system encoding')
    parser.add_option(b'-o', b'--output-encoding', default=sys.getfilesystemencoding(), metavar=b'utf-8', help=b'specify output encoding, default to current console system encoding')
    parser.add_option(b'-r', b'--roman', action=b'store_true', help=b'change translation writing to roman (e.g.: output pinyin instead of Chinese charactors for Chinese. It only valid for some of the target languages)')
    options, args = parser.parse_args(argv[1:])
    if not options.target_language:
        print(b'Error: missing target language!')
        parser.print_help()
        return
    ...
    import fileinput
    inputs = fileinput.input(args, mode=b'rb')
