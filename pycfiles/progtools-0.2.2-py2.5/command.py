...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/progtools/command.py
...
class Command(object):
    ...
    def run_with_argv(self, argv=None, parser=None, verbose=True):
        ...
        return program.saferun(self.run, [args, options], usage=self.usage, verbose=verbose)
...
class HelpCommand(Command):
    ...
    def _display_help(self, name):
        ...
        if not command.synopsis and not command.usage and not command.description:
            print 'No documentation'
...
class Registry(object):
    ...
    def unregister(self, command):
        ...
        """
        key_list = [ key for key in self.__commands if type(self.__commands[key]) == command
                   ]
        if not key_list:
