...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/xbus/broker/core/back/envelope.py
# Compiled at: 2016-06-27 03:37:50
# Size of source mod 2**32: 22678 bytes
import asyncio
from xbus.broker.core.back.event import Event
__author__ = 'jgavrel'

class Envelope(object):
    __doc__ = 'An Envelope instance represents a transactional unit and controls its\n    execution through the network. It can contain several events.'

    def __init__(self, envelope_id: str, db=None,
    ...
    def new_event(self, event_id, type_name, type_id):
        ...
        """
        event = Event(self.envelope_id, event_id, type_name, type_id, self.redis_pool, self.loop)
    ...
    def end_envelope(self):
        ...
        for node in consumer_nodes:
            task = asyncio.async(self.consumer_end_envelope(node), loop=self.loop)
            tasks.append(task)
    ...
    def worker_start_event(self, node, event) -> bool:
        ...
        if self.stopped:
            ...
            call = node.recipient.socket.call.start_event(self.envelope_id, event.event_id, event.type_name)
            try:
            ...
            if success:
                ...
                node.next_trigger()
                return True
            yield from self.db.log_event_errors(reply, self.envelope_id, event.event_id, node.node_id)
            asyncio.async(self.stop_envelope(), loop=self.loop)
            return False

    @asyncio.coroutine
    def worker_send_item(self, node, event, indices: list, data: bytes, forward_index: int) -> bool:
        ...
        while qsize:
            ...
            res = yield from self._worker_send_item(node, event, qindices, qdata, forward_index - qsize)
            if res is False:
                return False
    ...
    def _worker_send_item(self, node, event, indices: list, data: bytes, forward_index: int):
        call = node.recipient.socket.call.send_item(self.envelope_id, event.event_id, indices, data)
        try:
        ...
        if success:
            for child_id in node.children:
                ...
                for i, (rep_indices, rep_data) in enumerate(reply):
                    ...
                    task = coro(child, event, rep_indices, rep_data, node.sent + i)
    ...
    @asyncio.coroutine
    def worker_end_event(self, node, event, nb_items: int, immediate_reply: bool) -> bool:
        ...
        while qsize:
            qsize -= 1
            qindices, qdata = yield from node.pop_queue()
            res = yield from self._worker_send_item(node, event, qindices, qdata, nb_items - qsize - 1)
            if res is False:
                return False

        call = node.recipient.socket.call.end_event(self.envelope_id, event.event_id)
        try:
        ...
        if success:
            for child_id in node.children:
                ...
                asyncio.async(coro(child, event, node.sent, immediate_reply), loop=self.loop)
    ...
    def worker_end_envelope(self, node) -> bool:
        ...
        if self.stopped:
            ...
            if success:
                return True
            yield from self.db.log_event_errors(reply, self.envelope_id, None, node.node_id)
            return False
    ...
    def consumer_start_event(self, node, event) -> bool:
        ...
        for recipient in node.recipients:
            call = recipient.socket.call.start_event(self.envelope_id, event.event_id, event.type_name)
        ...
        try:
            ...
            errors = [reply for success, replies in res if not success for reply in replies]
        except (TypeError, ValueError):
        ...
        if not errors:
            ...
            node.next_trigger()
            return True
        yield from self.db.log_event_errors(errors, self.envelope_id, event.event_id, node.node_id)
        asyncio.async(self.stop_envelope(), loop=self.loop)

    @asyncio.coroutine
    def consumer_send_item(self, node, event, indices: list, data: bytes, forward_index: int) -> bool:
        ...
        while qsize:
            qsize -= 1
            qindices, qdata = yield from node.pop_queue()
            res = yield from self._consumer_send_item(node, event, qindices, qdata, forward_index - qsize)
            if res is False:
                return False
    ...
    def _consumer_send_item(self, node, event, indices: list, data: bytes, forward_index: int):
        tasks = []
        for recipient in node.recipients:
            call = recipient.socket.call.send_item(self.envelope_id, event.event_id, indices, data)
    ...
    @asyncio.coroutine
    def consumer_end_event(self, node, event, nb_items: int, immediate_reply: bool) -> tuple:
        ...
        while qsize:
            qsize -= 1
            qindices, qdata = yield from node.pop_queue()
            res = yield from self._consumer_send_item(node, event, qindices, qdata, nb_items - qsize - 1)
            if res is False:
                return False

        tasks = []
        for recipient in node.recipients:
            call = recipient.socket.call.end_event(self.envelope_id, event.event_id)
        ...
        try:
            ...
            errors = [reply for success, replies in reply_data if not success for reply in replies]
        except (TypeError, ValueError):
            errors = [([], 'Malformed reply to end_envelope.')]
        except asyncio.TimeoutError:
            errors = [([], 'Consumer timed out.')]
        ...
        if immediate_reply:
            ...
            immediate_reply_data = first_reply_data[1]
    ...
    def consumer_end_envelope(self, node) -> bool:
        ...
        if self.stopped:
            ...
            if not errors:
                return True
            yield from self.db.log_event_errors(errors, self.envelope_id, None, node.node_id)
            asyncio.async(self.stop_envelope(), loop=self.loop)
            return False
