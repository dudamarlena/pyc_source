...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/xbus/broker/core/db.py
# Compiled at: 2016-06-27 03:37:50
# Size of source mod 2**32: 21279 bytes
...
class DatabaseUtils(object):
    ...
    def check_event_access(self, profile_id: str, type_id: str) -> bool:
        ...
        query = query.where(and_(emitter_profile_event_type_rel.c.profile_id == profile_id, emitter_profile_event_type_rel.c.event_id == type_id))
        with (yield from self.dbengine) as (conn):
            cr = yield from conn.execute(query)
    ...
    def log_new_envelope(self, envelope_id: str, emitter_id: str):
        ...
        insert = insert.values(id=envelope_id, state='open', emitter_id=emitter_id, posted_date=func.localtimestamp())
        with (yield from self.dbengine) as (conn):
    ...
    @asyncio.coroutine
    def log_new_event(self, event_id: str, envelope_id: str, emitter_id: str, type_id: str, estimate: int, origin_event_id: str=None):
        ...
        """
        insert = event.insert()
        insert = insert.values(id=event_id, envelope_id=envelope_id, emitter_id=emitter_id, type_id=type_id, estimated_items=estimate, execution_state='wait', tracking_state='unprocessed', origin_event_id=origin_event_id)
        with (yield from self.dbengine) as (conn):
            yield from conn.execute(insert)
    ...
    def log_inactive_consumers(self, event_id: str, inactive: list):
        ...
        insert = insert.values([dict(event_id=event_id, node_id=i[0], role_id=i[1], was_unavailable=i[2]) for i in inactive])
        with (yield from self.dbengine) as (conn):
            yield from conn.execute(insert)
    ...
    def check_consumer_inactivity(self, role_id: str):
        ...
        query = query.where(and_(event_consumer_inactive_rel.c.role_id == role_id, event_consumer_inactive_rel.c.retry_event_id.is_(None)))
        with (yield from self.dbengine) as (conn):
            cr = yield from conn.execute(query)
            res = yield from cr.fetchall()
            if res:
                return list(zip(row[0] for row in res))[0]
            else:
                return []

    @asyncio.coroutine
    def update_active_consumers(self, event_id: str, role_ids: list, retry_event_id: str):
        ...
        update = update.where(and_(event_consumer_inactive_rel.c.event_id == event_id, event_consumer_inactive_rel.c.role_id.in_(role_ids)))
    ...
    def get_waiting_events(self):
        ...
        query = query.where(and_(event.c.execution_state == 'wait', event.c.retry_event_id.is_(None)))
        with (yield from self.dbengine) as (conn):
            cr = yield from conn.execute(query)
            res = yield from cr.fetchall()
            return (row.as_tuple() for row in res)
    ...
    def update_event_state_exec(self, event_id: str):
        ...
        update = event.update()
        update = update.where(event.c.id == event_id)
        update = update.values(execution_state='exec', started_date=func.localtimestamp())
        with (yield from self.dbengine) as (conn):
            yield from conn.execute(update)
    ...
    def update_events_state_done(self, done_event_ids, part_event_ids):
        ...
        if done_event_ids:
            update_done = update_done.where(event.c.id.in_(done_event_ids))
            update_done = update_done.values(execution_state='done', done_date=func.localtimestamp())
        update_part = event.update()
        if part_event_ids:
            update_part = update_part.where(event.c.id.in_(part_event_ids))
            update_part = update_part.values(execution_state='part', done_date=func.localtimestamp())
        with (yield from self.dbengine) as (conn):
            if done_event_ids:
    ...
    @asyncio.coroutine
    def log_event_errors(self, errors, envelope_id, event_id=None, node_id=None):
        ...
        """
        error_iter = (dict(id=uuid4(), envelope_id=envelope_id, event_id=event_id, node_id=node_id, items=items, message=message, error_date=datetime.utcnow(), state='unprocessed') for items, message in self.error_format(errors))
        with (yield from self.dbengine) as (conn):
