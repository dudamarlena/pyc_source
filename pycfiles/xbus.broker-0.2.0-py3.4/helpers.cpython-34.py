...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/xbus/broker/model/helpers.py
...
def get_event_tree(dbengine, event_type_id):
    query = select([
     ...
     func.array_agg(event_node_rel.c.child_id, type_=UUIDArray(remove_null=True)).label('child_ids')])
    query = query.where(event_node.c.type_id == event_type_id)
    query = query.select_from(join(event_node, event_node_rel, event_node_rel.c.parent_id == event_node.c.id, isouter=True))
...
def get_consumer_roles(dbengine):
    ...
    query = query.select_from(join(service, role, role.c.service_id == service.c.id))
