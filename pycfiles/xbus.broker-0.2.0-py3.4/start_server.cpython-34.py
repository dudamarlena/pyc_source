...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/xbus/broker/cli/start_server.py
...
def signal_handler(_signal, frame):
    ...
    place!
    """
    logger.warning('received signal {} during frame {}'.format(_signal, frame))
...
def get_engine(config):
    ...
    """
    dbengine = yield from create_engine(dsn=config.get('database', 'sqlalchemy.dburi'))
    return dbengine
...
def start_all(config, loop=None) -> None:
    ...
    coroutines = [
     get_frontserver(get_engine, config, front_socket_name, b2f_socket_name, loop=loop),
     get_backserver(get_engine, config, back_socket_name, b2f_socket_name, loop=loop)]
