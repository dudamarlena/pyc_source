...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/xbus/broker/core/front/rpc.py
# Compiled at: 2016-06-27 03:37:50
# Size of source mod 2**32: 27843 bytes
import asyncio, json, logging, aiozmq
...
class XbusBrokerFront(XbusBrokerBase):
    ...
    def login(self, login: str, password: str) -> str:
        ...
        if emitter_id and validate_password(password, emitter_pwd):
            ...
            info = {'id': emitter_id,  'login': login,  'profile_id': emitter_profile_id}
    ...
    def start_envelope(self, token: str) -> str:
        ...
        info = {'emitter_id': emitter_id, 
         'events': {},  'trigger': asyncio.Future(loop=self.loop), 
         'forward': True}
        self.envelopes[envelope_id] = info
        yield from self.db.log_new_envelope(envelope_id, emitter_id)
        asyncio.async(self.backend_start_envelope(envelope_id), loop=self.loop)
        return envelope_id

    @rpc.method
    @asyncio.coroutine
    def start_event(self, token: str, envelope_id: str, event_name: str, estimate: int) -> str:
        ...
        if envelope_closed:
            return ''
        type_id, immediate_reply = yield from self.db.find_event_type_by_name(event_name)
        access = yield from self.db.check_event_access(profile_id, type_id)
        if access is False:
            log.warning('Access denied. Emitter profile: %s. Event type: %s - %s.' % (
             profile_id, event_name, type_id))
            return ''
        event_id = self.new_event()
        info = {'envelope_id': envelope_id, 
        ...
        yield from self.db.log_new_event(event_id, envelope_id, emitter_id, type_id, estimate)
        if envelope_forward:
            asyncio.async(self.backend_start_event(envelope_id, event_id, type_id, event_name), loop=self.loop)
        return event_id

    @rpc.method
    @asyncio.coroutine
    def send_item(self, token: str, envelope_id: str, event_id: str, data: bytes) -> bool:
        ...
        if event_forward:
            asyncio.async(self.backend_send_item(envelope_id, event_id, index, data), loop=self.loop)
        return True
    ...
    def end_event(self, token: str, envelope_id: str, event_id: str) -> tuple:
        ...
        result = (True, None)
        if event_forward:
            coro = self.backend_end_event(envelope_id, event_id, sent_items, immediate_reply)
    ...
    def end_envelope(self, token: str, envelope_id: str) -> bool:
        ...
        if envelope_forward:
            asyncio.async(self.backend_end_envelope(envelope_id), loop=self.loop)
        return True
    ...
    def cancel_envelope(self, token: str, envelope_id: str) -> bool:
        ...
        if envelope_forward:
            asyncio.async(self.backend_cancel_envelope(envelope_id), loop=self.loop)
        return True
    ...
    @asyncio.coroutine
    def backend_start_event(self, envelope_id: str, event_id: str, type_id: str, type_name: str) -> bool:
        ...
        if forward is False:
            ...
            code, msg, inactive_ids = yield from self.backend.call.start_event(envelope_id, event_id, type_id, type_name)
            if code == 0:
                event_info['sent'] = 0
    ...
    @asyncio.coroutine
    def backend_send_item(self, envelope_id: str, event_id: str, index: int, data: bytes):
        ...
        code, msg = yield from self.backend.call.send_item(envelope_id, event_id, index, data)
        if code == 0:
            event_info['sent'] += 1
            if event_info['trigger']._callbacks:
                event_info['trigger'].set_result(True)
    ...
    @asyncio.coroutine
    def backend_end_event(self, envelope_id: str, event_id: str, nb_items: int, immediate_reply: bool) -> tuple:
        ...
        call_data = yield from self.backend.call.end_event(envelope_id, event_id, nb_items, immediate_reply)
    ...
    def backend_end_envelope(self, envelope_id: str):
        ...
        while any(not evt.get('be_closed', False) and evt.get('forward', True) for evt in events):
            trigger_res = yield from envelope_info['trigger']
            if trigger_res is False:
                return False

        log.debug('Front: forwarding end_envelope to back for envelope: {}'.format(envelope_id))
...
def get_frontserver(engine_callback, config, socket, b2fsocket, loop=None):
    ...
    front_from_back_zqm = yield from rpc.serve_rpc(front2back, bind=b2fsocket, loop=loop)
    coroutines = [
...
__all__ = [
 'XbusBrokerFront', 'get_frontserver']