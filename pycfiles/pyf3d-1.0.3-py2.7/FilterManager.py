...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyF3D/FilterManager.py
# Compiled at: 2017-04-27 11:57:47
import pyopencl as cl, time, concurrent.futures as cf, numpy as np, ClAttributes, FilterAttributes, filters.MedianFilter as mf, filters.FFTFilter as fft, filters.BilateralFilter as bf, filters.MaskFilter as mskf, filters.MMFilterDil as mmdil, filters.MMFilterEro as mmero, filters.MMFilterClo as mmclo, filters.MMFilterOpe as mmope, FilterClasses as fc, threading
startIndex = 0
...
def runPipeline(image, pipeline, platform=None):
    ...
    with cf.ThreadPoolExecutor(len(platform)) as (e):
        for i in range(len(platform)):
            ...
            kwargs = {'image': image, 'pipeline': pipeline, 'attr': atts, 'platform': p, 'sliceCount': maxSliceCount, 'index': index, 'stacks': stacks}
...
def run_FFTFilter(image, FFTChoice='Forward', platform=None):
    ...
    """
    pipeline = [
     fft.FFTFilter(FFTChoice=FFTChoice)]
    stacks = runPipeline(image, pipeline, platform=platform)
    return reconstruct_final_image(stacks)
...
def run_BilateralFilter(image, spatialRadius=3, rangeRadius=30, platform=None):
    ...
    """
    pipeline = [
     bf.BilateralFilter(spatialRadius=spatialRadius, rangeRadius=rangeRadius)]
    stacks = runPipeline(image, pipeline, platform=platform)
    return reconstruct_final_image(stacks)
...
def run_MaskFilter(image, maskChoice='mask3D', mask='StructuredElementL', L=3, platform=None):
    ...
    """
    pipeline = [
     mskf.MaskFilter(maskChoice=maskChoice, mask=mask, L=L)]
    stacks = runPipeline(image, pipeline, platform=platform)
    return reconstruct_final_image(stacks)
...
def run_MMFilterDil(image, mask='StructuredElementL', L=3, platform=None):
    ...
    """
    pipeline = [
     mmdil.MMFilterDil(mask=mask, L=L)]
    stacks = runPipeline(image, pipeline, platform=platform)
    return reconstruct_final_image(stacks)
...
def run_MMFilterEro(image, mask='StructuredElementL', L=3, platform=None):
    ...
    """
    pipeline = [
     mmero.MMFilterEro(mask=mask, L=L)]
    stacks = runPipeline(image, pipeline, platform=platform)
    return reconstruct_final_image(stacks)
...
def run_MMFilterClo(image, mask='StructuredElementL', L=3, platform=None):
    ...
    """
    pipeline = [
     mmclo.MMFilterClo(mask=mask, L=L)]
    stacks = runPipeline(image, pipeline, platform=platform)
    return reconstruct_final_image(stacks)
...
def run_MMFilterOpe(image, mask='StructuredElementL', L=3, platform=None):
    ...
    """
    pipeline = [
     mmope.MMFilterOpe(mask=mask, L=L)]
    stacks = runPipeline(image, pipeline, platform=platform)
    return reconstruct_final_image(stacks)
