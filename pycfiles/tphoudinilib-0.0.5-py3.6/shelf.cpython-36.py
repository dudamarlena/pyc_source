...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpHoudiniLib/core/shelf.py
...
class HoudiniShelf(abstract_shelf.AbstractShelf, object):

    def __init__(self, name='HoudiniShelf', label_background=(0, 0, 0, 0), label_color=(0.9, 0.9, 0.9), category_icon=None):
        super(HoudiniShelf, self).__init__(name=name, label_background=label_background, label_color=label_color, category_icon=category_icon)
        self.name = name
    ...
    def create(self, delete_if_exists=True):
        ...
        if delete_if_exists:
            ...
            assert not gui.shelf_set_exists(shelf_set_name=(self.name)), 'Shelf Set with name {} already exists!'.format(self.name)
    ...
    def add_button(self, label, tooltip=None, icon='customIcon.png', command=None, double_command=None, command_type='python'):
        ...
        :return:
        """
        return gui.create_shelf_tool(tool_name=('{}_{}'.format(self.name, label)),
          tool_label=label,
    ...
    def add_separator(self):
        ...
        :return:
        """
        return tpDccLib.Dcc.add_shelf_separator(shelf_name=(self.name))
                ...
                if item != category_name:
                    ...
                    for i in item_data:
                        ...
                        if annotation == 'separator':
                            ...
                            dcc = i.get('dcc')
                            if tpDccLib.Dcc.get_name() not in dcc:
                                continue
