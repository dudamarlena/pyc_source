...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpHoudiniLib/core/gui.py
...
def create_shelf_tool(tool_name, tool_label, tool_script, tool_type='python', icon=None, help=None):
    ...
    if tool_type == 'python':
        ...
        if tool_type == 'hscript':
            language = hou.scriptLanguage.HScript
    if language is None:
        tp.logger.warning('Impossible to create shelf tool {} because script language {} is not supported by Houdini'.format(tool_name, tool_type))
        return
    else:
