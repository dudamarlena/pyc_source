...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/material_ui/flatui/popups.py
# Compiled at: 2015-03-20 17:22:30
import sys
...
class FlatPopup(ModalView):
    ...
    separator_color = ListProperty([47 / 255.0, 167 / 255.0, 212 / 255.0, 1.0])
    ...
    def __init__(self, **kargs):
        if 'separator_height' not in kargs.keys():
...
class AlertPopup(FlatPopup):
    ...
    ok_button_color = ListProperty([0, 0.59, 0.53, 1])
    ok_button_color_down = ListProperty([0, 0.41, 0.36, 1])
    ...
    cancel_button_color = ListProperty([0, 0.59, 0.53, 1])
    cancel_button_color_down = ListProperty([0, 0.41, 0.36, 1])

    def __init__(self, **kargs):
        if 'title' not in kargs.keys():
            kargs['title'] = 'Warning'
        if 'title_size' not in kargs.keys():
            kargs['title_size'] = dp(16)
        if 'size_hint' not in kargs.keys():
            kargs['size_hint'] = (0.4, 0.3)
        if 'title_color' not in kargs.keys():
            kargs['title_color'] = (0, 0, 0, 0.8)
        super(AlertPopup, self).__init__(**kargs)
        ok_button = flatui.FlatButton(text=self.ok_button_text, markup=True, color=self.ok_button_color, color_down=self.ok_button_color_down)
        ok_button.bind(on_press=self.on_ok)
        cancel_button = flatui.FlatButton(text=self.cancel_button_text or '', markup=True, color=self.cancel_button_color, color_down=self.cancel_button_color_down)
        cancel_button.bind(on_press=self.on_cancel)
        button_bar = BoxLayout(orientation='horizontal', size_hint=(1, None), height=dp(55), spacing=dp(10), padding=[10, 10, 10, 10])
        button_bar.add_widget(BoxLayout())
        if self.cancel_button_text:
            button_bar.add_widget(cancel_button)
        button_bar.add_widget(ok_button)
        self.content = BoxLayout(orientation='vertical')
        lbl = Label(text=self.text, size_hint=(1, 0.9), color=self.title_color)
    ...
    def on_ok(self, *args):
        self.dismiss()
        if self.ok_button_on_press:
            self.ok_button_on_press(*args)

    def on_cancel(self, *args):
        self.dismiss()
        if self.cancel_button_on_press:
            self.cancel_button_on_press(*args)
...
class OkButtonPopup(FlatPopup):
    ...
    def __init__(self, **kargs):
        ...
        if 'title_size' not in kargs.keys():
            kargs['title_size'] = dp(16)
        if 'size_hint' not in kargs.keys():
            kargs['size_hint'] = (0.8, 0.5)
        if 'title_color' not in kargs.keys():
            kargs['title_color'] = (0, 0, 0, 0.8)
        super(OkButtonPopup, self).__init__(**kargs)
        ok_button = flatui.FlatButton(text=self.ok_button_text, markup=True, size_hint=(0.2,
                                                                                        1), color=self.ok_button_color, color_down=self.ok_button_color_down)
        ok_button.bind(on_press=self.on_ok)
        button_bar = BoxLayout(orientation='horizontal', size_hint=(1, None), height=dp(55), spacing=dp(10), padding=[10, 10, 10, 10])
        button_bar.add_widget(BoxLayout(size_hint=(0.8, 1)))
        button_bar.add_widget(ok_button)
        self.content = BoxLayout(orientation='vertical')
        lbl = Label(text=self.text, color=self.title_color, size_hint=(1, 0.8))
        self.content.add_widget(lbl)
        self.content.add_widget(button_bar)
        return
...
class PopupListView(FlatPopup):
    ...
    def _build_list_view(self, kargs):
        self._list_view = ListView(adapter=self._build_adapter(), propagate_selection_to_data=True)
    ...
    def adapter_converter(self):
        return lambda i, o: {'is_selected': o['is_selected'], 
           'size_hint_y': None, 
    ...
    def _build_adapter(self):
        self.list_adapter = ListAdapter(cls=ListItemButton, data=self.list_data, args_converter=self.adapter_converter(), selection_mode='single', allow_empty_selection=True)
