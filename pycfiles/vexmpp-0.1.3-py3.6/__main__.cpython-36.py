...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vexmpp/vex/__main__.py
# Compiled at: 2017-02-05 20:20:57
# Size of source mod 2**32: 4480 bytes
import os, sys, asyncio, logging
from getpass import getpass
from nicfit.aio import Application
from nicfit.console import perr
import aiodns, OpenSSL.SSL
from ..jid import Jid
from .. import version
from ..errors import XmppError
from ..stanzas import Presence
from ..protocols import iqversion, stream_mgmt
from ..client import Credentials, DEFAULT_C2S_PORT, ClientStream, ClientStreamCallbacks, TlsOpts
...
async def main(args):
    ...
    try:
        ...
        stream = await ClientStream.connect((Credentials(jid, password)), host=(args.host),
          port=(args.port),
...
about = 'Simple XMPP client. The user will be prompted for a login password unless the environment variable VEX_PASSWD is set.'
...
app.arg_parser.add_argument('--register', action='store_true', help='Register for an account before logging in.')
app.arg_parser.add_argument('--host', help='Alternative server for connecting')
app.arg_parser.add_argument('--port', type=int, default=DEFAULT_C2S_PORT, help='Alternative port for connecting')
app.arg_parser.add_argument('--disconnect', action='store_true', help='Disconnect once stream negotiation completes.')
optgroup = app.arg_parser.add_argument_group('Stream feature options')
optgroup.add_argument('--tls', action='store', default='on',
  choices=[e.name for e in TlsOpts],
  help="TLS setting, 'on' by default.")
optgroup.add_argument('--stream-mgmt', action='store_true', default=False, help='Enable stream management (XEP 198).')
logging.basicConfig()
