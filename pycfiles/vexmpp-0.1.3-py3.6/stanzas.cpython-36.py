...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vexmpp/stanzas.py
# Compiled at: 2017-02-05 18:34:07
# Size of source mod 2**32: 16423 bytes
import uuid, functools
from copy import deepcopy
from lxml import etree
from .namespaces import XML_NS_URI, STREAM_NS_URI, CLIENT_NS_URI, SERVER_NS_URI, STANZA_ERROR_NS_URI, STREAM_ERROR_NS_URI
from .jid import Jid
...
class StreamHeader(Stanza):
    XPATH = (
     '/stream:stream', {'stream': STREAM_NS_URI})

    def __init__(self, ns=CLIENT_NS_URI, to=None, frm=None, version='1.0', lang='en', id=None, xml=None):
        if xml is not None:
            if not xml.tag == '{%s}stream' % STREAM_NS_URI:
            ...
            super().__init__(('{%s}stream' % STREAM_NS_URI), nsmap={'stream':STREAM_NS_URI, 
             None:ns})
    ...
    def toXml(self, pprint=False, encoding='utf-8'):
        header = "<stream:stream xmlns:stream='%s' xmlns='%s' " % (
         STREAM_NS_URI, self.defaultNamespace)
        if self.lang:
            header += "xml:lang='%s' " % self.lang
        if self.version:
...
class StreamFeatures(Stanza):
    XPATH = (
     '/stream:features', {'stream': STREAM_NS_URI})

    def __init__(self, xml=None):
        if xml is not None:
            if not xml.tag == '{%s}features' % STREAM_NS_URI:
                raise AssertionError
            elif not xml.nsmap['stream'] == STREAM_NS_URI:
                raise AssertionError
            super().__init__(xml=xml)
        else:
            super().__init__(('{%s}features' % STREAM_NS_URI), nsmap={'stream': STREAM_NS_URI})
...
class StreamError(Stanza, RuntimeError):
    XPATH = (
     '/stream:error', {'stream': STREAM_NS_URI})

    def __init__(self, error=None, xml=None):
        if not error is not None:
            if not xml is not None:
                raise AssertionError
            else:
                assert xml is not None and xml.tag == '{%s}error' % STREAM_NS_URI
                assert xml.nsmap['stream'] == STREAM_NS_URI
            super().__init__(xml=xml)
        else:
            super().__init__(('{%s}error' % STREAM_NS_URI), nsmap={'stream': STREAM_NS_URI})
            self.error = error
    ...
    def error(self, err):
        ...
        if not isinstance(err, errors.StreamError):
            raise ValueError('error attribute must be of type hiss.xmpp.errors.StreamError')
        self.xml = err.xml
...
class Iq(Stanza):
    ...
    def __init__(self, to=None, frm=None, type='get', id=None, request=None, xml=None, id_prefix=None, attrs=None):
        if xml is not None:
            if not xml.tag == 'iq':
                raise AssertionError
            elif not None not in xml.nsmap:
                raise AssertionError
        ...
        if request:
            name, ns = request
            self.xml.append(etree.Element(('{%s}%s' % (ns, name)), nsmap={None: ns}))
...
class Presence(Stanza):
    ...
    SHOW_XA = 'xa'
    ORDERED_SHOWS = [
     SHOW_CHAT, None, SHOW_AWAY, SHOW_XA, SHOW_DND]

    def __init__(self, to=None, frm=None, type=TYPE_AVAILABLE, priority=None, show=None, status=None, xml=None, attrs=None):
        if xml is not None:
            if not xml.tag == 'presence':
                raise AssertionError
            elif not None not in xml.nsmap:
                raise AssertionError
            super().__init__(xml=xml, attrs=attrs)
    ...
    def __lt__(self, rhs):
        if self.priority < rhs.priority:
            ...
            if self.priority > rhs.priority:
                return False
            if self.ORDERED_SHOWS.index(self.show) <= self.ORDERED_SHOWS.index(rhs.show):
                return False
            return True
    ...
    def type(self):
        ...
        if t:
            ...
            return Presence.TYPE_AVAILABLE
...
class Message(Stanza):
    ...
    def __init__(self, to=None, frm=None, type=TYPE_CHAT, subject=None, body=None, thread=None, xml=None, attrs=None):
        if xml is not None:
            if not xml.tag == 'message':
                raise AssertionError
            elif not None not in xml.nsmap:
                raise AssertionError
            super().__init__(xml=xml, attrs=attrs)
    ...
    if elem.tag == 'presence':
        ...
        if elem.tag == 'message':
            ...
            if elem.tag == 'iq':
                ...
                if elem.tag == '{%s}stream' % STREAM_NS_URI:
                    return StreamHeader(xml=elem)
                if elem.tag == '{%s}features' % STREAM_NS_URI:
                    return StreamFeatures(xml=elem)
            if elem.tag == '{%s}error' % STREAM_NS_URI:
