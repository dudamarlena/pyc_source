...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vexmpp/vex/util.py
# Compiled at: 2017-02-05 20:07:13
...
def getXData(form, creds):
    ...
    for field in form.findall('{jabber:x:data}field'):
        ...
        pout('{label} ({var}): {value}'.format(label=(field.get('label')),
          var=var,
          value=(value.text if value else '')),
          end='')
        if not field.find('{jabber:x:data}value'):
            if var in ('username', 'password'):
                _appendValue(field, creds.jid.user if var == 'username' else creds.password)
            else:
                resp = _prompt()
