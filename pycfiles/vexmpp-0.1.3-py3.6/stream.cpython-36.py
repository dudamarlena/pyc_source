...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vexmpp/stream.py
# Compiled at: 2017-09-20 18:14:05
# Size of source mod 2**32: 11877 bytes
...
if 'VEX_TIMED_WAITS' in os.environ:
    if int(os.environ['VEX_TIMED_WAITS']):
        from .metrics import ValueMetric
        stream_wait_met = ValueMetric('stream:wait_time', type_=float)
else:
    stream_wait_met = None
_ENFORCE_TIMEOUTS = bool('VEX_ENFORCE_TIMEOUTS' in os.environ and int(os.environ['VEX_ENFORCE_TIMEOUTS']))
...
class Stream(asyncio.Protocol):
    __doc__ = 'Base class for XMPP streams.'

    def __init__(self, creds, state_callbacks=None, mixins=None, default_timeout=None):
        self.creds = creds
        ...
        for mixin in self._mixins:
            for name, obj in mixin._exports:
                if name in self.__dict__:
                    raise ValueError("Mixin '%s' exports ambiguous data named '%s'" % (
                     str(mixin), name))
                else:
    ...
    def connected(self):
        if not self._transport:
            ...
            if getattr(self._transport, '_closing'):
    ...
    async def sendAndWaitIq(self, child_ns, to=None, child_name='query', type='get', raise_on_error=False, timeout=None, id_prefix=None):
        iq = Iq(to=to, type=type, request=(child_name, child_ns), id_prefix=id_prefix)
        resp = await self.sendAndWait(iq, raise_on_error=raise_on_error, timeout=timeout)
        return resp
        ...
        try:
            try:
                ...
                if stream_wait_met:
                    ...
                    log.debug('Stream wait - time: {:.3f} min/max/avg: {:.6f}/{:.6f}/{:.6f}'.format(stream_wait_met.value, stream_wait_met.min, stream_wait_met.max, stream_wait_met.average))
                return match
            except asyncio.TimeoutError as ex:
                raise asyncio.TimeoutError('Timeout ({}s) while waiting for xpaths: {}'.format(timeout, xpaths)) from ex
...
class _StreamWaitFuture(asyncio.Future):
    ...
    def matchStanza(self, queued_stanza):
        log.debug(f"MatchStanza: {queued_stanza.stanza.toXml()} xpaths: {{0}} - @{{1}}".format(self._xpaths, id(self._task)))
        if self._task in queued_stanza.task_set:
            ...
            for xp, nsmap in self._xpaths:
                log.debug('MatchStanza: Testing xpath {} against stanza {}'.format((
                 xp, nsmap), stanza.toXml()))
