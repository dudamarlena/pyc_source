...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vexmpp/suelta/sasl.py
# Compiled at: 2017-02-05 18:38:05
# Size of source mod 2**32: 13512 bytes
from .util import hashes
from .saslprep import saslprep
SESSION = {'answers':{},  'passwords':{},  'sec_queries':{},  'stash':{},  'stash_file':''}
MECHANISMS = {}
MECH_SEC_SCORES = {}
...
class SASL(object):
    ...
    def __init__(self, host, service, mech=None, username=None, min_sec=0, request_values=None, sec_query=None, tls_active=None, def_realm=None):
        """
    ...
    def reset_stash_id(self, username):
        ...
        self.stash_id = '\x00'.join(self.testkey)
    ...
    def choose_mechanism(self, mechs, force_plain=False):
        ...
        if force_plain:
            ...
            if self.user is not None:
                ...
                if requested_mech == '*':
                    if self.user in ('', 'anonymous', None):
                        requested_mech = 'ANONYMOUS'
                if requested_mech != '*':
                    if requested_mech in MECHANISMS:
                        if requested_mech in MECH_SEC_SCORES:
