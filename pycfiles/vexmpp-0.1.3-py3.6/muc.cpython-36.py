...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vexmpp/protocols/muc.py
# Compiled at: 2017-02-05 17:57:31
# Size of source mod 2**32: 7371 bytes
from .. import stream
...
def selfPresenceXpath(nick_jid):
    ...
    xpath = "/presence[@from='{}']/mu:x/mu:status[@code='110']".format(nick_jid.full)
...
class RosterItem:
    ...
    def __str__(self):
        return 'RosterItem [nickname: {nickname}, jid: {jid}, affiliation: {affiliation}, role: {role}]'.format(nickname=(self.nickname),
...
class RoomInfo:
    __doc__ = 'A container for MUC room information including occupant roster.'
...
class MucMixin(stream.Mixin):
    __doc__ = 'This class provides MUC room and presence tracking for a stream.'
    ...
    async def onStanza(self, stream, stanza):
        ...
        if pres.type == pres.TYPE_AVAILABLE:
            roster_item = RosterItem((muc_jid.nick), item_elem=item, self_presence=self_presence)
...
async def enterRoom(stream, room, service, nick, password=None, config_new_room_callback=None, timeout=None):
    ...
    ROOM_OWNER_PRESENCE_XPATH = (
     ...
     {'mu': NS_URI_USER})
    if pres.xml.xpath((ROOM_OWNER_PRESENCE_XPATH[0]), namespaces=(ROOM_OWNER_PRESENCE_XPATH[1])) and pres.xml.xpath((ROOM_CREATED_PRESENCE_XPATH[0]), namespaces=(ROOM_CREATED_PRESENCE_XPATH[1])):
        if config_new_room_callback is None:
            ...
            room_config = await stream.sendAndWait(iq, raise_on_error=True, timeout=timeout)
            room_config = config_new_room_callback(room_config)
            await stream.sendAndWait(room_config, raise_on_error=True, timeout=timeout)
    return pres
