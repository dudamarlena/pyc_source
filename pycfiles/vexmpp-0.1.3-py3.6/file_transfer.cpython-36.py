...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vexmpp/protocols/file_transfer.py
# Compiled at: 2017-02-05 18:06:46
# Size of source mod 2**32: 13300 bytes
import os, asyncio
...
from ..errors import BadRequestStanzaError, XmppError, PolicyViolationStanzaError, FeatureNotImplementedStanzaError
...
class File:
    ...
    def __init__(self, info, maxsize=None, tempdir=None):
        ...
        self._tmp_fd, self._tmp_name = mkstemp(suffix='.incoming', dir=(str(self._tmp_dir)))
    ...
    def addBlock(self, data):
        if not self._tmp_fd:
            ...
            try:
                ...
                if self._maxsize:
                    if self._data_size > self._maxsize:
                        raise OverflowError('File exceeds size limit of {:d} bytes.'.format(self._maxsize))
            except:
                self.close(False)
                raise
    ...
    def _verify(self):
        ...
        if len(sequences) != sequences[(-1)] + 1:
            ...
            if full_set.difference(self._seq_set):
                raise FileTransferError('Not all data blocks received, missing sequences {}'.format(full_set.difference(self._seq_set)))
        if self._data_size != self.info.size:
            raise FileTransferError('Not all bytes received, read {:d} bytes but expected {:d}'.format(self._data_size, self.info.size))
        if self.info.md5:
            md5 = self.md5
            if self.info.md5.lower() != md5:
                raise FileTransferError('File hash mismatch, computed {} but expected {}'.format(md5, self.info.md5))
...
async def receiveFile(stream, si_request, maxsize=None, tempdir=None, timeout=None):
    no_valid_streams_error = etree.Element('no-valid-streams', nsmap={None: SI_NS_URI})
    si_elem = si_request.find('./si:si', namespaces={'si': SI_NS_URI})
    try:
        if si_elem.attrib['profile'] != FT_NS_URI:
            raise BadRequestStanzaError('Profile not understood', app_err=etree.Element('bad-profile', nsmap={None: SI_NS_URI}))
        else:
            sid = si_elem.attrib['id']
            file_elem = si_request.find('.//f:file', namespaces={'f': FT_NS_URI})
            form = si_request.find(".//f:feature/x:x[@type='form']", namespaces={'x':'jabber:x:data', 
             'f':FEATURE_NS_URI})
            ...
            if field is None:
                raise BadRequestStanzaError('no stream-method field', app_err=no_valid_streams_error)
            methods = field.xpath('./x:option/x:value/text()', namespaces={'x': xdata.NS_URI})
            if IBB_NS_URI not in methods:
                raise BadRequestStanzaError('Only in-band-bytestreams is supported', app_err=no_valid_streams_error)
    ...
    file_info = FileInfo(name=(file_elem.get('name')), size=(int(file_elem.get('size'))),
    ...
    new_file = await receiveFileCoro(stream, open_iq, file_info, maxsize=maxsize,
      tempdir=tempdir,
      timeout=timeout)
    return new_file
...
async def sendFile(stream, to_jid, filename, description=None, timeout=None):
    ...
    file_info = FileInfo(name=(path.name), size=(path.stat().st_size), desc=description,
      date=None,
      md5=(h.hexdigest()),
      range=None,
      sid=sid)
    ...
    feat.append(form)
    try:
        response = await stream.sendAndWait(request, timeout=timeout, raise_on_error=True)
    except XmppError as ex:
        raise FileTransferError(ex)

    si_elem = response.find('./si:si', namespaces={'si': SI_NS_URI})
    methods = si_elem.xpath('.//x:field/x:value/text()', namespaces={'x': xdata.NS_URI})
    if not methods or methods[0] not in [IBB_NS_URI]:
        stream.send(response.errrorResponse(BadRequestStanzaError('Only in-band-bytestreams is supported',
          app_err=etree.Element('no-valid-streams', nsmap={None: SI_NS_URI}))))
...
async def ibbSendFile(stream, to_jid, file_path, sid, block_size=BLOCK_SIZE, timeout=None):
...
async def ibbReceiveFile(stream, request, file_info, maxsize=None, tempdir=None, timeout=None):
    ...
    while not done:
        data_xp = "/iq[@from='{frm}' and @type='set']/ibb:data[@sid='{sid}']".format(frm=(from_jid.full),
          sid=sid)
        close_xp = "/iq[@from='{frm}' and @type='set']/ibb:close[@sid='{sid}']".format(frm=(from_jid.full),
          sid=sid)
        try:
            iq = await stream.wait([(data_xp, nsmap), (close_xp, nsmap)], timeout=timeout)
