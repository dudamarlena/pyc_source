...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vexmpp/client.py
# Compiled at: 2017-09-20 20:09:29
# Size of source mod 2**32: 7933 bytes
...
from .protocols import resourcebind, iqroster, presence, iqversion, entity_time, disco, iqregister, muc
...
class Credentials:
    __doc__ = 'Client credentials, e.g. a user JID and password.'
...
class ClientStream(Stream):
    ...
    def __init__(self, creds, tls_opt=None, state_callbacks=None, mixins=None, default_timeout=None, register_cb=None):
        self._tls_opt = tls_opt or TlsOpts.on
        self._register_cb = register_cb
        super().__init__(creds, state_callbacks=state_callbacks, mixins=mixins, default_timeout=default_timeout)
    ...
    async def negotiate(self, timeout=None):
        ...
        if self._tls_opt == TlsOpts.required:
            if tls_elem is None:
                raise Error('TLS required by client but not offered by server.')
        if self._tls_opt == TlsOpts.off:
            if tls_elem is not None:
                if starttls.isRequired(tls_elem):
        ...
        if sm_elem is not None:
            if hasattr(self, 'stream_mgmt_opts'):
                await stream_mgmt.handle(self, sm_elem, sm_opts=(self.stream_mgmt_opts),
                  timeout=timeout)
        for mixin in self._mixins:
            await mixin.postSession(self)

    @classmethod
    async def connect(Class, creds, host=None, port=DEFAULT_C2S_PORT, loop=None, timeout=None, **stream_kwargs):
        """Connect and negotiate a stream with the server.
         The connected stream is returned."""
        loop = loop or asyncio.get_event_loop()
        if type(creds) is tuple:
            creds = Credentials(*creds)
        host, port = await resolveHostPort(host if host else creds.jid.host, port, loop)
        peer = (host.host, int(port))
        log.verbose('Connecting %s...' % str(peer))
        state_callbacks = stream_kwargs['state_callbacks'] if 'state_callbacks' in stream_kwargs else None
        signalEvent(state_callbacks, 'connecting', peer[0], peer[1])

        def _sslContext():
            from OpenSSL.SSL import Context, SSLv23_METHOD, OP_NO_SSLv2, OP_NO_SSLv3, VERIFY_PEER
        ...
        ProtocolFactory = (functools.partial)(Class, creds, default_timeout=timeout, **stream_kwargs)
        conn = create_starttls_connection(loop, ProtocolFactory, *peer, **{'use_starttls':True, 
         'ssl_context':_sslContext(), 
         'server_hostname':creds.jid.host})
        try:
            connected = False
            transport, stream = await asyncio.wait_for(conn, timeout)
            connected = True
            ...
            if not connected:
                signalEvent(state_callbacks, 'connectionFailed', peer[0], peer[1], ex)
            else:
