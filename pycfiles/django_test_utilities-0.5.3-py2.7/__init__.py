...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/test_utilities/__init__.py
# Compiled at: 2013-10-22 12:01:07
...
class Client(DjangoClient):
    ...
    def login_as(self, user=None, username='', password='testpassword', permissions=None, *args, **kwargs):
        if not user:
            if not username:
                username = ('').join(random.choice(string.ascii_letters + string.digits) for i in xrange(random.randint(6, 20)))
            user = User.objects.create(username=username, is_active=True, *args, **kwargs)
        user.set_password(password)
        user.save()
        if permissions:
            add_user_permissions(user, permissions)
        assert self.login(username=user.username, password=password), "Can't login with user"
        return user
...
class TestCaseExtensionMixin(object):
    ...
    def assertMessageContains(self, response, text, level=None):
        ...
        if len(matches) == 1:
            msg = matches[0]
            if level is not None and msg.level != level and DEFAULT_TAGS[msg.level] != level:
    ...
    def __init__(self, objects=[], model=None, query=None, *args, **kwargs):
        if model is None:
            if len(objects):
                ...
                raise ValueError('If objects is empty, a model must be included')
        pk_list = []
        for r in objects:
