...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/imaging/views.py
...
def iframe_form(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            new_image = form.save()
            response_dict = {'title': form.cleaned_data['title'], 
               'alt': form.cleaned_data['alt'], 
               'name': form.cleaned_data['name'][:25], 
               'id': new_image.pk, 
               'image': new_image.imaging_thumbnail.url}
            form = AjaxUploadForm()
            return render_to_response('imaging/iframe_form.html', {'form': form, 'callback': simplejson.dumps(response_dict)}, context_instance=RequestContext(request))
        return render_to_response('imaging/iframe_form.html', {'form': form}, context_instance=RequestContext(request))
    else:
        form = AjaxUploadForm()
        return render_to_response('imaging/iframe_form.html', {'form': form}, context_instance=RequestContext(request))
