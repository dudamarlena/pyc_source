...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/emrt/necd/content/subscriptionconfiguration.py
...
LDAP_TO_LOCAL = {ROLE_SE: partial(user_has_ldap_role, LDAP_SECTOREXP), 
...
ROLE_TRANSLATOR = {ROLE_SE: 'Sector Expert', 
   ROLE_LR: 'Lead Reviewer', 
   ROLE_CP: 'Counter Part', 
   ROLE_MSA: 'Member State Coordinator', 
   ROLE_MSE: 'Member State Expert'}
NOTIFICATIONS_PER_ROLE = {ROLE_SE: {'observation_finalisation_denied': True, 
             ...
             'question_to_ms': True, 
             'new_comment': True}, 
   ROLE_LR: {'conclusion_to_comment': True, 
             'observation_finalisation_request': True, 
             'question_answered': True, 
             'question_ready_for_approval': True, 
             'question_to_counterpart': True, 
             'new_comment': True}, 
   ROLE_MSA: {'answer_acknowledged': True, 
              'observation_finalised': True, 
              'observation_recalled': True, 
              'question_to_ms': True, 
              'comment_to_msa': True}, 
   ROLE_CP: {'conclusion_to_comment': True, 
             'question_to_counterpart': True, 
             'new_comment': True}, 
   ROLE_MSE: {'answer_to_msexperts': True, 
              'question_answered': True, 
              'comment_to_mse': True}}
NOTIFICATION_NAMES = {ROLE_SE: {'observation_finalisation_denied': 'Observation finalisation denied by LR', 
   ...
   ROLE_LR: {'conclusion_to_comment': 'Conclusion to comment by you as LR', 
             ...
             'new_comment': 'New comment is added'}, 
   ROLE_MSA: {'answer_acknowledged': 'Answer acknowledged by sector expert', 
              'observation_finalised': 'Observation finalised by LR', 
              'observation_recalled': 'Observation recalled by LR', 
              'question_to_ms': 'Question to be answered by your country', 
              'comment_to_msa': 'New comment from MS Expert'}, 
   ROLE_CP: {'conclusion_to_comment': 'Conclusion to comment by you as counterpart', 
             'question_to_counterpart': 'Question to comment by you as counterpart', 
             'new_comment': 'New comment is added'}, 
   ROLE_MSE: {'answer_to_msexperts': 'New question to comment by you as MS expert', 
              'question_answered': 'Question answered by MS', 
...
class SubscriptionConfigurationReview(SubscriptionConfigurationMixin):
    ...
    def _user_roles(self, context, user):
        ...
        return tuple(role for role, checker in LDAP_TO_LOCAL.items() if checker(groups=groups, ldap_wrapper=ldap_wrapper))
...
class SaveSubscriptionsReview(SubscriptionConfigurationReview, SaveSubscriptionsMixin):
