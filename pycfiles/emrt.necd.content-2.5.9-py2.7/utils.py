...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/emrt/necd/content/utils.py
# Compiled at: 2019-05-17 05:23:08
import string, simplejson as json
from itertools import chain
from operator import itemgetter
import concurrent.futures, plone.api as api
...
def user_has_ldap_role(ldap_name, user=None, groups=None, ldap_wrapper=ldap_inventory):
    _user = user if user else api.user.get_current()
    _groups = groups if groups else _user.getGroups()
    return any(tuple(group for group in _groups if group.startswith(ldap_wrapper(ldap_name))))


def get_user_sectors(user):
    return set(chain(*[ [ n for n in g.split('-') if 'sector' in n and n[(-1)].isdigit() ] for g in user.getGroups()
                      ]))
