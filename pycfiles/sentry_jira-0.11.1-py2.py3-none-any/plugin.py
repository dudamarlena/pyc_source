...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/getsentry/sentry-jira/sentry_jira/plugin.py
...
class JIRAPlugin(IssuePlugin):
    ...
    def get_initial_form_data(self, request, group, event, **kwargs):
        initial = {'summary': self._get_group_title(request, group, event), 
    ...
    def create_issue(self, request, group, form_data, **kwargs):
        ...
        try:
            ...
            if e.status_code == 500:
                ...
                errdict['__all__'] = [
                 e.json['errorMessages']]
            else:
                errdict['__all__'] = [
                 'Something went wrong, Sounds like a configuration issue: code %s' % e.status_code]
            return (
             None, errdict)
    ...
    def view(self, request, group, **kwargs):
        ...
        if not (has_auth_configured and self.is_configured(project=group.project, request=request)):
            if self.auth_provider:
                providers = settings.AUTH_PROVIDERS if hasattr(settings, 'AUTH_PROVIDERS') else settings.SENTRY_AUTH_PROVIDERS
                required_auth_settings = providers[self.auth_provider]
            else:
                required_auth_settings = None
            return self.render(self.not_configured_template, {'title': self.get_title(), 
            ...
            if self.needs_auth(project=group.project, request=request):
                return self.render(self.needs_auth_template, {'title': self.get_title(), 
                   'project': group.project})
            issue_key = GroupMeta.objects.get_value(group, '%s:tid' % self.get_conf_key(), None)
            if issue_key:
            ...
            try:
                form = self.new_issue_form(request.POST or None, initial=self.get_initial_form_data(request, group, event), jira_client=self.get_jira_client(group.project), project_key=self.get_option('default_project', group.project), ignored_fields=self.get_option('ignored_fields', group.project))
            except JIRAError as e:
                context = {'errorMessages': e.json.get('errorMessages', []) if e.json else [], 'title': self.get_new_issue_title()}
                return self.render(self.plugin_misconfigured_template, context)

            if request.POST and request.POST.get('changing_issuetype') == '0':
                if form.is_valid():
                    issue_id, error = self.create_issue(group=group, form_data=form.cleaned_data, request=request)
                    if error:
                        form.errors.update(error)
                        ignored_errors = [ v for k, v in error.items() if k in form.ignored_fields
                                         ]
                        if len(ignored_errors) > 0:
                            errs = form.errors['__all__']
                            errs.append('Validation Error on ignored field, check your plugin settings.')
            ...
            context = {'form': form, 'title': self.get_new_issue_title()}
    ...
    def handle_user_autocomplete(self, request, group, **kwargs):
        ...
        if '/rest/api/latest/user/' in url:
            isXML = False
            query['username'] = q
            query.pop('issueKey', False)
            query['project'] = project
        else:
            isXML = True
            query['query'] = q
            if query.get('fieldName'):
                query['fieldName'] = query['fieldName'][0]
            ...
            if isXML:
                for userxml in autocomplete_response.xml.findAll('users'):
                    users.append({'value': userxml.find('name').text, 
    ...
    def handle_issue_type_autocomplete(self, request, group):
        ...
        for issue_type in meta.json:
            issue_types.append({'value': issue_type['name'], 
    ...
    def post_process(self, group, event, is_new, is_sample, **kwargs):
        ...
        post_data = {'project': {'id': project}, 'summary': initial['summary'], 
        ...
        issue_id, error = self.create_issue(request={}, group=group, form_data=post_data)
