...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/uncompyle6/parsers/parse3.py
...
from uncompyle6.parsers.reducecheck import and_check, except_handler_else, ifelsestmt, ifstmt, iflaststmt, or_check, testtrue, tryelsestmtl3, tryexcept, while1stmt
...
class Python3Parser(PythonParser):
    ...
    def custom_build_class_rule(self, opname, i, token, tokens, customize, is_pypy):
        ...
        if self.version < 3.6:
            ...
            rule = 'build_class ::= LOAD_BUILD_CLASS mkfunc %s%s' % ('expr ' * (args_pos - 1) + 'kwarg ' * args_kw, call_function)
        else:
            ...
            if call_function.startswith('CALL_FUNCTION_KW'):
                ...
                if is_pypy:
                    (args_pos, args_kw) = self.get_pos_kw(call_fn_tok)
                    rule = 'build_class_kw ::= LOAD_BUILD_CLASS mkfunc %s%s%s' % ('expr ' * (args_pos - 1), 'kwarg ' * args_kw, call_function)
                else:
    ...
    def custom_classfunc_rule(self, opname, token, customize, next_token, is_pypy):
        ...
        if is_pypy and self.version >= 3.6:
            ...
            rule = 'call ::= expr ' + 'pos_arg ' * args_pos + 'kwarg ' * args_kw + token.kind
        else:
            token.kind = self.call_fn_name(token)
    ...
    def customize_grammar_rules(self, tokens, customize):
        ...
        customize_instruction_basenames = frozenset(('BUILD', 'CALL', 'CONTINUE', 'DELETE',
                                                     'GET', 'JUMP', 'LOAD', 'LOOKUP',
                                                     'MAKE', 'RETURN', 'RAISE', 'SETUP',
                                                     'UNPACK'))
        ...
        for (i, token) in enumerate(tokens):
            ...
            if opname_base == 'BUILD_CONST_KEY_MAP':
            ...
            elif opname in frozenset(('CALL_FUNCTION', 'CALL_FUNCTION_EX', 'CALL_FUNCTION_EX_KW',
                                      'CALL_FUNCTION_VAR', 'CALL_FUNCTION_VAR_KW')) or opname.startswith('CALL_FUNCTION_KW'):
                ...
                if opname == 'MAKE_CLOSURE_0':
                    ...
                    if self.is_pypy or i >= j and tokens[(i - j)] == 'LOAD_LAMBDA':
                        rule_pat = 'mklambda ::= %sload_closure LOAD_LAMBDA %%s%s' % ('pos_arg ' * args_pos, opname)
                        self.add_make_function_rule(rule_pat, opname, token.attr, customize)
                    if has_get_iter_call_function1:
                        rule_pat = 'generator_exp ::= %sload_closure load_genexpr %%s%s expr GET_ITER CALL_FUNCTION_1' % ('pos_arg ' * args_pos, opname)
                        self.add_make_function_rule(rule_pat, opname, token.attr, customize)
                        if has_get_iter_call_function1 and (self.is_pypy or i >= j and tokens[(i - j)] == 'LOAD_LISTCOMP'):
                ...
                if self.version <= 3.2:
                    if annotate_args > 0:
                        rule = 'mkfunc_annotate ::= %s%s%sannotate_tuple load_closure LOAD_CODE %s' % (kwargs_str, 'pos_arg ' * args_pos, 'annotate_arg ' * (annotate_args - 1), opname)
                    else:
                        rule = 'mkfunc ::= %s%sload_closure LOAD_CODE %s' % (kwargs_str, 'pos_arg ' * args_pos, opname)
                elif self.version == 3.3:
                    if annotate_args > 0:
                        rule = 'mkfunc_annotate ::= %s%s%sannotate_tuple load_closure LOAD_CODE LOAD_STR %s' % (kwargs_str, 'pos_arg ' * args_pos, 'annotate_arg ' * (annotate_args - 1), opname)
                    else:
                        rule = 'mkfunc ::= %s%sload_closure LOAD_CODE LOAD_STR %s' % (kwargs_str, 'pos_arg ' * args_pos, opname)
                    ...
                    if PYTHON3 or not self.is_pypy:
                        ...
                        load_op = 'LOAD_CONST'
                    if annotate_args > 0:
                        rule = 'mkfunc_annotate ::= %s%s%sannotate_tuple load_closure %s %s' % ('pos_arg ' * args_pos, kwargs_str, 'annotate_arg ' * (annotate_args - 1), load_op, opname)
                    else:
                        rule = 'mkfunc ::= %s%s load_closure LOAD_CODE %s %s' % ('pos_arg ' * args_pos, kwargs_str, load_op, opname)
                self.add_unique_rule(rule, opname, token.attr, customize)
                if args_kw == 0:
                    rule = 'mkfunc ::= %sload_closure load_genexpr %s' % ('pos_arg ' * args_pos, opname)
                    self.add_unique_rule(rule, opname, token.attr, customize)
                if self.version < 3.4:
                    rule = 'mkfunc ::= %sload_closure LOAD_CODE %s' % ('expr ' * args_pos, opname)
                    self.add_unique_rule(rule, opname, token.attr, customize)
                ...
                if self.version >= 3.6:
                    ...
                    if closure:
                        if args_pos:
                            rule = 'mklambda ::= %s%s%s%s' % ('expr ' * stack_count, 'load_closure ' * closure, 'BUILD_TUPLE_1 LOAD_LAMBDA LOAD_STR ', opname)
                        else:
                            rule = 'mklambda ::= %s%s%s' % ('load_closure ' * closure, 'LOAD_LAMBDA LOAD_STR ', opname)
                        self.add_unique_rule(rule, opname, token.attr, customize)
                    else:
                        rule = 'mklambda ::= %sLOAD_LAMBDA LOAD_STR %s' % ('expr ' * stack_count, opname)
                        self.add_unique_rule(rule, opname, token.attr, customize)
                    rule = 'mkfunc ::= %s%s%s%s' % ('expr ' * stack_count, 'load_closure ' * closure, 'LOAD_CODE LOAD_STR ', opname)
                    self.add_unique_rule(rule, opname, token.attr, customize)
                    if not PYTHON3 and self.is_pypy:
                        rule = 'mkfunc ::= %s%s%s%s' % ('expr ' * stack_count, 'load_closure ' * closure, 'LOAD_CODE LOAD_CONST ', opname)
                        self.add_unique_rule(rule, opname, token.attr, customize)
                    if has_get_iter_call_function1:
                    ...
                    if self.is_pypy or i >= 2 and tokens[(i - 2)] == 'LOAD_LAMBDA':
                        rule_pat = 'mklambda ::= %s%sLOAD_LAMBDA %%s%s' % ('pos_arg ' * args_pos, 'kwarg ' * args_kw, opname)
                        self.add_make_function_rule(rule_pat, opname, token.attr, customize)
                    continue
                if self.version < 3.6:
                    (args_pos, args_kw, annotate_args) = token.attr
                ...
                if args_kw == 0:
                    ...
                    kwargs = 'kwargs'
                if self.version < 3.3:
                    rule = 'mkfunc ::= %s %s%s%s' % (kwargs, 'pos_arg ' * args_pos, 'LOAD_CODE ', opname)
                    self.add_unique_rule(rule, opname, token.attr, customize)
                    rule = 'mkfunc ::= %s%s%s' % ('pos_arg ' * args_pos, 'LOAD_CODE ', opname)
                elif self.version == 3.3:
                    rule = 'mkfunc ::= %s %s%s%s' % (kwargs, 'pos_arg ' * args_pos, 'LOAD_CODE LOAD_STR ', opname)
                elif self.version > 3.5:
                    rule = 'mkfunc ::= %s%s %s%s' % ('pos_arg ' * args_pos, kwargs, 'LOAD_CODE LOAD_STR ', opname)
                elif self.version > 3.3:
                    rule = 'mkfunc ::= %s%s %s%s' % ('pos_arg ' * args_pos, kwargs, 'LOAD_CODE LOAD_STR ', opname)
                else:
                    rule = 'mkfunc ::= %s%sexpr %s' % (kwargs, 'pos_arg ' * args_pos, opname)
                self.add_unique_rule(rule, opname, token.attr, customize)
                if re.search('^MAKE_FUNCTION.*_A', opname):
                    if self.version >= 3.6:
                        rule = 'mkfunc_annotate ::= %s%sannotate_tuple LOAD_CODE LOAD_STR %s' % ('pos_arg ' * args_pos, 'call ' * (annotate_args - 1), opname)
                        self.add_unique_rule(rule, opname, token.attr, customize)
                        rule = 'mkfunc_annotate ::= %s%sannotate_tuple LOAD_CODE LOAD_STR %s' % ('pos_arg ' * args_pos, 'annotate_arg ' * (annotate_args - 1), opname)
                    if self.version >= 3.3:
                        if self.version == 3.3:
                        ...
                        rule = 'mkfunc_annotate ::= %s%s%sannotate_tuple LOAD_CODE LOAD_STR EXTENDED_ARG %s' % (pos_kw_tuple[0], pos_kw_tuple[1], 'call ' * (annotate_args - 1), opname)
                        self.add_unique_rule(rule, opname, token.attr, customize)
                        rule = 'mkfunc_annotate ::= %s%s%sannotate_tuple LOAD_CODE LOAD_STR EXTENDED_ARG %s' % (pos_kw_tuple[0], pos_kw_tuple[1], 'annotate_arg ' * (annotate_args - 1), opname)
                    else:
                        rule = 'mkfunc_annotate ::= %s%s%sannotate_tuple LOAD_CODE EXTENDED_ARG %s' % ('kwargs ' * args_kw, 'pos_arg ' * args_pos, 'annotate_arg ' * (annotate_args - 1), opname)
                        self.add_unique_rule(rule, opname, token.attr, customize)
                        rule = 'mkfunc_annotate ::= %s%s%sannotate_tuple LOAD_CODE EXTENDED_ARG %s' % ('kwargs ' * args_kw, 'pos_arg ' * args_pos, 'call ' * (annotate_args - 1), opname)
                    self.addRule(rule, nop_func)
    ...
    def reduce_is_invalid(self, rule, ast, tokens, first, last):
        ...
        if lhs in ('aug_assign1', 'aug_assign2') and ast[0][0] == 'and':
            ...
            if lhs == 'kwarg':
                ...
                if lhs in ('iflaststmt', 'iflaststmtl') and self.version == 3.6:
                    return ifstmt(self, lhs, n, rule, ast, tokens, first, last)
                elif rule == ('ifstmt', ('testexpr', '_ifstmts_jump')) and (self.version <= 3.0 or tokens[last] == 'RETURN_END_IF'):
                    return False
            ...
            if condition_jump.kind.startswith('POP_JUMP_IF'):
                ...
                if condition_jump2.kind.startswith('POP_JUMP_IF'):
                    if condition_jump != condition_jump2:
