...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/uncompyle6/verify.py
# Compiled at: 2020-04-27 23:06:35
"""
byte-code verification
"""
import operator, sys, xdis.std as dis
...
class CmpErrorConstsLen(VerifyCmpError):
    ...
    def __str__(self):
        return 'Consts length differs in %s:\n\n%i:\t%s\n\n%i:\t%s\n\n' % (repr(self.name), len(self.consts[0]), repr(self.consts[0]), len(self.consts[1]), repr(self.consts[1]))
...
class CmpErrorCode(VerifyCmpError):
    ...
    def __str__(self):
        ...
        return 'Code differs in %s at offset %s [%s] != [%s]\n\n' % (repr(self.name), self.index, repr(self.token1), repr(self.token2)) + s
...
class CmpErrorMember(VerifyCmpError):
    ...
    def __str__(self):
        return 'Member %s differs in %s:\n\t%s\n\t%s\n' % (repr(self.member), repr(self.name), repr(self.data[0]), repr(self.data[1]))


__IGNORE_CODE_MEMBERS__ = [
 'co_filename', 'co_firstlineno', 'co_lnotab', 'co_stacksize', 'co_names']
...
def cmp_code_objects(version, is_pypy, code_obj1, code_obj2, verify, name=''):
    ...
    """
    global JUMP_OPS
    assert iscode(code_obj1), 'cmp_code_object first object type is %s, not code' % type(code_obj1)
    assert iscode(code_obj2), 'cmp_code_object second object type is %s, not code' % type(code_obj2)
    ...
    for member in members:
        if member in __IGNORE_CODE_MEMBERS__ or verify != 'verify':
            ...
            while i1 < len(tokens1):
                if i2 >= len(tokens2):
                    if len(tokens1) == len(tokens2) + 2:
                        if tokens1[(-1)].kind == 'RETURN_VALUE' and tokens1[(-2)].kind == 'LOAD_CONST' and tokens1[(-2)].pattr is None and tokens1[(-3)].kind == 'RETURN_VALUE':
                            break
                        else:
                    ...
                    if tokens1[i1].kind != tokens2[i2].kind and tokens1[i1].kind == 'LOAD_CONST' == tokens2[i2].kind:
                        i = 1
                        while tokens1[(i1 + i)].kind == 'LOAD_CONST':
                            i += 1

                        if tokens1[(i1 + i)].kind.startswith(('BUILD_TUPLE', 'BUILD_LIST')) and i == int(tokens1[(i1 + i)].kind.split('_')[(-1)]):
                        ...
                        if tokens2[i2].kind == 'POP_JUMP_IF_TRUE':
                            ...
                            if tokens1[(i1 + 1)].kind == 'POP_JUMP_IF_TRUE':
                                ...
                                i2 += 1
                                continue
                    elif tokens1[i1].kind in ('JUMP_FORWARD', 'JUMP_BACK') and tokens1[(i1 - 1)].kind == 'RETURN_VALUE' and tokens2[(i2 - 1)].kind in ('RETURN_VALUE',
                                                                                                                                                       'RETURN_END_IF') and int(tokens1[i1].offset) not in targets1:
                        i1 += 1
                        continue
                        ...
                        i2 += 2
                        continue
                    elif tokens1[i1].kind == 'JUMP_FORWARD' and tokens2[i2].kind == 'JUMP_BACK' and tokens1[(i1 + 1)].kind == 'JUMP_BACK' and tokens2[(i2 + 1)].kind == 'JUMP_BACK' and int(tokens1[i1].pattr) == int(tokens1[i1].offset) + 3:
                        if int(tokens1[i1].pattr) == int(tokens1[(i1 + 1)].offset):
                            i1 += 2
                            i2 += 2
                            continue
                    elif tokens1[i1].kind == 'LOAD_NAME' and tokens2[i2].kind == 'LOAD_CONST' and tokens1[i1].pattr == 'None' and tokens2[i2].pattr is None:
                        pass
                    elif tokens1[i1].kind == 'LOAD_GLOBAL' and tokens2[i2].kind == 'LOAD_NAME' and tokens1[i1].pattr == tokens2[i2].pattr:
                        pass
                    elif tokens1[i1].kind == 'LOAD_ASSERT' and tokens2[i2].kind == 'LOAD_NAME' and tokens1[i1].pattr == tokens2[i2].pattr:
                        pass
                    elif tokens1[i1].kind == 'RETURN_VALUE' and tokens2[i2].kind == 'RETURN_END_IF':
                        pass
                    elif tokens1[i1].kind == 'BUILD_TUPLE_0' and tokens2[i2].pattr == ():
                        pass
                    else:
                        raise CmpErrorCode(name, tokens1[i1].offset, tokens1[i1], tokens2[i2], tokens1, tokens2)
            ...
            flags2 &= -161
            if flags1 != flags2:
                raise CmpErrorMember(name, 'co_flags', pretty_code_flags(flags1), pretty_code_flags(flags2))
...
def compare_code_with_srcfile(pyc_filename, src_filename, verify):
    ...
    """
    (version, timestamp, magic_int, code_obj1, is_pypy, source_size, sip_hash) = load_module(pyc_filename)
    if magic_int != PYTHON_MAGIC_INT:
        msg = "Can't compare code - Python is running with magic %s, but code is magic %s " % (PYTHON_MAGIC_INT, magic_int)
        return msg
    try:
...
def compare_files(pyc_filename1, pyc_filename2, verify):
    ...
    (version1, timestamp, magic_int1, code_obj1, is_pypy, source_size, sip_hash) = uncompyle6.load_module(pyc_filename1)
    (version2, timestamp, magic_int2, code_obj2, is_pypy, source_size, sip_hash) = uncompyle6.load_module(pyc_filename2)
