...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/uncompyle6/parser.py
...
class PythonParser(GenericASTBuilder):
    ...
    def __init__(self, SyntaxTree, start, debug):
        ...
        nt_list = [
         '_come_froms', '_stmts', 'attributes', 'come_froms', 'except_stmts', 'exprlist', 'importlist', 'kvlist', 'kwargs', 'l_stmts', 'stmts', 'print_items', 'imports_cont', 'kvlist_n', 'come_from_loops', 'importlist37']
        self.collect = frozenset(nt_list)
        self.keep_epsilon = frozenset(('kvlist_n', 'kvlist'))
        self.optional_nt |= frozenset(('come_froms', 'suite_stmts', 'l_stmts_opt',
                                       'c_stmts_opt', 'stmts_opt', 'stmt'))
        self.singleton = frozenset(('str', 'store', '_stmts', 'suite_stmts_opt', 'inplace_op'))
        self.insts = []
    ...
    def get_pos_kw(self, token):
        ...
        args_pos = token.attr & 255
        args_kw = token.attr >> 8 & 255
        return (args_pos, args_kw)

    def nonterminal(self, nt, args):
        n = len(args)
        if nt in self.collect:
            if n > 1:
                rv = hasattr(args[0], 'append') or GenericASTBuilder.nonterminal(self, nt, [args[0]])
            else:
                rv = args[0]
...
def get_python_parser(version, debug_parser=PARSER_DEFAULT_DEBUG, compile_mode='exec', is_pypy=False):
