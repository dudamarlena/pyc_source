...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/uncompyle6/show.py
...
def maybe_show_tree(walker, ast):
    ...
    if walker.showast:
        if hasattr(walker.showast, 'write'):
            ...
            stream = sys.stdout
        if isinstance(walker.showast, dict) and walker.showast.get('Full', False) and hasattr(walker, 'str_with_template'):
