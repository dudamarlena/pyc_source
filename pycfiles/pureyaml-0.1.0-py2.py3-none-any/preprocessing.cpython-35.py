...
# Embedded file name: build/bdist.linux-x86_64/egg/purewords/preprocessing.py
# Compiled at: 2018-08-07 00:24:26
...
def tokenize_sentence_and_filte_tokens(text_str, tokenizer, token_filters):
    tokenized_text_lst = tokenizer.cut(text_str)
    filted_tokens = []
    for token in tokenized_text_lst:
        filted_tokens.append(token_filters(token))
...
def split_sentence(sentence, min_sentence_length=30, max_sentence_length=200):
    splitted_sentences = []
    sentence = re.sub(' ;', ';', sentence)
...
def split_document(document, tokenizer, token_filters, min_sentence_length=30, max_sentence_length=200):
    ...
    for sentence in sentences:
        ...
        sentence = tokenize_sentence_and_filte_tokens(sentence, tokenizer, token_filters)
        sentence = re.sub(' ; ', ' ;', sentence)
        ...
        if sentence_word_length < min_sentence_length:
            ...
            splitted_sentences = split_sentence(sentence, min_sentence_length, max_sentence_length)
