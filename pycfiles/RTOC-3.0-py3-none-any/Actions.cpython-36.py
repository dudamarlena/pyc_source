...
# [GCC 8.4.0]
# Embedded file name: D:\Benutzer\haschtl\Dokumente\GIT\kellerlogger\RTOC\RTOC_GUI\Actions.py
# Compiled at: 2020-01-06 18:23:01
# Size of source mod 2**32: 32582 bytes
import os
...
class Actions:
    ...
    def connectButtons(self):
        ...
        self.actionEnglish.triggered.connect(partial(self.toggleLanguage, 'en', self.actionEnglish, [self.actionDeutsch], False))
    ...
    def exportDatabase(self):
        ...
        fname, mask = fileBrowser.getSaveFileName(self, translate('RTOC', 'Export database'), dir_path, 'File (*)')
        if fname:
            fileName = fname
            if mask == 'File (*)':
                overwrite = False
    ...
    def setBotToken(self):
        ans, ok = pyqtlib.text_message(self, translate('RTOC', 'Enter bot token'), translate('RTOC', 'Please create a bot in Telegram with "Botfather",\ngenerate a bot and enter its token here'), self.config['telegram']['token'])
        if ok:
    ...
    def setWebsocketPassword(self):
        ans, ok = pyqtlib.text_message(self, translate('RTOC', 'Enter Websocket-Password'), translate('RTOC', 'Protect your transfer from unauthorized users \nLeave empty to disable password'), self.config['websocket']['password'])
        if ok:
    ...
    def setWebsocketPort(self):
        ans, ok = pyqtlib.text_message(self, translate('RTOC', 'Enter Websocket Port'), translate('RTOC', 'Enter the port for Websocket-Communication with RTOC.'), str(self.config['websocket']['port']))
        if ok:
            try:
                ans = int(ans)
                if ans >= 0:
                    ...
                    pyqtlib.info_message(translate('RTOC', 'Error'), translate('RTOC', 'Please enter a value between 0 and 65535'), '')
            except Exception:
                logging.debug(traceback.format_exc())
                pyqtlib.info_message(translate('RTOC', 'Error'), translate('RTOC', 'Please enter a value between 0 and 65535'), translate('RTOC', 'You input was invalid.'))
    ...
    def loadSessionTriggered(self):
        ...
        fileBrowser.selectNameFilter('')
        fname, mask = fileBrowser.getOpenFileName(self, translate('RTOC', 'Load session'), dir_path, 'Json (*.json)')
        if fname:
            fileName = fname
            if mask == 'Json (*.json)':
    ...
    def saveSessionTriggered(self):
        ...
        fileBrowser.setNameFilters([
         'JSON-Datei (*.json)'])
        fileBrowser.selectNameFilter('')
        fname, mask = fileBrowser.getSaveFileName(self, translate('RTOC', 'Save session'), dir_path, 'JSON-Datei (*.json)')
        if fname:
            fileName = fname
            if mask == 'JSON-Datei (*.json)':
                overwrite = False
                if os.path.exists(fileName):
                    overwrite = pyqtlib.alert_message(translate('RTOC', 'Overwrite'), translate('RTOC', 'Do you want to overwrite the file or merge both files?'), translate('RTOC', 'With "Overwrite" the stored data will be lost.'), '', translate('RTOC', 'Overwrite'), translate('RTOC', 'Merge'))
    ...
    def exportDataTriggered(self):
        ...
        fileBrowser.selectNameFilter('')
        fname, mask = fileBrowser.getSaveFileName(self, translate('RTOC', 'Export'), dir_path, translate('RTOC', 'Excel-Table (*.xlsx);;CSV-File (*.csv)'))
        if fname:
            fileName = fname
    ...
    def importDataTriggered(self):
        ...
        fileBrowser.selectNameFilter('')
        fname, mask = fileBrowser.getOpenFileName(self, 'Export', dir_path, 'Table data (*.csv *.tsv, *.xls, *.xlsx, *.txt, *.mat, *.wav, *.wave, *)')
        if fname:
            fileName = fname
    ...
    def showAboutMessage(self):
        pyqtlib.info_message(translate('RTOC', 'About'), 'RealTime OpenControl ', translate('RTOC', 'RealTime OpenControl (RTOC) is a free open source software under the BSD-3 license.\n\nAll icons are provided under the Creative Commons Attribution-NoDerivs 3.0 Unported license by icons8 (https://icons8.de)\n\nCopyright (C) 2018 Sebastian Keller'))
    ...
    def toggleLanguage(self, newlang, button, otherbuttons, force=False):
        ...
        if newlang != self.config['global']['language'] or force is True:
            ...
            if force is False:
                pyqtlib.info_message(translate('RTOC', 'Language changed'), translate('RTOC', 'Please restart RTOC'), '')
    ...
    def checkUpdates(self):
        ...
        if current is not None:
            ...
            if not available:
                info = translate('RTOC', "Excuse me. I couldn't find RTOC on PyPi. Please have a look at 'https://pypi.org/project/RTOC/'.")
            else:
                text += translate('RTOC', ', Newest version: {}').format(available[0])
                if current == available[0]:
                    info = translate('RTOC', 'RTOC is up to date.')
                else:
                    info = translate('RTOC', 'New version available. Update in terminal:\n\n "pip3 install RTOC --upgrade"\n')
        else:
    ...
    def clearCache(self):
        ok = pyqtlib.alert_message(translate('RTOC', 'Clear cache'), translate('RTOC', 'Are you sure you want to empty the cache?'), translate('RTOC', 'This will cause all settings to be lost.'))
        if ok:
            self.plotStyles = {}
            self.logger.clearCache()

    def openPluginDownloader(self):
        self.pluginDownloader = PluginDownloader(self.config['global']['documentfolder'], 'Haschtl/RTOC-Plugins', self)
    ...
    def updateDeviceRAW(self):
        ...
        for name in self.logger.devicenames.keys():
            if self.logger.pluginStatus[name] is True:
                for fun in self.logger.pluginFunctions.keys():
                    hiddenFuncs = [
                     'loadGUI', 'updateT', 'stream', 'plot', 'event', 'close', 'cancel', 'start', 'setSamplerate', 'setDeviceName', 'setPerpetualTimer', 'setInterval', 'getDir', 'telegram_send_plot', 'telegram_send_photo', 'telegram_send_message', 'telegram_send_document']
                ...
                for fun in self.logger.pluginParameters.keys():
                    hiddenParams = [
                     'run', 'smallGUI', 'widget', 'lockPerpetialTimer', 'logger']
        ...
        for host in dicti.keys():
            for sig in dicti[host]:
    ...
    def deviceRAWCallback(self, strung):
        ...
        if len(devsplit) == 2:
            ...
            if '(' in function and function.endswith(')'):
                text, ok = pyqtlib.text_message(self, translate('RTOC', 'Call device-function'), strung, translate('RTOC', 'Function parameters'))
                if ok:
                    self.par = []
                    try:
                        exec('self.par = [' + text + ']')
                        ...
                        if host == 'local':
                            ...
                            ans = self.logger.remote.callFuncOrParam(host, plugin, function, self.par)
                    except Exception:
                        tb = traceback.format_exc()
                        logging.debug(tb)
                        pyqtlib.info_message(translate('RTOC', 'Error'), translate('RTOC', 'Function-arguments invalid'), translate('RTOC', 'Please enter valid arguments'))

            else:
                if host == 'local':
                    ans = self.logger.handleTcpPlugins({plugin: {'get': [function]}})
                    text, ok = pyqtlib.text_message(self, translate('RTOC', 'Change device parameter'), strung, str(ans[plugin]['get'][0]))
                    if ok:
                        self.par = []
                        try:
                            exec('self.par = ' + text)
                            ...
                            tb = traceback.format_exc()
                            logging.debug(tb)
                            pyqtlib.info_message(translate('RTOC', 'Error'), translate('RTOC', 'Value invalid'), translate('RTOC', 'Please enter a valid value for this parameter'))

                else:
                    current_value = self.logger.remote.getParam(host, plugin, function)
                    if current_value is not None:
                        text, ok = pyqtlib.text_message(self, translate('RTOC', 'Change device parameter'), strung, str(current_value))
                        if ok:
                            self.par = []
                            try:
                                exec('self.par = ' + text)
    ...
    def updateNetworkMenu(self):
        ...
        for s in self.config['websocket']['knownHosts'].keys():
            if s not in activeConnections:
                action = self.menuMit_Remotehost_verbinden.addAction(self.config['websocket']['knownHosts'][s][0] + ' (' + s + ')')
                action.triggered.connect(partial(self.connectHost, s, self.config['websocket']['knownHosts'][s][0], self.config['websocket']['knownHosts'][s][1]))
    ...
    def connectNewHost(self):
        ans, ok = pyqtlib.text_message(self, translate('RTOC', 'Connect with new host'), translate('RTOC', 'Connect to new remote RTOC. Enter a valid address\n(Including port ":")'), '127.0.0.1:5050')
        if ok:
            ans2, ok = pyqtlib.text_message(self, translate('RTOC', 'Enter name'), translate('RTOC', 'Enter a name for this host\n(May not contain "." or ":")'), 'RemoteRTOC')
            if ok:
                ans2 = ans2.replace('.', 'Dot').replace(':', 'DDot')
                if len(ans.split(':')) == 2:
                    if ans not in self.config['websocket']['knownHosts'].keys():
                        self.config['websocket']['knownHosts'][ans] = [
                         ans2, '']
    ...
    def searchRTOCServer(self):
        t = Thread(target=(self.logger.remote.searchWebsocketHosts), args=(
         5050, self.foundRTOCServerCallback))
        t.start()

    def foundRTOCServer(self, hostlist):
        strung = '\n'.join(hostlist)
        pyqtlib.info_message(translate('RTOC', 'Finished'), translate('RTOC', 'RTOC-Search completed'), translate('RTOC', '{} servers found:\n{}').format(len(hostlist) - 1, strung))
    ...
    def connectHost(self, host, name, password=''):
        ...
        retry = True
        host = hostsplit[0]
        self.logger.remote.getConnection(host, port)._Actions__password = password
        while retry:
            retry = False
            status = self.logger.remote.getConnection(host, port).status
            if status == 'protected':
                text, ok2 = pyqtlib.text_message(None, translate('RTOC', 'Password'), translate('RTOC', 'The RTOC server {} is password-protected. Please enter your password.').format(hostname), translate('RTOC', 'Websocket-Password'))
                if ok2:
                    self.logger.remote.connect(host, port, name, text)
                    retry = True
                    ok3 = pyqtlib.alert_message(translate('RTOC', 'Save password'), translate('RTOC', 'Do you want to save this password?'), '')
                    if ok3:
                        self.logger.config['websocket']['knownHosts'][hostname][1] = text
            elif status == 'connected':
                pyqtlib.info_message(translate('RTOC', 'Connection established'), translate('RTOC', 'Connection to {} on port {} established.').format(host, port), '')
                ...
                if status == 'wrongPassword':
                    ...
                    if status == 'error':
                        ...
                        if status == 'connecting...':
                            ...
                            if status == 'closed':
                                print(status)
                                pyqtlib.info_message('Connection closed', 'The connection has been accidently closed', "Maybe you're trying to connect to an SSL-encrypted port, which is not on Port 443.".format(status))
                            else:
                                print(status)
                                pyqtlib.info_message('End of the universe', 'You reached the end of the universe', "This shouldn't happen. Status {}".format(status))
