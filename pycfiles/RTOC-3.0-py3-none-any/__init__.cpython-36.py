...
# [GCC 8.4.0]
# Embedded file name: D:\Benutzer\haschtl\Dokumente\GIT\kellerlogger\RTOC\__init__.py
# Compiled at: 2019-12-27 22:58:31
# Size of source mod 2**32: 10103 bytes
import sys, os, json, getopt, traceback, logging as log
...
def configureRTOC(arg):
    ...
    if arg == 'list':
        ...
        for key in config.keys():
            if key not in ('csv_profiles', 'telegram_chat_ids', 'lastSessions', 'grid',
                           'plotLegendEnabled', 'blinkingIdentifier', 'scriptWidget',
                           'deviceWidget', 'signalsWidget', 'pluginsWidget', 'eventWidget',
                           'newSignalSymbols', 'plotLabelsEnabled', 'plotGridEnabled',
                           'plotLegendEnabled', 'signalStyles', 'plotInverted', 'plotRate',
        ...
        splitted = arg.split('=')
        if len(splitted) != 2:
            logging.info('Please provide options like this: "python3 -m RTOC -c websocketserver=False\nYour entry didn\'t include a "="')
            sys.exit(1)
        else:
    ...
    try:
        if t != bool:
            ...
            if splitted[1].lower() in ('true', '1', 't', 'y', 'yes', 'yeah', 'yup',
                                       'certainly', 'uh-huh', 'ja', 'j', 'jupp',
                                       'jawohl'):
                newValue = True
            else:
                newValue = False
...
def setStyleSheet(app, myapp):
    ...
    with open(packagedir + '/RTOC_GUI/ui/darkmode.html', 'r') as (myfile):
        stylesheet = myfile.read().replace('\n', '')
    stylesheet = stylesheet.replace('/RTOC_GUI/ui/icons', os.path.join(packagedir, 'data', 'ui', 'icons').replace('\\', '/'))
    app.setStyleSheet(stylesheet)
    return (app, myapp)
