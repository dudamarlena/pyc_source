...
# [GCC 8.4.0]
# Embedded file name: D:\Benutzer\haschtl\Dokumente\GIT\kellerlogger\RTOC\RTOC_GUI\RTPlotWidget.py
# Compiled at: 2019-10-07 13:58:21
...
class RTPlotWidget(QtWidgets.QWidget, RTPlotActions):
    ...
    def removeSignal(self, id, devicename, signalname):
        ...
        self.deviceTreeWidgetItems[self.devices.index(devicename)].removeChild(self.signalTreeWidgetItems[idx])
        if self.deviceTreeWidgetItems[self.devices.index(devicename)].childCount() == 0:
            self.treeWidget.removeItemWidget(self.deviceTreeWidgetItems[self.devices.index(devicename)], 0)
    ...
    def addSignalRAW(self, signalObject):
        ...
        self.deviceTreeWidgetItems[self.devices.index(devicename)].addChild(self.signalTreeWidgetItems[idx])
        self.treeWidget.setItemWidget(self.signalTreeWidgetItems[idx], 0, self.signalObjects[idx])
        self.treeWidget.setItemWidget(self.signalTreeWidgetItems[idx], 1, self.signalObjects[idx].label)
    ...
    def signalClickedAction(self, devicename, signalname):
        r = 0.001
        if abs(self.mouseX - self.lastSignalClick[0]) < r:
            if abs(self.mouseY - self.lastSignalClick[1]) < r:
    ...
    def startDragTreeWidget(self, actions):
        self.self._drag_info = {'oldWidget':'', 
