...
# [GCC 8.4.0]
# Embedded file name: D:\Benutzer\haschtl\Dokumente\GIT\kellerlogger\RTOC\RTOC_Import.py
# Compiled at: 2019-05-13 18:50:31
# Size of source mod 2**32: 25487 bytes
import csv, os
...
import traceback, re, logging as log
...
class RTOC_Import(QtWidgets.QMainWindow):
    ...
    def loadCsvAction(self):
        fileName, _ = QtWidgets.QFileDialog.getOpenFileName(self, translate('csveditor', 'CSV-Datei öffnen'), QtCore.QDir.homePath(), 'Tabelle (*.csv *.tsv, *.xls, *.xlsx, *.txt, *.mat, *)')
        if fileName:
            self.loadCsv(fileName)
    ...
    def writeCsv(self, fileName):
        ...
        fileName, _ = QtWidgets.QFileDialog.getSaveFileName(self, translate('csveditor', 'CSV-Datei speichern'), QtCore.QDir.homePath() + '/' + self.fname + '.csv', 'CSV Files (*.csv)')
        if fileName:
    ...
    def contextMenuEventCSV(self, event):
        ...
        addColumnBeforeAction = QtWidgets.QAction(translate('csveditor', 'Spalte einfügen davor'), self)
        addColumnBeforeAction.triggered.connect(lambda : self.addColumnBeforeByContext(event))
        addColumnAfterAction = QtWidgets.QAction(translate('csveditor', 'Spalte einfügen danach'), self)
    ...
    def removeCurrentSignal(self):
        ...
        if idx < self.signalListWidget.count() - 1:
