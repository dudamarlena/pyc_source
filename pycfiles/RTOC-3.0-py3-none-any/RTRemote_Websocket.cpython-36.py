...
# [GCC 8.4.0]
# Embedded file name: D:\Benutzer\haschtl\Dokumente\GIT\kellerlogger\RTOC\RTLogger\RTRemote_Websocket.py
# Compiled at: 2019-12-28 00:19:39
...
class _SingleConnection:
    ...
    def __init__(self, host='127.0.0.1', port=5050, name='RemoteDevice', password='', logger=None, ssl=False):
        self.logger = logger
        self.host = host
        self.name = name
        self.port = port
    ...
    def initAfterConnection(self):
        ...
        self.RTwebsocket.send(logger='info')
    ...
    def handle_signalList(self, signalList):
        ...
        self.connected = True
        self.status = 'connected'
        if signalList != self.siglist:
            self.siglist = signalList
            if self.updateRemoteCallback is not None:
                self.updateRemoteCallback()
            if self.siglist != []:
                selection = []
...
class RTRemote:
    ...
    def connect(self, hostname=None, port=None, name=None, password=None, ssl=None):
        ...
        for c in self.connections:
            if c.host == hostname:
                if c.port == port:
                    c._RTRemote__password = password
                    c.ssl = ssl
                    if not c.connected:
                        c.connect()
        ...
        newConnection = _SingleConnection(hostname, port, name, password, self.logger, ssl)
