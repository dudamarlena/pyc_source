...
# [GCC 8.4.0]
# Embedded file name: D:\Benutzer\haschtl\Dokumente\GIT\kellerlogger\RTOC\RTOC_GUI\scriptSubWidget.py
# Compiled at: 2019-06-12 19:15:05
...
class ScriptSubWidget(QtWidgets.QWidget):
    ...
    def __init__(self, logger, scriptstr, filepath=''):
        ...
        self.triggerWidget.triggerSamplerateSpinBox.valueChanged.connect(self.updateTriggerSamplerate)
        self.triggerWidget.triggerSignals.itemSelectionChanged.connect(self.updateSelectedTriggerSignals)
        ...
        self._ScriptSubWidget__updater = Thread(target=(self.samplerateTriggeredThread))
        self._ScriptSubWidget__updater.start()
    ...
    def saveScript(self):
        if self.filepath == '':
            ...
            self.filepath, mask = fileBrowser.getSaveFileName(self, translate('RTOC', 'Save script as...'), self.filepath, 'Python (*.py)')
        if self.filepath:
    ...
    def errorButton(self):
        if self.startScriptButton.isChecked():
            if self.startScriptButton.styleSheet != 'QToolButton{background-color: #8c2020}':
    ...
    def samplerateTriggeredThread(self):
        ...
        while self.run:
            ...
            if self.scriptSamplerate != 0:
                ...
                if self.scriptEnabled:
                    if self.triggerMethod is 'samplerate':
                        self.executeScript()
    ...
    def closeEvent(self, event):
        if self.saved:
            ...
            ok = pyqtlib.alert_message(translate('RTOC', 'Warning'), translate('RTOC', 'Do you want to save the script?'), '', '', translate('RTOC', 'Yes'), translate('RTOC', 'No'))
        if ok is not None:
            if ok is True:
                self.saveScript()
            self.run = False
