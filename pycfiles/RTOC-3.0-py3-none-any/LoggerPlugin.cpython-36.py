...
# [GCC 8.4.0]
# Embedded file name: D:\Benutzer\haschtl\Dokumente\GIT\kellerlogger\RTOC\LoggerPlugin.py
# Compiled at: 2020-01-06 09:41:50
# Size of source mod 2**32: 25147 bytes
...
class LoggerPlugin:
        ...
        **seperate**
        ...
        if self._cb:
            ...
            if slist is not None:
                ...
                snames = []
                for sig in slist:
                    if type(sig) == list and len(sig) == 3:
                        y.append(sig[0])
                        snames.append(sig[1])
                        unit.append(sig[2])
                        x.append(now)
    ...
    def setPerpetualTimer(self, fun, samplerate=None, interval=None, old=False):
        ...
        try:
            self._samplerate = samplerate
            self._pluginThread = Thread(target=(self._LoggerPlugin__updateT), args=(fun,))
            return True
        except Exception:
            self._pluginThread = None
            return False
