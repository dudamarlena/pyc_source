...
# [GCC 8.4.0]
# Embedded file name: D:\Benutzer\haschtl\Dokumente\GIT\kellerlogger\RTOC\RTOC.py
# Compiled at: 2020-01-06 17:17:48
# Size of source mod 2**32: 30665 bytes
"""
...
class _SubWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, logger, selfself, idx, title='SubWindow'):
        super(_SubWindow, self).__init__()
...
class RTOC(QtWidgets.QMainWindow, Actions):
    __doc__ = 'GUI-code is not documented.'
        ...
        if self.logger.database is not None:
            ...
            if self.config['postgresql']['active']:
                if self.logger.database.status != 'connected':
                    pyqtlib.info_message(translate('RTOC', 'Database error'), translate('RTOC', 'Could not connect to PostgreSQL database.'), translate('RTOC', 'Please adjust the PostgreSQL options in the settings and restart RTOC afterwards.'))
            else:
        ...
        for plugin in self.logger.remote.devicenames.keys():
            if plugin not in donotreloadplugins:
                ...
                if self.logger.remote.pluginStatus[plugin] is True:
                    button.setChecked(True)
                name = plugin.split(':')
                button.clicked.connect(partial(self.logger.remote.toggleDevice, name[0], name[1], button))
                self.deviceLayout.addWidget(button)
    ...
    def newPlotWidget(self):
        ...
        if self.activePlotWidgetIndex == 0:
            ...
            plotWidget = _SubWindow(self.logger, self, self.activePlotWidgetIndex, 'Graph ' + str(len(self.plotWidgets) + 1))
            self.plotWidgets.append(plotWidget)
    ...
    def moveSignal(self, oldIdx, newIdx, signalItem):
        if oldIdx != newIdx:
            if signalItem.childCount() == 0:
                ...
                if signalObject is not None:
                    if signalLabel is not None:
                ...
                self._drag_info = {'oldWidget':'', 
                 'newWidget':'',  'signalObjects':[]}
            else:
                ...
                try:
                    ...
                    if callable(invert_op):
                        ...
                        if self.logger.pluginObjects[deviceName].smallGUI is None:
                            ...
                            if self.logger.pluginObjects[deviceName].smallGUI is True:
                    ...
                    logging.debug(tb)
                    pyqtlib.info_message(translate('RTOC', 'Error'), translate('RTOC', 'Error loading plugin-GUI. Please check your code.'), tb)

            else:
                pyqtlib.info_message(translate('RTOC', 'Error'), translate('RTOC', 'Error loading plugin. Please make sure the plugin is correct.'), errors)
                button.setChecked(False)
            self.scriptWidget.updateListWidget()
            self.updateDeviceRAW()

    def addNewSignal(self, id, devicename, signalname, dataunit):
        self.plotWidgets[self.activePlotWidgetIndex].addSignal2.emit(devicename, signalname, id, dataunit)
        self.scriptWidget.updateListWidget()

    def newDataCallback(self, devicename, signalname):
        for plot in self.plotWidgets:
            for signal in plot.signalObjects:
                if signal.devicename == devicename:
    ...
    def loadSession(self, fileName='restore.json', clear=None):
        if clear is None:
            clear = pyqtlib.alert_message(translate('RTOC', 'Merge sessions'), translate('RTOC', 'Do you want to remove the current session?'), translate('RTOC', 'Unsaved data will be lost.'), '', translate('RTOC', 'Yes'), translate('RTOC', 'No'))
        else:
            if clear:
                self.clearData()
    ...
    def closeEvent(self, event):
        if self.systemTrayAction.isChecked():
            if not self.forceQuit:
                ...
                self.tray_icon.showMessage(translate('RTOC', 'RealTime OpenControl'), t, self.app_icon, 2000)
        else:
            self.savePlotStyles()
            if len(self.logger.database.signals()) > 0:
                if not self.logger.config['postgresql']['active']:
                    ok = pyqtlib.tri_message(translate('RTOC', 'Save'), translate('RTOC', 'Do you want to save the current session?'), '')
            else:
                ok = False
            if ok is not None:
                if ok is True:
                    self.logger.database.exportJSON(self.config['global']['documentfolder'] + '/restore.json', True)
                else:
                    if ok is False:
    ...
    def readSettings(self):
        ...
        if self.settings.value('devicesGeometry') is not None:
    ...
    def dragEnterEvent(self, e):
        ...
        if e.mimeData().hasUrls or e.mimeData().hasText:
            ...
            if elementContent == '':
                ...
                text = 'Datentyp erkannt: ' + elementType
    ...
    def loadWav(self, filename):
        try:
            ...
            text, ok = pyqtlib.text_message(self, translate('RTOC', 'Enter signalname'), translate('RTOC', 'Specify a name for the signal to be imported.'), os.path.splitext(str(filename))[0].split('/')[(-1)] + '.Signal')
            if ok:
                y = list(data)
        ...
        if e.mimeData().hasText:
            ...
            if elementType == 'pfad':
                if elementContent.endswith('.json'):
                    ...
                    if elementContent.endswith('.wav') or elementContent.endswith('.wave'):
