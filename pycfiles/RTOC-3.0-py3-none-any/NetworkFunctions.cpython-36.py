...
# [GCC 8.4.0]
# Embedded file name: D:\Benutzer\haschtl\Dokumente\GIT\kellerlogger\RTOC\RTLogger\NetworkFunctions.py
# Compiled at: 2020-01-06 08:50:03
# Size of source mod 2**32: 19041 bytes
import os, traceback
...
class NetworkFunctions:
    ...
    def toggleTelegramBot(self, value=None):
        ...
        if telegramBot is not None:
            ...
            logging.error('Telegram for python not installed. Please install with "pip3 install python-telegram-bot"')
            return False
        ...
        if value is None:
            ...
            if value is True:
                try:
                    ...
                    self._NetworkFunctions__tcpserver = Thread(target=(self._tcpListener))
                    self._NetworkFunctions__tcpserver.start()
                ...
                if type(pluginDicts[plugin]) == dict:
                    for call in pluginDicts[plugin].keys():
                        if call == 'start' and type(pluginDicts[plugin][call]) == bool:
                            ...
                            if '()' in call:
                                pluginDicts[plugin][call] = (self.callPluginFunction)(plugin, call.replace('()', ''), *pluginDicts[plugin][call])
                            else:
                                pluginDicts[plugin][call] = self.getPluginParameter(plugin, call, pluginDicts[plugin][call])
                        ...
                        if call == 'export':
                            if type(loggerDict[call]) == list and len(loggerDict[call]) <= 2:
                                ...
                                if call == 'info':
                                    ...
                                    self.database.database.exportJSON(self.config['global']['documentfolder'] + '/restore.json', None, True)
                                    self.save_config()
