...
# [GCC 8.4.0]
# Embedded file name: D:\Benutzer\haschtl\Dokumente\GIT\kellerlogger\RTOC\RTOC_GUI\settingsWidget.py
# Compiled at: 2019-12-27 23:08:39
# Size of source mod 2**32: 13415 bytes
...
class SettingsWidget(QtWidgets.QWidget):
    ...
    def __init__(self, config, defaultconfig, parent=None):
        ...
        self.darkmodeCheckBox.stateChanged.connect(partial(self.toggle, self.darkmodeCheckBox, ['GUI', 'darkmode']))
        self.autoShowCheckBox.stateChanged.connect(partial(self.toggle, self.autoShowCheckBox, ['GUI', 'autoShowGraph']))
        self.antialiasingCheckBox.stateChanged.connect(partial(self.toggle, self.antialiasingCheckBox, ['GUI', 'antiAliasing']))
        self.openGLCheckBox.stateChanged.connect(partial(self.toggle, self.openGLCheckBox, ['GUI', 'openGL']))
        self.weaveCheckBox.stateChanged.connect(partial(self.toggle, self.weaveCheckBox, ['GUI', 'useWeave']))
        self.restoreWidgetPositionCheckBox.stateChanged.connect(partial(self.toggle, self.weaveCheckBox, ['GUI', 'restoreWidgetPosition']))
        ...
        self.backupAutoOnCloseCheckbox.stateChanged.connect(partial(self.toggle, self.postgresqlCheckbox, ['backup', 'autoOnClose']))
        self.backupLoadOnOpenCheckbox.stateChanged.connect(partial(self.toggle, self.postgresqlCheckbox, ['backup', 'loadOnOpen']))
        self.postgresqlCheckbox.stateChanged.connect(partial(self.toggle, self.postgresqlCheckbox, ['postgresql', 'active']))
        self.postgresqlUserLineEdit.textChanged.connect(partial(self.changeName, ['postgresql', 'user']))
        self.postgresqlPasswordLineEdit.textChanged.connect(partial(self.changeName, ['postgresql', 'password']))
        self.postgresqlHostLineEdit.textChanged.connect(partial(self.changeName, ['postgresql', 'host']))
        self.postgresqlNameLineEdit.textChanged.connect(partial(self.changeName, ['postgresql', 'database']))
        self.postgresqlPortSpinBox.valueChanged.connect(partial(self.changeValue, ['postgresql', 'port']))
    ...
    def initView(self):
        ...
        day = timestamp // 86400
        hour = (timestamp - day * 24 * 60 * 60) // 3600
    ...
    def checkPostgreSQL(self):
        if self.config['postgresql']['active']:
            if psycopg2 is not None:
                try:
                    connection = psycopg2.connect(user=(self.config['postgresql']['user']),
    ...
    def setBackupDir(self):
        ...
        fname, mask = fileBrowser.getExistingDirectory(self, translate('RTOC', 'Set backup path'), dir_path)
        if fname:
