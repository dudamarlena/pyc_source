...
# [GCC 8.4.0]
# Embedded file name: D:\Benutzer\haschtl\Dokumente\GIT\kellerlogger\RTOC\RTOC_GUI\stylePlotGUI.py
# Compiled at: 2019-06-12 20:37:43
# Size of source mod 2**32: 16012 bytes
from PyQt5 import QtCore
from PyQt5 import uic
from PyQt5 import QtWidgets, QtGui
import pyqtgraph as pg, os, sys
...
class plotStyler(QtWidgets.QDialog):
    ...
    def setGUIStyle(self, symbol, brush):
        if 'style' in symbol.keys():
            ...
            if style == QtCore.Qt.SolidLine:
                ...
                if style == QtCore.Qt.DotLine:
                    idx = 1
                else:
                    if style == QtCore.Qt.DashLine:
                        idx = 2
                    else:
                        if style == QtCore.Qt.DashDotLine:
                            idx = 3
                        else:
                            if style == QtCore.Qt.DashDotDotLine:
                                idx = 4
            ...
            if 'shadowColor' in symbol.keys():
                ...
                if type(self.shadowColor) == str:
                    ...
                    if type(self.shadowColor) == tuple:
                        self.shadowColorButton.setStyleSheet('background-color:' + str(self.shadowColor))
                    elif self.shadowColor is not None:
            ...
            if 'color' in brush.keys():
                ...
                if type(self.symbolColor) == str:
                    ...
                    if type(self.symbolColor) == tuple:
                        self.symbolColorButton.setStyleSheet('background-color:' + str(self.symbolColor))
                    elif self.shadowColor is not None:
            ...
            if 'pen' in brush.keys():
                ...
                if type(self.symbolPenColor) == str:
                    ...
                    if type(self.symbolPenColor) == tuple:
                        self.symbolPenColorButton.setStyleSheet('background-color:' + str(self.symbolPenColor))
                    elif self.shadowColor is not None:
    ...
    def getGUIStyle(self):
        ...
        if style == translate('RTOC', 'Line'):
            ...
            if style == translate('RTOC', 'Dots (D)'):
                symbol['style'] = QtCore.Qt.DotLine
            else:
                if style == translate('RTOC', 'Strokes (S)'):
                    symbol['style'] = QtCore.Qt.DashLine
                else:
                    if style == translate('RTOC', 'D S'):
                        symbol['style'] = QtCore.Qt.DashDotLine
                    else:
                        if style == translate('RTOC', 'D D S'):
        ...
        if self.lineColor is not None:
            ...
            if style in [translate('RTOC', 'Rectangle'), translate('RTOC', 'Diamond')]:
                ...
                if style == translate('RTOC', 'Circle'):
                    brush['style'] = 'o'
                else:
                    if style == 'X':
                        brush['style'] = 'x'
                    else:
                        if style == translate('RTOC', 'Square'):
                            brush['style'] = 's'
                        else:
                            if style == translate('RTOC', 'Triangle'):
                                brush['style'] = 't'
                            else:
                                if style == 'Plus':
                                    brush['style'] = '+'
...
def setStyle(plot, symbol={}, brush={}):
    if symbol != {}:
        if 'width' not in symbol.keys():
            ...
            pen = pg.mkPen(color=(symbol['color']), width=(int(symbol['width'])),
              style=(int(symbol['style'])))
            plot.setPen(pen)
            if symbol['shadowWidth'] is not None and symbol['shadowStyle'] is not None:
                plot.setShadowPen(color=(symbol['shadowColor']), width=(int(symbol['shadowWidth'])),
                  style=(int(symbol['shadowStyle'])),
                  cosmetic=True)
            else:
