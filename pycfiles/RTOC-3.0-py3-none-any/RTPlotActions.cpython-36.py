...
# [GCC 8.4.0]
# Embedded file name: D:\Benutzer\haschtl\Dokumente\GIT\kellerlogger\RTOC\RTOC_GUI\RTPlotActions.py
# Compiled at: 2019-12-27 23:08:18
# Size of source mod 2**32: 23804 bytes
import pyqtgraph as pg, pyqtgraph.console as pgconsole
...
class RTPlotActions:
    ...
    def initPlotWidget(self):
        ...
        self.plot.getPlotItem().ctrlMenu = None
        axis = pyqtlib.TimeAxisItem(orientation='bottom')
        axis.attachToPlotItem(self.plot.getPlotItem())
        ...
        self.plotMouseLabel = pg.TextItem('', color=(200, 200, 200),
          fill=(200, 200, 200, 50),
          html=None)
    ...
    def initPlotViewWidget(self):
        ...
        self.plotViewButton.setStyleSheet('QToolButton:checked, QToolButton:pressed,QToolButton::menu-button:pressed { background-color: #76797C;border: 1px solid #76797C;padding: 5px;}')
        self.plotViewButton.menu().addAction(action)
        self.plotViewButton.clicked.connect(lambda : self.plotViewButton.menu().popup(self.plotViewButton.mapToGlobal(QtCore.QPoint(0, 35))))
        ...
        self.plotViewWidget.gridButton.setStyleSheet('QToolButton:checked, QToolButton:pressed,QToolButton::menu-button:pressed { background-color: #76797C;border: 1px solid #76797C;padding: 5px;}')
    ...
    def initROIS(self):
        self.rois = []
        self.rect = pg.RectROI(pos=(1, 1), size=(100, 100), removable=True, pen=pg.mkPen((pg.mkColor(9, 169, 188)), width=3,
    ...
    def initCrosshair(self):
        ...
        self.CrossHairLabel = pg.TextItem('', color=(200, 200, 200),
          fill=(200, 200, 200, 50),
          html=None)
    ...
    def initCutTool(self):
        self.cutVLine1 = pg.InfiniteLine(angle=90, movable=True, label='{value:0.2f}', labelOpts={'position':0.1, 
         'color':(200, 200, 100),  'fill':(200, 200, 200, 50),  'movable':True})
        self.cutVLine2 = pg.InfiniteLine(angle=90, movable=True, label='{value:0.2f}', labelOpts={'position':0.1, 
         'color':(200, 200, 100),  'fill':(200, 200, 200, 50),  'movable':True})
    ...
    def initMeasureTool(self):
        ...
        self.measureWidget.setWindowFlags(QtCore.Qt.FramelessWindowHint | QtCore.Qt.WindowStaysOnTopHint)
        uic.loadUi(packagedir + '/ui/messtoolDialog.ui', self.measureWidget)
        self.measureWidget.setStyleSheet(self.styleSheet().replace('background-color: rgba(49, 54, 59, .9)', 'background-color: rgba(34, 36, 38, 0.9)'))
    ...
    def updateROI(self, roi):
        ...
        self.measureWidget.ROIXMaxLabel.setText(str(round(list(roi.pos())[0] + list(roi.size())[0], 4)) + 's')
        self.measureWidget.ROIYMinLabel.setText(str(round(list(roi.pos())[1], 4)))
        self.measureWidget.ROIYMaxLabel.setText(str(round(list(roi.pos())[1] + list(roi.size())[1], 4)))
    ...
    def mouseMoveEvents(self, event):
        ...
        self.measureWidget.move(self.measureWidget.x() + delta.x(), self.measureWidget.y() + delta.y())
        self.measureWidget.oldPos = event.globalPos()
