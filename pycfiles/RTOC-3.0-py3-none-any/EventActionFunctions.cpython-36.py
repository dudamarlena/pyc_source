...
# [GCC 8.4.0]
# Embedded file name: D:\Benutzer\haschtl\Dokumente\GIT\kellerlogger\RTOC\RTLogger\EventActionFunctions.py
# Compiled at: 2020-01-06 16:33:26
# Size of source mod 2**32: 14253 bytes
...
class EventActionFunctions:
    ...
    def loadGlobalEvents(self):
        ...
        if not os.path.exists(userpath):
            ...
            if os.path.exists(userpath + '/globalEvents.json'):
                ...
                self.globalEvents = {'myevent1': {'cond':'Generator.Square.latest >= 2', 
                              'text':'It is big', 
                              'return':' ', 
                              'priority':0, 
                              'id':'testID', 
    ...
    def loadGlobalActions(self):
        ...
        if os.path.exists(userpath + '/globalActions.json'):
            ...
            self.globalActions = {'action2': {'listenID':[
                          'testID'], 
                         'parameters':'', 
                         'script':'Generator.gen_level = 1', 
                         'active':False}}
            return
            empty = {'listenID':[],  'parameters':'', 
             'script':'', 
             'active':False}
            ...
            for actionId in action['listenID']:
                if actionId == id:
                    if action['active']:
                        ...
                        print('Performing global action for eventID: ' + str(id) + ' with value: ' + str(value))
    ...
    def editGlobalEvent(self, name='testEvent', cond='', text='TestEvent', priority=0, retur='', id='testID', trigger='rising', sname='', dname='', active=True):
        self.globalEvents[name] = {'cond':cond, 
         'text':text,  'priority':priority, 
         'return':retur,  'id':id,  'trigger':trigger,  'sname':sname,  'dname':dname,  'active':active}
    ...
    def addGlobalEvent(self, name='testEvent', cond='', text='TestEvent', priority=0, retur='', id='testID', trigger='rising', sname='', dname='', active=True):
        ...
        name = countname
        self.globalEvents[name] = {'cond':cond,  'text':text,  'priority':priority, 
         'return':retur,  'id':id,  'trigger':trigger,  'sname':sname,  'dname':dname,  'active':active}
    ...
    def triggerGlobalEvent(self, key):
        if key in self.globalEvents.keys():
            ...
            if ok:
                ...
                self.database.addNewEvent(text=(event['text']), sname=(event['sname']), dname=(event['dname']), value=(event['return']),
                  priority=(event['priority']),
                  id=(event['id']))
                return (True, text)
