...
# [GCC 8.4.0]
# Embedded file name: D:\Benutzer\haschtl\Dokumente\GIT\kellerlogger\RTOC\RTOC_GUI\scriptHelpWidget.py
# Compiled at: 2019-12-30 18:07:33
...
class ScriptHelpWidget(QtWidgets.QWidget):
    ...
    def updateListWidget(self):
        ...
        for element in self.logger.pluginParameters.keys():
            if self.logger.pluginStatus[element.split('.')[0]] == True and element.split('.')[1] not in ('run',
                                                                                                         'smallGUI',
                                                                                                         'widget',
                                                                                                         'samplerate',
                                                                                                         'lockPerpetialTimer'):
        ...
        for element in self.logger.pluginFunctions.keys():
            if self.logger.pluginStatus[element.split('.')[0]] == True and element.split('.')[1] not in ('loadGUI',
                                                                                                         'updateT',
                                                                                                         'stream',
                                                                                                         'plot',
                                                                                                         'event',
