...
# [GCC 8.4.0]
# Embedded file name: D:\Benutzer\haschtl\Dokumente\GIT\kellerlogger\RTOC\RTLogger\ScriptFunctions.py
# Compiled at: 2020-01-02 14:39:07
...
class ScriptFunctions:
        ...
        for host in self.remote.devices.keys():
            for device in self.remote.devices[host].keys():
                for function in self.remote.devices[host][device]['functions']:
                    strung = "self.remote.callFuncOrParam2('" + host + "','" + device + "','" + function + "',None)"
    ...
    def replaceSignalNames(self, s):
        for name in self.database.signalNames():
            ...
            if sigId != -1:
                s = s.replace(name[0] + '.' + name[1] + '.x', 'np.array(self.database.signals()[' + str(sigId) + '][2])')
                s = s.replace(name[0] + '.' + name[1] + '.y', 'np.array(self.database.signals()[' + str(sigId) + '][3])')
                s = s.replace(name[0] + '.' + name[1] + '.latest', 'self.database.signals()[' + str(sigId) + '][3][-1]')
                s = s.replace(name[0] + '.' + name[1], 'np.array(self.database.signals()[' + str(sigId) + '][2]), np.array(self.database.signals()[' + str(sigId) + '][3])')
    ...
    def generateTriggerCode(self, scriptStr):
        ...
        for item in scriptStr.split('\n'):
            if 'trig ' in item:
                ...
                scriptStr = scriptStr.replace(line, 'if self.triggerExpressionHandler("' + expression + '"):')
