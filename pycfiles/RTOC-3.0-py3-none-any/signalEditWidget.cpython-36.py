...
# [GCC 8.4.0]
# Embedded file name: D:\Benutzer\haschtl\Dokumente\GIT\kellerlogger\RTOC\RTOC_GUI\signalEditWidget.py
# Compiled at: 2019-12-27 23:12:59
# Size of source mod 2**32: 11351 bytes
from PyQt5 import QtWidgets
from PyQt5.QtCore import QCoreApplication
from PyQt5 import uic
import time, os, numpy as np, sys
...
class SignalEditWidget(QtWidgets.QWidget):
    ...
    def toggleEvents(self):
        if self.eventButton.isChecked():
            if self.self.logger.config['GUI']['showEvents']:
        ...
        if dialog.exec_():
            ...
            for key in brush.keys():
                if brush[key] is None:
                    ...
                    if type(brush[key]) not in [int, float, str]:
    ...
    def cutSignal(self):
        if not self.plotWidget.cutButton.isChecked():
            pyqtlib.info_message(translate('RTOC', 'Info'), translate('RTOC', 'Select cutting tool first'), translate('RTOC', 'You need to select the cutting area first.'))
        else:
            ...
            if x1 == x2:
                ...
                if xmin != xmax:
                    ...
                    if maxIdx > minIdx:
                        signalname = self.self.logger.database.getSignalName(self.id)[1] + '_cut' + str(int(xmax - xmin)) + 's'
    ...
    def exportSignal(self):
        ...
        fname, mask = fileBrowser.getSaveFileName(self, translate('RTOC', 'Export'), dir_path, translate('RTOC', 'CSV file (*.csv)'))
        if fname:
            fileName = fname
    ...
    def renameSignal(self):
        name, ok = pyqtlib.text_message(self, translate('RTOC', 'Rename'), translate('RTOC', 'Please enter a new name'), self.self.logger.database.getSignalName(self.id)[1])
        if name != '':
            if ok:
    ...
    def submitModification(self):
        ok = pyqtlib.alert_message(translate('RTOC', 'Warning'), translate('RTOC', 'Data will be changed permanently'))
        if ok:
    ...
    def sendRemote(self):
        ...
        item, ok = pyqtlib.item_message(None, (translate('RTOC', 'Select host')),
          'Please select a known host', textlist, stylesheet='')
        if ok:
            ...
            for idx2, s in enumerate(self.self.logger.config['websocket']['knownHosts'].keys()):
                if idx == idx2:
                    ...
                    client = RTWebsocketClient(host, port, password).connect().send(x=(list(signal[2])), y=(list(signal[3])),
                      dname=(self.self.logger.config['global']['name'] + ':' + name[0]),
                      sname=(name[1]),
                      unit=(signal[4]))