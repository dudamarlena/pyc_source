...
# [GCC 8.4.0]
# Embedded file name: D:\Benutzer\haschtl\Dokumente\GIT\kellerlogger\RTOC\RTLogger\telegramBot.py
# Compiled at: 2019-12-30 18:07:10
# Size of source mod 2**32: 130364 bytes
import time
...
import os, traceback, psutil, datetime as dt, logging as log
...
class telegramBot:
    ...
    def check_chat_id(self, chat_id):
        ...
        if chat_id is not None:
            if str(chat_id) not in list(self.telegram_clients.keys()):
                ...
                user = {'eventlevel':self.logger.config['telegram']['default_eventlevel'], 
                 'shortcuts':[[], []],  'permission':ad,  'first_name':first_name,  'last_name':last_name,  'menu':'menu',  'preview':False}
                self.telegram_clients[str(chat_id)] = user
                self.saveClients()
            else:
                if type(self.telegram_clients[str(chat_id)]) != dict:
                ...
                if chat_id not in self.current_plugin.keys():
                    ...
                    self.signals_range[chat_id] = [time.time() - 86400, time.time()]

    def sendEvent(self, message, devicename, signalname, priority):
        ptext = [
         translate('RTOC', '_Information_'), translate('RTOC', '*Warning*'), translate('RTOC', '*_Error_*')][priority]
        ...
        for id in self.telegram_clients.keys():
            ...
            if priority >= self.telegram_clients[id]['eventlevel'] and self.telegram_clients[id]['permission'] in ('read',
                                                                                                                   'write',
                                                                                                                   'admin'):
        ...
        try:
            ...
            if WELCOME_MESSAGE:
                for client in self.telegram_clients.keys():
                    self.send_message(int(client), translate('RTOC', '{} was started.').format(self.logger.config['global']['name']))
    ...
    def send_message(self, chat_id, text, parse_mode=ParseMode.MARKDOWN, disable_notification=True, delete=True):
        if self.bot == None:
            ...
            if not self._permissionCheck(chat_id):
                return
            try:
                lastMessage = self.bot.send_message(chat_id,
                  text=text, disable_notification=True, parse_mode=(ParseMode.MARKDOWN))
            except Exception:
                print(traceback.format_exc())
                lastMessage = self.bot.send_message(chat_id,
    ...
    def _sendMenuMessage_inline(self, bot, chat_id, buttonlist, text, n_cols):
        ...
        try:
            lastMessage = self.bot.send_message(chat_id,
              text=text, reply_markup=reply_markup, disable_notification=True, parse_mode=(ParseMode.MARKDOWN))
            self._deleteMessageIfOld(chat_id, lastMessage)
        except Exception:
            try:
                lastMessage = self.bot.send_message(chat_id,
                  text=text, reply_markup=reply_markup, disable_notification=True)
                self._deleteMessageIfOld(chat_id, lastMessage)
            except Exception:
    ...
    def inlineMenuHandler(self, update, context):
        self.textHandler(self.bot, None, context['callback_query']['data'], context['callback_query']['message']['chat']['id'])
    ...
    def textHandler(self, bot, update, altStrung=None, alt_chat_id=None):
        ...
        if self.telegram_clients[str(chat_id)]['menu'] == 'menu':
            ...
            if self.telegram_clients[str(chat_id)]['menu'] == 'adjustEventNotification':
                self.adjustEventNotificationHandlerAns(bot, chat_id, strung)
            else:
                if self.telegram_clients[str(chat_id)]['menu'] == 'plugins':
                    self.devicesHandlerAns(bot, chat_id, strung)
                else:
                    if self.telegram_clients[str(chat_id)]['menu'] == 'plugin':
                        self.deviceHandlerAns(bot, chat_id, strung)
                    else:
                        if self.telegram_clients[str(chat_id)]['menu'] == 'pluginfunctions':
                            self.deviceFunctionsHandlerAns(bot, chat_id, strung)
                        else:
                            if self.telegram_clients[str(chat_id)]['menu'] == 'pluginsamplerate':
                                self.deviceSamplerateHandlerAns(bot, chat_id, strung)
                            else:
                                if self.telegram_clients[str(chat_id)]['menu'] == 'pluginparameters':
                                    self.deviceParametersHandlerAns(bot, chat_id, strung)
                                else:
                                    if self.telegram_clients[str(chat_id)]['menu'] == 'call' or self.telegram_clients[str(chat_id)]['menu'] == 'callShortcut':
                                        self.deviceCallHandlerAns(bot, chat_id, strung)
                                    else:
                                        if self.telegram_clients[str(chat_id)]['menu'] == 'signals':
                                            self.signalsHandlerAns(bot, chat_id, strung)
                                        else:
                                            if self.telegram_clients[str(chat_id)]['menu'] == 'signalSelectRange':
                                                self.signalsSelectRangeHandlerAns(bot, chat_id, strung)
                                            else:
                                                if self.telegram_clients[str(chat_id)]['menu'] == 'createEvent':
                                                    self.createEventHandlerAns(bot, chat_id, strung)
                                                else:
                                                    if self.telegram_clients[str(chat_id)]['menu'] == 'settings':
                                                        self.settingsHandlerAns(bot, chat_id, strung)
                                                    else:
                                                        if self.telegram_clients[str(chat_id)]['menu'] == 'settingsGeneral':
                                                            self.settingsGeneralHandlerAns(bot, chat_id, strung)
                                                        else:
                                                            if self.telegram_clients[str(chat_id)]['menu'] == 'settingsBot':
                                                                self.settingsBotHandlerAns(bot, chat_id, strung)
                                                            else:
                                                                if self.telegram_clients[str(chat_id)]['menu'] == 'settingsBackup':
                                                                    self.settingsBackupHandlerAns(bot, chat_id, strung)
                                                                else:
                                                                    if self.telegram_clients[str(chat_id)]['menu'] == 'resize':
                                                                        self.resizeHandlerAns(bot, chat_id, strung)
                                                                    else:
                                                                        if self.telegram_clients[str(chat_id)]['menu'] == 'shortcut':
                                                                            self.addShortcutAns(bot, chat_id, strung)
                                                                        else:
                                                                            if self.telegram_clients[str(chat_id)]['menu'] == 'globalSamplerate':
                                                                                self.globalSamplerateHandlerAns(bot, chat_id, strung)
                                                                            else:
                                                                                if self.telegram_clients[str(chat_id)]['menu'] == 'backupResampling':
                                                                                    self.backupResamplingHandlerAns(bot, chat_id, strung)
                                                                                else:
                                                                                    if self.telegram_clients[str(chat_id)]['menu'] == 'automation':
                                                                                        self.automationHandlerAns(bot, chat_id, strung)
                                                                                    else:
                                                                                        if self.telegram_clients[str(chat_id)]['menu'] == 'globalEvents':
                                                                                            self.globalEventsHandlerAns(bot, chat_id, strung)
                                                                                        else:
                                                                                            if self.telegram_clients[str(chat_id)]['menu'] == 'globalEvent':
                                                                                                self.globalEventHandlerAns(bot, chat_id, strung)
                                                                                            else:
                                                                                                if self.telegram_clients[str(chat_id)]['menu'] == 'globalActions':
                                                                                                    self.globalActionsHandlerAns(bot, chat_id, strung)
                                                                                                else:
                                                                                                    if self.telegram_clients[str(chat_id)]['menu'] == 'globalAction':
                                                                                                        self.globalActionHandlerAns(bot, chat_id, strung)
                                                                                                    else:
                                                                                                        if self.telegram_clients[str(chat_id)]['menu'].endswith(':signals'):
    ...
    def menuHandler(self, bot, chat_id):
        ...
        self.sendMenuMessage(bot, chat_id, commands, translate('RTOC', '*Mainmenu*'), '', 1, False)
    ...
    def menuHandlerAns(self, bot, chat_id, strung):
        if strung in self.menuCommands:
            ...
            if strung in list(self.logger.userActions.keys()):
                self.executeUserAction(bot, chat_id, strung)
            else:
                if strung in self.createShortcutList(bot, chat_id):
                    self.callShortcut(bot, chat_id, strung)
                else:
                    if self.telegram_clients[str(chat_id)]['permission'] in ('write',
                                                                             'admin') and strung in self.writeMenuCommands:
                    ...
                    elif self.telegram_clients[str(chat_id)]['preview']:
                        if strung == translate('RTOC', 'Exit preview'):
                            self.telegram_clients[str(chat_id)]['preview'] = False
    ...
    def adjustEventNotificationHandler(self, bot, chat_id):
        self.adjustEventNotificationCommands = [
         translate('RTOC', 'No notifications'), translate('RTOC', 'Only errors'), translate('RTOC', 'Warnings'), translate('RTOC', 'All events')]
    ...
    def adjustEventNotificationHandlerAns(self, bot, chat_id, strung):
        if strung in self.adjustEventNotificationCommands:
            ...
            if i <= 3:
                ...
                self.saveClients()
                self.send_message(chat_id=chat_id, text=(translate('RTOC', 'Settings applied')))
        self.settingsHandler(bot, chat_id)
    ...
    def sendLatest(self, bot, chat_id):
        ...
        for name in names:
            ...
            value = sig[1]
            unit = sig[2]
            strung = strung + name + '\t | ' + str(round(value, 2)) + '\t | ' + str(unit) + '\n'
    ...
    def devicesHandler(self, bot, chat_id):
        ...
        commands.sort()
        self.sendMenuMessage(bot, chat_id, commands, translate('RTOC', '*Devices*'))
    ...
    def deviceHandler(self, bot, chat_id, device):
        ...
        if device in self.logger.pluginStatus.keys():
            if self.logger.pluginStatus[device] == True:
                ...
                if self.logger.pluginStatus[device] == False:
                    commands = [
                     translate('RTOC', 'Start device')]
                    samplestr = ''
                else:
                    commands = [
                     translate('RTOC', 'Device-error')]
                    samplestr = ''
        else:
            commands = []
    ...
    def deviceHandlerAns(self, bot, chat_id, strung):
        if strung == translate('RTOC', 'Stop device'):
            ...
            if strung == translate('RTOC', 'Start device'):
                self.logger.startPlugin(self.current_plugin[chat_id])
                self.deviceHandler(bot, chat_id, self.current_plugin[chat_id])
            else:
                if strung == translate('RTOC', 'Functions'):
                    self.deviceFunctionsHandler(bot, chat_id)
                else:
                    if strung == translate('RTOC', 'Parameters'):
                        self.deviceParametersHandler(bot, chat_id)
                    else:
                        if strung == translate('RTOC', 'Change samplerate'):
    ...
    def deviceSamplerateHandlerAns(self, bot, chat_id, strung):
        ...
        if strung == self.BACKBUTTON:
            ...
            try:
                ...
                self.send_message(chat_id, translate('RTOC', 'Samplerate was changed'))
                self.deviceHandler(bot, chat_id, name)
            except:
                print(traceback.format_exc())
                self.send_message(chat_id, translate('RTOC', 'I did not understand your message'))
    ...
    def deviceFunctionsHandler(self, bot, chat_id):
        ...
        for fun in self.logger.pluginFunctions.keys():
            hiddenFuncs = [
             'loadGUI', 'updateT', 'stream', 'plot', 'event', 'createTCPClient', 'close', 'cancel', 'start', 'setSamplerate', 'setDeviceName', 'setPerpetualTimer', 'setInterval', 'getDir', 'telegram_send_message', 'telegram_send_photo', 'telegram_send_document', 'telegram_send_plot']
    ...
    def deviceParametersHandler(self, bot, chat_id):
        ...
        for fun in self.logger.pluginParameters.keys():
            hiddenParams = [
             'run', 'smallGUI', 'widget', 'samplerate', 'lockPerpetialTimer', 'logger']
    ...
    def deviceCallHandler(self, bot, chat_id, strung):
        if strung is not None:
            ...
            if self.current_call[chat_id].endswith('()') and commands == []:
                ...
                ok, ans = (self.logger.callPluginFunction)(self.current_plugin[chat_id], func, *self.temp)
                self._pluginCall_chat_id = None
                if ans is not None:
            ...
            if self.current_call[chat_id].endswith(')'):
                ...
                value = self.logger.getPluginParameter(self.current_plugin[chat_id], 'get', [
                 self.current_call[chat_id]])
            if value == False:
                devtext = self.current_plugin[chat_id] + '.' + self.current_call[chat_id]
    ...
    def deviceCallHandlerAns(self, bot, chat_id, strung):
        if strung == self.BACKBUTTON:
            ...
            if strung == translate('RTOC', 'Create shortcut'):
                self.addShortcut(bot, chat_id, strung)
            else:
                if strung == translate('RTOC', 'Remove shortcut'):
                    ...
                    if strung == translate('RTOC', 'No parameters'):
                        ...
                        try:
                            ...
                            logging.warning(chat_id, text=('_' + strung + ' ist not a valid Format._'))
                        ...
                        if self.current_call[chat_id].endswith(')'):
                            ...
                            self._pluginCall_chat_id = chat_id
                            self.logger.getPluginParameter(self.current_plugin[chat_id], self.current_call[chat_id], self.temp)
                            self._pluginCall_chat_id = None
                            if self.telegram_clients[str(chat_id)]['menu'] == 'callShortcut':
                                self.menuHandler(bot, chat_id)
                            else:
        ...
        if chat_id not in self.signals_selected.keys():
            ...
            for signalname in availableSignals:
                ...
                if sname in self.signals_selected[chat_id]:
                    ...
                    if not self.get_telegram_client(chat_id, 'signalSubmenu', True):
                        commands.append(sname)
                    elif signalname[0] not in commands:
    ...
    def signalsHandlerAns(self, bot, chat_id, strung):
        ...
        if chat_id not in self.signals_selected.keys():
            ...
            if strung == self.BACKBUTTON:
                ...
                multiSelectorList = []
                for unit in units:
                    multiSelectorList.append(translate('RTOC', 'Select all with unit "{}"').format(unit))
                ...
                if strung == translate('RTOC', 'Request graph'):
                    ...
                    if dontplot_signals != []:
                        self.send_message(chat_id=chat_id, text=(translate('RTOC', '{} can not be plotted.').format(','.join(dontplot_signals))))
                    if plot_signals != []:
                        range = list(self.signals_range[chat_id])
                        self.send_message(chat_id=chat_id, text=(translate('RTOC', "I'm going to create a graph with {} signals. This may take a while.").format(len(plot_signals))))
                        (self.sendSignalPlot)(bot, chat_id, plot_signals, *range)
                    else:
                        self.send_message(chat_id=chat_id, text=(translate('RTOC', 'No signals selected.')))
                else:
                    if strung == translate('RTOC', 'Show events'):
                        if 'Events' not in self.signals_selected[chat_id]:
                            self.signals_selected[chat_id].append('Events')
                        self.send_message(chat_id=chat_id, text=(translate('RTOC', 'Events will be plotted.')))
                    else:
                        if strung == translate('RTOC', 'Hide events'):
                            if 'Events' in self.signals_selected[chat_id]:
                                idx = self.signals_selected[chat_id].index('Events')
                                self.signals_selected[chat_id].pop(idx)
                                self.send_message(chat_id=chat_id, text=(translate('RTOC', 'Events will not be plotted.')))
                            else:
                                self.send_message(chat_id=chat_id, text=(translate('RTOC', "You can't hide events, they are already hidden!")))
                        else:
                            if strung.startswith('x '):
                                strung = strung.replace('x ', '')
                                if strung in self.signals_selected[chat_id]:
                                    idx = self.signals_selected[chat_id].index(strung)
                                    self.signals_selected[chat_id].pop(idx)
                                    self.send_message(chat_id=chat_id, text=(translate('RTOC', 'Signal removed from selection.')))
                            else:
                                if self.get_telegram_client(chat_id, 'signalSubmenu', True):
                                    if strung in self.logger.database.deviceNames():
                                        self.signalsDeviceSubHandler(bot, chat_id, strung)
                                        return
                                if strung == translate('RTOC', 'Delete selected signals!'):
                                    ...
                                    self.signals_selected[chat_id] = []
                                    self.send_message(chat_id=chat_id, text=(translate('RTOC', 'Selected signals have been deleted!')))
                                else:
                                    if strung == translate('RTOC', 'Delete selected events!'):
                                        xmin, xmax = self.signals_range[chat_id]
                                        for sigName in self.signals_selected[chat_id]:
                                            if sigName != 'Events':
                                                sigID = (self.logger.database.getSignalID)(*self.str2signal(sigName))
                                        ...
                                        self.send_message(chat_id=chat_id, text=(translate('RTOC', 'Events of selected signals have been deleted!')))
                                    else:
                                        if strung == translate('RTOC', 'Select period'):
                                            self.signalsSelectRangeHandler(bot, chat_id)
                                            return
                                        if strung == translate('RTOC', 'Select all'):
                                            self.signals_selected[chat_id] == []
                                            self.signals_selected[chat_id] = ['.'.join(s) for s in self.logger.database.signalNames()]
                                        else:
                                            if strung == translate('RTOC', 'Deselect all'):
                                                self.signals_selected[chat_id] = []
                                            else:
                                                if strung in multiSelectorList:
                                                    for unit in units:
                                                        if '"' + str(unit) + '"' in strung:
                                                            self.signals_selected[chat_id] == []
                                                            self.signals_selected[chat_id] = ['.'.join(s) for s in self.logger.database.signalNames(units=[unit])]

                                                    self.signalsHandler(bot, chat_id, True)
                                                    return
                                                if strung == translate('RTOC', 'Download selected signals!'):
                                                    bot.send_chat_action(chat_id=chat_id, action=(ChatAction.UPLOAD_DOCUMENT))
                                                    self.send_message(chat_id=chat_id, text=(translate('RTOC', "I'm collecting the data now...")))
                                                    sigIDs = [(self.logger.database.getSignalID)(*self.str2signal(i)) for i in self.signals_selected[chat_id]]
                                                    xmin, xmax = self.signals_range[chat_id]
                                                    dir = self.logger.config['global']['documentfolder']
                                                    self.logger.database.signalsToCSV(sigIDs,
                                                      (dir + '/telegram_export'), xmin, xmax, database=True)
                                                    bot.send_document(chat_id=chat_id, document=(open(dir + '/telegram_export.csv', 'rb')))
                                                    bot.send_document(chat_id=chat_id, document=(open(dir + '/telegram_export.txt', 'rb')))
                                                else:
                                                    if strung == translate('RTOC', 'View: Devices'):
                                                        self.telegram_clients[str(chat_id)]['signalSubmenu'] = False
                                                        self.saveClients()
                                                    else:
                                                        if strung == translate('RTOC', 'View: Signals'):
    ...
    def signalsDeviceSubHandlerAns(self, bot, chat_id, strung):
        if strung == self.BACKBUTTON:
            ...
            if strung == translate('RTOC', 'All'):
                ...
                self.send_message(chat_id=chat_id, text=(translate('RTOC', 'All signals of {} selected.').format(dev)))
            else:
                self.selectSignal(bot, chat_id, strung)
        ...
        if len(a) == 2:
            if strung not in self.signals_selected[chat_id]:
                ...
                if evs == []:
                    ...
                    self.send_message(chat_id, translate('RTOC', 'Signal selected:\n{}\nEvents: {}').format(t, evtext), ParseMode.MARKDOWN, True, False)
            return True
        else:
            return False
    ...
    def signalsSelectRangeHandlerAns(self, bot, chat_id, strung):
        if strung == self.BACKBUTTON:
            ...
            if strung == translate('RTOC', 'Last minute'):
                ...
                if strung == translate('RTOC', 'Last 10 minutes'):
                    xmax = time.time()
                    xmin = xmax - 600
                else:
                    if strung == translate('RTOC', 'Last hour'):
                        xmax = time.time()
                        xmin = xmax - 3600
                    else:
                        if strung == translate('RTOC', 'Last 24h'):
                            xmax = time.time()
                            xmin = xmax - 86400
                        else:
                            if strung == translate('RTOC', 'Last week'):
                                xmax = time.time()
                                xmin = xmax - 604800
                            else:
                                if strung == translate('RTOC', 'Last month'):
                                    xmax = time.time()
                                    xmin = xmax - 2678400
                                else:
                                    if strung == translate('RTOC', 'All'):
                ...
                if not found:
                    self.send_message(chat_id=chat_id, text=(translate('RTOC', 'Please send me a period I can understand.')))
                    return
                self.signals_range[chat_id] = [
                 xmin, xmax]
                xmin = dt.datetime.fromtimestamp(xmin).strftime('%d.%m.%Y %H:%M:%S')
            ...
            if len(strung.split('.')) == 2:
                ...
                if strung == self.BACKBUTTON:
                    ...
                    if self.selectedSignalForEvent is not None:
                        ...
                        if isValue:
                            self.logger.database.addDataCallback(y=value,
                              unit=[unit],
                              dname=device,
                              snames=[signal])
                            ...
                            if strung.startswith('*'):
                                if strung.endswith('*'):
                                    prio = 2
                                    strung = strung.replace('*', '')
                            elif strung.startswith('_'):
                                if strung.endswith('_'):
                                    prio = 1
    ...
    def settingsHandler(self, bot, chat_id):
        ...
        if self.telegram_clients[str(chat_id)]['permission'] in ('write', 'admin'):
            ...
            total = str(round(obj_Disk.total / 1073741824.0, 3))
            used = str(round(obj_Disk.used / 1073741824.0, 3))
            free = str(round(obj_Disk.free / 1073741824.0, 3))
    ...
    def settingsHandlerAns(self, bot, chat_id, strung):
        if strung == self.BACKBUTTON:
            ...
            if strung == translate('RTOC', 'Set notification level'):
                self.adjustEventNotificationHandler(bot, chat_id)
            else:
                if strung == translate('RTOC', 'General'):
                    if self.telegram_clients[str(chat_id)]['permission'] in ('write',
                                                                             'admin'):
                        self.settingsGeneralHandler(bot, chat_id)
                        return
                    if strung == translate('RTOC', 'Telegram-Bot'):
                        if self.telegram_clients[str(chat_id)]['permission'] == 'admin':
                            self.settingsBotHandler(bot, chat_id)
                            return
                else:
                    if strung == translate('RTOC', 'Backup'):
                        if self.telegram_clients[str(chat_id)]['permission'] == 'admin':
        ...
        if strung == self.BACKBUTTON:
            ...
            for id in ids:
                ...
                if self.telegram_clients[id]['permission'] == 'admin':
                    ...
                    if self.telegram_clients[id]['permission'] == 'custom':
                        a = translate('RTOC', 'Custom')
                    else:
                        if self.telegram_clients[id]['permission'] == 'read':
                            a = translate('RTOC', 'Read')
                        else:
                            if self.telegram_clients[id]['permission'] == 'write':
            ...
            self.sendMenuMessage(bot, chat_id, commands, text)
            return
        elif strung == translate('RTOC', 'Custom-Preview'):
            if self.telegram_clients[str(chat_id)]['permission'] == 'admin':
            ...
            elif strung == translate('RTOC', 'Write-Preview'):
                if self.telegram_clients[str(chat_id)]['permission'] == 'admin':
                    self.telegram_clients[str(chat_id)]['permission'] = 'write'
                    self.telegram_clients[str(chat_id)]['preview'] = True
                    self.telegram_clients[str(chat_id)]['menu'] = 'menu'
                    self.logger.config['telegram']['onlyAdmin'] = False
                elif strung == translate('RTOC', 'Read-Preview'):
                    if self.telegram_clients[str(chat_id)]['permission'] == 'admin':
                    ...
                    if self.helper[chat_id] == 'users':
                        ...
                        return
                    if strung == translate('RTOC', 'OnlyAdmin: On'):
                        self.logger.config['telegram']['onlyAdmin'] = False
                        self.logger.save_config()
                        self.sendStartStopMessage(chat_id, 'OnlyAdmin-Mode', False, True)
                        self.settingsBotHandler(bot, chat_id)
                        return
                if strung == translate('RTOC', 'OnlyAdmin: Off'):
            ...
            if strung == translate('RTOC', 'Telegram-Bot: Off'):
    ...
    def settingsBackupHandlerAns(self, bot, chat_id, strung):
        if strung == self.BACKBUTTON and self.helper[chat_id] != None:
            ...
            if strung == self.BACKBUTTON:
                ...
                if strung == translate('RTOC', '**Delete all data**'):
                    ...
                    if strung == translate('RTOC', 'Delete all data'):
                        ...
                        if strung == translate('RTOC', 'Create backup now'):
                            self.helper[chat_id] = None
                            self.logger.database.pushToDatabase()
                            self.send_message(chat_id, translate('RTOC', 'Backup was created.'))
                            return
                        else:
                            if strung == translate('RTOC', 'Auto-resampling: {} Hz').format(self.logger.config['backup']['resample']) or strung == translate('RTOC', 'Auto-resampling: Off'):
                                self.backupResamplingHandler(bot, chat_id)
                                return
                            else:
                                if strung == translate('RTOC', 'Backup: On'):
                                    ...
                                    if strung == translate('RTOC', 'Backup: Off'):
                                        ...
                                        if strung == translate('RTOC', 'Download database'):
                                            ...
                                            if strung == translate('RTOC', 'Resample database'):
                                                ...
                                                return
                                            if self.helper[chat_id] == 'resample':
                                                try:
                                                    samplerate = float(strung)
                                                except:
                            ...
                            if strung == translate('RTOC', 'Load backup on start: On'):
                                ...
                                self.settingsBackupHandler(bot, chat_id)
                                return
                        if strung == translate('RTOC', 'Load backup on start: Off'):
                            ...
                            self.settingsBackupHandler(bot, chat_id)
                            return
                    if strung == translate('RTOC', 'Backup-Interval: {}s').format(self.logger.config['backup']['intervall']):
                        liset = [translate('RTOC', 'Off'),
                        ...
                        return
                if self.helper[chat_id] == 'Backup-Interval':
                    if strung == translate('RTOC', 'Off'):
                        seconds = 0
                    else:
                        if strung == translate('RTOC', '1x per minute'):
                            seconds = 60
                        else:
                            if strung == translate('RTOC', '2x per hour'):
                                seconds = 1800
                            else:
                                if strung == translate('RTOC', '1x per hour'):
                                    seconds = 3600
                                else:
                                    if strung == translate('RTOC', '2x per day'):
                                        seconds = 43200
                                    else:
                                        if strung == translate('RTOC', '1x per day'):
                                            seconds = 86400
                                        else:
                                            if strung == translate('RTOC', '2x per week'):
                                                seconds = 302400.0
                                            else:
                                                if strung == translate('RTOC', '1x per week'):
                                                    seconds = 604800
                                                else:
                                                    try:
    ...
    def backupResamplingHandlerAns(self, bot, chat_id, strung):
        if strung == self.BACKBUTTON:
            ...
            if strung == translate('RTOC', 'Off'):
                ...
                self.logger.save_config()
                self.send_message(chat_id, translate('RTOC', 'Resampling has been deactivated'))
                self.settingsBackupHandler(bot, chat_id)
        try:
            samplerate = float(strung)
            self.logger.config['backup']['resample'] = samplerate
            self.logger.save_config()
            self.send_message(chat_id, translate('RTOC', 'Resampling has been changed'))
            self.settingsBackupHandler(bot, chat_id)
        except:
            print(traceback.format_exc())
            self.send_message(chat_id, translate('RTOC', 'I did not understand your message'))
    ...
    def globalSamplerateHandlerAns(self, bot, chat_id, strung):
        if strung == self.BACKBUTTON:
            ...
            try:
                ...
                self.send_message(chat_id, translate('RTOC', 'Samplerate has been changed'))
                self.settingsGeneralHandler(bot, chat_id)
            except:
                print(traceback.format_exc())
                self.send_message(chat_id, translate('RTOC', 'I did not understand your message'))
    ...
    def resizeHandlerAns(self, bot, chat_id, strung):
        if strung == self.BACKBUTTON:
            ...
            if value:
                ...
                self.send_message(chat_id=chat_id, text=(translate('RTOC', 'Invalid input')))
                self.settingsGeneralHandler(bot, chat_id)

    def sendSignalPlot(self, bot, chat_id, signalnames, xmin_plot, xmax_plot):
        if plt is None:
            self.send_message(chat_id=chat_id, text=(translate('RTOC', '**I couldn\'t create a plot.**\nYou need to install "matplotlib" and "tkinker" on my server.**')))
            return
        else:
            if type(signalnames) == str:
                ...
                for idx, signalname in enumerate(signalnames):
                    ...
                    signal = self.logger.database.getSignal_byName((a[0]),
                      (a[1]), xmin=xmin_plot, xmax=xmax_plot, database=True, maxN=1000)
                    if signal is not None:
                        if len(signal[2]) > 0:
                            if min(signal[2]) < xmin:
                                xmin = min(signal[2])
                            elif max(signal[2]) > xmax:
                                xmax = max(signal[2])
                            else:
                            ...
                            try:
                                ...
                                if abs(signal[2][0] - signal[2][(-1)]) > 300:
                                    xfmt = mdates.DateFormatter(translate('RTOC', '%d.%m %H:%M'))
                                else:
                                    xfmt = mdates.DateFormatter(translate('RTOC', '%d.%m %H:%M:%S'))
                                plt.plot(dates, (list(signal[3])), label=(signalname + '[' + signal[4] + ']'))
                            except Exception as error:
                                print(error)
                                print(traceback.format_exc())
                                logging.error(traceback.format_exc())
                    ...
                    self.send_message(chat_id=chat_id, text=(translate('RTOC', 'Plot completed at {}%.').format(round(proc))))
            ...
            if plotEvents:
                try:
                    for evID in self.logger.database.events().keys():
                        ...
                        if event[4] > xmin_plot:
                            if event[4] < xmax_plot:
                                prio = event[6]
                                text = event[3]
            ...
            if filetype in ('jpg', 'png', 'bmp', 'jpeg'):
                bot.send_photo(chat_id=chat_id, photo=(open(dir + '/telegram_export.' + filetype, 'rb')),
                  caption=t,
                  timeout=1000)
            else:
                bot.send_document(chat_id=chat_id, document=(open(dir + '/telegram_export.' + filetype, 'rb')),
                  timeout=1000)
    ...
    def createPlotToolTip(self, xmin, xmax, sigLen):
        ...
        try:
            if self.logger.config['postgresql']['active']:
                ...
                line1 = str(dt.timedelta(seconds=duration)) + '/ ~ ' + str(dt.timedelta(seconds=maxduration))
    ...
    def createSignalInfoStr(self, x, y):
        ...
        try:
            if self.logger.config['postgresql']['active']:
                ...
                line1 = str(dt.timedelta(seconds=duration)) + '/ ~ ' + str(dt.timedelta(seconds=maxduration))
                line2 = str(len(list(x))) + '/' + str(self.logger.config['global']['recordLength'])
            count = 20
            if len(x) <= count:
                count = len(x)
            if count > 1:
    ...
    def removeShortcut(self, bot, chat_id, strung):
        if self.current_call[chat_id] is not None:
            ...
            if strung in self.telegram_clients[str(chat_id)]['shortcuts'][1]:
                idx = self.telegram_clients[str(chat_id)]['shortcuts'][1].index(strung)
                self.telegram_clients[str(chat_id)]['shortcuts'][1].pop(idx)
                self.telegram_clients[str(chat_id)]['shortcuts'][0].pop(idx)
                self.current_call[chat_id] = None
                self.menuHandler(bot, chat_id)
                self.saveClients()
            else:
    ...
    def callShortcut(self, bot, chat_id, strung):
        if strung in self.telegram_clients[str(chat_id)]['shortcuts'][0]:
            idx = self.telegram_clients[str(chat_id)]['shortcuts'][0].index(strung)
            call = self.telegram_clients[str(chat_id)]['shortcuts'][1][idx].split('.')
    ...
    def globalEventsHandlerAns(self, bot, chat_id, strung):
        if strung == self.BACKBUTTON:
            ...
            if strung in self.logger.printGlobalEvents():
                self.signals_selected[chat_id] = strung
                self.globalEventHandler(bot, chat_id)
                return
            if strung == translate('RTOC', 'Create new global event'):
    ...
    def globalEventHandler(self, bot, chat_id):
        ...
        if event['priority'] == 0:
            ...
            if event['trigger'] == 'rising':
                ...
                if event['trigger'] == 'falling':
                    commands += [translate('RTOC', 'Trigger: Falling')]
                else:
                    if event['trigger'] == 'both':
    ...
    def globalEventHandlerAns(self, bot, chat_id, strung):
        if type(self.signals_selected[chat_id]) != str:
            ...
            if strung == self.BACKBUTTON:
                ...
                self.globalEventHandler(bot, chat_id)
                return
            if strung == translate('RTOC', 'Active'):
                self.logger.globalEvents[name[0]]['active'] = False
                self.logger.saveGlobalEvents()
            else:
                if strung == translate('RTOC', 'Inactive'):
                    self.logger.globalEvents[name[0]]['active'] = True
                    self.logger.saveGlobalEvents()
                else:
                    if strung == translate('RTOC', 'Edit condition'):
                        ...
                        return
                    if strung == translate('RTOC', 'Set assigned signal'):
                                            ...
                                            if strung == translate('RTOC', 'Priority: ') + translate('RTOC', 'Warning'):
                                                ...
                                                if strung == translate('RTOC', 'Priority: ') + translate('RTOC', 'Error'):
                                                    ...
                                                    if strung == translate('RTOC', 'Test event'):
                                                        ...
                                                        if self.helper[chat_id] == 'editText':
                                                            self.helper[chat_id] = None
                                                            self.logger.globalEvents[name[0]]['text'] = strung
                                                            self.logger.saveGlobalEvents()
                                                        else:
                                                            if self.helper[chat_id] == 'editEventID':
                                                                self.helper[chat_id] = None
                                                                self.logger.globalEvents[name[0]]['id'] = strung
                                                                self.logger.saveGlobalEvents()
                                                            else:
                                                                if self.helper[chat_id] == 'changeCondition':
                                                                    self.helper[chat_id] = None
    ...
    def globalActionsHandlerAns(self, bot, chat_id, strung):
        if strung == self.BACKBUTTON:
            ...
            if strung in self.logger.printGlobalActions():
                self.signals_selected[chat_id] = strung
                self.globalActionHandler(bot, chat_id)
                return
            if strung == translate('RTOC', 'Create new global action'):
        ...
        if type(self.signals_selected[chat_id]) != str:
            ...
            if strung == translate('RTOC', 'Active'):
                ...
                if strung == translate('RTOC', 'Inactive'):
                    ...
                    if strung == translate('RTOC', 'Edit code'):
                        ...
                        if strung == translate('RTOC', 'Select eventID'):
                            ...
                            self.sendMenuMessage(bot, chat_id, commands, text)
                            return
                        if strung == translate('RTOC', 'Delete action'):
                            self.helper[chat_id] = None
                            self.logger.removeGlobalAction(name[0])
                            self.logger.saveGlobalActions()
                            self.globalActionsHandler(bot, chat_id)
                            return
                        if strung == translate('RTOC', 'Test action'):
                            self.helper[chat_id] = None
                            ...
                            self.send_message(chat_id, text)
                            return
                    if self.helper[chat_id] == 'selectIDs':
                        self.helper[chat_id] = None
            ...
            if len(ans) == 2:
                try:
                    ...
                    if ans[0] == 'picture':
                        ...
                        if ans[0] == 'document':
                            bot.send_document(chat_id=chat_id, document=(open(ans[1], 'rb')))
                        else:
                            if ans[0] == 'text':
