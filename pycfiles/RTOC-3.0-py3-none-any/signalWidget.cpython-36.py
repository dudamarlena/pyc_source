...
# [GCC 8.4.0]
# Embedded file name: D:\Benutzer\haschtl\Dokumente\GIT\kellerlogger\RTOC\RTOC_GUI\signalWidget.py
# Compiled at: 2019-09-05 20:30:36
# Size of source mod 2**32: 27859 bytes
from PyQt5 import QtCore
from PyQt5 import QtGui
from PyQt5 import QtWidgets
import pyqtgraph as pg, random, time, os, sys
...
class SignalWidget(QtWidgets.QWidget):
    ...
    def __init__(self, plotWidget, logger, devicename, signalname, id, unit, remotehost=None):
        ...
        self.initLabel()
        if self.signalname in lib.column(self.logger.database.signalNames(), 1):
            if False:
        ...
        self.display_text = pg.TextItem(text='', color=(200, 200, 200),
          fill=(200, 200, 200, 50),
          anchor=(1, 1))
        ...
        self.plot.curve.sigClicked.connect(partial(self.plotWidget.signalClickedAction, self.devicename, self.signalname))
        setPlotStyle(self.plot, symbol, brush)
        self.labelItem = pg.TextItem((devicename + '.' + signalname), color=(symbol['color']),
          html=None,
          anchor=(0, 0.5))
        self.labelItem.setPos(0, 0)
        self.initMenu()
        self.editWidget.xTimeBaseButton.setChecked(self.xTimeBase)
        self.editWidget.labelButton.setChecked(self.plotWidget.plotViewWidget.labelButton.isChecked())
        if self.logger.config['GUI']['plotLabelsEnabled']:
            self.labelItem.hide()
        self.editWidget.toggleLabel()
        self.signalModifications = [
         0, 0, 1, 1]
        self.updateLegend()
    ...
    def setLegendColor(self, color, bgcolor=None):
        if bgcolor is None:
            self.legend.setStyleSheet('QLabel { background-color : ' + str(color) + '; color : ' + str(color) + '; }')
        else:
            self.legend.setStyleSheet('QLabel { background-color : ' + str(bgcolor) + '; color : ' + str(color) + '; }')
    ...
    def updateLabel(self, x, y, current_unit):
        if len(y) > 0:
            ...
            if delayed >= self.signalTimeOut:
                if self.label.styleSheet() != 'background-color: rgb(113, 100, 29)':
    ...
    def updateEvents(self, offsetX, offsetY, scaleX, scaleY, ctime, current_time):
        ...
        for idx0, e in enumerate(self.plotWidget.self.eventWidget.events):
            event = [
             e[1], e[2], e[0], e[5], e[6], e[3], e[4], e[7]]
            oldEvent = False
            if event[6] == self.signalname and event[5] == self.devicename:
                ...
                if not oldEvent:
                    eventItem = pg.TextItem((str(event[1])), color=(200, 200, 200),
                      html=None,
                      anchor=(0, 0.5))
                    ...
                    self.plotWidget.plot.addItem((self.eventItems[idx].vLine),
                      ignoreBounds=True)
                    if self.showEvents:
                        if self.logger.config['GUI']['showEvents']:
