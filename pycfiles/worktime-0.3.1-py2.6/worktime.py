...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/worktime/worktime.py
# Compiled at: 2010-12-31 11:44:12
"""
NAME
...
def worktime(start, end, days, output):
    ...
    for index in range(day_diff.days):
        ...
        pages += contents % {'title': title}

    print >> output, framework % {'title': title, 
       'css': css, 
       'css_print': css_print, 
       'contents': pages, 
       'footer': footer}
...
def main(argv=None):
    ...
    days = range(7)
    try:
        (opts, args) = getopt(argv[1:], 'sedwWhv', [
         'start=',
         'end=',
         'days=',
         'weekdays',
        ...
        for (option, value) in opts:
            if option in ('-s', '--start'):
                ...
                values.extend([1, 1][0:3 - len(values)])
