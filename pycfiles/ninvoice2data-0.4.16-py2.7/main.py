...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/ninvoice2data/main.py
...
def extract_data(invoicefile, templates=None, input_module=pdftotext):
    ...
    Notes
    -----
    Import required `input_module` when using ninvoice2data as a library
    ...
    Examples
    --------
    When using `ninvoice2data` as an library

    >>> from ninvoice2data.input import pdftotext
    >>> extract_data("ninvoice2data/test/pdfs/oyo.pdf", None, pdftotext)
...
def create_parser():
    ...
    parser.add_argument('--output-format', choices=output_mapping.keys(), default='none', help='Choose output format. Default: none')
    parser.add_argument('--output-name', '-o', dest='output_name', default='invoices-output', help='Custom name for output file. Extension is added based on chosen format.')
    parser.add_argument('--debug', dest='debug', action='store_true', help='Enable debug information.')
    parser.add_argument('--copy', '-c', dest='copy', help='Copy and rename processed PDFs to specified folder.')
    parser.add_argument('--move', '-m', dest='move', help='Move and rename processed PDFs to specified folder.')
    parser.add_argument('--filename-format', dest='filename', default='{date} {invoice_number} {desc}.pdf', help='Filename format to use when moving or copying processed PDFs.Default: "{date} {invoice_number} {desc}.pdf"')
    parser.add_argument('--template-folder', '-t', dest='template_folder', help='Folder containing invoice templates in yml file. Always adds built-in templates.')
    parser.add_argument('--exclude-built-in-templates', dest='exclude_built_in_templates', default=False, help='Ignore built-in templates.', action='store_true')
    parser.add_argument('input_files', type=argparse.FileType('r'), nargs='+', help='File or directory to analyze.')
    return parser
...
def main(args=None):
    ...
    for f in args.input_files:
        ...
        if res:
            ...
            output.append(res)
            if args.copy:
                filename = args.filename.format(date=res['date'].strftime('%Y-%m-%d'), invoice_number=res['invoice_number'], desc=res['desc'])
