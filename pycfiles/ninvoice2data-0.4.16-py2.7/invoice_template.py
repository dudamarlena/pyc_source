...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/ninvoice2data/extract/invoice_template.py
# Compiled at: 2019-02-05 09:05:41
"""
...
class InvoiceTemplate(OrderedDict):
    ...
    def parse_date(self, value):
        """Parses date and returns date after parsing"""
        res = dateparser.parse(value, date_formats=self.options['date_formats'], languages=self.options['languages'])
        logger.debug('result of date parsing=%s', res)
        if res == None:
            logger.debug('result of date parsing override=%s', res)
            res = value
        return res
