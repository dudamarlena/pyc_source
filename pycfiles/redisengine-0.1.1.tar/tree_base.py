...
# [GCC 8.4.0]
# Embedded file name: /Users/joekavalieri/git/redisengine/redisengine/core/tree_base.py
# Compiled at: 2016-03-25 15:37:24
import logging
...
class BaseTreeCore(object):
    ...
    def _complex_field_names(self):
        return [ field_name for field_name, field in self._fields.items() if field.is_complex
               ]

    @cached_property
    def _hash_field_names(self):
        return [ field_name for field_name, field in self._fields.items() if not field.is_complex
               ]

    @cached_property
    def _complex_fields(self):
        return {field_name:field for field_name, field in self._fields.items() if field.is_complex if field.is_complex}

    @cached_property
    def _hash_fields(self):
        return {field_name:field for field_name, field in self._fields.items() if not field.is_complex if not field.is_complex}
    ...
    def _delete(self):
        if not self._created and self._conn.delete(self.pk):
            ...
            if assoc_fields:
                dbg_msg += (', together with the following associated fields: {}').format(assoc_fields)
    ...
    def validate(self, fields=None, clean=True):
        ...
        if fields is None:
            fields = [ (self._fields.get(name), getattr(self, name)) for name in self._fields_ordered ]
