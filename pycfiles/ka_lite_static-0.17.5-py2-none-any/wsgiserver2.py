...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/cherrypy/cherrypy/wsgiserver/wsgiserver2.py
...
class HTTPRequest(object):
    ...
    def read_request_line(self):
        ...
        self.qs = qs
        sp = (
         int(self.server.protocol[5]), int(self.server.protocol[7]))
        if sp[0] != rp[0]:
    ...
    def simple_response(self, status, msg=''):
        ...
        status = str(status)
        buf = [
         self.server.protocol + SPACE + status + CRLF,
         'Content-Length: %s\r\n' % len(msg),
    ...
    def write(self, chunk):
        ...
        if self.chunked_write and chunk:
            buf = [
             hex(len(chunk))[2:], CRLF, chunk, CRLF]
    ...
    def send_headers(self):
        ...
        if status == 413:
            ...
            if 'content-length' not in hkeys:
...
class WorkerThread(threading.Thread):
    ...
    def __init__(self, server):
        ...
        self.work_time = 0
        self.stats = {'Requests': lambda s: self.requests_seen + (self.start_time is None and trueyzero or self.conn.requests_seen), 
           ...
           'Read Throughput': lambda s: s['Bytes Read'](s) / (s['Work Time'](s) or 1e-06), 
           'Write Throughput': lambda s: s['Bytes Written'](s) / (s['Work Time'](s) or 1e-06)}
...
class HTTPServer(object):
    ...
    def clear_stats(self):
        ...
        self.stats = {'Enabled': False, 
           ...
           'Socket Errors': 0, 
           'Requests': lambda s: not s['Enabled'] and -1 or sum([ w['Requests'](w) for w in s['Worker Threads'].values() ], 0), 
           'Bytes Read': lambda s: not s['Enabled'] and -1 or sum([ w['Bytes Read'](w) for w in s['Worker Threads'].values() ], 0), 
           'Bytes Written': lambda s: not s['Enabled'] and -1 or sum([ w['Bytes Written'](w) for w in s['Worker Threads'].values() ], 0), 
           'Work Time': lambda s: not s['Enabled'] and -1 or sum([ w['Work Time'](w) for w in s['Worker Threads'].values() ], 0), 
           'Read Throughput': lambda s: not s['Enabled'] and -1 or sum([ w['Bytes Read'](w) / (w['Work Time'](w) or 1e-06) for w in s['Worker Threads'].values()
                               ], 0), 
           'Write Throughput': lambda s: not s['Enabled'] and -1 or sum([ w['Bytes Written'](w) / (w['Work Time'](w) or 1e-06) for w in s['Worker Threads'].values()
                               ], 0), 
           'Worker Threads': {}}
    ...
    def bind(self, family, type, proto=0):
        ...
        if hasattr(socket, 'AF_INET6') and family == socket.AF_INET6 and self.bind_addr[0] in ('::',
                                                                                               '::0',
                                                                                               '::0.0.0.0'):
            try:
...
class WSGIGateway(Gateway):
    ...
    def write(self, chunk):
        ...
        """
        assert self.started_response, 'WSGI write called before start_response.'
        chunklen = len(chunk)
