...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/python-dateutil/dateutil/rrule.py
...
M366MASK = tuple([
 1] * 31 + [2] * 29 + [3] * 31 + [4] * 30 + [5] * 31 + [6] * 30 + [7] * 31 + [8] * 31 + [9] * 30 + [10] * 31 + [11] * 30 + [12] * 31 + [1] * 7)
...
class rrule(rrulebase):
    ...
    def _iter(self):
        ...
        getdayset = {YEARLY: ii.ydayset, MONTHLY: ii.mdayset, 
           WEEKLY: ii.wdayset, 
           DAILY: ii.ddayset, 
           HOURLY: ii.ddayset, 
           MINUTELY: ii.ddayset, 
        ...
        if freq < HOURLY:
            ...
            gettimeset = {HOURLY: ii.htimeset, MINUTELY: ii.mtimeset, 
...
class _iterinfo(object):
    ...
    def stimeset(self, hour, minute, second):
        return (
         datetime.time(hour, minute, second, tzinfo=self.rrule._tzinfo),)
...
class _rrulestr(object):
    _freq_map = {'YEARLY': YEARLY, 'MONTHLY': MONTHLY, 
       'WEEKLY': WEEKLY, 
       'DAILY': DAILY, 
       'HOURLY': HOURLY, 
       'MINUTELY': MINUTELY, 
       'SECONDLY': SECONDLY}
    _weekday_map = {'MO': 0, 'TU': 1, 'WE': 2, 'TH': 3, 'FR': 4, 
       'SA': 5, 'SU': 6}
    ...
    def _parse_rfc(self, s, dtstart=None, cache=False, unfold=False, forceset=False, compatible=False, ignoretz=False, tzinfos=None):
        global parser
        if compatible:
            forceset = True
            unfold = True
        ...
        if not forceset and len(lines) == 1 and (s.find(':') == -1 or s.startswith('RRULE:')):
            ...
            if forceset or len(rrulevals) > 1 or rdatevals or exrulevals or exdatevals:
                ...
                for value in exdatevals:
                    for datestr in value.split(','):
                        rset.exdate(parser.parse(datestr, ignoretz=ignoretz, tzinfos=tzinfos))
