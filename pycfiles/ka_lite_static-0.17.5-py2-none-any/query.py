...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/django/django/db/models/sql/query.py
...
from django.db.models.sql.constants import QUERY_TERMS, ORDER_DIR, SINGLE, ORDER_PATTERN, JoinInfo
...
class Query(object):
    ...
    def combine(self, rhs, connector):
        ...
        if not self.model == rhs.model:
            ...
            if not self.distinct_fields == rhs.distinct_fields:
                raise AssertionError('Cannot combine queries with different distinct fields.')
                self.remove_inherited_models()
                change_map = {}
                used = set()
                ...
                if rhs.where:
                    ...
                    self.where or self.where.add(EverythingNode(), AND)
            else:
                if self.where:
                    w = self.where_class()
    ...
    def add_filter(self, filter_expr, connector=AND, negate=False, trim=False, can_reuse=None, process_extras=True, force_having=False):
        ...
        if value is None:
            ...
            if callable(value):
                ...
                if connections[DEFAULT_DB_ALIAS].features.interprets_empty_strings_as_nulls and lookup_type == 'exact' and value == '':
                    value = True
                    lookup_type = 'isnull'
        ...
        if having_clause or force_having:
            if (
             alias, col) not in self.group_by:
                ...
                if len(join_list) > 1:
                    for j_alias in join_list:
                        if self.alias_map[j_alias].join_type == self.LOUTER:
                            ...
                            entry = self.where_class()
                            entry.add((
                             Constraint(j_alias, j_col, None), 'isnull', True), AND)
                            entry.negate()
    ...
    def setup_joins(self, names, opts, alias, dupe_multis, allow_many=True, allow_explicit_fk=False, can_reuse=None, negate=False, process_extras=True):
            ...
            if direct:
                if m2m:
                    ...
                    if int_alias == table2 and from_col2 == to_col2:
                        ...
                        alias = self.join((
                         int_alias, table2, from_col2, to_col2), dupe_multis, exclusions, nullable=True, reuse=can_reuse)
                ...
                if m2m:
                    ...
                    if cached_data:
                        ...
                        if orig_field.model is local_field.model:
                            ...
                            target = opts.pk
                        orig_opts._join_cache[name] = (
                         table, from_col, to_col,
                         opts, target)
        ...
        return (
         field, target, opts, joins, last, extra_filters)
    ...
    def _extra_select(self):
        if self._extra_select_cache is not None:
            ...
            if self.extra_select_mask is not None:
                self._extra_select_cache = SortedDict([ (k, v) for k, v in self.extra.items() if k in self.extra_select_mask
                                                      ])
                return self._extra_select_cache
            else:
                return self.extra
