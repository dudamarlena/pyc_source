...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/django/django/core/management/templates.py
# Compiled at: 2018-07-11 18:15:30
import cgi, errno, mimetypes, os, posixpath, re, shutil, stat, sys, tempfile
try:
...
class TemplateCommand(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
     make_option('--template', action='store', dest='template', help='The dotted import path to load the template from.'),
     make_option('--extension', '-e', dest='extensions', action='append', default=['py'], help='The file extension(s) to render (default: "py"). Separate multiple extensions with commas, or use -e multiple times.'),
     make_option('--name', '-n', dest='files', action='append', default=[], help='The file name(s) to render. Separate multiple extensions with commas, or use -n multiple times.'))
    ...
    def handle(self, app_or_project, name, target=None, **options):
        ...
        if not re.search('^[_a-zA-Z]\\w*$', name):
            if not re.search('^[_a-zA-Z]', name):
        ...
        if target is None:
            ...
            context = Context(dict(options, **{base_name: name, 
               base_directory: top_dir}), autoescape=False)
