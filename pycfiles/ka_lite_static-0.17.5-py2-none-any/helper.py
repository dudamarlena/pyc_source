...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/cherrypy/cherrypy/test/helper.py
...
import re, sys, time, warnings, cherrypy
...
def get_tst_config(overconf={}):
    ...
    if _testconfig is None:
        conf = {'scheme': 'http', 'protocol': 'HTTP/1.1', 
           'port': 54583, 
           'host': '127.0.0.1', 
           'validate': False, 
           'conquer': False, 
...
class CPWebCase(webtest.WebCase):
    script_name = ''
    scheme = 'http'
    available_servers = {'wsgi': LocalWSGISupervisor, 'wsgi_u': get_wsgi_u_supervisor, 
    ...
    def _setup_server(cls, supervisor, conf):
        ...
        if isinstance(conf, basestring):
            ...
            conf = conf or {}
        baseconf = conf.copy()
        baseconf.update({'server.socket_host': supervisor.host, 'server.socket_port': supervisor.port, 
    ...
    def assertErrorPage(self, status, message=None, pattern=''):
        ...
        epage = epage.replace(esc('<pre id="traceback"></pre>'), esc('<pre id="traceback">') + ntob('(.*)') + esc('</pre>'))
    ...
    def assertEqualDates(self, dt1, dt2, seconds=None):
        ...
        if dt1 > dt2:
            ...
            diff = dt2 - dt1
        assert diff < datetime.timedelta(seconds=seconds), '%r and %r are not within %r seconds.' % (
         dt1, dt2, seconds)
        return
