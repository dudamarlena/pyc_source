...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/cherrypy/cherrypy/test/test_proxy.py
...
class ProxyTest(helper.CPWebCase):
    ...
    def setup_server():
        ...
        class Root:
            ...
            xhost.exposed = True
            xhost._cp_config = {'tools.proxy.local': 'X-Host', 'tools.trailing_slash.extra': True}
    ...
    def testProxy(self):
        self.getPage('/')
        self.assertHeader('Location', '%s://www.mydomain.test%s/dummy' % (
        ...
        for sn in script_names:
            ...
            self.getPage(sn + '/newurl', headers=[
