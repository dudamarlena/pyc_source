...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/django/django/dispatch/saferef.py
...
def safeRef(target, onDelete=None):
    ...
    if hasattr(target, '__self__') and target.__self__ is not None:
        if not hasattr(target, '__func__'):
            raise AssertionError("safeRef target %r has __self__, but no __func__, don't know how to create reference" % (target,))
...
class BoundNonDescriptorMethodWeakref(BoundMethodWeakref):
    ...
    def __init__(self, target, onDelete=None):
        ...
        onDelete -- optional callback which will be called
            ...
            which will be passed a pointer to this object.
        """
        assert getattr(target.__self__, target.__name__) == target, "method %s isn't available as the attribute %s of %s" % (
