...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/django/django/core/management/commands/makemessages.py
...
plural_forms_re = re.compile('^(?P<value>"Plural-Forms.+?\\\\n")\\s*$', re.MULTILINE | re.DOTALL)
STATUS_OK = 0
...
def process_file(file, dirpath, potfile, domain, verbosity, extensions, wrap, location, stdout=sys.stdout):
    ...
    if domain == 'djangojs' and file_ext in extensions:
        ...
        with open(work_file, 'w') as (fp):
            fp.write(src_data)
        cmd = 'xgettext -d %s -L C %s %s --keyword=gettext_noop --keyword=gettext_lazy --keyword=ngettext_lazy:1,2 --keyword=pgettext:1c,2 --keyword=npgettext:1c,2,3 --from-code UTF-8 --add-comments=Translators -o - "%s"' % (
        ...
        if is_templatized:
            ...
            with open(os.path.join(dirpath, thefile), 'w') as (fp):
                fp.write(content)
        work_file = os.path.join(dirpath, thefile)
        cmd = 'xgettext -d %s -L Python %s %s --keyword=gettext_noop --keyword=gettext_lazy --keyword=ngettext_lazy:1,2 --keyword=ugettext_noop --keyword=ugettext_lazy --keyword=ungettext_lazy:1,2 --keyword=pgettext:1c,2 --keyword=npgettext:1c,2,3 --keyword=pgettext_lazy:1c,2 --keyword=npgettext_lazy:1c,2,3 --from-code UTF-8 --add-comments=Translators -o - "%s"' % (
         domain, wrap, location, work_file)
    else:
        return
...
def write_po_file(pofile, potfile, domain, locale, verbosity, stdout, copy_pforms, wrap, location, no_obsolete):
    ...
    os.unlink(potfile)
    if no_obsolete:
        msgs, errors, status = _popen('msgattrib %s %s -o "%s" --no-obsolete "%s"' % (
...
def make_messages(locale=None, domain='django', verbosity=1, all=False, extensions=None, symlinks=False, ignore_patterns=None, no_wrap=False, no_location=False, no_obsolete=False, stdout=sys.stdout):
    ...
    match = re.search('(?P<major>\\d+)\\.(?P<minor>\\d+)', output)
    if match:
        xversion = (
    ...
    if locale is not None:
        ...
        for locale in locales:
            ...
            if os.path.exists(potfile):
                os.unlink(potfile)
            for dirpath, file in find_files('.', ignore_patterns, verbosity, stdout, symlinks=symlinks):
                try:
                    process_file(file, dirpath, potfile, domain, verbosity, extensions, wrap, location, stdout)
                except UnicodeDecodeError:
                    stdout.write('UnicodeDecodeError: skipped file %s in %s' % (file, dirpath))

            if os.path.exists(potfile):
                write_po_file(pofile, potfile, domain, locale, verbosity, stdout, not invoked_for_django, wrap, location, no_obsolete)
...
class Command(NoArgsCommand):
    option_list = NoArgsCommand.option_list + (
     make_option('--locale', '-l', default=None, dest='locale', help='Creates or updates the message files for the given locale (e.g. pt_BR).'),
     make_option('--domain', '-d', default='django', dest='domain', help='The domain of the message files (default: "django").'),
     make_option('--all', '-a', action='store_true', dest='all', default=False, help='Updates the message files for all existing locales.'),
     make_option('--extension', '-e', dest='extensions', help='The file extension(s) to examine (default: "html,txt", or "js" if the domain is "djangojs"). Separate multiple extensions with commas, or use -e multiple times.', action='append'),
     ...
     make_option('--no-wrap', action='store_true', dest='no_wrap', default=False, help="Don't break long message lines into several lines"),
     make_option('--no-location', action='store_true', dest='no_location', default=False, help="Don't write '#: filename:line' lines"),
     make_option('--no-obsolete', action='store_true', dest='no_obsolete', default=False, help='Remove obsolete message strings'))
    ...
    def handle_noargs(self, *args, **options):
        ...
        make_messages(locale, domain, verbosity, process_all, extensions, symlinks, ignore_patterns, no_wrap, no_location, no_obsolete, self.stdout)