...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/django-tastypie-legacy/tastypie/resources.py
...
from tastypie.utils import dict_strip_unicode_keys, is_valid_jsonp_callback_value, string_to_python, trailing_slash
...
class NOT_AVAILABLE():
    ...
    def __str__(self):
        return b'No such data is available.'
...
class ResourceOptions(object):
    ...
    allowed_methods = [b'get', b'post', b'put', b'delete', b'patch']
    ...
    default_format = b'application/json'
    filtering = {}
    ordering = []
    object_class = None
    queryset = None
    ...
    collection_name = b'objects'
    detail_uri_name = b'pk'

    def __new__(cls, meta=None):
        overrides = {}
        if meta:
            for override_name in dir(meta):
                if not override_name.startswith(b'_'):
                    overrides[override_name] = getattr(meta, override_name)

        allowed_methods = overrides.get(b'allowed_methods', [b'get', b'post', b'put', b'delete', b'patch'])
        if overrides.get(b'list_allowed_methods', None) is None:
            overrides[b'list_allowed_methods'] = allowed_methods
        if overrides.get(b'detail_allowed_methods', None) is None:
            overrides[b'detail_allowed_methods'] = allowed_methods
        if six.PY3:
...
class DeclarativeMetaclass(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        try:
            ...
            parents.reverse()
            for p in parents:
                parent_fields = getattr(p, b'base_fields', {})
                for field_name, field_object in parent_fields.items():
                    attrs[b'base_fields'][field_name] = deepcopy(field_object)

        except NameError:
            pass

        for field_name, obj in attrs.copy().items():
            if hasattr(obj, b'dehydrated_type'):
                field = attrs.pop(field_name)
                declared_fields[field_name] = field

        attrs[b'base_fields'].update(declared_fields)
        attrs[b'declared_fields'] = declared_fields
        new_class = super(DeclarativeMetaclass, cls).__new__(cls, name, bases, attrs)
        opts = getattr(new_class, b'Meta', None)
        new_class._meta = ResourceOptions(opts)
        if not getattr(new_class._meta, b'resource_name', None):
            class_name = new_class.__name__
            name_bits = [ bit for bit in class_name.split(b'Resource') if bit ]
            resource_name = (b'').join(name_bits).lower()
            new_class._meta.resource_name = resource_name
        if getattr(new_class._meta, b'include_resource_uri', True):
            if b'resource_uri' not in new_class.base_fields:
                new_class.base_fields[b'resource_uri'] = fields.CharField(readonly=True, verbose_name=b'resource uri')
        else:
            if b'resource_uri' in new_class.base_fields and b'resource_uri' not in attrs:
                del new_class.base_fields[b'resource_uri']
            for field_name, field_object in new_class.base_fields.items():
                if hasattr(field_object, b'contribute_to_class'):
...
class Resource(six.with_metaclass(DeclarativeMetaclass)):
    ...
    def wrap_view(self, view):
        ...
        def wrapper(request, *args, **kwargs):
            try:
                ...
                varies = getattr(self._meta.cache, b'varies', [])
                if varies:
                    patch_vary_headers(response, varies)
                if self._meta.cache.cacheable(request, response):
                    if self._meta.cache.cache_control():
                        patch_cache_control(response, **self._meta.cache.cache_control())
                if request.is_ajax() and not response.has_header(b'Cache-Control'):
                    patch_cache_control(response, no_cache=True)
                return response
            except (BadRequest, fields.ApiFieldError) as e:
                data = {b'error': sanitize(e.args[0]) if getattr(e, b'args') else b''}
                return self.error_response(request, data, response_class=http.HttpBadRequest)
            except ValidationError as e:
                data = {b'error': sanitize(e.messages)}
                return self.error_response(request, data, response_class=http.HttpBadRequest)
            except Exception as e:
                if hasattr(e, b'response'):
                    return e.response
                if settings.DEBUG and getattr(settings, b'TASTYPIE_FULL_DEBUG', False):
                    raise
                if request.META.get(b'SERVER_NAME') == b'testserver':
                    raise
    ...
    def _handle_500(self, request, exception):
        ...
        NOT_FOUND_EXCEPTIONS = (
         NotFound, ObjectDoesNotExist, Http404)
        if isinstance(exception, NOT_FOUND_EXCEPTIONS):
            response_class = HttpResponseNotFound
            response_code = 404
        if settings.DEBUG:
            data = {b'error_message': sanitize(six.text_type(exception)), b'traceback': the_trace}
            return self.error_response(request, data, response_class=response_class)
        send_broken_links = getattr(settings, b'SEND_BROKEN_LINK_EMAILS', False)
        if not response_code == 404 or send_broken_links:
            log = logging.getLogger(b'django.request.tastypie')
            log.error(b'Internal Server Error: %s' % request.path, exc_info=True, extra={b'status_code': response_code, b'request': request})
        got_request_exception.send(self.__class__, request=request)
        data = {b'error_message': getattr(settings, b'TASTYPIE_CANNED_ERROR', b'Sorry, this request could not be processed. Please try again later.')}
        return self.error_response(request, data, response_class=response_class)
    ...
    def base_urls(self):
        ...
        """
        return [
         url(b'^(?P<resource_name>%s)%s$' % (self._meta.resource_name, trailing_slash), self.wrap_view(b'dispatch_list'), name=b'api_dispatch_list'),
         url(b'^(?P<resource_name>%s)/schema%s$' % (self._meta.resource_name, trailing_slash), self.wrap_view(b'get_schema'), name=b'api_get_schema'),
         url(b'^(?P<resource_name>%s)/set/(?P<%s_list>.*?)%s$' % (self._meta.resource_name, self._meta.detail_uri_name, trailing_slash), self.wrap_view(b'get_multiple'), name=b'api_get_multiple'),
         url(b'^(?P<resource_name>%s)/(?P<%s>.*?)%s$' % (self._meta.resource_name, self._meta.detail_uri_name, trailing_slash), self.wrap_view(b'dispatch_detail'), name=b'api_dispatch_detail')]
    ...
    def serialize(self, request, data, format, options=None):
        ...
        """
        options = options or {}
        if b'text/javascript' in format:
    ...
    def deserialize(self, request, data, format=b'application/json'):
        """
        ...
        Mostly a hook, this uses the ``Serializer`` from ``Resource._meta``.
        """
        deserialized = self._meta.serializer.deserialize(data, format=request.META.get(b'CONTENT_TYPE', format))
        return deserialized
    ...
    def dispatch_list(self, request, **kwargs):
        ...
        """
        return self.dispatch(b'list', request, **kwargs)
    ...
    def dispatch_detail(self, request, **kwargs):
        ...
        Relies on ``Resource.dispatch`` for the heavy-lifting.
        """
        return self.dispatch(b'detail', request, **kwargs)
    ...
    def dispatch(self, request_type, request, **kwargs):
        ...
        """
        allowed_methods = getattr(self._meta, b'%s_allowed_methods' % request_type, None)
        if b'HTTP_X_HTTP_METHOD_OVERRIDE' in request.META:
            request.method = request.META[b'HTTP_X_HTTP_METHOD_OVERRIDE']
        request_method = self.method_check(request, allowed=allowed_methods)
        method = getattr(self, b'%s_%s' % (request_method, request_type), None)
        if method is None:
    ...
    def remove_api_resource_names(self, url_dict):
        ...
        for key in [b'api_name', b'resource_name']:
            try:
    ...
    def method_check(self, request, allowed=None):
        ...
        allows = (b',').join([ meth.upper() for meth in allowed ])
        if request_method == b'options':
            response = HttpResponse(allows)
            response[b'Allow'] = allows
    ...
    def throttle_check(self, request):
        ...
        if throttle:
            ...
            if isinstance(throttle, int) and not isinstance(throttle, bool):
                response[b'Retry-After'] = throttle
            elif isinstance(throttle, datetime):
                response[b'Retry-After'] = format_date_time(mktime(throttle.timetuple()))
            raise ImmediateHttpResponse(response=response)
    ...
    def build_bundle(self, obj=None, data=None, request=None, objects_saved=None, via_uri=None):
        ...
        return Bundle(obj=obj, data=data, request=request, objects_saved=objects_saved, via_uri=via_uri)
    ...
    def resource_uri_kwargs(self, bundle_or_obj=None):
        ...
        """
        kwargs = {b'resource_name': self._meta.resource_name}
        if self._meta.api_name is not None:
            kwargs[b'api_name'] = self._meta.api_name
        if bundle_or_obj is not None:
            kwargs.update(self.detail_uri_kwargs(bundle_or_obj))
        return kwargs

    def get_resource_uri(self, bundle_or_obj=None, url_name=b'api_dispatch_list'):
        """
        ...
        """
        if bundle_or_obj is not None:
            url_name = b'api_dispatch_detail'
        try:
    ...
    def get_via_uri(self, uri, request=None):
        ...
        if found_at == -1:
            raise NotFound(b"An incorrect URL was provided '%s' for the '%s' resource." % (uri, self.__class__.__name__))
        chomped_uri = chomped_uri[found_at:]
        try:
            for url_resolver in getattr(self, b'urls', []):
                result = url_resolver.resolve(chomped_uri)
                if result is not None:
                    view, args, kwargs = result
                    break
            else:
                raise Resolver404(b"URI not found in 'self.urls'.")

        except Resolver404:
            raise NotFound(b"The URL provided '%s' was not a link to a valid resource." % uri)
        ...
        for field_name, field_object in self.fields.items():
            ...
            if callable(field_use_in):
                if not field_use_in(bundle):
                    continue
            elif field_use_in not in [b'all', b'list' if for_list else b'detail']:
                continue
            if field_object.dehydrated_type == b'related':
            ...
            method = getattr(self, b'dehydrate_%s' % field_name, None)
            if method:
    ...
    def full_hydrate(self, bundle):
        ...
        for field_name, field_object in self.fields.items():
            if field_object.readonly is True:
                continue
            method = getattr(self, b'hydrate_%s' % field_name, None)
            if method:
    ...
    def hydrate_m2m(self, bundle):
        ...
        """
        if bundle.obj is None:
            raise HydrationError(b"You must call 'full_hydrate' before attempting to run 'hydrate_m2m' on %r." % self)
        for field_name, field_object in self.fields.items():
    ...
    def build_schema(self):
        ...
        """
        data = {b'fields': {}, b'default_format': self._meta.default_format, 
           b'allowed_list_http_methods': self._meta.list_allowed_methods, 
           b'allowed_detail_http_methods': self._meta.detail_allowed_methods, 
           b'default_limit': self._meta.limit}
        if self._meta.ordering:
            data[b'ordering'] = self._meta.ordering
        if self._meta.filtering:
            data[b'filtering'] = self._meta.filtering
        try:
        ...
        for field_name, field_object in self.fields.items():
            data[b'fields'][field_name] = {b'default': field_object.default, b'type': field_object.dehydrated_type, 
               b'nullable': field_object.null, 
               b'blank': field_object.blank, 
               b'readonly': field_object.readonly, 
               b'help_text': field_object.help_text, 
               b'unique': field_object.unique, 
               b'primary_key': True if field_name == pk_field_name else False, 
               b'verbose_name': field_object.verbose_name or field_name.replace(b'_', b' ')}
            if field_object.dehydrated_type == b'related':
                if field_object.is_m2m:
                    related_type = b'to_many'
                else:
                    related_type = b'to_one'
                data[b'fields'][field_name][b'related_type'] = related_type
    ...
    def generate_cache_key(self, *args, **kwargs):
        ...
        """
        smooshed = [ b'%s=%s' % (key, value) for key, value in kwargs.items() ]
        return b'%s:%s:%s:%s' % (self._meta.api_name, self._meta.resource_name, (b':').join(args), (b':').join(sorted(smooshed)))
    ...
    def cached_obj_get_list(self, bundle, **kwargs):
        ...
        """
        cache_key = self.generate_cache_key(b'list', **kwargs)
    ...
    def cached_obj_get(self, bundle, **kwargs):
        ...
        commonly-accessed data faster.
        """
        cache_key = self.generate_cache_key(b'detail', **kwargs)
    ...
    def error_response(self, request, errors, response_class=None):
        ...
        desired_format = None
        if request:
            if request.GET.get(b'callback', None) is None:
                try:
                    desired_format = self.determine_format(request)
                except BadRequest:
                    pass
                ...
                desired_format = b'application/json'
        if not desired_format:
            desired_format = self._meta.default_format
        try:
            serialized = self.serialize(request, errors, desired_format)
        except BadRequest as e:
            error = b'Additional errors occurred, but serialization of those errors failed.'
            if settings.DEBUG:
                error += b' %s' % e
            return response_class(content=error, content_type=b'text/plain')
    ...
    def get_list(self, request, **kwargs):
        ...
        bundles = [ self.full_dehydrate(self.build_bundle(obj=obj, request=request), for_list=True) for obj in to_be_serialized[self._meta.collection_name]
                  ]
    ...
    def get_detail(self, request, **kwargs):
        ...
        try:
            ...
            return http.HttpMultipleChoices(b'More than one resource is found at this URI.')
    ...
    def post_list(self, request, **kwargs):
        ...
        of serialized data.
        """
        deserialized = self.deserialize(request, request.body, format=request.META.get(b'CONTENT_TYPE', b'application/json'))
    ...
    def put_list(self, request, **kwargs):
        ...
        if self._meta.collection_name not in deserialized:
            raise BadRequest(b"Invalid data sent: missing '%s'" % self._meta.collection_name)
        basic_bundle = self.build_bundle(request=request)
    ...
    def patch_list(self, request, **kwargs):
        ...
        deleted_collection_name = b'deleted_%s' % collection_name
        if collection_name not in deserialized:
            raise BadRequest(b"Invalid data sent: missing '%s'" % collection_name)
        if len(deserialized[collection_name]) and b'put' not in self._meta.detail_allowed_methods:
            raise ImmediateHttpResponse(response=http.HttpMethodNotAllowed())
        bundles_seen = []
        for data in deserialized[collection_name]:
            if b'resource_uri' in data:
                uri = data.pop(b'resource_uri')
                try:
        ...
        if deleted_collection:
            if b'delete' not in self._meta.detail_allowed_methods:
                raise ImmediateHttpResponse(response=http.HttpMethodNotAllowed())
        ...
        if not self._meta.always_return_data:
            ...
            to_be_serialized = {b'objects': [ self.full_dehydrate(b, for_list=True) for b in bundles_seen
                         ]}
            to_be_serialized = self.alter_list_data_to_serialize(request, to_be_serialized)
    ...
    def update_in_place(self, request, original_bundle, new_data):
        ...
        kwargs = {self._meta.detail_uri_name: self.get_bundle_detail_data(original_bundle), 
           b'request': request}
    ...
    def get_schema(self, request, **kwargs):
        ...
        Should return a HttpResponse (200 OK).
        """
        self.method_check(request, allowed=[b'get'])
        self.is_authenticated(request)
        self.throttle_check(request)
        self.log_throttled_access(request)
        bundle = self.build_bundle(request=request)
    ...
    def get_multiple(self, request, **kwargs):
        ...
        self.is_authenticated(request)
        self.throttle_check(request)
        kwarg_name = b'%s_list' % self._meta.detail_uri_name
        obj_identifiers = kwargs.get(kwarg_name, b'').split(b';')
        objects = []
        not_found = []
        base_bundle = self.build_bundle(request=request)
        ...
        if len(not_found):
            object_list[b'not_found'] = not_found
        self.log_throttled_access(request)
...
class ModelDeclarativeMetaclass(DeclarativeMetaclass):
    ...
    def __new__(cls, name, bases, attrs):
        meta = attrs.get(b'Meta')
        if meta and hasattr(meta, b'queryset'):
            setattr(meta, b'object_class', meta.queryset.model)
        new_class = super(ModelDeclarativeMetaclass, cls).__new__(cls, name, bases, attrs)
        include_fields = getattr(new_class._meta, b'fields', [])
        excludes = getattr(new_class._meta, b'excludes', [])
        field_names = list(new_class.base_fields.keys())
        for field_name in field_names:
            if field_name == b'resource_uri':
                continue
        ...
        if getattr(new_class._meta, b'include_absolute_url', True):
            if b'absolute_url' not in new_class.base_fields:
                new_class.base_fields[b'absolute_url'] = fields.CharField(attribute=b'get_absolute_url', readonly=True)
        elif b'absolute_url' in new_class.base_fields and b'absolute_url' not in attrs:
            del new_class.base_fields[b'absolute_url']
        return new_class
...
class BaseModelResource(Resource):
    ...
    def api_field_from_django_field(cls, f, default=fields.CharField):
        ...
        if internal_type == b'DateField':
            result = fields.DateField
        elif internal_type == b'DateTimeField':
    ...
    def get_fields(cls, fields=None, excludes=None):
        ...
        for f in cls._meta.object_class._meta.fields:
            ...
            kwargs = {b'attribute': f.name, 
               b'help_text': f.help_text, 
               b'verbose_name': f.verbose_name}
            if f.null is True:
                kwargs[b'null'] = True
            kwargs[b'unique'] = f.unique
            if not f.null and f.blank is True:
                kwargs[b'default'] = b''
                kwargs[b'blank'] = True
            if f.get_internal_type() == b'TextField':
                kwargs[b'default'] = b''
            if f.has_default():
                kwargs[b'default'] = f.default
            if getattr(f, b'auto_now', False):
                kwargs[b'default'] = f.auto_now
            if getattr(f, b'auto_now_add', False):
                kwargs[b'default'] = f.auto_now_add
            final_fields[f.name] = api_field_class(**kwargs)
            final_fields[f.name].instance_name = f.name

        return final_fields

    def check_filtering(self, field_name, filter_type=b'exact', filter_bits=None):
        """
        ...
        if field_name not in self._meta.filtering:
            raise InvalidFilterError(b"The '%s' field does not allow filtering." % field_name)
        if self._meta.filtering[field_name] not in (ALL, ALL_WITH_RELATIONS):
            if filter_type not in self._meta.filtering[field_name]:
                raise InvalidFilterError(b"'%s' is not an allowed filter on the '%s' field." % (filter_type, field_name))
        if self.fields[field_name].attribute is None:
            raise InvalidFilterError(b"The '%s' field has no 'attribute' for searching with." % field_name)
        if len(filter_bits):
            if not getattr(self.fields[field_name], b'is_related', False):
                raise InvalidFilterError(b"The '%s' field does not support relations." % field_name)
            if not self._meta.filtering[field_name] == ALL_WITH_RELATIONS:
                raise InvalidFilterError(b"Lookups are not allowed more than one level deep on the '%s' field." % field_name)
            related_resource = self.fields[field_name].get_related_resource(None)
            return [
             self.fields[field_name].attribute] + related_resource.check_filtering(filter_bits[0], filter_type, filter_bits[1:])
        else:
            return [
    ...
    def filter_value_to_python(self, value, field_name, filters, filter_expr, filter_type):
        ...
        if filter_type in ('in', 'range') and len(value):
            if hasattr(filters, b'getlist'):
                value = []
                for part in filters.getlist(filter_expr):
                    value.extend(part.split(b','))
    ...
    def build_filters(self, filters=None):
        ...
        if filters is None:
            filters = {}
        qs_filters = {}
        if getattr(self._meta, b'queryset', None) is not None:
        ...
        for filter_expr, value in filters.items():
            ...
            qs_filter = b'%s%s%s' % (db_field_name, LOOKUP_SEP, filter_type)
    ...
    def apply_sorting(self, obj_list, options=None):
        ...
        if options is None:
            options = {}
        parameter_name = b'order_by'
        if b'order_by' not in options:
            if b'sort_by' not in options:
                return obj_list
            warnings.warn(b"'sort_by' is a deprecated parameter. Please use 'order_by' instead.")
            parameter_name = b'sort_by'
        order_by_args = []
        if hasattr(options, b'getlist'):
            ...
            for order_by in order_bits:
                ...
                order = b''
                if order_by_bits[0].startswith(b'-'):
                    field_name = order_by_bits[0][1:]
                    order = b'-'
                if field_name not in self.fields:
                    raise InvalidSortError(b"No matching '%s' field for ordering on." % field_name)
                if field_name not in self._meta.ordering:
                    raise InvalidSortError(b"The '%s' field does not allow ordering." % field_name)
                if self.fields[field_name].attribute is None:
                    raise InvalidSortError(b"The '%s' field has no 'attribute' for ordering with." % field_name)
                order_by_args.append(b'%s%s' % (order, LOOKUP_SEP.join([self.fields[field_name].attribute] + order_by_bits[1:])))
    ...
    def obj_get_list(self, bundle, **kwargs):
        ...
        """
        filters = {}
        if hasattr(bundle.request, b'GET'):
        ...
        try:
            ...
            raise BadRequest(b'Invalid resource lookup data provided (mismatched type).')
    ...
    def obj_get(self, bundle, **kwargs):
        ...
        field_names.append(b'pk')
        kwargs = dict([ (k, v) for k, v in kwargs.items() if k in field_names ])
        try:
            object_list = self.get_object_list(bundle.request).filter(**kwargs)
            stringified_kwargs = (b', ').join([ b'%s=%s' % (k, v) for k, v in kwargs.items() ])
            if len(object_list) <= 0:
                raise self._meta.object_class.DoesNotExist(b"Couldn't find an instance of '%s' which matched '%s'." % (self._meta.object_class.__name__, stringified_kwargs))
            elif len(object_list) > 1:
                raise MultipleObjectsReturned(b"More than '%s' matched '%s'." % (self._meta.object_class.__name__, stringified_kwargs))
            bundle.obj = object_list[0]
            self.authorized_read_detail(object_list, bundle)
            return bundle.obj
        except ValueError:
            raise NotFound(b'Invalid resource lookup data provided (mismatched type).')
    ...
    def lookup_kwargs_with_identifiers(self, bundle, kwargs):
        ...
        for identifier in kwargs:
            ...
            if field_object.readonly is True or field_object.is_related:
                continue
            method = getattr(self, b'hydrate_%s' % identifier, None)
            if method:
                bundle = method(bundle)
            if field_object.attribute:
                value = field_object.hydrate(bundle)
    ...
    def obj_update(self, bundle, skip_errors=False, **kwargs):
        ...
        if not bundle_detail_data or arg_detail_data and bundle_detail_data != arg_detail_data:
            ...
            try:
                ...
                raise NotFound(b'A model instance matching the provided arguments could not be found.')
    ...
    def obj_delete_list(self, bundle, **kwargs):
        ...
        if hasattr(deletable_objects, b'delete'):
    ...
    def obj_delete(self, bundle, **kwargs):
        ...
        the instance.
        """
        if not hasattr(bundle.obj, b'delete'):
            try:
    ...
    def create_identifier(self, obj):
        return b'%s.%s.%s' % (obj._meta.app_label, get_module_name(obj._meta), obj.pk)
    ...
    def save_related(self, bundle):
        ...
        for field_name, field_object in self.fields.items():
            ...
            if bundle.data.get(field_name):
                if hasattr(bundle.data[field_name], b'keys'):
                    related_bundle = related_resource.build_bundle(obj=related_obj, data=bundle.data.get(field_name), request=bundle.request, objects_saved=bundle.objects_saved)
                    ...
                    related_bundle = related_resource.build_bundle(obj=related_obj, request=bundle.request, objects_saved=bundle.objects_saved)
                    related_resource.save(related_bundle)
                    related_obj = related_bundle.obj
            if related_obj:
    ...
    def save_m2m(self, bundle):
        ...
        for field_name, field_object in self.fields.items():
            ...
            if isinstance(field_object.attribute, six.string_types):
                ...
                if callable(field_object.attribute):
                    related_mngr = field_object.attribute(bundle)
                if not related_mngr:
                    continue
                if hasattr(related_mngr, b'clear'):
                    related_mngr.clear()
                related_objs = []
                for related_bundle in bundle.data[field_name]:
                    related_resource = field_object.get_related_resource(bundle.obj)
                    updated_related_bundle = related_resource.build_bundle(obj=related_bundle.obj, data=related_bundle.data, request=bundle.request, objects_saved=bundle.objects_saved, via_uri=related_bundle.via_uri)
...
class NamespacedModelResource(ModelResource):
    ...
    def _build_reverse_url(self, name, args=None, kwargs=None):
        namespaced = b'%s:%s' % (self._meta.urlconf_namespace, name)
...
def convert_post_to_VERB(request, verb):
    ...
    if request.method == verb:
        ...
        try:
            ...
            request._load_post_and_files()
            request.META[b'REQUEST_METHOD'] = verb
...
def convert_post_to_put(request):
    return convert_post_to_VERB(request, verb=b'PUT')


def convert_post_to_patch(request):
    return convert_post_to_VERB(request, verb=b'PATCH')