...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/peewee/playhouse/postgres_ext.py
...
class _JsonLookupBase(_LookupNode):
    ...
    def contains_any(self, *keys):
        return Expression(self.as_json(True), OP.JSONB_CONTAINS_ANY_KEY, Passthrough(list(keys)))

    def contains_all(self, *keys):
        return Expression(self.as_json(True), OP.JSONB_CONTAINS_ALL_KEYS, Passthrough(list(keys)))
...
class BinaryJSONField(IndexedFieldMixin, JSONField):
    ...
    def contains_any(self, *items):
        return Expression(self, OP.JSONB_CONTAINS_ANY_KEY, Passthrough(list(items)))

    def contains_all(self, *items):
        return Expression(self, OP.JSONB_CONTAINS_ALL_KEYS, Passthrough(list(items)))
...
OP.update(HKEY='key', HUPDATE='H@>', HCONTAINS_DICT='H?&', HCONTAINS_KEYS='H?', HCONTAINS_KEY='H?|', HCONTAINS_ANY_KEY='H||', ACONTAINS='A@>', ACONTAINS_ANY='A||', TS_MATCH='T@@', JSONB_CONTAINS='JB@>', JSONB_CONTAINED_BY='JB<@', JSONB_CONTAINS_ANY_KEY='JB?|', JSONB_CONTAINS_ALL_KEYS='JB?&', JSONB_EXISTS='JB?', CAST='::')
...
class PostgresqlExtCompiler(QueryCompiler):
    ...
    def _parse_json_lookup(self, node, alias_map, conv):
        ...
        for part in node.parts:
            part_sql, part_params = self.parse_node(part, alias_map, conv)
    ...
    def get_parse_map(self):
        ...
        parse_map.update(object_slice=self._parse_object_slice, json_lookup=self._parse_json_lookup, json_path=self._parse_json_path)
        return parse_map
...
class ServerSideSelectQuery(SelectQuery):
    ...
    def _execute(self):
        sql, params = self.sql()
        return self.database.execute_sql(sql, params, require_commit=False, named_cursor=True)


PostgresqlExtDatabase.register_fields({'datetime_tz': 'timestamp with time zone', 
   'hash': 'hstore', 
   'json': 'json', 
   'jsonb': 'jsonb', 
   'tsvector': 'tsvector'})
PostgresqlExtDatabase.register_ops({OP.HCONTAINS_DICT: '@>', 
   OP.HCONTAINS_KEYS: '?&', 
   OP.HCONTAINS_KEY: '?', 
...
def LateralJoin(lhs, rhs, join_type='LEFT', condition=True):
    return Clause(lhs, SQL('%s JOIN LATERAL' % join_type), rhs, SQL('ON %s', condition))