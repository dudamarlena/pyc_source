...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/django-tastypie-legacy/tastypie/cache.py
...
class NoCache(object):
    ...
    def cacheable(self, request, response):
        ...
        cached.
        """
        return bool(request.method == b'GET' and response.status_code == 200)

    def cache_control(self):
        """
        No-op for returning values for cache-control
        """
        return {b'no_cache': True}
...
class SimpleCache(NoCache):
    ...
    def __init__(self, cache_name=b'default', timeout=None, public=None, private=None, *args, **kwargs):
        """
    ...
    def cache_control(self):
        control = {b'max_age': self.timeout, 
           b's_maxage': self.timeout}
        if self.public is not None:
            control[b'public'] = self.public
        if self.private is not None:
            control[b'private'] = self.private
        return control