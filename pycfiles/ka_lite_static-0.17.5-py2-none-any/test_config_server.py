...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/cherrypy/cherrypy/test/test_config_server.py
# Compiled at: 2018-07-11 18:15:31
"""Tests for the CherryPy configuration system."""
import os, sys
localDir = os.path.join(os.getcwd(), os.path.dirname(__file__))
import socket, time, cherrypy
...
class ServerConfigTests(helper.CPWebCase):
    ...
    def testMaxRequestSizePerHandler(self):
        ...
        self.getPage('/tinyupload', method='POST', headers=[
         ('Content-Type', 'text/plain'),
         ('Content-Length', '100')], body='x' * 100)
        self.assertStatus(200)
        self.assertBody('x' * 100)
        self.getPage('/tinyupload', method='POST', headers=[
         ('Content-Type', 'text/plain'),
         ('Content-Length', '101')], body='x' * 101)
