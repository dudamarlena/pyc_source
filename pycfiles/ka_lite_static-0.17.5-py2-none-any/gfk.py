...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/peewee/playhouse/gfk.py
...
class GFKField(object):
    ...
    def get_obj(self, instance):
        ...
        if data.get(self.model_type_field) and data.get(self.model_id_field):
            ...
            query = model_class.select().where(model_class._meta.primary_key == data[self.model_id_field])
            return query.get()
...
class ReverseGFK(object):
    ...
    def __get__(self, instance, instance_type=None):
        if instance:
            return self.model_class.select().where((self.model_type_field == instance._meta.db_table) & (self.model_id_field == instance._get_pk_value()))
        else:
            return self.model_class.select().where(self.model_type_field == instance_type._meta.db_table)
    ...
    def __set__(self, instance, value):
        ...
        if isinstance(value, SelectQuery) and value.model_class == self.model_class:
            UpdateQuery(self.model_class, {self.model_type_field: mtv, 
               self.model_id_field: miv}).where(value._where).execute()
