...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/peewee/playhouse/sqlite_ext.py
...
"""
import inspect, math, sqlite3, struct
from peewee import *
...
class SqliteQueryCompiler(QueryCompiler):
    ...
    def _create_table(self, model_class, safe=False, options=None):
        clause = super(SqliteQueryCompiler, self)._create_table(model_class, safe=safe)
...
class FTSModel(VirtualModel):
    ...
    def search(cls, term, alias='score'):
        ...
        return cls.select(cls, cls.rank().alias(alias)).where(cls.match(term)).order_by(SQL(alias).desc())
    ...
    def search_bm25(cls, term, field=None, k=1.2, b=0.75, alias='score'):
        ...
        if field is None:
            field = find_best_search_field(cls)
        return cls.select(cls, cls.bm25(field, k, b).alias(alias)).where(cls.match(term)).order_by(SQL(alias).desc())
...
def ClosureTable(model_class, foreign_key=None):
    ...
    class BaseClosureTable(VirtualModel):
        ...
        def descendants(cls, node, depth=None, include_node=False):
            query = model_class.select(model_class, cls.depth.alias('depth')).join(cls, on=primary_key == cls.id).where(cls.root == node)
        ...
        def ancestors(cls, node, depth=None, include_node=False):
            query = model_class.select(model_class, cls.depth.alias('depth')).join(cls, on=primary_key == cls.root).where(cls.id == node)
            if depth:
                query = query.where(cls.depth == depth)
            elif not include_node:
                query = query.where(cls.depth > 0)
    ...
    class Meta:
        ...
        options = {'tablename': model_class._meta.db_table, 
...
OP.MATCH = 'match'
SqliteExtDatabase.register_ops({OP.MATCH: 'MATCH'})
