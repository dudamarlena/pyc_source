...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/cherrypy/cherrypy/test/benchmark.py
...
import re, sys, time, traceback, cherrypy
...
"""
    parse_patterns = [
     (
      ...
      ntob('^Complete requests:\\s*(\\d+)')),
     (
      'failed_requests', 'Failed',
      ntob('^Failed requests:\\s*(\\d+)')),
     (
      'requests_per_second', 'req/sec',
      ntob('^Requests per second:\\s*([0-9.]+)')),
     (
      'time_per_request_concurrent', 'msec/req',
      ntob('^Time per request:\\s*([0-9.]+).*concurrent requests\\)$')),
     (
      'transfer_rate', 'KB/sec',
      ntob('^Transfer rate:\\s*([0-9.]+)'))]
