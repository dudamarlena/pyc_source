...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/django-tastypie-legacy/tastypie/authentication.py
# Compiled at: 2018-07-11 18:15:32
from __future__ import unicode_literals
import base64, hmac, time, uuid
...
class Authentication(object):
    ...
    def get_identifier(self, request):
        ...
        """
        return b'%s_%s' % (request.META.get(b'REMOTE_ADDR', b'noaddr'), request.META.get(b'REMOTE_HOST', b'nohost'))
...
class BasicAuthentication(Authentication):
    ...
    def __init__(self, backend=None, realm=b'django-tastypie', **kwargs):
    ...
    def _unauthorized(self):
        ...
        response[b'WWW-Authenticate'] = b'Basic Realm="%s"' % self.realm
        return response
    ...
    def is_authenticated(self, request, **kwargs):
        ...
        ``HttpResponse`` if you need something custom.
        """
        if not request.META.get(b'HTTP_AUTHORIZATION'):
            return self._unauthorized()
        else:
            try:
                auth_type, data = request.META[b'HTTP_AUTHORIZATION'].split()
                if auth_type.lower() != b'basic':
                    return self._unauthorized()
                user_pass = base64.b64decode(data).decode(b'utf-8')
            except:
                return self._unauthorized()

            bits = user_pass.split(b':', 1)
            if len(bits) != 2:
                return self._unauthorized()
            if self.backend:
    ...
    def get_identifier(self, request):
        ...
        """
        return request.META.get(b'REMOTE_USER', b'nouser')
...
class ApiKeyAuthentication(Authentication):
    ...
    def extract_credentials(self, request):
        authorization = request.META.get(b'HTTP_AUTHORIZATION', b'')
        if authorization and authorization.lower().startswith(b'apikey '):
            auth_type, data = authorization.split()
            username, api_key = data.split(b':', 1)
        else:
            username = request.GET.get(b'username') or request.POST.get(b'username')
            api_key = request.GET.get(b'api_key') or request.POST.get(b'api_key')
    ...
    def is_authenticated(self, request, **kwargs):
        ...
        try:
            ...
            user = User.objects.select_related(b'api_key').get(**lookup_kwargs)
...
class SessionAuthentication(Authentication):
    ...
    def is_authenticated(self, request, **kwargs):
        ...
        if request.method in ('GET', 'HEAD', 'OPTIONS', 'TRACE'):
            ...
            if getattr(request, b'_dont_enforce_csrf_checks', False):
                return request.user.is_authenticated()
            csrf_token = _sanitize_token(request.COOKIES.get(settings.CSRF_COOKIE_NAME, b''))
            if request.is_secure():
                referer = request.META.get(b'HTTP_REFERER')
                if referer is None:
                    return False
                good_referer = b'https://%s/' % request.get_host()
                if not same_origin(referer, good_referer):
                    return False
            request_csrf_token = request.META.get(b'HTTP_X_CSRFTOKEN', b'')
...
class DigestAuthentication(Authentication):
    ...
    def __init__(self, backend=None, realm=b'django-tastypie', **kwargs):
        ...
        if python_digest is None:
            raise ImproperlyConfigured(b"The 'python_digest' package could not be imported. It is required for use with the 'DigestAuthentication' class.")
        return

    def _unauthorized(self):
        response = HttpUnauthorized()
        new_uuid = uuid.uuid4()
        opaque = hmac.new(str(new_uuid).encode(b'utf-8'), digestmod=sha1).hexdigest()
        response[b'WWW-Authenticate'] = python_digest.build_digest_challenge(timestamp=time.time(), secret=getattr(settings, b'SECRET_KEY', b''), realm=self.realm, opaque=opaque, stale=False)
        return response
    ...
    def is_authenticated(self, request, **kwargs):
        ...
        if not request.META.get(b'HTTP_AUTHORIZATION'):
            return self._unauthorized()
        try:
            auth_type, data = request.META[b'HTTP_AUTHORIZATION'].split(b' ', 1)
            if auth_type.lower() != b'digest':
                return self._unauthorized()
        except:
            return self._unauthorized()

        digest_response = python_digest.parse_digest_credentials(request.META[b'HTTP_AUTHORIZATION'])
        if not python_digest.validate_nonce(digest_response.nonce, getattr(settings, b'SECRET_KEY', b'')):
            return self._unauthorized()
        user = self.get_user(digest_response.username)
        api_key = self.get_key(user)
        if user is False or api_key is False:
            return self._unauthorized()
        expected = python_digest.calculate_request_digest(request.method, python_digest.calculate_partial_digest(digest_response.username, self.realm, api_key), digest_response)
    ...
    def get_identifier(self, request):
        ...
        This implementation returns the user's username.
        """
        if hasattr(request, b'user'):
            if hasattr(request.user, b'username'):
...
class OAuthAuthentication(Authentication):
    ...
    def __init__(self, **kwargs):
        ...
        if oauth2 is None:
            raise ImproperlyConfigured(b"The 'python-oauth2' package could not be imported. It is required for use with the 'OAuthAuthentication' class.")
        if oauth_provider is None:
            raise ImproperlyConfigured(b"The 'django-oauth-plus' package could not be imported. It is required for use with the 'OAuthAuthentication' class.")
        return
    ...
    def is_authenticated(self, request, **kwargs):
        ...
        if self.is_valid_request(request):
            ...
            consumer = store.get_consumer(request, oauth_request, oauth_request.get_parameter(b'oauth_consumer_key'))
            try:
                token = store.get_access_token(request, oauth_request, consumer, oauth_request.get_parameter(b'oauth_token'))
            except oauth_provider.store.InvalidTokenError:
                return oauth_provider.utils.send_oauth_error(oauth2.Error(_(b'Invalid access token: %s') % oauth_request.get_parameter(b'oauth_token')))
            ...
            if consumer and token:
                ...
                return True
            return oauth_provider.utils.send_oauth_error(oauth2.Error(_(b'You are not allowed to access this resource.')))
        return oauth_provider.utils.send_oauth_error(oauth2.Error(_(b'Invalid request parameters.')))
    ...
    def is_valid_request(self, request):
        ...
        """
        auth_params = request.META.get(b'HTTP_AUTHORIZATION', [])
