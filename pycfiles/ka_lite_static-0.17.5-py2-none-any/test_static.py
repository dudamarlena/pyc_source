...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/cherrypy/cherrypy/test/test_static.py
...
class StaticTest(helper.CPWebCase):
    ...
    def setup_server():
        ...
        root = Root()
        root.static = Static()
        rootconf = {'/static': {'tools.staticdir.on': True, 
                       'tools.staticdir.dir': 'static', 
                       'tools.staticdir.root': curdir}, 
           '/style.css': {'tools.staticfile.on': True, 
                          'tools.staticfile.filename': os.path.join(curdir, 'style.css')}, 
           '/docroot': {'tools.staticdir.on': True, 
                        'tools.staticdir.root': curdir, 
                        'tools.staticdir.dir': 'static', 
                        'tools.staticdir.index': 'index.html'}, 
           '/error': {'tools.staticdir.on': True, 
                      'request.show_tracebacks': True}, 
           '/404test': {'tools.staticdir.on': True, 
                        'tools.staticdir.root': curdir, 
                        'tools.staticdir.dir': 'static', 
                        'error_page.404': error_page_404}}
        rootApp = cherrypy.Application(root)
        rootApp.merge(rootconf)
        test_app_conf = {'/test': {'tools.staticdir.index': 'index.html', 
    ...
    def test_index(self):
        ...
        self.assertMatchesBody('This resource .* <a href=([\'"])%s/docroot/\\1>%s/docroot/</a>.' % (
         self.base(), self.base()))
    ...
    def test_config_errors(self):
        ...
        if sys.version_info >= (3, 3):
            errmsg = ntob('TypeError: staticdir\\(\\) missing 2 required positional arguments')
        else:
            errmsg = ntob('TypeError: staticdir\\(\\) takes at least 2 (positional )?arguments \\(0 given\\)')
    ...
    def test_file_stream(self):
        ...
        while remaining > 0:
            ...
            s, h, b = helper.webtest.openURL(ntob('/tell'), headers=[], host=self.HOST, port=self.PORT, http_conn=newconn)
            if not b:
            ...
            if tell_position >= BIGFILE_SIZE:
                if expected < BIGFILE_SIZE / 2:
                    self.fail('The file should have advanced to position %r, but has already advanced to the end of the file. It may not be streamed as intended, or at the wrong chunk size (64k)' % expected)
            elif tell_position < expected:
                self.fail('The file should have advanced to position %r, but has only advanced to position %r. It may not be streamed as intended, or at the wrong chunk size (65536)' % (
...
def error_page_404(status, message, traceback, version):
    import os.path
    return static.serve_file(os.path.join(curdir, 'static', '404.html'), content_type='text/html')