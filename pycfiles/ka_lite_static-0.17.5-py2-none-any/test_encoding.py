...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/cherrypy/cherrypy/test/test_encoding.py
# Compiled at: 2018-07-11 18:15:31
import gzip, sys, cherrypy
from cherrypy._cpcompat import BytesIO, IncompleteRead, ntob, ntou
europoundUnicode = ntou(b'\x80\xa3')
sing = ntou('\\u6bdb\\u6cfd\\u4e1c: Sing, Little Birdie?', 'escape')
...
class EncodingTests(helper.CPWebCase):
    ...
    def setup_server():
        ...
        class Root:
            ...
            def cookies_and_headers(self):
                ...
                cherrypy.response.headers['Some-Header'] = 'My dög has fleas'
                return 'Any content'

            cookies_and_headers.exposed = True

            def reqparams(self, *args, **kwargs):
                return ntob(', ').join([ (': ').join((k, v)).encode('utf8') for k, v in sorted(cherrypy.request.params.items())
                                       ])
            ...
            nontext._cp_config = {'tools.encode.text_only': False, 'tools.encode.add_charset': True}
    ...
    def test_urlencoded_decoding(self):
        ...
        (
         self.getPage('/', method='POST', headers=[
        ...
        body = ntob('q=£')
        (
         self.getPage('/reqparams', method='POST', headers=[
          ('Content-Type', 'application/x-www-form-urlencoded'),
          (
           'Content-Length', str(len(body)))], body=body),)
        self.assertBody(ntob('q: £'))
        body = ntob(b'\xff\xfeq\x00=\xff\xfe\xa3\x00')
        (
         self.getPage('/reqparams', method='POST', headers=[
    ...
    def test_decode_tool(self):
        body = ntob(b'\xff\xfeq\x00=\xff\xfe\xa3\x00')
        (
         self.getPage('/decode/extra_charset', method='POST', headers=[
          ('Content-Type', 'application/x-www-form-urlencoded'),
          (
           'Content-Length', str(len(body)))], body=body),)
        self.assertBody(ntob('q: £'))
        ...
        body = ntob('q=£')
        (
         self.getPage('/decode/force_charset', method='POST', headers=[
          ('Content-Type', 'application/x-www-form-urlencoded'),
          (
           'Content-Length', str(len(body)))], body=body),)
    ...
    def test_multipart_decoding(self):
        body = ntob(('\r\n').join([
         ...
         '',
         b'\xff\xfea\x00b\x00\x1c c\x00',
         '--X',
         'Content-Type: text/plain;charset=utf-16',
         'Content-Disposition: form-data; name="submit"',
         '',
         b'\xff\xfeC\x00r\x00e\x00a\x00t\x00e\x00',
         '--X--']))
        (
         self.getPage('/reqparams', method='POST', headers=[
          ...
          (
           'Content-Length', str(len(body)))], body=body),)
        self.assertBody(ntob('submit: Create, text: ab“c'))
    ...
    def test_multipart_decoding_no_charset(self):
        ...
        (
         self.getPage('/reqparams', method='POST', headers=[
          ...
          (
           'Content-Length', str(len(body)))], body=body),)
        self.assertBody(ntob('submit: Create, text: “'))
    ...
    def testGzip(self):
        ...
        if cherrypy.server.protocol_version == 'HTTP/1.0' or getattr(cherrypy.server, 'using_apache', False):
            self.getPage('/gzip/noshow_stream', headers=[
             ('Accept-Encoding', 'gzip')])
            self.assertHeader('Content-Encoding', 'gzip')
            self.assertInBody(b'\x1f\x8b\x08\x00')
        else:
            self.assertRaises((ValueError, IncompleteRead), self.getPage, '/gzip/noshow_stream', headers=[
             ('Accept-Encoding', 'gzip')])
