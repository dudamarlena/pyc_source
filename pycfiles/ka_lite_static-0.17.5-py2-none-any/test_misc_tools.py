...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/cherrypy/cherrypy/test/test_misc_tools.py
...
def setup_server():
    ...
    conf = {'/referer': {'tools.referer.on': True, 'tools.referer.pattern': 'http://[^/]*example\\.com'}, 
       '/referer/reject': {'tools.referer.accept': False, 'tools.referer.accept_missing': True}, 
...
class RefererTest(helper.CPWebCase):
    ...
    def testReferer(self):
        ...
        self.getPage('/referer/accept', headers=[
        ...
        self.assertStatus(200)
        self.assertBody('Accepted!')
        self.getPage('/referer/reject', headers=[
         ('Referer', 'http://www.example.com/')])
        self.assertErrorPage(403, 'Forbidden Referer header.')
...
class AcceptTest(helper.CPWebCase):
    ...
    def test_Accept_Tool(self):
        ...
        self.getPage('/accept/feed', headers=[
