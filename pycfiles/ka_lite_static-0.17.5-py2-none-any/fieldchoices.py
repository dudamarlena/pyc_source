...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/django/django/contrib/databrowse/plugins/fieldchoices.py
...
class FieldChoicePlugin(DatabrowsePlugin):
    ...
    def model_index_html(self, request, model, site):
        ...
        if not fields:
            return b''
        return format_html(b'<p class="filter"><strong>View by:</strong> {0}</p>', format_html_join(b', ', b'<a href="fields/{0}/">{1}</a>', ((f.name, force_text(capfirst(f.verbose_name))) for f in fields.values())))
        ...
        if easy_instance_field.field in self.field_dict(easy_instance_field.model.model).values():
            return [
             b'%s%s/%s/%s/' % (
              ...
              urlquote(easy_instance_field.raw_value, safe=b''))]

    def model_view(self, request, model_databrowse, url):
        self.model, self.site = model_databrowse.model, model_databrowse.site
        self.fields = self.field_dict(self.model)
        if not self.fields:
            raise http.Http404(b'The requested model has no fields.')
        if url is None:
            return self.homepage_view(request)
        else:
            url_bits = url.split(b'/', 1)
    ...
    def homepage_view(self, request):
        ...
        return render_to_response(b'databrowse/fieldchoice_homepage.html', {b'root_url': self.site.root_url, b'model': easy_model, b'field_list': field_list})
    ...
    def field_view(self, request, field, value=None):
        ...
        if value is not None:
            ...
            return render_to_response(b'databrowse/fieldchoice_detail.html', {b'root_url': self.site.root_url, b'model': easy_model, b'field': easy_field, b'value': value, b'object_list': obj_list})
        else:
            obj_list = [ v[field.name] for v in self.model._default_manager.distinct().order_by(field.name).values(field.name) ]
            return render_to_response(b'databrowse/fieldchoice_list.html', {b'root_url': self.site.root_url, b'model': easy_model, b'field': easy_field, b'object_list': obj_list})