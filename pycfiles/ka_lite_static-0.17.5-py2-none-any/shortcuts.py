...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/peewee/playhouse/shortcuts.py
...
def model_to_dict(model, recurse=True, backrefs=False, only=None, exclude=None, seen=None, extra_attrs=None, fields_from_query=None):
    ...
    for field in model._meta.get_fields():
        ...
        if isinstance(field, ForeignKeyField) and recurse:
            if field_data:
                ...
                field_data = model_to_dict(rel_obj, recurse=recurse, backrefs=backrefs, only=only, exclude=exclude, seen=seen)
            else:
                field_data = {}
    ...
    if backrefs:
        for related_name, foreign_key in model._meta.reverse_rel.items():
            ...
            related_query = getattr(model, related_name + '_prefetch', getattr(model, related_name))
            for rel_obj in related_query:
                accum.append(model_to_dict(rel_obj, recurse=recurse, backrefs=backrefs, only=only, exclude=exclude))
...
def dict_to_model(model_class, data, ignore_unknown=False):
    ...
    for key, value in data.items():
        if key in meta.fields:
            ...
            raise AttributeError('Unrecognized attribute "%s" for model class %s.' % (
             key, model_class))
        is_foreign_key = isinstance(field, ForeignKeyField)
        if not is_backref and is_foreign_key and isinstance(value, dict):
            setattr(instance, field.name, dict_to_model(field.rel_model, value, ignore_unknown))
        elif is_backref and isinstance(value, (list, tuple)):
            instances = [ dict_to_model(field.model_class, row_data, ignore_unknown) for row_data in value
                        ]
