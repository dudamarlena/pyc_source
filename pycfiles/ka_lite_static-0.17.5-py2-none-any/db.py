...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/South/south/tests/db.py
...
try:
    ...
    IntegrityError = (
     DjangoIntegrityError, SQLServerIntegrityError)
except ImportError:
...
class TestOperations(unittest.TestCase):
    ...
    @skipUnless(db.supports_foreign_keys, 'Foreign keys can only be deleted on engines that support them.')
    def test_recursive_foreign_key_delete(self):
        """
        Test that recursive foreign keys are deleted correctly (see #1065)
        """
        Test = db.mock_model(model_name='Test', db_table='test_rec_fk_del', db_tablespace='', pk_field_name='id', pk_field_type=models.AutoField, pk_field_args=[])
    ...
    def test_alter_char_default(self):
        ...
        if 'oracle' in db.backend_name:
            ...
            self.assertRaises(IntegrityError, db.execute, 'INSERT INTO test_altercd (eggs) values (12)')
    ...
    @skipUnless(db.has_check_constraints, 'Only applies to databases that support CHECK constraints.')
    ...
    @skipIf(db.backend_name == 'sqlite3', "SQLite backend doesn't support this yet.")
    ...
    def test_datetime_default(self):
        ...
        for cols in ['col1,col2', 'col2,col3']:
            ...
            self.assertRaises(IntegrityError, db.execute, statement, [end_of_world, end_of_world])
            db.rollback_transaction()
