...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/South/south/migration/base.py
...
class Migrations(with_metaclass(MigrationsMetaclass, list)):
    ...
    if getattr(settings, 'SOUTH_USE_PYC', False):
        MIGRATION_FILENAME = re.compile('(?!__init__)[0-9a-zA-Z_]*(\\.pyc?)?$')
    else:
        MIGRATION_FILENAME = re.compile('(?!__init__)[0-9a-zA-Z_]*(\\.py)?$')
    ...
    def migrations_dir(self):
        ...
        try:
            ...
            try:
                ...
                raise exceptions.InvalidMigrationModule(application=self.application.__name__, module=module_path)
            else:
        ...
        for f in os.listdir(dirname):
            if self.MIGRATION_FILENAME.match(os.path.basename(f)):
                ...
                if os.path.isdir(full_path):
                    if not (os.path.isfile(os.path.join(full_path, '__init__.py')) or getattr(settings, 'SOUTH_USE_PYC', False) and os.path.isfile(os.path.join(full_path, '__init__.pyc'))):
                        continue
                filenames.append(f)
