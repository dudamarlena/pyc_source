...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/django/django/forms/formsets.py
...
class BaseFormSet(object):
    ...
    def __init__(self, data=None, files=None, auto_id=b'id_%s', prefix=None, initial=None, error_class=ErrorList):
    ...
    def management_form(self):
        ...
        if self.is_bound:
            ...
            if not form.is_valid():
                raise ValidationError(b'ManagementForm data is missing or has been tampered with')
        else:
    ...
    @property
    def empty_form(self):
        form = self.form(auto_id=self.auto_id, prefix=self.add_prefix(b'__prefix__'), empty_permitted=True)
    ...
    def deleted_forms(self):
        ...
        if not self.is_valid() or not self.can_delete:
            raise AttributeError(b"'%s' object has no attribute 'deleted_forms'" % self.__class__.__name__)
...
def formset_factory(form, formset=BaseFormSet, extra=1, can_order=False, can_delete=False, max_num=None):
    ...
    attrs = {b'form': form, b'extra': extra, b'can_order': can_order, 
       b'can_delete': can_delete, b'max_num': max_num, 
       b'absolute_max': absolute_max}
