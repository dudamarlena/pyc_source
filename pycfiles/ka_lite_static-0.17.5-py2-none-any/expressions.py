...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/django/django/db/models/sql/expressions.py
...
class SQLEvaluator(object):
    ...
    def prepare_leaf(self, node, query, allow_joins):
        ...
        if len(field_list) == 1 and node.name in query.aggregate_select.keys():
            self.contains_aggregate = True
            self.cols.append((node, query.aggregate_select[node.name]))
        else:
            try:
                dupe_multis = False if self.reuse is None else True
                field, source, opts, join_list, last, _ = query.setup_joins(field_list, query.get_meta(), query.get_initial_alias(), dupe_multis, can_reuse=self.reuse)
                ...
                raise FieldError('Cannot resolve keyword %r into field. Choices are: %s' % (
                 self.name, [ f.name for f in self.opts.fields ]))
    ...
    def evaluate_node(self, node, qn, connection):
        ...
        return (
         connection.ops.combine_expression(node.connector, expressions), expression_params)
