...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/slimit/slimit/visitors/minvisitor.py
...
class ECMAMinifier(object):
    ...
    def visit_BinOp(self, node):
        if node.op in ('instanceof', 'in'):
            template = '%s %s %s'
        elif node.op == '+' and isinstance(node.right, ast.UnaryOp) and node.right.op == '++' and not node.right.postfix:
            template = '%s%s %s'
        else:
            template = '%s%s%s'
        if getattr(node, '_parens', False):
