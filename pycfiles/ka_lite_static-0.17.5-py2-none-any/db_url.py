...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/peewee/playhouse/db_url.py
...
def connect(url, **connect_params):
    ...
    if database_class is None:
        if database_class in schemes:
            raise RuntimeError('Attempted to use "%s" but a required library could not be imported.' % parsed.scheme)
        else:
