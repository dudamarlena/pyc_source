...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/django/django/contrib/humanize/templatetags/humanize.py
...
def ordinal(value):
    ...
    if value % 100 in (11, 12, 13):
        return b'%d%s' % (value, suffixes[0])
    return b'%d%s' % (value, suffixes[(value % 10)])
...
def apnumber(value):
    ...
    if not 0 < value < 10:
        return value
    return (
     _(b'one'), _(b'two'), _(b'three'), _(b'four'), _(b'five'), _(b'six'), _(b'seven'), _(b'eight'), _(b'nine'))[(value - 1)]
    ...
    if not isinstance(value, date):
        ...
        if value < now:
            ...
            if delta.seconds == 0:
                return _(b'now')
            if delta.seconds < 60:
                return ungettext(b'a second ago', b'%(count)s seconds ago', delta.seconds) % {b'count': delta.seconds}
            if delta.seconds // 60 < 60:
                count = delta.seconds // 60
                return ungettext(b'a minute ago', b'%(count)s minutes ago', count) % {b'count': count}
            count = delta.seconds // 60 // 60
            return ungettext(b'an hour ago', b'%(count)s hours ago', count) % {b'count': count}
        else:
            delta = value - now
            if delta.days != 0:
            ...
            if delta.seconds == 0:
                return _(b'now')
            if delta.seconds < 60:
                return ungettext(b'a second from now', b'%(count)s seconds from now', delta.seconds) % {b'count': delta.seconds}
            if delta.seconds // 60 < 60:
                count = delta.seconds // 60
                return ungettext(b'a minute from now', b'%(count)s minutes from now', count) % {b'count': count}
            count = delta.seconds // 60 // 60
            return ungettext(b'an hour from now', b'%(count)s hours from now', count) % {b'count': count}
        return