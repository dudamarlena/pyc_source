...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/cherrypy/cherrypy/test/test_core.py
# Compiled at: 2018-07-11 18:15:31
"""Basic tests for the CherryPy core: request handling."""
import os
localDir = os.path.dirname(__file__)
import sys, types, cherrypy
...
class CoreRequestHandlingTest(helper.CPWebCase):
    ...
    def setup_server():
        ...
        class Root:
            ...
            def defct(self, newct):
                newct = 'text/%s' % newct
                cherrypy.config.update({'tools.response_headers.on': True, 'tools.response_headers.headers': [
                                                    (
                                                     'Content-Type', newct)]})
        ...
        class Redirect(Test):
            ...
            class Error:
                _cp_config = {'tools.err_redirect.on': True, 'tools.err_redirect.url': '/errpage', 
        ...
        class MultiHeader(Test):
            ...
            header_list = cherrypy.tools.append_headers(header_list=[
             ...
             (
              ntob('WWW-Authenticate'), ntob('Basic realm="foo"'))])(header_list)
    ...
    def testSlashes(self):
        ...
        self.assertMatchesBody('<a href=([\'"])%s/redirect/[?]id=3\\1>%s/redirect/[?]id=3</a>' % (
         self.base(), self.base()))
        if self.prefix():
            self.getPage('')
            self.assertStatus(301)
        ...
        self.assertStatus(301)
        self.assertMatchesBody('<a href=([\'"])%s/redirect/by_code[?]code=307\\1>%s/redirect/by_code[?]code=307</a>' % (
         self.base(), self.base()))
    ...
    def testRedirect(self):
        ...
        self.assertMatchesBody('<a href=([\'\\"])(.*)\\/some\\/url\\#%s\\1>\\2\\/some\\/url\\#%s</a>' % (
         frag, frag))
    ...
    def test_cherrypy_url(self):
        ...
        self.getPage('/url/leaf?path_info=page1', headers=[
         (
          'Host', host)])
