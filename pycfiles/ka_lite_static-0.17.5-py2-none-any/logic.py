...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/South/south/tests/logic.py
...
class TestBrokenMigration(Monkeypatcher):
    installed_apps = [
     'fakeapp', 'otherfakeapp', 'brokenapp']

    def test_broken_dependencies(self):
        self.assertRaises(exceptions.DependsOnUnmigratedApplication, Migrations.calculate_dependencies, force=True)


class TestMigration(Monkeypatcher):
    installed_apps = [
     'fakeapp', 'otherfakeapp']
    ...
    def test_str(self):
        ...
        self.assertEqual(['fakeapp:0001_spam',
         'fakeapp:0002_eggs',
         'fakeapp:0003_alter_spam'], migrations)

    def test_repr(self):
        migrations = [ repr(m) for m in self.fakeapp ]
        self.assertEqual(['<Migration: fakeapp:0001_spam>',
         '<Migration: fakeapp:0002_eggs>',
         '<Migration: fakeapp:0003_alter_spam>'], migrations)

    def test_app_label(self):
        self.assertEqual(['fakeapp', 'fakeapp', 'fakeapp'], [ m.app_label() for m in self.fakeapp ])

    def test_name(self):
        self.assertEqual(['0001_spam', '0002_eggs', '0003_alter_spam'], [ m.name() for m in self.fakeapp ])

    def test_full_name(self):
        self.assertEqual(['fakeapp.migrations.0001_spam',
         'fakeapp.migrations.0002_eggs',
         'fakeapp.migrations.0003_alter_spam'], [ m.full_name() for m in self.fakeapp ])
    ...
    def test_migration(self):
        ...
        self.assertEqual([M1, M2, M3], [ m.migration().Migration for m in self.fakeapp ])
        self.assertRaises(exceptions.UnknownMigration, self.fakeapp['9999_unknown'].migration)

    def test_previous(self):
        self.assertEqual([None,
         self.fakeapp['0001_spam'],
         self.fakeapp['0002_eggs']], [ m.previous() for m in self.fakeapp ])
        return
    ...
    def test_dependencies(self):
        ...
        self.assertEqual([
         ...
         set([self.fakeapp['0002_eggs']])], [ m.dependencies for m in self.fakeapp ])
        self.assertEqual([
         set([self.fakeapp['0001_spam']]),
         set([self.otherfakeapp['0001_first']]),
         set([
          self.otherfakeapp['0002_second'],
          self.fakeapp['0003_alter_spam']])], [ m.dependencies for m in self.otherfakeapp ])
    ...
    def test_forwards_plan(self):
        self.assertEqual([
         ...
         [
          self.fakeapp['0001_spam'],
          self.fakeapp['0002_eggs'],
          self.fakeapp['0003_alter_spam']]], [ m.forwards_plan() for m in self.fakeapp ])
        self.assertEqual([
         [
          self.fakeapp['0001_spam'],
         ...
         [
          ...
          self.otherfakeapp['0003_third']]], [ m.forwards_plan() for m in self.otherfakeapp ])
    ...
    def test_dependencies(self):
        self.assertEqual([
         ...
         set([
          self.deps_a['0003_a'],
          self.deps_b['0003_b']]),
         set([self.deps_a['0004_a']])], [ m.dependencies for m in self.deps_a ])
        self.assertEqual([
         set([]),
         set([
          self.deps_b['0001_b'],
         ...
         set([self.deps_b['0004_b']])], [ m.dependencies for m in self.deps_b ])
        self.assertEqual([
         set([]),
         ...
         set([
          self.deps_c['0004_c'],
          self.deps_a['0002_a']])], [ m.dependencies for m in self.deps_c ])
    ...
    def test_forwards_plan(self):
        self.assertEqual([[self.deps_a['0001_a']],
         ...
         [
          ...
          self.deps_b['0003_b'],
          self.deps_a['0004_a'],
          self.deps_a['0005_a']]], [ m.forwards_plan() for m in self.deps_a ])
        self.assertEqual([[self.deps_b['0001_b']],
         ...
         [
          ...
          self.deps_b['0003_b'],
          self.deps_b['0004_b'],
          self.deps_b['0005_b']]], [ m.forwards_plan() for m in self.deps_b ])
        self.assertEqual([[self.deps_c['0001_c']],
         ...
         [
          ...
          self.deps_a['0001_a'],
          self.deps_a['0002_a'],
          self.deps_c['0005_c']]], [ m.forwards_plan() for m in self.deps_c ])
    ...
    def test_backwards_plan(self):
        self.assertEqual([
         ...
         [
          ...
          self.deps_a['0004_a']],
         [
          self.deps_a['0005_a']]], [ m.backwards_plan() for m in self.deps_a ])
        self.assertEqual([
         [
          self.deps_b['0005_b'],
          self.deps_b['0004_b'],
         ...
         [
          ...
          self.deps_b['0004_b']],
         [
          self.deps_b['0005_b']]], [ m.backwards_plan() for m in self.deps_b ])
        self.assertEqual([
         [
          self.deps_c['0005_c'],
          self.deps_c['0004_c'],
         ...
         [
          ...
          self.deps_c['0004_c']],
         [
          self.deps_c['0005_c']]], [ m.backwards_plan() for m in self.deps_c ])
...
class TestMigrations(Monkeypatcher):
    ...
    def test_migration(self):
        ...
        self.assertRaises(exceptions.UnknownMigration, migration['0001_jam'].migration)
    ...
    def test_full_name(self):
        names = [
         'fakeapp', 'otherfakeapp']
        self.assertEqual([ n + '.migrations' for n in names ], [ Migrations(n).full_name() for n in names ])
...
class TestMigrationLogic(Monkeypatcher):
    ...
    def test_apply_migrations(self):
        ...
        self.assertListEqual((
         ('fakeapp', '0001_spam'),
         ('fakeapp', '0002_eggs'),
         ('fakeapp', '0003_alter_spam')), MigrationHistory.objects.values_list('app_name', 'migration'))
    ...
    def test_migration_merge_forwards(self):
        ...
        self.assertListEqual((('fakeapp', '0002_eggs'), ), MigrationHistory.objects.values_list('app_name', 'migration'))
        self.assertRaises(exceptions.InconsistentMigrationHistory, migrate_app, migrations, target_name=None, fake=False)
        self.assertRaises(exceptions.InconsistentMigrationHistory, migrate_app, migrations, target_name='zero', fake=False)
        try:
            ...
            self.assertEqual([
             (
              ...
              migrations['0001_spam'])], e.problems)
    ...
    def test_alter_column_null(self):
        ...
        self.assertListEqual((
         ('fakeapp', '0001_spam'),
         ('fakeapp', '0002_eggs')), MigrationHistory.objects.values_list('app_name', 'migration'))
    ...
    def test_dependencies(self):
        ...
        self.assertEqual([fakeapp['0001_spam'],
         fakeapp['0002_eggs'],
         fakeapp['0003_alter_spam']], fakeapp['0003_alter_spam'].forwards_plan())
        self.assertEqual([
         fakeapp['0001_spam'],
         ...
         otherfakeapp['0003_third']], otherfakeapp['0003_third'].forwards_plan())
...
class TestMigrationUtils(Monkeypatcher):
    ...
    def test_get_app_label(self):
        self.assertEqual('southtest', get_app_label(self.create_fake_app('southtest.models')))
        self.assertEqual('baz', get_app_label(self.create_fake_app('foo.bar.baz.models')))
...
class TestUtils(unittest.TestCase):
    ...
    def test_depends(self):
        ...
        graph = {'A1': [], 'A2': [
           ...
           'B1': []}
        self.assertEqual([
         'B1', 'A1', 'A2', 'A3'], depends('A3', lambda n: graph[n]))
        graph = {'A1': [], 'A2': [
                'A1'], 
           'A3': [
                'A2', 'A1', 'B2'], 
           'B1': [], 'B2': [
                'B1']}
        self.assertEqual([
         'B1', 'B2', 'A1', 'A2', 'A3'], depends('A3', lambda n: graph[n]))
        graph = {'A1': [], 'A2': [
                'A1', 'B1'], 
           'A3': [
                'A2'], 
        ...
        graph = {'A1': [], 'A2': [
           ...
           'C1': [
                'B1']}
        self.assertEqual([
         'B1', 'C1', 'B2', 'A1', 'A2', 'A3'], depends('A3', lambda n: graph[n]))
        graph = {'A1': [], 'A2': [
                'A1'], 
           'A3': [
                'A2', 'B2', 'A1', 'C1'], 
           ...
           'C3': [
                'C2']}
        self.assertEqual([
         'A1', 'B1', 'C1', 'C2', 'B2', 'A2', 'A3'], depends('A3', lambda n: graph[n]))
