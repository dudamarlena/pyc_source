...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/South/south/tests/db_mysql.py
...
class TestMySQLOperations(with_metaclass(TestMySQLOperationsMeta, unittest.TestCase)):
    ...
    def _create_foreign_tables(self, main_name, reference_name):
        Foreign = db.mock_model(model_name='Foreign', db_table=reference_name, db_tablespace='', pk_field_name='id', pk_field_type=models.AutoField, pk_field_args=[])
    ...
    def test_rename_constrained_table(self):
        ...
        self.assertEquals(len(constraints), 1)
        rtable, rcolumn = db._lookup_constraint_references(renamed_table, constraints[0])
    ...
    def test_renamed_referenced_table(self):
        ...
        constraints = db._find_foreign_constraints(main_table, 'foreign_id')
        self.assertEquals(len(constraints), 1)
        rtable, rcolumn = db._lookup_constraint_references(main_table, constraints[0])
