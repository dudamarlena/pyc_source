...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/django/django/forms/forms.py
...
__all__ = (
 b'BaseForm', b'Form')
NON_FIELD_ERRORS = b'__all__'

def pretty_name(name):
    """Converts 'first_name' to 'First name'"""
    if not name:
        return b''
    return name.replace(b'_', b' ').capitalize()
...
class DeclarativeFieldsMetaclass(type):
    ...
    def __new__(cls, name, bases, attrs):
        attrs[b'base_fields'] = get_declared_fields(bases, attrs)
...
class BaseForm(object):
    ...
    def __init__(self, data=None, files=None, auto_id=b'id_%s', prefix=None, initial=None, error_class=ErrorList, label_suffix=b':', empty_permitted=False):
    ...
    def __getitem__(self, name):
        ...
        try:
            ...
            raise KeyError(b'Key %r not found in Form' % name)
    ...
    def add_prefix(self, field_name):
        ...
        """
        return self.prefix and b'%s-%s' % (self.prefix, field_name) or field_name
    ...
    def _html_output(self, normal_row, error_row, row_ender, help_text_html, errors_on_separate_row):
        ...
        for name, field in self.fields.items():
            ...
            if bf.is_hidden:
                if bf_errors:
                    top_errors.extend([ b'(Hidden field %s) %s' % (name, force_text(e)) for e in bf_errors ])
                ...
                if bf.label:
                    ...
                    if self.label_suffix:
                        if label[(-1)] not in b':?.!':
                            label = format_html(b'{0}{1}', label, self.label_suffix)
        ...
        if hidden_fields:
            ...
            if output:
                ...
                if not last_row.endswith(row_ender):
                    last_row = normal_row % {b'errors': b'', b'label': b'', b'field': b'', 
                       b'help_text': b'', b'html_class_attr': html_class_attr}
        ...
        if self._changed_data is None:
            ...
            for name, field in self.fields.items():
                ...
                if not field.show_hidden_initial:
                    ...
                    initial_value = hidden_widget.value_from_datadict(self.data, self.files, initial_prefixed_name)
...
class BoundField(object):
    ...
    def label_tag(self, contents=None, attrs=None):
        ...
        if id_:
            attrs = attrs and flatatt(attrs) or b''
            contents = format_html(b'<label for="{0}"{1}>{2}</label>', widget.id_for_label(id_), attrs, contents)
        else:
    ...
    def _auto_id(self):
        ...
        if auto_id and b'%s' in smart_text(auto_id):
