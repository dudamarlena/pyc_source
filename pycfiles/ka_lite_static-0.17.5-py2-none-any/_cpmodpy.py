...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/cherrypy/cherrypy/_cpmodpy.py
...
def setup(req):
    ...
    cherrypy.config.update({'log.screen': False, 'tools.ignore_headers.on': True, 
       'tools.ignore_headers.headers': [
                                      'Range']})
...
def read_process(cmd, args=''):
    ...
    try:
        ...
        cmd_not_found = re.search(ntob('(not recognized|No such file|not found)'), firstline, re.IGNORECASE)
        if cmd_not_found:
...
class ModPythonServer(object):
    ...
    def start(self):
        ...
        conf_data = self.template % {'port': self.port, 'loc': self.loc, 
           'opts': opts, 
