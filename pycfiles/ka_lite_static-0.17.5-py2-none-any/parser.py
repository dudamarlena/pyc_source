...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/slimit/slimit/parser.py
...
class Parser(object):
    ...
    def __init__(self, lex_optimize=True, lextab=lextab, yacc_optimize=True, yacctab=yacctab, yacc_debug=False):
        ...
        self.parser = ply.yacc.yacc(module=self, optimize=yacc_optimize, debug=yacc_debug, tabmodule=yacctab, start='program')
        self._error_tokens = {}
    ...
    def _raise_syntax_error(self, token):
        raise SyntaxError('Unexpected token (%s, %r) at %s:%s between %s and %s' % (
         token.type, token.value, token.lineno, token.lexpos,
    ...
    def p_property_assignment(self, p):
        ...
        if len(p) == 4:
            ...
            p[0] = ast.SetPropAssign(prop_name=p[2], parameters=p[4], elements=p[7])
    ...
    def p_conditional_expr(self, p):
        ...
        if len(p) == 2:
            ...
            p[0] = ast.Conditional(predicate=p[1], consequent=p[3], alternative=p[5])
    ...
    def p_iteration_statement_6(self, p):
        ...
        """
        p[0] = ast.ForIn(item=ast.VarDecl(identifier=p[4], initializer=p[5]), iterable=p[7], statement=p[9])
    ...
    def p_function_declaration(self, p):
        ...
        """
        if len(p) == 8:
            p[0] = ast.FuncDecl(identifier=p[2], parameters=None, elements=p[6])
        else:
            p[0] = ast.FuncDecl(identifier=p[2], parameters=p[4], elements=p[7])
        return
    ...
    def p_function_expr_1(self, p):
        ...
        """
        if len(p) == 7:
            p[0] = ast.FuncExpr(identifier=None, parameters=None, elements=p[5])
        else:
            p[0] = ast.FuncExpr(identifier=None, parameters=p[3], elements=p[6])
        return
    ...
    def p_function_expr_2(self, p):
        ...
        """
        if len(p) == 8:
            p[0] = ast.FuncExpr(identifier=p[2], parameters=None, elements=p[6])
        else:
            p[0] = ast.FuncExpr(identifier=p[2], parameters=p[4], elements=p[7])
        return
