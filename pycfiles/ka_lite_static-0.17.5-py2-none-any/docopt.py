...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/docopt/docopt.py
    ...
    def fix_identities(self, uniq=None):
        ...
        if not hasattr(self, 'children'):
            ...
            for i, c in enumerate(self.children):
                if not (hasattr(c, 'children') or c in uniq):
...
def parse_pattern(source, options):
    tokens = TokenStream(re.sub('([\\[\\]\\(\\)\\|]|\\.\\.\\.)', ' \\1 ', source), DocoptLanguageError)
...
def printable_usage(doc):
    usage_split = re.split('([Uu][Ss][Aa][Gg][Ee]:)', doc)
