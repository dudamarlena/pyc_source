...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/peewee/playhouse/djpeewee.py
...
class DjangoTranslator(object):
    ...
    def get_django_field_map(self):
        ...
        return [
         ...
         (
          djf.DateTimeField, DateTimeField),
         (
    ...
    def _translate_model(self, model, mapping, max_depth=None, backrefs=False, exclude=None):
        if exclude and model in exclude:
            ...
            attrs = {}
            field_key = lambda field: (
             field.null and 1 or 0, field)
                ...
                if converted is ForeignKeyField:
                    if max_depth != 0:
                        ...
                        if model_name not in mapping:
                            mapping[model_name] = None
                            self._translate_model(related_model, mapping, max_depth=max_depth - 1, backrefs=backrefs, exclude=exclude)
                        if mapping[model_name] is None:
                            logger.warn('Cycle detected: %s: %s', model_field.name, model_name)
                            attrs[model_field.name] = IntegerField(db_column=model_field.column)
                        else:
                            related_name = model_field.rel.related_name or model_field.related_query_name()
                            if related_name.endswith('+'):
                                related_name = '__%s:%s:%s' % (
                                 options,
                                 model_field.name,
                                 related_name.strip('+'))
                            attrs[model_field.name] = ForeignKeyField(mapping[model_name], related_name=related_name, db_column=model_field.column)
                    else:
                        attrs[model_field.name] = IntegerField(db_column=model_field.column)
                elif converted:
            ...
            if backrefs:
                for rel_obj in options.get_all_related_objects():
                    if rel_obj.model._meta.object_name in mapping:
                        continue
                    self._translate_model(rel_obj.model, mapping, max_depth=max_depth - 1, backrefs=backrefs, exclude=exclude)

            for many_to_many in options.many_to_many:
                if not isinstance(many_to_many, djf.related.ManyToManyField):
                    continue
                self._translate_model(many_to_many.rel.through, mapping, max_depth=max_depth, backrefs=backrefs, exclude=exclude)
    ...
    def translate_models(self, *models, **options):
        ...
        if not recurse and max_depth:
            raise ValueError('Error, you cannot specify a max_depth when recurse=False.')
        else:
            if not recurse:
                max_depth = 0
                ...
                max_depth = -1
            for model in models:
                self._translate_model(model, mapping, max_depth=max_depth, backrefs=backrefs, exclude=exclude)
