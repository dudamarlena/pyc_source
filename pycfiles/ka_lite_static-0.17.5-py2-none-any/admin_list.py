...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/django/django/contrib/admin/templatetags/admin_list.py
...
def date_hierarchy(cl):
    ...
    if cl.date_hierarchy:
        ...
        if year_lookup and month_lookup:
            days = cl.query_set.filter(**{year_field: year_lookup, month_field: month_lookup}).dates(field_name, b'day')
            return {b'show': True, 
        ...
        if year_lookup:
            months = cl.query_set.filter(**{year_field: year_lookup}).dates(field_name, b'month')
            return {b'show': True, 
               b'back': {b'link': link({}), 
        ...
        years = cl.query_set.dates(field_name, b'year')
        return {b'show': True, 
