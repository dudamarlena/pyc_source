...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/cherrypy/cherrypy/test/webtest.py
...
"""
import pprint, re, socket, sys, time, traceback, types
...
class ReloadingTestLoader(TestLoader):
    ...
    def loadTestsFromName(self, name, module=None):
        ...
        if isinstance(obj, types.ModuleType):
            ...
            if (py3k and isinstance(obj, type) or isinstance(obj, (type, types.ClassType))) and issubclass(obj, TestCase):
            ...
            if isinstance(obj, types.UnboundMethodType):
                ...
                if hasattr(obj, '__call__'):
                    ...
                    if not isinstance(test, TestCase) and not isinstance(test, TestSuite):
                        raise ValueError('calling %s returned %s, not a test' % (
                         obj, test))
                    return test
