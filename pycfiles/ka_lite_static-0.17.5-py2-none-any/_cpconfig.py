...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/cherrypy/cherrypy/_cpconfig.py
...
Config.environments = environments = {'staging': {'engine.autoreload.on': False, 
               'checker.on': False, 
               'tools.log_headers.on': False, 
               'request.show_tracebacks': False, 
               'request.show_mismatched_params': False}, 
   'production': {'engine.autoreload.on': False, 
                  'checker.on': False, 
                  'tools.log_headers.on': False, 
                  'request.show_tracebacks': False, 
                  'request.show_mismatched_params': False, 
                  'log.screen': False}, 
   'embedded': {'engine.autoreload.on': False, 
                'checker.on': False, 
                'tools.log_headers.on': False, 
                'request.show_tracebacks': False, 
                'request.show_mismatched_params': False, 
                ...
                'engine.SIGHUP': None, 
                'engine.SIGTERM': None}, 
   'test_suite': {'engine.autoreload.on': False, 
                  'checker.on': False, 
                  'tools.log_headers.on': False, 
...
def _engine_namespace_handler(k, v):
    ...
    deprecated = {'autoreload_on': 'autoreload.on', 
       'autoreload_frequency': 'autoreload.frequency', 
       'autoreload_match': 'autoreload.match', 
       'reload_files': 'autoreload.files', 
       'deadlock_poll_freq': 'timeout_monitor.frequency'}
    if k in deprecated:
        engine.log('WARNING: Use of engine.%s is deprecated and will be removed in a future version. Use engine.%s instead.' % (
         k, deprecated[k]))
    if k == 'autoreload_on':
        if v:
