...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/django-tastypie-legacy/tastypie/test.py
...
class TestApiClient(object):
    ...
    def get_content_type(self, short_format):
        ...
        this case).
        """
        return self.serializer.content_types.get(short_format, b'json')

    def get(self, uri, format=b'json', data=None, authentication=None, **kwargs):
        """
...
https://docs.djangoproject.com/en/dev/topics/testing/#module-django.test.client
        ...
        kwargs[b'HTTP_ACCEPT'] = content_type
        if data is not None:
            kwargs[b'data'] = data
        if authentication is not None:
            kwargs[b'HTTP_AUTHORIZATION'] = authentication
        return self.client.get(uri, **kwargs)

    def post(self, uri, format=b'json', data=None, authentication=None, **kwargs):
        """
...
https://docs.djangoproject.com/en/dev/topics/testing/#module-django.test.client
        ...
        """
        content_type = self.get_content_type(format)
        kwargs[b'content_type'] = content_type
        if data is not None:
            kwargs[b'data'] = self.serializer.serialize(data, format=content_type)
        if authentication is not None:
            kwargs[b'HTTP_AUTHORIZATION'] = authentication
        return self.client.post(uri, **kwargs)

    def put(self, uri, format=b'json', data=None, authentication=None, **kwargs):
        """
...
https://docs.djangoproject.com/en/dev/topics/testing/#module-django.test.client
    ...
    def patch(self, uri, format=b'json', data=None, authentication=None, **kwargs):
        """
...
https://docs.djangoproject.com/en/dev/topics/testing/#module-django.test.client
        ...
        if authentication is not None:
            kwargs[b'HTTP_AUTHORIZATION'] = authentication
        parsed = urlparse(uri)
        r = {b'CONTENT_LENGTH': len(kwargs[b'data']), 
           b'CONTENT_TYPE': content_type, 
           b'PATH_INFO': self.client._get_path(parsed), 
           b'QUERY_STRING': parsed[4], 
           b'REQUEST_METHOD': b'PATCH', 
           b'wsgi.input': FakePayload(kwargs[b'data'])}
        r.update(kwargs)
        return self.client.request(**r)

    def delete(self, uri, format=b'json', data=None, authentication=None, **kwargs):
        """
...
class ResourceTestCase(TestCase):
    ...
    def get_credentials(self):
        ...
        """
        raise NotImplementedError(b'You must return the class for your Resource to test.')
    ...
    def create_basic(self, username, password):
        ...
        """
        import base64
        return b'Basic %s' % base64.b64encode((b':').join([username, password]).encode(b'utf-8')).decode(b'utf-8')
    ...
    def create_apikey(self, username, api_key):
        ...
        """
        return b'ApiKey %s:%s' % (username, api_key)
    ...
    def create_digest(self, username, api_key, method, uri):
        ...
        new_uuid = uuid.uuid4()
        opaque = hmac.new(str(new_uuid).encode(b'utf-8'), digestmod=sha1).hexdigest().decode(b'utf-8')
        return python_digest.build_authorization_request(username, method.upper(), uri, 1, digest_challenge=python_digest.build_digest_challenge(time.time(), getattr(settings, b'SECRET_KEY', b''), b'django-tastypie', opaque, False), password=api_key)
    ...
    def create_oauth(self, user):
        ...
        resource, _ = Resource.objects.get_or_create(url=b'test', defaults={b'name': b'Test Resource'})
        consumer, _ = Consumer.objects.get_or_create(key=b'123', defaults={b'name': b'Test', 
           b'description': b'Testing...'})
        token, _ = Token.objects.get_or_create(key=b'foo', token_type=Token.ACCESS, defaults={b'consumer': consumer, 
           b'resource': resource, 
           b'secret': b'', 
           b'user': user})
        oauth_data = {b'oauth_consumer_key': b'123', 
           b'oauth_nonce': b'abc', 
           b'oauth_signature': b'&', 
           b'oauth_signature_method': b'PLAINTEXT', 
           b'oauth_timestamp': str(int(time.time())), 
           b'oauth_token': b'foo'}
        return b'OAuth %s' % (b',').join([ key + b'=' + value for key, value in oauth_data.items() ])
    ...
    def assertHttpAccepted(self, resp):
        ...
        self.assertNotIn(b'Content-Type', resp)
    ...
    def assertValidJSONResponse(self, resp):
        ...
        * The content is valid JSON
        ...
        self.assertTrue(resp[b'Content-Type'].startswith(b'application/json'))
    ...
    def assertValidXMLResponse(self, resp):
        ...
        * The content is valid XML
        """
        self.assertHttpOK(resp)
        self.assertTrue(resp[b'Content-Type'].startswith(b'application/xml'))
    ...
    def assertValidYAMLResponse(self, resp):
        ...
        * The content is valid YAML
        """
        self.assertHttpOK(resp)
        self.assertTrue(resp[b'Content-Type'].startswith(b'text/yaml'))
    ...
    def assertValidPlistResponse(self, resp):
        ...
        * The content is valid binary plist data
        """
        self.assertHttpOK(resp)
        self.assertTrue(resp[b'Content-Type'].startswith(b'application/x-plist'))
    ...
    def deserialize(self, resp):
        ...
        serialized data.
        """
        return self.serializer.deserialize(resp.content, format=resp[b'Content-Type'])

    def serialize(self, data, format=b'application/json'):
        """
