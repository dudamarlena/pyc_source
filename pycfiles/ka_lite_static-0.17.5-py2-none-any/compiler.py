...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/django/django/db/models/sql/compiler.py
            ...
            for field in ordering:
                ...
                if '.' in field:
                    ...
                    for table, col, order in self.find_ordering_name(field, self.query.model._meta, default_order=asc):
                        if (
                         table, col) not in processed_pairs:
        ...
        if not restricted and self.query.max_depth and cur_depth > self.query.max_depth:
            ...
            for f, model in opts.get_fields_with_model():
                ...
                if model:
                    ...
                    for int_model in opts.get_base_chain(model):
                        ...
                        alias = self.query.join((alias, int_opts.db_table, lhs_col,
                         int_opts.pk.column), exclusions=used, promote=promote)
                ...
                alias = self.query.join((alias, table, f.column,
                 f.rel.get_related_field().column), exclusions=used.union(avoid), promote=promote)
                used.add(alias)
                    ...
                    if chain is not None:
                        for int_model in chain:
                            ...
                            alias = self.query.join((
                             alias, int_opts.db_table, lhs_col, int_opts.pk.column), exclusions=used, promote=True, reuse=used)
                            alias_chain.append(alias)
                            for dupe_opts, dupe_col in dupe_set:
                                self.query.update_dupe_avoidance(dupe_opts, dupe_col, alias)
                        ...
                        if dupe_set or dedupe:
                            ...
                            if dedupe:
                                dupe_set.add((opts, f.column))
                    alias = self.query.join((
                     alias, table, f.rel.get_related_field().column, f.column), exclusions=used.union(avoid), promote=True)
                    used.add(alias)
            ...
            for row in rows:
                ...
                if resolve_columns:
                    if fields is None:
                        ...
                        only_load = self.deferred_to_columns()
                        if only_load:
                            fields = [ f for f in fields if f.model._meta.db_table not in only_load or f.column in only_load[f.model._meta.db_table] ]
                        if has_aggregate_select:
                            fields = fields[:aggregate_start] + [ None for x in range(0, aggregate_end - aggregate_start) ] + fields[aggregate_start:]
...
class SQLInsertCompiler(SQLCompiler):
    ...
    def as_sql(self):
        ...
        if has_fields:
            params = values = [ [ f.get_db_prep_save(getattr(obj, f.attname) if self.query.raw else f.pre_save(obj, True), connection=self.connection) for f in fields ] for obj in self.query.objs
                              ]
        else:
            ...
            fields = [
             None]
        can_bulk = not any(hasattr(field, 'get_placeholder') for field in fields) and not self.return_id and self.connection.features.has_bulk_insert
        if can_bulk:
            placeholders = [
             [
              '%s'] * len(fields)]
    ...
    def execute_sql(self, return_id=False):
        if not not (return_id and len(self.query.objs) != 1):
            ...
            return return_id and cursor or None
    ...
    def as_sql(self):
        ...
        if not self.query.values:
            ...
            if where:
                result.append('WHERE %s' % where)
            return (
             (' ').join(result), tuple(update_params + params))
...
class SQLAggregateCompiler(SQLCompiler):
    ...
    def as_sql(self, qn=None):
        ...
        sql = 'SELECT %s FROM (%s) subquery' % (
         (', ').join([ aggregate.as_sql(qn, self.connection) for aggregate in self.query.aggregate_select.values()
            ]),
         self.query.subquery)
