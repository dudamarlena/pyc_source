...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/cherrypy/cherrypy/test/test_conn.py
...
def setup_server():
    ...
    class Root:
        ...
        def upload(self):
            assert cherrypy.request.method == 'POST', "'POST' != request.method %r" % cherrypy.request.method
...
class ConnectionCloseTests(helper.CPWebCase):
    ...
    def _streaming(self, set_cl):
        if cherrypy.server.protocol_version == 'HTTP/1.1':
            ...
            if set_cl:
                self.getPage('/stream?set_cl=Yes', headers=[
