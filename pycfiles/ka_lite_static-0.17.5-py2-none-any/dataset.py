...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/peewee/playhouse/dataset.py
...
class DataSet(object):
    ...
    def update_cache(self, table=None):
        if table:
            ...
            dependencies = None
        updated = self._introspector.generate_models(skip_invalid=True, table_names=[ related._meta.db_table for related in dependencies ])
    ...
    def thaw(self, table, format='csv', filename=None, file_obj=None, strict=False, **kwargs):
        self._check_arguments(filename, file_obj, format, self._export_formats)
        if filename:
...
class Table(object):
    ...
    def _create_model(self):
        ...
        return type(str(self.name), (
         self.dataset._base_model,), {'Meta': Meta})
    ...
    def update(self, columns=None, conjunction=None, **data):
        ...
        return self._apply_where(self.model_class.update(**data), filters, conjunction).execute()
...
class JSONExporter(Exporter):
    ...
    def export(self, file_obj, **kwargs):
        json.dump(list(self.query), file_obj, default=JSONExporter.default, **kwargs)
