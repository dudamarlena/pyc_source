...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/slimit/slimit/visitors/ecmavisitor.py
...
class ECMAVisitor(object):
    ...
    def visit_Block(self, node):
        s = '{\n'
        self.indent_level += 2
        s += ('\n').join(self._make_indent() + self.visit(child) for child in node)
        self.indent_level -= 2
        s += '\n' + self._make_indent() + '}'
        return s
    ...
    def visit_GetPropAssign(self, node):
        ...
        if getattr(node, '_parens', False):
            template = '(%s)' % template
        self.indent_level += 2
        body = ('\n').join(self._make_indent() + self.visit(el) for el in node.elements)
        self.indent_level -= 2
    ...
    def visit_Case(self, node):
        s = 'case %s:\n' % self.visit(node.expr)
        self.indent_level += 2
        elements = ('\n').join(self._make_indent() + self.visit(element) for element in node.elements)
        if elements:
            s += elements + '\n'
        self.indent_level -= 2
        return s
    ...
    def visit_Object(self, node):
        s = '{\n'
        self.indent_level += 2
        s += (',\n').join(self._make_indent() + self.visit(prop) for prop in node.properties)
        self.indent_level -= 2
        if node.properties:
            s += '\n'
        s += self._make_indent() + '}'
