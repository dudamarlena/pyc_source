...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/cherrypy/cherrypy/test/test_request_obj.py
...
defined_http_methods = ('OPTIONS', 'GET', 'HEAD', 'POST', 'PUT', 'DELETE', 'TRACE',
                        'PROPFIND')
...
class RequestObjectTests(helper.CPWebCase):
    ...
    def testParams(self):
        ...
        self.assertBody('args: %s kwargs: %s' % (
         ...
         [
          (
           b'Gruy\xe8re', ntou(b'Bulgn\xe9ville'))]))
    ...
    def testParamErrors(self):
        for uri in ('/paramerrors/one_positional?param1=foo', '/paramerrors/one_positional_args?param1=foo',
                    '/paramerrors/one_positional_args/foo', '/paramerrors/one_positional_args/foo/bar/baz',
                    ...
                    '/paramerrors/no_positional', '/paramerrors/no_positional_args/foo',
                    '/paramerrors/no_positional_args/foo/bar/baz', '/paramerrors/no_positional_args_kwargs?param1=foo&param2=bar',
                    '/paramerrors/no_positional_args_kwargs/foo?param2=bar', '/paramerrors/no_positional_args_kwargs/foo/bar/baz?param2=bar&param3=baz',
                    '/paramerrors/no_positional_kwargs?param1=foo&param2=bar', '/paramerrors/callable_object'):
            self.getPage(uri)
        ...
        for uri in ('/paramerrors/raise_type_error', '/paramerrors/raise_type_error_with_default_param?x=0',
    ...
    def testHeaderElements(self):
        ...
        self.assertStatus('200 OK')
        self.assertBody('da\nen-gb;q=0.8\nen;q=0.7')
        self.getPage('/headerelements/get_elements?headername=Content-Type', headers=[
         ('Content-Type', 'text/html; charset=utf-8;')])
        self.assertStatus(200)
        self.assertBody('text/html;charset=utf-8')

    def test_repeated_headers(self):
        self.getPage('/headers/Accept-Charset', headers=[
    ...
    def test_encoded_headers(self):
        self.assertEqual(httputil.decode_TEXT(ntou('=?utf-8?q?f=C3=BCr?=')), ntou(b'f\xfcr'))
        if cherrypy.server.protocol_version == 'HTTP/1.1':
            u = ntou(b'\\u212bngstr\xf6m', 'escape')
            c = ntou('=E2=84=ABngstr=C3=B6m')
            self.getPage('/headers/ifmatch', [
             (
              'If-Match', ntou('=?utf-8?q?%s?=') % c)])
            self.assertBody(ntob('Ångström'))
            self.assertHeader('ETag', ntou('=?utf-8?b?4oSrbmdzdHLDtm0=?='))
            self.getPage('/headers/ifmatch', [
             (
              'If-Match', ntou('=?utf-8?q?%s?=') % (c * 10))])
            self.assertBody(ntob('Ångström') * 10)
