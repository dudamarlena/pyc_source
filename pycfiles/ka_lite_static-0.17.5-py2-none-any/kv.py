...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/peewee/playhouse/kv.py
...
class KeyStore(object):
    ...
    def _upsert(self, key, value):
        self.model.insert(**{self.key.name: key, 
    ...
    def __setitem__(self, node, value):
        if isinstance(node, Node):
            ...
            try:
                ...
                self.model.update(**{self.value.name: value}).where(self.key == node).execute()
