...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/django/django/contrib/admin/filters.py
...
class RelatedFieldListFilter(FieldListFilter):
    ...
    def choices(self, cl):
        ...
        for pk_val, val in self.lookup_choices:
            yield {'selected': self.lookup_val == smart_text(pk_val), 
               'query_string': cl.get_query_string({self.lookup_kwarg: pk_val}, [
                              self.lookup_kwarg_isnull]), 
               'display': val}

        if isinstance(self.field, models.related.RelatedObject) and self.field.field.null or hasattr(self.field, 'rel') and self.field.null:
            yield {'selected': bool(self.lookup_val_isnull), 'query_string': cl.get_query_string({self.lookup_kwarg_isnull: 'True'}, [
                              self.lookup_kwarg]), 
               'display': EMPTY_CHANGELIST_VALUE}
        return


FieldListFilter.register(lambda f: hasattr(f, 'rel') and bool(f.rel) or isinstance(f, models.related.RelatedObject), RelatedFieldListFilter)
...
class BooleanFieldListFilter(FieldListFilter):
    ...
    def choices(self, cl):
        for lookup, title in (
         ...
         (
          '0', _('No'))):
            yield {'selected': self.lookup_val == lookup and not self.lookup_val2, 'query_string': cl.get_query_string({self.lookup_kwarg: lookup}, [
                              self.lookup_kwarg2]), 
               'display': title}

        if isinstance(self.field, models.NullBooleanField):
            yield {'selected': self.lookup_val2 == 'True', 'query_string': cl.get_query_string({self.lookup_kwarg2: 'True'}, [
                              self.lookup_kwarg]), 
               'display': _('Unknown')}
        return
...
class ChoicesFieldListFilter(FieldListFilter):
    ...
    def choices(self, cl):
        ...
        for lookup, title in self.field.flatchoices:
            yield {'selected': smart_text(lookup) == self.lookup_val, 
