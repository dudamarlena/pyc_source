...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/peewee/playhouse/csv_utils.py
...
class Loader(_CSVReader):
    ...
    def __init__(self, db_or_model, file_or_name, fields=None, field_names=None, has_header=True, sample_size=10, converter=None, db_table=None, pk_in_csv=False, **reader_kwargs):
        ...
        if isinstance(db_or_model, Database):
            ...
            self.model = None
            self.db_table = db_table or os.path.splitext(os.path.basename(self.filename))[0]
        else:
    ...
    def get_converter(self):
        return self.converter or RowConverter(self.database, has_header=self.has_header, sample_size=self.sample_size)

    def analyze_csv(self):
        converter = self.get_converter()
        header, rows = converter.extract_rows(self.file_or_name, **self.reader_kwargs)
        if rows:
    ...
    def load(self):
        ...
        if not self.field_names and not self.has_header:
            self.field_names = [ 'field_%d' % i for i in range(len(self.fields)) ]
...
def load_csv(db_or_model, file_or_name, fields=None, field_names=None, has_header=True, sample_size=10, converter=None, db_table=None, **reader_kwargs):
    loader = Loader(db_or_model, file_or_name, fields, field_names, has_header, sample_size, converter, db_table, **reader_kwargs)
    return loader.load()
