...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/python-dateutil/dateutil/relativedelta.py
...
7. If the 'weekday' argument is present, calculate the weekday,
   ...
   (0, 1) or (0, -1) won't change the day.
    ...
    def __add__(self, other):
        if isinstance(other, relativedelta):
            ...
            if not isinstance(other, datetime.date):
                ...
                if not (self.months and 1 <= abs(self.months) <= 12):
    ...
    def __mul__(self, other):
        f = float(other)
        return relativedelta(years=int(self.years * f), months=int(self.months * f), days=int(self.days * f), hours=int(self.hours * f), minutes=int(self.minutes * f), seconds=int(self.seconds * f), microseconds=int(self.microseconds * f), leapdays=self.leapdays, year=self.year, month=self.month, day=self.day, weekday=self.weekday, hour=self.hour, minute=self.minute, second=self.second, microsecond=self.microsecond)
