...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/cherrypy/cherrypy/_cpchecker.py
# Compiled at: 2018-07-11 18:15:31
import os, warnings, cherrypy
        ...
        for sn, app in cherrypy.tree.apps.items():
            ...
            for key in app.config.keys():
                ...
                if key_atoms[:len(sn_atoms)] == sn_atoms:
                    warnings.warn('The application mounted at %r has config entries that start with its script name: %r' % (
                     sn,
                     key))
        ...
        for sn, app in cherrypy.tree.apps.items():
            ...
            for key in app.config.keys():
                if key.startswith('[') or key.endswith(']'):
                    warnings.warn('The application mounted at %r has config section names with extraneous brackets: %r. Config *files* need brackets; config *dicts* (e.g. passed to tree.mount) do not.' % (
                     sn, key))
                        ...
                        if os.path.isabs(dir):
                            ...
                            if root:
                                ...
                                if os.path.exists(testdir):
                                    msg += '\nIf you meant to serve the filesystem folder at %r, remove the leading slash from dir.' % (
                                     testdir,)
                        elif not root:
