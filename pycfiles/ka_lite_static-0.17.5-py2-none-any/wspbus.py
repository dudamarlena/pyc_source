...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/cherrypy/cherrypy/process/wspbus.py
...
"""
import atexit, os, sys, threading, time, traceback as _traceback, warnings
...
class Bus(object):
    ...
    def __init__(self):
        ...
        self.listeners = dict([ (channel, set()) for channel in ('start', 'stop', 'exit',
                                                                 'graceful', 'log',
                                                                 'main')
                              ])
        self._priorities = {}
    ...
    def block(self, interval=0.1):
        ...
        for t in threading.enumerate():
            if t != threading.currentThread() and t.isAlive() and not isinstance(t, threading._MainThread):
