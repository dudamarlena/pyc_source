...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-jkXn_D/South/south/db/mysql.py
...
def copy_column_constraints(func):
    ...
    def _column_cp(self, table_name, column_old, column_new, *args, **opts):
        try:
            ...
            if refs is not None:
                ...
                if ftable and fcolumn:
                    fk_sql = self.foreign_key_sql(table_name, column_new, ftable, fcolumn)
        ...
        try:
            ...
            for cname, rtable, rcolumn in reverse:
                fk_sql = self.foreign_key_sql(rtable, rcolumn, table_name, column_new)
                self.add_deferred_sql(fk_sql)
...
class DatabaseOperations(generic.DatabaseOperations):
            ...
            for column_set in cols:
                ...
                if kind == 'FOREIGN KEY':
                    self._constraint_cache[db_name][table][column].add((kind,
                     constraint))
                    self._constraint_references[db_name][(table, constraint)] = (
                     ref_table, ref_column)
                    self._reverse_cache[db_name].setdefault(ref_table, {})
                    self._reverse_cache[db_name][ref_table].setdefault(ref_column, set())
    ...
    def rename_table(self, old_table_name, table_name):
        super(DatabaseOperations, self).rename_table(old_table_name, table_name)
