...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/geckopush/geckopush.py
# Compiled at: 2015-11-18 12:30:38
"""
...
class Dashboard(object):
    ...
    def __repr__(self):
        return ('<DASHBOARD OBJECT, API KEY: {api_key}>').format(api_key=self.api_key)
...
class Widget(object):
    ...
    def __str__(self):
        print ('Dashboard: {}, Widget_Key: {}').format(self.dashboard, self.widget_key)

    def __repr__(self):
        print ('<geckopush Object (Dasboard: {}; Widget Key: {}>').format(self.dashboard, self.widget_key)
    ...
    def push(self):
        self._assemble_data()
        _api_endpoint = ('https://push.geckoboard.com/v1/send/{widget_key}').format(widget_key=self.widget_key)
        _payload_json = json.dumps(self.payload).encode('utf-8')
        _request = urllib.request.Request(url=_api_endpoint, data=_payload_json)
        ...
        try:
            ...
            print ('API Success ({}): {}').format(_name_truncated, _api_status['success'])
            return _api_status
...
class BarChart(Widget):
    ...
    def __init__(self, data=None, x_axis_labels=None, x_axis_type=None, y_axis_format=None, y_axis_unit=None, *args, **kwargs):
    ...
    def add(self, x_axis_labels=None, x_axis_type=None, y_axis_format=None, y_axis_unit=None):
    ...
    def _assemble_data(self):
        ...
        _data = {'x_axis': {}, 'y_axis': {}, 'series': self.data}
...
class BulletGraph(Widget):
    ...
    def __init__(self, orientation=None, label=None, axis=None, red_start=None, red_end=None, amber_start=None, amber_end=None, green_start=None, green_end=None, measure_start=None, measure_end=None, projected_start=None, projected_end=None, comparative=None, sublabel=None, *args, **kwargs):
        super(BulletGraph, self).__init__(*args, **kwargs)
        self.orientation = orientation
        self.data = []
        _necessary = [
         label, axis, red_start, red_end, amber_start, amber_end,
    ...
    def add_data(self, label, axis, red_start, red_end, amber_start, amber_end, green_start, green_end, measure_start, measure_end, projected_start, projected_end, comparative, sublabel=None):
        if len(self.data) >= 4:
            raise GeckoboardException('Bullet Graphs support a maximum of 4 multiples.')
        self._all_or_none(label, axis, red_start, red_end, amber_start, amber_end, green_start, green_end, measure_start, measure_end, projected_start, projected_end, comparative)
        _item_payload = {'label': label, 
           'axis': {'point': axis}, 
           'range': {'red': {'start': red_start, 
                             'end': red_end}, 
                     'amber': {'start': amber_start, 
                               'end': amber_end}, 
                     'green': {'start': green_start, 
                               'end': green_end}}, 
           'measure': {'current': {'start': measure_start, 
                                   'end': measure_end}, 
                       'projected': {'start': projected_start, 
                                     'end': projected_end}}, 
           'comparative': {'point': comparative}}
    ...
    @staticmethod
    def _all_or_none(label, axis, red_start, red_end, amber_start, amber_end, green_start, green_end, measure_start, measure_end, projected_start, projected_end, comparative):
        """
    ...
    def _assemble_data(self):
        _data = {'orientation': self.orientation, 
           'item': None}
        _item = self.data
        if len(_item) == 1:
            _data['item'] = _item[0]
...
class Funnel(Widget):
    ...
    def __init__(self, value=None, label=None, funnel_type=None, percentage=None, *args, **kwargs):
...
class GeckoMeter(Widget):
    ...
    def __init__(self, item=None, min_value=None, max_value=None, *args, **kwargs):
    ...
    def add_data(self, item, min_value, max_value):
        if self.item is not None or self.min_value is not None or self.max_value is not None:
    ...
    def _assemble_data(self):
        if self.item is None or self.min_value is None or self.max_value is None:
            raise GeckoboardException('Widget missing required data.')
...
class HighCharts(Widget):
    ...
    def _assemble_data(self):
        ...
        if self.highchart is None:
            raise GeckoboardException('Widget missing required data.')
        _data = {'highchart': self.highchart}
        self._assemble_payload(_data)
        return
...
class Leaderboard(Widget):
    ...
    def __init__(self, label=None, value=None, previous_rank=None, number_format=None, unit=None, *args, **kwargs):
    ...
    def _assemble_data(self):
        if len(self.data) > 22:
            raise GeckoboardException('Leaderboard widget accepts a max of 22 labels')
        elif len(self.data) == 0:
            raise GeckoboardException('Must add at least one value.')
        _data = {'items': self.data}
...
class LineChart(Widget):
    ...
    def __init__(self, data=None, name=None, incomplete_from=None, series_type=None, x_axis_labels=None, x_axis_type=None, y_axis_format=None, y_axis_unit=None, *args, **kwargs):
    ...
    def add_data(self, data, name=None, incomplete_from=None, series_type=None, *args, **kwargs):
        _series = {'data': data}
        if name is not None:
            _series['name'] = name
    ...
    def _assemble_data(self):
        if len(self.data) == 0:
            raise GeckoboardException('Must add at least one value.')
        self._label_data_check()
        _data = {'series': self.data, 
           'y_axis': {}, 'x_axis': {}}
        if self.x_axis_labels is not None:
            _data['x_axis']['labels'] = self.x_axis_labels
        if self.x_axis_type is not None:
...
class List(Widget):
    ...
    def __init__(self, text=None, name=None, color=None, description=None, *args, **kwargs):
...
class Map(Widget):
    ...
    def __init__(self, city_name=None, country_code=None, region_code=None, latitude=None, longitude=None, ip=None, host=None, color=None, size=None, *args, **kwargs):
        super(Map, self).__init__(*args, **kwargs)
        self.data = []
        self._data_check(city_name, country_code, region_code, latitude, longitude, ip, host)
        self.add_data(city_name, country_code, region_code, latitude, longitude, ip, host, color, size)

    @staticmethod
    def _data_check(city_name=None, country_code=None, region_code=None, latitude=None, longitude=None, ip=None, host=None):
        """
        ...
        if bool(latitude) == bool(not longitude):
            raise GeckoboardException('Widget missing required data.')
        _combos = [
         any((city_name, country_code, region_code)),
         any((latitude, longitude)),
         bool(ip),
         bool(host)]
        if sum(_combos) > 1:
            raise GeckoboardException('Too much data.  Add one point at a time.')
        return

    def add_data(self, city_name=None, country_code=None, region_code=None, latitude=None, longitude=None, ip=None, host=None, color=None, size=None, *args, **kwargs):
        self._data_check(city_name, country_code, region_code, latitude, longitude, ip, host)
        _point = {}
    ...
    def _assemble_data(self, *args, **kwargs):
        _data = {'points': {'point': self.data}}
        self._assemble_payload(_data)


class Monitoring(Widget):

    def __init__(self, status=None, downtime=None, responsetime=None, *args, **kwargs):
        super(Monitoring, self).__init__(*args, **kwargs)
        self.status = status
        self.downtime = downtime
        self.responsetime = responsetime

    def add_data(self, status, downtime=None, responsetime=None, *args, **kwargs):
...
class NumberAndSecondaryStat(Widget):
    ...
    def __init__(self, primary_value=None, secondary_value=None, text=None, prefix=None, metric_type=None, absolute=None, *args, **kwargs):
    ...
    def add_data(self, primary_value, secondary_value=None, text=None, prefix=None, *args, **kwargs):
        if secondary_value is None and text is not None:
            _item = {'value': primary_value}
            if text is not None:
                _item['text'] = text
            ...
            _item_1 = {'value': primary_value}
            self.data.append(_item_1)
            if isinstance(secondary_value, int) or isinstance(secondary_value, float):
                _item_2 = {'value': secondary_value}
            ...
            _item_1 = {'value': primary_value, 'text': text}
            self.data.append(_item_1)
            if isinstance(secondary_value, int) or isinstance(secondary_value, float):
                _item_2 = {'value': secondary_value}
                self.data.append(_item_2)
            elif isinstance(secondary_value, list):
                raise GeckoboardException('Secondary value can not be a list when both text and secondary value supplied.')
        elif secondary_value is None and text is None:
            _item = {'value': primary_value}
            self.data.append(_item)
        else:
            raise GeckoboardException('Widget accepts text or a secondary value only.')
        return
...
class PieChart(Widget):
    ...
    def _assemble_data(self, *args, **kwargs):
        _item = {'item': self.data}
        self._assemble_payload(_item)


class RAG(Widget):

    def __init__(self, text=None, value=None, prefix=None, reverse_type=None, color=None, *args, **kwargs):
    ...
    def add_data(self, text, value=None, prefix=None, color=None, *args, **kwargs):
        if len(self.data) > 3:
            raise GeckoboardException('This widget accepts a maximum of 3 items')
        _color = {'text': text}
        if value is not None:
            _color['value'] = value
        if prefix is not None:
