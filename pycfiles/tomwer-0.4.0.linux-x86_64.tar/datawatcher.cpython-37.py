...
# [GCC 8.4.0]
# Embedded file name: /users/payno/.local/share/virtualenvs/tomwer_venc/lib/python3.7/site-packages/tomwer/gui/datawatcher/datawatcher.py
# Compiled at: 2020-03-06 02:01:31
# Size of source mod 2**32: 23008 bytes
__authors__ = [
 'C. Nemoz', 'H. Payno']
__license__ = 'MIT'
__date__ = '18/02/2018'
...
class DataWatcherWidget(_DataWatcher, qt.QMainWindow):
    ...
    obsStatusToWidgetStatus = {'not processing':'Not processed', 
     'none found':'Running', 
     'starting':'Running', 
     'started':'Running', 
     ...
     'failure':'Failed'}
    _animatedStates = ('none found', 'parsing', 'waiting for acquisition ending', 'starting',
                       'started')
    ...
    def _initClass(self):
        ...
        self._controlAction = actions._ControlAction(parent=self)
        self._observationsAction = actions._ObservationAction(parent=self)
        self._configurationAction = actions._ConfigurationAction(parent=self)
        self._historyAction = actions._HistoryAction(parent=self)
        ...
        for widget in (self.getControlWindow(), self.getObservationWidget(),
         self.getConfigWindow(), self.getHistoryWindow()):
            widget.setVisible(False)

        self._controlAction.toggled[bool].connect(self.getControlWindow().setVisible)
        self._observationsAction.toggled[bool].connect(self.getObservationWidget().setVisible)
        self._configurationAction.toggled[bool].connect(self.getConfigWindow().setVisible)
    ...
    def _changeCentralWidget(self, widget, action):
        actions = (
         self._historyAction, self._observationsAction,
         self._configurationAction, self._controlAction)
        ...
        _widgets = (
         self.getControlWindow(), self.getObservationWidget(),
         self.getHistoryWindow(), self.getConfigWindow())
    ...
    def _getInitPath(self):
        ...
        if 'DATADIR' in os.environ:
            ...
            self._qlInfo.setText('note : environment variable DATADIR found, $DATADIR setted has the root of the observe folder')
            myFont = self._qlInfo.font()
            myFont.setItalic(True)
            self._qlInfo.setFont(myFont)
        else:
            self._qlInfo.setText("note : no DATADIR environment variable setted. Can't set a default root directory for observation")
            myFont = self._qlInfo.font()
            myFont.setItalic(True)
            self._qlInfo.setFont(myFont)
        return initPath
    ...
    def _getFolderSelection(self):
        ...
        self._DataWatcherWidget__stateLabel = qt.QLabel(parent=widget)
        self.animated_icon.register(self._DataWatcherWidget__stateLabel)
        self._setStateIcon(silxicons.getQIcon('remove'))
        self._DataWatcherWidget__stateLabel.setFixedWidth(30)
        layout.addWidget(self._DataWatcherWidget__stateLabel)
        widget.setLayout(layout)
        return widget
    ...
    def _setFolderPath(self):
        ...
        if not defaultDirectory is None:
            if os.path.isdir(defaultDirectory) or os.path.isdir(self.DEFAULT_DIRECTORY):
                defaultDirectory = self.DEFAULT_DIRECTORY
        elif defaultDirectory is None:
    ...
    def _connectObserverThread(self):
        if self.observationThread is not None:
            if self.obsThIsConnected is False:
                self.observationThread.observations.sigObsStatusReceived.connect(self._scanStatusChanged)
    ...
    def _disconnectObserverThread(self):
        if self.observationThread is not None:
            if self.obsThIsConnected is True:
                self.observationThread.observations.sigObsStatusReceived.disconnect(self._scanStatusChanged)
    ...
    def _initStatusView(self):
        ...
        """
        self._DataWatcherWidget__threadAnimation = QWaiterThread(0.1)
        self._DataWatcherWidget__threadAnimation.finished.connect(self._updateAnimatedIcon)

    def _updateStatusView(self):
        """Update the processing state"""
        if self.currentStatus in self._animatedStates:
            if not self._DataWatcherWidget__threadAnimation.isRunning():
                self._DataWatcherWidget__threadAnimation.start()
            elif self._DataWatcherWidget__threadAnimation is not None:
                self._DataWatcherWidget__threadAnimation.wait()
            ...
            if self.currentStatus == 'failure':
                ...
                if self.currentStatus == 'not processing':
    ...
    def _setStateIcon(self, icon):
        ...
        if icon is None:
            self._DataWatcherWidget__stateLabel.setPixmap(qt.QIcon().pixmap(30, state=(qt.QIcon.On)))
        else:
            self._DataWatcherWidget__stateLabel.setPixmap(icon.pixmap(30, state=(qt.QIcon.On)))
    ...
    def _signalScanReady(self, scan):
        if type(scan) is str:
            try:
                ...
                try:
                    logger.error('Fail to create a TomoBase instance from', scan, 'Reason is', e)
                    return
                finally:
                    e = None
                    del e
        ...
        self.lastFoundScans.add(_scan.path)
    ...
    def _createDataWatcher(self):
        self.observationThread = _QDataWatcherObserver(observationClass=(self._getObservationClass()), obsMethod=(self.obsMethod),
    ...
    def _launchObservation(self):
        ...
        if self.isObserving is False:
            ...
            if not (self.observationThread is None or self.observationThread.isRunning()):
                if self._initObservation() is False:
                    self.currentStatus = self._setCurrentStatus('failure')
                    logger.info('failed on observation', extra={logconfig.DOC_TITLE: self._scheme_title})
                    return
            self.observationThread.start()
            if self.loopObservationThread is None:
                self.loopObservationThread = QWaiterThread(self.getTimeBreakBetweenObservation())
                self.loopObservationThread.finished.connect(self._launchObservation)
            self.loopObservationThread.isRunning() or self._connectObserverThread()
