...
# [GCC 8.4.0]
# Embedded file name: /users/payno/.local/share/virtualenvs/tomwer_venc/lib/python3.7/site-packages/tomwer/test/utils/utilstest.py
# Compiled at: 2019-08-19 02:52:33
# Size of source mod 2**32: 14906 bytes
...
PACKAGE = 'pyFAI'
DATA_KEY = 'PYFAI_DATA'
import os, sys, threading, unittest, logging, shutil, contextlib, tempfile, getpass, functools
...
class TestContext(object):
    ...
    def __init__(self):
        ...
        self.resources = ExternalResources(PACKAGE, timeout=(self.timeout),
          env_key=DATA_KEY,
    ...
    def get_options(self):
        ...
        if self.options is None:
            ...
            parser.add_argument('-d', '--debug', dest='debug', help='run in debugging mode', default=False,
              action='store_true')
            parser.add_argument('-i', '--info', dest='info', help='run in more verbose mode ', default=False,
              action='store_true')
            parser.add_argument('-f', '--force', dest='force', help='force the build of the library', default=False,
              action='store_true')
            parser.add_argument('-r', '--really-force', dest='remove', help='remove existing build and force the build of the library',
              default=False,
              action='store_true')
    ...
    def _initialize_tmpdir(self):
        ...
        if not self._tempdir:
            with self.sem:
                if not self._tempdir:
                    self._tempdir = tempfile.mkdtemp('_' + getpass.getuser(), self.name + '_')
    ...
    def clean_up(self):
        ...
        with self.sem:
            if not self._tempdir:
                ...
                return os.path.isdir(self._tempdir) or None
