...
# [GCC 8.4.0]
# Embedded file name: /users/payno/.local/share/virtualenvs/tomwer_venc/lib/python3.7/site-packages/tomwer/gui/reconstruction/lamino/tofu/tofuoutput.py
# Compiled at: 2020-01-08 09:31:39
# Size of source mod 2**32: 29496 bytes
__authors__ = [
 'H. Payno']
__license__ = 'MIT'
__date__ = '01/06/2018'
...
from tomwer.core.process.reconstruction.lamino.tofu import SLICE_STACK_TYPE, ROTATION_CENTER_TYPE, LAMINO_ANGLE_TYPE, PSI_ANGLE_TYPE
...
class OutputTofuWidget(_TofuOptionLoader, qt.QWidget):
    ...
    def __init__(self, parent):
        ...
        self._OutputTofuWidget__scanID = None
        ...
        self._planeDisplay.setSizePolicy(qt.QSizePolicy.Preferred, qt.QSizePolicy.Preferred)
        ...
        options = {'x-region':_getterSetter(getter=self._region.getXRegion, setter=self._region.setXRegionFrmStr), 
         'y-region':_getterSetter(getter=self._region.getYRegion, setter=self._region.setYRegionFrmStr), 
         'volume-angle-x':_getterSetter(getter=self.getVolumeAngleX, setter=self.setVolumeAngleX), 
         'volume-angle-y':_getterSetter(getter=self.getVolumeAngleY, setter=self.setVolumeAngleY), 
         'volume-angle-z':_getterSetter(getter=self.getVolumeAngleZ, setter=self.setVolumeAngleZ), 
    ...
    def loadFromScan(self, scanID):
        self._OutputTofuWidget__scanID = scanID
        self._updateOutput()

    def _updateOutput(self):
        if self._OutputTofuWidget__scanID is None:
            return
        try:
            if self.isOutputFolderLocked() is False:
                self.setOutput(os.path.join(self._OutputTofuWidget__scanID, self._getNameExtension()))
    ...
    def _getNameExtension(self):
        ...
        if self._scan_type == SLICE_STACK_TYPE:
            if self._volumeAngleGrp._grpPlane.getPlane() == 'XY':
                return 'xySlice'
            if self._volumeAngleGrp._grpPlane.getPlane() == 'XZ':
                return 'xzSlice'
            if self._volumeAngleGrp._grpPlane.getPlane() == 'YZ':
                return 'yzSlice'
            if self._volumeAngleGrp._grpPlane.getPlane() == 'manual':
                return 'Slice'
        else:
            if self._scan_type == ROTATION_CENTER_TYPE:
                return 'xCenter'
            if self._scan_type == LAMINO_ANGLE_TYPE:
                return 'ctAngle'
            if self._scan_type in (PSI_ANGLE_TYPE, 'psi angle'):
                return 'rotC'
            return 'Slice'
...
class PlaneGroup(qt.QGroupBox):
    ...
    def __init__(self, parent):
        ...
        self._rBtns = (
         self._XYPlaneRB, self._XZPlaneRB, self._YZPlaneRB,
         self._autoPlaneRB)
        self._XYPlaneRB.setChecked(True)
        for btn in self._rBtns:
            btn.toggled.connect(self._PlaneGroup__planeChangedCllbck)
    ...
    def setPlane(self, plane):
        if not plane in ('XY', 'XZ', 'YZ', 'manual'):
...
class StepGroup(qt.QGroupBox):
    ...
    def __init__(self, parent):
        ...
        self._xcenter = 0.0
        self._StepGroup__mediumLimit = settings._N_SLICE_LIMITS_MEDIUM
        self._StepGroup__highLimit = settings._N_SLICE_LIMITS_HIGH
        self._defaultStyleSheet = ''
        self._mediumLimitStyleSheet = 'QLineEdit {' + self._convertToStyleSheetColor(settings._COLOR_MEDIUM) + '}'
        self._highLimitStyleSheet = 'QLineEdit {' + self._convertToStyleSheetColor(settings._COLOR_HIGH) + '}'
        self.setLayout(qt.QGridLayout())
        ...
        self._warningLabel.setToolTip('Number of slice to reconstruct seems to high, might fail')
        self.layout().addWidget(self._warningLabel, 1, 7, 1, 1)
        self.layout().addWidget(qt.QLabel('selection mode:'), 2, 0)
        self._selModeTooltips = {self._NB_SLICE_NAME: 'Will pick n slices from centered in the center, spaced of step size', 
         self._RANGE_NAME: 'Will pick slices from a range, each spaced of step size'}
        ...
        self._selectionMode.setItemData(idx, self._selModeTooltips[self._NB_SLICE_NAME], qt.Qt.ToolTipRole)
        self._selectionMode.addItem(self._RANGE_NAME)
        idx = self._selectionMode.findText(self._RANGE_NAME)
        self._selectionMode.setItemData(idx, self._selModeTooltips[self._RANGE_NAME], qt.Qt.ToolTipRole)
    ...
    def getRegion(self):
        ...
        if self._zSB.isVisible() is True:
            start_from = self.getZ() or 0.0
        return (float(self._fromLE.text()) + start_from,
    ...
    def setRegion(self, region):
        if type(region) is str:
            try:
                ...
                _logger.warning('Fail to setRegion range. given string does not fir the standard (from, to, stepSize)')
                return
    ...
    def _updateLimitsColor(self, *argv, **kwargs):
        ...
        if self._isUpperHightLimit() is True:
            ...
            if self._isUpperMediumLimit() is True:
    ...
    def _isUpperMediumLimit(self):
        return self._getNSlices() > self._StepGroup__mediumLimit

    def _isUpperHightLimit(self):
        return self._getNSlices() > self._StepGroup__highLimit
    ...
    def setHighLimit(self, val):
        if val == self._StepGroup__highLimit:
            return
        if val is None:
            self.resetHighLimit()
            return
        if val <= 0.0:
            ...
            return
        self._StepGroup__highLimit = val
...
class _OutputPathWidget(qt.QGroupBox):
    ...
    def __init__(self, parent):
        ...
        self._qteFolderSelected.setToolTip('If not output folder is given, then this will generate a "dry run"')
    ...
    def setOutput(self, text):
        if self.forceLbsram() is True:
            if text.startswith(get_lbsram_path()) is False:
    ...
    def getOutput(self):
        ...
        if self._qteFolderSelected.text() == '':
            return
        return self._OutputPathWidget__getOutputWithLbsramExtension()
    ...
    def _updateLbsramStatus(self, toggle):
        if toggle is True:
            self._qteFolderSelected.setText(self._OutputPathWidget__getOutputWithLbsramExtension())

    def __getOutputWithLbsramExtension(self):
        _output = self._qteFolderSelected.text()
        if self.forceLbsram() is True:
            if _output.startswith(get_lbsram_path()) is False:
