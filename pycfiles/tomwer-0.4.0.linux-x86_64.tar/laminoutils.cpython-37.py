# uncompyle6 version 3.7.4
# Python bytecode 3.7 (3394)
# Decompiled from: Python 3.6.9 (default, Apr 18 2020, 01:56:04) 
# [GCC 8.4.0]
# Embedded file name: /users/payno/.local/share/virtualenvs/tomwer_venc/lib/python3.7/site-packages/tomwer/core/utils/laminoutils.py
# Compiled at: 2019-08-19 02:52:33
# Size of source mod 2**32: 2862 bytes
__authors__ = [
 'H. Payno']
__license__ = 'MIT'
__date__ = '22/06/2018'
import fabio, os
from tomwer.core.log import TomwerLogger
logger = TomwerLogger(__name__)

def getMotorInformationFromEDfFile(_file):
    """

    :param _file: file to the edf file containing header with information
    :return: dictionary with motor name as key and motor position as value
    :rtype: dict
    """
    if not type(_file) is str:
        raise AssertionError
    else:
        assert _file.endswith('.edf')
        warn = os.path.isfile(_file) or '%s is not a valid file. Cannot retrieve motor information' % _file
        logger.warning(warn)
        return
    with fabio.open(_file) as (dsc):
        header = dsc.header
    if 'motor_mne' not in header:
        logger.warning('key "motor_mne" not found in the file header.Cannot retrieve motor information')
        return
    if 'motor_pos' not in header:
        logger.warning('key "motor_pos" not found in the file header.Cannot retrieve motor information')
        return
    motor_mne_lst = header['motor_mne'].split(' ')
    motor_pos_lst = header['motor_pos'].split(' ')
    if len(motor_mne_lst) != len(motor_pos_lst):
        logger.warning('Incoherent number of motor_mne and motor_pos.Cannot retrieve motor information')
        return
    res = {}
    for mne, pos in zip(motor_mne_lst, motor_pos_lst):
        res[mne] = pos

    return res