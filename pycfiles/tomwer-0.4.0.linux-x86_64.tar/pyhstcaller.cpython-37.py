...
# [GCC 8.4.0]
# Embedded file name: /users/payno/.local/share/virtualenvs/tomwer_venc/lib/python3.7/site-packages/tomwer/core/process/reconstruction/pyhstcaller.py
# Compiled at: 2020-03-06 02:01:31
# Size of source mod 2**32: 74604 bytes
__author__ = [
...
import socket, subprocess, typing, os
logger = TomwerLogger(__name__)

class PyHSTCaller(object):
    __doc__ = 'Simple class containing information to be able to call PyHST\n    '
    REC_EXT = '.rec'
    PAR_EXT = '.par'
    ...
    def isvalid(exec_dir, pyhst_exe):
        ...
        :rtype: bool
        """
        if exec_dir is None or pyhst_exe is None:
            return False
        return os.path.isfile(os.path.join(exec_dir, pyhst_exe))

    def _get_basefile_name_with_pag_des(self, paganin_opts: dict, scan_path: str, slice: typing.Union[(None, int)]) -> str:
        ...
        :rtype: str
        """
        if not 'DB' in paganin_opts:
            raise AssertionError('requested to determine basefile name')
        elif not 'DB2' in paganin_opts:
            raise AssertionError('requested to determine basefile name')
        else:
            ...
            if pag_mod == PaganinMode.off:
                raise ValueError('This function should not be call if paganin mode is off')
            else:
                if pag_mod in (PaganinMode.on, PaganinMode.both):
                    if slice is None:
                        try:
                            return '_'.join((scan_basename + 'pag',
                             '%04d' % int(paganin_opts['DB'])))
                        except Exception as e:
                            try:
                                return '_'.join((scan_basename,
                                 'pag%04.4i' % paganin_opts['DB']))
                            finally:
                                e = None
                                del e
                        ...
                        try:
                            return '_'.join((scan_basename + 'slice_pag',
                             '%04d' % int(paganin_opts['DB'])))
                        except Exception as e:
                            try:
                                return '_'.join(scan_basename + 'slice_pag%04.4i' % paganin_opts['DB'])
                            finally:
                                e = None
                                del e

                else:
                    if pag_mod == PaganinMode.multi:
                        if slice is None:
                            multi_pag_val = 'db%04.4i-db%04.4i' % (paganin_opts['DB'], paganin_opts['DB2'])
                            return '_'.join((scan_basename + 'multipag', multi_pag_val))
                        multi_pag_val = 'db%04.4i-db%04.4i' % (
                         paganin_opts['DB'], paganin_opts['DB2'])
                        return '_'.join((scan_basename + 'slice_multipag', multi_pag_val))

    def process(self, scan, dry_run=False, rec_slices_only=True, full_rec_to_dest=True, slices_rec_to_dest=False):
        """
        ...
        :type: bool
        """
        if not isinstance(scan, TomoBase):
            ...
            calling several time the same .par file"""
                if not rp is not None:
                    raise AssertionError
                elif not rp.paganin.mode in PaganinMode:
                    raise AssertionError
                elif rp.paganin.mode is PaganinMode.multi:
                    pass
                elif rp.paganin.mode in (PaganinMode.on, PaganinMode.both):
                    rp.paganin.db2 = '(-1, )'
                else:
                    if rp.paganin.mode is PaganinMode.off:
                        rp.paganin.db = '(-1, )'
                        rp.paganin.db2 = '(-1, )'
                return rp
            ...
            if recons_params.ft.do_test_slice:
                ...
                if fixed_slice is FixedSliceMode.middle:
                    n_slice = scan.get_dim_2() or 2048
        ...
        for slice in slices:
            for recons_param in recons_list:
                ...
                self._process_frm_one_slice_one_rp_set(scan=scan, slice=slice,
    ...
    def _process_frm_one_slice_one_rp_set(self, scan: TomoBase, slice: typing.Union[(int, None)], recons_param: dict, cuda_devices: typing.Union[(None, typing.Iterable)], has_several_pag: bool, run_recons: bool, convert_lbsram_to_dest: bool, dry_run: bool) -> None:
        """
        ...
        :type: TomoBase
        :param slice: slice index or None. If None then we will process the
                      entire volume.
        :type: Union[int, None]
        :param recons_param: reconstruction parameters. warning: this set of
                             ...
                             function
        :type: dict
        :param cuda_devices: list of cuda devices to use for reconstruction
        :type: Union[None, Iterable]
        :param has_several_pag: should we run several Paganin reconstruction
                                for this scan.
        :type: bool
        :param run_recons: if True then call pyhst within the created .par file
                           (depending on dry-run parameter of course)
        :type: bool
        :param convert_lbsram_to_dest: if True then convert all path to the
                                       'dest folder'.
        :type: bool
        :param dry_run: if True then skip call to pyhst and only create .par
                        and .rec files
        :type: bool
        """
        if not isinstance(recons_param, dict):
            raise AssertionError
        else:
            if not isinstance(slice, (type(None), int)):
                raise AssertionError
            else:
                if slice is None:
                ...
                if PaganinMode.from_value(recons_param['PAGANIN']['MODE']) == PaganinMode.off:
                    ...
                    base_file_name = self._get_basefile_name_with_pag_des(paganin_opts=(recons_param['PAGANIN']), scan_path=(scan.path),
                      slice=slice)
                    ...
                    if slice is None:
                        ...
                        par_file_name = base_file_name + '_%04d.par' % slice
            try:
                if not recons_param['FTAXIS']['DO_AXIS_CORRECTION'] or recons_param['FTAXIS']['USE_TOMWER_AXIS'] or recons_param['FTAXIS']['TRY_USE_OLD_TOMWER_AXIS']:
                    cor = self.get_center_of_rotation(scan=scan,
                      use_tomwer_axis=(recons_param['FTAXIS']['USE_TOMWER_AXIS']),
                      use_old_tomwer_processes=(recons_param['FTAXIS']['TRY_USE_OLD_TOMWER_AXIS']),
                      output_file=correct_file_name)
                else:
                    cor = None
                correct_file = None
                self.make_par_file(dirname=(scan.path), scan=scan,
                  recons_param=recons_param,
                  out_base_file_name=base_file_name,
                  correct_file=correct_file_name,
                  slice=slice,
                ...
                if recons_param['PYHSTEXE']['MAKE_OAR_FILE']:
                    ...
                    self.make_rec_file(rec_file=rec_file_name, recons_params=recons_param,
                    ...
                    if recons_param['PYHSTEXE']['VERBOSE']:
                        ...
                        pyhst_out = os.path.join(scan.path, recons_param['PYHSTEXE']['VERBOSE_FILE'])
                if run_recons is True:
                    try:
                        self.execute(exec_dir=exec_dir, pyhst_exe=pyhst_exe,
    ...
    def get_center_of_rotation(self, scan: TomoBase, use_tomwer_axis: bool, use_old_tomwer_processes: str, output_file: str):
        """
        ...
        if use_tomwer_axis:
            if scan.axis_params is not None:
                if scan.axis_params.value is not None:
                    logger.info('load center of rotation from previous axis process: %s' % scan.axis_params.value)
                    return scan.axis_params.value
        if use_old_tomwer_processes:
            if scan.process_file is not None:
                if os.path.exists(scan.process_file):
                    ...
                    cor = AxisRP.get_cor_position_frm_tomwer_file(file_path=(scan.process_file))
                    logger.info('load center of rotation from %s: %s' % (scan.process_file, cor))
                    return cor

    def execute(self, exec_dir: str, pyhst_exe: str, par_file: str, cuda_devices: list, dry_run: bool=False, pyhst_out: typing.Union[(None, str)]=None) -> bool:
        """
        ...
        if not par_file is not None:
            ...
            if dry_run is False:
                ...
                if not PyHSTCaller.isvalid(pyhst_exe=pyhst_exe, exec_dir=exec_dir):
                    raise ValueError('physt executable definition, %s, %s is invalid' % (exec_dir, pyhst_exe))
            try:
                ...
                if dry_run is False:
                    if pyhst_out is not None:
                        with open(pyhst_out, 'w') as (fout):
                            self._PyHSTCaller__execute(pyhst_exe=pyhst_exe, par_file=par_file, gpu_cmd_opt=gpu_cmd_opt,
                              stdout=fout)
                    else:
                        self._PyHSTCaller__execute(pyhst_exe=pyhst_exe, par_file=par_file, gpu_cmd_opt=gpu_cmd_opt,
                          stdout=None)
    ...
    def __execute(self, pyhst_exe, par_file, gpu_cmd_opt, stdout):
        logger.info('processing pyhst reconstruction... (%s)' % par_file)
        subprocess.call((' '.join((pyhst_exe, par_file, gpu_cmd_opt))), shell=True,
          stdout=stdout,
          stderr=stdout)
    ...
    def make_par_file(self, dirname, scan, recons_param, slice, convert_path, par_file_path: str, out_base_file_name=None, options=None, correct_file=None, cor=None):
        """
        ...
        * argument 9: offset : position of rotation axis with respect to middle in pixels
        ...
        :param TomoBase scan: scan to reconstruct
        ...
        if not isinstance(scan, TomoBase):
            ...
            recons_param['FT']['NUM_LAST_IMAGE'] = scan.get_tomo_n()
            recons_param['FT']['NUM_IMAGE_1'] = scan.get_dim_1()
            recons_param['FT']['NUM_IMAGE_2'] = scan.get_dim_2()
            scan_range = scan.get_scan_range()
            recons_param['FT']['IMAGE_PIXEL_SIZE_1'] = scan.get_pixel_size() / metricsystem.micrometer if scan.get_pixel_size() is not None else None
            recons_param['FT']['CORRECT_FLATFIELD'] = True
            recons_param['FT']['FF_FILE_INTERVAL'] = scan.get_ff_interval()
            ...
            direc = dirname
            fileParameter = self._loadDefaultPar(recons_param=recons_param, out_base_file_name=out_base_file_name,
              direc=direc,
            ...
            if convert_path:
                fileParameter['output_file'] = self._remove_lbs_head(fileParameter['output_file'])
            if slice is None:
            ...
            if paganin == PaganinMode.multi:
                if fileParameter['multipag_extra_files'] == 1:
                    fileParameter['extra_output_file'] = self._remove_lbs_head(out_base_file_name)
        ...
        with open(par_file_path, 'wb') as (fidparfile):
            ...
            if convert_path:
                fileParameter['background_file'] = self._remove_lbs_head(dirin=(fileParameter['background_file']))
                fileParameter['ff_prefix'] = self._remove_lbs_head(dirin=(fileParameter['ff_prefix']))
            if 'PAG_LENGTH' in recons_param['PAGANIN']:
                ...
                fidparfile.write(bytes('# generated by tomwer - %s\n\n' % tomwer_version.version, 'UTF-8'))
                fidparfile.write(bytes('# Parameters defining the projection file series\n', 'UTF-8'))
                ...
                fidparfile.write(bytes('NUM_FIRST_IMAGE = %d # No. of first projection file\n' % fileParameter['num_first_image'], 'UTF-8'))
                fidparfile.write(bytes('NUM_LAST_IMAGE = %d # No. of last projection file\n' % fileParameter['num_last_image'], 'UTF-8'))
                fidparfile.write(bytes('NUMBER_LENGTH_VARIES = %s\n' % fileParameter['number_length_varies'], 'UTF-8'))
                fidparfile.write(bytes('LENGTH_OF_NUMERICAL_PART = %d # No. of characters\n' % length_of_numerical_part, 'UTF-8'))
                fidparfile.write(bytes('FILE_POSTFIX = %s\n' % fileParameter['file_postfix'], 'UTF-8'))
                fidparfile.write(bytes('FILE_INTERVAL = %d # Interval between input files\n' % fileParameter['file_interval'], 'UTF-8'))
                fidparfile.write(bytes('\n# Parameters defining the projection file format\n', 'UTF-8'))
                if num_image_1 is not None:
                    fidparfile.write(bytes('NUM_IMAGE_1 = %d # Number of pixels horizontally\n' % num_image_1, 'UTF-8'))
                else:
                    fidparfile.write(bytes('NUM_IMAGE_1 = ??? # Number of pixels horizontally\n', 'UTF-8'))
                if num_image_2 is not None:
                    fidparfile.write(bytes('NUM_IMAGE_2 = %d # Number of pixels vertically\n' % num_image_2, 'UTF-8'))
                else:
                    fidparfile.write(bytes('NUM_IMAGE_2 = ??? # Number of pixels vertically\n', 'UTF-8'))
                if image_pixel_size is not None:
                    fidparfile.write(bytes('IMAGE_PIXEL_SIZE_1 = %f # Pixel size horizontally (microns)\n' % image_pixel_size, 'UTF-8'))
                else:
                    fidparfile.write(bytes('IMAGE_PIXEL_SIZE_1 = ??? # Pixel size horizontally (microns)\n', 'UTF-8'))
                if fileParameter['image_pixel_size_2'] is not None:
                    fidparfile.write(bytes('IMAGE_PIXEL_SIZE_2 = %f # Pixel size vertically\n' % fileParameter['image_pixel_size_2'], 'UTF-8'))
                else:
                    fidparfile.write(bytes('IMAGE_PIXEL_SIZE_2 = ??? # Pixel size vertically\n', 'UTF-8'))
                fidparfile.write(bytes('\n# Parameters defining background treatment\n', 'UTF-8'))
                if fileParameter['correct']:
                    fidparfile.write(bytes('SUBTRACT_BACKGROUND = %s # Subtract background from data\n' % fileParameter['subtract_background'], 'UTF-8'))
                    fidparfile.write(bytes('BACKGROUND_FILE = %s\n' % fileParameter['background_file'], 'UTF-8'))
                else:
                    fidparfile.write(bytes('SUBTRACT_BACKGROUND = %s # No background subtraction\n' % fileParameter['subtract_background'], 'UTF-8'))
                    fidparfile.write(bytes('BACKGROUND_FILE = %s\n' % not_available, 'UTF-8'))
                fidparfile.write(bytes('\n# Parameters defining flat-field treatment\n', 'UTF-8'))
                if fileParameter['correct']:
                    fidparfile.write(bytes('CORRECT_FLATFIELD = %s # Divide by flat-field image\n' % fileParameter['correct_flatfield'], 'UTF-8'))
                    fidparfile.write(bytes('FLATFIELD_CHANGING = %s # Series of flat-field files\n' % fileParameter['flatfield_changing'], 'UTF-8'))
                    fidparfile.write(bytes('FLATFIELD_FILE = %s\n' % os.path.basename(fileParameter['flatfield_file']), 'UTF-8'))
                    fidparfile.write(bytes('FF_PREFIX = %s\n' % fileParameter['ff_prefix'], 'UTF-8'))
                    fidparfile.write(bytes('FF_NUM_FIRST_IMAGE = %d # No. of first flat-field file\n' % fileParameter['ff_num_first_image'], 'UTF-8'))
                    fidparfile.write(bytes('FF_NUM_LAST_IMAGE = %d # No. of last flat-field file\n' % fileParameter['ff_num_last_image'], 'UTF-8'))
                    fidparfile.write(bytes('FF_NUMBER_LENGTH_VARIES = %s\n' % fileParameter['number_length_varies'], 'UTF-8'))
                    fidparfile.write(bytes('FF_LENGTH_OF_NUMERICAL_PART = %d # No. of characters\n' % length_of_numerical_part, 'UTF-8'))
                    fidparfile.write(bytes('FF_POSTFIX = %s\n' % fileParameter['file_postfix'], 'UTF-8'))
                    if fileParameter['ff_file_interval'] is not None:
                        fidparfile.write(bytes('FF_FILE_INTERVAL = %d # Interval between flat-field files\n' % fileParameter['ff_file_interval'], 'UTF-8'))
                    else:
                        fidparfile.write(bytes('FF_FILE_INTERVAL = ??? # Interval between flat-field files\n', 'UTF-8'))
                else:
                    fidparfile.write(bytes('CORRECT_FLATFIELD = %s # No flat-field correction\n' % fileParameter['correct_flatfield'], 'UTF-8'))
                    fidparfile.write(bytes('FLATFIELD_CHANGING = %s\n' % not_available, 'UTF-8'))
                    fidparfile.write(bytes('FLATFIELD_FILE = %s\n' % not_available, 'UTF-8'))
                    fidparfile.write(bytes('FF_PREFIX = %s\n' % not_available, 'UTF-8'))
                    fidparfile.write(bytes('FF_NUM_FIRST_IMAGE = %s\n' % not_available, 'UTF-8'))
                    fidparfile.write(bytes('FF_NUM_LAST_IMAGE = %s\n' % not_available, 'UTF-8'))
                    fidparfile.write(bytes('FF_NUMBER_LENGTH_VARIES = %s\n' % not_available, 'UTF-8'))
                    fidparfile.write(bytes('FF_LENGTH_OF_NUMERICAL_PART = %s\n' % not_available, 'UTF-8'))
                    fidparfile.write(bytes('FF_POSTFIX = %s\n' % not_available, 'UTF-8'))
                    fidparfile.write(bytes('FF_FILE_INTERVAL = %s\n' % not_available, 'UTF-8'))
                fidparfile.write(bytes('\nTAKE_LOGARITHM = %s # Take log of projection values\n' % fileParameter['take_logarithm'], 'UTF-8'))
                if not fileParameter['doubleffcorrection'] == []:
                    fidparfile.write(bytes('DOUBLEFFCORRECTION = "filtered_mean.edf" # double flatfield\n', 'UTF-8'))
                else:
                    fidparfile.write(bytes('\n# Parameters defining experiment\n', 'UTF-8'))
                    fidparfile.write(bytes('ANGLE_BETWEEN_PROJECTIONS = %f # Increment angle in degrees\n' % fileParameter['angle_between_projections'], 'UTF-8'))
                    fidparfile.write(bytes('ROTATION_VERTICAL = %s\n' % fileParameter['rotation_vertical'], 'UTF-8'))
                    if fileParameter['rotation_axis_position'] is not None:
                        fidparfile.write(bytes('ROTATION_AXIS_POSITION = %f # Position in pixels\n' % fileParameter['rotation_axis_position'], 'UTF-8'))
                    else:
                        fidparfile.write(bytes('ROTATION_AXIS_POSITION = ??? # Position in pixels\n', 'UTF-8'))
                    fidparfile.write(bytes('\n# Parameters defining reconstruction\n', 'UTF-8'))
                    fidparfile.write(bytes('OUTPUT_SINOGRAMS = %s # Output sinograms to files or not\n' % fileParameter['output_sinograms'], 'UTF-8'))
                    fidparfile.write(bytes('OUTPUT_RECONSTRUCTION = %s # Reconstruct and save or not\n' % fileParameter['output_reconstruction'], 'UTF-8'))
                    if fileParameter['start_voxel_1'] is not None:
                        fidparfile.write(bytes('START_VOXEL_1 =   %4d # X-start of reconstruction volume\n' % fileParameter['start_voxel_1'], 'UTF-8'))
                    else:
                        fidparfile.write(bytes('START_VOXEL_1 =   ??? # X-start of reconstruction volume\n', 'UTF-8'))
                    if fileParameter['start_voxel_2'] is not None:
                        fidparfile.write(bytes('START_VOXEL_2 =   %4d # Y-start of reconstruction volume\n' % fileParameter['start_voxel_2'], 'UTF-8'))
                    else:
                        fidparfile.write(bytes('START_VOXEL_2 =   ??? # Y-start of reconstruction volume\n', 'UTF-8'))
                    if fileParameter['start_voxel_3'] is not None:
                        fidparfile.write(bytes('START_VOXEL_3 =   %4d # Z-start of reconstruction volume\n' % fileParameter['start_voxel_3'], 'UTF-8'))
                    else:
                        fidparfile.write(bytes('START_VOXEL_3 =   ??? # Z-start of reconstruction volume\n', 'UTF-8'))
                    if fileParameter['end_voxel_1'] is not None:
                        fidparfile.write(bytes('END_VOXEL_1 =   %4d # X- of reconstruction volume\n' % fileParameter['end_voxel_1'], 'UTF-8'))
                    else:
                        fidparfile.write(bytes('END_VOXEL_1 =   %4d # X- of reconstruction volume\n', 'UTF-8'))
                    if fileParameter['end_voxel_2'] is not None:
                        fidparfile.write(bytes('END_VOXEL_2 =   %4d # Y- of reconstruction volume\n' % fileParameter['end_voxel_2'], 'UTF-8'))
                    else:
                        fidparfile.write(bytes('END_VOXEL_2 =   ??? # Y- of reconstruction volume\n', 'UTF-8'))
                if fileParameter['end_voxel_3'] is not None:
                    fidparfile.write(bytes('END_VOXEL_3 =   %4d # Z- of reconstruction volume\n' % fileParameter['end_voxel_3'], 'UTF-8'))
                else:
                    fidparfile.write(bytes('END_VOXEL_3 =   ??? # Z- of reconstruction volume\n', 'UTF-8'))
            fidparfile.write(bytes('OVERSAMPLING_FACTOR = %d # 0 = Linear, 1 = Nearest pixel\n' % fileParameter['oversampling_factor'], 'UTF-8'))
            fidparfile.write(bytes('ANGLE_OFFSET = %f # Reconstruction rotation offset angle in degrees\n' % recons_param['FT']['ANGLE_OFFSET_VALUE'], 'UTF-8'))
            fidparfile.write(bytes('\n# Parameters extra features PyHST\n', 'UTF-8'))
            fidparfile.write(bytes('DO_CCD_FILTER = %s # CCD filter (spikes)\n' % fileParameter['do_ccd_filter'], 'UTF-8'))
            fidparfile.write(bytes('CCD_FILTER = "%s"\n' % fileParameter['ccd_filter'], 'UTF-8'))
            assert isinstance(fileParameter['ccd_filter_para'], dict)
            fidparfile.write(bytes('CCD_FILTER_PARA = %s\n' % fileParameter['ccd_filter_para'], 'UTF-8'))
            fidparfile.write(bytes('DO_SINO_FILTER = %s # Sinogram filter (rings)\n' % fileParameter['do_sino_filter'], 'UTF-8'))
            fidparfile.write(bytes('SINO_FILTER = "%s"\n' % 'SINO_Filter', 'UTF-8'))
            if num_image_1 is not None:
                fidparfile.write(bytes("ar = Numeric.ones(%d,'f')\n" % num_image_1, 'UTF-8'))
            fidparfile.write(bytes('ar[0]=0.0\n', 'UTF-8'))
            if fileParameter['correct_rings_nb'] > 0:
                fidparfile.write(bytes('ar[2:%d]=0.0\n' % 2 * (fileParameter['correct_rings_nb'] + 1), 'UTF-8'))
            fidparfile.write(bytes('SINO_FILTER_PARA = {"FILTER": ar }\n', 'UTF-8'))
            fidparfile.write(bytes('DO_AXIS_CORRECTION = NO # Axis correction\n', 'UTF-8'))
            correct_file = ''
            fidparfile.write(bytes("OPTIONS= { 'padding':'%s', 'axis_to_the_center':'%s', 'avoidhalftomo':'%s'} # Padding and position axis\n" % (
             fileParameter['padding'], fileParameter['axis_to_the_center'], fileParameter['avoidhalftomo']), 'UTF-8'))
            if paganin is not PaganinMode.off:
                fidparfile.write(bytes('\n# Parameters for Paganin reconstruction\n', 'UTF-8'))
                fidparfile.write(bytes('# delta over beta ratio for Paganin phase retrieval = %1.2f \n' % PAG['DB'], 'UTF-8'))
                fidparfile.write(bytes('DO_PAGANIN = 1 \n', 'UTF-8'))
                fidparfile.write(bytes('PAGANIN_Lmicron = %f \n' % PAG['DB'], 'UTF-8'))
                ...
                if fileParameter['do_unsharp'] == 1:
                    ...
                    fidparfile.write(bytes('PUS = %f \n' % recons_param['PAGANIN']['UNSHARP_SIGMA'], 'UTF-8'))
                    fidparfile.write(bytes('PUC = %f \n' % recons_param['PAGANIN']['UNSHARP_COEFF'], 'UTF-8'))
            if paganin is PaganinMode.multi:
                fidparfile.write(bytes('\n# Multi_paganin\n', 'UTF-8'))
                fidparfile.write(bytes('# 2nd delta over beta ratio for multi Paganin phase retrieval = %1.2f \n' % PAG['DB2'], 'UTF-8'))
                fidparfile.write(bytes('MULTI_PAGANIN_PARS={}\n', 'UTF-8'))
                fidparfile.write(bytes('MULTI_PAGANIN_PARS["BONE_Lmicron"]= %f \n' % PAG['DB2'], 'UTF-8'))
                fidparfile.write(bytes('MULTI_PAGANIN_PARS["THRESHOLD"]= %1.3f\n' % PAG['THRESHOLD'], 'UTF-8'))
                fidparfile.write(bytes('MULTI_PAGANIN_PARS["DILATE"]= %1.0f\n' % PAG['DILATE'], 'UTF-8'))
                fidparfile.write(bytes('MULTI_PAGANIN_PARS["MEDIANR"]= %1.0f\n' % recons_param['PAGANIN']['MEDIANR'], 'UTF-8'))
                if fileParameter['multipag_keep_bone'] == 1:
                    fidparfile.write(bytes('MULTI_PAGANIN_PARS["PAGBONE_FILE"] = "%s"\n' % fileParameter['output_file_bone'], 'UTF-8'))
                if fileParameter['multipag_keep_mask'] == 1:
                    fidparfile.write(bytes('MULTI_PAGANIN_PARS["MASKBONE_FILE"] = "%s"\n' % fileParameter['output_file_mask'], 'UTF-8'))
                if fileParameter['multipag_keep_abs'] == 1:
                    fidparfile.write(bytes('MULTI_PAGANIN_PARS["ABSBONE_FILE"] = "%s"\n' % fileParameter['output_file_abs'], 'UTF-8'))
                if fileParameter['multipag_keep_corr'] == 1:
                    fidparfile.write(bytes('MULTI_PAGANIN_PARS["CORRBONE_FILE"] = "%s"\n' % fileParameter['output_file_corr'], 'UTF-8'))
                if fileParameter['multipag_keep_soft'] == 1:
                    fidparfile.write(bytes('MULTI_PAGANIN_PARS["CORRECTEDVOL_FILE"] = "%s"\n' % fileParameter['output_file_soft'], 'UTF-8'))
            if fileParameter['half_acquisition'] == 1:
                fidparfile.write(bytes('PENTEZONE=300\n', 'UTF-8'))
            if FT['ZEROOFFMASK'] == 0:
                fidparfile.write(bytes('ZEROOFFMASK=0\n', 'UTF-8'))
            fidparfile.write(bytes('TRYEDFCONSTANTHEADER=%d \n' % (1 - FT['FIXHD']), 'UTF-8'))
            if 'zeroclipvalue' not in fileParameter:
                fidparfile.write(bytes('ZEROCLIPVALUE = %g # Minimum value of radiographs after flat / before log\n' % fileParameter['zeroclipvalue'], 'UTF-8'))
            if 'oneclipvalue' not in fileParameter:
                fidparfile.write(bytes('ONECLIPVALUE = %g # Maximum value of radiographs after flat / before log\n' % fileParameter['oneclipvalue'], 'UTF-8'))
            if fileParameter['do_projection_median'] == 'YES':
                fidparfile.write(bytes('DO_PROJECTION_MEDIAN = %s # Calculate median of all projections\n' % fileParameter['do_projection_median'], 'UTF-8'))
                fidparfile.write(bytes('PROJECTION_MEDIAN_FILENAME = "%s" # Name output file median calculation\n' % fileParameter['projection_median_filename'], 'UTF-8'))
            if fileParameter['do_projection_mean'] == 'YES':
                fidparfile.write(bytes('DO_PROJECTION_MEAN = %s # Calculate median of all projections\n' % fileParameter['do_projection_mean'], 'UTF-8'))
                fidparfile.write(bytes('PROJECTION_MEDIAN_FILENAME = "%s" # Name output file mean calculation\n' % fileParameter['projection_mean_filename'], 'UTF-8'))
            fidparfile.write(bytes('\n# Parameters defining output file / format\n', 'UTF-8'))
            fidparfile.write(bytes('OUTPUT_FILE = %s\n' % fileParameter['output_file'], 'UTF-8'))
            ...
            if BG['TYPE'] == 'c':
                ...
                fidparfile.write(bytes('SOURCE_X = %f\n' % fileParameter['source_x'], 'UTF-8'))
                fidparfile.write(bytes('SOURCE_Y = %f\n' % fileParameter['source_y'], 'UTF-8'))
                fidparfile.write(bytes('SOURCE_DISTANCE = %f\n' % fileParameter['source_distance'], 'UTF-8'))
            if BG['TYPE'] == 'f':
                fidparfile.write(bytes('\n# Conical and Fan beam reconstruction\n', 'UTF-8'))
    ...
    def make_rec_file(self, rec_file, par_file, recons_params):
        ...
        if recFile is None or prefix is None:
            raise IOError('Failed to load parameters `OUTPUT_FILE` and/or `FILE_PREFIX` in the reconstruction parameters file')
        else:
        ...
        with open(rec_file, 'wb') as (fidshell):
            ...
            fidshell.write(bytes('GET_OS="/csadmin/common/scripts/get_os.share"\n', 'UTF-8'))
            fidshell.write(bytes('os=`$GET_OS`\n', 'UTF-8'))
            fidshell.write(bytes('export PYHST=' + self.execDir + recons_params['PYHSTEXE']['EXE'] + '\n', 'UTF-8'))
            ...
            fidshell.write(bytes('        ' + str(utilsMakeOAR) + ' ' + prefix_full + ' 1\n', 'UTF-8'))
            fidshell.write(bytes('        oarsub -q gpu  -S ./' + prefix_full + '.oar\n', 'UTF-8'))
        ...
        os.chmod(rec_file, 511)

    @staticmethod
    def _loadDefaultPar(recons_param, out_base_file_name, direc, image_pixel_size, corr, nvue, refon, num_image_1, num_image_2, slice, convert_lbsram, cor, scan_range):
        if not type(recons_param) is dict:
            raise AssertionError
        elif not out_base_file_name is not None:
            raise AssertionError
        else:
            defaultParams = {}
            FT = recons_param['FT']
            ...
            defaultParams['subtract_background'] = defaultParams['correct_flatfield'] = corr
            ...
            if slice is None:
                ...
                defaultParams['start_voxel_3'] = slice + 1
            defaultParams['end_voxel_1'] = defaultParams['end_voxel_2'] = num_image_1
            if recons_param['FT']['HALF_ACQ']:
                if cor is None:
                    cor = num_image_1 // 2
                defaultParams['end_voxel_1'] = defaultParams['end_voxel_2'] = num_image_1 + abs(2.0 * cor)
            else:
                if slice is None:
                    defaultParams['end_voxel_3'] = num_image_2
                else:
                    defaultParams['end_voxel_3'] = slice + 1
                ...
                defaultParams['correct_rings_nb'] = 2
                ...
                if paganin is PaganinMode.off:
                    ...
                    if paganin in (PaganinMode.on, PaganinMode.both):
                        ...
                        if paganin is PaganinMode.multi:
                            ...
                            defaultParams['multipag_extra_files'] = defaultParams['multipag_keep_bone'] == 1 or defaultParams['multipag_keep_soft'] == 1 or defaultParams['multipag_keep_mask'] == 1 or defaultParams['multipag_keep_abs'] == 1 or defaultParams['multipag_keep_corr'] == 1
                            defaultParams['output_file_bone'] = os.path.join(direc, out_base_file_name + 'multipag_bone' + output_file_ext)
                            defaultParams['output_file_soft'] = os.path.join(direc, out_base_file_name + 'multipag_soft' + output_file_ext)
                            defaultParams['output_file_abs'] = os.path.join(direc, out_base_file_name + 'multipag_abs' + output_file_ext)
                            defaultParams['output_file_corr'] = os.path.join(direc, out_base_file_name + 'multipag_corr' + output_file_ext)
                            defaultParams['output_file_mask'] = os.path.join(direc, out_base_file_name + 'multipag_mask' + output_file_ext)
                        else:
                            raise ValueError('Paganin mode not recognized')
        keys_defining_path = ('output_file', 'background_file', 'ff_prefix', 'flatfield_file',
                              'output_file_bone', 'output_file_soft', 'output_file_abs',
                              'output_file_corr', 'output_file_mask')
        if convert_lbsram:
