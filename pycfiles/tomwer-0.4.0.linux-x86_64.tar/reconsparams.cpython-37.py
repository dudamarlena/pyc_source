...
# [GCC 8.4.0]
# Embedded file name: /users/payno/.local/share/virtualenvs/tomwer_venc/lib/python3.7/site-packages/tomwer/core/process/reconstruction/ftseries/params/reconsparams.py
...
class ReconsParams(_ReconsParam):
    ...
    def to_dict(self):
        _dict = {'FT':self.ft.to_dict() if self.ft else None, 
    ...
    def copy(self, other_rp):
        ...
        if other_rp is None:
            ...
            def copy_all_recons_params():
                ...
                other_sub_rps = (other_rp.ft, other_rp.pyhst, other_rp.axis,
                 other_rp.paganin, other_rp.beam_geo, other_rp.dkrf)
            ...
            if isinstance(other_rp, ReconsParams):
                ...
                if isinstance(other_rp, FTRP):
                    self._copy_ft_rp(other_rp)
                else:
                    if isinstance(other_rp, PyhstRP):
                        self._copy_pyhst_rp(other_rp)
                    else:
                        if isinstance(other_rp, AxisRP):
                            self._copy_axis_rp(other_rp)
                        else:
                            if isinstance(other_rp, PaganinRP):
                                self._copy_paganin_rp(other_rp)
                            else:
                                if isinstance(other_rp, BeamGeoRP):
                                    self._copy_beam_geo_rp(other_rp)
                                else:
                                    if isinstance(other_rp, DKRFRP):
                                        self._copy_dkrf_rp(other_rp)
                                    else:
                                        raise TypeError('Cannot copy %s, because type %s is not recognized' % (
