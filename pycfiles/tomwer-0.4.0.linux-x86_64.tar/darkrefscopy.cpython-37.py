...
# [GCC 8.4.0]
# Embedded file name: /users/payno/.local/share/virtualenvs/tomwer_venc/lib/python3.7/site-packages/tomwer/core/process/reconstruction/darkref/darkrefscopy.py
...
__license__ = 'MIT'
__date__ = '05/02/2018'
import os, shutil, tempfile, fabio, numpy
...
class DarkRefsCopyWorker(DarkRefsWorker):
    ...
    def setRefsFromScan(self, scanID):
        if os.path.isdir(scanID) is False:
            ...
            if self._hasRef(scanID) is False:
                ...
                return False
                self.cleanSaveFiles()
                darkHSTFiles = sorted(DarkRefs.getDarkHSTFiles(scanID, prefix=(self.getDarkHstPrefix())))
                refHSTFiles = sorted(DarkRefs.getRefHSTFiles(scanID, prefix=(self.getRefHstPrefix())))
    ...
    def _copyTo(self, scanID):
        if os.path.isdir(scanID) is False:
            logger.warning(scanID + ' is not a directory. Cannot copy reference to it.')
            return
    ...
    def _copyDarkTo(self, scanID):
        ...
        if os.path.isfile(self._darkHST) is False:
            logger.error(self._darkHST + ' is not a file. Cannot be used as reference for dark')
            return
        if os.path.isdir(scanID) is False:
    ...
    def _copyRefTo(self, outputdir):
        ...
        def normalize(data, when):
            ...
            if srCurrent is None:
                logger.warning("Can't find information about srCurrent,set to default value")
                srCurrent = self.DEFAULT_SRCURRENT
            if data.shape != darkData.shape:
                logger.warning('Image and dark have different shapes.Cannot normalize. (Dark ref file in %s)' % self._darkHST)
                return (data, srCurrent)
            return (
             (data * srCurrent).astype(numpy.float32) + darkData, srCurrent)

        data = fabio.open(self._refHST).data
        header = fabio.open(self._refHST).header
        tomo_N = utils.getTomo_N(outputdir)
        if tomo_N in (None, -1):
            logger.error("Can't find the number of projection. Fail to create reference")
    ...
    def _getNoRefMessage(self, fileType, dir):
        mess = "No %s found in the given directory %s. Won't be able to copy them" % (
         fileType, dir)
        return mess
    ...
    def _createFlatFieldRef(self, _file):
        ...
        def normalize(data):
            srCurrent = utils.getClosestSRCurrent(scan=(os.path.dirname(_file)), refFile=_file)
            if srCurrent in (None, -1):
                logger.warning("Can't find information about srCurrent,set to default value for normalization")
                srCurrent = self.DEFAULT_SRCURRENT
            ...
            with fabio.open(self._darkHST) as (file_desc):
                darkData = file_desc.data
            if data.shape != darkData.shape:
                err = 'cannot normalize data from %s, has different dimensions' % self._darkHST
                logger.error(err)
                return (data, srCurrent)
            return (
             (data - darkData).astype(numpy.float32) / srCurrent, srCurrent)
    ...
    def _createDarkRef(self, _file):
        ...
        shutil.copy(src=_file, dst=(self._darkHST))
    ...
    def _hasDarkHSTRef(self, scanID):
        return len(DarkRefs.getDarkHSTFiles(scanID, prefix=(self.getDarkHstPrefix()))) > 0

    def _hasRefHSTRef(self, scanID):
        return len(DarkRefs.getRefHSTFiles(scanID, prefix=(self.getRefHstPrefix()))) > 0
