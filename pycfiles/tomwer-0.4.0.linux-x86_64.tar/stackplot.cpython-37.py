...
# [GCC 8.4.0]
# Embedded file name: /users/payno/.local/share/virtualenvs/tomwer_venc/lib/python3.7/site-packages/tomwer/gui/stackplot.py
# Compiled at: 2020-03-03 10:59:33
# Size of source mod 2**32: 25188 bytes
__author__ = [
 'P. Paleo', 'H. Payno']
__license__ = 'MIT'
__date__ = '24/05/2016'
...
class _QImageStackPlot(qt.QWidget):
    ...
    IMG_LOADING = numpy.load(tomwer.resources._resource_filename(('%s.%s' % ('hourglass',
    ...
    def __init__(self, parent, sliderVertical=False):
        ...
        if hasattr(self._plot, 'setAxesDisplayed'):
            ...
            self._qslider = qt.QSlider((qt.Qt.Vertical if sliderVertical is True else qt.Qt.Horizontal),
    ...
    def showImage(self, index):
        ...
        if image is not None:
            if image.data is not None:
                ...
                self._plot.addImage(data=data, legend=(str(index)),
                  replace=True)
...
class _QImageFileStackPlot(_QImageStackPlot):
    ...
    def __init__(self, parent):
        ...
        self._qslider.setToolTip('The loading of the image will start when the             slider will be released')
        self._qspinbox.setToolTip('To start the loading press enter on the spin             box or release the slider')
        self.layout().addWidget(self._QImageFileStackPlot__buildFileInfo())
    ...
    def __buildFileInfo(self):
        ...
        self._openWithImjButton = qt.QPushButton(open_icon, 'open with imagej', parent=self)
        self._openWithImjButton.setSizePolicy(qt.QSizePolicy.Minimum, qt.QSizePolicy.Minimum)
        layout.addWidget(self._openWithImjButton)
    ...
    def _connectModeControl(self, mode):
        if mode == LAZY_LOADING:
            ...
            if mode in (ON_SHOW_LOADING, ASAP_LOADING):
...
LOADING_MODES = (
 LAZY_LOADING, ASAP_LOADING, ON_SHOW_LOADING)
...
class _LoadingModeToolButton(qt.QToolButton):
    __doc__ = 'Tool button to switch keep aspect ratio of a plot'
    sigLoadModeChanged = qt.Signal(str)

    def __init__(self, parent=None):
        super(_LoadingModeToolButton, self).__init__(parent=parent)
        self._states = {ON_SHOW_LOADING: _loadingModeState(icon=(icons.getQIcon('low_speed')), state='load data only when requiring',
                            action='will load data only when requiring'), 
         
         LAZY_LOADING: _loadingModeState(icon=(icons.getQIcon('medium_low_speed')), state='load data when seems convenient',
                         action='will load data when seems convenient'), 
         
         ASAP_LOADING: _loadingModeState(icon=(icons.getQIcon('high_speed')), state='load data as soon as possible',
...
class XAxisInvertedAction(PlotAction):
    __doc__ = 'QAction controlling X orientation on a :class:`.PlotWidget`.\n\n    :param plot: :class:`.PlotWidget` instance on which to operate\n    :param parent: See :class:`QAction`\n    '

    def __init__(self, plot, parent=None):
        self._states = {False:(
          icons.getQIcon('plot-xleft'),
          'Orient X axis left'), 
         True:(
          icons.getQIcon('plot-xright'),
          'Orient X axis right')}
        self._inverted = False
        icon, tooltip = self._states[self.isInverted()]
        super(XAxisInvertedAction, self).__init__(plot,
          icon=icon,
          text='Invert X Axis',
          tooltip=tooltip,
