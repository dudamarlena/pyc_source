...
# [GCC 8.4.0]
# Embedded file name: /users/payno/.local/share/virtualenvs/tomwer_venc/lib/python3.7/site-packages/tomwer/gui/reconstruction/lamino/tofu/tofuexpert.py
# Compiled at: 2019-08-19 02:52:33
# Size of source mod 2**32: 20594 bytes
__authors__ = [
 'H. Payno']
__license__ = 'MIT'
__date__ = '01/06/2018'
...
class ExpertTofuWidget(_TofuOptionLoader, qt.QWidget):
    ...
    def __init__(self, parent):
        ...
        self._retriesGB.layout().addWidget(qt.QLabel('n retries:', parent=(self._retriesGB)), 0, 0)
        ...
        self._retriesGB.layout().addWidget(qt.QLabel('timeout:', parent=(self._retriesGB)), 1, 0)
        ...
        self.getAdditionalPreprocessOptions = self._qteOptions.getAdditionalPreprocessOptions
        self.addAdditionalPreprocessOption = self._qteOptions.addAdditionalPreprocessOption
        self.setAdditionalPreprocessOptions = self._qteOptions.setAdditionalPreprocessOptions
        self.getHighLimit = self._gpuGrp.getHighLimit
        options = {'verbose':_getterSetter(getter=self.requireVerbose, setter=self.setVerbose), 
         'retries':_getterSetter(getter=self.getNRetries, setter=self.setNRetries), 
         'retry-timeout':_getterSetter(getter=self.getNRetries, setter=self.setRetriesTimeout), 
         'slices-per-device':_getterSetter(getter=self.getSlicePerDevice, setter=self.setSlicePerDevice), 
         'slice-memory-coeff':_getterSetter(getter=self.getSliceMemoryCoeff, setter=self.setSliceMemoryCoeff)}
...
class OptionEdit(qt.QWidget):
    ...
    def __init__(self, parent):
        ...
        self._addAdditionalReconsOptions()
        self._tabWidget.addTab(self._qteAddPreprocessOptions, 'Additional preprocess options')
        ...
        self._qteTofuPreprocessHelp = qt.QLabel('', parent=(self._preprocessHelpScrollArea))
        self._addTofuPreprocessHelp()
        self._preprocessHelpScrollArea.setWidget(self._qteTofuPreprocessHelp)
        self._tabWidget.addTab(self._preprocessHelpScrollArea, 'See tofu preprocess help')
        self._qteAddReconsOptions.setToolTip("If you want some options to be passed to tofu reco which are not present in 'options managed' then add the option and value if necessary in the list")
        self._qteAddPreprocessOptions.setToolTip("If you want some options to be passed to tofu preprocess which are not present in 'options managed' then add the option and value if necessary in the list")
        self._qteFixedOptions.setToolTip('This list all the options that are generated automatically with there values (if necessary) by the interface')
    ...
    def _addAdditionalReconsOptions(self):
        self._qteAddReconsOptions.setText(' '.join(('--resize 0', '--projection-padding-mode clamp_to_edge',
                                                    '--genreco-padding-mode clamp_to_edge')))

    def _addAdditionalPreprocessOptions(self):
        self._qteAddPreprocessOptions.setText(' '.join(('--resize 0', '--projection-padding-mode clamp_to_edge',
                                                        '--fix-nan-and-inf', '--projection-filter none')))

    def addAdditionalRecoOption(self, option, value):
        self._qteAddReconsOptions.setText(' '.join((self._qteAddReconsOptions.toPlainText(),
         '--' + option + ' ' + str(value))))

    def addAdditionalPreprocessOption(self, option, value):
        self._qteAddPreprocessOptions.setText(' '.join((self._qteAddPreprocessOptions.toPlainText(),
         '--' + option + ' ' + str(value))))
    ...
    def _getNodeParameters(self):
        ...
        return (
         _ddict, self.getAdditionalRecoOptions())

    def _addTofuRecoHelp(self):
        if has_tofu() is True:
            try:
                process = subprocess.Popen(['tofu', 'reco', '--help'], stdout=(subprocess.PIPE),
    ...
    def _addTofuPreprocessHelp(self):
        if has_tofu() is True:
            try:
                process = subprocess.Popen(['tofu', 'preprocess', '--help'], stdout=(subprocess.PIPE),
                  stderr=(subprocess.PIPE))
                stdout, stderr = process.communicate()
                if len(stdout.decode('utf-8')) > 0:
                ...
                try:
                    ...
                    self._qteTofuPreprocessHelp.setText('Tofu preprocess help not found')
...
class GpuGroupBox(qt.QGroupBox):
    ...
    def __init__(self, parent):
        ...
        self._slicePerDevice.setSizePolicy(qt.QSizePolicy.Expanding, qt.QSizePolicy.Minimum)
        ...
        for _radioButton in (self._slicePerDeviceRB, self._sliceMemoryCoeffRB,
         self._autoRB):
            _radioButton.toggled.connect(self._GpuGroupBox__gpuOptionChange)

        self._slicePerDevice._slicePerDevice.valueChanged.connect(self._GpuGroupBox__gpuOptionChange)
        self._sliceMemoryCoeff.textChanged.connect(self._GpuGroupBox__gpuOptionChange)
