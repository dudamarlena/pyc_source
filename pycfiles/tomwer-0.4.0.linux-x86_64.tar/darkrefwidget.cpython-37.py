...
# [GCC 8.4.0]
# Embedded file name: /users/payno/.local/share/virtualenvs/tomwer_venc/lib/python3.7/site-packages/tomwer/gui/reconstruction/darkref/darkrefwidget.py
# Compiled at: 2020-03-06 02:01:31
# Size of source mod 2**32: 16260 bytes
__authors__ = [
 'H. Payno']
__license__ = 'MIT'
__date__ = '10/01/2018'
...
class _WhatCheckBox(qt.QWidget):
    ...
    def __init__(self, parent, text):
        ...
        for mode in (dkrf.Method.median, dkrf.Method.average):
...
class _TabGeneral(qt.QWidget):
    __doc__ = 'Widget with the general information for dark and ref process'
    ...
    def __init__(self, parent):
        ...
        self._rmOptionCB = qt.QCheckBox(parent=(self._grpOptions), text='remove raw files when done')
        self.sigRmToggled = self._rmOptionCB.toggled
        self._skipOptionCB = qt.QCheckBox(parent=(self._grpOptions), text='skip if already existing')
...
class _TabExpert(qt.QWidget):
    __doc__ = 'Expert process for dark and ref'
    ...
    def __init__(self, parent):
        ...
        self._patternsWidget.layout().addWidget(qt.QLabel('dark file pattern', parent=(self._patternsWidget)), 0, 0)
        ...
        self._patternsWidget.layout().addWidget(qt.QLabel('ref file pattern', parent=(self._patternsWidget)), 1, 0)
...
class DarkRefTab(qt.QTabWidget):
    ...
    def __init__(self, parent, reconsparams):
        ...
        extraSandBoxInfo = qt.QLabel('This sand box allow you to play withregular expressions and check the validity of a defined pattern versus acquisition names. This way you can be insure of the regular expression behavior', parent=(self.tabSandBox))
    ...
    def _disconnectToReconsParams(self):
        ...
        try:
            self.tabGeneral.sigDarkChanged.disconnect(self.recons_params.__class__.dark_calc_method.setter)
            self.tabGeneral.sigRefChanged.disconnect(self.recons_params.__class__.ref_calc_method.setter)
            self.tabGeneral.sigRmToggled.disconnect(self.recons_params._set_remove_opt)
            self.tabGeneral.sigSkipToggled.disconnect(self.recons_params._set_skip_if_exist)
            self.tabExpert.sigDarkPatternEdited.disconnect(self.recons_params.__class__.dark_pattern.setter)
            self.tabExpert.sigRefPatternEdited.disconnect(self.recons_params.__class__.ref_pattern.setter)
    ...
    def _connectToReconsParams(self):
        assert self.recons_params
        self.tabGeneral.sigDarkChanged.connect(self.recons_params.__class__.dark_calc_method.setter)
        self.tabGeneral.sigRefChanged.connect(self.recons_params.__class__.ref_calc_method.setter)
        ...
        self.tabExpert.sigDarkPatternEdited.connect(self.recons_params.__class__.dark_pattern.setter)
        self.tabExpert.sigRefPatternEdited.connect(self.recons_params.__class__.ref_pattern.setter)
    ...
    def setReconsParams(self, recons_params):
        if not isinstance(recons_params, (QReconsParams, _QDKRFRP)):
    ...
    def loadStructs(self, structs):
        ...
        def warningKeyNotHere(key):
            logger.warning('%s key not present in the given struct, cannot load value for it.' % key)
