...
# [GCC 8.4.0]
# Embedded file name: /users/payno/.local/share/virtualenvs/tomwer_venc/lib/python3.7/site-packages/tomwer/gui/reconstruction/lamino/tofu/projections.py
# Compiled at: 2020-03-06 02:01:31
# Size of source mod 2**32: 35873 bytes
__authors__ = [
 'H. Payno']
__license__ = 'MIT'
__date__ = '01/06/2018'
...
class InputProjectionsWidget(_TofuOptionLoader, qt.QWidget):
    __doc__ = 'TabWidget containing all the information relative to the\n    flat field correction'
    ...
    def __init__(self, parent):
        ...
        options = {'ffc-when':_getterSetter(getter=self.getWhenApplyFFC, setter=self.setWhenApplyFFC), 
         'number':_getterSetter(getter=self.getNumber, setter=self.setNumber), 
         'reduction-mode':_getterSetter(getter=self._ffcWidget.getMethod, setter=self._ffcWidget.setMethod), 
         'darks':_getterSetter(getter=self._ffcWidget.getDarks, setter=self._ffcWidget.setDarks), 
         'flats':_getterSetter(getter=self._ffcWidget.getFlats, setter=self._ffcWidget.setFlats), 
         'flats2':_getterSetter(getter=self._ffcWidget.getSecondFlats, setter=self._ffcWidget.setSecondFlats), 
         'absorptivity':_getterSetter(getter=self._ffcWidget.requireAbsorptivity, setter=self._ffcWidget.setAbsortivity), 
         'fix-nan-and-inf':_getterSetter(getter=self._ffcWidget.removeNanAndInf, setter=self._ffcWidget.setNanAndInf), 
         'dark-scale':_getterSetter(getter=self._ffcWidget.getDarkScale, setter=self._ffcWidget.setDarkScale), 
         'axis-angle-x':_getterSetter(getter=self.rotationAngle._laminoAngle.getAngle, setter=self.rotationAngle._laminoAngle.setAngle), 
         'axis-angle-y':_getterSetter(getter=self.rotationAngle._axisAngleY.getAngle, setter=self.rotationAngle._axisAngleY.setAngle), 
         'axis-angle-z':_getterSetter(getter=self.rotationAngle._axisAngleZ.getAngle, setter=self.rotationAngle._axisAngleZ.setAngle), 
         'center-position-x':_getterSetter(getter=self.centeringWidget.getXCenter, setter=self.centeringWidget.setXCenter), 
         'center-position-z':_getterSetter(getter=self.centeringWidget.getZCenter, setter=self.centeringWidget.setZCenter), 
         'overall-angle':_getterSetter(getter=self.rotationAngle._overallAngle.getAngle, setter=self.rotationAngle._setOverallAngleI), 
         'pixel-size':_getterSetter(getter=self.getPixelSize, setter=self.setPixelSize), 
         'half-acquisition':_getterSetter(getter=self.isHalfAcquisition, setter=self._halfAcqCB.setChecked), 
         'blend':_getterSetter(getter=self.isBlend, setter=self.setBlend), 
         'adjust-mean':_getterSetter(getter=self.isAdjustMean, setter=self.setAdjustMean)}
        _TofuOptionLoader.__init__(self, options=options, childs=[
         self._phaseRetrieval])
    ...
    def loadFromScan(self, scanID):
        ...
        if pixelSize is not None:
            if self.pixelSizeIsLocked() is False:
                self.setPixelSize(float(pixelSize))


class _FFCConfigWidget(qt.QGroupBox):
    __doc__ = 'Widget dedicated to the parameters used for the flat field correction'
    ...
    def __init__(self, parent):
        ...
        self._FFCConfigWidget__absortivityLabel = qt.QLabel('absorptivity (*)')
        self._FFCConfigWidget__absortivityLabel.setToolTip('Activating absortivity will deactivate phase retrieval')
        self.layout().addWidget(self._FFCConfigWidget__absortivityLabel, 2, 0)
...
class _FFCWhenWidget(qt.QWidget):
    ...
    def __init__(self, parent):
        ...
        self._onTheFlyQRB.setToolTip('process flat field correction during reconstruction. Avoid multiple io read.')
        ...
        self._preProcQRB.setToolTip('make flat field on a preprocessing step. Can be interesting if you intend to test several reconstruction parameters')
...
class CenteringTofuGroupBox(qt.QGroupBox):
    ...
    def __init__(self, parent):
        ...
        self._lockButton.setSizePolicy(qt.QSizePolicy.Expanding, qt.QSizePolicy.Expanding)
...
class AngleIllustrations(qt.QWidget):
    ...
    def __init__(self, parent):
        ...
        self._laminoAngleIllustration = qt.QWidget()
        self._laminoAngleIllustration = _IllustrationWidget(parent=self, img='lamino_angle')
        ...
        self._illustrations.setSizePolicy(qt.QSizePolicy.Expanding, qt.QSizePolicy.Expanding)
    ...
    def setActiveIllustration(self, name):
        assert name in ('lamino angle', PSI_CHAR + ' angle',
         THETA_CHAR + ' angle')
...
class RotationAngleGroupBox(qt.QGroupBox):
    ...
    def __init__(self, parent):
        ...
        self._laminoAngle = _AngleWidget(parent=(self._angles), name='rotation axis tilt',
          defaultVal=90,
          lockable=True,
          information=(self._showLaminoAngleInfo))
        self._angles.layout().addWidget(self._laminoAngle)
        self._axisAngleY = _AngleWidget(parent=(self._angles), name=(PSI_CHAR + ' angle'),
          lockable=True,
          information=(self._showPsiAngleInfo))
        self._angles.layout().addWidget(self._axisAngleY)
        self._axisAngleZ = _AngleWidget(parent=(self._angles), name=(THETA_CHAR + ' angle'),
          lockable=True)
        self._angles.layout().addWidget(self._axisAngleZ)
        self._axisAngleZ.setVisible(False)
        self._overallAngle = _AngleWidget(parent=self, name='overall angle', defaultVal=360,
          lockable=True)
    ...
    def getInfoLaminoDialog(self):
        if self._infoLamino is None:
            self._infoLamino = _IllustrationDialog(parent=self, img='lamino_angle',
              title='information about lamino angle')
        return self._infoLamino

    def getPsiLaminoDialog(self):
        if self._infoPsi is None:
            self._infoPsi = _IllustrationDialog(parent=self, img='psi_angle',
              title=(' '.join(('information about',
             PSI_CHAR,
             'angle'))))
    ...
    def loadFromScan(self, scanID):
        ...
        if os.path.isfile(_info_file) is True:
            ...
            if self._laminoAngle.isLocked() is False:
                if 'ctangle' in info:
                    self._laminoAngle.setAngle(90.0 - float(info['ctangle']))
            if self._overallAngle.isLocked() is False:
                if 'scanrange' in info:
...
class PhaseRetrievalWidget(_TofuOptionLoader, qt.QGroupBox):
    ...
    def __init__(self, parent):
        ...
        self._retrievalMethodCB.setCurrentIndex(self._retrievalMethodCB.findText(self.DEFAULT_METHOD))
        ...
        self.toggled.connect(self._PhaseRetrievalWidget__toggleCallback)
        options = {'retrieval-method':_getterSetter(getter=self._getRetrievalMethod,
           setter=self.setRetrievalMethod), 
         'energy':_getterSetter(getter=self._getEnergy, setter=self.setEnergy), 
         'propagation-distance':_getterSetter(getter=self._getPropagationDistance,
           setter=self.setPropagationDistance), 
         'regularization-rate':_getterSetter(getter=self._getRegularizationRate,
           setter=self.setRegularizationRate), 
         'thresholding-rate':_getterSetter(getter=self._getThresholdingRate,
           setter=self.setThresholdingRate)}
    ...
    def loadFromScan(self, scanID):
        ...
        if os.path.isfile(_info_file) is True:
            ...
            if energy is not None:
                if self.energyIsLocked() is False:
