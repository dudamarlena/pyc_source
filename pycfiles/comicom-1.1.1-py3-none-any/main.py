...
# Embedded file name: build/bdist.macosx-10.6-universal/egg/comicnamer/main.py
# Compiled at: 2010-09-01 14:11:05
__doc__ = 'Main comicnamer utility functionality\nModified from http://github.com/dbr/tvnamer\n'
import os, logging
try:
    import readline
except ImportError:
...
from utils import Config, FileFinder, FileParser, Renamer, warn, getIssueName, applyCustomInputReplacements, applyCustomOutputReplacements, formatIssueNumbers, makeValidFilename
from comicnamer_exceptions import SeriesNotFound, IssueNotFound, IssueNameNotFound, UserAbort, InvalidPath, NoValidFilesFoundError, InvalidFilename, DataRetrievalError
...
def getDestinationFolder(issue):
    ...
    """
    destdir = Config['move_files_destination'] % {'seriesname': makeValidFilename(issue.seriesname), 
...
def findFiles(paths):
    ...
    valid_files = []
    for cfile in paths:
        cur = FileFinder(cfile, with_extension=Config['valid_extensions'], recursive=Config['recursive'])
        try:
...
def comicnamer(paths):
    ...
    if Config['move_files_enable']:
        import warnings
        warnings.warn('The move_files feature is still under development. Be very careful with it.\nIt has not been heavily tested, and is not recommended for general use yet.')
    p('####################')
...
def main():
    ...
    if opts.saveconfig is not None:
        ...
        json.dump(configToSave, open(opts.saveconfig, 'w+'), sort_keys=True, indent=4)
        opter.exit(0)
    if opts.showconfig:
