...
# [GCC 8.4.0]
# Embedded file name: /home/cbe-master/realfast/anaconda/envs/deployment/lib/python2.7/site-packages/rtpipe/reproduce.py
# Compiled at: 2017-03-12 11:38:54
import numpy as np, rtpipe.RT as rt, rtpipe.parseparams as pp, rtpipe.parsecands as pc, pickle, logging, os, matplotlib.pyplot as plt
from matplotlib.backends.backend_agg import FigureCanvasAgg
import numpy.ma as ma, matplotlib
...
def plot_cand(candsfile, candloc=[], candnum=-1, threshold=0, savefile=True, returndata=False, outname='', newplot=True, returnstate=False, **kwargs):
    ...
    if 'memory_limit' in d:
        ...
        im, data = rt.pipeline_reproduce(d, candloc, product='imdata')
        if savefile:
            loclabel = (
...
def list_cands(candsfile, threshold=0.0):
...
def refine_cands(candsfile, threshold=0, scaledm=2.1, scalepix=2, scaleuv=1.0, chans=[], savepkl=True):
    ...
    if 'snr2' in d['features']:
        ...
        for i, snr in enumerate(snrs):
            if snr > 0:
                d, cands = refine_cand(candsfile, threshold=threshold, candnum=i, scaledm=scaledm, scalepix=scalepix, scaleuv=scaleuv, chans=chans)
                if cands:
...
def make_cand_plot(d, im, data, loclabel, version=2, snrs=[], outname=''):
    ...
    l1arcm = l1 * 180.0 * 60.0 / np.pi
    m1arcm = m1 * 180.0 * 60.0 / np.pi
    if version == 1:
        ...
        dd = np.concatenate((spectra[(Ellipsis, 0)], np.zeros_like(spectra[(Ellipsis, 0)]), spectra[(Ellipsis, 1)]), axis=1)
        ...
        fov = np.degrees(1.0 / d['uvres']) * 60.0
        ...
        if version == 2:
            ...
            ax.text(left, start - 8 * space, 'dt = ' + str(np.round(d['inttime'] * d['dtarr'][dtind], 3) * 1000.0) + ' ms' + ' (index ' + str(dtind) + ')', fontname='sans-serif', transform=ax.transAxes, fontsize='small')
            ...
            left, width = 0.75, 0.4 / 3.0
            bottom, height = (0.2, 0.7)
            rect_dynsp1 = [left, bottom, width / 3.0, height]
            rect_dynsp2 = [left + width / 3.0, bottom, width / 3.0, height]
            rect_dynsp3 = [left + 2.0 * width / 3.0, bottom, width / 3.0, height]
            rect_lc1 = [left, bottom - 0.1, width / 3.0, 0.1]
            rect_lc2 = [left + width / 3.0, bottom - 0.1, width / 3.0, 0.1]
            rect_lc3 = [left + 2.0 * width / 3.0, bottom - 0.1, width / 3.0, 0.1]
            rect_sp = [left + width, bottom, 0.2 / 3.0, height]
            ...
            spectra = np.swapaxes(data.real, 0, 1)
            dd1 = spectra[(Ellipsis, 0)]
            dd2 = spectra[(Ellipsis, 0)] + spectra[(Ellipsis, 1)]
            dd3 = spectra[(Ellipsis, 1)]
            colormap = 'viridis'
            ...
            ax_sp = fig.add_axes(rect_sp, sharey=ax_dynsp3)
            [ label.set_visible(False) for label in ax_sp.get_yticklabels() ]
            n = int((2.0 * len(spectra) ** 0.5 / snrobs) ** 2)
            if n == 0:
                dd1avg = dd1
                dd3avg = dd3
            else:
            ...
            ax_dynsp3.set_xlabel('LL')
            ax_dynsp3.xaxis.set_label_position('top')
            ax_sp.plot(dd2avgcrop[:, len(dd2avgcrop[0]) / 2] / 2.0, range(len(dd2avgcrop)), 'k.')
            ...
            sbeam = np.degrees(d['uvoversample'] / uvdist) * 60.0
            xratio = len(im[0]) / fov
            yratio = len(im) / fov
            mult = 5
            ...
            if len(snrs):
                ...
                bins = np.array([ (bin_edges[i] + bin_edges[(i + 1)]) / 2.0 for i in range(len(vals)) ])
                vals = np.array(vals)
                if not nonegs:
...
def convertloc(candsfile, candloc, memory_limit):
    ...
    nskip0 = (86400 * (d0['segmenttimes'][(segment, 0)] - d0['starttime_mjd']) / d0['inttime']).astype(int)
    ...
    readints = d['readints']
    nskips = [ (86400 * (d['segmenttimes'][(segment, 0)] - d['starttime_mjd']) / d['inttime']).astype(int) for segment in range(d['nsegments'])
             ]
    ...
    return [
     scan, segment_new, candint_new, dmind, dtind, beamnum]
