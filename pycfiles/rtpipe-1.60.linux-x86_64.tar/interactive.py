...
# [GCC 8.4.0]
# Embedded file name: /home/cbe-master/realfast/anaconda/envs/deployment/lib/python2.7/site-packages/rtpipe/interactive.py
# Compiled at: 2017-06-21 14:33:50
...
def plotall(data, circleinds=[], crossinds=[], edgeinds=[], htmlname=None, noiseplot=None, url_path='plots', fileroot=None):
    ...
    """
    if not circleinds:
        circleinds = calcinds(data, np.abs(data['snrs']).min())
    if not crossinds:
    ...
    imkur_min = min(imkur)
    imkur_max = max(imkur)
    dmt = Figure(plot_width=950, plot_height=500, toolbar_location='left', x_axis_label='Time (s; relative)', y_axis_label='DM (pc/cm3)', x_range=(time_min, time_max), y_range=(dm_min, dm_max), output_backend='webgl', tools=TOOLS)
    loc = Figure(plot_width=450, plot_height=400, toolbar_location='left', x_axis_label='l1 (rad)', y_axis_label='m1 (rad)', x_range=(
     l1_min, l1_max), y_range=(m1_min, m1_max), tools=TOOLS, output_backend='webgl')
    stat = Figure(plot_width=450, plot_height=400, toolbar_location='left', x_axis_label='Spectral std', y_axis_label='Image kurtosis', x_range=(specstd_min, specstd_max), y_range=(
     imkur_min, imkur_max), tools=TOOLS, output_backend='webgl')
    norm = Figure(plot_width=450, plot_height=400, toolbar_location='left', x_axis_label='SNR observed', y_axis_label='SNR expected', tools=TOOLS, output_backend='webgl')
    source = ColumnDataSource(data=dict({(key, tuple([ value[i] for i in circleinds if i not in edgeinds ])) for key, value in data.iteritems()}))
    ...
    if edgeinds:
        sourceedge = ColumnDataSource(data=dict({(key, tuple([ value[i] for i in edgeinds ])) for key, value in data.iteritems()}))
...
def plotdmt(data, circleinds=[], crossinds=[], edgeinds=[], url_path=None, fileroot=None, tools='hover,tap,pan,box_select,wheel_zoom,reset', plot_width=950, plot_height=500):
    """ Make a light-weight dm-time figure """
    fields = [
     'dm', 'time', 'sizes', 'colors', 'snrs', 'key']
    if not circleinds:
        circleinds = range(len(data['snrs']))
    datalen = len(data['dm'])
    inds = circleinds + crossinds + edgeinds
    dm = [ data['dm'][i] for i in inds ]
    dm_min = min(min(dm), max(dm) / 1.2)
    ...
    time_min = min(time)
    time_max = max(time)
    source = ColumnDataSource(data=dict({(key, tuple([ value[i] for i in circleinds if i not in edgeinds ])) for key, value in data.iteritems() if key in fields}))
    dmt = Figure(plot_width=plot_width, plot_height=plot_height, toolbar_location='left', x_axis_label='Time (s; relative)', y_axis_label='DM (pc/cm3)', x_range=(time_min, time_max), y_range=(dm_min, dm_max), output_backend='webgl', tools=tools)
    dmt.circle('time', 'dm', size='sizes', fill_color='colors', line_color=None, fill_alpha=0.2, source=source)
    if crossinds:
        sourceneg = ColumnDataSource(data=dict({(key, tuple([ value[i] for i in crossinds ])) for key, value in data.iteritems() if key in fields}))
        dmt.cross('time', 'dm', size='sizes', fill_color='colors', line_alpha=0.3, source=sourceneg)
    if edgeinds:
        sourceedge = ColumnDataSource(data=dict({(key, tuple([ value[i] for i in edgeinds ])) for key, value in data.iteritems() if key in fields}))
        dmt.circle('time', 'dm', size='sizes', line_color='colors', fill_color='colors', line_alpha=0.5, fill_alpha=0.2, source=sourceedge)
    hover = dmt.select(dict(type=HoverTool))
    hover.tooltips = OrderedDict([('SNR', '@snrs'), ('key', '@key')])
    if url_path and fileroot:
...
def plotloc(data, circleinds=[], crossinds=[], edgeinds=[], url_path=None, fileroot=None, tools='hover,tap,pan,box_select,wheel_zoom,reset', plot_width=450, plot_height=400):
    ...
    m1_max = max(m1)
    source = ColumnDataSource(data=dict({(key, tuple([ value[i] for i in circleinds if i not in edgeinds ])) for key, value in data.iteritems() if key in fields}))
    loc = Figure(plot_width=plot_width, plot_height=plot_height, toolbar_location='left', x_axis_label='l1 (rad)', y_axis_label='m1 (rad)', x_range=(
     l1_min, l1_max), y_range=(m1_min, m1_max), tools=tools, output_backend='webgl')
    loc.circle('l1', 'm1', size='sizes', line_color=None, fill_color='colors', fill_alpha=0.2, source=source)
    if crossinds:
        sourceneg = ColumnDataSource(data=dict({(key, tuple([ value[i] for i in crossinds ])) for key, value in data.iteritems() if key in fields}))
...
def plotstat(data, circleinds=None, crossinds=None, edgeinds=None, url_path=None, fileroot=None, tools='hover,tap,pan,box_select,wheel_zoom,reset', plot_width=450, plot_height=400):
    ...
    fields = [
     'imkur', 'specstd', 'sizes', 'colors', 'snrs', 'key']
    if not circleinds:
        circleinds = range(len(data['snrs']))
    datalen = len(data['dm'])
    inds = circleinds + crossinds + edgeinds
    ...
    imkur_max = max(imkur)
    source = ColumnDataSource(data=dict({(key, tuple([ value[i] for i in circleinds if i not in edgeinds ])) for key, value in data.iteritems() if key in fields}))
    stat = Figure(plot_width=plot_width, plot_height=plot_height, toolbar_location='left', x_axis_label='Spectral std', y_axis_label='Image kurtosis', x_range=(specstd_min, specstd_max), y_range=(
     imkur_min, imkur_max), tools=tools, output_backend='webgl')
    stat.circle('specstd', 'imkur', size='sizes', line_color=None, fill_color='colors', fill_alpha=0.2, source=source)
    if crossinds:
        sourceneg = ColumnDataSource(data=dict({(key, tuple([ value[i] for i in crossinds ])) for key, value in data.iteritems() if key in fields}))
...
def plotnorm(data, circleinds=[], crossinds=[], edgeinds=[], url_path=None, fileroot=None, tools='hover,tap,pan,box_select,wheel_zoom,reset', plot_width=450, plot_height=400):
    ...
    fields = [
     'zs', 'sizes', 'colors', 'abssnr', 'key', 'snrs']
    if not circleinds:
        circleinds = range(len(data['snrs']))
    source = ColumnDataSource(data=dict({(key, tuple([ value[i] for i in circleinds if i not in edgeinds ])) for key, value in data.iteritems() if key in fields}))
    norm = Figure(plot_width=plot_width, plot_height=plot_height, toolbar_location='left', x_axis_label='SNR observed', y_axis_label='SNR expected', tools=tools, output_backend='webgl')
    norm.circle('abssnr', 'zs', size='sizes', line_color=None, fill_color='colors', fill_alpha=0.2, source=source)
    if crossinds:
        sourceneg = ColumnDataSource(data=dict({(key, tuple([ value[i] for i in crossinds ])) for key, value in data.iteritems() if key in fields}))
        norm.cross('abssnr', 'zs', size='sizes', line_color='colors', line_alpha=0.3, source=sourceneg)
...
def plotnoisecum(noisepkl, fluxscale=1, plot_width=450, plot_height=400):
    ...
    noiseplot = Figure(plot_width=plot_width, plot_height=plot_height, toolbar_location='above', x_axis_label=('Image noise (Jy; cal scaling {0:.3})').format(fluxscale), y_axis_label='Cumulative fraction', tools='pan, wheel_zoom, reset')
...
def plotnoisedist(noisepkl, plot_width=450, plot_height=400):
    ...
    pl = Figure(plot_width=plot_width, plot_height=plot_height, toolbar_location='above', x_axis_label='Flag fraction', y_axis_label='Image noise (sys)', tools='pan, wheel_zoom, reset')
    ...
    zeronoisefrac = float(len(np.where(imstd == 0.0)[0])) / len(imstd)
    logger.info(('{0:.0%} of noise images are zeros').format(zeronoisefrac))
    return (
     pl, imstd_med, flagfrac_med)
...
def calcfluxscale(d, imstd_med, flagfrac_med):
    ...
    eta = {'L': 0.92, 'S': 0.92, 'C': 0.8, 'X': 0.8}
    sefd = {'L': 420, 'S': 370, 'C': 310, 'X': 250}
...
def readdata(mergepkl=None, d=None, cands=None, sizerange=(2, 70)):
    ...
    l1 = prop[:, l1col]
    m1 = prop[:, m1col]
    time = np.array([ 86400 * d['segmenttimesdict'][scan[i]][(seg[i], 0)] + d['inttime'] * candint[i] for i in range(len(loc)) ])
    ...
    zs = normprob(d, snrs)
    data = dict(snrs=snrs, dm=dm, l1=l1, m1=m1, time=time, specstd=specstd, scan=scan, imkur=imkur, sizes=sizes, colors=colors, key=key, zs=zs, abssnr=abssnr)
    return data
...
def normprob(d, snrs, inds=None, version=2):
    ...
    if d.has_key('goodintcount'):
        ...
        nints = d['nints']
    ndms = len(d['dmarr'])
    dtfactor = np.sum([ 1.0 / i for i in d['dtarr'] ])
...
def colorsat(l, m):
    ...
    green = 0.5 * (1 + np.cos(np.angle(lm) + 6.28 / 3))
    blue = 0.5 * (1 + np.cos(np.angle(lm) - 6.28 / 3))
...
def filterdata(data, plinds, d, threshold, ignorestr):
    ...
    plinds['cir'] = calcinds(data, thresh0, ignoret=ignoret)
    plinds['cro'] = calcinds(data, -1 * thresh0, ignoret=ignoret)
    plinds['edg'] = calcinds(data, thresh1, ignoret=ignoret)
    ...
    logger.info(('Estimated total on target time: {} s\n').format(calcontime(data, inds=plinds['cir'] + plinds['cro'] + plinds['edg'])))
...
def parseignoret(ignorestr):
    if ',' in ignorestr:
        ...
        assert (len(ignorelist) / 2.0).is_integer(), 'ignorestr be pairs of comma-delimited values.'
...
def displayplot(data, plinds, plottype, scaling, fileroot, url_path='http://www.aoc.nrao.edu/~claw/plots'):
    ...
    plotdict = {'dmt': plotdmt, 'norm': plotnorm, 'loc': plotloc, 
       'stat': plotstat, 'all': plotall}
    sizedict = {'dmt': [900, 500], 'norm': [700, 700], 'loc': [700, 700], 'stat': [
              700, 700]}
    sortinds = sorted(set(plinds['cir'] + plinds['cro'] + plinds['edg']))
    ...
    if plottype != 'all':
        ...
        pl = plotdict[plottype](data, circleinds=plinds['cir'], crossinds=plinds['cro'], edgeinds=plinds['edg'], url_path=url_path, fileroot=fileroot, plot_width=wid, plot_height=hei)
    else:
        pl = plotall(data, circleinds=plinds['cir'], crossinds=plinds['cro'], edgeinds=plinds['edg'], url_path=url_path, fileroot=fileroot)
    hdl = show(pl)
