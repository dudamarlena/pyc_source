...
# [GCC 8.4.0]
# Embedded file name: /home/cbe-master/realfast/anaconda/envs/deployment/lib/python2.7/site-packages/rtpipe/parsesdm.py
# Compiled at: 2017-09-19 17:39:36
import logging
...
import pwkit.environments.casa.tasks as tasklib, pwkit.environments.casa.util as casautil, sdmpy, rtpipe.parseparams as pp
...
def get_metadata(filename, scan, paramfile='', **kwargs):
    ...
    d['radec'] = [ (prop['ra'], prop['dec']) for sr, prop in sources.iteritems() if prop['source'] == d['source']
                 ][0]
    ...
    d['freq_orig'] = np.array([ np.mean(spwch[i:i + d['read_fdownsample']]) for i in range(0, len(spwch), d['read_fdownsample'])
                              ], dtype='float32') / 1000000000.0
    if not len(d['chans']):
    ...
    u = u * d['freq_orig'][0] * (1000000000.0 / 300000000.0) * -1
    v = v * d['freq_orig'][0] * (1000000000.0 / 300000000.0) * -1
    d['urange'][d['scan']] = u.max() - u.min()
    d['vrange'][d['scan']] = v.max() - v.min()
    d['dishdiameter'] = float(str(sdm['Antenna'][0].dishDiameter).strip())
    d['uvres_full'] = np.round(d['dishdiameter'] / (0.3 / d['freq'].min()) / 2).astype('int')
    if not all(('npixx_full' in d, 'npixy_full' in d)):
        ...
        largerx = np.where(powers - rangex / d['uvres_full'] > 0, powers, powers[(-1,
                                                                                  -1)])
        p2x, p3x = np.where(largerx == largerx.min())
        largery = np.where(powers - rangey / d['uvres_full'] > 0, powers, powers[(-1,
                                                                                  -1)])
    ...
    if 'VLA' in str(sdm['ExecBlock'][0]['telescopeName']):
        configid = [ str(row.configDescriptionId) for row in sdm['Main'] if d['scan'] == int(row.scanNumber)
                   ][0]
        antids = [ str(row.antennaId) for row in sdm['ConfigDescription'] if configid == row.configDescriptionId
                 ][0].split(' ')[2:]
        d['ants'] = [ int(str(row.name).lstrip('ea')) for antid in antids for row in sdm['Antenna'] if antid == str(row.antennaId)
                    ]
    else:
        if 'GMRT' in str(sdm['ExecBlock'][0]['telescopeName']):
    ...
    d['pols_orig'] = [ pol for pol in str(sdm['Polarization'][0].corrType).strip().split(' ') if pol in ('XX',
                                                                                                         'YY',
                                                                                                         'XY',
                                                                                                         'YX',
                                                                                                         'RR',
...
def read_bdf_segment(d, segment=-1):
    ...
    if segment != -1:
        ...
        assert segment < d['nsegments'], 'segment %d is too big for nsegments %d' % (segment,
         d['nsegments'])
        readints = d['readints']
        ...
        logger.info('Reading scan %d, segment %d/%d, times %s to %s' % (
         ...
         qa.time(qa.quantity(d['segmenttimes'][(segment, 0)], 'd'), form=[
          'hms'], prec=9)[0],
         qa.time(qa.quantity(d['segmenttimes'][(segment, 1)], 'd'), form=[
          'hms'], prec=9)[0]))
    else:
        nskip = 0
        readints = 0
    ...
    if d['applyonlineflags'] and segment > -1:
        sdm = getsdm(d['filename'], bdfdir=d['bdfdir'])
        allantdict = dict(zip([ str(ant.antennaId) for ant in sdm['Antenna'] ], [ int(str(ant.name).lstrip('ea')) for ant in sdm['Antenna']
                                                                                ]))
        antflags = [ (allantdict[str(flag.antennaId).split(' ')[2]], int(flag.startTime) / 86400000000000.0, int(flag.endTime) / 86400000000000.0) for flag in sdm['Flag']
                   ]
        logger.info('Found online flags for %d antenna/time ranges.' % len(antflags))
        blarr = calc_blarr(d)
        timearr = np.linspace(d['segmenttimes'][segment][0], d['segmenttimes'][segment][1], d['readints'])
        badints_cum = []
        for antflag in antflags:
            ...
            for badint in badints:
                data[(badint, badbls)] = complex(0.0, 0.0)
    ...
    dfreq = np.array([ d['spw_reffreq'][(i + 1)] - d['spw_reffreq'][i] for i in range(len(d['spw_reffreq']) - 1)
                     ])
    dfreqneg = [ df for df in dfreq if df < 0 ]
    if len(dfreqneg) <= 1:
        if len(dfreqneg) == 1:
            logger.info('Rolling spw frequencies to increasing order: %s' % str(d['spw_reffreq']))
            rollch = np.sum([ d['spw_nchan'][ss] for ss in range(np.where(dfreq < 0)[0][0] + 1)
                            ])
    ...
    if d['read_tdownsample'] > 1 or d['read_fdownsample'] > 1:
        ...
        if d['read_fdownsample'] > 1:
            ...
            for i in range(fsize):
                data2[:, :, i, :] = data[:, :,
                 i * d['read_fdownsample']:(i + 1) * d['read_fdownsample']].mean(axis=2)
...
def calc_uvw(sdmfile, scan=0, datetime=0, radec=(), bdfdir=''):
    ...
    if datetime == 0 and len(radec) == 0:
        ...
        positions = [ str(station.position).strip().split(' ') for station in sdm['Station'] if station.stationId in stationidlist
                    ]
...
def get_uvw_segment(d, segment=-1):
    ...
    if segment != -1:
        ...
        datetime = qa.time(qa.quantity((t1 + t0) / 2, 'd'), form=[
         'ymdhms'], prec=9)[0]
        logger.debug('Calculating uvw for segment %d' % segment)
    else:
        datetime = 0
    u, v, w = calc_uvw(d['filename'], d['scan'], datetime=datetime, bdfdir=d['bdfdir'])
    u = u * d['freq_orig'][0] * (1000000000.0 / 300000000.0) * -1
    v = v * d['freq_orig'][0] * (1000000000.0 / 300000000.0) * -1
    w = w * d['freq_orig'][0] * (1000000000.0 / 300000000.0) * -1
    return (
     u.astype('float32'), v.astype('float32'), w.astype('float32'))
...
def read_scans(sdmfile, bdfdir=''):
    ...
    for scan in sdm.scans():
        ...
        try:
            ...
            endmjd = startmjd + nints * interval / 86400
