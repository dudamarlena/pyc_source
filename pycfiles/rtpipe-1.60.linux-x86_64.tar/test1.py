...
# [GCC 8.4.0]
# Embedded file name: /home/cbe-master/realfast/anaconda/envs/deployment/lib/python2.7/site-packages/rtpipe/test1.py
# Compiled at: 2016-10-24 18:03:38
import realtime.parsesdm as ps, realtime.RT as rt, realtime.parsecal as pc, realtime.rtlib_cython as rtlib, argparse, numpy as n, multiprocessing.sharedctypes as mps
...
if test == 'all' or test == 'wterm':
    ...
    readints = n.round(86400 * (t1 - t0) / state['inttime'], 0).astype(int) / state['read_downsample']
    data_mem = mps.RawArray(mps.ctypes.c_float, long(readints * state['nbl'] * state['nchan'] * state['npol']) * 2)
    ...
    if state['gainfile']:
        ...
        sols.setselection(state['segmenttimes'][segment].mean(), state['freq'] * 1000000000.0)
    ...
    if state['dmarr'][0] > 0:
        dt = 1
        rtlib.dedisperse_resample(data, state['freq'], state['inttime'], state['dmarr'][0], dt, verbose=1)
...
# global data ## Warning: Unused global