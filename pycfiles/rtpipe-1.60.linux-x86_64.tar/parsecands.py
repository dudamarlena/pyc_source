...
# [GCC 8.4.0]
# Embedded file name: /home/cbe-master/realfast/anaconda/envs/deployment/lib/python2.7/site-packages/rtpipe/parsecands.py
# Compiled at: 2017-02-28 18:18:55
...
def read_candidates(candsfile, snrmin=0, snrmax=999, returnstate=False):
    ...
    if isinstance(cands, dict):
        ...
        loc, prop = cands
        if not (isinstance(loc, np.ndarray) and isinstance(prop, np.ndarray)):
            raise AssertionError('if cands object is tuple, contents must be two ndarrays')
            if len(loc) or returnstate:
                return (loc, prop, d)
            return (loc, prop)
...
def read_noise(noisefile):
    ...
    if len(noises[0]) == 4:
        for noise in noises:
            seg.append(noise[0])
            noiseperbl.append(noise[1])
            flagfrac.append(noise[2])
            imnoise.append(noise[3])

        return (
         np.array(seg), np.array(noiseperbl), np.array(flagfrac), np.array(imnoise))
    if len(noises[0]) == 5:
        for noise in noises:
            scan.append(noise[0])
            seg.append(noise[1])
            noiseperbl.append(noise[2])
            flagfrac.append(noise[3])
            imnoise.append(noise[4])

        return (
         np.array(scan), np.array(seg), np.array(noiseperbl), np.array(flagfrac), np.array(imnoise))
...
def merge_segments(filename, scan, cleanup=True, sizelimit=0):
    ...
    if not os.path.exists(os.path.join(workdir, 'cands_' + fileroot + '_sc' + str(scan) + '.pkl')):
        ...
        if sizelimit and len(cands):
            ...
            candsize = sys.getsizeof(cands[cands.keys()[0]]) / 1000000.0
...
def cleanup(workdir, fileroot, scans=[]):
    ...
    os.chdir(workdir)
    for scan in scans:
        merge_segments(fileroot, scan, cleanup=True, sizelimit=2.0)
...
def merge_noises(pkllist, outroot=''):
    ...
    """
    if not isinstance(pkllist, list):
        raise AssertionError('pkllist must be list of file names')
        outroot = outroot or ('_').join(pkllist[0].split('_')[1:-1])
    ...
    pkllist.sort(key=lambda i: int(i.rstrip('.pkl').split('_sc')[1]))
    ...
    for pklfile in pkllist:
        ...
        allnoise += [ [scan] + list(noise) for noise in noises ]
...
def merge_cands(pkllist, outroot='', remove=[], snrmin=0, snrmax=999):
    ...
    if remove:
        ...
        ww = np.ones(len(mergetimes), dtype=bool)
        nranges = len(remove)
...
def merge_scans(workdir, fileroot, scans, snrmin=0, snrmax=999):
    ...
    pkllist = [ ff for ff in [ os.path.join(workdir, ('cands_{0}_sc{1}.pkl').format(fileroot, scan)) for scan in scans
                             ] if os.path.exists(ff)
              ]
    merge_cands(pkllist, outroot=fileroot, snrmin=snrmin, snrmax=snrmax)
    pkllist = [ ff for ff in [ os.path.join(workdir, ('noise_{0}_sc{1}.pkl').format(fileroot, scan)) for scan in scans
                             ] if os.path.exists(ff)
              ]
...
def mock_fluxratio(candsfile, mockcandsfile, dmbin=0):
    ...
    mocki = [ i for i in loc2[:, 1].astype(int) ]
    rat = []
    newloc = []
    newprop = []
    for i in mocki:
        try:
            detind = list(loc[(dmselect, 1)]).index(i)
