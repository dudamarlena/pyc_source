...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pickey/__init__.py
# Compiled at: 2008-07-12 06:35:20
...
def str2kcs(kcsname):
    ...
    for kcs in kcsarr:
        if kcs.lower() == 'digit':
            mark_digit = True
        elif kcs.lower() == 'lower':
            mark_lower = True
        elif kcs.lower() == 'upper':
            mark_upper = True
        elif kcs.lower() == 'letter':
            mark_lower = True
            mark_upper = True
...
def str2clr(clrname, rgb=None):
    ...
    if lcn[0] == '#' and len(lcn) == 4:
        ...
        return (
         int(lcn[1:3], 16), int(lcn[3:5], 16), int(lcn[5:7], 16))
...
class PicKey(object):
    ...
    def render_kwargs_get(self, name, func, **kwargs):
        for cfg in self.cfglist:
            if cfg + name in self.configs:
                self.render_kwargs[name] = func(self.configs[(cfg + name)], **kwargs)

    def get_png(self):
        key = strjoin(map(lambda idx: self.keycset[int(random() * len(self.keycset))], xrange(self.keylen)), '')
        png = GetPNG(key, (self.keylen * self.charwidth), self.charheight, **self.render_kwargs)
        return (key, png)
