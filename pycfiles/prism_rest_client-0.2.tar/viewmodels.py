...
# Embedded file name: build/bdist.macosx-10.9-intel/egg/prism_rest/viewmodels.py
# Compiled at: 2014-01-03 23:43:04
import re, json, types, logging, collections
...
class _base(object):
    ...
    def __call__(self, func):
        ...
        for modelCls in modelClses.itervalues():
            assert issubclass(modelCls, AbstractViewModel), '%s decorator requires view models are subclasses of BaseViewModel' % self.__class__.__name__

        def wrapper(inst, *args, **kwargs):
            assert isinstance(inst, BaseView), '%s decorator only supported for instances of BaseView.' % self.__class__.__name__
            data = None
            if getattr(inst.request, 'body', None):
                data = json.loads(text_(inst.request.body, inst.request.charset), object_hook=JSONDecoder(inst.request))
            model = None
    ...
    def register_model(cls, mcls):
        ...
        assert issubclass(mcls, AbstractViewModel), 'All view models must decend from BaseViewModel.'
    ...
    def get_model(cls, version, dbinst):
        ...
        if name not in models:
            raise ViewModelNotFoundError, 'No view model matching %s found' % name
    ...
    def get_model_by_name(cls, model_name, model_version):
        ...
        if not model:
            raise ViewModelNotFoundError, 'No view model matching %s found' % model_name
        return model
    ...
    def serialize(self, data):
        ...
        if self._isSerialized(data):
            ...
            if not self.static_model:
                output['metadata'] = AttrDict({'type': self.model_type or self.model_name, 
...
class BaseCollectionViewModel(AbstractViewModel):
    ...
    def serialize(self, data):
        if self._isSerialized(data):
            ...
            output = {'metadata': {'type': self.model_type or self.model_name, 
                            'version': self.version, 
                            'count': len(data), 
                            'limit': len(data), 
                            'per_page': len(data), 
