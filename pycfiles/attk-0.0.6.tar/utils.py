...
# [GCC 8.4.0]
# Embedded file name: attk/utils.py
# Compiled at: 2017-06-27 14:29:55
import sys, getopt, os, numpy as np, librosa, subprocess
from itertools import groupby
from operator import itemgetter
from numpy import ma
from aubio import source, pitch
from moviepy.audio.io import AudioFileClip
import glob, random, fnmatch, warnings
...
def find_media_paths(dir_path):
    ...
    media_paths = [ item for item in media_paths if item.lower()[-4:] in ('.mp3', '.mp4',
                                                                          '.wav') ]
    return media_paths


def temp_wav_path(media_path):
    if media_path.lower()[-4:] == '.wav':
        return media_path
    ...
    subprocess.call(['ffmpeg', '-y', '-i', media_path, wav_path])
    return wav_path
