...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/HTSeq/__init__.py
# Compiled at: 2014-02-27 11:42:28
   ...
   """
    ...
    def __eq__(self, other):
        if not isinstance(other, GenomicFeature):
            return False
        return self.name == other.name and self.type == other.type and self.iv == other.iv
...
_re_fasta_header_line = re.compile('>\\s*(\\S+)\\s*(.*)')
   ...
   """
    ...
    def __iter__(self):
        ...
        while True:
            ...
            if qual == '':
                if id1 != '':
                    warnings.warn('Number of lines in FASTQ file is not a multiple of 4. Discarding the last, incomplete record')
                break
            if not qual.endswith('\n'):
                qual += '\n'
            if not id1.startswith('@'):
                raise ValueError("Primary ID line in FASTQ file doesnot start with '@'. Either this is not FASTQ data or the parser got out of sync.")
            if not id2.startswith('+'):
                raise ValueError("Secondary ID line in FASTQ file doesnot start with '+'. Maybe got out of sync.")
            if len(id2) > 2 and id1[1:] != id2[1:]:
                raise ValueError('Primary and secondary ID line in FASTQdisagree.')
            yield SequenceWithQualities(seq[:-1], id1[1:-1], qual[:-1], self.qual_scale)
   ...
   """
    ...
    def parse_line_bare(dummy, line):
        if line[(-1)] == '\n':
            line = line[:-1]
        res = {}
        res['machine'], res['run_number'], res['lane'], res['tile'], res['x_coord'], res['y_coord'], res['index_string'], res['read_nbr'], res['read_seq'], res['qual_str'], res['chrom'], res['contig'], res['pos'], res['strand'], res['match_descr'], res['single_read_algnt_score'], res['paired_read_algnt_score'], res['partner_chrom'], res['partner_contig'], res['partner_offset'], res['partner_strand'], res['passed_filtering'] = line.split('\t')
        return res
    ...
    def __iter__(self):
        for line in FileOrSequence.__iter__(self):
            ...
            if fields['read_nbr'] != '1':
                warnings.warn('Paired-end read encountered. PE is so far supported only for ' + 'SAM files, not yet for SolexaExport. All PE-related fields are ignored. ')
            record.read = SequenceWithQualities(fields['read_seq'], '%s:%s:%s:%s:%s#0' % (fields['machine'], fields['lane'], fields['tile'],
             fields['x_coord'], fields['y_coord']), fields['qual_str'], self.qualscale)
            ...
            if fields['pos'] == '':
                ...
                if fields['chrom'] == '':
                    chrom = fields['contig']
                record.iv = GenomicInterval(chrom, start, start + len(fields['read_seq']), strand)
            yield record
        ...
        while len(almnt_list) > 0:
            ...
            for a2 in almnt_list:
                ...
                if not (a1.aligned and a2.aligned):
                    break
                if a1.iv.chrom == a2.mate_start.chrom and a1.iv.start == a2.mate_start.pos and a2.iv.chrom == a1.mate_start.chrom and a2.iv.start == a1.mate_start.pos:
                    break
            else:
                if a1.mate_aligned:
                    mate_missing_count[0] += 1
                    if mate_missing_count[0] == 1:
                        warnings.warn('Read ' + a1.read.name + ' claims to have an aligned mate ' + 'which could not be found in an adjacent line.')
                a2 = None
    ...
    def __iter__(self):
        ...
        for line in FileOrSequence.__iter__(self):
            if line.startswith('track'):
                ...
                continue
            elif self.stepType == 'fixed':
                yield (
                 GenomicInterval(chrom, pos, pos + span, '.'), float(line.strip()))
                pos += step
