...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/HTSeq/scripts/qa.py
...
def main():
    ...
    optParser = optparse.OptionParser(usage='%prog [options] read_file', description='This script take a file with high-throughput sequencing reads ' + '(supported formats: SAM, Solexa _export.txt, FASTQ, Solexa ' + '_sequence.txt) and performs a simply quality assessment by ' + 'producing plots showing the distribution of called bases and ' + 'base-call quality scores by position within the reads. The ' + 'plots are output as a PDF file.', epilog='Written by Simon Anders (sanders@fs.tum.de), European Molecular Biology ' + ' Laboratory (EMBL). (c) 2010. Released under the terms of the GNU General ' + " Public License v3. Part of the 'HTSeq' framework, version %s." % HTSeq.__version__)
    optParser.add_option('-t', '--type', type='choice', dest='type', choices=('sam',
                                                                              'bam',
                                                                              'solexa-export',
                                                                              'fastq',
                                                                              'solexa-fastq'), default='sam', help='type of read_file (one of: sam [default], bam, ' + 'solexa-export, fastq, solexa-fastq)')
    optParser.add_option('-o', '--outfile', type='string', dest='outfile', help='output filename (default is <read_file>.pdf)')
    optParser.add_option('-r', '--readlength', type='int', dest='readlen', help='the maximum read length (when not specified, the script guesses from the file')
    optParser.add_option('-g', '--gamma', type='float', dest='gamma', default=0.3, help='the gamma factor for the contrast adjustment of the quality score plot')
    optParser.add_option('-n', '--nosplit', action='store_true', dest='nosplit', help='do not split reads in unaligned and aligned ones')
    optParser.add_option('-m', '--maxqual', type='int', dest='maxqual', default=41, help='the maximum quality score that appears in the data (default: 41)')
    if len(sys.argv) == 1:
    ...
    def plot_bases(arr):
        ...
        pyplot.text(readlen * 0.7, 0.9, 'A', color='red')
        pyplot.text(readlen * 0.75, 0.9, 'C', color='darkgreen')
        pyplot.text(readlen * 0.8, 0.9, 'G', color='lightgreen')
        pyplot.text(readlen * 0.85, 0.9, 'T', color='orange')
        pyplot.text(readlen * 0.9, 0.9, 'N', color='grey')
    ...
    if twoColumns:
        ...
        pyplot.title('non-aligned reads\n%.0f%% (%.3f million)' % (
         100.0 * nreads_U / (nreads_U + nreads_A), nreads_U / 1000000.0))
        pyplot.subplot(222)
        plot_bases(base_arr_A_n)
        pyplot.title('aligned reads\n%.0f%% (%.3f million)' % (
         100.0 * nreads_A / (nreads_U + nreads_A), nreads_A / 1000000.0))
        pyplot.subplot(223)
        pyplot.pcolor(qual_arr_U_n.T ** gamma, cmap=pyplot.cm.Greens, norm=pyplot.normalize(0, 1))
        pyplot.axis((0, readlen - 1, 0, max_qual + 1))
        pyplot.xlabel('position in read')
        pyplot.ylabel('base-call quality score')
        pyplot.subplot(224)
        pyplot.pcolor(qual_arr_A_n.T ** gamma, cmap=pyplot.cm.Greens, norm=pyplot.normalize(0, 1))
        pyplot.axis((0, readlen - 1, 0, max_qual + 1))
        pyplot.xlabel('position in read')
    else:
        pyplot.subplot(211)
        plot_bases(base_arr_U_n)
        pyplot.ylabel('proportion of base')
        pyplot.title('%.3f million reads' % (nreads_U / 1000000.0))
        pyplot.subplot(212)
        pyplot.pcolor(qual_arr_U_n.T ** gamma, cmap=pyplot.cm.Greens, norm=pyplot.normalize(0, 1))
        pyplot.axis((0, readlen - 1, 0, max_qual + 1))
        pyplot.xlabel('position in read')
