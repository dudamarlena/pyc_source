...
# [GCC 8.4.0]
# Embedded file name: /Users/kohei/anaconda3/lib/python3.5/site-packages/extheano/nodebuffer.py
# Compiled at: 2016-04-27 03:22:27
...
class NodeBuffer(object):
    __doc__ = ' Wrap a shared variable and buffer the assignments on it. '
    ...
    def _extract_updater(self):
        ...
        if self._node is self._buffer:
            ...
            updater = (
             self._node, self._buffer)
            self._buffer = self._node
            return updater


class NodeDescriptor(object):
    __doc__ = ' A descriptor that provides us a simple access to `NodeBuffer`'
...
class BufferSet(object):
    __doc__ = ' Pool of `NodeBuffer`s. Another access.'

    def __init__(self, **key_node_pairs):
        self.__dict__['_buffer_dict'] = {key:NodeBuffer(node) for key, node in key_node_pairs.items()}
...
class Scanner(object):
    __doc__ = ' Wrapper of `theano.scan` '
...
class UpdateCollector(object):
    __doc__ = ' Collect updates from `NodeBuffer`s '
