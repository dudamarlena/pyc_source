...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kitt/actions.py
# Compiled at: 2013-05-30 09:15:08
from kivy.logger import Logger
log = Logger.getChild('KiTT')
import gtk, sys, imp, json, os.path
...
class Actions:
    ...
    def __init__(self, actions_config, gestures_config):
        ...
        """
        self._actions = dict(pinch_in=[], pinch_out=[], two_swipe_up=[], two_swipe_down=[], two_swipe_left=[], two_swipe_right=[], three_swipe_up=[], three_swipe_down=[], three_swipe_left=[], three_swipe_right=[], four_swipe_up=[], four_swipe_down=[], four_swipe_left=[], four_swipe_right=[])
        ...
        try:
            with open(os.path.expanduser(actions_config)) as (config):
                ...
                for engine in config['engines']:
                    plugin = imp.load_source('kitt.plugin_%s' % engine, '%s/plugin_%s.py' % (os.path.dirname(__file__), engine))
    ...
    def dispatch(self, gestures, gdb):
        ...
        if u is 4 and d == l == r == 0:
            gesture = 'four_swipe_up'
        else:
            if d is 4 and u == l == r == 0:
                gesture = 'four_swipe_down'
            elif l is 4 and u == d == r == 0:
                gesture = 'four_swipe_left'
            elif r is 4 and u == d == l == 0:
                gesture = 'four_swipe_right'
            elif u is 3 and d == l == r == 0:
                gesture = 'three_swipe_up'
            elif d is 3 and u == l == r == 0:
                gesture = 'three_swipe_down'
            elif l is 3 and u == d == r == 0:
                gesture = 'three_swipe_left'
            elif r is 3 and u == d == l == 0:
                gesture = 'three_swipe_right'
            elif u is 2 and d == l == r == 0:
                gesture = 'two_swipe_up'
            elif d is 2 and u == l == r == 0:
                gesture = 'two_swipe_down'
            elif l is 2 and u == d == r == 0:
                gesture = 'two_swipe_left'
            elif r is 2 and u == d == l == 0:
