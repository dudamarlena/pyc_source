...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kitt/listener.py
# Compiled at: 2013-05-30 09:15:19
...
class Listener(EventDispatcher):
    ...
    def on_touch_up(self, touch):
        ...
        gestures = map(lambda g: self._gdb.find(make_gesture('', zip(touch.ud['line'].points[::2], touch.ud['line'].points[1::2])), minscore=0.7), self._multitouches)
        if not self._actions.dispatch(gestures, self._gdb):
            for touch in self._multitouches:
                log.debug('Touch:\t%s' % self._gdb.gesture_to_str(make_gesture('', zip(touch.ud['line'].points[::2], touch.ud['line'].points[1::2]))))
