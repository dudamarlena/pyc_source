...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kitt/kitt.py
# Compiled at: 2013-05-30 08:45:36
import sys, argparse, threading
from daemon import Daemon
import pyudev, pyudev.glib
...
def run():
    ...
    """
    parser = argparse.ArgumentParser(prog=sys.argv[0], description='KiTT: Kivy Touch Tool')
    parser.add_argument('-v', '--verbose', dest='verbose', action='store_true', help='Enable verbose output')
    parser.add_argument('-c', '--config', dest='config', action='store', default='~/.kivy/kitt_config.json', help='Select the gesture plugin')
    parser.add_argument('-g', '--gestures', dest='gestures', action='store', default='~/.kivy/kitt_gestures.json', help='Select the gesture plugin')
    parser.add_argument('-p', '--pid', dest='pid', action='store', default='/tmp/kitt.pid', help='Where to store PID file')
    subparsers = parser.add_subparsers(help='Daemon commands', dest='commands')
    ...
    subparsers.add_parser('start', help='Start daemon').set_defaults(func=do_start)
    subparsers.add_parser('stop', help='Stop daemon').set_defaults(func=do_stop)
    subparsers.add_parser('foreground', help='Start in foreground').set_defaults(func=do_foreground)
