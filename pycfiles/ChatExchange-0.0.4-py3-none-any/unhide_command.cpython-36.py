...
# Embedded file name: build\bdist.win-amd64\egg\chatette_qiu\cli\interactive_commands\unhide_command.py
# Compiled at: 2019-03-27 03:00:31
# Size of source mod 2**32: 4830 bytes
__doc__ = '\nModule `chatette_qiu.cli.interactive_commands.unhide_command`.\nContains the strategy class that represents the interacive mode command\n`unhide` which restores a unit definition that has been hidden.\n'
...
class UnhideCommand(CommandStrategy):
    ...
    def execute(self, facade):
        ...
        if len(self.command_tokens) < 3:
            self.print_wrapper.error_log('Missing some arguments\nUsage: ' + 'unhide <unit-type> "<unit-name>"')
            return
        ...
        if unit_regex is None:
            ...
            unit_names = [unit_name for unit_name in HideCommand.stored_units[unit_type.name] if unit_regex.match(unit_name)]
            if len(unit_names) == 0:
    ...
    def execute_on_unit(self, facade, unit_type, unit_name, variation_name=None):
        if variation_name is None:
            try:
                ...
                self.print_wrapper.write(unit_type.name.capitalize() + " '" + unit_name + "' was successfully restored.")
            except KeyError:
                self.print_wrapper.error_log(unit_type.name.capitalize() + " '" + unit_name + "' was not " + 'previously hidden.')
            except ValueError:
                self.print_wrapper.error_log(unit_type.name.capitalize() + " '" + unit_name + "' is already defined " + 'in the parser.')
        ...
        try:
            ...
            self.print_wrapper.error_log(unit_type.name.capitalize() + " '" + unit_name + "' is not defined.")
            return
        else:
            try:
                rules = HideCommand.stored_variations[unit_type.name][unit_name][variation_name]
                if variation_name in unit.variations:
                    self.print_wrapper.error_log("Variation '" + variation_name + "' is already defined for " + unit_type.name + " '" + unit_name + "'.")
                    return
                unit.add_rules(rules, variation_name)
                self.print_wrapper.write("Variation '" + variation_name + "' of " + unit_type.name + " '" + unit_name + "' was successfully restored.")
            except KeyError:
                self.print_wrapper.error_log("Variation '" + variation_name + "' of " + unit_type.name + " '" + unit_name + "' was not " + 'previously hidden.')
