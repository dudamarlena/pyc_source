...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/idavoll/gateway.py
...
import urllib, urlparse, simplejson
...
class CreateResource(resource.Resource):
    ...
    def http_POST(self, request):
        ...
        def toResponse(nodeIdentifier):
            ...
            return http.Response(responsecode.OK, stream=stream, headers={'Content-Type': contentType})
...
class DeleteResource(resource.Resource):
    ...
    def http_POST(self, request):
        ...
        def gotStream(_):
            ...
            raise http.HTTPError(http.Response(responsecode.BAD_REQUEST, 'No URI given'))
        ...
        def doDelete(nodeIdentifier, data):
            if data:
                ...
                redirectURI = None
            return self.backend.deleteNode(nodeIdentifier, self.owner, redirectURI)
        ...
        def trapNotFound(failure):
            ...
            return http.StatusResponse(responsecode.NOT_FOUND, 'Node not found')

        def trapXMPPURIParseError(failure):
            failure.trap(XMPPURIParseError)
            return http.StatusResponse(responsecode.BAD_REQUEST, 'Malformed XMPP URI: %s' % failure.value)
...
class PublishResource(resource.Resource):
    ...
    def checkMediaType(self, request):
        ...
        if not ctype:
            raise http.HTTPError(http.StatusResponse(responsecode.BAD_REQUEST, 'No specified Media Type'))
        if ctype.mediaType != 'application' or ctype.mediaSubtype != 'atom+xml' or ctype.params.get('type') != 'entry' or ctype.params.get('charset', 'utf-8') != 'utf-8':
            raise http.HTTPError(http.StatusResponse(responsecode.UNSUPPORTED_MEDIA_TYPE, 'Unsupported Media Type: %s' % http_headers.generateContentType(ctype)))
...
class RemoteSubscriptionService(service.Service, PubSubClient):
    ...
    def subscribeCallback(self, jid, nodeIdentifier, callback):
        ...
        def callbackForLastItem(items):
            ...
            if not atomEntries:
                return
            self._postTo([callback], jid, nodeIdentifier, atomEntries[0], 'application/atom+xml;type=entry')
        ...
        d.addCallback(lambda _: self.storage.addCallback(jid, nodeIdentifier, callback))
        return d
    ...
    def itemsReceived(self, event):
        ...
        if len(atomEntries) == 1:
            ...
            payload = constructFeed(service, nodeIdentifier, atomEntries, title='Received item collection')
    ...
    def deleteReceived(self, event):
        ...
        self.callCallbacks(service, nodeIdentifier, eventType='DELETED', redirectURI=redirectURI)

    def _postTo(self, callbacks, service, nodeIdentifier, payload=None, contentType=None, eventType=None, redirectURI=None):
        if not callbacks:
            return
        else:
            postdata = None
            nodeURI = getXMPPURI(service, nodeIdentifier)
            headers = {'Referer': nodeURI.encode('utf-8'), 'PubSub-Service': service.full().encode('utf-8')}
            if payload:
            ...
            def postNotification(callbackURI):
                f = getPageWithFactory(str(callbackURI), method='POST', postdata=postdata, headers=headers)
                d = f.deferred
    ...
    def callCallbacks(self, service, nodeIdentifier, payload=None, contentType=None, eventType=None, redirectURI=None):

        def eb(failure):
            failure.trap(error.NoCallbacks)

        d = self.storage.getCallbacks(service, nodeIdentifier)
        d.addCallback(self._postTo, service, nodeIdentifier, payload, contentType, eventType, redirectURI)
        d.addErrback(eb)
        d.addErrback(log.err)
...
class RemoteItemsResource(resource.Resource):
    ...
    def render(self, request):
        try:
            ...
            return http.StatusResponse(responsecode.BAD_REQUEST, 'The argument max_items has an invalid value.')
        else:
            try:
                uri = request.args['uri'][0]
            except KeyError:
                return http.StatusResponse(responsecode.BAD_REQUEST, 'No URI for the remote node provided.')

            try:
                (jid, nodeIdentifier) = getServiceAndNode(uri)
            except XMPPURIParseError:
                return http.StatusResponse(responsecode.BAD_REQUEST, 'Malformed XMPP URI: %s' % uri)

        def respond(items):
            """Create a feed out the retrieved items."""
            contentType = http_headers.MimeType('application', 'atom+xml', {'type': 'feed'})
            atomEntries = extractAtomEntries(items)
            feed = constructFeed(jid, nodeIdentifier, atomEntries, 'Retrieved item collection')
            payload = feed.toXml().encode('utf-8')
            return http.Response(responsecode.OK, stream=payload, headers={'Content-Type': contentType})
...
class GatewayClient(service.Service):
    ...
    def startService(self):
        self.port = reactor.listenTCP(self.callbackPort, channel.HTTPFactory(self.site))
    ...
    def ping(self):
        f = getPageWithFactory(self._makeURI(''), method='HEAD', agent=self.agent)
        return f.deferred

    def create(self):
        f = getPageWithFactory(self._makeURI('create'), method='POST', agent=self.agent)
    ...
    def delete(self, xmppURI, redirectURI=None):
        ...
        if redirectURI:
            ...
            postdata = None
            headers = None
        f = getPageWithFactory(self._makeURI('delete', query), method='POST', postdata=postdata, headers=headers, agent=self.agent)
        return f.deferred

    def publish(self, entry, xmppURI=None):
        query = xmppURI and {'uri': xmppURI}
        f = getPageWithFactory(self._makeURI('publish', query), method='POST', postdata=entry.toXml().encode('utf-8'), headers={'Content-Type': MIME_ATOM_ENTRY}, agent=self.agent)
        return f.deferred.addCallback(simplejson.loads)

    def listNodes(self):
        f = getPageWithFactory(self._makeURI('list'), method='GET', agent=self.agent)
        return f.deferred.addCallback(simplejson.loads)

    def subscribe(self, xmppURI):
        params = {'uri': xmppURI, 'callback': 'http://%s:%s/callback' % (self.callbackHost,
                      self.callbackPort)}
        f = getPageWithFactory(self._makeURI('subscribe'), method='POST', postdata=simplejson.dumps(params), headers={'Content-Type': MIME_JSON}, agent=self.agent)
        return f.deferred

    def unsubscribe(self, xmppURI):
        params = {'uri': xmppURI, 'callback': 'http://%s:%s/callback' % (self.callbackHost,
                      self.callbackPort)}
        f = getPageWithFactory(self._makeURI('unsubscribe'), method='POST', postdata=simplejson.dumps(params), headers={'Content-Type': MIME_JSON}, agent=self.agent)
        return f.deferred
    ...
    def items(self, xmppURI, maxItems=None):
        ...
        f = getPageWithFactory(self._makeURI('items', query), method='GET', agent=self.agent)
        return f.deferred