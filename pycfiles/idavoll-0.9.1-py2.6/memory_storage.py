...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/idavoll/memory_storage.py
# Compiled at: 2009-06-18 09:36:39
import copy
...
class Storage:
    ...
    def __init__(self):
        rootNode = CollectionNode('', jid.JID('localhost'), copy.copy(self.defaultConfig['collection']))
    ...
    def getAffiliations(self, entity):
        ...
        return defer.succeed([ (node.nodeIdentifier, node._affiliations[entity]) for (name, node) in self._nodes.iteritems() if entity in node._affiliations
                             ])
...
class Node:
    ...
    def getSubscriptions(self, state=None):
        return defer.succeed([ subscription for subscription in self._subscriptions.itervalues() if state is None or subscription.state == state
                             ])
    ...
    def addSubscription(self, subscriber, state, options):
        if self._subscriptions.get(subscriber.full()):
            ...
            subscription = Subscription(self.nodeIdentifier, subscriber, state, options)
    ...
    def isSubscribed(self, entity):
        for (subscriber, subscription) in self._subscriptions.iteritems():
            if jid.internJID(subscriber).userhost() == entity.userhost() and subscription.state == 'subscribed':
                return defer.succeed(True)

        return defer.succeed(False)

    def getAffiliations(self):
        affiliations = [ (jid.internJID(entity), affiliation) for (entity, affiliation) in self._affiliations.iteritems()
                       ]
