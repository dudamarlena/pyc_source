...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/idavoll/pgsql_storage.py
...
class Storage:
    ...
    defaultConfig = {'leaf': {'pubsub#persist_items': True, 
                'pubsub#deliver_payloads': True, 
                'pubsub#send_last_published_item': 'on_sub'}, 
       'collection': {'pubsub#deliver_payloads': True, 
                      'pubsub#send_last_published_item': 'on_sub'}}
    ...
    def _getNode(self, cursor, nodeIdentifier):
        ...
        if row.node_type == 'leaf':
            configuration = {'pubsub#persist_items': row.persist_items, 'pubsub#deliver_payloads': row.deliver_payloads, 
            ...
            return node
        if row.node_type == 'collection':
            configuration = {'pubsub#deliver_payloads': row.deliver_payloads, 'pubsub#send_last_published_item': row.send_last_published_item}
    ...
    def createNode(self, nodeIdentifier, owner, config):
        return self.dbpool.runInteraction(self._createNode, nodeIdentifier, owner, config)
    ...
    def _createNode(self, cursor, nodeIdentifier, owner, config):
        ...
        cursor.execute('SELECT 1 from entities where jid=%s', owner)
        if not cursor.fetchone():
            cursor.execute('INSERT INTO entities (jid) VALUES (%s)', owner)
...
class Node:
    ...
    def _checkNodeExists(self, cursor):
        cursor.execute('SELECT node_id FROM nodes WHERE node=%s', self.nodeIdentifier)
        if not cursor.fetchone():
            raise error.NodeNotFound()
    ...
    def _getSubscriptions(self, cursor, state):
        ...
        for row in rows:
            ...
            subscriptions.append(Subscription(self.nodeIdentifier, subscriber, row.state, options))

        return subscriptions

    def addSubscription(self, subscriber, state, config):
        return self.dbpool.runInteraction(self._addSubscription, subscriber, state, config)
    ...
    def _addSubscription(self, cursor, subscriber, state, config):
        ...
        try:
            cursor.execute('INSERT INTO entities (jid) VALUES (%s)', userhost)
        except cursor._pool.dbapi.OperationalError:
            pass
    ...
    def removeSubscription(self, subscriber):
        return self.dbpool.runInteraction(self._removeSubscription, subscriber)
...
class GatewayStorage(object):
    ...
    def _countCallbacks(self, cursor, service, nodeIdentifier):
        ...
        """
        cursor.execute('SELECT count(*) FROM callbacks\n                          WHERE service=%s and node=%s', service.full(), nodeIdentifier)
    ...
    def addCallback(self, service, nodeIdentifier, callback):
        ...
        def interaction(cursor):
            cursor.execute('SELECT 1 FROM callbacks\n                              WHERE service=%s and node=%s and uri=%s', service.full(), nodeIdentifier, callback)
            if cursor.fetchall():
                return
            cursor.execute('INSERT INTO callbacks\n                              (service, node, uri) VALUES\n                              (%s, %s, %s)', service.full(), nodeIdentifier, callback)
