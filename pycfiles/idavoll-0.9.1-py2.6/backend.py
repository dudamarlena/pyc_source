...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/idavoll/backend.py
# Compiled at: 2009-09-03 12:26:25
"""
...
class BackendService(service.Service, utility.EventDispatcher):
    ...
    nodeOptions = {'pubsub#persist_items': {'type': 'boolean', 'label': 'Persist items to storage'}, 
       'pubsub#deliver_payloads': {'type': 'boolean', 'label': 'Deliver payloads with event notifications'}, 
       'pubsub#send_last_published_item': {'type': 'list-single', 'label': 'When to send the last published item', 
                                           'options': {'never': 'Never', 
                                                       'on_sub': 'When a new subscription is processed'}}}
    subscriptionOptions = {'pubsub#subscription_type': {'type': 'list-single', 'options': {'items': 'Receive notification of new items only', 
                                                'nodes': 'Receive notification of new nodes only'}}, 
       'pubsub#subscription_depth': {'type': 'list-single', 'options': {'1': 'Receive notification from direct child nodes only', 
    ...
    def _doPublish(self, node, items, requestor):
        ...
        d.addCallback(self._doNotify, node.nodeIdentifier, items, deliverPayloads)
        return d
    ...
    def _doNotify(self, result, nodeIdentifier, items, deliverPayloads):
        ...
        self.dispatch({'items': items, 'nodeIdentifier': nodeIdentifier}, '//event/pubsub/notify')
    ...
    def getNotifications(self, nodeIdentifier, items):
        ...
        def toNotifications(subscriptions, nodeIdentifier, items):
            ...
            for subscription in subscriptions:
                if subscription.options.get('pubsub#subscription_type', 'items') == 'items':
                    subs = subsBySubscriber.setdefault(subscription.subscriber, set())
    ...
    def _sendLastPublished(self, subscription, node):
        ...
        def notifyItem(items):
            if items:
                reactor.callLater(0, self.dispatch, {'items': items, 'nodeIdentifier': node.nodeIdentifier, 
                   'subscription': subscription}, '//event/pubsub/notify')

        config = node.getConfiguration()
        sendLastPublished = config.get('pubsub#send_last_published_item', 'never')
    ...
    def getItems(self, nodeIdentifier, requestor, maxItems=None, itemIdentifiers=None):
        d = self.storage.getNode(nodeIdentifier)
        d.addCallback(_getAffiliation, requestor)
    ...
    def _doNotifyRetraction(self, itemIdentifiers, nodeIdentifier):
        self.dispatch({'itemIdentifiers': itemIdentifiers, 'nodeIdentifier': nodeIdentifier}, '//event/pubsub/retract')
    ...
    def _doPreDelete(self, result, redirectURI):
        ...
        if affiliation != 'owner':
            raise error.Forbidden()
        data = {'nodeIdentifier': node.nodeIdentifier, 'redirectURI': redirectURI}
        d = defer.DeferredList([ cb(data) for cb in self._callbackList
                               ], consumeErrors=1)
...
class PubSubServiceFromBackend(PubSubService):
    ...
    """
    implements(IDisco)
    _errorMap = {error.NodeNotFound: ('item-not-found', None, None), 
       ...
       error.NoInstantNodes: ('not-acceptable', 'unsupported', 'instant-nodes'), 
       ...
       error.NodeNotPersistent: ('feature-not-implemented', 'unsupported', 'persistent-node'), 
       error.NoRootNode: ('bad-request', None, None), 
       error.NoCollections: ('feature-not-implemented', 'unsupported', 'collections'), 
       error.NoPublishing: ('feature-not-implemented', 'unsupported', 'publish')}
    ...
    def _preDelete(self, data):
        ...
        d.addCallback(lambda subscribers: self.notifyDelete(self.serviceJID, nodeIdentifier, subscribers, redirectURI))
        return d
    ...
    def setConfiguration(self, requestor, service, nodeIdentifier, options):
        d = self.backend.setNodeConfiguration(nodeIdentifier, options, requestor)
        return d.addErrback(self._mapErrors)

    def items(self, requestor, service, nodeIdentifier, maxItems, itemIdentifiers):
        d = self.backend.getItems(nodeIdentifier, requestor, maxItems, itemIdentifiers)
        return d.addErrback(self._mapErrors)

    def retract(self, requestor, service, nodeIdentifier, itemIdentifiers):
        d = self.backend.retractItem(nodeIdentifier, itemIdentifiers, requestor)
        return d.addErrback(self._mapErrors)
...
components.registerAdapter(PubSubServiceFromBackend, IBackendService, IPubSubService)