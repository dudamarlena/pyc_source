...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/idavoll/test/test_storage.py
    ...
    def test_getSubscriptions(self):
        ...
        def cb(subscriptions):
            ...
            for subscription in subscriptions:
                if subscription.nodeIdentifier == 'pre-existing' and subscription.subscriber == SUBSCRIBER and subscription.state == 'subscribed':
                    found = True
...
class MemoryStorageStorageTestCase(unittest.TestCase, StorageTests):
    ...
    def setUp(self):
        ...
        self.s = Storage()
        self.s._nodes['pre-existing'] = LeafNode('pre-existing', OWNER, defaultConfig)
        self.s._nodes['to-be-deleted'] = LeafNode('to-be-deleted', OWNER, None)
        self.s._nodes['to-be-reconfigured'] = LeafNode('to-be-reconfigured', OWNER, defaultConfig)
        self.s._nodes['to-be-purged'] = LeafNode('to-be-purged', OWNER, None)
        subscriptions = self.s._nodes['pre-existing']._subscriptions
        subscriptions[SUBSCRIBER.full()] = Subscription('pre-existing', SUBSCRIBER, 'subscribed')
        subscriptions[SUBSCRIBER_TO_BE_DELETED.full()] = Subscription('pre-existing', SUBSCRIBER_TO_BE_DELETED, 'subscribed')
        subscriptions[SUBSCRIBER_PENDING.full()] = Subscription('pre-existing', SUBSCRIBER_PENDING, 'pending')
...
class PgsqlStorageStorageTestCase(unittest.TestCase, StorageTests):
    ...
    def setUp(self):
        ...
        self.dbpool = adbapi.ConnectionPool('pyPgSQL.PgSQL', database='pubsub_test', cp_reconnect=True, client_encoding='utf-8')
    ...
    def init(self, cursor):
        ...
        cursor.execute('INSERT INTO entities (jid) VALUES (%s)', OWNER.userhost())
        cursor.execute("INSERT INTO affiliations\n                          (node_id, entity_id, affiliation)\n                          SELECT node_id, entity_id, 'owner'\n                          FROM nodes, entities\n                          WHERE node='pre-existing' AND jid=%s", OWNER.userhost())
        cursor.execute('INSERT INTO entities (jid) VALUES (%s)', SUBSCRIBER.userhost())
        cursor.execute("INSERT INTO subscriptions\n                          (node_id, entity_id, resource, state)\n                          SELECT node_id, entity_id, %s, 'subscribed'\n                          FROM nodes, entities\n                          WHERE node='pre-existing' AND jid=%s", (
         SUBSCRIBER.resource,
         SUBSCRIBER.userhost()))
        cursor.execute('INSERT INTO entities (jid) VALUES (%s)', SUBSCRIBER_TO_BE_DELETED.userhost())
        cursor.execute("INSERT INTO subscriptions\n                          (node_id, entity_id, resource, state)\n                          SELECT node_id, entity_id, %s, 'subscribed'\n                          FROM nodes, entities\n                          WHERE node='pre-existing' AND jid=%s", (
         SUBSCRIBER_TO_BE_DELETED.resource,
         SUBSCRIBER_TO_BE_DELETED.userhost()))
        cursor.execute('INSERT INTO entities (jid) VALUES (%s)', SUBSCRIBER_PENDING.userhost())
        cursor.execute("INSERT INTO subscriptions\n                          (node_id, entity_id, resource, state)\n                          SELECT node_id, entity_id, %s, 'pending'\n                          FROM nodes, entities\n                          WHERE node='pre-existing' AND jid=%s", (
         SUBSCRIBER_PENDING.resource,
         SUBSCRIBER_PENDING.userhost()))
        cursor.execute('INSERT INTO entities (jid) VALUES (%s)', PUBLISHER.userhost())
    ...
    def cleandb(self, cursor):
        ...
        cursor.execute('DELETE FROM entities WHERE jid=%s', OWNER.userhost())
        cursor.execute('DELETE FROM entities WHERE jid=%s', SUBSCRIBER.userhost())
        cursor.execute('DELETE FROM entities WHERE jid=%s', SUBSCRIBER_TO_BE_DELETED.userhost())
        cursor.execute('DELETE FROM entities WHERE jid=%s', SUBSCRIBER_PENDING.userhost())
        cursor.execute('DELETE FROM entities WHERE jid=%s', PUBLISHER.userhost())
