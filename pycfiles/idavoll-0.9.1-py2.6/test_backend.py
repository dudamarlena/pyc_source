...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/idavoll/test/test_backend.py
...
class BackendTest(unittest.TestCase):
    ...
    def test_interfaceIBackend(self):
        self.assertTrue(verifyObject(iidavoll.IBackendService, backend.BackendService(None)))
        return
    ...
    def test_getDefaultConfiguration(self):
        ...
        class TestStorage:
            ...
            def getDefaultConfiguration(self, nodeType):
                return {'pubsub#persist_items': True, 
    ...
    def test_getNodeConfiguration(self):
        ...
        class testNode:
            ...
            def getConfiguration(self):
                return {'pubsub#deliver_payloads': True, 'pubsub#persist_items': False}
    ...
    def test_setNodeConfiguration(self):
        ...
        self.backend = backend.BackendService(self.storage)
        self.storage.backend = self.backend
        options = {'pubsub#deliver_payloads': True, 'pubsub#persist_items': False}
    ...
    def test_notifyOnSubscription(self):
        ...
        class TestNode:
            ...
            def getConfiguration(self):
                return {'pubsub#deliver_payloads': True, 'pubsub#persist_items': False, 'pubsub#send_last_published_item': 'on_sub'}

            def getItems(self, maxItems):
                return [ITEM]

            def addSubscription(self, subscriber, state, options):
                self.subscription = pubsub.Subscription('node', subscriber, state, options)
                return defer.succeed(None)
...
class PubSubServiceFromBackendTest(unittest.TestCase):
    ...
    def test_preDelete(self):
        ...
        def notifyDelete(service, nodeIdentifier, subscribers, redirectURI=None):
    ...
    def test_preDeleteRedirect(self):
        ...
        self.assertTrue(verifyObject(iwokkel.IPubSubService, s))
        self.assertNotIdentical(None, s.backend.preDeleteFn)
        data = {'nodeIdentifier': 'test', 'redirectURI': uri}
        d2 = s.backend.preDeleteFn(data)
        return defer.DeferredList([d1, d2], fireOnOneErrback=1)
    ...
    def test_getDefaultConfiguration(self):
        ...
        class TestBackend(BaseTestBackend):
            ...
            def getDefaultConfiguration(self, nodeType):
                options = {'pubsub#persist_items': True, 'pubsub#deliver_payloads': True, 
