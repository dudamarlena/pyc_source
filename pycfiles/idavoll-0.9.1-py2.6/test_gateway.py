...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/idavoll/test/test_gateway.py
# Compiled at: 2009-07-09 12:36:06
"""
...
TEST_ENTRY.addElement('id', content='urn:uuid:1225c695-cfb8-4ebb-aaaa-80da344efa6a')
    ...
    def test_deleteNotificationWithRedirect(self):
        ...
        def onNotification(data, headers):
            try:
                self.assertTrue(headers.hasHeader('Event'))
                self.assertEquals(['DELETED'], headers.getRawHeaders('Event'))
                self.assertEquals(['<%s>; rel=alternate' % redirectURI], headers.getRawHeaders('Link'))
            except:
                self.client.deferred.errback()
            else:
                self.client.deferred.callback(None)
