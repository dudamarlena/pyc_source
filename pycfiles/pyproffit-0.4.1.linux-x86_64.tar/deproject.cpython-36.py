...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyproffit/deproject.py
# Compiled at: 2019-11-19 08:35:18
# Size of source mod 2**32: 43446 bytes
...
def list_params(rad, sourcereg, nrc=None, nbetas=6):
    ...
    allbetas = np.linspace(0.6, 3.0, nbetas)
    nrc = len(allrc)
    nbetas = len(allbetas)
        ...
        if self.profile is None:
            ...
            if self.dens is None:
                ...
                if len(self.rout) == len(self.profile.bins):
                    plt.errorbar(rkpc, (self.dens), xerr=erkpc, yerr=[self.dens - self.dens_lo, self.dens_hi - self.dens], fmt='.', color='C0', elinewidth=2, markersize=7,
                      capsize=3)
                else:
