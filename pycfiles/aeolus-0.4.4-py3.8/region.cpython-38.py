...
@dataclass
class BoundsRect:
    __doc__ = 'Bounding longitudes and latitudes of a given lon-lat rectangle.'
...
class Region(object):
    ...
    def constraint(self):
        ...
        if self.bounds.west < self.bounds.east:
            ...
            cnstr &= iris.Constraint(longitude=(lambda x: self.bounds.west <= x or x <= self.bounds.east))
        return cnstr
