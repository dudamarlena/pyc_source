...
# [GCC 8.4.0]
# Embedded file name: /home/timo/Desktop/projects/SBtab/pypi_installer/sbtab/SBtab.py
# Compiled at: 2018-11-09 05:27:44
# Size of source mod 2**32: 41400 bytes
"""
SBtab
=====
...
class SBtabTable:
    ...
    def _singular(self):
        ...
        if header_row_count > 1:
            raise SBtabError('There are more than one SBtab tables in this file. Please use the SBtabDocument class instead of SBtabTable.')
    ...
    def _preprocess_table_string(self, table_string):
        ...
        for row in table_string.split('\n'):
            ...
            row.replace('%s,%s' % (self.delimiter, self.delimiter), '%s%s' % (self.delimiter, self.delimiter))
    ...
    def _handle_row(self, row, delimiter):
        ...
        row = self._dequote(row)
        if "'" in row or '{' in row or '[' in row:
            iterators = re.finditer("('.*?')%s" % delimiter, row)
            indices = [0]
            for i in iterators:
                ...
                if delimiter == ',':
                    if running_json and not item.endswith("}'"):
                        ...
                        if item.startswith("'{"):
                            ...
                            if running_json and item.endswith("}'"):
                                jsons.append(item)
                                while '' in jsons:
                                    jsons.remove('')
                                ...
                                if running_jlist and not item.endswith("]'"):
                                    if item.strip() != '':
                                        jlist.append(item)
                                else:
                                    if item.startswith("'["):
                                        ...
                                        if running_jlist and item.endswith("]'"):
                                            jlist.append(item)
                                            while '' in jlist:
                                                jlist.remove('')
                                            ...
                                            if item.startswith("'") and not item.startswith("'{") and not item.endswith("}'") and not item.startswith("'[") and not item.endswith("]'"):
                                                items.append(item)
                                            else:
                                                if item == '':
                                                    items += ['']
    ...
    def _initialize_table(self):
        ...
        self.table_id, self.table_type, self.table_name, self.table_document, self.table_version = self._get_table_information()
    ...
    def _get_table_information(self):
        ...
        try:
            ...
            raise SBtabError('The TableID of the SBtab is not defined!')
        ...
        return (
         table_id, table_type, table_name, table_document, table_version)
        ...
        if not filename.endswith('tsv') and not filename.endswith('csv'):
            ...
            if self.delimiter == ',':
                ...
                if self.delimiter == ';':
                    ...
                    raise SBtabError('The file extension is missing and the delimiter is no standard.')
        try:
            f = open(filename, 'w')
    ...
    @staticmethod
    def from_data_frame(df, table_id, table_type, table_name='', document_name='', document='', unit='', sbtab_version='1.0'):
        ...
        header_strings = [
         '!!SBtab'] + list(map(lambda x: "%s='%s'" % x, header))
...
class SBtabDocument:
    ...
    def add_sbtab(self, sbtab):
        ...
        if valid_type:
            ...
            if sbtab.table_type in self.type_to_sbtab:
                ...
                self.type_to_sbtab[sbtab.table_type] = [
                 sbtab]
            self._get_doc_row_attributes()
            return True
    ...
    def add_sbtab_string(self, sbtab_string, filename):
        ...
        if not self.filename:
            self.filename = filename
        try:
            sbtab_amount = misc.count_tabs(sbtab_string)
        except:
    ...
    def check_type_validity(self, ttype):
        ...
        try:
            ...
            raise SBtabError('The definition file could not be found to establish supported table types.')
    ...
    def _get_doc_row_attributes(self):
        ...
        if not self.doc_row:
            ...
            try:
                self.name = self.get_custom_doc_information('Document')
            except:
                pass
        ...
        if not filename.endswith('tsv') and not filename.endswith('csv'):
            ...
            if delimiter == ',':
                ...
                if delimiter == ';':
                    ...
                    raise SBtabError('The file extension is missing and the delimiter is not set.')
        try:
            f = open(filename, 'w')
            f.write(self.to_str())
            f.close()
