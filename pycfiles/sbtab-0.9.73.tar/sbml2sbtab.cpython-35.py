...
# [GCC 8.4.0]
# Embedded file name: /home/timo/Desktop/projects/SBtab/pypi_installer/sbtab/sbml2sbtab.py
# Compiled at: 2018-10-25 03:40:01
# Size of source mod 2**32: 29297 bytes
"""
...
supported_table_types = [
 'Compartment', 'Compound', 'Reaction', 'Quantity']
...
class SBMLDocument:
    ...
    def __init__(self, sbml_model, filename):
        ...
        if filename.endswith('.xml') or filename.endswith('.sbml'):
            ...
            raise ConversionError('Wrong extension of file %s.' % filename)
    ...
    def convert_to_sbtab(self):
        ...
        return (
         sbtab_doc, self.warnings)

    def compartment_sbtab(self):
        """
        build a compartment SBtab
        """
        sbtab_compartment = '!!SBtab SBtabVersion="1.0" Document="%s" TableType="Compartment" TableName="Compartment"\n' % self.filename
        columns = [
        ...
        sbtab_compartment = SBtab.SBtabTable(sbtab_compartment, self.filename + '_compartment.tsv')
            ...
            try:
                ...
                for j, annotation in enumerate(annotations):
                    ...
                    if col_name not in columns:
                        ...
                        sbtab_compartment.change_value_by_name(compartment.getId(), col_name, annotation[0])

            except:
                self.warnings.append('Could not add the annotation %s forcompartment %s' % annotations[1], compartment.getId())
    ...
    def compound_sbtab(self):
        ...
        """
        sbtab_compound = '!!SBtab SBtabVersion="1.0" Document="%s" TableType="Compound" TableName="Compound"\n' % self.filename
        columns = [
         '!ID', '!Name', '!Location', '!IsConstant',
        ...
        for species in self.model.getListOfSpecies():
            ...
            try:
                value_row[2] = species.getCompartment()
            except:
                pass

            try:
                value_row[3] = str(species.getConstant())
            except:
                pass
            ...
            try:
                value_row[5] = str(species.getInitialConcentration())
            except:
                pass

            try:
                value_row[6] = str(species.getHasOnlySubstanceUnits())
            except:
                pass
        ...
        sbtab_compound = SBtab.SBtabTable(sbtab_compound, self.filename + '_compound.tsv')
        for i, species in enumerate(self.model.getListOfSpecies()):
            try:
                ...
                for j, annotation in enumerate(annotations):
                    ...
                    if col_name not in columns:
                        new_column = [
                         ''] * (self.model.getNumSpecies() + 1)
                        new_column[0] = col_name
                        new_column[i + 1] = annotation[0]
                        columns.append(col_name)
                        sbtab_compound.add_column(new_column)
                    else:
                        sbtab_compound.change_value_by_name(species.getId(), col_name, annotation[0])

            except:
                self.warnings.append('Could not add the annotation %s forcompound %s' % annotations[1], species.getId())
            ...
            if eve.getNumEventAssignments() > 1:
                ...
                try:
                    ...
                    for i, annotation in enumerate(annot_tuples):
                        if '!Identifiers:' + annotation[1] not in header:
    ...
    def fbc_objective(self):
        ...
        sbtab_fbc = '!!SBtab SBtabVersion="1.0" Document="%s" TableType="FbcObjective" TableName="FBC Objective"\n' % self.filename
        columns = [
         '!ID', '!Name', '!Type', '!Active', '!Objective']
        ...
        sbtab_fbc = SBtab.SBtabTable(sbtab_fbc, self.filename + '_fbc_objective.tsv')
        return sbtab_fbc
    ...
    def fbc_gene(self):
        ...
        """
        fbc_plugin = self.model.getPlugin('fbc')
        sbtab_fbc_gene = '!!SBtab SBtabVersion="1.0" Document="%s" TableType="Gene" TableName="FBC Gene"\n' % self.filename
        columns = [
        ...
        sbtab_fbc_gene = SBtab.SBtabTable(sbtab_fbc_gene, self.filename + '_fbc_gene.tsv')
    ...
    def reaction_sbtab(self):
        ...
        """
        sbtab_reaction = '!!SBtab SBtabVersion="1.0" Document="%s" TableType="Reaction" TableName="Reaction"\n' % self.filename
        columns = [
         '!ID', '!Name', '!ReactionFormula', '!Location',
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            self.warnings.append('Gene Association Information for reaction %s could not be written.' & reaction.getId())
        ...
        sbtab_reaction = SBtab.SBtabTable(sbtab_reaction, self.filename + '_reaction.tsv')
        for i, reaction in enumerate(self.model.getListOfReactions()):
            try:
                ...
                for j, annotation in enumerate(annotations):
                    ...
                    if col_name not in columns:
                        new_column = [
                         ''] * (self.model.getNumReactions() + 1)
                        new_column[0] = col_name
                        new_column[i + 1] = annotation[0]
                        columns.append(col_name)
                        sbtab_reaction.add_column(new_column)
                    else:
                        sbtab_reaction.change_value_by_name(reaction.getId(), col_name, annotation[0])

            except:
                self.warnings.append('Could not add the annotation %s forreaction %s' % annotations[1], reaction.getId())
    ...
    def _gene_ass_recursive(self, ga, r_id):
        ...
        if type(ga) == libsbml.FbcOr or type(ga) == libsbml.FbcAnd:
            ...
            ass_list += '(' + sub_list + ')'
            return ass_list
        if type(ga) == libsbml.GeneProductRef:
    ...
    def quantity_sbtab(self):
        ...
        if not parameters:
            return False
        sbtab_quantity = '!!SBtab SBtabVersion="1.0" Document="%s" TableType="Quantity" TableName="Quantity"\n' % self.filename
        columns = [
         '!ID', '!Parameter:SBML:parameter:id', '!Value',
         '!Unit', '!Type']
        ...
        sbtab_quantity = SBtab.SBtabTable(sbtab_quantity, self.filename + '_quantity.tsv')
        for i, quantity in enumerate(local_parameters):
            try:
                ...
                for j, annotation in enumerate(annotations):
                    ...
                    if col_name not in columns:
                        new_column = [
                         ''] * (len(local_parameters) + 1)
                        new_column[0] = col_name
                        new_column[i + 1] = annotation[0]
                        columns.append(col_name)
                        sbtab_quantity.add_column(new_column)
                    else:
                        sbtab_quantity.change_value_by_name(quantity.getId(), col_name, annotation[0])

            except:
                self.warnings.append('Could not add the annotation %s forquantity %s' % annotations[1], quantity.getId())

        for parameter in self.model.getListOfParameters():
            value_row = [
             ''] * len(sbtab_quantity.columns)
        ...
        for i, parameter in enumerate(self.model.getListOfParameters()):
            try:
                ...
                for j, annotation in enumerate(annotations):
                    ...
                    if col_name not in columns:
                        new_column = [
                         ''] * (self.model.getNumParameters() + 1)
                        new_column[0] = col_name
                        new_column[i + 1] = annotation[0]
                        columns.append(col_name)
                        sbtab_quantity.add_column(new_column)
                    else:
                        sbtab_quantity.change_value_by_name(parameter.getId(), col_name, annotation[0])

            except:
                self.warnings.append('Could not add the annotation %s forquantity %s' % annotations[1], parameter.getId())
