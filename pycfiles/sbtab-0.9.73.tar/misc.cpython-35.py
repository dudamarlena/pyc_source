...
# [GCC 8.4.0]
# Embedded file name: /home/timo/Desktop/projects/SBtab/pypi_installer/sbtab/misc.py
# Compiled at: 2018-11-08 08:05:54
# Size of source mod 2**32: 12336 bytes
import re, string, libsbml, numpy, scipy, scipy.linalg, scipy.optimize, random, copy, math, os
try:
    from . import SBtab
except:
    import SBtab
...
def validate_file_extension(file_name, file_type):
    ...
    sbtab => tsv, csv, xlsx
    """
    if file_type == 'sbml' and file_name[-3:] == 'xml':
        return True
    if file_type == 'sbml':
        return False
    if file_type == 'sbtab' and file_name[-3:] == 'tsv':
        return True
    if file_type == 'sbtab' and file_name[-3:] == 'csv':
        return True
    if file_type == 'sbtab' and file_name[-4:] == 'xlsx':
        return True
    if file_type == 'sbtab':
        return False
    return False
...
def sbtab_to_html(sbtab, filename=None, mode='sbtab_online'):
    ...
    def _build_main(sbtab, sbtab_def):
        ...
        no_link = [
         '(', ')', '+', '-', '<=>', 'or', 'and', 'FbcOr', 'FbcAnd']
        try:
            col2description, col2link = find_descriptions(sbtab_def, sbtab.table_type)
        except:
        ...
        for row in sbtab.value_rows:
            try:
                html += '<tr id="%s" style="line-height:1.5;">' % row[sbtab.columns_dict['!ID']]
            except:
...
urns = [
 'obo.chebi', 'kegg.compound', 'kegg.reaction', 'obo.go', 'obo.sgd', 'biomodels.sbo', 'ec-code', 'kegg.orthology', 'uniprot', 'hmdb']
