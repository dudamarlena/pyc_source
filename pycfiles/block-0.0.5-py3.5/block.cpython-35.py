...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/block/block.py
# Compiled at: 2017-04-11 20:12:41
# Size of source mod 2**32: 9280 bytes
import numpy as np, scipy.sparse.linalg as sla, scipy.sparse as sp
try:
    import torch
...
def block(rows, dtype=None, arrtype=None):
    if not _is_list_or_tup(rows) or len(rows) == 0 or np.any([not _is_list_or_tup(row) for row in rows]):
...
def block_diag(elems, dtype=None, arrtype=None):
    n = len(elems)
    return block([[0] * i + [elem] + [0] * (n - 1 - i) for i, elem in enumerate(elems)], dtype=dtype, arrtype=arrtype)
...
def _get_backend(rows, dtype, arrtype):
    ...
    if arrtype == sla.LinearOperator:
        return LinearOperatorBackend(dtype)
    if arrtype is not None and re.search('torch\\..*Tensor', repr(arrtype)):
        return TorchBackend(dtype)
    if arrtype is not None and re.search('torch\\..*(Variable|Parameter)', repr(arrtype)):
    ...
    for row in rows:
        for elem in row:
            ...
            if tb.is_complete(elem):
                return TorchBackend(type(elem))
            if lob.is_complete(elem):
                return LinearOperatorBackend(elem.dtype)
            if tvb.is_complete(elem):
...
class TorchBackend(Backend):
    ...
    def is_complete(self, x):
        return re.search('torch\\..*Tensor', str(x.__class__)) is not None and x.ndimension() == 2
...
class LinearOperatorBackend(Backend):
    ...
    def build_eye(self, n):
        ...
        return sla.LinearOperator(shape=(n, n), matvec=identity, rmatvec=identity, matmat=identity, dtype=self.dtype)
    ...
    def build_full(self, shape, fill_val):
        ...
        if fill_val == 0:
            ...
            return sla.LinearOperator(shape=shape, matvec=matvec, rmatvec=rmatvec, matmat=matmat, dtype=self.dtype)
    ...
    def build(self, rows):
        col_sizes = [lo.shape[1] if self.is_complete(lo) else lo[1] for lo in rows[0]]
        col_idxs = np.cumsum([0] + col_sizes)
        row_sizes = [row[0].shape[0] if self.is_complete(row[0]) else row[0][0] for row in rows]
        ...
        def matvec(v):
            ...
            for row, i, j in zip(rows, row_idxs[:-1], row_idxs[1:]):
                out[i:j] = sum(lo.matvec(v[k:l]) for lo, k, l in zip(row, col_idxs[:-1], col_idxs[1:]) if self.is_complete(lo))
        ...
        def rmatvec(v):
            ...
            for col, i, j in zip(cols, col_idxs[:-1], col_idxs[1:]):
                out[i:j] = sum(lo.rmatvec(v[k:l]) for lo, k, l in zip(col, row_idxs[:-1], row_idxs[1:]) if self.is_complete(lo))
        ...
        def matmat(M):
            ...
            for row, i, j in zip(rows, row_idxs[:-1], row_idxs[1:]):
                out[i:j] = sum(lo.matmat(M[k:l]) for lo, k, l in zip(row, col_idxs[:-1], col_idxs[1:]) if self.is_complete(lo))

            return out

        return sla.LinearOperator(shape=(m, n), matvec=matvec, rmatvec=rmatvec, matmat=matmat, dtype=self.dtype)
