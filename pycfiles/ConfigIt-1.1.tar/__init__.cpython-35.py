...
# Compiled at: 2019-07-04 00:45:12
# Size of source mod 2**32: 18998 bytes
...
import os, re, sys, json, copy, inspect
...
class Field(object):
    ...
    def __init__(self, default=NOTHING, dont_dump=False, printable=True):
        self.name = None
        self._value = default
    ...
    def get_value(self, check_dont_dump=False, check_printable=False):
        ...
        **CN Doc**
        ...
        if self._config_object is NOTHING:
            raise AttributeError("Field.get_value() can't be called without initialized with ")
        if check_dont_dump and self.dont_dump:
            raise DontDumpError
        if check_printable:
        ...
        if self._getter_method is NOTHING:
            if self._value is NOTHING:
                raise ValueNotSetError('{}.{} has not set a value yet!'.format(self.__class__.__name__, self.name))
            else:
                return self._value
        else:
...
class BaseConfigClass(object):
    ...
    def to_json(self, check_dont_dump=True, check_printable=False):
        return json.dumps(self.to_dict(check_dont_dump=check_dont_dump, check_printable=check_printable), indent=4, sort_keys=False)
    ...
    def to_cloudformation_config_data(self):
        ...
        return {to_big_camel_case(key):value for key, value in self.to_dict().items()}
