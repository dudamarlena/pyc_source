...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\debug_a\monitor\single_monitor.py
# Compiled at: 2018-05-12 02:41:01
...
def pv_relationship(code, interval=300, plot=False):
    ...
    data['timestamp'] = data['datetime'].apply(lambda x: int(datetime.strptime(x, '%Y-%m-%d %H:%M').timestamp()))
...
def _single_notify(code):
    ...
    if not isinstance(code, str):
        ...
        assert len(code) == 6, 'A股代码长度应该为6, 当前code(值：%s)长度为%i' % (
         code, len(code))
    rt = bars(code)
    name = rt['name'][0]
    ...
    message = '【{0}】当前价【{1}】元，今日累计波动【{2}】元，成交量【{3}】万元。'.format(name, price, wave, vol)
    return message
    ...
    while loop:
        ...
        for i in bar.index:
            try:
                ...
                if max_buy > bgd:
                    msg_b = '{0}({1}): {2}，挂单数量 {3}手'.format(code, name, labels.GDBD['name'], str(max_buy))
                    logger.info(msg_b)
                if max_sell > bgd:
                    msg_s = '{0}({1}): {2}，挂单数量 {3}手'.format(code, name, labels.GDSD['name'], str(max_sell))
                    logger.info(msg_s)
            except:
                continue
