...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\debug_a\xueqiu\comment.py
...
def get_comments(code, sleep=1):
    ...
    for i in range(1, total_page + 1):
        ...
        for r in res:
            com = {'text':BeautifulSoup(r['text'], 'lxml').text, 
             'id':r['id'], 
             'time':r['timeBefore'], 
...
def _get_sub_comments(comment_id):
    ...
    """
    sub_com_url = 'https://xueqiu.com/statuses/comments.json?id={comment_id}&count=20&page=1&reply=true&asc=false&type=status&split=true'
