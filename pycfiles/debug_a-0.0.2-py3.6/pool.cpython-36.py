...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\debug_a\trader\pool.py
...
def _verify_share(share):
    base = {'code':None, 
     ...
     'price':None}
    try:
        if not share.keys() == base.keys():
            raise AssertionError('share中应当包含这些key：%s' % str(base.keys()))
        elif not share['level'] in (1, 2, 3):
            raise AssertionError('level取值必须是[1, 2, 3]中的一个，当前值：%s' % str(share['level']))
        return True
    except:
        return False
...
class StockPool:
    ...
    def __init__(self, name):
        self.name = name
        self.path = os.path.join(POOL_DIR, '%s.pool' % self.name)
