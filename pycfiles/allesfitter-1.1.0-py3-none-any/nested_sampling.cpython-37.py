...
# [GCC 8.4.0]
# Embedded file name: /Users/mx/Dropbox (MIT)/Science/Code/allesfitter/allesfitter/nested_sampling.py
# Compiled at: 2019-12-15 19:29:04
    ...
    if config.BASEMENT.settings['ns_modus'] == 'static':
        ...
        if config.BASEMENT.settings['multiprocess']:
            with closing(Pool(processes=(config.BASEMENT.settings['multiprocess_cores']))) as (pool):
                ...
                sampler = dynesty.NestedSampler(ns_lnlike, ns_prior_transform, ndim, pool=pool,
            ...
            sampler = dynesty.NestedSampler(ns_lnlike, ns_prior_transform, ndim, bound=bound,
              sample=sample,
              nlive=nlive)
            sampler.run_nested(dlogz=tol, print_progress=(config.BASEMENT.settings['print_progress']))
        t1 = timer()
        ...
        if config.BASEMENT.settings['ns_modus'] == 'dynamic':
            ...
            if config.BASEMENT.settings['multiprocess']:
                with closing(Pool(processes=(config.BASEMENT.settings['multiprocess_cores']))) as (pool):
                    logprint('\nRunning on', config.BASEMENT.settings['multiprocess_cores'], 'CPUs.')
                    sampler = dynesty.DynamicNestedSampler(ns_lnlike, ns_prior_transform, ndim, pool=pool,
                      queue_size=(config.BASEMENT.settings['multiprocess_cores']),
                      bound=bound,
                      sample=sample)
                    sampler.run_nested(nlive_init=nlive, dlogz_init=tol, print_progress=(config.BASEMENT.settings['print_progress']))
            else:
                sampler = dynesty.DynamicNestedSampler(ns_lnlike, ns_prior_transform, ndim, bound=bound,
                  sample=sample)
