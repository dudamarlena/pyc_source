...
# [GCC 8.4.0]
# Embedded file name: /Users/mx/Dropbox (MIT)/Science/Code/allesfitter/deriver.py
# Compiled at: 2018-11-27 12:05:03
"""
...
import os, numpy as np, matplotlib.pyplot as plt, pickle
...
def derive(samples, mode, output_units='jup'):
    ...
    for companion in companions:
        ...
        derived_samples[companion + '_R_companion'] = star['R_star'] * get_params(companion + '_rr') * R_sun / R_earth
        suffix = 'earth'
        if np.mean(derived_samples[(companion + '_R_companion')]) > 8.0:
            derived_samples[companion + '_R_companion'] = derived_samples[(companion + '_R_companion')] * R_earth / R_jup
            suffix = 'jup'
        derived_samples[companion + '_depth_undiluted'] = 1000.0 * get_params(companion + '_rr') ** 2
        ...
        derived_samples[companion + '_w'] = arccos_d(get_params(companion + '_f_c') / np.sqrt(derived_samples[(companion + '_e')]))
        if companion + '_K' in config.BASEMENT.params:
            ...
            derived_samples[companion + '_q'] = 1.0 / (derived_samples[(companion + '_a')] / a_1 - 1.0)
            if suffix == 'earth':
                derived_samples[companion + '_M_companion'] = derived_samples[(companion + '_q')] * star['M_star'] * M_sun / M_earth
            elif suffix == 'jup':
                derived_samples[companion + '_M_companion'] = derived_samples[(companion + '_q')] * star['M_star'] * M_sun / M_jup
        else:
            derived_samples[companion + '_M_companion'] = None
        if config.BASEMENT.settings['secondary_eclipse'] is True:
            derived_samples[companion + '_dt_occ'] = get_params(companion + '_period') / 2.0 * (1.0 + 4.0 / np.pi * derived_samples[(companion + '_e')] * cos_d(derived_samples[(companion + '_w')]))
        else:
            derived_samples[companion + '_dt_occ'] = None
        derived_samples[companion + '_b_tra'] = 1.0 / derived_samples[(companion + '_R_star/a')] * get_params(companion + '_cosi') * ((1.0 - derived_samples[(companion + '_e')] ** 2) / (1.0 + derived_samples[(companion + '_e')] * sin_d(derived_samples[(companion + '_w')])))
        if config.BASEMENT.settings['secondary_eclipse'] is True:
            derived_samples[companion + '_b_occ'] = 1.0 / derived_samples[(companion + '_R_star/a')] * get_params(companion + '_cosi') * ((1.0 - derived_samples[(companion + '_e')] ** 2) / (1.0 - derived_samples[(companion + '_e')] * sin_d(derived_samples[(companion + '_w')])))
        else:
            derived_samples[companion + '_b_occ'] = None
        derived_samples[companion + '_T_tra_tot'] = get_params(companion + '_period') / np.pi * 24.0 * np.arcsin(derived_samples[(companion + '_R_star/a')] * np.sqrt((1.0 + get_params(companion + '_rr')) ** 2 - derived_samples[(companion + '_b_tra')] ** 2) / sin_d(derived_samples[(companion + '_i')]))
        derived_samples[companion + '_T_tra_full'] = get_params(companion + '_period') / np.pi * 24.0 * np.arcsin(derived_samples[(companion + '_R_star/a')] * np.sqrt((1.0 - get_params(companion + '_rr')) ** 2 - derived_samples[(companion + '_b_tra')] ** 2) / sin_d(derived_samples[(companion + '_i')]))
        for inst in config.BASEMENT.settings['inst_phot']:
            ...
            if config.BASEMENT.settings['secondary_eclipse'] is True:
                ...
                for i in tqdm(range(N_samples)):
                    ...
                    derived_samples[(companion + '_depth_occ_max_diluted_' + inst)][i] = (np.max(model) - np.min(model)) * 1000000.0
                    derived_samples[(companion + '_depth_occ_norm_diluted_' + inst)][i] = (1.0 - np.min(model)) * 1000000.0

        for inst in config.BASEMENT.settings['inst_phot']:
            dil = get_params('light_3_' + inst)
            derived_samples[companion + '_depth_diluted_' + inst] = derived_samples[(companion + '_depth_undiluted')] * (1.0 - dil)
            derived_samples[companion + '_depth_occ_max_undiluted_' + inst] = derived_samples[(companion + '_depth_occ_max_diluted_' + inst)] / (1.0 - dil)
            derived_samples[companion + '_depth_occ_norm_undiluted_' + inst] = derived_samples[(companion + '_depth_occ_norm_diluted_' + inst)] / (1.0 - dil)
    ...
    with open(os.path.join(config.BASEMENT.outdir, mode + '_derived_table.csv'), 'wb') as (outfile):
        with open(os.path.join(config.BASEMENT.outdir, mode + '_derived_latex_table.txt'), 'wb') as (f):
            with open(os.path.join(config.BASEMENT.outdir, mode + '_derived_latex_cmd.txt'), 'wb') as (f_cmd):
    ...
    fig = corner(x, range=[
     0.999] * len(names), labels=names, quantiles=[
