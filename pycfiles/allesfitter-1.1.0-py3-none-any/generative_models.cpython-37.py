...
# [GCC 8.4.0]
# Embedded file name: /Users/mx/Dropbox (MIT)/Science/Code/allesfitter/allesfitter/generative_models.py
# Compiled at: 2020-03-04 17:54:18
# Size of source mod 2**32: 19489 bytes
"""
...
def translate_physical_to_ellc(period=1.0, R_companion=1.0, M_companion=1.0, R_companion_unit='Rearth', M_companion_unit='Mearth', R_host=1.0, M_host=1.0, incl=90, ecc=0, omega=0):
    if R_companion_unit == 'Rearth':
        R_companion_with_unit = R_companion * u.Rearth
    else:
        if R_companion_unit == 'Rjup':
            R_companion_with_unit = R_companion * u.Rjup
        else:
            if R_companion_unit == 'Rsun':
                R_companion_with_unit = R_companion * u.Rsun
            elif M_companion_unit == 'Mearth':
                M_companion_with_unit = M_companion * u.Mearth
            else:
                if M_companion_unit == 'Mjup':
                    M_companion_with_unit = M_companion * u.Mjup
                else:
                    if R_companion_unit == 'Msun':
                        R_companion_with_unit = R_companion * u.Msun
            a = (G / (4 * np.pi ** 2) * (period * u.d) ** 2 * (M_host * u.Msun + M_companion_with_unit)) ** 0.3333333333333333
            a = a.to(u.AU)
            ...
            a = a.value
            return (
             radius_1, radius_2, cosi, a, f_c, f_s)


def translate_physical_to_allesfitter(period=1.0, R_companion=1.0, M_companion=1.0, R_companion_unit='Rearth', M_companion_unit='Mearth', R_host=1.0, M_host=1.0, incl=90, ecc=0, omega=0, ldc=[
 0.6, 0.2], ld='quad'):
    if R_companion_unit == 'Rearth':
        R_companion_with_unit = R_companion * u.Rearth
    else:
        if R_companion_unit == 'Rjup':
            ...
            a = a.value
            return (
             rr, rsuma, cosi, a, f_c, f_s, ldc_transformed)
...
def make_lc_model(time, flux=None, flux_err=None, epoch=0.0, period=1.0, R_companion=1.0, M_companion=1.0, R_companion_unit='Rearth', M_companion_unit='Mearth', R_host=1.0, M_host=1.0, incl=90, ecc=0, omega=0, ldc=[
 0.6, 0.2], ld='quad', dil=0, sbratio=0, show_plot=False, save_plot=False, save_csv=False, fname_plot='lc.pdf', fname_csv='lc.csv'):
    ...
    if show_plot or save_plot:
        ...
        ax.set(ylabel='Flux', xlabel='Time $\\mathrm{(BJD_{TDB})}$')
        ...
        ax.plot((phi * period * 24.0 * 60), flux2, 'k.', color='lightgrey', rasterized=True, zorder=10)
        ax.errorbar((phase * period * 24.0 * 60), phaseflux, yerr=phaseflux_err, fmt='b.', zorder=11)
        ax.plot((xx_phase * period * 24.0 * 60), model_flux_xx_phase, 'r-', lw=2, zorder=12)
        ax.set(xlim=[-0.03 * period * 24.0 * 60, 0.03 * period * 24.0 * 60])
        ax.set(xlabel='Time (min.)')
        ax = fig.add_subplot(gs[(2, 0)])
        ax.set_axis_off()
        ax.text(0, 0.95, ('$T_0$ = ' + str(epoch) + ' $\\mathrm{BJD_{TDB}}$'), transform=(ax.transAxes))
        ...
        ax.text(0, 0.25, ('$\\sqrt{e} \\cos{\\omega}$ = ' + np.format_float_positional(f_c, 5, False)), transform=(ax.transAxes))
        ax.text(0, 0.15, ('$\\sqrt{e} \\sin{\\omega}$ = ' + np.format_float_positional(f_s, 5, False)), transform=(ax.transAxes))
...
def inject_rv_model(time, rv, rv_err, epoch, period, R_companion=1.0, M_companion=1.0, R_companion_unit='Rearth', M_companion_unit='Mearth', R_host=1.0, M_host=1.0, sbratio=0, incl=90, ecc=0, omega=0, dil=0, ldc=[
 0.6, 0.2], ld='quad', show_plot=False, save_plot=False, save_csv=False, outdir='', fname_plot=None, fname_csv=None):
    """
    Inputs:
    -------
    time : array of float
    ...
    if R_companion_unit == 'Rearth':
        ...
        def get_model_rv(xx):
            return ellc.rv(t_obs=xx, radius_1=radius_1,
              radius_2=radius_2,
              sbratio=sbratio,
              incl=incl,
              t_zero=epoch,
