...
# Compiled at: 2019-01-23 21:06:31
"""
...
def gp_decor(x, y, yerr=None, ind_in=None, ind_out=None, period=None, epoch=None, width=None, width_2=None, secondary_eclipse=False, systematics_timescale=None, mean=1.0, nwalkers=50, thin_by=50, burn_steps=2500, total_steps=5000, bin_width=None, gp_code='celerite', method='median_posterior', chunk_size=2000, Nsamples_detr=10, Nsamples_plot=10, xlabel='x', ylabel='y', ydetr_label='ydetr', outdir='gp_decor', fname=None, fname_summary=None, multiprocess=False, multiprocess_cores=None):
    """
    Required Input:
    ---------------
    x : array of float
    ...
    if (gp_code == 'celerite') & ('celerite' not in sys.modules):
        ...
        if (gp_code == 'george') & ('george' not in sys.modules):
        ...
        params, params_ll, params_ul = get_params_from_samples(samples, keys)
        with open(os.path.join(outdir, fname + 'table.csv'), 'w') as (f):
            f.write('name,median,ll,ul\n')
            for i, key in enumerate(keys):
                f.write(names[i] + ',' + str(params[key]) + ',' + str(params_ll[key]) + ',' + str(params_ul[key]) + '\n')
    ...
    plt.close(fig)
    fig = corner.corner(samples, labels=names, show_titles=True, title_kwargs={'fontsize': 12})
