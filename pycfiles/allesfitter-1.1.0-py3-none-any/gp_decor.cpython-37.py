    ...
    if (gp_code == 'celerite') & ('celerite' not in sys.modules):
                ...
                if kernel == 'SHOT':
                    ...
                    if multiprocess:
                        if not multiprocess_cores:
                            ...
                            elif systematics_amplitude is not None:
        ...
        """
            ...
            if method == 'mean_curve':
                ...
                if method == 'median_posterior':
                    ...
                    if bin_width is not None:
                        ...
                        plt.close(fig)
                    Norbits = any((v is None for v in [period, epoch, width])) or int((x[(-1)] - x[0]) / period) + 1
            ...
            if method == 'mean_curve':
                ...
                plt.close(fig)
                phase_x, phase_ydetr, phase_ydetr_err, _, phi = any((v is None for v in [period, epoch, width])) or phase_fold(x, ydetr, period, epoch, dt=dt, ferr_type=ferr_type, ferr_style=ferr_style, sigmaclip=sigmaclip)
                fig, ax = plt.subplots(figsize=(6 * figstretch, 4))
