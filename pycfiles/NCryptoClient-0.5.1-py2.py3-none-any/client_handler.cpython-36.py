...
# [GCC 8.4.0]
# Embedded file name: D:\dev\PyCharm Projects\NCryptoClient\NCryptoClient\Transmitter\client_handler.py
# Compiled at: 2018-04-19 19:45:57
# Size of source mod 2**32: 12284 bytes
"""
...
class MsgHandler(QThread):
    ...
    def __init__(self, ipv4_address, port_number, socket_family=socket.AF_INET, socket_type=socket.SOCK_STREAM, wait_time=0.05):
        """
        Конструктор.
    ...
    def _server_to_client_personal_msg(self, msg_dict):
        ...
        @return: -
        """
        msg_string = '[{}] @{}> {}'.format(get_formatted_date(msg_dict['time']), msg_dict['from'], msg_dict['message'])
    ...
    def _server_to_client_join_chat(self, msg_dict):
        ...
        @return: -
        """
        msg_string = '[{}] @Server> {} joined {} chatroom.'.format(get_formatted_date(msg_dict['time']), msg_dict['login'], msg_dict['room'])
    ...
    def _server_to_client_leave_chat(self, msg_dict):
        ...
        @return: -
        """
        msg_string = '[{}] @Server> {} left {} chatroom.'.format(get_formatted_date(msg_dict['time']), msg_dict['login'], msg_dict['room'])
        self.add_message_signal.emit(msg_dict['room'], msg_string)
    ...
    def _server_to_client_quantity(self, msg_dict):
        ...
        @return: -
        """
        alert_msg = '[{}] @Server> Amount of contacts: {}'.format(get_current_time(), msg_dict['quantity'])
    ...
    def _server_to_client_alert(self, msg_dict):
        ...
        if str(msg_dict['response'])[0] in ('1', '2'):
            if self._main_window.get_auth_state():
                alert_msg = '[{}] @Server> Alert {}: {}'.format(get_current_time(), msg_dict['response'], msg_dict['alert'])
                self.add_log_signal.emit(alert_msg)
                self._handle_alert_message(msg_dict['alert'])
            elif msg_dict['response'] == 200:
    ...
    def _server_to_client_error(self, msg_dict):
        ...
        if str(msg_dict['response'])[0] in ('4', '5'):
            if self._main_window.get_auth_state():
                error_msg = '[{}] @Server> Error {}: {}'.format(get_current_time(), msg_dict['response'], msg_dict['error'])
                self.add_log_signal.emit(error_msg)
            else:
                if msg_dict['response'] == 401:
                    self.show_message_box_signal.emit('Invalid authentication data!', 'Authentication has failed! Try again!')
                else:
                    self.show_message_box_signal.emit('Unknown error!', 'An unknown error has occured! Try again!')
    ...
    def _handle_alert_message(self, message_text):
        ...
        if re.fullmatch(re_left, message_text) is not None:
            ...
            return
        re_added = re.compile("^(Contact '((#[A-Za-z_\\d]{3,31})|([A-Za-z_\\d]{3,32}))' has been successfully added!)$")
        if re.fullmatch(re_added, message_text) is not None:
            contact_name = message_text.split("'")[1]
            self.add_contact_signal.emit(contact_name)
            return
        re_removed = re.compile("^(Contact '((#[A-Za-z_\\d]{3,31})|([A-Za-z_\\d]{3,32}))' has been successfully removed!)$")
        if re.fullmatch(re_removed, message_text) is not None:
            contact_name = message_text.split("'")[1]
            self.remove_contact_signal.emit(contact_name)
            return
        self.show_message_box_signal.emit('Incorrect message format!', 'Could not parse message from the server!')