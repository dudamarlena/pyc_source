...
# [GCC 8.4.0]
# Embedded file name: /code/s3direct/tests.py
# Compiled at: 2019-07-26 08:24:58
import hashlib, json
...
class WidgetTestCase(TestCase):
    ...
    def test_directory_object_key(self):
        ...
        self.assertEqual(response.status_code, 200)
        policy_dict = json.loads(response.content.decode())
        self.assertEqual(policy_dict['object_key'], 'uploads/folder/%s' % data['name'])

    def test_directory_object_key_with_leading_slash(self):
        data = {'dest': 'accidental-leading-slash', 
           'name': 'filename.jpg', 
           'type': 'image/jpeg', 
           'size': 1000}
        self.client.login(username='admin', password='admin')
        ...
        self.assertEqual(response.status_code, 200)
        policy_dict = json.loads(response.content.decode())
        self.assertEqual(policy_dict['object_key'], 'uploads/folder/filename.jpg')

    def test_directory_object_key_with_trailing_slash(self):
        data = {'dest': 'accidental-trailing-slash', 
           'name': 'filename.jpg', 
           'type': 'image/jpeg', 
           'size': 1000}
        self.client.login(username='admin', password='admin')
    ...
    def test_function_object_key_with_args(self):
        data = {'dest': 'function-object-key-args', 
           'name': 'filename.jpg', 
           'type': 'image/jpeg', 
           'size': 1000}
        self.client.login(username='admin', password='admin')
        ...
        self.assertEqual(response.status_code, 200)
        policy_dict = json.loads(response.content.decode())
        self.assertEqual(policy_dict['object_key'], settings.S3DIRECT_DESTINATIONS['function-object-key-args']['key_args'] + '/' + data['name'])

    def test_policy_conditions(self):
        self.client.login(username='admin', password='admin')
        data = {'dest': 'policy-conditions', 
           'name': 'filename.jpg', 
           'type': 'image/jpeg', 
           'size': 1000}
        response = self.client.post(reverse('s3direct'), data)
        self.assertEqual(response.status_code, 200)
        policy_dict = json.loads(response.content.decode())
        self.assertEqual(policy_dict['bucket'], 'astoragebucketname')
        self.assertEqual(policy_dict['acl'], 'authenticated-read')
        self.assertEqual(policy_dict['cache_control'], 'max-age=2592000')
        self.assertEqual(policy_dict['content_disposition'], 'attachment')
        self.assertEqual(policy_dict['server_side_encryption'], 'AES256')

    def test_custom_existence_optimisation_true(self):
        data = {'dest': 'allow-existence-optimisation', 
           'name': 'filename.jpg', 
           'type': 'image/jpeg', 
           'size': 1000}
        response = self.client.post(reverse('s3direct'), data)
    ...
    def test_custom_existence_optimisation_false(self):
        data = {'dest': 'disallow-existence-optimisation', 
           'name': 'filename.jpg', 
           'type': 'image/jpeg', 
           'size': 1000}
        response = self.client.post(reverse('s3direct'), data)
    ...
    def test_custom_existence_optimisation_unset(self):
        data = {'dest': 'unset-existence-optimisation', 
           'name': 'filename.jpg', 
           'type': 'image/jpeg', 
           'size': 1000}
        response = self.client.post(reverse('s3direct'), data)
    ...
    def test_custom_region_bucket(self):
        data = {'dest': 'custom-region-bucket', 
           'name': 'filename.jpg', 
           'type': 'image/jpeg', 
           'size': 1000}
        response = self.client.post(reverse('s3direct'), data)
        self.assertEqual(response.status_code, 200)
        policy_dict = json.loads(response.content.decode())
        self.assertEqual(policy_dict['endpoint'], 'https://s3.cn-north-1.amazonaws.com.cn')

    def test_optional_param_content_disposition_callable(self):
        data = {'dest': 'optional-content-disposition-callable', 
           'name': 'filename.jpg', 
           'type': 'image/jpeg', 
           'size': 1000}
        response = self.client.post(reverse('s3direct'), data)
    ...
    def test_optional_param_cache_control_non_callable(self):
        data = {'dest': 'optional-cache-control-non-callable', 
           'name': 'filename.jpg', 
           'type': 'image/jpeg', 
           'size': 1000}
        response = self.client.post(reverse('s3direct'), data)
...
class SignatureViewTestCase(TestCase):
    ...
    def create_dummy_signing_request(self):
        ...
        canonical_request = ('{request_method}\n/{bucket}/{object_key}\nhost={host}\nx-amz-date:{request_datetime}\n\nhost;x-amz-date\n{hashed_payload}').format(request_method='GET', bucket='abucketname', object_key='an/object/key', host='s3.amazonaws.com', request_datetime=datetime.strftime(signing_date, '%Y%m%dT%H%M%SZ'), hashed_payload='blahblahblah')
        credential_scope = ('{request_date}/{region}/s3/aws4_request').format(request_date=datetime.strftime(signing_date, '%Y%m%d'), region='eu-west-1')
        hashed_canonical_request = hashlib.sha256(canonical_request.encode('utf-8')).hexdigest()
        string_to_sign = ('{algorithm}\n{request_datetime}\n{credential_scope}\n{hashed_canonical_request}').format(algorithm='AWS-HMAC-SHA256', request_datetime=datetime.strftime(signing_date, '%Y%m%dT%H%M%SZ'), credential_scope=credential_scope, hashed_canonical_request=hashed_canonical_request)
        return (
         string_to_sign, signing_date)
    ...
    def test_signing(self):
        ...
        response = self.client.post(reverse('s3direct-signing'), data={'to_sign': string_to_sign, 
           'datetime': datetime.strftime(signing_date, '%Y%m%dT%H%M%SZ'), 
           'dest': 'login-not-required'}, enforce_csrf_checks=True)
    ...
    def test_signing_with_protected(self):
        ...
        response = self.client.post(reverse('s3direct-signing'), data={'to_sign': string_to_sign, 
           'datetime': datetime.strftime(signing_date, '%Y%m%dT%H%M%SZ'), 
           'dest': 'login-required'}, enforce_csrf_checks=True)
        expected = '{"s3ObjKey": "%s"}' % self.EXPECTED_SIGNATURE
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.content, expected.encode('utf-8'))
...
class AWSCredentialsTest(TestCase):
    ...
    def test_retrieves_aws_credentials_from_botocore(self, botocore_mock):
        credentials_mock = mock.Mock(token='token', secret_key='secret_key', access_key='access_key')
