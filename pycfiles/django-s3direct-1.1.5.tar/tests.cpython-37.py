...
# Size of source mod 2**32: 18971 bytes
import hashlib, json
...
class SignatureViewTestCase(TestCase):
    ...
    def create_dummy_signing_request(self):
        ...
        canonical_request = '{request_method}\n/{bucket}/{object_key}\nhost={host}\nx-amz-date:{request_datetime}\n\nhost;x-amz-date\n{hashed_payload}'.format(request_method='GET',
          bucket='abucketname',
          object_key='an/object/key',
          host='s3.amazonaws.com',
          request_datetime=(datetime.strftime(signing_date, '%Y%m%dT%H%M%SZ')),
          hashed_payload='blahblahblah')
        credential_scope = '{request_date}/{region}/s3/aws4_request'.format(request_date=(datetime.strftime(signing_date, '%Y%m%d')),
          region='eu-west-1')
        hashed_canonical_request = hashlib.sha256(canonical_request.encode('utf-8')).hexdigest()
        string_to_sign = '{algorithm}\n{request_datetime}\n{credential_scope}\n{hashed_canonical_request}'.format(algorithm='AWS-HMAC-SHA256',
          request_datetime=(datetime.strftime(signing_date, '%Y%m%dT%H%M%SZ')),
    ...
    def test_signing(self):
        ...
        response = self.client.post((reverse('s3direct-signing')),
          data={'to_sign':string_to_sign, 
...
class AWSCredentialsTest(TestCase):
    ...
    def test_retrieves_aws_credentials_from_botocore(self, botocore_mock):
        credentials_mock = mock.Mock(token='token',
