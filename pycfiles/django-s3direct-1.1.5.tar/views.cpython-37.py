...
# [GCC 8.4.0]
# Embedded file name: /code/s3direct/views.py
# Compiled at: 2019-07-27 06:26:35
...
from .utils import get_aws_credentials, get_aws_v4_signature, get_aws_v4_signing_key, get_s3direct_destinations, get_key
...
def get_upload_params(request):
    ...
    if cl_range:
        if not cl_range[0] <= file_size <= cl_range[1]:
    ...
    if not (aws_credentials.secret_key and aws_credentials.access_key):
        resp = json.dumps({'error': 'AWS credentials config missing.'})
        return HttpResponseServerError(resp, content_type='application/json')
    upload_data = {'object_key':get_key(key, file_name, dest), 
