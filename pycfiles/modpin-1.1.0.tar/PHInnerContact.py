...
# [GCC 8.4.0]
# Embedded file name: /home/patricia/patricia/modppi/./src/SBI/structure/contacts/inner/PHInnerContact.py
...
class PHInnerContact(Interface):
    ...
    def __init__(self, protein=None, threshold_type='min', threshold_distance=8):
        ...
        hetero_centers = protein.geometric_center(structure=False, hetero=True, water=False, by_residue=True)
        super(PHInnerContact, self).__init__(chain1=protein, chain2=protein, threshold_type=threshold_type, threshold_distance=threshold_distance, chain1_centres=protein_centers, chain2_centers=hetero_centers)
    ...
    def _build(self):
        if len(self.protein.aminoacids) == 0 or len(self.protein.heteroatoms) == 0:
            return
        super(PHInnerContact, self)._build()
        for i in range(len(self._filtered[0])):
            new_contact = ContactAH(aminoacid=self.protein.aminoacids[self._filtered[0][i]], heteroatom=self.protein.heteroatoms[self._filtered[1][i]], threshold_type=self.threshold_type, threshold_distance=self.threshold_distance)
