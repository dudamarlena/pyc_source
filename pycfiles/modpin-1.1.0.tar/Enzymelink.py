...
# [GCC 8.4.0]
# Embedded file name: ./src/SBI/databases/Enzymelink.py
...
class Enzymelink(object):
    ...
    def __init__(self, local=None):
        ...
        self.__name__ = 'databases.Enzymelink'
    ...
    def _parse_enzymedat(self):
        ...
        reactions_dic = {}
        enzymes = []
        cofactor_separator = re.compile('; | or |;| and ')
        multiple_reac_sepr = re.compile('\\.\\(\\d+\\)\\s')
        noisy_words_equal = re.compile('^A |^An |secondary | a | an ')
        unspaced_summator = re.compile(' \\+|\\+ ')
        unspaced_equalate = re.compile(' \\=|\\= ')
        summator_separator = re.compile(' \\+ | or ')
        equalate_separator = re.compile(' \\= ')
        cardinal_numerical = re.compile('^(\\d+)\\s+')
        fd = open(self._dfile)
        for line in fd:
            line = re.sub("'", "\\'", line)
...
class Enzyme(object):
    ...
    def _transfers(self):
        if self.description.startswith('Transferred entry'):
            d = self.description
            d.strip('.')
            aggregator_separator = re.compile(',|and')
