...
# [GCC 8.4.0]
# Embedded file name: /home/patricia/patricia/modppi/./src/SBI/structure/protein/Arch.py
...
class Arch(StorableObject, JSONer):
    ...
    def __init__(self, source, ss1, ss2, internalss, distanceAA, structure, torsionsCA, PsiPhi, order):
        self._source = source
        self._ss1 = ss1
        self._ss2 = ss2
        self._intss = internalss
        self._inttxt = []
        self._inttyp = ''
        ...
        self._angles()
        self._torgls = [
         self._angle_torsion_vector(self._angls[0], 180),
    ...
    def _calculate_hydrogen_bond_matrix(self):
        ...
        if matrix_min_border_index > 1:
            matrix_min_border_index -= 2
        else:
            matrix_min_border_index = 0
        if matrix_max_border_index < len(self._str.aminoacids) - 2:
            matrix_max_border_index += 2
        else:
            matrix_max_border_index = len(self._str.aminoacids) - 1
        ...
        for j in range(matrix_size):
            for i in range(matrix_size):
                ...
                if matrix[i][j] <= -0.6:
                    ...
                    if position_i <= ss1_end_index and position_j >= ss2_ini_index or position_i >= ss2_ini_index and position_j <= ss1_end_index:
                        hbond_ss += 1
                    elif position_i > ss1_end_index and position_i < ss2_ini_index or position_j > ss1_end_index and position_j < ss2_ini_index:
                        hbond_loop += 1
    ...
    def _distance_torsion_vector(self):
        ...
        if self._dist <= 7.5:
            data[border1], data[border2] = ('s', 's')
        elif self._dist <= 10:
            data[border1], data[border2] = ('m', 'm')
        elif self._dist <= 20:
            data[border1], data[border2] = ('l', 'l')
        else:
            data[border1], data[border2] = ('L', 'L')
        return ('').join(data)
    ...
    def as_dict(self):
        return {'id': self.identifier, 'type': self.type, 
    ...
    def line_format(self, sequence=False):
        ...
        if sequence:
            data.extend([self.aminoacid_sequence, self.structure_sequence])
        return ('\t').join(data)
