...
# [GCC 8.4.0]
# Embedded file name: /home/patricia/patricia/modppi/./src/SBI/structure/contacts/inner/PPInnerContact.py
...
class PPInnerContact(Interface):
    ...
    def __init__(self, protein=None, threshold_type='cb', threshold_distance=12):
        ...
        super(PPInnerContact, self).__init__(chain1=protein, chain2=protein, threshold_type=threshold_type, threshold_distance=threshold_distance, chain1_centres=protein_centers, chain2_centers=protein_centers)
    ...
    def _build(self):
        if len(self.protein.aminoacids) == 0:
            return
        ...
        for i in range(len(self._filtered[0])):
            if self._filtered[0][i] != self._filtered[0][j]:
                new_contact = ContactAA(aminoacid1=self.protein.aminoacids[self._filtered[0][i]], aminoacid2=self.protein.aminoacids[self._filtered[1][i]], threshold_type=self.threshold_type, threshold_distance=self.threshold_distance)
