...
# [GCC 8.4.0]
# Embedded file name: /home/patricia/patricia/modppi/./src/SBI/structure/chain/ChainOfProtein.py
...
class ChainOfProtein(Chain):
    ...
    def calculate_archs(self, limit_internal_ss=100, limit_distance=False, allowed_gaps=0):
        if not self.has_dssp:
            self.calculate_dssp()
        if not self.has_valid_secondary_structures:
        ...
        SShelper.calculate_archs(self, limit_internal_ss, limit_distance, allowed_gaps)
    ...
    def archs2json(self, as_string=True):
        data = {'ID': self.globalID, 'SEQ': self.gapped_protein_sequence, 
           'STR': self.gapped_protein_secondary_structure, 
           'ARCHS': [ x.json_format(False) for x in self.archs ], 'SUPER': [ x.json_format(False) for x in self.superarchs ]}
        if as_string:
            return repr(data)
        return data