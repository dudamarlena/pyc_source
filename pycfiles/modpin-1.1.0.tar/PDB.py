...
# [GCC 8.4.0]
# Embedded file name: /home/patricia/patricia/modppi/./src/SBI/structure/PDB.py
...
class PDB(StorableObject):
    ...
    def __init__(self, pdb_file=None, dehydrate=False, header=False, onlyheader=False, biomolecule=False):
        """
        ...
        if self.pdb_file is not None:
            ...
            self._read_PDB_file(header=header, onlyheader=onlyheader, biomolecule=biomolecule)
        if dehydrate:
            self.dehydrate()
        return
    ...
    def pdb_file(self, value):
        ...
        if self._pdb_file is not None:
            raise AttributeError(('The PDB object is loaded from file {0}. To load the new file {1} create a new PDB object').format(self._pdb_file.full, value))
    ...
    def duplicate(self, hetero=True, water=False, NMR=False):
        ...
        for chain in self.chains:
            new_PDB.add_chain(chain=chain.duplicate(hetero=hetero, water=water))

        if NMR:
            for chain in self._NMR_chains:
                new_PDB.add_chain(chain=chain.duplicate(hetero=hetero, water=water), NMR=True)
    ...
    def apply_biomolecule_matrices(self, keepchains=False, water=True):
        ...
        for matrix in self.header.biomolecules:
            PDB_list.append(self._apply_matrix(matrix=matrix, keepchains=keepchains, water=water))
            ...
            if chain.chain not in chainsNIDs and chain_change:
                ...
                if cluster_by_alternative_id:
                    ...
                    if not self._COMPND.has_key(chain.alternative_id):
                        self._COMPND.setdefault(chain.alternative_id, []).append(chain.alternative_id)
    ...
    def fuse_chains(self, chains_ids):
        ...
        if len(self._chain_id.intersection(set(chains_ids))) < len(chains_ids):
            raise AttributeError('Some of the given chains to fues do not exist')
        error_counter = 0
        ...
        for c in chains_ids:
            ...
            if error_counter == 2:
                raise AttributeError('Fuse different kinds of structural chain is not possible\n')
    ...
    def FASTA_format(self, gapped=True, protein=True, nucleotide=False):
        ...
        for c in self.chains:
            if isinstance(c, ChainOfProtein) and protein:
                lines.append(('>{0}\t{1}').format(c.globalID, c.aminoacids[0].identifier))
                if gapped:
            ...
            if isinstance(c, ChainOfNucleotide) and nucleotide:
                lines.append(('>{0}\t{1}').format(c.globalID, c.nucleotides[0].identifier))
                if gapped:
    ...
    def IDX_format(self, protein=True, nucleotide=False):
        ...
        for c in self.chains:
            ...
            if isinstance(c, ChainOfNucleotide) and nucleotide:
                lines.append(('>{0}\t{1}').format(c.globalID, c.nucleotide_idx()))
    ...
    def FASTA_IDX(self, protein=True, nucleotide=False):
        ...
        for c in self.chains:
            if isinstance(c, ChainOfProtein) and protein:
                ...
                data['IDX'].append(('>{0}\t{1}').format(c.globalID, c.protein_idx))
            if isinstance(c, ChainOfNucleotide) and nucleotide:
                data['FASTA'].append(('>{0}\n{1}').format(c.globalID, c.gapped_nucleotide_sequence()))
                data['IDX'].append(('>{0}\t{1}').format(c.globalID, c.nucleotide_idx()))
