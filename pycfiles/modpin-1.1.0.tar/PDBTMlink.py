...
# [GCC 8.4.0]
# Embedded file name: ./src/SBI/databases/PDBTMlink.py
        ...
        for xmlfile in Path.list_files(os.path.join(self._local, 'pdbtm/database/'), '*.xml'):
            ...
            for line in fdxml:
                if line.startswith('    <TMRES>'):
                    xmldata.tmres = line
                elif line.startswith('    <TMTYPE'):
                    xmldata.tmtype = line
                elif line.startswith('    <PDBKWRES'):
                    xmldata.kwres = line
...
class TM(object):
    section_types = {'1': 'Side1', '2': 'Side2', 'i': 'Inside', 'o': 'Outside', 'B': 'Beta-strand', 
       'H': 'alpha-helix', 'C': 'coil', 'I': 'membrane-inside', 
       'L': 'membrane-loop', 'F': 'interfacial helix', 'U': 'unknown localizations'}
    ...
    @tmres.setter
    def tmres(self, value):
        self._tmres = TM._clean_xml(value)
    ...
    @tmtype.setter
    def tmtype(self, value):
        self._tmtype = TM._clean_xml(value)
    ...
    @kwres.setter
    def kwres(self, value):
        self._kwres = TM._clean_xml(value)
