...
# [GCC 8.4.0]
# Embedded file name: /home/patricia/patricia/modppi/./src/SBI/external/blast/BlastHit.py
...
"""
import re
from SBI.sequence import IndexedSeqAli
...
class BlastHit(IndexedSeqAli):
    ...
    def __init__(self, name, length, iteration=0, e_value=None, align_length=None, identities=None, positives=None, gaps=None, qseq=None, hseq=None, qpos=None, hpos=None, score_seq=None):
        """
        ...
        """
        super(BlastHit, self).__init__(sequences=[str(qseq), str(hseq)], sequenceInits=[
         qpos, hpos], identities=identities, positives=positives, gaps=gaps)
        self.add_alignment_pattern(alipatt=score_seq, method='blast')
        self._sequenceID = str(name)
        self._length = int(length)
        self._iteration = int(iteration)
        self._e_value = e_value
        self._align_length = int(align_length)
        self._used = False
    ...
    def query_threshold_coord(self):
        return [
         self._first_segment_value(0), self._last_segment_value(0)]
    ...
    def hit_threshold_coord(self):
        return [
         self._first_segment_value(1), self._last_segment_value(1)]
    ...
    def _check_valid_alignment_seq_type(self, seq_type):
        ...
        METHOD: _check_valid_alignment_seq_type()
            ...
            =>Returns BOOLEAN:
                                ...
                                False otherwise
        """
        if seq_type not in self._get_valid_alignment_seq_types():
    ...
    def __getitem__(self, key):
        try:
            ...
            if not isinstance(key, slice):
                ...
                newali = self.__class__(name=self._sequenceID, length=self._length, iteration=self._iteration, qseq=sequences[0], hseq=sequences[1], hpos=seqInits[1], qpos=seqInits[0], align_length=len(sequences[0]), score_seq=self._alipatt[index[0] - 1:index[1] - 1:index[2]])
