...
# [GCC 8.4.0]
# Embedded file name: /home/patricia/patricia/modppi/./src/SBI/structure/protein/SecondaryStructure.py
...
class SecondaryStructure(object):
    ...
    def get_moment_of_inertia_length(self, workp):
        if workp == 'f11':
            p1, p2 = self._get_coordinate(self._f11), self._get_coordinate(self._endp)
        elif workp == 'f44':
            p1, p2 = self._get_coordinate(self._inip), self._get_coordinate(self._f44)
        return p2 - p1 + 1
    ...
    def _jacobi_angles(self, workp):
        ...
        if workp == 'f11':
            self._eigf11 = np.asarray(eigenVec[:, 2]).reshape(-1)
        elif workp == 'f44':
            self._eigf44 = np.asarray(eigenVec[:, 2]).reshape(-1)
    ...
    def _process_betas(self):
        ...
        while not end:
            ...
            if new_me._f11 == new_me._inip or advance >= advance_limit:
                end = True
            advance += 1
        ...
        while not end:
            ...
            if new_me._f44 == new_me._endp or advance >= advance_limit:
                end = True
            advance += 1
            if difference < degree_dif_threshold:
