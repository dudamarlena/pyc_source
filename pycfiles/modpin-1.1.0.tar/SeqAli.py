...
# [GCC 8.4.0]
# Embedded file name: /home/patricia/patricia/modppi/./src/SBI/sequence/SeqAli.py
...
class SeqAli(object):
    ...
    def __init__(self, sequences, sequenceInits, identities=None, positives=None, gaps=None):
        ...
        if len(sequences) != len(sequenceInits):
            raise AttributeError('One Sequence Init is required for each sequence\n')
        self._seq = []
    ...
    @property
    def identities_pec(self):
        return 100 * float(self._identities) / self._aligned_aa
    ...
    @property
    def positives_pec(self):
        return 100 * float(self._positives) / self._aligned_aa
    ...
    def _alignment_position_from_sequence_position(self, refseq, pos):
        if not self._seq[refseq].is_gapped:
            ...
            for x in range(len(self._seq[refseq])):
                if pregaps and self._seq[refseq][x] == '-':
                    continue
                pregaps = False
    ...
    def _last_segment_value(self, refseq):
        for x in reversed(range(len(self._segment[refseq]))):
            if self._segment[refseq][x] != '-':
                return self._segment[refseq][x]
    ...
    def overlap(self, qrefseq, mappedrefseq, otherAli):
        ...
        me = self.get_section_from_sequence_position(qrefseq, self._first_segment_value(qrefseq), self._last_segment_value(qrefseq))
        he = otherAli.get_section_from_sequence_position(qrefseq, otherAli._first_segment_value(qrefseq), otherAli._last_segment_value(qrefseq))
        if me._first_segment_value(mappedrefseq) > he._last_segment_value(mappedrefseq) or me._last_segment_value(mappedrefseq) < he._first_segment_value(mappedrefseq):
            return 0
        if me._first_segment_value(mappedrefseq) == he._first_segment_value(mappedrefseq) and me._last_segment_value(mappedrefseq) == he._last_segment_value(mappedrefseq):
            return 1
    ...
    def get_coverage_of_sequence(self, refseq, original_seqlength, ini=None, end=None):
        ...
        if ini is None and end is None:
            return 0
        else:
    ...
    def evaluate_Rost_twilight_zone(self, equation='ID', parameter=0):
        if self.is_multiple:
            raise SAE(code=2, value=self._num_seq)
        if equation == 'ID':
    ...
    def _search_segments(self):
        ...
        for x in range(len(self._seq[0])):
            ...
            for i in range(self._num_seq):
                if x == 0:
                    if bool(int(profile_str[i])):
                        self._segment[i].append(self._sequence_position_id(i, counter[i]))
                    else:
                        self._segment[i].append('-')
                else:
                    if profile_str != previous_profile:
                        if bool(int(previous_profile[i])):
                            self._segment[i].append(self._sequence_position_id(i, counter[i] - 1))
                        else:
                            self._segment[i].append('-')
                        if bool(int(profile_str[i])):
                            self._segment[i].append(self._sequence_position_id(i, counter[i]))
