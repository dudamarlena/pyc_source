...
# [GCC 8.4.0]
# Embedded file name: /home/patricia/patricia/modppi/./src/SBI/structure/header/Header.py
...
class PDBHeader(JSONer):
    ...
    def __init__(self):
        ...
        self._deprecated = []
        self._chaindict = {}
        self._workchain = None
        self._molecules = {}
        self._sites = {}
        self._hetero = {}
        self._dbrefs = []
        self._secstr = []
        self._symmetryM = BioMolecule(0)
        self._biomolecM = []
        return
    ...
    def get_chainheader(self, chain):
        ...
        return ChainHeader(self._chaindict[chain], self.experiment, self.pdb, chain)

    def has_hetero(self, heteroID):
        return heteroID in self.hetero.keys()

    def as_dict(self):
        data = {'chains': sorted(self.chains), 'date': self.date, 'classification': self.header, 
           'title': self.title, 'deprecated': self.deprecated, 
           'id': self.pdb, 'keywords': self.keywords, 
           'molecules': [], 'experiment': {}, 'dbrefs': [], 'sites': [], 'hetatm': [], 'sstruct': [], 'symmetry': self.symmetry_matrix.as_dict(), 
...
class ChainHeader(JSONer):
    ...
    def dbreferences(self):
        if 'DBREF' not in self._data:
            return []
        return self._data['DBREF']

    @property
    def hetero(self):
        if 'HETATM' not in self._data:
            return []
        return self._data['HETATM']

    @property
    def secondary_structures(self):
        if 'SSTRUC' not in self._data:
            return []
    ...
    def as_dict(self):
        data = {'id': self.pdb, 'chain': self.chain, 'experiment': {}, 'dbrefs': [], 'sites': [], 'hetatm': [], 'secstruct': [], 'molecule': self.molecule.as_dict()}
