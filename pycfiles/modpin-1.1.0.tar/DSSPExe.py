...
# [GCC 8.4.0]
# Embedded file name: /home/patricia/patricia/modppi/./src/SBI/external/DSSP/DSSPExe.py
...
class DSSPExe(object):
    default_config_file = os.path.join(os.path.normpath(os.path.join(os.path.dirname(__file__), '..')), 'configSBI.txt')
    ...
    def __init__(self, pdbfile, dsspfile, cleanpdb=False, cleandssp=False):
        ...
        if DSSPExe._exe is None:
            DSSPExe._exe = Executable(executable=DSSPExe._config.get('dssp', 'executable'), path=DSSPExe._config.get('dssp', 'path'))
        self._execute()
        self._parse()
    ...
    @property
    def empty_dssp(self):
        return DSSP(secondary_structure='-', accessibility=1, AAtype='X')
        ...
        for line in file_fd:
            ...
            if read:
                if line[13:14] != '!':
                    ...
                    if ss == ' ':
                        ss = '-'
                    self._dsspdata.append(DSSP(secondary_structure=ss, accessibility=buried, AAtype=aa))
                    self._dsspdata[(-1)].add_hydrogen_links(line[39:50], line[50:61], line[61:72], line[72:84])
                    if readline > 0:
