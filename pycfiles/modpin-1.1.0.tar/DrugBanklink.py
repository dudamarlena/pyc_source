...
# [GCC 8.4.0]
# Embedded file name: ./src/SBI/databases/DrugBanklink.py
    ...
    def _process_drugs(self, targets):
        ...
        for line in z.open(z.namelist()[0]):
            if line.startswith('  <drug'):
                ...
                drug = drugs[(-1)]
                drug.type = line
            elif line.startswith('    <drugbank-id'):
                drug.dbid = line
                ...
                drug.cas = line
            elif line.startswith('      <secondary-accession-number'):
                drug.secondary_accession_numbers = line
            elif line.startswith('      <group'):
                drug.groups = line
            elif line.startswith('        <kind'):
                propkind = line
            elif line.startswith('        <value'):
                drug.dproperty = (propkind, line)
            elif line.startswith('      <category'):
                drug.categories = line
            elif line.startswith('        <resource>'):
                propkind = line
            elif line.startswith('        <identifier>'):
                drug.external = (propkind, line)
...
class Drug(object):
    properties_of_interest = [
     'Hydrophobicity', 'Isoelectric Point', 'Molecular Weight', 'Molecular Formula']
    ...
    @type.setter
    def type(self, value):
        self._type = re.search('type\\="(\\w+\\s*\\w+)"', value).group(1)
    ...
    @dbid.setter
    def dbid(self, value):
        self._dbid = re.search('>(DB\\d+)</', value).group(1)
    ...
    @name.setter
    def name(self, value):
        self._name = re.search('>([\\S\\s]+)</', value).group(1)
    ...
    @property
    def secondary_accession_numbers(self):
        return self._san

    @secondary_accession_numbers.setter
    def secondary_accession_numbers(self, value):
        self._san.append(re.search('>([\\S\\s]+)</', value).group(1))
    ...
    def groups(self, value):
        self._grps.append(re.search('>([\\S\\s]+)</', value).group(1))
    ...
    def categories(self, value):
        self._cate.append(re.search('>([\\S\\s]+)</', value).group(1))
    ...
    def synonym(self, value):
        self._synm.append(re.search('>([\\S\\s]+)</', value).group(1))
