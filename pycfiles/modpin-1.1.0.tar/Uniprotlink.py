...
# [GCC 8.4.0]
# Embedded file name: ./src/SBI/databases/Uniprotlink.py
...
class Uniprotlink(object):
    ...
    def __init__(self, local=None):
        ...
        self.__name__ = 'databases.Uniprotlink'
    ...
    def _process(self):
        self._parse_uniprot_file(source=os.path.join(self.local, 'uniprot_sprot.dat.gz'), destination=self._swsfile, fasta=self._swsfast, code='swissprot')
        self._parse_uniprot_file(source=os.path.join(self.local, 'uniprot_trembl.dat.gz'), destination=self._trbfile, fasta=self._trbfast, code='trembl')
