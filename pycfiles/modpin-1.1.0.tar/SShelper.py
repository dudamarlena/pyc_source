...
# [GCC 8.4.0]
# Embedded file name: /home/patricia/patricia/modppi/./src/SBI/structure/protein/SShelper.py
...
def locate_valid_secondarystructures(pdb):
    ...
    for residue in pdb.aminoacids:
        ...
        if reading_type not in SecondaryStructure.structure_regions:
            ...
            counter, start_post, reading_type, added = (
             0, aa, ss, False)
            if ss in SecondaryStructure.structure_regions:
                start_post, reading_type, counter = aa, ss, 1
        previous_aa = aa
...
def calculate_archs(pdb, limit_internal_ss=100, limit_distance=False, allowed_gaps=0):
    number = 1
    for i in range(len(pdb.secondary_structures)):
        for j in range(i + 1, len(pdb.secondary_structures)):
            ...
            secondstructurepair = Arch(pdb.globalID, pdb.secondary_structures[i], pdb.secondary_structures[j], j - i - 1, f3 - f2 - 1, newss, pdb._torsionsCA[f1 + gapcorrectini:f4 + 1 + gapcorrectend], pdb._psiphi[(f1 + gapcorrectini) * 3:(f4 + gapcorrectend) * 3 + 6], number)
            if secondstructurepair.is_superarch and limit_distance and secondstructurepair.cartesian_distance > limit_distance:
                break
...
def calculate_torsionsCA(pdb, accuracy=0):
    for i in range(0, len(pdb.aminoacids) - 3):
        ...
        ca2 = aa2.ca
        ca3 = aa3.ca
        if not isinstance(ca0, AtomOfAminoAcid) or not isinstance(ca1, AtomOfAminoAcid) or not isinstance(ca2, AtomOfAminoAcid) or not isinstance(ca3, AtomOfAminoAcid):
            pdb._torsionsCA += '-'
            continue
        ...
        if a5 < 0:
            a5 += 180
        u = (
         v01[1] * v12[2] - v01[2] * v12[1], v01[2] * v12[0] - v01[0] * v12[2], v01[0] * v12[1] - v01[1] * v12[0])
        ...
        if accuracy in accuracy_desc:
            ...
            if a6 >= 90 and a6 < 160:
                pdb._torsionsCA += 'B'
            if a6 >= 160 and a6 < 260:
                pdb._torsionsCA += 'C'
            if a6 >= 260 and a6 <= 360:
                pdb._torsionsCA += 'D'
...
def calculate_psiphi(pdb):
    ...
    for i in range(3 * (len(pdb.aminoacids) - 1)):
        ...
        if not isinstance(at0, AtomOfAminoAcid) or not isinstance(at1, AtomOfAminoAcid) or not isinstance(at2, AtomOfAminoAcid) or not isinstance(at3, AtomOfAminoAcid) or step == 2 and uncalculable > 0:
            if step < 2:
                uncalculable += 1
            elif uncalculable > 0:
                pdb._psiphi += '---'
        ...
        if step == 2:
            ...
            if omega[(-2)] >= -60 and omega[(-2)] <= 60 and len(omega) > 2:
                pdb._psiphi += 'CIS'
            elif phi[(-2)] >= -180 and phi[(-2)] < -20:
                if phi[(-2)] >= -180 and phi[(-2)] < -140:
                    if psi >= -180 and psi < -140:
                        pdb._psiphi += 'E11'
                    elif psi >= -140 and psi < -100:
                        pdb._psiphi += 'E12'
                    elif psi >= -100 and psi < -60:
                        pdb._psiphi += 'I13'
                    elif psi >= -60 and psi < -20:
                        pdb._psiphi += 'N14'
                    elif psi >= -20 and psi < 20:
                        pdb._psiphi += 'N15'
                    elif psi >= 20 and psi < 60:
                        pdb._psiphi += 'N16'
                    elif psi >= 60 and psi < 100:
                        pdb._psiphi += 'E17'
                    elif psi >= 100 and psi < 140:
                        pdb._psiphi += 'E18'
                    elif psi >= 140 and psi <= 180:
                        pdb._psiphi += 'E19'
