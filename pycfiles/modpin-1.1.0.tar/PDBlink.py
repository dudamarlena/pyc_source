...
# [GCC 8.4.0]
# Embedded file name: ./src/SBI/databases/PDBlink.py
...
@oliva's lab
"""
import sys, os, copy, subprocess, warnings, urllib, ftplib
...
class PDBlink(object):
    ...
    @PDBseq.setter
    def PDBseq(self, value):
        self._PDBseq = os.path.abspath(value)
    ...
    def sync_PDB(self, log_file=None):
        ...
        command = [
         'rsync', '-rlpt', '-v', '-z', '--port=' + PDBrsync['port'], PDBrsync['address'], self.local]
        p = subprocess.Popen(command, stdout=open(log_file, 'w') if log_file is not None else subprocess.PIPE, stderr=subprocess.PIPE)
