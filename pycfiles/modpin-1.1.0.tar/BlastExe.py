...
# [GCC 8.4.0]
# Embedded file name: /home/patricia/patricia/modppi/./src/SBI/external/blast/BlastExe.py
...
class BlastExe(object):
    ...
    def __init__(self, database, search_type='prot'):
        ...
        self._exe = Executable(executable=self._configurator.get('blast', 'executable'), path=self._configurator.get('blast', 'path'), variable_path=self._configurator.get('blast', 'variable_path'))
    ...
    @property
    def selfHit(self):
        return self._selfHit

    @selfHit.setter
    def selfHit(self, vaule):
        self._selfHit = value

    @property
    def hitIDformat(self):
        return self._hitIDformat
    ...
    @property
    def overwritte(self):
        return self._overwritte

    @overwritte.setter
    def overwritte(self, value):
        self._overwritte = value
    ...
    @clean_files.setter
    def clean_files(self, value):
        self._clean_files = value
    ...
    def execute_query(self, query_file=None, blast_output_file=None):
        ...
        newFasta.load()
        if len(re.sub('[Xx]', '', newFasta.sequences[0].sequence)) == 0:
    ...
    def _check_database(self, database):
        ...
        formatdb_files = []
        if self._search_type == 'nucl':
            formatdb_sufix = ['.nhr', '.nin', '.nsq']
        else:
            if self._search_type == 'prot':
                formatdb_sufix = ['.phr', '.pin', '.psq']
        ...
        if len(formatdb_files) > 0:
            try:
                dbexe = Executable(executable=self._configurator.get('blast', 'dbformatexe'), path=self._configurator.get('blast', 'path'), variable_path=self._configurator.get('blast', 'variable_path'))
