...
# [GCC 8.4.0]
# Embedded file name: /home/patricia/patricia/modppi/./src/SBI/structure/contacts/interface/PHInterface.py
...
class PHInterface(Interface):
    ...
    def __init__(self, protein=None, heteroatom=None, threshold_type='min', threshold_distance=8, protein_centers=None, heteroatom_centers=None):
        ...
        if heteroatom_centers is None:
            heteroatom_centers = heteroatom.geometric_center(structure=False, hetero=True, water=False, by_residue=True)
        super(PHInterface, self).__init__(chain1=protein, chain2=heteroatom, threshold_type=threshold_type, threshold_distance=threshold_distance, chain1_centres=protein_centers, chain2_centers=heteroatom_centers)
        return
    ...
    @property
    def heteroatoms_id(self):
        return self._chain2.globalID

    @property
    def heteroatoms_centers(self):
        return self._chain1_centers
    ...
    def _build(self):
        if len(self.protein.aminoacids) == 0 or len(self.heteroatoms.heteroatoms) == 0:
            return
        super(PHInterface, self)._build()
        for i in range(len(self._filtered[0])):
            new_contact = ContactAH(aminoacid=self.protein.aminoacids[self._filtered[0][i]], heteroatom=self.heteroatoms.heteroatoms[self._filtered[1][i]], threshold_type=self.threshold_type, threshold_distance=self.threshold_distance)
