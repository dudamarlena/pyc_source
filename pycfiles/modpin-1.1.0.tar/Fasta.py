...
# [GCC 8.4.0]
# Embedded file name: /home/patricia/patricia/modppi/./src/SBI/sequence/Fasta.py
    ...
    def retrieve(self, seqID, allbut=False, prefix_size=None):
        ...
        if isinstance(seqID, basestring):
            if len(self) == 0:
                ...
                for line in self.file.descriptor:
                    if line.startswith('>'):
                        if prefix_size is not None:
                            sid = line.lstrip('>').split()[0].strip()[:prefix_size]
                        else:
                ...
                if seqID not in self._seqfinder:
        ...
        if isinstance(seqID, set):
            ...
            if len(self) == 0:
                ...
                for line in self.file.descriptor:
                    if line.startswith('>'):
                        if prefix_size is not None:
                            ...
                            sid = line.lstrip('>').split()[0].strip()
                        if not allbut and sid in seqID or allbut and sid not in seqID:
                ...
                for queryID in seqID:
                    if queryID not in self._seqfinder:
