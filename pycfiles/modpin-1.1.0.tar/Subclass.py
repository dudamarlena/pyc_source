...
# [GCC 8.4.0]
# Embedded file name: /home/patricia/patricia/modppi/src/archdb/src/Subclass.py
...
class Subclass(object):
    ...
    def add_topology(self, info, method):
        dist_code = {'A': '0-2', 'B': '2-4', 'C': '4-6', 'D': '6-8', 'E': '8-10', 'F': '10-12', 'G': '12-14', 'H': '14-16', 'I': '16-18', 'J': '18-20', 
           'K': '20-22', 'L': '22-24', 'M': '24-26', 'N': '26-28', 'O': '28-30', 'P': '30-32', 'Q': '32-34', 'R': '34-36', 'S': '36-38', 'T': '38-40', 
           '?': '40-42'}
        ...
        if method == 'DS':
            ...
            delta_code = {'A': '0-30', 'B': '30-60', 'C': '60-90', 'D': '90-120', 'E': '120-180', 'F': '180-210', 'G': '210-240', 'H': '240-270', 'I': '270-300', 'J': '300-330', 'K': '330-360'}
            theta_code = {'A': '0-30', 'B': '30-60', 'C': '60-90', 'D': '90-120', 'E': '120-180', 'F': '180-210', 'G': '210-240', 'H': '240-270', 'I': '270-300', 
               'J': '300-330', 'K': '330-360'}
    ...
    def _motif(self, full):
        data = [ re.sub(',.\\Z', '', re.sub('[\\(|\\)]', '', x.split('=')[(-1)])) for x in re.split('\\)\\(', full) ]
    ...
    def add_consensus(self, info, method, loops):
        ...
        if method == 'MCL':
            ...
            for loop in self.loops:
                ...
                if the_first:
                    z0 = [
                     loop.prel + z1 - 4, loop.prel + z1 + 4 + int(self.length)]
                    the_first = False
...
class Consensus(object):
    ...
    def toSQL(self, classcode):
        length = self.length
        if self.method == 'MCL' and length == 4:
            length = '4S'
        else:
            if self.method == 'MCL' and length == 4.5:
                length = '4M'
            else:
                length = int(length)
...
class Cclass(object):
    ...
    @property
    def lastsubclass(self):
        return self._lastsubclass
    ...
    def toSQL(self, method):
        ...
        for l in self.subclasses:
            ...
            for s in self.subclasses[l]:
                if method == 'MCL' and s.identifier in donesubclass:
                    continue
                if s.size >= 3:
                    info[l].setdefault(s.consensus, []).append(s)
                    if method == 'MCL':
                        donesubclass.add(s.identifier)
...
class Loop(object):
    ...
    def loop_ID_pos(self):
        return tuple([self.loopID, self.position])
