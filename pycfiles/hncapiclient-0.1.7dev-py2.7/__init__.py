...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\hncapiclient\__init__.py
# Compiled at: 2013-05-23 14:39:17
...
>>> from hncapiclient import TextField, IntegerField, DateField
...
__all__ = [
 'Mapping', 'Field', 'TextField', 'FloatField',
 'IntegerField', 'LongField', 'BooleanField', 'DecimalField', 'PictureField', 'BaseUnitFieldDateField',
    ...
    class Proxy(list):
        ...
        def append(self, *args, **kwargs):
            if args or not isinstance(self.field, DictField):
                if len(args) != 1:
                    raise TypeError('append() takes exactly one argument (%s given)' % len(args))
                value = args[0]
            else:
                value = kwargs
        ...
        def insert(self, idx, *args, **kwargs):
            if args or not isinstance(self.field, DictField):
                if len(args) != 1:
                    raise TypeError('insert() takes exactly 2 arguments (%s given)' % len(args))
                value = args[0]
            else:
                value = kwargs
