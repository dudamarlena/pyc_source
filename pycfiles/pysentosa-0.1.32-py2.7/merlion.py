...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pysentosa/merlion.py
...
class Merlion(object):
    ...
    def __init__(self, account=None):
        ...
        if self.account == 'DU198456':
            print "ERROR: You haven't set your IB account yet.\nYou just need to open ~/.sentosa/sentosa.yml, find account under global section, and replace the demo account with your own IB account."
            import sys
            sys.exit(-1)
        ...
        while self.s1:
            ...
            if msg is not None and msg.index('|') > 0:
                ...
                if len(v) == 3:
                    ...
                    if v[1] == str(BID_PRICE) and float(v[2]) > 0:
    ...
    def track_symbol(self, target):
        if isinstance(target, list):
            self.interest_list.extend(target)
