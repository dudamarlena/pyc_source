...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/credstuffer/user_account.py
        ...
        if self.notify is not None:
            if 'mail' in self.notify:
                if ('smtp' and 'port' and 'sender' and 'receiver' and 'password') in kwargs.keys():
                    if all([val is not None for val in kwargs.values()]):
    ...
    def send_notification(self, username, password):
        ...
        try:
            with open(self.root_dir + '/' + self.credential_file + '.txt', 'a') as (file):
                self.logger.info('writing credential (username: {}, password: {}) to file {}'.format(username, password, self.credential_file))
