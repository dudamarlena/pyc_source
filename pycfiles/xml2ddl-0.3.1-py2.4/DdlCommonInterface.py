...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/xml2ddl/DdlCommonInterface.py
# Compiled at: 2005-09-27 19:05:07
import re, os
...
class DdlCommonInterface:
    ...
    def __init__(self, strDbms):
        ...
        self.params = {'add_table': ['CREATE TABLE %(table_name)s (\n\t%(col_defs)s%(primary_keys)s)%(extra)s'], 'drop_table': ['DROP TABLE %(table_name)s%(cascade)s'], 'rename_table': ['ALTER TABLE %(table_name)s RENAME TO %(new_table_name)s'], 'table_desc': ['COMMENT ON TABLE %(table)s IS %(desc)s'], 'add_column': ['ALTER TABLE %(table_name)s ADD %(column_name)s %(column_type)s'], 'drop_column': ['ALTER TABLE %(table_name)s DROP %(column_name)s'], 'rename_column': ['ALTER TABLE %(table_name)s RENAME %(old_col_name)s TO %(new_col_name)s'], 'change_col_type': ['ALTER TABLE %(table_name)s ALTER %(column_name)s TYPE %(column_type)s'], 'column_desc': ['COMMENT ON COLUMN %(table)s.%(column)s IS %(desc)s'], 'drop_default': ['ALTER TABLE %(table_name)s ALTER %(column_name)s DROP DEFAULT'], 'alter_default': ['ALTER TABLE %(table_name)s ALTER %(column_name)s SET DEFAULT %(new_default)s'], 'add_relation': ['ALTER TABLE %(tablename)s ADD CONSTRAINT %(constraint)s FOREIGN KEY (%(thiscolumn)s) REFERENCES %(othertable)s(%(fk)s)%(ondelete)s%(onupdate)s'], 'drop_relation': ['ALTER TABLE %(tablename)s DROP CONSTRAINT %(constraintname)s'], 'create_view': ['CREATE VIEW %(viewname)s AS %(contents)s'], 'drop_view': ['DROP VIEW %(viewname)s'], 'create_function': ["CREATE FUNCTION %(functionname)s(%(arguments)s) RETURNS %(returns)s AS '\n%(contents)s'%(language)s"], 'drop_function': ['DROP FUNCTION %(functionname)s(%(params)s)'], 'add_key_constraint': ['ALTER TABLE %(table_name)s ADD CONSTRAINT %(pk_constraint)s PRIMARY KEY (%(keys)s)'], 'drop_key_constraint': ['ALTER TABLE %(table_name)s DROP CONSTRAINT %(constraint_name)s'], 'add_index': ['CREATE INDEX %(index_name)s ON %(table_name)s (%(col_names)s)'], 'drop_index': ['DROP INDEX %(index_name)s'], 'unquoted_id': re.compile('^[A-Za-z][A-Za-z0-9_]+$'), 'max_id_len': {'default': 256}, 'has_auto_increment': False, 'keywords': ['NULL', 'SELECT', 'FROM'], 'quote_l': '"', 'quote_r': '"', 'drop_constraints_on_col_rename': False, 'drop_table_has_cascade': False, 'can_change_table_comment': True}

    def addTable(self, strTableName, colDefs, keys, strTableStuff, diffs):
        info = {'table_name': self.quoteName(strTableName), 'col_defs': (',\n\t').join(colDefs), 'primary_keys': '\n', 'extra': strTableStuff}
        if len(keys) > 0:
    ...
    def dropTable(self, strTableName, strCascade, diffs):
        info = {'table_name': self.quoteName(strTableName), 'cascade': strCascade}
    ...
    def renameTable(self, strTableOld, strTableNew, diffs):
        info = {'table_name': self.quoteName(strTableOld), 'new_table_name': self.quoteName(strTableNew)}
        for strDdl in self.params['rename_table']:
            diffs.append(('Rename Table', strDdl % info))

    def addTableComment(self, tableName, desc, ddls):
        info = {'table': tableName, 'desc': self.quoteString(desc)}
    ...
    def addColumn(self, strTableName, strColName, strColType, nAfter, diffs):
        """ nAfter not used yet """
        info = {'table_name': self.quoteName(strTableName), 'column_name': self.quoteName(strColName), 'column_type': strColType}
        for strDdl in self.params['add_column']:
            diffs.append(('Add Column', strDdl % info))

    def dropColumn(self, strTableName, strColName, diffs):
        info = {'table_name': self.quoteName(strTableName), 'column_name': self.quoteName(strColName)}
    ...
    def renameColumn(self, strTableName, strOldName, strNewName, strNewColType, diffs):
        info = {'table_name': self.quoteName(strTableName), 'old_col_name': self.quoteName(strOldName), 'new_col_name': self.quoteName(strNewName), 'column_type': strNewColType}
    ...
    def addColumnComment(self, strTableName, strColumnName, strDesc, strColType, ddls):
        info = {'table': strTableName, 'column': strColumnName, 'desc': self.quoteString(strDesc), 'type': strColType + ' '}
    ...
    def addKeyConstraint(self, strTableName, keylist, diffs):
        info = {'table_name': self.quoteName(strTableName), 'pk_constraint': self.quoteName(getPkContraintName(strTableName)), 'keys': (', ').join(keylist)}
    ...
    def dropKeyConstraint(self, strTable, strConstraintName, diffs):
        info = {'table_name': strTable, 'constraint_name': strConstraintName}
    ...
    def addIndex(self, strTableName, strIndexName, cols, ddls):
        ...
        info = {'index_name': self.quoteName(strIndexName), 'table_name': self.quoteName(strTableName), 'col_names': (', ').join(cols)}
        for strDdl in self.params['add_index']:
            ddls.append(('Add Index', strDdl % info))

    def deleteIndex(self, strTableName, strIndexName, diffs):
        info = {'index_name': self.quoteName(strIndexName), 'table_name': strTableName}
    ...
    def addRelation(self, strTableName, strRelationName, strColumn, strFkTable, strFk, strOnDelete, strOnUpdate, diffs):
        info = {'tablename': self.quoteName(strTableName), 'thiscolumn': self.quoteName(strColumn), 'othertable': self.quoteName(strFkTable), 'constraint': self.quoteName(strRelationName), 'ondelete': '', 'onupdate': ''}
        if len(strFk) > 0:
            info['fk'] = strFk
        else:
    ...
    def dropRelation(self, strTableName, strRelationName, diffs):
        info = {'tablename': self.quoteName(strTableName), 'constraintname': strRelationName}
    ...
    def addAutoIncrement(self, strTableName, strColName, strDefault, strPreDdl, strPostDdl):
        info = {'table_name': strTableName, 'col_name': strColName, 'seq_name': getSeqName(strTableName, strColName), 'ai_trigger': getAiTriggerName(strTableName, strColName)}
        if self.params['has_auto_increment']:
            return ' AUTO_INCREMENT'
        if self.dbmsType == 'firebird':
            strPreDdl.append(('Add Autoincrement Generator', 'CREATE GENERATOR %(seq_name)s' % info))
            strPostDdl.append(('Add Autoincrement Trigger', 'CREATE TRIGGER %(ai_trigger)s FOR %(table_name)s\n                BEFORE INSERT AS\n                BEGIN\n                    NEW.%(col_name)s = GEN_ID(%(seq_name)s, 1);\n                END' % info))
            return ''
        strPreDdl.append(('Add Autoincrement', 'CREATE SEQUENCE %(seq_name)s' % info))
        if strDefault:
    ...
    def dropAutoIncrement(self, strTableName, col, diffs):
        ...
        if self.params['has_auto_increment']:
            ...
            return
        if self.dbmsType == 'firebird':
            diffs.append(('Drop Autoincrement Trigger', 'DROP TRIGGER %(ai_trigger)s' % info))
            diffs.append(('Drop Autoincrement', 'DROP GENERATOR %(seq_name)s' % info))
            return
        diffs.append(('Drop Autoincrement', 'DROP SEQUENCE %(seq_name)s' % info))
        self.dropDefault(strTableName, col, diffs)

    def dropDefault(self, strTableName, col, diffs):
        info = {'table_name': self.quoteName(strTableName), 'column_name': self.quoteName(col.get('name')), 'column_type': self.retColTypeEtc(col)}
    ...
    def changeColDefault(self, strTableName, strColumnName, strNewDefault, strColType, diffs):
        info = {'table_name': self.quoteName(strTableName), 'column_name': self.quoteName(strColumnName), 'new_default': strNewDefault, 'column_type': strColType}
    ...
    def doChangeColType(self, strTableName, strColumnName, strNewColType, diffs):
        info = {'table_name': strTableName, 'column_name': strColumnName, 'column_type': strNewColType}
    ...
    def dropView(self, strOldViewName, diffs):
        info = {'viewname': self.quoteName(strOldViewName)}
    ...
    def addView(self, strNewViewName, strContents, attribs, diffs):
        info = {'viewname': self.quoteName(strNewViewName), 'contents': strContents}
    ...
    def dropFunction(self, strOldFunctionName, argumentList, diffs):
        if argumentList:
            ...
            paramList = ''
        info = {'functionname': self.quoteName(strOldFunctionName), 'params': (', ').join(paramList)}
    ...
    def addFunction(self, strNewFunctionName, argumentList, strReturn, strContents, attribs, diffs):
        info = {'functionname': self.quoteName(strNewFunctionName), 'arguments': (', ').join(argumentList), 'returns': strReturn, 'contents': strContents.replace("'", "''")}
...
g_dbTypes = [
 'postgres', 'postgres7', 'mysql', 'mysql4', 'oracle', 'firebird']