...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/xml2ddl/xml2ddl.py
# Compiled at: 2005-09-27 19:07:06
import re, os
...
class Xml2Ddl:
    ...
    def _setDefaults(self):
        ...
        self.params = {'drop-tables': True, 'output_primary_keys': True, 'output_references': True, 'output_indexes': True, 'add_dataset': True}
    ...
    def addRelations(self, doc):
        ...
        for relation in relations:
            self.ddlInterface.addRelation(strTableName, getRelationName(relation), relation.getAttribute('column'), relation.getAttribute('table'), relation.getAttribute('fk'), relation.getAttribute('ondelete'), relation.getAttribute('onupdate'), self.ddls)
    ...
    def addDataset(self, doc):
        ...
        for dataset in datasets:
            ...
            self.ddls.append(('dataset', 'INSERT INTO %s (%s) VALUES (%s)' % (self.ddlInterface.quoteName(strTableName), (', ').join(cols), (', ').join(vals))))
...
def parseCommandLine():
    ...
    parser.add_option('-d', '--drop', action='store_true', dest='bDrop', default=True, help='Drop Tables')
