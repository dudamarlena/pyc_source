...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/xml2ddl/FirebirdInterface.py
...
class FbDownloader(DownloadCommon):
    ...
    def convertTypeId(self, nType):
        types = {261: 'blob', 14: 'char', 40: 'cstring', 11: 'd_float', 27: 'double', 10: 'float', 16: 'int64', 8: 'integer', 9: 'quad', 7: 'smallint', 12: 'date', 13: 'time', 35: 'timestamp', 37: 'varchar'}
        strType = ''
        if nType in types:
            strType = types[nType]
    ...
    def getFunctionDefinition(self, strSpecifiName):
        ...
        for row in rows:
            ...
            elif strParamName.lower() == 'ret':
                rets.append(strType)
            else:
                rets.append(strParamName + ' ' + strType)

        return (
         strProcName.lower(), args, (',').join(rets), '', strDefinition)
...
class DdlFirebird(DdlCommonInterface):
    ...
    def addFunction(self, strNewFunctionName, argumentList, strReturn, strContents, attribs, diffs):
        ...
        diffs.append(('Add function', 'CREATE PROCEDURE %(functionname)s(%(arguments)s) RETURNS (ret %(returns)s) AS \n%(contents)s;' % info))

    def dropFunction(self, strOldFunctionName, argumentList, diffs):
        info = {'functionname': self.quoteName(strOldFunctionName)}
        diffs.append(('Drop function', 'DROP PROCEDURE %(functionname)s' % info))