...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/xml2ddl/PostgreSQLInterface.py
# Compiled at: 2005-09-27 21:43:12
...
class PgDownloader(DownloadCommon):
    ...
    def _fixTypeNames(self, type):
        fixNames = {'int4': 'integer', 'int': 'integer', 'bool': 'boolean', 'float8': 'double precision', 'int8': 'bigint', 'serial8': 'bigserial', 'serial4': 'serial', 'float4': 'real', 'int2': 'smallint', 'character varying': 'varchar'}
        if type in fixNames:
    ...
    def decodeLength(self, type, atttypmod):
        ...
        if type == 'varchar':
            return (
             atttypmod - VARHDRSZ, None)
        if type == 'numeric':
            atttypmod -= VARHDRSZ
            return (atttypmod >> 16 & 65535, atttypmod & 65535)
    ...
    def getFunctionDefinition(self, strSpecifiName):
        ...
        strDefinition = re.compile('\\s*DECLARE\\s+.*BEGIN', re.DOTALL | re.MULTILINE).sub('BEGIN', strDefinition)
...
class DdlPostgres(DdlCommonInterface):
    ...
    def __init__(self, strDbms):
        ...
        if self.dbmsType == 'postgres7':
            self.params['change_col_type'] = [
             'ALTER TABLE %(table_name)s ADD tmp_%(column_name)s %(column_type)s', 'UPDATE %(table_name)s SET tmp_%(column_name)s = %(column_name)s', 'ALTER TABLE %(table_name)s DROP %(column_name)s', 'ALTER TABLE %(table_name)s RENAME tmp_%(column_name)s TO %(column_name)s']
    ...
    def addFunction(self, strNewFunctionName, argumentList, strReturn, strContents, attribs, diffs):
        ...
        diffs.append(('Add view', "CREATE FUNCTION %(functionname)s(%(arguments)s) RETURNS %(returns)s AS '\n%(contents)s'%(language)s" % info))