...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/xml2ddl/diffxml2ddl.py
# Compiled at: 2005-09-27 19:11:11
import re, os, sys, xml2ddl, copy, optparse
...
class DiffXml2Ddl:
    ...
    def _defaults(self):
        ...
        self.params = {'drop_constraints_on_col_rename': False, 'drop_table_has_cascade': False, 'can_change_table_comment': True}
    ...
    def normalizedColType(self, strColTypeEtc):
        ...
        if strColTypeEtc == 'number' or strColTypeEtc == 'decimal':
            ...
            strColTypeEtc = strColTypeEtc.replace('number', 'decimal')
        strColTypeEtc = strColTypeEtc.replace('number ', 'int ')
        strColTypeEtc = strColTypeEtc.replace('decimal ', 'int ')
        strColTypeEtc = strColTypeEtc.replace('varchar2', 'varchar')
        return strColTypeEtc
    ...
    def renameIndex(self, strTableName, old, new):
        ...
        self.ddli.renameIndex(strTableName, getIndexName(strTableName, old), getIndexName(strTableName, new), strColumns.split(','), self.diffs)
    ...
    def changeIndex(self, strTableName, old, new):
        ...
        if strColumnsOld != strColumnsNew:
            self.ddli.changeIndex(strTableName, getIndexName(strTableName, old), getIndexName(strTableName, new), strColumnsNew.split(','), self.diffs)
    ...
    def diffFunction(self, ignore, oldFunction, newFunction):
        ...
        if strOldContents != strNewContents:
            ...
            self.ddli.updateFunction(newFunction.getAttribute('name'), newFunction.getAttribute('arguments').split(','), newFunction.getAttribute('returns'), strDefinition, attribs, self.diffs)
    ...
    def diffFiles(self, strOldFilename, strNewFilename):
        self.old_xml = xml2ddl.readMergeDict(strOldFilename)
    ...
    def ddlSorter(self, a, b):
        orderDict = {'Create Table': 0, 'Drop Table': 0, 'Rename Table': 0, 'Add Table Comment': 0, 'Add Column': 0, 'Drop Column': 0, 'Rename column': 0, 'Add view': 0, 'Drop view': -99, 'Add Column comment': 0, 'Add key constraint': 0, 'Drop key constraint': 0, 'Add Index': 0, 'Drop Index': -99, 'Add Relation': 0, 'Drop Relation': -99, 'Add Autoincrement Generator': -1, 'Add Autoincrement Trigger': 0, 'Add Autoincrement': 0, 'Drop Autoincrement Trigger': 0, 'Drop Autoincrement': 99, 'Change Col Type': -9, 'Change Default': -5, 'Drop Default': -5, 'Add for change type': -4, 'Copy the data over for change type': -3, 'Drop the old column for change type': -2, 'Rename column for change type': -1, 'Add function': 0, 'Drop function': 0}
        if a and b:
...
def parseCommandLine():
    ...
    parser.add_option('-b', '--dbms', dest='strDbms', metavar='DBMS', default='postgres', help='Output for which Database System')
