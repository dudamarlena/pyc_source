...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/xml2ddl/MySqlInterface.py
...
class MySqlDownloader(DownloadCommon):
    ...
    def getTableColumns(self, strTable):
        ...
        """
        re_size_prec = re.compile('(\\w+)\\((\\d+),\\s*(\\d+)\\)')
        re_size = re.compile('(\\w+)\\((\\d+)\\)')
    ...
    def getTableIndexes(self, strTableName):
        ...
        for index in indexes:
            ...
            if Key_name in keyMap:
                ...
                keyMap[Key_name] = (
                 Key_name, [Column_name], bIsUnique, bIsPrimary, Packed)
    ...
    def getTableRelations(self, strTableName):
        """ Returns 
            ...
            or []
        """
        re_ref = re.compile('\\s*CONSTRAINT\\s+`(\\w+)`\\s+FOREIGN KEY\\s+\\(([^)]+)\\)\\s+REFERENCES\\s+`(\\w+)`\\s+\\(([^)]+)\\)( ON DELETE (?:[A-Z]+))?( ON UPDATE (?:[A-Z]+))?')
        delType = ''
        updateType = ''
    ...
    def getFunctionDefinition(self, strSpecifiName):
        ...
        strCreate = rows[0][2]
        re_createFunc = re.compile('\\s* CREATE \\s+ FUNCTION \\s+ ([\\w`\\.-]*) \\s* \\( ([^)]*) \\) \\s+ RETURNS \\s+ (\\w+) \\s+ (.*)', re.DOTALL | re.MULTILINE | re.VERBOSE)
...
class DdlMySql(DdlCommonInterface):
    ...
    def addFunction(self, strNewFunctionName, argumentList, strReturn, strContents, attribs, diffs):
        ...
        info = {'functionname': self.quoteName(strNewFunctionName), 'arguments': (', ').join(argumentList), 'returns': strReturn, 'contents': strContents.replace("'", "''"), 'language': ''}
        if 'language' in attribs:
            info['language'] = ' LANGUAGE %s' % attribs['language']
        diffs.append(('Add function', 'CREATE FUNCTION %(functionname)s(%(arguments)s) RETURNS %(returns)s %(language)s\n%(contents)s' % info))

    def dropFunction(self, strOldFunctionName, argumentList, diffs):
        info = {'functionname': self.quoteName(strOldFunctionName)}
        diffs.append(('Drop function', 'DROP FUNCTION %(functionname)s' % info))