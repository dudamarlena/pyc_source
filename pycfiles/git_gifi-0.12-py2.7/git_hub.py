...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gifi/git_hub.py
# Compiled at: 2017-06-29 06:49:46
...
def _create_pull_request(repo):
    ...
    pull_request_parameters = {'title': title, 'body': body, 
       'head': head, 
...
def _configuration(repo=None):
    repo = get_repo(repo)
    return Configuration(repo, 'github', {'login': (
               NOT_SET, 'Github login'), 
       'access-token': (
                      NOT_SET, 'Github access token')})
