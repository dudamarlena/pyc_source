...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gifi/epic.py
# Compiled at: 2017-06-27 08:03:57
import feature
...
def current():
    ...
    if feature.is_on_feature_branch(repo):
        ...
        return (
         f.target_remote, f.target_branch)
    else:
...
def configuration(repo=None):
    repo = get_repo(repo)
    return Configuration(repo, 'epic', {'all': ('origin/master', 'A list of coma separated epic remote/branch`es')})
