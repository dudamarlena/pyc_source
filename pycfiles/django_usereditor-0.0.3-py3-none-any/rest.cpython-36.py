...
# [GCC 8.4.0]
# Embedded file name: C:\Python36\lib\site-packages\usereditor\rest.py
# Compiled at: 2020-01-27 08:17:03
# Size of source mod 2**32: 6936 bytes
...
class UserSerializer(serializers.ModelSerializer):
    ...
    full_name = fields.SerializerMethodField(label=(_('Full name')), read_only=True, display_form=(fields.DisplayMode.SUPPRESS))
        ...
        if request:
            if request.user:
                if not request.user.is_staff:
                    if action.name in ('add', 'edit', 'delete'):
                        return True
    ...
    def update_user_settings(instance, validated_data, *args, **kwargs):
        ...
        if email:
            if not EmailAddress.objects.filter(user=instance, email=email).exists():
    ...
    class Meta:
        ...
        fields = ('id', 'full_name', 'username', 'password', 'first_name', 'last_name',
                  'is_staff', 'is_superuser', 'is_active', 'email', 'email_verified')
        changed_flds = {'id': dict(display=(DisplayMode.HIDDEN))}
        for f in ('first_name', 'last_name', 'is_superuser', 'is_active'):
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    queryset = get_user_model().objects.exclude(username__in=('admin', )).annotate(un=(Concat(Coalesce('first_name', Value('')), Value(' '), Coalesce('last_name', 'username')))).prefetch_related('emailaddress_set').all()
...
router.register('rest/users', UserViewSet, 'usersitem')