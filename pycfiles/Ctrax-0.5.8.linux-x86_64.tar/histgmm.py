...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/Ctrax/histgmm.py
# Compiled at: 2013-09-24 00:46:30
from matplotlib.mlab import normpdf
import numpy as num
from scipy.linalg.basic import eps, kron
import sys
...
def histgmm(x, w, maxiters=100, minchange=None, minsigma=None):
    ...
    mu, sigma, prior, score1 = histgmm_main(mu0, sigma0, gamma, prior, w, x, maxiters, minchange, minsigma, n, m)
    ...
    if isprior.any():
        ...
        a = (x[:, isprior].T - kron(num.ones((n, 1)), mu0[(0, isprior)])).T
    ...
    if isprior.any():
        ...
        a = (x[:, isprior].T - kron(num.ones((n, 1)), mu0[(1, isprior)])).T
        sigma0[(1, isprior)] = num.sqrt((tmp * a ** 2).sum(0) / prior[(1, isprior)])
    sigma0[sigma0 < minsigma] = minsigma
    mu2, sigma2, prior2, score2 = histgmm_main(mu0, sigma0, gamma, prior, w, x, maxiters, minchange, minsigma, n, m)
    inds2 = num.array((score2, 0.0)) > num.array((score1, 1.0))
    if inds2.any():
        mu[:, inds2] = mu2[:, inds2]
...
def histgmm_main(mu0, sigma0, gamma, prior, w, x, maxiters, minchange, minsigma, n, m):
    """Do EM."""
    ...
    for iter in range(maxiters):
        if PRINT_PROGRESS:
            print iter,
            sys.stdout.flush()
        p1 = normpdf(make_vect(x[:, ischange]).T, make_vect(kron(num.ones((n, 1)), mu0[(0, ischange)])).T, make_vect(kron(num.ones((n, 1)), sigma0[(0, ischange)]))) * make_vect(kron(num.ones((n, 1)), prior[(0, ischange)]))
        p2 = normpdf(make_vect(x[:, ischange]).T, make_vect(kron(num.ones((n, 1)), mu0[(1, ischange)])).T, make_vect(kron(num.ones((n, 1)), sigma0[(1, ischange)]))) * make_vect(kron(num.ones((n, 1)), prior[(1, ischange)]))
        tmp = p1 + p2
        p1[tmp < eps] = 0.5
        p2[tmp < eps] = 0.5
        tmp[tmp < eps] = 1
        ...
        if c_and_p.any():
            ...
            a = (x[:, c_and_p].T - kron(num.ones((n, 1)), mu[(0, c_and_p)])).T
        ...
        if c_and_p.any():
            ...
            a = (x[:, c_and_p].T - kron(num.ones((n, 1)), mu[(1, c_and_p)])).T
            sigma[(1, c_and_p)] = num.sqrt((tmp * a ** 2).sum(0) / prior[(1, c_and_p)])
        sigma[sigma < minsigma] = minsigma
        ischange = num.logical_or(num.abs(mu - mu0) >= minchange, num.abs(sigma - sigma0) >= minchange).any(0)
        if not ischange.any():
            break
        mu0 = mu.copy()
        sigma0 = sigma.copy()
    ...
    if PRINT_PROGRESS:
        print
    p1 = normpdf(make_vect(x).T, make_vect(kron(num.ones((n, 1)), mu[0, :])).T, make_vect(kron(num.ones((n, 1)), sigma[0, :]))) * make_vect(kron(num.ones((n, 1)), prior[0, :]))
    p2 = normpdf(make_vect(x).T, make_vect(kron(num.ones((n, 1)), mu[1, :])).T, make_vect(kron(num.ones((n, 1)), sigma[1, :]))) * make_vect(kron(num.ones((n, 1)), prior[1, :]))
    p1.reshape((n, m))
    p2.reshape((n, m))
    a = make_vect(w) * (make_vect(gamma) * p1 + (1 - make_vect(gamma)) * p2)
    score = a.sum(0)
    return (
     mu, sigma, prior, score)