...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/Ctrax/ufmf.py
# Compiled at: 2013-09-24 00:46:30
from __future__ import division
import sys, struct, collections, warnings, os.path, hashlib, os, stat, numpy, numpy as np
from numpy import nan
import logging
logging.basicConfig(level=logging.DEBUG)
import time, math, FlyMovieFormat as FMF
...
FMT = {1: BaseDict(HEADER='<IIdII', CHUNKHEADER='<dI', SUBHEADER='<II', TIMESTAMP='d'), 
   3: BaseDict(HEADER='<4sIQHHB', CHUNKID='<B', KEYFRAME1='<B', KEYFRAME2='<cHHd', POINTS1='<dH', POINTS2='<HHHH'), 
   2: BaseDict(HEADER='<4sILHHB', CHUNKID='<B', KEYFRAME1='<B', KEYFRAME2='<cHHd', POINTS1='<dH', POINTS2='<HHHH'), 
   4: BaseDict(HEADER='<4sIQHHBB', CHUNKID='<B', KEYFRAME1='<B', KEYFRAME2='<cHHd', POINTS1='<dI', POINTS2='<HHHH', POINTS3='<HH')}
KEYFRAME_CHUNK = 0
FRAME_CHUNK = 1
INDEX_DICT_CHUNK = 2
...
class UfmfV1(UfmfBase):
    ...
    def __init__(self, filename, seek_ok=True, use_conventional_named_mean_fmf=True):
    ...
    def readframes(self):
        ...
        while 1:
            ...
            for ptnum in range(n_pts):
                ...
                intup = struct.unpack(FMT[1].SUBHEADER, subbuf)
                xmin, ymin = intup
                if xmin < self._last_safe_x and ymin < self._last_safe_y:
...
class _UFmfV3LowLevelReader(object):
    ...
    def _read_keyframe_chunk(self):
        ...
        intup = struct.unpack(FMT[self._version].KEYFRAME2, self._fd_read(self._keyframe2_sz))
    ...
    def _read_frame_chunk(self):
        ...
        (i.e. curpos-1)
        """
        intup = struct.unpack(FMT[self._version].POINTS1, self._fd_read(self._points1_sz))
        timestamp, n_pts = intup
        regions = []
        for ptno in range(n_pts):
            intup = struct.unpack(FMT[self._version].POINTS2, self._fd_read(self._points2_sz))
            xmin, ymin, w, h = intup
            if self._coding == 'rgb24':
...
class _UFmfV4LowLevelReader(_UFmfV3LowLevelReader):
    ...
    def _read_frame_chunk(self, isbackcompat=False):
        ...
        if self._isfixedsize == 1:
            ...
            for ptno in range(n_pts):
                intup = struct.unpack(self._FMT_POINTS2, self._fd_read(self._points2_sz))
                xmin, ymin, w, h = intup
                if self._coding == 'rgb24':
                    lenbuf = w * h * self._bytesperpixel
                    buf = self._fd_read(lenbuf)
...
class _UFmfV3Indexer(object):
    ...
    def _create_index(self):
        self._index = {'keyframe': collections.defaultdict(dict), 'frame': dict(timestamp=[], loc=[])}
        ...
        for keyframe_type in self._index['keyframe'].keys():
            for key in self._index['keyframe'][keyframe_type].keys():
                self._index['keyframe'][keyframe_type][key] = np.array(self._index['keyframe'][keyframe_type][key])

        for key in self._index['frame'].keys():
            self._index['frame'][key] = np.array(self._index['frame'][key])
...
class UfmfV3(UfmfBase):
    ...
    def __init__(self, file, mode='rb', ignore_preexisting_index=False, is_ok_to_write_regenerated_index=True, short_file_ok=False, raise_write_errors=False, index_progress=False):
        """
        **Arguments**
        ...
        **Optional keyword arguments**
        ...
        ufmf_str, self._version, index_location, self._max_width, self._max_height, coding_str_len = intup
        assert ufmf_str == 'ufmf'
        if not expected_version == self._version:
            ...
            self._points2_sz = struct.calcsize(FMT[self._version].POINTS2)
            if index_location == 0:
                tmp = _UFmfV3Indexer(self._fd, self._version, ignore_preexisting_index=ignore_preexisting_index, short_file_ok=short_file_ok, index_progress=index_progress)
                self._index = tmp.get_index()
                return is_ok_to_write_regenerated_index or None
            loc = tmp.get_expected_index_chunk_location()
            self._fd.seek(loc)
            try:
                index_dict_location = self._fd.tell() + 1
                ...
                self._fd.truncate()
                self._fd.seek(0)
                buf = struct.pack(FMT[self._version].HEADER, 'ufmf', self._version, index_dict_location, self._max_width, self._max_height, len(self._coding))
                self._fd.write(buf)
            except IOError as err:
                if raise_write_errors:
                    raise
                else:
                    warnings.warn('IO error when trying to save .ufmf index for %s (was .ufmf file opened read-only?)' % (
                     file,))
...
class UfmfV4(UfmfV3):
    ...
    def __init__(self, file, mode='rb', ignore_preexisting_index=False, is_ok_to_write_regenerated_index=True, short_file_ok=False, raise_write_errors=False, index_progress=False):
        ...
        **Optional keyword arguments**
        ...
        buf = self._r._fd_read(bufsz)
        intup = struct.unpack(FMT[expected_version].HEADER, buf)
        ufmf_str, self._version, index_location, self._max_width, self._max_height, self._isfixedsize, coding_str_len = intup
        ...
        if not expected_version == self._version:
            ...
            try:
                ...
                self._fd.truncate()
                self._fd.seek(0)
                buf = struct.pack(FMT[self._version].HEADER, 'ufmf', self._version, index_dict_location, self._max_width, self._max_height, self._isfixedsize, len(self._coding))
                self._fd.write(buf)
            except IOError as err:
                if raise_write_errors:
                    raise
...
class FlyMovieEmulator(object):
    ...
    def __init__(self, filename, darken=0, allow_no_such_frame_errors=False, white_background=False, abs_diff=False, **kwargs):
        self._ufmf = Ufmf(filename, **kwargs)
        ...
        if isinstance(self._ufmf, UfmfV1):
            ...
            if self._ufmf.use_conventional_named_mean_fmf:
                if not white_background == False:
                    raise AssertionError
            self.white_background = white_background
            self.abs_diff = abs_diff
            if self.abs_diff:
                raise (isinstance(self._ufmf, UfmfV1) and self._ufmf.use_conventional_named_mean_fmf or NotImplementedError)('abs_diff currently requires UfmfV1 and use_conventional_named_mean_fmf')
    ...
    def get_mean_for_timestamp(self, timestamp):
        if isinstance(self._ufmf, UfmfV1):
            ...
            mean_im, tmp_timestamp = self._ufmf.get_keyframe_for_timestamp('mean', timestamp)
            return mean_im
    ...
    def get_next_frame(self, _return_more=False):
        ...
        for timestamp, regions in self._ufmf.readframes():
            if isinstance(self._ufmf, UfmfV1):
                if self._ufmf.use_conventional_named_mean_fmf:
                    tmp = self._ufmf.get_mean_for_timestamp(timestamp, _return_more=_return_more)
                    if _return_more:
            ...
            if self._isfixedsize:
                ...
                elif self._last_frame.ndim == 3:
                    for ptno in range(npts):
                        self._last_frame[locs[(1, ptno)]:locs[(1, ptno)] + h, locs[(0, ptno)]:locs[(0, ptno)] + w, :] = im[:, :, :, ptno]
        ...
        if isinstance(self._ufmf, UfmfV3):
            ...
            try:
                ...
                if int(os.environ.get('UFMF_FORCE_CACHE', '0')):
                    ...
                    warnings.warn('While attempting to open cache in %s: %s  (set environment variable UFMF_FORCE_CACHE=1 to raise)' % (
                     cache_fname, err))
...
def UfmfSaver(file, frame0=None, timestamp0=None, **kwargs):
...
class UfmfSaverV1(UfmfSaverBase):
    ...
    def __init__(self, filename, frame0, timestamp0, image_radius=10):
        ...
        self.file.write(struct.pack(FMT[1].HEADER, self.version, self.image_radius, self.timestamp0, self.width, self.height))
...
class UfmfSaverV3(UfmfSaverBase):
    ...
    def __init__(self, file, coding='MONO8', frame0=None, timestamp0=None, max_width=None, max_height=None, xinc_yinc=None):
        ...
        buf = struct.pack(FMT[self.version].HEADER, 'ufmf', self.version, 0, self.max_width, self.max_height, len(self.coding))
        self.file.write(buf)
        ...
        self._index = {'keyframe': collections.defaultdict(dict), 'frame': {}}
    ...
    def add_keyframe(self, keyframe_type, image_data, timestamp):
        ...
        if np_image_data.dtype == np.uint8:
            ...
            if np_image_data.dtype == np.float32:
                dtype = 'f'
                strides1 = 4
            else:
        ...
        b = chr(KEYFRAME_CHUNK) + chr(char2) + keyframe_type
    ...
    def add_frame(self, origframe, timestamp, point_data):
        ...
        if len(point_data):
            for this_point_data in point_data:
                ...
                w_radius = w // 2
                h_radius = h // 2
                xmin = int(round(xidx - w_radius) // self.xinc * self.xinc)
                xmin = max(0, xmin)
                xmax = xmin + w
                ...
                if newxmax != xmax:
                    w = newxmax - xmin
                    xmax = newxmax
                ymin = int(round(yidx - h_radius) // self.yinc * self.yinc)
                ymin = max(0, ymin)
                ymax = ymin + h
    ...
    def close(self):
        ...
        self.file.seek(0)
        buf = struct.pack(FMT[self.version].HEADER, 'ufmf', self.version, loc, self.max_width, self.max_height, len(self.coding))
        self.file.write(buf)
        if self._file_opened:
            self.file.close()
            self._file_opened = False
    ...
    def _add_frame_regions(self, timestamp, regions):
        if len(regions):
            for kf_type in self._cached_keyframes.keys():
                ...
                super(AutoShrinkUfmfSaverV3, self).add_keyframe(kf_type, kf_image_data, kf_timestamp)
