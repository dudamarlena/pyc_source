...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/Ctrax/ExpBGFGModel.py
# Compiled at: 2013-09-24 00:57:36
r"""
...
"""
import getopt, sys, os.path, pickle, numpy as num, scipy.ndimage.morphology as morph, scipy.ndimage.filters as filters, scipy.io
from params import params
import bg, movies, annfiles as annot, estconncomps as est
if params.interactive:
    import wx
DEBUG = True
if DEBUG:
    import pdb, matplotlib.pyplot as plt, matplotlib.cm as cm

class ExpBGFGModel():

    def __init__(self, movienames=None, annnames=None, picklefile=None, LoG_hsize=21, LoG_sigma=5.0, LoG_nonmaxsup_sz=5, LoG_thresh=0.5, difftype='darkfglightbg', obj_detection_dist_nbins=100, isback_dilation_hsize=10, fg_min_sigma=1.0, bg_min_sigma=1.0, min_always_bg_mask_frac=1.0, min_frac_cc_covered=0.01, min_area_cc_covered=1, backsub_n_bg_std_thresh=10.0, backsub_n_bg_std_thresh_low=1.0, backsub_do_use_morphology=False, backsub_opening_radius=0, backsub_closing_radius=0):
        """
        ...
        if picklefile is not None:
            ...
            try:
                ...
                fid.close()
                model['obj_detection_dist_centers'] = (model['obj_detection_dist_edges'][1:] + model['obj_detection_dist_edges'][:-1]) / 2.0
    ...
    def est_marginals(self):
        ...
        for i in range(self.nmovies):
            if DEBUG:
                print 'Computing per-movie model for movie %d' % i
            self.compute_models_permovie(i)

        if DEBUG:
            print 'Computing foreground appearance marginal'
        self.est_fg_appearance_marginal()
        if DEBUG:
            print 'Computing background appearance marginal'
        self.est_bg_appearance_marginal()
        if DEBUG:
            print 'Computing object detection distance marginal'
        self.est_obj_detection_marginal()
        if DEBUG:
            print 'Estimating always_bg_mask'
    ...
    def est_fg_appearance_marginal(self):
        ...
        pool_data(self.fg_nsamples_px, self.fg_mu_px, self.fg_sigma_px, params.prior_fg_nsamples_pool, params.prior_fg_pool_radius_factor, self.fg_nsamples, self.fg_mu, self.fg_sigma, self.fg_poolradius, self.fg_min_sigma)
        self.fg_log_Z = 0.5 * num.log(2 * num.pi) + num.log(self.fg_sigma)
    ...
    def est_bg_appearance_marginal(self):
        ...
        pool_data(self.bg_nsamples_px, self.bg_mu_px, self.bg_sigma_px, params.prior_bg_nsamples_pool, params.prior_bg_pool_radius_factor, self.bg_nsamples, self.bg_mu, self.bg_sigma, self.bg_poolradius, self.bg_min_sigma)
        self.bg_log_Z = 0.5 * num.log(2 * num.pi) + num.log(self.bg_sigma)
    ...
    def compute_models_permovie(self, i):
        ...
        self.trx = annot.AnnotationFile(self.annnames[i], bg_imgs=self.bg_imgs, doreadtrx=True, readonly=True)
        self.setup_backsub()
        self.firstframe = self.trx.firstframetracked
        self.lastframe = self.trx.lastframetracked
        self.framessample = num.unique(num.round(num.linspace(self.firstframe + 1, self.lastframe, params.prior_nframessample)).astype(int))
        self.update_always_bg_mask()
        if DEBUG:
            print 'Collecting foreground and background data...'
        for j in range(len(self.framessample)):
            ...
            if DEBUG:
                print 'Collecting data for frame %d' % self.frame
            self.isfore_mask()
    ...
    def compute_log_lik_dist_obj(self, r0=0, r1=num.inf):
        ...
        morph.distance_transform_edt(~self.isobj, distances=self.obj_detection_dist[r0:r1])
        nr = r1 - r0
    ...
    def compute_log_lik(self, r0=0, r1=num.inf):
        ...
        self.log_lik_given_fore = self.log_lik_appearance_given_fore + self.log_lik_dist_obj_given_fore
        self.log_lik_given_back = self.log_lik_appearance_given_back + self.log_lik_dist_obj_given_back
    ...
    def update_obj_detection_marginal(self):
        ...
        counts, edges = num.histogram(self.obj_detection_dist[self.isfore], bins=self.obj_detection_dist_edges)
        self.obj_detection_dist_counts_fg += counts
        counts, edges = num.histogram(self.obj_detection_dist[self.isback], bins=self.obj_detection_dist_edges)
    ...
    def init_obj_detection(self):
        ...
        edges1 = num.arange(-0.5, n1 - 0.5)
        edges2 = num.unique(num.round(num.exp(num.linspace(num.log(n1 + 0.5), num.log(num.sqrt(self.nr ** 2 + self.nc ** 2) + 0.5), n2 + 1)) - 0.5) + 0.5)
        ...
        self.obj_detection_dist_centers = (self.obj_detection_dist_edges[1:] + self.obj_detection_dist_edges[:-1]) / 2.0
    ...
    def make_LoG_filter(self):
        ...
        self.LoG_fil[absfil < 1e-05 * num.max(absfil)] = 0
    ...
    def LoGfilter(self, r0=0, r1=num.inf):
        ...
        filters.correlate(self.im[r00:r11].astype(float), self.LoG_fil, output=self.LoG_fil_out[r00:r11], mode='nearest')
        if self.difftype == 'other':
            self.LoG_fil_out[r00:r11] = num.abs(self.LoG_fil_out[r00:r11])
        filters.maximum_filter(self.LoG_fil_out[r00:r11], size=self.LoG_nonmaxsup_sz, output=self.LoG_fil_max[r00:r11], mode='constant', cval=0)
        self.isobj = num.logical_and(self.LoG_fil_out[r0:r1] == self.LoG_fil_max[r0:r1], self.LoG_fil_out[r0:r1] >= self.LoG_thresh)
    ...
    def show(self):
        ...
        plt.figure()
        plt.plot(self.obj_detection_dist_centers, self.obj_detection_dist_frac_bg, 'k.-', self.obj_detection_dist_centers, self.obj_detection_dist_frac_fg, 'r.-')
        plt.legend(('bg', 'fg'))
    ...
    def showtest(self, moviename=None, moviei=0, nframessample=4, cmin=-num.infty, cmax=num.infty):
        ...
        vmin_noninf[idx] = vmin_noninf[idx] - dv[idx] * 0.025
        idx = vmax > vmax_noninf
        vmax_noninf[idx] = vmax_noninf[idx] + dv[idx] * 0.025
        for j in range(nc):
            for i in range(nr):
...
def update_clim(x, vmin, vmax, vmin_noninf, vmax_noninf, j):
    ...
    return (
     vmin, vmax, vmin_noninf, vmax_noninf)
...
def pool_data(nsamples_px, mu_px, sigma_px, nsamples_pool, pool_radius_factor, nsamples, mu, sigma, poolradius, min_sigma):
    """
    ...
    while radius <= maxradius and notdone.any():
        ...
        if not newdone.any():
            ...
            if DEBUG:
                print 'no pixels newly with enough samples, incrementing radius to %d' % radius
            continue
        ...
        mu[(r, c)] = integral_image_sum(int_mu_px, r0, r1, c0, c1) / nsamples[(r, c)]
        sigma[(r, c)] = num.sqrt(integral_image_sum(int_sigma_px, r0, r1, c0, c1) / nsamples[(r, c)])
        if DEBUG:
            print 'n pixels added at radius %d: %d' % (radius, len(r))
        radius = max(radius + 1, int(round(radius * pool_radius_factor)))
...
def test1():
    ...
    """
    movienames = [
     'E:\\Data\\FlyBowl\\Test1\\GMR_13B06_AE_01_TrpA_Rig1Plate01BowlA_20101007T161822/movie.ufmf']
...
def test2():
    ...
    """
    movienames = [
     '/media/data/data3/pGAWBCyO-GAL4_TrpA_Rig1Plate01Bowl1_20100820T140408/movie.fmf']
...
def main():
    ...
    fg_min_sigma = 1.0
    bg_min_sigma = 1.0
    min_always_bg_mask_frac = 1.0
    min_frac_cc_covered = 0.01
    min_area_cc_covered = 1
    backsub_n_bg_std_thresh = 10.0
    backsub_n_bg_std_thresh_low = 1.0
    ...
    if paramsFileName != '':
        ...
        fid.close()
    model = ExpBGFGModel(movienames, annnames, LoG_hsize=LoG_hsize, LoG_sigma=LoG_sigma, LoG_nonmaxsup_sz=LoG_nonmaxsup_sz, LoG_thresh=LoG_thresh, difftype=difftype, obj_detection_dist_nbins=obj_detection_dist_nbins, isback_dilation_hsize=isback_dilation_hsize, fg_min_sigma=fg_min_sigma, bg_min_sigma=bg_min_sigma, min_always_bg_mask_frac=min_always_bg_mask_frac, min_frac_cc_covered=min_frac_cc_covered, min_area_cc_covered=min_area_cc_covered, backsub_n_bg_std_thresh=backsub_n_bg_std_thresh, backsub_n_bg_std_thresh_low=backsub_n_bg_std_thresh_low, backsub_do_use_morphology=backsub_do_use_morphology, backsub_opening_radius=backsub_opening_radius, backsub_closing_radius=backsub_closing_radius)
    model.est_marginals()
