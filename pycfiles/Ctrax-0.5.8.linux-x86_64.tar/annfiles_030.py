...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/Ctrax/annfiles_030.py
# Compiled at: 2016-04-23 00:51:51
import os, tempfile, shutil, pickle, time, numpy as num
...
class AnnotationFile():
    ...
    def common_initialization(self):
        ...
        self.buffer = []
        self.nbuffer = max(params.anndata_nbuffer, self.maxlookback) + 1
        self.lookup = []
            ...
            while True:
                ...
                if parameter == 'bg_type' and self.bg_imgs is not None:
                    try:
                        ...
                        print 'converted background type setting to Ctrax 0.2+ compatibility'
                    except ValueError:
    ...
    def WriteMAT(self, filename):
        ...
        if self.nbuffer < nframes:
            msgtxt = 'Saving to a MAT-file will require loading all %d frames of tracked data into memory, which may require a prohibitive amount of RAM. ' % nframes
            if params.feedback_enabled:
                a = wx.MessageBox(msgtxt + 'Proceed?', 'Store to MAT file?', wx.YES | wx.NO)
                if a == wx.NO:
                    return
                wx.Yield()
            else:
