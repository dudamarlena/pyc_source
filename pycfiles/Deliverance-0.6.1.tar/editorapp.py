...
# [GCC 8.4.0]
# Embedded file name: /home/egj/Code/deliverance/Deliverance/deliverance/editor/editorapp.py
# Compiled at: 2011-11-07 14:38:20
...
class Editor(object):
    ...
    def __init__(self, base_dir=None, filename=None, title=None, force_syntax=None):
    ...
    def __call__(self, environ, start_response):
        ...
        if req.path_info_peek() == '.media':
            ...
            if req.method not in ('GET', 'POST'):
                resp = exc.HTTPMethodNotAllowed('Bad method: %s' % req.method, allow='GET,POST')
            if os.path.isdir(filename):
                if req.method == 'POST':
                    resp = self.save_create(req, filename)
                else:
                    resp = req.path.endswith('/') or exc.HTTPMovedPermanently(add_slash=True)
            else:
    ...
    def save_file(self, req, filename):
        ...
        f.write(content)
        f.close()
        return exc.HTTPFound(location=self.edit_url(req, filename))
    ...
    def edit_file(self, req, filename):
        ...
        body = self.edit_template.substitute(content=content, filename=filename, title=title, req=req, edit_url=self.edit_url(req, filename), syntax=syntax)
        resp = Response(body=body)
        resp.cache_expires()
        return resp

    edit_template = HTMLTemplate.from_filename(os.path.join(os.path.dirname(__file__), 'editor_template.html'))
    ...
    def save_create(self, req, dir):
        ...
        if os.path.exists(filename):
            return exc.HTTPForbidden('The file %s already exists, you cannot upload over it' % filename)
        else:
            f = open(filename, 'wb')
            f.write(content)
            f.close()
    ...
    def view_dir(self, req, dir):
        ...
        children = [ os.path.join(dir, name) for name in os.listdir(dir) if name not in self.skip_files
                   ]
        ...
        body = self.view_dir_template.substitute(req=req, dir=dir, show_parent=show_parent, title=title, basename=os.path.basename, dirname=os.path.dirname, isdir=os.path.isdir, children=children, edit_url=edit_url)
        resp = Response(body=body)
        resp.cache_expires()
        return resp

    view_dir_template = HTMLTemplate.from_filename(os.path.join(os.path.dirname(__file__), 'view_dir_template.html'))