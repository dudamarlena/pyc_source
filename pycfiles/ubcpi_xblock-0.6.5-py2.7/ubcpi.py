...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/ubcpi/ubcpi.py
# Compiled at: 2018-06-27 18:28:38
...
def validate_options(options):
    ...
    if not any(error in [_('Minimum Characters'), _('Maximum Characters')] for error in errors) and int(options['rationale_size']['max']) <= int(options['rationale_size']['min']):
...
class PeerInstructionXBlock(XBlock, MissingDataFetcherMixin, PublishEventMixin):
    ...
    question_text = Dict(default={'text': _('<p>Where does most of the mass in a fully grown tree originate?</p>'), 'image_url': '', 
       'image_position': 'below', 'image_show_fields': 0, 'image_alt': ''}, scope=Scope.content, help=_("The question the students see. This question appears above the possible answers which you set below. You can use text, an image or a combination of both. If you wish to add an image to your question, press the 'Add Image' button."))
    options = List(default=[{'text': _('Air'), 'image_url': '', 'image_position': 'below', 'image_show_fields': 0, 'image_alt': ''}, {'text': _('Soil'), 'image_url': '', 'image_position': 'below', 'image_show_fields': 0, 'image_alt': ''}, {'text': _('Water'), 'image_url': '', 'image_position': 'below', 'image_show_fields': 0, 'image_alt': ''}], scope=Scope.content, help=_('The possible options from which the student may select'))
    rationale_size = Dict(default={'min': 1, 'max': MAX_RATIONALE_SIZE}, scope=Scope.content, help=_('The minimum and maximum number of characters a student is allowed for their rationale.'))
    correct_answer = Integer(default=0, scope=Scope.content, help=_('The correct option for the question'))
    correct_rationale = Dict(default={'text': _('Photosynthesis')}, scope=Scope.content, help=_('The feedback for student for the correct answer'))
    stats = Dict(default={'original': {}, 'revised': {}}, scope=Scope.user_state_summary, help=_('Overall stats for the instructor'))
    seeds = List(default=[{'answer': 0, 'rationale': _('Tree gets carbon from air.')}, {'answer': 1, 'rationale': _('Tree gets minerals from soil.')}, {'answer': 2, 'rationale': _('Tree drinks water.')}], scope=Scope.content, help=_('Instructor configured examples to give to students during the revise stage.'))
    sys_selected_answers = Dict(default={}, scope=Scope.user_state_summary, help=_('System selected answers to give to students during the revise stage.'))
    other_answers_shown = Dict(default={}, scope=Scope.user_state, help=_('Stores the specific answers of other students shown, for a given student.'))
    algo = Dict(default={'name': 'simple', 'num_responses': '#'}, scope=Scope.content, help=_('The algorithm for selecting which answers to be presented to students'))
    has_score = True
    start = DateTime(default=None, scope=Scope.settings, help=_('ISO-8601 formatted string representing the start date of this assignment. We ignore this.'))
    due = DateTime(default=None, scope=Scope.settings, help=_('ISO-8601 formatted string representing the due date of this assignment. We ignore this.'))
    weight = Float(default=1, display_name=_('Problem Weight'), help=_('Defines the number of points each problem is worth. If the value is not set, the problem is worth the sum of the option point values.'), values={'min': 0, 'step': 0.1}, scope=Scope.settings)
    ...
    def studio_view(self, context=None):
        ...
        frag.initialize_js('PIEdit', {'display_name': self.ugettext(self.display_name), 
           'weight': self.weight, 
           ...
           'algo': self.algo, 
           'algos': {'simple': self.ugettext('System will select one of each option to present to the students.'), 
                     'random': self.ugettext('Completely random selection from the response pool.')}, 
           'image_position_locations': {'above': self.ugettext('Appears above'), 
    ...
    def get_base_url_path_for_course_assets(cls, course_key):
        ...
        if course_key is None:
            ...
            url_path = cls.serialize_asset_key_with_slash(course_key.make_asset_key('asset', placeholder_id).for_branch(None))
    ...
    def student_view(self, context=None):
        ...
        js_vals = {'answer_original': answers.get_vote(0), 'rationale_original': answers.get_rationale(0), 
           ...
           'all_status': {'NEW': STATUS_NEW, 'ANSWERED': STATUS_ANSWERED, 'REVISED': STATUS_REVISED}, 'lang': translation.get_language()}
    ...
    def record_response(self, answer, rationale, status):
        ...
        if self.correct_answer == len(self.options):
            ...
            corr_ans_text = (
             self.options[self.correct_answer].get('text'),)
        event_dict = {'answer': answer, 'answer_text': self.options[answer].get('text'), 
        ...
        if not answers.has_revision(0) and status == STATUS_NEW:
            ...
            offer_answer(self.sys_selected_answers, answer, rationale, student_item['student_id'], self.algo, self.options)
            self.other_answers_shown = get_other_answers(self.sys_selected_answers, self.seeds, self.get_student_item_dict, self.algo, self.options)
            event_dict['other_student_responses'] = self.other_answers_shown
            self.publish_event_from_dict(self.event_namespace + '.original_submitted', event_dict)
            return event_dict['other_student_responses']
        if answers.has_revision(0) and not answers.has_revision(1) and status == STATUS_ANSWERED:
            ...
            self.publish_event_from_dict(self.event_namespace + '.revised_submitted', event_dict)
        else:
    ...
    def get_current_stats(self):
        ...
        """
        self.stats = {'original': {int(k):v for k, v in self.stats['original'].iteritems()}, 'revised': {int(k):v for k, v in self.stats['revised'].iteritems()}}
        return self.stats
    ...
    def get_persisted_data(self, other_answers):
        ...
        """
        answers = self.get_answers_for_student()
        ret = {'answer_original': answers.get_vote(0), 
