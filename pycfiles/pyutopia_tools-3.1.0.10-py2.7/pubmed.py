...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/utopia/tools/pubmed.py
# Compiled at: 2017-06-19 21:06:42
import utopia.tools.eutils, difflib, htmlentitydefs, json, re, urllib, urllib2
...
def _split(title):
    ...
    title = re.sub('(\\d+\\.\\d+)A', '\\1 A', title)
    title = (' AND ').join(word + '[titl]' for word in title.split() if word.lower() not in utopia.tools.eutils.STOPWORDS)
    return title
...
_PubmedArticle_fields = {'MedlineCitation/Article[1]/ArticleTitle/text()': 'title', 
...
def parse_PubmedArticle(article):
    ...
    for path, key in _PubmedArticle_fields.iteritems():
        ...
        for author_elem in article.xpath('MedlineCitation/Article/AuthorList/Author'):
            authors.append(((' ').join(author_elem.xpath('LastName/text()')) + ', ' + (' ').join(author_elem.xpath('ForeName/text()'))).strip(', '))
