...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/utopia/tools/nlm.py
# Compiled at: 2017-06-19 12:54:55
import copy, re, utopia.citation
...
def parse(nlm):
    ...
    if nlm is not None:
        ...
        if nlm is None:
            return
        metadata = {}
        metadata['self'] = _extract(nlm, {'publication-title': 'front/journal-meta//journal-title[1]/text()', 
        ...
        authors = [ ('{0}, {1}').format(etree.tostring(a.find('surname', ''), method='text', encoding=unicode, with_tail=False), etree.tostring(a.find('given-names', ''), method='text', encoding=unicode, with_tail=False)).strip(', ') for a in author_elems
                  ]
        ...
        for ref in nlm.xpath('back/ref-list/ref'):
            ...
            authors = [ ('{0}, {1}').format(etree.tostring(a.find('surname', ''), method='text', encoding=unicode, with_tail=False), etree.tostring(a.find('given-names', ''), method='text', encoding=unicode, with_tail=False)).strip(', ') for a in authorelems
                      ]
        ...
        for paragraph in nlm.xpath('front/article-meta/abstract//p | body//p'):
            ...
            for i in xrange(len(xref_stack) - 2, 0, -2):
                ...
                if len(before) > 0 and before in '({[' and len(after) > 0 and after in ')}]':
                    text_stack[i - 1] = re.sub('[({[](\\s*)$', '\\1', text_stack[(i - 1)])
                    text_stack[i + 1] = re.sub('^(\\s*)[)}\\]]', '\\1', text_stack[(i + 1)])
                    text_stack[i] = before + text_stack[i] + after

            for i in xrange(1, len(xref_stack), 2):
                before = (' ').join(text_stack[:i]).strip()
                label = text_stack[i].strip()
                after = (' ').join(text_stack[i + 1:]).strip()
                if len(xref_stack[i]) > 1:
                    label = re.sub('[)}\\]]?([-‐‑‒–—―,])\\s?[({[]?', '\\1', label)
                else:
                    label = re.sub('[)}\\]]?([-‐‑‒–—―,]\\s?)[({[]?', '\\1', label)
                before = re.sub('\\s+', ' ', before)[-max_length:].strip()
                label = re.sub('\\s+', ' ', label)
                after = re.sub('\\s+', ' ', after)[:max_length].strip()
        ...
        for table_wrap in nlm.xpath('//table-wrap'):
            ...
            if id is not None:
                queries = {'label': 'label/text()', 'caption': '(caption/title|caption/p)[1]', 
                ...
                if 'caption' in table:
                    table['caption'] = re.sub('\\s+', ' ', etree.tostring(table['caption'], method='text', encoding=unicode).strip())
                if 'xml' in table:
                    table['xml'] = etree.tostring(table['xml'], encoding='utf8')
