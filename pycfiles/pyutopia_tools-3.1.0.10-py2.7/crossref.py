...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/utopia/tools/crossref.py
# Compiled at: 2017-04-20 05:36:11
import json, re, socket, urllib, urllib2
...
def resolve(doi):
    ...
    if len(persons) > 0:
        citation['authors'] = [ etree.tostring(person.find('surname'), with_tail=False, method='text', encoding=unicode) + ', ' + etree.tostring(person.find('given_name'), with_tail=False, method='text', encoding=unicode) for person in persons ]
    ...
    pages = ('-').join(p for p in (citation.get('first_page'), citation.get('last_page')) if p is not None)
    if len(pages) > 0:
...
def search(title):
    citations = []
    url = ('http://search.crossref.org/dois?{0}').format(urllib.urlencode({'version': '1', 
       'access': api_key, 
       'format': 'json', 
       'op': 'OR', 
       'q': title.encode('utf8')}))
    ...
    for item in data:
        citation = {'title': item['title'], 'year': item['year'], 
...
def freeform_search(freeform):
    ...
    request = urllib2.Request(url, headers={'Content-Type': 'application/json'}, data=json.dumps(freeform))
        ...
        if message is not None:
            ...
            for old_key, new_key in {'DOI': 'doi'}.items():
                ...
                if old_key in message:
                    ...
                    if new_key == 'doi':
                        citation['links'] = [
                         {'type': 'article', 'url': ('http://dx.doi.org/{}').format(citation['identifiers'][new_key]), 
                            'mime': 'text/html', 
                            'title': "Show on publisher's website"}]
            ...
            for author in message.get('author', []):
                ...
                if 'given' in author:
                    name += (', {}').format(author.get('given', 'Unknown'))
                authors.append(name)
            ...
            for old_key, new_key in {'title[]': 'title', 'subtitle[]': 'subtitle', 
