...
# [GCC 8.4.0]
# Embedded file name: /Users/ana/dev/dexy-clean/dexy/plugin.py
...
class PluginMeta(cashew.PluginMeta):
    ...
    def apply_prefix(cls, modname, alias):
        if modname.startswith('dexy_'):
            if modname not in PluginMeta.official_dexy_plugins:
...
class TemplatePlugin(Plugin, metaclass=PluginMeta):
    ...
    aliases = []
    _settings = {'no-jinja-filter': 'Listed entries should not be made into jinja filters.'}
