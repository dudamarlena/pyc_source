...
# [GCC 8.4.0]
# Embedded file name: /Users/ana/dev/dexy-clean/dexy/wrapper.py
...
import chardet, dexy.batch, dexy.doc, dexy.parser, dexy.reporter, dexy.utils, logging, logging.handlers, os, pickle, posixpath, shutil, sys, textwrap, time, traceback, uuid
...
class Wrapper(object):
    ...
    state_transitions = ((None, 'new'), ('new', 'valid'), ('valid', 'valid'), ('valid', 'walked'),
                         ('walked', 'checked'), ('checked', 'running'), ('running', 'error'),
                         ('running', 'ran'))
    ...
    def update_attributes_from_kwargs(self, kwargs):
        ...
        for key, value in kwargs.items():
            if key not in dexy.utils.defaults:
    ...
    def deprecate_logs_directory(self):
        ...
        if os.path.exists(log_dir):
            if os.path.exists(safety_file):
                if not os.path.exists(deprecation_notice_file):
                    deprecation_notice = "            Dexy no longer has a separate '{log_dir}' directory for the log file.\n            The logfile can now be found at: {log_path}\n".format(log_path=(self.log_path()),
                      log_dir=log_dir)
    ...
    def setup_log(self):
        ...
        handler = logging.handlers.RotatingFileHandler((self.log_path()),
          encoding='utf-8')
    ...
    def map_files(self):
        ...
        for dirpath, dirnames, filenames in os.walk('.', followlinks=True):
            for x in exclude:
                if x in dirnames and x not in self.include:
                    dirnames.remove(x)
    ...
    def add_data_to_lookup_nodes(self, key, data):
        if key not in self.lookup_nodes:
            self.lookup_nodes[key] = []
        if data not in self.lookup_nodes[key]:
            self.lookup_nodes[key].append(data)

    def add_data_to_lookup_sections(self, key, data):
        if key not in self.lookup_sections:
            self.lookup_sections[key] = []
        if data not in self.lookup_sections[key]:
