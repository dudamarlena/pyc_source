...
class SubprocessFilter(Filter):
    ...
    _settings = {'args':('Arguments to be passed to the executable.', ''), 
     ...
     'make-dummy-output':('Whether to make a dummy output file when one is not generated and add-new-files is True.',
 False), 
     'env':(
      'Dictionary of key-value pairs to be added to environment for runs.', {}), 
     'executable':('The executable to be run', None), 
     'initial-timeout':('', 10), 
     'path-extensions':(
      'strings to extend path with', []), 
    ...
    def default_command_string_args(self):
        args = {'args':' '.join([self.setting('args'), self.setting('clargs')]), 
    ...
    def handle_subprocess_proc_return(self, command, exitcode, stderr, compiled=False):
        ...
        if exitcode is None:
            ...
            if exitcode == 127:
                raise (compiled or dexy.exceptions.InactivePlugin)(self.alias)
            else:
                if exitcode != 0:
    ...
    def setup_env(self):
        ...
        if self.setting('path-extensions'):
            paths = [
             env['PATH']] + self.setting('path-extensions')
            env['PATH'] = ':'.join(paths)
        return env
        ...
        for dirpath, subdirs, filenames in os.walk(wd):
            ...
            for filename in filenames:
                ...
                if relpath in self._files_workspace_populated_with:
                    continue
                elif isinstance(add_new_files, list):
                    ...
                    msg = is_valid_file_extension or 'Not adding filename %s, does not match patterns: %s'
                    args = (filepath, ', '.join(add_new_files))
                    self.log_debug(msg % args)
                    continue
                else:
                    if isinstance(add_new_files, bool):
    ...
    def copy_canonical_file(self):
        ...
        if self.setting('add-new-files'):
            if self.setting('make-dummy-output'):
                if not self.output_data.is_cached():
...
class SubprocessStdoutFilter(SubprocessFilter):
    ...
    _settings = {'write-stderr-to-stdout':False, 
...
class SubprocessCompileFilter(SubprocessFilter):
    ...
    _settings = {'add-new-files':False, 
     'check-return-code':False, 
     'compiled-extension':('Extension which compiled files end with.', '.o'), 
     'compiler-command-string':('Command string to call compiler.', '%(prog)s %(compiler_args)s %(script_file)s -o %(compiled_filename)s'), 
...
class SubprocessFormatFlagFilter(SubprocessFilter):
    ...
    _settings = {'ext-to-format': ('A dict of mappings from file extensions to format flags that need to be passed on the command line, e.g. for ragel with ruby host language .rb => -R', {})}
...
class SubprocessExtToFormatFilter(SubprocessFilter):
    ...
    _settings = {'format-specifier':('The string used to specify the format switch, include trailing space if needed.',
 None), 
     'ext-to-format':(
      'A dict of mappings from file extensions to format parameters that need to be passed on the command line, e.g. for ghostscript .png => png16m', {})}
...
class SubprocessStdoutTextFilter(SubprocessStdoutFilter):
    ...
    _settings = {'command-string':'%(prog)s %(args)s "%(text)s"', 
     'input-extensions':[
      '.txt'], 
     'output-extensions':[
      '.txt']}
