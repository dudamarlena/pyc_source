...
# [GCC 8.4.0]
# Embedded file name: /Users/ana/dev/dexy-clean/dexy/filters/confluence.py
# Compiled at: 2020-03-18 15:12:33
# Size of source mod 2**32: 13563 bytes
...
class ConfluenceRESTAPI(DexyFilter):
    ...
    _settings = {'publish':False, 
     ...
     'upload-attachments':('If True, attachments will automatically be uploaded. Can also be a list of file extensions.',
 True), 
     'skip-attachments':('A list of file extensions which should not be uploaded as attachments.', None), 
     'fix-attachment-paths':('If True, automatically replace attachement filenames with uploaded paths in document content.',
 True), 
     ...
     'custom-mime-types':('Map of file extensions to mime types to supplement the python mimetypes module.',
 None)}
    ...
    def attachment_minor_edit_setting(self):
        return self.bool_setting_to_string(self.setting('attachment-minor-edit'))

    def page_minor_edit_setting(self):
        return self.bool_setting_to_string(self.setting('page-minor-edit'))
    ...
    def post_file(self, path, canonical_name, filepath):
        ...
        with open(filepath, 'rb') as (fileref):
            files = {'file':(canonical_name, fileref, mimetype),  'comment':str(self.setting('attachment-comment')), 
             'minorEdit':self.attachment_minor_edit_setting()}
            response = (requests.post)(
 self.url_for_path(path), files=files, **self.credentials_with_headers(no_check))
        self.handle_response_code(response)
        return response.json()

    def delete_path(self, path):
        response = (requests.delete)(
         (self.url_for_path(path)), **self.credentials())
        self.handle_response_code(response)
        return response.json()

    def json_post_path(self, path, data=None):
        response = (requests.post)(
 self.url_for_path(path), data=json.dumps(data), **self.credentials_with_json_content_type())
        self.handle_response_code(response)
        return response.json()
 ...
 self.url_for_path(path), data=json.dumps(data), **self.credentials_with_json_content_type())
    ...
    def ancestors(self):
        if self.setting('parent-page-id') is not None:
            return [
             {'type':'page', 
              'id':self.setting('parent-page-id')}]
        return []
    ...
    def update_existing_page(self, page_id, page_text):
        ...
        data = {'type':'page', 
         ...
         'version':{'number':page_version + 1, 
          'minorEdit':self.bool_setting_to_string(self.setting('page-minor-edit'))}}
    ...
    def upload_attachments(self, page_id):
        ...
        attachment_ids = dict(((att['title'], att['id']) for att in existing_attachments))
        attachments = {}
        for input_doc in self.doc.walk_input_docs():
            ...
            if not isinstance(self.setting('upload-attachments'), bool):
                if ext not in self.setting('upload-attachments'):
                    self.log_info('Skipping %s because %s not in %s' % (canonical_name, ext, self.setting('upload-attachments')))
                    continue
                elif self.setting('skip-attachments') is not None and ext in self.setting('skip-attachments'):
