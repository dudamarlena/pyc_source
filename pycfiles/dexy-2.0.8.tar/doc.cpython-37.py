...
# [GCC 8.4.0]
# Embedded file name: /Users/ana/dev/dexy-clean/dexy/doc.py
# Compiled at: 2020-03-18 15:12:33
# Size of source mod 2**32: 12965 bytes
import dexy.exceptions, dexy.filter, dexy.node, os, pickle, shutil, stat, time

class Doc(dexy.node.Node):
    __doc__ = '\n    A single Dexy document.\n    '
    aliases = ['doc']
    _settings = {'contents':('Custom contents for a virtual document.', None), 
     'ws-template':('custom website template to apply.', None), 
     'data-type':('Alias of custom data class to use to store document content.', None), 
     'shortcut':("A nickname for document so you don't have to use full key.", None), 
     'title':('Custom title.', None), 
     'output-name':('Override default canonical name.', None), 
     'output':('Whether document should be included in output/ and output-site/', None)}
            ...
            if output_name.startswith('/'):
                ...
                try:
                    if '%' in output_name:
                        ...
                        if '{' in self.setting('output-name'):
    ...
    def setup(self):
        ...
        for i, f in enumerate(self.filters):
            ...
            f.setup(filter_key, storage_key, prev_filter, next_filter, filter_settings_from_args)
            prev_filter = f
    ...
    def setup_initial_data(self):
        ...
        settings = {'canonical-name':self.name,  'canonical-output':canonical_output, 
         'shortcut':self.setting('shortcut'), 
         'output-name':self.setting('output-name'), 
         'title':self.setting('title')}
        self.initial_data = dexy.data.Data.create_instance(self.data_class_alias(), self.name, self.ext, storage_key, settings, self.wrapper)
    ...
    def datas(self):
        ...
        """
        return [
         self.initial_data] + [f.output_data for f in self.filters]
    ...
    def check_cache_elements_present(self):
        ...
        return all((os.path.exists(d.storage.last_data_file()) or os.path.exists(d.storage.this_data_file()) for d in self.datas()))
    ...
    def save_runtime_info(self):
        ...
        been cached.
        """
        info = {'runtime-args':self.runtime_args, 
    ...
    def run(self):
        if self.wrapper.directory != '.':
            if self.wrapper.directory not in self.name:
                print(('skipping', self.name, 'not in', self.wrapper.directory))
                return
        else:
            self.start_time = time.time()
            if self.name in self.wrapper.filemap:
                self.doc_changed or self.initial_data.is_cached() or self.initial_data.copy_from_file(self.name)
            else:
    ...
    def batch_info(self):
        return {'input-data':self.initial_data.args_to_data_init(), 
