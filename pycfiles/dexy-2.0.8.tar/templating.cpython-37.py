...
# [GCC 8.4.0]
# Embedded file name: /Users/ana/dev/dexy-clean/dexy/filters/templating.py
...
import dexy.exceptions, jinja2, jinja2.ext, os, re, traceback
...
class PassThroughWhitelistUndefined(jinja2.StrictUndefined):
    ...
    def __call__(self, *args, **kwargs):
        ...
        if name in self.call_whitelist:
            msgargs = {'name':name,  'argstring':','.join((self.wrap_arg(a) for a in args))}
...
class TemplateFilter(DexyFilter):
    ...
    def template_plugins(self):
        ...
        return [instance for instance in TemplatePlugin.__iter__(self) if instance.alias not in self.setting('skip-plugins')]
...
class JinjaFilter(TemplateFilter):
    ...
    aliases = ['jinja']
    _settings = {'block-start-string':('Tag to indicate the start of a block.', '{%'), 
     ...
     'jinja-extensions':(
      'List of jinja extensions to activate.', ['jinja2.ext.do']), 
     'workspace-includes':[
      '.jinja'], 
    ...
    _not_jinja_settings = ('changetags', 'jinja-path', 'workspace-includes', 'filters',
                           'assertion-passed-indicator', 'jinja-extensions')
    TEX_TAGS = {'block_start_string':'<%', 
     'block_end_string':'%>', 
     'variable_start_string':'<<', 
     'variable_end_string':'>>', 
     'comment_start_string':'<#', 
     'comment_end_string':'#>'}
    LYX_TAGS = {'block_start_string':'<%', 
     'block_end_string':'%>', 
     'variable_start_string':'<<', 
     'variable_end_string':'>>', 
     'comment_start_string':'<<#', 
    ...
    def setup_jinja_env(self, loader=None):
        ...
        for k, v in self.setting_values().items():
            ...
            if k in self.__class__._settings and k not in self._not_jinja_settings:
                env_attrs[underscore_k] = v

        env_attrs['undefined'] = PassThroughWhitelistUndefined
        if self.ext in ('.tex', '.wiki'):
            if self.setting('changetags'):
    ...
    def handle_jinja_exception(self, e, input_text, template_data):
        ...
        if isinstance(e, UndefinedError) or isinstance(e, TypeError):
            m = re.search('File \\"<template>\\", line ([0-9]+), in top\\-level template code', traceback.format_exc())
            if m:
        ...
        args = {'error_type':e.__class__.__name__, 
         'key':self.key, 
         'lineno':e.lineno, 
         'message':e.message, 
    ...
    def jinja_template_filters(self):
        ...
        for alias in self.setting('filters'):
            ...
            for k, v in methods.items():
                if k not in template_plugin.setting('no-jinja-filter'):
