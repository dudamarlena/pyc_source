...
# [GCC 8.4.0]
# Embedded file name: /Users/ana/dev/dexy-clean/dexy/filters/templating_plugins.py
# Compiled at: 2020-03-18 15:12:33
# Size of source mod 2**32: 23043 bytes
...
import calendar, dexy.commands, dexy.commands.cite, dexy.data, dexy.exceptions, dexy.plugin, inflection, inspect, jinja2, json, markdown, operator, os, pygments, pygments.formatters, random, re, time, uuid
...
class Random(TemplatePlugin):
    ...
    def run(self):
        return {'random':(
          'The Python random module.', random), 
         'shuffle':(
          'Random shuffle not in place.', self.shuffle)}
...
class PrettyPrintHtml(TemplatePlugin):
    ...
    aliases = ['bs4']
    _settings = {'no-jinja-filter': ['BeautifulSoup']}
    ...
    def run(self):
        return {'prettify_html':(
          'Pretty-print HTML using BeautifulSoup', self.prettify_html), 
         'BeautifulSoup':(
          'The BeautifulSoup module.', BeautifulSoup)}
...
class LoadYaml(TemplatePlugin):
    ...
    def run(self):
        return {'load_yaml': ('Safely load YAML from a file.', self.load_yaml)}
...
class ParseYaml(TemplatePlugin):
    ...
    def run(self):
        return {'parse_yaml': ('Safely load YAML from text.', self.parse_yaml)}
...
class Debug(TemplatePlugin):
    ...
    def run(self):
        return {'debug':(
          'A debugging method - prints content to command line stdout.', self.debug), 
         'throw':(
          'A debugging utility which raises exception with argument.', self.throw), 
         'raise':(
          'Alias for `throw`.', self.throw)}
...
class Inflection(TemplatePlugin):
    ...
    aliases = ['inflection']
    _settings = {'methods': ('Methods of the inflection module to expose.',
                 [
...
class PrettyPrint(TemplatePlugin):
    ...
    def run(self):
        import pprint
        return {'pprint':(
          'Pretty prints Python objects.', pprint.pformat), 
         'pformat':(
          'Pretty prints Python objects.', pprint.pformat)}
...
class PrettyPrintJson(TemplatePlugin):
    ...
    def run(self):
        return {'ppjson': ('Pretty prints valid JSON.', self.ppjson)}
...
class ReplaceJinjaFilters(TemplatePlugin):
    ...
    def run(self):
        return {'indent': ("Jinja's indent function.", self.do_indent)}
...
class Assertions(TemplatePlugin):
    ...
    def do_assert_does_not_contain(self, doc, shouldnt_contain):
        ...
        assert shouldnt_contain not in str(doc), msg
        return self.decorate_response(doc)
...
class Head(TemplatePlugin):
    ...
    def run(self):
        return {'head': ('Returns the first n lines of input string.', self.head)}
...
class Tail(TemplatePlugin):
    ...
    def run(self):
        return {'tail': ('Returns the last n lines of inptu string.', self.tail)}
...
class PythonDatetime(TemplatePlugin):
    ...
    def run(self):
        ...
        return {'datetime':(
          'The Python datetime module.', datetime), 
         'calendar':(
          'A Calendar instance from Python calendar module.', calendar), 
         'caldates':(
          'List of calendar dates in current month.', caldates), 
         'cal':(
          'Shortcut for `calendar`.', calendar), 
         'today':(
          'Result of datetime.today().', today), 
         'month':(
          'Current month.', month), 
         'year':(
          'Current year.', year)}
...
class PygmentsHighlight(TemplatePlugin):
    ...
    def run(self):
        return {'highlight':(
          'Pygments syntax highlighter.', self.highlight), 
         'pygmentize':(
          'Pygments syntax highlighter.', self.highlight)}
    ...
    def run(self):
        ...
        for k, v in variables.items():
            if isinstance(v, tuple):
                ...
                formatted_variables[k] = (
                 'User-provided variable.', v)
...
class Inputs(TemplatePlugin):
    ...
    def run(self):
        ...
        if hasattr(self.filter_instance, 'output_data'):
            ...
            output_data = None
        return {'a':(
          "Another way to reference 'd'. Deprecated.", d), 
         'args':(
          'The document args.', self.filter_instance.doc.args), 
         'd':(
          "The 'd' object.", d), 
         'f':(
          'The filter instance for this document.', self.filter_instance), 
         's':(
          'The data instance for this document.', output_data), 
         'w':(
          'The wrapper for the dexy run.', self.filter_instance.doc.wrapper)}
...
class D(UserDict):
    ...
    def key_or_name_index(self, ref):
        ...
        if ref in self._input_doc_names:
