...
# [GCC 8.4.0]
# Embedded file name: /Users/ana/dev/dexy-clean/dexy/filters/id.py
# Compiled at: 2020-03-18 15:12:33
# Size of source mod 2**32: 15770 bytes
...
tokens = ('AMP', 'AT', 'CODE', 'COLONS', 'DBLQUOTE', 'END', 'IDIOCLOSE', 'IDIOOPEN',
          'EXP', 'IDIO', 'NEWLINE', 'SGLQUOTE', 'WHITESPACE', 'WORD')
states = (('idiostart', 'exclusive'), ('idio', 'exclusive'))
...
class Id(PygmentsFilter):
    ...
    _settings = {'examples':[
     ...
     'skip-extensions':('Because |idio gets applied to *.*, need to make it easy to skip non-textual files.',
 '.odt'), 
     ...
     'ply-write-tables':('Whether to generate parser table files (which will be stored in ply-outputdir and named ply-tabmodule).',
 1), 
     'ply-outputdir':("Location relative to where you run dexy in which ply will store table files. Defaults to dexy's log directory.",
 None), 
     'ply-parsetab':('Name of parser tabfile (.py extension will be added) to be stored in ply-outputdir if ply-write-tables set to 1.',
 'id_parsetab'), 
     'ply-lextab':('Name of lexer tabfile (.py extension will be added) to be stored in ply-outputdir if ply-optimize is set to 1.',
 'id_lextab'), 
...
def start_new_section(lexer, position, lineno, new_level, name=None):
    if name:
        if lexer.remove_leading and len(lexer.sections) == 1 and current_section_empty(lexer):
            lexer.sections = []
    else:
    ...
    lexer.sections.append({'name':name.rstrip(), 
     'position':position, 
     'lineno':lineno, 
     'contents':'', 
     'level':lexer.level})
