...
class PythonTest(PythonIntrospection):
    ...
    aliases = ['pytest']
    _settings = {'run-tests':('Whether to run tests or just return test source code.', True), 
     'test-passed-when-not-run':('Value which should be used for test_passed when tests are not run.', True), 
     'load-tests-mode':("'dir' to find tests by location relative to package dir, 'name' if tests are in their own module.",
 'dir'), 
     'path-to-tests-dir':('Path from package dir to tests dir.', '../tests'), 
     'chdir':('Path from package dir to chdir to.', None), 
     'nose-argv':(
    ...
    def run_test(self, test):
        ...
        config = nose.config.Config(logStream=noselogs)
