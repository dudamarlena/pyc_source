...
# [GCC 8.4.0]
# Embedded file name: /Users/ana/dev/dexy-clean/dexy/filters/pyg.py
# Compiled at: 2020-03-18 15:12:33
# Size of source mod 2**32: 11885 bytes
...
import dexy.commands, dexy.exceptions, posixpath, pygments.lexers.web
pygments_lexer_cache = {}
file_ext_to_lexer_alias_cache = {'.pycon':'pycon', 
 '.rbcon':'rbcon', 
 '.Rd':'latex', 
 '.svg':'xml', 
 '.jinja':'jinja'}
...
class SyntaxHighlightRstFilter(DexyFilter):
    ...
    aliases = ['pyg4rst']
    _settings = {'n':('Number of chars to indent.', 2), 
     'data-type':'sectioned'}
...
class SyntaxHighlightAsciidoctor(DexyFilter):
    ...
    aliases = ['asciisyn']
    _settings = {'lexer':("Specify lexer if can't be detected fro mfilename.", None), 
     'data-type':'sectioned'}
...
class PygmentsFilter(DexyFilter):
    ...
    MARKUP_OUTPUT_EXTENSIONS = ['.html', '.tex', '.svg', '.txt']
    LEXER_ERR_MSG = "Pygments doesn't know how to syntax highlight files like '%s' (for '%s').    You might need to specify the lexer manually."
    _settings = {'examples':[
      'pygments', 'pygments-image', 'pygments-stylesheets'], 
     'input-extensions':[
      '.*'], 
 ...
 True), 
     ...
     'noclasses':('If set to true, token <span> tags will not use CSS classes, but inline styles.',
 None), 
     'full':("Pygments formatter option: output a 'full' document\n                including header/footer tags.",
 None), 
 ...
 None), 
    ...
    def lexer_alias(self, ext):
        if self.setting('lexer'):
            ...
            if (ext == '.Makefile' or ext) == '':
                if 'Makefile' in self.input_data.name:
                    return 'makefile'
            try:
    ...
    def create_lexer_instance(self):
        ...
        if lexer_alias in pygments_lexer_cache:
            if not lexer_args:
    ...
    def create_formatter_instance(self):
        if self.setting('line-numbers'):
            if not self.setting('linenos'):
                self.update_settings({'linenos': self.setting('line-numbers')})
        formatter_args = self.constructor_args('formatter', {'lineanchors': self.output_data.web_safe_document_key()})
