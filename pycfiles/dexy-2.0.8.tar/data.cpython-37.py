...
class Data(dexy.plugin.Plugin, metaclass=dexy.plugin.PluginMeta):
    ...
    state_transitions = ((None, 'new'), ('new', 'ready'), ('ready', 'ready'))
    ...
    def args_to_data_init(self):
        ...
        """
        return (
         self.alias, self.key, self.ext, self.storage_key, self.setting_values())
    ...
    def output_name(self):
        ...
        def relativize(path):
            if output_root == '.':
                return path
            if os.path.abspath(output_root) in os.path.abspath(path):
...
class Generic(Data):
    ...
    def from_json(self):
        ...
        if self._data and not isinstance(self._data, str):
...
class Sectioned(Data):
    ...
    aliases = ['sectioned']
    _settings = {'storage-type': 'jsonsectioned'}
...
class KeyValue(Data):
    ...
    aliases = ['keyvalue']
    _settings = {'storage-type': 'sqlite3'}
