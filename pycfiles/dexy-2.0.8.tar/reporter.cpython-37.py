...
# [GCC 8.4.0]
# Embedded file name: /Users/ana/dev/dexy-clean/dexy/reporter.py
# Compiled at: 2020-03-18 15:12:33
# Size of source mod 2**32: 7194 bytes
from dexy.utils import file_exists
import dexy.plugin, os, shutil, sys

class Reporter(dexy.plugin.Plugin, metaclass=dexy.plugin.PluginMeta):
    __doc__ = '\n    Base class for types of reporter.\n    '
    _settings = {'default':('Whether to run this report by default. Should be\n            False for reports with side effects.',
 True), 
     'dir':('Directory where report output will be written.', None), 
     'filename':('Name of report file to generate (used when the\n                report consists of just a single file).',
 None), 
     'in-cache-dir':('Whether to create report dir in the cache\n                directory (instead of project root).',
 False), 
     'no-delete':(
      'List of file names not to delete when resetting\n                report dir (only effective if report dir is cleaned\n                element-wise).', ['.git', '.nojekyll']), 
     'plugins':(
      'List of template plugin aliases which should be\n                included in jinja environment.',
      [
       'debug', 'inflection', 'builtins', 'operator', 'datetime',
       'pprint', 'pygments', 'markdown']), 
     'run-for-wrapper-states':(
      'Wrapper states for which report is\n                valid.', ['ran']), 
     'readme-filename':('Name of README file, or None to omit.', 'README.md'), 
     'readme-contents':('Contents to be written to README file.', 'This directory was generated by the %(alias)s dexy reporter and may be deleted without notice.\n'), 
 ...
 '.dexy-generated')}
    ...
    def remove_reports_dir(self, wrapper, keep_empty_dir=False):
        ...
        if file_exists(self.report_dir()):
            msg = file_exists(self.safety_filepath()) or "Please remove directory %s, Dexy wants to write a report\n            here but there's already a file or directory in this location."
            ...
            if file_exists(self.report_dir()):
                if keep_empty_dir:
                    for f in os.listdir(self.report_dir()):
                        if f not in self.setting('no-delete'):
