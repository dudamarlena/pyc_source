...
# [GCC 8.4.0]
# Embedded file name: /Users/ana/dev/dexy-clean/dexy/filter.py
...
import dexy.doc, dexy.exceptions, dexy.plugin, dexy.utils, os, posixpath
...
class Filter(dexy.plugin.Plugin, metaclass=dexy.plugin.PluginMeta):
    ...
    nodoc_settings = [
     'help', 'nodoc']
    _settings = {'added-in-version':('Dexy version when this filter was first available.', ''), 
     ...
     'input-extensions':(
      ...
      [
       '.*']), 
     'keep-originals':('Whether, if additional-doc-filters are specified, the\n                original unmodified docs should also be added.',
 False), 
     ...
     'output':("Whether to output results of this filter by default by\n                reporters such as 'output' or 'website'.",
 False), 
     ...
     'workspace-exclude-filters':(
      ...
      [
       'pyg']), 
     'override-workspace-exclude-filters':('If True, document will be populated to other workspaces\n                ignoring workspace-exclude-filters.',
 False), 
     'workspace-includes':('If set to a list of filenames or extensions, only these will\n                be populated to working dir.',
 None)}
    ...
    def setup(self, key, storage_key, prev_filter, next_filter, custom_settings):
        ...
        settings.update({'canonical-name':self.calculate_canonical_name(), 
         'canonical-output':self.is_canonical_output()})
        self.output_data = dexy.data.Data.create_instance(self.data_class_alias(self.ext), self.key, self.ext, self.storage_key, settings, self.doc.wrapper)

    def is_canonical_output(self):
        if self.input_data.setting('canonical-output') == True:
            return True
        if self.input_data.setting('canonical-output') == False:
            return False
        if self.setting('output'):
            return True
        return
    ...
    def set_extension(self):
        ...
        if '.*' in next_filter_accepts:
            ...
            if ext_from_map:
                if ext_from_map not in next_filter_accepts:
    ...
    def add_doc(self, doc_name, doc_contents=None, doc_args=None):
        ...
        if posixpath.sep not in doc_name:
            ...
            if isinstance(additional_doc_settings, list):
                ...
                if isinstance(additional_doc_settings, dict):
 ...
 self.doc.wrapper, [], contents=contents, **args)
            ...
            if isinstance(additional_doc_filters, str):
                ...
                if isinstance(additional_doc_filters, list):
    ...
    def include_input_in_workspace(self, inpt):
        ...
        if workspace_includes is not None:
            if inpt.ext in workspace_includes:
                ...
                return True
            if inpt.output_data().basename() in workspace_includes:
            ...
            if not inpt.filters:
                ...
                return True
            if inpt.filters[(-1)].setting('override-workspace-exclude-filters'):
            ...
            if workspace_exclude_filters is None:
                ...
                return True
            if any((a in workspace_exclude_filters for a in inpt.filter_aliases)):
    ...
    def populate_workspace(self):
        ...
        for i, inpt in enumerate(input_docs):
            if not self.include_input_in_workspace(inpt):
                ...
                if parent_dir not in already_created_dirs:
                    try:
    ...
    def resolve_conflict(self, doc, conflict_docs):
        ...
        """
        conflict_docs = [d for d in conflict_docs if not 'pyg' in d.key if not 'idio' in d.key]
        conflict_docs.sort()
...
class AliasFilter(DexyFilter):
    ...
    aliases = ['-']
    _settings = {'preserve-prior-data-class': True}
...
def filter_aliases_by_tag():
    ...
    return [(tag,
     [(filter_instance.alias, filter_instance.setting('help')) for filter_instance in sorted((tags_filters[tag]), key=(attrgetter('alias')))]) for tag in tags]