...
# [GCC 8.4.0]
# Embedded file name: /Users/ana/dev/dexy-clean/dexy/reporters/website/classes.py
# Compiled at: 2020-03-18 15:12:33
# Size of source mod 2**32: 19231 bytes
...
import dexy.data, dexy.exceptions, dexy.filters.templating_plugins, inspect, jinja2, os, posixpath, urllib
...
class Website(Output):
 ...
 None)}}
    ...
    def should_process(self, doc):
        if doc.key_with_class() not in self.wrapper.batch.docs:
            return False
        else:
            if doc.state not in ('ran', 'consolidated'):
                ...
                if not hasattr(doc, 'output_data'):
                    return False
                return doc.output_data().output_name() or False
            msg = doc.output_data().is_canonical_output() or 'skipping %s - not canonical'
    ...
    def process_doc(self, doc):
        ...
        if output_ext == '.html':
            ...
            if isinstance(doc.output_data(), dexy.data.Sectioned):
    ...
    def process_html(self, doc):
        if doc.setting('ws-template') == False:
            ...
            if self.detect_html_header(doc):
                doc.setting('ws-template') or self.log_debug('  found html tag in output of %s' % doc.key)
                self.write_canonical_data(doc)
            else:
                self.apply_and_render_template(doc)
    ...
    def detect_html_header(self, doc):
        ...
        return any((html_fragment in str(doc.output_data()) for html_fragment in fragments))
    ...
    def template_file_and_path(self, doc):
        ...
        if ws_template:
            template_file = isinstance(ws_template, bool) or ws_template
        else:
    ...
    def jinja_environment(self, template_path, additional_args=None):
        ...
        dirs = [
         '.', os.path.dirname(__file__), os.path.dirname(template_path)]
    ...
    def template_environment(self, doc, template_path):
        ...
        raw_env_data.update(self.website_specific_template_environment(doc.output_data(), {'template_source': ('The directory containing the template file used.',
                             template_path)}))
    ...
    def help(self, data):
        ...
        navobj = env['navobj'][1]
        root = navobj.nodes['/']
        members = [(name, obj) for name, obj in inspect.getmembers(root) if not name.startswith('__')]
    ...
    def website_specific_template_environment(self, data, initial_args=None):
        ...
        env_data.update({'link':(
          'Function to create link to other page.',
          partial(self.link, data)), 
         'section':(
          'Function to create link to section on any page.',
          partial(self.section, data)), 
         'navigation':(
          "DEPRECATED. 'navigation' object.", {}), 
         'nav':(
          "The node for the current document's directory.",
          self._navobj.nodes[('/%s' % current_dir)]), 
         'root':(
          'The root node of the navigation tree.',
          self._navobj.nodes['/']), 
         'navobj':(
          "DEPRECATED. Same as 'navtree'.",
          self._navobj), 
         'navtree':(
          'The complete navigation tree for the website.',
          self._navobj), 
         'page_title':(
         ...
         'current_dir':(
          ...
          current_dir), 
         's':(
          "'%s' type data object representing the current doc." % data.alias,
          data), 
         'd':(
          "'%s' type data object representing the current doc." % data.alias,
          data), 
         'source':(
          'Output name of current doc.',
          data.output_name()), 
         'wrapper':(
    ...
    def section(self, data, section_name=None, url_base=None, link_text=None):
        ...
        if not matching_nodes:
            ...
            if len(matching_nodes) > 1:
                ...
                anchor = section['id']
                link_text = link_text or section_name
...
class Navigation(object):
    ...
    def populate_lookup_table(self, batch):
        for data in batch:
            ...
            if parent_dir not in self.lookup_table:
