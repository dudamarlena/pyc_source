...
# [GCC 8.4.0]
# Embedded file name: /Users/ana/dev/dexy-clean/dexy/filters/git.py
...
def generate_commit_info(commit):
    ...
    for i, patch in enumerate(diff):
        patches.append({'old-file-path':patch.old_file_path, 
         'new-file-path':patch.new_file_path, 
         'hunks':[{'old-start':hunk.old_start,  'old-lines':hunk.old_lines,  'new-start':hunk.new_start,  'new-lines':hunk.new_lines,  'lines':hunk.lines} for hunk in patch.hunks]})

    commit_info = {'author-name':commit.author.name, 
...
class GitBase(DexyFilter):
    ...
    aliases = []
    _settings = {'reference':('The reference to use.', None), 
...
class GitLog(GitBase):
    ...
    aliases = ['gitlog']
    _settings = {'output-extensions': ['.txt']}
