...
# [GCC 8.4.0]
# Embedded file name: /Users/ana/dev/dexy-clean/dexy/filters/xxml.py
...
class XmlSectionFilter(DexyFilter):
    ...
    _settings = {'input-extensions':[
     ...
     'unique-attributes':(
      'Elements to be added if they have this attribute, to be treated as globally unique.', ['id']), 
     'qualified-attributes':(
      'Elements to be added if they have this attribute, to be qualified by element type.', ['name']), 
     'data-type':'keyvalue', 
     'output-extensions':[
      '.json', '.sqlite3']}
    ...
    def process(self):
        if not self.output_data.state == 'ready':
            raise AssertionError
        elif self.setting('pygments'):
        ...
        elif self.input_data.ext in ('.xml', '.txt'):
            parser = etree.XMLParser()
        else:
            if self.input_data.ext == '.html':
