...
# [GCC 8.4.0]
# Embedded file name: /Users/ana/dev/dexy-clean/dexy/parser.py
# Compiled at: 2020-03-18 15:12:33
# Size of source mod 2**32: 6457 bytes
import copy, dexy.doc, dexy.exceptions, dexy.plugin, posixpath
...
class AbstractSyntaxTree:
    ...
    def add_node(self, node_key, **kwargs):
        ...
        if node_key not in self.lookup_table:
            self.lookup_table[node_key] = {}
        self.lookup_table[node_key].update(kwargs)
        if 'inputs' not in self.lookup_table[node_key]:
    ...
    def walk(self):
        ...
        def create_dexy_node(key, *inputs, **kwargs):
            if key not in self.wrapper.nodes:
