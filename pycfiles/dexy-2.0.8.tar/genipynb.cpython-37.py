...
# [GCC 8.4.0]
# Embedded file name: /Users/ana/dev/dexy-clean/dexy/filters/genipynb.py
...
class MarkdownJupyter(MarkdownSections):
    ...
    def process_code(self, source, language, metadata=None):
        ...
        return {'cell_type':'code',  'collapsed':self.setting('collapsed'), 
         'metadata':metadata or {}, 
         'language':language, 
         'input':source, 
         'outputs':[],  'prompt_number':None}

    def process_heading(self, level, text, metadata=None):
        return {'cell_type':'heading', 
         'level':level, 
         'metadata':metadata or {}, 
         'source':[
          text]}
    ...
    def process_text(self, input_text):
        ...
        notebook = {'nbformat':self.setting('nbformat'), 
         'nbformat_minor':self.setting('nbformat-minor'), 
         'metadata':{'name': workbook_name}, 
         'worksheets':[
          {'cells': cells}]}
        if self.setting('pprint'):
