...
# [GCC 8.4.0]
# Embedded file name: /Users/ana/dev/dexy-clean/dexy/storage.py
...
import dexy.exceptions, dexy.plugin, os, shutil, sqlite3
...
class GenericStorage(Storage):
    ...
    def write_data(self, data, filepath=None):
        ...
        if os.path.exists(self.this_data_file()):
            filepath == self.this_data_file() or shutil.copyfile(self.this_data_file(), filepath)
        else:
...
class Sqlite3KeyValueStorage(GenericStorage):
    ...
    def persist(self):
        if self.connected_to == 'existing' and not os.path.exists(self.data_file(read=False)):
            raise AssertionError
        else:
            if self.connected_to == 'working':
