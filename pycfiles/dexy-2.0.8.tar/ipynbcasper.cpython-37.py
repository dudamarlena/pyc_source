...
# [GCC 8.4.0]
# Embedded file name: /Users/ana/dev/dexy-clean/dexy/filters/ipynbcasper.py
...
class IPythonCasper(SubprocessFilter):
    ...
    def configure_casper_script(self, wd, port, cellmetas):
        ...
        if not os.path.exists(scriptfile):
            ...
            with open(scriptfile, 'r') as (f):
                js = f.read()
        args = {'width':self.setting('width'),  'height':self.setting('height'), 
         'port':port, 
    ...
    def launch_ipython(self, env):
        ...
        proc = subprocess.Popen(command, shell=False, cwd=wd,
          stdin=None,
    ...
    def parse_additional_ipython_args(self):
        ...
        if raw_ipython_args:
            if isinstance(raw_ipython_args, str):
                ...
                if isinstance(raw_ipython_args, list):
