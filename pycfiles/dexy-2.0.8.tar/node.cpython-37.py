...
# [GCC 8.4.0]
# Embedded file name: /Users/ana/dev/dexy-clean/dexy/node.py
# Compiled at: 2020-03-18 15:12:33
# Size of source mod 2**32: 11206 bytes
from dexy.utils import md5_hash
from dexy.utils import os_to_posix
import dexy.doc, dexy.plugin, fnmatch, json, re

class Node(dexy.plugin.Plugin, metaclass=dexy.plugin.PluginMeta):
    __doc__ = '\n    base class for Nodes\n    '
    _settings = {}
    aliases = ['node']
    state_transitions = (('new', 'cached'), ('cached', 'consolidated'), ('new', 'uncached'),
                         ('uncached', 'running'), ('running', 'ran'))
 ...
 'contents']):
    ...
    def input_nodes(self, with_parent_inputs=False):
        ...
        if with_parent_inputs:
            if hasattr(self, 'parent'):
                if self not in self.parent.inputs:
    ...
    def setup(self):
        ...
        for filepath, fileinfo in self.wrapper.filemap.items():
            if fnmatch.fnmatch(filepath, file_pattern):
                ...
                if except_p:
                    if re.search(except_p, filepath):
