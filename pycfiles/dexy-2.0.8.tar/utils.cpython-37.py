...
# [GCC 8.4.0]
# Embedded file name: /Users/ana/dev/dexy-clean/dexy/utils.py
# Compiled at: 2020-03-18 15:12:33
# Size of source mod 2**32: 9682 bytes
import dexy.exceptions, hashlib, inspect, json, logging, os, platform, posixpath, re, shutil, tempfile, time, yaml
...
def transition(obj, new_state):
    ...
    attempted_transition = (
     obj.state, new_state)
    if attempted_transition not in obj.__class__.state_transitions:
        msg = '%s -> %s'
...
def value_for_hyphenated_or_underscored_arg(arg_dict, arg_name_hyphen, default=None):
    if '-' not in arg_name_hyphen:
        if '_' in arg_name_hyphen:
...
def indent(s, spaces=4):
    return '\n'.join(('%s%s' % (' ' * spaces, line) for line in s.splitlines()))