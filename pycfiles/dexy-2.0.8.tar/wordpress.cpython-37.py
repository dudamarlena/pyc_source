...
# [GCC 8.4.0]
# Embedded file name: /Users/ana/dev/dexy-clean/dexy/filters/wordpress.py
# Compiled at: 2020-03-18 15:12:33
# Size of source mod 2**32: 6739 bytes
from dexy.filters.api import ApiFilter
import dexy.exceptions, json, xmlrpc.client
try:
    import mimetypes
    wp_aliases = [
     'wp', 'wordpress']
...
class WordPressFilter(ApiFilter):
    ...
    aliases = wp_aliases
    _settings = {'blog-id':('The wordpress blog id.', 0), 
    ...
    def upload_page_content(self):
        ...
        if post_id:
            ...
            self.api().metaWeblog.editPost(post_id, self.read_param('username'), self.read_param('password'), document_config, publish)
        else:
            self.log_debug('Making newPost API call.')
            post_id = self.api().metaWeblog.newPost(self.setting('blog-id'), self.read_param('username'), self.read_param('password'), document_config, publish)
            document_config['postid'] = post_id
        self.log_debug('Making getPost API call.')
        post_info = self.api().metaWeblog.getPost(post_id, self.read_param('username'), self.read_param('password'))
    ...
    def upload_image_content(self):
        with open(self.input_data.storage.data_file(), 'rb') as (f):
            ...
            upload_file = {'name':self.work_input_filename(), 
             'type':mimetypes.types_map[self.prev_ext], 
             'bits':image_base_64, 
             'overwrite':'true'}
            upload_result = self.api().wp.uploadFile(self.setting('blog-id'), self.read_param('username'), self.read_param('password'), upload_file)
