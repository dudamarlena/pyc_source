...
# [GCC 8.4.0]
# Embedded file name: /Users/dominicmonn/Documents/Private/socialize/socialize/services/group_service.py
...
class GroupService(Service):
    ...
    def join_group(self, groupname, passphrase=None):
        ...
        self.check_reponse(r, success='Successfully joined the group ' + groupname, error="Couldn't join group " + groupname + '. Please check if the groupname is right (Yes, we care about uppercases).')

    def leave_group(self, groupname):
        r = self.post('groups/' + groupname + '/leave/', data={})
        self.check_reponse(r, success='Successfully left the group ' + groupname, error="Couldn't leave group " + groupname + '. Please check if the groupname is right (Yes, we care about uppercases).')

    def delete_group(self, groupname):
        r = self.post('groups/' + groupname + '/delete/', data={})
        self.check_reponse(r, success='Successfully deleted the group ' + groupname, error="Couldn't delete group " + groupname + '. Please check if the groupname is right (Yes, we care about uppercases).')
    ...
    def create_group(self, groupname, description=None, passphrase=None):
        r = self.post('groups/' + groupname + '/', data={'description': str(description), 'passphrase': str(passphrase)})
