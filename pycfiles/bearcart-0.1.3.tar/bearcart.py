...
# [GCC 8.4.0]
# Embedded file name: /Users/Story/bearcart/bearcart/bearcart.py
# Compiled at: 2013-05-12 12:36:06
"""
Rickshaw
-------
...
class Chart(object):
    ...
    def __init__(self, data=None, width=750, height=400, plt_type='line', colors=None, x_time=True, y_zero=False, **kwargs):
        ...
        """
        self.defaults = {'x_axis': True, 'y_axis': True, 'hover': True, 'legend': True}
        self.env = Environment(loader=PackageLoader('bearcart', 'templates'))
        if colors:
            self.colors = {key:("'{0}'").format(value) for key, value in colors.iteritems()}
        else:
            self.colors = None
            ...
            if val:
                if not self.x_axis_time:
                    if att == 'x_axis':
                        ...
                        render_vars = {'x_hover': 'xFormatter: function(x){return Math.floor(x / 10) * 10}'}
    ...
    def transform_data(self, data):
        ...
        def type_check(value):
            ...
            if pd.isnull(value):
                ...
                if isinstance(value, pd.tslib.Timestamp) or isinstance(value, pd.Period):
        ...
        objectify = lambda dat: [ {'x': type_check(x), 'y': type_check(y)} for x, y in dat.iteritems()
        ]
        self.raw_data = data
        if isinstance(data, pd.Series):
            ...
            self.json_data = [ {'name': x[0], 'data': objectify(x[1])} for x in data.iteritems() ]
    ...
    def _build_graph(self):
        ...
        for index, dataset in enumerate(self.json_data):
            ...
            template_vars.append({'name': str(dataset['name']), 'color': self.colors[dataset['name']], 
               'data': ('json[{0}].data').format(index)})

        variables = {'dataset': template_vars, 'width': self.width, 'height': self.height, 
    ...
    def create_chart(self, html_path='index.html', data_path='data.json', js_path=None, css_path=None):
