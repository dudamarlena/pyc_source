...
# Embedded file name: build/bdist.linux-i686/egg/collective/contentgenerator/UserGenerator.py
...
class UserSetup:
    ...
    def __init__(self):
        self.out = []
        self.forenames = [
         'Alex', 'Ben', 'Cath', 'Daniel', 'Eli', 'Freddy', 'Gwen', 'Herbie', 'Ivan', 'Jay', 'Kiash', 'Len', 'Max', 'Nobby', 'Oscar', 'Phil', 'Quentin', 'Rachel', 'Sophie', 'Toby', 'Una', 'Victor', 'Wendy', 'Xen', 'Yvonne', 'Zachary']
        self.surnames = [
         'Anderson', 'Barnes', 'Crewe', 'Douglas', 'Emmins', 'Farquar', 'Gaskill', 'Hughes', 'Illiad', 'Jones', 'Kash', 'Lyons', 'Moon', 'Norman', 'Order', 'Paul', 'Quail', 'Robert', 'Simon', 'Timms', 'Underhill', 'Vernon', 'Wally', 'Xavier', 'Zeb']
        self.groups = [
         'ant', 'bee', 'cat', 'dog', 'elephant', 'fish', 'goat', 'hippo', 'insect', 'jackdaw', 'kangaroo', 'lion', 'monkey', 'newt', 'ostrich', 'penguin', 'quail', 'rabbit', 'snake', 'tiger', 'uakari', 'vole', 'whale', 'xiphias', 'yak', 'zebra']
    ...
    def customize(self, portal, context, settings={}, logging='log'):
        ...
        self.createGroups(portal, settings.get('memberships', 10), settings.get('local', False))
        if logging == 'debug':
            error = ''
                    ...
                    if item.isPrincipiaFolderish:
                        for subitem in item.objectValues():
                            if subitem.isPrincipiaFolderish:
                                mem_tool.setLocalRoles(obj=subitem, member_ids=(random.choice(self.groupid),), member_role=random.choice(self.role))
        ...
        for fname in self.forenames:
            ...
            for mname in self.forenames:
                ...
                for sname in self.surnames:
                    ...
                    self.userid.append(fname[0].lower() + mname[0].lower() + sname.lower())
                    self.ucount += 1
    ...
    def createUsers(self, portal):
        ...
        for i in range(self.ucount):
            reg_tool.addMember(self.userid[i], 'testpw', properties={'email': self.userid[i] + '@test.plone.org', 'fullname': self.fullname[i], 'username': self.userid[i]})
