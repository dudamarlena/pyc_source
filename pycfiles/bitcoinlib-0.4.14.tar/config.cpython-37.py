...
# [GCC 8.4.0]
# Embedded file name: /home/lennart/code/bitcoinlib/bitcoinlib/config/config.py
# Compiled at: 2020-03-19 14:58:12
# Size of source mod 2**32: 11700 bytes
import os, sys, locale, platform, datetime
...
SCRIPT_TYPES_LOCKING = {'p2pkh':[
  'OP_DUP', 'OP_HASH160', 'hash-20', 'OP_EQUALVERIFY', 'OP_CHECKSIG'], 
 'p2sh':[
  'OP_HASH160', 'hash-20', 'OP_EQUAL'], 
 'p2wpkh':[
  'OP_0', 'hash-20'], 
 'p2wsh':[
  'OP_0', 'hash-32'], 
 'multisig':[
  'op_m', 'multisig', 'op_n', 'OP_CHECKMULTISIG'], 
 'p2pk':[
  'public_key', 'OP_CHECKSIG'], 
 'nulldata':[
  'OP_RETURN', 'return_data']}
SCRIPT_TYPES_UNLOCKING = {'sig_pubkey':[
  'signature', 'SIGHASH_ALL', 'public_key'], 
 'p2sh_multisig':[
  'OP_0', 'multisig', 'redeemscript'], 
 'p2sh_p2wpkh':[
  'OP_0', 'OP_HASH160', 'redeemscript', 'OP_EQUAL'], 
 'p2sh_p2wsh':[
  'OP_0', 'push_size', 'redeemscript'], 
 'locktime_cltv':[
  'locktime_cltv', 'OP_CHECKLOCKTIMEVERIFY', 'OP_DROP'], 
 'locktime_csv':[
...
SEQUENCE_LOCKTIME_DISABLE_FLAG = 2147483648
SEQUENCE_LOCKTIME_TYPE_FLAG = 4194304
SEQUENCE_LOCKTIME_GRANULARITY = 9
SEQUENCE_LOCKTIME_MASK = 65535
SEQUENCE_ENABLE_LOCKTIME = 4294967294
SEQUENCE_REPLACE_BY_FEE = 4294967293
...
if os.name == 'nt' and locale.getpreferredencoding() != 'UTF-8':
    ...
    if locale.getpreferredencoding() != 'UTF-8':
        raise EnvironmentError("Locale is currently set to '%s'. This library needs the locale set to UTF-8 to function properly" % locale.getpreferredencoding())
    ...
    def read_config():
        global ALLOW_DATABASE_THREADS
        global BCL_CONFIG_FILE
        global BCL_DATABASE_DIR
        global BCL_DATA_DIR
        global BCL_INSTALL_DIR
        global BCL_LOG_FILE
        global DEFAULT_DATABASE
        global DEFAULT_DATABASE_CACHE
        global DEFAULT_LANGUAGE
        global DEFAULT_NETWORK
        global DEFAULT_WITNESS_TYPE
        global ENABLE_BITCOINLIB_LOGGING
        global LOGLEVEL
        global SERVICE_CACHING_ENABLED
        ...
        if not config_file_name:
            ...
            if not BCL_CONFIG_FILE.exists():
                ...
                default_databasefile_cache = config_get('locations', 'default_databasefile_cache', fallback='bitcoinlib_cache.sqlite')
                ...
                if full_db_test and full_db_test in (0, False, 'False', 'false', 'FALSE'):
                    UNITTESTS_FULL_DATABASE_TEST = False
            if not data:
                return False
            return True
