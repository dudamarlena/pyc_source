...
# Embedded file name: build/bdist.linux-x86_64/egg/cray/jobs.py
...
def get_jobs(job_prefix=None):
    ...
    return (j.lstrip(config.job_prefix() + '/') for j in cs3.list_subdirectories(config.bucket(), prefix))
...
def get_job_config(job):
    return cs3.load_json(config.bucket(), '{}/{}/config.json'.format(config.job_prefix(), job))
...
def build_job_archive(tempdir, config_path, ssh_dir, fix_windows_path, docker_image, environment):
    pwd = getcwd()
    if fix_windows_path:
        ...
        if environment:
            ...
            env_args = ''
    cmd = 'docker run -v {}:/build {} -v {}:/output {} --rm {}'.format(pwd, ssh_binding, tempdir, env_args, docker_image)
    exit_code = subprocess.call(cmd, shell=True)
    if exit_code != 0:
        if env_args:
            cmd = 'docker run -v {}:/build {} -v {}:/output {} --rm {}'.format(pwd, ssh_binding, tempdir, ' '.join((f"-e {ei.split('=')[0]}=..." for ei in environment)), docker_image)
        raise Exception("'{}' submission failed: exited with code '{}'".format(cmd, exit_code))
