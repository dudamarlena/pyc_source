...
# Embedded file name: /home/c24b/projets/crawtext/tldextract/tldextract.py
...
PUBLIC_SUFFIX_LIST_URLS = ('http://mxr.mozilla.org/mozilla-central/source/netwerk/dns/effective_tld_names.dat?raw=1',
...
class TLDExtract(object):
    ...
    def __init__(self, cache_file=CACHE_FILE, suffix_list_url=PUBLIC_SUFFIX_LIST_URLS, fetch=True, fallback_to_snapshot=True, include_psl_private_domains=False):
        """
        ...
        """
        if not fetch:
            LOG.warning("The 'fetch' argument is deprecated. Instead of specifying fetch, you should specify suffix_list_url. The equivalent of fetch=False would be suffix_list_url=None.")
        ...
        if not (self.suffix_list_urls or self.cache_file or self.fallback_to_snapshot):
            raise ValueError('The arguments you have provided disable all ways for tldextract to obtain data. Please provide a suffix list data, a cache_file, or set `fallback_to_snapshot` to `True`.')
    ...
    def __call__(self, url):
        ...
        ExtractResult(subdomain='forums', domain='bbc', suffix='co.uk')
        """
        netloc = SCHEME_RE.sub('', url).partition('/')[0].partition('?')[0].partition('#')[0].split('@')[(-1)].partition(':')[0].rstrip('.')
    ...
    def _get_tld_extractor(self):
        ...
        if not tlds:
            if self.fallback_to_snapshot:
                ...
                raise Exception('tlds is empty, but fallback_to_snapshot is set to false. Cannot proceed without tlds.')
...
def get_tlds_from_raw_suffix_list_data(suffix_list_source, include_psl_private_domains=False):
    ...
    tld_finder = re.compile('^(?P<tld>[.*!]*\\w[\\S]*)', re.UNICODE | re.MULTILINE)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Parse hostname from a url or fqdn')
    parser.add_argument('--version', action='version', version='%(prog)s ' + distribution.version)
    parser.add_argument('input', metavar='fqdn|url', type=unicode, nargs='*', help='fqdn or url')
