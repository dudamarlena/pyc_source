...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\extasy\selenium\selenium_driver.py
...
class SeleniumDriver(object):
    ...
    def resolve_element_key(self, context, element_type, element_key, **kw):
        if not context:
            return element_key
        return SeleniumElementSelector.element(element_type, element_key, **kw)
    ...
    def click_element_at(self, element_selector, x, y):
        self.selenium.click_at(element_selector, '%s,%s' % (x, y))

    def double_click_element(self, element_selector):
        self.selenium.double_click(element_selector)
    ...
    def get_element_text(self, element_selector):
        ...
        element_selector = element_selector.replace('"', '\\"')
        ...
        try:
            ...
            raise ValueError('The tag for element selector %s is %s and EXTasy only supports the following tags: %s', (
    ...
    def get_element_id(self, element_selector):
        script = 'this.page().findElement("%s").id;'
        script_return = self.selenium.get_eval(script % element_selector)
        return script_return != 'null' and script_return or ''
    ...
    def get_table_rows(self, table_selector):
        ...
        for row_index in range(row_count):
            row = []
            cell_count = int(self.get_xpath_count(table_selector + '/tbody/tr[%d]/td' % (row_index + 1)))
    ...
    def __unicode__(self):
        return "SeleniumDriver at '%s:%s' using '%s' browser." % (extasy.settings.get('selenium-server', 'localhost'),
         extasy.settings.get('selenium-port', 4444),
         extasy.settings.browser_to_run)