...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\extasy\step_definitions\page.py
...
def see_title(context, title=None):
    ...
    assert actual_title == title, 'Page title is "%(actual_title)s" when it should be "%(expected_title)s"' % {'actual_title': actual_title, 
...
def contains_markup(context, markup=None):
    ...
    assert markup in page_html, 'Page should contain "%(markup)s", but it does\'t' % {'markup': markup}
...
def does_not_contain_markup(context, markup=None):
    page_html = extasy.selenium.getDriver().get_body_source()
    assert markup not in page_html, 'Page should not contain "%(markup)s", but it does' % {'markup': markup}
