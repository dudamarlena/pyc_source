...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\extasy\step_definitions\window.py
...
def _getxpath(title=None):
    if title:
        return "(//div[contains(concat(' ',@class, ' '),' x-window ')]/descendant::span[contains(concat(' ',@class, ' '),' x-window-header-text ') and .='%(title)s'])[last()]" % {'title': title}
    return "(//div[contains(concat(' ',@class, ' '),' x-window ')])[last()]"


def _get_close_button_xpath(title=None):
    if title:
        return "(//span[contains(concat(' ',@class, ' '),' x-window-header-text ') and .='%(title)s']/ancestor::div[contains(concat(' ',@class, ' '),' x-window ')])[last()]/descendant::div[contains(concat(' ',@class, ' '),' x-tool-close ')]" % {'title': title}
    return "(//div[contains(concat(' ',@class, ' '),' x-window ')])[last()]/descendant::div[contains(concat(' ',@class, ' '),' x-tool-close ')]"
