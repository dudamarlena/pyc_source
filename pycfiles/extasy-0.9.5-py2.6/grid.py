...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\extasy\step_definitions\grid.py
# Compiled at: 2010-11-20 06:10:24
import extasy, extasy.selenium, re
from extasy import *

def _getxpath(title=None):
    if title:
        return "(%(scope_xpath)s//div[contains(concat(' ',@class, ' '),' x-grid-panel ')]/descendant::span[contains(concat(' ',@class, ' '),' x-panel-header-text ') and .='%(title)s'])[last()]" % {'scope_xpath': extasy.scope.get_xpath(), 
           'title': title}
    return "(%(scope_xpath)s//div[contains(concat(' ',@class, ' '),' x-grid-panel ')])[last()]" % {'scope_xpath': extasy.scope.get_xpath()}


def _get_line_xpath(title=None, line=None):
    if title:
        return "((%(scope_xpath)s//span[contains(concat(' ',@class, ' '),' x-panel-header-text ') and .='%(title)s']/ancestor::div[contains(concat(' ',@class, ' '),' x-grid-panel ')])[last()])/descendant::div[contains(concat(' ',@class, ' '),' x-grid3-row ')][%(line)s]//div[@class=contains( concat( ' ', @class, ' '), ' x-grid3-cell-inner ') ][1]" % {'scope_xpath': extasy.scope.get_xpath(), 
           'title': title, 
           'line': line}
    return "((%(scope_xpath)s//div[contains(concat(' ',@class, ' '),' x-grid-panel ')])[last()])/descendant::div[contains(concat(' ',@class, ' '),' x-grid3-row ')][%(line)s]//div[@class=contains( concat( ' ', @class, ' '), ' x-grid3-cell-inner ') ][1]" % {'scope_xpath': extasy.scope.get_xpath(), 
       'line': line}


def _get_lines_xpath(title=None):
    if title:
        return "((%(scope_xpath)s//span[contains(concat(' ',@class, ' '),' x-panel-header-text ') and .='%(title)s']/ancestor::div[contains(concat(' ',@class, ' '),' x-grid-panel ')])[last()])/descendant::div[contains(concat(' ',@class, ' '),' x-grid3-row ')]" % {'scope_xpath': extasy.scope.get_xpath(), 
           'title': title}
    return "((%(scope_xpath)s//div[contains(concat(' ',@class, ' '),' x-grid-panel ')])[last()])/descendant::div[contains(concat(' ',@class, ' '),' x-grid3-row ')]" % {'scope_xpath': extasy.scope.get_xpath()}
...
def open_column_config_menu(context, title=None, column=None):
    ...
    col_xpath = "(%(grid_xpath)s)//div[ contains( concat( ' ', @class, ' ' ), ' x-grid3-hd-inner ' ) and .='%(column)s']" % {'grid_xpath': grid_xpath, 
       'column': column}
    col_config_xpath = "%(col_xpath)s//a[ contains( concat( ' ', @class, ' ' ), ' x-grid3-hd-btn ' ) ]" % {'col_xpath': col_xpath}
