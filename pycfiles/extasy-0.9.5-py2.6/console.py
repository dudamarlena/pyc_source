...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\extasy\console.py
# Compiled at: 2010-11-20 06:10:24
from finder import extasy_find_steps_modules, find_text_specs, find_before_all, find_before_each, find_after_all, find_after_each, extasy_find_hook_steps
import pyhistorian
...
if 'windows' in os.environ.get('OS', '').lower():
    ...
    def set_terminal_color(color):
        colors = [
         'black', 'blue', 'green', 'aqua', 'red', 'purple', 'yellow', 'white', 'gray', 'light blue', 'light green', 'light aqua', 'light red', 'light purple', 'light yellow']
...
def extasy_run_command(args, values):
    ...
    for (index, story) in enumerate(files):
        ...
        story_runner = StoryRunner(open(story).read(), sys.stdout, colored=colored, modules=steps_modules, language=values.language, before_all=before_all_methods, before_each=before_each_methods, after_all=after_all_methods, after_each=after_each_methods)
