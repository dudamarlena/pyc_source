...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\extasy\step_definitions\radio.py
...
def _getxpath(label, group_label=None):
    if group_label:
        return "(%(scope_xpath)s//label[contains(concat(' ',@class, ' '),' x-form-item-label ') and .='%(group_label)s']/../descendant::label[contains(concat(' ',@class, ' '),' x-form-cb-label ') and .='%(label)s']/../input[contains(concat(' ',@class, ' '),' x-form-radio ')])[last()]" % {'scope_xpath': extasy.scope.get_xpath(), 
           'label': label, 
           'group_label': group_label}
    return "(%(scope_xpath)s//label[contains(concat(' ',@class, ' '),' x-form-cb-label ') and .='%(label)s']/../input[contains(concat(' ',@class, ' '),' x-form-radio ')])[last()] | (%(scope_xpath)s//label[contains(concat(' ',@class, ' '),' x-form-item-label ') and .='%(label)s']/../descendant::input[contains(concat(' ',@class, ' '),' x-form-radio ')])[last()]" % {'scope_xpath': extasy.scope.get_xpath(), 
       'label': label}
