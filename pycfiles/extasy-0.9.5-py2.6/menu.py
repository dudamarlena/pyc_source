...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\extasy\step_definitions\menu.py
# Compiled at: 2010-11-20 06:10:24
import extasy, extasy.selenium
from extasy import *

def _getxpath(title):
    return "(//a[contains(concat(' ',@class, ' '),' x-menu-item ')]/span[.='%(title)s'])[last()]/.." % {'title': title}


def _get_menu_numeric_field_xpath(operator_text):
    operatorsDict = {'=': 'ux-rangemenu-eq', 
       '>': 'ux-rangemenu-gt', 
       '<': 'ux-rangemenu-lt'}
    return "//li[contains(concat(' ',@class, ' '),' x-menu-list-item ')]/img[contains(concat(' ',@class, ' '),' %(class_name)s ')]/../input" % {'class_name': operatorsDict[operator_text]}
