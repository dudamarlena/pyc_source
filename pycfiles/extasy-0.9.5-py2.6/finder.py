...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\extasy\finder.py
...
def _extasy_find_all_steps_modules(dirs, suffix='.py'):
    ...
    for dirname in dirs:
        ...
        modules = [ __import__(filename[:-3]) for filename in os.listdir(dirname) if filename.endswith(suffix)
                  ]
        all_modules += modules
        del sys.path[0]
...
def pycukes_find_hook_steps(name, dirname):
    sys.path.insert(0, dirname)
    modules = [ __import__(filename[:-3]) for filename in os.listdir(dirname) if filename == 'env.py'
              ]
    del sys.path[0]
    before_all_meths = []
