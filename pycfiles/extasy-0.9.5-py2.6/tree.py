...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\extasy\step_definitions\tree.py
# Compiled at: 2010-11-20 06:10:24
import extasy, extasy.selenium
from extasy import *

def _get_node_xpath(title):
    return "(%(scope_xpath)s//a[contains(concat(' ',@class, ' '),' x-tree-node-anchor ')]/span[.='%(title)s'])[last()]/.." % {'scope_xpath': extasy.scope.get_xpath(), 
       'title': title}


def _get_node_open_button_xpath(title):
    return "(%(scope_xpath)s//a[contains(concat(' ',@class, ' '),' x-tree-node-anchor ')]/span[.='%(title)s'])[last()]/ancestor::div[contains(concat(' ',@class, ' '),' x-tree-node-el ')]/descendant::img[contains(concat(' ',@class, ' '),' x-tree-elbow-plus ') or contains(concat(' ',@class, ' '),' x-tree-elbow-end-plus ')]" % {'scope_xpath': extasy.scope.get_xpath(), 
       'title': title}


def _get_node_close_button_xpath(title):
    return "(%(scope_xpath)s//a[contains(concat(' ',@class, ' '),' x-tree-node-anchor ')]/span[.='%(title)s'])[last()]/ancestor::div[contains(concat(' ',@class, ' '),' x-tree-node-el ')]/descendant::img[contains(concat(' ',@class, ' '),' x-tree-elbow-minus ') or contains(concat(' ',@class, ' '),' x-tree-elbow-end-minus ')]" % {'scope_xpath': extasy.scope.get_xpath(), 
       'title': title}
