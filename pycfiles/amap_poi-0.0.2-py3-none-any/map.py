...
# Embedded file name: C:\Users\dsx\Code\amaptor\amaptor\classes\map.py
# Compiled at: 2017-06-30 13:33:03
import os, logging
...
class Map(object):
    ...
    def _index_frames(self):
        ...
        for layout in self.project.layouts:
            for frame in layout.frames:
                if frame.map and frame.map.name == self.name:
    ...
    def insert_layer(self, reference_layer, insert_layer_or_layerfile, insert_position='BEFORE'):
        ...
        if isinstance(insert_layer_or_layerfile, Layer):
            ...
            insert_layer_or_layerfile.map = self
        else:
            if PRO:
            ...
            if not isinstance(insert_layer_or_layerfile, layer_type):
                raise (
                 RuntimeError, "provided object is not an Layer instance and can't be added to a map")
    ...
    def set_extent(self, extent_object, set_frame='ALL', add_buffer=True, buffer_factor=0.05):
        """
    ...
    def zoom_to_layer(self, layer, set_frame='ALL', add_buffer=True, buffer_factor=0.05):
        """
    ...
    def _export(self, out_path, layout, mapping_function, mp_function, layout_function, extension, **kwargs):
        ...
        elif isinstance(layout, arcpy._mp.Layout) or isinstance(layout, Layout):
    ...
    def export_pdf(self, out_path, layout='ALL', **kwargs):
        ...
        new_kwargs = {}
        allowed_kwargs = [
         'resolution', 'image_quality', 'image_compression', 'embed_fonts', 'layers_attributes', 'georef_info', 'jpeg_compression_quality']
