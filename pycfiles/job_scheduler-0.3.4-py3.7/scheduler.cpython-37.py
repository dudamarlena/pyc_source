...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/haydn/scheduler/scheduler.py
# Compiled at: 2020-02-09 15:24:39
# Size of source mod 2**32: 9416 bytes
import calendar
import haydn.event_emitter as event_emitter
...
class Scheduler(EventEmitter):
    ...
    def add_schedule(self, schedule, options=None):
        ...
        if schedule.schedule_time.end_datetime is not None:
            if schedule.schedule_time.end_datetime <= now:
    ...
    def next_time(self, schedule):
        ...
        if repeat == ScheduleTime.REPEAT_YEARLY:
            return current_datetime + timedelta(days=(365 * repeat_interval))
        if repeat == ScheduleTime.REPEAT_MONTHLY:
            return self._Scheduler__add_months(current_datetime, repeat_interval)
        if repeat == ScheduleTime.REPEAT_WEEKLY:
            return current_datetime + timedelta(weeks=repeat_interval)
        if repeat == ScheduleTime.REPEAT_DAILY:
            return current_datetime + timedelta(days=repeat_interval)
        if repeat == ScheduleTime.REPEAT_WEEKDAYS:
            return current_datetime + timedelta(days=repeat_interval)
        if repeat == ScheduleTime.REPEAT_DAY:
            return current_datetime + timedelta(weeks=repeat_interval)
        if repeat == ScheduleTime.REPEAT_HOURLY:
            return current_datetime + timedelta(hours=repeat_interval)
        if repeat == ScheduleTime.REPEAT_MINUTELY:
            return current_datetime + timedelta(minutes=repeat_interval)
        if repeat == ScheduleTime.REPEAT_SECOND:
