...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/bytecodehacks/rationalize.py
...
binaryops = {'BINARY_ADD': operator.add, 
...
unaryops = {'UNARY_POS': operator.pos, 
...
def simplifyjumps(co):
    ...
    while i < len(cs):
        ...
        if firstlook:
            if op.__class__ in PYBLOCK:
                ...
                if op2.__class__ == SETUP_LOOP:
                    ...
                    trystack.pop()
        if i >= 1:
            if cs[(i - 1)].__class__ in UNCOND and not (cs.find_labels(i) or op.__class__ in PYENDBLOCK):
                cs.remove(op)
                firstlook = 1
                continue
        ...
        if op.__class__ in CONDJUMP and loopstack[(-1)]:
            ...
            if cs[(i + 2)].__class__ == BREAK_LOOP and cs[preendindex].__class__ == POP_TOP:
                ...
                firstlook = 0
                continue
            elif cs[(destindex + 1)].__class__ == BREAK_LOOP and cs[preendindex].__class__ == POP_TOP:
...
def removeconstjump(co):
    ...
    while i < len(cs):
        ...
        if op.__class__ in CONDJUMP and cs[(i - 1)].__class__ == LOAD_CONST:
            if op.__class__ == JUMP_IF_FALSE and cc[cs[(i - 1)].arg] or op.__class__ == JUMP_IF_TRUE and not cc[cs[(i - 1)].arg]:
                cs.remove(cs[(i - 1)])
                cs.remove(cs[(i - 1)])
                cs.remove(cs[(i - 1)])
                i = i - 2
