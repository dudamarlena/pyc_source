...
# Embedded file name: build/bdist.linux-x86_64/egg/amplitf/phasespace/four_body_helicity_phasespace.py
...
class FourBodyHelicityPhaseSpace:
    """FourBodyHelicityPhaseSpace"""

    def __init__(self, ma1, ma2, mb1, mb2, md, ma1a2range=None, mb1b2range=None, costharange=None, costhbrange=None, mab1range=None, mab2range=None):
        """
          Constructor
        """
        self.ma1 = ma1
    ...
    def simple_inside(self, x):
        ...
        phi = self.phi(x)
        inside = tf.logical_and(tf.logical_and(tf.greater(ctha, self.costhamin), tf.less(ctha, self.costhamax)), tf.logical_and(tf.greater(cthb, self.costhbmin), tf.less(cthb, self.costhbmax)))
        inside = tf.logical_and(inside, tf.logical_and(tf.greater(phi, -math.pi), tf.less(phi, math.pi)))
    ...
    def inside(self, x):
        ...
        if self.mab1range or self.mab2range:
            ...
            if self.mab1range:
                ...
                inside = tf.logical_and(inside, tf.logical_and(tf.greater(mab1, self.mab1range[0]), tf.less(mab1, self.mab1range[1])))
            if self.mab2range:
                mab2 = atfk.mass(pa1 + pa2 + pb2)
                inside = tf.logical_and(inside, tf.logical_and(tf.greater(mab2, self.mab2range[0]), tf.less(mab2, self.mab2range[1])))
        return inside
    ...
    def unfiltered_sample(self, size, maximum=None):
        ...
        """
        v = [
         tf.random.uniform([size], (self.ma1a2min), (self.ma1a2max), dtype=(atfi.fptype())),
