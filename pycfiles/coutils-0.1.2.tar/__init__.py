...
# Embedded file name: /Users/Palin/Code/courts-db/courts_db/__init__.py
# Compiled at: 2020-03-19 19:33:41
...
def find_court_ids_by_name(court_str):
    ...
    """
    assert type(court_str) == six.text_type, b"court_str is not a text type, it's of type %s" % type(court_str)
    court_matches = set()
...
def filter_courts_by_date(matches, date_found, strict_dates=False):
    ...
    :return: List of court IDs matched
    """
    assert type(date_found) is datetime, b"date_found is not a date object, it's of type %s" % type(date_found)
    results = [ court for court in courts if court[b'id'] in matches ]
    filtered_results = []
    for result in results:
        for date_object in result[b'dates']:
            date_start = date_object[b'start']
            date_end = date_object[b'end']
            if strict_dates == False:
                if date_start == None:
                    date_start = b'1600-01-01'
                if date_end == None:
                    date_end = b'2100-01-01'
            if strict_dates:
                if date_start == None:
                    continue
                if date_end == None:
                    date_end = b'2100-01-01'
            date_start = datetime.strptime(date_start, b'%Y-%m-%d')
            date_end = datetime.strptime(date_end, b'%Y-%m-%d')
            if date_start <= date_found <= date_end:
                filtered_results.append(result[b'id'])
...
def filter_courts_by_bankruptcy(matches, bankruptcy):
    results = [ court for court in courts if court[b'id'] in matches ]
    if bankruptcy:
        return [ court[b'id'] for court in results if court[b'type'] == b'bankruptcy' ]
    return [ court[b'id'] for court in results if court[b'type'] != b'bankruptcy' ]
...
def find_court_by_id(court_id):
    ...
    """
    return [ court for court in courts if court[b'id'] == court_id ]
