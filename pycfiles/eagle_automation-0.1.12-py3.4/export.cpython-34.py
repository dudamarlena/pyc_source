...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/eagle_automation/export.py
# Compiled at: 2015-08-23 07:56:09
# Size of source mod 2**32: 11637 bytes
"""
...
import os, re, sys, docopt, tempfile, itertools, subprocess, logging
...
class EagleScriptExport:
    ...
    def export(self, in_path, layers, out_paths):
        ...
        cmd = [
         config.EAGLE, '-C' + script_string, in_path]
        try:
            subprocess.call(cmd)
...
class EagleBOMExport(EagleScriptExport):
            ...
            for bom_path in self.bom_path:
                        ...
                        for prefix, packages in out_bom.items():
                            for package, items in packages.items():
                                for value, devices in items.items():
                                    range_list = ranges([int(re.sub('[a-zA-Z]*', '', d['designator'])) for d in devices])
                                    row = [prefix,
                                     package,
                                     value,
                                     len(devices),
                ...
                if '.xlsx' in bom_path:
                    ...
                    try:
                        ...
                        bom_writer.set_column(0, 0, 4.5)
                        bom_writer.set_column(1, 1, 14.5)
                        bom_writer.set_column(2, 2, 16)
                        bom_writer.set_column(3, 3, 2)
                        bom_writer.set_column(4, 5, 50.0)
    ...
    def write_script(self, extension, layers, out_paths):
        ...
        ulp.write(self.ULP_TEMPLATE % {'out_path': os.path.join(self.ulp_dir, 'bom.json')})
...
class EagleMountSMDExport(EagleScriptExport):
    ...
    def write_script(self, extension, layers, out_paths):
        ...
        for layer, out_path in zip(layers, out_paths):
            pp_id = layer['pp_id']
            assert '"' not in out_path
            ulp.write(self.ULP_TEMPLATE % {'out_path': out_path, 
             'pp_id': pp_id})
...
def export_main(verbose=False):
    args = docopt.docopt(__doc__.format(base=sys.argv[0], command=sys.argv[1], types=', '.join(out_types.keys()), layers=', '.join(config.LAYERS.keys())))
    log.debug('Arguments:\n{}'.format(repr(args)))
    layers = []
    out_paths = []
