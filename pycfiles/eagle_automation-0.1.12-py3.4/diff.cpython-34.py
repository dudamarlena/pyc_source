...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/eagle_automation/diff.py
# Compiled at: 2015-08-24 04:54:53
# Size of source mod 2**32: 6440 bytes
...
import os, sys, docopt, difflib, tempfile, subprocess, logging
...
def to_png(in_path, page):
    ...
    if extension == 'brd':
        ...
        out_paths = [os.path.join(workdir, layer + '.png') for layer in config.LAYERS.keys()]
    else:
        if extension == 'sch':
            layers = [{'layers': ['ALL']}]
            out_paths = [
             os.path.join(workdir, 'all.png')]
        else:
            os.rmdir(workdir)
            raise BadExtension
...
def diff_visual(from_file, to_file, page=0, output=None):
    with tempfile.TemporaryDirectory() as (workdir):
        ...
        preview = False
        if not output:
            output = os.path.join(workdir, '{from_file}-{to_file}.pdf'.format(from_file=os.path.basename(from_file).split('.')[0], to_file=os.path.basename(to_file).split('.')[0]))
            preview = True
        if not page:
            try:
        ...
        for page in range(first, last + 1):
            ...
            fname = '{from_file}-{to_file}-page_{page}.pdf'.format(from_file=os.path.basename(from_file.split('.')[0]), to_file=os.path.basename(to_file.split('.')[0]), page=page)
