...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-x86_64/egg/HistView/main.py
...
import Util.Utils as UTIL, API.HistologyAPI
...
def get_log_conf(targetdir=os.getcwd(), loglevel='WARN'):
    ...
    conf = {'version': 1, 
       ...
       'formatters': {'standard': {'format': '%(asctime)s [%(levelname)s] %(name)s: %(message)s'}, 
                      'verbose': {'format': '%(asctime)s %(levelname)s %(name)s %(message)s [%(filename)s@%(lineno)d in %(funcName)s]'}, 
                      'simple': {'format': '%(asctime)s: %(message)s'}}, 
       'handlers': {'error': {'level': 'ERROR', 
                              'class': 'logging.handlers.TimedRotatingFileHandler', 
                              'formatter': 'verbose', 
                              'filename': os.path.join(targetdir, 'HistViewError.log'), 
                              'when': 'midnight', 
                              'backupCount': 30}, 
                    'client': {'level': loglevel, 
                               'class': 'logging.handlers.TimedRotatingFileHandler', 
                               'formatter': 'verbose', 
...
logging.config.dictConfig(get_log_conf(targetdir=os.path.join(os.getcwd(), 'log'), loglevel=logging.WARN))
...
def json_test():
    ...
    """
    f1 = copy.deepcopy({k:v for k, v in self._messages.items() if k == 'receiver_fan' if k == 'receiver_fan'})
    f2 = copy.deepcopy({k:v for k, v in self._messages.items() if k == 'receiver_fan' if k == 'receiver_fan'})
    f1['receiver_fan']['on'] = True
    h1 = copy.deepcopy({k:v for k, v in self._messages.items() if k == 'heater' if k == 'heater'})
    h2 = copy.deepcopy({k:v for k, v in self._messages.items() if k == 'heater' if k == 'heater'})
    h1['heater']['on'] = True
...
class UISimService(object):
    ...
    """
    _messages = {'receiver_fan': {'on': False}, 'heater': {'on': False, 'setpoint': 0.0}, 
       'stilldata': {}}
