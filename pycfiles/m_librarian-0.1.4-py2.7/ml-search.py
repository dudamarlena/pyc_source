...
# [GCC 8.4.0]
# Embedded file name: build/scripts-2.7/ml-search.py
...
from m_librarian.search import mk_search_conditions, search_authors, search_books, search_extensions, search_genres, search_languages
...
def _search_authors(case_sensitive, search_type, args):
    if (args.surname or args.name or args.misc_name) and args.fullname:
        sys.stderr.write('Cannot search by names and full name at the same time\n')
    ...
    authors = search_authors(search_type, case_sensitive, values, expressions, orderBy=('surname',
                                                                                        'name',
                                                                                        'misc_name'))
    if args.count:
        ...
        for author in authors:
            print(encode(author.fullname), encode('(%s: %d)' % (_('books'), author.count)), end=' ')
            if args.verbose >= 1:
...
def _search_books(case_sensitive, search_type, args):
    if args.get and args.get_many:
        sys.stderr.write('Cannot get one book and many books at the same time\n')
        main_parser.print_help()
        sys.exit(1)
    join_expressions = []
    ...
    if case_sensitive is None:
        ...
        test_values.update(_get_values(args, 'surname', 'name', 'misc_name', 'fullname', 'ext', 'gname', 'gtitle', 'lang'))
    ...
    if avalues:
        ...
        if value:
            expressions.append((
             CONCAT(Author.q.surname, ' ', Author.q.name, ' ', Author.q.misc_name),
             decode(value)))
        conditions = mk_search_conditions(Author, search_type, case_sensitive, avalues, expressions)
    ...
    if evalues:
        ...
        conditions = mk_search_conditions(Extension, search_type, case_sensitive, evalues)
        join_expressions.extend(conditions)
    gvalues = {}
    ...
    if gvalues:
        ...
        conditions = mk_search_conditions(Genre, search_type, case_sensitive, gvalues)
        join_expressions.extend(conditions)
    lvalues = {}
    if args.lang:
    ...
    if lvalues:
        ...
        conditions = mk_search_conditions(Language, search_type, case_sensitive, lvalues)
        join_expressions.extend(conditions)
    books = search_books(search_type, case_sensitive, values, join_expressions, orderBy=('series',
                                                                                         'ser_no',
                                                                                         'title'), use_filters=args.filters)
    if args.count:
        ...
        if args.get_many:
            ...
            if args.get:
                count = books.count()
                if count != 1:
                    sys.stderr.write('There must be exactly 1 book for --get; (found %d).\n' % count)
                    sys.stderr.write('Use --get-many N to download more than one book.\n')
                    sys.exit(1)
            count = 0
            for book in books:
                ...
                if args.verbose >= 3:
                    ...
                    print(' ', encode(_('Extension')), ':', encode(book.extension.name))
...
def _search_extensions(case_sensitive, search_type, args):
    ...
    if case_sensitive is None:
        ...
        values = {}
    extensions = search_extensions(search_type, case_sensitive, values, orderBy='name')
    if args.count:
        ...
        for ext in extensions:
            print(encode(ext.name), encode('(%s: %d)' % (_('books'), ext.count)), end=' ')
            if args.verbose >= 1:
...
def _search_genres(case_sensitive, search_type, args):
    ...
    if args.count:
        ...
        for genre in genres:
            ...
            fullname = (' ').join(names)
            print(encode(fullname), encode('(%s: %d)' % (_('books'), genre.count)), end=' ')
            if args.verbose >= 1:
...
def _search_languages(case_sensitive, search_type, args):
    ...
    if case_sensitive is None:
        ...
        values = {}
    languages = search_languages(search_type, case_sensitive, values, orderBy='name')
    if args.count:
        ...
        for lang in languages:
            print(encode(lang.name), encode('(%s: %d)' % (_('books'), lang.count)), end=' ')
            if args.verbose >= 1:
...
if __name__ == '__main__':
    ...
    main_parser.add_argument('-i', '--ignore-case', action='store_true', help='ignore case (default is to guess)')
    main_parser.add_argument('-I', '--case-sensitive', action='store_true', help="don't ignore case")
    main_parser.add_argument('-t', '--start', action='store_true', help='search substring at the start (this is the default)')
    main_parser.add_argument('-s', '--substring', action='store_true', help='search substring anywhere')
    main_parser.add_argument('-f', '--full', action='store_true', help='match the entire string')
    main_parser.add_argument('-c', '--count', action='store_true', help='count instead of output')
    main_parser.add_argument('-v', '--verbose', action='count', default=0, help='output more details about books; repeat for even more details')
    ...
    parser.add_argument('--filters', action='store_true', help='use filters from config')
    parser.add_argument('-P', '--path', help='path to the library archives')
    parser.add_argument('--download-to', nargs='?', const=None, default=os.path.curdir, help='download directory')
    parser.add_argument('-F', '--format', help='download format, default is %%f')
    parser.add_argument('--get', action='store_true', help='download exactly one book')
    parser.add_argument('--get-many', type=int, help='download at most this many books')
    ...
    if args.case_sensitive:
        if args.ignore_case:
            sys.stderr.write('Cannot search case sensitive and case insensitive at the same time\n')
            main_parser.print_help()
            sys.exit(1)
        else:
            case_sensitive = True
