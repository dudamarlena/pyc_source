...
# [GCC 8.4.0]
# Embedded file name: build/lib.linux-i686-2.7/m_librarian/db.py
# Compiled at: 2018-06-11 09:54:59
from __future__ import print_function
import os
from sqlobject import SQLObject, StringCol, UnicodeCol, IntCol, BoolCol, ForeignKey, DateCol, DatabaseIndex, RelatedJoin, connectionForURI, sqlhub, SQLObjectNotFound, dberrors
from .compat import string_type
from .config import get_config
__all__ = [
 'Author', 'Book', 'Extension', 'Genre', 'Language',
...
class Author(SQLObject):
    ...
    books = RelatedJoin('Book', otherColumn='book_id', intermediateTable='author_book', createRelatedTable=False)
...
class Book(SQLObject):
    authors = RelatedJoin('Author', intermediateTable='author_book', createRelatedTable=False)
    genres = RelatedJoin('Genre', intermediateTable='book_genre', createRelatedTable=False)
    ...
    def author_list(self):
        return (', ').join([ a.fullname for a in self.authors ])
    ...
    def genre_title_list(self):
        return (', ').join([ g.title for g in self.genres ])
...
class Genre(SQLObject):
    ...
    title = UnicodeCol(notNull=True)
    count = IntCol(notNull=True)
    books = RelatedJoin('Book', otherColumn='book_id', intermediateTable='book_genre', createRelatedTable=False)
    title_idx = DatabaseIndex(title)
    count_idx = DatabaseIndex(count)
...
def insert_author(surname, name, misc_name):
    try:
        return Author.full_name_idx.get(surname=surname, name=name, misc_name=misc_name)
    except SQLObjectNotFound:
