...
# [GCC 8.4.0]
# Embedded file name: build/lib.linux-i686-2.7/m_librarian/inp.py
# Compiled at: 2018-06-04 15:28:26
import os
from zipfile import ZipFile
from sqlobject import sqlhub
from sqlobject.sqlbuilder import Select
from .db import Author, Book, Extension, Genre, Language, insert_name, insert_author
__all__ = [
 'import_inpx']
EOT = chr(4)
...
def import_inp_line(archive, parts):
    authors, genres, title, series, ser_no, file, size, lib_id, deleted, extension, date, language = parts
    try:
        ser_no = int(ser_no)
    except ValueError:
        ser_no = None
    ...
    book = Book(title=title, series=series, ser_no=ser_no, archive=archive, file=file, size=size, lib_id=lib_id, deleted=deleted, extension=extension_row, date=date, language=language_row)
...
def import_inp(archive, inp):
    ...
    for file, in connection.queryAll(connection.sqlrepr(Select(Book.q.file, Book.q.archive == archive))):
    ...
    for line in inp:
        ...
        if _archive and _archive not in archives:
            ...
            for file, in connection.queryAll(connection.sqlrepr(Select(Book.q.file, Book.q.archive == _archive))):
