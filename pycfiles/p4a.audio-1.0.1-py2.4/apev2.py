...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/p4a/audio/ogg/thirdparty/mutagen/apev2.py
...
class APEv2(DictMixin, Metadata):
    ...
    def __setitem__(self, key, value):
        ...
        if not isinstance(value, _APEValue):
            if isinstance(value, unicode):
                ...
                value = APEValue(('\x00').join(map(utf8, value)), TEXT)
            else:
                try:
    ...
    def save(self, filename=None):
        ...
        tags = ('').join(tags)
        header = 'APETAGEX%s%s' % (
         struct.pack('<4I', 2000, len(tags) + 32, num_tags, HAS_HEADER | HAS_FOOTER | IS_HEADER), '\x00' * 8)
        fileobj.write(header)
        fileobj.write(tags)
        footer = 'APETAGEX%s%s' % (
         struct.pack('<4I', 2000, len(tags) + 32, num_tags, HAS_HEADER | HAS_FOOTER), '\x00' * 8)
...
class _APEValue(object):
    ...
    def _internal(self, key):
        return '%s%s\x00%s' % (struct.pack('<2I', len(self.value), self.kind << 1), key, self.value)
...
class APETextValue(_APEValue):
    ...
    def __iter__(self):
        ...
        return iter(unicode(self).split('\x00'))

    def __getitem__(self, index):
        return unicode(self).split('\x00')[index]

    def __len__(self):
        return self.value.count('\x00') + 1
    ...
    def __setitem__(self, index, value):
        ...
        self.value = ('\x00').join(values).encode('utf-8')
...
class APEv2File(FileType):
    ...
    def score(filename, fileobj, header):
        ...
        return ('APETAGEX' in footer) + header.startswith('MP+') - header.startswith('ID3')
