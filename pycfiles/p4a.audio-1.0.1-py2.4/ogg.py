...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/p4a/audio/ogg/thirdparty/mutagen/ogg.py
...
class OggPage(object):
    ...
    def __init__(self, fileobj=None):
        ...
        if len(header) == 0:
            raise EOFError
        try:
            (oggs, self.version, self.__type_flags, self.position, self.serial, self.sequence, crc, segments) = struct.unpack('<4sBBqIIiB', header)
        except struct.error:
            raise error('unable to read full header; got %r' % header)

        if oggs != 'OggS':
            raise error('read %r, expected %r, at 0x%x' % (oggs, 'OggS', fileobj.tell() - 27))
        if self.version != 0:
    ...
    def __repr__(self):
        attrs = ['version', 'position', 'serial', 'sequence', 'offset', 'complete', 'continued', 'first', 'last']
        values = [ '%s=%r' % (attr, getattr(self, attr)) for attr in attrs ]
        return '<%s %s, %d bytes in %d packets>' % (type(self).__name__, (' ').join(values), sum(map(len, self.packets)), len(self.packets))
    ...
    def write(self):
        ...
        """
        data = [
         struct.pack('<4sBBqIIi', 'OggS', self.version, self.__type_flags, self.position, self.serial, self.sequence, 0)]
        lacing_data = []
    ...
    def from_packets(klass, packets, sequence=0, default_size=4096, wiggle_room=2048):
