...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/p4a/audio/browser/audio.py
# Compiled at: 2007-11-27 08:43:07
import datetime, urllib, Acquisition, AccessControl
...
class IAudioListedSingle(interface.Interface):
    ...
    def safe_audio(obj=None, pos=None, relevance=None):
        pass
...
class AudioListedSingle(FeatureMixin):
    ...
    def __init__(self, context, request):
        ...
        self.portal_url = cmfutils.getToolByName(context, 'portal_url').getPortalObject().absolute_url()
    ...
    def single(self, obj=None, pos=None, relevance=None):
        return self.template(audio=self.safe_audio(obj=obj, pos=pos, relevance=relevance))
    ...
    def safe_audio(self, obj=None, pos=None, relevance=None):
        ...
        creation_time = datetime.date(creation_time.year(), creation_time.month(), creation_time.day())
        creation_time = formatting.fancy_date_interval(creation_time)
        if self.has_contenttagging_support():
            tagview = component.getMultiAdapter((contentobj, self.request), interface=interface.Interface, name='tag_info')
            tags = ({'name': obj, 'url': tagview.tag_url(obj)} for obj in tagview.contextual_tags())
        else:
            tags = []
        avgrating = None
        rating_count = None
        if self.has_contentrating_support():
            ratingview = component.getMultiAdapter((contentobj, self.request), interface=interface.Interface, name='user_rating_view')
        ...
        if self.has_commenting_support():
            ...
            if last is not None:
                ...
                created = datetime.date(created.year(), created.month(), created.day())
        ...
        audio = {'title': obj.title, 'content_author': author_username, 'content_author_name': author, 'creation_time': creation_time, 'tags': tags, 'description': description, 'album': obj.album, 'artist': obj.artist, 'mime_type': contentobj.getContentType, 'year': obj.year, 'idtrack': obj.idtrack, 'url': contentobj.absolute_url(), 'size': formatting.fancy_data_size(fsize), 'length': formatting.fancy_time_amount(length, show_legend=False), 'icon': contentobj.getIcon(), 'playerwidget': self._playerwidget(obj), 'has_image': has_image, 'imageurlwidget': self._imageurlwidget(obj), 'avgrating': avgrating, 'rating_count': rating_count, 'player_count': self.player_count(), 'relevance': relevance, 'commenting_count': commenting_count, 'commenting_last': commenting_last}
        if pos is not None:
...
class AudioView(object):
    ...
    def __init__(self, context, request):
        ...
        self.media_player = component.queryAdapter(self.audio_info.file, interfaces.IMediaPlayer, mime_type)

    def title(self):
        return self.audio_info.title

    def description(self):
        return self.audio_info.description
    ...
    def idtrack(self):
        return self.audio_info.idtrack

    def variable_bit_rate(self):
        return self.audio_info.variable_bit_rate

    def audio_type(self):
        return self.audio_info.audio_type
...
class AudioEditForm(formbase.EditForm):
    ...
    label = _('Edit Audio Data')

    def display_tags(self):
        username = AccessControl.getSecurityManager().getUser().getId()
        return username == self.context.getOwner().getId() and has_contenttagging_support(self.context)
    ...
    def update(self):
        ...
        if self.display_tags():
            ...
            field = schema.TextLine(__name__='tags', title='Tags', description='Enter as many tags as you like, separated by spaces.  If you need a tag with spaces, encase the tag in double quotes; the double quotes will be ignored.', required=False)
    ...
    def handle_edit_action(self, action, data):
        changed = applyChanges(self.context, self.form_fields, data, self.adapters)
        if changed:
            attrs = objectevent.Attributes(interfaces.IAudio, *changed)
            event.notify(objectevent.ObjectModifiedEvent(self.context, attrs))
...
class AudioEditMacros(formbase.PageForm):
    ...
    def __getitem__(self, name):
        if hasattr(self.template, 'macros'):
            ...
            raise TypeError('Could not lookup macros on the configured template')
...
class AudioContainerEditForm(formbase.EditForm):
    ...
    label = 'Edit Audio Container Data'
...
class AudioStreamerView(object):
    ...
    def __call__(self):
        ...
        response.setHeader('Content-Disposition', 'attachment; filename="%s.m3u"' % file_sans_ext)
...
class AudioContainerView(formbase.PageDisplayForm, FeatureMixin):
    ...
    form_fields = form.FormFields(interfaces.IAudioContainer)
    label = 'View Audio Container Info'
    ...
    def should_display_summary(self):
        return self.audio_info.artist or self.audio_info.year or self.audio_info.genre
