...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/p4a/audio/ogg/thirdparty/mutagen/m4a.py
...
_CONTAINERS = [
 'moov', 'udta', 'trak', 'mdia', 'meta', 'ilst', 'stbl', 'minf', 'stsd']
_SKIP_SIZE = {'meta': 4}
__all__ = [
 'M4A', 'Open', 'delete']
...
class Atom(object):
    ...
    def __repr__(self):
        ...
        if self.children is None:
            return '<%s name=%r length=%r offset=%r>' % (klass, self.name, self.length, self.offset)
        else:
            children = ('\n').join([ ' ' + line for child in self.children for line in repr(child).splitlines() ])
            return '<%s name=%r length=%r offset=%r\n%s>' % (klass, self.name, self.length, self.offset, children)
        return
...
class M4ATags(Metadata):
    ...
    def __key_sort((key1, v1), (key2, v2)):
        order = [
         b'\xa9nam', b'\xa9ART', b'\xa9wrt', b'\xa9alb', b'\xa9gen', 'gnre', 'trkn', 'disk', b'\xa9day', 'cpil', 'tmpo', b'\xa9too', '----', 'covr', b'\xa9lyr']
        order = dict(zip(order, range(len(order))))
        last = len(order)
        return cmp(order.get(key1[:4], last), order.get(key2[:4], last)) or cmp(len(v1), len(v2)) or cmp(v1, v2)
    ...
    def save(self, filename):
        ...
        for (key, value) in items:
            render = self.__atoms.get(key[:4], (None, M4ATags.__render_text))[1]
    ...
    def __render_freeform(self, key, value):
        ...
        value = struct.pack('>I4s2I', len(value) + 16, 'data', 1, 0) + value
    ...
    def __render_tempo(self, key, value):
        if 0 <= value < 1 << 16:
            return self.__render_data(key, 21, cdata.to_ushort_be(value))
        else:
    ...
    def __render_compilation(self, key, value):
        return self.__render_data(key, 21, chr(bool(value)))

    def __parse_cover(self, atom, data):
        self[atom.name] = data[16:]

    def __render_cover(self, key, value):
        return self.__render_data(key, 13, value)
    ...
    def __render_text(self, key, value):
        return self.__render_data(key, 1, value.encode('utf-8'))

    def delete(self, filename):
        self.clear()
        self.save(filename)

    __atoms = {'----': (__parse_freeform, __render_freeform), 'trkn': (__parse_pair, __render_pair), 'disk': (__parse_pair, __render_pair_no_trailing), 'gnre': (__parse_genre, None), 'tmpo': (__parse_tempo, __render_tempo), 'cpil': (__parse_compilation, __render_compilation), 'covr': (__parse_cover, __render_cover)}
...
class M4AInfo(object):
    ...
    def pprint(self):
        return 'MPEG-4 audio, %.2f seconds, %d bps' % (self.length, self.bitrate)
