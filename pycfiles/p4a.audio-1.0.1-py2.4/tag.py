...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/p4a/audio/mp3/thirdparty/eyeD3/tag.py
...
class TagHeader:
    ...
    def parse(self, f):
        ...
        TRACE_MSG('TagHeader [flags]: unsync(%d) extended(%d) experimental(%d) footer(%d)' % (self.unsync, self.extended, self.experimental, self.footer))
        tagSizeStr = f.read(4)
        TRACE_MSG('TagHeader [size string]: 0x%02x%02x%02x%02x' % (ord(tagSizeStr[0]), ord(tagSizeStr[1]), ord(tagSizeStr[2]), ord(tagSizeStr[3])))
    ...
    def render(self, tagLen=None):
        ...
        data += bin2bytes([not not self.unsync, not not self.extended, not not self.experimental, not not self.footer, 0, 0, 0, 0])
...
class ExtendedTagHeader:
    ...
    def parse(self, fp, header):
        ...
        if header.minorVersion == 4:
            ...
            TRACE_MSG('Extended header size (not including 4 byte size): %d' % (sz - 4))
            data = fp.read(sz - 4)
            ...
            TRACE_MSG('Extended header size (not including 4 bytes size): %d' % sz)
            tmpFlags = fp.read(2)
            ps = fp.read(4)
...
class Tag:
    ...
    def __saveV2Tag(self, version):
        ...
        if rewriteFile or 10 + len(headerData) + len(extHeaderData) + len(frameData) >= currTagSize:
            TRACE_MSG('File rewrite required')
            rewriteFile = 1
            paddingSize = 2048
        else:
            paddingSize = currTagSize - (len(headerData) + len(extHeaderData) + len(frameData))
        frameData += '\x00' * paddingSize
        TRACE_MSG('Rendering %s tag header with size %d' % (versionToString(self.getVersion()), len(frameData)))
...
class Genre:
    ...
    def parse(self, genreStr):
        ...
        if regex.match(genreStr):
            ...
            return
        raise GenreException("Genre string cannot be parsed with '%s': %s" % (regex.pattern, genreStr))
        return
...
class TagFile:
    ...
    def rename(self, name, fsencoding):
        ...
        try:
            ...
            raise TagException("Error renaming '%s' to '%s'" % (self.fileName, new_name))
...
class Mp3AudioFile(TagFile):
    ...
    def __init__(self, fileName, tagVersion=ID3_ANY_VERSION):
        ...
        TRACE_MSG('mp3 header %x found at position: %d (0x%x)' % (frameHead, f.tell() - 4, f.tell() - 4))
        try:
