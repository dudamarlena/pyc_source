...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/p4a/audio/mp3/thirdparty/eyeD3/frames.py
# Compiled at: 2007-11-27 08:43:08
...
timeStampFormats = [
 '%Y', '%Y-%m', '%Y-%m-%d', '%Y-%m-%dT%H', '%Y-%m-%dT%H:%M', '%Y-%m-%dT%H:%M:%S']
ARTIST_FID = 'TPE1'
BAND_FID = 'TPE2'
CONDUCTOR_FID = 'TPE3'
REMIXER_FID = 'TPE4'
COMPOSER_FID = 'TCOM'
ARTIST_FIDS = [ARTIST_FID, BAND_FID, CONDUCTOR_FID, REMIXER_FID, COMPOSER_FID]
ALBUM_FID = 'TALB'
TITLE_FID = 'TIT2'
SUBTITLE_FID = 'TIT3'
...
URL_FIDS = [URL_COMMERCIAL_FID, URL_COPYRIGHT_FID, URL_AUDIOFILE_FID, URL_ARTIST_FID, URL_AUDIOSRC_FID, URL_INET_RADIO_FID, URL_PAYMENT_FID, URL_PUBLISHER_FID]
USERURL_FID = 'WXXX'
PLAYCOUNT_FID = 'PCNT'
UNIQUE_FILE_ID_FID = 'UFID'
BPM_FID = 'TBPM'
PUBLISHER_FID = 'TPUB'
obsoleteFrames = {'EQUA': 'Equalisation', 'IPLS': 'Involved people list', 'RVAD': 'Relative volume adjustment', 'TDAT': 'Date', 'TORY': 'Original release year', 'TRDA': 'Recording dates', 'TYER': 'Year'}
...
KNOWN_BAD_FRAMES = [
 '\x00\x00MP', '\x00MP3', ' MP3', 'MP3e', '\x00MP', ' MP', 'MP3', 'COM ', 'TCP ', 'CM1 ']
LATIN1_ENCODING = '\x00'
UTF_16_ENCODING = '\x01'
...
class FrameHeader:
    ...
    def __init__(self, tagHeader=None):
        if tagHeader:
            ...
            self.setVersion([DEFAULT_ID3_MAJOR_VERSION, DEFAULT_ID3_MINOR_VERSION])
    ...
    def parse2_2(self, f):
        ...
        if self.isFrameIdValid(frameId):
            TRACE_MSG('FrameHeader [id]: %s (0x%x%x%x)' % (frameId_22, ord(frameId_22[0]), ord(frameId_22[1]), ord(frameId_22[2])))
            self.id = frameId
            sz = f.read(3)
            self.dataSize = bin2dec(bytes2bin(sz, 8))
            TRACE_MSG('FrameHeader [data size]: %d (0x%X)' % (self.dataSize, self.dataSize))
    ...
    def parse(self, f):
        TRACE_MSG('FrameHeader [start byte]: %d (0x%X)' % (f.tell(), f.tell()))
        if self.minorVersion == 2:
            return self.parse2_2(f)
        frameId = f.read(4)
        if self.isFrameIdValid(frameId):
            TRACE_MSG('FrameHeader [id]: %s (0x%x%x%x%x)' % (frameId, ord(frameId[0]), ord(frameId[1]), ord(frameId[2]), ord(frameId[3])))
            self.id = frameId
            sz = f.read(4)
            if self.minorVersion == 3:
                self.dataSize = bin2dec(bytes2bin(sz, 8))
            ...
            TRACE_MSG('FrameHeader [flags]: ta(%d) fa(%d) ro(%d) co(%d) en(%d) gr(%d) un(%d) dl(%d)' % (self.tagAlter, self.fileAlter, self.readOnly, self.compressed, self.encrypted, self.grouped, self.unsync, self.dataLenIndicator))
...
class UserTextFrame(TextFrame):
    ...
    def __unicode__(self):
        return '<%s (%s): {Desc: %s} %s>' % (self.getFrameDesc(), self.header.id, self.description, self.text)
...
class UserURLFrame(URLFrame):
    ...
    def __unicode__(self):
        return '<%s (%s): %s [Encoding: %s] [Desc: %s]>' % (self.getFrameDesc(), self.header.id, self.url, self.encoding, self.description)
...
class CommentFrame(Frame):
    ...
    def _set(self, data, frameHeader=None):
        ...
        try:
            ...
            if strictID3():
                raise FrameException('[CommentFrame] Invalid language code: [%s] %s' % (ex.object, ex.reason))
            else:
                self.lang = ''
    ...
    def __unicode__(self):
        return '<%s (%s): %s [Lang: %s] [Desc: %s]>' % (self.getFrameDesc(), self.header.id, self.comment, self.lang, self.description)
...
class LyricsFrame(Frame):
    ...
    def _set(self, data, frameHeader=None):
        ...
        try:
            ...
            if strictID3():
                raise FrameException('[LyricsFrame] Invalid language code: [%s] %s' % (ex.object, ex.reason))
            else:
                self.lang = ''
    ...
    def __unicode__(self):
        return '<%s (%s): %s [Lang: %s] [Desc: %s]>' % (self.getFrameDesc(), self.header.id, self.lyrics, self.lang, self.description)
    ...
    def parse(self, f, tagHeader):
        ...
        while sizeLeft > 0:
            ...
            if not frameHeader.parse(tagBuffer):
                paddingSize = sizeLeft
                break
            TRACE_MSG('FrameSet: Reading %d (0x%X) bytes of data from byte pos %d (0x%X)' % (frameHeader.dataSize, frameHeader.dataSize, tagBuffer.tell(), tagBuffer.tell()))
        ...
        if UNIQUE_FILE_ID_FRAME_RX.match(fid) and self[fid]:
            ...
            for frm in ufid_frames:
                if frm.owner_id == frame.owner_id:
                    raise FrameException('Multiple %s frames not allowed with the same owner ID (%s)' % (fid, frame.owner_id))
