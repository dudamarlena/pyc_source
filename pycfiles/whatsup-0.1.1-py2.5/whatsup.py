...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/whatsup/whatsup.py
# Compiled at: 2009-07-08 12:00:45
"""
whatsup: a view with webob
"""
import datetime, os, smtplib, urllib, urllib2
...
class WhatsupView(object):
    defaults = {'auto_reload': 'False', 'contacts': None, 
       'sites': None, 
       'ping': True, 
       'ping_frequency': 30, 
    ...
    def __init__(self, **kw):
        ...
        self.response_functions = {'GET': self.get, 'POST': self.post}
        templates_dir = resource_filename(__name__, 'templates')
        self.loader = TemplateLoader(templates_dir, auto_reload=self.auto_reload)
        self.status = {}
        self.errors = {}
        if self.ping:
    ...
    def get(self, request):
        ...
        data = {'contacts': self.contact_data(), 'sites': self.sites_data()}
    ...
    def post(self, request):
        ...
        if path == 'fixed':
            ...
            self.status[site] = {'down': False, 'date': datetime.datetime.now(), 
               'message': fix}
    ...
    def configuration(self, path):
        if os.path.isdir(path):
            files = [ os.path.join(path, f) for f in os.listdir(path) if f[-4:] == '.ini' ]
        else:
            files = [
             path]
    ...
    def send_email(self, from_addr, recipients, message, **headers):
        ...
        message = ('\n').join(('%s: %s' % (header, value) for (header, value) in headers.items())) + '\n\n' + message
        session = smtplib.SMTP(self.smtp_server, self.smtp_port)
        if self.smtp_user:
            session.login(self.smtp_user.encode('utf-8'), self.smtp_password.encode('utf-8'))
