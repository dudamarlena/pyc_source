...
# [GCC 8.4.0]
# Embedded file name: build/bdist.freebsd-7.0-STABLE-i386/egg/infrae/plone/relations/form/_context.py
# Compiled at: 2008-06-13 05:56:36
...
class PloneRelationEditContext(Explicit):
    ...
    def setPrefix(self, uid=None, noset=False):
        ...
        if not noset and self._context_schema:
            setUpWidgets(self, self._context_schema, IInputWidget, prefix=self._context_prefix, names=self._context_names, context=self.context.plone)
    ...
    def setRenderedValue(self, obj):
        ...
        setUpEditWidgets(self, self._context_schema, obj, prefix=self._context_prefix, names=self._context_names, context=self.context.plone)
    ...
    def hasInput(self):
        ...
        if self._rendered:
            ...
            if button not in self.request.form:
                return False
        for name in self._context_names:
    ...
    def error(self):
        ...
        if self._errors:
            ...
            for key in keys:
                ...
                errormessages.append(getMultiAdapter((self._errors[key], self.request), IWidgetInputErrorView).snippet())
