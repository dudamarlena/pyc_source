...
# [GCC 8.4.0]
# Embedded file name: build/bdist.freebsd-7.0-STABLE-i386/egg/infrae/plone/relations/form/_view.py
# Compiled at: 2008-06-12 08:17:36
    ...
    def getRelationToRender(self):
        ...
        for value in self.context.cached_values:
            ...
            for obj in value['objects']:
                data_obj = dict(title=obj.Title(), url=obj.absolute_url())
                objs.append(data_obj)

            data['objects'] = objs
            if self.context.relationUseContext():
                context = value['context']
                data['context'] = dict(title=context.Title(), url=context.absolute_url())
            items.append(data)
...
class PloneRelationEditView(Explicit):
    ...
    def _prepareData(self):
        ...
        count = 0
        for value in self.context.cached_values:
            data = dict(nb=count, modify=False)
            objs = []
            for obj in value['objects']:
                obj_id = self.context.getIdForObject(obj)
                data_obj = dict(title=obj.Title(), uid=obj_id, url=obj.absolute_url())
                objs.append(data_obj)
                ids.append(obj_id)
    ...
    def renderEditContext(self):
        ...
        if self.context.edited_context:
            all += renderElement('input', type='hidden', value=self.context.edited_context, name='%s.modify_uid' % self.getName())
        return all
