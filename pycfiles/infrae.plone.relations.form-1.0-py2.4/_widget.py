...
# [GCC 8.4.0]
# Embedded file name: build/bdist.freebsd-7.0-STABLE-i386/egg/infrae/plone/relations/form/_widget.py
...
class PloneRelationMixin(object):
    ...
    def relationUseContext(self):
        return self.field.context_schema is not None
...
class PloneRelationEditWidget(Explicit, SimpleInputWidget, PloneRelationMixin):
    ...
    def _contextModification(self, i, modified_context_id):
        ...
        elif context_id == modified_context_id:
    ...
    def _generateRelations(self):
        ...
        if self.add_widget:
            if self.add_widget.hasInput():
                target_ids = self.add_widget.getInputValue()
            else:
                context_id = None
                raise target_ids or AssertionError
            if self.relationUseContext():
