...
# Embedded file name: build/bdist.linux-x86_64/egg/pysmi/compiler.py
    ...
    *readers*, *searchers* and *borrowers*.
    ...
    def compile(self, *mibnames, **options):
        ...
        *mibnames* and may be performed for all MIBs referred to from
            ...
            for source in self._sources:
                ...
                try:
                    ...
                    for mibTree in self._parser.parse(fileData):
                        ...
                        parsedMibs[mibInfo.name] = (
                         fileInfo, mibInfo, mibTree)
                        if mibname in failedMibs:
        ...
        debug.logger & debug.flagCompiler and debug.logger('MIBs analyzed %s, MIBs failed %s' % (len(parsedMibs), len(failedMibs)))
        ...
        for mibname in parsedMibs.copy():
            ...
            comments = [
             'ASN.1 source %s' % fileInfo.path, 'Produced by %s-%s at %s' % (packageName, packageVersion, time.asctime()), 'On host %s platform %s version %s by user %s' % (platform_info[1], platform_info[0], platform_info[2], user_info[0]), 'Using Python version %s' % sys.version.split('\n')[0]]
            try:
                (mibInfo, mibData) = self._codegen.genCode(mibTree, symbolTableMap, comments=comments, genTexts=options.get('genTexts'), textFilter=options.get('textFilter'))
                builtMibs[mibname] = (
                 fileInfo, mibInfo, mibData)
                del parsedMibs[mibname]
            ...
            for borrower in self._borrowers:
                ...
                try:
                    ...
                    borrowedMibs[mibname] = (
                     fileInfo, MibInfo(name=mibname, imported=[]), fileData)
                    del failedMibs[mibname]
        ...
        for mibname in borrowedMibs.copy():
            ...
            for searcher in self._searchers:
                ...
                if options.get('noDeps') and mibname not in canonicalMibNames:
                    ...
                    processed[mibname] = statusBorrowed.setOptions(path=fileInfo.path, file=fileInfo.file, alias=fileInfo.name)
                del borrowedMibs[mibname]
        ...
        for mibname in builtMibs.copy():
            ...
            try:
                ...
                if mibname not in processed:
                    processed[mibname] = statusCompiled.setOptions(path=fileInfo.path, file=fileInfo.file, alias=fileInfo.name, oid=mibInfo.oid, oids=mibInfo.oids, identity=mibInfo.identity, revision=mibInfo.revision, enterprise=mibInfo.enterprise, compliance=mibInfo.compliance)
            except error.PySmiError:
                (exc_class, exc, tb) = sys.exc_info()
                exc.handler = self._codegen
                exc.mibname = mibname
        ...
        debug.logger & debug.flagCompiler and debug.logger('MIBs modified: %s' % (', ').join([ x for x in processed if processed[x] in ('compiled',
                                                                                                                                        'borrowed') ]))
        return processed
    ...
    def buildIndex(self, processedMibs, **options):
        (platform_info, user_info) = self._get_system_info()
        comments = [
         'Produced by %s-%s at %s' % (packageName, packageVersion, time.asctime()), 'On host %s platform %s version %s by user %s' % (platform_info[1], platform_info[0], platform_info[2], user_info[0]), 'Using Python version %s' % sys.version.split('\n')[0]]
        try:
            self._writer.putData(self.indexFile, self._codegen.genIndex(processedMibs, comments=comments, old_index_data=self._writer.getData(self.indexFile)), dryRun=options.get('dryRun'))
        except error.PySmiError:
            (exc_class, exc, tb) = sys.exc_info()
