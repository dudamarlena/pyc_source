...
# Embedded file name: build/bdist.linux-x86_64/egg/pysmi/reader/ftpclient.py
...
class FtpReader(AbstractReader):
    ...
    def __init__(self, host, locationTemplate, timeout=5, ssl=False, port=21, user='anonymous', password='anonymous@'):
    ...
    def getData(self, mibname, **options):
        ...
        for (mibalias, mibfile) in self.getMibVariants(mibname, **options):
            ...
            conn.close()
            return (
             MibInfo(path='ftp://%s%s' % (self._host, location), file=mibfile, name=mibalias, mtime=mtime), data)
