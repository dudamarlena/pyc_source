...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/uchicago_cs_setup_script/script.py
# Compiled at: 2019-09-23 22:13:36
# Size of source mod 2**32: 21371 bytes
import os, click, getpass, sys, yaml, requests
from gitlab import Gitlab
from gitlab.exceptions import HttpError
import stat, git, time, subprocess
...
def get_gitlab_repo_data(gitlab, gitlab_group_name, gitlab_upstream_repo_name, repo):
    ...
    if repo is not None:
        ...
        return (
         gitlab_upstream_repo, gitlab_project)
...
def cmd(course_id, cnetid, password, config_dir, repo, local_repo_path, timeout, skip_ssl_verify, skip_push, verbose):
    ...
    if skip_ssl_verify:
        ...
        if os.path.exists(ca_bundle):
            ...
            verify_ssl = True
        gitlab = connect_to_gitlab(gitlab_hostname=config['gitlab-hostname'], username=username, password=password, gitlab_ssl=config['gitlab-ssl'], verify_ssl=verify_ssl)
