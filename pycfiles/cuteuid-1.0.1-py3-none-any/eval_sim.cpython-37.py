...
# Embedded file name: build/bdist.linux-x86_64/egg/benchmarks/eval_sim.py
# Compiled at: 2020-04-17 02:17:06
# Size of source mod 2**32: 13811 bytes
import sys, argparse, logging, time

def pase_info(seq):
    info = {'SVLEN':0, 
     'END':0,  'SVTYPE':'',  'RE':0,  'CHR2':''}
    for i in seq.split(';'):
        ...
        if svtype not in ans:
            if svtype == 'INS':
                for i in call[svtype]:
                    for key in ans:
                        for j in ans[key]:
                            if i[0] == j[0] and abs(i[1] - j[1]) <= offect and float(min(i[3], j[3]) / max(i[3], j[3])) >= bias:
                                i[-1] = 1
                                j[3 + opt] = 1
                                if i[4] == genotype[j[0]]:
                                    i[-1] = 2
...
typetrans = {'insertion':'INS',  'deletion':'DEL', 
 'inversion':'INV', 
...
USAGE = '\tEvaluate SV callset generated by simulations.\n\tAuthor: Tao Jiang\n\tEmail: tjiang@hit.edu.cn\n'
