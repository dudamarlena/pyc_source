...
# Embedded file name: build/bdist.linux-x86_64/egg/benchmarks/eval_trio.py
# Compiled at: 2020-04-20 05:15:21
                    ...
                    if i[0] != j[0]:
                        ...
                        if svtype == 'INS':
                            if abs(i[1] - j[1]) <= offect:
                                if float(min(i[3], j[3]) / max(i[3], j[3])) >= bias:
                                    i[-1] = 1
                    if svtype == 'BND':
                        if i[2] == j[2] and i[4] == j[4]:
                            if abs(i[1] - j[1]) <= offect:
                                if abs(i[3] - j[3]) <= offect:
                                    i[-1] = 1
                                elif max(i[1] - offect, j[1]) <= min(i[2] + offect, j[2]):
                                    if float(min(i[3], j[3]) / max(i[3], j[3])) >= bias:
                                        i[-1] = 1
