...
# Embedded file name: build/bdist.linux-x86_64/egg/cuteSV/CommandRunner.py
# Compiled at: 2020-04-17 02:15:05
# Size of source mod 2**32: 5159 bytes
from string import Template
import tempfile, subprocess, signal, logging, os, stat, sys
...
def exe(cmd, timeout=-1):
    ...
    try:
        ...
        logging.error('Command was taking too long. Automatic Timeout Initiated after %d' % timeout)
...
class Command:
    ...
    def asDict(self):
        return {'CMD':self.cmd, 
         'JOBNAME':self.jobname,  'STDOUT':self.stdout, 
         'STDERR':self.stderr}
...
class CommandRunner:
    ...
    def __call__(self, cmds, wDir=None, id=None):
        ...
        for chunk, commands in enumerate(partition(cmds, self.njobs)):
            ...
            submit = Command(outScript.name, id + '_chunk%d' % chunk, os.path.join(wDir, id + '_chunk%d.out' % chunk), os.path.join(wDir, id + '_chunk%d.err' % chunk))
    ...
    def checkTemplate(self):
        ...
        """
        temp.update({'CMD':'test',  'STDOUT':'testo', 
         'STDERR':'teste', 
         'JOBNAME':'testn'})
        try:
