...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pysugar/pysugar.py
# Compiled at: 2007-01-04 13:11:52
from elementsoap import ElementSOAP
from elementtree.ElementTree import tostring, dump
import md5, xml
from pytz import timezone
import datetime, base64, urllib2, types
...
class SugarService(ElementSOAP.SoapService):
    ...
    def login(self, user, password):
        ...
        ElementSOAP.SoapElement(request, 'application_name', 'string', self.application_name)
        ...
        if not error_element.findtext('number') == '0':
            raise SugarLoginError('number: %s, name: %s, desc: %s' % (error_element.findtext('number'), error_element.findtext('name'), error_element.findtext('description')))
    ...
    def set_entry(self, session_id, module, item):
        ...
        if error:
            ...
            raise SugarError('number: %s, name: "%s", desc: "%s"' % (error, name, desc))
    ...
    def get_entry_list(self, session_id, module, query, order_by, offset, selection, max_result, deleted):
        """
    ...
    def get_relationships(self, session_id, module_name, module_id, related_module, related_module_query, deleted=False):
        """
        ...
        *              'Users'
        ...
        ElementSOAP.SoapElement(request, 'related_module', 'string', related_module)
        ElementSOAP.SoapElement(request, 'related_module_query', 'string', related_module_query)
    ...
    def set_relationship(self, session_id, module1, module1_id, module2, module2_id):
        """
    ...
    def prune_meetings(self, session_id, date_from, date_to):
        ...
        """
        assert isinstance(date_from, datetime.date), 'date_from should be a datetime.date instance'
        assert isinstance(date_to, datetime.date), 'date_to should be a datetime.date instance'
        action = 'prune_meetings'
        request = ElementSOAP.SoapRequest(action)
        ElementSOAP.SoapElement(request, 'session', 'string', session_id)
        ElementSOAP.SoapElement(request, 'date_from', 'string', date_from.strftime('%Y-%m-%d'))
        ElementSOAP.SoapElement(request, 'date_to', 'string', date_to.strftime('%Y-%m-%d'))
        response = self.call(action, request)
        ret = response.find('return')
        error_elem = ret.find('error')
        error = int(error_elem.findtext('number'))
...
class SugarSession:
    ...
    def __init__(self, username, password, base_url, debug=False, user_management=False, nusoapfile='soap.php'):
        """
    ...
    def __validate_login(self):
        ...
        if not self._session_id:
            raise SugarCredentialError('A valid session id is required to use this method')
    ...
    def login(self, username, password):
        ...
        if self._debug:
            print "We are logged-in with user-id: '%s'" % self.get_user_id()
            print "Our session id is: '%s'" % self._session_id

    def get_entry_list(self, module, query, order_by, offset, selection, max_result, deleted):
        """
        ...
        return self.service.get_entry_list(self._session_id, module, query, order_by, offset, selection, max_result, deleted)
    ...
    def get_entry(self, module, id, selection):
        ...
        """
        self.__validate_login()
        return self.service.get_entry(self._session_id, module, id, selection)
    ...
    def set_entries(self, module, items):
        ...
        """
        self.__validate_login()
        return self.service.set_entries(self._session_id, module, items)
    ...
    def get_module_fields(self, module_name):
        self.__validate_login()
        return self.soap_proxy.get_module_fields(self._session_id, module_name)
    ...
    def create_user(self, user_name, password):
        ...
        self.__validate_login()
        res = self.user_proxy.create_user(self._session_id, user_name, password)
        if res.id == '-1':
            print res.error
        return res.id
    ...
    def get_gmt_time(self):
        ...
        minute = int(minute)
        second = int(second)
        return datetime.datetime(year, month, day, hour, minute, second, 0, tzinfo=utc)
    ...
    def get_user_role_ids(self, user_id):
        if not self.user_management:
            raise SugarError('User Management is not initialized')
        self.__validate_login()
        return self.user_proxy.get_user_role_ids(self._session_id, user_id)

    def remove_user_role(self, role_id, user_id):
        if not self.user_management:
            raise SugarError('User Management is not initialized')
        self.__validate_login()
        return self.user_proxy.remove_user_role(self._session_id, role_id, user_id)

    def get_relationships(self, module_name, module_id, related_module, related_module_query, deleted=False):
        """
        session_id: a valid session id
        module_name: a module name such as 'Accounts'
        module_id: a valid object id in the module name
        ...
        *              'Users'
        ...
        """
        self.__validate_login()
        return self.service.get_relationships(self._session_id, module_name, module_id, related_module, related_module_query, deleted)

    def set_relationship(self, module1, module1_id, module2, module2_id):
        self.__validate_login()
        return self.service.set_relationship(self._session_id, module1, module1_id, module2, module2_id)
    ...
    def prune_meetings(self, date_from, date_to):
        ...
        """
        self.__validate_login()
        return self.service.prune_meetings(self._session_id, date_from, date_to)