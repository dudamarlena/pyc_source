...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pysugar/sugarobjects.py
# Compiled at: 2007-01-04 15:31:59
...
class SugarModuleCollection:
    ...
    def add(self, module_name, module_class):
        self.modules[module_name] = SugarModule(self, module_name, module_class)
...
class SugarModule(object):
    ...
    def post(self, callback=None):
        ...
        element_list.extend([ e for e in self.elements.values() if e.ismodified() ])
        for (i, batch) in enumerate(split_seq(element_list, self.batch_size)):
            ...
            if callback is not None:
                callback(self, i * self.batch_size + len(batch), len(element_list))
...
class SugarObject(object):
    ...
    def get_post_dict(self):
        ...
        for prop in self.sugar_properties:
            if prop._get_modified(self):
                post_dict[prop.field_name] = prop._to_sugar_value(prop._get_raw_value(self))

        return post_dict

    def post(self):
        new_id = self.module.collection.backend.set_entry(self.module.name, self.get_post_dict())
        self.finalize_post(new_id)

    def finalize_post(self, new_id):
        if self.__id is None:
            self.__id = new_id
            self.module.new_elements.pop(self.module.new_elements.index(self))
            self.module.elements[new_id] = self
            self.invalidate()
        elif self.__id != new_id:
            raise SugarOperationnalError('Posted object %s and received a new id: %s' % (self.__id, new_id))
        return

    def load(self):
        d = self.module.collection.backend.get_entry(self.module.name, self.id, '')
        for prop in self.sugar_properties:
...
class SugarField(object):
    ...
    def __init__(self, name, field_name, module=None, read_only=False, receive_only=False, send_only=False, mandatory=True):
        self.name = name
    ...
    def _get_modified(self, sugar_o):
        return self.is_loaded(sugar_o) and getattr(sugar_o, self.modified_attr)
    ...
    def _load_value(self, sugar_o, value):
        self.__set_raw_value(sugar_o, self._from_sugar_value(value))
        self.__set_modified(sugar_o, False)

    def _set_value(self, sugar_o, value):
        if not sugar_o.isnew() and not self.is_loaded(sugar_o):
            sugar_o.load()
        self.__set_raw_value(sugar_o, value)
        self.__set_modified(sugar_o, True)

    def _get_value(self, sugar_o):
        if not sugar_o.isnew() and not hasattr(sugar_o, self.value_attr):
            sugar_o.load()
...
class SugarRelationField(SugarField):
    ...
    def get_value(self, sugar_o):
        ...
        return sugar_o.module.collection.modules[self.module].get(id)
...
def create_property(sugar_field):
    return property(fget=SugarPropertyGetter(sugar_field), fset=SugarPropertySetter(sugar_field))
...
def sugar_field(name, read_only=False, receive_only=False, send_only=False, mandatory=True):
    field = SugarSimpleField(name, name, read_only=read_only, receive_only=receive_only, send_only=send_only, mandatory=mandatory)
    return (
     field, create_property(field))


sugar_str_field = sugar_field

def sugar_relation_field(property_name, field_name, module, read_only=False, receive_only=False, send_only=False, mandatory=True):
    field = SugarRelationField(property_name, field_name, module=module, read_only=read_only, receive_only=receive_only, send_only=send_only, mandatory=mandatory)
    return (
     field, create_property(field))


def sugar_datetime_field(name, read_only=False, receive_only=False, send_only=False, mandatory=True):
    field = SugarDatetimeField(name, name, read_only=read_only, receive_only=receive_only, send_only=send_only, mandatory=mandatory)
    return (
     field, create_property(field))


def sugar_date_field(name, read_only=False, receive_only=False, send_only=False, mandatory=True):
    field = SugarDateField(name, name, read_only=read_only, receive_only=receive_only, send_only=send_only, mandatory=mandatory)
    return (
     field, create_property(field))


def sugar_time_field(name, read_only=False, receive_only=False, send_only=False, mandatory=True):
    field = SugarTimeField(name, name, read_only=read_only, receive_only=receive_only, send_only=send_only, mandatory=mandatory)
    return (
     field, create_property(field))


def sugar_integer_field(name, read_only=False, receive_only=False, send_only=False, mandatory=True):
    field = SugarIntegerField(name, name, read_only=read_only, receive_only=receive_only, send_only=send_only, mandatory=mandatory)
    return (
     field, create_property(field))


def sugar_bool_field(name, read_only=False, receive_only=False, send_only=False, mandatory=True):
    field = SugarBooleanField(name, name, read_only=read_only, receive_only=receive_only, send_only=send_only, mandatory=mandatory)
    return (
     field, create_property(field))