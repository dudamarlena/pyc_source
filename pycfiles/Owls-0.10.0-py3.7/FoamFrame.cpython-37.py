...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/Owls/FoamFrame.py
# Compiled at: 2018-11-11 03:24:41
# Size of source mod 2**32: 25877 bytes
...
def read_sets(folder, name='None', search=FPNUMBER, **kwargs):

    def setsfolder(folder):
        p = os.path.join(folder, 'postProcessing')
        if os.path.exists(p):
            return os.path.join(p, 'sets')
        return os.path.join(folder, 'sets')

    return FoamFrame(folder=setsfolder(folder), search_files=False, search_pattern=search, 
     name=name, show_func='plot', 
     preHooks=None, exclude=[
 'processor'], **kwargs)


def read_lag(folder, files, skiplines=1, name='None', cloud='[A-Za-z]*Cloud1', preHooks=None, decomposed=False, **kwargs):
    search = FPNUMBER + '/lagrangian/' + cloud
    search = search if not decomposed else 'processor[0-9]?/' + search
    return FoamFrame(folder=folder, search_files=files, search_pattern=search, 
     name=name, skiplines=skiplines, 
...
def read_eul(folder, files, skiplines=1, name='None', decomposed=False, preHooks=None, **kwargs):
    ...
    return FoamFrame(folder=folder, search_files=files, search_pattern=search, 
     name=name, skiplines=skiplines, 
     show_func='scatter', preHooks=preHooks, **kwargs)


def read_exp(folder, name='None', search='', **kwargs):
    return FoamFrame(folder=folder, search_files=False, search_pattern=search, 
     name=name, show_func='scatter', **kwargs)
...
def read_log(folder, keys, log_name='log', plot_properties=False, name='None', **kwargs):
    ...
    ff.properties = Props(origins=origins,
      name=folder,
...
class FoamFrame(DataFrame):
    ...
    def __init__(self, *args, **kwargs):
        ...
        times_range = kwargs.get('times_range', 'all')
        ...
        if folder is None:
            ...
            if preHooks:
                ...
                self.properties = Props(origins, name, plot_properties, folder, symb, show_func)
    ...
    @staticmethod
    def from_dict(input_dict, name='None', plot_properties=None, symb='.', show_func='scatter'):
    ...
    def rename_idx(self, search, replace):
        """ rename field names based on regex """
        self.index = Index([(t, replace if x == search else x, i) for t, x, i in list(self.index)], names=(self.index.names))
    ...
    def draw(self, x, y, z, title, func, figure, data=None, legend_prefix='', titles=None, **kwargs):
        data = data if isinstance(data, DataFrame) else self
        return (rcParams['plotWrapper'].draw)(x=x, 
         y=y, z=z, data=data, title=title, func=func, 
         figure=figure, legend_prefix='', 
         titles=None, properties=self.properties, **kwargs)

    def histo_data(self, y, weights, bins):
        return np.histogram(y,
    ...
    def histogram(self, y, x=None, title='', figure=False, weights=False, **kwargs):
        ...
        return (self.draw)(x='centres', y='hist', z=None, data=df, 
         title=title, func='quad', figure=figure, **kwargs)
    ...
    def cdf(self, y, x=None, title='', figure=False, weights=False, **kwargs):
        ...
        return (self.draw)(x='centres', y='hist', z=None, data=df, 
         title=title, func='line', figure=figure, **kwargs)
    ...
    def show(self, y, x='Pos', figure=False, overlay='Field', style=None, legend_prefix='', post_pone_style=False, row=None, titles=None, **kwargs):
        ...
        def create_figure(y_, f, title='', legend=''):
            if kwargs.get('title'):
                ...
                kwargs.pop('title')
            return (getattr(self, self.properties.show_func))(y=y_, 
             x=x, figure=f, legend_prefix=legend_prefix + legend, 
             title=title, **kwargs)
                ...
                if self.grouped:
                    ...
                    if overlay == 'Group':
                        for yi in y:
                            f = arow.get(yi)
                            for group in groups:
                                arow[yi] = (self.at('Group', group).show)(x=x, 
                                 y=yi, title=yi, figure=(yi, f), overlay='Field', 
                                 legend_prefix=legend_prefix, 
                                 legend=str(group), **kwargs)[yi]

                    if overlay == 'Field':
                        for group in groups:
                            f = arow.get(group)
                            field = self.at('Group', group)
                            arow[group] = (field.show)(x=x, y=y, title=str(group), figure=(
 group, f), 
                             overlay='Field', post_pone_style=True, 
                             legend_prefix=legend_prefix, **kwargs)[group]
    ...
    def rolling_mean(self, y, x='Pos', n=10, weight=False):
        ...
        bins = {y: [self.filter(name=x, field=(lambda x: l < x < u))[y].mean() for l, u in bds]}
        bins.update({x: [(l + u) / 2.0 for l, u in bds]})
        return self.from_dict(bins, name=('rl' + self.properties.name),
    ...
    def weighted_rolling_mean(self, y, x='Pos', n=10, weight=False):
        ...
        delta = (upper - lower) / n
        bds = [(lower + i * delta, lower + (i + 1) * delta) for i in range(n)]
        bins = {y: [np.average(a=(self.filter(name=x, field=(lambda x: l < x < u))[y]),
              weights=(self.filter(name=x, field=(lambda x: l < x < u))[weight])) for l, u in bds]}
        bins.update({x: [(l + u) / 2.0 for l, u in bds]})
        return self.from_dict(bins, name=('rl' + self.properties.name),
          plot_properties=(self.properties.plot_properties),
          show_func='plot')
