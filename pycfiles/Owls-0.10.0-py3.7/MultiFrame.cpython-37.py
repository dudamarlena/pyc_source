...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/Owls/MultiFrame.py
...
def multiframes(folder, names, reader, **kwargs):
    ...
    return MultiFrame([reader(f, name=n, **kwargs) for f, n in zip(folder, names)])


class MultiFrame:
    __doc__ = ' Class for storage of multiple case items\n        or faceted data from FoamFrame\n    '

    @staticmethod
    def from_dict(input_dict, **kwargs):
        return MultiFrame(cases=[(FoamFrame.from_dict)(d, name=name, **kwargs) for name, d in input_dict.items()])

    def __mul__(self, other):
        """ multiply self with other, e.g. Foo() * 7 """
        if isinstance(other, NumberTypes):
            return MultiFrame(OrderedDict([(i, c.__mul__(other)) for i, c in self.cases.items()]))
        return MultiFrame(OrderedDict([(i, c.__mul__(other.cases[i])) for i, c in self.cases.items()]))

    def __rmul__(self, other):
        """ multiply other with self, e.g. 7 * Foo() """
        if isinstance(other, NumberTypes):
            return MultiFrame(OrderedDict([(i, c.__rmul__(other)) for i, c in self.cases.items()]))
        return MultiFrame(OrderedDict([(i, c.__rmul__(other.cases[i])) for i, c in self.cases.items()]))

    def __truediv__(self, other):
        """ multiply self with other, e.g. Foo() * 7 """
        if isinstance(other, NumberTypes):
            return MultiFrame(OrderedDict([(i, c.__truediv__(other)) for i, c in self.cases.items()]))
        return MultiFrame(OrderedDict([(i, c.__rtruediv__(other.cases[i])) for i, c in self.cases.items()]))

    def __rtruediv__(self, other):
        """ multiply self with other, e.g. Foo() * 7 """
        if isinstance(other, NumberTypes):
            return MultiFrame(OrderedDict([(i, c.__rtruediv__(other)) for i, c in self.cases.items()]))
        return MultiFrame(OrderedDict([(i, c.__rtruediv__(other.cases[i])) for i, c in self.cases.items()]))

    def __add__(self, other):
        """ add self with other """
        return MultiFrame(OrderedDict([(i, c.__add__(other.cases[i])) for i, c in self.cases.items()]))

    def __radd__(self, other):
        """ add other with self """
        return MultiFrame(OrderedDict([(i, c.__radd__(other.cases[i])) for i, c in self.cases.items()]))

    def __sub__(self, other):
        """ add self with other """
        return MultiFrame(OrderedDict([(i, c.__sub__(other.cases[i])) for i, c in self.cases.items()]))

    def __rsub__(self, other):
        """ add other with self """
        return MultiFrame(OrderedDict([(i, c.__rsub__(other.cases[i])) for i, c in self.cases.items()]))

    def __repr__(self):
        s = 'MultiFrame with {} entries:\n'.format(len(self.cases))
        s += '\n'.join(['{}\n{}:\n{}'.format('================================================================================', name, c.describe()) for name, c in self.cases.items()])
        return s
    ...
    def __getitem__(self, field):
        ...
        return MultiFrame(OrderedDict([(key, serie[field]) for key, serie in self.cases.items()]))
    ...
    def fillna(self, value, **kwargs):
        if not kwargs.get('inplace', False):
            return MultiFrame(OrderedDict([(key, (serie.fillna)(value, **kwargs)) for key, serie in self.cases.items()]))
    ...
    def time_average(self, suffix='Avg', time_start=0.0):
        ...
        return MultiFrame([case.time_average(suffix, time_start) for cname, case in self.cases.items()])
    ...
    def unselect(self, cases):
        """ select a specific item """
        return MultiFrame(OrderedDict([(key, serie) for key, serie in self.cases.items() if key not in cases]))
    ...
    def show(self, y, x='Pos', z=False, overlay='Field', style=None, filename=None, show=True, **kwargs):
        ...
        row = (self.cases[cases[0]].show)(x=x, 
         y=y, overlay=overlay, legend_prefix=cases[0], 
         style=style, post_pone_style=True, 
         titles=y, filename=filename, **kwargs)
        for c, d, col in zip(cases[1:], dashes, rcParams['plotWrapper'].colored[1:]):
            row = (self.cases[c].show)(x=x, 
             y=y, overlay=overlay, legend_prefix=c, 
             style=style, row=row, 
             post_pone_style=True, line_dash=d, 
             titles=y, color=col, **kwargs)
    ...
    def on(self, case, func, **kwargs):
        ...
        return MultiFrame([case_ if cname != case else n for cname, case_ in self.cases.items()])
