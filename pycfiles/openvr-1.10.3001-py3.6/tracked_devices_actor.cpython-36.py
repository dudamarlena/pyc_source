...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\openvr\tracked_devices_actor.py
# Compiled at: 2019-07-06 13:51:24
# Size of source mod 2**32: 9469 bytes
import time
...
class TrackedDeviceMesh(object):
        ...
        try:
            ...
            if model is not None:
                for v in range(model.unVertexCount):
                    ...
                    vertices0.append(float(vd.vPosition.v[0]))
                    vertices0.append(float(vd.vPosition.v[1]))
                    vertices0.append(float(vd.vPosition.v[2]))
                    vertices0.append(float(vd.vNormal.v[0]))
                    vertices0.append(float(vd.vNormal.v[1]))
                    vertices0.append(float(vd.vNormal.v[2]))
                    vertices0.append(float(vd.rfTextureCoord[0]))
                    vertices0.append(float(vd.rfTextureCoord[1]))
    ...
    def _try_load_texture(self):
        try:
            ...
            GL.glTexImage2D(GL.GL_TEXTURE_2D, 0, GL.GL_RGBA, self.texture_map.unWidth, self.texture_map.unHeight, 0, GL.GL_RGBA, GL.GL_UNSIGNED_BYTE, self.texture_map.rubTextureMapData)
