...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/tests/unit/config_test.py
...
def setup_os1():
    ...
    """
    home_map = {'root': '/var/root', 'user1': full_path('user1_homedir')}
    mock_env = {'SUDO_USER': 'user1', 'USER': 'root', 
       'HOME': home_map['user1']}
    ...
    group_patch = mock.patch('salve.ugo.get_group_from_username', mock_get_group)
    setup_patches(mock.patch.dict('os.environ', mock_env), group_patch, expanduser_patch)
...
def setup_os2():
    ...
    mock_get_group = lambda x: 'nogroup'
    group_patch = mock.patch('salve.ugo.get_group_from_username', mock_get_group)
    setup_patches(mock.patch.dict('os.environ', mock_env), expanduser_patch, group_patch)
...
def setup_os3():
    ...
    mock_get_group = lambda x: 'nogroup'
    group_patch = mock.patch('salve.ugo.get_group_from_username', mock_get_group)
    setup_patches(mock.patch.dict('os.environ', mock_env), mock.patch('os.path.expanduser', mock_expanduser), group_patch)
...
def valid_config1():
    ...
    """
    conf = config.SALVEConfig(filename=full_path('single_section_single_attr.ini'))
...
def multiple_env_overload():
    ...
    """
    conf = config.SALVEConfig(filename=full_path('two_sections.ini'))
...
def missing_config():
    ...
    """
    conf = config.SALVEConfig(filename=full_path('NONEXISTENT_FILE'))
...
def invalid_file():
    ...
    try:
        ...
        assert 'Encountered an error while parsing' + ' your configuration file(s).' in e.message
...
def template_sub():
    ...
    assert conf.template('$HOME/bin/program') == paths.pjoin(full_path('user1_homedir'), 'bin/program')