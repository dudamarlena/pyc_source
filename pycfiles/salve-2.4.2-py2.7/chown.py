...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/salve/action/modify/chown.py
...
class ChownAction(with_metaclass(abc.ABCMeta, ModifyAction)):
    ...
    Is an ABC.
    """
    verification_codes = ModifyAction.verification_codes.extend('NOT_ROOT', 'SKIP_EXEC')
    ...
    def verify_can_exec(self, filesys):
        ...
        logger.info('Chown: Checking target exists, "%s"' % self.target)
        if not filesys.exists(self.target):
            return self.verification_codes.NONEXISTENT_TARGET
        logger.info('Chown: Checking if execution can be skipped, ' + '"%s"' % self.target)
        if ugo.name_to_uid(self.user) == filesys.stat(self.target).st_uid and ugo.name_to_gid(self.group) == filesys.stat(self.target).st_gid:
