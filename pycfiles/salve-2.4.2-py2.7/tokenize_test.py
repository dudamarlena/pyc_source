...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/tests/unit/parser/tokenize_test.py
...
def ensure_TokenizationException(filename):
    ...
    e = ensure_except(tokenize.TokenizationException, tokenize_filename, path)
    assert paths.clean_path(e.file_context.filename) == paths.clean_path(path)
...
empty_block_tys = [
 Token.types.BLOCK_START, Token.types.BLOCK_END]
...
class TestTokenizeMockedGlobals(MockedGlobals):
    ...
    def invalid_id_nofail(self):
        ...
        assert_tokens_match_types(tokens, id_and_start_tys + id_and_template_tys + [Token.types.BLOCK_END] + id_and_empty_block_tys)
    ...
    def attribute_with_spaces(self):
        ...
        assert_tokens_match_types(tokens, id_and_start_tys + 2 * id_and_template_tys + [Token.types.BLOCK_END])
    ...
    def primary_attr_block_followed_by_block(self):
        ...
        assert_tokens_match_types(tokens, id_and_template_tys + id_and_start_tys + 2 * id_and_template_tys + [Token.types.BLOCK_END])
    ...
    def primary_attr_block_empty_body(self):
        ...
        assert_tokens_match_types(tokens, id_and_template_tys + empty_block_tys)
