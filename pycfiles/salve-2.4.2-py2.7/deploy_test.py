...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/tests/unit/cli/deploy_test.py
# Compiled at: 2015-11-14 16:26:10
import mock
from nose.tools import istest
from salve.cli import deploy
from salve.exceptions import SALVEException, ActionException, BlockException, ParsingException, TokenizationException
from salve.context import ExecutionContext, FileContext
from tests.util import ensure_except, ensure_SystemExit_with_code, scratch, assert_substr
startup_v3_warning = 'STARTUP [WARNING] Deprecation Warning: --directory ' + 'will be removed in version 3 as ' + '--version3-relative-paths becomes the default.'
...
class TestWithScratchdir(scratch.ScratchContainer):
    ...
    def deploy_salve_exception(self):
        ...
        assert_substr(self.stderr.getvalue(), 'STARTUP [ERROR] no such file, line -1: message string')
    ...
    def deploy_block_exception(self):
        ...
        assert_substr(err, '[DEBUG] SALVE Execution Phase Transition ' + '[STARTUP] -> [PARSING]')
        assert_substr(err, 'PARSING [ERROR] no such file, ' + 'line -1: message string')
    ...
    def deploy_action_exception(self):
        ...
        with mock.patch('salve.cli.deploy.run_on_manifest', mock_run):
            ensure_SystemExit_with_code(1, deploy.main, fake_args)
        assert_substr(self.stderr.getvalue(), 'COMPILATION [ERROR] no such file, line -1: message string')
    ...
    def deploy_tokenization_exception(self):
        ...
        with mock.patch('salve.cli.deploy.run_on_manifest', mock_run):
            ensure_SystemExit_with_code(1, deploy.main, fake_args)
        assert_substr(self.stderr.getvalue(), 'PARSING [ERROR] no such file, line -1: message string')
