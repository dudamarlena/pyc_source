...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/tests/unit/block/directory_test.py
...
from .helpers import check_list_act, check_dir_create_act, check_dir_chown_act, check_dir_chmod_act, check_file_backup_act, check_file_copy_act, assign_block_attrs

def check_actions_against_defaults(create=None, chmod=None, chown=None, chown_chmod_pair=None):
    if chown_chmod_pair:
        chmod, chown = disambiguate_by_class(modify.DirChmodAction, chown_chmod_pair[0], chown_chmod_pair[1])
    if create:
...
def make_dir_block(action='copy', source='/a/b/c', target='/p/q/r', user='user1', group='nogroup', mode='755'):
...
class TestWithScratchdir(ScratchWithExecCtx):
    ...
    def empty_dir_copy_compile(self):
        ...
        with mock.patch('os.walk', lambda d: []):
            act = make_dir_block(action='copy', mode=None, user=None, group=None).compile()
