...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/salve/action/modify/dir_chmod.py
...
class DirChmodAction(ChmodAction, DirModifyAction):
    ...
    def __str__(self):
        return 'DirChmodAction(target=' + str(self.target) + ',mode=' + ('{0:o}').format(self.mode) + ',recursive=' + str(self.recursive) + ',context=' + repr(self.file_context) + ')'

    def verify_can_exec(self, filesys):
        ExecutionContext().transition(ExecutionContext.phases.VERIFICATION)
        logger.info('DirChmod: Checking if target exists, "%s"' % self.target)
        ...
        logger.info('DirChmod: Checking if user is target owner, ' + '"%s"' % self.target)
        ...
        if self.recursive:
            for directory, subdirs, files in filesys.walk(self.target):
                for sd in subdirs:
                    ...
                    synth = DirChmodAction(target, ('{0:o}').format(self.mode), self.file_context, recursive=False)
                    synth(filesys)

                for f in files:
                    target = paths.pjoin(directory, f)
                    synth = FileChmodAction(target, ('{0:o}').format(self.mode), self.file_context)
                    synth(filesys)