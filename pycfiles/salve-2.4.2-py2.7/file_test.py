...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/tests/unit/block/file_test.py
...
from .helpers import check_list_act, check_file_backup_act, check_file_create_act, check_file_copy_act, check_file_chmod_act, check_file_chown_act, assign_block_attrs

def check_actions_against_defaults(copy=None, create=None, chmod=None, chown=None, backup=None, chown_chmod_pair=None):
    if chown_chmod_pair:
        chmod, chown = disambiguate_by_class(modify.FileChmodAction, chown_chmod_pair[0], chown_chmod_pair[1])
    if copy:
...
def make_file_block(action='copy', source='/a/b/c', target='/p/q/r', user='user1', group='nogroup', mode='600'):
    b = FileBlock(dummy_file_context)
    assign_block_attrs(b, action=action, source=source, target=target, user=user, group=group, mode=mode)
    return b
...
class TestWithScratchdir(ScratchWithExecCtx):
    ...
    def file_copy_compile(self):
        ...
        check_actions_against_defaults(copy=act.actions[1], backup=act.actions[0], chown_chmod_pair=(
         act.actions[2], act.actions[3]))
    ...
    def file_create_compile(self):
        ...
        check_actions_against_defaults(create=act.actions[0], chown_chmod_pair=(
         act.actions[1], act.actions[2]))
    ...
    def file_copy_nouser(self):
        ...
        check_list_act(act, 3)
        check_actions_against_defaults(copy=act.actions[1], backup=act.actions[0], chmod=act.actions[2])
        return
    ...
    def file_create_nouser(self):
        ...
        check_actions_against_defaults(create=act.actions[0], chmod=act.actions[1])
        return
    ...
    def file_copy_nogroup(self):
        ...
        check_list_act(act, 3)
        check_actions_against_defaults(backup=act.actions[0], copy=act.actions[1], chmod=act.actions[2])
        return
    ...
    def file_copy_nomode(self):
        ...
        check_list_act(act, 3)
        check_actions_against_defaults(backup=act.actions[0], copy=act.actions[1], chown=act.actions[2])
        return
    ...
    def file_create_nomode(self):
        ...
        check_list_act(act, 2)
        check_actions_against_defaults(create=act.actions[0], chown=act.actions[1])
        return
    ...
    def file_path_expand_fail_notarget(self):
        ...
        """
        b = make_file_block(action='create', target=None)
        ensure_except(BlockException, b.expand_file_paths, 'file/root/directory')
        return