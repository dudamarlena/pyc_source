...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/tests/unit/action/backup_test.py
...
def patch_filebackup_autoverify_nolog(function):
    return mock.patch.multiple('salve.action.backup.FileBackupAction', verify_can_exec=lambda self, fs: self.verification_codes.OK, write_log=mock.MagicMock())(function)
...
class TestWithScratchdir(scratch.ScratchContainer):
    ...
    def file_target_name(self, mock_cp, mock_mkdir):
        ...
        assert os.path.basename(act.dst) == '9bfabef5ffd7f5df84171393643' + 'e7ceeba916e64876ace612ca8d2' + '0ad0ffd69e0ecd284ca7899f4ba' + 'b6805c06f881296d20f619e714b' + 'efb255e23fdf09ef0eed', act.dst
    ...
    def file_symlink_target_name(self, mock_cp, mock_mkdir):
        ...
        act(ConcreteFilesys())
        mock_mkdir.assert_called_once_with('/etc/salve/backup/files')
        assert os.path.basename(act.dst) == '55ae75d991c770d8f3ef07cbfde' + '124ffce9c420da5db6203afab70' + '0b27e10cf9'
        cp_act = mock_cp.call_args[0][0]
    ...
    def backupaction_is_abstract(self):
        ...
        """
        ensure_except(TypeError, backup.BackupAction, '/a/b/c', dummy_file_context)
    ...
    def file_to_str(self):
        ...
        filename = full_path('file1.txt')
        act = backup.FileBackupAction(filename, dummy_file_context)
        assert str(act) == 'FileBackupAction(src=' + filename + ',backup_dir=' + '/etc/salve/backup,backup_log=/etc/salve/backup.log' + ',context=' + str(dummy_file_context) + ')'
    ...
    def dir_verify_no_source(self):
        ...
        assert act.verify_can_exec(ConcreteFilesys()) == backup.DirBackupAction.verification_codes.NONEXISTENT_SOURCE
    ...
    def dir_execute_no_source(self):
        ...
        expected = 'VERIFICATION [WARNING] ' + 'no such file: DirBackup: Non-Existent source dir ' + '"%s"\n' % dirname
