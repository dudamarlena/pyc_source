...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/tests/unit/action/create_test.py
...
def filecreate_to_str():
    ...
    assert str(fc) == 'FileCreateAction(dst=a,context=' + repr(dummy_file_context) + ')'
...
def dircreate_to_str():
    ...
    assert str(dc) == 'DirCreateAction(dst=a,context=' + repr(dummy_file_context) + ')'