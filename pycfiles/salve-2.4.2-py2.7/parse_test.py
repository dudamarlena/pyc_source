...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/tests/unit/parser/parse_test.py
...
def ensure_ParsingException(tokens=None, filename=None):
    if tokens and filename:
        raise ValueError('Invalid test: uses both tokens list and ' + 'filename in ensure_ParsingException()')
    e = None
    if tokens:
        e = ensure_except(parse.ParsingException, parse.parse_tokens, tokens)
    elif filename:
        e = ensure_except(parse.ParsingException, parse_filename, full_path(filename))
    else:
        assert False
    assert filename is None or paths.clean_path(e.file_context.filename) == paths.clean_path(full_path(filename))
    return
