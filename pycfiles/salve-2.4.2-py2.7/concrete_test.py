...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/tests/unit/filesys/concrete_test.py
# Compiled at: 2015-11-06 23:45:35
import os, hashlib, mock
...
class TestWithScratchdir(scratch.ScratchContainer):
    ...
    def file_chmod(self):
        ...
        assert not fs.exists(full_name)
        fs.touch(full_name)
        fs.chmod(full_name, 425)
        st_result = fs.stat(full_name)
        assert hasattr(st_result, 'st_mode')
        assert st_result.st_mode & 511 == 425, oct(st_result.st_mode)
        full_name = self.get_fullname('b')
        assert not fs.exists(full_name)
        fs.touch(full_name)
        fs.chmod(full_name, 350)
        st_result = fs.stat(full_name)
        assert hasattr(st_result, 'st_mode')
        assert st_result.st_mode & 511 == 350, oct(st_result.st_mode)
