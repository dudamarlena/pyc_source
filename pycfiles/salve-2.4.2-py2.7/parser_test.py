...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/tests/integration/parser_test.py
...
def assert_parsing_fails(name, lineno=None, exception_type=TokenizationException):
    path = full_path(name)
    e = ensure_except(exception_type, parse_filename, path)
    sctx = e.file_context
    assert lineno and sctx.lineno == lineno
...
class TestWithScratchContainer(scratch.ScratchContainer):
    ...
    def missing_value_raises_TE(self):
        ...
        verifies the context of the raised exception.
        """
        assert_parsing_fails('missing_attr_val.manifest', lineno=5)
    ...
    def double_open_raises_TE(self):
        ...
        verifies the context of the raised exception.
        """
        assert_parsing_fails('double_open.manifest', lineno=3)
    ...
    def invalid_block_id_raises_PE(self):
        ...
        verifies the context of the raised exception.
        """
        assert_parsing_fails('invalid_block_id.manifest', lineno=7, exception_type=ParsingException)