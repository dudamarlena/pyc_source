...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/tests/unit/action/copy_test.py
...
class TestWithScratchdir(scratch.ScratchContainer):
    ...
    def __init__(self):
        ...
        os.chmod(self.get_fullname('r_only_file_1'), 292)
        self.make_dir('no_perms_dir_1')
        os.chmod(self.get_fullname('no_perms_dir_1'), 0)
        self.write_file('no_perms_file_1', 'file with no allowed perms')
        os.chmod(self.get_fullname('no_perms_file_1'), 0)
    ...
    def filecopy_execute(self):
        ...
        """
        fcp = copy.FileCopyAction(self.get_fullname('rw_file_1'), os.path.join(self.get_fullname('rw_dir_1'), 'c'), self.dummy_file_context)
        fcp(ConcreteFilesys())
    ...
    def canexec_unwritable_target(self):
        ...
        """
        fcp = copy.FileCopyAction(self.get_fullname('rw_file_1'), self.get_fullname('r_only_file_1'), self.dummy_file_context)
        assert fcp.verify_can_exec(ConcreteFilesys()) == fcp.verification_codes.UNWRITABLE_TARGET
    ...
    def canexec_unwritable_target_dir(self):
        ...
        """
        fcp = copy.FileCopyAction(self.get_fullname('rw_file_1'), self.get_fullname('no_perms_dir_1/otherfile'), self.dummy_file_context)
        assert fcp.verify_can_exec(ConcreteFilesys()) == fcp.verification_codes.UNWRITABLE_TARGET
    ...
    def canexec_unreadable_source(self):
        ...
        """
        fcp = copy.FileCopyAction(self.get_fullname('no_perms_file_1'), self.get_fullname('rw_file_1'), self.dummy_file_context)
        assert fcp.verify_can_exec(ConcreteFilesys()) == fcp.verification_codes.UNREADABLE_SOURCE
    ...
    def dir_canexec_unreadable_source(self):
        ...
        """
        dcp = copy.DirCopyAction(self.get_fullname('no_perms_dir_1'), self.get_fullname('rw_dir_1'), self.dummy_file_context)
        assert dcp.verify_can_exec(ConcreteFilesys()) == dcp.verification_codes.UNREADABLE_SOURCE
    ...
    def dir_canexec_unwritable_target(self):
        ...
        """
        dcp = copy.DirCopyAction(self.get_fullname('rw_dir_1'), self.get_fullname('no_perms_dir_1/abc'), self.dummy_file_context)
        assert dcp.verify_can_exec(ConcreteFilesys()) == dcp.verification_codes.UNWRITABLE_TARGET
    ...
    def stringification_test_generator(self):
        ...
        for klass, name in class_tuples:
            ...
            def check_func():
                ...
                assert str(act) == ('{0}(src=a,dst=b/c,context={1})').format(name, repr(self.dummy_file_context))
    ...
    @istest
    @mock.patch('salve.action.copy.DirCopyAction.verify_can_exec', lambda self, fs: self.verification_codes.UNREADABLE_SOURCE)
    def dircopy_unreadable_source_execute(self):
        """
        Unit: Directory Copy Action Execution, Unreadable Source
        """
        ExecutionContext().transition(ExecutionContext.phases.EXECUTION, quiet=True)
        dcp = copy.DirCopyAction('a', 'b/c', self.dummy_file_context)
        dcp(ConcreteFilesys())
        err = self.stderr.getvalue()
        expected = 'EXECUTION [WARNING] no such file: ' + 'DirCopy: Non-Readable source directory "a"\n'
        assert_substr(expected, err)

    @istest
    @mock.patch('salve.action.copy.DirCopyAction.verify_can_exec', lambda self, fs: self.verification_codes.UNWRITABLE_TARGET)
    def dircopy_unwritable_target_execute(self):
        ...
        dcp(ConcreteFilesys())
        err = self.stderr.getvalue()
        expected = 'EXECUTION [WARNING] no such file: ' + 'DirCopy: Non-Writable target directory "b/c"\n'
        assert_substr(expected, err)