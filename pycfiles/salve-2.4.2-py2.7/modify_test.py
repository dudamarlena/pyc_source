...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/tests/unit/action/modify_test.py
...
def mock_uid_and_gid(uid, gid):
    ...
    def wrapper(f):
        return mock.patch('salve.ugo.name_to_uid', lambda x: uid)(mock.patch('salve.ugo.name_to_gid', lambda x: gid)(f))
...
class TestWithScratchdir(scratch.ScratchContainer):
    ...
    def filechown_verify_nonroot(self, mock_stat):
        ...
        act = modify.FileChownAction(a_name, 'user1', 'nogroup', self.file_context)
    ...
    def filechmod_verify_nonowner(self):
        ...
        """
        ExecutionContext().transition(ExecutionContext.phases.VERIFICATION, quiet=True)
        self.write_file('a', '')
        a_name = self.get_fullname('a')
    ...
    def dirchown_verify_nonroot(self, mock_stat):
        ...
        a_name = self.get_fullname('a')
        ensure_uid_and_gid_mismatch(mock_stat)
        act = modify.DirChownAction(a_name, 'user1', 'nogroup', self.file_context)
        code = act.verify_can_exec(ConcreteFilesys())
        assert code == act.verification_codes.NOT_ROOT, str(code)
    ...
    def dirchown_execute_nonroot(self, mock_verify, mock_lchown):
        ...
        act = modify.DirChownAction('a', 'user1', 'nogroup', self.file_context, recursive=True)
        mock_verify.return_value = act.verification_codes.NOT_ROOT
        act(ConcreteFilesys())
        assert not mock_lchown.called
        err = self.stderr.getvalue()
        expected = 'EXECUTION [WARNING] DirChown: Cannot Chown as Non-Root User'
        assert_substr(err, expected)
    ...
    def dirchmod_execute_nonowner(self, mock_verify, mock_chmod):
        ...
        """
        ExecutionContext().transition(ExecutionContext.phases.EXECUTION, quiet=True)
        act = modify.DirChmodAction('a', '755', self.file_context, recursive=True)
        mock_verify.return_value = act.verification_codes.UNOWNED_TARGET
        act(ConcreteFilesys())
    ...
    @mock.patch('os.chmod')
    @mock.patch('salve.action.modify.DirChmodAction.verify_can_exec')
    def dirchmod_execute_nonrecursive_nonroot_nonowner(self, mock_verify, mock_chmod):
        """
    ...
    def filechmod_verify_root(self):
        ...
        assert act.verify_can_exec(ConcreteFilesys()) == act.verification_codes.OK
    ...
    def filechown_verify(self, mock_stat):
        ...
        """
        act = modify.FileChownAction('a', 'user1', 'nogroup', self.file_context)
        ensure_uid_and_gid_mismatch(mock_stat)
        assert act.verify_can_exec(ConcreteFilesys()) == act.verification_codes.OK
    ...
    def stringification_test_generator(self):
        ...
        for klass, name in class_tuples:
            ...
            def check_func():
                ...
                assert str(act) == ('{0}({1},context={2})').format(name, (',').join([ ('{0}={1}').format(k, v) for k, v in args ]), repr(self.file_context))
