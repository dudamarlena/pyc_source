...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/tests/system/dir_test.py
...
class TestWithScratchdir(system.RunScratchContainer):
    ...
    def copy_unwritable_target_parent(self):
        ...
        os.chmod(fullname, 256)
        self.run_on_manifest('1.man')
        assert self.exists('2')
        assert not self.exists('2/1')
        assert len(self.listdir('2')) == 0
        err = self.stderr.getvalue()
        expected = ('VERIFICATION [WARNING] %s, line 1: DirCreate: ' + 'Non-Writable target dir "%s"\n') % (
