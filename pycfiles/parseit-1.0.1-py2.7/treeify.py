...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\parseit\treeify.py
# Compiled at: 2017-11-14 21:04:57
...
def color_name(name):
    rgb = (
     hash(name) % 255, (hash(name) >> 4) % 255, (hash(name) >> 8) % 255)
...
def add_nodes(graph, result):
    ...
    for child in result[1]:
        if child[0].isupper():
            child_node = pydot.Node(('TOKEN{}').format(n), label=("{}('{}')").format(child[0], child[1]), fillcolor=color_name(child[0])[0], style='filled', fontcolor=color_name(child[0])[1])
            n += 1
