...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\parseit\parseit.py
# Compiled at: 2017-11-15 18:57:48
...
def lex(input_str, token_types):
    ...
    while True:
        ...
        if length == 0:
            throw_parse_error(original_input, pos, 'Invalid token')
        pos += length
...
def consume_bare_rulesegment(input_str, rulesegment, tokens, at, rules, token_types):
    ...
    if rulesegment.isupper():
        if head[0] == rulesegment:
            ...
            throw_parse_error(input_str, head[2], ('Unexpected token: expected {}, got {}').format(rulesegment, head[0]))
    else:
