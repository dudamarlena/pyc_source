...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/formic/formic.py
# Compiled at: 2013-07-14 07:44:57
...
def is_root(directory):
    r"""Returns true if the directory is root (eg / on UNIX or c:\ on Windows)"""
...
class Matcher(object):
    ...
    def __ne__(self, other):
        return not isinstance(other, type(self)) or self.pattern != other.pattern
...
class Pattern(object):
    ...
    * *sections*: A list of :class:`Section` instances. Each :class:`Section`
    ...
    def _simplify(elements):
        ...
        for element in elements:
            if element == '..':
                raise FormicError(("Invalid glob: Cannot have '..' in a glob: {0}").format(('/').join(elements)))
            elif element == '.':
                pass
...
class PatternSet(object):
    ...
    def __str__(self):
        return ('PatternSet (All files? {0}) [{1}] ').format(self.all_files(), (', ').join(str(pat) for pat in self.patterns))
    ...
    **exclude**.
    ...
    def __init__(self, label, directory, based_on=None, unmatched=None):
        ...
        if based_on:
            ...
            self.parent = None
        self.matched_inherit = PatternSet()
        self.matched_and_subdir = PatternSet()
        self.matched_no_subdir = PatternSet()
        self.unmatched = PatternSet()
        if self.parent:
            self.unmatched.extend(self.parent.matched_and_subdir)
            self.unmatched.extend(self.parent.unmatched)
            self.parent_has_patterns = self.parent.parent_has_patterns or not self.parent.matched_inherit.empty()
        else:
    ...
    def _find_parent(self, path_elements):
        ...
        if not self.path_elements:
            ...
            if self.path_elements == path_elements[0:len(self.path_elements)]:
                return self
    ...
    def match(self, files):
        ...
        if not files:
            return set()
        if self.matched_inherit.all_files() or self.matched_and_subdir.all_files() or self.matched_no_subdir.all_files():
            return set(files)
        unmatched = set(files)
        matched = set()
    ...
    def no_possible_matches_in_subdirs(self):
        ...
        return not self.parent_has_patterns and self.matched_inherit.empty() and self.matched_and_subdir.empty() and self.unmatched.empty()

    def __str__(self):
        return ('FileSetState {0} in {1}/:\n\tInherit: {2}\n\tthis&subdir: {3}\n\tthis-only: {4}\n\tunmatched: {5}').format(self.label, ('/').join(self.path_elements), self.matched_inherit, self.matched_and_subdir, self.matched_no_subdir, self.unmatched)


def get_initial_default_excludes():
    r"""Returns a the default excludes as a list of Patterns.
...
class FileSet(object):
    r"""An implementation of the Ant FileSet class.
    ...
    * You can provide and alternate function to :func:`os.walk()` that, for
    ...
    def __init__(self, include, exclude=None, directory=None, default_excludes=True, walk=walk, symlinks=True):
        self.include = FileSet._preprocess(include)
        if not self.include:
            raise FormicError('No include globs have been specified- nothing to find')
    ...
    def _receive(self, root, directory, dirs, files, include, exclude):
        ...
        if not self.symlinks:
            ...
            files = [ file_name for file_name in files if not path.islink(where + file_name)
                    ]
    ...
    def files(self):
        ...
        for root, dirs, files in self.walk(directory):
            ...
            matched, include, exclude = self._receive(directory, rel_dir_name, dirs, files, include, exclude)
    ...
    def __str__(self):
        return ('FileSet [directory={0}, include={1}, exclude={2}, symlinks? {3}]').format(self.directory if self.directory else ('CWD ({0})').format(getcwd()), self.include.patterns, self.exclude.patterns, self.symlinks)