...
# Embedded file name: /home/hvelarde/forcontent/idg/src/brasil.gov.portal/src/brasil/gov/portal/tests/test_init_content.py
...
class InitContentTestCase(unittest.TestCase):
    ...
    def test_conteudos_publicados(self):
        ...
        for oId in ids:
            o = self.portal[oId]
            self.assertEqual(self.wt.getInfoFor(o, 'review_state'), 'published')

    @unittest.expectedFailure
    def test_assuntos_available(self):
        self.assertTrue('assuntos' in self.portal.objectIds(), 'Pasta Assuntos não disponível')
        pasta = self.portal['assuntos']
        self.assertEqual('Assuntos', pasta.title, 'Título não aplicado')
    ...
    def test_assuntos_ordering(self):
        ...
        self.assertEqual(oId, pasta.objectIds()[(-1)], 'Ordenação não aplicada')

    @unittest.expectedFailure
    def test_imagens_available(self):
        self.assertTrue('imagens' in self.portal.objectIds(), 'Pasta Imagens não disponível')
        pasta = self.portal['imagens']
        self.assertEqual('Imagens', pasta.title, 'Título não aplicado')
    ...
    def test_servicos_available(self):
        self.assertTrue('servicos' in self.portal.objectIds(), 'Pasta Servicos não disponível')
        pasta = self.portal['servicos']
        self.assertEqual('Serviços', pasta.title, 'Título não aplicado')
    ...
    def test_acesso_a_informacao_available(self):
        self.assertTrue('acesso-a-informacao' in self.portal.objectIds(), 'Conheça este órgão')
        pasta = self.portal['acesso-a-informacao']
        self.assertEqual('Acesso à Informação', pasta.title, 'Título não aplicado')
    ...
    def test_folders_in_root_are_plone_default_ordering(self):
        ...
        """
        all_ordered = all([ isinstance(self.portal[pasta_id].getOrdering(), DefaultOrdering) for pasta_id in self.portal.objectIds() if IFolder.providedBy(self.portal[pasta_id])
                          ])
    ...
    def test_portlet_menu_de_relevancia(self):
        ...
        coluna = '++contextportlets++plone.leftcolumn'
        mapping = portal.restrictedTraverse(coluna)
        self.assertEqual(mapping['relevancia'].root, '/menu-de-relevancia')
        self.assertEqual(mapping['relevancia'].name, '')
    ...
    def test_portlet_assuntos(self):
        ...
        coluna = '++contextportlets++plone.leftcolumn'
        mapping = portal.restrictedTraverse(coluna)
        self.assertEqual(mapping['assuntos'].root, '/assuntos')
        self.assertEqual(mapping['assuntos'].name, 'Assuntos')
    ...
    def test_portlet_acesso_a_informacao(self):
        ...
        coluna = '++contextportlets++plone.leftcolumn'
        mapping = portal.restrictedTraverse(coluna)
        self.assertEqual(mapping['acesso-a-informacao'].root, '/acesso-a-informacao')
        self.assertEqual(mapping['acesso-a-informacao'].name, 'Acesso à Informação')
        self.assertEqual(mapping['acesso-a-informacao'].currentFolderOnly, False)
    ...
    def test_eventos_available(self):
        ...
        self.assertIn('eventos', self.portal.objectIds(), 'Pasta Eventos não disponível')
        folder = self.portal['eventos']
        self.assertEqual('Eventos', folder.title, 'Título não aplicado')
    ...
    def test_eventos_created(self):
        ...
        folder = self.portal['eventos']
        eventos = [
         {'id': 'evento-1', 'start': '2017-01-01 10:00:00', 
            'end': '2017-01-01 11:00:00'},
         {'id': 'evento-2', 'start': '2017-01-05 11:00:00', 
            'end': '2017-01-06 12:00:00'},
         {'id': 'evento-3', 'start': '2017-01-10 12:00:00', 
            'end': '2017-01-11 13:00:00'}]
        d_format = '%Y-%m-%d %H:%M:%S'
        for evento in eventos:
            obj_evento = folder[evento['id']]
            self.assertEqual(evento['start'], obj_evento.start.strftime(d_format))
            self.assertEqual(evento['end'], obj_evento.end.strftime(d_format))
