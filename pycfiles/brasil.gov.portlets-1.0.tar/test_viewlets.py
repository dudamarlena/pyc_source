...
# Embedded file name: /home/hvelarde/forcontent/idg/src/brasil.gov.portal/src/brasil/gov/portal/tests/test_viewlets.py
...
class LogoViewletTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.portal.title = 'Portal Brasil'
        self.portal.title_1 = 'Portal'
        self.portal.title_2 = 'Brasil'
        self.portal.orgao = 'Presidencia da República'
        self.portal.description = 'O portal do Brasil'
    ...
    def test_title_2_class(self):
        ...
        self.portal.title_2 = 'Desenvolvimento Social e Combate à Fome'
        viewlet = self.viewlet()
...
class ServicosViewletTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with api.env.adopt_roles(['Manager']):
            ...
            api.content.create(type='Link', container=self.servicos, id='servico-1', title='Servico 1', remoteUrl='http://www.google.com')
            api.content.create(type='Link', container=self.servicos, id='servico-2', title='Servico 2', remoteUrl='http://www.plone.org')
    ...
    def test_servicos(self):
        ...
        self.assertEqual(servicos[0].Title, 'Servico 1')
        self.assertEqual(servicos[1].Title, 'Servico 2')

    def test_render(self):
        viewlet = self.viewlet()
        render = viewlet.render()
        self.assertIn('http://www.google.com', render)
        self.assertIn('http://www.plone.org', render)
...
class NITFBylineViewletTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.request = self.layer['request']
        with api.env.adopt_roles(['Manager']):
            properties = dict(fullname='Machado de Assis', location='Cosme Velho')
            api.user.create(username='machado', email='machado@brasil.gov.br', properties=properties)
            self.conteudo = api.content.create(type='collective.nitf.content', container=self.portal, id='minha-noticia')
            self.conteudo.byline = 'Machado de Assis'
    ...
    def test_autor_inexistente(self):
        self.conteudo.byline = 'Erico Verissimo'
        viewlet = self.viewlet()
    ...
    def test_authorname(self):
        viewlet = self.viewlet()
        self.assertEqual(viewlet.authorname(), 'Machado de Assis')

    def test_mostra_autor(self):
        u"""Testa se o método retorna configuração para esconder o autor."""
        viewlet = self.viewlet()
    ...
    def test_mostra_data(self):
        u"""Testa se o método retorna configuração para esconder a data."""
        viewlet = self.viewlet()
    ...
    def test_esconde_autor(self):
        u"""Testa se o autor deixa de ser exibido se configuração para esconder
    ...
    def test_esconde_data(self):
        u"""Testa se a data deixa de ser exibida se configuração para esconder
...
class DocumentBylineViewletTestCase(unittest.TestCase):
    ...
    def setUp(self):
        u"""Cria uma página para testes."""
        self.portal = self.layer['portal']
        self.request = self.layer['request']
        with api.env.adopt_roles(['Manager']):
            self.conteudo = api.content.create(type='Document', container=self.portal, id='minha-pagina')

    def viewlet(self):
        """Retorna a viewlet DocumentBylineViewlet."""
        viewlet = DocumentBylineViewlet(self.conteudo, self.request, None, None)

        @implementer(IViewView)
        class Parent(object):
            u"""Classe que simula um parent. Para que a viewlet
    ...
    def test_exibe_tracos(self):
        u"""Confirma que a viewlet não está exibindo dois traços, quando a data
        ...
        with api.env.adopt_roles(['Manager']):
            ...
            viewlet = self.viewlet()
            render = viewlet.render()
            self.assertEqual(render.count('—'), 1)
            esconde_autor()
            render = viewlet.render()
            self.assertEqual(render.count('—'), 0)
