...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pandaspipe/merger.py
# Compiled at: 2016-04-19 05:11:22
import inspect, logging, pandas as pd
...
class Merger(PipelineEntity):
    ...
    def __call__(self, *dfs):
        ...
        if len(rest_channels) != 0 and self._simple_merger:
            output_simple_channel = filter(lambda output_channel: len(filter(lambda merger_tuple: output_channel in merger_tuple[2], self._mergers)) == 0, self.output_channels)[0]
    ...
    def register(self, pipeline):
        ...
        merge_functions = inspect.getmembers(self, predicate=lambda func: isinstance(func, inspect.types.MethodType) and getattr(func, 'is_merger', False))
