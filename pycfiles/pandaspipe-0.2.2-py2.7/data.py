...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pandaspipe/data.py
# Compiled at: 2016-04-19 05:11:22
import pandas as pd, sys, logging, inspect
...
class DataSource(PipelineEntity):
    ...
    def register(self, pipeline):
        ...
        import_functions = inspect.getmembers(self, predicate=lambda func: isinstance(func, inspect.types.MethodType) and getattr(func, 'is_importing', False))
...
class DataOutlet(PipelineEntity):
    ...
    def register(self, pipeline):
        _log.info('Register DataSource %s in %s pipeline' % (self.__class__.__name__, pipeline.name))
        export_functions = inspect.getmembers(self, predicate=lambda func: isinstance(func, inspect.types.MethodType) and getattr(func, 'is_exporting', False))
