...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pandaspipe/pipeline.py
# Compiled at: 2016-04-19 05:11:22
import abc, sys, inspect, types, itertools, networkx as nx
        ...
        *Description*
        ...
        while len(active_nodes) > 0:
            ...
            for channel in active_dfs.keys():
                if channel not in output_channels and len([ active_node for active_node in active_nodes if channel in active_node.input_channels ]) == 0:
...
class GraphBuilder:
    ...
    def _build_inchannel_connections(self):
        all_channels = set(itertools.chain(*map(lambda x: set(itertools.chain(x.input_channels, x.output_channels)), self.entities)))
        for channel in all_channels:
            channel_nodes = filter(lambda x: x.type == 'node' and channel in x.input_channels and channel in x.output_channels, self.entities)
    ...
    def _build_multichannel_connections(self):
        for node in filter(lambda x: x.type in ('bignode', 'node') and x.input_channels != x.output_channels, self.entities):
