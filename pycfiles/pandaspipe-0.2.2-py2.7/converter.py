...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pandaspipe/converter.py
...
def converter(func=None, target=None):
    ...
    def process(dec_func):
        ...
        dec_func.column_converter_info.append(target if target is not None else dec_func.__name__)
        return dec_func
...
class Converter(PipelineEntity):
    ...
    def register(self, pipeline):
        ...
        map_functions = inspect.getmembers(self, predicate=lambda func: isinstance(func, inspect.types.MethodType) and getattr(func, 'is_converter', False))
