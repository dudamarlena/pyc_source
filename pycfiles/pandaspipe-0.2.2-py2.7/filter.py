...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pandaspipe/filter.py
# Compiled at: 2016-04-19 05:11:22
...
def columns_condition(func=None, required_columns=()):
    ...
    def process(dec_func):
        ...
        dec_func.columns_condition_info.append(required_columns if len(required_columns) != 0 else dec_func.func_code.co_varnames[1:])
        return dec_func
...
class Filter(PipelineEntity):
    ...
    def register(self, pipeline):
        ...
        map_functions = inspect.getmembers(self, predicate=lambda func: isinstance(func, inspect.types.MethodType) and getattr(func, 'is_condition', False))
    ...
    def __call__(self, df):
        ...
        for condition_function, target, direct_use in self.column_conditions:
            if direct_use:
                ...
                new_index = filter(lambda i: i is not None, map(lambda i, *args: i if condition_function(*args) else None, df.index, df[target]))
                df = df.loc[new_index]

        for condition_function, condition_columns in self.columns_conditions:
            new_index = filter(lambda i: i is not None, map((lambda i, *args: i if condition_function(*args) else None), df.index, *[ df[column] for column in columns if column in condition_columns
                                                                                                                                    ]))
