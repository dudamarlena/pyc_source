...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pandaspipe/mapper.py
...
class Mapper(PipelineEntity):
    ...
    def register(self, pipeline):
        ...
        map_functions = inspect.getmembers(self, predicate=lambda func: isinstance(func, inspect.types.MethodType) and getattr(func, 'is_mapping', False))
    ...
    def __call__(self, df):
        ...
        if len(df) != 0:
            for map_function, result, map_columns in self.reduce_mapping:
                ...
                new_df[result] = map(map_function, *args)
