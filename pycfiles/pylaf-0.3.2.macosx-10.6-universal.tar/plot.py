...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.6/site-packages/PyLAF/plot.py
# Compiled at: 2010-08-23 23:01:39
...
class Plot(BasePlot):
    ...
    def clear_line(self):
        while len(self.ax.lines):
            ...
            line._lineFunc = None
            for (key, item) in self.ax.xaxis.callbacks.callbacks['units'].items():
                if item == line.recache:
                    del self.ax.xaxis.callbacks.callbacks['units'][key]

            for (key, item) in self.ax.yaxis.callbacks.callbacks['units'].items():
                if item == line.recache:
                    del self.ax.yaxis.callbacks.callbacks['units'][key]

        self.ax.ignore_existing_data_limits = True
        self.ax._get_lines._clear_color_cycle()
        return
    ...
    def _changed_autoscale(self):
        ...
        if not self._passed_changed_xylim:
            self._sig_in()

    def _update_xlim(self, e):
        self._passed_update_xlim = True
        xlim = self.ax.get_xlim()
        self.xmin.set(xlim[0])
        self.xmax.set(xlim[1])
        self._passed_update_xlim = False

    def _update_ylim(self, e):
        self._passed_update_ylim = True
        ylim = self.ax.get_ylim()
        self.ymin.set(ylim[0])
        self.ymax.set(ylim[1])
        self._passed_update_ylim = False
...
class Constellation(Plot):
    ...
    def __init__(self, master=None, cnf={}, **kw):
        if not kw.has_key('figsize'):
            kw['figsize'] = (4, 4)
        Plot.__init__(self, master, cnf, **kw)
        self.samplerate = Port(1.0).bind(self._sig_in)
        self.alpha = Port(1.0).bind(self._sig_in)
        self.begin = Port(0.0).bind(self._sig_in)
        self.end = Port(100.0).bind(self._sig_in)
...
class Oscilloscope(Plot):
    ...
    def _sig_in(self):
        ...
        time = 1.0 / samplerate * arange(sig.shape[(-1)])
        self.clear()
...
class Spectrum(Plot):
    ...
    def _sig_in(self):
        ...
        try:
            ...
            freq = fftpack.fftshift(fftpack.fftfreq(sig.shape[(-1)], 1.0 / samplerate))
            value = 10 * log10(real(v * v.conj()) / 2 / 50) - 30
            self.clear()
            for v in value:
                self.ax.plot(freq, v, self.style.get())
...
if __name__ == '__main__':
    ...
    o = App(master, Spectrum)
    o.pack()
    osc = App(Toplevel(master), Oscilloscope)
    osc.pack()
    sms = App(Toplevel(master), Smith)
    sms.pack()
    sms.sig_in.set(rand(1000) + complex(0.0, 1.0) * rand(1000))
    o.sig_in.link(osc.sig_in)
    o.samplerate.link(osc.samplerate)
    o.samplerate.set(250.0)
    osc.xmin.set(0)
    osc.xmax.set(1000)
    d = 1.0 / o.samplerate.get()
    t = arange(1000000.0) * d
    y = 1000.0 * sin(2 * pi * 9.12 * t)
    y = y + 787.3 * (sin(2 * pi * 10 * t) > 0.0)
    o.sig_in.set(y)
    o = App(Tkinter.Toplevel(master), Plot3D)
    o.pack()
    Tkinter.mainloop()