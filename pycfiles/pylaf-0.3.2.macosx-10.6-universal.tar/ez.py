...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.6/site-packages/PyLAF/ez.py
# Compiled at: 2011-03-16 07:50:57
import Tkinter, ttk, core, gui, mpl, uuid, inspect
PRIMARY_PORTS = {gui.Button: 'value', gui.Entry: 'value', 
   gui.Label: 'value', 
   gui.EntryTable: 'value', 
   gui.LabelTable: 'value', 
   gui.MultipleEntries: 'value', 
   mpl.BasePlot: 'sigin', 
   mpl.Oscilloscope: 'sigin', 
   mpl.Gauss: 'sigin', 
   mpl.Spectrum: 'sigin'}
...
class FactoryPort:
    ...
    @classmethod
    def new_context(cls):
        cls.CONTEXT.insert(0, (cls.SEQUENCE, cls.ITEMS))
        cls.SEQUENCE = []
        cls.ITEMS = {}

    @classmethod
    def resume(cls):
        context = cls.CONTEXT.pop(0)
        (cls.SEQUENCE, cls.ITEMS) = context
    ...
    def grid(self, **kw):
        ...
        def option(row=0, column=0, rowspan=0, columnspan=0, **kw):
            return kw
...
class Grid(gui.Grid):
    ...
    def _initialize(cls):
        ...
        try:
            ...
            cls._primary = {gui.Button: 'value', gui.Entry: 'value', 
               gui.Label: 'value', 
               gui.EntryTable: 'value', 
               gui.LabelTable: 'value', 
               gui.MultipleEntries: 'value'}
    ...
    @classmethod
    def _append(cls, klass, name=None, label=None, map=None, geometry=None, label_widget=None, label_geometry=None, cnf=None, *args, **kwargs):
        cls._ezcounter += 1
        try:
            cls._ezlock
        except AttributeError:
            ...
            option = (
             args, kwargs)
            cls._ezconfig.append((klass, name, option, label, map, geometry, label_widget, label_geometry))

        klass = cls._ezconfig[(cls._ezcounter - 1)][0]
        map = cls._ezconfig[(cls._ezcounter - 1)][4]
        if not cls._primary.has_key(klass):
            return
        else:
            if not map.has_key(cls._primary[klass]):
                return
            return map[cls._primary[klass]]
    ...
    def _genport(cls, component, id, value, link, bind):
        ...
        if component == None:
            ...
            setattr(component, id, core.Port(value))

            def plink(src, target):
                if src.get() == None:
                    target.link(src, set=True)
                else:
                    src.link(target)
                return
            ...
            if not link == None:
                if type(link) == tuple:
                    for o in link:
                        plink(getattr(component, id), o)

                else:
                    plink(getattr(component, id), link)
            if bind == None:
                try:
                    bind = getattr(component, '_%s' % id)
                except AttributeError:
                    return

            if type(bind) == tuple:
                for o in bind:
                    getattr(component, id).bind(o)
    ...
    @classmethod
    def append(cls, klass, name=None, label=None, map=None, component=None, value=None, link=None, bind=None, geometry=None, label_widget=None, label_geometry=None, cnf=None, *args, **kwargs):
        u"""
        ウィジェットの生成ルールを登録する。
        ...
        **kwargs:       ウィジェット生成オプション。masterは無効。
        ...
        if not id == None:
            cls._genport(component, id, value, link, bind)
        return id
...
class Plot(Tkinter.Frame):
    ...
    @classmethod
    def newComponent(cls, component):
        ...
        for port in FactoryPort.SEQUENCE:
            ...
            if kwargs.has_key('name'):
                ...
                cname = extract_name(port)
            if not component.children.has_key(cname):
                klass = FactoryPort.ITEMS[port]['widget_klass']
                FactoryPort.new_context()
                child = klass(component, name=cname)
                FactoryPort.resume()

                def option(name=None, frametext=None, **kw):
                    return kw

                for key in option(**kwargs):
                    getattr(child, key).set(kwargs[key])
    ...
    def __init__(self, master=None, cnf={}, **kw):
        ...
        for (klass, name, map, kw) in self._ezconfig:
            if kw.has_key('name'):
                ...
                cname = name
            gui.Layout.Embed(self, klass, name=cname, text=name).pack(side=Tkinter.LEFT)

    def assign(self, component):

        def text(frametext=None, **kw):
            return frametext
        ...
        for (klass, name, map, kw) in self._ezconfig:
            if kw.has_key('name'):
                ...
                cname = name
            if component.children.has_key(cname):
                child = component.children[cname]
            else:
                child = klass(component, name=cname)

                def option(frametext=None, **kw):
                    return kw

                for key in option(**kw):
                    getattr(child, key).set(kw[key])

                self.children[cname].assign(child)
            s = text(**kw)
            if s:
                self.children[cname].configure(text=s)
            if map == None:
                map = {}
            if self._primary.has_key(klass):
                primary = self._primary[klass]
                if not map.has_key(primary):
                    map[primary] = name
            for key in map:
                dest = getattr(child, key)
...
if __name__ == '__main__':
    ...
    class BasicSample(gui.ComponentWithGUI):
        ...
        def __init__(self, master=None, name=None):
            ...
            self.entryq = core.Port(0.0).bind(self._entry)
            self.Console.append(gui.Entry, label='entry', name='entry', component=self, value=0.0)
            self.Console.append(gui.Entry, label='entry', name='entry1', component=self, value=0.0, bind=self._entry)
            self.Console.append(gui.Entry, label='entry', name='entrxy', map=core.kw(value='entry'))
            self.entry = core.Port(0.0).bind(self._entry)
            print self.Console.append(gui.Entry, name='1entry', label='entry', component=self, value=0.0)
            print self.Console.append(gui.Entry, name='entry2', label='entry', component=self, value=0.0)
            self.id = self.Console.append(gui.Entry, label='entry', component=self)
            self.Console.append(gui.Button, text='実行', component=self, bind=self._button)
            id = self.Console.append(gui.Button, text='実行')
            ...
            class Test(core.Component):
                ...
                def __init__(self, master=None, name=None):
                    ...
                    self.value = core.Port(0.0).bind(self._value)
                    self.value2 = core.Port(1.0).bind(self._value2)
                    self.value3 = core.Port(2.0).bind(self._value3)

                def _value(self):
                    print 'value:', self.value.get()

                def _value2(self):
                    print 'value2:', self.value2.get()

                def _value3(self):
                    print 'value3:', self.value3.get()

            class MyEntry(gui.Entry):
                pass
