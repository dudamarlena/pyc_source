...
# [GCC 8.4.0]
# Embedded file name: /home/tiziano/Development/StarterSquad/prudentia/prudentia/cli.py
...
os.environ['ANSIBLE_LOOKUP_PLUGINS'] = path.join(cwd, 'plugins', 'lookup') + ':~/.ansible/plugins/lookup:/usr/share/ansible/plugins/lookup'
os.environ['ANSIBLE_ACTION_PLUGINS'] = path.join(cwd, 'plugins', 'action') + ':~/.ansible/plugins/action:/usr/share/ansible/plugins/action'
...
def parse(args=None):
    ...
    parser.add_argument('-v', '--version', action='version', version='%(prog)s ' + prudentia_ver + ', Ansible ' + ansible_ver)
    parser.add_argument('provider', choices=Providers.keys(), help='use one of the available providers')
    parser.add_argument('commands', nargs='*', default='', help='optional quoted list of commands to run with the chosen provider')
    if len(sys.argv) == 1:
        parser.print_help()
        sys.exit(1)
