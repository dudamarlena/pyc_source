...
# [GCC 8.4.0]
# Embedded file name: /home/tiziano/Development/StarterSquad/prudentia/prudentia/vagrant.py
...
class VagrantProvider(FactoryProvider):
    ...
    def __init__(self):
        ...
        self.template_env = Environment(loader=FileSystemLoader(this_path), auto_reload=True, undefined=jinja2.StrictUndefined)
    ...
    def define_box(self):
        ...
        if not vagrant_boxes:
            ...
            print('Please search for a suitable one at https://atlas.hashicorp.com/boxes/search.')
    ...
    def redefine_box(self, previous_box):
        ...
        return Box(previous_box.name, playbook, hostname, ip, self.DEFAULT_USER, self.DEFAULT_PWD, ext)
    ...
    def status(self, box):
        ...
        pattern = '.*{0}\\s*(.*?) \\(virtualbox\\).*'.format(box.name)
