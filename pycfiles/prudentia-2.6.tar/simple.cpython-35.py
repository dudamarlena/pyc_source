...
# [GCC 8.4.0]
# Embedded file name: /home/tiziano/Development/StarterSquad/prudentia/prudentia/simple.py
# Compiled at: 2019-02-10 12:34:50
# Size of source mod 2**32: 11601 bytes
import logging, os, pwd, random
...
class SimpleCli(Cmd):
    ...
    @staticmethod
    def help_set():
        print('Sets the value of an Ansible extra variable. During provisioning it will forcibly override the one defined in any playbook.\n')
    ...
    @staticmethod
    def help_unset():
        print('Unsets an existing Ansible extra variable. If this action is invoked without parameter it will show the current set of variables.\n')
    ...
    @staticmethod
    def help_decrypt():
        print('Provides the password that will be used to decrypt Ansible vault files. For more information visit http://docs.ansible.com/playbooks_vault.html.\n')
    ...
    @staticmethod
    def help_facts():
        print('Gathers and shows useful information about the box. Accepts optional parameter to filter shown properties.')
...
class SimpleProvider(object):
    ...
    def set_var(self, var, value):
        if var in self.extra_vars:
            print("NOTICE: Variable '{0}' is already set to this value: '{1}' and it will be overwritten.".format(var, self.extra_vars[var]))
    ...
    def load_tags(self, box=None):
        for b in [box] if box else self.boxes():
            if not os.path.exists(b.playbook):
                print("WARNING: Box '{0}' points to a NON existing playbook. Please `reconfigure` or `unregister` the box.\n".format(b.name))
            else:
    ...
    def provision(self, box, tags):
        self.provisioned = provisioning.run_playbook(playbook_file=box.playbook, inventory_file=provisioning.generate_inventory(box), loader=self.loader, remote_user=box.get_remote_user(), remote_pass=box.get_remote_pwd(), transport=box.get_transport(), extra_vars=self.extra_vars, only_tags=tags)
