...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ps_screen.py
# Compiled at: 2014-11-14 18:24:54
import click, psutil, re, subprocess, shlex
...
def print_child(p, depth=1, columns=0):
    output = '  ' * depth + click.style(str(p.pid), fg='cyan', bold=True) + ' ' + collapse_home(clean((' ').join(p.cmdline())))
...
def table_row(process, full=False):
    return [
     ...
     (full or last_cmd)(process) if 1 else '']


@click.command()
@click.option('--full/--compact', is_flag=True, default=False, help='full output lists a full subprocess tree')
def ps_screen(full=False):
    screens = [ p for p in psutil.process_iter() if p.name() in ('screen', 'tmux')
              ]
