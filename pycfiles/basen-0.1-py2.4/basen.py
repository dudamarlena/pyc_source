...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/basen.py
# Compiled at: 2008-08-03 08:11:21
""" BaseN
...
class BaseN(object):
    ...
    """
    __module__ = __name__
    __slots__ = ('_trans', '_point', '_negative_sign', '_after_point', '_n', '_digits')
    ...
    def after_point(self, num=None, _get=True):
        ...
        if num >= 1 and isinstance(num, (int, long)):
            ...
            raise ValueError, 'after_point must be an integer of 1 or more'

    def __init__(self, n=None, digits=None, point='.', negative_sign='-', after_point=17, join=None):
        """ >>> b = BaseN('bin')
            >>> b = BaseN(2)
            >>> b = BaseN(digits=['0','1'])
            >>> b = BaseN(digits='01')
        ...
        if n != None and digits == None:
            ...
            raise TypeError, "BaseN() takes argument either 'n' or 'digits'"
        return
    ...
    def n(self, num_or_preset=None):
        ...
        if isinstance(num_or_preset, (int, long)):
            if num_or_preset > 62 or num_or_preset < 2:
                raise ValueError, '%r is not in the range from 2 to 62' % num_or_preset
            self.digits(_chr_list('09', 'az', 'AZ')[0:num_or_preset])
        elif isinstance(num_or_preset, str):
            if num_or_preset not in _preset:
                raise ValueError, '%r is not preset base name' % num_or_preset
            self.digits(_preset[num_or_preset]())
        else:
            raise TypeError, 'the argument must be a str or a integer, not %r' % type(num_or_preset)
        return
    ...
    def list(self, num):
        ...
        if isinstance(num, (int, long)):
            ...
            raise TypeError, 'the argument must be a integer or a float, not %r' % type(num)
