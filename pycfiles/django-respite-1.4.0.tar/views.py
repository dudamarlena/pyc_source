...
# [GCC 8.4.0]
# Embedded file name: /Users/jgorset/code/python/libraries/respite/respite/views/views.py
# Compiled at: 2012-11-02 10:36:58
...
class Views(object):
    ...
    def options(self, request, map, *args, **kwargs):
        ...
        return self._render(request=request, template='options', context={'options': options}, status=200, headers={'Allow': (', ').join(options.keys())})
    ...
    def _render(self, request, template=None, status=200, context={}, headers={}, prefix_template_path=True):
        ...
        if template:
            ...
            try:
                response = render(request=request, template_name=template_path, dictionary=context, status=status, content_type='%s; charset=%s' % (format.content_type, settings.DEFAULT_CHARSET))
            except TemplateDoesNotExist:
                try:
                    response = HttpResponse(content=serializers.find(format)(context).serialize(request), content_type='%s; charset=%s' % (format.content_type, settings.DEFAULT_CHARSET), status=status)
                except serializers.UnknownSerializer:
                    raise self.Error('No template exists at %(template_path)s, and no serializer found for %(format)s' % {'template_path': template_path, 
                       'format': format})
