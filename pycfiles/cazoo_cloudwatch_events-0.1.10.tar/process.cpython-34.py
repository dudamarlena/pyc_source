...
# Embedded file name: C:\Users\shu\PycharmProjects\py34\cazipcode-project\cazipcode\pkg\fuzzywuzzy\process.py
...
def extractWithoutOrder(query, choices, processor=default_processor, scorer=default_scorer, score_cutoff=0):
    ...
    if len(processed_query) == 0:
        logging.warning("Applied processor reduces input query to empty string, all comparisons will have score 0. [Query: '{0}']".format(query))
    if scorer in [fuzz.WRatio, fuzz.QRatio,
     fuzz.token_set_ratio, fuzz.token_sort_ratio,
     fuzz.partial_token_set_ratio, fuzz.partial_token_sort_ratio] and processor == utils.full_process:
        processor = no_process
    try:
...
def extract(query, choices, processor=default_processor, scorer=default_scorer, limit=5):
    ...
    return sorted(sl, key=lambda i: i[1], reverse=True)
...
def extractBests(query, choices, processor=default_processor, scorer=default_scorer, score_cutoff=0, limit=5):
    ...
    return sorted(best_list, key=lambda i: i[1], reverse=True)
