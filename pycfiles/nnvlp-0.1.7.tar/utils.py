...
# [GCC 8.4.0]
# Embedded file name: /Users/hoangpham/git_project/nnvlp-package/nnvlp/utils.py
...
def get_character_indexes(sentences, char_alphabet, max_char_length):
    index_sentences = []
...
def generate_character_data(sentences, max_sent_length, max_char_length, char_alphabet):
    index_sentences = get_character_indexes(sentences, char_alphabet)
    char = construct_tensor_char(index_sentences, max_sent_length, max_char_length, char_alphabet)
    return char
...
def get_predict_list(predictions, masks, label_alphabet):
    batch_size, max_length = np.shape(masks)
    prediction_output = []
    for i in range(batch_size):
        for j in range(max_length):
            if masks[(i, j)] > 0.0:
                prediction = predictions[0][(i, j)] + 1
                prediction = label_alphabet.get_instance(prediction)
                prediction_output.append(prediction)

    return prediction_output


def predict_2_file(input_file, output_file, prediction):
    with codecs.open(input_file, 'r', 'utf-8') as (f1):
        with codecs.open(output_file, 'r', 'utf-8') as (f2):
            cnt = 0
            for line in f1:
                line = line.strip()
                if len(line) > 0:
                    line = line + ' ' + prediction[cnt]
                    f2.write(line + '\n')
                    cnt += 1
                else:
                    f2.write('\n')
...
def output_predictions(predictions, masks, label_alphabet):
    predict_labels = []
    ...
    for i in range(batch_size):
        ...
        for j in range(max_length):
            if masks[(i, j)] > 0.0:
                prediction = predictions[0][(i, j)] + 1
                predict_label.append(label_alphabet.get_instance(prediction))
...
def crf_loss(energies, targets, masks):
    ...
    def inner_function(energies_one_step, targets_one_step, mask_one_step, prior_partition, prev_label, tg_energy):
        ...
        partition_t = T.switch(mask_one_step.dimshuffle(0, 'x'), theano_logsumexp(energies_one_step + partition_shuffled, axis=1), prior_partition)
        return [
    ...
    (partitions, _, target_energies), _ = theano.scan(fn=inner_function, outputs_info=initials, sequences=[
     energies_shuffled[1:], targets_shuffled[1:],
     masks_shuffled[1:]])
...
def iterate_minibatches(inputs, masks=None, char_inputs=None, batch_size=10, shuffle=False):
    ...
    for start_idx in range(0, len(inputs), batch_size):
        ...
        yield (
         inputs[excerpt], masks[excerpt], char_inputs[excerpt])