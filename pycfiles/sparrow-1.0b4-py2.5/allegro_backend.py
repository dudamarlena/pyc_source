...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/sparrow/allegro_backend.py
...
from sparrow.utils import parse_sparql_result, ntriples_to_json, ntriples_to_dict
...
class AllegroTripleStore(SesameTripleStore):
    ...
    def connect(self, dburi):
        ...
        try:
            (resp, content) = self._http.request('%s/repositories?%s' % (self._url, params), 'POST')
        except socket.error:
...
def start_server(host, port, path):
    ...
    allegro_dir = join(part_dir, [ d for d in os.listdir(part_dir) if d.startswith('agraph')
                                 ][0])
