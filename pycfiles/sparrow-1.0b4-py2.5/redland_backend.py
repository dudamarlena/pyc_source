...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/sparrow/redland_backend.py
...
class RedlandTripleStore(BaseBackend):
    ...
    def contexts(self):
        return [ c.literal_value['string'].decode('utf8') for c in self._model.get_contexts()
               ]
...
def model_from_uri(uri=None, **opts):
    if RDF is None:
        raise ConnectionError('Redland support is not available, install from librdf.org')
    options = []
    ...
    if uri == None or uri == 'memory':
        ...
        store = RDF.Storage(storage_name='hashes', name='memory_store', options_string=options)
    elif uri.startswith('sqlite://'):
        file = uri[9:]
        try:
            store = RDF.Storage(storage_name='sqlite', name=file, options_string=options)
        except RDF.RedlandError:
            raise ConnectionError("Can't connect to %s" % uri)

    elif uri.startswith('bdb://'):
        file = uri[6:]
        if not os.path.isfile(file + '-sp2o.db') and 'new=' not in options:
            options += ",new='yes'"
        options += ",hash-type='bdb'"
        store = RDF.Storage(storage_name='hashes', name=file, options_string=options)
        ...
        if '#' in db:
            ...
            name = 'main'
        options += ",host='%s',port='%s',database='%s',user='%s',password='%s'" % (
         host, port, db, user, pwd)
        try:
            store = RDF.Storage(storage_name='mysql', name=name, options_string=options)
        except RDF.RedlandError:
            raise ConnectionError("Can't connect to %s" % uri)
