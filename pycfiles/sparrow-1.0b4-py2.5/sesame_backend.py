...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/sparrow/sesame_backend.py
...
class SesameTripleStore(BaseBackend):
    ...
    def connect(self, dburi):
        ...
        try:
            (resp, content) = self._http.request('%s/repositories' % self._url, 'GET', headers={'Accept': 'application/sparql-results+xml'})
        except socket.error:
            raise ConnectionError('Can not connect to repository, is it running?')

        if resp['status'] != '200':
            raise ConnectionError('Can not connect to server: %s' % resp['status'])
    ...
    def contexts(self):
        (resp, content) = self._http.request('%s/repositories/%s/contexts' % (self._url, self._name), 'GET', headers={'Accept': 'application/sparql-results+xml'})
        return [ c['contextID']['value'].split(':', 1)[1] for c in parse_sparql_result(content)
               ]

    def _get_mimetype(self, format):
        return {'ntriples': 'text/plain', 'rdfxml': 'application/rdf+xml', 
    ...
    def register_prefix(self, prefix, namespace):
        ...
        (resp, content) = self._http.request('%s/repositories/%s/namespaces/%s' % (
         self._url, self._name, prefix), 'PUT', body=namespace, headers={'Content-length': clength})
        if resp['status'] != '204':
    ...
    def _add(self, file, format, context, base_uri=None):
        ...
        (resp, content) = self._http.request('%s/repositories/%s/statements?%s' % (
         self._url, self._name, params), 'POST', body=data, headers={'Content-type': ctype, 'Content-length': clength})
        if resp['status'] != '204':
            raise TripleStoreError(content)
    ...
    def _serialize(self, format, context, pretty=False):
        ...
        ctype = self._get_mimetype(format)
        (resp, content) = self._http.request('%s/repositories/%s/statements?context=%s' % (
         self._url, self._name, context), 'GET', headers={'Accept': ctype})
        if resp['status'] != '200':
            raise TripleStoreError(content)
    ...
    def _remove(self, file, format, context, base_uri=None):
        ...
        params = urlencode(params)
        (resp, content) = self._http.request('%s/repositories/%s/statements?%s' % (
         self._url, self._name, params), 'DELETE', body=data, headers={'Content-type': ctype, 'Content-length': clength})
        if resp['status'] != '204':
            raise TripleStoreError(content)
    ...
    def count(self, context=None):
        ...
        (resp, content) = self._http.request('%s/repositories/%s/size%s' % (
         self._url, self._name, context), 'GET')
        if resp['status'] != '200':
            raise TripleStoreError(content)
        return int(content)

    def select(self, sparql):
        params = urlencode({'query': sparql, 'queryLn': 'SPARQL', 
           'infer': 'false'})
        (resp, content) = self._http.request('%s/repositories/%s?%s' % (
         self._url, self._name, params), 'GET', headers={'Accept': 'application/sparql-results+xml'})
        if resp['status'] != '200':
    ...
    def construct(self, sparql, format):
        ...
        params = urlencode({'query': sparql, 'queryLn': 'SPARQL', 
           'infer': 'false'})
        (resp, content) = self._http.request('%s/repositories/%s?%s' % (
         self._url, self._name, params), 'GET', headers={'Accept': ctype})
        if resp['status'] != '200':
            raise QueryError(content)
        if content.startswith('Server error:'):
            raise QueryError(content[14:])
...
def start_server(host, port, uri, id, title, path):
    ...
    if not os.path.isfile(join(tomcat_dir, 'webapps', 'openrdf-sesame.war')):
        shutil.copyfile(join(sesame_dir, 'war', 'openrdf-sesame.war'), join(tomcat_dir, 'webapps', 'openrdf-sesame.war'))
    if not os.path.isfile(join(tomcat_dir, 'webapps', 'openrdf-workbench.war')):
        shutil.copyfile(join(sesame_dir, 'war', 'openrdf-workbench.war'), join(tomcat_dir, 'webapps', 'openrdf-workbench.war'))
...
def configure_server(host, port, uri, id, title, path):
    ...
    proc.communicate('connect http://%s:%s/openrdf-sesame.' % (host, port) + SESAME_TEST_TEMPLATE + template)