...
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/filewatch/tail.py
# Compiled at: 2015-01-11 19:33:47
import buftok, logging, os, time, watch
...
class Tail(object):
    OPEN_WARN_INTERVAL = int(os.environ.get('FILEWATCH_OPEN_WARN_INTERVAL', '300'))
    ...
    def __init__(self, opts=None):
        ...
        self.statcache = {}
        self.opts = {'sincedb_write_interval': 10, 
           'stat_interval': 1, 
           'discover_interval': 5, 
           'exclude': [], 'start_at_new_files': 'end'}
        self.opts.update(opts)
        if not self.opts.get('sincedb_path'):
            if os.environ.get('HOME'):
                self.opts['sincedb_path'] = os.path.join(os.environ['HOME'], '.sincedb')
            if os.environ.get('SINCEDB_PATH'):
                self.opts['sincedb_path'] = os.environ['SINCEDB_PATH']
        if not self.opts.get('sincedb_path'):
            raise NoSinceDBPathGiven('No HOME or SINCEDB_PATH set in environment. I need one of these set so I can keep track of the files I am following.')
    ...
    def subscribe(self, func):
        ...
        def _func(event, path):
            if event in [watch.Watch.create, watch.Watch.create_initial]:
                if path in self.files:
                    self.logger.debug('%s for %s: already exists in self.files', event, path)
                    return
                if self._open_file(path, event):
                    self._read_file(path, func)
            elif event == watch.Watch.modify:
                if path not in self.files:
                    self.logger.debug(':modify for %s, does not exist in self.files', path)
                    if self._open_file(path, event):
                        self._read_file(path, func)
                else:
                    self._read_file(path, func)
            elif event == watch.Watch.delete:
                self.logger.debug(':delete for %s, deleted from self.files', path)
                if path in self.files:
                    self._read_file(path, func)
        ...
        self.watch.subscribe(_func, self.opts['stat_interval'], self.opts['discover_interval'])
    ...
    def _open_file(self, path, event):
        ...
        try:
            ...
            if now - self.lastwarn.get(path) > Tail.OPEN_WARN_INTERVAL:
                ...
                self.logger.debug('(warn supressed) failed to open %s: %s', path, e)
            del self.files[path]
            return False

        stat = os.stat(path)
        inode = (
         str(stat.st_ino), stat.st_dev)
        self.statcache[path] = inode
        if inode in self.sincedb:
            last_size = self.sincedb[inode]
            self.logger.debug('%s: sincedb last value %s, cur size %s', path, self.sincedb[inode], stat.st_size)
            if last_size <= stat.st_size:
                self.logger.debug('%s: sincedb: seeking to %s', path, last_size)
                self.files[path].seek(last_size)
            else:
                self.logger.debug('%s: last value size is greater than current value, starting over', path)
            ...
            if self.opts['start_at_new_files'] == 'beginning':
                self.logger.debug('%s: initial create, no sincedb, seeking to beginning of file', path)
                self.files[path].seek(0)
                self.sincedb[inode] = 0
            else:
                self.logger.debug('%s: initial create, no sincedb, seeking to end %s', path, stat.st_size)
    ...
    def _read_file(self, path, func):
        ...
        if changed:
            ...
            if delta >= self.opts['sincedb_write_interval']:
                self.logger.debug('writing sincedb (delta since last write = %s)', delta)
                self._sincedb_write()
        ...
        try:
            with open(path, 'r') as (db):
                for line in db:
                    ...
                    inode = (ino, long(dev))
                    self.logger.debug('_sincedb_open: setting %s to %s', inode, long(pos))
    ...
    def _sincedb_write(self):
        ...
        try:
            ...
            self.logger.warn('_sincedb_write rename/sync failed: %s -> %s: %s', tmp, path, e)