...
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/filewatch/watch.py
# Compiled at: 2015-01-12 12:13:20
import fnmatch, glob, logging, os, time
        ...
        for path, info in self._files.items():
            try:
                ...
                if e.errno == os.errno.ENOENT:
                    ...
                    self.logger.debug('%s: stat failed (%s), deleting from @files', path, e)
            ...
            if inode != info.inode:
                self.logger.debug('%s: old inode was %s, new is %s', path, info.inode, inode)
                func(Watch.delete, path)
                func(Watch.create, path)
            elif stat.st_size < info.size:
                self.logger.debug('%s: file rolled, new size is %s, old size %s', path, stat.st_size, info.size)
                func(Watch.delete, path)
                func(Watch.create, path)
            elif stat.st_size > info.size:
                self.logger.debug('%s: file grew, old size %s, new size %s', path, info.size, stat.st_size)
    ...
    def _discover_file(self, path, initial=False):
        ...
        for file in globbed_dirs:
            ...
            if not os.path.isfile(file):
                continue
            self.logger.debug('_discover_file: %s: new: %s (exclude is %s)', path, file, self._exclude)
            skip = False
            for pattern in self._exclude:
                if fnmatch.fnmatch(os.path.basename(file), pattern):
                    self.logger.debug('_discover_file: %s: skipping because it matches exclude %s', file, pattern)
                    skip = True
                    break
