...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/adminlteui/admin.py
# Compiled at: 2020-01-21 04:26:45
# Size of source mod 2**32: 14149 bytes
...
def get_image_box():
    if get_option(option_name='site_logo'):
        return ImageBox(get_option(option_name='site_logo'))
    else:
        return ''


class GeneralOptionForm(forms.Form):
    site_title = forms.CharField(label=(_('Site Title')), widget=(widgets.AdminTextInputWidget()),
      help_text=(_("Text to put at the end of each page's tag title.")))
    site_header = forms.CharField(label=(_('Site Header')), widget=(widgets.AdminTextInputWidget()),
      help_text=(_("Text to put in base page's tag b.")))
    site_logo = forms.ImageField(label=(_('Site Logo')), widget=(forms.ClearableFileInput()),
      required=False,
    ...
    avatar_field = forms.CharField(label=(_('Avatar Field')), widget=(widgets.AdminTextInputWidget()),
      required=False,
      help_text=(_('which field is avatar.')))
    show_avatar = forms.BooleanField(label=(_('Show Avatar')),
      required=False)
        ...
        try:
            ...
            if not self.data.get('show_avatar'):
                try:
                    ...
                    obj = Options.objects.create(option_name='show_avatar', option_value='off')
                    obj.save()
                ...
                try:
                    ...
                    if data_item == 'site_logo':
                        if not settings.MEDIA_ROOT or not settings.MEDIA_URL:
                            self.errors[data_item] = _('site_logo depends on setting.MEDIA_URL and setting.MEDIA_ROOT.')
                            return False
                        if not self.files.get(data_item) or self.data.get(data_item) == '':
                            continue
                        handle_uploaded_file(self.files.get(data_item), self.files.get(data_item).name)
                        obj.option_value = settings.MEDIA_URL.lstrip('/') + self.files.get(data_item).name
                    else:
                    ...
                    if data_item == 'site_logo':
                        if not settings.MEDIA_ROOT or not settings.MEDIA_URL:
                            ...
                            return False
                        handle_uploaded_file(self.files.get(data_item), self.files.get(data_item).name)
                        obj = Options.objects.create(option_name=data_item,
                          option_value=(settings.MEDIA_URL.lstrip('/') + self.files.get(data_item).name),
...
class OptionsAdmin(admin.ModelAdmin):
    ...
    def get_urls(self):
        ...
        urls = [
         path('general_option/', self.admin_site.admin_view((self.general_option_view),
           cacheable=True),
           name='general_option')]
    ...
    def general_option_view(self, request):
        if request.user.has_perm('django_admin_settings.add_options') is False:
            if request.user.has_perm('django_admin_settings.change_options') is False:
                return HttpResponseForbidden(format_html('<h1>403 Forbidden</h1>'))
        else:
            context = dict(self.admin_site.each_context(request))
            if request.method == 'GET':
                initial_value = {'site_title':get_option(option_name='site_title') or admin.AdminSite.site_title, 
                 'site_header':get_option(option_name='site_header') or admin.AdminSite.site_header, 
                 'welcome_sign':get_option(option_name='welcome_sign'), 
                 'site_logo':ImageBox(get_option(option_name='site_logo')) if get_option(option_name='site_logo') else '', 
                 'show_avatar':True if get_option(option_name='show_avatar') == 'on' else False, 
                 'avatar_field':get_option(option_name='avatar_field') or 'request.user.head_avatar'}
                form = GeneralOptionForm(initial=initial_value)
            else:
                form = GeneralOptionForm(request.POST, request.FILES)
                if form.save():
                    initial_value = {'site_title':get_option(option_name='site_title') or admin.AdminSite.site_title, 
                     'site_header':get_option(option_name='site_header') or admin.AdminSite.site_header, 
                     'welcome_sign':get_option(option_name='welcome_sign'), 
...
class MenuAdmin(TreeAdmin):
    list_display = ('name', 'position', 'link_type', 'display_link', 'display_content_type',
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name == 'content_type':
            return self.ContentTypeModelChoiceField(queryset=(ContentType.objects.all()),
              required=False,
              label=(_('ContentType')),
    ...
    def changeform_view(self, request, object_id=None, form_url='', extra_context=None):
        try:
            ...
            messages.error(request, _('Exception raised while add node: %s') % _(force_str(e)))
            return HttpResponseBadRequest(_('Exception raised while add node: %s') % _(force_str(e)))

    def get_urls(self):
        base_urls = super().get_urls()
        urls = [
         path('exchange_menu/', self.admin_site.admin_view((self.exchange_menu_view),
           cacheable=True),
           name='exchange_menu')]
        return urls + base_urls
    ...
    def exchange_menu_view(self, request):
        if request.user.has_perm('django_admin_settings.view_menu') is False:
            ...
            if request.is_ajax():
                ...
                try:
                    use_custom_menu = Options.objects.get(option_name='USE_CUSTOM_MENU')
                except Options.DoesNotExist:
                    use_custom_menu = Options.objects.create(option_name='USE_CUSTOM_MENU',
                      option_value='0')
    ...
    def display_link(self, obj):
        if obj.link:
            if '/' in obj.link:
                return format_html('<i class="fa fa-check text-green"></i> {}'.format(obj.link))
            try:
                reverse(obj.link)
                return format_html('<i class="fa fa-check text-green"></i> {}'.format(obj.link))
            except Exception as e:
                return format_html('<i class="fa fa-close text-red"></i> {}'.format(obj.link))
