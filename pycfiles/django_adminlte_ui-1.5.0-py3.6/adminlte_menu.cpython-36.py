...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/adminlteui/templatetags/adminlte_menu.py
...
from adminlteui.templatetags.adminlte_options import get_adminlte_option, get_adminlte_settings
...
def get_menu(context, request, position='left'):
    ...
    if not isinstance(request, HttpRequest):
        ...
        if use_custom_menu.get('USE_CUSTOM_MENU', '0') == '1':
            if use_custom_menu.get('valid') is True:
        ...
        for app in available_apps:
            if app.get('app_label') == 'django_admin_settings':
                if request.user.has_perm('django_admin_settings.add_options') or request.user.has_perm('django_admin_settings.change_options'):
                    app.get('models').insert(0, {'name':_('General Options'), 
                     'object_name':'Options', 
                     'perms':{'add':True, 
                      'change':True, 
                      'delete':True, 
                ...
                for model in app.get('models', []):
                    model['icon'] = get_adminlte_settings().get('icons', {}).get(app['app_label'], {}).get(model['name'].lower())
