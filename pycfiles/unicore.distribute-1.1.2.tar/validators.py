...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/universalcore/unicore.distribute/unicore/distribute/api/validators.py
...
def validate_schema(request):
    ...
    repo = get_repository(os.path.join(storage_path, request.matchdict['name']))
    ...
    if not validate(schema, data):
        ...
        request.errors.add('body', 'schema', 'Data does not match the schema for %s' % (content_type,))
    elif uuid is not None and data['uuid'] != uuid:
        request.errors.status = 400
        request.errors.add('body', 'uuid', 'Payload UUID does not match URL UUID.')
    else:
...
class CreateRepoColanderSchema(MappingSchema):
    repo_url = SchemaNode(String(), location='body', validator=repo_url_type_schema_validator)
    repo_name = SchemaNode(String(), location='body', validator=repo_name_validator, missing=None)