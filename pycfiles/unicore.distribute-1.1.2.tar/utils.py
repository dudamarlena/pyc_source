...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/universalcore/unicore.distribute/unicore/distribute/utils.py
...
class UCConfigParser(ConfigParser):
    ...
    def get_dict(self, section, option):
        ...
        :returns: dict
        """
        return dict(re.split('\\s*=\\s*', value) for value in self.get_list(section, option))
...
def list_schemas(repo):
    ...
    :returns: dict
    """
    schema_files = glob.glob(os.path.join(repo.working_dir, '_schemas', '*.avsc'))
    schemas = {}
...
def list_content_types(repo):
    ...
    """
    schema_files = glob.glob(os.path.join(repo.working_dir, '_schemas', '*.avsc'))
    return [ os.path.splitext(os.path.basename(schema_file))[0] for schema_file in schema_files
           ]
...
def get_mapping(repo, content_type):
    ...
    """
    try:
        with open(os.path.join(repo.working_dir, '_mappings', '%s.json' % (content_type,)), 'r') as (fp):
            return json.load(fp)
    except IOError:
...
def format_repo(repo):
    ...
    """
    commit = repo.commit()
    return {'name': os.path.basename(repo.working_dir), 
...
def format_content_type(repo, content_type):
    ...
    return [ dict(model_obj) for model_obj in storage_manager.iterate(model_class)
           ]
...
def get_es_settings(config):
    ...
    return {'urls': [
              es_host or config.get('es.host', 'http://localhost:9200')]}
...
def add_model_item_to_pull_dict(storage_manager, path, pull_dict):
    if path.endswith('.json'):
        ...
        pull_dict[(model.__module__ + '.' + model.__class__.__name__)].append(dict(model))
        return True
    return False
...
def pull_repository_files(repo, commit_id):
    ...
    try:
        ...
        for diff_added in diff.iter_change_type('A'):
            add_model_item_to_pull_dict(sm, diff_added.b_blob.path, changed_files)

        for diff_modified in diff.iter_change_type('M'):
            add_model_item_to_pull_dict(sm, diff_modified.b_blob.path, changed_files)
