...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/universalcore/unicore.distribute/unicore/distribute/tests/base.py
...
class DistributeTestCase(TestCase):
    ...
    def mk_workspace(self, working_dir=None, name=None, url='http://localhost', index_prefix=None, auto_destroy=None, author_name='Test Kees', author_email='kees@example.org'):
    ...
    def add_schema(self, workspace, model_class):
        ...
        workspace.sm.store_data(os.path.join('_schemas', '%(namespace)s.%(name)s.avsc' % schema), schema_string, 'Writing the schema.')

    def add_mapping(self, workspace, model_class):
        im = ESManager(None, None, None)
        mapping = im.get_mapping_type(model_class).get_mapping()
        workspace.sm.store_data(os.path.join('_mappings', '%s.%s.json' % (model_class.__module__,
         model_class.__name__)), json.dumps(mapping), 'Writing the mapping.')
        return
