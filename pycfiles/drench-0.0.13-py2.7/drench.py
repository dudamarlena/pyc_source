...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/drench/drench.py
# Compiled at: 2014-01-25 22:45:08
"""Geo lookup provided courtesy of freegeoip.net"""
import argparse, requests, socket, tparser, hashlib, reactor, peer, time, os, random, json
...
class VisListener(Listener):
    ...
    def read(self):
        ...
        assert self.torrent.vis_write_sock is self.torrent.switchboard.vis_write_sock
...
class Torrent(object):
    ...
    def __init__(self, torrent_path, directory='', port=55308, download_all=False, visualizer=None):
        ...
        self.switchboard = Switchboard(dirname=dirname, file_list=file_list, piece_length=self.piece_length, num_pieces=self.num_pieces, multifile=multifile, download_all=download_all, vis_socket=vis_socket)
        return
    ...
    def tracker_request(self):
        ...
        if self.torrent_dict['announce'].startswith('udp'):
            ...
            self.r = requests.get(self.torrent_dict['announce'], params=payload)
    ...
    def get_peer_ips(self):
        ...
        while presponse:
            peer_ip = (
             ('.').join(str(x) for x in presponse[0:4]),
...
def main():
    ...
    argparser.add_argument('torrent_path', help='Location of the .torrent file')
    argparser.add_argument('-p', '--port', nargs='?', default=DEFAULT_PORT, help=('Which port to use. Default is {}').format(DEFAULT_PORT))
    argparser.add_argument('-d', '--directory', nargs='?', default=DEFAULT_DIR, help=('Where to save downloaded files. Default is {}').format(DEFAULT_DIR))
    argparser.add_argument('-a', '--all', action='store_true', help='Set flag to download all files in torrent', default=False)
    argparser.add_argument('-v', '--visualizer', help='Colon-separated address and port for runningvisualizer. \ne.g., "127.0.0.1:8000', default=None, type=open_socket)
    ...
    mytorrent = Torrent(torrent_path, directory=directory, port=port, download_all=download_all, visualizer=visualizer)
    with mytorrent:
