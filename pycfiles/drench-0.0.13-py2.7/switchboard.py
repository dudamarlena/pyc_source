...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/drench/switchboard.py
# Compiled at: 2014-01-23 16:47:34
import os, bitarray, copy, json
...
def get_want_file_pos(file_list):
    ...
    if all_answer == 'n':
        for j, tfile in enumerate(file_list):
            while 1:
                file_answer = raw_input(('Do you want {}? (y/n): ').format(os.path.join(*tfile['path'])))
...
def get_heads_tails(want_file_pos=[], file_starts=[], num_pieces=0, piece_length=0):
    heads_tails = []
    for i in want_file_pos:
        head_tail = get_head_tail(want_index=i, file_starts=file_starts, num_pieces=num_pieces, piece_length=piece_length)
        heads_tails.append(head_tail)

    return heads_tails


def get_head_tail(want_index=0, file_starts=[], num_pieces=0, piece_length=0):
...
class Switchboard(object):
    ...
    def __init__(self, dirname='', file_list=[], piece_length=0, num_pieces=0, multifile=False, download_all=False, vis_socket=None):
        self.dirname = dirname
        self.file_list = copy.deepcopy(file_list)
        self.piece_length = piece_length
        self.num_pieces = num_pieces
        self.file_starts = get_file_starts(self.file_list) if multifile else [
         0]
        ...
        if self.download_all:
            ...
            for i in self.want_file_pos:
                if multifile:
                    thisfile = open(os.path.join(*self.file_list[i]['path']), 'w')
                else:
                    thisfile = open(self.file_list[i]['path'], 'w')
                self.outfiles.append(thisfile)

        self.heads_and_tails = get_heads_tails(want_file_pos=self.want_file_pos, file_starts=self.file_starts, num_pieces=self.num_pieces, piece_length=self.piece_length)
        self.bitfield = build_bitfield(self.heads_and_tails, num_pieces=self.num_pieces)
        self.vis_init()
    ...
    def get_next_want_file(self, byte_index, block):
        ...
        """
        while block:
            rightmost = get_rightmost_index(byte_index=byte_index, file_starts=self.file_starts)
            if rightmost in self.want_file_pos:
                return (rightmost, byte_index, block)
            file_start = self.file_starts[rightmost]
    ...
    def write(self, byte_index, block):
        ...
        if file_list_index is not None and write_byte_index is not None and block is not '':
