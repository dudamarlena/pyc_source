...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/drench/peer.py
...
class Peer(object):
    ...
    def __init__(self, sock, reactor, torrent, in_location):
        ...
        self.save_state = {'state': self.states['reading_length'], 'length': 0, 
           'message_id': None, 'message': '', 
           'remainder': ''}
    ...
    def ppiece(self, content):
        ...
        if self.piece.complete:
            ...
            if hashlib.sha1(piece_bytes).digest() == self.torrent.torrent_dict['info']['pieces'][20 * piece_index:20 * piece_index + 20]:
                print 'hash matches'
                piece_dict = {'kind': 'piece', 'peer': self.sock.getpeername(), 'piece_index': piece_index}
    ...
    def init_piece(self):
        ...
        for i in range(self.torrent.num_pieces):
            ...
            if self.torrent.switchboard.bitfield[i] is True and self.bitfield[i] is True:
    ...
    def request_all(self):
        ...
        request_dict = {'kind': 'request', 'peer': self.sock.getpeername(), 'piece': self.piece.index}
    ...
    def request_block(self, block_index):
        ...
        if self.piece.index == self.torrent.last_piece and byte_index == self.piece.last_block:
