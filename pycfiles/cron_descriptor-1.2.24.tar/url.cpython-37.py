...
# Embedded file name: /private/var/folders/pb/598z8h910dvf2wrvwnbyl_2m0000gn/T/pip-install-65c3rg8f/urllib3/urllib3/util/url.py
...
def _encode_invalid_chars(component, allowed_chars, encoding='utf-8'):
    ...
    is_percent_encoded = len(percent_encodings) == uri_bytes.count('%')
    encoded_component = bytearray()
    for i in range(0, len(uri_bytes)):
        byte = uri_bytes[i:i + 1]
        byte_ord = ord(byte)
        if is_percent_encoded and byte == '%' or :
            if byte.decode() in allowed_chars:
                encoded_component.extend(byte)
                continue
        encoded_component.extend('%' + hex(byte_ord)[2:].encode().zfill(2).upper())
    ...
    if host:
        if isinstance(host, six.binary_type):
            ...
            if is_ipv6:
                ...
                return six.ensure_str('.'.join([_idna_encode(label) for label in host.split('.')]))
    return host


def _idna_encode(name):
    if name:
        if any([ord(x) > 128 for x in name]):
            try:
                import idna
            except ImportError:
...
def parse_url(url):
    ...
    try:
        ...
        normalize_uri = scheme is None or 
        if scheme:
            scheme = scheme.lower()
        elif authority:
