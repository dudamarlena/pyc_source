...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\steam_game_scale\steam_game_scale.py
# Compiled at: 2016-05-27 23:05:43
# Size of source mod 2**32: 36443 bytes
import tkinter as tk, tkinter.font as tkfont
tk.font = tkfont
from PIL import Image, ImageTk
import sys, os
from steam_game_scale.api import steam_api
from steam_game_scale.url_images import url_image
from steam_game_scale.player import steam_player
from steam_game_scale.game import steam_game
from steam_game_scale.scale import playtime_weights
import traceback, yaml, webbrowser
...
def print_toplevel(msg):
    ...
    img = ImageTk.PhotoImage(file=os.path.join(os.path.dirname(__file__), 'steam_game_scale.png'))
...
class params_input:
    ...
    def init_steamid_prompt_listbox(self):
        ...
        for steamid_name in sorted(set(self.loaded_steamids.keys())):
            self.params_dialog.steamid_prompt_listbox.insert(self.params_dialog.steamid_prompt_listbox.size(), '{}: [{}]'.format(steamid_name, self.loaded_steamids[steamid_name]))
    ...
    def init_api_key_prompt_listbox(self):
        ...
        for api_key_name in sorted(set(self.loaded_api_keys.keys())):
            self.params_dialog.api_key_prompt_listbox.insert(self.params_dialog.api_key_prompt_listbox.size(), '{}: [{}]'.format(api_key_name, '*****************'))
    ...
    def __init__(self):
        self.saved_filename = os.path.join(os.path.dirname(__file__), 'saved_params.yaml')
        self.loaded_params = {}
