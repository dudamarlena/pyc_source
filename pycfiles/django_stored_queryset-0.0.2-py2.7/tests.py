...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/test/stored/tests.py
# Compiled at: 2012-05-02 19:20:11
...
class StoredQuerySetBaseTestCase(TestCase):
    ...
    def test_get_model(self):
        ...
        self.assertRaises(ContentType.DoesNotExist, lambda : FilterQuerySet('Q(a=1)', 'stored.zzzztmodel').model)
...
class FilterQuerySetTestCase(TestCase):
    queries = [
     {'query': "Q(headline__startswith='{headline}')", 'selector': 'stored.tmodel', 
        'literals': {'headline': '"head1"'}, 'result': [
                 1, 10]},
     {'query': "Q(headline__startswith='{headline}')&Q(id__in={idlist})", 'selector': 'stored.tmodel', 
        'literals': {'headline': '"head1"', 'idlist': '[1,10]'}, 'override_literals': {'idlist': '[1,]'}, 'result': [
                 1, 10], 
        'override_result': [
                          1]},
     {'query': 'Q(related__in=related_qset)', 'selector': 'stored.tmodel', 
        'eval': {'related_qset': lambda : Rel0.objects.all()[:1]}, 
        'result': [
                 1]},
     {'query': 'Q(related__in=related_qset)&~Q(id__in=[2,3])', 'selector': 'stored.tmodel', 
        'eval': {'related_qset': lambda : Rel0.objects.all()[:1]}, 
        'result': [
                 1]},
     {'query': 'Q(related__in=related_qset)&~Q(id__in={idlist})', 'selector': 'stored.tmodel', 
        'eval': {'related_qset': lambda : Rel0.objects.all()[:1]}, 
        'literals': {'idlist': '[2,3]'}, 'override_literals': {'idlist': '[1,2,3]'}, 'result': [
                 1], 
        'override_result': []}]
    ...
    def test_queries(self):
        for params in self.queries:
            ...
            if params.get('override_literals', {}):
                ...
                restored_override_result = restored_query.literals(**params['override_literals'])(**evals).values_list('id')
            self.assert_(flatten(result) == params['result'], '1:%s:%s!=%s' % (params['query'],
             flatten(result),
             params['result']))
            self.assert_(flatten(restored_result) == params['result'], '2:%s:%s!=%s' % (params['query'],
             flatten(restored_result),
             params['result']))
            self.assert_(flatten(restored_result) == params['result'], '3:%s:%s!=%s' % (params['query'],
             flatten(restored_result),
             params['result']))
            if params.get('override_literals', {}):
                self.assert_(flatten(override_result) == params['override_result'], '4:%s:%s!=%s' % (params['query'],
                 flatten(override_result),
                 params['override_result']))
                self.assert_(flatten(restored_override_result) == params['override_result'], '5:%s:%s!=%s' % (params['query'],
    ...
    def test_query_evals(self):
        query = FilterQuerySet('Q(headline__startswith=headline)&Q(id__in={idlist})', 'stored.tmodel', idlist='[1,2]')
