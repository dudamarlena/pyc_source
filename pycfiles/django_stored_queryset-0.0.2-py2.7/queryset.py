...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/stored/queryset.py
# Compiled at: 2012-05-02 19:12:32
...
def patch_query_set(stored):
    ...
    if not getattr(orig_get_query_set, 'eval_appent', False):
        ...
        stored.manager.get_query_set = get_query_set.__get__(stored.manager, stored.manager.__class__)
...
class FilterQuerySet(object):
    ...
    def get_model(self):
        if not hasattr(self, '_model'):
            if isinstance(self.selector, int):
                ...
                kwargs = dict(zip(('app_label', 'model'), self.selector.split('.')))
    ...
    def __setstate__(self, dict):
        self.__init__(dict['query'], dict['selector'], dict['_manager_name'], **dict['_literals'])
