...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\rapidpg\levelmgr\collision.py
# Compiled at: 2014-07-08 22:02:05
# Size of source mod 2**32: 16164 bytes
...
class Level:
    ...
    def __init__(self, config, data, tiles, backgrounds=None, player=None, animations=tuple()):
        """
        ...
        self.camera = None
        self.camera = Camera(self.config['resolution'], Rect((0, 0), (self.level_width,
         self.level_height)), self.player.speed)
        self.backgrounds = []
        if backgrounds:
    ...
    def _gravity(self):
        ...
        if collision_test:
            closest_rect = Level.find_min([self.interpreted[x] for x in collision_test])
            delta = closest_rect.y - (self.player.rect.y + self.player.rect.height)
    ...
    def _jump_action(self):
        ...
        collision_test = inflated.collidelistall(self.interpreted)
        if collision_test:
            closest_rect = Level.find_max([self.interpreted[x] for x in collision_test])
            delta = math.fabs(self.player.rect.y - (closest_rect.y + closest_rect.height))
            self.player.move(0, -delta)
            self.player.jumping = False
        else:
    ...
    def _left_action(self):
        ...
        """
        collision_test = self.player.rect.move(-self.player.speed, 0).collidelist(self.interpreted)
        if collision_test != -1:
            delta = math.fabs(self.player.rect.x - (self.interpreted[collision_test].x + self.interpreted[collision_test].width))
    ...
    def _right_action(self):
        ...
        collision_test = self.player.rect.move(self.player.speed, 0).collidelist(self.interpreted)
        if collision_test != -1:
            delta = math.fabs(self.interpreted[collision_test].x - (self.player.rect.x + self.player.rect.width))
            self.player.move(delta, 0)
        else:
            self._bg_left()
            self.player.move(self.player.speed, 0)
    ...
    def get_dimensions(self):
        ...
        """
        return (
         len(self.data[0]) * self.tile_width,
    ...
    def interpret(self):
        ...
        for line in range(len(raw)):
            for char in range(len(raw[line])):
                if Level._is_processed(char * self.tile_width, (line + 1) * self.tile_width, rect_list):
                    continue
                if raw[line][char] in collide_set:
                        ...
                        while True:
                            try:
                                ...
                                while True:
                                    try:
                                        tmp_line += 1
                                        if raw[tmp_line][tmp_char] not in collide_set:
                                            break
                                        possible_height += 1
                                    except IndexError:
                                        break
                    ...
                    rect_list.append(Rect(char * tile_width, line * tile_width, width * tile_width, height * tile_width))
                    continue
