...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\rapidpg\level_converter.py
# Compiled at: 2014-06-30 18:38:40
# Size of source mod 2**32: 9716 bytes
"""
...
def convert():
    parser = argparse.ArgumentParser(prog='Rapid Pygame level converter', description='Convert Rapid Pygame level\n                       file into Tiled map file or vise versa', epilog='Refer to the documentation specifics about the formats')
    parser.add_argument('file', metavar='file', type=str, help='the Rapid Pygame level file or Tiled map file to convert')
    parser.add_argument('-o', '--output', metavar='result', type=str, help='the name of the converted file')
    parser.add_argument('--per-line', metavar='n', type=int, default=10, help='the number of tile images per line when generating tile set for Tiled. Default is 10')
    ...
    try:
        ...
        if exit_node is not None:
            header += 'exit {0} {1} {2} {3}\n'.format(exit_node.get('x'), exit_node.get('y'), exit_node.get('width'), exit_node.get('height'))
        ...
        for l in background_config:
            _, value = l.split(' ', 1)
            ET.SubElement(root, 'imagelayer', {'name': value, 
             'width': level_width, 
             'height': level_height})

        tile_set = ET.SubElement(root, 'tileset', {'firstgid': '1', 
         'name': file_name, 
         'tilewidth': str(tile_width), 
         'tileheight': str(tile_width)})
        ET.SubElement(tile_set, 'image', {'source': file_name + '.png', 
         'width': str(width), 
         'height': str(height)})
        config = parse_config(raw_config)
        if 'animations' in config:
            animation_layer = ET.SubElement(root, 'objectgroup', name='Animations', width=level_width, height=level_height)
            for folder, interval, x, y in config['animations']:
                ET.SubElement(animation_layer, 'object', {'name': str(folder), 
                 'type': str(interval), 
                 'x': str(x), 
                 'y': str(y)})

        layer = ET.SubElement(root, 'layer', {'name': file_name, 
        ...
        if 'exit' in config or 'spawn' in config:
            ...
            if 'exit' in config:
                x, y, w, h = config['exit']
                ET.SubElement(control_layer, 'object', {'type': 'exit', 
                 'x': str(x), 
                 'y': str(y), 
                 'width': str(w), 
                 'height': str(h)})
            if 'spawn' in config:
                x, y = config['spawn']
                ET.SubElement(control_layer, 'object', {'type': 'spawn', 
                 'x': str(x), 
                 'y': str(y)})
