...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/archipel/archipelHypervisor.py
...
"""
import datetime, libvirt, random, sqlite3, string, uuid as moduuid, xmpp
...
class TNArchipelHypervisor(TNArchipelEntity, archipelLibvirtEntity.TNArchipelLibvirtEntity, TNHookableEntity, TNAvatarControllableEntity, TNTaggableEntity):
    ...
    def init_vocabulary(self):
        ...
        registrar_items = [
         {'commands': ['capabilities'], 'parameters': [], 'method': self.message_capabilities, 
            'permissions': [
                          'capabilities'], 
            'description': 'Get my libvirt capabilities'},
         {'commands': ['nodeinfo'], 'parameters': [], 'method': self.message_nodeinfo, 
            'permissions': [
                          'nodeinfo'], 
            'description': 'Get my node informations'},
         {'commands': ['libvirt uri', 'migration info'], 'parameters': [], 'method': self.message_migration_info, 
            'permissions': [
                          'migrationinfo'], 
            'description': 'Get my migration informations'},
         {'commands': ['ip'], 'parameters': [], 'method': self.message_ip, 
            'permissions': [
                          'ip'], 
            'description': 'Get my IP address'},
         {'commands': ['roster', 'vms', 'virtual machines', 'domains'], 'parameters': [], 'method': self.message_roster, 
            'permissions': [
                          'rostervm'], 
            'description': 'Get the content of my roster'},
         {'commands': ['alloc'], 'parameters': [{'name': 'name', 'description': 'The name of the vm. If not given, it will be generated'}], 'method': self.message_alloc, 
            'permissions': [
                          'alloc'], 
            'description': 'Allocate a new virtual machine'},
         {'commands': ['free'], 'parameters': [{'name': 'identifier', 'description': 'The name or the UUID of the vm to free'}], 'method': self.message_free, 
            'permissions': [
                          'free'], 
            'description': 'Free a virtual machine'},
         {'commands': ['clone'], 'parameters': [{'name': 'identifier', 'description': 'The name or the UUID of the vm to clone'}], 'method': self.message_clone, 
            'permissions': [
                          'clone'], 
    ...
    def get_vm_by_uuid(self, uuid):
        ...
        @return: the virtual machine or None
        """
        if uuid.lower() not in self.virtualmachines:
            return None
        else:
    ...
    def clone(self, uuid, requester, wanted_name=None):
        ...
        newvm.register_hook('HOOK_VM_INITIALIZE', method=newvm.clone, user_info={'definition': xmldesc, 'path': xmppvm.folder, 'parentvm': xmppvm}, oneshot=True)
        newvm_thread.start()
    ...
    def migration_info(self):
        ...
        return {'libvirt_uri': uri, 'base_folder': self.configuration.get('VIRTUALMACHINE', 'vm_base_path')}
