...
# [GCC 8.4.0]
# Embedded file name: /marrow/mailer/transport/imap.py
...
class IMAPTransport(object):
    __slots__ = ('ephemeral', 'host', 'ssl', 'port', 'username', 'password', 'folder',
                 'connection')
    ...
    def deliver(self, message):
        result = self.connection.append(self.folder, '', message.date.timetuple() if message.date else datetime.now(), bytes(message))
        if result[0] != b'OK':
