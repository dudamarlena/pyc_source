...
class Message(object):
    __doc__ = 'Represents an e-mail message.'
    ...
    @property
    def id--- This code section failed: ---

 L. 109         0  LOAD_FAST                'self'
                2  LOAD_ATTR                _id
                4  POP_JUMP_IF_FALSE    18  'to 18'
                6  LOAD_FAST                'self'
                8  LOAD_ATTR                _processed
 ...
 L. 110        18  LOAD_GLOBAL              make_msgid
               20  CALL_FUNCTION_0       0  '0 positional arguments'
               22  LOAD_FAST                'self'
               24  STORE_ATTR               _id

 L. 111        26  LOAD_CONST               False
               28  LOAD_FAST                'self'
               30  STORE_ATTR               _processed
             32_0  COME_FROM            16  '16'
             32_1  COME_FROM            10  '10'

 L. 112        32  LOAD_FAST                'self'
               34  LOAD_ATTR                _id
               36  RETURN_VALUE     
               -1  RETURN_LAST      
...
Parse error at or near `RETURN_VALUE' instruction at offset 36
    ...
    def attach(self, name, data=None, maintype=None, subtype=None, inline=False, filename=None, filename_charset='', filename_language='', encoding=None):
        ...
        if encoding:
            ...
            if filename_charset or filename_language:
                ...
                if sys.version_info < (3, 0):
                    filename = (
                     filename_charset.encode('utf-8'), filename_language.encode('utf-8'), filename.encode('utf-8'))
                else:
                    filename = (
                     filename_charset, filename_language, filename)
        if inline:
            if sys.version_info < (3, 0):
