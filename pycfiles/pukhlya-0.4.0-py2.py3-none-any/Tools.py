...
# Embedded file name: puke/Tools.py
# Compiled at: 2012-10-27 20:37:21
...
def combine(in_files, out_file, verbose=False, replace=None):
    ...
    combined = ''
    for f in in_files:
        if __get_ext(f) not in ('css', 'scss'):
            continue
        isCSS = True
...
def jsdoc3(files, destination, template=None, fail=True):
    ...
    out = Std()
    output = sh('cd "%s"; java -classpath lib/js.jar org.mozilla.javascript.tools.shell.Main -debug -modules nodejs_modules -modules rhino_modules -modules . jsdoc.js      --destination "%s" --template "%s" %s' % (jsdoc, destination, template, '"' + ('" "').join(files) + '"'), header='Generating js doc v3', output=False, std=out)
