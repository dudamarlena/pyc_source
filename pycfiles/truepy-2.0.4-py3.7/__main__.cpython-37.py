...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/truepy/__main__.py
# Compiled at: 2020-01-23 12:02:01
# Size of source mod 2**32: 7979 bytes
import argparse, getpass, sys, cryptography.hazmat.primitives.serialization, cryptography.x509
...
def main(action, action_arguments, **args):
    try:
        ...
        raise RuntimeError('%s requires additional arguments', action.__name__)
...
def show(license_file, issuer_certificate, license_file_password, **args):
    ...
    print('\tsubject:\t%s' % ('"%s"' % license.data.subject if license.data.subject else '<none>'))
    print('\tconsumer_type:\t%s' % ('"%s"' % license.data.consumer_type if license.data.consumer_type else '<none>'))
    print('\tinformation:\t%s' % ('"%s"' % license.data.info if license.data.info else '<none>'))
    print('\textra data:\t%s' % ('"%s"' % license.data.extra if license.data.extra else '<none>'))


@action
def issue(license_file, license_description, issuer_certificate, issuer_key, license_file_password, **args):
    ...
    """
    try:
        license_data_parameters = dict(((p.strip() for p in i.split('=', 1)) for i in license_description.split(',')))
    except Exception as e:
        try:
            raise RuntimeError('Invalid license data description (%s): %s', license_description, e)
        finally:
            e = None
            del e
    ...
    try:
        ...
        try:
            raise RuntimeError('Incomplete license data description (%s): %s', license_description, e)
        finally:
            e = None
            del e

    license = License.issue(issuer_certificate, issuer_key, license_data=license_data)
...
class PasswordAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, value, option_string=None):
        ...
        destination = ' '.join((s for s in self.dest.split('_') if not s == 'password'))
        if password == '-':
            password = getpass.getpass('Please enter password for %s:' % destination)
        setattr(namespace, self.dest, self.get_value(value[:-1] if isinstance(value, list) else [value], password))
...
class CertificateAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, value, option_string=None):
        with open(value, 'rb') as (f):
            data = f.read()
        certificate = None
        for file_type in ('pem', 'der'):
            try:
                loader = getattr(cryptography.x509, 'load_%s_x509_certificate' % file_type)
        ...
        if certificate is None:
            raise argparse.ArgumentError(self, 'Failed to load certificate')
        else:
...
class KeyAction(PasswordAction):
    ...
    def get_value(self, value, password):
        ...
        for file_type in ('pem', 'der'):
            try:
                loader = getattr(cryptography.hazmat.primitives.serialization, 'load_%s_x509_certificate' % file_type)
                return loader(data, password, backends.default_backend())
            except:
                pass

        raise argparse.ArgumentError(self, 'Failed to load key')
...
class ActionAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, value, option_string=None):
        try:
            ...
            raise argparse.ArgumentError(self, 'Unknown action')

        setattr(namespace, self.dest, action)


parser = argparse.ArgumentParser(prog='truepy',
...
parser.add_argument('--issuer-key',
  help='The private key to the certificate and the password; pass "-" as password to read it from stdin.',
  nargs=2,
  const=None,
  action=KeyAction)
parser.add_argument('--license-file-password',
...
parser.add_argument('action',
  help=('The action to perform; this can be any of %s' % ', '.join(ACTIONS.keys())),
  nargs=1,
  action=ActionAction)
parser.add_argument('action_arguments',
