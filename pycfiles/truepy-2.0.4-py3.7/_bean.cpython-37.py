...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/truepy/_bean.py
...
def value_to_xml(value, tag_name, class_name=None):
    ...
    if class_name is None:
        ...
        o = ElementTree.Element('object', attrib={'class': class_name})
...
def _serialize_object(value):
    ...
    property_names = sorted((k for k, v in o.__class__.__dict__.items() if isinstance(getattr(o.__class__, k), property)))
    java_wrapper = ElementTree.Element('java', attrib={'version':'1.0', 
     'class':'java.beans.XMLDecoder'})
    container = ElementTree.SubElement(java_wrapper, 'object', attrib={'class': class_name})
    for property_name in property_names:
        container.append(serialize(property_name, getattr(o, property_name)))
...
def serialize(value):
    ...
    property_names = sorted((k for k, v in value.__class__.__dict__.items() if isinstance(getattr(value.__class__, k), property)))
    xml = ElementTree.Element('object', attrib={'class': class_name})
    for property_name in property_names:
        property_value = serialize(getattr(value, property_name))
        property_container = ElementTree.SubElement(xml, 'void', attrib={'property': snake_to_camel(property_name)})
...
def to_document(e):
    ...
    :rtype: str
    """
    return '<?xml version="1.0" encoding="utf-8"?><java version="1.0" class="java.beans.XMLDecoder">%s</java>' % tostring(e)


class UnknownFragmentException(Exception):
    __doc__ = 'The exception raised by a deserialiser when it cannot handle an XML\n    fragment.\n    '
