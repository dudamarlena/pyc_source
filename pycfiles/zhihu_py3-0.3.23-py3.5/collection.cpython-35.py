...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/zhihu/collection.py
# Compiled at: 2016-05-07 01:37:29
# Size of source mod 2**32: 10992 bytes
from .common import *
from .base import BaseZhihu

class Collection(BaseZhihu):
    __doc__ = '收藏夹，请使用``ZhihuClient.collection``方法构造对象.'

    @class_common_init(re_collection_url)
    def __init__(self, url, owner=None, name=None, follower_num=None, session=None):
        """创建收藏夹类实例.
    ...
    def owner(self):
        ...
        motto = self.soup.find('div', id='zh-single-answer-author-info').div.text
        photo_url = PROTOCOL + self.soup.find('img', class_='zm-list-avatar-medium')['src'].replace('_m', '_r')
        return Author(url, name, motto, photo_url=photo_url, session=self._session)
        ...
        while gotten_feed_num == 20:
            ...
            for zm_item in zm_items:
                ...
                try:
                    ...
                    yield CollectActivity(act_type, act_time, self.owner, self, answer)
                except AttributeError:
                    act_type = CollectActType.CREATE_COLLECTION
                    yield CollectActivity(act_type, act_time, self.owner, self)
    ...
    def _page_get_questions(self, soup):
        ...
        for question_tag in question_tags:
            if question_tag.h2 is not None:
                ...
                yield Question(question_url, question_title, session=self._session)
        ...
        for tag in answer_tags:
            ...
            if url_tag is None:
                ...
                if upvote_num.isdigit():
                    ...
                    upvote_num = None
                answer = Answer(answer_url, question, author, upvote_num, session=self._session)
                yield answer


class CollectActivity:
    __doc__ = '收藏夹操作, 请使用``Collection.logs``构造对象.'
