...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/zhihu/answer.py
# Compiled at: 2016-08-31 08:16:06
# Size of source mod 2**32: 13730 bytes
import json
...
class Answer(BaseZhihu):
    __doc__ = '答案类，请使用``ZhihuClient.answer``方法构造对象.'

    @class_common_init(re_ans_url)
    def __init__(self, url, question=None, author=None, upvote_num=None, content=None, session=None):
        """创建答案类实例.
    ...
    def id(self):
        ...
        :rtype: int
        """
        return int(re.match('.*/(\\d+)/$', self.url).group(1))
    ...
    def question(self):
        ...
        question_link = self.soup.find('h2', class_='zm-item-title').a
        url = Zhihu_URL + question_link['href']
        title = question_link.text.strip()
        followers_num = int(self.soup.find('div', class_='zh-question-followers-sidebar').div.a.strong.text)
        answers_num = int(re_get_number.match(self.soup.find('div', class_='zh-answers-title').h3.a.text).group(1))
        return Question(url, title, followers_num, answers_num, session=self._session)
    ...
    def upvote_num(self):
        ...
        :rtype: int
        """
        return int(self.soup.find('div', class_='zm-item-vote-info')['data-votecount'])
    ...
    def creation_time(self):
        ...
        """
        return datetime.fromtimestamp(int(self.soup.find('div', class_='zm-item-answer')['data-created']))
    ...
    def collect_num(self):
        ...
        :rtype: int
        """
        element = self.soup.find('a', {'data-za-a': 'click_answer_collected_count'})
        if element is None:
            return 0
        else:
    ...
    def collections(self):
        ...
        while gotten_feed_num >= 10:
            ...
            for zm_item in soup.find_all('div', class_='zm-item'):
                ...
                yield Collection(url, owner=owner, name=name, follower_num=follower_num, session=self._session)
    ...
    def save(self, filepath=None, filename=None, mode='html'):
        ...
        if mode not in ('html', 'md', 'markdown'):
            raise ValueError("`mode` must be 'html', 'markdown' or 'md', got {0}".format(mode))
        file = get_path(filepath, filename, mode, self.question.title, self.question.title + '-' + self.author.name)
    ...
    def _parse_author_soup(self, soup):
        ...
        if author_tag.string is None:
            ...
            numbers = [int(re_get_number.match(x.get_text()).group(1)) for x in numbers_tag]
            return Author(author_url, author_name, author_motto, None, numbers[2], numbers[3], numbers[0], numbers[1], photo_url, session=self._session)
        else:
            return ANONYMOUS
        ...
        while page <= pages:
            ...
            for comment_item in comment_items:
                ...
                if comment_item['author'].get('url') is not None:
                    ...
                    author_obj = Author(a_url, a_name, photo_url=a_photo_url, session=self._session)
                else:
