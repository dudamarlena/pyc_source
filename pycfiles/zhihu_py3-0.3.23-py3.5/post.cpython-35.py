...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/zhihu/post.py
# Compiled at: 2016-04-02 12:53:51
# Size of source mod 2**32: 6118 bytes
from .common import *
from .base import BaseZhihu, JsonAsSoupMixin

class Post(JsonAsSoupMixin, BaseZhihu):
    __doc__ = '专栏文章类，请使用``ZhihuClient.post``方法构造对象.'

    @class_common_init(re_post_url)
    def __init__(self, url, column=None, author=None, title=None, upvote_num=None, comment_num=None, session=None):
        """创建专栏文章类实例.
    ...
    def save(self, filepath=None, filename=None, mode='md'):
        ...
        self._make_soup()
        file = get_path(filepath, filename, mode, self.column.name, self.title + '-' + self.author.name)
    ...
    def upvoters(self):
        ...
        json = self._session.get(Post_Get_Upvoter.format(self.slug), headers=headers).json()
        for au in json:
            try:
                yield Author(au['profileUrl'], au['name'], au['bio'], photo_url=au['avatar']['template'].format(id=au['avatar']['id'], size='r'), session=self._session)
            except ValueError:
                yield ANONYMOUS