...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/zhihu/me.py
...
class Me(Author):
    __doc__ = '封装了相关操作（如点赞，关注问题）的类。\n    请使用 :meth:`.ZhihuClient.me` 方法获取实例。\n    '

    def __init__(self, url, name, motto, photo_url, session):
        super(Me, self).__init__(url, name, motto, photo_url=photo_url, session=session)
    ...
    def vote(self, something, vote='up'):
        ...
        if isinstance(something, Post):
            ...
            if something.author.url == self.url:
                return False
            put_url = Upvote_Article_Url.format(something.column_in_name, something.slug)
        ...
        raise ValueError('argument something need to be zhihu.Answer or zhihu.Post object.')
    ...
    def thanks(self, answer, thanks=True):
        ...
        data = {'_xsrf': answer.xsrf, 
         'aid': answer.aid}
        res = self._session.post(Thanks_Url if thanks else Cancel_Thanks_Url, data=data)
        return res.json()['r'] == 0
    ...
    def follow(self, something, follow=True):
        ...
        if isinstance(something, Author):
            ...
            return res.json()['r'] == 0
        if isinstance(something, Question):
            data = {'_xsrf': something.xsrf, 
        ...
        if isinstance(something, Collection):
            ...
            res = self._session.post(Follow_Collection_Url if follow else Unfollow_Collection_Url, data=data)
            return res.json()['r'] == 0
        raise ValueError('argument something need to be zhihu.Author, zhihu.Question, Zhihu.Topic or Zhihu.Collection object.')
    ...
    def add_comment(self, answer, content):
        ...
        data = {'method': 'add_comment', 
         ...
         '_xsrf': answer.xsrf}
        res = self._session.post(Answer_Add_Comment_URL, data=data)
        return res.json()['r'] == 0
    ...
    def unhelpful(self, answer, unhelpful=True):
        ...
        data = {'_xsrf': answer.xsrf, 
         'aid': answer.aid}
        res = self._session.post(Unhelpful_Url if unhelpful else Cancel_Unhelpful_Url, data=data)
        return res.json()['r'] == 0