...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/zhihu/column.py
# Compiled at: 2016-04-02 08:40:56
# Size of source mod 2**32: 3392 bytes
from .common import *
from .base import BaseZhihu, JsonAsSoupMixin

class Column(JsonAsSoupMixin, BaseZhihu):
    __doc__ = '专栏类，请使用``ZhihuClient.column``方法构造对象.'

    @class_common_init(re_column_url)
    def __init__(self, url, name=None, follower_num=None, post_num=None, session=None):
        """创建专栏类实例.
    ...
    def posts(self):
        ...
        for offset in range(0, (self.post_num - 1) // 10 + 1):
            self._session.headers.update(Host='zhuanlan.zhihu.com')
            res = self._session.get(Column_Posts_Data.format(self._in_name, offset * 10))
            soup = res.json()
            self._session.headers.update(Host=origin_host)
            for post in soup:
    ...
    def _parse_post_data(self, post):
        ...
        author = Author(post['author']['profileUrl'], post['author']['name'], post['author']['bio'], photo_url=photo_url, session=self._session)
        title = post['title']
        upvote_num = post['likesCount']
        comment_num = post['commentsCount']
        print(url)
        return Post(url, self, author, title, upvote_num, comment_num, session=self._session)