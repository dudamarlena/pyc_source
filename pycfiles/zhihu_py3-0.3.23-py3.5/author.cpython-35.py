...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/zhihu/author.py
# Compiled at: 2016-09-13 00:01:27
# Size of source mod 2**32: 23549 bytes
import json
from .base import BaseZhihu
from .common import *

class BanException(Exception):
    __doc__ = '当尝试获取被反屏蔽系统限制的用户资料时，将会引发此异常'


class Author(BaseZhihu):
    __doc__ = '用户类，请使用``ZhihuClient.answer``方法构造对象.'

    @class_common_init(re_author_url, True)
    def __init__(self, url, name=None, motto=None, follower_num=None, question_num=None, answer_num=None, upvote_num=None, thank_num=None, photo_url=None, session=None):
        """创建用户类实例.
    ...
    def _gen_soup(self, content):
        ...
        self._nav_list = self.soup.find('div', class_='profile-navbar').find_all('a')
    ...
    def id(self):
        ...
        if self.url is not None:
            return re.match('^.*/([^/]+)/$', self.url).group(1)
        return ''
    ...
    def motto(self):
        ...
        if self.url is None:
            return ''
        if self.soup is not None:
            bar = self.soup.find('div', class_='title-section')
    ...
    def followee_num(self):
        ...
        if self.url is None:
            ...
            number = int(self.soup.find('div', class_='zm-profile-side-following').a.strong.text)
            return number
    ...
    def follower_num(self):
        ...
        if self.url is None:
            ...
            number = int(self.soup.find('div', class_='zm-profile-side-following zg-clear').find_all('a')[1].strong.text)
            return number
    ...
    def thank_num(self):
        ...
        if self.url is None:
            ...
            number = int(self.soup.find('span', class_='zm-profile-header-user-thanks').strong.text)
            return number
    ...
    def weibo_url(self):
        ...
        if self.url is None:
            ...
            tmp = self.soup.find('a', class_='zm-profile-header-user-weibo')
            if tmp is not None:
                return tmp['href']
            return 'unknown'
    ...
    def _find_user_profile(self, t):
        ...
        if self.url is None:
            ...
            res = self.soup.find('span', class_=t)
    ...
    def questions(self):
        ...
        for page_index in range(1, (self.question_num - 1) // 20 + 2):
            html = self._session.get(self.url + 'asks?page=' + str(page_index)).text
            soup = BeautifulSoup(html)
            question_links = soup.find_all('a', class_='question_link')
            question_datas = soup.find_all('div', class_='zm-profile-section-main')
            for link, data in zip(question_links, question_datas):
                url = Zhihu_URL + link['href']
                title = link.text.strip()
                answer_num = int(re_get_number.match(data.div.contents[4]).group(1))
                follower_num = int(re_get_number.match(data.div.contents[6]).group(1))
                q = Question(url, title, follower_num, answer_num, session=self._session)
                yield q
    ...
    def answers(self):
        ...
        for page_index in range(1, (self.answer_num - 1) // 20 + 2):
            ...
            for q, upvote in zip(questions, upvotes):
                ...
                yield Answer(answer_url, question, self, upvote_num, session=self._session)
    ...
    def _follow_ee_ers(self, t, skip=0):
        ...
        while gotten_date_num == 20:
            ...
            for html in json_data['msg']:
                ...
                numbers = [int(re_get_number.match(x.text).group(1)) for x in soup.find_all('a', class_='zg-link-gray-normal')]
                try:
                    yield Author(author_url, author_name, author_motto, *numbers, session=self._session)
                except ValueError:
                    yield ANONYMOUS
    ...
    def collections(self):
        ...
        for page_index in range(1, (collection_num - 1) // 20 + 2):
            html = self._session.get(self.url + 'collections?page=' + str(page_index)).text
            soup = BeautifulSoup(html)
            collections_names = soup.find_all('a', class_='zm-profile-fav-item-title')
            collection_follower_nums = soup.find_all('div', class_='zm-profile-fav-bio')
            for c, f in zip(collections_names, collection_follower_nums):
                ...
                yield Collection(c_url, self, c_name, c_fn, session=self._session)
    ...
    def columns(self):
        ...
        for column_tag in column_tags:
            ...
            if len(numbers) == 1:
                ...
                post_num = int(re_get_number.match(numbers[1]).group(1))
            yield Column(url, name, follower_num, post_num, session=self._session)
    ...
    def followed_topics(self):
        ...
        if self.followed_topic_num > 0:
            ...
            if tag is not None:
                for a in tag.find_all('a'):
                    yield Topic(Zhihu_URL + a['href'], a.img['alt'], session=self._session)
            ...
            if self.followed_topic_num > 7:
                ...
                while gotten_data_num == 20:
                    ...
                    offset += gotten_data_num
                    topic_item = BeautifulSoup(j['msg'][1]).find_all('div', class_='zm-profile-section-item')
