...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/zhihu/common.py
# Compiled at: 2016-09-12 06:54:34
...
re_question_url = re.compile('^https?://www\\.zhihu\\.com/question/\\d+(\\?sort=created|/?)$')
re_question_url_std = re.compile('^https?://www\\.zhihu\\.com/question/\\d+/?')
re_ans_url = re.compile('^https?://www\\.zhihu\\.com/question/\\d+/answer/\\d+/?$')
re_author_url = re.compile('^https?://www\\.zhihu\\.com/(?:people|org)/[^/]+/?$')
re_collection_url = re.compile('^https?://www\\.zhihu\\.com/collection/\\d+/?$')
re_column_url = re.compile('^http://zhuanlan\\.zhihu\\.com/([^/]+)/?$')
re_post_url = re.compile('^http://zhuanlan\\.zhihu\\.com/p/(\\d+)/?$')
re_topic_url = re.compile('^https?://www\\.zhihu\\.com/topic/(\\d+)/?$')
re_a2q = re.compile('(.*)/answer/.*')
re_collection_url_split = re.compile('.*(/c.*)')
re_get_number = re.compile('[^\\d]*(\\d+).*')
re_del_empty_line = re.compile('\\n*(.*)\\n*')
...
def parser_author_from_tag(author):
    ...
    if author_link is None:
        ...
        author_motto = motto_span['title'] if motto_span is not None else ''
...
def answer_content_process(content):
    ...
    del content['class']
    soup = BeautifulSoup('<html><head><meta charset="utf-8"></head><body></body></html>')
...
def get_path(path, filename, mode, default_path, default_name):
    if path is None:
        path = os.path.join(os.getcwd(), remove_invalid_char(default_path))
    if filename is None:
    ...
    while gotten_data_num == 20:
        ...
        for div in follower_divs:
            if div.a is not None:
                ...
                numbers = [re_get_number.match(a.text).group(1) for a in div.find_all('a', target='_blank')]
                try:
                    yield Author(author_url, author_name, author_motto, *numbers, session=session)
                except ValueError:
                    yield ANONYMOUS
