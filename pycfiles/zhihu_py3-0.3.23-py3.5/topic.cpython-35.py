...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/zhihu/topic.py
# Compiled at: 2016-08-31 08:16:06
# Size of source mod 2**32: 17794 bytes
import time
...
class Topic(BaseZhihu):
    __doc__ = '答案类，请使用``ZhihuClient.topic``方法构造对象.'
    ...
    def tid(self):
        ...
        :rtype: int
        """
        return int(self.soup.find('div', id='zh-topic-desc')['data-resourceid'])
    ...
    def parents(self):
        ...
        if parent_topic_tag is None:
            ...
            for topic_tag in parent_topic_tag.find_all('a'):
                yield Topic(Zhihu_URL + topic_tag['href'], topic_tag.text.strip(), session=self._session)
    ...
    def children(self):
        ...
        if child_topic_tag is None:
            return []
        if '共有' not in child_topic_tag.contents[(-2)].text:
            ...
            while flag == 'load':
                params['child'] = child
                res = self._session.post(Topic_Get_Children_Url, params=params, data=data)
                j = map(lambda x: x[0], res.json()['msg'][1])
                *topics, last = j
                for topic in topics:
                    yield Topic(Zhihu_URL + '/topic/' + topic[2], topic[1], session=self._session)

                flag = last[0]
                child = last[2]
                if flag == 'topic':
                    yield Topic(Zhihu_URL + '/topic/' + last[2], last[1], session=self._session)
    ...
    def top_answers(self):
        ...
        while True:
            ...
            for ans, up, q, au in zip(answers, upvotes, questions, authors):
                ...
                yield Answer(answer_url, question, author, upvote, session=self._session)
    ...
    def questions(self):
        ...
        while True:
            ...
            for qu_div in questions:
                ...
                creation_time = datetime.fromtimestamp(int(qu_div.h2.span['data-timestamp']) // 1000)
                yield Question(url, title, creation_time=creation_time, session=self._session)
            ...
            for div in answers_div:
                if not div.textarea:
                    ...
                    if not author_link:
                        ...
                        author_motto = author_motto_span['title'] if author_motto_span else ''
                    author = Author(author_url, author_name, author_motto, session=self._session)
                    body = div.find('div', class_='zm-item-rich-text')
                    answer_url = Zhihu_URL + body['data-entry-url']
                    upvote_num = int(div.find('div', class_='zm-item-vote-info')['data-votecount'])
                    yield Answer(answer_url, question, author, upvote_num, session=self._session)
