...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/zhihu/question.py
# Compiled at: 2016-08-31 08:16:06
# Size of source mod 2**32: 15258 bytes
import json, time
...
class Question(BaseZhihu):
    __doc__ = '问题类，请使用``ZhihuClient.question``方法构造对象.'

    @class_common_init(re_question_url, trailing_slash=False)
    def __init__(self, url, title=None, followers_num=None, answer_num=None, creation_time=None, author=None, session=None):
        """创建问题类实例.
        ...
        self._id = int(re.match('.*/(\\d+)', self.url).group(1))
        self._author = author
    ...
    def qid(self):
        ...
        :rtype: int
        """
        return int(self.soup.find('div', id='zh-question-detail')['data-resourceid'])
    ...
    def title(self):
        ...
        :rtype: str
        """
        return self.soup.find('h2', class_='zm-item-title').text.replace('\n', '')
    ...
    def answers(self):
        ...
        if self._url.endswith('sort=created'):
            ...
            for page in range(1, max_page + 1):
                ...
                authors = answers_wrap.find_all('div', class_='zm-item-answer-author-info')
                urls = answers_wrap.find_all('a', class_='answer-date-link')
                up_num = answers_wrap.find_all('div', class_='zm-item-vote-info')
                contents = answers_wrap.find_all('div', class_='zm-editable-content')
                assert len(authors) == len(urls) == len(up_num) == len(contents)
                for author, url, up_num, content in zip(authors, urls, up_num, contents):
                    a_url, name, motto, photo = parser_author_from_tag(author)
                    author_obj = Author(a_url, name, motto, photo_url=photo, session=self._session)
                    url = Zhihu_URL + url['href']
                    up_num = int(up_num['data-votecount'])
                    content = answer_content_process(content)
                    yield Answer(url, self, author_obj, up_num, content, session=self._session)
            ...
            for i in range(0, (self.answer_num - 1) // pagesize + 1):
                if i == 0:
                    ...
                    r = self._session.post(Question_Get_More_Answer_URL, data=data, headers=new_header)
    ...
    def _parse_answer_html(self, answer_html):
        ...
        upvote_num = int(soup.find('div', class_='zm-item-vote-info')['data-votecount'])
        content = soup.find('div', class_='zm-editable-content')
        content = answer_content_process(content)
        a_url, name, motto, photo = parser_author_from_tag(author)
        author = Author(a_url, name, motto, photo_url=photo, session=self._session)
        return Answer(answer_url, self, author, upvote_num, content, session=self._session)