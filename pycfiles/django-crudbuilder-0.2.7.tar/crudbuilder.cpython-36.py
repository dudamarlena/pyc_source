...
# [GCC 8.4.0]
# Embedded file name: /home/sa/workspace/django-crudbuilder/crudbuilder/templatetags/crudbuilder.py
# Compiled at: 2017-11-10 10:39:41
...
def get_model_fields(obj, detail_exclude=None):
    ...
    for name in dir(model):
        if name not in excludes and isinstance(getattr(model, name, None), property):
...
def get_pagination_context(page, pages_to_show=11, url=None, size=None, extra=None, parameter_name='page'):
    ...
    if pages_to_show < 1:
        raise ValueError('Pagination pages_to_show should be a positiveinteger, you specified {pages}'.format(pages=pages_to_show))
    ...
    if half_page_num < 0:
        ...
        if last_page < num_pages:
            ...
            if first_page > 1:
                first_page -= 1
            elif pages_back is not None and pages_back > 1:
                pages_back -= 1
            else:
                pages_back = None
    pages_shown = []
    ...
    if url:
        url = force_text(url)
        url = re.sub('\\?{0}\\=[^\\&]+'.format(parameter_name), '?', url)
        url = re.sub('\\&{0}\\=[^\\&]+'.format(parameter_name), '', url)
        if '?' in url:
            url += '&'
        else:
    ...
    return {'bootstrap_pagination_url':url,  'num_pages':num_pages, 
