...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/tests/test_sparse.py
# Compiled at: 2011-01-21 12:49:11
import os
...
def simple_session_app(environ, start_response):
    ...
    if environ['PATH_INFO'].startswith('/invalid'):
        ...
        if not session:
            ...
            return [
             'No session id of %s found.' % sess_id]
    ...
    start_response('200 OK', [('Content-type', 'text/plain')])
    return [
     'The current value is: %d, session id is %s' % (session['value'],
      session.id)]
...
def test_session():
    app = TestApp(SessionMiddleware(simple_session_app, data_dir='./cache', type='mongodb', url=uri, sparse_collection=True))
    res = app.get('/')
