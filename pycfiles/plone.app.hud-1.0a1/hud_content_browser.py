...
# [GCC 8.4.0]
# Embedded file name: /home/matej/workarea/plone.hud/plone.app.hud/src/plone/app/hud/hud_content_browser.py
# Compiled at: 2013-09-11 17:45:19
...
import datetime, locale, logging, math, pytz, re
ITEMS_PER_PAGE = 50
...
class ContentBrowserPanelView(HUDPanelView):
    ...
    def render(self):
        ...
        self.group_url = ('{url}/@@usergroup-groupmembership?groupname={{groupid}}').format(url=self.portal_url)
        self.user_url = ('{url}/@@user-information?userid={{userid}}').format(url=self.portal_url)
    ...
    @cache(lambda method, self: 'cache_key', get_cache=lambda fun, *args, **kwargs: RAMCacheAdapter(content_browser_cache))
    def _get_all_results(self):
        ...
        items = {self.portal_id: {'children': {}, 'item': {'title': self.portal.title, 
        ...
        logger.info(('End of database scan. Elapsed time is {0} seconds.').format(self.process_time))
        return items
    ...
    def add_item(self, item, items):
        ...
        for tail_part in tail_list:
            current_parent['children'][tail_part] = {'children': {}, 'item': None, 
               'countall': 0}
    ...
    def get_kbytes(self, size_in_text):
        ...
        matched = re.match('^\\D*(\\d+[\\.\\,]?\\d*)\\s*([BKMGTbkmgt]{0,2}).*$', size_in_text)
        if matched is None:
            return
        else:
    ...
    def format_datetime_friendly_ago(self, date):
        ...
        if date is None:
            ...
            since = now - date
            seconds = since.seconds + since.microseconds / 1000000.0 + since.days * 86400
            ...
            if hours > 0:
                return ('{0} {1} {2} {3}').format(hours, _('hours'), minutes, _('minutes ago'))
            if minutes > 0:
                return ('{0} {1}').format(minutes, _('minutes ago'))
            return _('few seconds ago')
            return
