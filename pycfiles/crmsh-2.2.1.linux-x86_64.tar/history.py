...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/crmsh/history.py
# Compiled at: 2016-05-20 13:41:15
import os, time, re, glob, ConfigParser
from . import config
...
_LOG_FILES = ('ha-log.txt', 'ha-log', 'cluster-log.txt', 'messages', 'journal.log',
              'pacemaker.log')
...
class Report(object):
    ...
    """
    live_recent = 21600
    short_live_recent = 60
    nodecolors = ('NORMAL', 'GREEN', 'CYAN', 'MAGENTA', 'YELLOW', 'WHITE', 'BLUE',
                  'RED')
    session_sub = 'session'
    ...
    def get_nodes(self):
        ...
        nodes = sorted([ os.path.basename(p) for p in os.listdir(self.loc) if check_node(p)
                       ])
    ...
    def _report_setup_source(self):
        ...
        for descname in ('description.txt', 'report.summary'):
            ...
            if os.path.isfile(self.desc):
                ...
                self._creation_time = time.strftime('%a %d %b %H:%M:%S %Z %Y', time.localtime(yr))
    ...
    def info(self):
        ...
        utils.page_string(('\n').join(('Source: %s' % self.source,
         ...
         'Nodes: %s' % (' ').join([ self._str_nodecolor(x, x) for x in self.node_l
                         ]),
    ...
    def pe_detail_format(self, t_obj):
        ...
        l += utils.get_cib_attributes(t_obj.path(), 'cib', ('update-client', 'update-user',
                                                            'update-origin'), ('no-client',
                                                                               'no-user',
                                                                               'no-origin'))
    ...
    def pelist(self, a=None, verbose=False):
        ...
        if not self.prepare_source(no_live_update=self.prevent_live_update()):
            ...
            if isinstance(a, (tuple, list)):
                ...
                a = [
                 a, a]
            l = [ verbose and self.pe_detail_format(t_obj) or t_obj.path() for t_obj in self.logparser.get_transitions() if pe_file_in_range(t_obj.pe_file, a) ]
            if verbose:
                l = [
                 pe_details_hdr, pe_details_sep] + l
            return l
    ...
    def show_transitions(self):
        if not self.prepare_source(no_live_update=self.prevent_live_update()):
            return []
        return [
         '%-30s  %-15s %-15s Tags' % ('Time', 'Name', 'Node')] + [ t.description() for t in self.logparser.get_transitions() ]
    ...
    def save_state(self, sdir):
        ...
        p.set(self.rpt_section, 'dir', self.source == 'live' and sdir or self.source)
        p.set(self.rpt_section, 'from_time', self.from_dt and logtime.human_date(self.from_dt) or '')
        p.set(self.rpt_section, 'to_time', self.to_dt and logtime.human_date(self.to_dt) or '')
    ...
    def manage_session(self, subcmd, name):
        ...
        if subcmd == 'save' and os.path.exists(session_dir):
            ...
            return False
        if subcmd in ('load', 'pack', 'update', 'delete') and not os.path.exists(session_dir):
    ...
    def manage_excludes(self, cmd, arg=None):
        ...
        if cmd == 'show':
            ...
            for i in range(len(self.log_filter_out)):
                arg.set(self.log_section, 'exclude_%d' % i, self.log_filter_out[i])
