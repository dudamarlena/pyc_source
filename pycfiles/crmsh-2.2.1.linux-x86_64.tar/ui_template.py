...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/crmsh/ui_template.py
...
class Template(command.UI):
    ...
    def do_delete(self, context, name, force=''):
        ...
        if name == self.curr_conf:
            if not force and not config.core.force and not utils.ask('Do you really want to remove config %s which is in use?' % self.curr_conf):
                return False
            self.curr_conf = ''
    ...
    @command.skill_level('administrator')
    @command.completers(compl.join(compl.call(utils.listconfigs), compl.choice(['replace', 'update'])), compl.call(utils.listconfigs))
