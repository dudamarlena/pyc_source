...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/crmsh/scripts.py
# Compiled at: 2016-05-04 07:56:27
import os, re, subprocess, getpass, time, shutil, socket, random
...
_action_shortdescs = {'cib': 'Configure cluster resources', 
...
class Text(object):
    ...
    def __str__(self):
        ...
        if self._kind == self.SHORTDESC:
            return self._parse()
        if self._kind == self.CIB:
...
def _parse_hawk_workflow(scriptname, scriptfile):
    ...
    data = {'version': 2.2, 'name': scriptname, 
       ...
       'dir': None, 
       'steps': [], 'actions': []}
    paramstep = {'shortdesc': _strip(('').join(xml.xpath('./parameters/stepdesc/text()'))), 
       'parameters': []}
    ...
    for item in xml.xpath('./templates/template'):
        templatestep = {'shortdesc': _strip(('').join(item.xpath('./stepdesc/text()'))), 
           'name': item.get('name'), 
           'required': True, 
           'parameters': []}
        data['steps'].append(templatestep)
        _parse_hawk_template(scriptfile, item.get('name'), item.get('type', item.get('name')), templatestep, data['actions'])
...
def _process_agent_include(script, include):
    ...
    if 'name' not in include:
        include['name'] = name
    step = _listfindpend(name, script['steps'], lambda x: x.get('name'), lambda : {'name': name, 
       'longdesc': '', 
       'shortdesc': '', 
       'parameters': []})
    ...
    step['parameters'].append({'name': 'id', 
    ...
    step['value'] = Text.cib(script, _make_cib_for_agent('' if hoist else step['name'], agent, step, include.get('ops', '')))
    if hoist:
        step['name'] = ''
        if hoist_from:
...
def _process_script_include(script, include):
    ...
    for incparam in include.get('parameters', []):
        if 'step' in incparam and 'name' not in incparam:
            _merge_step_params(_lookup_step(name, scriptstep.get('steps', []), incparam['step']), incparam['parameters'])
        else:
            _merge_step_param(_lookup_step(name, scriptstep.get('steps', []), ''), incparam)
...
def _resolve_script(name):
    for p in list_scripts():
        if p.endswith('main.yml') and os.path.dirname(p).endswith('/' + name):
            return p
        if p.endswith('.yml') and os.path.splitext(os.path.basename(p))[0] == name:
            return p
        if p.endswith('.xml') and os.path.splitext(os.path.basename(p))[0] == name:
            return p
...
def _tempname(prefix):
    return '%s-%s%s' % (prefix,
     hex(int(time.time()))[2:],
     hex(random.randint(0, 281474976710656))[2:])
...
def _print_debug(printer, local_node, hosts, workdir, opts):
    ...
    if hosts:
        for host, result in _parallax_call(printer, hosts, "if [ -f '%s' ]; then cat '%s'; fi" % (dbglog, dbglog), opts).iteritems():
...
def _run_cleanup(printer, has_remote_actions, local_node, hosts, workdir, opts):
    ...
    if has_remote_actions and hosts and workdir:
        cleanscript = os.path.join(workdir, 'crm_clean.py')
        for host, result in _parallax_call(printer, hosts, '%s %s' % (cleanscript,
         workdir), opts).iteritems():
            if isinstance(result, parallax.Error):
...
def common_params():
    ...
    return [
     ('nodes', None, 'List of nodes to execute the script for'),
     ...
     ('sudo', 'no', 'If set, crm will prompt for a sudo password and use sudo when appropriate'),
...
_IDENT_RE = re.compile('^([a-z0-9_#$-][^\\s=]*)$', re.IGNORECASE)
    ...
    try:
        if scriptdir is not None:
            if os.path.basename(scriptdir) == script['name']:
                cmd = [
                 'mkdir', '-p', os.path.dirname(workdir)]
            else:
                cmd = [
                 'mkdir', '-p', workdir]
            if options.regression_tests:
...
def _create_remote_workdirs(printer, hosts, path, opts):
    ...
    ok = True
    for host, result in _parallax_call(printer, hosts, 'mkdir -p %s' % os.path.dirname(path), opts).iteritems():
        if isinstance(result, parallax.Error):
            printer.error(host, 'Start: %s' % result)
            ok = False

    if not ok:
        msg = 'Failed to connect to one or more of these hosts via SSH: %s' % (', ').join(h[0] for h in hosts)
...
def _copy_to_remote_dirs(printer, hosts, path, opts):
    ...
    ok = True
    for host, result in _parallax_copy(printer, hosts, path, path, opts).iteritems():
        if isinstance(result, parallax.Error):
...
class RunActions(object):
    ...
    def _update_state(self):
        if self.dry_run:
            return True
        json.dump(self.data, open(self.statefile, 'w'))
        return _copy_to_all(self.printer, self.workdir, self.hosts, self.local_node, self.statefile, self.dstfile, self.opts)
    ...
    def copy_file(self, nodes, src, dst):
        if not self._is_local(nodes):
            ok = _copy_to_all(self.printer, self.workdir, self.hosts, self.local_node, src, dst, self.opts)
        else:
            ok = _copy_local(self.printer, self.workdir, self.local_node, src, dst)
    ...
    def execute_shell(self, nodes, cmdscript):
        ...
        if self.dry_run:
            ...
            _chmodx(tmpf)
            if not self._is_local(nodes):
                ok = _copy_to_remote_dirs(self.printer, self.hosts, tmpf, self.opts)
                if not ok:
                    self.result = False
                else:
    ...
    def _process_remote(self, cmdline, is_json_output):
        ...
        if self.dry_run:
            ...
            if config.core.debug:
                self.printer.print_command(self.hosts, cmdline)
            for host, result in _parallax_call(self.printer, self.hosts, cmdline, self.opts).iteritems():
                if isinstance(result, parallax.Error):
