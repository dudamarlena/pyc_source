...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/crmsh/handles.py
# Compiled at: 2016-05-04 07:56:27
import re
headmatcher = re.compile('\\{\\{(\\#|\\^)?([A-Za-z0-9\\#\\$:_-]+)\\}\\}')
...
def _parse(template, context, strict):
    ...
    while template:
        ...
        if head is None:
            ret += template
            break
        istart, iend, prefix, key = (
         head.start(0), head.end(0), head.group(1), head.group(2))
        if istart > 0:
            ret += template[:istart]
