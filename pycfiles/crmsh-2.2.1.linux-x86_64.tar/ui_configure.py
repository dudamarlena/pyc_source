...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/crmsh/ui_configure.py
...
_id_show_list = compl.join(_id_list, compl.choice(['xml', 'changed']), compl.call(_type_completions), compl.call(_tag_completions))
...
def stonith_resource_list(args):
    return [ x.obj_id for x in cib_factory.get_elems_on_type('type:primitive') if x.node.get('class') == 'stonith'
           ]
...
class CompletionHelp(object):
    ...
    def help(cls, topic, helptxt):
        if cls.lasttopic == topic and time.time() - cls.laststamp < cls.timeout:
            return
        if helptxt:
            import readline
...
def primitive_complete_complex(args):
    ...
    if agent is None:
        ...
        completers_set = {'params': _prim_params_completer, 
...
class CibConfig(command.UI):
    ...
    @command.skill_level('administrator')
    @command.completers(_f_group_id_list, compl.choice(['add', 'remove']), _prim_id_list, compl.choice(['after', 'before']), _prim_id_list)
    def do_modgroup(self, context, group_id, subcmd, prim_id, *args):
        ...
        if subcmd not in ('add', 'remove'):
            ...
            if args:
                ...
                if len(args) != 2:
                    context.fatal_error("Expected 'after|before <id>' (%d arguments given)" % len(args))
                after_before = args[0]
            ...
            if subcmd == 'add':
                if after_before == 'after':
                    sed_s = 's/ %s( |$)/& %s /' % (ref_member_id, prim_id)
                else:
                    sed_s = 's/ %s( |$)/ %s& /' % (ref_member_id, prim_id)
            else:
                sed_s = 's/ %s( |$)/ /' % prim_id
    ...
    def _stop_if_running(self, rscs):
        ...
        if len(to_stop) > 0:
            ok = all(set_deep_meta_attr(rsc, 'target-role', 'Stopped', commit=False) for rsc in to_stop)
            if not ok or not cib_factory.commit():
                raise ValueError('Failed to stop one or more running resources: %s' % (', ').join(to_stop))
        return len(to_stop)
    ...
    def _commit(self, force=False, replace=False):
        ...
        rc2 = cib_factory.has_no_primitives() or self._verify(mkset_obj('xml', 'changed'), mkset_obj('xml'))
        if rc1 and rc2:
    ...
    def __conf_object(self, cmd, *args):
        ...
        if cmd in constants.cib_cli_map.values() and not cib_factory.is_elem_supported(cmd):
    ...
    @command.skill_level('administrator')
    @command.completers_repeating(compl.null, ui_ra.complete_class_provider_type, primitive_complete_complex)
    ...
    @command.skill_level('administrator')
    @command.completers_repeating(compl.null, compl.call(schema.rng_attr_values, 'rsc_order', 'kind'), top_rsc_tmpl_id_list)
