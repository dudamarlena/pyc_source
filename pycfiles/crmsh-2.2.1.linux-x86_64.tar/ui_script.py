...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/crmsh/ui_script.py
...
class Script(command.UI):
    ...
    def do_convert(self, context, workflow, outdir='.', category='basic'):
        ...
        def scriptsorter(item):
            order = [
             'version', 'name', 'category', 'shortdesc', 'longdesc', 'include', 'parameters', 'steps', 'actions']
    ...
    def _json_list(self, context, cmd):
        ...
        for name in scripts.list_scripts():
            try:
                script = scripts.load_script(name)
                if script is not None:
                    print json.dumps({'name': name, 'category': script['category'].lower(), 
                       'shortdesc': script['shortdesc'], 
                       'longdesc': scripts.format_desc(script['longdesc'])})
            except ValueError as err:
                print json.dumps({'name': name, 'error': str(err)})
    ...
    def _json_show(self, context, cmd):
        ...
        if len(cmd) < 2:
            ...
            if script is None:
                return False
            print json.dumps({'name': script['name'], 'category': script['category'].lower(), 
               'shortdesc': script['shortdesc'], 
               'longdesc': scripts.format_desc(script['longdesc']), 
    ...
    def _json_verify(self, context, cmd):
        ...
        if len(cmd) < 3:
            ...
            for action in actions:
                obj = {'name': str(action.get('name', '')), 'shortdesc': str(action.get('shortdesc', '')), 'longdesc': str(action.get('longdesc', '')), 
                   'text': str(action.get('text', '')), 
