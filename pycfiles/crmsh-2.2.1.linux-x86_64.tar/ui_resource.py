...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/crmsh/ui_resource.py
# Compiled at: 2016-05-25 17:54:31
from . import command
    ...
    for c in node.iterchildren():
        if c.tag == 'meta_attributes':
            ...
            if nvpair is not None:
                l.append(nvpair)
        elif force_children or xmlutil.is_child_rsc(c) and not c.getparent().tag == 'group':
...
def set_deep_meta_attr_node(target_node, attr, value):
    ...
    if config.core.manage_children != 'never' and (xmlutil.is_group(target_node) or xmlutil.is_clone(target_node) and xmlutil.cloned_el(target_node) == 'group'):
        odd_children = get_children_with_different_attr(target_node, attr, value)
        for c in odd_children:
            if config.core.manage_children == 'always' or config.core.manage_children == 'ask' and utils.ask('Do you want to override %s for child resource %s?' % (
             attr, c.get('id'))):
    ...
    xmlutil.xml_processnodes(target_node, xmlutil.is_emptynvpairs, xmlutil.rmnodes)
...
def set_deep_meta_attr(rsc, attr, value, commit=True):
    ...
    def update_obj(obj):
        ...
        node = obj.node
        obj.set_updated()
        if not (node.tag == 'primitive' and node.getparent().tag == 'group'):
...
class RscMgmt(command.UI):
    ...
    @command.wait
    @command.completers_repeating(compl.resources, compl.nodes, compl.choice(['reboot', 'forever', 'force']))
    ...
    def do_failcount(self, context, rsc, cmd, node, value=None):
        ...
        return ui_utils.manage_attr(context.get_command_name(), self.rsc_failcount, rsc, cmd, node, value)
    ...
    def do_param(self, context, rsc, cmd, param, value=None):
        ...
        return ui_utils.manage_attr(context.get_command_name(), self.rsc_param, rsc, cmd, param, value)

    @command.skill_level('administrator')
    @command.completers(compl.resources, compl.choice(['set', 'stash', 'unstash', 'delete', 'show', 'check']))
    def do_secret(self, context, rsc, cmd, param, value=None):
        ...
        return ui_utils.manage_attr(context.get_command_name(), self.rsc_secret, rsc, cmd, param, value)
    ...
    def do_meta(self, context, rsc, cmd, attr, value=None):
        ...
        meta <rsc> show <attr>"""
        return ui_utils.manage_attr(context.get_command_name(), self.rsc_meta, rsc, cmd, attr, value)
    ...
    def do_utilization(self, context, rsc, cmd, attr, value=None):
        ...
        return ui_utils.manage_attr(context.get_command_name(), self.rsc_utilization, rsc, cmd, attr, value)
