...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/crmsh/ui_history.py
# Compiled at: 2016-05-04 07:56:27
import os, sys, time, re, bz2
from . import config
from . import command
...
ptest_options = [
 '@v+', 'nograph', 'scores', 'actions', 'utilization']
...
class History(command.UI):
    ...
    def _set_source(self, src, live_from_time=None):
        ...
        to_time = ''
        if src == 'live':
            from_time = time.ctime(live_from_time and live_from_time or time.time() - 3600)
        else:
            from_time = ''
    ...
    def do_limit(self, context, from_time='', to_time=''):
        ...
        if options.history == 'live' and not from_time:
            from_time = time.ctime(time.time() - 3600)
        return self._set_period(from_time, to_time)
    ...
    @command.skill_level('administrator')
    @command.completers_repeating(compl.join(compl.call(lambda : crm_report().peinputs_list()), compl.choice(['v'])))
    ...
    @command.skill_level('administrator')
    @command.completers(compl.join(compl.call(lambda : crm_report().peinputs_list()), compl.choice(['log', 'showdot', 'save'])))
    def do_transition(self, context, *args):
        ...
        if argl:
            ...
            f = self._get_pe_byidx(-1)
        if subcmd == 'save' and len(argl) > 1 or subcmd in ('show', 'showdot', 'log') and argl:
    ...
    def _diffhtml(self, s1, s2, t1, t2):
        ...
        fromlines = s1.split('\n')
        tolines = s2.split('\n')
        diff_l = difflib.HtmlDiff(wrapcolumn=60).make_table(fromlines, tolines, t1, t2)
        return ('').join(diff_l)
    ...
    @command.skill_level('administrator')
    @command.completers(compl.join(compl.call(lambda : crm_report().peinputs_list()), compl.choice(['live'])), compl.choice(['status']))
    ...
    @command.skill_level('administrator')
    @command.completers(compl.join(compl.call(lambda : crm_report().peinputs_list()), compl.choice(['live'])))
    ...
    @command.skill_level('administrator')
    @command.completers(compl.join(compl.call(lambda : crm_report().peinputs_list()), compl.choice(['live'])), compl.join(compl.call(lambda : crm_report().peinputs_list()), compl.choice(['live'])))
    ...
    @command.skill_level('administrator')
    @command.completers(compl.call(lambda : crm_report().session_subcmd_list()), compl.call(lambda : crm_report().session_list()))
    def do_session(self, context, subcmd=None, name=None):
        ...
        if name:
            ...
            if subcmd not in ('list', 'update', 'pack'):
                syntax_err(subcmd, context='session')
                return False
            if subcmd in ('update', 'pack') and not self.current_session:
