...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/crmsh/config.py
...
def configure_libdir():
    ...
    """
    dirs = ('/usr/lib64', '/usr/libexec', '/usr/lib', '/usr/local/lib64', '/usr/local/libexec',
            '/usr/local/lib')
    try:
        import sysconfig
...
_PATHLIST = {'datadir': ('/usr/share', '/usr/local/share', '/opt'), 
...
class opt_color(object):
    ...
    def __init__(self, val):
        self.default = val
        self.completions = ('black', 'blue', 'green', 'cyan', 'red', 'magenta', 'yellow',
                            'white', 'bold', 'blink', 'dim', 'reverse', 'underline',
                            'normal')
...
DEFAULTS = {'core': {'editor': opt_program('EDITOR', ('vim', 'vi', 'emacs', 'nano')), 
   ...
   'path': {'sharedir': opt_dir('%(datadir)s/crmsh'), 
   ...
   'color': {'style': opt_multichoice('color', ('plain', 'color-always', 'color', 'uppercase')), 
