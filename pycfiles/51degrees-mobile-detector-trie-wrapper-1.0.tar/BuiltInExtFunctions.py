...
# Embedded file name: \Ft\Xml\XPath\BuiltInExtFunctions.py
# Compiled at: 2006-08-27 19:00:10
...
def BaseUri(context, arg=None):
    ...
    if arg is None:
        ...
        raise TypeError('%r must be a node-set, not a %s' % (arg, Types.g_xpathPrimitiveTypes.get(type(arg), type(arg).__name__)))
    return node.baseURI or ''
    return
...
If.arguments = (
 Types.BooleanType, Types.ObjectType, Types.ObjectType)
...
Indent.arguments = (
 Types.StringType, Types.NumberType, Types.StringType)
...
def ParseXml(context, src, parameters=None):
    ...
    if not uri:
        uri = OsPathToUri('__parse-xml-extension-function__', attemptAbsolute=1)
    uri += '%sXML-string-%s' % (uri.find('#') + 1 and ';' or '#', str(time.time()))
...
def SharePath(context):
    ...
    """
    return unicode(Ft.GetConfigVar('LOCALSTATEDIR'), 'us-ascii', errors='replace')
...
Join.arguments = (
 Types.NodesetType, Types.StringType)
...
ExtFunctions = {(FT_EXT_NAMESPACE, 'base-uri'): BaseUri, (FT_EXT_NAMESPACE, 'decode'): Decode, (FT_EXT_NAMESPACE, 'encode'): Encode, (FT_EXT_NAMESPACE, 'ends-with'): EndsWith, (FT_EXT_NAMESPACE, 'escape-xml'): EscapeXml, (FT_EXT_NAMESPACE, 'generate-uuid'): GenerateUuid, (FT_EXT_NAMESPACE, 'if'): If, (FT_EXT_NAMESPACE, 'import-string'): ImportString, (FT_EXT_NAMESPACE, 'indent'): Indent, (FT_EXT_NAMESPACE, 'join'): Join, (FT_EXT_NAMESPACE, 'match'): Match, (FT_EXT_NAMESPACE, 'normalize-eol'): NormalizeEol, (FT_EXT_NAMESPACE, 'ospath2uri'): OsPath2Uri, (FT_EXT_NAMESPACE, 'parse-date'): ParseDate, (FT_EXT_NAMESPACE, 'pytime-to-exslt'): PytimeToExslt, (FT_EXT_NAMESPACE, 'parse-xml'): ParseXml, (FT_EXT_NAMESPACE, 'random'): Random, (FT_EXT_NAMESPACE, 'range'): Range, (FT_EXT_NAMESPACE, 'replace'): Replace, (FT_EXT_NAMESPACE, 'resolve-url'): ResolveUrl, (FT_EXT_NAMESPACE, 'resolve-path'): ResolvePath, (FT_EXT_NAMESPACE, 'sha-hash'): ShaHash, (FT_EXT_NAMESPACE, 'share-path'): SharePath, (FT_EXT_NAMESPACE, 'bin-path'): BinPath, (FT_EXT_NAMESPACE, 'uri2ospath'): Uri2OsPath, (FT_EXT_NAMESPACE, 'version'): Version, (FT_EXT_NAMESPACE, 'wrap'): Wrap, (FT_EXT_NAMESPACE, 'strftime'): StrFTime}
InsecureExtFunctions = {(FT_EXT_NAMESPACE, 'spawnv'): Spawnv, (FT_EXT_NAMESPACE, 'system'): System, (FT_EXT_NAMESPACE, 'env-var'): EnvVar}
