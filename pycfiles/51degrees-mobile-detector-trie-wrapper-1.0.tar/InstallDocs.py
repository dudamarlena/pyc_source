...
# Embedded file name: \Ft\Lib\DistExt\InstallDocs.py
...
class InstallDocs(Command):
    ...
    user_options = [
     (
      'install-dir=', 'd', 'directory to install documentation to'), ('build-dir=', 'b', 'build directory (where to install from)'), ('force', 'f', 'force installation (overwrite existing files)'), ('skip-build', None, 'skip the build steps')]
    boolean_options = [
     'force', 'skip-build']
    ...
    def render_documents(self):
        ...
        for document in self.documents:
            ...
            if document_mtime is None:
                ...
                self.announce('rendering %s -> %s' % (document.source, filename), 2)
                try:
                    ...
                    if DEBUG:
                        raise
                    raise DistutilsFileError('could not render %s (%s)' % (document.source, exc))
    ...
    def render_document(self, document, stylesheet, outfile):
        ...
        try:
            ...
            isrc.close()
        params = {'name': self.distribution.get_name(), 'version': self.distribution.version, 'fullname': self.distribution.get_fullname(), 'author': self.distribution.author, 'author-email': self.distribution.author_email}
        ...
        try:
            ...
            try:
                try:
                    self._xslt_processor.run(isrc, topLevelParams=params, outputStream=stream)
                    stream.write('\n')
                finally:
                    stream.close()
            except:
    ...
    def get_stylesheet_obj(self, stylesheet):
        ...
        if uri is None:
            raise DistutilsFileError("no stylesheet file defined for '%s'" % stylesheet)
