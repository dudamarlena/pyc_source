...
# Embedded file name: \Ft\Xml\ThirdParty\Xvif\xmlchargen.py
# Compiled at: 2003-12-07 17:45:34
...
intervall = re.compile('^\\[#x([0-9A-F]+)-#x([0-9A-F]+)\\]$')
...
class CharacterParser:
    ...
    def as_re_class(self):
        ...
        for a in xalts:
            if type(a) is types.TupleType:
                alts.append('%s-%s' % (uniclass(a[0]), uniclass(a[1])))
            else:
...
NameChar = union(Letter, Digit, CharacterParser('#x002E | #x002D | #x005F | #x003A'), CombiningChar, Extender)
FirstNameChar = union(Letter, CharacterParser('#x003A | #x005F'))
...
if __name__ == '__main__':
    ...
    print 'import re'
    for n in ['BaseChar', 'Ideographic', 'CombiningChar', 'Digit', 'Extender', 'Letter', 'NameChar', 'Name', 'Names', 'Nmtoken', 'Nmtokens', 'FirstNCNameChar', 'NCNameChar', 'NCName']:
        print '_re_%s = None' % n
        print 'def re_%s():' % n
