...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/notedrive/baidu/token_.py
# Compiled at: 2019-11-18 02:48:07
# Size of source mod 2**32: 4143 bytes
import os, demjson, requests
...
class AccessToken:
    ...
    def get_authorization_code(self):
        url1 = 'https://openapi.baidu.com/oauth/2.0/authorize?response_type=code&client_id={}&redirect_uri={}&scope=netdisk'.format(self.secret['token']['client_id'], 'oob')
        print(url1)
    ...
    def init_refresh_token(self):
        ...
        url2 = 'https://openapi.baidu.com/oauth/2.0/token?grant_type=authorization_code&code={}&client_id={}&client_secret={}&scope=basic,netdisk&redirect_uri=oob'.format(self.secret['token']['authorization_code'], self.secret['token']['client_id'], self.secret['token']['client_secret'])
        print(url2)
    ...
    def refresh_access_token(self):
        url3 = 'https://openapi.baidu.com/oauth/2.0/token?grant_type=refresh_token&refresh_token={}&client_id={}&scope=basic+netdisk&client_secret={}'.format(self.secret['token']['refresh_token'], self.secret['token']['client_id'], self.secret['token']['client_secret'])
        try:
            print(url3)
    ...
    def get_access_token(self):
        ...
        if self.secret['token']['refresh_token'] is not None:
    ...
    def read(self):
        ...
        if self.secret is None:
            self.secret = {'token': {}}
        if not ('token' not in self.secret.keys() or isinstance(self.secret['token'], dict)):
            self.secret['token'] = {}
