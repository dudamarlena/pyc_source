...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/notedrive/baidu/drive.py
# Compiled at: 2020-04-19 10:26:09
# Size of source mod 2**32: 31144 bytes
import hashlib, os
        ...
        while 1:
            if self.mutex.acquire():
                if self.position > self.file_size - 1:
                    ...
                    return
                interval = (
                 self.position, self.position + self.chunk)
                self.position += self.chunk + 1
                self.mutex.release()
            resp = self.session.get((self.url),
              headers={'Range': 'bytes=%s-%s' % interval})
...
class SuperDownloaderM(object):
    ...
    def __init__(self, session, thread_num=10, queue_size=10, chunk=1048576):
        """
        :param session: 构造好上下文的session
        :param thread_num: 下载线程数
        :param queue_size: 队列的大小，默认10
    ...
    def download(self, meta, overwrite=True):
        ...
        if os.path.exists(local_path):
            if not overwrite:
                if meta['md5'] == get_file_md5(local_path):
                    info(file_name + 'the same md5,file has been exist!, pass')
                    return True
        if meta['size'] < 10485760:
    ...
    def download_single(self, meta):
        ...
        with open(local_path, mode=mode) as (f):
            with tqdm(initial=(np.round(first_byte / self.chunk, 2)), total=(np.round(file_size / self.chunk, 2)),
              unit='MB',
              desc=file_name) as (pbar):
                while position < file_size:
                    interval = (
                     position, position + self.chunk)
    ...
    def _consume(self, fp, meta):
        with tqdm(total=(np.round(meta['size'] / self.chunk)), unit='MB',
...
class BaiDuDrive(object):
    ...
    def quota(self):
        ...
        :return quota: \x7fuint64   空间配额，单位为字节。
        :return used: uint64\x7f   \x7f已使用空间大小，单位为字节。
        """
    ...
    def upload(self, local_path, yun_path, overwrite=True):
        ...
        if overwrite:
            ...
            ondup = 'newcopy'
        params = {'method':'upload',  'path':yun_path, 
         'ondup':ondup}
    ...
    def meta(self, yun_path):
        ...
        :param yun_path:需要获取文件属性的目录，以/开头的绝对路径。如：/apps/album/a/b/c
         注意：: 路径长度限制为1000   非必传   文件名或路径名开头结尾不能是“.”或空白字符，空白字符包括: 
, 
,       , 空格, \x00, \x0b
...
,       , 空格, \x00, \x0b
    ...
    def list_deep(self, yun_dir, by='name', order='asc', limit='0-100', depth=0, max_depth=5):
        ...
        for file in files:
            ...
            paths.append(file)
            if file['isdir'] == 1:
                paths.extend(self.list_deep((file['path']), by=by, order=order, limit=limit, depth=depth, max_depth=max_depth))
    ...
    def request(self, method, uri, headers=None, params=None, data=None, files=None, stream=None, base_url=None):
        ...
        if params is None:
            params = {}
        params.update({'app_id': params.get('app_id', None) or APP_ID})
        url = base_url + uri
    ...
    def parse_path(yun_path, loc_path=None, isdir=True):
        ...
        if isdir:
            ...
            os.path.exists(loc_path) or os.makedirs(loc_path)
        else:
            ...
            if loc_path is None:
                ...
                if loc_dir is None or loc_dir == '':
                    ...
                    if loc_dir[0] != '/':
                        ...
                        os.path.exists(loc_dir) or os.makedirs(loc_dir)
        return (
         yun_path, loc_path)
