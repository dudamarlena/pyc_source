...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/timelog/lib.py
# Compiled at: 2011-06-22 15:05:08
import fileinput
...
PATTERN = '^([0-9]{4}-[0-9]{2}-[0-9]{2} [0-9:]{8},[0-9]{3}) (GET|POST|PUT|DELETE|HEAD) "(.*)" \\((.*)\\) (.*)'
CACHED_VIEWS = {}
...
def generate_table_from(data):
    ...
    for item in data:
        ...
        try:
            stdev = '%.2f' % (sdsq / (len(data[item]['times']) - 1)) ** 0.5
