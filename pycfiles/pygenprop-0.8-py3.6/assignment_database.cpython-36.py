...
class Sample(Base):
    __doc__ = 'A table of samples representing genomes and metagenomes for which genome properties has been run on.'
...
class PropertyAssignment(Base):
    __doc__ = 'An assignment for a genome property (YES, NO, or PARTIAL).'
...
class StepAssignment(Base):
    __doc__ = 'An assignment for a genome property step. If present then it is assumed the step is present (e.g. YES).'
...
class InterProScanMatch(Base):
    __doc__ = 'Contains information about an InterProScan match supporting a step.'
...
class Sequence(Base):
    __doc__ = 'Contains information about protein sequence.'
