...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/pygenprop/results.py
# Compiled at: 2020-02-12 23:26:22
# Size of source mod 2**32: 32587 bytes
"""
...
from pygenprop.assignment_database import Base, Sample, PropertyAssignment, StepAssignment, InterProScanMatch, Sequence, step_match_association_table
...
class GenomePropertiesResults(object):
    ...
    def get_results(self, *property_identifiers, steps=False, names=False):
        ...
        if names:
            ...
            named_results['Property_Name'] = named_results['Property_Identifier'].apply(lambda property_identifier: self.tree[property_identifier].name)
            if steps:
                named_results['Step_Name'] = named_results[['Property_Identifier', 'Step_Number']].apply((lambda row: self.get_step_name(row['Property_Identifier'], row['Step_Number'])),
                  axis=1)
    ...
    def get_property_result(self, genome_property_id, sample=None):
        ...
        try:
            ...
            property_result = [
             'NO'] * len(property_results.columns)
    ...
    def remove_results_with_shared_assignments(results, only_drop_no_assignments=False):
        ...
        if only_drop_no_assignments:
            results_to_drop = [column for column in single_value_columns if results_transposed[column].unique()[0] == 'NO']
        else:
            results_to_drop = [column for column in single_value_columns]
    ...
    def to_json(self, file_handle=None):
        ...
        """
        json_data = {'sample_names':self.sample_names, 
         'property_tree':self.generate_json_tree(self.tree.root)}
        if file_handle:
        ...
        for step in genome_properties_root.steps:
            ...
            if step_child_properties:
                ...
                step_dict = {'step_id':step.number, 
                 'name':step.name, 
                 'enabled':False, 
                 'result':self.get_step_result(genome_properties_root.id, step.number)}
    ...
    def to_assignment_database(self, engine: SQLAlchemyEngine, drop_existing=True):
        ...
        for sample_name in self.sample_names:
            ...
            for property_identifier in self.properties:
                property_result = self.get_property_result(property_identifier, sample=(sample.name))
                ...
                for step_number in self.get_step_numbers_for_property(property_identifier):
                    ...
                    if step_result == 'YES':
                        current_steps_assignments.append(StepAssignment(number=step_number, property_assignment=property_assignment))
                    else:
                        continue
...
def load_sample_assignments_from_database(sample_cache, sample):
    ...
    for property_assignment in sample.property_assignments:
        ...
        for step_assignment in property_assignment.step_assignments:
            sample_cache.cache_step_assignment(property_assignment.identifier, step_assignment.number, step_assignment.assignment)
...
class GenomePropertiesResultsWithMatches(GenomePropertiesResults):
    ...
    def __init__(self, *genome_properties_results: AssignmentCacheWithMatches, properties_tree: GenomePropertiesTree):
        ...
        all_matches = []
        for assignment in genome_properties_results:
            step_matches = property_identifiers.merge((assignment.matches), left_on='Signature_Accession', right_index=True,
              copy=False)
            step_matches.drop((assignment.get_unshared_identifiers(properties_tree)), axis=0,
              level=0,
              errors='ignore',
              inplace=True)
    ...
    def to_assignment_database(self, engine: SQLAlchemyEngine, drop_existing=True):
        ...
        for sample in current_session.query(Sample):
            unique_interproscan = self._get_unique_interproscan_matches(sample.name).apply((self.create_interproscan_match),
              axis=1).tolist()
            ...
            for interproscan in unique_interproscan:
                ...
                unique_interproscan_dict[interproscan.interpro_signature][interproscan.sequence_identifier][interproscan.expected_value] = interproscan

            for property_assignment in sample.property_assignments:
                for step_assignment in property_assignment.step_assignments:
                    step_matches = self.get_step_matches((property_assignment.identifier), (step_assignment.number), sample=(sample.name))
                    if step_matches is not None:
                        wrapper = partial((self.connect_step_assignments_to_interproscan_matches), step_assignment=step_assignment,
    ...
    def _top_step_matches(self):
        ...
        return self.step_matches.sort_values('E-value').groupby(['Sample_Name',
         'Property_Identifier',
         'Step_Number'])[('Signature_Accession', 'Protein_Accession', 'E-value', 'Sequence')].first()
    ...
    def create_skbio_protein_sequence(match_row):
        ...
        """
        metadata = {'id':match_row['Protein_Accession'], 
         'description':'(From ' + match_row['Sample_Name'] + ')'}
    ...
    def create_interproscan_match(match_row):
        ...
        """
        return InterProScanMatch(sequence_identifier=(match_row['Protein_Accession']), interpro_signature=(match_row['Signature_Accession']),
...
def load_assignment_caches_from_database_with_matches(engine):
    ...
    assignment_caches = []
    for sample in current_session.query(Sample):
        query_part_one = current_session.query(InterProScanMatch.interpro_signature, InterProScanMatch.sequence_identifier, InterProScanMatch.expected_value, Sequence.sequence)
