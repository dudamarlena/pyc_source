...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/percolate/hpc.py
# Compiled at: 2015-12-12 17:04:41
# Size of source mod 2**32: 24925 bytes
"""
...
import numpy as np, networkx as nx, scipy.stats, simoa
...
def _ndarray_dtype(fields):
    ...
    Helper function
    """
    return [(np.str_(key), values) for key, values in fields]
...
def bond_sample_states(perc_graph, num_nodes, num_edges, seed, spanning_cluster=True, auxiliary_node_attributes=None, auxiliary_edge_attributes=None, spanning_sides=None, **kwargs):
    r"""
    ...
    if spanning_cluster and len(spanning_sides) != 2:
        raise ValueError('Spanning cluster is to be detected, but auxiliary nodes of less or more than 2 types (sides) given.')
...
def bond_microcanonical_statistics(perc_graph, num_nodes, num_edges, seed, spanning_cluster=True, auxiliary_node_attributes=None, auxiliary_edge_attributes=None, spanning_sides=None, **kwargs):
    """
    ...
    """
    sample_states = bond_sample_states(perc_graph=perc_graph, num_nodes=num_nodes, num_edges=num_edges, seed=seed, spanning_cluster=spanning_cluster, auxiliary_node_attributes=auxiliary_node_attributes, auxiliary_edge_attributes=auxiliary_edge_attributes, spanning_sides=spanning_sides)
    return np.fromiter(sample_states, dtype=microcanonical_statistics_dtype(spanning_cluster), count=num_edges + 1)
...
def bond_canonical_statistics(microcanonical_statistics, convolution_factors, **kwargs):
    ...
    """
    spanning_cluster = 'has_spanning_cluster' in microcanonical_statistics.dtype.names
    ret = np.empty(1, dtype=canonical_statistics_dtype(spanning_cluster))
    if spanning_cluster:
        ret['percolation_probability'] = np.sum(convolution_factors * microcanonical_statistics['has_spanning_cluster'])
    ret['max_cluster_size'] = np.sum(convolution_factors * microcanonical_statistics['max_cluster_size'])
    ret['moments'] = np.sum(convolution_factors[:, np.newaxis] * microcanonical_statistics['moments'], axis=0)
    return ret
...
def bond_initialize_canonical_averages(canonical_statistics, **kwargs):
    """
    ...
    """
    spanning_cluster = 'percolation_probability' in canonical_statistics.dtype.names
    ret = np.empty_like(canonical_statistics, dtype=canonical_averages_dtype(spanning_cluster=spanning_cluster))
    ret['number_of_runs'] = 1
    if spanning_cluster:
        ret['percolation_probability_mean'] = canonical_statistics['percolation_probability']
        ret['percolation_probability_m2'] = 0.0
    ret['max_cluster_size_mean'] = canonical_statistics['max_cluster_size']
...
def bond_reduce(row_a, row_b):
    ...
    """
    spanning_cluster = 'percolation_probability_mean' in row_a.dtype.names and 'percolation_probability_mean' in row_b.dtype.names and 'percolation_probability_m2' in row_a.dtype.names and 'percolation_probability_m2' in row_b.dtype.names
    ret = np.empty_like(row_a)

    def _reducer(key, transpose=False):
        mean_key = '{}_mean'.format(key)
        m2_key = '{}_m2'.format(key)
        res = simoa.stats.online_variance(*[(row['number_of_runs'], row[mean_key].T if transpose else row[mean_key], row[m2_key].T if transpose else row[m2_key]) for row in [row_a, row_b]])
        ret[mean_key], ret[m2_key] = (res[1].T, res[2].T) if transpose else res[1:]
...
def finalize_canonical_averages(number_of_nodes, ps, canonical_averages, alpha):
    """
    Finalize canonical averages
    """
    spanning_cluster = 'percolation_probability_mean' in canonical_averages.dtype.names and 'percolation_probability_m2' in canonical_averages.dtype.names
    ret = np.empty_like(canonical_averages, dtype=finalized_canonical_averages_dtype(spanning_cluster=spanning_cluster))
    ...
    def _transform(original_key, final_key=None, normalize=False, transpose=False):
        if final_key is None:
            final_key = original_key
        keys_mean = ['{}_mean'.format(key) for key in [original_key, final_key]]
        keys_std = [
        ...
        result = np.sqrt((array.T if transpose else array) / (n - 1))
        ret[keys_std[1]] = result.T if transpose else result
        if normalize:
        ...
        result = scipy.stats.t.interval(1 - alpha, df=n - 1, loc=mean, scale=scale)
        ret[key_ci][(Ellipsis, 0)], ret[key_ci][(Ellipsis, 1)] = [my_array.T for my_array in result] if transpose else result
