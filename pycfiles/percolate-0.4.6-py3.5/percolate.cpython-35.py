...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/percolate/percolate.py
# Compiled at: 2015-12-12 17:04:41
# Size of source mod 2**32: 37687 bytes
"""
...
import copy, numpy as np, scipy.stats, networkx as nx
...
def _microcanonical_average_max_cluster_size(max_cluster_size, alpha):
    ...
    if max_cluster_size_sample_std:
        ...
        ret['max_cluster_size_ci'] = scipy.stats.t.interval(1 - alpha, df=runs - 1, loc=max_cluster_size_sample_mean, scale=max_cluster_size_sample_std / sqrt_n)
...
def _microcanonical_average_moments(moments, alpha):
    ...
    for k in range(5):
        if moments_sample_std[k]:
            old_settings = np.seterr(all='raise')
            ret['moments_ci'][k] = scipy.stats.t.interval(1 - alpha, df=runs - 1, loc=moments_sample_mean[k], scale=moments_sample_std[k] / sqrt_n)
            np.seterr(**old_settings)
        else:
...
def microcanonical_averages(graph, runs=40, spanning_cluster=True, model='bond', alpha=alpha_1sigma, copy_result=True):
    ...
    if alpha <= 0.0 or alpha >= 1.0:
        raise ValueError('alpha needs to be a float in the interval (0, 1)')
    run_iterators = [sample_states(graph, spanning_cluster=spanning_cluster, model=model, copy_result=False) for _ in range(runs)]
    ret = dict()
        ...
        for key, value in microcanonical_averages_arrays.items():
            ...
            if key in ('max_cluster_size', 'spanning_cluster'):
                ...
                if key == 'moments':
                    ...
                    if key == 'moments_ci':
                        ret[key][:, p_index] = np.sum(np.rollaxis(np.tile(binomials, (5,
                                                                                      2,
                                                                                      1)), 2, 1) * value, axis=1)
                    else:
...
def statistics(graph, ps, spanning_cluster=True, model='bond', alpha=alpha_1sigma, runs=40):
    ...
    """
    my_microcanonical_averages = microcanonical_averages(graph=graph, runs=runs, spanning_cluster=spanning_cluster, model=model, alpha=alpha)
