...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/percolate/test/test_percolate.py
# Compiled at: 2015-12-12 17:04:41
# Size of source mod 2**32: 7262 bytes
...
import pytest, inspect, percolate, numpy as np, scipy.stats
...
def test_sample_state_signature():
    _test_signature(percolate.sample_states, [
...
def grid_3x3_graph(request):
    ...
    ret.add_edges_from([(i, i + j) for i in [1, 4, 7] for j in [-1, 1]])
    ret.add_edges_from([(i, i + j) for i in [3, 4, 5] for j in [-3, 3]])
    if request.param:
...
def test_initial_iteration(grid_3x3_graph):
    ...
    ret = next(percolate.sample_states(grid_3x3_graph, spanning_cluster=spanning_cluster))
    assert ret['n'] == 0
...
def state_it(grid_3x3_graph):
    spanning_cluster = grid_3x3_graph.graph['span']
    ret = percolate.sample_states(grid_3x3_graph, spanning_cluster=spanning_cluster)
    return (
...
def test_max_cluster_size(state_it):
    ...
    for state in state_it[0]:
        if not state['max_cluster_size'] == max_cluster_sizes[state['n']]:
            raise AssertionError
...
def test_moments(state_it):
    ...
    moments = np.array([
     ...
     [
      0, 0, 0, 0, 0]])
    for state in state_it[0]:
        if not np.array_equal(moments[state['n']], state['moments']):
            raise AssertionError
...
def test_spanning(state_it):
    ...
    for state in state_it[0]:
        if not state['has_spanning_cluster'] == has_spanning_cluster[state['n']]:
            raise AssertionError
...
def test_microcanonical_averages_signature():
    _test_signature(percolate.microcanonical_averages, [
...
def test_microcanonical_averages_initial_iteration(grid_3x3_graph):
    spanning_cluster = grid_3x3_graph.graph['span']
    ret = next(percolate.microcanonical_averages(grid_3x3_graph, spanning_cluster=spanning_cluster))
    assert ret['n'] == 0
    ...
    if spanning_cluster:
        assert ret['spanning_cluster'] == 0.023809523809523808
        np.testing.assert_allclose(np.array([0, 1]) + np.array([1, -1]) * scipy.stats.beta.cdf(ret['spanning_cluster_ci'], a=1, b=41), scipy.stats.norm.cdf(-1) * np.ones(2))


def test_microcanonical_averages_number_of_iterations(grid_3x3_graph):
    spanning_cluster = grid_3x3_graph.graph['span']
    n = 0
    for microcanonical_average in percolate.microcanonical_averages(grid_3x3_graph, spanning_cluster=spanning_cluster):
...
def test_binomials_normalized():
    np.testing.assert_allclose(percolate.percolate._binomial_pmf(1000, 0.01).sum(), 1.0)


def test_binomials_close_to_scipy():
    np.testing.assert_allclose(percolate.percolate._binomial_pmf(100, 0.1), scipy.stats.binom.pmf(np.arange(101), n=100, p=0.1))