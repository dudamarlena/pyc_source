...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/macroeco/models/_curves.py
...
class curve(object):
    ...
    def fit_lsq(self, x, y_obs, params_start=None):
        ...
        params_fit, _, _, msg, ier = optimize.leastsq(residuals, params_start, args=(
         x, y_obs), full_output=True)
        if ier > 4:
            raise ValueError, 'Least squares fit did not converge with message %s' % msg
...
class sampling_sar_gen(curve):
    ...
    def _upscale_direct(self, a, S, N, sad_k, ssad_k, approx):
        ...
        S_calc = optimize.brentq(up_fxn, S, a * S, xtol=1e-05)
        return S_calc
    ...
    def _downscale_iterative(self, a, S, N, sad_k, ssad_k, approx, delta=2):
        ...
        while a_down > a:
            ...
            S_down = self._downscale_direct(1 / delta, S_down, N_down, sad_k, ssad_k, approx)
    ...
    def _upscale_iterative(self, a, S, N, sad_k, ssad_k, approx, delta=2):
        ...
        while a_up < a:
            ...
            a_up = a_up * delta
            S_up = self._upscale_direct(delta, S_up, N_up, sad_k, ssad_k, approx)
            S_chain.append(S_up)
            N_up = N_up * delta
...
sampling_sar = sampling_sar_gen(name='sampling_sar', parameters='S0,N0,sad_k,ssad_k', iterative=False)
sampling_ear = sampling_sar_gen(name='sampling_ear', parameters='S0,N0,sad_k,ssad_k', iterative=False, ear=True)
sampling_sar_iterative = sampling_sar_gen(name='sampling_sar', parameters='S0,N0,sad_k,ssad_k', iterative=True)
...
class mete_sar_gen(sampling_sar_gen):
    ...
    def _vals(self, x, S0, N0, approx=True):
        sampling_sar = sampling_sar_gen(self.name, self.parameters, self.iterative, self.ear)
...
mete_sar_iterative = mete_sar_gen(name='mete_iterative_sar', parameters='S0,N0', iterative=True)
    ...
    for i in xrange(doublings + 1):
        if i == 0:
            ...
            def S2A_calc(x, SA, N2A):
                return (SA + N2A * (1 - x) / (x - x ** (N2A + 1)) * (1 - x ** N2A / (N2A + 1))) / x ** (-1)

            def x_calc(x, SA, N2A):
                return S2A_calc(x, SA, N2A) / N2A * x * (x ** N2A - 1) / (x - 1) - (x ** N2A * -lerchphi(x, 1, N2A + 1) - np.log(1 - x)) - 1e-23

            x = optimize.brentq(x_calc, 1e-24, 0.9999999999999999, args=(SA, N2A), xtol=1e-16, maxiter=1000, disp=True) + 1e-23
...
class _nbinom_ztrunc():
    ...
    def pmf(self, x):
        mu = self.N / self.S
        return dist.nbinom_ztrunc.pmf(x, mu, self.k) / dist.nbinom_ztrunc.cdf(self.N, mu, self.k)
