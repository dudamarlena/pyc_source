...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/macroeco/main/_main.py
# Compiled at: 2015-10-07 18:42:13
from __future__ import division
import sys, os, shutil, warnings, inspect, configparser, threading as thread, traceback, copy, logging, numpy as np, pandas as pd, matplotlib as mpl
mpl.use('agg')
...
def main(param_path='parameters.txt'):
    ...
    filefmt = logging.Formatter(time.strftime('%Y/%m/%d %H:%M:%S %p', time.localtime()) + ' - %(name)s - %(levelname)s - %(message)s')
    ...
    if len(bad_params[0]) > 0:
        logging.warning('Possible formatting error(s) in' + ' %s: parameters %s on lines %s' % (
...
def _do_format(options):
    datapath = os.path.normpath(os.path.join(options['param_dir'], options['data']))
...
def _function_location(options):
    func_name = options['analysis'].split('.')[0]
    ...
    if func_name in emp_members:
        ...
        raise ValueError, "No analysis of type '%s' is available" % options['analysis']
    return module
...
def _get_args_kwargs(options, module):
    ...
    for arg_name in arg_names:
        ...
        try:
            ...
            raise ValueError, 'Value for required argument %s not provided' % arg_name
    ...
    for kw_name in kw_names:
        if kw_name in options.keys():
            try:
                ...
                raise ValueError, 'Value for optional argument %s is invalid' % kw_name
...
def _emp_extra_options(options):
    ...
    """
    metadata_path = os.path.normpath(os.path.join(options['param_dir'], options['metadata']))
...
def _arg_kwarg_lists(module, analysis):
    exec 'arg_and_kwd_names, _, _, kw_defaults = inspect.getargspec(%s.%s)' % (
     module, analysis)
    if kw_defaults:
    ...
    try:
        ...
        if obj_meth[1] not in ('fit_mle', 'translate_args'):
            arg_names += eval(module + '.' + obj_meth[0] + '.' + "shapes.replace(' ','').split(',')")
        if obj_meth[1] == 'rvs':
...
def _fit_models(options, core_results):
    ...
    for core_result in core_results:
        ...
        for model in models:
            ...
            stat_names, stats = _get_comparison_stat(core_result, values, model, fits)
...
def _write_core_tables(options, module, core_results):
    ...
    elif type(core_results) == type(pd.DataFrame()):
...
def _get_file_path(spid, options, file_name):
    return os.path.join(options['run_dir'], '%i_%s' % (spid + 1, file_name))
...
def _write_comparison_plot_table(spid, models, options, core_results, fit_results):
    ...
    df.to_csv(f_path, index=False, float_format='%.4f')
...
def _output_cdf_plot(core_result, spid, models, options, fit_results):
    ...
    _save_table_and_plot(spid, models, options, fit_results, 'data_pred_cdf', df, calc_func, plot_exec_str)


def output_pdf_plot(core_result, spid, models, options, fit_results):
    """ Function for plotting pdf/pmf """
    hist_bins = 11
    emp_hist, edges = np.histogram(core_result['y'].values, hist_bins, normed=True)
    ...
    _save_table_and_plot(spid, models, options, fit_results, 'data_pred_pdf', df, calc_func, plot_exec_str)