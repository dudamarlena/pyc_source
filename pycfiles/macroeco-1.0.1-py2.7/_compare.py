...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/macroeco/compare/_compare.py
# Compiled at: 2015-10-07 18:42:13
from __future__ import division
import numpy as np, scipy as sp, scipy.stats as stats, pandas as pd
from ..misc import doc_sub
_data_doc = 'data : iterable\n        Data for analysis'
_model_doc = 'model : obj\n        Scipy frozen distribution object. When freezing, keyword args ``loc``\n        and ``scale`` should only be included if they represent a parameter.\n    '
...
def lrt(data, model_full, model_reduced, df=None):
    ...
    if not df:
        df = len(model_full.args) + len(model_full.kwds) - len(model_reduced.args) - len(model_reduced.kwds)
...
def r_squared(obs, pred, one_to_one=False, log_trans=False):
    ...
    if log_trans:
        ...
        pred = np.log(pred)
    if one_to_one:
        r_sq = 1 - sum_of_squares(obs, pred) / sum_of_squares(obs, np.mean(obs))
    else:
