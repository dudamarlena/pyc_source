...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/macroeco/models/_distributions.py
# Compiled at: 2015-10-07 18:42:13
from __future__ import division
import sys
from decimal import Decimal
import numpy as np, numpy.random as nprand
from scipy.stats.distributions import rv_discrete, rv_continuous
import scipy.stats as stats, scipy.optimize as optim, scipy.special as special, scipy.integrate as integrate
...
class rv_continuous_meco(rv_continuous):
    ...
    def translate_args(self, *args):
        """{0}"""
        raise NotImplementedError, 'translate_args method not implemented for this distribution'

    @doc_sub(_doc_fit_mle)
    def fit_mle(self, *args):
        """{0}"""
        return self.fit(floc=0, fscale=1, *args)[:-2]
...
class rv_discrete_meco(rv_discrete):
    ...
    def fit_mle(self, *args):
        """{0}"""
        raise NotImplementedError, 'fit_mle method not implemented for this distribution'
    ...
    def rvs_alt(self, *args, **kwargs):
        """{0}"""
        l = kwargs.get('l', 1)
        b = kwargs.get('b', 100000.0)
        size = kwargs.get('size', 1)
        model_cdf = self.cdf(np.arange(l, b + 1), *args)
        unif_rands = np.random.random(size)
        model_rands = np.array([ np.where(tx <= model_cdf)[0][0] + l for tx in unif_rands
                               ])
        return model_rands
...
class dgamma_gen(rv_discrete_meco):
    ...
    def _pmf(self, x, alpha, theta):
        ...
        b = np.atleast_1d(b)
        x = np.atleast_1d(x)
        eq = lambda val, talpha, ttheta: np.exp((talpha - 1) * np.log(val) - val / ttheta)
...
class nbinom_gen(rv_discrete_meco):
    ...
    def _logpmf(self, x, mu, k_agg):
        p = self._get_p_from_mu(mu, k_agg)
        coeff = special.gammaln(k_agg + x) - special.gammaln(x + 1) - special.gammaln(k_agg)
...
class nbinom_ztrunc_gen(rv_discrete_meco):
    ...
    def _pmf(self, x, mu, k_agg):
        x = np.atleast_1d(x)
        norm = np.exp(special.gammaln(k_agg + x) - (special.gammaln(k_agg) + special.gammaln(x + 1)))
    ...
    def _stats(self, mu, k_agg):
        ...
        eta = 1 - omega
        mu = mu
        var = k_agg * eta * (1 + k_agg * eta) / (omega ** 2 * (1 - omega ** k_agg)) - mu ** 2
        return (mu, var, None, None)
...
def _nbinom_ztrunc_p(mu, k_agg):
    ...
    p = optim.brentq(p_eq, 1e-10, 10000000000.0, args=(mu, k_agg))
    return p
...
def _cnbinom_logpmf(n_i, n, a, k_agg):
    return _ln_choose(n_i + k_agg - 1, n_i) + _ln_choose(n - n_i + k_agg / a - k_agg - 1, n - n_i) - _ln_choose(n + k_agg / a - 1, n)
...
class logser_gen(rv_discrete_meco):
    ...
    def translate_args(self, mu):
        ...
        return optim.brentq(eq, 1e-16, 0.9999999999999999, args=mu, disp=True)

    @inherit_docstring_from(rv_continuous_meco)
    def fit_mle(self, data):
        return (
         self.translate_args(np.mean(data)),)
...
b : float
    ...
    def _cdf(self, x, p, b):
        ...
        if p[0] < 1:
            ...
            cdf_list = [ sum(self.pmf(range(1, int(x_i) + 1), p[0], b[0])) for x_i in x
                       ]
        ...
        if p < 1:
            ...
            for rand_num in rand_list:
                ...
                if y(1) > 0:
                    ...
                    out.append(int(round(bisect(y, 1, b))))
...
def _trunc_logser_solver(bins, b):
    ...
    if bins == b:
        ...
        BOUNDS = [
         0, 1]
        DIST_FROM_BOUND = 1e-15
        m = np.array(np.arange(1, np.int(b) + 1))
        y = lambda x: np.sum(x ** m / b * bins) - np.sum(x ** m / m)
        p = optim.bisect(y, BOUNDS[0] + DIST_FROM_BOUND, min((sys.float_info[0] / bins) ** (1 / b), 2), xtol=1.490116e-08, maxiter=1000)
    return p
...
class plnorm_gen(rv_discrete_meco):
    ...
    def rank(self, n, mu, sigma, crit=0.5, upper=10000, xtol=1):
        """%(super)s
...
----------------------
        ...
        """
        return _make_rank(self, n, mu, sigma, crit=crit, upper=upper, xtol=xtol)
    ...
    def _pmf(self, x, mu, sigma):
        ...
        xabove = x > approx_cut
        if np.sum(xbelow) > 0:
            pmf[xbelow] = plognorm_intg_vec(x[xbelow], mu[xbelow], sigma[xbelow])
        if np.sum(xabove) > 0:
            z = (np.log(x[xabove]) - mu[xabove]) / sigma[xabove]
            pmf_above = (1 + (z ** 2 + np.log(x[xabove]) - mu[xabove] - 1) / (2 * x[xabove] * sigma[xabove] ** 2)) * np.exp(-0.5 * z ** 2) / (np.sqrt(2 * np.pi) * sigma[xabove] * x[xabove])
...
----------------------
    ...
    def _cdf(self, x, mu, sigma):
        ...
        sigma = np.atleast_1d(sigma)
        norm = 1 - plognorm_intg_vec(0, mu[0], sigma[0])
        cdf_vals = (plnorm.cdf(x, mu, sigma) - plnorm.cdf(0, mu[0], sigma[0])) / norm
        cdf_vals = np.atleast_1d(cdf_vals)
        cdf_vals[x < 1] = 0
        return cdf_vals


plnorm_ztrunc = plnorm_ztrunc_gen(name='plnorm_ztrunc', shapes='mu, sigma')

def plognorm_intg(x, mu, sigma):
    eq = lambda t, x, mu, sigma: np.exp(t * x - np.exp(t) - 0.5 * ((t - mu) / sigma) ** 2)
    intg = integrate.quad(eq, -np.inf, np.inf, args=(x, mu, sigma))[0]
    norm = np.exp(-0.5 * np.log(2 * np.pi * sigma ** 2) - special.gammaln(x + 1))
    return norm * intg
...
def _expon_solve_lam_from_mu(mu, b):
    ...
    def lam_eq(lam, mu, b):
        ...
        return (1 - (lam * b + 1) * np.exp(-lam * b)) / (lam - lam * np.exp(-lam * b) + Decimal(1e-32)) - mu
...
class lognorm_gen(rv_continuous_meco):
    ...
    def fit_mle(self, data, fix_mean=False):
        ...
        if not fix_mean:
            ...
            mean = np.mean(data)
            mle = lambda sigma, x, mean: -1 * np.sum(self._pdf_w_mean(x, mean, sigma))
            sigma = optim.fmin(mle, np.array([np.std(np.log(data), ddof=1)]), args=(
             data, mean), disp=0)[0]
...
def _make_rank(dist_obj, n, mu, sigma, crit=0.5, upper=10000, xtol=1):
    ...
    for i, tq in enumerate(qs[(~qs_less)]):
        j = ind + i
        try:
            rank[j] = np.abs(np.ceil(optim.brentq(brute_ppf, -1, upper, args=(
             tq,), xtol=xtol)))
        except ValueError:
