...
# [GCC 8.4.0]
# Embedded file name: /Users/gman/Documents/code/Flask-Blogging/test/test_sqlastorage.py
...
class TestSQLiteStorage(FlaskBloggingTestCase):
    ...
    def test_user_post_table_consistency(self):
        user_id = 1
        post_id = 5
        pid = self.storage.save_post(title='Title', text='Sample Text', user_id='user',
          tags=['hello', 'world'])
        posts = self.storage.get_posts()
        self.assertEqual(len(posts), 1)
        self.storage.save_post(title='Title', text='Sample Text', user_id='newuser',
          tags=['hello', 'world'],
          post_id=pid)
        self.assertEqual(len(posts), 1)
    ...
    def test_tag_post_uniqueness(self):
        self.storage.save_post(title='Title', text='Sample Text', user_id='user',
          tags=['tags'])
        table_name = 'tag_posts'
        metadata = self._meta
        table = metadata.tables[table_name]
    ...
    def test_user_post_uniqueness(self):
        pid = self.storage.save_post(title='Title1', text='Sample Text', user_id='testuser',
          tags=[
         'hello', 'world'])
        table_name = 'user_posts'
    ...
    def test_get_post_by_id(self):
        pid1 = self.storage.save_post(title='Title1', text='Sample Text1', user_id='testuser',
          tags=[
         'hello', 'world'])
    ...
    def _create_dummy_data(self):
        for i in range(20):
            ...
            self.storage.save_post(title=('Title%d' % i), text=('Sample Text%d' % i),
              user_id=user,
              tags=tags)
            time.sleep(1)


@unittest.skipUnless(HAS_MYSQL, 'Package mysql-python needs to be install to run this test.')
class TestMySQLStorage(TestSQLiteStorage):

    def _create_storage(self):
        self._engine = create_engine('mysql+mysqldb://root:@localhost/flask_blogging')
        self._meta = sqla.MetaData()
        self.storage = SQLAStorage((self._engine), metadata=(self._meta))
        self._meta.create_all(bind=(self._engine))
...
class TestPostgresStorage(TestSQLiteStorage):
    ...
    def _create_storage(self):
        self._engine = create_engine('postgresql+psycopg2://postgres:@localhost/flask_blogging')
        self._meta = sqla.MetaData()
        self.storage = SQLAStorage((self._engine), metadata=(self._meta))
        self._meta.create_all(bind=(self._engine))
