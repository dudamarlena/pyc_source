...
# [GCC 8.4.0]
# Embedded file name: /users/payno/.local/share/virtualenvs/tomwer_venc/lib/python3.7/site-packages/nxtomomill/app/tomoedf2nx.py
# Compiled at: 2020-03-12 09:27:26
# Size of source mod 2**32: 17947 bytes
"""
...
__license__ = 'MIT'
__date__ = '16/01/2020'
import os, fabio, numpy, h5py, logging
...
def main(argv):
    ...
    """
    import argparse
    parser = argparse.ArgumentParser(description='convert data acquired as edf to hdf5 - nexus compliant file format.')
    ...
    fileout_h5 = utils.get_file_name(file_name=(options.output_file), extension=(options.file_extension),
      check=True)
    ...
    with h5py.File(fileout_h5, 'w') as (h5d):
        ...
        for refs_to_find in REFS_NAME:
            ...
            refs_urls = scan.get_refs_url(scan_path=(scan.path), prefix=refs_to_find,
              ignore=TO_IGNORE)
            if len(refs_urls) > 0:
                break
        ...
        data_dataset = h5d.create_dataset('/entry/instrument/detector/data', shape=(
         n_frames, scan.dim_2, scan.dim_1),
          dtype=frame_type)
        keys_dataset = h5d.create_dataset('/entry/instrument/detector/image_key', shape=(
         n_frames,),
          dtype=(numpy.int32))
        keys_control_dataset = h5d.create_dataset('/entry/instrument/detector/image_key_control', shape=(
         n_frames,),
          dtype=(numpy.int32))
        h5d['/entry/sample/name'] = os.path.basename(scan.path)
        proj_angle = scan.scan_range / scan.tomo_n
        distance = scan.retrieve_information(scan=(os.path.abspath(scan.path)), ref_file=None,
          key='Distance',
          type_=float,
          key_aliases=[
         'distance'])
        ...
        pixel_size = scan.retrieve_information(scan=(os.path.abspath(scan.path)), ref_file=None,
          key='PixelSize',
          type_=float,
          key_aliases=[
         'pixelSize'])
        h5d['/entry/instrument/detector/x_pixel_size'] = pixel_size
        h5d['/entry/instrument/detector/x_pixel_size'].attrs['unit'] = 'mm'
        h5d['/entry/instrument/detector/y_pixel_size'] = pixel_size
        h5d['/entry/instrument/detector/y_pixel_size'].attrs['unit'] = 'mm'
        energy = scan.retrieve_information(scan=(os.path.abspath(scan.path)), ref_file=None,
          key='Energy',
          type_=float,
          key_aliases=[
         'energy'])
        h5d['/entry/beam/incident_energy'] = energy
        h5d['/entry/beam/incident_energy'].attrs['unit'] = 'keV'
        rotation_dataset = h5d.create_dataset('/entry/sample/rotation_angle', shape=(
         n_frames,),
          dtype=(numpy.float32))
        x_dataset = h5d.create_dataset('/entry/sample/x_translation', shape=(
         n_frames,),
          dtype=(numpy.float32))
        y_dataset = h5d.create_dataset('/entry/sample/y_translation', shape=(
         n_frames,),
          dtype=(numpy.float32))
        z_dataset = h5d.create_dataset('/entry/sample/z_translation', shape=(
         n_frames,),
          dtype=(numpy.float32))
        nf = 0
        ...
        def read_url(url) -> tuple:
            ...
            try:
                ...
                _logger.debug('Error while opening %s with fabio', (url.file_path()),
                  exc_info=True)
        ...
        for proj_index in proj_indexes:
            ...
            if iref_pj < len(ref_projs):
                if ref_projs[iref_pj] == nproj:
                    nf = store_refs(ref_indexes, scan.tomo_n, ref_projs[iref_pj], refs_urls, nf, data_dataset, keys_dataset, keys_control_dataset, x_dataset, y_dataset, z_dataset, rotation_dataset, rot_angle_index, x_trans_index, y_trans_index, z_trans_index)
                    iref_pj += 1
                    print('nf out %d ' % nf)
            ...
            elif MOTOR_POS in header:
                str_mot_val = header[MOTOR_POS].split(' ')
                if compute_rotangle is True:
                    if nproj < scan.tomo_n:
