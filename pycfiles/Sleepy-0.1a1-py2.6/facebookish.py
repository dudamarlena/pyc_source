...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.4-i386/egg/sleepy/facebookish.py
...
class Post(object):
    ...
    def _event(self):
        if not self.link:
            ...
            if not match:
                return
            return Event(cacher(cache_name='facebook_events', key=self.object_id, createfunc=lambda : facebook.GraphAPI().get_object(self.object_id)))
            return
        ...
        def _property(prop):
            try:
                if len(prop['text']) < 10:
                    ...
                    prop['text'] = date_str(self.event.start, smusher=None, time_space=False)
                    return prop
...
posts = Entries(entry_class=Post, user_config_key='facebook_user', cache_name='facebook_posts', createfunc=lambda user: facebook.GraphAPI().get_connections(user, 'posts')['data'])