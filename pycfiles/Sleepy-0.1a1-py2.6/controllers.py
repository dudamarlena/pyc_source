...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.4-i386/egg/sleepy/controllers.py
...
class TemplateController(WSGIController):
    ...
    def template(self, page):
        ...
        try:
            getattr(self.Prep, '_all')()
        except AttributeError:
            pass

        try:
            getattr(self.Prep, page)()
        except AttributeError:
            pass

        try:
            return render_mako(s('/pages/{{ page }}.mako', page=page))
...
class ResourceController(WSGIController):
    ...
    def __before__(self, names_path, **kwargs):
        url.environ['SCRIPT_NAME'] = ''
    ...
    def create(self, **kwargs):
        try:
            ret = self._res.create(self._path, self._res.tipe.item.tipe.create_obj(request.params))
    ...
    def update(self, **kwargs):
        try:
            ret = self._res.update(self._path, self._res.tipe.create_obj(request.params))
            return ret
        except Exception:
            self._handle_error()

    def resurrection(self, **kwargs):
        return self._res.resurrection(self._path)

    def resurrect(self, **kwargs):
        try:
    ...
    def reorder(self, **kwargs):
        try:
            return self._res.reorder(self._path, request.params['other'])
        except Exception:
            self._handle_error()
