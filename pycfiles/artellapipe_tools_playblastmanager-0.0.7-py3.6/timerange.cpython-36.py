...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/tools/playblastmanager/plugins/timerange.py
# Compiled at: 2020-05-03 22:03:42
# Size of source mod 2**32: 9910 bytes
"""
...
class TimeRangeWidget(plugin.PlayblastPlugin, object):
    ...
    def get_outputs(self):
        ...
        if mode == TimeRanges.RANGE_TIME_SLIDER:
            ...
            if mode == TimeRanges.RANGE_START_END:
                start = self.start.value()
                end = self.end.value()
            else:
                if mode == TimeRanges.CURRENT_FRAME:
                    frame = tp.Dcc.get_current_frame()
                    start = frame
                    end = frame
                else:
                    if mode == TimeRanges.CUSTOM_FRAMES:
