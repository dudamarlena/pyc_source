...
# [GCC 8.4.0]
# Embedded file name: /Users/x/code/OONI/ooni-probe/ooni/nettests/blocking/web_connectivity.py
...
class WebConnectivityTest(httpt.HTTPTest, dnst.DNSTest):
    ...
    requiredTestHelpers = {'backend': 'web-connectivity', 
       'dns-discovery': 'dns-discovery'}
    requiredOptions = [
     'backend', 'dns-discovery']
    requiresRoot = False
    requiresTor = False
    followRedirects = True
    ignorePrivateRedirects = True
    simpleOptions = [{'name': 'url', 'type': 'text'}, {'name': 'file', 'type': 'file/url'}]
    factor = 0.7
    resolverIp = None
    ...
    def setUp(self):
        ...
        self.control = {'tcp_connect': {}, 'dns': {'addrs': [], 'failure': None}, 
    ...
    def control_request(self, sockets):
        ...
        self.control = yield self.web_connectivity_client.control(http_request=self.input, tcp_connect=sockets, http_request_headers=REQUEST_HEADERS)
    ...
    def compare_http_experiments(self, experiment_http_response):
        ...
        if str(self.control['http_request']['status_code'])[0] != '5':
            self.report['status_code_match'] = self.control['http_request']['status_code'] == experiment_http_response.code
    ...
    def compare_dns_experiments(self, experiment_dns_answers):
        if self.control['dns']['failure'] is not None and self.control['dns']['failure'] == self.report['dns_experiment_failure']:
            ...
            if len(control_addrs.intersection(experiment_addrs)) > 0:
                return True
            experiment_asns = set(map(lambda x: geoip.ip_to_location(x)['asn'], experiment_addrs))
            control_asns = set(map(lambda x: geoip.ip_to_location(x)['asn'], control_addrs))
    ...
    def determine_blocking(self, experiment_http_response, experiment_dns_answers):
        ...
        if experiment_http_failure is None and control_http_failure is None:
            got_expected_web_page = (self.report['body_length_match'] is True or self.report['headers_match'] is True or self.report['title_match'] is True) and self.report['status_code_match'] is not False
        if dns_consistent == True and tcp_connect == False and experiment_http_failure is not None:
            ...
            blocking = 'http-diff'
        elif dns_consistent == True and tcp_connect == True and experiment_http_failure is not None and control_http_failure is None:
            ...
            blocking = 'dns'
        elif dns_consistent == False and got_expected_web_page == False and (self.control['dns']['failure'] is not None or control_http_failure != experiment_http_failure):
            blocking = 'dns'
        return blocking
    ...
    def test_web_connectivity(self):
        ...
        if self.report['http_experiment_failure'] == None and self.report['dns_experiment_failure'] == None and self.report['blocking'] in (False, None):
