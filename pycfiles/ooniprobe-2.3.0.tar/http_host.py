...
# [GCC 8.4.0]
# Embedded file name: /Users/x/code/OONI/ooni-probe/docs/source/../../ooni/nettests/manipulation/http_host.py
...
class UsageOptions(usage.Options):
    optParameters = [
     [
      'backend', 'b', None,
      'URL of the test backend to use. Should be listening on port 80 and be a HTTPReturnJSONHeadersHelper (ex. http://1.1.1.1).'],
     [
      'content', 'c', None, 'The file to read from containing the content of a block page.']]
...
class HTTPHost(httpt.HTTPTest):
    ...
    def test_filtering_add_tab_to_host(self):
        ...
        headers = {}
        headers['Host'] = [self.input + '\t']
        response = yield self.doRequest(self.localOptions['backend'], headers=headers)
        self.check_for_censorship(response.body, test_name)
