...
# [GCC 8.4.0]
# Embedded file name: /Users/x/code/OONI/ooni-probe/docs/source/../../ooni/nettests/manipulation/captiveportal.py
...
class CaptivePortal(httpt.HTTPTest, dnst.DNSTest):
    ...
    @defer.inlineCallbacks
    def http_content_match_fuzzy_opt(self, experimental_url, control_result, headers=None, fuzzy=False):
        """
        ...
        if fuzzy:
            ...
            if not match:
                ...
                defer.returnValue(r)
        elif str(response_content) != str(control_result):
            ...
            r = (
             True, response_code, response_headers)
            defer.returnValue(r)
        return
    ...
    def check_0x20_to_auth_ns(self, hostname, sample_size=None):
        ...
        ret = {'result': name_match and serial_match, 
    ...
    def compare_random_hostnames(self, hostname_count=None, hostname_length=None):
        ...
        for x in range(hostname_count):
            ...
            response_match, response_address = yield self.dns_resolve_match(random_hostname, control[0])
        ...
        if len(intersection) == 1:
            log.msg('All %d random hostnames properly resolved to NXDOMAIN.' % hostname_count)
    ...
    def ms_dns_cp_test(self):
        ...
        log.msg('test...')
        msmatch, ms_dns_result = yield self.dns_resolve_match('dns.msftncsi.com', '131.107.255.255')
        ret = {'result': msmatch, 
    ...
    def run_vendor_tests(self, *a, **kw):
        ...
        @defer.inlineCallbacks
        def compare_content(status_func, fuzzy, experiment_url, control_result, control_code, headers, test_name):
            log.msg('')
            log.msg('Running the %s test...' % test_name)
            content_match, experiment_code, experiment_headers = yield cm(experiment_url, control_result, headers, fuzzy)
        ...
        for vt in vendor_tests:
            ...
            test_name = vt[4]
            args = (
             experiment_url, control_result, control_code, headers, test_name)
    ...
    def control(self, experiment_result, args):
        ...
        content_match, experiment_code, experiment_headers = yield cm(experiment_url, control_result)
        ...
        if status_match and content_match:
            ...
            log.msg("Retrying '%s' with fuzzy match enabled." % experiment_url)
            fuzzy_match, experiment_code, experiment_headers = yield cm(experiment_url, control_result, fuzzy=True)
            if fuzzy_match:
                self.report['result'] = True
            else:
