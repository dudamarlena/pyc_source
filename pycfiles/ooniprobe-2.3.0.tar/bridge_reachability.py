...
# [GCC 8.4.0]
# Embedded file name: /Users/x/code/OONI/ooni-probe/ooni/nettests/blocking/bridge_reachability.py
# Compiled at: 2017-02-06 12:45:02
...
class BridgeReachability(nettest.NetTestCase):
    ...
    description = 'This test examines the reachability of Tor bridges in your network.'
    author = 'Arturo Filastò'
    version = '0.1.2'
    usageOptions = UsageOptions
    inputFile = [
     'file', 'f', None,
     'File containing bridges to test reachability for. They should be one per line IP:ORPort or TransportType IP:ORPort (ex. obfs2 127.0.0.1:443).']
    requiredOptions = [
     'file']
    ...
    def postProcessor(self, measurements):
        ...
        details = {'address': self.report['bridge_address'], 'transport_name': self.report['transport_name'], 
    ...
    def test_full_tor_connection(self):
        ...
        config.ControlPort = random.randint(16384, 65536)
        config.SocksPort = random.randint(16384, 65536)
        ...
        if transport_name == None:
            ...
            try:
                config.ClientTransportPlugin = onion.bridge_line(transport_name, self.obfsproxy_logfile)
            except onion.UnrecognizedTransport:
                log.err("Unable to test bridge because we don't recognize the %s transport" % transport_name)
                ...
                log.err('Unable to test bridge because %s is not installed' % bin_name)
                self.report['error'] = 'missing-%s' % bin_name
                return
            except onion.OutdatedObfsproxy:
                log.err('The obfsproxy version you are using appears to be outdated.')
            ...
            log.debug("Using ClientTransportPlugin '%s'" % config.ClientTransportPlugin)
        ...
        d = txtorcon.launch_tor(config, reactor, timeout=self.timeout, progress_updates=updates)
        ...
        def write_log(_):
            ...
            try:
                try:
                    with open(os.path.join(self.tor_datadir, 'pt_state', 'obfs4proxy.log')) as (f):
                        self.report['obfsproxy_log'] = f.read()
                except:
                    pass
