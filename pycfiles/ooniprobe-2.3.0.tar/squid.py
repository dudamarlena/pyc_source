...
# [GCC 8.4.0]
# Embedded file name: /Users/x/code/OONI/ooni-probe/docs/source/../../ooni/nettests/experimental/squid.py
...
class SquidTest(httpt.HTTPTest):
    ...
    def test_cacheobject(self):
        ...
        log.msg('Testing Squid proxy presence by sending a request for cache_object')
        headers = {}
        self.report['trans_http_proxy'] = None
        method = 'GET'
        body = 'cache_object://localhost/info'
        return self.doRequest(self.localOptions['backend'], method=method, body=body, headers=headers, body_processor=process_body)
    ...
    def test_search_bad_request(self):
        ...
        def process_headers(headers):
            ...
            if 'X-Squid-Error' in headers:
                log.msg('Detected the presence of a transparent HTTP squid proxy')
                self.report['trans_http_proxy'] = True
            else:
                log.msg('Did not detect the presence of transparent HTTP squid proxy')
                self.report['transparent_http_proxy'] = False
        ...
        return self.doRequest(self.localOptions['backend'], method=method, headers=headers, headers_processor=process_headers)
    ...
    def test_squid_headers(self):
        ...
        def process_headers(headers):
            ...
            """
            squid_headers = {'via': '.* \\((squid.*)\\)', 'x-cache': 'MISS from (\\w+)', 
               'x-cache-lookup': 'MISS from (\\w+:?\\d+?)'}
            self.report['transparent_http_proxy'] = False
            for key in squid_headers.keys():
                if key in headers:
                    ...
                    if m:
                        log.msg('Detected the presence of squid transparent HTTP Proxy')
                        self.report['transparent_http_proxy'] = True
        ...
        method = 'GET'
        self.report['transparent_http_proxy'] = None
        d = self.doRequest(self.localOptions['backend'], method=method, headers=headers, headers_processor=process_headers)
        return d