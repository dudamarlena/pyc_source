...
# [GCC 8.4.0]
# Embedded file name: /Users/x/code/OONI/ooni-probe/ooni/nettests/blocking/dns_consistency.py
...
class DNSConsistencyTest(dnst.DNSTest):
    ...
    def setUp(self):
        if not self.localOptions['testresolvers'] and not self.localOptions['testresolver']:
            self.test_resolvers = []
    ...
    def test_a_lookup(self):
        ...
        try:
            ...
            if not control_answers:
                log.err('Got no response from control DNS server %s:%d, perhaps the DNS resolver is down?' % self.control_dns_server)
                self.report['errors']['%s:%d' % self.control_dns_server] = 'no_answer'
        except:
            self.report['errors']['%s:%d' % self.control_dns_server] = 'error'
