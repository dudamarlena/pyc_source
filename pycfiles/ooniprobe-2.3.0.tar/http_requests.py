...
# [GCC 8.4.0]
# Embedded file name: /Users/x/code/OONI/ooni-probe/docs/source/../../ooni/nettests/blocking/http_requests.py
...
class HTTPRequestsTest(httpt.HTTPTest):
    ...
    def requirements(self):
        if not self.localOptions['url'] and not self.localOptions['file']:
            raise MissingInput('You did not specify either a URL with -u or an input file with -f')
    ...
    def test_get_experiment(self):
        ...
        return self.doRequest(self.url, method='GET', use_tor=False, headers=self.headers)

    def test_get_control(self):
        log.msg('Performing GET request to %s over Tor' % self.url)
        return self.doRequest(self.url, method='GET', use_tor=True, headers=self.headers)
    ...
    def postProcessor(self, measurements):
        ...
        if experiment and control:
            if hasattr(experiment, 'body') and hasattr(control, 'body') and experiment.body and control.body:
                self.report['control_cloudflare'] = False
                if 'Attention Required! | CloudFlare' in control.body:
                    log.msg('The control body contains a blockpage from cloudflare. This will skew our results.')
                    self.report['control_cloudflare'] = True
                self.compare_body_lengths(len(control.body), len(experiment.body))
            if hasattr(experiment, 'headers') and hasattr(control, 'headers') and experiment.headers and control.headers:
