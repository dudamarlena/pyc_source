...
# [GCC 8.4.0]
# Embedded file name: /Users/x/code/OONI/ooni-probe/ooni/nettests/manipulation/dns_spoof.py
...
class DNSSpoof(scapyt.ScapyTest):
    ...
    def test_a_lookup(self):
        question = IP(dst=self.resolverAddr) / UDP() / DNS(rd=1, qd=DNSQR(qtype='A', qclass='IN', qname=self.hostname))
    ...
    def test_control_a_lookup(self):
        question = IP(dst=self.controlResolverAddr) / UDP() / DNS(rd=1, qd=DNSQR(qtype='A', qclass='IN', qname=self.hostname))
        log.msg('Performing query to %s with %s:%s' % (
