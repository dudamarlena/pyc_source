...
# [GCC 8.4.0]
# Embedded file name: /Users/x/code/OONI/ooni-probe/docs/source/../../ooni/nettests/manipulation/traceroute.py
...
class Traceroute(scapyt.BaseScapyTest):
    ...
    def run_traceroute(self, protocol):
        ...
        for ttl in xrange(st.ttl_min, st.ttl_max):
            matchedPackets = filter(lambda x: x.ttl == ttl, st.matched_packets.keys())
    ...
    def addToReport(self, packet, response):
        if packet.proto == 1:
            self.report['test_icmp_traceroute']['hops'].append({'ttl': packet.ttl, 'rtt': response.time - packet.time, 'address': response.src})
        elif packet.proto == 6:
            self.report['test_tcp_traceroute'][('hops_%s' % packet.dport)].append({'ttl': packet.ttl, 'rtt': response.time - packet.time, 'address': response.src, 
               'sport': response.sport})
        else:
            self.report['test_udp_traceroute'][('hops_%s' % packet.dport)].append({'ttl': packet.ttl, 'rtt': response.time - packet.time, 'address': response.src, 
               'sport': response.sport})