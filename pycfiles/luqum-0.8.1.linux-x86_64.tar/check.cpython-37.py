...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.7/dist-packages/luqum/check.py
# Compiled at: 2018-10-25 12:49:30
...
class LuceneCheck:
    ...
    def check_group(self, item, parents):
        if parents:
            if isinstance(parents[(-1)], tree.SearchField):
                yield 'Group misuse, after SearchField you should use Group :\xa0%s' % parents[(-1)]

    @_check_children
    def check_field_group(self, item, parents):
        if not (parents and isinstance(parents[(-1)], tree.SearchField)):
            yield 'FieldGroup misuse, it must be used after SearchField :\xa0%s' % (parents[(-1)] if parents else item)
    ...
    def check_word(self, item, parents):
        ...
        if self.zeal:
            if self.invalid_term_chars_re.search(item.value):
    ...
    def _check_not_operator(self, item, parents):
        ...
        if self.zeal:
            if isinstance(parents[(-1)], tree.OrOperation):
                yield "Prohibit or Not really means 'AND NOT' " + 'wich is inconsistent with OR operation in %s' % parents[(-1)]
    ...
    def _check_final_operation(self, node, context):
        ...
        if prefix:
            ...
            if fullname in self.nested_prefixes:
                raise NestedSearchFieldException('"{expr}" can\'t be directly attributed to "{field}" as it is a nested field'.format(expr=(str(node)),
                  field=fullname))
            else:
                if fullname in self.object_prefixes:
                    raise NestedSearchFieldException('"{expr}" can\'t be directly attributed to "{field}" as it is an object field'.format(expr=(str(node)),
                      field=fullname))
                else:
                    if len(prefix) > 1:
                        unknown_field = self.sub_fields is not None and self.object_fields is not None and fullname not in self.sub_fields and fullname not in self.object_fields and fullname not in self.nested_fields
                        if unknown_field:
                            raise ObjectSearchFieldException('"{expr}" attributed to unknown nested or object field "{field}"'.format(expr=(str(node)),
                              field=fullname))
