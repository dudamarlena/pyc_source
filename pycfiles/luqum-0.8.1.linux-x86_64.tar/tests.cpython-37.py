...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.7/dist-packages/luqum/tests.py
# Compiled at: 2016-04-12 04:48:27
...
class TestLexer(TestCase):
    ...
    def test_basic(self):
        lexer.input('subject:test desc:(house OR car)^3 AND "big garage"~2 dirt~0.3 OR foo:{a TO z*]')
...
class TestParser(TestCase):
    ...
    def test_simple_field(self):
        tree = SearchField('subject', Word('test'))
        parsed = parser.parse('subject:test')
        self.assertEqual(str(parsed), str(tree))
        self.assertEqual(parsed, tree)

    def test_minus(self):
        tree = AndOperation(AndOperation(Minus(Word('test')), Minus(Word('foo'))), Minus(Word('bar')))
        parsed = parser.parse('-test AND -foo AND NOT bar')
        self.assertEqual(str(parsed), str(tree))
        self.assertEqual(parsed, tree)

    def test_plus(self):
        tree = AndOperation(AndOperation(Plus(Word('test')), Word('foo')), Plus(Word('bar')))
        parsed = parser.parse('+test AND foo AND +bar')
        self.assertEqual(str(parsed), str(tree))
        self.assertEqual(parsed, tree)

    def test_phrase(self):
        tree = AndOperation(Phrase('"a phrase (AND a complicated~ one)"'), Phrase('"Another one"'))
        parsed = parser.parse('"a phrase (AND a complicated~ one)" AND "Another one"')
        self.assertEqual(str(parsed), str(tree))
        self.assertEqual(parsed, tree)

    def test_approx(self):
        tree = AndOperation(Proximity(Phrase('"foo bar"'), 3), AndOperation(Proximity(Phrase('"foo baz"'), 1), AndOperation(Fuzzy(Word('baz'), Decimal('0.3')), Fuzzy(Word('fou'), Decimal('0.5')))))
        parsed = parser.parse('"foo bar"~3 "foo baz"~ baz~0.3 fou~')
        self.assertEqual(str(parsed), str(tree))
        self.assertEqual(parsed, tree)

    def test_boost(self):
        tree = AndOperation(Boost(Phrase('"foo bar"'), Decimal('3.0')), Boost(Group(AndOperation(Word('baz'), Word('bar'))), Decimal('2.1')))
        parsed = parser.parse('"foo bar"^3 (baz AND bar)^2.1')
        self.assertEqual(str(parsed), str(tree))
        self.assertEqual(parsed, tree)

    def test_groups(self):
        tree = OrOperation(Word('test'), Group(AndOperation(SearchField('subject', FieldGroup(OrOperation(Word('foo'), Word('bar')))), Word('baz'))))
        parsed = parser.parse('test OR (subject:(foo OR bar) AND baz)')
        self.assertEqual(str(parsed), str(tree))
        self.assertEqual(parsed, tree)

    def test_range(self):
        tree = AndOperation(SearchField('foo', Range(Word('10'), Word('100'), True, True)), SearchField('bar', Range(Word('a*'), Word('*'), True, False)))
        parsed = parser.parse('foo:[10 TO 100] AND bar:[a* TO *}')
        self.assertEqual(str(parsed), str(tree))
        self.assertEqual(parsed, tree)

    def test_flavours(self):
        tree = SearchField('somedate', Range(Word('now/d-1d+7H'), Word('now/d+7H'), True, True))
        parsed = parser.parse('somedate:[now/d-1d+7H TO now/d+7H]')
        self.assertEqual(str(parsed), str(tree))
        self.assertEqual(parsed, tree)

    def test_combinations(self):
        tree = AndOperation(SearchField('subject', Word('test')), AndOperation(SearchField('desc', FieldGroup(OrOperation(Word('house'), Word('car')))), Minus(Proximity(Phrase('"approximatly this"'), 3))))
...
class TestPrettify(TestCase):
    big_tree = AndOperation(Group(OrOperation(Word('baaaaaaaaaar'), Word('baaaaaaaaaaaaaz'))), Word('fooooooooooo'))
    fat_tree = AndOperation(SearchField('subject', FieldGroup(OrOperation(Word('fiiiiiiiiiiz'), AndOperation(Word('baaaaaaaaaar'), Word('baaaaaaaaaaaaaz'))))), AndOperation(Word('fooooooooooo'), Word('wiiiiiiiiiz')))
...
class TestCheck(TestCase):
    ...
    def test_check_ok(self):
        query = AndOperation(SearchField('f', FieldGroup(AndOperation(Boost(Proximity(Phrase('"foo bar"'), 4), '4.2'), Minus(Range('100', '200'))))), Group(OrOperation(Fuzzy(Word('baz'), '.8'), Plus(Word('fizz')))))
        check = LuceneCheck()
    ...
    def test_zealous_or_not(self):
        query = OrOperation(Minus(Word('foo')), Word('bar'))
