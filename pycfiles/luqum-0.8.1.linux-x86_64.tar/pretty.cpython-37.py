...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.7/dist-packages/luqum/pretty.py
# Compiled at: 2017-04-10 05:24:00
...
class _StickMarker:
    __doc__ = 'Use in list between two elements that must stick together\n    '
...
class Prettifier(object):
    __doc__ = 'Class to generate a pretty printer.\n    '
    ...
    def _get_chains(self, element, parent=None):
        ...
        if isinstance(element, BaseOperation):
            ...
            if isinstance(element, BaseGroup):
                yield '('
                ...
                if isinstance(element, SearchField):
                    yield element.name + ':'
                    yield _STICK_MARKER
    ...
    def _count_chars(self, element):
        ...
        if isinstance(element, list):
            ...
            return (
             with_counts, sum((n + 1 for c, n in with_counts)) - 1)
    ...
    def _concatenates(self, chain_with_counts, char_counts, level=0, in_one_liner=False):
        ...
        elements = [self._concatenates(c, n, level=new_level, in_one_liner=one_liner) if isinstance(c, list) else c for c, n in chain_with_counts]
