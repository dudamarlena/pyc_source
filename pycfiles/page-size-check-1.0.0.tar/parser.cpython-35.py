...
# [GCC 8.4.0]
# Embedded file name: /home/carlos/projects/page-size-check/page_size_check/parser.py
# Compiled at: 2018-10-18 08:34:21
# Size of source mod 2**32: 6418 bytes
...
class HarFileParser:
    ...
    def parse_log_entries(self):
        ...
        for entry in self.log_entries:
            ...
            entry_info = {'url': entry['request']['url'], 
             'time': entry['time'], 
             'status': entry['response']['status'], 
             'body_size': entry['response']['bodySize'] / 1048576, 
             'headers_size': entry['response']['headersSize'] / 1048576}
            entry_info['total_size'] = entry_info['body_size'] + entry_info['headers_size']
            entries_resume[mime_type]['entries'].append(entry_info)
            entries_resume[mime_type]['total_size'] += entry_info['total_size']
            entries_resume[mime_type]['total_time'] += entry_info['time']
    ...
    def get_dom_content_loaded_time(driver):
        script = 'return window.performance.timing.domContentLoadedEventStart - window.performance.timing.navigationStart;'
    ...
    def mimetype_resources_to_csv(self, entries_resume, total_page_size):
        ...
        with open(file_path, 'a+', newline='') as (csv_file):
            field_names = [
             'page_url', 'mime_type', 'n_entries', 'total_size', 'average_size', 'percentage_size',
            ...
            for mime_type, entries in entries_resume.items():
                ...
                writer.writerow({'page_url': self.url, 
                 'mime_type': mime_type, 
    ...
    def resources_to_csv(self, entries_resume):
        ...
        with open(file_path, 'a+', newline='') as (csv_file):
            field_names = [
             'page_url', 'resource_url', 'mime_type', 'size', 'time']
            writer = csv.DictWriter(csv_file, fieldnames=field_names)
            if not file_exists:
                writer.writeheader()
            for mime_type, entries in entries_resume.items():
    ...
    def resume_to_csv(self, total_page_size, dom_content_loaded):
        ...
        with open(file_path, 'a+', newline='') as (csv_file):
            field_names = [
             'page_url', 'num_entries', 'total_size', 'finish_time', 'domcontent_loaded', 'load_time']
            writer = csv.DictWriter(csv_file, fieldnames=field_names)
            if not file_exists:
                writer.writeheader()
            writer.writerow({'page_url': self.url, 
