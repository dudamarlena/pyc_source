...
# [GCC 8.4.0]
# Embedded file name: /private/var/folders/q3/1b9f00755fngs2554s60x4_h0000gn/T/pycharm-packaging/web3/web3/utils/events.py
# Compiled at: 2018-05-28 04:44:24
# Size of source mod 2**32: 6841 bytes
import itertools
from eth_abi import decode_abi, decode_single, encode_single
from eth_abi.abi import process_type
from eth_utils import encode_hex, event_abi_to_log_topic, is_list_like, to_tuple
...
from .abi import exclude_indexed_event_inputs, get_abi_input_names, get_indexed_event_inputs, map_abi_data, normalize_event_input_types
...
def construct_event_topic_set(event_abi, arguments=None):
    ...
    encoded_args = [[None if option is None else encode_hex(encode_single(arg['type'], option)) for option in arg_options] for arg, arg_options in zipped_abi_and_args]
    topics = [[event_topic] + list(permutation) if any(value is not None for value in permutation) else [event_topic] for permutation in (itertools.product)(*encoded_args)]
    return topics
...
def construct_event_data_set(event_abi, arguments=None):
    ...
    zipped_abi_and_args = [(arg, normalized_args.get(arg['name'], [None])) for arg in non_indexed_args]
    encoded_args = [[None if option is None else encode_hex(encode_single(arg['type'], option)) for option in arg_options] for arg, arg_options in zipped_abi_and_args]
    data = [list(permutation) if any(value is not None for value in permutation) else [] for permutation in (itertools.product)(*encoded_args)]
    return data
...
def get_event_data(event_abi, log_entry):
    ...
    if event_abi['anonymous']:
        ...
        if not log_entry['topics']:
            ...
            if event_abi_to_log_topic(event_abi) != log_entry['topics'][0]:
        ...
        if len(log_topics) != len(log_topic_types):
            raise ValueError('Expected {0} log topics.  Got {1}'.format(len(log_topic_types), len(log_topics)))
        ...
        if duplicate_names:
            raise ValueError("Invalid Event ABI:  The following argument names are duplicated between event inputs: '{0}'".format(', '.join(duplicate_names)))
