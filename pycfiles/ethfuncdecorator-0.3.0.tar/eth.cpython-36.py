...
# [GCC 8.4.0]
# Embedded file name: /private/var/folders/q3/1b9f00755fngs2554s60x4_h0000gn/T/pycharm-packaging/web3/web3/eth.py
# Compiled at: 2018-05-28 04:44:24
# Size of source mod 2**32: 11856 bytes
from eth_account import Account
from eth_utils import apply_to_return_value, is_checksum_address, is_string
from hexbytes import HexBytes
from web3.contract import Contract
from web3.iban import Iban
from web3.module import Module
from web3.utils.blocks import select_method_for_block_identifier
from web3.utils.decorators import deprecated_for
from web3.utils.empty import empty
from web3.utils.encoding import to_hex
from web3.utils.filters import BlockFilter, LogFilter, TransactionFilter
from web3.utils.toolz import assoc, merge
from web3.utils.transactions import assert_valid_transaction_params, extract_valid_transaction_params, get_buffered_gas_estimate, get_required_transaction, replace_transaction, wait_for_transaction_receipt
...
class Eth(Module):
    ...
    def getBalance(self, account, block_identifier=None):
        ...
        return self.web3.manager.request_blocking('eth_getBalance', [
         account, block_identifier])

    def getStorageAt(self, account, position, block_identifier=None):
        if block_identifier is None:
            block_identifier = self.defaultBlock
        return self.web3.manager.request_blocking('eth_getStorageAt', [
         account, position, block_identifier])

    def getCode(self, account, block_identifier=None):
        if block_identifier is None:
            block_identifier = self.defaultBlock
        return self.web3.manager.request_blocking('eth_getCode', [
         account, block_identifier])
    ...
    def getBlock(self, block_identifier, full_transactions=False):
        ...
        """
        method = select_method_for_block_identifier(block_identifier,
          if_predefined='eth_getBlockByNumber',
          if_hash='eth_getBlockByHash',
          if_number='eth_getBlockByNumber')
        return self.web3.manager.request_blocking(method, [
    ...
    def getTransaction(self, transaction_hash):
        return self.web3.manager.request_blocking('eth_getTransactionByHash', [
         transaction_hash])
    ...
    def getTransactionReceipt(self, transaction_hash):
        return self.web3.manager.request_blocking('eth_getTransactionReceipt', [
         transaction_hash])

    def getTransactionCount(self, account, block_identifier=None):
        if block_identifier is None:
            block_identifier = self.defaultBlock
        return self.web3.manager.request_blocking('eth_getTransactionCount', [
         account,
         block_identifier])
    ...
    def sendTransaction(self, transaction):
        if 'from' not in transaction:
            if is_checksum_address(self.defaultAccount):
                transaction = assoc(transaction, 'from', self.defaultAccount)
        if 'gas' not in transaction:
            transaction = assoc(transaction, 'gas', get_buffered_gas_estimate(self.web3, transaction))
        return self.web3.manager.request_blocking('eth_sendTransaction', [
         transaction])

    def sendRawTransaction(self, raw_transaction):
        return self.web3.manager.request_blocking('eth_sendRawTransaction', [
         raw_transaction])

    def sign(self, account, data=None, hexstr=None, text=None):
        message_hex = to_hex(data, hexstr=hexstr, text=text)
        return self.web3.manager.request_blocking('eth_sign', [account, message_hex])
    ...
    def call(self, transaction, block_identifier=None):
        ...
        if block_identifier is None:
            block_identifier = self.defaultBlock
        return self.web3.manager.request_blocking('eth_call', [
    ...
    def estimateGas(self, transaction):
        if 'from' not in transaction:
            if is_checksum_address(self.defaultAccount):
                transaction = assoc(transaction, 'from', self.defaultAccount)
        return self.web3.manager.request_blocking('eth_estimateGas', [
         transaction])

    def filter(self, filter_params=None, filter_id=None):
        if filter_id:
            if filter_params:
                raise TypeError('Ambiguous invocation: provide either a `filter_params` or a `filter_id` argument. Both were supplied.')
            if is_string(filter_params):
                if filter_params == 'latest':
                    filter_id = self.web3.manager.request_blocking('eth_newBlockFilter', [])
                    return BlockFilter(self.web3, filter_id)
                if filter_params == 'pending':
                    filter_id = self.web3.manager.request_blocking('eth_newPendingTransactionFilter', [])
                    return TransactionFilter(self.web3, filter_id)
                raise ValueError('The filter API only accepts the values of `pending` or `latest` for string based filters')
            elif isinstance(filter_params, dict):
                _filter_id = self.web3.manager.request_blocking('eth_newFilter', [
                 filter_params])
            ...
            raise TypeError('Must provide either filter_params as a string or a valid filter object, or a filter_id as a string or hex.')

    def getFilterChanges(self, filter_id):
        return self.web3.manager.request_blocking('eth_getFilterChanges', [filter_id])

    def getFilterLogs(self, filter_id):
        return self.web3.manager.request_blocking('eth_getFilterLogs', [filter_id])

    def getLogs(self, filter_params):
        return self.web3.manager.request_blocking('eth_getLogs', [filter_params])

    def uninstallFilter(self, filter_id):
        return self.web3.manager.request_blocking('eth_uninstallFilter', [filter_id])
