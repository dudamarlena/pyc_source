...
# [GCC 8.4.0]
# Embedded file name: /private/var/folders/q3/1b9f00755fngs2554s60x4_h0000gn/T/pycharm-packaging/web3/web3/utils/filters.py
# Compiled at: 2018-05-28 04:44:24
# Size of source mod 2**32: 5643 bytes
import re
from eth_utils import is_list_like, is_string
from web3.utils.threads import TimerClass
from web3.utils.validation import validate_address
from .events import construct_event_data_set, construct_event_topic_set

def construct_event_filter_params(event_abi, contract_address=None, argument_filters=None, topics=None, fromBlock=None, toBlock=None, address=None):
    filter_params = {}
    if topics is None:
        topic_set = construct_event_topic_set(event_abi, argument_filters)
    else:
        topic_set = [
         topics] + construct_event_topic_set(event_abi, argument_filters)
    if len(topic_set) == 1:
        if is_list_like(topic_set[0]):
        ...
        if contract_address:
            if is_list_like(address):
                ...
                if is_string(address):
                    ...
                    raise ValueError('Unsupported type for `address` parameter: {0}'.format(type(address)))
    if address:
    ...
    if 'address' not in filter_params:
        ...
        if is_list_like(filter_params['address']):
...
class Filter:
    ...
    def _format_log_entries(self, log_entries=None):
        if log_entries is None:
            ...
            formatted_log_entries = [self.format_entry(log_entry) for log_entry in log_entries]
...
def construct_data_filter_regex(data_filter_set):
    return re.compile('^' + '|'.join('0x' + ''.join((ZERO_32BYTES if v is None else v[2:]) for v in data_filter) for data_filter in data_filter_set) + '$')
...
class ShhFilter(Filter):
    ...
    def __init__(self, *args, **kwargs):
        self.poll_interval = kwargs.pop('poll_interval', self.poll_interval)
        (super().__init__)(*args, **kwargs)
