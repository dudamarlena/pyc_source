...
# [GCC 8.4.0]
# Embedded file name: /private/var/folders/q3/1b9f00755fngs2554s60x4_h0000gn/T/pycharm-packaging/web3/web3/contract.py
# Compiled at: 2018-05-28 04:44:24
# Size of source mod 2**32: 46876 bytes
...
from eth_utils import add_0x_prefix, encode_hex, function_abi_to_4byte_selector, is_text, to_tuple
from web3.exceptions import BadFunctionCallOutput, BlockNumberOutofRange, FallbackNotFound, MismatchedABI, NoABIEventsFound, NoABIFunctionsFound
from web3.utils.abi import fallback_func_abi_exists, filter_by_type, get_abi_output_types, get_constructor_abi, map_abi_data, merge_args_and_kwargs
from web3.utils.blocks import is_hex_encoded_block_hash
from web3.utils.contracts import encode_abi, find_matching_event_abi, find_matching_fn_abi, get_function_info, prepare_transaction
from web3.utils.datatypes import PropertyCheckingFactory
from web3.utils.decorators import combomethod, deprecated_for
...
from web3.utils.normalizers import BASE_RETURN_NORMALIZERS, normalize_abi, normalize_address, normalize_bytecode
from web3.utils.toolz import compose, partial
from web3.utils.transactions import fill_transaction_defaults
DEPRECATED_SIGNATURE_MESSAGE = "The constructor signature for the `Contract` object has changed. Please update your code to reflect the updated function signature: 'Contract(address)'.  To construct contract classes use the 'Contract.factory(...)' class method."
ACCEPTABLE_EMPTY_STRINGS = [
 '0x', b'0x', '', b'']

class ContractFunctions:
    __doc__ = 'Class containing contract function objects\n    '
    _function_names = []
    ...
    def __init__(self, abi, web3, address=None):
        if abi:
            ...
            for func in self._functions:
                ...
                setattr(self, func['name'], ContractFunction.factory((func['name']),
                  web3=web3,
    ...
    def __getattr__(self, function_name):
        if '_functions' not in self.__dict__:
            raise NoABIFunctionsFound('The abi for this contract contains no function definitions. ', 'Are you sure you provided the correct contract abi?')
        else:
            if function_name not in self.__dict__['_functions']:
                raise MismatchedABI("The function '{}' was not found in this contract's abi. ".format(function_name), 'Are you sure you provided the correct contract abi?')
            else:
...
class ContractEvents:
    __doc__ = 'Class containing contract event objects\n    '
    _event_names = []
    ...
    def __init__(self, abi, web3, address=None):
        if abi:
            ...
            for event in self._events:
                ...
                setattr(self, event['name'], ContractEvent.factory((event['name']),
                  web3=web3,
                  contract_abi=(self.abi),
                  address=address,
    ...
    def __getattr__(self, event_name):
        if '_events' not in self.__dict__:
            raise NoABIEventsFound('The abi for this contract contains no event definitions. ', 'Are you sure you provided the correct contract abi?')
        else:
            if event_name not in self.__dict__['_events']:
                raise MismatchedABI("The event '{}' was not found in this contract's abi. ".format(event_name), 'Are you sure you provided the correct contract abi?')
            else:
...
class Contract:
    ...
    def __init__(self, address=None):
        ...
        if self.web3 is None:
            ...
            raise self.address or TypeError('The address argument is required to instantiate a contract.')
    ...
    def factory(cls, web3, class_name=None, **kwargs):
        kwargs['web3'] = web3
        normalizers = {'abi':normalize_abi, 
         'address':partial(normalize_address, kwargs['web3'].ens), 
         'bytecode':normalize_bytecode, 
         'bytecode_runtime':normalize_bytecode}
        contract = PropertyCheckingFactory((class_name or cls.__name__),
          (
         cls,),
          kwargs,
    ...
    def deploy(cls, transaction=None, args=None, kwargs=None):
        ...
        if not cls.bytecode:
            raise ValueError("Cannot deploy a contract that does not have 'bytecode' associated with it")
        if 'data' in deploy_transaction:
            raise ValueError('Cannot specify `data` for contract deployment')
        if 'to' in deploy_transaction:
            raise ValueError('Cannot specify `to` for contract deployment')
 ...
 cls.bytecode, *args, **kwargs)
    ...
    def encodeABI(cls, fn_name, args=None, kwargs=None, data=None):
        ...
        """
        fn_abi, fn_selector, fn_arguments = get_function_info(cls.abi, fn_name, args, kwargs)
        if data is None:
            data = fn_selector
 ...
 event_abi, contract_address=self.address, 
         argument_filters=argument_filters, **filter_meta_params)
    ...
    def estimateGas(self, transaction=None):
        ...
        class Caller:
            ...
            def __getattr__(self, function_name):
                callable_fn = functools.partial(estimate_gas_for_function, contract.abi, contract.address, contract.web3, function_name, estimate_transaction)
                return callable_fn
    ...
    def call(self, transaction=None):
        ...
        if 'to' not in call_transaction:
            if isinstance(self, type):
                raise ValueError('When using `Contract.call` from a contract factory you must provide a `to` address with the transaction')
            else:
                raise ValueError('Please ensure that this contract instance has an address.')
        contract = self
        ...
        class Caller:
            ...
            def __getattr__(self, function_name):
                callable_fn = functools.partial(call_contract_function, contract.abi, contract.web3, contract.address, contract._return_data_normalizers, function_name, call_transaction, 'latest')
                return callable_fn
    ...
    def transact(self, transaction=None):
        ...
        class Transactor:
            ...
            def __getattr__(self, function_name):
                callable_fn = functools.partial(transact_with_contract_function, contract.abi, contract.address, contract.web3, function_name, transact_transaction)
                return callable_fn
    ...
    def buildTransaction(self, transaction=None):
        ...
        if isinstance(self, type):
            if 'to' not in built_transaction:
                raise ValueError('When using `Contract.buildTransaction` from a contract factory you must provide a `to` address with the transaction')
        ...
        class Caller:
            ...
            def __getattr__(self, function_name):
                callable_fn = functools.partial(build_transaction_for_function, contract.abi, contract.address, contract.web3, function_name, built_transaction)
                return callable_fn
    ...
    @classmethod
    def _prepare_transaction(cls, fn_name, fn_args=None, fn_kwargs=None, transaction=None):
        return prepare_transaction((cls.abi), (cls.address),
          (cls.web3),
    ...
    def _find_matching_fn_abi(cls, fn_identifier=None, args=None, kwargs=None):
        return find_matching_fn_abi((cls.abi), fn_identifier=fn_identifier,
          args=args,
          kwargs=kwargs)
    ...
    def get_fallback_function(abi, web3, address=None):
        if abi:
            if fallback_func_abi_exists(abi):
                return ContractFunction.factory('fallback',
                  web3=web3,
                  contract_abi=abi,
                  address=address,
    ...
    def _encode_constructor_data(cls, args=None, kwargs=None):
        ...
        if constructor_abi:
            ...
            deploy_data = add_0x_prefix(encode_abi((cls.web3), constructor_abi, arguments, data=(cls.bytecode)))
        else:
    ...
    def _encode_data_in_transaction(self, *args, **kwargs):
        ...
        if constructor_abi:
            ...
            if not kwargs:
                kwargs = {}
            arguments = merge_args_and_kwargs(constructor_abi, args, kwargs)
            data = add_0x_prefix(encode_abi((self.web3), constructor_abi, arguments, data=(self.bytecode)))
        else:
...
class ConciseMethod:
    ALLOWED_MODIFIERS = {
     'call', 'estimateGas', 'transact', 'buildTransaction'}
    ...
    def __call__(self, *args, **kwargs):
        return (self._ConciseMethod__prepared_function)(*args, **kwargs)
        ...
        if not kwargs:
            ...
            if len(kwargs) == 1:
                ...
                if modifier not in self.ALLOWED_MODIFIERS:
                    raise TypeError('The only allowed keyword arguments are: %s' % self.ALLOWED_MODIFIERS)
            else:
    ...
    def __init__(self, classic_contract, method_class=ConciseMethod):
        ...
        for fn_name in self._classic_contract.functions:
            if fn_name in protected_fn_names:
                ...
                _concise_method = method_class(_classic_method, self._classic_contract._return_data_normalizers)
...
class ImplicitMethod(ConciseMethod):
    ...
    def __call_by_default(self, args):
        function_abi = find_matching_fn_abi((self._function.contract_abi), fn_identifier=(self._function.function_identifier),
          args=args)
        if 'constant' in function_abi.keys():
            return function_abi['constant']
        else:
            return False

    def __call__(self, *args, **kwargs):
        if not kwargs:
            if not self._ImplicitMethod__call_by_default(args):
...
class ContractFunction:
    ...
    def _set_function_info(self):
        self.abi = find_matching_fn_abi(self.contract_abi, self.function_identifier, self.args, self.kwargs)
        if self.function_identifier is FallbackFn:
            self.selector = encode_hex(b'')
        else:
            if is_text(self.function_identifier):
 ...
 call_transaction,
 block_id, *(self.args), **self.kwargs)
 ...
 self.web3,
 self.function_identifier,
 transact_transaction, *(self.args), **self.kwargs)
 ...
 self.web3,
 self.function_identifier,
 estimate_gas_transaction, *(self.args), **self.kwargs)
    ...
    def buildTransaction(self, transaction=None):
        ...
        if not self.address:
            if 'to' not in built_transaction:
                raise ValueError('When using `ContractFunction.buildTransaction` from a Contract factoryyou must provide a `to` address with the transaction')
        if self.address:
            if 'to' in built_transaction:
 ...
 self.web3,
 self.function_identifier,
 built_transaction, *(self.args), **self.kwargs)
...
class ContractEvent:
    ...
    @combomethod
    def createFilter(self, *, argument_filters=None, fromBlock=None, toBlock='latest', address=None, topics=None):
        """
        ...
        data_filter_set, event_filter_params = construct_event_filter_params((self._get_event_abi()),
...
def call_contract_function(abi, web3, address, normalizers, function_identifier, transaction, block_id=None, *args, **kwargs):
    """
    Helper function for interacting with a contract function using the
    `eth_call` API.
    """
    call_transaction = prepare_transaction(abi,
      address,
      web3,
    ...
    try:
        ...
        is_missing_code_error = return_data in ACCEPTABLE_EMPTY_STRINGS and web3.eth.getCode(address) in ACCEPTABLE_EMPTY_STRINGS
        if is_missing_code_error:
            msg = 'Could not transact with/call contract function, is contract deployed correctly and chain synced?'
        else:
            msg = 'Could not decode contract function call {} return data {} for output_types {}'.format(function_identifier, return_data, output_types)
...
def parse_block_identifier(web3, block_identifier):
    ...
    if isinstance(block_identifier, int):
        ...
        if block_identifier in ('latest', 'earliest', 'pending'):
            return block_identifier
    else:
        if isinstance(block_identifier, bytes) or is_hex_encoded_block_hash(block_identifier):
            return web3.eth.getBlock(block_identifier).number
        raise BlockNumberOutofRange


def transact_with_contract_function(abi, address, web3, function_name=None, transaction=None, *args, **kwargs):
    """
    Helper function for interacting with a contract function by sending a
    transaction.
    """
    transact_transaction = prepare_transaction(abi,
      address,
      web3,
...
def estimate_gas_for_function(abi, address, web3, fn_identifier=None, transaction=None, *args, **kwargs):
    """Estimates gas cost a function call would take.

    Don't call this directly, instead use :meth:`Contract.estimateGas`
    on your contract instance.
    """
    estimate_transaction = prepare_transaction(abi,
      address,
      web3,
...
def build_transaction_for_function(abi, address, web3, function_name=None, transaction=None, *args, **kwargs):
    """Builds a dictionary with the fields required to make the given transaction

    Don't call this directly, instead use :meth:`Contract.buildTransaction`
    on your contract instance.
    """
    prepared_transaction = prepare_transaction(abi,
      address,
      web3,
      fn_identifier=function_name,
      fn_args=args,
