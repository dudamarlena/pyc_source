...
# [GCC 8.4.0]
# Embedded file name: /private/var/folders/q3/1b9f00755fngs2554s60x4_h0000gn/T/pycharm-packaging/web3/web3/providers/eth_tester/defaults.py
# Compiled at: 2018-05-28 04:44:24
# Size of source mod 2**32: 10979 bytes
...
from web3.utils.toolz import compose, curry, excepts
...
def client_version(eth_tester, params):
    ...
    return 'EthereumTester/{version}/{platform}/python{v.major}.{v.minor}.{v.micro}'.format(version=__version__,
      v=(sys.version_info),
...
def null_if_excepts(exc_type, fn):
    return excepts(exc_type, fn, static_return(None))
...
API_ENDPOINTS = {'web3':{'clientVersion':client_version, 
  'sha3':compose(encode_hex, keccak, decode_hex, without_eth_tester(operator.itemgetter(0)))}, 
 'net':{'version':not_implemented, 
  'peerCount':not_implemented, 
  'listening':not_implemented}, 
 'eth':{'protocolVersion':not_implemented, 
  'syncing':not_implemented, 
  'coinbase':compose(operator.itemgetter(0), call_eth_tester('get_accounts')), 
  'mining':not_implemented, 
  'hashrate':not_implemented, 
  'gasPrice':not_implemented, 
  'accounts':call_eth_tester('get_accounts'), 
  'blockNumber':compose(operator.itemgetter('number'), call_eth_tester('get_block_by_number', fn_kwargs={'block_number': 'latest'})), 
  'getBalance':call_eth_tester('get_balance'), 
  'getStorageAt':not_implemented, 
  'getTransactionCount':call_eth_tester('get_nonce'), 
  'getBlockTransactionCountByHash':null_if_block_not_found(compose(len, operator.itemgetter('transactions'), call_eth_tester('get_block_by_hash'))), 
  'getBlockTransactionCountByNumber':null_if_block_not_found(compose(len, operator.itemgetter('transactions'), call_eth_tester('get_block_by_number'))), 
  'getUncleCountByBlockHash':null_if_block_not_found(compose(len, operator.itemgetter('uncles'), call_eth_tester('get_block_by_hash'))), 
  'getUncleCountByBlockNumber':null_if_block_not_found(compose(len, operator.itemgetter('uncles'), call_eth_tester('get_block_by_number'))), 
  ...
  'call':call_eth_tester('call'), 
  'estimateGas':call_eth_tester('estimate_gas'), 
  ...
  'getTransactionByHash':null_if_transaction_not_found(call_eth_tester('get_transaction_by_hash')), 
  ...
  'getTransactionReceipt':null_if_transaction_not_found(compose(apply_formatter_if(compose(is_null, operator.itemgetter('block_number')), static_return(None)), call_eth_tester('get_transaction_receipt'))), 
  ...
  'uninstallFilter':excepts(FilterNotFound, compose(is_null, call_eth_tester('delete_filter')), static_return(False)), 
 ...
 'debug':{'backtraceAt':not_implemented, 
 ...
 'personal':{'ecRecover':not_implemented, 
  'importRawKey':call_eth_tester('add_account'), 
  'listAccounts':call_eth_tester('get_accounts'), 
  'lockAccount':excepts(ValidationError, compose(static_return(True), call_eth_tester('lock_account')), static_return(False)), 
  'newAccount':create_new_account, 
  'unlockAccount':excepts(ValidationError, compose(static_return(True), call_eth_tester('unlock_account')), static_return(False)), 
  'sendTransaction':personal_send_transaction, 
  'sign':not_implemented}, 
 'testing':{'timeTravel': call_eth_tester('time_travel')}, 
 'txpool':{'content':not_implemented, 
  'inspect':not_implemented, 
  'status':not_implemented}, 
 'evm':{'mine':call_eth_tester('mine_blocks'), 
