...
# [GCC 8.4.0]
# Embedded file name: /private/var/folders/q3/1b9f00755fngs2554s60x4_h0000gn/T/pycharm-packaging/web3/web3/utils/transactions.py
...
def get_buffered_gas_estimate(web3, transaction, gas_buffer=100000):
    ...
    if gas_estimate > gas_limit:
        raise ValueError('Contract does not appear to be deployable within the current network gas limits.  Estimated: {0}. Current gas limit: {1}'.format(gas_estimate, gas_limit))
...
def get_required_transaction(web3, transaction_hash):
    ...
    if not current_transaction:
        raise ValueError('Supplied transaction with hash {} does not exist'.format(transaction_hash))
    return current_transaction


def extract_valid_transaction_params(transaction_params):
    extracted_params = {key:transaction_params[key] for key in VALID_TRANSACTION_PARAMS if key in transaction_params}
    if extracted_params.get('data') is not None:
        ...
        if extracted_params.get('data') is None:
            if transaction_params.get('input') is not None:
...
def prepare_replacement_transaction(web3, current_transaction, new_transaction):
    if current_transaction['blockHash'] is not None:
        raise ValueError('Supplied transaction with hash {} has already been mined'.format(current_transaction['hash']))
    elif 'nonce' in new_transaction:
        if new_transaction['nonce'] != current_transaction['nonce']:
            raise ValueError('Supplied nonce in new_transaction must match the pending transaction')
    elif 'nonce' not in new_transaction:
        ...
        if 'gasPrice' in new_transaction:
            ...
            if generated_gas_price:
                if generated_gas_price > minimum_gas_price:
