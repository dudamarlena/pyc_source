...
# [GCC 8.4.0]
# Embedded file name: /private/var/folders/q3/1b9f00755fngs2554s60x4_h0000gn/T/pycharm-packaging/web3/web3/middleware/cache.py
# Compiled at: 2018-05-28 04:44:24
# Size of source mod 2**32: 13590 bytes
import functools, threading, time, lru
...
def construct_simple_cache_middleware(cache_class, rpc_whitelist=SIMPLE_CACHE_RPC_WHITELIST, should_cache_fn=_should_cache):
    """
...
_simple_cache_middleware = construct_simple_cache_middleware(cache_class=(functools.partial(lru.LRU, 256)))
TIME_BASED_CACHE_RPC_WHITELIST = {
 'eth_coinbase',
 'eth_accounts'}

def construct_time_based_cache_middleware(cache_class, cache_expire_seconds=15, rpc_whitelist=TIME_BASED_CACHE_RPC_WHITELIST, should_cache_fn=_should_cache):
    """
    Constructs a middleware which caches responses based on the request
...
_time_based_cache_middleware = construct_time_based_cache_middleware(cache_class=(functools.partial(lru.LRU, 256)))
...
def construct_latest_block_based_cache_middleware(cache_class, rpc_whitelist=BLOCK_NUMBER_RPC_WHITELIST, average_block_time_sample_size=240, default_average_block_time=15, should_cache_fn=_should_cache):
    """
    Constructs a middleware which caches responses based on the request
    ...
    def latest_block_based_cache_middleware(make_request, web3):
        ...
        def _update_block_info_cache():
            ...
            if avg_block_time == 0:
                ...
                avg_block_time_age_in_blocks = (time.time() - avg_block_time_updated_at) / avg_block_time
            if avg_block_time_age_in_blocks >= avg_block_sample_size:
                latest_block = web3.eth.getBlock('latest')
                ancestor_block_number = max(0, latest_block['number'] - average_block_time_sample_size)
                ...
                if sample_size != 0:
                    block_info[AVG_BLOCK_TIME_KEY] = (latest_block['timestamp'] - ancestor_block['timestamp']) / sample_size
                else:
        ...
        def middleware(method, params):
            lock_acquired = lock.acquire(blocking=False)
            try:
                should_try_cache = lock_acquired and method in rpc_whitelist and not _is_latest_block_number_request(method, params)
                if should_try_cache:
...
_latest_block_based_cache_middleware = construct_latest_block_based_cache_middleware(cache_class=(functools.partial(lru.LRU, 256)),
