...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/autoforms/admin.py
...
class FormAdmin(admin.ModelAdmin):
    ...
    def embed(self, request, id):
        ...
        return render_to_response('autoforms/admin/form_embed.html', {'code': code, 'form': form, 'is_popup': True, 'title': _('Embed code')}, context_instance=RequestContext(request))
    ...
    def get_urls(self):
        ...
        form_urls = patterns('', (
         '^(?P<id>\\d+)/preview/$', self.admin_site.admin_view(self.preview)), (
         '^(?P<id>\\d+)/data/$', self.admin_site.admin_view(self.data)), (
         '^(?P<id>\\d+)/embed/$', self.admin_site.admin_view(self.embed)), (
         '^(?P<id>\\d+)/data/export/(?P<format>\\w+)/$', self.admin_site.admin_view(self.export)))
...
class FieldAdmin(admin.ModelAdmin):
    list_display = [
     'label', 'form', 'name', 'type', 'required', 'order']
