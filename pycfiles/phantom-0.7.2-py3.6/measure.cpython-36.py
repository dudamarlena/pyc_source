...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\phantom\measures\measure.py
# Compiled at: 2019-12-18 07:24:29
# Size of source mod 2**32: 20926 bytes
"""
...
def define_plane(x1, x2, y1, y2, z1, z2, height, obj=None):
    ...
    return (
     [
      v_points[0], v_points[1], v_points[2], p4], alfa)
...
def detect_plane(img, obj, height):
    ...
    for second_line in lines[1:]:
        p21 = (
         second_line[0][0], second_line[0][1], 1)
    ...
    y_points = []
    for point in points:
        if point[0] > median[0] - std[0] and point[0] < median[0] + std[0] and point[1] > median[1] - std[1] and point[1] < median[1] + std[1]:
    ...
    y_points = []
    for point in points:
        if point[0] > median[0] - std[0] * 1.5 and point[0] < median[0] + std[0] * 1.5 and point[1] > median[1] - std[1] * 1.5 and point[1] < median[1] + std[1] * 1.5:
    ...
    z_points = []
    for point in points:
        if point[1] * median[1] > 0 and point[0] > median[0] - std[0] * 1.5 and point[0] < median[0] + std[0] * 1.5 and point[1] > median[1] - std[1] * 1.5 and point[1] < median[1] + std[1] * 1.5:
            z_points.append(point)

    vanishing_z = np.mean(z_points, axis=0)
    p1 = (
     vanishing_x[0], vanishing_x[1], 1)
    ...
    return (
     [
      np.array([vanishing_x[0], vanishing_x[1], 1]), np.array([vanishing_y[0], vanishing_y[1], 1]), np.array([vanishing_z[0], vanishing_z[1], 1]), p4], alfa)