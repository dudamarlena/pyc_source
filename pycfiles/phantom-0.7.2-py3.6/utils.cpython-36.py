...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\phantom\utils.py
# Compiled at: 2019-10-11 19:43:15
# Size of source mod 2**32: 5357 bytes
"""
...
def draw_delauney(img, subdiv, *, color=(255, 0, 0), thick=1):
    ...
    for t in tri_list:
        ...
        cv2.line(ret, p1, p2, color, thick)
        cv2.line(ret, p2, p3, color, thick)
        cv2.line(ret, p3, p1, color, thick)
...
def image_grid(images, grid, *, borders=False, colors=None, text_index=False, size=(64, 64)):
    """
    ...
    if colors is None:
        colors = [
         (255, 0, 0)] * len(images)
    size_w, size_h = size
    ret = np.zeros((grid_h * size_h, grid_w * size_w, 3))
