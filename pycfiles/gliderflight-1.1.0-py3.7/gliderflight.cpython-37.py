...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gliderflight/gliderflight.py
# Compiled at: 2020-02-17 09:08:47
# Size of source mod 2**32: 39081 bytes
...
class ModelParameters(object):
    ...
    def __init__(self, parameterised_parameters_dict):
        ...
        self._ModelParameters__aoa_parameter_changed = True
        self.define(AR=7, Omega=(43 * np.pi / 180.0), S=0.1, ah=3.8, eOsborne=0.8, Cd1_hull=9.7, epsilon=5e-10)
    ...
    def define(self, **kw):
        ...
        for k, v in kw.items():
            ...
            elif k in self._parameterised_parameters:
    ...
    def has_aoa_parameter_changed(self):
        ...
        rv : bool
            test result
        """
        rv = self._ModelParameters__aoa_parameter_changed
        return rv
    ...
    def undefined_parameters(self):
        ...
        """
        return [i for i in self.parameters if self.__dict__[i] == None if i not in self._parameterised_parameters]
...
class GliderModel(object):
    ...
    def convert_pressure_Vbp_to_SI(self, m_water_pressure, m_de_oil_vol):
        ...
        """
        pressure = m_water_pressure * 100000.0
        Vbp = m_de_oil_vol * 1e-06
        return (pressure, Vbp)
    ...
    def compute_FB_and_Fg(self, pressure, rho, Vbp, mg=None, Vg=None):
        ...
        if Vg is None:
            Vg = self.Vg
        g = self.G
        FB = g * rho * (Vg * (1.0 - self.epsilon * pressure) + Vbp)
        Fg = mg * g
        return (FB, Fg)
    ...
    def compute_dhdt(self, time, pressure):
        ...
        """
        return -np.gradient(pressure * 100000.0) / np.gradient(time) / self.RHO0 / self.G
...
class SteadyStateGliderModel(ModelParameters, GliderModel):
    ...
    def solve_for_angle_of_attack(self, pitch):
        ...
        if isinstance(self.Cd0, float):
            return self._SteadyStateGliderModel__solve_angle_of_attack_for_constant_parameters(pitch)
        ...
        for i, (_pitch, _Cd0) in enumerate(zip(pitch, self.Cd0)):
            s = np.sign(_pitch)
            _SteadyStateGliderModel__pitch = np.abs(_pitch)
            if _SteadyStateGliderModel__pitch < 0.157:
                aoa[i] = 0
            else:
                aoa[i:i + 1] = s * fsolve((self.model_fun), (_SteadyStateGliderModel__pitch / 50), args=(_SteadyStateGliderModel__pitch, _Cd0), full_output=0)
    ...
    def __solve_angle_of_attack_for_constant_parameters(self, pitch):
        if self.has_aoa_parameter_changed() or self.ifun is None:
            ...
            for i, _pitch in enumerate(self.pitch_i):
                tmp = fsolve((self.model_fun), (_pitch / 50),
                  args=(
                 _pitch, self.Cd0),
                  full_output=1)
        ...
        idx = np.where(np.logical_and(np.abs(pitch) >= self.pitch_i.min(), np.abs(pitch) <= self.pitch_i.max()))[0]
        aoa = np.zeros_like(pitch)
    ...
    def solve(self, data=None):
        ...
        pressure, Vbp = self.convert_pressure_Vbp_to_SI(data['pressure'], data['buoyancy_change'])
...
class DynamicGliderModel(ModelParameters, GliderModel):
    ...
    def __init__(self, dt=None, rho0=None, k1=0.2, k2=0.92, alpha_linear=90, alpha_stall=90, max_depth_considered_surface=0.5):
        ...
        self.k1 = k1
        self.k2 = k2
        self.alpha_linear = alpha_linear * np.pi / 180
        self.alpha_stall = 90 * np.pi / 180
        self.dt = dt
    ...
    def RK4(self, h, M, FBg, pitch, rho, at_surface, Cd0, u, w):
        ...
        for i in range(N - 1):
            ...
            if _at_surface:
                ...
                k1_u, k1_w = self._DynamicGliderModel__compute_k(_u, _w, _rho, _pitch, _FBg, _m11, _m12, _m21, _m22, h, _Cd0)
                _FBg, _pitch, _rho, _at_surface, _m11, _m12, _m21, _m22 = data_mid[i]
                _DynamicGliderModel__u = _u + k1_u * 0.5
                _DynamicGliderModel__w = _w + k1_w * 0.5
                k2_u, k2_w = self._DynamicGliderModel__compute_k(_DynamicGliderModel__u, _DynamicGliderModel__w, _rho, _pitch, _FBg, _m11, _m12, _m21, _m22, h, _Cd0)
                _DynamicGliderModel__u = _u + k2_u * 0.5
                _DynamicGliderModel__w = _w + k2_w * 0.5
                k3_u, k3_w = self._DynamicGliderModel__compute_k(_DynamicGliderModel__u, _DynamicGliderModel__w, _rho, _pitch, _FBg, _m11, _m12, _m21, _m22, h, _Cd0)
                _FBg, _pitch, _rho, _at_surface, _m11, _m12, _m21, _m22 = data[(i + 1)]
                _DynamicGliderModel__u = _u + k3_u
                _DynamicGliderModel__w = _w + k3_w
                k4_u, k4_w = self._DynamicGliderModel__compute_k(_DynamicGliderModel__u, _DynamicGliderModel__w, _rho, _pitch, _FBg, _m11, _m12, _m21, _m22, h, _Cd0)
    ...
    def integrate(self, data, dt=None):
        ...
        u = np.zeros_like(FB)
        w = np.zeros_like(FB)
        threshold = self.max_depth_considered_surface * 10000.0
...
class Calibrate(object):
    ...
    def AND(self, mask):
        ...
        mask : array of bool or bool
        """
        self.mask &= self._Calibrate__ensure_booltype(mask)
    ...
    def set_input_data(self, time, pressure, pitch, buoyancy_change, density, dhdt=None, u_relative=None, w_relative=None, U_relative=None, **kwds):
        """ Sets the input data
        time 
        pressure
        pitch
        ...
        self.input_data = dict(time=time, pressure=pressure, pitch=pitch, buoyancy_change=buoyancy_change, density=density,
          dhdt=dhdt,
          u_relative=u_relative,
          w_relative=w_relative,
          U_relative=U_relative)
        mask = np.zeros(time.shape, 'int').astype(bool)
        self.set_mask(mask)
    ...
    def calibrate(self, *p, constraints=('dhdt', ), weights=None, verbose=False):
        ...
        """
        constraints = self._Calibrate__ensure_iterable(constraints)
        x0 = [self.__dict__[i] / REFERENCE_VALUES[i] for i in p]
...
class SteadyStateCalibrate(SteadyStateGliderModel, Calibrate):
    __doc__ = ' Steady-state glider flight model, with calibration interface '
...
class DynamicCalibrate(DynamicGliderModel, Calibrate):
    __doc__ = ' Dynamic glider flight model, with calibration interface '
