...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gliderflight/glidertrim.py
# Compiled at: 2018-12-12 08:09:00
# Size of source mod 2**32: 9341 bytes
import configparser, glob, os, sys, numpy as np, matplotlib
matplotlib.use('TkAgg')
from matplotlib import pyplot
from matplotlib.dates import epoch2num, DateFormatter
import gsw, dbdreader, gliderflight
DEFAULTS = dict(target_density=1025, mg=65, Vg=0.065, minlimitdepth=10, maxlimitdepth=30, cond_a=1.0,
  cond_b=0.0,
  buoyancy_engine='shallow',
  latitude=54,
  longitude=8,
...
class UI(object):
    ...
    UNITS = dict(mg='kg', Vg='m^3', minlimitdepth='m', maxlimitdepth='m', cond_a='m/S', cond_b='-',
      buoyancy_engine='shallow|deep',
      latitude='decimal deg',
      longitude='decimal deg',
      calibrate_epsilon='yes|no',
    ...
    def configure(self):
        ...
        for k, v in self.settings.items():
            if k in UI.FLOAT_OPTIONS:
                ...
                if k == 'calibrate_epsilon':
                    if self.settings['buoyancy_engine'] == 'shallow':
                        self.settings[k] = 'no'
                        continue
                s = '%s (%s)' % (k, UI.UNITS[k])
...
class Model(object):
    ...
    def read_dbds(self, fns, settings):
        if settings['buoyancy_engine'] == 'shallow':
            ...
            if settings['buoyancy_engine'] == 'deep':
        ...
        data = dict(time=tctd, pressure=P, pitch=pitch, buoyancy_change=buoyancy_change,
          battpos=battpos,
          density=density)
        self.model.input_data = data
        mask = np.zeros(tctd.shape, 'int').astype(bool)
    ...
    def estimate_pitch_relation(self, settings):
        b = self.model.input_data['buoyancy_change'] * 1e-06
        p = self.model.input_data['pitch']
        bp = self.model.input_data['battpos'] * 0.0256
        P = self.model.input_data['pressure'] * 0.001
        if settings['calibrate_epsilon'] == 'yes':
            x = np.vstack([b, bp, np.ones(bp.shape), P])
        else:
            x = np.vstack([b, bp, np.ones(bp.shape)])
        condition = np.logical_and(abs(p) > 11 * np.pi / 180.0, np.isfinite(b))
    ...
    def report(self, results, settings):
        ...
        print('Weight change          : %f (g)' % ((settings['target_density'] - self.model.mg / self.model.Vg) * self.model.Vg * 1000.0))
        print()
    ...
    def graphic_report(self, results, settings):
        ...
        ax[0].plot((np.convolve(mr.ww, np.ones(15) / 15, 'same')), z,
          label='w_water', color='C5')
        ...
        rho_g = self.model.mg / (self.model.Vg * (1 - self.model.epsilon * 1e-10 * Pi * 100000.0))
        ax[1].plot(rho_g, zi, label='Glider density', color='C2')
        for i in range(1, 5):
            rho_g = (0.05 * i + self.model.mg) / (self.model.Vg * (1 - self.model.epsilon * 1e-10 * Pi * 100000.0))
