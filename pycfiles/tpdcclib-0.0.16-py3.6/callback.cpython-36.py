...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDccLib/abstract/callback.py
# Compiled at: 2020-01-16 21:51:46
# Size of source mod 2**32: 20937 bytes
...
class SimpleCallback(AbstractCallback):
    ...
    def _shutdown(self, *args):
        tp.logger.debug('Started: ({}) {} Shutdown'.format(str(self._notifier), self.__class__.__name__))
        for entry in self._registry:
            tp.logger.debug('{}._shutdown - Disconnecting ({})'.format(str(self._notifier), self.__class__.__name__, str(entry)))
            entry.token = self._disconnect(entry.token)

        del self._registry[:]
        (super(self.__class__, self)._shutdown)(*args)
        tp.logger.debug('Complete: ({}) {} Shutdown'.format(str(self._notifier), self.__class__.__name__))
    ...
    def register(self, fn, owner=None):
        ...
        tp.logger.debug('Started: ({}) {} Register - fn:"{}", owner:"{}", entry:"{}"'.format(str(self._notifier), self.__class__.__name__, str(fn), owner, str(entry)))
        if not entry:
            token = self._connect(fn) if self.connected else None
            tp.logger.debug('({}) {} Register - token:"{}"'.format(str(self._notifier), self.__class__.__name__, str(token)))
            self._registry.append(SimpleCallback.RegistryEntry(fn, token, owner=owner))
        tp.logger.debug('Completed: ({}) {} Register'.format(str(self._notifier), self.__class__.__name__))
    ...
    def unregister(self, fn):
        ...
        """
        entry = next((e for e in self._registry if e.callback == fn), None)
        tp.logger.debug('Started: ({}) {} Unregister - fn:"{}", entry:"{}"'.format(str(self._notifier), self.__class__.__name__, str(fn), str(entry)))
        if entry:
    ...
    def unregister_owner_callbacks(self, owner):
        ...
        tp.logger.debug('Started: ({}) {} Unregister - owner:"{}", entry:"{}"'.format(str(self._notifier), self.__class__.__name__, str(owner), str(entry)))
        if entry:
            self._disconnect(entry.token)
            self._registry.remove(entry)
        tp.logger.debug('Completed: ({}) {} Unregister'.format(str(self._notifier), self.__class__.__name__))
...
class FilterCallback(AbstractCallback, object):
    ...
    def register(self, fn, owner=None):
        ...
        @param owner: class, owner of the callback
        """
        tp.logger.debug('Started: ({}) {} Register - fn:"{}", owner:"{}", IsEmpty:"{}"'.format(str(self._notifier), self.__class__.__name__, str(fn), owner, bool(self.empty)))
        if self.empty:
            self._token = self._connect(self._notify)
            tp.logger.debug('({}) {} Register - token:"{}"'.format(str(self._notifier), self.__class__.__name__, str(self._token)))
    ...
    def unregister(self, fn):
        ...
        """
        entry = next((e for e in self._registry if e.callback == fn), None)
        tp.logger.debug('Started: ({}) {} Unregister - fn:"{}", IsEmpty:"{}"'.format(str(self._notifier), self.__class__.__name__, str(fn), bool(self.empty)))
        if entry:
            self._registry.remove(entry)
        if self.empty:
            if self.connected:
                tp.logger.debug('({}) {} Unregister token:"{}"'.format(str(self._notifier), self.__class__.__name__, str(self._token)))
                self._token = self._disconnect(self._token)
        tp.logger.debug('Completed: ({}) {} Unregister'.format(str(self._notifier), self.__class__.__name__))
...
class PythonTickCallback(ICallback, object):
    ...
    def register(cls, fn):
        ...
        if cls.tick_threads.get(fn_id, None) is not None:
            tp.logger.warning('{} already registered with PythonTickNotifier'.format(str(fn)))
            return
        else:
