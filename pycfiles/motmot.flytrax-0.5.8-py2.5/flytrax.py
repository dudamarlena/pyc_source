...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/motmot/flytrax/flytrax.py
# Compiled at: 2009-06-01 23:14:50
from __future__ import division
import sys, threading, Queue, time, socket, math, struct, os, pkg_resources, traxio
try:
    import trax_udp_sender
except ImportError:
    import flytrax.trax_udp_sender as trax_udp_sender

import motmot.wxvideo.wxvideo as wxvideo, motmot.imops.imops as imops, motmot.FastImage.FastImage as FastImage, motmot.realtime_image_analysis.realtime_image_analysis as realtime_image_analysis, numpy, motmot.wxvalidatedtext.wxvalidatedtext as wxvt, wx
from wx import xrc
import scipy.io
RESFILE = pkg_resources.resource_filename(__name__, 'flytrax.xrc')
...
class Tracker(trax_udp_sender.UDPSender):
    ...
    def __init__(self, wx_parent):
        ...
        self.frame = RES.LoadFrame(self.wx_parent, 'FLYTRAX_FRAME')
        ...
        self.mask_y_center = {}
        self.mask_radius = {}
        self.realtime_mask_x_center = {}
        self.realtime_mask_y_center = {}
        self.realtime_mask_radius = {}
        ...
        self.roi_display_sz = FastImage.Size(100, 100)
        self.roi_save_fmf_sz = FastImage.Size(100, 100)
        self.roi_send_sz = FastImage.Size(20, 20)
        send_to_ip_enabled_widget = xrc.XRCCTRL(self.frame, 'SEND_TO_IP_ENABLED')
        send_to_ip_enabled_widget.Bind(wx.EVT_CHECKBOX, self.OnEnableSendToIP)
    ...
    def camera_starting_notification(self, cam_id, pixel_format=None, max_width=None, max_height=None):
        ...
        wx.EVT_BUTTON(ctrl, ctrl.GetId(), self.OnTakeBgImage)
        ctrl = xrc.XRCCTRL(per_cam_panel, 'LOAD_BG_IMAGE')
        self.widget2cam_id[ctrl] = cam_id
        wx.EVT_BUTTON(ctrl, ctrl.GetId(), self.OnLoadBgImage)
        ctrl = xrc.XRCCTRL(per_cam_panel, 'ONGOING_BG_UPDATES')
        self.widget2cam_id[ctrl] = cam_id
        wx.EVT_CHECKBOX(ctrl, ctrl.GetId(), self.OnEnableOngoingBg)
        ...
        self.widget2cam_id[ctrl] = cam_id
        validator = wxvt.setup_validated_integer_callback(ctrl, ctrl.GetId(), self.OnSetNumBackgroundImages)
        ...
        self.widget2cam_id[ctrl] = cam_id
        validator = wxvt.setup_validated_integer_callback(ctrl, ctrl.GetId(), self.OnSetBackgroundUpdateInterval)
        ...
        wx.EVT_CHECKBOX(tracking_enabled_widget, tracking_enabled_widget.GetId(), self.OnTrackingEnabled)
        use_roi2_widget = xrc.XRCCTRL(per_cam_panel, 'USE_ROI2')
        self.widget2cam_id[use_roi2_widget] = cam_id
        wx.EVT_CHECKBOX(use_roi2_widget, use_roi2_widget.GetId(), self.OnUseROI2)
        ...
        self.widget2cam_id[ctrl] = cam_id
        validator = wxvt.setup_validated_float_callback(ctrl, ctrl.GetId(), self.OnClearThreshold, ignore_initial_value=True)
        self.xrcid2validator[cam_id]['CLEAR_THRESHOLD'] = validator
        ctrl = xrc.XRCCTRL(per_cam_panel, 'DIFF_THRESHOLD')
        self.widget2cam_id[ctrl] = cam_id
        validator = wxvt.setup_validated_float_callback(ctrl, ctrl.GetId(), self.OnDiffThreshold, ignore_initial_value=True)
        self.xrcid2validator[cam_id]['DIFF_THRESHOLD'] = validator
        ctrl = xrc.XRCCTRL(per_cam_panel, 'HISTORY_BUFFER_LENGTH')
        self.widget2cam_id[ctrl] = cam_id
        validator = wxvt.setup_validated_integer_callback(ctrl, ctrl.GetId(), self.OnHistoryBuflen, ignore_initial_value=True)
        ...
        wx.EVT_BUTTON(start_recording_widget, start_recording_widget.GetId(), self.OnStartRecording)
        stop_recording_widget = xrc.XRCCTRL(per_cam_panel, 'STOP_RECORDING')
        self.widget2cam_id[stop_recording_widget] = cam_id
        wx.EVT_BUTTON(stop_recording_widget, stop_recording_widget.GetId(), self.OnStopRecording)
        ...
        self.widget2cam_id[ctrl] = cam_id
        wxvt.setup_validated_integer_callback(ctrl, ctrl.GetId(), self.OnSaveNthFrame)
        self.OnSaveNthFrame(force_cam_id=cam_id)
        self.save_data_prefix_widget[cam_id] = xrc.XRCCTRL(per_cam_panel, 'SAVE_DATA_PREFIX')
        ...
        wx.EVT_CHECKBOX(view_mask_mode_widget, view_mask_mode_widget.GetId(), self.OnViewMaskMode)
        ...
        wx.EVT_COMMAND_SCROLL(mask_x_center_widget, mask_x_center_widget.GetId(), self.OnScrollMaskXCenter)
        ...
        wx.EVT_COMMAND_SCROLL(mask_y_center_widget, mask_y_center_widget.GetId(), self.OnScrollMaskYCenter)
        ...
        wx.EVT_COMMAND_SCROLL(mask_radius_widget, mask_radius_widget.GetId(), self.OnScrollMaskRadius)
        ...
        roi2_radius = 61
        ra = realtime_image_analysis.RealtimeAnalyzer(lbrt, max_width, max_height, max_num_points, roi2_radius)
    ...
    def OnTakeBgImage(self, event):
        ...
        if not ctrl.GetValue() and cam_id in self.trx_writer:
            dlg = wx.MessageDialog(self.wx_parent, 'Saving data - cannot take background image', 'FlyTrax error', wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def _process_frame_extract_roi(self, points, roi_sz, fibuf, buf_offset, full_frame_live, max_frame_size):
        n_pts = len(points)
        if n_pts:
            pt = points[0]
        ...
        if fibuf.size == max_frame_size:
            ...
            software_roi = tmp.get_8u_copy(tmp.size)
        return (software_roi, (x0, y0))

    def process_frame(self, cam_id, buf, buf_offset, timestamp, framenumber):
        if self.pixel_format[cam_id] == 'YUV422':
            buf = imops.yuv422_to_mono8(numpy.asarray(buf))
        ...
        if self.tracking_enabled[cam_id].isSet():
            points = realtime_analyzer.do_work(fibuf, timestamp, framenumber, use_roi2, use_cmp=use_cmp)
            self.roi_sz_lock.acquire()
            try:
            ...
            (roi_display, (display_x0, display_y0)) = self._process_frame_extract_roi(points, roi_display_sz, fibuf, buf_offset, full_frame_live, max_frame_size)
            (roi_save_fmf, (fmf_save_x0, fmf_save_y0)) = self._process_frame_extract_roi(points, roi_save_fmf_sz, fibuf, buf_offset, full_frame_live, max_frame_size)
            (roi_send, (udp_send_x0, udp_send_y0)) = self._process_frame_extract_roi(points, roi_send_sz, fibuf, buf_offset, full_frame_live, max_frame_size)
            n_pts = len(points)
            if n_pts:
                pt = points[0]
                (x, y, area, slope, eccentricity) = pt[:5]
                numdata = (
                 x, y, slope, fmf_save_x0, fmf_save_y0, timestamp, area, framenumber)
                ...
                if self.send_over_ip.isSet() and runthread_remote_host is not None:
                    ...
                    databuf1 = struct.pack('cBLdfffffBBII', 'e', cam_no, framenumber, timestamp, x, y, area, slope, eccentricity, roi_send.size.w, roi_send.size.h, udp_send_x0, udp_send_y0)
                ...
                while 1:
                    ...
                    if trx_writer is not None:
                        ...
                        if framenumber % self.save_nth_frame[cam_id] == 0:
                            trx_writer.write_data(roi_img=roi_img, posx=posx, posy=posy, orientation=orientation, windowx=windowx, windowy=windowy, timestamp=timestamp, area=area)
    ...
    def OnStartRecording(self, event):
        ...
        self.bg_update_lock.release()
        if bg_image is None:
            dlg = wx.MessageDialog(self.wx_parent, 'No background image (%s)- cannot save data' % cam_id, 'FlyTrax error', wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def update_screen(self):
        ...
        if have_new_image:
            ...
            for orig_pt in orig_points:
                ...
                ox0 = ox0 - roi_display_left
                oy0 = oy0 - roi_display_bottom
            ...
            self.live_canvas.update_image_and_drawings('camera', im, format=format)
            self.live_canvas.Refresh(eraseBackground=False)
            if BGROI_IM:
                self.bgroi_canvas.update_image_and_drawings('camera', bgroi_im, format=format)
                self.bgroi_canvas.Refresh(eraseBackground=False)
            if DEBUGROI_IM:
                self.debugroi_canvas.update_image_and_drawings('camera', debugroi_im, format=format, points=points, linesegs=linesegs)
