...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/msg_topgen/generate.py
# Compiled at: 2018-01-29 08:29:07
...
def generate_connectors(graph, node, nbrdict, node_type):
    ...
    if node not in nx.get_node_attributes(graph, 'def_conn'):
        for out in nbrdict.keys():
            if node_type[out] == 'router':
                connectors.append({'name': out, 
                   'host': out, 
                   'port': get_neighbor_port(graph, out), 
                   'role': 'inter-router'})
                ...
                connectors.append({'name': out, 
                   ...
                   'role': 'route-container'})
                link_route.append({'prefix': node + '_queue', 
                   'connection': out, 
                   'dir': 'in'})
                link_route.append({'prefix': node + '_queue', 
                   'connection': out, 
...
def generate_connection_settings(graph, node):
    ...
    if isinstance(node_attributes, dict):
        for component, attributes in node_attributes.items():
            if isinstance(attributes, list) and attributes and component not in ('listener',
                                                                                 'address',
                                                                                 'connector',
                                                                                 'linkRoute',
                                                                                 'router'):
