...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/msg_topgen/arg_parser.py
# Compiled at: 2017-12-05 08:29:10
import argparse, shlex, sys, yaml
        ...
        if line.startswith('['):
            ...
            if state == 'hosts':
                ...
                for var in variables:
                    if '=' not in var:
                        msg('E', 'Expected key=value host variable assignment, got: %s' % var)
                    key, val = var.split('=', 1)
                    if 'hosts' in state:
...
class Config:
    ...
    def args_parse(self):
        ...
        parser.add_argument('-o', '--output-dir', action='store', dest='out_dir', help='Path to output dir', required=False)
        required = parser.add_argument_group('required arguments')
        required.add_argument('-c', '--config-file', action='store', dest='config_file', help='Path to config file', required=True)
