...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/mlocs/__init__.py
# Compiled at: 2013-04-13 16:18:28
...
def toLoc(maiden):
    ...
    lon = -90
    lat = -90
    lets = re.findall('([A-Xa-x])([A-Xa-x])', maiden)
    nums = re.findall('(\\d)(\\d)', maiden)
    i = 0
    tot = 0
    ...
    for x, y in lets:
        val[i * 2] = (
         ord(string.upper(x)) - A, ord(string.upper(y)) - A)
        i += 1
        tot += 1
