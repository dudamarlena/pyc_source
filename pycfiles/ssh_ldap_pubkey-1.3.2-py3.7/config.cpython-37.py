...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ssh_ldap_pubkey/config.py
...
def parse_config(content):
    ...
    Returns:
        dict: Parsed options.
    """
    return {match.group(1).lower():match.group(2).strip() for match in (re.match('^(\\w+)\\s+([^#]+)', line) for line in content.splitlines()) if match if match}
...
def parse_tls_reqcert_opt(value):
    ...
    if value:
        return {'never':ldap.OPT_X_TLS_NEVER,  'allow':ldap.OPT_X_TLS_ALLOW,  'try':ldap.OPT_X_TLS_TRY,  'demand':ldap.OPT_X_TLS_DEMAND,  'hard':ldap.OPT_X_TLS_HARD}[value.lower()]


def parse_scope_opt(value):
    """Convert `scope` option to ldap's `SCOPE_*` constant."""
    if value:
        return {'base':ldap.SCOPE_BASE,  'one':ldap.SCOPE_ONELEVEL,  'sub':ldap.SCOPE_SUBTREE}[value.lower()]
