...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ssh_ldap_pubkey/__init__.py
# Compiled at: 2019-08-20 19:18:36
# Size of source mod 2**32: 9955 bytes
from __future__ import absolute_import, print_function
import base64, ldap, struct, sys
...
class LdapSSH(object):
    ...
    def connect(self):
        ...
        """
        conf = self.conf
        if not (conf.uris and conf.base):
        ...
        try:
            ...
            if conf.sasl == 'GSSAPI':
                ...
                return
            if conf.ssl == 'start_tls':
                if conf.ldap_version >= 3:
                    conn.start_tls_s()
            if conf.bind_dn:
                if conf.bind_pass:
    ...
    def add_pubkey(self, login, password, pubkey):
        ...
        if self._has_pubkey(dn, pubkey):
            raise PubKeyAlreadyExistsError('Public key %s already exists.' % keyname(pubkey), 1)
        ...
        try:
            ...
            raise ConfigError("LDAP server doesn't define schema for attribute: %s" % conf.pubkey_attr, 1)
    ...
    def _find_pubkeys(self, dn):
        conf = self.conf
        result = self._conn.search_s(dn,
          (ldap.SCOPE_BASE), attrlist=[conf.pubkey_attr])
    ...
    def _remove_pubkey(self, dn, pubkey):
        ...
        modlist = [
         (
          ldap.MOD_DELETE, conf.pubkey_attr, _encode(pubkey))]
        try:
            self._conn.modify_s(dn, modlist)
        except ldap.OBJECT_CLASS_VIOLATION:
