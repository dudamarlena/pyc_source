...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/vagoth/tests/registry_mixin.py
# Compiled at: 2013-12-29 04:28:26
...
class RegistryMixin(object):
    ...
    def mixin_setUp(self):
        ...
        self.registry.add_node('0xdeadbeef', node_type='hv', node_name='node001.example.com', tenant='mytenant', definition={'name': '0xdeadbeef', 'fqdn': 'node001.example.com'}, metadata={'mymetakey': 'mymetavalue'}, tags={'tag1': True, 'tag2': 'somevalue'}, unique_keys=[
         'node001_uniquekey'])
    ...
    def test_initial_object(self):
        ...
        self.assertEqual(node.definition, {'name': '0xdeadbeef', 'fqdn': 'node001.example.com'})
        self.assertEqual(node.metadata, {'mymetakey': 'mymetavalue'})

    def test_get_nodes(self):
        self.assertEqual(1, len(list(self.registry.get_nodes())))
        self.assertEqual(1, len(list(self.registry.get_nodes(tenant='mytenant'))))
        self.assertEqual(0, len(list(self.registry.get_nodes(tenant='nonexistant'))))
        self.assertEqual(1, len(list(self.registry.get_nodes(tags={'tag1': None}))))
        self.assertEqual(1, len(list(self.registry.get_nodes(tags={'tag2': 'somevalue'}))))
        self.assertEqual(0, len(list(self.registry.get_nodes(tags={'tag2': 'wrongvalue'}))))
        self.assertEqual(1, len(list(self.registry.get_nodes(parent=None))))
        self.assertEqual(1, len(list(self.registry.get_nodes(node_type='hv'))))
        self.assertEqual(1, len(list(self.registry.get_nodes(tenant='mytenant', parent=None, tags={'tag2': 'somevalue'}, node_type='hv'))))
        self.assertEqual(0, len(list(self.registry.get_nodes(tenant='wrongtenant', parent=None, tags={'tag2': 'somevalue'}, node_type='hv'))))
        return
    ...
    def test_update_metadata(self):
        self.registry.update_metadata('0xdeadbeef', {'one': 'two'})
    ...
    def test_change_name(self):
        ...
        self.assertEqual('0xdeadbeef', self.registry.get_node_by_key('VAGOTH_NAME_foo.example.com').id)
        self.assertRaises(exceptions.NodeNotFoundException, self.registry.get_node_by_key, 'VAGOTH_NAME_node001.example.com')
    ...
    def test_change_keys(self):
        ...
        self.assertEqual(node.id, '0xdeadbeef')
        self.assertRaises(exceptions.NodeNotFoundException, self.registry.get_node_by_key, 'node001_uniquekey')

    def test_delete_node(self):
        self.registry.delete_node('0xdeadbeef')
        self.assertRaises(exceptions.NodeNotFoundException, self.registry.get_node, '0xdeadbeef')
