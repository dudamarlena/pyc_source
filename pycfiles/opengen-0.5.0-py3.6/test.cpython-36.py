...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/opengen/test/test.py
# Compiled at: 2020-05-11 18:43:50
# Size of source mod 2**32: 15471 bytes
import unittest, casadi.casadi as cs, opengen as og, subprocess
...
class RustBuildTestCase(unittest.TestCase):
    ...
    def solverConfig(cls):
        solver_config = og.config.SolverConfiguration().with_lbfgs_memory(15).with_tolerance(0.0001).with_initial_tolerance(0.0001).with_delta_tolerance(0.0001).with_initial_penalty(15.0).with_penalty_weight_update_factor(10.0).with_max_inner_iterations(155).with_max_duration_micros(100000000.0).with_max_outer_iterations(50).with_sufficient_decrease_coefficient(0.05).with_cbfgs_parameters(1.5, 1e-10, 1e-12)
    ...
    def setUpOnlyF1(cls):
        ...
        meta = og.config.OptimizerMeta().with_optimizer_name('only_f1')
        problem = og.builder.Problem(u, p, phi).with_aug_lagrangian_constraints(f1, set_c).with_constraints(bounds)
        build_config = og.config.BuildConfiguration().with_open_version(RustBuildTestCase.OPEN_RUSTLIB_VERSION).with_build_directory(RustBuildTestCase.TEST_DIR).with_build_mode('debug').with_tcp_interface_config(tcp_interface_config=tcp_config).with_build_c_bindings()
        og.builder.OpEnOptimizerBuilder(problem, metadata=meta,
          build_configuration=build_config,
          solver_configuration=(cls.solverConfig())).build()
    ...
    def setUpOnlyF2(cls):
        ...
        meta = og.config.OptimizerMeta().with_optimizer_name('only_f2')
    ...
    def setUpPlain(cls):
        ...
        meta = og.config.OptimizerMeta().with_optimizer_name('plain')
    ...
    def setUpRosPackageGeneration(cls):
        ...
        bounds = og.constraints.Ball2(None, 1.5)
        meta = og.config.OptimizerMeta().with_optimizer_name('rosenbrock_ros')
        problem = og.builder.Problem(u, p, phi).with_constraints(bounds).with_penalty_constraints(c)
        ros_config = og.config.RosConfiguration().with_package_name('parametric_optimizer').with_node_name('open_node').with_rate(35).with_description('really cool ROS node')
        build_config = og.config.BuildConfiguration().with_open_version(RustBuildTestCase.OPEN_RUSTLIB_VERSION).with_build_directory(RustBuildTestCase.TEST_DIR).with_build_mode('debug').with_build_c_bindings().with_ros(ros_config)
        og.builder.OpEnOptimizerBuilder(problem, metadata=meta,
          build_configuration=build_config,
          solver_configuration=(cls.solverConfig())).build()
    ...
    def setUpOnlyParametricF2(cls):
        ...
        p = cs.MX.sym('p', 3)
        f2 = u[0] - p[2]
        phi = og.functions.rosenbrock(u, cs.vertcat(p[0], p[1]))
        bounds = og.constraints.Ball2(None, 1.5)
        tcp_config = og.config.TcpServerConfiguration(bind_port=4599)
        meta = og.config.OptimizerMeta().with_optimizer_name('parametric_f2')
        problem = og.builder.Problem(u, p, phi).with_penalty_constraints(f2).with_constraints(bounds)
        build_config = og.config.BuildConfiguration().with_open_version(RustBuildTestCase.OPEN_RUSTLIB_VERSION).with_build_directory(RustBuildTestCase.TEST_DIR).with_build_mode('debug').with_tcp_interface_config(tcp_interface_config=tcp_config).with_build_c_bindings()
        solver_config = og.config.SolverConfiguration().with_tolerance(1e-06).with_initial_tolerance(0.0001).with_delta_tolerance(1e-05)
    ...
    def test_rectangle_empty(self):
        xmin = [
         -1, 2]
        xmax = [-2, 4]
        with self.assertRaises(Exception) as (_RustBuildTestCase__context):
    ...
    def test_rust_build_only_f1(self):
        ...
        response = mng.call(p=[2.0, 10.0], initial_guess=(response['solution']),
    ...
    def test_rust_build_parametric_f2(self):
        ...
        status = response.get()
        self.assertEqual('Converged', status.exit_status)
        self.assertTrue(status.f2_norm < 0.0001)
        mng.kill()
    ...
    def c_bindings_helper(optimizer_name):
        ...
        p.communicate()
        rc1 = p.returncode
        p = subprocess.Popen([RustBuildTestCase.TEST_DIR + '/' + optimizer_name + '/optimizer'], stdout=(subprocess.DEVNULL))
        p.communicate()
        rc2 = p.returncode
        return (
         rc1, rc2)
