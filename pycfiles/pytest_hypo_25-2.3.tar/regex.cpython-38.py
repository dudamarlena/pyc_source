...
# [GCC 8.4.0]
# Embedded file name: C:\Users\User\AppData\Local\Temp\pip-install-oj_abz_z\hypothesis\hypothesis\strategies\_internal\regex.py
# Compiled at: 2020-01-12 08:06:33
# Size of source mod 2**32: 16946 bytes
import operator, re, sre_constants as sre, sre_parse, sys
...
BYTES_DIGIT = {b for b in BYTES_ALL if re.match(b'\\d', b) if re.match(b'\\d', b)}
BYTES_SPACE = {b for b in BYTES_ALL if re.match(b'\\s', b) if re.match(b'\\s', b)}
BYTES_WORD = {b for b in BYTES_ALL if re.match(b'\\w', b) if re.match(b'\\w', b)}
BYTES_LOOKUP = {sre.CATEGORY_DIGIT: BYTES_DIGIT, 
...
@st.composite
def reuse_group--- This code section failed: ---

 L.  68         0  LOAD_FAST                'draw'
                2  LOAD_GLOBAL              GROUP_CACHE_STRATEGY
                4  CALL_FUNCTION_1       1  ''
                6  STORE_FAST               'cache'

 L.  69         8  SETUP_FINALLY        20  'to 20'

 L.  70        10  LOAD_FAST                'cache'
               12  LOAD_FAST                'group_name'
               14  BINARY_SUBSCR    
               16  POP_BLOCK        
               18  RETURN_VALUE     
...
class CharactersBuilder:
    ...
    def strategy(self):
        ...
        if self._negate:
            ...
            return st.characters(blacklist_categories=(self._categories | {'Cc', 'Cs'}),
        ...
        return st.characters(whitelist_categories=(self._categories),
    ...
    def add_category(self, category):
        ...
        if category == sre.CATEGORY_DIGIT:
            ...
            if category == sre.CATEGORY_NOT_DIGIT:
                self._categories |= UNICODE_CATEGORIES - UNICODE_DIGIT_CATEGORIES
            else:
                if category == sre.CATEGORY_SPACE:
                    self._categories |= UNICODE_SPACE_CATEGORIES
                    self._whitelist_chars |= UNICODE_SPACE_CHARS if self._unicode else SPACE_CHARS
                else:
                    if category == sre.CATEGORY_NOT_SPACE:
                        self._categories |= UNICODE_CATEGORIES - UNICODE_SPACE_CATEGORIES
                        self._blacklist_chars |= UNICODE_SPACE_CHARS if self._unicode else SPACE_CHARS
                    else:
                        if category == sre.CATEGORY_WORD:
                            self._categories |= UNICODE_WORD_CATEGORIES
                            self._whitelist_chars.add('_')
                        else:
                            if category == sre.CATEGORY_NOT_WORD:
    ...
    def add_char(self, char):
        ...
        if self._ignorecase:
            if re.match((re.escape(c)), (c.swapcase()), flags=(re.IGNORECASE)) is not None:
...
def maybe_pad(draw, regex, strategy, left_pad_strategy, right_pad_strategy):
    ...
    if left_pad:
        if regex.search(left_pad + result):
            result = left_pad + result
    right_pad = draw(right_pad_strategy)
    if right_pad:
        if regex.search(result + right_pad):
            result += right_pad
    return result
...
def base_regex_strategy(regex, parsed=None):
    ...
    return clear_cache_after_draw(_strategy(parsed, Context(flags=(regex.flags)), isinstance(regex.pattern, str)))
    ...
    if fullmatch:
        ...
        if parsed[0][0] == sre.AT:
            if parsed[0][1] == sre.AT_BEGINNING_STRING:
                ...
                if parsed[0][1] == sre.AT_BEGINNING:
                    if regex.flags & re.MULTILINE:
...
def _strategy(codes, context, is_unicode):
    ...
    if not isinstance(codes, tuple):
        ...
        i = 0
        if i < len(codes):
            if codes[i][0] == sre.LITERAL:
                if not context.flags & re.IGNORECASE:
                    j = i + 1
                    if j < len(codes):
                        if codes[j][0] == sre.LITERAL:
                            j += 1
                    else:
                        if i + 1 < j:
                            strategies.append(st.just(empty.join([to_char(charcode) for _, charcode in codes[i:j]])))
                            i = j
    ...
    if code == sre.LITERAL:
        ...
        if context.flags & re.IGNORECASE:
            if c != c.swapcase():
                if re.match(re.escape(c), c.swapcase(), re.IGNORECASE) is not None:
            ...
            for charset_code, charset_value in value:
                ...
                if code == sre.ANY:
                    ...
                    if code in (sre.MIN_REPEAT, sre.MAX_REPEAT):
                        ...
                        return st.lists((recurse(subregex)), min_size=at_least, max_size=at_most).map(empty.join)
