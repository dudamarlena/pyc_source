...
# [GCC 8.4.0]
# Embedded file name: C:\Users\User\AppData\Local\Temp\pip-install-oj_abz_z\hypothesis\hypothesis\control.py
...
def target(observation: float, label: str='') -> None:
    ...
    if not math.isfinite(observation):
        ...
        if label in context.data.target_observations:
            raise InvalidArgument('Calling target(%r, label=%r) would overwrite target(%r, label=%r)' % (
