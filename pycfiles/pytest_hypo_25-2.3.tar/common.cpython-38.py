...
# [GCC 8.4.0]
# Embedded file name: C:\Users\User\AppData\Local\Temp\pip-install-oj_abz_z\hypothesis\hypothesis\internal\conjecture\shrinking\common.py
# Compiled at: 2020-01-12 08:06:33
# Size of source mod 2**32: 6896 bytes
...
class Shrinker:
    __doc__ = 'A Shrinker object manages a single value and a predicate it should\n    satisfy, and attempts to improve it in some direction, making it smaller\n    and simpler.'

    def __init__(self, initial, predicate, random, full=False, debug=False, name=None, **kwargs):
        (self.setup)(**kwargs)
        ...
        self._Shrinker__seen = set()
        self.debugging_enabled = debug

    @property
    def calls(self):
        return len(self._Shrinker__seen)
    ...
    def delegate(self, other_class, convert_to, convert_from, **kwargs):
        ...
        (self.call_shrinker)(
         ...
         (lambda v: self.consider(convert_from(v))), **kwargs)
    ...
    def incorporate(self, value):
        ...
        if not self.left_is_better(value, self.current):
            if value != self.current:
                if value == value:
                    self.debug('Rejected %r as worse than self.current=%r' % (value, self.current))
            return False
        if value in self._Shrinker__seen:
            return False
        self._Shrinker__seen.add(value)
        if self._Shrinker__predicate(value):
