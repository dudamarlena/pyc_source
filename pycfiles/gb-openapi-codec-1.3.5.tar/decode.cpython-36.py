...
# [GCC 8.4.0]
# Embedded file name: /Users/adam/Projects/crowdcomms/gb-openapi-codec/gb_openapi_codec/python-openapi-codec/openapi_codec/decode.py
# Compiled at: 2017-08-11 00:43:42
# Size of source mod 2**32: 8912 bytes
                ...
                for parameter in parameters:
                    ...
                    if location == 'body':
                        ...
                        if expanded is not None:
                            expanded_fields = [Field(name=field_name, location='form', required=is_required, schema=coreschema.String(description=field_description)) for field_name, is_required, field_description in expanded if not any([field.name == field_name for field in fields])]
...
def _select_encoding(consumes, form=False):
    ...
    """
    if form:
        preference = ['multipart/form-data',
...
def _expand_schema(schema):
    ...
    if schema_type == ['object'] or schema_type == 'object':
        if schema_properties:
            return [(key, key in schema_required, schema_properties[key].get('description')) for key in schema_properties.keys()]
...
def get_dicts(item, dereference_using=None):
    ...
    if dereference_using:
        return [dereference(value['$ref'], dereference_using) if is_json_pointer(value) else value for value in ret]
    else:
        return ret
