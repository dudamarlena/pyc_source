...
# [GCC 8.4.0]
# Embedded file name: /homes/students/weigl/workspace1/restflow/restflow/hf3lint.py
# Compiled at: 2014-08-27 04:58:45
...
__version__ = '0.1'
import re
REGEX_NATURAL = re.compile('^\\d+$')
REGEX_INTEGER = re.compile('^[+-]?\\d+$')
REGEX_FLOAT = re.compile('^[+-]?(\\d+(\\.\\d*)?|\\.\\d+)([eE][+-]?\\d+)?$')
...
class HF3DataLint(_RuleDispatcher):
    FIELDS = {'Param': {'OutputPathAndPrefix': Checkers.is_str, 
                 ...
                 'Instationary': {'SolveInstationary': Checkers.oneOf('1', '2'), 
                                  'DampingFactor': 1.0, 
                 ...
                 'Boundary': {'DirichletMaterial1': Checkers.is_natural_number, 
                 ...
                 'LinearSolver': {'SolverName': Checkers.oneOf('CG', 'GMRES'), 
                                  ...
                                  'PreconditionerName': Checkers.oneOf('SGAUSS_SEIDEL', 'NOPRECOND', 'JACOBI', 'GAUSS_SEIDEL', 'SGAUSS_SEIDEL', 'SOR', 'SSOR', 'ILU', 'ILU2', 'ILU_P', 'ILUpp'), 
...
class BCDataLint(_RuleDispatcher):
    ...
    def _check_exists(self, data, path, prefixpath=None):
        ...
        self.assert_error(t, 'Field does not exists', (prefixpath + '.' if prefixpath else '') + path)
        return t
    ...
    def _rule_FixedConstraintsBCs(self, data):
        ...
        self._check_entry(data, rootpath, NumberOfFixedDirichletPoints, fDPoints, fDisplacements)
    ...
    def _rule_DisplacementConstraintsBCs(self, data):
        ...
        dDPoints = 'dDPoints'
        self._check_entry(data, rootpath, NumberOfDisplacedDirichletPoints, dDPoints, dDisplacements)
    ...
    def _rule_ForceOrPressureBCs(self, data):
        ...
        self._check_entry(data, rootpath, NumberOfForceOrPressureBCPoints, ForceOrPressureBCPoints, ForcesOrPressure)
    ...
    def _check_entry(self, data, rootpath, numberOf, points, displacement):
        ...
        if struct is None:
            ...
            all_exists = self._check_exists(struct, numberOf, rootpath) and self._check_exists(struct, points, rootpath) and self._check_exists(struct, displacement, rootpath)
            if not all_exists:
                return
            ...
            def check_points(field):
                ...
                self.assert_error(exp == found, 'Amount of points mismatch (expected %s, found %s)' % (exp, found), rootpath + '.' + field)

            check_points(points)
            check_points(displacement)
            self._check_points_format(rootpath + '.' + displacement, get(displacement, struct))
            self._check_points_format(rootpath + '.' + points, get(points, struct))
            return
...
def print_report(report):
    ...
    for r in report:
        ...
        print _FORMAT.format(type=r.level, number=r.number, msg=r.message, color='\x1b[%dm' % color, nocolor='\x1b[0m', grey='\x1b[37m', path=r.path)
