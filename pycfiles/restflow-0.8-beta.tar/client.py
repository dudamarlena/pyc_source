...
# [GCC 8.4.0]
# Embedded file name: /homes/students/weigl/workspace1/restflow/restflow/client.py
# Compiled at: 2014-08-29 13:38:20
"""Client for restflow
...
class _LowLevel(object):
    ...
    def _handle_json(self, resp):
        ...
        if resp.status_code == 200:
            return resp.json()
        raise HTTPResponseError('HTTPRequest %s answered with %d. Content: %s' % (resp.url, resp.status_code, resp.text), response=resp)
    ...
    def set_config(self, token, hf3_config, bc_config):
        resp = self.session.put(self.url('/session/{token}', token=token), data={'hf3': json.dumps(hf3_config), 'bc': json.dumps(bc_config)})
        return self._handle_json(resp)

    def set_property(self, token, update, config='hf3'):
        resp = self.session.post(self.url('/session/{token}', token=token), data={'update': json.dumps(update), 'config': config})
        return self._handle_json(resp)
    ...
    def get_result(self, token, step, function, target):
        ...
        with open(target, 'wb') as (fp):
            with closing(requests.get(self.url('/session/{token}/result/{step}/{type}', token=token, step=step, type=function), stream=True)) as (r):
    ...
    def upload_mesh(self, filename=None, url=None):
        ...
        if filename:
            ...
            m = MultipartEncoder(fields={'file': (
                      ...
                      open(filename, 'rb'),
                      'text/plain')})
            r = requests.post(self.url('asset'), data=m, headers={'Content-Type': m.content_type})
        elif url:
...
class RestApiException(BaseException):
    pass