...
# [GCC 8.4.0]
# Embedded file name: /Users/f1nal/Dropbox/python/jet-bridge/src/packages/jet_bridge/jet_bridge/utils/create_config.py
...
def create_config(config_not_set):
    ...
    if 'project' not in settings.USE_DEFAULT_CONFIG:
        project = prompt((promt_message('<green><b>Enter your project unique name</b></green>')),
          default=(settings.PROJECT))
        print_formatted_text('')
    else:
        project = settings.PROJECT
    if 'token' not in settings.USE_DEFAULT_CONFIG:
        token = prompt((promt_message('<green><b>Enter your Jet Bridge token</b></green>')),
          default=(settings.TOKEN))
        print_formatted_text('')
    else:
        token = settings.TOKEN
    if 'address' not in settings.USE_DEFAULT_CONFIG:
        address = prompt((promt_message('<green><b>Which host to run Jet Bridge on?</b></green>\n<i>Default is {}</i>'.format('0.0.0.0 (any IP)'))),
          default=(settings.ADDRESS or '0.0.0.0'))
        print_formatted_text('')
    else:
        address = settings.ADDRESS or '0.0.0.0'
    port = prompt((promt_message('<green><b>Which port to run Jet Bridge on?</b></green>\n<i>Default is {}</i>'.format('8888'))),
      validator=Validator.from_callable(port_is_valid,
      error_message='Incorrect port',
      move_cursor_to_end=True),
      default=('{0}'.format(settings.PORT) if settings.PORT else '8888'))
    print_formatted_text('')
    database_engine = prompt((promt_message('<green><b>Which database do you use?</b></green>\n<i>Should be one of: {}</i>'.format(', '.join(map(lambda x: x['name'], engines))))),
      validator=Validator.from_callable(database_engine_is_valid,
    ...
    while True:
        ...
        if database_engine == 'sqlite':
            prompts = 1
            database_name = prompt((promt_message('<green><b>Enter your database file path</b></green>')),
              validator=Validator.from_callable(is_file_exists,
            ...
            database_host = prompt((promt_message(message)),
              validator=Validator.from_callable(is_not_empty,
              error_message='Database host is required',
              move_cursor_to_end=True),
              default=default)
            print_formatted_text('')
            default_port = list(map(lambda x: six.text_type(x.get('default_port', '')), filter(lambda x: x['name'] == database_engine, engines)))[0]
            database_port = prompt((promt_message('<green><b>Enter your database port</b></green>')),
              validator=Validator.from_callable(is_not_empty,
            ...
            print_formatted_text('')
            database_name = prompt((promt_message('<green><b>Enter your database name</b></green>')),
              validator=Validator.from_callable(is_not_empty,
              error_message='Database name is required',
              move_cursor_to_end=True),
              default=(database_name or settings.DATABASE_NAME or ''))
            print_formatted_text('')
            database_user = prompt((promt_message('<green><b>Enter your database user</b></green>')),
              validator=Validator.from_callable(is_not_empty,
              error_message='Database user is required',
              move_cursor_to_end=True),
              default=(database_user or settings.DATABASE_USER or ''))
            print_formatted_text('')
            database_password = prompt((promt_message('<green><b>Enter your database password</b></green>')),
              default=(database_password or settings.DATABASE_PASSWORD or ''))
        engine_url = build_engine_url(database_engine, database_host, database_port, database_name, database_user, database_password)
        if not engine_url:
            pass
        else:
    ...
    config_content = {'ADDRESS':address, 
     'PORT':port, 
     'CONFIG':config, 
     'PROJECT':project, 
     'TOKEN':token, 
