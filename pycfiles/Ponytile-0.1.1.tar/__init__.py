...
# Embedded file name: /Library/Frameworks/Python.framework/Versions/2.6/lib/python2.6/site-packages/ponyexpress/__init__.py
# Compiled at: 2011-09-15 18:55:42
from flask import Flask, session, redirect, url_for, request, render_template, jsonify
import core, couch, formencode, gearman
...
try:
    ...
    print e
    app.config.update(DEBUG=True, GEARMAN_SERVERS='localhost:4730', SMTP_STRING='localhost|25', COUCHDB_CONN='127.0.0.1:', COUCH_CONN='http://127.0.0.1:5984', COUCH_DB='ponyexpress')
...
def index():
    ...
    return render_template('index.html', session=session, gm_status=gm_status, gm_version=gm_version, gm_workers=gm_workers, gm_ping=gm_ping)
...
def create_template():
    ...
    lc = couch.LocalTemplate(lang=form.get('lang'), subject=form.get('subject'), body=form.get('body'))
    doc = couch.PonyExpressTemplate(_id=form.get('_id'), name=form.get('name'), format=form.get('format'), contents=[
     lc])
    doc.save()
    ...
    try:
        ...
        for (k, v) in form.iteritems():
            if not k == 'contents':
                ...
                for content in v:
                    if all([ content.get(key, None) for key in ('lang', 'subject',
                                                                'body') ]):
...
def list_templates():
    ...
    return render_template('list_templates.html', results=view.all())
