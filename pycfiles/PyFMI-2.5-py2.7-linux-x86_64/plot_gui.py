...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyfmi/common/plotting/plot_gui.py
...
import fnmatch, re
try:
    import wx, wx.lib.agw.customtreectrl as wxCustom, wx.lib.agw.aui as aui
except ImportError:
...
class MainGUI(wx.Frame):
    ...
    def __init__(self, parent, ID, filename=None):
        ...
        wx.Frame.__init__(self, parent, ID, self.title, wx.DefaultPosition, wx.Size(self.sizeHeightDefault, self.sizeLengthDefault))
        self.ResultFiles = []
        self.PlotVariables = [[]]
        self.ResultIndex = 0
        self.PlotIndex = 0
        ...
        self.plotPanels = [
         PlotPanel(self.noteBook, self.grid, move=self.move, zoom=self.zoom)]
        ...
        self.mainSizer.Add(self.verticalSplitter, 1, wx.EXPAND)
        self.SetSizer(self.mainSizer)
        self.SetMinSize((self.sizeHeightMin, self.sizeLengthMin))
    ...
    def OnMenuMove(self, event):
        ...
        for i in range(self.noteBook.GetPageCount()):
            self.noteBook.GetPage(i).UpdateSettings(move=self.move, zoom=self.zoom)
    ...
    def OnMenuOpen(self, event):
        dlg = wx.FileDialog(self, 'Open result file(s)', wildcard='Supported files (.txt, .mat, .csv)|*.txt;*.mat;*.csv|Text files (.txt)|*.txt|MATLAB files (.mat)|*.mat|Comma-Separated Values files (.csv)|*.csv|All files (*.*)|*.*', style=wx.FD_MULTIPLE)
    ...
    def OnMenuSaveFig(self, event):
        dlg = wx.FileDialog(self, 'Choose a filename to save to', wildcard='Portable Network Graphics (*.png)|*.png|Encapsulated Postscript (*.eps)|*.eps|Enhanced Metafile (*.emf)|*.emf|Portable Document Format (*.pdf)|*.pdf|Postscript (*.ps)|*.ps|Raw RGBA bitmap (*.raw *.rgba)|*.raw;*.rgba|Scalable Vector Graphics (*.svg *.svgz)|*.svg;*.svgz', style=wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT)
        if dlg.ShowModal() == wx.ID_OK:
            self.SetStatusText('Saving figure...')
            IDPlot = self.noteBook.GetSelection()
    ...
    def OnMenuAxisLabels(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            plotWindow.UpdateSettings(axes=[xmin, xmax, ymin, ymax], title=title, xlabel=xlabel, ylabel=ylabel, xscale=xscale, yscale=yscale)
    ...
    def OnCloseTab(self, event):
        ...
        self.PlotVariables.pop(event.GetSelection())
        self.plotPanels.pop(event.GetSelection())
    ...
    def _OpenFile(self, filename):
        ...
        if n.find('\\') > n.find('/'):
            ...
            res_name = n.split('/')[(-1)]
        failToLoad = False
        self.SetStatusText('Loading ' + n + '...')
        if n.lower().endswith('.txt'):
            try:
                self.ResultFiles.append((res_name, ResultDymolaTextual(n)))
            except (JIOError, IOError):
                self.SetStatusText('Could not load ' + n + '.')
                failToLoad = True

        elif n.lower().endswith('.mat'):
            try:
            ...
            try:
                ...
                self.SetStatusText('Could not load ' + n + ". Trying with delimiter ','.")
                try:
        ...
        if failToLoad:
            ...
            self.tree.AddTreeNode(self.ResultFiles[(-1)][1], self.ResultFiles[(-1)][0], self.filterPanel.checkBoxTimeVarying.GetValue(), self.filterPanel.checkBoxParametersConstants.GetValue(), self.filterPanel.GetFilter())
            self.ResultIndex += 1
            self.SetStatusText('')
...
class VariableTree(wxCustom.CustomTreeCtrl):
        ...
        if showTimeVarying == False or showParametersConstants == False or filter != None:
            while child != itemParent and child != None:
                ...
                if found_child in top_siblings:
                    print (
                     'Found child in top siblings, ', self.GetItemText(found_child))
                    continue
                if data == None:
                    print (
                     'Found (wrong) child:', self.GetItemText(found_child))
                    raise Exception
                try:
                    data['timevarying']
                except KeyError:
                    print (
                     'Found (wrong (exception)) child:', self.GetItemText(found_child))
                    raise Exception
    ...
    def AddHiddenItems(self, showTimeVarying=None, showParametersConstants=None, filter=None):
        ...
        while i < len(self.hidden_children):
            ...
            if data['timevarying'] and showTimeVarying == True and (filter == None or filter != None and matching == True) or not data['timevarying'] and showParametersConstants == True and (filter == None or filter != None and matching == True):
                ...
                if data['item_checked'] is not None:
...
class DialogLinesLegends(wx.Dialog):
    ...
    def __init__(self, parent, plotPage):
        ...
        legendPositions = [
         'Hide', 'Best', 'Upper Right', 'Upper Left', 'Lower Left', 'Lower Right', 'Right', 'Center Left', 'Center Right', 'Lower Center', 'Upper Center', 'Center']
...
class DialogAxisLabels(wx.Dialog):
    ...
    def __init__(self, parent, plotPage):
        ...
        self.plotYAxisMin = wx.TextCtrl(self, -1, '' if settings['YAxisMin'] == None else str(settings['YAxisMin']), style=wx.TE_LEFT, size=(150,
                                                                                                                                             -1))
        self.plotYAxisMax = wx.TextCtrl(self, -1, '' if settings['YAxisMax'] == None else str(settings['YAxisMax']), style=wx.TE_LEFT, size=(150,
                                                                                                                                             -1))
        self.plotXAxisMin = wx.TextCtrl(self, -1, '' if settings['XAxisMin'] == None else str(settings['XAxisMin']), style=wx.TE_LEFT, size=(150,
                                                                                                                                             -1))
        self.plotXAxisMax = wx.TextCtrl(self, -1, '' if settings['XAxisMax'] == None else str(settings['XAxisMax']), style=wx.TE_LEFT, size=(150,
                                                                                                                                             -1))
        self.plotTitle = wx.TextCtrl(self, -1, settings['Title'], style=wx.TE_LEFT, size=(150,
                                                                                          -1))
        self.plotXLabel = wx.TextCtrl(self, -1, settings['XLabel'], style=wx.TE_LEFT, size=(150,
                                                                                            -1))
        self.plotYLabel = wx.TextCtrl(self, -1, settings['YLabel'], style=wx.TE_LEFT, size=(150,
                                                                                            -1))
    ...
    def GetValues(self):
        ...
        return (
         xmax, xmin, ymax, ymin, title, xlabel, ylabel, xscale, yscale)
...
class FilterPanel(wx.Panel):
    ...
    def __init__(self, main, parent, tree, **kwargs):
        ...
        self.checkBoxParametersConstants = wx.CheckBox(self, -1, ' Parameters / Constants')
        self.checkBoxTimeVarying = wx.CheckBox(self, -1, ' Time-Varying', size=(140,
                                                                                -1))
...
class PlotPanel(wx.Panel):
    ...
    def SetDefaultSettings(self):
        ...
        self.settings['LegendPosition'] = 0
        return
    ...
    def DrawZoom(self):
        try:
            ...
            self.DrawRectZoom(drawNew=False)
            self.DrawSettings()
    ...
    def DrawRectZoom(self, drawNew=True):
        ...
        try:
            ...
            dc.DrawRectangle(*self._lastZoomRect)
    ...
    def UpdateSettings(self, grid=None, title=None, xlabel=None, ylabel=None, axes=None, move=None, zoom=None, xscale=None, yscale=None, legendposition=None):
        """
