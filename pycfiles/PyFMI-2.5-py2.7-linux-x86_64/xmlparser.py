...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyfmi/common/xmlparser.py
...
def _translate_variable_category(category):
    ...
    if category == 'state':
        return STATE
    if category == 'dependentConstant':
        return DEPENDENT_CONSTANT
    if category == 'independentConstant':
        return INDEPENDENT_CONSTANT
    if category == 'dependentParameter':
        return DEPENDENT_PARAMETER
    if category == 'independentParameter':
...
def _translate_fundamental_type(type):
    ...
    if isinstance(type, Enumeration):
        return ENUMERATION
    raise XMLException('Unknown type for variable: ' + str(type))
...
def _parse_XML(filename, schemaname=''):
    ...
    if schemaname:
        ...
        if not result:
            raise XMLException('The XML file: %s is not valid                 according to the XMLSchema: %s.' % (
             filename, schemaname))
    return element_tree
...
class ModelDescription():
    ...
    def _fill_attributes(self, root):
        ...
        """
        self._attributes = {'fmiVersion': '', 'modelName': '', 
           'modelIdentifier': '', 
           'guid': '', 
           'description': '', 
           'author': '', 
    ...
    def get_value_reference(self, variablename, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_value_reference', variablename)
        else:
            sv = self._model_variables_dict.get(variablename)
            if sv != None:
                return sv.get_value_reference()
            raise XMLException('Variable: ' + str(variablename) + ' was not found             in the model.')
            return
    ...
    def is_alias(self, variablename, ignore_cache=False):
        ...
        if not ignore_cache:
            ...
            raise XMLException('Variable: ' + str(variablename) + ' was not found             in model.')
            return
    ...
    def get_aliases_for_variable(self, variablename, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_aliases_for_variable', variablename)
        else:
            aliasnames = []
            isnegated = []
            ...
            if variable != None:
                for sv in self.get_model_variables():
                    if sv.get_value_reference() == variable.get_value_reference() and sv.get_name() != variablename:
                        aliasnames.append(sv.get_name())
                        isnegated.append(sv.get_alias() == NEGATED_ALIAS)

                return (
                 aliasnames, isnegated)
            return
    ...
    def get_variability(self, variablename, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_variability', variablename)
        else:
            sv = self._model_variables_dict.get(variablename)
            if sv != None:
    ...
    def get_variable_nominal_attributes(self, include_alias=True, ignore_cache=False):
        """ 
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_variable_nominal_attributes', include_alias)
        else:
            nominals = []
            scalarvariables = self.get_model_variables()
            if include_alias:
    ...
    def get_variable_fixed_attributes(self, include_alias=True, ignore_cache=False):
        """ 
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_variable_fixed_attributes', include_alias)
        else:
            fixeds = []
            scalarvariables = self.get_model_variables()
            if include_alias:
    ...
    def get_variable_start_attributes(self, include_alias=True, ignore_cache=False):
        """ 
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_variable_start_attributes', include_alias)
        start_attributes = []
        scalarvariables = self.get_model_variables()
        if include_alias:
            for sv in scalarvariables:
    ...
    def get_all_real_variables(self, include_alias=True, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_all_real_variables', include_alias)
    ...
    def get_all_string_variables(self, include_alias=True, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_all_string_variables', include_alias)
    ...
    def get_all_integer_variables(self, include_alias=True, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_all_integer_variables', include_alias)
    ...
    def get_all_boolean_variables(self, include_alias=True, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_all_boolean_variables', include_alias)
    ...
    def _get_all_variables(self, type, include_alias):
        ...
        if include_alias:
            ...
            return typevars
        for sv in scalarvariables:
            if isinstance(sv.get_fundamental_type(), type) and sv.get_alias() == NO_ALIAS:
                typevars.append(sv)
    ...
    def get_p_opt_variable_names(self, include_alias=True, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_p_opt_variable_names', include_alias)
        vrefs = []
        names = []
        scalarvariables = self.get_model_variables()
        if include_alias:
            for sv in scalarvariables:
                if sv.get_variability() == PARAMETER and sv.get_fundamental_type().get_free() == True:
                    vrefs.append(sv.get_value_reference())
                    names.append(sv.get_name())

            return zip(tuple(vrefs), tuple(names))
        for sv in scalarvariables:
            if sv.get_alias() == NO_ALIAS and sv.get_variability() == PARAMETER and sv.get_fundamental_type().get_free() == True:
                vrefs.append(sv.get_value_reference())
                names.append(sv.get_name())
    ...
    def get_dx_variable_names(self, include_alias=True, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_dx_variable_names', include_alias)
        vrefs = []
        names = []
        scalarvariables = self.get_model_variables()
        if include_alias:
            ...
            return zip(tuple(vrefs), tuple(names))
        for sv in scalarvariables:
            if sv.get_alias() == NO_ALIAS and sv.get_variable_category() == DERIVATIVE:
                vrefs.append(sv.get_value_reference())
                names.append(sv.get_name())
    ...
    def get_x_variable_names(self, include_alias=True, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_x_variable_names', include_alias)
        vrefs = []
        names = []
        scalarvariables = self.get_model_variables()
        if include_alias:
            ...
            return zip(tuple(vrefs), tuple(names))
        for sv in scalarvariables:
            if sv.get_alias() == NO_ALIAS and sv.get_variable_category() == STATE:
                vrefs.append(sv.get_value_reference())
                names.append(sv.get_name())
    ...
    def get_u_variable_names(self, include_alias=True, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_u_variable_names', include_alias)
        vrefs = []
        names = []
        scalarvariables = self.get_model_variables()
        if include_alias:
            for sv in scalarvariables:
                if sv.get_causality() == INPUT and sv.get_variable_category() == ALGEBRAIC:
                    vrefs.append(sv.get_value_reference())
                    names.append(sv.get_name())

            return zip(tuple(vrefs), tuple(names))
        for sv in scalarvariables:
            if sv.get_alias() == NO_ALIAS and sv.get_causality() == INPUT and sv.get_variable_category() == ALGEBRAIC:
                vrefs.append(sv.get_value_reference())
                names.append(sv.get_name())
    ...
    def get_w_variable_names(self, include_alias=True, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_w_variable_names', include_alias)
        vrefs = []
        names = []
        scalarvariables = self.get_model_variables()
        if include_alias:
    ...
    def get_p_opt_start(self, include_alias=True, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_p_opt_start', include_alias)
        vrefs = []
        start_attributes = []
        scalarvariables = self.get_model_variables()
        if include_alias:
    ...
    def get_p_opt_nominal(self, include_alias=True, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_p_opt_nominal', include_alias)
        vrefs = []
        nominal_attributes = []
        scalarvariables = self.get_model_variables()
        if include_alias:
            for sv in scalarvariables:
                ftype = sv.get_fundamental_type()
                if sv.get_variability() == PARAMETER and ftype.get_free() == True:
                    vrefs.append(sv.get_value_reference())
        ...
        for sv in scalarvariables:
            ftype = sv.get_fundamental_type()
            if sv.get_alias() == NO_ALIAS and sv.get_variability() == PARAMETER and ftype.get_free() == True:
                vrefs.append(sv.get_value_reference())
                nominal_attributes.append(ftype.get_nominal())
    ...
    def get_p_opt_initial_guess(self, include_alias=True, ignore_cache=False):
        ...
        if include_alias:
            for sv in scalarvariables:
                ftype = sv.get_fundamental_type()
                if not isinstance(ftype, String) and not isinstance(ftype, Enumeration) and sv.get_variability() == PARAMETER and ftype.get_free() == True:
                    vrefs.append(sv.get_value_reference())
        ...
        for sv in scalarvariables:
            ftype = sv.get_fundamental_type()
            if not isinstance(ftype, String) and not isinstance(ftype, Enumeration) and sv.get_alias() == NO_ALIAS and sv.get_variability() == PARAMETER and ftype.get_free() == True:
                vrefs.append(sv.get_value_reference())
                initial_values.append(ftype.get_initial_guess())
    ...
    def get_dx_initial_guess(self, include_alias=True, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_dx_initial_guess', include_alias)
        vrefs = []
        initial_values = []
        scalarvariables = self.get_model_variables()
        if include_alias:
            for sv in scalarvariables:
                ftype = sv.get_fundamental_type()
                if not isinstance(ftype, String) and not isinstance(ftype, Enumeration) and sv.get_variable_category() == DERIVATIVE:
                    vrefs.append(sv.get_value_reference())
                    initial_values.append(ftype.get_initial_guess())
        ...
        for sv in scalarvariables:
            ftype = sv.get_fundamental_type()
            if not isinstance(ftype, String) and not isinstance(ftype, Enumeration) and sv.get_alias() == NO_ALIAS and sv.get_variable_category() == DERIVATIVE:
                vrefs.append(sv.get_value_reference())
                initial_values.append(ftype.get_initial_guess())
    ...
    def get_x_initial_guess(self, include_alias=True, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_x_initial_guess', include_alias)
        vrefs = []
        initial_values = []
        scalarvariables = self.get_model_variables()
        if include_alias:
            for sv in scalarvariables:
                ftype = sv.get_fundamental_type()
                if not isinstance(ftype, String) and not isinstance(ftype, Enumeration) and sv.get_variable_category() == STATE:
                    vrefs.append(sv.get_value_reference())
                    initial_values.append(ftype.get_initial_guess())
        ...
        for sv in scalarvariables:
            ftype = sv.get_fundamental_type()
            if not isinstance(ftype, String) and not isinstance(ftype, Enumeration) and sv.get_alias() == NO_ALIAS and sv.get_variable_category() == STATE:
                vrefs.append(sv.get_value_reference())
                initial_values.append(ftype.get_initial_guess())
    ...
    def get_u_initial_guess(self, include_alias=True, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_u_initial_guess', include_alias)
        vrefs = []
        initial_values = []
        scalarvariables = self.get_model_variables()
        if include_alias:
            for sv in scalarvariables:
                ftype = sv.get_fundamental_type()
                if not isinstance(ftype, String) and not isinstance(ftype, Enumeration) and sv.get_causality() == INPUT and sv.get_variable_category() == ALGEBRAIC:
                    vrefs.append(sv.get_value_reference())
                    initial_values.append(ftype.get_initial_guess())
        ...
        for sv in scalarvariables:
            ftype = sv.get_fundamental_type()
            if not isinstance(ftype, String) and not isinstance(ftype, Enumeration) and sv.get_alias() == NO_ALIAS and sv.get_causality() == INPUT and sv.get_variable_category() == ALGEBRAIC:
                vrefs.append(sv.get_value_reference())
                initial_values.append(ftype.get_initial_guess())
    ...
    def get_w_initial_guess(self, include_alias=True, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_w_initial_guess', include_alias)
        vrefs = []
        initial_values = []
        scalarvariables = self.get_model_variables()
        if include_alias:
    ...
    def get_p_opt_min(self, include_alias=True, ignore_cache=False):
        ...
        if include_alias:
            for sv in scalarvariables:
                ftype = sv.get_fundamental_type()
                if not isinstance(ftype, String) and not isinstance(ftype, Boolean) and sv.get_variability() == PARAMETER and ftype.get_free() == True:
                    vrefs.append(sv.get_value_reference())
        ...
        for sv in scalarvariables:
            ftype = sv.get_fundamental_type()
            if not isinstance(ftype, String) and not isinstance(ftype, Boolean) and sv.get_alias() == NO_ALIAS and sv.get_variability() == PARAMETER and ftype.get_free() == True:
                vrefs.append(sv.get_value_reference())
                min_values.append(ftype.get_min())
    ...
    def get_dx_min(self, include_alias=True, ignore_cache=False):
        ...
        if include_alias:
            for sv in scalarvariables:
                ftype = sv.get_fundamental_type()
                if not isinstance(ftype, String) and not isinstance(ftype, Boolean) and sv.get_variable_category() == DERIVATIVE:
                    vrefs.append(sv.get_value_reference())
                    min_values.append(ftype.get_min())
        ...
        for sv in scalarvariables:
            ftype = sv.get_fundamental_type()
            if not isinstance(ftype, String) and not isinstance(ftype, Boolean) and sv.get_alias() == NO_ALIAS and sv.get_variable_category() == DERIVATIVE:
                vrefs.append(sv.get_value_reference())
                min_values.append(ftype.get_min())
    ...
    def get_x_min(self, include_alias=True, ignore_cache=False):
        ...
        if include_alias:
            for sv in scalarvariables:
                ftype = sv.get_fundamental_type()
                if not isinstance(ftype, String) and not isinstance(ftype, Boolean) and sv.get_variable_category() == STATE:
                    vrefs.append(sv.get_value_reference())
                    min_values.append(ftype.get_min())
        ...
        for sv in scalarvariables:
            ftype = sv.get_fundamental_type()
            if not isinstance(ftype, String) and not isinstance(ftype, Boolean) and sv.get_alias() == NO_ALIAS and sv.get_variable_category() == STATE:
                vrefs.append(sv.get_value_reference())
                min_values.append(ftype.get_min())
    ...
    def get_u_min(self, include_alias=True, ignore_cache=False):
        ...
        if include_alias:
            for sv in scalarvariables:
                ftype = sv.get_fundamental_type()
                if not isinstance(ftype, String) and not isinstance(ftype, Boolean) and sv.get_causality() == INPUT and sv.get_variable_category() == ALGEBRAIC:
                    vrefs.append(sv.get_value_reference())
                    min_values.append(ftype.get_min())
        ...
        for sv in scalarvariables:
            ftype = sv.get_fundamental_type()
            if not isinstance(ftype, String) and not isinstance(ftype, Boolean) and sv.get_alias() == NO_ALIAS and sv.get_causality() == INPUT and sv.get_variable_category() == ALGEBRAIC:
                vrefs.append(sv.get_value_reference())
                min_values.append(ftype.get_min())
    ...
    def get_dx_islinear(self, include_alias=True, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_dx_islinear', include_alias)
        vrefs = []
        is_linear = []
        scalarvariables = self.get_model_variables()
        if include_alias:
    ...
    def get_x_islinear(self, include_alias=True, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_x_islinear', include_alias)
        vrefs = []
        is_linear = []
        scalarvariables = self.get_model_variables()
        if include_alias:
    ...
    def get_u_islinear(self, include_alias=True, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_u_islinear', include_alias)
        vrefs = []
        is_linear = []
        scalarvariables = self.get_model_variables()
        if include_alias:
    ...
    def get_w_islinear(self, include_alias=True, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_w_islinear', include_alias)
        vrefs = []
        is_linear = []
        scalarvariables = self.get_model_variables()
        if include_alias:
    ...
    def get_dx_linear_timed_variables(self, include_alias=True, ignore_cache=False):
        """ 
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_dx_linear_timed_variables', include_alias)
        tot_timepoints = []
        scalarvariables = self.get_model_variables()
        if include_alias:
            for sv in scalarvariables:
    ...
    def get_x_linear_timed_variables(self, include_alias=True, ignore_cache=False):
        """ 
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_x_linear_timed_variables', include_alias)
        tot_timepoints = []
        scalarvariables = self.get_model_variables()
        if include_alias:
            for sv in scalarvariables:
    ...
    def get_u_linear_timed_variables(self, include_alias=True, ignore_cache=False):
        """ 
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_u_linear_timed_variables', include_alias)
        tot_timepoints = []
        scalarvariables = self.get_model_variables()
        if include_alias:
            for sv in scalarvariables:
    ...
    def get_w_linear_timed_variables(self, include_alias=True, ignore_cache=False):
        """ 
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_w_linear_timed_variables', include_alias)
        tot_timepoints = []
        scalarvariables = self.get_model_variables()
        if include_alias:
            for sv in scalarvariables:
    ...
    def get_p_opt_value_reference(self, ignore_cache=False):
        ...
        """
        if not ignore_cache:
            return self.function_cache.get(self, 'get_p_opt_value_reference', None)
        else:
            vrefs = []
            for sv in self.get_model_variables():
                if sv.get_variability() == PARAMETER and sv.get_fundamental_type().get_free() == True:
...
class DisplayUnitDefinition():
    ...
    def __init__(self, element):
        ...
        copy.
        """
        self._attributes = {'displayUnit': '', 'gain': '1.0', 
           'offset': '0.0'}
        self._attributes.update(element.attrib)
...
class Type():
    ...
    def __init__(self, element):
        ...
        """
        self._attributes = {'name': '', 'description': ''}
        self._attributes.update(element.attrib)
...
class RealType():
    ...
    def __init__(self, element):
        ...
        """
        self._attributes = {'quantity': '', 'unit': '', 
           'displayUnit': '', 
...
class DefaultExperiment():
    ...
    def __init__(self, element):
        ...
        """
        self._attributes = {'startTime': '', 'stopTime': '', 
           'tolerance': ''}
        self._attributes.update(element.attrib)
...
class ScalarVariable():
    ...
    def __init__(self, element):
        ...
        """
        self._attributes = {'name': '', 'valueReference': '', 
           'description': '', 
        ...
        if e_ftype.tag == 'Real':
            ...
            raise XMLException('ScalarVariable: ' + self._attributes['name'] + ' does not have a valid fundamental type.')
...
class Real():
    ...
    def __init__(self, element):
        ...
        """
        self._attributes = {'declaredType': '', 'quantity': '', 
           'unit': '', 
           'displayUnit': '', 
           'relativeQuantity': 'false', 
           'min': '', 
...
class Boolean():
    ...
    def __init__(self, element):
        ...
        """
        self._attributes = {'declaredType': '', 'start': '', 
           'fixed': '', 
           'free': '', 
           'initialGuess': ''}
        self._attributes.update(element.attrib)
...
class TimePoint():
    ...
    def __init__(self, element):
        ...
        """
        self._attributes = {'index': '', 'isLinear': ''}
        self._attributes.update(element.attrib)
