...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/zopyx/slimp/slimplayer.py
# Compiled at: 2007-06-10 09:06:13
...
def main():
    parser = OptionParser()
    parser.add_option('-d', '--rootdir', dest='rootdir', default='/raid/media2/media/mp3', help='root directory')
    parser.add_option('-i', '--init-playlist', dest='init', action='store_true', help='init the playlist')
    parser.add_option('-s', '--scan-mp3', dest='scanmp3', action='store_true', help='rescan MP3 directory')
    parser.add_option('-n', '--min-tracks', dest='min_tracks', type='int', help='minimal number of elements in playlist', default=5)
    ...
    if files:
        ...
        for i in range(0, current_track):
            slim.playlist_delete(0)
