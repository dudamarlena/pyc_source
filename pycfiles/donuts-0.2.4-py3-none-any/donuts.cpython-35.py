...
# [GCC 8.4.0]
# Embedded file name: /Users/jmcc/Dropbox/PythonScripts/astropy/Donuts/build/lib.macosx-10.12-x86_64-3.5/donuts/donuts.py
# Compiled at: 2017-10-23 11:59:15
...
class Donuts(object):
    __doc__ = 'This class provides methods for measuring shifts between\n    a series of images of the same star field. First we initialise\n    the object and generate a reference image. Subsequent images are\n    aligned to this frame of this reference image.\n\n    Attributes\n    ----------\n    None\n    '

    def __init__(self, refimage, image_ext=0, exposure='EXPTIME', normalise=True, subtract_bkg=True, prescan_width=0, overscan_width=0, scan_direction='x', border=64, ntiles=32, image_class=Image):
    ...
    def construct_object(self, filename):
        ...
        image.trim(prescan_width=self.prescan_width, overscan_width=self.overscan_width, scan_direction=self.scan_direction, border=self.border)
        if self.normalise:
            image.normalise(exposure_keyword=self.exposure_keyname)
        if self.subtract_bkg:
            image.remove_background(ntiles=self.ntiles)
