...
# [GCC 8.4.0]
# Embedded file name: /Users/jmcc/Dropbox/PythonScripts/astropy/Donuts/build/lib.macosx-10.12-x86_64-3.5/donuts/image.py
# Compiled at: 2017-10-23 11:42:37
# Size of source mod 2**32: 13879 bytes
import numpy as np
...
class Image(object):
    __doc__ = 'Low level class which handles the image transformations and cross\n    correlation with another :class:`~donuts.image.Image` class.\n    '
    ...
    def normalise(self, exposure_keyword='EXPOSURE'):
        ...
        try:
            ...
            log.warning('Exposure time keyword "{0}" not found, assuming 1.0'.format(exposure_keyword))
            self.exposure_time_value = 1.0

        if self.raw_region is None:
            raise RuntimeError('Image region has not been computed.Please ensure the `#trim` method has been called')
        self.raw_region = self.raw_region / self.exposure_time_value
        return self

    def trim(self, prescan_width=0, overscan_width=0, scan_direction='x', border=64):
    ...
    def remove_background(self, ntiles=32):
        ...
        self.sky_background = self._generate_bkg_map(data=self.raw_region, tile_num=ntiles, tilesizex=tilesize_x, tilesizey=tilesize_y)
    ...
    def compute_projections(self):
        ...
        * otherwise use the background subtracted pixels
        ...
        if self.backsub_region is None and self.raw_region is None:
            ...
            if self.backsub_region is None:
                region = self.raw_region
            else:
                region = self.backsub_region
            if not len(region.shape) == 2:
    ...
    def _assert_projections(self):
        ...
        if self.proj_x is None or self.proj_y is None:
            raise ValueError('Projections for %s have not been computed. Please call the #compute_projections method' % self)
    ...
    def _find_solution(self, z_pos, phi_ref_check_m):
        ...
        if z_pos[0][0] <= len(phi_ref_check_m) / 2 and z_pos[0][0] != 0:
            lra = [
             z_pos[0][0] - 1, z_pos[0][0], z_pos[0][0] + 1]
            ...
            if z_pos[0][0] > len(phi_ref_check_m) / 2 and z_pos[0][0] != len(phi_ref_check_m) - 1:
                lra = [
                 z_pos[0][0] - 1, z_pos[0][0], z_pos[0][0] + 1]
                tst[0] = phi_ref_check_m[lra[0]].real
                tst[1] = phi_ref_check_m[lra[1]].real
                ...
                if z_pos[0][0] == len(phi_ref_check_m) - 1:
                    lra = [
                     -1, 0, 1]
    ...
    def _generate_bkg_map(self, data, tile_num, tilesizex, tilesizey):
        ...
        try:
            bkgmap = resize(coarse, (
             tilesizey * tile_num, tilesizex * tile_num), mode='edge')
        except ValueError:
            bkgmap = resize(coarse, (
             tilesizey * tile_num, tilesizex * tile_num), mode='nearest')
