...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/qtipy/config.py
# Compiled at: 2014-05-14 17:14:30
from __future__ import unicode_literals
import logging
logging.debug(b'Loading config.py')
from .qt import *
from .utils import CONVERT_TYPE_TO_XML, CONVERT_TYPE_FROM_XML
import os, sys, re, base64, numpy as np, types
...
class ConfigManager(QObject):
    ...
    def set_many(self, keyvalues, trigger_update=True):
        ...
        for k, v in list(keyvalues.items()):
            ...
            logging.debug(b'Workflow config; setting %s to %s' % (k, v))
    ...
    def add_handler(self, key, handler, mapper=(
 lambda x: x, lambda x: x)):
        ...
        """
        if type(mapper) == dict or type(mapper) == OrderedDict:
        ...
        if key in self.handlers:
            ...
            handler.setter = types_MethodType(globals().get(b'_set_%s' % type(handler).__name__), handler, type(handler))
            handler.getter = types_MethodType(globals().get(b'_get_%s' % type(handler).__name__), handler, type(handler))
            handler.updater = types_MethodType(globals().get(b'_event_%s' % type(handler).__name__), handler, type(handler))
            print b'Add handler %s for %s' % (type(handler).__name__, key)
    ...
    def getXMLConfig(self, root):
        config = et.SubElement(root, b'Config')
        for ck, cv in list(self.config.items()):
            co = et.SubElement(config, b'ConfigSetting')
            co.set(b'id', ck)
            t = type(cv).__name__
            co.set(b'type', type(cv).__name__)
    ...
    def setXMLConfig(self, root):
        config = {}
        for xconfig in root.findall(b'Config/ConfigSetting'):
            if xconfig.get(b'type') in CONVERT_TYPE_FROM_XML:
                v = CONVERT_TYPE_FROM_XML[xconfig.get(b'type')](xconfig)
            config[xconfig.get(b'id')] = v
