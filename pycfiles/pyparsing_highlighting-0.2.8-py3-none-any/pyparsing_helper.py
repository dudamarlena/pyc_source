...
# Embedded file name: build/bdist.linux-i686/egg/pyparsing_helper.py
# Compiled at: 2009-01-14 14:37:16
...
import pyparsing, time, threading, functools, string, optparse, sys
__version__ = '0.1.2'
optparser = optparse.OptionParser()
optparser.add_option('-n', '--num-targets', help='Number of target panes to display', dest='num_targets', default=3, action='store', type='int')
optparser.add_option('-l', '--lines', help='Height of grammar pane (in lines)', dest='grammar_height', default=40, action='store', type='int')
optparser.add_option('-w', '--width', help='Width of pane (in characters)', dest='grammar_width', default=40, action='store', type='int')
...
class Application(Frame):
    ...
    def __init__(self, master=None, grammar_width=40, grammar_height=40, num_targets=3, grammar=None):
    ...
    def createWidgets(self):
        ...
        self.import_type_button1 = Radiobutton(self, variable=self.import_type, command=self.reparse, value='from pyparsing import *', text='from pyparsing import *')
        self.import_type_button2 = Radiobutton(self, variable=self.import_type, command=self.reparse, value='import pyparsing', text='import pyparsing')
        ...
        self.parse_type_button1 = Radiobutton(self, variable=self.parse_type, value='parse', text='parse', command=self.reparse)
        self.parse_type_button2 = Radiobutton(self, variable=self.parse_type, value='scan', text='scan/search', command=self.reparse)
        self.parse_type_button3 = Radiobutton(self, variable=self.parse_type, value='transform', text='transform', command=self.reparse)
        ...
        for target_num in range(0, self.num_targets):
            ...
            result = Text(self, height=self.grammar_height / self.num_targets, width=self.grammar_width, takefocus=0, yscrollcommand=scrl.set)
