...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/featureflow/test_eventlog.py
# Compiled at: 2020-03-01 20:21:49
...
class EventLogTests(unittest2.TestCase):
    ...
    def test_only_returns_events_greater_than_last_id(self):
        self.Model.process(stream='Bah bah black sheep')
        events = list(self.Settings.event_log.subscribe(last_id=b'',
          raise_when_empty=True))
        last_id, _ = events[(-1)]
        self.Model.process(stream='Humpty dumpty sat on a wall')
        next_events = list(self.Settings.event_log.subscribe(last_id=last_id,
          raise_when_empty=True))
    ...
    def test_events_are_logged_when_event_log_is_configured(self):
        ...
        subscription = self.Settings.event_log.subscribe(last_id='',
          raise_when_empty=True)
