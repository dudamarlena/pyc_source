...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/featureflow/model.py
...
class MetaModel(type):
    ...
    def _ensure_persistence_settings(cls):
        if not issubclass(cls, PersistenceSettings):
            raise NoPersistenceSettingsError('The class {cls} is not a PersistenceSettings subclass'.format(cls=(cls.__name__)))
...
class BaseModel(object, metaclass=MetaModel):
    ...
    def exists(cls, _id=None, feature=None):
        if feature:
            if not feature.store:
                raise ValueError('feature must have store=True')
        if _id is None:
            try:
                _id = cls.id_provider.new_id()
            except:
                raise ValueError('_id must be provided explicitly, or it must be static')
    ...
    def process(cls, raise_if_exists=False, **kwargs):
        ...
        if raise_if_exists:
            if cls.exists(_id):
                raise ModelExistsError(('{_id} is already stored in the database'.format)(**locals()))
        try:
            del kwargs['_id']
        except KeyError:
            pass
