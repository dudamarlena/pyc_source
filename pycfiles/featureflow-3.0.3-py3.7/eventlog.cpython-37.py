...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/featureflow/eventlog.py
# Compiled at: 2020-03-01 20:30:48
# Size of source mod 2**32: 3193 bytes
import lmdb, binascii, os, time, redis, json
...
class RedisChannel(object):
    ...
    def subscribe(self, raise_when_empty=False):
        if raise_when_empty:
            raise NotImplementedError('raise_when_empty=True is not supported for RedisChannel')
    ...
    def publish(self, _id, message):
        self.r.publish(self.channel, json.dumps({'_id':_id,  'message':message}))
...
class EventLog(object):
    ...
    def append(self, data):
        with self.env.begin(write=True) as (txn):
            _id = hex(int(time.time() * 1000000.0)).encode() + binascii.hexlify(os.urandom(8))
