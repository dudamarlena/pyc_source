...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/featureflow/feature.py
# Compiled at: 2019-03-01 22:03:23
# Size of source mod 2**32: 13242 bytes
from .var import Var
from collections import OrderedDict
import inspect
from .datawriter import DataWriter, BytesIODataWriter, ClobberDataWriter
from .decoder import JSONDecoder, Decoder, GreedyTextDecoder, DecoderNode, BZ2Decoder, PickleDecoder
...
class Feature(object):
    ...
    def __init__(self, extractor, needs=None, store=False, encoder=None, decoder=None, key=None, data_writer=None, persistence=None, **extractor_args):
    ...
    def _fixup_needs(self):
        ...
        for k, v in list(self.needs.items()):
            try:
                ...
                class AspectExtractor(KeySelector):
    ...
    def _handle_callable_extractor(self):
        if inspect.isclass(self.extractor):
            if issubclass(self.extractor, Node):
                return
        if callable(self.extractor):
            self.extractor_args = dict(func=self.extractor, **self.extractor_args)
            self.extractor = FunctionalNode
            return
        raise ValueError('extractor must be either a Node or a callable')

    def __repr__(self):
        return ('{cls}(key = {key}, store = {store})'.format)(cls=self.__class__.__name__, **self.__dict__)
    ...
    def version(self):
        if isinstance(self.needs, dict):
            dependencies = dict(((k, (v.extractor)(**v.extractor_args)) for k, v in list(self.needs.items())))
        else:
            dependencies = [(f.extractor)(**f.extractor_args) for f in self.needs]
        e = (self.extractor)(needs=dependencies, **self.extractor_args)
        return e.version

    def copy(self, extractor=None, needs=None, store=None, data_writer=None, persistence=None, extractor_args=None):
        """
        Use self as a template to build a new feature, replacing
        values in kwargs
        """
        f = Feature(
 extractor or self.extractor, needs=needs, 
         store=self.store if store is None else store, 
         encoder=self.encoder, 
         decoder=self.decoder, 
         key=self.key, 
         data_writer=data_writer, 
         persistence=persistence, **extractor_args or self.extractor_args)
        f._fixup_needs()
        return f
    ...
    def _can_compute(self, _id, persistence):
        ...
        """
        if self.store:
            if self._stored(_id, persistence):
                return True
        if self.is_root:
            return False
        return all([n._can_compute(_id, persistence) for n in self.dependencies])
    ...
    def _build_partial(self, _id, persistence):
        ...
        for feat in list(features.values()):
            ...
            if feat.key == self.key:
                stream = e.find_listener(lambda x: isinstance(x, BytesIODataWriter))
    ...
    def _partial(self, _id, features=None, persistence=None):
        ...
        nf = self.copy(extractor=(DecoderNode if is_cached else self.extractor),
          store=(root or should_store),
          needs=(dict()),
          data_writer=data_writer,
          persistence=(self.persistence),
          extractor_args=(dict(decodifier=(self.decoder), version=(self.version)) if is_cached else self.extractor_args))
        if root:
            features = dict()
        ...
        for k, v in list(self.extractor_args.items()):
            if isinstance(v, Var) and v.name == k:
                try:
                    ...
                    raise ValueError(('{k} is a Var, but it was not provided'.format)(**locals()))
...
class CompressedFeature(Feature):
 ...
 extractor, needs=needs, 
         store=store, 
         encoder=BZ2Encoder, 
         decoder=BZ2Decoder(), 
         key=key, **extractor_args)


class PickleFeature(Feature):

    def __init__(self, extractor, needs=None, store=False, key=None, data_writer=None, **extractor_args):
 ...
 extractor, needs=needs, 
         ...
         key=key, 
         data_writer=data_writer, **extractor_args)
...
class ClobberPickleFeature(PickleFeature):
 ...
 extractor, needs=needs, 
         store=store, 
         key=key, 
         data_writer=ClobberDataWriter, **extractor_args)


class JSONFeature(Feature):

    def __init__(self, extractor, needs=None, store=False, key=None, encoder=JSONEncoder, data_writer=None, **extractor_args):
...
class ClobberJSONFeature(JSONFeature):
    ...
    def __init__(self, extractor, needs=None, store=False, key=None, encoder=JSONEncoder, **extractor_args):
