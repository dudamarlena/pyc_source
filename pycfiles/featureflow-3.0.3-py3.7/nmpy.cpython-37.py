...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/featureflow/nmpy.py
# Compiled at: 2020-03-01 20:16:06
...
class PackedNumpyEncoder(NumpyEncoder):
    ...
    def _pack_recarray(self, recarr):
        ...
        for name in list(fields.keys()):
            view = recarr[name].copy().view(np.uint8).reshape(recarr.shape + (-1, ))
...
class StreamingNumpyDecoder(Decoder):
    ...
    def __iter__(self, flo):
        ...
        for chunk in chunked(flo, chunk_size):
            ...
            yield _np_from_buffer(chunk, (
             n_examples,) + metadata.shape, metadata.dtype)
            count += 1

        if count == 0:
            yield _np_from_buffer(memoryview(b''), (0, ) + metadata.shape, metadata.dtype)


class NumpyFeature(Feature):

    def __init__(self, extractor, needs=None, store=False, key=None, encoder=NumpyEncoder, decoder=GreedyNumpyDecoder(), **extractor_args):
