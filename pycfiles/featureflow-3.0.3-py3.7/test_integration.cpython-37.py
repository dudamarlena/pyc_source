...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/featureflow/test_integration.py
# Compiled at: 2019-03-01 22:03:23
# Size of source mod 2**32: 64469 bytes
...
from .feature import Feature, JSONFeature, CompressedFeature, ClobberJSONFeature, ClobberPickleFeature, TextFeature
from .data import *
...
import traceback
data_source = {'mary':b'mary had a little lamb little lamb little lamb', 
...
class CharacterCountNonGeneratorProcessMethod(Node):
    ...
    def __init__(self, needs=None):
        super(CharacterCountNonGeneratorProcessMethod, self).__init__(needs=needs)
...
class Broken(Node):
    ...
    def __init__(self, raise_on_process=True, raise_on_enqueue=False, needs=None):
...
class BaseTest(object):
    ...
    def test_clobber_feature_does_not_delete_previous_iteration_of_data(self):
        settings = self.Settings.clone(id_provider=UserSpecifiedIdProvider(key='_id'))

        class Document(BaseModel, settings):
            stream = Feature(TextStream, store=True)
            words = Feature(Tokenizer, needs=stream, store=False)
            count = ClobberJSONFeature(IncrementalWordCount,
              needs=words,
              store=True,
              raise_for_chunk=2)
        ...
        self.assertTrue(count_key in Document.database, ('Database should still contain {count_key}'.format)(**locals()))
    ...
    def test_can_overwrite_instead_of_append_pickle_feature_data(self):
        ...
        class Document(BaseModel, self.Settings):
            stream = Feature(TextStream, store=True)
            words = Feature(Tokenizer, needs=stream, store=False)
            count = ClobberPickleFeature(IncrementalWordCount,
              needs=words, store=True)
    ...
    def test_can_nest_aspect_call_inside_of_dict_dependency(self):
        ...
        class Document(BaseModel, self.Settings):
            ...
            cat = Feature(ExplicitConcatenate,
    ...
    def test_functional_node_returning_generator_replaces_process_method(self):
        ...
        class Split(BaseModel):
            stream = Feature(TextStream, store=False)
            passthrough = Feature((lambda x: (y.lower() for y in x.decode())),
              needs=stream, store=False)
    ...
    def test_can_check_if_document_exists_using_static_id(self):
        settings = self.Settings.clone(id_provider=(StaticIdProvider('SOME_ID')))
    ...
    def test_existence_check_fails_if_supplied_feature_is_not_stored(self):
        ...
        _id = Document.process(stream='humpty', _id='blah')
        self.assertRaises(ValueError, lambda : Document.exists(_id, Document.count))
    ...
    def test_can_use_node_that_validates_its_dependency_list(self):
        ...
        class D1(BaseModel, self.Settings):
            ...
            words = Feature(Tokenizer, needs=stream, store=False)
            count = JSONFeature(WordCount, needs=words, store=True)
            timestamp = JSONFeature(TimestampEmitter,
              version='1',
              needs=stream,
              store=True)
    ...
    def test_graph_including_non_generator_process_method_raises(self):
        ...
        class D(BaseModel, self.Settings):
            stream = Feature(TextStream, store=True)
            length = Feature(CharacterCountNonGeneratorProcessMethod,
              needs=stream,
              store=True)
            total = Feature(Total, needs=length, store=True)

        self.assertRaises(InvalidProcessMethod, lambda : D.process(stream='mary'))
    ...
    def test_can_use_string_for_remote_url(self):
        ...
        try:
            ...
            self.assertRaises(HTTPError, lambda : D.process(stream=url))
        finally:
            p.kill()
    ...
    def test_can_get_size_in_bytes_of_key(self):
        ...
        key = self.Settings.key_builder.build(_id, D.stream.key, D.stream.version)
        self.assertEqual(len(data_source['mary']), self.Settings.database.size(key))
    ...
    def test_can_use_alternate_decoder_for_stored_feature(self):
        ...
        count_as_text = Doc.count(_id=_id,
          decoder=(Decoder()),
          persistence=(self.Settings)).read()
    ...
    def test_raises_when_processing_same_id_twice(self):
        ...
        self.assertRaises(ModelExistsError, lambda : Document.process(stream='humpty',
          _id='blah',
    ...
    def test_can_aggregate_word_counts_from_multiple_inputs(self):
        ...
        class Contrived(BaseModel, self.Settings):
            ...
            aggregate = JSONFeature(WordCountAggregator,
              needs=[count1, count2], store=True)
    ...
    def test_stored_features_are_not_rewritten_when_computing_dependent_feature(self):
        ...
        class Timestamps(BaseModel):
            ...
            cat = Feature(Concatenate,
              needs=[t1, t2], store=False)
    ...
    def test_can_process_multiple_documents_and_then_aggregate_word_count(self):
        ...
        class DocumentWordCount(BaseModel):
            counts = Feature(FeatureAggregator,
              cls=Doc,
              feature=(Doc.count),
              store=False)
            total_count = JSONFeature(WordCountAggregator,
              store=True,
              needs=counts)

        Doc.process(stream='mary')
        Doc.process(stream='humpty')
        _id3 = DocumentWordCount.process(counts=(self.Settings.database))
    ...
    def test_document_with_multiple_roots(self):
        ...
        self.assertEqual(len(data_source['mary']) + len(data_source['humpty']), len(data))
    ...
    def test_exception_is_thrown_if_all_kwargs_are_not_provided(self):
        ...
        self.assertRaises(KeyError, lambda : Doc.process(stream1='mary'))
    ...
    def test_get_meaningful_error_when_no_settings_base_class_and_process_is_called(self):
        self.assertRaises(NoPersistenceSettingsError, lambda : Document.process(stream='mary'))

    def test_get_meaningful_error_when_no_settings_base_class_and_getattr_is_called(self):
        ...
        _id = Doc.process(stream='mary')
        doc = Document(_id)
        self.assertRaises(NoPersistenceSettingsError, lambda : doc.stream.read())
    ...
    def test_feature_that_takes_a_variable(self):
        ...
        class Numbers(BaseModel, self.Settings):
            ...
            stringify = Feature((lambda x: ''.join(map(str, x))),
              needs=add1, store=True)
    ...
    def test_unstored_feature_with_multiple_inputs_can_be_computed(self):
        ...
        class Doc3(BaseModel, self.Settings):
            ...
            cat = Feature(Concatenate,
              needs=[uppercase, lowercase], store=False)
    ...
    def test_can_read_computed_property_when_dependencies_are_in_different_data_stores(self):
        ...
        class Split(BaseModel):
            stream = Feature(TextStream, store=False)
            uppercase = Feature(ToUpper,
              needs=stream, store=True, persistence=settings1)
            lowercase = Feature(ToLower,
              needs=stream, store=True, persistence=settings2)
            cat = Feature(Concatenate,
              needs=[uppercase, lowercase], store=False)
    ...
    def test_can_register_multiple_dependencies_on_base_model_derived_class(self):
        db1 = InMemoryDatabase(key_builder=(self.Settings.key_builder))
        settings = self.Settings.clone(database=db1,
          id_provider=(IntegerIdProvider()))
    ...
    def test_can_use_bz2_compression_encoder_and_decoder(self):
        ...
        db = self.Settings.database
        key_builder = self.Settings.key_builder
        stream = db.read_stream(key_builder.build(_id, 'lowercase', A.lowercase.version))
...
class FileSystemTest(BaseTest, unittest2.TestCase):
    ...
    def setUp(self):
        ...
        class Settings(PersistenceSettings):
            id_provider = UuidProvider()
            key_builder = StringDelimitedKeyBuilder()
            database = FileSystemDatabase(path=(self._dir),
