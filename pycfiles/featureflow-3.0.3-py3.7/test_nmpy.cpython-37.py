...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/featureflow/test_nmpy.py
...
class BaseNumpyTest(object):
    ...
    def _build_doc(self):
        ...
        class Doc(BaseModel, self.Settings):
            ...
            packed = NumpyFeature(PassThrough,
              needs=feat,
    ...
    def _arrange(self, shape=None, dtype=None):
        cls = self._build_doc()
        arr = np.recarray(shape, dtype=dtype) if isinstance(dtype, list) else np.zeros(shape, dtype=dtype)
...
class GreedyNumpyTest(BaseNumpyTest, unittest2.TestCase):
    ...
    def _register_database(self, settings_class):
        return settings_class.clone(database=InMemoryDatabase(key_builder=(settings_class.key_builder)))
...
class GreedyNumpyOnDiskTest(BaseNumpyTest, unittest2.TestCase):
    ...
    def _register_database(self, settings_class):
        self._dir = mkdtemp()
        return settings_class.clone(database=FileSystemDatabase(path=(self._dir),
...
class GreedyNumpyLmdbTest(BaseNumpyTest, unittest2.TestCase):
    ...
    def _register_database(self, settings_class):
        self._dir = mkdtemp()
        return settings_class.clone(database=LmdbDatabase(path=(self._dir),
          map_size=10000000,
          key_builder=(settings_class.key_builder)))
