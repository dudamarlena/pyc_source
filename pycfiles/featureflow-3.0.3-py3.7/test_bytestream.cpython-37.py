...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/featureflow/test_bytestream.py
# Compiled at: 2019-03-06 21:24:36
# Size of source mod 2**32: 5000 bytes
from .bytestream import BytesWithTotalLength, ByteStream, ZipWrapper, iter_zip
import unittest2, sys, tempfile, subprocess, requests
...
class BytestreamTests(unittest2.TestCase):
    ...
    def test_can_accept_chunksize_implementing_int(self):
        ...
        bio = BytesIO()
        bio.write(os.urandom(int(100000.0)))
        bio.seek(0)
...
class BytesWithTotalLengthTests(unittest2.TestCase):
    ...
    def test_left_add(self):
        self.assertEqual(b'fakeblah', BytesWithTotalLength(b'fake', 100) + b'blah')

    def test_right_add(self):
        self.assertEqual(b'blahfake', b'blah' + BytesWithTotalLength(b'fake', 100))
...
class IterZipTests(unittest2.TestCase):
    ...
    def test_iter_zip_yields_open_zip_files(self):
        ...
        with list(iter_zip(bio))[0] as (z):
            self.assertFalse(z.zipfile.closed, 'zipfile should be open, but was closed')
