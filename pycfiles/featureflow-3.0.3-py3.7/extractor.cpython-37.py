...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/featureflow/extractor.py
# Compiled at: 2019-03-01 22:03:23
# Size of source mod 2**32: 10502 bytes
...
class InvalidProcessMethod(Exception):
    ...
    def __init__(self, cls):
        msg = '{name}._process method must be a generator'.format(name=(cls.__name__))
...
class Node(object):
    ...
    def __init__(self, needs=None):
        ...
        self._dependecy_names = dict(((id(v), k) for k, v in list(self._needs.items())))
    ...
    def _finalized(self):
        ...
        """
        return len(self._finalized_dependencies) >= self.dependency_count and len(self._enqueued_dependencies) >= self.dependency_count
    ...
    def version(self):
        ...
        try:
            ...
            if closure is None or self.closure_fingerprint is None:
                return h.hexdigest()
            d = dict(((name, cell.cell_contents) for name, cell in zip(f.co_freevars, closure)))
...
class Graph(dict):
    ...
    def remove_dead_nodes(self, features):
        ...
        while nodes:
            ...
            if extractor.is_leaf:
                feature.store or extractor.disconnect()
                try:
    ...
    def process(self, **kwargs):
        ...
        if len(intersection) < len(roots):
            raise KeyError('the keys {kw} were provided to the process() method, but the keys for the root extractors were {r}'.format(kw=(list(kwargs.keys())),
              r=(list(roots.keys()))))
        ...
        with ExitStack() as (stack):
            ...
            generators = [roots[k].process(v, queue=queue) for k, v in list(graph_args.items())]
