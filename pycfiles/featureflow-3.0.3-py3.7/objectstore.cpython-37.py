...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/featureflow/objectstore.py
...
class ObjectStoreDatabase(Database):
    ...
    def __init__(self, container_name, username, api_key, region, cdn_ttl_seconds=900, key_builder=None):
    ...
    def _get_token(self):
        resp = requests.post('https://identity.api.rackspacecloud.com/v2.0/tokens',
          data=(json.dumps({'auth': {'RAX-KSKEY:apiKeyCredentials': {'username':self.username, 
                                                  'apiKey':self.api_key}}})),
    ...
    def _create_container(self):
        resp = requests.put(('{endpoint}/{container_name}'.format)(**self.__dict__),
          headers={'X-Auth-Token': self.token})
        resp.raise_for_status()
        resp2 = requests.put(('{cdn_endpoint}/{container_name}'.format)(**self.__dict__),
          headers={'X-Auth-Token':self.token, 
         'X-CDN-Enabled':'True', 
    ...
    def _uri(self, key):
        ...
        return '{endpoint}/{container_name}/{key}'.format(endpoint=(self.endpoint),
          container_name=(self.container_name),
          key=key)

    def _cdn_uri(self, key):
        key = urllib.parse.quote(key, safe='')
        return '{cdn_uri}/{key}'.format(cdn_uri=(self.cdn_uri),
          key=key)

    def write_stream(self, key, content_type):
        return WriteStream(self._uri(key), content_type, self.token, self._get_token)
    ...
    def _all_keys(self):
        ...
        resp = requests.get(container_uri,
          headers={'X-Auth-Token': self.token})
    ...
    def __del__(self):
        ...
        resp = requests.delete(container_uri,
          headers={'X-Auth-Token': self.token})
        resp.raise_for_status()