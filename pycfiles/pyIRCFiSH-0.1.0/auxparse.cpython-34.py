...
prefix_match = re.compile('\\(([A-Za-z0-9]+)\\)(.+)')
...
def banmask_parse(string, supported_extban):
    ...
    if not hostmask:
        return ret
    ret.nick, ret.user, ret.host = hostmask.nick, hostmask.username, hostmask.host
    return ret
...
class CTCPMessage:
    ...
    def __repr__(self):
        return 'CTCPMessage(msgtype={}, command={}, target={}, param={})'.format(repr(self.msgtype), repr(self.command), repr(self.target), repr(self.param))