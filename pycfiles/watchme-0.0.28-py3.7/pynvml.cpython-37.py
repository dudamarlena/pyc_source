...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/watchme/watchers/gpu/pynvml.py
# Compiled at: 2020-04-10 14:08:51
# Size of source mod 2**32: 57529 bytes
"""
...
"""
from ctypes import byref, CDLL, create_string_buffer, c_char_p, c_char, c_int, c_double, c_uint, c_ulong, c_ulonglong, POINTER, Structure, Union
...
class NVMLError(Exception):
    ...
    _errcode_to_string = {NVML_ERROR_UNINITIALIZED: 'Uninitialized', 
...
def _extractNVMLErrorsAsClasses():
    ...
    Each NVML Error gets a new NVMLError subclass. This way try,
    except blocks can filter appropriate exceptions more easily.
...
class c_nvmlUnitFanSpeeds_t(_PrintableStructure):
    _fields_ = [
     (
      'fans', c_nvmlUnitFanInfo_t * 24), ('count', c_uint)]
...
nvmlEventTypeNone = 0
nvmlEventTypeAll = nvmlEventTypeNone | nvmlEventTypeSingleBitEccError | nvmlEventTypeDoubleBitEccError | nvmlEventTypePState | nvmlEventTypeClock | nvmlEventTypeXidCriticalError
...
nvmlClocksThrottleReasonUnknown = 9223372036854775808
nvmlClocksThrottleReasonNone = 0
nvmlClocksThrottleReasonAll = nvmlClocksThrottleReasonNone | nvmlClocksThrottleReasonGpuIdle | nvmlClocksThrottleReasonApplicationsClocksSetting | nvmlClocksThrottleReasonSwPowerCap | nvmlClocksThrottleReasonHwSlowdown | nvmlClocksThrottleReasonUnknown
...
def nvmlSystemGetNVMLVersion():
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_version.value)
...
def nvmlSystemGetProcessName(pid):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_name.value)
...
def nvmlUnitGetCount():
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_count.value)
...
def nvmlUnitGetTemperature(unit, type):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_temp.value)
...
def nvmlDeviceGetBoardId(handle):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_id.value)
...
def nvmlDeviceGetMultiGpuBoard(handle):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_multiGpu.value)
...
def nvmlDeviceGetBrand(handle):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_type.value)
...
def nvmlDeviceGetSerial(handle):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_serial.value)
...
def nvmlDeviceGetMinorNumber(handle):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_minor_number.value)
...
def nvmlDeviceGetUUID(handle):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_uuid.value)
...
def nvmlDeviceGetInforomConfigurationChecksum(handle):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_checksum.value)
...
def nvmlDeviceGetDisplayMode(handle):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_mode.value)
...
def nvmlDeviceGetPersistenceMode(handle):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_state.value)
...
def nvmlDeviceGetClockInfo(handle, type):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_clock.value)
...
def nvmlDeviceGetFanSpeed(handle):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_speed.value)
...
def nvmlDeviceGetPowerState(handle):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_pstate.value)
...
def nvmlDeviceGetPowerManagementMode(handle):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_pcapMode.value)
...
def nvmlDeviceGetPowerManagementLimit(handle):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_limit.value)
...
def nvmlDeviceGetPowerUsage(handle):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_watts.value)
...
def nvmlDeviceGetPcieReplayCounter(handle):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_replay.value)
...
def nvmlDeviceGetSupportedEventTypes(handle):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_eventTypes.value)
...
def nvmlDeviceGetCurrPcieLinkGeneration(handle):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(gen.value)
...
def nvmlDeviceGetCurrPcieLinkWidth(handle):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(width.value)
...
def nvmlDeviceGetSupportedClocksThrottleReasons(handle):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_reasons.value)
...
def nvmlDeviceGetIndex(handle):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_index.value)
...
def nvmlDeviceGetViolationStatus(device, perfPolicyType):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_violTime)
...
def nvmlDeviceGetPcieThroughput(device, counter):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_util.value)
...
def nvmlDeviceGetTopologyCommonAncestor(device1, device2):
    ...
    _nvmlCheckReturn(ret)
    return ensureUtfEncoding(c_level.value)


def ensureUtfEncoding(value):
