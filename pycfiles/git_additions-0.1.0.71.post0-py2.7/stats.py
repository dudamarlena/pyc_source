...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/git_additions/stats/stats.py
# Compiled at: 2018-12-30 14:16:11
...
class Stats(object):
        ...
        try:
            ...
            for commit in repo.walk(repo.head.target, pygit2.GIT_SORT_TOPOLOGICAL):
                if previous_commit is not None:
                    ...
                    print('%s%s%s <%s>%s' % (
                     Fore.YELLOW, commit.author.name, Style.DIM, commit.author.email, Style.NORMAL), end=' ')
    ...
    def short(repo):
        ...
        for key in report.keys():
            ...
            print('%s%s%s <%s>%s' % (Fore.YELLOW, author[0], Style.DIM, author[1], Style.NORMAL), ' ' * (max_author_length - len(key)), end=' ')
            print('%sf: %s' % (Fore.BLUE, report[key]['files_changed']), ' ' * (len(str(total_files_changed)) - len(str(report[key]['files_changed']))), end=' ')
            print('%s+: %s' % (Fore.GREEN, report[key]['insertions']), ' ' * (len(str(total_insertions)) - len(str(report[key]['insertions']))), end=' ')
