...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/git_additions/users/users.py
...
class Users(object):
    ...
    def report():
        ...
        print(('Name').ljust(max_name_length), '\t', ('Email').ljust(max_email_length), '\t', 'Commits')
        print('_' * max_name_length, '\t', '_' * max_email_length, '\t', '_______')
        for email, author in authors.items():
            print(author['author'].name.ljust(max_name_length), '\t', author['author'].email.ljust(max_email_length), '\t', author['commits'])