...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/tacs/tests.py
# Compiled at: 2019-12-18 21:17:01
# Size of source mod 2**32: 10626 bytes
import logging, tempfile, unittest, tacs.config
...
def get_cfg(cls=CN):
    ...
    cfg.register_renamed_key('EXAMPLE.OLD.KEY',
      'EXAMPLE.NEW.KEY',
...
class TestCfg(unittest.TestCase):
    ...
    def test_merge_cfg_from_list(self):
        ...
        if not len(cfg.TRAIN.SCALES) > 0:
            raise AssertionError
        else:
            if not cfg.TRAIN.SCALES[0] != 100:
                raise AssertionError
            else:
                if not cfg.MODEL.TYPE != 'foobar':
                    raise AssertionError
                else:
                    if not cfg.NUM_GPUS != 2:
                        raise AssertionError
                    else:
    ...
    def test_load_from_python_file(self):
        ...
        if not cfg.TRAIN.HYPERPARAMETER_1 == 0.9:
            raise AssertionError
        else:
            cfg = get_cfg()
