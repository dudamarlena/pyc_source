...
# [GCC 8.4.0]
# Embedded file name: /home/postlund/pyatv_dev/pyatv/pyatv/mrp/srp.py
# Compiled at: 2019-09-26 00:19:20
# Size of source mod 2**32: 8171 bytes
"""Prototype code for MRP."""
import os, uuid, binascii, hashlib, logging, curve25519
...
class Credentials:
    __doc__ = 'Identifiers and encryption keys used by MRP.'
    ...
    def __str__(self):
        ...
        return '{0}:{1}:{2}:{3}'.format(binascii.hexlify(self.ltpk).decode('utf-8'), binascii.hexlify(self.ltsk).decode('utf-8'), binascii.hexlify(self.atv_id).decode('utf-8'), binascii.hexlify(self.client_id).decode('utf-8'))
...
def hkdf_expand(salt, info, shared_secret):
    ...
    hkdf = HKDF(algorithm=(hashes.SHA512()),
      length=32,
      salt=(salt.encode()),
      info=(info.encode()),
...
class SRPAuthHandler:
    __doc__ = 'Handle SRP crypto routines for auth and key derivation.'
    ...
    def verify1(self, credentials, session_pub_key, encrypted):
        ...
        device_info = self._verify_public.serialize() + credentials.client_id + session_pub_key
    ...
    def verify2(self):
        ...
        """
        output_key = hkdf_expand('MediaRemote-Salt', 'MediaRemote-Write-Encryption-Key', self._shared)
        input_key = hkdf_expand('MediaRemote-Salt', 'MediaRemote-Read-Encryption-Key', self._shared)
        log_binary(_LOGGER, 'Keys', Output=output_key, Input=input_key)
        return (output_key, input_key)

    def step1(self, pin):
        """First pairing step."""
        context = SRPContext('Pair-Setup',
          (str(pin)), prime=(constants.PRIME_3072),
    ...
    def step3(self):
        """Third pairing step."""
        ios_device_x = hkdf_expand('Pair-Setup-Controller-Sign-Salt', 'Pair-Setup-Controller-Sign-Info', binascii.unhexlify(self._client_session_key))
        self._session_key = hkdf_expand('Pair-Setup-Encrypt-Salt', 'Pair-Setup-Encrypt-Info', binascii.unhexlify(self._client_session_key))
    ...
    def step4(self, encrypted_data):
        ...
        log_binary(_LOGGER, 'Device',
          Identifier=atv_identifier,
          Signature=atv_signature,
          Public=atv_pub_key)
        return Credentials(atv_pub_key, self._signing_key.to_seed(), atv_identifier, self.pairing_id)