...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/deckenmagicplugin/magicplugin.py
# Compiled at: 2010-06-10 10:48:08
from urllib import urlretrieve
import urlparse, sqlite3, os, conjunto
formatos = [
 '', 'Standard', 'Block', 'Extended', 'Legacy', 'Vintage']
standard = [
 '', 'roe', 'wwk', 'zen', 'm10', 'arb', 'cfx', 'ala']
extended = standard + ['9e', 'eve', 'shm',
 'mt', 'lw', 'fut', 'pc', 'ts', 'tsts', 'cs', 'ai', 'ia', 'di', 'gp', 'rav', 'sok', 'bok',
 'chk', '5dn', 'ds', 'mi']
others = [
 'sc', 'le', 'on', 'ju', 'tr', 'od', 'ap', 'ps', 'in', 'pr', 'ne', 'mm', 'ud', 'ui', 'us',
 'ex', 'sh', 'tp', 'wl', 'vi', 'mr', 'hl', 'fe', 'dk', 'lg', 'aq', 'an', '8e', '7e', '6e', '5e',
 '4e', 'rv', 'un', 'be', 'al']
legacy = extended + others
vintage = legacy
sets_por_formato = {'Standard': standard, 
   'Block': [
           'roe', 'wwk', 'zen'], 
   'Extended': extended, 
   'Legacy': legacy, 
   'Vintage': vintage}
palavras_tipos = {'': '', 
   'Creature': " (tipo like '%Creature%' and tipo not like '%Enchant Creature%') ", 
   'Artifact': " (tipo like '%Artifact%' and tipo not like '%Creature%' )", 
   'Planeswalker': " tipo like '%Planeswalker%' ", 
   'Enchantment': " tipo like '%Enchant%' ", 
   'Sorcery': " tipo = 'Sorcery' ", 
   'Instant': " tipo like '%Instant%' ", 
   'Land': " (tipo like '%Land%' and tipo not like '%Enchant Land%')  "}
...
class Query:
    ...
    def all_cards(self, filtros):
        if filtros:
            self.c.execute(self.select + ' where %s order by nome' % filtros)
        else:
...
class MagicPlugin:
    ...
    def get_select_filter_values(self, name, conn):
        filter_values = {'Card Format': formatos, 
           'Card Set': [
                      ''] + conjunto.names_sets(conn, legacy), 
    ...
    def value_columns_by_card(self, card):
        return [
         card['nome'], card['sigla'], card['numero']]
