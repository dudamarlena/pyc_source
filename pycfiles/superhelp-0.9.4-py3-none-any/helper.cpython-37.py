...
# [GCC 8.4.0]
# Embedded file name: /home/g/projects/superhelp/build/lib/superhelp/helper.py
# Compiled at: 2020-04-16 17:16:13
# Size of source mod 2**32: 13443 bytes
import argparse, ast
...
BlockDets = namedtuple('BlockDets', 'element, pre_block_code_str, block_code_str, first_line_no')
BlockDets.pre_block_code_str.__doc__ = 'The code up until the line we are interested in needs to be run - it may depend on names from earlier'
MessageDets = namedtuple('MessageDets', 'code_str, message, first_line_no, warning')
MessageDets.__doc__ += 'All the bits and pieces that might be needed to craft a message'
t = True
f = False
do_test = t
do_html = t
...
def get_tree(snippet):
    try:
        ...
        try:
            raise SyntaxError(f"Oops - something seems broken in the snippet - details: {e}")
        finally:
            e = None
            del e
...
def get_blocks_dets(xml, snippet_lines):
    ...
    """
    blocks_dets = []
    all_elements = xml.xpath('body')[0].getchildren()
    for element in all_elements:
        ...
        blocks_dets.append(BlockDets(element, pre_block_code_str, block_code_str, first_line_no))
...
def complete_message(message, *, source):
    ...
    try:
        ...
        raise TypeError(f"message had no keys - message: {message} (type {type(message).__name__})")

    if conf.BRIEF not in message_keys:
        raise Exception(f"'{source}' gave a message lacking the mandatory '{conf.BRIEF}' level")
...
def get_message_dets_from_input(advisor_dets, advisor_input, code_str, *, first_line_no):
...
def get_ancestor_block_element(element):
    ...
    ancestor_block_element = ancestor_elements[2]
...
def get_filtered_blocks_dets(advisor_dets, xml, blocks_dets):
    ...
    filt_block_elements = set([get_ancestor_block_element(element) for element in matching_elements])
    filtered_blocks_dets = [block_dets for block_dets in blocks_dets if block_dets.element in filt_block_elements]
...
def get_block_level_messages_dets(blocks_dets, xml):
    ...
    """
    messages_dets = []
    all_advisors_dets = advisors.FILT_BLOCK_ADVISORS + advisors.ANY_BLOCK_ADVISORS
    for advisor_dets in all_advisors_dets:
        ...
        if element_filtering:
            filtered_blocks_dets = get_filtered_blocks_dets(advisor_dets, xml, blocks_dets)
            blocks_dets2use = filtered_blocks_dets
            logging.debug(f"'{advisor_dets.advisor_name}' has element filtering for {len(blocks_dets2use)} matching blocks")
        else:
            blocks_dets2use = blocks_dets
        for block_dets in blocks_dets2use:
            message_dets = get_message_dets_from_input(advisor_dets, block_dets, (block_dets.block_code_str),
...
def get_overall_snippet_messages_dets(snippet, blocks_dets):
    ...
    for advisor_dets in advisors.SNIPPET_ADVISORS:
        message_dets = get_message_dets_from_input(advisor_dets, blocks_dets, snippet,
          first_line_no=None)
        if message_dets:
            messages_dets.append(message_dets)
...
def get_messages_dets_from_xml(xml, snippet):
    ...
    messages_dets_from_blocks = get_block_level_messages_dets(blocks_dets, xml)
    messages_dets.extend(messages_dets_from_blocks)
    messages_dets_from_snippet = get_overall_snippet_messages_dets(snippet, blocks_dets)
...
def get_separated_messages_dets(snippet):
    ...
    if None in messages_dets:
        raise Exception('messages_dets in meant to be a list of MessageDets named tuples yet a None item was found')
    ...
    if not overall_messages_dets:
        if not block_messages_dets:
            message = {conf.BRIEF: conf.NO_ADVICE_MESSAGE}
...
def display_messages(displayer, snippet, messages_dets, *, message_level=conf.BRIEF, in_notebook=False):
...
def get_error_messages_dets(e, snippet):
    message = {conf.BRIEF: layout_comment(f"            #### No advice sorry :-(\n\n            Unable to provide advice - some sort of problem.\n\n            Details: {e}\n            ")}
    message = complete_message(message, source=(conf.SYSTEM_MESSAGE))
    overall_messages_dets = [
...
def _get_displayer_module(displayer):
    ARG2DISPLAYER = {'html':html_displayer,  'cli':cli_displayer}
    displayer_module = ARG2DISPLAYER.get(displayer)
    if displayer_module is None:
        logging.info('Display is currently suppressed - please supply a displayer if you want advice displayed')
    return displayer_module


def superhelp(snippet=None, *, file_path=None, displayer='html', message_level=conf.EXTRA, in_notebook=False):
    """
    ...
    if displayer_module:
        res = display_messages(displayer_module, snippet, messages_dets, message_level=message_level,
...
def shelp():
    ...
    parser.add_argument('-s', '--snippet', type=str, required=False,
    ...
    parser.add_argument('-l', '--level', type=str, required=False,
      default='Extra',
      help='What level of help do you want? Brief, Main, or Extra?')
    parser.add_argument('-d', '--displayer', type=str, required=False,
      default=default_displayer,
      help='Where do you want your help shown? html, cli, etc')
    args = parser.parse_args()
    displayer = args.displayer if do_displayer else None
    superhelp((args.snippet), file_path=(args.file_path),
      displayer=displayer,
