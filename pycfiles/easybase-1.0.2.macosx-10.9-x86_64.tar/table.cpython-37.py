...
# [GCC 8.4.0]
# Embedded file name: /Users/wgzhao/Codes/easybase/py3/lib/python3.7/site-packages/easybase/table.py
# Compiled at: 2019-12-20 08:50:47
# Size of source mod 2**32: 22355 bytes
"""
...
def make_ordered_row(sorted_columns, include_timestamp):
    ...
    return OrderedDict(((column.columnName, cellfn(column.cell)) for column in sorted_columns))


class Table(object):
    __doc__ = 'HBase table abstraction class.\n\n    This class cannot be instantiated directly; use :py:meth:`Connection.table`\n    instead.\n    '
    ...
    def row(self, row, columns=None, timestamp=None, timerange=None, maxversions=1, include_timestamp=False):
        ...
        if columns is not None:
            ...
            if timerange is not None:
                if not isinstance(timerange, (tuple, list)):
    ...
    def rows(self, rows, columns=None, timestamp=None, timerange=None, maxversions=1, include_timestamp=False):
        ...
        return [(r.row, make_row(r.columnValues, include_timestamp)) for r in results]

    def scan(self, row_start=None, row_stop=None, row_prefix=None, columns=None, filter=None, timerange=None, include_timestamp=False, batch_size=1000, scan_batching=None, limit=None, reversed=False, max_version=1):
        ...
        * The `sorted_columns` argument is only available when using
        ...
        cols = make_columns(columns)
        tt = make_timerange(timerange)
        tscan = TScan(startRow=row_start,
          stopRow=row_stop,
          timeRange=tt,
          columns=cols,
          caching=batch_size,
        ...
        try:
            while 1:
                ...
                items = self.connection.client.getScannerRows(scan_id, how_many)
                if not items:
                    return
                n_fetched += len(items)
            ...
            logger.debug("Closed scanner (id=%d) on '%s' (%d returned, %d fetched)", scan_id, self.name, n_returned, n_fetched)
    ...
    def puts(self, rows):
        ...
        for rk, item in iteritems(rows):
            ...
            tput = TPut(row=(rk.encode()), columnValues=cols, durability=(item.get('wal', True)),
    ...
    def counter_inc(self, row, column, value=1):
        ...
        :rtype: int
        """
        return self.connection.client.atomicIncrement(self.name, row, column, value)
