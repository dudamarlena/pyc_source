...
# [GCC 8.4.0]
# Embedded file name: /Users/aitjcize/Work/cppman/cppman/main.py
# Compiled at: 2018-08-18 07:47:23
import gzip, importlib, os, re, shutil, sqlite3, subprocess, sys, urllib.request
...
class Cppman(Crawler):
    ...
    def rebuild_index(self):
        ...
        self.db_cursor.execute('CREATE TABLE "cplusplus.com" (name VARCHAR(255), url VARCHAR(255), std VARCHAR(255))')
        self.db_cursor.execute('CREATE TABLE "cppreference.com" (name VARCHAR(255), url VARCHAR(255), std VARCHAR(255))')
        try:
            try:
                ...
                self.db_conn.commit()
                duplicates = self.db_cursor.execute('SELECT name, COUNT(name) AS NON FROM "cplusplus.com" GROUP BY NAME HAVING (NON > 1)').fetchall()
                for name, num in duplicates:
                    dump = self.db_cursor.execute('SELECT name, url FROM "cplusplus.com" WHERE name="%s"' % name).fetchall()
                    for n, u in dump:
                        if u not in self.name_exceptions:
                            ...
                            self.db_cursor.execute('UPDATE "cplusplus.com" SET name="%s", url="%s" WHERE url="%s"' % (
                             new_name, u, u))
    ...
    def insert_index(self, table, name, url, std=''):
        ...
        names = name.split(',')
        if len(names) > 1:
            m = re.match('^\\s*(.*?::(?:operator)?)([^:]*)\\s*$', names[0])
            if m:
                prefix = m.group(1)
                names[0] = m.group(2)
                names = [ prefix + n for n in names ]
        for n in names:
            self.db_cursor.execute('INSERT INTO "%s" (name, url, std) VALUES ("%s", "%s", "%s")' % (
             table, n.strip(), url, std))

    def cache_all(self):
        """Cache all available man pages"""
        respond = input('By default, cppman fetches pages on-the-fly if corresponding page is not found in the cache. The "cache-all" option is only useful if you want to view man pages offline. Caching all contents will take several minutes, do you want to continue [y/N]? ')
    ...
    def man(self, pattern):
        ...
        try:
            try:
                page_name, url = cursor.execute('SELECT name,url FROM "%s" WHERE name="%s" ORDER BY LENGTH(name)' % (
                 environ.source, pattern)).fetchone()
            except TypeError:
                try:
                    page_name, url = cursor.execute('SELECT name,url FROM "%s" WHERE name="std::%s" ORDER BY LENGTH(name)' % (
                     environ.source, pattern)).fetchone()
                except TypeError:
                    try:
                        page_name, url = cursor.execute('SELECT name,url FROM "%s" WHERE name LIKE "%%%s%%" ORDER BY LENGTH(name)' % (
                         environ.source, pattern)).fetchone()
                    except TypeError:
        ...
        columns = util.get_width() if self.force_columns == -1 else self.force_columns
        pid = os.fork()
        if pid == 0:
            os.execl('/bin/sh', '/bin/sh', environ.pager_script, pager_type, self.get_page_path(environ.source, page_name), str(columns), environ.pager_config, page_name)
        return pid
    ...
    def find(self, pattern):
        ...
        conn = sqlite3.connect(environ.index_db)
        cursor = conn.cursor()
        selected = cursor.execute('SELECT * FROM "%s" WHERE name LIKE "%%%s%%" ORDER BY LENGTH(name)' % (
         environ.source, pattern)).fetchall()
        pat = re.compile('(%s)' % pattern, re.I)
        if selected:
            for name, url, std in selected:
                if os.isatty(sys.stdout.fileno()):
                    print pat.sub('\\033[1;31m\\1\\033[0m', name) + (' \x1b[1;33m[%s]\x1b[0m' % std if std else '')
                else:
