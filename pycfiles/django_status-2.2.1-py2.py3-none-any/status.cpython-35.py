...
# [GCC 8.4.0]
# Embedded file name: /home/perdy/Development/django-status/status/management/commands/status.py
# Compiled at: 2016-09-29 08:27:33
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        subparsers = parser.add_subparsers(title='status', description='Status checks', dest='status', parser_class=lambda **kwargs: CommandParser(self, **kwargs))
        parser_health = subparsers.add_parser(self.STATUS_HEALTH, help=_('Health checks'))
        parser_health.add_argument('-f', '--output_format', type=str, choices=self.OUTPUT_FORMAT_CHOICES, default=self.OUTPUT_FORMAT_YAML, help='Output format')
        parser_health.add_argument('-e', '--error_on_fail', action='store_true', default=False, help='Output format')
        parser_stats = subparsers.add_parser(self.STATUS_STATS, help=_('Components stats'))
        parser_stats.add_argument('-f', '--output_format', type=str, choices=self.OUTPUT_FORMAT_CHOICES, default=self.OUTPUT_FORMAT_YAML, help='Output format')
