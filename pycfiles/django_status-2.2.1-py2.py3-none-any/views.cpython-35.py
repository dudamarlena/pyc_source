...
# [GCC 8.4.0]
# Embedded file name: /home/perdy/Development/django-status/status/api/views.py
# Compiled at: 2016-09-29 07:57:24
...
class RootAPIView(ProviderMixin, APIView):
    ...
    def options(self, request, *args, **kwargs):
        ...
        :return: Rendered response.
        """
        context = {resource.name:{provider_name:self.get_provider_url(request, resource.name, provider_name) for provider_name in resource.providers.keys()} for resource in self.resources.values()}
        return self.render_to_response(context)
...
class ResourceAPIView(ProviderMixin, APIView):
    ...
    def options(self, request, *args, **kwargs):
        ...
        :return: Rendered response.
        """
        context = {name:self.get_provider_url(request, self.resource.name, name) for name in self.resource.providers.keys()}
        return self.render_to_response(context)
