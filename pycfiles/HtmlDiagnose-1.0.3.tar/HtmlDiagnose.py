...
# [GCC 8.4.0]
# Embedded file name: /Users/robin/DO/GitHub/HtmlDiagnose/HtmlDiagnose/HtmlDiagnose.py
# Compiled at: 2020-03-19 05:11:51
...
class HTMLSyntax(HTMLParser):
    ...
    def __init__(self):
        ...
        self.stack = []
        self.singleline = [
         'input', 'br', 'meta', 'link', 'img', 'base', 'hr']
        return
...
def getAllLinks(path, html):
    ...
    """
    links = []
    for h in re.findall('<a [^/]*href=["\\\']([^\\s^\\\'^"]+)["\\\']', html):
        if h.startswith('tel:'):
            continue
        elif '://' not in h:
...
def main():
    ...
    parser.add_option('-s', '--statefile', action='store', dest='state', default='', help='state file to save fetch state')
    parser.add_option('-H', '--header', action='append', dest='headers', default=[], help="set http request headers, like 'curl -H'")
    parser.add_option('-r', '--ignore', action='append', dest='ignores', default=[], help="add ignore url param, exp: -i 'random'")
    parser.add_option('-e', '--exclude', action='append', dest='excludes', default=[], help='exclude url pattern')
    parser.add_option('-i', '--include', action='append', dest='includes', default=[], help='include url pattern')
    parser.add_option('-n', '--new', action='store_true', dest='isnew', default=False, help='fetch with new state')
    parser.add_option('-g', '--goon', action='store_true', dest='isgoon', default=False, help='go on when occur error')
