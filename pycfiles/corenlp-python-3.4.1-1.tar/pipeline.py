...
# Embedded file name: build/bdist.linux-x86_64/egg/corenlp/pipeline.py
# Compiled at: 2014-04-18 16:02:27
...
def dir2dir(in_dir, out_dir=None, annotators=None, mem=None, libdir=None, libver=None, threads=None):
    files = []
    for txt_file in os.listdir(in_dir):
        fpath = os.path.join(in_dir, txt_file)
        files.append(fpath)

    files2dir(files, out_dir=out_dir, annotators=annotators, mem=mem, libdir=libdir, libver=libver, threads=threads)


def files2dir(files, out_dir=None, annotators=None, mem=None, libdir=None, libver=None, threads=None):
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    cmd = ('java -Xmx{} -cp {} {} -annotators {} -filelist {} -outputDirectory {} -threads {} -replaceExtension').format(mem, cpath, _pipeline_class, (',').join(annotators), filelist.name, out_dir, threads)
