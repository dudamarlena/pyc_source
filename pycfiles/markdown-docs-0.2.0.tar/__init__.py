...
# [GCC 8.4.0]
# Embedded file name: /Users/sebastian/git/markdown-docs/markdowndocs/__init__.py
# Compiled at: 2013-04-26 13:43:23
...
"""
VERSION = '0.2.0'
import os, sys, shutil, tempfile, argparse, markdowndocs.generator, markdowndocs.web_server
from markdowndocs.markdown_file import MarkdownFile
from markdowndocs.log_handler import LOGGER as logger

def main():
    """ Main function """
    parser = argparse.ArgumentParser(description='markdown-docs markdown documentation generator')
    parser.add_argument('-d', '--directory', help='Root directory to parse from (default: current dir)')
    parser.add_argument('-o', '--output', help='Output directory to store HTML files in')
    parser.add_argument('--version', action='store_true', help='Print version information')
    parser.add_argument('generate', nargs='?', default=False, help='Generate HTML')
    parser.add_argument('serve', nargs='?', default=True, help='Start a local web server to serve the documentation')
    ...
    if args.output:
        ...
        try:
            ...
            if errno == 17:
                ...
                logger.error(('Error creating {}: {}').format(destination_root_dir, errmsg))
                sys.exit(1)
    ...
    try:
        ...
        logger.info(('Generating documentation for {:d} markdown files..').format(len(markdown_files)))
        ...
        if args.generate:
            logger.info(('HTML output can be found in {}').format(destination_root_dir))
    finally:
        if temp_dir_used:
            logger.debug(('Removing temporary folder: {}').format(destination_root_dir))
        ...
        for filename in filenames:
            try:
                ...
                if extension in ('md', 'mdown', 'markdown'):
                    md_file = MarkdownFile(os.path.join(dirpath, filename), source_dir, destination_root_dir)
