...
# [GCC 8.4.0]
# Embedded file name: /Users/sebastian/git/markdown-docs/markdowndocs/generator.py
# Compiled at: 2013-04-26 13:55:15
...
"""
import os, shutil, datetime, jinja2, markdown
...
def generate_html(markdown_files):
    ...
    for markdown_file in markdown_files:
        logger.debug(('Generating HTML for {}..').format(markdown_file.source_file))
        try:
        ...
        with open(markdown_file.source_file, 'r') as (file_handle):
            try:
                ...
                logger.warning(('UnicodeError when reading {}. Skipping.').format(markdown_file.source_file))

        with open(markdown_file.destination_file, 'w') as (file_handle):
            markdown_object = markdown.Markdown(extensions=[
             'meta',
             'toc',
             'tables',
            ...
            if 'title' in markdown_object.Meta:
                markdown_file.set_metadata('title', markdown_object.Meta['title'][0])
            html = template.render({'title': markdown_file.get_metadata('title'), 
               'destination_root_dir': markdown_file.destination_root_dir, 
               'markdown_html': markdown_html, 
               'generation_timestamp': datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M')})
...
def generate_index_page(markdown_files):
    ...
    with open(index_path, 'w') as (file_handle):
        file_handle.write(template.render({'markdown_metadata': markdown_metadata, 
           'generation_timestamp': datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M')}))
...
def import_static_files(destination_root_dir):
    ...
    if os.path.exists(os.path.join(destination_root_dir, '_markdown-docs_static')):
        shutil.rmtree(os.path.join(destination_root_dir, '_markdown-docs_static'), ignore_errors=True)
    shutil.copytree(os.path.join(os.path.dirname(__file__), 'static'), os.path.join(destination_root_dir, '_markdown-docs_static'))