...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/examples/tidigits/xvec.py
# Compiled at: 2019-07-19 06:01:43
...
with N.args_scope([
 ...
 'Dense', dict(activation=(K.linear), b_init=None)]):
    f = N.Sequence([
     ...
     N.TimeDelayedConv(n_new_features=512, n_time_context=5),
     N.TimeDelayedConv(n_new_features=512, n_time_context=7, name='LatentTDNN'),
     ...
     N.Dense(512), N.BatchNorm(activation=(K.relu)),
     N.Dense(num_units=n_classes, activation=(K.linear), b_init=(init_ops.constant_initializer(0)))],
      debug=1)
y_logit = f(X)
y_proba = tf.nn.softmax(y_logit)
z1 = K.ComputationGraph(y_proba).get(roles=(N.Dense), scope='LatentDense', beginning_scope=False)[0]
z2 = K.ComputationGraph(y_proba).get(roles=(N.TimeDelayedConv), scope='LatentTDNN', beginning_scope=False)[0]
...
updates = K.optimizers.Adam(lr=0.0001).minimize(loss=ce,
...
task.set_valid_task(func=f_score, data=valid, freq=training.Timer(percentage=0.8),
  name='valid')
task.run()
y_pred_proba, Z1_test, Z2_test = make_dnn_prediction(functions=[
 f_pred_proba, f_z1, f_z2],
  X=X_test_data,
  title='TEST')
...
plda = PLDA(n_phi=100, random_state=(K.get_rng().randint(1000000000.0)), n_iter=12,
  labels=labels,
  verbose=0)
plda.fit(np.concatenate([Z1_train, Z1_valid], axis=0), np.concatenate([y_train, y_valid], axis=0))
