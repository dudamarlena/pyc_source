...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/odin/bay/distributions/negative_binomial_disp.py
# Compiled at: 2019-09-17 07:10:34
...
class NegativeBinomialDisp(distribution.Distribution):
    ...
    """
        ...
        with tf.name_scope(name) as (name):
            ...
            with tf.control_dependencies([assert_util.assert_positive(loc), assert_util.assert_positive(disp)] if validate_args else []):
                self._loc = tf.identity(loc, name='loc')
                self._disp = tf.identity(disp, name='disp')
        super(NegativeBinomialDisp, self).__init__(dtype=(self._loc.dtype),
          reparameterization_type=(reparameterization.NOT_REPARAMETERIZED),
          validate_args=validate_args,
          allow_nan_stats=allow_nan_stats,
          parameters=parameters,
          graph_parents=[
         self._loc, self._disp],
          name=name)
    ...
    def _batch_shape_tensor(self):
        return tf.broadcast_dynamic_shape(tf.shape(input=(self.loc)), tf.shape(input=(self.disp)))
    ...
    def _sample_n(self, n, seed=None):
        ...
        concentration = r
        rate = (1 - p) / p
        rate = tf.random.gamma(shape=[n], alpha=concentration,
          beta=rate,
          dtype=(self.dtype),
          seed=(stream()))
        return tf.random.poisson(lam=rate, shape=[], dtype=(self.dtype),
          seed=(stream()))
    ...
    def _log_unnormalized_prob(self, x, eps=1e-08):
        if self.validate_args:
            x = distribution_util.embed_check_nonnegative_integer_form(x)
        eps = tf.cast(eps, dtype=(self.dtype))
        log_loc_disp_eps = tf.math.log(self.disp + self.loc + eps)
        return self.disp * (tf.math.log(self.disp + eps) - log_loc_disp_eps) + x * (tf.math.log(self.loc + eps) - log_loc_disp_eps)

    def _log_normalization(self, x):
        if self.validate_args:
            x = distribution_util.embed_check_nonnegative_integer_form(x)
        return tf.math.lgamma(self.disp) + tf.math.lgamma(x + 1) - tf.math.lgamma(x + self.disp)
