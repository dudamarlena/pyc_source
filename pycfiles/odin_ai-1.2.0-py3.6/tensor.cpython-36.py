...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/odin/backend/tensor.py
# Compiled at: 2019-08-27 10:21:22
# Size of source mod 2**32: 18794 bytes
from __future__ import absolute_import, division, print_function
import copy, inspect, numbers
...
def length_norm(x, axis=-1, epsilon=1e-12, ord=2):
  ...
  """
    ...
    if ord not in (1, 2):
        ...
        if ord == 2:
            x_norm = tf.sqrt(tf.maximum(tf.reduce_sum((x ** 2), axis=axis, keepdims=True), epsilon))
        else:
            x_norm = tf.maximum(tf.reduce_sum((tf.abs(x)), axis=axis, keepdims=True), epsilon)
    return x / x_norm
...
def log_norm(x, axis=1, scale_factor=10000, eps=1e-08):
  ...
  """
    ...
    return tf.math.log1p(x / (tf.reduce_sum(x, axis=axis, keepdims=True) + eps) * scale_factor)
...
def logreduceexp(x, reduction_function=tf.reduce_mean, axis=None, name=None):
  ...
  """
    with tf.name_scope(name, 'logreduceexp'):
        ...
        y = tf.log(reduction_function((tf.exp(x - x_max)), axis=axis, keepdims=True)) + x_max
        return tf.squeeze(y)
...
def to_llr(x, name=None):
    r""" Convert a matrix of probabilities into log-likelihood ratio
  ...
  """
    with tf.name_scope(name, 'log_likelihood_ratio', [x]):
        ...
        for j in range(nb_classes):
            scores_copy = tf.transpose(tf.gather(tf.transpose(x), [i for i in range(nb_classes) if i != j]))
...
def to_nonzeros(x, value):
    x = tf.where(tf.equal(x, 0.0), tf.zeros_like(x) + value, x)
    return x
  ...
  """
    with tf.name_scope(name, 'LowerTriangle'):
        if k == 0:
            ...
            if k < 0:
                return tf.subtract(m, tf.matrix_band_part(input=m, num_lower=(np.abs(k) - 1), num_upper=(-1)),
                  name=name)
...
def prior2weights(prior, exponential=False, min_value=0.1, max_value=None, norm=False):
    """ TODO: finish this
  ...
  """
    ...
    if exponential:
        prior = sorted([(i, p) for i, p in enumerate(prior)], key=(lambda x: x[(-1)]),
          reverse=False)
    ...
    if min_value is not None:
        if max_value is not None:
            ...
            prior = (max_value - min_value) * (prior - np.min(prior)) / (np.max(prior) - np.min(prior)) + min_value
    if norm:
...
def upsample(x, scale, axes, method='nn', name=None):
  ...
  """
    with tf.name_scope(name, 'Upsample'):
        ...
        axes = [1, 2] if axes is None else [i % ndims for i in as_tuple(axes)]
        sorted(axes)
        ...
        if method == 'nn':
            ...
            if method.lower() == 'pad_margin':
                paddings = [[0, 0] if i not in axes else [tf.cast(tf.ceil(input_shape[i] * (scale_map[i] - 1) / 2), 'int32'), tf.cast(tf.floor(input_shape[i] * (scale_map[i] - 1) / 2), 'int32')] for i in range(ndims)]
            ...
            return set_shape(x, shape=[s * scale_map[i] if is_number(s) else None for i, s in enumerate(input_shape_int)])
...
def dot(x, y, name=None):
  ...
  """
    with tf.name_scope(name, 'DotProduct'):
        shapeX = [tf.shape(x)[i] if d is None else d for i, d in enumerate(x.shape.as_list())]
        shapeY = [tf.shape(y)[i] if d is None else d for i, d in enumerate(y.shape.as_list())]
        ndimX = x.shape.ndims
        ndimY = y.shape.ndims
        if ndimX > 2:
    ...
    with tf.name_scope(name, 'BatchedDot'):
        ...
        if ndimY > 3:
            ...
            outshapeY = [
             shapeY[(-1)]]
        output_shape = shapeX[:-1] + outshapeY
        output = tf.reshape((tf.matmul(x, y)), [i if i is not None else -1 for i in output_shape],
          name=name)
        return output
...
def switch(condition, then_expression, else_expression, name=None):
    with tf.name_scope(name, 'switch'):
        ...
        if cond_ndims > 1:
            if condition.shape[(-1)] != x_shape[(-1)]:
                cond_shape = tf.shape(condition)
                condition = tf.reshape(condition, [cond_shape[i] for i in range(cond_ndims - 1)])
  ...
  """
    with tf.name_scope(name):
        if outdim == 1:
            ...
            input_shape = [tf.shape(x)[i] if d is None else d for i, d in enumerate(x.shape.as_list())]
...
def repeat(x, n, axes=None, name='Repeat'):
  ...
  """
    if axes is not None:
        ...
        return tf.tile(x, multiples=[n[axes.index(i)] if i in axes else 1 for i in range(ndim)],
          name=name)
    else:
        n = int(n)
...
def renorm_rms(X, axis=1, target_rms=1.0, name='RescaleRMS'):
  ...
  """
    with tf.name_scope(name):
        ...
        X_rms = l2norm / D
        X_rms = tf.where((tf.equal(X_rms, 0.0)), x=tf.ones_like(X_rms, dtype=(X_rms.dtype.base_dtype)),
          y=X_rms)
