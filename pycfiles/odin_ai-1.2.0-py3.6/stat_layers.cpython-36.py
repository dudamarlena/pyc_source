...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/odin/networks/stat_layers.py
# Compiled at: 2019-08-29 08:27:43
# Size of source mod 2**32: 11440 bytes
...
class DenseDeterministic(Dense):
    ...
    def __init__(self, units, activation=None, use_bias=True, kernel_initializer='glorot_uniform', bias_initializer='zeros', kernel_regularizer=None, bias_regularizer=None, activity_regularizer=None, kernel_constraint=None, bias_constraint=None, **kwargs):
        (super(DenseDeterministic, self).__init__)(units=units, activation=activation, 
         use_bias=use_bias, 
...
class DenseDistribution(Sequential):
    ...
    def __init__(self, units, posterior, prior=None, activation='linear', use_bias=True, call_mode=Statistic.DIST, posterior_kwargs={}, name='DenseDistribution'):
        if not prior is None:
            ...
            if not isinstance(call_mode, Statistic):
                ...
                if isinstance(posterior, DistributionLambda):
                    ...
                    if isinstance(posterior, type) and issubclass(posterior, DistributionLambda) or isinstance(posterior, Callable):
                        args = inspect.getfullargspec(posterior).args
                        posterior_kwargs = {i:j for i, j in posterior_kwargs.items() if i in args}
                        layer = posterior(units, **posterior_kwargs)
                    else:
                        raise ValueError('No support for posterior of type: %s' % str(type(layer)))
            assert isinstance(layer, DistributionLambda), 'The callable must return instance of DistributionLambda, but given: %s' % str(type(layer))
        ...
        for arg in inspect.getfullargspec(self.__init__).args:
            if arg not in locals():
                raise ValueError('Invalid subclassing %s, the arguments to __init__ must contain DenseDistribution arguments.' % str(self.__class__))

    def get_config(self):
        config = {'name':self.name,  'configs':{'units':self._units, 
    ...
    def from_config(cls, config, custom_objects=None):
        ...
        kw = {i:config['configs'][i] for i in inspect.getfullargspec(cls.__init__).args if i in config['configs']}
    ...
    def _apply_distribution(self, x):
        if hasattr(x, '_distribution'):
            if x._distribution == self._last_distribution:
            ...
            if prior is None:
                ...
                if self._prior is None:
                    ...
                    if self.posterior is None:
                        raise RuntimeError('DenseDistribution must be called to create the distribution before calculating the kl-divergence.')
                kullback_div = kl_divergence(q=(self.posterior), p=prior,
    ...
    def log_prob(self, x):
        ...
        assert self.units == x.shape[(-1)], 'Number of features mismatch, units=%d  input_shape=%s' % (
         self.units, str(x.shape))
        if self.posterior is None:
            raise RuntimeError('DenseDistribution must be called to create the distribution before calculating the log-likelihood.')
