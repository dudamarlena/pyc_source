...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/examples/nist_sre/train_evec.py
# Compiled at: 2019-05-31 02:42:19
...
from helpers import get_model_path, prepare_dnn_data, BATCH_SIZE, EPOCH, LEARNING_RATE, SCORE_SYSTEM_ID, GRADIENT_CLIPPING
EXP_DIR, MODEL_PATH, LOG_PATH = get_model_path(system_name='xvec')
stdio(LOG_PATH)
train, valid, all_speakers = prepare_dnn_data(save_dir=EXP_DIR)
...
if len(all_models) > 0:
    ...
    x_vec = N.deserialize(path=(all_models[SCORE_SYSTEM_ID]), force_restore_vars=True)
else:
    with N.args_scope([
...
optimizer = K.optimizers.RMSProp(lr=LEARNING_RATE, name='Xrms', clipnorm=clipvalue,
  clip_alg='total_norm')
...
updates = optimizer.minimize(loss=ce,
  roles=[
...
f_train = K.function(inputs, [ce, acc, optimizer.norm], updates=updates,
  training=True)
print('Building testing functions ...')
f_score = K.function(inputs, [ce, acc], training=False)
f_z = K.function(inputs=X, outputs=z, training=False)
print('Start training ...')
task = training.MainLoop(batch_size=BATCH_SIZE, seed=1234, shuffle_level=2,
  allow_rollback=False,
  verbose=4)
task.set_checkpoint(path=MODEL_PATH, obj=x_vec, increasing=True,
  max_checkpoint=(-1))
task.set_callbacks([
 training.NaNDetector(task_name='train', patience=(-1)),
 training.CheckpointEpoch(task_name='train', epoch_percent=0.5)])
task.set_train_task(func=f_train, data=train, epoch=EPOCH,
  name='train')
task.set_valid_task(func=f_score, data=valid, freq=training.Timer(percentage=0.5),
  name='valid')
task.run()