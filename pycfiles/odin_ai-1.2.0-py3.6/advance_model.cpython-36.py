...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/odin/networks/advance_model.py
# Compiled at: 2019-08-30 09:59:11
# Size of source mod 2**32: 19977 bytes
from __future__ import absolute_import, division, print_function
import copy, inspect, os, shutil
from tempfile import mkdtemp
from types import ModuleType
from typing import Callable
import dill, tensorflow as tf
...
class AdvanceModel(Model):
    ...
    def optimizer(self):
        if self._optimizer_weights is not None:
            if self._optimizer is not None:
                if len(self._optimizer.weights) == len(self._optimizer_weights):
    ...
    def __getstate__(self):
        ...
        if optimizer is not None:
            optimizer = [optimizer.__class__,
             ...
             optimizer.get_weights() if self._optimizer_weights is None else self._optimizer_weights]
            optimizer = dill.dumps(optimizer)
        return (
         configs, weights, optimizer)
    ...
    """
        history = (super(AdvanceModel, self).fit)(x=x, y=y, 
    ...
    def get_config(self):
        ...
        for key, val in self.__dict__.items():
            if not key in ('_parameters', 'history'):
                if key in self.parameters:
                    ...
                    if isinstance(val, Layer):
                        ...
                        if isinstance(val, tf.Variable):
                            continue
                        if key not in default_keys and 'self.' + key not in default_source and 'self.' + key in source:
                            try:
        ...
        for layer in self.layers:
            layer_configs.append({'class_name':layer.__class__.__name__, 
                    ...
                    if isinstance(obj, type):
                        ...
                        if isinstance(obj, ModuleType):
                            ...
                            if inspect.isfunction(obj) or inspect.ismethod(obj):
                                ...
                                raise ValueError('Cannot process value with type %s for custom_objects' + ' (module, type or callable are supported)' % str(type(obj)))

            else:
                if isinstance(model_objects, dict):
                    custom_objects.update(model_objects)
                else:
                    raise ValueError('Class %s should return custom_objects of type dictionary or list, but the returned value is %s' % (
