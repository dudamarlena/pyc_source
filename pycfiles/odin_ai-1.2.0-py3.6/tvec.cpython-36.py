...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/examples/tidigits/tvec.py
# Compiled at: 2019-05-31 02:42:53
# Size of source mod 2**32: 6828 bytes
...
from odin.utils import Progbar, unique_labels, as_tuple_of_shape, stdio, ctext, args_parse, get_formatted_datetime, batching
from utils import prepare_data, make_dnn_prediction, visualize_latent_space, get_exp_path
...
stdio(LOG_PATH)
train, valid, X_test_name, X_test_true, X_test_data, labels = prepare_data(feat=(args.feat), label=(args.task))
...
z1 = K.ComputationGraph(y_proba).get(roles=(N.Pool), scope='PoolOutput1', beginning_scope=False)[0]
z2 = K.ComputationGraph(y_proba).get(roles=(N.Pool), scope='PoolOutput2', beginning_scope=False)[0]
z3 = K.ComputationGraph(y_proba).get(scope='LatentDense', beginning_scope=False)[0]
...
updates = K.optimizers.Adam(lr=0.001).minimize(loss=ce,
...
task.set_valid_task(func=f_score, data=valid, freq=training.Timer(percentage=0.8),
  name='valid')
task.run()
y_pred_proba, Z1_test, Z2_test, Z3_test = make_dnn_prediction(functions=[
 f_pred_proba, f_z1, f_z2, f_z3],
  X=X_test_data,
  title='TEST')
print('Test Latent:', Z1_test.shape, Z2_test.shape, Z3_test.shape)
y_pred = np.argmax(y_pred_proba, axis=(-1))
evaluate(y_true=X_test_true, y_pred_proba=y_pred_proba, labels=labels, title='Test set (Deep prediction)',
...
plda = PLDA(n_phi=200, random_state=(K.get_rng().randint(1000000000.0)), n_iter=12,
  labels=labels,
  verbose=0)
plda.fit(np.concatenate([Z3_train, Z3_valid], axis=0), np.concatenate([y_train, y_valid], axis=0))
y_pred_log_proba = plda.predict_log_proba(Z3_test)
evaluate(y_true=X_test_true, y_pred_log_proba=y_pred_log_proba, labels=labels, title='Test set (PLDA - Latent prediction)',
  path=(os.path.join(EXP_DIR, 'test_latent.pdf')))
visualize_latent_space(X_org=X_test_data, X_latent=Z1_test, name=X_test_name,
  labels=X_test_true,
  title='latent1')
visualize_latent_space(X_org=X_test_data, X_latent=Z2_test, name=X_test_name,
  labels=X_test_true,
  title='latent2')
