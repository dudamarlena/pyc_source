...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/odin/visual/figures.py
# Compiled at: 2019-05-31 02:46:13
# Size of source mod 2**32: 75844 bytes
from __future__ import print_function, absolute_import, division
import os, sys, copy, warnings, colorsys, itertools
...
def generate_random_colors(n, seed=1234, lightness_value=None, return_hsl=False, return_hex=True):
    if seed is not None:
        rand = np.random.RandomState(seed)
    n = int(n)
    colors = []
    all_hue = np.linspace(0.0, 0.88, num=n)
    for i, hue in enumerate(all_hue):
        saturation = 0.6 + rand.rand(1)[0] / 2.5
        if lightness_value is None:
            lightness = 0.25 + rand.rand(1)[0] / 1.4
        else:
        ...
        if return_hsl:
            ...
            colors.append(rgb if not return_hex else '#{:02x}{:02x}{:02x}'.format(int(rgb[0] * 255), int(rgb[1] * 255), int(rgb[2] * 255)))
...
def generate_random_colormaps(n, seed=1234, bicolors=False):
    ...
    interpolate_hsl = lambda h, s, l: [
     ...
     (
      h, l, s),
     (
      h, l - 0.1, min(s + 0.1, 1.0))]
    if bicolors:
        base_colors = generate_random_colors((n * 2), lightness_value=0.5, seed=seed, return_hsl=True)
        base_colors = list(zip(base_colors[:n], base_colors[n:]))
    else:
        base_colors = generate_random_colors(n, lightness_value=0.5, seed=seed, return_hsl=True)
    for i, c in enumerate(base_colors):
        if bicolors:
            cA, cB = c
            colors = [(colorsys.hls_to_rgb)(*i) for i in interpolate_hsl(*cB)[::-1] + interpolate_hsl(*cA)]
        else:
            hue, saturation, lightness = c
            colors = [(colorsys.hls_to_rgb)(*i) for i in interpolate_hsl(*c)]
        color_maps.append(LinearSegmentedColormap.from_list(name=('Colormap%d' % i),
          colors=colors,
          N=256,
          gamma=1))
...
def to_axis(ax, is_3D=False):
  ...
  """
    ...
    if is_3D:
        ...
        assert ax is not None and isinstance(ax, (Axes3D, Number, tuple, list)), 'Axes3D must be used for 3D plot (z is given)'
...
def time_ticks(locs, *args, **kwargs):
  ...
  """
    ...
    if n_ticks is not None:
        positions = np.linspace(0, (len(locs) - 1), n_ticks, endpoint=True).astype(int)
        locs = locs[positions]
        times = times[positions]
    formats = {'ms':lambda t: '{:d}ms'.format(int(1000.0 * t)), 
     's':'{:0.2f}s'.format, 
     'm':lambda t: '{:d}:{:02d}'.format(int(t / 60.0), int(np.mod(t, 60.0))), 
     'h':lambda t: '{:d}:{:02d}:{:02d}'.format(int(t / 3600.0), int(np.mod(t / 60.0, 60.0)), int(np.mod(t, 60.0)))}
    if time_fmt is None:
        if max(times) > 3600.0:
            time_fmt = 'h'
        else:
...
def tile_raster_images(X, tile_shape=None, tile_spacing=(2, 2), spacing_value=0.0):
...
def plot_gridSpec(nrow, ncol, wspace=None, hspace=None):
  ...
  """
    from matplotlib import pyplot as plt
    grid = plt.GridSpec(nrows=nrow, ncols=ncol, wspace=wspace,
      hspace=hspace)
    yield grid
...
def plot_vline(x, ymin=0.0, ymax=1.0, color='r', ax=None):
    from matplotlib import pyplot as plt
...
def plot_comparison_track(Xs, legends, tick_labels, line_colors=None, line_styles=None, linewidth=1.0, marker_size=33, marker_styles=None, fontsize=10, draw_label=True, title=None):
  ...
  """
    if len(Xs) != len(legends):
        raise ValueError('Number of series (len(Xs)) is: %d different from number of legends: %d' % (
         len(Xs), len(legends)))
    else:
        nb_series = len(Xs)
        if len(Xs[0]) != len(tick_labels):
            raise ValueError("Number of points for each series is: %d different from number of xticks' labels: %d" % len(Xs[0], len(tick_labels)))
        nb_points = len(Xs[0])
        from matplotlib import pyplot as plt
        default_marker_styles = [
         'o', '^', 's', '*', '+', 'X', '|', 'D', 'H', '8']
        if marker_styles is None:
            if nb_series <= len(default_marker_styles):
        ...
        for idx, X in enumerate(Xs):
            ...
            for i, j in enumerate(X):
                ...
                if draw_label:
                    ax.text(i, (1.01 * j), s=(str(j)), ha='center', va='bottom', fontsize=fontsize)

            inited = True

        plt.gca().set_xticks(np.arange(len(tick_labels)))
        plt.gca().set_xticklabels(tick_labels, rotation=(-60), fontsize=fontsize)
        plt.legend(handlers, legends, bbox_to_anchor=(1.05, 1),
          loc=2,
          borderaxespad=0.0,
          fontsize=fontsize)
        if title is not None:
...
def plot_histogram(x, bins=80, ax=None, normalize=False, range_0_1=False, kde=False, covariance_factor=None, color='blue', color_kde='red', alpha=0.6, centerlize=False, linewidth=1.2, fontsize=12, title=None):
    """
  x: histogram
  ...
  """
    if isinstance(x, (tuple, list)):
        ...
        ax.bar(((hist_bins[:-1] + hist_bins[1:]) / 2 - width / 2), hist, width=width,
          color=color,
          alpha=alpha)
        ...
        if kde:
            if not normalize:
                ...
                if centerlize:
                    xx = np.linspace(np.min(x) - np.abs(max_val) / 2, np.max(x) + np.abs(max_val) / 2, 100)
                else:
                    xx = np.linspace(np.min(x), np.max(x), 100)
            yy = density(xx)
            ax.plot(xx, yy, color=color_kde,
...
def plot_histogram_layers(Xs, bins=50, ax=None, normalize=False, range_0_1=False, kde=False, covariance_factor=None, layer_name=None, layer_color=None, legend_loc='upper center', legend_ncol=5, legend_colspace=0.4, grid=True, fontsize=12, title=None):
    """
  ...
  """
    if isinstance(Xs, np.ndarray):
        ...
        layer_name = _check_arg_length(layer_name, n=num_classes, dtype=string_types,
          default='',
          converter=(lambda x: str(x)))
        layer_color = _check_arg_length(layer_color, n=num_classes, dtype=string_types,
          default='blue',
          converter=(lambda x: str(x)))
        legends = []
        for name, a, c, z, x in zip(layer_name, np.linspace(0.6, 0.9, num_classes)[::-1], layer_color, np.linspace(0, 100, num_classes), Xs):
            if range_0_1:
                x = (x - np.min(x, axis=0, keepdims=True)) / (np.max(x, axis=0, keepdims=True) - np.min(x, axis=0, keepdims=True))
            hist, hist_bins = np.histogram(x, bins=bins, density=normalize)
            width = (hist_bins[1] - hist_bins[0]) / 1.36
            _ = ax.bar(left=((hist_bins[:-1] + hist_bins[1:]) / 2 - width / 2), height=hist,
              width=width,
              zs=z,
              zdir='y',
              color=c,
            ...
            if kde:
                ...
                ax.plot(xs=xx, ys=zz, zs=yy, color=c,
                  alpha=a,
                  linewidth=1.2,
                  linestyle='-.')
            if len(name) > 0:
        ...
        if len(legends) > 0:
            legends = ax.legend([i[1] for i in legends], [i[0] for i in legends], markerscale=1.5,
              scatterpoints=1,
...
def _parse_scatterXYZ(x, y, z):
    if not x is not None:
        raise AssertionError('`x` cannot be None')
    else:
        x = np.squeeze(x)
        if y is not None:
            y = np.squeeze(y)
...
def _validate_color_marker_size_legend(n_samples, color, marker, size, is_colormap=False):
    ...
    if isinstance(color, (string_types, LinearSegmentedColormap)):
        ...
        if isinstance(marker, string_types):
            ...
            marker = None
        default_size = 8
        legend = [[None] * n_samples,
         [
          None] * n_samples,
         [
          None] * n_samples]
        seed = 1234
        create_label_map = lambda labs, def_val, fn_gen: {labs[0]: def_val} if len(labs) == 1 else {i:j for i, j in zip(labs, fn_gen((len(labs)), seed=seed))}
        if color is None:
            color = [
             0] * n_samples
        else:
    ...
    if isinstance(size, Number):
        ...
        if size is None:
            ...
            legend[2] = size
        assert len(color) == n_samples, 'Given %d variable for `color`, but require %d samples' % (len(color), n_samples)
        assert len(marker) == n_samples, 'Given %d variable for `marker`, but require %d samples' % (len(marker), n_samples)
        assert len(size) == n_samples, 'Given %d variable for `size`, but require %d samples' % (len(size), n_samples)
    color_labels = np.unique(color)
    color_map = create_label_map(color_labels, default_color, generate_random_colormaps if is_colormap else generate_random_colors)
...
def _downsample_scatter_points(x, y, z, n_samples, *args):
    ...
    if n_samples is not None:
        if n_samples < len(x):
            ...
            if z is not None:
                z = np.array(z)[ids]
            args = [np.array(a)[ids] if isinstance(a, (tuple, list, np.ndarray)) else a for a in args]
    return [
     len(x), x, y, z] + args
...
def plot_scatter_layers(x_y_val, ax=None, layer_name=None, layer_color=None, layer_marker=None, size=4.0, z_ratio=4, elev=None, azim=88, ticks_off=True, grid=True, surface=True, wireframe=False, wireframe_resolution=10, colorbar=False, colorbar_horizontal=False, legend_loc='upper center', legend_ncol=3, legend_colspace=0.4, fontsize=8, title=None):
  ...
  """
    ...
    layer_name = _check_arg_length(dat=layer_name, n=num_classes, dtype=string_types,
      default='',
      converter=(lambda x: str(x)))
    layer_color = _check_arg_length(dat=layer_color, n=num_classes, dtype=string_types,
      default='Blues',
      converter=(lambda x: plt.get_cmap(str(x))))
    layer_marker = _check_arg_length(dat=layer_marker, n=num_classes, dtype=string_types,
      default='o',
      converter=(lambda x: str(x)))
    size = _check_arg_length(dat=size, n=num_classes, dtype=Number,
      default=4.0,
      converter=(lambda x: float(x)))
    legends = []
    for idx, (alpha, z) in enumerate(zip(np.linspace(0.05, 0.4, num_classes), np.linspace(min_z / 4, max_z / 4, num_classes))):
        x, y, val = x_y_val[idx]
        num_samples = len(x)
        z = np.full(shape=(num_samples,), fill_value=z)
        _ = ax.scatter(x, y, z, c=val, s=(size[idx]), marker=(layer_marker[idx]), cmap=(layer_color[idx]))
        if surface or wireframe:
            x, y = np.meshgrid(np.linspace(min(x), max(x), wireframe_resolution), np.linspace(min(y), max(y), wireframe_resolution))
            z = np.full_like(x, fill_value=(z[0]))
            if surface:
                ax.plot_surface(X=x, Y=y, Z=z, color=(layer_color[idx](0.5)),
                  edgecolor='none',
                  alpha=alpha)
            if wireframe:
                ax.plot_wireframe(X=x, Y=y, Z=z, linewidth=0.8, color=(layer_color[idx](0.8)),
                  alpha=(alpha + 0.1))
            name = layer_name[idx]
            if len(name) > 0:
                legends.append((name, _))
            if colorbar:
                cba = plt.colorbar(_, shrink=0.5, pad=0.01, orientation=('horizontal' if colorbar_horizontal else 'vertical'))
                if len(name) > 0:
    ...
    if len(legends) > 0:
        legends = ax.legend([i[1] for i in legends], [i[0] for i in legends], markerscale=1.5,
          ...
          handletextpad=0.1)
        for i, c in enumerate(layer_color):
            legends.legendHandles[i].set_color(c(0.8))
    ...
    if elev is not None or azim is not None:
        ax.view_init(elev=(ax.elev if elev is None else elev), azim=(ax.azim if azim is None else azim))
    return ax


def plot_scatter_heatmap(x, val, y=None, z=None, ax=None, colormap='bwr', marker='o', size=4.0, alpha=0.8, elev=None, azim=None, ticks_off=True, grid=True, colorbar=False, colorbar_horizontal=False, colorbar_ticks=None, legend_enable=True, legend_loc='upper center', legend_ncol=3, legend_colspace=0.4, n_samples=None, fontsize=8, title=None):
    """
  Parameters
  ----------
  ...
  """
    ...
    if not len(x) == len(y) == len(val):
        ...
        min_val = np.min(val)
        max_val = np.max(val)
        assert isinstance(colormap, (string_types, LinearSegmentedColormap)), '`colormap` can be string or instance of matplotlib Colormap, but given: %s' % type(colormap)
    n_samples, x, y, z, val, marker, size = _downsample_scatter_points(x, y, z, n_samples, val, marker, size)
    colormap, marker, size, legend = _validate_color_marker_size_legend(n_samples, colormap, marker, size)
    axes = []
    legend_name = []
    for idx, (style, name) in enumerate(legend.items()):
        ...
        kwargs = {'c':val_, 
         'vmin':min_val,  'vmax':max_val,  'cmap':style[0], 
         'marker':style[1],  's':style[2],  'alpha':alpha}
        if is_3D_mode:
        ...
        if colorbar:
            if idx == 0:
                cba = plt.colorbar(_, shrink=0.99, pad=0.01, orientation=('horizontal' if colorbar_horizontal else 'vertical'))
                if colorbar_ticks is not None:
                    cba.set_ticks(np.linspace(min_val, max_val, num=(len(colorbar_ticks))))
                    cba.set_ticklabels(colorbar_ticks)
                else:
                    cba.set_ticks(np.linspace(min_val, max_val, num=7))
            cba.ax.tick_params(labelsize=fontsize)

    if len(legend_name) > 0:
        if bool(legend_enable):
            legend = ax.legend(axes, legend_name, markerscale=1.5, scatterpoints=1,
              scatteryoffsets=[0.375, 0.5, 0.3125],
              loc=legend_loc,
              bbox_to_anchor=(0.5, -0.01),
              ncol=(int(legend_ncol)),
...
def plot_scatter(x, y=None, z=None, color='b', marker='.', size=4.0, alpha=1, linewidths=None, linestyle='-', facecolors=None, edgecolors=None, elev=None, azim=None, ticks_off=True, grid=True, legend_enable=True, legend_loc='upper center', legend_ncol=3, legend_colspace=0.4, n_samples=None, fontsize=8, ax=None, title=None):
  ...
  """
    ...
    n_samples, x, y, z, color, marker, size = _downsample_scatter_points(x, y, z, n_samples, color, marker, size)
    color, marker, size, legend = _validate_color_marker_size_legend(n_samples, color, marker, size)
    axes = []
    legend_name = []
    for style, name in legend.items():
        ...
        if is_3D_mode:
            _ = ax.scatter(x_, y_, z_, color=(style[0]),
              marker=(style[1]),
              s=(style[2]),
              alpha=alpha,
            ...
            _ = ax.scatter(x_, y_, color=(style[0]),
              marker=(style[1]),
              s=(style[2]),
              alpha=alpha,
              linewidths=linewidths,
...
def plot_text_scatter(X, text, ax=None, font_weight='bold', font_size=8, font_alpha=0.8, elev=None, azim=None, title=None):
    """
  Parameters
  ----------
  X : numpy.ndarray
  ...
  """
    if not X.ndim == 2:
        raise AssertionError('Only support `X` two dimension array, but given: %s' % str(X.shape))
    else:
        if X.shape[1] == 2:
            is_3D = False
        else:
            ...
            assert len(text) == len(X), '`text` length: %d is different from `X` length: %d' % (len(text), len(X))
            from matplotlib import pyplot as plt
            ...
            for x, t in zip(X, text):
                if is_3D:
                    plt.gca().text((x[0]), (x[1]), (x[2]), t, color=(plt.cm.tab20((labels.index(t) + 1) / float(len(labels)))),
                      fontdict=font_dict)
                else:
                    plt.text((x[0]), (x[1]), t, color=(plt.cm.tab20((labels.index(t) + 1) / float(len(labels)))),
                      fontdict=font_dict)
...
def plot_ellipses(mean, sigma, color, alpha=0.75, ax=None):
  ...
  """
    ...
    mean = mean.ravel()
    if not len(mean) == 2:
        raise AssertionError('mean must be vector of size 2')
    elif not sigma.shape == (2, 2):
        raise AssertionError('sigma must be matrix of shape (2, 2)')
    if ax is None:
        ax = plt.gca()
    covariances = sigma ** 2
...
def plot_indices(idx, x=None, ax=None, alpha=0.3, ymin=0.0, ymax=1.0):
    from matplotlib import pyplot as plt
    ax = ax if ax is not None else plt.gca()
    x = range(idx.shape[0]) if x is None else x
    for i, j in zip(idx, x):
        if i:
            ax.axvline(x=j, ymin=ymin, ymax=ymax, color='r',
              linewidth=1,
              alpha=alpha)
...
def plot_multiple_features(features, order=None, title=None, fig_width=4, sharex=False):
  ...
  """
    ...
    if isinstance(features, (tuple, list)):
        ...
        if order is not None:
            ...
            if isinstance(features, OrderedDict):
                ...
                keys = sorted(features.keys() if isinstance(features, Mapping) else [k for k, v in features])
                order = []
    ...
    features = [(name, features[name]) for name in order if name in features if isinstance(features[name], np.ndarray) if features[name].ndim <= 4]
...
def plot_spectrogram(x, vad=None, ax=None, colorbar=False, linewidth=0.5, vmin='auto', vmax='auto', title=None):
    ...
    if x.ndim > 2:
        ...
        if vad is not None:
            ...
            if len(vad) != x.shape[1]:
                raise ValueError('Length of VAD must equal to signal length, but length[vad]={} != length[signal]={}'.format(len(vad), x.shape[1]))
            ...
            img = ax.imshow(x, cmap=colormap, interpolation='kaiser', alpha=0.9, vmin=vmin,
              vmax=vmax,
              origin='lower')
            if vad is not None:
                for i, j in enumerate(vad):
                    if j:
                        ax.axvline(x=i, ymin=0, ymax=1, color='r', linewidth=linewidth, alpha=0.3)
    ...
    if fig is None:
        ...
        if titles is not None:
            ...
            if len(titles) != x.shape[0]:
                raise ValueError('Titles must have the same length withthe number of images!')
...
def plot_Cnorm(cnorm, labels, Ptrue=[0.1, 0.5], ax=None, title=None, fontsize=12):
    from matplotlib import pyplot as plt
    cmap = plt.cm.Blues
    ...
    if len(Ptrue) != cnorm.shape[0]:
        raise ValueError('`Cnorm` was calculated for %d Ptrue values, but given only %d values for `Ptrue`: %s' % (
    ...
    for i, j in itertools.product(range(len(Ptrue)), range(len(labels))):
        ...
        fs = fontsize
        text = '%.2f' % cnorm[(i, j)]
        plt.text(j, i, text, weight=weight,
          color=color,
          fontsize=fs,
    ...
    ax.grid(False)
    title = 'Cnorm: %.6f' % np.mean(cnorm) if title is None else '%s (Cnorm: %.6f)' % (str(title), np.mean(cnorm))
    ax.set_title(title, fontsize=(fontsize + 2), weight='semibold')
    return ax


def plot_confusion_matrix(cm, labels=None, ax=None, fontsize=12, colorbar=False, title=None):
    from matplotlib import pyplot as plt
    cmap = plt.cm.Blues
    ax = to_axis(ax, is_3D=False)
    if labels is None:
    ...
    ax.set_xlabel('Predicted label', fontsize=fontsize)
    worst_index = {i:np.argmax([val if j != i else -1 for j, val in enumerate(row)]) for i, row in enumerate(cm)}
...
def _ppndf(cum_prob):
  ...
  """
    ...
    norm_dev[centerindexes] = adj_prob[centerindexes] * (((A3 * R[centerindexes] + A2) * R[centerindexes] + A1) * R[centerindexes] + A0)
    norm_dev[centerindexes] = norm_dev[centerindexes] / ((((B4 * R[centerindexes] + B3) * R[centerindexes] + B2) * R[centerindexes] + B1) * R[centerindexes] + 1.0)
...
def plot_detection_curve(x, y, curve, xlims=None, ylims=None, ax=None, labels=None, legend=True, title=None, linewidth=1.2, pointsize=8.0):
    """
  Parameters
  ----------
  ...
  """
    ...
    if curve == 'det':
        ...
        xticklabels = [str(i)[:-2] if '.0' == str(i)[-2:] else str(i) if i > 99.99 else str(i) for i in xticks * 100]
        if xlims is None:
            xlims = (
             max(min(np.min(i) for i in x), xticks[0]),
             min(max(np.max(i) for i in x), xticks[(-1)]))
        xlims = ([val for i, val in enumerate(xticks) if val <= xlims[0] or i == 0][(-1)] + eps, [val for i, val in enumerate(xticks) if val >= xlims[1] or i == len(xticks) - 1][0] - eps)
        if ylims is None:
            ylims = (
             max(min(np.min(i) for i in y), xticks[0]),
             min(max(np.max(i) for i in y), xticks[(-1)]))
        ylims = ([val for i, val in enumerate(xticks) if val <= ylims[0] or i == 0][(-1)] + eps, [val for i, val in enumerate(xticks) if val >= ylims[1] or i == len(xticks) - 1][0] - eps)
        xticks = _ppndf(xticks)
        yticks, yticklabels = xticks, xticklabels
        xlims, ylims = _ppndf(xlims), _ppndf(ylims)
        name_fmt = lambda name, dcf, eer: 'EER=%.2f;minDCF=%.2f' % (eer * 100, dcf * 100) if name is None else '%s (EER=%.2f;minDCF=%.2f)' % (name, eer * 100, dcf * 100)
        labels_new = []
        for count, (Pfa, Pmiss, name) in enumerate(zip(x, y, labels)):
            ...
            lines.append(((Pfa, Pmiss),
             {'lw':linewidth, 
              'label':name,  'linestyle':'-' if count % 2 == 0 else '-.'}))
        ...
        if curve == 'roc':
            ...
            xlims = (0, 1)
            ylims = (0, 1)
            name_fmt = lambda name, auc: 'AUC=%.2f' % auc if name is None else '%s (AUC=%.2f)' % (name, auc)
            labels_new = []
    ...
    if legend:
        if any(i is not None for i in labels):
            plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.0)


def plot_colorbar(colormap, vmin=0, vmax=1, ax=None, orientation='vertical', tick_location=None, tick_labels=None, label=None):
    """
  ...
  """
    ...
    if ax is None or isinstance(ax, mpl.figure.Figure):
        ...
        if isinstance(ax, mpl.axes.Axes):
            ...
            mappable.set_array([])
            cb1 = plt.colorbar(mappable, ax=ax, pad=(0.03 if orientation == 'vertical' else 0.1),
              shrink=0.7,
              aspect=25)
        else:
...
def plot_save(path='/tmp/tmp.pdf', figs=None, dpi=180, tight_plot=False, clear_all=True, log=False):
    """
  Parameters
  ----------
  clear_all: bool
...
def plot_save_show(path, figs=None, dpi=180, tight_plot=False, clear_all=True, log=True):
