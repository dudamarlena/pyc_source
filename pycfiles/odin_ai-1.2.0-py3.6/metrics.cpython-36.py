...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/odin/backend/metrics.py
# Compiled at: 2019-09-17 06:02:11
# Size of source mod 2**32: 22075 bytes
...
def binary_accuracy(y_true, y_pred, threshold=0.5, reduction=tf.reduce_mean, name=None):
    """ Non-differentiable """
    with tf.name_scope(name, 'binary_accuracy', [y_pred, y_true, threshold]):
        ...
        match_values = tf.cast((tf.equal(tf.cast(y_pred, 'int32'), tf.cast(y_true, 'int32'))),
          dtype='int32')
        return reduction(match_values)


def categorical_accuracy(y_true, y_pred, top_k=1, reduction=tf.reduce_mean, name=None):
    """ Non-differentiable """
...
def compute_Cavg(y_llr, y_true, cluster_idx=None, Ptrue=0.5, Cfa=1.0, Cmiss=1.0, probability_input=False):
  ...
  """
    ...
    distribution = tf.clip_by_value(tf.reduce_sum(y_true, axis=0), 1e-07, 1000000000.0)
...
def compute_Cnorm(y_true, y_score, Ptrue=[
 0.1, 0.5], Cfa=1.0, Cmiss=1.0, probability_input=False):
  ...
  """
    ...
    if len(y_true) != len(y_score):
        raise ValueError('There are %d partitions for `y_true`, but %d partitions for `y_score`.' % (
    ...
    for scores, labels in zip(y_score, y_true):
        actual_TP_per_class = np.lib.arraysetops.unique(ar=labels, return_counts=True)[1]
        if probability_input:
    ...
    if y_true.ndim >= 2:
        ...
        if sample_weight is not None:
            if len(sample_weight) != len(y_score):
                raise ValueError('Provided `sample_weight` for %d samples, but got scores for %d samples.' % (
