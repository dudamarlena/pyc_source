...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/examples/nist_sre/analyze.py
# Compiled at: 2019-01-24 05:01:19
# Size of source mod 2**32: 6119 bytes
...
from odin.utils import ctext, mpi, Progbar, catch_warnings_ignore, stdio, get_logpath
from sklearn.metrics import accuracy_score, log_loss, f1_score
from helpers import FEATURE_RECIPE, FEATURE_NAME, PATH_ACOUSTIC_FEATURES, MINIMUM_UTT_DURATION, ANALYSIS_DIR, EXP_DIR, filter_utterances, prepare_dnn_data
...
if not FEATURE_RECIPE.replace('_', '') in os.path.basename(BASE_DIR):
    raise AssertionError
else:
    assert FEATURE_NAME in os.path.basename(BASE_DIR)
    all_model = sorted([name for name in os.listdir(BASE_DIR) if 'model.ai.' in name],
      key=(lambda x: int(x.split('.')[(-1)])))
    assert len(all_model) > 0, 'Cannot find any model.ai. at path: %s' % BASE_DIR
MODEL = os.path.join(BASE_DIR, all_model[(-1)])
stdio(get_logpath(name='analyze.log', increasing=True, odin_base=False,
  root=ANALYSIS_DIR))
...
train, valid, all_speakers, ds = prepare_dnn_data(save_dir=BASE_DIR, feat_name=feat_name,
  utt_length=(int(utt_length)),
...
def make_prediction(feeder, title):
    ...
    y_pred = []
    y_true = []
    for name, idx, X, y in feeder.set_batch(batch_size=100000, batch_mode='file',
      seed=None,
      shuffle_level=0):
