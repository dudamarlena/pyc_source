...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/odin/visual/bashplot.py
# Compiled at: 2019-07-08 07:47:15
# Size of source mod 2**32: 26700 bytes
  ...
  """
    ...
    if len(graphs) == 1:
        ...
        def normalizing_lines(text):
            ...
            lines = [l + ' ' * (maxlen - len(l)) if len(l) < maxlen else l for l in lines]
            return lines
...
def print_dist(d, height=12, pch='o', show_number=False, title=None):
  ...
  """
    ...
    try:
        if isinstance(d, Mapping):
            d = d.items()
        orig_d = [(str(name), int(count)) for name, count in d]
        d = [(str(name)[::-1].replace('-', '|').replace('_', '|'), count) for name, count in d]
        ...
        raise ValueError('`d` must be distribution dictionary contains pair of: label_name -> disitribution_count, error: "%s"' % str(e))

    nb_lines = int(height) + 1 + 1 + max_labels
    unit = (max_count - min_count) / height
    fig = ''
    fig += ctext('Unit: ', 'red') + '10^%d' % max(len(str(max_count)) - MAXIMUM_YLABEL, 0) + '  '
    fig += ctext('Total: ', 'red') + str(sum(count for name, count in d)) + '\n'
    for line in range(nb_lines):
        value = max_count - unit * line
        if line % 2 == 0:
            if line <= int(height):
                fig += ctext((('%' + str(MAXIMUM_YLABEL) + 's') % str(int(value))[:MAXIMUM_YLABEL]),
                  color='red')
        else:
            fig += ' ' * MAXIMUM_YLABEL
        fig += '|' if line <= int(height) else ' '
        if line == int(height):
            fig += ''.join([ctext((pch + ' '), color=(LABEL_COLOR[(i % len(LABEL_COLOR))])) for i in range(len(d))])
        else:
            if line == int(height) + 1:
                ...
                if line > int(height) + 1:
                    for i, lab in enumerate(labels):
                        fig += ctext(' ' if len(lab) == 0 else lab.pop(), LABEL_COLOR[(i % len(LABEL_COLOR))]) + ' '

                else:
                    for i, (name, count) in enumerate(d):
                        fig += ctext(pch if count - value >= 0 else ' ', LABEL_COLOR[(i % len(LABEL_COLOR))]) + ' '
...
def print_confusion(arr, labels=None, side_bar=True, inc_stats=True, float_precision=2):
    """
  Parameters
  ----------
  ...
  """
    ...
    if arr.shape[0] != arr.shape[1]:
        ...
        if inc_stats or side_bar:
            for i in range(nb_classes):
                TP = arr[(i, i)]
                FN = arr_sum_row[i] - arr[(i, i)]
                FP = arr_sum_col[i] - arr[(i, i)]
                precision = 0.0 if TP + FP == 0 else TP / (TP + FP)
                recall = 0.0 if TP + FN == 0 else TP / (TP + FN)
                f1 = 0.0 if precision == 0.0 or recall == 0.0 else 2 / (1 / precision + 1 / recall)
                fa = 0.0 if total_samples - arr_sum_row[i] == 0 else FP / (total_samples - arr_sum_row[i])
                info[i] = (precision, recall, f1, fa, arr_sum_row[i])

        with warnings.catch_warnings():
            warnings.simplefilter('ignore')
            arr = np.nan_to_num(arr.astype('float64') / arr_sum_row[:, None])
        fig = ' ' * (4 * nb_classes + max_label_length + 2)
        if side_bar:
    ...
    for row_id, row in enumerate(arr):
        ...
        if row[most_misclassified] == 0.0:
            most_misclassified = None
        for col_id, col in enumerate(row):
            col = _float2str(col, fp=float_precision)
            if col_id == row_id:
                row_text += ctext(col, color=('blue' if col == '1.0' else 'cyan')) + ' '
            elif most_misclassified is not None:
                if col_id == most_misclassified:
                    row_text += ctext(col, color='red') + ' '
            else:
                row_text += col + ' '

        if side_bar:
            info_str = [_float2str(val) if i < nb_info - 1 else '%d' % np.round(val) for i, val in enumerate(info[row_id])]
            fig += row_text + ' ' + '|'.join(info_str) + '\n'
        else:
            fig += row_text + '\n'

    labels = [[c for c in i.replace('-', '|').replace('_', '|')[::-1]] for i in labels]
...
def print_hist(f, height=20.0, bincount=None, binwidth=None, pch='o', colour='default', title='', xlab=True, showSummary=False, regular=False):
    """ Plot histogram.
   1801|       oo
   1681|       oo
   1561|      oooo
...
def print_bar(f, height=20.0, bincount=None, binwidth=None, pch='o', colour='default', title='', xlab=None, showSummary=False, regular=False):
    """ Plot bar.
    ...
    if len(f) == 1:
        ...
        if pch is None:
            ...
            if numbers[0].is_integer():
                int_mode = True
            min_orig = min(numbers)
            max_orig = max(numbers)
            numbers = [1000 * (i - min_orig) / (max_orig - min_orig + 1e-08) for i in numbers]
            n = len(numbers)
            min_val = min(numbers)
            max_val = max(numbers)
            mean = sum(numbers) / n
            ...
            bins = list(calc_bins(n, 0.0, n + 0.0, bincount, binwidth))
            ...
            for y in ys:
                if int_mode:
                    ylab = '%d' % int(y * (max_orig - min_orig + 1e-08) / 1000 + min_orig)
                else:
                    ylab = '%.3f' % float(y * (max_orig - min_orig + 1e-08) / 1000 + min_orig)
                if ylab in used_labs:
                    ylab = ''
                else:
                    used_labs.add(ylab)
...
def print_scatter(xs, ys, size=None, pch='o', colour='red', title=None):
    """ Scatter plot.
  ----------------------
  |                 *   |
  |               *     |
