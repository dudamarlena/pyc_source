...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/odin/stats.py
# Compiled at: 2019-07-29 09:33:45
# Size of source mod 2**32: 15926 bytes
...
def prior2weights(prior, exponential=False, min_value=0.1, max_value=None, norm=False):
  ...
  """
    ...
    prior = 1.0 / nonzero_prior * np.max(nonzero_prior)
    if exponential:
...
def _split_list(x, rng, train=0.6, idfunc=None, inc_test=True):
    ...
    if N == 1:
        ...
        rets = (
         flatten_list((x_id[i] for i in id_list[:train]), level=1),
        ...
        if inc_test:
            ...
            rets += ([],)
        assert sum(len(r) for r in rets) == len(x), 'Number of returned data inconsitent from original data, %d != %d' % (
         sum(len(r) for r in rets), len(x))
    return rets


def train_valid_test_split(x, train=0.6, cluster_func=None, idfunc=None, inc_test=True, seed=None):
  ...
  """
    ...
    for name, clus in clusters.items():
        _1, _2, _3 = _split_list(clus, rng, train=train, idfunc=idfunc, inc_test=inc_test)
        train_list += _1
        valid_list += _2
        test_list += _3
...
def freqcount(x, key=None, count=1, normalize=False, sort=False, pretty_return=False):
    """ x: list, iterable
  ...
  """
    ...
    freq = OrderedDict([(k, freq[k] / s if normalize else freq[k]) for k in sorted(freq.keys())])
    if sort:
...
def sampling_iter(it, k, p=None, return_iter=True, seed=1234, progress_bar=None):
  ...
  """
    ...
    if progress_bar is not None:
        ...
        if not isinstance(progress_bar, Progbar):
            raise AssertionError('`progress_bar` must be instance of odin.utils.progbar.Progbar, but given: %s' % str(type(progress_bar)))
    if p is None:
        random.seed(seed)
        ret = []
...
def classification_diagnose(X, y_true, y_pred, num_samples=8, return_list=False, top_n=None, seed=1234):
    """
  Return
  ------
  ...
  """
    if y_true.ndim == 2:
        ...
        if y_true.ndim != 1:
            raise ValueError('Only support 1-D or 2-D one-hot `y_true`, given `y_true` with shape: %s' % str(y_true.shape))
        if y_pred.ndim == 2:
            y_pred = np.argmax(y_pred, axis=(-1))
        elif y_pred.ndim != 1:
            raise ValueError('Only support 1-D or 2-D one-hot `y_pred`, given `y_pred` with shape: %s' % str(y_pred.shape))
    if not len(y_true) == len(y_pred) == len(X):
        raise AssertionError('Inconsistent number of samples')
    else:
        ...
        outputs = OrderedDict()
        for (true, pred), samples in sorted((miss.items()), key=(lambda x: len(x[(-1)])),
          reverse=True):
            rand.shuffle(samples)
            outputs[(true, pred)] = X[samples[:num_samples]] if isinstance(X, np.ndarray) else [X[i] for i in samples]

        if top_n is not None:
            if isinstance(top_n, Number):
                top_n = int(top_n)
                assert top_n >= 1
...
def classification_report(y_pred, y_true, labels):
  ...
  """
    ...
    s += 'Report:\n'
    s += str(classification_report(y_true, y_pred, labels=labels, digits=3, target_names=target_names))
    return s
...
def summary(x, axis=None, shorten=False, float_precision=2):
  ...
  """
    if isinstance(x, Iterator):
        ...
        if not shorten:
            x = x.ravel()
            samples = ', '.join([str(i) for i in np.random.choice(x, size=(min(8, len(x))), replace=False).tolist()])
            s += '***** Summary *****\n'
            s += '    Min : %s\n' % (fmt % min_)
            s += '1st Qu. : %s\n' % (fmt % qu1)
            s += ' Median : %s\n' % (fmt % median)
