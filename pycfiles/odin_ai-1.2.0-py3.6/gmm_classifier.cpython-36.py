...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/odin/ml/gmm_classifier.py
# Compiled at: 2019-05-31 02:46:17
...
class GMMclassifier(BaseEstimator, ClassifierMixin, Evaluable):
    ...
    def __getstate__(self):
        if not self.is_fitted:
            raise RuntimeError('The GMMclassifier have not been fitted, nothing to pickle!')
    ...
    def __setstate__(self, states):
        self._strategy, self._n_components, self._covariance_type, self._max_iter, self._n_init, self._init_params, self._labels, self._feat_dim, self._gmm, self._normalizer = states
            ...
            if isinstance(y, (tuple, list)):
                ...
                if y is not None:
                    ...
                    if self.feat_dim != feat_dim:
                        raise ValueError('Initialized with `feat_dim`=%d, given data with %d dimensions' % (
                         self.feat_dim, feat_dim))
                    if self.nb_classes != len(classes):
                        raise ValueError('Initialized with `nb_classes`=%d, given data with %d classes' % (
            ...
            if self._gmm is None:
                if self._strategy == 'ova':
                    ...
                    for n_components in as_tuple((self._n_components), t=int, N=(self.nb_classes)):
                        gmm = GaussianMixture(n_components=n_components, covariance_type=(self._covariance_type),
                          max_iter=(self._max_iter),
                          n_init=(self._n_init),
                          init_params=(self._init_params),
                          random_state=(rand.randint(0, 1000000000.0)))
                        self._gmm.append(gmm)

                else:
                    if self._strategy == 'all':
                        gmm = GaussianMixture(n_components=(self.nb_classes), covariance_type=(self._covariance_type),
                          max_iter=(self._max_iter),
                          n_init=(self._n_init),
                          init_params=(self._init_params),
    ...
    def score_samples(self, X):
        X = self.initialize(X)
        if self._strategy == 'ova':
            scores = np.concatenate([gmm.score_samples(X)[:, None] for k, gmm in enumerate(self._gmm)],
              axis=(-1))
        else:
            if self._strategy == 'all':
