...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/odin/ml/scoring.py
# Compiled at: 2019-01-24 05:01:19
# Size of source mod 2**32: 10299 bytes
...
def compute_class_avg(X, y, classes, sorting=True):
  ...
  """
    ...
    return np.concatenate([np.mean((X[(y == i)]), axis=0, keepdims=True) for i in classes],
      axis=0)
...
class VectorNormalizer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        if self._centering:
            ...
            if self._wccn:
                W = compute_wccn(X, y, classes=None, class_avg=enroll)
            else:
                W = 1
        self._W = W
        if self._wccn:
...
class Scorer(BaseEstimator, TransformerMixin, Evaluable):
    ...
    def __init__(self, centering=True, wccn=True, lda=True, concat=False, method='cosine', labels=None):
        ...
        if method not in ('cosine', 'svm'):
            raise ValueError('`method` must be one of the following: cosine, svm; but given: "%s"' % method)
        self._method = method
    ...
    def fit(self, X, y):
        if isinstance(X, (tuple, list)):
            ...
            if self.method == 'svm':
                ...
                X = 2 * (X - self._normalizer.vmin) / (self._normalizer.vmax - self._normalizer.vmin) - 1
                self._svm = SVC(C=1, kernel='rbf', gamma='auto', coef0=1, shrinking=True,
                  random_state=0,
                  probability=True,
                  tol=0.001,
                  cache_size=10000.0,
