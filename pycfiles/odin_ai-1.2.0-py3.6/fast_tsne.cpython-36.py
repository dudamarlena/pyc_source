...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/odin/ml/fast_tsne.py
# Compiled at: 2019-05-31 02:46:18
...
def fast_tsne(*X, n_components=2, n_samples=None, perplexity=30.0, early_exaggeration=8.0, learning_rate=200.0, n_iter=1000, n_iter_without_progress=300, min_grad_norm=1e-07, metric='euclidean', init='random', verbose=0, random_state=1234, method='barnes_hut', angle=0.5, n_jobs=4):
    """
  Parameters
  ----------
    ...
    if not len(X) > 0:
        ...
        if isinstance(X[0], (tuple, list)):
            ...
            if n_samples is not None:
                ...
                assert n_samples > 0
                new_X = []
                rand = random_state if isinstance(random_state, np.random.RandomState) else np.random.RandomState(seed=random_state)
                for x in X:
            ...
            try:
                ...
                try:
                    ...
                    wprint('Install MulticoreTSNE from `pip install git+https://github.com/DmitryUlyanov/Multicore-TSNE.git` to accelerate the T-SNE on multiple CPU cores.')
                    try:
        ...
        if len(X_new) == 1:
            ...
            mpi = MPI(jobs=X_new, func=apply_tsne, batch=1, ncpu=(min(len(X_new), cpu_count() - 1)))
