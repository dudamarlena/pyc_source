...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/odin/fuel/loaders.py
# Compiled at: 2019-07-22 04:51:40
# Size of source mod 2**32: 15250 bytes
from __future__ import print_function
import os, sys, base64, shutil, inspect
from six import add_metaclass
from zipfile import ZipFile, ZIP_DEFLATED
from abc import ABCMeta, abstractproperty
import numpy as np
from odin.utils.crypto import unzip_aes
from odin.utils import get_file, get_script_path, ctext, get_datasetpath, string_normalize
from odin.fuel.dataset import Dataset

def parse_dataset(dataset_name):
    fn_norm = lambda x: string_normalize(x, lower=True, remove_whitespace='',
      remove_non_alphanumeric=True)
    dataset_name = fn_norm(dataset_name)
    all_datasets = []
    for name, member in inspect.getmembers(sys.modules[__name__]):
        if isinstance(member, type) and issubclass(member, DataLoader) or 'load_' == name[:5]:
    ...
    if len(match_dataset) == 0:
        match_dataset = [name for name, _ in all_datasets if dataset_name == name[:len(dataset_name)]]
    if len(match_dataset) == 0:
        raise RuntimeError("Cannot find dataset with name '%s', all available datasets are: %s" % (
         dataset_name,
         ', '.join([i[0] for i in all_datasets])))
    if len(match_dataset) > 1:
        raise RuntimeError("Found multiple dataset for name: '%s', all the candidates are: %s" % (
         dataset_name,
...
class DataLoader(object):
    ...
    def get_name(clazz, ext=''):
        name = clazz.__name__
        name = name if ext is None or len(ext) == 0 else '_'.join([name, ext])
        return name
    ...
    def get_dataset(clazz, ext='', override=False):
        ...
        if os.path.isfile(out_path):
            raise RuntimeError('Found a file at path: %s, we need a folder to unzip downloaded files.' % out_path)
        else:
        ...
        if md5_checksum is not None:
            if not ds.md5 == md5_checksum:
                raise AssertionError('MD5 checksum mismatch for dataset: %s' % ds.path)
        return ds
...
class TIDIGITS(DataLoader):
    __doc__ = ' Audio digits dataset\n  Encrypted and required password for access\n  '

    @classmethod
    def md5(clazz, ext=''):
        return ''
...
class FSDD(object):
    ...
    """
        ...
        if not os.path.exists(dat_path) or len(os.listdir(dat_path)) != 1501:
            if not os.path.exists(zip_path):
                ...
                all_records = [os.path.join(record_path, i) for i in os.listdir(record_path)]
        ...
        if os.path.exists(tmp_path):
            shutil.rmtree(tmp_path)
        all_files = [os.path.join(dat_path, i) for i in os.listdir(dat_path) if '.wav' in i]
        meta = np.genfromtxt((os.path.join(dat_path, 'metadata.csv')), dtype=str,
          delimiter=',')
...
class openSMILEsad(DataLoader):
    __doc__ = ' This dataset contains 2 files:\n  * lstmvad_rplp18d_12.net\n  * rplp18d_norm.dat\n  '
...
def load_lre_sad():
  ...
  """
    ...
    path = get_file(fname=(os.path.basename(link)), origin=link,
...
def load_sre_list():
    ...
    link = str(base64.decodebytes(link), 'utf-8')
    ds_path = get_datasetpath(name='SRE_FILES', root='~', is_folder=False,
      override=False)
    if os.path.exists(ds_path):
        if len(os.listdir(ds_path)) != 24:
...
def load_voxceleb_list():
    ...
    link = str(base64.decodebytes(link), 'utf-8')
    ds_path = get_datasetpath(name='voxceleb_lists', root='~', is_folder=False,
      override=False)
    if not os.path.exists(ds_path):
        path = get_file(fname=(os.path.basename(link)), origin=link,
          outdir=get_datasetpath(root='~'))
