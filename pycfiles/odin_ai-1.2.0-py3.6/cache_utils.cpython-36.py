...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/odin/utils/cache_utils.py
# Compiled at: 2019-08-17 03:53:53
...
def get_cache_memory():
    ...
    if _memory is None:
        _memory = Memory(cachedir=__cache_dir, mmap_mode=None,
          compress=False,
          verbose=0)
    return _memory
  ...
  """
    ...
    """
        return FunctionMaker.create(func,
          'return decorated(%(signature)s)', dict(decorated=(dec(func))),
          __wrapped__=func)

    return decorator_apply(get_cache_memory().cache, function)


__CACHE = defaultdict(lambda : ([], []))
...
def cache_memory(func, *attrs):
  ...
  """
    ...
    if builtins.any(not isinstance(i, string_types) for i in attrs):
        raise ValueError('Tracking attribute must be string represented name of attribute, but given attributes have types: {}'.format(tuple(map(type, attrs))))
    if len(attrs) > 0:
        attrs = sorted(attrs)
        ...
        def wrapper(*args, **kwargs):
            if strict_mode:
                if any(isinstance(a, string_types) and a == '__cache__' for a in args):
                    args = tuple([a for a in args if not isinstance(a, string_types) or a != '__cache__'])
                else:
