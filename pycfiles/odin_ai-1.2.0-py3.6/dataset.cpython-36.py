...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/odin/fuel/dataset.py
# Compiled at: 2019-09-24 10:12:51
# Size of source mod 2**32: 19445 bytes
...
from odin.fuel.mmap_array import MmapArray, MmapArrayWriter, read_mmaparray_header
from odin.utils import Progbar, UnitTimer, as_tuple, ctext, eprint, flatten_list, get_file, is_callable, is_string, wprint
__all__ = [
 'Dataset']
...
def _infer_separator(path):
    ...
    with open(path, 'r') as (f):
        ...
        raise RuntimeError('CSV file with the first line: `%s`, cannot match separator in known list: `%s`' % (
         line, str(all_sep)))


_audio_ext = ('.3gp', '.aa', '.aac', '.aax', '.act', '.aiff', '.amr', '.ape', '.au',
              '.awb', '.dct', '.dss', '.dvf', '.flac', '.gsm', '.ivs', '.m4a', '.m4b',
              '.m4p', '.mmf', '.mp3', '.mpc', '.msv', '.nsf', '.ogg,', '.opus', '.raw',
              '.sln', '.tta', '.vox', '.wav', '.wma', '.wv', '.webm', '.sph', '.pcm')
_image_ext = ('.tif', '.tiff', '.gif', '.jpeg', '.jpg', '.jif', '.jfif', '.jp2', '.jpx',
              '.j2k', '.j2c', '.fpx', '.pcd', '.png', '.pdf')
...
def _parse_data_descriptor(path, read_only):
    ...
    if not os.path.isfile(path):
        ...
        if file_ext in _audio_ext:
            return [
             (
              file_name, ('audio', 'unknown', None, path))]
        if file_ext in _image_ext:
            return [
             (
              file_name, ('image', 'unknown', None, path))]
        if file_ext in ('.txt', ):
            return [
             (
              file_name, ('txt', 'unknown', None, path))]
        ...
        try:
            ...
            data = MmapArray(path)
            assert np.dtype(dtype) == data.dtype and shape == data.shape, 'Metadata mismatch for MmapArray'
...
class Dataset(object):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if not is_string(path):
            raise ValueError('`path` for Dataset must be string, but given object with type: %s' % type(path))
        path = os.path.abspath(path)
        if path in Dataset._Dataset__INSTANCES:
            return Dataset._Dataset__INSTANCES[path]
        else:
            new_instance = super(Dataset, cls).__new__(cls)
            Dataset._Dataset__INSTANCES[path] = new_instance
            return new_instance
        ...
        if path is not None:
            ...
            if os.path.isfile(path):
                if '.zip' in os.path.basename(path):
                    self._load_archive(path, extract_path=(path.replace(os.path.basename(path), '')),
                      read_only=read_only)
        ...
        for fname in files:
            if 'readme' == fname[:6].lower():
                ...
                with open(readme_path, 'r') as (readme_file):
                    ...
                    readme = ['  ' + i[:-1] for i in readme if len(i) > 0 if i != '\n']
            ...
            if data is None:
                ...
                for key, d in data:
                    if key in self._data_map:
                        raise ValueError('Found duplicated data with follow info: {}'.format(key))
                    else:
                        self._data_map[key] = d

    def __getstate__(self):
        if not self._new_args_called:
            raise RuntimeError('You must use argument `protocol=cPickle.HIGHEST_PROTOCOL` when using `pickle` or `cPickle` to be able pickling Dataset.')
        self._new_args_called = False
    ...
    def _load_archive(self, path, extract_path, read_only):
        ...
        try:
            ...
            if not os.path.isdir(extract_path):
                raise ValueError('Extract path must be path folder, but path={} is a file'.format(extract_path))
            extract_path = os.path.join(extract_path, os.path.basename(path).replace('.zip', ''))
            if os.path.isdir(extract_path):
                if set(os.listdir(extract_path)) == set(allfile):
                    self._set_path(extract_path, read_only=read_only)
                    return
            if not os.path.exists(extract_path):
                os.mkdir(extract_path)
            maxlen = max([len(i) for i in allfile])
            pb = Progbar(target=(len(allfile)), name='[Dataset] Loading Archive', print_summary=True,
              print_report=True)
            ...
            raise IOError('Error loading archived dataset, path:{}, error:{}.'.format(path, e))
    ...
    def archive(self):
        ...
        prog = Progbar(target=(len(files)), name='[Dataset] Archiving', print_report=True,
          print_summary=True)
        maxlen = max([len(os.path.basename(i)) for i in files])
        for i, f in enumerate(files):
            zfile.write(f, os.path.basename(f))
            prog['Data'] = ('Archiving: %-' + str(maxlen) + 's') % os.path.basename(f)
            prog.add(1)

        zfile.close()
        return path

    def copy(self, destination, indices_filter=None, data_filter=None, override=False):
    ...
    def close(self, name=None):
        if name is None:
            ...
            if self.path in Dataset._Dataset__INSTANCES:
                del Dataset._Dataset__INSTANCES[self.path]
    ...
    """
        ...
        if indices is None:
            raise RuntimeError("Cannot find prefix: '%s' for feature with name: '%s', all available name with given prefix are: %s" % (
             prefix, feat_name,
             ','.join([k for k in self.keys() if prefix == k[:len(k)]])))
        return indices
    ...
    """
        assert isinstance(name, string_types), 'name must be given as string types.'
        path = os.path.join(self.path, name)
        with open(path, 'wb') as (f):
            pickle.dump(value, f)
        self._data_map[name] = (value.dtype if hasattr(value, 'dtype') else str(type(value)),
    ...
    def get_md5_checksum(self, excluded_name=[]):
        ...
        md5_text = ''
        all_data_items = {i:j for i, j in self._data_map.items() if i not in excluded_name}
        for name, (dtype, shape, data, path) in sorted((all_data_items.items()), key=(lambda x: x[0])):
    ...
    def __str__(self):
        padding = '  '
        s = [
         '==========  ' + ctext('Dataset:%s Total:%d Size:%.2f(MB)', 'magenta') % (self.path, len(self._data_map), self.size) + '  ==========']
        s += self._readme_info
        s += [ctext('DATA:', 'yellow'),
         '----']
        longest_name = 0
        ...
        format_str = padding + '%-' + str(longest_name + 2) + 's  %-' + str(longest_dtype) + 's' + ctext(':', 'yellow') + '%-' + str(longest_shape) + 's  path:%-' + str(longest_file) + 's'
