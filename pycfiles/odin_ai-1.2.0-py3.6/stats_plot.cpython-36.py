...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/odin/visual/stats_plot.py
# Compiled at: 2019-08-27 04:32:18
# Size of source mod 2**32: 11212 bytes
    ...
    if fig is None:
        ...
        if isinstance(fig, (tuple, list)):
            ...
            if not isinstance(fig, plt.Figure):
                raise ValueError('fig could be tuple of figsize, None or instance of Figure.')
    ...
    if observed is None:
        if expected is None:
            ...
            if observed is not None:
                if not len(observed) == n:
                    raise AssertionError
            if expected is not None:
                if not len(expected) == n:
                    raise AssertionError
...
def _get_sort_indices(observed, expected, sort_by, sort_ascending):
    if sort_by is not None:
        if 'observed' in str(sort_by).lower():
            ...
            if 'expected' in str(sort_by).lower():
...
def plot_series_statistics(observed=None, expected=None, total_stdev=None, explained_stdev=None, color_set='Set2', xscale='linear', yscale='linear', xlabel='feature', ylabel='value', y_cutoff=None, sort_by='expected', sort_ascending=True, despine=True, legend_enable=True, legend_title=None, legend_loc='best', alpha=None, markersize=1.0, linewdith=1.2, fontsize=8, ax=None, title=None, return_handles=False, return_indices=False):
  ...
  """
    ...
    ax = to_axis2D(ax)
    observed, expected, total_stdev, explained_stdev = _preprocess_series(observed, expected, total_stdev, explained_stdev)
    if isinstance(color_set, (tuple, list)):
        observed_color, expected_color, expected_total_standard_deviations_color, expected_explained_standard_deviations_color = color_set
    else:
    ...
    indices = np.arange(len(observed) if observed is not None else len(expected)) + 1
    handles = []
    if legend_title is not None:
        _, = ax.plot([], marker='None',
          linestyle='None',
            ...
            if expected is not None:
                ...
                if total_stdev is not None:
                    ...
                    ax.fill_between(indices,
                      ...
                      zorder=0,
                      alpha=alpha)
                    _ = matplotlib.patches.Patch(label='Stdev(Total)', color=expected_total_standard_deviations_color)
                    handles.append(_)
                if explained_stdev is not None:
                    ...
                    ax.fill_between(indices,
                      ...
                      zorder=1,
                      alpha=alpha)
                    _ = matplotlib.patches.Patch(label='Stdev(Explained)',
                      color=expected_explained_standard_deviations_color)
                    handles.append(_)
            if legend_enable:
...
def plot_relative_series(X, row_name=None, col_name=None, linestyle='--', linewidth=1, markerstyle='o', markersize=32, grid=True, fontsize=12, text_rotation=0, ax=None):
  ...
  """
    ...
    X = np.asarray(X)
    if not X.ndim == 2:
        raise AssertionError('Matrix must be provided for X, for example, each system in row, and different score type in column')
    else:
        n_row, n_col = X.shape
        if row_name is None:
    ...
    ax.set_yticklabels(['%.2f' % i for i in np.linspace(min_, max_, 5)], fontsize=fontsize)
    if bool(grid):
