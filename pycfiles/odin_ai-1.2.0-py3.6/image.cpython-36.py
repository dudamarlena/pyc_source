...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/odin/preprocessing/image.py
# Compiled at: 2019-01-24 05:01:19
# Size of source mod 2**32: 9793 bytes
...
def apply_transform(x, transform_matrix, fill_mode='nearest'):
  ...
  """
    ...
    channel_images = [ndi.interpolation.affine_transform(x_channel, final_affine_matrix, final_offset, order=0, mode=fill_mode, cval=0.0) for x_channel in x]
...
def read(path, grayscale=False, crop=None, scale=None, target_size=None, transpose=None, resample_mode=2):
    """
  Parameters
  ----------
  grayscale: bool
  ...
  """
    ...
    for t in transpose:
        ...
        if target_size:
            ...
            size = img.size
            if size != target_size:
                new_img = Image.new('L' if grayscale else 'RGB', target_size, 0 if grayscale else (0,
                                                                                                   0,
                                                                                                   0))
