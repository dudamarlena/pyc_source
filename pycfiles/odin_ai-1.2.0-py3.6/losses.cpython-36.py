...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/odin/backend/losses.py
# Compiled at: 2019-07-12 11:22:16
  ...
  **Returns**
  ...
  """
    with tf.name_scope('contrastive_loss_andre'):
        ...
        return (
         loss, between_class, within_class)


def cosine_similarity(y_true, y_pred, weights=1.0, unit_norm=True, one_vs_all=True, name=None):
    """
  Parameters
  ----------
  ...
  """
    with tf.name_scope(name, 'cosine_similarity', (y_true, y_pred, weights)):
        ...
        if unit_norm:
            y_true /= to_nonzeros(tf.linalg.norm(y_true, ord=2, axis=(-1), keep_dims=True), 1.0)
            y_pred /= to_nonzeros(tf.linalg.norm(y_pred, ord=2, axis=(-1), keep_dims=True), 1.0)
        if one_vs_all:
            scores = tf.matmul(tf.transpose(y_true), y_pred)
        else:
            scores = 1 - tf.reduce_sum((tf.multiply(y_true, y_pred)), axis=(-1, ),
              keep_dims=True)
    return scores


def bayes_crossentropy(y_true, y_pred, nb_classes=None, reduction=tf.reduce_mean, name=None):
    with tf.name_scope(name, 'bayes_crossentropy', [y_true, y_pred]):
        ...
        if y_true.shape.ndims == 1:
            if nb_classes is None:
                raise Exception('y_pred and y_true must be one_hot encoded, otherwise you have to provide nb_classes.')
        ...
        prob_distribution = dimshuffle(distribution / tf.reduce_sum(distribution), ('x',
                                                                                    0))
...
def correntropy_regularize(x, sigma=1.0):
    """
  Note
  ----
