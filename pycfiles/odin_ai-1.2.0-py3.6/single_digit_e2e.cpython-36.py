...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/examples/tidigits/single_digit_e2e.py
# Compiled at: 2019-05-31 02:42:43
# Size of source mod 2**32: 14267 bytes
...
from odin.visual import print_dist, print_confusion, print_hist, plot_scatter, plot_figure, plot_spectrogram, plot_save, plot_confusion_matrix, generate_random_colors, generate_random_marker
from odin.utils import get_logpath, get_modelpath, get_datasetpath, get_figpath, Progbar, unique_labels, chain, as_tuple_of_shape, stdio, ctext, ArgController
FEAT = [
 'mspec', 'sad']
...
if not os.path.isdir(path):
    raise AssertionError("Cannot find preprocessed feature at: %s, try to run 'odin/examples/features.py'" % path)
else:
    ds = F.Dataset(path, read_only=True)
    assert all(f in ds for f in FEAT), 'Cannot find features with name: %s' % FEAT
    indices = [(name, (s, e)) for name, (s, e) in list(ds['indices'].items()) if len(name.split('_')[(-1)]) == 1]
    ...
    print(print_dist(d=freqcount(train, key=gender_digit), show_number=True,
      title='Training distribution'))
    print(print_dist(d=freqcount(test, key=gender_digit), show_number=True,
    ...
    train, valid = train_valid_test_split(x=train, train=0.6, inc_test=False, idfunc=split_spkID,
      seed=(K.get_rng().randint(0, 1000000000.0)))
...
recipes = [
 ...
 F.recipes.Sequencing(frame_length=max_length, step_length=1, end='pad',
   pad_mode='post',
   pad_value=0,
   data_idx=None),
...
train = F.Feeder(F.IndexedData(data=data, indices=train), dtype='float32',
  ncpu=6,
  buffer_size=(len(digits)),
  batch_mode='batch')
valid = F.Feeder(F.IndexedData(data=data, indices=valid), dtype='float32',
  ncpu=2,
  buffer_size=(len(digits)),
  batch_mode='batch')
test = F.Feeder(F.IndexedData(data=data, indices=test), dtype='float32',
  ncpu=1,
  buffer_size=1,
  batch_mode='file')
...
inputs = [K.placeholder(shape=((None, ) + shape[1:]), dtype='float32', name=('input%d' % i)) for i, shape in enumerate(train.shape)]
...
f_latent = N.Sequence([
 N.Flatten(outdim=3),
 N.CudnnRNN(num_units=128, num_layers=1, is_bidirectional=False, rnn_mode='lstm')],
  debug=True,
  name='Latent')
...
updates = K.optimizers.RMSProp(lr=0.0001).minimize(loss=ce,
  roles=[K.role.Weight, K.role.Bias])
K.initialize_all_variables()
print('Building training functions ...')
f_train = K.function(inputs=inputs, outputs=[ce, acc, cm], updates=updates,
  training=True)
...
task.set_valid_task(f_test, valid, freq=training.Timer(percentage=0.8),
  name='valid')
task.run()
...
def evaluate_latent(fn, feeder, title):
    y_true = []
    Z = []
    for outputs in Progbar(feeder.set_batch(batch_mode='file'), name=title,
      print_report=True,
      print_summary=False,
    ...
    if Z.shape[(-1)] > 3:
        Z = fast_tsne(Z, n_components=3, n_jobs=8, random_state=(K.get_rng().randint(0, 1000000000.0)))
    ...
    for i, azim in enumerate((15, 60, 120)):
        plot_scatter(x=(Z[:, 0]), y=(Z[:, 1]), z=(Z[:, 2]), ax=(1, 3, i + 1), size=4,
          color=Z_color,
          marker=Z_marker,
          azim=azim,
...
def evaluate_feeder(feeder, title):
    ...
    for gen in range(len(genders)):
        ...
        if len(y_true) == 0:
            ...
            plot_confusion_matrix(cm, labels=digits, fontsize=8, ax=(
             1, 4, gen + 1),
