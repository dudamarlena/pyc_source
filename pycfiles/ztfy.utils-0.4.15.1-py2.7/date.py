...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ztfy/utils/date.py
# Compiled at: 2015-03-26 10:17:45
...
def formatDate(value, format=EXT_DATE_FORMAT, request=None):
    ...
    if value.year >= 1900:
        ...
        return format.replace('%d', str(value.day)).replace('%m', str(value.month)).replace('%Y', str(value.year))
        return
