...
# [GCC 8.4.0]
# Embedded file name: /home/miph/Development/logdog/python-logdog/logdog/roles/pollers/file_watcher.py
# Compiled at: 2015-04-04 17:40:53
...
class FileWatcher(BasePoller):
    defaults = BasePoller.defaults.copy_and_update(poll_sleep_policy=b'utils.policies.growing-sleep', greedy_policy=b'utils.policies.mostly-greedy', greedy_file_reading=True, start_delay=2.5)
    ...
    def __str__(self):
        return (b'WATCHER:{!s}').format(self.input)

    def _prepare_message(self, data):
        msg = super(FileWatcher, self)._prepare_message(data)
        msg.source = getattr(self.input, b'name', msg.source)
        return msg
    ...
    def poll(self):
        ...
        try:
            self.input.open()
            logger.info(b'[%s] Opened %s.', self, self.input)
        except IOError as e:
            logger.error(b'[%s] Stopping watching because of errors (%s)...', self, e)
            self.started = False
            yield self.stop()
            return
        ...
        while self.started:
            ...
            if data:
                ...
                logger.debug(b'[%s] Sleep on watching %ss.', self, self.poll_sleep_policy.cur_interval)
