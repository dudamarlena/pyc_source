...
# [GCC 8.4.0]
# Embedded file name: /home/miph/Development/logdog/python-logdog/logdog/core/config.py
# Compiled at: 2015-04-04 18:00:20
...
class Config(ObjectDict):
    ...
    namespace_default = b'*'
    subconfig_namespace_delimiter = b'@'
    _classes_cache = {}
    _conf_path_cache = {}
    ...
    def find_conf(self, name, fallback=None):
        ...
        if self.subconfig_namespace_delimiter in name:
            ...
            extra_conf_relative = (b'{}.{}{}').format(name, self.subconfig_namespace_delimiter, extra_conf)
        ...
        return (target, name, namespace.split(b','))
        ...
        if _cache is not None:
            ...
            if isinstance(conf, basestring):
                ...
                try:
                    ...
                    if name != b'default':
                        fallback = fallback if fallback else (b'').join(name.rpartition(b'.')[:-1] + ('default', ))
            ...
            if isinstance(cls, basestring):
                try:
                    ...
                    logger.warning(b'Could not import "%s".', cls)

            if b'namespaces' in conf:
                if tuple(namespaces) == (self.namespace_default,):
                    namespaces = tuple(conf[b'namespaces'])
                else:
                    namespaces = tuple(set(namespaces).union(conf[b'namespaces']))
            self._classes_cache[initial_name] = (
             cls, name, conf, tuple(namespaces))
    ...
    def find_and_construct_class(self, name, fallback=None, args=(), kwargs=None):
        ...
        kw = defaults
        kw.update(kw.pop(b'**', {}))
        kw[b'namespaces'] = set(kw.get(b'namespaces', ())).union(ns)
        kw[b'config_name'] = name
        args = kw.pop(b'*', args)
        if hasattr(found_cls, b'factory'):
            found_cls = found_cls.factory
        args = handle_as_list(args)
        if callable(found_cls):
            return found_cls(*args, **kw)
        raise ConfigError((b'{} is not a class. Cannot be imported or called.').format(found_cls))
...
class ConfigLoader(object):
    ...
    def _load_user_config(self):
        if not os.path.exists(self._path):
            raise ConfigError((b'[{}] Config file does not exist.').format(self._path))
        with open(self._path, b'r') as (f):
    ...
    def load_config(self, default_only=False):
        if not default_only:
            user_config = self._load_user_config() if 1 else Config()
            default_config = copy.deepcopy(logdog_default_config)
            raise (isinstance(user_config, dict) or ConfigError)((b'[{}] Invalid config file: must have "key: value" format.').format(self._path))
            ...
            if isinstance(cfg, (str, text_type)):
                ...
                if is_importable(key):
                    try:
                        ...
                        logger.debug(b'[%s] Faced non-importable path: "%s: %s".', self._path, key, ret)
