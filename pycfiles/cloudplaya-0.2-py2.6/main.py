...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cloudplaya/main.py
# Compiled at: 2012-07-12 20:20:44
import os, sys
...
class Authenticate(Command):
    ...
    def add_options(self, parser):
        parser.add_option('--username', default=None, help='the username to log in as')
        parser.add_option('--password', default='%(name)s', help='the password for your user')
        return

    def run(self):
        try:
            if self.client.authenticate(self.options.username, self.options.password):
...
class DownloadAlbum(Command):
    ...
    DEFAULT_FORMAT = os.path.join('%(artist_name)s', '%(album_name)s', '%(track_num)02d. %(title)s.%(extension)s')

    def add_options(self, parser):
        parser.add_option('--format', default=self.DEFAULT_FORMAT, help='format string for filenames')
        parser.add_option('--artist', default=None, help='the artist name for listed songs')
        parser.add_option('--album', default=None, help='the album name for listed songs')
        parser.add_option('-o', '--out-directory', default=None, help='the directory to save the files')
        return
...
class GetAlbums(Command):
    ...
    def add_options(self, parser):
        parser.add_option('--artist', default=None, help='list albums by the given artist')
        parser.add_option('--format', default=self.DEFAULT_FORMAT, help='format string for each listed album')
        return
...
class GetAlbum(Command):
    ...
    def add_options(self, parser):
        parser.add_option('--artist', default=None, help='the name of the artist')
        parser.add_option('--name', default=None, help='the name of the album')
        parser.add_option('--format', default=self.DEFAULT_FORMAT, help='format string for the album')
        return
    ...
    def run(self):
        if not self.options.artist or not self.options.name:
            ...
            sys.exit(1)
        try:
            album = self.client.get_album(self.options.artist, self.options.name)
            self.output_item_formatted(album)
        except RequestError, e:
            sys.stderr.write('Failed to get albums: %s\n' % e)
            sys.exit(1)
...
class GetSongs(Command):
    ...
    def add_options(self, parser):
        ...
        parser.add_option('--artist', default=None, help='the artist name for listed songs')
        parser.add_option('--album', default=None, help='the album name for listed songs')
        parser.add_option('--title', default=None, help='the title of the song')
        return
...
def parse_options(args):
    ...
    parser.add_option('--session-file', default=None, help='the session file to use')
    ...
    parser = OptionParser(usage='%prog ' + command_name + ' [options]', version='%prog ' + VERSION)
...
def main():
    ...
    if not client.authenticated and command != COMMANDS['authenticate']:
        sys.stderr.write('You must authenticate before you can use cloudplaya:\n')
        sys.stderr.write('    $ cloud-playa authenticate --username <username> --password <password>\n')
        sys.exit(2)
