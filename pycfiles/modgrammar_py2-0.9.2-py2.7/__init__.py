...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/modgrammar/__init__.py
# Compiled at: 2012-11-11 04:35:00
import sys, re, textwrap, modgrammar.util
...
class GrammarClass(type):
    ...
    def __reduce__(cls):
        ...
        if lookup == cls:
            ...
            return (
             _gclass_reconstructor, (cls.__name__, cls.__bases__, cdict))
...
class ANY(Terminal):
    ...
    def grammar_parse(cls, text, index, sessiondata):
        ...
        yield (
         1, cls(text.string, index, index + 1))
        yield util.error_result(index, cls)
        return
...
class OR_Operator(Grammar):
    ...
    @classmethod
    def __class_init__(cls, attrs):
        if 'grammar_desc' not in attrs and cls.grammar:
...
class Repetition(Grammar):
    ...
    def grammar_resolve_refs(cls, refmap={}, recurse=True, follow=False, missing_ok=False, skip=None):
        ...
        cls.grammar = util.RepeatingTuple(len=old_grammar.len, *cls.grammar)
    ...
    def grammar_ebnf_lhs(cls, opts):
        ...
        if cls.grammar_min == 0:
            ...
            descs = [
             ('{0} * {1}').format(cls.grammar_min, ename)]
    ...
    def grammar_parse(cls, text, index, sessiondata):
        ...
        if greedy:
            while matchlen >= cls.grammar_min:
                yield (
                 matchlen, cls(string, index, index + matchlen))
                matchlen -= 1
