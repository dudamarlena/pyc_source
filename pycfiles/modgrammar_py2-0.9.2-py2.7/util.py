...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/modgrammar/util.py
# Compiled at: 2012-11-11 04:35:00
import re, traceback, sys, modgrammar
...
_anongrammar_attrs = ('grammar_collapse', 'grammar_desc', 'grammar_name', 'grammar_whitespace',
                      'grammar_tags')
...
def make_classdict(base, grammar, kwargs, **defaults):
    ...
    if 'grammar_whitespace' not in cdict and base.grammar_whitespace is None:
...
def get_calling_module(stack=None):
    ...
    for s in reversed(stack):
        ...
        if __file__.startswith(filename) or modgrammar.__file__.startswith(filename):
            continue
        else:
