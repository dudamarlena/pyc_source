...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/modgrammar/grammar_py2.py
...
class Grammar(object):
    ...
    grammar_hashattrs = ('grammar_name', 'grammar', 'grammar_min', 'grammar_max', 'grammar_collapse',
                         'grammar_greedy', 'grammar_whitespace')
    ...
    def grammar_parse(cls, text, index, sessiondata):
        ...
        while True:
            while True:
                if not greedy and len(objs) >= grammar_min:
                    yield (
                     pos - index, cls(text.string, index, pos, objs))
                    objs = list(objs)
        ...
        elif len(cls.grammar) == 1 and best_error[0] == first_pos and cls.grammar_desc != cls.grammar_name:
            yield util.error_result(index, cls)
        else:
