...
# Compiled at: 2018-09-30 22:34:46
# Size of source mod 2**32: 27594 bytes
...
CLASSIFIER_FN = '/tmp/action-optimizer-last-classifier.pkl.gz'
...
class Optimizer(object):
    ...
    def analyze(self):
        ...
        for column_name, ct in zip(column_names, column_types):
            if not ct in (DATE, NUMERIC):
                assert ct[0] == '{' and ct[(-1)] == '}', 'Invalid type: %s' % ct
                if ct[0] == '{':
        ...
        arff = ArffFile(relation='supplements-training')
        arff.class_attr_name = CLASS_ATTR_NAME
        arff.relation = 'optimizer-training'
        row_count = 0
        ...
        training_fn = os.path.join(BASE_DIR, 'fixtures/supplements.arff')
        print('Writing arff...')
        ...
        if self.no_train:
            assert os.path.isfile(CLASSIFIER_FN), 'If training is disabled, then a classifier file must exist to re-use, but %s does not exist.' % CLASSIFIER_FN
            print('Loading classifier from file %s...' % CLASSIFIER_FN)
            classifier = EnsembleClassifier.load(CLASSIFIER_FN)
        ...
        if abs((last_full_day_date - date.today()).days) > 1 and raw_input('Last full day is over 1 day ago. Continue? [yn]:').lower()[0] != 'y':
            sys.exit(1)
        print('=' * 80)
        print('ranges:')
        ...
        classifier.save(CLASSIFIER_FN)
        print('Classifier saved to %s.' % CLASSIFIER_FN)
        print('=' * 80)
        total = len(queries)
        i = 0
