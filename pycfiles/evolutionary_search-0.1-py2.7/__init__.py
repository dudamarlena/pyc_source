...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/evolutionary_search/__init__.py
# Compiled at: 2016-01-05 19:07:10
...
def _evalFunction(individual, name_values, X, y, scorer, cv, iid, fit_params, verbose=0, error_score='raise'):
    parameters = _individual_to_params(individual, name_values)
    score = 0
    n_test = 0
    for train, test in cv:
        _score, _, _ = _fit_and_score(estimator=individual.est, X=X, y=y, scorer=scorer, train=train, test=test, verbose=verbose, parameters=parameters, fit_params=fit_params, error_score=error_score)
        if iid:
...
class EvolutionaryAlgorithmSearchCV(BaseSearchCV):
    ...
    def __init__(self, estimator, params, scoring=None, cv=4, refit=True, verbose=False, population_size=50, gene_mutation_prob=0.1, gene_crossover_prob=0.5, tournament_size=3, generations_number=10, gene_type=None, n_jobs=1, iid=True, pre_dispatch='2*n_jobs', error_score='raise', fit_params=None):
        super(EvolutionaryAlgorithmSearchCV, self).__init__(estimator=estimator, scoring=scoring, fit_params=fit_params, n_jobs=n_jobs, iid=iid, refit=refit, cv=cv, verbose=verbose, pre_dispatch=pre_dispatch, error_score=error_score)
        self.params = params
    ...
    def _fit(self, X, y, parameter_dict):
        ...
        toolbox.register('evaluate', _evalFunction, name_values=name_values, X=X, y=y, scorer=self.scorer_, cv=cv, iid=self.iid, verbose=self.verbose, error_score=self.error_score, fit_params=self.fit_params)
