...
# Embedded file name: /home/winkidney/.virtualenvs/side-project/lib/python2.7/site-packages/cmdtree/types.py
# Compiled at: 2016-08-30 00:42:53
...
class ParamTypeFactory(object):
    ...
    def convert(self, value):
        raise NotImplementedError(('type converter should be implemented before usedfor value {value}').format(value=value))
...
class IntRange(IntParamType):
    ...
    def convert(self, value):
        ...
        if self.min is not None and rv < self.min or self.max is not None and rv > self.max:
            if self.min is None:
                self.fail('%s is bigger than the maximum valid value %s.' % (
                 rv, self.max))
            elif self.max is None:
                self.fail('%s is smaller than the minimum valid value %s.' % (
                 rv, self.min))
            else:
...
class FloatParamType(ParamTypeFactory):
    ...
    def convert(self, value):
        try:
            ...
            self.fail('%s is not a valid floating point value' % value)
...
class Choices(UnprocessedParamType):
    ...
    def __call__(self):
        ...
        :rtype: dict
        """
        return {'type': self.type or self.convert, 
