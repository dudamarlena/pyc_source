...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/should_dsl/matchers.py
# Compiled at: 2012-10-31 22:44:49
import re, sys, copy
...
def equal_to_ignoring_case():
    ...
    return (
     lambda x, y: lower(x) == lower(y), '%r is %sequal to %r ignoring case')
...
class Have(object):
    ...
    def match(self, lvalue):
        ...
        if hasattr(self._lvalue, self._collection_name):
            ...
            raise TypeError('target does not have a %r collection, nor it is an iterable' % self._collection_name)
...
class Change(object):
    ...
    def message_for_failed_should(self):
        if self._by is not None:
            ...
            if self._only_to:
                ...
                return 'result should have changed, but is still %r' % self._before_result
            return
    ...
    def _to_callable(self, objekt):
        ...
        raise TypeError('parameter passed to change must be a callable ' + 'or a iterable having a callable as its first element')
