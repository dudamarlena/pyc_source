...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/should_dsl/dsl.py
# Compiled at: 2012-07-24 00:14:06
import sys, re
...
class Should(object):
    ...
    def add_matcher(self, matcher_object):
        if hasattr(matcher_object, 'func_name') or isinstance(matcher_object, FunctionType):
            function, message, not_for_should, not_for_should_not = self._process_custom_matcher_function(matcher_object)
            ...
            class GeneratedMatcher(object):
                ...
                def _build_message(self, not_):
                    try:
                        ...
                        return self._message % {'expected': self._arg, 
                           'not': not_, 
                           'actual': self._value}
...
class _PredicateMatcher(object):
    ...
    def _display_verb(self, attr_name):
        return self._is_method(getattr(self._value, attr_name)) and 'return' or 'be'
