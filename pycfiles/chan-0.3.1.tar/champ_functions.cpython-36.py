...
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/champ/champ_functions.py
# Compiled at: 2019-05-22 07:48:25
# Size of source mod 2**32: 21614 bytes
...
def create_coefarray_from_partitions(partition_array, A_mat, P_mat, C_mat=None, nprocesses=0):
   ...
   :type nprocesses: int
   :return: size :math:`M\times\text{Dim}` array of coefficients for each partition. Dim can be 2 (single layer)    or 3 (multilayer)
...
def create_halfspaces_from_array(coef_array):
    r"""
    create a list of halfspaces from an array of coefficent.  Each half space is defined by     the inequality\:
    ...
    offs = np.sum((nvs * pts), axis=0)
    return np.vstack((-nvs, offs)).T


def sort_points(points):
    """For 2D case we sort the points along the gamma axis in assending order.     For the 3D case we sort the points clockwise around the center of mass .
...
def get_interior_point(hs_list, num_bound):
    ...
    try:
        ...
        if res.status == 0:
            ...
            warnings.warn({1:'Interior point calculation: scipy.optimize.linprog exceeded iteration limit',  2:'Interior point calculation: scipy.optimize.linprog problem is infeasible. Fallback will fail.', 
             3:'Interior point calculation: scipy.optimize.linprog problem is unbounded'}[res.status], RuntimeWarning)
    except ValueError:
        pass

    warnings.warn("Interior point calculation: falling back to 'small step' approach.", RuntimeWarning)
    z_vals = [-1.0 * offset / normal[(-1)] for normal, offset in zip(normals, offsets) if np.abs(normal[(-1)]) > np.power(10.0, -15)]
    dim = hs_list.shape[1] - 1
    intpt = np.array([0 for _ in range(dim - 1)] + [np.max(z_vals)])
    internal_step = np.array([1e-06 for _ in range(dim)])
...
def calculate_coefficient(com_vec, adj_matrix):
    ...
    for i, val in enumerate(com_vec):
        try:
            ...
            raise TypeError('Community labels must be hashable- isinstance(%s,Hashable): ' % str(val), isinstance(val, Hashable))
...
def _calculate_coefficient_parallel(comvec_mat):
    """
    wrapper function for calc coefficient with single parameter for use with the     multiprocessing map call
...
def point_comparator(pt1, pt2):
    if not len(pt1) == len(pt2):
        raise AssertionError('dimension of points must match')
    else:
...
def get_intersection(coef_array, max_pt=None):
   ...
   :param max_pt: Upper bound for the domains (in the xy plane). This will restrict the convex hull     to be within the specified range of gamma/omega (such as the range of parameters originally searched using Louvain).
   ...
   :return: dictionary mapping the index of the elements in the convex hull to the points defining the boundary
    ...
    if not singlelayer:
        ...
        boundary_halfspaces.extend([np.array([-1.0, 0, 0]),
         np.array([0, -1.0, 0])])
        num_boundary += 2
        if max_pt is not None:
        ...
        try:
            ...
            joggled = True
            halfspaces[num_input_halfspaces][-1] = -1e-05
            halfspaces[(num_input_halfspaces + 1)][-1] = -1e-05
        ...
        if joggled:
            ...
            if max(max_xy_intersections) < 0.01:
                raise ValueError(('All intersections are less than ({:.3f},{:.3f}). Invalid input set, try setting max_pt.'.format)(*max_xy_intersections))
...
def _random_plane():
    ...
    min_offset = -min(0, normal[0], normal[1], normal[0] + normal[1])
    max_offset = -max(normal[2], normal[2] + normal[0], normal[2] + normal[1], normal[2] + normal[0] + normal[1])
    offset = uniform(0.75 * min_offset + 0.25 * max_offset, 0.25 * min_offset + 0.75 * max_offset)
...
def PolyArea(pts):
    """Calculate the area of a set of pts (assumes that the points are sorted in      order clockwise around exterior"""
...
def get_number_of_communities(partition, min_com_size=0):
    """
        ...
        :param partition: list with community assignments (labels must be of hashable type      e.g. int,string, etc...).
        :type partition: list
