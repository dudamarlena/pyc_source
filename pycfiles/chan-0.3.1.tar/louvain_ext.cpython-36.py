...
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/champ/louvain_ext.py
# Compiled at: 2019-05-22 08:08:46
...
import itertools, igraph as ig, louvain, numpy as np, tqdm
...
def run_louvain_windows(graph, gamma, nruns, weight=None, node_subset=None, attribute=None, output_dictionary=False):
    """
        Call the louvain method with igraph as input directly.  This is needed for windows system       because tmp files cannot be closed and reopened
...
def parallel_louvain(graph, start=0, fin=1, numruns=200, maxpt=None, numprocesses=None, attribute=None, weight=None, node_subset=None, progress=None):
    r"""
    ...
    if iswin:
        ...
        for i, gam in enumerate(np.linspace(start, fin, numruns)):
            cpart_ens = run_louvain_windows(graph=graph, nruns=1, gamma=gam, node_subset=node_subset, attribute=attribute,
              weight=weight)
            if i == 0:
...
def _create_multilayer_igraphs_from_super_adj_igraph(intralayer_igraph, layer_vec):
    """
        For falling back on the normal louvain method.  We use the single layer intralayer_igraph to     create igraph representations for each of the layers.
...
def create_multilayer_igraph_from_edgelist(intralayer_edges, interlayer_edges, layer_vec, inter_directed=False, intra_directed=False):
    """
...
def run_louvain_multilayer(intralayer_graph, interlayer_graph, layer_vec, weight='weight', resolution=1.0, omega=1.0, nruns=1):
    ...
    for run in range(nruns):
        ...
        outparts.append({'partition':np.array(finalpartition),  'resolution':resolution, 
         'coupling':omega, 
         'orig_mod':0.5 / mu * _get_modularity_from_partobj_list(reversed_partobj) + omega * coupling_partition_rev.quality(), 
         'int_edges':A, 
         'exp_edges':P, 
         'int_inter_edges':C})
...
def parallel_multilayer_louvain(intralayer_edges, interlayer_edges, layer_vec, gamma_range, ngamma, omega_range, nomega, maxpt=None, numprocesses=2, progress=True, intra_directed=False, inter_directed=False):
    """
    ...
    if not hasattr(louvain, 'RBConfigurationVertexPartitionWeightedLayers'):
        warnings.warn('RBConfigurationVertexPartitionWeightedLayers not present in louvain package.  Falling back on creating igraph for each layer.  Note for networks with many layers this can result in considerable slowdown.')
    logging.debug('time {:.4f}'.format(time() - t))
    ...
    all_part_dicts = [pt for partrun in parts_list_of_list for pt in partrun]
    outensemble = PartitionEnsemble(graph=intralayer_graph, interlayer_graph=interlayer_graph, layer_vec=layer_vec,
      listofparts=all_part_dicts,
      maxpt=maxpt)
    return outensemble


def parallel_multilayer_louvain_from_adj(intralayer_adj, interlayer_adj, layer_vec, gamma_range, ngamma, omega_range, nomega, maxpt=None, numprocesses=2, progress=True, intra_directed=False, inter_directed=False):
