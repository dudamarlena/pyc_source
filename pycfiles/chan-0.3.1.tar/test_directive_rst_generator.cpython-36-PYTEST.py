...
# Embedded file name: /Users/jeremyr/dev/perso/champollion/test/unit/directive/test_directive_rst_generator.py
# Compiled at: 2017-07-03 19:20:41
# Size of source mod 2**32: 48632 bytes
import builtins as @py_builtins, _pytest.assertion.rewrite as @pytest_ar, pytest, champollion.directive.rst_generator
...
def test_get_rst_class_elements(options, expected):
    ...
    """
    environment = {'class': {'test.Class1':{'id':'test.Class1', 
                'name':'Class1', 
                'description':'A super class\n\n.. note:: A note.', 
                'line_number':2}, 
               'test.Class2':{'id':'test.Class2', 
                'name':'Class2', 
                'description':None, 
                'line_number':8}, 
               'test._Class3':{'id':'test._Class3', 
                'name':'_Class3', 
                'description':'A private class\n\n.. note:: A note.', 
                'line_number':16}, 
               'test.Class4':{'id':'test.Class4', 
                'name':'Class4', 
                'description':'An interesting class\n\n.. note:: A note.', 
                'line_number':25}}}
    @py_assert1 = champollion.directive
    @py_assert3 = @py_assert1.rst_generator
    @py_assert5 = @py_assert3.get_rst_class_elements
    @py_assert8 = 'test.module'
    @py_assert10 = 'test/module'
    @py_assert13 = @py_assert5(environment, @py_assert8, @py_assert10, **options)
    @py_assert15 = @py_assert13 == expected
    if not @py_assert15:
        @py_format17 = @pytest_ar._call_reprcompare(('==', ), (@py_assert15,), ('%(py14)s\n{%(py14)s = %(py6)s\n{%(py6)s = %(py4)s\n{%(py4)s = %(py2)s\n{%(py2)s = %(py0)s.directive\n}.rst_generator\n}.get_rst_class_elements\n}(%(py7)s, %(py9)s, %(py11)s, **%(py12)s)\n} == %(py16)s', ), (@py_assert13, expected)) % {'py0':@pytest_ar._saferepr(champollion) if 'champollion' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(champollion) else 'champollion',  'py2':@pytest_ar._saferepr(@py_assert1),  'py4':@pytest_ar._saferepr(@py_assert3),  'py6':@pytest_ar._saferepr(@py_assert5),  'py7':@pytest_ar._saferepr(environment) if 'environment' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(environment) else 'environment',  'py9':@pytest_ar._saferepr(@py_assert8),  'py11':@pytest_ar._saferepr(@py_assert10),  'py12':@pytest_ar._saferepr(options) if 'options' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(options) else 'options',  'py14':@pytest_ar._saferepr(@py_assert13),  'py16':@pytest_ar._saferepr(expected) if 'expected' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(expected) else 'expected'}
        @py_format19 = 'assert %(py18)s' % {'py18': @py_format17}
        raise AssertionError(@pytest_ar._format_explanation(@py_format19))
    @py_assert1 = @py_assert3 = @py_assert5 = @py_assert8 = @py_assert10 = @py_assert13 = @py_assert15 = None
...
def test_get_rst_attribute_elements(options, expected):
    ...
    """
    class_environment = {'attribute': {'test.Class.attribute1':{'id':'test.Class.attribute1', 
                    'name':'attribute1', 
                    'description':'A super attribute\n\n.. note:: A note.', 
                    'line_number':2}, 
                   'test.Class.attribute2':{'id':'test.Class.attribute2', 
                    'name':'attribute2', 
                    'description':None, 
                    'line_number':8}, 
                   'test.Class._attribute3':{'id':'test.Class._attribute3', 
                    'name':'_attribute3', 
                    'description':'A private attribute\n\n.. note:: A note.', 
                    'line_number':16}, 
                   'test.Class.attribute4':{'id':'test.Class.attribute4', 
                    'name':'attribute4', 
                    'description':'An interesting attribute\n\n.. note:: A note.', 
                    'line_number':25}}}
    @py_assert1 = champollion.directive
    @py_assert3 = @py_assert1.rst_generator
    @py_assert5 = @py_assert3.get_rst_attribute_elements
    @py_assert9 = @py_assert5(class_environment, **options)
    @py_assert11 = @py_assert9 == expected
    if not @py_assert11:
        @py_format13 = @pytest_ar._call_reprcompare(('==', ), (@py_assert11,), ('%(py10)s\n{%(py10)s = %(py6)s\n{%(py6)s = %(py4)s\n{%(py4)s = %(py2)s\n{%(py2)s = %(py0)s.directive\n}.rst_generator\n}.get_rst_attribute_elements\n}(%(py7)s, **%(py8)s)\n} == %(py12)s', ), (@py_assert9, expected)) % {'py0':@pytest_ar._saferepr(champollion) if 'champollion' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(champollion) else 'champollion',  'py2':@pytest_ar._saferepr(@py_assert1),  'py4':@pytest_ar._saferepr(@py_assert3),  'py6':@pytest_ar._saferepr(@py_assert5),  'py7':@pytest_ar._saferepr(class_environment) if 'class_environment' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(class_environment) else 'class_environment',  'py8':@pytest_ar._saferepr(options) if 'options' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(options) else 'options',  'py10':@pytest_ar._saferepr(@py_assert9),  'py12':@pytest_ar._saferepr(expected) if 'expected' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(expected) else 'expected'}
...
def test_get_rst_method_elements(options, expected):
    ...
    """
    class_environment = {'method': {'test.Class.method1':{'id':'test.Class.method1', 
                 'name':'method1', 
                 'description':'A super method\n\n.. note:: A note.', 
                 'line_number':2}, 
                'test.Class.method2':{'id':'test.Class.method2', 
                 'name':'method2', 
                 'description':None, 
                 'line_number':8}, 
                'test.Class._method3':{'id':'test.Class._method3', 
                 'name':'_method3', 
                 'description':'A private method\n\n.. note:: A note.', 
                 'line_number':16}, 
                'test.Class.constructor':{'id':'test.Class.constructor', 
                 'name':'constructor', 
                 'description':'A constructor.', 
                 'line_number':25}}}
    @py_assert1 = champollion.directive
    @py_assert3 = @py_assert1.rst_generator
    @py_assert5 = @py_assert3.get_rst_method_elements
    @py_assert9 = @py_assert5(class_environment, **options)
    @py_assert11 = @py_assert9 == expected
    if not @py_assert11:
...
def test_get_rst_function_elements(options, expected):
    ...
    """
    environment = {'function': {'test.function1':{'id':'test.function1', 
                   'name':'function1', 
                   'description':'A super function\n\n.. note:: A note.', 
                   'line_number':2}, 
                  'test.function2':{'id':'test.function2', 
                   'name':'function2', 
                   'description':None, 
                   'line_number':8}, 
                  'test._function3':{'id':'test._function3', 
                   'name':'_function3', 
                   'description':'A private function\n\n.. note:: A note.', 
                   'line_number':16}, 
                  'test.function4':{'id':'test.function4', 
                   'name':'function4', 
                   'description':'An interesting function\n\n.. note:: A note.', 
                   'line_number':25}}}
    @py_assert1 = champollion.directive
    @py_assert3 = @py_assert1.rst_generator
    @py_assert5 = @py_assert3.get_rst_function_elements
    @py_assert8 = 'test.module'
    @py_assert10 = 'test/module'
    @py_assert13 = @py_assert5(environment, @py_assert8, @py_assert10, **options)
    @py_assert15 = @py_assert13 == expected
...
def test_get_rst_data_elements(options, expected):
    ...
    """
    environment = {'data': {'test.DATA1':{'id':'test.DATA1', 
               'name':'DATA1', 
               'description':'A super data\n\n.. note:: A note.', 
               'line_number':2}, 
              'test.DATA2':{'id':'test.DATA2', 
               'name':'DATA2', 
               'description':None, 
               'line_number':8}, 
              'test._DATA3':{'id':'test._DATA3', 
               'name':'_DATA3', 
               'description':'A private data\n\n.. note:: A note.', 
               'line_number':16}, 
              'test.DATA4':{'id':'test.DATA4', 
               'name':'DATA4', 
               'description':'An interesting data\n\n.. note:: A note.', 
               'line_number':25}}}
    @py_assert1 = champollion.directive
    @py_assert3 = @py_assert1.rst_generator
    @py_assert5 = @py_assert3.get_rst_data_elements
    @py_assert8 = 'test.module'
    @py_assert10 = 'test/module'
    @py_assert13 = @py_assert5(environment, @py_assert8, @py_assert10, **options)
    @py_assert15 = @py_assert13 == expected
