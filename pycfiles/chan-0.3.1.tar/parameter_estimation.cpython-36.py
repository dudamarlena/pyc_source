...
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/champ/parameter_estimation.py
# Compiled at: 2018-07-11 16:32:03
# Size of source mod 2**32: 14198 bytes
import louvain
...
def iterative_monolayer_resolution_parameter_estimation(G, gamma=1.0, tol=0.01, max_iter=25, verbose=False):
    """
    ...
    def maximize_modularity(resolution_param):
        return louvain.find_partition(G, (louvain.RBConfigurationVertexPartition), resolution_parameter=resolution_param, weights='weight')
    ...
    for iteration in range(max_iter):
        ...
        if omega_in == 0 or omega_in == 1:
            ...
            if verbose:
                print('Iter {:>2}: {} communities with Q={:.3f} and gamma={:.3f}->{:.3f}'.format(iteration, len(part), part.q, last_gamma, gamma))
            if abs(gamma - last_gamma) < tol:
                break
    else:
        if verbose:
            print('Gamma failed to converge within {} iterations. Final move of {:.3f} was not within tolerance {}'.format(max_iter, abs(gamma - last_gamma), tol))
...
def iterative_multilayer_resolution_parameter_estimation(G_intralayer, G_interlayer, layer_vec, gamma=1.0, omega=1.0, gamma_tol=0.01, omega_tol=0.05, omega_max=1000, max_iter=25, model='temporal', verbose=False):
    """
    ...
    if 'weight' not in G_intralayer.es:
        ...
        if model is 'multiplex':
            ...
            def update_omega(theta_in, theta_out, p, K):
                if theta_out == 0:
                    if p < 1.0:
                        return log(1 + p * K / (1 - p)) / (2 * log(theta_in))
                    return omega_max
                else:
                    if p < 1.0:
                        return log(1 + p * K / (1 - p)) / (2 * (log(theta_in) - log(theta_out)))
                    return omega_max
    ...
    def maximize_modularity(intralayer_resolution, interlayer_resolution):
        ...
        intralayer_part = louvain.RBConfigurationVertexPartitionWeightedLayers(G_intralayer, layer_vec=layer_vec, weights='weight', resolution_parameter=intralayer_resolution)
    ...
    def estimate_SBM_parameters(partition):
        ...
        theta_out = sum(2 * m_t[t] - 2 * m_t_in[t] for t in range(T)) / sum(2 * m_t[t] - sum_kappa_t_sqr[t] / (2 * m_t[t]) for t in range(T)) if K > 1 else 0
    ...
    part, K, last_gamma, last_omega = (None, None, None, None)
    for iteration in range(max_iter):
        ...
        if not 0.0 <= p <= 1.0:
            raise ValueError('gamma={:.3f}, omega={:.3f} resulted in impossible estimate p={:.3f}'.format(gamma, omega, p))
        last_gamma, last_omega = gamma, omega
        gamma = update_gamma(theta_in, theta_out)
        omega = update_omega(theta_in, theta_out, p, K)
        if verbose:
            print('Iter {:>2}: {} communities with Q={:.3f}, gamma={:.3f}->{:.3f}, omega={:.3f}->{:.3f}, and p={:.3f}'.format(iteration, K, part.q, last_gamma, gamma, last_omega, omega, p))
        if abs(gamma - last_gamma) < gamma_tol and abs(omega - last_omega) < omega_tol:
            break
    else:
        if verbose:
            print('Parameters failed to converge within {} iterations. Final move of ({:.3f}, {:.3f}) was not within tolerance ({}, {})'.format(max_iter, abs(gamma - last_gamma), abs(omega - last_omega), gamma_tol, omega_tol))

    if verbose:
        print('Returned {} communities with Q={:.3f}, gamma={:.3f}, and omega={:.3f}'.format(K, part.q, gamma, omega))
    return (
     gamma, omega, part)