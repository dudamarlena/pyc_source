...
# [GCC 8.4.0]
# Embedded file name: /home/anders/work/python/django-likertblock/likertblock/models.py
# Compiled at: 2016-05-03 06:25:02
...
class Questionnaire(models.Model):
    ...
    def submit(self, user, data):
        ...
        for k in data.keys():
            if k.startswith('likert-question'):
                ...
                Response.objects.create(submission=s, question=question, value=data[k])

    def redirect_to_self_on_submit(self):
        return True

    def unlocked(self, user):
        return Submission.objects.filter(questionnaire=self, user=user).count() > 0

    def edit_form(self):

        class EditForm(forms.Form):
            description = forms.CharField(widget=forms.widgets.Textarea(), initial=self.description)
            alt_text = '<a href="' + reverse('edit-questionnaire', args=[self.id]) + '">manage questions</a>'
    ...
    def create(self, request):
        return Questionnaire.objects.create(description=request.POST.get('description', ''))

    @classmethod
    def create_from_dict(self, d):
        q = Questionnaire.objects.create(description=d.get('description', ''))
    ...
    def as_dict(self):
        return dict(description=self.description, questions=[ q.as_dict() for q in self.question_set.all() ])
    ...
    def import_from_dict(self, d):
        ...
        for q in d['questions']:
            Question.objects.create(questionnaire=self, text=q['text'])
...
class Question(models.Model):
    ...
    def user_responses(self, user):
        qs = Submission.objects.filter(user=user, questionnaire=self.questionnaire).order_by('-submitted')
        if len(qs) == 0:
