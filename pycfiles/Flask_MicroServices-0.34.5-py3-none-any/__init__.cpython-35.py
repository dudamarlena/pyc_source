...
# [GCC 8.4.0]
# Embedded file name: E:\Projects\flask-microservices\flask_microservices\__init__.py
# Compiled at: 2017-01-26 07:33:51
# Size of source mod 2**32: 5489 bytes
from flask import Blueprint, Flask, __version__
from flask.globals import _request_ctx_stack
from flask.helpers import send_from_directory
from flask.templating import DispatchingJinjaLoader
from .exceptions import InvalidModulePath, InvalidURLFunction, InvalidURLName, InvalidURLPattern, InvalidURLRule, UnspecifiedURLMethods
...
def url(rule, view_func, name=None, methods=['GET']):
    ...
    if not isinstance(rule, str) or not rule:
        raise InvalidURLRule('Received an invalid urlpattern `rule`: {}.'.format(rule))
    if not callable(view_func):
        raise InvalidURLFunction('Received an invalid `view_func`: {}.'.format(view_func))
    if not isinstance(name, str) and name is not None or isinstance(name, str) and not name:
        raise InvalidURLName('Received an invalid URL `name`: {}.'.format(name))
    if not isinstance(methods, list) or len(list(filter(None, methods))) < 1:
        raise UnspecifiedURLMethods('Received an empty list, or list of null strings.')
