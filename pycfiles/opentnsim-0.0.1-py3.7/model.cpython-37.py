...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\transport_network_analysis\model.py
# Compiled at: 2019-03-21 16:05:34
# Size of source mod 2**32: 5023 bytes
"""Vessel generator."""
import json, logging, uuid, simpy, networkx as nx, pyproj, shapely.geometry, datetime, time, random
    ...
    def arrival_process(self, environment, path, arrival_distribution, scenario, arrival_process):
        ...
        if type(arrival_distribution) == int:
            self.inter_arrival_times = [
             3600 / arrival_distribution] * 24
        else:
            if type(arrival_distribution) == list and len(arrival_distribution) == 24:
                self.inter_arrival_times = [3600 / n for n in arrival_distribution]
            else:
                if type(arrival_distribution) == list:
