...
# [GCC 8.4.0]
# Embedded file name: c:\github\testchain\test_chain\test_chain_meta.py
# Compiled at: 2017-01-06 10:03:45
    ...
    def done_decorators(*args, **kwargs):
        ...
        if hash_ not in results:
            try:
                ...
                if history.count('done_decorators') == 1:
                    raise
...
class TestChainMeta(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        for i in range(len(ring)):
            ...
            if key.startswith('test_') and getattr(original_func, 'original_func', None) is None and (inspect.ismethod(original_func) or inspect.isfunction(original_func)):
