...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/spoon/routing/meshrouter.py
# Compiled at: 2006-11-22 17:46:45
"""
    ...
    def handleNetworkTraffic(self, t, obj):
        ...
        if obj.dst == self.hub.nodeId:
            ...
            if route[0] == INFINITY:
                ...
                return
            if route[1] == t:
                self.hub._log.error('Immediate routing loop detected, something very bad is happening!                  Received message for node %d from our route for it, %d' % (obj.dst, t.NodeId))
