...
# [GCC 8.4.0]
# Embedded file name: w:\projects\django-rest-framework-mongoengine\rest_framework_mongoengine\serializers.py
# Compiled at: 2020-01-02 08:02:47
# Size of source mod 2**32: 37993 bytes
...
from rest_framework_mongoengine.validators import UniqueTogetherValidator, UniqueValidator
from .repr import serializer_repr
from .utils import COMPOUND_FIELD_TYPES, get_field_info, get_field_kwargs, get_generic_embedded_kwargs, get_nested_embedded_kwargs, get_nested_relation_kwargs, get_relation_kwargs, has_default, is_abstract_model
...
def raise_errors_on_nested_writes(method_name, serializer, validated_data):
    if not not any(isinstance(field, serializers.BaseSerializer) and not isinstance(field, EmbeddedDocumentSerializer) and key in validated_data for key, field in serializer.fields.items()):
        raise AssertionError('The `.{method_name}()` method does not support writable nestedfields by default.\nWrite an explicit `.{method_name}()` method for serializer `{module}.{class_name}`, or set `read_only=True` on nested serializer fields.'.format(method_name=method_name,
          module=(serializer.__class__.__module__),
          class_name=(serializer.__class__.__name__)))
    elif not not any('.' in field.source and key in validated_data and isinstance(validated_data[key], (list, dict)) for key, field in serializer.fields.items()):
        raise AssertionError('The `.{method_name}()` method does not support writable dotted-source fields by default.\nWrite an explicit `.{method_name}()` method for serializer `{module}.{class_name}`, or set `read_only=True` on dotted-source serializer fields.'.format(method_name=method_name,
          module=(serializer.__class__.__module__),
          class_name=(serializer.__class__.__name__)))


class DocumentSerializer(serializers.ModelSerializer):
    __doc__ = " Serializer for Documents.\n\n    Recognized primitve fields:\n\n        * ``StringField``\n        * ``URLField``\n        * ``EmailField``\n        * ``IntField``\n        * ``LongField``\n        * ``FloatField``\n        * ``DecimalField``\n        * ``BooleanField``\n        * ``DateTimeField``\n        * ``ComplexDateTimeField``\n        * ``ObjectIdField``\n        * ``SequenceField`` (assumes it has integer counter)\n        * ``UUIDField``\n        * ``GeoPointField``\n        * ``GeoJsonBaseField`` (all those fields)\n        * ``DateField``\n\n    Compound fields: ``ListField`` and ``DictField`` are mapped to corresponding DRF fields, with respect to nested field specification.\n\n    The ``ReferenceField`` is handled like ``ForeignKey`` in DRF: there nested serializer autogenerated if serializer depth greater then 0, otherwise it's handled by it's own (results as ``str(id)``).\n\n    For ``EmbeddedDocumentField`` also nested serializer autogenerated for non-zero depth, otherwise it is skipped. TODO: THIS IS PROBABLY WRONG AND SHOULD BE FIXED.\n\n    Generic fields ``GenericReferenceField`` and ``GenericEmbeddedDocumentField`` are handled by their own with corresponding serializer fields.\n\n    Not well supported or untested:\n\n        ``FileField``\n        ``ImageField``\n        ``BinaryField``\n\n    All other fields are mapped to ``DocumentField`` and probably will work wrong.\n    "
    serializer_field_mapping = {me_fields.StringField: drf_fields.CharField, 
    ...
    def create(self, validated_data):
        ...
        try:
            ...
            msg = 'Got a `TypeError` when calling `%s.objects.create()`. This may be because you have a writable field on the serializer class that is not a valid argument to `%s.objects.create()`. You may need to make the field read-only, or override the %s.create() method to handle this correctly.\nOriginal exception text was: %s.' % (
             ModelClass.__name__,
             ModelClass.__name__,
             type(self).__name__,
             exc)
            ...
            msg = 'Got a `ValidationError` when calling `%s.objects.create()`. This may be because request data satisfies serializer validations but not Mongoengine`s. You may need to check consistency between %s and %s.\nIf that is not the case, please open a ticket regarding this issue on https://github.com/umutbozkurt/django-rest-framework-mongoengine/issues\nOriginal exception was: %s' % (
             ModelClass.__name__,
             ModelClass.__name__,
             type(self).__name__,
             exc)
        ...
        for key, value in validated_data.items():
            try:
                ...
                if isinstance(field, EmbeddedDocumentSerializer):
                    me_data[key] = field.recursive_save(value) if value is not None else value
                elif isinstance(field, serializers.ListSerializer) or isinstance(field, serializers.ListField):
                    if isinstance(field.child, EmbeddedDocumentSerializer):
                        me_data[key] = []
                        for datum in value:
                            me_data[key].append(field.child.recursive_save(datum))

                elif isinstance(field, drfm_fields.DictField):
    ...
    def get_fields(self):
        if not hasattr(self, 'Meta'):
            raise AssertionError('Class {serializer_class} missing "Meta" attribute'.format(serializer_class=(self.__class__.__name__)))
        elif not hasattr(self.Meta, 'model'):
            raise AssertionError('Class {serializer_class} missing "Meta.model" attribute'.format(serializer_class=(self.__class__.__name__)))
        else:
        ...
        if model is None:
            ...
            for field_name in field_names:
                if field_name in declared_fields:
                    ...
                    field_class, field_kwargs = self.build_field(field_name, self.field_info, model, depth, depth_embedding)
        ...
        if fields:
            if fields != ALL_FIELDS:
                if not isinstance(fields, (list, tuple)):
                    raise TypeError('The `fields` option must be a list or tuple or "__all__". Got %s.' % type(fields).__name__)
        else:
            if exclude:
                if not isinstance(exclude, (list, tuple)):
                    raise TypeError('The `exclude` option must be a list or tuple. Got %s.' % type(exclude).__name__)
            assert not (fields and exclude), "Cannot set both 'fields' and 'exclude' options on serializer {serializer_class}.".format(serializer_class=(self.__class__.__name__))
        if fields is None:
            if exclude is None:
                warnings.warn("Creating a ModelSerializer without either the 'fields' attribute or the 'exclude' attribute is deprecated since 3.3.0. Add an explicit fields = '__all__' to the {serializer_class} serializer.".format(serializer_class=(self.__class__.__name__)), DeprecationWarning)
        if fields == ALL_FIELDS:
            ...
            if fields is not None:
                ...
                for field_name in required_field_names:
                    assert field_name in fields, "The field '{field_name}' was declared on serializer {serializer_class}, but has not been included in the 'fields' option.".format(field_name=field_name,
        ...
        if exclude is not None:
            for field_name in exclude:
                if '.' not in field_name:
                    assert field_name in fields, "The field '{field_name}' was included on serializer {serializer_class} in the 'exclude' option, but does not match any model field.".format(field_name=field_name,
                      serializer_class=(self.__class__.__name__))
    ...
    def get_default_field_names(self, declared_fields, model_info):
        return [
         model_info.pk.name] + list(declared_fields.keys()) + list(model_info.fields.keys()) + list(model_info.references.keys()) + list(model_info.embedded.keys())
    ...
    def get_customization_for_nested_field(self, field_name):
        ...
        if exclude:
            if not isinstance(exclude, (list, tuple)):
                raise TypeError('The `exclude` option must be a list or tuple. Got %s.' % type(exclude).__name__)
        if not not (fields and exclude):
            raise AssertionError("Cannot set both 'fields' and 'exclude' options on serializer {serializer_class}.".format(serializer_class=(self.__class__.__name__)))
        else:
            if fields is None:
                if exclude is None:
                    warnings.warn("Creating a ModelSerializer without either the 'fields' attribute or the 'exclude' attribute is deprecated since 3.3.0. Add an explicit fields = '__all__' to the {serializer_class} serializer.".format(serializer_class=(self.__class__.__name__)), DeprecationWarning)
    ...
    def build_standard_field(self, field_name, model_field):
        ...
        if 'choices' in field_kwargs:
            ...
            valid_kwargs = set(('read_only', 'write_only', 'required', 'default', 'initial',
        ...
        if not issubclass(field_class, drf_fields.CharField):
            if not issubclass(field_class, drf_fields.ChoiceField):
                field_kwargs.pop('allow_blank', None)
        if field_class is drf_fields.BooleanField:
            if field_kwargs.get('allow_null', False):
    ...
    def build_compound_field(self, field_name, model_field, child_field):
        if isinstance(model_field, me_fields.ListField):
            ...
            if isinstance(model_field, me_fields.DictField):
    ...
    def serializer_reference_field_factory(self, field_class):
        return type(self.serializer_reference_base_field.__name__, (
         self.serializer_reference_base_field,), {'pk_field_class': field_class})

    def build_reference_field(self, field_name, relation_info, nested_depth):
        field_class = relation_info.related_model or self.serializer_reference_generic
        ...
        if not issubclass(field_class, drfm_fields.DocumentField):
            ...
            try:
                ...
                raise KeyError('Referencing a model with a ' + related_class.__name__ + ' primary key is not supported')
    ...
    def get_uniqueness_extra_kwargs(self, field_names, extra_kwargs):
        ...
        for field_name in unique_fields:
            uniq_extra_kwargs[field_name] = {'required':True, 
             'validators':[
              UniqueValidator(queryset=(model.objects))]}
        ...
        for idx in uniq_indexes:
            if not idx.get('unique', False):
                ...
                if len(field_set) > 1 and field_names.issuperset(set(field_set)):
                    validators.append(UniqueTogetherValidator(queryset=(model.objects),
                      fields=field_set))
...
class EmbeddedDocumentSerializer(DocumentSerializer):
    __doc__ = ' Serializer for EmbeddedDocuments.\n\n    Skips id field and uniqueness validation.\n    When saving, skips calling instance.save\n    '
    _saving_instances = False

    def get_default_field_names(self, declared_fields, model_info):
        return list(declared_fields.keys()) + list(model_info.fields.keys()) + list(model_info.references.keys()) + list(model_info.embedded.keys())
            ...
            if key not in validated_data:
                try:
                    ...
                    if not isinstance(field, drf_fields.SkipField):
                        msg = 'Field %s is missing from validated data,but is not a SkipField!' % key
