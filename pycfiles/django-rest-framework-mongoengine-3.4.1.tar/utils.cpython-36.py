...
# [GCC 8.4.0]
# Embedded file name: w:\projects\django-rest-framework-mongoengine\rest_framework_mongoengine\utils.py
...
def get_relation_info(model_field):
    return RelationInfo(model_field=model_field,
...
def get_field_info(model):
    ...
    def add_field(name, field):
        if isinstance(field, REFERENCING_FIELD_TYPES):
            ...
            if isinstance(field, EMBEDDING_FIELD_TYPES):
    ...
    return FieldInfo(pk, fields, references, fields_and_pk, embedded)
...
def get_field_kwargs(field_name, model_field):
    ...
    if model_field.default:
        if not isinstance(model_field, me_fields.ComplexBaseField):
...
def get_relation_kwargs(field_name, relation_info):
    ...
    kwargs = {}
    if related_model:
        if not issubclass(related_model, EmbeddedDocument):
