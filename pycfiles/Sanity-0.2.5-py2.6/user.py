...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/sanity/views/user.py
# Compiled at: 2010-06-21 12:10:53
...
def register():
    ...
    if not config.REGISTRATION:
        flash('Registration is turned off. Please contact an administrator.')
...
def edit():
    ...
    if request.method == 'POST':
        ...
        if '@' not in email:
            ...
            db.session.commit()
            return redirect(url_for('frontend.index'))
    return render_template('user/edit.html', error=error, groups=groups, user=g.user)
