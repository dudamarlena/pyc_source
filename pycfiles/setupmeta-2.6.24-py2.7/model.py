...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/setupmeta/model.py
# Compiled at: 2020-03-31 05:40:31
"""
Model of our view on how setup.py + files in a project can come together
"""
import inspect, io, os, re, sys, setuptools, setupmeta.versioning
...
RE_EMAIL = re.compile('(.+)[\\s<>()\\[\\],:;]+([^@]+@[a-zA-Z0-9._-]+)')
RE_PY_VALUE = re.compile('^__([a-z_]+)__\\s*=\\s*u?[\\\'"](.+?)[\\\'"]\\s*(#.+)?$')
RE_DOC_VALUE = re.compile('^([a-z_]+)\\s*[:=]\\s*(.+?)(\\s*#.+)?$')
RE_DESCRIPTION = re.compile('^[\\W\\s]*((([\\w\\-]+)\\s*[:-])?\\s*(.+))$', re.IGNORECASE)
...
class PackageInfo:
    ...
    _canonical_names = {'classifier': 'classifiers', 
...
class Requirements:
    ...
    def __init__(self, pkg_info):
        ...
        self.test = self.get_requirements(False, 'tests/requirements.txt', 'requirements-dev.txt', 'dev-requirements.txt', 'test-requirements.txt', 'requirements-test.txt')
    ...
    def __init__(self, upstream):
        ...
        for package in packages:
            if package and '.' not in package:
                self.merge(SimpleModule(package, '__about__.py'), SimpleModule(package, '__version__.py'), SimpleModule(package, '__init__.py'), SimpleModule('src', package, '__about__.py'), SimpleModule('src', package, '__version__.py'), SimpleModule('src', package, '__init__.py'))
