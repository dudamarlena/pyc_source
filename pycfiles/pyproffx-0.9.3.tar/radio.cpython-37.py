...
# Embedded file name: build/bdist.linux-x86_64/egg/pyproffit/radio.py
# Compiled at: 2020-04-23 04:51:54
# Size of source mod 2**32: 25561 bytes
...
class Radio:
    ...
    def __init__(self, data=None, center_choice=None, maxrad=None, binsize=None, center_ra=None, center_dec=None, binning='linear', bins=None):
        """
    ...
    def SBprofile(self, ellipse_ratio=1.0, ellipse_angle=0.0, angle_low=0.0, angle_high=360.0, voronoi=False):
        ...
        if self.data.img is None:
            ...
            if self.data.rmsmap is None:
                ...
                if not self.custom:
                    ...
                    nbin = self.nbin
                profile, eprof, area, effexp, bkgprof = (
                 np.empty(self.nbin), np.empty(self.nbin),
                 np.empty(self.nbin), np.empty(self.nbin), np.empty(self.nbin))
        ...
        for i in range(nbin):
            ...
            if voronoi:
                ...
                profile[i] = np.average((img[id]), weights=(1.0 / rmsmap[id] ** 2)) / pixsize ** 2 - bkgprof[i]
                eprof[i] = np.sqrt(1.0 / np.sum(1.0 / rmsmap[id] ** 2)) / pixsize ** 2
                area[i] = nv * pixsize ** 2
