...
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyproffit/data.py
# Compiled at: 2019-11-11 09:06:49
# Size of source mod 2**32: 8785 bytes
import numpy as np
            ...
            if 'circle' in lreg[i]:
                ...
                if 'ellipse' in lreg[i]:
                    ...
                    if regtype == 'fk5':
                        ...
                        if '"' in rad1:
                            rad1 = float(rad1.split('"')[0]) / self.pixsize / 60.0
                            rad2 = float(rad2.split('"')[0]) / self.pixsize / 60.0
                        else:
                            if "'" in rad1:
                                rad1 = float(rad1.split("'")[0]) / self.pixsize
                                rad2 = float(rad2.split("'")[0]) / self.pixsize
                            else:
                                rad1 = float(rad1) / self.pixsize * 60.0
                                rad2 = float(rad2) / self.pixsize * 60.0
                        wc = np.array([[xsrc, ysrc]])
                        pixcrd = self.wcs_inp.wcs_world2pix(wc, 1)
                        xsrc = pixcrd[0][0] - 1.0
                        ysrc = pixcrd[0][1] - 1.0
                    ...
                    ellang = angle * np.pi / 180.0 + np.pi / 2.0
                    aoverb = rad1 / rad2
