...
# Embedded file name: build/bdist.linux-x86_64/egg/pyproffit/miscellaneous.py
# Compiled at: 2020-04-23 04:41:07
# Size of source mod 2**32: 5071 bytes
import numpy as np
from scipy.spatial.distance import cdist
from scipy.stats import poisson

def logbinning(binsize, maxrad):
    nbin = int(maxrad / binsize * 60.0 + 0.5)
    bins = np.arange(binsize / 60.0 / 2.0, (nbin + 1.5) * binsize / 60.0, binsize / 60.0)
    ebins = np.ones(nbin) * binsize / 60.0 / 2.0
    binedge = binsize / 60.0 / 2
    db = 0.0
    i = 0
    while db < binsize / 60.0:
        bb = nbin / np.log10(maxrad / binsize * 60.0) * (np.log10(binedge) - np.log10(binsize / 60.0))
        base1 = np.log10(binsize / 60.0) + np.log10(maxrad / binsize * 60.0) * (bb - 1) / nbin
        base2 = np.log10(binsize / 60.0) + np.log10(maxrad / binsize * 60.0) * bb / nbin
        ebe = np.power(10.0, base2) - np.power(10.0, base1)
        db = ebe
        binedge = bins[i]
        i = i + 1
        if i > nbin:
    ...
    binedge = binedge + binsize / 60.0 / 2.0
    thisbin = binedge
    b2 = 1
    while thisbin < maxrad:
        base1 = np.log10(binedge) + np.log10(maxrad / binsize * 60.0) * (b2 - 1) / nbin
        base2 = np.log10(binedge) + np.log10(maxrad / binsize * 60.0) * b2 / nbin
        thisbin = 0.5 * (np.power(10.0, base1) + np.power(10.0, base2))
        ebe = 0.5 * (np.power(10.0, base2) - np.power(10.0, base1))
        bins[i] = thisbin
        ebins[i] = ebe
        b2 = b2 + 1
        i = i + 1
...
def clean_bkg(img, bkg):
    ...
    prob = 1.0 - poisson.cdf(timg[id], tbkg[id])
