...
# Embedded file name: build/bdist.linux-x86_64/egg/pyproffit/deproject.py
# Compiled at: 2020-04-23 04:51:54
# Size of source mod 2**32: 48075 bytes
import astropy.cosmology as cosmo
import numpy as np, pymc3 as pm, time
...
def plot_multi_methods(profs, deps, labels=None, outfile=None):
    if len(profs) != len(deps):
        ...
        for i in range(len(deps)):
            ...
            plt.errorbar(rkpc, (dep.dens), xerr=erkpc, yerr=[dep.dens - dep.dens_lo, dep.dens_hi - dep.dens], fmt='.', color=('C%d' % i),
              elinewidth=2,
              markersize=7,
              capsize=3,
              label=(labels[i]))
...
def fbul19(R, z, Runit='kpc'):
    ...
    M = 1.3333333333333333 * np.pi * 500 * rho_cz * R ** 3
    zpiv = 0.45
    Mpiv = 635000000000000.0
    ...
    Mgas = 10000000000000.0 * A * (M / Mpiv) ** Bprime * (efunc / efuncpiv) ** 0.6666666666666666 * ((1 + z) / (1 + zpiv)) ** gamma
    return Mgas
...
def list_params(rad, sourcereg, nrc=None, nbetas=6, min_beta=0.6):
    ...
    allrc = np.logspace(np.log10(rfit[2]), np.log10(rfit[(npfit - 1)] / 2.0), nrc)
    allbetas = np.linspace(min_beta, 3.0, nbetas)
    nrc = len(allrc)
    nbetas = len(allbetas)
...
def calc_int_operator(a, b, pars):
    ...
    beta = np.repeat(pars[:, 0], npt).reshape(npars, npt)
    rc = np.repeat(pars[:, 1], npt).reshape(npars, npt)
    base = 1.0 + np.power(rads / rc, 2)
    expon = -3.0 * beta + 1.5
    func_base = 2.0 * np.pi * np.power(base, expon) / (3 - 6 * beta) * rc ** 2
...
def list_params_density(rad, sourcereg, z, nrc=None, nbetas=6, min_beta=0.6):
    ...
    if nrc is None:
        nrc = np.max([int(npfit / nsh), 1])
    allrc = np.logspace(np.log10(rfit[2]), np.log10(rfit[(npfit - 1)] / 2.0), nrc) * kpcp
    allbetas = np.linspace(min_beta, 3.0, nbetas)
    nrc = len(allrc)
    nbetas = len(allbetas)
    rc = allrc.repeat(nbetas)
...
def Deproject_Multiscale_Stan(deproj, bkglim=None, nmcmc=1000, back=None, samplefile=None, nrc=None, nbetas=6, depth=10, min_beta=0.6):
    ...
    import pystan, stan_utility as su
    ...
    datas = dict(K=K, cts_tot=(counts.astype(int)), cts_back=bkgcounts, N=(K.shape[0]), M=(K.shape[1]), norm0=norm0)
    tinit = time.time()
    print('Running MCMC...')
    ...
    pmcl = np.percentile(allsb, 15.850000000000001, axis=1)
    pmch = np.percentile(allsb, 84.15, axis=1)
...
class MyDeprojVol:
    ...
    def deproj_vol(self):
        ...
        nbin = len(ro)
        volconst = 1.3333333333333333 * np.pi
        volmat = np.zeros((nbin, nbin))
        for iring in list(reversed(range(0, nbin))):
            volmat[(iring, iring)] = volconst * ro[iring] ** 3 * (1.0 - (ri[iring] / ro[iring]) ** 2.0) ** 1.5
            for ishell in list(reversed(range(iring + 1, nbin))):
                f1 = (1.0 - (ri[iring] / ro[ishell]) ** 2.0) ** 1.5 - (1.0 - (ro[iring] / ro[ishell]) ** 2.0) ** 1.5
                f2 = (1.0 - (ri[iring] / ri[ishell]) ** 2.0) ** 1.5 - (1.0 - (ro[iring] / ri[ishell]) ** 2.0) ** 1.5
...
def medsmooth(prof):
    ...
    Y0 = 3.0 * prof[0] - 2.0 * prof[1]
    xx = np.array([Y0, prof[0], prof[1]])
    smoothed[0] = np.median(xx)
    Y0 = 3.0 * prof[(nbin - 1)] - 2.0 * prof[(nbin - 2)]
...
def EdgeCorr(nbin, rin_cm, rout_cm, em0):
    mrad = [
     rin_cm[(nbin - 1)], rout_cm[(nbin - 1)]]
    edge0 = (mrad[0] + mrad[1]) * mrad[0] * mrad[1] / rout_cm ** 3
    edge1 = 2.0 * rout_cm / mrad[1] + np.arccos(rout_cm / mrad[1])
    edge2 = rout_cm / mrad[1] * np.sqrt(1.0 - rout_cm ** 2 / mrad[1] ** 2)
    edget = edge0 * (-1.0 + 2.0 / np.pi * (edge1 - edge2))
    j = np.where(rin_cm != 0)
    ...
    edge2[j] = rout_cm[j] / mrad[1] * (np.sqrt(1.0 - rin_cm[j] ** 2 / mrad[1] ** 2) - np.sqrt(1.0 - rout_cm[j] ** 2 / mrad[1] ** 2))
    edget[j] = edge0[j] * (1.0 - 2.0 / np.pi * (edge1[j] - edge2[j]) / (rout_cm[j] / rin_cm[j] - 1.0))
...
def OP(deproj, nmc=1000):
    ...
    allres = np.linalg.solve(vol, emres * (1.0 - ct))
...
class Deproject:
    ...
    def __init__(self, z=None, profile=None, cf=None, f_abund='aspl'):
        ...
        if f_abund == 'angr':
            nhc = 1.1994722322178242
            mup = 0.6125
            mu_e = 1.1738
        elif f_abund == 'aspl':
            nhc = 1.172745396974317
            mup = 0.5994
            mu_e = 1.1548
        elif f_abund == 'grsa':
            nhc = 1.1737089201877935
            mup = 0.6
            mu_e = 1.1555
        else:
            nhc = 1.172745396974317
    ...
    def Density(self, rout=None):
        ...
        if z is not None and cf is not None:
            transf = 4.0 * (1.0 + z) ** 2 * 116640000.0 / np.pi / 1e-14 / self.nhc / Mpc * 1000.0
            ...
            pmcdl = np.percentile(alldens, 15.850000000000001, axis=1)
            pmcdh = np.percentile(alldens, 84.15, axis=1)
            self.dens = pmcd
            self.dens_lo = pmcdl
            self.dens_hi = pmcdh
            self.rout = rout
    ...
    def PlotSB(self, outfile=None):
        if self.profile is None:
            ...
            ax.set_xscale('log')
            ax.set_yscale('log')
            ax.errorbar((prof.bins), (prof.counts / prof.area / prof.effexp), xerr=(prof.ebins), yerr=(prof.eprof), fmt='d', color='r',
              elinewidth=2,
              markersize=7,
              capsize=0,
              label='Data')
            ...
            pmcl = np.percentile(allsb, 15.850000000000001, axis=1) / prof.area / prof.effexp + prof.bkgprof
            pmch = np.percentile(allsb, 84.15, axis=1) / prof.area / prof.effexp + prof.bkgprof
    ...
    def CountRate(self, a, b, plot=True, outfile=None):
        if self.samples is None:
            ...
            intlo = np.percentile(allint[1, :] - allint[0, :], 15.850000000000001)
            inthi = np.percentile(allint[1, :] - allint[0, :], 84.15)
    ...
    def Ncounts(self, plot=True, outfile=None):
        if self.samples is None:
            ...
            self.rec_counts, self.rec_counts_lo, self.rec_counts_hi = np.percentile(allnc, [50.0, 15.850000000000001, 84.15], axis=1)
            ncv = np.sum(allnc, axis=0)
            pnc = np.median(ncv)
            pncl = np.percentile(ncv, 15.850000000000001)
            pnch = np.percentile(ncv, 84.15)
    ...
    def Mgas(self, radius, plot=True, outfile=None):
        if not self.samples is None:
            if self.z is None or self.cf is None:
            ...
            rkpc = prof.bins * kpcp
            erkpc = prof.ebins * kpcp
            nhconv = mh * self.mu_e * self.nhc * kpc ** 3 / msun
            rad = prof.bins
            sourcereg = np.where(rad < self.bkglim)
            transf = 4.0 * (1.0 + self.z) ** 2 * 116640000.0 / np.pi / 1e-14 / self.nhc / Mpc * 1000.0
            ...
            alldens = np.sqrt(np.dot(Kdens, np.exp(self.samples.T)) / self.cf * transf)
            volmat = np.repeat(4.0 * np.pi * rkpc ** 2 * 2.0 * erkpc, alldens.shape[1]).reshape(len(prof.bins), alldens.shape[1])
            mgas = np.cumsum((alldens * nhconv * volmat), axis=0)
            f = interp1d(rkpc, mgas, axis=0)
            mgasdist = f(radius)
            mg, mgl, mgh = np.percentile(mgasdist, [50.0, 15.850000000000001, 84.15])
            if plot:
                plt.clf()
                fig = plt.figure(figsize=(13, 10))
                ax_size = [0.14, 0.12,
    ...
    def PlotMgas(self, rout=None, outfile=None, xscale='kpc', scaling_relation=None):
        if not self.samples is None:
            ...
            alldens = np.sqrt(np.dot(Kdens, np.exp(self.samples.T)) / self.cf * transf)
            volmat = np.repeat(4.0 * np.pi * rkpc ** 2 * 2.0 * erkpc, alldens.shape[1]).reshape(len(rout), alldens.shape[1])
            mgasdist = np.cumsum((alldens * nhconv * volmat), axis=0)
            mg, mgl, mgh = np.percentile(mgasdist, [50.0, 15.850000000000001, 84.15], axis=1)
            self.mg = mg
            self.mgl = mgl
            self.mgh = mgh
            ...
            self.r500, self.r500_l, self.r500_h = np.percentile(rkpc[np.argmin((np.abs(Mgasdist / mgasdist - 1)), axis=0)], [50.0, 15.850000000000001, 84.15])
            self.m500, self.m500_l, self.m500_h = 1.3333333333333333 * np.pi * 500 * rho_cz * self.r500 ** 3, 1.3333333333333333 * np.pi * 500 * rho_cz * self.r500_l ** 3, 1.3333333333333333 * np.pi * 500 * rho_cz * self.r500_h ** 3
    ...
    def CSB(self, rin=40.0, rout=400.0, plot=True, outfile=None):
        if not self.samples is None:
            if self.z is None:
            ...
            sourcereg = np.where(prof.bins < self.bkglim)
            pars = list_params(prof.bins, sourcereg, self.nrc, self.nbetas, self.min_beta)
            Kin = calc_int_operator(prof.bins[0] / 2.0, rin / kpcp, pars)
            allvin = np.dot(Kin, np.exp(self.samples.T))
            Kout = calc_int_operator(prof.bins[0] / 2.0, rout / kpcp, pars)
            allvout = np.dot(Kout, np.exp(self.samples.T))
            medcsb = np.median((allvin[1, :] - allvin[0, :]) / (allvout[1, :] - allvout[0, :]))
            csblo = np.percentile((allvin[1, :] - allvin[0, :]) / (allvout[1, :] - allvout[0, :]), 15.850000000000001)
            csbhi = np.percentile((allvin[1, :] - allvin[0, :]) / (allvout[1, :] - allvout[0, :]), 84.15)
