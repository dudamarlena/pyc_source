...
# Embedded file name: build/bdist.linux-x86_64/egg/pyproffit/profextract.py
# Compiled at: 2020-04-23 13:09:17
# Size of source mod 2**32: 28841 bytes
...
class Profile:
    ...
    def __init__(self, data=None, center_choice=None, maxrad=None, binsize=None, center_ra=None, center_dec=None, binning='linear', centroid_region=None, bins=None):
        """
        ...
        if method == 'custom_ima' and not center_ra is None:
            if center_dec is None:
                print('Error: please provide X and Y coordinates')
                return
            self.cx = center_ra - 1.0
            self.cy = center_dec - 1.0
            self.ellangle = None
            self.ellratio = None
            ...
            if method == 'custom_fk5':
                if not center_ra is None:
                    if center_dec is None:
                        print('Error: please provide X and Y coordinates')
                        return
                    self.cra = center_ra
                    ...
                    if centroid_region is not None:
                        ...
                        regrad = np.max(np.array([data.axes[0], data.axes[1]]) / 2.0)
                    ...
                    if center_ra is None or center_dec is None:
                        ...
                        xc_temp, yc_temp = data.axes[1] / 2.0, data.axes[0] / 2.0
                    else:
                    ...
                    if data.exposure is None or data.filth is not None:
                        ...
                        region = np.where(np.logical_and(np.logical_and(np.hypot(xc_temp - xp, yc_temp - yp) < regrad, img > 0), data.exposure > 0.0))
                        nonzero = np.where(data.exposure > 0.0)
                        print('Denoising image...')
                        bkg = np.mean(img[nonzero])
                        imgc = clean_bkg(img, bkg)
                        x = np.repeat(xp[region], imgc[region])
                        y = np.repeat(yp[region], imgc[region])
                        weights = np.repeat(1.0 / data.exposure[region], img[region])
                        print('Running PCA...')
        ...
        if binning == 'log':
            ...
            self.bins = (bins + np.roll(bins, -1))[:self.nbin] / 2.0
            self.ebins = (np.roll(bins, -1) - bins)[:self.nbin] / 2.0
            self.custom = True
        else:
            print('Unknown binning option ' + binning + ', reverting to linear')
            self.islogbin = False

    def SBprofile(self, ellipse_ratio=1.0, ellipse_angle=0.0, angle_low=0.0, angle_high=360.0, voronoi=False):
        data = self.data
        img = data.img
        ...
        if not self.custom:
            if self.islogbin:
                ...
                nbin = int(self.maxrad / self.binsize * 60.0 + 0.5)
                self.bins = np.arange(self.binsize / 60.0 / 2.0, (nbin + 0.5) * self.binsize / 60.0, self.binsize / 60.0)
                self.ebins = np.ones(nbin) * self.binsize / 60.0 / 2.0
                self.nbin = nbin
        else:
            nbin = self.nbin
        profile, eprof, counts, area, effexp, bkgprof, bkgcounts = (
         np.empty(self.nbin), np.empty(self.nbin),
         np.empty(self.nbin), np.empty(self.nbin), np.empty(self.nbin), np.empty(self.nbin), np.empty(self.nbin))
        ...
        if ellipse_ratio is not None:
            ...
            self.ellratio = 1.0
        tta = ellipse_angle - 90.0
        if tta < -90.0 or tta > 270.0:
            print('Error: input angle must be between 0 and 360 degrees')
            return
        else:
            ellang = tta * np.pi / 180.0
            ...
            if angle_low != 0.0:
                if angle_high != 360.0:
                    if angle_low < 0.0:
                        anglow = np.deg2rad(np.fmod(angle_low, 360.0) + 360.0)
                    else:
                        anglow = np.deg2rad(np.fmod(angle_low, 360.0))
                    if angle_high < 0.0:
                        anghigh = np.deg2rad(np.fmod(angle_high, 360.0) + 360.0)
                    else:
                        anghigh = np.deg2rad(np.fmod(angle_high, 360.0))
                else:
                    anglow = 0.0
                    anghigh = 2.0 * np.pi
                angles = np.arctan2(y - self.cy, x - self.cx)
                aneg = np.where(angles < 0.0)
                angles[aneg] = angles[aneg] + 2.0 * np.pi
                if anghigh < anglow:
                    anghigh = anghigh + 2.0 * np.pi - anglow
                else:
                    anghigh = anghigh - anglow
                if angle_high < angle_low:
                    zcross = np.where(angles < np.deg2rad(angle_low))
                    angles[zcross] = angles[zcross] + 2.0 * np.pi - anglow
        ...
        for i in range(nbin):
            if i == 0:
                id = np.where(np.logical_and(np.logical_and(np.logical_and(np.logical_and(rads >= 0, rads < np.round(self.bins[i] + self.ebins[i], 5) + tol), exposure > 0.0), angles >= 0.0), angles <= anghigh))
            else:
                id = np.where(np.logical_and(np.logical_and(np.logical_and(np.logical_and(rads >= np.round(self.bins[i] - self.ebins[i], 5) + tol, rads < np.round(self.bins[i] + self.ebins[i], 5) + tol), exposure > 0.0), angles >= 0.0), angles <= anghigh))
            nv = len(img[id])
            if voronoi:
                errmap = data.errmap
                profile[i] = np.average((img[id]), weights=(1.0 / errmap[id] ** 2))
                eprof[i] = np.sqrt(1.0 / np.sum(1.0 / errmap[id] ** 2))
            elif nv > 0:
    ...
    def MedianSB(self):
        ...
        for i in range(self.nbin):
            id = np.where(np.logical_and(np.logical_and(rads >= self.bins[i] - self.ebins[i], rads < self.bins[i] + self.ebins[i]), errmap > 0.0))
    ...
    def PSF(self, psffunc=None, psffile=None, psfimage=None, psfpixsize=None, sourcemodel=None):
            ...
            if psffile is not None:
                ...
                if psfpixsize is not None:
                    ...
                    if psfpixsize == 0.0:
                        ...
                        return
                fpsf.close()
            if not psfimage is not None or psfpixsize is None or psfpixsize <= 0.0:
        ...
        rads = np.hypot(x - self.cx, y - self.cy) * self.data.pixsize
        kernel = None
        if psffunc is not None:
            psfmin = 1e-07
            kernel = psffunc(rads)
            norm = np.sum(kernel)
            frmax = lambda x: psffunc(x) * 2.0 * np.pi * x / norm - psfmin
            if frmax(exposure.shape[0] / 2) < 0.0:
                rmax = brentq(frmax, 1.0, exposure.shape[0]) / self.data.pixsize
                npix = int(rmax)
            else:
        ...
        for n in range(nbin):
            if n == 0:
                ...
                sort_list.append(np.where(np.logical_and(rads >= np.round(rad[n] - erad[n], 5) + tol, rads < np.round(rad[n] + erad[n], 5) + tol)))
    ...
    def Plot(self, model=None, outfile=None, axes=None):
        if self.profile is None:
            ...
            plt.xscale('log')
            plt.yscale('log')
            plt.errorbar((self.bins), (self.profile), xerr=(self.ebins), yerr=(self.eprof), fmt='o', color='black', elinewidth=2, markersize=7,
              capsize=0,
              mec='black',
              label='Brightness')
            ...
            if model is not None:
                ...
                plt.errorbar((self.bins), chi, yerr=(np.ones(len(self.bins))), fmt='o', color='black', elinewidth=2, markersize=7,
                  capsize=0,
                  mec='black')
                plt.xlabel('Radius [arcmin]', fontsize=28)
                plt.ylabel('$\\chi$', fontsize=28)
                plt.xscale('log')
                xl = np.logspace(np.log10(self.bins[0] / 2.0), np.log10(self.bins[(len(self.bins) - 1)] * 2.0), 100)
                ...
                if axes is None:
                    ...
                    ymin = np.min(chi[reg]) - 1.0
                    ymax = np.max(chi[reg]) + 1.0
    ...
    def Backsub(self, fitter):
        ...
        if self.profile is None:
            ...
            return
        val = np.power(10.0, fitter.minuit.values['bkg'])
        eval = val * np.log(10.0) * fitter.minuit.errors['bkg']
