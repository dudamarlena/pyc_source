...
# Embedded file name: build/bdist.linux-x86_64/egg/cirrus/r53.py
# Compiled at: 2012-05-30 23:02:13
import logging, tempfile, dns, dns.zone
...
class Zone:
    ...
    def _compare(self, from_zone, to_zone):
        ...
        deletes = {}
        updates = {}
        for key in to_records.iterkeys():
            name = key[0]
            rtype = key[1]
    ...
    def _get_rrsets(self, ltype=None, lname=None):
        ...
        return (
         simple_bind, number, last_type, last_name)
    ...
    def get_host(self, host, ltype=None, lname=None):
        ...
        """
        rrecords = self.conn.get_all_rrsets(self.id, ltype, lname)
        if host[:-1] == '.':
            fqdn = host
        else:
            fqdn = host + '.'
        for rrecord in rrecords:
            ...
            if fqdn == rrecord.name:
                if len(rrecord.resource_records) == 0 and rrecord.alias_hosted_zone_id is not None:
                    ...
                    return (
                     rrecord.type, rrecord.resource_records, rrecord.ttl)
