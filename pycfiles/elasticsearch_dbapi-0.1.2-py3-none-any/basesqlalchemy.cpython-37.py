...
# [GCC 8.4.0]
# Embedded file name: /opt/vol1/home/dpgaspar/workarea/preset/elasticsearch-dbapi/es/basesqlalchemy.py
# Compiled at: 2019-10-23 13:11:41
# Size of source mod 2**32: 7392 bytes
...
class BaseESDialect(default.DefaultDialect):
    ...
    _not_supported_column_types = [
     'object', 'nested']
    _map_parse_connection_parameters = {'verify_certs':parse_bool_argument, 
    ...
    def get_columns(self, connection, table_name, schema=None, **kwargs):
        ...
        array_columns_ = connection.execute(f"SHOW ARRAY_COLUMNS FROM {table_name}").fetchall()
        if len(array_columns_[0]) == 0:
            array_columns = []
        else:
            array_columns = [col_name[0] for col_name in array_columns_]
        result = connection.execute(query)
        return [{'name':row.column,  'type':get_type(row.mapping),  'nullable':True,  'default':None} for row in result if row.mapping not in self._not_supported_column_types if row.column not in array_columns]
...
def get_type(data_type):
    type_map = {'bytes':types.LargeBinary, 
