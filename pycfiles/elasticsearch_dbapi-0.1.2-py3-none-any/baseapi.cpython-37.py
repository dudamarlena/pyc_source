...
# [GCC 8.4.0]
# Embedded file name: /opt/vol1/home/dpgaspar/workarea/preset/elasticsearch-dbapi/es/baseapi.py
# Compiled at: 2019-10-23 12:56:00
...
def check_closed(f):
    ...
    def g(self, *args, **kwargs):
        if self.closed:
            raise exceptions.Error('{klass} already closed'.format(klass=(self.__class__.__name__)))
...
class BaseConnection(object):
    __doc__ = 'Connection to an ES Cluster '
    ...
    def commit(self):
        ...
        Not supported.
        """
        raise exceptions.NotSupportedError('This is a readonly dbapi commit is not supported')
...
class BaseCursor(object):
    ...
    def executemany(self, operation, seq_of_parameters=None):
        raise exceptions.NotSupportedError('`executemany` is not supported, use `execute` instead')
    ...
    def elastic_query(self, query: str, csv=False):
        ...
        try:
            ...
            try:
                raise exceptions.ProgrammingError(f"Error ({e.error}): {e.info['error']['reason']}")
            finally:
                e = None
                del e
