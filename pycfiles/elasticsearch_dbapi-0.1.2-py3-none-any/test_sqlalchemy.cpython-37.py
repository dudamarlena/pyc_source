...
# [GCC 8.4.0]
# Embedded file name: /opt/vol1/home/dpgaspar/workarea/preset/elasticsearch-dbapi/es/tests/test_sqlalchemy.py
# Compiled at: 2019-10-24 04:49:00
...
class TestData(unittest.TestCase):
    ...
    def test_connection_https_and_auth(self, mock_elasticsearch):
        ...
        self.connection = self.engine.connect()
        mock_elasticsearch.assert_called_once_with('https://localhost:9200',
          http_auth=('user', 'password'))
    ...
    def test_connection_https_and_params(self, mock_elasticsearch):
        ...
        """
        mock_elasticsearch.return_value = None
        self.engine = create_engine('elasticsearch+https://localhost:9200/?verify_certs=False&use_ssl=False')
        self.connection = self.engine.connect()
        mock_elasticsearch.assert_called_once_with('https://localhost:9200',
    ...
    def test_connection_params(self, mock_elasticsearch):
        ...
        """
        mock_elasticsearch.return_value = None
        self.engine = create_engine('elasticsearch+http://localhost:9200/?http_compress=True&maxsize=100&timeout=3')
        self.connection = self.engine.connect()
        mock_elasticsearch.assert_called_once_with('http://localhost:9200',
    ...
    def test_connection_params_value_error(self, mock_elasticsearch):
        ...
        with self.assertRaises(ValueError):
            self.engine = create_engine('elasticsearch+http://localhost:9200/?http_compress=cena')
    ...
    def test_connection_sniff(self, mock_elasticsearch):
        ...
        """
        mock_elasticsearch.return_value = None
        self.engine = create_engine('elasticsearch+http://localhost:9200/?sniff_on_start=True&sniff_on_connection_fail=True&sniffer_timeout=3&sniff_timeout=4&max_retries=10&retry_on_timeout=True')
        self.connection = self.engine.connect()
        mock_elasticsearch.assert_called_once_with('http://localhost:9200',
          sniff_on_start=True,
