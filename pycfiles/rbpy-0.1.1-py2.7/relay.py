...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/rbpy/relay.py
# Compiled at: 2016-07-06 21:39:35
...
class Box(object):
    ...
    def __init__(self, private_code, pub_id):
        ...
        self.pub_id = pub_id
        self.relaybox_url = b'https://api.relaybox.io/message/{type}'
    ...
    def make_header(self, json_body):
        ...
        """
        h = hmac.new(self.private_code, msg=json_body.encode(b'utf-8'), digestmod=hashlib.sha256)
        header = {b'Content-Type': b'application/json', 
           b'X-APP-PUB': self.pub_id, 
           b'X-APP-HASH': h.hexdigest(), 
           b'X-APP-API-VERSION': b'1.0.0'}
        return header
    ...
    def send(self, message, public=False):
        ...
        """
        body = {b'message': message, b'options': {b'public': public}}
        json_body = json.dumps(body)
        headers = self.make_header(json_body)
        r = requests.post(self.relaybox_url.format(type=b'send'), json_body, headers=headers)
    ...
    def get(self):
        ...
        headers = self.make_header(json_body)
        r = requests.post(self.relaybox_url.format(type=b'get'), json_body, headers=headers)
        if r.status_code == requests.codes.ok:
            return r.json()