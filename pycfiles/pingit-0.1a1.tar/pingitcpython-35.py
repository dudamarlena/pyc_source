...
# [GCC 8.4.0]
# Embedded file name: bin/pingit
# Compiled at: 2019-01-13 08:14:14
# Size of source mod 2**32: 23847 bytes
__version__ = '0.1a1'
if __name__ == '__main__':
    import sys, os, json, git, logging
    from logging import info, warning, error
    from arghandler import *
    REMOTES = 'remotes'
    TOKEN = 'token'
    DETACHED = '~detached~'
    ARG_PATH = {TOKEN: ('-p', '--path'), 'default': '.', 
     'help': 'Use different path than current directory.'}
    ARG_INPUT = {TOKEN: ('-i', '--input'), 'default': None, 
     'help': 'Input file. If not specified, read from STDIN.'}
    ARG_OUTPUT = {TOKEN: ('-o', '--output'), 'default': None, 
     'help': 'Output file. If not specified, write to STDOUT.'}
    ARG_OUTPATH = {TOKEN: ('-o', '--output'), 'default': '.', 
     'help': 'Output directory. If not specified, write to current ' + 'directory.'}
    ARG_LIST = {TOKEN: ('-l', '--list'), 'default': None, 
     'help': 'Instead of a recursive search for repos, take this list.'}
    ARG_VERBOSE = {TOKEN: ('-v', '--verbose'), 'action': 'store_true', 
    ...
    class Progress(git.remote.RemoteProgress):
        __doc__ = 'Handler for showing progress of long-running git commands.'
            ...
            with open(args.list, 'r') as (listfile):
                for line in listfile:
                    ...
                    if not add_repo(relpath, rootpath, repos):
                        warning('No git repository found in %s' % (relpath,))
    ...
    def cmd_export(parser, context, args):
        parser.description = 'Generate a description of a directory ' + 'containing nested git repositories.'
        add_arguments(parser, ARG_PATH, ARG_OUTPUT)
        args = parser.parse_args(args)
        repos, rootpath = get_repos(args)
        output = {}
        for reponame, repo in sorted(repos.items()):
            output[reponame] = {REMOTES: {remote.name:list(remote.urls) for remote in repo.remotes}}

        output_file_or_stdout(args, output, lambda file, data: json.dump(data, file))
    ...
    def cmd_import(parser, context, args):
        ...
        for repo_name, repo_config in sorted(input.items()):
            try:
                ...
                repo_path = os.path.abspath(os.path.join(args.output, repo_name))
                try:
                ...
                if remote not in remotes:
                    selection = {remote:urls for remote, urls in remotes.items() if len(urls) > 0 if len(urls) > 0}
                ...
                error('Unable to deflate %s: %s' % (repo_name, repr(e)))
    ...
    def cmd_list(parser, context, args):
        parser.description = 'List all git repos in the current or a given directory.'
        add_arguments(parser, ARG_PATH, ARG_OUTPUT)
        args = parser.parse_args(args)
        repos, rootpath = find_repos(args.path)
        paths = [os.path.abspath(os.path.join(args.path, path)) for path in list(sorted(repos.keys()))]
    ...
    def cmd_ping(parser, context, args):
        parser.description = 'Ping the remotes of all git repos and report issues.'
        ...
        for repopath, repo in sorted(repos.items()):
            ...
            try:
                ...
                if branch == DETACHED or len(repo.active_branch.log()) == 0:
                    ...
                    for remote in repo.remotes:
                        commits_behind = list(repo.iter_commits('%s..%s/%s' % (branch, remote, branch)))
                    ...
                    for remote in repo.remotes:
                        commits_ahead = list(repo.iter_commits('%s/%s..%s' % (remote, branch, branch)))
    ...
    def cmd_pull(parser, context, args):
        ...
        parser.add_argument('--rebase', action='store_true', help='Use rebase instead of merge to apply upstream.')
        add_arguments(parser, ARG_PATH, ARG_LIST, ARG_VERBOSE)
        args = parser.parse_args(args)
        repos, rootpath = get_repos(args)
        for repopath, repo in sorted(repos.items()):
            for remote in repo.remotes:
                try:
                    ...
                    if args.verbose:
                        kwargs['progress'] = Progress()
                    remote.pull(*['--rebase'] if args.rebase else [], **kwargs)
                except git.exc.GitCommandError as e:
    ...
    def cmd_archive(parser, context, args):
        ...
        parser.add_argument('--sep', default='__', help='Token separator for archive name composition.')
        parser.add_argument('--format', default='gztar', help='Archive output format. Choose from: ' + 'tar, gztar, bztar, xztar, zip')
        parser.add_argument('--no-timestamp', action='store_true', help='Do not include timestamps in archive filename.')
        ...
        for repopath, repo in sorted(repos.items()):
            try:
                ...
                archive_basename = os.path.join(args.output, args.sep.join(levels))
                info('Archiving %s to %s' % (repopath, archive_basename))
                shutil.make_archive(archive_basename, args.format, rootpath, repopath)
    ...
    def cmd_version(parser, context, args):
        ...
        info('(2019) EMS Research Group TU Ilmenau, ' + 'http://github.com/EMS-TU-Ilmenau/PINGIT, ' + 'https://www.tu-ilmenau.de/it-ems')
