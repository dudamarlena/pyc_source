...
class _DatastoreCollectionClient:
    ...
    def list(self, page: int=1, item_per_page: int=25) -> Union[(DtlError, List[DatastoreCollection])]:
        ...
        """
        res = self.http_client.make_authed_request(self.service_uri + f"/datastore-collections?page={page}&size={item_per_page}", HttpMethod.GET)
        if isinstance(res, DtlError):
            return res
        else:
            return _parse_list(_datastore_collection_from_dict)(res)

    def search(self, by_name: Optional[List[str]]=None, by_description: Optional[List[str]]=None, page: int=1, item_per_page: int=25) -> Union[(DtlError, List[DatastoreCollection])]:
        """
