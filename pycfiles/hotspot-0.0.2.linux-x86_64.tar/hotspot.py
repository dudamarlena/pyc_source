...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/hotspot/hotspot.py
# Compiled at: 2014-07-08 09:19:07
"""
hotspot - Performance report generator.
"""
import ConfigParser, argparse, datetime, logging, math, matplotlib.pyplot, multiprocessing, numpy, os
try:
    import cPickle as pickle
except ImportError:
    import pickle

import platform, re, scipy.stats, socket, subprocess, time
...
class Log:
    ...
    def __init__(self):
        ...
        self.logdir = ('{0}/.hotspot/{1}/{2}').format(os.path.expanduser('~'), program, self.timestamp)
...
class Config:
    ...
    def load(self):
        ...
        self.parser = argparse.ArgumentParser(description=description, epilog=epilog, version='0.0.1')
        realpath = os.path.dirname(os.path.realpath(__file__))
        configpath = realpath + '/../cfg/hotspot.cfg'
        self.parser.add_argument('--config', '-c', help='path to configuration', default=configpath)
        self.parser.add_argument('--debug', '-d', action='store_true', help='enable verbose logging')
        self.parser.add_argument('--force', '-f', action='store_true', help='do not reuse cache')
...
class Section:
    ...
    def command(self, cmd):
        ...
        output = None
        elapsed = -1
        output_suffix = ('/../{0}.{1}.output').format(self.name, self.counter[self.name])
        elapsed_suffix = ('/../{0}.{1}.elapsed').format(self.name, self.counter[self.name])
...
class SanitySection(Section):
    ...
    def gather(self):
        ...
        test = (' && ').join([self.dir,
         self.build,
         self.run.format(self.cores, self.first, self.program),
         'cd -'])
...
class BenchmarkSection(Section):
    ...
    def gather(self):
        ...
        metrics = [
         ('success', 'Success=(\\d+.*)', None),
         ('hpl', 'HPL_Tflops=(\\d+.*)', 'TFlops'),
         ('dgemm', 'StarDGEMM_Gflops=(\\d+.*)', 'GFlops'),
         ('ptrans', 'PTRANS_GBs=(\\d+.*)', 'GBs'),
         ('random', 'StarRandomAccess_GUPs=(\\d+.*)', 'GUPs'),
         ('stream', 'StarSTREAM_Triad=(\\d+.*)', 'MBs'),
         ('fft', 'StarFFT_Gflops=(\\d+.*)', 'GFlops')]
...
class WorkloadSection(Section):
    ...
    def gather(self):
        ...
        buckets, bins, patches = matplotlib.pyplot.hist(times, bins=number, normed=True)
        matplotlib.pyplot.plot(bins, scipy.stats.norm.pdf(bins, loc=numpy.mean(array), scale=array.std()), 'r--')
...
class ScalingSection(Section):
    ...
    def gather(self):
        """Run program."""
        cleanup = ('cd {0}; {1}; {2}').format(self.dir, self.clean, self.build.format(self.cflags))
        self.command(cleanup).output
        data = {}
        outputs = []
        for size in range(int(self.first), int(self.last) + 1, int(self.increment)):
...
class ThreadsSection(Section):
    ...
    def gather(self):
        ...
        for core in range(1, int(self.tags['cores']) + 1):
            run = self.tags['run'].format(core, self.tags['last'], self.tags['program'])
        ...
        self.tags['gustafson'] = '%.5f' % (1024 - serial * 1023)
        ...
        matplotlib.pyplot.xticks(range(0, int(self.tags['cores'])), range(1, int(self.tags['cores']) + 1))
        matplotlib.pyplot.ylabel('time in seconds')
...
class ProfileSection(Section):
    ...
    def gather(self):
        ...
        command = (' && ').join([self.tags['build'].format('-O3 -g -pg'),
         self.tags['run'].format(self.tags['cores'], self.tags['first'], self.tags['program']),
...
def main():
    ...
    for key, value in sorted(tags.iteritems()):
        ...
        template = template.replace('@@' + key.upper() + '@@', value.replace('%', ''))
