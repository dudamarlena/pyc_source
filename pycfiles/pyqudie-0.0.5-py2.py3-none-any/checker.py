...
# Embedded file name: build/bdist.linux-x86_64/egg/pyquchk/checker.py
# Compiled at: 2013-09-22 16:48:07
import sys
...
__all__ = [
 'for_all', 'Result', 'ReturnedFalse', 'CheckError',
...
class Result(object):
    ...
    def __init__(self, verdict, passed, datas, data=None, noshrink_data=None, exception=None, traceback=None):
    ...
    def fail(cls, datas, data, noshrink_data, exception, traceback):
        return Result('FAIL', False, datas, data, noshrink_data, exception, traceback)

    def reraise(self):
        exception = get_CheckError(self.data, self.noshrink_data, self.exception)
...
class Checker(object):
    ...
    def __init__(self, func, args, ntests, nshrinks, nassume, ignore_return=False):
        ...
        arg_arbs = {argname:get_arbitrary(self.args[argname]) for argname in self.args}
        ...
        while n < self.ntests:
            ...
            try:
                ...
                if assume_fail_cnt > self.nassume:
                    raise RuntimeError('Could not satisfy assume() after %d of test cases.' % self.nassume)
                continue
            except Exception as e:
                ...
                try:
                    ...
                    return Result.fail(datas, shrinked_kwargs, kwargs, e, traceback)
