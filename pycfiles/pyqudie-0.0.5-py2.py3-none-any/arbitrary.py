...
# Embedded file name: build/bdist.linux-x86_64/egg/pyquchk/arbitraries/arbitrary.py
# Compiled at: 2013-09-22 13:34:04
__doc__ = "Contains :class:`Arbitrary` class and the machinery it needs to work.\nYou shouldn't need anything other than this class from here.\n"
from abc import ABCMeta, abstractmethod
import re, inspect, six
...
class Arbitrary(six.with_metaclass(ABCMeta)):
    ...
    def args_for_sample(*args, **kwargs):
        ...
        def wrapper(method):
            ...
            m = re.match('\\s*@Arbitrary.args_for_sample\\((.*)\\)', method_src)
            argsdef = m.group(1)
            method.args_for_sample = {'args': args, 
               'kwargs': kwargs, 
               'def': argsdef}
            return method
...
class FilteredArbitrary(Arbitrary):
    ...
    def could_generate(self, x):
        return self.inner_arb.could_generate(x) and self.predicate(x)
