...
# Embedded file name: build/bdist.linux-x86_64/egg/pypeerassets/__main__.py
# Compiled at: 2018-10-11 07:31:40
# Size of source mod 2**32: 10279 bytes
__doc__ = 'contains main protocol logic like assembly of proof-of-timeline and parsing deck info'
import concurrent.futures
from typing import Iterator, Generator, Optional
from pypeerassets.protocol import Deck, CardBundle, CardTransfer, validate_card_issue_modes
from pypeerassets.provider import Provider, RpcNode
from pypeerassets.pautils import deck_parser, find_deck_spawns, card_bundle_parser, tx_serialization_order, find_tx_sender
from pypeerassets.exceptions import EmptyP2THDirectory
from pypeerassets.transactions import nulldata_script, tx_output, p2pkh_script, make_raw_transaction, Transaction, Locktime
...
def find_all_valid_decks(provider: Provider, deck_version: int, prod: bool=True) -> Generator:
    ...
    if isinstance(provider, RpcNode):
        ...
        try:
            deck_spawns = (provider.getrawtransaction(i, 1) for i in provider.listtransactions(p2th))
...
def deck_spawn(provider: Provider, deck: Deck, inputs: dict, change_address: str, locktime: int=0) -> Transaction:
    ...
    txouts = [
     tx_output(network=(deck.network), value=(pa_params.P2TH_fee), n=0,
       script=p2pkh_script(address=p2th_addr, network=(deck.network))),
     tx_output(network=(deck.network), value=(Decimal(0)), n=1,
       script=(nulldata_script(deck.metainfo_to_protobuf))),
     tx_output(network=(deck.network), value=change_sum, n=2,
       script=p2pkh_script(address=change_address, network=(deck.network)))]
    unsigned_tx = make_raw_transaction(network=(deck.network), inputs=(inputs['utxos']),
      outputs=txouts,
      locktime=(Locktime(locktime)))
    return unsigned_tx


def deck_transfer(provider: Provider, deck: Deck, inputs: list, change_address: str) -> Transaction:
    """
...
def card_bundler(provider: Provider, deck: Deck, tx: dict) -> CardBundle:
    ...
    return CardBundle(deck=deck, blockhash=(tx['blockhash']),
      txid=(tx['txid']),
      timestamp=(tx['time']),
      blockseq=(tx_serialization_order(provider, tx['blockhash'], tx['txid'])),
...
def find_card_bundles(provider: Provider, deck: Deck) -> Optional[Iterator]:
    ...
    try:
        raw_txns = (provider.getrawtransaction(i, 1) for i in provider.listtransactions(deck.p2th_address))
    except TypeError:
        raise EmptyP2THDirectory({'error': 'No cards found on this deck.'})
...
def card_transfer(provider: Provider, card: CardTransfer, inputs: dict, change_address: str, locktime: int=0) -> Transaction:
    ...
    outs = [
     tx_output(network=(provider.network), value=(pa_params.P2TH_fee),
       n=0,
       script=p2pkh_script(address=(card.deck_p2th), network=(provider.network))),
     tx_output(network=(provider.network), value=(Decimal(0)),
       n=1,
       script=(nulldata_script(card.metainfo_to_protobuf)))]
    for addr, index in zip(card.receiver, range(len(card.receiver))):
        outs.append(tx_output(network=(provider.network), value=(Decimal(0)), n=(index + 2), script=p2pkh_script(address=addr, network=(provider.network))))

    change_sum = Decimal(inputs['total'] - network_params.min_tx_fee - pa_params.P2TH_fee)
    outs.append(tx_output(network=(provider.network), value=change_sum,
      n=(len(outs) + 1),
      script=p2pkh_script(address=change_address, network=(provider.network))))
    unsigned_tx = make_raw_transaction(network=(provider.network), inputs=(inputs['utxos']),
      outputs=outs,
      locktime=(Locktime(locktime)))
    return unsigned_tx