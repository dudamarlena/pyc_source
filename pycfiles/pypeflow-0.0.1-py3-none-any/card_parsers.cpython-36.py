...
# Embedded file name: build/bdist.linux-x86_64/egg/pypeerassets/card_parsers.py
# Compiled at: 2018-06-29 14:42:01
# Size of source mod 2**32: 2004 bytes
__doc__ = 'parse cards according to deck issue mode'
...
def once_parser(cards: list) -> Optional[list]:
    ...
    """
    first_issue = next(i for i in cards if i.type == 'CardIssue')
    filtered = [i for i in cards if i.type == 'CardIssue' if i != first_issue]
...
def mono_parser(cards: list) -> Optional[list]:
    ...
    for c in cards:
        c.amount = [
         amount_to_exponent(exponent_to_amount(c.amount[0], decimals), decimals)]
