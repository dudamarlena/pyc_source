...
# Embedded file name: build/bdist.linux-x86_64/egg/pypeerassets/pautils.py
# Compiled at: 2018-10-11 06:56:19
...
from pypeerassets.exceptions import InvalidDeckSpawn, InvalidDeckMetainfo, InvalidDeckIssueMode, InvalidDeckVersion, InvalidNulldataOutput, InvalidCardIssue, DeckP2THImportError, P2THImportFailed
from pypeerassets.exceptions import InvalidCardTransferP2TH, CardVersionMismatch, CardNumberOfDecimalsMismatch, RecieverAmountMismatch
...
def deck_parser(args: Tuple[(Provider, dict, int, str)], prod: bool=True) -> Optional[Deck]:
    ...
    try:
        ...
        d = parse_deckspawn_metainfo(read_tx_opreturn(raw_tx['vout'][1]), deck_version)
        if d:
            d['id'] = raw_tx['txid']
            try:
...
def load_deck_p2th_into_local_node(provider: RpcNode, deck: Deck) -> None:
    ...
    if not check_addr['isvalid']:
        if not check_addr['ismine']:
...
def card_bundle_parser(bundle: CardBundle, debug=False) -> Iterator:
    ...
    try:
        ...
        card_metainfo = parse_card_transfer_metainfo(read_tx_opreturn(bundle.vouts[1]), bundle.deck.version)
    ...
    if not card_metainfo['number_of_decimals'] == bundle.deck.number_of_decimals:
        raise CardNumberOfDecimalsMismatch({'error': 'Number of decimals does not match.'})
