...
# Embedded file name: build/bdist.linux-x86_64/egg/pypeerassets/transactions.py
# Compiled at: 2018-08-30 07:56:30
# Size of source mod 2**32: 9933 bytes
__doc__ = 'transaction assembly/dissasembly'
...
from btcpy.structs.script import CoinBaseScriptSig, NulldataScript, P2pkhScript, P2shScript, ScriptBuilder, ScriptSig, StackData
from btcpy.structs.transaction import CoinBaseTxIn, Locktime, MutableTransaction as BtcPyMutableTx, MutableTxIn, Stream, Transaction as BtcPyTx, TxIn, TxOut, Witness
...
class Transaction(BtcPyTx):
    ...
    def __init__(self, version: int, ins: list, outs: list, locktime: Locktime, txid: str=None, network=PeercoinMainnet, timestamp: int=0) -> None:
    ...
    def from_json(cls, tx_json, network=PeercoinMainnet):
        return cls(version=(tx_json['version']),
    ...
    def to_mutable(self):
        return MutableTransaction(version=(self.version),
    ...
    def __str__(self):
        return 'Transaction(version={}, ins=[{}], outs=[{}], locktime={}, timestamp={} '.format(self.version, ', '.join(str(txin) for txin in self.ins), ', '.join(str(out) for out in self.outs), self.locktime, self.timestamp)


class MutableTransaction(Transaction, BtcPyMutableTx):

    def __init__(self, version, ins, outs, locktime, network=PeercoinMainnet, timestamp=0):
    ...
    def to_immutable(self):
        return Transaction((self.version),
...
class TransactionParser(BtcPyTxParser):
    ...
    def get_next_tx(self, mutable=False):
        ...
        if segwit:
            ...
            txins = [CoinBaseTxIn(*txin_data[2:], **{'witness': Witness(wit)}) if isinstance(txin_data[2], CoinBaseScriptSig) else TxIn(*txin_data, **{'witness': Witness(wit)}) for txin_data, wit in zip(txins_data, witness)]
        else:
            txins = [CoinBaseTxIn(*txin_data[2:]) if isinstance(txin_data[2], CoinBaseScriptSig) else TxIn(*txin_data) for txin_data in txins_data]
        locktime = self._locktime()
        if len(txins) > 1:
            if isinstance(txins[0], CoinBaseTxIn):
...
def p2pkh_script(network: str, address: str) -> P2pkhScript:
    ...
    addr = Address.from_string(network=network_params, string=address)
...
def tx_output(network: str, value: Decimal, n: int, script: ScriptSig) -> TxOut:
    """create TxOut object"""
    network_params = net_query(network)
    return TxOut(network=network_params, value=(int(value * network_params.to_unit)),
      n=n,
      script_pubkey=script)


def make_raw_transaction(network: str, inputs: list, outputs: list, locktime: Locktime, timestamp: int=int(time()), version: int=1) -> MutableTransaction:
    """create raw transaction"""
    network_params = net_query(network)
    if network_params.name.startswith('peercoin'):
        return MutableTransaction(version=version,
          ins=inputs,
          outs=outputs,
          locktime=locktime,
...
def sign_transaction(provider: Provider, unsigned: MutableTransaction, key: Kutil) -> Transaction:
