...
# [GCC 8.4.0]
# Embedded file name: /home/narendra/.pyenv/versions/aws/lib/python3.6/site-packages/tvarit_api/api.py
# Compiled at: 2019-09-13 03:24:25
# Size of source mod 2**32: 3351 bytes
import pyotp, requests, requests.auth
...
class TvaritAPI:
    ...
    def __init__(self, auth, host='localhost', port=None, url_path_prefix='', protocol='http', verify=True):
        ...
        def construct_api_url():
            params = {'protocol':self.url_protocol, 
             'host':self.url_host, 
    ...
    def __getattr__(self, item):
        ...
        def __request_runnner(url, json=None, headers=None, params=None):
            _TvaritAPI__url = '%s%s' % (self.url, url)
            runner = getattr(self.s, item.lower())
            r = runner(_TvaritAPI__url,
              json=json, headers=headers, params=params, auth=(self.auth), verify=(self.verify))
            if 500 <= r.status_code < 600:
                raise TvaritServerError('Server Error {0}: {1}'.format(r.status_code, r.content.decode('ascii', 'replace')))
            else:
                if r.status_code == 400:
                    ...
                    if r.status_code == 401:
                        ...
                        if 400 <= r.status_code < 500:
                            raise TvaritClientError('Client Error {0}: {1}'.format(r.status_code, r.text))
            return r.json()

        return _TvaritAPI__request_runnner