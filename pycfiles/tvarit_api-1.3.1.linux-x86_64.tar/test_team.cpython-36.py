...
# [GCC 8.4.0]
# Embedded file name: /home/narendra/.pyenv/versions/aws/lib/python3.6/site-packages/test/api/test_team.py
# Compiled at: 2019-07-17 12:15:45
# Size of source mod 2**32: 7843 bytes
import sys, unittest, requests_mock
sys.path.append('.')
...
class TeamsTestCase(unittest.TestCase):
    ...
    def setUp(self):
        self.cli = Tvarit(('admin', 'admin'),
          host='localhost', url_path_prefix='', protocol='http')
    ...
    def test_get_team(self, m):
        m.get('http://localhost/api/teams/1',
          json={'id':1, 
         'orgId':1, 
         'name':'MyTestTeam', 
         'email':'', 
    ...
    def test_add_team(self, m):
        m.post('http://localhost/api/teams',
          json={'message':'Team created',  'teamId':2})
        team = {'name':'MySecondTestTeam', 
    ...
    def test_get_team_members(self, m):
        m.get('http://localhost/api/teams/1/members',
          json=[
         {'orgId':1, 
          'teamId':1, 
          'userId':3, 
    ...
    def test_add_team_member(self, m):
        m.post('http://localhost/api/teams/1/members',
    ...
    def test_remove_team_member(self, m):
        m.delete('http://localhost/api/teams/13/members/2',
    ...
    def test_get_team_preferences(self, m):
        m.get('http://localhost/api/teams/1/preferences',
          json={'theme':'', 
         'homeDashboardId':0,  'timezone':''})
    ...
    def test_update_team_preferences(self, m):
        m.put('http://localhost/api/teams/1/preferences',
