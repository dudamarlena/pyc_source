...
def add_keys(apps, schema_editor):
    ...
    key = b''
    try:
        key = b'APP_LANDING_PAGE_URL'
        Setting.objects.get(key=key)
    except Exception:
        Setting.objects.create(title=b'لدنینگ پیج نرم افزار', key=key, value=b'https://www.aparnik.com/', value_type=b's', is_show=False, is_variable_in_home=True)
