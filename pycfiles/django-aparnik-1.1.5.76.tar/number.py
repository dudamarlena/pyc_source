...
human_powers = (
 N_('million'), N_('billion'), N_('trillion'), N_('quadrillion'),
...
def intword(value, format='%.1f'):
    ...
    for ordinal, power in enumerate(powers[1:], 1):
        if value < power:
            ...
            return ((' ').join([format, _(human_powers[(ordinal - 1)]).decode('utf-8')]) % chopped).encode('utf-8')
...
def apnumber(value):
    ...
    if not 0 < value < 10:
        return str(value)
    return (
     _('one'), _('two'), _('three'), _('four'), _('five'), _('six'),
