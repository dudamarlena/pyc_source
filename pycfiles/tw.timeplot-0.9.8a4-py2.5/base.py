...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/tw/timeplot/base.py
# Compiled at: 2009-09-21 17:01:46
import os, tw
from tw.api import Widget, JSLink, CSSLink, Link
from tw.core.resources import registry, working_set, Requirement
from defaults import __TIMEPLOT_VERSION__

class TimelineLinkMixin(Link):
    params = {'version': '(string) select the yui version you would like to use. Default version is: ' + __TIMEPLOT_VERSION__}
    extension = 'js'
    version = __TIMEPLOT_VERSION__
    ...
    def filename(self):
        ...
        basename = self.basename.replace('\\', os.sep)
        basename = os.sep.join(('static', self.version, basename))
