...
# [GCC 8.4.0]
# Embedded file name: /Users/taborc/git/spog/flask_extras/flask_extras/decorators.py
# Compiled at: 2016-12-05 14:34:43
"""App view decorators."""
        ...
        def inner(*args, **kwargs):
            if headers:
                ...
                if len(s1) != len(matches):
                    raise ValueError(('Missing required header(s): {}').format(list(diff)))
    ...
    def outer(func, *args, **kwargs):
        ...
        def inner(*args, **kwargs):
            if cookies:
                ...
                diff = s1.difference(s2)
                if len(s1) != len(matches):
                    raise ValueError(('Missing required cookie(s): {}').format(list(diff)))
            return func(*args, **kwargs)
        ...
        def inner(*args, **kwargs):
            if params:
                if isinstance(params, list):
                    ...
                    diff = s1.difference(s2)
                    if len(s1) != len(matches):
                        raise ValueError(('Missing required arg(s): {}').format(list(diff)))
                else:
                    for param, val in params.items():
                        arg = request.args.get(param)
                        if arg is None:
                            raise ValueError(('Missing param `{}`').format(param))
                        if arg != val:
                            raise ValueError(('Invalid value `{}` for param {}.').format(arg, param))
        ...
        def inner(*args, **kwargs):
            if request.method == 'POST':
                if values:
                    ...
                    diff = s1.difference(s2)
                    if len(s1) != len(matches):
                        raise ValueError(('Missing required form field(s): {}').format(list(diff)))
            return func(*args, **kwargs)
