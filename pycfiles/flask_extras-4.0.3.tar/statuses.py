...
# [GCC 8.4.0]
# Embedded file name: /Users/taborc/git/spog/flask_extras/flask_extras/views/statuses.py
# Compiled at: 2016-09-06 20:14:31
...
def page_400(error):
    """400 page."""
    return render_template('status_codes/400.html', code=400, desc='bad request', error=error)


def page_401(error):
    """401 page."""
    return render_template('status_codes/401.html', code=401, desc='unauthorized access', error=error)


def page_403(error):
    """403 page."""
    return render_template('status_codes/403.html', code=403, desc='forbidden', error=error)


def page_404(error):
    """404 page."""
    return render_template('status_codes/404.html', code=404, desc='not found', error=error)


def page_500(error):
    """500 page."""
    return render_template('status_codes/500.html', code=500, desc='internal server error', error=error)


def page_503(error):
    """503 page."""
    return render_template('status_codes/500.html', code=503, desc='service unavailable', error=error)
...
def _get_viewfuncs():
    ...
    """
    return {name:func for name, func in globals().iteritems() if _isview(name, func) if _isview(name, func)}
