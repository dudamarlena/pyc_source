...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ropgadget/loaders/pe.py
...
class PEFlags(object):
    IMAGE_MACHINE_INTEL_386 = 332
    IMAGE_MACHINE_AMD_8664 = 34404
...
class PE(object):
    ...
    def __getPEOffset(self):
        ...
        if self.__binary[self.__PEOffset:self.__PEOffset + 4] != unhexlify('50450000'):
    ...
    def getDataSections(self):
        ...
        for section in self.__sections_l:
            if section.Characteristics & 2147483648:
                ret += [
                 {'name': section.Name, 
    ...
    def getExecSections(self):
        ret = []
        for section in self.__sections_l:
            if section.Characteristics & 536870912:
                ret += [
                 {'name': section.Name, 
                    'offset': section.PointerToRawData, 
                    'size': section.SizeOfRawData, 
    ...
    def getArchMode(self):
        if self.__IMAGE_OPTIONAL_HEADER.Magic == PEFlags.IMAGE_NT_OPTIONAL_HDR32_MAGIC:
            ...
            if self.__IMAGE_OPTIONAL_HEADER.Magic == PEFlags.IMAGE_NT_OPTIONAL_HDR64_MAGIC:
                return CS_MODE_64
            else:
