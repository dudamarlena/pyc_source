...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ropgadget/ropchain/arch/ropmakerx64.py
...
class ROPMakerX64(object):
    ...
    def __buildRopChain(self, write4where, popDst, popSrc, xorSrc, xorRax, incRax, popRdi, popRsi, popRdx, syscall):
        ...
        if dataAddr is None:
            ...
            print "\tp += pack('<Q', 0x%016x) # @ .data + 8" % (dataAddr + 8)
            self.__padding(popRsi, {'rdi': dataAddr})
            print "\tp += pack('<Q', 0x%016x) # %s" % (popRdx['vaddr'], popRdx['gadget'])
            print "\tp += pack('<Q', 0x%016x) # @ .data + 8" % (dataAddr + 8)
            self.__padding(popRdx, {'rdi': dataAddr, 'rsi': dataAddr + 8})
            print "\tp += pack('<Q', 0x%016x) # %s" % (xorRax['vaddr'], xorRax['gadget'])
            self.__padding(xorRax, {'rdi': dataAddr, 'rsi': dataAddr + 8})
            for i in range(59):
                print "\tp += pack('<Q', 0x%016x) # %s" % (incRax['vaddr'], incRax['gadget'])
                self.__padding(incRax, {'rdi': dataAddr, 'rsi': dataAddr + 8})
    ...
    def __generate(self):
        ...
        if not xorRax:
            ...
            instr = [
             incRax, incEax, incAx, addRax, addEax, addAx]
