...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/dear/io/_dc_audioread.py
# Compiled at: 2012-04-27 00:49:21
from _io_base import AudioBase
import audioread, struct, numpy, itertools
support_formats = [
 'mp3', 'wav', 'aiff']
...
class Audio(AudioBase):
    ...
    _max_pulse_value = 32768

    def _pcm_bin2num(self, bins, channels, join=False):
        pcms = numpy.array(struct.unpack('<' + 'h' * (len(bins) / 2), bins), float)
        ...
        for i in range(channels):
            chs.append(numpy.array([ pcms[i] for i in xrange(i, len(pcms), channels) ]))

        if not join:
            return chs
        return sum(chs) / channels

    def walk(self, win=1024, step=None, start=0, end=None, join_channels=True):
        if not step:
            step = win
        if not end:
            end = self._duration
        ...
        if win_len <= step * 1.5:
            ...
            if len(win_buf) > 0:
                win_buf += '\x00' * (win_len - len(win_buf))
            ...
            if join_channels:
                ...
                if len(win_buf) > 0:
                    yield numpy.append(win_buf, numpy.zeros([win - len(win_buf)], numpy.double))
            else:
                win_buf = [ numpy.array([], dtype=numpy.double) for i in range(fh.channels)
                          ]
                for p, buf in enumerate(fh):
                    if p < start_point:
                        continue
                    ...
                    while len(win_buf[0]) > win:
                        yield [ win_buf[i][:win] for i in range(fh.channels)
                              ]
                        win_buf = [ win_buf[i][step:] for i in range(fh.channels)
                                  ]

            if len(win_buf[0]) > 0:
                yield [ numpy.append(win_buf[i], numpy.zeros([win - len(win_buf[i])], numpy.double)) for i in range(fh.channels)
                      ]
        fh.close()