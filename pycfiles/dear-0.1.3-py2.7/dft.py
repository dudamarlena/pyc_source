...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/dear/spectrum/dft.py
...
class Spectrum(SpectrumBase):
    ...
    def pre_calculate(win, win_shape, pre=True):
        var = {'WL': win, 
           'W': win_shape(win)}
        if pre:
            arr = 2.0 * numpy.pi * numpy.arange(win) / win
            PRE = numpy.array([ var['W'] * (numpy.cos(arr * k) - numpy.sin(arr * k) * complex(0.0, 1.0)) for k in xrange(win / 2 + 1)
                              ])
            var['PRE'] = PRE
    ...
    def walk(self, win=1024, step=512, start=0, end=None, join_channels=True, win_shape=numpy.hamming, mpre=False):
