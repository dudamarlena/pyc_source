...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/dear/spectrogram.py
# Compiled at: 2012-06-08 02:18:53
...
def plot_spectrogram(spec, Xd=(0, 1), Yd=(0, 1), norm=colo.LogNorm(vmin=1e-06), figname=None):
    x_min, x_max = Xd
    y_min, y_max = Yd
    ...
    for ch, data in enumerate(spec):
        ...
        im = NonUniformImage(ax, interpolation='bilinear', cmap=cm.gray_r, norm=norm)
...
if __name__ == '__main__':
    ...
    if graph == 'dft':
        ...
        for t, freqs in enumerate(gram.walk(Q=Q, freq_base=55.0, freq_max=7040, hop=hop, start=st, end=to, join_channels=True)):
            if t % 100 == 0:
        ...
        print 'total:', int((r_to - st) / hop)
        for t, freqs in enumerate(gram.walk(N=N, freq_base=55.0, freq_max=7040, hop=hop, start=st, end=to, resize_win=rw)):
            if t % 100 == 0:
                sys.stdout.write('%d...' % t)
                sys.stdout.flush()
            spec[0].append(freqs)
        ...
        print 'total:', int((r_to - st) / hop)
        for t, freqs in enumerate(gram.walk(N=N, freq_base=freqs[0], freq_max=freqs[1], start=st, end=to, combine=combine, twin=win, thop=hop)):
            if t % 100 == 0:
                sys.stdout.write('%d...' % t)
                sys.stdout.flush()
            spec[0].append(freqs)
        ...
        print ''
    dBmax, dBmin = (-15.0, -70.0)
    if graph in ('dft', 'cqt', 'cnt'):
        magmin = 10 ** (dBmin / 20)
        for g in spec:
            for i, frame in enumerate(g):
                g[i] = 20 * np.log10(np.maximum(frame / 20.0, magmin))
