...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/dear/analysis/mfcc.py
...
class MFCCs(DFTSpectrum):
    ...
    @staticmethod
    def mel2freq(mel):
        return 700 * (np.exp(mel / 1000.0 * np.log(1 + 1000.0 / 700.0)) - 1)
    ...
    def walk(self, N=20, freq_min=0, freq_max=7040.0, win=2048, step=1024, start=0, end=None, win_shape=np.hanning):
        samplerate = self.audio.samplerate
        nqfreq = samplerate / 2.0
        N = int(N)
