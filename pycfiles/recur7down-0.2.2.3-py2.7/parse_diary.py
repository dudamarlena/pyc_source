...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/recur7down/parse_diary.py
# Compiled at: 2018-01-22 01:41:29
import sys
try:
    reload(sys)
...
class Hparser(object):
    ...
    def _product(self):
        try:
            self.pid = self.tree.xpath('//div[@class="value product-name"]/a/@href')[0].split('/')[(-1)]
        except Exception as e:
            print 'X',

    def _content(self):
        try:
            self.content = self.tree.xpath('//li[@class="diary-item"]')
            self.dn = len(self.content)
        except Exception as e:
            print e, self.diaryID, 'X content',
    ...
    def __repr__(self):
        return ('diary: {} of product: {} has {} posts, all {} posts finished parsing').format(self.diaryID, self.pid, self.dn, self.N)
