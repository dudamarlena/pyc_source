...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\spiderlib\bak.py
# Compiled at: 2020-03-24 22:03:17
# Size of source mod 2**32: 22482 bytes
from typing import Union, Optional
name = 'spiderlib'
author = '吴超  QQ 377486624'
import re, subprocess, time, traceback, os, platform, redis, asyncio
from pyppeteer import launch
from lxml import html
import pymysql, numpy as np, base64, requests
...
class Template:
    ...
    def __init__(self, urls: list, expresses: dict, next: str, fields_tag: dict, fields: dict, is_list: bool, hooker: Hooker=Hooker()):
        ...
        """
        if not urls:
            raise AssertionError('urls参数不能空')
        else:
...
class Spider:
    ...
    def list(self, urls: Union[(list, str)]='', expresses: dict={}, fields_tag: str='', fields: dict={}, next: str='', hooker: Hooker=Hooker()):
        ...
        :return:
        """
        return self._Spider__page(urls=urls, is_list=True, expresses=expresses, fields_tag=fields_tag, fields=fields, next=next, hooker=hooker)
    ...
    def page(self, urls: Union[(list, str)]='', expresses: dict={}, fields_tag: str='', fields: dict={}, hooker: Hooker=Hooker()):
        ...
        :return:
        """
        return self._Spider__page(urls=urls, is_list=False, expresses=expresses, fields_tag=fields_tag, fields=fields, hooker=hooker)
    ...
    def __pre_save(self, page: Page) -> None:
        ...
        for k, v in page.template.fields.items():
            if str(v) not in expresses_keys:
    ...
    def __run(self):
        ...
        while self.scheduler.len():
            ...
            f = asyncio.ensure_future(self._Spider__download(page))
            self.event_loop.run_until_complete(f)
            normal_flag = True
            if f.result():
                if page.template.fields:
                    normal_flag = self._Spider__save(page)
            self._Spider__after_save(page, normal_flag)
