...
# Embedded file name: build/bdist.linux-x86_64/egg/FormatTransformer/TestGenericInput.py
# Compiled at: 2016-03-14 06:00:41
import TransformManager as t, nx_format_adapter as n, cif_format_adapter as cf, CifFile.drel as drel
from CifFile import CifDic
import unittest, os, numpy, math
...
class GITransformToNXTestCase(unittest.TestCase):
    ...
    def testPlainTransform(self):
        ...
        out_bundle = [
         'incident wavelength', 'wavelength id']
        self.transformer.manage_transform(out_bundle, 'testfiles/adsc_jrh_testfile.cif', 'cif', self.nx_filename, 'nexus')
    ...
    def testAxisTransform(self):
        ...
        out_bundle = [
         'simple detector axis id', 'simple detector axis vector mcstas', 'simple detector axis offset mcstas']
        self.transformer.manage_transform(out_bundle, 'testfiles/adsc_jrh_testfile.cif', 'cif', 'adsc_jrh_testfile.nx', 'nexus')
...
class GITransformFromNXTestCase(unittest.TestCase):
    ...
    def testPlainTransform(self):
        """Test translation that doesnt need dREL"""
        out_bundle = [
         'wavelength id', 'incident wavelength']
        self.transformer.manage_transform(out_bundle, 'testfiles/nexus-multi-image.nx', 'nexus', self.cf_filename, 'cif')
        self.check_vals('incident wavelength', [0.711955], 'Real', 0.001)

    def testAxisTransform(self):
        """Test that axes are transformed correctly"""
        out_bundle = [
         'axis id', 'axis vector', 'axis offset']
        self.transformer.manage_transform(out_bundle, 'testfiles/adsc_jrh_testfile.nx', 'nexus', 'axis_test.cif', 'cif')
...
class GITransformToNXImageTestCase(unittest.TestCase):
    ...
    def testImageTransform(self):
        ...
        out_bundle = [
         ...
         'simple scan data']
        self.transformer.manage_transform(out_bundle, 'testfiles/multi-image-test.cif', 'cif', self.nx_filename, 'nexus')
        self.nxa.open_file(self.nx_filename)
        self.nxa.open_data_unit()
    ...
    def testImageAxes(self):
        ...
        out_bundle = [
         'simple scan frame scan id', 'simple scan data',
...
class GITransformFromNXImageTestCase(unittest.TestCase):
    ...
    def testImageTransform(self):
        ...
        import numpy
        out_bundle = [
         '2D data', '2D data identifier']
        self.transformer.manage_transform(out_bundle, 'testfiles/nexus-multi-image.nx', 'nexus', self.cif_filename, 'cif')
...
class RoundTripTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.cif_bundle = ['incident wavelength', 'wavelength id', 'axis id',
         ...
         'data frame scan id']
        self.transformer.manage_transform(self.nexus_bundle, 'testfiles/multi-image-test.cif', 'cif', self.nexus_file, 'nexus')
        nxa = n.NXAdapter(n.canonical_groupings)
        nxa.open_file(self.nexus_file)
        print nxa.filehandle.tree
        self.transformer.manage_transform(self.cif_bundle, self.nexus_file, 'nexus', 'round_trip_return.cif', 'cif')
