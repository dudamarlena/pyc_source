...
# Embedded file name: /Users/jgsilva/Astrophysics/Packages/ACTIN/actin/actin_files/ac_calc_ind.py
# Compiled at: 2019-12-05 17:08:51
# Size of source mod 2**32: 14179 bytes
from __future__ import print_function
from __future__ import division
import os, sys, numpy as np, ac_settings as ac_set, ac_get_win
...
def check_lines(wave, sel_lines, verb=False):
    ...
    if type(wave[0]) is np.ndarray:
        ...
        spec_type = '2d'
    if type(wave[0]) in [np.float, np.float64]:
    ...
    for k in range(rows):
        ...
        if spec_type == '2d':
            ...
            for i in range(len(wave)):
                if min_wave > wave[i][0]:
                    if max_wave < wave[i][(-1)]:
                        order.append(i)
...
def calc_ind(sel_lines):
    ...
    for i in range(len(sel_ind)):
        var = [sel_lines['ind_var'][k] for k in range(rows) if ind_ids[k] == sel_ind[i]]
        flux = [sel_lines['flux'][k] for k in range(rows) if ind_ids[k] == sel_ind[i]]
        err = [sel_lines['error'][k] for k in range(rows) if ind_ids[k] == sel_ind[i]]
        flg = [sel_lines['flg'][k] for k in range(rows) if ind_ids[k] == sel_ind[i]]
        frac_neg = [sel_lines['frac_neg'][k] for k in range(rows) if ind_ids[k] == sel_ind[i]]
        snr = [sel_lines['snr'][k] for k in range(rows) if ind_ids[k] == sel_ind[i]]
        ln_c = [sel_lines['ln_c'][k] for k in range(rows) if ind_ids[k] == sel_ind[i]]
        npixels = [sel_lines['npixels'][k] for k in range(rows) if ind_ids[k] == sel_ind[i]]
