...
# Embedded file name: /Users/jgsilva/Astrophysics/Packages/ACTIN/actin/actin_files/ac_tools.py
# Compiled at: 2019-12-05 16:51:45
# Size of source mod 2**32: 14182 bytes
...
def compute_flux(wave, flux, blaze, noise, ln_ctr, ln_win, bandtype, frac=True, test_plot=False):
    ...
    if bandtype == 'tri':
        ...
        bandfunc = -np.abs(wave - ln_ctr) / ln_win + 1.0
        bandfunc = np.where(bandfunc > 0, bandfunc, bandfunc * 0.0)
    if bandtype == 'sq':
        wmin = ln_ctr - ln_win / 2.0
        wmax = ln_ctr + ln_win / 2.0
        cond = (wave > wmin) & (wave < wmax)
        bandfunc = np.ones(len(wave))
        bandfunc = np.where(cond, 1.0, 0.0)
    if frac == False:
        flux_win = flux[cond]
        wave_win = wave[cond]
    ...
    if frac == True:
        ...
        if bandtype == 'sq':
            ...
            if bandtype == 'tri':
                ...
                if bandwidth == wmax - wmin:
                    pass
                else:
                    print('*** bandwidth test 1 ERROR')
            bandwidth = frac_r * px_size_win[(-1)] + sum(px_size[cond][1:]) + px_size_win[0] * frac_l
            if bandwidth == wmax - wmin:
                pass
            else:
                print('*** bandwidth test 2 ERROR')
    if not noise:
        noise = 0.0
    ...
    if test_plot == True:
        ...
        plt.show()
    return (
     f_sum, f_sum_var, bandfunc, npixels, flg_negflux, r_neg_ln)
...
def check_negflux(flux, verb=True):
    ...
    if neg_pixs > 0:
        ...
        flg_negflux = 'negFlux'
    return (
     r_neg_ln, neg_flux, tot_flux, flg_negflux)
