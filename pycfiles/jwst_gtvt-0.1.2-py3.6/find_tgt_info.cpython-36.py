...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/jwst_gtvt/find_tgt_info.py
# Compiled at: 2020-04-23 12:18:44
# Size of source mod 2**32: 41692 bytes
...
def allowed_max_sun_roll(sun_p):
    ...
    if sun_p > 2.5 * D2R:
        max_sun_roll = abs_max_sun_roll - 1.7 * D2R * (sun_p - 2.5 * D2R) / 2.7 / D2R
    else:
        max_sun_roll = abs_max_sun_roll
    max_sun_roll -= 0.1 * D2R
    return max_sun_roll


def allowed_max_vehicle_roll(sun_ra, sun_dec, ra, dec):
    vehicle_pitch = math.pi / 2.0 - angular_sep(sun_ra, sun_dec, ra, dec)
    sun_roll = 5.2 * D2R
    last_sun_roll = 0.0
...
def get_target_ephemeris(desg, start_date, end_date, smallbody=False):
    ...
    if smallbody:
        ...
        bodytype = 'majorbody'
    obj = Horizons(id=desg, location='500@-170', id_type=bodytype, epochs={'start':start_date, 
     'stop':end_date,  'step':'1d'})
...
def window_summary_line(fixed, wstart, wend, pa_start, pa_end, ra_start, ra_end, dec_start, dec_end, cvz=False):
    ...
    if cvz:
        ...
        line = ' {:15} {:11} {:11.2f} '.format(Time(wstart, format='mjd', out_subfmt='date').isot, Time(wend, format='mjd', out_subfmt='date').isot, wend - wstart)
...
def main(args, fixed=True):
    ...
    search_start = Time((args.start_date), format='iso').mjd if args.start_date is not None else 58849.0
    search_end = Time((args.end_date), format='iso').mjd if args.end_date is not None else 60309.0
    if not 58849.0 <= search_start <= 60309.0:
    ...
    if fixed:
        if args.ra.find(':') > -1:
            ra = convert_ddmmss_to_float(args.ra) * 15.0 * D2R
    ...
    if not args.no_verbose:
        print(('Checked interval [{}, {}]'.format(Time(search_start, format='mjd', out_subfmt='date').isot, Time((search_start + span), format='mjd', out_subfmt='date').isot)),
          file=table_output)
    if pa == 'X':
    ...
    if 1 == 1:
        ...
        for itime in range(istart, iend):
            ...
            if iflag:
                if not tgt_is_in:
                    ...
                    if fixed:
                        if not args.no_verbose:
                            print((fmt.format(Time(atime, format='mjd', out_subfmt='date').isot, minV3PA, maxV3PA, minNIRCam_PA, maxNIRCam_PA, minNIRSpec_PA, maxNIRSpec_PA, minNIRISS_PA, maxNIRISS_PA, minMIRI_PA, maxMIRI_PA, minFGS_PA, maxFGS_PA)),
                              file=table_output)
                    else:
                        args.no_verbose or print((fmt.format(Time(atime, format='mjd', out_subfmt='date').isot, ra[i] * R2D, dec[i] * R2D, minV3PA, maxV3PA, minNIRCam_PA, maxNIRCam_PA, minNIRSpec_PA, maxNIRSpec_PA, minNIRISS_PA, maxNIRISS_PA, minMIRI_PA, maxMIRI_PA, minFGS_PA, maxFGS_PA)),
                          file=table_output)
            else:
                tgt_is_in = False
                times.append(Time(atime, format='mjd').datetime)
        ...
        tab = Table([times, minV3PA_data, maxV3PA_data, minNIRCam_PA_data, maxNIRCam_PA_data,
         ...
         minMIRI_PA_data, maxMIRI_PA_data, minFGS_PA_data, maxFGS_PA_data],
          names=('Date', 'V3PA min', 'V3PA max', 'NIRCam min', 'NIRCam max', 'NIRSpec min',
                 'NIRSpec max', 'NIRISS min', 'NIRISS max', 'MIRI min', 'MIRI max',
                 'FGS min', 'FGS max'))
...
def get_table(ra, dec, instrument=None, start_date=None, end_date=None, save_table=None, v3pa=None, fixed=True, verbose=True):
    ...
    if save_table is not None:
        ...
        ECL_FLAG = False
        A_eph = EPH.Ephemeris((join(dirname(abspath(__file__)), 'horizons_EM_jwst_wrt_sun_2020-2024.txt')), ECL_FLAG, verbose=(args.no_verbose))
        search_start = Time(start_date, format='iso').mjd if start_date is not None else 58849.0
        search_end = Time(end_date, format='iso').mjd if end_date is not None else 60309.0
        if not 58849.0 <= search_start <= 60309.0:
            if start_date is not None:
                raise ValueError('Start date {} outside of available ephemeris {} to {}'.format(start_date, '2020-01-01', '2023-12-31'))
        if not 58849.0 <= search_end <= 60309.0:
        ...
        pa = 'X'
        if ra.find(':') > -1:
            ra = convert_ddmmss_to_float(ra) * 15.0 * D2R
    ...
    if 1 == 1:
        ...
        for itime in range(istart, iend):
            ...
            if iflag:
                ...
                if fixed:
                    if verbose:
                        print((fmt.format(Time(atime, format='mjd', out_subfmt='date').isot, V3PA, minV3PA, maxV3PA, nomNIRCam_PA, minNIRCam_PA, maxNIRCam_PA, nomNIRSpec_PA, minNIRSpec_PA, maxNIRSpec_PA, nomNIRISS_PA, minNIRISS_PA, maxNIRISS_PA, nomMIRI_PA, minMIRI_PA, maxMIRI_PA, nomFGS_PA, minFGS_PA, maxFGS_PA)),
                          file=table_output)
                else:
                    if verbose:
                        print((fmt.format(Time(atime, format='mjd', out_subfmt='date').isot, ra[i] * R2D, dec[i] * R2D, V3PA, minV3PA, maxV3PA, nomNIRCam_PA, minNIRCam_PA, maxNIRCam_PA, nomNIRSpec_PA, minNIRSpec_PA, maxNIRSpec_PA, nomNIRISS_PA, minNIRISS_PA, maxNIRISS_PA, nomMIRI_PA, minMIRI_PA, maxMIRI_PA, nomFGS_PA, minFGS_PA, maxFGS_PA)),
                          file=table_output)
            else:
                tgt_is_in = False
                times.append(Time(atime, format='mjd').datetime)
        ...
        tab = Table([times, V3PA_data, minV3PA_data, maxV3PA_data, nomNIRCam_PA_data, minNIRCam_PA_data, maxNIRCam_PA_data,
         ...
         nomMIRI_PA_data, minMIRI_PA_data, maxMIRI_PA_data, nomFGS_PA_data, minFGS_PA_data, maxFGS_PA_data],
          names=('Date', 'V3PA', 'V3PA min', 'V3PA max', 'NIRCam nom', 'NIRCam min',
                 'NIRCam max', 'NIRSpec nom', 'NIRSpec min', 'NIRSpec max', 'NIRISS nom',
                 'NIRISS min', 'NIRISS max', 'MIRI nom', 'MIRI min', 'MIRI max',
                 'FGS nom', 'FGS min', 'FGS max'))
    return tab
