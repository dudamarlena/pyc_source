...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/jwst_gtvt/rotationsx.py
# Compiled at: 2020-02-10 11:10:52
# Size of source mod 2**32: 32522 bytes
...
class GalacticPole(object):
    __doc__ = 'Represents coordinates of galactic pole.'
...
class NumericList(list):
    __doc__ = 'List class that supports multiplication.  Only valid for numbers.'
...
class Vector(object):
    __doc__ = 'Class to encapsulate vector data and operations.'
    ...
    def create_matrix(self):
        ...
        column = [
         self.x, self.y, self.z]
        return Matrix(map(lambda element: [element], column))
...
NULL_3D_VECTOR = Vector(0, 0, 0)

class CelestialVector(Vector):
    __doc__ = 'Class to encapsulate a unit vector on the celestial sphere.'
    ...
    def update_cartesian(self, x=None, y=None, z=None):
        ...
        if x != None:
            ...
            if z != None:
                self.z = z
            self.ra = atan2(self.y, self.x)
            if self.ra < 0:
                self.ra += 2 * pi
        self.dec = math2.asin2(self.z)
    ...
    def transform_frame(self, new_frame):
        ...
        if new_frame == 'gal':
            if self.frame == 'eq':
                b = math2.asin2(cos(self.dec) * cos(NGP.longitude) * cos(self.ra - NGP.latitude) + sin(self.dec) * sin(NGP.longitude))
                l = atan2(sin(self.dec) - sin(b) * sin(NGP.longitude), cos(self.dec) * sin(self.ra - NGP.latitude) * cos(NGP.longitude)) + NGP.anode
                result = CelestialVector(l, b, degrees=False)
        else:
            if new_frame == 'eq' and self.frame == 'gal':
                l = self.ra
                b = self.dec
                dec = math2.asin2(cos(b) * cos(NGP.longitude) * sin(l - NGP.anode) + sin(b) * sin(NGP.longitude))
                ra = atan2(cos(b) * cos(l - NGP.anode), sin(b) * cos(NGP.longitude) - cos(b) * sin(NGP.longitude) * sin(l - NGP.anode)) + NGP.latitude
                result = CelestialVector(ra, dec, degrees=False)
            else:
                if new_frame == 'gal' and self.frame == 'ec' or new_frame == 'ec' and self.frame == 'gal':
...
class Attitude(CelestialVector):
    __doc__ = 'Defines an Observatory attitude by adding a position angle.'
...
def vel_ab(U, Vel):
    ...
    c = 299790.0
    Beta = Vel * (1.0 / c)
    rgamma = math.sqrt(1.0 - dot(Beta, Beta))
    ubeta = dot(U, Beta)
    return (U * rgamma + Beta * (1.0 + (1.0 - rgamma) * ubeta / dot(Beta, Beta))) * (1.0 / (1.0 + ubeta))
...
def QX(angle):
    ...
    return Quaternion(Vector(math.sin(-angle / 2.0), 0.0, 0.0), math.cos(angle / 2.0))


def QY(angle):
    """Creates rotation quaternion about Y axis, assuming coordinate frame is rotated"""
    return Quaternion(Vector(0.0, math.sin(-angle / 2.0), 0.0), math.cos(angle / 2.0))


def QZ(angle):
    """Creates rotation quaternion about Z axis, assuming coordinate frame is rotated"""
    return Quaternion(Vector(0.0, 0.0, math.sin(-angle / 2.0)), math.cos(angle / 2.0))


def Qmake_a_point(V):
    """Creates a pure Q, i.e. defines a pointing not a rotation"""
    return Quaternion(V, 0.0)
...
are 321 mapped to ra,-dec,-pa"""
    ...
    q3 = Q.q3
    q4 = Q.q4
    ra = math.atan2(2.0 * (q1 * q2 - q3 * q4), q1 * q1 - q2 * q2 - q3 * q3 + q4 * q4)
    if ra < 0.0:
        ra += PI2
    arg = min(1.0, max(-1.0, 2.0 * (q1 * q3 + q2 * q4)))
    dec = math.asin(arg)
    pa = math.atan2(-2.0 * (q2 * q3 - q1 * q4), -q1 * q1 - q2 * q2 + q3 * q3 + q4 * q4)
    if pa < 0.0:
        pa += PI2
    return (
     ra, dec, pa)
...
def cvt_body2inertial_Q_to_c1c2pa_tuple(Q):
    ...
    r21 = 2.0 * (Q.q1 * Q.q2 + Q.q3 * Q.q4)
    r31 = 2.0 * (Q.q1 * Q.q3 - Q.q2 * Q.q4)
    r32 = 2.0 * (Q.q2 * Q.q3 + Q.q1 * Q.q4)
...
class Quaternion:
    __doc__ = 'This representation is used by Wertz and Markley '
    ...
    def __mul__(self, rs):
        ...
        Q = Quaternion(Vector(0.0, 0.0, 0.0), 0.0)
