...
# [GCC 8.4.0]
# Embedded file name: c:\dev\pylib\visvis\vvmovie\images2swf.py
# Compiled at: 2016-03-22 04:56:47
# Size of source mod 2**32: 30061 bytes
""" Module images2swf
    ...
    def Append(self, bits):
        if isinstance(bits, BitArray):
            ...
            if isinstance(bits, int):
                bits = str(bits)
            raise isinstance(bits, string_types) or ValueError('Append bits as strings or integers!')
        for bit in bits:
...
class SetBackgroundTag(ControlTag):
    __doc__ = ' Set the color in 0-255, or 0-1 (if floats given). '
