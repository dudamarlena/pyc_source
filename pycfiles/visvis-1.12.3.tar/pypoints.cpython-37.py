...
# [GCC 8.4.0]
# Embedded file name: c:\dev\pylib\visvis\utils\pypoints.py
# Compiled at: 2019-03-28 09:13:26
# Size of source mod 2**32: 57800 bytes
""" Module pypoints
...
class BasePoints(object):
    ...
    def angle(self, *p):
        ...
        if len(p) == 1:
            ...
            if len(p) == 0:
                ...
                p = is_Point_or_Pointset(p) or Point(p)
    ...
    def __sub__(self, p):
        ...
        p1, p2 = self, p
        if SHOW_SUBTRACTBUG_WARNING:
            if p1 is not p3:
                try:
                    raise ValueError()
                except Exception:
                    print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
                    print('Note that previous versions of pypoints contained a bug in the minus operator. To prevent this message, use the subtract method instead of the operator. To get the same behavior as before, replace "A-B" with "B.subtract(A)". Info: https://github.com/almarklein/visvis/issues/30. Trace:')
                    self._showTrace()
                    print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
    ...
    def _resize_if_required(self, n=None):
        ...
        if n is None:
            ...
            if n > internalLen:
                ...
                if n * 4 <= internalLen and internalLen > 16:
...
class Aarray(np.ndarray):
    ...
    sampling = property(_get_sampling, _set_sampling, None, 'A tuple with the sample distance for each dimension.')
    ...
    origin = property(_get_origin, _set_origin, None, 'A tuple with the origin for each dimension.')
