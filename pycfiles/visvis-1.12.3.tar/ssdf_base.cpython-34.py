...
# [GCC 8.4.0]
# Embedded file name: d:\dev\pylib\visvis\utils\ssdf\ssdf_base.py
    ...
    if isstruct(ob1) or isinstance(ob1, dict):
        ...
        if isinstance(ob1, (tuple, list)):
            ...
            if isinstance(ob1, VirtualArray):
                ...
                if np and isinstance(ob1, np.ndarray):
                    if not isinstance(ob2, np.ndarray):
                        return '<type does not match>'
                    if not (ob1.shape == ob2.shape and ob1.dtype == ob2.dtype and (ob1 == ob2).sum() == ob1.size):
                        return '<array does not match>'
                elif not ob1 == ob2:
...
class SSDFWriter:
    ...
    def flatten_tree(self, block, sort=False):
        ...
        if sort:
            if listOfLists and block._type == _TYPE_DICT:
