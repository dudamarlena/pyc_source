        ...
        if isinstance(ob1, (tuple, list)):
            ...
            if isinstance(ob1, VirtualArray):
                if not isinstance(ob2, VirtualArray):
                    return '<type does not match>'
                if ob1.shape == ob2.shape:
                    if not (ob1.dtype == ob2.dtype and ob1.data == ob2.data):
                        return '<array does not match>'
            elif np and isinstance(ob1, np.ndarray):
                if not isinstance(ob2, np.ndarray):
                    return '<type does not match>'
                if ob1.shape == ob2.shape:
                    if not (ob1.dtype == ob2.dtype and (ob1 == ob2).sum() == ob1.size):
                        return '<array does not match>'
            elif not ob1 == ob2:
...
def _isvalidname(name):
    ...
    """
    return name and isinstance(name, string_types) or None
...
class Struct(object):
    ...
    def __init__(self, a_dict=None):
        if a_dict is None:
            return
            tmp = isinstance(a_dict, dict) or isstruct(a_dict) or 'Struct can only be initialized with a Struct or a dict.'
