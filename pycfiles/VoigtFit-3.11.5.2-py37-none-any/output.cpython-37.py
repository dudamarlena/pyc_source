...
# [GCC 8.4.0]
# Embedded file name: /Users/krogager/Projects/VoigtFit/build/lib/VoigtFit/output.py
# Compiled at: 2020-03-29 15:55:41
# Size of source mod 2**32: 75357 bytes
...
import scipy.interpolate as spline2d
import numpy as np
from . import Asplund
...
default_comp = {'color':'b', 
 'ls':'-',  'alpha':1.0, 
 'lw':1.0,  'ymin':0.87, 
 'ymax':0.92,  'text':None, 
 'loc':'above'}
default_highlight_comp = {'color':'DarkOrange',  'ls':'-',  'alpha':0.7, 
 'lw':2.0,  'ymin':0.85, 
 'ymax':1.0,  'text':None, 
 'loc':'above'}
default_line = {'color':'r', 
 'ls':'-',  'lw':1.0, 
 'alpha':1.0}
default_hl_line = {'color':'orange',  'ls':'--',  'lw':1.0, 
 'alpha':1.0}
...
class CompProp(object):
    ...
    def get_text_props(self, ion):
        ...
        if self.properties[ion]['loc'].lower() == 'top':
            ...
            if self.properties[ion]['loc'].lower() == 'bottom':
...
def plot_all_lines(dataset, plot_fit=True, rebin=1, fontsize=12, xmin=None, xmax=None, ymin=None, ymax=None, max_rows=4, filename=None, subsample_profile=1, npad=50, residuals=True, norm_resid=False, legend=True, loc='left', show=True, default_props={}, element_props={}, highlight_props=None, label_all_ions=False, xunit='vel', line_props=None, hl_line_props=None):
    """
    ...
    for contents in pages:
        ...
        if len(contents) == 1:
            ...
            fig.subplots_adjust(left=0.1, right=0.98, top=0.98, hspace=0.03,
              bottom=0.14)
        if len(contents) % 2 == 1:
            if len(contents) > 1:
                add_on = 1
                    ...
                    if line_tag not in list(regionidx_of_line.keys()):
                        ...
                        for idx in range(num_regions):
                            ...
                            if num > rows * columns:
                                ...
                                _, LIV = plot_single_line(dataset, line_tag, index=idx, axis=ax, **plot_line_kwargs)
                        ...
                        if num <= len(contents) - (1 - add_on):
                            ...
                            ax.set_xlabel('${\\rm Wavelength\\ \\ (\\AA)}$', fontsize=12)
                        else:
...
def plot_single_line(dataset, line_tag, index=0, plot_fit=False, loc='left', rebin=1, nolabels=False, axis=None, fontsize=12, subsample_profile=1, xmin=None, xmax=None, ymin=None, ymax=None, show=True, npad=50, legend=True, residuals=False, norm_resid=False, default_props={}, element_props={}, highlight_props=None, label_all_ions=False, xunit='velocity', line_props=None, hl_line_props=None, sort_f=True):
    """
    ...
    if isinstance(highlight_props, list):
        ...
        if highlight_props is None:
            ...
            if line_tag not in dataset.all_lines:
                ...
                for line in region.lines:
                    l0 = line.l0
                    delta_v = (l0 * (dataset.redshift + 1) - l_ref) / l_ref * 299792.0
                    if not np.abs(delta_v) <= 200:
            ...
            if plot_fit and not isinstance(dataset.best_fit, dict):
                if isinstance(dataset.pars, dict):
                    if residuals:
                        divider = make_axes_locatable(ax)
                        cax = divider.append_axes('top', size='20%', pad=0.0, sharex=ax)
                    else:
                        if isinstance(kernel, float):
                            ...
                            if isinstance(kernel, np.ndarray):
                                if not kernel.shape[0] == len(x_orig) * kernel_nsub:
                        ...
                        for line in list(dataset.lines.values()):
                            if line.active:
                                ...
                                l_center = l0 * (dataset.redshift + 1.0)
                                ...
                                for n in range(n_comp):
                                    ...
                                    tau += voigt.Voigt(wl_line, l0, f, (10 ** logN),
                                      (100000.0 * b), gam, z=z)
                                    if ion in list(highlight_props.keys()):
                                        tau_hl += voigt.Voigt(wl_line, l0, f, (10 ** logN),
                                          (100000.0 * b), gam, z=z)
                                        if xunit == 'vel':
            ...
            if residuals:
                if plot_fit:
                    ...
                    ymax = ymax or max(1.0 + 4 * np.nanmedian(err[view_part]), 1.08)
                ...
                if xunit == 'wl':
                    ...
                    ax.errorbar(x, spectrum, err, ls='', color='gray', lw=1.0)
                    ax.plot(x, spectrum, color='k', drawstyle='steps-mid', lw=1.0)
            else:
                ax.plot(vel, masked_range, color='0.7', drawstyle='steps-mid', lw=0.9)
                ax.errorbar(vel, spectrum, err, ls='', color='gray', lw=1.0)
                ax.plot(vel, spectrum, color='k', drawstyle='steps-mid', lw=1.0)
        ax.axhline(0.0, ls='--', color='0.7', lw=0.7)
        if plot_fit and not isinstance(dataset.best_fit, dict):
            if isinstance(dataset.pars, dict):
                if xunit == 'wl':
                ...
                if residuals:
                    ...
                    if xunit == 'wl':
                        cax.plot(x, masked_resid, color='0.7', drawstyle='steps-mid',
                          lw=0.9)
                        cax.plot(x, resid, color='k', drawstyle='steps-mid', lw=1.0)
                    else:
                        cax.plot(vel, masked_resid, color='0.7', drawstyle='steps-mid',
                          lw=0.9)
                        cax.plot(vel, resid, color='k', drawstyle='steps-mid', lw=1.0)
                    if norm_resid:
                        ...
                        if xunit == 'wl':
                            cax.errorbar(x, resid, err, ls='', color='gray', lw=1.0)
                            cax.plot(x, (3 * err), ls=':', color='crimson', lw=1.0)
                            cax.plot(x, (-3 * err), ls=':', color='crimson', lw=1.0)
                        else:
                            cax.errorbar(vel, resid, err, ls='', color='gray', lw=1.0)
                            cax.plot(vel, (3 * err), ls=':', color='crimson', lw=1.0)
                            cax.plot(vel, (-3 * err), ls=':', color='crimson', lw=1.0)
                        res_min = np.nanmax(4 * err)
                        res_max = np.nanmin(-4 * err)
                    cax.axhline(0.0, ls='--', color='0.7', lw=0.7)
            ...
            ax.axhline((1.0 + cont_err), ls=':', color='gray')
            ax.axhline((1.0 - cont_err), ls=':', color='gray')
            ...
            if loc == 'right':
                ...
                if loc == 'left':
                    ...
                    loc = 'left'
            if legend:
                ax.text(label_x, 0.08, line_string, va='bottom', ha=loc, transform=(ax.transAxes),
                  fontsize=fontsize,
    ...
    if line_tag not in dataset.all_lines:
        ...
        if isinstance(dataset.best_fit, dict) or isinstance(dataset.pars, dict):
            if isinstance(kernel, float):
                ...
                if isinstance(kernel, np.ndarray):
                    if not kernel.shape[0] == len(x) * kernel_nsub:
                        raise AssertionError
                elif kernel_nsub > 1:
...
def plot_excitation(dataset, molecule):
    ...
    for num in range(Jmax + 1):
        ...
        err = pars[par_name].stderr if pars[par_name].stderr else 0.0
        logN_err.append(err)
    ...
    y_err = np.log(10.0) * logN_err
    ax.errorbar(E, y, y_err, color='k', marker='s', ls='')
    ax.set_xlabel('Energy ${\\rm E}_J-{\\rm E}_0$ (K)', fontsize=14)
    ax.set_ylabel('${\\rm ln}\\left(\\frac{N_J}{N_0}\\ \\frac{g_0}{g_J}\\right)$', fontsize=14)
    ...
    T_01_err = tuple(np.percentile(T_dist, [16.0, 84.0]) - T_01)
    T_array = np.linspace(0.0, np.max(E), 100)
    ax.plot(T_array, -T_array / T_01, 'k:')
    ax.text(0.98, 0.98, ('$T_{01} = %.0f_{%+.0f}^{%+.0f}$ K' % ((T_01,) + T_01_err)),
      ha='right',
      va='top',
...
def show_H2_bands(ax, z, bands, Jmax, color='blue', short_labels=False):
    ...
    for band, this_Jmax in zip(bands, Jmax):
        if band not in list(molecules.H2.keys()):
            ...
            if 'B' in band:
                y0 = 0.55
                y1 = y0 + nu % 2 * (0.9 - y0) / float(N_lines)
                y2 = y1 + (0.9 - y0) / float(N_lines)
            else:
                if N_lines == 1:
                    y0 = 0.67
                    y1 = y0
                ...
                for tag in transitions[j]:
                    ...
                    for label in labels:
                        ...
                        if np.fabs(x_0 - label_x) <= label_threshold:
                            if label_x > x_0:
                                x_new = x_0 - label_threshold / 2.0
                                label_x += label_threshold / 2.0
                            else:
                                x_new = x_0 + label_threshold / 2.0
                                label_x -= label_threshold / 2.0
                            label.set_position((x_new, y_0))

                    text = ax.text(label_x, y1, ('%i' % j), fontsize=10, color=color, ha='center',
                      va=va,
                      transform=(ax.transAxes),
                      clip_on=True)
                    labels.append(text)
            ...
            band_x = (min(band_l0) + max(band_l0)) / 2.0
            if short_labels:
                band_str = 'L%i' if 'B' in band else 'W%i'
                band_str = band_str % nu
            else:
                band_str = band
        ax.text(band_x, ((y2 + 0.02) * (ymax - ymin) + ymin), band_str, color=color, clip_on=True,
          ha='center')

    plt.draw()


def plot_H2(dataset, n_rows=None, xmin=None, xmax=None, ymin=-0.1, ymax=2.5, short_labels=False, rebin=1, smooth=0):
    """
    ...
    molecular_lines = [line for line in list(dataset.lines.values()) if molecule in line.tag]
    profile = evaluate_profile(wl_profile, (dataset.best_fit), (dataset.redshift), molecular_lines,
      (dataset.components), kernel, sampling=3)
    if not xmin:
        xmin = min(min_wl)
    if not xmax:
        xmax = max(max_wl)
    if not n_rows:
        n_rows = round((xmax - xmin) / 120.0)
    width = 10.0
    ...
    for num in range(n_rows):
        ...
        show_H2_bands(ax, z, bands, Jmax, color='blue', short_labels=short_labels)
...
def print_metallicity(dataset, params, logNHI, err=0.1):
    ...
    for ion in sorted(dataset.components.keys()):
        ...
        metal_array = logN_tot - logNHI - (solar_abundance - 12.0)
    ...
    if individual:
        for reg_num, region in enumerate(dataset.regions):
            ...
            if dataset.best_fit and region.has_active_lines():
                p_obs = voigt.evaluate_profile(wl, (dataset.best_fit), (dataset.redshift), (list(dataset.lines.values())),
                  (dataset.components), (region.kernel),
                  kernel_nsub=nsub)
            else:
...
def save_individual_components(dataset, filename, path=''):
    ...
    if path:
        ...
        if dataset.best_fit:
            ...
            for ion, lines in list(lines_per_ion.items()):
                n_comp = len(dataset.components[ion])
                for n in range(n_comp):
                    ...
                    for chunk in dataset.data:
                        ...
                        for line in lines:
                            ...
                            tau += voigt.Voigt(profile_wl, l0, f, (10 ** logN), (100000.0 * b), gam, z=z)
