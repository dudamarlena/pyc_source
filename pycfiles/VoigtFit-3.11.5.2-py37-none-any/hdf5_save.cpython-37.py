...
# [GCC 8.4.0]
# Embedded file name: /Users/krogager/Projects/VoigtFit/build/lib/VoigtFit/hdf5_save.py
...
def save_hdf_dataset(ds, fname, verbose=True):
    ...
    with h5py.File(fname, 'w') as (hdf):
        ...
        for ion, ds_comps in list(ds.components.items()):
            ...
            for cnum, comp in enumerate(ds_comps):
                ...
                for key, val in list(comp.options.items()):
...
def load_dataset_from_hdf(fname):
    ...
    with h5py.File(fname, 'r') as (hdf):
        ...
        for chunk in list(data.values()):
            ...
            if 'nsub' in list(chunk.attrs.keys()):
            ...
            if 'mask' in list(chunk.keys()):
                mask = np.array(chunk['mask'])
            else:
                mask = np.ones_like(wl, dtype=bool)
            ds.add_data(wl, flux, res, err=error,
              normalized=norm,
              nsub=nsub,
              mask=mask)
        ...
        for reg in list(hdf_regions.values()):
            ...
            v = reg.attrs['velspan']
            if 'specID' in list(reg.attrs.keys()):
            ...
            if 'kernel_fwhm' in list(reg.attrs.keys()):
        ...
        if 'fine_lines' in hdf:
            ...
            if len(fine_lines) > 0:
                for ground_state, line_tags in list(fine_lines.items()):
            ...
            if len(comps) > 0:
                for n, comp in enumerate(comps.values()):
                    ...
                    if 'best_fit' in hdf:
                        ...
                        ds.best_fit.add(b_name, value=b, vary=(opts['var_b']), min=0.0)
        ...
        if 'best_fit' in hdf:
            ...
            for parname, cheb_par in list(cheb_group.items()):
