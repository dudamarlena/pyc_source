...
# [GCC 8.4.0]
# Embedded file name: /Users/krogager/Projects/VoigtFit/build/lib/VoigtFit/regions.py
# Compiled at: 2020-03-29 15:55:40
# Size of source mod 2**32: 20417 bytes
...
class Region:
    ...
    def normalize(self, plot=True, norm_method='linear', z_sys=None):
        ...
        plt.plot(x, (self.flux), color='k', drawstyle='steps-mid', label=lines_title_string)
        plt.xlabel(x_label)
        if norm_method == 'linear':
            ...
            if norm_method == 'spline':
                ...
                xk = points[:, 0]
                yk = points[:, 1]
                cont_spline = spline(xk, yk, s=0.0)
                ...
                plt.cla()
                plt.plot(x, new_flux, color='k', drawstyle='steps-mid', label=lines_title_string)
                plt.xlabel(x_label)
                plt.title('Normalized')
                plt.axhline(1.0, ls='--', color='k')
                plt.axhline((1.0 + e_continuum / np.mean(continuum)), ls=':', color='gray')
                plt.axhline((1.0 - e_continuum / np.mean(continuum)), ls=':', color='gray')
                plt.draw()
    ...
    def define_mask(self, z=None, dataset=None, telluric=True, z_sys=None):
        ...
        x_label = 'Wavelength  [Å]'
        if z_sys is not None:
            l_ref = self.lines[0].l0 * (z_sys + 1.0)
        ...
        plt.plot(x, (self.flux), color='k', drawstyle='steps-mid', lw=0.5, label=lines_title)
        plt.xlabel(x_label)
        mask_line = plt.plot(x, masked_spectrum, color='r', lw=1.5, drawstyle='steps-mid',
          zorder=0)
        plt.legend()
        if telluric:
            ...
            if len(flux_T) > 0:
                plt.plot(x_T, ((flux_T / flux_T.max() + 1.2) * cont), color='orange',
                  alpha=0.7,
                  lw=0.5)
        if z is not None:
            for line in self.lines:
                ...
                if dataset is not None:
                    ...
                    for n in range(n_comp):
                        ...
                        if z_sys is not None:
                            plt.axvline(((l0 * (z + 1) - l_ref) / l_ref * 299792.458), ls=':',
                              color='r',
                              lw=0.4)
                        else:
                            plt.axvline((l0 * (z + 1)), ls=':', color='r', lw=0.4)

                elif z_sys is not None:
                    plt.axvline(((l0 * (z + 1) - l_ref) / l_ref * 299792.458), ls=':',
                      color='r',
                      lw=0.4)
                else:
    ...
    def unpack(self):
        ...
        return (
         self.wl, self.flux, self.err, self.mask)
    ...
    def generate_label(self, active_only=True, ignore_finelines=True):
        ...
        if active_only and not ignore_finelines:
            ...
            if active_only and ignore_finelines:
                for line in self.lines:
