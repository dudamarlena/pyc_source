...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\my_xml.py
# Compiled at: 2011-01-17 14:02:03
"""
...
class MyXml(object):
    ...
    def __init__(self, name, attr=None):
        ...
        assert attr is None or isinstance(attr, dict), 'attr must be a dictionary (or missing)'
        self._name = name
        self._attr = attr
        return
    ...
    def __getattr__(self, name):
        ...
        if self._attr is not None and name.startswith('at_'):
...
class MyNode(MyValue, MyXml):
    ...
    def __repr__(self):
        ...
        if self._attr:
            ret += ' ' + (' ').join('%s="%s"' % (key, val) for (key, val) in self._attr.items())
        if self:
...
class MyNodesList(list, MyXml):
    ...
    def add(self, node):
        ...
        assert node.node_name == self.node_name, 'NodesList accepts only nodes with same name'
        self.append(node)
        return self
...
class MyNodesDict(dict, MyXml):
    ...
    def xpath(self, path):
        ...
        if dic.node_name:
            ...
            path = path[1:]
        for itm in path:
            if not isinstance(dic, MyNodesDict):
                return EMPTY_NODE
            dic = dic.get(itm)
            if not dic:
                return dic
...
def _get1item(text):
    ...
    if contains:
        j = counter = 0
        for mtch in re.finditer('</?%s(?:\\s+[^<]*?[^/])?>' % name, text, re.S | re.I):
            if mtch.group()[1] == '/':
                counter -= 1
            else:
                counter += 1
...
def _object2xml(name, obj):
    ...
    if isinstance(obj, dict):
        ...
        if isinstance(obj, list) or isinstance(obj, tuple) or isinstance(obj, set) or isinstance(obj, GeneratorType):
