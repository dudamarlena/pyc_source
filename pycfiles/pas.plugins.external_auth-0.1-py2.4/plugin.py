...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pas/plugins/external_auth/plugin.py
# Compiled at: 2012-07-17 09:37:16
"""PAS plugin for external authentication"""
import re, random
from string import letters, digits, punctuation
from urllib import quote
from urllib2 import urlopen, ProxyHandler, ProxyBasicAuthHandler, HTTPBasicAuthHandler, HTTPPasswordMgrWithDefaultRealm as pwd_mgr
...
from Products.PluggableAuthService.interfaces.plugins import IGroupsPlugin, ILoginPasswordHostExtractionPlugin, IAuthenticationPlugin, IUserEnumerationPlugin, IGroupEnumerationPlugin, IChallengePlugin, ICredentialsResetPlugin
...
def manage_add(container, id='external_auth', title='', REQUEST=None):
    ...
    if REQUEST is not None:
        REQUEST.RESPONSE.redirect('%s/manage_workspace?manage_tabs_message=%s+successfully+added.' % (id, container.absolute_url()))
    return


tal_doc = 'TALES expression. Headers are available in context as variables, as are more common "here", "modules", and "re" (the standard re module). Are also avilable login except in login_header, and user_id in group_header.'
...
class IExternalAuthPlugin(Interface):
    __module__ = __name__
...
class ExternalAuthPlugin(BasePlugin, Folder):
    ...
    __module__ = __name__
    implements(IExternalAuthPlugin, ILoginPasswordHostExtractionPlugin, IAuthenticationPlugin, IChallengePlugin, ICredentialsResetPlugin)
    meta_type = 'External authentication'
    security = ClassSecurityInfo()
    manage_options = ({'label': 'configure', 'action': 'manage_configure'},) + (Folder.manage_options[0],) + BasePlugin.manage_options
    _properties = Folder._properties + BasePlugin._properties + ({'id': 'user_container', 'label': 'External User Folder', 'type': 'string', 'mode': 'w'}, {'id': 'group_container', 'label': "External Group Manager (works with IZODBManagerInterface for removePrincipalFromGroup and addPrincipalToGroup, a shame we don't have some IGroupContainer)", 'type': 'string', 'mode': 'w'}, {'id': 'uid_header', 'label': 'Header to take the uid from.' + tal_doc, 'type': 'string', 'mode': 'w'}, {'id': 'login_header', 'label': 'Header to take the login from.' + tal_doc, 'type': 'string', 'mode': 'w'}, {'id': 'groups_header', 'label': 'Header to take the list of groups from.' + tal_doc, 'type': 'string', 'mode': 'w'}, {'id': 'use_in_session_cache', 'label': 'Whether to cache credentials in SESSION object.', 'type': 'boolean', 'mode': 'w'}, {'id': 'log_error_message', 'label': 'Whether to log errors in tales expressions.', 'type': 'boolean', 'mode': 'w'}, {'id': 'login_script_name', 'type': 'string', 'mode': 'w'}, {'id': 'uid_script_name', 'type': 'string', 'mode': 'w'}, {'id': 'groups_script_name', 'type': 'string', 'mode': 'w'}, {'id': 'up_script_name', 'type': 'string', 'mode': 'w'}, {'id': 'redirect_to_external_auth', 'type': 'boolean', 'mode': 'w'}, {'id': 'external_login_url', 'type': 'string', 'mode': 'w'}, {'id': 'redirect_logout', 'type': 'boolean', 'mode': 'w'}, {'id': 'external_logout_url', 'type': 'string', 'mode': 'w'}, {'id': 'http_login', 'type': 'string', 'mode': 'w'}, {'id': 'http_password', 'type': 'string', 'mode': 'w'}, {'id': 'proxy_url', 'type': 'string', 'mode': 'w'}, {'id': 'proxy_login', 'type': 'string', 'mode': 'w'}, {'id': 'proxy_password', 'type': 'string', 'mode': 'w'})
    ...
    def resetCredentials(self, request, response):
        ...
        if tales:
            ...
            try:
                url = Expression(tales)(expr_context(self, {}, came_from=quote(from_)))
            except Exception, e:
                self._error_occured_in_tales('Error evaluating expression "%s" determining external auth url used to logout.' % tales, e)
            else:
                if self.redirect_logout:
                    ...
                    if self.http_login:
                        http_auth.add_password(realm=None, uri=url, user=self.http_login, passwd=self.http_password)
                    proxy_auth = ProxyBasicAuthHandler(pwd_mgr())
                    if self.proxy_url:
                        proxy_support = ProxyHandler({'http': self.proxy_url, 'https': self.proxy_url})
                        if self.proxy_login:
                            proxy_auth.add_password(realm=None, uri=self.proxy_url, user=self.proxy_login, passwd=self.proxy_password)
                    else:
    ...
    def challenge(self, request, response, **kw):
        ...
        if self.redirect_to_external_auth:
            ...
            if not (url and url.strip()) and request.AUTHENTICATED_USER.getUserName() != 'Anonymous User':
                return 0
            response.redirect(url, lock=1)
            return 1
        else:
    ...
    def get_login_url(self, request):
        ...
        url = ''
        try:
            url = Expression(tales)(expr_context(self, request.environ, came_from=quote(came_from)))
        except Exception, e:
            self._error_occured_in_tales('Error evaluating expression "%s" determining external auth url.' % tales, e)
    ...
    def extractCredentials(self, request):
        ...
        try:
            ...
            self._error_occured_in_tales('Error evaluating expression "%s" for determining login of user.' % tales, e)
            return {}
        ...
        if user_id:
            try:
                user_id = Expression(user_id)(expr_context(self, env, login=login))
            except Exception, e:
                self._error_occured_in_tales('Error evaluating expression "%s" for id of user "%s".' % (tales, login), e)
                return {}

        else:
            user_id = login
        creds = {'login': login, 'password': password, 'remote_host': remote_host, 'remote_address': remote_address, 'user_id': user_id}
        ...
        try:
            groups = Expression(tales)(expr_context(self, env, login=login, user_id=user_id))
        except Exception, e:
            self._error_occured_in_tales('Error evaluating expression "%s" for groups of user "%s".' % (tales, login), e)
            groups = ()
    ...
    def authenticateCredentials(self, credentials):
        try:
            ...
            if password != _external_auth + self.id:
                return
            ...
            if not user:
                try:
                    ...
                    log.error('Could not add user "%s" with login "%s" for the following reason:' % (user_id, login))
                    log.exception(e)
                    return
                else:
                    user = pas.getUserById(user_id)
            ...
            for (prop, (env, tales)) in credentials['user_properties'].items():
                try:
                    ...
                    for s in user.listPropertysheets():
                        ...
                        while True:
                            if user[s].hasProperty(prop):
                                ...
                                value = Expression(tales)(expr_context(self, env, groups=groups, login=login, user_id=user_id, property=prop, current=orig))
                                if orig != value:
                                    try:
                                        transaction.begin()
                                        user[s].setProperty(user, prop, value)
                                        transaction.commit()
                                        log.debug('Update property "%s" : "%s" -> "%s" of %s' % (prop, orig, value, login))
                                    except ConflictError:
                                        transaction.abort()
                                        tries += 1
                                        if tries > 2:
                                            log.error('Not updating %s for %s due to too much conflicts.' % (prop, login))
                                            break
                                        else:
                                            sleep(randint(1, 9) / 10.0)
                                    else:
                                        break
                                break
                            break
                    ...
                    if orig is none:
                        log.error('No such a property "%s" defined for user %s"' % (prop, login))
                        continue
                except Exception, e:
                    log.error('Could not retrieve or update property "%s" defined for user %s"' % (prop, login))
                    log.exception(e)
                    continue
            ...
            if gc:
                ...
                for g in groups - existing:
                    try:
                        pas_get(gc).addGroup(g, title=g, description='Automatically added by %s.' % self.id)
                    except Exception, e:
                        log.error('Could not add group "%s" for the following reason:' % g)
                        log.exception(e)
            ...
            def duck_type(obj):
                return hasattr(obj, 'removePrincipalFromGroup') and hasattr(obj, 'addPrincipalToGroup')

            old_groups = set.union(*(set(gm.getGroupsForPrincipal(user)) for (_, gm) in gms if duck_type(gm)))
                ...
                for (_, gm) in gms:
                    if g in (e['id'] for e in gm.enumerateGroups()):
                        try:
                            ...
                            log.error('Could not remove %s to group "%s" for the following reason:' % (user_id, g))
                            log.exception(e)
                        else:
                ...
                for (_, gm) in gms:
                    if g in (e['id'] for e in gm.enumerateGroups()):
                        try:
                            ...
                            log.error('Could not add %s to group "%s" for the following reason:' % (user_id, g))
                            log.exception(e)
                        else:
    ...
    def get_managed_user_properties(self):
        return tuple((dict((('name', e[6:]), ('id', e), ('use_default', self.getProperty(e.replace('uprop_', 'udef_', 1))), ('tal', self.getProperty(e)))) for e in self.propertyIds() if e.startswith('uprop_')))

    security.declareProtected(manage_users, 'manage_update_ea')

    def manage_update_ea(self, user_container, group_container, login_script_name, uid_script_name, groups_script_name, up_script_name, **form):
        fg = form.get
        self.use_in_session_cache = bool(fg('use_in_session_cache', False))
        self.log_error_message = bool(fg('log_error_message', False))
        self.redirect_to_external_auth = bool(fg('redirect_to_external_auth', False))
        ...
        up_params = ('groups=groups', 'current=current', 'property=property') + groups_params
        ...
        if add_prop.strip():
            ...
            self.manage_addProperty('udef_' + add_prop, True, 'boolean')
        return
