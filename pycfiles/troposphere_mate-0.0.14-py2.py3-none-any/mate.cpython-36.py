...
# Compiled at: 2020-02-15 20:15:44
# Size of source mod 2**32: 17707 bytes
"""
...
import importlib, warnings, troposphere
...
from .tagger import update_tags_for_template
...
def convert_to_mate_resource(troposphere_resource, troposphere_mate_template):
    ...
    :rtype: Mixin
    """
    troposphere_mate_module_name = troposphere_resource.__class__.__module__.replace('troposphere.', 'troposphere_mate.')
    troposphere_mate_module = importlib.import_module(troposphere_mate_module_name)
    troposphere_mate_aws_resource_class = getattr(troposphere_mate_module, troposphere_resource.__class__.__name__)
    kwargs = {'title':troposphere_resource.title, 
    ...
    kwargs = {k:v for k, v in kwargs.items() if v is not None if v is not None}
...
def convert_to_mate_output(troposphere_output, troposphere_mate_template):
    ...
    :rtype: Output
    """
    kwargs = {'title': troposphere_output.title}
    kwargs = {k:v for k, v in kwargs.items() if v is not None if v is not None}
    for key, value in troposphere_output.resource.items():
        kwargs[key] = value

    mtdt.initiate_default_template_metadata(troposphere_mate_template)
    DependsOn = troposphere_mate_template.metadata[mtdt.TROPOSPHERE_METADATA_FIELD_NAME][mtdt.TemplateLevelField.OUTPUTS].get(troposphere_output.title, {}).get(mtdt.TemplateLevelField.OUTPUTS_DEPENDS_ON, [])
...
def is_x_depends_on_y(res_x, res_y):
    ...
    try:
        ...
        if isinstance(res_x.DependsOn, string_types):
            ...
            if isinstance(res_x.DependsOn, (list, tuple)):
...
class Template(troposphere.Template):
    ...
    def from_dict(cls, dct):
        ...
        tropo_mate_tpl.outputs = {k:convert_to_mate_output(v, tropo_mate_tpl) for k, v in tropo_tpl.outputs.items()}
        tropo_mate_tpl.parameters = tropo_tpl.parameters
        tropo_mate_tpl.resources = {k:convert_to_mate_resource(v, tropo_mate_tpl) for k, v in tropo_tpl.resources.items()}
    ...
    def add_output(self, output, ignore_duplicate=False):
        ...
        self.metadata[mtdt.TROPOSPHERE_METADATA_FIELD_NAME][mtdt.TemplateLevelField.OUTPUTS].setdefault(output.title, {})
        self.metadata[mtdt.TROPOSPHERE_METADATA_FIELD_NAME][mtdt.TemplateLevelField.OUTPUTS][output.title][mtdt.TemplateLevelField.OUTPUTS_DEPENDS_ON] = getattr(output, mtdt.TemplateLevelField.OUTPUTS_DEPENDS_ON)
    ...
    def remove_resource(self, resource, ignore_not_exists=False, remove_dependent=True, _to_delete_resources=None, _to_delete_outputs=None, _first_time_called=False):
        """
        ...
        if not remove_dependent:
            warnings.warn('`remove_dependent=False` might leave invalid dependencies relationship. For example X depends on Y, you removed Y but forgot to remove X!')
        else:
            ...
            if not isinstance(resource, AWSObject):
                resource = self.resources[resource_logic_id]
    ...
    def iter_nested_template(self, depth_first=True, _templates=None, _first_time_called=False):
        """
        ...
        if _templates is None:
            ...
            for resource in self.resources.values():
                if resource.resource_type == cloudformation.Stack.resource_type:
                    ...
                    if depth_first:
                        template.iter_nested_template(depth_first=depth_first,
...
class Parameter(troposphere.Parameter):
    ...
    def __init__(self, title, Type=REQUIRED, Default=NOTHING, NoEcho=NOTHING, AllowedValues=NOTHING, AllowedPattern=NOTHING, MaxLength=NOTHING, MinLength=NOTHING, MaxValue=NOTHING, MinValue=NOTHING, Description=NOTHING, ConstraintDescription=NOTHING, **kwargs):
        processed_kwargs = preprocess_init_kwargs(title=title, 
         ...
         ConstraintDescription=ConstraintDescription, **kwargs)
        (super(Parameter, self).__init__)(**processed_kwargs)


class Output(troposphere.Output):

    def __init__(self, title, Value=REQUIRED, Description=NOTHING, Export=NOTHING, DependsOn=NOTHING, **kwargs):
        processed_kwargs = preprocess_init_kwargs(title=title, 
         Value=Value, 
         Description=Description, 
