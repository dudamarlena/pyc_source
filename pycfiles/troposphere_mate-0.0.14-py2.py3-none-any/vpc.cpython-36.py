...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/troposphere_mate-project/troposphere_mate/canned/vpc.py
# Compiled at: 2019-12-24 20:51:58
# Size of source mod 2**32: 13967 bytes
try:
...
class VPCTier(MultiEnvBasicConfig):
    ...
    def get_nth_public_subnet_cidr(self, nth):
        return '10.{}.{}.0/24'.format(self.VPC_CIDR_ID.get_value(), (nth - 1) * 2)

    def get_nth_private_subnet_cidr(self, nth):
        return '10.{}.{}.0/24'.format(self.VPC_CIDR_ID.get_value(), (nth - 1) * 2 + 1)

    PUBLIC_SUBNET_CIDR_LIST = Derivable()

    @PUBLIC_SUBNET_CIDR_LIST.getter
    def get_PUBLIC_SUBNET_CIDR_LIST(self):
        return [self.get_nth_public_subnet_cidr(nth) for nth in range(1, 1 + self.N_PUBLIC_SUBNET.get_value())]

    PRIVATE_SUBNET_CIDR_LIST = Derivable()

    @PRIVATE_SUBNET_CIDR_LIST.getter
    def get_PUBLIC_SUBNET_CIDR_LIST(self):
        return [self.get_nth_private_subnet_cidr(nth) for nth in range(1, 1 + self.N_PUBLIC_SUBNET.get_value())]
    ...
    def check_use_nat_gw_per_private_subnet_flag(self, value):
        if value is True:
            if self.N_PUBLIC_SUBNET.get_value() < self.N_PRIVATE_SUBNET.get_value():
                raise ValueError('If you want to use 1 independent Nat GW Per Private Subnet, then the number of public subnet has to be greater than private subnet.')
    ...
    vpc = None
    public_subnet_list = None
    private_subnet_list = None
    subnet_list = None
    igw = None
    eip_list = None
    ngw_list = None
    public_route_table = None
    sg_for_ssh_from_anywhere = None
    ...
    def do_create_template(self):
        ...
        for ind, cidr in enumerate(self.PUBLIC_SUBNET_CIDR_LIST.get_value()):
            subnet = ec2.Subnet(('PublicSubnet{}'.format(ind + 1)),
              template=template,
              CidrBlock=cidr,
              VpcId=(Ref(vpc)),
              AvailabilityZone=(AZPropertyValues.get_nth_az(ind + 1)),
              MapPublicIpOnLaunch=True,
              Tags=Tags(Name=(helper_fn_sub('{}/subnet/public%s' % (ind + 1,), param_env_name))),
              DependsOn=[
             vpc])
        ...
        for ind, cidr in enumerate(self.PRIVATE_SUBNET_CIDR_LIST.get_value()):
            subnet = ec2.Subnet(('PrivateSubnet{}'.format(ind + 1)),
              template=template,
              CidrBlock=cidr,
              VpcId=(Ref(vpc)),
              AvailabilityZone=(AZPropertyValues.get_nth_az(ind + 1)),
              MapPublicIpOnLaunch=False,
              Tags=Tags(Name=(helper_fn_sub('{}/subnet/priavte%s' % (ind + 1,), param_env_name))),
              DependsOn=[
             vpc])
        ...
        igw = ec2.InternetGateway('IGW',
          template=template)
        igw_attach_vpc = ec2.VPCGatewayAttachment('IGWAttachVPC',
          template=template,
          VpcId=(Ref(vpc)),
        ...
        for ind in range(1, 1 + self.NUMBER_OF_NAT_GW.get_value()):
            eip = ec2.EIP(('EIP{}'.format(ind)),
              template=template,
              Domain='vpc',
              DependsOn=[
             igw_attach_vpc])
            eip_list.append(eip)
            ngw = ec2.NatGateway(('NGW{}'.format(ind)),
              template=template,
              AllocationId=(GetAtt(eip, 'AllocationId')),
              SubnetId=(Ref(public_subnet_list[(ind - 1)])),
              Tags=Tags(Name=(helper_fn_sub('{}/ngw/public%s' % ind, param_env_name))),
              DependsOn=eip_list)
            ngw_list.append(ngw)

        self.eip_list = eip_list
        self.ngw_list = ngw_list
        public_route_table = ec2.RouteTable('PublicRouteTable',
          template=template,
          VpcId=(Ref(vpc)),
          Tags=Tags(Name=(helper_fn_sub('{}/public-routes', param_env_name))))
        self.public_route_table = public_route_table
        public_route_default = ec2.Route('PublicRouteDefault',
          template=template,
        ...
        for ind, subnet in enumerate(public_subnet_list):
            route_table_association = ec2.SubnetRouteTableAssociation(('PublicSubnet{}RouteTableAssociation'.format(ind + 1)),
              template=template,
              RouteTableId=(Ref(public_route_table)),
              SubnetId=(Ref(subnet)),
              DependsOn=[
             public_route_table, subnet])

        if self.USE_NAT_GW_PER_PRIVATE_SUBNET_FLAG.get_value() is True:
            for ind, subnet in enumerate(private_subnet_list):
                private_route_table = ec2.RouteTable(('PrivateRouteTable{}'.format(ind + 1)),
                  template=template,
                  VpcId=(Ref(vpc)),
                  Tags=Tags(Name=(helper_fn_sub('{}/private-routes%s' % (ind + 1,), param_env_name))),
                  DependsOn=[
                 vpc])
                private_route_default = ec2.Route(('PrivateRoute{}Default'.format(ind + 1)),
                  template=template,
                  RouteTableId=(Ref(private_route_table)),
                  DestinationCidrBlock='0.0.0.0/0',
                  NatGatewayId=(Ref(ngw_list[ind])),
                  DependsOn=[
                 private_route_table, ngw_list[ind]])
                route_table_association = ec2.SubnetRouteTableAssociation(('PrivateSubnet{}RouteTableAssociation'.format(ind + 1)),
                  template=template,
                  RouteTableId=(Ref(private_route_table)),
                  SubnetId=(Ref(subnet)),
                  DependsOn=[
                 private_route_table, subnet])

        else:
            private_route_table = ec2.RouteTable('PrivateRouteTable',
              template=template,
              VpcId=(Ref(vpc)),
              Tags=Tags(Name=(helper_fn_sub('{}/private-routes', param_env_name))),
              DependsOn=[
             vpc])
            private_route_default = ec2.Route('PrivateRouteDefault',
              template=template,
              RouteTableId=(Ref(private_route_table)),
              DestinationCidrBlock='0.0.0.0/0',
              NatGatewayId=(Ref(ngw_list[0])),
              DependsOn=[
             private_route_table, ngw_list[0]])
            for ind, subnet in enumerate(private_subnet_list):
                route_table_association = ec2.SubnetRouteTableAssociation(('PrivateSubnet{}RouteTableAssociation'.format(ind + 1)),
                  template=template,
                  RouteTableId=(Ref(private_route_table)),
        ...
        sg_for_ssh_from_anywhere = ec2.SecurityGroup('SGForSSHFromAnywhere',
          template=template,
        ...
        output_list.extend([
         ...
         Output('PublicSubnetIds',
           ...
           Value=(Join(',', [Ref(subnet) for subnet in self.public_subnet_list])),
        ...
        common_tags = dict(Name=(self.ENVIRONMENT_NAME.get_value()),
