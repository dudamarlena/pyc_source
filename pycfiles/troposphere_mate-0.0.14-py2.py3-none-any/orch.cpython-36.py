...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/troposphere_mate-project/troposphere_mate/core/orch.py
...
class Orchestration(object):
    ...
    def __init__(self):
        self._master_tier = None
        self._tiers = OrderedDict()
    ...
    def _validate_canned_klass(self, canned_klass):
        ...
        if canned_klass.rel_path is None:
            raise ValueError("You have to specify ``{}.rel_path`` class variable! It is where the template file actually stored. For example: './99-master.json'. ".format(canned_klass.__name__))
    ...
    def plan(self, workspace_dir):
        ...
        **中文文档**
        ...
        config_data_mapper = OrderedDict()
        for env_tag in env_tag_list:
            ...
            config_data_mapper[env_tag] = json.load((p.abspath),
              ignore_comments=True, verbose=False)
        ...
        for can_id_list, env_tag in pipeline:
            ...
            deploy_workspace_dir = Path(workspace_dir, '{}-{}'.format(str(deploy_execution_counter).zfill(3), env_tag))
