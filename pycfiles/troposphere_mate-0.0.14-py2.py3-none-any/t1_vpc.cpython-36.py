...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/troposphere_mate-project/troposphere_mate/orch_example/web_app/t1_vpc.py
    ...
    def do_create_template(self):
        ...
        for ind, cidr in enumerate(self.PUBLIC_SUBNET_CIDR_LIST.get_value()):
            subnet = ec2.Subnet(('PublicSubnet{}'.format(ind + 1)),
              ...
              Tags=Tags(Name=('{}/ngw/public{}'.format(self.ENVIRONMENT_NAME.get_value(), ind + 1))),
              DependsOn=[
             vpc])
        ...
        for ind, cidr in enumerate(self.PRIVATE_SUBNET_CIDR_LIST.get_value()):
            subnet = ec2.Subnet(('PrivateSubnet{}'.format(ind + 1)),
              ...
              AvailabilityZone=(AZPropertyValues.get_nth_az(ind + 1)),
              MapPublicIpOnLaunch=True,
              Tags=Tags(Name=('{}/ngw/priavte{}'.format(self.ENVIRONMENT_NAME.get_value(), ind + 1))),
              DependsOn=[
             vpc])
        ...
        for ind in range(1, 1 + self.NUMBER_OF_NAT_GW.get_value()):
            ...
            ngw = ec2.NatGateway(('NGW{}'.format(ind)),
              ...
              Tags=Tags(Name=('{}/ngw/public{}'.format(self.ENVIRONMENT_NAME.get_value(), ind))))
            ngw_list.append(ngw)
        ...
        public_route_table = ec2.RouteTable('PublicRouteTable',
          template=template,
          VpcId=(Ref(vpc)),
          Tags=Tags(Name=('{}/public-routes'.format(self.ENVIRONMENT_NAME.get_value()))))
        ...
        if self.USE_NAT_GW_PER_PRIVATE_SUBNET_FLAG.get_value() is True:
            for ind, subnet in enumerate(private_subnet_list):
                private_route_table = ec2.RouteTable(('PrivateRouteTable{}'.format(ind + 1)),
                  template=template,
                  VpcId=(Ref(vpc)),
                  Tags=Tags(Name=('{}/private-routes{}'.format(self.ENVIRONMENT_NAME.get_value(), ind + 1))))
            ...
            private_route_table = ec2.RouteTable('PrivateRouteTable',
              template=template,
              VpcId=(Ref(vpc)),
              Tags=Tags(Name=('{}/private-routes'.format(self.ENVIRONMENT_NAME.get_value()))))
        ...
        sg_for_ssh = ec2.SecurityGroup('SGForSSH',
          template=template,
        ...
        for subnet in subnet_list:
            output = Output((subnet.title), Description=('{} ID'.format(subnet.title)),
              Value=(Ref(subnet)))
...
if __name__ == '__main__':
    ...
    can = VPCTier(PROJECT_NAME='myprojet',
      STAGE='dev',
      VPC_CIDR_ID=192,
      N_PUBLIC_SUBNET=3,
      N_PRIVATE_SUBNET=3,
