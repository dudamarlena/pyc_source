...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/troposphere_mate-project/troposphere_mate/associate/lbd_permission.py
# Compiled at: 2020-02-12 21:46:34
...
from ..core.metadata import TROPOSPHERE_METADATA_FIELD_NAME, ResourceLevelField
...
class LinkerApi(LinkerApi_):
    ...
    class LbdPermissionWithTokenApiAuthorizer(Linker):
        ...
        def associate(self, lbd_permission, lbd_func, api_authorizer, rest_api, authorizer_type_is_token=True, token_authorizer_header='auth', **kwargs):
            ...
            lbd_permission.SourceArn = Sub('arn:aws:execute-api:${Region}:${AccountId}:${RestApiId}/authorizers/${AuthorizerId}', {'Region':{'Ref': 'AWS::Region'}, 
             'AccountId':{'Ref': 'AWS::AccountId'}, 
             'RestApiId':{'Ref': rest_api}, 
             'AuthorizerId':Ref(api_authorizer)})
            if authorizer_type_is_token:
                api_authorizer.Type = 'TOKEN'
                api_authorizer.IdentitySource = 'method.request.header.{}'.format(token_authorizer_header)
                api_authorizer.RestApiId = Ref(rest_api)
                api_authorizer.AuthorizerUri = Sub('arn:aws:apigateway:${Region}:lambda:path/2015-03-31/functions/${AuthorizerFunctionArn}/invocations', {'Region':{'Ref': 'AWS::Region'}, 
    ...
    class LbdPermissionForEventRuleToTriggerLambdaFunc(Linker):
        ...
        def associate(self, lbd_permission, lbd_func, event_rule, **kwargs):
            ...
            if target_id not in target_ids:
                target = events.Target(Id=target_id,
    ...
    class LbdPermissionForApiMethodTriggerLambdaFunc(Linker):
        ...
        def associate(self, lbd_permission, lbd_func, api_method, **kwargs):
            lbd_permission.FunctionName = GetAtt(lbd_func, 'Arn')
            lbd_permission.Action = 'lambda:InvokeFunction'
            lbd_permission.Principal = 'apigateway.amazonaws.com'
            try:
                lbd_permission.SourceArn = Sub('arn:aws:execute-api:${Region}:${AccountId}:${RestApiId}/*/${HttpMethod}/${ResourcePath}', {'Region':{'Ref': 'AWS::Region'}, 
                 'AccountId':{'Ref': 'AWS::AccountId'}, 
                 'RestApiId':Ref(api_method.RestApiId), 
                 'HttpMethod':api_method.HttpMethod, 
                 'ResourcePath':api_method.Metadata[TROPOSPHERE_METADATA_FIELD_NAME][ResourceLevelField.ApiResource.FULL_PATH]})
            except:
                pass
