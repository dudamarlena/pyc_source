...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/troposphere_mate-project/troposphere_mate/core/associate_linker.py
# Compiled at: 2020-02-12 21:49:02
...
def x_depends_on_y(x, y):
    ...
    if DEPENDS_ON not in x.resource:
        ...
        if x.resource[DEPENDS_ON] is None:
            x.resource[DEPENDS_ON] = [
             y_logic_id]
        else:
            if isinstance(x.resource[DEPENDS_ON], list):
                if y_logic_id not in x.resource[DEPENDS_ON]:
                    x.resource[DEPENDS_ON].append(y_logic_id)
            elif y_logic_id != x.resource[DEPENDS_ON]:
                x.resource[DEPENDS_ON] = [
...
class Linker(object):
    ...
    def __init__(self):
        ...
        self._creation_index = Linker._creation_index
...
_linker_rtype_keys = [k for k in Linker.__dict__ if k.startswith('rtype')]
...
def _get_fields_by_mro(klass, field_class, ordered=False):
    ...
    return sum((_get_fields((getattr(base, '_declared_linkers', base.__dict__)), field_class, ordered=ordered) for base in mro[:0:-1]), [])
    ...
    def associate(cls, *args, **kwargs):
        ...
        **中文文档**
        ...
        if identifier in cls._declared_linkers:
            linker_class = cls._declared_linkers[identifier]
        else:
            msg = "Association for {} has't not supported yet in troposphere_mate! Please submit your sugggestion to https://github.com/MacHu-GWU/troposphere_mate-project/issues".format(identifier)
