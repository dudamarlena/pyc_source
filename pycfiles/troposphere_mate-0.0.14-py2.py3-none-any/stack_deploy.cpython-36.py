...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/troposphere_mate-project/troposphere_mate/core/stack_deploy.py
# Compiled at: 2020-03-13 23:01:51
...
def upload_template(s3_client, template_content, bucket_name, prefix=DEFAULT_CLOUDFORMATION_TEMPLATE_UPLOAD_PREFIX):
    """
    ...
    s3_client.put_object(Body=template_content,
      Bucket=bucket_name,
      Key=s3_key)
...
def package(s3_client, template, bucket_name, prefix=DEFAULT_CLOUDFORMATION_TEMPLATE_UPLOAD_PREFIX, verbose=False, _is_master=True):
    """
    ...
    for _, resource in template.resources.items():
        if resource.resource_type == Stack.resource_type:
            try:
                ...
                msg = 'you have to call ``troposphere_mate.link_stack_template(stack, template)`` method to associate nested ``cloudformation.Stack`` with a ``Template`` then you can use the aws cli ``cloudformation package`` equivalent function ``troposphere_mate.package``.'
                raise NotImplementedError(msg)

            package(s3_client=s3_client,
              template=(resource._template),
              bucket_name=bucket_name,
              prefix=prefix,
              _is_master=False,
              verbose=verbose)
            template_url = upload_template(s3_client=s3_client,
              template_content=resource._template.to_json(indent=4, sort_keys=True),
              bucket_name=bucket_name,
              prefix=prefix)
            resource.TemplateURL = template_url
            if verbose:
                msg = "upload nested template AWS::CloudFormation::Stack '{}' to: {}".format(resource.title, template_url)
                print(msg)
...
def link_stack_template(stack, template):
    ...
    **中文文档**
    ...
    stack.Metadata[mtdt.TROPOSPHERE_METADATA_FIELD_NAME][mtdt.ResourceLevelField.CftStack.IS_NESTED_STACK] = True
    object.__setattr__(stack, '_template', template)


def deploy_stack(cf_client, stack_name, template_url, stack_tags=None, stack_parameters=None, execution_role_arn=None, include_iam=False):
    """
    ...
    **中文文档**
    ...
    """
    try:
        res = cf_client.describe_stacks(StackName=stack_name)
        if len(res['Stacks']) == 1:
    ...
    Parameters = [{'ParameterKey':key,  'ParameterValue':value} for key, value in stack_parameters.items()]
    ...
    create_or_update_stack_kwargs = dict(StackName=stack_name,
...
class StackManager(object):
    ...
    def deploy(self, template, stack_name, stack_tags=None, stack_parameters=None, execution_role_arn=None, include_iam=False):
        """
        ...
        :type include_iam: bool
        """
        template_url = package((self.s3_client), template,
          (self.cft_bucket),
          prefix=DEFAULT_CLOUDFORMATION_TEMPLATE_UPLOAD_PREFIX,
          verbose=False,
          _is_master=True)
        return deploy_stack((self.cf_client),
          stack_name,
          template_url,
    ...
    def delete(self, stack_name, retain_resources=None, execution_role_arn=None):
        """
        ...
        delete_stack_kwargs = dict(StackName=stack_name,
