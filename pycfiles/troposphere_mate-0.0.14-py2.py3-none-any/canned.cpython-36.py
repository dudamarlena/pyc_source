...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/troposphere_mate-project/troposphere_mate/core/canned.py
# Compiled at: 2019-10-08 01:09:11
...
def camelcase(name):
    return ''.join([word[0].upper() + word[1:].lower() for word in slugify(name).split('-')])
...
def helper_fn_sub(text, *params):
    ...
    """
    return Sub((text.format)(*['${' + param.title + '}' for param in params]), {param.title:Ref(param) for param in params})
...
class Canned(ConfigClass):
    ...
    def to_file(self, json_or_yml='json', overwrite=False, **kwargs):
        if self.template is None:
            raise Exception('Before dumping to template file, Canned Template has to call Canned.create_template() method first!')
        try:
            abspath = self.abspath
        except:
            raise ValueError('you have to specify `Canned.root_dir` and `Canned.rel_path` first to derive the template path.')

        if os.path.exists(abspath):
            if overwrite is False:
                raise EnvironmentError('%s already exists! You can use .to_file(..., overwrite=True) to enable overwrite.')
...
class MultiEnvBasicConfig(Canned):
    ...
    def get_COMMON_TAGS(self):
        return dict(Name=(self.ENVIRONMENT_NAME.get_value()),
    ...
    def post_create_template_hooker(self, **kwargs):
        ...
        """
        self.template.update_tags((self.COMMON_TAGS.get_value()),
          overwrite=False)


class ServerlessConfig(MultiEnvBasicConfig):
