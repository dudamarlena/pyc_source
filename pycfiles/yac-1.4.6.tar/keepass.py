...
# [GCC 8.4.0]
# Embedded file name: /Users/x0ox/Dropbox/ActiveDev/yac/yac/lib/keepass.py
# Compiled at: 2018-01-05 13:51:05
...
class KeepassLoader:
    ...
    def __init__(self, vault_path='', vault_pwd=''):
        ...
        try:
            ...
            if not vault_path and not vault_pwd:
    ...
    def load_secrets(self, secrets, service_params):
        for secret in secrets:
            if 'source' in secrets[secret] and 'keepass' in secrets[secret]['source'] and 'lookup' not in secrets[secret]:
                secret_value = self.find_secret_value(secrets[secret]['value']['group'], secrets[secret]['value']['title'], secrets[secret]['value']['field'])
                if secret_value:
            ...
            elif 'source' in secrets[secret] and 'keepass' in secrets[secret]['source'] and 'lookup' in secrets[secret]:
                service_params.update({secret: secrets[secret]})
                for setpoint in secrets[secret]['value']:
                    secret_value = self.find_secret_value(secrets[secret]['value'][setpoint]['group'], secrets[secret]['value'][setpoint]['title'], secrets[secret]['value'][setpoint]['field'])
                    if secret_value:
    ...
    def load_cached_vault_path(self):
        ...
        if not vault_path:
            vault_path = path_wizard('KeePass Vault Path', 'Path to the KeePass vault file for secrets lookup', [], True)
    ...
    def load_cached_vault_pwd(self):
        ...
        if not vault_pwd:
            vault_pwd = string_wizard('KeePass Vault Password', 'The master key for the KeePass vault', [], True)
