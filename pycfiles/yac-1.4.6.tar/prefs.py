...
# [GCC 8.4.0]
# Embedded file name: /Users/x0ox/Dropbox/ActiveDev/yac/yac/cli/prefs.py
...
def main():
    ...
    parser.add_argument('--current', help='show the preferences currently configured', action='store_true')
    parser.add_argument('--show', help='show a set of vpc preferences from the registry (arg is preferences name in registry)')
    parser.add_argument('--set', help='set the vpc preferences to use when building stacks. ' + '(arg is prefs name for registry lookup or a path to vpc preferences file)')
    parser.add_argument('--list', help='list keys of all vpc preferences in the registry', action='store_true')
    parser.add_argument('--share', help='publish vpc preferences to the registry so they can be easily shared.' + ' arg is path to file containing the preferences', type=lambda x: is_valid_file(parser, x))
