...
# [GCC 8.4.0]
# Embedded file name: /Users/x0ox/Dropbox/ActiveDev/yac/yac/lib/container/start.py
# Compiled at: 2017-11-16 20:28:41
import sys, docker.utils, os, json, subprocess, urlparse
...
def start(image_tag, envs, alias, volumes_bindings, volume_mapping, volumes_from, port_bindings, connection_str, start_cmd, files_to_load, template_vars, image_source='local', create_only=False):
    ...
    if alias:
        ...
        if existing_container:
            ...
            docker_client.stop(container=existing_container.get('Id'), timeout=30)
    ...
    if template_vars:
        ...
        rendered_envs = envs
    if start_cmd:
        container = docker_client.create_container(image=image_tag, name=alias, environment=rendered_envs, volumes=volumes_bindings, host_config=volume_host_config, command=start_cmd, stdin_open=True, tty=True)
    else:
        container = docker_client.create_container(image=image_tag, name=alias, environment=rendered_envs, volumes=volumes_bindings, host_config=volume_host_config, stdin_open=True, tty=True)
    if container and container.get('Id') and not create_only:
        response = docker_client.start(container=container.get('Id'), port_bindings=port_bindings, volumes_from=volumnes_from_with_ids(volumes_from, connection_str))
        if response:
...
def execute(container_name, exec_cmd, connection_str):
    docker_client = get_docker_client(connection_str)
    execution = docker_client.exec_create(container=container_name, cmd=exec_cmd, stdout=True, stderr=True, tty=True)
    if execution and execution.get('Id'):
        response = docker_client.exec_start(exec_id=execution.get('Id'), tty=True)
        if response:
