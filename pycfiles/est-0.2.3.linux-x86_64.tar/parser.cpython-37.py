...
# [GCC 8.4.0]
# Embedded file name: /users/payno/.local/share/virtualenvs/est_venv/lib/python3.7/site-packages/est/pushworkflow/scheme/parser.py
# Compiled at: 2019-12-02 10:03:40
# Size of source mod 2**32: 13623 bytes
...
_scheme = namedtuple('_scheme', [
 'title', 'version', 'description', 'nodes', 'links', 'annotations'])
_node = namedtuple('_node', [
 'id', 'title', 'name', 'position', 'project_name', 'qualified_name',
 'version', 'data'])
_data = namedtuple('_data', [
 'format', 'data'])
_link = namedtuple('_link', [
 'id', 'source_node_id', 'sink_node_id', 'source_channel', 'sink_channel',
 'enabled'])
_annotation = namedtuple('_annotation', [
 'id', 'type', 'params'])
_text_params = namedtuple('_text_params', [
 'geometry', 'text', 'font'])
_arrow_params = namedtuple('_arrow_params', [
 'geometry', 'color'])
_nxNode = namedtuple('_node', [
 'id', 'class_', 'properties', 'data', 'qualified_name'])
_nxNodeProperty = namedtuple('_property', [
 'name', 'class_', 'value'])
_nxLink = namedtuple('_nxLink', [
 'port', 'relation'])
_nxRelation = namedtuple('_relation', [
...
def tuple_eval(source):
    ...
    if not all((isinstance(el, (ast.Str, ast.Num)) or isinstance(el, ast.UnaryOp) and isinstance(el.op, (ast.UAdd, ast.USub)) and isinstance(el.operand, ast.Num) for el in node.body.elts)):
...
def parse_ows_etree_v_2_0(tree):
    ...
    for annot in tree.findall('annotations/*'):
        if annot.tag == 'text':
            ...
            annotation = _annotation(id=(annot.get('id')),
              type='text',
    ...
    return _scheme(version=(scheme.get('version')),
...
def parse_moml_etree(tree):
    ...
    def convertToOrangeLinks(nx_links, nx_relations):
        ...
        for linkid, link in links.items():
            l = _link(id=(link['id']), source_node_id=(link['source_node_id']),
    ...
    for node in tree.findall('.//entity'):
        ...
        for property in node.findall('property'):
            _property = _nxNodeProperty(name=(property.get('name')), class_=(property.get('class')),
              value=(property.get('value')))
            node_properties.append(_property)

        nodes.append(_nxNode(id=node_id, class_=node_class,
...
def resolve_replaced(scheme_desc, registry):
    ...
    for desc in widgets:
        ...
        input_repl = {}
        for idesc in desc.inputs or []:
            for repl_qname in idesc.replaces or []:
                input_repl[repl_qname] = idesc.name

        output_repl = {}
        for odesc in desc.outputs:
            for repl_qname in odesc.replaces or []:
                output_repl[repl_qname] = odesc.name

        replacements_channels[desc.qualified_name] = (
         input_repl, output_repl)

    nodes = scheme_desc.nodes
    for i, node in list(enumerate(nodes)):
        if not registry.has_widget(node.qualified_name):
            if node.qualified_name in replacements:
                qname = replacements[node.qualified_name]
                desc = registry.widget(qname)
                nodes[i] = node._replace(qualified_name=(desc.qualified_name), project_name=(desc.project_name))
...
if __name__ == '__main__':
    if not (len(sys.argv) != 2 or sys.argv[1].lower().endswith('.ows')):
