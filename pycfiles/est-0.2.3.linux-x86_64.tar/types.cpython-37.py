...
# [GCC 8.4.0]
# Embedded file name: /users/payno/.local/share/virtualenvs/est_venv/lib/python3.7/site-packages/est/core/types.py
# Compiled at: 2020-03-12 11:12:46
# Size of source mod 2**32: 31324 bytes
__authors__ = [
 'H. Payno']
__license__ = 'MIT'
__date__ = '06/11/2019'
from silx.io.url import DataUrl
import est.io, copy, numpy, logging, h5py, tempfile, os, shutil
try:
    import larch
except ImportError:
...
class XASObject(object):
    ...
    def spectra(self, energy_spectra):
        ...
        if spectra is None:
            self._XASObject__spectra = []
            self._XASObject__energy = energy
        else:
            assert energy is not None
            self._XASObject__spectra.clear()
            assert isinstance(spectra, (list, tuple, numpy.ndarray))
            if isinstance(spectra, numpy.ndarray):
                assert spectra.ndim is 3
                self._XASObject__dim1 = spectra.shape[1]
                self._XASObject__dim2 = spectra.shape[2]
                for y_i_spectrum in range(spectra.shape[1]):
                    for x_i_spectrum in range(spectra.shape[2]):
                        self.addSpectrum(Spectrum(energy=energy, mu=(spectra[:, y_i_spectrum, x_i_spectrum])))

            else:
                if dim1 is None or dim2 is None:
                    raise ValueError('If you want to set spectra from a list/tuple of Spectrum you should specify the spectra dimensions')
                self._XASObject__dim1 = dim1
                self._XASObject__dim2 = dim2
    ...
    def _setSpectra(self, spectra):
        self._XASObject__spectra = spectra
    ...
    def addSpectrum(self, spectrum):
        self._XASObject__spectra.append(spectrum)

    @property
    def dim1(self):
        return self._XASObject__dim1

    def forceDim1(self, value):
        assert type(value) is int
        self._XASObject__dim1 = value

    def forceDim2(self, value):
        assert type(value) is int
        self._XASObject__dim2 = value

    @property
    def dim2(self):
        return self._XASObject__dim2

    @property
    def energy(self):
        return self._XASObject__energy

    @energy.setter
    def energy(self, energy):
        self._XASObject__energy = energy
        if len(self._XASObject__spectra) > 0:
            if len(self._XASObject__spectra[0].energy) != len(energy):
                _logger.warning('spectra and energy have incoherent dimension')

    @property
    def configuration(self):
        return self._XASObject__configuration

    @configuration.setter
    def configuration(self, configuration):
        if not configuration is None:
            assert isinstance(configuration, dict)
        self._XASObject__configuration = configuration or {}
    ...
    def to_dict(self, with_process_details=True):
        ...
        res = {'configuration':self.configuration, 
         'spectra':XASObject._spectra_volume(spectra=self.spectra, key='mu',
           dim_1=self.dim1,
           dim_2=self.dim2), 
         'energy':self.energy, 
         'dim1':self.dim1, 
        ...
        if with_process_details is True:
            ...
            res['current_processing_index'] = self._XASObject__processing_index
        return res
    ...
    def absorbed_beam(self):
        return XASObject._spectra_volume(spectra=(self.spectra), key='mu',
          dim_1=(self.dim1),
          dim_2=(self.dim2))
    ...
    def _spectra_volume(spectra, key, dim_1, dim_2, relative_to='energy'):
        ...
        if len(spectra) is 0:
            return
        if not len(spectra) == dim_1 * dim_2:
        ...
        for i_spectrum, spectrum in enumerate(spectra):
            try:
                ...
                if _has_larch:
                    if isinstance(value, larch.symboltable.Group):
    ...
    def load_frm_dict(self, ddict):
        ...
        if 'linked_h5_file' in ddict:
            ...
            self._XASObject__processing_index = ddict['current_processing_index']
        self.spectra = (energy, spectra, dim1, dim2)
    ...
    @staticmethod
    def from_file(h5_file, entry='scan1', spectra_path='data/absorbed_beam', energy_path='data/energy', configuration_path='configuration'):
        spectra_url = DataUrl(file_path=h5_file, data_path=('/'.join((entry, spectra_path))),
          scheme='silx')
        energy_url = DataUrl(file_path=h5_file, data_path=('/'.join((entry, energy_path))),
          scheme='silx')
        if configuration_path is None:
            config_url = None
        else:
            config_url = DataUrl(file_path=h5_file, data_path=('/'.join((entry, configuration_path))),
              scheme='silx')
        sp, en, conf = est.io.read_xas(spectra_url=spectra_url, channel_url=energy_url,
    ...
    def copy(self, create_h5_file=False):
        ...
        if create_h5_file is True:
            new_h5_file = os.path.join(tempfile.mkdtemp(), '_'.join((self._XASObject__entry_name, '_flow.h5')))
            if self.has_linked_file():
                if os.path.exists(self.linked_h5_file):
                    shutil.copy(src=(self.linked_h5_file), dst=new_h5_file)
            res.link_to_h5(new_h5_file)
        return res

    def __eq__(self, other):
        return isinstance(other, XASObject) and numpy.array_equal(self.energy, other.energy) and self.dim1 == other.dim1 and self.dim2 == other.dim2 and self.configuration == other.configuration and self.spectra_equal(self.spectra, other.spectra)
    ...
    def n_spectrum(self):
        ...
        if self._XASObject__spectra is None:
            return 0
        return len(self._XASObject__spectra)
    ...
    def linked_h5_file(self):
        return self._XASObject__h5_file
    ...
    def link_to_h5(self, h5_file):
        ...
        :return: 
        """
        self._XASObject__h5_file = h5_file

    def has_linked_file(self):
        return self._XASObject__h5_file is not None

    def get_next_processing_index(self):
        self._XASObject__processing_index += 1
        return self._XASObject__processing_index
    ...
    def register_processing(self, process, data):
        ...
        """
        est.io.write_xas_proc((self.linked_h5_file), entry=(self._XASObject__entry_name), processing_order=(self.get_next_processing_index()),
          process=process,
          data=data,
          overwrite=True)
    ...
    def get_process_flow(self):
        ...
        if not self.linked_h5_file:
            _logger.warning('process flow is store in the linked .h5 file. Ifno link is defined then this information is notstored')
            return {}
        recognized_process = est.io.get_xasproc((self.linked_h5_file), entry=(self._XASObject__entry_name))
        know_process = ('pymca_normalization', 'pymca_exafs', 'pymca_ft', 'pymca_k_weight',
                        'larch_autobk', 'larch_mback', 'larch_mback_norm', 'larch_pre_edge',
                        'larch_xftf')
    ...
    def copy_process_flow_to(self, h5_file_target):
        ...
        entry = self.entry
        with h5py.File(self._XASObject__h5_file, 'w') as (source_hdf):
...
class Spectrum(_Spectrum_Base):
    ...
    def __init__(self, energy=None, mu=None):
        ...
        self.energy = energy
        self.mu = mu
        self._Spectrum__chi = None
        self._Spectrum__k_values = None
        self._Spectrum__normalized_mu = None
        self._Spectrum__normalized_energy = None
        self._Spectrum__pre_edge = None
        self._Spectrum__post_edge = None
        self._Spectrum__e0 = None
        self._Spectrum__other_parameters = {}
        self.ft = {}
        self._Spectrum__key_mapper = {self._MU_KEY: self.__class__.mu, 
    ...
    @property
    def energy(self):
        return self._Spectrum__energy

    @energy.setter
    def energy(self, energy):
        if not isinstance(energy, numpy.ndarray):
            assert energy is None
        self._Spectrum__energy = energy

    @property
    def mu(self):
        return self._Spectrum__mu

    @mu.setter
    def mu(self, mu):
        if not isinstance(mu, numpy.ndarray):
            assert mu is None
        self._Spectrum__mu = mu

    @property
    def chi(self):
        return self._Spectrum__chi

    @chi.setter
    def chi(self, chi):
        self._Spectrum__chi = chi

    @property
    def k(self):
        return self._Spectrum__k_values

    @k.setter
    def k(self, k):
        self._Spectrum__k_values = k

    @property
    def normalized_mu(self):
        return self._Spectrum__normalized_mu

    @normalized_mu.setter
    def normalized_mu(self, mu):
        if not isinstance(mu, numpy.ndarray):
            assert mu is None
        self._Spectrum__normalized_mu = mu
    ...
    @norm.setter
    def norm(self, value):
        self._Spectrum__normalized_mu = value

    @property
    def normalized_energy(self):
        return self._Spectrum__normalized_energy

    @normalized_energy.setter
    def normalized_energy(self, energy):
        if not isinstance(energy, numpy.ndarray):
            assert energy is None
        self._Spectrum__normalized_energy = energy

    @property
    def pre_edge(self):
        return self._Spectrum__pre_edge

    @pre_edge.setter
    def pre_edge(self, value):
        self._Spectrum__pre_edge = value
    ...
    @property
    def post_edge(self):
        return self._Spectrum__post_edge

    @post_edge.setter
    def post_edge(self, value):
        self._Spectrum__post_edge = value

    @property
    def e0(self):
        return self._Spectrum__e0

    @e0.setter
    def e0(self, e0):
        self._Spectrum__e0 = e0

    @property
    def ft(self):
        return self._Spectrum__ft

    @ft.setter
    def ft(self, ft):
        if isinstance(ft, _FT):
            self._Spectrum__ft = ft
        else:
            self._Spectrum__ft = _FT(ddict=ft)

    @property
    def shape(self):
        _energy_len = 0
        if self._Spectrum__energy is not None:
            _energy_len = len(self._Spectrum__energy)
        _mu_len = 0
        if self._Spectrum__mu is not None:
            _mu_len = len(self._Spectrum__mu)
        return (_energy_len, _mu_len)

    def extra_keys(self):
        return self._Spectrum__other_parameters.keys()

    def __getitem__(self, key):
        """Need for pymca compatibility"""
        if key in self._Spectrum__key_mapper:
            return self._Spectrum__key_mapper[key].fget(self)
        return self._Spectrum__other_parameters[key]

    def __setitem__(self, key, value):
        """Need for pymca compatibility"""
        if not key:
            raise AssertionError
        elif key in self._Spectrum__key_mapper:
            self._Spectrum__key_mapper[key].fset(self, value)
        else:
            self._Spectrum__other_parameters[key] = value

    def __contains__(self, item):
        return item in self._Spectrum__key_mapper or item in self._Spectrum__other_parameters
    ...
    def load_frm_dict(self, ddict):
        ...
        if '_larch_grp_members' in ddict:
            if _has_larch is False:
                _logger.warning("larch is not installed but the Spectrum requires it, won't be able to load data relative to larch")
    ...
    def to_dict(self):
        ...
        res.update(self._Spectrum__other_parameters)
        return res
    ...
    def __str__(self):
        ...
        for key in self._Spectrum__other_parameters:
    ...
    def keys(self):
        keys = list(self._Spectrum__other_parameters.keys())
        keys += list(self._Spectrum__key_mapper.keys())
        return keys
...
class _FT(object):
    ...
    def __init__(self, ddict):
        self._FT__radius = None
        self._FT__intensity = None
        self._FT__imaginary = None
        self._FT__other_parameters = {}
        self._FT__key_mapper = {self._RADIUS_KEY: self.__class__.radius, 
    ...
    @property
    def radius(self):
        return self._FT__radius

    @radius.setter
    def radius(self, radius):
        self._FT__radius = radius

    @property
    def intensity(self):
        return self._FT__intensity

    @intensity.setter
    def intensity(self, intensity):
        self._FT__intensity = intensity

    @property
    def imaginary(self):
        return self._FT__imaginary

    @imaginary.setter
    def imaginary(self, imaginery):
        self._FT__imaginary = imaginery

    def __getitem__(self, key):
        """Need for pymca compatibility"""
        if key in self._FT__key_mapper:
            return self._FT__key_mapper[key].fget(self)
        return self._FT__other_parameters[key]

    def __setitem__(self, key, value):
        """Need for pymca compatibility"""
        if key in self._FT__key_mapper:
            self._FT__key_mapper[key].fset(self, value)
        else:
            self._FT__other_parameters[key] = value

    def __contains__(self, item):
        return item in self._FT__key_mapper or item in self._FT__other_parameters

    def to_dict(self):
        res = {self._RADIUS_KEY: self.radius, 
         self._INTENSITY_KEY: self.intensity, 
         self._IMAGINARY_KEY: self.imaginary}
        res.update(self._FT__other_parameters)
        return res
...
class Sample(object):
    __doc__ = 'Description of the sample. Needed for writing valid nx file'
