...
# [GCC 8.4.0]
# Embedded file name: /users/payno/.local/share/virtualenvs/est_venv/lib/python3.7/site-packages/est/pushworkflow/workflow.py
# Compiled at: 2019-12-02 10:03:40
# Size of source mod 2**32: 13071 bytes
...
def exec_process(callback_info, input_, properties):
    logger.debug('processing', str(callback_info), 'with input', input_, 'and', properties, 'as properties')
    if type(input_) is tuple:
        data_name, data = input_
    else:
        data_name = None
        data = input_
    callback_info = _callback_info(callback=(callback_info.callback), handler=(callback_info.handler),
    ...
    def _trigger_process(self, inData):
        ...
        if isinstance(inData, WorkflowException):
            ...
            if self.errorHandler is not None:
                ...
                exceptionDict = {'errorMessage':workflowException.errorMessage, 
            ...
            self.af.call(self.actorWrapper.actor.callback, inData, self.actorWrapper.actor.properties)
        else:
            self.af = AsyncFactory(self.actorWrapper.actor.callback, self.triggerDownStreamActor)
            self.af.call(self.actorWrapper.actor.callback, inData, self.actorWrapper.actor.properties)
...
class JoinUntilStopSignal(object):
    ...
    def trigger(self, inData):
        ...
        if self._can_stop:
            if self._nprocess_waited <= self._nprocess_received:
                newInData = {}
                for data in self.listInData:
                    newInData.update(data)
...
class RouterActor(object):
    ...
    def connect(self, actor, expectedValue='other'):
        if expectedValue != 'other':
            if expectedValue not in self.listPort:
