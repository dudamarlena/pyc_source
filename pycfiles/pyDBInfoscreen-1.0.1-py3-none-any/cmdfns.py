...
# Embedded file name: build/bdist.linux-i686/egg/pydbgr/processor/cmdfns.py
# Compiled at: 2013-03-16 22:39:32
__doc__ = ' Common command-parsing routines such as check command argument\ncounts, to parse a string for an integer, or check a string for an\non/off setting value.\n'
import sys, types
...
def get_an_int(errmsg, arg, msg_on_error, min_value=None, max_value=None):
    ...
    if min_value and ret_value < min_value:
        errmsg('Expecting integer value to be at least %d, got: %d.' % (min_value, ret_value))
        return
    elif max_value and ret_value > max_value:
        errmsg('Expecting integer value to be at most %d, got: %d.' % (max_value, ret_value))
        return
    return ret_value
