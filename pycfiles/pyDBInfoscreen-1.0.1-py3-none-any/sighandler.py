...
# Embedded file name: build/bdist.linux-i686/egg/pydbgr/lib/sighandler.py
# Compiled at: 2013-03-23 01:53:50
__doc__ = 'Signal handlers.'
import signal
...
class SignalManager:
    ...
    def __init__(self, dbgr, ignore_list=None, default_print=True):
        ...
        if ignore_list is None:
            ignore_list = [
             'SIGALRM', 'SIGCHLD', 'SIGURG', 'SIGIO', 'SIGCLD', 'SIGVTALRMSIGPROF', 'SIGWINCH', 'SIGPOLL', 'SIGWAITING', 'SIGLWP', 'SIGCANCEL', 'SIGTRAP', 'SIGTERM', 'SIGQUIT', 'SIGILL']
    ...
    def print_info_signal_entry(self, signame):
        ...
        if signame not in list(self.sigs.keys()):
            self.dbgr.intf[(-1)].msg(self.info_fmt % (signame, 'No', 'No', 'No', 'Yes', description))
            return
        sig_obj = self.sigs[signame]
        self.dbgr.intf[(-1)].msg(self.info_fmt % (signame, YN(sig_obj.b_stop), YN(sig_obj.print_method is not None), YN(sig_obj.print_stack), YN(sig_obj.pass_along), description))
        return
