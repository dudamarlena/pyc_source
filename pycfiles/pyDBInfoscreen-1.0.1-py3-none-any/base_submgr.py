...
# Embedded file name: build/bdist.linux-i686/egg/pydbgr/processor/command/base_submgr.py
# Compiled at: 2013-03-24 02:10:34
...
class SubcommandMgr(Mbase_cmd.DebuggerCommand):
    ...
    def _load_debugger_subcommands(self, name):
        ...
        class_prefix = string.capitalize(name)
        ...
        for module_name in mod.__modules__:
            ...
            try:
                ...
                print 'Error importing name %s module %s: %s' % (import_name, module_name, sys.exc_info()[0])
                continue

            classnames = [ classname for (classname, classvalue) in inspect.getmembers(command_mod, inspect.isclass) if 'DebuggerCommand' != classname if classname.startswith(class_prefix) ]
    ...
    def help(self, args):
        ...
        if cmd:
            ...
            if doc:
                ...
                self.proc.intf[(-1)].errmsg('Sorry - author mess up. ' + 'No help registered for subcommand: ' + subcmd_name + ', of command: ' + self.name)
        else:
            cmds = [ cmd for cmd in self.cmds.list() if re.match('^' + subcmd_name, cmd) ]
            if cmds == []:
                self.errmsg('No %s subcommands found matching /^%s/. Try "help".' % (self.name, subcmd_name))
            else:
                self.section('Subcommand(s) of "%s" matching /^%s/:' % (self.name, subcmd_name))
    ...
    def summary_help(self, subcmd_name, subcmd):
        self.msg_nocr('%s (%d) %-11s -- ' % (self.name, subcmd.min_abbrev, subcmd_name))
