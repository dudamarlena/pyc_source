...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pydrizzle\outputimage.py
...
no = False
RESERVED_KEYS = [
 'NAXIS', 'BITPIX', 'DATE', 'IRAF-TLM', 'XTENSION', 'EXTNAME', 'EXTVER']
EXTLIST = (
 'SCI', 'WHT', 'CTX')
DTH_KEYWORDS = [
 'CD1_1', 'CD1_2', 'CD2_1', 'CD2_2', 'CRPIX1',
...
class OutputImage():
    ...
    def writeFITS(self, template, sciarr, whtarr, ctxarr=None, versions=None, extlist=EXTLIST, overwrite=yes):
        ...
        prihdu.header.update('NDRIZIM', len(self.parlist), comment='Drizzle, No. images drizzled onto output')
        self.addDrizKeywords(prihdu.header, versions)
        if scihdr:
            del scihdr['OBJECT']
            if 'CCDCHIP' in scihdr:
                scihdr.update('CCDCHIP', '-999')
            ...
            if self.wcs:
                ...
                if scihdr['ctype1'].find('SIP') > -1:
                    ...
                    for k in scihdr.items():
                        if k[0][:2] in ('A_', 'B_') or k[0][:3] in ('IDC', 'SCD') and k[0] != 'IDCTAB' or k[0][:6] in ('SCTYPE',
                                                                                                                       'SCRVAL',
                                                                                                                       'SNAXIS',
                                                                                                                       'SCRPIX'):
                            del scihdr[k[0]]
    ...
    def addDrizKeywords(self, hdr, versions):
        ...
        for pl in self.parlist:
            ...
            hdr.update(_keyprefix + 'VER', pl['driz_version'][:44], comment='Drizzle, task version')
            hdr.update(_keyprefix + 'GEOM', 'User parameters', comment='Drizzle, source of geometric information')
            hdr.update(_keyprefix + 'DATA', pl['data'][:64], comment='Drizzle, input data image')
            hdr.update(_keyprefix + 'DEXP', pl['exptime'], comment='Drizzle, input image exposure time (s)')
            hdr.update(_keyprefix + 'OUDA', pl['outdata'][:64], comment='Drizzle, output data image')
            hdr.update(_keyprefix + 'OUWE', pl['outweight'][:64], comment='Drizzle, output weighting image')
            hdr.update(_keyprefix + 'OUCO', pl['outcontext'][:64], comment='Drizzle, output context image')
            hdr.update(_keyprefix + 'MASK', _driz_mask_name[:64], comment='Drizzle, input weighting image')
            if pl['wt_scl'] == 'exptime':
                ...
                _wtscl = pl['wt_scl']
            hdr.update(_keyprefix + 'WTSC', _wtscl, comment='Drizzle, weighting factor for input image')
            hdr.update(_keyprefix + 'KERN', pl['kernel'], comment='Drizzle, form of weight distribution kernel')
            hdr.update(_keyprefix + 'PIXF', pl['pixfrac'], comment='Drizzle, linear size of drop')
            hdr.update(_keyprefix + 'COEF', pl['coeffs'][:64], comment='Drizzle, coefficients file name ')
            hdr.update(_keyprefix + 'XGIM', pl['xgeoim'][:64], comment='Drizzle, X distortion image name ')
            hdr.update(_keyprefix + 'YGIM', pl['ygeoim'][:64], comment='Drizzle, Y distortion image name ')
            hdr.update(_keyprefix + 'LAM', pl['plam'], comment='Drizzle, wavelength applied for transformation (nm)')
            hdr.update(_keyprefix + 'SCAL', pl['scale'], comment='Drizzle, scale (pixel size) of output image')
            hdr.update(_keyprefix + 'ROT', float('%0.8f' % pl['rot']), comment='Drizzle, rotation angle, degrees anticlockwise')
            hdr.update(_keyprefix + 'XSH', pl['xsh'], comment='Drizzle, X shift applied')
            hdr.update(_keyprefix + 'YSH', pl['ysh'], comment='Drizzle, Y shift applied')
            hdr.update(_keyprefix + 'SFTU', 'output', comment='Drizzle, units used for shifts')
            hdr.update(_keyprefix + 'SFTF', 'output', comment='Drizzle, frame in which shifts were applied')
            hdr.update(_keyprefix + 'EXKY', 'EXPTIME', comment='Drizzle, exposure keyword name in input image')
            hdr.update(_keyprefix + 'INUN', 'counts', comment='Drizzle, units of input image - counts or cps')
            hdr.update(_keyprefix + 'OUUN', self.units, comment='Drizzle, units of output image - counts or cps')
            hdr.update(_keyprefix + 'FVAL', pl['fillval'], comment='Drizzle, fill value for zero weight output pix')
            OFF = 0.5
            hdr.update(_keyprefix + 'INXC', float(pl['blotnx'] // 2) + OFF, comment='Drizzle, reference center of input image (X)')
            hdr.update(_keyprefix + 'INYC', float(pl['blotny'] // 2) + OFF, comment='Drizzle, reference center of input image (Y)')
            hdr.update(_keyprefix + 'OUXC', float(pl['outnx'] // 2) + OFF, comment='Drizzle, reference center of output image (X)')
            hdr.update(_keyprefix + 'OUYC', float(pl['outny'] // 2) + OFF, comment='Drizzle, reference center of output image (Y)')
