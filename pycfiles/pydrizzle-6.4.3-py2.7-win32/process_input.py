...
def process_input(input, output=None, ivmlist=None, updatewcs=True, prodonly=False, shiftfile=None):
    ivmlist = None
    oldasndict = None
    if isinstance(input, list) == False and ('_asn' in input or '_asc' in input):
    ...
    elif isinstance(input, list) == False and input[0] == '@':
        f = open(input[1:])
        line = f.readline()
        f.close()
...
def waiver2mef(sciname, newname=None, convert_dq=True):
    ...
    def convert(file):
        ...
        try:
            newimage = fileutil.openImage(file, writefits=True, fitsname=newfilename, clobber=True)
            del newimage
            return newfilename
        except IOError:
...
def checkNGOODPIX(filelist):
    ...
    for inputfile in filelist:
        if fileutil.getKeyword(inputfile, 'instrume') == 'ACS' or fileutil.getKeyword(inputfile, 'instrume') == 'STIS':
...
def buildEmptyDRZ(input, output):
    ...
    fitsobj[0].header['FILENAME'] = str(output)
    _drzsuffix = 'drz'
    if 'drc' in output:
        _drzsuffix = 'drc'
