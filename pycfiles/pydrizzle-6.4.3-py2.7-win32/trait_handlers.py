...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pydrizzle\traits102\trait_handlers.py
...
from trait_base import strx, SequenceTypes, Undefined, NumericFuncs, StringTypes, CoercableFuncs, trait_editors, class_of
from trait_errors import TraitError
from trait_delegates import TraitEvent
Trait = None

class TraitHandler:
    __traits_metadata__ = {'type': 'trait'}

    def validate(self, object, name, value):
        raise TraitError, "The '%s' trait of %s instance has an unknown type. Contact the developer to correct the problem." % (
         name, class_of(object))

    def setattr(self, object, name, value, default):
        return object._set_trait_value(object, name, self.validate(object, name, value), default)
...
class TraitRange(TraitHandler):
    ...
    def validate(self, object, name, value):
        try:
            ...
            if (self.low is None or self.low <= cvalue) and (self.high is None or self.high >= cvalue):
                return cvalue
        except:
            pass
    ...
    def get_editor(self, trait):
        ...
        if auto_set is None:
            auto_set = True
        return trait_editors().TraitEditorRange(self, cols=trait.cols or 3, auto_set=auto_set)
...
class TraitString(TraitHandler):
    ...
    def __init__(self, dic={}, **keywords):
        ...
        self.maxlen = max(self.minlen, dic.get('maxlen', 0) or keywords.get('maxlen', sys.maxint))
    ...
    def validate(self, object, name, value):
        try:
            value = strx(value)
            if self.minlen <= len(value) <= self.maxlen and self.match(value) is not None:
                return value
        except:
            pass
...
class TraitMapReverse(TraitHandler):
    ...
    def setattr(self, object, name, value, default):
        ...
        if object.__dict__.get(name[:-1], Undefined) not in self.map[value]:
            object._set_trait_value(object, name[:-1], self.map[value][0], Undefined)
        return value
...
class TraitComplex(TraitHandler):
    ...
    def mapped_info(self):
        return (' or ').join([ x.info() for x in self.handlers + self.map_handlers
                             ])
    ...
    def get_editor(self, trait):
        ...
        if self.is_mapped():
            the_editors.extend([ x.get_editor(trait) for x in self.map_handlers
                               ])
...
class TraitReadOnly(TraitHandler):
    ...
    def validate(self, object, name, value):
        ...
        if no_key:
            return value
        if value is not Undefined and (no_key or getattr(object, name) is Undefined):
            return value
...
class TraitList(TraitHandler):
    ...
    def __init__(self, trait=None, min_items=None, max_items=None, items=True):
        if trait is None:
            trait = AnyValue
    ...
    def validate(self, object, name, value):
        if isinstance(value, list) and self.min_items <= len(value) <= self.max_items:
    ...
    def info(self):
        ...
        elif self.max_items == sys.maxint:
...
class TraitListObject(list):
    ...
    def __setitem__(self, key, value):
        try:
            list.__setitem__(self, key, self.trait.item_trait.setter.validate(self.object, self.name, value))
    ...
    def __setslice__(self, i, j, values):
        ...
        if self.trait.min_items <= len(self) + delta <= self.trait.max_items:
            try:
                ...
                list.__setslice__(self, i, j, [ validate(object, name, value) for value in values ])
                if self.name_items is not None:
                    setattr(self.object, self.name_items, 0)
                return
            except TraitError as excp:
    ...
    def append(self, value):
        if self.trait.min_items <= len(self) + 1 <= self.trait.max_items:
            try:
                list.append(self, self.trait.item_trait.setter.validate(self.object, self.name, value))
                if self.name_items is not None:
                    setattr(self.object, self.name_items, 0)
                return
            except TraitError as excp:
    ...
    def extend(self, xlist):
        if self.trait.min_items <= len(self) + len(xlist) <= self.trait.max_items:
            object = self.object
            name = self.name
            validate = self.trait.item_trait.setter.validate
            try:
                list.extend(self, [ validate(object, name, value) for value in xlist
                                  ])
                if self.name_items is not None:
                    setattr(self.object, self.name_items, 0)
                return
    ...
    def len_error(self, len):
        raise TraitError, "The '%s' trait of %s instance must be %s, but you attempted to change its length to %d element%s." % (
