...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/rdf/parser.py
...
can plugin to rdf. If you are wanting to invoke a parser you likely
want to do so through the Graph class parse method.

"""
import os, __builtin__, warnings
...
from rdf.term import URIRef, Namespace
...
headers = {'Accept': 'application/rdf+xml,text/rdf+n3;q=0.9,application/xhtml+xml;q=0.5', 
   'User-agent': 'rdf-%s (http://rdflib.net/; eikeon@eikeon.com)' % __version__}
...
def create_input_source(source=None, publicID=None, location=None, file=None, data=None):
    """
    ...
    if source is not None:
        if isinstance(source, InputSource):
            input_source = source
        elif isinstance(source, _StringTypes):
            location = source
