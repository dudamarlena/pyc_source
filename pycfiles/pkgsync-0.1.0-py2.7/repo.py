...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pkgsync/repo.py
# Compiled at: 2013-03-02 17:41:30
import os, sys, re, hashlib, urllib2, urlparse
...
class Repository(object):
    ...
    def _parse_basename(self, basename, package_name):
        if basename.endswith('.egg'):
            parser = re.compile('^(?P<package_name>%s)-(?P<version>.*)(-)(?P<pyversion>py[\\d\\.]+)(?P<extension>\\.egg)$' % package_name)
        else:
            parser = re.compile('^(?P<package_name>%s)-(?P<version>.*)(?P<extension>\\.zip|\\.tgz|\\.tar\\.gz|\\.tar\\.bz2)$' % package_name)
        r = parser.search(basename)
        if r:
            return r.groupdict()

    def _md5_anchor(self, link):
        r = re.compile('\\#md5=(?P<md5_digest>[a-z0-9]{32})')
        ...
        for link in dom.getElementsByTagName('a'):
            ...
            if parsed and parsed['package_name'] == package_name:
                if not versions or parsed['version'] in versions:
                    yield DistributionLink(url=urlparse.urljoin(self._package_index(package_name), link.attributes['href'].value), md5_digest=self._md5_anchor(link.attributes['href'].value), version=parsed['version'], basename=basename)
