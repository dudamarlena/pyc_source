...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pkgsync/sync.py
...
class Sync(object):
    ...
    def _excluded(self, package_name):
        return package_name in self.exclude.keys() and not self.exclude[package_name]
    ...
    def _package_list(self):
        if self.include:
            ...
            return self.include
        return dict([ (p, []) for p in self.source.package_names() if not self._excluded(p)
                    ])
