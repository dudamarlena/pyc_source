...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/tests/idl.py
# Compiled at: 2012-03-25 15:33:05
import pywidl

def emitSimpleType(typedef):
    if typedef.type == pywidl.SimpleType.DOMSTRING:
        return 'DOMString'
    if typedef.type == pywidl.SimpleType.OBJECT:
        return 'object'
    if typedef.type == pywidl.SimpleType.DATE:
        return 'Date'
    if typedef.type == pywidl.SimpleType.BOOLEAN:
        return 'boolean'
    if typedef.type == pywidl.SimpleType.BYTE:
        return 'byte'
    if typedef.type == pywidl.SimpleType.OCTET:
        return 'octet'
    if typedef.type == pywidl.SimpleType.FLOAT:
        return 'float'
    if typedef.type == pywidl.SimpleType.DOUBLE:
        return 'double'
    if typedef.type == pywidl.SimpleType.VOID:
        return 'void'
    if typedef.type == pywidl.SimpleType.ANY:
        return 'any'
    if typedef.type == pywidl.SimpleType.SHORT:
        return 'short'
    if typedef.type == pywidl.SimpleType.UNSIGNED_SHORT:
        return 'unsigned short'
    if typedef.type == pywidl.SimpleType.LONG:
        return 'long'
    if typedef.type == pywidl.SimpleType.UNSIGNED_LONG:
        return 'unsigned long'
    if typedef.type == pywidl.SimpleType.LONG_LONG:
        return 'long long'
    if typedef.type == pywidl.SimpleType.UNSIGNED_LONG_LONG:
...
def emitArgument(argument):
    if argument.extended_attributes:
        extended_attributes = '%s ' % emitExtendedAttributes('', argument.extended_attributes)
    else:
        extended_attributes = ''
    type = emitType(argument.type)
    if argument.ellipsis:
        type = '%s...' % type
    a = '%(extended_attributes)s%(type)s %(name)s' % {'extended_attributes': extended_attributes, 
       'type': type, 
       'name': argument.name}
...
def emitExtendedAttributes(indent, attributes):
    if not attributes:
        return ''
    return '%s[%s]' % (indent, (',\n%s ' % indent).join([ emitExtendedAttribute(attribute) for attribute in attributes ]))
...
def renderAttribute(out, attribute):
    ...
    if attribute.readonly:
        specifier = 'readonly %s' % specifier
    if attribute.inherit:
        specifier = 'inherit %s' % specifier
    if attribute.stringifier:
        specifier = 'stringifier %s' % specifier
    renderExtendedAttributes(out, '  ', attribute.extended_attributes)
    print >> out, '  %(specifier)s %(type)s %(name)s;' % {'specifier': specifier, 
...
def renderExceptionField(out, field):
    ...
    print >> out, '  %(type)s %(name)s;' % {'type': emitType(field.type), 
       'name': field.name}


def renderOperation(out, operation):
    qualifiers = []
    if operation.stringifier:
        qualifiers.append('stringifier')
    if operation.static:
        qualifiers.append('static')
    if operation.getter:
        qualifiers.append('getter')
    if operation.setter:
        qualifiers.append('setter')
    if operation.creator:
        qualifiers.append('creator')
    if operation.deleter:
        qualifiers.append('deleter')
    if operation.legacycaller:
        qualifiers.append('legacycaller')
    specifier = emitType(operation.return_type)
    if operation.name:
        specifier = '%s %s' % (specifier, operation.name)
    renderExtendedAttributes(out, '  ', operation.extended_attributes)
    print >> out, '  %(qualifiers)s%(specifier)s(%(arguments)s);' % {'qualifiers': ('').join([ qualifier + ' ' for qualifier in qualifiers ]), 
       'specifier': specifier, 
...
def renderTypedef(out, typedef):
    ...
    if typedef.type_extended_attributes:
        t = '%s %s' % (
...
def render(definitions=[], source=None, output=None, template=None, template_type=None, **kwargs):
