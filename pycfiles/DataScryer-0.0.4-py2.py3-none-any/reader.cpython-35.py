...
# [GCC 8.4.0]
# Embedded file name: D:\Programmieren\dataScryer\datascryer\influxdb\reader.py
# Compiled at: 2016-06-28 05:57:38
# Size of source mod 2**32: 2564 bytes
import json, logging, datascryer
...
class InfluxDBReader:
    ...
    def request_past(host, service, performance_label, lookback, command=None):
        ...
        if 'results' not in json_object or len(json_object['results']) == 0 or 'series' not in json_object['results'][0] or len(json_object['results'][0]['series']) == 0 or 'columns' not in json_object['results'][0]['series'][0] or len(json_object['results'][0]['series'][0]['columns']) < 2:
            ...
            return
        if 'time' not in json_object['results'][0]['series'][0]['columns'] or 'value' not in json_object['results'][0]['series'][0]['columns']:
            logging.getLogger(__name__).warn('Neither time nor value is in columns: ' + query + '\n' + json_object['results'][0]['series'][0]['columns'])
        if json_object['results'][0]['series'][0]['columns'][0] == 'time' and json_object['results'][0]['series'][0]['columns'][1] == 'value':
