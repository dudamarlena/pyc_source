...
# [GCC 8.4.0]
# Embedded file name: D:\Programmieren\dataScryer\datascryer\default_methods\simple_linear_regression.py
# Compiled at: 2016-08-15 04:26:32
...
class SimpleLinearRegression(ForecastMethod):
    ...
    def calc_slr(self, lookback_data):
        ...
        if b == 0:
            ...
            return (
             lambda x: b * x + a, lambda y: (y - a) / b)

    def calc_forecast(self, options, forecast_start, forecast_range, forecast_interval, lookback_range, lookback_data):
        return self.gen_forecast_data(func=self.calc_slr(lookback_data=lookback_data)[0], forecast_start=forecast_start, forecast_range=forecast_range, forecast_interval=forecast_interval)

    def calc_intersection(self, options, forecast_start, forecast_range, forecast_interval, lookback_range, lookback_data, y):
        return self.gen_intersection(self.gen_forecast_data(func=self.calc_slr(lookback_data=lookback_data)[0], forecast_start=forecast_start, forecast_range=forecast_range, forecast_interval=forecast_interval), forecast_start=forecast_start, y=y)
...
def example_wiki():
    ...
    data_y = [52.21, 53.12, 54.48, 55.84, 57.2, 58.57, 59.93, 61.29, 63.11, 64.47, 66.28, 68.1, 69.92, 72.19, 74.46]
    ...
    try:
        ...
        plt.scatter(label='v', *zip(*data))
        plt.scatter(label='f', *zip(*forecast))
