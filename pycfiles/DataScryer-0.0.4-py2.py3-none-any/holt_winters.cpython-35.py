...
# Compiled at: 2016-09-16 02:47:02
# Size of source mod 2**32: 13553 bytes
from __future__ import division
import logging, math, time, numpy, scipy.optimize
...
class HoltWinters(ForecastMethod):
    ...
    def calc_forecast(self, options, forecast_start, forecast_range, forecast_interval, lookback_range, lookback_data):
        ...
        with open('out.txt', 'a') as (the_file):
            i = 0
            for e in trimmed_data:
                the_file.write('data value=%d %d\n' % (e, int(time.time()) + i))
                i += 100

            for e in forecast:
                the_file.write('forecast value=%d %d\n' % (e, int(time.time()) + i))
                i += 100
