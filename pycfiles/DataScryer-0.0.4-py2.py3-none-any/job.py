...
# Compiled at: 2016-09-14 05:02:56
...
class Job(threading.Thread):
    ...
    def start_calculation(self, conf):
        start = time.time()
        lookback_data = InfluxDBReader.request_past(host=self.__host, service=self.__service, performance_label=conf[LABEL], lookback=conf[LOOKBACK_RANGE])
        ...
        if 'calc_forecast' in dir(my_class):
            ...
            start = time.time()
            if forecast_data:
                InfluxDBWriter.write_forecast(data=forecast_data, host=self.__host, service=self.__service, performance_label=conf[LABEL])
                if log_peformance():
            ...
            anormaly_data = my_class.search_anomaly(options=conf[METHOD_OPTIONS], lookback_range=conf[LOOKBACK_RANGE], lookback_data=lookback_data)
            print ('ANORMAL: ', anormaly_data)
            if log_peformance():
                logging.getLogger(__name__).debug('Calculation data of %s %s %s: %s took %dms' % (
                 self.__host, self.__service, conf[LABEL], self.get_method(conf), delta_ms(start)))
