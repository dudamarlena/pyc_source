...
# [GCC 8.4.0]
# Embedded file name: D:\Programmieren\dataScryer\datascryer\mode\drawing.py
# Compiled at: 2016-08-11 03:26:46
...
def forecast_mode(time_for_calculations=10):
    if Config.data['livestatus']['protocol'] == 'tcp':
        l = livestatus.Connector(ip=(
         Config.data['livestatus']['address'], Config.data['livestatus']['port']))
    else:
        if Config.data['livestatus']['protocol'] == 'unix':
            l = livestatus.Connector(soc=Config.data['livestatus']['address'])
        else:
            sys.exit('Livestatus protocol is unkown: ' + Config.data['livestatus']['protocol'] + '. Allowed are tcp/unix')
    ...
    if Config.data['main']['daemon']:
        ...
        logging.getLogger(__name__).info('Running just once, and wait %ds to finish calculations' % time_for_calculations)
        start = time.time()
        ...
        try:
            ...
            for host, hv in host_services.items():
                ...
                for service, sv in hv.items():
                    config_names.append({'host': host, 'service': service, 'command': sv['command'], 'perf_labels': sv['perf_labels']})
