...
# [GCC 8.4.0]
# Embedded file name: D:\Programmieren\dataScryer\datascryer\mode\check.py
# Compiled at: 2016-08-10 03:37:53
...
def check_mode(args):
    ...
    lookback_data = InfluxDBReader.request_past(host=args.host, service=args.service, performance_label=args.performanceLabel, lookback=lookback_range)
    if not lookback_data:
        plugin_exit(Returncodes.Unknown, 'Could not fetch data from InfluxDB')
    result = MethodCollector.classes[method].calc_intersection(options={}, forecast_start=Job.calc_start_date(lookback_data[(len(lookback_data) - 1)][0], forecast_interval), forecast_range=forecast_range, forecast_interval=forecast_interval, lookback_range=lookback_range, lookback_data=lookback_data, y=args.value)
    casted_result = None
    try:
    ...
    if args.warn and args.warn < casted_result:
        plugin_exit(Returncodes.Warning, 'Reached in: ' + str(casted_result) + args.unit, casted_result, args.unit, args.warn, args.crit)
    else:
        if args.crit and args.crit < casted_result:
            plugin_exit(Returncodes.Critical, 'Reached in: ' + str(casted_result) + args.unit, casted_result, args.unit, args.warn, args.crit)
        else:
            plugin_exit(Returncodes.OK, 'Reached in: ' + str(casted_result) + ' ' + args.unit + ' ', casted_result, args.unit, args.warn, args.crit)
