...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\gulpy\task.py
# Compiled at: 2017-01-14 11:51:56
...
def _run_async(task_list: list) -> None:
    ...
    results = [pool.apply_async(_run, (task, start_time)) for task in task_list]
    for task in results:
        task.get()


def run_task(*tasks_to_execute: [
 str, list]) -> None:
