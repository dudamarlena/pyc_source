...
# [GCC 8.4.0]
# Embedded file name: /Users/adambogdal/git/freepacktbook/freepacktbook/freepacktbook.py
...
class FreePacktBook(object):
    ...
    def claim_free_ebook(self):
        ...
        if self.should_claim(book):
            response = self.session.post(book['claim_url'], data={'g-recaptcha-response': self.get_recaptcha_response(book['site_key'])})
    ...
    def get_book_details(self, page=None):
        ...
        return {'title': title, 
           ...
           'url': self.url, 
           'id': book_id, 
           'site_key': re.search("Packt.offers.onLoadRecaptcha\\('(.+?)'\\)", page.getText()).groups()[0]}
