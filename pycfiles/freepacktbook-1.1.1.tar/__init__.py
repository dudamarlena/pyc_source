...
# [GCC 8.4.0]
# Embedded file name: /Users/adambogdal/git/freepacktbook/freepacktbook/__init__.py
# Compiled at: 2018-06-19 10:14:38
...
def download_parser(description):
    ...
    parser.add_argument('--formats', nargs='+', metavar='FORMAT', help='ebook formats (epub, mobi, pdf)')
    parser.add_argument('--with-code-files', action='store_true', help='download code files')
    return parser
...
def claim_free_ebook():
    ...
    parser.add_argument('--pushover', action='store_true', help='send Pushover notification')
    ...
    client = FreePacktBookAnticaptcha(anticaptcha_key=environ.get('ANTICAPTCHA_KEY'), email=environ.get('PACKTPUB_EMAIL'), password=environ.get('PACKTPUB_PASSWORD'))
    book = client.claim_free_ebook()
    if args.download:
        download_ebooks(client, parser, [book])
    if args.slack:
        slack_notification = SlackNotification(environ.get('SLACK_URL'), environ.get('SLACK_CHANNEL'))
        slack_notification.notify(book)
    if args.pushover:
        pushover_notification = PushoverNotification(environ.get('PUSHOVER_USER'), environ.get('PUSHOVER_TOKEN'))
...
def download_ebooks(client=None, parser=None, books=None):
    ...
    if client is None:
        client = FreePacktBook(environ.get('PACKTPUB_EMAIL'), environ.get('PACKTPUB_PASSWORD'))
    if books is None:
