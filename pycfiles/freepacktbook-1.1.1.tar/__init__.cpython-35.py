...
# Compiled at: 2017-02-01 17:12:50
...
class FreePacktBook(object):
    ...
    def auth_required(func, *args, **kwargs):
        ...
        def decorated(self, *args, **kwargs):
            if 'SESS_live' not in self.session.cookies:
                response = self.session.post(self.url, {'email': self.email, 
    ...
    def download_file(self, url, file_path, override=False):
        ...
        if not path.exists(file_path) or override:
            ...
            chunk_size = 1024
            progress = tqdm(total=total, leave=True, unit_scale=chunk_size, unit='B', desc='Downloading %s' % (filename,))
    ...
    def get_book_details(self, page=None):
        ...
        book_id = re.search('claim/(\\d+)/', claim_url).groups()[0]
        ...
        for line in lines:
            if not line.get('nid'):
                ...
                books.append({'title': title, 
                 'book_url': self.base_url + line.find('div', {'class': 'product-thumbnail'}).a['href'], 
