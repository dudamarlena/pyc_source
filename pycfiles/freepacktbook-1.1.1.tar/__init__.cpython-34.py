...
# Compiled at: 2016-02-11 13:00:23
...
class FreePacktBook(object):
    ...
    def get_book_details(self, page=None):
        ...
        return {'title': summary.find('div', {'class': 'dotd-title'}).getText().strip(), 
    ...
    def download_book(self, book, destination_dir='.', formats=None, override=False):
        ...
        for book_format in pbar:
            url = self.download_url % {'book_id': book['id'], 
             'format': book_format}
    ...
    def my_books(self):
        ...
        for line in lines:
            if not line.get('nid'):
                continue
            books.append({'title': line.find('div', {'class': 'title'}).getText().strip(), 
...
def claim_free_ebook():
    if not all([environ.get('PACKTPUB_EMAIL'),
     environ.get('PACKTPUB_PASSWORD')]):
        raise ImproperlyConfiguredError('Env variables PACKTPUB_EMAIL and PACKTPUB_PASSWORD are required.')
