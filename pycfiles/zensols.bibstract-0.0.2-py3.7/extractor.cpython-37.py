...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/bibstract/extractor.py
...
class RegexFileParser(object):
    __doc__ = 'Finds all instances of the citation references in a file.\n\n    '
    MULTI_REF_REGEX = re.compile('\\s*,\\s*')
    ...
    def find(self, fileobj: TextIOWrapper):
        for line in fileobj.readlines():
            ...
            refs = chain.from_iterable(map(lambda r: re.split(self.MULTI_REF_REGEX, r), refs))
            self.collector.update(refs)


class Extractor(object):
    __doc__ = 'Extracts references, parses the BibTex master source file, and exports\n    matching references from the LaTex file.\n\n    '
    TEX_FILE_REGEX = re.compile('.+\\.(?:tex|sty|cls)$')
    REF_REGEX = re.compile('\\{([a-zA-Z0-9,]+?)\\}')
    ...
    def _is_tex_file(self, path: Path) -> bool:
        ...
        """
        return path.is_file() and self.TEX_FILE_REGEX.match(path.name) is not None
