...
# Embedded file name: build/bdist.linux-x86_64/egg/pyp_beagle/beagle_summary_catalogue.py
...
from beagle_utils import prepare_data_saving, BeagleDirectories, getPathForData, data_exists, ID_COLUMN_LENGTH
...
def get1DInterval(param_values, probability, levels):
    ...
    interval = list()
    for lev in levels:
        low, high = f_interp([0.5 * (1.0 - lev / 100.0), 1.0 - 0.5 * (1.0 - lev / 100.0)])
...
class BeagleSummaryCatalogue(object):
    ...
    def __init__(self, file_name=None, credible_intervals=None, config_file=None, hdu_col=None, n_proc=1):
        ...
        self.exclude_columns = [
         'probability', 'ln_likelihood', 'chi_square', 'n_data']
    ...
    def compute(self, file_list, overwrite=False):
        ...
        for hdu in self.hdu_col:
            ...
            for col_index in range(len(hdulist[hdu_name].columns)):
                ...
                if col_.name not in columnNames:
                    continue
                new_columns.append(fits.Column(name=col_.name + '_mean', format=col_.format, unit=col_.unit))
                new_columns.append(fits.Column(name=col_.name + '_median', format=col_.format, unit=col_.unit))
                for lev in self.credible_intervals:
                    new_columns.append(fits.Column(name=col_.name + '_' + ('{:.2f}').format(lev), format='2' + col_.format[(-1)], unit=col_.unit))
        ...
        if self.n_proc > 1:
            ...
            data = pool.map(self.compute_single, file_list, (
    ...
    def make_latex_table(self, param_names, IDs=None, summary_statistics='median', average_errors=True, significant_digits=1):
                                ...
                                for lev in self.credible_intervals:
                                    ...
                                    if average_errors:
                                        ...
                                        if is_log:
                                            ...
                                            sum_stat = np.log10(sum_stat)
                                        n_ = int(np.ceil(np.log10(abs(sum_stat)))) + 1
                                        print(' & $' + to_precision(sum_stat, n + n_) + '_{-' + to_precision(error_low, n) + '}^{+' + to_precision(error_up, n) + '}$', end='')
