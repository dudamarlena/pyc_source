...
# Embedded file name: build/bdist.linux-x86_64/egg/pyp_beagle/dependencies/FillBetweenStep.py
...
def fill_between_steps(ax, x, y1, y2=0, step_where='pre', **kwargs):
    ...
    **kwargs will be passed to the matplotlib fill_between() function.
    ...
    if step_where not in {'pre', 'post', 'mid'}:
        raise ValueError(("where must be one of {{'pre', 'post', 'mid'}} You passed in {wh}").format(wh=step_where))
    if np.isscalar(y1):
