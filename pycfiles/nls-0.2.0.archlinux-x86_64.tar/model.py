...
# [GCC 8.4.0]
# Embedded file name: /home/loki/.virtualenvs/nls/lib/python2.7/site-packages/nls/model.py
...
class Problem(object):
    ...
    def model(self, *args, **kwargs):
        ...
        if kwargs.get('model') in ('2d', str(Model2D)):
    ...
    def fabricateModel1D(self, *args, **kwargs):
        kwargs['dx'] = 0.1 if 'dx' not in kwargs else kwargs['dx']
        kwargs['dt'] = 0.001 if 'dt' not in kwargs else kwargs['dt']
        kwargs['t0'] = 0.0 if 't0' not in kwargs else kwargs['t0']
        kwargs['u0'] = 0.1 if 'u0' not in kwargs else kwargs['u0']
...
class AbstractModel(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.coeffs[3] = 1.0
        self.coeffs[4] = 1.0
        self.coeffs[5] = 4.0 * self.originals['tilde_g'] / self.originals['R']
        self.coeffs[10] = 0.0
        self.coeffs[11] = 1.0 / (n0 * self.originals['gamma_R'])
        self.coeffs[12] = 1.0
        self.coeffs[13] = self.originals['R'] * phi0 ** 2 / self.originals['gamma_R']
        self.coeffs[14] = 0.0
        return

    def __repr__(self):
        from pprint import pformat
        return pformat({'dt': self.dt, 'dx': self.dx, 'order': self.order, 'num_nodes': self.num_nodes, 'num_iters': self.num_iters, 'pumping': self.pumping, 'originals': self.originals}) + '\n' + str(self.coeffs)
...
class Solution(object):
    ...
    def visualize1d(self, *args, **kwargs):
        ...
        p = self.model.pumping(x)
        u = (self.solution.conj() * self.solution).real
        n = self.model.coeffs[11] * p / (self.model.coeffs[12] + self.model.coeffs[13] * u)
    ...
    def visualize2d(self, *args, **kwargs):
        ...
        def contour_plot(subplot_number, value, label, name, labels):
            levels = linspace(0.0, value.max() + 0.001, 11)
        ...
        if 'stream' in kwargs and kwargs['stream']:
            stream_plot(1, (u, angle(self.solution)), 'phase gradient', 'Condensate streams', ('x',
                                                                                               'y'))
            density_plot(2, (u, self.solution), 'density', 'Density distribution of BEC.', ('x',
                                                                                            'y'))
        else:
            ...
            helper_plot(2, u, 'density', 'Density distribution of BEC.', ('x', 'y',
                                                                          'u'))
            helper_plot(3, n, 'reservoir', 'Density distribution of reservoir.', ('x',
                                                                                  'y',
                                                                                  'n'))
    ...
    def store(self, filename=None, label=None, desc=None, date=None):
        ...
        def storeWithFileLikeObject(file_like):
            content = {'elapsed_time': self.elapsed_time, 
