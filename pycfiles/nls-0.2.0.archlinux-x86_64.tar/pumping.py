...
# [GCC 8.4.0]
# Embedded file name: /home/loki/.virtualenvs/nls/lib/python2.7/site-packages/nls/pumping.py
...
class OpMulPumping(AbstractPumping):
    ...
    def __repr__(self):
        return repr(self.lhs) + ' x ' + repr(self.rhs)
...
class GaussianRingPumping1D(OpSumPumping):
    ...
    def __init__(self, power=1.0, radius=0.0, variation=5.0):
        super(GaussianRingPumping1D, self).__init__(GaussianPumping1D(power, +radius, 0.0, variation), GaussianPumping1D(power, -radius, 0.0, variation))
...
class GaussianElipticPumping2D(AbstractPumping):
    ...
    def __call__(self, x, y, t=None):
        ...
        rp = sqrt(x ** 2 + y ** 2)
        return self.power * (exp(-(re - rp) ** 2 / (2 * self.var)) + exp(-(re + rp) ** 2 / (2 * self.var)))

    def __repr__(self):
        return '<class GaussianElipticPumping2D(AbstractPumping)>'
...
class RectangularRingPumping1D(OpSumPumping):
    ...
    def __init__(self, power=10.0, radius=10.0, width=2.0):
        super(RectangularRingPumping1D, self).__init__(RectangularPumping1D(power, +radius, width), RectangularPumping1D(power, -radius, width))