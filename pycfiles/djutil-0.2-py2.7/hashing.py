...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\djutil\hashing.py
# Compiled at: 2013-08-27 09:29:35
...
def make_upload_to(base_path, by_fk_field=None):
    ...
    def upload_to(instance, filename):
        ...
        if by_fk_field:
            ...
            salt, hash_fk = generate_hash(force_str(fk_value), salt=b'')
            fk_subdir = (b'{hash}/').format(hash=hash_fk[:6])
        return (b'{path}{fk_subdir}{hash}{extension}').format(path=base_path, fk_subdir=fk_subdir, hash=hash_instance[:10] + hash_time[:6], extension=extension)
