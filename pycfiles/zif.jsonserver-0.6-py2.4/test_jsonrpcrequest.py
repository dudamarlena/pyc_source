...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/zif/jsonserver/tests/test_jsonrpcrequest.py
...
from zif.jsonserver.jsoncomponent import JSONReader, JSONWriter
from zif.jsonserver.interfaces import IJSONReader, IJSONWriter
...
class JSONRPCTests(unittest.TestCase):
    ...
    """
    __module__ = __name__
    _testEnv = {'PATH_INFO': '/folder/item2/view/', 'QUERY_STRING': '', 'SERVER_URL': 'http://foobar.com', 'HTTP_HOST': 'foobar.com', 'CONTENT_LENGTH': '0', 'REQUEST_METHOD': 'POST', 'HTTP_AUTHORIZATION': 'Should be in accessible', 'GATEWAY_INTERFACE': 'TestFooInterface/1.0', 'HTTP_OFF_THE_WALL': "Spam 'n eggs", 'HTTP_ACCEPT_CHARSET': 'ISO-8859-1, UTF-8;q=0.66, UTF-16;q=0.33'}
    ...
    def setUp(self):
        ...
        class View(object):
            ...
            def action(self, a, kw1=None):
                return 'Parameter[type: %s; value: %s' % (type(a).__name__, `a`)

            def keyworded(self, a, kw1='spam'):
                return 'kw1: [type: %s; value: %s]' % (type(kw1).__name__, `kw1`)
    ...
    def testTraversal(self):
        ...
        self.failUnlessEqual(action(*req._args), 'Parameter[type: int; value: 1')

    def testKeyword(self):
        req = self._createRequest({}, ParamTestCall())
        req.processInputs()
        action = req.traverse(self.app)
        self.failUnlessEqual(action(*req._args, **req.form), "kw1: [type: unicode; value: u'aaa']")
    ...
    def test1_1spec2_p(self):
        ...
        req.processInputs()
        action = req.traverse(self.app)
        self.failUnlessEqual(action(*req._args, **req.form), 'Parameter[type: int; value: 1')
