...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/omr/exam.py
...
class Form():
    ...
    def _check_size(self, img):
        ...
        if not num.all(pctdiff <= self.size_tolerance):
            raise StandardError(('image size outside form tolerance {} != {}').format(img.shape, self.expected_size))
    ...
    def _overlay_ref_fit(self, img, mean, fit, off=25):
        ...
        for (x0, x1, y0, y1), (x_off, y_off), (cx, cy) in zip(self.refzone, fit, centers):
            img = plus(img, cx, cy, val=120, r=15)
            img = plus(img, cx + x_off, cy + y_off, val=0)
            img = plus(img, x0, y0, val=150)
            img = plus(img, x1, y1, val=150)
            img = plus(img, x0 + x_off, y0 + y_off, val=0)
