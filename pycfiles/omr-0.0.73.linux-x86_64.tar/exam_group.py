...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/omr/exam_group.py
# Compiled at: 2014-05-06 14:12:50
...
def write_exam_group(images, choices, outdir):
    ...
    counts[:, 0] = range(1, 1 + choices.shape[1])
    counts[:, 1] = choices[0, :]
    counts[:, 2] = score_by_question
    ...
    savetxt(join(outdir, 'questioninfo.csv'), counts, fmt='%i', delimiter=',', header=(',').join(counts_header))
...
def write_xls_images(workbook, name_images, scores, title=None, header=['Info', 'Score', 'File'], height=23, width=[47, 5, 20], scale=[0.65, 0.65]):
