# uncompyle6 version 3.7.4
# Python bytecode 3.5 (3350)
# Decompiled from: Python 3.6.9 (default, Apr 18 2020, 01:56:04) 
# [GCC 8.4.0]
# Embedded file name: /Users/dmlhllnd/dev/src/ark/ark/ext/ark_classes.py
# Compiled at: 2016-09-05 18:24:48
# Size of source mod 2**32: 676 bytes
import ark

@ark.hooks.register('page_classes')
def add_classes(classes, page):
    if page['is_single'] and 'classes' in page['record'] and isinstance(page['record']['classes'], str):
        for userclass in page['record']['classes'].split(','):
            classes.append(userclass.strip())

    return classes