...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/purge/__init__.py
# Compiled at: 2018-01-04 05:07:58
...
import json, traceback, pika, treq, yaml
...
class PurgeService(service.Service):
    ...
    def connect(self):
        ...
        d = cc.connectTCP(self.config.get('rabbit-host', 'localhost'), self.config.get('rabbit-port', 5672))
    ...
    def setup_connection(self, connection):
        ...
        self.queue_object, self.consumer_tag = yield self.channel.basic_consume(queue=queue_name, no_ack=False, exclusive=True)
            ...
            if body:
                ...
                try:
                    if domain:
                        ...
                        response = yield treq.request('PURGE', 'http://' + self.config.get('proxy-address', '127.0.0.1') + path, headers=final_headers, timeout=10)
                    else:
                        response = yield treq.request('PURGE', 'http://' + self.config.get('proxy-address', '127.0.0.1') + path, timeout=10, headers=headers)
