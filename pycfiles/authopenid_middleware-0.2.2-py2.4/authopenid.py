...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/authopenid_middleware/authopenid.py
# Compiled at: 2009-10-29 02:04:54
...
import Cookie, struct, socket, md5
...
class AuthOpenIdHandler(object):
    ...
    def __init__(self, app, app_conf, authopenid_conf):
        ...
        self.cookie_use_agent = bool(authopenid_conf.get('cookieuseagent', 'False'))
    ...
    def __call__(self, environ, start_response):
        ...
        if not environ.has_key(self.session_middleware):
            raise Exception('The session middleware %r is not present. Have you set up the session middleware?' % self.session_middleware)
        ...
        try:
            ...
            if request_ is None:
                ...
                if self.sreg_required or self.sreg_optional:
                    sreg_request = sreg.SRegRequest(required=self.sreg_required, optional=self.sreg_optional, policy_url=self.sreg_policyurl)
                ...
                if request_.shouldSendRedirect():
                    ...
                    start_response('301 Redirect', [('Content-type', 'text/html'), ('Location', redirect_url), ('Set-Cookie', cookie.output()[len('Set-Cookie: '):])])
                    return []
                else:
                    ...
                    start_response('200 OK', [
                     ('Content-Type', 'text/html'), ('Content-Length', str(len(content))), ('Set-Cookie', cookie.output()[len('Set-Cookie: '):])])
                    return [
                     content]
    ...
    def _get_referer(self, environ):
        ...
        if 'referer' not in session and 'HTTP_REFERER' in environ:
            ...
            if mark == -1:
                mark = 0
            if not referer[:mark].endswith(self.verify_url) and not referer[:mark].endswith(self.process_url):
...
def load_config(options, app_conf, prefix):
    ...
    for (key, value) in options.items():
        if merged.has_key(key):
            warnings.warn('Key %s with value %r set in the config file is being ' + 'replaced with value %r set in the application' % (key, auth_conf_options[key], value))
