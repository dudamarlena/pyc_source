...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\tikon\Clima\Estad_futuro.py
# Compiled at: 2016-08-26 12:42:31
# Size of source mod 2**32: 16029 bytes
import calendar as calendario, datetime as ft, math as mat, random as aleatorio, numpy as np
...
def generarmeteo(dic, fecha_inic, fecha_fin):
    ...
    if fecha_inic < meses[0] or fecha_fin > meses[(-1)].replace(day=(calendario.monthrange(meses[(-1)].year, meses[(-1)].month))):
        return 'Error: Faltan datos de clima mensuales para generar datos diario en el plazo especificado.'
    else:
        paráms_wgen = True
        for p in ('Prom_rad_dia_seco_mens', 'Varia_rad_dia_seco_mens', 'Prom_rad_dia_lluv_mens',
                  'Varia_rad_dia_lluv_mens', 'Prom_temp_dia_seco_mens', 'Varia_temp_dia_seco_mens',
                  'Prom_temp_dia_lluv_mens', 'Varia_temp_dia_lluv_mens', 'Tdía_mín_prom_mens',
        ...
        for p in ('Rad_prom_mens', 'Tdia_máx_prom_mens', 'Prec_total_prom_mens', 'Prom_días_prec_mens',
...
def wgenparám(parám, fecha_inic, fecha_fin):
    ...
    for mes in range(inic, fin + 1):
        if abs(prom_días_prec_mens) < 0.0001 or abs(prec_total_prom_mens) < 0.0001:
...
def simmeteogenparám(parám, lat, fecha_inic, fecha_fin):
    ...
    sdiff = abs(410.0 - 3.21 * lat - 0.35 * savg)
    ...
    fourier_prom_temp_dia_lluv_mens[0] = fourier_tdía_máx_prom_mens[0] - (1.0 - prom_prob_lluv) * tdiff
...
def generar(parám, lat, fecha_inic, fecha_fin):
    ...
    for n, f in enumerate(fechas):
        ...
        a = np.array([[0.567, 0.086, -0.002], [0.253, 0.504, -0.05], [-0.006, -0.039, 0.244]])
