...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\tikon\Matemáticas\Pruebas.py
# Compiled at: 2017-08-25 13:07:47
# Size of source mod 2**32: 3384 bytes
from warnings import warn as avisar
import matplotlib.pyplot as dib, numpy as np, pymc, scipy.stats as estad, tikon.Matemáticas.Distribuciones as Ds, tikon.Matemáticas.Incert as Inc
n_rep = 10000
dibujar = False
con_errores = []
...
for nombre, dist in sorted(dists.items()):
    if dist['tipo'] != 'cont':
        ...
        if not dist['scipy'] is None:
            if dist['pymc'] is None:
                pass
            else:
                ...
                if mín == -np.inf:
                    ...
                    dist_scipy = Inc.ajustar_dist(datos=núms, límites=(dist['límites']), cont=True, lista_dist=[
                     nombre])[0]
                    puntos_scipy = dist_scipy.rvs(size=n_rep)
                    dist_pymc = Inc.ajustar_dist(datos=núms, límites=(dist['límites']), cont=True, usar_pymc=True, nombre=nombre,
                      lista_dist=[nombre])[0]
                    if isinstance(dist_pymc, pymc.Stochastic):
                        puntos_pymc = np.array([dist_pymc.rand() for _ in range(n_rep)])
                    else:
                        if isinstance(dist_pymc, pymc.Deterministic):
                            ...
                            raise ValueError
                        scipy_de_pymc = Inc.ajustar_dist(datos=puntos_pymc, límites=(dist['límites']), cont=True, lista_dist=[
                         nombre])[0]
                        if dibujar:
