...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\tikon\Coso.py
# Compiled at: 2017-11-13 15:12:11
# Size of source mod 2**32: 88301 bytes
import copy as copiar, json, math as mat, os, random, time
from datetime import datetime as ft
from warnings import warn as avisar
import SALib.analyze.sobol as sobol, SALib.sample.saltelli as saltelli, SALib.sample.fast_sampler as fast_sampler, SALib.analyze.fast as fast, SALib.sample.morris as morris_muestra, SALib.analyze.morris as morris_anlz, SALib.sample.latin as latin, SALib.analyze.delta as delta, SALib.analyze.dgsm as dgsm, SALib.sample.ff as ff_muestra, SALib.analyze.ff as ff_anlz, numpy as np, pymc
...
class Coso(object):
    ...
    def __init__(símismo, nombre, proyecto):
        ...
        """
        símismo.receta = dict(coefs={}, estr={}, info={})
    ...
    @staticmethod
    def _estab_a_priori(dic_ecs, dic_parám, ubic_parám, rango, certidumbre, dibujar, inter=None, archivo=None, título=None):
        """
        ...
        if certidumbre > 1:
            ...
            if not 0.0 < certidumbre <= 1:
                ...
                texto_dist = Incert.rango_a_texto_dist(líms=líms, rango=rango, certidumbre=certidumbre, cont=True)
                if inter is None:
                    if 'inter' in dic_ecs:
    ...
    def _borrar_dist(cls, d, nombre):
        ...
        for ll in list(d):
            ...
            if type(v) is dict:
                ...
                if type(v) is str and ll == str(nombre):
                    d.pop(ll)
    ...
    def _renombrar_dist(cls, d, nombre_ant, nombre_nuevo):
        ...
        for ll, v in d.items():
            if type(v) is dict:
                ...
                if type(v) is str and ll == nombre_ant:
                    d[nombre_nuevo] = d[ll]
                    d.pop(ll)
...
class Simulable(Coso):
    ...
    def __init__(símismo, nombre, proyecto):
        ...
        símismo.dic_simul = {'d_predics_exps':{},  'd_l_í_valid':{},  'matrs_valid':{},  'd_l_m_valid':{},  'd_l_m_predics_v':{},  'd_obs_valid':{},  'd_obs_calib':{},  'l_m_obs_todas':[],  'd_l_í_calib':{},  'l_m_preds_todas':[],  'l_ubics_m_preds':[],  'd_calib':{},  'copia_d_calib':{}}
        símismo.predics_exps = símismo.dic_simul['d_predics_exps']
        símismo.predics = {}
    ...
    def simular(símismo, exper=None, nombre=None, paso=1, tiempo_final=None, n_rep_parám=100, n_rep_estoc=100, calibs='Todos', usar_especificadas=False, detalles=True, dibujar=True, directorio_dib=None, mostrar=True, opciones_dib=None, dib_dists=True, valid=False):
        """
        ...
        lista_calibs = símismo._filtrar_calibs(calibs=calibs, l_paráms=lista_paráms, usar_especificadas=usar_especificadas)
        Incert.trazas_a_dists(id_simul=nombre, l_d_pm=lista_paráms, l_trazas=lista_calibs, formato='valid', comunes=(calibs == 'Comunes'),
        ...
        símismo._prep_dic_simul(exper=exper, n_rep_estoc=n_rep_estoc, n_rep_paráms=n_rep_parám, paso=paso, n_pasos=(dic_argums['n_pasos']),
          detalles=detalles,
          tipo=('valid' if valid else 'simul'))
    ...
    def calibrar(símismo, nombre=None, aprioris=None, exper=None, paso=1, n_rep_estoc=10, n_iter=10000, quema=100, extraer=10, dibujar=False):
        """
        ...
        guardar_json({'parám_{}'.format(i):n for i, n in enumerate(nombres)}, os.path.join(os.path.split(__file__)[0], 'paráms.txt'))
        if aprioris is None:
            aprioris = [
             '0']
        ...
        dic_argums['paso'] = paso
        dic_argums['detalles'] = False
        dic_argums['devolver_calib'] = True
        símismo._prep_dic_simul(exper=exper, n_rep_estoc=n_rep_estoc, n_rep_paráms=1, paso=paso, n_pasos=(dic_argums['n_pasos']),
          detalles=False,
          tipo='calib')
        d_obs = símismo.dic_simul['d_obs_calib']
        símismo.ModBayes = ModBayes(función=(símismo._simul_exps), dic_argums=dic_argums,
          d_obs=d_obs,
    ...
    def guardar_calib(símismo, descrip, utilizador, contacto=''):
        ...
        símismo.receta['Calibraciones'][nb] = dict(Descripción=descrip, Fecha=ahora,
    ...
    def validar(símismo, exper, nombre=None, calibs=None, paso=1, n_rep_parám=50, n_rep_estoc=50, usar_especificadas=False, detalles=True, guardar=False, dibujar=True, mostrar=False, opciones_dib=None, dib_dists=True):
        """
        ...
        símismo.simular(nombre=nombre, exper=exper, paso=paso, n_rep_parám=n_rep_parám, n_rep_estoc=n_rep_estoc, calibs=calibs,
    ...
    def sensibilidad(símismo, nombre, exper, n, método='Sobol', calibs=None, por_dist_ingr=0.95, detalles=False, usar_especificadas=True, opciones_sens=False, dibujar=False):
        """
        ...
        lista_calibs = símismo._filtrar_calibs(calibs=calibs, l_paráms=lista_paráms, usar_especificadas=usar_especificadas)
        lista_dists = Incert.trazas_a_dists(id_simul=nombre, l_d_pm=lista_paráms, l_trazas=lista_calibs, formato='sensib',
          comunes=False,
          l_lms=lista_líms)
        lista_líms_efec = Incert.dists_a_líms(l_dists=lista_dists, por_dist_ingr=por_dist_ingr)
        n_paráms = len(lista_paráms)
        problema = {'num_vars':n_paráms, 
        ...
        if método_mín == 'sobol':
            ...
            conv_ops_anlz = {'calc_segundo_orden':'calc_second_order',  'núm_remuestreos':'num_resamples',  'nivel_conf':'conf_level', 
             'paralelo':'parallel',  'n_procesadores':'n_processors'}
            ...
            if método_mín == 'fast':
                ...
                if método_mín == 'morris':
                    conv_ops_muestrear = {'núm_niveles':'num_levels',  'salto_cuadr':'grid_jump',  'traj_optimal':'optimal_trajectories', 
                     'opt_local':'local_optimization'}
                    conv_ops_anlz = {'núm_remuestreos':'num_resamples',  'nivel_conf':'conf_level',  'salto_cuadr':'grid_jump', 
                     'núm_niveles':'num_levels'}
                    ops_muestrear = {conv_ops_muestrear[a]:val for a, val in opciones_sens.items() if a in conv_ops_muestrear}
                    ops_anlz = {conv_ops_anlz[a]:val for a, val in opciones_sens.items() if a in conv_ops_anlz}
        ...
        símismo.simular(exper=exper, nombre=nombre, calibs=nombre, detalles=detalles, dibujar=False, mostrar=False, dib_dists=False,
          n_rep_parám=n_rep_parám,
          n_rep_estoc=1,
                ...
                for índ, m in d.items():
                    ...
                    if len(m.shape) == 2:
                        for i, prm in enumerate(nombres_paráms):
                            ...
                            Arte.graficar_línea(datos=(m[i, :]), etiq_x='Día', etiq_y=('{}: {}'.format(método, índ)), título=título,
                              directorio=direc)
                        ...
                        if len(m.shape) == 3:
                            for i, prm_1 in enumerate(nombres_paráms):
                                for j, prm_2 in enumerate(nombres_paráms):
                                    ...
                                    Arte.graficar_línea(datos=(m[i, j, :]), etiq_x='Día', etiq_y=('{}: {}'.format(método, índ)), título=título,
                                      directorio=direc)

                        else:
                            raise ValueError('Número de ejes ({}) inesperado. Quejarse al programador.'.format(len(m.shape)))

        return (
         nombres_paráms, resultado)
    ...
    def _prep_dic_simul(símismo, exper, n_rep_estoc, n_rep_paráms, paso, n_pasos, detalles, tipo):
        ...
        símismo._gen_dic_predics_exps(exper=exper, n_rep_estoc=n_rep_estoc, n_rep_parám=n_rep_paráms, paso=paso,
          n_pasos=n_pasos,
          detalles=detalles)
        símismo.dic_simul['copia_d_predics_exps'] = copiar.deepcopy(símismo.dic_simul['d_predics_exps'])
        if tipo == 'valid' or tipo == 'calib':
            símismo._gen_dics_valid(exper=exper, paso=paso, n_pasos=n_pasos, n_rep_estoc=n_rep_estoc,
              n_rep_parám=n_rep_paráms)
        if tipo == 'calib':
    ...
    def _simul_exps(símismo, paso, n_pasos, extrn, detalles, devolver_calib):
        ...
        if devolver_calib:
            ...
            print('Procesando predicciones: \n\tSimul: {}\n\tValid: {}\n\tCalib: {}\n\tTotal: {}'.format(dif_p_s, dif_p_v, dif_p_c, dif_p_s + dif_p_v + dif_p_c))
    ...
    def _procesar_valid(símismo):
        ...
        for t_dist, l_matr in d_l_m_valid.items():
            for i, m_v in enumerate(l_matr):
                ...
                if len(días_v_i):
                    m_v[(..., días_v_i)] = días_p_i[1]
                    m_v[(..., días_v_i)] -= días_p_i[0]
                    m_v[(..., días_v_i)] *= días_p_i[2]
                    m_v[(..., días_v_i)] += días_p_i[0]
    ...
    def _filtrar_calibs(símismo, calibs, l_paráms, usar_especificadas):
        ...
        if type(calibs) is str:
            if calibs not in ('Todos', 'Comunes', 'Correspondientes'):
                calibs = [calibs]
        else:
        ...
        l_calibs_por_parám = []
        for d_p in l_paráms:
            if usar_especificadas:
                if 'especificado' in d_p:
                    calibs_p = [
                     'especificado']
            else:
    ...
    def dibujar_calib(símismo):
            ...
            if l is None:
                ...
                for ll, v in d.items():
                    if type(v) is dict:
                        ...
                        if isinstance(v, pymc.Stochastic) or isinstance(v, pymc.Deterministic):
                            u.append(ll)
                            l.append((u.copy(), v))
                            u.pop()
                            continue
        ...
        for ubic, dist in lista_dists:
            directorio = os.path.join(directorio_base, 'Proyectos', os.path.splitext(símismo.proyecto)[0], símismo.nombre, 'Gráficos calibración', símismo.ModBayes.id, ubic[0])
            ...
            try:
                ...
                Arte.graficar_dists(dists=[dist], valores=vals, título=título,
                  archivo=archivo)
