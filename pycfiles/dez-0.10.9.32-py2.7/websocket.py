...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dez/network/websocket.py
...
def parse_frame(buf):
    ...
    if mask:
        unmasked = [ mask_bytes[(i % 4)] ^ py23ord(b) for b, i in zip(payload, list(range(len(payload)))) ]
...
class WebSocketConnection(object):
    ...
    def _recv(self, data):
        ...
        while payload:
            try:
                self.report('Payload parsed: "%s"' % (payload,))
...
def startwebsocketproxy():
    ...
    print (
     'running WebSocket server on port', options.port)
