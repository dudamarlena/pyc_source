...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dez/http/static.py
# Compiled at: 2020-04-19 20:02:12
        ...
        if isdir:
            if not self._try_index(req, path):
                ...
                return self.__respond(req, data=[
                 '<b>%s</b><br><br>' % (url,),
                 '<a href=%s>..</a><br>' % (os.path.split(url)[0],)] + [ '<a href=%s>%s</a><br>' % (parse.quote('%s/%s' % (url, child)), child) for child in os.listdir(path)
                                                                       ])
        else:
