...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dez/http/reverseproxy.py
...
BIG_FILES = ['mp3', 'png', 'jpg', 'jpeg', 'gif', 'pdf', 'csv', 'mov',
 'zip', 'doc', 'docx', 'jar', 'data', 'db', 'xlsx', 'geojson']
...
class ReverseProxy(object):
    ...
    def _302(self, conn, domain, path):
...
def parse_options():
    ...
    parser.add_option('-o', '--override_redirect', action='store_true', dest='override_redirect', default=False, help='prevent 302 redirect of large files (necessary if incoming host matters to target)')
    parser.add_option('-c', '--cert', dest='cert', default=None, help='your ssl certificate -- if port is unspecified, uses port 443')
    parser.add_option('-k', '--key', dest='key', default=None, help='your ssl key -- if port is unspecified, uses port 443')
    parser.add_option('-s', '--ssl_redirect', dest='ssl_redirect', default=None, help="if specified, 302 redirect ALL requests to https (port 443) application at specified host ('auto' leaves host unchanged) - ignores config")
    parser.add_option('-m', '--monitor', dest='monitor', default=None, help='listen on specified port for /_report requests (default: None)')
