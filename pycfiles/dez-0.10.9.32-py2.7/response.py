...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dez/http/server/response.py
...
def renderResponse(data='', version_major=1, version_minor=0, status='200 OK', headers={}):
    if isinstance(data, list):
        data = ('').join([ isinstance(d, binary_type) and d or d.encode() for d in data if d ])
...
class HTTPResponse(object):
    ...
    def render(self):
        response = renderResponse(self.buffer, self.version_major, self.version_minor, self.status, self.headers)
        self.buffer = []
...
class WSGIResponse(object):
    ...
    def setup_environ(self, body):
        ...
        environ['REMOTE_ADDR'] = self.request.conn.addr
