...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dsutils/preprocess.py
# Compiled at: 2018-04-10 23:29:06
...
class PreprocessDataFrame(object):
    ...
    def test_stdnorm(self):
        ...
        import warnings
        warnings.filterwarnings('ignore')
        if not self._stdnorm:
            print('train_stdnorm on training data first')
        try:
            self.test.shape
    ...
    def test_labelencode(self):
        if not self._lbencode:
            print('train_labelencode on training data first')
        try:
            self.test.shape
    ...
    def Taxi_train_preprocess(self, add_tip_pct=False, rm_tip_amount=False, add_XYdistance=True, kp_all_positive=False, pos_cols=[
 'Passenger_count', 'Trip_distance', 'Fare_amount', 'Extra', 'MTA_tax', 'Tip_amount',
 'Tolls_amount', 'improvement_surcharge', 'Total_amount'], to_be_normalize_cols=[
 'lpep_pickup_datetime', 'Lpep_dropoff_datetime', 'Pickup_longitude',
 'Pickup_latitude', 'Dropoff_longitude', 'Dropoff_latitude', 'Passenger_count',
 'Trip_distance', 'Fare_amount', 'Extra', 'MTA_tax', 'Tolls_amount',
 'improvement_surcharge', 'Total_amount', 'duration_min', 'speed',
 'xydistance', 'xyPCA_pickup1', 'xyPCA_pickup2', 'xyPCA_dropoff1',
 'xyPCA_dropoff2', 'cluster_pickup', 'cluster_dropoff', 'pickup_weekday',
 'pickup_hour'], to_be_dummies_cols=[
 'VendorID', 'Store_and_fwd_flag',
 'RateCodeID', 'Trip_type', 'Payment_type']):
    ...
    def Taxi_test_preprocess(self, add_tip_pct=False, rm_tip_amount=False, add_XYdistance=True, kp_all_positive=False, pos_cols=[
 'Passenger_count', 'Trip_distance', 'Fare_amount', 'Extra', 'MTA_tax', 'Tip_amount',
 'Tolls_amount', 'improvement_surcharge', 'Total_amount'], to_be_normalize_cols=[
 'lpep_pickup_datetime', 'Lpep_dropoff_datetime', 'Pickup_longitude',
 'Pickup_latitude', 'Dropoff_longitude', 'Dropoff_latitude', 'Passenger_count',
...
def plot_cluster(train, N=None):
    ...
    ax.scatter(train['Pickup_longitude'].values[:N], train['Pickup_latitude'].values[:N], s=5, lw=0, c=train['cluster_pickup'][:N].values, cmap='tab20', alpha=0.1)
    ax.set_xlim(xlim)
    ax.set_ylim(ylim)
