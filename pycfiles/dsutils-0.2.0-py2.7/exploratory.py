...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dsutils/exploratory.py
# Compiled at: 2018-04-10 00:14:48
from __future__ import division, print_function
import pandas as pd
from time import time
import scipy, matplotlib.pyplot as plt, seaborn as sns, numpy as np, math, warnings
...
class PlotHist(object):
    ...
    def __init__(self, df, col='Trip_distance', upper_std=3):
        ...
        print(('{} of {} outliers detected {:.2f}% remain of feature {} at {} std').format(self.outlier_n, self.n, (self.n - self.outlier_n) / self.n * 100, self.col, self.upper_std))
...
def plot_coordinates(df, limit=True, xlimit=[-74.1, -73.7], ylimit=[40.55, 40.95], figsize=[
 20, 20], frame=False, include_pickup=True, include_dropoff=True, xy_eq_scale=True, JFK=[
 -73.7851, 40.6463], LGA=[
 -73.8685, 40.772], EWR=[
 -74.1815, 40.6895]):
    ...
    ax.add_patch(patches.Rectangle(JFK, 0.01, 0.01, color='g', linewidth=2, fill=False))
    if not frame:
        plt.axis('off')
    ax.grid(False)
    return ax
...
def test_ANOVA(df, value, category, cutoff=0.05):
    ...
    print()
    if p > cutoff:
        print(('average values of {} by different categories :{} are statistically the SAME on significant level {}').format(value, category, cutoff))
        return True
    else:
        print(('Average values of {} by different categories :{} are statistically DIFFERENT on significant level {}').format(value, category, cutoff))
        return False