...
# Embedded file name: build/bdist.linux-i686/egg/hjb/hjbclient.py
# Compiled at: 2006-06-04 09:52:23
...
__all__ = [
 'session_object_types', 'compose_url', 'invoke_connection', 'url_from_response', 'invoke_hjb', 'HJBError', 'SessionObjects', 'HJBClient', 'SimpleMessagingScenario', 'main']
__docformat__ = 'restructuredtext en'
_connection_url_rx = re.compile('^(.*connection-\\d+)')
log = logger_for('hjb.client')
session_object_types = [
 'consumers', 'producers', 'browsers', 'subscribers']
...
def invoke_connection(host, url, cmd='/start', connection_rx=_connection_url_rx, method='GET'):
    ...
    return invoke_hjb(host, match.groups()[0] + cmd, method)
...
def invoke_hjb(host, url, method='POST', params=None, headers=None):
    ...
    conn.request(method, _qq(url), urlencode(params), headers)
...
class SessionObjects(object):
    ...
    def __init__(self, session_object_responses):
        """
        `session_object_responses` is the map of
        *session_object_type*: [list of responses of that type]

        obtained during invocation of `HJBClient.create_session`
    ...
    def _update_url_lists(self):
        ...
        def add_url_list(object_type):
            ...
            return (object_type,
             map(url_from_response, responses.get(found_type, [])))

        responses = self._responses
        self.__dict__.update(dict((add_url_list(t) for t in session_object_types)))

    def __str__(self):
        return ('').join([ '\n' + t + ': ' + str(getattr(self, t)) for t in session_object_types if hasattr(self, t) ])
...
class HJBClient(object):
    ...
    def send_message(self, message, send_url, message_type='Text', config={}):
        ...
        response = invoke_hjb(self.hostname, send_url, params=params, headers=config.get('headers', {}))

    def get_messages(self, receive_url, config={}):
        """Obtain an iterator over the messages retrieved from the consumer at receive_url."""
        response = invoke_hjb(self.hostname, receive_url, params=config.get('receive', {}), headers=config.get('headers', {}))
    ...
    def add_new_session_objects(self, session_url, config):
        ...
        def create(object_type, object_args, created):
            ...
            return created.setdefault(object_type, []).append(invoke_hjb(self.hostname, compose_url(session_url, 'create-' + object_type[:-1]), params=object_args, headers=config.get('headers', {})))
    ...
    def create_session(self, provider, factory_name, config, connection_url=None):
        ...
        """
        session_url = self.create_session_only(provider, factory_name, config, connection_url)[0]
        log.debug('... creating session at URI %s', session_url)
        return (
         session_url, self.add_new_session_objects(session_url, config))

    def create_session_only(self, provider, factory_name, config, connection_url=None):
        """Make a new JMS session on the HJB Server.
        ...
        response = invoke_hjb(self.hostname, compose_url(connection_url, 'create'), params=config.get('session', {}), headers=config.get('headers', {}))
        return (
         response.getheader('Location'), response)

    def register_destination(self, provider, destination, config):
        """Register a JMS destination on the HJB server"""
        self._create_admin_object(provider, compose_url('destination', destination), config)

    def register_factory(self, provider, factory_name, config):
        """Register a JMS connection factory on the HJB server"""
        self._create_admin_object(provider, factory_name, config)

    def make_connection(self, provider, factory_name, config):
        """ Creates a new JMS connection on the HJB server."""
        self.register_factory(provider, factory_name, config)
        create_url = compose_url(self.root, provider, factory_name, 'create')
        log.debug('... attempting to create conection with %s', create_url)
        response = invoke_hjb(self.hostname, create_url, params=config.get('connection', {}), headers=config.get('headers', {}))
        log.debug('Created connection %s', response.getheader('Location'))
        return (
         response.getheader('Location'), response)

    def _create_admin_object(self, provider, object_name, config):
        invoke_hjb(self.hostname, compose_url(self.root, provider, 'register'), params=config.get('provider', {}), headers=config.get('headers', {}))
        return invoke_hjb(self.hostname, compose_url(self.root, provider, object_name, 'register'), method='GET', headers=config.get('headers', {}))
...
class SimpleMessagingScenario(object):
    ...
    def create_another_session(self, session_config):
        ...
        (session_url, session_objects) = self.client.create_session(self.provider, self.factory, combined_config, self._connection_url)
    ...
    def _register_destination(self, destination):
        ...
        self.client.register_destination(self.provider, destination, self.config)

    def _make_the_connection(self):
        """Create a connection using this scenario's provider, factory and configuration"""
        self._connection_url = self.client.make_connection(self.provider, self.factory, self.config)[0]

    def _register_factory(self, factory):
        """Register this scenario's connection factory"""
        self.client.register_factory(self.provider, factory, self.config)

    def _admin_url(self, name):
        return compose_url(self.client.root, self.provider, name)

    def _provider_url(self):
        return compose_url(self.client.root, self.provider)
