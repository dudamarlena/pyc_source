...
# Embedded file name: build/bdist.linux-i686/egg/hjb/hjbmessages.py
# Compiled at: 2006-06-04 06:12:03
__doc__ = '\nContains classes used to process messages sent and received from a HJB Server\n'
from cStringIO import StringIO
from hjbtypes import hjbdecode
__all__ = [
 'message_types', 'message_java_classes', 'message_version', 'message_version_header', 'message_class_header', 'HJBMessage', 'HJBMapMessage', 'HJBStreamMessage', 'HJBObjectMessage', 'HJBBytesMessage', 'MessageReader']
...
def text_as_dict(text):
    as_key_value_text = [ l.strip().split('=', 1) for l in StringIO(text).readlines() if l.strip() ]
    return dict(((k, hjbdecode(v)) for (k, v) in as_key_value_text))


class MessageReader(object):
    """I provide access to HJB Messages in a HTTP Response object"""
    __module__ = __name__
    ...
    def next_message(self):
        ...
        if message_class_header not in headers:
            return HJBMessage(('').join(payload), headers)
        else:
            return java_hjb_link[headers[message_class_header]](payload=('').join(payload), headers=headers)
...
message_hjb_classes = [
 HJBMessage, HJBMapMessage, HJBObjectMessage, HJBStreamMessage, HJBBytesMessage]
message_types = dict(((_cls.message_type, _cls) for _cls in message_hjb_classes))
