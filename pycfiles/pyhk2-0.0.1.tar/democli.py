...
# Embedded file name: build/bdist.linux-i686/egg/hjb/democli.py
# Compiled at: 2006-06-04 06:29:03
...
def get_demo_command_classes():
    ...
    return dict(((to_underscore_separated(clazz.__name__), clazz) for clazz in find_subclasses(hjb.demo.Demo)))
...
class DemoCommand(object):
    ...
    def _generate_execute(self, args, **kw):
        ...
        """
        subcommand = args[0]
        if subcommand not in demo_commands:
            self.usage('%s is not a recognised subcommand. These available subcommands are: %s' % (subcommand, demo_commands_as_text), exit_with=1)
