...
# Embedded file name: build/bdist.linux-i686/egg/hjb/demo.py
# Compiled at: 2006-06-04 05:28:09
__doc__ = 'Contains clases that demonstrate various messaging use cases\nusing a JMS Messaging provider, HJB and pyhjb.\n'
...
def create_memory_logging_handler(**kw):
    """Create a logging handler that buffers logged messages in memory."""
    return PeriodicFlushMemoryHandler(5, target_generator=create_file_logging_handler)


def create_file_logging_handler(logfile='C:\\Temp\\hjb_debug.log', level=logging.DEBUG):
    """Create a logging handler that sends logged messages to a file."""
...
def create_jms_logging_handler(demo, level=logging.INFO):
    """Create a logging handler that sends logged messages to a JMS destination."""
    dsc = demo.scenario
    logging_scenario = SimpleMessagingScenario(HJBClient(dsc.client.hostname, dsc.client.root), dsc.provider, dsc.factory, [
     demo.aliases['logmessage']], deepcopy(dsc.config))
    jms_logging_config = {'producers': [{'destination-url': logging_scenario.full_url_of(demo.aliases['logmessage'])}]}
...
class Demo(object):
    ...
    :ivar scenario: an instance's `hjb.hjbclient.SimpleMessagingScenario`
    ...
    def find_sending_url_for(self, message):
        ...
        Uses `self.aliases` to route messages to different producers
        depending on type of the message or the value of the
        *hjb_send_to* header in the message when that is present and
        the destinations available in `self.scenario`.
    ...
    def create_heartbeat(self, name):
        """Create a `demoutil.Heartbeat` that fires heartbeat messages.
        
        Assumes that `self.scenario` has a first session with a configured
        heartbeat producer. 
        ...
        sc = self.scenario
        return Heartbeat(str(self), sc.client, self.find_sending_url_for(dummy_message))
...
class SendEachJmsMessageType(Demo):
    ...
    def _create_session_config(self):
        ...
        sc = self.scenario
        destinations = self.get_producer_aliases()
        return {'producers': [ {'destination-url': sc.full_url_of(self.aliases[d])} for d in destinations ]}
...
class PrintMessagesOnQueue(Demo):
    ...
    def _create_session_config(self):
        ...
        """
        sc = self.scenario
        return {'browsers': [{'destination-url': sc.full_url_of(self.aliases[self.receiver_name])}], 'producers': [{'destination-url': sc.full_url_of(self.aliases['heartbeat'])}]}
...
class ReceiveLotsOfMessages(Demo):
    ...
    def _create_session_config(self):
        ...
        """
        sc = self.scenario
        return {'consumers': [{'destination-url': sc.full_url_of(self.aliases[self.receiver_name])}]}

    def run(self, max_tries=10, time_out=3000, receiver_url=None, handler_generators=None):
        ...
        try:
            while max_tries < 0 or missed < max_tries and count < max_tries:
                try:
                    messages = sc.client.get_messages(receiver_url, config={'receive': {'timeout': str(hjb_long(time_out))}})
                    count = count + 1
...
class MonitorHeartbeats(ReceiveLotsOfMessages):
    ...
    def _create_session_config(self):
        ...
        """
        sc = self.scenario
        return {'consumers': [{'destination-url': sc.full_url_of(self.aliases['heartbeat'])}]}
    ...
    def run(self):
        ...
        while True:
            try:
                super(MonitorHeartbeats, self).run(max_tries=-1, time_out=3000, handler_generators=[monitor.create_handler_generator], receiver_url=heartbeat_retrieval_url)
            except:
                monitor.stop()
                del monitor
                raise
...
class ReceiveJmsLogs(ReceiveLotsOfMessages):
    ...
    def _create_session_config(self):
        ...
        """
        sc = self.scenario
        return {'subscribers': [{'destination-url': sc.full_url_of(self.aliases['logmessage']), 'subscriber-name': 'hjbclient'}]}
    ...
    def run(self):
        ...
        super(ReceiveJmsLogs, self).run(max_tries=-1, time_out=3000, handler_generators=[accept_jms_log], receiver_url=log_retrieval_url)
...
class SendAndWaitForConversationResponse(ReceiveLotsOfMessages):
    ...
    This method is designed to be run at the same time as another
    process running the demo `EchoInConversation`.
        
    The producer selects a message at random those produced by
    `self.message_generator`, adding a header with a specific value
    that is known to the receiving demo process (the value of
    `self.conversation_id`).
        
    The consumer waits for a message identified by
    `self.conversation_id`.  This is done by using a JMS message
    selector expression to configure the consumer.  The consumer times
    out if no message is received after waiting for the timeout
    period.

    The producer sends the message, then the consumer waits for a
    response with same conversation_id, eventually timing out if none
    is received.

    :ivar conversation_id: a specific value known by both the sending
                           and receiving demos.

    :ivar message_generator: a generator that produces HJB encoded JMS
                             messages.

    """
    __module__ = __name__

    def __init__(self, scenario, aliases, receiver_name='Text', sender_name='Map', conversation_id='hjb_demo', message_generator=five_simple_messages_generator()):
    ...
    def _create_session_config(self):
        ...
        sc = self.scenario
        al = self.aliases
        self.configured_producers = {self.sender_name: al[self.sender_name], 'heartbeat': al['heartbeat']}
        destinations = self.get_producer_aliases()
        return {'producers': [ {'destination-url': sc.full_url_of(self.aliases[d])} for d in destinations ], 'consumers': [{'destination-url': sc.full_url_of(al[self.receiver_name]), 'message-selector': "hjb_conversation_id = '%s'" % self.conversation_id}]}
    ...
    def send_a_message(self):
        ...
        request.headers = {'hjb_demo': hjb_bool(True), 'hjb_send_to': self.sender_name, 'hjb_conversation_id': self.conversation_id}
        sc = self.scenario
        sc.client.send_message(request, self.find_sending_url_for(request))
    ...
    def run(self, time_out=30000, **kw):
        ...
        try:
            ...
            sys.stdout.flush()
            super(SendAndWaitForConversationResponse, self).run(max_tries=1, time_out=time_out, **kw)
        finally:
            print
            try:
                heartbeat.stop()
...
class EchoInConversation(SendAndWaitForConversationResponse):
    ...
    running the `SendAndWaitForConversationResponse` demo.
    ...
    def __init__(self, scenario, aliases, receiver_name='Map', sender_name='Text', conversation_id='hjb_demo'):
        super(EchoInConversation, self).__init__(scenario, aliases, receiver_name=receiver_name, sender_name=sender_name, conversation_id=conversation_id)
    ...
    def run(self, time_out=30000):
        ...
        try:
            heartbeat.start()
            ReceiveLotsOfMessages.run(self, max_tries=1, time_out=time_out, handler_generators=[self.create_handler_generator])
        finally:
            print
            try:
                heartbeat.stop()
    ...
    def create_handler_generator(self, message):
        """Create a handler_generator function suitable for use by `ReceiveLotsOfMessages.run()`."""
        cid = self.conversation_id
        if cid != message.headers.get('hjb_conversation_id', None):
            print '\r... [**   ERROR   **] bad headers: ' + str(message.headers)
            log.warning('message did not have the expected conversation_id [%s]; headers were: %s', cid, str(message.headers))
            return

        def handle_message():
            """Create a new message with the same body, and send it to the return destination.
            ...
            response.headers = {'hjb_demo': hjb_bool(True), 'hjb_send_to': self.sender_name, 'hjb_conversation_id': self.conversation_id}
            sc = self.scenario
            sc.client.send_message(response, self.find_sending_url_for(response))
...
class SendLotsOfMessages(SendEachJmsMessageType):
    ...
    def __init__(self, scenario, aliases, message_generator=five_simple_messages_generator()):
