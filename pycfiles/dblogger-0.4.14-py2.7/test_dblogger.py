...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dblogger/tests/test_dblogger.py
# Compiled at: 2015-04-26 18:06:04
from __future__ import absolute_import
import os, yaml, time, logging, random, math, uuid, subprocess, pytest, sys, kvlayer, yakonfig
...
def client(request, namespace_string, redis_address):
    config = dict(namespace=namespace_string, storage_type='redis', app_name='dbltest', storage_addresses=[
     redis_address])
    print config
    yakonfig.set_global_config(dict(kvlayer=config))
    client = kvlayer.client()
    client.setup_namespace(dict(existing_table_1=2, existing_table_2=2))
...
def test_queries_cli(client):
    ...
    child = subprocess.Popen([
     ...
     '--storage-address', client._config['storage_addresses'][0]], stderr=subprocess.PIPE, stdout=subprocess.PIPE)
    child.wait()
...
def test_queries_cli2(client):
    ...
    child = subprocess.Popen([
     ...
     '--begin', '1998-01-03T08'], stderr=subprocess.PIPE, stdout=subprocess.PIPE)
    child.wait()
    out = child.stdout.read()
    err = child.stderr.read()
