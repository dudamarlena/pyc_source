...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dblogger/query.py
...
import argparse, logging, re, sys, time, dblogger
...
import kvlayer, streamcorpus, yakonfig
...
def main():
    parser = argparse.ArgumentParser(description='retrieve log messages from kvlayer')
    parser.add_argument('--begin', default=None, help='YYYY-MM-DDTHH:MM:SS.MMMMMMZ in UTC can be truncated at any depth')
    parser.add_argument('--end', default=None, help='YYYY-MM-DDTHH:MM:SS.MMMMMMZ in UTC can be truncated at any depth')
    parser.add_argument('--past', type=int, default=60, metavar='N', help='show the most recent N seconds of logs, default N=60. If negative, then scan from earliest moment *upto* N seconds ago.')
    parser.add_argument('--clear', action='store_true', default=False, help='delete all messages in scan')
    parser.add_argument('-y', '--yes', default=False, action='store_true', dest='assume_yes', help='assume "yes" and require no input for confirmation questions.')
        ...
        if args.end:
            ...
            if args.clear:
                ...
                if not args.assume_yes:
                    response = raw_input(('Delete all logs between {!r} and {!r} in {!r}?  Enter namespace: ').format(args.begin, args.end, namespace))
