...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dblogger/tests/test_dblogger_throughput.py
# Compiled at: 2015-04-26 18:06:04
import time, pytest, kvlayer, logging, multiprocessing, yakonfig
from dblogger.tests.test_dblogger import client
...
def test_queries_throughput(client):
    ...
    for x in xrange(num_tasks):
        results.append(pool.apply_async(worker, args=(client._config, num_records)))
    ...
    logger.critical('finished logging %d messages in %.1f sec --> %.1f per sec', expected_count, elapsed, float(expected_count) / elapsed)
    count = 0
