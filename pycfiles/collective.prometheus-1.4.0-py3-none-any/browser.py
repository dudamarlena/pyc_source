...
# [GCC 8.4.0]
# Embedded file name: /Users/rob/Developer/Projects/Purdue/Metrics/collective.prometheus/src/collective/prometheus/browser.py
# Compiled at: 2019-11-22 09:14:06
import sys
...
class Prometheus(BrowserView):
    ...
    def _getdbs(self):
        ...
        if filestorage == '*':
            ...
            for filestorage in db.getDatabaseNames():
                yield (
                 db[filestorage], '_%s' % filestorage)
    ...
    def zopethreads(self):
        ...
        return [
         metric('zope_total_threads', total_threads, 'gauge', 'The number of running Zope threads'),
         metric('zope_free_threads', free_count, 'gauge', 'The number of Zope threads not in use')]
    ...
    def _zopecache(self, db, suffix):
        return [
         metric('zope_cache_objects' + suffix, db.database_size(), 'gauge', 'The number of objects in the Zope cache'),
         metric('zope_cache_memory' + suffix, db.cache_length(), 'gauge', 'Memory used by the Zope cache'),
         metric('zope_cache_size' + suffix, db.cache_size(), 'gauge', 'The number of objects that can be stored in the Zope cache')]
    ...
    def _zodbactivity(self, db, suffix):
        ...
        return [
         metric('zodb_load_count' + suffix, chart['total_load_count'], 'counter', 'ZODB load count'),
         metric('zodb_store_count' + suffix, chart['total_store_count'], 'counter', 'ZODB store count'),
         metric('zodb_connections' + suffix, chart['total_connections'], 'gauge', 'ZODB connections')]

    def _zopeconnections(self, db, suffix):
        zodb = db._p_jar.db()
        result = []
        sorted_cache_details = sorted(zodb.cacheDetailSize(), key=lambda m: m['connection'])
        for conn_id, conn_data in enumerate(sorted_cache_details):
            total = conn_data.get('size', 0)
            active = metric(('zope_connection_{}_active_objects').format(conn_id), conn_data.get('ngsize', 0), 'gauge', 'Active Zope Objects')
            result.append(active)
            total = metric(('zope_connection_{}_total_objects').format(conn_id), conn_data.get('size', 0), 'gauge', 'Total Zope Objects')
            result.append(total)
