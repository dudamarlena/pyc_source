...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/haoda/report/xilinx/hls.py
# Compiled at: 2020-04-29 15:13:37
# Size of source mod 2**32: 8760 bytes
import io, sys, xml.etree.ElementTree as ET
...
class HlsResources:
    ...
    """
        ...
        if obj is None:
            ...
            return
        raise TypeError('obj must be an xml.etree.ElementTree.Element object or another HlsResources object or an io.TextIOBase object or None')

    def init_from_xml_element(self, elem: ET.Element, item: str='Resources') -> 'HlsResources':
    ...
    """
        ...
        for resource in HlsResources.RESOURCES:
            self[resource] = int(elem.findtext('/'.join(('AreaEstimates', item, resource))))
    ...
    def __str__(self) -> str:
        return 'module: %16s, %s' % (self.name or 'n/a',
         ', '.join('%s: %7d' % item for item in self))
...
class HlsPerformance:
    __doc__ = 'An object representing the HLS performance estimation.\n\n  Attributes:\n    name: Optional name of the module.\n    ii: Integer of pipeline II.\n    depth: Integer of pipeline depth.\n  '

    def __init__(self, obj: Union[('HlsPerformance', ET.Element, TextIO, None)]=None) -> None:
    ...
    """
        ...
        if obj is None:
            return
        raise TypeError('obj must be an xml.etree.ElementTree.Element object or another HlsPerformances object or an io.TextIOBase object or None')
...
def available_resources(obj: Union[(TextIO, str)]) -> HlsResources:
  ...
  """
    if isinstance(obj, io.TextIOBase):
        return HlsResources().init_from_xml_element(ET.parse(obj).getroot(), 'AvailableResources')
    if isinstance(obj, str):
        with open(obj, 'r') as (fd):
