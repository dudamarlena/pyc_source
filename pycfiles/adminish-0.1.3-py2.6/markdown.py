...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/adminish/markdown.py
# Compiled at: 2009-02-25 17:52:39
...
class CommentProcessor(Preprocessor):
    ...
    def run(self, lines):
        ...
        for line in lines:
            ...
            for pos in ends:
                events.append((pos, 'end'))

            events.sort(key=lambda i: i[0])
            cursorPos = 0
            for (n, event) in enumerate(events):
                if inComment is True and event[1] == 'end':
                    cursorPos = event[0] + 3
                    inComment = False
                elif inComment is False and event[1] == 'start':
                    outLine += line[cursorPos:event[0]]
                    cursorPos = event[0]
                    inComment = True
            ...
            if len(events) == 0:
                if inComment is False:
                    outLine = line
                if len(events) > 0 and cursorPos < len(line) and inComment is False:
                    outLine += line[cursorPos:]
                outLine.startsWith('%%') or out.append(outLine)
...
class Markdown():
    ...
    def _processSection(self, parent_elem, lines, inList=0, looseList=0):
        ...
        if not lines:
            return
        processFn = {'ul': self._processUList, 'ol': self._processOList, 
        ...
        if inList:
            ...
            if len(paragraph) and paragraph[0].startswith('#'):
                ...
                if parent_elem.nodeName == 'li' and not (looseList or parent_elem.childNodes) or paragraph[(-1)].strip().startswith('<'):
                    el = parent_elem
                else:
...
def markdown(text, extensions=[], encoding=None, safe_mode=False):
    ...
    extension_configs = {}
    md = Markdown(text, extensions=extensions, extension_configs=extension_configs, safe_mode=safe_mode)
    return md.toString()
...
def parse_options():
    try:
        ...
        if len(sys.argv) == 2:
            return {'input': sys.argv[1], 'output': None, 
               'message_threshold': CRITICAL, 
               'safe': False, 
               'extensions': [], 'encoding': None}
        else:
    ...
    if not len(args) == 1:
        ...
        if not options.extensions:
            options.extensions = []
        return {'input': input_file, 'output': options.filename, 'message_threshold': options.verbose, 
           'safe': options.safe, 
