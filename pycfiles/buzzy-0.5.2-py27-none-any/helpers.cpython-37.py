...
# Embedded file name: build/bdist.linux-x86_64/egg/explorer/parts/helpers.py
# Compiled at: 2020-05-05 16:50:24
...
import explore.models as CorpusModel
...
def _get_specs_and_corpus(search_from, searches, corpora, slug):
    ...
    if not corpora:
        ...
        return int(search_from) or (
         0, _get_corpus(slug))
    exists = searches[str(search_from)]
    return (
     int(search_from), _get_corpus(slug).iloc[exists[(-1)]])
...
def _get_cols(corpus, add_governor):
    ...
    longs = [(i, _capitalize_first(SHORT_TO_LONG_NAME.get(i, i)).replace('_', ' ')) for i in col_order if i in possible]
...
def _update_frequencies(df, deletable, content_table):
    ...
    if not content_table:
        df = df.reset_index()
    if not multicols:
        columns = [{'name':i.lstrip('_'),  'id':i,  'deletable':deletable and '_' + i not in names,  'hideable':True} for i in df.columns]
        return (
         columns, df.to_dict('rows'))
    columns = [{'name':[x.strip('_') for x in i], 
     'id':'-'.join(i),  'deletable':['_' + x in names for x in i], 
     'hideable':True} for i in df.columns]
    rows = []
...
def _update_concordance(df, deletable):
    ...
    columns = [{'name':_capitalize_first(SHORT_TO_COL_NAME.get(i, i)),  'id':i,  'deletable':i not in cannot_delete and deletable,  'hideable':True} for i in df.columns]
    return (
     columns, df.to_dict('rows'))
