...
# Embedded file name: build/bdist.linux-x86_64/egg/explorer/parts/tabs.py
# Compiled at: 2020-05-05 16:52:19
# Size of source mod 2**32: 20185 bytes
__doc__ = '\nbuzzword explorer: build the explore page and its tabs\n'
import json, dash_core_components as dcc, dash_daq as daq, dash_html_components as html, dash_table
...
def _build_dataset_space(df, config):
    ...
    pieces = [
     dcc.Dropdown(id='search-target',
       options=cols,
       value='w',
       style={**{'width':'200px', 
      'fontFamily':'monospace'}, **(style.NEAR_FRONT)}),
     html.Div(id='matching-box',
       children=[
      daq.BooleanSwitch(theme=DAQ_THEME,
        className='colour-off',
        id='skip-switch',
        on=False,
        style={**{'verticalAlign': 'top'}, **(style.MARGIN_5_MONO)}),
      html.Div(id='matching-text',
     ...
     dcc.Dropdown(id='gram-select',
       options=grams,
       placeholder='What to return from search',
       disabled=False,
       style={**{'width':'240px', 
      'fontFamily':'monospace'}, **(style.FRONT)}),
     html.Button('Search', id='search-button')]
    pieces = [html.Div(piece, style=(style.CELL_MIDDLE_35)) for piece in pieces]
    search_space = html.Div(pieces,
      style={**{'fontFamily': 'bold'}, **(style.VERTICAL_MARGINS)})
    columns = [{'name':_capitalize_first(SHORT_TO_COL_NAME.get(i, i)).replace('_', ' '),  'id':i,  'deletable':False,  'hideable':True} for i in df.columns]
    data = df.to_dict('rows')
    conll_table = dash_table.DataTable(id='conll-view',
      columns=columns,
      data=data,
      editable=True,
    ...
    conll_table = dcc.Loading(type='default',
      id='loading-main',
      fullscreen=True,
...
def _build_frequencies_space(corpus, table, config):
    ...
    show_check = dcc.Dropdown(placeholder='Features to show',
      multi=True,
      id='show-for-table',
      options=cols,
      value=[],
    ...
    subcorpora_drop = dcc.Dropdown(id='subcorpora-for-table',
      options=([
     dict(value='_corpus', label='Entire corpus')] + cols),
      placeholder='Feature for index',
      style={**(style.MARGIN_5_MONO), **(style.FRONT)})
    subcorpora_drop = html.Div(subcorpora_drop, style=(style.TSTYLE))
    relative_drop = dcc.Dropdown(id='relative-for-table',
    ...
    sort_drop = dcc.Dropdown(id='sort-for-table',
      style={**(style.MARGIN_5_MONO), **(style.NEAR_FRONT)},
      options=[
     {'label':'Total', 
      'value':'total'},
    ...
    freq_table = dcc.Loading(type='default',
      children=[
     dash_table.DataTable(id='freq-table',
       columns=columns,
       data=data,
       editable=False,
       style_cell={**(style.HORIZONTAL_PAD_5), **{'maxWidth':'145px', 
      'minWidth':'60px'}},
       filter_action='native',
       sort_action='native',
       sort_mode='multi',
      ...
      style_index] + style.STRIPES),
       merge_duplicate_headers=True,
       export_format='xlsx',
       export_headers='display')])
    sty = {**{'width': '20%'}, **(style.CELL_MIDDLE_35), **(style.MARGIN_5_MONO)}
    multi = html.Span(children=[
     daq.BooleanSwitch(theme=DAQ_THEME,
       id='multiindex-switch',
       on=False,
       disabled=True,
       style={**(style.MARGIN_5_MONO), **(style.TSTYLE)}),
     html.Div(children='Multicolumn mode',
       id='multiindex-text',
       style={**(style.MARGIN_5_MONO), **(style.TSTYLE), **{'whiteSpace': 'nowrap'}})],
      style={**(style.CELL_MIDDLE_35), **(style.TSTYLE)})
    content = html.Span(children=[
     daq.BooleanSwitch(theme={**DAQ_THEME, **{'primary': '#47d153'}},
       id='content-table-switch',
       on=False,
       style={**(style.MARGIN_5_MONO), **(style.TSTYLE)}),
     html.Div(children='Show content, not frequency',
       style={**(style.MARGIN_5_MONO), **(style.TSTYLE)})],
      style={**(style.CELL_MIDDLE_35), **(style.TSTYLE)})
    gen = 'Generate table'
...
def _build_concordance_space(df, config):
    ...
    if isinstance(df, Corpus):
        ...
        cols = _get_cols(df, config['add_governor'])
        show_check = dcc.Dropdown(multi=True,
    ...
    print('Done!')
    just = [
     'left', 'match', 'right', 'file', 's', 'i']
    if 'speaker' in df.columns:
        just.append('speaker')
    df = df[just]
    columns = [{'name':_capitalize_first(SHORT_TO_COL_NAME.get(i, i)),  'id':i,  'deletable':i not in ('left', 'match', 'right'),  'hideable':True} for i in df.columns]
    style_data = [
     style.STRIPES[0], style.INDEX[0]] + style.CONC_LMR
    data = df.to_dict('rows')
    rule = 'display: inline; white-space: inherit; overflow: inherit; text-overflow: inherit;'
    conc = dcc.Loading(type='default',
      children=[
     dash_table.DataTable(id='conc-table',
       css=[
...
def _build_chart_space(table, config):
    ...
    """
    charts = []
    for chart_num, kind in ((1, 'stacked_bar'), (2, 'line'), (3, 'area'), (4, 'heatmap'),
                            (5, 'bar')):
        table_from = [
         dict(value=0, label=(_make_table_name('initial')))]
        dropdown = dcc.Dropdown(id=f"chart-from-{chart_num}",
          options=table_from,
          value=0,
          style=(style.MARGIN_5_MONO))
        types = [dict(label=(_capitalize_first(i).replace('_', ' ')), value=i) for i in sorted(CHART_TYPES)]
        chart_type = dcc.Dropdown(id=f"chart-type-{chart_num}",
          options=types,
          value=kind,
          style=(style.MARGIN_5_MONO))
        transpose = daq.BooleanSwitch(theme=DAQ_THEME,
        ...
        toolbar = [
         dropdown, chart_type, top_n, transpose, update]
        widths = {dropdown: '65%', 
         chart_type: '25%', 
         top_n: '3%', 
         transpose: '4%', 
        ...
        chart_data = dict(id=f"chart-{chart_num}",
          figure=figure,
          style={'height':'60vh', 
         'width':'95vw'})
...
def make_explore_page(corpus, table, config, configs):
    ...
    show = html.Button('Show',
      id='show-this-dataset', style={**(style.MARGIN_5_MONO), **(style.FRONT)})
    ...
    dropdown = dcc.Dropdown(id='search-from',
      options=search_from,
      value=0,
      disabled=True)
    drop_style = {**{'fontFamily':'monospace', 
     'width':'50%'}, **(style.HORIZONTAL_PAD_5), **(style.BLOCK_MIDDLE_35), **(style.FRONT)}
    nav = {k:v for k, v in style.NAV_HEADER.items() if k != 'paddingTop' if k != 'paddingTop'}
    top_bit = [
     html.Img(src='../../static/bolt.jpg',
       height=42,
       width=38,
