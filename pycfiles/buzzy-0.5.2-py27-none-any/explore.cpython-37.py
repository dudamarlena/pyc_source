...
# Embedded file name: build/bdist.linux-x86_64/egg/buzzword/parts/explore.py
# Compiled at: 2019-08-24 18:30:44
...
from buzzword.parts.helpers import _get_specs_and_corpus, _translate_relative, _update_datatable, _tuple_or_list, _cast_query
from buzzword.parts.strings import _make_search_name, _make_table_name, _search_error, _table_error
...
for i in range(1, 6):
    ...
    @app.callback(Output(f"chart-{i}", 'figure'), [
...
@app.callback([
 ...
 State('url', 'pathname')])
def _new_search(n_clicks, cleared, search_from, skip, col, search_string, search_from_options, current_cols, current_data, conf, session_search, session_clicks_clear, url):
    """
    ...
    if n_clicks is None:
        ...
        this_search = [
         specs, col, skip, search_string]
        exists = next((i for i in session_search.values() if this_search == list(i)[:4]), False)
        if exists:
        ...
        if found_results:
            ...
            current_cols, current_data = _update_datatable(corpus,
              df, drop_govs=add_governor, deletable=True)
        if not msg:
...
@app.callback([
 ...
 State('url', 'pathname')])
def _new_table(n_clicks, prev_cols, prev_data, current_cols, current_data, search_from, show, subcorpora, relkey, sort, table_from_options, nv1, conf, session_search, session_tables, session_click_table, url):
    """
