...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dictionaryutils/__init__.py
...
import glob, logging, requests, os, yaml, json
...
def add_default_schema(dictionary):
    ...
    schemas = {schema['id']:dictionary.resolve_schema(schema, deepcopy(schema)) for path, schema in yamls.items() if path not in dictionary.exclude}
...
def dump_schemas_from_dir(directory):
    ...
    with visit_directory(directory):
        ...
        if '_settings.yaml' not in result:
...
class DataDictionary(object):
    ...
    def __init__(self, lazy=False, root_dir=None, definitions_paths=None, metaschema_path=None, url=None, local_file=None):
        ...
        self.schema = dict()
        self.resolvers = dict()
        self.metaschema = load_yaml(os.path.join(MOD_DIR, 'schemas', self.metaschema_path))
        if not lazy:
    ...
    def load_data(self, directory=None, url=None, local_file=None):
        ...
        yamls, resolvers = load_schemas_from_dir(os.path.join(MOD_DIR, 'schemas'))
        if url:
            yamls, resolvers = load_schemas_from_url(url,
              (self.logger), schemas=yamls, resolvers=resolvers)
        else:
            if local_file:
                yamls, resolvers = load_schemas_from_file(local_file,
                  schemas=yamls, resolvers=resolvers)
            else:
        ...
        schemas = {schema['id']:self.resolve_schema(schema, deepcopy(schema)) for path, schema in yamls.items() if path not in self.exclude}
    ...
    def allow_nulls(self):
        ...
        for node_properties in self.schema.values():
            ...
            for prop_id, prop in node_properties.get('properties', {}).items():
                if prop_id not in required and 'type' in prop and 'null' not in prop['type']:
                    if not isinstance(prop['type'], list):
                        prop['type'] = [
                         prop['type']]
                    prop['type'] += ['null']
                else:
                    if prop_id not in required:
                        if 'enum' in prop:
                            if None not in prop['enum']:
                                prop['enum'] = isinstance(prop['enum'], list) or [
                                 prop['enum']]
                        prop['enum'] += [None]