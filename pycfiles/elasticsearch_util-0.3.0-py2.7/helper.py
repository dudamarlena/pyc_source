...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\elasticsearch_util\helper.py
# Compiled at: 2018-05-16 14:07:02
import atexit, datetime, functools, hashlib, logging, os, socket, threading, time, traceback, urllib2
...
class ElasticSearchHelper(object):
    ...
    @classmethod
    def get_instance(cls, host, index, port=9200, use_ssl=False, verify_certs=True, connection_class=elasticsearch.RequestsHttpConnection, **kwargs):
        """
        Factory method for easy construction
        :rtype: ElasticSearchHelper
        """
        client = elasticsearch.Elasticsearch(hosts=[{'host': host, 'port': port}], use_ssl=use_ssl, verify_certs=verify_certs, connection_class=connection_class, **kwargs)
    ...
    def create_data_record(self, data_dict):
        ...
        assert not self.is_conflicting_keys(data_dict, self.default_values), 'Conflicting keys between default_values and extra_values'
        source_dict.update(self.default_values)
        return {'_index': self.get_full_index(), 
           '_type': 'python_log', 
           '_source': source_dict}
    ...
    def default_values(self):
        data_dict = {Constants.HOST_KEY: socket.getfqdn(), 
        ...
        assert not self.is_conflicting_keys(data_dict, self.extra_values), 'Conflicting keys between default_values and extra_values'
    ...
    def log_feature(self, feature_name, feature_duration_seconds=None, levelname=Constants.INFO_VALUE, **kwargs):
        ...
        :return:
        """
        data_dict = {Constants.FEATURE_NAME_KEY: feature_name, 
    ...
    def log_feature_error(self, feature_name, feature_duration_seconds=None, **kwargs):
        ...
        """
        self.log_feature(feature_name, feature_duration_seconds=feature_duration_seconds, levelname=Constants.ERROR_VALUE, **kwargs)
        ...
        def decorator(function):
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    try:
                        self.log_feature_error(feature_name, duration_seconds, exc_text=exc_text, exc_hash=exc_hash, exc_message=e.message, **feature_kwargs)
                    except Exception as e:
