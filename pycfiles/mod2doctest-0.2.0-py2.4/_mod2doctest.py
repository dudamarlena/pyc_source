...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\mod2doctest\_mod2doctest.py
# Compiled at: 2010-09-21 19:58:51
...
"""
import sys, os, inspect, subprocess, re, doctest, time

class FLAGS(object):
    r"""Option flags used to control docstring output
...
DEFAULT_DOCTEST_FLAGS = doctest.ELLIPSIS | doctest.REPORT_ONLY_FIRST_FAILURE | doctest.NORMALIZE_WHITESPACE

def convert(python_cmd, src=True, target=True, add_testmod=True, flags=FLAGS.ALL, break_on_newlines=True, run_doctest=False, doctest_flags=DEFAULT_DOCTEST_FLAGS, fn_process_input=None, fn_process_docstr=None, fn_title_docstr=None):
    """
    ...
    if inspect.ismodule(src):
        ...
        raise SystemError, "'src' %s must be a valid module or file path, or string ..." % obj
    ...
    pinput = '\n\n' + pinput.strip() + '\n'
    shell = subprocess.Popen(args=[python_cmd, '-i'], shell=False, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
...
_REGEX_INPUT_FIX_COMMENTS = re.compile('^(#.*)', flags=re.MULTILINE)

def _input_fix_comments(input):
    """Puts a newline after comments -- needed for post processing."""
    return _REGEX_INPUT_FIX_COMMENTS.sub('\\1\\n', input)


_REGEX_REMOVE_M2D_PRINT = re.compile('\\nm2d_print.*\\([\\\'"]*(.*?)[\\\'"]*\\).*')

def _input_remove_m2d_print(docstr):
    return _REGEX_REMOVE_M2D_PRINT.sub('\\n#\\1\\n', docstr)


_REGEX_INPUT_FIX_WHITESPACE_0 = re.compile('(\\n[ \\t]+)((\\n[ \\t]*)*)\\n')
_REGEX_INPUT_FIX_WHITESPACE_1 = re.compile('\\n((\\n[ \\t]+)*)\\n')
_REGEX_INPUT_FIX_WHITESPACE_2 = re.compile('(\\n[ \\t]+.*)\n                                            \\n(?!else|elif|except|finally)\n                                            (\\S+.*)', flags=re.VERBOSE)
...
def _input_fix_whitespace(input):
    ...
    input = _REGEX_INPUT_FIX_WHITESPACE_0.sub('\\1\\n', input)
    input = _REGEX_INPUT_FIX_WHITESPACE_1.sub('\\n\\n', input)
    return _REGEX_INPUT_FIX_WHITESPACE_2.sub('\\1\\n\\n\\2', input)


_REGEX_INPUT_FIX_WS_RSTRIP = re.compile('(\\S)[ \\t]*\\n')

def _input_fix_ws_rstrip(input):
    """Right strips line that contain non whitespace characters."""
    return _REGEX_INPUT_FIX_WS_RSTRIP.sub('\\1\\n', input)


_REGEX_NEM = re.compile('\n^if\\s+__name__\\s*==\\s*[\'"]__main__[\'"]\\s*:.*\n(\\n[ \\t].*)*', flags=re.MULTILINE | re.VERBOSE)
...
_REGEX_OUTPUT_FIXUP = re.compile('^[ \\t]*$', flags=re.MULTILINE)

def _docstr_fix_blanklines(docstr):
    return _REGEX_OUTPUT_FIXUP.sub('<BLANKLINE>', docstr)
...
def _docstr_save(docstr, src, target, input, add_testmod):
    ...
    if add_testmod and src is not target:
        ...
        add_testmod = ''
    output = 'r%s\n%s\n%s' % (docstr, add_testmod, input)
    open(target, 'w').write(output)
    return target


_REGEX_DOCSTR_FIX_COMMENTS = re.compile('(\\n>>>\\s*#.*)\\n... ')

def _docstr_fix_comments(docstr):
    return _REGEX_DOCSTR_FIX_COMMENTS.sub('\\1', docstr)


_REGEX_LEFT_SHIFT_COMMENTS = re.compile('^>>>\\s*#\\s*')
...
def _docstr_left_shift_comments(docstr):
    ...
    for line in docstr.split('\n'):
        ...
        if line == '' and count == 0:
            ...
            nline = _REGEX_LEFT_SHIFT_COMMENTS.sub('', line)
            if line == nline:
                count = 0
                in_break = False
            else:
...
_REGEX_THREE_NEWLINES = re.compile('\\n{2,}')
_REGEX_END_STUFF = re.compile('(\\n\\s*(>>>|<BLANKLINE>)[ |\\t]*)*$')

def _docstr_fixup(docstr):
    docstr = docstr.replace('"""', "'''")
    docstr = _REGEX_THREE_NEWLINES.sub('\\n\\n\\n', docstr)
    return _REGEX_END_STUFF.sub('\\n', docstr)


_REGEX_NEWLINE_TO_WS = re.compile('(\\\\?>>>\\s*\\n){2,}')
...
_REGEX_ELLIPSE_MEM_ID = re.compile('<(?:(?:\\w+\\.)*)(.*? at 0x)\\w+>')

def _docstr_ellipse_mem_id(docstr):
    return _REGEX_ELLIPSE_MEM_ID.sub('<...\\1...>', docstr)


_REGEX_ELLIPSE_TRACEBACK = re.compile('\n                                    (Traceback.*)\n                                    (?:(?:\\n[ |\\t]+.*)*)\n                                    (\\n\\w+.*)\n                                    ', flags=re.MULTILINE | re.VERBOSE)

def _docstr_ellipse_traceback(docstr):
    return _REGEX_ELLIPSE_TRACEBACK.sub('\\1\\n    ...\\2', docstr)


_REGEX_LOCAL_PATH = re.compile('\n(\\s+[\'|\\"]+)\n(?:/+|[a-zA-Z]:\\\\+)\n.*\n(?:/+|\\\\+)\n((?:\\w|\\.)+[\'|\\"]+)\n', flags=re.VERBOSE)

def _docstr_ellipse_paths(output):
    return _REGEX_LOCAL_PATH.sub('\\1...\\2', output)
