...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\mod2doctest\mod2doctest.py
# Compiled at: 2010-09-26 00:04:05
...
"""
import sys, os, inspect, subprocess, re, doctest, time, pdb
DEFAULT_DOCTEST_FLAGS = doctest.ELLIPSIS | doctest.REPORT_ONLY_FIRST_FAILURE | doctest.NORMALIZE_WHITESPACE

def convert(python_cmd, src=True, target='_doctest', add_autogen=True, add_testmod=True, ellipse_memid=True, ellipse_traceback=True, ellipse_path=True, run_doctest=False, doctest_flags=DEFAULT_DOCTEST_FLAGS, fn_process_input=None, fn_process_docstr=None, fn_title_docstr=None):
    """
...
_RE_NEM = re.compile('^if\\s+__name__\\s*==\\s*[\'"]+__main__[\'"]+\\s*:', flags=re.IGNORECASE)
...
_RE_SPLIT_TRACEBACK = re.compile('\n                                  (Traceback.*\n                                  (?:\\n[ |\\t]+.*)*\n                                  \\n\\w+.*)\n                                  ', flags=re.MULTILINE | re.VERBOSE)
_RE_OUTPUT_FIXUP = re.compile('^[ \\t]*$', flags=re.MULTILINE)

def _docstr_fix_blanklines(docstr):
    return _RE_OUTPUT_FIXUP.sub('<BLANKLINE>', docstr)
...
_RE_ELLIPSE_MEM_ID = re.compile('<(?:(?:\\w+\\.)*)(.*? at 0x)\\w+>')

def _docstr_ellipse_mem_id(docstr):
    return _RE_ELLIPSE_MEM_ID.sub('<...\\1...>', docstr)


_RE_ELLIPSE_TRACEBACK = re.compile('\n                                    (Traceback.*)\n                                    (?:(?:\\n[ |\\t]+.*)*)\n                                    (\\n\\w+.*)\n                                    ', flags=re.MULTILINE | re.VERBOSE)

def _docstr_ellipse_traceback(docstr):
    return _RE_ELLIPSE_TRACEBACK.sub('\\1\\n    ...\\2', docstr)


_RE_LOCAL_PATH = re.compile('\n                             (\\s+[\'|\\"]+)\n                             (?:/+|[a-zA-Z]:\\\\+)\n                             .*\n                             (?:/+|\\\\+)\n                             ((?:\\w|\\.)+[\'|\\"]+)\n                             ', flags=re.VERBOSE)

def _docstr_ellipse_paths(output):
    return _RE_LOCAL_PATH.sub('\\1...\\2', output)


_RE_PRINT_MARKER = re.compile('(?:>>>|...)\\s#[>|]')
