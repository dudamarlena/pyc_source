...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\UnityPy\files\SerializedFile.py
...
class SerializedFile(File):
    ...
    def set_version(self, string_version):
        ...
        self.build_type = BuildType(re.findall('([^\\d.])', string_version)[0])
        version_split = re.split('\\D', string_version)
    ...
    def read_serialized_type(self):
        ...
        if self.header.version >= 13:
            if self.header.version < 16 and type_.class_id < 0 or self.header.version >= 16 and type_.class_id == 114:
                type_.script_id = self.reader.read_bytes(16)
            type_.old_type_hash = self.reader.read_bytes(16)
    ...
    def read_type_tree5(self, type_tree):
        ...
        for i in range(number_of_nodes):
            ...
            type_tree_node.type = read_string(string_buffer_reader, type_tree_node.type_str_offset)
            type_tree_node.name = read_string(string_buffer_reader, type_tree_node.name_str_offset)
    ...
    def save(self) -> bytes:
        ...
        if header.version >= 9:
            ...
            fileheader_writer.write_u_int(header_writer.Length + data_writer.Length + fileheader_size + align_length)
            fileheader_writer.write_u_int(header.version)
            fileheader_writer.write_u_int(header_writer.Length + fileheader_size + align_length)
            fileheader_writer.write_boolean(header.endian == '>')
            fileheader_writer.write_bytes(header.reserved)
            return fileheader_writer.bytes + header_writer.bytes + b'\x00' * align_length + data_writer.bytes
        else:
            fileheader_size = 16
            metadata_size = header_writer.Length
            fileheader_writer.write_u_int(metadata_size)
            fileheader_writer.write_u_int(metadata_size + data_writer.Length + fileheader_size)
            fileheader_writer.write_u_int(header.version)
    ...
    def save_serialized_type(self, typ: SerializedType, header: SerializedFileHeader, writer: EndianBinaryWriter):
        ...
        if header.version >= 13:
            if header.version < 16 and typ.class_id < 0 or header.version >= 16 and typ.class_id == 114:
                writer.write_bytes(typ.script_id)
            writer.write_bytes(typ.old_type_hash)
        if self._enable_type_tree:
    ...
    def save_type_tree5(self, nodes: list, writer: EndianBinaryWriter, str_data=b''):
        ...
        strings_values = [(node.type_str_offset, node.name_str_offset) for node in nodes]
