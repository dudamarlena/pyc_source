...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\UnityPy\classes\AnimationClip.py
...
class QuaternionCurve:
    ...
    def __init__(self, reader):
        self.curve = AnimationCurve(reader, reader.read_quaternion)
        ...
        for index in (0, end, chunkStride // 4):
            for i in range(itemCountInChunk):
                ...
                while bits < self.m_BitSize:
                    x |= uint(self.m_Data[indexPos] >> bitPos << bits)
...
class PackedQuatVector:
    ...
    def UnpackQuats(self):
        ...
        for i in range(self.m_NumItems):
            ...
            lastComponent = flags & 3
            q[lastComponent] = math.sqrt(1 - sum)
            if flags & 4 != 0:
...
class Vector3Curve:
    ...
    def __init__(self, reader):
        self.curve = AnimationCurve(reader, reader.read_vector3)
        self.path = reader.read_aligned_string()
...
class FloatCurve:
    ...
    def __init__(self, reader):
        self.curve = AnimationCurve(reader, reader.read_float)
...
class xform:
    ...
    def __init__(self, reader):
        version = reader.version
        self.t = reader.read_vector3() if version[0] > 5 or version[0] == 5 and version[1] >= 4 else Vector3(reader.read_vector4())
        self.q = reader.read_quaternion()
        self.s = reader.read_vector3() if version[0] > 5 or version[0] == 5 and version[1] >= 4 else Vector3(reader.read_vector4())
...
class HumanGoal:
    ...
    def __init__(self, reader):
        ...
        if version[0] >= 5:
            self.m_HintT = reader.read_vector3() if version[0] > 5 or version[0] == 5 and version[1] >= 4 else Vector3(reader.read_vector4())
...
class HumanPose:
    ...
    def __init__(self, reader):
        ...
        self.m_LookAtPosition = reader.read_vector3() if version[0] > 5 or version[0] == 5 and version[1] >= 4 else Vector3(reader.read_vector4())
        ...
        if version[0] > 5 or version[0] == 5 and version[1] >= 2:
            ...
            self.m_TDoFArray = [reader.read_vector3() if version[0] > 5 or version[0] == 5 and version[1] >= 4 else Vector3(reader.read_vector4()) for _ in range(numTDof)]
        ...
        for frameIndex in range(2, len(frameList) - 1):
            ...
            for curveKey in frame.keyList:
                ...
                while i >= 0:
                    ...
                    try:
                        preCurveKey = [x for x in preFrame.keyList if x.index == curveKey.index][0]
...
class ClipMuscleConstant:
    ...
    def __init__(self, reader):
        ...
        self.m_AverageSpeed = reader.read_vector3() if version[0] > 5 or version[0] == 5 and version[1] >= 4 else Vector3(reader.read_vector4())
...
class AnimationClipBindingConstant:
    ...
    def __init__(self, reader):
        ...
        self.pptrCurveMapping = [PPtr(reader) for _ in range(numMappings)]
...
class AnimationClip(NamedObject):
    ...
    def __init__(self, reader):
        ...
        self.m_CompressedRotationCurves = [CompressedAnimationCurve(reader) for _ in range(numCRCurves)]
        if version[0] > 5 or version[0] == 5 and version[1] >= 3:
