...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\UnityPy\classes\Shader.py
# Compiled at: 2020-03-30 16:40:06
# Size of source mod 2**32: 16440 bytes
...
class Shader(NamedObject):
    ...
    def __init__(self, reader):
        ...
        if version[0] == 5 and version[1] >= 5 or version[0] > 5:
            ...
            self.platforms = [ShaderCompilerPlatform(x) for x in reader.read_u_int_array()]
...
class TextureParameter:
    ...
    def __init__(self, reader):
        ...
        if version[0] > 2017 or version[0] == 2017 and version[1] >= 3:
...
class ConstantBuffer:
    ...
    def __init__(self, reader):
        ...
        if version[0] > 2017 or version[0] == 2017 and version[1] >= 3:
            ...
            self.m_StructParams = [StructParameter(reader) for _ in range(numStructParams)]
...
class SerializedSubProgram:
    ...
    def __init__(self, reader):
        ...
        self.m_TextureParams = [TextureParameter(reader) for _ in range(numTextureParams)]
        ...
        self.m_ConstantBuffers = [ConstantBuffer(reader) for _ in range(numConstantBuffers)]
        numConstantBufferBindings = reader.read_int()
        self.m_ConstantBufferBindings = [BufferBinding(reader) for _ in range(numConstantBufferBindings)]
...
class SerializedProgram:
    ...
    def __init__(self, reader):
        ...
        self.m_SubPrograms = [SerializedSubProgram(reader) for _ in range(numSubPrograms)]
...
class SerializedShader:
    ...
    def __init__(self, reader):
        ...
        self.m_Dependencies = [SerializedShaderDependency(reader) for _ in range(numDependencies)]
