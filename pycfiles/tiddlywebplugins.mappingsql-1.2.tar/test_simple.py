...
# [GCC 8.4.0]
# Embedded file name: /Users/cdent/src/tiddlyweb-plugins/mappingsqlstore/test/test_simple.py
# Compiled at: 2011-04-09 06:18:53
...
def test_search():
    ...
    store = Store('tiddlywebplugins.mappingsql', {'db_config': 'sqlite:///test.db'}, {'tiddlyweb.config': config, 
       'tiddlyweb.query': {'field_one': [
                                       'fat']}})
    tiddlers = list(store.search(''))
    assert tiddlers[0].title == 'monkey'
    store = Store('tiddlywebplugins.mappingsql', {'db_config': 'sqlite:///test.db'}, {'tiddlyweb.config': config, 
       'tiddlyweb.query': {'field_two': [
                                       'clean']}})
    tiddlers = list(store.search(''))
    assert len(tiddlers) == 0
    store = Store('tiddlywebplugins.mappingsql', {'db_config': 'sqlite:///test.db'}, {'tiddlyweb.config': config, 
       'tiddlyweb.usersign': {'name': 'zow'}, 'tiddlyweb.query': {'field_two': [
                                       'clean']}})
    tiddlers = list(store.search(''))
