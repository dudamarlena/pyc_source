...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mautrix/client/api/user_data.py
...
class UserDataMethods(BaseClientAPI):
    ...
    async def search_users(self, search_query: str, limit: Optional[int]=10) -> UserSearchResults:
        """
        ...
        """
        content = await self.api.request(Method.POST, Path.user_directory.search, {'search_term':search_query, 
         'limit':limit})
        try:
            return UserSearchResults([User.deserialize(user) for user in content['results']], content['limited'])
    ...
    async def set_displayname(self, displayname: str) -> None:
        ...
        """
        await self.api.request(Method.PUT, Path.profile[self.mxid].displayname, {'displayname': displayname})
    ...
    async def set_avatar_url(self, avatar_url: str) -> None:
        ...
        """
        await self.api.request(Method.PUT, Path.profile[self.mxid].avatar_url, {'avatar_url': avatar_url})
