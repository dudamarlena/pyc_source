...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mautrix/appservice/state_store/json.py
...
class JSONStateStore(StateStore):
    ...
    def save(self, file: str) -> None:
        if isinstance(file, str):
            ...
            output = file
        json.dump({'registrations':list(self.registrations), 
         'members':{room_id:{user_id: member.serialize()} for room_id, members in self.members.items() for user_id, member in members.items()}, 
         'power_levels':{room_id:levels.serialize() for room_id, levels in self.power_levels.items()}}, output)
        if isinstance(file, str):
            output.close()
    ...
    def load(self, file: str) -> None:
        ...
        if 'members' in data:
            self.members = {room_id:{user_id: Member.deserialize(content)} for room_id, members in data['members'].items() for user_id, content in members.items()}
        if 'power_levels' in data:
            self.power_levels = {room_id:PowerLevelStateEventContent.deserialize(content) for room_id, content in data['power_levels'].items()}
        if isinstance(file, str):
            input_source.close()
