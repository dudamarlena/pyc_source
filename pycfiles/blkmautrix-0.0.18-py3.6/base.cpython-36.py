...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mautrix/util/db/base.py
...
class BaseClass:
    ...
    def _constraint_to_clause(self, constraint: Constraint) -> ClauseElement:
        return and_(*[column == self.__dict__[name] for name, column in constraint.columns.items()])
    ...
    def _insert_values(self: T) -> Dict[(str, Any)]:
        ...
        return {column_name:self.__dict__[column_name] for column_name in self.column_names if column_name in self.__dict__}
    ...
    def _upsert_postgres(self: T, conn: Connection) -> None:
        conn.execute((pg_insert(self.t).values)(**self._insert_values).on_conflict_do_update(constraint=(self.t.primary_key),
