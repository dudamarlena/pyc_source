...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mautrix/client/api/rooms.py
# Compiled at: 2019-11-21 01:01:47
# Size of source mod 2**32: 23857 bytes
from typing import Optional, List, Union, Dict, Any
import asyncio
from ...errors import MatrixResponseError, MatrixRequestError, MRoomInUse
from ...api import Method, JSON, Path
from .types import UserID, RoomID, RoomAlias, StateEvent, RoomDirectoryVisibility, RoomAliasInfo, RoomCreatePreset, DirectoryPaginationToken, RoomDirectoryResponse, Serializable, StrippedStateEvent
...
class RoomMethods(BaseClientAPI):
    ...
    async def create_room(self, alias_localpart: Optional[str]=None, visibility: RoomDirectoryVisibility=RoomDirectoryVisibility.PRIVATE, preset: RoomCreatePreset=RoomCreatePreset.PRIVATE, name: Optional[str]=None, topic: Optional[str]=None, is_direct: bool=False, invitees: Optional[List[UserID]]=None, initial_state: Optional[InitialState]=None, room_version: str=None, creation_content: JSON=None, power_level_content_override: JSON=None) -> RoomID:
        """
        ...
        Args:
            ...
            power_level_content_override: initial power level configuration
        ...
        """
        content = {'visibility':visibility.value, 
         'is_direct':is_direct, 
         'preset':preset.value}
        if alias_localpart:
        ...
        if topic:
            content['topic'] = topic
        if initial_state:
            content['initial_state'] = [event.serialize() if isinstance(event, Serializable) else event for event in initial_state]
        if room_version:
            content['room_version'] = room_version
        if creation_content:
            content['creation_content'] = creation_content
        if power_level_content_override:
            content['power_level_content_override'] = power_level_content_override
    ...
    async def add_room_alias(self, room_id: RoomID, alias_localpart: str, override: bool=False) -> None:
        """
    ...
    async def join_room_by_id(self, room_id: RoomID, third_party_signed: JSON=None) -> RoomID:
        ...
        """
        content = await self.api.request(Method.POST, Path.rooms[room_id].join, {'third_party_signed': third_party_signed} if third_party_signed is not None else None)
        try:
            return content['room_id']
        except KeyError:
            raise MatrixResponseError('`room_id` not in response.')

    async def join_room(self, room_id_or_alias: Union[(RoomID, RoomAlias)], servers: List[str]=None, third_party_signed: JSON=None, max_retries: int=4) -> RoomID:
        """
        ...
        tries = 0
        content = {'third_party_signed': third_party_signed} if third_party_signed is not None else None
        query_params = {'servers': servers} if servers is not None else None
        while tries <= max_retries:
            try:
                content = await self.api.request((Method.POST), (Path.join[room_id_or_alias]), content=content,
                ...
                if tries <= max_retries:
                    wait = tries * 10
                    self.log.exception(f"Failed to join room {room_id_or_alias}, retrying in {wait} seconds...")
    ...
    async def invite_user(self, room_id: RoomID, user_id: UserID) -> None:
        ...
        """
        await self.api.request(Method.POST, Path.rooms[room_id].invite, {'user_id': user_id})
    ...
    async def kick_user(self, room_id: RoomID, user_id: UserID, reason: Optional[str]='') -> None:
        ...
        """
        await self.api.request(Method.POST, Path.rooms[room_id].kick, {'user_id':user_id, 
    ...
    async def ban_user(self, room_id: RoomID, user_id: UserID, reason: Optional[str]='') -> None:
        ...
        Args:
            ...
            reason: The reason the user has been kicked. This will be supplied as the ``reason`` on
                the target's updated `m.room.member`_ event.
        """
        await self.api.request(Method.POST, Path.rooms[room_id].ban, {'user_id':user_id, 
         'reason':reason or None})
    ...
    async def get_room_directory_visibility(self, room_id: RoomID) -> RoomDirectoryVisibility:
        ...
        try:
            ...
            raise MatrixResponseError(f"Invalid value for `visibility` in response: {resp['visibility']}")

    async def set_room_directory_visibility(self, room_id: RoomID, visibility: RoomDirectoryVisibility) -> None:
        """
        ...
        """
        await self.api.request(Method.PUT, Path.directory.list.room[room_id], {'visibility': visibility.value})

    async def get_room_directory(self, limit: Optional[int]=None, server: Optional[str]=None, since: Optional[DirectoryPaginationToken]=None, search_query: Optional[str]=None, include_all_networks: Optional[bool]=None, third_party_instance_id: Optional[str]=None) -> RoomDirectoryResponse:
        """
        ...
        if search_query is None:
            method = Method.GET if (include_all_networks is None and third_party_instance_id is None) else (Method.POST)
            content = {}
        ...
        content = await self.api.request(method, (Path.publicRooms), content, query_params=query_params)
