...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mautrix/client/api/events.py
# Compiled at: 2019-11-21 01:01:47
# Size of source mod 2**32: 24026 bytes
from typing import Awaitable, Dict, Optional, List
from ...api import Method, JSON, Path
from ...errors import MatrixResponseError
from .types import UserID, RoomID, EventID, FilterID, SyncToken, PaginationDirection, StateEvent, EventType, StateEventContent, MessageEventContent, Member, Event, ContentURI, PaginatedMessages, SerializerError, MessageType, RelatesTo, Format, ImageInfo, BaseFileInfo, TextMessageEventContent, MediaMessageEventContent, PresenceState, EventContent, ReactionEventContent, RelationType
...
class EventMethods(BaseClientAPI):
    ...
    def sync(self, since: SyncToken=None, timeout: int=30000, filter_id: FilterID=None, full_state: bool=False, set_presence: PresenceState=None) -> Awaitable[JSON]:
        """
    ...
    async def get_state_event(self, room_id: RoomID, event_type: EventType, state_key: Optional[str]=None) -> StateEventContent:
        """
        ...
        """
        content = await self.api.request(Method.GET, Path.rooms[room_id].state[event_type][state_key])
        try:
    ...
    async def get_joined_members(self, room_id: RoomID) -> Dict[(UserID, Member)]:
        ...
        try:
            return {user_id:Member.deserialize(event) for user_id, event in content['joined'].items()}
        except KeyError:
            raise MatrixResponseError('`joined` not in response.')
        except SerializerError as e:
            raise MatrixResponseError('Invalid member objects in response') from e

    async def get_messages(self, room_id: RoomID, direction: PaginationDirection, from_token: SyncToken, to_token: Optional[SyncToken]=None, limit: Optional[int]=None, filter_json: Optional[str]=None) -> PaginatedMessages:
        """
        ...
        content = await self.api.request((Method.GET), (Path.rooms[room_id].messages), query_params=query_params)
        try:
            return PaginatedMessages(content['start'], content['end'], [Event.deserialize(event) for event in content['chunk']])
        except KeyError:
    ...
    async def send_state_event(self, room_id: RoomID, event_type: EventType, content: StateEventContent, state_key: Optional[str]='', **kwargs) -> EventID:
        """
    ...
    async def send_message_event(self, room_id: RoomID, event_type: EventType, content: EventContent, **kwargs) -> EventID:
        """
    ...
    def send_message(self, room_id: RoomID, content: MessageEventContent, **kwargs) -> Awaitable[EventID]:
        """
    ...
    def react(self, room_id: RoomID, event_id: EventID, key: str) -> Awaitable[EventID]:
        content = ReactionEventContent(relates_to=RelatesTo(rel_type=(RelationType.ANNOTATION), event_id=event_id,
          key=key))
        return self.send_message_event(room_id, EventType.REACTION, content)

    def send_text(self, room_id: RoomID, text: str, html: Optional[str]=None, msgtype: MessageType=MessageType.TEXT, relates_to: Optional[RelatesTo]=None, **kwargs) -> Awaitable[EventID]:
        """
        ...
        if html:
            if not text:
                text = html
            content = TextMessageEventContent(msgtype=msgtype, body=text, format=(Format.HTML),
              formatted_body=html)
        else:
    ...
    def send_notice(self, room_id: RoomID, text: str, html: Optional[str]=None, relates_to: Optional[RelatesTo]=None, **kwargs) -> Awaitable[EventID]:
        """
    ...
    def send_emote(self, room_id: RoomID, text: str, html: Optional[str]=None, relates_to: Optional[RelatesTo]=None, **kwargs) -> Awaitable[EventID]:
        """
    ...
    def send_file(self, room_id: RoomID, url: ContentURI, info: Optional[BaseFileInfo]=None, file_name: str=None, file_type: MessageType=MessageType.FILE, relates_to: Optional[RelatesTo]=None, **kwargs) -> Awaitable[EventID]:
        """
        ...
        return (self.send_message)(room_id, 
         MediaMessageEventContent(url=url, info=info, body=file_name, relates_to=relates_to,
           msgtype=file_type), **kwargs)

    def send_sticker(self, room_id: RoomID, url: ContentURI, info: Optional[ImageInfo], text: Optional[str]='', relates_to: Optional[RelatesTo]=None, **kwargs) -> Awaitable[EventID]:
        """
        ...
        return (self.send_message_event)(room_id, (EventType.STICKER), 
         MediaMessageEventContent(url=url, info=info, body=text, relates_to=relates_to), **kwargs)

    def send_image(self, room_id: RoomID, url: ContentURI, info: Optional[ImageInfo]=None, file_name: str=None, relates_to: Optional[RelatesTo]=None, **kwargs) -> Awaitable[EventID]:
        """
        ...
        Returns:
            The ID of the event that was sent.
        """
        return (self.send_file)(room_id, url, info, file_name, (MessageType.IMAGE), relates_to, **kwargs)

    async def redact(self, room_id: RoomID, event_id: EventID, reason: Optional[str]='', **kwargs) -> EventID:
        """
