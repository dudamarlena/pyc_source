...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mautrix/client/client.py
# Compiled at: 2019-11-21 01:01:47
# Size of source mod 2**32: 13348 bytes
...
from .api.types import EventType, MessageEvent, StateEvent, StrippedStateEvent, Event, FilterID, Filter, TypingEvent
...
class Client(ClientAPI):
    __doc__ = 'Client is a high-level wrapper around the client API.'
    store: ClientStore
    ...
    def on(self, var: Union[(EventHandler, EventType, InternalEventType)]) -> Union[(EventHandler, Callable[([EventHandler], EventHandler)])]:
        """
    ...
    def add_event_handler(self, event_type: Union[(InternalEventType, EventType)], handler: EventHandler) -> None:
        """
    ...
    def remove_event_handler(self, event_type: Union[(EventType, InternalEventType)], handler: EventHandler) -> None:
        """
    ...
    async def dispatch_event(self, event: Event, source: SyncStream) -> None:
        ...
        elif event.type.is_state:
            if event.state_key is None:
                self.log.debug(f"Not sending {event.event_id} to handlers: expected state_key.")
                return
        else:
            if event.type.is_account_data:
                if not source & SyncStream.ACCOUNT_DATA:
                    self.log.debug(f"Not sending {event.event_id} to handlers: got account_data event type in non-account_data sync stream.")
                    return
            if event.type.is_ephemeral:
                if not source & SyncStream.EPHEMERAL:
                    self.log.debug(f"Not sending {event.event_id} to handlers: got ephemeral event type in non-ephemeral sync stream.")
                    return
        setattr(event, 'source', source)
        await self.dispatch_manual_event((event.type), event, include_global_handlers=True)

    async def dispatch_manual_event(self, event_type: Union[(EventType, InternalEventType)], data: Any, include_global_handlers: bool=False) -> None:
    ...
    def dispatch_internal_event(self, event_type: InternalEventType, **kwargs: Any) -> Awaitable[None]:
        kwargs['source'] = SyncStream.INTERNAL
        return self.dispatch_manual_event(event_type, kwargs, include_global_handlers=False)
    ...
    def handle_sync(self, data: JSON) -> None:
        ...
        for room_id, room_data in rooms.get('join', {}).items():
            for raw_event in room_data.get('state', {}).get('events', []):
                ...
                asyncio.ensure_future(self.dispatch_event((StateEvent.deserialize(raw_event)), source=(SyncStream.JOINED_ROOM | SyncStream.STATE)),
                  loop=(self.loop))

            for raw_event in room_data.get('timeline', {}).get('events', []):
                raw_event['room_id'] = room_id
                asyncio.ensure_future(self.dispatch_event((Event.deserialize(raw_event)), source=(SyncStream.JOINED_ROOM | SyncStream.TIMELINE)),
                  loop=(self.loop))

            for raw_event in room_data.get('ephemeral', {}).get('events', []):
                raw_event['room_id'] = room_id
                asyncio.ensure_future(self.dispatch_event((TypingEvent.deserialize(raw_event)), source=(SyncStream.EPHEMERAL)),
                  loop=(self.loop))

        for room_id, room_data in rooms.get('invite', {}).items():
            events = room_data.get('invite_state', {}).get('events', [])
            for raw_event in events:
                raw_event['room_id'] = room_id

            raw_invite = next(raw_event for raw_event in events if raw_event.get('type', '') == 'm.room.member' if raw_event.get('state_key', '') == self.mxid)
            ...
            invite.unsigned.invite_room_state = [StrippedStateEvent.deserialize(raw_event) for raw_event in events if raw_event != raw_invite]
            asyncio.ensure_future(self.dispatch_event(invite, source=(SyncStream.INVITED_ROOM | SyncStream.STATE)),
              loop=(self.loop))
        ...
        for room_id, room_data in rooms.get('leave', {}).items():
            for raw_event in room_data.get('timeline', {}).get('events', []):
                if 'state_key' in raw_event:
                    raw_event['room_id'] = room_id
                    asyncio.ensure_future(self.dispatch_event((StateEvent.deserialize(raw_event)), source=(SyncStream.LEFT_ROOM | SyncStream.TIMELINE)),
                      loop=(self.loop))
    ...
    async def _start(self, filter_id: Optional[FilterID]) -> None:
        ...
        while True:
            try:
                ...
                self.log.exception(f"Sync request errored, waiting {fail_sleep} seconds before continuing")
                await self.dispatch_internal_event((InternalEventType.SYNC_ERRORED), error=e, sleep_for=fail_sleep)
            ...
            data['net.maunium.mautrix'] = {'is_initial':is_initial, 
             'is_first':is_first}
