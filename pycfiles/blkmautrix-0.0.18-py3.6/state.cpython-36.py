...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mautrix/client/api/types/event/state.py
# Compiled at: 2019-11-21 01:01:47
# Size of source mod 2**32: 7340 bytes
...
class PowerLevelStateEventContent(SerializableAttrs['PowerLevelStateEventContent']):
    __doc__ = 'The content of a power level event.'
    users = attr.ib(default=(attr.Factory(dict)), metadata={'omitempty': False})
    users: Dict[(UserID, int)]
    users_default: int = 0
    events = attr.ib(default=(attr.Factory(dict)), metadata={'omitempty': False})
    ...
    def get_event_level(self, event_type: EventType) -> int:
        return self.events.get(event_type, self.state_default if event_type.is_state else self.events_default)
...
class Membership(SerializableEnum):
    __doc__ = 'A room membership state.'
    JOIN = 'join'
    LEAVE = 'leave'
    INVITE = 'invite'
    BAN = 'ban'
...
class MemberStateEventContent(SerializableAttrs['MemberStateEventContent']):
    __doc__ = 'The content of a membership event.'
...
class StrippedStateUnsigned(BaseUnsigned, SerializableAttrs['StrippedStateUnsigned']):
    __doc__ = 'Unsigned information sent with state events.'
...
class StrippedStateEvent(SerializableAttrs['StrippedStateEvent']):
    __doc__ = 'Stripped state events included with some invite events.'
    ...
    sender: UserID = None
    unsigned = None
    unsigned: Optional[StrippedStateUnsigned]

    @property
    def prev_content(self) -> StateEventContent:
        if self.unsigned:
            if self.unsigned.prev_content:
...
class StateUnsigned(StrippedStateUnsigned, SerializableAttrs['StateUnsigned']):
    ...
    invite_room_state: Optional[List[StrippedStateEvent]]


state_event_content_map = {EventType.ROOM_POWER_LEVELS: PowerLevelStateEventContent, 
...
class StateEvent(BaseRoomEvent, SerializableAttrs['StateEvent']):
    __doc__ = 'A room state event.'
    state_key: str
    content: StateEventContent
    unsigned = None
    unsigned: Optional[StateUnsigned]
