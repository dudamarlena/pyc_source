...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/frida/__init__.py
...
try:
    ...
    print(b'')
    print(b'***')
    if str(ex).startswith(b'No module named '):
        print(b'Frida native extension not found')
        print(b'Please check your PYTHONPATH.')
    else:
        print(b'Failed to load the Frida native extension: %s' % ex)
        if sys.version_info[0] == 2:
            current_python_version = b'%d.%d' % sys.version_info[:2]
        else:
            current_python_version = b'%d.x' % sys.version_info[0]
        print(b'Please ensure that the extension was compiled for Python ' + current_python_version + b'.')
    print(b'***')
    print(b'')
    raise ex
...
def get_local_device(**kwargs):
    return get_device_matching((lambda d: d.type == b'local'), timeout=0, **kwargs)


def get_remote_device(**kwargs):
    return get_device_matching((lambda d: d.type == b'remote'), timeout=0, **kwargs)


def get_usb_device(timeout=0, **kwargs):
    return get_device_matching((lambda d: d.type == b'usb'), timeout, **kwargs)
