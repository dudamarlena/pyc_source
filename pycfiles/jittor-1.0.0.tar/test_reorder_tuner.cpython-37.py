...
# [GCC 8.4.0]
# Embedded file name: /home/cjld/new_jittor/jittor/python/jittor/test/test_reorder_tuner.py
...
class TestReorderTuner(unittest.TestCase):
    ...
    def test(self):
        ...
        assert candidates == {'order0':[
          0], 
         'order1':[0, 1],  'order2':[0, 1, 2]}
    ...
    def test_with_split(self):
        ...
        with jt.log_capture_scope(log_v=0,
          ...
          compile_options={'split0':4, 
         'split1':4,  'split2':4,  'test_reorder_tuner':gid}) as (logs):
            b = a + a
            b.data
        logs = find_log_with_re(logs, 'Run tuner reorder: confidence\\((.*)\\) candidates\\((.*)\\)$')
        assert len(logs) == 1
        ...
        assert candidates == {'order0':[
          0], 
         'order1':[0, 1],  'order2':[0, 1, 2],  'order3':[
          0, 1, 2], 
         'order4':[0, 1, 2],  'order5':[0, 1, 2]}, candidates
    ...
    def test_searcher(self):
        ...
        with jt.log_capture_scope(log_v=0,
          ...
          jit_search_kernel=1,
          compile_options={'compile_shape':1, 
         'test_reorder_tuner':gid}) as (logs):
            b = a + a
            b.data
        ...
        assert best == {'compile_shape':1, 
         'order0':0,  'order1':0,  'order2':0,  'test_reorder_tuner':gid}
