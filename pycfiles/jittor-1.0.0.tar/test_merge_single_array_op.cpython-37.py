...
# [GCC 8.4.0]
# Embedded file name: /home/cjld/new_jittor/jittor/python/jittor/test/test_merge_single_array_op.py
# Compiled at: 2020-03-20 04:44:53
# Size of source mod 2**32: 4422 bytes
import unittest, jittor as jt, numpy as np, random
...
def test(shape, op1, op2):
    ...
    with jt.log_capture_scope(log_v=0, log_vprefix='fused_op.cc=100') as (logs):
        d__ = d.data
    logs = find_log_with_re(logs, 'Jit (fused )?op key (not )?found: \\[opkey0:array\\]\\[opkey1')
    assert len(logs) == 1
    a_ = a.data
    b_ = b.data
    d_ = op2(op1(a_, n), b_)
...
class TestSingleArray(unittest.TestCase):
    ...
    def test5(self):
        with jt.flag_scope(use_cuda=1):
            ...
            w = jt.make_var([x.shape[(-1)], 10], init=(lambda *a: ((jt.random)(*a) - f32(0.5)) / f32(x.shape[(-1)]) ** f32(0.5)))
    ...
    def test3(self):
        ...
        def check(shape1, shape2):
            ...
            if not (ja.shape == aa.shape and (ja == aa).all()):
                raise AssertionError(f"{ja}, {aa}")
