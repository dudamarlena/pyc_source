...
# [GCC 8.4.0]
# Embedded file name: /home/cjld/new_jittor/jittor/python/jittor/test/test_atomic_tuner.py
...
class TestAtomicTunerClass(unittest.TestCase):
    ...
    def check(self, model, std_log):
        ...
        with jt.log_capture_scope(log_silent=1,
          log_v=0,
          log_vprefix='atomic_tuner_pass.cc=100') as (logs):
            y = model(x).numpy()
        with jt.log_capture_scope(log_v=0,
          exclude_pass='atomic') as (logs2):
            y_std = model(x).numpy()
