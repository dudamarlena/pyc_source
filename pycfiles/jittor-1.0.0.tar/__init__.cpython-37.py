...
# [GCC 8.4.0]
# Embedded file name: /home/cjld/new_jittor/jittor/python/jittor_utils/__init__.py
# Compiled at: 2020-04-11 06:08:16
# Size of source mod 2**32: 7130 bytes
from multiprocessing import Pool
import subprocess as sp, os, re, sys, inspect, datetime, contextlib, threading, time
...
class LogWarper:
    ...
    def V(self, verbose, *msg):
        (self._log)('i', verbose, *msg)
...
def find_cache_path():
    ...
    try:
        if 'cache_name' in os.environ:
            ...
            r = sp.run(['git', 'branch'], cwd=(os.path.dirname(__file__)), stdout=(sp.PIPE), stderr=(sp.PIPE))
...
def get_cc_type(cc_path):
    ...
    if 'clang' in bname:
        return 'clang'
    if 'icc' in bname or 'icpc' in bname:
        return 'icc'
    if 'g++' in bname:
        return 'g++'
