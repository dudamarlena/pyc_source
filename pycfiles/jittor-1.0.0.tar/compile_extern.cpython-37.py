...
# [GCC 8.4.0]
# Embedded file name: /home/cjld/new_jittor/jittor/python/jittor/compile_extern.py
# Compiled at: 2020-04-15 04:17:02
...
def install_mkl(root_folder):
    ...
    if not os.path.isfile(os.path.join(dirname, 'examples', 'test')):
        ...
        assert 0 == os.system(f"cd {dirname}/examples && {cc_path} -std=c++14 cpu_cnn_inference_f32.cpp -Ofast -lmkldnn -I ../include -L ../lib -o test && LD_LIBRARY_PATH=../lib/ ./test")
...
def setup_mkl():
    ...
    mkl_ops = compile_custom_ops(mkl_op_files, extra_flags=f" -I'{mkl_include_path}' -L'{mkl_lib_path}' -lmkldnn -Wl,-rpath='{mkl_lib_path}' ")
...
def install_cub(root_folder):
    ...
    if not os.path.isfile(os.path.join(dirname, 'examples', 'test')):
        ...
        with tarfile.open(fullname, 'r') as (tar):
            tar.extractall(root_folder)
        assert 0 == os.system(f"cd {dirname}/examples && {nvcc_path} device/example_device_radix_sort.cu -O2 -I.. -o test && ./test")
    return dirname
...
def setup_cuda_lib(lib_name, link=True, extra_flags=''):
    ...
    if len(culib_src_files) == 0:
        return
    culib_ops = compile_custom_ops(culib_src_files, extra_flags=f" -I'{jt_cuda_include}' -I'{jt_culib_include}' {link_flags} {extra_flags} ")
...
def setup_nccl():
    ...
    nccl_ops = compile_custom_ops(nccl_src_files, extra_flags=f" -I'{nccl_include_path}' {mpi_compile_flags} ")
...
def setup_mpi():
    global has_mpi
    global mpi
    global mpi_compile_flags
    global mpi_flags
    global mpi_link_flags
    global mpi_ops
    global mpicc_path
    global use_mpi
    ...
    mpi = compile_custom_ops(mpi_src_files, extra_flags=f" {mpi_flags} ",
      return_module=True,
