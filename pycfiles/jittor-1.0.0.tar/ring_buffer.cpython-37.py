...
# [GCC 8.4.0]
# Embedded file name: /home/cjld/new_jittor/jittor/python/jittor_utils/ring_buffer.py
# Compiled at: 2020-03-29 04:52:06
# Size of source mod 2**32: 8451 bytes
import multiprocessing as mp, numpy as np, ctypes, random, pickle, ctypes
...
class RingBuffer:
    ...
    def send_raw(self, data):
        ...
        with self.allocator.lock:
            ...
            window = np.ndarray(shape=(data.shape), dtype=(data.dtype), buffer=(self.buffer),
              offset=location)
            window[:] = data
    ...
    def recv_raw(self, nbytes, shape, dtype):
        with self.allocator.lock:
            ...
            data = np.ndarray(shape=shape, dtype=dtype, buffer=(self.buffer),
              offset=location).copy()
            self.allocator.cv.notify()
