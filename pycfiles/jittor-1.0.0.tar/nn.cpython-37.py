...
# [GCC 8.4.0]
# Embedded file name: /home/cjld/new_jittor/jittor/python/jittor/nn.py
# Compiled at: 2020-04-16 03:46:57
# Size of source mod 2**32: 15949 bytes
import jittor as jt
...
def batch_norm(x, is_train, eps=1e-05, momentum=0.1):
...
def cross_entropy_loss(output, target, ignore_index=None):
    ...
    if ignore_index is not None:
        target = jt.ternary(target == ignore_index, jt.array(-1).broadcast(target), target)
...
class Adam(object):
    ...
    def __init__(self, parameters, lr, eps=1e-08, betas=(0.9, 0.999), weight_decay=0):
        self.lr = lr
        self.eps = eps
        self.betas = betas
...
class Dropout(Module):
    ...
    def __init__(self, p=0.5, is_train=False):
        if not (p >= 0 and p <= 1):
            raise AssertionError('dropout probability has to be between 0 and 1, but got {}'.format(p))
        self.p = p
...
class Conv(Module):
    ...
    def execute(self, x):
        ...
        xx = x.reindex([N, self.out_channels, C, oh, ow, Kh, Kw], [
         'i0',
         'i2',
         f"i3*{self.stride[0]}-{self.padding[0]}+i5*{self.dilation[0]}",
         f"i4*{self.stride[1]}-{self.padding[1]}+i6*{self.dilation[1]}"])
...
class ConvTranspose(Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, output_padding=0, groups=1, bias=True, dilation=1):
        ...
        self.group = groups
        if not groups == 1:
            raise AssertionError('Group conv not supported yet.')
        else:
            self.kernel_size = kernel_size if isinstance(kernel_size, tuple) else (kernel_size, kernel_size)
            self.stride = stride if isinstance(stride, tuple) else (stride, stride)
            self.dilation = dilation if isinstance(dilation, tuple) else (dilation, dilation)
    ...
    def execute(self, x):
        ...
        shape = (N, i, o, H, W, h, w)
        xx = x.broadcast(shape, (2, 5, 6))
        ww = self.weight.broadcast(shape, (0, 3, 4))
        y = (ww * xx).reindex_reduce('add', out_shape, [
         'i0',
         'i2',
         f"i3*{stride_h}-{padding_h}+i5*{dilation_h}",
         f"i4*{stride_w}-{padding_w}+i6*{dilation_w}"])
        if self.bias is not None:
            b = self.bias.broadcast(y.shape, [0, 2, 3])
            y = y + b
        return y
