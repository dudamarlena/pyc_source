...
# [GCC 8.4.0]
# Embedded file name: /home/cjld/new_jittor/jittor/python/jittor/test/test_fused_op.py
# Compiled at: 2020-03-20 04:44:53
# Size of source mod 2**32: 12248 bytes
import os
...
class TestFusedOp(unittest.TestCase):
    ...
    def test_fuse_reduce2(self):
        ...
        v = a.data[0]
        if not (np.allclose(v * 10, b.data) and v == c.data and v == d.data):
            raise AssertionError((v, b.data, c.data, d.data))
    ...
    def test_profile_fused_op_split(self):
        ...
        def check(n, m, k, cs, rs, rtp):
            ...
            b = jt.random([1, m, k])
            (a.data, b.data)
            with performance_test_scope(20,
              (20000000000 // (n * m * k)), compile_options={'split0':16, 
             'split1':6,  'split2':16,  'order0':0, 
             'order1':1,  'order2':1,  'order3':0, 
             'order4':0,  'order5':0,  'restride':rs, 
             'unroll':2,  'vectorize':2,  'compile_shapes':cs}) as (report):
                c = (a * b).sum(1)
            c.data
            ...
            assert (np.abs(nc - c.data) < 0.01).all(), np.abs(nc - c.data).max()
            tp = float(report[(-1)][(-1)])
            assert tp > rtp * 1000000000, (tp, rtp)
    ...
    def test_profile_fused_op_restride(self):
        ...
        def check(n, m, k, cs, rs, pa, rtp):
            ...
            b = jt.random([1, m, k])
            (a.data, b.data)
            with performance_test_scope(0,
              (20000000000 // (n * m * k)), compile_options={'order0':0, 
             'order1':0,  'order2':0,  'split0':64, 
             'split1':60,  'split2':64,  'split3':16, 
             'split4':6,  'split5':16,  'order3':0, 
             'order4':1,  'order5':1,  'order6':0, 
             'order7':0,  'order8':0,  'restride':rs, 
             'vectorize':2,  'unroll':2,  'compile_shapes':cs, 
             'parallel':pa}) as (report):
                c = (a * b).sum(1)
            c.sync()
            na = a.data.reshape((n, m))
            nb = b.data.reshape((m, k))
        ...
        check(64, 60, 64, 0, 0, 0, 31)
        check(64, 60, 64, 0, 1, 0, 25)
        check(64, 60, 64, 1, 0, 0, 42)
        check(3520, 3300, 3520, 0, 0, 0, 20)
        check(3520, 3300, 3520, 1, 1, 0, 37)
        check(3520, 3300, 3520, 0, 1, 0, 36)
        check(3521, 3301, 3520, 0, 1, 0, 36)
        check(3521, 3301, 3521, 0, 1, 0, 36)
        check(3535, 3315, 3535, 0, 1, 0, 34)
        check(1024, 960, 1024, 0, 1, 0, 35)
        check(3520, 3300, 3520, 0, 1, 0, 36)

    @unittest.skipIf(skip_slow_test, 'Skip slow test')
    def test_profile_fused_op_split3(self):
        n, m, k = (6400, 6000, 6400)
        a = jt.random([n, m, 1])
        b = jt.random([1, m, k])
        (
         a.data, b.data)
        with performance_test_scope(0,
          (400000000000 // (n * m * k)), compile_options={'order0':0, 
         'order1':0,  'order2':0,  'split0':256, 
         'split1':240,  'split2':256,  'order3':0, 
         'order4':1,  'order5':1,  'split3':64, 
         'split4':60,  'split5':64,  'order6':0, 
         'order7':1,  'order8':1,  'split6':16, 
         'split7':6,  'split8':16,  'order9':0, 
         'order10':0,  'order11':0,  'restride':1, 
         'unroll':2,  'vectorize':2}):
            c = (a * b).sum(1)
        c.data
        na = a.data.reshape((n, m))
        nb = b.data.reshape((m, k))
    ...
    def test_profile_fused_op_parallel(self):
        ...
        def check(n, m, k, cs, rs, pa, rtp):
            ...
            b = jt.random([1, m, k])
            (a.data, b.data)
            with performance_test_scope(2,
              30, compile_options={'order0':0, 
             'order1':0,  'order2':0,  'split0':64, 
             'split1':60,  'split2':64,  'split3':16, 
             'split4':6,  'split5':16,  'order3':0, 
             'order4':1,  'order5':1,  'order6':0, 
        ...
        check(1024, 960, 1024, 1, 1, 0, 35)
        check(1024, 960, 1024, 1, 1, 1, 60)
        check(1024, 960, 1024, 0, 1, 0, 35)
        check(1024, 960, 1024, 0, 1, 1, 60)
        check(1029, 965, 1029, 0, 1, 1, 50)
