...
# [GCC 8.4.0]
# Embedded file name: /home/cjld/new_jittor/jittor/python/jittor/test/test_parallel_pass.py
# Compiled at: 2020-03-20 04:44:53
# Size of source mod 2**32: 6503 bytes
...
class SimpleAsmParser:
    ...
    def count_instructions(self, func_name, ins_name):
        ...
        assert f is not None, f"function {func_name} not found"
        count = 0
        for ins in f:
            if ins_name in ins:
                count += 1
...
class TestParallelPass(unittest.TestCase):
    ...
    def check(self, use_int32):
        ...
        b = jt.random((n, n))
        (a.data, b.data)
        with jt.profile_scope(compile_options={'compile_shapes':1, 
         'parallel':1},
    ...
    def test_int32_align(self):
        ...
        if jt.flags.cc_type == 'clang':
            raise ca > 1 and cu <= 1 or AssertionError((ca, cu))
...
class TestParallelPass3(unittest.TestCase):
    ...
    def reduce_check(self, ndim, depth, tdim, rdim, has_atomic, order=[], split=[], **args):
        ...
        with jt.profile_scope(compile_options=config,
