...
# [GCC 8.4.0]
# Embedded file name: /home/cjld/new_jittor/jittor/python/jittor/test/test_asm_tuner.py
...
class TestAsmTuner(unittest.TestCase):
    ...
    def check_cc(self, content, check_movnt):
        ...
        with open(self.src_path, 'w') as (f):
            f.write(content)
        cmd = jt.flags.python_path + ' ' + jt.flags.jittor_path + '/utils/asm_tuner.py --cc_path=' + jt.flags.cc_path + " '" + self.src_path + "'" + ' -DJIT -DJIT_cpu ' + jt.flags.cc_flags + " -o '" + self.so_path + "'"
        self.run_cmd(cmd)
