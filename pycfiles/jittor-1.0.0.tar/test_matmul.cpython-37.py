...
# [GCC 8.4.0]
# Embedded file name: /home/cjld/new_jittor/jittor/python/jittor/test/test_matmul.py
...
@jt.var_scope('linear')
def linear(x, n):
    w = jt.make_var([x.shape[(-1)], n], init=(lambda *a: ((jt.random)(*a) - f32(0.5)) / f32(x.shape[(-1)]) ** f32(0.5)))
...
def test_matmul(s1, s2):
    ...
    logs = find_log_with_re(logs, 'Jit op key (not )?found: (mkl)|(cublas)_matmul.*')
    assert len(logs) == 1


def matmul2(a, b, tp):
    if len(a.shape) >= 2:
        raise len(b.shape) == 2 or AssertionError
    elif tp == 0:
        shape = [
...
def test_matmul2(s1, s2, t1, t2, dtype='float32'):
    if not t1:
        ...
        with jt.log_capture_scope(log_v=0, log_vprefix='op.cc=100') as (logs):
            c__ = c.data
        if not np.allclose(c_, c__):
            raise AssertionError
    else:
        logs = find_log_with_re(logs, 'Jit op key (not )?found: (mkl)|(cublas)_matmul.*')
        if dtype.startswith('float') and not jt.flags.use_cuda:
            if dtype == 'float32':
                if not len(logs) == 1:
                    raise AssertionError
...
class TestMatmul(unittest.TestCase):
    ...
    def test_backward(self):
        ...
        assert abs(loss_mean.data - result) < 1e-06, [loss_mean.data, result]
        jt.clean()
    ...
    def test_backward_once(self):
        ...
        for i, (x, y) in enumerate(get_data(n)):
            ...
            with jt.log_capture_scope(log_v=0, log_vprefix='op.cc=100') as (logs):
                jt.sync_all()
            logs = find_log_with_re(logs, 'Jit op key (not )?found: (mkl)_matmul.*')
            assert len(logs) == 1
            with jt.log_capture_scope(log_silent=1, log_v=0, log_vprefix='op.cc=100,exe=1000') as (logs_b):
                ...
                jt.sync_all()
            logs_b = find_log_with_re(logs_b, 'Jit op key (not )?found: (mkl)_matmul.*')
    ...
    def test_backward_cuda(self):
        with jt.flag_scope(use_cuda=1):
            ...
            result = 0.00038617782411165535
            assert abs(loss_mean.data - result) < 1e-06
            jt.clean()
    ...
    def test_backward_once_cuda(self):
        with jt.flag_scope(use_cuda=1):
            ...
            for i, (x, y) in enumerate(get_data(n)):
                ...
                with jt.log_capture_scope(log_v=0, log_vprefix='op.cc=100') as (logs):
                    jt.sync_all()
                logs = find_log_with_re(logs, 'Jit op key (not )?found: (cublas)_matmul.*')
                assert len(logs) == 1
                with jt.log_capture_scope(log_silent=1, log_v=0, log_vprefix='op.cc=100,exe=1000') as (logs_b):
                    gs = jt.grad(pred_y, x)
                    gs2 = jt.grad(pred_y, model.linear1.weight)
                    jt.sync_all()
                logs_b = find_log_with_re(logs_b, 'Jit op key (not )?found: (cublas)_matmul.*')
                assert len(logs_b) == 2, len(logs_b)
