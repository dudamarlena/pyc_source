...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/tor_access/__init__.py
# Compiled at: 2016-06-21 23:55:20
"""
    ...
    def require(self, **kwargs):
        ...
        def actual(handler):
            if not issubclass(handler, tornado.web.RequestHandler):
                ...
                ACL[category] = ACL.get(category, None) or {}
...
def check_access(handler, roleneed):
    if isinstance(roleneed, MasterRoleNeed):
        ...
        if handler.__needcheck__.get('url', None):
            if checkname not in roleneed.nodes:
                raise HTTPError(403)
        ...
        if ctx_params:
            for ctx_param in ctx_params.split(','):
                ...
                if ctx_val not in roleneed.ctx_vals.get(ctx_param, ()):
                    raise HTTPError(403)
