...
# [GCC 8.4.0]
# Embedded file name: /Users/nweir/code/cosmiq_repos/solaris/solaris/data/coco.py
# Compiled at: 2020-04-08 16:03:55
...
import json, os, pandas as pd, geopandas as gpd, logging

def geojson2coco(image_src, label_src, output_path=None, image_ext='.tif', matching_re=None, category_attribute=None, score_attribute=None, preset_categories=None, include_other=True, info_dict=None, license_dict=None, recursive=False, override_crs=False, explode_all_multipolygons=False, remove_all_multipolygons=False, verbose=0):
    ...
    if isinstance(image_src, str):
        ...
        if isinstance(image_src, dict):
            ...
            if 'images' in image_src.keys():
                ...
                image_ref = image_src
            image_ref = {image['file_name']:image['id'] for image in image_ref}
        else:
            logger.debug('Non-COCO formatted image set provided. Generating image fname:id dict with arbitrary ID integers.')
            image_list = _get_fname_list(image_src, recursive=recursive, extension=image_ext)
            image_ref = dict(zip(image_list, list(range(1, len(image_list) + 1))))
    logger.debug('Preparing label filename list.')
    label_list = _get_fname_list(label_src, recursive=recursive, extension='json')
    ...
    if do_matches:
        ...
        if matching_re is not None:
            ...
            logger.debug('matching_re is none, getting full filenames without extensions for matching.')
            im_names['match_substr'] = im_names['image_fname'].apply(lambda x: os.path.splitext(os.path.split(x)[1])[0])
            im_names['match_substr'] = im_names['match_substr'].astype(str)
            label_names['match_substr'] = label_names['label_fname'].apply(lambda x: os.path.splitext(os.path.split(x)[1])[0])
            label_names['match_substr'] = label_names['match_substr'].astype(str)
        ...
        for gj in tqdm(label_list):
            ...
            if remove_all_multipolygons is True:
                if explode_all_multipolygons is True:
                    ...
                    if do_matches:
                        ...
                        if len(image_ref) == 1:
                            if len(label_list) == 1:
                                ...
                                logger.debug('Converting to pixel coordinates.')
                                curr_gdf = geojson_to_px_gdf(curr_gdf, override_crs=override_crs,
                            ...
                            curr_gdf = curr_gdf[['image_id', 'label_fname', 'category_str',
                             'geometry']]
                label_df = pd.concat([label_df, curr_gdf], axis='index', ignore_index=True,
                  sort=False)

        logger.info('Finished loading labels.')
        logger.info('Generating COCO-formatted annotations.')
        coco_dataset = df_to_coco_annos(label_df, geom_col='geometry',
        ...
        if license_dict is not None:
            ...
            for license_name, license_url in license_dict.items():
                coco_licenses.append({'name':license_name,  'url':license_url, 
                 'id':license_idx})
                license_idx += 1
...
def df_to_coco_annos(df, output_path=None, geom_col='geometry', image_id_col=None, category_col=None, score_col=None, preset_categories=None, supercategory_col=None, include_other=True, starting_id=1, verbose=0):
    ...
    if preset_categories is not None and category_col is None:
        ...
        if preset_categories is not None:
            if category_col is not None:
                ...
                include_other or logger.info('Filtering out objects not contained in  preset_categories')
                temp_df = temp_df.loc[temp_df[category_col].isin(category_names), :]
            else:
                logger.info('Setting category to "other" for objects outside of preset category list.')
                ...
                if 'other' not in category_dict.keys():
                    ...
                    preset_categories.append({'id':other_id,  'name':'other', 
                     'supercategory':'other'})
        else:
            ...
            temp_df['annotation_id'] = list(range(starting_id, starting_id + len(temp_df)))
            ...
            def _row_to_coco(row, geom_col, category_id_col, image_id_col, score_col):
                ...
                if score_col is None:
                    return {'id':row['annotation_id'],  'image_id':int(row[image_id_col]), 
                     'category_id':int(row[category_id_col]), 
                     'segmentation':[
                      polygon_to_coco(row[geom_col])], 
                     'area':row['area'], 
                     'bbox':row['bbox'], 
                     'iscrowd':0}
                return {'id':row['annotation_id'],  'image_id':int(row[image_id_col]), 
            ...
            coco_annotations = temp_df.apply(_row_to_coco, axis=1, geom_col=geom_col, category_id_col='category_id',
              image_id_col=image_id_col,
              score_col=score_col).tolist()
            coco_categories = coco_categories_dict_from_df(temp_df,
              category_id_col='category_id', category_name_col=category_col,
...
def _coco_category_name_id_dict_from_list(category_list):
    ...
    category_dict = {category['name']:category['id'] for category in category_list}
