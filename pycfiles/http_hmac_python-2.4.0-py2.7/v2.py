...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/httphmac/v2.py
# Compiled at: 2016-04-23 12:10:32
from .base_signer import BaseSigner, BaseResponseSigner
import base64, collections, hashlib, hmac, re, time
try:
...
class V2Signer(BaseSigner):
    ...
    def parse_auth_headers(self, authorization):
        ...
        """
        matches = re.findall('(\\w+)="(.*?)"', authorization)
        return dict(matches)
    ...
    def matches(self, header):
        ...
        """
        if re.match('(?i)^\\s*acquia-http-hmac.*?version=\\"2\\.0\\".*?$', header) is not None:
            return True
        else:
            return False
