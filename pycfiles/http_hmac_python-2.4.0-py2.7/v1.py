...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/httphmac/v1.py
# Compiled at: 2016-01-13 06:35:32
...
class V1Signer(BaseSigner):
    ...
    def matches(self, header):
        ...
        """
        if re.match('(?i)^\\s*Acquia\\s*[^:]+\\s*:\\s*[0-9a-zA-Z\\\\+/=]+\\s*$', header) is not None:
            return True
        else:
            return False
    ...
    def parse_auth_headers(self, authorization):
        ...
        """
        m = re.match('^(?i)Acquia\\s+(.*?):(.+)$', authorization)
        if m is not None:
