...
# [GCC 8.4.0]
# Embedded file name: /home/ungarj/virtualenvs/mapchete/lib/python3.5/site-packages/mapchete/processes/contours.py
...
def execute(mp, resampling='nearest', interval=100, field='elev', base=0, td_matching_method='gdal', td_matching_max_zoom=None, td_matching_precision=8, td_fallback_to_higher_zoom=False, clip_pixelbuffer=0, **kwargs):
    """
    ...
    with mp.open('dem') as (dem):
        ...
        dem_data = dem.read(resampling=resampling, matching_method=td_matching_method, matching_max_zoom=td_matching_max_zoom, matching_precision=td_matching_precision, fallback_to_higher_zoom=td_fallback_to_higher_zoom)
        if dem_data.mask.all():
            logger.debug('raster empty')
            return 'empty'
    logger.debug('calculate hillshade')
    contours = mp.contours(dem_data, interval=interval, field=field, base=base)
    if clip_geom:
        logger.debug('clipping output with geometry')
        clip_geom = mp.tile.bbox.difference(unary_union([shape(i['geometry']) for i in clip_geom]).buffer(clip_pixelbuffer * mp.tile.pixel_x_size))
        out_contours = []
        for contour in contours:
            out_geom = shape(contour['geometry']).intersection(clip_geom)
            if not out_geom.is_empty:
                out_contours.append(dict(contour, geometry=mapping(out_geom)))
