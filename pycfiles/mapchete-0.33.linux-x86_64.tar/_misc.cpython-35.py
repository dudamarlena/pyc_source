...
# [GCC 8.4.0]
# Embedded file name: /home/ungarj/virtualenvs/mapchete/lib/python3.5/site-packages/mapchete/io/_misc.py
# Compiled at: 2019-10-03 11:49:00
...
GDAL_HTTP_OPTS = dict(GDAL_DISABLE_READDIR_ON_OPEN=True, CPL_VSIL_CURL_ALLOWED_EXTENSIONS='.tif, .ovr, .jp2, .png, .xml', GDAL_HTTP_TIMEOUT=30, GDAL_HTTP_MAX_RETRY=3, GDAL_HTTP_RETRY_DELAY=5)
...
def get_best_zoom_level(input_file, tile_pyramid_type):
    ...
    with rasterio.open(input_file, 'r') as (src):
        xmin, ymin, xmax, ymax = reproject_geometry(segmentize_geometry(box(src.bounds.left, src.bounds.bottom, src.bounds.right, src.bounds.top), get_segmentize_value(input_file, tile_pyramid)), src_crs=src.crs, dst_crs=tile_pyramid.crs).bounds
        x_dif = xmax - xmin
        y_dif = ymax - ymin
        size = float(src.width + src.height)
        avg_resolution = x_dif / float(src.width) * (float(src.width) / size) + y_dif / float(src.height) * (float(src.height) / size)
...
def tile_to_zoom_level(tile, dst_pyramid=None, matching_method='gdal', precision=8):
    ...
    def width_height(bounds):
        try:
            l, b, r, t = reproject_geometry(box(*bounds), src_crs=tile.crs, dst_crs=dst_pyramid.crs).bounds
        except ValueError:
...
def get_boto3_bucket(bucket_name):
    ...
    return boto3.resource('s3', endpoint_url='https://' + url if url and not url.startswith(('http://',
                                                                                             'https://')) else url).Bucket(bucket_name)
