...
# [GCC 8.4.0]
# Embedded file name: /home/ungarj/virtualenvs/mapchete/lib/python3.5/site-packages/mapchete/index.py
# Compiled at: 2019-12-28 05:27:02
# Size of source mod 2**32: 18799 bytes
"""
...
import fiona, logging, operator, os
from rasterio.dtypes import _gdal_typename
from shapely.geometry import mapping
import xml.etree.ElementTree as ET
from xml.dom import minidom
from mapchete.config import get_zoom_levels
from mapchete.io import path_exists, path_is_remote, get_boto3_bucket, raster, relative_path
logger = logging.getLogger(__name__)
spatial_schema = {'geometry': 'Polygon', 
 'properties': {'tile_id': 'str:254', 
                'zoom': 'int', 
                'row': 'int', 
                'col': 'int'}}

def zoom_index_gen(mp=None, out_dir=None, zoom=None, geojson=False, gpkg=False, shapefile=False, txt=False, vrt=False, fieldname='location', basepath=None, for_gdal=True, threading=False):
    """
    ...
    for zoom in get_zoom_levels(process_zoom_levels=zoom):
        with ExitStack() as (es):
            index_writers = []
            if geojson:
                index_writers.append(es.enter_context(VectorFileWriter(driver='GeoJSON', out_path=_index_file_path(out_dir, zoom, 'geojson'), crs=mp.config.output_pyramid.crs, fieldname=fieldname)))
            if gpkg:
                index_writers.append(es.enter_context(VectorFileWriter(driver='GPKG', out_path=_index_file_path(out_dir, zoom, 'gpkg'), crs=mp.config.output_pyramid.crs, fieldname=fieldname)))
            if shapefile:
                index_writers.append(es.enter_context(VectorFileWriter(driver='ESRI Shapefile', out_path=_index_file_path(out_dir, zoom, 'shp'), crs=mp.config.output_pyramid.crs, fieldname=fieldname)))
            if txt:
                index_writers.append(es.enter_context(TextFileWriter(out_path=_index_file_path(out_dir, zoom, 'txt'))))
            if vrt:
                index_writers.append(es.enter_context(VRTFileWriter(out_path=_index_file_path(out_dir, zoom, 'vrt'), output=mp.config.output, out_pyramid=mp.config.output_pyramid)))
            logger.debug('use the following index writers: %s', index_writers)

            def _worker(tile):
                tile_path = _tile_path(orig_path=mp.config.output.get_path(tile), basepath=basepath, for_gdal=for_gdal)
                indexes = [i for i in index_writers if not i.entry_exists(tile=tile, path=tile_path)]
                if indexes:
                    output_exists = mp.config.output.tiles_exist(output_tile=tile)
                else:
                    output_exists = None
                return (
                 tile, tile_path, indexes, output_exists)

            with concurrent.futures.ThreadPoolExecutor() as (executor):
                for task in concurrent.futures.as_completed(executor.submit(_worker, i) for i in mp.config.output_pyramid.tiles_from_geom(mp.config.area_at_zoom(zoom), zoom)):
...
def _tile_path(orig_path=None, basepath=None, for_gdal=True):
    path = os.path.join(basepath, '/'.join(orig_path.split('/')[-3:])) if basepath else orig_path
    if for_gdal and path.startswith(('http://', 'https://')):
        return '/vsicurl/' + path
    else:
        if for_gdal and path.startswith('s3://'):
            return path.replace('s3://', '/vsis3/')
        return path


class VectorFileWriter:
    __doc__ = 'Writes GeoJSON or GeoPackage files.'
    ...
    def __init__(self, out_path=None, crs=None, fieldname=None, driver=None):
        ...
        if self._append:
            if os.path.isfile(self.path):
                ...
                self.sink = fiona.open(self.path, 'w', driver=self.driver, crs=crs, schema=schema)
                self._existing = {}
        else:
            if os.path.isfile(self.path):
                logger.debug('read existing entries')
    ...
    def write(self, tile, path):
        if not self.entry_exists(tile=tile):
            ...
            self.sink.write({'geometry': mapping(tile.bbox), 
             'properties': {'tile_id': str(tile.id), 
                            'zoom': str(tile.zoom), 
                            'row': str(tile.row), 
                            'col': str(tile.col), 
                            self.fieldname: path}})
...
class TextFileWriter:
    __doc__ = 'Writes tile paths into text file.'
...
class VRTFileWriter:
    __doc__ = 'Generates GDAL-style VRT file.'
    ...
    def __init__(self, out_path=None, output=None, out_pyramid=None):
        ...
        if path_exists(self.path):
            if self._bucket:
                ...
                for obj in self.bucket_resource.objects.filter(Prefix=key):
                    if obj.key == key:
                        self._existing = {k:v for k, v in self._xml_to_entries(obj.get()['Body'].read().decode())}
    ...
    def _xml_to_entries(self, xml_string):
        for entry in next(ET.ElementTree(ET.fromstring(xml_string)).getroot().iter('VRTRasterBand')).iter('ComplexSource'):
