...
# [GCC 8.4.0]
# Embedded file name: /home/ungarj/virtualenvs/mapchete/lib/python3.5/site-packages/mapchete/formats/default/gtiff.py
# Compiled at: 2020-03-05 08:05:21
# Size of source mod 2**32: 24121 bytes
"""
...
import logging, math, numpy as np, numpy.ma as ma, os, rasterio
...
from mapchete.io.raster import write_raster_window, prepare_array, memory_file, read_raster_no_crs, extract_from_array, read_raster_window
from mapchete.tile import BufferedTile
from mapchete.validate import deprecated_kwargs
logger = logging.getLogger(__name__)
METADATA = {'driver_name': 'GTiff', 
 'data_type': 'raster', 
 'mode': 'rw'}
GTIFF_DEFAULT_PROFILE = {'blockysize': 256, 
 'blockxsize': 256, 
 'tiled': True, 
 'dtype': 'uint8', 
 'compress': 'lzw', 
...
class GTiffOutputReaderFunctions:
    ...
    def empty(self, process_tile):
        ...
        return ma.masked_array(data=np.full((
         profile['count'],) + process_tile.shape, profile['nodata'], dtype=profile['dtype']), mask=True)
    ...
    def for_web(self, data):
        ...
        """
        return (
         memory_file(prepare_array(data, masked=True, nodata=self.output_params['nodata'], dtype=self.profile()['dtype']), self.profile()),
         'image/tiff')
    ...
    def _set_attributes(self, output_params):
        self.path = output_params['path']
        self.file_extension = '.tif'
        self.output_params = dict(output_params, nodata=output_params.get('nodata', GTIFF_DEFAULT_PROFILE['nodata']))
        self._bucket = self.path.split('/')[2] if self.path.startswith('s3://') else None


class GTiffTileDirectoryOutputReader(GTiffOutputReaderFunctions, base.TileDirectoryOutputReader):
    ...
    def empty(self, process_tile):
        ...
        profile = self.profile(process_tile)
        return ma.masked_array(data=np.full((
         profile['count'],) + process_tile.shape, profile['nodata'], dtype=profile['dtype']), mask=True, fill_value=profile['nodata'])
    ...
    def profile(self, tile=None):
        ...
        """
        dst_metadata = dict(GTIFF_DEFAULT_PROFILE, count=self.output_params['bands'], dtype=self.output_params['dtype'], driver='GTiff', nodata=self.output_params['nodata'])
        dst_metadata.pop('transform', None)
        if tile is not None:
            dst_metadata.update(crs=tile.crs, width=tile.width, height=tile.height, affine=tile.affine)
        else:
        ...
        try:
            if 'compression' in self.output_params:
                warnings.warn(DeprecationWarning("use 'compress' instead of 'compression'"))
...
class GTiffTileDirectoryOutputWriter(GTiffTileDirectoryOutputReader, base.TileDirectoryOutputWriter):
    ...
    def write(self, process_tile, data):
        ...
        if isinstance(data, tuple) and len(data) == 2 and isinstance(data[1], dict):
            ...
            tags = {}
        data = prepare_array(data, masked=True, nodata=self.output_params['nodata'], dtype=self.profile(process_tile)['dtype'])
        if data.mask.all():
            ...
            for tile in self.pyramid.intersecting(process_tile):
                ...
                write_raster_window(in_tile=process_tile, in_data=data, out_profile=self.profile(out_tile), out_tile=out_tile, out_path=out_path, tags=tags, bucket_resource=bucket_resource)


class GTiffSingleFileOutputWriter(GTiffOutputReaderFunctions, base.SingleFileOutputWriter):
    ...
    def __init__(self, output_params, **kwargs):
        ...
        if self.cog or 'overviews' in output_params:
            self.overviews = True
            self.overviews_resampling = output_params.get('overviews_resampling', 'nearest')
            self.overviews_levels = output_params.get('overviews_levels', [2 ** i for i in range(1, self.zoom + 1)])
        else:
    ...
    def prepare(self, process_area=None, **kwargs):
        bounds = snap_bounds(bounds=Bounds(*process_area.intersection(box(*self.output_params['delimiters']['effective_bounds'])).bounds), pyramid=self.pyramid, zoom=self.zoom) if process_area else self.output_params['delimiters']['effective_bounds']
        height = math.ceil((bounds.top - bounds.bottom) / self.pyramid.pixel_x_size(self.zoom))
        width = math.ceil((bounds.right - bounds.left) / self.pyramid.pixel_x_size(self.zoom))
        logger.debug('output raster bounds: %s', bounds)
        logger.debug('output raster shape: %s, %s', height, width)
        self._profile = dict(GTIFF_DEFAULT_PROFILE, driver='GTiff', transform=Affine(self.pyramid.pixel_x_size(self.zoom), 0, bounds.left, 0, -self.pyramid.pixel_y_size(self.zoom), bounds.top), height=height, width=width, count=self.output_params['bands'], crs=self.pyramid.crs, **({k:self.output_params.get(k, GTIFF_DEFAULT_PROFILE[k]) for k in GTIFF_DEFAULT_PROFILE.keys()}), **({'bigtiff': self.output_params.get('bigtiff', 'NO')}))
        logger.debug('single GTiff profile: %s', self._profile)
        self.in_memory = self.in_memory if self.in_memory is False else height * width < IN_MEMORY_THRESHOLD
        if path_exists(self.path):
            if self.output_params['mode'] != 'overwrite':
                raise MapcheteConfigError('single GTiff file already exists, use overwrite mode to replace')
        else:
        ...
        if self.cog or path_is_remote(self.path):
            if self.in_memory:
                ...
                self.dst = self._ctx.enter_context(rasterio.open(self._tempfile.name, 'w+', **self._profile))
        else:
            self.dst = self._ctx.enter_context(rasterio.open(self.path, 'w+', **self._profile))
    ...
    def tiles_exist(self, process_tile=None, output_tile=None):
        ...
        if process_tile:
            return any(not self.read(tile).mask.all() for tile in self.pyramid.intersecting(process_tile))
        if output_tile:
    ...
    def write(self, process_tile, data):
        ...
        if data.mask.all():
            ...
            for tile in self.pyramid.intersecting(process_tile):
                out_tile = BufferedTile(tile, self.pixelbuffer)
                write_window = from_bounds(*out_tile.bounds, width=self.dst.width).round_lengths(pixel_precision=0).round_offsets(pixel_precision=0)
                if _window_in_out_file(write_window, self.dst):
                    logger.debug('write data to window: %s', write_window)
                    self.dst.write(extract_from_array(in_raster=data, in_affine=process_tile.affine, out_tile=out_tile) if process_tile != out_tile else data, window=write_window)
        ...
        try:
            if not exc_type:
                if self.overviews and self.dst is not None:
                    logger.debug('build overviews using %s resampling and levels %s', self.overviews_resampling, self.overviews_levels)
                    self.dst.build_overviews(self.overviews_levels, Resampling[self.overviews_resampling])
                    self.dst.update_tags(ns='rio_overview', resampling=self.overviews_resampling)
                if self.cog:
                    if path_is_remote(self.path):
                        logger.debug('upload to %s', self.path)
                        with NamedTemporaryFile() as (tmp_dst):
                            copy(self.dst, tmp_dst.name, copy_src_overviews=True, **self._profile)
                            self._bucket_resource.upload_file(Filename=tmp_dst.name, Key='/'.join(self.path.split('/')[3:]))
                    else:
                        logger.debug('write to %s', self.path)
                        copy(self.dst, self.path, copy_src_overviews=True, **self._profile)
                elif path_is_remote(self.path):
                    logger.debug('upload to %s', self.path)
                    if self.in_memory:
                        self._bucket_resource.put_object(Body=self._memfile, Key='/'.join(self.path.split('/')[3:]))
                    else:
                        self._bucket_resource.upload_file(Filename=self._tempfile.name, Key='/'.join(self.path.split('/')[3:]))
        finally:
            self._ctx.close()
