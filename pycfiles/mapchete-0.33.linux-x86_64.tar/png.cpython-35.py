...
# [GCC 8.4.0]
# Embedded file name: /home/ungarj/virtualenvs/mapchete/lib/python3.5/site-packages/mapchete/formats/default/png.py
# Compiled at: 2019-07-19 03:33:44
# Size of source mod 2**32: 7072 bytes
"""
PNG process output.
...
"""
import logging, numpy as np, numpy.ma as ma
from mapchete.config import validate_values
from mapchete.formats import base
from mapchete.io import get_boto3_bucket
from mapchete.io.raster import write_raster_window, prepare_array, memory_file, read_raster_no_crs
from mapchete.tile import BufferedTile
logger = logging.getLogger(__name__)
METADATA = {'driver_name': 'PNG', 
 'data_type': 'raster', 
 'mode': 'w'}
PNG_DEFAULT_PROFILE = {'dtype': 'uint8', 
 'driver': 'PNG', 
 'count': 4, 
 'nodata': 0}
...
class OutputDataReader(base.TileDirectoryOutputReader):
    ...
    def __init__(self, output_params, **kwargs):
        ...
        self.output_params = dict(output_params, nodata=output_params.get('nodata', PNG_DEFAULT_PROFILE['nodata']), dtype=PNG_DEFAULT_PROFILE['dtype'])
    ...
    def profile(self, tile=None):
        ...
        if tile is not None:
            dst_metadata.update(width=tile.width, height=tile.height, affine=tile.affine, crs=tile.crs)
        try:
    ...
    def empty(self, process_tile):
        ...
        """
        bands = self.output_params['bands'] if 'bands' in self.output_params else PNG_DEFAULT_PROFILE['count']
        return ma.masked_array(data=ma.zeros((bands,) + process_tile.shape), mask=ma.zeros((bands,) + process_tile.shape), dtype=PNG_DEFAULT_PROFILE['dtype'])
    ...
    def _prepare_array_for_png(self, data):
        ...
        if len(data) == 1:
            rgba = np.stack((
             ...
             np.where(data[0].data == self.output_params['nodata'], 0, 255).astype('uint8')))
        else:
            if len(data) == 2:
                ...
                if len(data) == 3:
                    rgba = np.stack((
                     ...
                     np.where(data[0].data == self.output_params['nodata'], 0, 255).astype('uint8', copy=False)))
                else:
                    if len(data) == 4:
    ...
    def write(self, process_tile, data):
        ...
        if data.mask.all():
            ...
            for tile in self.pyramid.intersecting(process_tile):
                ...
                write_raster_window(in_tile=process_tile, in_data=data, out_profile=self.profile(out_tile), out_tile=out_tile, out_path=out_path, bucket_resource=bucket_resource)