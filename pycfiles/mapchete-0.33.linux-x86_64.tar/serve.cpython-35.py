...
# [GCC 8.4.0]
# Embedded file name: /home/ungarj/virtualenvs/mapchete/lib/python3.5/site-packages/mapchete/cli/default/serve.py
# Compiled at: 2019-05-17 05:49:30
# Size of source mod 2**32: 4604 bytes
"""Command line utility to serve a Mapchete process."""
import click, logging, logging.config, os, pkgutil
...
@utils.opt_debug
@utils.opt_logfile
def serve(mapchete_file, port=None, internal_cache=None, zoom=None, bounds=None, overwrite=False, readonly=False, memory=False, input_file=None, debug=False, logfile=None):
    """
    ...
    """
    app = create_app(mapchete_files=[
     mapchete_file], zoom=zoom, bounds=bounds, single_input_file=input_file, mode=_get_mode(memory, readonly, overwrite), debug=debug)
    if os.environ.get('MAPCHETE_TEST') == 'TRUE':
        logger.debug("don't run flask app, MAPCHETE_TEST environment detected")
    else:
        app.run(threaded=True, debug=True, port=port, host='0.0.0.0', extra_files=[
         mapchete_file])


def create_app(mapchete_files=None, zoom=None, bounds=None, single_input_file=None, mode='continue', debug=None):
    """Configure and create Flask app."""
    from flask import Flask, render_template_string
    app = Flask(__name__)
    mapchete_processes = {os.path.splitext(os.path.basename(mapchete_file))[0]:mapchete.open(mapchete_file, zoom=zoom, bounds=bounds, single_input_file=single_input_file, mode=mode, with_cache=True, debug=debug) for mapchete_file in mapchete_files}
    ...
    def index():
        ...
        return render_template_string(pkgutil.get_data('mapchete.static', 'index.html').decode('utf-8'), srid=pyramid_srid, process_bounds=process_bounds, is_mercator=pyramid_srid == 3857, process_names=mapchete_processes.keys())

    @app.route('/'.join([
     '', 'wmts_simple', '1.0.0', '<string:mp_name>', 'default',
     grid, '<int:zoom>', '<int:row>', '<int:col>.<string:file_ext>']), methods=[
     'GET'])
    def get(mp_name, zoom, row, col, file_ext):
        """Return processed, empty or error (in pink color) tile."""
        logger.debug('received tile (%s, %s, %s) for process %s', zoom, row, col, mp_name)
        return _tile_response(mapchete_processes[mp_name], web_pyramid.tile(zoom, row, col), debug)
