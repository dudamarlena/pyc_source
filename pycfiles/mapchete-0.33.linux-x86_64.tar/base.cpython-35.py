...
# [GCC 8.4.0]
# Embedded file name: /home/ungarj/virtualenvs/mapchete/lib/python3.5/site-packages/mapchete/formats/base.py
# Compiled at: 2019-12-05 05:30:52
# Size of source mod 2**32: 15511 bytes
"""
...
import logging, numpy as np, numpy.ma as ma, os
...
class OutputDataBaseFunctions:
    ...
    def __init__(self, output_params, readonly=False, **kwargs):
        ...
        self.pyramid = BufferedTilePyramid(grid=output_params['grid'], metatiling=output_params['metatiling'], pixelbuffer=output_params['pixelbuffer'])
    ...
    def extract_subset(self, input_data_tiles=None, out_tile=None):
        ...
        if self.METADATA['data_type'] == 'raster':
            ...
            return extract_from_array(in_raster=prepare_array(mosaic.data, nodata=self.output_params['nodata'], dtype=self.output_params['dtype']), in_affine=mosaic.affine, out_tile=out_tile)
        if self.METADATA['data_type'] == 'vector':
...
class OutputDataWriter(OutputDataReader):
    ...
    def output_is_valid(self, process_data):
        ...
        """
        if self.METADATA['data_type'] == 'raster':
            return is_numpy_or_masked_array(process_data) or is_numpy_or_masked_array_with_tags(process_data)
        if self.METADATA['data_type'] == 'vector':
    ...
    def output_cleaned(self, process_data):
        ...
        if self.METADATA['data_type'] == 'raster':
            if is_numpy_or_masked_array(process_data):
                return process_data
            if is_numpy_or_masked_array_with_tags(process_data):
                data, tags = process_data
                return (
                 self.output_cleaned(data), tags)
    ...
    def streamline_output(self, process_data):
        if isinstance(process_data, str) and process_data == 'empty':
            ...
            if process_data is None:
                ...
                if self.output_is_valid(process_data):
                    return self.output_cleaned(process_data)
                raise MapcheteProcessOutputError('invalid output type: %s' % type(process_data))
...
class TileDirectoryOutputReader(OutputDataReader):
    ...
    def tiles_exist(self, process_tile=None, output_tile=None):
        ...
        if process_tile:
            return any(path_exists(self.get_path(tile)) for tile in self.pyramid.intersecting(process_tile))
        if output_tile:
            return path_exists(self.get_path(output_tile))

    def _read_as_tiledir(self, out_tile=None, td_crs=None, tiles_paths=None, profile=None, validity_check=False, indexes=None, resampling=None, dst_nodata=None, gdal_opts=None, **kwargs):
        """
        Read reprojected & resampled input data.
        ...
        """
        return _read_as_tiledir(data_type=self.METADATA['data_type'], out_tile=out_tile, td_crs=td_crs, tiles_paths=tiles_paths, profile=profile, validity_check=validity_check, indexes=indexes, resampling=resampling, dst_nodata=dst_nodata, gdal_opts=gdal_opts, **{k:v for k, v in kwargs.items() if k != 'data_type' if k != 'data_type'})
...
def is_numpy_or_masked_array_with_tags(data):
    return isinstance(data, tuple) and len(data) == 2 and is_numpy_or_masked_array(data[0]) and isinstance(data[1], dict)
...
def _read_as_tiledir(data_type=None, out_tile=None, td_crs=None, tiles_paths=None, profile=None, validity_check=False, indexes=None, resampling=None, dst_nodata=None, gdal_opts=None, **kwargs):
    logger.debug('reading data from CRS %s to CRS %s', td_crs, out_tile.tp.crs)
    if data_type == 'vector':
        if tiles_paths:
            return read_vector_window([path for _, path in tiles_paths], out_tile, validity_check=validity_check)
        else:
            return []
    elif data_type == 'raster':
        if tiles_paths:
            return read_raster_window([path for _, path in tiles_paths], out_tile, indexes=indexes, resampling=resampling, src_nodata=profile['nodata'], dst_nodata=dst_nodata, gdal_opts=gdal_opts)
        bands = len(indexes) if indexes else profile['count']
        return ma.masked_array(data=np.full((
         bands, out_tile.height, out_tile.width), profile['nodata'], dtype=profile['dtype']), mask=True)