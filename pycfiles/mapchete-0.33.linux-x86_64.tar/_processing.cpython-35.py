...
# [GCC 8.4.0]
# Embedded file name: /home/ungarj/virtualenvs/mapchete/lib/python3.5/site-packages/mapchete/_processing.py
# Compiled at: 2020-03-23 15:53:29
# Size of source mod 2**32: 23905 bytes
...
class TileProcess:
    ...
    def __init__(self, tile=None, config=None, skip=False):
        ...
        self.tile = config.process_pyramid.tile(*tile) if isinstance(tile, tuple) else tile
        self.skip = skip
        ...
        self.output_reader = None if skip or not config.baselevels else config.output_reader
    ...
    def _execute(self):
        if self.config_baselevels:
            ...
            if self.tile.zoom > max(self.config_baselevels['zooms']):
                pass
            return self._interpolate_from_baselevel('higher')
        process_func = get_process_func(process_path=self.process_path, config_dir=self.config_dir)
        try:
            with Timer() as (t):
                process_data = process_func(MapcheteProcess(tile=self.tile, params=self.process_func_params, input=self.input, output_params=self.output_params), **self.process_func_params)
    ...
    def _interpolate_from_baselevel(self, baselevel=None):
        ...
        output_tiles = list(self.output_reader.pyramid.tiles_from_bounds(tile.bounds, tile.zoom)) if tile.pixelbuffer > self.output_reader.pyramid.pixelbuffer else self.output_reader.pyramid.intersecting(tile)
        with Timer() as (t):
            if baselevel == 'higher':
                parent_tile = self.tile.get_parent()
                process_data = raster.resample_from_array(self.output_reader.read(parent_tile), in_affine=parent_tile.affine, out_tile=self.tile, resampling=self.config_baselevels['higher'], nodata=self.output_reader.output_params['nodata'])
            elif baselevel == 'lower':
                if self.output_reader.pyramid.pixelbuffer:
                    lower_tiles = set([y for y in chain(*[self.output_reader.pyramid.tiles_from_bounds(x.bounds, x.zoom + 1) for x in output_tiles])])
                else:
                    lower_tiles = [y for y in chain(*[x.get_children() for x in output_tiles])]
                mosaic = raster.create_mosaic([(lower_tile, self.output_reader.read(lower_tile)) for lower_tile in lower_tiles], nodata=self.output_reader.output_params['nodata'])
                process_data = raster.resample_from_array(in_raster=mosaic.data, in_affine=mosaic.affine, out_tile=self.tile, resampling=self.config_baselevels['lower'], nodata=self.output_reader.output_params['nodata'])
...
class MapcheteProcess(object):
    ...
    def __init__(self, tile=None, params=None, input=None, output_params=None, config=None):
    ...
    def write(self, data, **kwargs):
        """Deprecated."""
        raise DeprecationWarning('Please return process output data instead of using self.write().')
    ...
    def read(self, **kwargs):
        ...
        """
        raise DeprecationWarning('Read existing output from within a process is deprecated')
    ...
    def hillshade(self, elevation, azimuth=315.0, altitude=45.0, z=1.0, scale=1.0):
        ...
        hillshade : array
        """
        return commons_hillshade.hillshade(elevation, self.tile, azimuth, altitude, z, scale)
    ...
    def contours(self, elevation, interval=100, field='elev', base=0):
        ...
        """
        return commons_contours.extract_contours(elevation, self.tile, interval=interval, field=field, base=base)
    ...
    def clip(self, array, geometries, inverted=False, clip_buffer=0):
        ...
        clipped array : array
        """
        return commons_clip.clip_array_with_vector(array, self.tile.affine, geometries, inverted=inverted, clip_buffer=clip_buffer * self.tile.pixel_x_size)


class Executor:
    __doc__ = 'Wrapper class to be used with multiprocessing or billiard.'

    def __init__(self, start_method='spawn', max_workers=None, multiprocessing_module=multiprocessing):
        """Set attributes."""
        self.start_method = start_method
        self.max_workers = max_workers or os.cpu_count()
        self.multiprocessing_module = multiprocessing_module
        logger.debug('init %s Executor with start_method %s and %s workers', self.multiprocessing_module, self.start_method, self.max_workers)
        self._pool = self.multiprocessing_module.get_context(self.start_method).Pool(self.max_workers)

    def as_completed(self, func=None, iterable=None, fargs=None, fkwargs=None, chunksize=1):
        ...
        if self.max_workers == 1:
            ...
            logger.debug('open multiprocessing.Pool and %s %s workers', self.start_method, self.max_workers)
            for finished_task in self._pool.imap_unordered(partial(_exception_wrapper, func, fargs, fkwargs), iterable, chunksize=chunksize):
                yield finished_task
...
class FinishedTask:
    __doc__ = 'Wrapper class to encapsulate exceptions.'
...
def _run_on_single_tile(process=None, tile=None):
    ...
    return _execute_and_write(tile_process=TileProcess(tile=tile, config=process.config, skip=process.config.mode == 'continue' and process.config.output_reader.tiles_exist(tile)), output_writer=process.config.output)


def _run_area(process=None, zoom_levels=None, multi=None, max_chunksize=None, multiprocessing_module=None, multiprocessing_start_method=None, skip_output_check=False):
    logger.debug('run process on area')
    zoom_levels.sort(reverse=True)
    if process.config.output.write_in_parent_process:
        for process_info in _run_multi(func=_execute, zoom_levels=zoom_levels, process=process, multi=multi, multiprocessing_start_method=multiprocessing_start_method, multiprocessing_module=multiprocessing_module, write_in_parent_process=True, skip_output_check=skip_output_check):
            yield process_info

    else:
        for process_info in _run_multi(func=_execute_and_write, fkwargs=dict(output_writer=process.config.output), zoom_levels=zoom_levels, process=process, multi=multi, multiprocessing_start_method=multiprocessing_start_method, multiprocessing_module=multiprocessing_module, write_in_parent_process=False, skip_output_check=skip_output_check):
            yield process_info
...
def _filter_skipable(process=None, tiles=None, todo=None, target_set=None):
    ...
    for tile, skip in process.skip_tiles(tiles=tiles):
        if skip and tile not in target_set:
            yield ProcessInfo(tile=tile, processed=False, process_msg='output already exists', written=False, write_msg='nothing written')
        else:
            todo.add(tile)


def _run_multi(func=None, zoom_levels=None, process=None, multi=None, multiprocessing_start_method=None, multiprocessing_module=None, write_in_parent_process=False, fkwargs=None, skip_output_check=False):
    ...
    with Timer() as (t):
        with Executor(max_workers=workers, start_method=multiprocessing_start_method, multiprocessing_module=multiprocessing_module) as (executor):
            for i, zoom in enumerate(zoom_levels):
                if skip_output_check:
                    todo = process.get_process_tiles(zoom)
                else:
                    todo = set()
                    for process_info in _filter_skipable(process=process, tiles=process.get_process_tiles(zoom), todo=todo, target_set=overview_parents if process.config.baselevels and i else None):
                        num_processed += 1
                        logger.info('tile %s/%s finished: %s, %s, %s', num_processed, total_tiles, process_info.tile, process_info.process_msg, process_info.write_msg)
                        yield process_info

                for task in executor.as_completed(func=func, iterable=(TileProcess(tile=tile, config=process.config, skip=process.mode == 'continue' and process.config.output_reader.tiles_exist(tile) if skip_output_check else False) for tile in todo), fkwargs=fkwargs):
                    if write_in_parent_process:
                        output_data, process_info = task.result()
                        process_info = _write(process_info=process_info, output_data=output_data, output_writer=process.config.output)
                    else:
                        process_info = task.result()
                    if not skip_output_check and process.config.baselevels and process_info.processed and process_info.tile.zoom > 0:
...
def _execute(tile_process=None):
    ...
    if tile_process.skip:
        ...
        return (
         None,
         ProcessInfo(tile=tile_process.tile, processed=False, process_msg='output already exists', written=False, write_msg='nothing written'))
    else:
        with Timer() as (t):
            try:
        ...
        return (output,
         ProcessInfo(tile=tile_process.tile, processed=True, process_msg=processor_message, written=None, write_msg=None))
...
def _write(process_info=None, output_data=None, output_writer=None):
    if process_info.processed:
        ...
        if output_data is None:
            ...
            return ProcessInfo(tile=process_info.tile, processed=process_info.processed, process_msg=process_info.process_msg, written=False, write_msg=message)
        else:
            with Timer() as (t):
                output_writer.write(process_tile=process_info.tile, data=output_data)
            message = 'output written in %s' % t
            logger.debug((process_info.tile.id, message))
            return ProcessInfo(tile=process_info.tile, processed=process_info.processed, process_msg=process_info.process_msg, written=True, write_msg=message)
    else:
        return process_info
...
def _execute_and_write(tile_process=None, output_writer=None):
    ...
    return _write(process_info=process_info, output_data=output_data, output_writer=output_writer)