...
# [GCC 8.4.0]
# Embedded file name: /home/ungarj/virtualenvs/mapchete/lib/python3.5/site-packages/mapchete/_core.py
# Compiled at: 2020-03-22 06:20:31
# Size of source mod 2**32: 18777 bytes
"""Main module managing processes."""
from cachetools import LRUCache
import concurrent.futures, logging, multiprocessing, threading
...
def open(config, mode='continue', zoom=None, bounds=None, single_input_file=None, with_cache=False, debug=False):
    """
    ...
    """
    return Mapchete(MapcheteConfig(config, mode=mode, zoom=zoom, bounds=bounds, single_input_file=single_input_file, debug=debug), with_cache=with_cache)
...
class Mapchete(object):
    ...
    def skip_tiles(self, tiles=None):
        ...
        def _skip(config, tile):
            return (
             tile, config.output_reader.tiles_exist(tile))

        if self.config.mode == 'continue':
            with concurrent.futures.ThreadPoolExecutor() as (executor):
                for future in concurrent.futures.as_completed(executor.submit(_skip, self.config, tile) for tile in tiles):
                    yield future.result()
    ...
    def batch_process(self, zoom=None, tile=None, multi=None, max_chunksize=1, multiprocessing_module=None, multiprocessing_start_method='fork', skip_output_check=False):
        """
        ...
        skip_output_check : bool
            ...
            starting to process;
        """
        list(self.batch_processor(zoom=zoom, tile=tile, multi=multi or multiprocessing.cpu_count(), max_chunksize=max_chunksize, multiprocessing_module=multiprocessing_module or multiprocessing, multiprocessing_start_method=multiprocessing_start_method, skip_output_check=skip_output_check))

    def batch_processor(self, zoom=None, tile=None, multi=None, max_chunksize=1, multiprocessing_module=None, multiprocessing_start_method='fork', skip_output_check=False):
        """
        ...
        if tile:
            yield _run_on_single_tile(process=self, tile=self.config.process_pyramid.tile(*tuple(tile)))
        else:
            for process_info in _run_area(process=self, zoom_levels=list(_get_zoom_level(zoom, self)), multi=multi or multiprocessing.cpu_count(), max_chunksize=max_chunksize, multiprocessing_module=multiprocessing_module or multiprocessing, multiprocessing_start_method=multiprocessing_start_method, skip_output_check=skip_output_check):
                yield process_info
    ...
    def count_tiles(self, minzoom, maxzoom, init_zoom=0):
        ...
        """
        if (
         minzoom, maxzoom) not in self._count_tiles_cache:
            self._count_tiles_cache[(minzoom, maxzoom)] = count_tiles(self.config.area_at_zoom(), self.config.process_pyramid, minzoom, maxzoom, init_zoom=0)
    ...
    def execute(self, process_tile, raise_nodata=False):
        ...
        try:
            return self.config.output.streamline_output(TileProcess(tile=process_tile, config=self.config).execute())
    ...
    def write(self, process_tile, data):
        ...
        if self.config.mode == 'continue' and self.config.output.tiles_exist(process_tile):
    ...
    def get_raw_output(self, tile, _baselevel_readonly=False):
        ...
        tile = self.config.baselevels['tile_pyramid'].tile(*tile.id) if _baselevel_readonly else tile
        ...
        if self.config.mode == 'memory':
            ...
            return self._extract(in_tile=process_tile, in_data=self._execute_using_cache(process_tile), out_tile=tile)
        process_tile = self.config.process_pyramid.intersecting(tile)[0]
        if tile.pixelbuffer > self.config.output.pixelbuffer:
            output_tiles = list(self.config.output_pyramid.tiles_from_bounds(tile.bounds, tile.zoom))
        else:
    ...
    def _read_existing_output(self, tile, output_tiles):
        return self.config.output.extract_subset(input_data_tiles=[(output_tile, self.read(output_tile)) for output_tile in output_tiles], out_tile=tile)
    ...
    def _extract(self, in_tile=None, in_data=None, out_tile=None):
        ...
        return self.config.output.extract_subset(input_data_tiles=[
         (
          in_tile, in_data)], out_tile=out_tile)
    ...
    def __exit__(self, exc_type, exc_value, exc_traceback):
        ...
        self.config.output.close(exc_type=exc_type, exc_value=exc_value, exc_traceback=exc_traceback)
        if self.with_cache:
