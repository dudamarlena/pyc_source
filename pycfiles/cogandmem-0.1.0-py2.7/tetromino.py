...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/cogandmem/tetromino.py
# Compiled at: 2018-06-23 18:15:57
"""
...
import sys, numpy as np, time, pygame
from pygame.locals import *
import experiment, text, generic
LINES_PER_LEVEL = 10
BASE_FALL_FREQUENCY = 0.35
FALL_FREQUENCY_INCREASE_PER_LEVEL = 0.01
NEW_PIECES_START_Y = -2
...
PIECES = {'S': S_SHAPE_TEMPLATE, 
   'Z': Z_SHAPE_TEMPLATE, 'J': J_SHAPE_TEMPLATE, 'L': L_SHAPE_TEMPLATE, 
...
def fit_board_to_screen(blank_horizontal, blank_top, blank_bottom, top=(), left=(), right=(), f=None, min_cell_size=1, columns=10, rows=20):
    ...
    if max_width >= max_height:
        ...
        while cell_size >= min_cell_size:
            ...
            raise ValueError('A board cannot be created with the given parameters.')
    ...
    x, y = get_board_boundaries(cell_size, screen_height - int(screen_height - blank_bottom * screen_height), columns, rows)
    return (
     cell_size, x, y)
...
class Piece:
        ...
        for x in xrange(TEMPLATE_WIDTH):
            for y in xrange(TEMPLATE_HEIGHT):
                if shape_to_draw[y][x] != BLANK:
                    b.draw_box(x_coord + x * b.cell_size, y_coord + y * b.cell_size, self.colour, format='pixel')
    ...
    def draw_as_next_piece(self, b, f, centre, antialias=True):
        ...
        """
        screen = pygame.display.get_surface()
        next_surface, next_rect = text.render_string('next:', f, TEXT_COLOUR, BACKGROUND_COLOUR, antialiasing=antialias)
        next_rect.center = centre
        screen.blit(next_surface, next_rect)
        piece_coordinates = (
         centre[0], centre[1] + next_rect.height // 2 + 2)
...
class GameBoard:
    ...
    def draw_box(self, x, y, colour, format='box'):
        ...
        """
        if colour != BLANK:
            assert format == 'box' or format == 'pixel', 'The format parameter must be "box" or "pixel".'
            if format == 'box':
                x, y = self.board_to_pixels(x, y)
            screen = pygame.display.get_surface()
            pygame.draw.rect(screen, COLOURS[colour], (
             x + 1, y + 1, self.cell_size - 1, self.cell_size - 1))
            pygame.draw.rect(screen, LIGHT_COLOURS[colour], (
...
def draw_status(score, level, score_centre, level_centre, f, antialias=True):
    ...
    score_surface, score_rect = text.render_string(score_string, f, TEXT_COLOUR, BACKGROUND_COLOUR, antialiasing=antialias)
    level_surface, level_rect = text.render_string(level_string, f, TEXT_COLOUR, BACKGROUND_COLOUR, antialiasing=antialias)
...
def distractor(duration, game_font, columns=10, rows=20, top_exclude=0.025, right_exclude=0.025, bottom_exclude=0.025, left_exclude=0.025, antialias=True, instructions='', continue_instructions='', finish_instructions='', instruction_font=None, instruction_line_size=None, continue_font=None, text_colour=WHITE, background_colour=BLACK, split_sentences=False, sentences_end=('.', '!', '?'), sentences_end_exceptions=(), advance_instructions=(K_SPACE,), reverse_instructions=(K_LEFT, K_BACKSPACE), fall_frequency=0.3, c=None, fps=30, sound_file=None, press_to_quit=(K_ESCAPE,), files=(), data_file=None):
    ...
    Returns:
        ...
        losses: the number of times the pieces reached the top of the game
            board.
    """
    assert columns > 4 and rows > 4, 'The columns and rows variables must exceed 4.'
    assert fps > 0, 'Frame rate (fps) must be positive.'
    assert 0 <= top_exclude + bottom_exclude < 1, 'The sum of top_exclude and bottom_exclude must be between 0 and 1.'
    assert 0 <= left_exclude + right_exclude < 1, 'The sum of left_exclude and right_exclude must be between 0 and 1.'
    lines = 0
    losses = 0
    ...
    cell_size, x_margin, top_margin = fit_board_to_screen(left_exclude + right_exclude, top_exclude, bottom_exclude, right='next piece', f=game_font, columns=columns, rows=rows)
    next_string = 'next:'
    next_width, next_height = game_font.size(next_string)
    x_next = midpoint(window_width - int(right_exclude * window_width), x_margin + columns * cell_size)
    y_next = top_margin + int(0.05 * rows * cell_size)
    next_coordinates = (x_next, y_next)
    game_board = GameBoard(x_margin, top_margin, cell_size, columns=columns, rows=rows)
    falling_piece = Piece(game_board)
    next_piece = Piece(game_board)
    if instructions:
        text.display_text_until_keypress(instructions, instruction_font, text_colour, background_colour, proportion_width=left_exclude + right_exclude, proportion_height=top_exclude + bottom_exclude, main_line=instruction_line_size, break_sentences=split_sentences, sentence_terminators=sentences_end, terminator_exceptions=sentences_end_exceptions, bottom_message=continue_instructions, bottom_font=continue_font, bottom_line=continue_line_size, advance_keys=advance_instructions, reverse_keys=reverse_instructions, quit_keys=press_to_quit, ticker=c, frame_rate=fps, files=files)
    if sound_file:
    ...
    while time.time() < finish_time:
        ...
        game_board.draw()
        next_piece.draw_as_next_piece(game_board, game_font, next_coordinates, antialias=antialias)
        if falling_piece:
