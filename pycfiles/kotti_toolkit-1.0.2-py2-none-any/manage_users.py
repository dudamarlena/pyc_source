...
# [GCC 8.4.0]
# Embedded file name: /home/ubuntu/workspace/src/kotti_toolkit/kotti_toolkit/scripts/manage_users.py
# Compiled at: 2018-09-19 16:18:36
import os, csv, sys, json, re, uuid, time, operator, random, transaction, progress.bar, gzip, click, ConfigParser, subprocess
...
def create_user(config_file, username, title, password, email, roles=[], groups=[], registered=False, pyramid_env=None, close_pyramid_env=False):
    ...
    with transaction.manager:
        ...
        if principal:
            ...
            return
        user = Principal(username, title=title, email=email, active=True, password=password, groups=list(groups))
...
def create_group(config_file, group_name, title=None, email='merle@dpk.com', groups=[], pyramid_env=None, close_pyramid_env=False):
    ...
    with transaction.manager:
        ...
        if not principal:
            group = Principal(name=group_name, title=title, email=email, active=True, groups=groups)
...
def delete_users(config_file, pyramid_env=None, close_pyramid_env=False):
    ...
    with transaction.manager:
        ...
        for principal in principals.search(match='any', active=True).all():
            if (principal.name != 'admin' or 'role:admin' not in principal.groups) and not principal.name.startswith('group'):
    ...
    pyramid_env['closer']()
    print ('{} to delete all non-admin users, i.e. {} users').format(get_end_time(start_time), count)
    if close_pyramid_env:
        pyramid_env['closer']()