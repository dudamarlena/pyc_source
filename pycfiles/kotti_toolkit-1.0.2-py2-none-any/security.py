...
# [GCC 8.4.0]
# Embedded file name: /home/ubuntu/workspace/src/kotti_toolkit/kotti_toolkit/security.py
# Compiled at: 2018-09-19 17:05:10
import transaction
...
from sqlalchemy import Column, ForeignKey, Integer, Unicode, Float, Boolean, Date, String
...
def create_user(username, title, password, email, roles=[], groups=[], active=True):
    ...
    if principal:
        ...
        user = Principal(username, title=title, email=email, active=active, password=password, groups=groups)
...
def create_group(title, email, name='', groups=[], roles=[]):
    ...
    if not principal:
        groups = merge_list(groups, roles)
        group = Principal(name=name, title=title, email=email, active=True, groups=groups)
