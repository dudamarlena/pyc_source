...
# [GCC 8.4.0]
# Embedded file name: /Users/mgod/Documents/Sandbox/pigpy/tests/test_reports.py
# Compiled at: 2009-06-10 23:36:27
...
class test_reports(unittest.TestCase):
    ...
    def test_simple_plan_ordering(self):
        ...
        self.failUnless(re.match('.*1.*2.*3.*', plan.pigfile, flags=re.DOTALL), 'Pigfile should match .*1.*2.*3.*, instead is:\n%s' % plan.pigfile)
        plan.add(four)
        self.failUnless(re.match('.*1.*2.*3.*4.*', plan.pigfile, flags=re.DOTALL), 'Pigfile should match .*1.*2.*3.*4.*, instead is:\n%s' % plan.pigfile)
    ...
    def test_plan_partial_ordering(self):
        ...
        self.failUnless(re.match('(.*1.*2)(.*3.*4.*)|(.*4.*3.*)', plan.pigfile, flags=re.DOTALL))
        plan.add(five)
        self.failUnless(re.match('(.*1.*2)(((.*3.*4.*5.*)|(.*4.*3.*5.*))|(.*3.*5.*4.*))', plan.pigfile, flags=re.DOTALL))
    ...
    def test_parents_get_filled_into_code_by_name(self):
        ...
        child_report = Report('child', '%(this)s=%(parent_report)s', parents={'parent_report': base_report})
        plan = Plan(reports=[child_report])
        self.failUnless('child=my_name' in plan.pigfile, 'Parent report %s did not get properly filled into %s report' % (
    ...
    def test_reports_in_plan_get_saved(self):
        ...
        self.failUnless(re.match('.*STORE.*saved.*INTO.*saved.*', plan.pigfile, flags=re.DOTALL), 'Report %s should have a save statement. Pigfile is:\n%s' % (
    ...
    def test_report_with_dependencies_get_saves(self):
        ...
        self.failUnless(re.match('.*STORE.*cached.*LOAD.*cached.*bears.*narwhal.*', plan.pigfile, flags=re.DOTALL), 'Report %s should have been saved and loaded, pigfile is:\n%s' % (
    ...
    def test_report_with_multiple_childre_but_no_dependencies_does_not_get_cached(self):
        ...
        self.failIf(re.match('.*STORE.*cached.*LOAD.*cached.*bears.*narwhal.*', plan.pigfile, flags=re.DOTALL), 'Report %s should not have been cached, pigfile is:\n%s' % (
         base_report, plan.pigfile))
    ...
    def test_report_with_cached_child_does_not_get_cached(self):
        ...
        plan = Plan(reports=[first_child, second_child])
        self.failIf(re.match('.*STORE.*not_cached.*LOAD.*not_cached.*bears.*narwhal.*', plan.pigfile, flags=re.DOTALL), 'Report %s should not have been cached, pigfile is:\n%s' % (
         base_report, plan.pigfile))
        self.failUnless(re.match('.*STORE.*cached.*LOAD.*cached.*some_column.*another.*', plan.pigfile, flags=re.DOTALL), 'Report %s should have been saved and loaded, pigfile is:\n%s' % (
         base_report, plan.pigfile))
