...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/metro_tiny/management/commands/metrotinyimport.py
# Compiled at: 2012-03-18 16:29:27
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for line in open(filename).readlines():
            ...
            if ll.endswith(':'):
                name = ll.rstrip(':')
                (metro_line, created) = MetroLine.objects.get_or_create(city=city, name=name)
                if created:
                    metro_line_count += 1
            elif metro_line is not None:
                name = ll
                (metro_st, created) = MetroStation.objects.get_or_create(line=metro_line, name=name)
                if created:
                    metro_st_count += 1
