...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gridjug/test/test_gridjug.py
# Compiled at: 2015-08-24 13:16:46
# Size of source mod 2**32: 5239 bytes
import inspect, os, tempfile, gridjug, jug, pytest
...
ON_NLD_LOGIN = ON_NLD_CLUSTER and os.environ.get('CLUSTERNAME', None) == 'login'
TEMP_DIR = None
if ON_NLD_LOGIN:
    TEMP_DIR = os.path.join(THIS_DIR, 'tmp')
    if not os.path.isdir(TEMP_DIR):
        os.mkdir(TEMP_DIR)
NLD_GRIDMAP_PARAMS = {'local': False,  'require_cluster': True, 
 'temp_dir': TEMP_DIR, 
 'quiet': False, 
...
def test_execute_specify_jugdir(tmpdir):
    ...
    assert not jugdir.listdir()
    gridjug.grid_jug(jugfile=PRIMES_JUGFILE, jugdir=jugdir.strpath, local=True)
    assert jugdir.listdir()
...
def test_failing(tmpdir):
    jugdir = tmpdir
    res = gridjug.grid_jug(jugfile=FAILING_JUGFILE, jugdir=jugdir.strpath, local=True)
    for result in res:
...
def test_failing_keep_going(tmpdir):
    jugdir = tmpdir
    gridjug.grid_jug(jugfile=FAILING_JUGFILE, jugdir=jugdir.strpath, local=True, keep_going=True)
    _, jugspace = jug.init(jugfile=FAILING_JUGFILE, jugdir=jugdir.strpath)
    for n, task in zip(range(2, 11), jugspace['primes10']):
        if not task.can_load() == (n != 6):
            raise AssertionError
...
def test_jug_capture_stdout(capsys):
    gridjug.grid_jug(jugfile=PRIMES_JUGFILE, local=True, capture_jug_stdout=True)
    out = capsys.readouterr()[0]
    assert not out
...
def jugdir():
    ...
    assert not os.listdir(ret)
    return ret


@pytest.mark.skipif(not ON_NLD_LOGIN, reason='Not on NLD cluster login node')
def test_nld_execute_specify_jugdir(jugdir):
    gridjug.grid_jug(jugfile=PRIMES_JUGFILE, jugdir=jugdir, **NLD_GRIDMAP_PARAMS)
    assert os.listdir(jugdir)
...
def test_nld_failing(jugdir):
    with pytest.raises(RuntimeError):
        gridjug.grid_jug(jugfile=FAILING_JUGFILE, jugdir=jugdir, **NLD_GRIDMAP_PARAMS)


@pytest.mark.skipif(not ON_NLD_LOGIN, reason='Not on NLD cluster login node')
def test_nld_failing_keep_going(jugdir):
    gridjug.grid_jug(jugfile=FAILING_JUGFILE, jugdir=jugdir, keep_going=True, **NLD_GRIDMAP_PARAMS)
