...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/wisestork/gc_correct.py
...
:license: GPL-3.0
"""
import numpy as np, warnings, statsmodels.nonparametric.smoothers_lowess as statlow
...
def correct(inputs, fasta, frac_n=0.1, frac_r=0.0001, lowess_iter=3, lowess_frac=0.1):
    ...
    if lowess_frac * len(reads) < 4:
        ...
        delta = 0.01 * len(gcs)
    lowess = statlow.lowess(reads, gcs, return_sorted=False, delta=delta,
      frac=lowess_frac,
