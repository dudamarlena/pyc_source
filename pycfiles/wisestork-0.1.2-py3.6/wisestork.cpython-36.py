...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/wisestork/wisestork.py
...
@click.option('--frac-n', '-n', type=(click.FLOAT), default=0.1, help='Maximum fraction of N-bases per bin. Default = 0.1')
@click.option('--frac-r', '-r', type=(click.FLOAT), default=0.0001, help='Minimum fraction of reads per bin. Default = 0.0001')
@click.option('--iter', '-t', type=(click.INT), default=3, help='Number of iterations for LOWESS function. Default = 3')
@click.option('--frac-lowess', '-l', type=(click.FLOAT), default=0.1, help='Fraction of data to use for LOWESS function. Default = 0.1')
def gcc_cli(**kwargs):
    """
    ...
    gc_correct(input=input_path, output=output, reference=reference, frac_r=frac_r,
      frac_n=frac_n,
      iter=iter,
...
@generic_option(shared_options)
@click.option('--input', '-I', type=click.Path(exists=True), required=True, multiple=True,
  help='Path(s) to input BEDs')
@click.option('--output', '-O', type=(click.Path()), required=True, help='Path to output BED file')
