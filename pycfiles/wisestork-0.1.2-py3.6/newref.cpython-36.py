...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/wisestork/newref.py
...
class ReferenceBinGenerator(object):
    ...
    def __init__(self, inputs, n_bins, reference, binsize=int(1000000.0), binfile=None):
        """
        ...
        self._ReferenceBinGenerator__bins = self.get_all_bins()
        self._ReferenceBinGenerator__idx = 0
    ...
    def __next__(self):
        if self._ReferenceBinGenerator__idx == len(self._ReferenceBinGenerator__bins):
            raise StopIteration
        nearest = self.get_nearest_at_idx(self._ReferenceBinGenerator__idx)
        cur = self._ReferenceBinGenerator__bins[self._ReferenceBinGenerator__idx]
        filtered = self.filter_bins(cur, nearest)
        self._ReferenceBinGenerator__idx += 1
        return (cur, filtered)
    ...
    def get_nearest_at_idx(self, idx):
        e_dist = len(self._ReferenceBinGenerator__bins) - idx
        if idx == 0 or 0 < idx <= self.n_bins // 2:
            vals = self._ReferenceBinGenerator__bins[:self.n_bins]
        else:
            if e_dist <= self.n_bins // 2:
                vals = self._ReferenceBinGenerator__bins[-self.n_bins:]
            else:
                vals = self._ReferenceBinGenerator__bins[idx - self.n_bins // 2:idx + int(math.ceil(self.n_bins / 2))]
        return vals
