...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/adminbrowse/tests.py
# Compiled at: 2010-12-02 18:01:09
...
from adminbrowse import link_to_change, link_to_changelist, related_list, link_to_url, truncated_field, AutoBrowseModelAdmin
...
urlpatterns = patterns('', ('^foo/admin/bar/', include(test_site.urls)))
...
class TestChangeLink(TestCase):
    ...
    def test_call_returns_html(self):
        ...
        self.assertEqual(self.link(self.books[1]).strip(), '<span class="change-link"><a href="%s" title="Go to author"></a> Ernest Hemingway</span>' % url)
        url = '/foo/admin/bar/adminbrowse/person/3/'
        self.assertEqual(self.link(self.books[3]).strip(), '<span class="change-link"><a href="%s" title="Go to author"></a> Kurt Vonnegut</span>' % url)

    def test_short_description_defaults_to_verbose_name(self):
        self.assertEqual(self.link.short_description, 'author')
...
class TestOneToManyChangeListLink(TestCase):
    ...
    def test_call_returns_html(self):
        ...
        self.assertEqual(self.link(self.people[1]).strip(), '<span class="changelist-link"><a href="%s" title="%s">3</a></span>' % (
         url, title))
        url = '/foo/admin/bar/adminbrowse/book/?author__pid__exact=3'
        title = 'List books with this author'
        self.assertEqual(self.link(self.people[2]).strip(), '<span class="changelist-link"><a href="%s" title="%s">2</a></span>' % (
         url, title))

    def test_short_description_defaults_to_field_name(self):
        self.assertEqual(self.link.short_description, 'bibliography')
    ...
    def test_text_sets_rendered_link_text(self):
        link = link_to_changelist(Person, 'bibliography', text='List bibliography')
        url = '/foo/admin/bar/adminbrowse/book/?author__pid__exact=3'
        title = 'List books with this author'
        self.assertEqual(link(self.people[2]).strip(), '<span class="changelist-link"><a href="%s" title="%s">List bibliography</a></span>' % (
         url, title))

    def test_callable_text_gets_called_with_value(self):
        link = link_to_changelist(Person, 'bibliography', text=lambda x: 'List books (%s)' % len(x))
        url = '/foo/admin/bar/adminbrowse/book/?author__pid__exact=3'
        title = 'List books with this author'
        self.assertEqual(link(self.people[2]).strip(), '<span class="changelist-link"><a href="%s" title="%s">List books (2)</a></span>' % (
         url, title))
...
class TestIndirectManyToManyChangeListLink(TestCase):
    ...
    def test_call_returns_html(self):
        ...
        self.assertEqual(self.link(self.genres[0]).strip(), '<span class="changelist-link"><a href="%s" title="%s">3</a></span>' % (
         url, title))

    def test_short_description_defaults_to_field_name(self):
        self.assertEqual(self.link.short_description, 'collection')
...
class TestDefaultRelatedNameChangeListLink(TestCase):
    ...
    def test_short_description_is_accessor_with_spaces(self):
        self.assertEqual(self.one_to_many.short_description, 'logentry set')
        self.assertEqual(self.many_to_many.short_description, 'user set')
...
class TestDirectManyToManyChangeListLink(TestCase):
    ...
    def test_call_returns_html(self):
        ...
        self.assertEqual(self.link(self.books[4]).strip(), '<span class="changelist-link"><a href="%s" title="%s">2</a></span>' % (
         url, title))

    def test_short_description_defaults_to_verbose_name(self):
        self.assertEqual(self.link.short_description, 'categories')
...
class TestOneToManyRelatedList(TestCase):
    ...
    def test_short_description_defaults_to_field_name(self):
        self.assertEqual(self.column.short_description, 'bibliography')

    def test_call_returns_comma_separated_list(self):
        self.assertEqual(self.column(self.people[2]), "Cat's Cradle, Slaughterhouse-Five")
    ...
    def test_sep_sets_string_separator(self):
        ...
        self.assertEqual(column(self.people[2]), "Cat's Cradle ~ Slaughterhouse-Five")
...
class TestIndirectManyToManyRelatedList(TestCase):
    ...
    def test_call_returns_comma_separated_list(self):
        self.assertEqual(self.column(self.genres[1]), 'The Old Man and the Sea')
...
class TestURLColumn(TestCase):
    ...
    def test_call_returns_link_html(self):
        self.assertEqual(self.column(self.people[0]), '<a href="http://example.com/twain" target="_blank" class="external" title="Open URL in a new window">http://example.com/twain</a>')

    def test_target_sets_link_target(self):
        column = link_to_url(Person, 'website', target='test')
        self.assertEqual(column(self.people[0]), '<a href="http://example.com/twain" target="test" class="external" title="Open URL">http://example.com/twain</a>')

    def test_classes_sets_link_class(self):
        column = link_to_url(Person, 'website', classes=['one', 'two'])
        self.assertEqual(column(self.people[0]), '<a href="http://example.com/twain" target="_blank" class="one two" title="Open URL in a new window">http://example.com/twain</a>')
...
class TestTruncatedTextColumn(TestCase):
    ...
    def test_call_returns_text_truncated_to_max_length(self):
        self.assertEqual(self.column(self.people[2]), 'http://example.com/vonne…')
        self.assertEqual(self.column(self.people[0]), 'http://example.com/twain')

    def test_max_length_sets_length_before_truncation(self):
        column = truncated_field(Person, 'website', 8)
        self.assertEqual(column(self.people[0]), 'http://e…')
...
class TestAutoBrowseModelAdmin(TestCase):
    ...
    def setUp(self):
        ...
        class BookAdmin(AutoBrowseModelAdmin):
            list_display = [
             'title', 'author', 'categories', 'loc_url']
