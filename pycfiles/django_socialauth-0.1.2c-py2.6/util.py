...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/openid_consumer/util.py
...
if openid.__version__ < '2.1.0':
    ...
    class DjangoOpenIDStore(OpenIDStore):
        ...
        def storeAssociation(self, server_url, association):
            assoc = Association(server_url=server_url, handle=association.handle, secret=base64.encodestring(association.secret), issued=association.issued, lifetime=association.issued, assoc_type=association.assoc_type)
            assoc.save()
        ...
        def removeAssociation(self, server_url, handle):
            assocs = list(Association.objects.filter(server_url=server_url, handle=handle))
        ...
        def storeNonce(self, nonce):
            (nonce, created) = Nonce.objects.get_or_create(nonce=nonce, defaults={'expires': int(time.time())})
