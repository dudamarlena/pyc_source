...
# Embedded file name: build/bdist.macosx-10.3-i386/egg/aha/controller/makocontroller.py
# Compiled at: 2011-04-03 21:47:44
__doc__ = ' makocontroller.py - Classes handing mako as a template engine.\n\n$Id: makocontroller.py 644 2010-08-10 04:15:42Z ats $\n'
...
def get_lookup(du=False):
    ...
    if not tlookup:
        ...
        if config.debug:
            tlookup = TemplateLookup(directories=tpdirs, disable_unicode=du, input_encoding=charset, output_encoding=charset)
        else:
            tlookup = TemplateLookup(directories=tpdirs, disable_unicode=du, input_encoding=charset, output_encoding=charset, filesystem_checks=False, cache_type='memcached', cache_dir='.', cache_url='memcached://')
...
def patch_beaker():
    ...
    """
    import sys, google.appengine.api.memcache
    ...
    beaker.ext.memcached.MemcachedNamespaceManager.get_creation_lock = lambda x, y: synchronization.null_synchronizer()
