...
# Embedded file name: build/bdist.macosx-10.3-i386/egg/aha/widget/tests/test_fields.py
# Compiled at: 2010-10-20 22:45:02
...
class TestBasefield(TestCase):
    ...
    def test_validate(self):
        ...
        bf = TextField(name='foo', args=c, validator=(
         v.Int(), v.OneOf([1, 2, 3])))
        assert_equal(bf.validate('1')[0], 1)
        ...
        bf = TextField(name='foo', args=c, validator=(
         v.Int(), v.OneOf([1, 2, 3])), required=True)
        r = bf.validate('')
    ...
    def test_fields(self):
        ...
        rf = RadioField(name='foo', args=c, values=(
         ('vfoo', 'v1'), ('vbar', 'v2')))
        body = rf.render_body()
        for v in ('vfoo', 'vbar'):
        ...
        cg = CheckboxGroup(name='foo', args=c, values=(
         ('vfoo', 'v1'), ('vbar', 1)))
        body = cg.render_body()
        for v in ('vfoo', 'vbar'):
            assert_true('>%s<' % v in body)
        ...
        assert_true('checked' in body)
        v = validators
        cg2 = CheckboxGroup(name='foo', args=c, values=(
         ('vfoo', 'v1'), ('vbar', 'v2')), validator=v.Int())
        ...
        sf = SelectField(name='foo', args=c, values=(
         ('vfoo', 'v1'), ('vbar', 'v2')))
        body = sf.render_body()
        for v in ('vfoo', 'vbar'):
