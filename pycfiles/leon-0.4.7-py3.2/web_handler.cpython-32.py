...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/leon/web_handler.py
# Compiled at: 2013-05-08 11:00:47
import os, sys, re, inspect, json, cherrypy
...
class FnHandler:
    ...
    def __init__(self, fn, url):
        route_syntax_as_re = re.sub('/:(\\w+)', '/(?P<\\1>\\w+)', url + '([/?]|$)')
        self.url = url
...
class WebHandler(object):
    ...
    def __init__(self, config=None):
        ...
        conf = {'/': {'tools.sessions.on': True, 
         ...
         'leon': {'deployment_mode': 'production'}}
        app.merge(conf)
