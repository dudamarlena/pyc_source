...
# [GCC 8.4.0]
# Embedded file name: /home/crito/Work/docar/tests/serializer_tests.py
# Compiled at: 2012-09-26 11:24:43
from nose.tools import eq_
from mock import patch
from docar import Document, Collection, fields
from docar.serializers import Serializer, DropinJsonSerializer, CollectionJsonSerializer, MEDIA_TYPES
...
class when_the_old_json_serializer_is_used(unittest.TestCase):
    ...
    method."""
    expected = {'cnames': [{'record': 'aa.record'}],  'instances': 2, 
     'label': 'app', 
     'published': True, 
     'link': {'href': 'app_uri', 
              'rel': 'self'}, 
     'postgres': {'href': 'postgres_uri', 
                  'rel': 'related'}, 
     'user': {'name': 'crito'}}
    expected_collection = {'items': [{'id': 1,  'link': {'rel': 'item',  'href': 'instance_uri'}}],  'size': 1, 
     'link': {'rel': 'self',  'href': 'instance_collection_uri'}}
    fixture = {'cnames': [{'record': 'aa.record'}],  'instances': 2, 
     'label': 'app', 
     'published': True, 
     'postgres': {'id': 1},  'user': {'name': 'crito'}}
    ...
    def it_serializes_a_collection_correctly(self):
        ...
        eq_(json.dumps(self.expected_collection), DropinJsonSerializer.dump(instances))
...
class when_collection_plus_json_serialization_is_needed(unittest.TestCase):
    ...
    def it_serializes_errors(self):
        expected = {'collection': {'error': {'title': 'Error title', 
                                  'code': 404, 
                                  'message': 'Not found'}}}
        j = CollectionJsonSerializer.error(status=404, message='Not found', title='Error title')
        eq_(expected, json.loads(j))

    def it_serializes_a_document_to_json(self):
        article = article_factory()
        expected = {'collection': {'items': [
                                  {'data': [{'name': 'slug',  'value': 'article-slug'}, {'name': 'title',  'value': 'article title'}],  'links': [{'href': 'editor-uri',  'rel': 'item'}],  'href': 'article-uri'}]}}
    ...
    def it_serializes_a_collection_to_json(self):
        ...
        expected = {'collection': {'items': [
                                  {'data': [{'name': 'slug',  'value': 'article-slug'}, {'name': 'title',  'value': 'article title'}],  'links': [{'href': 'editor-uri',  'rel': 'item'}],  'href': 'article-uri'},
                                  {'data': [{'name': 'slug',  'value': 'article-slug'}, {'name': 'title',  'value': 'article 2 title'}],  'links': [{'href': 'editor-uri',  'rel': 'item'}],  'href': 'article-uri'},
                                  {'data': [{'name': 'slug',  'value': 'article-slug'}, {'name': 'title',  'value': 'article 3 title'}],  'links': [{'href': 'editor-uri',  'rel': 'item'}],  'href': 'article-uri'}]}}
