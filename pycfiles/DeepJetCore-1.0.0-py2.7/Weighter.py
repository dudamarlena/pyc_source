...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/DeepJetCore/Weighter.py
...
class Weighter(object):
    ...
    def __eq__(self, other):
        ...
        return self.Axixandlabel == other.Axixandlabel and all(self.axisX == other.axisX) and all(self.axisY == other.axisY) and comparator(self.hists, other.hists) and comparator(self.removeProbabilties, other.removeProbabilties) and self.classes == other.classes and self.refclassidx == other.refclassidx and self.undefTruth == other.undefTruth and comparator(self.binweights, other.binweights) and comparator(self.distributions, other.distributions) and (self.xedges == other.xedges).all() and (self.yedges == other.yedges).all()
    ...
    def addDistributions(self, Tuple):
        ...
        if not useonlyoneclass:
            ...
            selidxs = [
             numpy.zeros(len(xtuple), dtype='int') < 1]
    ...
    def createRemoveProbabilitiesAndWeights(self, referenceclass='isB'):
        ...
        if len(self.classes) > 0 and len(self.classes[0]):
            self.Axixandlabel = [
             self.nameX, self.nameY] + self.classes
        else:
            self.Axixandlabel = [
