...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/DeepJetCore/training/DeepJet_callbacks.py
# Compiled at: 2018-07-31 05:35:54
"""
...
class DeepJet_callbacks(object):
    ...
    def __init__(self, model, stop_patience=-1, lr_factor=0.5, lr_patience=-1, lr_epsilon=0.001, lr_cooldown=4, lr_minimum=1e-05, outputDir='', minTokenLifetime=5, checkperiod=10, plotLossEachEpoch=True):
        ...
        if lr_patience > 0:
            self.reduce_lr = ReduceLROnPlateau.ReduceLROnPlateau(monitor='val_loss', factor=lr_factor, patience=lr_patience, mode='min', verbose=1, epsilon=lr_epsilon, cooldown=lr_cooldown, min_lr=lr_minimum)
            self.callbacks.append(self.reduce_lr)
        self.modelbestcheck = ModelCheckpoint(outputDir + '/KERAS_check_best_model.h5', monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=False)
