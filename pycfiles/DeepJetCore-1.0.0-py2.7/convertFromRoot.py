...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/DeepJetCore/bin/convertFromRoot.py
# Compiled at: 2018-07-27 04:17:54
"""
...
try:
    ...
    print '\nDatastructures modules not found. Please define a DeepJetCore submodule and add it to your PYTHONPATH variable.'
...
if args.batch and not (args.usemeansfrom or args.testdatafor):
    raise ValueError('When running in batch mode you should also provide a means source through the --usemeansfrom option')
if args.v:
...
dc = DataCollection(nprocs=1 if args.nothreads else -1, useRelativePaths=True if not args.noRelativePaths else False)
if len(nchilds):
...
if testdatafor:
    ...
    dc.createTestDataForDataCollection(testdatafor, infile, outPath, outname=args.batch if args.batch else 'dataCollection.dc', batch_mode=bool(args.batch))
elif recover:
    dc.recoverCreateDataFromRootFromSnapshot(recover)
elif args.means:
    dc.convertListOfRootFiles(infile, traind(class_args) if class_args else traind(), outPath, means_only=True, output_name='batch_template.dc')
else:
    dc.convertListOfRootFiles(infile, traind(class_args) if class_args else traind(), outPath, usemeansfrom, output_name=args.batch if args.batch else 'dataCollection.dc', batch_mode=bool(args.batch))