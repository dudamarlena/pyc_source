...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/DeepJetCore/training/training_base.py
# Compiled at: 2018-07-31 08:52:36
...
class training_base(object):
    ...
    def __init__(self, splittrainandtest=0.85, useweights=False, testrun=False, resumeSilently=False, renewtokens=True, collection_class=DataCollection, parser=None):
        if parser is None:
        ...
        if not isNewTraining:
            kfile = self.outputDir + '/KERAS_check_model_last.h5' if os.path.isfile(self.outputDir + '/KERAS_check_model_last.h5') else self.outputDir + '/KERAS_model.h5'
    ...
    def setModel(self, model, **modelargs):
        ...
        self.keras_model = model(self.keras_inputs, self.train_data.getNClassificationTargets(), self.train_data.getNRegressionTargets(), **modelargs)
    ...
    def compileModel(self, learningrate, **compileargs):
        if not self.keras_model:
    ...
    def trainModel(self, nepochs, batchsize, stop_patience=-1, lr_factor=0.5, lr_patience=-1, lr_epsilon=0.003, lr_cooldown=6, lr_minimum=1e-06, maxqsize=20, checkperiod=10, **trainargs):
        ...
        callbacks = DeepJet_callbacks.DeepJet_callbacks(self.keras_model, stop_patience=stop_patience, lr_factor=lr_factor, lr_patience=lr_patience, lr_epsilon=lr_epsilon, lr_cooldown=lr_cooldown, lr_minimum=lr_minimum, outputDir=self.outputDir, checkperiod=checkperiod)
        nepochs = nepochs - self.trainedepoches
        print('starting training')
        self.keras_model.fit_generator(self.train_data.generator(), steps_per_epoch=self.train_data.getNBatchesPerEpoch(), epochs=nepochs, callbacks=callbacks.callbacks, validation_data=self.val_data.generator(), validation_steps=self.val_data.getNBatchesPerEpoch(), max_q_size=maxqsize, **trainargs)
        ...
        return (
         self.keras_model, callbacks.history)