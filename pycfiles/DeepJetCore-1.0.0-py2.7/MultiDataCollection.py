...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/DeepJetCore/MultiDataCollection.py
...
"""
    ...
    def __init__(self, infiles=None, nprocs=-1, add_ys=[], flags=[]):
        ...
        self.additional_ys = []
        if infiles:
            self.collections = [ DataCollection(i, cpu_count() / len(infiles) if nprocs == -1 else nprocs / len(infiles)) for i in infiles
                               ]
        if flags:
            self.setFlags(flags)
        if add_ys:
    ...
    def readFromFile(self, infiles):
        self.collections = [ DataCollection(i, cpu_count() / len(infiles) if self.nprocs == -1 else self.nprocs / len(infiles)) for i in infiles
                           ]
        ...
        for zipped in izip(*generators):
            ...
            for xyw, flag, add_y in zip(zipped, flags, add_ys):
                ...
                for template in add_y:
                    y_to_add = np.hstack([ ones * i for i in template ]) if hasattr(template, '__iter__') else ones * template
                    y.append(y_to_add)

                if self.flags:
                    if len(flag) != len(y):
                        raise ValueError('Flags (if any) and total Y number MUST be the same! Got: %d and %d' % (
                         len(flag), len(y)))
