...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/DeepJetCore/DataCollection.py
# Compiled at: 2018-07-31 06:07:56
"""
...
class DataCollection(object):
    ...
    def __iadd__(self, other):
        ...
        self.isTrain = self.isTrain and other.isTrain
        if type(self.dataclass) != type(other.dataclass):
            raise ValueError('The two DataCollections were made with a different data class type! %s, and %s' % (
             type(self.dataclass), type(other.dataclass)))
        if self.weighter != other.weighter:
            raise ValueError('The two DataCollections have different weights')
        ...
        if not all(self.means == other.means):
            raise ValueError('The two DataCollections head different means')
    ...
    def createTestDataForDataCollection(self, collectionfile, inputfile, outputDir, outname='dataCollection.dc', batch_mode=False):
        import copy
        self.readFromFile(collectionfile)
        self.dataclass.remove = False
        self.dataclass.weight = True
        self.readRootListFromFile(inputfile)
        self.createDataFromRoot(self.dataclass, outputDir, False, dir_check=not batch_mode)
    ...
    def createDataFromRoot(self, dataclass, outputDir, redo_meansandweights=True, means_only=False, dir_check=True):
        """
        ...
        if os.path.isdir(outputDir) and dir_check:
            ...
            if redo_meansandweights and (td.remove or td.weight):
                ...
                self.weighter = td.produceBinWeighter(self.originRoots)
                self.weighter.printHistos(outputDir)
            if redo_meansandweights:
                logging.info('producing means and norms')
                self.means = td.produceMeansFromRootFile(self.originRoots, limit=self.meansnormslimit)
            if means_only:
                return
    ...
    def __writeData_async_andCollect(self, startindex, outputDir):
        ...
        def __collectWriteInfo(successful, samplename, sampleentries, outputDir):
            ...
            self.writeToFile(outputDir + '/snapshot_tmp.dc')
    ...
    def convertListOfRootFiles(self, inputfile, dataclass, outputDir, takemeansfrom='', means_only=False, output_name='dataCollection.dc', batch_mode=False):
        newmeans = True
        if takemeansfrom:
            self.readFromFile(takemeansfrom)
            newmeans = False
        self.readRootListFromFile(inputfile)
        self.createDataFromRoot(dataclass, outputDir, newmeans, means_only=means_only, dir_check=not batch_mode)
        ...
        class tdreader(object):
            ...
            def __readNext(self):
                ...
                def startRead(counter, filename, shuffleseed):
                    ...
                    while excounter < 10:
                        try:
                            self.tdlist[counter].readIn_async(filename, ramdiskpath='/dev/shm/', randomseed=shuffleseed)
                            break
