...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/DeepJetCore/TrainData.py
...
import numpy, logging, threading, multiprocessing
...
class TrainData(object):
    ...
    def __init__(self):
        ...
        self.weightbranchX = ''
        self.weightbranchY = ''
        self.weight_binX = numpy.array([-1000000000000.0, 1000000000000.0], dtype=float)
        self.weight_binY = numpy.array([-1000000000000.0, 1000000000000.0], dtype=float)
    ...
    def __createArr(self, shapeinfo):
        ...
        shared_array = numpy.ctypeslib.as_array(shared_array_base)
    ...
    def readIn_async(self, fileprefix, read_async=True, shapesOnly=False, ramdiskpath='', randomseed=None):
        ...
        if read_async:
            if 'meta' in readfile[-4:]:
                ...
                filebase = readfile[:-4]
                for i in range(len(self.w_list)):
                    self.readthreadids.append(startReading(self.w_list[i].ctypes.data, filebase + 'w.' + str(i), list(self.w_list[i].shape), isRamDisk))

                for i in range(len(self.x_list)):
                    self.readthreadids.append(startReading(self.x_list[i].ctypes.data, filebase + 'x.' + str(i), list(self.x_list[i].shape), isRamDisk))

                for i in range(len(self.y_list)):
                    self.readthreadids.append(startReading(self.y_list[i].ctypes.data, filebase + 'y.' + str(i), list(self.y_list[i].shape), isRamDisk))

            else:
                self.readthread = multiprocessing.Process(target=_read_arrs_, args=(
                 self.w_list,
                 self.x_list,
                 self.y_list,
                 self.readdone,
        ...
        elif 'meta' in readfile[-4:]:
            from DeepJetCore.compiled.c_readArrThreaded import readBlocking
            filebase = readfile[:-4]
            self.readthreadids = []
            for i in range(len(self.w_list)):
                readBlocking(self.w_list[i].ctypes.data, filebase + 'w.' + str(i), list(self.w_list[i].shape), isRamDisk)

            for i in range(len(self.x_list)):
                readBlocking(self.x_list[i].ctypes.data, filebase + 'x.' + str(i), list(self.x_list[i].shape), isRamDisk)

            for i in range(len(self.y_list)):
                readBlocking(self.y_list[i].ctypes.data, filebase + 'y.' + str(i), list(self.y_list[i].shape), isRamDisk)
    ...
    def readTreeFromRootToTuple(self, filenames, limit=None, branches=None):
        ...
        if isinstance(filenames, list):
            ...
            print('add files')
            nparray = root2array(filenames, treename=self.treename, stop=limit, branches=usebranches)
            print('done add files')
            return nparray
            print('add files')
        else:
            fileTimeOut(filenames, 120)
    ...
    def make_empty_weighter(self):
        ...
        if self.remove or self.weight:
            weighter.setBinningAndClasses([
             self.weight_binX, self.weight_binY], self.weightbranchX, self.weightbranchY, self.truthclasses)
        return weighter
