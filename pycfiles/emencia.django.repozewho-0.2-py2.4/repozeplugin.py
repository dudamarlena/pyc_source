...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/emencia/django/repozewho/repozeplugin.py
# Compiled at: 2009-07-02 12:54:45
from repoze.who.plugins.form import *
from repoze.who.interfaces import IAuthenticator, IMetadataProvider, IIdentifier, IChallenger
...
class RedirectingDjangoFormPlugin(RedirectingFormPlugin):
    ...
    def __init__(self, login_form_url, login_handler_path, logout_handler_path, rememberer_name, login_form_id='username', password_form_id='password', *args, **kwargs):
        RedirectingFormPlugin.__init__(self, login_form_url, login_handler_path, logout_handler_path, rememberer_name)
        self.login_form_id = login_form_id
        self.password_form_id = password_form_id
    ...
    def identify(self, environ):
        ...
        if path_info == self.logout_handler_path:
            ...
            form.update(query)
            try:
                login = form[self.login_form_id]
                password = form[self.password_form_id]
...
class DjangoAuthenticatorPlugin(object):
    ...
    def __init__(self, login_field='username'):
    ...
    def authenticate(self, environ, identity):
        ...
        try:
            user = User.objects.get(**{self.login_field: login})
...
class Django2ZwookUserModelPlugin(object):
    ...
    def add_metadata(self, environ, identity):
        ...
        user = User.objects.get(**{self.login_field: username})
        roles = ['Authenticated']
        roles.extend([ group['name'] for group in user.groups.values() ])
        user = {'login': username, 'roles': roles}
        identity.update(user)


def make_redirecting_plugin(login_form_url=None, login_handler_path='/login_handler', logout_handler_path='/logout_handler', login_form_id='username', password_form_id='password', rememberer_name=None):
    ...
    plugin = RedirectingDjangoFormPlugin(login_form_url, login_handler_path, logout_handler_path, rememberer_name, login_form_id=login_form_id, password_form_id=password_form_id)
    return plugin


def make_django_auth_plugin(login_field='username'):
    return DjangoAuthenticatorPlugin(login_field)