...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/heccer/__init__.py
...
class HeccerOptions(heccer_base.HeccerOptions):
    ...
    def __init__(self, options=0, corrections=0, interval_set=0, interval_start=-0.1, interval_end=0.05, activator_set=0, concentration_gate_start=4e-05, concentration_gate_end=0.3, interval_entries=3000, small_table_size=149):
...
class Heccer():
    ...
    def __init__(self, name='Untitled', model=None, pts=None, ped=None, peq=None, iOptions=None, dStep=None, pinter=None, filename=None):
        """!
        @brief Constructor
    ...
    def Construct(self, model=None):
        ...
        result = heccer_base.HeccerConstruct(heccer, model_source, model_name, None, None)
        if result == 1:
            self._is_constructed = True
        return result
    ...
    def SetParameter(self, path, field, value):
        ...
        if self.GetCore() is None:
            ...
            serial = self._model_source.GetSerial(path)
            result_msg = heccer_base.HeccerAddressableSet(self.GetCore(), serial, field, value)
    ...
    def GetCompartmentAddress(self, intermediary=-1, field='Vm'):
        ...
        if self.GetCore() is None:
            ...
            if not isinstance(intermediary, int):
                ...
                serial = intermediary
            address = heccer_base.HeccerAddressCompartmentVariable(self.GetCore(), serial, field)
            if address == None:
                from errors import HeccerAddressError
