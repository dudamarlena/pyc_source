...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_sugarcrm/views.py
...
class CRMUserViewSet(viewsets.ViewSet):
    ...
    def handle_exception(self, exc):
        if isinstance(exc, backend.SugarCRMBackendError):
            ...
            self.crm.save()
            return super(CRMUserViewSet, self).handle_exception(CRMNotOnline('Error appeared during request to CRM API.'))
    ...
    def get_filtered_users(self, request):
        supported_filters = [
         'first_name', 'last_name', 'user_name', 'status']
    ...
    def partial_update(self, request, crm_uuid, pk=None):
        ...
        if old_user is None or int(old_user.is_admin):
            ...
            serializer = serializers.CRMUserSerializer(data=request.data, instance=old_user, context=self.get_serializer_context())
            serializer.is_valid(raise_exception=True)
            ...
            signals.user_post_save.send(sender=models.CRM, old_user=old_user, new_user=new_user, crm=self.crm, created=False)
    ...
    def password(self, request, crm_uuid, pk=None):
        ...
        if serializer.validated_data.get('notify'):
            utils.sms_user_password(self.crm, user.phone_mobile, password)
        event_logger.sugarcrm_user.info('SugarCRM user {user_name} password has been reset.', event_type='sugarcrm_user_password_reset', event_context={'user_name': user.user_name, 
           'crm': self.crm})
