...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_sugarcrm/tasks.py
...
def provision_crm(crm_uuid):
    chain(schedule_crm_instance_provision.si(crm_uuid), wait_for_crm_template_group_provision.si(crm_uuid), init_crm_api_url.si(crm_uuid), init_crm_quotas.si(crm_uuid)).apply_async(link=set_online.si(crm_uuid), link_error=set_erred.si(crm_uuid))
...
def stop_and_destroy_crm(crm_uuid, force=False):
    ...
    chain(schedule_crm_instance_stopping.si(crm_uuid), wait_for_crm_instance_state.si(crm_uuid, state='Offline'), set_offline.si(crm_uuid), schedule_deletion.si(crm_uuid), schedule_crm_instance_deletion.si(crm_uuid)).apply_async(link=delete.si(crm_uuid), link_error=error_callback)
...
def wait_for_crm_instance_state(crm_uuid, state, erred_state='Erred'):
    ...
    logger.info('Checking state for CRM "%s" (UUID: %s) instance. Current value: %s.', crm.name, crm.uuid.hex, current_state)
    if current_state == erred_state:
        message = 'CRM "%s" (UUID: %s) instance with UUID %s become erred. Check OpenStack app logs for more details.' % (
         crm.name, crm.uuid.hex, details['uuid'])
...
def wait_for_crm_template_group_provision(crm_uuid):
    ...
    logger.info('Checking state of CRM "%s" (UUID: %s) provision result. Current state message: %s.', crm.name, crm.uuid.hex, details['state_message'])
    if details['is_erred']:
...
def init_crm_api_url(crm_uuid):
    ...
    if not external_ips:
        raise SugarCRMBackendError('Cannot use OpenStack instance with name "%s" for CRM - it does not have external IP.' % crm.name)
    crm.api_url = ('{protocol}://{external_ip}').format(protocol=settings.get_option('protocol'), external_ip=external_ips[0])
