...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_sugarcrm/serializers.py
...
class ServiceSerializer(structure_serializers.BaseServiceSerializer):
    SERVICE_ACCOUNT_FIELDS = {'backend_url': 'URL of template group that describes OpenStack instance provision with default parameters(required, e.g.: http://example.com/api/template-groups/16c7675752244f5d9e870a2cb0cfeb02/)', 
       'username': 'NodeConductor user username (e.g. Username)', 
       'password': 'NodeConductor user password (e.g. Password)'}
    SERVICE_ACCOUNT_EXTRA_FIELDS = {'license_code': 'License code that will be used for SugarCRM activation', 
       'user_data': 'User data that will be passed to CRMs OpenStack instance on creationWord {password} will be replaced with auto-generated admin password', 
...
class ServiceProjectLinkSerializer(structure_serializers.BaseServiceProjectLinkSerializer):
    ...
    class Meta(structure_serializers.BaseServiceProjectLinkSerializer.Meta):
        ...
        extra_kwargs = {'service': {'lookup_field': 'uuid', 'view_name': 'sugarcrm-detail'}}
...
class CRMSerializer(structure_serializers.PublishableResourceSerializer):
    service = serializers.HyperlinkedRelatedField(source='service_project_link.service', view_name='sugarcrm-detail', read_only=True, lookup_field='uuid')
    service_project_link = serializers.HyperlinkedRelatedField(view_name='sugarcrm-spl-detail', queryset=models.SugarCRMServiceProjectLink.objects.all())
    user_count = serializers.IntegerField(min_value=0, default=models.CRM.Quotas.user_count.default_limit, write_only=True)
    quotas = quotas_serializers.BasicQuotaSerializer(many=True, read_only=True)

    class Meta(structure_serializers.PublishableResourceSerializer.Meta):
        model = models.CRM
        view_name = 'sugarcrm-crms-detail'
        fields = structure_serializers.PublishableResourceSerializer.Meta.fields + ('api_url',
                                                                                    'user_count',
                                                                                    'quotas',
                                                                                    'instance_url')
        read_only_fields = structure_serializers.PublishableResourceSerializer.Meta.read_only_fields + ('api_url',
                                                                                                        'instance_url')
    ...
    def validate(self, attrs):
        if not self.instance:
            ...
            if quota.is_exceeded(delta=delta):
                raise serializers.ValidationError('Service project link users count quota is over limit (limit: %s, required: %s)' % (
    ...
    def validate(self, attrs):
        ...
        if not self.instance:
            ...
            if user_count_quota.is_exceeded(delta=1):
                raise serializers.ValidationError('User count quota is over limit (users count: %s, max users count: %s).' % (
