...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_sugarcrm/backend.py
# Compiled at: 2016-09-28 11:51:43
import logging, md5, urlparse
...
class SugarCRMBaseBackend(ServiceBackend):
    ...
    def provision(self, crm, user_count):
        ...
        send_task('sugarcrm', 'provision_crm')(crm.uuid.hex)

    def destroy(self, crm, force=False):
        crm.state = crm.States.STOPPING_SCHEDULED
        crm.save()
        send_task('sugarcrm', 'stop_and_destroy_crm')(crm.uuid.hex, force=force)
...
class SugarCRMBackend(SugarCRMBaseBackend):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.nc_client = self.NodeConductorOpenStackClient(self.template_url, self.settings.username, self.settings.password)
    ...
    def schedule_crm_instance_provision(self, crm):
        ...
        user_data = user_data.format(password=admin_password, license_code=self.settings.get_option('license_code'))
        template_data = [
         {'name': crm.name, 
            'user_data': user_data}]
        response = self.nc_client.post(self.template_url + 'provision/', json=template_data)
        if not response.ok:
            raise SugarCRMBackendError('Cannot provision OpenStack instance for CRM "%s": response code - %s, response content: %s.Request URL: %s, request body: %s' % (
    ...
    def schedule_crm_instance_stopping(self, crm):
        ...
        if not response.ok:
            raise SugarCRMBackendError('Cannot stop OpenStack instance for CRM "%s": response code - %s, response content: %s.Request URL: %s, request body: %s' % (
             crm.name, response.status_code, response.content, response.request.url, response.request.body))
    ...
    def schedule_crm_instance_deletion(self, crm):
        ...
        if not response.ok:
            raise SugarCRMBackendError('Cannot delete OpenStack instance for CRM "%s": response code - %s, response content: %s.Request URL: %s, request body: %s' % (
             crm.name, response.status_code, response.content, response.request.url, response.request.body))
    ...
    def pull_crm_sla(self, crm):
        ...
        for item in instance.sla_items.all():
            crm.sla_items.update_or_create(period=item.period, defaults={'value': item.value, 'agreed_value': item.agreed_value})

        for state_transition in instance.state_items.all():
            crm.state_items.update_or_create(period=state_transition.period, timestamp=state_transition.timestamp, defaults={'state': state_transition.state})
    ...
    def get_crm_instance_details(self, crm):
        ...
        if not response.ok:
            raise SugarCRMBackendError('Cannot get details of CRMs instance: response code - %s, response content: %s.Request URL: %s' % (
    ...
    def get_crm_template_group_result_details(self, crm):
        ...
        if not response.ok:
            raise SugarCRMBackendError('Cannot get CRMs provision result details: response code - %s, response content: %s.Request URL: %s' % (
             response.status_code, response.content, response.request.url))
        return response.json()
    ...
    def create_user(self, user_name, password, last_name, **kwargs):
        ...
        try:
            user = self.sugar_client.create_user(user_name=user_name, user_hash=encoded_password, last_name=last_name, **kwargs)
        except (requests.exceptions.RequestException, sugarcrm.SugarError) as e:
            raise SugarCRMBackendError('Cannot create user %s on CRM "%s". Error: %s' % (user_name, self.crm.name, e))
    ...
    def update_user(self, user, **kwargs):
        ...
        try:
            ...
            raise SugarCRMBackendError('Cannot update user %s on CRM "%s". Error: %s' % (user.user_name, self.crm.name, e))
    ...
    def delete_user(self, user):
        try:
            ...
            raise SugarCRMBackendError('Cannot delete user with id %s from CRM "%s". Error: %s' % (user.id, self.crm.name, e))
    ...
    def get_user(self, user_id):
        try:
            ...
            raise SugarCRMBackendError('Cannot get user with id %s from CRM "%s". Error: %s' % (user_id, self.crm.name, e))
    ...
    def get_stats(self):
        links = models.CRM.objects.filter(service_project_link__service__settings=self.settings)
        quota_values = models.CRM.get_sum_of_quotas_as_dict(links, quota_names=('user_count', ))
        stats = {'user_count_quota': quota_values.get('user_count', -1.0), 
