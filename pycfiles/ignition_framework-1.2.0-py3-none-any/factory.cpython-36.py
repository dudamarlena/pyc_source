...
# [GCC 8.4.0]
# Embedded file name: /home/dvs/src/accanto-github/ignition/ignition/templates/factory.py
# Compiled at: 2020-05-12 05:02:48
# Size of source mod 2**32: 8389 bytes
import logging, jinja2 as jinja, random, os, ignition, re
...
class DriverGenRequest:
    ...
    def __init__(self, driver_types, app_name, version='1.0.0', port=None, module_name=None, description=None, docker_name=None, helm_name=None, helm_node_port=None):
        ...
        self._DriverGenRequest__validate_module_name(module_name)
        self.module_name = module_name
        self.description = description
        if docker_name == None:
            docker_name = self.generate_docker_name(self.app_name)
        self._DriverGenRequest__validate_docker_name(docker_name)
        self.docker_name = docker_name
        if helm_name == None:
            helm_name = self.generate_helm_name(self.app_name)
        self._DriverGenRequest__validate_helm_name(helm_name)
...
class DriverProducer:
    ...
    def __render_template(self, template_path, render_props):
        ...
        for item in os.listdir(template_path):
            ...
            if os.path.isdir(full_item_path):
                self._DriverProducer__render_directory(template_path, template_env, file_name_env, full_item_path, self.target_location, render_props)
            else:
                self._DriverProducer__render_file(template_path, template_env, file_name_env, full_item_path, self.target_location, render_props)
    ...
    def __render_directory(self, template_base_path, template_env, file_name_env, template_dir_path, target_parent_path, render_props):
        ...
        for item in os.listdir(template_dir_path):
            ...
            if os.path.isdir(full_item_path):
                self._DriverProducer__render_directory(template_base_path, template_env, file_name_env, full_item_path, new_dir_path, render_props)
            else:
                self._DriverProducer__render_file(template_base_path, template_env, file_name_env, full_item_path, new_dir_path, render_props)
    ...
    def produce(self):
        templates = self._DriverProducer__determine_templates()
        self._DriverProducer__create_target_location()
        render_props = self._DriverProducer__create_render_props()
        for template in templates:
            self._DriverProducer__render_template(template, render_props)