...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.6/dist-packages/diffval/test.py
# Compiled at: 2010-10-25 18:32:46
import difflib, os
    ...
    def run(self):
        ...
        if self._session:
            ...
            if self._session._code != 0:
                if 'stdout' not in self._expects:
                ...
                if self._session._code is not None:
                    self._results['stderr'] += [
                     '<<< Session exited with code ' + str(self._session._code) + ' >>>']
                else:
                    self._results['stderr'] += [
    ...
    def _checksuccess(self):
        ...
        for pair in self._expects.keys():
            ...
            if pair == 'stdout':
                ...
                filename += '.err'
            if not diff(self._expects[pair], self._results[pair], filename, 'Actual output', log=self._log):
                ok = False
