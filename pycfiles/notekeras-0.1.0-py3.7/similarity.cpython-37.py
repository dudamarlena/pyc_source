...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/notekeras/model/similarity.py
# Compiled at: 2020-04-13 07:43:17
...
class SimilarityDataset(object):
    ...
    def __init__(self, annotation_path, tfrecord_path='./data/dataset.tfrecord', repeat=5, batch_size=4):
        ...
        with os.path.exists(self.tfrecord_path) or tf.io.TFRecordWriter(self.tfrecord_path) as (writer):
            for annotation in tqdm(self.annotations.values):
                ...
                features = tf.train.Features(feature={'label':tf.train.Feature(int64_list=tf.train.Int64List(value=[label])), 
                 ...
                 'image1':tf.train.Feature(bytes_list=tf.train.BytesList(value=[image1.astype(np.float32).tostring()])), 
                 'image2':tf.train.Feature(bytes_list=tf.train.BytesList(value=[image2.astype(np.float32).tostring()]))})
...
class SimilarityModel:
    ...
    def debug(self):
        ...
        plot_model((self.model), to_file=(self.root + 'data/models/similarity-body-expand.png'), show_shapes=True, expand_nested=True)

    def train(self, dataset: SimilarityDataset, log_dir='.data/similarity/log', lr=0.001, steps_per_epoch=None):
        ...
        self.model.fit((dataset.build()), steps_per_epoch=steps_per_epoch,
          callbacks=[
