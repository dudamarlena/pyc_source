...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/notekeras/layer/embedding.py
# Compiled at: 2020-04-03 03:36:32
...
class TokenEmbedding(Embedding):
    __doc__ = 'Embedding layer with weights returned.'

    def compute_output_shape(self, input_shape):
        return [
         super(TokenEmbedding, self).compute_output_shape(input_shape), (self.input_dim, self.output_dim)]

    def compute_mask(self, inputs, mask=None):
        return [
         super(TokenEmbedding, self).compute_mask(inputs, mask), None]

    def call(self, inputs):
        return [
         super(TokenEmbedding, self).call(inputs), tf.identity(self.embeddings)]


class EmbeddingSimilarity(Layer):
    __doc__ = 'Calculate similarity between features and token embeddings with bias term.'
...
class EmbeddingSim(Layer):
    ...
    def __init__(self, use_bias=True, initializer='zeros', regularizer=None, constraint=None, stop_gradient=False, **kwargs):
        """Initialize the layer.
    ...
    def build(self, input_shape):
        if self.use_bias:
            ...
            self.bias = self.add_weight(shape=(token_num,), initializer=(self.initializer),
              regularizer=(self.regularizer),
              constraint=(self.constraint),
              name='bias')
...
class EmbeddingRet(Embedding):
    ...
    def compute_output_shape(self, input_shape):
        return [
         super(EmbeddingRet, self).compute_output_shape(input_shape), (self.input_dim, self.output_dim)]

    def compute_mask(self, inputs, mask=None):
        return [
         super(EmbeddingRet, self).compute_mask(inputs, mask), None]

    def call(self, inputs):
        return [
         super(EmbeddingRet, self).call(inputs), tf.identity(self.embeddings)]


class PositionEmbedding(Layer):
    __doc__ = 'Turn integers (positions) into dense vectors of fixed size.\n    eg. [[-4], [10]] -> [[0.25, 0.1], [0.6, -0.2]]\n\n    Expand mode: negative integers (relative position) could be used in this mode.\n        # Input shape\n            2D tensor with shape: `(batch_size, sequence_length)`.\n\n        # Output shape\n            3D tensor with shape: `(batch_size, sequence_length, output_dim)`.\n\n    Add mode:\n        # Input shape\n            3D tensor with shape: `(batch_size, sequence_length, feature_dim)`.\n\n        # Output shape\n            3D tensor with shape: `(batch_size, sequence_length, feature_dim)`.\n\n    Concat mode:\n        # Input shape\n            3D tensor with shape: `(batch_size, sequence_length, feature_dim)`.\n\n        # Output shape\n            3D tensor with shape: `(batch_size, sequence_length, feature_dim + output_dim)`.\n    '
    MODE_EXPAND = 'expand'
    MODE_ADD = 'add'
    MODE_CONCAT = 'concat'

    def __init__(self, input_dim, output_dim, mode=MODE_EXPAND, embeddings_initializer='uniform', embeddings_regularizer=None, activity_regularizer=None, embeddings_constraint=None, mask_zero=False, **kwargs):
        """
    ...
    def build(self, input_shape):
        if self.mode == self.MODE_EXPAND:
            self.embeddings = self.add_weight(shape=(
             self.input_dim * 2 + 1, self.output_dim),
    ...
    def call(self, inputs, **kwargs):
        if self.mode == self.MODE_EXPAND:
            ...
            return K.gather(self.embeddings, K.minimum(K.maximum(inputs, -self.input_dim), self.input_dim) + self.input_dim)
            input_shape = K.shape(inputs)
            if self.mode == self.MODE_ADD:
                batch_size, seq_len, output_dim = input_shape[0], input_shape[1], input_shape[2]
            else:
                batch_size, seq_len, output_dim = input_shape[0], input_shape[1], self.output_dim
        pos_embeddings = K.tile(K.expand_dims((self.embeddings[:seq_len, :self.output_dim]), axis=0), [
         batch_size, 1, 1])
        if self.mode == self.MODE_ADD:
...
class TrigPosEmbedding(Layer):
    __doc__ = 'Position embedding use sine and cosine functions.\n\n    See: https://arxiv.org/pdf/1706.03762\n\n    Expand mode:\n        # Input shape\n            2D tensor with shape: `(batch_size, sequence_length)`.\n\n        # Output shape\n            3D tensor with shape: `(batch_size, sequence_length, output_dim)`.\n\n    Add mode:\n        # Input shape\n            3D tensor with shape: `(batch_size, sequence_length, feature_dim)`.\n\n        # Output shape\n            3D tensor with shape: `(batch_size, sequence_length, feature_dim)`.\n\n    Concat mode:\n        # Input shape\n            3D tensor with shape: `(batch_size, sequence_length, feature_dim)`.\n\n        # Output shape\n            3D tensor with shape: `(batch_size, sequence_length, feature_dim + output_dim)`.\n    '
    MODE_EXPAND = 'expand'
    MODE_ADD = 'add'
    MODE_CONCAT = 'concat'
    ...
    def call(self, inputs, mask=None):
        ...
        even_embd = K.sin(K.dot(K.expand_dims(pos_input, -1), K.expand_dims(1.0 / K.pow(10000.0, K.cast(evens, K.floatx()) / K.cast(output_dim, K.floatx())), 0)))
        odd_embd = K.cos(K.dot(K.expand_dims(pos_input, -1), K.expand_dims(1.0 / K.pow(10000.0, K.cast(odds - 1, K.floatx()) / K.cast(output_dim, K.floatx())), 0)))
...
def get_embedding(inputs, token_num, pos_num, embed_dim, dropout_rate=0.1, trainable=True):
    ...
    """
    embeddings = [
     TokenEmbedding(input_dim=token_num, output_dim=embed_dim,
       mask_zero=True,
       trainable=trainable,
       name='Embedding-Token')(inputs[0]),
     Embedding(input_dim=2, output_dim=embed_dim,
       trainable=trainable,
       name='Embedding-Segment')(inputs[1])]
    embeddings[0], embed_weights = embeddings[0]
    embed_layer = Add(name='Embedding-Token-Segment')(embeddings)
    embed_layer = PositionEmbedding(input_dim=pos_num, output_dim=embed_dim,
      mode=(PositionEmbedding.MODE_ADD),
      trainable=trainable,
      name='Embedding-Position')(embed_layer)
    return (embed_layer, embed_weights)


def get_custom_objects():
    return {'EmbeddingRet':EmbeddingRet, 
