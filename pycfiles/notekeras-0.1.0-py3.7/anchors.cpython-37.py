...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/notekeras/model/retinanet/utils/anchors.py
# Compiled at: 2020-04-25 22:49:17
# Size of source mod 2**32: 13139 bytes
import numpy as np
import tensorflow.keras.backend as K
import keras_retinanet.utils.compute_overlap as compute_overlap

class AnchorParameters:
    __doc__ = ' The parameteres that define how anchors are generated.\n\n    Args\n        sizes   : List of sizes to use. Each size corresponds to one feature level.\n        strides : List of strides to use. Each stride correspond to one feature level.\n        ratios  : List of ratios to use per location in a feature map.\n        scales  : List of scales to use per location in a feature map.\n    '
...
AnchorParameters.default = AnchorParameters(sizes=[
 32, 64, 128, 256, 512],
  strides=[
 8, 16, 32, 64, 128],
  ratios=(np.array([0.5, 1, 2], K.floatx())),
  scales=(np.array([1, 1.2599210498948732, 1.5874010519681994], K.floatx())))
anchor_parameter_default = AnchorParameters(sizes=[
 32, 64, 128, 256, 512],
  strides=[
 8, 16, 32, 64, 128],
  ratios=(np.array([0.5, 1, 2], K.floatx())),
...
def anchor_targets_bbox(anchors, image_group, annotations_group, num_classes, negative_overlap=0.4, positive_overlap=0.5):
    ...
    regression_batch = np.zeros((batch_size, anchors.shape[0], 5), dtype=(K.floatx()))
...
def shift(shape, stride, anchors):
    ...
    A = anchors.shape[0]
    K = shifts.shape[0]
    all_anchors = anchors.reshape((1, A, 4)) + shifts.reshape((1, K, 4)).transpose((1,
                                                                                    0,
                                                                                    2))
