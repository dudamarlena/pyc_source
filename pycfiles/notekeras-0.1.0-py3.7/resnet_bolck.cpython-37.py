...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/notekeras/component/resnet_bolck.py
# Compiled at: 2020-04-22 05:02:36
# Size of source mod 2**32: 27544 bytes
...
def basic_1d(filters, stage=0, block=0, kernel_size=3, numerical_name=False, stride=None, freeze_bn=False):
    ...
    def f(x):
        y = keras.layers.ZeroPadding1D(padding=1,
 ...
 kernel_size, use_bias=False, 
        ...
        if block == 0:
            shortcut = (keras.layers.Conv1D)(filters,
 1, strides=stride, 
             use_bias=False, 
...
def bottleneck_1d(filters, stage=0, block=0, kernel_size=3, numerical_name=False, stride=None, freeze_bn=False):
 ...
 1, use_bias=False, 
        ...
        if block == 0:
            shortcut = (keras.layers.Conv1D)(filters * 4,
 1, strides=stride, 
             use_bias=False, 
             name='res{}{}_branch1'.format(stage_char, block_char), **parameters)(x)
            shortcut = BatchNormalizationFreeze(axis=axis,
...
def basic_2d(filters, stage=0, block=0, kernel_size=3, numerical_name=False, stride=None, freeze_bn=False):
    ...
    def f(x):
        ...
        y = BatchNormalizationFreeze(axis=axis, epsilon=1e-05, freeze=freeze_bn, name=('bn{}{}_branch2a'.format(stage_char, block_char)))(y)
        ...
        y = BatchNormalizationFreeze(axis=axis, epsilon=1e-05, freeze=freeze_bn, name=('bn{}{}_branch2b'.format(stage_char, block_char)))(y)
        if block == 0:
            ...
            shortcut = BatchNormalizationFreeze(axis=axis, epsilon=1e-05, freeze=freeze_bn, name=('bn{}{}_branch1'.format(stage_char, block_char)))(shortcut)
        else:
            shortcut = x
...
def bottleneck_2d(filters, stage=0, block=0, kernel_size=3, numerical_name=False, stride=None, freeze_bn=False):
    ...
    def f(x):
        ...
        y = BatchNormalizationFreeze(axis=axis, epsilon=1e-05, freeze=freeze_bn, name=('bn{}{}_branch2c'.format(stage_char, block_char)))(y)
        if block == 0:
...
def time_distributed_basic_2d(filters, stage=0, block=0, kernel_size=3, numerical_name=False, stride=None, freeze_bn=False):
    ...
    def f(x):
        ...
        y = keras.layers.TimeDistributed((keras.layers.Conv2D)(filters, kernel_size, strides=stride, use_bias=False, **parameters),
          name=('res{}{}_branch2a'.format(stage_char, block_char)))(y)
        y = keras.layers.TimeDistributed(BatchNormalizationFreeze(axis=axis, epsilon=1e-05, freeze=freeze_bn),
          name=('bn{}{}_branch2a'.format(stage_char, block_char)))(y)
        ...
        y = keras.layers.TimeDistributed(BatchNormalizationFreeze(axis=axis, epsilon=1e-05, freeze=freeze_bn),
          name=('bn{}{}_branch2b'.format(stage_char, block_char)))(y)
        if block == 0:
            shortcut = keras.layers.TimeDistributed((keras.layers.Conv2D)(filters, (1, 1), strides=stride, use_bias=False, **parameters),
              name=('res{}{}_branch1'.format(stage_char, block_char)))(x)
            shortcut = keras.layers.TimeDistributed(BatchNormalizationFreeze(axis=axis, epsilon=1e-05, freeze=freeze_bn),
              name=('bn{}{}_branch1'.format(stage_char, block_char)))(shortcut)
        else:
            shortcut = x
        y = keras.layers.Add(name=('res{}{}'.format(stage_char, block_char)))([y, shortcut])
...
def time_distributed_bottleneck_2d(filters, stage=0, block=0, kernel_size=3, numerical_name=False, stride=None, freeze_bn=False):
    ...
    def f(x):
        y = keras.layers.TimeDistributed((keras.layers.Conv2D)(filters, (1, 1), strides=stride, use_bias=False, **parameters),
        ...
        y = keras.layers.TimeDistributed(BatchNormalizationFreeze(axis=axis, epsilon=1e-05, freeze=freeze_bn),
          name=('bn{}{}_branch2c'.format(stage_char, block_char)))(y)
        if block == 0:
            shortcut = keras.layers.TimeDistributed((keras.layers.Conv2D)(filters * 4, (1, 1), strides=stride, use_bias=False, **parameters),
              name=('res{}{}_branch1'.format(stage_char, block_char)))(x)
            shortcut = keras.layers.TimeDistributed(keras.layers.BatchNormalization(axis=axis, epsilon=1e-05, freeze=freeze_bn),
              name=('bn{}{}_branch1'.format(stage_char, block_char)))(shortcut)
        else:
            shortcut = x
        y = keras.layers.Add(name=('res{}{}'.format(stage_char, block_char)))([y, shortcut])
