...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/notekeras/utils/util.py
...
def extract_embeddings_generator(model, texts, poolings=None, vocabs=None, cased=False, batch_size=4, cut_embed=True, output_layer_num=1):
    ...
    """
    if isinstance(model, (str, type(''))):
        paths = get_checkpoint_paths(model)
        model = load_trained_model_from_checkpoint(config_file=(paths.config),
    ...
    def _batch_generator():
        ...
        def _pad_inputs():
            ...
            return [
             np.array(tokens), np.array(segments)]

        for text in texts:
            if isinstance(text, (str, type(''))):
    ...
    if poolings is not None:
        if isinstance(poolings, (str, type(''))):
            poolings = [
             poolings]
        else:
            outputs = []
...
def extract_embeddings(model, texts, poolings=None, vocabs=None, cased=False, batch_size=4, cut_embed=True, output_layer_num=1):
    """Extract embeddings from texts.
    ...
    :return: A list of numpy arrays representing the embeddings.
    """
    return [embedding for embedding in extract_embeddings_generator(model, texts, poolings, vocabs, cased, batch_size, cut_embed, output_layer_num)]