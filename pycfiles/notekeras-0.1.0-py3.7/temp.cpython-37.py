...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/notekeras/model/temp.py
# Compiled at: 2020-04-18 07:40:53
# Size of source mod 2**32: 26470 bytes
...
def preprocess_input(x, data_format=None, version=1):
    ...
    elif not data_format in {'channels_first', 'channels_last'}:
            ...
            if pooling == 'max':
                ...
                if weights == 'vggface':
                    if include_top:
                        ...
                        weights_path = get_file('rcmalli_vggface_tf_notop_resnet50.h5', RESNET50_WEIGHTS_PATH_NO_TOP, cache_subdir=VGGFACE_DIR)
...
def VGGFace(include_top=True, model='vgg16', weights='vggface', input_tensor=None, input_shape=None, pooling=None, classes=None):
    ...
    """
    if weights not in {None, 'vggface'}:
        raise ValueError('The `weights` argument should be either `None` (random initialization) or `vggface` (pre-training on VGGFace Datasets).')
    if model == 'vgg16':
        if classes is None:
            classes = 2622
        if weights == 'vggface':
    ...
    if model == 'resnet50':
        ...
        return RESNET50(include_top=include_top, input_tensor=input_tensor, input_shape=input_shape, pooling=pooling, weights=weights,
          classes=classes)
    if model == 'senet50':
        if classes is None:
            classes = 8631
