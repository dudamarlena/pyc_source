...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/notekeras/tokenizer.py
# Compiled at: 2019-12-03 07:55:50
# Size of source mod 2**32: 12689 bytes
import unicodedata
TOKEN_PAD = '[PAD]'
TOKEN_UNK = '[UNK]'
TOKEN_CLS = '[CLS]'
...
TOKEN_END = '[END]'
__all__ = [
 'get_base_dict', 'TOKEN_PAD', 'TOKEN_UNK', 'TOKEN_CLS', 'TOKEN_SEP', 'TOKEN_MASK', 'TOKEN_START',
 'TOKEN_END']
...
class Tokenizer2(object):
    ...
    def __init__(self, tokens=None, token_cls=TOKEN_CLS, token_sep=TOKEN_SEP, token_unk=TOKEN_UNK, pad_index=0):
...
class Tokenizer(object):
    ...
    def _tokenize(self, text):
        ...
        spaced = ''
        for ch in text:
            if not self._is_punctuation(ch):
                ...
                if self._is_space(ch):
                    spaced += ' '
            if not ord(ch) == 0:
                if ord(ch) == 65533 or self._is_control(ch):
                    continue
                else:
                    spaced += ch
