...
# Compiled at: 2020-04-17 23:33:08
...
IOU_LOSS_THRESH = 0.5
md5 = [
 'd985be8c5e5a2346dbc2ea1aedd7157d', 'aef3adffaec9270e6c4e1c2dcd13792a', '34d0f01fe6a13b2924ff363e7660ed47',
...
class Dataset(object):
    ...
    def __init__(self, annotation_path, tfrecord_path='./data/dataset.tfrecord', anchors=None, classes=None, repeat=5, batch_size=4):
        ...
        self.dataset = None
        self.build()
        self.dataset_iterator = self.dataset.batch(batch_size).repeat(repeat)
    ...
    def build(self):
        with os.path.exists(self.tfrecord_path) or tf.io.TFRecordWriter(self.tfrecord_path) as (writer):
            for annotation in tqdm(self.annotations):
                image, label_box1, label_box2, label_box3 = self.parse_annotations(annotation)
                features = tf.train.Features(feature={'image':tf.train.Feature(bytes_list=tf.train.BytesList(value=[image.astype(np.float32).tostring()])), 
                 'label1':tf.train.Feature(bytes_list=tf.train.BytesList(value=[label_box1.astype(np.float32).tostring()])), 
                 'label2':tf.train.Feature(bytes_list=tf.train.BytesList(value=[label_box2.astype(np.float32).tostring()])), 
                 'label3':tf.train.Feature(bytes_list=tf.train.BytesList(value=[label_box3.astype(np.float32).tostring()]))})
        ...
        def parse_fn(example_proto):
            ...
            return (
             (
              image, label1, label2, label3), np.zeros(image.shape[0]))
    ...
    def preprocess_true_boxes(self, true_boxes):
        ...
        assert (true_boxes[(Ellipsis, 4)] < self.num_classes).all(), 'class id must be less than num_classes'
        ...
        y_true = [np.zeros((m, grid_shapes[l][0], grid_shapes[l][1], len(anchor_mask[l]), 5 + self.num_classes), dtype='float32') for l in range(num_layers)]
        anchors = np.expand_dims(self.anchors, 0)
        anchor_maxes = anchors / 2.0
        anchor_mins = -anchor_maxes
        valid_mask = boxes_wh[(Ellipsis, 0)] > 0
        for b in range(m):
            ...
            intersect_area = intersect_wh[(Ellipsis, 0)] * intersect_wh[(Ellipsis,
                                                                         1)]
            box_area = wh[(Ellipsis, 0)] * wh[(Ellipsis, 1)]
            anchor_area = anchors[(Ellipsis, 0)] * anchors[(Ellipsis, 1)]
    ...
    def get_random_data(self, annotation_line, random=True, max_boxes=20, jitter=0.3, hue=0.1, sat=1.5, val=1.5, proc_img=True):
        ...
        def hsv_to_rgb(hsv):
            ...
            in_shape = hsv.shape
            hsv = np.array(hsv,
              copy=False, dtype=(np.promote_types(hsv.dtype, np.float32)),
              ndmin=2)
            h = hsv[(Ellipsis, 0)]
            s = hsv[(Ellipsis, 1)]
            v = hsv[(Ellipsis, 2)]
        ...
        def rgb_to_hsv(arr):
            ...
            arr = np.asarray(arr)
            if arr.shape[(-1)] != 3:
                raise ValueError('Last dimension of input array must be 3; shape {} was found.'.format(arr.shape))
            in_shape = arr.shape
            arr = np.array(arr,
              copy=False, dtype=(np.promote_types(arr.dtype, np.float32)),
              ndmin=2)
            ...
            ipos = delta > 0
            idx = (arr[(Ellipsis, 0)] == arr_max) & ipos
            out[(idx, 0)] = (arr[(idx, 1)] - arr[(idx, 2)]) / delta[idx]
            idx = (arr[(Ellipsis, 1)] == arr_max) & ipos
            out[(idx, 0)] = 2.0 + (arr[(idx, 2)] - arr[(idx, 0)]) / delta[idx]
            idx = (arr[(Ellipsis, 2)] == arr_max) & ipos
            out[(idx, 0)] = 4.0 + (arr[(idx, 0)] - arr[(idx, 1)]) / delta[idx]
            out[(Ellipsis, 0)] = out[(Ellipsis, 0)] / 6.0 % 1.0
            out[(Ellipsis, 1)] = s
            out[(Ellipsis, 2)] = arr_max
        ...
        x[(Ellipsis, 0)] += hue
        x[(Ellipsis, 0)][(x[(Ellipsis, 0)] > 1)] -= 1
        x[(Ellipsis, 0)][(x[(Ellipsis, 0)] < 0)] += 1
        x[(Ellipsis, 1)] *= sat
        x[(Ellipsis, 2)] *= val
        x[x > 1] = 1
        x[x < 0] = 0
...
class YoloDataset(object):
    ...
    def build(self):
        ...
        for annotation in annotations:
            image, label_box1, label_box2, label_box3 = self.parse_annotations(annotation)
            inputs = (
             image, label_box1, label_box2, label_box3)
...
class YoloBody:
    ...
    def build(self):
        ...
        model_loss = Lambda(yolo_loss, output_shape=(4, ),
         ...
         'num_classes':self.num_classes,  'ignore_thresh':0.5})([
    ...
    def train(self, dataset: Dataset, log_dir='.data/yolo/log', lr=0.001, epochs=1, steps_per_epoch=None):
        logging = TensorBoard(log_dir=log_dir)
        checkpoint = ModelCheckpoint((log_dir + '/checkpoint'),
          monitor='val_loss')
        reduce_lr = ReduceLROnPlateau(monitor='loss', factor=0.1, patience=3, verbose=1)
        early_stopping = EarlyStopping(monitor='loss', min_delta=0, patience=10, verbose=1)
        self.train_model.compile(optimizer=Adam(lr=lr), loss={'loss2': lambda y_true0, y_pred: y_pred})
        self.train_model.fit((dataset.dataset_iterator), epochs=epochs,
          steps_per_epoch=steps_per_epoch,
          callbacks=[
         logging, checkpoint, reduce_lr, early_stopping])

    def train_iterator(self, dataset: YoloDataset, lr=0.001, steps_per_epoch=None, epochs=1):
        reduce_lr = ReduceLROnPlateau(monitor='loss', factor=0.5, patience=1, verbose=1, min_lr=1e-05)
        early_stopping = EarlyStopping(monitor='loss', min_delta=0, patience=10, verbose=1)
        self.train_model.compile(optimizer=optimizers.Adadelta(lr=lr), loss={'xy':lambda y_true0, y_pred: y_pred, 
         ...
         'conf':1.0, 
         'class':1.0})
        self.train_model.fit((dataset.build()), epochs=epochs,
          steps_per_epoch=steps_per_epoch,
          callbacks=[
    ...
    def predict_result(self, image_path=None, original_image=None):
        ...
        return (
         original_image, self.decodes(self.yolo_model.predict(image_data)[::-1]))
    ...
    def debug(self):
        ...
        plot_model((self.yolo_model), to_file=(self.root + 'models/yolo-body-expand.png'), show_shapes=True, expand_nested=True)
        plot_model((self.train_model), to_file=(self.root + 'models/yolo-train.png'), show_shapes=True)
        plot_model((self.train_model), to_file=(self.root + 'models/yolo-train-expand.png'), show_shapes=True, expand_nested=True)
