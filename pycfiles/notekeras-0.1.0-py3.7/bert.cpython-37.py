...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/notekeras/model/bert.py
# Compiled at: 2020-04-03 02:36:22
# Size of source mod 2**32: 22819 bytes
import numpy as np
...
import notekeras.component.transformer as get_encoder_custom_objects
...
from notekeras.layer.extract import Extract
...
class BertModel(keras.models.Model):
    ...
    def __init__(self, token_num, pos_num=512, seq_len=512, embed_dim=768, transformer_num=12, head_num=12, feed_forward_dim=3072, dropout_rate=0.1, attention_activation=None, feed_forward_activation='gelu', training=True, trainable=None, output_layer_num=1, use_task_embed=False, task_num=10, use_adapter=False, adapter_units=None, **kwargs):
        """Get BERT model.
    ...
    def _build(self):
        if self.attention_activation == 'gelu':
            ...
            if self.feed_forward_activation == 'gelu':
                ...
                embed_layer, embed_weights = get_embedding(inputs, token_num=(self.token_num),
                  embed_dim=(self.embed_dim),
                  pos_num=(self.pos_num),
                  dropout_rate=(self.dropout_rate))
                if self.use_task_embed:
                    task_input = keras.layers.Input(shape=(1, ), name='Input-Task')
                    embed_layer = TaskEmbedding(input_dim=(self.task_num), output_dim=(self.embed_dim),
                      mask_zero=False,
                      name='Embedding-Task')([
            ...
            transformed = EncoderList(encoder_num=(self.transformer_num), head_num=(self.head_num),
              ...
              adapter_activation=gelu)(embed_layer)
            if self.training:
                mlm_dense_layer = keras.layers.Dense(units=(self.embed_dim), activation=(self.feed_forward_activation),
                  name='MLM-Dense')(transformed)
                mlm_norm_layer = LayerNormalization(name='MLM-Norm')(mlm_dense_layer)
                mlm_pred_layer = EmbeddingSimilarity(name='MLM-Sim')([mlm_norm_layer, embed_weights])
                masked_layer = Masked(name='MLM')([mlm_pred_layer, inputs[(-1)]])
                extract_layer = Extract(index=0, name='Extract')(transformed)
                nsp_dense_layer = keras.layers.Dense(units=(self.embed_dim), activation='tanh', name='NSP-Dense')(extract_layer)
    ...
    def get_non_train(self):
        ...
        return (
         self.inputs, transformed)

    def compile_model(self, weight_decay=0.01, decay_steps=100000, warmup_steps=10000, learning_rate=0.0001):
        ...
        """
        self.compile(optimizer=AdamWarmup(decay_steps=decay_steps, warmup_steps=warmup_steps,
...
def get_model(token_num, pos_num=512, seq_len=512, embed_dim=768, transformer_num=12, head_num=12, feed_forward_dim=3072, dropout_rate=0.1, attention_activation=None, feed_forward_activation='gelu', training=True, trainable=None, output_layer_num=1, use_task_embed=False, task_num=10, use_adapter=False, adapter_units=None):
    """Get BERT model.
    ...
    if attention_activation == 'gelu':
        ...
        if feed_forward_activation == 'gelu':
            ...
            embed_layer, embed_weights = get_embedding(inputs, token_num=token_num,
              embed_dim=embed_dim,
              pos_num=pos_num,
              dropout_rate=dropout_rate)
            if use_task_embed:
                task_input = keras.layers.Input(shape=(1, ), name='Input-Task')
                embed_layer = TaskEmbedding(input_dim=task_num, output_dim=embed_dim,
                  mask_zero=False,
                  name='Embedding-Task')([
                 embed_layer, task_input])
                inputs = inputs[:2] + [task_input, inputs[(-1)]]
        ...
        transformed = EncoderList(encoder_num=transformer_num, head_num=head_num,
          ...
          layer_depth=5)(embed_layer)
        if training:
            mlm_dense_layer = keras.layers.Dense(units=embed_dim, activation=feed_forward_activation,
              name='MLM-Dense')(transformed)
            mlm_norm_layer = LayerNormalization(name='MLM-Norm')(mlm_dense_layer)
            mlm_pred_layer = EmbeddingSimilarity(name='MLM-Sim')([mlm_norm_layer, embed_weights])
            masked_layer = Masked(name='MLM')([mlm_pred_layer, inputs[(-1)]])
...
def compile_model(model, weight_decay=0.01, decay_steps=100000, warmup_steps=10000, learning_rate=0.0001):
    """Compile the model with warmup optimizer and sparse cross-entropy loss.
    ...
    :return: The compiled model.
    """
    model.compile(optimizer=AdamWarmup(decay_steps=decay_steps, warmup_steps=warmup_steps,
      learning_rate=learning_rate,
      weight_decay=weight_decay,
      weight_decay_pattern=[
     'embeddings', 'kernel', 'W1', 'W2', 'Wk', 'Wq', 'Wv',
     'Wo']),
      loss=(keras.losses.sparse_categorical_crossentropy))
...
def gen_batch_inputs(sentence_pairs, token_dict, token_list, seq_len=512, mask_rate=0.15, mask_mask_rate=0.8, mask_random_rate=0.1, swap_sentence_rate=0.5, force_mask=True):
        ...
        for token in tokens:
            ...
            if token not in base_dict and np.random.random() < mask_rate:
                ...
                if r < mask_mask_rate:
                    ...
                    if r < mask_mask_rate + mask_random_rate:
                        while True:
