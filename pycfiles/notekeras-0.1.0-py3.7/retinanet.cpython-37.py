...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/notekeras/model/retinanet/models/retinanet.py
# Compiled at: 2020-04-26 08:34:34
# Size of source mod 2**32: 15478 bytes
import numpy as np
from tensorflow import keras
import tensorflow.keras as K
from tensorflow.keras.layers import Conv2D, Add, Activation, Input, Permute, Reshape, Concatenate
...
class RetinaNetModel(Model):
    ...
    def default_submodels(self):
        ...
        return [
         (
          'regression', self.default_regression_model(4)),
         (
          'classification', self.default_classification_model())]

    def default_regression_model(self, num_values, pyramid_feature_size=256, regression_feature_size=256, name='regression_submodel'):
    ...
    def default_classification_model(self, pyramid_feature_size=256, prior_probability=0.01, classification_feature_size=256, name='classification_submodel'):
        ...
        outputs = inputs
        for i in range(4):
            outputs = Conv2D(filters=classification_feature_size, activation='relu', 
             name='pyramid_classification_{}'.format(i), 
             kernel_initializer=keras.initializers.glorot_normal(seed=None), 
             bias_initializer='zeros', **options)(outputs)

        outputs = Conv2D(filters=self.num_classes * self.num_anchors, kernel_initializer=keras.initializers.glorot_normal(seed=None), 
         bias_initializer=PriorProbability(probability=prior_probability), 
         name='pyramid_classification', **options)(outputs)
        if K.image_data_format() == 'channels_first':
...
class RetinaNetBox(Model):
    ...
    def __init__(self, model=None, nms=True, class_specific_filter=True, name='retinanet-bbox', anchor_params=None, nms_threshold=0.5, score_threshold=0.05, max_detections=300, parallel_iterations=32, **kwargs):
        ...
        detections = FilterDetections(nms=nms, class_specific_filter=class_specific_filter,
