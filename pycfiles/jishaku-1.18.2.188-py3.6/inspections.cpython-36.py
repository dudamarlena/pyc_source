...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jishaku/repl/inspections.py
...
"""
import collections, functools, inspect, os
INSPECTIONS = []
...
def compat_operation_inspection(obj):
    ...
    for operation, member in POSSIBLE_OPS.items():
        if f"__{member}__" in this_dict:
            if check_not_slot(obj, f"__{member}__"):
                operations.append(operation)
            else:
                if f"__r{member}__" in this_dict:
                    if check_not_slot(obj, f"r__{member}__"):
                        operations.append(operation)
