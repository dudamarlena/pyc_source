...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jishaku/paginators.py
# Compiled at: 2020-03-05 23:53:06
# Size of source mod 2**32: 15026 bytes
"""
jishaku.paginators
~~~~~~~~~~~~~~~~~~
...
__all__ = ('EmojiSettings', 'PaginatorInterface', 'PaginatorEmbedInterface', 'WrappedPaginator',
           'FilePaginator')
EmojiSettings = collections.namedtuple('EmojiSettings', 'start back forward end close')
EMOJI_DEFAULT = EmojiSettings(start='⏮',
  back='◀',
  forward='▶',
  end='⏭',
  close='⏹')
...
class PaginatorInterface:
    ...
    def __init__(self, bot: commands.Bot, paginator: commands.Paginator, **kwargs):
        ...
        self.task = None
        self.send_lock = asyncio.Event()
        self.update_lock = asyncio.Semaphore(value=(kwargs.pop('update_max', 2)))
        if self.page_size > self.max_page_size:
            raise ValueError(f"Paginator passed has too large of a page size for this interface. ({self.page_size} > {self.max_page_size})")
    ...
    async def send_to(self, destination: discord.abc.Messageable):
        ...
        if not self.sent_page_reactions:
...
class WrappedPaginator(commands.Paginator):
    ...
    def add_line(self, line='', *, empty=False):
        ...
        while len(line) > true_max_size:
            ...
            if wrapped or self.force_wrap:
                super().add_line(line[0:true_max_size - 1])
                line = line[true_max_size - 1:]
            else:
                raise ValueError(f"Line of length {original_length} had sequence of {len(line)} characters (max is {true_max_size}) that WrappedPaginator could not wrap with delimiters: {self.wrap_on}")
...
class FilePaginator(commands.Paginator):
    ...
    _FilePaginator__encoding_regex = re.compile(b'coding[=:]\\s*([-\\w.]+)')
    ...
    def __init__(self, fp, line_span=None, language_hints=(), **kwargs):
        ...
        try:
            ...
            encoding_match = self._FilePaginator__encoding_regex.search(raw_content[:128])
            if encoding_match:
