...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mueddit/leven.py
...
class RelPos:
    __doc__ = 'Position ("i^#e"), except it\'s relative to some outside i.'
    ...
    def subsumes(self, other):
        ...
        if r <= 0:
            return False
        return abs(other.offset - self.offset) <= r
...
class CharVec:
    __doc__ = 'Characteristic (_not_ character) vector.'
    ...
    def subrange(self, sz, sh):
        ...
        if sz:
            return CharVec(self.bits >> sh - 1 & power_mask[(sz - 1)], sz)
        return CharVec(0, 0)
...
class Elementary:
    ...
    def elem_delta(self, i, w, rel_pos, char_vec):
        ...
        if rel_pos.edit < self.n:
            return self.deltaI(rel_pos, loc_char_vec)
        return self.deltaII(rel_pos, loc_char_vec)
