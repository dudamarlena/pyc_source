...
# [GCC 8.4.0]
# Embedded file name: /home/edward/github/ImageDataExtractor/imagedataextractor/rdf_functions.py
# Compiled at: 2019-11-27 09:58:23
# Size of source mod 2**32: 7508 bytes
import matplotlib.pyplot as plt, math, logging
...
def particle_size_histogram(arealist, filtered, imgname, outputpath='', conversion=0):
    ...
    plt.hist(filtered, bins=(len(arealist) + 1), range=(bins.min(), bins.max()), edgecolor='black', linewidth=1.2, rwidth=0.9,
      label='Filtered',
      alpha=0.6,
      color='darkorange')
...
def remove_outliers(areas):
    if len(areas) > 1:
        mu = np.median(areas)
        std = np.std(areas)
        filtered = [x for x in areas if x < mu + 1.75 * std if x > mu - 1 * std]
    else:
        filtered = areas
    return filtered