...
# [GCC 8.4.0]
# Embedded file name: imagedataextractor/photo_splitting.py
...
def python_2_unicode_compatible(klass):
    ...
    if six.PY2:
        if b'__str__' not in klass.__dict__:
            raise ValueError(b'Define __str__() on %s to use @python_2_unicode_compatible' % klass.__name__)
        if b'__repr__' not in klass.__dict__:
            raise ValueError(b'Define __repr__() on %s to use @python_2_unicode_compatible' % klass.__name__)
        klass.__unicode__ = klass.__str__
        klass._unicode_repr = klass.__repr__
        klass.__str__ = lambda self: self.__unicode__().encode(b'utf-8')
        klass.__repr__ = lambda self: self._unicode_repr().encode(b'ascii', errors=b'backslashreplace')
    return klass
...
class Rect(object):
    ...
    def __repr__(self):
        return b'%s(left=%s, right=%s, top=%s, bottom=%s)' % (
         self.__class__.__name__, self.left, self.right, self.top, self.bottom)

    def __str__(self):
        return b'<%s (%s, %s, %s, %s)>' % (self.__class__.__name__, self.left, self.right, self.top, self.bottom)
...
def get_photos(img, min_edge=0.05, min_fill=0.8, binary_threshold=0.98):
    ...
    """
    log.debug(b'Detecting photos in image')
    photos = []
    binary = binarize(img, binary_threshold)
    min_length_px = np.around(min_edge * min(binary.shape))
    log.debug(b'Minimum photo edge length: %s * shortest image dimension = %s pixels', min_edge, min_length_px)
    ...
    log.debug(b'Found photos: %s', photos)
    return photos
