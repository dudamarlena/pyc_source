...
# Size of source mod 2**32: 27411 bytes
...
def find_text_and_bar(thresholdedimg, gimg, rows, cols, show=False, printer=False, black_text=False):
    ...
    filteredvertices, boundingrectanglewidths, boundingrectanglecoords = find_draw_contours(dilatedimg,
      151, minarea=10, testing=show, annotate=False, bounding_rect=True, restrict_rectangles=False)
    ...
    for i in boundingrectanglecoords:
        ...
        if region_size > rows * cols / 100:
            box_filteredvertices, box_boundingrectanglewidths, box_boundingrectanglecoords = find_draw_contours((thresholdedimg[i[1]:i[1] + i[3], i[0]:i[0] + i[2]]),
              151, minarea=10, testing=show, annotate=show, bounding_rect=True, restrict_rectangles=True)
            ...
            for potential_box in box_boundingrectanglecoords:
                ...
                box_in_box_filteredvertices = find_draw_contours(box_in_box,
                  151, minarea=10, testing=show, annotate=show)
                ...
                if not conversion or not scalevalue:
                    for pixel_value in reversed(range(5, 40, 5)):
                        if not conversion or not scalevalue:
                            ...
                            digits = '.0123456789 '
                            if 'um' in greekengtextfromimage_edited_um:
                                if any(char in digits for char in greekengtextfromimage_edited_um):
                                    if len(greekengtextfromimage_edited_um.split('um')[0]) > 0:
                                        if greekengtextfromimage_edited_um.split('um')[0][(-1)] == ' ' or greekengtextfromimage_edited_um.split('um')[0][(-1)].isdigit() == True:
                                            conversion = 1e-06
                            ...
                            if scalevalue == None and 'nm' in greekengtextfromimage_edited_nm and any(char in digits for char in greekengtextfromimage_edited_nm):
                                if len(greekengtextfromimage_edited_nm.split('nm')[0]) > 0:
                                    if greekengtextfromimage_edited_nm.split('nm')[0][(-1)] == ' ' or greekengtextfromimage_edited_nm.split('nm')[0][(-1)].isdigit() == True:
                                        conversion = 1e-09
                                ...
                                if 'um' in greekengtextfromimage_raw_um:
                                    if any(char in digits for char in greekengtextfromimage_raw_um):
                                        if len(greekengtextfromimage_raw_um.split('um')[0]) > 0:
                                            if greekengtextfromimage_raw_um.split('um')[0][(-1)] == ' ' and greekengtextfromimage_raw_um.split('um')[0][(-2)].isdigit() == True or greekengtextfromimage_raw_um.split('um')[0][(-1)].isdigit() == True:
                                                conversion = 1e-06
                                                if all(char in digits for char in greekengtextfromimage_raw_um.split('um')[0]):
                                                    ...
                                                    boxes = raw_um_boxes
                                    if scalevalue == None:
                                        if 'nm' in greekengtextfromimage_raw_nm:
                                            if any(char in digits for char in greekengtextfromimage_raw_nm):
                                                if len(greekengtextfromimage_raw_nm.split('nm')[0]) > 0:
                                                    if greekengtextfromimage_raw_nm.split('nm')[0][(-1)] == ' ' or greekengtextfromimage_raw_nm.split('nm')[0][(-1)].isdigit() == True:
                                                        conversion = 1e-09
                                    ...
                                    if scalevalue == None:
                                        if 'um' in greekengtextfromimage_raw_inverted_um:
                                            if any(char in digits for char in greekengtextfromimage_raw_inverted_um):
                                                if len(greekengtextfromimage_raw_inverted_um.split('um')[0]) > 0:
                                                    if greekengtextfromimage_raw_inverted_um.split('um')[0][(-1)] == ' ' or greekengtextfromimage_raw_inverted_um.split('um')[0][(-1)].isdigit() == True:
                                                        conversion = 1e-06
                                ...
                                if scalevalue == None:
                                    if 'nm' in greekengtextfromimage_raw_inverted_nm:
                                        if any(char in digits for char in greekengtextfromimage_raw_inverted_nm):
                                            if len(greekengtextfromimage_raw_inverted_nm.split('nm')[0]) > 0:
                                                if greekengtextfromimage_raw_inverted_nm.split('nm')[0][(-1)] == ' ' or greekengtextfromimage_raw_inverted_nm.split('nm')[0][(-1)].isdigit() == True:
                                                    conversion = 1e-09
    ...
    for potential_region in potential_scale_bar_regions:
        ...
        filteredvertices, boundingrectanglewidths, boundingrectanglecoords = find_draw_contours(regionsurroundingscalebar,
          151, minarea=10, nofilter=True, testing=show, annotate=show, bounding_rect=True, restrict_rectangles=True, restrict_rectangles_color=restrict_rectangles_color)
        first_scale_bar_rectangles = [a for a in boundingrectanglecoords if float(a[2]) / a[3] >= 1.3 if a[2] < interp_scale * cols / 1.5]
