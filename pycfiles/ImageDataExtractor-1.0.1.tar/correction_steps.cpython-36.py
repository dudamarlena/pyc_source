...
# [GCC 8.4.0]
# Embedded file name: /home/edward/github/ImageDataExtractor/imagedataextractor/correction_steps.py
# Compiled at: 2019-08-30 11:43:54
# Size of source mod 2**32: 28388 bytes
        ...
        for point in vertices:
            ...
            if len(inlaycoords) > 0:
                for inlay in inlaycoords:
                    ...
                    inlaycorners = (
                     (
                      inlay[0] - growby, inlay[1] - growby), (inlay[0] + inlay[2] + growby, inlay[1] - growby),
                     (
                      inlay[0] + inlay[2] + growby, inlay[1] + inlay[3] + growby), (inlay[0] - growby, inlay[1] + inlay[3] + growby))
                    if point[0] >= inlaycorners[0][0] and point[0] <= inlaycorners[1][0] and point[1] >= inlaycorners[0][1] and point[1] <= inlaycorners[3][1]:
                        inlayintersections += 1

        if borderintersections < 2:
            if inlayintersections < 2:
    ...
    if testing == True:
        ...
        for inlay in inlaycoords:
            ...
            inlaycorners = ((inlay[0] - growby, inlay[1] - growby), (inlay[0] + inlay[2] + growby, inlay[1] - growby),
             (
              inlay[0] + inlay[2] + growby, inlay[1] + inlay[3] + growby), (inlay[0] - growby, inlay[1] + inlay[3] + growby))
            cv2.rectangle(test_img, (inlaycorners[0]), (inlaycorners[2]), (255, 0,
                                                                           0), thickness=1)

        draw_contours(test_img, removedvertices, imgname='edge removed particles')
    return (
     edgecorrectedvertices, particlediscreteness)
...
def particle_metrics_from_vertices(img, gimg, rows, cols, filteredvertices, invert=False):
    ...
    return (
     colorlist, arealist, avgcolormean, avgcolorstdev, avgarea)
...
def false_positive_correction(filteredvertices, arealist, colorlist, avgcolormean, avgcolorstdev, testing=False, gimg=None):
    ...
    for i in range(len(colorlist)):
        if (colorlist[i][0] < avgcolormean * 0.6 and colorlist[i][1] < avgcolorstdev * 0.25 or colorlist[i][0] < 2 or colorlist[i][0] > 240 or colorlist[i][2] > 250) == True:
...
def cluster_breakup_correction(filteredvertices, rows, cols, arealist, avgarea, blocksize, testing=False, detailed_testing=False):
                ...
                for j in range(defects.shape[0]):
                    ...
                    if d / 256.0 > arealist[i] ** 0.5 / 15.0:
                        ...
                        if midpoint[0] - far[0] == 0:
                            ...
                            maskimg[:] = (0, 0, 0)
                            l = int(avgarea ** 0.5 / 4)
                            cv2.ellipse(maskimg, far, (int(0.9 * l), 2 * l), angle, 0, 180, (255,
                                                                                             255,
                                                                                             255), -1)
                            oppositeface = []
                            ...
                            if visualizemasking == True:
                                ...
                                cv2.circle(maskingvisualization, start, 2, (255, 255,
                                                                            255), -1)
                                cv2.circle(maskingvisualization, end, 2, (255, 255,
                                                                          255), -1)
                                cv2.circle(maskingvisualization, far, 2, (0, 0, 255), -1)
                                cv2.ellipse(maskingvisualization, far, (int(0.9 * l), 2 * l), angle, 0, 180, (255,
                                                                                                              255,
                                                                                                              255), 1)
                    ...
                    if len(contours) > 1:
                        ...
                        cv2.line(updatingimg, extconnectionpoint1, extconnectionpoint2, (0,
                                                                                         0,
                                                                                         0), thickness=2)
                        cv2.line(breakupimg, extconnectionpoint1, extconnectionpoint2, (0,
                                                                                        255,
                                                                                        0), thickness=1)
                        if visualizemasking == True:
                            cv2.circle(maskingvisualization, tuple(pointpairs[distances.index(min(distances))][0]), 2, (255,
                                                                                                                        0,
                                                                                                                        255), -1)
                            cv2.circle(maskingvisualization, tuple(pointpairs[distances.index(min(distances))][1]), 2, (255,
                                                                                                                        0,
                                                                                                                        255), -1)
    ...
    brokenupvertices = find_draw_contours(updatingimg, blocksize, minarea=(avgarea / 10), annotate=True, testing=testing, nofilter=True, contours_color=(255,
                                                                                                                                                         255,
                                                                                                                                                         255))
    indextoremove = list(set(indextoremove))
    for i in sorted(indextoremove, reverse=True):
...
def discreteness_index_and_ellipse_fitting(edgecorrectedvertices, img, rows, cols, imgstdev, testing=False):
    ...
    for i in edgecorrectedvertices:
        ...
        cv2.putText(ellimg, (str(particleindex)), (xcom + 3, ycom + 3), (cv2.FONT_HERSHEY_COMPLEX), 0.4, (255,
                                                                                                          0,
                                                                                                          255), thickness=1)
        bufferfromborder = 3
        ...
        if len(hull) > 4:
            ...
            blues = []
            for pt in i:
                if pt[0][0] > bufferfromborder:
                    if pt[0][0] < cols - bufferfromborder:
                        if pt[0][1] > bufferfromborder:
                            if pt[0][1] < rows - bufferfromborder:
                                if pt[0][0] < xcom:
            ...
            if ellipseDI > particleDI:
                if ellipse_color_stdev < particle_color_stdev * 1.25:
    ...
    if testing == True:
        show_image(ellimg)
    return (
     ellipsefittedvertices, particlediscreteness)