...
# [GCC 8.4.0]
# Embedded file name: /home/edward/github/ImageDataExtractor/imagedataextractor/scale_reading.py
# Compiled at: 2019-06-04 05:21:35
# Size of source mod 2**32: 27384 bytes
...
def read_image(thresholdedimg, unit):
    ...
    return (
     greekengtextfromimage, text_boxes)
...
def detect_scale_bar_and_inlays(gimg, imgmean, imgstdev, rows, cols, show=False):
    ...
    if scalebar == None and scalevalue == None and conversion == 1:
        ...
        cv2.rectangle(scalebarimg, (0, rows - frame_constant), (cols, rows), (255,
                                                                              255,
                                                                              255), -1)
        cv2.rectangle(scalebarimg, (0, 0), (cols, frame_constant), (255, 255, 255), -1)
        cv2.rectangle(scalebarimg, (cols - frame_constant, 0), (cols, rows), (255,
                                                                              255,
                                                                              255), -1)
...
def find_text_and_bar(thresholdedimg, gimg, rows, cols, show=False, printer=False, black_text=False):
    ...
    filteredvertices, boundingrectanglewidths, boundingrectanglecoords = find_draw_contours(dilatedimg, 151, minarea=10, testing=show, annotate=False, bounding_rect=True, restrict_rectangles=False)
    ...
    for i in boundingrectanglecoords:
        ...
        if region_size > rows * cols / 100:
            box_filteredvertices, box_boundingrectanglewidths, box_boundingrectanglecoords = find_draw_contours(thresholdedimg[i[1]:i[1] + i[3], i[0]:i[0] + i[2]], 151, minarea=10, testing=show, annotate=show, bounding_rect=True, restrict_rectangles=True)
            ...
            for potential_box in box_boundingrectanglecoords:
                ...
                box_in_box_filteredvertices = find_draw_contours(box_in_box, 151, minarea=10, testing=show, annotate=show)
                if len(box_in_box_filteredvertices) > 2:
                    i = (
                     i[0] + potential_box[0], i[1] + potential_box[1], potential_box[2], potential_box[3])
                    is_box_in_box = True
                break
                    ...
                    for pixel_value in reversed(range(5, 40, 5)):
                        if not conversion or not scalevalue:
                            ...
                            if pixel_value == 35:
                                ...
                                digits = '.0123456789 '
                                if 'um' in greekengtextfromimage_edited_um and any(char in digits for char in greekengtextfromimage_edited_um) and len(greekengtextfromimage_edited_um.split('um')[0]) > 0 and (greekengtextfromimage_edited_um.split('um')[0][(-1)] == ' ' or greekengtextfromimage_edited_um.split('um')[0][(-1)].isdigit() == True):
                                    conversion = 1e-06
                                    ...
                                    if scalevalue == None and 'nm' in greekengtextfromimage_edited_nm and any(char in digits for char in greekengtextfromimage_edited_nm):
                                        pass
                            if len(greekengtextfromimage_edited_nm.split('nm')[0]) > 0 and (greekengtextfromimage_edited_nm.split('nm')[0][(-1)] == ' ' or greekengtextfromimage_edited_nm.split('nm')[0][(-1)].isdigit() == True):
                                conversion = 1e-09
                            ...
                            if pixel_value == 35:
                                pass
                        if 'um' in greekengtextfromimage_raw_um and any(char in digits for char in greekengtextfromimage_raw_um) and len(greekengtextfromimage_raw_um.split('um')[0]) > 0 and (greekengtextfromimage_raw_um.split('um')[0][(-1)] == ' ' and greekengtextfromimage_raw_um.split('um')[0][(-2)].isdigit() == True or greekengtextfromimage_raw_um.split('um')[0][(-1)].isdigit() == True):
                            conversion = 1e-06
                            if all(char in digits for char in greekengtextfromimage_raw_um.split('um')[0]):
                                ...
                                boxes = raw_um_boxes
                            if scalevalue == None and 'nm' in greekengtextfromimage_raw_nm and any(char in digits for char in greekengtextfromimage_raw_nm) and len(greekengtextfromimage_raw_nm.split('nm')[0]) > 0 and (greekengtextfromimage_raw_nm.split('nm')[0][(-1)] == ' ' or greekengtextfromimage_raw_nm.split('nm')[0][(-1)].isdigit() == True):
                                conversion = 1e-09
                                if all(char in digits for char in greekengtextfromimage_raw_nm.split('nm')[0]):
                                    ...
                                    boxes = raw_nm_boxes
                                if scalevalue == None and 'um' in greekengtextfromimage_raw_inverted_um and any(char in digits for char in greekengtextfromimage_raw_inverted_um) and len(greekengtextfromimage_raw_inverted_um.split('um')[0]) > 0 and (greekengtextfromimage_raw_inverted_um.split('um')[0][(-1)] == ' ' or greekengtextfromimage_raw_inverted_um.split('um')[0][(-1)].isdigit() == True):
                                    conversion = 1e-06
                                    ...
                                    if scalevalue == None and 'nm' in greekengtextfromimage_raw_inverted_nm and any(char in digits for char in greekengtextfromimage_raw_inverted_nm) and len(greekengtextfromimage_raw_inverted_nm.split('nm')[0]) > 0 and (greekengtextfromimage_raw_inverted_nm.split('nm')[0][(-1)] == ' ' or greekengtextfromimage_raw_inverted_nm.split('nm')[0][(-1)].isdigit() == True):
                                        conversion = 1e-09
            ...
            if conversion != None and scalevalue != None:
                ...
                if printer == True:
                    print('worked at pixel_value ', str(pixel_value + 5))
    ...
    for potential_region in potential_scale_bar_regions:
        ...
        filteredvertices, boundingrectanglewidths, boundingrectanglecoords = find_draw_contours(regionsurroundingscalebar, 151, minarea=10, nofilter=True, testing=show, annotate=show, bounding_rect=True, restrict_rectangles=True, restrict_rectangles_color=restrict_rectangles_color)
        ...
        if len(potential_scale_bar_widths) > 0:
            ...
            potential_scale_bars.append([
             (int(scalebarboundingrectangle[0] + surrounding_region_y1), int(scalebarboundingrectangle[1] + surrounding_region_x1),
    ...
    return (
     scalebar, scalevalue, conversion, inlaycoords)