...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/libpme.py
# Compiled at: 2017-04-12 12:40:40
...
class PME(object):
    ...
    def __init__(self, filename=False, damaged=False):
        ...
        if filename:
            ...
            self.chunks = [
             [
              b'\x00\x00\x00\x00', b'IHDR', b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00', b''],
             [
              b'\x00\x00\x00\x00', b'IDAT', b'', b''],
             [
              b'\x00\x00\x00\x00', b'IEND', b'', b'']]
    ...
    def __setattr__(self, name, value):
        ...
        if name in ('width', 'height', 'bit_depth', 'color_type', 'compression_method',
                    'filter_method', 'interlace_method'):
            self.recalculate_IHDR()

    def _calculate_crc(self, label, binary):
        calculated = zlib.crc32(label)
        calculated = zlib.crc32(binary, calculated) & 4294967295
        return calculated
    ...
    def _index(self, index):
        if type(index) == list:
            ...
            if type(index) == str:
                index = bytes(index)
            if type(index) == bytes:
                index = [x[1] for x in self.chunks].index(index)
            return index
