...
# [GCC 8.4.0]
# Embedded file name: d:\pythonforsource\msnoise_stack\msnoise\msnoise\test\tests.py
# Compiled at: 2020-02-25 09:40:35
# Size of source mod 2**32: 23960 bytes
import datetime, glob, logging, os, re, shutil, traceback, unittest
...
class MSNoiseTests(unittest.TestCase):
    ...
    def test_001_S01installer(self):
        ...
        try:
            ret = main(tech=1, prefix=(self.prefix), filename='testmsnoise.sqlite')
    ...
    def test_004_set_and_get_filters(self):
        ...
        filters.append(f)
        for f in filters:
            update_filter(db, f.ref, f.low, f.mwcs_low, f.high, f.mwcs_high, f.rms_threshold, f.mwcs_wlen, f.mwcs_step, f.used)

        dbfilters = get_filters(db)
        for i, filter in enumerate(dbfilters):
            for param in ('low', 'mwcs_low', 'high', 'mwcs_high', 'rms_threshold',
                          'mwcs_wlen', 'mwcs_step', 'used'):
                self.failUnlessEqual(eval('filter.%s' % param), eval('filters[i].%s' % param))
    ...
    def test_007_update_stations(self):
        ...
        stations = pd.read_csv((os.path.join(path, 'test', 'extra', 'stations.csv')),
          header=None,
          index_col=0,
          names=[
         'X', 'Y', 'altitude'])
    ...
    def test_015_check_cc_files(self, format='MSEED'):
        from ..api import connect, get_filters, get_station_pairs, get_components_to_compute
        db = connect()
            ...
            for components in get_components_to_compute(db):
                for sta1, sta2 in get_station_pairs(db):
                    pair = '%s_%s_%s_%s' % (sta1.net, sta1.sta,
                     sta2.net, sta2.sta)
                    tmp = os.path.join('STACKS', '%02i' % filter.ref, '001_DAYS', components, pair, '2010-09-01.%s' % format)
                    print('checking', tmp)
        ...
        for filter in get_filters(db):
            for components in get_components_to_compute(db):
                for sta1, sta2 in get_station_pairs(db):
                    pair = '%s_%s_%s_%s' % (sta1.net, sta1.sta,
                     sta2.net, sta2.sta)
                    tmp1 = os.path.join('STACKS', '%02i' % filter.ref, '001_DAYS', components, pair, '2010-09-01.MSEED')
                    tmp2 = os.path.join('STACKS', '%02i' % filter.ref, '001_DAYS', components, pair, '2010-09-01.SAC')
                    tmp1 = read(tmp1)
                    tmp2 = read(tmp2)
    ...
    def test_029_create_fake_new_files(self):
        for f in sorted(glob.glob(os.path.join(self.data_folder, '2010', '*', 'HHZ.D', '*'))):
            st = read(f)
            for tr in st:
    ...
    def test_031_autocorr(self):
        from ..api import connect, update_config
        import os
        db = connect()
        update_config(db, 'remove_response', 'N')
        update_config(db, 'components_to_compute', '')
        update_config(db, 'components_to_compute_single_station', 'ZZ')
        db.close()
        os.system('msnoise reset DA')
        self.test_010_new_jobs()
        self.test_013_s03compute_cc()
    ...
    def test_100_plot_cctfime(self):
        ...
        for sta1, sta2 in get_station_pairs(db):
            ...
            for filter in get_filters(db):
                main(sta1, sta2, (filter.ref), 'ZZ', 1, show=False, outfile='?.png')
    ...
    def test_102_plot_distance(self):
        ...
        self.assertTrue((os.path.isfile(fn)), msg=("%s doesn't exist" % fn))
        main(filterid=1, components='ZZ', show=False, outfile='?_refilter.png', refilter='0.2:0.9')
    ...
    def test_212_crondays_weeks(self):
        ...
        self.assertEqual(parsed_crondays, datetime.timedelta(days=14))
    ...
    def test_214_crondays_hours(self):
        ...
        self.assertEqual(parsed_crondays, datetime.timedelta(seconds=43200))
    ...
    def test_215_crondays_weeks_days_hours(self):
        ...
        self.assertEqual(parsed_crondays, datetime.timedelta(days=17, seconds=43200))
    ...
    def test_216_crondays_weeks_hours(self):
        ...
        self.assertEqual(parsed_crondays, datetime.timedelta(days=7, seconds=21600))
    ...
    def test_219_crondays_weeks_days_hours_optional_blank(self):
        ...
        self.assertEqual(parsed_crondays, datetime.timedelta(days=25, seconds=43200))
    ...
    def test_999_S01installer(self):
        if 'TRAVIS_OS_NAME' not in os.environ:
            ...
            if os.environ['TRAVIS_OS_NAME'] != 'linux':
                print('Seems not to be running on a Linux machine, skipping MySQL test')
                return
        else:
            import shutil
            ...
            from ..s000installer import main
            try:
                ret = main(tech=2, username='root', password='', hostname='localhost',
                  database='msnoise',
                  prefix='')
                self.failUnlessEqual(ret, 0)
            except:
