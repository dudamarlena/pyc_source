...
# [GCC 8.4.0]
# Embedded file name: C:\Users\stuc400\PycharmProjects\smart_preprocess\dmreader\utils.py
# Compiled at: 2017-08-18 02:33:08
# Size of source mod 2**32: 32845 bytes
    ...
    def __setattr__(self, key, value):
        ...
        if isinstance(value, dict):
            ...
            value = DictionaryTreeBrowser(value,
              double_lines=(self._double_lines))
        super(DictionaryTreeBrowser, self).__setattr__(slugified_key, {'key':key, 
         '_dtb_value_':value})
...
def fsdict(nodes, value, dictionary):
    ...
    if node not in dictionary:
        dictionary[node] = {}
    elif len(nodes) != 0 and isinstance(dictionary[node], dict):
...
def find_subclasses(mod, cls):
    ...
    """
    return dict([(name, obj) for name, obj in inspect.getmembers(mod) if inspect.isclass(obj) if issubclass(obj, cls)])
...
def map_result_construction(signal, inplace, result, ragged, sig_shape=None, lazy=False):
    ...
    if ragged:
        ...
        if not sig._lazy:
            if sig.data.shape == result.shape:
                if np.can_cast(result.dtype, sig.data.dtype):
                    sig.data[:] = result
        else:
            sig.data = result
...
def add_scalar_axis(signal):
    ...
    am.remove(am._axes)
    am._append_axis(size=1, scale=1,
      offset=0,
      name='Scalar',
      navigate=False)