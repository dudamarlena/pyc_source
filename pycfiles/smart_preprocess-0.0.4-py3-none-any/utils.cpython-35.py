...
# [GCC 8.4.0]
# Embedded file name: E:\E_Documents\Python_Code\smart_preprocess\smart_preprocess\smart_preprocess\dmreader\utils.py
# Compiled at: 2018-08-22 10:53:42
# Size of source mod 2**32: 32822 bytes
from operator import attrgetter
import warnings, inspect, copy, types
from io import StringIO
import codecs, collections, tempfile, unicodedata
...
_slugify_strip_re_data = ''.join(c for c in map(chr, np.delete(np.arange(256), [
    ...
    def _get_print_items(self, padding='', max_len=78):
        ...
        for key_, value in iter(sorted(self.__dict__.items())):
            if key_.startswith('_'):
                pass
            elif not isinstance(key_, types.MethodType):
                ...
                if j == eoi - 1:
                    ...
                    symbol = s_middle
                if preferences.General.dtb_expand_structures and (isinstance(value, list) or isinstance(value, tuple)):
                    iflong, strvalue = check_long_string(value, max_len)
                    if iflong:
                        key += ' <list>' if isinstance(value, list) else ' <tuple>'
                        value = DictionaryTreeBrowser({'[%d]' % i:v for i, v in enumerate(value)}, double_lines=True)
                    else:
                        string += '%s%s%s = %s\n' % (
    ...
    def __setattr__(self, key, value):
        ...
        if isinstance(value, dict):
            if self.has_item(slugified_key):
                self.get_item(slugified_key).add_dictionary(value, double_lines=self._double_lines)
                return
            value = DictionaryTreeBrowser(value, double_lines=self._double_lines)
        super(DictionaryTreeBrowser, self).__setattr__(slugified_key, {'key': key, '_dtb_value_': value})
            ...
            if not isinstance(item_, types.MethodType):
                ...
                if key in ('_db_index', '_double_lines'):
                    ...
                    if isinstance(item_['_dtb_value_'], DictionaryTreeBrowser):
                        ...
                        if hasattr(item_['_dtb_value_'], '_to_dictionary'):
...
def find_subclasses(mod, cls):
    ...
    """
    return dict([(name, obj) for name, obj in inspect.getmembers(mod) if inspect.isclass(obj) and issubclass(obj, cls)])
...
def stack(signal_list, axis=None, new_axis_name='stack_element', lazy=None, **kwargs):
    ...
    if len(signal_list) > 1:
        ...
        newdata = da.stack(datalist, axis=0) if axis is None else da.concatenate(datalist, axis=axis.index_in_array)
        if axis_input is None:
            ...
            signal.metadata.General.title = 'Stack of ' + first.metadata.General.title
        ...
        if axis_input is None:
            axis_input = signal.axes_manager[complex(-1.0, 1.0)].index_in_axes_manager
            step_sizes = 1
        ...
        if np.all([s.metadata.has_item('Signal.Noise_properties.variance') for s in signal_list]):
            variance = stack([s.metadata.Signal.Noise_properties.variance for s in signal_list], axis)
...
def transpose(*args, signal_axes=None, navigation_axes=None, optimize=False):
    ...
    return [sig.transpose(signal_axes=signal_axes, navigation_axes=navigation_axes, optimize=optimize) for sig in args]
...
def create_map_objects(function, nav_size, iterating_kwargs, **kwargs):
    ...
    iterators = tuple((signal[1]._iterate_signal() if isinstance(signal[1], BaseSignal) else signal[1]) for signal in iterating_kwargs)
    ...
    def figure_out_kwargs(data):
        ...
        for k, v in iterating_kwargs:
            if isinstance(v, BaseSignal) and isinstance(_kwargs[k], np.ndarray) and len(_kwargs[k]) == 1:
...
def map_result_construction(signal, inplace, result, ragged, sig_shape=None, lazy=False):
    from hyperspy.signals import BaseSignal
    ...
    if ragged:
        ...
        if not sig._lazy and sig.data.shape == result.shape and np.can_cast(result.dtype, sig.data.dtype):
            sig.data[:] = result
        else:
            sig.data = result
