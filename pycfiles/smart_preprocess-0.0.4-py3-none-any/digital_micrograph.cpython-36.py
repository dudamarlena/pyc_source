...
# [GCC 8.4.0]
# Embedded file name: C:\Users\stuc400\PycharmProjects\smart_preprocess\dmreader\digital_micrograph.py
# Compiled at: 2018-07-26 09:24:14
# Size of source mod 2**32: 40929 bytes
...
class DigitalMicrographReader(object):
    ...
    def parse_file(self):
        ...
        self.parse_tags(number_of_root_tags,
          group_name='root',
    ...
    def get_data_reader(self, enc_dtype):
        dtype_dict = {2:(
         ...
         12:(
          iou.read_double, 8, 'l'), 
         15:(
          self.read_struct, None, 'struct'), 
         18:(
          self.read_string, None, 'c'), 
         20:(
          self.read_array, None, 'array')}
    ...
    def read_array(self, size, enc_eltype, extra=None, skip=False):
        ...
        if skip is True:
            if enc_eltype not in self._complex_type:
                ...
                data = {'size':size,  'endian':self.endian, 
                 'size_bytes':size_bytes, 
                 'offset':self.f.tell()}
        ...
        elif enc_eltype in self.simple_type:
            data = [eltype(self.f, self.endian) for element in range(size)]
            if enc_eltype == 4:
                if data:
                    data = ''.join([chr(i) for i in data])
        else:
            if enc_eltype in self._complex_type:
    ...
    def parse_tag_header(self):
        ...
        return {'tag_id':tag_id,  'tag_name_length':tag_name_length, 
         'tag_name':tag_name}
...
class ImageObject(object):
    ...
    def names(self):
        ...
        if self.signal_type == 'EELS':
            ...
            if self.signal_type in ('EDS', 'EDX'):
    ...
    def record_by(self):
        if self._record_by is not None:
            ...
            if 'ImageTags.Meta_Data.Format' in self.imdict and self.imdict.ImageTags.Meta_Data.Format in ('Spectrum image',
                                                                                                          'Spectrum') or 'ImageTags.spim' in self.imdict:
    ...
    def to_spectrum(self):
        if 'ImageTags.Meta_Data.Format' in self.imdict and self.imdict.ImageTags.Meta_Data.Format == 'Spectrum image' or 'ImageTags.spim' in self.imdict:
            if len(self.scales) > 2:
                return True
        return False
    ...
    def dtype(self):
        ...
        imdtype_dict = {0:'not_implemented', 
         ...
         6:'uint8', 
         7:'int32', 
         8:np.dtype({'names':['B', 'G', 'R', 'A'],  'formats':[
           'u1', 'u1', 'u1', 'u1']}), 
         9:'int8', 
         10:'uint16', 
         11:'uint32', 
         ...
         13:'complex128', 
         14:'bool', 
         23:np.dtype({'names':['B', 'G', 'R', 'A'],  'formats':[
           'u1', 'u1', 'u1', 'u1']}), 
         27:'complex64', 
         28:'complex128'}
    ...
    def _get_data_array(self):
        ...
        if self.imdict.ImageData.DataType in (27, 28):
            count = int(count / 2)
        data = np.fromfile((self.file), dtype=(self.dtype),
          count=count)
        if need_to_close:
            self.file.close()
        return data
    ...
    def unpack_packed_complex(self, tmpdata):
        ...
        data[N + 1:2 * N, N:2 * N] = complexdata.reshape((N - 1),
          N,
          order=(self.order))
