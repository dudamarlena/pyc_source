...
# [GCC 8.4.0]
# Embedded file name: E:\E_Documents\Python_Code\smart_preprocess\smart_preprocess\smart_preprocess\dmreader\digital_micrograph.py
# Compiled at: 2018-08-22 10:53:06
# Size of source mod 2**32: 40905 bytes
import os, logging, dateutil.parser, numpy as np, dmreader.trait_base as t, dmreader.utils_readfile as iou
from dmreader.exceptions import DM3TagIDError, DM3DataTypeError, DM3TagTypeError
import dmreader.tools
from dmreader.utils import DictionaryTreeBrowser
...
class DigitalMicrographReader(object):
    ...
    def parse_file(self):
        ...
        self.parse_tags(number_of_root_tags, group_name='root', group_dict=self.tags_dict)

    def parse_header(self):
        self.dm_version = iou.read_long(self.f, 'big')
        if self.dm_version not in (3, 4):
            raise NotImplementedError('Currently we only support reading DM versions 3 and 4 but this file seems to be version %s ' % self.dm_version)
        self.skipif4()
    ...
    def parse_tags(self, ntags, group_name='root', group_dict={}):
        ...
        for tag in range(ntags):
            ...
            if infoarray_size > 3:
                ...
                if enctype == 15:
                    ...
                    if enctype == 20:
                        ...
                        if enc_eltype == 15:
                            ...
                            data = self.read_array(size=size, enc_eltype=enc_eltype, extra={'definition': definition}, skip=skip)
                        else:
                            if enc_eltype == 18:
                                _logger.debug('Reading array of strings')
                                string_length = self.parse_string_definition()
                                size = iou.read_long(self.f, 'big')
                                data = self.read_array(size=size, enc_eltype=enc_eltype, extra={'length': string_length}, skip=skip)
                            elif enc_eltype == 20:
                                _logger.debug('Reading array of arrays')
                                el_length, enc_eltype = self.parse_array_definition()
                                size = iou.read_long(self.f, 'big')
                                data = self.read_array(size=size, enc_eltype=enc_eltype, extra={'size': el_length}, skip=skip)
                    else:
                        raise IOError('Invalided infoarray size ', infoarray_size)
                    group_dict[tag_name] = data
            elif tag_header['tag_id'] == 20:
                if not tag_name:
                ...
                group_dict[tag_name] = {}
                self.parse_tags(ntags=ntags, group_name=tag_name, group_dict=group_dict[tag_name])
            else:
                _logger.debug('File address:', self.f.tell())
                raise DM3TagIDError(tag_header['tag_id'])

    def get_data_reader(self, enc_dtype):
        dtype_dict = {2: (iou.read_short, 2, 'h'), 
         3: (iou.read_long, 4, 'l'), 
         4: (iou.read_ushort, 2, 'H'), 
         5: (iou.read_ulong, 4, 'L'), 
         6: (iou.read_float, 4, 'f'), 
         7: (iou.read_double, 8, 'd'), 
         8: (iou.read_boolean, 1, 'B'), 
         9: (iou.read_char, 1, 'b'), 
         10: (iou.read_byte, 1, 'b'), 
         11: (iou.read_double, 8, 'l'), 
         12: (iou.read_double, 8, 'l'), 
         15: (self.read_struct, None, 'struct'), 
         18: (self.read_string, None, 'c'), 
         20: (self.read_array, None, 'array')}
    ...
    def read_array(self, size, enc_eltype, extra=None, skip=False):
        ...
        """
        eltype = self.get_data_reader(enc_eltype)[0]
        if skip is True:
            ...
            if enc_eltype in self.simple_type:
                data = [eltype(self.f, self.endian) for element in range(size)]
                if enc_eltype == 4 and data:
    ...
    def get_image_dictionaries(self):
        ...
        if 'Thumbnails' in self.tags_dict:
            thumbnail_idx = [tag['ImageIndex'] for key, tag in self.tags_dict['Thumbnails'].items()]
        else:
            thumbnail_idx = []
        images = [image for key, image in self.tags_dict['ImageList'].items() if int(key.replace('TagGroup', '')) not in thumbnail_idx]
        return images
...
class ImageObject(object):
    ...
    def units(self):
        dimensions = self.imdict.ImageData.Calibrations.Dimension
        len_diff = len(self.shape) - len(dimensions)
        return (tuple([dimension[1].Units if dimension[1].Units else '' for dimension in dimensions]) + ('', ) * len_diff)[::-1]
    ...
    def names(self):
        ...
        elif self.signal_type in ('EDS', 'EDX') and 'keV' in self.units:
    ...
    def record_by(self):
        if self._record_by is not None:
            ...
            if len(self.scales) == 1:
                return 'spectrum'
            if ('ImageTags.Meta_Data.Format' in self.imdict and self.imdict.ImageTags.Meta_Data.Format in ('Spectrum image',
                                                                                                           'Spectrum') or 'ImageTags.spim' in self.imdict) and len(self.scales) == 2:
                return 'spectrum'
            return 'image'

    @property
    def to_spectrum(self):
        if ('ImageTags.Meta_Data.Format' in self.imdict and self.imdict.ImageTags.Meta_Data.Format == 'Spectrum image' or 'ImageTags.spim' in self.imdict) and len(self.scales) > 2:
            return True
        else:
            return False
    ...
    def dtype(self):
        ...
        imdtype_dict = {0: 'not_implemented', 
         1: 'int16', 
         2: 'float32', 
         3: 'complex64', 
         5: 'float32', 
    ...
    def signal_type(self):
        if 'ImageTags.Meta_Data.Signal' in self.imdict:
            ...
            if 'ImageTags.spim.eels' in self.imdict:
                return 'EELS'
            return ''
    ...
    def _get_data_array(self):
        ...
        if self.imdict.ImageData.DataType in (27, 28):
            count = int(count / 2)
    ...
    def size(self):
        if self.imdict.ImageData.DataType in (27, 28):
            if self.imdict.ImageData.Data.size % 2:
                raise IOError('ImageData.Data.size should be an even integer for this datatype.')
            else:
    ...
    def get_data(self):
        ...
        if self.imdict.ImageData.DataType == 5:
    ...
    def unpack_new_packed_complex(self, data):
        packed_shape = (
         self.shape[0], int(self.shape[1] / 2 + 1))
    ...
    def unpack_packed_complex(self, tmpdata):
        ...
        if shape[0] != shape[1] or len(shape) > 2:
            raise IOError('Unable to read this DM file in packed complex format. Please report the issue to the HyperSpy developers providing the file if possible')
        N = int(self.shape[0] / 2)
        data = np.zeros(shape, dtype='complex64')
        data[(N, 0)] = tmpdata[0]
        data[(0, 0)] = tmpdata[1]
        data[(N, N)] = tmpdata[(2 * N ** 2)]
        data[(0, N)] = tmpdata[(2 * N ** 2 + 1)]
        for i in range(N):
            start = 2 * i * N + 2
            stop = start + 2 * (N - 1) - 1
            step = 2
            ...
            data[i, N + 1:2 * N] = realpart + imagpart * complex(0.0, 1.0)
        ...
        realpart = tmpdata[start:stop:step]
        imagpart = tmpdata[start + 1:stop + 1:step]
        data[N + 1:2 * N, 0] = realpart + imagpart * complex(0.0, 1.0)
        start = 2 * N ** 2 + 2
        stop = start + 2 * (N - 1) - 1
        step = 2
        realpart = tmpdata[start:stop:step]
        imagpart = tmpdata[start + 1:stop + 1:step]
        data[N, N + 1:2 * N] = realpart + imagpart * complex(0.0, 1.0)
        start = stop + 1
        stop = start + 2 * N * (N - 1) - 1
        step = 2
        realpart = tmpdata[start:stop:step]
        imagpart = tmpdata[start + 1:stop + 1:step]
        complexdata = realpart + imagpart * complex(0.0, 1.0)
    ...
    def get_axes_dict(self):
        return [{'name': name, 'size': size, 'index_in_array': i, 'scale': scale, 'offset': offset, 'units': str(units)} for i, (name, size, scale, offset, units) in enumerate(zip(self.names, self.shape, self.scales, self.offsets, self.units))]
    ...
    def _get_microscope_name(self, ImageTags):
        try:
            ...
            if 'Microscope' in ImageTags['Microscope_Info'].keys():
    ...
    def get_mapping(self):
        ...
        mapping = {'ImageList.TagGroup0.ImageTags.DataBar.Acquisition Date': (
         ...
         'ImageList.TagGroup0.ImageTags.Microscope Info.Voltage': (
                                                                   'Acquisition_instrument.TEM.beam_energy', lambda x: x / 1000.0), 
         
         'ImageList.TagGroup0.ImageTags.Microscope Info.Stage Position.Stage Alpha': ('Acquisition_instrument.TEM.Stage.tilt_alpha', None), 
         
         'ImageList.TagGroup0.ImageTags.Microscope Info.Stage Position.Stage X': (
                                                                                  'Acquisition_instrument.TEM.Stage.x', lambda x: x * 0.001), 
         
         'ImageList.TagGroup0.ImageTags.Microscope Info.Stage Position.Stage Y': (
                                                                                  'Acquisition_instrument.TEM.Stage.y', lambda x: x * 0.001), 
         
         'ImageList.TagGroup0.ImageTags.Microscope Info.Stage Position.Stage Z': (
                                                                                  'Acquisition_instrument.TEM.Stage.z', lambda x: x * 0.001), 
         ...
         'ImageList.TagGroup0.ImageTags.Microscope Info.Probe Current (nA)': ('Acquisition_instrument.TEM.beam_current', None), 
        ...
        if 'Microscope_Info' in self.imdict.ImageTags.keys():
            ...
            if 'Imaging_Mode' in self.imdict.ImageTags['Microscope_Info'].keys():
                is_diffraction = 'DIFFRACTION' == self.imdict.ImageTags.Microscope_Info.Imaging_Mode
            if is_TEM:
                if is_diffraction:
                    mapping.update({'ImageList.TagGroup0.ImageTags.Microscope Info.Indicated Magnification': ('Acquisition_instrument.TEM.camera_length', None)})
                else:
                    mapping.update({'ImageList.TagGroup0.ImageTags.Microscope Info.Indicated Magnification': ('Acquisition_instrument.TEM.magnification', None)})
            else:
                mapping.update({'ImageList.TagGroup0.ImageTags.Microscope Info.STEM Camera Length': ('Acquisition_instrument.TEM.camera_length', None), 
                 
                 'ImageList.TagGroup0.ImageTags.Microscope Info.Indicated Magnification': ('Acquisition_instrument.TEM.magnification', None)})
            mapping.update({'ImageList.TagGroup0.ImageTags': (
        ...
        if self.signal_type == 'EELS':
            ...
            mapping.update({'ImageList.TagGroup0.ImageTags.EELS.Acquisition.Date': (
                                                                     'General.date',
                                                                     self._get_date), 
             ...
             'ImageList.TagGroup0.ImageTags.EELS.Experimental Conditions.' + 'Collection semi-angle (mrad)': ('Acquisition_instrument.TEM.Detector.EELS.collection_angle', None), 
             
             'ImageList.TagGroup0.ImageTags.EELS.Experimental Conditions.' + 'Convergence semi-angle (mrad)': ('Acquisition_instrument.TEM.convergence_angle', None), 
             ...
             'ImageList.TagGroup0.ImageTags.EELS.Acquisition.Number_of_frames': ('Acquisition_instrument.TEM.Detector.EELS.frame_number', None), 
             ...
             'ImageList.TagGroup0.ImageTags.EELS Spectrometer.Instrument name': ('Acquisition_instrument.TEM.Detector.EELS.spectrometer', None)})
        else:
            if self.signal_type == 'EDS_TEM':
                mapping.update({'ImageList.TagGroup0.ImageTags.EDS.Acquisition.Date': (
                                                                        'General.date',
                                                                        self._get_date), 
                ...
                if 'DigiScan' in self.imdict.ImageTags.keys():
                    mapping.update({'ImageList.TagGroup0.ImageTags.DigiScan.Sample Time': (
                                                                            'Acquisition_instrument.TEM.dwell_time',
                                                                            lambda x: x / 1000000.0)})
                else:
                    mapping.update({'ImageList.TagGroup0.ImageTags.Acquisition.Parameters.Detector.' + 'exposure_s': ('Acquisition_instrument.TEM.Camera.exposure', None)})
        mapping.update({'ImageList.TagGroup0.ImageData.Calibrations.Brightness.Units': (
                                                                         'Signal.quantity',
                                                                         self._get_quantity), 
         
         'ImageList.TagGroup0.ImageData.Calibrations.Brightness.Scale': ('Signal.Noise_properties.Variance_linear_model.gain_factor', None), 
         
         'ImageList.TagGroup0.ImageData.Calibrations.Brightness.Origin': ('Signal.Noise_properties.Variance_linear_model.gain_offset', None)})
        return mapping
...
def file_reader(filename, record_by=None, order=None, lazy=False):
    ...
    with open(filename, 'rb') as (f):
        ...
        dm.parse_file()
        images = [ImageObject(imdict, f, order=order, record_by=record_by) for imdict in dm.get_image_dictionaries()]
        imd = []
