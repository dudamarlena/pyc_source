...
# [GCC 8.4.0]
# Embedded file name: /home/conrad/prog/galileo-engineio/venv/lib/python3.6/site-packages/tests/asyncio/test_asyncio_client.py
# Compiled at: 2019-08-30 19:22:58
# Size of source mod 2**32: 45447 bytes
import asyncio, sys, time, unittest, six
if six.PY3:
...
class TestAsyncClient(unittest.TestCase):
    ...
    def test_connect_polling(self):
        ...
        c._connect_polling.mock.assert_called_once_with('http://foo', {}, 'engine.io')
        c = asyncio_client.AsyncClient()
        c._connect_polling = AsyncMock(return_value='foo')
        self.assertEqual(_run(c.connect('http://foo', transports=['polling'])), 'foo')
        c._connect_polling.mock.assert_called_once_with('http://foo', {}, 'engine.io')
        c = asyncio_client.AsyncClient()
        c._connect_polling = AsyncMock(return_value='foo')
        self.assertEqual(_run(c.connect('http://foo', transports=['polling',
         'websocket'])), 'foo')
        c._connect_polling.mock.assert_called_once_with('http://foo', {}, 'engine.io')
    ...
    def test_connect_websocket(self):
        ...
        self.assertEqual(_run(c.connect('http://foo', transports=['websocket'])), 'foo')
        c._connect_websocket.mock.assert_called_once_with('http://foo', {}, 'engine.io')
        c = asyncio_client.AsyncClient()
        c._connect_websocket = AsyncMock(return_value='foo')
        self.assertEqual(_run(c.connect('http://foo', transports='websocket')), 'foo')
        c._connect_websocket.mock.assert_called_once_with('http://foo', {}, 'engine.io')
    ...
    def test_connect_query_string(self):
        ...
        c._connect_polling.mock.assert_called_once_with('http://foo?bar=baz', {}, 'engine.io')

    def test_connect_custom_headers(self):
        c = asyncio_client.AsyncClient()
        c._connect_polling = AsyncMock(return_value='foo')
        self.assertEqual(_run(c.connect('http://foo', headers={'Foo': 'Bar'})), 'foo')
        c._connect_polling.mock.assert_called_once_with('http://foo', {'Foo': 'Bar'}, 'engine.io')
    ...
    def test_disconnect_polling(self):
        ...
        c._trigger_event.mock.assert_called_once_with('disconnect', run_async=False)
    ...
    def test_polling_connection_failed(self, _time):
        ...
        self.assertRaises(exceptions.ConnectionError, _run, c.connect('http://foo',
          headers={'Foo': 'Bar'}))
        c._send_request.mock.assert_called_once_with('GET',
    ...
    def test_polling_connection_no_open_packet(self):
        ...
        c._send_request = AsyncMock()
        c._send_request.mock.return_value.status = 200
        c._send_request.mock.return_value.read = AsyncMock(return_value=(payload.Payload(packets=[
    ...
    def test_polling_connection_with_more_packets(self):
        ...
        self.assertEqual(c._receive_packet.mock.call_args_list[0][0][0].packet_type, packet.NOOP)
    ...
    def test_polling_connection_upgraded(self):
        ...
        c._send_request.mock.return_value.read = AsyncMock(return_value=(payload.Payload(packets=[
         packet.Packet(packet.OPEN, {'sid':'123', 
          'upgrades':['websocket'],  'pingInterval':1000, 
          'pingTimeout':2000})]).encode()))
    ...
    @mock.patch('engineio.client.time.time', return_value=123.456)
    @mock.patch('engineio.asyncio_client.websockets.connect', new=AsyncMock(side_effect=[
     websockets.exceptions.InvalidURI]))
    def test_websocket_connection_failed(self, _time):
        c = asyncio_client.AsyncClient()
        self.assertRaises(exceptions.ConnectionError, _run, c.connect('http://foo', transports=['websocket'], headers={'Foo': 'Bar'}))
        asyncio_client.websockets.connect.mock.assert_called_once_with('ws://foo/engine.io/?transport=websocket&EIO=3&t=123.456',
    ...
    def test_websocket_upgrade_failed(self, _time):
        c = asyncio_client.AsyncClient()
        c.sid = '123'
        self.assertFalse(_run(c.connect('http://foo',
          transports=['websocket'])))
        asyncio_client.websockets.connect.mock.assert_called_once_with('ws://foo/engine.io/?transport=websocket&EIO=3&sid=123&t=123.456',
          extra_headers={})

    @mock.patch('engineio.asyncio_client.websockets.connect', new=(AsyncMock()))
    def test_websocket_connection_no_open_packet(self):
        asyncio_client.websockets.connect.mock.return_value.recv = AsyncMock(return_value=(packet.Packet(packet.CLOSE).encode()))
        c = asyncio_client.AsyncClient()
        self.assertRaises(exceptions.ConnectionError, _run, c.connect('http://foo', transports=['websocket']))

    @mock.patch('engineio.client.time.time', return_value=123.456)
    @mock.patch('engineio.asyncio_client.websockets.connect', new=(AsyncMock()))
    def test_websocket_connection_successful(self, _time):
        ws = asyncio_client.websockets.connect.mock.return_value
        ws.recv = AsyncMock(return_value=(packet.Packet(packet.OPEN, {'sid':'123', 
         'upgrades':[],  'pingInterval':1000,  'pingTimeout':2000}).encode()))
        c = asyncio_client.AsyncClient()
        c._ping_loop = AsyncMock()
        c._read_loop_polling = AsyncMock()
    ...
    def test_websocket_upgrade_no_pong(self):
        ...
        on_connect = mock.MagicMock()
        c.on('connect', on_connect)
        self.assertFalse(_run(c.connect('ws://foo', transports=[
         'websocket'])))
    ...
    def test_websocket_upgrade_successful(self):
        ws = asyncio_client.websockets.connect.mock.return_value
        ws.recv = AsyncMock(return_value=(packet.Packet(packet.PONG, 'probe').encode()))
        ws.send = AsyncMock()
        c = asyncio_client.AsyncClient()
        c.sid = '123'
        c.base_url = 'http://foo'
        ...
        on_connect = mock.MagicMock()
        c.on('connect', on_connect)
        self.assertTrue(_run(c.connect('ws://foo', transports=[
         'websocket'])))
        time.sleep(0.1)
        c._ping_loop.mock.assert_called_once_with()
        c._read_loop_polling.mock.assert_not_called()
        ...
        self.assertEqual(c.transport(), 'websocket')
        self.assertEqual(c.ws, ws)
        self.assertEqual(ws.send.mock.call_args_list[0], (('2probe',), ))
        self.assertEqual(ws.send.mock.call_args_list[1], (('5',), ))
    ...
    def test_receive_message_packet(self):
        ...
        c._trigger_event.mock.assert_called_once_with('message',
          {'foo': 'bar'}, run_async=True)
    ...
    def test_trigger_event_function_async_error(self):
        ...
        c.on('message', handler=foo_handler)
        fut = _run(c._trigger_event('message', 'bar', run_async=True))
        self.assertRaises(ZeroDivisionError, asyncio.get_event_loop().run_until_complete, fut)
    ...
    def test_ping_loop_disconnect(self):
        ...
        _run(c._ping_loop())
        self.assertEqual(c._send_packet.mock.call_args_list[0][0][0].encode(), b'2')
    ...
    def test_read_loop_polling_no_response(self, _time):
        ...
        self.assertEqual(c.state, 'disconnected')
        c.queue.put.mock.assert_called_once_with(None)
        c._send_request.mock.assert_called_once_with('GET', 'http://foo&t=123.456')
        c._trigger_event.mock.assert_called_once_with('disconnect', run_async=False)
    ...
    def test_read_loop_polling(self):
        ...
        c._send_request.mock.side_effect = [
         mock.MagicMock(status=200, read=AsyncMock(return_value=(payload.Payload(packets=[
        ...
        self.assertEqual(c._receive_packet.mock.call_args_list[0][0][0].encode(), b'2')
        self.assertEqual(c._receive_packet.mock.call_args_list[1][0][0].encode(), b'6')
    ...
    def test_read_loop_websocket_no_response(self):
        ...
        c.queue.put = AsyncMock()
        c.ws = mock.MagicMock()
        c.ws.recv = AsyncMock(side_effect=(websockets.exceptions.ConnectionClosed(1, 'foo')))
        c.write_loop_task = AsyncMock()()
        c.ping_loop_task = AsyncMock()()
        _run(c._read_loop_websocket())
        self.assertEqual(c.state, 'disconnected')
    ...
    def test_write_loop_polling_one_packet(self):
        ...
        p = payload.Payload(packets=[
         packet.Packet(packet.MESSAGE, {'foo': 'bar'})])
        c._send_request.mock.assert_called_once_with('POST',
    ...
    def test_write_loop_websocket_three_packets(self):
        ...
        self.assertEqual(c.ws.send.mock.call_args_list[0][0][0], '4{"foo":"bar"}')
    ...
    def test_write_loop_websocket_bad_connection(self):
        ...
        c.queue.get_nowait = mock.MagicMock(side_effect=[
         RuntimeError])
        c.ws = mock.MagicMock()
        c.ws.send = AsyncMock(side_effect=(websockets.exceptions.ConnectionClosed(1, 'foo')))
        _run(c._write_loop())
        self.assertEqual(c.state, 'connected')