...
# [GCC 8.4.0]
# Embedded file name: /home/conrad/prog/galileo-engineio/venv/lib/python3.6/site-packages/tests/common/test_client.py
# Compiled at: 2019-08-30 19:22:58
# Size of source mod 2**32: 43632 bytes
import json, logging, time, unittest, six
if six.PY3:
...
class TestClient(unittest.TestCase):
    ...
    def test_create(self):
        ...
        for attr in ('base_url', 'transports', 'sid', 'upgrades', 'ping_interval',
                     'ping_timeout', 'http', 'ws', 'read_loop_task', 'write_loop_task',
                     'queue'):
    ...
    def test_invalid_transports(self):
        c = client.Client()
        self.assertRaises(ValueError, (c.connect), 'http://foo', transports=[
         'foo', 'bar'])
    ...
    def test_connect_polling(self):
        ...
        c._connect_polling.assert_called_once_with('http://foo', {}, 'engine.io')
        c = client.Client()
        c._connect_polling = mock.MagicMock(return_value='foo')
        self.assertEqual(c.connect('http://foo', transports=['polling']), 'foo')
        c._connect_polling.assert_called_once_with('http://foo', {}, 'engine.io')
        c = client.Client()
        c._connect_polling = mock.MagicMock(return_value='foo')
        self.assertEqual(c.connect('http://foo', transports=[
         'polling', 'websocket']), 'foo')
        c._connect_polling.assert_called_once_with('http://foo', {}, 'engine.io')

    def test_connect_websocket(self):
        c = client.Client()
        c._connect_websocket = mock.MagicMock(return_value='foo')
        self.assertEqual(c.connect('http://foo', transports=['websocket']), 'foo')
        c._connect_websocket.assert_called_once_with('http://foo', {}, 'engine.io')
        c = client.Client()
        c._connect_websocket = mock.MagicMock(return_value='foo')
        self.assertEqual(c.connect('http://foo', transports='websocket'), 'foo')
        c._connect_websocket.assert_called_once_with('http://foo', {}, 'engine.io')
    ...
    def test_connect_query_string(self):
        ...
        c._connect_polling.assert_called_once_with('http://foo?bar=baz', {}, 'engine.io')

    def test_connect_custom_headers(self):
        c = client.Client()
        c._connect_polling = mock.MagicMock(return_value='foo')
        self.assertEqual(c.connect('http://foo', headers={'Foo': 'Bar'}), 'foo')
        c._connect_polling.assert_called_once_with('http://foo', {'Foo': 'Bar'}, 'engine.io')
    ...
    def test_polling_connection_failed(self, _send_request, _time):
        c = client.Client()
        self.assertRaises((exceptions.ConnectionError), (c.connect), 'http://foo', headers={'Foo': 'Bar'})
        _send_request.assert_called_once_with('GET',
          'http://foo/engine.io/?transport=polling&EIO=3&t=123.456', headers={'Foo': 'Bar'})
    ...
    def test_polling_connection_no_open_packet(self, _send_request):
        ...
        _send_request.return_value.content = payload.Payload(packets=[
         packet.Packet(packet.CLOSE, {'sid':'123', 
          'upgrades':[],  'pingInterval':10,  'pingTimeout':20})]).encode()
        c = client.Client()
        self.assertRaises(exceptions.ConnectionError, c.connect, 'http://foo')
    ...
    def test_polling_connection_successful(self, _send_request):
        _send_request.return_value.status_code = 200
        _send_request.return_value.content = payload.Payload(packets=[
         packet.Packet(packet.OPEN, {'sid':'123', 
          'upgrades':[],  'pingInterval':1000,  'pingTimeout':2000})]).encode()
        c = client.Client()
        ...
        self.assertEqual(c.base_url, 'http://foo/engine.io/?transport=polling&EIO=3&sid=123')
    ...
    def test_polling_connection_with_more_packets(self, _send_request):
        ...
        _send_request.return_value.content = payload.Payload(packets=[
         packet.Packet(packet.OPEN, {'sid':'123', 
          'upgrades':[],  'pingInterval':1000,  'pingTimeout':2000}),
        ...
        self.assertEqual(c._receive_packet.call_args_list[0][0][0].packet_type, packet.NOOP)
    ...
    def test_polling_connection_upgraded(self, _send_request):
        ...
        _send_request.return_value.content = payload.Payload(packets=[
         packet.Packet(packet.OPEN, {'sid':'123', 
          'upgrades':['websocket'],  'pingInterval':1000,  'pingTimeout':2000})]).encode()
        c = client.Client()
    ...
    @mock.patch('engineio.client.time.time', return_value=123.456)
    @mock.patch('engineio.client.websocket.create_connection', side_effect=[
     ConnectionError])
    def test_websocket_connection_failed(self, create_connection, _time):
        c = client.Client()
        self.assertRaises((exceptions.ConnectionError), (c.connect), 'http://foo', transports=[
         'websocket'],
          headers={'Foo': 'Bar'})
        create_connection.assert_called_once_with('ws://foo/engine.io/?transport=websocket&EIO=3&t=123.456',
          header={'Foo': 'Bar'},
          cookie=None)
    ...
    def test_websocket_upgrade_failed(self, create_connection, _time):
        ...
        create_connection.assert_called_once_with('ws://foo/engine.io/?transport=websocket&EIO=3&sid=123&t=123.456',
          header={}, cookie=None)
    ...
    def test_websocket_connection_no_open_packet(self, create_connection):
        create_connection.return_value.recv.return_value = packet.Packet(packet.CLOSE).encode()
        c = client.Client()
        self.assertRaises((exceptions.ConnectionError), (c.connect), 'http://foo', transports=[
         'websocket'])
    ...
    def test_websocket_connection_successful(self, create_connection):
        create_connection.return_value.recv.return_value = packet.Packet(packet.OPEN, {'sid':'123', 
         'upgrades':[],  'pingInterval':1000,  'pingTimeout':2000}).encode()
        c = client.Client()
        c._ping_loop = mock.MagicMock()
        c._read_loop_polling = mock.MagicMock()
        ...
        on_connect.assert_called_once_with()
        self.assertIn(c, client.connected_clients)
        self.assertEqual(c.base_url, 'ws://foo/engine.io/?transport=websocket&EIO=3')
        self.assertEqual(c.sid, '123')
        self.assertEqual(c.ping_interval, 1)
        self.assertEqual(c.ping_timeout, 2)
        self.assertEqual(c.upgrades, [])
        ...
        self.assertEqual(create_connection.call_args[1], {'header':{},  'cookie':None})
    ...
    def test_websocket_connection_with_cookies(self, create_connection):
        ...
        time.sleep(0.1)
        self.assertEqual(len(create_connection.call_args_list), 1)
        self.assertEqual(create_connection.call_args[1], {'header':{},  'cookie':'key=value; key2=value2'})
    ...
    def test_websocket_upgrade_successful(self, create_connection):
        create_connection.return_value.recv.return_value = packet.Packet(packet.PONG, six.text_type('probe')).encode()
        c = client.Client()
        c.sid = '123'
        c.base_url = 'http://foo'
        c.current_transport = 'polling'
        ...
        on_connect.assert_not_called()
        self.assertNotIn(c, client.connected_clients)
        self.assertEqual(c.base_url, 'http://foo')
        self.assertEqual(c.sid, '123')
        self.assertEqual(c.transport(), 'websocket')
        self.assertEqual(c.ws, create_connection.return_value)
        self.assertEqual(create_connection.return_value.send.call_args_list[0], (
         (
          packet.Packet(packet.PING, six.text_type('probe')).encode(),),))
        self.assertEqual(create_connection.return_value.send.call_args_list[1], (
         (
    ...
    def test_receive_message_packet(self):
        ...
        c._trigger_event.assert_called_once_with('message', {'foo': 'bar'}, run_async=True)
    ...
    def test_engineio_url(self):
        c = client.Client()
        self.assertEqual(c._get_engineio_url('http://foo', 'bar', 'polling'), 'http://foo/bar/?transport=polling&EIO=3')
        self.assertEqual(c._get_engineio_url('http://foo', 'bar', 'websocket'), 'ws://foo/bar/?transport=websocket&EIO=3')
        self.assertEqual(c._get_engineio_url('ws://foo', 'bar', 'polling'), 'http://foo/bar/?transport=polling&EIO=3')
        self.assertEqual(c._get_engineio_url('ws://foo', 'bar', 'websocket'), 'ws://foo/bar/?transport=websocket&EIO=3')
        self.assertEqual(c._get_engineio_url('https://foo', 'bar', 'polling'), 'https://foo/bar/?transport=polling&EIO=3')
        self.assertEqual(c._get_engineio_url('https://foo', 'bar', 'websocket'), 'wss://foo/bar/?transport=websocket&EIO=3')
        self.assertEqual(c._get_engineio_url('http://foo?baz=1', 'bar', 'polling'), 'http://foo/bar/?baz=1&transport=polling&EIO=3')
        self.assertEqual(c._get_engineio_url('http://foo#baz', 'bar', 'polling'), 'http://foo/bar/?transport=polling&EIO=3')
    ...
    def test_read_loop_polling(self):
        ...
        self.assertEqual(c._receive_packet.call_args_list[0][0][0].encode(), b'2')
        self.assertEqual(c._receive_packet.call_args_list[1][0][0].encode(), b'6')
    ...
    def test_write_loop_polling_one_packet(self):
        ...
        p = payload.Payload(packets=[
         packet.Packet(packet.MESSAGE, {'foo': 'bar'})])
        c._send_request.assert_called_once_with('POST',
          'http://foo', body=(p.encode()), headers={'Content-Type': 'application/octet-stream'})
    ...
    def test_signal_handler(self, original_handler):
        ...
        clients[1].start_background_task.assert_called_once_with((clients[1].disconnect),
          abort=True)