...
# [GCC 8.4.0]
# Embedded file name: /home/conrad/prog/galileo-engineio/venv/lib/python3.6/site-packages/engineio/server.py
# Compiled at: 2019-08-30 19:22:57
# Size of source mod 2**32: 26429 bytes
import gzip, importlib, logging, uuid, zlib, six
...
class Server(object):
    ...
    def __init__(self, async_mode=None, ping_timeout=60, ping_interval=25, max_http_buffer_size=100000000, allow_upgrades=True, http_compression=True, compression_threshold=1024, cookie='io', cors_allowed_origins=None, cors_credentials=True, logger=False, json=None, async_handlers=True, monitor_clients=None, **kwargs):
        ...
        self.sockets = {}
        self.handlers = {}
        self.start_service_task = monitor_clients if monitor_clients is not None else self._default_monitor_clients
        if json is not None:
        ...
        for mode in modes:
            try:
                self._async = importlib.import_module('engineio.async_drivers.' + mode)._async
                asyncio_based = self._async['asyncio'] if 'asyncio' in self._async else False
        ...
        if self.is_asyncio_based():
            if 'asyncio' not in self._async or not self._async['asyncio']:
                raise ValueError('The selected async_mode is not asyncio compatible')
        if not self.is_asyncio_based():
            if 'asyncio' in self._async:
                if self._async['asyncio']:
                    raise ValueError('The selected async_mode requires asyncio and must use the AsyncServer class')
    ...
    def transport(self, sid):
        ...
        """
        if self._get_socket(sid).upgraded:
            return 'websocket'
        else:
            return 'polling'
    ...
    def _upgrades(self, sid, transport):
        ...
        if not self.allow_upgrades or self._get_socket(sid).upgraded or self._async['websocket'] is None or transport == 'websocket':
            return []
        else:
            return [
             'websocket']
    ...
    def _cors_headers(self, environ):
        ...
        if allowed_origins is not None:
            if environ.get('HTTP_ORIGIN', '') not in allowed_origins:
                return []
        else:
