...
# [GCC 8.4.0]
# Embedded file name: /home/conrad/prog/galileo-engineio/venv/lib/python3.6/site-packages/engineio/asyncio_client.py
# Compiled at: 2019-08-30 19:22:57
# Size of source mod 2**32: 22993 bytes
import asyncio
try:
    import aiohttp
except ImportError:
...
class AsyncClient(client.Client):
    ...
    async def _connect_polling(self, url, headers, engineio_path):
        ...
        if aiohttp is None:
            ...
            r = await self._send_request('GET',
              (self.base_url + self._get_url_timestamp()), headers=headers)
            if r is None:
                self._reset()
                raise exceptions.ConnectionError('Connection refused by the server')
            if r.status != 200:
                raise exceptions.ConnectionError('Unexpected status code {} in server response'.format(r.status))
            try:
    ...
    async def _connect_websocket(self, url, headers, engineio_path):
        ...
        if websockets is None:
            ...
            try:
                ws = await websockets.connect((websocket_url + self._get_url_timestamp()),
    ...
    async def _ping_loop(self):
        ...
        while self.state == 'connected':
            ...
            try:
                await asyncio.wait_for(self.ping_loop_event.wait(), self.ping_interval)
    ...
    async def _read_loop_polling(self):
        ...
        while self.state == 'connected':
            ...
            r = await self._send_request('GET', self.base_url + self._get_url_timestamp())
            if r is None:
                self.logger.warning('Connection refused by the server, aborting')
                await self.queue.put(None)
                break
            if r.status != 200:
                self.logger.warning('Unexpected status code %s in server response, aborting', r.status)
                await self.queue.put(None)
                break
            try:
                p = payload.Payload(encoded_payload=(await r.read()))
    ...
    async def _write_loop(self):
        ...
        while self.state == 'connected':
            ...
            except (self.queue.Empty, asyncio.TimeoutError, asyncio.CancelledError):
            ...
            if self.current_transport == 'polling':
                ...
                r = await self._send_request('POST',
                ...
                try:
                    ...
                    self.logger.info('Write loop: WebSocket connection was closed, aborting')
                    break
