...
# [GCC 8.4.0]
# Embedded file name: /home/conrad/prog/galileo-engineio/venv/lib/python3.6/site-packages/tests/asyncio/test_asyncio_server.py
# Compiled at: 2019-08-30 19:22:58
# Size of source mod 2**32: 38915 bytes
import asyncio, gzip, json, logging, sys, unittest, zlib, six
if six.PY3:
...
class TestAsyncServer(unittest.TestCase):
    ...
    def test_async_mode_aiohttp(self):
        ...
        self.assertEqual(s._async['translate_request'], async_aiohttp.translate_request)
        self.assertEqual(s._async['make_response'], async_aiohttp.make_response)
    ...
    def test_async_modes_wsgi(self):
        self.assertRaises(ValueError, (asyncio_server.AsyncServer), async_mode='eventlet')
        self.assertRaises(ValueError, (asyncio_server.AsyncServer), async_mode='gevent')
        self.assertRaises(ValueError, (asyncio_server.AsyncServer), async_mode='gevent_uwsgi')
        self.assertRaises(ValueError, (asyncio_server.AsyncServer), async_mode='threading')
    ...
    def test_jsonp_not_supported(self, import_module):
        a = self.get_async_mock({'REQUEST_METHOD':'GET',  'QUERY_STRING':'j=abc'})
        import_module.side_effect = [a]
        s = asyncio_server.AsyncServer()
        ...
        self.assertEqual(a._async['make_response'].call_args[0][0], '400 BAD REQUEST')
    ...
    def test_connect(self, import_module):
        ...
        self.assertIn(('Content-Type', 'application/octet-stream'), a._async['make_response'].call_args[0][1])
        packets = payload.Payload(encoded_payload=(a._async['make_response'].call_args[0][2])).packets
    ...
    def test_connect_async_request_response_handlers(self, import_module):
        a = self.get_async_mock()
        a._async['translate_request'] = AsyncMock(return_value=(a._async['translate_request'].return_value))
        a._async['make_response'] = AsyncMock(return_value=(a._async['make_response'].return_value))
        import_module.side_effect = [a]
        s = asyncio_server.AsyncServer()
        _run(s.handle_request('request'))
        self.assertEqual(len(s.sockets), 1)
        self.assertEqual(a._async['make_response'].mock.call_count, 1)
        self.assertEqual(a._async['make_response'].mock.call_args[0][0], '200 OK')
        self.assertIn(('Content-Type', 'application/octet-stream'), a._async['make_response'].mock.call_args[0][1])
        packets = payload.Payload(encoded_payload=(a._async['make_response'].mock.call_args[0][2])).packets
        self.assertEqual(len(packets), 1)
        self.assertEqual(packets[0].packet_type, packet.OPEN)
        self.assertIn('upgrades', packets[0].data)
        self.assertEqual(packets[0].data['upgrades'], ['websocket'])
    ...
    def test_connect_b64_with_1(self, import_module):
        a = self.get_async_mock({'REQUEST_METHOD':'GET',  'QUERY_STRING':'b64=1'})
        import_module.side_effect = [a]
        s = asyncio_server.AsyncServer(allow_upgrades=False)
        ...
        self.assertEqual(a._async['make_response'].call_count, 1)
        self.assertEqual(a._async['make_response'].call_args[0][0], '200 OK')
        self.assertIn(('Content-Type', 'text/plain; charset=UTF-8'), a._async['make_response'].call_args[0][1])
        _run(s.send('1', b'\x00\x01\x02', binary=True))
        a._async['translate_request'].return_value = {'REQUEST_METHOD':'GET', 
         'QUERY_STRING':'sid=1&b64=1'}
        _run(s.handle_request('request'))
        self.assertEqual(a._async['make_response'].call_args[0][2], b'6:b4AAEC')

    @mock.patch('importlib.import_module')
    def test_connect_b64_with_true(self, import_module):
        a = self.get_async_mock({'REQUEST_METHOD':'GET',  'QUERY_STRING':'b64=true'})
        import_module.side_effect = [a]
        s = asyncio_server.AsyncServer(allow_upgrades=False)
        s._generate_id = mock.MagicMock(return_value='1')
        _run(s.handle_request('request'))
    ...
    def test_connect_b64_with_0(self, import_module):
        a = self.get_async_mock({'REQUEST_METHOD':'GET',  'QUERY_STRING':'b64=0'})
        import_module.side_effect = [a]
        s = asyncio_server.AsyncServer(allow_upgrades=False)
        s._generate_id = mock.MagicMock(return_value='1')
        _run(s.handle_request('request'))
        ...
        _run(s.handle_request('request'))
        self.assertEqual(a._async['make_response'].call_args[0][2], b'\x01\x04\xff\x04\x00\x01\x02')

    @mock.patch('importlib.import_module')
    def test_connect_b64_with_false(self, import_module):
        a = self.get_async_mock({'REQUEST_METHOD':'GET',  'QUERY_STRING':'b64=false'})
        import_module.side_effect = [a]
        s = asyncio_server.AsyncServer(allow_upgrades=False)
        s._generate_id = mock.MagicMock(return_value='1')
        _run(s.handle_request('request'))
    ...
    def test_connect_transport_websocket(self, import_module, AsyncSocket):
        a = self.get_async_mock({'REQUEST_METHOD':'GET',  'QUERY_STRING':'transport=websocket'})
        import_module.side_effect = [a]
        AsyncSocket.return_value = self._get_mock_socket()
        s = asyncio_server.AsyncServer()
        ...
        _run(s.handle_request('request'))
        self.assertEqual(s.sockets['123'].send.mock.call_args[0][0].packet_type, packet.OPEN)

    @mock.patch('engineio.asyncio_socket.AsyncSocket')
    @mock.patch('importlib.import_module')
    def test_connect_transport_websocket_closed(self, import_module, AsyncSocket):
        a = self.get_async_mock({'REQUEST_METHOD':'GET',  'QUERY_STRING':'transport=websocket'})
        import_module.side_effect = [a]
        AsyncSocket.return_value = self._get_mock_socket()
        s = asyncio_server.AsyncServer()
    ...
    def test_connect_transport_invalid(self, import_module):
        a = self.get_async_mock({'REQUEST_METHOD':'GET',  'QUERY_STRING':'transport=foo'})
        import_module.side_effect = [a]
        s = asyncio_server.AsyncServer()
        _run(s.handle_request('request'))
        self.assertEqual(a._async['make_response'].call_count, 1)
    ...
    def test_connect_cors_allowed_origin(self, import_module):
        a = self.get_async_mock({'REQUEST_METHOD':'GET',  'QUERY_STRING':'',  'HTTP_ORIGIN':'b'})
        import_module.side_effect = [a]
    ...
    def test_connect_cors_not_allowed_origin(self, import_module):
        a = self.get_async_mock({'REQUEST_METHOD':'GET',  'QUERY_STRING':'',  'HTTP_ORIGIN':'c'})
        import_module.side_effect = [a]
        s = asyncio_server.AsyncServer(cors_allowed_origins=['a', 'b'])
        _run(s.handle_request('request'))
        headers = a._async['make_response'].call_args[0][1]
    ...
    def test_connect_cors_options(self, import_module):
        a = self.get_async_mock({'REQUEST_METHOD':'OPTIONS',  'QUERY_STRING':''})
        import_module.side_effect = [a]
        s = asyncio_server.AsyncServer(cors_credentials=False)
        _run(s.handle_request('request'))
        headers = a._async['make_response'].call_args[0][1]
        self.assertIn(('Access-Control-Allow-Methods', 'OPTIONS, GET, POST'), headers)
    ...
    def test_connect_event_rejects(self, import_module):
        ...
        self.assertEqual(a._async['make_response'].call_args[0][0], '401 UNAUTHORIZED')
    ...
    def test_method_not_found(self, import_module):
        ...
        _run(s.handle_request('request'))
        self.assertEqual(len(s.sockets), 0)
        self.assertEqual(a._async['make_response'].call_args[0][0], '405 METHOD NOT FOUND')

    @mock.patch('importlib.import_module')
    def test_get_request_with_bad_sid(self, import_module):
        a = self.get_async_mock({'REQUEST_METHOD':'GET',  'QUERY_STRING':'sid=foo'})
        import_module.side_effect = [a]
        s = asyncio_server.AsyncServer()
        _run(s.handle_request('request'))
        self.assertEqual(len(s.sockets), 0)
    ...
    def test_post_request_with_bad_sid(self, import_module):
        a = self.get_async_mock({'REQUEST_METHOD':'POST',  'QUERY_STRING':'sid=foo'})
        import_module.side_effect = [a]
        s = asyncio_server.AsyncServer()
        _run(s.handle_request('request'))
        self.assertEqual(len(s.sockets), 0)
    ...
    def test_send(self, import_module):
        ...
        self.assertEqual(mock_socket.send.mock.call_args[0][0].packet_type, packet.MESSAGE)
    ...
    def test_gzip_compression_threshold(self, import_module):
        ...
        self.assertRaises(IOError, self._gzip_decompress, a._async['make_response'].call_args[0][2])
    ...
    def test_compression_disabled(self, import_module):
        a = self.get_async_mock({'REQUEST_METHOD':'GET',  'QUERY_STRING':'sid=foo', 
         'HTTP_ACCEPT_ENCODING':'gzip'})
        import_module.side_effect = [a]
        s = asyncio_server.AsyncServer(http_compression=False, compression_threshold=0)
        s.sockets['foo'] = mock_socket = self._get_mock_socket()
        mock_socket.handle_get_request.mock.return_value = [
         packet.Packet((packet.MESSAGE), data='hello')]
        _run(s.handle_request('request'))
