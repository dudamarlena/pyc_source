...
# [GCC 8.4.0]
# Embedded file name: /home/conrad/prog/galileo-engineio/venv/lib/python3.6/site-packages/tests/asyncio/test_async_asgi.py
# Compiled at: 2019-08-30 19:22:58
# Size of source mod 2**32: 9421 bytes
import os, sys, unittest, six
if six.PY3:
...
class AsgiTests(unittest.TestCase):
    ...
    def test_create_app(self):
        app = async_asgi.ASGIApp('eio',
          'other_app', static_files='static_files', engineio_path='/foo')
    ...
    def test_engineio_routing(self):
        ...
        mock_server.handle_request.mock.assert_called_once_with(scope, 'receive', 'send')
    ...
    def test_static_file_routing(self):
        ...
        app = async_asgi.ASGIApp('eio', static_files={'/foo': {'content_type':'text/html',  'filename':root_dir + '/index.html'}})
        scope = {'type':'http', 
         'path':'/foo'}
        receive = AsyncMock(return_value={'type': 'http.request'})
        send = AsyncMock()
        _run(app(scope, receive, send))
        send.mock.assert_called_with({'type':'http.response.body',  'body':b'<html></html>\n'})
    ...
    def test_lifespan_startup(self):
        ...
        send = AsyncMock()
        _run(app(scope, receive, send))
        send.mock.assert_called_once_with({'type': 'lifespan.startup.complete'})
    ...
    def test_lifespan_shutdown(self):
        ...
        send = AsyncMock()
        _run(app(scope, receive, send))
        send.mock.assert_called_once_with({'type': 'lifespan.shutdown.complete'})
    ...
    def test_not_found(self):
        ...
        send = AsyncMock()
        _run(app(scope, receive, send))
        send.mock.assert_any_call({'type':'http.response.start', 
         'status':404,  'headers':[
          (b'Content-Type', b'text/plain')]})
        send.mock.assert_any_call({'type':'http.response.body',  'body':b'not found'})

    def test_translate_request(self):
        receive = AsyncMock(return_value={'type':'http.request',  'body':b'hello world'})
        send = AsyncMock()
        environ = _run(async_asgi.translate_request({'type':'http', 
         'method':'PUT', 
         'headers':[
          (b'a', b'b'), (b'c-c', b'd'), (b'c_c', b'e'),
    ...
    def test_translate_websocket_request(self):
        ...
        send = AsyncMock()
        _run(async_asgi.translate_request({'type':'websocket', 
         'headers':[
          (b'a', b'b'), (b'c-c', b'd'), (b'c_c', b'e'),
          (b'content-type', b'application/json'),
          (b'content-length', b'123')], 
    ...
    def test_make_response(self):
        ...
        _run(async_asgi.make_response('202 ACCEPTED', [('foo', 'bar')], b'payload', environ))
        environ['asgi.send'].mock.assert_any_call({'type':'http.response.start', 
         'status':202,  'headers':[
          (b'foo', b'bar')]})
        environ['asgi.send'].mock.assert_any_call({'type':'http.response.body', 
         'body':b'payload'})