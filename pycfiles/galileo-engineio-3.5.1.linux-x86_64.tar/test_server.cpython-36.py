...
# [GCC 8.4.0]
# Embedded file name: /home/conrad/prog/galileo-engineio/venv/lib/python3.6/site-packages/tests/common/test_server.py
# Compiled at: 2019-08-30 19:22:58
# Size of source mod 2**32: 39476 bytes
import gzip, importlib, json, logging, sys, time, unittest, zlib, six
if six.PY3:
...
class TestServer(unittest.TestCase):
    ...
    def test_async_mode_gevent_uwsgi(self, import_module):
        ...
        self.assertEqual(s._async['websocket'], async_gevent_uwsgi.uWSGIWebSocket)
    ...
    def test_async_mode_gevent_uwsgi_without_uwsgi(self, import_module):
        ...
        self.assertRaises(ValueError, (server.Server), async_mode='gevent_uwsgi')
        del sys.modules['gevent']
        del sys.modules['gevent.queue']
        del sys.modules['gevent.event']
        del sys.modules['uwsgi']
    ...
    def test_connect(self):
        ...
        self.assertIn(('Content-Type', 'application/octet-stream'), start_response.call_args[0][1])
    ...
    def test_connect_b64_with_1(self):
        ...
        self.assertIn(('Content-Type', 'text/plain; charset=UTF-8'), start_response.call_args[0][1])
    ...
    @mock.patch('engineio.socket.Socket.poll', side_effect=(exceptions.QueueEmpty))
    ...
    @mock.patch('engineio.socket.Socket', return_value=mock.MagicMock(connected=False, closed=False))
    def test_connect_transport_websocket(self, Socket):
        s = server.Server()
        s._generate_id = mock.MagicMock(return_value='123')
        environ = {'REQUEST_METHOD':'GET',  'QUERY_STRING':'transport=websocket'}
        start_response = mock.MagicMock()
        Socket().closed = False
        s.handle_request(environ, start_response)
        self.assertEqual(s.sockets['123'].send.call_args[0][0].packet_type, packet.OPEN)
    ...
    def test_connect_cors_allowed_origin(self):
        ...
        environ = {'REQUEST_METHOD':'GET',  'QUERY_STRING':'',  'HTTP_ORIGIN':'b'}
        start_response = mock.MagicMock()
        s.handle_request(environ, start_response)
        headers = start_response.call_args[0][1]
    ...
    def test_connect_cors_not_allowed_origin(self):
        s = server.Server(cors_allowed_origins=['a', 'b'])
        environ = {'REQUEST_METHOD':'GET',  'QUERY_STRING':'',  'HTTP_ORIGIN':'c'}
        start_response = mock.MagicMock()
        s.handle_request(environ, start_response)
        headers = start_response.call_args[0][1]
    ...
    def test_connect_cors_headers_one_origin(self):
        ...
        environ = {'REQUEST_METHOD':'GET',  'QUERY_STRING':'',  'HTTP_ORIGIN':'a'}
        start_response = mock.MagicMock()
        s.handle_request(environ, start_response)
        headers = start_response.call_args[0][1]
    ...
    def test_cors_request_headers(self):
        s = server.Server()
        environ = {'REQUEST_METHOD':'GET',  'HTTP_ACCESS_CONTROL_REQUEST_HEADERS':'Foo, Bar'}
        start_response = mock.MagicMock()
        s.handle_request(environ, start_response)
        headers = start_response.call_args[0][1]
    ...
    def test_method_not_found(self):
        ...
        start_response = mock.MagicMock()
        s.handle_request(environ, start_response)
        self.assertEqual(start_response.call_args[0][0], '405 METHOD NOT FOUND')
    ...
    def test_send(self):
        ...
        self.assertEqual(mock_socket.send.call_args[0][0].packet_type, packet.MESSAGE)
    ...
    def test_get_request_error(self):
        s = server.Server()
        mock_socket = self._get_mock_socket()
        mock_socket.handle_get_request = mock.MagicMock(side_effect=[
    ...
    def test_post_request_error(self):
        s = server.Server()
        mock_socket = self._get_mock_socket()
        mock_socket.handle_post_request = mock.MagicMock(side_effect=[
         exceptions.EngineIOError])
        s.sockets['foo'] = mock_socket
        environ = {'REQUEST_METHOD':'POST',  'QUERY_STRING':'sid=foo'}
        start_response = mock.MagicMock()
        s.handle_request(environ, start_response)
    ...
    def test_gzip_compression(self):
        ...
        mock_socket.handle_get_request = mock.MagicMock(return_value=[
         packet.Packet((packet.MESSAGE), data='hello')])
        s.sockets['foo'] = mock_socket
        environ = {'REQUEST_METHOD':'GET',  'QUERY_STRING':'sid=foo',  'HTTP_ACCEPT_ENCODING':'gzip,deflate'}
        start_response = mock.MagicMock()
        r = s.handle_request(environ, start_response)
        self.assertIn(('Content-Encoding', 'gzip'), start_response.call_args[0][1])
    ...
    def test_deflate_compression(self):
        ...
        mock_socket.handle_get_request = mock.MagicMock(return_value=[
         packet.Packet((packet.MESSAGE), data='hello')])
        s.sockets['foo'] = mock_socket
        environ = {'REQUEST_METHOD':'GET',  'QUERY_STRING':'sid=foo',  'HTTP_ACCEPT_ENCODING':'deflate;q=1,gzip'}
        start_response = mock.MagicMock()
        r = s.handle_request(environ, start_response)
        self.assertIn(('Content-Encoding', 'deflate'), start_response.call_args[0][1])
        zlib.decompress(r[0])
    ...
    def test_gzip_compression_threshold(self):
        ...
        mock_socket.handle_get_request = mock.MagicMock(return_value=[
         packet.Packet((packet.MESSAGE), data='hello')])
        s.sockets['foo'] = mock_socket
        environ = {'REQUEST_METHOD':'GET',  'QUERY_STRING':'sid=foo',  'HTTP_ACCEPT_ENCODING':'gzip'}
        start_response = mock.MagicMock()
        r = s.handle_request(environ, start_response)
    ...
    def test_compression_unknown(self):
        ...
        mock_socket.handle_get_request = mock.MagicMock(return_value=[
         packet.Packet((packet.MESSAGE), data='hello')])
        s.sockets['foo'] = mock_socket
        environ = {'REQUEST_METHOD':'GET',  'QUERY_STRING':'sid=foo',  'HTTP_ACCEPT_ENCODING':'rar'}
        start_response = mock.MagicMock()
        r = s.handle_request(environ, start_response)
        for header, value in start_response.call_args[0][1]:
            self.assertNotEqual(header, 'Content-Encoding')
    ...
    def test_compression_no_encoding(self):
        ...
        mock_socket.handle_get_request = mock.MagicMock(return_value=[
         packet.Packet((packet.MESSAGE), data='hello')])
        s.sockets['foo'] = mock_socket
        environ = {'REQUEST_METHOD':'GET',  'QUERY_STRING':'sid=foo',  'HTTP_ACCEPT_ENCODING':''}
        start_response = mock.MagicMock()
        r = s.handle_request(environ, start_response)
        for header, value in start_response.call_args[0][1]:
            self.assertNotEqual(header, 'Content-Encoding')
    ...
    def test_cookie(self):
        ...
        start_response = mock.MagicMock()
        s.handle_request(environ, start_response)
        self.assertIn(('Set-Cookie', 'sid=123'), start_response.call_args[0][1])
