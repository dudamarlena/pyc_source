...
# [GCC 8.4.0]
# Embedded file name: /home/conrad/prog/galileo-engineio/venv/lib/python3.6/site-packages/engineio/async_drivers/sanic.py
...
def create_route(app, engineio_server, engineio_endpoint):
    ...
    endpoint.
    """
    app.add_route((engineio_server.handle_request), engineio_endpoint, methods=[
     'GET', 'POST', 'OPTIONS'])
    try:
...
def translate_request(request):
    ...
    environ = {'wsgi.input':AwaitablePayload(request.body), 
...
def make_response(status, headers, payload, environ):
    ...
    return HTTPResponse(body_bytes=payload, content_type=content_type, status=(int(status.split()[0])),
