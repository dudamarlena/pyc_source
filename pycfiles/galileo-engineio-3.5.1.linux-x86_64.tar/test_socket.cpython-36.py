...
# [GCC 8.4.0]
# Embedded file name: /home/conrad/prog/galileo-engineio/venv/lib/python3.6/site-packages/tests/common/test_socket.py
# Compiled at: 2019-08-30 19:22:58
# Size of source mod 2**32: 16964 bytes
import time, unittest, six
if six.PY3:
...
class TestSocket(unittest.TestCase):
    ...
    def test_message_async_handler(self):
        ...
        mock_server._trigger_event.assert_called_once_with('message', 'sid', 'foo',
          run_async=True)
    ...
    def test_invalid_packet(self):
        mock_server = self._get_mock_server()
        s = socket.Socket(mock_server, 'sid')
        self.assertRaises(exceptions.UnknownPacketError, s.receive, packet.Packet(packet.OPEN))
    ...
    def test_polling_read_error(self):
        ...
        environ = {'REQUEST_METHOD':'GET',  'QUERY_STRING':'sid=foo'}
        start_response = mock.MagicMock()
        self.assertRaises(exceptions.QueueEmpty, s.handle_get_request, environ, start_response)
    ...
    def test_polling_write(self):
        ...
        environ = {'REQUEST_METHOD':'POST',  'QUERY_STRING':'sid=foo',  'CONTENT_LENGTH':len(p), 
    ...
    def test_polling_write_too_large(self):
        ...
        environ = {'REQUEST_METHOD':'POST',  'QUERY_STRING':'sid=foo',  'CONTENT_LENGTH':len(p), 
         'wsgi.input':six.BytesIO(p)}
        self.assertRaises(exceptions.ContentTooLongError, s.handle_post_request, environ)

    def test_upgrade_handshake(self):
        mock_server = self._get_mock_server()
        s = socket.Socket(mock_server, 'foo')
        s._upgrade_websocket = mock.MagicMock()
        environ = {'REQUEST_METHOD':'GET',  'QUERY_STRING':'sid=foo',  'HTTP_CONNECTION':'Foo,Upgrade,Bar', 
    ...
    def test_upgrade(self):
        ...
        mock_server._async['websocket'].assert_called_once_with(s._websocket_handler)
    ...
    def test_upgrade_twice(self):
        ...
        environ = 'foo'
        start_response = 'bar'
        self.assertRaises(IOError, s._upgrade_websocket, environ, start_response)
    ...
    def test_upgrade_no_probe(self):
        ...
        s.connected = True
        ws = mock.MagicMock()
        ws.wait.return_value = packet.Packet(packet.NOOP).encode(always_bytes=False)
    ...
    def test_upgrade_no_upgrade_packet(self):
        ...
        s._websocket_handler(ws)
        ws.send.assert_called_once_with(packet.Packet((packet.PONG),
          data=probe).encode(always_bytes=False))
    ...
    def test_send_after_close(self):
        ...
        s = socket.Socket(mock_server, 'sid')
        s.close(wait=False)
        self.assertRaises(exceptions.SocketIsClosedError, s.send, packet.Packet(packet.NOOP))
    ...
    def test_close_after_close(self):
        ...
        mock_server._trigger_event.assert_called_once_with('disconnect', 'sid', run_async=False)
        s.close()
        self.assertEqual(mock_server._trigger_event.call_count, 1)
