...
# [GCC 8.4.0]
# Embedded file name: /home/conrad/prog/galileo-engineio/venv/lib/python3.6/site-packages/engineio/client.py
...
import signal, threading, time, six
...
class Client(object):
    ...
    def connect(self, url, headers={}, transports=None, engineio_path='engine.io', certfile=None):
        ...
        return getattr(self, '_connect_' + self.transports[0])(url, headers, engineio_path, certfile)
    ...
    def send(self, data, binary=None):
        ...
        """
        self._send_packet(packet.Packet((packet.MESSAGE), data=data, binary=binary))
    ...
    def _connect_polling(self, url, headers, engineio_path, certfile=None):
        """Establish a long-polling connection to the Engine.IO server."""
        if requests is None:
            self.logger.error('requests package is not installed -- cannot send HTTP requests!')
            return
        else:
            ...
            r = self._send_request('GET',
              (self.base_url + self._get_url_timestamp()), headers=headers, certfile=certfile)
            if r is None:
                self._reset()
                raise exceptions.ConnectionError('Connection refused by the server')
            if r.status_code != 200:
                raise exceptions.ConnectionError('Unexpected status code {} in server response'.format(r.status_code))
            try:
                p = payload.Payload(encoded_payload=(r.content))
            except ValueError:
                six.raise_from(exceptions.ConnectionError('Unexpected response from server'), None)
            ...
            if 'websocket' in self.upgrades:
                if 'websocket' in self.transports:
                    if self._connect_websocket(url, headers, engineio_path, certfile):
                        return
    ...
    def _connect_websocket(self, url, headers, engineio_path, certfile=None):
        """Establish or upgrade to a WebSocket connection with the server."""
        if websocket is None:
            self.logger.warning('websocket-client package not installed, only polling transport is available')
            return False
        else:
            ...
            sslopt = {'ca_certs': certfile} if certfile else None
            try:
                ws = websocket.create_connection((websocket_url + self._get_url_timestamp()),
    ...
    def _receive_packet(self, pkt):
        ...
        self.logger.info('Received packet %s data %s', packet_name, pkt.data if not isinstance(pkt.data, bytes) else '<binary>')
        if pkt.packet_type == packet.MESSAGE:
            ...
            if pkt.packet_type == packet.PONG:
                ...
                if pkt.packet_type == packet.NOOP:
                    pass
                else:
    ...
    def _send_request(self, method, url, headers=None, body=None, certfile=None):
        if self.http is None:
            self.http = requests.Session()
            if certfile:
                self.http.verify = certfile
        try:
    ...
    def _get_engineio_url(self, url, engineio_path, transport):
        ...
        if parsed_url.scheme in ('https', 'wss'):
            scheme += 's'
        return '{scheme}://{netloc}/{path}/?{query}{sep}transport={transport}&EIO=3'.format(scheme=scheme,
    ...
    def _read_loop_polling(self):
        ...
        while self.state == 'connected':
            ...
            r = self._send_request('GET', self.base_url + self._get_url_timestamp())
            if r is None:
                self.logger.warning('Connection refused by the server, aborting')
                self.queue.put(None)
                break
            if r.status_code != 200:
                self.logger.warning('Unexpected status code %s in server response, aborting', r.status_code)
                self.queue.put(None)
                break
            try:
                p = payload.Payload(encoded_payload=(r.content))
    ...
    def _write_loop(self):
        ...
        while self.state == 'connected':
            ...
            if self.current_transport == 'polling':
                ...
                r = self._send_request('POST',
                  (self.base_url), body=(p.encode()), headers={'Content-Type': 'application/octet-stream'})
                for pkt in packets:
                    self.queue.task_done()
