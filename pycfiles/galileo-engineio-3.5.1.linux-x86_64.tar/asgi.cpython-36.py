...
class ASGIApp:
    ...
    async def __call__(self, scope, receive, send):
        if scope['type'] in ('http', 'websocket'):
            if scope['path'].startswith('/{0}/'.format(self.engineio_path)):
    ...
    async def serve_static_file(self, scope, receive, send):
        ...
        if event['type'] == 'http.request':
            if scope['path'] in self.static_files:
                content_type = self.static_files[scope['path']]['content_type'].encode('utf-8')
...
async def translate_request(scope, receive, send):
    ...
    environ = {'wsgi.input':AwaitablePayload(payload), 
...
async def make_response(status, headers, payload, environ):
    ...
    await environ['asgi.send']({'type':'http.response.start',  'status':int(status.split(' ')[0]), 
     'headers':headers})
...
class WebSocket(object):
    ...
    async def send(self, message):
        ...
        if isinstance(message, bytes):
            ...
            msg_text = message
        await self.asgi_send({'type':'websocket.send',  'bytes':msg_bytes, 
         'text':msg_text})
