...
# [GCC 8.4.0]
# Embedded file name: /home/conrad/prog/galileo-engineio/venv/lib/python3.6/site-packages/tests/asyncio/test_asyncio_socket.py
# Compiled at: 2019-08-30 19:22:58
# Size of source mod 2**32: 17900 bytes
...
class TestSocket(unittest.TestCase):
    ...
    def _get_mock_server(self):
        ...
        mock_server._async = {'asyncio':True,  'create_route':mock.MagicMock(), 
    ...
    def test_message_async_handler(self):
        ...
        _run(s.receive(packet.Packet((packet.MESSAGE), data='foo')))
        mock_server._trigger_event.mock.assert_called_once_with('message',
          'sid', 'foo', run_async=True)

    def test_invalid_packet(self):
        mock_server = self._get_mock_server()
        s = asyncio_socket.AsyncSocket(mock_server, 'sid')
        self.assertRaises(exceptions.UnknownPacketError, _run, s.receive(packet.Packet(packet.OPEN)))
    ...
    def test_polling_read_error(self):
        ...
        s = asyncio_socket.AsyncSocket(mock_server, 'foo')
        environ = {'REQUEST_METHOD':'GET',  'QUERY_STRING':'sid=foo'}
        self.assertRaises(exceptions.QueueEmpty, _run, s.handle_get_request(environ))
    ...
    def test_polling_write_too_large(self):
        ...
        environ = {'REQUEST_METHOD':'POST',  'QUERY_STRING':'sid=foo',  'CONTENT_LENGTH':len(p), 
         'wsgi.input':self._get_read_mock_coro(p)}
        self.assertRaises(exceptions.ContentTooLongError, _run, s.handle_post_request(environ))
    ...
    def test_upgrade_no_probe(self):
        ...
        ws = mock.MagicMock()
        ws.wait = AsyncMock()
        ws.wait.mock.return_value = packet.Packet(packet.NOOP).encode(always_bytes=False)
    ...
    def test_upgrade_no_upgrade_packet(self):
        ...
        _run(s._websocket_handler(ws))
        ws.send.mock.assert_called_once_with(packet.Packet((packet.PONG),
    ...
    def test_send_after_close(self):
        ...
        self.assertRaises(exceptions.SocketIsClosedError, _run, s.send(packet.Packet(packet.NOOP)))
    ...
    def test_close_after_close(self):
        ...
        mock_server._trigger_event.mock.assert_called_once_with('disconnect', 'sid')
        _run(s.close())
        self.assertEqual(mock_server._trigger_event.mock.call_count, 1)
