...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/scripts/populate_mistakes_database.py
# Compiled at: 2016-02-25 08:59:06
...
def store(name, res, wiki, p_number, value):
    ...
    if wiki not in item.sitelinks:
        return False
    insert_statement = 'INSERT INTO kian_mistakes (qid, property, value, wiki_name, name_wiki, status, prob, wd_value) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)'
    db = MySQLdb.connect(host='tools-db', db='s52709__kian_p', read_default_file='~/replica.my.cnf', charset='utf-8')
    cursor = db.cursor()
    ...
    with model.wiki_data_file as (f):
        for line in f:
            ...
            if name and name != line.split('\t')[0]:
                ...
                if name in model.wikidata_data_w and res < 1 - second_thrashhold:
                    store(name, res, model.wiki, p_number, q_number)
                elif name in model.wikidata_data_wo and res > second_thrashhold:
                    store(name, res, model.wiki, p_number, q_number)
                a = []
            name = line.split('\t')[0]
