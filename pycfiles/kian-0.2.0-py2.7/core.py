...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/kian/core.py
# Compiled at: 2016-03-18 19:30:41
import math, random, codecs, os
try:
    basestring
except:
    basestring = str
...
class Kian(object):
    ...
    def __init__(self, model=None, lambda_para=0.0005, no_iter=250, alpha=0.1, epi=0.2, training_set=None, slow=False, skewed=False, balance_with_replacement=False):
        ...
        if skewed:
            if balance_with_replacement:
                training_set_temp = self.balance_training_set_replacement(training_set_temp)
            else:
    ...
    def balance_training_set_replacement(tr_set):
        ...
        for out in tr_set2:
            if no_outputs[out] < highest_no:
                training_set += [ random.sample(tr_set2[out], 1)[0] for i in range(highest_no)
                                ]
            else:
                training_set += tr_set2[out]
    ...
    def cost_function(theta, training_set, reg=True, lambda_para=1):
        ...
        for case in training_set:
            ...
            for i in range(len(y)):
                sum_cos -= case[(-1)][i] * math.log(y[i]) + (1 - case[(-1)][i]) * math.log(1 - y[i])
            ...
            for i in range(2):
                for j in range(len(theta[i])):
                    for k in range(len(theta[i][j])):
                        if not k or not j:
                            sum_cos += lambda_para * theta[i][j][k] * theta[i][j][k] / (2 * m)
                ...
                for ii in range(2):
                    for j in range(len(Delta[ii])):
                        for k in range(len(Delta[ii][j])):
                            ...
                            if not j or not k:
                                D[ii][j][k] += self.lambda_para * theta[ii][j][k]
