...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/error_explainer/check_runner.py
# Compiled at: 2020-05-06 06:30:59
...
from error_explainer.checks import check_quote_error, check_invalid_function_def, check_invalid_function_name, check_missing_function_def_parts, check_missing_brackets, check_mismatched_bracket_type, check_print_missing_brackets, check_missing_colon, check_invalid_assignment_expr, BracketErrorType, check_docstring_quote_error, check_invalid_indentation, check_coma_used_instead_of_period
from error_explainer.messages import get_formatted_message
from error_explainer.utils import get_root_node, find_error_nodes, get_lines, tokenize_line, read_file, get_line_location_start, get_location_on_line, get_line_location_end
...
def docstring_error_check(filename: str) -> NoReturn:
    ...
    if result is not None:
        add_message('missing_docstring_quotes',
...
def quote_errors_check(filename: str) -> NoReturn:
    ...
    if results is not None:
        if len(results) == 2:
            if any(leaf for leaf in results if leaf.get_code().strip() == "'"):
                if any(leaf for leaf in results if leaf.get_code().strip() == '"'):
                    add_message('mismatched_quotes',
                      line_start=(get_line_location_start(results[0])),
                      line_end=(get_line_location_end(results[1])))
        else:
            for res in results:
                if len(res.get_code().strip()) == 3:
                    add_message('invalid_quotes_triple',
...
def indentation_errors_check(filename: str) -> NoReturn:
    ...
    if indent_check_result is not None:
        ...
        add_message(f"invalid_indentation.{indent_error_type.value}",
            ...
            if tokens[0].string == 'def':
                ...
                if invalid_function_name_res == '=':
                    ...
                    if invalid_function_name_res is not None:
                        if invalid_function_name_res == '(':
                            print(tokens)
                            add_message('invalid_function_bracket',
                              line_end=(get_line_location_end(error)))
                        else:
...
def missing_brackets_check(filename: str) -> NoReturn:
    ...
    for error in found_errors:
        ...
        if normal_brackets + square_brackets + curly_brackets != 0 and check_mismatched_bracket_type(filename) is None:
            ...
            if start == end:
                ...
                if normal_brackets < 0:
                    add_message('missing_brackets.normal.opening.m',
                      count=(abs(normal_brackets)),
                      line_start=(get_line_location_start(error)),
                      line_end=(get_line_location_end(error)))
                else:
                    if normal_brackets > 0:
                        add_message('missing_brackets.normal.closing.m',
                          count=(abs(normal_brackets)),
                          line_start=(get_line_location_start(error)),
                          line_end=(get_line_location_end(error)))
                    else:
                        if square_brackets < 0:
                            add_message('missing_brackets.square.opening.m',
                              count=(abs(square_brackets)),
                              line_start=(get_line_location_start(error)),
                              line_end=(get_line_location_end(error)))
                        else:
                            if square_brackets > 0:
                                add_message('missing_brackets.square.closing.m',
                                  count=(abs(square_brackets)),
                                  line_start=(get_line_location_start(error)),
                                  line_end=(get_line_location_end(error)))
                            else:
                                if curly_brackets < 0:
                                    add_message('missing_brackets.curly.opening.m',
                                      count=(abs(curly_brackets)),
                                      line_start=(get_line_location_start(error)),
                                      line_end=(get_line_location_end(error)))
                                elif curly_brackets > 0:
                                    add_message('missing_brackets.curly.closing.m',
                                      count=(abs(curly_brackets)),
                                      line_start=(get_line_location_start(error)),
                                      line_end=(get_line_location_end(error)))
    ...
    for error in found_errors:
        ...
        if normal_brackets + square_brackets + curly_brackets != 0:
            ...
            if mismatched_bracket_type_res == BracketErrorType.NORMAL_SQUARE:
                add_message('mismatched_brackets.square.normal',
                  line_start=(get_line_location_start(error)),
                  line_end=(get_line_location_end(error)))
            else:
                if mismatched_bracket_type_res == BracketErrorType.NORMAL_CURLY:
                    add_message('mismatched_brackets.curly.normal',
                      line_start=(get_line_location_start(error)),
                      line_end=(get_line_location_end(error)))
                elif mismatched_bracket_type_res == BracketErrorType.CURLY_SQUARE:
                    add_message('mismatched_brackets.curly.square',
                      line_start=(get_line_location_start(error)),
                      line_end=(get_line_location_end(error)))
    ...
    for error in found_errors:
        ...
        if res is not None:
            add_message('missing_colon',
              line_end=(get_line_location_end(error)), statement=res)
...
def coma_instead_of_period_check(filename: str) -> NoReturn:
    ...
    if coma_check_res is not None:
        for res in coma_check_res:
            add_message('coma_instead_of_period',
              line_start=(get_line_location_start(res)))