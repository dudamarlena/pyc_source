...
# [GCC 8.4.0]
# Embedded file name: build/bdist.darwin-8.8.0-Power_Macintosh/egg/dap/responses/html/templess.py
# Compiled at: 2006-11-07 12:03:07
"""
...
def is_iterable_not_string(v):
    ...
    if type(v) in [str, unicode, xmlstring, objectcontext] or isinstance(v, node):
        return False
    try:
        iter(v)
    except TypeError:
...
class elnode(list, node):
    ...
    def _start_node_start(self, attrs):
        return '<%s%s' % (self.name, self._serialize_attrs(attrs))
    ...
    def _serialize_attrs(self, attrs):
        ...
        return ' %s' % (' ').join([ '%s="%s"' % (k, self._strconvert(v)) for (k, v) in items ])
    ...
    def convert(self, context, parent=None):
        ...
        if cvalue is None:
            ...
            for ccontext in cvalue:
                if isinstance(ccontext, dict) or isinstance(ccontext, objectcontext):
                    newparent = parent
                    if not replacekey:
                        newparent = elnode(self.name, attrs, parent, self.charset)
                    for child in self:
                    ...
                    if isinstance(ccontext, node):
                        ...
                        textnode(self._strconvert(ccontext), newparent, self.charset)
                    ret.append(newparent)
...
class treebuilderhandler(nanosax.nshandler):
    ...
    def startel(self, name, attrs):
        ...
        self.current = templessnode(name, attrs, self.current, self.templess_prefix, self.charset)
...
class cgitemplate:
    ...
    def __init__(self, template, content_type='text/html; charset=UTF-8', error_template='templates/cgierror.html', headers={}):
    ...
    def handle_exception(self, exc, e, tb):
        ...
        try:
            t = template(fp)
            context = {'exception': str(exc), 'message': str(e), 'traceback': ('\n').join(tblist), 'tblist': [ {'line': str(l)} for l in tblist ], 'footer': __footer__}
            ret = t.render_to_string(context)
        finally:
            fp.close()
