...
# [GCC 8.4.0]
# Embedded file name: build/bdist.darwin-8.8.0-Power_Macintosh/egg/dap/responses/html/nanosax.py
# Compiled at: 2006-11-07 12:03:10
"""
...
class nsparser(object):
    ...
    TYPE_COMMENT = 4
    TYPE_CDATA = 5
    _reg_name = re.compile('^[\\w\\:\\-]+$', re.U)
    _reg_start = re.compile('^([\\w\\:\\-]+)(\\s+(([\\w\\:\\-]+)=(("(?=([^"]*)")[^"]*")|(\'(?=([^\']*)\')[^\']*\'))))*$')
    _reg_attr = re.compile('([\\w\\:\\-]+)((\\="(?=([^">]*)"))|(\\=\\\'(?=([^\\\'>]*)\\\')))', re.U)
    _reg_xml_decl = re.compile('<\\?xml.*?>', re.S)
    _reg_encoding = re.compile('encoding="([^"]+)"')
    _reg_pi = re.compile('<\\?.*?>', re.S)
    _reg_dtd_1 = re.compile('<!DOCTYPE\\s+[\\w\\:\\-]+\\s+\\[.*?\\]>', re.S)
    _reg_dtd_2 = re.compile('<!DOCTYPE\\s+[\\w\\:\\-]+\\s+SYSTEM\\s+.*?>')
    ...
    def _parse_into_chunks(self, xml):
        ...
        while xml:
            ...
            if xml.startswith('<![CDATA['):
                ...
                data = xml[2:endpos]
                name = data.strip()
                self._test(self._reg_name.match(name), currline, "illegal element name '%s' in end tag" % (name,))
                startname = namestack.pop()
                self._test(name.strip() == startname.strip(), currline, "closing tag '%s' doesn't match openingtag '%s'" % (name, startname))
                offset += endpos + 1
                ...
                if data[(-1)] == '/':
                    ...
                    issingle = True
                name = data.split()[0]
                self._test(self._reg_name.match(name), currline, "illegal element name '%s' for tag" % (name,))
                offset += endpos + 1
                if not issingle:
...
class echohandler(nshandler):
    ...
    def startel(self, name, attrs):
        ...
        if len(attrs):
            self.buffer.append(' ')
            self.buffer += (' ').join(('%s="%s"' % (k, v) for (k, v) in attrs.iteritems()))
        self.buffer.append('>')
