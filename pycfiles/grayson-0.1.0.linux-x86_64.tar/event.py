...
# [GCC 8.4.0]
# Embedded file name: /home/scox/dev/grayson/venv/lib/python2.7/site-packages/grayson/debug/event.py
# Compiled at: 2012-06-29 15:40:49
...
class EventStream(object):
    ...
    def getEventContext(self, bufferSize=0):
        return EventContext(stream=self, bufferSize=bufferSize)
    ...
    def flush(self, event, context):
        ...
        context.eventBuffer.append(event)
        self.transmit({'clientId': event['clientId'], 
           'flowId': event['flowId'], 
           'type': 'composite', 
    ...
    def normalize(self, event):
        ...
        return GraysonUtil.relativize(object=event, keys=[
         'flowId', 'workdir', 'graph'], username=event['clientId'])
    ...
    def sendJobStatusEvent(self, username, flowId, jobid, status, logdir='', evt_time=None, aux={}, context=None):
        if not evt_time:
            evt_time = time.time()
        logger.debug('event-stream:send:job-status: user(%s) wfid(%s) jobid(%s) status(%s) logdir(%s) time(%s)', username, flowId, jobid, status, logdir, evt_time)
        self.publish({'clientId': username, 
           'flowId': flowId, 
           'type': 'jobstatus', 
           'job': jobid, 
           'time': evt_time, 
           'state': status, 
           'logdir': logdir}, aux, context)
    ...
    def sendEndEvent(self, username, flowId, aux={}, context=None):
        ...
        self.flush({'clientId': username, 
           'flowId': flowId, 
           'type': 'endEventStream', 
           'time': time.time()}, context)
