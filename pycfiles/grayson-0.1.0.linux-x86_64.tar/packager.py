...
# [GCC 8.4.0]
# Embedded file name: /home/scox/dev/grayson/venv/lib/python2.7/site-packages/grayson/packager.py
# Compiled at: 2012-03-02 14:59:52
""" system """
import logging, os, string, shutil, sys, time, zipfile, glob
...
class GraysonPackager:
    ...
    def verifyArchive(self, input_file):
        ...
        try:
            zip_file = zipfile.ZipFile(file=input_file, mode='r', compression=zipfile.ZIP_DEFLATED)
            try:
                zip_file.testzip()
    ...
    def unpackArchive(self, input_file, output_dir='.'):
        self.logger.info('** grayson:unpack-archive: (%s) to directory: (%s)', input_file, output_dir)
        try:
            zip_file = zipfile.ZipFile(file=input_file, mode='r', compression=zipfile.ZIP_DEFLATED)
            try:
                for info in zip_file.infolist():
...
def test_grayson_packager():
    ...
    GraysonPackager.package(patterns=[model_patterns], output_file=output_file, logLevel=log_level)
    GraysonPackager.verify(input_file=output_file, logLevel=log_level)
    unpack_output_dir = os.path.join(output_dir, 'unpack')
    GraysonPackager.unpack(input_file=output_file, output_dir=unpack_output_dir, logLevel=log_level)
