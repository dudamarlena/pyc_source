...
# [GCC 8.4.0]
# Embedded file name: /home/scox/dev/grayson/venv/lib/python2.7/site-packages/net/amqp.py
# Compiled at: 2012-05-07 09:56:36
import sys, time, traceback, logging, pika
from os.path import exists, normpath
from pika.adapters import SelectConnection
from pika.adapters import AsyncoreConnection
from pika.connection import ConnectionParameters
from pika import BasicProperties
import pika.log, socket
...
class GraysonAMQP(object):
    ...
    def connect(self, hostname='127.0.0.1'):
        self.channel = None
        logger.info('grayson:amqp:connect host: %s, port: %s', self.amqpSettings.hostname, self.amqpSettings.port)
        connectionParameters = ConnectionParameters(host=self.amqpSettings.hostname, virtual_host=self.amqpSettings.vhost, port=self.amqpSettings.port)
    ...
    def on_channel_open(self, channel_):
        ...
        try:
            self.channel.queue_declare(queue=self.amqpSettings.queue.name, passive=False, durable=False, exclusive=False, auto_delete=False, callback=self.on_queue_declared)
        except:
            traceback.print_exc()
...
class GraysonAMQPTransmitter(GraysonAMQP):
    ...
    def transmit(self, frame):
        while len(self.buffer) > 0:
            ...
            try:
                self.channel.basic_publish(exchange='', routing_key=self.amqpSettings.queue.name, body=message)
            except:
                traceback.print_exc()
...
class GraysonAMQPReceiver(GraysonAMQP):
    ...
    def do_receive(self, frame):
        self.channel.basic_consume(self.handle_delivery, queue=self.amqpSettings.queue.name)
...
__all__ = [
 'GraysonAMQPReceiver', 'GraysonAMQPTransmitter']