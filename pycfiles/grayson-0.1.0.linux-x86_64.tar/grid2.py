...
# [GCC 8.4.0]
# Embedded file name: /home/scox/dev/grayson/venv/lib/python2.7/site-packages/grayson/debug/grid2.py
# Compiled at: 2012-06-30 06:49:48
import logging, getopt, glob, json, os, re, sys, string, traceback, time
...
class EventDetailScanner(object):
    ...
    def detectEventDetails(self, event, logdir, status):
        ...
        if jobId.startswith('stage_in') or jobId.startswith('stage_out'):
            ...
            if lines:
                ...
                while len(lines) > pair * 4 + 4:
                    offset = pair * 4
                    transfer = {'sourceSite': lines[(offset + 0)], 
        ...
        if status == WorkflowStatus.STATUS_FAILED:
            execution = self.getExecutionData(logdir, jobId)
            if execution:
                aux['detail'] = {'stdout': GraysonUtil.ceilingString(execution['stdout'], maxLength=500, fromEnd=True), 'stderr': GraysonUtil.ceilingString(execution['stderr'], maxLength=500, fromEnd=True)}
...
class WorkflowStatus(object):
    ...
    STATUS_MAP = {SUBMIT: STATUS_PENDING, 
...
class WorkflowMonitor(Processor):
    ...
    def examineWorkflow(self):
        ...
        output = []
        executor = Executor({'condorHome': os.environ['CONDOR_HOME'], 
           'sched_id': sched_id})
        executor.execute(command="${condorHome}/bin/condor_q ${sched_id} -format '%s' JobStatus", pipe=True, processor=lambda n: output.append(n))
    ...
    def runCycle(self):
        ...
        self.cycleEventsA = 0
        self.cycleEventsT = 0
        self.status, self.maxTimestamp = self.scanner.getEvents(processor=self, since=self.maxTimestamp, daxen=self.daxen)
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug('monitor cycle: flow[%s] maxTimestamp[in->%s, out->%s] events[cycle-accepted->%s, cycle-all->%s, total->%s] daxen:%s wf[%s]', self.workflowId, maxTimestampIn, self.maxTimestamp, self.cycleEventsA, self.cycleEventsT, self.totalEvents, self.daxen, self.workflowId)
        return self.status
    ...
    def sendTranslatedEvent(self, event, status, logdir):
        self.eventStream.sendJobStatusEvent(username=self.username, flowId=self.workflowId, jobid=event.name, status=self.translateExitCode(event), logdir=logdir, evt_time=event.timestamp, aux=self.detailScanner.detectEventDetails(event, logdir, status))

    def translateExitCode(self, event):
        if event.state in WorkflowStatus.STATUS_MAP:
            return WorkflowStatus.STATUS_MAP[event.state]
        else:
            return
...
class WorkflowMonitorTask(Processor):
    ...
    def __init__(self, workflowRoot, amqpSettings=None, eventBufferSize=10):
        ...
        self.eventStream = EventStream(amqpSettings=amqpSettings, workflowRoot=workflowRoot, eventBufferSize=eventBufferSize)
        self.monitors = {}
...
if __name__ == '__main__':
    ...
    workdir = sys.argv[1]
    monitor = WorkflowMonitorTask(workdir)
    monitor.execute()