...
# [GCC 8.4.0]
# Embedded file name: /home/scox/dev/grayson/venv/lib/python2.7/site-packages/grayson/jsonparser.py
# Compiled at: 2012-04-20 17:47:21
import os, sys, xml.sax, urllib2, logging, traceback, re, json
...
class Graph:
    ...
    def getNodeByLabel(self, label):
        if label in self.nodeLabelMap:
            return self.nodeLabelMap[label]
        else:
            return
        ...
        for edge in self.edges:
            ...
            try:
                ...
                if not source:
                    ...
                    logging.debug('--edge(id=%s)(src=(id=%s):%s, dst=(id=%s):%s), type(%s)', edge.getId(), source.getId(), source.getLabel(), target.getId(), target.getLabel(), edge.getType())
...
class JSONHandler(object):
    ...
    def parse(self, stream):
        ...
        for key, value in syntax.items():
            self.parseElement(elementKey=key, model=model, allowListVariant=value)
        ...
        for node in nodes:
            text = unicode(json.dumps(node.getType(), sort_keys=True, indent=2, separators=(',',
                                                                                            ':')))
            print text
            node.setType(text)
        ...
        if elementKey in model:
            ...
            if allowListVariant:
                if isinstance(element, basestring):
                    ...
                    node = self.getNode(element, {'type': elementKey})
                elif isinstance(element, list):
                    for key in element:
                        logging.info('--element(%s) %s', elementKey, key)
                        node = self.getNode(key, {'type': elementKey})
