...
# [GCC 8.4.0]
# Embedded file name: /home/scox/dev/grayson/venv/lib/python2.7/site-packages/myproxy/utils/openssl.py
...
class OpenSSLConfig(SafeConfigParser, object):
    ...
    filePath = property(fget=_getFilePath, fset=_setFilePath, doc='file path for configuration file')
    ...
    caDir = property(fget=_getCADir, fset=setCADir, doc='directory path for CA configuration files')
    ...
    def _setReqDN(self, reqDN):
        ...
        invalidKw = [ k for k in dict if k not in OpenSSLConfig._certReqDNParamName
                    ]
        if invalidKw:
    ...
    reqDN = property(fget=_getReqDN, fset=_setReqDN, doc='Distinguished Name for certificate request')
    ...
    def read(self):
        ...
        for section, val in self._sections.items():
            newSection = section
            self._sections[newSection.strip()] = dict([ (opt, self._filtOptVal(optVal)) for opt, optVal in val.items()
                                                      ])
    ...
    def _setReqDN(self):
        ...
        try:
            self._reqDN = {'O': self.get('req_distinguished_name', '0.organizationName_default'), 
               'OU': self.get('req_distinguished_name', '0.organizationalUnitName_default')}
        except Exception as e:
            raise OpenSSLConfigError('Setting content of Distinguished Name from file "%s": %s' % (
             self._filePath, e))