...
# [GCC 8.4.0]
# Embedded file name: /home/scox/dev/grayson/venv/lib/python2.7/site-packages/grayson/compiler/compiler.py
# Compiled at: 2012-08-08 18:16:24
""" system """
import getopt, glob, json, logging, os, re, shlex, shutil, socket, string, subprocess, sys, time, traceback
...
class Operator(object):
    ...
    def getDynamicIndex(properties):
        if Operator.DYNAMIC_INDEX in properties:
            return properties[Operator.DYNAMIC_INDEX]
        else:
            return
...
class GraysonCompiler:
    ...
    def __init__(self, namespace, version, logManager, outputDir, clean=False, appHome=None):
        ...
        if path:
            ...
            self.compilerContext.modelPath = [
             '.', 'model']
    ...
    def parse(self, models=[], graph=None):
        ...
        if len(final_models) > 0:
            ...
            self.graph = parser.parseMultiple(final_models, path=self.compilerContext.modelPath, graph=graph)
        else:
            message = 'Empty model list. Supply at least one model file.'
            logger.error(message)
            raise ValueError(message)
        for path in self.graph.fileNames:
            if path not in self.compilerContext.allModelPaths:
    ...
    def addElement(self, id, element):
        ...
        if elementType:
            if elementType not in self.typeIndex:
            ...
            logger.debug('ast:add-elem:type-index: name=%s type=%s cid=(%s) type=(%s)', element.getLabel(), element.getType(), self.compilerId, elementType)
        ...
        if removeEdges:
            ...
            for edge in edges:
                if edge.getTarget() == id or edge.getSource() == id:
                    ...
                    logger.debug('      removing edge: %s src:%s target:%s', id, edge.getSource(), edge.getTarget())
    ...
    def getReferenceByLabel(self, label):
        if label in self.referenceIndex:
            return self.referenceIndex[label]
        else:
            return
    ...
    def dereference(self, original, regex=False):
        ...
        if original:
            ...
            if not type == self.ATTR_REFERENCE:
                ...
                if value:
                    logger.debug('ast:dereference: (%s) to object (%s) of type (%s)', original.getLabel(), value.getLabel(), value.getType())
                else:
    ...
    def validateGraph(self):
        ...
        nodeKeys = list(set(map(lambda x: x.getId(), self.graph.getNodes())))
        edgeKeys = list(set(map(lambda edge: (edge.getSource(),
         edge.getTarget()), self.graph.getEdges())))
        edgeFilter = []
        for edge in edgeKeys:
            ...
            if source and target:
                tag = 'allowCycle'
                if tag not in source.getContext() and tag not in target.getContext():
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            for k in edgeKeys:
                ...
                if source and target:
                    logger.debug(' edges: id(%8s->%10s) src(%8s: [%10s]) trg(%8s: [%10s])', k[0], k[1], source.getId(), source.getLabel(), target.getId(), target.getLabel())

        cycleDetector = CycleDetector(nodes=nodeKeys, edges=edgeKeys)
        cycle = cycleDetector.detect_cycle()
        if cycle:
            buffer = [
             'Workflow models may not contain cycles.',
    ...
    def addExecutable(self, jobContext, source):
        ...
        if self.getWorkflowManagementSystem().isShellExecutionEnabled():
            installed = 'true'
        exe = self.workflowModel.addExecutable(jobId=job.getId(), name=jobLabel, path=path, version=self.version, site=site, exe_arch=architecture, installed=installed)
        self.getTransformationCatalog().addEntry(jobName=jobLabel, location=path, transfer='INSTALLED' if installed == 'true' else None, cluster=site, namespace=self.namespace, version=self.version)
    ...
    def addOutputFile(self, jobContext, fileElement, edge):
        ...
        fileElement.setDaxNode(file)
        jobContext.outFiles[fileElement.getLabel()] = (
         fileElement, arg)
    ...
    def addInputFile(self, jobContext, fileElement, edge):
        ...
        if not site:
            site = self.ATTR_LOCAL
        logger.debug('ast:add-input: arg=%s file=%s job=%s site=%s', arg, fileName, job.getNode().getLabel(), site)
        fileURL = (self.getFileURL(fileElement, site),)
        ...
        self.getReplicaCatalog().addEntry(fileName, self.getFileURL(fileElement, site), site)
    ...
    def createComponentCompiler(self, elementName, version):
        ...
        compiler = GraysonCompiler(namespace=elementName, version=version, logManager=self.logManager, clean=self.clean, appHome=self.appHome, outputDir=self.getOutputDir())
    ...
    def executeCompiler(self, compiler, elementName, outputFile):
        ...
        if not self.isPackaging():
            ...
            self.getCompilerPlugin().notifyWorkflowCreation(self.getLogicalName(elementName), outputFileName, isTopWorkflow)
    ...
    def ast_compileWorkflow(self, element):
        ...
        if context:
            ...
            for node in nodes:
                text = node.getType()
                logger.debug('ast:compile-wf:node: %s context: %s text: %s', element.getLabel(), context, text)
                if text:
    ...
    def ast_synthesizeInstance(self, graph, is_chained, last_instance, operator, operand, context, origins=[]):
        ...
        if type == self.MAP:
            ...
            for targetId in targets:
                ...
                if target:
                    node = self.ast_executeMapOperator(parent_graph=graph, operator=operand, element=target, input_context=context, originEdges=origins)
    ...
    def ast_executeMapOperator(self, parent_graph, operator, element, input_context={}, originEdges=[]):
        ...
        logger.debug('ast:map:each: %s type: %s variable: %s chained: %s', each, type(each), variable, is_chained)
        ...
        for edge in inboundEdges:
            ...
            if edgeElement:
                ...
                if edgeElement.getType() == self.INPUT:
                    logger.debug('ast:map:origin-edge: inheriting edge (%s) from (%s, %s) to (%s, %s)', edgeElement.getId(), source.getId(), source.getLabel(), target.getId(), target.getLabel())
        ...
        if type(each) == dict:
            if MAP__START in each and MAP__END in each:
                ...
                for c in range(start, end):
                    ...
                    context[variable] = c
                    last_instance = self.ast_synthesizeInstance(synthetic_graph, is_chained, last_instance, operator, element, context, originEdges)
            ...
            if string.find(each, FILE_PROTO) == 0:
                ...
                configuration = {'outputName': element.getLabel(), 
                ...
                jobType = {'type': 'job', 
                   ...
                   'profiles': {'env': {'PYTHONPATH': os.environ['PYTHONPATH'], 
                ...
                job = self.graft(old=operator, new=self.ast_addNode(id=operator.getId(), label=operator.getLabel(), typeObj=jobType))
                for edge in origins:
                    source = self.getElement(edge.getSource())
                    output = self.ast_addNode(id='%s_synth' % source.getId(), label=source.getLabel(), typeObj={'type': 'reference'})
                ...
                exe = self.synthesizeExecutable(targetJob=job, label='graysonc', path=self.getProperty(self.MAP)['graysonHome'])
                ...
                dax = self.graft(old=element, new=self.ast_addNode(id=element.getId(), label=element.getLabel(), typeObj=typeObj))
        ...
        if not is_dynamic:
            ...
            syntheticNode = self.ast_bindSyntheticNode(parent_graph, operator, element, syntheticId, syntheticLabel, syntheticType, originEdges)
    ...
    def copyEdge(self, graph, edge, source, target, edgeType=None):
        ...
        logger.debug('qqq ast:copy-edge: from (%s(%s)->%s(%s))type(%s)', source.getLabel(), source.getId(), target.getLabel(), target.getId(), edgeType)
        return edgeCopy
    ...
    def addEdge(self, graph, edgeId, sourceId, targetId, type=''):
        ...
        logger.debug('ast:add-edge: id(%s) srcid(%s) targid(%s) type(%s)', edgeId, sourceId, targetId, type)
        return edge
    ...
    def findOrCopy(self, element, graph):
        ...
        if not end_element:
            end_element = graph.addNode('%s_synth' % element.getId(), element.getLabel(), element.getNode().getType())
        return end_element
                ...
                if isRoot:
                    ...
                    for targetId in targets:
                        ...
                        if target:
                            ...
                            targetEdges = []
                            node = self.ast_executeMapOperator(parent_graph=self.graph, operator=operator, element=target, input_context={}, originEdges=[])
                            if not is_dynamic:
                                ...
                                logger.debug('map:operator: added synthetic dax node: %s (%s)', node.getLabel(), node.getType())
        ...
        for job in jobs:
            ...
            for pointcut in self.compilerContext.aspects:
                ...
                if matchesBefore:
                    ...
                    edge = self.addEdge(graph=self.graph, edgeId='%s_asp_%s' % (aspectInstance.getId(), job.getId()), sourceId=aspectInstance.getId(), targetId=job.getId())
                if matchesAfter:
                    logger.debug('weave-job-advice(before): job=%s id=%s type=%s pointcut=%s', job.getLabel(), job.getId(), job.getType(), pointcut.getLabel())
                    aspectInstance = self.ast_weaveAspectCompileAspect(pointcut, aspectName, job)
                    edge = self.addEdge(graph=self.graph, edgeId='%s_asp_%s' % (aspectInstance.getId(), job.getId()), sourceId=job.getId(), targetId=aspectInstance.getId())
    ...
    def ast_weaveAspects(self):
        ...
        for edge in edges:
            ...
            for pointcut in self.compilerContext.aspects:
                ...
                logger.debug('weave:egg:match---- pat:%s file:%s jobpat:%s job:%s %s', pattern, fileElement.getLabel(), jobPattern, jobNode.getLabel(), pointcut.getId())
                ...
                if matchesJob and matchesFile:
                    logger.debug('[aspect-apply]: %s job:%s file:%s from: %s, patt: %s', activationKey, jobNode.getLabel(), fileElement.getLabel(), fromElement, pattern)
                    aspectActivations[activationKey] = pointcut
                    if toElement:
                        self.ast_weaveAspectFromFileToJob(pointcut=pointcut, edge=edge, aspectName=aspectName, jobNode=jobNode, fileElement=fileElement, variable=variable)
                    elif fromElement:
                        self.ast_weaveAspectFromJobToFile(pointcut=pointcut, edge=edge, aspectName=aspectName, jobNode=jobNode, fileElement=fileElement, variable=variable)
    ...
    def ast_weaveAspectCompileAspect(self, pointcut, aspectName, jobNode, variableValue=None, variable=None):
        ...
        if not variable:
            variable = ''
            variableValue = ''
        aspectElement = self.ast_addNode(id='%s.%s' % (aspectName, synthId), label='%s.%s' % (aspectName, number), typeObj=typeObj, context={variable: variableValue})
        aspectElement.getContext()['allowCycle'] = True
        self.ast_compileWorkflow(aspectElement)
        logger.debug('--weaving: generated synthetic aspect node name=(%s) id=(%s) from=(%s) cid=(%s)', aspectElement.getLabel(), aspectElement.getId(), variableValue, self.compilerId)
    ...
    def ast_weaveAspectCreateSynthFile(self, fileElement):
        ...
        return self.ast_addNode(id=synthFileId, label=fileElement.getLabel(), typeObj={'type': 'reference'})
    ...
    def setInboundEdgesTarget(self, old, new, skip=[]):
        ...
        for edge in edges:
            ...
            targetId = edge.getTarget()
            target = self.getElement(targetId)
            if target and source and targetId not in skip:
    ...
    def setOutboundEdgesSource(self, old, new, skip=[]):
        ...
        for edge in edges:
            ...
            source = self.getElement(sourceId)
            target = self.getElement(edge.getTarget())
            if sourceId not in skip and target and not target.getId() == new.getId():
    ...
    def synthesizeExecutable(self, targetJob, label=None, path=None):
        executableElement = None
        executableRef = self.ast_addNode(id='%s_exe_ref' % targetJob.getId(), label='%s.sh' % targetJob.getLabel(), typeObj='{ "type" : "reference" }')
        edge = self.addEdge(graph=self.graph, edgeId='%s_exe_edge' % targetJob.getId(), sourceId=executableRef.getId(), targetId=targetJob.getId())
        logger.debug('ast:synthesize-reference-to-executable: id(%s) label(%s) type(%s)', executableRef.getId(), executableRef.getLabel(), executableRef.getType())
        ...
        if not concrete:
            ...
            executableType = {'type': 'executable', 'path': path}
            executableElement = self.ast_addNode(id='%s_exe' % targetJob.getId(), label='%s.sh' % targetJob.getLabel(), typeObj=json.dumps(executableType))
            logger.debug('ast:synthesize-executable: id(%s) label(%s) type(%s)', executableElement.getId(), executableElement.getLabel(), executableElement.getType())
        return executableElement
    ...
    def ast_mapNodes(self):
        ...
        if self.isTopModel():
            ...
            for contextModel in contextModelElements:
                ...
                if fileName not in self.getTopModel():
        ...
        if self.isContextModel():
            ...
            for aspect in aspectElements:
                ...
                if pointcuts:
                    for key in pointcuts:
                        ...
                        pointcutElement = self.ast_addNode(id='%s.%s' % (aspect.getId(), key), label=key, typeObj=pointcut)
                        pointcutElement.set(self.ASPECT, aspect.getLabel())
            ...
            for elementType in [self.JOB, self.DAX]:
                ...
                for element in elements:
                    ...
                    if element.getLabel() not in seenJobs:
    ...
    def ast_addPointcut(self, pointcutElement):
        ...
        if pointcutKey not in self.compilerContext.seenPointcuts:
    ...
    def ast_buildSubWorkflows(self, jobs):
        ...
        for element in elements:
            if element.getLabel() not in self.compilerContext.processedWorkflows:
                ...
                if not element.get('compile') == 'no':
                    ...
                    self.getReplicaCatalog().addEntry(fileName, 'file://%s/%s' % (self.getOutputDir(), fileName), 'local')
    ...
    def ast_addAncestor(self, target, ancestor, tab='', recurse=True):
        logger.debug('%s-ast:inherit:target: parent=(id=%s,label=%s) target=(id=%s,label=%s)', tab, ancestor.getId(), ancestor.getLabel(), target.getId(), target.getLabel())
        target.addAncestor(ancestor)
        logger.debug('object %s after inheriting %s\n%s', target.getLabel(), ancestor.getLabel(), json.dumps(target.getProperties(), indent=3, sort_keys=True))
        ...
        for job in jobs:
            ...
            for edge in edgeElements:
                ...
                if source and source.getId() == jobId:
                    ...
                    logger.debug('add-job-edge: source(%s,%s)-edge(%s)->target(%s,%s)', source.getId(), source.getLabel(), edge.getId(), job.getId(), job.getLabel())
    ...
    def getExecuteArguments(self, workflow=None, other=[]):
        ...
        return self.getWorkflowManagementSystem().getExecuteArguments(self.getSites(), workflow, other)
    ...
    def addSite(self, clusterId, site, entry):
        ...
        if clusterId not in self.getSites():
    ...
    def mapModelRelationships(self):
        ...
        seen = []
        for element in elements:
            if element.getLabel() not in seen:
    ...
    def package(self, additionalFiles):
        ...
        conf = {self.CONF_INPUT_PROPERTIES: self.getInputModelProperties(), 
        ...
        GraysonPackager.package(patterns=contextualizedMap, output_file=outputArchive, relativeTo=os.getcwd())
    ...
    def unpack(self, archive, outputdir):
        ...
        return (
         unpackdir, os.path.join(unpackdir, baseModelName))

    @staticmethod
    def compile(models=[], output=sys.stdout, modelPath=None, namespace='app', version='1.0', logLevel=None, logDir=None, appHome=None, clean=False, outputdir='.', modelProperties={}, execute=False, sites='local', toLogFile=None, packaging=False, packagingAdditionalFiles=[], plugin=CompilerPlugin()):
        ...
        compiler = GraysonCompiler(namespace=namespace, version=version, logManager=logManager, clean=clean, appHome=appHome, outputDir=outputdir)
        ...
        if packaging:
            ...
            if execute:
                ...
                compiler.getWorkflowManagementSystem().executeWorkflow(compiler.getSites(), workflowName, plugin)
        return compiler