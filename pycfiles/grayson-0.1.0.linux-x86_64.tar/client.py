...
# [GCC 8.4.0]
# Embedded file name: /home/scox/dev/grayson/venv/lib/python2.7/site-packages/myproxy/client.py
...
class _HostCheck(SSL.Checker.Checker):
    ...
    def __init__(self, myProxyServerDN=os.environ.get('MYPROXY_SERVER_DN'), cnHostPfx='', **kw):
    ...
    def __call__(self, peerCert, host=None):
        ...
        """
        cnHostPfx = isinstance(self.cnHostPfx, basestring) and self.cnHostPfx or ''
...
class MyProxyClient(object):
    ...
    defProxyFile = sys.platform == 'win32' and 'proxy' or sys.platform in ('linux2',
                                                                           'darwin') and '/tmp/x509up_u%s' % os.getuid() or None
    propertyDefaults = {'hostname': 'localhost', 
       'port': 7512, 
       'serverDN': '', 
       'serverCNPrefix': '', 
       'openSSLConfFilePath': '', 
    ...
    def __init__(self, cfgFilePath=None, **prop):
        ...
        self.hostname = os.environ.get('MYPROXY_SERVER', MyProxyClient.propertyDefaults['hostname'])
        self.port = int(os.environ.get('MYPROXY_SERVER_PORT', MyProxyClient.propertyDefaults['port']))
        self.serverDN = os.environ.get('MYPROXY_SERVER_DN', MyProxyClient.propertyDefaults['serverDN'])
    ...
    def _setHostname(self, val):
        if not isinstance(val, basestring):
            raise AttributeError('Expecting string type for hostname attribute')
        self._hostname = val

    hostname = property(fget=_getHostname, fset=_setHostname, doc='hostname of MyProxy server')
    ...
    port = property(fget=_getPort, fset=_setPort, doc='Port number for MyProxy server')
    ...
    def _setServerDN(self, val):
        if not isinstance(val, basestring):
            raise AttributeError('Expecting string type for serverDN attribute')
        self._serverDN = val

    serverDN = property(fget=_getServerDN, fset=_setServerDN, doc='Distinguished Name for MyProxy Server Certificate')
    ...
    def _setServerCNPrefix(self, val):
        if not isinstance(val, basestring):
            raise AttributeError('Expecting string type for serverCNPrefix attribute')
        self._serverCNPrefix = val

    serverCNPrefix = property(fget=_getServerCNPrefix, fset=_setServerCNPrefix, doc="Prefix if any for Server Certificate DN Common Name e.g. 'host/'")
    ...
    def _setOpenSSLConfFilePath(self, val):
        if not isinstance(val, basestring):
            raise AttributeError('Expecting string type for openSSLConfFilePath attribute')
    ...
    openSSLConfFilePath = property(fget=_getOpenSSLConfFilePath, fset=_setOpenSSLConfFilePath, doc='file path for OpenSSL config file')
    ...
    def _setProxyCertMaxLifetime(self, val):
        if isinstance(val, basestring):
            ...
            raise AttributeError('Expecting int type for proxyCertMaxLifetime attribute')

    proxyCertMaxLifetime = property(fget=_getProxyCertMaxLifetime, fset=_setProxyCertMaxLifetime, doc='Default max. lifetime allowed for Proxy Certificate retrieved - used by store method')
    ...
    def _setProxyCertLifetime(self, val):
        if isinstance(val, basestring):
            ...
            raise AttributeError('Expecting int type for proxyCertLifetime attribute')

    proxyCertLifetime = property(fget=_getProxyCertLifetime, fset=_setProxyCertLifetime, doc='Default proxy cert. lifetime used in logon request')
    ...
    def _setCACertFilePath(self, val):
        ...
        if isinstance(val, basestring):
            ...
            raise AttributeError('Expecting string type for caCertFilePath attribute')
        return

    caCertFilePath = property(fget=_getCACertFilePath, fset=_setCACertFilePath, doc='CA certificate file path - MyProxy server certificate must validate against it and/or any present in caCertDir')
    ...
    def _setCACertDir(self, val):
        ...
        if isinstance(val, basestring):
            ...
            raise AttributeError('Expecting string or None type for caCertDir attribute')
        return

    caCertDir = property(fget=_getCACertDir, fset=_setCACertDir, doc='directory containing PEM encoded CA certificates.  Use along with caCertFilePath setting to validate MyProxy server certificate')
    ...
    def _initConnection(self, ownerCertFile=None, ownerKeyFile=None, ownerPassphrase=None):
        ...
        if self.caCertFilePath or self.caCertDir:
            context.load_verify_locations(cafile=self.caCertFilePath, capath=self.caCertDir)
        ...
        if ownerCertFile:
            try:
                ...
                raise MyProxyClientConfigError('Loading certificate and private key for SSL connection [also check CA certificate settings]: %s' % e)
        ...
        hostCheck = _HostCheck(host=self.hostname, myProxyServerDN=self.serverDN, cnHostPfx=self.serverCNPrefix)
    ...
    def writeProxyFile(cls, proxyCert, proxyPriKey, userX509Cert, filePath=None):
        ...
        if filePath is None:
            MyProxyClientConfigError('Error setting proxy file path - invalid platform?')
        outStr = proxyCert + proxyPriKey + userX509Cert
        open(MyProxyClient.defProxyFile, 'w').write(outStr)
        try:
            os.chmod(MyProxyClient.defProxyFile, 384)
        except Exception as e:
    ...
    def info(self, username, ownerCertFile=None, ownerKeyFile=None, ownerPassphrase=None):
        ...
        if not ownerCertFile:
            if globusLoc:
                ownerCertFile = os.path.join(globusLoc, *MyProxyClient._hostCertSubDirPath)
                ownerKeyFile = os.path.join(globusLoc, *MyProxyClient._hostKeySubDirPath)
            else:
                raise MyProxyClientError('No client authentication cert. and private key file were given')
        conn = self._initConnection(ownerCertFile=ownerCertFile, ownerKeyFile=ownerKeyFile, ownerPassphrase=ownerPassphrase)
        ...
        return (
         not bool(respCode), errorTxt, field)

    def changePassphrase(self, username, passphrase, newPassphrase, ownerCertFile=None, ownerKeyFile=None, ownerPassphrase=None):
    ...
    def destroy(self, username, ownerCertFile=None, ownerKeyFile=None, ownerPassphrase=None, credname=None):
    ...
    def put(self, username, passphrase, certFile, keyFile, passphraseCallback, ownerCertFile=None, ownerKeyFile=None, ownerPassphraseCallback=None, lifetime=None, retrievers=None, force=True, credowner=None, credname=None, creddesc=None):
        ...
        if not force:
            if self.info(username, ownerCertFile=ownerCertFile, ownerKeyFile=ownerKeyFile, ownerPassphrase=ownerPassphrase)[0]:
                raise MyProxyClientError('Credentials already exist for user: %s' % username)
        conn = self._initConnection(ownerCertFile=ownerCertFile, ownerKeyFile=ownerKeyFile, ownerPassphrase=ownerPassphrase)
        conn.connect((self.hostname, self.port))
        conn.write('0')
        ...
        if isinstance(certFile, Certificate):
            ...
            usercert = Certificate(certFile, keyFile, callback=passphraseCallback)
    ...
    def store(self, username, passphrase, certFile, keyFile, ownerCertFile=None, ownerKeyFile=None, ownerPassphrase=None, lifetime=None, force=True):
        """Upload credentials to the server
