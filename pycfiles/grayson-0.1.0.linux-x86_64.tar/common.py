...
# [GCC 8.4.0]
# Embedded file name: /home/scox/dev/grayson/venv/lib/python2.7/site-packages/grayson/view/common.py
# Compiled at: 2012-03-02 14:59:52
import os, json, string, traceback, logging, pwd
...
app_context = {GraysonWebConst.TITLE: GraysonWebConst.GRAYSON, 
...
class ViewUtil:
    ...
    def form_workflow_path(user, file_name=''):
        return os.path.join(settings.GRAYSONWEB_WORKFLOW_ROOT, user.username, file_name)
    ...
    @staticmethod
    def get_response(template, request, context={}, mimeType=GraysonWebConst.MIME_HTML, status=200):
        context[GraysonWebConst.APP] = app_context
        return HttpResponse(loader.get_template(template).render(RequestContext(request, context)), mimeType, status, mimeType)
