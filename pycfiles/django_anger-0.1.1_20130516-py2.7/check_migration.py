...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/django_anger/check_migration.py
# Compiled at: 2013-05-13 21:14:30
"""
    ...
    for model, fields in models.iteritems():
        for field_name, field_def in fields.iteritems():
            ...
            if target not in models:
                raise ValidationError(("Field '{}' has ForeignKey to model '{}', which is not frozen.").format(field_name, target))
    ...
    for line in f:
        for model in models.keys():
            if ("'{}':").format(model) in line:
                if model in found:
                    raise ValidationError(("Model '{}' frozen twice.").format(model))
                found[model] = True
            ...
            for field in models[current_model].keys():
                if line.startswith(("'{}': (").format(field)):
                    if field in fields_found:
                        raise ValidationError(("Field '{}' of model '{}' frozen twice.").format(field, current_model))
...
def check_model_names(f):
    ...
    for model, fields in models.iteritems():
        ...
        if lowercase_model_name.lower() != lowercase_model_name:
            raise ValidationError(("Expected lower-case app.model_name, but found '{}'.").format(lowercase_model_name))
        if capitalized_model_name.lower() != lowercase_model_name:
            raise ValidationError(("Meta object_name '{}' does not match expected model name '{}'.").format(capitalized_model_name, lowercase_model_name))
...
def check_gratuitous_frozen_models(f):
    ...
    for model in models:
        ...
        if app not in complete_apps and model not in related_targets:
            raise ValidationError(("Model '{}' frozen but not used.").format(model))
