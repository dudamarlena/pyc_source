...
# Embedded file name: /home/madjuice/Documents/Python/crosspredict/crosspredict/crossval/_crossval.py
...
class CrossModelFabric(ABC):
    ...
    def transform(self, df):
        ...
        for fold, (train, val) in enumerate(self.iterator.split(df)):
            ...
            predict.loc[X_val.index] += model.predict((X_val[model.feature_name()].astype(float)), num_iteration=(self.num_boost_optimal)) / self.iterator.n_repeats
    ...
    def predict(self, test):
        ...
        for fold in self.models.keys():
            model = self.models[fold]
            predict += model.predict((test[model.feature_name()].astype(float)),
    ...
    def shap(self, df: pd.DataFrame):
        ...
        sns.barplot(x='value', y='feature', data=best_features_melt, estimator=(np.mean),
          order=cols_best)
        return (
         fig, shap_feature_stats.reset_index())
