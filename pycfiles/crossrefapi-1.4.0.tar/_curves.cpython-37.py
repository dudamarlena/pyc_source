...
# Embedded file name: /home/madjuice/Documents/Python/crosspredict/crosspredict/report_binary/_curves.py
# Compiled at: 2020-04-05 10:18:58
...
class CurveFabric(ABC):
    ...
    def _draw_series_to_bar_plot(self, df_series, ax, label='Count'):
        ...
        if df_series.index.dtype == '<M8[ns]':
            plt.xticks(ind, df_series.index.map(lambda x: x.strftime('%Y-%m')).sort_values(ascending=True))
        else:
...
class GenGINICurve(CurveFabric):
    ...
    def fit(self, df):
        self.series_auc = df.groupby(self._col_generation_deals).apply(lambda x:         if len(x[self.col_target].unique()) > 1:
100 * (2 * roc_auc_score(x[self.col_target], x[self.col_score]) - 1) # Avoid dead code: 0)
...
class MeanProbCurve(CurveFabric):
    ...
    def fit(self, df):
        self.pd_deciles, self.pd_bins = pd.qcut((df[self.col_score]), q=(np.arange(0, 1.000001, 0.1)),
          duplicates='drop',
          retbins=True)
        self.mean_prob = df.groupby(self.pd_deciles)[self.col_target].agg(['count', 'mean'])
        return self

    def plot(self, ax=None, title=None, **kwargs):
        self._draw_series_to_bar_plot((self.mean_prob['count']),
          ax, label='Count')
        ax_twinx = ax.twinx()
        self._draw_series_to_line_plot((self.mean_prob['mean']),
          ax_twinx, label='Risk')
        if title:
            ax.set_title(title, fontsize=14, fontweight='bold')
...
class RocAucCurve(CurveFabric):
    ...
    def fit(self, df):
        self.fpr, self.tpr, _ = roc_curve(df[self.col_target], df[self.col_score])
    ...
    def plot(self, ax=None, title=None, **kwargs):
        if ax is None:
            fig, ax = plt.subplots()
        viz = RocCurveDisplay(fpr=(self.fpr),
          tpr=(self.tpr),
...
class PrecisionRecallCurve(CurveFabric):
    ...
    def fit(self, df):
        self.precision, self.recall, _ = precision_recall_curve(df[self.col_target], df[self.col_score])
        self.average_precision = average_precision_score(df[self.col_target], df[self.col_score])
        return self

    def plot(self, ax=None, title=None, **kwargs):
        if ax is None:
            fig, ax = plt.subplots()
        viz = PrecisionRecallDisplay(precision=(self.precision),
          recall=(self.recall),
