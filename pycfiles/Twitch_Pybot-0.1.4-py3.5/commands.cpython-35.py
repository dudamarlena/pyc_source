...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pybot/features/commands.py
...
class Commands:
    ...
    def hook(self, con, msg, event):
        if event == 'user_privmsg':
            ...
            if checkIfCommand(text, '!command', 'add') and self.con.isMod(name):
                ...
                if checkIfCommand(text, '!command', 'remove') and self.con.isMod(name):
