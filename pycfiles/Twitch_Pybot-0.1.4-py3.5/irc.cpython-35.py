...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pybot/irc.py
# Compiled at: 2016-07-13 16:56:07
# Size of source mod 2**32: 16186 bytes
import json, os, random, socket, sys, threading, time, urllib.request, traceback, pybot.globals as globals
...
class irc:
    ...
    def __init__(self, hook):
        ...
        self.chat_timeout_max = 60
        self.chat_check_mods = 30
        self.chat_time = 0
        ...
        if filterList != '':
            pybotPrint('[pybot.irc] %s filters loaded' % filterList, 'log')
    ...
    def msg(self, text, show=True, parse=False):
        if self.connected == True:
            ...
            if show:
                pybotPrint(self.nick + ' : ' + text, 'usermsg')
                ...
                for msg in messageList:
                    ...
                    if msg == 'ERROR':
                        ...
                        if 'PRIVMSG' not in msg:
                            ...
                            if 'JOIN' in msg and self.nick not in msg:
                                ev = 'user_join'
                                name = self.getPrivMsgName(msg)
                                self.totalUsers += 1
                                pybotPrint('[IRC] %s has joined the channel (%s)' % (name, self.totalUsers), 'irc')
                            if 'PART' in msg and self.nick not in msg:
                                ev = 'user_part'
                                name = self.getPrivMsgName(msg)
    ...
    def isMod(self, name):
        if name.lower() == self.channel.replace('#', '').lower():
            return True
            try:
