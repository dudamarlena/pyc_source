...
# Compiled at: 2017-02-22 05:37:48
...
class FrameRateTimer(pgg.IObserver):
    ...
    def __bool__(self):
        ...
        elif n_init_items >= len(self._timestamps):
            raise IndexError('Not enough data collected')
        timestamps = self._timestamps[n_init_items:]
        if use_numpy:
            intervals = timestamps[1:] - timestamps[0]
            frame_rates = np.array([ (i + 1) / (interval / np.timedelta64(1, 's')) for i, interval in enumerate(intervals)
                                   ])
            return np.min(frame_rates) >= self._frame_rate
        else:
            intervals = [ (timestamp - timestamps[0]).total_seconds() for timestamp in timestamps[1:] ]
...
class FileChecker(pgg.IObserver):
    ...
    def assert_frame_dimensions(self, frame_width, frame_height):
        for frame_desc in self._frame_descs:
            if frame_desc.cols != frame_width or frame_desc.rows != frame_height:
                return False

        return True

    def assert_frame_data_lengths(self, colour, frame_width, frame_height):
        exp_data_length = pgg.VideoFrame.required_data_length(colour, frame_width, frame_height)
        for frame_desc in self._frame_descs:
