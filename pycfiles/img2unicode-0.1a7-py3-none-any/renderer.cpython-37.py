...
# [GCC 8.4.0]
# Embedded file name: /home/matraszek/PycharmProjects/img2unicode/build/lib/img2unicode/renderer.py
# Compiled at: 2020-05-03 17:04:51
# Size of source mod 2**32: 6808 bytes
import logging
from pathlib import Path
import PIL, PIL.Image, numpy as np, skimage.filters, skimage.transform, skimage.feature, skimage
...
class Renderer:
        ...
        if isinstance(path_or_img, PIL.Image.Image):
            ...
            if isinstance(path_or_img, (str, Path)):
                ...
                if isinstance(path_or_img, np.ndarray):
                    ...
                    if isinstance(path_or_img, bytes):
    ...
    def prerender(self, path_or_img, optimizer=None, templates=DEFAULT_TEMPLATES, show=False, **kwargs):
        img, chars, fgs, bgs = (self.optimize)(path_or_img, optimizer, **kwargs)
        cs_all = templates.base_16x8.clip(0.0, 1.0)
        ...
        recov_img = uncubify(recov.reshape(-1, 16, 8, 3), img.shape).clip(0.0, 1.0)
        if show:
...
class GammaRenderer(Renderer):
    ...
    def _prepare_image(self, img):
        ...
        img_edges = skimage.feature.canny(skimage.transform.downscale_local_mean(imgl, (2,
                                                                                        2)), 1).astype('float')
