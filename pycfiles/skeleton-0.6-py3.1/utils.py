...
# [GCC 8.4.0]
# Embedded file name: /Users/dinoboff/Documents/workspace/skeleton/src/build/lib/skeleton/utils.py
# Compiled at: 2010-05-11 19:01:15
"""
Skeleton Helpers
"""
from contextlib import closing
import codecs, logging, optparse, os, re, stat, sys
...
def vars_to_optparser(variables, parser=None):
    ...
    for var in variables:
        if not VALID_OPTION_NAME.match(var.name):
            continue
        parser.add_option('--%s' % var.name.lower().replace('_', '-'), dest=var.name, help=var.full_description, metavar=var.name.split('_')[(-1)].upper())
...
def insert_into_file(file_path, marker, text, marker_tag='-*-', keep_indent=True, keep_marker=True, encoding='UTF-8'):
    ...
    marker_re = re.compile('^(\\s*).*%s.*$' % marker_pattern)
    edited = False
    new_content = []
