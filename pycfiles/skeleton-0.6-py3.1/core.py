...
# [GCC 8.4.0]
# Embedded file name: /Users/dinoboff/Documents/workspace/skeleton/src/build/lib/skeleton/core.py
# Compiled at: 2010-05-11 19:01:15
"""
Core skeleton component
"""
import codecs, collections, datetime, functools, logging, optparse, os, shutil, sys, weakref
...
class Skeleton(collections.MutableMapping):
    ...
    def real_src(self):
        ...
        """
        if self.src is None:
            raise AttributeError('The src attribute of the %s Skeleton is not set' % self.__class__.__name__)
    ...
    def keys(self):
        ...
        """
        return list(set(list(self.set_variables.keys()) + list(self._defaults.keys())))
    ...
    def write(self, dst_dir, run_dry=False):
        ...
        for dir_path, dir_names, file_names in os.walk(real_src):
            rel_dir_path = dir_path[real_src_len:].lstrip('\\/')
            for file_name in file_names:
                src = os.path.join(dir_path, file_name)
                dst = os.path.join(dst_dir, rel_dir_path, self._format_file_name(file_name, dir_path))
                self._copy_file(src, dst)

            for dir_name in dir_names:
                src = os.path.join(dir_path, dir_name)
                dst = os.path.join(dst_dir, rel_dir_path, self.template_formatter(dir_name))
    ...
    def cmd(cls, argv=None):
        ...
        logging.basicConfig(level=options.verbose_, format='%(levelname)s - %(message)s')
    ...
    def configure_parser(self):
        ...
        parser.add_option('-q', '--quiet', action='store_const', const=logging.FATAL, dest='verbose_')
        parser.add_option('-v', '--verbose', action='store_const', const=logging.INFO, dest='verbose_')
        parser.add_option('-d', '--debug', action='store_const', const=logging.DEBUG, dest='verbose_')
