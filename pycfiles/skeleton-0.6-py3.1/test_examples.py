...
# [GCC 8.4.0]
# Embedded file name: /Users/dinoboff/Documents/workspace/skeleton/src/build/lib/skeleton/tests/test_examples.py
# Compiled at: 2010-05-11 10:16:00
"""
Tests the BDS, GPL and LGPL skeleton
"""
import subprocess, sys, unittest
from skeleton.examples.basicpackage import BasicPackage, main
from skeleton.examples.licenses import BSD, BSD_THIRD_CLAUSE, GPL, LGPL, NoLicense, LicenseChoice
...
class TestBSD(TestCase):
    ...
    def test_write_3clause(self):
        ...
        with TempDir() as (tmp):
            skel.write(tmp.path)
            self.assertEqual(skel['third_clause'], BSD_THIRD_CLAUSE.format(organization='Foo inc'))
            self.assertTrue(tmp.exists('LICENSE'))
...
class TestLicenseChoice(TestCase):
    ...
    def test_lgpl_run(self):
        ...
        """
        resps = [
         'Foo', 'Damien Lebrun', 'dinoboff@gmail.com', 'LGPL']
...
class TestBasicModule(TestCase):
    ...
    def test_write(self):
        ...
        """
        skel = BasicModule(module_name='foo', author='Damien Lebrun', author_email='dinoboff@gmail.com')
        with TempDir() as (tmp):
            skel.write(tmp.path)
...
class TestBasicPackage(TestCase):
    ...
    def test_write(self):
        ...
        """
        skel = BasicPackage(project_name='foo', package_name='foo', author='Damien Lebrun', author_email='dinoboff@gmail.com')
        with TempDir() as (tmp):
            skel.write(tmp.path)
    ...
    def test_write_with_bsd(self):
        ...
        """
        skel = BasicPackage(project_name='foo', package_name='foo', author='Damien Lebrun', author_email='dinoboff@gmail.com', license='BSD')
        with TempDir() as (tmp):
            skel.write(tmp.path)
            self.assertTrue(tmp.exists('LICENSE'))
    ...
    def test_write_namespaces(self):
        ...
        """
        skel = BasicPackage(project_name='foo.bar.baz', package_name='foo.bar.baz', author='Damien Lebrun', author_email='dinoboff@gmail.com')
        with TempDir() as (tmp):
            skel.write(tmp.path)
            self.assertEqual(set(skel['ns_packages']), set(['foo', 'foo.bar']))
            self.assertEqual(set(skel['packages']), set(['foo', 'foo.bar', 'foo.bar.baz']))
            self.assertTrue(tmp.exists('distribute_setup.py'))
            self.assertTrue(tmp.exists('MANIFEST.in'))
            self.assertTrue(tmp.exists('README.rst'))
            self.assertTrue(tmp.exists('LICENSE'))
    ...
    def test_main(self):
        ...
        resps = [
         'foo', 'foo', 'Damien Lebrun', 'dinoboff@gmail.com', 'BSD', '']
        self.input_mock.side_effect = lambda x: resps.pop(0)
        with TempDir() as (tmp):
            main([tmp.path])
            self.assertTrue(tmp.exists('distribute_setup.py'))
            ...
            proc = subprocess.Popen([
             sys.executable, setup, 'egg_info'], shell=False, stdout=subprocess.PIPE)
            self.assertEqual(proc.wait(), 0)
            proc = subprocess.Popen([
             sys.executable, setup, '--classifiers'], shell=False, stdout=subprocess.PIPE)
            self.assertEqual(proc.wait(), 0)
            classifiers = proc.stdout.read().decode().splitlines()
            self.assertTrue('License :: OSI Approved' in classifiers)
            self.assertTrue('License :: OSI Approved :: BSD License' in classifiers)
