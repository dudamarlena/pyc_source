...
# [GCC 8.4.0]
# Embedded file name: /Users/dinoboff/Documents/workspace/skeleton/src/build/lib/skeleton/tests/test_core.py
# Compiled at: 2010-05-11 19:01:16
"""
...
from skeleton.core import Skeleton, Var, TemplateKeyError, FileNameKeyError, Bool
THIS_YEAR = datetime.datetime.utcnow().year

class WithDefault(Skeleton):
    """Skeleton with variables with default"""
    variables = [
     Var('foo'), Var('bar', default=2), Var('baz')]
...
class TestSkeleton(TestCase):
    ...
    def test_check_var_with_default_var(self):
        ...
        try:
            ...
            self.fail('check_variables() should not raise KayError if the missing variable has a default.')
    ...
    def test_write_create_dst_dir(self):
        ...
        with TempDir() as (tmp_dir):
            ...
            self.assertEqual(open(tmp_dir.join('missing-dir/foo.txt')).read().strip(), 'foo')
            self.assertEqual(open(tmp_dir.join('missing-dir/bar/baz.txt')).read().strip(), 'baz')
    ...
    def test_write_static_file(self):
        ...
        with TempDir() as (tmp_dir):
            skel.write(tmp_dir.path)
            self.assertEqual(open(tmp_dir.join('foo.txt')).read().strip(), 'foo')
            self.assertEqual(open(tmp_dir.join('bar/baz.txt')).read().strip(), 'baz')
    ...
    def test_write_dynamic_content(self):
        ...
        with TempDir() as (tmp_dir):
            skel.write(tmp_dir.path)
            self.assertEqual(open(tmp_dir.join('foo.txt')).read().strip(), 'foo')
            self.assertEqual(open(tmp_dir.join('bar/baz.txt')).read().strip(), 'foo <replaced> bar')
    ...
    def test_write_dynamic_file_names(self):
        ...
        with TempDir() as (tmp_dir):
            skel.write(tmp_dir.path)
            self.assertEqual(open(tmp_dir.join('foo.txt')).read().strip(), 'foo')
            self.assertEqual(open(tmp_dir.join('bar/replaced-name.txt')).read().strip(), 'baz')
    ...
    def test_write_file_name_fails(self):
        ...
        with TempDir() as (tmp_dir):
            try:
                skel.write(tmp_dir.path)
                self.fail('An exception should be raised')
            except (FileNameKeyError,) as exc:
    ...
    def test_run_with_var(self):
        ...
        with TempDir() as (tmp_dir):
            ...
            self.assertEqual(open(tmp_dir.join('foo.txt')).read().strip(), 'foo')
            self.assertEqual(open(tmp_dir.join('bar/baz.txt')).read().strip(), 'foo <input replacement> bar')
...
class TestDefaultTemplate(unittest.TestCase):
    ...
    def test_template_formatter(self):
        ...
        self.assertEqual(skel.template_formatter('foo {bar} baz'), 'foo substituted baz')

    def test_formatter_raise_key_error(self):
        """Tests template formatting a variable not set"""
        skel = Skeleton(bar='substituted')
        self.assertRaises(KeyError, skel.template_formatter, 'foo {bar} {fooz} baz')

    def test_template_use_default(self):
        """Tests the template uses the default value if the variable is not set
        """
        skel = WithDefault(foo=1, baz=3)
        self.assertEqual(skel.template_formatter('{foo} {bar} {baz}'), '1 2 3')
