...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/label/tests/test_run.py
...
def test_list_short(app, label_store):
    label_store.put(Label('c1', 'c2', 'annotator', CorefValue.Positive, epoch_ticks=1234567890))
    app.runcmd('list', [])
    assert app.stdout.getvalue() == 'c1 ==(1) c2 by annotator at 2009-02-13 23:31:30\n'


def test_list_two(app, label_store):
    label_store.put(Label('c1', 'c2', 'a1', CorefValue.Positive, epoch_ticks=1234567890))
    label_store.put(Label('c1', 'c2', 'a2', CorefValue.Negative, epoch_ticks=1234567890))
    app.runcmd('list', [])
    assert app.stdout.getvalue() == 'c1 ==(1) c2 by a1 at 2009-02-13 23:31:30\nc1 !=(0) c2 by a2 at 2009-02-13 23:31:30\n'


def test_list_collapses(app, four_labels):
    app.runcmd('list', [])
    assert app.stdout.getvalue() == 'c1 ==(1) c2 by a1 at 2009-02-13 23:31:32\nc1 !=(0) c2 by a2 at 2009-02-13 23:31:30\n'
...
def test_list_subtopics(app, label_store):
    label_store.put(Label('c1', 'c2', 'a1', CorefValue.Positive, epoch_ticks=1234567890, subtopic_id1='s1', subtopic_id2='s2'))
    app.runcmd('list', [])
    assert app.stdout.getvalue() == 'c1(s1) ==(1) c2(s2) by a1 at 2009-02-13 23:31:30\n'
...
def test_load_short(app, label_store, two_item_dump, tmpdir):
    ...
    assert len(list(label_store.everything(include_deleted=True))) == len(two_item_dump)
    assert len(list(label_store.everything(include_deleted=False))) == len(two_item_dump)
...
def test_load_full(app, label_store, four_item_dump, tmpdir):
    ...
    app.runcmd('load', [str(fn)])
    assert len(list(label_store.everything(include_deleted=True))) == len(four_item_dump)
...
def test_get_negative(app, four_labels):
    ...
    assert app.stdout.getvalue() == 'c1 !=(0) c2 by a2 at 2009-02-13 23:31:30\n'


def test_connected(app, label_store):
    label_store.put(Label('c1', 'c2', 'a1', CorefValue.Positive, epoch_ticks=1234567890))
    label_store.put(Label('c3', 'c2', 'a1', CorefValue.Positive, epoch_ticks=1234567890))
...
def test_connected_negative(app, label_store):
    label_store.put(Label('c1', 'c2', 'a1', CorefValue.Positive, epoch_ticks=1234567890))
    label_store.put(Label('c3', 'c2', 'a1', CorefValue.Negative, epoch_ticks=1234567890))
    app.runcmd('connected', ['c2'])
    assert app.stdout.getvalue() == 'c1 ==(1) c2 by a1 at 2009-02-13 23:31:30\n'