...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/label/tests/test_label_store.py
...
def test_everything_two(label_store):
    ...
    @qc
    def _(cid1a=id_, cid1b=id_, ann1=id_, v1=coref_value, t1=time_value, cid2a=id_, cid2b=id_, ann2=id_, v2=coref_value, t2=time_value):
        label_store.delete_all()
...
def test_everything_subtopic_id(label_store):
    ...
    @qc
    def _(cid1a=id_, cid1b=id_, sid1a=id_, sid1b=id_, ann1=id_, v1=coref_value, t1=time_value, cid2a=id_, cid2b=id_, sid2a=id_, sid2b=id_, ann2=id_, v2=coref_value, t2=time_value):
        label_store.delete_all()
        l1 = Label(cid1a, cid1b, ann1, v1, epoch_ticks=t1, subtopic_id1=sid1a, subtopic_id2=sid1b)
        l2 = Label(cid2a, cid2b, ann2, v2, epoch_ticks=t2, subtopic_id1=sid2a, subtopic_id2=sid2b)
        label_store.put(l1)
        label_store.put(l2)
        if l1.same_subject_as(l2):
            if l1.epoch_ticks == l2.epoch_ticks:
                ...
                expected = list(sorted([l1, l2]))[0:1]
        elif cid1a == cid2a and sid1a == sid2a or cid1a == cid2b and sid1a == sid2b:
            expected = list(sorted([l1, l2]))
        else:
            expected = [
             l1]
        assert list(label_store.everything(content_id=cid1a, subtopic_id=sid1a)) == expected
...
def test_everything_prefix(label_store):
    ...
    @qc
    def _(pfx1=str_letters(length=int_(1, 8)), pfx2=str_letters(length=int_(1, 8)), sfx1=str_letters(length=int_(1, 12)), sfx2=str_letters(length=int_(1, 12)), ann=id_, v=coref_value):
        label_store.delete_all()
        cid1 = pfx1 + sfx1
        cid2 = pfx1 + sfx2
        l = Label(cid1, cid2, ann, v)
...
def test_negative_label_inference(label_store):
    ...
    assert frozenset(map(get_pair, inference)) == frozenset(correct_pairs)
