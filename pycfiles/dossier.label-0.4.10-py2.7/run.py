...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/label/run.py
...
def dict_to_label(d):
    return Label(content_id1=d['content_id1'], content_id2=d['content_id2'], annotator_id=d['annotator_id'], value=CorefValue(d['value']), subtopic_id1=d.get('subtopic_id1', None), subtopic_id2=d.get('subtopic_id2', None), epoch_ticks=d.get('epoch_ticks', None), rating=d.get('rating', None))
...
class App(yakonfig.cmd.ArgParseCmd):
    ...
    def args_list(self, p):
        p.add_argument('--include-deleted', action='store_true', help='When set, show deleted labels.')
        p.add_argument('--substring', help='Case-sensitive string to filter content_ids')
        p.add_argument('--ignore-case', action='store_true', help='Switch substring filtering to case-insensitive')
        p.add_argument('--delete', action='store_true', help='Delete labels that are listed; you will be asked to approve the displayed list.')

    def do_list(self, args):
        labels = self.label_store.everything(include_deleted=args.include_deleted)

        def substring_match(k):
            if args.substring in k.content_id1 or args.substring in k.content_id2:
                return True
            if args.ignore_case:
                if args.substring.lower() in k.content_id1.lower() or args.substring.lower() in k.content_id2.lower():
                    return True
        ...
        if args.delete:
            response = raw_input('Are you sure that you want to delete %d labels? [y/N]: ' % len(to_delete))
    ...
    def args_dump_all(self, p):
        p.add_argument('--exclude-deleted', action='store_true', help='When set, only the most recent labels are dumped.')

    def do_dump_all(self, args):
        labels = list(imap(label_to_dict, self.label_store.everything(include_deleted=not args.exclude_deleted)))
    ...
    def args_load(self, p):
        p.add_argument('fpath', nargs='?', default=None, help='File path containing label data. When absent, stdin is used.')
        return
    ...
    def args_get(self, p):
        p.add_argument('content_id', type=str, help='Show all labels directly ascribed to content_id')
        p.add_argument('--value', type=int, choices=[-1, 0, 1], default=None, help='Only show labels with this coreferent value.')
        return
    ...
    def args_connected(self, p):
        p.add_argument('content_id', type=str, help='Show all labels connected to content_id')
...
def main():
    p = argparse.ArgumentParser(description='Interact with DossierStack truth data.')
    app = App()
    app.add_arguments(p)
