...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/label/relation_label.py
...
class RelationStrength(enum.Enum):
    ...
    def is_positive(self):
        return self == RelationStrength.WEAK or self == RelationStrength.STRONG or self == RelationStrength.AKA
...
class RelationLabel(Container, Hashable):
    ...
    def __init__(self, content_id1, content_id2, annotator_id, rel_strength, epoch_ticks=None, meta=None):
    ...
    def __hash__(self):
        return hash(self.content_id1) ^ hash(self.content_id2) ^ hash(self.annotator_id) ^ hash(self.epoch_ticks) ^ hash(self.rel_strength)
...
class RelationLabelStore(object):
    ...
    def _value_from_label(self, label):
        ...
        """
        unser_val = (
         label.rel_strength.value, label.meta)
        return cbor.dumps(unser_val)

    def _label_from_kvlayer(self, key, val):
        """Make a label from a kvlayer row.
        """
        content_id1, content_id2, annotator_id, inverted_epoch_ticks = key
        epoch_ticks = time_complement(inverted_epoch_ticks)
        rel_strength, meta = cbor.loads(val)
        return RelationLabel(content_id1, content_id2, annotator_id, RelationStrength(rel_strength), epoch_ticks=epoch_ticks, meta=meta)
    ...
    def get_related_ids(self, content_id, min_strength=None):
        ...
        """
        related_labels = self.get_related(content_id, min_strength=min_strength)
        related_idents = set()
