...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/label/label.py
# Compiled at: 2016-07-21 12:01:11
"""dossier.label.label
...
class Label(Container, Hashable):
    ...
    def __init__(self, content_id1, content_id2, annotator_id, value, subtopic_id1=None, subtopic_id2=None, epoch_ticks=None, rating=None, meta=None):
        """Create a new label.
        ...
        if value == CorefValue.Positive:
            ...
            rating = 0
        if content_id2 < content_id1 or content_id2 == content_id1 and subtopic_id2 < subtopic_id1:
    ...
    def as_dict(self):
        ...
        return {'content_id1': self.content_id1, 
           'subtopic_id1': self.subtopic_id1, 'content_id2': self.content_id2, 
           'subtopic_id2': self.subtopic_id2, 'annotator_id': self.annotator_id, 
           'value': self.value, 'epoch_ticks': self.epoch_ticks, 
           'rating': self.rating, 'meta': self.meta}
    ...
    def __contains__(self, v):
        ...
        if isinstance(v, tuple) and len(v) == 2:
            ...
            return cid == cid1 and subid == subid1 or cid == cid2 and subid == subid2
        else:
    ...
    def other(self, content_id):
        ...
        if content_id == self.content_id2:
    ...
    def changed(self, other):
        ...
        coref/rating values.
        """
        return self.same_subject_as(other) and (self.value != other.value or self.rating != other.rating)
    ...
    def diff(old, new):
        ...
        change = map(itemgetter(1), ifilter(lambda (o, n): o.changed(n), product(old, new)))
        unwrap = attrgetter('lab')
        return {'add': map(unwrap, add), 
           'delete': map(unwrap, delete), 
           'change': map(unwrap, change)}

    def __hash__(self):
        return hash(self.content_id1) ^ hash(self.content_id2) ^ hash(self.subtopic_id1) ^ hash(self.subtopic_id2) ^ hash(self.annotator_id) ^ hash(self.epoch_ticks) ^ hash(self.value) ^ hash(self.rating)

    def __str__(self):
        res = repr(self.content_id1)[1:-1]
    ...
    def __repr__(self):
        return ('Label(content_id1={0.content_id1}, content_id2={0.content_id2}, annotator_id={0.annotator_id}, value={0.value}, subtopic_id1={0.subtopic_id1}, subtopic_id2={0.subtopic_id2}, epoch_ticks={0.epoch_ticks}, rating={0.rating})').format(self)
...
class LabelStore(object):
    ...
    TABLE = 'label'
    _kvlayer_namespace = {TABLE: (
             str, str, str, str, str, long)}
    ...
    def _keys_from_label(self, label):
        k1 = (
         label.content_id1, label.content_id2,
    ...
    def get(self, cid1, cid2, annotator_id, subid1='', subid2=''):
        ...
        """
        t = (
         cid1, cid2, subid1, subid2, annotator_id)
    ...
    def _label_from_kvlayer(self, k, v):
        ...
        content_id1, content_id2, subtopic_id1, subtopic_id2, annotator_id, inverted_epoch_ticks = k
        ...
        if len(v) > 1:
            meta = cbor.loads(v[1:])
        return Label(content_id1=content_id1, content_id2=content_id2, annotator_id=annotator_id, value=value, subtopic_id1=subtopic_id1, subtopic_id2=subtopic_id2, epoch_ticks=epoch_ticks, rating=rating, meta=meta)
    ...
    def directly_connected(self, ident):
        ...
        return self.everything(include_deleted=False, content_id=content_id, subtopic_id=subtopic_id)
        ...
        while todo:
            ...
            for label in self.directly_connected(ident):
                if label.value != CorefValue.Positive:
                    continue
                ident1, ident2 = idents_from_label(label, subtopic=ident_has_subtopic(ident))
    ...
    def negative_inference(self, content_id):
        ...
        more information.
        """
        neg_labels = ifilter(lambda l: l.value == CorefValue.Negative, self.directly_connected(content_id))
    ...
    def negative_label_inference(self, label):
        ...
        for comp_label in cid2_comp:
            comp_cids = (
             comp_label.content_id1,
             comp_label.content_id2)
            for comp_cid in comp_cids:
                if not comp_cid == label.content_id2:
                    yield Label(label.content_id1, comp_cid, 'auto', CorefValue.Negative)
        ...
        for comp_label in cid1_comp:
            ...
            for comp_cid in comp_cids:
                if not comp_cid == label.content_id1:
                    yield Label(label.content_id2, comp_cid, 'auto', CorefValue.Negative)
    ...
    def _filter_keys(self, content_id=None, prefix=None, subtopic_id=None):
        ...
        def accept(kvp):
            content_id1, content_id2, subtopic_id1, subtopic_id2, annotator_id, inverted_epoch_ticks = kvp[0]
            is_sorted = content_id1 < content_id2 or content_id1 == content_id2 and subtopic_id1 <= subtopic_id2
    ...
    def everything(self, include_deleted=False, content_id=None, subtopic_id=None, prefix=None):
        ...
        labels = ifilter(self._filter_keys(content_id, prefix, subtopic_id), labels)
...
def expand_labels(labels, subtopic=False):
    ...
    for ident1, ident2 in combinations(connected_component, 2):
        if normalize_pair(ident1, ident2) not in data_backed:
            ...
            yield Label(cid1, cid2, annotator, CorefValue.Positive, subtopic_id1=subid1, subtopic_id2=subid2)
