...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/label/tests/bench_label_objects.py
...
cid, subid, ann_id, ticks = (
 'cid', 'subid', 'annid', int(time.time() * 1000))
coref_value = CorefValue.Positive
Label = namedtuple('_Label', 'content_id1 content_id2 subtopic_id1 ' + 'subtopic_id2 annotator_id epoch_ticks ' + 'value')

class LabelSlots(object):
    __slots__ = [
     'cid1', 'cid2', 'subid1', 'subid2', 'ann_id', 'ticks', 'val']
...
def run_benchmark(name):
    print(name)
    seconds = timeit('%s()' % name, setup='from __main__ import %s' % name, number=10000)
    print(seconds)
    print()
