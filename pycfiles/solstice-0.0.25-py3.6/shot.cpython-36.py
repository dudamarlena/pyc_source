...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/solstice/core/shot.py
...
class SolsticeShot(shot.ArtellaShot, object):
    ...
    def get_sequence(self):
        ...
        if not sequence_id:
            LOGGER.warning('Impossible to retrieve sequence name because shot data does not contains "{}" attribute.\nSequence Data: {}'.format(sequence_attr, self._shot_data))
            return
        else:
            if self._name_dict:
                if sequence_id in self._name_dict:
