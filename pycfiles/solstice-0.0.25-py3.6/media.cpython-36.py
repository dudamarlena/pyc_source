...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/solstice/managers/media.py
# Compiled at: 2020-05-03 22:16:37
# Size of source mod 2**32: 8551 bytes
"""
...
class SolsticeMediaManager(media.MediaManager, object):
    ...
    def stamp_image(self, source, output, config_dict=None):
        ...
        for i in range(total_frames):
            ...
            empty_frame = image.create_empty_image(empty_frame_path,
              resolution_x=res_x, resolution_y=res_y, background_color=[92, 92, 92])
        ...
        for i, (frame, empty_frame) in enumerate(frames_dict.items()):
            ...
            stream = ffmpeglib.draw_text(stream,
              task_name, x=40, y=(text_margin_y + font_height),
              font_file=font_file,
              font_size=font_size)
            stream = ffmpeglib.draw_text(stream,
              task_comment, x=40, y=(text_margin_y + font_height * 2 + 25),
              font_file=font_file,
              font_size=font_size)
            current_frame = int(float(start_frame)) + i
            fps_text = '{} ({})'.format(current_frame, i + 1)
            stream = ffmpeglib.draw_text(stream,
              fps_text, x=40, y=(res_y - font_height - text_margin_y - 70), font_file=font_file,
              font_size=font_size)
            camera_text_width = QFontMetrics(text_font).width(camera)
            stream = ffmpeglib.draw_text(stream,
              camera, x=(res_x / 2 - camera_text_width), y=(res_y - font_height - text_margin_y - 70),
              font_file=font_file,
              font_size=font_size)
            ...
            stream = ffmpeglib.draw_text(stream,
              fps_text, x=(res_x / 2 - fps_focal_length_width), y=(res_y - font_height - text_margin_y - 20),
              font_file=font_file,
              font_size=font_size)
            ...
            stream = ffmpeglib.draw_text(stream,
              user, x=(res_x - user_shot_text_width - text_margin_x), y=(res_y - font_height - text_margin_y - 70),
              font_file=font_file,
              font_size=font_size)
            stream = ffmpeglib.draw_text(stream,
              shot_name, x=(res_x - user_shot_text_width - text_margin_x), y=(res_y - font_height - text_margin_y - 20),
              font_file=font_file,
              font_size=font_size)
        ...
        if not frame_outputs:
            ...
            if not os.path.isfile(video_file_path):
                ...
                return
            draw_timestamp_stream = ffmpeglib.draw_timestamp_on_video(video_file_path,
              text='Time: ', x=40, y=(res_y - font_height - text_margin_y - 20), font_file=font_file,
              font_size=font_size,
              timecode_rate=fps,
              timecode=framecode)
    ...
    def _get_temp_file_path(self, file_path, suffix=None, index=None, padding=4):
        ...
        temp_file_path = path_utils.clean_path(os.path.join(file_path_dir, '{}.{}'.format(new_file_path, file_path_split[(-1)])))
