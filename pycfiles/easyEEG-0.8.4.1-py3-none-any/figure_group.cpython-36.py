...
# [GCC 8.4.0]
# Embedded file name: D:\Coding\py\py3\experiments\easyEEG_dist\easyEEG\graph\figure_group.py
# Compiled at: 2018-05-22 19:50:44
# Size of source mod 2**32: 8804 bytes
from ..default import *
...
def plot(self, plot_params=None, save=False, return_fig=False):
    ...
    if 'style' in plot_params:
        ...
        if plot_params['plot_type'][0] == 'direct':
            ...
            if plot_params['plot_type'][0] == 'matrix':
                matrix_plot(self.data, plot_params['x_title'], plot_params['y_title'], plot_params, self.annotation, fig)
            else:
                if plot_params['plot_type'][0] == 'float':
                    float_plot(self.data, plot_params['xy_locs'], plot_params, self.annotation, fig)
                else:
...
def matrix_plot(data, x_axis, y_axis, plot_params, annotation=None, fig=None):
    ...
    if 'title' not in plot_params:
        ...
        if plot_params['plot_type'][1] == 'topograph':
            if 'zlim' not in plot_params:
                plot_params['zlim'] = (
                 data.min().min(), data.max().max())
            else:
                if 'cbar_title' not in plot_params:
                    plot_params['cbar_title'] = ''
                if 'color' not in plot_params:
                    plot_params['color'] = plt.cm.jet
                if 'chan_locs' not in plot_params:
                    from .. import io
    ...
    x_axis_values = list(OrderedDict.fromkeys(data.index.get_level_values(x_axis)))
    y_axis_values = list(OrderedDict.fromkeys(data.index.get_level_values(y_axis)))
    ...
    for idx, data_name in enumerate(itertools.product(y_axis_values, x_axis_values)):
        ...
        fig.add_subplot(ax)
        select_subplot_type(plot_params['plot_type'][1], ax, data_cells[data_name], annotation_cells[data_name], plot_params)
    ...
    cb1 = matplotlib.colorbar.ColorbarBase(ax, cmap=(plot_params['color']), norm=matplotlib.colors.Normalize(vmin=(plot_params['zlim'][0]), vmax=(plot_params['zlim'][1])),
