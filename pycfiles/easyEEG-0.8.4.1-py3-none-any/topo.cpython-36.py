...
# [GCC 8.4.0]
# Embedded file name: D:\Coding\py\py3\experiments\easyEEG_dist\easyEEG\algorithms\topo.py
# Compiled at: 2018-05-09 06:58:22
...
def topography(self, step_size='1ms', win_size='1ms', sample='mean', sig_limit=0.05):
    ...
    def to_topo(case_raw_data):
        ...
        data_with_subject.columns = data_with_subject.columns.get_level_values(1)
    ...
    minmax = (
     topo_batch.min().min(), topo_batch.max().max())
...
def frequency_topography(self, step_size='1ms', win_size='1ms', sample='mean', sig_limit=0.05, target=10):
    if isinstance(target, (int, float)):
        ...
        if len(target) == 2:
            if isinstance(target[0], (int, float)):
                if isinstance(target[1], (int, float)):
                    freqs = np.arange(target[0], target[1])
    else:
        raise Exception('Unsupported value for "target". The value should be a number, or a list of two numbers')
    if freqs[0] == 0:
        freqs += 0.001

    def cwt(name, data):
        cwt_result = signal.cwt((np.array(data)[0]),
          (signal.ricker), widths=freqs).mean(axis=0)
    ...
    def to_topo(case_raw_data):
        ...
        data_without_subject.columns = data_without_subject.columns.get_level_values(1)
        if len(data_without_subject.index.get_level_values('condition_group').unique()) == 2:
            ...
            data_with_subject = sampling(data_with_subject, step_size, win_size, sample)
            data_with_subject.columns = data_with_subject.columns.get_level_values(1)
            topo_result = subtract(data_with_subject,
              between='condition_group', align='channel')
            stats_result = roll_on_levels_and_compare(data_with_subject, (stats_methods.t_test), levels=['time', 'channel'], between='condition_group', in_group='subject', prograssbar=False)
            return (
             topo_result, stats_result)
        else:
