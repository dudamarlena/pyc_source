...
# Embedded file name: /Users/charman/src/concrete-python/tests/test_fetch_wrapper.py
...
from concrete.util import FetchCommunicationServiceWrapper, FetchCommunicationClientWrapper
...
def fetch_client_wrapper_triple():
    ...
    port = 2
    return (
     host, port, FetchCommunicationClientWrapper(host, port))
...
def test_enter(mock_create_socket, mock_create_transport, mock_create_protocol, mock_client, fetch_client_wrapper_triple):
...
def test_serve(mock_create_server, fetch_service_wrapper):
    ...
    mock_create_server.assert_called_once_with(fetch_service_wrapper.processor, host, port)
