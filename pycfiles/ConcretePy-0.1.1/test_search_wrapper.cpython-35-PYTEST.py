...
# Embedded file name: /Users/charman/src/concrete-python/tests/test_search_wrapper.py
...
from concrete.util import SearchServiceWrapper, SearchClientWrapper
...
def search_client_wrapper_triple():
    ...
    port = 2
    return (
     host, port, SearchClientWrapper(host, port))
...
def test_enter(mock_create_socket, mock_create_transport, mock_create_protocol, mock_client, search_client_wrapper_triple):
...
def test_serve(mock_create_server, search_service_wrapper):
    ...
    mock_create_server.assert_called_once_with(search_service_wrapper.processor, host, port)
