...
# Embedded file name: /Users/charman/src/concrete-python/tests/test_search_proxy_wrapper.py
...
from concrete.util import SearchProxyServiceWrapper, SearchProxyClientWrapper
...
def proxy_client_wrapper_triple():
    ...
    port = 2
    return (
     host, port, SearchProxyClientWrapper(host, port))
...
def test_enter(mock_create_socket, mock_create_transport, mock_create_protocol, mock_client, proxy_client_wrapper_triple):
...
def test_serve(mock_create_server, proxy_service_wrapper):
    ...
    mock_create_server.assert_called_once_with(proxy_service_wrapper.processor, host, port)
