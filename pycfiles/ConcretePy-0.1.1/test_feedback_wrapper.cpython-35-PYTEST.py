...
# Embedded file name: /Users/charman/src/concrete-python/tests/test_feedback_wrapper.py
...
from concrete.util import FeedbackServiceWrapper, FeedbackClientWrapper
...
def feedback_client_wrapper_triple():
    ...
    port = 2
    return (
     host, port, FeedbackClientWrapper(host, port))
...
def test_enter(mock_create_socket, mock_create_transport, mock_create_protocol, mock_client, feedback_client_wrapper_triple):
...
def test_serve(mock_create_server, feedback_service_wrapper):
    ...
    mock_create_server.assert_called_once_with(feedback_service_wrapper.processor, host, port)
