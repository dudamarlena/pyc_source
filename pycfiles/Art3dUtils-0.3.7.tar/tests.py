...
# [GCC 8.4.0]
# Embedded file name: /home/max/Projects/art3dutils/art3dutils/tests/tests.py
# Compiled at: 2013-03-26 05:47:51
import unittest, mako.template
from xml.dom.minidom import parseString
from art3dutils.models import Project, Quarter, Building, Section, Floor, Apartment, populate_session
...
class ModelTest(unittest.TestCase):
    ...
    def test_get_apartments(self):
        ...
        self.assertEqual(8, len(floor.apartments))
        self.assertEqual([8, 7, 6, 5, 4, 3, 2, 1], [ apt.number for apt in floor.get_apartments() ])
        self.assertEqual([1, 2, 3, 4, 5, 6, 7, 8], [ apt.number for apt in floor.get_apartments(sort_key='number') ])
        self.assertEqual([1, 3, 4, 5, 6, 7, 8], [ apt.number for apt in floor.get_apartments(sort_key='number', available_only=True)
                                                ])
    ...
    def test_area_coords(self):
        apartment = Apartment.fetch_all('test', 1, 1, 1, 3, 1)[0]
        self.assertEqual('283,2,283,185,222,185,222,224,166,224,166,351,92,351,92,49,200,49,200,2,282,2,283,2', apartment.area_coords())

    def test_render_svg(self):
        apartment = Apartment.fetch_all('test', 1, 1, 1, 3, 1)[0]
        self.assertEqual('<?xml version="1.0" ?><!DOCTYPE svg  PUBLIC \'-//W3C//DTD SVG 1.1//EN\'  \'http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd\'><svg enable-background="new 0.0 0.0                         770 425" height="425px" version="1.1" viewBox="0.0 0.0 770 425" width="770px" x="0px" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" y="0px">\n    <g id="SELECTION">\n        <polygon fill="none" points="283.055,2.934 283.064,185.447 222.389,185.451 222.391,224.283 166.576,224.287 166.576,351.707          92.552,351.711 92.552,49.543 200.954,49.537 200.954,2.963 282.818,2.963     " style="fill:none"/>\n    </g>\n</svg>', apartment.render_outline())
    ...
    def test_exceptions(self):
        ...
        self.assertRaises(ApartmentError, cairo_floor_plan_svg, 'no_path', 'no_path', apartment, 'red', 1, 1, 'red')
...
class UtilityTest(unittest.TestCase):
    ...
    def test_close_path(self):
        ...
        closed_coords = '158.027,320.265 57.99,320.265 57.99,299.073 7.972,299.073 7.972,238.635 15.948,238.635 15.948,223.536 7.972,223.536 7.972,90.699 54.864,90.699 54.864,183.19 98.318,183.19 98.318,199.141 106.446,199.141 106.446,232.045 158.027,232.045 158.027,320.265'
        closed = close_path(closed_coords)
        self.assertEqual(closed, closed_coords)
        open_coords = '158.027,320.265 57.99,320.265 57.99,299.073 7.972,299.073 7.972,238.635 15.948,238.635 15.948,223.536 7.972,223.536 7.972,90.699 54.864,90.699 54.864,183.19 98.318,183.19 98.318,199.141 106.446,199.141 106.446,232.045 158.027,232.045'
    ...
    def test_process_polygon_node(self):
        ...
        self.assertEqual(points_rows[0], '283.055,2.934 283.064,185.447 222.389,185.451 222.391,224.283 166.576,224.287 166.576,351.707 92.552,351.711 92.552,49.543 200.954,49.537 200.954,2.963 282.818,2.963 283.055,2.934')
        self.assertEqual(points_rows[2], '331.606,229.132 437.192,229.132 437.192,422.078 331.606,422.078 331.606,229.132')
        self.assertEqual('386.111,451.387 386.111,460.85 308.896,460.85 308.896,330.807 466.497,330.807 466.497,451.387 386.111,451.387', points_rows[5])
        self.assertEqual('359.679,148.894 356.887,148.894 356.747,148.894 257.555,148.894 257.555,266.026 313.678,266.026 313.678,249.552 336.673,249.552 353.878,232.338 353.878,229.309 356.957,229.309 356.957,226.516 359.679,226.516 404.424,226.516 404.424,148.894 359.679,148.894', points_rows[6])
