...
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/exx.py
# Compiled at: 2018-04-27 06:35:55
...
class exx(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(exx, self).describe(), {'id': 'exx', 
           'name': 'EXX', 
           'countries': 'CN', 
           'rateLimit': 1000 / 10, 
           'has': {'fetchOrder': True, 
           ...
           'urls': {'logo': 'https://user-images.githubusercontent.com/1294454/37770292-fbf613d0-2de4-11e8-9f79-f2dc451b8ccb.jpg', 
                    'api': {'public': 'https://api.exx.com/data/v1', 
    ...
    def fetch_markets(self):
        ...
        for i in range(0, len(ids)):
            ...
            precision = {'amount': int(market['amountScale']), 
    ...
    def fetch_ticker(self, symbol, params={}):
        ...
        ticker = self.publicGetTicker(self.extend({'currency': market['id']}, params))
    ...
    def fetch_tickers(self, symbols=None, params={}):
        ...
        for i in range(0, len(ids)):
            id = ids[i]
            if id not in list(self.marketsById.keys()):
                continue
    ...
    def fetch_order_book(self, symbol, limit=None, params={}):
        self.load_markets()
        orderbook = self.publicGetDepth(self.extend({'currency': self.market_id(symbol)}, params))
    ...
    def fetch_trades(self, symbol, since=None, limit=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        trades = self.publicGetTrades(self.extend({'currency': market['id']}, params))
    ...
    def parse_order(self, order, market=None):
        ...
        fee = None
        if 'fees' in order:
            fee = {'cost': self.safe_float(order, 'fees'), 'currency': market['quote']}
    ...
    def create_order(self, symbol, type, side, amount, price=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.privateGetOrder(self.extend({'currency': market['id'], 
           'type': side, 
           'price': price, 
    ...
    def cancel_order(self, id, symbol=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        result = self.privateGetCancel(self.extend({'id': id, 
           'currency': market['id']}, params))
        return result

    def fetch_order(self, id, symbol=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        order = self.privateGetGetOrder(self.extend({'id': id, 
           'currency': market['id']}, params))
    ...
    def fetch_open_orders(self, symbol=None, since=None, limit=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        orders = self.privateGetOpenOrders(self.extend({'currency': market['id']}, params))
    ...
    def sign(self, path, api='public', method='GET', params={}, headers=None, body=None):
        ...
        if api == 'public':
            ...
            query = self.urlencode(self.keysort(self.extend({'accesskey': self.apiKey, 
