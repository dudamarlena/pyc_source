...
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/base/exchange.py
# Compiled at: 2018-04-27 06:34:17
__doc__ = 'Base exchange class'
__version__ = '0.0.1'
...
__all__ = [
 'Exchange']
import logging, base64, calendar, collections, datetime
from email.utils import parsedate
import functools, gzip, hashlib, hmac, io, json, math
...
class Exchange(object):
    ...
    logger = None
    userAgent = None
    userAgents = {'chrome': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/62.0.3202.94 Safari/537.36', 
       'chrome39': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.71 Safari/537.36'}
    verbose = False
    markets = None
    symbols = None
    fees = {'trading': {'fee_loaded': False, 
                   'percentage': True}, 
       'funding': {'fee_loaded': False, 
                   'withdraw': {}, 'deposit': {}}}
    ids = None
    currencies = None
    tickers = None
    ...
    currencies = None
    options = None
    requiredCredentials = {'apiKey': True, 
       'secret': True, 
       'uid': False, 
       'login': False, 
       'password': False, 
    ...
    def __init__(self, config={}):
        ...
        self.tokenBucket = self.extend({'refillRate': 1.0 / self.rateLimit, 
           'delay': 1.0, 
           'capacity': 1.0, 
           'defaultCost': 1.0}, getattr(self, 'tokenBucket') if hasattr(self, 'tokenBucket') else {})
    ...
    def safe_string(dictionary, key, default_value=None):
        if key is not None and key in dictionary and dictionary[key] is not None:
    ...
    def jwt(request, secret, algorithm=hashlib.sha256, alg='HS256'):
        header = Exchange.encode(Exchange.json({'alg': alg, 
           'typ': 'JWT'}))
    ...
    def set_markets(self, markets, currencies=None):
        ...
        for i in range(0, len(values)):
            values[i] = self.extend(self.fees['trading'], {'precision': self.precision, 'limits': self.limits}, values[i])
        ...
        if currencies:
            ...
            base_currencies = [ {'id': market['baseId'] if 'baseId' in market else market['base'], 'code': market['base']} for market in values if 'base' in market
                              ]
            quote_currencies = [ {'id': market['quoteId'] if 'quoteId' in market else market['quote'], 'code': market['quote']} for market in values if 'quote' in market
                               ]
    ...
    def parse_bid_ask(self, bidask, price_key=0, amount_key=0):
        return [
         float(bidask[price_key]), float(bidask[amount_key])]
    ...
    def fetch_l2_order_book(self, symbol, limit=None, params={}):
        ...
        return self.extend(orderbook, {'bids': self.sort_by(self.aggregate(orderbook['bids']), 0, True), 
           'asks': self.sort_by(self.aggregate(orderbook['asks']), 0)})

    def parse_order_book(self, orderbook, timestamp=None, bids_key='bids', asks_key='asks', price_key=0, amount_key=1):
        return {'bids': self.sort_by(self.parse_bids_asks(orderbook[bids_key], price_key, amount_key) if bids_key in orderbook and isinstance(orderbook[bids_key], list) else [], 0, True), 
    ...
    def parse_position(self, position, market=None):
        self.raise_error(NotSupported, details='parse_position() is not implemented yet')

    def parse_positions(self, positions, market=None, since=None, limit=None):
        array = self.to_array(positions)
        array = [ self.parse_position(position, market) for position in array ]
        array = self.sort_by(array, 'timestamp')
        symbol = market['symbol'] if market else None
        return self.filter_by_symbol_since_limit(array, symbol, since, limit)
    ...
    def filter_by_array(self, objects, key, values=None, indexed=True):
        ...
        if indexed:
            return self.index_by(result, key)
        else:
            return result
    ...
    def create_position(self, symbol, type, side, amount, price=None, params={}):
        self.raise_error(NotSupported, details='create_position() not implemented yet')

    def create_future_long_position_market(self, symbol, amount, params={}):
        return self.create_position(symbol, 'market', 'buy', amount, None, params)

    def create_future_short_position_market(self, symbol, amount, params={}):
        return self.create_position(symbol, 'market', 'sell', amount, None, params)

    def create_future_long_position_limit(self, symbol, amount, *args):
        return self.create_position(symbol, 'limit', 'buy', amount, *args)

    def create_future_short_position_limit(self, symbol, amount, *args):
        return self.create_position(symbol, 'limit', 'sell', amount, *args)

    def close_position(self, symbol, params={}):
        self.raise_error(NotSupported, details='close_position() not implemented yet')

    def close_future_long_position_market(self, symbol, amount, *args):
        return self.close_position(symbol, 'market', 'sell', amount, None, *args)

    def close_future_short_position_market(self, symbol, amount, *args):
        return self.close_position(symbol, 'market', 'buy', amount, None, *args)

    def close_future_long_position_limit(self, symbol, amount, *args):
        return self.close_position(symbol, 'limit', 'sell', amount, *args)

    def close_future_short_position_limit(self, symbol, amount, params={}):
        return self.close_position(symbol, 'limit', 'buy', amount, params={})
