...
# Embedded file name: /Users/bogdankyryliuk/src/oss/incubator-superset/superset/migrations/versions/bebcf3fed1fe_convert_dashboard_v1_positions.py
# Compiled at: 2020-01-16 13:27:41
# Size of source mod 2**32: 23236 bytes
__doc__ = 'Migrate dashboard position_json data from V1 to V2\n\nRevision ID: bebcf3fed1fe\nRevises: fc480c87706c\nCreate Date: 2018-07-22 11:59:07.025119\n\n'
import collections, json, sys, uuid
...
dashboard_slices = Table('dashboard_slices', Base.metadata, Column('id', Integer, primary_key=True), Column('dashboard_id', Integer, ForeignKey('dashboards.id')), Column('slice_id', Integer, ForeignKey('slices.id')))
...
def get_empty_layout():
    return {DASHBOARD_VERSION_KEY: 'v2', 
     DASHBOARD_ROOT_ID: {'type':DASHBOARD_ROOT_TYPE, 
                         'id':DASHBOARD_ROOT_ID, 
                         'children':[
                          DASHBOARD_GRID_ID]}, 
     
     DASHBOARD_GRID_ID: {'type':DASHBOARD_GRID_TYPE, 
                         'id':DASHBOARD_GRID_ID, 
                         'children':[]}}


def get_header_component(title):
    return {'id':DASHBOARD_HEADER_ID, 
...
def get_chart_holder(position):
    ...
    if code is not None:
        ...
        return {'type':MARKDOWN_TYPE, 
         ...
         'children':[],  'meta':{'width':width, 
          'height':height,  'code':markdown_content}}
    return {'type':CHART_TYPE, 
     'id':'DASHBOARD_CHART_TYPE-{}'.format(generate_id()), 
     'children':[],  'meta':{'width':width, 
      'height':height,  'chartId':int(slice_id)}}
...
def can_reduce_column_width(column_component, root):
    return column_component['type'] == COLUMN_TYPE and column_component['meta']['width'] > GRID_MIN_COLUMN_COUNT and all([is_wide_leaf_component(root[childId]) or  for childId in column_component['children']])
...
def merge_position(position, bottom_line, last_column_start):
    ...
    taller_indexes = [i for i, value in enumerate(bottom_line) if i >= col if value > bottom_line[col]]
    current_row_value = bottom_line[col]
    if len(taller_indexes) > 0:
        if taller_indexes[0] - col + 1 < size_x:
        ...
        while len(next_row):
            ...
            if len(available_columns_index):
                ...
                if len(available_columns_index) > 1:
                    idx = sorted(available_columns_index,
    ...
    for i, dashboard in enumerate(dashboards):
        ...
        if not is_v2_dash(position_json):
            ...
            for slice in slices:
                ...
                if not position:
                    position = {'col':new_slice_counter % NUMBER_OF_CHARTS_PER_ROW * DEFAULT_CHART_WIDTH + 1, 
                     'row':last_row_id + int(new_slice_counter / NUMBER_OF_CHARTS_PER_ROW) * DEFAULT_CHART_WIDTH, 
                ...
                if form_data:
                    if viz_type in ('markup', 'separator'):
