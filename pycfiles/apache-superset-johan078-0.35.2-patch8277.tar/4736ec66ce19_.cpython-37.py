...
def upgrade():
    ...
    for foreign in ('columns', 'metrics'):
        ...
        with op.batch_alter_table(foreign, naming_convention=conv) as (batch_op):
            ...
            for name in names:
                batch_op.drop_constraint((name or ),
                  type_='foreignkey')
    ...
    try:
        with op.batch_alter_table('datasources', naming_convention=conv) as (batch_op):
            batch_op.drop_constraint((generic_find_uq_constraint_name('datasources', {'datasource_name'}, insp) or ),
              type_='unique')
...
def downgrade():
    ...
    with op.batch_alter_table('datasources', naming_convention=conv) as (batch_op):
        batch_op.drop_constraint((generic_find_fk_constraint_name('datasources', {'cluster_name'}, 'clusters', insp) or ),
          type_='foreignkey')
        batch_op.drop_constraint((generic_find_uq_constraint_name('datasources', {'cluster_name', 'datasource_name'}, insp) or ),
          type_='unique')
