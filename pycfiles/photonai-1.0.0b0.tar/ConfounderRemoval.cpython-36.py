...
# [GCC 8.4.0]
# Embedded file name: /home/nwinter/PycharmProjects/photon_projects/photon_core/photonai/modelwrapper/ConfounderRemoval.py
...
class ConfounderRemoval(BaseEstimator, TransformerMixin):
    ...
    def _validate_dimension(self, X, sample_ols_confounder):
        if X.shape[0] != sample_ols_confounder.shape[0]:
            err_msg = 'Number of samples (N=' + str(X.shape[0]) + ') is not the same as number of cases (N=' + str(sample_ols_confounder.shape[0]) + ') in confounder!'
    ...
    def transform(self, X, y=None, **kwargs):
        ...
        for i in range(X.shape[1]):
            ...
            X_new[:, i] = np.asarray(residuum_feature_vector)
