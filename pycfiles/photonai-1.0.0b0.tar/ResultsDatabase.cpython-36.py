...
# [GCC 8.4.0]
# Embedded file name: /home/nwinter/PycharmProjects/photon_projects/photon_core/photonai/investigator/app/main/model/ResultsDatabase.py
# Compiled at: 2019-02-21 07:32:38
# Size of source mod 2**32: 14793 bytes
from pymodm import connect, MongoModel, EmbeddedMongoModel, fields
from pymongo.errors import DocumentTooLarge
from enum import Enum
import numpy as np, pandas as pd, pickle, pprint
...
class MDBHelper:
    ...
    def aggregate_metrics(folds, metrics):
        ...
        for metric_item in metrics:
            for op in operations:
                value_list_train = [fold.training.metrics[metric_item] for fold in folds if fold.training is not None if metric_item in fold.training.metrics]
                if value_list_train:
                    metrics_train.append(MDBFoldMetric(operation=op, metric_name=metric_item, value=(calculate_single_metric(op, value_list_train))))
                value_list_validation = [fold.validation.metrics[metric_item] for fold in folds if fold.validation is not None if metric_item in fold.validation.metrics]
                if value_list_validation:
                    metrics_test.append(MDBFoldMetric(operation=op, metric_name=metric_item, value=(calculate_single_metric(op, value_list_validation))))
    ...
    def get_metric(config_item, operation, metric, train=True):
        if train:
            metric = [item.value for item in config_item.metrics_train if item.operation == str(operation) if item.metric_name == metric if not config_item.config_failed]
        else:
            metric = [item.value for item in config_item.metrics_test if item.operation == str(operation) if item.metric_name == metric if not config_item.config_failed]
        if len(metric) == 1:
            return metric[0]
        else:
            return metric
...
class MongoDBWriter:
    ...
    def write_predictions_file(self, result_tree):
        if self.save_settings.save_predictions or self.save_settings.save_best_config_predictions:
            ...
            save_df = pd.DataFrame(data={'indices':indices_array,  'fold':fold_nr_array,  'y_pred':y_pred_array, 
             'y_true':y_true_array})
    ...
    def print_outer_fold(self, outer_fold):
        ...
        if outer_fold.best_config is not None:
            outer_fold_text.append('\n-------------------------------------------------------------------\nOUTER FOLD {}\n-------------------------------------------------------------------\nBest Config:\n{}\n\nNumber of samples training {}\nNumber of samples test {}\n            \n            '.format(outer_fold.fold_nr, pp.pformat(outer_fold.best_config.human_readable_config), outer_fold.best_config.inner_folds[0].number_samples_training, outer_fold.best_config.inner_folds[0].number_samples_validation))
