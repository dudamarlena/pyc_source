...
# Size of source mod 2**32: 10476 bytes
import numpy as np
...
import model.BestConfigPlot as BestConfigPlot
import model.BestConfigTrace as BestConfigTrace
...
def show_pipeline(storage, name):
    try:
        ...
        return render_template('outer_folds/index.html', pipe=pipe,
...
def create_performance_overview_plots_new(pipe):
    ...
    for metric in metrics:
        overview_plot = PlotlyPlot(('overview_plot_' + metric), (metric.replace('_', ' ')), show_legend=False, margin={'r':30, 
         'l':30})
        for fold in pipe.outer_folds:
            overview_plot_train_trace = PlotlyTrace(('train_fold_' + str(fold.fold_nr)), trace_color='train_color_bold', trace_size=10)
            overview_plot_test_trace = PlotlyTrace(('test_fold_' + str(fold.fold_nr)), trace_color='alternative_test_color_bold', trace_size=10)
            if fold.best_config:
        ...
        test_dummy_trace = PlotlyTrace('dummy_test', trace_size=4, trace_color='dummy_color', trace_type='bar', width=0.1,
          opacity=0)
        for dummy_test in pipe.dummy_estimator.train:
            if dummy_test.metric_name == metric and dummy_test.operation == 'FoldOperations.MEAN':
                test_dummy_trace.add_x('dummy')
                test_dummy_trace.add_y(dummy_test.value)

        for metric_train in pipe.metrics_train:
            if metric_train.metric_name == metric and metric_train.operation == 'FoldOperations.MEAN':
                training_mean_trace.add_x('train')
                training_mean_trace.add_y(metric_train.value)

        for metric_test in pipe.metrics_test:
            if metric_test.metric_name == metric and metric_test.operation == 'FoldOperations.MEAN':
...
def create_prediction_plot(handler):
    ...
    return (
     predictions_plot_train, predictions_plot_test)