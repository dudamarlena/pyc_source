...
# Compiled at: 2019-09-02 10:37:57
# Size of source mod 2**32: 27072 bytes
from sklearn.utils.metaestimators import _BaseComposition
import numpy as np, datetime, os, pickle, uuid, shutil
...
class PhotonPipeline(_BaseComposition):
                ...
                if not self._final_estimator.is_estimator:
                    if self.caching:
                        if self.current_config is not None:
                            X, y, kwargs = self.load_or_save_cached_data((self._final_estimator.name), X, y, kwargs, (self._final_estimator), initial_X=initial_X)
                    else:
    ...
    def load_or_save_cached_data(self, name, X, y, kwargs, transformer, fit=False, needed_for_further_computation=False, initial_X=None):
        if self.single_subject_caching or self.skip_loading:
            if not needed_for_further_computation:
            ...
            if not self.skip_loading or needed_for_further_computation:
                processed_X, processed_y, processed_kwargs = PHOTONDataHelper.resort_splitted_data(processed_X, processed_y, processed_kwargs, PHOTONDataHelper.stack_results(list_of_idx_non_cached, list_of_idx_cached))
    ...
    def _caching_fit_transform(self, X, y, kwargs, fit=False):
        ...
        for num, (name, transformer) in enumerate(self.elements[:-1]):
            if not self.caching or self.current_config is None or hasattr(transformer, 'skip_caching') and transformer.skip_caching:
                X, y, kwargs = (self._do_timed_fit_transform)(name, transformer, fit, X, y, **kwargs)
            else:
                if self.cache_man.check_cache(name):
                    ...
                    if num + 1 == num_steps:
                        if not self.skip_loading:
                            X, y, kwargs = self.load_or_save_cached_data(last_cached_item, X, y, kwargs, transformer, fit, initial_X=initial_X)
                else:
