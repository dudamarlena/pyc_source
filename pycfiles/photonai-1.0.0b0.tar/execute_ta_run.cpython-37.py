...
# [GCC 8.4.0]
# Embedded file name: /home/nwinter/PycharmProjects/photon_projects/photon_core/photonai/optimization/smac/execute_ta_run.py
# Compiled at: 2019-10-11 10:34:00
...
class MyExecuteTARun(ExecuteTARun):
    ...
    def start(self, config: Configuration, instance: str, cutoff: float=None, seed: int=12345, instance_specific: str='0', capped: bool=False):
        ...
        """
        if self.runhistory.config_ids.get(config):
            if self.runhistory.get_runs_for_config(config):
                return (None, None, 1, None)
        raise NotImplementedError('PHOTONs seed management is not implemented yet. At this juncture we can not run a config multiple times correctly.If your minR=maxR==1 and this error occurred some other things went wrong.Please contact us.')