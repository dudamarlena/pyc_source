...
# Compiled at: 2019-10-17 07:03:26
...
def plot_scatter(folds, name, title, colormap='nipy_spectral', trace_color=None):
    ...
    for y_true, y_pred in folds:
        c = color[(fold_nr - 1)]
        if trace_color:
            validation_trace = PlotlyTrace(('Fold {}'.format(fold_nr)), 'markers', 'scatter', trace_size=7, trace_color=trace_color)
        else:
        ...
        if trace_color:
            regr_trace = PlotlyTrace(('r={0:.2f}'.format(r)), 'lines', 'scatter', trace_color=trace_color)
        else:
