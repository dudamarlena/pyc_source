...
# Compiled at: 2019-09-02 12:22:05
...
class PhotonBaseConstraint:
    ...
    def __init__(self, strategy='first', metric='', threshold: float=None, margin: float=0, **kwargs):
        ...
        try:
            ...
            raise ValueError('Your strategy: ' + str(strategy) + ' is not supported yet. Please use one of ' + str([x.name for x in PhotonBaseConstraint.ENUM_STRATEGY]))
...
class MinimumPerformance(PhotonBaseConstraint):
    ...
    def __init__(self, metric='', threshold=1, strategy='first'):
...
class DummyPerformance(PhotonBaseConstraint):
    ...
    def __init__(self, metric='', margin=1.0, strategy='first'):
