...
# [GCC 8.4.0]
# Embedded file name: /home/nwinter/PycharmProjects/photon_projects/photon_core/photonai/neuro/brain_atlas.py
# Compiled at: 2019-10-31 13:44:55
# Size of source mod 2**32: 22598 bytes
import glob, inspect, time
from os import path
...
class BrainMask(BaseEstimator):
    ...
    def transform(self, X, y=None, **kwargs):
        if self.affine is None or self.shape is None:
            self.affine, self.shape = BrainMask.get_format_info_from_first_image(X)
        elif isinstance(self.mask_image, str):
