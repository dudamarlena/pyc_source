...
# [GCC 8.4.0]
# Embedded file name: /home/nwinter/PycharmProjects/photon_projects/photon_core/photonai/base/PhotonFolds.py
# Compiled at: 2019-09-02 11:43:57
...
import datetime, numpy as np, uuid
...
class FoldInfo:
    ...
    @staticmethod
    def generate_folds(cv_strategy, X, y=None, groups=None, eval_final_performance=True, test_size=0.2):
        """
...
class OuterFoldManager:
    ...
    def _generate_inner_folds(self):
        self.inner_folds = FoldInfo.generate_folds(self.cross_validaton_info.inner_cv, self._validation_X, self._validation_y, self._validation_group)
    ...
    def fit(self, X, y=None, groups=None, **kwargs):
        ...
        for current_config in self.optimizer.ask:
            ...
            hp = TestPipeline(pipe_ctor, current_config, (self.optimization_info),
        ...
        if tested_config_counter > 0:
            ...
            if self.cross_validaton_info.eval_final_performance:
                ...
                test_score_mdb = (TestPipeline.score)(optimum_pipe, self._test_X, self._test_y, indices=self.cross_validaton_info.outer_folds[self.outer_fold_id].test_indices, 
                ...
                train_score_mdb = (TestPipeline.score)(optimum_pipe, self._validation_X, self._validation_y, indices=self.cross_validaton_info.outer_folds[self.outer_fold_id].train_indices, 
                 metrics=self.optimization_info.metrics, 
                 save_predictions=self.save_best_config_predictions, 
