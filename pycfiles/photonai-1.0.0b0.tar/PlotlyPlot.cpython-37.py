...
# [GCC 8.4.0]
# Embedded file name: /home/nwinter/PycharmProjects/photon_projects/photon_core/photonai/investigator/app/main/model/PlotlyPlot.py
# Compiled at: 2019-11-11 15:14:39
# Size of source mod 2**32: 6754 bytes
import model.PlotlyTrace as PlotlyTrace

class PlotlyPlot:
    __doc__ = ' Class which prepares given data to plot in views\n    author: Julian Gebker\n    version: 1.0.0\n    '

    def __init__(self, plot_name: str, title: str, traces: list=None, show_legend: bool=True, xlabel: str='', ylabel: str='', regression_line: bool=False, margin=None):
        """ Constructor
...
class PiePlotlyPlot(PlotlyPlot):
    ...
    def __init__(self, plot_name, title, traces=None, show_legend=True):
        super(PiePlotlyPlot, self).__init__(plot_name=plot_name, title=title, traces=traces, show_legend=show_legend)
        self.result_dict = {'layout':{'title':'Time Monitor Pie Chart',  'showlegend':True, 
          'annotations':[]}, 
         'data':[]}

    def append(self, labels, values, name, colors, domain):
        self.result_dict.append({'labels':labels,  'values':values, 
         'type':'pie', 
         'name':name, 
        ...
        self.result_dict['layout']['annotations'].append({'x':0.225, 
         'y':1, 
         'font':{'size': 16}, 
         'text':'Plot 1', 
         'xref':'paper', 
