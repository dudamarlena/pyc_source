...
# [GCC 8.4.0]
# Embedded file name: /home/nwinter/PycharmProjects/photon_projects/photon_core/photonai/optimization/smac/smac_new.py
...
class SMACOptimizer(PhotonBaseOptimizer):
    ...
    def __init__(self, run_obj='quality', wallclock_limit: float=float('inf'), min_r=1, max_r=1, run_limit=40):
        if __found__:
    ...
    def tell(self, config, performance):
        ...
        self.optimizer.runhistory.add(config=config, cost=(performance[1]), time=0, status=(StatusType.SUCCESS), instance_id='',
          seed=5)
        self.runtime += 1
