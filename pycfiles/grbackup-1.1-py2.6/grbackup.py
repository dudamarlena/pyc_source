...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/grbackup/grbackup.py
# Compiled at: 2013-06-09 14:08:25
import sys, imp, os, logging, getpass
...
def get_plugins():
    ...
    """
    plugins_folder = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'grb_plugins')
    plugins = {}
...
def check_params(options):
    if options.cmd_list and options.cmd_backup or not (options.cmd_list or options.cmd_backup):
...
def make_usage(plugins):
    ...
    for (name, m) in plugins.items():
        ...
        for (i, col2) in enumerate(descr):
            ...
            lines.append(formatted_str.format(col1, col2, alignment='<', length=maxlen))
...
def get_params(plugins):
    ...
    auth_group.add_option('-e', '--email', dest='email', help='gmail account')
    auth_group.add_option('-p', '--password', dest='pwd', help='account password')
    parser.add_option_group(auth_group)
    cmd_group = OptionGroup(parser, 'Command Options')
    cmd_group.add_option('-b', '--backup', dest='cmd_backup', default=False, action='store_true', help='backup items')
    cmd_group.add_option('-l', '--list', dest='cmd_list', default=False, action='store_true', help='list items')
    parser.add_option_group(cmd_group)
    scope_group = OptionGroup(parser, 'Scope Options')
    scope_group.add_option('-a', '--all', dest='scope_all', default=False, action='store_true', help='processing all items')
    scope_group.add_option('-s', '--subscriptions', dest='scope_subs', action='store_true', default=False, help='processing subscriptions only')
    scope_group.add_option('-t', '--topics', dest='scope_topics', default=False, action='store_true', help='processing topics only')
    scope_group.add_option('-x', '--starred', dest='scope_starred', default=False, action='store_true', help='processing starred topics only')
    ...
    other_group.add_option('-w', '--workers', dest='workers', default=1, type=int, help='number of workers [default: %default]')
    other_group.add_option('-o', '--output', dest='output', default='simple://', help='output uri')
    other_group.add_option('-n', '--count', dest='count', default=200, help='the number of topics that can be read at once [default: %default]')
    other_group.add_option('-c', '--coding', dest='coding', default='utf8', help='output coding [default: %default]')
    other_group.add_option('-v', '--verbose', action='store_true', dest='verbose', default=False, help='verbose output')
...
def main(options, args, plugins):
    ...
    with plugin_output.writer(options) as (plugin_writer):
        if options.scope_subs:
            ...
            pool.add_task(map, lambda item: plugin_writer.put_starred(item), g.starred(options.count))
            for subscription in g.subscriptions:
                pool.add_task(fetch_subscription, g, plugin_writer, subscription, options)
