...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/grbackup/grb_plugins/mongodb_output.py
# Compiled at: 2013-06-09 14:07:41
...
def add_option_group(parser):
    ...
    mongodb_group.add_option('--mongodb-db', dest='mongodb_db', type='str', default='greader', help='the name of the database[default: %default]')
    mongodb_group.add_option('--mongodb-scol', dest='mongodb_subscriptions', default='subscriptions', type='str', help='collection name for subscriptions[default: %default]')
    mongodb_group.add_option('--mongodb-tcol', dest='mongodb_topics', default='topics', type='str', help='collection name for topics[default: %default]')
    mongodb_group.add_option('--mongodb-tstar', dest='mongodb_starred', default='starred', type='str', help='collection name for starred items[default: %default]')
    mongodb_group.add_option('--mongodb-w', dest='mongodb_w', default=1, type=int, help='<int> Write operations will block until they have been replicated to the specified number [default: %default]')
    mongodb_group.add_option('--mongodb-j', dest='mongodb_j', default=False, action='store_true', help='block until write operations have been committed to the journal [default: %default]')
    parser.add_option_group(mongodb_group)


class WriteMongoDB(object):

    def __init__(self, uri, collection_subsr, collection_topics, collection_starred, w, j, db=None):
        self.uri = uri
...
class writer(object):
    ...
    def __enter__(self):
        return WriteMongoDB(self._output, self.subsr, self.topics, self.starred, self.w, self.j, db=self.db)
