...
# Embedded file name: build\bdist.win32\egg\chirp\compare\ccompare.py
...
def run_comparisons(storager, comparator, shm_dict, shm_manager, consumer, nworkers=1, cout=None):
    """
...
def main(argv=None, cout=None):
    ...
    if method is None:
        ...
        if do_test:
            return run_test(signal_dir, comparator, cout=cout)
        try:
            ...
            storager = storage_class(comparator, location=(sparts[1] if len(sparts) > 1 else None), signals=signal_dir, **store_options)
        except ImportError as e:
