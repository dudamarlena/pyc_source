...
# Embedded file name: build\bdist.win32\egg\chirp\common\plg.py
...
class pitchtrace(object):
    ...
    def __init__(self, tgrid, pmmse, pvar, **kwargs):
        ...
        """
        self.tgrid = tgrid
        if self.nframes != pmmse.shape[0]:
            raise ValueError("Dimensions of MMSE estimate don't match time grid")
        if self.nframes != pvar.shape[0]:
            raise ValueError("Dimensions of variance estimate don't match time grid")
        if pmmse.shape[1] != pvar.shape[1]:
            raise ValueError("Number of chains doesn't match for mean and variance")
        self.pmmse = pmmse
        self.pvar = pvar
        self.estimates = dict()
        for k, v in kwargs.items():
            if v is None:
                continue
            if self.nframes != v.shape[0]:
                raise ValueError("Dimensions of %s don't match time grid" % k)
    ...
    @property
    def nframes(self):
        return self.tgrid.size

    @property
    def nchains(self):
        return self.pmmse.shape[1]
    ...
    def write(self, cout):
        ...
        fmt = [
         '%6.2f'] + [ _fmt(v) for k, v in rec.dtype.descr[1:] ]
    ...
    with open(filename, 'rt') as (fp):
        for line in fp:
            ...
            if m:
                ...
                values = (
                 current_element,) + tuple(float(x) for x in line.split())
