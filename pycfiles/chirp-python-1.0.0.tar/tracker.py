...
# Embedded file name: build\bdist.win32\egg\chirp\pitch\tracker.py
...
class tracker(_configurable):
    ...
    """
    options = dict(nfft=512, shift=30, winsize=401, tfr_order=5, tfr_tm=6.0, tfr_flock=0.01, tfr_tlock=5, freq_range=(0.01,
                                                                                                                      0.4), pitch_range=(0.02,
                                                                                                                                         0.25), lobes=7, lobe_decay=0.85, neg_ampl=0.35, neg_width=9, remask_likelihood=True, max_jump=80, particles=500, pow_thresh=10000.0, row_thresh=0.02, rwalk_scale=2, chains=5, btrace=False, min_loglike=-100)
    ...
    def track(self, signal, mask=None, cout=None, raw=False, **kwargs):
        ...
        if signal.ndim == 1:
            ...
            if options['remask_likelihood'] and mask is not None:
                ...
                rows = (self.template.fgrid >= self.template.pgrid[0]) & (self.template.fgrid <= self.template.pgrid[(-1)])
            ...
            for chain in xrange(chains):
                if cout:
                    cout.write('+ Chain %d: forward particle filter\n' % chain)
                ...
                if btrace:
                    if cout:
                        cout.write('+ Chain %d: reverse Vitterbi filter\n' % chain)
        ...
        return (
         starttime, pitch_mmse, pitch_var, pitch_map, {'stim.pow': nandecibels(specpow)})
...
def cpitch(argv=None, cout=None, cerr=None, **kwargs):
    ...
    if len(args) < 1:
        ...
        pcm = fp.read()
        samplerate = fp.sampling_rate / 1000.0
        ...
        if maskfile is not None and os.path.exists(maskfile):
            ...
            for startcol, mspec, imask in mask.split(spec, elems, tgrid, fgrid, cout=cout):
                try:
                    ...
                    ptrace = pitchtrace(T, (pitch_mmse * samplerate), (pitch_var * samplerate * samplerate), **stats)
