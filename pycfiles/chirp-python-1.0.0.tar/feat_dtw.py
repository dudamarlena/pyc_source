...
# Embedded file name: build\bdist.win32\egg\chirp\compare\feat_dtw.py
...
class feat_dtw(base_comparison, _configurable):
    ...
    """
    options = dict(cost_matrix=[(1, 1, 1), (1, 2, 2), (2, 1, 2)], metric='euclidean', dynamic_cost=True)
    config_sections = ('dtw', )
    ...
    def compare(self, ref, tgt):
        ...
        return (
         pathlen(p, q), dcost, dcost / min(ref.size, tgt.size))
...
metrics = {'euclidean': dist_euclidean, 'cosine': dist_cos}
...
def dtw(M, C=None):
    ...
    return (
     nx.asarray(p[::-1], dtype='i'), nx.asarray(q[::-1], dtype='i'), D)
