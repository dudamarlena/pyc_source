...
# Embedded file name: build\bdist.win32\egg\chirp\gui\wxgeom.py
...
def poly_to_path(poly, **kwargs):
    ...
    vertices = concatenate([asarray(poly.exterior)] + [ asarray(r) for r in poly.interiors ])
    codes = concatenate([ring_coding(poly.exterior)] + [ ring_coding(r) for r in poly.interiors ])
