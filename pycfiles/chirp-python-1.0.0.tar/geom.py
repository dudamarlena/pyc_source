...
# Embedded file name: build\bdist.win32\egg\chirp\common\geom.py
...
class elementlist(list):
    ...
    def range(self):
        ...
        if len(self) == 0:
            ...
            return (
             min([ x[0] for x in self.intervals ] + [ x.bounds[0] for x in self.polys ]),
    ...
    def element_type(el):
        ...
        if isinstance(el, geometry.Polygon):
            return 'poly'
        if isinstance(el, geometry.base.BaseGeometry):
            return 'geom'
...
def split_polygons(p1, p2):
    return (
     p1.difference(p2), p2.difference(p1), p1.intersection(p2))
