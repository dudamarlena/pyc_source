...
# Embedded file name: build\bdist.win32\egg\chirp\gui\chirpgui.py
# Compiled at: 2013-12-18 14:52:48
__doc__ = '\nDefine spectrographic elements of acoustic signals\n\n'
from __future__ import division
import os, sys, ewave, wx
...
class SpecPicker(SpecViewer, DrawMask, PitchOverlayMixin):
    ...
    def keyhandler(self, event):
        ...
        if key == 's':
            ...
            if not isinstance(val, geom.Polygon):
                return
            newgeoms = []
...
class ChirpGui(wx.Frame):
    ...
    def create_spec_controls(self, parent):
        ...
        controls.Bind(wx.EVT_TEXT_ENTER, self.OnSpecControl)
    ...
    def load_file(self, fname):
        ...
        if file_type == _el_ext:
            ...
            if el:
                self.status.SetStatusText('Loaded %d elements from %s' % (len(el), fname))
        elif file_type == _pitch_ext:
            try:
                self.spec.plot_plg(fname)
                self.status.SetStatusText('Loaded pitch data from %s' % fname)
            except Exception as e:
                dlg = wx.MessageDialog(self, str(e), ('Error reading pitch data from {}').format(fname), style=wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
            ...
            try:
                ...
                dlg = wx.MessageDialog(self, str(e), ('Error reading {}').format(fname), style=wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                return
    ...
    def on_open(self, event):
        fdlg = wx.FileDialog(self, 'Select a file to open', wildcard='WAV files (*.wav)|*.wav|Element files (*.ebl)|*.ebl|Pitch files (*.plg)|*.plg|Configuration files (*.cfg)|*.cfg', style=wx.FD_OPEN | wx.FD_FILE_MUST_EXIST)
        val = fdlg.ShowModal()
        if val == wx.ID_OK:
    ...
    def on_save_params(self, event):
        fdlg = wx.FileDialog(self, 'Choose file to save configuration to', wildcard='Config files (*.cfg)|*.cfg', style=wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT)
        val = fdlg.ShowModal()
        if not val == wx.ID_OK:
            return
        outfile = os.path.join(fdlg.GetDirectory(), fdlg.GetFilename())
        try:
            self.configfile.update('spectrogram', window_len=float(self.win_size.GetValue()), window_shift=float(self.shift_size.GetValue()), dynrange=float(self.dynrange.GetValue()), spec_method=self.spec_method.GetValue(), colormap=self.cmap.GetValue())
    ...
    def on_calc_pitch(self, event):
        ...
        if len(elems) < 1:
            elems = self.spec.selections
        if len(elems) < 1:
            elems = None
        else:
            elems = wxgeom.patches_to_elist(elems)
        try:
            try:
...
def main(argv=None):
    ...
    p.add_argument('-c', '--config', default=os.path.join(os.getcwd(), 'chirp.cfg'), help='specify an alternative config file to use (default=%(default)s')
