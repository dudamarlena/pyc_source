...
# [GCC 8.4.0]
# Embedded file name: /home/hoang/Desktop/vn_core_nlp/vn_core_nlp/semantic_role_labeling/vnSRL-1.0.0/lib.py
# Compiled at: 2016-05-17 03:08:38
"""
...
def importWordEmbedding():
    ...
    return (
     wordEmbeddingGlove, wordEmbeddingSkipGram)
...
def isPhraseType(ss):
    ...
    """
    specialTag = [
     'MDP', 'UCP', 'WHAP', 'WHNP', 'WHPP', 'WHRP', 'WHVP', 'WHXP']
...
def readTestData(input_dir):
    """Read data from file and store in a list
    """
    f = codecs.open(input_dir, 'r', 'utf-8')
    corpus = []
    for sent in f.readlines():
        ...
        sentNew = filter(lambda a: a != 'vietsrl', sentNew)
...
def getPredicate(listTree):
    ...
    for tree in listTree:
        listPredicateV = tree.search_nodes(name='V-H')
        listPredicateA = tree.search_nodes(name='A-H')
...
def getVoice(tree):
    ...
    """
    if len(tree.search_nodes(word='bị')) > 0:
        for node in tree.search_nodes(word='bị'):
            if node.name == 'V-H':
                for sister in node.get_sisters():
                    if sister.name == 'SBAR':
                        return 0

    if len(tree.search_nodes(word='được')) > 0:
        for node in tree.search_nodes(word='được'):
            if node.name == 'V-H':
                for sister in node.get_sisters():
                    if sister.name == 'SBAR':
                        return 0
...
def ilpSolving(densityMatrix, predicateType):
    ...
    if predicateType != 'V' and predicateType != 'VP' and predicateType != 'Vb':
        tempDict3 = []
        for i in range(shape[1]):
...
def output2File(listLabel, listPredicate, listChunkVer, corpus, listCount, listWord, leLabel, output_dir):
    """Writing output of system to file
    """
    f = codecs.open(output_dir, 'w', 'utf-8')
    count1 = 0
    none = leLabel.transform(['None'])[0]
    for i in range(len(listChunkVer)):
