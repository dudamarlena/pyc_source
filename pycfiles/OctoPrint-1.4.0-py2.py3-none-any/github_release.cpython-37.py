...
# [GCC 8.4.0]
# Embedded file name: C:\Devel\OctoPrint\OctoPrint\src\octoprint\plugins\softwareupdate\version_checks\github_release.py
# Compiled at: 2020-02-26 04:08:42
# Size of source mod 2**32: 13184 bytes
...
def _filter_out_latest(releases, sort_key=None, include_prerelease=False, commitish=None):
    ...
    if sort_key is None:
        ...
        if include_prerelease:
            if commitish:
                filter_function = lambda rel: not rel['draft'] and (not rel['prerelease'] or rel['target_commitish'] in commitish)
            else:
                filter_function = lambda rel: not rel['draft']
        releases = list(filter(filter_function, releases))
        return releases or nothing
...
def _get_latest_release(user, repo, compare_type, include_prerelease=False, commitish=None, force_base=True):
    ...
    releases = r.json()
    required_fields = {
     'name', 'tag_name', 'html_url', 'draft', 'prerelease', 'published_at', 'target_commitish'}
    releases = list(filter(lambda rel: set(rel.keys()) & required_fields == required_fields, releases))
    comparable_factory = _get_comparable_factory(compare_type, force_base=force_base)
    sort_key = lambda release: comparable_factory(_get_sanitized_version(release['tag_name']))
    return _filter_out_latest(releases, sort_key=sort_key,
...
def _get_sanitized_compare_type(compare_type, custom=None):
    if (compare_type not in ('python', 'python_unequal', 'semantic', 'semantic_unequal',
                             'unequal', 'custom') or compare_type) == 'custom':
        if custom is None:
            compare_type = 'python'
    return compare_type
...
def _get_comparable_factory(compare_type, force_base=True):
    ...
    if compare_type in ('semantic', 'semantic_unequal'):
...
def _is_current(release_information, compare_type, custom=None, force_base=True):
    ...
    try:
        return comparator(comparable_factory(sanitized_local), comparable_factory(sanitized_remote))
    except Exception:
...
def get_latest(target, check, custom_compare=None, online=True):
    ...
    information = dict(local=dict(name=current, value=current),
    ...
    if prerelease_channel:
        ...
        compare_type = _get_sanitized_compare_type((check.get('release_compare', 'python')), custom=custom_compare)
        remote_name, remote_tag, release_notes = _get_latest_release((check['user']), (check['repo']),
          compare_type,
          include_prerelease=include_prerelease,
          commitish=commitish,
    ...
    return (
     information,
     _is_current(information, compare_type,
