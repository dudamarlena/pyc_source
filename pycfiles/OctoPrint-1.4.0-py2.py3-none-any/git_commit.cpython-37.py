...
# [GCC 8.4.0]
# Embedded file name: C:\Devel\OctoPrint\OctoPrint\src\octoprint\plugins\softwareupdate\version_checks\git_commit.py
...
import errno, subprocess, sys, logging
...
def get_latest(target, check, online=True):
    ...
    if checkout_folder is None:
        ...
        if returncode != 0:
            return (None, True)
        information = dict(local=dict(name=('Commit %s' % local_commit), value=local_commit),
