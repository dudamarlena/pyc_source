...
# [GCC 8.4.0]
# Embedded file name: /Users/cmusselle/Mango/Workspace/masonry/package/src/masonry/main.py
    ...
    if args['init']:
        ...
        if not args['PROJECT']:
            if project_templates:
                ...
                raise ValueError('No known projects to choose from. Add one using the PROJECT argument.')
        else:
            project_dir, template = parse_project_argument(args['PROJECT'])
        output_dir = Path(args['--output']).resolve()
        initialise_project(project=project_dir,
          template=template,
        ...
        if args['add']:
            ...
            add_template(templates=(args['TEMPLATE']), project_dir=(args['--output']))
        elif args['check']:
            template = None
            if not args['PROJECT']:
                if project_templates:
                ...
                project_dir, template = parse_project_argument(args['PROJECT'])
            check_templates(template_collection_path=template_collection_path, template=template)
