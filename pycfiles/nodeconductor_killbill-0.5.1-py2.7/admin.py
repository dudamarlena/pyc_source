...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_killbill/admin.py
...
class InvoiceAdmin(admin.ModelAdmin):
    ...
    def get_urls(self):
        my_urls = patterns('', url('^sync/$', self.admin_site.admin_view(self.sync)), url('^move_date/$', self.admin_site.admin_view(self.move_date)))
        ...
        for model in PayableMixin.get_all_models():
            for resource in model.objects.all():
                try:
                    ...
                    self.message_user(request, "Can't post usage for %s: %s" % (resource, e), level=messages.ERROR)
