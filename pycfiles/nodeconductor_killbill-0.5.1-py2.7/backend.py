...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_killbill/backend.py
# Compiled at: 2016-09-25 09:47:37
import re, json, requests, urlparse, logging
...
class KillBillBackend(object):
    ...
    def __getattr__(self, name):
        try:
            ...
            raise KillBillError("Method '%s' is not implemented for class '%s'" % (name, self.api.__class__.__name__))
    ...
    def get_or_create_client(self):
        try:
            ...
            self.customer.billing_backend_id = self.api.add_client(email='%s@example.com' % self.customer.uuid, name=self.customer.name, uuid=self.customer.uuid.hex)
            self.customer.save(update_fields=['billing_backend_id'])
    ...
    def sync_invoices(self):
        ...
        for invoice in self.api.get_invoices(client_id):
            ...
            if cur_invoice:
                ...
                cur_invoice = self.customer.killbill_invoices.create(backend_id=invoice['backend_id'], date=invoice['date'], amount=invoice['amount'])
...
class KillBillAPI(object):
    ...
    def __init__(self, api_url=None, username=None, password=None, api_key=None, api_secret=None, **kwargs):
        if not all((api_url, api_key, api_secret)):
            raise KillBillError('Missed billing credentials. They must be supplied explicitly or defined within settings.NODECONDUCTOR_KILLBILL.BACKEND')
        self.currency = kwargs.get('currency', 'USD')
        self.credentials = dict(api_url=api_url, api_key=api_key, api_secret=api_secret, auth=(
         username, password))
    ...
    def _parse_invoice_data(self, raw_invoice):
        ...
        invoice = dict(backend_id=raw_invoice['invoiceId'], date=self._parse_date(raw_invoice['invoiceDate']), due_date=target_date + timedelta(hours=hours_in_month()), end_date=target_date, start_date=target_date - timedelta(hours=hours_in_month()), invoice_number=raw_invoice['invoiceNumber'], currency=raw_invoice['currency'], amount=raw_invoice['amount'], items=[])
        for item in raw_invoice['items']:
            if item['amount']:
                ...
                if not fields:
                    ...
                    continue
                invoice['items'].append(dict(backend_id=item['invoiceItemId'], name=item['usageName'] or item['description'], service=fields['service_name'], project=fields['project_name'], resource=fields['resource_name'], currency=item['currency'], amount=item['amount']))
    ...
    def add_client(self, email=None, name=None, uuid=None, **kwargs):
        account = self.accounts.create(name=name, email=email, externalKey=uuid, currency=self.currency)
    ...
    def add_subscription(self, client_id, resource):
        try:
            ...
            self.update_subscription_fields(subscription_id, resource_name=resource.full_name, project_name=resource.project.full_name, service_name=resource.service_project_link.service.full_name)
        ...
        subscription = self.subscriptions.create(productName=product_name, productCategory='STANDALONE', accountId=client_id, externalKey=resource.uuid.hex, billingPeriod='MONTHLY', priceList='DEFAULT')
        self.set_subscription_fields(subscription['subscriptionId'], resource_name=resource.full_name, project_name=resource.project.full_name, service_name=resource.service_project_link.service.full_name)
    ...
    def add_usage(self, subscription_id, usage_data):
        ...
        self.usages.create(subscriptionId=subscription_id, unitUsageRecords=[ {'unitType': unit, 'usageRecords': [{'recordDate': today, 'amount': str(amount)}]} for unit, amount in usage_data.items()
                                                                            ])

    def get_dry_invoice(self, client_id, subscription_id):
        subscription = self.subscriptions.get(subscription_id)
        data = self.invoices.request('invoices/dryRun', method='POST', accountId=client_id, targetDate=subscription['chargedThroughDate'])
        total = 0
        items = []
        for item in data['items']:
            if item['amount'] and item['subscriptionId'] == subscription_id:
                total += item['amount']
                items.append({'name': item['description'].replace(' (usage item)', ''), 
                   'amount': item['amount']})
    ...
    def set_subscription_fields(self, subscription_id, **data):
        ...
        self.subscriptions._object_query(subscription_id, 'customFields', method='POST', data=json.dumps(fields))
    ...
    def update_subscription_fields(self, subscription_id, **data):
        try:
            ...
            self.subscriptions._object_query(subscription_id, 'customFields', method='DELETE', customFieldList=flist)
    ...
    def propagate_pricelist(self):
        ...
        for cid in priceitems:
            ...
            for priceitem in DefaultPriceListItem.objects.filter(resource_content_type=cid):
                usage_name = re.sub('[\\s:;,+%&$@/]+', '', ('{}-{}-{}').format(priceitem.item_type, priceitem.key, cid))
                unit_name = UNIT_PREFIX + usage_name
                usage = E.usage(E.billingPeriod('MONTHLY'), E.tiers(E.tier(E.blocks(E.tieredBlock(E.unit(unit_name), E.size('1'), E.prices(E.price(E.currency(self.currency), E.value(str(priceitem.value / 60)))), E.max('44640'))))), name=usage_name, billingMode='IN_ARREAR', usageType='CONSUMABLE')
                usages.append(usage)
                units.add(unit_name)
                priceitem.units = unit_name
                priceitem.save(update_fields=['units'])

            plan = E.plan(E.product(product_name), E.finalPhase(E.duration(E.unit('UNLIMITED')), E.recurring(E.billingPeriod('MONTHLY'), E.recurringPrice(E.price(E.currency(self.currency), E.value('0')))), usages, type='EVERGREEN'), name=plan_name)
            prods.append(E.product(E.category('STANDALONE'), name=product_name))
            plans.append(plan)
            plannames.append(plan_name)

        xsi = 'http://www.w3.org/2001/XMLSchema-instance'
        catalog = ElementMaker(nsmap={'xsi': xsi}).catalog(E.effectiveDate(datetime.utcnow().isoformat('T')), E.catalogName('NodeConductor'), E.recurringBillingMode('IN_ADVANCE'), E.currencies(E.currency(self.currency)), E.units(*[ E.unit(name=u) for u in units ]), prods, E.rules(E.changePolicy(E.changePolicyCase(E.policy('END_OF_TERM'))), E.changeAlignment(E.changeAlignmentCase(E.alignment('START_OF_SUBSCRIPTION'))), E.cancelPolicy(E.cancelPolicyCase(E.productCategory('STANDALONE'), E.policy('IMMEDIATE')), E.cancelPolicyCase(E.policy('END_OF_TERM'))), E.createAlignment(E.createAlignmentCase(E.alignment('START_OF_SUBSCRIPTION'))), E.billingAlignment(E.billingAlignmentCase(E.alignment('ACCOUNT'))), E.priceList(E.priceListCase(E.toPriceList('DEFAULT')))), plans, E.priceLists(E.defaultPriceList(E.plans(*[ E.plan(n) for n in plannames ]), name='DEFAULT')), **{('{{{}}}schemaLocation').format(xsi): 'CatalogSchema.xsd'})
        xml = etree.tostring(catalog, xml_declaration=True, pretty_print=True, standalone=False, encoding='UTF-8')
...
class KillBill(object):
    ...
    class BaseResource(object):
        ...
        def _object_query(self, uuid, entity=None, method='GET', data=None, **kwargs):
            return self.request(('/').join([self.path, uuid, entity or '']), method=method, data=data, **kwargs)
        ...
        def request(self, url, method='GET', data=None, verify=False, **kwargs):
            ...
            headers = {'User-Agent': 'NodeConductor/%s' % __version__, 'Accept': 'application/json', 
            ...
            try:
                response = getattr(requests, method.lower())(url, params=kwargs, data=data, auth=self.auth, headers=headers, verify=verify)
            ...
            if response.status_code == codes.created:
                ...
                try:
                    if response_type == 'xml':
                        data = etree.fromstring(response.text.encode('utf-8'), etree.XMLParser(ns_clean=True, recover=True, encoding='utf-8'))
                    elif response_type == 'json' and response.text:
                        data = response.json()
                    else:
                        data = response.text
                except ValueError as e:
                    raise KillBillError('Incorrect response from Killbill backend %s: %s' % (url, e))
