...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_killbill/views.py
...
class InvoiceViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def items(self, request, uuid=None):
        try:
            ...
            return Response({'Detail': 'Cannot retrieve data from invoice backend'}, status=status.HTTP_400_BAD_REQUEST)

    @decorators.list_route()
    def annual_report(self, request):
        group_by = request.query_params.get('group_by', 'customer')
        if group_by not in ('customer', 'month'):
            return Response({'Detail': 'group_by parameter can be only `month` or `customer`'}, status=status.HTTP_400_BAD_REQUEST)
        else:
            truncate_date = connection.ops.date_trunc_sql('month', 'date')
            invoices = Invoice.objects.all()
            invoices_values = invoices.extra({'month': truncate_date}).values('month', 'customer__name').annotate(Sum('amount'))
            ...
            pisa.pisaDocument(StringIO.StringIO(render_to_string('nodeconductor_killbill/annual_report.html', context)), result)
