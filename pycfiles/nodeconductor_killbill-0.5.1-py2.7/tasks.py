...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_killbill/tasks.py
...
def update_today_usage_of_resource(resource_str):
    with transaction.atomic():
        ...
        if not resource.billing_backend_id:
            logger.warning("Can't update usage for resource %s which is not subscribed to backend", resource_str)
            return
        numerical = cs_backend.NUMERICAL
        content_type = ContentType.objects.get_for_model(resource)
        units = {(item.item_type, None if item.item_type in numerical else item.key):item.units for item in DefaultPriceListItem.objects.filter(resource_content_type=content_type)}
        now = timezone.now()
