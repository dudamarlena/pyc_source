...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/story_parser/parser.py
# Compiled at: 2009-10-27 16:35:12
import doctest, re
...
class RegexInternationalized(object):
    _all_regexes = {'pt-br': {'story_regex': 'História: (.+)', 'in_order_to_regex': 'Para que (.+)', 
                 'as_a_regex': 'Como um (.+)', 
                 'i_want_to_regex': 'Eu quero (.+)', 
                 'so_that_regex': 'Para que (.+)', 
                 'scenario_regex': 'Cenário(?: \\d+)?: (.+)', 
                 'given_regex': 'Dado que (.+)', 
                 'when_regex': 'Quando (.+)', 
                 'then_regex': 'Então (.+)', 
                 'and_regex': 'E (.+)'}, 
       'en-us': {'story_regex': 'Story: (.+)', 'in_order_to_regex': 'In order to (.+)', 
                 'as_a_regex': 'As an? (.+)', 
                 'i_want_to_regex': 'I want to (.+)', 
                 'so_that_regex': 'So that (.+)', 
                 'scenario_regex': 'Scenario(?: \\d+)?: (.+)', 
                 'given_regex': 'Given (.+)', 
                 'when_regex': 'When (.+)', 
                 'then_regex': 'Then (.+)', 
                 'and_regex': 'And (.+)'}}
...
class StoriesParser(object):
    ...
    def _get_story_block(self, index):
        story_block = [
         self._lines[index]]
        index += 1
        while index < len(self._lines) and re.match(self._regexes['story_regex'], self._lines[index]) is None:
    ...
    def _parse_stories(self):
        for story_block in self._get_story_blocks():
            ...
            self._stories.append(StoryParsed(title, role, feature, business_value, story_block[:4], scenarios))
