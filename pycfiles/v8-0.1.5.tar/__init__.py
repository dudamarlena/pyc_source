...
# [GCC 8.4.0]
# Embedded file name: /home/lex/projects/python-v8/libv8/build/gyp/pylib/gyp/__init__.py
# Compiled at: 2014-04-20 15:47:30
import copy, gyp.input, optparse, os.path, re, shlex, sys, traceback
    ...
    for name, metadata in options._regeneration_metadata.iteritems():
        ...
        if action == 'append':
            ...
            if action == 'store_true' and value or action == 'store_false' and not value:
                ...
                print >> sys.stderr, 'Warning: environment regeneration unimplemented for %s flag %r env_name %r' % (
                 action, opt,
                 env_name)
        else:
            print >> sys.stderr, 'Warning: regeneration unimplemented for action %r flag %r' % (
             action, opt)
...
def gyp_main(args):
    ...
    if not options.depth:
        ...
        if not options.depth:
            raise GypError('Could not automatically locate src directory.  This isa temporary Chromium feature that will be removed.  Use--depth as a workaround.')
    ...
    for format in set(options.formats):
        params = {'options': options, 'build_files': build_files, 
