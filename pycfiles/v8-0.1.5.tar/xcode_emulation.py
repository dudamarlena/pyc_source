...
# [GCC 8.4.0]
# Embedded file name: /home/lex/projects/python-v8/libv8/build/gyp/pylib/gyp/xcode_emulation.py
# Compiled at: 2014-04-20 15:47:30
"""
This module contains classes that help to emulate xcodebuild behavior on top of
other build systems, such as make and ninja.
"""
import copy, gyp.common, os, os.path, re, shlex, subprocess, sys, tempfile
...
class XcodeSettings(object):
    ...
    def GetProductType(self):
        ...
        if self._IsBundle():
            return {'executable': 'com.apple.product-type.application', 
               'loadable_module': 'com.apple.product-type.bundle', 
               'shared_library': 'com.apple.product-type.framework'}[self.spec['type']]
        else:
            return {'executable': 'com.apple.product-type.tool', 
               'loadable_module': 'com.apple.product-type.library.dynamic', 
               'shared_library': 'com.apple.product-type.library.dynamic', 
               'static_library': 'com.apple.product-type.library.static'}[self.spec['type']]

    def GetMachOType(self):
        """Returns the MACH_O_TYPE of this target."""
        if not self._IsBundle() and self.spec['type'] == 'executable':
            return ''
        return {'executable': 'mh_execute', 'static_library': 'staticlib', 
           'shared_library': 'mh_dylib', 
           'loadable_module': 'mh_bundle'}[self.spec['type']]
    ...
    def _GetStandaloneExecutableSuffix(self):
        ...
        return {'executable': '', 'static_library': '.a', 
           'shared_library': '.dylib', 
           'loadable_module': '.so'}[self.spec['type']]

    def _GetStandaloneExecutablePrefix(self):
        return self.spec.get('product_prefix', {'executable': '', 
           'static_library': 'lib', 
           'shared_library': 'lib', 
           'loadable_module': ''}[self.spec['type']])

    def _GetStandaloneBinaryPath(self):
        """Returns the name of the non-bundle binary represented by this target.
    E.g. hello_world. Only valid for non-bundles."""
        assert not self._IsBundle()
        assert self.spec['type'] in ('executable', 'shared_library', 'static_library',
                                     'loadable_module'), 'Unexpected type %s' % self.spec['type']
    ...
    def _AppendPlatformVersionMinFlags(self, lst):
        ...
        if 'IPHONEOS_DEPLOYMENT_TARGET' in self._Settings():
            ...
            if sdk_path_basename.lower().startswith('iphonesimulator'):
                ...
                self._Appendf(lst, 'IPHONEOS_DEPLOYMENT_TARGET', '-miphoneos-version-min=%s')
    ...
    compilations."""
        ...
        if self._Settings().get('GCC_STRICT_ALIASING') == 'YES':
            ...
            if self._Settings().get('GCC_STRICT_ALIASING') == 'NO':
    ...
    def GetInstallName(self):
        ...
        if '$' in install_name:
            assert install_name in ('$(DYLIB_INSTALL_NAME_BASE:standardizepath)/$(WRAPPER_NAME)/$(PRODUCT_NAME)',
             default_install_name), "Variables in LD_DYLIB_INSTALL_NAME are not generally supported yet in target '%s' (got '%s')" % (
       ...
       has the same value in all configurations and throws otherwise."""
        ...
        for configname in sorted(self.xcode_settings.keys()):
            if is_first_pass:
                ...
                is_first_pass = False
            elif not result == self.xcode_settings[configname].get(setting, None):
                raise AssertionError("Expected per-target setting for '%s', got per-config setting (target %s)" % (
                 setting, self.spec['target_name']))
    ...
    def _XcodeVersion(self):
        if len(XcodeSettings._xcode_version_cache) == 0:
            ...
            if build:
                build = build.split()[(-1)]
            XcodeSettings._xcode_version_cache = (
             version, build)
        ...
        if configname not in XcodeSettings._plist_cache:
            ...
            if self.isIOS:
                ...
                if configname.endswith('iphoneos'):
                    ...
                    cache['CFBundleSupportedPlatforms'] = [
                     'iPhoneSimulator']
...
def IsMacBundle(flavor, spec):
  ...
  resources into that directory."""
    ...
    assert is_mac_bundle and spec['type'] != 'none', 'mac_bundle targets cannot have type none (target "%s")' % spec['target_name']
  ...
  * |extra_env| is a dict of env variables that should be exported when
  ...
  """
    ...
    if not info_plist:
        ...
        return (
         info_plist, dest_plist, defines, extra_env)
...
def _GetXcodeEnv(xcode_settings, built_products_dir, srcroot, configuration, additional_settings=None):
  ...
  """
    ...
    env = {'BUILT_PRODUCTS_DIR': built_products_dir, 
...
def GetSortedXcodeEnv(xcode_settings, built_products_dir, srcroot, configuration, additional_settings=None):
    env = _GetXcodeEnv(xcode_settings, built_products_dir, srcroot, configuration, additional_settings)
    ...
    for target_dict in targets.values():
        for config_name in target_dict['configurations'].keys():
            ...
            if target_dict['toolset'] == 'target':
                new_config_dict['xcode_settings']['ARCHS'] = [
                 'armv7']
