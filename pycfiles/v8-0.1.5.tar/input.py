...
# [GCC 8.4.0]
# Embedded file name: /home/lex/projects/python-v8/libv8/build/gyp/pylib/gyp/input.py
...
import compiler, copy, gyp.common, multiprocessing, optparse, os.path, re, shlex, signal, subprocess, sys, threading, time, traceback
from gyp.common import GypError
linkable_types = [
 'executable', 'shared_library', 'loadable_module']
dependency_sections = [
...
def LoadTargetBuildFilesParallel(build_files, data, aux_data, variables, includes, depth, check, generator_input_info):
    ...
    try:
        ...
        while parallel_state.dependencies or parallel_state.pending:
            ...
            aux_data_in = {}
            global_flags = {'path_sections': globals()['path_sections'], 
               'non_configuration_keys': globals()['non_configuration_keys'], 
               'multiple_toolsets': globals()['multiple_toolsets']}
            if not parallel_state.pool:
                parallel_state.pool = multiprocessing.Pool(8)
            parallel_state.pool.apply_async(CallLoadTargetBuildFile, args=(
             global_flags, dependency,
             data_in, aux_data_in,
             variables, includes, depth, check, generator_input_info), callback=parallel_state.LoadTargetBuildFileCallback)
                    ...
                    if command_string == 'pymod_do_main':
                        ...
                        try:
                            ...
                            try:
                                ...
                                raise GypError('Error importing pymod_do_mainmodule (%s): %s' % (
                                 parsed_contents[0], e))
    ...
    if conditions_key not in the_dict:
        ...
        for condition in conditions_list:
            ...
            if len(condition) != 2 and len(condition) != 3:
                raise GypError(conditions_key + ' ' + condition[0] + ' must be length 2 or 3, not ' + str(len(condition)))
            ...
            try:
                ...
                syntax_error = SyntaxError("%s while evaluating condition '%s' in %s at character %d." % (
                 str(e.args[0]), e.text, build_file, e.offset), e.filename, e.lineno, e.offset, e.text)
                raise syntax_error
        ...
        if not dependencies:
            ...
            for dependency in dependencies:
                ...
                if not dependency_node:
                    raise GypError("Dependency '%s' not found while trying to load target %s" % (
                     dependency, target))
...
def DoDependentSettings(key, flat_list, targets, dependency_nodes):
    for target in flat_list:
        ...
        if key == 'all_dependent_settings':
            ...
            if key == 'direct_dependent_settings':
...
def SetUpConfigurations(target, target_dict):
    ...
    if 'default_configuration' not in target_dict:
        concrete = [ i for i in target_dict['configurations'].iterkeys() if not target_dict['configurations'][i].get('abstract') ]
    ...
    for configuration in target_dict['configurations'].keys():
        ...
        for key in configuration_dict.keys():
            if key in invalid_configuration_keys:
                raise GypError('%s not allowed in the %s configuration, found in target %s' % (
                 key, configuration, target))


def ProcessListFiltersInDict(name, the_dict):
    r"""Process regular expression and exclusion-based filters on lists.
...
def ValidateTargetType(target, target_dict):
  ...
  """
    VALID_TARGET_TYPES = ('executable', 'loadable_module', 'static_library', 'shared_library',
                          'none')
    target_type = target_dict.get('type', None)
    if target_type not in VALID_TARGET_TYPES:
        raise GypError("Target %s has an invalid target type '%s'.  Must be one of %s." % (
         target, target_type, ('/').join(VALID_TARGET_TYPES)))
    if target_dict.get('standalone_static_library', 0) and not target_type == 'static_library':
        raise GypError('Target %s has type %s but standalone_static_library flag is only valid for static_library type.' % (
         target,
         target_type))
    return
...
def ValidateSourcesInTarget(target, target_dict, build_file):
    if target_dict.get('type', None) not in ('static_library', 'shared_library'):
        ...
        for source in sources:
            ...
            is_compiled_file = ext in ('.c', '.cc', '.cpp', '.cxx', '.m', '.mm', '.s',
                                       '.S')
  ...
  """
    ...
    for rule in rules:
        ...
        if rule_extension in rule_extensions:
            raise GypError(('extension %s associated with multiple rules, ' + 'target %s rules %s and %s') % (
             rule_extension, target,
             rule_extensions[rule_extension]['rule_name'],
             rule_name))
        rule_extensions[rule_extension] = rule
        if 'rule_sources' in rule:
            raise GypError('rule_sources must not exist in input, target %s rule %s' % (
             target, rule_name))
        rule_sources = []
        source_keys = ['sources']
...
def ValidateRunAsInTarget(target, target_dict, build_file):
    ...
    if not isinstance(run_as, dict):
        raise GypError("The 'run_as' in target %s from file %s should be a dictionary." % (
         target_name, build_file))
    action = run_as.get('action')
    if not action:
        raise GypError("The 'run_as' in target %s from file %s must have an 'action' section." % (
         target_name, build_file))
    if not isinstance(action, list):
        raise GypError("The 'action' for 'run_as' in target %s from file %s must be a list." % (
         target_name, build_file))
    working_directory = run_as.get('working_directory')
    if working_directory and not isinstance(working_directory, str):
        raise GypError("The 'working_directory' for 'run_as' in target %s in file %s should be a string." % (
         target_name, build_file))
    environment = run_as.get('environment')
    if environment and not isinstance(environment, dict):
        raise GypError("The 'environment' for 'run_as' in target %s in file %s should be a dictionary." % (
         target_name, build_file))
...
def VerifyNoCollidingTargets(targets):
  ...
  """
    ...
    for target in targets:
        ...
        key = subdir + ':' + name
        if key in used:
            raise GypError('Duplicate target name "%s" in directory "%s" used both in "%s" and "%s".' % (
             name, subdir, gyp, used[key]))
        used[key] = gyp
...
def Load(build_files, variables, includes, depth, generator_input_info, check, circular_check, parallel, root_targets):
    SetGeneratorGlobals(generator_input_info)
    ...
    if parallel:
        LoadTargetBuildFilesParallel(build_files, data, aux_data, variables, includes, depth, check, generator_input_info)
    else:
        for build_file in build_files:
            try:
                LoadTargetBuildFile(build_file, data, aux_data, variables, includes, depth, check, True)
            except Exception as e:
