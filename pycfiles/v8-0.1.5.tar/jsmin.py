...
# [GCC 8.4.0]
# Embedded file name: /home/lex/projects/python-v8/libv8/tools/jsmin.py
# Compiled at: 2012-05-22 05:16:05
...
class JavaScriptMinifier(object):
    ...
    """
        ...
        while True:
            ...
            if identifier_second_char != 0:
                new_identifier = self.CharFromNumber(identifier_second_char - 1) + new_identifier
    ...
    """
        ...
        for line in re.split('\\n', text):
            ...
            slash_quoted_regexp = '(?<![\\w$\'\\")\\]])/(?:(?=\\()|(?:[^()/\\\\]|\\\\.)+)(?:\\([^/\\\\]|\\\\.)*/'
            line = re.sub(('|').join([double_quoted_string,
             single_quoted_string,
             slash_quoted_regexp,
             '( )+']), self.RemoveSpaces, line)
            line = re.sub(('|').join([double_quoted_string,
             single_quoted_string,
             slash_quoted_regexp,
             '(?<![a-zA-Z_0-9$%]) | (?![a-zA-Z_0-9$%])()']), self.RemoveSpaces, line)
            if self.nesting == 0:
                re.sub('([a-zA-Z0-9_$%]+)', self.LookAtIdentifier, line)
            function_declaration_regexp = '\\bfunction( [\\w$%]+)?\\([\\w$%,]+\\)\\{'
            if re.search('\\?', line):
                block_trailing_colon = ''
            else:
                block_trailing_colon = '(?![:\\w$%])'
            variable_use_regexp = '(?<![.\\w$%])[\\w$%]+' + block_trailing_colon
            line = re.sub(('|').join([double_quoted_string,
             single_quoted_string,
             slash_quoted_regexp,
             '\\{',
             ...
             variable_use_regexp]), self.Declaration, line)
