...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/bayesdb/data_utils.py
# Compiled at: 2015-02-12 15:25:14
import sys, csv, copy, pandas, re, numpy, json, prettytable
from math import pi
import utils
...
def gen_data(gen_seed, num_clusters, num_cols, num_rows, max_mean_per_category=10, max_std=1, max_mean=None):
    if max_mean is None:
...
def gen_factorial_data(gen_seed, num_clusters, num_cols, num_rows, num_splits, max_mean_per_category=10, max_std=1, max_mean=None):
    random_state = numpy.random.RandomState(gen_seed)
    data_list = []
    inverse_permutation_indices_list = []
    for data_idx in xrange(num_splits):
        data_i, zs_i = gen_data(gen_seed=random_state.randint(sys.maxint), num_clusters=num_clusters, num_cols=num_cols / num_splits, num_rows=num_rows, max_mean_per_category=max_mean_per_category, max_std=max_std, max_mean=max_mean)
...
def gen_numerical_metadata(column_data, parameters=None):
    res = dict(modeltype='normal_inverse_gamma', value_to_code=dict(), code_to_value=dict(), parameters=None)
    return res
...
def gen_cyclic_metadata(column_data, parameters=None):
    ...
    elif 'min' not in parameters or 'max' not in parameters:
    ...
    return dict(modeltype='vonmises', value_to_code=dict(), code_to_value=dict(), parameters=parameters)
...
def gen_categorical_metadata(column_data, parameters=None):
    ...
    ret = dict(modeltype='symmetric_dirichlet_discrete', value_to_code=value_to_code, code_to_value=code_to_value, parameters=parameters)
    return ret


metadata_generator_lookup = dict(numerical=gen_numerical_metadata, cyclic=gen_cyclic_metadata, categorical=gen_categorical_metadata, ignore=gen_ignore_metadata, key=gen_ignore_metadata)
...
def gen_M_c_from_T(T, cctypes=None, colnames=None, parameters=None, codebook=None):
    ...
    for col_idx, colname in enumerate(colnames):
        if codebook and colname in codebook:
            ...
            colname_codebook = {'description': 'No description', 
               'short_name': colname, 
               'value_map': None}
    ...
    M_c = dict(name_to_idx=name_to_idx, idx_to_name=idx_to_name, column_metadata=column_metadata, column_codebook=column_codebook)
    return M_c
...
def gen_M_c_from_T_with_colnames(T, colnames):
    num_rows = len(T)
    num_cols = len(T[0])
    gen_numerical_metadata = lambda : dict(modeltype='normal_inverse_gamma', value_to_code=dict(), code_to_value=dict())
    column_metadata = [ gen_numerical_metadata() for col_idx in range(num_cols)
                      ]
    name_to_idx = dict(zip(colnames, range(num_cols)))
    idx_to_name = dict(zip(map(str, range(num_cols)), colnames))
    M_c = dict(name_to_idx=name_to_idx, idx_to_name=idx_to_name, column_metadata=column_metadata)
    return M_c


def gen_factorial_data_objects(gen_seed, num_clusters, num_cols, num_rows, num_splits, max_mean=10, max_std=1, send_data_inverse_permutation_indices=False):
    T, data_inverse_permutation_indices = gen_factorial_data(gen_seed, num_clusters, num_cols, num_rows, num_splits, max_mean, max_std)
    T = T.tolist()
...
def discretize_data(T, discretize_indices):
    ...
    T_array[:, discretize_indices] = numpy.array(T_array[:, discretize_indices], dtype=int)
...
def numerical_or_ignore_from_file_with_colnames(filename, cctypes, max_rows=None, gen_seed=0):
    ...
    with open(filename) as (fh):
        csv_reader = csv.reader(fh)
        header = csv_reader.next()
        T = numpy.array([ [ col for col, flag in zip(row, colmask) if flag ] for row in csv_reader ], dtype=float).tolist()
        num_rows = len(T)
...
def get_pop_indices(cctypes, colnames):
    ...
    pop_columns = [ colname for cctype, colname in zip(cctypes, colnames) if cctype in ('ignore',
                                                                                        'key')
                  ]
...
def read_data_objects(filename, max_rows=None, gen_seed=0, cctypes=None, colnames=None):
...
def guess_column_types(T, colnames_full, count_cutoff=20, warn_ratio=0.02, warn_cardinality=7):
    ...
    for column_idx, column_data in enumerate(T_transposed):
        ...
        if column_type == 'categorical':
            ...
            warnings.append('Column "%s" is NOT categorical but has a low percentage of distinct values ( actual: %0.2f   <   warn threshold: %0.2f ). Convert to categorical using UPDATE SCHEMA if appropriate.' % (
             colnames_full[column_idx], distinct_ratio, warn_ratio))

    return (
     column_types, warnings)
...
def gen_T_and_metadata(colnames, raw_T, max_rows=None, gen_seed=0, cctypes=None, parameters=None, codebook=None):
