...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/bayesdb/tester.py
# Compiled at: 2015-02-12 15:25:14
from bayesdb.client import Client
import StringIO, os, sys, time, pickle
...
class TestClient(Client):
    ...
    def __init__(self, test_id, csv_filename, model_filename=None, num_models=10, num_analyze_iterations=250, num_analyze_minutes=None, schema_update_commands=[], key_column=0):
        """
        ...
        self._initialize(model_filename, num_models, num_analyze_iterations, num_analyze_minutes, schema_update_commands, key_column)

    def _initialize(self, model_filename=None, num_models=10, num_analyze_iterations=250, num_analyze_minutes=None, schema_update_commands=[], key_column=0):
        commands = []
        ...
        for cmd in commands:
            ...
            sys.stdout = out
            super(TestClient, self).execute(cmd.replace('<btable>', self.btable_name), pretty=True, key_column=key_column, debug=False, yes=True, force_output=True)
    ...
    def __call__(self, call_input, pretty=True, timing=False, wait=False, plots=None, yes=False, debug=False, pandas_df=True, pandas_output=True, key_column=None, return_raw_result=False, ignore_output=False, force_output=True):
        """ Calls BayesDB Client, redirects and saves output, and returns data.
        """
        out = StringIO.StringIO()
        oldstdout = sys.stdout
        sys.stdout = out
        res = super(TestClient, self).execute(call_input.replace('<btable>', self.btable_name), pretty, timing, wait, plots, yes, debug, pandas_df, pandas_output, key_column, return_raw_result, force_output)
        sys.stdout = oldstdout
        self.output.append(out.getvalue())
