...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/bayesdb/client_remote.py
# Compiled at: 2015-02-12 15:25:14
import os, math, json, requests, prettytable, pandas, numpy as np, pylab
from matplotlib.colors import LogNorm, Normalize
from matplotlib.ticker import MaxNLocator
import matplotlib.gridspec as gs, matplotlib.cm, pdb
default_args = {'pretty': True, 
   'timing': False, 
   'wait': False, 
   'plots': None, 
   'yes': True, 
    ...
    for i in range(len(columns)):
        for j in range(len(columns)):
            if j == 0 and i < len(columns) - 1:
                ...
                create_plot(data, pylab.subplot(gsp[(i, j)], adjustable='box', aspect=1), False, False, columns[i], horizontal=True, compress=True, super_compress=super_compress)
                ...
                data = parse_data_for_hist(sub_colnames, sub_data, M_c)
                create_plot(data, pylab.subplot(gsp[(i, j)], adjustable='box', aspect=1), False, False, columns[(j - 2)], horizontal=False, compress=True, super_compress=super_compress)
                ...
                data = parse_data_for_hist(sub_colnames, sub_data, M_c)
                create_plot(data, pylab.subplot(gsp[(i, j)]), False, False, horizontal=True, compress=True, super_compress=super_compress)


def plot_matrix(matrix, column_names, title='', filename=None):
    fig = pylab.figure()
    fig.set_size_inches(16, 12)
    pylab.imshow(matrix, interpolation='none', cmap=matplotlib.cm.Blues, vmin=0, vmax=1)
    pylab.colorbar()
    ...
    def execute(self, bql_string, **kwargs):
        ...
        if not type(result) != int:
            ...
            if 'matrix' in result and (plots or client_dict['filename']):
                plot_matrix(result['matrix'], result['column_names'], result['title'], client_dict['filename'])
                if pretty:
            ...
            if 'plot' in client_dict and client_dict['plot'] and (plots or client_dict['filename']):
                ...
                plot_general_histogram(result['columns'], result['data'], result['M_c'], client_dict['filename'], client_dict['scatter'], remove_key=plot_remove_key)
                return self.pretty_print(result)
