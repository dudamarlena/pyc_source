...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/bayesdb/persistence_layer.py
# Compiled at: 2015-02-12 15:25:14
import os, sys, datetime, json, cPickle as pickle, shutil, contextlib, threading, re
from threading import RLock
import data_utils, utils, bayesdb.settings as S
...
class PersistenceLayer(object):
    ...
    def gen_default_codebook(self, M_c_full):
        ...
        if 'column_codebook' in M_c_full:
            ...
            if not empty_codebook_entires:
                return
        column_codebook = [
         None] * len(M_c_full['name_to_idx'])
        for colname, idx in M_c_full['name_to_idx'].iteritems():
            column_codebook[idx] = {'description': 'No description.', 
               'short_name': colname, 
               'value_map': None}
    ...
    def update_schema(self, tablename, mappings):
        ...
        try:
            ...
            print 'WARNING: resetting parameters to defaults. Please check these values with DESCRIBE and adjust them manually if necessary.'
            parameters_full = []
            for md in M_c_full['column_metadata']:
                if 'dirichlet' in md['modeltype']:
                    params = {'cardinality': len(md['value_to_code'])}
                elif 'vonmises' in md['modeltype']:
                    params = {'min': 0.0, 'max': 6.283185307179586}
                else:
                    params = None
        ...
        for colname, mapping in mappings.items():
            ...
            if cctypes_full[cidx] == 'key':
                raise utils.BayesDBError('Error: %s is already set as the table key. To change its type, reload the table using CREATE BTABLE and choose a different key column.' % colname.lower())
            elif cctype == 'key':
    ...
    def update_models(self, tablename, modelids, X_L_list, X_D_list, diagnostics_data_list, increment_iterations_list=None, increment_time_list=None):
        """
        ...
        for idx, modelid in enumerate(modelids):
            if increment_iterations_list is not None:
                ...
                increment_time = 0
            self.update_model(tablename, X_L_list[idx], X_D_list[idx], diagnostics_data_list[idx], modelid, increment_iterations, increment_time)

        return

    def update_model(self, tablename, X_L, X_D, diagnostics_data, modelid, increment_iterations=0, increment_time=0):
        ...
        if increment_iterations > 0:
            ...
            elif model['diagnostics']['iterations'][(-1)] != diagnostics_data['iterations'] and model['diagnostics']['time'][(-1)] != diagnostics_data['time']:
                for key, value in diagnostics_data.iteritems():
