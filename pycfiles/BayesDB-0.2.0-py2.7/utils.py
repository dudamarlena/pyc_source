...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/bayesdb/utils.py
# Compiled at: 2015-02-12 15:25:14
import inspect, numpy, os, re, inspect, ast, pylab, matplotlib.cm, time, pandas, math
from collections import Counter
import data_utils as du, select_utils, functions
...
def get_imputation_and_confidence_from_samples(M_c, X_L, col_idx, samples):
    ...
    column_component_suffstats_i = get_column_component_suffstats_i(M_c, X_L, col_idx)
    imputation_confidence = imputation_confidence_function(samples, e, column_component_suffstats_i)
...
def get_column_component_suffstats_i(M_c, X_L, col_idx):
    ...
    column_component_suffstats_i = view_state_i['column_component_suffstats'][local_col_idx]
...
def numerical_imputation_confidence(samples, imputed, column_component_suffstats_i):
...
modeltype_to_imputation_function = {'normal_inverse_gamma': numerical_imputation, 
   'symmetric_dirichlet_discrete': categorical_imputation}
modeltype_to_imputation_confidence_function = {'normal_inverse_gamma': numerical_imputation_confidence, 
...
def histogram_table(data, columns, M_c, remove_key=True):
    ...
    if len(data) > 0:
        ...
        summary_data = pandas.DataFrame({'bin_minimum': bin_mins, 
           'bin_maximum': bin_maxs, 
           'frequency': bin_freqs, 
           'probability': bin_probs})
        columns = [
         'bin_minimum', 'bin_maximum', 'frequency', 'probability']
