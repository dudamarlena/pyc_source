...
# [GCC 8.4.0]
# Embedded file name: /home/fabian/devs/novix/python/pyplan-core/pyplan_core/cubepy/cube.py
# Compiled at: 2020-04-29 16:08:41
# Size of source mod 2**32: 62356 bytes
...
class Cube(object):
    __doc__ = 'Wrapper around numpy.ndarray with named and labelled axes. The API aims to be as similar to ndarray API as\n    possible. Moreover it allows automatic axis matching and alignment in operations among cubes.\n    '
                            ...
                            if fillWithNan:
                                ...
                                if self.values.dtype.kind.lower() == 'o':
                                    if len(self.values) > 0:
                                        ...
                                        if kindToString(self.values.dtype.kind) == 'numeric':
                                            _extraValues[:] = 0
                                        else:
    ...
    def squeeze(self, axis=None):
        ...
        if axis is None:
            ...
            new_axes = tuple((a for a in self.axes if len(a) != 1 or a is not axisInstance))
        ...
        if _kind == 'numeric':
            if defaultValue == 0:
                if self.values.dtype.kind == 'f':
                    defaultValue = 0.0
        return self.apply(lambda x:         if np.isnan(x):
defaultValue # Avoid dead code: x)
    ...
    def clearInf(self, defaultValue=0):
        ...
        if _kind == 'numeric':
            if defaultValue == 0:
                if self.values.dtype.kind == 'f':
                    defaultValue = 0.0
        return self.apply(lambda x:         if np.isinf(x):
defaultValue # Avoid dead code: x)
    ...
    def clearAll(self, defaultValue=0):
        ...
        if _kind == 'numeric':
            if defaultValue == 0:
                if self.values.dtype.kind == 'f':
                    defaultValue = 0.0
        return self.apply(lambda x:         if np.isinf(x) or np.isnan(x):
defaultValue # Avoid dead code: x)
    ...
    def filter(self, filter_by, values=None):
        ...
        if hasattr(filter_by, 'axes'):
            filter_by = filter_by.axes
        result = self
        for item in filter_by:
            if not is_axis(item) or self.has_axis(item.name):
    ...
    def _group(self, axis, func, sorted=True, *args):
        ...
        for value in unique_values:
            ...
            sub_cube = (np.apply_along_axis)(func, old_axis_index, sub_cube, *args)
    ...
    def selectOptimalFunction(self, partA, partB, fn):
        res = fn
        if fn is np.add:
            if isinstance(partA, self.__class__):
                if isinstance(partB, self.__class__):
                    if partA.values.dtype.kind in {'S', 'U'} or partB.values.dtype.kind in {'S', 'U'}:
                        res = np.vectorize(vectoradd)
            elif isinstance(partB, str):
                res = np.vectorize(vectoradd)
            elif fn is np.multiply:
                if isinstance(partA, self.__class__):
                    if not (isinstance(partB, self.__class__)):
                        if not partA.values.dtype.kind in {'b'} or partB.values.dtype.kind in {'S', 'U'}:
                            res = np.vectorize(vectormul)
        elif isinstance(partB, bool):
            res = np.vectorize(vectormul)
        return res
...
def _convertNpFunction(fnSource):
    ...
    fndic = {"<ufunc 'equal'>":np.ndarray.__eq__, 
    ...
    if str(fnSource) in fndic:
        return fndic[str(fnSource)]
    return fnSource
