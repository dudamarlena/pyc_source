...
# Embedded file name: /tmp/pip-install-ncu5lfw4/arelle/arelle/ModelFormulaObject.py
# Compiled at: 2018-08-09 04:11:41
# Size of source mod 2**32: 126465 bytes
...
aspectFromToken = {'location': Aspect.LOCATION, 'concept': Aspect.CONCEPT, 
    ...
    def compile(self):
        for arcrole in self.descendantArcroles:
            for modelRel in self.modelXbrl.relationshipSet(arcrole).fromModelObject(self):
                ...
                elif toModelObject is not None:
                    self.modelXbrl.error('formula:internalError', _('Invalid formula object %(element)s'), modelObject=self, element=toModelObject.elementQname)
...
class ModelFormulaRules:
                    ...
                    if name == 'qname':
                        ...
                        if ruleElt.getparent().localName == 'concept':
                            ...
                            if qnDim in self.aspectValues:
                                self.modelXbrl.error('xbrlfe:conflictingAspectRules', _('Dimension %(dimension)s aspect has multiple rules in formula.'), modelObject=self, dimension=qnDim)
                        ...
                        if name == 'qnameExpression':
                            ...
                            if name == 'omit' and ruleElt.getparent().get('dimension') is not None:
                                ...
                                if name == 'value' and ruleElt.getparent().get('dimension') is not None:
                                    self.aspectValues[qname(ruleElt.getparent(), ruleElt.getparent().get('dimension'))] = XmlUtil.child(ruleElt, '*', '*')
                                else:
                                    if name == 'xpath' and ruleElt.getparent().get('dimension') is not None:
                                            ...
                                            if name == 'instant':
                                                ...
                                                if name == 'duration':
                                                    ...
                                                    if name == 'forever':
                                                        ...
                                                        if name == 'unit' and ruleElt.get('augment') is not None:
    ...
    def evaluateRule(self, xpCtx, aspect):
        ...
        if aspect in (Aspect.MULTIPLY_BY, Aspect.DIVIDE_BY):
            ...
            return ()
        if xpCtx and aspect in self.aspectProgs:
...
class ModelParameter(ModelFormulaResource):
    ...
    def init(self, modelDocument):
        ...
        if self.parameterQname in self.modelXbrl.qnameParameters:
            self.modelXbrl.error('xbrlve:parameterNameClash', _('Parameter name used on multiple parameters %(name)s'), modelObject=self, name=self.parameterQname)
        else:
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        ...
        for fact in facts:
            ...
            for modelRel in relationships:
                if (isFromAxis and isinstance(modelRel.fromModelObject, ModelObject) and factQname == modelRel.fromModelObject.qname or not isFromAxis and isinstance(modelRel.toModelObject, ModelObject) and factQname == modelRel.toModelObject.qname) and (hasNoTest or self.evalTest(xpCtx, modelRel)):
                    factOk = True
                    break

            if not factOk and axis.startswith('sibling') and factQname != sourceQname and not concept_relationships(xpCtx, None, (sourceQname, linkrole, arcrole, 'parent', 1, linkQname, arcQname)) and not concept_relationships(xpCtx, None, (factQname, linkrole, arcrole, 'parent', 1, linkQname, arcQname)) and concept_relationships(xpCtx, None, (factQname, linkrole, arcrole, 'child', 1, linkQname, arcQname)):
                factOk = True
            if not factOk:
                if axis.endswith('-or-self') and sourceQname == XbrlConst.qnXfiRoot and not concept_relationships(xpCtx, None, (factQname, linkrole, arcrole, 'parent', 1, linkQname, arcQname)) and concept_relationships(xpCtx, None, (factQname, linkrole, arcrole, 'child', 1, linkQname, arcQname)):
                    factOk = True
...
class ModelExplicitDimension(ModelFilter):
        ...
        if not hasattr(self, 'dimQnameExpressionProg'):
            ...
            if self.isFilterStatic:
                ...
                if dimConcept is None or not dimConcept.isExplicitDimension:
                    self.modelXbrl.error('xfie:invalidExplicitDimensionQName', _('%(dimension)s is not an explicit dimension concept QName.'), modelObject=self, dimension=self.dimQname)
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        ...
        for fact in facts:
            ...
            if dimConcept is None or not dimConcept.isExplicitDimension:
                self.modelXbrl.error('xfie:invalidExplicitDimensionQName', _('%(dimension)s is not an explicit dimension concept QName.'), modelObject=self, dimension=dimQname)
                return set()
            if fact.isItem:
                        ...
                        for memberModel in self.memberProgs:
                            ...
                            if memberModel.qname:
                                ...
                                if memConcept is None:
                                    return set()
                            if (not memberModel.axis or memberModel.axis.endswith('-self')) and matchMemQname == memQname:
                                factOk = True
                                break
