...
# Embedded file name: /tmp/pip-install-ncu5lfw4/arelle/arelle/FileSource.py
# Compiled at: 2018-08-09 04:11:41
# Size of source mod 2**32: 35791 bytes
...
archivePathSeparators = (
 '.zip' + os.sep, '.tar.gz' + os.sep, '.eis' + os.sep, '.xml' + os.sep, '.xfd' + os.sep, '.frm' + os.sep, '.taxonomyPackage.xml' + os.sep) + (('.zip/', '.tar.gz/', '.eis/', '.xml/', '.xfd/', '.frm/', '.taxonomyPackage.xml/') if os.sep != '/' else ())
POST_UPLOADED_ZIP = os.sep + 'POSTupload.zip'
SERVER_WEB_CACHE = os.sep + '_HTTP_CACHE'
XMLdeclaration = re.compile('<\\?xml[^><\\?]*\\?>', re.DOTALL)
TAXONOMY_PACKAGE_FILE_NAMES = (
...
class ArchiveFileIOError(IOError):
    ...
    def __init__(self, fileSource, errno, fileName):
        super(ArchiveFileIOError, self).__init__(errno, _('Archive {}').format(fileSource.url), fileName)
        self.fileName = fileName
...
class FileSource:
    ...
    def __init__(self, url, cntlr=None, checkIfXmlIsEis=False):
        ...
        try:
            ...
            l = file.read(128)
            file.close()
            if re.match('\\s*(<[?]xml[^?]+[?]>)?\\s*<(cor[a-z]*:|sdf:)?edgarSubmission', l):
                self.isEis = True
        ...
        if not self.isOpen:
                ...
                if self.isEis:
                    ...
                    try:
                        ...
                        while more:
                            ...
                            if len(l) < 8:
                                break
                            if len(buf) == 0 and l.startswith('<?xml '):
                                buf = l + file.read()
                                break
                    ...
                    if firstline.startswith('application/x-xfdl;content-encoding="asc-gzip"'):
            ...
            try:
                ...
                if len(metadataFiles) != 1:
                    raise IOError(_('Taxonomy package must contain one and only one metadata file: {0}.').format(', '.join(metadataFiles)))
                taxonomyPackage = PackageManager.parsePackage(self.cntlr, self.url)
                fileSourceDir = os.path.dirname(self.baseurl) + os.sep
                self.mappedPaths = dict((prefix, remapping if isHttpUrl(remapping) else fileSourceDir + remapping.replace('/', os.sep)) for prefix, remapping in taxonomyPackage['remappings'].items())
            except EnvironmentError as err:
                self.logError(err)
                return
    ...
    def isInArchive(self, filepath, checkExistence=False):
        ...
        if archiveFileSource is None:
            return False
        if checkExistence:
            archiveFileName = filepath[len(archiveFileSource.basefile) + 1:].replace('\\', '/')
            return archiveFileName in archiveFileSource.dir
        return True

    def isMappedUrl(self, url):
        if self.mappedPaths is not None:
            return any(url.startswith(mapFrom) for mapFrom in self.mappedPaths)
        return False
    ...
    def fileSourceContainingFilepath(self, filepath):
        if self.isOpen and (filepath.startswith(self.basefile) or filepath.startswith(self.baseurl)):
            return self
        ...
        if referencedFileParts is not None:
            ...
            if not self.isOpen or referencedArchiveFile != self.basefile and referencedArchiveFile != self.baseurl:
            ...
            if archiveFileSource.isTarGz:
                ...
                if archiveFileSource.isEis:
                    ...
                    if archiveFileSource.isXfd:
                        ...
                        raise ArchiveFileIOError(self, errno.ENOENT, archiveFileName)
                    elif archiveFileSource.isInstalledTaxonomyPackage and filepath.startswith(archiveFileSource.basefile):
    ...
    def exists(self, filepath):
        ...
        if archiveFileSource is not None:
            if filepath.startswith(archiveFileSource.basefile):
                ...
                archiveFileName = filepath[len(archiveFileSource.baseurl) + 1:]
            if archiveFileSource.isZip or archiveFileSource.isTarGz or archiveFileSource.isEis or archiveFileSource.isXfd or archiveFileSource.isRss or self.isInstalledTaxonomyPackage:
                pass
                ...
                if self.isEis:
                    ...
                    if self.isXfd:
                        ...
                        for data in self.xfdDocument.iter(tag='data'):
                            outfn = data.findtext('filename')
                            if outfn:
                                if len(outfn) > 2 and outfn[0].isalpha() and outfn[1] == ':' and outfn[2] == '\\':
                                    pass
                                else:
                                    files.append(outfn)
                            ...
                            for dsElt in XmlUtil.descendants(self.rssDocument, None, 'item'):
                                ...
                                if not instDoc:
                                    ...
                                    files.append((
                                     ...
                                     '{0}\n {1}\n {2}\n {3}\n {4}'.format(XmlUtil.text(XmlUtil.descendant(dsElt, edgr, 'companyName')), XmlUtil.text(XmlUtil.descendant(dsElt, edgr, 'formType')), XmlUtil.text(XmlUtil.descendant(dsElt, edgr, 'filingDate')), XmlUtil.text(XmlUtil.descendant(dsElt, edgr, 'cikNumber')), XmlUtil.text(XmlUtil.descendant(dsElt, edgr, 'period'))),
...
def openFileStream(cntlr, filepath, mode='r', encoding=None):
    ...
    if filestream is None:
        ...
        cntlr.webCache.retrieve(cntlr.webCache.cacheFilepathToUrl(filepath), filestream=filestream)
        if cntlr.isGAE:
...
def openXmlFileStream(cntlr, filepath, stripDeclaration=False):
    ...
    hdrBytes = openedFileStream.read(512)
    encoding = XmlUtil.encoding(hdrBytes, default=cntlr.modelManager.disclosureSystem.defaultXmlEncoding if cntlr else 'utf-8')
    ...
    if text is None:
        ...
        return (
         FileNamedStringIO(filepath, initial_value=text), encoding)


def stripDeclarationBytes(xml):
    xmlStart = xml[0:120]
    indexOfDeclaration = xmlStart.find('<?xml')
    if indexOfDeclaration >= 0:
        indexOfDeclarationEnd = xmlStart.find('?>', indexOfDeclaration)
...
def gaeSet(key, bytesValue):
    ...
    if valueSize < GAE_MEMCACHE_MAX_ITEM_SIZE:
