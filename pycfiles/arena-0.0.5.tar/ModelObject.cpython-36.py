...
# Embedded file name: D:\ProgramData\lib\site-packages\arelle\ModelObject.py
# Compiled at: 2018-02-26 09:10:06
# Size of source mod 2**32: 16219 bytes
...
class ModelObject(etree.ElementBase):
    ...
    def _init(self):
        ...
        if parent is not None:
            if hasattr(parent, 'modelDocument'):
    ...
    def vQname(self, validationModelXbrl=None):
        if validationModelXbrl is not None:
            if validationModelXbrl != self.modelXbrl:
    ...
    def elementDeclaration(self, validationModelXbrl=None):
        ...
        if validationModelXbrl is not None:
            if validationModelXbrl != elementModelXbrl:
    ...
    def _textNodes(self, recurse=False):
        if self.text:
            if getattr(self, 'xValid', 0) != VALID_NO_CONTENT:
                yield self.text
        for c in self.iterchildren():
            if recurse:
                if isinstance(c, ModelObject):
    ...
    def genLabel(self, role=None, fallbackToQname=False, fallbackToXlinkLabel=False, lang=None, strip=False, linkrole=None):
        ...
        if fallbackToQname:
            ...
            if fallbackToXlinkLabel:
                if hasattr(self, 'xlinkLabel'):
