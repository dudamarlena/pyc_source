...
# Embedded file name: D:\ProgramData\lib\site-packages\arelle\FunctionXfi.py
# Compiled at: 2018-02-26 09:10:06
# Size of source mod 2**32: 65126 bytes
...
def call(xc, p, localname, args):
    try:
        if localname not in xfiFunctions:
...
def instance(xc, p, args, i=0):
    if i >= len(args):
        ...
        if isinstance(xbrliXbrl, ModelObject):
            if xbrliXbrl.elementQname == XbrlConst.qnXbrliXbrl:
                return xbrliXbrl.modelXbrl
        if isinstance(xbrliXbrl, ModelXbrl):
            return xbrliXbrl
...
def xbrlTuple(xc, args, i=0):
    ...
    modelTuple = args[i][0]
    if isinstance(modelTuple, (ModelFact, ModelInlineFact)):
        if modelTuple.isTuple:
            return modelTuple
...
def unit_numerator(xc, p, args):
    if len(args) != 1:
        ...
        unit = args[0][0]
        if isinstance(unit, ModelObject):
            if unit.localName == 'unit':
                if unit.namespaceURI == XbrlConst.xbrli:
                    measuresParent = XmlUtil.descendant(unit, XbrlConst.xbrli, 'unitNumerator')
                    if measuresParent is None:
                        measuresParent = unit
...
def measure_name(xc, p, args):
    if len(args) != 1:
        ...
        if len(args[0]) != 1:
            raise XPathContext.FunctionArgType(1, 'xbrl:measure')
        unit = args[0][0]
        if isinstance(unit, ModelObject):
            if unit.localName == 'measure':
                if unit.namespaceURI == XbrlConst.xbrli:
...
def parent_child(args, parentName, childName, findDescendant=False):
    if len(args) != 1:
        ...
        if isinstance(parent, ModelObject):
            if parent.localName == parentName:
                if parent.namespaceURI == XbrlConst.xbrli:
...
def is_period_type(args, periodElement):
    if len(args) != 1:
        ...
        period = args[0][0]
        if isinstance(period, ModelObject):
            if period.localName == 'period':
                if period.namespaceURI == XbrlConst.xbrli:
...
def period_datetime(p, args, periodElement):
    if len(args) != 1:
        ...
        if len(args[0]) != 1:
            raise XPathContext.FunctionArgType(1, 'xbrl:period')
        period = args[0][0]
        if isinstance(period, ModelObject) == 1:
            if period.localName == 'period':
                if period.namespaceURI == XbrlConst.xbrli:
...
def infer_precision_decimals(xc, p, args, attrName):
    if len(args) != 1:
        ...
        if len(args[0]) != 1:
            raise XPathContext.FunctionArgType(1, 'xbrl:item', args[0])
        else:
            modelItem = xc.modelItem(args[0][0])
...
def checkXffFunctionUse(xc, p, functionName):
    if xc.progHeader is not None:
        if xc.progHeader.element is not None:
            try:
            ...
            if modelResourceElt is None or modelResourceElt.localName not in ('formula',
    ...
    if xc.variableSet is not None:
        if xc.variableSet.implicitFiltering == 'false':
...
def uncovered_aspects(xc, p, args, dimensionAspects=False):
    ...
    if xc.progHeader is not None:
        if xc.progHeader.element is not None:
            if xc.progHeader.element.localName not in ('formula', 'consistencyAssertion',
    ...
    return [a if dimensionAspects else aspectToToken.get(a) for a in uncoveredAspects if a != Aspect.DIMENSIONS if isinstance(a, QName) == dimensionAspects]
...
def s_equal_test(node1, node2):
    if isinstance(node1, (ModelFact, ModelInlineFact)):
        if node1.isItem:
            if isinstance(node2, (ModelFact, ModelInlineFact)):
                if node2.isItem:
                    return c_equal_test(node1, node2) and u_equal_test(node1, node2) and XbrlUtil.xEqual(node1, node2) and node1.xAttributes.get('precision') == node2.xAttributes.get('precision') and node1.xAttributes.get('decimals') == node2.xAttributes.get('decimals')
    if isinstance(node1, ModelObject):
        ...
        if isinstance(node1, ModelAttribute):
...
def date_equal_test(xc, p, args, instantEndDate):
    if len(args) != 2:
        ...
        raise isinstance(date1, (DateTime, datetime.date)) or XPathContext.FunctionArgType(2, 'xbrldi:dateUnion')
...
def non_nil_facts_in_instance(xc, p, args):
    if len(args) != 1:
        raise XPathContext.FunctionNumArgs()
    inst = instance(xc, p, args)
    return [f for f in inst.factsInInstance if f.isItem or f.isTuple if not f.isNil]
...
def concept_custom_attribute(xc, p, args):
    ...
    if qnAttr is None:
        raise XPathContext.FunctionArgType(2, 'xs:QName')
...
def concepts_from_local_name(xc, p, args):
    ...
    if len(args) == 2:
        ...
        return [c.qname for c in xc.modelXbrl.nameConcepts.get(localName, ()) if c.isItem or c.isTuple if bool(nsPattern.search(c.qname.namespaceURI))]
    else:
...
def concepts_from_local_name_pattern(xc, p, args):
    ...
    if len(args) == 2:
        nsPattern = re.compile(stringArg(xc, args, 1, 'xs:string'))
        return [c.qname for c in xc.modelXbrl.qnameConcepts.values() if c.isItem or c.isTuple if bool(localNamePattern.search(c.name)) if bool(nsPattern.search(c.qname.namespaceURI))]
    else:
        return [c.qname for c in xc.modelXbrl.qnameConcepts.values() if c.isItem or c.isTuple if bool(localNamePattern.search(c.name))]
...
def filter_member_network_selection(xc, p, args):
    if len(args) != 5:
        ...
        if axis not in ('descendant-or-self', 'child-or-self', 'descendant', 'child'):
            return ()
...
def filter_member_DRS_selection(xc, p, args):
    ...
    if axis not in ('DRS-descendant', 'DRS-child'):
        ...
        if dimConcept is None or dimConcept.qname is None or dimConcept.qname.namespaceURI == XbrlConst.xbrli:
            ...
            if not dimConcept.isDimensionItem:
                    ...
                    if hasHcRel.arcrole == XbrlConst.all:
                        ...
                        for hcDimRel in xc.modelXbrl.relationshipSet(XbrlConst.hypercubeDimension, dimELR).fromModelObject(hcConcept):
                            if dimConcept == hcDimRel.toModelObject:
                                filter_member_DRS_members(xc, xc.modelXbrl.relationshipSet(XbrlConst.dimensionDomain, hcDimRel.targetRole or dimELR).fromModelObject(dimConcept), axis, memConcept, False, set(), memSelectionQnames)
...
def filter_member_DRS_members(xc, fromRels, axis, memConcept, inSelection, visited, memSelectionQnames):
    for rel in fromRels:
        ...
        if rel.fromModelObject == memConcept or inSelection:
            memSelectionQnames.add(toConceptQname)
            nestedSelection = True
        if toConceptQname not in visited and (not nestedSelection or axis == 'DRS-descendant'):
            visited.add(toConcept)
            filter_member_DRS_members(xc, xc.modelXbrl.relationshipSet(XbrlConst.domainMember, rel.targetRole or rel.linkrole).fromModelObject(toConcept), axis, memConcept, nestedSelection, visited, memSelectionQnames)
...
def fact_dim_value(xc, p, args, dimType):
    ...
    dimConcept = xc.modelXbrl.qnameConcepts.get(qnDim)
    if dimConcept is None or not dimConcept.isDimensionItem:
        raise XPathContext.XPathException(p, 'xfie:invalid{0}DimensionQName'.format(dimType), _('Argument 1 {0} is not a dimension concept QName.').format(qnDim))
    if context is not None:
...
def fact_has_explicit_dimension(xc, p, args):
    ...
    return dimValue is not None and (isinstance(dimValue, QName) or dimValue.isExplicit)
...
def fact_explicit_dimension_value_value(xc, p, args):
    ...
    if context is not None:
        ...
        if qn == ():
            raise XPathContext.FunctionArgType(2, 'xbrl:QName')
        dimConcept = xc.modelXbrl.qnameConcepts.get(qn)
...
def fact_dimension_s_equal2(xc, p, args):
    ...
    if context1 is not None:
        if context2 is not None:
            ...
            if qn == ():
                raise XPathContext.FunctionArgType(3, 'xbrl:QName')
            modelConcept = xc.modelXbrl.qnameConcepts.get(qn)
            if modelConcept is None or not modelConcept.isDimensionItem:
                ...
                if dimValue2 is not None:
                    if isinstance(dimValue2, ModelDimensionValue):
...
def concept_label(xc, p, args):
    ...
    if srcConcept is None:
        ...
        if not labelroleURI:
            labelroleURI = XbrlConst.standardLabel
...
def arcrole_definition(xc, p, args):
    ...
    if modelArcroleTypes is not None:
        if len(modelArcroleTypes) > 0:
            arcroledefinition = modelArcroleTypes[0].definition
            if arcroledefinition is not None:
                return arcroledefinition
    return ()
...
def fact_footnotes(xc, p, args):
    if len(args) > 6:
        ...
        if not arcroleURI:
            arcroleURI = XbrlConst.factFootnote
        footnoteroleURI = stringArg(xc, args, 3, 'xs:string', emptyFallback='')
        footnoteroleURI = footnoteroleURI or XbrlConst.footnote
...
def concept_relationships(xc, p, args, nestResults=False):
    ...
    if not 4 <= lenArgs <= 8:
        ...
        if not linkroleURI:
            ...
            if axis not in ('descendant', 'child', 'ancestor', 'parent', 'sibling',
                            'sibling-or-self'):
                raise XPathContext.FunctionArgType(3, "'descendant', 'child', 'ancestor', 'parent', 'sibling' or 'sibling-or-self'", errCode='xfie:InvalidConceptRelationParameters')
            ...
            if lenArgs > 4:
                ...
                if axis in ('child', 'parent', 'sibling', 'sibling-or-self'):
                    if generations != 1:
                        raise XPathContext.FunctionArgType(4, "generations must be 1 for 'child', 'parent', 'sibling' or 'sibling-or-self' axis", errCode='xfie:InvalidConceptRelationParameters')
            else:
                if axis in ('child', 'parent', 'sibling', 'sibling-or-self'):
                    generations = 1
                else:
        ...
        if qnSource == XbrlConst.qnXfiRoot:
            ...
            if axis == 'descendant':
                ...
                if axis == 'ancestor':
                    ...
                    if axis in ('sibling', 'sibling-or-self'):
                        rels = relationshipSet.toModelObject(srcConcept)
                        if rels:
            ...
            if generations > 1 or generations == 0 and conceptQname not in visited:
                ...
                if axis == 'descendant':
                    ...
                    if relationshipSet.arcrole == 'XBRL-dimensions':
                        ...
                        stepRels = [rel for rel in stepRelationshipSet.toModelObject(concept) if rel.consectuiveLinkrole == modelRel.linkrole]
                    else:
                        stepRelationshipSet = relationshipSet
...
def distinct_nonAbstract_parent_concepts(xc, p, args):
    ...
    if relationshipSet:
        for rel in relationshipSet.modelRelationships:
            ...
            if isinstance(fromModelObject, ModelConcept) and isinstance(toModelObject, ModelConcept) and not fromModelObject.isAbstract and not toModelObject.isAbstract:
...
def create_element(xc, p, args):
    if not 2 <= len(args) <= 4:
        ...
        if attrArg:
            if len(attrArg) & 1 or any(not isinstance(arg, (QName, _STR_BASE)) or isinstance(arg, _STR_BASE) and NCNamePattern.match(arg) is None for i in range(0, len(attrArg), 2) for arg in (
             attrArg[i],)):
                raise XPathContext.FunctionArgType(1, '((xs:qname|xs:string),xs:anyAtomicValue)', errCode='xfie:AttributesNotNameValuePairs')
            else:
                attrParam = [(attrArg[i], attrArg[(i + 1)]) for i in range(0, len(attrArg), 2)]
        else:
            attrParam = None
        ...
        if scratchpadXmlDocUrl in xc.modelXbrl.urlDocs:
            ...
            modelDocument = ModelDocument.create((xc.modelXbrl), (ModelDocument.Type.UnknownXML),
              scratchpadXmlDocUrl,
              initialXml="<xfc:dummy xmlns:xfc='http://www.xbrl.org/2012/function/creation'/>")
        newElement = XmlUtil.addChild((modelDocument.xmlRootElement), qn,
