...
# Embedded file name: /tmp/pip-install-ncu5lfw4/arelle/arelle/ModelVersReport.py
# Compiled at: 2018-08-09 04:11:41
# Size of source mod 2**32: 74021 bytes
__doc__ = '\nCreated on Nov 11, 2010\n\n@author: Mark V Systems Limited\n(c) Copyright 2010-2013 Mark V Systems Limited, All rights reserved.\n'
import os, re, datetime
from collections import defaultdict
from arelle import XbrlConst, XbrlUtil, XmlUtil, UrlUtil, ModelXbrl, ModelDocument, ModelVersObject, Version, XmlValidate
...
def create(modelXbrlFromDTS, modelXbrlToDTS):
    ...
    modelVersReport = ModelVersReport(ModelDocument.Type.VERSIONINGREPORT)
...
authoritiesEquivalence = {'http://xbrl.iasb.org': 'IFRS', 'http://xbrl.ifrs.org': 'IFRS', 
 'http://xbrl.us': 'XBRL-US', 'http://fasb.org': 'XBRL-US', 'http://xbrl.sec.gov': 'XBRL-US'}
dateRemovalPattern = re.compile('[/]?(draft-)?(19|20)[0-9][0-9](-[01][0-9](-[0-3][0-9])?)?')
numberRemovalPattern = re.compile('[/]?[0-9][0-9\\.]*')

class ModelVersReport(ModelDocument.ModelDocument):
    """ModelVersReport"""

    def __init__(self, modelXbrl, type=ModelDocument.Type.VERSIONINGREPORT, uri=None, filepath=None, xmlDocument=None):
    ...
    def versioningReportDiscover(self, rootElement):
        ...
        """
        XmlValidate.validate(self.modelXbrl, rootElement)
        ...
        try:
            for modelObject in rootElement.iterdescendants():
                if isinstance(modelObject, ModelObject):
                    ...
                    if ns == XbrlConst.ver:
                        if ln == 'action':
                            ...
                            actionRelatedFromMdlObjs = []
                            actionRelatedToMdlObjs = []
                    if (ln == 'fromDTS' or ln == 'toDTS') and not getattr(self, ln):
                        ...
                        if schemaRefElts:
                            if len(schemaRefElts) == 1 and schemaRefElts[0].get('{http://www.w3.org/1999/xlink}href') is not None:
                                DTSmodelXbrl = ModelXbrl.load(self.modelXbrl.modelManager, schemaRefElts[0].get('{http://www.w3.org/1999/xlink}href'), 'loading validation report', base=self.baseForElement(schemaRefElts[0]))
                            else:
                                DTSmodelXbrl = ModelXbrl.create(self.modelXbrl.modelManager, newDocumentType=ModelDocument.Type.DTSENTRIES, url=self.uri[:-4] + '-' + ln + '.dts', isEntry=True)
                                ...
                                for schemaRefElt in schemaRefElts:
                                    if schemaRefElt.get('{http://www.w3.org/1999/xlink}href') is not None:
                                        doc = ModelDocument.load(DTSmodelXbrl, schemaRefElt.get('{http://www.w3.org/1999/xlink}href'), base=self.baseForElement(schemaRefElt))
                                        DTSdoc.referencesDocument[doc] = 'import'
                                        doc.inDTS = True

                            if DTSmodelXbrl is not None:
                                setattr(self, ln, DTSmodelXbrl)
                        else:
                            if ln in ('namespaceRename', 'roleChange') and modelAction is not None:
                                modelAction.events.append(modelObject)
                    elif not self.fromDTS is None:
                        if self.toDTS is None:
                            pass
                else:
                    if ns in (XbrlConst.vercu, XbrlConst.vercb):
                        if ln in ('conceptRename', 'conceptAdd', 'conceptDelete') and modelAction is not None:
                            modelAction.events.append(modelObject)
                        if ln == 'conceptRename':
                        ...
                        if ns in (XbrlConst.vercd, XbrlConst.verce):
                            if ln in {'conceptIDChange', 'tupleContentModelChange', 'conceptBlockChange', 'conceptReferenceAdd', 'conceptDefaultChange', 'conceptAttributeChange', 'conceptReferenceDelete', 'conceptAbstractChange', 'conceptBalanceChange', 'conceptSubstitutionGroupChange', 'conceptLabelDelete', 'conceptNillableChange', 'conceptFinalChange', 'conceptAttributeDelete', 'conceptLabelAdd', 'conceptLabelChange', 'conceptFixedChange', 'conceptReferenceChange', 'conceptPeriodTypeChange', 'conceptTypeChange', 'conceptAttributeAdd'} and modelAction is not None:
                                modelAction.events.append(modelObject)
                        else:
                            if ns == XbrlConst.verrels:
                                if ln in {'relationshipSetModelDelete', 'relationshipSetModelAdd', 'relationshipSetModelChange'}:
                                    if modelAction is not None:
                                        modelAction.events.append(modelObject)
                                    modelRelationshipSetEvent = modelObject
                                else:
                                    if ln in ('fromRelationshipSet', 'toRelationshipSet'):
                                    ...
                                    elif ln == 'relationships' and modelRelationshipSet is not None:
                ...
                if ln in ('aspectModelChange', 'aspectModelAdd', 'aspectModelDelete'):
                    ...
                    if ln in ('fromAspects', 'toAspects'):
                        ...
                        if ln in ('concepts', 'explicitDimension', 'typedDimension',
                                  'segment', 'scenario', 'entityIdentifier', 'period',
                                  'location', 'unit'):
            ...
            self.modelXbrl.modelManager.addToLog('discovery: {0} error {1}'.format(os.path.basename(self.uri), err))
    ...
    def diffDTSes(self, reportOutput, fromDTS, toDTS, assignment='technical', schemaDir=None):
        ...
        :type schemaDir: str
        """
        versReportFile = str(reportOutput)
        ...
        if ':' in assignment:
            ...
            if assignment.startswith('technical'):
                categoryType = 'technicalCategory'
            else:
                if assignment.startswith('business'):
                    categoryType = 'businessCategory'
                else:
                    categoryType = 'errataCategory'
            import io
            file = io.StringIO('<nsmap><!-- Generated by Arelle(r) version {3} at {4} --><report  xmlns="http://xbrl.org/2013/versioning-base"  xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"  xmlns:link="http://www.xbrl.org/2003/linkbase"  xmlns:xlink="http://www.w3.org/1999/xlink"><!-- link:linkbaseRef xlink:type="simple"  xlink:arcrole="http://www.w3.org/1999/xlink/properties/linkbase"  xlink:title="documentation"  xlink:href="sample.xml"/ --><fromDTS>{0}</fromDTS><toDTS>{1}</toDTS><assignment id="versioningTask"><{2}/></assignment></report></nsmap>'.format(''.join(['<link:schemaRef xlink:type="simple" xlink:href="{0}"/>'.format(self.relativeUri(uri)) for uri in self.entryURIs(fromDTS)]), ''.join(['<link:schemaRef xlink:type="simple" xlink:href="{0}"/>'.format(self.relativeUri(uri)) for uri in self.entryURIs(toDTS)]), categoryType, Version.version, format(datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC'))))
            ...
            if schemaDir is not None:
                ...
                self.reportElement.set('{http://www.w3.org/2001/XMLSchema-instance}schemaLocation', ' '.join(schemaLocations))
            self.modelXbrl.modelManager.showStatus(_('Checking report file'))
            self.modelXbrl.modelDocument = self
    ...
    def diffNamespaces(self):
        ...
        self.diffURIs(fromNSes, toNSes, 'namespaceRename', (
         self.rolePathlessDatelessMatchPattern, self.roleNumlessMatchPattern, self.roleNoFromToMatchPattern), self.namespaceRenameFromURI, self.namespaceRenameToURI)

    def diffRoles(self):
        self.roleChangeFromURI = {}
        self.roleChangeToURI = {}
        self.diffURIs(set(self.fromDTS.roleTypes.keys()), set(self.toDTS.roleTypes.keys()), 'roleChange', (
         self.rolePathlessDatelessMatchPattern, self.roleNumlessMatchPattern, self.roleNoFromToMatchPattern), self.roleChangeFromURI, self.roleChangeToURI)
    ...
    def roleNumlessMatchPattern(self, role):
        ...
        return ''.join((c if str.isalpha(c) or c == '/' else '') for c in basepart) + (sep + lastpart if lastpart else '')
    ...
    def diffConcepts(self):
        ...
        for fromConceptQname, fromConcept in self.fromDTS.qnameConcepts.items():
            if not fromConcept.isItem and not fromConcept.isTuple:
                ...
                if toConceptQname in self.toDTS.qnameConcepts:
                    ...
                    if fromConcept.isItem and toConcept.isItem and fromConcept.typeQname != self.fromDTSqname(toConcept.typeQname):
                    ...
                    if fromConcept.isItem and toConcept.isItem:
                        ...
                        if fromConcept.isTuple and toConcept.isTuple:
                            fromType = fromConcept.type
                            toType = toConcept.type
                            if fromType is not None and toType is not None and not XbrlUtil.nodesCorrespond(self.fromDTS, fromType, toType, self.toDTS):
                            ...
                            for concept, attrs in ((fromConcept, fromCustAttrs), (toConcept, toCustAttrs)):
                                for attrName, attrValue in concept.items():
                                    ...
                                    if attrName not in ('abstract', 'block', 'default',
                                                        'final', 'fixed', 'form',
                                                        'id', 'maxOccurs', 'minOccurs',
                                                        'name', 'nillable', 'ref',
                                                        'substitutionGroup', 'type') and attrQname.namespaceURI != XbrlConst.xbrli and attrQname.namespaceURI != XbrlConst.xbrldt:
                            ...
                            for event, arcroles in (('vercd:conceptLabel', (XbrlConst.conceptLabel, XbrlConst.elementLabel)),
                             (
                              'vercd:conceptReference', (XbrlConst.conceptReference, XbrlConst.elementReference))):
                                fromResources = {}
                                toResources = {}
                                for dts, concept, resources in ((self.fromDTS, fromConcept, fromResources), (self.toDTS, toConcept, toResources)):
                                    for arcrole in arcroles:
                                        ...
                                        if resourcesRelationshipSet:
                                            for rel in resourcesRelationshipSet.fromModelObject(concept):
                                                ...
                                                key = (
                                                 rel.linkrole, arcrole, resource.role, resource.xmlLang, rel.linkQname, rel.qname, resource.qname) + XbrlUtil.attributes(dts, rel.arcElement, exclusions=(XbrlConst.xlink, 'use', 'priority', 'order', 'id')) + XbrlUtil.attributes(dts, resource, exclusions=XbrlConst.xlink)
        ...
        for toConceptQname, toConcept in self.toDTS.qnameConcepts.items():
            if (toConcept.isItem or toConcept.isTuple) and toConceptQname not in toConceptsMatched:
    ...
    def diffRelationshipSet(self, arcrole):
        ...
        for dts, linkRoleUris, otherRoleUris, roleChanges, e1, e2, isFrom in ((self.fromDTS, fromLinkRoleUris, toLinkRoleUris, self.roleChangeFromURI, 'relationshipSetModelDelete', 'fromRelationshipSet', True),
         (
            ...
            for linkRoleUri in linkRoleUris:
                if not (linkRoleUri in otherRoleUris or linkRoleUri in roleChanges):
                    ...
                    for fromRoot in fromRoots:
                        ...
                        if toRootConcept is not None and toRootConcept not in toRoots:
                    ...
                    for toRoot in toRoots:
                        ...
                        if fromRootConcept is not None and fromRootConcept not in fromRoots:
    ...
    def diffRelationships(self, fromConcept, toConcept, fromRelationshipSet, toRelationshipSet):
        ...
        for i, fromRel in enumerate(fromRels):
            ...
            if toRel is not None and isinstance(toRel.toModelObject, ModelConcept) and toRel.toModelObject.qname == toTgtQname:
                fromRelAttrs = XbrlUtil.attributes(self.modelXbrl, fromRel.arcElement, exclusions=relationshipSetArcAttributesExclusion)
                toRelAttrs = XbrlUtil.attributes(self.modelXbrl, toRel.arcElement, exclusions=relationshipSetArcAttributesExclusion, ns2ns1Tbl=self.namespaceRenameToURI)
    ...
    def diffDimensionDefaults(self):
        ...
        for dts, dimDefaults, otherDimDefaults, otherDTSqname, e1, e2 in ((self.fromDTS, fromDimDefaults, toDimDefaults, self.toDTSqname, 'aspectModelDelete', 'fromAspects'),
         (
                ...
                if not otherDTSpriItemDRSrels:
                    ...
                    if priItemInheritRels:
                        priItemNetwork = self.createInstanceAspectsEvent('drsNetwork', (
                         ...
                         ('axis', 'descendant-or-self')), eventParent=priItem)
                    for dimRel, isNotAll, isClosed in self.DRSdimRels(dts, priItemDRSrels):
                        ...
                        explDim = self.createInstanceAspectsEvent('typedDimension' if dimConcept.isTypedDimension else 'explicitDimension', (
                         (
                          'name', dimConcept.qname),) + ((('excluded', 'true'), ) if isNotAll else ()), comment=self.typedDomainElementComment(dimConcept), eventParent=aspectEvent)
                        for domRel in self.DRSdomRels(dts, dimRel):
                            ...
                            if domHasMemRels:
                                self.createInstanceAspectsEvent('drsNetwork', (
                                 ...
                                 (
                                  'arcrole', XbrlConst.domainMember),
                                 ('axis', 'descendant-or-self')), eventParent=member)
                             ...
                             (
                              otherDTSpriItemQname, toDimRel, 'toAspects', False)):
                                if dimRel is not None:
                                    ...
                                    for domRel in self.DRSdomRels(dts, dimRel):
                                        domHasMemRels = dts.relationshipSet(XbrlConst.domainMember, linkrole).fromModelObject(priItemConcept)
                                        self.createInstanceAspectsEvent('member', (('name', domRel.toModelObject.qname),) + ((('linkrole', domRel.linkrole), ('arcrole', XbrlConst.domainMember), ('axis', 'DRS-descendant-or-self')) if domHasMemRels else ()), eventParent=explDim)

    def DRSdimRels(self, dts, priItemDRSrels):
        return [(dimRel, hcRel.arcrole == XbrlConst.notAll, hcRel.isClosed) for hcRel in priItemDRSrels for dimRel in dts.relationshipSet(XbrlConst.hypercubeDimension, hcRel.consecutiveLinkrole).fromModelObject(hcRel.toModelObject)]
        ...
        for i, fromRel in enumerate(fromRels):
            ...
            if toRel is not None and isinstance(toRel.toModelObject, ModelConcept) and toRel.toModelObject.qname == toTgtQname:
                ...
                if fromRelAttrs != toRelAttrs:
                    ...
                    self.DRSdiff(fromTgtConcept, fromRel.consecutiveLinkrole, toTgtConcept, toRel.consecutiveLinkrole, arcrole, diffs)
            else:
    ...
    def DRShcDiff(self, fromDTS, fromPriItemDRSrels, toDTS, toPriItemDRSrels):
        ...
        for i, fromHcRelKey in enumerate(fromHcRels.keys()):
            ...
            try:
                ...
                fromRelAttrs = XbrlUtil.attributes(self.modelXbrl, fromHcRel.arcElement, exclusions=relationshipSetArcAttributesExclusion)
                toRelAttrs = XbrlUtil.attributes(self.modelXbrl, toHcRel.arcElement, exclusions=relationshipSetArcAttributesExclusion, ns2ns1Tbl=self.namespaceRenameToURI)
                if fromRelAttrs != toRelAttrs:
    ...
    def typedDomainIsDifferent(self, fromDimConcept, toDimConcept):
        try:
            ...
            isCorresponding = fromTypedDomain is not None and toTypedDomain is not None and XbrlUtil.sEqual(self.fromDTS, fromTypedDomain, toTypedDomain, excludeIDs=XbrlUtil.ALL_IDs_EXCLUDED, dts2=self.toDTS, ns2ns1Tbl=self.namespaceRenameToURI)
    ...
    def DRSdimsDiff(self, fromDTS, fromPriItemDRSrels, toDTS, toPriItemDRSrels):
        ...
        for i, fromDimKey in enumerate(fromDims.keys()):
            ...
            if toDimRel is not None:
                ...
                mbrDiffs = self.DRSdiff(fromDimConcept, fromDimRel.consecutiveLinkrole, toDimConcept, toDimRel.consecutiveLinkrole, XbrlConst.dimensionDomain)
                dimsCorrespond = True
                if fromDimConcept.isTypedDimension:
                    ...
                    if toDimConcept.isTypedDimension:
                        dimsCorrespond = False
    ...
    def toDTSqname(self, fromDTSqname):
        if fromDTSqname is not None and fromDTSqname.namespaceURI in self.namespaceRenameFromURI:
            return qname(self.namespaceRenameFromURI[fromDTSqname.namespaceURI], fromDTSqname.localName)
        return fromDTSqname

    def fromDTSqname(self, toDTSqname):
        if toDTSqname and toDTSqname.namespaceURI in self.namespaceRenameToURI:
            return qname(self.namespaceRenameToURI[toDTSqname.namespaceURI], toDTSqname.localName)
        return toDTSqname
    ...
    def createConceptEvent(self, eventNS, eventName, fromConcept=None, toConcept=None, action=None, fromCustomAttribute=None, toCustomAttribute=None, fromResource=None, toResource=None, fromValue=None, toValue=None, fromResourceText=None, toResourceText=None):
        ...
        if fromCustomAttribute is not None:
            if fromCustomAttribute.namespaceURI:
        ...
        if toCustomAttribute is not None:
            if toCustomAttribute.namespaceURI:
    ...
    def createInstanceAspectsEvent(self, eventName, eventAttributes=None, comment=None, eventParent=None):
        if eventParent is None:
            eventParent = self.createAction()
        eventElement = XmlUtil.addChild(eventParent, XbrlConst.verdim, 'verdim:' + eventName, attributes=tuple((name, XmlUtil.addQnameValue(self.reportElement, val) if isinstance(val, QName) else val) for name, val in eventAttributes) if eventAttributes else None)
