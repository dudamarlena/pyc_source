...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/site-packages/wagtail_feeds/feeds.py
# Compiled at: 2018-05-08 10:24:10
# Size of source mod 2**32: 8771 bytes
import json
...
try:
    feed_app_settings = RSSFeedsSettings.for_site(site=Site.objects.get(is_default_site=True))
...
class BasicFeed(Feed):
    ...
    def items(self):
        if feed_item_date_field:
            return feed_model.objects.live().order_by('-' + feed_item_date_field)
        else:
