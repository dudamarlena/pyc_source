...
# [GCC 8.4.0]
# Embedded file name: /Users/clareau/dat/Research/AryeeResearch/lareau_dev/mgatk/mgatk/cli.py
# Compiled at: 2020-01-27 16:00:07
...
def main(mode, input, output, name, mito_genome, ncores, cluster, jobs, barcode_tag, barcodes, min_barcode_reads, nhmax, nmmax, keep_duplicates, umi_barcode, max_javamem, proper_pairs, base_qual, alignment_quality, emit_base_qualities, nsamples, keep_samples, ignore_samples, keep_temp_files, keep_qc_bams, skip_r, snake_stdout):
    """
    ...
    if mode == 'bcall':
        if barcode_tag == 'X':
            ...
            os.path.exists(input + '.bai') or click.echo(gettime() + 'Attempting to index: ' + input)
            pysam.index(input)
            ...
            if mito_length == bam_length:
                ...
                if bam_length == 16569:
                    ...
                    if bam_length == 16571:
                        ...
                        quit()
            barc_quant_file = barcode_known or of + '/final/barcodeQuants.tsv'
    ...
    if mode == 'call' or mode == 'one':
        ...
        dict1 = {'input_directory':sqs(input), 
         'output_directory':sqs(output),  'script_dir':sqs(script_dir),  'fasta_file':sqs(fastaf), 
         'mito_chr':sqs(mito_chr),  'mito_length':sqs(mito_length),  'base_qual':sqs(base_qual), 
         'remove_duplicates':sqs(remove_duplicates),  'umi_barcode':sqs(umi_barcode),  'alignment_quality':sqs(alignment_quality), 
         'emit_base_qualities':sqs(emit_base_qualities),  'proper_paired':sqs(proper_pairs), 
         'NHmax':sqs(nhmax), 
         'NMmax':sqs(nmmax),  'max_javamem':sqs(max_javamem)}
        if mode == 'call':
            snakeclust = ''
            njobs = int(jobs)
            if njobs > 0:
    ...
    if mode == 'gather' or mode == 'call':
        ...
        dict2 = {'mgatk_directory':sqs(mgatk_directory), 
         'name':sqs(name),  'script_dir':sqs(script_dir)}
