...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/CryDeer/database.py
# Compiled at: 2017-01-06 22:20:30
...
class Database:
    ...
    def get_new_item_id(self):
        query = self.session.query(Item).order_by(Item.id)
        if query.count() > 0:
            return query[(-1)].id + 1
        else:
            return 0
    ...
    def get_new_info_id(self):
        query = self.session.query(Info).order_by(Info.id)
        if query.count() > 0:
            return query[(-1)].id + 1
        else:
            return 0
    ...
    def find_item(self, nu):
        try:
            item = self.session.query(Item).filter(Item.nu == nu).one()
            return item
    ...
    def delete_info(self, nu):
        try:
            query = self.session.query(Info).filter(Info.nu == nu)
            for info in query:
    ...
    def display(self):
        for instance in self.session.query(Item).order_by(Item.id):
            print(instance)
