...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.7/site-packages/hydraulic/pipes.py
# Compiled at: 2020-01-31 09:10:08
# Size of source mod 2**32: 17775 bytes
"""
...
class StraightPipe:
    ...
    def Dict(self):
        p1, p2 = self.points
        d = {'p1':p1.Dict(),  'p2':p2.Dict(),  'd':self.radius * 2,  'heat_exhcange':self.heat_exchange, 
         'length':float(self.length),  'fQ':float(self.fQ), 
         'name':self.name}
        return d
...
class StraightPipe3D(StraightPipe):
    ...
    def CADVolume(self):
        ...
        axis.Normalize()
        return p3D.HollowCylinder((0.5 * (self.points[0] + self.points[1])), axis, (self.radius),
          (self.radius + 0.001), (self.length),
          name=(self.name))
...
class Bend(SingularPipe):
    ...
    def __init__(self, start_point, interior_point, end_point, arc, diameter, heat_exchange=True, name=''):
        ...
        self.fQ = 32 * length_d / (math.pi * self.radius ** 3)
        self.n_equations = 2
    ...
    def Dict(self):
        ...
        p2 = self.end_point
        d = {'p1':p1.Dict(),  'p':p.Dict(),  'p2':p2.Dict(),  'd':self.radius * 2, 
         'length':float(self.length),  'heat_exhcange':self.heat_exchange, 
         'fQ':float(self.fQ), 
         'name':self.name}
        return d
...
class Bend2D(Bend):
    ...
    def __init__(self, start_point, interior_point, end_point, diameter, heat_exchange=True, name=''):
        arc = vm.Arc2D(start_point, interior_point, end_point)
        Bend.__init__(self, start_point, interior_point, end_point, arc, diameter, heat_exchange, name)
...
class UserDefined(SingularPipe):
    ...
    def __init__(self, p1, p2, fQ, heat_exchange=True, name=''):
    ...
    def CADVolume(self):
        ...
        l = axis.Norm()
        axis.Normalize()
        return p3D.HollowCylinder((0.5 * (self.points[0] + self.points[1])), axis, 0.001,
          0.002, l,
          name=(self.name))
    ...
    def Dict(self):
        p1, p2 = self.points
        d = {'p1':p1.Dict(),  'p2':p2.Dict(),  'heat_exhcange':self.heat_exchange, 
         'fQ':float(self.fQ), 
         'name':self.name}
        return d
...
class JunctionPipe:
    ...
    def __init__(self, central_point, other_points, diameter, name=''):
        ...
        self.fQs = [32 * ld / (math.pi * self.radius ** 3) for ld in length_d]
    ...
    def SystemMatrix(self, constant):
        ...
        matrix_generator = [
         [
          0, 0] + [0, 1] * len(self.active_points)]
    ...
    def Repr1D(self, j, points_index):
        line = 'Line({}) = [{},{}];\n'.format(j, points_index[self.active_points[0]], points_index[self.active_points[1]])
        return (line, [j])
