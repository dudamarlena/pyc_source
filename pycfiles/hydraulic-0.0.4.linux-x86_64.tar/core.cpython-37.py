...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.7/site-packages/hydraulic/core.py
# Compiled at: 2020-01-31 09:10:08
# Size of source mod 2**32: 20868 bytes
"""
Hydraulic circuit definition
"""
import math, numpy as npy, networkx as nx
...
mbd_angle = [
 0, 45, 90]
mbd_LD = [0, 15, 58]
enl_rap = [0.25, 0.5, 0.75, 1]
enl_LD = [30, 20, 6.5, 0]
ctr_rap = [1, 1.3333333333333333, 2, 4]
ctr_LD = [0, 6.5, 11, 15]
...
class Circuit:
    ...
    def GenerateGraph(self):
        ...
        if self.points:
            if self.pipes:
                active_points = {point for pipe in self.pipes for point in pipe.active_points}
                graph.add_nodes_from(active_points, node_type='point',
                  node_shape='o')
                graph.add_nodes_from((self.pipes), node_type='pipe',
                  node_shape='s')
                graph.add_nodes_from((self.boundary_conditions), node_type='boundary_condition',
                  node_shape='^')
    ...
    def ResolutionSettings(self):
        ...
        return (
         pressures_dict, flows_dict, equations_dict, neqs, nvars)
...
class Circuit2D(Circuit):
    ...
    def Export1D(self, name='Generated_Circuit', path='', size=1.0, index=0):
        """
        ...
        if index:
            return (
             pts_index, n_pipes + added_elems, physicals)
...
class Circuit3D(Circuit):
    ...
    def FreeCADExport(self, filename='An_unamed_circuit', python_path='python', path_lib_freecad='/usr/lib/freecad/lib', export_types='fcstd'):
        model = self.CADModel()
    ...
    def ToThermal(self, fluid_input_points, fluid_output_points):
        ...
        for pipe in self.circuit.pipes:
            ...
            if pipe.__class__ == hyp.JunctionPipe:
                ...
                block = th.Junction(input_nodes, output_nodes, input_flows, self.circuit.fluid, 'b' + str(block_count))
            else:
                j = self.flows_dict[(pipe, pipe.points[0])]
                th_node = th.Node('t' + str(block_count))
                wall_nodes.append(th_node)
                nodes.append(th_node)
                block = th.ThermalPipe(block_nodes + [th_node], self.solution[j], self.circuit.fluid, 'b' + str(block_count))
            block_count += 1
        ...
        interface_nodes = {'input':[point2node[fip] for fip in fluid_input_points],  'wall_nodes':wall_nodes, 
         'output':[point2node[fop] for fop in fluid_output_points]}
        thermal_circuit = th.Circuit(nodes, blocks)
        thermohydraulic_ciruit = th.ThermohydraulicCircuit(self.circuit, thermal_circuit, interface_nodes, point2node, pipe2block)
        return thermohydraulic_ciruit

    def DisplaySolution(self, x3D=vm.X3D, y3D=vm.Y3D, position=False, color_map=None, max_width=0.025):
        """
        ...
        flows = {}
        if position:
            graph_pos = {point:npy.array((npy.dot(point.vector, x3D.vector), npy.dot(point.vector, y3D.vector))) for point in list(self.circuit.graph.nodes) if graph.nodes[point]['node_type'] == 'point'}
            for pipe in pipes:
        ...
        for ppc in pipe_points_couples:
            if ppc in flows_dict.keys():
                ...
                edge_position = (
                 graph_pos[ppc[0]], graph_pos[ppc[1]])
        ...
        for ppc, flow in flows.items():
            ...
            if add_link:
                ...
                ax.add_patch(patches.FancyArrow(x, y, dx, dy, width=(abs(width * flow)), length_includes_head=True,
                  head_width=(abs(width * flow) * 2),
                  edgecolor='k',
                  facecolor='gray'))

        press_values = [self.solution[pressures_dict[point]] for point in points]
        press_pts = nx.draw_networkx_nodes(graph, pos=graph_pos,
          nodelist=points,
          node_color=press_values,
          node_size=100,
          cmap=color_map)
        nx.draw_networkx_nodes(graph, pos=graph_pos,
          nodelist=pipes,
          node_shape='s',
          node_color='k',
          node_size=50)
