...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.7/site-packages/hydraulic/thermal.py
# Compiled at: 2020-01-31 09:10:08
# Size of source mod 2**32: 23623 bytes
"""
...
class Junction(Block):
    ...
    def __init__(self, input_nodes, output_nodes, input_flows, fluid=water, name=''):
    ...
    def SystemEquations(self):
        ...
        for n_node in range(lon):
            line_generator = [
             0] * (len(self.input_nodes + self.output_nodes) * 2)
...
class Circuit:
    ...
    def AddResistor(self, node, resistance_factor, area_factor, boundary_condition):
        bc_node = boundary_condition.nodes[0]
        resistor = Resistor([node, bc_node], resistance_factor, area_factor)
    ...
    def Draw(self):
        ...
        nodes = [node for node in self.graph.nodes if self.graph.nodes[node]['node_type'] == 'node']
        blocks = [node for node in self.graph.nodes if self.graph.nodes[node]['node_type'] == 'block']
        nx.draw_networkx_nodes((self.graph), pos=graph_pos,
          nodelist=nodes,
          node_color='r',
          node_size=100)
        nx.draw_networkx_nodes((self.graph), pos=graph_pos,
...
class ThermalResult:
    ...
    def Display(self, ax=None, color_map='jet', max_width=0.01, position=None, x3D=vm.X3D, y3D=vm.Y3D):
        """
            ...
            if position is None:
                ...
                for bnc in block_node_couples:
                    if bnc in self.circuit.flows_dict.keys():
                        ...
                        edge_position = (
                         graph_pos[bnc[0]], graph_pos[bnc[1]])
                ...
                for bnc, heat_flow in heat_flows.items():
                    ...
                    if add_link:
                        ...
                        ax.add_patch(patches.FancyArrow(x, y, dx, dy, width=(abs(width * heat_flow)), length_includes_head=True,
                          head_width=(abs(width * heat_flow) * 2),
                          edgecolor='k',
                          facecolor='gray'))

                temp_values = [self.solution[self.circuit.temperature_dict[node]] for node in nodes]
                temp_pts = nx.draw_networkx_nodes(graph, pos=graph_pos,
                  nodelist=nodes,
                ...
                graph_pos = {node:(point.Dot(x3D), point.Dot(y3D)) for node, point in position.items()}
                ...
                edges = [edge for edge in graph.edges() if edge[0] in nodes if edge[1] in block_pos.keys()]
                temp_values = [self.solution[self.circuit.temperature_dict[node]] for node in nodes]
                temp_pts = nx.draw_networkx_nodes(graph, pos=graph_pos,
                  nodelist=nodes,
                  node_color=temp_values,
                  node_size=100,
                  cmap=color_map)
                nx.draw_networkx_nodes(graph, pos=block_pos,
                  nodelist=(block_pos.keys()),
                  node_shape='s',
                  node_color='k',
                  node_size=10)
                graph_pos.update(block_pos)
                nx.draw_networkx_edges(graph, pos=graph_pos,
                  edgelist=edges)
        ax.axis('equal')
...
class ThermohydraulicCircuit:
    ...
    def __init__(self, hydraulic_circuit, thermal_circuit, interface_nodes, point2node, pipe2block):
    ...
    def Draw(self, x3D=vm.X3D, y3D=vm.Y3D, position=False):
        ...
        """
        if position:
            graph_pos = {node:(point.Dot(x3D), point.Dot(y3D)) for node, point in self.node2point.items()}
            thermal_graph = self.thermal_circuit.graph
            nodes = graph_pos.keys()
            nx.draw_networkx_nodes(thermal_graph, pos=graph_pos,
              nodelist=nodes,
              node_color='r',
              node_size=100)
            block_pos = {}
            ...
            nx.draw_networkx_nodes(thermal_graph, pos=block_pos,
              nodelist=blocks,
              node_shape='s',
              node_color='k',
              node_size=10)
            graph_pos.update(block_pos)
            edges = [edge for edge in thermal_graph.edges if edge[0] in nodes if edge[1] in blocks]
            nx.draw_networkx_edges(thermal_graph, pos=graph_pos,
              edgelist=edges)
        else:
