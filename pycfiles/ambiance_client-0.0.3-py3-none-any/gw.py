...
# Embedded file name: build/bdist.linux-x86_64/egg/ambhas/gw.py
# Compiled at: 2012-02-15 04:06:11
...
class GW_1D:
    ...
    def run_model(self, hini, t):
        ...
        self.h = h + self.hmin
        self.discharge = u[:t] - self.sy * np.diff(h)
...
class GW_2D_yz:
    ...
    def run_model(self, hini, t):
        ...
        """
        u = self.r * self.R - self.Dnet
        h = np.empty((t + 1, self.nlayer))
        h[0, :] = np.array(hini) - self.hmin
        for k in range(t):
    ...
    def ens(self, lam_lim, sy_lim, r_lim, hmin_lim, ens, hini, h_obs, t):
        ...
        lam_ens = np.tile(lam_lim[0, :], (ens, 1)) + (np.tile(lam_lim[1, :], (ens, 1)) - np.tile(lam_lim[0, :], (ens, 1))) * R_v
        R_v = np.random.rand(ens, nlayer)
        sy_ens = np.tile(sy_lim[0, :], (ens, 1)) + (np.tile(sy_lim[1, :], (ens, 1)) - np.tile(sy_lim[0, :], (ens, 1))) * R_v
        r_ens = r_lim[0] + (r_lim[1] - r_lim[0]) * np.random.rand(ens)
        hmin_ens = hmin_lim[0] + (hmin_lim[1] - hmin_lim[0]) * np.random.rand(ens)
...
if __name__ == '__main__':
    ...
    xi = np.linspace(GT[0] + GT[1] / 2, GT[0] + GT[1] * (dataset.RasterXSize - 0.5), dataset.RasterXSize)
    yi = np.linspace(GT[3] + GT[5] / 2, GT[3] + GT[5] * (dataset.RasterYSize - 0.5), dataset.RasterYSize)
