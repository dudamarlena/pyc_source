...
# Embedded file name: /Users/maxk/Projects/OpenSource/arango-python/arango/aql.py
# Compiled at: 2013-12-07 05:05:59
# Size of source mod 2**32: 12512 bytes
import logging
...
class Func(object):
        ...
        for key, val in d.items():
            if isinstance(val, (list, tuple)):
                ...
                if isinstance(val, dict):
                    ...
                    if issubclass(type(val), AQLQuery):
                        val = val.build_query()
                    else:
                        if issubclass(type(val), Variable):
                            val = val.value
                        else:
                            val = '"{}"'.format(val)
            pairs.append('"{name}": {value}'.format(name=key, value=val))
    ...
    def build_query(self):
        ...
        """
        return '{}({})'.format(self.name, ', '.join(self.proceed_list(self.args)))
...
class AQLQuery(object):
    ...
    def collect(self, *pairs, **kwargs):
        ...
        """
        if len(pairs) % 2 != 0:
            raise ValueError('Arguments should be pairs variable-name and expression')
    ...
    def expr_return(self):
        ...
        if isinstance(return_expr, dict):
            ...
            return '{{{}}}'.format(', '.join(pairs))
        if return_expr and isinstance(return_expr, (tuple, list)):
    ...
    def expr_nested(self):
        ...
        for n, expr in enumerate(self.nested_expr):
            if not issubclass(type(expr), AQLQuery):
                raise ValueError('Nested expressions have to besubclass of AQLQuery')
    ...
    def expr_let(self):
        ...
        for name, expr in self.let_expr:
            pairs.append('LET {name} = {expr}'.format(name=name, expr=self.process_expr(expr)))
    ...
    def expr_collect(self):
        ...
        for pairs, into in self.collect_expr:
            ...
            collect.append('COLLECT {pairs}{into}'.format(pairs=', '.join(exprs), into=into))
    ...
    def build_nested_query(self, n):
        ...
        return 'FOR {for_var} IN {for_expr}\n'.format(for_var=for_var, for_expr=self.expr_for)
    ...
    def build_query(self):
        ...
        if self._built_query is not None and self._no_cache is False:
            ...
            query = '\n            FOR {for_var} IN {for_expr}\n                {for_nested}{let_expr}{filter_expr}\n                {collect_expr}{sort_expr}{limit_expr}\n            RETURN\n                {return_expr}\n        '.format(for_var=self.for_var, for_expr=self.expr_for, for_nested=self.expr_nested, let_expr=self.expr_let, filter_expr=self.expr_filter, collect_expr=self.expr_collect, sort_expr=self.expr_sort, limit_expr=self.expr_limit, return_expr=self.expr_return)
            logger.debug(query)
    ...
    def execute(self, wrapper=None):
        ...
        return Cursor(self.connection, self.build_query(), **self.cursor_args)
