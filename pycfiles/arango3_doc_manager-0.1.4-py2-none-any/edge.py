...
# Embedded file name: /Users/maxk/Projects/OpenSource/arango-python/arango/edge.py
# Compiled at: 2013-12-04 14:42:04
import copy, logging
from .utils import parse_meta
from .mixins import ComparsionMixin
from .document import Document
from .exceptions import EdgeAlreadyCreated, EdgeNotYetCreated, EdgeIncompatibleDataType, DocumentIncompatibleDataType
...
class Edges(object):
    ...
    def count(self):
        ...
        raise NotImplementedError('This functionality not implemented yet.Use ``connection.query`` with custom wrapper')
...
class Edge(ComparsionMixin):
    ...
    def __init__(self, collection=None, _id=None, _rev=None, _from=None, _to=None, **kwargs):
        self.connection = collection.connection
        self.collection = collection
        self._body = None
        self._id = _id
    ...
    def from_document(self):
        ...
        if not self._from:
            ...
            if not self._from_document:
                self._from_document = Document(collection=self.collection, connection=self.connection, id=self._from)
    ...
    def to_document(self):
        ...
        if not self._to:
            ...
            if not self._to_document:
                self._to_document = Document(collection=self.collection, connection=self.connection, id=self._to)
    ...
    def __eq__(self, other):
        ...
        if super(Edge, self).__eq__(other) is False:
            return False
        if self._from == other._from and self._to == other._to:
            return True
        return False

    def __repr__(self):
        return ('<ArangoDB Edge: Id {0}/{1}, From {2} to {3}>').format(self._id, self._rev, self._from, self._to)
    ...
    def create(self, from_doc, to_doc, body=None, **kwargs):
        ...
        **document-handle** or instances of ``Document`` object.
        ...
        if self.id is not None:
            raise EdgeAlreadyCreated(('This edge already created with id {0}').format(self.id))
        ...
        data.update({'_from': from_doc_id, 
           '_to': to_doc_id})
        response = self.connection.post(self.connection.qs(self.EDGE_PATH, **params), data=json.dumps(body or {}))
    ...
    def delete(self):
        ...
        """
        response = self.connection.delete(self.DELETE_EDGE_PATH.format(self.id))
    ...
    def update(self, body, from_doc=None, to_doc=None, save=True, **kwargs):
        ...
        if not self._id or not self._from or not self._to:
            raise EdgeNotYetCreated('Sorry, you try to update Edge which is not yet created')
        ...
        if not issubclass(type(body), dict) and body is not None:
            raise EdgeIncompatibleDataType('Body should be None (empty) or instance or subclass of `dict` data type')
        if body is not None:
    ...
    def save(self, **kwargs):
        ...
        data.update({'_from': self._from, 
           '_to': self._to})
        response = self.connection.put(self.UPDATE_EDGE_PATH.format(self.id), data=data, **kwargs)
