...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\acerim\aceclasses.py
...
import numpy as np, pandas as pd, gdal
...
class AceDataset(object):
    ...
    **kwargs : ...
    ...
    def __init__(self, dataset, nlat=None, slat=None, wlon=None, elon=None, radius=None, ppd=None, **kwargs):
        ...
        if isinstance(dataset, str):
            ...
            args = [
             nlat, slat, wlon, elon, radius, ppd]
    ...
    def get_roi(self, lat, lon, rad, wsize=1, mask_crater=False, plot_roi=False, get_extent=False):
        """
        ...
        if minlat < self.slat or maxlat > self.nlat:
            raise ImportError(('Latitude ({},{}) out of dataset bounds                               ({},{})').format(minlat, maxlat, self.slat, self.nlat))
...
class CraterDataFrame(pd.DataFrame):
    ...
    **kwargs :
    ...
    def __init__(self, data=None, index_col=None, latcol=None, loncol=None, radcol=None, **kwargs):
        """
        ...
        for i in range(len(colnames)):
            if not attrs[i]:
                findcol = [ colnames[i] == col.strip().lower() or colabbrevs[i] == col.strip().lower() for col in self.columns
                          ]
                if any(findcol):
                    attrs[i] = self.columns[np.where(findcol)[0][0]]
                    continue
                elif colnames[i] == 'radius':
                    finddiam = [ 'diameter' == col.strip().lower() or 'diam' == col.strip().lower() for col in self.columns
                               ]
                    if any(finddiam):
                        ...
                        attrs[i] = 'radius'
                        continue
                raise ImportError(('Unable to infer {} column from header.                                   Specify {}col in                                   constructor.').format(colnames[i], colabbrevs[i]))
