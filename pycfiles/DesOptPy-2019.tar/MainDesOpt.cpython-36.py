...
# [GCC 8.4.0]
# Embedded file name: /home/wehrle/opt/DesOptPy/DesOptPy/MainDesOpt.py
# Compiled at: 2019-04-13 10:03:50
# Size of source mod 2**32: 43401 bytes
"""
-------------------------------------------------------------------------------
Title:          MainDesOpt.py
Version:        1.4α
...
-------------------------------------------------------------------------------
"""
import os, shutil, sys, inspect, pyOpt
...
import scipy.io as spio, time, copy, datetime, getpass, multiprocessing, platform
...
def DesOpt(SysEq, x0, xU, xL, xDis=[], gc=[], hc=[], SensEq=[], Alg='SLSQP', SensCalc='FD', DesVarNorm=True, nf=1, deltax=0.001, StatusReport=False, ResultReport=False, Video=False, nDoE=0, DoE='LHS+Corners', SBDO=False, Approx=[], Debug=False, PrintOut=True, OptNameAdd='', AlgOptions=[], KeepEval=False, Alarm=True):
    """
    :type OptNode: object
    """
    global nEval
    if Debug:
        ...
        def OptSysEq(x):
            ...
            if KeepEval:
                ...
                for filename in glob.glob('*.*'):
                    if filename[0:len(OptName)] != OptName and filename not in ('desVars.csv',
                                                                                'desVarsNorm.csv',
                                                                                'constraints.csv',
                                                                                'objFct_maxCon.csv',
                                                                                'initial1.html'):
            ...
            nEval += 1
            if StatusReport:
                OptHis2HTML.OptHis2HTML(OptName, Alg, AlgOptions, DesOptDir, x0, xL, xU, DesVarNorm, inform[0], OptTime0)
            if len(xDis) > 0:
                nD = len(xDis)
                gDis = [[]] * 2 * nD
                for ii in range(nD):
        ...
        if xDis is not []:
            for ii in range(np.size(xDis, 0)):
                xExpand0 = np.ones(xDis[ii]) * 1.0 / xDis[ii]
        ...
        if pyOptAlg:
            ...
            if np.size(x0) == 1:
                ...
                if np.size(x0) > 1:
                    for ii in range(np.size(x0)):
                        OptProb.addVar(('x' + str(ii + 1)), 'c', value=(x0norm[ii]), lower=(xLnorm[ii]),
                          upper=(xUnorm[ii]))
            ...
            if PrintOut:
                print(OptProb)
            if Alg in ('MMA', 'FSQP', 'GCMMA', 'CONMIN', 'SLSQP', 'PSQP', 'KSOPT',
                       'ALGENCAN', 'NLPQLP', 'IPOPT'):
                if SensCalc == 'OptSensEq':
                    if DesVarNorm not in ('None', None, False):
                        fOpt, xOpt, inform = OptAlg(OptProb, sens_type=OptSensEqNorm,
                          store_hst=OptName)
                    else:
                    ...
                    if SensCalc == 'ParaFD':
                        if DesVarNorm not in ('None', None, False):
                            fOpt, xOpt, inform = OptAlg(OptProb, sens_type=OptSensEqParaFDNorm,
                              store_hst=OptName)
                        else:
                            fOpt, xOpt, inform = OptAlg(OptProb, sens_type=OptSensEqParaFD,
                              store_hst=OptName)
                    else:
                        if SensCalc == 'AD':
                            if DesVarNorm not in ('None', None, False):
                                fOpt, xOpt, inform = OptAlg(OptProb, sens_type=OptSensEqNormAD,
                                  store_hst=OptName)
                            else:
                                fOpt, xOpt, inform = OptAlg(OptProb, sens_type=OptSensEqAD,
                                  store_hst=OptName)
                        else:
                            fOpt, xOpt, inform = OptAlg(OptProb, sens_type=SensCalc, sens_step=deltax,
                              store_hst=OptName)
            else:
                if Alg in ('SDPEN', 'SOLVOPT'):
            ...
            if Alg == 'ralg':
                ...
                p = NLP(f, x0, c=g, lb=xL, ub=xU, iprint=50, maxIter=10000, maxFunEvals=10000000.0,
                  name='NLP_1')
                    ...
                    if Alg == 'fmincon':
                        ...
                        if Alg[:5] == 'PyGMO':
                            ...
                            class OptProbPyGMO:
                                ...
                                def fitness(self, x):
                                    ...
                                    if nEval == 1:
                                        ...
                                        HistData = pyOpt.History(OptName, 'w', optimizer=AlgInst, opt_prob=OptName)
                                    HistData.write(x, 'x')
                            ...
                            if Alg[6:] == 'monte_carlo':
                                ...
                                if len(gc) > 0:
                                    algo = pg.algorithm(pg.cstrs_self_adaptive(iters=(AlgOptions.gen), algo=(eval('pg.' + Alg[6:] + '(' + str(AlgOptions.nIndiv) + ')'))))
                                    pop.problem.c_tol = [1e-06] * len(gc)
                                else:
                            ...
                            if Alg[:5] == 'scipy':
                                ...
                                if Alg == 'SteepestDescentSUMT':
                                    ...
                                    if Alg == 'NewtonSUMT':
                                        ...
                                        raise Exception('Error on line ' + str(inspect.currentframe().f_lineno) + ' of file ' + __file__ + ': algorithm misspelled or not supported')
        if StatusReport == 1:
            OptHis2HTML.OptHis2HTML(OptName, Alg, AlgOptions, DesOptDir, x0, xL, xU, DesVarNorm, inform['text'], OptTime0)
        OptTime1 = time.time()
        ...
        NewRead = True
        if NewRead:
            fIter, xIter, gIter, gGradIter, fGradIter, inform = OptReadHis(OptName, Alg, AlgOptions, x0, xL, xU, DesVarNorm)
        ...
        if np.size(gc) > 0:
            ...
            if np.size(fGradIter) > 0:
                ...
                lambda_c = lamActive
                kktOpt, Opt1Order, OptRes, KKTmax = CheckKKT(lambda_c, fGradOpt, gAllGradActiveOpt, gAllOptActive)
                SPg = CalcShadowPrice(lambda_c, gcAllActive, gAllActiveType, DesVarNorm)
            else:
                SPg = np.array([])
        ...
        output.close()
        np.savez(OptName + '_OptSol', x0, xOpt, xOptNorm, xIter, xIterNorm, xIter, xIterNorm, fOpt, fIter, fIterNorm, gIter, gMaxIter, gOpt, fGradIter, gGradIter, fGradOpt, gGradOpt, OptName, OptModel, OptTime, loctime, today, computerName, operatingSystem, architecture, nProcessors, userName, Alg, DesVarNorm, KKTmax)
        spio.savemat((OptName + '_OptSol.mat'), OptSolData, oned_as='row')
        os.chdir(MainDir)
        if LocalRun:
            if Debug is False:
                try:
                    shutil.move(RunDir + os.sep + OptName, ResultsDir + os.sep + OptName + os.sep + 'RunFiles' + os.sep)
                except:
                    print('Run files not deleted from ' + RunDir + os.sep + OptName)
                    shutil.copytree(RunDir + os.sep + OptName, ResultsDir + os.sep + OptName + os.sep + 'RunFiles' + os.sep)

        if ResultReport:
            print('Entering preprocessing mode')
            OptResultReport.OptResultReport(OptName, OptAlg, DesOptDir, diagrams=1, tables=1,
              lyx=1)
        if Video:
        ...
        if PrintOut:
            ...
            if np.size(lambda_c) > 0:
                print('Lagrangian multipliers = ' + str(lambda_c.reshape(np.size(lambda_c))))
