...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nubo/clouds/digitalocean.py
# Compiled at: 2013-04-13 07:56:21
"""     
...
class DigitalOcean(BaseCloud):
    ...
    def get_ssh_key_id(self):
        ...
        uploaded_key = [ key.id for key in self.driver.ex_list_ssh_keys() if key.name == self.ssh_key_name
                       ]
        if uploaded_key:
    ...
    def deploy(self, image_id, size_idx=0, location_idx=0, name='test'):
        ...
        if not key_id:
            uploaded_key = self.driver.ex_create_ssh_key(self.ssh_key_name, open(self.ssh_public_key).read())
