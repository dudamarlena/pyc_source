...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nubo/clouds/ec2.py
# Compiled at: 2013-04-13 15:46:57
"""     
    nubo.clouds.ec2
    ===============
...
class AmazonEC2(BaseCloud):
    ...
    def list_images(self, limit=20, keyword=''):
        ...
        too much output."""
        ami_images = [ image for image in self.driver.list_images() if 'ami-' in image.id
                     ]
        if not limit:
            limit = 20
        return [ image for image in ami_images if keyword.lower() in image.name.lower() or not keyword
               ][:limit]
    ...
    def deploy(self, image_id, size_idx=0, location_idx=0, name='test'):
        ...
        if not key_id:
            key = self.driver.ex_import_keypair(self.ssh_key_name, self.ssh_public_key)
            key_id = key['keyName']
        ...
        return self.startup({'size': size, 
           'image': Image, 'name': name, 'location': location, 
           'ex_keyname': key_id, 'ex_securitygroup': __name__})