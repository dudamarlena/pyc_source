...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nubo/clouds/base.py
# Compiled at: 2014-05-18 17:10:27
"""     
    nubo.clouds.base
    ================
...
"""
import sys, time, socket, logging, hashlib
...
CLOUDS_MAPPING = {'EC2_US_EAST': 'nubo.clouds.ec2.AmazonEC2', 
...
def node2dict(node):
    ...
    fields = ('id', 'name', 'state', 'public_ips', 'private_ips', 'image', 'size',
              'extra')
    values = {}
    for field in fields:
...
class BaseCloud(object):
    ...
    def __init__(self, ssh_private_key=None, login_as='root'):
        ...
        DriverClass = get_driver(provider)
        self.driver = DriverClass(**AVAILABLE_CLOUDS[CLOUDS_MAPPING[self.PROVIDER_NAME]])
        return
    ...
    def is_running(self, node_id):
        ...
        running_ids = [ node2dict(node)['id'] for node in self.driver.list_nodes() ]
    ...
    def list_images(self, limit=None, keyword=''):
        ...
        images = [ image for image in self.driver.list_images() if keyword.lower() in image.name.lower() or not keyword
                 ]
        if limit:
            return images[:limit]
        return images
