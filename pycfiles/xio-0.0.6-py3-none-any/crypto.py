...
# [GCC 8.4.0]
# Embedded file name: /apps/xio/core/lib/crypto/crypto.py
# Compiled at: 2018-12-07 08:05:33
from .common import *
import sys, hashlib, base64, uuid, json, time
...
class Key:
    ...
    def __init__(self, priv=None, token=None, seed=None):
        ...
        if token:
            ...
            self.token = (token or self.generateToken)() if 1 else token
    ...
    def account(self, scheme=None):
        ...
        if scheme == 'xio':
            return self
        if scheme not in self._accounts:
    ...
    def generateToken(self, scheme=None, body=None):
        ...
        iat = int(time.time())
        exp = iat + 3600
        iss = (scheme or self).address if 1 else self.account(scheme).address
        body = body or {'sub': to_string(iss)}
        body.update({'iss': to_string(iss), 
    ...
    def recoverToken(self, token, scheme=None):
        ...
        headerstr, bodystr, sig = [ base64.urlsafe_b64decode(part) for part in token.split('.') ]
        header = json.loads(headerstr)
        body = json.loads(bodystr)
        sig = sig.decode('utf-8')
        scheme = header.get('alg')
        assert scheme
        h = (scheme or self)._handler if 1 else self.account(scheme)
        assert h
