...
# [GCC 8.4.0]
# Embedded file name: /apps/xio/core/handlers.py
...
import os, sys, traceback, collections, hashlib, base64, uuid, yaml, copy, json, requests, inspect, time
        ...
        for name in dir(self.handler):
            if name[0] != '_':
                ...
                if callable(h):
                    ...
                    self.api[name.lower()] = {'handler':h,  'input':{'params': params}}
...
class HttpHandler:
    ...
    def __call__(self, req):
        ...
        params = query or {}
        if not data or isinstance(data, list) or isinstance(data, dict):
    ...
    def __call__(self, req):
        ...
        if req.GET:
            if not req.path:
                ...
                if indexpath:
                    return FileHandler(indexpath)(req)
