...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/texty/tag.py
# Compiled at: 2015-12-15 04:46:08
# Size of source mod 2**32: 4320 bytes
import os, pandas as pd

class LabMTHelper:
    __doc__ = ' A simple class to score text with LabMT and pandas,\n    based on the example provided by  Finn Årup Nielsen\n    at http://neuro.imm.dtu.dk/wiki/LabMT  '

    def __init__(self, from_web=True, local_path=''):
        if from_web:
            source = 'http://www.plosone.org/article/fetchSingleRepresentation.action?uri=info:doi/10.1371/journal.pone.0026752.s001'
        else:
            if local_path:
                source = local_path
            else:
                source = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'journal.pone.0026752.s001.txt')
        try:
            labmt_csv = pd.read_csv(source, skiprows=2, sep='\t', index_col=0)
        except OSError as e:
            if from_web:
                print('\n\nThere were problems loading', source)
                print("If you have a local copy, you can set 'from_web=False' and specify 'local_path'.\n\n")
            else:
                print('\n\nCould not open', source)
                print("If you don't have a local copy, you can set 'from_web=True' and load it from the original publication.\n\n")
            raise e
        else:
    ...
    def score_tokens(self, tokens):
        score = sum([self.happiness.get(tok.lower(), 0.0) for tok in tokens]) / len(tokens)
        return score
...
class SpacyHelper:
    __doc__ = ' A wrapper for spaCy that provides some formatting and transformations.'

    def __init__(self, tag=True, parse=True, ner=True):
        try:
            import spacy.en
        except ImportError as e:
            print("\n\nPlease make sure that spaCy is installed ('pip3 install spacy') and that the English data is downloaded ('python3 -m spacy.en.download').\n\n")
            raise e
        else:
