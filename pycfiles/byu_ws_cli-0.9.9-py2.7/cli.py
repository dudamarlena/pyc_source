...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/byu_ws_cli/cli.py
# Compiled at: 2014-06-14 10:25:55
"""
...
def setup_options_and_run():
    ...
    parser.add_option('-e', '--encoding-type', help='one of %s or %s [%s]' % (oit.ENCODING_URL, oit.ENCODING_NONCE, oit.ENCODING_URL), default=oit.ENCODING_URL)
    parser.add_option('-k', '--key-type', help='one of %s or %s [%s]' % (oit.KEY_TYPE_API, oit.KEY_TYPE_WSSESSION, oit.KEY_TYPE_API), default=oit.KEY_TYPE_API)
    ...
    parser.add_option('-o', '--other-headers', help='add arbitrary headers to the HTTP request using the following format. "key:value[,key2:value2]"', action='callback', type='string', default={}, callback=other_headers_callback)
    parser.add_option('--actor-in-hash', action='store_true', help='Put the actor in the hash (new way), default False.')
...
class CallWebServiceCLI(object):
    ...
    def _generate_headers(self, options, requestBody, url):
        headerValue = oit.get_http_authorization_header(credentials.key, credentials.shared_secret, options.key_type, options.encoding_type, url, requestBody, options.actor, options.content_type, options.http_method, demo=self._options.demo, actorInHash=self._options.actor_in_hash)
        headers = {'Authorization': headerValue}
        if options.content_type:
            headers['Content-Type'] = options.content_type
        if options.accept:
            headers['Accept'] = options.accept
        headers.update(options.other_headers)
        return headers
    ...
    def main(self):
        ...
        response, status, headers, response_full = oit.send_ws_request(url, self._options.http_method, requestBody, headers=headers, verify=verify)
