...
# [GCC 8.4.0]
# Embedded file name: /home/nazrul/www/python/Contributions/apps/hybrid-access-control-system/hacs/views/errors.py
# Compiled at: 2016-06-27 12:14:25
...
__all__ = [str(x) for x in ('page_not_found', 'server_error', 'bad_request', 'permission_denied',
                            'maintenance_mode', 'service_unavailable', 'http_method_not_permitted')]
...
def maintenance_mode(request):
    ...
    if request.META.get('HTTP_ACCEPT'):
        ...
        if request.is_ajax() and not request.META.get('HTTP_ACCEPT'):
            json_response = True
        elif request.is_ajax():
            if 'text/html' not in request.META.get('HTTP_ACCEPT').lower():
                json_response = True
    if json_response:
        data['meta'] = {'status': 503,  'reason': ugettext('Service is not available, maintenance in progress')}
        data['contents'] = None
    ...
    if request.META.get('HTTP_ACCEPT'):
        ...
        if request.is_ajax() and not request.META.get('HTTP_ACCEPT'):
            ...
            if 'text/html' not in request.META.get('HTTP_ACCEPT').lower():
                json_response = True
    if json_response:
        data['meta'] = {'status': 503,  'reason': ugettext('503: Service is not available.')}
        data['contents'] = None
        return JsonResponse(data=data, status=503, reason=data['meta']['reason'])
    ...
    if request.META.get('HTTP_ACCEPT'):
        ...
        if request.is_ajax() and not request.META.get('HTTP_ACCEPT'):
            ...
            if 'text/html' not in request.META.get('HTTP_ACCEPT').lower():
                json_response = True
    if json_response:
        data['meta'] = {'status': 405,  'reason': ugettext('%s HTTP method is not permitted' % request.method)}
        data['contents'] = None
