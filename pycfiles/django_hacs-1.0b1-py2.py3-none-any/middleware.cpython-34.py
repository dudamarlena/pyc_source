...
# [GCC 8.4.0]
# Embedded file name: /home/nazrul/www/python/Contributions/apps/hybrid-access-control-system/hacs/middleware.py
# Compiled at: 2016-07-13 11:26:10
...
class DynamicRouteMiddleware(object):
    ...
    def _validate(self):
        ...
        try:
            site_middleware_position = settings.MIDDLEWARE_CLASSES.index('django.contrib.sites.middleware.CurrentSiteMiddleware')
        except ValueError:
            errors.append(_('django.contrib.sites.middleware.CurrentSiteMiddleware need to be added into settings.MIDDLEWARE_CLASSES'))
        else:
            if site_middleware_position > settings.MIDDLEWARE_CLASSES.index(self.name):
                errors.append(_("django.contrib.sites.middleware.CurrentSiteMiddleware's position should be before " + self.name))
            if errors:
...
class FirewallMiddleware(object):
    ...
    def process_request(self, request):
        ...
        elif not getattr(request, 'urlconf', None):
    ...
    def set_auth_user_settings(self, request, force_update=True):
        ...
        try:
            user_route_rules = ContentTypeRoutingRules.objects.get(site=request.site, content_type=ContentType.objects.get_for_model(UserModel), object_id=request.user.is_authenticated() and request.user.pk or 0)
        except ContentTypeRoutingRules.DoesNotExist:
            initial_data.update({'has_own_rules': False})
        else:
            generate_urlconf_file_on_demand(user_route_rules.route)
            initial_data.update({'urlconf': get_generated_urlconf_module(get_generated_urlconf_file(user_route_rules.route.route_name)), 
    ...
    def set_auth_group_settings(self, request, group, force_update=True):
        ...
        if group_settings_cache.get('urlconf', None) is not None and not force_update:
            return
        try:
            group_route_rules = ContentTypeRoutingRules.objects.get(content_type=ContentType.objects.get_for_model(Group), object_id=group.id, site=request.site)
        except ContentTypeRoutingRules.DoesNotExist:
            pass
        else:
            generate_urlconf_file_on_demand(group_route_rules.route)
            group_settings_cache.update({'urlconf': get_generated_urlconf_module(get_generated_urlconf_file(group_route_rules.route.route_name)), 
    ...
    def _validate(self):
        ...
        errors = []
        try:
            dynamic_route_middleware_position = settings.MIDDLEWARE_CLASSES.index(DynamicRouteMiddleware.name)
        except ValueError:
            errors.append(_('%s need to be added into settings.MIDDLEWARE_CLASSES' % DynamicRouteMiddleware.name))
        else:
            if dynamic_route_middleware_position > settings.MIDDLEWARE_CLASSES.index(self.name):
                errors.append(_(DynamicRouteMiddleware.name + "'s position should be before " + self.name))
            return True
            ...
            if user_settings['groups']:
                ...
                for group_cache_key, group_natural_key in user_settings['groups']:
                    ...
                    try:
                        ...
                        resolver.resolve(request.path_info)
                        user_settings.update({'allowed_http_methods': cache.get(group_cache_key).get('allowed_http_methods'), 
    ...
    def _calculate_anonymous_urlconf(self, request):
        ...
        try:
            ...
            try:
                user_route = ContentTypeRoutingRules.objects.get(site=request.site, content_type=ContentType.objects.get_for_model(UserModel), object_id=0)
            ...
            cache.set(cache_key, user_settings, 86400)
            return urlconf


__all__ = (
 'DynamicRouteMiddleware', 'FirewallMiddleware')