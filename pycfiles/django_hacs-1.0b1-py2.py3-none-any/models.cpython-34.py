...
# [GCC 8.4.0]
# Embedded file name: /home/nazrul/www/python/Contributions/apps/hybrid-access-control-system/hacs/models.py
# Compiled at: 2016-07-12 15:04:27
...
class SiteRoutingRules(models.Model):
    __doc__ = '\n    '
    route = models.ForeignKey(RoutingTable, on_delete=models.CASCADE, db_column='route_id', db_constraint=False, related_name='hacs_route_sites')
    site = models.OneToOneField(Site, on_delete=models.CASCADE, unique=True, null=False, blank=False, related_name='hacs_site_routes')
    allowed_method = SequenceField(_('Allowed Method'), null=True, blank=True)
    blacklisted_uri = models.CharField(_('blacklisted uri'), max_length=255, null=True, blank=True, help_text=_('regex formatted uri those will be treated as blacklisted and request will be discarded by firewall'))
    whitelisted_uri = models.CharField(max_length=255, null=True, blank=True, help_text=_('regex formatted uri those will be treated as whitelisted and request will be discarded by firewall if uri not match'))
    is_active = models.BooleanField(_('Is Active'), null=False, blank=True, default=True)
    maintenance_mode = models.BooleanField(_('Maintenance Mode'), null=False, blank=True, default=False, help_text=_('Firewall will only response maintenance view and prevent any further execution for all request if it is on'))
    created_on = models.DateTimeField(_('Created On'), blank=True, default=timezone.now)
    updated_on = models.DateTimeField(_('Last updated'), null=True, blank=True)
...
class ContentTypeRoutingRulesManager(models.Manager):
    ...
    def get_by_natural_key(self, site_nk, content_type_nk, object_id):
        ...
        return self.get(site=site, content_type=ContentType.objects.db_manager(self.db).get_by_natural_key(*content_type_nk), object_id=object_id)


@python_2_unicode_compatible
class ContentTypeRoutingRules(models.Model):
    __doc__ = '\n    '
    route = models.ForeignKey(RoutingTable, on_delete=models.CASCADE, db_column='route_id', db_constraint=False, related_name='hacs_route_contenttypes', validators=[])
    site = models.ForeignKey(Site, on_delete=models.CASCADE, null=False, blank=False, related_name='hacs_site_contenttypes_at_routing_rules')
    content_type = models.ForeignKey(ContentType, on_delete=models.CASCADE, validators=[ContentTypeValidator()])
    object_id = models.PositiveIntegerField()
    content_object = GenericForeignKey('content_type', 'object_id')
    allowed_method = SequenceField(_('Allowed Method'), null=True, blank=True)
    blacklisted_uri = models.CharField(_('blacklisted uri'), max_length=255, null=True, blank=True, help_text=_('regex formatted uri those will be treated as blacklisted and request will be discarded by firewall'))
    whitelisted_uri = models.CharField(_('whitelisted uri'), max_length=255, null=True, blank=True, help_text='regex formatted uri those will be treated as whitelisted and request will be discarded by firewall if uri not match')
    is_active = models.BooleanField(_('Is Active'), null=False, blank=True, default=True)
    created_on = models.DateTimeField(_('Created On'), blank=True, default=timezone.now)
    updated_on = models.DateTimeField(_('Last updated'), null=True, blank=True)
    ...
    natural_key.dependencies = [
     'hacs.RoutingTable', 'sites.Site', 'contenttypes.ContentType']

    def __str__(self):
        """"""
        return "%s:%s:%s's routing rules" % (self.site.domain,
