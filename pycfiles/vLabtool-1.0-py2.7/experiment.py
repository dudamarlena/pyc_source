...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vLabtool/experiment.py
# Compiled at: 2015-08-18 13:07:50
import os
...
import custom_widgets as Widgets, numpy as np, sys
...
class ConvenienceClass:
    ...
    def __init__(self):
        ...
        try:
            import scipy.optimize as optimize, scipy.fftpack as fftpack
        except ImportError:
...
class Experiment(QtGui.QMainWindow, template_exp.Ui_MainWindow, Widgets.CustomWidgets):
    ...
    def addCurve(self, plot, name='', col=(255, 255, 255), axis='left'):
        if len(name):
            curve = self.pg.PlotCurveItem(name=name)
        else:
            curve = self.pg.PlotCurveItem()
        plot.addItem(curve)
