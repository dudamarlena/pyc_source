...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vLabtool/NRF_NODE.py
# Compiled at: 2015-07-17 09:10:39
import time
...
class RadioLink:
    ...
    def I2C_scan(self):
        ...
        x = self.NRF.transaction([self.I2C_COMMANDS | self.SCAN_I2C | 128], timeout=500)
        if not x:
            return []
        if not sum(x):
            return []
    ...
    def configI2C(self, freq):
        self.__selectMe__()
        brgval = int(32000000.0 / freq / 4 - 1)
        print brgval
