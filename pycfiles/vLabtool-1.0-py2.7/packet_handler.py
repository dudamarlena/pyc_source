...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vLabtool/packet_handler.py
# Compiled at: 2015-08-11 13:21:40
...
class Handler(object):
    ...
    def __sendInt__(self, val):
        """
                ...
                """
        if not self.loadBurst:
            self.fd.write(InttoString(val))
        else:
            self.burstBuffer += InttoString(val)
    ...
    def __sendByte__(self, val):
        ...
        if type(val) == int:
            if not self.loadBurst:
                ...
                self.burstBuffer += chr(val)
        elif not self.loadBurst:
            self.fd.write(val)
        else:
    ...
    def __getLong__(self):
        ...
        ss = self.fd.read(4)
        if len(ss) == 4:
            return ord(ss[0]) | ord(ss[1]) << 8 | ord(ss[2]) << 16 | ord(ss[3]) << 24
        else:
            print '.'
            return -1
