...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vLabtool/interface.py
# Compiled at: 2015-08-18 14:01:03
import os
...
import packet_handler, I2C_class, SPI_class, NRF24L01_class, MCP4728_class, NRF_NODE
from achan import *
from digital_channel import *
import serial, string, fcntl, time, sys, numpy as np, math
...
class Interface(object):
    ...
    def __init__(self, timeout=1.0, **kwargs):
        ...
        for a in allAnalogChannels:
            self.analogInputSources[a] = analogInputSource(a)
        ...
        self.analog_channel_names = [
         'CH1', 'CH2', 'CH3', 'CH4', 'CH5', 'CH6', 'CH7', 'I2V', '5V', '9V', 'IN1', 'SEN']
        ...
        self.DDS_MAX_FREQ = 268435454
        self.DDS_CLOCK = 8000000.0
        self.map_reference_clock(4, 'wavegen')
        self.__selectSensorChannel__(0)
        for a in ['CH1', 'CH2']:
            self.set_gain(a, 0)

        self.SOCKET_CAPACITANCE = 4.2e-11
        time.sleep(0.001)
    ...
    def capture1(self, ch, ns, tg):
        ...
        time.sleep(1e-06 * self.samples * self.timebase + 0.01)
    ...
    def capture_traces(self, num, samples, tg, channel_one_input='CH1', CH123SA=0, **kwargs):
        ...
        if num == 1:
            ...
            self.H.__sendByte__(CAPTURE_ONE)
            self.H.__sendByte__(CHOSA | triggerornot)
        elif num == 2:
            if self.timebase < 1.75:
                self.timebase = 1.75
            if samples > self.MAX_SAMPLES / 2:
                samples = self.MAX_SAMPLES / 2
            self.achans[0].set_params(channel=channel_one_input, length=samples, timebase=self.timebase, resolution=TEN_BIT, source=self.analogInputSources[channel_one_input])
            ...
            self.H.__sendByte__(CAPTURE_TWO)
            self.H.__sendByte__(CHOSA | triggerornot)
        self.samples = samples
        self.H.__sendInt__(samples)
        self.H.__sendInt__(int(self.timebase * 8))
        self.H.__get_ack__()
    ...
    def capture_highres_traces(self, channel, samples, tg, **kwargs):
        ...
        self.H.__sendByte__(CAPTURE_12BIT)
        self.H.__sendByte__(CHOSA | triggerornot)
        self.samples = samples
        self.H.__sendInt__(samples)
        self.H.__sendInt__(int(self.timebase * 8))
    ...
    def __fetch_channel__(self, channel_number):
        ...
        if samples % self.data_splitting:
            ...
            data += self.H.fd.read(2 * (samples % self.data_splitting))
            self.H.__get_ack__()
        for a in range(samples):
            self.buff[a] = ord(data[(a * 2)]) | ord(data[(a * 2 + 1)]) << 8
    ...
    def set_gain(self, channel, gain):
        ...
        if self.analogInputSources[channel].gainPGA == None:
            ...
            if channel in self.multiplexedChannels:
                for a in self.multiplexedChannels:
                    self.analogInputSources[a].setGain(self.gain_values[gain])

            self.H.__sendByte__(ADC)
            self.H.__sendByte__(SET_PGA_GAIN)
            self.H.__sendByte__(self.analogInputSources[channel].gainPGA)
            self.H.__sendByte__(gain)
            self.H.__get_ack__()
    ...
    def get_average_voltage(self, channel_name, sleep=0):
        ...
        if sleep:
            self.H.__sendByte__(chosa | 128)
        else:
            self.H.__sendByte__(chosa)
        self.H.__getInt__()
        V_sum = self.H.__getInt__()
        self.H.__get_ack__()
        return self.analogInputSources[channel_name].calPoly12(V_sum / 16.0)
    ...
    def get_freq(self, channel='ID1', timeout=0.1):
        ...
        timeout_msb = int(timeout * 64000000.0) >> 16
        ...
        if tmt >= timeout_msb:
            return 0
        freq = lambda t: 1024000000.0 / t if t else 0
        y = x[1] - x[0]
        return freq(y)
    ...
    def setup_comparator(self, level=7, digital_filter=3):
        """ 
                ...
                **Arguments** 
                ...
                """
        print 1.0 / 4 * 3.3 + level / 32.0 * 3.3
        self.H.__sendByte__(TIMING)
    ...
    def LA_capture1(self, waiting_time=0.1, trigger=0):
        ...
        return (data[4][0], tmp / 64000000.0)
    ...
    def __start_one_channel_LA_backup__(self, trigger=1, channel='ID1', maximum_time=67, **args):
        ...
        if 'trigger_channels' in args and trigger & 1:
            ...
            trigger |= 1 << self.__calcDChan__(channel) + 4
        ...
        for a in self.dchans:
            ...
            a.maximum_time = maximum_time * 1000000.0
            a.mode = EVERY_EDGE
            self.clear_buffer(0, self.MAX_SAMPLES / 2)
            self.H.__sendByte__(TIMING)
            self.H.__sendByte__(START_ONE_CHAN_LA)
            ...
            if 'trigger_channel' in args:
                ...
                tredge = args.get('trigger_edge', 0)
                if tredge != -1:
                    self.H.__sendByte__(aqchan << 4 | tredge << 1 | 1)
                else:
                    self.H.__sendByte__(0)
            else:
                self.H.__sendByte__(0)
    ...
    def start_two_channel_LA(self, trigger=1, maximum_time=67):
        ...
        self.H.__sendByte__(modes[1] << 4 | modes[0])
        self.H.__sendByte__(chans[1] << 4 | chans[0])
        self.H.__get_ack__()
        n = 0
    ...
    def get_LA_initial_states(self):
        ...
        self.H.__get_ack__()
        if A == 0:
            A = self.MAX_SAMPLES / 4
        if B == 0:
            B = self.MAX_SAMPLES / 4
        if C == 0:
            C = self.MAX_SAMPLES / 4
        if D == 0:
            D = self.MAX_SAMPLES / 4
        if A < 0:
            A = 0
        if B < 0:
            B = 0
        ...
        if D < 0:
            D = 0
        return (
         A, B, C, D, [s & 1 != 0, s & 2 != 0, s & 4 != 0, s & 8 != 0])
    ...
    def set_state(self, **kwargs):
        ...
        if kwargs.has_key('OD1'):
            data |= 64 | kwargs.get('OD1') << 2
        if kwargs.has_key('OD2'):
            data |= 128 | kwargs.get('OD2') << 3
        if kwargs.has_key('SQR1'):
            data |= 16 | kwargs.get('SQR1')
        if kwargs.has_key('SQR2'):
            data |= 32 | kwargs.get('SQR2') << 1
        if kwargs.has_key('SQR3'):
            data |= 64 | kwargs.get('SQR3') << 2
        if kwargs.has_key('SQR4'):
            data |= 128 | kwargs.get('SQR4') << 3
    ...
    def __get_capacitor_range__(self, ctime):
        ...
        self.H.__get_ack__()
        V = V_sum * 3.3 / 16 / 4095
        C = -ctime * 1e-06 / 10000.0 / np.log(1 - V / 3.3)
        return (V, C)
    ...
    def get_capacitance(self, current_range, trim, Charge_Time):
        """
                ...
                """
        self.H.__sendByte__(COMMON)
        currents = [0.00055, 5.5e-07, 5.5e-06, 5.5e-05]
        ...
        time.sleep(Charge_Time * 1e-06 + 0.02)
        V = 3.3 * self.H.__getInt__() / 4095
        self.H.__get_ack__()
        Charge_Current = currents[current_range] * (100 + trim) / 100.0
        C = Charge_Current * Charge_Time * 1e-06 / V - self.SOCKET_CAPACITANCE
        print 'Current if C=470pF :', V * (4.7e-10 + self.SOCKET_CAPACITANCE) / (Charge_Time * 1e-06)
    ...
    def get_inductance(self):
        ...
        c1 = 1.09017e-09
        l1 = 9.68246e-06
        f3 = self.get_high_freq('LMETER')
        if f3 > 1:
            return 1.0 / (c1 * f3 * f3 * 4 * math.pi * math.pi) - l1
        else:
            return 0
    ...
    def write_flash(self, page, location, string_to_write):
        """
                ...
                **Arguments** 
                ...
                """
        while len(string_to_write) < 16:
            string_to_write += '.'

        self.H.__sendByte__(FLASH)
        self.H.__sendByte__(WRITE_FLASH)
        self.H.__sendByte__(page)
        self.H.__sendByte__(location)
    ...
    def write_bulk_flash(self, location, bytearray):
        ...
        self.H.__sendByte__(FLASH)
        self.H.__sendByte__(WRITE_BULK_FLASH)
        self.H.__sendInt__(len(bytearray))
        self.H.__sendByte__(location)
    ...
    def get_temperature(self):
        """
                ...
                """
        V = self.get_ctmu_voltage(30, 3, 0)
    ...
    def set_sine(self, frequency, register=0):
        ...
        freq_setting = int(round(1.0 * frequency * self.DDS_MAX_FREQ / self.DDS_CLOCK))
        self.H.__sendByte__(WAVEGEN)
        self.H.__sendByte__(SET_WG)
        self.H.__sendByte__(14 + register)
        self.H.__sendInt__(freq_setting & 16383)
        self.H.__sendInt__(freq_setting >> 14 & 16383)
        self.H.__get_ack__()
        self.SPI.set_parameters(1, 7, 1, 0)
        return frequency
    ...
    def WS2812B(self, cols):
        ...
        for col in cols:
            R = reverse_bits(int(col[0]))
            G = reverse_bits(int(col[1]))
            B = reverse_bits(int(col[2]))
            self.H.__sendByte__(G)
            self.H.__sendByte__(R)
            self.H.__sendByte__(B)
    ...
    def sqr1(self, freq, duty_cycle, echo=False):
        ...
        prescaler = 0
        while prescaler <= 3:
            wavelength = 64000000.0 / freq / p[prescaler]
            if wavelength < 65525:
                break
            prescaler += 1
        ...
        if prescaler == 4:
            print 'out of range'
            return
        high_time = wavelength * duty_cycle / 100.0
    ...
    def sqr4_pulse(self, freq, h0, p1, h1, p2, h2, p3, h3):
        """
                ...
                **Arguments** 
                ...
                """
        wavelength = int(64000000.0 / freq)
        if wavelength > 65535:
    ...
    def sqr4_continuous(self, freq, h0, p1, h1, p2, h2, p3, h3):
        ...
        wavelength = int(64000000.0 / freq)
        params = 0
        if wavelength > 16776960:
            print 'frequency too low.'
            return
        if wavelength > 4194240:
            wavelength = int(64000000.0 / freq / 256)
            params = 3
        elif wavelength > 524280:
            params = 2
            wavelength = int(64000000.0 / freq / 64)
        elif wavelength > 65535:
            params = 1
            wavelength = int(64000000.0 / freq / 8)
        params |= 32
        self.H.__sendByte__(WAVEGEN)
        self.H.__sendByte__(SQR4)
        self.H.__sendInt__(wavelength)
    ...
    def map_reference_clock(self, scaler, *args):
        ...
        if 'wavegen' in args:
            self.DDS_CLOCK = 128000000.0 / (1 << scaler)
        self.H.__get_ack__()
    ...
    def read_program_address(self, address):
        ...
        self.H.__sendInt__(address & 65535)
        self.H.__sendInt__(address >> 16 & 65535)
        v = self.H.__getInt__()
        self.H.__get_ack__()
        return v
    ...
    def enableUartPassthrough(self, baudrate, persist=False):
        ...
        self.H.__sendInt__(int(round(64000000.0 / baudrate / 4 - 1)))
        print 'BRGVAL:', int(round(64000000.0 / baudrate / 4 - 1))
        time.sleep(0.1)
