...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/universalcore/unicore.webhooks/unicore/webhooks/views.py
# Compiled at: 2016-06-21 11:57:16
from colander import MappingSchema, SchemaNode, String, Boolean, OneOf, url
from cornice.resource import resource, view
from pyramid import httpexceptions
from pyramid.view import view_config
from sqlalchemy.orm.exc import NoResultFound, MultipleResultsFound
from unicore.webhooks.models import DBSession, Webhook
from unicore.webhooks.events import WebhookDeleted, WebhookCreated, WebhookUpdated

class WebhookSchema(MappingSchema):
    url = SchemaNode(String(), location='body', validator=url)
    event_type = SchemaNode(String(), location='body', validator=OneOf([ value for value, label in Webhook.TYPES ]))
...
class WebhooksResource(object):
    ...
    def __init__(self, request):
        ...
        self.webhooks = DBSession.query(Webhook).filter(Webhook.owner == request.authenticated_userid)
        self.config = request.registry.settings

    @view(renderer='json')
    def collection_get(self):
        return [ webhook.to_dict() for webhook in self.webhooks.all()
               ]

    @view(renderer='json')
    def get(self):
        return self.webhooks.filter(Webhook.uuid == self.request.matchdict['uuid']).one().to_dict()

    @view(renderer='json')
    def delete(self):
        webhook = self.webhooks.filter(Webhook.uuid == self.request.matchdict['uuid']).one()
    ...
    def collection_post(self):
        ...
        webhook = Webhook(owner=self.request.authenticated_userid, url=url, event_type=event_type, active=active)
        data = webhook.to_dict()
