...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/pytq-project/pytq/scheduler_mongodb.py
...
class MongoDBScheduler(BaseDBTableBackedScheduler, Encoder):
    ...
    def _col(self):
        import warnings
        warnings.warn(DeprecationWarning('this property will be deprecated soon!'))
    ...
    def _get_finished_id_set(self):
        ...
        """
        return set([ doc['_id'] for doc in self.col.find({}, {'_id': True}) ])

    def _default_post_process(self, task):
        """
        Save output_data into ``out`` field.
        """
        self.col.update({'_id': task.id}, {'$set': {self.output_key: self._encode(task.output_data)}}, upsert=True)
