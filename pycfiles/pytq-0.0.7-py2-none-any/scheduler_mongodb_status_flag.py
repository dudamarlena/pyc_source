...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/pytq-project/pytq/scheduler_mongodb_status_flag.py
...
class MongoDBStatusFlagScheduler(MongoDBScheduler, StatusFlag):
    ...
    def __init__(self, logger=None, collection=None, duplicate_flag=None, update_interval=None):
    ...
    def _default_is_duplicate(self, task):
        ...
        filters = {'_id': task.id, 
           self.status_key: {'$gte': self.duplicate_flag}, self.edit_at_key: {'$gte': n_sec_ago}}
    ...
    def _get_finished_id_set(self):
        ...
        wanted = {'_id': True}
        return set([ doc['_id'] for doc in self._col.find(filters, wanted) ])

    def _default_post_process(self, task):
        """
        Save output_data into ``out`` field.
        """
        self._col.update({'_id': task.id}, {'$set': {self.output_key: self._encode(task.output_data), 
                    self.status_key: self.duplicate_flag, 
                    self.edit_at_key: datetime.utcnow()}}, upsert=True)