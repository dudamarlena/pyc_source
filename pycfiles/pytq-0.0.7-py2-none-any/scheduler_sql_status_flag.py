...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/pytq-project/pytq/scheduler_sql_status_flag.py
# Compiled at: 2017-11-29 22:47:38
...
class SqlStatusFlagScheduler(SqlScheduler, StatusFlag):
    ...
    def __init__(self, logger=None, uri=None, table=None, duplicate_flag=None, update_interval=None):
        super(SqlStatusFlagScheduler, self).__init__(logger=logger, uri=uri, table=table)
        self.pre_process_duplicate_flag_and_update_interval(duplicate_flag, update_interval)

    def prepare_table(self, table):
        if table is not None:
            self.table = table
        if self.table is None:
            raise NotImplementedError('Please specify table name or sqlalchemy.Table!')
        if isinstance(self.table, sa.Table):
            ...
            table = sa.Table(self.table, metadata, id_column, sa.Column(self.out_key, sa.PickleType()), sa.Column(self.status_key, sa.Integer()), sa.Column(self.edit_at_key, sa.DateTime()))
            metadata.create_all(self.engine)
            self.table = table
        return
    ...
    def _default_is_duplicate(self, task):
        ...
        sql = sa.select([
         sa.func.count(self.id_col)]).where(sa.and_(self.id_col == task.id, self.status_col >= self.duplicate_flag, self.edit_at_col >= n_sec_ago))
    ...
    def _get_finished_id_set(self):
        ...
        """
        n_sec_ago = datetime.utcnow() - timedelta(seconds=self.update_interval)
        sql = sa.select([self.id_col]).where(sa.and_(self.status_col >= self.duplicate_flag, self.edit_at_col >= n_sec_ago))
    ...
    def _default_post_process(self, task):
        ...
        try:
            ...
            row.pop(self.id_key)
            upd = self.table.update().where(self.id_col == task.id).values(**row)
