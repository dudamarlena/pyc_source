...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/pytq-project/pytq/pkg/loggerFactory/logger.py
# Compiled at: 2017-11-23 13:02:07
import sys, string, random, logging
...
class StreamOnlyLogger(BaseLogger):
    ...
    **中文文档**
    ...
    def __init__(self, name=None, stream_level=DebugLevel.info, stream_format=DEFAULT_STREAM_FORMAT):
...
class SingleFileLogger(BaseLogger):
    ...
    **中文文档**
    ...
    def __init__(self, name=None, path=None, logging_level=DebugLevel.debug, stream_level=DebugLevel.info, logging_format=DEFAULT_LOG_FORMAT, stream_format=DEFAULT_STREAM_FORMAT, reset=False):
        logger = get_logger_by_name(name)
        if path is None:
...
class FileRotatingLogger(BaseLogger):
    ...
    **中文文档**
    ...
    def __init__(self, name=None, path=None, logging_level=DebugLevel.debug, stream_level=DebugLevel.info, logging_format=DEFAULT_LOG_FORMAT, stream_format=DEFAULT_STREAM_FORMAT, max_bytes=1000000000, backup_count=10):
        logger = logging.getLogger(name)
        if path is None:
            raise ValueError('Please specify a file as log path!')
        logger.setLevel(logging_level)
...
class TimeRotatingLogger(BaseLogger):
    ...
    **中文文档**
    ...
    def __init__(self, name=None, path=None, logging_level=DebugLevel.debug, stream_level=DebugLevel.info, logging_format=DEFAULT_LOG_FORMAT, stream_format=DEFAULT_STREAM_FORMAT, rotate_on_when='D', interval=1, backup_count=30):
        logger = logging.getLogger(name)
        if path is None:
            raise ValueError('Please specify a file as log path!')
        logger.setLevel(logging_level)
