...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/pytq-project/pytq/scheduler_sql.py
# Compiled at: 2017-11-29 22:47:10
...
class SqlScheduler(BaseDBTableBackedScheduler, Encoder):
    ...
    def prepare_table(self, table):
        ...
        if isinstance(self.table, sa.Table):
            ...
            table = sa.Table(self.table, metadata, id_column, sa.Column(self.out_key, sa.PickleType()))
            metadata.create_all(self.engine)
            self.table = table
        return
    ...
    def _default_is_duplicate(self, task):
        ...
        """
        sql = sa.select([
         sa.func.count(self.id_col)]).where(self.id_col == task.id)
    ...
    def get(self, id):
        sql = sa.select([self.out_col]).where(self.id_col == id)
        row = self.connection.execute(sql).fetchone()
        return row[0]