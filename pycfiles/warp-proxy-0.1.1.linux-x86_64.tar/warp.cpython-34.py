...
# [GCC 8.4.0]
# Embedded file name: /home/devunt/.virtualenvs/warp/lib/python3.4/site-packages/warp.py
# Compiled at: 2014-06-17 08:54:07
...
REGEX_HOST = re.compile('(.+?):([0-9]{1,5})')
REGEX_CONTENT_LENGTH = re.compile('\\r\\nContent-Length: ([0-9]+)\\r\\n')
REGEX_PROXY_CONNECTION = re.compile('\\r\\nProxy-Connection: (.+)\\r\\n')
REGEX_CONNECTION = re.compile('\\r\\nConnection: (.+)\\r\\n')
REGEX_USER_AGENTS_WITHOUT_PROXY_CONNECTION_HEADER = re.compile('\\r\\nUser-Agent: .*(Firefox|Opera).+\\r\\n')
clients = {}
...
def process_warp(client_reader, client_writer):
    ...
    if head[0] == 'CONNECT':
        try:
            try:
                ...
                tasks = [asyncio.Task(relay_stream(client_reader, req_writer)),
    ...
    try:
        ...
        def generate_dummyheaders():
            ...
            import string
            return ['X-%s: %s\r\n' % (generate_rndstrs(string.ascii_uppercase, 16), generate_rndstrs(string.ascii_letters + string.digits, 128)) for _ in range(32)]
...
def main():
    ...
    parser.add_argument('-H', '--host', default='127.0.0.1', help='Host to listen [default: %(default)s]')
    parser.add_argument('-p', '--port', type=int, default=8800, help='Port to listen [default: %(default)d]')
    parser.add_argument('-v', '--verbose', action='count', default=0, help='Print verbose')
