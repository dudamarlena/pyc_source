...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/setupmeta/versioning.py
# Compiled at: 2020-04-30 20:17:34
import io, os, re, setupmeta
from setupmeta.scm import Git, Snapshot, Version
BUMPABLE = ('major minor patch').split()
RE_VERSIONING = re.compile('^(branch(\\([\\w\\s,\\-]+\\))?:)?(.*?)([ +@#%^/]!?(.*))?(;(.*))?$')
...
class Strategy:
    ...
    def from_meta(cls, given):
        if not given:
            ...
            data = dict(main='{major}.{minor}.{patch}{post}', extra='{dirty}', separator='', branches='master', hook=None)
