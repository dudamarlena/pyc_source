...
# [GCC 8.4.0]
# Embedded file name: /mnt/data/projects/gluu/cluster-mgr/clustermgr/core/ldifschema_utils.py
# Compiled at: 2020-01-28 04:38:15
...
class OpenDjSchema(LDIFParser):
    ...
    def handle(self, dn, entry):
        self.schema = {'dn': dn, 
           ...
           'objectClasses': [], 'attributeTypes': [], 'ldapSyntaxes': entry.get('ldapSyntaxes', [])}
    ...
    def add_attribute_to_class(self, class_name, attribute_name):
        ...
        if c:
            if attribute_name not in c.may:
                may_list = list(c.may)
    ...
    def add_attribute(self, oid, names, syntax, origin, desc='', sup=(), substr='', equality='', single_value=False, obsolete=False, ordering=None, x_ordered=None, syntax_len=None, collective=False, no_user_mod=False, usage=0):
        a = AttributeType()
        a.oid = oid
        a.names = tuple(names)
        a.syntax = syntax
