...
# [GCC 8.4.0]
# Embedded file name: /mnt/data/projects/gluu/cluster-mgr/clustermgr/core/ldap_functions.py
...
class LdapOLC(object):
    ...
    def configureOxIDPAuthentication(self, servers):
        ...
        Returns:
            ldap modify result
        """
        if self.conn.search(search_base='ou=configuration,o=gluu', search_scope=BASE, search_filter='(objectclass=*)', attributes=[
         'oxIDPAuthentication']):
            ...
            if r:
                ...
                return self.conn.modify(r[0]['dn'], {'oxIDPAuthentication': [MODIFY_REPLACE, oxidp_s]})

    def changeOxCacheConfiguration(self, method, server_string=None, redis_password=None):
        result = self.conn.search(search_base='ou=configuration,o=gluu', search_scope=BASE, search_filter='(objectclass=*)', attributes=[
    ...
    def get_appliance_attributes(self, *args):
        ...
        Returns:
            the ldap entry
        """
        self.conn.search(search_base='o=gluu', search_filter='(objectclass=gluuAppliance)', search_scope=SUBTREE, attributes=list(args))
    ...
    def set_ldap_cache_cleanup_interval(self, interval=-1):
        result = self.conn.search(search_base='ou=oxauth,ou=configuration,o=gluu', search_scope=BASE, search_filter='(objectclass=*)', attributes=[
         'oxAuthConfDynamic'])
        ...
        return self.conn.modify(self.conn.response[0]['dn'], {'oxAuthConfDynamic': [MODIFY_REPLACE, oxAuthConfDynamic_json]})
...
def getLdapConn(addr, dn, passwd):
    ...
    try:
        ...
        if has_request_context():
            flash(('Connection to LDAPserver {0} at port 1636 failed: {1}').format(addr, e), 'danger')
        return
