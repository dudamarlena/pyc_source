...
# [GCC 8.4.0]
# Embedded file name: /mnt/data/projects/gluu/cluster-mgr/clustermgr/core/utils.py
# Compiled at: 2020-05-09 08:47:08
import re, os, hashlib, string, random, shlex, subprocess, uuid, base64, socket, json
...
def get_opendj_replication_status():
    ...
    installer = Installer(primary_server, app_conf.gluu_version, logger_task_id=-1, server_os=None)
    cmd = ("OPENDJ_JAVA_HOME=/opt/jre /opt/opendj/bin/dsreplication status -n -X -h {} -p 4444 -I admin -w $'{}'").format(primary_server.ip, app_conf.replication_pw.replace("'", "\\'"))
...
def modify_etc_hosts(host_ip, old_hosts):
    hosts = {'ipv4': {'127.0.0.1': ['localhost']}, 'ipv6': {'::1': ['ip6-localhost', 'ip6-loopback']}}
    for l in old_hosts.split('\n'):
        ...
        if ls:
            if not ls[0] == '#':
                ...
                for h in lss[1:]:
                    if h not in hosts[h_type][ip_addr]:
...
def run_and_log(c, cmd, tid, sid):
    ...
    if result[2].strip():
        if 'Redirecting to /bin/systemctl' in result[2]:
            ...
            wlogger.log(tid, ('An error occurrued while executing {}: {}').format(cmd, result[2]), 'error', server_id=sid)
    else:
        wlogger.log(tid, ('Command was run successfully: {}').format(cmd), 'success', server_id=sid)
    return result
...
def make_nginx_proxy_conf(exception=None):
    ...
    nginx_backends = []
    server_list = []
    nginx_tmp_file = os.path.join(app.root_path, 'templates', 'nginx', 'nginx.temp')
...
def is_hostname_resolved(hostname):
    ...
    try:
        ...
        result = ('Unable to resolve hostname {}. Please check the DNS record, or add the hostname to /etc/hosts of the cluster manager machine.').format(hostname)
