...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/spider_common/notify/scrapy/extensions/signal_notify_extension.py
# Compiled at: 2019-04-10 03:29:11
...
class ScrapySignalHandler(object):
    ...
    def from_crawler(cls, crawler):
        ...
        if signals_config is None:
            ...
            if isinstance(signals_config, list):
    ...
    def spider_error(self, failure, response, spider):
        ...
        self.send_notification({'title':'{} Spider Error'.format(spider.name), 
         'content':content})

    def item_error(self, item, response, spider, failure):
        content = 'Item Pipeline on {0}, traceback: {1}\nitem: {2}'.format(type(item), failure.getTraceback(), json.dumps(dict(**item)))
        self.send_notification({'title':'{1} Spider {1} item error'.format(spider.name, type(item).__name__), 
         'content':content})

    def spider_opened(self, spider):
        content = 'opened spider {}'.format(spider.name)
        self.send_notification({'title':'{} Spider Opened'.format(spider.name), 
         'content':content})

    def spider_closed(self, spider, reason):
        content = 'closed spider {}, reason: {}'.format(spider.name, reason)
        self.send_notification({'title':'{} Spider Closed'.format(spider.name), 
         'content':content})

    def spider_idle(self, spider):
        self.send_notification({'title':'{} Spider turn idle'.format(spider.name), 
         'content':''})