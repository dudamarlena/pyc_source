...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pymysqlbatchimport/run.py
# Compiled at: 2018-06-16 02:58:19
# Size of source mod 2**32: 2145 bytes
import os, shutil, configparser, inspect, pathlib
...
class App:
    ...
    def run(cnf_file=DEFAULT_CONFIG_FILE):
        ...
        visitor = LoadDataInFile(cnf_script[App.SCRIPT_CACHE_DIR], cnf_script[App.SCRIPT_DB_DBNAME], cnf_script[App.SCRIPT_DB_TABLE], cnf_load_data_infile)
        filebench = FileBench(cnf_script[App.SCRIPT_DATA_DIR], cnf_script[App.SCRIPT_FINISHED_DIR])
        iterator = iter(filebench.get_unprocessed_files())
        with LoadDataInFile.context(host=(cnf_script[App.SCRIPT_HOST]),
          database=(cnf_script[App.SCRIPT_DB_DBNAME]),
          user=(cnf_script[App.SCRIPT_USER]),
          password=(cnf_script[App.SCRIPT_PASSWORD]),
          autocommit=True,
          allow_local_infile=True) as (ctx_obj):
