...
# [GCC 8.4.0]
# Embedded file name: /Users/stalk/develop/django_affiliate/django-affiliate/affiliate/models.py
# Compiled at: 2015-10-31 12:19:11
...
class AbstractAffiliate(models.Model):
    user = models.OneToOneField(settings.AUTH_USER_MODEL, verbose_name=_(b'user'))
    reward_amount = models.DecimalField(_(b'reward amount'), max_digits=16, decimal_places=5, default=app_settings.REWARD_AMOUNT)
    reward_percentage = models.BooleanField(_(b'in percent'), default=app_settings.REWARD_PERCENTAGE)
    ...
    class Meta:
        abstract = True
        verbose_name = _(b'affiliate')
        verbose_name_plural = _(b'affiliates')
    ...
    def calc_affiliate_reward(self, total_price):
        if self.reward_percentage:
            return total_price * (self.reward_amount / D(b'100'))
        else:
            return self.reward_amount

    @property
    def quantized_reward_amount(self):
        return self.reward_amount.quantize(D(b'0.01'))
