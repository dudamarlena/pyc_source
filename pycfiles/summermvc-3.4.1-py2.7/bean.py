...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/summermvc/bean.py
# Compiled at: 2018-05-31 07:32:53
__all__ = [
 'Bean', 'is_bean_class',
 'component_scan_package', 'component_scan_path']
...
class Bean(object):
    ...
    def from_bean_class(cls, bean_class):
        ...
        bean_object = Bean()
        bean_object.__extract_attribute(bean_class, *args, **kwargs)
        return bean_object
    ...
    def __extract_attribute(self, bean_class, name=None, is_singleton=True):
        ...
        for attr_name, attr_value in get_declared_fields(self._cls):
            if isinstance(attr_value, AutoWiredField):
                self._auto_wired[attr_name] = attr_value.auto_wired or ('').join(w.capitalize() for w in attr_name.split('_'))
