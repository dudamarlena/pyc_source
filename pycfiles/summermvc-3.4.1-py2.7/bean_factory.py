# uncompyle6 version 3.7.4
# Python bytecode 2.7 (62211)
# Decompiled from: Python 3.6.9 (default, Apr 18 2020, 01:56:04) 
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/summermvc/bean_factory.py
# Compiled at: 2018-05-30 05:31:20
__all__ = [
 'BeanFactory']
__authors__ = ['Tim Chow']
import threading, types, re
from .exception import *
from .bean import *
from .decorator import *
from .reflect import get_declared_methods
from .wrapper import wrapper

class BeanFactory(object):

    def __init__(self, bean_classes):
        self._name_to_obj = {}
        self._lock = threading.RLock()
        self._name_to_bean = {}
        self._before_advice = {}
        self._around_advice = {}
        self._after_returning_advice = {}
        self._after_throwing_advice = {}
        self._after_advice = {}
        for bean_class in bean_classes:
            bean = Bean.from_bean_class(bean_class)
            if bean.name in self._name_to_bean:
                raise DuplicateBeanNameError
            self._name_to_bean[bean.name] = bean
            self.__initialize_advices(bean)

        self.__weave(bean_classes)

    def __initialize_advices(self, bean):
        order = get_aspect(bean.cls)
        if order is None:
            return
        else:
            for attr_name, attr in get_declared_methods(bean.cls):
                point_cut = get_before(attr)
                if point_cut is not None:
                    self._before_advice.setdefault(point_cut, []).append((
                     bean.name, attr_name, order))
                point_cut = get_around(attr)
                if point_cut is not None:
                    self._around_advice.setdefault(point_cut, []).append((
                     bean.name, attr_name, order))
                point_cut = get_after_throwing(attr)
                if point_cut is not None:
                    self._after_throwing_advice.setdefault(point_cut, []).append((
                     bean.name, attr_name, order))
                point_cut = get_after_returning(attr)
                if point_cut is not None:
                    self._after_returning_advice.setdefault(point_cut, []).append((
                     bean.name, attr_name, order))
                point_cut = get_after(attr)
                if point_cut is not None:
                    self._after_advice.setdefault(point_cut, []).append((
                     bean.name, attr_name, order))

            return

    def __weave(self, bean_classes):
        generate_advices = '{{ADVICE_TYPE}}s = []\nfor point_cut, advices in self._{{ADVICE_TYPE}}.iteritems():\n    if not re.match(point_cut.rstrip("$")+"$",\n            "%s %s" % (class_name, attr_name)):\n        continue\n    for advice in advices:\n        {{ADVICE_TYPE}}s.append((getattr(self.get_bean(advice[0]),\n            advice[1]), advice[2]))\n{{ADVICE_TYPE}}s = [advice[0] for advice in\n    sorted({{ADVICE_TYPE}}s, key=lambda t: t[1], reverse=True)]\n'
        for bean_class in bean_classes:
            class_name = bean_class.__name__
            for attr_name, attr in get_declared_methods(bean_class):
                for advice_type in ['before_advice', 'around_advice', 'after_returning_advice', 'after_throwing_advice',
                 'after_advice']:
                    exec (generate_advices.replace('{{ADVICE_TYPE}}', advice_type),
                     globals(), locals())

                if before_advices or around_advices or after_returning_advices or after_throwing_advices or after_advices:
                    setattr(bean_class, attr_name, wrapper(before_advices, around_advices, after_returning_advices, after_throwing_advices, after_advices)(getattr(bean_class, attr_name)))

    def get_bean(self, name):
        return self.__get_bean(name, {}, {})

    def __get_bean(self, name, creating, created):
        if name in self._name_to_obj:
            return self._name_to_obj[name]
        if name not in self._name_to_bean:
            raise BeanNotFoundError
        bean = self._name_to_bean[name]
        if bean.is_singleton:
            with self._lock:
                if name in self._name_to_obj:
                    return self._name_to_obj[name]
                else:
                    obj = bean.cls()
                    self.__process_denpendency(obj, bean, creating, created)
                    self._name_to_obj[name] = obj
                    bean.post_construct(obj)
                    return obj

        obj = bean.cls()
        self.__process_denpendency(obj, bean, creating, created)
        bean.post_construct(obj)
        return obj

    def __process_denpendency(self, obj, bean, creating, created):
        creating[bean.name] = obj
        for attr_name, attr_value in bean.properties.iteritems():
            setattr(obj, attr_name, attr_value)

        for attr_name, dependency_bean_name in bean.auto_wired.iteritems():
            if dependency_bean_name in creating:
                setattr(obj, attr_name, creating[dependency_bean_name])
                continue
            if dependency_bean_name in created:
                setattr(obj, attr_name, created[dependency_bean_name])
                continue
            setattr(obj, attr_name, self.__get_bean(dependency_bean_name, creating, created))

        del creating[bean.name]
        created[bean.name] = obj

    def destroy(self):
        for name in self._name_to_obj:
            obj = self._name_to_obj[name]
            bean = self._name_to_bean[name]
            try:
                bean.pre_destroy(obj)
            except:
                import traceback
                traceback.format_exc()

    def iter_beans(self):
        for item in self._name_to_bean.iteritems():
            yield item