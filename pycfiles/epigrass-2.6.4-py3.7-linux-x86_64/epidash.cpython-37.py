...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/Epigrass/epidash.py
# Compiled at: 2020-04-13 13:04:26
...
def get_sims():
    if is_epigrass_folder:
        ...
        return [s[0] for s in sims if not s[0].endswith('_meta') if not s[0].endswith('e')]
    return ['No simulations found']


def generate_table(dataframe, max_rows=10):
    return html.Table([
     html.Tr([html.Th(col) for col in dataframe.columns])] + [html.Tr([html.Td(dataframe.iloc[i][col]) for col in dataframe.columns]) for i in range(min(len(dataframe), max_rows))])
...
@app.callback(Output(component_id='series-plot', component_property='figure'), [
...
@app.callback(Output(component_id='columns', component_property='options'), [
 Input(component_id='sim-drop', component_property='value')])
def fill_columns(sim_name):
    try:
        df = load_sim(sim_name)
        return [{'label':c,  'value':c} for c in df.columns if c not in ('geocode,time',
                                                                         'name',
                                                                         'lat', 'longit')]
...
@app.callback(Output(component_id='localities', component_property='options'), [
 Input(component_id='sim-drop', component_property='value')])
