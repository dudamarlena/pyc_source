...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/Epigrass/simobj.py
# Compiled at: 2020-04-13 15:37:53
# Size of source mod 2**32: 69159 bytes
"""
This Module contains the definitions of objects for spatial simulation on geo reference spaces.
"""
from __future__ import absolute_import
from __future__ import print_function
import sys, multiprocessing, time, json
...
class siteobj(object):
    ...
    def createModel(self, modtype='', name='model1', v=[], bi=None, bp=None):
        ...
        try:
            self.ts.append(list(bi.values()))
        except KeyError as ke:
            try:
                if self.vnames == ['Exposed', 'Infectious', 'Susceptible']:
                    self.ts = [
                     [bi[vn] for vn in ('e', 'i', 's')]]
                else:
                    raise KeyError('%s' % ke)
            finally:
                ke = None
    ...
    def runModel(self, parallel=True):
        ...
        if self.thetalist != []:
            ...
            self.infector = dict([i for i in self.thetalist if i[1] > 0])
        else:
            theta = 0
            self.infector = {}
    ...
    def handle(self, res):
        ...
        """
        pipe = redisclient.pipeline()
        last_state, Lpos, migInf = pipe.lindex('{}:ts'.format(self.geocode), -1).get('{}:Lpos'.format(self.geocode)).get('{}:migInf'.format(self.geocode)).execute()
        Lpos = float(Lpos)
        migInf = float(migInf)
    ...
    def getTheta(self, npass, delay):
        ...
        if self.stochtransp == 0:
            theta = npass * migInf / float(self.totpop)
        else:
            try:
    ...
    def getThetaindex(self):
        ...
        if self.thidx:
            return self.thidx
        self.thidx = thidx = sum([(i.fmig + i.bmig) / 2.0 for i in self.edges]) / len(self.parentGraph.site_list)
        return thidx
    ...
    def doStats(self):
        ...
        return [
         self.centrality, self.degree, self.thidx, self.betweeness]
    ...
    def getBetweeness(self):
        ...
        for i in self.parentGraph.shortPathList:
            if self not in i and self in i[2]:
                B += 1
...
class edge(object):
    ...
    def __init__(self, source, dest, fmig=0, bmig=0, Leng=0):
        ...
        self.fmig = float(fmig)
        self.bmig = float(bmig)
        self.parentGraph = None
        self.length = Leng
        self.delay = 0
        self.ftheta = []
...
class graph(object):
    ...
    def addEdge(self, graph_edge):
        ...
        if graph_edge.source.geocode not in self.site_dict:
            raise KeyError('Edge source does not belong to the graph')
        if graph_edge.dest.geocode not in self.site_dict:
    ...
    def getEdge(self, src, dst):
        ...
        None is returned otherwise.
        """
        match = [edge for edge in self.edge_list if edge.source == src if edge.dest == dst]
        l = len(match)
        if l == 1:
            return match[0]
        if l > 1:
    ...
    def getAllPairs(self):
        ...
        for sitei in six.iterkeys(g):
            j = 0
            for sitej in list(g.keys())[:i]:
                sp = self.shortestPath(g, sitei, sitej)
                lsp = self.getShortestPathLength(sitei, sp)
    ...
    def getConnMatrix(self):
        ...
        """
        try:
            if self.connmatrix.any():
                return self.connmatrix
        except:
            pass

        if not self.graphdict:
            self.getGraphdict()
        ...
        for i, sitei in enumerate(site_list):
            for j, sitej in enumerate(site_list[:i]):
                if sitei == sitej:
                    pass
                else:
    ...
    def getPiIndex(self):
        ...
        if self.length:
            ...
            l = self.getLength()
        lsp = [len(i[2]) for i in self.shortPathList]
        lpidx = lsp.index(max(lsp))
        lp = self.shortPathList[lpidx][2]
        Dd = 0
    ...
    def doStats(self):
        ...
        return [
         self.allPairs, self.cycles, self.wienerD, self.meanD, self.diameter, self.length,
    ...
    def getEpistats(self):
        ...
        return [
         self.episize, self.epispeed, self.infectedcities,
...
class priorityDictionary(dict):
    ...
    def __setitem__(self, key, val):
        ...
        if len(heap) > 2 * len(self):
            self._priorityDictionary__heap = [(v, k) for k, v in six.iteritems(self)]
