...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/sciplot/__init__.py
# Compiled at: 2016-04-07 16:50:26
"""
...
def min_max_dif(values):
    ...
    return (
     min(difs), sorted_values[(-1)] - sorted_values[0])
...
class BasePlot(object):
    ...
    def _draw_plot(self, fig=None, ax=None):
        ...
        ax.margins(x=0.03, y=0.03, tight=True)
...
class Network(BasePlot):
    ...
    as of 2016-02). You may have to add `C:\\Program Files (x86)\\Graphviz2.38\x08in`
    ...
    def _draw_plot(self, fig=None, ax=None):
        ...
        if self.directed:
            for start, end in self.data:
                ...
                ax.add_patch(FancyArrowPatch(posA=(start_x, start_y), posB=end_pos, color='k', arrowstyle=self.arrowstyle, mutation_scale=30, connectionstyle='arc3'))
    ...
    def _draw(self, keys, values, ax):
        ...
        if len(rects) <= self.max_direct_labels:
            ...
            for rect in rects:
                ...
                ax.text(rect.get_x() + rect.get_width() / 2.0, y, value, ha='center', va='top')
