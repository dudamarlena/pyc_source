...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/app/app.py
# Compiled at: 2018-04-28 06:28:34
# Size of source mod 2**32: 12638 bytes
import os, gym, random, warnings, argparse, numpy as np, pandas as pd, tensorflow as tf
...
if not is_ci_enabled:
    print('CI disabled')
    import gym_gomoku, nesgym_super_mario_bros
warnings.simplefilter('ignore')
parser = argparse.ArgumentParser()
parser.add_argument('--environments', default='act', type=str, help='Show a list of environments available')
parser.add_argument('--env_name', default='pacman', type=str, help='Generated environment name')
parser.add_argument('--environment_name', default='MsPacman-v0', type=str, help='The gym environment name')
parser.add_argument('--output_stats_filename', type=str, help='Statistics about turn saved on a csv file')
...
parser.add_argument('--action_type', default='conditional', type=str, help='Kind of usage for action sample')
...
parser.add_argument('--pre_defined_state_size', default='gym', type=str, help='Observation shape based state size')
...
stats = {'observations':statistics.observations,  'rewards':statistics.rewards,  'output':{'done':statistics.output.done, 
  'info':statistics.output.info,  'timestep':{'iteration':statistics.output.timestep.iteration, 
...
def composed_sample(s=2, vm=None):
    if vm:
        ...
        gen_list_based_sample = lambda subdued_limit: [gen_sample() for _ in range(subdued_limit)]
...
def main(argv):
    ...
    if is_environments_gen(args):
        ...
        if is_environments_list(args):
            ...
            if is_environments_act(args):
                env = gym.make(args.environment_name)
                if is_action_type('dqn', args):
                    if args.pre_defined_state_size == 'nesgym':
                        state_size = 172032
                    else:
                        if args.pre_defined_state_size == 'gym':
                            state_size = env.observation_space.shape[0]
                        else:
                            if args.pre_defined_state_size == 'gym-atari':
                                state_size = 100800
                            else:
                                if args.pre_defined_state_size == 'gym-atari-extend':
                                    state_size = 120000
                                else:
                                    if args.pre_defined_state_size == 'gym-gomoku':
                                        state_size = 361
                ...
                for i_episode in range(i_episodes):
                    ...
                    for t in range(timesteps):
                        try:
                            try:
                                if args.render == 'present':
                                    env.render()
                                else:
                                    if args.render == 'presented':
                                    ...
                                    if args.action_type == 'alternate':
                                        ...
                                        if args.action_type == 'specific':
                                            action = env.action_space.sample()
                                        else:
                                            if args.action_type == 'conditional':
                                                action_choice = i_episodes
                                                action = random_action_space_sample_choice(action_choice, env, factor)
                                            else:
                                                if args.action_type == 'numerical':
                                    ...
                                    if done:
                                        ...
                                        if is_action_type('dqn', args):
                                            print('Episode: {}/{}, score: {}, e: {:.2}'.format(i_episode, i_episodes, t, agent.epsilon))
