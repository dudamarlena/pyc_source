...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/geopyspark/geotrellis/geotiff.py
# Compiled at: 2018-12-10 10:02:50
# Size of source mod 2**32: 4683 bytes
"""This module contains functions that create ``RasterLayer`` from files."""
from geopyspark import get_spark_context
from geopyspark.geotrellis.constants import LayerType, DEFAULT_MAX_TILE_SIZE, DEFAULT_PARTITION_BYTES, DEFAULT_CHUNK_SIZE, DEFAULT_GEOTIFF_TIME_TAG, DEFAULT_GEOTIFF_TIME_FORMAT, DEFAULT_S3_CLIENT
from geopyspark.geotrellis.layer import RasterLayer
__all__ = [
 'get']

def get(layer_type, uri, crs=None, max_tile_size=DEFAULT_MAX_TILE_SIZE, num_partitions=None, chunk_size=DEFAULT_CHUNK_SIZE, partition_bytes=DEFAULT_PARTITION_BYTES, time_tag=DEFAULT_GEOTIFF_TIME_TAG, time_format=DEFAULT_GEOTIFF_TIME_FORMAT, delimiter=None, s3_client=DEFAULT_S3_CLIENT):
    ...
    """
    inputs = {k:v for k, v in locals().items() if v is not None if v is not None}
    ...
    if isinstance(uri, list):
        srdd = geotiff_rdd.get(pysc._jsc.sc(), key, inputs.pop('uri'), inputs, partition_bytes)
    else:
        srdd = geotiff_rdd.get(pysc._jsc.sc(), key, [
         inputs.pop('uri')], inputs, partition_bytes)
