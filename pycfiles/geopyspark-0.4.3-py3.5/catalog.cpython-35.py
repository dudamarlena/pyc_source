...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/geopyspark/geotrellis/catalog.py
# Compiled at: 2018-12-10 10:02:50
# Size of source mod 2**32: 17412 bytes
"""Methods for reading, querying, and saving tile layers to and from GeoTrellis Catalogs.
"""
import json
from shapely.geometry import Polygon, MultiPolygon, Point
import shapely.wkb, pytz
...
__all__ = [
 'read_layer_metadata', 'read_value', 'query', 'write', 'AttributeStore']
_cached_stores = {}

def read_layer_metadata(uri, layer_name, layer_zoom):
...
def read_value(uri, layer_name, layer_zoom, col, row, zdt=None):
...
def query(uri, layer_name, layer_zoom=None, query_geom=None, time_intervals=None, query_proj=None, num_partitions=None):
    ...
    if query_geom is None:
        ...
        if isinstance(query_geom, Extent):
            ...
            if isinstance(query_geom, (Polygon, MultiPolygon, Point)):
                ...
                if isinstance(query_geom, bytes):
                    pass
                else:
        ...
        srdd = reader.query(uri, layer_name, layer_zoom, query_geom, time_intervals, query_proj, num_partitions)
        layer_type = LayerType._from_key_name(srdd.keyClassName())
    return TiledRasterLayer(layer_type, srdd)


def write(uri, layer_name, tiled_raster_layer, index_strategy=IndexingMethod.ZORDER, time_unit=None, time_resolution=None, store=None):
...
class AttributeStore(object):
    ...
    class Attributes(object):
        __doc__ = 'Accessor class for all attributes for a given layer'
