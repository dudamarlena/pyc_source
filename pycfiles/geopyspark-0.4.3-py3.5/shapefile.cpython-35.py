...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/geopyspark/geotools/shapefile.py
...
def get(uri, extensions=[
 '.shp', '.SHP'], num_partitions=None, s3_client=DEFAULT_S3_CLIENT):
    r"""Creates an ``RDD[Feature]`` from Shapefile(s) that are located on the local file system, ``HDFS``,
    or ``S3``.
