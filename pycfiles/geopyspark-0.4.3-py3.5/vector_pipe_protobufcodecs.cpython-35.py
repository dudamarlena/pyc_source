...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/geopyspark/vector_pipe/vector_pipe_protobufcodecs.py
...
ensure_pyspark()
from geopyspark.vector_pipe.protobuf.featureMessages_pb2 import ProtoFeature, ProtoFeatureCellValue, ProtoMetadata, ProtoTags, ProtoTag, ProtoCellValue
...
def from_pb_properties(pb_metadata):
    ...
    return Properties(element_id=pb_metadata.id, user=pb_metadata.user, uid=pb_metadata.uid, changeset=pb_metadata.changeset, version=pb_metadata.version, minor_version=pb_metadata.minorVersion, timestamp=time, visible=pb_metadata.visible, tags=tags)
...
def from_pb_feature_cellvalue(pb_feature_cellvalue):
    ...
    cellvalue = CellValue(pb_feature_cellvalue.cellValue.value, pb_feature_cellvalue.cellValue.zindex)
...
def to_pb_properties(metadata):
    ...
    return ProtoMetadata(id=metadata[0], user=metadata[1], uid=metadata[2], changeset=metadata[3], version=metadata[4], minorVersion=metadata[5], timestamp=str(metadata[6]), visible=metadata[7], tags=pb_tags)
