...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/geopyspark/geotrellis/s3.py
...
def set_s3_credentials(credentials, uri_type):
    ...
    if credentials:
        if uri_type not in _S3_URI_PREFIXES:
            raise RuntimeError('Cannot set S3 credentials for unrecognized URI type {}'.format(uri_type))
...
def is_s3_uri(uri):
    ...
    """
    return any(uri.startswith(prefix + '://') for prefix in _S3_URI_PREFIXES)