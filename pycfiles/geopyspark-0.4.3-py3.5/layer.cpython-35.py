...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/geopyspark/geotrellis/layer.py
# Compiled at: 2018-12-10 10:02:50
# Size of source mod 2**32: 104924 bytes
"""
...
from geopyspark.geotrellis.protobufcodecs import multibandtile_decoder, projected_extent_decoder, temporal_projected_extent_decoder, spatial_key_decoder, space_time_key_decoder
...
from geopyspark.geotrellis import Metadata, Tile, LocalLayout, GlobalLayout, LayoutDefinition, crs_to_proj4, _convert_to_unix_time, HashPartitionStrategy, SpatialPartitionStrategy, SpaceTimePartitionStrategy, RasterizerOptions
from geopyspark.geotrellis.histogram import Histogram
from geopyspark.geotrellis.constants import Operation, Neighborhood as nb, ResampleMethod, ClassificationStrategy, CellType, LayerType, StorageMethod, ColorSpace, Compression, TimeUnit, NO_DATA_INT
from geopyspark.geotrellis.neighborhood import Neighborhood
__all__ = [
 'RasterLayer', 'TiledRasterLayer', 'Pyramid']

def _reclassify(srdd, value_map, data_type, classification_strategy, replace_nodata_with, fallback_value, strict):
    new_dict = {}
    ...
    if data_type is int:
        return srdd.reclassify(new_dict, ClassificationStrategy(classification_strategy).value, replace_nodata_with, fallback_value, strict)
    else:
        return srdd.reclassifyDouble(new_dict, ClassificationStrategy(classification_strategy).value, replace_nodata_with, fallback_value, strict)


def _to_geotiff_rdd(pysc, srdd, storage_method, rows_per_strip, tile_dimensions, compression, color_space, color_map, head_tags, band_tags):
    ...
    if storage_method == StorageMethod.STRIPED:
        if rows_per_strip:
            scala_storage = pysc._gateway.jvm.geotrellis.raster.io.geotiff.Striped.apply(rows_per_strip)
        else:
            scala_storage = pysc._gateway.jvm.geotrellis.raster.io.geotiff.Striped.apply()
    else:
        scala_storage = pysc._gateway.jvm.geotrellis.raster.io.geotiff.Tiled(*tile_dimensions)
    if color_map:
        return srdd.toGeoTiffRDD(scala_storage, compression, color_space, color_map.cmap, head_tags or {}, band_tags or [])
    else:
        return srdd.toGeoTiffRDD(scala_storage, compression, color_space, head_tags or {}, band_tags or [])
...
def _reproject(target_crs, layout, resample_method, partition_strategy, layer):
    ...
    if isinstance(layout, Metadata):
        ...
        return TiledRasterLayer(layer.layer_type, srdd)
    if isinstance(layout, TiledRasterLayer):
    ...
    def get_partition_strategy(self):
        ...
        if partition_name:
            if partition_name == 'HashPartitioner':
                ...
                if partition_name == 'SpaceTimePartitioner':
                    scala_partitioner = self.srdd.rdd().partitioner().get()
                    return SpaceTimePartitionStrategy(TimeUnit(scala_partitioner.getTimeUnit()), self.getNumPartitions(), scala_partitioner.getBits(), scala_partitioner.getTimeResolution())
                scala_partitioner = self.srdd.rdd().partitioner().get()
...
class RasterLayer(CachableLayer, TileLayer):
    ...
    def from_numpy_rdd(cls, layer_type, numpy_rdd):
        r"""Create a ``RasterLayer`` from a numpy RDD.
        ...
        if layer_type == LayerType.SPATIAL:
            srdd = pysc._gateway.jvm.geopyspark.geotrellis.ProjectedRasterLayer.fromProtoEncodedRDD(reserialized_rdd._jrdd)
        else:
            srdd = pysc._gateway.jvm.geopyspark.geotrellis.TemporalRasterLayer.fromProtoEncodedRDD(reserialized_rdd._jrdd)
    ...
    def to_geotiff_rdd(self, storage_method=StorageMethod.STRIPED, rows_per_strip=None, tile_dimensions=(256, 256), compression=Compression.NO_COMPRESSION, color_space=ColorSpace.BLACK_IS_ZERO, color_map=None, head_tags=None, band_tags=None):
        r"""Converts the rasters within this layer to GeoTiffs which are then converted to bytes.
        ...
        Returns:
            RDD[(K, bytes)]
        """
        result = _to_geotiff_rdd(self.pysc, self.srdd, storage_method, rows_per_strip, tile_dimensions, compression, color_space, color_map, head_tags, band_tags)
        key = LayerType(self.layer_type)._key_name(False)
        ser = ProtoBufSerializer.create_image_rdd_serializer(key_type=key)
        return create_python_rdd(result, ser)
    ...
    def bands(self, band):
        r"""Select a subsection of bands from the ``Tile``\s within the layer.
    ...
    def map_tiles(self, func):
        ...
        return RasterLayer.from_numpy_rdd(self.layer_type, python_rdd.mapValues(lambda tile: func(tile.cells)))
    ...
    def map_cells(self, func):
        ...
        return RasterLayer.from_numpy_rdd(self.layer_type, python_rdd.mapValues(lambda tile: tile_func(*tile)))
    ...
    def tile_to_layout(self, layout=LocalLayout(), target_crs=None, resample_method=ResampleMethod.NEAREST_NEIGHBOR, partition_strategy=None):
    ...
    def reclassify(self, value_map, data_type, classification_strategy=ClassificationStrategy.LESS_THAN_OR_EQUAL_TO, replace_nodata_with=None, fallback_value=None, strict=False):
        ...
        Returns:
            :class:`~geopyspark.geotrellis.layer.RasterLayer`
        """
        srdd = _reclassify(self.srdd, value_map, data_type, classification_strategy, replace_nodata_with, fallback_value, strict)
        return RasterLayer(self.layer_type, srdd)
...
class TiledRasterLayer(CachableLayer, TileLayer):
    ...
    def from_numpy_rdd(cls, layer_type, numpy_rdd, metadata, zoom_level=None):
        ...
        if layer_type == LayerType.SPATIAL:
            if zoom_level:
                srdd = spatial_tiled_raster_layer.fromProtoEncodedRDD(reserialized_rdd._jrdd, zoom_level, json.dumps(metadata))
            else:
                srdd = spatial_tiled_raster_layer.fromProtoEncodedRDD(reserialized_rdd._jrdd, json.dumps(metadata))
        else:
            if zoom_level:
                srdd = temporal_tiled_raster_layer.fromProtoEncodedRDD(reserialized_rdd._jrdd, zoom_level, json.dumps(metadata))
            else:
                srdd = temporal_tiled_raster_layer.fromProtoEncodedRDD(reserialized_rdd._jrdd, json.dumps(metadata))
        return cls(layer_type, srdd)
    ...
    def map_tiles(self, func):
        ...
        """
        python_rdd = self.to_numpy_rdd()
        return TiledRasterLayer.from_numpy_rdd(self.layer_type, python_rdd.mapValues(lambda tile: func(tile)), self.layer_metadata, self.zoom_level)
    ...
    def map_cells(self, func):
        ...
        return TiledRasterLayer.from_numpy_rdd(self.layer_type, python_rdd.mapValues(lambda tile: tile_func(*tile)), self.layer_metadata, self.zoom_level)

    def aggregate_by_cell(self, operation):
        r"""Computes an aggregate summary for each cell of all of the values for each key.
    ...
    def convert_data_type(self, new_type, no_data_value=None):
        ...
        if no_data_value is not None:
            if 'bool' in new_type:
                ...
                raise ValueError('Cannot add user defined types to raw values')
            no_data_constant = new_type + 'ud' + str(no_data_value)
            return TiledRasterLayer(self.layer_type, self.srdd.convertDataType(no_data_constant))
        else:
            return TiledRasterLayer(self.layer_type, self.srdd.convertDataType(new_type))
    ...
    def tile_to_layout(self, layout, target_crs=None, resample_method=ResampleMethod.NEAREST_NEIGHBOR, partition_strategy=None):
    ...
    def focal(self, operation, neighborhood=None, param_1=None, param_2=None, param_3=None, partition_strategy=None):
        ...
        if isinstance(neighborhood, Neighborhood):
            srdd = self.srdd.focal(operation, neighborhood.name, neighborhood.param_1, neighborhood.param_2, neighborhood.param_3, partition_strategy)
        else:
            if isinstance(neighborhood, (str, nb)):
                param_1 = param_1 or 0.0
                param_2 = param_2 or 0.0
                param_3 = param_3 or 0.0
                srdd = self.srdd.focal(operation, nb(neighborhood).value, float(param_1), float(param_2), float(param_3), partition_strategy)
            else:
                if not neighborhood and operation == Operation.ASPECT.value:
    ...
    def stitch(self):
        r"""Stitch all of the rasters within the Layer into one raster.
    ...
    def save_stitched(self, path, crop_bounds=None, crop_dimensions=None):
        r"""Stitch all of the rasters within the Layer into one raster and then saves it to a given
        path.
    ...
    def mask(self, geometries, partition_strategy=None, options=RasterizerOptions()):
    ...
    def get_point_values(self, points, resample_method=None):
        r"""Returns the values of the layer at given points.
                ...
                if self.layer_type == LayerType.SPATIAL:
                    for key, value in scala_result.items():
                        dict_result[ided_labels[key]] = (
                         ided_points[key], list(value) if value else None)

                else:
                    for key, value_map in scala_result.items():
                        converted_values = [(parser.parse(k), list(v)) for k, v in value_map.items()]
                        dict_result[ided_labels[key]] = (
                         ided_points[key], converted_values)

                    for remaining_id in set(ided_points.keys()).difference(set(scala_result.keys())):
                        dict_result[ided_labels[remaining_id]] = (
                         ided_points[remaining_id], [(None, None)])
    ...
    def _process_operation(self, value, operation):
        if isinstance(value, int) or isinstance(value, float):
            ...
            if isinstance(value, TiledRasterLayer):
    ...
    def local_max(self, value):
        r"""Determines the maximum value for each cell of each ``Tile`` in the layer.
    ...
    def __str__(self):
        return 'TiledRasterLayer(layer_type={}, zoom_level={}, is_floating_point_layer={})'.format(self.layer_type, self.zoom_level, self.is_floating_point_layer)
...
class Pyramid(CachableLayer):
    ...
    def __str__(self):
        return 'Pyramid(layer_type={}, max_zoom={}, num_levels={}, is_cached={})'.format(self.layer_type, self.max_zoom, len(self.levels), self.is_cached)
