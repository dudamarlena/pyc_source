...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/geopyspark/geotrellis/rasterio.py
# Compiled at: 2018-12-10 10:02:50
# Size of source mod 2**32: 4318 bytes
import os, math, geopyspark as gps
...
def _read_windows(uri, xcols, ycols, bands, crs_to_proj4):
    ...
    with rasterio.open(uri) as (dataset):
        ...
        windows = [((x, min(width - 1, x + xcols)), (y, min(height - 1, y + ycols))) for x in range(0, tile_cols, xcols) for y in range(0, tile_rows, ycols)]
...
def get(data_source, xcols=DEFAULT_MAX_TILE_SIZE, ycols=DEFAULT_MAX_TILE_SIZE, bands=None, crs_to_proj4=crs_to_proj4):
    r"""Creates an ``RDD`` of windows represented as the key value pair: ``(ProjectedExtent, Tile)``
    ...
    if isinstance(data_source, (list, str)):
        if isinstance(data_source, str):
            data_source = [
             data_source]
        return pysc.parallelize(data_source, len(data_source)).flatMap(lambda ds: _read_windows(ds, xcols, ycols, bands, crs_to_proj4))
    else:
