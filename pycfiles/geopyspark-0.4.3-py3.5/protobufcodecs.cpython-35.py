...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/geopyspark/geotrellis/protobufcodecs.py
# Compiled at: 2018-12-10 10:02:50
# Size of source mod 2**32: 20391 bytes
"""Contains the various encoding/decoding methods to bring values to/from Python from Scala."""
from functools import partial
import datetime, numpy as np
from geopyspark.geopyspark_utils import ensure_pyspark
ensure_pyspark()
from geopyspark.geotrellis import Extent, ProjectedExtent, TemporalProjectedExtent, SpatialKey, SpaceTimeKey, Tile, _convert_to_unix_time
...
def from_pb_projected_extent(pb_projected_extent):
    ...
    if pb_projected_extent.crs.epsg is not 0:
        return ProjectedExtent(extent=from_pb_extent(pb_projected_extent.extent), epsg=pb_projected_extent.crs.epsg)
    else:
        return ProjectedExtent(extent=from_pb_extent(pb_projected_extent.extent), proj4=pb_projected_extent.crs.proj4)
...
def from_pb_temporal_projected_extent(pb_temporal_projected_extent):
    ...
    if pb_temporal_projected_extent.crs.epsg is not 0:
        return TemporalProjectedExtent(extent=from_pb_extent(pb_temporal_projected_extent.extent), epsg=pb_temporal_projected_extent.crs.epsg, instant=instant)
    else:
        return TemporalProjectedExtent(extent=from_pb_extent(pb_temporal_projected_extent.extent), proj4=pb_temporal_projected_extent.crs.proj4, instant=instant)
...
def from_pb_space_time_key(pb_space_time_key):
    ...
    """
    return SpaceTimeKey(col=pb_space_time_key.col, row=pb_space_time_key.row, instant=datetime.datetime.utcfromtimestamp(pb_space_time_key.instant / 1000))
