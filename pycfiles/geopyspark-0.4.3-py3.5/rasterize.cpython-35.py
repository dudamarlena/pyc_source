...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/geopyspark/geotrellis/rasterize.py
...
from geopyspark.vector_pipe.vector_pipe_protobufcodecs import feature_cellvalue_decoder, feature_cellvalue_encoder
__all__ = [
 'rasterize', 'rasterize_features']

def rasterize(geoms, crs, zoom, fill_value, cell_type=CellType.FLOAT64, options=None, partition_strategy=None):
    ...
    if isinstance(geoms, (list, tuple)):
        ...
        srdd = rasterizer(pysc._jsc.sc(), wkb_geoms, crs, zoom, float(fill_value), CellType(cell_type).value, options, partition_strategy)
    else:
        wkb_rdd = geoms.map(lambda geom: dumps(geom))
        wkb_rdd._bypass_serializer = True
        srdd = rasterizer(wkb_rdd._jrdd.rdd(), crs, zoom, float(fill_value), CellType(cell_type).value, options, partition_strategy)
    return TiledRasterLayer(LayerType.SPATIAL, srdd)


def rasterize_features(features, crs, zoom, cell_type=CellType.FLOAT64, options=None, zindex_cell_type=CellType.INT8, partition_strategy=None):
    r"""Rasterizes a collection of :class:`~geopyspark.vector_pipe.Feature`\s.
    ...
    srdd = rasterizer(reserialized_rdd._jrdd.rdd(), crs, zoom, CellType(cell_type).value, options, CellType(zindex_cell_type).value, partition_strategy)
    return TiledRasterLayer(LayerType.SPATIAL, srdd)