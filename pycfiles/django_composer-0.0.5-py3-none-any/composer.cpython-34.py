...
# [GCC 8.4.0]
# Embedded file name: /home/ben/repos/django-composer/composer/templatetags/composer.py
# Compiled at: 2015-08-31 13:14:32
...
class ComposerNode(template.Node):
    ...
    def render(self, context):
        ...
        if self.name not in elements:
            context['composer_elements'][self.name] = Element.objects.create(template_name=template, name=self.name, is_dynamic=self.dynamic, context_example=serialize_context(el_context), content=str(self.content))
...
def do_composer_dynamic(parser, token):
    ...
    try:
        ...
        raise template.TemplateSyntaxError('%r tag requires either 1 or 2 arguments' % token.contents.split()[0])
