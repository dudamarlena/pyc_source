...
# [GCC 8.4.0]
# Embedded file name: /home/naufraghi/.virtualenvs/react-vs-elm/lib/python3.5/site-packages/git_svn_clone_externals.py
# Compiled at: 2016-09-21 03:11:03
...
def git_svn_rebase(path='.'):
    logger.info('git svn rebase %s', path)
    with git_stasher(path):
...
def svn_info(git=False):
    cmd = ['svn', 'info']
    if git:
        cmd = [
         'git'] + cmd
    info = check_output(cmd).decode('utf8')
    info_dict = {}
...
def normalize_externals(repo_root, externals):
    for root, external in externals:
        ...
        if len(parts) == 2:
            ...
            rev = rev.strip('-r')
        yield (
         rev, extpath_join(repo_root, uri), os.path.normpath(os.path.join(root, path)))
...
def run():
    parser = GitSvnArgumentParser(description='git svn clone and follow svn:externals,                                                all unknown arguments are forwarded                                                (use -r HEAD for a shallow clone)')
