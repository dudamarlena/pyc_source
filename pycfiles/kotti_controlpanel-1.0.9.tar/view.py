...
# [GCC 8.4.0]
# Embedded file name: /home/ubuntu/workspace/kotti_controlpanel/kotti_controlpanel/views/view.py
...
class BaseSettingViews(BaseView):
    ...
    @view_config(name='controlpanel-dump', permission='admin', root_only=True, renderer='kotti_controlpanel:templates/controlpanel-dump.pt')
    def dump_all_settings(self):
        settings = util.get_settings()
        return {'all_settings': settings, 
           'UCP_LINKS': CONTROL_PANEL_LINKS}

    @view_config(name='controlpanel', custom_predicates=(
     is_root,), permission='admin', renderer='kotti_controlpanel:templates/controlpanel.pt')
    def view(self):
        setting_id = self.request.params.get('setting_id')
        if not setting_id:
            settings_form_views = []
            for settings in SETTINGS.values():
                settings_form_views.append(settings)

            return {'settings': settings_form_views, 
               'UCP_LINKS': CONTROL_PANEL_LINKS}
        settings = SETTINGS.get(setting_id)
        if not settings:
            return httpexc.HTTPNotFound()
        args = {'title': settings.title, 'description': settings.description, 
           'name': settings.name, 
        ...
        link1 = Link('controlpanel', title=_('Control Panel'))
        if link1 not in links:
            links.append(link1)
        if self.request.has_permission('admin'):
            link2 = Link('controlpanel-dump', title=_('All Settings'))
            if link2 not in links:
                links.append(link2)
        template = settings.template or 'kotti_controlpanel:templates/settings.pt'
        return render_to_response(template, {'settings': settings, 
           'settings_form': form, 
           'cp_links': links, 
           'UCP_LINKS': CONTROL_PANEL_LINKS}, request=self.request)