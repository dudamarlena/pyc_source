...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/namedstruct/stdprim.py
# Compiled at: 2017-08-22 03:32:36
"""
...
from namedstruct.namedstruct import nstruct, prim, raw, char, enum, varchr, cstr, optional, bitfield, darray
int8 = prim('b', 'int8')
uint8 = prim('B', 'uint8')
boolean = prim('?', 'bool')
int16 = prim('h', 'int16')
...
def fit_to_size(value, size):
    if len(value) >= size:
        ...
        return '\x00' * (size - len(value)) + value
...
def fit_to_size_le(value, size):
    if len(value) >= size:
        ...
        return value + '\x00' * (size - len(value))
...
def create_binary(value, size, littleendian=False):
    if value is None:
        ...
        if isinstance(value, int) or isinstance(value, _long):
            if littleendian:
