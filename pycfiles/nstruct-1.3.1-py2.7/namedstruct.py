...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/namedstruct/namedstruct.py
# Compiled at: 2018-10-22 23:39:06
"""
Created on 2015/7/8
...
import struct, logging, warnings
...
class NamedStruct(object):
    ...
    def _tostream(self, stream, skipprepack=False):
        ...
        if paddingSize > datasize:
            stream.write('\x00' * (paddingSize - datasize))
        return paddingSize
    ...
    def _subclass(self, parser):
        ...
        """
        _set(self, '_sub', parser._create(memoryview(getattr(self, '_extra', '')), self._target))
        try:
    ...
    def __getstate__(self):
        ...
        if t is not None and t in NamedStruct._pickleNames:
            ...
            return (
             self._tobytes(), self._parser, self._target)
            return
...
def _to_str(dumped_val, encoding='utf-8', ordered=True):
    ...
    if isinstance(dumped_val, dict):
        ...
        if isinstance(dumped_val, (list, tuple)):
            return [ _to_str(v, encoding) for v in dumped_val ]
        if isinstance(dumped_val, bytes):
            try:
...
class Parser(object):
    ...
    def __init__(self, base=None, criteria=_never, padding=8, initfunc=None, typedef=None, classifier=None, classifyby=None, prepackfunc=None):
        """
...
class FormatParser(Parser):
    ...
    def __init__(self, fmt, properties, sizefunc=None, prepackfunc=None, base=None, criteria=_never, padding=8, endian='>', initfunc=None, typedef=None, classifier=None, classifyby=None):
        ...
        self.emptydata = '\x00' * self.struct.size
        ...
        for p in self.properties:
            if len(p) > 1:
                if isinstance(result[start], bytes):
                    v = [ r.rstrip('\x00') for r in result[start:start + p[1]] ]
                else:
...
class SequencedParser(Parser):
    ...
    def _parseinner(self, buffer, namedstruct, copy=False, useall=True):
        ...
        if useall:
            size = len(buffer)
        elif self.sizefunc is not None:
            size = self.sizefunc(s)
            if size < start:
...
class PrimitiveParser(object):
    ...
    def parse(self, buffer, inlineparent=None):
        ...
        if len(buffer) < self.struct.size:
            ...
            try:
                return (
                 self.struct.unpack(buffer[:self.struct.size])[0], self.struct.size)
            except struct.error as exc:
                raise BadFormatError(exc)
...
class fixedstruct(typedef):
    ...
    def _compile(self):
        return FormatParser(self.format, self.properties, self.sizefunc, self.prepackfunc, None if self.base is None else self.base.parser(), self.criteria, self.padding, self.endian, self.initfunc, self.nstructtype, self.classifier, self.classifyby)
...
class nstruct(typedef):
    ...
    *extend* option defined in the base class. If the sub-classed type overrides a field again with *extend*
    ...
    def __init__(self, *members, **arguments):
        ...
        """
        params = [
         'size', 'prepack', 'base', 'criteria', 'endian', 'padding', 'lastextra', 'name', 'inline', 'init', 'classifier', 'classifyby', 'formatter', 'extend']
        for k in arguments:
            if k not in params:
        ...
        if self.base is not None:
            ...
            for m in members:
                ...
                if isinstance(t, str):
                    ...
                    t = nstruct(padding=1, endian=self.endian, *t)
        ...
        if lastinline_format:
            if not seqs:
                self.fixedstruct = fixedstruct(('').join(lastinline_format), lastinline_properties, self.sizefunc, self.prepackfunc, self.base, self.criteria, self.padding, self.endian, self.readablename, self.inlineself, self.initfunc, self, self.classifier, self.classifyby)
            ...
            self.fixedstruct = fixedstruct('', (), self.sizefunc, self.prepackfunc, self.base, self.criteria, self.padding, self.endian, self.readablename, self.inlineself, self.initfunc, self, self.classifier, self.classifyby)
            self._inline = self.fixedstruct.inline()
            self.lastextra = False
            self.propertynames = []
        else:
    ...
    def _compile(self):
        ...
        if hasattr(self, '_parser'):
            ...
            if hasattr(self, 'fixedstruct'):
                ...
                p = SequencedParser([ (t.parser(), name) for t, name in self.seqs ], self.sizefunc, self.prepackfunc, self.lastextra, None if self.base is None else self.base.parser(), self.criteria, self.padding, self.initfunc, self, self.classifier, self.classifyby)
            self._parser = p
...
class bitfield(typedef):
    ...
    *bitfield* are mini-struct with bit fields. It splits a integer primitive type like uint32
    ...
    def __init__(self, basetype, *properties, **arguments):
        ...
        """
        params = [
         'name', 'init', 'extend', 'formatter', 'prepack']
        for k in arguments:
            if k not in params:
                warnings.warn(StructDefWarning('Parameter %r is not recognized, is there a spelling error?' % (k,)))
