...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/infrae/plone/relations/schema/_relation.py
...
class ManyToManyRelationship(object):
    ...
    def __init__(self, context):
        ...
        self.util = getUtility(IComplexRelationshipContainer, name=self._name, context=context)
    ...
    def createRelationship(self, targets, sources=None, relation=None, interfaces=(), rel_factory=None, default_deletion=True):
    ...
    def getRelationshipChains(self, target=None, source=None, relation=_marker, state=_marker, context=_marker, rel_filter=None, maxDepth=1, minDepth=None, transitivity=None):
        self._initializeDirection(target, source)
        return self.util.findRelationships(self._sources, self._targets, relation, state, context, maxDepth=maxDepth, minDepth=minDepth, filter=rel_filter, transitivity=transitivity)

    def getRelationships(self, target=None, source=None, relation=_marker, state=_marker, context=_marker, rel_filter=None):
        rels = self.getRelationshipChains(target, source, relation, state, context, rel_filter)
        for chain in rels:
            yield chain[0]
    ...
    def getTargets(self, relation=_marker, state=_marker, context=_marker, rel_filter=None, maxDepth=1, minDepth=None, transitivity=None):
        return self._findTargetTokens(self.context, relation, state, context, maxDepth=maxDepth, minDepth=minDepth, filter=rel_filter, transitivity=transitivity)
    ...
    def getSources(self, relation=_marker, state=_marker, context=_marker, rel_filter=None, maxDepth=1, minDepth=None, transitivity=None):
        return self._findSourceTokens(self.context, relation, state, context, maxDepth=maxDepth, minDepth=minDepth, filter=rel_filter, transitivity=transitivity)

    def deleteRelationship(self, target=None, source=None, relation=_marker, state=_marker, context=_marker, rel_filter=None, multiple=False, remove_all_targets=False, ignore_missing=False, remove_all_sources=False):
        if target is None:
            remove_all_targets = True
        rels = self.getRelationships(target=target, source=source, relation=relation, state=state, context=context, rel_filter=rel_filter)
        rels = list(rels)
        ...
        for rel in rels:
            ...
            if len(rel.sources) > 1:
                if remove_all_sources or self._sources is None:
                    ...
                    raise pa_interfaces.TooManyResultsError, 'One of the relationships to be deleted has multiple sources and targets.'
