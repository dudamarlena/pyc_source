...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/spirol/impls/spirol_array_iter.py
# Compiled at: 2015-01-09 08:24:59
...
class spirol_array_iter(spirol_interface):
    ...
    def __init__(self, array, size, verbose=True, print_max_items=NUM_MAX_ITEMS_TO_PRINT, **kwargs):
        """
        ...
        self._print_max_items = int(print_max_items) if print_max_items else spirol_array_iter.NUM_MAX_ITEMS_TO_PRINT
        self._kwargs = kwargs
        kwargs['corner'] = SpirolCorner.validate(kwargs.get('corner', self.DEFAULT_CORNER))
        kwargs['direction'] = SpirolDirection.validate(kwargs.get('direction', self.DEFAULT_DIRECTION))

    def _scanners(self):
        _scanners = {SpirolDirection.CLOCKWISE: self.clockwise, 
    ...
    @property
    def clockwise(self):
        shifts = {SpirolCorner.TOP_LEFT: 0, SpirolCorner.TOP_RIGHT: 1, 
           SpirolCorner.BOTTOM_RIGHT: 2, 
           SpirolCorner.BOTTOM_LEFT: 3}
        return shift_left(['scan_right', 'scan_down', 'scan_left', 'scan_up'], shifts[self.corner])

    @property
    def counter_clock(self):
        shifts = {SpirolCorner.TOP_LEFT: 0, SpirolCorner.BOTTOM_LEFT: 1, 
           SpirolCorner.BOTTOM_RIGHT: 2, 
           SpirolCorner.TOP_RIGHT: 3}
        return shift_left(['scan_down', 'scan_right', 'scan_up', 'scan_left'], shifts[self.corner])
    ...
    def __iter__(self):
        ...
        _mapping = {SpirolDirection.CLOCKWISE: {'scan_right': [
                                                    scan_right, increments_right_cw_start, increments_right], 
                                       'scan_down': [
                                                   scan_down, increments_down_cw_start, increments_down], 
                                       'scan_left': [
                                                   scan_left, increments_left_cw_start, increments_left], 
                                       'scan_up': [
                                                 scan_up, increments_up_cw_start, increments_up]}, 
           SpirolDirection.COUNTER_CLOCK: {'scan_right': [
                                                        scan_right, increments_right_cc_start,
                                                        increments_right], 
                                           'scan_down': [
                                                       scan_down, increments_down_cc_start,
                                                       increments_down], 
                                           'scan_left': [
                                                       scan_left, increments_left_cc_start,
                                                       increments_left], 
                                           'scan_up': [
                                                     scan_up, increments_up_cc_start,
                                                     increments_up]}}
        ...
        try:
            while True:
                for i in scanners:
                    ...
                    try:
                        for result in m[0](row, col, row_end, col_end, row_start, col_start):
