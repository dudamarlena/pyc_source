...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/plists/v1writer.py
# Compiled at: 2015-03-22 02:40:32
import cStringIO
...
def writerFor(obj):
    if isinstance(obj, dict):
        return writeDict
    return {v1parser.Token: writeToken, dict: writeDict, 
       list: writeList, 
       str: writeString, 
       bool: writeBoolean}[type(obj)]
...
def writeBoolean(value, outstream=None, indentStr=None, level=0):
    outstream = outstream or cStringIO.StringIO()
    outstream.write('YES' if value else 'NO')
    return outstream
