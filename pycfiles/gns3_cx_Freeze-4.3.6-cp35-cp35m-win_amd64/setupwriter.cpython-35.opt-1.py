...
# [GCC 8.4.0]
# Embedded file name: \.\cx_Freeze\setupwriter.py
...
class SetupWriter(object):
    ...
    def GetBooleanValue(self, label, default=False):
        ...
        while 1:
            response = self.GetValue(label, defaultResponse, separator='? ').lower()
    ...
    def PopulateFromCommandLine(self):
        ...
        self.script = self.GetValue('Python file to make executable from', self.script)
        self.executableName = self.GetValue('Executable file name', self.defaultExecutableName)
        ...
        while 1:
            self.setupFileName = self.GetValue('Save setup script to', self.setupFileName)
