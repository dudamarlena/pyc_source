...
# [GCC 8.4.0]
# Embedded file name: \.\cx_Freeze\dist.py
# Compiled at: 2016-04-18 03:20:12
# Size of source mod 2**32: 13967 bytes
import distutils.command.bdist_rpm, distutils.command.build, distutils.command.install, distutils.core, distutils.dir_util, distutils.dist, distutils.errors, distutils.log, distutils.util, distutils.version, os, sys, cx_Freeze
from cx_Freeze.common import normalize_to_list
__all__ = [
 'bdist_rpm', 'build', 'build_exe', 'install', 'install_exe',
 'setup']
...
class build_exe(distutils.core.Command):
    ...
    user_options = [
     ...
     ('namespace-packages=', None, 'comma-separated list of namespace packages to include'),
     ('replace-paths=', None, 'comma-separated list of paths to replace in included modules'),
     ('path=', None, 'comma-separated list of paths to search'),
     ('init-script=', 'i', 'name of script to use during initialization'),
     ('base=', None, 'name of base executable to use'),
     ('compressed', 'c', 'create a compressed zipfile'),
     ('copy-dependent-files', None, 'copy all dependent files'),
     ('create-shared-zip', None, 'create a shared zip file containing shared modules'),
     ('append-script-to-exe', None, 'append the script module to the exe'),
     ('include-in-shared-zip', None, 'include the script module in the shared zip file'),
     ('icon', None, 'include the icon along with the frozen executable(s)'),
     ('constants=', None, 'comma-separated list of constants to include'),
     ('include-files=', 'f', 'list of tuples of additional files to include in distribution'),
     ('include-msvcr=', None, 'include the Microsoft Visual C runtime files'),
     ('zip-includes=', None, 'list of tuples of additional files to include in zip file'),
     ('bin-includes', None, 'list of names of files to include when determining dependencies'),
    ...
    boolean_options = [
     'compressed', 'copy_dependent_files',
    ...
    def build_extension(self, name, moduleName=None):
        ...
        return os.path.join(sourceDir, command.build_lib, command.get_ext_filename(moduleName))
    ...
    def run(self):
        ...
        freezer = cx_Freeze.Freezer(self.distribution.executables, [
         constantsModule], self.includes, self.excludes, self.packages, self.replace_paths, self.compressed, self.optimize, self.copy_dependent_files, self.init_script, self.base, self.path, self.create_shared_zip, self.append_script_to_exe, self.include_in_shared_zip, self.build_exe, icon=self.icon, includeMSVCR=self.include_msvcr, includeFiles=self.include_files, binIncludes=self.bin_includes, binExcludes=self.bin_excludes, zipIncludes=self.zip_includes, silent=self.silent, namespacePackages=self.namespace_packages, binPathIncludes=self.bin_path_includes, binPathExcludes=self.bin_path_excludes, metadata=metadata)
        freezer.Freeze()
...
class install_exe(distutils.core.Command):
    ...
    def finalize_options(self):
        ...
        self.set_undefined_options('install', ('install_exe', 'install_dir'), ('force',
                                                                               'force'), ('skip_build',
                                                                                          'skip_build'))
