...
# [GCC 8.4.0]
# Embedded file name: \.\cx_Freeze\finder.py
# Compiled at: 2016-04-18 03:20:12
# Size of source mod 2**32: 32554 bytes
"""
Base class for finding modules.
"""
import dis, imp, marshal, opcode, os, pkgutil, re, sys, types, zipfile, cx_Freeze.hooks
...
try:
    ...
    _identifier_re = re.compile('^[a-z_]\\w*$', re.I)
...
class ZipModulesCache(object):
    __doc__ = 'A cache of module and package locations within zip files.'
    ...
    def record_loadable_module(self, nameparts, actual_path, zip, ispkg=False):
        ...
        if (parent_dir, modulename) not in self.loadable_modules:
            self.loadable_modules[(parent_dir, modulename)] = (
             zip, ideal_path, actual_path, ispkg)


class ModuleFinder(object):

    def __init__(self, includeFiles=None, excludes=[], path=None, replacePaths=None, copyDependentFiles=True, bootstrap=False, compress=True):
    ...
    def _EnsureFromList(self, caller, packageModule, fromList, deferredImports):
        ...
        for path in module.path:
            ...
            for fileName in fileNames:
                ...
                if os.path.isdir(fullName):
                    ...
                    subModule = self._InternalImportModule(subModuleName, deferredImports)
                    if subModule is None:
                        raise ImportError('No module named %r' % subModuleName)
                module.globalNames[name] = None
                if subModule.path and recursive:
                    self._ImportAllSubModules(subModule, deferredImports, recursive)
        ...
        while deferredImports:
            ...
            for caller, packageModule, subModuleNames in deferredImports:
                if packageModule.inImport and skipInImport:
                    ...
                    self._EnsureFromList(caller, packageModule, subModuleNames, newDeferredImports)

            deferredImports = newDeferredImports

    def _ImportModule(self, name, deferredImports, caller=None, relativeImportIndex=0, namespace=False):
        """Attempt to find the named module and return it or None if no module
           by that name could be found."""
        if relativeImportIndex == 0:
            module = self._InternalImportModule(name, deferredImports, namespace=namespace)
        else:
            if relativeImportIndex < 0:
                parent = self._DetermineParent(caller)
                if parent is not None:
                    fullName = '%s.%s' % (parent.name, name)
                    module = self._InternalImportModule(fullName, deferredImports, namespace=namespace)
    ...
    def _InternalImportModule(self, name, deferredImports, namespace=False):
        ...
        if pos < 0:
            ...
            parentName = name[:pos]
            parentModule = self._InternalImportModule(parentName, deferredImports, namespace=namespace)
    ...
    def _LoadModule(self, name, fp, path, info, deferredImports, parent=None, namespace=False):
        """Load the module, given the information acquired by the finder."""
        suffix, mode, type = info
        if type == imp.PKG_DIRECTORY:
            return self._LoadPackage(name, path, parent, deferredImports, namespace)
        module = self._AddModule(name)
        module.file = path
        ...
        if type == imp.PY_SOURCE:
            ...
            if module.code is not None:
                if self.replacePaths:
                    ...
                    module.code = self._ReplacePathsInCode(topLevelModule, module.code)
    ...
    def _ReplacePathsInCode(self, topLevelModule, co):
        ...
        if sys.version_info[0] < 3:
            return types.CodeType(co.co_argcount, co.co_nlocals, co.co_stacksize, co.co_flags, co.co_code, tuple(constants), co.co_names, co.co_varnames, newFileName, co.co_name, co.co_firstlineno, co.co_lnotab, co.co_freevars, co.co_cellvars)
        return types.CodeType(co.co_argcount, co.co_kwonlyargcount, co.co_nlocals, co.co_stacksize, co.co_flags, co.co_code, tuple(constants), co.co_names, co.co_varnames, newFileName, co.co_name, co.co_firstlineno, co.co_lnotab, co.co_freevars, co.co_cellvars)
        ...
        while opIndex < numOps:
            ...
            if op == LOAD_CONST:
                ...
                if name not in module.excludeNames:
                    ...
                    if importedModule is not None and fromList and fromList != ('*', ) and importedModule.path is not None:
                        self._EnsureFromList(module, importedModule, fromList, deferredImports)
            if op == IMPORT_FROM and topLevel:
                if is3:
                    op = code[opIndex]
                ...
                if op == IMPORT_STAR and topLevel and importedModule is not None:
    ...
    def IncludeFile(self, path, moduleName=None):
        ...
        info = (
         ext, 'r', imp.PY_SOURCE)
        deferredImports = []
        module = self._LoadModule(moduleName, open(path, 'U'), path, info, deferredImports)
    ...
    def IncludeModule(self, name, namespace=False):
        ...
        deferredImports = []
        module = self._ImportModule(name, deferredImports, namespace=namespace)
    ...
    def ReportMissingModules(self):
        ...
        if self._badModules:
            ...
            sys.stdout.write('This is not necessarily a problem - the modules may not be needed on this platform.\n')
            sys.stdout.write('\n')
