...
# Size of source mod 2**32: 32690 bytes
"""
Base class for freezing scripts into executables.
"""
import datetime, distutils.sysconfig, imp, marshal, os, shutil, socket, stat, struct, sys, time, zipfile, cx_Freeze
__all__ = [
...
class Freezer(object):
    ...
    def __init__(self, executables, constantsModules=[], includes=[], excludes=[], packages=[], replacePaths=[], compress=None, optimizeFlag=0, copyDependentFiles=None, initScript=None, base=None, path=None, createLibraryZip=None, appendScriptToExe=None, appendScriptToLibrary=None, targetDir=None, binIncludes=[], binExcludes=[], binPathIncludes=[], binPathExcludes=[], icon=None, includeFiles=[], zipIncludes=[], silent=False, namespacePackages=[], metadata=None, includeMSVCR=False):
    ...
    def _AddVersionResource(self, fileName):
        ...
        versionInfo = VersionInfo(self.metadata.version, comments=self.metadata.long_description, description=self.metadata.description, company=self.metadata.author, product=self.metadata.name)
        stamp(fileName, versionInfo)

    def _CopyFile(self, source, target, copyDependentFiles=False, includeMode=False):
    ...
    def _FreezeExecutable(self, exe):
        ...
        self._CopyFile(exe.base, exe.targetName, exe.copyDependentFiles, includeMode=True)
        ...
        if not exe.appendScriptToLibrary:
            ...
            if not self.createLibraryZip and exe.copyDependentFiles:
                scriptModule = None
            self._WriteModules(fileName, exe.initScript, finder, exe.compress, exe.copyDependentFiles, scriptModule)
    ...
    def _GetDefaultBinIncludes(self):
        ...
        if sys.platform == 'win32':
            ...
            return [
             pythonDll, 'gdiplus.dll', 'mfc71.dll', 'msvcp71.dll',
             'msvcr71.dll']
        else:
    ...
    def _GetDependentFiles(self, path):
        ...
        if dependentFiles is None:
            ...
            dependentFiles = self.dependentFiles[path] = [f for f in dependentFiles if self._ShouldCopyFile(f)]
    ...
    def _GetModuleFinder(self, argsSource=None):
        if argsSource is None:
            argsSource = self
        finder = cx_Freeze.ModuleFinder(self.includeFiles, argsSource.excludes, argsSource.path, argsSource.replacePaths, argsSource.copyDependentFiles, compress=argsSource.compress)
    ...
    def _IncludeMSVCR(self, exe):
        ...
        if msvcRuntimeDll is not None and msvcRuntimeDll == 'msvcr90.dll':
            if struct.calcsize('P') == 4:
                ...
                arch = 'amd64'
            manifest = MSVCR_MANIFEST_TEMPLATE.strip().replace('{PROC_ARCH}', arch)
    ...
    def _VerifyCanAppendToLibrary(self):
        if not self.createLibraryZip:
            raise ConfigError('script cannot be appended to library zip if one is not being created')
    ...
    def _VerifyConfiguration(self):
        ...
        if self.appendScriptToLibrary is None:
            self.appendScriptToLibrary = self.createLibraryZip and not self.appendScriptToExe
    ...
    def _WriteModules(self, fileName, initScript, finder, compress, copyDependentFiles, scriptModule=None):
        ...
        for module in modules:
            if module.code is None and module.file is not None:
                ...
                if baseFileName != module.name and module.name != 'zlib':
                    ...
                    generatedFileName = 'ExtensionLoader_%s.py' % module.name.replace('.', '_')
                    module.code = compile(EXTENSION_LOADER_SOURCE % fileName, generatedFileName, 'exec')
    ...
    def Freeze(self):
        ...
        if self.createLibraryZip:
            ...
            self._RemoveFile(fileName)
            self._WriteModules(fileName, self.initScript, self.finder, self.compress, self.copyDependentFiles)
        for sourceFileName, targetFileName in self.includeFiles:
            if os.path.isdir(sourceFileName):
                for path, dirNames, fileNames in os.walk(sourceFileName):
                    ...
                    if 'CVS' in dirNames:
                        dirNames.remove('CVS')
                    fullTargetDir = os.path.join(self.targetDir, targetFileName, shortPath)
                    ...
                    for fileName in fileNames:
                        ...
                        self._CopyFile(fullSourceName, fullTargetName, copyDependentFiles=False)

            else:
                fullName = os.path.join(self.targetDir, targetFileName)
                self._CopyFile(sourceFileName, fullName, copyDependentFiles=False)
...
class Executable(object):
    ...
    def __init__(self, script, initScript=None, base=None, path=None, targetDir=None, targetName=None, includes=None, excludes=None, packages=None, replacePaths=None, compress=None, copyDependentFiles=None, appendScriptToExe=None, appendScriptToLibrary=None, icon=None, namespacePackages=None, shortcutName=None, shortcutDir=None):
        self.script = script
        self.initScript = initScript
        self.base = base
        self.path = path
...
class ConstantsModule(object):
    ...
    def __init__(self, releaseString=None, copyright=None, moduleName='BUILD_CONSTANTS', timeFormat='%B %d, %Y %H:%M:%S'):
