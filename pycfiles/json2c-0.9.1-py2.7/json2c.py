...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\json2c.py
# Compiled at: 2014-07-07 15:43:15
import re, os, sys, math, json, argparse, collections
    ...
    if isinstance(obj, type(None)):
        ...
        if isinstance(obj, dict):
            ...
            for k, v in obj.items():
                ...
                if k in ('auto', 'break', 'case', 'char', 'const', 'continue', 'default',
            ...
            return (
             collections.OrderedDict(zip(obj.keys(), vals)), CStruct(types=tuple(typs), fields=tuple(obj.keys())))
        if isinstance(obj, list):
            obj, typs = zip(*[ obj2cjson(x) for x in obj ])
            return (
             obj, CList(types=tuple(typs)))
        return


def cjson_unify(cjson, **kw):
    obj, typ = cjson
    if isinstance(typ, CNull):
        return (obj, typ)
    if isinstance(typ, CInt):
        return (obj, typ)
    if isinstance(typ, CFloat):
        return (obj, typ)
    if isinstance(typ, CString):
        return (obj, typ)
    if isinstance(typ, CBool):
        return (obj, typ)
    ...
    if len(set(typs)) == 1:
        ...
        if not kw.get('no_null_to_ptr', False):
            if all([ isinstance(t, CPointer) for t in typs ]):
                objs2, typ = cjson_unify_list([ o[0] for o in objs if o != None ], [ t.type for t in typs if not isinstance(t.type, CNull) ], **kw)
                n = 0
        ...
        if not kw.get('no_null_to_obj', False):
            if all([ isinstance(t, CNull) or isinstance(t, CStruct) for t in typs ]) and any([ isinstance(t, CNull) for t in typs ]):
        ...
        if not kw.get('no_coerce_missing', False):
            if all([ isinstance(t, CStruct) for t in typs ]):
                if len(set([ t.fields for t in typs ])) > 1:
                    ...
                    objs = [ collections.OrderedDict([ (k, o.get(k, None)) for k in fields ]) for o in objs
                           ]
                    typs = [ CStruct(types=tuple([ t.types[t.fields.index(k)] if k in t.fields else CNull() for k in fields ]), fields=fields) for t in typs
                           ]
                    return cjson_unify_list(objs, typs, **kw)
        if not kw.get('no_coerce_matching', False):
            if all([ isinstance(t, CStruct) for t in typs ]) and len(set([ tuple(t.fields) for t in typs ])) == 1:
                types = []
                ...
                return (
                 objs, CStruct(types=tuple(types), fields=tuple(objs[0].keys())))
        ...
        if not kw.get('no_null_to_int', False):
            if all([ isinstance(t, CNull) or isinstance(t, CInt) for t in typs ]) and all([ o >= 0 for o in objs if o != None ]):
        ...
        if not kw.get('no_array_to_ptr', False):
            if all([ isinstance(t, CArray) for t in typs ]) and len(set([ t.length for t in typs ])) > 1:
...
def cjson_repr(typ, obj, name, typenames=set(), typedefs=[], idnt='', **kw):
    ...
    if isinstance(typ, CNull):
        name = kw.get('null_type', 'void*')
    if isinstance(typ, CInt):
        name = kw.get('int_type', 'long')
    if isinstance(typ, CFloat):
        name = kw.get('float_type', 'double')
    if isinstance(typ, CBool):
        name = kw.get('bool_type', 'bool')
    if isinstance(typ, CString):
        name = kw.get('string_type', 'const char*')
    if isinstance(typ, CPointer):
    ...
    if isinstance(typ, CStruct):
        ...
        for t, f in zip(typ.types, typ.fields):
            subtype, subdecl, typenames, typedefs = cjson_repr(t, NoObject, (name + joiner + (f[0].upper() + f[1:] if kw.get('field_capitalize', False) else f)), typenames, typedefs, **kw)
            subtypes.append(subtype)

        if name not in typenames:
            typenames.add(name)
            typedefs.append('typedef struct {\n' + ('').join([ idnt_by + t + ' ' + f + ';\n' for t, f in zip(subtypes, typ.fields) ]) + '} ' + name + ';')
    if obj is NoObject:
        ...
        if isinstance(typ, CInt):
            decl = idnt + repr(obj)
        if isinstance(typ, CFloat):
            decl = idnt + (kw.get('nan_value', 'NAN') if math.isnan(obj) else repr(obj))
        if isinstance(typ, CBool):
            decl = idnt + (kw.get('true_value', 'true') if obj else kw.get('false_value', 'false'))
        if isinstance(typ, CString):
            if obj is None:
                decl = idnt + kw.get('null_value', 'NULL')
            else:
        ...
        if isinstance(typ, CStruct):
            ...
            if kw.get('no_designated', False):
                decl = idnt + '{\n' + (', \n').join([ cjson_repr(t, o, (name + joiner + (f[0].upper() + f[1:] if kw.get('field_capitalize', False) else f)), typenames, typedefs, (idnt + idnt_by), **kw)[1] for o, t, f in zip(obj.values(), typ.types, typ.fields) ]) + '\n' + idnt + '}'
            else:
                decl = idnt + '{\n' + (', \n').join([ idnt + idnt_by + '.' + f + ' = ' + cjson_repr(t, o, (name + joiner + (f[0].upper() + f[1:] if kw.get('field_capitalize', False) else f)), typenames, typedefs, (idnt + idnt_by), **kw)[1].strip() for o, t, f in zip(obj.values(), typ.types, typ.fields) ]) + '\n' + idnt + '}'
...
def json2c_file(filename, **kw):
    ...
    if 'variable' not in kw:
        kw['variable'] = typename + kw.get('field_infix', '_') + ('Object' if kw.get('field_capitalize', False) else 'object')
    ...
    if not kw.get('no_math', False):
        header = '#include <math.h>\n' + header
    if not kw.get('no_stdbool', False):
        header = '#include <stdbool.h>\n' + header
    if not kw.get('no_stdlib', False):
        header = '#include <stdlib.h>\n' + header
    if not kw.get('no_guard', False):
        headerguard = os.path.splitext(os.path.split(headerfile)[1])[0] + '_h'
        if not kw.get('no_guard_uppercase', False):
...
def main():
    ...
    json2c_file(**{k:v for k, v in parser.parse_args().__dict__.items() if v != None if v != None})
