...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/zaggregator/utils.py
# Compiled at: 2018-06-14 17:33:03
# Size of source mod 2**32: 3965 bytes
from difflib import SequenceMatcher
from fuzzywuzzy import fuzz, StringMatcher
import os, sys, logging as log, psutil, json, zaggregator
...
def parent_has_single_child(proc) -> bool:
    ...
    """
    if not proc or not proc.parent:
        return False
        try:
            parent = proc.parent()
            if not parent:
...
def is_kernel_thread(proc) -> bool:
    ...
    if isinstance(proc, psutil.Process):
        if os.getpgid(proc.pid) == 0:
            return True
    elif isinstance(proc, zaggregator.ProcessMirror) and proc._pgid == 0:
        return True
    return False
...
def is_proc_similar_to(proc1, proc2) -> bool:
    ...
    """
    if fuzzy_match(proc1.cmdline(), proc2.cmdline(), threshold=90) and proc1.cwd() == proc2.cwd():
        return True
    return False
