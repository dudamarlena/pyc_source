...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/sella/optimize/stepper.py
# Compiled at: 2020-01-27 19:11:41
...
class RationalFunctionOptimization(BaseStepper):
    ...
    def get_s(self, alpha):
        ...
        dVda = V1 @ (V1.T @ dAda @ V[:, self.order] / (L1 - L[self.order]))
        dsda = V[:-1, self.order] / V[(-1, self.order)] + alpha / V[(-1, self.order)] * dVda[:-1] - V[:-1, self.order] * alpha / V[(-1, self.order)] ** 2 * dVda[(-1)]
        return (s, dsda)


class PartitionedRationalFunctionOptimization(RationalFunctionOptimization):
    synonyms = [
     'prfo', 'p-rfo', 'partitioned rational function optimization']

    def _stepper_init(self):
        self.Vmax = self.H.evecs[:, :self.order]
        self.Vmin = self.H.evecs[:, self.order:]
        self.max = RationalFunctionOptimization((self.Vmax.T @ self.g), (self.H.project(self.Vmax)),
          order=(self.Vmax.shape[1]))
        self.min = RationalFunctionOptimization((self.Vmin.T @ self.g), (self.H.project(self.Vmin)),
          order=0)
...
_all_steppers = [
 QuasiNewton, RationalFunctionOptimization,
