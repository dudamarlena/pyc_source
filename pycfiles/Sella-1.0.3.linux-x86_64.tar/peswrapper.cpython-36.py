...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/sella/peswrapper.py
# Compiled at: 2020-01-27 19:12:50
# Size of source mod 2**32: 15685 bytes
import numpy as np
...
class PES:
    ...
    def __init__(self, atoms, H0=None, constraints=None, eigensolver='jd0', trajectory=None, eta=0.0001, v0=None, proj_trans=True, proj_rot=True):
        self.atoms = atoms
    ...
    def set_constraints(self, c):
        ...
        self.cons = get_constraints((self.atoms), (self.con_user),
          (self.target_user),
    ...
    def get_scons(self):
        ...
        scons = -Ucons @ np.linalg.lstsq((self.get_drdx() @ Ucons), (self.get_res()),
          rcond=None)[0]
        return scons
    ...
    def _update(self, feval=True):
        ...
        if new_point:
            self.last = self.curr
            self.curr = dict(x=x, f=f, g=g, drdx=drdx, Ucons=Ucons, Unred=Unred,
              Ufree=Ufree,
              L=L)
            self._update_H()
        else:
    ...
    def diag(self, gamma=0.5, threepoint=False, maxiter=None):
        ...
        if P.B is None:
            ...
            P = P.asarray()
        Hproj = NumericalHessian(self._calc_eg, self.get_x(), self.get_g(), self.eta, threepoint, Unred)
        Hc = self.get_Hc()
        rayleigh_ritz((Hproj - Unred.T @ Hc @ Unred), gamma, P, v0=v0, method=(self.eigensolver),
          maxiter=maxiter)
        Vs = Hproj.Vs
        AVs = Hproj.AVs
    ...
    def converged(self, fmax, cmax=1e-05):
...
class InternalPES(PES):
    ...
    def __init__(self, atoms, *args, H0=None, angles=True, dihedrals=True, extra_bonds=None, **kwargs):
        (PES.__init__)(self, atoms, *args, H0=None, **kwargs)
        self.int, self.cons, self.dummies = get_internal(atoms, self.con_user, self.target_user)
    ...
    def update_internals(self, dx):
        self._update(True)
        out = get_internal((self.atoms), (self.con_user), (self.target_user), dummies=(self.dummies),
          conslast=(self.cons))
        ...
        self.curr.update(x=x, g=g, drdx=drdx, Ufree=Ufree, Unred=Unred,
          Ucons=Ucons,
          L=L,
          B=B,
          Binv=Binv)
