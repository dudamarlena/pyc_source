...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/sella/internal/int_find2.py
...
def get_dummy(atoms, nbonds, c10y, i, j, k, atol):
    ...
    for n in [i, k]:
        for m in sorted((c10y[n, :nbonds[n]]), key=(lambda x: atoms.get_distance(x, n))):
            if m in [i, j, k]:
                pass
            else:
...
def find_angles(atoms, atol, bonds, nbonds, c10y, dummies=None, dinds=None):
    ...
    for i, j in bond_constraints:
        ...
        assert abs(1 - rij) < 1e-05, rij

    for i, j, k in angle_constraints:
        aijk = allatoms.get_angle(i, j, k)
        assert abs(90 - aijk) < 1e-05, aijk

    for i, j, k, l in dihedral_constraints:
        dijkl = allatoms.get_dihedral(i, j, k, l)
        assert abs(180 - dijkl) < 1e-05, dijkl

    return (
     np.array(angles, dtype=(np.int32)).reshape((-1, 3)),
     dummies,
     dinds,
