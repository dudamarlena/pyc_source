...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/sella/aseopt.py
...
_default_kwargs = dict(minimum=dict(delta0=0.1, sigma_inc=1.15,
  sigma_dec=0.9,
  rho_inc=1.035,
  rho_dec=100,
  method='rsrfo',
  eig=False),
  saddle=dict(delta0=0.0013, sigma_inc=1.15,
  sigma_dec=0.65,
  rho_inc=1.035,
  rho_dec=5.0,
  method='rsprfo',
...
class Sella(Optimizer):
    ...
    def __init__(self, atoms, restart=None, logfile='-', trajectory=None, master=None, force_consistent=False, delta0=None, sigma_inc=None, sigma_dec=None, rho_dec=None, rho_inc=None, order=1, eig=None, eta=0.0001, peskwargs=None, method=None, gamma=0.4, constraints_tol=1e-05, H0=None, append_trajectory=None, **kwargs):
        if order == 0:
        ...
        if trajectory is not None:
            ...
            if isinstance(atoms, PESWrapper):
                ...
                asetraj = None
                self.pes = PESWrapper(atoms, atoms.calc, trajectory=trajectory, **kwargs)
            Optimizer.__init__(self, atoms, restart, logfile, asetraj, master, force_consistent)
            if delta0 is None:
    ...
    def step(self):
        if not self.initialized:
            ...
            if self.method == 'gmtrm':
                s, smag, self.xi, bound_clip = rs_newton(self.pes, self.glast, self.delta, self.ord, self.xi)
            else:
                if self.method == 'rsrfo':
                    s, smag, self.xi, bound_clip = rs_rfo(self.pes, self.glast, self.delta, self.ord, self.xi)
                else:
                    if self.method == 'rsprfo':
                        s, smag, self.xi, bound_clip = rs_prfo(self.pes, self.glast, self.delta, self.ord, self.xi)
                    else:
                        raise RuntimeError("Don't know what to do for method", self.method)
                ev = self.eig and self.pes.lams is not None and np.any(self.pes.lams[:self.ord] > 0)
    ...
    def converged(self, forces=None):
        ...
        return np.all(np.abs(self.pes.res) < self.constraints_tol) and (forces ** 2).sum(1).max() < self.fmax ** 2
