...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/sella/constraints.py
# Compiled at: 2020-01-27 12:54:08
# Size of source mod 2**32: 8924 bytes
from collections import Sequence
import numpy as np
from ase.constraints import FixCartesian, FixAtoms, FixBondLengths, FixInternals
from sella.internal.int_classes import Constraints
_con_kinds = [
 'cart', 'bonds', 'angles', 'dihedrals',
    ...
    if not isinstance(arg, Sequence):
        ...
        if n == 2:
            if not isinstance(arg[0], Sequence) or len(arg[0]) != 3:
                raise ValueError('Invalid angle constraint:', arg)
            _add_check(arg[0], con)
            target.append(arg[1] * np.pi / 180.0)
        else:
            if n == 3:
                _add_check(arg, con)
                target.append((atoms.get_angle)(*arg) * np.pi / 180.0)
            else:
                raise ValueError('Invalid angle constraint:', arg)
...
def _add_dihedral(atoms, arg, con, target):
    if not isinstance(arg, Sequence):
        ...
        if n == 2:
            ...
            if n == 4:
                _add_check(arg, con)
                target.append((atoms.get_dihedral)(*arg) * np.pi / 180.0)
            else:
                raise ValueError('Invalid dihedral constraint:', arg)
    ...
    if not isinstance(arg, Sequence):
        ...
        if n == 2:
            ...
            if n == 4:
                ...
                target.append((a1 + a2) * np.pi / 180.0)
            else:
                raise ValueError('Invalid angle sum constraint:', arg)
...
_con_adders = dict(cart=_add_cart, bonds=_add_bond, angles=_add_angle, dihedrals=_add_dihedral,
  angle_sums=_add_angle_sum,
...
def merge_internal_constraints(con_user, target_user, bcons, acons, dcons, adiffs):
    con_out = con_user.copy()
    target_out = target_user.copy()
    for arg in bcons:
        con_out['bonds'].append(_sort_indices(arg))
        target_out['bonds'].append(1.0)

    for arg in acons:
        con_out['angles'].append(_sort_indices(arg))
        target_out['angles'].append(np.pi / 2.0)
    ...
    for arg in adiffs:
        ...
        target_out['angle_diffs'].append(0.0)
...
def get_constraints(atoms, con, target, dummies=None, dinds=None, proj_trans=True, proj_rot=True, **kwargs):
    target_all = []
 ...
 target_all, dummies=dummies, 
     dinds=dinds, 
     proj_trans=proj_trans, 
     proj_rot=proj_rot, **con_arrays)
        ...
        if isinstance(constraint, FixAtoms):
            ...
            if isinstance(constraint, FixBondLengths):
                ...
                if isinstance(constraint, FixInternals):
                    ...
                    raise ValueError('Sella does not know how to handle the ASE {} constraint object!'.format(constraint))
