...
# [GCC 8.4.0]
# Embedded file name: /Users/mh12/sanger-pathogens/Farm_blast/farm_blast/tests/pipeline_test.py
# Compiled at: 2015-02-02 06:21:17
# Size of source mod 2**32: 7147 bytes
import sys, os, filecmp, subprocess, shutil, unittest
...
class TestPipeline(unittest.TestCase):
    ...
    def test_make_setup_script_ref_already_indexed(self):
        ...
        print('set -e', file=f)
        print('fastaq to_fasta -s', os.path.abspath(self.qry), '- |', 'fastaq chunker --skip_all_Ns - query.split 100 1', file=f)
        f.close()
    ...
    def test_make_setup_script_with_union_for_act(self):
        ...
        f = open(expected_script, 'w')
        print('set -e', file=f)
        print('fastaq merge', os.path.abspath(self.ref), '- |', 'fastaq to_fasta -s - reference.fa', file=f)
        print('makeblastdb -dbtype nucl -in reference.fa', file=f)
        print('fastaq merge', os.path.abspath(self.qry), '- |', 'fastaq to_fasta -s - - |', 'fastaq chunker --skip_all_Ns - query.split 100 1', file=f)
        f.close()
        self.assertTrue(filecmp.cmp(expected_script, test_script))
        os.unlink(expected_script)
        os.unlink(test_script)
    ...
    def test_make_setup_job(self):
        ...
        expected = '-R "select[mem>1000] rusage[mem=1000]" -M1000 -o 01.setup.sh.o -e 01.setup.sh.e -J name.setup bash 01.setup.sh'
        self.assertTrue(str(self.p.setup_job).endswith(expected))

    def test_make_start_array_job(self):
        self.p._make_start_array_job()
        expected = '-o 02.run_array.sh.o -e 02.run_array.sh.e -J name.start_array bash 02.run_array.sh'
        self.assertTrue(str(self.p.start_array_job).endswith(expected))

    def test_make_array_job(self):
        self.p._make_array_job()
        expected = '-R "select[mem>500] rusage[mem=500]" -M500 -o tmp.array.o.%I -e tmp.array.e.%I -J "name.array[1-$n]%100" blastn -task blastn -db ' + self.ref + ' -query query.split.\\$LSB_JOBINDEX -out tmp.array.out.\\$LSB_JOBINDEX -outfmt 6'
    ...
    def test_make_combine_job(self):
        ...
        expected = '-n 2 -R "span[hosts=1] select[mem>500] rusage[mem=500]" -M500 -o 03.combine.sh.o -e 03.combine.sh.e -J name.combine bash 03.combine.sh'
