...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/dame/executor.py
# Compiled at: 2020-05-04 18:35:31
# Size of source mod 2**32: 8759 bytes
import platform, shutil, subprocess, uuid
from pathlib import Path
import docker, validators
...
def get_singularity():
    try:
        ...
        if SINGULARITY_LOCAL_BIN.exists():
...
def build_parameter(parameters):
    ...
    for _parameter in parameters:
        ...
        elif 'has_fixed_resource' in _parameter:
...
def build_command_line(resource, _dir):
    ...
    if parameters is not None:
        ...
        line += ' {}'.format(l)
    return (
     has_software_image, line, src_path)
...
def prepare_execution(setup_path):
    ...
    return (
     src_path, execution_dir, setup_cmd_line, setup_name, image)
...
def run_docker(component_cmd, execution_dir, component_dir, setup_name, image, volumes):
    ...
    res = client.containers.run(command=component_cmd, image=image,
      volumes=volumes,
