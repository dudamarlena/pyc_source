...
# [GCC 8.4.0]
# Embedded file name: /home/bill/py3/.virtualenvs/pytest/lib/python2.7/site-packages/Csys/MailInternet.py
# Compiled at: 2009-11-24 21:17:20
"""Internet mail utilities
...
_reHeaderBody = re.compile('(.*?\\n\\n)(.*)', re.DOTALL)
_typeRE = type(_reHeaderBody)
_receivedPatterns = (
 re.compile('^from\\s+(?P<helo>\\S+)\\s*\\((?P<dnsname>\\S*)\\s*\\[(?P<ip>[^]]*)\\]\\).*\\sby\\s(?P<by>\\S+).*^\\s*id[^;]*;\\s*(?P<date>.*)$', re.MULTILINE | re.DOTALL),
 re.compile('^from\\s*(?P<helo>\\S+)\\s*\\((?P<dnsname>\\S*)\\s*\\[(?P<ip>[^]]*)\\]\\).*\\sby\\s(?P<by>\\S+).*^\\s*for[^;]*;\\s*(?P<date>.*)$', re.MULTILINE | re.DOTALL),
 re.compile('^from\\s*(?P<helo>[-\\.\\d]+)\\s*\\(\\[(?P<ip>[^]]*)\\]\\).*^\\s*for[^;]*;\\s*(?P<date>.*)$', re.MULTILINE | re.DOTALL),
 re.compile('\\[(?P<ip>[^\\]]*)\\].*\\sby\\s(?P<by>\\S+).*^\\s*for[^;]*;\\s*(?P<date>.*)$', re.MULTILINE | re.DOTALL))
_dateRE = re.compile('^(\\w+,\\s+\\d{1,2}\\s+\\w+\\s+)(\\d{1,3})(\\s+\\d{1,2}:\\d{1,2}:\\d{1,2}.*)$')

class Received(Csys.CSClass):
    __doc__ = Csys.detab('\n\tClass to extract relevant info from Received: headers\n\t')
    _attributes = {'helo': None, 
       'dnsname': None, 
       'ip': None, 
       'date': None, 
       'by': None, 
    ...
    def __init__(self, received):
        ...
        if received.startswith('Received:'):
            received = received[8:]
        if _verbose:
...
class Message(Csys.CSClass):
    _attributes = {'msg': None, 
       'msgstr': None, 
       'headerstr': None, 
       'bodystr': None, 
       'sender': None, 
       '_HdrDict': {}, 'sa_spam': False, 
       'sa_score': 0, 
       'sa_required': 0, 
       'sa_symbols': '', 
       'sa_version': '', 
    ...
    def __init__(self, *args):
        ...
        if self.sender is None:
            self.sender = self._getSender()
        return

    def body(self):
        return self.bodystr.split('\n')

    def print_header(self, fh=sys.stdout):
        fh.write(self.headerstr)

    def print_body(self, fh=sys.stdout):
        fh.write(self.bodystr)

    def print_msg(self, fh=sys.stdout):
        fh.write(self.msg.as_string())
    ...
    def __str__(self):
        return self.msg.as_string(False)
    ...
    def checkSpamAssassin(self, user, host='localhost', port=SPAMD_PORT, subjectprefix='***SPAM***'):
        ...
        self.rmSAHeaders()
        self.sa_spam, self.sa_score, self.sa_required = is_spam, score, required
        ...
        if score > 0:
            self.replace('X-Spam-Level', '*' * int(score))
        self.replace('X-Spam-Status', '%s, score=%.2f required=%.2f tests=%s\n\twith spamd' % (
    ...
    def rmSAHeaders(self):
        ...
        for pattern in self._saCommentPatterns:
            try:
                newsubject = pattern.sub('', subject)
            except:
                continue
        ...
        if whl or mxips:
            ...
            for received in received_hdrs:
                ...
                if received.ip:
                    try:
                        ipaddr = received.ip
                        if not (ipaddr.startswith('127.') or isPrivate(ipaddr) or ipaddr in mxips or whl and inrbl(ipaddr, whl)):
                            return received
                    except:
                        pass
...
class PostfixConfig(Csys.CSClass):
    ...
    _varPattern = re.compile('\\$([{}0-9a-z_]*)', re.IGNORECASE)
...
class MBOX(Csys.CSClass):
    ...
    def __init__(self, fname):
        ...
        if hasattr(fname, 'readline'):
            ...
            if fname.endswith('.gz'):
                fh = Csys.popen('zcat %s' % fname)
            elif fname.endswith('.bz2'):
                fn = Csys.popen('bzcat %s' % fname)
            else:
                fh = open(fname)
            self.fh = fh
            while True:
    ...
    def nextMessage(self):
        ...
        if not lastline:
            ...
            if msglines:
                return MailInternet(('').join(msglines))
            return
...
class SendmailDirect(Csys.CSClass):
    """Send Message directly to domain server bypassing
        ...
        """
    _attributes = dict(toaddrs='', fromaddr='', domain='', msg='', verbose=False, hostname='', _errors=[])
...
def _mksalt(size=10):
    ...
    if size < 2:
        raise Csys.Error('mksalt: size %d < 2' % size)
    n = len(_saltchars) - 1
    salt = ''
    while size > 0:
...
class MIMEData(MIMENonMultipart):
    ...
    def __init__(self, ctype, csubtype, data=None, fname=None, **kw):
        ...
        if fname:
            if 'name' not in kw:
                kw['name'] = os.path.basename(fname)
...
if __name__ == '__main__':
    ...
    container['Subject'] = 'testing mime parts'
    container['Date'] = rfcDate()
    intro = MIMEText('This is a test.  This is only a test')
    zipfile = 'RBTEST.zip'
