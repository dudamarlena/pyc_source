...
# [GCC 8.4.0]
# Embedded file name: /home/bill/py3/.virtualenvs/pytest/lib/python2.7/site-packages/Csys/nfpipe.py
...
def main():
    ...
    def setOptions():
        ...
        parser.add_option('-d', '--debugfile', action='store', type='string', dest='debugfile', default=os.path.join(os.path.expanduser('~/tmp/cvs'), os.path.splitext(Csys.Config.progname)[0] + '.log'), help='Debug Output in this file')
        parser.add_option('--sleeptime', action='store', type='int', dest='sleeptime', default=5, help='Sleep Time in Seconds (default 5)')
    ...
    if options.s != 'Changes':
        ...
        tmpdir = os.path.join(debugdir, 'work.%d' % int(time.time()))
        Csys.mkpath(tmpdir)
        os.chdir(tmpdir)
        pid = os.fork()
        if pid:
