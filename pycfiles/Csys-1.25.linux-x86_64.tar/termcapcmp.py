...
# [GCC 8.4.0]
# Embedded file name: /home/bill/py3/.virtualenvs/pytest/lib/python2.7/site-packages/Csys/termcapcmp.py
# Compiled at: 2011-10-05 15:30:44
import Csys, os, os.path, sys, re, Csys.Termcap
...
try:
    ...
    termcapRawMap = [('ae', 'end alternate character set'),
...
def main():
    ...
    def setOptions():
        ...
        parser.add_option('-a', '--all', action='store_true', dest='all', default=False, help='Print all entries including undefined')
        parser.add_option('-d', '--diffs', action='store_true', dest='diffs', default=False, help='Print only differences')
        parser.add_option('-n', '--nopadremove', action='store_true', dest='nopadremove', default=False, help='Do not remove padding')
        parser.add_option('-t', '--terms', action='store_true', dest='terms', default=False, help='Compare terminal entries in same termcap')
        parser.add_option('-x', '--xml', action='store_true', dest='xml', default=False, help='Generate DocBool XML table output')
        return parser
    ...
    if options.terms:
        try:
            ...
            print 'Usage %s -t termcap term1 term2 ...' % Csys.Config.progname
            sys.exit(1)
    ...
    for termcap in termcaps:
        ...
        for term in terms:
            ...
            fieldNames |= set([ fieldPattern.sub('\\1', f) for f in fields[1:] ])
        ...
        if not xml:
            ...
            for tc in compareTermcaps:
                if tc.fname:
                    print '\t\t<entry align="LEFT">%s</entry>' % tc.fname
