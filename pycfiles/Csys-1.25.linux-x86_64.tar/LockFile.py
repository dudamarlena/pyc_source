...
# [GCC 8.4.0]
# Embedded file name: /home/bill/py3/.virtualenvs/pytest/lib/python2.7/site-packages/Csys/LockFile.py
# Compiled at: 2011-10-05 15:43:55
"""File Locking Utilities, similar to perl LockFile::Simple
...
class Simple:
    ...
    _attributes = {'max': 30, 
       ...
       'wafter': 20, 
       'autoclean': False, 
       'lock_by_file': {}, 'wfunc': sys.stderr.write, 
       'efunc': None, 
       'format': None}
    ...
    def __init__(self, file, dolock, **kwargs):
        ...
        for key, val in kwargs.items():
            if key in d:
                ...
                raise Error('Simple: invalid key %s' % key)

        if not self.wfunc:
            self.warn = False
        self._acs_genLockfile()
        ...
        if self.is_locked:
            if os.access(lockfile, os.F_OK):
                ...
                if l == self.stamp:
                    ...
                    raise Error('unlock: cannot lock %s owned by %s' % (lockfile, l))
            else:
                self.is_locked = False
    ...
    def _lockfile(self):
        ...
        format = re.sub('%%', '\\01', format)
        format = re.sub('%', '\\02', format)
        format = re.sub('\\02f', file, format)
        format = re.sub('\\02D', lambda x, s=file: dirname(s), format)
        format = re.sub('\\02F', lambda x, s=file: basename(s), format)
        format = re.sub('\\02', '%')
        format = re.sub('\\01', '%')
        return format
    ...
    def lock(self, tryonly=False):
        ...
        if self.is_locked:
            ...
            file, format, max, delay = (
             self.file, self.format, self.max, self.delay)
            if self.stale:
                self._acs_stale()
            if self.hold:
                self._acs_check()
    ...
    def _acs_check(self):
        ...
        if int(time.time()) - mtime > hold:
            try:
                ...
                raise Error('Cannot unlink %s' % lockfile)
    ...
    def _acs_stale(self):
        ...
        try:
            ...
            raise Error('cannot unlink stale %s' % lockfile)
