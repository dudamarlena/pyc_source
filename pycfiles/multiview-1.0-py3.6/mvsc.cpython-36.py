...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\multiview\mvsc.py
# Compiled at: 2017-12-21 13:53:05
# Size of source mod 2**32: 15728 bytes
...
def mvsc(x, is_distance, k, sigmas=None, neighbours=None, clustering=True):
    ...
    mvcpc = MVCPC(k=k)
    cpc_result_evalues, cpc_result_evectors = mvcpc.fit_transform(lap_matrix)
    cpc_result_evectors = (cpc_result_evectors.T / np.linalg.norm(cpc_result_evectors, axis=1)).T
...
class MVSC(BaseEstimator):
    ...
    def fit_transform(self, x, is_distance):
        ...
        if len(x) != len(is_distance):
            raise ValueError('Data samples and is_distance lengths does notmatch. Data sample length: %d, is_distance length: %d' % (
        ...
        kmeans_clust, cpcresult_evalues, cpcresult_evectors, my_sigmas = mvsc(x, is_distance, self.k, self.sigmas, self.neighbours, self.clustering)
    ...
    def get_params(self, deep=True):
        return {'sigmas':self.sigmas, 
         'neighbours':self.neighbours,  'clustering':self.clustering}
