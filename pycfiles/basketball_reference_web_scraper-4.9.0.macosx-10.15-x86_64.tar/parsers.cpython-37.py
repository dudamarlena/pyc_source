...
# [GCC 8.4.0]
# Embedded file name: /Users/jaebradley/.virtualenvs/basketball_reference_web_scraper/lib/python3.7/site-packages/basketball_reference_web_scraper/parsers.py
# Compiled at: 2020-03-05 11:58:34
# Size of source mod 2**32: 25841 bytes
import re
...
class PositionAbbreviationParser:
    ...
    def from_abbreviations(self, abbreviations):
        parsed_positions = list(map(lambda position_abbreviation: self.from_abbreviation(position_abbreviation), abbreviations.split('-')))
...
class LeagueAbbreviationParser:
    ...
    def from_abbreviations(self, abbreviations):
        if abbreviations is None:
            return []
        return [self.from_abbreviation(abbreviation=league_abbreviation) for league_abbreviation in abbreviations.split('/')]


class PlayerBoxScoreOutcomeParser:

    def __init__(self, outcome_abbreviation_parser, formatted_outcome_regex=PLAYER_SEASON_BOX_SCORES_OUTCOME_REGEX, outcome_abbreviation_regex_group_name='outcome_abbreviation', margin_of_victory_regex_group_name='margin_of_victory'):
    ...
    def parse_outcome_abbreviation(self, formatted_outcome):
        return self.search_formatted_outcome(formatted_outcome=formatted_outcome).group(self.outcome_abbreviation_regex_group_name)

    def parse_outcome(self, formatted_outcome):
        return self.outcome_abbreviation_parser.from_abbreviation(abbreviation=self.parse_outcome_abbreviation(formatted_outcome=formatted_outcome))

    def parse_margin_of_victory(self, formatted_outcome):
        return int(self.search_formatted_outcome(formatted_outcome=formatted_outcome).group(self.margin_of_victory_regex_group_name))
...
class ScoresParser:
    ...
    def __init__(self, scores_regex, away_team_score_group_name='away_team_score', home_team_score_group_name='home_team_score'):
    ...
    def parse_away_team_score(self, formatted_scores):
        return int(self.parse_scores(formatted_scores=formatted_scores).group(self.away_team_score_group_name))

    def parse_home_team_score(self, formatted_scores):
        return int(self.parse_scores(formatted_scores=formatted_scores).group(self.home_team_score_group_name))
...
class ScheduledStartTimeParser:
    ...
    def parse_start_time(self, formatted_date, formatted_time_of_day):
        if formatted_time_of_day is not None:
            if formatted_time_of_day not in ('', ' '):
...
class SearchResultNameParser:
    ...
    def parse(self, search_result_name):
        return re.search(self.search_result_name_regex, search_result_name).group(self.result_name_regex_group_name).strip()


class ResourceLocationParser:

    def __init__(self, resource_location_regex, resource_type_regex_group_name='resource_type', resource_identifier_regex_group_name='resource_identifier'):
...
class ScheduledGamesParser:
    ...
    def parse_games(self, games):
        return [{'start_time':self.start_time_parser.parse_start_time(formatted_date=game.start_date, formatted_time_of_day=game.start_time_of_day),  'away_team':self.team_name_parser.parse_team_name(team_name=game.away_team_name),  'home_team':self.team_name_parser.parse_team_name(team_name=game.home_team_name),  'away_team_score':str_to_int(value=game.away_team_score, default=None),  'home_team_score':str_to_int(value=game.home_team_score, default=None)} for game in games]
...
class PlayerAdvancedSeasonTotalsParser:
    ...
    def parse(self, totals):
        return [{'slug':str(total.slug),  'name':str(total.name),  'positions':self.position_abbreviation_parser.from_abbreviations(total.position_abbreviations),  'age':str_to_int(total.age, default=None),  'team':self.team_abbreviation_parser.from_abbreviation(total.team_abbreviation),  'games_played':str_to_int(total.games_played),  'minutes_played':str_to_int(total.minutes_played),  'player_efficiency_rating':str_to_float(total.player_efficiency_rating),  'true_shooting_percentage':str_to_float(total.true_shooting_percentage),  'three_point_attempt_rate':str_to_float(total.three_point_attempt_rate),  'free_throw_attempt_rate':str_to_float(total.free_throw_attempt_rate),  'offensive_rebound_percentage':str_to_float(total.offensive_rebound_percentage),  'defensive_rebound_percentage':str_to_float(total.defensive_rebound_percentage),  'total_rebound_percentage':str_to_float(total.total_rebound_percentage),  'assist_percentage':str_to_float(total.assist_percentage),  'steal_percentage':str_to_float(total.steal_percentage),  'block_percentage':str_to_float(total.block_percentage),  'turnover_percentage':str_to_float(total.turnover_percentage),  'usage_percentage':str_to_float(total.usage_percentage),  'offensive_win_shares':str_to_float(total.offensive_win_shares),  'defensive_win_shares':str_to_float(total.defensive_win_shares),  'win_shares':str_to_float(total.win_shares),  'win_shares_per_48_minutes':str_to_float(total.win_shares_per_48_minutes),  'offensive_box_plus_minus':str_to_float(total.offensive_plus_minus),  'defensive_box_plus_minus':str_to_float(total.defensive_plus_minus),  'box_plus_minus':str_to_float(total.plus_minus),  'value_over_replacement_player':str_to_float(total.value_over_replacement_player),  'is_combined_totals':total.is_combined_totals} for total in totals]
...
class PlayerSeasonTotalsParser:
    ...
    def parse(self, totals):
        return [{'slug':str(total.slug),  'name':str(total.name),  'positions':self.position_abbreviation_parser.from_abbreviations(total.position_abbreviations),  'age':str_to_int(total.age, default=None),  'team':self.team_abbreviation_parser.from_abbreviation(total.team_abbreviation),  'games_played':str_to_int(total.games_played),  'games_started':str_to_int(total.games_started),  'minutes_played':str_to_int(total.minutes_played),  'made_field_goals':str_to_int(total.made_field_goals),  'attempted_field_goals':str_to_int(total.attempted_field_goals),  'made_three_point_field_goals':str_to_int(total.made_three_point_field_goals),  'attempted_three_point_field_goals':str_to_int(total.attempted_three_point_field_goals),  'made_free_throws':str_to_int(total.made_free_throws),  'attempted_free_throws':str_to_int(total.attempted_free_throws),  'offensive_rebounds':str_to_int(total.offensive_rebounds),  'defensive_rebounds':str_to_int(total.defensive_rebounds),  'assists':str_to_int(total.assists),  'steals':str_to_int(total.steals),  'blocks':str_to_int(total.blocks),  'turnovers':str_to_int(total.turnovers),  'personal_fouls':str_to_int(total.personal_fouls),  'points':str_to_int(total.points)} for total in totals]
...
class TeamTotalsParser:
    ...
    def parse(self, first_team_totals, second_team_totals):
        return [
         self.parse_totals(team_totals=first_team_totals,
           opposing_team_totals=second_team_totals),
         self.parse_totals(team_totals=second_team_totals,
    ...
    def parse_totals(self, team_totals, opposing_team_totals):
        ...
        if str_to_int(team_totals.points) > str_to_int(opposing_team_totals.points):
            ...
            if str_to_int(team_totals.points) < str_to_int(opposing_team_totals.points):
                ...
                outcome = None
        return {'team':current_team,  'outcome':outcome, 
...
class PlayerBoxScoresParser:
    ...
    def __init__(self, team_abbreviation_parser, location_abbreviation_parser, outcome_abbreviation_parser, seconds_played_parser):
        self.team_abbreviation_parser = team_abbreviation_parser
    ...
    def parse(self, box_scores):
        return [{'slug':str(box_score.slug),  'name':str(box_score.name),  'team':self.team_abbreviation_parser.from_abbreviation(box_score.team_abbreviation),  'location':self.location_abbreviation_parser.from_abbreviation(box_score.location_abbreviation),  'opponent':self.team_abbreviation_parser.from_abbreviation(box_score.opponent_abbreviation),  'outcome':self.outcome_abbreviation_parser.from_abbreviation(box_score.outcome),  'seconds_played':self.seconds_played_parser.parse(box_score.playing_time),  'made_field_goals':str_to_int(box_score.made_field_goals),  'attempted_field_goals':str_to_int(box_score.attempted_field_goals),  'made_three_point_field_goals':str_to_int(box_score.made_three_point_field_goals),  'attempted_three_point_field_goals':str_to_int(box_score.attempted_three_point_field_goals),  'made_free_throws':str_to_int(box_score.made_free_throws),  'attempted_free_throws':str_to_int(box_score.attempted_free_throws),  'offensive_rebounds':str_to_int(box_score.offensive_rebounds),  'defensive_rebounds':str_to_int(box_score.defensive_rebounds),  'assists':str_to_int(box_score.assists),  'steals':str_to_int(box_score.steals),  'blocks':str_to_int(box_score.blocks),  'turnovers':str_to_int(box_score.turnovers),  'personal_fouls':str_to_int(box_score.personal_fouls),  'game_score':str_to_float(box_score.game_score)} for box_score in box_scores]
...
class PlayerSeasonBoxScoresParser:
    ...
    def parse(self, box_scores):
        return [{'date':datetime.strptime(str(box_score.date), '%Y-%m-%d').date(),  'team':self.team_abbreviation_parser.from_abbreviation(box_score.team_abbreviation),  'location':self.location_abbreviation_parser.from_abbreviation(box_score.location_abbreviation),  'opponent':self.team_abbreviation_parser.from_abbreviation(box_score.opponent_abbreviation),  'outcome':self.outcome_parser.parse_outcome(formatted_outcome=box_score.outcome),  'seconds_played':self.seconds_played_parser.parse(box_score.playing_time),  'made_field_goals':str_to_int(box_score.made_field_goals),  'attempted_field_goals':str_to_int(box_score.attempted_field_goals),  'made_three_point_field_goals':str_to_int(box_score.made_three_point_field_goals),  'attempted_three_point_field_goals':str_to_int(box_score.attempted_three_point_field_goals),  'made_free_throws':str_to_int(box_score.made_free_throws),  'attempted_free_throws':str_to_int(box_score.attempted_free_throws),  'offensive_rebounds':str_to_int(box_score.offensive_rebounds),  'defensive_rebounds':str_to_int(box_score.defensive_rebounds),  'assists':str_to_int(box_score.assists),  'steals':str_to_int(box_score.steals),  'blocks':str_to_int(box_score.blocks),  'turnovers':str_to_int(box_score.turnovers),  'personal_fouls':str_to_int(box_score.personal_fouls),  'points_scored':str_to_int(box_score.points_scored),  'game_score':str_to_float(box_score.game_score),  'plus_minus':str_to_int(box_score.plus_minus)} for box_score in box_scores if box_score.is_active]
...
class PlayByPlaysParser:
    ...
    def format_data(self, current_period, play_by_play, away_team, home_team):
        return {'period':self.period_details_parser.parse_period_number(period_count=current_period), 
         ...
         'description':play_by_play.away_team_play_description if play_by_play.is_away_team_play else play_by_play.home_team_play_description}
...
class SearchResultsParser:
    ...
    def parse(self, nba_aba_baa_players):
        return {'players': [{'name':self.search_result_name_parser.parse(search_result_name=result.resource_name),  'identifier':self.search_result_location_parser.parse_resource_identifier(resource_location=result.resource_location),  'leagues':set(self.league_abbreviation_parser.from_abbreviations(abbreviations=(result.league_abbreviations)))} for result in nba_aba_baa_players]}
...
class PlayerDataParser:
    ...
    def parse(self, player):
        return {'name':player.name, 
         'identifier':self.search_result_location_parser.parse_resource_identifier(resource_location=player.resource_location), 
         'leagues':set((self.league_abbreviation_parser.from_abbreviation(abbreviation=abbreviation) for abbreviation in player.league_abbreviations))}