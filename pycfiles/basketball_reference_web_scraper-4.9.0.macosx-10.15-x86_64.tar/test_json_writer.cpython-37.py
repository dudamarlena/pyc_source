...
# [GCC 8.4.0]
# Embedded file name: /Users/jaebradley/.virtualenvs/basketball_reference_web_scraper/lib/python3.7/site-packages/tests/unit/test_json_writer.py
...
class TestJSONWriter(TestCase):
    ...
    def test_writing_to_memory_with_default_options(self, json_dumps):
        options = mock.Mock(custom_options=None,
    ...
    def test_writing_to_memory_with_custom_options(self, json_dumps):
        options = mock.Mock(custom_options={'jae':'baebae', 
         'bae':'jadley'},
          should_write_to_file=mock.Mock(return_value=False))
        self.writer.write(data=(self.mock_data), options=options)
        options.should_write_to_file.assert_called_once_with()
        json_dumps.assert_called_once_with((self.mock_data),
    ...
    def test_writing_to_file_with_default_options(self, json_dump):
        ...
        with mock.patch('builtins.open', mock.mock_open()) as (mock_file):
            options = mock.Mock(file_path=file_path,
            ...
            json_dump.assert_called_once_with((self.mock_data),
              (mock_file()),
              cls=(self.mock_encoder),
              sort_keys=True,
              indent=4)
