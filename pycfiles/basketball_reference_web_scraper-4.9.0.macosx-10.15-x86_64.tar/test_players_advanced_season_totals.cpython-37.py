...
# [GCC 8.4.0]
# Embedded file name: /Users/jaebradley/.virtualenvs/basketball_reference_web_scraper/lib/python3.7/site-packages/tests/integration/client/test_players_advanced_season_totals.py
...
class BaseTestPlayerAdvancedSeasonTotalsCSVOutput(TestCase):
    ...
    def setUp(self):
        self.output_file_path = os.path.join(os.path.dirname(__file__), '../output/player_advanced_season_totals_{year}.csv'.format(year=(self.year)))
        self.expected_output_file_path = os.path.join(os.path.dirname(__file__), '../output/expected/player_advanced_season_totals_{year}.csv'.format(year=(self.year)))

    def tearDown(self):
        os.remove(self.output_file_path)

    def assert_player_advanced_season_totals_csv(self):
        players_advanced_season_totals(season_end_year=(self.year),
...
class BaseTestPlayerAdvancedSeasonTotalsJSONOutput(TestCase):
    ...
    def setUp(self):
        self.output_file_path = os.path.join(os.path.dirname(__file__), '../output/player_advanced_season_totals_{year}.json'.format(year=(self.year)))
        self.expected_output_file_path = os.path.join(os.path.dirname(__file__), '../output/expected/player_advanced_season_totals_{year}.json'.format(year=(self.year)))
    ...
    def assert_player_advanced_season_totals_json(self):
        ...
        with open(self.output_file_path, 'r') as (output_file):
            with open(self.expected_output_file_path, 'r') as (expected_output_file):
                self.assertEqual(json.load(output_file), json.load(expected_output_file))
...
class TestPlayerAdvancedSeasonTotalsInMemoryOutput(TestCase):
    ...
    def test_first_2018_players_advanced_season_totals_row(self):
        result = players_advanced_season_totals(season_end_year=2018)
        self.assertEqual(result[0], {'age':24, 
    ...
    def test_last_2018_players_advanced_season_totals_row(self):
        result = players_advanced_season_totals(season_end_year=2018)
        self.assertEqual(result[604], {'age':20, 
    ...
    def test_players_advanced_season_totals_json(self):
        expected_output_file_path = os.path.join(os.path.dirname(__file__), '../output/expected/player_advanced_season_totals_2018.json')
        ...
        with open(expected_output_file_path, 'r') as (expected_output):
            self.assertEqual(json.loads(result), json.load(expected_output))