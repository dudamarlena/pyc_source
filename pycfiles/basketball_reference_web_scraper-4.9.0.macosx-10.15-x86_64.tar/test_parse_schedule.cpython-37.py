...
# [GCC 8.4.0]
# Embedded file name: /Users/jaebradley/.virtualenvs/basketball_reference_web_scraper/lib/python3.7/site-packages/tests/integration/parsers/test_parse_schedule.py
# Compiled at: 2020-03-05 11:58:35
# Size of source mod 2**32: 3869 bytes
import os
...
class TestScheduleParser(TestCase):
    ...
    def setUp(self):
        self.october_2001_html = requests.get('https://www.basketball-reference.com/leagues/NBA_2001_games.html').text
        self.october_2018_html = requests.get('https://www.basketball-reference.com/leagues/NBA_2018_games-october.html').text
        self.schedule_with_future_games_html_file = open(os.path.join(os.path.dirname(__file__), '../files/NBA_2019_games-april.html'))
        self.schedule_with_future_games_html = self.schedule_with_future_games_html_file.read()
        self.parser = ScheduledGamesParser(start_time_parser=(ScheduledStartTimeParser()),
    ...
    def test_parse_october_2001_schedule(self):
        ...
        expected_datetime = pytz.timezone('US/Eastern').localize(datetime(year=2000, month=10, day=31, hour=19, minute=30)).astimezone(pytz.utc)
    ...
    def test_parse_future_game(self):
        ...
        parsed_schedule = self.parser.parse_games(games=(page.rows))
        first_game = parsed_schedule[0]
        expected_first_game_start_time = pytz.timezone('US/Eastern').localize(datetime(year=2019, month=4, day=1, hour=19, minute=30)).astimezone(pytz.utc)
