...
# [GCC 8.4.0]
# Embedded file name: /Users/jaebradley/.virtualenvs/basketball_reference_web_scraper/lib/python3.7/site-packages/basketball_reference_web_scraper/writers.py
# Compiled at: 2020-03-05 11:58:34
# Size of source mod 2**32: 6003 bytes
import csv, json
...
BOX_SCORE_COLUMN_NAMES = [
 'slug', 'name'] + SHARED_COLUMN_NAMES
PLAYER_SEASON_BOX_SCORE_COLUMN_NAMES = [
 'date'] + SHARED_COLUMN_NAMES
...
class WriteOptions:
    ...
    def __eq__(self, other):
        if isinstance(other, WriteOptions):
            return self.file_path == other.file_path and self.mode == other.mode and self.custom_options == other.custom_options
        return False


class JSONWriter:
    DEFAULT_OPTIONS = {'sort_keys':True, 
     'indent':4}
    ...
    def write(self, data, options):
        output_options = self.DEFAULT_OPTIONS if options.custom_options is None else merge_two_dicts(first=(self.DEFAULT_OPTIONS),
...
class SearchResultsCSVWriter(CSVWriter):
    ...
    def format_rows(self, data):
        return [self.row_formatter.format(row_data=row_data) for row_data in data['players']]
...
class RowFormatter:
    ...
    def format_field_data(field_name, field_data):
        if field_name in ('away_team', 'home_team', 'team', 'location', 'opponent',
                          'outcome', 'relevant_team', 'period_type', 'leagues'):
    ...
    def format(self, row_data):
        return {data_field_name:self.format_field_data(field_name=data_field_name, field_data=(row_data[data_field_name])) for data_field_name in self.data_field_names}