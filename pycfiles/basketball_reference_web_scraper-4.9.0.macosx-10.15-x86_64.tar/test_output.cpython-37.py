...
# [GCC 8.4.0]
# Embedded file name: /Users/jaebradley/.virtualenvs/basketball_reference_web_scraper/lib/python3.7/site-packages/tests/unit/test_output.py
...
class TestOutput(TestCase):
    ...
    def test_return_values_when_output_type_is_none(self):
        self.assertEqual(self.values, output(values=(self.values),
          output_type=None,
    ...
    def test_output_json_when_output_write_option_is_none_and_no_custom_options(self, mock_json_writer):
        ...
        mock_json_writer_instance.write.assert_called_once_with(data=(self.values),
          options=WriteOptions(file_path=(self.output_file_path),
          mode=(OutputWriteOption.WRITE),
          custom_options=None))

    @mock.patch('basketball_reference_web_scraper.output.JSONWriter')
    def test_output_json_when_output_write_option_is_append_and_no_custom_options(self, mock_json_writer):
        mock_json_writer_instance = mock.Mock(write=(mock.Mock()))
        mock_json_writer.return_value = mock_json_writer_instance
        output(values=(self.values),
          output_type=(OutputType.JSON),
    ...
    @mock.patch('basketball_reference_web_scraper.output.JSONWriter')
    def test_output_json_when_output_write_option_is_none_and_custom_options(self, mock_json_writer):
        mock_json_writer_instance = mock.Mock(write=(mock.Mock()))
        mock_json_writer.return_value = mock_json_writer_instance
        output(values=(self.values),
          output_type=(OutputType.JSON),
          output_file_path=(self.output_file_path),
          csv_writer=None,
          json_options={'jae':'baebae', 
         'bae':'jadley'})
        mock_json_writer.assert_called_once_with(encoder=BasketballReferenceJSONEncoder)
        mock_json_writer_instance.write.assert_called_once_with(data=(self.values),
          options=WriteOptions(file_path=(self.output_file_path),
          mode=(OutputWriteOption.WRITE),
          custom_options={'jae':'baebae', 
         'bae':'jadley'}))

    @mock.patch('basketball_reference_web_scraper.output.JSONWriter')
    def test_output_json_when_output_write_option_is_append_and_custom_options(self, mock_json_writer):
        mock_json_writer_instance = mock.Mock(write=(mock.Mock()))
        mock_json_writer.return_value = mock_json_writer_instance
        output(values=(self.values),
          output_type=(OutputType.JSON),
    ...
    def test_output_csv_when_output_write_option_is_none_and_no_custom_options(self):
        ...
        csv_writer.write.assert_called_once_with(data=(self.values),
          options=WriteOptions(file_path=(self.output_file_path),
          mode=(OutputWriteOption.WRITE)))
