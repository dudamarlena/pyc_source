...
# [GCC 8.4.0]
# Embedded file name: /Users/jaebradley/.virtualenvs/basketball_reference_web_scraper/lib/python3.7/site-packages/tests/integration/client/test_players_season_totals.py
# Compiled at: 2020-03-05 11:58:34
# Size of source mod 2**32: 4488 bytes
import json, os
...
class BaseTestPlayerSeasonTotalsJSONFileOutput(TestCase):
    ...
    def setUp(self):
        self.output_file_path = os.path.join(os.path.dirname(__file__), '../output/player_season_totals_{year}.json'.format(year=(self.year)))
        self.expected_output_file_path = os.path.join(os.path.dirname(__file__), '../output/expected/player_season_totals_{year}.json'.format(year=(self.year)))

    def tearDown(self):
        os.remove(self.output_file_path)

    def assert_player_season_totals_file(self):
        players_season_totals(season_end_year=(self.year),
...
class BaseTestPlayerSeasonTotalsCSVFileOutput(TestCase):
    ...
    def setUp(self):
        self.output_file_path = os.path.join(os.path.dirname(__file__), '../output/player_season_totals_{year}.csv'.format(year=(self.year)))
        self.expected_output_file_path = os.path.join(os.path.dirname(__file__), '../output/expected/player_season_totals_{year}.csv'.format(year=(self.year)))
...
class Test2018PlayerSeasonTotalsInMemoryJSONOutput(TestCase):
    ...
    def setUp(self):
        self.expected_output_file_path = os.path.join(os.path.dirname(__file__), '../output/expected/player_season_totals_2018.json')
    ...
    def test_players_season_totals_json(self):
        ...
        with open(self.expected_output_file_path, 'r') as (expected_results_file):
            self.assertEqual(json.loads(result), json.load(expected_results_file))
