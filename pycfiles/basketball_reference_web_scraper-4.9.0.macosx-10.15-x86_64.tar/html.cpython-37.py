...
# [GCC 8.4.0]
# Embedded file name: /Users/jaebradley/.virtualenvs/basketball_reference_web_scraper/lib/python3.7/site-packages/basketball_reference_web_scraper/html.py
# Compiled at: 2020-03-05 11:58:34
# Size of source mod 2**32: 26590 bytes
import re
...
class PlayerAdvancedSeasonTotalsTable:
    ...
    def get_rows(self, include_combined_totals=False):
        ...
        for row_html in self.html.xpath(self.rows_query):
            ...
            if include_combined_totals is True:
                if row.is_combined_totals is True or row.is_combined_totals is False:
...
class BoxScoresPage:
    ...
    def statistics_tables(self):
        return [StatisticsTable(table_html) for table_html in self.html.xpath('//table[contains(@class, "stats_table")]')]
...
class DailyLeadersPage:
    ...
    def daily_leaders(self):
        return [PlayerGameBoxScoreRow(row_html) for row_html in self.html.xpath('//table[@id="stats"]//tbody/tr[not(contains(@class, "thead"))]')]
...
class RegularSeasonPlayerBoxScoresTable(PlayerSeasonBoxScoresTable):
    ...
    @property
    def rows_query(self):
        return '//tbody/tr[not(contains(@class, "thead"))]'
...
class PlayByPlayPage:
    ...
    def team_names_query(self):
        return '//*[@id="content"]//div[@class="scorebox"]//div[@itemprop="performer"]//a[@itemprop="name"]'
...
class PlayByPlayTable:
    ...
    @property
    def rows(self):
        return [PlayByPlayRow(html=row_html) for row_html in self.html[1:]]
...
class PlayByPlayRow:
    ...
    def has_play_by_play_data(self):
        return not self.is_start_of_period and self.html[1].get('colspan') != '5' and self.timestamp_cell.get('aria-label') != 'Time'
...
class SchedulePage:
    ...
    @property
    def rows(self):
        return [ScheduleRow(html=row) for row in self.html.xpath(self.rows_query) if row.text_content() != 'Playoffs']
...
class SearchPage:
    ...
    def nba_aba_baa_players_pagination_links_query(self):
        return '{NBA_ABA_PLAYERS_CONTENT_QUERY}/div[@class="search-pagination"]/a'.format(NBA_ABA_PLAYERS_CONTENT_QUERY=(self.nba_aba_baa_players_content_query))

    @property
    def nba_aba_baa_player_search_items_query(self):
        return '{NBA_ABA_PLAYERS_CONTENT_QUERY}/div[@class="search-item"]'.format(NBA_ABA_PLAYERS_CONTENT_QUERY=(self.nba_aba_baa_players_content_query))
    ...
    def nba_aba_baa_players(self):
        return [PlayerSearchResult(html=result_html) for result_html in self.html.xpath(self.nba_aba_baa_player_search_items_query)]
