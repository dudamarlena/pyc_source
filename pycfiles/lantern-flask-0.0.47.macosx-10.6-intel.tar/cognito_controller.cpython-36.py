...
# [GCC 8.4.0]
# Embedded file name: /Users/jceciliano/coding/Lantern/lantern-flask/.virtualenv/lib/python3.6/site-packages/lantern_flask/controllers/cognito_controller.py
# Compiled at: 2018-11-30 11:45:11
# Size of source mod 2**32: 3975 bytes
import json
...
class CognitoController(object):
    ...
    def get(self, attribute_name, request, json_parse=False):
        ...
        if not request:
            ...
            if self.debug:
                _local_header = {'authorizer':'{"claims": {"custom:fullName": "local_user", "cognito:username": "local_user", "email": "local_user@lantern.tech", "email_verified": true}}',  'Authorization':'{}'.format(settings.get('LOCAL_TOKEN', ''))}
    ...
    def get_default(self, name, request, header_attribute_name='authorizer', container_node='claims'):
        ...
        """
        header_value = self.get(attribute_name=header_attribute_name,
          request=request,
          json_parse=True)
        if container_node:
            header_node = header_value.get(container_node, None)
            if not header_node:
                raise ValueError("'{}' container_node not found in header (inside: {})".format(container_node, header_attribute_name))
            requested_value = header_node.get(name, None)
            if not requested_value:
                raise ValueError("'{}' value empty/not found in {}.{}".format(name, header_attribute_name, container_node))
