...
# Size of source mod 2**32: 29562 bytes
"""
Base class for finding modules.
"""
import dis, imp, importlib.machinery, importlib.util, logging, marshal, opcode, os, pkgutil, sys, tokenize, types, zipfile, cx_Freeze.hooks
            ...
            if ext not in ('.pyc', '.pyo'):
                ...
                if '__pycache__' in baseName:
                    if not baseName.endswith(sys.implementation.cache_tag):
                        ...
                        baseName = os.path.splitext(importlib.util.source_from_cache(archiveName))[0]
...
class ModuleFinder(object):
    ...
    def __init__(self, include_files=None, excludes=None, path=None, replace_paths=None, zip_include_all_packages=False, zip_exclude_packages=None, zip_include_packages=None, constants_module=None, zip_includes=None):
    ...
    def _AddModule(self, name, path=None, file_name=None, parent=None):
        ...
        if module is None:
            module = self._modules[name] = Module(name, path, file_name, parent)
            self.modules.append(module)
            if name in self._bad_modules:
                logging.debug('Removing module [%s] from list of bad modules', name)
                del self._bad_modules[name]
            if self.zip_include_all_packages and module.name not in self.zip_exclude_packages or module.name in self.zip_include_packages:
    ...
    def _FindModule(self, name, path, namespace):
        try:
            ...
            if namespace and name in sys.modules:
                ...
                return (
                 None, list(module.__path__)[0], info)
            if path is None:
                path = []
    ...
    def _ScanCode(self, co, module, deferredImports, topLevel=True):
        ...
        arguments = []
        importedModule = None
        method = dis._unpack_opargs if sys.version_info[:3] >= (3, 5, 2) else self._UnpackOpArgs
