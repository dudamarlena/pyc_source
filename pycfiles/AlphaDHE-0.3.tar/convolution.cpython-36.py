...
# Embedded file name: /home/tom/.local/miniconda/lib/python3.6/site-packages/alphacsc/utils/convolution.py
# Compiled at: 2019-06-05 05:26:13
# Size of source mod 2**32: 8202 bytes
__doc__ = 'Convolutional utilities for dictionary learning'
...
def _dense_convolve_multi(z_i, ds):
    """Convolve z_i[k] and ds[k] for each atom k, and return the sum."""
    return np.sum([[np.convolve(zik, dkp) for dkp in dk] for zik, dk in zip(z_i, ds)], 0)
...
def _choose_convolve_multi(z_i, D=None, n_channels=None):
    ...
    n_channels : int
        Number of channels
    """
    if not z_i.shape[0] == D.shape[0]:
        raise AssertionError
    else:
        if is_lil(z_i):
            cython_code._assert_cython()
            if D.ndim == 2:
                return cython_code._fast_sparse_convolve_multi_uv(z_i,
                  D, n_channels, compute_D=True)
            else:
...
@numba.jit((numba.float64[:, :, :], numba.float64[:, :]), cache=True, nopython=True)
    ...
    for k0 in range(n_atoms):
        for k1 in range(n_atoms):
            for t in range(n_times_atom):
                G[k0, :, t] += np.sum(ztz[k0, k1, t:t + n_times_atom] * v[k1]) * u[k1, :]
...
def tensordot_convolve(ztz, D):
    ...
    G = np.zeros(D.shape)
    for t in range(n_times_atom):
        G[:, :, t] = np.tensordot((ztz[:, :, t:t + n_times_atom]), D_revert, axes=(
         [
          1, 2], [0, 2]))
...
def sort_atoms_by_explained_variances(D_hat, z_hat, n_channels):
    ...
    for kk in range(n_atoms):
        variances[kk] = construct_X_multi((z_hat[:, kk:kk + 1]), (D_hat[kk:kk + 1]),
