...
# Embedded file name: /home/tom/.local/miniconda/lib/python3.6/site-packages/alphacsc/update_z_multi.py
...
def update_z_multi(X, D, reg, z0=None, solver='l-bfgs', solver_kwargs=dict(), loss='l2', loss_params=dict(), freeze_support=False, return_ztz=False, timing=False, n_jobs=1, random_state=None, debug=False):
    ...
    parallel_seeds = [rng.randint(4294967296) for _ in range(n_trials)]
    if z0 is None:
        z0 = np.zeros((n_trials, n_atoms, n_times_valid))
    else:
        delayed_update_z = delayed(_update_z_multi_idx)
        results = Parallel(n_jobs=n_jobs)(delayed_update_z((X[i]), D, reg, (z0[i]), debug, solver, solver_kwargs, freeze_support, loss, loss_params=loss_params, return_ztz=return_ztz, timing=timing, random_state=seed) for i, seed in enumerate(parallel_seeds))
        z_hats, pobj, times = [], [], []
        if loss == 'l2':
            if return_ztz:
...
def _update_z_multi_idx(X_i, D, reg, z0_i, debug, solver='l-bfgs', solver_kwargs=dict(), freeze_support=False, loss='l2', loss_params=dict(), return_ztz=False, timing=False, random_state=None):
    t_start = time.time()
    ...
    def func_and_grad(zi):
        return gradient_zi(Xi=X_i, zi=zi, D=D, constants=constants, reg=reg,
          return_func=True,
          flatten=True,
          loss=loss,
    ...
    if z0_i is None:
        ...
        if solver == 'l-bfgs':
            ...
            z_hat, f, d = optimize.fmin_l_bfgs_b(func_and_grad,
              x0=z0_i, fprime=None, args=(), approx_grad=False, bounds=bounds,
              factr=factr,
              maxiter=maxiter,
              callback=callback)
        else:
            if solver in ('ista', 'fista'):
                ...
                def prox(z_hat, step_size=0):
                    return np.maximum(z_hat - step_size * reg, 0.0)

                z0_i = z0_i.ravel()
                output = fista(objective, grad, prox, step_size=None, x0=z0_i, adaptive_step_size=True, 
                 timing=timing, name='Update z', **fista_kwargs)
                if timing:
                ...
                if solver == 'lgcd':
                    tol = solver_kwargs.get('tol', 0.001)
                    n_seg = solver_kwargs.get('n_seg', 'auto')
                    max_iter = solver_kwargs.get('max_iter', 1000000000000000.0)
                    strategy = solver_kwargs.get('strategy', 'greedy')
                    output = _coordinate_descent_idx(X_i,
                      D, constants, reg=reg, z0=z0_i, max_iter=max_iter, tol=tol, strategy=strategy,
                      n_seg=n_seg,
                    ...
                    raise ValueError("Unrecognized solver %s. Must be 'ista', 'fista', 'l-bfgs', or 'lgcd'." % solver)
        if not is_lil(z_hat):
