...
# Embedded file name: /home/tom/.local/miniconda/lib/python3.6/site-packages/alphacsc/online_dictionary_learning.py
...
class OnlineCDL(ConvolutionalDictionaryLearning):
    ...
    def __init__(self, n_atoms, n_times_atom, reg=0.1, n_iter=60, n_jobs=1, solver_z='lgcd', solver_z_kwargs={}, unbiased_z_hat=False, solver_d='alternate_adaptive', solver_d_kwargs={}, rank1=True, window=False, uv_constraint='separate', lmbd_max='scaled', eps=1e-10, D_init=None, D_init_params={}, alpha=0.8, batch_size=1, batch_selection='random', verbose=10, random_state=None):
        super().__init__(n_atoms,
          ...
          algorithm_params=dict(alpha=alpha, batch_size=batch_size, batch_selection=batch_selection),
          n_jobs=n_jobs,
    ...
    def partial_fit(self, X, y=None):
        ...
        z_hat, ztz, ztX = update_z_multi(X,
          (self._D_hat), reg=(self.reg_), solver=(self.solver_z),
        ...
        if self.verbose > 5:
            print('[{}] sparsity: {:.3e}'.format(self.name, z_nnz.sum() / z_hat.size))
        if np.all(z_nnz == 0):
            import warnings
            warnings.warn('Regularization parameter `reg` is too large and all the activations are zero. The atoms has not been updated.', UserWarning)
            return z_hat
        else:
            d_kwargs = dict(verbose=(self.verbose), eps=1e-08)
            d_kwargs.update(self.solver_d_kwargs)
            if self.rank1:
                self._D_hat = update_uv(X, z_hat, uv_hat0=self._D_hat, constants=self.constants, solver_d=self.solver_d, 
                 uv_constraint=self.uv_constraint, loss=self.loss, 
                 loss_params=self.loss_params, window=self.window, **d_kwargs)
            else:
                self._D_hat = update_d(
 X, z_hat, D_hat0=self._D_hat, constants=self.constants, solver_d=self.solver_d, 
                 uv_constraint=self.uv_constraint, loss=self.loss, 
                 loss_params=self.loss_params, window=self.window, **d_kwargs)
            return z_hat
    ...
    def _ensure_fit_init(self, X):
        ...
        if not X.ndim == 3:
            ...
            self.constants['ztX'] = np.zeros((self.n_atoms, n_channels,
             self.n_times_atom))
            self._D_hat = init_dictionary(X,
              (self.n_atoms), (self.n_times_atom), rank1=(self.rank1), window=(self.window),
    ...
    def _check_param_partial_fit(self):
        assert self.loss == 'l2', 'partial_fit is implemented only for loss={}.'.format(self.loss)