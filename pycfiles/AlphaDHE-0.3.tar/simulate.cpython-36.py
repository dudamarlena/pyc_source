...
# Embedded file name: /home/tom/.local/miniconda/lib/python3.6/site-packages/alphacsc/simulate.py
# Compiled at: 2019-06-04 04:10:26
# Size of source mod 2**32: 3584 bytes
import numpy as np
from .utils import check_random_state, construct_X

def simulate_data(n_trials, n_times, n_times_atom, n_atoms, random_state=42, constant_amplitude=False):
    """Simulate the data.
    ...
    z = get_activations(rng, (n_atoms, n_trials, n_times - n_times_atom + 1), constant_amplitude=constant_amplitude)
    X = construct_X(z, ds)
    if not X.shape == (n_trials, n_times):
...
def get_activations(rng, shape_z, constant_amplitude=False):
    ...
    for idx in range(n_atoms):
        starts.append(rng.randint(low=0, high=n_times_valid, size=(
         n_trials,)))
