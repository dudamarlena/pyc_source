...
# Embedded file name: /home/tom/.local/miniconda/lib/python3.6/site-packages/alphacsc/utils/optim.py
...
def fista(f_obj, f_grad, f_prox, step_size, x0, max_iter, verbose=0, momentum=False, eps=None, adaptive_step_size=False, debug=False, scipy_line_search=True, name='ISTA', timing=False):
    ...
    for ii in range(max_iter):
        t_update = time.time()
        if verbose > 1:
            if t_update - last_up > 1:
                print(('\r[FISTA:PROGRESS] {:.0f}s - {:7.2%} iterations'.format(t_update - t_start, ii / max_iter)),
                  end='',
                  flush=True)
        has_restarted = False
        ...
        if adaptive_step_size:
            ...
            def compute_obj_and_step(step_size, return_x_hat=False):
                x_hat = f_prox((x_hat_aux - step_size * grad), step_size=step_size)
                pobj = f_obj(x_hat)
                if return_x_hat:
                    return (pobj, x_hat)
                else:
            ...
            if scipy_line_search:
                ...
                step_size, obj_uv = optimize.linesearch.scalar_search_armijo(compute_obj_and_step,
                  obj_uv, (-norm_grad), c1=1e-05, alpha0=step_size,
                  amin=MIN_STEP_SIZE)
                ...
                obj_uv, x_hat_aux, step_size = _adaptive_step_size(f,
                  obj_uv, alpha=step_size)
...
def power_iteration(lin_op, n_points=None, b_hat_0=None, max_iter=1000, tol=1e-07, random_state=None):
    ...
    if hasattr(lin_op, 'dot'):
        ...
        if callable(lin_op):
            msg = 'power_iteration require n_points argument when lin_op is callable'
