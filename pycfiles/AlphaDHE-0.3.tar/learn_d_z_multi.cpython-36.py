...
# Embedded file name: /home/tom/.local/miniconda/lib/python3.6/site-packages/alphacsc/learn_d_z_multi.py
# Compiled at: 2019-06-11 08:01:24
# Size of source mod 2**32: 21149 bytes
...
def learn_d_z_multi(X, n_atoms, n_times_atom, n_iter=60, n_jobs=1, lmbd_max='fixed', reg=0.1, loss='l2', loss_params=dict(gamma=0.1, sakoe_chiba_band=10, ordar=10), rank1=True, uv_constraint='separate', eps=1e-10, algorithm='batch', algorithm_params=dict(), detrending=None, detrending_params=dict(), solver_z='l-bfgs', solver_z_kwargs=dict(), solver_d='alternate_adaptive', solver_d_kwargs=dict(), D_init=None, D_init_params=dict(), unbiased_z_hat=False, use_sparse_z=False, stopping_pobj=None, raise_on_increase=True, verbose=10, callback=None, random_state=None, name='DL', window=False, sort_atoms=False):
    ...
    """
    if not lmbd_max in ('fixed', 'scaled', 'per_atom', 'shared'):
        raise AssertionError("lmbd_max should be in {'fixed', 'scaled', 'per_atom', 'shared'}, not '{}'".format(lmbd_max))
    else:
        ...
        D_hat = init_dictionary(X, n_atoms, n_times_atom, D_init=D_init, rank1=rank1,
        ...
        def compute_z_func(X, z_hat, D_hat, reg=None):
            return update_z_multi(X, D_hat, reg=reg, z0=z_hat, solver=solver_z,
              solver_kwargs=z_kwargs,
              loss=loss,
        ...
        d_kwargs = dict(verbose=verbose, eps=1e-08)
        ...
        def compute_d_func(X, z_hat, D_hat, constants):
            if rank1:
                return update_uv(X, z_hat, uv_hat0=D_hat, constants=constants, b_hat_0=b_hat_0, 
                 solver_d=solver_d, uv_constraint=uv_constraint, 
                 loss=loss, loss_params=loss_params, 
                 window=window, **d_kwargs)
            else:
                return update_d(X, z_hat, D_hat0=D_hat, constants=constants, b_hat_0=b_hat_0, 
                 solver_d=solver_d, uv_constraint=uv_constraint, 
                 loss=loss, loss_params=loss_params, 
                 window=window, **d_kwargs)
        ...
        end_iter_func = get_iteration_func(eps, stopping_pobj, callback, lmbd_max, name, verbose, raise_on_increase)
        kwargs = dict(X=X, D_hat=D_hat, z_hat=z_hat, compute_z_func=compute_z_func, compute_d_func=compute_d_func,
          obj_func=obj_func,
            ...
            if algorithm == 'greedy':
                ...
                if algorithm == 'online':
                    ...
                    if algorithm == 'stochastic':
                        ...
                        raise NotImplementedError("Algorithm '{}' is not implemented to learn dictionary atoms.".format(algorithm))
        if sort_atoms:
            D_hat, z_hat = sort_atoms_by_explained_variances(D_hat,
              z_hat, n_channels=n_channels)
        if unbiased_z_hat:
            start_unbiased_z_hat = time.time()
            z_hat, _, _ = update_z_multi(X,
              D_hat, reg=0, z0=z_hat, n_jobs=n_jobs, solver=solver_z, solver_kwargs=solver_z_kwargs,
              freeze_support=True,
              loss=loss,
              loss_params=loss_params)
            if verbose > 1:
                print('[{}] Compute the final z_hat with support freeze in {:.2f}s'.format(name, time.time() - start_unbiased_z_hat))
    ...
    reg *= std_X
    return (
     pobj, times, D_hat, z_hat, reg)


def _batch_learn(X, D_hat, z_hat, compute_z_func, compute_d_func, obj_func, end_iter_func, n_iter=100, lmbd_max='fixed', reg=None, verbose=0, greedy=False, random_state=None, name='batch', uv_constraint='separate', window=False):
    reg_ = reg
    constants = {}
    ...
    if greedy:
        ...
        if n_iter < n_atoms * n_iter_by_atom:
            raise ValueError('The greedy method needs at least %d iteration to learn %d atoms. Got only n_iter=%d. Please increase n_iter.' % (
    ...
    for ii in range(n_iter):
        ...
        if greedy:
            if ii % n_iter_by_atom == 0:
                if D_hat.shape[0] < n_atoms:
                    new_atom = get_max_error_dict(X, z_hat, D_hat, uv_constraint=uv_constraint,
                      window=window)[0]
                    ...
                    if lmbd_max not in ('fixed', 'scaled'):
                        ...
                        start = time.time()
                        z_hat, constants['ztz'], constants['ztX'] = compute_z_func(X,
                          z_hat, D_hat, reg=reg_)
                        times.append(time.time() - start)
                        pobj.append(obj_func(X, z_hat, D_hat, reg=reg_))
                        z_nnz, z_size = lil.get_nnz_and_size(z_hat)
                        if verbose > 5:
                            print('[{}] sparsity: {:.3e}'.format(name, z_nnz.sum() / z_size))
                            print('[{}] Objective (z) : {:.3e}'.format(name, pobj[(-1)]))
                        if np.all(z_nnz == 0):
                            import warnings
                            warnings.warn('Regularization parameter `reg` is too large and all the activations are zero. No atoms has been learned.', UserWarning)
                            break
                    start = time.time()
                    ...
                    if len(null_atom_indices) > 0:
                        ...
                        D_hat[k0] = get_max_error_dict(X, z_hat, D_hat, uv_constraint, window=window)[0]
                        if verbose > 5:
    ...
    return (
     pobj, times, D_hat, z_hat)


def _online_learn(X, D_hat, z_hat, compute_z_func, compute_d_func, obj_func, end_iter_func, n_iter=100, verbose=0, random_state=None, lmbd_max='fixed', reg=None, alpha=0.8, batch_selection='random', batch_size=1, name='online', uv_constraint='separate', window=False):
    reg_ = reg
    constants = {}
            ...
            if verbose > 1:
                ...
                if lmbd_max not in ('fixed', 'scaled'):
                    ...
                    if batch_selection == 'random':
                        ...
                        if batch_selection == 'cyclic':
                            i_slice = ii * batch_size % n_trials
                            i0 = slice(i_slice, i_slice + batch_size)
                        else:
                            raise NotImplementedError("the '{}' batch_selection strategy for the online learning is not implemented.".format(batch_selection))
...
def get_iteration_func(eps, stopping_pobj, callback, lmbd_max, name, verbose, raise_on_increase):
    ...
    def end_iteration(X, z_hat, D_hat, pobj, iteration):
        if callable(callback):
            ...
            if (dz < eps or du < eps) and lmbd_max in ('fixed', 'scaled'):
                if dz < 0:
                    if raise_on_increase:
                        raise RuntimeError('The z update have increased the objective value by {}.'.format(dz))
                else:
                    if du < -1e-10:
                        if dz > 1e-12:
                            if raise_on_increase:
                                raise RuntimeError('The d update have increased the objective value by {}.(dz={})'.format(du, dz))
                    if dz < eps and du < eps:
                        if verbose == 1:
                            print('')
                        print('[{}] Converged after {} iteration, (dz, du) = {:.3e}, {:.3e}'.format(name, iteration + 1, dz, du))
                        return True
            if stopping_pobj is not None:
                if pobj[(-1)] < stopping_pobj:
                    return True
        return False
