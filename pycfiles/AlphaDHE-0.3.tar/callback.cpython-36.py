...
# Embedded file name: /home/tom/.local/miniconda/lib/python3.6/site-packages/alphacsc/viz/callback.py
# Compiled at: 2019-06-04 04:10:26
# Size of source mod 2**32: 13475 bytes
import itertools, copy as cp, os, json, datetime, mne, numpy as np, matplotlib as mpl, matplotlib.pyplot as plt
from scipy import sparse
from .tools import get_calling_script, positive_hash
COLORS = [
 '#4C72B0', '#55A868', '#C44E52', '#8172B2', '#CCB974', '#64B5CD']
DEFAULT_CB = {'atom':{},  'z_hat':{},  'pobj':{}}
PRINT_KWARGS = {'n_atoms':'K = {}', 
 'n_times_atom':'L={}', 
 'reg':'$\\lambda$={}', 
 'solver_z':'{}'}
...
def plot_activations_density(z_hat, n_times_atom, sfreq=1.0, threshold=0.01, bandwidth='auto', axes=None, t_min=0, plot_activations=False, colors=None):
    """
    Parameters
    ----------
    ...
    if sparse.isspmatrix_lil(z_hat[0]):
        ...
        if axes is None:
            fig, axes = plt.subplots(n_atoms, num='density', figsize=(
             8, 2 + n_atoms * 3))
    ...
    for ax, activations, color in zip(axes.ravel(), z_hat_sum, colors):
        ...
        if n_elements == 0:
            ...
            if plot_activations:
                ax.plot((time_instants[selection]), (activations[selection] / activations[selection].max()),
                  '.', color='k')
            window = np.blackman(bandwidth)
            smooth_activations = np.convolve(activations, window, 'same')
            ax.fill_between(time_instants, smooth_activations, color=color, alpha=0.5)
    ...
    def _onclick(event):
        ...
        for jj in range(len(plot_types)):
            if orig_ax._plot_types[jj] == 'plot':
                ...
                if orig_ax._plot_types[jj] == 'stem':
...
def plot_callback(X, info, n_atoms, layout=None):
    ...
    fig_z, axes_z = plt.subplots(nrows=n_atoms, num='z', figsize=(10, 8), sharex=True,
      sharey=True)
    ...
    def callback(X, uv_hat, z_hat, reg):
        ...
        this_info['sfreq'] = 1.0
        patterns = mne.EvokedArray((uv_hat[:n_atoms_plot, :n_channels].T), this_info,
          tmin=0)
        patterns.plot_topomap(times=(np.arange(n_atoms_plot)), layout=layout,
          axes=axes_topo,
          time_unit='s',
          time_format='Atom%01d',
          show=False)
        if axes[0].lines == []:
        ...
        if n_trials == 1:
            ...
            extent = [
             times_z[0], times_z[(-1)], 1, len(z_hat[0])]
            for k in range(n_atoms):
                im = axes_z[k].imshow((z_hat[:, k]), cmap='hot', origin='lower', extent=extent,
                  clim=(
                 0.0, z_hat.max()),
                  aspect='auto')

            fig_z.subplots_adjust(right=0.8)
            cbar_ax = fig_z.add_axes([0.86, 0.1, 0.03, 0.8])
...
def get_callback_csc(csc_kwargs, config=DEFAULT_CB, info={}):
    ...
    ncols = int(np.ceil(n_atoms / 5.0))
    nrows = min(5, n_atoms)
    plt.close('all')
    figs = {}
    for name, conf in config.items():
        if not name in ('atom', 'z_hat', 'Xhat', 'topo', 'pobj'):
            ...
            if name == 'pobj':
                nc = nr = 1
                width = 10
            f, axes = plt.subplots(nrows=nr, ncols=nc, num=fname, figsize=(
             width * nc, 10),
              squeeze=False,
              sharex=shared_axes,
              sharey=shared_axes)
            text = [PRINT_KWARGS.get(k, '{}={{}}'.format(k)).format(v) for k, v in csc_kwargs.items() if k in PRINT_KWARGS]
            text = '\t'.join(text)
    ...
    def callback(X, uv_hat, z_hat, pobj):
        ...
        if 'topo' in figs:
            ...
            for idx, ax in enumerate(axes.ravel()):
                ax.clear()
                mne.viz.plot_topomap((uv_hat[idx, :n_channels]), info, axes=ax,
                  show=False)
        ...
        if 'atom' in figs:
            ...
            if csc_kwargs.get('rank1', True):
                ...
                plot_or_replot([uv_hat[(k, i0[k])] for k in range(n_atoms)], axes)
        if 'Xhat' in figs:
            X_hat = [np.convolve(z_k, uvk[n_channels:]) for z_k, uvk in zip(z_hat[0], uv_hat)]
            axes = figs['Xhat'][1].ravel()
            plot_or_replot(X_hat, axes)
        if 'z_hat' in figs:
            axes = figs['z_hat'][1].ravel()
            plot_activations_density(z_hat, n_times_atom, sfreq=sfreq, plot_activations=False,
              axes=axes,
              bandwidth='auto',
              t_min=t_min)
        if 'pobj' in figs:
            ...
            cost = np.array([pobj])
            plot_or_replot(cost - np.min(cost) + 1e-06, axes)
