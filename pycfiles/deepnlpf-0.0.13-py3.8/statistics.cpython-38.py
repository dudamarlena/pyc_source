...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/deepnlpf/statistics.py
# Compiled at: 2020-04-16 03:51:39
...
import deepnlpf.models.statistics as s
...
class Statistics(object):
    ...
    def generate_sentencelist(self):
        ...
        """
        for sentence in self._documents['sentences']:
            self.list_sentences.append(PreProcessing().remove_special_characters(sentence))
        else:
    ...
    def frequency_of_words(self):
        ...
        for word in self.word_list:
            if word.lower() is not wordlistfreq:
                item = {'word':word.lower(),  'freq':self.word_list.count(word)}
    ...
    def full_statistics(self, save=False):
        ...
        document = {'_id_dataset':self._id_dataset, 
    ...
    def hist_token_frequency_by_class_grammtical(self):
        ...
        for item in Counter(self.split_pos_tag(self.list_sentences)).items():
            ...
            fig.update_layout(title_text='Frequency of Tokens to Grammatical Class',
    ...
    def hist_token_frequency_by_sentences(self, _title_text='Number Tokens for Sentences', _xaxis_title_text='Number Tokens.', _yaxis_title_text='Sentences Frequency'):
        ...
        """
        fig = go.Figure(data=[
         go.Histogram(x=(self.cont_tokens_sentence(self.list_sentences)))])
        fig.update_layout(title_text=_title_text,
    ...
    def word_cloud(self, quant_word=50):
        ...
        for word in self.word_list:
            ...
            colors = [plotly.colors.DEFAULT_PLOTLY_COLORS[random.randrange(1, 10)] for i in range(quant_word)]
            weights = [random.randint(15, 35) for i in range(quant_word)]
            data = go.Scatter(x=[random.random() for i in range(quant_word)], y=[random.random() for i in range(quant_word)],
              mode='text',
              text=words,
              marker={'opacity': 0.3},
              textfont={'size':weights, 
             'color':colors})
            layout = go.Layout({'xaxis':{'showgrid':False,  'showticklabels':False,  'zeroline':False},  'yaxis':{'showgrid':False, 
