...
# [GCC 8.4.0]
# Embedded file name: /home/vilmos/Projects/python-iptables/iptc/ip6tc.py
# Compiled at: 2018-04-16 20:15:05
import ctypes as ct, socket
...
class ip6t_ip6(ct.Structure):
    ...
    def __init__(self):
        self.smsk.s6_addr = self.dmsk.s6_addr = 4080
...
_libiptc, _ = find_library('ip6tc', 'iptc')
...
class Rule6(Rule):
    ...
    def __eq__(self, rule):
        ...
        if set(rule._matches) != set([ x for x in rule._matches if x in self._matches
            ...
            return False
        if self.src == rule.src and self.dst == rule.dst and self.protocol == rule.protocol and self.in_interface == rule.in_interface and self.out_interface == rule.out_interface:
            return True
        return False
    ...
    def get_src(self):
        ...
        if self.entry.ipv6.invflags & ip6t_ip6.IP6T_INV_SRCIP:
            src = ('').join([src, '!'])
        try:
            addr = socket.inet_ntop(socket.AF_INET6, self.entry.ipv6.src.s6_addr)
        except socket.error:
    ...
    def _addr2in6addr(self, addr):
        ...
        ina = in6_addr()
        try:
            ina.s6_addr = arr.from_buffer_copy(socket.inet_pton(socket.AF_INET6, addr))
        except socket.error:
    ...
    def set_src(self, src):
        if src[0] == '!':
            ...
            self.entry.ipv6.invflags &= ~ip6t_ip6.IP6T_INV_SRCIP & ip6t_ip6.IP6T_INV_MASK
        ...
        neta = in6_addr()
        try:
            neta.s6_addr = arr.from_buffer_copy(socket.inet_pton(socket.AF_INET6, netm))
        except socket.error:
    ...
    def get_dst(self):
        ...
        if self.entry.ipv6.invflags & ip6t_ip6.IP6T_INV_DSTIP:
            dst = ('').join([dst, '!'])
        try:
            addr = socket.inet_ntop(socket.AF_INET6, self.entry.ipv6.dst.s6_addr)
        except socket.error:
            raise IPTCError('error in internal state: invalid address')
    ...
    def set_dst(self, dst):
        if dst[0] == '!':
            ...
            self.entry.ipv6.invflags &= ~ip6t_ip6.IP6T_INV_DSTIP & ip6t_ip6.IP6T_INV_MASK
    ...
    def set_in_interface(self, intf):
        if intf[0] == '!':
            ...
            self.entry.ipv6.invflags &= ~ip6t_ip6.IP6T_INV_VIA_IN & ip6t_ip6.IP6T_INV_MASK
    ...
    def get_out_interface(self):
        ...
        mask = bytearray(_IFNAMSIZ)
        mask[:(len(self.entry.ipv6.outiface_mask))] = self.entry.ipv6.outiface_mask
        if mask[0] == 0:
            return None
        else:
            for i in range(_IFNAMSIZ):
    ...
    def set_out_interface(self, intf):
        if intf[0] == '!':
            ...
            self.entry.ipv6.invflags &= ~ip6t_ip6.IP6T_INV_VIA_OUT & ip6t_ip6.IP6T_INV_MASK
        if len(intf) >= _IFNAMSIZ:
            raise ValueError('interface name %s too long' % intf)
        masklen = len(intf) + 1
        if intf[(len(intf) - 1)] == '+':
    ...
    def set_protocol(self, proto):
        ...
        if proto[0] == '!':
            ...
            self.entry.ipv6.flags &= ~ip6t_ip6.IP6T_F_PROTO & ip6t_ip6.IP6T_F_MASK
            proto = proto[1:]
        else:
            self.entry.ipv6.invflags &= ~ip6t_ip6.IP6T_INV_PROTO & ip6t_ip6.IP6T_INV_MASK
