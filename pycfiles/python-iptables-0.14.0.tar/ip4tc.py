...
# [GCC 8.4.0]
# Embedded file name: /home/vilmos/Projects/python-iptables/iptc/ip4tc.py
# Compiled at: 2019-03-23 18:52:17
import os, re, shlex, sys, ctypes as ct, socket, struct, weakref
from .util import find_library, load_kernel
from .xtables import XT_INV_PROTO, NFPROTO_IPV4, NFPROTO_IPV6, XTablesError, xtables, xt_align, xt_counters, xt_entry_target, xt_entry_match
__all__ = [
 'Table', 'Chain', 'Rule', 'Match', 'Target', 'Policy', 'IPTCError']
try:
...
class ipt_ip(ct.Structure):
    ...
    def __init__(self):
        self.smsk.s_addr = self.dmsk.s_addr = 4294967295
...
class ipt_entry(ct.Structure):
    ...
    _fields_ = [
     ...
     (
      'nfcache', ct.c_uint),
     (
      'target_offset', ct.c_uint16),
     (
      'next_offset', ct.c_uint16),
     (
      'comefrom', ct.c_uint),
     (
...
_libiptc, _ = find_library('ip4tc', 'iptc')
    ...
    def parse(self, parameter, value):
        ...
        if is_str:
            ...
            try:
                ...
                raise TypeError('Invalid parameter value: must be string or list of strings')
...
class Match(IPTCModule):
    ...
    *target.set_tos*, *target.and_tos*, *target.or_tos* and *target.xor_tos*,
    ...
    def __eq__(self, match):
        ...
        if self.match.u.match_size == match.match.u.match_size and self.match.u.user.name == match.match.u.user.name and self.match.u.user.revision == match.match.u.user.revision and self.match_buf[basesz:self.usersize] == match.match_buf[basesz:match.usersize]:
            return True
        return False
    ...
    def _parse(self, argv, inv, entry):
        self._xt.parse_match(argv, inv, self._module, entry, ct.cast(self._ptrptr, ct.POINTER(ct.c_void_p)), self._orig_parse, self._orig_options)
    ...
    def _update_pointers(self):
        self._ptr = ct.cast(ct.byref(self._match_buf), ct.POINTER(xt_entry_match))
        self._ptrptr = ct.cast(ct.pointer(self._ptr), ct.POINTER(ct.POINTER(xt_entry_match)))
...
class Target(IPTCModule):
    ...
    *target.set_tos*, *target.and_tos*, *target.or_tos* and *target.xor_tos*,
        ...
        if goto is not None:
            if not isinstance(goto, bool):
                ...
                if goto:
                    ...
                    ipstruct.flags &= ~_F_GOTO
            self._xt = xtables(rule.nfproto)
            module = self._is_standard_target() and self._xt.find_target('') or self._xt.find_target(name)
            real_name = module and getattr(module[0], 'real_name', None) or None
            if real_name:
                self._name = real_name.decode()
                self._orig_parse = getattr(module[0], 'x6_parse', None)
            ...
            raise (module or XTablesError)("can't find target %s" % name)
        self._module = module[0]
    ...
    def __eq__(self, targ):
        ...
        if self.target.u.target_size != targ.target.u.target_size or self.target.u.user.name != targ.target.u.user.name or self.target.u.user.revision != targ.target.u.user.revision:
            return False
        if self.target.u.user.name == '' or self.target.u.user.name == 'standard' or self.target.u.user.name == 'ACCEPT' or self.target.u.user.name == 'DROP' or self.target.u.user.name == 'RETURN' or self.target.u.user.name == 'ERROR' or self._is_standard_target():
            return True
        if self._target_buf[basesz:self.usersize] == targ._target_buf[basesz:targ.usersize]:
            return True
        return False
    ...
    def _parse(self, argv, inv, entry):
        self._xt.parse_target(argv, inv, self._module, entry, ct.cast(self._ptrptr, ct.POINTER(ct.c_void_p)), self._orig_parse, self._orig_options)
    ...
    def _update_pointers(self):
        ...
        self._ptrptr = ct.cast(ct.pointer(self._ptr), ct.POINTER(ct.POINTER(xt_entry_target)))
...
class Rule(object):
    ...
    Each rule has three parts:
        ...
        * One target.  This determines what happens with the packet if it is
          matched.
    """
    protocols = {0: 'all', socket.IPPROTO_AH: 'ah', 
    ...
    def __eq__(self, rule):
        ...
        if len(self._matches) != len(rule._matches):
            return False
        if set(rule._matches) != set([ x for x in rule._matches if x in self._matches
                                     ]):
            return False
        if self.src == rule.src and self.dst == rule.dst and self.protocol == rule.protocol and self.fragment == rule.fragment and self.in_interface == rule.in_interface and self.out_interface == rule.out_interface:
            return True
        return False
    ...
    def set_src(self, src):
        if src[0] == '!':
            ...
            self.entry.ip.invflags &= ~ipt_ip.IPT_INV_SRCIP & ipt_ip.IPT_INV_MASK
        slash = src.find('/')
        if slash == -1:
            addr = src
    ...
    def set_dst(self, dst):
        if dst[0] == '!':
            ...
            self.entry.ip.invflags &= ~ipt_ip.IPT_INV_DSTIP & ipt_ip.IPT_INV_MASK
        slash = dst.find('/')
        if slash == -1:
            addr = dst
            netm = '255.255.255.255'
    ...
    def set_in_interface(self, intf):
        if intf[0] == '!':
            ...
            self.entry.ip.invflags &= ~ipt_ip.IPT_INV_VIA_IN & ipt_ip.IPT_INV_MASK
        ...
        if intf[(len(intf) - 1)] == '+':
            intf = intf[:-1]
            masklen -= 2
        self.entry.ip.iniface = ('').join([intf.encode(),
         '\x00' * (_IFNAMSIZ - len(intf))])
        self.entry.ip.iniface_mask = ('').join([b'\xff' * masklen,
         '\x00' * (_IFNAMSIZ - masklen)])
    ...
    def set_out_interface(self, intf):
        if intf[0] == '!':
            ...
            self.entry.ip.invflags &= ~ipt_ip.IPT_INV_VIA_OUT & ipt_ip.IPT_INV_MASK
        if len(intf) >= _IFNAMSIZ:
            raise ValueError('interface name %s too long' % intf)
        masklen = len(intf) + 1
        if intf[(len(intf) - 1)] == '+':
            intf = intf[:-1]
            masklen -= 2
        self.entry.ip.outiface = ('').join([intf.encode(),
         '\x00' * (_IFNAMSIZ - len(intf))])
        self.entry.ip.outiface_mask = ('').join([b'\xff' * masklen,
         '\x00' * (_IFNAMSIZ - masklen)])
    ...
    def set_protocol(self, proto):
        ...
        if proto[0] == '!':
            ...
            self.entry.ip.invflags &= ~ipt_ip.IPT_INV_PROTO & ipt_ip.IPT_INV_MASK
        if proto.isdigit():
    ...
    def _set_rule(self, entry):
        if not entry:
            ...
            return
        self.entry = ct.cast(ct.pointer(entry), ct.POINTER(self._entry_type()))[0]
        ...
        if matchsz:
            ...
            while entrysz + off < entry.target_offset:
                match = ct.cast(ct.byref(entry.elems, off), ct.POINTER(xt_entry_match))[0]
                m = Match(self, match=match)
                self.add_match(m)
                off += m.size

        target = ct.cast(ct.byref(entry, entry.target_offset), ct.POINTER(xt_entry_target))[0]
        self.target = Target(self, target=target)
        jump = self.chain.table.get_target(entry)
        if jump:
    ...
    def to_dict(self):
        ...
        if self.nfproto == NFPROTO_IPV4 and self.dst != '0.0.0.0/0.0.0.0':
            ...
            if self.nfproto == NFPROTO_IPV6 and self.dst != '::/0':
                d['dst'] = self.dst
            if self.protocol != 'ip':
...
class Table(object):
    ...
    *libiptc* API calls one by one, and take low-level iptables structs as
    ...
    SECURITY = 'security'
    ALL = [
     'filter', 'mangle', 'raw', 'nat', 'security']
    _cache = dict()
    ...
    def get_policy(self, chain):
        ...
        if not self.builtin_chain(chain):
            ...
            cntrs = xt_counters()
            pol = self._iptc.iptc_get_policy(chain.encode(), ct.pointer(cntrs), self._handle).decode()
            if not pol:
                raise IPTCError("can't get policy on chain %s: %s" % (
                 chain, self.strerror()))
            return (
             Policy(pol), (cntrs.pcnt, cntrs.bcnt))

    @autocommit
    def append_entry(self, chain, entry):
        """Appends rule *entry* to *chain*."""
        rv = self._iptc.iptc_append_entry(chain.encode(), ct.cast(entry, ct.c_void_p), self._handle)
        if rv != 1:
    ...
    def insert_entry(self, chain, entry, position):
        ...
        rv = self._iptc.iptc_insert_entry(chain.encode(), ct.cast(entry, ct.c_void_p), position, self._handle)
        if rv != 1:
    ...
    def replace_entry(self, chain, entry, position):
        ...
        rv = self._iptc.iptc_replace_entry(chain.encode(), ct.cast(entry, ct.c_void_p), position, self._handle)
        if rv != 1:
    ...
    def delete_entry(self, chain, entry, mask):
        ...
        rv = self._iptc.iptc_delete_entry(chain.encode(), ct.cast(entry, ct.c_void_p), mask, self._handle)
        if rv != 1:
