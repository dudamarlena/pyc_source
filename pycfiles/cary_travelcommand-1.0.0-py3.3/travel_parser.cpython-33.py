...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cary_travelcommand/travel_parser.py
# Compiled at: 2015-08-06 07:12:06
    ...
    def add_airports_from_file(self, filename):
        ...
        with open(filename) as (f):
            ...
            for row in r:
                identifier = row['code']
                if identifier != '':
                    self.airports[identifier] = dict(name=row['name'], city=row['city'], state=row['state'], country=row['country'])
                    continue
        ...
        with open(filename) as (f):
            for r in f.readlines():
                if len(r) == 290:
                    ...
                    city = r[144:204]
                    if id != '':
                        self.stations[id] = dict(city=city, name=city, country='United Kingdom')
                    else:
                        continue
...
TEST_AIRPORTS = dict(LHR=dict(city='London', name='London Heathrow', country='United Kingdom'), DUB=dict(city='Dublin', name='Dublin', country='Ireland'), IAD=dict(city='Washington', name='Dulles International', country='United States'))
TestAirportLookup = AirportLookup(from_dict=TEST_AIRPORTS)
TEST_TRAINS = dict(KGX=dict(city='London', name='Kings Cross', country='United Kingdom'), CBG=dict(city='Cambridge', name='Cambridge', country='United Kingdom'), PAD=dict(city='London', name='Paddington', country='United Kingdom'), OXF=dict(city='Oxford', name='Oxford', country='United Kingdom'))
TestTrainLookup = TrainStationLookup(from_dict=TEST_TRAINS)

def route_parser(lookups):
    loc = Word(alphas, min=3, max=3)
    loc.setParseAction(lambda s, l, t: [tok.upper() for tok in t])
    route_connector = oneOf('- ->')
    route = loc.setResultsName('start') + Optional(route_connector).suppress() + loc.setResultsName('end')
    ...
    route.setParseAction(lambda s, l, t: dict(start=t.start, end=t.end, mode=route_mode(t.start, t.end)))
    return route
...
def year_guess_today(guess_month, guess_day):
    return year_guess(datetime.date.today(), guess_month, guess_day)
...
def date_parser():
    ...
    date_v1 = year.setResultsName('year') + Optional(datesep).suppress() + nummonth.setResultsName('month') + Optional(datesep).suppress() + day.setResultsName('day')
    date_v2 = day.setResultsName('day') + month_parser().setResultsName('month') + Optional(year).setResultsName('year')
...
def date_range_parser():
    ...
    date_range_v1 = date_parser().setResultsName('start') + Optional(dates_separator.suppress() + date_parser().setResultsName('end'))
    ...
    def date_range_v2_parse(s, l, t):
        try:
            ...
            return dict(start=datetime.date(year, start_month, t.start_day), end=datetime.date(year, t.end_month, end_day))
        except Exception as e:
            raise e

    day = Word(nums).setParseAction(strToInt)
    year = Word(nums).setParseAction(strToInt)
    date_range_v2 = day.setResultsName('start_day') + Optional(month_parser().setResultsName('start_month')) + Optional(dates_separator).suppress() + Optional(day).setResultsName('end_day') + month_parser().setResultsName('end_month') + Optional(year).setResultsName('year')
...
def trip_leg_parser(lookups):
    anyloc = OneOrMore(Word(alphas)) + Optional(OneOrMore(CaselessLiteral(',').suppress() + OneOrMore(Word(alphas))))
    anyloc.setParseAction(lambda s, l, t: ', '.join(t))
    trip_leg = Optional(route_parser(lookups).setResultsName('route')) + date_range_parser().setResultsName('dates') + Optional(Optional('staying').suppress() + CaselessLiteral('in').suppress() + anyloc.setResultsName('staying_in'))
