...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cary_travelcommand/travel_calculations.py
# Compiled at: 2015-08-04 10:26:55
...
def cost_for_day(day, season):
    if season is not None:
        return dict(day=day, lodging=int(season['lodging']), lodging_multiplier=1.0, meals=int(season['meals']), incidentals=int(season['incidentals']), mie_multiplier=1.0)
    else:
        return
        return
...
def perdiem_costs_by_query(days, query):
    if not query['found']:
        result = dict(travel_cost=None, matched_location=None)
    else:
        topmatch = query['closest_matches'][0]
        result = dict(travel_cost=None, matched_location=topmatch['location'], dates=[cost_for_day(day, season_for_day(day, topmatch['seasons'])) for day in days])
    return result
...
def trip_costs_by_perdiems(trip, perdiems):
    ...
    def calc_legs(trip, perdiems):
        return [dict(route=leg['route'], dates=leg['dates'], staying_in=leg['staying_in'], costs=perdiem_costs_by_query(days_from_leg(leg), perdiem)) for leg, perdiem in zip(trip, perdiems)]
...
def trip_costs(trip, pd_db):
    ...
    legs = trip_costs_by_perdiems(trip, perdiems)
    return legs
    ...
    def route_costs(leg, estimators):
        for est in estimators:
            ...
            if est.services(route['mode']):
                return est.single_estimate(route['start'], route['end'], leg['dates']['start'])
    ...
    def set_actual_perdiem_costs(leg):
        if leg['costs']['matched_location'] is not None:
            for day in leg['costs']['dates']:
                day['lodging_actual'] = day['lodging'] * day['lodging_multiplier']
                day['mie_actual'] = (day['meals'] + day['incidentals']) * day['mie_multiplier']
