...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/app_plugins/management/commands/sync_plugins.py
# Compiled at: 2009-03-30 22:54:40
...
class Command(NoArgsCommand):
    option_list = NoArgsCommand.option_list + (
     make_option('--delete', action='store_true', dest='delete', help='delete the REMOVED Plugin and PluginPoint instances. '),)
    help = 'Syncs the registered plugin and plugin points with the model versions.'
...
def sync_app_plugins(delete_removed=False, verbosity=1):
    ...
    for (app_label, lib) in libraries.iteritems():
        for label in lib.plugins:
            ...
            default = construct_template_path(lib.app_name, point_label, options.get('ext', '.html'))
    ...
    for pp in PluginPoint.objects.exclude(status=REMOVED):
        ...
        for app in settings.INSTALLED_APPS:
            ...
            elif p.status == REMOVED and bFound:
                p.status = ENABLED
                p.template = template
                p.registered = False
