...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/app_plugins/templatetags/app_plugins.py
# Compiled at: 2008-07-22 01:32:02
...
APP_PLUGINS_CACHE_PARAMS = getattr(settings, 'APP_PLUGINS_CACHE_PARAMS', {'cull_frequency': 4, 'max_entries': 3000, 
   'timeout': 259200})
...
def callback(func, variables, context, takes_context):
    ...
    for variable in variables:
        ...
        if found_kwd:
            try:
                ...
                raise TemplateSyntaxError("Expected keyword assignemnt, found '%s' instead" % variable.var)
            else:
                if params and not varkw and name not in params:
                    raise TemplateSyntaxError("%s got an unexpected keyword argument '%s'" % (name, var))
                if var in kwdargs:
                    raise TemplateSyntaxError("%s got multiple values for keyword argument '%s'" % (
                     name, var))
    ...
    if num_args > num_params and not varargs or num_all > num_params and not varkw:
    ...
    if num_args != num_req:
        if num_args > num_req:
            if not varargs:
                ...
                if not_allowed:
                    raise TemplateSyntaxError('%s got multiple values for keyword arguments: %s' % (
                     name, (', ').join(not_allowed)))
        elif not varkw:
            required = set(params[num_args:-num_default])
            missing = required - set(kwdargs)
            if missing:
                raise TemplateSyntaxError('%s takes at least %s non-keyword arguments (%s given)' % (
...
def inclusion_kwdtag(register, file_name, context_class=Context, takes_context=False):
    ...
    def dec(func):
        ...
        class InclusionKwdNode(Node):
            ...
            def render(self, context):
                ...
                res = self.nodelist.render(context_class(new_context, autoescape=context.autoescape))
                context.pop()
                context.pop()
                context.pop()
                return res
...
def app_plugin(context, app, name, plugin=None, user=None, args=None, ext='.html', **extra_args):
    validate_name(app)
    validate_name(name)
    if plugin is None:
        try:
            plugin = models.Plugin.objects.get(label=('.').join([app, name]))
...
def plugin_point(context, name, point=None, user=None, ext='.html', **args):
    ...
    if point is None:
        ...
        if apps is None:
            tpls = ((app, models.construct_template_path(app, name, ext)) for app in app_names)
