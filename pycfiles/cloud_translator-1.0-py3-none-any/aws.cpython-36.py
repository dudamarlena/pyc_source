...
# Embedded file name: build\bdist.win-amd64\egg\cloud_training\aws.py
# Compiled at: 2017-10-30 16:57:22
# Size of source mod 2**32: 5968 bytes
import base64, datetime, json, logging, subprocess
...
class Aws(object):
    ...
    def spot_price(self, instance_type: str) -> dict:
        ...
        prices = {}
        if res:
            if res['SpotPriceHistory']:
                for zone in res['SpotPriceHistory']:
                    prices[zone['AvailabilityZone']] = zone['SpotPrice']

        return prices

    def create_spot_request(self, instance_type: str, image_id: str, root_snapshot_id: str, root_vol_size: int, training_vol_size: int, user_data: str, key_name: str, spot_price: float):
        ...
        tmp_file.close()
        args = [
         'ec2', 'request-spot-instances', '--spot-price', '%.3f' % spot_price, '--type', 'one-time',
    ...
    def cancel_spot_request(self, request_id: str=None):
        ...
        res = self.run(args)
        if not res or not res['CancelledSpotInstanceRequests'] or res['CancelledSpotInstanceRequests'][0]['State'] != 'cancelled':
            return False
        else:
            return True
