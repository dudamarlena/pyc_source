...
# Embedded file name: build\bdist.win-amd64\egg\cloud_training\configure.py
# Compiled at: 2017-10-29 21:39:00
...
def get_aws_credentials_settings(profile: str='default'):
    ...
    return get_profile_settings(get_aws_credentials_file_path(), get_aws_profile_name(profile), {'aws_access_key_id':None, 
     'aws_secret_access_key':None})


def get_aws_config_settings(profile: str='default'):
    """Returns AWS CLI settings from the "config" file for a specific profile."""
    return get_profile_settings(get_aws_config_file_path(), get_aws_profile_name(profile), {'region': None})


def get_ct_settings(profile: str='default'):
    """Returns "CloudTraining" settings for a specific profile."""
    return get_profile_settings(get_ct_config_file_path(), profile, {'s3_bucket':None, 
     'image_id':None, 
...
def get_all_settings(profile: str='default'):
    ...
    return {**(get_aws_credentials_settings(profile)), **(get_aws_config_settings(profile)), **(get_ct_settings(profile))}
