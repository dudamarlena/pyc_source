...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/shuyucms/core/management/commands/collecttemplates.py
...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
     ...
     make_option(b'-t', b'--template', dest=b'template', help=b'The template name and relative path of a single template to copy, eg: blog/blog_post_list.html'),
    ...
    def handle(self, *apps, **options):
        ...
        if apps:
            ...
            apps = [ a for a in settings.INSTALLED_APPS if a.split(b'.')[0] in ('shuyucms',
                                                                                'cartridge')
                   ]
        for app in apps:
