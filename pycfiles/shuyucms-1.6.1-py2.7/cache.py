...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/shuyucms/utils/cache.py
# Compiled at: 2016-05-20 23:26:46
...
from shuyucms.utils.device import device_from_request, hostid_from_request
...
def cache_installed():
    ...
    return has_key and settings.CACHES and not settings.TESTING and set(('shuyucms.core.middleware.UpdateCacheMiddleware',
                                                                         'shuyucms.core.middleware.FetchFromCacheMiddleware')).issubset(set(settings.MIDDLEWARE_CLASSES))


def cache_key_prefix(request):
    """
    Cache key for shuyucms's cache middleware.
    """
    cache_key = b'%s.%s.%s.' % (
