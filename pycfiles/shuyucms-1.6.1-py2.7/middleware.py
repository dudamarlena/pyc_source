...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/shuyucms/core/middleware.py
...
from shuyucms.core.management import DEFAULT_USERNAME, DEFAULT_PASSWORD
from shuyucms.utils.cache import cache_key_prefix, nevercache_token, cache_get, cache_set, cache_installed
from shuyucms.utils.device import templates_for_device, templates_for_host
...
class _Deprecated(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        msg = b'shuyucms.core.middleware.%s is deprecated.' % self.old
        if self.new:
            msg += b' Please change the MIDDLEWARE_CLASSES setting to use shuyucms.core.middleware.%s' % self.new
        warn(msg)
...
class AdminLoginInterfaceSelectorMiddleware(object):
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        login_type = request.POST.get(b'shuyucms_login_interface')
...
class UpdateCacheMiddleware(object):
    """
    Response phase for shuyucms's cache middleware. Handles caching
...
class FetchFromCacheMiddleware(object):
    """
    Request phase for shuyucms cache middleware. Return a response
...
class RedirectFallbackMiddleware(object):
    ...
    shuyucms's approach for determining the current site.
    """
