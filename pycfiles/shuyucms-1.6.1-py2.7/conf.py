...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/shuyucms/utils/conf.py
# Compiled at: 2016-05-25 12:12:19
...
def set_dynamic_settings(s):
    ...
    add_to_builtins(b'shuyucms.template.loader_tags')
    ...
    if s[b'TESTING']:
        ...
        if s.get(b'USE_SOUTH'):
            ...
            if b'compressor' in s[b'INSTALLED_APPS']:
                ...
                s[b'COMPRESS_OFFLINE_CONTEXT'][b'settings'] = shuyucms_settings
            if b'shuyucms.accounts' in s[b'INSTALLED_APPS']:
                auth_backend = b'shuyucms.core.auth_backends.shuyucmsBackend'
            ...
            if b'shuyucms.blog' in s[b'INSTALLED_APPS']:
                append(b'INSTALLED_APPS', b'shuyucms.generic')
            if b'shuyucms.generic' in s[b'INSTALLED_APPS']:
                s.setdefault(b'COMMENTS_APP', b'shuyucms.generic')
                append(b'INSTALLED_APPS', b'django.contrib.comments')
            try:
                move(b'INSTALLED_APPS', b'shuyucms.boot', 0)
            except ValueError:
                pass
