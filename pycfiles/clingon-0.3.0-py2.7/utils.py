...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/clingon/utils.py
# Compiled at: 2016-09-05 07:34:59
from builtins import input as user_input
import glob, inspect, os.path, sys
...
class AreYouSure(object):
    """This class implements a binary (True or False) oracle
       ...
       or always False.
    """
    french_defaults = dict(message='Etes-vous sûr', yes=('oui', 'o'), yes_default='non', default='défault', all_yes=('TOUS', ), all_no=('AUCUN', ))
    spanish_defaults = dict(message='¿Estás seguro', yes=('sí', 's'), yes_default='no', default='defecto', all_yes=('TODOS', ), all_no=('NINGUN', ))

    @auto_update_attrs_from_kwargs
    def __init__(self, message='Are you sure', output='stdout', yes=('yes', 'y'), yes_ignore_case=True, yes_default='no', default='default', all_yes=('ALL', ), all_no=('NONE', ), all_ignore_case=False):
        self.reset_all()
...
def read_configuration(file, path=None):
    ...
    if ext == '.py':
        ...
        with open(file, 'rb') as (f):
            exec compile(f.read(), file, 'exec') in {}, options
