...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\vis2048\vis.py
# Compiled at: 2018-10-28 10:37:28
...
class GameVis(tkinter.Frame):
    ...
    def __init__(self, game=None, grid_padding=5, font_size=40, font_name='Verdana', font_style='bold', **kwargs):
        """
        ...
        for i in range(self.game.width):
            for j in range(self.game.width):
                ...
                if new_number is None:
                    ...
                    self.grid_cells[i][j].configure(text=(str(new_number)), bg=(BG_COLOR_TILE[new_number]), fg=(FG_COLOR_DICT[new_number]))
    ...
    def _show_msg(self, msg):
        ...
        msg_lbl.place(relx=0.5, rely=0.5, anchor=(tkinter.CENTER))
    ...
    def _init_vis(self):
        ...
        for i in range(width):
            ...
            for j in range(width):
                ...
                t = tkinter.Label(cell, text='', bg=(BG_COLOR_TILE[None]), justify=(tkinter.CENTER), font=(self.font),
                  width=4,
                  height=2)
                t.grid()
                grid_row.append(t)
    ...
    def _key_down(self, func):
        ...
        def handler(event):
            for gamevis in self.all_gamevis:
                if not gamevis.game.won():
                    if gamevis.game.stuck():
                        pass
                    else:
                        func(gamevis)
