...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/djangopypi/models.py
...
class PackageInfoField(models.Field):
    description = 'Python Package Information Field'
...
class Package(models.Model):
    name = models.CharField(max_length=255, unique=True, primary_key=True, editable=False)
    auto_hide = models.BooleanField(default=True, blank=False)
    allow_comments = models.BooleanField(default=True, blank=False)
    owners = models.ManyToManyField(User, blank=True, related_name='packages_owned')
    maintainers = models.ManyToManyField(User, blank=True, related_name='packages_maintained')
...
class Release(models.Model):
    ...
    @property
    def summary(self):
        return self.package_info.get('summary', '')

    @property
    def description(self):
        return self.package_info.get('description', '')
    ...
    @models.permalink
    def get_absolute_url(self):
        return ('djangopypi-release', (),
         {'package': self.package.name, 'version': self.version})


class Distribution(models.Model):
    release = models.ForeignKey(Release, related_name='distributions', editable=False)
    ...
    pyversion = models.CharField(max_length=16, blank=True, choices=settings.DJANGOPYPI_PYTHON_VERSIONS)
...
class Review(models.Model):
    ...
    class Meta:
        verbose_name = _('release review')
        verbose_name_plural = _('release reviews')
...
class MirrorLog(models.Model):
    ...
    releases_added = models.ManyToManyField(Release, blank=True, related_name='mirror_sources')
