...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/djangopypi/decorators.py
...
def user_maintains_package(login_url=None, redirect_field_name=REDIRECT_FIELD_NAME):
    ...
    def decorator(view_func):
        ...
        def _wrapped_view(request, package, *args, **kwargs):
            if request.user.is_authenticated() and (request.user.packages_owned.filter(name=package).count() > 0 or request.user.packages_maintained.filter(name=package).count() > 0):
                return view_func(request, package=package, *args, **kwargs)
            path = urlquote(request.get_full_path())
            tup = (login_url, redirect_field_name, path)
            return HttpResponseRedirect('%s?%s=%s' % tup)
