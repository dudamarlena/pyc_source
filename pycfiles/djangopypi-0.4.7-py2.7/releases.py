...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/djangopypi/views/releases.py
...
def details(request, package, version, **kwargs):
    ...
    return render_to_response(kwargs['template_name'], kwargs['extra_context'], context_instance=RequestContext(request), mimetype=kwargs['mimetype'])
...
def manage_metadata(request, package, version, **kwargs):
    ...
    if not release:
        raise Http404('Version %s does not exist for %s' % (version,
         package))
    if release.metadata_version not in settings.DJANGOPYPI_METADATA_FORMS:
        raise Http404()
    kwargs['extra_context'][kwargs['template_object_name']] = release
...
def manage_files(request, package, version, **kwargs):
    ...
    if request.method == 'POST':
        formset = kwargs['formset_factory'](data=request.POST, files=request.FILES, **kwargs['formset_kwargs'])
        if formset.is_valid():
            formset.save()
    ...
    kwargs['extra_context'].setdefault('upload_form', kwargs['upload_form_factory']())
    return render_to_response(kwargs['template_name'], kwargs['extra_context'], context_instance=RequestContext(request), mimetype=kwargs['mimetype'])
...
def upload_file(request, package, version, **kwargs):
    ...
    kwargs.setdefault('post_save_redirect', reverse('djangopypi-release-manage-files', kwargs={'package': package, 'version': version}))
