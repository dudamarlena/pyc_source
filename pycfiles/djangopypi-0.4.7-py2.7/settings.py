...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/djangopypi/settings.py
...
if not hasattr(settings, 'DJANGOPYPI_OS_NAMES'):
    settings.DJANGOPYPI_OS_NAMES = (('aix', 'AIX'),
     ('beos', 'BeOS'),
     ('debian', 'Debian Linux'),
     ('dos', 'DOS'),
     ('freebsd', 'FreeBSD'),
...
if not hasattr(settings, 'DJANGOPYPI_ARCHITECTURES'):
    settings.DJANGOPYPI_ARCHITECTURES = (('alpha', 'Alpha'),
     ('hppa', 'HPPA'),
     ('ix86', 'Intel'),
     ('powerpc', 'PowerPC'),
     ('sparc', 'Sparc'),
...
if not hasattr(settings, 'DJANGOPYPI_DIST_FILE_TYPES'):
    settings.DJANGOPYPI_DIST_FILE_TYPES = (('sdist', 'Source'),
...
if not hasattr(settings, 'DJANGOPYPI_PYTHON_VERSIONS'):
    settings.DJANGOPYPI_PYTHON_VERSIONS = (('any', 'Any i.e. pure python'),
     ('2.1', '2.1'),
     ('2.2', '2.2'),
     ('2.3', '2.3'),
     ('2.4', '2.4'),
...
if not hasattr(settings, 'DJANGOPYPI_METADATA_FIELDS'):
    settings.DJANGOPYPI_METADATA_FIELDS = {'1.0': ('platform', 'summary', 'description', 'keywords', 'home_page', 'author', 'author_email',
 'license'), 
       '1.1': ('platform', 'supported_platform', 'summary', 'description', 'keywords', 'home_page',
 'download_url', 'author', 'author_email', 'license', 'classifier', 'requires', 'provides',
 'obsoletes'), 
       '1.2': ('platform', 'supported_platform', 'summary', 'description', 'keywords', 'home_page',
 'download_url', 'author', 'author_email', 'maintainer', 'maintainer_email', 'license',
 'classifier', 'requires_dist', 'provides_dist', 'obsoletes_dist', 'requires_python',
...
if not hasattr(settings, 'DJANGOPYPI_METADATA_FORMS'):
    ...
    settings.DJANGOPYPI_METADATA_FORMS = {'1.0': Metadata10Form, '1.1': Metadata11Form, 
       '1.2': Metadata12Form}
...
if not hasattr(settings, 'DJANGOPYPI_ACTION_VIEWS'):
    ...
    settings.DJANGOPYPI_ACTION_VIEWS = {'file_upload': distutils.register_or_upload, 
       'submit': distutils.register_or_upload, 
       'list_classifiers': distutils.list_classifiers}
if not hasattr(settings, 'DJANGOPYPI_XMLRPC_COMMANDS'):
    from djangopypi.views import xmlrpc
    settings.DJANGOPYPI_XMLRPC_COMMANDS = {'list_packages': xmlrpc.list_packages, 
