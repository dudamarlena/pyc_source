...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/djangopypi/views/xmlrpc.py
...
class XMLRPCResponse(HttpResponse):
    ...
    def __init__(self, params=(), methodresponse=True, *args, **kwargs):
        super(XMLRPCResponse, self).__init__(xmlrpclib.dumps(params, methodresponse=methodresponse), *args, **kwargs)
...
def list_packages(request):
    return XMLRPCResponse(params=(list(Package.objects.all().values_list('name', flat=True)),), content_type='text/xml')
...
def release_urls(request, package_name, version):
    ...
    try:
        for dist in Package.objects.get(name=package_name).releases.get(version=version).distributions.all():
            dists.append({'url': '%s%s' % (base_url, dist.get_absolute_url()), 
