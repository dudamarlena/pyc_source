...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/djangopypi/views/packages.py
    ...
    try:
        ...
        if settings.DJANGOPYPI_PROXY_MISSING:
            return HttpResponseRedirect('%s/%s/%s/' % (
             ...
             proxy_folder,
             package))
        raise Http404('%s is not a registered package' % (package,))
...
def search(request, **kwargs):
    ...
    if form.is_valid():
        ...
        kwargs['queryset'] = Package.objects.filter(Q(name__contains=q) | Q(releases__package_info__contains=q)).distinct()
    return index(request, **kwargs)
...
def manage_versions(request, package, **kwargs):
    ...
    if request.method == 'POST':
        ...
        if formset.is_valid():
            formset.save()
            return create_update.redirect(kwargs.get('post_save_redirect', None), package)
