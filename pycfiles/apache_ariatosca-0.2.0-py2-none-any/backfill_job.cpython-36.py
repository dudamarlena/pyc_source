...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/jobs/backfill_job.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 34864 bytes
...
from airflow.exceptions import AirflowException, DagConcurrencyLimitReached, NoAvailablePoolSlot, PoolNotFound, TaskConcurrencyLimitReached
...
class BackfillJob(BaseJob):
    ...
    class _DagRunTaskStatus:
        """BackfillJob._DagRunTaskStatus"""

        def __init__(self, to_run=None, running=None, skipped=None, succeeded=None, failed=None, not_ready=None, deadlocked=None, active_runs=None, executed_dag_run_dates=None, finished_runs=0, total_runs=0):
            """
    ...
    def __init__(self, dag, start_date=None, end_date=None, mark_success=False, donot_pickle=False, ignore_first_depends_on_past=False, ignore_task_deps=False, pool=None, delay_on_limit_secs=1.0, verbose=False, conf=None, rerun_failed_tasks=False, run_backwards=False, *args, **kwargs):
        """
    ...
    def _manage_executor_state(self, running):
        ...
        for key, state in list(executor.get_event_buffer().items()):
            if key not in running:
                ...
                if (state == State.FAILED or state == State.SUCCESS) and (ti.state == State.RUNNING or ti.state == State.QUEUED):
    ...
    def _get_dag_run(self, run_date, session=None):
        ...
        run = DagRun.find(dag_id=(self.dag.dag_id), execution_date=run_date,
          session=session)
        ...
        if respect_dag_max_active_limit:
            if current_active_dag_count >= self.dag.max_active_runs:
                return
        run = run or self.dag.create_dagrun(run_id=run_id,
    ...
    def _process_backfill_task_instances(self, ti_status, executor, pickle_id, start_date=None, session=None):
        ...
        while (len(ti_status.to_run) > 0 or len(ti_status.running) > 0) and len(ti_status.deadlocked) == 0:
                ...
                if ti.state == State.SUCCESS:
                    ...
                    if self.rerun_failed_tasks:
                        ...
                        if key in ti_status.running:
                            ti_status.running.pop(key)
                        return
                backfill_context = DepContext(deps=RUN_DEPS,
                ...
                for task in self.dag.topological_sort():
                    for key, ti in list(ti_status.to_run.items()):
                        if task.task_id != ti.task_id:
                            ...
                            if open_slots <= 0:
                                raise NoAvailablePoolSlot('Not scheduling since there are %s open slots in pool %s'.format(open_slots, task.pool))
                            num_running_task_instances_in_dag = DAG.get_num_task_instances((self.dag_id),
                              states=(self.STATES_COUNT_AS_RUNNING))
                            if num_running_task_instances_in_dag >= self.dag.concurrency:
                                raise DagConcurrencyLimitReached('Not scheduling since DAG concurrency limit is reached.')
                            if task.task_concurrency:
                                num_running_task_instances_in_task = DAG.get_num_task_instances(dag_id=(self.dag_id),
                                  task_ids=[
                                 task.task_id],
                                  states=(self.STATES_COUNT_AS_RUNNING))
            ...
            if ti_status.not_ready:
                if ti_status.not_ready == set(ti_status.to_run):
                    if len(ti_status.running) == 0:
    ...
    def _collect_errors(self, ti_status, session=None):
        ...
        if ti_status.deadlocked:
            ...
            if deadlocked_depends_on_past:
                err += 'Some of the deadlocked tasks were unable to run because of "depends_on_past" relationships. Try running the backfill with the option "ignore_first_depends_on_past=True" or passing "-I" at the command line.'
    ...
    def _execute_for_run_dates(self, run_dates, ti_status, executor, pickle_id, start_date, session=None):
        ...
        processed_dag_run_dates = self._process_backfill_task_instances(ti_status=ti_status,
          executor=executor,
          pickle_id=pickle_id,
