...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/migrations/versions/74effc47d867_change_datetime_to_datetime2_6_on_mssql_.py
# Compiled at: 2019-09-11 02:44:38
# Size of source mod 2**32: 17526 bytes
__doc__ = 'change datetime to datetime2(6) on MSSQL tables\n\nRevision ID: 74effc47d867\nRevises: 6e96a59344a4\nCreate Date: 2019-08-01 15:19:57.585620\n\n'
...
def upgrade():
    ...
    if conn.dialect.name == 'mssql':
        ...
        with op.batch_alter_table('task_reschedule') as (task_reschedule_batch_op):
            ...
            task_reschedule_batch_op.alter_column(column_name='execution_date', type_=mssql.DATETIME2(precision=6),
              nullable=False)
    ...
    for constraint, columns in constraint_dict.items():
        if 'execution_date' in columns:
            if constraint[1].lower().startswith('primary'):
                operator.drop_constraint((constraint[0]),
                  type_='primary')
...
def modify_execution_date_with_constraint(conn, batch_operator, table_name, type_, nullable):
    ...
    batch_operator.alter_column(column_name='execution_date',
      type_=type_,
      nullable=nullable)
