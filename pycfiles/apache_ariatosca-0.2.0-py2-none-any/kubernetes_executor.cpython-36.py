...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/contrib/executors/kubernetes_executor.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 37890 bytes
...
class KubernetesExecutorConfig:
    ...
    def __init__(self, image=None, image_pull_policy=None, request_memory=None, request_cpu=None, limit_memory=None, limit_cpu=None, limit_gpu=None, gcp_service_account_key=None, node_selectors=None, affinity=None, annotations=None, volumes=None, volume_mounts=None, tolerations=None, labels=None):
        self.image = image
    ...
    def from_dict(obj):
        if obj is None:
            ...
            return KubernetesExecutorConfig(image=(namespaced.get('image', None)),
...
class KubeConfig:
    ...
    def __init__(self):
        ...
        self.worker_container_repository = configuration.get(self.kubernetes_section, 'worker_container_repository')
        ...
        self.kube_image_pull_policy = configuration.get(self.kubernetes_section, 'worker_container_image_pull_policy')
        ...
        self.git_ssh_known_hosts_configmap_name = conf.get(self.kubernetes_section, 'git_ssh_known_hosts_configmap_name')
        self.git_sync_credentials_secret = conf.get(self.kubernetes_section, 'git_sync_credentials_secret')
        ...
        self.git_sync_container_repository = conf.get(self.kubernetes_section, 'git_sync_container_repository')
        self.git_sync_container_tag = conf.get(self.kubernetes_section, 'git_sync_container_tag')
        self.git_sync_container = '{}:{}'.format(self.git_sync_container_repository, self.git_sync_container_tag)
        ...
        if kube_client_request_args:
            ...
            if self.kube_client_request_args['_request_timeout']:
                if isinstance(self.kube_client_request_args['_request_timeout'], list):
    ...
    def _validate(self):
        if not self.dags_volume_claim:
            if not self.dags_volume_host:
                if not self.dags_in_image:
                    if not self.git_repo or not self.git_branch or not self.git_dags_folder_mount_point:
                        raise AirflowConfigException('In kubernetes mode the following must be set in the `kubernetes` config section: `dags_volume_claim` or `dags_volume_host` or `dags_in_image` or `git_repo and git_branch and git_dags_folder_mount_point`')
        if self.git_repo:
            if self.git_user or self.git_password:
                if self.git_ssh_key_secret_name:
                    raise AirflowConfigException('In kubernetes mode, using `git_repo` to pull the DAGs: for private repositories, either `git_user` and `git_password` must be set for authentication through user credentials; or `git_ssh_key_secret_name` must be set for authentication through ssh key, but not both')
...
class KubernetesJobWatcher(multiprocessing.Process, LoggingMixin, object):
    ...
    def run(self):
        ...
        while True:
            try:
                self.resource_version = self._run(kube_client, self.resource_version, self.worker_uuid, self.kube_config)
            except Exception:
    ...
    def _run(self, kube_client, resource_version, worker_uuid, kube_config):
        ...
        for event in (watcher.stream)((kube_client.list_namespaced_pod), (self.namespace), **kwargs):
            ...
            self.process_status(task.metadata.name, task.status.phase, task.metadata.labels, task.metadata.resource_version)
            ...
            if status == 'Failed':
                ...
                if status == 'Succeeded':
                    ...
                    if status == 'Running':
                        ...
                        self.log.warning('Event: Invalid state: %s on pod: %s with labels: %s with resource_version: %s', status, pod_id, labels, resource_version)
...
class AirflowKubernetesScheduler(LoggingMixin):
    ...
    def _make_kube_watcher(self):
        ...
        watcher = KubernetesJobWatcher(self.namespace, self.watcher_queue, resource_version, self.worker_uuid, self.kube_config)
        watcher.start()
        return watcher
    ...
    def run_next(self, next_job):
        ...
        pod = self.worker_configuration.make_pod(namespace=(self.namespace),
 ...
 pod_id, self.namespace, body=client.V1DeleteOptions(), **self.kube_config.kube_client_request_args)
    ...
    def _labels_to_key(self, labels):
        ...
        try:
            ...
            self.log.warning('Error while retrieving labels; labels: %s; exception: %s', labels, e)
            return

        with create_session() as (session):
            tasks = session.query(TaskInstance).filter_by(execution_date=ex_time).all()
            self.log.info('Checking %s task instances.', len(tasks))
            for task in tasks:
                if self._make_safe_label_value(task.dag_id) == dag_id:
                    if self._make_safe_label_value(task.task_id) == task_id:
                        if task.execution_date == ex_time:
                            self.log.info('Found matching task %s-%s (%s) with current state of %s', task.dag_id, task.task_id, task.execution_date, task.state)
                            dag_id = task.dag_id
                            task_id = task.task_id
                            return (
                             dag_id, task_id, ex_time, try_num)

        self.log.warning('Failed to find and match task details to a pod; labels: %s', labels)
...
class KubernetesExecutor(BaseExecutor, LoggingMixin):
        ...
        def _create_or_update_secret(secret_name, secret_path):
            try:
                return (self.kube_client.create_namespaced_secret)(
                 ...
                 kubernetes.client.V1Secret(data={'key.json': base64.b64encode(open(secret_path, 'r').read())},
                   metadata=kubernetes.client.V1ObjectMeta(name=secret_name)), **(self.kube_config).kube_client_request_args)
            except ApiException as e:
                if e.status == 409:
                    return (self.kube_client.replace_namespaced_secret)(
                     ...
                     kubernetes.client.V1Secret(data={'key.json': base64.b64encode(open(secret_path, 'r').read())},
                       metadata=kubernetes.client.V1ObjectMeta(name=secret_name)), **(self.kube_config).kube_client_request_args)
                self.log.exception('Exception while trying to inject secret. Secret name: %s, error details: %s', secret_name, e)
                raise

        if self.kube_config.gcp_service_account_keys:
            name_path_pair_list = [{'name':account_spec.strip().split('=')[0],  'path':account_spec.strip().split('=')[1]} for account_spec in self.kube_config.gcp_service_account_keys.split(',')]
    ...
    def start(self):
        ...
        self.kube_scheduler = AirflowKubernetesScheduler(self.kube_config, self.task_queue, self.result_queue, self.kube_client, self.worker_uuid)
            ...
            try:
                ...
                try:
                    try:
                        ...
                        self.log.warning('ApiException when attempting to run task, re-queueing. Message: %s' % json.loads(e.body)['message'])
                        self.task_queue.put(task)
                    except HTTPError as e:
                        self.log.warning('HTTPError when attempting to run task, re-queueing. Exception: %s', str(e))
                        self.task_queue.put(task)
