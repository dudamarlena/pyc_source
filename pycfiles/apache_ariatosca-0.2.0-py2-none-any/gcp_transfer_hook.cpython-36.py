...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/contrib/hooks/gcp_transfer_hook.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 14596 bytes
import json, time
...
YEAR = 'year'
NEGATIVE_STATUSES = {
 GcpTransferOperationStatus.FAILED, GcpTransferOperationStatus.ABORTED}
...
class GCPTransferServiceHook(GoogleCloudBaseHook):
    ...
    def get_conn(self):
        ...
        if not self._conn:
            ...
            self._conn = build('storagetransfer',
    ...
    def get_transfer_job(self, job_name, project_id=None):
        ...
        :rtype: dict
        """
        return self.get_conn().transferJobs().get(jobName=job_name,
          projectId=project_id).execute(num_retries=(self.num_retries))
    ...
    def update_transfer_job(self, job_name, body):
        ...
        """
        body = self._inject_project_id(body, BODY, PROJECT_ID)
        return self.get_conn().transferJobs().patch(jobName=job_name,
    ...
    def list_transfer_operations(self, filter):
        ...
        while request is not None:
            ...
            request = conn.transferOperations().list_next(previous_request=request,
    ...
    def _inject_project_id(self, body, param_name, target_key):
        ...
        if not body.get(target_key):
            raise AirflowException('The project id must be passed either as `{}` key in `{}` parameter or as project_id extra in GCP connection definition. Both are not set!'.format(target_key, param_name))
        return body
