...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/utils/helpers.py
# Compiled at: 2019-09-11 03:47:35
# Size of source mod 2**32: 15337 bytes
from __future__ import absolute_import
from __future__ import division
from __future__ import print_function
from __future__ import unicode_literals
import errno, imp, sys, warnings, psutil
...
DEFAULT_TIME_TO_WAIT_AFTER_SIGTERM = configuration.conf.getint('core', 'KILLED_TASK_CLEANUP_TIME')
...
def validate_key(k, max_length=250):
    if not isinstance(k, basestring):
        ...
        if len(k) > max_length:
            raise AirflowException('The key has to be less than {0} characters'.format(max_length))
        else:
            if not KEY_REGEX.match(k):
                raise AirflowException('The key ({k}) has to be made of alphanumeric characters, dashes, dots and underscores exclusively'.format(k=k))
            else:
                return True
    ...
    for index, up_task in enumerate(tasks[:-1]):
        ...
        if isinstance(up_task, BaseOperator):
            ...
            if not isinstance(up_task, Iterable) or not isinstance(down_task, Iterable):
                raise TypeError('Chain not supported between instances of {up_type} and {down_type}'.format(up_type=(type(up_task)),
                  down_type=(type(down_task))))
            else:
                if len(up_task) != len(down_task):
                    raise AirflowException('Chain not supported different length Iterable but get {up_len} and {down_len}'.format(up_len=(len(up_task)),
                      down_len=(len(down_task))))
                else:
...
class AirflowImporter(object):
    ...
    def _load_attribute(self, attribute):
        ...
        if not module:
            ...
            if module not in self._loaded_modules:
                ...
                warnings.warn("Importing '{i}' directly from '{m}' has been deprecated. Please import from '{m}.[operator_module]' instead. Support for direct imports will be dropped entirely in Airflow 2.0.".format(i=attribute,
                  m=(self._parent_module.__name__)), DeprecationWarning)
    ...
    def __getattr__(self, attribute):
        ...
        if attribute in self._attribute_modules:
...
def render_log_filename(ti, try_number, filename_template):
    ...
    if filename_jinja_template:
        ...
        return filename_template.format(dag_id=(ti.dag_id), task_id=(ti.task_id),
