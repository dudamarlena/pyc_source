...
# Embedded file name: codechefcli/auth.py
# Compiled at: 2018-03-28 05:24:04
import os, requests
from bs4 import BeautifulSoup
from .decorators import login_required
from .utils.constants import BASE_URL, COOKIES_FILE_PATH, EMPTY_AUTH_DATA_MSG, INCORRECT_CREDS_MSG, LOGIN_SUCCESS_MSG, LOGOUT_SUCCESS_MSG, SESSION_LIMIT_MSG
...
def get_other_active_sessions(session_limit_html):
    ...
    return (
     action, {inp['name']:inp['value'] for inp in inputs})
...
def disconnect_active_sessions(session, disconnect_form_html):
    ...
    disconnect_req_obj = request(session, 'POST', disconnect_url, data=other_active_sessions)
        ...
        with requests.Session() as (session):
            ...
            if req_obj.status_code == 200:
                if 'Session limit exceeded' in req_obj.text:
                    ...
                    save_cookies = False
                if save_cookies:
                    username_cookie = Cookie(version=0, name='username', value=username, port=None, port_specified=False, domain='www.codechef.com', domain_specified=False, domain_initial_dot=False, path='/', path_specified=True, secure=False, expires=None, discard=False, comment=None, comment_url=None, rest={'HttpOnly': None}, rfc2109=False)
