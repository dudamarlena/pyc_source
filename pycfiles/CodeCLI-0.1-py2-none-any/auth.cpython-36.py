...
# Embedded file name: /home/sachin/random/codechef-cli/codechefcli/auth.py
...
def disconnect_active_sessions(session, disconnect_form_html):
    ...
    if disconnect_req_obj.status_code == 200:
        resps = [
         {'data': LOGIN_SUCCESS_MSG}]
    else:
        if disconnect_req_obj.status_code == 503:
            resps = [
             {'code': 503}]
    return resps
...
def login(username, password, disconnect_sessions):
    ...
    if username and password:
        data = {'name':username, 
         'pass':password,  'form_id':'new_login_form'}
        with requests.Session() as (session):
            ...
            if req_obj.status_code == 200:
                if 'Session limit exceeded' in req_obj.text:
                    ...
                    if 'Logout' in req_obj.text:
                        resps = [
                         {'data': LOGIN_SUCCESS_MSG}]
                    else:
                        resps = [
                         {'data':INCORRECT_CREDS_MSG, 
                          'code':400}]
                        save_cookies = False
                if save_cookies:
                    username_cookie = Cookie(version=0, name='username', value=username, port=None, port_specified=False,
