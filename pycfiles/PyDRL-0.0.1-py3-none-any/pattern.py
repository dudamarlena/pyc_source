...
# Embedded file name: build\bdist.win32\egg\pydrizzle\pattern.py
...
class ParDict(dict):
    ...
    def __str__(self):
        ...
        if self['ysh'] == None:
            ...
            _ysh = '%0.4f' % self['ysh']
        if self['driz_mask'] == None:
            _driz_mask = ''
        else:
            _driz_mask = self['driz_mask']
        if self['single_driz_mask'] == None:
            _sdriz_mask = ''
        else:
            _sdriz_mask = self['single_driz_mask']
        _str = ''
...
class Pattern(object):
    ...
    def __init__(self, filename, output=None, pars=None):
        ...
        self.imtype = imtype.Imtype(filename, handle=self.image_handle, dqsuffix=self.pars['dqsuffix'])
        if image_handle:
            image_handle.close()
    ...
    def addMembers(self, filename):
        ...
        for i in range(self.nmembers):
            ...
            self.members.append(Exposure(_sciname, idckey=self.idckey, dqname=_dqname, mask=_masklist, pa_key=self.pa_key, parity=self.PARITY[detector], idcdir=self.pars['idcdir'], group_indx=group_indx[i], binned=self.binned, handle=self.image_handle, extver=i + 1, exptime=self.exptime[0], mt_wcs=self.pars['mt_wcs']))
    ...
    def buildMetachip(self, update=yes):
        ...
        _shape = (
         _wcs_ref.naxis1, _wcs_ref.naxis2, pscale)
        ...
        cen = (xsize / 2.0, ysize / 2.0)
        meta_wcs.recenter()
        _nref = meta_range['nref']
        for member in self.members:
            member.corners['corrected'] -= (_nref[0] / 2.0, _nref[1] / 2.0)
        ...
        if update:
            if not _model_ref.refpix['centered']:
                ...
                for member in self.members:
                    ...
                    _refpix['XDELTA'] -= _nref[0] / 2.0
                    _refpix['YDELTA'] -= _nref[1] / 2.0
    ...
    def transformMetachip(self, ref):
        ...
        if ref.wcs.naxis1 != 0 and ref.wcs.naxis2 != 0:
            ...
            _delta_cens = (
             ref.wcs.naxis1 / _scale - _in_wcs.naxis1, ref.wcs.naxis2 / _scale - _in_wcs.naxis2)
        else:
            _delta_cens = (0.0, 0.0)
        ...
        if _angle != 0.0:
            ...
            _range = [_xrange[1] - _xrange[0] + _delta_cens[0] / 2.0, _yrange[1] - _yrange[0] + _delta_cens[1] / 2.0]
        else:
            _range = [
             _naxis1, _naxis2]
        ...
        _crpix = (
         _in_wcs.crpix1 + _delta_cen[0] / 2.0,
         _in_wcs.crpix2 + _delta_cen[1] / 2.0)
        if _scale != 0.0:
            ...
            _delta_crpix = (0.0, 0.0)
        _out_wcs.crpix1 = _crpix[0] - _delta_crpix[0] / 2.0
        _out_wcs.crpix2 = _crpix[1] - _delta_crpix[1] / 2.0
        _out_wcs.recenter()
        return
    ...
    def buildPars(self, ref=None):
        ...
        for member in self.members:
            ...
            _scale = 1.0 / np.sqrt(abxt[0] ** 2 + abxt[1] ** 2)
            _delta_x = abxt[2]
            _delta_y = cdyt[2]
            parameters = ParDict()
    ...
    def computeOffsets(self, parity=None, refchip=None):
        ...
        if len(self.members) == 1:
            ...
            if not ref_model:
                ...
                ref_v2v3 = [0.0, 0.0]
                ref_xy = [0.0, 0.0]
                ref_pmat = np.array([[1.0, 0.0], [0.0, 1.0]])
            for member in self.members:
                ...
                if theta == None:
                    theta = 0.0
                chipcen = (
                 memwcs.naxis1 / 2.0 + memwcs.offset_x,
                 memwcs.naxis2 / 2.0 + memwcs.offset_y)
                xypos = np.dot(ref_pmat, v2v3 - ref_v2v3) / ref_scale
    ...
    def DeltaXYtoOffset(self, delta):
        ...
        delta_ra = (_wcs.crval1 - new_rd[0]) * 3600.0
        delta_dec = (_wcs.crval2 - new_rd[1]) * 3600.0
        return (
         delta_ra, delta_dec)