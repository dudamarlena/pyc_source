...
# Embedded file name: build\bdist.win32\egg\pydrizzle\traits102\wxtrait_sheet.py
...
def init_control(control, object, trait_name, description, handler, original_value=Undefined):
...
class TraitSheetAppHandler(TraitSheetHandler):
    ...
    def close(self, trait_sheet, object):
        rc = True
        if self.modified:
            dlg = wx.wxMessageDialog(trait_sheet, 'Changes have been made.\nDiscard changes?', '%s Traits' % object.__class__.__name__)
    ...
    def init(self, trait_sheet, object):
        ...
        vsizer.Add(wx.wxStaticLine(trait_sheet, -1), 1, wx.wxEXPAND | wx.wxTOP, 4)
...
class TraitSheetApp(wx.wxApp):
    ...
    def OnInit(self):
        self.SetTopWindow(TraitSheetDialog(self.object, self.traits, TraitSheetAppHandler(self)))
        return True
...
class TraitSheetDialog(wx.wxDialog):
    ...
    def __init__(self, object, traits=None, handler=default_trait_sheet_handler, parent=None, title=None):
        ...
        sw.SetSizer(sizer)
        sw.SetSize(wx.wxSize(tsdx + (tsdy > max_dy) * scrollbar_dx, min(tsdy, max_dy)))
...
class TraitSheet(wx.wxPanel):
        ...
        if pg.orientation == 'horizontal':
            ...
            for pge in pg.values:
                if isinstance(pge, TraitGroup):
                    ...
                    psizer.Add(self.add_page(pge, parent, box, default_style), 0, wx.wxEXPAND | wx.wxALL, 2)
                else:
                    if sizer is None:
                        cols = 1 + show_labels
                    ...
                    if name == '-':
                        for i in range(cols):
                            sizer.Add(wx.wxStaticLine(parent, -1), 0, wx.wxTOP | wx.wxBOTTOM | wx.wxEXPAND, 2)
                    ...
                    self.add_trait(parent, sizer, pge_object, name, label, editor, style)
    ...
    def add_trait(self, parent, sizer, object, trait_name, description, editor, style):
        if description is not None:
            suffix = ':'[description[-1:] == '?':]
            label = wx.wxStaticText(parent, -1, description + suffix, style=wx.wxALIGN_RIGHT)
        ...
        if style == 'custom':
            ...
            control = editor.readonly_editor(object, trait_name, description, self.handler, parent)
        else:
            if style == 'simple':
                control = editor.simple_editor(object, trait_name, description, self.handler, parent)
            else:
                control = editor.text_editor(object, trait_name, description, self.handler, parent)
...
class wxTraitEditor(TraitEditor):
    ...
    def readonly_editor(self, object, trait_name, description, handler, parent):
        control = wx.wxTextCtrl(parent, -1, self.str(object, trait_name), style=wx.wxTE_READONLY)
    ...
    def text_editor(self, object, trait_name, description, handler, parent):
        control = wx.wxTextCtrl(parent, -1, self.str(object, trait_name), style=wx.wxTE_PROCESS_ENTER)
    ...
    def on_text_enter(self, event):
        ...
        try:
            self.set(control.object, control.trait_name, control.GetValue(), control.handler)
            return True
    ...
    def error(self, description, excp, parent):
        dlg = wx.wxMessageDialog(parent, str(excp), description + ' value error', wx.wxOK | wx.wxICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
...
class TraitEditorInstance(wxTraitEditor):
    ...
    def on_popup_control(self, control):
        ...
        if isinstance(object, HasTraits):
            ...
            if wrap:
                traits = TraitGroup(show_border=False, style='simple', *traits)
...
class TraitEditorText(wxTraitEditor):
    ...
    def popup_editor(self, object, trait_name, description, handler, parent=None):
        while True:
            dialog = wx.wxTextEntryDialog(parent or object.window, 'Enter the new %s value:' % trait_name, defaultValue=getattr(object, trait_name))
            if dialog.ShowModal() != wx.wxID_OK:
                return
            try:
                self.set(object, trait_name, self.get_value(dialog), handler)
                return True
            except TraitError as excp:
    ...
    def on_key(self, event):
        ...
        try:
            self.set(owner.object, owner.trait_name, self.get_value(control), owner.handler)
            color = wx.wxWHITE
        except:
            color = error_color
...
class TraitEditorEnum(wxTraitEditor):
    ...
    def simple_editor(self, object, trait_name, description, handler, parent):
        control = wx.wxChoice(parent, -1, wx.wxPoint(0, 0), wx.wxSize(100, 20), self.all_values())
        try:
...
class TraitEditorImageEnum(TraitEditorEnum):
    ...
    def popup_editor(self, object, trait_name, description, handler, parent=None):
        return TraitEditorImageDialog(object, trait_name, description, parent, handler, self).ShowModal()

    def readonly_editor(self, object, trait_name, description, handler, parent):
        control = ImageControl(parent, bitmap_cache(self.current_value(object, trait_name) + self.suffix, False, self.path))
        TraitMonitor(object, trait_name, control, self.on_trait_change_image)
        return control

    def simple_editor(self, object, trait_name, description, handler, parent):
        control = self.readonly_editor(object, trait_name, description, handler, parent)
    ...
    def custom_editor(self, object, trait_name, description, handler, parent):
        panel = wx.wxPanel(parent, -1)
        self.create_image_grid(panel, getattr(object, trait_name), self.on_click)
        return panel
    ...
    def create_image_grid(self, parent, cur_value, handler):
        ...
        for value in self.all_values():
            control = ImageControl(parent, bitmap_cache(value + self.suffix, False, self.path), value == cur_value, handler)
            control.value = value
    ...
    def on_click(self, control):
        parent = control.GetParent()
        self.set(parent.object, parent.trait_name, control.value, parent.handler)
...
class TraitEditorCheckList(wxTraitEditor):
    ...
    def simple_editor(self, object, trait_name, description, handler, parent):
        control = wx.wxChoice(parent, -1, wx.wxPoint(0, 0), wx.wxSize(100, 20), self.all_labels())
        try:
            control.SetSelection(self.all_values().index(self.current_value(object, trait_name)[0]))
        except:
            pass
    ...
    def on_trait_change_choice(self, control, new):
        try:
            control.SetSelection(self.all_values().index(self.parse_value(new)[0]))
        except:
            pass
...
class TraitEditorBoolean(wxTraitEditor):
    ...
    def readonly_editor(self, object, trait_name, description, handler, parent):
        ...
        TraitMonitor(object, trait_name, control, self.on_trait_change_readonly)
...
class TraitEditorRange(TraitEditorEnum):
    ...
    def simple_editor(self, object, trait_name, description, handler, parent):
        if self.is_float or abs(self.high - self.low) > 100:
            self.format = '%d'
            if self.is_float:
                self.format = '%%.%df' % max(0, 4 - int(log10(self.high - self.low)))
            panel = wx.wxPanel(parent, -1)
            ...
            ivalue = int(float(fvalue - self.low) / (self.high - self.low) * 10000)
            label_lo = wx.wxStaticText(panel, -1, '999.999', style=wx.wxALIGN_RIGHT | wx.wxST_NO_AUTORESIZE)
            sizer.Add(label_lo, 0, wx.wxALIGN_CENTER)
            panel.slider = slider = wx.wxSlider(panel, -1, ivalue, 0, 10000, size=wx.wxSize(100, 20), style=wx.wxSL_HORIZONTAL | wx.wxSL_AUTOTICKS)
            ...
            panel.text = text = wx.wxTextCtrl(panel, -1, fvalue_text, size=wx.wxSize(60, 20), style=wx.wxTE_PROCESS_ENTER)
            ...
            return panel
        value = getattr(object, trait_name)
        control = wx.wxSpinCtrl(parent, -1, str(value), min=self.low, max=self.high, initial=value)
        wx.EVT_SPINCTRL(parent, control.GetId(), self.on_value_changed)
        TraitMonitor(object, trait_name, control, self.on_trait_change_spinner)
        return control

    def custom_editor(self, object, trait_name, description, handler, parent):
        if type(self.low) is FloatType or abs(self.high - self.low) > 15:
            return self.simple_editor(object, trait_name, description, handler, parent)
        control = TraitEditorEnum.custom_editor(self, object, trait_name, description, handler, parent)
    ...
    def on_enter(self, event):
        ...
        try:
            ...
            if slider is not None:
                slider.SetValue(int((float(value) - self.low) / (self.high - self.low) * 10000))
        except TraitError as excp:
            self.error(control.description, excp, control)
    ...
    def on_key(self, event):
        control = event.GetEventObject()
        try:
            setattr(control.object, control.trait_name, control.GetValue().strip())
            color = wx.wxWHITE
        except:
            color = error_color
    ...
    def on_scroll(self, event):
        ...
        value = self.low + float(event.GetPosition()) / 10000.0 * (self.high - self.low)
        control.text.SetValue(self.format % value)
        event_type = event.GetEventType()
        if event_type == wx.wxEVT_SCROLL_ENDSCROLL or self.auto_set and event_type == wx.wxEVT_SCROLL_THUMBTRACK:
            self.set(control.object, control.trait_name, value, control.handler)
...
class TraitEditorComplex(TraitEditorText):
    ...
    def custom_editor(self, object, trait_name, description, handler, parent):
        ...
        for editor in self.editors:
            control = editor.custom_editor(object, trait_name, description, handler, panel)
...
class TraitEditorList(wxTraitEditor):
    ...
    def readonly_editor(self, object, trait_name, description, handler, parent):
        return self.list_editor(object, trait_name, description, handler, parent, 'readonly_editor')

    def text_editor(self, object, trait_name, description, handler, parent):
        return self.list_editor(object, trait_name, description, handler, parent, 'text_editor')

    def simple_editor(self, object, trait_name, description, handler, parent):
        return self.list_editor(object, trait_name, description, handler, parent, 'simple_editor')

    def custom_editor(self, object, trait_name, description, handler, parent):
        return self.list_editor(object, trait_name, description, handler, parent, 'custom_editor')

    def list_editor(self, object, trait_name, description, handler, parent, type):
        trait_handler = self.trait_handler
        resizable = trait_handler.min_items != trait_handler.max_items and type != 'readonly_editor'
        ...
        if is_fake:
            values = [
             item_trait.default_value]
        for value in values:
            width = height = 0
            if resizable:
                control = ImageControl(list_pane, bitmap_cache('list_editor', False), -1, self.popup_menu)
                width, height = control.GetSize()
                width += 4
            try:
                proxy = ListItemProxy(object, trait_name, index, item_trait, value)
                pcontrol = editor(proxy, 'value', description, handler, list_pane)
                if resizable:
                    control.pcontrol = pcontrol
                init_control(pcontrol, proxy, 'value', description, handler, value)
            except:
                if not is_fake:
                    raise
        ...
        if is_fake:
            ...
            pcontrol.Destroy()
        rows = [
         self.rows, 1][(type == 'simple_editor')]
        list_pane.SetSize(wx.wxSize(width + (trait_handler.max_items > rows) * scrollbar_dx, height * rows))
    ...
    def add_item(self, offset):
        ...
        control = ImageControl(list_pane, bitmap_cache('list_editor', False), -1, self.popup_menu)
        proxy = ListItemProxy(list_pane.object, list_pane.trait_name, index, item_trait, value)
        pcontrol = list_pane.editor(proxy, 'value', list_pane.description, list_pane.handler, list_pane)
        control.pcontrol = pcontrol
        init_control(pcontrol, proxy, 'value', list_pane.description, list_pane.handler, value)
        controls[index:index] = [(control, pcontrol)]
        self.reload_sizer(controls, -2)

    def empty_list(self, list_pane):
        control = ImageControl(list_pane, bitmap_cache('list_editor', False), -1, self.popup_empty_menu)
        control.is_empty = True
        proxy = ListItemProxy(list_pane.object, list_pane.trait_name, -1, None, None)
...
class TraitEditorFile(TraitEditorText):
    ...
    def simple_editor(self, object, trait_name, description, handler, parent):
        panel = wx.wxPanel(parent, -1)
        sizer = wx.wxBoxSizer(wx.wxHORIZONTAL)
        control = wx.wxTextCtrl(panel, -1, self.str(object, trait_name), style=wx.wxTE_PROCESS_ENTER)
        init_control(control, object, trait_name, description, handler)
        wx.EVT_KILL_FOCUS(control, self.on_enter)
...
class TraitEditorImageDialog(wx.wxDialog):
    ...
    def __init__(self, object, trait_name, description, parent, handler, editor):
        ...
        if editor.mapped:
            trait_name == '_'
        editor.create_image_grid(self, getattr(object, trait_name), self.on_click)
        position_near(parent, self)
    ...
    def on_click(self, control):
        self.editor.set(self.object, self.trait_name, control.value, self.handler)
...
standard_colors = {'aquamarine': wx.wxNamedColour('aquamarine'), 
...
def num_to_color(object, name, value):
    num = int(value)
    return wx.wxColour(num // 65536, num // 256 & 255, num & 255)


num_to_color.info = 'a number, which in hex is of the form 0xRRGGBB, where RR is red, GG is green, and BB is blue'
...
class TraitEditorColor(wxTraitEditor):
    ...
    def popup_editor(self, object, trait_name, description, handler, parent=None):
        if not hasattr(parent, 'is_custom'):
            parent.ReleaseMouse()
            return TraitEditorColorDialog(object, trait_name, description, parent, handler, self).ShowModal()
        else:
            ...
            if dialog.ShowModal() == wx.wxID_OK:
                ...
                self.set(object, trait_name, self.from_wx_color(dialog.GetColourData().GetColour(), panel), handler)
                return True
            return False

    def create_editor(self, object, trait_name, description, handler, parent, factory):
        control = factory(self, object, trait_name, description, handler, parent)
        self.set_color(control, self.to_wx_color(object, trait_name))
        try:
            getattr(object, trait_name + '_')
            TraitMonitor(object, trait_name, control, self.on_trait_change_text)
            TraitMonitor(object, trait_name + '_', control, self.on_trait_change_color)
        except:
    ...
    def readonly_editor(self, object, trait_name, description, handler, parent):
        return self.create_editor(object, trait_name, description, handler, parent, wxTraitEditor.readonly_editor)

    def simple_editor(self, object, trait_name, description, handler, parent):
        return self.create_editor(object, trait_name, description, handler, parent, wxTraitEditor.simple_editor)

    def custom_editor(self, object, trait_name, description, handler, parent):
        panel = wx.wxPanel(parent, -1)
        sizer = wx.wxBoxSizer(wx.wxHORIZONTAL)
        control = panel.color = self.simple_editor(object, trait_name, description, handler, panel)
        ...
        for i in range(len(color_samples)):
            control = wx.wxButton(panel, -1, '', size=wx.wxSize(18, 18))
    ...
    def on_click(self, event):
        control = event.GetEventObject()
        parent = control.GetParent()
        self.set(parent.object, parent.trait_name, self.from_wx_color(control.GetBackgroundColour(), parent), parent.handler)
...
color_trait = Trait('white', wx.wxColour, wx.wxColourPtr, standard_colors, num_to_color, editor=color_editor)
clear_color_trait = Trait('clear', None, wx.wxColour, wx.wxColourPtr, standard_colors, {'clear': None}, num_to_color, editor=color_editor)
...
class TraitEditorColorDialog(wx.wxDialog):
    ...
    def __init__(self, object, trait_name, description, parent, handler, editor):
        wx.wxDialog.__init__(self, parent, -1, 'Choose ' + description)
        wx.EVT_CLOSE(self, self.on_close_dialog)
        panel = editor.custom_editor(object, trait_name, description, handler, self)
        panel.object = object
...
font_families = {'default': wx.wxDEFAULT, 
   'decorative': wx.wxDECORATIVE, 
   'roman': wx.wxROMAN, 
   'script': wx.wxSCRIPT, 
   'swiss': wx.wxSWISS, 
   'modern': wx.wxMODERN}
font_styles = {'slant': wx.wxSLANT, 
   'italic': wx.wxITALIC}
font_weights = {'light': wx.wxLIGHT, 
   'bold': wx.wxBOLD}
font_noise = [
 'pt', 'point', 'family']
...
def str_to_font(object, name, value):
    try:
        ...
        return wx.wxFont(point_size, family, style, weight, underline, (' ').join(facename))
    except:
        pass
...
str_to_font.info = "a string describing a font (e.g. '12 pt bold italic swiss family Arial' or 'default 12')"
...
class TraitEditorFont(wxTraitEditor):
    ...
    def popup_editor(self, object, trait_name, description, handler, parent=None):
        ...
        if dialog.ShowModal() == wx.wxID_OK:
            self.set(object, trait_name, self.from_wx_font(dialog.GetFontData().GetChosenFont()), handler)
            return True
        return False
    ...
    def custom_editor(self, object, trait_name, description, handler, parent):
        ...
        control = panel.facename = wx.wxChoice(panel, -1, wx.wxPoint(0, 0), wx.wxSize(100, 20), self.all_facenames())
        sizer2.Add(control, 2, wx.wxEXPAND)
        wx.EVT_CHOICE(panel, control.GetId(), self.on_value_changed)
        control = panel.pointsize = wx.wxChoice(panel, -1, wx.wxPoint(0, 0), wx.wxSize(30, 20), point_sizes)
    ...
    def str_value(self, font):
        weight = {wx.wxLIGHT: ' Light', wx.wxBOLD: ' Bold'}.get(font.GetWeight(), '')
        style = {wx.wxSLANT: ' Slant', wx.wxITALIC: ' Italic'}.get(font.GetStyle(), '')
    ...
    def on_value_changed(self, event):
        ...
        font = wx.wxFont(pointsize, wx.wxDEFAULT, wx.wxNORMAL, wx.wxNORMAL, faceName=facename)
        self.set(control.object, control.trait_name, self.from_wx_font(font), control.handler)
...
font_trait = Trait('Arial 10', wx.wxFont, wx.wxFontPtr, str_to_font, editor=TraitEditorFont())
...
class ImageControl(wx.wxWindow):
    _selectedPenDark = wx.wxPen(wx.wxSystemSettings_GetColour(wx.wxSYS_COLOUR_3DSHADOW), 1, wx.wxSOLID)
    _selectedPenLight = wx.wxPen(wx.wxSystemSettings_GetColour(wx.wxSYS_COLOUR_3DHIGHLIGHT), 1, wx.wxSOLID)

    def __init__(self, parent, bitmap, selected=None, handler=None):
        wx.wxWindow.__init__(self, parent, -1, size=wx.wxSize(bitmap.GetWidth() + 10, bitmap.GetHeight() + 10))
        self._bitmap = bitmap
            ...
            if selected != self._selected:
                if selected:
                    for control in self.GetParent().GetChildren():
                        if isinstance(control, ImageControl) and control.Selected():
    ...
    def _on_paint(self, event=None):
        ...
        pens = [
         self._selectedPenLight, self._selectedPenDark]
