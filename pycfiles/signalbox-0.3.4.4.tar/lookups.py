...
# [GCC 8.4.0]
# Embedded file name: /Users/ben/dev/signalbox/signalbox/lookups.py
...
class UserLookup(ModelLookup):
    ...
    def get_query(self, request, q):
        ...
        return User.objects.filter(Q(first_name__istartswith=q) | Q(last_name__istartswith=q) | Q(email__icontains=q) | Q(username__icontains=q))
...
class MembershipLookup(ModelLookup):
    ...
    def get_query(self, request, term):
        if not request.user.is_staff:
            return HttpResponseForbidden()
        queryset = Membership.objects.filter(Q(user__first_name__istartswith=term) | Q(user__last_name__istartswith=term) | Q(user__email__icontains=term) | Q(user__username__icontains=term))
        return queryset
...
class ObservationLookup(ModelLookup):
    ...
    def get_query(self, request, term):
        if not request.user.is_staff:
            return HttpResponseForbidden()
        queryset = Observation.objects.filter(Q(dyad__user__username__istartswith=term) | Q(id__istartswith=term))
        return queryset
