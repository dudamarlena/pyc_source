...
# [GCC 8.4.0]
# Embedded file name: /Users/ben/dev/signalbox/signalbox/models/observation.py
# Compiled at: 2014-08-27 19:26:12
import random, itertools, twilio
...
import observation_timing_functions as tf, observation_methods.default as default, observation_helpers as hlp
...
class ReminderInstance(models.Model):
    ...
    def send_reminder(self):
        ...
        success, statusmessage = getattr(self, '_send_%s_reminder' % (self.reminder.kind,))()
        self.sent = success
        self.save()
        return (
         bool(success), str(statusmessage))
    ...
    def _send_email_reminder(self):
        ...
        subject, message = hlp.format_message_content(self.reminder.subject, self.reminder.message, self.observation)
    ...
    def _send_sms_reminder(self):
        ...
        try:
            result = client.sms.messages.create(to=to_number, from_=from_number, body=message, status_callback=current_site_url() + reverse('sms_callback'))
            self.observation.add_data(key='external_id', value=result.sid)
            self.observation.add_data(key='reminder', value='%s (sent to number ending %s)' % (message, to_number[-3:]))
            return (
             True, str(result.sid))
...
class ObservationManager(models.Manager):
    ...
    def personalised(self, user, due_now=False):
        ...
        if user.groups.filter(name='Assessors').count() > 0:
            allobs = allobs.exclude(created_by_script__isnull=True).exclude(created_by_script__breaks_blind=True)
        return allobs
...
class Observation(models.Model):
    ...
    created_by_script = models.ForeignKey('signalbox.Script', blank=True, null=True, editable=False)
    status = models.IntegerField(choices=settings.STATUS_CHOICES, default=0, db_index=True)
    due_original = models.DateTimeField(help_text='Original scheduled time to make this Observation.', verbose_name='time scheduled', db_index=True)
    due = models.DateTimeField(help_text='Time the Obs. will actually be made.\n        This can be affected by many things including user requests for a later call', verbose_name='time due', db_index=True, null=True)
    last_attempted = models.DateTimeField(blank=True, null=True, db_index=True)
    offset = models.IntegerField(blank=True, null=True, help_text="Offset added to deterministic time by using the random\n        `jitter' parameter of the parent rule.", verbose_name='random offset')
    ...
    def add_reminders(self):
        ...
        instances = [ ReminderInstance(reminder=i.reminder, observation=self, due=i.calculate_date(self)) for i in scriptreminders
                    ]
    ...
    def still_open(self):
        ...
        if not closes:
            return True
        return bool(self.due < datetime.now() and datetime.now() < closes)
    ...
    def completion_data(self):
        ...
        if not self.asker:
            ...
            answered = Answer.objects.select_related().filter(reply__observation=self).exclude(other_variable_name='page_id')
            answered_questions = set([ (i.question, i.other_variable_name) for i in answered ])
            try:
    ...
    def open_for_data_entry(self, request=None):
        ...
        """
        if request and (request.user.has_perm('signalbox.can_double_enter') or request.user.is_superuser):
            return (True, '')
        if self.status == 1:
            return (False, "You've already completed this survey.")
        if not self.still_open:
            return (False,
             'The gap between recieving your email or SMS and answering the\n                question was too long. Please wait for the next signal to complete your\n                next task.')
    ...
    def make_reply(self, request, external_id=None, success_url=None, entry_method=None):
        ...
        reply, isnew = Reply.objects.get_or_create(asker=self.asker, observation=self, user=user, external_id=external_id, entry_method=entry_method, complete=False)
        if not isnew:
    ...
    def variables_which_differ_between_multiple_replies(self):
        ...
        if replies.count() < 2:
            return []
        answer_sets = [ set(reply.answer_set.all().values_list('question__id', 'answer')) for reply in replies ]
...
class TextMessageCallback(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        users = User.objects.filter(Q(userprofile__mobile__iendswith=self.from_()[-9:]) | Q(userprofile__mobile__iendswith=self.to_()[-9:]))
        if users.count():
