...
# [GCC 8.4.0]
# Embedded file name: /Users/ben/dev/signalbox/signalbox/models/reply.py
...
ENTRY_METHOD_LOOKUP = {'double_entry': 'Double entry by an administrator', 
   'preview': 'Preview', 
...
class ReplyManager(models.Manager):
    ...
    def authorised(self, user):
        ...
        if user.is_superuser:
            return qs
        qs = qs.exclude(observation__created_by_script__allow_display_of_results=False)
        if not user.groups.filter(name='Clinicians').count() > 0:
            qs = qs.exclude(observation__created_by_script__is_clinical_data=True)
        return qs
...
class Reply(models.Model, ProcessManager):
    ...
    def answers_ordered_as_per_original_asker(self):
        ...
        ordered_answers = [ answers_dict.get(i.variable_name, Answer(question=i, pk=j)) for j, i in enumerate(ordered_questions)
                          ]
        remaining_variables = set(unordered_anwers) - set(ordered_answers)
        map(lambda x: ordered_answers.append(answers_dict.get(x.variable_name(), x)), remaining_variables)
    ...
    def mapping_of_answers_and_scores(self):
        ...
        :rtype: dict
        """
        mapping = {supergetattr(i, 'question.variable_name', i.other_variable_name):i.answer for i in self.answer_set.all()}
    ...
    observation = models.ForeignKey('signalbox.Observation', blank=True, null=True)
    membership = models.ForeignKey('signalbox.Membership', blank=True, null=True)
    user = models.ForeignKey(settings.AUTH_USER_MODEL, blank=True, null=True, related_name='reply_user', help_text="IMPORTANT: this is not necessarily the user providing the data (i.e. a\n        patient) but could be an assessor or admin person doing double entry from paper. It\n        could also be null, where data is added by an AnonymousUser (e.g. Twilio or external\n        API which doesn't authenticate.)")
    ...
    originally_collected_on = models.DateField(null=True, blank=True, help_text='Set this if the date that data were entered into the system is not the date\n        that the participant originally provided the responses (e.g. when retyping paper data)')
    complete = models.BooleanField(default=False, db_index=True)
    token = ShortUUIDField(auto=True)
    external_id = models.CharField(null=True, blank=True, max_length=100, help_text='Reference for external API, e.g. Twilio')
    entry_method = models.CharField(choices=[ (k, v) for k, v in ENTRY_METHOD_LOOKUP.items() ], max_length=100, null=True, blank=True)
    ...
    def number_non_empty_answers(self):
        return len([ i for i in self.answer_set.all().exclude(other_variable_name='page_id').exclude(question__q_type='instruction') if i.answer
                   ])
