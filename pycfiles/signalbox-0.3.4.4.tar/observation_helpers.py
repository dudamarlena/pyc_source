...
# [GCC 8.4.0]
# Embedded file name: /Users/ben/dev/signalbox/signalbox/models/observation_helpers.py
...
def send_sms(_to, _from, message, client, callback_url=None):
    return client.sms.messages.create(to=_to, from_=_from, body=message, status_callback=callback_url)


def get_email_address_details(observation):
    """Get the address detsils for an observation -> ((to_address, ), from_address)"""
    to_address = (
     observation.dyad.user.email,)
...
def get_email_message_parts(observation):
    ...
    subject, message = format_message_content(script.script_subject, script.script_body, observation)
    return (
