...
# [GCC 8.4.0]
# Embedded file name: /Users/ben/dev/signalbox/ask/views/asker_text_editing.py
...
from ask.views.parse_definitions import block, yaml_header, make_question_dict, make_page_dict, add_scoresheet_to_question, ispage, isnotpage

class TextEditForm(forms.Form):
    text = forms.CharField(required=True, widget=forms.widgets.Textarea(attrs={'rows': 100}))
    ...
    def clean(self):
        ...
        try:
            ...
            raise forms.ValidationError(('There was a problem parsing the text:\n\n {}').format(e))
        ...
        if invalid_names:
            raise forms.ValidationError(('Invalid variable names ({})').format((', ').join(invalid_names)))
        ...
        if naughtyquestions.count():
            raise forms.ValidationError(('Some variable names are already in use by other questionnaires ({})').format((', ').join([ i.variable_name for i in naughtyquestions ])))
        for i in blocks:
            if i.calculated_score:
                ...
                if not varset.issubset(questions_set):
                    raise forms.ValidationError(("Not all variables for scoresheet '{}' were matched: {}").format(i.calculated_score.name, (', ').join(varset.difference(questions_set))))

        self.cleaned_data.update({'asker': asker, 
    ...
    def save(self):
        ...
        questionsbypage = filter(bool, [ filter(isnotpage, i[1]) for i in groupby(blocks, ispage) ])
        ...
        questionsbypage_d = [ [ make_question_dict(i) for i in j ] for j in questionsbypage
                            ]
        [ [ q.update({'page': p, 'order': i}) for i, q in enumerate(plist) ] for plist, p in zip(questionsbypage_d, pages_obs)
        ]
        ...
        [ [ add_scoresheet_to_question(question, parseresult) for question, parseresult in zip(pageq, pagep) ] for pageq, pagep in zip(questionsbypage_obs, questionsbypage)
        ]
        asker.save()
        return asker
...
def edit_asker_as_text(request, asker_id):
    ...
    form = TextEditForm(request.POST or None, asker=asker, locked=nreplies, initial={'text': asker.as_markdown()})
    if form.is_valid():
        form.save()
        return HttpResponseRedirect(reverse('edit_asker_as_text', args=(asker.id,)))
    else:
        return render_to_response('admin/ask/text_asker_edit.html', {'form': form, 'asker': asker}, context_instance=RequestContext(request))