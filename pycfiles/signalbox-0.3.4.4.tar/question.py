...
# [GCC 8.4.0]
# Embedded file name: /Users/ben/dev/signalbox/ask/models/question.py
...
class QuestionAsset(models.Model):
    ...
    asset = models.FileField(upload_to='questionassets', blank=True, null=True, help_text='Can be an image (.jpg, .png, or .gif), audio file (.m4a, .mp4, or .mp3) or\n        movie (.mp4 only) for display as part of the question text.')
    template = models.CharField(max_length=255, choices=ASSET_TEMPLATE_CHOICES)
...
class Question(models.Model):
    ...
    def check_if_protected(self):
        nonpreviewanswercount = self.answer_set.exclude(reply__entry_method='preview').count()
    ...
    def dict_for_yaml(self):
        ...
        return {self.variable_name: {k:v for k, v in d.items() if v if v}}
    ...
    order = models.IntegerField(default=-1, verbose_name='Page order', help_text='The order in which items will apear in the page.')
    ...
    text = models.TextField(blank=True, null=True, help_text=safe_help(settings.QUESTION_TEXT_HELP_TEXT))
    variable_name = models.SlugField(default='', max_length=32, unique=True, validators=[
     valid.first_char_is_alpha, valid.illegal_characters,
    ...
    q_type = models.CharField(choices=[ (i, i) for i in FIELD_NAMES ], blank=False, max_length=100, default='instruction')
    ...
    def preview_html(self):
        ...
        form = PageForm(None, None, page=None, questions=[self], reply=None, request=request)
        return form
    ...
    def as_markdown(self):
        ...
        return self.MARKDOWN_FORMAT.format(**{'variable_name': iden, 
    ...
    def clean_fields(self, *args, **kwargs):
        ...
        if self.q_type in 'slider' and not self.widget_kwargs:
            errors['widget_kwargs'] = [
             'No default settings for slider added (need min, max, value). E.g. {"value":50,"min":0,"max":100}']
        if 'range-slider' in self.q_type and not self.widget_kwargs:
            errors['widget_kwargs'] = [
             'Default settings for slider added (need values [a,b], min, max). E.g. {"values":[40,60],"min":0,"max":100}. Optional extra attributes: {"units":"%" } ']
        ...
        if self.required and not fieldclass.response_possible:
            errors['required'] = [
             "This type of question (%s) doesn't allow a response." % (self.q_type,)]
        if errors:
...
class Choice(models.Model):
    ...
    def natural_key(self):
        return (
         self.choiceset, self.label, self.score)

    natural_key.dependencies = ['ask.choiceset']
    choiceset = models.ForeignKey('ask.ChoiceSet', blank=True, null=True)
    is_default_value = models.BooleanField(default=False, db_index=True, help_text='Indicates whether the value will be checked by default.')
...
class ChoiceSet(models.Model):
    ...
    def dict_for_yaml(self):
        sd = {i.order:{'score': i.score, 'label': i.label, 'is_default_value': i.is_default_value} for i in self.get_choices()}
        sd = {k:{a:b for a, b in v.items() if b is not None if b is not None} for k, v in sd.items()}
        return {self.name: sd}
    ...
    def as_markdown(self):
        ...
        return ('\n').join([ self.MARKDOWN_FORMAT.format(**{'isdefault': c.get('isdefault', '') and '*' or '', 'score': c['score'], 'mapped_score': _formatmappedscore(c), 'label': c['label']}) for i, c in self.yaml.items()
                           ])
    ...
    def get_choices(self):
        ...
        if self.yaml:
            try:
                return sorted([ Choice(choiceset=self, score=choice.get('score'), mapped_score=choice.get('mapped_score', choice.get('score')), is_default_value=choice.get('isdefault', False), label=choice.get('label', ''), order=i) for i, choice in self.yaml.items()
...
class ShowIf(models.Model):
    ...
    previous_question = models.ForeignKey('ask.Question', blank=True, null=True, related_name='previous_question', help_text='For previous values, enter the name of the question which\n        will already have been answered in this survey (technically, within this Reply)')
    summary_score = models.ForeignKey('signalbox.ScoreSheet', blank=True, null=True, help_text='A summary score to be calculated based on answer already entered in the current\n        Reply.')
    values = models.CharField(max_length=255, blank=True, null=True, help_text='CASE INSENSITIVE values to match, comma separated. The question is shown if\n        any value matches')
    ...
    def clean(self):
        ...
        if not self.values and not (self.less_than or self.more_than):
            raise ValidationError('Please specify a list of values to match, or a range.')
        return
    ...
    def operator_as_string(self):
        ...
        mapping = {(True, False, False, False): ('<{}').format(self.less_than), 
    ...
    def as_simplified_dict(self):
        return {'previous_value_name': self.thing_to_be_tested(), 'conditions': self.conditions_as_strings()}
