...
# [GCC 8.4.0]
# Embedded file name: /Users/ben/dev/signalbox/signalbox/views/back.py
# Compiled at: 2014-08-27 19:26:12
import magic, operator, itertools, floppyforms
...
from signalbox.models import Membership, Observation, Script, Study, Reply, StudyCondition, Answer
...
class ReplyReassignmentDetailForm(forms.ModelForm):
    observation = selectable.AutoCompleteSelectField(label='Search for observation ID or part of a username to find the correct observation', lookup_class=ObservationLookup, required=True)
...
class ReplyReassignmentDetail(UpdateView):
    ...
    def form_valid(self, form):
        messages.info(self.request, ('Link to observation {} was successfully made.').format(self.object.observation.id))
...
class AnswerFileView(SingleObjectMixin, DownloadView):
    ...
    def get_mimetype(self):
        return magic.from_buffer(self.get_object().upload.read(1024), mime=True)
...
def send_password_reset(request, user_id):
    ...
    return HttpResponseRedirect(reverse('participant_overview', args=(
     user.id,)))
...
class CreateMembershipForm(forms.ModelForm):
    ...
    user = selectable.AutoCompleteSelectField(label='Type the name of the participant', lookup_class=UserLookup, required=True)
    relates_to = selectable.AutoCompleteSelectField(label='(Optional) patient to link this membership to:', lookup_class=MembershipLookup, required=False)
    date_randomised = forms.DateField(help_text='Format: dd/mm/yyyy', required=False)
...
def add_participant(request, study_id=None, user_id=None):
    ...
    if request.POST and form.is_valid():
        form.save()
        return HttpResponseRedirect(reverse('participant_overview', args=(
         form.cleaned_data['user'].id,)))
    else:
        return render_to_response('admin/signalbox/add_participant.html', {'form': form}, RequestContext(request, {}))
...
def add_observations_for_membership(request, membership_id):
    ...
    if mem.condition:
        ...
        messages.warning(request, 'User not allocated to a study condition yet.')
    return HttpResponseRedirect(reverse('admin:signalbox_membership_change', args=(mem.id,)))
...
def _preview_message(script, subject, body):
    ...
    participant, new = User.objects.get_or_create(first_name='Jon', last_name='Doe', username='jondoe')
    therapist, new = User.objects.get_or_create(first_name='Sigmund', last_name='Freud', username='sfreud')
    ...
    therapistmembership = Membership(study=therapiststudy, user=therapist, relates_to=participantmembership)
    observation = Observation(dyad=therapistmembership, created_by_script=script, token='d5044bcc-a06b-11e1-9c18-0023dfaac8d4')
...
def script_message_preview(request):
    if request.POST:
        ...
        message = _preview_message(script, subject=request.POST.get('subject', ''), body=request.POST.get('body', ''))
...
def preview_timings(request, klass='Study', pk=None):
    ...
    if klass == 'StudyCondition':
        ...
        scripts = [obj]
    datelist = [ {'script': script, 'times': script.datetimes_with_natural_syntax()} for script in scripts
               ]
    datelist = itertools.chain(*[ [ (i, j['script']) for i in j['times'] ] for j in datelist ])
...
def show_todo(request):
    ...
    return render_to_response('admin/signalbox/todo_list.html', {'item_list': todo_list, 'results': []}, context_instance=RequestContext(request))
...
def resolve_double_entry_conflicts(request, study_id=None, observation_id=None):
    ...
    if not study_id and not observation_id:
        studies = [ (i, len(i.unresolved_observations_with_duplicate_replies()), len(i.observations_with_duplicate_replies())) for i in Study.objects.all()
                  ]
        studies = sorted([ i for i in studies if i[2] ], key=lambda a: a[1], reverse=True)
        pagevars = {'studies': studies}
        return render_to_response('admin/signalbox/answer/list_studies_with_duplicates.html', pagevars, context_instance=RequestContext(request))
    pagevars = {}
    if study_id:
        pagevars['study'] = get_object_or_404(Study, id=study_id)
        pagevars['with_dupes'] = pagevars['study'].observations_with_duplicate_replies()
        return render_to_response('admin/signalbox/answer/list_duplicates.html', pagevars, context_instance=RequestContext(request))
    if observation_id:
        pagevars['observation'] = get_object_or_404(Observation, id=observation_id)
        return render_to_response('admin/signalbox/answer/resolve_duplicates.html', pagevars, context_instance=RequestContext(request))
...
def resend_observation_signal(request, obs_id):
    ...
    if request.user.has_perm('signalbox.can_force_send'):
        ...
        messages.warning(request, "You don't have permission to resend this observation.")
    return HttpResponseRedirect(reverse('admin:signalbox_observation_change', args=(str(observation.id),)))
...
class ObservationView(StaffuserRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return Observation.objects.personalised(self.request.user).filter(dyad__id=self.kwargs['pk']).order_by('due')
...
class ReplyUpdateAfterDoubleEntry(StaffuserRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        return reverse('membership_observations_todo', args=(
         self.object.observation.dyad.id,))
