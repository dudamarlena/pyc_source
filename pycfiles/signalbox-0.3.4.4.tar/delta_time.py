...
# [GCC 8.4.0]
# Embedded file name: /Users/ben/dev/signalbox/signalbox/utilities/delta_time.py
...
class NTime(object):
    ...
    def __init__(self, reftime=None):
        ...
        year, month, week, day, hour, minute, second = [ plural(i) for i in ('year month week day hour minute second').split()
                                                       ]
        ...
        quantified_offset = Optional(in_ | after_) + qty('qty') + (year | month | week | day | hour | minute | second)('unit') + Optional(ago_, default=1)('futureorpast')
        ...
        weekday = MatchFirst([ (CL(i) | CL(i[:3]) | CL(i[:4])).setParseAction(replaceWith(j)) for i, j in zip(WEEKDAYNAMES, RELATIVE_DAYS)
                             ])
        ...
        month_matches_ = [ MatchFirst([CL(j), CL(j[:3])]).setParseAction(replaceWith(i)) for i, j in list(enumerate(calendar.month_name))[1:]
                         ]
        monthname = MatchFirst(month_matches_).setResultsName('month')
        nth_ = Suppress(oneOf(['st', 'nd', 'rd', 'th']))
        day_matches_ = [ (CL(str(i)) + nth_).setParseAction(lambda t: int(t[0])) for i in reversed(range(1, 32))
                       ]
        ...
        def _process_date_time(toks):
            if toks.time and toks.date:
                ...
                toks['datetime'] = toks.date.replace(hour=t.hour, minute=t.minute, second=t.second, microsecond=t.microsecond) + offset
            else:
