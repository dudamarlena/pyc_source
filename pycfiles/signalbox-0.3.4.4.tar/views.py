...
# [GCC 8.4.0]
# Embedded file name: /Users/ben/dev/signalbox/twiliobox/views.py
...
def initialise_call(request, observation_token):
    ...
    reply = observation.make_reply(request, entry_method='twilio', external_id=request.POST.get('CallSid', None))
...
def save_answer(reply, question, querydict):
    ...
    if question:
        ...
        if not settings.USE_VERSIONING and prev_answers.count():
            prev_answers.delete()
    answer, created = Answer.objects.get_or_create(reply=reply, question=question, page=question and question.page)
    answer.answer = user_answer
    answer.meta = (extra_json,)
    answer.choices = question and question.choices_as_json()
    answer.save(force_save=True)
    return answer
...
def play(request, reply_token, question_index=0):
    ...
    if not reply:
        return reply_to_twilio(say_or_play_phrase(twiml.Response(), 'No reply found for this token.'))
    else:
        asker = reply.asker
        ...
        if not request.POST:
            ...
            return reply_to_twilio(vfun(twiml.Response(), thequestion, repeat_url, reply))
...
TWILIO_SMS_FIELDS = ('sid Date_Created Date_Updated Date_Sent Account_Sid From_ To Body Status Direction Price Api_Version Uri').lower().split(' ')
TWILIO_CALL_FIELDS = ('Sid Parent_Call_Sid Date_Created Date_Updated Account_Sid To From Phone_Number_Sid Status Start_Time End_Time Duration Price Direction Answered_By Forwarded_From Caller_Name Uri').lower().split(' ')
...
def answerphone(request):
    ...
    reply = Reply(asker=asker, external_id=incoming_data['CallSid'], user=anonymous_user, entry_method='answerphone')
    reply.save()
    answers = [ Answer(reply=reply, other_variable_name=i, answer=j) for i, j in incoming_data.items()
              ]
