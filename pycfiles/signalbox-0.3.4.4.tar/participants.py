...
# [GCC 8.4.0]
# Embedded file name: /Users/ben/dev/signalbox/signalbox/views/participants.py
...
def show_followups_outstanding(request):
    ...
    return render_to_response('manage/followups_outstanding.html', {'users': everyone}, RequestContext(request))
...
def find_participant(request):
    ...
    if request.POST and form.is_valid():
        ...
        return render_to_response('manage/find_participant.html', {'form': form}, RequestContext(request))
...
def participant_overview(request, pk):
    ...
    messageform = UserMessageForm(request.POST or None, initial={'message_to': user, 'message_type': 'SMS'})
    if request.POST and contactrecordform.is_valid():
        ...
        return render_to_response('manage/participant_overview.html', {'participant': participant, 
           'contactrecordform': contactrecordform, 
           'messageform': messageform}, RequestContext(request))
...
def edit_participant(request, pk):
    ...
    try:
        pform = UserProfileForm(request.POST or None, instance=user.userprofile, request=request)
    except UserProfile.DoesNotExist:
        pform = UserProfileForm(request.POST or None, instance=UserProfile(user=user), request=request)
    ...
    if pform.is_valid():
        ...
        return render_to_response('manage/edit_participant.html', {'form': pform, 'title': title, 'user': user}, RequestContext(request))