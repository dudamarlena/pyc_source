...
# [GCC 8.4.0]
# Embedded file name: /Users/ben/dev/signalbox/ask/models/asker.py
# Compiled at: 2014-08-27 19:26:12
from collections import OrderedDict
from datetime import datetime, timedelta
from functools import partial
import functools, itertools, json, re
...
class Asker(models.Model):
    ...
    slug = models.SlugField(max_length=128, help_text='To use in URLs etc', unique=True, verbose_name='Reference code')
    success_message = models.TextField(default='Questionnaire complete.', help_text='Message which appears in banner on users screen on completion of the  questionnaire.')
    redirect_url = models.CharField(max_length=128, default='/profile/', help_text='"URL to redirect to when Questionnaire is complete.')
    ...
    def anonymous_reply_stats(self):
        ...
        return {'Complete total': replies.filter(complete=True).count(), 
    ...
    def get_absolute_url(self):
        return reverse('preview_asker', kwargs={'asker_id': self.id, 'page_num': 0})
    ...
    def json_export(self):
        ...
        QUESTION_FIELDS = ('natural_key', 'variable_name', 'order', 'required', 'help_text',
                           'text', 'choiceset.pk', 'q_type', 'showif')
        CHOICESET_FIELDS = ('natural_key', 'name')
        CHOICE_FIELDS = ('choiceset.natural_key', 'is_default_value', 'order', 'label',
                         'score')
        qdicts = [ _asdict(i, QUESTION_FIELDS) for i in questions ]
        [ d.update({'order': i}) for i, d in enumerate(qdicts) ]
        output = {'meta': {'name': self.slug, 'show_progress': self.show_progress, 
                    'generated': str(datetime.now())}, 
           'questions': qdicts, 
           'choicesets': [ _asdict(i, CHOICESET_FIELDS) for i in choicesets ], 'choices': [ _asdict(i, CHOICE_FIELDS) for i in choices ], 'scoresheets': [ i.as_simplified_dict() for i in self.scoresheets() ]}
    ...
    def as_markdown(self):
        ...
        :rtype: string
        """
        _fields = ('slug name steps_are_sequential redirect_url finish_on_last_page             show_progress success_message step_navigation system_audio').split()
        metayaml = yaml.safe_dump({i:getattr(self, i) for i in _fields}, default_flow_style=False)
        _pages_questions = [ (i.as_markdown(), [ j.as_markdown() for j in i.get_questions() ]) for i in self.askpage_set.all()
                           ]
