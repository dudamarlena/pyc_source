...
# [GCC 8.4.0]
# Embedded file name: /Users/ben/dev/signalbox/signalbox/models/observationcreator.py
...
class ObservationCreator(models.Model):
    ...
    def make_new_observations(self, reply):
        ...
        observations = [ Observation(label=render_string_with_context(self.script.label, {'script': self.script, 'membership': membership}), due_original=time, dyad=reply.observation.dyad, created_by_script=self.script) for time in times
                       ]
        [ i.save() for i in observations ]
        [ o.add_data(key='created', value=('Observation created by script id: {}             ({}) in reply: {}').format(self.id, self, reply.token)) for o in observations
        ]
        return observations
