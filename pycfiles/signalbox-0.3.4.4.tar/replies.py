...
# [GCC 8.4.0]
# Embedded file name: /Users/ben/dev/signalbox/signalbox/views/replies.py
...
def start_data_entry(request, observation_token, success_url=None, entry_method=None, *args, **kwargs):
    """
    ...
    if not ok_to_start:
        ...
        reply = observation.make_reply(request, entry_method=entry_method, success_url=success_url)
        if observation.created_by_script.external_asker_url:
            observation.status = -4
            observation.save()
            return HttpResponseRedirect(observation.created_by_script.parse_external_asker_url(reply))
...
def use_ad_hoc_script(request, membership_id, script_id):
    ...
    assert script in membership.study.ad_hoc_scripts.all(), 'Script not available for use by this study.'
    entering_for_self = request.user == membership.user
    assert request.user.is_staff or entering_for_self, 'User not staff and not entering data for herself.'
    observations = script.make_observations(membership=membership)
    observations = sorted(observations, key=operator.attrgetter('due_original'))
    ...
    return start_data_entry(request, first_ob.token, return_url, 'ad_hoc_script')
...
def use_adhoc_asker(request, membership_id, asker_id):
    ...
    reply = Reply(asker=asker, user=request.user, membership=membership, entry_method='ad_hoc')
    reply.save()
