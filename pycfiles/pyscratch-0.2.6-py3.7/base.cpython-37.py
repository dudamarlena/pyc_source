...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pyscratch\base.py
# Compiled at: 2019-06-03 08:02:17
...
def text(text_str, x=-120, y=-120, font_name=None, size=40, color=(128, 128, 128)):
    ...
    text_image = font.render(text_str, True, color)
...
def refresh_events(start_time):
    ...
    for event_name, time_list in events.items():
        ...
        for event_time in time_list:
            if event_time > start_time - time_piece:
...
def update_screen():
    ...
    for s in list(sprites_in_game.values()):
        if s.sprite.get_locked() or s.showing:
            rect = s.rect.copy()
            ...
            if s.text_end_time is not None:
                if time.perf_counter() > s.text_end_time:
                    s.text = None
...
class Sprite(object):
    ...
    def __init__(self, sprite_name, name=None, x=0, y=0):
        ...
        for file_name in os.listdir(name):
            ...
            self.costume[file_name_key] = os.path.join(name, file_name)
    ...
    def move(self, steps):
        ...
        direction_pi = math.pi * (direction / 180)
    ...
    def say_for_seconds(self, text_str, secs=2):
        text_image = font.render(str(text_str), True, (128, 128, 128))
