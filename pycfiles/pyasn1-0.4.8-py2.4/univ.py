...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyasn1/type/univ.py
...
noValue = NoValue()
__all__ = [
 'Integer', 'Boolean', 'BitString', 'OctetString', 'Null', 'ObjectIdentifier', 'Real', 'Enumerated', 'SequenceOfAndSetOfBase', 'SequenceOf', 'SetOf', 'SequenceAndSetBase', 'Sequence', 'Set', 'Choice', 'Any', 'NoValue', 'noValue']
...
class Real(base.SimpleAsn1Type):
    ...
    def prettyIn(self, value):
        if isinstance(value, tuple):
            if len(value) == 3 and (not isinstance(value[0], numericTypes) or not isinstance(value[1], intTypes) or not isinstance(value[2], intTypes)):
...
class SequenceAndSetBase(base.ConstructedAsn1Type):
    ...
    def setComponentByPosition(self, idx, value=noValue, verifyConstraints=True, matchTags=True, matchConstraints=True):
        ...
        elif verifyConstraints or matchTags or matchConstraints:
            if componentTypeLen:
                subComponentType = componentType.getTypeByPosition(idx)
                if subComponentType is not noValue:
                    subtypeChecker = self.strictConstraints and subComponentType.isSameTypeWith or subComponentType.isSuperTypeOf
                    if not subtypeChecker(value, verifyConstraints and matchTags, verifyConstraints and matchConstraints):
                        raise componentType[idx].openType or error.PyAsn1Error('Component value is tag-incompatible: %r vs %r' % (value, componentType))
