...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyasn1/type/namedtype.py
...
class NamedTypes(object):
    ...
    *namedTypes: :class:`~pyasn1.type.namedtype.NamedType`
    ...
    def __init__(self, *namedTypes, **kwargs):
        ...
        self.__hasOptionalOrDefault = any([ True for namedType in self.__namedTypes if namedType.isDefaulted or namedType.isOptional ])
        self.__hasOpenTypes = any([ True for namedType in self.__namedTypes if namedType.openType ])
        self.__requiredComponents = frozenset([ idx for (idx, nt) in enumerate(self.__namedTypes) if not nt.isOptional if not nt.isDefaulted ])
    ...
    def __computeAmbiguousTypes(self):
        ...
        for (idx, namedType) in reversed(tuple(enumerate(self.__namedTypes))):
            if namedType.isOptional or namedType.isDefaulted:
                partialAmbiguousTypes = (
                 namedType,) + partialAmbiguousTypes
            else:
                partialAmbiguousTypes = (
                 namedType,)
