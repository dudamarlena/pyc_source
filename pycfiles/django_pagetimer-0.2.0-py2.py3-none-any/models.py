...
# [GCC 8.4.0]
# Embedded file name: /home/anders/work/python/django-pagetimer/pagetimer/models.py
# Compiled at: 2016-05-11 11:50:30
...
def retention_enforcers():
    ...
    if hasattr(settings, 'PAGETIMER_MAX_RETENTION_INTERVAL'):
        enforcers.append(MaxIntervalRetentionEnforcer(settings.PAGETIMER_MAX_RETENTION_INTERVAL))
    if hasattr(settings, 'PAGETIMER_MAX_RETENTION_COUNT'):
        enforcers.append(MaxCountRetentionEnforcer(settings.PAGETIMER_MAX_RETENTION_COUNT))
    return enforcers
...
class PageVisitManager(models.Manager):
    ...
    def log_visit(self, username, session_key, path, ipaddress):
        if username == '':
            username = 'anonymous'
        p = PageVisit(username=username, session_key=process_session_key(session_key), path=path, ipaddress=ipaddress)
        p.save()
        self.max_retention()
        return p
    ...
    def summarize(self):
        return dict(count=self.all().count(), earliest=self.earliest(), latest=self.latest(), users=self.values('username').distinct().count(), sessions=self.values('session_key').distinct().count(), ipaddresses=self.values('ipaddress').distinct().count(), paths=self.values('path').distinct().count())
