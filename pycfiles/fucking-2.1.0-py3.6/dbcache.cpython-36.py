...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\Fucking\fuck\dbcache.py
# Compiled at: 2019-11-25 18:02:25
...
def init():
    if not os.path.exists(DB_PATH):
        ...
        conn = connect()
        conn.execute('CREATE TABLE IF NOT EXISTS system (version varchar(20) default "1.0.0",                 token varchar(255),                 cookie varchar(255))')
        for n in range(4):
            conn.execute('CREATE TABLE IF NOT EXISTS diction_{0} (id integer primary key autoincrement not null,                   word VARCHAR(255) not null,                   symbols VARCHAR(255),                   means VARCHAR(255) NOT NULL)'.format(str(n)))
