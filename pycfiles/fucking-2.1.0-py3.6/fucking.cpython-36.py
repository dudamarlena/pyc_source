...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\Fucking\fucking.py
# Compiled at: 2019-11-25 18:17:02
...
def _fetch_data(keyword):
    ...
    try:
        ...
        gtk = re.findall("window.gtk = '(.*?)';", r.text)[0]
        sign = get_sign(keyword, gtk)
        formdata['sign'] = sign
        formdata.update({'token': re.findall("token: '(.*?)',", r.text)[0]})
