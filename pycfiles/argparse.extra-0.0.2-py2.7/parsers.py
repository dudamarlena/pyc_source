...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/argparse/extra/parsers.py
# Compiled at: 2011-12-01 06:25:42
...
class DictArgumentParser(argparse.ArgumentParser):
    ...
    def from_dict(self, argsdict):
        ...
        for k, v in argsdict.items():
            add_args = dict((k, v) for k, v in v.items() if not isinstance(v, dict) and k not in self.defaults_only)
            if isinstance(k, basestring) and k.startswith('sub:'):
                if not subparsers:
                    subparsers = self.add_subparsers()
                add_args.update({'argsdict': v, 
    ...
    def __init__(self, argsdict={}, defaults_only=[], **kwargs):
        ...
        if argsdict:
            self.from_dict(dict((k, v) for k, v in argsdict.items() if isinstance(v, dict)))
            defaults_only = dict((k, v) for k, v in argsdict.items() if k in self.defaults_only)
            if defaults_only:
...
class GetArgparserTestCase(unittest.TestCase):
    ...
    def test_simple_argsdict(self):
        argsdict = {'--no-virtualenv': {'dest': 'use_virtualenv', 'action': 'store_false', 
                               'default': False}, 
           '--no-distutils': {'dest': 'use_setuptools', 'action': 'store_true', 
                              'default': False}, 
           '--no-requirements': {'dest': 'use_requirements', 'action': 'store_false', 
                                 'default': True}, 
           'sub:test': {'description': 'aa', 
                        'help': 'help'}}
        args, argv = DictArgumentParser(argsdict).parse_known_args(['test'])
        self.assert_(args.__dict__ == {'use_virtualenv': False, 'use_requirements': True, 
           'use_setuptools': False})
        args, argv = DictArgumentParser(argsdict).parse_known_args(['--no-requirements', 'test'])
        self.assert_(args.__dict__ == {'use_virtualenv': False, 'use_requirements': False, 
           'use_setuptools': False})
    ...
    def test_defaults_only(self):
        ...
        argsdict = {'--no-virtualenv': {'dest': 'use_virtualenv', 'action': 'store_false', 
           ...
           '--no-requirements': {'dest': 'use_requirements', 'action': 'store_false', 
                                 'default': True}, 
           'sub:test0': {'description': 'aa', 
                         'help': 'help', 
                         'func': test_func0}, 
           'sub:test1': {'description': 'aa', 
                         'help': 'help', 
                         'func': test_func1}}
