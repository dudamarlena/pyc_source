...
# Embedded file name: /home/tiziano/Development/StarterSquad/prudentia/prudentia/domain.py
# Compiled at: 2019-02-10 12:34:50
# Size of source mod 2**32: 5120 bytes
import json, os
from os import path
import sys, ansible.constants as C
...
class Environment(object):
    ...
    def __init__(self, id_env, general_type=None, box_extra_type=None, envs_path=DEFAULT_ENVS_PATH, file_name=DEFAULT_ENV_FILE_NAME):
        self.id_env = id_env
...
class Box(object):
    ...
    def get_remote_user(self):
        ...
        return C.DEFAULT_REMOTE_USER

    def get_remote_pwd(self):
        if self.remote_pwd:
            return self.remote_pwd
        return C.DEFAULT_REMOTE_PASS
    ...
    def __repr__(self):
        values = [
         self.playbook, self.hostname, self.ip, self.remote_user,
    ...
    def from_json(json_obj, extra_type):
        return Box(json_obj.get('name'), json_obj.get('playbook'), json_obj.get('hostname'), json_obj.get('ip'), json_obj.get('remote_user'), json_obj.get('remote_pwd'), extra_type.from_json(json_obj.get('extra')) if extra_type else None)

    def __eq__(self, other):
        return other is not None and self.name == other.name and self.playbook == other.playbook and self.hostname == other.hostname and self.ip == other.ip and self.remote_user == other.remote_user and self.remote_pwd == other.remote_pwd