...
# Embedded file name: /home/tiziano/Development/StarterSquad/prudentia/prudentia/utils/provisioning.py
# Compiled at: 2019-02-10 13:13:44
...
def load_vault_secrets(loader, vault_ids, vault_password_files=None):
    ...
    vault_ids = build_vault_ids(vault_ids, vault_password_files)
...
def run_playbook(playbook_file, inventory_file, loader, remote_user=C.DEFAULT_REMOTE_USER, remote_pass=C.DEFAULT_REMOTE_PASS, transport=C.DEFAULT_TRANSPORT, extra_vars=None, only_tags=None):
    variable_manager = get_variable_manager(loader)
    variable_manager.extra_vars = {} if extra_vars is None else extra_vars
    loader._FILE_CACHE = dict()
    ...
    loader.set_vault_secrets(load_vault_secrets(loader=loader, vault_ids=vault_ids, vault_password_files=options.vault_password_files))
    display.verbosity = options.verbosity
    pbex = PlaybookExecutor(playbooks=[
     playbook_file], inventory=inv, variable_manager=variable_manager, loader=loader, options=options, passwords={'conn_pass': remote_pass} if remote_pass else {})
    start = datetime.now()
    results = pbex.run()
...
def run_play(play_ds, inventory_file, loader, remote_user, remote_pass, transport, extra_vars=None):
    ...
    try:
        try:
            ...
            display.verbosity = options.verbosity
            tqm = TaskQueueManager(inventory=inv, variable_manager=variable_manager, loader=loader, options=options, passwords={'conn_pass': remote_pass} if remote_pass else {}, stdout_callback='minimal', run_additional_callbacks=C.DEFAULT_LOAD_CALLBACK_PLUGINS, run_tree=False)
            result = tqm.run(play)
...
def get_inventory(loader, variable_manager, inventory_file):
    try:
        ...
        return inventory.Inventory(loader=loader, variable_manager=variable_manager, host_list=inventory_file)
...
def create_user(box, loader):
    ...
    if 'root' not in user:
        ...
        res = run_play(play_ds=dict(hosts=box.hostname, gather_facts='no', tasks=loader.load_from_file(sudo_user_play)), inventory_file=generate_inventory(box), loader=loader, remote_user='root', remote_pass=box.get_remote_pwd(), transport=box.get_transport(), extra_vars={'ssh_seed_user': 'root', 'user': user, 'group': user, 'home': user_home})
    else:
...
def gather_facts(box, filter_value, loader):
    return run_play(play_ds=dict(hosts=box.hostname, gather_facts='no', tasks=[
     {'setup': 'filter={0}'.format(filter_value)}]), inventory_file=generate_inventory(box), loader=loader, remote_user=box.get_remote_user(), remote_pass=box.get_remote_pwd(), transport=box.get_transport())