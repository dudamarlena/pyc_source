...
# Embedded file name: /home/tiziano/Development/StarterSquad/prudentia/prudentia/ssh.py
...
class SshProvider(SimpleProvider):
    ...
    def define_box(self):
        ...
        pwd = io.input_value('password for the remote user', default_description='ssh key', mandatory=False, hidden=True)
    ...
    def redefine_box(self, previous_box):
        ...
        if previous_box.remote_pwd:
            pwd = io.input_value('password for the remote user', default_value=previous_box.remote_pwd, default_description='*****', mandatory=False, hidden=True)
        else:
            pwd = io.input_value('password for the remote user', default_description='ssh key', mandatory=False, hidden=True)
