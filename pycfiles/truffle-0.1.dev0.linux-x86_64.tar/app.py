...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/truffle/app.py
...
def index(file_name='', lineno=None):
    ...
    print file_index
    return render_template('index.html', directory_tree=directory_tree, file_name=file_name, lineno=lineno, code_text=code_text, search_results=None, file_index=file_index, index_fname=index_fname)
...
def _get_functions_in_file():
    ...
    for function_name in scan_path:
        if function_name.find(file_name) != -1:
            function_obj = {'functionName': function_name, 'docstringLength': False, 
...
def _run_search():
    ...
    return render_template('index.html', directory_tree=directory_tree, file_name='', lineno='', code_text='', search_results=hits, file_index='', index_fname='')


@app.route('/_flow_tree', methods=['GET'])
def _get_function_tree():
    return render_template('callgraph.html', forest=project_index.forest, rootDir=log_dir)
