...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/truffle/indextools.py
# Compiled at: 2017-07-12 01:46:15
"""
Author: Amol Kapoor
Description: Provides API for indexing code base.
"""
import json, os, global_constants as gc, parsers.pyparser as pyparser, text_index
...
def get_files(code_dir):
    ...
    for dirpath, _, filenames in os.walk(code_dir):
        py_fnames = [ os.path.join(dirpath, f) for f in filenames if f.endswith(gc.SUPPORTED_LANGS)
                    ]
    ...
    def _append_call(self, source, caller):
        ...
        for _, file_obj in self.project_index.iteritems():
            if source in file_obj['functions']:
                file_obj['functions'][source]['calling_functions'].append(caller)
                break
    ...
    def _get_tree(self, func_name, func_obj):
        ...
        for call_name in func_obj['calls']:
            if call_name in self.functions:
                tree['children'].append(self._get_tree(call_name, self.functions[call_name]))
            else:
                tree['children'].append({'name': call_name, 
                   'children': []})
