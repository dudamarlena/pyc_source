...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/truffle/parsers/pywalker.py
# Compiled at: 2017-07-09 02:18:47
"""
@Author: AmoL Kapoor
Node walker.
"""
...
class FileWalker(ast.NodeVisitor):
    ...
    def get_data(self):
        """Return data."""
        return (
         self.functions, self.variables, self.imported_modules,
    ...
    def _get_var_name(self, var_name_clone=None):
        ...
        if len(var_name_clone) == 2 and var_name_clone[(-1)] in self.imported_functions:
            import_funcname, import_module = self.imported_functions[var_name_clone[(-1)]]
    ...
    def _process_functiondef(self, node):
        ...
        self.functions[function_name] = {'calls': {}, 'lineno': node.lineno, 
           'calling_functions': [], 'name': node.name, 
           'fname': self.fname, 
    ...
    def _process_call(self, node):
        ...
        self.calls[var_name] = {'source': true_var_name, 'caller': ('.').join([self.fname] + self.context + [
                    str(node.lineno)])}
        if self.context:
    ...
    def _process_variable(self, node):
        ...
        if var_name not in self.variables:
            true_var_name_list, isimport = self._get_var_name(list(self.var_name[::-1]))
            if not isimport:
                true_var_name_list = [
                 self.fname] + true_var_name_list + [
                 str(node.lineno)]
            elif not ('.').join(true_var_name_list).startswith(self.root_dir):
                current_dir = ('.').join(self.fname.split('.')[:-1])
                true_var_name_list = [current_dir] + true_var_name_list
