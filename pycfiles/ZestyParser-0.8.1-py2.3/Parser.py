...
# [GCC 8.4.0]
# Embedded file name: build/bdist.darwin-8.9.1-i386/egg/ZestyParser/Parser.py
# Compiled at: 2007-04-30 17:11:20
"""
@version: 0.8.1
@author: Adam Atlas
...
"""
__all__ = (
 'ZestyParser', 'NotMatched', 'ParseError', 'CallbackFor', 'DebugNote')

class Error(Exception):
    __module__ = __name__


class NotMatched(Error):
...
class ParseError(Error):
    ...
    def __init__(self, parser, message):
        ...
        @type message: str
        """
        (self.parser, self.message, self.coord) = (
         parser, message, parser.coord())
...
class ZestyParser:
    ...
    def iter(self, token, skip=None, until=None):
        ...
        while 1:
            if skip:
                self.skip(skip)
            if until and self.skip(until[0]):
                break
            r = self.scan(token)
            if self.last:
                yield r
