...
# [GCC 8.4.0]
# Embedded file name: build/bdist.darwin-8.9.1-i386/egg/ZestyParser/Tokens.py
# Compiled at: 2007-04-30 17:11:26
"""
...
__all__ = (
 'Placeholder', 'AbstractToken', 'TokenWrapper', 'RE', 'Raw', 'Token', 'RawToken', 'CompositeToken', 'TokenSequence', 'TakeToken', 'TokenSeries', 'EmptyToken', 'Default', 'Skip', 'Omit', 'Only', 'Defer', 'Lookahead', 'Negative', 'EOF', 'Whitespace', 'Const', 'Inf')
rstack = []
replstack = []
Inf = -1
    ...
    def __call__(self, parser, origCursor):
        ...
        while i != self.max:
            ...
            if i != 0 and self.delimiter is not None:
                ...
                if self.skip:
                    parser.skip(self.skip)
            if self.prefix and not parser.skip(self.prefix):
                break
            t = parser.scan(self.desc)
            if parser.last is None:
                break
            if self.postfix and not parser.skip(self.postfix):
                break
            if i != 0 and self.includeDelimiter:
                o.append(d)
            o.append(t)
            i += 1
