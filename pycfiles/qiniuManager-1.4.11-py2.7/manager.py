...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/qiniuManager/manager.py
# Compiled at: 2017-10-05 22:52:05
from __future__ import print_function
import os, sys, time, json, urllib, sqlite3, hashlib, fnmatch
...
def db_ok(func):
    ...
    def func_wrapper(self, *args, **kwargs):
        if self.db and self.cursor:
            ...
            return (
             False, ('`{}` 访问失败,请检查访问权限').format(self.config_path))
...
def get_md5(path):
    ...
    if os.path.exists(path):
        ...
        with open(path, 'rb') as (handle):
            for chuck in iter(lambda : handle.read(4096), ''):
...
class Config(object):
    ...
    def init_db(self):
        try:
            ...
            self.cursor.execute(('CREATE TABLE IF NOT EXISTS {} (id INTEGER PRIMARY KEY AUTOINCREMENT,access VARCHAR(64) NOT NULL UNIQUE,secret VARCHAR(64) NOT NULL, x INTEGER NOT NULL DEFAULT 0)').format(self.API_keys))
            self.cursor.execute(("CREATE TABLE IF NOT EXISTS {} (id INTEGER PRIMARY KEY AUTOINCREMENT,name VARCHAR(50) NOT NULL DEFAULT '',alias VARCHAR(50) NOT NULL DEFAULT '',as_default INTEGER NOT NULL DEFAULT 0)").format(self.SPACE_ALIAS))
    ...
    def get_one_access(self):
        ...
        if not fetch:
            ...
            return (
             decrypt(fetch[1]) if fetch[3] else fetch[1],
    ...
    def add_access(self, access, secret):
        ...
        def add_key():
            self.cursor.execute(("insert into {} (access, secret, x) VALUES ('{}', '{}', 1)").format(self.API_keys, encrypt(access), encrypt(secret)))
    ...
    def set_space(self, space, alias=''):
        ...
        if result:
            if alias:
                self.cursor.execute(("update {} set alias = '{}', as_default=1 WHERE id = {}").format(self.SPACE_ALIAS, alias, result[0]))
            else:
                self.cursor.execute(('update {} set as_default=1 WHERE id = {}').format(self.SPACE_ALIAS, result[0]))
        else:
            self.cursor.execute(("insert into {} (name, alias, as_default) VALUES ('{}', '{}', 1)").format(self.SPACE_ALIAS, space, alias))
...
class Qiniu(object):
    ...
    def print_debug(feed, entity=True):
        ...
        :return: None
        """
        print('\x1b[01;33mEnv:\x1b[00m')
        print(('\n').join([ ('\x1b[01;31m{}\x1b[00m {}').format(i, j) for i, j in [('Tool Ver.', __version__),
         (
          'Py Ver.', sys.version)]
                          ]))
        print(('\x1b[01;33mResponse:\x1b[00m @{}').format(time.time()))
        if feed.status:
            print(('{}').format(feed.status['status']))
        print(('\n').join([ ('{} : {}').format(i, feed.headers[i]) for i in feed.headers ]))
        if entity:
            print(('\n{}').format(feed.data), '\n\x1b[01;33mEOF\x1b[00m')
    ...
    def regular_download_link(self, target, space=None):
        ...
        return (
         True, os.path.join(host, urllib.quote(target)))
    ...
    def download(self, target, space=None, directory=None, is_debug=False):
        ...
        if not feed.status or not int(feed.status['code']) == 200:
            ...
            return (False, ('\x1b[01;31m{}\x1b[00m not exist !').format(target))
        end = time.time()
        size = os.path.getsize(save_path)
        return (True,
         ('\x1b[01;31m{}\x1b[00m downloaded @speed \x1b[01;32m{}/s\x1b[00m').format(target, unit_change(size / (end - start))))
    ...
    def rename(self, target, to_target, space=None, is_debug=False):
        ...
        url = self.manager_host + ('/move/{}/{}/force/false').format(urlsafe_base64_encode(('{}:{}').format(space, target)), urlsafe_base64_encode(('{}:{}').format(space, to_target)))
        manager_rename.request(url, headers={'Authorization': ('QBox {}').format(self.auth.token_of_request(url))})
        feed = http.SockFeed(manager_rename)
        feed.disable_progress = True
        feed.http_response()
        if 'error' in feed.data:
            data = json.loads(feed.data.decode())
            return (
             False, ('发生错误: \x1b[01;31m{}\x1b[00m').format(data['error']))
        else:
            return (
             True, ('\x1b[01;31m{}\x1b[00m now RENAME as \x1b[01;32m{}\x1b[00m').format(target, to_target))
    ...
    def remove(self, file_pattern, space=None, stop=True):
        ...
        def del_item(target):
            ...
            manager_remove.request(url, headers={'Authorization': ('QBox {}').format(self.auth.token_of_request(url))})
            feed = http.SockFeed(manager_remove)
            feed.disable_progress = True
            feed.http_response()
            ret = feed.data
            if 'error' in ret:
                return (False, ('发生错误: \x1b[01;31m{}\x1b[00m').format(json.loads(ret.decode())['error']))
            else:
                return (
                 True, ('`\x1b[01;31m{}\x1b[00m` 已从 \x1b[01;34m{}\x1b[00m 中删除').format(target, space))
        ...
        if info:
            ...
            targets = list(filter(lambda x: fnmatch.fnmatch(x['key'], ('{}').format(file_pattern)), data))
            if stop:
                if len(targets) <= 0:
                    choice = yield (
                     '', space)
                    if choice is True:
                        yield (
                         False, ('{} 没有该文件 {}').format(space, file_pattern))
                    else:
                        yield (
                         False, '')
                elif len(targets) == 1:
    ...
    def check(self, target, space=None, is_debug=False):
        ...
        manager_check.request(url, headers={'Authorization': ('QBox {}').format(self.auth.token_of_request(url))})
        ...
        if int(feed.status['code']) == 612:
            print(('文件 \x1b[01;31m{target}\x1b[00m 不存在于 \x1b[01;32m{space}\x1b[00m').format(target=target, space=space))
            ...
            data = json.loads(feed.data.decode())
            if 'error' in data:
                return (False, ('发生错误: \x1b[01;31m{}\x1b[00m').format(data['error']))
            ret = ('  {}  {}  {}\r\n').format('Space', '.' * (self.COL_WIDTH - len('Space')), ('\x1b[01;32m{}\x1b[00m').format(space))
            ret += ('  {}  {}  {}\r\n').format('Filename', '·' * (self.COL_WIDTH - len('filename')), target)
            ret += ('  {}  {}  {} ({})\r\n').format('Size', '·' * (self.COL_WIDTH - len('size')), ('\x1b[01;37m{}\x1b[00m').format(unit_change(data['fsize'])), data['fsize'])
            ret += ('  {}  {}  {}\r\n').format('MimeType', '·' * (self.COL_WIDTH - len('MimeType')), data['mimeType'])
            ret += ('  {}  {}  {} ({})').format('Date', '·' * (self.COL_WIDTH - len('date')), ('\x1b[01;37m{}\x1b[00m').format(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(data['putTime'] / 10000000))), data['putTime'])
            return (True, ret)

    @auth
    def list(self, space=None, reverse=True, by_date=True, is_debug=False, get_sum=True, find_pattern=None, greater=None, littler=None):
        u"""
        列出指定空间或默认空间中的所有文件
        :param space: str => 指定空间或默认空间
        ...
        if data:
            ...
            if greater is not None:
                chew = sorted(filter(lambda x: x['fsize'] >= greater, data), key=sort_tool, reverse=reverse)
            else:
                if littler is not None:
                    chew = sorted(filter(lambda x: x['fsize'] <= littler, data), key=sort_tool, reverse=reverse)
                else:
                    chew = sorted(filter(lambda x: fnmatch.fnmatch(x['key'], ('{}').format(find_pattern)), data) if find_pattern else data, key=sort_tool, reverse=reverse)
                for i in chew:
                    self.total_size += i['fsize']

            tmp = ('\r\n').join([ ('  {}  {}  {}').format(i['key'], '·' * (self.COL_WIDTH - str_len(('{}').format(i['key']))), unit_change(i['fsize'])) for i in chew
                                ])
            if tmp:
                ret = ('\x1b[01;32m{}\x1b[00m\r\n').format(space) + tmp
            else:
                return (
                 True, '')
            if get_sum:
                ret += ('\r\n\r\n  \x1b[01;31m{}\x1b[00m  \x1b[01;32m{}\x1b[00m  \x1b[01;31m{}\x1b[00m').format('Total', '·' * (self.COL_WIDTH - len('total')), unit_change(self.total_size))
            return (True, ret)
        else:
            return (
             False, ('\x1b[01;31m{}\x1b[00m 空间不存在或没有文件').format(space))
            return

    @auth
    def list_all(self, reverse=True, by_date=True, find_pattern=None, greater=None, littler=None):
        u"""
        ...
        if not spaces:
            ...
            if find_pattern or greater or littler:
                chew = []
                for i in spaces:
                    state, result = self.list(space=i[0], reverse=reverse, by_date=by_date, get_sum=False, find_pattern=find_pattern, greater=greater, littler=littler)
    ...
    def __get_list_in_space(self, space, mute=False, is_debug=False):
        ...
        space_list.request(url, headers={'Authorization': ('QBox {}').format(self.auth.token_of_request(url))})
        ...
        if is_debug:
            self.print_debug(feed)
        if not feed.data:
            return (('\x1b[01;31m{}\x1b[00m 空间不存在').format(space), [])
        else:
            data = json.loads(feed.data.decode())
            if 'error' in data:
                return (('发生错误: \x1b[01;31m{}\x1b[00m @\x1b[01;35m{}\x1b[00m').format(data['error'], space), [])
            return ('', data['items'])
    ...
    def upload(self, abs_path, space=None):
        ...
        if not data:
            ...
            mkfile.request(file_url, 'POST', {'Authorization': ('UpToken {}').format(self.pre_upload_info[3])}, data=(',').join(self.block_status))
        ...
        try:
            labor = http.HTTPCons()
            labor.request(('{0}/mkblk/{1}').format(self.pre_upload_info[(-1)], len(data)), 'POST', {'Authorization': ('UpToken {}').format(self.pre_upload_info[3])}, data=data)
