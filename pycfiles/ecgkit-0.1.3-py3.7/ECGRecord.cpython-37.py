...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/ecgkit/ECGRecord/ECGRecord.py
# Compiled at: 2019-06-14 07:40:23
# Size of source mod 2**32: 27533 bytes
...
class ECGRecord(object):
        ...
        if format == 'qt' or format == 'qtdb':
            ...
            if format == 'ludb':
                ...
                if format == 'unlabeled':
                    ...
                    if format == 'mat' or format == 'matlab':
                        self._load_mat(path)
                    else:
    ...
    def _load_qt(self, path):
        ...
        if list(np.unique(np.array(self.symbol[0]))) != ['(', ')', 'N', 'p', 't']:
            if list(np.unique(np.array(self.symbol[0]))) != ['(', ')', 'N', 'p', 't', 'u']:
    ...
    def _load_ludb(self, path):
        ...
        for lead in ('i', 'ii', 'iii', 'avr', 'avl', 'avf', 'v1', 'v2', 'v3', 'v4',
            ...
            if list(np.unique(np.array(symbol_1))) != ['(', ')', 'N', 'p', 't']:
                if list(np.unique(np.array(symbol_1))) != ['(', ')', 'N', 'p', 't', 'u']:
                    print('Invalid symbols in ECG annotations.')
                    raise ValueError
    ...
    def _generate_beatlabel_from_label(self):
        if self.format == 'qt' or self.format == 'qtdb':
            ...
            if self.format == 'ludb':
                ...
                if not self.format == 'unlabeled':
                    if self.format == 'mat' or self.format == 'matlab':
    ...
    def _preprocess_qt(self, left_rates=None, right_rates=None, fixed_len=None):
        ...
        return (
         np.array(segments0), np.array(labels0), np.array(raw_lengths0), self.diagnose)
    ...
    def rs_ratio(self, lead):
        ...
        ratio_list = []
        for i in range(len(self.beat_label)):
            if self.beat_label[i]['Ronset']:
                if self.beat_label[i]['Roffset'] and self.beat_label[i]['Poffset'] and self.beat_label[i]['Tonset']:
