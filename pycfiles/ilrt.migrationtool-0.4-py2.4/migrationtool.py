...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/ilrt/migrationtool/browser/migrationtool.py
...
class SiteMigrationTool(BaseMTool):
        ...
        if getattr(self, 'options', ()) == ():
            ...
            try:
                for item in menu.getMenuItems(self, getattr(self, 'REQUEST', {})):
                    zope2menu.append({'label': item['title'], 'action': str(item['action'])})
        ...
        if self.methodlists.has_key(self.migration_id):
            if method in self.methodlists.get(self.migration_id, []):
                try:
                    ...
                    out.append('Error: Sorry this egg, ' + str(self.egg) + ', has no migrations                         - please add a migrations folder with migration scripts in')
                    return out
                except:
    ...
    def registerUpgradePath(self, oldversion, newversion, function):
        """ Basic register func """
        self._upgradePaths[oldversion.lower()] = [
         newversion.lower(), function]

    def registerDowngradePath(self, oldversion, newversion, function):
        """ Basic register func """
        self._downgradePaths[oldversion.lower()] = [
         newversion.lower(), function]
    ...
    def traceout(self, out, msg='Your migration scripts caused a fatal error please correct it'):
    ...
    def loadMigrations(self, reload=0):
        ...
        if reload or not getattr(self, 'migrations', None):
            ...
            try:
                ...
                try:
                    migrationlist = [ m for m in dir(migrations) if not m.startswith('__') ]
                except:
                    migrationlist = []
                else:
                    if not migrationlist:
                        out.append('Error: the migrations folder has no initialized                             migrations - check you have added them in the init')
                    util_methods = dir(utils)
                    for m in migrationlist:
                        m = str(m)
                        try:
                            migration = getattr(migrations, m)
                            methodlist = [ me for me in dir(migration) if not me.startswith('__') ]
                        ...
                        if 'VERSIONS' in methodlist:
                            if 'upgrade' in methodlist:
                                try:
                                    self.registerUpgradePath(migration.VERSIONS[0], migration.VERSIONS[1], migration.upgrade)
                                    if hasattr(migration, 'downgrade'):
                                        self.registerDowngradePath(migration.VERSIONS[1], migration.VERSIONS[0], migration.downgrade)
                                ...
                                out.append("Error: Sorry migration script '" + m + "' has no upgrade function please add                                    one ie. def upgrade(self, portal):")
                        else:
                            out.append("Error: Sorry migration script '" + m + "' has no VERSIONS please add one                                     ie. VERSIONS = ('1','2')")
    ...
    def knownVersions(self):
        ...
        versions = [ LooseVersion(x) for x in Set(self._upgradePaths.keys() + self._downgradePaths.keys()) if x != current_version ]
        versions.sort()
        return versions
    ...
    def setInstanceVersion(self, version):
        ...
        return self.manage_results(self, out=(('Instance initialised to version %s' % version, zLOG.INFO),))
    ...
    def _migrate(self, version, dirn='up'):
        ...
        if not path_dict.has_key(version):
            return (
             None, ('Migration completed at version %s' % version,))
