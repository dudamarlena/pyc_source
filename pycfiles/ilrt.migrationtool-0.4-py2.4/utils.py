...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/ilrt/migrationtool/browser/utils.py
...
def runGenericSetupSteps(portal, out, profile='mysite.theme:default', steps=[]):
    ...
    if not steps:
        try:
            ...
            ssout = mtool.traceout(ssout, 'Generic setup steps failed for %s' % profile)

    else:
        try:
            gsetup.manage_importSelectedSteps(context_id=context_id, ids=steps, run_dependencies=True)
        except ValueError:
            ssout = mtool.traceout(ssout, 'Generic setup steps failed for %s' % profile)
...
def editSiteProps(portal, out, sheet='site_properties', prop='enable_link_integrity_checks', value=False):
...
def pickleAttributes(portal, out, attributes, path, action='save'):
    ...
    def write_pickle(out, attr, pickled_attr):
        ...
        try:
            ...
            try:
                ...
                out.append('Sorry failed to write to %s due to %s'(filepath, str(os.error)))
    ...
    if action == 'load':
        ...
        if attrs:
            out.append('You have saved your attributes from %s to %s' % (str(attrs)[1:-1], SAVEPATH))
        else:
            out.append('The object %s has none of the listed attributes' % obj.getId())