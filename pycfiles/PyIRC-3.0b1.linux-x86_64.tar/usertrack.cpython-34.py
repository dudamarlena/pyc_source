...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.4/site-packages/PyIRC/extensions/usertrack.py
# Compiled at: 2015-10-15 10:42:58
# Size of source mod 2**32: 25787 bytes
...
from PyIRC.auxparse import prefix_parse, who_flag_parse, status_prefix_parse, userhost_parse
...
class User:
    ...
    def __repr__(self):
        keys = ('nick', 'username', 'host', 'gecos', 'account', 'server', 'secure',
                'operator', 'signon', 'ip', 'realhost', 'channels')
...
class UserTrack(BaseExtension):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.add_user(basicrfc.nick, user=self.username, gecos=self.gecos)
    ...
    def update_username_host(self, hostmask_or_line):
        ...
        if hasattr(hostmask_or_line, 'hostmask'):
            ...
            if hasattr(hostmask_or_line, 'nick'):
    ...
    def close(self, _):
        timers = chain(self.u_expire_timers.values(), self.who_timers.values())
        for timer in timers:
            try:
    ...
    def prefix(self, _, setter, target, mode):
        ...
        user = self.get_user(hostmask.nick)
        if user is None:
            user = self.add_user(hostmask.nick, username=hostmask.username, host=hostmask.host)
    ...
    def burst(self, _, scope):
        ...
        if not user:
            user = self.add_user(target.nick, username=target.username, host=target.host, gecos=scope.gecos, account=scope.account)
        else:
    ...
    def part(self, _, scope):
        ...
        user = self.get_user(target.nick)
        if not user:
            _logger.warning('Got a part/kick for a user not found: %s (in %s)', target.nick, channel)
            return
        if channel not in user.channels:
            _logger.warning('Got a part/kick for a user not in a channel: %s (in %s)', target.nick, channel)
            return
    ...
    def message(self, _, line):
        ...
        if not self.get_user(hostmask.nick):
            if not self.do_timeout:
                return
            user = self.add_user(hostmask.nick, user=hostmask.username, host=hostmask.host)
