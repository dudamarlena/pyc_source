...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.4/site-packages/PyIRC/extensions/channeltrack.py
# Compiled at: 2015-10-08 05:15:23
# Size of source mod 2**32: 11246 bytes
...
class Channel:
    ...
    def __repr__(self):
        keys = ('name', 'modes', 'topic', 'topictime', 'topicwho', 'timestamp', 'url',
                'users')
...
class ChannelTrack(BaseExtension):
    ...
    def prefix(self, _, setter, target, mode):
        ...
        if channel is None:
            _logger.warning('Got a PREFIX event for an unknown channel: %s', target)
            return
    ...
    def topic(self, _, line):
        ...
        if line.command.lower() == 'topic':
            ...
            if channel is None:
                _logger.debug('Topic for unknown channel: {}'.format(line.params[0]))
                return
        ...
        if channel is None:
            _logger.debug('RPL_TOPIC for unknown channel: {}'.format(line.params[1]))
            return
    ...
    def no_topic(self, _, line):
        ...
        channel = self.get_channel(line.params[1])
        if channel is None:
            _logger.debug('RPL_NOTOPIC for unknown channel: {}'.format(line.params[1]))
            return
        channel.topic = ''
    ...
    def topic_who_time(self, _, line):
        ...
        channel = self.get_channel(line.params[1])
        if channel is None:
            _logger.debug('Topic time for unknown channel: {}'.format(line.params[1]))
            return
    ...
    def url(self, _, line):
        ...
        channel = self.get_channel(line.params[1])
        if channel is None:
            _logger.debug('URL for unknown channel: {}'.format(line.params[1]))
            return
    ...
    def timestamp(self, _, line):
        ...
        channel = self.get_channel(line.params[1])
        if channel is None:
            _logger.debug('Creation time for unknown channel: {}'.format(line.params[1]))
            return
