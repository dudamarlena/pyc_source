...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/beanstalk/multiserverconn.py
# Compiled at: 2015-07-11 09:52:37
import socket, select, random, logging, threading, asyncore, asynchat, errno, traceback, time, sys, copy, protohandler
...
class AsyncServerConn(object, asyncore.dispatcher):
    ...
    def __str__(self):
        ...
        return s % {'class': self.__class__.__name__, 'active': active_, 
           'ip': self.server, 
           'port': self.port, 
           'waiting': waiting_}
    ...
    def __getattribute__(self, attr):
        ...
        if res:
            ...
            def caller(*args, **kw):
                logger.info('Calling %s on %r with args(%s), kwargs(%s)', res.__name__, self, args, kw)
                func = self._do_interaction
                func(*res(*args, **kw))
                asyncore.loop(use_poll=True, timeout=ASYNCORE_TIMEOUT, count=ASYNCORE_COUNT)
                return self.result
    ...
    def _get_response(self):
        while True:
            ...
            if not recv:
                closedmsg = 'Remote server %(server)s:%(port)s has closed connection' % {'server': self.server, 'port': self.port}
                self.close()
...
class ServerPool(object):
    ...
    def retry_until_succeeds(func):
        ...
        def retrier(self, *args, **kwargs):
            while True:
                try:
                    value = func(self, *args, **kwargs)
                except ServerInUse as e:
                    logger.exception(e[0])
                    raise
                except protohandler.errors.Draining as e:
                    pass
                except protohandler.errors.NotConnected as e:
                    logger.warning(e[0])
                    server = e[1]
    ...
    def multi_interact(self, line, handler):
        for server in self.servers:
            ...
            except ServerInUse as (msg, server):
                logger.info(msg)
    ...
    def combine_stats(func):
        ...
        def combiner(self, *args, **kwargs):
            ...
            for a in appendables:
                try:
                    result['data'][a] = set(map(lambda x: x['data'][a], returned))
                except KeyError:
                    pass
