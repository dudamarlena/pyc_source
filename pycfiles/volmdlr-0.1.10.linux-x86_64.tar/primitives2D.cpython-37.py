...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.7/site-packages/volmdlr/primitives2D.py
# Compiled at: 2020-03-11 05:21:08
# Size of source mod 2**32: 15659 bytes
"""
...
class OpenedRoundedLineSegments2D(RoundedLineSegments, volmdlr.Wire2D):
    ...
    def __init__(self, points, radius, adapt_radius=False, name=''):
        primitives = RoundedLineSegments.__init__(self, points, radius, (volmdlr.LineSegment2D),
          (volmdlr.Arc2D), closed=False,
    ...
    def Rotation(self, center, angle, copy=True):
        if copy:
            return self.__class__([p.Rotation(center, angle, copy=True) for p in self.points],
              (self.radius),
              adapt_radius=(self.adapt_radius),
              name=(self.name))
        self.__init__([p.Rotation(center, angle, copy=True) for p in self.points], (self.radius),
          adapt_radius=(self.adapt_radius),
          name=(self.name))

    def Translation(self, offset, copy=True):
        if copy:
            return self.__class__([p.Translation(offset, copy=True) for p in self.points], (self.radius),
              adapt_radius=(self.adapt_radius), name=(self.name))
        self.__init__([p.Translation(offset, copy=True) for p in self.points], (self.radius),
          adapt_radius=(self.adapt_radius), name=(self.name))
    ...
    def OffsetSingleLine(self, line_index, offset):
        ...
        for i, point in enumerate(self.points[:-1] + self.closed * [self.points[(-1)]]):
            ...
            if not math.isclose((math.cos(alpha1)), 0, abs_tol=1e-09):
                if math.isclose((math.cos(alpha2)), 0, abs_tol=1e-09):
        ...
        if not dont_add_last_point:
            if not self.closed:
                new_linesegment2D_points.append(self.points[(-1)])
        rls2D = self.__class__(new_linesegment2D_points, (self.radius), (self.closed),
          adapt_radius=(self.adapt_radius))
        return rls2D
    ...
    def OffsetLines(self, line_indexes, offset):
        ...
        if math.isclose(dot1, 0, abs_tol=1e-09):
            return self.OffsetLines([line_indexes[0] - 1] + line_indexes, offset)
        if math.isclose(dot2, 0, abs_tol=1e-09):
        ...
        rls2D = self.__class__(new_linesegment2D_points, (self.radius), adapt_radius=(self.adapt_radius))
        return rls2D
...
class ClosedRoundedLineSegments2D(OpenedRoundedLineSegments2D, volmdlr.Contour2D):
    ...
    def __init__(self, points, radius, adapt_radius=False, name=''):
        primitives = RoundedLineSegments.__init__(self, points, radius, (volmdlr.LineSegment2D),
          (volmdlr.Arc2D), closed=True,
          adapt_radius=adapt_radius,
          name='')
        volmdlr.Contour2D.__init__(self, primitives, name)