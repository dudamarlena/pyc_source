...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.7/site-packages/volmdlr/code_aster.py
# Compiled at: 2020-03-11 05:21:08
# Size of source mod 2**32: 15257 bytes
"""
...
class PlanarNodeGroup:
    ...
    def CodeAsterExport(self, a=None):
        char = "DEFI_GROUP( reuse  = {}, MAILLAGE = {},\n            CREA_GROUP_NO = _F(NOM = '{}', OPTION = 'PLAN',\n            POINT = {}, VECT_NORMALE = {}, PRECISION = {},),); \n \n".format(self.mesh.name, self.mesh.name, self.name, self.point, self.normal, self.precision)
        if a is not None:
            a.write(char)
        else:
            return char
...
class NodeGroupAssembly:
    ...
    def CodeAsterExport(self, a=None):
        if self.typ == 'diff':
            char = "DEFI_GROUP( reuse  = {}, MAILLAGE = {},\n                CREA_GROUP_NO = _F(NOM = '{}', DIFFE = {},),); \n \n".format(self.mesh.name, self.mesh.name, self.name, [n.name for n in self.list_node_group])
        else:
            if self.typ == 'union':
                char = "DEFI_GROUP( reuse  = {}, MAILLAGE = {}, \n                CREA_GROUP_NO = _F(NOM = '{}', UNION = {},),); \n \n".format(self.mesh.name, self.mesh.name, self.name, [n.name for n in self.list_node_group])
            else:
                if self.typ == 'intersec':
...
class LimitedPlaneNodeGroup:
    ...
    def __init__(self, name, mesh, point, normal, list_dir, list_length):
        ...
        list_intersec = []
        self.list_node_group.append(PlanarNodeGroup(name=(self.name + '_0'), mesh=(self.mesh), point=(self.point),
          normal=(self.normal)))
        ...
        for i, li in enumerate(list_dir):
            ...
            radius1 = abs(list_length[i][1] / 2.0)
            point1 = tuple(point + npy.array(list_dir[i]) * list_length[i][1] / 2.0)
            self.list_node_group.append(CylinderNodeGroup(name=(self.name + '_' + str(compt)), mesh=(self.mesh),
              point=point1,
              normal=normal1,
              radius=radius1))
            compt += 1
            ...
            radius2 = abs(list_length[i][0] / 2.0)
            point2 = tuple(point + npy.array(list_dir[i]) * list_length[i][0] / 2.0)
            self.list_node_group.append(CylinderNodeGroup(name=(self.name + '_' + str(compt)), mesh=(self.mesh),
              point=point2,
              normal=normal2,
              radius=radius2))
            compt += 1
            list_union.append(self.list_node_group[(-1)])
            self.list_node_group.append(NodeGroupAssembly(name=(self.name + '_' + str(compt)), mesh=(self.mesh),
              typ='union',
              list_node_group=list_union))
            compt += 1
            list_intersec.append(self.list_node_group[(-1)])

        self.list_node_group.append(NodeGroupAssembly(name=(self.name), mesh=(self.mesh),
          typ='intersec',
...
class Mesh:
    ...
    def __init__(self, name, file_name_CAD, file_name_mesh=None, gmsh_path='/Users/Pierrem/DessIA/PowerPack/scripts/Gmsh.app/Contents/MacOS/./gmsh'):
...
class Material:
    ...
    def CodeAsterExport(self, a=None):
        char = "{} = DEFI_MATERIAU(ELAS = _F(E = {},\n                          NU = {},\n                          RHO = {},),); \n\n{} = AFFE_MATERIAU(MAILLAGE = {},\n                    AFFE = _F(TOUT = 'OUI',\n                            MATER = {},),); \n \n".format(self.name + '_1', self.E, self.nu, self.rho, self.name, self.mesh.name, self.name + '_1')
        if a is None:
            return char
        a.write(char)
...
class Model:
    ...
    def CodeAsterExport(self, a=None):
        char = "{} = AFFE_MODELE(MAILLAGE = {},\n                   AFFE = _F(TOUT = 'OUI',\n                           PHENOMENE = 'MECANIQUE',\n                           MODELISATION = '3D',),); \n \n".format(self.name, self.mesh.name)
        if a is None:
            return char
        a.write(char)
...
class BoundaryCondition:
    ...
    def CodeAsterExport(self, a=None):
        ...
        char = "{} = AFFE_CHAR_MECA(MODELE = {},\n                       DDL_IMPO = _F(GROUP_NO = '{}',\n                                   {}),); \n \n".format(self.name, self.modele.name, self.group.name, char_ddl)
        if a is None:
            return char
        a.write(char)
...
class MecaStat:
    ...
    def CodeAsterExport(self, a=None):
        ...
        char = '{} = MECA_STATIQUE(MODELE = {}, CHAM_MATER = {},\n                EXCIT = ({}),); \n \n'.format(self.name, self.modele.name, self.material.name, char_bound)
        if a is None:
            return char
        a.write(char)
...
class ResultAnalysis:
    ...
    def CodeAsterExport(self, a=None):
        char = "{} = CALC_CHAMP(reuse = {},\n                       RESULTAT = {},\n                       FORCE = '{}',);\n    \n{} = POST_RELEVE_T(ACTION = _F(OPERATION = 'EXTRACTION',\n                                 INTITULE = 'FORCE',\n                                 RESULTAT = {},\n                                 NOM_CHAM = '{}',\n                                 GROUP_NO = '{}',\n                                 RESULTANTE = ('DX', 'DY', 'DZ'),),);\n\nIMPR_TABLE(TABLE = {}, UNITE = 4); \n \n".format(self.simulation.name, self.simulation.name, self.simulation.name, self.typ, self.simulation.name + '_1', self.simulation.name, self.typ, self.group_no.name, self.simulation.name + '_1')
        if a is None:
            return char
        a.write(char)
...
class CodeAster:
    ...
    def __init__(self, name, file_name, mesh, node_group, modele, material, boundary, simulation, analysis):
        self.name = name
    ...
    def Run(self, path_code_aster='/opt/aster', version_code_aster='13.4'):
        ...
        actual_path = os.getcwd()
        os.system('cp {}/{}/share/aster/config.txt {}/.'.format(path_code_aster, version_code_aster, actual_path))
