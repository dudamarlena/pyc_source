...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.7/site-packages/volmdlr/primitives3D.py
# Compiled at: 2020-03-11 05:21:08
# Size of source mod 2**32: 39165 bytes
"""
Common primitives 3D
"""
import math, numpy as npy
...
class OpenedRoundedLineSegments3D(volmdlr.Wire3D, RoundedLineSegments):
    ...
    def __init__(self, points, radius, adapt_radius=False, name=''):
        primitives = RoundedLineSegments.__init__(self, points, radius, (volmdlr.LineSegment3D),
          (volmdlr.Arc3D),
          closed=False,
    ...
    def Rotation(self, center, angle, copy=True):
        if copy:
            return self.__class__([p.Rotation(center, angle, copy=True) for p in self.points], self.radius, self.closed, self.name)
        self.__init__([p.Rotation(center, angle, copy=True) for p in self.points], self.radius, self.closed, self.name)

    def Translation(self, offset, copy=True):
        if copy:
            return self.__class__([p.Translation(offset, copy=True) for p in self.points], self.radius, self.closed, self.name)
...
class Sphere(volmdlr.Primitive3D):
    ...
    def Volume(self):
        return 1.3333333333333333 * math.pi * self.radius ** 3
    ...
    def babylon_script(self, name='primitive_mesh'):
        ...
        extruded_profile = RevolvedProfile((self.position), (volmdlr.X3D), (volmdlr.Y3D), (volmdlr.Contour2D([line, arc])),
          (self.position), (volmdlr.X3D), name=(self.name))
    ...
    def frame_mapping(self, frame, side, copy=True):
        ...
        """
        if copy:
            return Sphere(self.center.frame_mapping(frame, side, copy), self.radius)
...
class Block(volmdlr.Shell3D):
    ...
    def __init__(self, frame: volmdlr.Frame3D, *, color: Tuple[(float, float, float)]=None, alpha: float=1.0, name: str=''):
        self.frame = frame
    ...
    def plot_data(self, x3D, y3D, marker=None, color='black', stroke_width=1, dash=False, opacity=1, arrow=False):
        lines = []
        for edge3D in self.Edges():
            lines.append(edge3D.plot_data(x3D, y3D, marker, color, stroke_width, dash, opacity, arrow))
...
class Cone(volmdlr.Primitive3D):
    ...
    def _bounding_box(self):
        ...
        x_bound = (
         pointA[0] - kx * self.radius, pointA[0] + kx * self.radius, pointB[0])
        xmin = min(x_bound)
        xmax = max(x_bound)
        y_bound = (
         pointA[1] - ky * self.radius, pointA[1] + ky * self.radius, pointB[1])
        ymin = min(y_bound)
        ymax = max(y_bound)
        z_bound = (
         pointA[2] - kz * self.radius, pointA[2] + kz * self.radius, pointB[2])
        zmin = min(z_bound)
        zmax = max(z_bound)
...
class ExtrudedProfile(volmdlr.Shell3D):
    ...
    def __init__(self, plane_origin, x, y, outer_contour2d, inner_contours2d, extrusion_vector, color=None, alpha=1.0, name=''):
        volmdlr.Primitive3D.__init__(self, name=name)
    ...
    def shell_faces(self):
        lower_contours = [
         self.outer_contour3d] + self.inner_contours3d
        ...
        return [
         lower_face] + [upper_face] + lateral_faces
    ...
    def frame_mapping(self, frame, side, copy=True):
        ...
        if copy:
            return ExtrudedProfile(self.plane_origin.frame_mapping(frame, side, copy), x, y, self.outer_contour2d, self.inner_contours2d, extrusion_vector)
        self.__init__(self.plane_origin.frame_mapping(frame, side, copy), x, y, self.outer_contour2d, self.inner_contours2d, extrusion_vector)
...
class RevolvedProfile(volmdlr.Shell3D):
    ...
    def __init__(self, plane_origin, x, y, contour2D, axis_point, axis, angle=2 * math.pi, name='', color=None, alpha=1):
        ...
        for nb in range(number_points_tesselation):
            ...
            for i, (pt1, pt2) in enumerate(points_pair):
                ...
                if pt1 == pt2 and pt1_next == pt2_next:
                    ...
                    if pt1 == pt2:
                        edges = [
                         volmdlr.LineSegment3D(pt1.copy(), pt2_next.copy()),
                         volmdlr.LineSegment3D(pt2_next.copy(), pt1_next.copy()),
                         volmdlr.LineSegment3D(pt1_next.copy(), pt1.copy())]
                    else:
                        if pt1_next == pt2_next:
                            edges = [
                             volmdlr.LineSegment3D(pt1.copy(), pt2.copy()),
    ...
    def frame_mapping(self, frame, side, copy=True):
        ...
        if side == 'old':
            ...
            if side == 'new':
                ...
                raise ValueError('side must be either old or new')
        if copy:
            return RevolvedProfile(self.plane_origin.frame_mapping(frame, side, copy), x, y, self.contour2D, self.axis_point.frame_mapping(frame, side, copy), axis, self.angle)
        self.__init__(self.plane_origin.frame_mapping(frame, side, copy), x, y, self.contour2D, self.axis_point.frame_mapping(frame, side, copy), axis, self.angle)


class Cylinder(RevolvedProfile):

    def __init__(self, position, axis, radius, length, color=None, alpha=1.0, name=''):
        self.position = position
        axis.Normalize()
        self.axis = axis
        self.radius = radius
        ...
        RevolvedProfile.__init__(self, position, axis, y, contour, position, axis, color=color,
          alpha=alpha,
          name=name)
    ...
    def FreeCADExport(self, ip):
        if self.radius > 0:
            ...
            position = 1000 * (self.position - self.axis * self.length / 2.0)
            x, y, z = position
            x = str(x)
            y = str(y)
            z = str(z)
    ...
    def babylon_script(self, name='primitive_mesh'):
        ...
        p3 = volmdlr.Point2D((0.5 * self.length, 0.0))
        p4 = volmdlr.Point2D((-0.5 * self.length, 0.0))
        l1 = volmdlr.LineSegment2D(p1, p2)
        l2 = volmdlr.LineSegment2D(p2, p3)
        l3 = volmdlr.LineSegment2D(p3, p4)
        l4 = volmdlr.LineSegment2D(p4, p1)
        extruded_profile = RevolvedProfile((self.position), (self.axis), normal_vector1, (volmdlr.Contour2D([l1, l2, l3, l4])),
    ...
    def frame_mapping(self, frame, side, copy=True):
        ...
        if side == 'old':
            ...
            if side == 'new':
                ...
                raise ValueError('side must be either old or new')
        if copy:
            return Cylinder(self.position.frame_mapping(frame, side, copy), axis, self.radius, self.length)
...
class HollowCylinder(Cylinder):
    ...
    def __init__(self, position, axis, inner_radius, outer_radius, length, color=None, alpha=1, name=''):
        volmdlr.Primitive3D.__init__(self, name=name)
        self.position = position
        axis.Normalize()
        self.axis = axis
    ...
    def frame_mapping(self, frame, side, copy=True):
        ...
        if side == 'old':
            ...
            if side == 'new':
                ...
                raise ValueError('side must be either old or new')
        if copy:
            return HollowCylinder(position=(self.position.frame_mapping(frame, side, copy)), axis=axis,
...
class HelicalExtrudedProfile(volmdlr.Primitive3D):
    ...
    def __init__(self, plane_origin, x, y, axis_point, axis, pitch, outer_contour2d, inner_contours2d=None, name=''):
