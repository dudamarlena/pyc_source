...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.7/site-packages/volmdlr/plot_data.py
# Compiled at: 2020-03-11 05:21:08
# Size of source mod 2**32: 7763 bytes
"""
...
import volmdlr as vm, jsonschema, json
import matplotlib.pyplot as plt
from matplotlib.patches import Arc, FancyArrow
from mpl_toolkits.mplot3d import Axes3D
import pkg_resources, tempfile, webbrowser
...
_jsonschema = {'definitions':{},  '$schema':'http://json-schema.org/draft-07/schema#', 
 ...
 'items':{'anyOf': [
            {'type':'object', 
             'required':[
              'type', 'data', 'color', 'marker', 'size', 'dash'], 
...
def plot_d3(plot_datas):
    ...
    volmdlr_path = pkg_resources.resource_filename(pkg_resources.Requirement('volmdlr'), 'volmdlr/templates')
    s = template.render(volmdlr_path=volmdlr_path,
...
def plot(plot_datas, ax=None):
    ...
    for plot_data in plot_datas:
        if plot_data['type'] == 'line':
            ...
            if plot_data['type'] == 'arc':
                pc = vm.Point2D((plot_data['cx'], plot_data['cy']))
                ax.add_patch(Arc(pc, (2 * plot_data['r']), (2 * plot_data['r']), angle=0, theta1=(plot_data['angle1'] * 0.5 / math.pi * 360),
