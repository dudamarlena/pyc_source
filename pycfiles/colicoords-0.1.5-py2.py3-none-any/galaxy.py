...
# Embedded file name: build/bdist.linux-x86_64/egg/colibri/galaxy.py
# Compiled at: 2020-04-14 11:56:34
import colibri.constants as const, colibri.cosmology as cc, colibri.halo as hc, numpy as np, scipy
...
class galaxy(hc.halo):
    ...
    def logistic_function(self, M, log_Mmin, sigma_logM):
        r"""
                ...
                :return: array   
                """
        arg = np.log10(M / 10.0 ** log_Mmin) / sigma_logM
    ...
    def step_function(self, M, logMmin):
        r"""
                ...
                :return: array   
                """
        return self.logistic_function(M, log_Mmin=log_Mmin, sigma_logM=1e-05)
    ...
    def galaxy_Pk(self, kind_central=None, kwargs_central={}, kind_satellite=None, kwargs_satellite={}, kwargs_mass_function={}, kwargs_concentration={}):
        r"""
        ...
        for iz in range(self.nz):
            for ik in range(self.nk):
                integrand_1h = 1.0 / n_avg[iz] ** 2.0 * dndM[iz] * (2.0 * Nc[iz] * Ns[iz] * nfw[(iz, ik)] + Ns[iz] ** 2.0 * nfw[(iz, ik)] ** 2) * M
                integrand_2h = 1.0 / n_avg[iz] * dndM[iz] * bias[iz] * (Nc[iz] + Ns[iz] * nfw[(iz, ik)]) * M
                P_g_1h[(iz, ik)] = np.trapz(integrand_1h, dx=dlnM) * (1 - np.exp(-(self.k[ik] / k_star[iz]) ** 2.0))
                P_g_2h[(iz, ik)] = np.trapz(integrand_2h, dx=dlnM) ** 2.0 * normalization[iz] ** 2 * power[(iz, ik)]
