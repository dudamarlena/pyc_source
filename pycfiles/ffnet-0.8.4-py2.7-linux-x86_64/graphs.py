...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ffnet/graphs.py
...
class ADiGraph(DiGraph):
    ...
    def remove_node(self, n):
        ...
        if temp._is_connected():
            ...
            raise ValueError('Node %s disconnects graph' % (n,))
    ...
    def remove_nodes_from(self, nlist):
        ...
        if temp._is_connected():
            ...
            raise ValueError('Nodes %s disconnect graph' % (nlist,))
    ...
    def depgraph(self, u, v=None, reverse=False):
        ...
        while node_removal:
            node_removal = 0
            for node in H.nodes():
                if not H.in_degree(node) and node != u or not H.out_degree(node) and node != v and v != None:
                    try:
                        H.remove_node(node)
                        node_removal = 1
                    except:
    ...
    def subgraph(self, nbunch):
        H = DiGraph.subgraph(self, nbunch)
...
class TestADigraph(unittest.TestCase):
    ...
    def testRemoveEdge(self):
        ...
        self.assertEqual(set(self.g.edges()), set([(1, 3), (2, 3), (0, 3), (1, 4), (2, 4), (0, 4), (4, 5), (0, 5)]))
    ...
    def testRemoveEdgesFrom(self):
        ...
        self.assertEqual(set(self.g.edges()), set([(3, 5), (4, 5), (1, 4), (2, 4), (0, 4)]))
