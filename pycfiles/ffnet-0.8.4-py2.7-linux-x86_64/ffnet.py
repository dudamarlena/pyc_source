...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ffnet/ffnet.py
...
def _norms(inarray, lower=0.0, upper=1.0):
    """
    ...
    return (
     array(limits), array(en), array(de))
...
def _ffconec(conec):
    ...
    """
    if len(conec) == 0:
        raise ValueError('Empty connectivity list')
    graph = NX.DiGraph()
...
class ffnet():
    ...
    def __repr__(self):
        info = 'Feed-forward neural network: \n' + 'inputs:  %4i \n' % len(self.inno) + 'hiddens: %4i \n' % len(self.hidno) + 'outputs: %4i \n' % len(self.outno) + 'connections and biases: %4i' % len(self.conec)
        return info
    ...
    def _set_dconec(self):
        conec = [ tuple(c) for c in self.conec ]
    ...
    def call(self, inp):
        ...
        if not isinstance(inp, ndarray):
            inp = array(inp, 'd')
        if inp.ndim == 1:
            output = netprop.normcall(self.weights, self.conec, self.units, self.inno, self.outno, self.eni, self.deo, inp)
            return output
        if inp.ndim == 2:
            output = netprop.normcall2(self.weights, self.conec, self.units, self.inno, self.outno, self.eni, self.deo, inp)
            return output
    ...
    def derivative(self, inp):
        ...
        """
        if self.dconecno is None:
            self._set_dconec()
        if not isinstance(inp, ndarray):
            inp = array(inp, 'd')
        if inp.ndim == 1:
            deriv = netprop.normdiff(self.weights, self.conec, self.dconecno, self.dconecmk, self.units, self.inno, self.outno, self.eni, self.ded, inp)
            return deriv
        else:
            if inp.ndim == 2:
                deriv = netprop.normdiff2(self.weights, self.conec, self.dconecno, self.dconecmk, self.units, self.inno, self.outno, self.eni, self.ded, inp)
                return deriv
            raise TypeError('Input is not valid')
            return
    ...
    def sqerror(self, input, target):
        ...
        err = netprop.sqerror(self.weights, self.conec, self.units, self.inno, self.outno, input, target)
        return err
    ...
    def sqgrad(self, input, target):
        ...
        """
        input, target = self._setnorm(input, target)
        g = netprop.grad(self.weights, self.conec, self.bconecno, self.units, self.inno, self.outno, input, target)
        return g
    ...
    def _testdata(self, input, target):
        ...
        try:
            if not isinstance(target, ndarray):
                target = array(target, 'd')
        except:
            raise ValueError('Target cannot be converted to numpy array')
        ...
        numip = input.shape[0]
        numop = target.shape[0]
        if numip != numop:
            raise ValueError('Data not aligned: input patterns %i, target patterns: %i' % (numip, numop))
        numi = len(self.inno)
        numiv = input.shape[1]
        if numiv != numi:
            raise ValueError('Inconsistent input data, input units: %i, input values: %i' % (numi, numiv))
        numo = len(self.outno)
        numov = target.shape[1]
        if numov != numo:
            raise ValueError('Inconsistent target data, target units: %i, target values: %i' % (numo, numov))
        return (input, target)
    ...
    def _setnorm(self, input=None, target=None):
        ...
        if input is None and target is None:
            self.inlimits = array([[0.15, 0.85]] * numi)
            self.outlimits = array([[0.15, 0.85]] * numo)
            self.eni = self.dei = array([[1.0, 0.0]] * numi)
            self.eno = self.deo = array([[1.0, 0.0]] * numo)
    ...
    def train_momentum(self, input, target, eta=0.2, momentum=0.8, maxiter=10000, disp=0):
        """
        ...
        self.weights = netprop.momentum(self.weights, self.conec, self.bconecno, self.units, self.inno, self.outno, input, target, eta, momentum, maxiter)
        if disp:
            err = netprop.sqerror(self.weights, self.conec, self.units, self.inno, self.outno, input, target)
            print 'Final error   --> 0.5*(sum of squared errors at output): %.15f' % err

    def train_rprop(self, input, target, a=1.2, b=0.5, mimin=1e-06, mimax=50.0, xmi=0.1, maxiter=10000, disp=0):
        """
        ...
        if disp:
            err = netprop.sqerror(self.weights, self.conec, self.units, self.inno, self.outno, input, target)
            print 'Initial error --> 0.5*(sum of squared errors at output): %.15f' % err
        self.weights, xmi = netprop.rprop(self.weights, self.conec, self.bconecno, self.units, self.inno, self.outno, input, target, a, b, mimin, mimax, xmi, maxiter)
        if disp:
            err = netprop.sqerror(self.weights, self.conec, self.units, self.inno, self.outno, input, target)
            print 'Final error   --> 0.5*(sum of squared errors at output): %.15f' % err
        return xmi
    ...
    def train_genetic(self, input, target, **kwargs):
        ...
        if 'upper' in kwargs:
            ...
            del kwargs['upper']
        if lower >= upper:
            raise ValueError('Wrong weights range: (%f, %f)' % (lower, upper))
        if 'individuals' not in kwargs:
            kwargs['individuals'] = 20
        ...
        self.weights = netprop.vmapa(self.weights, 0.0, 1.0, lower, upper)
    ...
    def train_cg(self, input, target, **kwargs):
        ...
        self.weights = optimize.fmin_cg(func, self.weights, fprime=fprime, args=extra_args, **kwargs)
        self.trained = 'cg'
    ...
    def train_bfgs(self, input, target, **kwargs):
        ...
        if 'bounds' not in kwargs:
            kwargs['bounds'] = ((-100.0, 100.0), ) * len(self.conec)
        func = netprop.func2
        extra_args = (self.conec, self.bconecno, self.units,
         self.inno, self.outno, input, target)
        self.weights = optimize.fmin_l_bfgs_b(func, self.weights, args=extra_args, **kwargs)[0]
        self.trained = 'bfgs'
    ...
    def train_tnc(self, input, target, nproc=1, **kwargs):
        ...
        if isinstance(nproc, int) and nproc > 1 or nproc in (None, 'ncpu'):
            ...
            extra_args = (self.conec, self.bconecno, self.units,
             self.inno, self.outno, input, target)
            res = optimize.fmin_tnc(func, self.weights, args=extra_args, **kwargs)
    ...
    def _train_tnc_mp(self, input, target, nproc=None, **kwargs):
        ...
        func = mpprop.mpfunc2
        res = optimize.fmin_tnc(func, self.weights, args=(
         pool, splitters, key), **kwargs)
        self.weights = res[0]
        self._clean_mp()
        return
    ...
    def test(self, input, target, iprint=1, filename=None):
        ...
        regress = []
        if iprint:
            print 'Testing results for %i testing cases:' % nump
        for o in xrange(numo):
            if iprint:
...
def savenet(net, filename):
    ...
    if cPickle.format_version >= '3.0':
        import warnings
        warnings.warn('Network files written with Python 3 not readable on Python 2')
