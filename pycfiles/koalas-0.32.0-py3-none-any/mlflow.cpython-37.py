...
# [GCC 8.4.0]
# Embedded file name: ./build/lib/databricks/koalas/mlflow.py
# Compiled at: 2019-10-03 20:45:45
# Size of source mod 2**32: 7060 bytes
"""
...
class PythonModelWrapper(object):
    ...
    def predict(self, data):
        ...
        if isinstance(data, DataFrame):
            return_col = (self._model_udf)(*data._internal.data_scols)
            column_index = [(col,) for col in data._sdf.select(return_col).columns]
            return Series(data._internal.copy(scol=return_col, column_index=column_index),
              anchor=data)
