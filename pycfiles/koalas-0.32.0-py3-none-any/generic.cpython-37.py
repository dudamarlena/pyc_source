...
# Size of source mod 2**32: 49505 bytes
"""
...
class _Frame(object):
    ...
    def cumprod(self, skipna: bool=True):
        ...
        def cumprod(scol):
            ...
            def negative_check(s):
                if not len(s) == 0:
                    assert ((s > 0) | s.isnull()).all(), 'values should be bigger than 0: %s' % s
                return s
    ...
    def to_csv(self, path=None, sep=',', na_rep='', columns=None, header=True, quotechar='"', date_format=None, escapechar=None, num_files=None, **options):
        ...
        if path is None:
            ...
            if isinstance(header, list):
                sdf = kdf._sdf.select([self._internal.scol_for(old_name).alias(new_name) for old_name, new_name in zip(data_columns, header)])
                header = True
            else:
    ...
    def median(self, accuracy=10000):
        ...
        if isinstance(kdf_or_ks, Series):
            ks = kdf_or_ks
            return self._reduce_for_stat_function((lambda _: F.expr('approx_percentile(`%s`, 0.5, %s)' % (ks.name, accuracy))),
              name='median')
...
def _resolve_col(kdf, col_like):
    if isinstance(col_like, ks.Series):
        assert kdf is col_like._kdf, 'Cannot combine column argument because it comes from a different dataframe'
        return col_like
