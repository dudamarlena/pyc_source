...
# [GCC 8.4.0]
# Embedded file name: ../kazam/backend/prefs.py
# Compiled at: 2019-08-18 01:04:54
...
class Prefs:
    ...
    def get_dirs(self):
        ...
        if 'XDG_VIDEOS_DIR' in paths and os.path.isdir(paths['XDG_VIDEOS_DIR']):
            ...
            if 'XDG_DOCUMENTS_DIR' in paths and os.path.isdir(paths['XDG_DOCUMENTS_DIR']):
                self.video_dest = paths['XDG_DOCUMENTS_DIR']
            else:
                if 'HOME_DIR' in paths and os.path.isdir(paths['HOME_DIR']):
    ...
    def get_screens(self):
        try:
            ...
            for i in range(self.default_screen.get_n_monitors()):
                ...
                self.logger.debug('  Monitor {0} - X: {1}, Y: {2}, W: {3}, H: {4}'.format(i, rect.x, rect.y, rect.width, rect.height))
                self.screens.append({'x':rect.x,  'y':rect.y, 
                 'width':rect.width, 
                 'height':rect.height})

            if self.default_screen.get_n_monitors() > 1:
                self.combined_screen = {'x':0, 
                 'y':0,  'width':self.default_screen.get_width(), 
                 'height':self.default_screen.get_height()}
                self.logger.debug('  Combined - X: 0, Y: 0, W: {0}, H: {1}'.format(self.default_screen.get_width(), self.default_screen.get_height()))
            else:
                self.combined_screen = None
        except:
