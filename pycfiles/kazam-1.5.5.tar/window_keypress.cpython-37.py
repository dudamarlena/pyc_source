...
# [GCC 8.4.0]
# Embedded file name: ../kazam/frontend/window_keypress.py
# Compiled at: 2019-08-17 21:55:54
# Size of source mod 2**32: 10401 bytes
import math, cairo, logging
...
class KeypressWindow(GObject.GObject):
    ...
    def show(self, ev_type, value, action):
        ...
        if not ev_type == 'KeyStr':
            ...
            if ev_type == 'KeyStr' or ev_type == 'KeySym':
                if action == 'Release':
                    if value.startswith('Shift'):
        ...
        if not all(self.modifiers):
            any(self.modifiers) or logger.debug('Fadeout!')
            self.fade_out(self.window)
        else:
            if ev_type == 'MouseButton':
                if action == 'Press':
                    if value in ('1', '2', '3'):
                        self.show_cursor()
