...
# [GCC 8.4.0]
# Embedded file name: ../kazam/app.py
# Compiled at: 2019-08-18 01:52:10
# Size of source mod 2**32: 46807 bytes
import os, imp, sys, locale, shutil, gettext, logging
...
try:
    ...
    if not gst_gi[0]:
        ...
        logger.debug('Gstreamer version detected: {}.{}.{}.{}'.format(gst_gi[0], gst_gi[1], gst_gi[2], gst_gi[3]))
except ImportError:
    logger.critical(_('Gstreamer 1.0 or higher required, bailing out.'))
    sys.exit(0)
...
class KazamApp(GObject.GObject):
    ...
    def cb_main_toggled(self, widget):
        ...
        if name == 'MAIN_SCREENCAST' and widget.get_active():
            ...
            if name == 'MAIN_SCREENSHOT' and widget.get_active():
                logger.debug('Main toggled: {0}'.format(name))
                ...
                if name == 'MAIN_BROADCAST' and widget.get_active():
                    logger.debug('Main toggled: {0}'.format(name))
    ...
    def cb_record_mode_toggled(self, widget):
        ...
        if widget.get_name() == 'MODE_WIN':
            if widget.get_active():
                ...
                self.chk_borders_pic.set_sensitive(True)
        if widget.get_name() == 'MODE_WIN':
            widget.get_active() or logger.debug('Window capture OFF.')
            self.chk_borders_pic.set_sensitive(False)
    ...
    def cb_area_selected(self, widget):
        logger.debug('Area selected: SX: {0}, SY: {1}, EX: {2}, EY: {3}'.format(self.area_window.startx, self.area_window.starty, self.area_window.endx, self.area_window.endy))
        logger.debug('Area selected: GX: {0}, GY: {1}, GX: {2}, GY: {3}'.format(self.area_window.g_startx, self.area_window.g_starty, self.area_window.g_endx, self.area_window.g_endy))
    ...
    def cb_counter_finished(self, widget):
        ...
        if self.main_mode in [MODE_SCREENCAST, MODE_WEBCAM, MODE_BROADCAST]:
            ...
            if not (self.main_mode == MODE_SCREENCAST and prefs.capture_keys):
                if not self.main_mode == MODE_BROADCAST or prefs.capture_keys_broadcast:
    ...
    def cb_stop_request(self, widget):
        ...
        if self.in_countdown:
            ...
            if self.main_mode == MODE_SCREENCAST and prefs.capture_keys or self.main_mode == MODE_BROADCAST:
                if prefs.capture_keys_broadcast:
                    self.keypress_viewer.stop()
    ...
    def cb_flush_done(self, widget):
        if self.main_mode == MODE_SCREENCAST and prefs.autosave_video:
            ...
            fname = get_next_filename(prefs.autosave_video_dir, prefs.autosave_video_file, CODEC_LIST[prefs.codec][3])
            ...
            if self.main_mode == MODE_SCREENCAST or self.main_mode == MODE_WEBCAM:
                self.done_recording = DoneRecording(self.icons, self.tempfile, prefs.codec, self.old_vid_path)
            ...
            if self.main_mode == MODE_SCREENSHOT:
                if self.outline_window:
                    ...
                    if prefs.autosave_picture:
                        fname = get_next_filename(prefs.autosave_picture_dir, prefs.autosave_picture_file, '.png')
    ...
    def cb_edit_request(self, widget, data):
        ...
        fname = get_next_filename(prefs.video_dest, prefs.autosave_video_file, CODEC_LIST[prefs.codec][3])
        shutil.move(self.tempfile, fname)
    ...
    def cb_check_cursor(self, widget):
        ...
        if name == 'chk_cursor':
            ...
            if name == 'chk_cursor_pic':
                prefs.capture_cursor_pic = widget.get_active()
            else:
                if name == 'chk_cursor_broadcast':
    ...
    def cb_check_speakers(self, widget):
        ...
        if name == 'chk_speakers':
            ...
            if name == 'chk_speakers_webcam':
                prefs.capture_speakers_webcam = widget.get_active()
            else:
                if name == 'chk_speakers_broadcast':
    ...
    def cb_check_microphone(self, widget):
        ...
        if name == 'chk_microphone':
            ...
            if name == 'chk_microphone_webcam':
                prefs.capture_microphone_webcam = widget.get_active()
            else:
                if name == 'chk_microphone_broadcast':
    ...
    def cb_check_keypresses(self, widget):
        ...
        if name == 'chk_keypresses':
            ...
            if name == 'chk_keypresses_broadcast':
    ...
    def run_counter(self):
        ...
        if not (prefs.sound):
            if not (self.main_mode == MODE_BROADCAST and prefs.capture_speakers_broadcast):
                if not self.main_mode == MODE_WEBCAM or prefs.capture_speakers_webcam:
                    try:
                ...
                audio_channels = 1
            if self.main_mode == MODE_SCREENCAST:
                if not (prefs.capture_microphone or self.main_mode == MODE_BROADCAST and prefs.capture_microphone_broadcast):
                    if not self.main_mode == MODE_WEBCAM or prefs.capture_microphone_webcam:
                        try:
        ...
        if self.main_mode in [MODE_SCREENCAST, MODE_WEBCAM, MODE_BROADCAST]:
            ...
            self.recorder.setup_sources(video_source, audio_source, audio2_source, prefs.area if (self.record_mode == MODE_AREA and self.main_mode not in [MODE_WEBCAM, MODE_BROADCAST]) else None, prefs.xid if (self.record_mode == MODE_WIN and self.main_mode not in [MODE_WEBCAM, MODE_BROADCAST]) else None, audio_channels, audio2_channels)
            self.recorder.connect('flush-done', self.cb_flush_done)
        else:
            if self.main_mode == MODE_SCREENSHOT:
                self.grabber = Grabber()
                self.grabber.setup_sources(video_source, prefs.area if self.record_mode == MODE_AREA else None, prefs.xid if self.record_mode == MODE_WIN else None)
                ...
                try:
                    if self.record_mode == MODE_AREA:
                        if prefs.area:
                            ...
                            self.outline_window = OutlineWindow(prefs.area[0], prefs.area[1], prefs.area[4], prefs.area[5])
