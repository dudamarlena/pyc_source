...
# [GCC 8.4.0]
# Embedded file name: ../kazam/backend/grabber.py
...
class Grabber(GObject.GObject):
    ...
    def setup_sources(self, video_source, area, xid, active=False, god=False):
        ...
        logger.debug('Grabber source: {0}, {1}, {2}, {3}'.format(self.video_source['x'], self.video_source['y'], self.video_source['width'], self.video_source['height']))
        ...
        if prefs.shutter_sound:
            ...
            if prefs.capture_cursor_pic:
                ...
                if self.xid and prefs.capture_borders_pic:
                    ...
                    c_picbuf.composite(self.pixbuf, rx, ry, rw, rh, px + dx - 6, py + dy - 2, 1.0, 1.0, GdkPixbuf.InterpType.BILINEAR, 255)
        else:
            scr, px, py = pntr_device.get_position()
            cur = scr.get_monitor_at_point(x, y)
            px = px - HW.screens[cur]['x']
            py = py - HW.screens[cur]['y']
            c_picbuf.composite(self.pixbuf, 0, 0, w - 1, h - 1, px - 6, py - 2, 1.0, 1.0, GdkPixbuf.InterpType.BILINEAR, 255)
    ...
    def save_capture(self, old_path):
        ...
        dialog, result, self.old_path = SaveDialog((_('Save capture')), (self.old_path),
          None, main_mode=MODE_SCREENSHOT)
