...
# [GCC 8.4.0]
# Embedded file name: ../kazam/frontend/window_select.py
...
class SelectWindow(GObject.GObject):
        ...
        if button == 1:
            ...
            for win in reversed(wins):
                if win.is_visible_on_workspace(workspace) and win.is_in_viewport(workspace):
                    ...
                    geometry = self.win_name.lower().startswith('kazam') or self.win_name.lower().startswith('desktop') or win.get_client_window_geometry()
                    self.geometry = geometry
                    if geometry[0] <= event.x_root <= geometry[0] + geometry[2]:
                        if geometry[1] <= event.y_root <= geometry[1] + geometry[3]:
    ...
    def cb_keypress_event(self, widget, event):
        ...
        if keycode == 36 or keycode == 104 or keycode == 9:
            self.gdk_win.set_cursor(self.last_cursor)
            self.window.hide()
            self.emit('window-canceled')
