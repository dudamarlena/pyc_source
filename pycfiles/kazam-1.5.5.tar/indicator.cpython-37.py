...
# [GCC 8.4.0]
# Embedded file name: ../kazam/frontend/indicator.py
...
class KazamSuperIndicator(GObject.GObject):
    __gsignals__ = {'indicator-pause-request':(
      GObject.SIGNAL_RUN_LAST,
      None,
      ()), 
     'indicator-unpause-request':(
      GObject.SIGNAL_RUN_LAST,
      None,
      ()), 
     'indicator-quit-request':(
      GObject.SIGNAL_RUN_LAST,
      None,
      ()), 
     'indicator-show-request':(
      GObject.SIGNAL_RUN_LAST,
      None,
      ()), 
     'indicator-stop-request':(
      GObject.SIGNAL_RUN_LAST,
      None,
      ()), 
     'indicator-start-request':(
      GObject.SIGNAL_RUN_LAST,
      None,
      ()), 
     'indicator-about-request':(
      GObject.SIGNAL_RUN_LAST,
      None,
      ())}
    ...
    def cb_hotkeys(self, key, action):
        ...
        if action == 'start-request':
            self.recording or self.on_menuitem_start_activate(None)
        else:
            if action == 'stop-request' and self.recording:
                self.on_menuitem_finish_activate(None)
            else:
                if action == 'pause-request' and self.recording:
                    ...
                    self.recording or self.emit('indicator-show-request')
                else:
...
try:
    ...
    class KazamIndicator(KazamSuperIndicator):
        ...
        def __init__(self, silent=False):
            ...
            self.silent = silent
            self.indicator = AppIndicator3.Indicator.new('kazam', 'kazam-stopped', AppIndicator3.IndicatorCategory.APPLICATION_STATUS)
        ...
        def blink(self):
            if self.blink_state != BLINK_STOP:
                ...
                if self.blink_state == BLINK_SLOW:
                    ...
                    if self.blink_state == BLINK_FAST:
