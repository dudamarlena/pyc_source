...
# [GCC 8.4.0]
# Embedded file name: ../kazam/backend/gstreamer.py
# Compiled at: 2019-08-17 21:55:54
# Size of source mod 2**32: 33754 bytes
import os, logging
...
class Screencast(GObject.GObject):
    ...
    def setup_sources(self, video_source, audio_source, audio2_source, area, xid, audio_channels, audio2_channels):
    ...
    def setup_video_source(self):
        if prefs.test:
            ...
            if self.mode in [MODE_SCREENSHOT, MODE_SCREENCAST, MODE_BROADCAST]:
                ...
                if self.mode == MODE_WEBCAM:
                    ...
                    if not abs(startx - endx) % 2:
                        if prefs.codec == CODEC_H264:
                            endx -= 1
                    if not abs(starty - endy) % 2:
                        if prefs.codec == CODEC_H264:
                            endy -= 1
                        ...
                        if self.mode == MODE_SCREENCAST:
                            ...
                            if self.mode == MODE_BROADCAST:
                                ...
                                if self.mode == MODE_WEBCAM:
                                    ...
                                    self.video_caps = Gst.caps_from_string(caps_str.format(int(prefs.framerate), width, height))
                                    self.f_video_caps = Gst.ElementFactory.make('capsfilter', 'vid_filter')
                                    self.f_video_caps.set_property('caps', self.video_caps)
                                    ...
                                    if prefs.codec == CODEC_H264:
                                        ...
                                        if prefs.codec == CODEC_HUFF:
                                            self.mux = Gst.ElementFactory.make('avimux', 'muxer')
                                            self.video_enc.set_property('bitrate', 500000)
                                        else:
                                            if prefs.codec == CODEC_JPEG:
                                                self.mux = Gst.ElementFactory.make('avimux', 'muxer')
    ...
    def setup_audio_sources(self):
        if not self.audio_source:
            ...
            if self.audio_source:
                ...
                if self.mode == MODE_BROADCAST:
                    audio_caps = self.audio2_source or ' '.join(['audio/x-raw, format=(string)S16LE, endianness=(int)1234,signed=(boolean)true, width=(int)16, depth=(int)16,',
            ...
            if self.audio2_source:
                ...
                if self.mode == MODE_BROADCAST:
                    audio_caps = self.audio_source or ' '.join(['audio/x-raw, format=(string)S16LE, endianness=(int)1234,signed=(boolean)true, width=(int)16, depth=(int)16,',
    ...
    def setup_links(self):
        ...
        if self.mode == MODE_WEBCAM and prefs.webcam_show_preview is True:
            self.cam_window = WebcamWindow(CAM_RESOLUTIONS[prefs.webcam_resolution][0], CAM_RESOLUTIONS[prefs.webcam_resolution][1], prefs.webcam_preview_pos)
