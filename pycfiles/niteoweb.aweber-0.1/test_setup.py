...
# [GCC 8.4.0]
# Embedded file name: /Users/zupo/work/niteoweb.plr/src/niteoweb.aweber/src/niteoweb/aweber/tests/test_setup.py
# Compiled at: 2013-02-09 05:24:43
...
class TestInstall(IntegrationTestCase):
    ...
    def test_disable_registration_for_anonymous(self):
        ...
        self.assertFalse('Add portal member' in [ r['name'] for r in self.portal.permissionsOfRole('Anonymous') if r['selected']
                                                ])

    def test_aweber_controlpanel_available(self):
        """Test if aweber control panel configlet is available."""
        view = getMultiAdapter((self.portal, self.portal.REQUEST), name='aweber-settings')
    ...
    def assert_record(self, record_name):
        ...
        record = registry.records[('niteoweb.aweber.interfaces.IAweberSettings.{0}').format(record_name)]
        self.assertIn(record_name, IAweberSettings)
        record.value = 'testvalue'
        self.assertEquals(record.value, 'testvalue')
