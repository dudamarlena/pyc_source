...
# [GCC 8.4.0]
# Embedded file name: /Users/zupo/work/niteoweb.plr/src/niteoweb.aweber/src/niteoweb/aweber/controlpanel.py
# Compiled at: 2013-02-08 05:19:32
...
def set_list_names(widgets):
    ...
    api.portal.set_registry_record('niteoweb.aweber.available_lists_record', [ l.name for l in account.lists ])
...
def parse_auth_code(widgets):
    ...
    api.portal.set_registry_record('niteoweb.aweber.interfaces.IAweberSettings.consumer_key', c_key)
    api.portal.set_registry_record('niteoweb.aweber.interfaces.IAweberSettings.consumer_secret', c_secret)
    api.portal.set_registry_record('niteoweb.aweber.interfaces.IAweberSettings.access_token', unicode(a_token))
    api.portal.set_registry_record('niteoweb.aweber.interfaces.IAweberSettings.access_secret', unicode(a_secret))
    api.portal.set_registry_record('niteoweb.aweber.interfaces.IAweberSettings.authorization_code', '')


class AweberSettingsEditForm(controlpanel.RegistryEditForm):
    """Form for configuring niteoweb.aweber."""
    schema = IAweberSettings
    label = _('Aweber settings')
    description = _("Configure integration with Aweber API.<br>\n    1. Enter App ID and click 'Get auth code'<br>\n    2. Follow the link on top of the page<br>\n    3. Enter your credentials\n    and copy over authorization code to second field<br>\n    4. Click second button to parse authorization code and update list names\n    <br>\n    5. Click Save button!\n    ")
    ...
    def handle_get_auth_action(self, action):
        ...
        url = ('https://auth.aweber.com/1.0/oauth/authorize_app/{0}').format(app_id)
        api.portal.show_message(message=("Visit '{0}' and copy authorization code to Authorization Code field").format(url), request=self.request)

    @button.buttonAndHandler(_('Parse auth code and update lists'), name='parse_auth')
    ...
    def handle_subscribe_user_action(self, action):
        ...
        try:
            aweberapi.subscribe_to_aweber_mailinglist(self.widgets['subscribe_email'].value, self.widgets['subscribe_fullname'].value)
            self.widgets['subscribe_email'].value = ''
            self.widgets['subscribe_fullname'].value = ''
        except Exception as e:
            api.portal.show_message(message=e.message, request=self.request, type='error')
    ...
    def reload_settings_page(self):
        ...
        self.context.REQUEST.response.redirect(api.portal.get().absolute_url() + '/@@aweber-settings')
