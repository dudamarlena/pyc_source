...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toml.py
# Compiled at: 2018-09-09 00:09:42
...
def _load_inline_object(line, currentlevel, _dict, multikey=False, multibackslash=False):
    ...
    while len(candidate_groups) > 0:
        ...
        if value[0] == value[(-1)] and value[0] in ('"', "'") or value[0] in '-0123456789' or value in ('true',
                                                                                                        'false') or value[0] == '[' and value[(-1)] == ']' or value[0] == '{' and value[(-1)] == '}':
...
def _load_array(a, _dict):
    ...
    if '[' not in a[1:-1] or '' != a[1:-1].split('[')[0].strip():
        ...
        if strarray:
            while b < len(a) - 1:
                ab = a[b].strip()
                while not _bounded_string(ab) or len(ab) > 2 and ab[0] == ab[1] == ab[2] and ab[(-2)] != ab[0] and ab[(-3)] != ab[0]:
                    a[b] = a[b] + ',' + a[(b + 1)]
                    ab = a[b].strip()
                    if b < len(a) - 2:
                        a = a[:b + 1] + a[b + 2:]
    ...
    for section in o:
        ...
        if not isinstance(o[section], dict):
            ...
            if arrayoftables:
                ...
                retstr += qsection + ' = ' + unicode(_dump_value(o[section])) + '\n'
