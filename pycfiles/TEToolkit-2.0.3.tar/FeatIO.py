...
# [GCC 8.4.0]
# Embedded file name: /home/yjin/Workspace/DiffChip/DiffChip2/DIFFCHIP/IO/FeatIO.py
# Compiled at: 2014-02-10 21:55:01
...
class PeakIO():
    ...
    def add(self, chromosome, start, end, summit=None, peak_height=None, number_tags=None, pvalue=None, fold_enrichment=None, fdr=None):
    ...
    def filter_fc(self, fc_low, fc_up=None):
        ...
        if fc_up:
            for chrom in chrs:
                new_peaks[chrom] = [ p for p in peaks[chrom] if p[7] >= fc_low if p[7] < fc_up ]
        ...
        for chrom in chrs:
            ...
            for i in xrange(len(peaks_chr)):
                if not prev_peak:
                    ...
                    continue
                elif peaks_chr[i][0] <= prev_peak[1]:
                    ...
                    prev_peak = (
                     s_new_peak, e_new_peak, l_new_peak, summit_new_peak, h_new_peak, peaks_chr[i][5] + prev_peak[5])
                else:
                    n_append(prev_peak)
                    prev_peak = peaks_chr[i]
...
class FWTrackII():
    ...
    def add_loc(self, chromosome, fiveendpos, strand, cnt):
        ...
        if not self.__pluscounts.has_key(chromosome):
            self.__minuscounts[chromosome] = []
            self.__pluscounts[chromosome] = []
        if strand == 0:
    ...
    def get_locations_by_chr(self, chromosome):
        ...
        if self.__pluscounts.has_key(chromosome):
            return (
             self.__pluscounts[chromosome], self.__minuscounts[chromosome])
    ...
    def get_counts_by_chr(self, chromosome, strand=0):
        """Return a tuple of two lists of locations for certain chromosome.
    ...
    def merge_plus_minus_locations_naive(self):
        ...
        for chrom in self.__pluscounts.keys():
            ...
            self.__pluscounts[chrom] = sorted(self.__pluscounts[chrom])
...
class FWTrackIII():
    ...
    def __init__(self, fw=0, anno=''):
        ...
        self.fw = fw
        self.chr = ''
        self.__locations = [array(BYTE4, []), array(BYTE4, [])]
        self.__pluscounts = []
        self.__minuscounts = []
        self.__sorted = False
        self.total = 0
