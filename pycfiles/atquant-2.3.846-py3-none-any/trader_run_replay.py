...
# [GCC 8.4.0]
# Embedded file name: E:\2.3BranchA\ToolBox\PythonToolBox\atquant\run_mode\trader_run_replay.py
...
from atquant.utils.datetime_func import int_date_predate
...
@apply_rule(verify_that('StrategyName').is_instance_of(str), verify_that('TradeFun').is_instance_of(types.FunctionType), verify_that('varFunParameter').is_instance_of(tuple), verify_that('AccountList').is_instance_of(list).is_empty_or_not(empty=False), verify_that('TargetList').is_instance_of(list).is_empty_or_not(empty=False), verify_that('KFrequency').is_valid_frequency(), verify_that('KFreNum').is_instance_of(int).is_greater_or_equal_than(1), verify_that('BeginDate').is_valid_date().is_future_date(), verify_that('RepalyDate').is_valid_date().is_future_date(), verify_that('FQ').is_valid_fq())
def traderRunReplayV2(StrategyName, TradeFun, varFunParameter, AccountList, TargetList, KFrequency, KFreNum, BeginDate, RepalyDate, FQ, AlgoTradeFun=None, varAlgoFunParameter=None):
    try:
        try:
            assert False, 'traderRunReplayV2, not support now!'
            write_syslog('Begin Prepare traderRunReplayV2', level='info', trace_debug=True)
            TradeFun = trace_time(TradeFun)
            KFrequency = KFrequency.lower()
            if KFrequency == 'tick':
                raise NotImplementedError('tick data not support now!')
            if KFrequency in ('sec', 'day', 'week', 'month', 'year'):
                raise NotImplementedError('%s refresh, coming soon' % KFrequency)
            ...
            if len(AccountList) != 1:
                raise ValueError(const_da.Enum_Const.ERROR_NOTSUPPORT_MULTI_ACCOUNT_REPLAYMODE.value)
            if BeginDate >= RepalyDate:
                raise ValueError(const_da.Enum_Const.ERROR_INPUT_BEGINDATE_REPLAYDATE.value)
            right_info = tb_command.atSendCmdGetLoginUserRight()
            if 0 == right_info:
                ToolBoxErrors.atmode_notsupport_error(const_da.Enum_Const.ERROR_NOTSUPPORT_REPLAY_VERSION.value)
            AccountChecker.check_run_replay_account(AccountList)
            ...
            ToolBoxErrors.at_mode_error(mode, 'backreplay')
        ...
        write_syslog('End Prepare traderRunReplayV2', level='info', trace_debug=True)

    try:
        try:
            use_time = SimpleTimer('traderRunReplayV2', 'sec')
            ReplayBeginTime = str(RepalyDate) + 'T000000.000'
            ReplayEndTime = ReplayBeginTime
            ...
            atRealTradeInitV2(TargetList, KFrequency, KFreNum, BeginDate, int_date_predate(RepalyDate), FQ)
            write_syslog('End RealTradeInitV2', level='info', trace_debug=True)
            tb_command.atSendCmdStartReplay(GVAR.g_ATraderStrategyName, ReplayBeginTime, ReplayEndTime, TargetList, KFrequency, KFreNum, 0)
...
def clear_up():
    try:
        ...
        tb_command.atSendCmdStopReplay()
    except Exception as e:
