...
# [GCC 8.4.0]
# Embedded file name: /tmp/Divmod-release/Imaginary/imaginary/pyparsing.py
# Compiled at: 2007-08-17 01:46:38
"""
...
import string, copy, sys, warnings, re
    ...
    def getName(self):
        ...
        if self.__name:
            ...
            if self.__parent:
                ...
                if len(self) == 1 and len(self.__tokdict) == 1 and self.__tokdict.values()[0][0][1] in (0,
                                                                                                        -1):
...
class ParserElement(object):
    ...
    def __add__(self, other):
        ...
        if not isinstance(other, ParserElement):
            warnings.warn('Cannot add element of type %s to ParserElement' % type(other), SyntaxWarning, stacklevel=2)
...
class White(Token):
    r"""Special matching class for matching whitespace.  Normally, whitespace is ignored
...
_printables_less_backslash = ('').join([ c for c in printables if c not in '\\]' ])
...
def _makeTags(tagStr, xml):
    ...
    if xml:
        ...
        openTag = Suppress('<') + Keyword(tagStr, caseless=True) + Dict(ZeroOrMore(Group(tagAttrName.setParseAction(downcaseTokens) + Suppress('=') + tagAttrValue))) + Optional('/', default=[False]).setResultsName('empty').setParseAction(lambda s, l, t: t[0] == '/') + Suppress('>')
...
alphas8bit = srange('[\\0xc0-\\0xd6\\0xd8-\\0xf6\\0xf8-\\0xfe]')
_escapedChar = Regex('\\\\.')
...
cStyleComment = Regex('\\/\\*[\\s\\S]*?\\*\\/').setName('C style comment')
htmlComment = Regex('<!--[\\s\\S]*?-->')
restOfLine = Regex('.*').leaveWhitespace()
dblSlashComment = Regex('\\/\\/.*').setName('// comment')
cppStyleComment = Regex('(\\/\\*[\\s\\S]*?\\*\\/)|(\\/\\/.*)').setName('C++ style comment')
