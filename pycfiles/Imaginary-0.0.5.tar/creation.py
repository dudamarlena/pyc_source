...
# [GCC 8.4.0]
# Embedded file name: /tmp/Divmod-release/Imaginary/imaginary/creation.py
...
def creationSuccess(player, creation):
    ...
    return events.Success(actor=player, target=creation, actorMessage=language.Sentence(['You create ', phrase, '.']), targetMessage=language.Sentence([player, ' creates you.']), otherMessage=language.Sentence([player, ' creates ', phrase, '.']))
...
class Create(NoTargetAction):
    ...
    registry.
    """
    expr = Literal('create') + Optional(White() + (Literal('an') | Literal('a') | Literal('the')).setResultsName('article')) + White() + targetString('typeName') + White() + Literal('named') + White() + targetString('name') + Optional(White() + restOfLine.setResultsName('description'))
    ...
    def do(self, player, line, typeName, name, description=None, article=None):
        ...
        for plug in getPlugins(IThingType, imaginary.plugins):
            if plug.type == typeName:
                proper = article == 'the'
                o = plug.getType()(store=player.store, name=name, description=description, proper=proper)
                break
        else:
            raise ActionFailure(events.ThatDoesntMakeSense(actor=player.thing, actorMessage=language.ExpressString("Can't find " + typeName + '.')))
...
class ListThingTypes(NoTargetAction):
    ...
    def do(self, player, line):
        ...
        """
        events.Success(actor=player.thing, actorMessage=[ (t, '\n') for t in listThingTypes() ]).broadcast()