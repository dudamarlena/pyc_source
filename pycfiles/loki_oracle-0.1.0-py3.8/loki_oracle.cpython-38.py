...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/loki_oracle.py
# Compiled at: 2020-01-20 22:19:41
...
if __name__ == '__main__':
    ...
    for env in ('db_group', 'db_uname', 'db_role', 'db_inst', 'db_version', 'db_alert_file_path',
                'loki_host'):
        if os.environ.get(env) is None:
            print('env {} is Nnone.'.format(env))
            sys.exit(1)
    else:
        db_alert_file_timestamp_regex = '^[A-Za-z]{3}\\s[A-Za-z]{3}\\s[0-9]{2}\\s[0-9]{2}:[0-9]{2}:[0-9]{2}\\s[0-9]{4}$'
        if db_version in (9, 10):
            db_alert_file_timestamp_regex = '^[A-Za-z]{3}\\s[A-Za-z]{3}\\s[0-9]{2}\\s[0-9]{2}:[0-9]{2}:[0-9]{2}\\sCST\\s[0-9]{4}$'
        main()