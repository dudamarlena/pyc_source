...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/tools/bugtracker/widgets/bugtracker.py
# Compiled at: 2020-04-15 10:42:37
# Size of source mod 2**32: 34497 bytes
"""
...
import os, sys, getpass, logging, platform, datetime, traceback, subprocess, webbrowser, collections
try:
...
class ArtellaBugTracker(artellapipe.ToolWidget):
    ...
    def has_tool(self):
        if self._tool:
            if hasattr(self._tool, 'config'):
                return True
        return False

    def get_current_data(self):
        data = {'department':self._departments_combo.currentText(), 
         'tool':{'name':self._tools_combo.currentData().get('name'), 
    ...
    def _fill_combos(self):
        ...
        if self._tool is not None:
            hasattr(self._tool, 'config') or LOGGER.warning('Given Tool is not a valid one. Specify manually the tool in the Bug Tracker ...')
        else:
            ...
            if tool_name:
                ...
                if tool_info:
                    ...
                    LOGGER.warning('Impossible to retrieve tool information. Specify manually the tool in the Bug Tracker ...')
            else:
...
class BugWidget(base.BaseWidget, object):
    ...
    def _get_cpu_info(self):
        ...
        try:
            ...
            out = subprocess.check_output(('python {}'.format(cpuinfo_py)), creationflags=134217728)
    ...
    def _get_gpu_info(self):
        ...
        try:
            ...
            for gpu in gpus_list:
                gpu_info['gpus'][gpu.uuid] = {'name':gpu.name,  'driver':gpu.driver, 
    ...
    def _get_disk_usage(self):
        try:
            ...
            def get_usage_and_free_percentages(total, used, free):
                return (
                 str(round(used / total * 100, 2)), str(round(free / total * 100, 2)))
            ...
            for p in paths_to_get_disk_usage_of:
                ...
                if not usage:
                    continue
                usage_percentage, free_percentage = get_usage_and_free_percentages(usage.total, usage.used, usage.free)
                disk_usage_dict['drives'][path_drive] = {'total':bytes2human(usage.total), 
    ...
    def _fill_bug_data(self):
        ...
        def _add_drives_info(row, column):
            ...
            for drive_letter, drive_info in drives.items():
                ...
                title_lbl.setStyleSheet('font-weight: bold')
                title_lbl.setAlignment(Qt.AlignRight)
                data_text = '{} of {} ({}%)'.format(drive_info['used'], drive_info['total'], drive_info['usage_percentage'])
                data_lbl = QLabel(data_text)
                data_lbl.setToolTip(data_text)
                data_lbl.setStatusTip(data_text)
                data_lbl.setStyleSheet('background-color: rgba(45, 85, 45, 50);')
        ...
        def _add_gpu_info(row, column):
            ...
            for i, gpu_id in enumerate(gpus.keys()):
                ...
                title_lbl.setStyleSheet('font-weight: bold')
                title_lbl.setAlignment(Qt.AlignRight)
                data_text = '{} of {} ({}%)'.format(str(round(gpus[gpu_id]['memoryUsed'] / 1000, 2)) + 'GB', str(round(gpus[gpu_id]['memoryTotal'] / 1000, 2)) + 'GB', str(round(float(gpus[gpu_id]['memoryUtil']) * 100, 2)))
                data_lbl = QLabel(data_text)
                data_lbl.setToolTip(data_text)
                data_lbl.setStatusTip(data_text)
                data_lbl.setStyleSheet('background-color: rgba(45, 85, 45, 50);')
    ...
    def _send_email_bug(self):
        if not self._project:
            ...
            if not self._project.emails:
                LOGGER.warning('Impossible to send bug because there is no emails defined in the project: {}'.format(project_name))
                return
            project_name = self._project.name.title()
    ...
    def _get_bug_data_for_sentry(self):
        ...
        return (
         os_data, cpu_data, gpu_data, disk_data)
        ...
        if not self._project:
            ...
            if not self._main:
                ...
                if not tool_name or not department:
                    LOGGER.warning('Impossible to send request because tool name ({}) or department are not valid ({})'.format(tool_name, department))
                    return False
