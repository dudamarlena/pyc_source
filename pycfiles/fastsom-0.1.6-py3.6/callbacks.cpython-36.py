...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fastsom/learn/callbacks.py
# Compiled at: 2020-04-24 11:45:18
# Size of source mod 2**32: 12421 bytes
"""
Callbacks for SOM.
"""
...
class SomTrainer(Callback):
    __doc__ = 'Base class for SOM training strategy callbacks.'
...
class TwoPhaseSomTrainer(SomTrainer):
    ...
    def __init__(self, model: Som, init_method: str, lr: Collection[float], *args, **kwargs) -> None:
        ...
        self._finetune_start = None
        self._rough_radiuses = None
        self._finetune_radiuses = None
    ...
    def _parameters(self):
        ...
        if self.init_method == 'random':
            ...
            if self.init_method.split('_')[0] == 'kmeans':
                rough_radius_divider_start = 8.0
                rough_radius_divider_end = 4.0
                fine_radius_divider_start = 36.0
                fine_radius_divider_end = np.inf
                rough_neigh_fn = neigh_gauss
                fine_neigh_fn = neigh_gauss
        return (
         rough_radius_divider_start, rough_radius_divider_end, rough_neigh_fn, fine_radius_divider_start, fine_radius_divider_end, fine_neigh_fn)
...
class OneCycleEmulatorSomTrainer(SomTrainer):
    __doc__ = 'Training callback that emulates the One Cycle LR growth.'
...
default_early_stopping_thresholds = dict(topologic_err=10.0,
  codebook_err=0.02)

class SomEarlyStoppingCallback(Callback):
    __doc__ = 'Performs early stopping checks based on given metric-wise thresholds.'
    ...
    def on_epoch_end(self, **kwargs):
        ...
        return {'stop_training': all(should_stop)}
