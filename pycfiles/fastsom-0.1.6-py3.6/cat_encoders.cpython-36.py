...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fastsom/datasets/cat_encoders.py
...
class CatEncoder(Callable):
    __doc__ = 'Transforms categorical features into continuous and back.'
...
class FastTextCatEncoder(CatEncoder):
    ...
    def __init__(self, cat_names, cont_names, *args, **kwargs):
    ...
    def fit(self, x_cat: np.ndarray, **kwargs) -> None:
        ...
        self._ft = FastText(sentences=(sentences.tolist()), size=(self.emb_size), window=(x_cat.shape[(-1)]),
          batch_words=(self.bs),
          min_count=1,
          sample=0,
          sg=1,
...
class CatEncoderType(Enum):
    __doc__ = 'Enum used to pick CatEncoders.'
    FAST_TEXT = 'fasttext'
    ONE_HOT = 'onehot'
...
def get_cat_encoder(tfm: CatEncoderTypeOrString, *tfm_args, **tfm_kwargs) -> Optional[CatEncoder]:
    ...
    if enum_eq(CatEncoderType.FAST_TEXT, tfm):
        ...
        if enum_eq(CatEncoderType.ONE_HOT, tfm):
