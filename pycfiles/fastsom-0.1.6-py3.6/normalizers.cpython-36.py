...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fastsom/datasets/normalizers.py
...
class VarianceNormalizer(Normalizer):
    __doc__ = 'Normalizer that uses mean and standard deviation.'
    ...
    def denormalize(self, data: Tensor) -> Tensor:
        ...
        def denorm(a, idx):
            return self.stds[idx] * a + self.means[idx]
...
class MinMaxScaler(Normalizer):
    __doc__ = 'Rescales data between a minimum and a maximum'
...
_NORMALIZERS = {'var':VarianceNormalizer(), 
 'minmax':MinMaxScaler(), 
