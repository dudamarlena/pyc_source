...
# Embedded file name: /Users/bryanbriney/git/abtools/abtools/_compare.py
# Compiled at: 2017-07-08 02:15:46
from __future__ import print_function
from collections import Counter
import itertools, math, multiprocessing as mp, os, random, sqlite3
from StringIO import StringIO
import subprocess as sp, sys, tempfile, time, urllib, uuid, numpy as np, pandas as pd, matplotlib as mpl, matplotlib.pyplot as plt, seaborn as sns
from abtools import log, mongodb

def parse_args():
    import argparse
    parser = argparse.ArgumentParser('Compares two or more antibody repertoires.')
    parser.add_argument('-o', '--output', dest='output', required=True, help='Output directory for figure and data files. Required.')
    parser.add_argument('-l', '--log', dest='log', help='Location for the log file.                         If not provided, log will be written to <output>/abcompare.log')
    parser.add_argument('-d', '--database', dest='db', required=True, help='Name of the MongoDB database to query. Required.')
    parser.add_argument('-1', '--collection1', dest='collection1', default=None, help='Name of the first MongoDB collection to query for comparison.                         If -1 and -2 are not provided, all collections in the given database will be processed iteratively.')
    parser.add_argument('-2', '--collection2', dest='collection2', default=None, help='Name of the second MongoDB collection to query for comparison.                         If not provided, the collection provided by -1 will be iteratively compared with all other collections in the database.                         If both -1 and -2 are not provided, all collections in the given database will be processed iteratively.')
    parser.add_argument('--collection-prefix', dest='collection_prefix', default=None, help="If supplied, will iteratively process only collections beginning with <collection_prefix>.                        if '-1' is also provided, it will be iteratively compared with all collections beginning with <collection_prefix>.")
    parser.add_argument('-i', '--ip', dest='ip', default='localhost', help="IP address for the MongoDB server.  Defaults to 'localhost'.")
    parser.add_argument('--port', dest='port', default=27017, type=int, help='Port for the MongoDB server.  Defaults to 27017.')
    ...
    parser.add_argument('-k', '--chunksize', dest='chunksize', type=int, default=100000, help='Number of sequences to use for each similarity calculation.                         Default is 100,000.')
    parser.add_argument('-I', '--iterations', dest='iterations', type=int, default=1000, help='Number of iterations of the similarity calculation to perform.                         Default is 10000.')
    parser.add_argument('-s', '--similarity-method', dest='method', choices=[
     'morisita-horn', 'kullback-leibler', 'jensen-shannon', 'jaccard', 'bray-curtis', 'renkonen', 'cosine'], default='morisita-horn', help="The similarity calculation to use.                         Options are 'morisita-horn', 'kullback-leibler' and 'jensen-shannon'.                         Note that kullback-leibler is actually a divergence measure (lower values indicate greater similarity)                         Default is 'morisita-horn'.")
    parser.add_argument('-c', '--control', dest='control_similarity', default=False, action='store_true', help='Plot the control similarity of the two collections')
    parser.add_argument('-C', '--chain', dest='chain', default='heavy', choices=['heavy', 'kappa', 'lambda'], help="Select the antibody chain to analyze.                         Options are 'heavy', 'kappa', and 'lambda'.                         Default is 'heavy'.")
    parser.add_argument('--debug', dest='debug', action='store_true', default=False, help='If set, will run in debug mode.')
    return parser


class Args(object):

    def __init__(self, output=None, log=None, db=None, collection1=None, collection2=None, collection_prefix=None, ip='localhost', port=27017, user=None, password=None, chunksize=100000, iterations=10000, method='marisita-horn', control_similarity=False, chain='heavy', debug=False):
        ...
        if chain not in ('heavy', 'kappa', 'lambda'):
            logging.info('\nERROR: Please select an appropriate chain.                 Valid choices are: heavy, kappa and lambda.\n')
            sys.exit(1)
        self.chain = chain
        self.debug = debug
...
def get_collection_pairs(db, args):
    if args.collection1 and args.collection2:
        ...
        return [
         (
          args.collection1, args.collection2)]
...
def query(db, collection, chain):
    ...
    sys.stdout.flush()
    c = db[collection]
    results = c.find({'chain': chain, 'prod': 'yes'}, {'_id': 0, 'v_gene.gene': 1, 'cdr3_len': 1})
...
def normalize(s1, s2):
    return (
     s1 / np.sum(s1), s2 / np.sum(s2))


def simdif_method(sample1, sample2, args):
    methods = {'morisita-horn': (mh_similarity, False, False), 'kullback-leibler': (
                          kl_divergence, True, True), 
       'jensen-shannon': (
                        js_similarity, False, True), 
       'jaccard': (
                 jaccard_similarity, False, False), 
       'bray-curtis': (
                     bc_similarity, False, True), 
       'renkonen': (
                  renkonen_similarity, False, True), 
       'cosine': (
                cosine_similarity, False, False)}
    method, continuous, normalize = methods[args.method]
    s1, s2 = random_sample_no_replacement(sample1, sample2, continuous, normalize, args)
    return method(s1, s2)
...
def calculate_similarities(s1, s2, args):
    ...
    return (
     median, counts, bins, similarities)
...
def make_sim_plot(counts, bins, median, ofile, output_dir):
    ...
    ax.text(text_xpos, text_ypos, ('median: {}').format(med_str), color='r', weight='semibold')
...
def write_data(s1, s2, median, counts, bins, similarities, ofile, args):
    ...
    ostring += '\n' + '=' * len(ostring) + '\n\n'
    ostring += ('method: {} {}\n\n').format(args.method.title(), 'divergence' if args.method == 'kullback-leibler' else 'similarity')
    ostring += ('median: {}\n\n').format(median)
...
def print_collection_info(collection):
    coll_string = ('{0}{1}{0}').format('               ', collection)
    logger.info('')
    logger.info('')
...
def print_multiple_pairs_info(pairs):
    logger.info('')
    collections = sorted(list(set([ c for tup in pairs for c in tup ])))
    logger.info(('Found {} collections: {}, and {}').format(len(collections), (', ').join(collections[:-1]), collections[(-1)]))
    logger.info('')
...
def main(args):
    ...
    for pair in pairs:
        ...
        median, counts, bins, similarities = calculate_similarities(s1_vgenes, s2_vgenes, args)
        ...
        if args.control_similarity:
            ...
            cmedian, ccounts, cbins, csimilarities = calculate_control_similarities(s1_vgenes, s2_vgenes, args)
