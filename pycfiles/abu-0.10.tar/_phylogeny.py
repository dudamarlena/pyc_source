...
# Embedded file name: /Users/bryanbriney/git/abtools/abtools/_phylogeny.py
# Compiled at: 2017-04-04 01:42:24
import os, subprocess as sp, sys, matplotlib as mpl, seaborn as sns
...
def parse_args():
    ...
    parser.add_argument('-i', '--input', dest='input', required=True, help='Input file, containing sequences in FASTA format with the timepoint appended to the sequence ID.                         Required.')
    parser.add_argument('-o', '--output', dest='output', required=True, help='Output directory for the FASTA, alignment, tree and figure files.                         Required')
    parser.add_argument('-r', '--root', dest='root', required=True, help='FASTA file containing a single sequence that will be used to root the phylogenetic tree.                         Required.')
    parser.add_argument('-m', '--mabs', dest='mabs', default=None, help='FASTA file containing sequences of monoclonal antibodies of the appropriate chain.')
    parser.add_argument('-a', '--alignment', dest='alignment', default=None, help="Provide a pre-aligned dataset.                         Ensure that the root sequence is labeled as 'root' and that all mAb sequences are pre-pended with 'mab_'.                         If --alignment is provided, -i, -r and -m are ignored.")
    parser.add_argument('-n', '--newick', dest='newick', default=None, help="Provide a pre-generated Newick tree file.                         Ensure that the root sequence is labeled as 'root' and that all mAb sequences are pre-pended with 'mab_'.                         If --newick is provided -i, -r, -m and -a are ignored.")
    parser.add_argument('-s', '--sample_id', dest='sample_id', default=None, help='Sample ID. If not provided, the sample ID will be inferred from the input file.')
    parser.add_argument('-t', '--timepoints', dest='timepoints', default=None, help="Tab-delimited file, of the following format (one per line):                         TimepointName    TimepointOrder    TimepointColor                        TimepointName is the name appended to the sequences in the input file.                        TimepointOrder is an integer that indicates the order in which the timepoints should be sorted.                        TimepointColor is an RGB or hex value that will be used to color the phylogenetic tree.                        If mAb sequences are provided, the 'mab' entry will be used to sort/color the mAb sequences.                        If not provided, colors will be automatically selected and timepoints will be determined by a simple                         sort of the raw timepoint values parsed from the input file.")
    parser.add_argument('-A', '--aa', dest='is_aa', default=False, action='store_true', help='If used, all input files contain amino acid sequences.                         Default is nucleotide sequences.')
    parser.add_argument('-D', '--delimiter', dest='delimiter', default='_', help="Delimiter that separates the timepoint and sequence ID.                         Cannot use ':', ';' or '=', since these can screw up the tree file.                         Default is '_'.")
    parser.add_argument('-S', '--scale', dest='scale', default=None, help='Scale for the resulting ete2 tree.                         If not provided, the ete2 default value will be used.')
    parser.add_argument('--label-nodes', dest='label_nodes', default='mab', help='Type of nodes to be labeled.                         Options are: all, none, no-root, mab, input, and root.')
    parser.add_argument('--label-fontsize', dest='label_fontsize', default=12, help='Fontsize for node lables. Default is 12.')
    parser.add_argument('-b', '--branch-vertical-margin', dest='branch_vertical_margin', default=None, help='Branch vertical margin for the resulting ete2 tree.                         If not provided, the ete2 default value will be used..')
    parser.add_argument('--sequence-key', dest='sequence_key', default='vdj_nt', help="If providing a list of sequence dicts, the dict key to use                         as the sequence for alignment and phylogeny.                         Default is 'vdj_nt'.")
    parser.add_argument('--name-key', dest='name_key', default='seq_id', help="If providing a list of sequence dicts, the dict key to use                         as the sequence name for alignment and phylogeny.                         Default is 'seq_id'.")
    parser.add_argument('--orientation', dest='tree_orientation', default=0, type=int, help='Tree orientation. If 0, tree will be drawn from left to right.                         if 1, tree will be drawn from right to left.                         Default is 0.')
    return parser


class Args(object):

    def __init__(self, input=None, output=None, root=None, mabs=None, alignment=None, newick=None, sample_id=None, is_aa=False, timepoints=None, delimiter='_', label_fontsize=12, label_nodes='mab', scale=None, branch_vertical_margin=None, tree_orientation=0, sequence_key='vdj_nt', name_key='seq_id'):
...
def make_tree(alignment, timepoints, args):
    return tree.make_tree(alignment, timepoints, args.delimiter, args.is_aa, args.scale, args.branch_vertical_margin, args.label_fontsize, args.label_nodes, args.tree_orientation)
