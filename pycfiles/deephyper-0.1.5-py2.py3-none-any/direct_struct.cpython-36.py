...
class DirectStructure(NxStructure):
    ...
    def __init__(self, input_shape, output_shape, *args, **kwargs):
        ...
        if type(input_shape) is tuple:
            ...
            if all(map(lambda x: type(x) is tuple, input_shape)):
    ...
    def connect(self, node1, node2):
        ...
        if not isinstance(node1, Node):
            ...
            raise nx.is_directed_acyclic_graph(self.graph) or StructureHasACycle(f"the connection between {node1} -> {node2} is creating a cycle in the structure's graph.")
