...
# [GCC 8.4.0]
# Embedded file name: /Users/romainegele/Documents/Argonne/deephyper/build/lib/deephyper/search/nas/baselines/ppo2/policies.py
# Compiled at: 2019-07-10 12:45:57
# Size of source mod 2**32: 7638 bytes
import gym, numpy as np, tensorflow as tf
from deephyper.search.nas.baselines.a2c.utils import fc
from deephyper.search.nas.baselines.common import tf_util
from deephyper.search.nas.baselines.common.distributions import make_pdtype
from deephyper.search.nas.baselines.common.input import observation_placeholder, encode_observation
...
from deephyper.search.nas.baselines.ppo2.layers import RNN
...
class PolicyWithValue(object):
    ...
    def __init__(self, env, observations, latent, dones, states=None, estimate_q=False, vf_latent=None, sess=None):
        ...
        **tensors       tensorflow tensors for additional attributes such as state or mask
        ...
        self.step_input = {'observations':observations,  'dones':self.dones}
        self.step_output = {'actions':self.action, 
         'values':self.value, 
        ...
        if self.states:
            ...
            self.step_output.update({'states':self.states['current'],  'next_states':self.states['next']})
        else:
    ...
    def step(self, **kwargs):
        return self.sess.run((self.step_output), feed_dict=(self.feed_dict)(**kwargs))

    def values(self, **kwargs):
        return self.sess.run({'values': self.value}, feed_dict=(self.feed_dict)(**kwargs))
...
def build_ppo_policy(env, policy_network, value_network=None, estimate_q=False, **policy_kwargs):
    ...
    def policy_fn(nbatch=None, nsteps=None, sess=None, observ_placeholder=None):
        ...
        with tf.variable_scope('current_rnn_memory'):
            ...
            if state_size > 0:
                state_placeholder = tf.placeholder(dtype=(tf.float32), shape=(nbatch, state_size), name='states')
                state_map['policy'] = state_placeholder[:,
                 0:policy_memory_size]
        ...
        with tf.variable_scope('policy_latent', reuse=(tf.AUTO_REUSE)):
            if isinstance(policy_network, RNN):
                ...
                policy_latent, next_policy_state = policy_network(encoded_x, dones, state_map['policy'])
        ...
        with tf.variable_scope('value_latent', reuse=(tf.AUTO_REUSE)):
            if value_network_ == 'shared':
                ...
                if isinstance(value_network_, RNN):
                    ...
                    value_latent, next_value_state = value_network_(encoded_x, dones, state_map['value'])
        ...
        with tf.name_scope('next_rnn_memory'):
            if state_size > 0:
                ...
                state_info = {'current':state_placeholder,  'next':next_states}
            else:
                state_info = None
