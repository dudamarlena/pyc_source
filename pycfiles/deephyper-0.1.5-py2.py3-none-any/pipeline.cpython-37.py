...
default_callbacks_config = {'ModelCheckpoint':dict(monitor='val_loss',
   mode='min',
   save_best_only=True,
   verbose=1,
   filepath='best_model.h5',
   save_weights_only=False), 
 'EarlyStopping':dict(monitor='val_loss',
   mode='min',
   verbose=1,
   patience=50), 
            ...
            for cb_name, cb_conf in callbacks_config.items():
                if cb_name in default_callbacks_config:
                    ...
                    if cb_name == 'ModelCheckpoint':
                        ...
                        if cb_name == 'TensorBoard':
                            if default_callbacks_config[cb_name]['beholder']:
                                callbacks.append(BeholderCB(logdir=(default_callbacks_config[cb_name]['log_dir']),
                                  sess=sess))
