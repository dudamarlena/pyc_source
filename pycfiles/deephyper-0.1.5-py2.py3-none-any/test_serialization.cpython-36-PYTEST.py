...
# [GCC 8.4.0]
# Embedded file name: /Users/romainegele/Documents/Argonne/deephyper/build/lib/deephyper/search/nas/baselines/common/tests/test_serialization.py
...
import gym, numpy as np, pytest, tensorflow as tf
from deephyper.search.nas.baselines.common.tests.envs.mnist_env import MnistEnv
from deephyper.search.nas.baselines.common.tf_util import make_session, get_session
...
learn_kwargs = {'deepq':{},  'a2c':{},  'acktr':{},  'acer':{},  'ppo2':{'nminibatches':1, 
  'nsteps':10}, 
 'trpo_mpi':{}}
network_kwargs = {'mlp':{},  'cnn':{'pad': 'SAME'}, 
 'lstm':{},  'cnn_lnlstm':{'pad': 'SAME'}}
...
def test_serialization(learn_fn, network_fn):
    ...
    if network_fn.endswith('lstm'):
        if learn_fn in ('acer', 'acktr', 'trpo_mpi', 'deepq'):
            return
    if network_fn.endswith('lstm'):
        if learn_fn == 'ppo2':
    ...
    with tempfile.TemporaryDirectory() as (td):
        ...
        for k, v in variables_dict1.items():
            np.testing.assert_allclose(v, (variables_dict2[k]), atol=0.01, err_msg=('saved and loaded variable {} value mismatch'.format(k)))
...
def test_coexistence(learn_fn, network_fn):
    ...
    if learn_fn == 'deepq':
        return
    if network_fn.endswith('lstm'):
        if learn_fn in ('acktr', 'trpo_mpi', 'deepq'):
            return
