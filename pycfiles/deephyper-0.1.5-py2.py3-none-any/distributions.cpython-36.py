...
# [GCC 8.4.0]
# Embedded file name: /Users/romainegele/Documents/Argonne/deephyper/build/lib/deephyper/search/nas/baselines/common/distributions.py
# Compiled at: 2019-07-10 12:45:57
# Size of source mod 2**32: 13637 bytes
import tensorflow as tf, numpy as np, deephyper.search.nas.baselines.common.tf_util as U
...
class MultiCategoricalPd(Pd):
    ...
    def __init__(self, nvec, flat):
        self.flat = flat
        self.categoricals = list(map(CategoricalPd, tf.split(flat, np.array(nvec, dtype=(np.int32)), axis=(-1))))
...
def make_pdtype(ac_space):
    ...
    if isinstance(ac_space, spaces.MultiBinary):
...
def validate_probtype(probtype, pdparam):
    ...
    if not np.abs(entval - entval_ll) < 3 * entval_ll_stderr:
