...
# [GCC 8.4.0]
# Embedded file name: /Users/romainegele/Documents/Argonne/deephyper/build/lib/deephyper/search/nas/baselines/ppo2/ppo2.py
...
from deephyper.search.nas.baselines.common import explained_variance
from deephyper.search.nas.baselines.common import set_global_seeds
from deephyper.search.nas.baselines.common.tf_util import display_var_info
from deephyper.search.nas.baselines.ppo2.policies import build_ppo_policy
from deephyper.search.nas.baselines.ppo2.runner import Runner
try:
...
def learn(*, network, env, total_timesteps, eval_env=None, seed=None, nsteps=128, ent_coef=0.0, lr=0.0003, vf_coef=0.5, max_grad_norm=0.5, gamma=0.99, lam=0.95, log_interval=10, nminibatches=1, noptepochs=4, cliprange=0.2, save_interval=10, load_path=None, model_fn=None, **network_kwargs):
    """
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    if isinstance(lr, float):
        ...
        elif isinstance(cliprange, float):
        ...
        while 1:
            ...
            if eval_env is not None:
                ...
                _eval_obs = eval_minibatch['observations']
                _eval_returns = eval_minibatch['returns']
                _eval_masks = eval_minibatch['masks']
                _eval_actions = eval_minibatch['actions']
                _eval_values = eval_minibatch['values']
                _eval_neglogpacs = eval_minibatch['neglogpacs']
                _eval_states = eval_minibatch['state']
            ...
            if not update % log_interval == 0:
                if update == 1:
                    ...
                    if MPI is None or MPI.COMM_WORLD.Get_rank() == 0:
                        logger.dumpkvs()
                if save_interval and not update % save_interval == 0:
                    if update == 1:
                        if logger.get_dir() and not MPI is None:
