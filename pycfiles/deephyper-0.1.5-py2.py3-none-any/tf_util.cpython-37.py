...
# [GCC 8.4.0]
# Embedded file name: /Users/romainegele/Documents/Argonne/deephyper/build/lib/deephyper/search/nas/baselines/common/tf_util.py
# Compiled at: 2019-07-10 12:45:57
# Size of source mod 2**32: 17032 bytes
import numpy as np, tensorflow as tf, copy, os, functools, collections, multiprocessing
...
def conv2d(x, num_filters, name, filter_size=(3, 3), stride=(1, 1), pad='SAME', dtype=tf.float32, collections=None, summary_tag=None):
    with tf.variable_scope(name):
        ...
        if summary_tag is not None:
            tf.summary.image(summary_tag, (tf.transpose(tf.reshape(w, [filter_size[0], filter_size[1], -1, 1]), [
             2, 0, 1, 3])),
              max_images=10)
...
def function(inputs, outputs, updates=None, givens=None):
    ...
    if isinstance(outputs, (dict, collections.OrderedDict)):
...
class _Function(object):
    ...
    def __init__(self, inputs, outputs, updates, givens):
        for inpt in inputs:
            if hasattr(inpt, 'make_feed_dict') or type(inpt) is tf.Tensor and not len(inpt.op.inputs) == 0 or False:
                raise AssertionError('inputs should all be placeholders, constants, or have a make_feed_dict method')
...
def get_placeholder(name, dtype, shape):
    if name in _PLACEHOLDER_CACHE:
        ...
        if out.graph == tf.get_default_graph():
            if not (dtype1 == dtype and shape1 == shape):
                raise AssertionError('Placeholder with name {} has already been registered and has shape {}, different from requested {}'.format(name, shape1, shape))
            return out
...
def display_var_info(vars):
    ...
    for v in vars:
        name = v.name
        if not '/Adam' in name:
            if 'beta1_power' in name or 'beta2_power' in name:
                continue
...
def _check_shape--- This code section failed: ---

 L. 407         0  LOAD_CONST               True
                2  RETURN_VALUE     
              4_0  COME_FROM            34  '34'
              4_1  COME_FROM            26  '26'

 L. 411         4  FOR_ITER             42  'to 42'
                6  UNPACK_SEQUENCE_2     2 
                8  STORE_FAST               'i'
               10  STORE_FAST               's_data'

 L. 412        12  LOAD_FAST                'squeezed_placeholder_shape'
               14  LOAD_FAST                'i'
               16  BINARY_SUBSCR    
               18  STORE_FAST               's_placeholder'

 L. 413        20  LOAD_FAST                's_placeholder'
               22  LOAD_CONST               -1
               24  COMPARE_OP               !=
               26  POP_JUMP_IF_FALSE     4  'to 4'
               28  LOAD_FAST                's_data'
               30  LOAD_FAST                's_placeholder'
               32  COMPARE_OP               !=
               34  POP_JUMP_IF_FALSE     4  'to 4'

 L. 414        36  LOAD_CONST               False
               38  RETURN_VALUE     
               40  JUMP_BACK             4  'to 4'
               42  POP_BLOCK        

 L. 416        44  LOAD_CONST               True
               46  RETURN_VALUE     
               -1  RETURN_LAST      
