...
# Embedded file name: build/bdist.linux-i686/egg/Products/SimpleChat/SimpleChat.py
# Compiled at: 2012-01-26 07:50:47
...
class SimpleChat(OFS.Folder.Folder, CAG):
    ...
    def chat_submit(self, text, REQUEST, RESPONSE, ajax=False, stamp='0-0'):
        ...
        if self.is_banned()[0]:
            ...
            ajax or RESPONSE.redirect(self.absolute_url() + '/?message=' + urllib.quote(message) + '&stamp2=' + urllib.quote(stamp) + '#archive-index-' + urllib.quote(stamp))
        else:
            return 'Success!'
    ...
    def get_username(self, request, REQUEST=None):
        ...
        if username:
            ...
            if not username and not request.get('username', ''):
                return (False, '', _('Register username below', request))
            (remote_address_valid, remote_address) = get_remote_address(request)
            if not remote_address_valid:
                return (False, '', _('No remote IP address found, contact site administrator', request))
