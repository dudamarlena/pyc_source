...
# Embedded file name: /Users/mafei/github/python-apptoolkit/apptoolkit/__init__.py
# Compiled at: 2018-11-29 03:48:54
__doc__ = '\n    apptoolkit\n    ~~~~~~~~~~~~\n\n    This module is the main apptoolkit.\n\n    :copyright: (c) 2017 by Ma Fei.\n'
import json, os, platform, re, subprocess, tempfile, xml.etree.cElementTree as ET
...
class ADB(object):
    ...
    def get_battery_status(self):
        ...
        """
        statusDict = {1: 'BATTERY_STATUS_UNKNOWN', 2: 'BATTERY_STATUS_CHARGING', 
    ...
    def get_app_starttotaltime(self, component):
        ...
        """
        time = self.shell('am start -W %s | %s TotalTime' % (component, find_util)).stdout.read().split(': ')[(-1)]
        return int(time)
...
class Device():
    ...
    def get_android_devices():
        ...
        for device_uid in android_devices_list:
            device_info = DeviceInfo(device_uid, 'Android', ADB(device_uid).get_android_version(), ADB(device_uid).get_sdk_version(), ADB(device_uid).get_product_brand(), ADB(device_uid).get_product_model(), ADB(device_uid).get_product_rom())
...
class Element(object):
    ...
    def __bound(self, attrib, name):
        ...
        if not coord:
            raise Exception('Not found this element(%s) in current activity' % name)
        return (
         int(coord[0]), int(coord[1]), int(coord[2]), int(coord[3]))
