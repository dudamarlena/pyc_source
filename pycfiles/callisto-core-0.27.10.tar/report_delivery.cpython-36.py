...
# [GCC 8.4.0]
# Embedded file name: /Users/Ari/src/callisto-core/callisto_core/reporting/report_delivery.py
# Compiled at: 2019-08-07 16:42:06
# Size of source mod 2**32: 16268 bytes
...
def report_as_pdf(report, data, recipient):
    return PDFFullReport(report=report, report_data=data).generate_pdf_report(recipient=recipient,
      report_id=(report.id))
...
class MatchReportContent(object):
    ...
    def from_form(cls, form):
        return cls(identifier=(form.cleaned_data.get('identifier')),
...
class PDFFullReport(PDFReport, ReportPageMixin):
    ...
    def generate_pdf_report(self, report_id, recipient):
        ...
        doc = SimpleDocTemplate(report_buffer,
          pagesize=letter,
          rightMargin=72,
          leftMargin=72,
          topMargin=72,
          bottomMargin=72)
        self.pdf_elements.extend(api.NotificationApi.get_cover_page(report_id=report_id, recipient=recipient))
        self.pdf_elements.extend(self.report_page(self.report))
        self.pdf_elements.append(Paragraph('Record Questions', self.section_title_style))
...
class PDFMatchReport(PDFReport, MatchPageMixin):
    ...
    def names_and_matching_identifiers(self, matches_with_reports):
        names = ', '.join(OrderedDict.fromkeys([match_report_content.perp_name.strip() for _, match_report_content in matches_with_reports if match_report_content.perp_name]))
        if len(names) < 1:
            names = '<i>None provided</i>'
        return Paragraph(f"Name(s): {names}<br/>Matching identifier: {self.identifier}", self.body_style)
    ...
    def generate_match_report(self, report_id, recipient):
        ...
        sorted_matches = sorted((self.matches),
          key=(lambda m: m.added.strftime('%Y-%m-%d %H:%M')), reverse=True)
        match_report_and_report_content = [(match, MatchReportContent(**json.loads(match.get_match(self.identifier)))) for match in sorted_matches]
        buffer = BytesIO()
        doc = SimpleDocTemplate(buffer,
          ...
          bottomMargin=72)
        self.pdf_elements.extend(api.NotificationApi.get_cover_page(report_id=report_id, recipient=recipient))
        self.pdf_elements.append(Paragraph(api.NotificationApi.report_title, self.report_title_style))
...
class PDFUserReviewReport(PDFReport, ReportPageMixin, MatchPageMixin):
    ...
    def cover_page(self):
        return [
         Image(os.path.join(settings.BASE_DIR, api.NotificationApi.logo_path), 3 * inch, 3 * inch),
         Spacer(1, 18),
    ...
    def match_pages_empty_identifier(self, matches):
        match_report_and_report_content = [(match, MatchReportContent.empty()) for match in matches]
