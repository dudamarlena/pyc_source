...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/zinc/zinc_request_processor.py
# Compiled at: 2014-01-24 20:14:21
...
class ZincRequestProcessor(object):
    ...
    @classmethod
    def process(klass, request_type, payload, zinc_base_url='https://api.zinc.io/v0', polling_interval=1.0, retries=2):
        processor = ZincAbstractProcessor()
        try:
            return processor.post_request(payload, request_type)
        except ZincError as e:
            if retries > 0:
                return ZincRequestProcessor.process(request_type, payload, zinc_base_url, polling_interval, retries - 1)
            raise e

    @classmethod
    def process_async(klass, request_type, payload, zinc_base_url='https://api.zinc.io/v0'):
        processor = ZincAbstractProcessor()
...
class ZincAbstractProcessor(object):
    ...
    def __init__(self, zinc_base_url='https://api.zinc.io/v0', polling_interval=1.0, request_timeout=180, get_request_timeout=5.0, post_request_timeout=10.0, get_request_retries=3):
