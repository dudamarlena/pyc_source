...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/zinc/zinc_wizard.py
# Compiled at: 2014-02-04 16:43:50
...
class ZincWizard(object):
    PROMPTS = {'search_query': 'What do you want to buy? (e.g. inflatable banana)', 
    ...
    def __init__(self, retailer='amazon', client_token='public', filename=None, gift=None):
    ...
    def get_product_name(self, response_data):
        ...
        print '\nProcessing request...\n'
        product_name_response = ZincRequestProcessor.process('search_products', {'client_token': self.client_token, 
    ...
    def get_product_variants(self, response_data):
        print '\nProcessing request...\n'
        async_response = ZincRequestProcessor.process_async('variant_options', {'client_token': self.client_token, 
           'retailer': self.retailer, 
    ...
    def get_retailer_credentials(self, response_data):
        ...
        response_data['retailer_credentials'] = {'email': email, 
           'password': password}

    def get_shipping_methods(self, response_data):
        self.shipping_address = self.retrieve_data('shipping_address')
        print '\nProcessing request...\n'
        shipping_response = ZincRequestProcessor.process_async('shipping_methods', {'client_token': self.client_token, 
           'retailer': self.retailer, 
    ...
    def get_store_card(self, response_data):
        ...
        if cc_token == None:
            ...
            print '\nProcessing request...\n'
            store_card_response = ZincRequestProcessor.process('store_card', {'client_token': self.client_token, 
               'retailer': self.retailer, 
    ...
    def get_review_order(self, response_data):
        shipping_method_id = self.select_shipping_methods(self.async_responses['shipping_response'].get_response())
        payment_method = {'prefer_use_gift_balance': True, 
           'cc_token': self.retrieve_data('cc_token'), 
           'security_code': self.get_security_code()}
        is_gift = self.get_is_gift()
        print '\nProcessing request...\n'
        review_order_response = ZincRequestProcessor.process('review_order', {'client_token': self.client_token, 
           'retailer': self.retailer, 
           'products': response_data['products'], 
           'shipping_address': self.shipping_address, 
           'is_gift': is_gift, 
    ...
    def get_place_order(self, response_data):
        ...
        if self.prompt_boolean(self.PROMPTS['place_order']):
            print '\nProcessing request...\n'
            place_order_response = ZincRequestProcessor.process('place_order', {'client_token': self.client_token, 
    ...
    def write_to_zincrc(self):
        ...
        if not os.path.isfile(filename) and self.prompt_boolean(self.PROMPTS['write_to_zincrc']):
            data = {'shipping_address': self.shipping_address, 'cc_token': self.retrieve_data('cc_token')}
    ...
    def get_credit_card_information(self):
        ...
        response = {}
        response['number'] = self.prompt(self.PROMPTS['credit_card']['number'], ValidationHelpers.validate_credit_card())
        response['expiration_month'] = self.prompt(self.PROMPTS['credit_card']['expiration_month'], ValidationHelpers.validate_number(12, 1))
        response['expiration_year'] = self.prompt(self.PROMPTS['credit_card']['expiration_year'], ValidationHelpers.validate_number(2100, 2010))
    ...
    def select_product_name(self, response):
        ...
        collected_number = self.prompt(prompt, ValidationHelpers.validate_number(len(descriptions) - 1))
    ...
    def select_product_variants(self, variants_response, product_info):
        ...
        if len(variants_response['variant_options']) > 1:
            ...
            description_number = self.prompt(prompt, ValidationHelpers.validate_number(len(descriptions) - 1))
    ...
    def select_shipping_methods(self, shipping_response):
        ...
        for i in xrange(len(shipping_response['shipping_methods'])):
            ...
            descriptions.append(str(i) + ') ' + current_method['name'] + ': ' + current_method['description'])
...
class AmazonDataFinder(object):
    ...
    def get_amazon_data(klass, asin):
        ...
        values = {'name': 'Michael Foord', 's': 'cb3dd4783d787a78a2c9a4e18b86a426', 
           'asin_list': asin}
        ...
        try:
            ...
            loaded_page = re.search('This ASIN/ISBN number is invalid', the_page, flags=re.DOTALL | re.IGNORECASE)
        ...
        try:
            ret_price = 0
            ret_description = ''
            price = re.search('<th>Image</th>.*?<th nowrap>Price:</th>\\s*<td nowrap><b>\\$(.*?)</b>', the_page, flags=re.DOTALL | re.IGNORECASE)
            if price:
                ret_price = price.group(1)
                description = re.search('<th>Image</th>.*?<li>([^<]*?)(<a.*?</a>)?</li>\\s*</ul>', the_page, flags=re.DOTALL | re.IGNORECASE)
            if description:
