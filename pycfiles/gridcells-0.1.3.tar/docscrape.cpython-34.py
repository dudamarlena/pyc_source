...
# [GCC 8.4.0]
# Embedded file name: /Users/lukas/work/development/gridcells/external/numpydoc/numpydoc/docscrape.py
# Compiled at: 2014-03-29 16:39:55
# Size of source mod 2**32: 16331 bytes
...
class NumpyDocString(object):
    ...
    def _parse(self):
        ...
        for section, content in self._read_sections():
            ...
            if section in ('Parameters', 'Returns', 'Raises', 'Warns', 'Other Parameters',
                           'Attributes', 'Methods'):
...
class ClassDoc(NumpyDocString):
    ...
    def methods(self):
        if self._cls is None:
            return []
        return [name for name, func in inspect.getmembers(self._cls) if (not name.startswith('_') or name in self.extra_public_methods) and isinstance(func, collections.Callable)]

    @property
    def properties(self):
        if self._cls is None:
            return []
        return [name for name, func in inspect.getmembers(self._cls) if not name.startswith('_') and (func is None or isinstance(func, property) or inspect.isgetsetdescriptor(func))]