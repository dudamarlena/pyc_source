...
# [GCC 8.4.0]
# Embedded file name: /home/magland/src/ephys-viz/generated/ephys_viz/ephys_viz/widgets/PlaceField/PlaceField.py
...
class PlaceField:
    ...
    def set_status(self, status):
        self.set_python_state(dict(status=status))

    def set_status_message(self, msg):
        self.set_python_state(dict(status_message=msg))
...
def _load_epochs(obj):
    ...
    epochs = [dict(id=id, label=id, start_time=(start_times[i]), stop_time=(stop_times[i])) for i, id in enumerate(ids)]
    return epochs
