...
# [GCC 8.4.0]
# Embedded file name: /home/magland/src/ephys-viz/generated/ephys_viz/ephys_viz/widgets/pycommon/autoextractors/autorecordingextractor.py
# Compiled at: 2019-11-20 17:58:20
# Size of source mod 2**32: 7526 bytes
import kachery as ka
from .bandpass_filter import bandpass_filter
import h5py, spikeextractors as se, numpy as np, hashlib, json, h5_to_json as h5j
            ...
            if 'kachery_config' in arg:
                ...
                if 'nwb_path' in arg:
                    ...
                    if path.endswith('.mda'):
                        ...
                        hash0 = _sha1_of_object(dict(timeseries_sha1=(ka.get_file_info(path, algorithm='sha1')['sha1']),
                          samplerate=samplerate))
                        setattr(self, 'hash', hash0)
                    else:
                        if path.endswith('.nwb.json'):
                            self._recording = NwbJsonRecordingExtractor(file_path=path)
                            hash0 = ka.get_file_info(path)['sha1']
                            setattr(self, 'hash', hash0)
                        else:
                            if path.endswith('.json'):
                                obj = path.endswith('.nwb.json') or ka.load_object(path)
                                ...
                                if ka.get_file_info(path + '/raw.mda'):
