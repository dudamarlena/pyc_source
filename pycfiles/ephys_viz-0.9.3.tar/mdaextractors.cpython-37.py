...
# [GCC 8.4.0]
# Embedded file name: /home/magland/src/ephys-viz/generated/ephys_viz/ephys_viz/widgets/pycommon/autoextractors/mdaextractors/mdaextractors.py
# Compiled at: 2019-11-19 14:13:33
# Size of source mod 2**32: 9851 bytes
from spikeextractors import RecordingExtractor
from spikeextractors import SortingExtractor
import kachery as ka, json, numpy as np
...
class MdaRecordingExtractor(RecordingExtractor):
    ...
    def __init__(self, *, recording_directory=None, timeseries_path=None, download=False, samplerate=None, geom=None, geom_path=None, params_path=None):
        ...
        if self._geom.shape[0] != X.N1():
            ...
            self._geom = np.zeros((X.N1(), 2))
        self._hash = ka.get_object_hash(dict(timeseries=(ka.get_file_hash(self._timeseries_path)),
    ...
    @staticmethod
    def write_recording(recording, save_path, params=dict(), raw_fname='raw.mda', params_fname='params.json', _preserve_dtype=False, in_blocks=False):
        if in_blocks:
...
class MdaSortingExtractor(SortingExtractor):
    ...
    def hash(self):
        return ka.get_object_hash(dict(firings=(ka.get_file_hash(self._firings_path)),
...
def is_url(path):
    path = path or ''
    return path.startswith('http://') or path.startswith('https://') or path.startswith('kbucket://') or path.startswith('sha1://') or path.startswith('sha1dir://')


def write_recording_blocks(recording, save_path, params=dict(), raw_fname='raw.mda', params_fname='params.json', _preserve_dtype=False):
    import math
    if not os.path.isdir(save_path):
        os.mkdir(save_path)
    channel_ids = recording.get_channel_ids()
