...
# [GCC 8.4.0]
# Embedded file name: /home/magland/src/ephys-viz/generated/ephys_viz/ephys_viz/widgets/pycommon/nwb_to_dict.py
# Compiled at: 2019-11-15 11:26:28
# Size of source mod 2**32: 5835 bytes
import h5py, numpy as np
from mountaintools import client as mt
from .mdaio import writenpy
import tempfile, shutil, mlprocessors as mlpr, simplejson
    ...
    if type(val) == str:
        ...
        if type(val) == int:
            ...
            if type(val) == np.ndarray:
                if name in ('data', 'timestamps', 'spike_times', 'image_mask', 'faces',
                            'vertices'):
...
def _handle_ndarray(x, *, opts, name, snapshot=False):
    if np.issubdtype(x.dtype, np.number):
        if snapshot:
            ...
            if x.size > 1000:
                raise Exception('Array is too large to include in file (need to use snapshot). name={}, shape={}'.format(name, x.shape))
            list0 = [_handle_list_or_val(val, opts=opts, name=name) for val in x.tolist()]
            return list0
    else:
        list0 = [_handle_list_or_val(val, opts=opts, name=('{}.{}'.format(name, ind))) for ind, val in enumerate(x.tolist())]
        return list0
