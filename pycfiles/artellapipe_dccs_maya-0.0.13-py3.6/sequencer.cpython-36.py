...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/dccs/maya/core/sequencer.py
# Compiled at: 2020-05-13 18:48:54
...
import os, logging, tpDcc.dccs.maya as maya
...
class SequencerShotExporter(object):
    ...
    def export_shot_animation_curves(self, export_file_path, start_frame, end_frame, sequencer_least_key, sequencer_great_key, **kwargs):
        """
        ...
        if not os.path.isdir(export_anim_dir):
            ...
            frames_range = (
             str(int(start_frame)), str(int(end_frame)))
            ...
            export_options = 'precision={};intValue={};nodeNames={};verboseUnits={};whichRange={};'.format(precision, int_value, node_names, verbose_units, which_range)
            export_options += 'range={0}:{0};'.format(frames_range)
            export_options += 'options={};hierarchy={};controlPoints={};shapes={};helpPictures={};'.format(options, hierarchy, control_points, shapes, help_pictures)
            export_options += 'useChannelBox={};copyKeyCmd={}'.format(use_channel_box, copy_key_cmd)
            export_options += '-time >%s:%s> -float >%s:%s> ' % (frames_range + frames_range)
            export_options += '-option {} -hierarchy {} -controlPoints {} -shape {} '.format(option, hierarchy, control_points, shape)
            ...
            insert_remove_keys = animation.InsertRemoveAnimCurveKeys(anim_curves=(self._anim_curves),
              start_frame=start_frame,
              end_frame=end_frame,
              sequence_least_key=sequencer_least_key,
              sequence_great_key=sequencer_great_key)
    ...
    def import_shot_animation_curves(self, import_file_path, start_frame, end_frame, sequence_start_frame, sequence_end_frame, first_frame=False, **kwargs):
        """
        ...
        if not os.path.isfile(import_file_path):
            ...
            LOGGER.info('Set temp key on ALL "{}" animation curve nodes on frame "{}"'.format(self._anim_curves, zero_k))
            range_to_delete = (
             zero_k + 1, all_keyframes[(-1)])
            animation.delete_keys_from_animation_curves_in_range(range_to_delete=range_to_delete,
              anim_curves=(self._anim_curves))
            ...
            maya.cmds.file(import_file_path, i=True, mergeNamespacesOnClash=False, options=import_options, typ='animImport', rpr='IM',
              pr=True,
              ignoreVersion=True,
