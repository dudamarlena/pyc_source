...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/dccs/maya/managers/playblasts.py
...
import logging, contextlib, tpDcc as tp
...
class MayaPlayblastsManager(playblasts.PlayblastsManager, object):
    ...
    def parse_current_scene(self):
        ...
        return {'start_frame':maya.cmds.playbackOptions(query=True, minTime=True), 
    ...
    def _generate_playblast(self, **kwargs):
        ...
        padding = 10
        with gui.create_independent_panel(width=(width + padding),
          height=(height + padding),
          off_screen=off_screen) as (panel):
            tp.Dcc.focus(panel)
            with contextlib.nested(applied_viewport_options(viewport_options, panel, self.config), applied_camera_options(camera_options, panel, self.config), applied_display_options(display_options, self.config), applied_viewport2_options(viewport2_options, self.config), gui.disable_inview_messages(), gui.maintain_camera_on_panel(panel=panel, camera=camera), gui.isolated_nodes(nodes=isolate, panel=panel), gui.reset_time()):
                if format != 'image':
        ...
        output = (maya.cmds.playblast)(compression=compression, 
         format=format, 
         percent=percent, 
         quality=quality, 
         viewer=viewer, 
 ...
 width, height], 
         ...
         framePadding=frame_padding, **extra_args)
        return output
