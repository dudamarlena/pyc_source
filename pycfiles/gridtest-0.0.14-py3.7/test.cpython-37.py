...
# Size of source mod 2**32: 31239 bytes
"""
...
class GridRunner:
            ...
            for test in tests:
                if 'self' in test.get('args', {}):
                    ...
                    if re.search('{{.+}}', test['args']['self']):
                        instance = re.sub('({{|}}|instance[.])', '', test['args']['self']).strip()
    ...
    def run(self, regexp=None, parallel=True, nproc=None, show_progress=True, verbose=False, interactive=False, name=None, cleanup=True, save=None, save_report=None, save_compact=False, save_metrics=None, report_template='report'):
    ...
    def get_grids(self):
        ...
        for parent, section in self.config.items():
            ...
            for name, grid in section.get('grids', {}).items():
                self.grids[name] = Grid(name=name,
                  params=grid,
                  filename=filename,
                  refs=(self.grids))
        ...
        for parent, section in self.config.items():
            for name, module in section.get('tests', {}).items():
                ...
                for entry in module:
                    ...
                    if 'grid' in entry:
                        if entry['grid'] not in self.grids:
                            bot.exit(f"{name} needs grid {entry['grid']} but not found in grids.")
                    if 'grid' in entry:
                        if entry['grid'] in self.grids:
                    ...
                    if not grid:
                        grid = Grid(name=name,
                          ...
                          filename=filename,
                          refs=(self.grids))
                        if 'self' in grid.args:
                            if 'grid' in grid.args['self']:
                                instance_grid = self.grids.get(grid.args['self']['grid'], [{}])
                        argsets = grid
                        if grid.cache:
