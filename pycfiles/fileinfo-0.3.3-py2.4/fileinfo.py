...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/fileinfo/fileinfo.py
# Compiled at: 2008-11-14 04:39:05
...
def loadPlugins():
    ...
    plugins = []
    plugins += plugging.getPluginsFromModule(investigator, investigator.BaseInvestigator)
    plugins += plugging.loadPlugins(investigator.BaseInvestigator, 'fileinfo_inv_plugin_*.py', folder=join(dirname(abspath(__file__)), 'plugins'), envVar=OS_ENVIRON_PLUGIN_VARNAME)
    return plugins
...
def collectAttrs(paths, options):
    ...
    for path in paths:
        ...
        for a in attrVals:
            for fi in fis:
                if a in fi.attrMap.keys() and fi not in fis2:
                    fis2.append(fi)
                matches = [ re.match(an, a) for an in fi.attrMap.keys() if re.match(an, a) ]
                if matches and fi not in fis2:
                    fis2.append(fi)
...
def process(*pathPatterns, **options):
    ...
    if sortVals:
        all_deco = [ [ getattr(rec, sv) for sv in sortVals ] + [rec] for rec in all ]
        all_deco.sort()
    ...
    if mode == 'abs':
        ...
        display = basename
    rows = [ [ getattr(rec, a) for a in attrVals ] + [display(rec.path)] for rec in all ]
...
def displayAsTable(rows):
    ...
    if len(rows) == 1:
        isNumCols = [ reduce(operator.__and__, [ type(c) in (int, long, float) or c == 'n/a' for c in col ]) for col in cols ]
    else:
...
def _main():
    ...
    for (k, v) in opts:
        if k in ('-h', '--help'):
            ...
            at = reduce(operator.add, [ C.attrMap.keys() for C in INVESTIGATOR_CLASSES ], [])
