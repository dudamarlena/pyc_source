...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/fileinfo/plugins/fileinfo_inv_plugin_pdf.py
# Compiled at: 2008-06-17 03:15:20
...
class PDFInvestigator(BaseInvestigator):
    ...
    __module__ = __name__
    attrMap = {'nimgs': 'getNumImages', 'npagesizes': 'getNumPdfPageSizes', 'npageoris': 'getNumPdfPageOrientations', 'title': 'getTitle', 'author': 'getAuthor', 'producer': 'getProducer', 'creationdate': 'getCreationDate', 'npages': 'getNumPdfPages'}
    totals = ('npages', 'nimgs')
    ...
    def getNumImages(self):
        ...
        expr = '\\d+ +\\d+ +obj.*?endobj\\s+(?:%.*?[\\r\\n])?'
        objPat = re.compile(expr, re.M | re.S)
        items = re.findall(objPat, self.content)
        for p in [ re.compile('/%s\\s*/%s' % (k, v), re.M | re.S) for (k, v) in [('Type', 'XObject'), ('Subtype', 'Image')] ]:
