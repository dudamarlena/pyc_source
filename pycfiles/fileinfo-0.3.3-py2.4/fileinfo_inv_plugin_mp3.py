...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/fileinfo/plugins/fileinfo_inv_plugin_mp3.py
# Compiled at: 2008-06-13 09:20:50
...
class MP3Investigator(BaseInvestigator):
    ...
    __module__ = __name__
    attrMap = {'artist': 'getArtist', 'album': 'getAlbum'}
    totals = ()
    tagDataMap = {'title': (3, 33), 'artist': (33, 63), 'album': (63, 93), 'year': (93, 97), 'comment': (97, 126), 'genre': (127, 128)}
        ...
        if tagdata[:3] == 'TAG':
            for (tag, (start, end)) in self.tagDataMap.items():
                ...
                if tag == 'genre':
                    ...
                    self.tags[tag] = data.replace('\x00', ' ').strip()
