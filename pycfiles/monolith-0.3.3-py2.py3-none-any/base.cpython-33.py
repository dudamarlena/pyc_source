...
# [GCC 8.4.0]
# Embedded file name: /Users/lukasz/develop/workspace/monolith/build/lib/monolith/cli/base.py
# Compiled at: 2013-12-01 16:11:32
...
class Parser(argparse.ArgumentParser):
    ...
    def _print_message(self, message, file=None):
        if file is None:
            file = self.stream
        super(Parser, self)._print_message(str(message), file)
        return
...
class ExecutionManager(object):
    ...
    def __init__(self, argv=None, stderr=None, stdout=None):
        ...
        for name, Command in list(self.get_commands_to_register().items()):
    ...
    def get_parser(self):
        ...
        *ExecutionManager*.
        ...
        for name, command in list(self.registry.items()):
    ...
    def autocomplete(self):
        ...
        cmd_names = list(self.get_commands().keys())
        if current:
            self.stdout.write(str(' '.join([name for name in cmd_names if name.startswith(current)])))
        sys.exit(1)
...
class SimpleExecutionManager(ExecutionManager):
    ...
    def get_commands_to_register(self):
        ...
        """
        return dict((key, get_class(value)) for key, value in list(self.simple_commands.items()))
