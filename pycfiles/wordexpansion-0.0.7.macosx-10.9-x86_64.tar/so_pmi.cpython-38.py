...
# [GCC 8.4.0]
# Embedded file name: /Library/Frameworks/Python.framework/Versions/3.8/lib/python3.8/site-packages/wordexpansion/so_pmi.py
# Compiled at: 2020-03-25 05:26:56
# Size of source mod 2**32: 8131 bytes
import jieba.posseg as pseg
import jieba, math, time
...
class ChineseSoPmi:
    ...
    def seg_corpus(self, train_data, seedword_txtfile):
        ...
        for word in sentiment_words:
            ...
            for line in open(train_data, encoding='utf-8'):
                ...
                count += 1
                if line:
                    seg_data.append([word.word for word in pseg.cut(line) if word.flag[0] not in ('u',
                                                                                                  'w',
                                                                                                  'x',
                                                                                                  'p',
                                                                                                  'q',
    ...
    def collect_candiwords(self, seg_data, cowords_list, seedword_txtfile):
        ...
        def collect_sentiwords(seedword_txtfile, word_dict):
            pos_words = set([line.strip().split('\t')[0] for line in open(seedword_txtfile, encoding='utf-8') if line.strip().split('\t')[1] == 'pos']).intersection(set(word_dict.keys()))
            neg_words = set([line.strip().split('\t')[0] for line in open(seedword_txtfile, encoding='utf-8') if line.strip().split('\t')[1] == 'neg']).intersection(set(word_dict.keys()))
...
def test():
    sopmier = ChineseSoPmi(inputtext_file='corpus.txt', seedword_txtfile='seed_words.txt',
