...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pocoo/pkg/core/l10n.py
...
TIME_DELTA_UNITS = [
 (
  3600 * 24 * 365, 'y'), (3600 * 24 * 30, 'M'), (3600 * 24 * 7, 'w'), (3600 * 24, 'd'), (3600, 'h'), (60, 'm')]
_ = lambda x: x
...
class DateFormatter(object):
    ...
    MONTHS_ABBR = [
     _('Jan'), _('Feb'), _('Mar'), _('Apr'), _('May:abbr'), _('Jun'), _('Jul'), _('Aug'), _('Sep'), _('Oct'), _('Nov'), _('Dec')]
    MONTHS_FULL = [
     _('January'), _('February'), _('March'), _('April'), _('May:full'), _('June'), _('July'), _('August'), _('September'), _('October'), _('November'), _('December')]
    ...
    def __repr__(self):
        return '<%s: [%s]>' % (self.__class__.__name__, (', ').join((str(i[7:]) for i in dir(self) if i.startswith('format_'))))
...
def format_timedelta(req, time1=None, time2=None):
    ...
    if isinstance(time1, datetime):
        time1 = time.mktime(time1.timetuple()) + time1.microsecond / 1000000.0
    if time2 is None:
        time2 = datetime.utcnow()
    if isinstance(time2, datetime):
        time2 = time.mktime(time2.timetuple()) + time2.microsecond / 1000000.0
    if time1 > time2:
...
def dateformat(date, context):
    ...
    if formatstr is None:
        formatstr = req.ctx.cfg.get('general', 'dateformat', DEFAULT_DATE_FORMAT)
...
def timeformat(date, context):
    ...
    if formatstr is None:
        formatstr = req.ctx.cfg.get('general', 'timeformat', DEFAULT_TIME_FORMAT)
    f = DateFormatter(req, date or None)
    return f.format(formatstr)
...
def datetimeformat(date, context):
    ...
    if formatstr is None:
        formatstr = req.ctx.cfg.get('general', 'datetimeformat', DEFAULT_DATETIME_FORMAT)
    f = DateFormatter(req, date or None)
    return f.format(formatstr)
