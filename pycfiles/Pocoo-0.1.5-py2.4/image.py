...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pocoo/utils/image.py
...
def is_supported_image(fp_or_data):
    ...
    if pil_installed:
        try:
            ...
            return True
    try:
        child = Popen(['identify', '-'], stdout=PIPE, stdin=PIPE, stderr=PIPE)
...
def resize_image(fp_or_data, width, height=None, filetype='image/png'):
    """
...
def _resize_im(fp, width, height, filetype):
    ...
    directly, it's just a helper for `resize_image`.
    """
    filetype = {'image/png': 'png', 'image/gif': 'gif', 'image/jpeg': 'jpeg'}[filetype]
    try:
        child = Popen(['convert', '-', '-resize', '%dx%d' % (width, height), '%s:-' % filetype], stdin=PIPE, stdout=PIPE, stderr=PIPE)
    except OSError, e:
        if e.errno == 2:
            raise _IMNotFound()
        raise
