...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pocoo/pkg/core/acl.py
...
from pocoo.pkg.core.db import users, groups, group_members, privileges, forums, posts, acl_mapping, acl_subjects, acl_objects
...
class AclManager(object):
    ...
    def can_access(self, privilege, obj):
        ...
        def do_check(obj, tendency):
            ...
            r = db.execute(meta.select([acl_mapping.c.state], (acl_mapping.c.priv_id == privileges.c.priv_id) & (acl_mapping.c.subject_id == s.subject_id) & (acl_mapping.c.object_id == o.object_id) & (privileges.c.name == privilege)))
            row = r.fetchone()
            if row is not None:
            ...
            if isinstance(self.subject, User):
                r = db.execute(meta.select([acl_mapping.c.state], (acl_mapping.c.object_id == o.object_id) & (acl_mapping.c.subject_id == groups.c.subject_id) & (groups.c.group_id == group_members.c.group_id) & (group_members.c.user_id == self.subject.user_id)))
                while True:
                    row = r.fetchone()
                    if row is None:
                        break
    ...
    def _set(self, privilege, obj, state):
        ...
        if not state:
            ...
            self.ctx.engine.execute(acl_mapping.delete((acl_mapping.c.priv_id == p.c.priv_id) & (acl_mapping.c.subject_id == self.subject.subject_id) & (acl_mapping.c.object_id == obj.object_id)))
            return
        priv_id = self._fetch_privilege(privilege)
        r = self.ctx.engine.execute(meta.select([acl_mapping.c.state], (acl_mapping.c.priv_id == priv_id) & (acl_mapping.c.subject_id == self.subject.subject_id) & (acl_mapping.c.object_id == obj.object_id)))
        row = r.fetchone()
        if row is not None:
            if row['state'] == state:
                return
            self._set(privilege, obj, 0)
        self.ctx.engine.execute(acl_mapping.insert(), priv_id=priv_id, subject_id=self.subject.subject_id, object_id=obj.object_id, state=state)
        return

    def _bootstrap(self):
        """This method is automatically called when subject_id is
        None and an subject_id is required."""
        r = self.ctx.engine.execute(acl_subjects.insert(), subject_type=self._type)
    ...
    def _bootstrap_object(self, obj):
        ...
        r = self.ctx.engine.execute(acl_objects.insert(), object_type=objtype)
    ...
    def _get_object_join(self, obj):
        ...
        if t == 'forum':
            return meta.select([forums.c.object_id], forums.c.forum_id == obj.forum_id)
        elif t == 'thread':
            return meta.select([posts.c.object_id], posts.c.post_id == obj.post_id)
        else:
    ...
    def _get_subject_join(self):
        ...
        if self._type == 'user':
            return meta.select([users.c.subject_id], users.c.user_id == self.subject.user_id)
        return meta.select([groups.c.subject_id], groups.c.group_id == self.subject.group_id)
    ...
    def _fetch_privilege(self, name):
        ...
        privilege."""
        r = self.ctx.engine.execute(meta.select([privileges.c.priv_id], privileges.c.name == name))
        row = r.fetchone()
        if row is not None:
            return row[0]
        r = self.ctx.engine.execute(privileges.insert(), name=name)
    ...
    def __repr__(self):
        ...
        if self.subject.subject_id is None:
            return '<%s %s:%d inactive>' % (self.__class__.__name__, self._type, id_)
        return '<%s %s:%d active as %d>' % (self.__class__.__name__, self._type, id_, self.subject.subject_id)