...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pocoo/settings.py
...
"""
import sys, os, new, re, cPickle as pickle
...
def load_config(iterable, filename, root, initial_state='root'):
    ...
    for (lno, rline) in enumerate(chain(iterable, [''])):
        ...
        if indent > indentlevels[(-1)]:
            if not new_state:
                raise InvalidConfigFile(filename, lno + 1, 'Unexpected indent')
        ...
        if curstate == 'root':
            if line.startswith('section ') and line.endswith(':'):
                ...
                if not sname:
                    raise InvalidConfigFile(filename, lno + 1, 'Section name is empty')
                new_state = 'section'
                ...
                try:
                    ...
                    raise InvalidConfigFile(filename, lno + 1, 'Included file not found')
                else:
                    root.setdefault('__includes__', []).append(iname)
                    raw.append((len(indentlevels), 'include', iname))
            else:
                raise InvalidConfigFile(filename, lno + 1, 'Only include and section allowed on top level')
            ...
            if len(splitted) == 1:
                raise InvalidConfigFile(filename, lno + 1, 'Bare values only allowed in a list or string')
            else:
            ...
            if value.endswith(':'):
                if value.startswith('list'):
                    ...
                    raise InvalidConfigFile(filename, lno + 1, 'Invalid value type')
                opts = value[len(new_state):-1].strip()
                if opts and (opts[0] != '[' or opts[(-1)] != ']'):
                    raise InvalidConfigFile(filename, lno + 1, 'Options must be enclosed in [brackets]')
            ...
            splitted = line.split('=', 1)
            if len(splitted) == 1:
                raise InvalidConfigFile(filename, lno + 1, 'Invalid dict entry (must be separated by =)')
            thingtofill[parse_value(splitted[0])] = parse_value(splitted[1])
        elif curstate == 'string':
            thingtofill += parse_value(line) + '\n'
        else:
            raise InvalidConfigFile(filename, lno + 1, "You'll shoot me for this error message, but something went wrong")
...
class Configuration(object):
    ...
    def load(self):
        ...
        try:
            (self.encoding, self._raw) = load_config(file(self.filename), self.filename, self._data)
        except IOError:
    ...
    def __repr__(self):
        return '<%s in %r>' % (self.__class__.__name__, self.root)
    ...
    def get(self, section, key, default=missing):
        ...
        try:
            ...
            if default is not missing:
                ...
                raise MissingConfigValue('mandatory value %s.%s not set' % (section, key))
    ...
    def get_int(self, section, key, default=missing):
        ...
        try:
            ...
            raise MissingConfigValue('config value %s.%s should be an integer but cannot be converted to one' % (section, key))
    ...
    def get_float(self, section, key, default=missing):
        ...
        try:
            ...
            raise MissingConfigValue('config value %s.%s should be a float but cannot be converted to one' % (section, key))
    ...
    def save(self, output=None):
        ...
        def encode_val(val):
            if val.lower() in ['true', 'yes', 'on', '1', '0', 'off', 'false']:
                val = val.lower()
            else:
        ...
        def write_value(indent, name, val):
            output.write(indent + name.encode(self.encoding) + ' = ' + encode_val(val) + '\n')
        ...
        def write_dict(indent, name, val):
            ...
            for (k, v) in val.items():
                output.write(indent + k.encode(self.encoding) + ' = ' + encode_val(v) + '\n')
...
class ConfigValue(object):
    ...
    def __repr__(self):
        return '<%s: %r>' % (self.__class__.__name__, '%s.%s' % (self.section, self.key))
