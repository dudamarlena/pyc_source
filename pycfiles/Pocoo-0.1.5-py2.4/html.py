...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pocoo/utils/html.py
...
hexcolor_re = re.compile('^\\#[a-fA-F0-9]{3}([a-fA-F0-9]{3})?$')
punctuation_re = re.compile('^(?P<lead>(?:%s)*)(?P<middle>.*?)(?P<trail>(?:%s)*)$' % (('|').join(map(re.escape, LEADING_PUNCTUATION)), ('|').join(map(re.escape, TRAILING_PUNCTUATION))))
...
def urlize(text, trim_limit=None, nofollow=False):
    ...
    for (i, word) in enumerate(words):
        ...
        if match:
            ...
            if middle.startswith('www.') or middle and not middle.startswith('http://') and '@' not in middle and middle[0] in string.letters + string.digits and (middle.endswith('.org') or middle.endswith('.net') or middle.endswith('.com')):
...
class SafeParser(HTMLParser):
    ...
    """
    __module__ = __name__
    DEPRECATED = {'b': ('strong', None), 'i': ('em', None), 'u': ('ins', None), 's': ('del', None), 'center': ('span', 'text-align: center'), 'big': ('span', 'font-size: x-large'), 'small': ('span', 'font-size: x-small'), 'strike': ('del', None), 'font': ('span', None)}
    ALLOWED = ('a', 'abbr', 'acronym', 'address', 'applet', 'area', 'b', 'big', 'blockquotebr',
               'button', 'caption', 'center', 'cite', 'code', 'dd', 'del', 'dfn',
               'div', 'dl', 'dt', 'em', 'font', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6',
               'hr', 'i', 'img', 'ins', 'label', 'legend', 'li', 'map', 'ol', 'p',
               'param', 'pre', 'q', 's', 'samp', 'small', 'strike', 'strong', 'sub',
    ...
    def _write_tag(self, name, attributes, close=False):
        ...
        for (key, value) in attributes.iteritems():
            tmp.append('%s=%s' % (key, quoteattr(value)))

        attributes = ''
        if tmp:
            attributes = ' ' + (' ').join(tmp)
        close = close and ' /' or ''
        self._result.append('<%s%s%s>' % (name, attributes, close))
        if not close:
    ...
    def _close_tag(self, name):
        ...
        while last_tag != name:
            ...
            self._result.append('</%s>' % last_tag)
    ...
    def _rewrite_font(self, attrs):
        ...
        if face is not None and self.font_face_re.search(face) is not None:
            ...
            tmp.append('font-family: %s' % (', ').join(parts))
        if size is not None and self.font_size_re.search(size) is not None:
            if size[0] in ('+', '-'):
                size = size[1:]
            size = int(size)
            tmp.append('font-size: %dpt' % (size * 3.8))
        if color is not None:
            try:
                tmp.append('color: %s' % translate_color(color))
            except ValueError:
                pass
        ...
        if tag not in self.ALLOWED:
            if not self._lock:
                self._lock[:] = (
                 tag,)
                return
        attrs = dict(((name, value) for (name, value) in attrs if not name.startswith('on') if name != 'style'))
        if tag == 'font':
            tag = 'span'
