...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pocoo/template.py
...
"""
import os, cPickle as pickle, md5, time, jinja, jinja.base, jinja.exceptions
...
class PocooLoader(object):
    ...
    def load(self, name, parent=None):
        if '::' in name:
            ...
            basename = 'default'
        try:
            return unicode(self.ctx.pkgmanager.get_resource('templates', basename, name, 'site'), 'utf-8')
        except MissingResource, e:
            raise jinja.exceptions.TemplateDoesNotExist(e.args[0])
        except UnicodeError, e:
            raise jinja.exceptions.TemplateCharsetError('Pocoo template must be encoded in utf-8', e)
    ...
    def load_and_compile(self, name, lib=None, parent=None):
        ...
        if self.diskcache:
            ...
            if not os.path.exists(disk_cache_name):
                ...
                try:
                    pickle.dump(nodelist, file(disk_cache_name, 'wb'), protocol=2)
                except IOError:
                    pass
...
def render_template(req, name, context):
    ...
    t_1 = time.time()
    ctx = req.ctx
    addvars = {'cobalt': '%s!cobalt' % req.ctx.scriptname}
    syscontext = {'REQUEST': req, 'pages': {}, 'system': {'template': addvars, 'pocoo': {'expose': True, 'url': 'http://www.pocoo.org/'}, 'conf': ctx.cfg.to_dict()}, 'base_url': req.ctx.make_external_url(''), 'user': req.user}
    ...
    nodelist = req.ctx.template_loader.load_and_compile('%s::%s' % (template_name, name), req.ctx._template_lib)
