...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pocoo/context.py
...
from pocoo.ihook import PackageManager, make_bogus_package, setup_importhook
...
from pocoo.exceptions import PocooRuntimeError, PackageImportError, PackageNotFound, InvalidConfigFile
PACKAGE_EXT = '.pkg'
...
class ApplicationContext(object):
    ...
    def __init__(self, root, is_cgi=False):
        ...
        self._regtypes = dict(components=self.register_component, tables=self.register_table, template_tags=self.register_template_tag, template_filters=self.register_template_filter, middlewares=self.register_middleware)
        if is_cgi:
    ...
    def setup_package(self, pkgname):
        ...
        """
        if self._init_done:
            raise PocooRuntimeError('ApplicationContext.setup_package() called after context initialization is complete')
        ...
        try:
            ...
            load_config(manifest.splitlines(), PACKAGE_MANIFEST_FILE, pmeta, initial_state='section')
        except IOError:
            raise PackageNotFound('Package "%s" (at %s) is missing manifest file "%s"' % (pkgname, pman.pkgs[pkgname], PACKAGE_MANIFEST_FILE))
        except InvalidConfigFile, e:
            raise PackageImportError('Package "%s": manifest file "%s" is invalid: %s' % (pkgname, PACKAGE_MANIFEST_FILE, str(e)))
        ...
        for dep in deps:
            ...
            if name not in self.packagemeta:
                try:
                    ...
                    raise PackageImportError(e.args[0] + ' (dependency needed by package "%s"' % pkgname), None, sys.exc_info()[2]

            installed = self.packagemeta[name].get('version', ('0', ))
            if ver > installed:
                raise PackageImportError('Package "%s" needs version %s of package "%s", but only version %s is available' % (pkgname, ('.').join(ver), name, ('.').join(installed)))

        try:
            pkg = __import__('%s.%s' % (self.pkg_prefix, pkgname), None, None, ['setup'])
        except ImportError, e:
            raise PackageImportError('Package "%s" cannot be imported: %s' % (pkgname, e)), None, sys.exc_info()[2]
        ...
        def import_from_pkg(thing):
            if '.' in thing:
                ...
                objname = thing
            objname = str(objname)
            return getattr(__import__('%s.%s%s' % (self.pkg_prefix, pkgname, module), None, None, [objname]), objname)
        ...
        try:
            ...
            raise PackageImportError('Object "%s" listed in manifest of package "%s" not importable: %s' % (thing, pkgname, e))
    ...
    def register_component(self, component, pkgname):
        ...
        for comptype in component.__bases__:
            if comptype is Component:
                raise PocooRuntimeError('The base component class %r does not need to be registered directly' % component.__name__)
    ...
    def register_table(self, table, pkgname):
        ...
        if table in self.tables:
            raise PocooRuntimeError('Table %r already registered' % table.name)
    ...
    def make_url(self, *link, **query):
        ...
        return pathjoin(self.scriptname, *map(str, link)) + query
    ...
    def make_external_url(self, *link, **query):
        ...
        """
        query = urlencode(query)
        return urljoin(self.serverpath, *map(str, link)) + query
