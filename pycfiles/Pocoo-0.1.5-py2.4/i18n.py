...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pocoo/pkg/core/i18n.py
...
class TranslatableTag(Node):
    ...
    """
    __module__ = __name__
    rules = {'long': [KeywordNode('trans')], 'plural': [KeywordNode('trans'), KeywordNode('pluralizing'), VariableNode()], 'short': [KeywordNode('trans'), ValueNode(), CollectionNode()]}
    ...
    def __init__(self, parser, matched_tag, handler_args, stack):
        ...
        if matched_tag == 'short':
            self._body_sg = [
             handler_args[1].resolve()]
            self._vars_sg = handler_args[2]
        else:
            (self._body_sg, self._vars_sg, self._body_pl, self._vars_pl) = self._forkparse(parser)
    ...
    def _forkparse(self, parser):
        ...
        while parser.tokens:
            ...
            if token.token_type == TOKEN_VAR:
                var = lib.parse(parser, 'print %s' % token.contents)
                ...
                raise TemplateSyntaxError("you can't use blocks inside of a `trans` tag.")

        return (
         sg, vars_sg, pl, vars_pl)
...
def get_request_languages(req):
    ...
    """
    if hasattr(req, 'user') and req.user is not None and req.user.language:
...
class Translator(object):
    ...
    def __repr__(self):
        return '<%s %r>' % (self.__class__.__name__, self.languages)
