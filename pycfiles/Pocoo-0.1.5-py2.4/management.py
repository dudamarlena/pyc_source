...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pocoo/management.py
...
from pocoo.utils.console import nocolor, darkgreen, red, purple, bold
...
def createinst(*args, **opts):
    ...
    def writefile(content, *pathitems, **flags):
        content = content % {'path': path, 'executable': sys.executable}
    ...
    writefile(tmpl.MANAGEPY, 'manage.py', mode=493)
...
def listcomp(*args, **opts):
    ...
    def keyfunc(x):
        ...
        if bytype:
            return (
             x[1], x[0], x[2].__module__, x[2].__name__)
        else:
            return (
             x[0], x[1], x[2].__module__, x[2].__name__)
    ...
    for (pkgname, comptype, comp) in complist:
        if bytype:
            ...
            print '    %s.%s%s %s' % (mod, comp.__name__, ' ' * (maxlens[2] - len(comp.__name__) - len(mod)), comptype)


@action('runserver', 'Run a development server, optionally without the reloader component.', [
 'no-reloader', 'no-debug'], [])
...
@action('createforum', 'Creates a new forum', [], ['<name>', '<description>', '<parent>'])
def createforum(*args, **opts):
    assert 3 >= len(args) >= 1, 'up to 3 arguments required'
    ctx = _create_ctx(True)
    forums = ctx.tables['core_forums']
    result = ctx.engine.execute(forums.insert(), name=args[0], description=len(args) == 2 and args[1] or '', thread_count=0, post_count=0, parent_id=len(args) == 3 and args[2] or None)
...
def _mk_pocoo_context():
    ...
    namespace = {'pocoo': pocoo, 'ctx': ctx, 'db': ctx.engine, 'tables': tables, 'lib': lib_metamodule()}
...
@action('execute', 'Execute a test file in a Pocoo context (use --help-context to learn what that is)', [
 'help-context'], ['<filename>'])
def execute(*args, **opts):
    if len(args) == 0 or len(opts) != 0:
        print 'Execute a file in a Pocoo context. A Pocoo context has an ApplicationContext\nprecreated for this instance and the following objects are available as globals:\n' + PREINITIALIZED
        return
    assert len(args) == 1
    namespace = _mk_pocoo_context()
...
def main(root='.'):
    ...
    try:
        ...
        for (name, handler) in _handlers.iteritems():
            ...
            sys.stderr.write('    %s %s%s%s\n       - %s\n' % (bold(name), opts, opts and args and ' ' or '', args, handler.desc))
    ...
    try:
        ...
        sys.stderr.write('>>> Usage: %s %s %s%s%s\n' % (sys.argv[0], action, (' ').join(('[--%s]' % opt for opt in handler.opts)), handler.opts and ' ' or '', (' ').join(handler.args)))
        sys.stderr.write('       %s\n' % handler.desc)
        return 1
    except SystemExit:
        raise
