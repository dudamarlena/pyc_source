...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fastachar/tkgui.py
# Compiled at: 2020-05-06 07:48:07
# Size of source mod 2**32: 40882 bytes
...
CONFIG = dict(linux=dict(INIFILE='fastacharrc', INIPATH='.config/fastachar'),
  win32=dict(INIFILE='fastachar.ini', INIPATH='.fastachar'))
DEFAULT_REGEX = dict(header_format='{ID}[ _]{SPECIES}', id='[A-Za-z0-9\\.]+',
  species='[A-Za-z_ ]+')
...
class ConfigFastachar(object):
    ...
    def get_path(self):
        ...
        path = os.path.join(home_dir, CONFIG[platform]['INIPATH'])
        os.makedirs(path, exist_ok=True)
        return os.path.join(path, CONFIG[platform]['INIFILE'])
...
class Case(object):
    ...
    def populate(self, filename, species, setA, setB, operation, regex_header_format, regex_id, regex_species):
        """
        ...
        """
        self.data = dict(filename=filename, species=species,
          setA=setA,
          setB=setB,
          operation=operation,
...
class Gui:
    ...
    def __init__(self):
        ...
        try:
            self.alignment.set_fasta_hdr_fmt(self.config.config['REGEX']['header_format'], self.config.config['REGEX']['id'], self.config.config['REGEX']['species'])
        except KeyError:
            pass
    ...
    def cb_open_fasta_file_for_hdr(self, parent, regexs):
        ...
        try:
            ...
            self.error_window(err_code=8, arg='Failed to interpret the regular expression patterns.', parent=parent)
            self.fasta_file_is_valid = False
            return
        else:
            self.fasta_file = self.fasta_file or filedialog.askopenfilename(defaultextension='.fas', filetypes=[
    ...
    def cb_set_regex(self):
        ...
        bt_open = Tk.Button(bottom_frame, text='Preview file', command=(partial(self.cb_open_fasta_file_for_hdr, toplevel, v)))
        bt_reset = Tk.Button(bottom_frame, text='Reset', command=(partial(self.cb_reset, v)))
        help_lines = fasta_doc.REGEX_HELP_TEXT.split('\n')
        bt_help = Tk.Button(bottom_frame, text='Help', command=(partial(self.cb_open_text_window, help_lines)))
    ...
    def cb_open_text_window(self, lines):
        ...
        report = Tk.Text(frame, state=(Tk.DISABLED), padx=10,
          pady=10,
    ...
    def about_window(self):
        ...
        """
        toplevel = Tk.Toplevel()
        label1 = Tk.Label(toplevel, text=(fasta_doc.ABOUT_TEXT), height=0, width=100, justify=(Tk.LEFT))
        label1.pack()
        label2 = Tk.Label(toplevel, text=(fasta_doc.DISCLAIMER), height=0, width=100, justify=(Tk.LEFT))
        label2.pack()
        bt = Tk.Button(toplevel, text='Close', command=(toplevel.destroy))
        bt.pack()
        toplevel.focus_force()
    ...
    def error_window(self, err_code, arg='', parent=None):
        ...
        report.config(state=(Tk.DISABLED))
        bt = Tk.Button(toplevel, text='Close', padx=10, pady=10, command=(toplevel.destroy))
        bt.pack()
        toplevel.focus_force()
    ...
    def create_layout(self):
        ...
        self.lb_sequences = (Tk.Listbox)(frame_sequences, xscrollcommand=sb_hor_sequences.set, 
         yscrollcommand=sb_sequences.set, **cnf_lb)
        self.lb_A = (Tk.Listbox)(frame_A, xscrollcommand=sb_hor_A.set, 
         yscrollcommand=sb_A.set, **cnf_lb)
        self.lb_B = (Tk.Listbox)(frame_B, xscrollcommand=sb_hor_B.set, 
         yscrollcommand=sb_B.set, **cnf_lb)
        ...
        Tk.Radiobutton(frame, text='Determine MDCs for species list A', variable=(self.operation_method),
          value=1).pack(anchor=(Tk.W))
        Tk.Radiobutton(frame, text='Determine potential MDCs for species list A', variable=(self.operation_method),
        ...
        frame_report.grid(row=4, column=0, columnspan=3, sticky=(Tk.W + Tk.E + Tk.N + Tk.S),
          padx=10,
          pady=10)
        ...
        self.report = Tk.Text(frame_report, state=(Tk.DISABLED), wrap=(Tk.NONE),
          yscrollcommand=(sb_report.set),
    ...
    def release_in_listbox(self, event):
        lbs = [
         self.lb_sequences, self.lb_A, self.lb_B]
        x = event.x_root
        y = event.y_root
        s = 0
        for lb in lbs:
    ...
    def populate_list_with_items(self, items, lb, delete_all=True):
        if delete_all:
            lb.delete(0, Tk.END)
        for item in items:
            s = '%s (%d)' % (item, self.data_nsequences[item])
            lb.insert(Tk.END, s)
    ...
    def cb_open_fasta_file(self):
        ...
        """
        self.fasta_file = filedialog.askopenfilename(defaultextension='.fas', filetypes=[
         ('fasta files', '.fas'), ('all files', '.*')],
          initialdir=(self.cwd),
          multiple=False,
          parent=(self.root),
    ...
    def cb_open_case_file(self):
        ...
        """
        case_file = filedialog.askopenfilename(defaultextension='.fc', filetypes=[
         ('case files', '.fc'), ('all files', '.*')],
          initialdir=(self.cwd),
          multiple=False,
          parent=(self.root),
        ...
        if case_file:
            ...
            if error == fasta_io.OK:
                ...
                if error == fasta_io.OK:
                    ...
                    self.populate_list_with_items((self.case.data['species']), (self.lb_sequences),
                      delete_all=True)
                    self.data[self.lb_A] = self.case.data['setA']
                    self.populate_list_with_items((self.case.data['setA']), (self.lb_A),
                      delete_all=True)
                    self.data[self.lb_B] = self.case.data['setB']
                    self.populate_list_with_items((self.case.data['setB']), (self.lb_B),
                      delete_all=True)
    ...
    def cb_save_case_file(self):
        ...
        error = fasta_io.OK
        if self.case.data:
            case_file = filedialog.asksaveasfilename(defaultextension='.fc', filetypes=[
             ('case files', '.fc'), ('all files', '.*')],
              initialdir=(self.cwd),
              parent=(self.root),
    ...
    def cb_save_report(self):
        ...
        """
        error = fasta_io.OK
        out_file = filedialog.asksaveasfilename(defaultextension='.txt', filetypes=[
        ...
        try:
            with open(out_file, 'w') as (fp):
                fp.write(self.report.get(1.0, Tk.END))
    ...
    def cb_save_report_xls(self):
        ...
        """
        error = fasta_io.OK
        out_file = filedialog.asksaveasfilename(defaultextension='.xls', filetypes=[
    ...
    def cb_run(self):
        ...
        self.case.clear()
        try:
            self.case.populate(self.fasta_file, self.data[self.lb_sequences], self.data[self.lb_A], self.data[self.lb_B], operation, self.alignment.pattern_dict['HEADER'], self.alignment.pattern_dict['ID'], self.alignment.pattern_dict['SPECIES'])
        except AttributeError:
            return
        else:
            memofile = StringIO()
            ...
            mdc_method = {1:'MDC',  2:'potential_MDC_only'}
            if operation in (1, 2):
