...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/wechat_sdk/core/conf.py
...
class WechatConf(object):
    ...
    def __init__(self, **kwargs):
        ...
        if kwargs.get(b'checkssl') is not True:
            disable_urllib3_warning()
        self.__token = kwargs.get(b'token')
        self.__appid = kwargs.get(b'appid')
        self.__appsecret = kwargs.get(b'appsecret')
        self.__encrypt_mode = kwargs.get(b'encrypt_mode', b'safe')
        self.__encoding_aes_key = kwargs.get(b'encoding_aes_key')
        self.__crypto = None
        self._update_crypto()
        self.__access_token_getfunc = kwargs.get(b'access_token_getfunc')
        self.__access_token_setfunc = kwargs.get(b'access_token_setfunc')
        self.__access_token_refreshfunc = kwargs.get(b'access_token_refreshfunc')
        self.__access_token = kwargs.get(b'access_token')
        self.__access_token_expires_at = kwargs.get(b'access_token_expires_at')
        self.__jsapi_ticket_getfunc = kwargs.get(b'jsapi_ticket_getfunc')
        self.__jsapi_ticket_setfunc = kwargs.get(b'jsapi_ticket_setfunc')
        self.__jsapi_ticket_refreshfunc = kwargs.get(b'jsapi_ticket_refreshfunc')
        self.__jsapi_ticket = kwargs.get(b'jsapi_ticket')
        self.__jsapi_ticket_expires_at = kwargs.get(b'jsapi_ticket_expires_at')
        self.__partnerid = kwargs.get(b'partnerid')
        self.__partnerkey = kwargs.get(b'partnerkey')
        self.__paysignkey = kwargs.get(b'paysignkey')
        return
    ...
    def access_token(self):
        u""" 获取当前 access token 值, 本方法会自行维护 access token 有效性 """
    ...
    def jsapi_ticket(self):
        u""" 获取当前 jsapi ticket 值, 本方法会自行维护 jsapi ticket 有效性 """
        self._check_appid_appsecret()
    ...
    def grant_access_token(self):
        ...
        if callable(self.__access_token_refreshfunc):
            ...
            return
        response_json = self.__request.get(url=b'https://api.weixin.qq.com/cgi-bin/token', params={b'grant_type': b'client_credential', 
           b'appid': self.__appid, 
           b'secret': self.__appsecret}, access_token=self.__access_token)
        self.__access_token = response_json[b'access_token']
        self.__access_token_expires_at = int(time.time()) + response_json[b'expires_in']
    ...
    def grant_jsapi_ticket(self):
        ...
        if callable(self.__jsapi_ticket_refreshfunc):
            ...
            return
        response_json = self.__request.get(url=b'https://api.weixin.qq.com/cgi-bin/ticket/getticket', params={b'type': b'jsapi'}, access_token=self.access_token)
        self.__jsapi_ticket = response_json[b'ticket']
        self.__jsapi_ticket_expires_at = int(time.time()) + response_json[b'expires_in']
    ...
    def get_access_token(self):
        ...
        **仅为兼容 v0.6.0 以前版本使用, 自行维护 access_token 请使用 access_token_setfunc 和 access_token_getfunc 进行操作**
        ...
        """
        self._check_appid_appsecret()
        return {b'access_token': self.access_token, 
           b'access_token_expires_at': self.__access_token_expires_at}
    ...
    def get_jsapi_ticket(self):
        ...
        **仅为兼容 v0.6.0 以前版本使用, 自行维护 jsapi_ticket 请使用 jsapi_ticket_setfunc 和 jsapi_ticket_getfunc 进行操作**
        ...
        """
        self._check_appid_appsecret()
        return {b'jsapi_ticket': self.jsapi_ticket, 
           b'jsapi_ticket_expires_at': self.__jsapi_ticket_expires_at}
    ...
    def _check_token(self):
        ...
        """
        if not self.__token:
            raise NeedParamError(b'Please provide Token parameter in the construction of class.')
    ...
    def _check_appid_appsecret(self):
        ...
        if not self.__appid or not self.__appsecret:
            raise NeedParamError(b'Please provide app_id and app_secret parameters in the construction of class.')
    ...
    def _update_crypto(self):
        ...
        if self.__encrypt_mode in ('compatible', 'safe') and self.__encoding_aes_key is not None:
            if self.__token is None or self.__appid is None:
                raise NeedParamError(b'Please provide token and appid parameters in the construction of class.')
