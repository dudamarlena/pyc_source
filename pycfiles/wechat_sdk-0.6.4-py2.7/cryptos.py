...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/wechat_sdk/lib/crypto/cryptos.py
# Compiled at: 2016-07-24 14:39:52
import base64, time, six, xmltodict
...
class WechatBaseCrypto(object):
    ...
    def _encrypt_message(self, msg, nonce, timestamp=None):
        ...
        return to_text(xml.format(encrypt=to_text(encrypt), signature=to_text(signature), timestamp=to_text(timestamp), nonce=to_text(nonce)))

    def _decrypt_message(self, msg, msg_signature, timestamp, nonce):
        u"""检验消息的真实性，并且获取解密后的明文
