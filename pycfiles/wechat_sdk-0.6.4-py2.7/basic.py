...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/wechat_sdk/basic.py
# Compiled at: 2016-05-08 12:41:06
import io, hashlib, requests, cgi, six
...
from .reply import TextReply, ImageReply, VoiceReply, VideoReply, MusicReply, Article, ArticleReply, GroupTransferReply
...
class WechatBasic(WechatBase):
    ...
    def __init__(self, token=None, appid=None, appsecret=None, partnerid=None, partnerkey=None, paysignkey=None, access_token=None, access_token_expires_at=None, jsapi_ticket=None, jsapi_ticket_expires_at=None, checkssl=False, conf=None):
        u"""
        ...
        if conf is not None:
            self.__conf = conf
        elif isinstance(token, WechatConf):
            self.__conf = token
        else:
            self.__conf = WechatConf(token=token, appid=appid, appsecret=appsecret, access_token=access_token, access_token_expires_at=access_token_expires_at, jsapi_ticket=jsapi_ticket, jsapi_ticket_expires_at=jsapi_ticket_expires_at, encrypt_mode='normal', partnerid=partnerid, partnerkey=partnerkey, paysignkey=paysignkey, checkssl=checkssl)
    ...
    @property
    def conf(self):
        u""" 获取当前 WechatConf 配置实例 """
        return self.__conf

    @conf.setter
    def conf(self, conf):
        u""" 设置当前 WechatConf 实例  """
        self.__conf = conf
        self.__request = WechatRequest(conf=self.__conf)
    ...
    def generate_jsapi_signature(self, timestamp, noncestr, url, jsapi_ticket=None):
        ...
        data = {'jsapi_ticket': jsapi_ticket, 'noncestr': noncestr, 
           'timestamp': timestamp, 
           'url': url}
        keys = list(data.keys())
        keys.sort()
    ...
    def parse_data(self, data, msg_signature=None, timestamp=None, nonce=None):
        ...
        if self.conf.encrypt_mode == 'safe':
            ...
            data = self.conf.crypto.decrypt_message(msg=data, msg_signature=msg_signature, timestamp=timestamp, nonce=nonce)
        try:
    ...
    def create_menu(self, menu_data):
        ...
        return self.request.post(url='https://api.weixin.qq.com/cgi-bin/menu/create', data=menu_data)
    ...
    def _upload_media_py2(self, media_type, media_file, extension=''):
        ...
        return self.request.post(url='https://api.weixin.qq.com/cgi-bin/media/upload', params={'type': media_type}, files={'media': (
                   filename, media_file, convert_ext_to_mime(extension))})
    ...
    def download_media(self, media_id):
        ...
        """
        return self.request.get('https://api.weixin.qq.com/cgi-bin/media/get', params={'media_id': media_id}, stream=True)
    ...
    def create_group(self, name):
        ...
        """
        return self.request.post(url='https://api.weixin.qq.com/cgi-bin/groups/create', data={'group': {'name': name}})
    ...
    def get_group_by_id(self, openid):
        ...
        :return: 返回的 JSON 数据包
        """
        return self.request.post(url='https://api.weixin.qq.com/cgi-bin/groups/getid', data={'openid': openid})
    ...
    def update_group(self, group_id, name):
        ...
        :return: 返回的 JSON 数据包
        """
        return self.request.post(url='https://api.weixin.qq.com/cgi-bin/groups/update', data={'group': {'id': int(group_id), 
                     'name': name}})
    ...
    def move_user(self, user_id, group_id):
        ...
        :return: 返回的 JSON 数据包
        """
        return self.request.post(url='https://api.weixin.qq.com/cgi-bin/groups/members/update', data={'openid': user_id, 
           'to_groupid': group_id})
    ...
    def get_user_info(self, user_id, lang='zh_CN'):
        ...
        :return: 返回的 JSON 数据包
        """
        return self.request.get(url='https://api.weixin.qq.com/cgi-bin/user/info', params={'openid': user_id, 
           'lang': lang})
    ...
    def send_text_message(self, user_id, content):
        ...
        :return: 返回的 JSON 数据包
        """
        return self.request.post(url='https://api.weixin.qq.com/cgi-bin/message/custom/send', data={'touser': user_id, 
           'msgtype': 'text', 
    ...
    def send_article_message(self, user_id, articles=None, media_id=None):
        ...
        if articles:
            ...
            for article in articles:
                article = Article(**article)
                articles_data.append({'title': article.title, 
    ...
    def create_qrcode(self, data):
        ...
        return self.request.post(url='https://api.weixin.qq.com/cgi-bin/qrcode/create', data=data)
    ...
    def set_template_industry(self, industry_id1, industry_id2):
        ...
        :return: 返回的 JSON 数据包
        """
        return self.request.post(url='https://api.weixin.qq.com/cgi-bin/template/api_set_industry', data={'industry_id1': str(industry_id1), 
    ...
    def get_template_id(self, template_id_short):
        ...
        :return: 返回的 JSON 数据包
        """
        return self.request.post(url='https://api.weixin.qq.com/cgi-bin/template/api_add_template', data={'template_id_short': str(template_id_short)})
    ...
    def send_template_message(self, user_id, template_id, data, url='', topcolor='#FF0000'):
        ...
        return self.request.post(url='https://api.weixin.qq.com/cgi-bin/message/template/send', data={'touser': user_id, 
    ...
    def _encrypt_response(self, response):
        if self.conf.encrypt_mode == 'safe':
            return self.conf.crypto.encrypt_message(msg=to_binary(response), nonce=generate_nonce(), timestamp=generate_timestamp())
        return response