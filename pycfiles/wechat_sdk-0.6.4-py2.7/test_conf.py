...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/tests/core/test_conf.py
# Compiled at: 2016-04-12 23:03:20
import time, unittest, xmltodict
...
class CoreConfTestCase(unittest.TestCase):
    ...
    def test_init_with_token_and_appid_appsecret(self):
        u""" 测试 Token AppID AppSecret 的初始化 """
        ...
        with HTTMock(api_weixin_mock):
            ...
            self.assertIn(conf._WechatConf__access_token_expires_at, range(access_token_expires_at - 10, access_token_expires_at + 11))
            ...
            self.assertIn(conf._WechatConf__access_token_expires_at, range(jsapi_ticket_expires_at - 10, jsapi_ticket_expires_at + 11))
    ...
    def test_init_with_compatible_encrypt_mode(self):
        u""" 测试兼容模式下的初始化 """
        conf = WechatConf(token=self.token, appid=self.appid, appsecret=self.appsecret, encrypt_mode='compatible', encoding_aes_key=self.encoding_aes_key)
        self.assertEqual(conf.token, self.token)
        self.assertEqual(conf.appid, self.appid)
        self.assertEqual(conf.appsecret, self.appsecret)
        self.assertEqual(conf.encoding_aes_key, self.encoding_aes_key)
        self.assertIsNotNone(conf.crypto)
        req = conf.crypto.decrypt_message(msg=self.compatible_request_message, msg_signature=self.msg_signature, timestamp=self.timestamp, nonce=self.nonce)
    ...
    def test_init_with_safe_encrypt_mode(self):
        u""" 测试安全模式下的初始化 """
        conf = WechatConf(token=self.token, appid=self.appid, appsecret=self.appsecret, encrypt_mode='safe', encoding_aes_key=self.encoding_aes_key)
        self.assertEqual(conf.token, self.token)
        self.assertEqual(conf.appid, self.appid)
        self.assertEqual(conf.appsecret, self.appsecret)
        self.assertEqual(conf.encoding_aes_key, self.encoding_aes_key)
        self.assertIsNotNone(conf.crypto)
        req = conf.crypto.decrypt_message(msg=self.safe_request_message, msg_signature=self.msg_signature, timestamp=self.timestamp, nonce=self.nonce)
        self.assertEqual(xmltodict.parse(req), xmltodict.parse(self.normal_request_message))
        origin_crypto = conf.crypto._WechatBaseCrypto__pc
        conf.crypto._WechatBaseCrypto__pc = TestBaseCrypto(key=conf.crypto._WechatBaseCrypto__key)
        resp = conf.crypto.encrypt_message(msg=self.response_message, nonce=self.nonce, timestamp=self.timestamp)
