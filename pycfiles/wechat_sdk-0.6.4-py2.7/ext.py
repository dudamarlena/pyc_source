...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/wechat_sdk/ext.py
# Compiled at: 2016-04-12 23:03:20
import hashlib, re, requests, json, random, time
...
class WechatExt(WechatBase):
    ...
    def __init__(self, username, password, token=None, cookies=None, appid=None, plugin_token=None, ifencodepwd=False, login=True, checkssl=False):
        u"""
    ...
    def login(self, verify_code=''):
        ...
        if not s:
            ...
            elif re.search('readtemplate', r.text):
    ...
    def get_plugin_token_appid(self):
        ...
        return {'plugin_token': self.__plugin_token, 
           'appid': self.__appid}
    ...
    def send_message(self, fakeid, content):
        ...
        payload = {'tofakeid': fakeid, 
           'type': 1, 
           'token': self.__token, 
           'content': content, 
           'ajax': 1}
        headers = {'x-requested-with': 'XMLHttpRequest', 
           'referer': ('https://mp.weixin.qq.com/cgi-bin/singlesendpage?t=message/send&action=index&tofakeid={fakeid}&token={token}&lang=zh_CN').format(fakeid=fakeid, token=self.__token), 
           'cookie': self.__cookies}
        r = requests.post(url, data=payload, headers=headers)
        try:
    ...
    def get_user_list(self, page=0, pagesize=10, groupid=0):
        ...
        :raises NeedLoginError: 操作未执行成功, 需要再次尝试登录, 异常内容为服务器返回的错误数据
        """
        url = ('https://mp.weixin.qq.com/cgi-bin/contactmanage?t=user/index&pagesize={pagesize}&pageidx={page}&type=0&groupid={groupid}&lang=zh_CN&f=json&token={token}').format(pagesize=pagesize, page=page, groupid=groupid, token=self.__token)
        headers = {'x-requested-with': 'XMLHttpRequest', 
           'referer': ('https://mp.weixin.qq.com/cgi-bin/contactmanage?t=user/index&pagesize={pagesize}&pageidx={page}&type=0&groupid=0&lang=zh_CN&token={token}').format(pagesize=pagesize, page=page, token=self.__token), 
           'cookie': self.__cookies}
    ...
    def get_group_list(self):
        ...
        :raises NeedLoginError: 操作未执行成功, 需要再次尝试登录, 异常内容为服务器返回的错误数据
        """
        url = ('https://mp.weixin.qq.com/cgi-bin/contactmanage?t=user/index&pagesize=10&pageidx=0&type=0&groupid=0&lang=zh_CN&f=json&token={token}').format(token=self.__token)
        headers = {'x-requested-with': 'XMLHttpRequest', 
           'referer': ('https://mp.weixin.qq.com/cgi-bin/contactmanage?t=user/index&pagesize=10&pageidx=0&type=0&groupid=0&lang=zh_CN&token=').format(token=self.__token), 
           'cookie': self.__cookies}
        r = requests.get(url, headers=headers)
        try:
    ...
    def get_news_list(self, page, pagesize=10):
        ...
        """
        begin = page * pagesize
        url = ('https://mp.weixin.qq.com/cgi-bin/appmsg?token={token}&lang=zh_CN&type=10&action=list&begin={begin}&count={pagesize}&f=json&random={random}').format(token=self.__token, begin=begin, pagesize=pagesize, random=round(random.random(), 3))
        headers = {'x-requested-with': 'XMLHttpRequest', 
           'referer': ('https://mp.weixin.qq.com/cgi-bin/masssendpage?t=mass/send&token={token}&lang=zh_CN').format(token=self.__token), 
           'cookie': self.__cookies}
        r = requests.get(url, headers=headers)
        try:
    ...
    def get_dialog_message(self, fakeid, last_msgid=0, create_time=0):
        ...
        """
        self._init_fakeid()
        url = ('https://mp.weixin.qq.com/cgi-bin/singlesendpage?tofakeid={fakeid}&action=sync&lastmsgfromfakeid={fromfakeid}&lastmsgid={last_msgid}&createtime={create_time}&token={token}&lang=zh_CN&f=json&ajax=1').format(fakeid=fakeid, fromfakeid=self.__fakeid, last_msgid=last_msgid, create_time=create_time, token=self.__token)
        headers = {'x-requested-with': 'XMLHttpRequest', 
    ...
    def send_news(self, fakeid, msgid):
        ...
        headers = {'x-requested-with': 'XMLHttpRequest', 
           'referer': ('https://mp.weixin.qq.com/cgi-bin/singlemsgpage?fromfakeid={fakeid}&msgid=&source=&count=20&t=wxm-singlechat&lang=zh_CN').format(fakeid=fakeid), 
           'cookie': self.__cookies}
        r = requests.post(url, data=payload, headers=headers)
        try:
            message = json.loads(r.text)
    ...
    def add_news(self, news):
        ...
        url = ('https://mp.weixin.qq.com/cgi-bin/operate_appmsg?lang=zh_CN&t=ajax-response&sub=create&token={token}').format(token=self.__token)
        payload = {'token': self.__token, 
           ...
           'AppMsgId': '', 
           'error': 'false'}
        headers = {'referer': ('https://mp.weixin.qq.com/cgi-bin/operate_appmsg?lang=zh_CN&sub=edit&t=wxm-appmsgs-edit-new&type=10&subtype=3&token={token}').format(token=self.__token), 
           'cookie': self.__cookies}
        i = 0
        for item in news:
    ...
    def upload_file(self, filepath):
        ...
        """
        self._init_ticket()
        url = ('https://mp.weixin.qq.com/cgi-bin/filetransfer?action=upload_material&f=json&ticket_id={ticket_id}&ticket={ticket}&token={token}&lang=zh_CN').format(ticket_id=self.__ticket_id, ticket=self.__ticket, token=self.__token)
        try:
            files = {'file': open(filepath, 'rb')}
        except IOError:
            raise ValueError('file not exist')

        payloads = {'Filename': filepath, 'folder': '/cgi-bin/uploads', 
           'Upload': 'Submit Query'}
        headers = {'referer': 'http://mp.weixin.qq.com/cgi-bin/indexpage?t=wxm-upload&lang=zh_CN&type=2&formId=1', 
           'cookie': self.__cookies}
    ...
    def send_file(self, fakeid, fid, type):
        ...
        if type == 4:
            type = 15
        url = ('https://mp.weixin.qq.com/cgi-bin/singlesend?t=ajax-response&f=json&token={token}&lang=zh_CN').format(token=self.__token)
        payloads = {}
        if type == 2 or type == 3:
            payloads = {'token': self.__token, 'lang': 'zh_CN', 
               'f': 'json', 
               'ajax': 1, 
               'random': random.random(), 
               'type': type, 
            ...
            if type == 15:
                payloads = {'token': self.__token, 'lang': 'zh_CN', 
                   ...
                   'appmsgid': fid, 
                   'imgcode': ''}
            headers = {'referer': ('https://mp.weixin.qq.com/cgi-bin/singlesendpage?tofakeid={fakeid}&t=message/send&action=index&token={token}&lang=zh_CN').format(fakeid=fakeid, token=self.__token), 
               'cookie': self.__cookies, 
    ...
    def get_file_list(self, type, page, count=10):
        ...
        :raises NeedLoginError: 操作未执行成功, 需要再次尝试登录, 异常内容为服务器返回的错误数据
        """
        url = ('https://mp.weixin.qq.com/cgi-bin/filepage?token={token}&lang=zh_CN&type={type}&random={random}&begin={begin}&count={count}&f=json').format(token=self.__token, type=type, random=round(random.random(), 3), begin=page * count, count=count)
        headers = {'x-requested-with': 'XMLHttpRequest', 
           'referer': ('https://mp.weixin.qq.com/cgi-bin/masssendpage?t=mass/send&token={token}&lang=zh_CN').format(token=self.__token), 
           'cookie': self.__cookies}
        r = requests.get(url, headers=headers)
    ...
    def get_user_info(self, fakeid):
        ...
        payloads = {'ajax': 1, 
           ...
           'fakeid': fakeid}
        headers = {'x-requested-with': 'XMLHttpRequest', 
           'referer': ('https://mp.weixin.qq.com/cgi-bin/getmessage?t=wxm-message&lang=zh_CN&count=50&token={token}').format(token=self.__token), 
           'cookie': self.__cookies}
        r = requests.post(url, data=payloads, headers=headers)
        try:
    ...
    def get_avatar(self, fakeid):
        ...
        :raises NeedLoginError: 操作未执行成功, 需要再次尝试登录, 异常内容为服务器返回的错误数据
        """
        url = ('https://mp.weixin.qq.com/misc/getheadimg?fakeid={fakeid}&token={token}&lang=zh_CN').format(fakeid=fakeid, token=self.__token)
        headers = {'x-requested-with': 'XMLHttpRequest', 
           'referer': ('https://mp.weixin.qq.com/cgi-bin/getmessage?t=wxm-message&lang=zh_CN&count=50&token={token}').format(token=self.__token), 
           'cookie': self.__cookies}
    ...
    def get_new_message_num(self, lastid=0):
        ...
        :return: 消息数目
        """
        url = ('https://mp.weixin.qq.com/cgi-bin/getnewmsgnum?f=json&t=ajax-getmsgnum&lastmsgid={lastid}&token={token}&lang=zh_CN').format(lastid=lastid, token=self.__token)
        payloads = {'ajax': 1, 
           'f': 'json', 
           'random': random.random(), 
           'lang': 'zh_CN', 
    ...
    def get_message_list(self, lastid=0, offset=0, count=20, day=7, star=False):
        ...
        if lastid == 0:
            lastid = ''
        url = ('https://mp.weixin.qq.com/cgi-bin/message?t=message/list&f=json&lang=zh_CN{star}&count={count}&day={day}&frommsgid={lastid}&offset={offset}&token={token}').format(star=star_param, count=count, day=day, lastid=lastid, offset=offset, token=self.__token)
        headers = {'x-requested-with': 'XMLHttpRequest', 
           'referer': ('https://mp.weixin.qq.com/cgi-bin/message?t=message/list&count=20&day=7&token={token}&lang=zh_CN').format(token=self.__token), 
           'cookie': self.__cookies}
        r = requests.get(url, headers=headers)
    ...
    def get_message_image(self, msgid, mode='large'):
        ...
        url = ('https://mp.weixin.qq.com/cgi-bin/getimgdata?token={token}&msgid={msgid}&mode={mode}&source=&fileId=0').format(msgid=msgid, token=self.__token, mode=mode)
        headers = {'x-requested-with': 'XMLHttpRequest', 
           'referer': ('https://mp.weixin.qq.com/cgi-bin/message?t=message/list&token={token}&count=20&day=7').format(token=self.__token), 
           'cookie': self.__cookies}
        r = requests.get(url, headers=headers, stream=True)
    ...
    def get_message_video(self, msgid):
        ...
        """
        url = ('https://mp.weixin.qq.com/cgi-bin/getvideodata?msgid={msgid}&fileid=&token={token}').format(msgid=msgid, token=self.__token)
        headers = {'x-requested-with': 'XMLHttpRequest', 
           'referer': ('https://mp.weixin.qq.com/cgi-bin/message?t=message/list&token={token}&count=20&day=7').format(token=self.__token), 
           'cookie': self.__cookies}
        r = requests.get(url, headers=headers, stream=True)
    ...
    def _init_self_information(self):
        ...
        headers = {'x-requested-with': 'XMLHttpRequest', 
           ...
           'cookie': self.__cookies}
        r = requests.get(url, headers=headers)
        ticket_id = re.search('user_name:\\"(.*)\\"', r.text)
        if not ticket_id:
            raise NeedLoginError(r.text)
        self.__ticket_id = ticket_id.group(1)
        ticket = re.search('ticket:\\"(.*)\\"', r.text)
        if not ticket:
            raise NeedLoginError(r.text)
        self.__ticket = ticket.group(1)
        fakeid = re.search('uin:\\"(.*)\\"', r.text)
        if not fakeid:
            raise NeedLoginError(r.text)
    ...
    def _init_plugin_token_appid(self):
        ...
        if not self.__plugin_token or not self.__appid:
            url = ('https://mp.weixin.qq.com/misc/pluginloginpage?action=stat_article_detail&pluginid=luopan&t=statistics/index&token={token}&lang=zh_CN').format(token=self.__token)
            headers = {'x-requested-with': 'XMLHttpRequest', 
               'referer': ('https://mp.weixin.qq.com/misc/pluginloginpage?action=stat_article_detail&pluginid=luopan&t=statistics/index&token={token}&lang=zh_CN').format(token=self.__token), 
               'cookie': self.__cookies}
            r = requests.get(url, headers=headers)
            plugin_token = re.search("pluginToken : '(\\S+)',", r.text)
            if not plugin_token:
                raise NeedLoginError(r.text)
            self.__plugin_token = plugin_token.group(1)
            appid = re.search("appid : '(\\S+)',", r.text)
            if not appid:
                raise NeedLoginError(r.text)
