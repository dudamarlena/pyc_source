...
# [GCC 8.4.0]
# Embedded file name: /home/travis/virtualenv/python3.7.1/lib/python3.7/site-packages/onepassword_tools/ssh_add/ssh_add.py
...
import click, pexpect, sys, os, textwrap
...
class SSHAdd:
    ...
    def _get_ssh_config_match_original_host(self):
        hosts = [
         self.item.get('Hostname', strict=False)]
    ...
    def add(self, search):
        if not self.onePasswordUtils.is_authenticated(check_mode='local'):
            ...
            if isinstance(item, list):
                ...
                if len(items) > 0:
                    import inquirer
                    choices = [inquirer.List('results', message='Which key do you want ?', choices=['%s %s' % (key.uuid, key.overview['title']) for key in items])]
                    ...
                    self.no_ssh_config or self.create_ssh_config_file()
                if self.key_has_been_registered(self.title):
    ...
    def create_ssh_config_file(self):
        ...
        config = textwrap.dedent(Template('            #uuid: $uuid\n            Match $match \n              IdentitiesOnly yes\n              IdentityFile $private_key_file_path\n              Hostname $to_host\n              $to_port\n              ').substitute(uuid=(self.item.get('uuid')),
        ...
        with open(ssh_config_file_path, 'w+') as (config_file):
            config_file.write(config)
            config_file.flush()
            os.chmod(config_file.name, 384)
    ...
    def init_path(path):
        ...
        if not os.path.isdir(path):
            ...
            if os.stat(path).st_mode & 511 != 448:
                os.chmod(path, 448)
