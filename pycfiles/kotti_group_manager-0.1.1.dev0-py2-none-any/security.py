...
# [GCC 8.4.0]
# Embedded file name: /home/ubuntu/workspace/src/kotti_group_manager/kotti_group_manager/security.py
# Compiled at: 2018-09-20 00:11:56
...
from kotti_toolkit.security import create_group, find_groups_by_email_domain, is_group
...
def create_groups_listing_page():
    ...
    if container_name not in root:
        listing_page = root[container_name] = Document(name=container_name, title=_('Groups'), description=_('List groups'), creation_date=datetime.now(), modification_date=datetime.now())
        return listing_page
...
def create_group_page_from_group(group):
    ...
    if group_name in root:
        ...
        group_page = root[group_name] = GroupPage(title=group.title, group_name=group.name, creation_date=datetime.now(), modification_date=datetime.now())
    ...
    if is_group(user):
        ...
        try:
            ...
            if email_domain in blacklisted_domains:
                raise ValueError(('Email domain for {} is black listed').format(user.email))
            ...
            if not group:
                ...
                group = create_group(title=group_title, email=('groupinfo@{}').format(email_domain), name=group_name)
                if group is False:
                    raise ValueError(('Group {} was not created').format(group_title))
            else:
                group_name = group.name.replace('group:', '')
