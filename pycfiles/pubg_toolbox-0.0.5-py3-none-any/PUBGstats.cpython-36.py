...
# Embedded file name: /home/suliman/Documents/Projects/pipTests/PUBGbot/pubgReports/PUBGstats.py
# Compiled at: 2019-08-02 21:47:16
...
def matchAnalysis(player, matchData):
    log = {'name':None, 
     'kills':None,  'knocks':None,  'assists':None,  'headshots':None, 
     'revives':None,  'heals':None,  'boosts':None, 
     'walk-distance':None,  'kill-rank':None,  'weapons-acquired':None, 
     'time-survived':None,  'damage-dealt':None, 
     'longest-kill':None,  'kill-streak':None,  'win-rank':None}
        ...
        for report in playerData['included']:
            if report['type'] == 'participant':
                if report['attributes']['stats']['killPlace'] == 1:
                    P1 = (
                     report['attributes']['stats']['name'], report['attributes']['stats']['kills'])
...
def getTeamMembersNames(player, mode, matchData):
    ...
    if mode == 'duo':
        ...
        if secondTeamMemberID == None:
            return
        for report in matchData['included']:
            if report['type'] == 'participant' and report['id'] == secondTeamMemberID:
                return report['attributes']['stats']['name']

    if mode == 'squad':
        for report in matchData['included']:
            if report['type'] == 'participant' and report['attributes']['stats']['name'] == player:
                currentPlayerTeamID = report['id']
                break
