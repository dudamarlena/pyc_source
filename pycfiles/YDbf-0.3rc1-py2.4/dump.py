...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/ydbf/dump.py
# Compiled at: 2009-07-13 05:00:56
"""
YDbf dumper script
"""
import sys
...
' passed to ydbf.dump, OptionParser saves it as
    ...
    like '
', '
', '    ' both in record and field separators
    """
...
def show_info(files):
    ...
    for f in files:
        ...
        header_info = {'filename': f, 'signature': hex(reader.sig), 'version': lib.SIGNATURES.get(reader.sig, 'N/A'), 'lang_code': hex(reader.raw_lang), 'encoding': lib.ENCODINGS.get(reader.raw_lang, ('n/a', 'N/A'))[0], 'language': lib.ENCODINGS.get(reader.raw_lang, ('n/a', 'N/A'))[1], 'records_number': str(reader.numrec), 'header_length': str(reader.lenheader), 'record_length': str(reader.recsize), 'last_change': str(reader.dt), 'fields_number': str(reader.numfields)}
        ...
        for (i, (name, type_, length, dec)) in enumerate(reader.fields):
            print '% 3d.  %s  %s  %s  %d' % (i + 1, name.ljust(20), type_, str(length).rjust(3), dec)
...
def parse_options(args):
    ...
    parser.add_option('-r', '--rs', dest='record_separator', action='callback', callback=_unescape_separator, default='\n', type='string', help='output record separator [default newline]')
    (
     parser.add_option('-f', '--fs', dest='field_separator', action='callback', callback=_unescape_separator, default=':', type='string', help='output field separator [default colon]'),)
    parser.add_option('-F', '--fields', dest='fields', action='callback', callback=_split_fields, default='', type='string', help='comma separated list of fields to print [default all]')
    (
     parser.add_option('-u', '--undef', dest='undef', type='string', default='', help='string to print for NULL values [default emptystring]'),)
    (
     parser.add_option('-t', '--table', dest='table', action='store_true', default=False, help='output in table format [default false]'),)
    parser.add_option('-o', '--output', dest='output', type='string', default='', help='output file')
    (
     parser.add_option('-i', '--info', dest='info', action='store_true', default=False, help='show info about file and exit'),)
