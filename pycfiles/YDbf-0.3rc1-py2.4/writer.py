...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/ydbf/writer.py
# Compiled at: 2009-07-14 02:24:42
"""
DBF writer
"""
__all__ = [
...
class YDbfWriter(object):
    ...
    def _defineLangCode(self):
        ...
        if not lang_code:
            raise ValueError('Encoding %s is not available for DBF, please use one of: %s' % (self.encoding, encodings))
        self.lang = lang_code[0]

    def _makeActions(self):
        self.actions = {'date': lambda val, size, dec: val and self.date2dbf(val) or '        ', 
        ...
        for (name, typ, size, dec) in self.fields:
            ...
            if not action:
                raise ValueError('Cannot find python-to-dbf converter for field %s (type %s)' % (name, typ))
    ...
    def _writeHeader(self):
        ...
        self.hdr = struct.pack(lib.HEADER_FORMAT, self.sig, year, month, day, self.numrec, self.lenheader, self.recsize, self.lang)
    ...
    def write(self, records):
        ...
        for rec in records:
            i += 1
            try:
                raw_rec = ('').join((self.converters[name](rec[name], size, dec) for (name, typ, size, dec) in self.fields))
                ...
                self.flush()
                raise RuntimeError('Error occured (%s: %s) while reading rec #%d. Record data: %s' % (err.__class__.__name__, err, i, rec))
