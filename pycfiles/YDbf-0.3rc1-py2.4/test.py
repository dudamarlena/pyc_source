...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/ydbf/test.py
# Compiled at: 2009-07-14 00:06:51
"""
Unit-tests for YDbf
"""
import datetime, unittest, tempfile, decimal, os
...
class TestYDbfReader(unittest.TestCase):
    ...
    def test_constructor(self, fh):
        ...
        self.assertEquals(YDbfReader(StringIO(dbf_data), use_unicode=True).encoding, 'ascii')
        self.assertEquals(YDbfReader(StringIO(dbf_data), use_unicode=False).encoding, None)
        self.assertEquals(YDbfReader(StringIO(dbf_data), use_unicode=False, encoding='cp866').encoding, None)
        self.assertEquals(YDbfReader(StringIO(dbf_data), use_unicode=True, encoding='cp866').encoding, 'cp866')
        return
    ...
    def test_header(self, fh):
        dbf = YDbfReader(fh)
        self.assertEqual(dbf._fields, [('_deletion_flag', 'C', 1, 0), ('INT_FLD', 'N', 4, 0), ('FLT_FLD', 'N', 5, 2), ('CHR_FLD', 'C', 6, 0), ('DTE_FLD', 'D', 8, 0), ('BLN_FLD', 'L', 1, 0)])
        self.assertEqual(dbf.fields, [('INT_FLD', 'N', 4, 0), ('FLT_FLD', 'N', 5, 2), ('CHR_FLD', 'C', 6, 0), ('DTE_FLD', 'D', 8, 0), ('BLN_FLD', 'L', 1, 0)])
        ...
        self.assertEqual(dbf.field_names, ['INT_FLD', 'FLT_FLD', 'CHR_FLD', 'DTE_FLD', 'BLN_FLD'])
    ...
    def test_call(self, fh):
        dbf = YDbfReader(fh)
        reference_data = [{'INT_FLD': 25, 'FLT_FLD': decimal.Decimal('12.34'), 'CHR_FLD': 'test', 'DTE_FLD': datetime.date(2006, 5, 7), 'BLN_FLD': True}, {'INT_FLD': 113, 'FLT_FLD': decimal.Decimal('1.01'), 'CHR_FLD': 'del', 'DTE_FLD': datetime.date(2006, 12, 23), 'BLN_FLD': False}]
    ...
    def test_read_deleted(self, fh):
        dbf = YDbfReader(fh)
        reference_data = [{'_deletion_flag': '', 'INT_FLD': 25, 'FLT_FLD': decimal.Decimal('12.34'), 'CHR_FLD': 'test', 'DTE_FLD': datetime.date(2006, 5, 7), 'BLN_FLD': True}, {'_deletion_flag': '', 'INT_FLD': 113, 'FLT_FLD': decimal.Decimal('1.01'), 'CHR_FLD': 'del', 'DTE_FLD': datetime.date(2006, 12, 23), 'BLN_FLD': False}, {'_deletion_flag': '*', 'INT_FLD': 7436, 'FLT_FLD': decimal.Decimal('0.50'), 'CHR_FLD': 'ex.', 'DTE_FLD': datetime.date(2006, 7, 15), 'BLN_FLD': True}]
...
class TestYdbfWriter(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.reference_data = [{'INT_FLD': 25, 'FLT_FLD': 12.34, 'CHR_FLD': 'test', 'DTE_FLD': datetime.date(2006, 5, 7), 'BLN_FLD': True}, {'INT_FLD': 113, 'FLT_FLD': 1.01, 'CHR_FLD': 'del', 'DTE_FLD': datetime.date(2006, 12, 23), 'BLN_FLD': False}, {'INT_FLD': 7436, 'FLT_FLD': 0.5, 'CHR_FLD': 'ex.', 'DTE_FLD': datetime.date(2006, 7, 15), 'BLN_FLD': True}]
        self.fields = [
         ('INT_FLD', 'N', 4, 0), ('FLT_FLD', 'N', 5, 2), ('CHR_FLD', 'C', 6, 0), ('DTE_FLD', 'D', 8, 0), ('BLN_FLD', 'L', 1, 0)]
        self.fh = StringIO()
    ...
    def test_wrongtype(self):
        fields = (
         ('INT_FLD', 'N', 4, 0), ('FLT_FLD', 'N', 5, 2), ('WRNG_FLD', b'\xd1', 6, 0), ('DTE_FLD', 'D', 8, 0), ('BLN_FLD', 'L', 1, 0))
        fh = StringIO('')
