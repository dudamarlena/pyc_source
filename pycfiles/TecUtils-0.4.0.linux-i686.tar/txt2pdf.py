...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/tecutils/txt2pdf.py
# Compiled at: 2012-08-03 18:16:23
"""
...
parser.add_option('-c', '--copies', help='number of copies, only valid with -p option', dest='copies', type='int')
parser.add_option('-g', help='print through ghostprint, only valid with -w option', dest='ghost', action='store_true')
parser.add_option('-m', help='use half letter as size of output, default letter', dest='letter_size', action='store_false')
parser.add_option('--output', help='use specific output file name', dest='output', action='store')
parser.add_option('-p', '--print', help='print file after converting', dest='printing', action='store_true')
parser.add_option('--printer', help='printer to send file, default: send to default printer', dest='printer', action='store')
parser.add_option('-w', help='use win32api to send file to print, only valid with -p option', dest='linux', action='store_false')
...
def ghost_print(archivo, copias=1, printer=None):
    ...
    for i in range(copias):
        if printer:
            p = subprocess.Popen(['gsprint.exe', archivo, '-printer %s' % printer], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        else:
            p = subprocess.Popen(['gsprint.exe', archivo], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def linux_print(archivo, copias=1, printer=None):
    ...
    import subprocess
    if printer:
        p = subprocess.Popen(['lp -n %d' % copias, '-d %s' % printer, archivo], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    else:
        p = subprocess.Popen(['lp -n %d' % copias, archivo], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout, stderr = p.communicate()
