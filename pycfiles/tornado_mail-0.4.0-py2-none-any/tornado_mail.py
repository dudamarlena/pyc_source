...
# [GCC 8.4.0]
# Embedded file name: tornado_mail.py
# Compiled at: 2017-02-10 23:09:01
import smtplib, time
...
def _has_newline(line):
    r"""Used by has_bad_header to check for \r or \n"""
...
class Connection(object):
    ...
    def send(self, message, envelope_from=None):
        ...
        if self.host:
            self.host.sendmail(sanitize_address(envelope_from or message.sender), list(sanitize_addresses(message.send_to)), message.as_string(), message.mail_options, message.rcpt_options)
        return
...
class Message(object):
    ...
    def __init__(self, subject='', recipients=None, body=None, html=None, alts=None, sender=None, cc=None, bcc=None, reply_to=None, date=None, charset=None, extra_headers=None, mail_options=None, rcpt_options=None):
        sender = sender
    ...
    def has_bad_headers(self):
        headers = [
         self.sender, self.reply_to] + self.recipients
...
class _Mail(_MailMixin):
    ...
    def __init__(self, server, username, password, port, use_tls, use_ssl, default_sender, debug, max_emails, suppress, ascii_attachments=False):
        self.server = server
...
class Mail(_MailMixin):
    ...
    def init_mail(self, config, debug=False, testing=False):
        return _Mail(config.get('MAIL_SERVER', '127.0.0.1'), config.get('MAIL_USERNAME'), config.get('MAIL_PASSWORD'), config.get('MAIL_PORT', 25), config.get('MAIL_USE_TLS', False), config.get('MAIL_USE_SSL', False), config.get('MAIL_DEFAULT_SENDER'), int(config.get('MAIL_DEBUG', debug)), config.get('MAIL_MAX_EMAILS'), config.get('MAIL_SUPPRESS_SEND', testing), config.get('MAIL_ASCII_ATTACHMENTS', False))
...
if __name__ == '__main__':
    config = dict(MAIL_SERVER='smtp.ym.163.com', MAIL_PORT=465, MAIL_USERNAME='ApeSo@finder.ren', MAIL_PASSWORD='723520wei', MAIL_USE_SSL=True, MAIL_DEFAULT_SENDER='ApeSo@finder.ren', MAIL_RECIPIENTS=[
     '819799762@qq.com', 'chiabingwei@163.com'])
    mail = Mail(config)
    _mail = mail.init_app(config)
    msg = Message('Hello Tornado', sender=config.get('MAIL_DEFAULT_SENDER'), recipients=config.get('MAIL_RECIPIENTS'))
    mail.send(msg, _mail)