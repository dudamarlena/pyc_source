...
# [GCC 8.4.0]
# Embedded file name: /Users/uranusjr/Documents/programming/python/django-crispy-forms/django-crispy-forms-ng/crispy_forms/tests/test_layout.py
# Compiled at: 2015-04-08 09:53:12
import django
...
from .forms import TestForm, TestForm2, TestForm3, CheckboxesTestForm, TestForm4, CrispyTestModel, TestForm5
from .utils import override_settings
from crispy_forms.bootstrap import InlineCheckboxes
from crispy_forms.helper import FormHelper
from crispy_forms.layout import Layout, Fieldset, MultiField, Row, Column, HTML, ButtonHolder, Div, Submit
...
class TestFormLayout(CrispyTestCase):
    ...
    def test_layout_fieldset_row_html_with_unicode_fieldnames(self):
        form_helper = FormHelper()
        form_helper.add_layout(Layout(Fieldset('Company Data', 'is_company', css_id='fieldset_company_data', css_class='fieldsets', title='fieldset_title', test_fieldset='123'), Fieldset('User Data', 'email', Row('password1', 'password2', css_id='row_passwords', css_class='rows'), HTML('<a href="#" id="testLink">test link</a>'), HTML('\n                        {% if flag %}{{ message }}{% endif %}\n                    '), 'first_name', 'last_name')))
        template = get_template_from_string('\n            {% load crispy_forms_tags %}\n            {% crispy form form_helper %}\n        ')
    ...
    def test_change_layout_dynamically_delete_field(self):
        ...
        form = TestForm()
        form_helper = FormHelper()
        form_helper.add_layout(Layout(Fieldset('Company Data', 'is_company', 'email', 'password1', 'password2', css_id='multifield_info'), Column('first_name', 'last_name', css_id='column_name')))
        del form.fields['email']
    ...
    def test_formset_layout(self):
        ...
        helper.layout = Layout(Fieldset('Item {{ forloop.counter }}', 'is_company', 'email'), HTML('{% if forloop.first %}Note for first form only{% endif %}'), Row('password1', 'password2'), Fieldset('', 'first_name', 'last_name'))
        html = render_crispy_form(form=formset, helper=helper, context={'csrf_token': _get_new_csrf_key()})
        django_version = django.VERSION
        self.assertInHTML('<input type="hidden" name="form-TOTAL_FORMS" value="3" id="id_form-TOTAL_FORMS" />', html, count=1)
        self.assertInHTML('<input id="id_form-INITIAL_FORMS" name="form-INITIAL_FORMS" type="hidden" value="0" />', html, count=1)
        if (1, 4) <= django_version < (1, 4, 4) or django_version < (1, 3, 6):
            needle = '<input type="hidden" name="form-MAX_NUM_FORMS" id="id_form-MAX_NUM_FORMS" />'
        else:
            needle = '<input type="hidden" name="form-MAX_NUM_FORMS" value="1000" id="id_form-MAX_NUM_FORMS" />'
    ...
    def test_modelformset_layout(self):
        ...
        django_version = django.VERSION
        self.assertInHTML('<input type="hidden" name="form-TOTAL_FORMS" value="3" id="id_form-TOTAL_FORMS" />', html, count=1)
        self.assertInHTML('<input type="hidden" name="form-INITIAL_FORMS" value="0" id="id_form-INITIAL_FORMS" />', html, count=1)
        if django_version >= (1, 4) and django_version < (1, 4, 4) or django_version < (1,
                                                                                        3,
                                                                                        6):
            needle = '<input type="hidden" name="form-MAX_NUM_FORMS" id="id_form-MAX_NUM_FORMS" />'
        else:
...
class TestUniformFormLayout(TestFormLayout):
    ...
    def test_layout_composition(self):
        form_helper = FormHelper()
        form_helper.add_layout(Layout(Layout(MultiField('Some company data', 'is_company', 'email', css_id='multifield_info')), Column('first_name', css_id='column_name', css_class='columns'), ButtonHolder(Submit('Save', 'Save', css_class='button white')), Div('password1', 'password2', css_id='custom-div', css_class='customdivs')))
        template = get_template_from_string('\n                {% load crispy_forms_tags %}\n                {% crispy form form_helper %}\n            ')
        c = Context({'form': TestForm(), 'form_helper': form_helper})
        html = template.render(c)
