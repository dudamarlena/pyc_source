...
# [GCC 8.4.0]
# Embedded file name: /Users/uranusjr/Documents/programming/python/django-crispy-forms/django-crispy-forms-ng/crispy_forms/tests/test_layout_objects.py
# Compiled at: 2015-04-08 09:51:17
# Size of source mod 2**32: 15693 bytes
...
from crispy_forms.bootstrap import PrependedAppendedText, AppendedText, PrependedText, InlineRadios, Tab, TabHolder, AccordionGroup, Accordion, Alert, InlineCheckboxes, FieldWithButtons, StrictButton
from crispy_forms.helper import FormHelper
from crispy_forms.layout import Layout, HTML, Field, MultiWidgetField
...
class TestLayoutObjects(CrispyTestCase):
    ...
    def test_multiwidget_field(self):
        ...
        test_form.helper.layout = Layout(MultiWidgetField('datetime_field', attrs=({'rel': 'test_dateinput'}, {'rel': 'test_timeinput',  'style': 'width: 30px;',  'type': 'hidden'})))
...
class TestBootstrapLayoutObjects(TestLayoutObjects):
    ...
    def test_prepended_appended_text(self):
        test_form = TestForm()
        test_form.helper = FormHelper()
        test_form.helper.layout = Layout(PrependedAppendedText('email', '@', 'gmail.com'), AppendedText('password1', '#'), PrependedText('password2', '$'))
        html = render_crispy_form(test_form)
        if self.current_template_pack == 'bootstrap':
    ...
    def test_accordion_and_accordiongroup(self):
        test_form = TestForm()
        test_form.helper = FormHelper()
        test_form.helper.layout = Layout(Accordion(AccordionGroup('one', 'first_name'), AccordionGroup('two', 'password1', 'password2')))
        html = render_crispy_form(test_form)
        if self.current_template_pack == 'bootstrap':
    ...
    def test_accordion_active_false_not_rendered(self):
        test_form = TestForm()
        test_form.helper = FormHelper()
        test_form.helper.layout = Layout(Accordion(AccordionGroup('one', 'first_name')))
        html = render_crispy_form(test_form)
        if self.current_template_pack == 'bootstrap':
        ...
        test_form.helper.layout = Layout(Accordion(AccordionGroup('one', 'first_name', active=False)))
        html = render_crispy_form(test_form)
    ...
    def test_tab_and_tab_holder(self):
        test_form = TestForm()
        test_form.helper = FormHelper()
        test_form.helper.layout = Layout(TabHolder(Tab('one', 'first_name', css_id='custom-name', css_class='first-tab-class'), Tab('two', 'password1', 'password2')))
        html = render_crispy_form(test_form)
        self.assertEqual(html.count('<li class="tab-pane active"><a href="#custom-name" data-toggle="tab">One</a></li>'), 1)
    ...
    def test_field_with_buttons(self):
        form = TestForm()
        form.helper = FormHelper()
        form.helper.layout = Layout(FieldWithButtons(Field('password1', css_class='span4'), StrictButton('Go!', css_id='go-button'), StrictButton('No!', css_class='extra'), StrictButton('Test', type='submit', name='whatever', value='something'), css_class='extra', autocomplete='off'))
        html = render_crispy_form(form)
    ...
    def test_hidden_fields(self):
        ...
        form.helper = FormHelper()
        form.helper.layout = Layout(AppendedText('password1', 'foo'), PrependedText('password2', 'bar'), PrependedAppendedText('email', 'bar'), InlineCheckboxes('first_name'), InlineRadios('last_name'))
        html = render_crispy_form(form)
