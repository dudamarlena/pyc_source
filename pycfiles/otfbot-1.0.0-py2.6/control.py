...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/otfbot/services/control.py
...
class botService(service.MultiService):
    ...
    def register_command(self, f, namespace=None, name=None):
        ...
        if name is None:
            name = f.__name__
        if not namespace:
            if f.__name__ not in self.commandTree:
        ...
        if name not in self.commandList:
        ...
        for element in args:
            if element in commandTree and type(commandTree[element]) == dict:
                ...
                if element in commandTree:
                    ...
                    if element in commandTree:
                        return repr(commandTree[element])
