...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/otfbot/lib/pluginSupport/pluginSupport.py
...
class pluginSupport:
    ...
    def depends_on_plugin(self, dependency, description='', service=''):
        ...
        plugins = self.config.get(service + 'Plugins', [], 'main', set_default=False)
        pluginsDisabled = self.config.get('pluginsDisabled', [], 'main')
        if dependency not in plugins or service + '.' + dependency in pluginsDisabled:
    ...
    def startPlugins(self, package=None):
        ...
        """
        plugins = self.config.get(self.pluginSupportName + 'Plugins', [], 'main', set_default=False)
        for plginName in plugins:
            if plginName not in self.config.get('pluginsDisabled', [], 'main'):
                ...
                if hasattr(pluginClass.Meta, 'service_depends'):
                    if not set(pluginClass.Meta.service_depends).issubset(self.root.namedServices.keys()):
                        self.logger.debug('%s (still) has unsatisfied dependencies: %s' % (
                         ...
                         list(set(pluginClass.Meta.service_depends) - set(self.root.namedServices.keys()))))
                        return
    ...
    def reloadPlugins(self):
        ...
        for chatPlugin in self.plugins.values():
            self.restartPlugin(chatPlugin.name.split('.')[(-1)])
    ...
    def stopPlugin(self, pluginName):
        """
            ...
            @type pluginName: string
        """
        pkg = self.pluginSupportPath.replace('/', '.') + '.' + pluginName
        pluginName = pkg.replace('otfbot.plugins.', '')
    ...
    def logerror(self, logger, plugin, exception):
        ...
        if type(exception) == self.DependencyMissing or type(exception) == self.ModuleMissing or type(exception) == self.ServiceMissing or type(exception) == self.PluginMissing:
            ...
            return
        if type(exception) == self.WontStart:
        ...
        for plugin_and_priority in self.callbacks[apifunction]:
            ...
            if hasattr(self, 'network'):
                if 'channel' in args and (type(args['channel']) == str or type(args['channel']) == unicode) and args['channel'][0] in '#+!&':
                    ...
                    plugins = self.config.get('pluginsDisabled', [], 'main', self.network, args['channel'], set_default=False)
