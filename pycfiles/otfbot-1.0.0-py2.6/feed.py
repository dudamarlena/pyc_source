...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/otfbot/plugins/ircClient/feed.py
...
class Plugin(chatMod.chatMod):
    ...
    def loadNews(self, url):
        self.feedLastLoaded[url] = int(time.time())
    ...
    def postNews(self, channel, url, feedPostMax):
        ...
        for (url, headline) in self.feedHeadlines[url]:
            if channel in channels:
                if url not in self.readUrls[channel]:
                    if numPostUrls > 0:
                        numPostUrls -= 1
                        self.bot.sendmsg(channel, url + ' - ' + headline)
    ...
    def postNewsLoop(self, channel, url, curWait=5.0, minWait=1.0, maxWait=60.0, factor=1.5, feedPostMax=3):
        ...
        self.callIDs[url] = self.bot.root.getServiceNamed('scheduler').callLater(newWait * 60, self.postNewsLoop, channel, url, newWait, minWait, maxWait, factor, feedPostMax)
