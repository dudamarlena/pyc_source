...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/otfbot/plugins/ircClient/kurs.py
...
import string, re, urllib, time

class Plugin(chatMod.chatMod):
    ku = 'http://de.old.finance.yahoo.com/d/quotes.csv?s=%s&f=' + 'nsl1d1t1cohgvc4x&e=.csv'
    ...
    def parseWKN(self, data, channel):
        ...
        for line in data.split('\n'):
            if line[:3] == ' <a':
                res.append(re.sub('.*s\\=([^"]*)">([^<]*)<.*', '\\1;\\2', line).split(';'))
