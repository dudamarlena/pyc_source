...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/fc/dump.py
...
def key_sorted_dict_str(mset):
    return (', ').join([ '%s: %d' % (k, mset[k]) for k in sorted(mset.iterkeys())
                       ])
...
def detailed_view(fc, features_to_show=None, max_keys=100, lexigraphic=False, display_only=True):
    ...
    if '#NAME' in fc:
        top_lines = [
         'NAME: %s' % key_sorted_dict_str(fc.get('#NAME'))]
    else:
        if '#NOM' in fc:
            top_lines = [
             'NOM: %s' % key_sorted_dict_str(fc.get('#NOM'))]
        elif 'NAME' in fc:
            top_lines = [
             'NAME: %s' % key_sorted_dict_str(fc.get('NAME'))]
        elif 'NOM' in fc:
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Shows the features for each feature FeatureCollection in a Chunk', usage='dossier_feature_dump file.fc')
    parser.add_argument('input', metavar='file.fb', nargs='?', default='-', help='defaults to stdin')
    parser.add_argument('-o', '--out', default=None, dest='output', help='file to write output to, default stdout')
    parser.add_argument('-f', '--feature', metavar='FEATURE_NAME', default=[], dest='features_to_show', action='append', help='only print these features')
    parser.add_argument('--column-view', default=False, action='store_true', help='include this multiset in print out')
    parser.add_argument('--max-keys', default=100, type=int, help='number of keys to show in counters')
    parser.add_argument('--lexigraphic', default=False, action='store_true', help='sort counter keys lexigraphically instead of by count')
    parser.add_argument('--limit', default=None, type=int, metavar='LIMIT', help='stop after reading LIMIT records')
    parser.add_argument('--count', action='store_true', help='Output a count of the FCs and nothing else.')
    parser.add_argument('--show-hidden', action='store_true', help='Show hidden features.')
    ...
    for fc in i_chunk:
        if args.column_view:
            ...
            out_str = detailed_view(fc, features_to_show=args.features_to_show, max_keys=args.max_keys, lexigraphic=args.lexigraphic, display_only=not args.show_hidden)
        if out_str:
