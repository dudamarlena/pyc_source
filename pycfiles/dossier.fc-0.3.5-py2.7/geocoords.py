...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/fc/geocoords.py
        ...
        if data is not None:
            for fname, coords in data.items():
                if fname not in self.data:
                    self.data[fname] = []
                for coord in coords:
                    if not len(coord) == 4 or not all(map(lambda x: isinstance(x, self.allowed_types), coord)):
                        raise Exception('expecting four-tuples of lon, lat, alt, time and got: %s' % repr(coord))
