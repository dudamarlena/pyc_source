...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/fc/feature_tokens.py
...
class FeatureTokens(MutableMapping):
    ...
    def tokens(self, si, k):
        ...
        for tokens in self[k]:
            yield [ si.body.sentences[tagid][sid].tokens[tid] for tagid, sid, tid in tokens
                  ]
    ...
    def char_ranges(self, si, k):
        for offsets in self.offsets(si, OffsetType.CHARS, k):
            yield [ (o.first, o.first + o.length) for o in offsets if o is not None
                  ]
    ...
    def to_dict(self):
        ...
        for fname in self:
            ...
            for tokens in self[fname]:
                d[fname].append([ (uni(tagid), sid, tid) for tagid, sid, tid in tokens
                                ])
    ...
    def from_dict(d):
        ...
        for fname in d:
            for tokens in d[fname]:
                ft[fname].append([ (uni(tagid), sid, tid) for tagid, sid, tid in imap(tuple, tokens)
                                 ])
    ...
    def __setitem__(self, k, v):
        self._tokens[uni(k)] = v

    def __delitem__(self, k):
        del self._tokens[uni(k)]
