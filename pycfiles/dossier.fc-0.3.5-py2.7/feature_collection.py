...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/fc/feature_collection.py
...
import logging, operator, cbor, pkg_resources, streamcorpus
...
class FeatureCollection(MutableMapping):
    ...
    **Feature collection computation:**
    ...
    def loads(cls, data):
        ...
        if metadata['v'] != 'fc01':
            raise SerializationError(('invalid CBOR version {!r} (expected "fc01")').format(metadata['v']))
        ...
        for name, feat in data.iteritems():
            ...
            if isinstance(feat, cbor.Tag):
                if feat.tag not in cbor_tags_to_names:
                    raise SerializationError('Unknown CBOR value (tag id: %d): %r' % (
                     feat.tag, feat.value))
                ...
                raise SerializationError('Unknown CBOR value (type: %r): %r' % (type(feat), feat))
            value = loads(feat)
    ...
    def to_dict(self):
        ...
        def is_non_native_sc(ty, encoded):
            return ty == 'StringCounter' and not is_native_string_counter(encoded)
    ...
    def generation(self):
        ...
        """
        return max(getattr(collection, 'generation', 0) for collection in self._features.itervalues())
...
class FeatureTypeRegistry(object):
    ...
    DEFAULT_FEATURE_TYPE_PREFIX_NAMES = {FeatureCollection.DISPLAY_PREFIX: 'StringCounter', 
    ...
    def add(self, name, obj):
        ...
        if name not in cbor_names_to_tags:
            print(name)
            raise ValueError('Unsupported feature type name: "%s". Allowed feature type names: %r' % (
             name, cbor_names_to_tags.keys()))
        if not is_valid_feature_instance(ro):
            raise ValueError('Constructor for "%s" returned "%r" which has an unknown sub type "%r". (mro: %r). Object must be an instance of one of the allowed types: %r' % (
    ...
    def feature_type_name(self, feat_name, obj):
        ...
        if ty not in self._inverse:
            raise SerializationError('Python type "%s" is not in the feature type registry: %r\n\nThe offending feature (%s): %r' % (
    ...
    def _get(self, type_name):
        ...
        if type_name not in self._registry:
            raise SerializationError('Feature type %r not in feature type registry: %r' % (
...
cbor_names_to_tags = {'StringCounter': 55800, 
   'Unicode': None, 
   'SparseVector': 55801, 
   'DenseVector': 55802, 
   'FeatureTokens': 55804, 
...
def is_counter(obj):
    return isinstance(obj, Counter) or getattr(obj, 'is_counter', False)
