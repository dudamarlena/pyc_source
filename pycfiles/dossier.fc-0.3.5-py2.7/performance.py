...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/fc/tests/performance.py
...
def perftest_throughput_feature_collection():
    with registry:
        ...
        fc['thing1'] = Thing(json.dumps(dict(one_mb=' ' * 1048576)))
        fc_str = fc.dumps()
...
def perftest_truncation_speed(counter_type):
    for x in range(4, 7):
        ...
        elapsed = time.time() - start_time
        rate = num / elapsed
        print('%s truncated a counter of size %d in %.3f sec --> %.1f items/sec' % (
         counter_type, num, elapsed, rate))
