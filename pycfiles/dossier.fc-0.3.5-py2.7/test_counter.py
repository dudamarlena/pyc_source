...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/fc/tests/test_counter.py
...
def test_default(counter_type):
    ...
    assert isinstance(mc['foo'], counter_type), 'failed and made %s' % type(mc['foo'])
    mc['foo'] -= counter_type(Counter('dog'))
    assert isinstance(mc['foo'], counter_type), 'failed and made %s' % type(mc['foo'])
...
def test_build_from_dict(counter_type):
    mc = FeatureCollection({'hello': counter_type(Counter('hello')), 
...
def test_eq(counter_type):
    mc1 = FeatureCollection({'hello': counter_type(Counter('hello')), 
       'goodbye': counter_type(Counter('goodbye'))})
    mc2 = FeatureCollection({'hello': counter_type(Counter('hello')), 
       'goodbye': counter_type(Counter('goodbye'))})
    mc3 = FeatureCollection({'hello': counter_type(Counter('hello')), 
