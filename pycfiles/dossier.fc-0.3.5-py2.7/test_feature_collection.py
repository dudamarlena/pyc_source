...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/fc/tests/test_feature_collection.py
...
def test_fc_eq(counter_type):
    fc1 = FeatureCollection({'hello': counter_type(Counter('hello')), 
       'goodbye': counter_type(Counter('goodbye'))})
    fc2 = FeatureCollection({'hello': counter_type(Counter('hello')), 
       'goodbye': counter_type(Counter('goodbye'))})
    fc3 = FeatureCollection({'hello': counter_type(Counter('hello')), 
...
def test_non_counter_features_serialize():
    fc = FeatureCollection({'NAME': 'foobaz'})
...
def test_non_counter_features_inequality():
    fc1 = FeatureCollection({'NAME': 'foobaz'})
    fc2 = FeatureCollection({'NAME': 'blahblahblah'})
    assert fc1 != fc2
