...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/one_quant_data/data_engine.py
# Compiled at: 2020-03-17 02:32:37
# Size of source mod 2**32: 31901 bytes
import tushare as ts, sys, os, random, pandas as pd, progressbar, numpy as np, sys, pickle, multiprocessing, time, json
...
def dataframe_to_db_replace(df, table_name, conn):
    ...
    try:
        for lnum in d.keys():
            ...
            values = ','.join(map(lambda x:             if not isinstance(x, str):
str(x) # Avoid dead code: "'" + pymysql.escape_string(x) + "'", list(line.values())))
...
class DataEngine:
    ...
    def __init__(self, config_file='./config.json'):
        ...
        api = config['api']
        cache = config['cache']
        self.tables = {'stock_trade_daily':'pro_stock_k_daily', 
        ...
        if api.get('name') == 'tushare_pro':
            ...
            session.execute('CREATE TABLE IF NOT EXISTS {} (                            `ts_code` VARCHAR(10),                            `trade_date` VARCHAR(8),                            `open` Float(7,2),                            `high` Float(7,2),                            `low` Float(7,2),                            `close` Float(7,2),                            `pre_close` Float(7,2),                            `change` Float(7,2),                            `pct_chg` Float(7,2),                            `vol` Float,                            `amount` Float,                            PRIMARY KEY (ts_code,trade_date),                            INDEX qkey (ts_code,trade_date))'.format(table_name))
            table_name = self.tables['stock_fq_daily']
            session.execute('CREATE TABLE IF NOT EXISTS {} (                            `ts_code` VARCHAR(10),                            `trade_date` VARCHAR(8),                            `adj_factor` Float,                            PRIMARY KEY (ts_code,trade_date),                            INDEX qkey (ts_code,trade_date))'.format(table_name))
            table_name = self.tables['stock_basic_daily']
            session.execute('CREATE TABLE IF NOT EXISTS {} (                            `ts_code` VARCHAR(10),                            `trade_date` VARCHAR(8),                            `close` Float(7,2),                            `turnover_rate` Float,                            `turnover_rate_f` Float,                            `volume_ratio` Float,                            `pe` Float,                            `pe_ttm` Float,                            `pb` Float,                            `ps` Float,                            `ps_ttm` Float,                            `dv_ratio` Float,                            `dv_ttm` Float,                            `total_share` Float,                            `float_share` Float,                            `free_share` Float,                            `total_mv` Float,                            `circ_mv` Float,                            PRIMARY KEY (ts_code,trade_date),                            INDEX qkey (ts_code,trade_date))'.format(table_name))
            table_name = self.tables['index_trade_daily']
            session.execute('CREATE TABLE IF NOT EXISTS {} (                            `ts_code` VARCHAR(10),                            `trade_date` VARCHAR(8),                            `close` Float(7,2),                            `open` Float(7,2),                            `high` Float(7,2),                            `low` Float(7,2),                            `pre_close` Float(7,2),                            `change` Float(7,2),                            `pct_chg` Float(7,2),                            `vol` Float,                            `amount` Float,                            PRIMARY KEY (ts_code,trade_date),                            INDEX qkey (ts_code,trade_date))'.format(table_name))
            table_name = self.tables['index_basic_daily']
            session.execute('CREATE TABLE IF NOT EXISTS {} (                            `ts_code` VARCHAR(10),                            `trade_date` VARCHAR(8),                            `total_mv` Float,                            `float_mv` Float,                            `total_share` Float,                            `float_share` Float,                            `free_share` Float,                            `turnover_rate` Float,                            `turnover_rate_f` Float,                            `pe` Float,                            `pe_ttm` Float,                            `pb` Float,                            PRIMARY KEY (ts_code,trade_date),                            INDEX qkey (ts_code,trade_date))'.format(table_name))
            table_name = self.tables['fina_mainbz_product']
            session.execute('CREATE TABLE IF NOT EXISTS {} (                            `ts_code` VARCHAR(10),                            `end_date` VARCHAR(8),                            `bz_item` VARCHAR(512),                            `bz_sales` Float,                            `bz_profit` Float,                            `bz_cost` Float,                            `curr_type` VARCHAR(4),                            `sync_date` VARCHAR(8),                            PRIMARY KEY (ts_code,end_date,bz_item,bz_sales),                            INDEX qkey (ts_code,end_date))'.format(table_name))
    ...
    def daily_basic(self, ts_code=None, trade_date=None, start_date=None, end_date=None):
        if not ts_code is not None:
            assert trade_date is not None
        if ts_code is not None:
            start_date, end_date = self._DataEngine__check_date_range(start_date, end_date)
    ...
    def sync_data_iterate_date(self):
        if self.api == 'offline':
            ...
            self._DataEngine__stock_basic = self.pro.stock_basic()
            self._DataEngine__stock_basic.to_sql((self.tables['stock_basic_info']), con=(self.conn), if_exists='replace', index=False)
            self.trade_dates = self.get_trade_dates(format_date_ts_pro(self.START_DATE))
            dates = self.trade_dates
            cached_dates = self._DataEngine__get_cached_trade_dates()
            ...
            self._DataEngine__prefetch_data()
    ...
    def sync_data_iterate_stock(self):
        if self.api == 'offline':
            print('you should use tushare_pro to sync data')
            return
            mainbz_product_dates = self._DataEngine__get_cached_cmd_groupby_stock('fina_mainbz_product', 'max(sync_date)')
            mainbz_district_dates = self._DataEngine__get_cached_cmd_groupby_stock('fina_mainbz_district', 'max(sync_date)')
            pbar = progressbar.ProgressBar().start()
            ...
            if total > 1:
                for i in range(total):
                    pbar.update(int(i / (total - 1) * 100))
                    if not mainbz_product_dates.get(stock_codes[i]) is None:
                        if mainbz_product_dates.get(stock_codes[i]) < update_date:
                            self._DataEngine__sync_mainbz_by_stock(stock_codes[i], 'P', mainbz_product_dates.get(stock_codes[i]))
                            time.sleep(1)
                        if mainbz_district_dates.get(stock_codes[i]) is None or mainbz_district_dates.get(stock_codes[i]) < update_date:
                            self._DataEngine__sync_mainbz_by_stock(stock_codes[i], 'D', mainbz_district_dates.get(stock_codes[i]))
                            time.sleep(1)

        else:
            self._DataEngine__sync_mainbz_by_stock(stock_codes[0], 'P', mainbz_product_dates.get(stock_codes[0]))
            self._DataEngine__sync_mainbz_by_stock(stock_codes[0], 'D', mainbz_district_dates.get(stock_codes[0]))
        pbar.finish()
    ...
    def cached_range(self):
        if self.cached_start is not None:
            if self.cached_end is not None:
                return (
    ...
    def get_cached_trade_dates(self):
        return self._DataEngine__get_cached_trade_dates()
    ...
    def get_trade_dates(self, start=START_DATE):
        ...
        return list(filter(lambda x: x >= start, self._DataEngine__get_cached_trade_dates()))
