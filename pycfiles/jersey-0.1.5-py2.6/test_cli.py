...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jersey/cases/test_cli.py
...
class OptionsCase(ProgramTestBase, TestCase):
    expectedUsageLines = ('Options:', '--version', '--help     Display this help and exit.')
...
class PluggableOptionsCase(PluggableTestBase, TestCase):
    expectedUsageLines = ('Options:', '--version', '--help     Display this help and exit.',
                          'Commands:', 'monkey      My name is Bingo!', 'frog        Quoi?')
    ...
    def test_commandsUsage(self):
        ...
        self.assertEquals(len(self.expectedUsageLines), len(usageLines), ('Unexpected usage statement\n{0!r}').format(('\n').join(usageLines)))
