...
MSG_RE = re.compile(b'^MSG\\s+(?P<subject>[^\\s\\r\\n]+)\\s+(?P<sid>[^\\s\\r\\n]+)\\s+(?P<reply>([^\\s\\r\\n]+)[^\\S\\r\\n]+)?(?P<size>\\d+)\\r\\n')
...
class NATSNoSubscribeClient:
    ...
    def request(self, subject: str, *, payload: bytes=b'', timeout: float=DEFAULT_REQUEST_TIMEOUT) -> Optional[NATSMessage]:
...
class NATSClient(NATSNoSubscribeClient):
    ...
    def request(self, subject: str, *, payload: bytes=b'', timeout: float=DEFAULT_REQUEST_TIMEOUT) -> NATSMessage:
        next_inbox = INBOX_PREFIX[:]
        next_inbox.extend(self._nuid.next_())
        reply_subject = next_inbox.decode()
