...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-6brxc_kc/pyppeteer/pyppeteer/network_manager.py
# Compiled at: 2020-03-24 13:42:06
# Size of source mod 2**32: 33162 bytes
...
class NetworkManager(EventEmitter):
    __doc__ = 'NetworkManager class.'
    Events = SimpleNamespace(Request='request',
      Response='response',
      RequestFailed='requestfailed',
      RequestFinished='requestfinished')

    def __init__(self, client, frameManager):
    ...
    async def setOfflineMode(self, value: bool) -> None:
        ...
        if self._offline == value:
            return
        self._offline = value
        await self._client.send('Network.emulateNetworkConditions', {'offline':self._offline, 
         'latency':0, 
    ...
    def _onRequestIntercepted(self, event: dict) -> None:
        if event.get('authChallenge'):
            ...
            if 'redirectUrl' in event:
                ...
                if request:
                    self._handleRequestRedirect(request, event.get('responseStatusCode', 0), event.get('responseHeaders', {}), False, False, None)
                    self._handleRequestStart(request._requestId, event.get('interceptionId', ''), event.get('redirectUrl', ''), event.get('isNavigationRequest', False), event.get('resourceType', ''), event.get('request', {}), event.get('frameId'), request._redirectChain)
                return
                ...
                if requestId:
                    ...
                    self._handleRequestStart(requestId, event['interceptionId'], event['request']['url'], event['isNavigationRequest'], event['resourceType'], event['request'], event['frameId'], [])
            else:
                self._requestHashToInterceptionIds.set(requestHash, event['interceptionId'])
                self._handleRequestStart(None, event['interceptionId'], event['request']['url'], event['isNavigationRequest'], event['resourceType'], event['request'], event['frameId'], [])
    ...
    def _handleRequestStart(self, requestId: Optional[str], interceptionId: str, url: str, isNavigationRequest: bool, resourceType: str, requestPayload: Dict, frameId: Optional[str], redirectChain: List['Request']) -> None:
        frame = None
        if frameId:
            if self._frameManager is not None:
    ...
    def _onRequestWillBeSent(self, event: dict) -> None:
        ...
        isNavigationRequest = event['requestId'] == event['loaderId'] and event['type'] == 'Document'
        self._handleRequestStart(event.get('requestId', ''), '', event.get('request', {}).get('url', ''), isNavigationRequest, event.get('type', ''), event.get('request', {}), event.get('frameId'), redirectChain)
...
class Request(object):
    ...
    async def continue_(self, overrides: Dict=None) -> None:
        ...
        * ``headers`` (dict): If set, change the request HTTP header.
        ...
        if overrides is None:
            overrides = {}
        elif not self._allowInterception:
    ...
    async def respond(self, response: Dict) -> None:
        ...
        * ``body`` (str|bytes): Optional response body.
        ...
        if self._url.startswith('data:'):
            ...
            try:
                await self._client.send('Network.continueInterceptedRequest', {'interceptionId':self._interceptionId, 
    ...
    async def abort(self, errorCode: str='failed') -> None:
        ...
        self._interceptionHandled = True
        try:
            await self._client.send('Network.continueInterceptedRequest', dict(interceptionId=(self._interceptionId),
...
class Response(object):
    __doc__ = 'Response class represents responses which are received by ``Page``.'
    ...
    def __init__(self, client: CDPSession, request: Request, status: int, headers: Dict[(str, str)], fromDiskCache: bool, fromServiceWorker: bool, securityDetails: Dict=None) -> None:
        ...
        if securityDetails:
            self._securityDetails = SecurityDetails(securityDetails['subjectName'], securityDetails['issuer'], securityDetails['validFrom'], securityDetails['validTo'], securityDetails['protocol'])
...
def generateRequestHash(request: dict) -> str:
    ...
    if not normalizedURL.startswith('data:'):
        ...
        for header in headers:
            ...
            header = header.lower()
            if header in ('accept', 'referer', 'x-devtools-emulate-network-conditions-client-id',
                          'cookie'):
                continue
...
class SecurityDetails(object):
    __doc__ = 'Class represents responses which are received by page.'
