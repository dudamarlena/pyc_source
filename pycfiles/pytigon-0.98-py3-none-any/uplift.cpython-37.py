...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-6brxc_kc/pyee/pyee/uplift.py
...
_PROXY_NEW_LISTENER_SETTINGS = dict(forward=(False, True),
  backward=(True, False),
  both=(True, True),
  neither=(False, False))

def uplift(cls, underlying, error_handling='new', proxy_new_listener='forward', *args, **kwargs):
    ...
    """
    new_proxy_new_listener, underlying_proxy_new_listener = _PROXY_NEW_LISTENER_SETTINGS[proxy_new_listener]
    new = cls(*args, **kwargs)
    uplift_error_handlers = dict(new=(
     new, new),
      underlying=(
