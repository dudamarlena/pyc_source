...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-6brxc_kc/tqdm/tqdm/std.py
# Compiled at: 2020-03-24 13:42:06
# Size of source mod 2**32: 55725 bytes
"""
...
from .utils import _supports_unicode, _environ_cols_wrapper, _range, _unich, _term_move_up, _unicode, WeakSet, _basestring, _OrderedDict, Comparable, _is_ascii, FormatReplace, disp_len, disp_trim, SimpleTextIOWrapper, CallbackIOWrapper
...
class Bar(object):
    ...
    ASCII = ' 123456789#'
    UTF = ' ' + ''.join(map(_unich, range(9615, 9607, -1)))
    BLANK = '  '
 ...
 L. 155         0  LOAD_FAST                'format_spec'
                2  POP_JUMP_IF_FALSE   124  'to 124'

 L. 156         4  LOAD_FAST                'format_spec'
                6  LOAD_CONST               -1
                8  BINARY_SUBSCR    
 ...
 L. 157        16  SETUP_EXCEPT         46  'to 46'

 L. 158        18  LOAD_GLOBAL              dict
               20  LOAD_FAST                'self'
               22  LOAD_ATTR                ASCII
               24  LOAD_FAST                'self'
               26  LOAD_ATTR                UTF
               ...
               36  LOAD_FAST                '_type'
               38  BINARY_SUBSCR    
               40  STORE_FAST               'charset'
               42  POP_BLOCK        
               44  JUMP_FORWARD         72  'to 72'
             46_0  COME_FROM_EXCEPT     16  '16'
 ...
 L. 160        60  LOAD_FAST                'self'
               62  LOAD_ATTR                charset
               64  STORE_FAST               'charset'
               66  POP_EXCEPT       
               68  JUMP_FORWARD         84  'to 84'
             70_0  COME_FROM            52  '52'
               70  END_FINALLY      
             72_0  COME_FROM            44  '44'

 L. 162        72  LOAD_FAST                'format_spec'
               74  LOAD_CONST               None
               76  LOAD_CONST               -1
               78  BUILD_SLICE_2         2 
               80  BINARY_SUBSCR    
               82  STORE_FAST               'format_spec'
             84_0  COME_FROM            68  '68'

 L. 163        84  LOAD_FAST                'format_spec'
               86  POP_JUMP_IF_FALSE   116  'to 116'

 L. 164        88  LOAD_GLOBAL              int
               90  LOAD_FAST                'format_spec'
               92  CALL_FUNCTION_1       1  '1 positional argument'
               94  STORE_FAST               'N_BARS'

 L. 165        96  LOAD_FAST                'N_BARS'
               98  LOAD_CONST               0
              100  COMPARE_OP               <
              102  POP_JUMP_IF_FALSE   122  'to 122'

 L. 166       104  LOAD_FAST                'N_BARS'
              106  LOAD_FAST                'self'
              108  LOAD_ATTR                default_len
              110  INPLACE_ADD      
              112  STORE_FAST               'N_BARS'
              114  JUMP_ABSOLUTE       136  'to 136'
            116_0  COME_FROM            86  '86'

 L. 168       116  LOAD_FAST                'self'
              118  LOAD_ATTR                default_len
              120  STORE_FAST               'N_BARS'
            122_0  COME_FROM           102  '102'
              122  JUMP_FORWARD        136  'to 136'
            124_0  COME_FROM             2  '2'

 L. 170       124  LOAD_FAST                'self'
              126  LOAD_ATTR                charset
              128  STORE_FAST               'charset'

 L. 171       130  LOAD_FAST                'self'
              132  LOAD_ATTR                default_len
              134  STORE_FAST               'N_BARS'
            136_0  COME_FROM           122  '122'

 L. 173       136  LOAD_GLOBAL              len
              138  LOAD_FAST                'charset'
              140  CALL_FUNCTION_1       1  '1 positional argument'
              142  LOAD_CONST               1
              144  BINARY_SUBTRACT  
              146  STORE_FAST               'nsyms'

 L. 174       148  LOAD_GLOBAL              divmod

 L. 175       150  LOAD_GLOBAL              int
              152  LOAD_FAST                'self'
              154  LOAD_ATTR                frac
              156  LOAD_FAST                'N_BARS'
              158  BINARY_MULTIPLY  
              160  LOAD_FAST                'nsyms'
              162  BINARY_MULTIPLY  
              ...
              172  STORE_FAST               'bar_length'
              174  STORE_FAST               'frac_bar_length'

 L. 177       176  LOAD_FAST                'charset'
              178  LOAD_CONST               -1
              180  BINARY_SUBSCR    
              182  LOAD_FAST                'bar_length'
              184  BINARY_MULTIPLY  
              186  STORE_FAST               'bar'

 L. 178       188  LOAD_FAST                'charset'
              190  LOAD_FAST                'frac_bar_length'
              192  BINARY_SUBSCR    
              194  STORE_FAST               'frac_bar'

 L. 181       196  LOAD_FAST                'bar_length'
              198  LOAD_FAST                'N_BARS'
              200  COMPARE_OP               <
              202  POP_JUMP_IF_FALSE   232  'to 232'

 L. 182       204  LOAD_FAST                'bar'
              206  LOAD_FAST                'frac_bar'
              208  BINARY_ADD       

 L. 183       210  LOAD_FAST                'charset'
              212  LOAD_CONST               0
              214  BINARY_SUBSCR    
              216  LOAD_FAST                'N_BARS'
              218  LOAD_FAST                'bar_length'
              220  BINARY_SUBTRACT  
              222  LOAD_CONST               1
              224  BINARY_SUBTRACT  
              226  BINARY_MULTIPLY  
...
class tqdm(Comparable):
    ...
    def format_meter(n, total, elapsed, ncols=None, prefix='', ascii=False, unit='it', unit_scale=False, rate=None, bar_format=None, postfix=None, unit_divisor=1000, **extra_kwargs):
        ...
        if total:
            if n >= total + 0.5:
                ...
                if unit_scale:
                    ...
                    format_dict = dict(n=n, 
                     n_fmt=n_fmt, total=total, total_fmt=total_fmt, elapsed=elapsed_str, 
                     elapsed_s=elapsed, ncols=ncols, 
                     desc=prefix or '', unit=unit, rate=inv_rate if (inv_rate and inv_rate > 1) else rate, 
                     rate_fmt=rate_fmt, 
                     rate_noinv=rate, rate_noinv_fmt=rate_noinv_fmt, 
                     rate_inv=inv_rate, rate_inv_fmt=rate_inv_fmt, 
                     postfix=postfix, 
                     unit_divisor=unit_divisor, remaining=remaining_str, 
                     remaining_s=remaining, l_bar=l_bar, 
                     r_bar=r_bar, **extra_kwargs)
                    if total:
                        frac = n / total
                        percentage = frac * 100
            ...
            if not _is_ascii(full_bar.charset):
                if _is_ascii(bar_format):
                    bar_format = _unicode(bar_format)
    ...
    def __new__(cls, *args, **kwargs):
        ...
        with cls.get_lock():
            ...
            if cls.monitor_interval:
                if not (cls.monitor is None or cls.monitor.report()):
                    try:
    ...
    def external_write_mode(cls, file=None, nolock=False):
        ...
        for inst in getattr(cls, '_instances', []):
            if not hasattr(inst, 'start_t') or inst.fp == fp or all((f in (sys.stdout, sys.stderr) for f in (fp, inst.fp))):
                inst.clear(nolock=True)
                ...
                if total is None:
                    if df_function == 'applymap':
                        ...
                        if isinstance(df, Series):
                            ...
                            if not _Rolling_and_Expanding is None:
                                axis = isinstance(df, _Rolling_and_Expanding) or kwargs.get('axis', 0)
                                if axis == 'index':
                                    axis = 0
                                else:
                                    if axis == 'columns':
...
Parse error at or near `None' instruction at offset -1
    ...
    def __init__(self, iterable=None, desc=None, total=None, leave=True, file=None, ncols=None, mininterval=0.1, maxinterval=10.0, miniters=None, ascii=None, disable=False, unit='it', unit_scale=False, dynamic_ncols=False, smoothing=0.3, bar_format=None, initial=0, position=None, postfix=None, unit_divisor=1000, write_bytes=None, lock_args=None, gui=False, **kwargs):
        ...
        if write_bytes is None:
            ...
            if file is None:
                ...
                if not (ncols is None and file in (sys.stderr, sys.stdout)):
                    if dynamic_ncols:
                        if dynamic_ncols:
                ...
                if bar_format and not ascii is True:
                    if not _is_ascii(ascii):
                        bar_format = _unicode(bar_format)
            ...
            with self._lock:
                if position is None:
                    ...
                    self.pos = -position
            self.sp = gui or self.status_printer(self.fp)
    ...
    def refresh(self, nolock=False, lock_args=None):
        ...
        if self.disable:
            ...
            if not nolock:
                if lock_args:
                    return (self._lock.acquire)(*lock_args) or False
                else:
                    self._lock.acquire()
            self.display()
            nolock or self._lock.release()
