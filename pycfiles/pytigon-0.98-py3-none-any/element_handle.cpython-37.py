...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-6brxc_kc/pyppeteer/pyppeteer/element_handle.py
# Compiled at: 2020-03-24 13:42:06
# Size of source mod 2**32: 18465 bytes
"""Element handle module."""
import copy, logging, math, os.path
...
class ElementHandle(JSHandle):
    ...
    async def _clickablePoint(self) -> Dict[(str, float)]:
        result = None
        try:
            result = await self._client.send('DOM.getContentQuads', {'objectId': self._remoteObject.get('objectId')})
        ...
        if not (result and result.get('quads')):
    ...
    async def _getBoxModel(self) -> Optional[Dict]:
        try:
            result = await self._client.send('DOM.getBoxModel', {'objectId': self._remoteObject.get('objectId')})
    ...
    async def uploadFile(self, *filePaths: str) -> dict:
        ...
        return await self._client.send('DOM.setFileInputFiles', {'objectId':objectId, 
         'files':files})
    ...
    async def boxModel(self) -> Optional[Dict]:
        ...
        * ``height`` (int): Element's height.
        ...
        return {'content':self._fromProtocolQuad(model.get('content')), 
    ...
    async def screenshot(self, options: Dict=None, **kwargs: Any) -> bytes:
        ...
        if boundingBox['width'] > original_viewport['width'] or boundingBox['height'] > original_viewport['height']:
            newViewport = {'width':max(original_viewport['width'], math.ceil(boundingBox['width'])), 
