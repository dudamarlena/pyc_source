...
# [GCC 8.4.0]
# Embedded file name: /home/sch/prj/pytigon/pytigon/appdata/plugins/standard/webview/cef/cefcontrol.py
# Compiled at: 2020-01-27 13:49:33
...
class WebRequestClient:
    ...
    def OnRequestComplete(self, web_request):
        ...
        self._data = self._resourceHandler._clientHandler._OnResourceResponse(self._resourceHandler._browser, self._resourceHandler._frame, web_request.GetRequest(), web_request.GetRequestStatus(), web_request.GetRequestError(), web_request.GetResponse(), self._data)
        if self._data:
...
class CEFControl(wx.Control):
    ...
    def __init__(self, parent, url='', size=(-1, -1), *args, **kwargs):
        ...
        if self.component:
            (wx.Control.__init__)(self, parent, *args, id=wx.ID_ANY, size=(1, 1), **kwargs)
            self.hidden = True
        else:
            (wx.Control.__init__)(self, parent, *args, id=wx.ID_ANY, size=size, **kwargs)
            self.hidden = False
        self.url = url
        self.size = None
        self.browser = None
