...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-6brxc_kc/websockets/websockets/headers.py
...
def parse_quoted_string(header: str, pos: int, header_name: str) -> Tuple[(str, int)]:
    ...
    return (
     _unquote_re.sub('\\1', match.group()[1:-1]), match.end())
...
def parse_list(parse_item: Callable[([str, int, str], Tuple[(T, int)])], header: str, pos: int, header_name: str) -> List[T]:
    """
...
def parse_upgrade_protocol(header: str, pos: int, header_name: str) -> Tuple[(UpgradeProtocol, int)]:
    ...
    return (
     cast(UpgradeProtocol, match.group()), match.end())
