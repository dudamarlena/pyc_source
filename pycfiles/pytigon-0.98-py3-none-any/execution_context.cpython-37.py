...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-6brxc_kc/pyppeteer/pyppeteer/execution_context.py
...
class ExecutionContext(object):
    __doc__ = 'Execution Context class.'
            ...
            if not args:
                if not helper.is_jsfunc(pageFunction):
                    try:
                        ...
                        _obj = await self._client.send('Runtime.evaluate', {'expression':expressionWithSourceUrl, 
    ...
    def _convertArgument(self, arg: Any) -> Dict:
        ...
        if objectHandle:
            if objectHandle._context != self:
                ...
                return objectHandle._remoteObject.get('objectId') or {'value': objectHandle._remoteObject.get('value')}
...
class JSHandle(object):
    ...
    async def getProperties(self) -> Dict[(str, 'JSHandle')]:
        ...
        response = await self._client.send('Runtime.getProperties', {'objectId':self._remoteObject.get('objectId', ''), 
         'ownProperties':True})
        result = dict()
    ...
    async def jsonValue(self) -> Dict:
        ...
        if objectId:
            response = await self._client.send('Runtime.callFunctionOn', {'functionDeclaration':'function() { return this; }', 
             'objectId':objectId, 
             'returnByValue':True, 
             'awaitPromise':True})
