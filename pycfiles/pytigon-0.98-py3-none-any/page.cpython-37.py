...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-6brxc_kc/pyppeteer/pyppeteer/page.py
# Compiled at: 2020-03-24 13:42:06
# Size of source mod 2**32: 69166 bytes
"""Page module."""
import asyncio, base64, json, logging, math, mimetypes
...
class Page(EventEmitter):
    ...
    PaperFormats = dict(letter={'width':8.5, 
     'height':11},
      legal={'width':8.5, 
     'height':14},
      tabloid={'width':11, 
    ...
    @staticmethod
    async def create(client: CDPSession, target: 'Target', ignoreHTTPSErrors: bool, setDefaultViewport: bool, screenshotTaskQueue: list=None) -> 'Page':
        ...
        await asyncio.gather(client.send('Target.setAutoAttach', {'autoAttach':True,  'waitForDebuggerOnStart':False}), client.send('Page.setLifecycleEventsEnabled', {'enabled': True}), client.send('Network.enable', {}), client.send('Runtime.enable', {}), client.send('Security.enable', {}), client.send('Performance.enable', {}), client.send('Log.enable', {}))
    ...
    def __init__(self, client, target, frameTree, ignoreHTTPSErrors, screenshotTaskQueue=None):
        super().__init__()
        self._closed = False
        self._client = client
        self._target = target
    ...
    def _onCertificateError(self, event: Any) -> None:
        if not self._ignoreHTTPSErrors:
            return
        self._client._loop.create_task(self._send('Security.handleCertificateError', {'eventId':event.get('eventId'), 
         'action':'continue'}))
    ...
    def _emitMetrics(self, event: Dict) -> None:
        self.emit(Page.Events.Metrics, {'title':event['title'], 
    ...
    def _onBindingCalled(self, event: Dict) -> None:
        ...
        try:
            self._client.send('Runtime.evaluate', {'expression':expression, 
    ...
    async def waitForNavigation(self, options: dict=None, **kwargs: Any) -> Optional[Response]:
        ...
        watcher = NavigatorWatcher(self._frameManager, mainFrame, timeout, options)
        responses = dict()
        listener = helper.addEventListener(self._networkManager, NetworkManager.Events.Response, lambda response: responses.__setitem__(response.url, response))
        result = await watcher.navigationPromise()
    ...
    async def waitForRequest(self, urlOrPredicate: Union[(str, Callable[([Request], bool)])], options: Dict=None, **kwargs: Any) -> Request:
        """Wait for request.
        ...
        * ``timeout`` (int|float): Maximum wait time in milliseconds, defaults
        ...
        return await helper.waitForEvent(self._networkManager, NetworkManager.Events.Request, predicate, timeout, self._client._loop)

    async def waitForResponse(self, urlOrPredicate: Union[(str, Callable[([Response], bool)])], options: Dict=None, **kwargs: Any) -> Response:
        """Wait for response.
        ...
        * ``timeout`` (int|float): Maximum wait time in milliseconds, defaults
        ...
        return await helper.waitForEvent(self._networkManager, NetworkManager.Events.Response, predicate, timeout, self._client._loop)
    ...
    async def _go(self, delta: int, options: dict) -> Optional[Response]:
        ...
        response = (await asyncio.gather(self.waitForNavigation(options), self._client.send('Page.navigateToHistoryEntry', {'entryId': entry.get('id')})))[0]
        return response
    ...
    async def _screenshotTask(self, format: str, options: dict) -> Union[(bytes, str)]:
        ...
        if clip:
            ...
            if options.get('fullPage'):
                ...
                await self._client.send('Emulation.setDeviceMetricsOverride', {'mobile':mobile, 
                 'width':width, 
                 'height':height, 
    ...
    async def pdf(self, options: dict=None, **kwargs: Any) -> bytes:
        ...
        * ``margin`` (dict): Paper margins, defaults to ``None``.
        ...
        marginTop = convertPrintParameterToInches(marginOptions.get('top')) or 0
        marginLeft = convertPrintParameterToInches(marginOptions.get('left')) or 0
        marginBottom = convertPrintParameterToInches(marginOptions.get('bottom')) or 0
        marginRight = convertPrintParameterToInches(marginOptions.get('right')) or 0
        result = await self._client.send('Page.printToPDF', dict(landscape=landscape,
...
supportedMetrics = ('Timestamp', 'Documents', 'Frames', 'JSEventListeners', 'Nodes',
