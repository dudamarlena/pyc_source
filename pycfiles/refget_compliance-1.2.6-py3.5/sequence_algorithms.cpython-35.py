...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/compliance_suite/sequence_algorithms.py
# Compiled at: 2018-10-08 04:57:54
# Size of source mod 2**32: 11357 bytes
import requests
SEQUENCE_ACCEPT_HEADER = {'Accept': 'text/vnd.ga4gh.refget.v1.0.0+plain'}
...
def sequence_invalid_encoding_406_error(test, runner):
    ...
    print('Here')
    base_url = str(runner.base_url)
    response = requests.get(base_url + SEQUENCE_MD5, headers={'Accept': 'embl/some_json'})
...
def sequence_start_end(test, runner):
    ...
    """
    base_url = str(runner.base_url)
    response = requests.get(base_url + SEQUENCE_MD5 + '?start=10&end=20', headers=SEQUENCE_ACCEPT_HEADER)
...
def sequence_start_end_success_cases(test, runner):
    ...
    for case in test.cases:
        _input = case[0]
        _output = case[1]
        response = requests.get(base_url + SEQUENCE_MD5 + _input[0], headers=SEQUENCE_ACCEPT_HEADER)
        case_output_object = {'api': SEQUENCE_MD5 + ':' + _input[0] + ':' + str(SEQUENCE_ACCEPT_HEADER)}
        if response.status_code == 200 and response.text == data[0].sequence[_input[1]:_input[2]] and int(response.headers['content-length']) == _output:
...
def sequence_range(test, runner):
    ...
    """
    base_url = str(runner.base_url)
    header = {'Accept': 'text/vnd.ga4gh.refget.v1.0.0+plain', 
     'Range': 'bytes=10-19'}
    response = requests.get(base_url + SEQUENCE_MD5, headers=header)
...
def sequence_range_success_cases(test, runner):
    ...
    for case in test.cases:
        ...
        response = requests.get(base_url + SEQUENCE_MD5, headers=header)
        case_output_object = {'api': SEQUENCE_MD5 + ':' + _input[0] + ':' + str(SEQUENCE_ACCEPT_HEADER)}
        if response.status_code == _output[0] and response.text == data[0].sequence[_input[1]:_input[2] + 1] and int(response.headers['content-length']) == _output[1]:
            case_output_object['result'] = 1
        else:
            case_output_object['result'] = -1
            test.result = -1
...
def sequence_circular(test, runner):
    ...
    for case in test.cases:
        _input = case[0]
        _output = case[1]
        response = requests.get(base_url + SEQUENCE_CIRCULAR + _input, headers=SEQUENCE_ACCEPT_HEADER)
        case_output_object = {'api': SEQUENCE_CIRCULAR + _input + ':' + str(SEQUENCE_ACCEPT_HEADER)}
        if response.status_code == 200 and response.text == _output[0] and int(response.headers['content-length']) == _output[1]:
            case_output_object['result'] = 1
        else:
            case_output_object['result'] = -1
            test.result = -1
...
def sequence_start_end_errors(test, runner):
    ...
    for case in test.cases:
        _input = case[0]
        _output = case[1]
        response = requests.get(base_url + 'sequence/' + _input[0] + _input[1], headers=SEQUENCE_ACCEPT_HEADER)
...
def sequence_range_errors(test, runner):
    ...
    for case in test.cases:
        ...
        response = requests.get(base_url + 'sequence/' + _input[0], headers=header)
