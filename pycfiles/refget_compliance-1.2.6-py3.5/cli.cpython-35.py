...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/compliance_suite/cli.py
# Compiled at: 2018-10-02 09:10:32
# Size of source mod 2**32: 3745 bytes
import click, os, json, tarfile, sys
...
def scan_for_errors(json):
    ...
    """
    high_level_summary = {}
    for high_level_name in ('test_info_implement', 'test_metadata_implement', 'test_sequence_implement',
                            'test_sequence_range'):
        result = 1
        for test in json[0]['test_results']:
            if high_level_name in test['parents'] and test['warning']:
                result = test['result']
                break

        high_level_summary[high_level_name] = {'result': result, 
...
@click.option('--file_path_name', '-fpn', default='web', help='to create a tar.gz file')
@click.option('--json_path', '--json', help='create a json file report. Setting this to "-" will emit to standard out')
