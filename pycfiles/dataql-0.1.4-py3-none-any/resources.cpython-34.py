...
# [GCC 8.4.0]
# Embedded file name: ./build/lib/dataql/resources.py
# Compiled at: 2015-06-15 17:14:16
# Size of source mod 2**32: 14116 bytes
...
class Resource(WithArgsMixin):
    ...
    def __repr__(self):
        ...
        result = '%(indent)s<%(cls)s[%(entry_name)s]%(name)s%(args)s%(filters)s />' % {'cls': self.__class__.__name__, 
...
class MultiResources(Resource):
    ...
    def __repr__(self):
        ...
        if self.resources:
            return '%(start)s\n%(sub)s\n%(indent)s</%(cls)s[%(entry_name)s]>' % {'start': parent_repr[:-3] + '>', 
...
class PosArg(Arg):
    __doc__ = 'Positioned argument. It has no name and no type.'
...
class NamedArg(Arg):
    __doc__ = 'Named argument. It has a name and a type.'