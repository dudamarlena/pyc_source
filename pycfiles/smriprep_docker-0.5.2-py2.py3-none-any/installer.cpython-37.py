...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-pn36swhz/setuptools/setuptools/installer.py
...
def _legacy_fetch_build_egg(dist, req):
    ...
    opts.clear()
    opts.update(((k, v) for k, v in dist.get_option_dict('easy_install').items() if k in ('find_links',
                                                                                          'site_dirs',
                                                                                          'index_url',
                                                                                          'optimize',
                                                                                          'site_dirs',
...
def fetch_build_egg(dist, req):
    ...
    try:
        ...
        dist.announce('WARNING: The pip package is not available, falling back to EasyInstall for handling setup_requires/test_requires; this is deprecated and will be removed in a future version.', log.WARN)
        ...
        with TemporaryDirectory() as (tmpdir):
            ...
            dist = pkg_resources.Distribution.from_filename(dist_location,
