...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-pn36swhz/pip/pip/_internal/index/package_finder.py
# Compiled at: 2020-02-14 17:24:54
# Size of source mod 2**32: 37542 bytes
...
__all__ = [
 'FormatControl', 'BestCandidateResult', 'PackageFinder']
logger = logging.getLogger(__name__)

def _check_link_requires_python(link, version_info, ignore_requires_python=False):
    """
    ...
    """
    try:
        is_compatible = check_requires_python((link.requires_python),
        ...
        if not is_compatible:
            ...
            if not ignore_requires_python:
                logger.debug('Link requires a different Python (%s not in: %r): %s', version, link.requires_python, link)
                return False
...
class LinkEvaluator(object):
    ...
    def __init__(self, project_name, canonical_name, formats, target_python, allow_yanked, ignore_requires_python=None):
        """
    ...
    def evaluate_link(self, link):
        ...
        version = None
        if link.is_yanked:
            if not self._allow_yanked:
                reason = link.yanked_reason or '<none given>'
                return (
                 False, 'yanked for reason: {}'.format(reason))
        if link.egg_fragment:
            ...
            if ext not in SUPPORTED_EXTENSIONS:
                return (
                 False, 'unsupported archive format: %s' % ext)
            if 'binary' not in self._formats:
                if ext == WHEEL_EXTENSION:
                    reason = 'No binaries permitted for %s' % self.project_name
                    return (False, reason)
            if 'macosx10' in link.path:
                if ext == '.zip':
            ...
            if ext == WHEEL_EXTENSION:
                try:
                    ...
                    if not wheel.supported(supported_tags):
                        ...
                        reason = "none of the wheel's tags match: {}".format(', '.join(file_tags))
                        return (
                         False, reason)
            ...
            if 'source' not in self._formats:
                if ext != WHEEL_EXTENSION:
                    return (
                     False, 'No sources permitted for %s' % self.project_name)
            if not version:
                version = _extract_version_from_fragment(egg_info, self._canonical_name)
            if not version:
                return (False, 'Missing project version for %s' % self.project_name)
            ...
            supports_python = _check_link_requires_python(link,
              version_info=(self._target_python.py_version_info), ignore_requires_python=(self._ignore_requires_python))
...
class BestCandidateResult(object):
    __doc__ = "A collection of candidates, returned by `PackageFinder.find_best_candidate`.\n\n    This class is only intended to be instantiated by CandidateEvaluator's\n    `compute_best_candidate()` method.\n    "
    ...
    def __init__(self, candidates, applicable_candidates, best_candidate):
        ...
        """
        if not set(applicable_candidates) <= set(candidates):
            raise AssertionError
        elif best_candidate is None:
            if applicable_candidates:
                raise AssertionError
        elif not best_candidate in applicable_candidates:
            raise AssertionError
...
class CandidateEvaluator(object):
    ...
    def get_applicable_candidates(self, candidates):
        ...
        versions = {str(v) for v in specifier.filter((str(c.version) for c in candidates),
          prereleases=allow_prereleases)}
        applicable_candidates = [c for c in candidates if str(c.version) in versions]
        filtered_applicable_candidates = filter_unallowed_hashes(candidates=applicable_candidates,
          hashes=(self._hashes),
    ...
    def sort_best_candidate(self, candidates):
        ...
        if link.is_yanked:
            reason = link.yanked_reason or '<none given>'
            msg = 'The candidate selected for download or install is a yanked version: {candidate}\nReason for being yanked: {reason}'.format(candidate=best_candidate,
              reason=reason)
            logger.warning(msg)
    ...
    def compute_best_candidate(self, candidates):
        ...
        return BestCandidateResult(candidates,
          applicable_candidates=applicable_candidates,
          best_candidate=best_candidate)


class PackageFinder(object):
    __doc__ = "This finds packages.\n\n    This is meant to match easy_install's technique for looking for\n    packages, by reading pages and looking for appropriate links.\n    "
    ...
    def create(cls, link_collector, selection_prefs, target_python=None):
        ...
        if target_python is None:
            target_python = TargetPython()
        candidate_prefs = CandidatePreferences(prefer_binary=(selection_prefs.prefer_binary),
          allow_all_prereleases=(selection_prefs.allow_all_prereleases))
        return cls(candidate_prefs=candidate_prefs,
    ...
    def make_link_evaluator(self, project_name):
        ...
        return LinkEvaluator(project_name=project_name,
    ...
    def get_install_candidate(self, link_evaluator, link):
        ...
        if not is_candidate:
            ...
            return
        return InstallationCandidate(name=(link_evaluator.project_name),
          link=link,
          version=(str(result)))
    ...
    def find_all_candidates(self, project_name):
        ...
        find_links_versions = self.evaluate_links(link_evaluator,
          links=(collected_links.find_links))
        page_versions = []
        for project_url in collected_links.project_urls:
            package_links = self.process_project_url(project_url,
              link_evaluator=link_evaluator)
            page_versions.extend(package_links)

        file_versions = self.evaluate_links(link_evaluator,
    ...
    def make_candidate_evaluator(self, project_name, specifier=None, hashes=None):
        ...
        return CandidateEvaluator.create(project_name=project_name,
          target_python=(self._target_python),
    ...
    def find_best_candidate(self, project_name, specifier=None, hashes=None):
        ...
        candidate_evaluator = self.make_candidate_evaluator(project_name=project_name,
          specifier=specifier,
          hashes=hashes)
    ...
    def find_requirement(self, req, upgrade):
        ...
        best_candidate_result = self.find_best_candidate((req.name),
          specifier=(req.specifier), hashes=hashes)
        ...
        def _format_versions(cand_iter):
            return ', '.join(sorted({str(c.version) for c in cand_iter},
              key=parse_version)) or 'none'
        ...
        if installed_version:
            if best_candidate is None or best_candidate.version <= installed_version:
                best_installed = True
        if not upgrade:
