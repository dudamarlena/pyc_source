...
# [GCC 8.4.0]
# Embedded file name: /tmp/src/smriprep/wrapper/build/lib/python3.7/functools.py
# Compiled at: 2019-09-12 11:41:56
# Size of source mod 2**32: 32916 bytes
...
class partialmethod(object):
    ...
    def __init__(*args, **keywords):
        if len(args) >= 2:
            ...
            if not args:
                raise TypeError("descriptor '__init__' of partialmethod needs an argument")
            else:
                if 'func' in keywords:
                    func = keywords.pop('func')
                    self, *args = args
                else:
                    raise TypeError("type 'partialmethod' takes at least one argument, got %d" % (len(args) - 1))
        args = tuple(args)
...
def _make_key(args, kwds, typed, kwd_mark=(
 object(),), fasttypes={
 int, str}, tuple=tuple, type=type, len=len):
