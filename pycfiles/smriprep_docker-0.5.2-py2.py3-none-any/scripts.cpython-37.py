...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-pn36swhz/pip/pip/_vendor/distlib/scripts.py
# Compiled at: 2020-02-14 17:24:54
# Size of source mod 2**32: 16998 bytes
...
def _enquote_executable(executable):
    if ' ' in executable:
        if executable.startswith('/usr/bin/env '):
            ...
            if ' ' in _executable:
                executable = _executable.startswith('"') or '%s "%s"' % (env, _executable)
...
class ScriptMaker(object):
    ...
    def _get_shebang(self, encoding, post_interp=b'', options=None):
        ...
        if sys.platform == 'cli':
            if '-X:Frames' not in post_interp:
                if '-X:FullFrames' not in post_interp:
    ...
    def _get_script_text(self, entry):
        return self.script_template % dict(module=(entry.prefix), import_name=(entry.suffix.split('.')[0]),
          func=(entry.suffix))
    ...
    def _copy_script(self, script, filenames):
        ...
        if not self.force:
            if not self._fileop.newer(script, outname):
        ...
        try:
            ...
            if not first_line:
                ...
                if match:
                    ...
                    encoding, lines = self._fileop.dry_run or detect_encoding(f.readline)
                    f.seek(0)
