...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-pn36swhz/pip/pip/_vendor/distlib/manifest.py
...
"""
import fnmatch, logging, os, re, sys
...
class Manifest(object):
                    ...
                    if action == 'global-exclude':
                        ...
                        if action == 'recursive-include':
                            ...
                            if action == 'recursive-exclude':
                                ...
                                if action == 'graft':
                                    self._include_pattern(None, prefix=dirpattern) or logger.warning('no directories found matching %r', dirpattern)
                                else:
                                    if action == 'prune':
                                        self._exclude_pattern(None, prefix=dirpattern) or logger.warning('no previously-included directories found matching %r', dirpattern)
                                    else:
    ...
    def _parse_directive(self, directive):
        ...
        if len(words) == 1:
            if words[0] not in ('include', 'exclude', 'global-include', 'global-exclude',
                                'recursive-include', 'recursive-exclude', 'graft',
                                'prune'):
            ...
            if action in ('include', 'exclude', 'global-include', 'global-exclude'):
                ...
                if action in ('recursive-include', 'recursive-exclude'):
                    if len(words) < 3:
        ...
        return (
         action, patterns, thedir, dir_pattern)
    ...
    def _translate_pattern(self, pattern, anchor=True, prefix=None, is_regex=False):
        ...
        if is_regex:
            ...
            if pattern:
                ...
                if _PYTHON_VERSION > (3, 2):
                    if pattern_re.startswith(start):
                        raise pattern_re.endswith(end) or AssertionError
                    else:
                        pattern_re = ''
                ...
                if prefix is not None:
                    if _PYTHON_VERSION <= (3, 2):
                        ...
                        if not (prefix_re.startswith(start) and prefix_re.endswith(end)):
                ...
                pattern_re = '%s%s%s%s.*%s%s' % (start, base, prefix_re, sep,
                 pattern_re, end)
        else:
            if anchor:
                if _PYTHON_VERSION <= (3, 2):
