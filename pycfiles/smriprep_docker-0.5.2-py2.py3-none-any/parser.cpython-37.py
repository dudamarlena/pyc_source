...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-pn36swhz/pip/pip/_vendor/pytoml/parser.py
# Compiled at: 2020-02-14 17:24:54
# Size of source mod 2**32: 10309 bytes
import re, sys
...
def loads(s, filename='<string>', translate=lambda t, x, v: v, object_pairs_hook=dict):
    ...
    def process_value(v, object_pairs_hook):
        ...
        if kind == 'array':
            if value:
                if any((k != value[0][0] for k, t, v, p in value[1:])):
...
_escapes = {'b':'\x08', 
 'n':'\n',  'r':'\r',  't':'\t',  '"':'"',  '\\':'\\', 
 'f':'\x0c'}
_basicstr_re = re.compile('[^"\\\\\\000-\\037]*')
_short_uni_re = re.compile('u([0-9a-fA-F]{4})')
_long_uni_re = re.compile('U([0-9a-fA-F]{8})')
_escapes_re = re.compile('[btnfr\\"\\\\]')
...
def _p_basicstr_content(s, content=_basicstr_re):
    ...
    while True:
        ...
        if s.consume_re(_newline_esc_re):
            continue
        if s.consume_re(_short_uni_re) or s.consume_re(_long_uni_re):
...
_key_re = re.compile('[0-9a-zA-Z-_]+')
...
_float_re = re.compile('[+-]?(?:0|[1-9](?:_?\\d)*)(?:\\.\\d(?:_?\\d)*)?(?:[eE][+-]?(?:\\d(?:_?\\d)*))?')
_basicstr_ml_re = re.compile('(?:""?(?!")|[^"\\\\\\000-\\011\\013-\\037])*')
_litstr_re = re.compile("[^'\\000\\010\\012-\\037]*")
_litstr_ml_re = re.compile("(?:(?:|'|'')(?:[^'\\000-\\010\\013-\\037]))*")

def _p_value(s, object_pairs_hook):
    pos = s.pos()
    if s.consume('true'):
        return (
         'bool', s.last(), True, pos)
    if s.consume('false'):
        return (
         'bool', s.last(), False, pos)
    if s.consume('"'):
        if s.consume('""'):
            s.consume('\n')
...
def _p_stmt(s, object_pairs_hook):
    ...
    if s.consume('['):
        ...
        if is_array:
            s.expect(']')
        return (
         'table_array' if is_array else 'table', keys, pos)
    key = _p_key(s)
    _p_ws(s)
    s.expect('=')
    _p_ws(s)
