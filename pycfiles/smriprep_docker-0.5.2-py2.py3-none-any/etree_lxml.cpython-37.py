...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-pn36swhz/pip/pip/_vendor/html5lib/treewalkers/etree_lxml.py
...
class TreeWalker(base.NonRecursiveTreeWalker):
    ...
    def getFirstChild(self, node):
        if isinstance(node, tuple):
            raise AssertionError('Text nodes have no children')
        if not len(node):
            assert node.text, 'Node has no children'
        if node.text:
            return (
             node, 'text')
        return node[0]
    ...
    def getNextSibling(self, node):
        if isinstance(node, tuple):
            node, key = node
            if not key in ('text', 'tail'):
                raise AssertionError('Text nodes are text or tail, found %s' % key)
            elif key == 'text':
                if len(node):
                    return node[0]
                return
