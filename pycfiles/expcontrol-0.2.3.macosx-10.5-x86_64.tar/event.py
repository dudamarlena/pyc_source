...
# [GCC 8.4.0]
# Embedded file name: /Users/jc01/miniconda2/envs/psychopyenv/lib/python2.7/site-packages/expcontrol/event.py
# Compiled at: 2016-05-14 09:01:34
"""Event and event sequence handling for expcontrol."""
import numpy, pandas
EVENTKEYS = [
 'name', 'condition', 'oncall', 'onframe', 'onend']
...
RESPKEYS = [
 'key', 'onresponse_score', 'onresponse_rt']
...
class Event(object):
    ...
    def __call__(self, controller, endtime, currentevlog=None, currentresplog=None):
        ...
        eventlog['oncall'] = self.oncall(controller, currentevlog, currentresplog)
        if controller.eyetracker:
            controller.eyetracker.message(self.name)
        skipahead = False
        resplog = prepresprow()
        while controller.clock() < endtime and not skipahead:
            eventlog['onframe'] = self.onframe(controller, currentevlog, currentresplog)
            response, resptime, frametime = controller()
            if len(response):
                thisresp = prepresprow(ind=resptime)
                thisresp['key'] = response
                thisresp['onresponse_score'], thisresp['onresponse_rt'] = self.onresponse(controller, response, resptime, currentevlog, currentresplog)
...
class EventSeq(Event):
    ...
    def __init__(self, events=[], **kwargs):
        ...
        self.eventskip = [ not len(thisev.skiponresponse) for thisev in self.events
                         ]

    def __call__(self, controller, endtime=0.0, currentevlog=None, currentresplog=None):
        """
...
class EventSeqRelTime(EventSeq):
    ...
    def __init__(self, *args, **kwargs):
        ...
        assert not self.duration, 'cannot set EventSeq duration with relative timings'

    def __call__(self, controller=None, endtime=0.0, currentevlog=None, currentresplog=None):
        outevlog = prepeventrow()
        outresplog = prepresprow()
        for ind, thisevent in enumerate(self.events):
            currenttime = controller.clock()
            thisevlog, thisresp = thisevent(controller, currenttime + self.timing[ind], currentevlog, currentresplog)
        ...
        return (
         super(EventSeqRelTime, self).__call__(controller, endtime, outevlog), outresplog)
...
class EventSeqAbsTime(EventSeq):
    ...
    def __init__(self, *args, **kwargs):
        ...
        assert numpy.isinf(self.duration) or self.duration >= realdur, 'event duration cannot exceed condition duration'
        assert not numpy.any(numpy.isinf(self.eventdur) | self.eventskip), 'EventSeqAbsTime is not supported for these events'
    ...
    def __call__(self, controller, endtime=0.0, currentevlog=None, currentresplog=None):
        ...
        for ind, thisevent in enumerate(self.events):
            thisevlog, thisresp = thisevent(controller, endtimes_trial[ind], currentevlog, currentresplog)
        ...
        return (
         super(EventSeqAbsTime, self).__call__(controller, endtime, evlog), resplog)
...
class FeedbackEvent(DrawEvent):
    ...
    def __init__(self, drawinstances, scorer, correctdraw=[], incorrectdraw=[], omitdraw=[], **kwargs):
    ...
    def oncall(self, controller, currentevlog, currentresplog):
        super(FeedbackEvent, self).oncall(controller, currentevlog, currentresplog)
...
class DetectionEvent(DrawEvent):
    ...
    def __init__(self, drawinstances, correct=None, minrt=0.0, **kwargs):
    ...
    def oncall(self, controller, currentevent, currentresp):
        ...
        """
        super(DetectionEvent, self).oncall(controller, currentevent, currentresp)
...
class SynchEvent(DetectionEvent):
    ...
    def __init__(self, drawinstances, targetkey, duration=numpy.inf, name='synch', **kwargs):
        super(SynchEvent, self).__init__(drawinstances, correct=targetkey, skiponresponse=targetkey, duration=duration, name=name, **kwargs)
    ...
    def onresponse(self, controller, response, resptime, currentevlog, currentresplog):
        ...
        waspulse[numpy.in1d(response, self.correct)] = 1.0
