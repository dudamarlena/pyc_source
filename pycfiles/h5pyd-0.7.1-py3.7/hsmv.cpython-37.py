...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/h5pyd/_apps/hsmv.py
...
def getFolder(domain, mode='r'):
    ...
    dir = h5pyd.Folder(domain, endpoint=endpoint, username=username, password=password,
      bucket=bucket,
      mode=mode)
    return dir
...
def getFile(domain, mode='r'):
    ...
    endpoint = cfg['hs_endpoint']
    bucket = cfg['hs_bucket']
    fh = h5pyd.File(domain, mode='r', endpoint=endpoint, username=username, password=password,
      bucket=bucket,
      use_cache=True)
    return fh
