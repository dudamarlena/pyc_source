...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/h5pyd/_hl/h5type_test.py
# Compiled at: 2019-12-23 13:52:32
# Size of source mod 2**32: 13040 bytes
...
class H5TypeTest(unittest.TestCase):
    ...
    def testBaseIntegerTypeItem(self):
        ...
        typeItem = h5type.getTypeResponse(typeItem)
        self.assertEqual(typeItem['class'], 'H5T_INTEGER')
        self.assertEqual(typeItem['base'], 'H5T_STD_I8LE')
    ...
    def testCreateBaseStringType(self):
        typeItem = {'class':'H5T_STRING', 
         'charSet':'H5T_CSET_ASCII',  'length':6}
        dt = h5type.createDataType(typeItem)
    ...
    def testCreateBaseUnicodeType(self):
        typeItem = {'class':'H5T_STRING', 
         'charSet':'H5T_CSET_UTF8',  'length':32}
        try:
            dt = h5type.createDataType(typeItem)
            self.assertTrue(False)
        except TypeError:
    ...
    def testCreateVLenStringType(self):
        typeItem = {'class':'H5T_STRING', 
         'charSet':'H5T_CSET_ASCII',  'length':'H5T_VARIABLE'}
        dt = h5type.createDataType(typeItem)
    ...
    def testCreateVLenUTF8Type(self):
        typeItem = {'class':'H5T_STRING', 
         'charSet':'H5T_CSET_UTF8',  'length':'H5T_VARIABLE'}
        dt = h5type.createDataType(typeItem)
        self.assertEqual(dt.name, 'object')
        self.assertEqual(dt.kind, 'O')
