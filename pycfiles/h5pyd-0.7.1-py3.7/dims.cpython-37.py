...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/h5pyd/_hl/dims.py
# Compiled at: 2020-01-04 20:47:32
...
class DimensionProxy(base.CommonStateObject):
    __doc__ = "Represents an HDF5 'dimension'."
    ...
    def label(self, val):
        ...
        try:
            ...
            rank = len(dset.shape)
            labels = {'shape':{'class':'H5S_SIMPLE', 
              'dims':[
               rank]}, 
             'type':{'class':'H5T_STRING', 
    ...
    def __getitem__(self, item):
        ...
        if dimlist_attr_json:
        ...
        elif self._dimension >= len(dimlist_attr_values):
            return
            ...
            if isinstance(item, int):
                ...
                if ref_id:
                    msg = ref_id.startswith('datasets/') or 'unexpected ref_id: {}'.format(ref_id)
                    raise IOError(msg)
                else:
    ...
    def attach_scale(self, dscale):
        ...
        if rsp['value'] != 'DIMENSION_SCALE':
            raise RuntimeError('{} is not a dimension scale'.format(dscale.name))
        else:
            try:
                rsp = dset.GET(dset.attrs._req_prefix + 'CLASS')
                if rsp['value'] == 'DIMENSION_SCALE':
                    raise RuntimeError('{} cannot attach a dimension scale to a dimension scale'.format(dset.name))
            except IOError:
                pass
            ...
            dimlist = {'creationProperties':{'nameCharEncoding': 'H5T_CSET_ASCII'}, 
             'shape':{'class':'H5S_SIMPLE', 
              'dims':[
               rank]}, 
             'type':{'base':{'base':'H5T_STD_REF_OBJ', 
               'class':'H5T_REFERENCE'}, 
              'class':'H5T_VLEN'}, 
             'value':value}
            ...
            try:
                ...
                old_reflist = {'creationProperties':{'nameCharEncoding': 'H5T_CSET_ASCII'},  'shape':{'class': 'H5S_SIMPLE'}, 
                 'type':{'class':'H5T_COMPOUND', 
                  'fields':[
                   {'name':'dataset', 
                    'type':{'base':'H5T_STD_REF_OBJ', 
                     'class':'H5T_REFERENCE'}},
                   {'name':'index', 
    ...
    def detach_scale(self, dscale):
        ...
        old_reflist = dscale.GET(req)
        if 'value' in old_reflist:
            if len(old_reflist['value']) > 0:
                new_refs = list()
                remove = [
                 'datasets/' + dset.id.id, self._dimension]
...
class DimensionManager(base.MappingHDF5, base.CommonStateObject):
    ...
    def create_scale(self, dset, name=''):
        ...
        """
        class_attr = {'creationProperties':{'nameCharEncoding': 'H5T_CSET_ASCII'}, 
         'shape':{'class': 'H5S_SCALAR'}, 
         'type':{'charSet':'H5T_CSET_ASCII', 
          'class':'H5T_STRING', 
          'length':16, 
          'strPad':'H5T_STR_NULLTERM'}, 
        ...
        name_attr = {'creationProperties':{'nameCharEncoding': 'H5T_CSET_ASCII'}, 
         'shape':{'class': 'H5S_SCALAR'}, 
         'type':{'charSet':'H5T_CSET_ASCII', 
          'class':'H5T_STRING', 
          'length':len(name) + 1, 
