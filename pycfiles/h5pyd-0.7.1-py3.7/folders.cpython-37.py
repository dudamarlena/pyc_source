...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/h5pyd/_hl/folders.py
# Compiled at: 2019-12-23 13:46:26
# Size of source mod 2**32: 12487 bytes
...
class Folder:
    ...
    def __init__(self, domain_name, pattern=None, query=None, mode=None, endpoint=None, username=None, password=None, bucket=None, api_key=None, logger=None, owner=None, batch_size=1000, **kwds):
        ...
        if domain_name[(-1)] != '/':
            ...
            if mode not in ('r', 'r+', 'w', 'w-', 'x', 'a'):
                ...
                if username is None:
                    ...
                    elif password is None and 'hs_password' in cfg:
            ...
            if 'hs_api_key' in cfg:
                ...
                self._http_conn = HttpConn((self._domain), endpoint=endpoint, username=username, password=password,
                  bucket=bucket,
                  api_key=api_key,
                  mode=mode,
                  logger=logger)
    ...
    def _getSubdomains(self):
        if self._http_conn is None:
            raise IOError(400, 'folder is not open')
        elif self._subdomains is not None:
            if not self._subdomain_marker:
                return 0
            else:
                req = '/domains'
                if self._domain is None:
