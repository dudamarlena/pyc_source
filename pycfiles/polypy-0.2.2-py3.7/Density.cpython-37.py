...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/polypy/Density.py
# Compiled at: 2018-11-28 10:20:56
...
class Density:
    ...
    def __init__(self, data, atom_type=None):
        ...
        if len(np.unique(self.data['label'])) > 1 and self.atom_type is None:
            ...
            if len(np.unique(self.data['label'])) > 1:
    ...
    def one_dimensional_density(self, Bin=None, direction=None, output=None):
        ...
        wr.line_plot(x, y, 'XCoordinate ($\\AA$)', 'Number Density', output)
        return (x, y)
