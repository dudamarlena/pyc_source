...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/polypy/Write.py
# Compiled at: 2018-12-06 12:39:30
...
def contour_plot(X, Y, Z, output):
    ...
    plt.xlabel('X Coordinate ($\\AA$)', fontsize=15)
    plt.ylabel('Y Coordinate ($\\AA$)', fontsize=15)
    plt.tick_params(labelsize=12)
    plt.savefig(output, dpi=600)
    plt.show()
    plt.close()
...
def combined_density_plot(X, Y, Y2, Z, output):
    ...
    ax1.set_xlabel('X Coordinate ($\\AA$)', fontsize=15)
    ax1.set_ylabel('Y Coordinate ($\\AA$)', fontsize=15)
