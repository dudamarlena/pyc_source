...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/nacc/uds3/filters.py
# Compiled at: 2019-12-05 17:01:18
# Size of source mod 2**32: 9809 bytes
import sys, csv, re, fileinput, configparser
...
def fill_value_of_fields(input_ptr, output_ptr, keysDict, blankCheck=False, defaultCheck=False):
    ...
    for record in reader:
        ...
        for col_name in list(keysDict.keys()):
            if col_name in list(record.keys()):
                if blankCheck:
                    if len(record[col_name]) > 0:
                        if record[col_name] != keysDict[col_name]:
                            record[col_name] = keysDict[col_name]
                            count += 1
                if defaultCheck and len(record[col_name]) == 0:
                    record[col_name] = keysDict[col_name]
                    count += 1
...
def filter_csv_all(Ptid, visit_num, visit_type, record):
    if record['ptid'] == Ptid:
        if record['visitnum'].lstrip('0') == visit_num:
            if re.search(visit_type, record['redcap_event_name']):
                return record
