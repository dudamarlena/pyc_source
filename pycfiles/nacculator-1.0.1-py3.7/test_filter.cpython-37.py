...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/tests/test_filter.py
# Compiled at: 2019-12-05 17:01:18
# Size of source mod 2**32: 14681 bytes
...
class TestFilters(unittest.TestCase):
    ...
    def test_filter_clean_ptid_removes_visits_in_nacc_current(self):
        ...
        with io.StringIO(redcap_data) as (data):
            with io.StringIO('') as (results):
                with io.StringIO(subjects) as (nacc_packet_file):
    ...
    def test_filter_remove_ptid(self):
        ...
        """
        filter_diction = {'ptid_format':'11\\d.*', 
    ...
    def test_filter_fill_default(self):
        ...
        with io.StringIO(redcap_data) as (data):
            with io.StringIO('') as (results):
                filters.fill_value_of_fields(data, results, fill_default_values, defaultCheck=True)
                results.seek(0)
                reader = csv.DictReader(results)
                for row in reader:
    ...
    def test_filter_update_field(self):
        ...
        with io.StringIO(redcap_data) as (data):
            with io.StringIO('') as (results):
                filters.fill_value_of_fields(data, results, fill_non_blank_values, blankCheck=True)
                results.seek(0)
                reader = csv.DictReader(results)
                for row in reader:
