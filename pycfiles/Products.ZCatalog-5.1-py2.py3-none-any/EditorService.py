...
# Embedded file name: build/bdist.freebsd-7.1-PRERELEASE-i386/egg/Products/XMLWidgets/EditorService.py
...
class EditorCache:
    ...
    def __init__(self, cache_size=CACHE_SIZE, cache_shrink_factor=CACHE_SHRINK_FACTOR):
...
class EditorSession(Persistent):
    ...
    def __init__(self):
        self.document_context = {'depth': 0, 'wr_name': None, 'node_path': None, 'widget_path': None}
        self.contexts = {}
        self.roots = []
        return
    ...
    def setContext(self, node, wr_name):
        ...
        self.contexts[node.getNodePath('widget')] = {'depth': self.getContext(node)['depth'] + 1, 'wr_name': wr_name, 'node_path': None, 'widget_path': None}
...
class EditorService(SimpleItem):
    ...
    manage_options = ({'label': 'Info', 'action': 'info_tab'}, {'label': 'Edit', 'action': 'manage_editorServiceCacheEditTab'}) + SimpleItem.manage_options
    security.declareProtected('View management screens', 'manage_editorServiceCacheEditTab')
    manage_editorServiceCacheEditTab = PageTemplateFile('www/editorServiceEditTab', globals(), __name__='manage_editorServiceCacheEditTab')
    security.declareProtected('View management screens', 'info_tab')
    manage_main = info_tab = PageTemplateFile('www/editorServiceInfoTab', globals(), __name__='info_tab')
    ...
    def manage_editorServiceCacheEdit(self, id, size):
        ...
        if not id:
            ...
            return self.manage_editorServiceCacheEditTab(manage_tabs_message='Id does not provide for an existing cache container.')
    ...
    security.declareProtected('Use XMLWidgets Editor Service', 'setDocumentEditor')
    ...
    security.declareProtected('Use XMLWidgets Editor Service', 'setEditor')

    def setEditor(self, node, wr_name):
        self._getEditorSession(node).setContext(node, wr_name)

    security.declareProtected('View', 'setViewer')

    def setViewer(self, wr_name):
        self.REQUEST.set('xmlwidgets_viewer', wr_name)

    security.declareProtected('Use XMLWidgets Editor Service', 'clearEditor')

    def clearEditor(self, node):
        self._getEditorSession(node).clearContext(node)

    security.declareProtected('Use XMLWidgets Editor Service', 'setNodeWidget')

    def setNodeWidget(self, node, widget_path):
        self._getEditorSession(node).setNodeWidget(node, widget_path)

    security.declareProtected('Use XMLWidgets Editor Service', 'clearNodeWidget')

    def clearNodeWidget(self, node):
        self._getEditorSession(node).clearNodeWidget(node)

    security.declareProtected('Use XMLWidgets Editor Service', 'pushRoot')

    def pushRoot(self, node, wr_name):
        self._getEditorSession(node).pushRoot(node, wr_name)

    security.declareProtected('Use XMLWidgets Editor Service', 'popRoot')

    def popRoot(self, node):
        self._getEditorSession(node).popRoot(node)

    security.declareProtected('Use XMLWidgets Editor Service', 'getRoot')
    ...
    security.declareProtected('Use XMLWidgets Editor Service', 'getNode')
    ...
    def renderElementsView(self, node):
        viewer = self.getViewer()
        return ('').join([ viewer.getWidget(child).render() for child in node.childNodes if child.nodeType == node.ELEMENT_NODE ])

    security.declareProtected('Use XMLWidgets Editor Service', 'render')
    ...
    def render(self, node):
        ...
        if data is None:
            ...
            if getattr(widget, 'cached', lambda : 0)():
                cache.cache(document_key, node_path, widget_path, data)
        return data

    security.declareProtected('Use XMLWidgets Editor Service', 'renderCache')
    renderCache = render
    security.declareProtected('Use XMLWidgets Editor Service', 'renderElements')

    def renderElements(self, node):
        return ('').join([ self.render(child) for child in node.childNodes if child.nodeType == node.ELEMENT_NODE ])

    security.declareProtected('Use XMLWidgets Editor Service', 'renderElementsCache')
    renderElementsCache = renderElements
    security.declareProtected('Use XMLWidgets Editor Service', 'invalidateWidgetCache')

    def invalidateWidgetCache(self, node, widget_path):
        """Invalidates cache for 'modeless' widgets"""
        document_key = self._getDocumentKey(node)
        cache = self._get_editor_cache()
        cache.invalidate(document_key, node.getNodePath('widget'), widget_path)

    security.declareProtected('Use XMLWidgets Editor Service', 'invalidateWidgetCaches')
    ...
    def invalidateWidgetCaches(self, node, widget_path, modes):
        ...
        cache = self._get_editor_cache()
        for mode in modes:
            cache.invalidate(document_key, node_path, widget_path + (mode,))

    security.declareProtected('Use XMLWidgets Editor Service', 'invalidateCaches')
    ...
    security.declareProtected('Use XMLWidgets Editor Service', 'invalidateChildCaches')
    ...
    security.declareProtected('Use XMLWidgets Editor Service', 'clearCache')

    def clearCache(self, node):
        cache = self._get_editor_cache()
        cache.clearDocument(self._getDocumentKey(node))

    security.declareProtected('Use XMLWidgets Editor Service', 'getWidget')
    ...
    security.declareProtected('Use XMLWidgets Editor Service', 'hasNodeWidget')

    def hasNodeWidget(self, node):
        context = self._getEditorSession(node).getContext(node)
        return context['node_path'] is not None

    security.declareProtected('Use XMLWidgets Editor Service', 'getNodeWidgetPath')
    ...
    def _getWidgetPathInContext(self, context, node):
        if context['node_path'] and node.getNodePath('widget') == context['node_path']:
    ...
    def _resolveWidgetPath(self, widget_path):
        ...
        for step in widget_path:
            ...
            if obj is None:
                raise AttributeError, 'XML Editor Service %s cannot acquire %s ("%s" not found)' % (self.getId(), ('/').join(widget_path), step)
    ...
    def _getEditorSession(self, node):
        try:
            document_key = self._getDocumentKey(node)
            return self.REQUEST.SESSION['xmlwidgets_service_editor'][document_key]
        except KeyError:
            return
    ...
    def _getDocumentKey(self, node):
        ...
        return (
         request['SERVER_URL'], tuple(request._script), path)

    def _get_editor_cache(self):
        global theEditorCache
        if self.cache_container_id is None:
            if theEditorCache is None:
                theEditorCache = EditorCache(cache_size=self.cache_size, cache_shrink_factor=self.cache_shrink_factor)
            return theEditorCache
        else:
            cc = getattr(self.aq_inner, self.cache_container_id)
            storage = getattr(cc, 'widget_editor_cache', None)
            if storage is None:
                cc.widget_editor_cache = PersistentEditorCache(cache_size=self.cache_size, cache_shrink_factor=self.cache_shrink_factor)
                cc._p_changed = 1
...
manage_addEditorServiceForm = PageTemplateFile('www/editorServiceAdd', globals(), __name__='manage_addEditorServiceForm')
