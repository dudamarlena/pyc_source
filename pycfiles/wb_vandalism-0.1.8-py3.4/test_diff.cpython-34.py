...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/wb_vandalism/features/tests/test_diff.py
...
def test_sitelinks():
    ...
    eq_(diff.number_added_sitelinks, pickle.loads(pickle.dumps(diff.number_added_sitelinks)))
    eq_(diff.number_changed_sitelinks, pickle.loads(pickle.dumps(diff.number_changed_sitelinks)))
    eq_(diff.number_removed_sitelinks, pickle.loads(pickle.dumps(diff.number_removed_sitelinks)))
...
def test_labels():
    ...
    eq_(diff.number_added_labels, pickle.loads(pickle.dumps(diff.number_added_labels)))
    eq_(diff.number_changed_labels, pickle.loads(pickle.dumps(diff.number_changed_labels)))
    eq_(diff.number_removed_labels, pickle.loads(pickle.dumps(diff.number_removed_labels)))
...
def test_descriptions():
    ...
    eq_(diff.number_added_descriptions, pickle.loads(pickle.dumps(diff.number_added_descriptions)))
    eq_(diff.number_changed_descriptions, pickle.loads(pickle.dumps(diff.number_changed_descriptions)))
    eq_(diff.number_removed_descriptions, pickle.loads(pickle.dumps(diff.number_removed_descriptions)))
...
def test_aliases():
    ...
    eq_(diff.number_added_aliases, pickle.loads(pickle.dumps(diff.number_added_aliases)))
    eq_(diff.number_removed_aliases, pickle.loads(pickle.dumps(diff.number_removed_aliases)))
...
def test_claims():
    ...
    eq_(diff.number_added_claims, pickle.loads(pickle.dumps(diff.number_added_claims)))
    eq_(diff.number_changed_claims, pickle.loads(pickle.dumps(diff.number_changed_claims)))
    eq_(diff.number_removed_claims, pickle.loads(pickle.dumps(diff.number_removed_claims)))
...
def test_identifiers():
    ...
    eq_(diff.number_changed_identifiers, pickle.loads(pickle.dumps(diff.number_changed_identifiers)))
...
def test_sources():
    ...
    eq_(diff.number_added_sources, pickle.loads(pickle.dumps(diff.number_added_sources)))
    eq_(diff.number_removed_sources, pickle.loads(pickle.dumps(diff.number_removed_sources)))
...
def test_qualifiers():
    ...
    eq_(solve(diff.number_added_qualifiers, cache=cache), 0)
    eq_(solve(diff.number_removed_qualifiers, cache=cache), 0)
    eq_(diff.number_added_qualifiers, pickle.loads(pickle.dumps(diff.number_added_qualifiers)))
    eq_(diff.number_removed_qualifiers, pickle.loads(pickle.dumps(diff.number_removed_qualifiers)))
...
def test_badges():
    ...
    eq_(diff.number_added_badges, pickle.loads(pickle.dumps(diff.number_added_badges)))
    eq_(diff.number_removed_badges, pickle.loads(pickle.dumps(diff.number_removed_badges)))


def test_mean_distance():
    cache = {item_doc: AARON_HALFAKER,  parent_item_doc: ALAN_TOURING}
    eq_(solve(diff.mean_distance_descriptions, cache=cache), 0.47916666666666663)
    eq_(solve(diff.mean_distance_labels, cache=cache), 0.6)
    cache = {item_doc: ALAN_TOURING,  parent_item_doc: ALAN_TOURING_OLD}
    eq_(solve(diff.mean_distance_descriptions, cache=cache), 0.01754385964912286)
    eq_(solve(diff.mean_distance_labels, cache=cache), 0.44727272727272727)
    eq_(diff.mean_distance_descriptions, pickle.loads(pickle.dumps(diff.mean_distance_descriptions)))
    eq_(diff.mean_distance_labels, pickle.loads(pickle.dumps(diff.mean_distance_labels)))
...
def test_languages():
    ...
    eq_(diff.proportion_of_language_added, pickle.loads(pickle.dumps(diff.proportion_of_language_added)))