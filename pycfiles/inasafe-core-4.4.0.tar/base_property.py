...
# [GCC 8.4.0]
# Embedded file name: /Users/ismailsunni/.qgis2/python/plugins/inasafe/safe/metadata/property/base_property.py
...
class BaseProperty(object):
    ...
    def dict(self):
        return {'name': self.name, 
           ...
           'allowed_python_types': [ t.__name__ for t in self.allowed_python_types
                                 ]}
    ...
    def is_allowed_type(self, value):
        ...
        if type(value) in [str, unicode]:
            try:
                ...
                error_message = 'We could not cast the string: "%s" to an allowed type (valid types: %s' % (
                 value, self.allowed_python_types)
                raise TypeError(error_message, e)

        else:
            error_message = 'The value %s (type: %s) is not of the correct type (valid types: %s' % (
             value, type(value), self.allowed_python_types)
    ...
    def _is_valid_xml_path(xml_path):
        if isinstance(xml_path, basestring):
            return True
        raise MetadataInvalidPathError('The xml path %s is invalid (type: %s)' % (
