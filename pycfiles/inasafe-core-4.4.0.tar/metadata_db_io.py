...
# [GCC 8.4.0]
# Embedded file name: /Users/ismailsunni/.qgis2/python/plugins/inasafe/safe/metadata/metadata_db_io.py
# Compiled at: 2018-03-19 11:25:21
"""Metadata DB IO implementation."""
import logging, os, sqlite3 as sqlite
from sqlite3 import OperationalError
from PyQt4.QtCore import QObject
from safe.common.exceptions import HashNotFoundError, UnsupportedProviderError
...
class MetadataDbIO(QObject):
    ...
    def setup_metadata_db_path(self):
        ...
        :rtype: str
        """
        self.metadata_db_path = str(setting('keywordCachePath', expected_type=unicode))
    ...
    def open_connection(self):
        ...
        if not os.path.exists(base_directory):
            try:
                ...
                LOGGER.exception('Could not create directory for metadata cache.')
                raise
    ...
    def get_cursor(self):
        ...
        try:
            ...
            if data is None:
                ...
                sql = 'create table metadata (hash varchar(32) primary key,json text, xml text);'
                LOGGER.debug(sql)
                cursor.execute(sql)
                cursor.fetchone()
            else:
    ...
    def are_metadata_file_based(layer):
        ...
        try:
            ...
            raise UnsupportedProviderError('Could not determine type for provider: %s' % layer.__class__.__name__)

        provider_dict = {'ogr': True, 
           'gdal': True, 
           'gpx': False, 
           'wms': False, 
           'spatialite': False, 
    ...
    def write_metadata_for_uri(self, uri, json=None, xml=None):
        ...
        try:
            try:
                cursor = self.get_cursor()
                sql = "select json, xml from metadata where hash = '%s';" % hash_value
                cursor.execute(sql)
                data = cursor.fetchone()
                if data is None:
                    cursor.execute('insert into metadata(hash, json, xml ) values(:hash, :json, :xml);', {'hash': hash_value, 'json': json, 'xml': xml})
                    self.connection.commit()
                else:
                    cursor.execute('update metadata set json=?, xml=? where hash = ?;', (
                     json, xml, hash_value))
                    self.connection.commit()
            except sqlite.Error:
                LOGGER.exception('Error writing metadata to SQLite db %s' % self.metadata_db_path)
                if self.connection is not None:
                    self.connection.rollback()
                raise
    ...
    def read_metadata_from_uri(self, uri, metadata_format):
        ...
        try:
            try:
                cursor = self.get_cursor()
                sql = "select %s from metadata where hash = '%s';" % (
