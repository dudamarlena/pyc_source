...
def switch(args):
    if args.state.lower() in ('on', '1', 'true'):
        ...
        if args.state.lower() in ('off', '0', 'false'):
            state = 'off'
        else:
            if args.state.lower() == 'toggle':
                state = 'toggle'
            else:
                if args.state.lower() == 'status':
                    state = 'status'
                else:
    ...
    if args.state.lower() in ('on', '1', 'true'):
        ...
        if args.state.lower() in ('off', '0', 'false'):
            ...
            if args.state.lower() == 'toggle':
                ...
                if args.state.lower() == 'sensor':
                    state = 'sensor'
                else:
                    if args.state.lower() == 'switch':
                        state = 'switch'
                    else:
...
def server(args):
    ...
    try:
        ...
        SocketIOServer((host, int(port)), app, policy_server=False,
...
def wemo():
    ...
    parser.add_argument('-v', '--human-readable', dest='human_readable', action='store_true',
      default=False,
