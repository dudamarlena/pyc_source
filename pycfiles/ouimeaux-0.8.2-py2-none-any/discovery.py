...
# [GCC 8.4.0]
# Embedded file name: /home/ian/src/github.com/iancmcc/ouimeaux/ouimeaux/discovery.py
...
class UPnP(object):
    ...
    def __init__(self, mcast_ip='239.255.255.250', mcast_port=1900, bind=None):
        if bind is None:
            ...
            if host.startswith('127.'):
                raise UPnPLoopbackException('Using %s as a callback IP for discovery will not be successful.')
            port = 54321
    ...
    def _response_received(self, message, address):
        ...
        if headers.get('x-user-agent', None) == 'redsonic':
            ...
            if location is not None and location not in self.clients:
                ...
                gevent.spawn(discovered.send, self, address=address, headers=headers)
        return
    ...
    def broadcast(self):
        ...
        request = ('\r\n').join(('M-SEARCH * HTTP/1.1', 'HOST:{mcast_ip}:{mcast_port}',
                                 'ST:upnp:rootdevice', 'MX:2', 'MAN:"ssdp:discover"',
