...
# [GCC 8.4.0]
# Embedded file name: /home/ian/src/github.com/iancmcc/ouimeaux/ouimeaux/environment.py
# Compiled at: 2018-08-26 09:51:15
# Size of source mod 2**32: 7925 bytes
import logging, gevent, requests
...
class Environment(object):
    ...
    def __init__(self, switch_callback=_NOOP, motion_callback=_NOOP, bridge_callback=_NOOP, maker_callback=_NOOP, with_discovery=True, with_subscribers=True, with_cache=_MARKER, bind=None, config_filename=None):
        """
    ...
    def _found_device(self, sender, **kwargs):
        ...
        if usn.startswith('uuid:Socket'):
            ...
            if usn.startswith('uuid:Lightswitch'):
                klass = LightSwitch
            else:
                if usn.startswith('uuid:Insight'):
                    klass = Insight
                else:
                    if usn.startswith('uuid:Sensor'):
                        klass = Motion
                    else:
                        if usn.startswith('uuid:Bridge'):
                            klass = Bridge
                        else:
                            if usn.startswith('uuid:Maker'):
                                klass = Maker
                            else:
