...
# [GCC 8.4.0]
# Embedded file name: /home/ian/src/github.com/iancmcc/ouimeaux/ouimeaux/cli.py
...
def scan(args, on_switch=NOOP, on_motion=NOOP, on_bridge=NOOP, on_maker=NOOP):
    try:
        env = Environment(on_switch, on_motion, on_bridge, on_maker, with_subscribers=False, bind=args.bind)
        env.start()
...
def status(args):
    ...
    def on_switch(switch):
        print (
         'Switch:', switch.name, '\t', _state(switch, args.human_readable))

    def on_motion(motion):
        print (
         'Motion:', motion.name, '\t', _state(motion, args.human_readable))

    def on_maker(maker):
        if maker.switch_mode:
            print (
             'Maker:', maker.name, '\t', 'Momentary State:', _state(maker, args.human_readable))
        else:
            print (
             'Maker:', maker.name, '\t', 'Persistent State:', _state(maker, args.human_readable))
        if maker.has_sensor:
            if args.human_readable:
                if maker.sensor_state:
                    sensorstate = 'Sensor not triggered'
    ...
    def on_bridge(bridge):
        ...
        for light in bridge.Lights:
            print (
             'Light:', light, '\t', bridge.light_get_state(bridge.Lights[light]))

        for group in bridge.Groups:
            print (
             'Group:', group, '\t', bridge.group_get_state(bridge.Groups[group]))
...
def server(args):
    ...
    try:
        ...
        try:
            ...
            print (
             'Invalid bind address configuration:', listen)
            sys.exit(1)
...
def wemo():
    ...
    parser.add_argument('-b', '--bind', default=None, help='ip:port to which to bind the response server. Default is localhost:54321')
    parser.add_argument('-d', '--debug', action='store_true', default=False, help='Enable debug logging')
    parser.add_argument('-e', '--exact-match', action='store_true', default=False, help='Disable fuzzy matching for device names')
    parser.add_argument('-v', '--human-readable', dest='human_readable', action='store_true', default=False, help='Print statuses as human-readable words')
    parser.add_argument('-t', '--timeout', type=int, default=5, help='Time in seconds to allow for discovery')
    subparsers = parser.add_subparsers()
    statusparser = subparsers.add_parser('status', help='Print status of WeMo devices')
    statusparser.set_defaults(func=status)
    stateparser = subparsers.add_parser('switch', help='Turn a WeMo Switch on or off')
    ...
    makerparser = subparsers.add_parser('maker', help='Get sensor or switch state of a Maker or Turn on or off')
    ...
    stateparser = subparsers.add_parser('light', help='Turn a WeMo LED light on or off')
    stateparser.add_argument('name', help='Name or alias of the device or group')
    stateparser.add_argument('state', help="'on' or 'off'")
    stateparser.add_argument('dim', nargs='?', type=int, help='Dim value 0 to 255')
    stateparser.set_defaults(func=light)
    listparser = subparsers.add_parser('list', help='List all devices found in the environment')
    listparser.set_defaults(func=list_)
    serverparser = subparsers.add_parser('server', help='Run the API server and web app')
