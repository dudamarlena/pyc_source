...
# [GCC 8.4.0]
# Embedded file name: /home/ian/src/github.com/iancmcc/ouimeaux/ouimeaux/subscribe.py
    ...
    def _resubscribe(self, url, sid=None):
        ...
        if sid is not None:
            ...
            headers.update({'CALLBACK':'<http://%s:%d>' % (host, self.port), 
             'NT':'upnp:event'})
        response = requests_request(method='SUBSCRIBE', url=url, headers=headers)
        if response.status_code == 412:
            if sid:
                requests_request(method='UNSUBSCRIBE', url=url, headers={'SID': sid})
