...
# [GCC 8.4.0]
# Embedded file name: /mnt/importer/osgeo_importer/api.py
# Compiled at: 2016-12-22 15:59:44
import json, logging, celery
...
class UploadedLayerResource(ModelResource):
    ...
    def prepend_urls(self):
        return [
         url(('^(?P<resource_name>{0})/(?P<pk>\\w[\\w/-]*)/configure{1}$').format(self._meta.resource_name, trailing_slash()), self.wrap_view('import_layer'), name='importer_configure')]
...
class UploadedDataResource(ModelResource):
    ...
    def prepend_urls(self):
        ...
        pu.extend([
         url(('^(?P<resource_name>{0})/(?P<pk>\\w[\\w/-]*)/import_all_layers{1}$').format(self._meta.resource_name, trailing_slash()), self.wrap_view('import_all_layers'), name='import_all_data')])
        return pu
