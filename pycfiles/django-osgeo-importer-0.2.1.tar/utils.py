...
# [GCC 8.4.0]
# Embedded file name: /mnt/importer/osgeo_importer/utils.py
# Compiled at: 2016-12-22 15:59:44
from cStringIO import StringIO
import collections
from datetime import datetime
import logging, os, re, shutil, sys, celery
...
GDAL_GEOMETRY_TYPES = {0: 'Unknown', 
   ...
   -2147483645: 'Polygon', 
   -2147483644: 'MultiPoint', 
   -2147483643: 'MultiLineString', 
   -2147483642: 'MultiPolygon', 
   -2147483641: 'GeometryCollection'}
...
def raster_import(infile, outfile, *args, **kwargs):
    ...
    vrt = gdal.AutoCreateWarpedVRT(indata, None, t_srs_prj, 0, 0.125)
    ...
    def upload(self, data, owner):
        ...
        if len(paths) > 1:
            ...
            for group_name, group in groups.items():
                leaders = [ path for path in group if any(path.endswith(ext) for ext in leader_exts)
                          ]
                if leaders:
        ...
        for each in finalfiles:
            ...
            if upfile_ext.lower() not in ('.prj', '.dbf', '.shx'):
                ...
                for layer in description:
                    ...
                    layer_basename = os.path.basename(layer.get('layer_name') or '')
                    upload_layer = UploadLayer(upload_file=upfile, name=upfile_basename, layer_name=layer_basename, fields=layer.get('fields', {}), index=layer.get('index'), feature_count=layer.get('feature_count', None), configuration_options=configuration_options)
                    upload.uploadlayer_set.add(upload_layer)

        upload.size = sum(upfile.file.size for upfile in upfiles)
        ...
        for uploaded_layer in uploaded_file.uploadlayer_set.all():
            ...
            for config in configs:
                if config['upload_file_name'] == uploaded_layer.name:
                    configuration_options = {'layer_owner': owner.username, 'index': 0}
                    import_result = import_object.delay(uploaded_layer.upload_file.id, configuration_options=configuration_options)
