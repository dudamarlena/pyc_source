...
# [GCC 8.4.0]
# Embedded file name: /vagrant/osgeo_importer/tests.py
# Compiled at: 2016-11-01 16:45:08
import os, json, unittest, logging, osgeo, gdal
...
from osgeo_importer.models import UploadedData, UploadFile, UploadLayer, validate_file_extension, ValidationError, validate_inspector_can_read
...
User = None
_TEST_FILES_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'importer-test-files'))
...
def get_layer_attr(layer, attr_value):
    ...
    """
    date_attrs = [ attr for attr in layer.attributes if attr.attribute == attr_value
                 ]
    if not date_attrs:
        return None
    else:
...
class UploaderTests(TestCase):
    ...
    def setUp(self):
        self.assertTrue(os.path.exists(_TEST_FILES_DIR), ('Test could not run due to missing test data at {0!r}').format(_TEST_FILES_DIR))
        ...
        self.catalog = Catalog(ogc_server_settings.internal_rest, *ogc_server_settings.credentials)
        ...
        for result in results:
            if result[1].get('raster'):
                ...
                if not path.endswith('zip'):
                    self.assertGreaterEqual(layer.attributes.count(), DataSource(path)[0].num_fields)
                layer_results.append(layer)
    ...
    def generic_api_upload(self, filenames, configs=None):
        ...
        response = client.post(reverse('uploads-new-json'), {'file': outfiles, 'json': json.dumps(configs)}, follow=True)
    ...
    def test_multi_upload(self):
        ...
        upload = self.generic_api_upload(filenames=[
         ...
         'boxes_with_date.zip',
         'point_with_date.geojson'], configs=[
    ...
    def test_geotiff_raster(self):
        ...
        """
        layer = self.generic_raster_import('test_grid.tif', configs=[
         {'index': 0}])
    ...
    def test_box_with_year_field(self):
        ...
        """
        layer = self.generic_import('boxes_with_year_field.shp', configs=[
         {'index': 0, 
            'convert_to_date': [
                              'date']}])
        date_attr = get_layer_attr(layer, 'date_as_date')
        self.assertEqual(date_attr.attribute_type, 'xsd:dateTime')
        configure_time(self.catalog.get_layer(layer.name).resource, attribute=date_attr.attribute)
    ...
    def test_boxes_with_date(self):
        ...
        """
        layer = self.generic_import('boxes_with_date.shp', configs=[
         {'index': 0, 
            'convert_to_date': [
                              'date'], 
            'start_date': 'date', 
    ...
    def test_boxes_with_date_gpkg(self):
        ...
        """
        layer = self.generic_import('boxes_with_date.gpkg', configs=[
         {'index': 0, 
            'convert_to_date': [
                              'date'], 
            'start_date': 'date', 
    ...
    def test_boxes_plus_raster_gpkg_by_index(self):
        ...
        """
        layer = self.generic_import('boxes_plus_raster.gpkg', configs=[
         {'index': 0, 
            'convert_to_date': [
                              'date'], 
            'start_date': 'date', 
            'configureTime': True}, {'index': 1}, {'index': 2}, {'index': 3}, {'index': 4}, {'index': 5}, {'index': 6}, {'index': 7}])
        date_attr = get_layer_attr(layer, 'date_as_date')
        self.assertEqual(date_attr.attribute_type, 'xsd:dateTime')
        configure_time(self.catalog.get_layer(layer.name).resource, attribute=date_attr.attribute)
        self.generic_time_check(layer, attribute=date_attr.attribute)
    ...
    def test_boxes_with_date_csv(self):
        ...
        """
        layer = self.generic_import('boxes_with_date.csv', configs=[
         {'index': 0, 
            'convert_to_date': [
                              'date']}])
        date_attr = get_layer_attr(layer, 'date_as_date')
    ...
    def test_boxes_with_iso_date(self):
        ...
        """
        layer = self.generic_import('boxes_with_date_iso_date.shp', configs=[
         {'index': 0, 
            'convert_to_date': [
                              'date']}])
        date_attr = get_layer_attr(layer, 'date_as_date')
    ...
    def test_boxes_with_date_iso_date_zip(self):
        """Tests the import of test_boxes_with_iso_date.
        """
        layer = self.generic_import('boxes_with_date_iso_date.zip', configs=[
         {'index': 0, 
            'convert_to_date': [
                              'date']}])
        date_attr = get_layer_attr(layer, 'date_as_date')
    ...
    def test_boxes_with_dates_bc(self):
        ...
        """
        layer = self.generic_import('boxes_with_dates_bc.shp', configs=[
         {'index': 0, 
            'convert_to_date': [
                              'date']}])
        date_attr = get_layer_attr(layer, 'date_as_date')
    ...
    def test_point_with_date(self):
        ...
        """
        layer = self.generic_import('point_with_date.geojson', configs=[
         {'index': 0, 
            'convert_to_date': [
                              'date']}])
    ...
    def test_boxes_with_end_date(self):
        ...
        """
        layer = self.generic_import('boxes_with_end_date.shp', configs=[
         {'index': 0, 
            'convert_to_date': [
                              'date', 'enddate'], 
            'start_date': 'date', 
        ...
        configure_time(self.catalog.get_layer(layer.name).resource, attribute=date_attr.attribute, end_attribute=end_date_attr.attribute)
    ...
    def test_us_states_kml(self):
        ...
        This layer has a date and an end date field that are typed correctly.
        """
        layer = self.generic_import('us_states.kml', configs=[
         {'index': 0}])
    ...
    def test_mojstrovka_gpx(self):
        ...
        This layer has a date and an end date field that are typed correctly.
        """
        layer = self.generic_import('mojstrovka.gpx', configs=[
         {'index': 0, 
            'convert_to_date': [
                              'time'], 
            'configureTime': True}])
    ...
    def generic_time_check(self, layer, attribute=None, end_attribute=None):
        ...
        """
        self.catalog._cache.clear()
        resource = self.catalog.get_resource(layer.name, store=layer.store, workspace=self.workspace)
    ...
    def test_us_shootings_csv(self):
        ...
        layer = self.generic_import(path, configs=[
         {'index': 0, 
            'convert_to_date': [
                              'Date']}])
        self.assertTrue(layer.name.startswith('us_shootings'))
        date_field = 'date'
        configure_time(self.catalog.get_layer(layer.name).resource, attribute=date_field)
    ...
    def test_sitins(self):
        ...
        if osgeo.gdal.__version__ < '2.0.0':
            self.skipTest('GDAL Version does not support open options')
        layer = self.generic_import('US_Civil_Rights_Sitins0.csv', configs=[
         {'index': 0, 
    ...
    def test_gdal_import(self):
        ...
        self.generic_import(path, configs=[
         {'index': 0, 
            'convert_to_date': [
                              'date']}])
    ...
    def test_wfs(self):
        ...
        ogr = OGRImport(wfs)
        configs = [{'layer_name': 'og:bugsites'}, {'layer_name': 'topp:states'}]
        layers = ogr.handle(configuration_options=configs)
    ...
    def test_file_add_view(self):
        ...
        with open(test_file('point_with_date.geojson')) as (stream):
            response = client.post(reverse('uploads-new'), {'file': stream}, follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_file_add_view_as_json(self):
        ...
        client.login_as_non_admin()
        with open(test_file('point_with_date.geojson')) as (stream):
            response = client.post(reverse('uploads-new-json'), {'file': stream}, follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_gdal_file_type(self):
        """Tests the describe fields functionality.
        """
        filenames = {'US_Shootings.csv': 'CSV', 
    ...
    def test_configure_view(self):
         ...
         {'index': 0, 
            ...
            'permissions': {'users': {'test': new_user_perms, 
                                      'AnonymousUser': [
                                                      ...
                                                      'view_resourcebase']}}}]
        response = client.post(('/importer-api/data-layers/{0}/configure/').format(upload.id), data=json.dumps(payload), content_type='application/json')
        self.assertTrue(response.status_code, 200)
        ...
        self.assertEqual(Layer.objects.all()[0].owner.username, self.non_admin_user.username)
    ...
    def test_configure_view_convert_date(self):
        ...
        payload = [
         {'index': 0, 
            ...
            'configureTime': True, 
            'editable': True}]
        response = client.get(('/importer-api/data-layers/{0}/configure/').format(upload.id))
        self.assertEqual(response.status_code, 405)
        response = client.post(('/importer-api/data-layers/{0}/configure/').format(upload.id))
    ...
    def naming_an_import(self):
        ...
        payload = {'index': 0, 
           ...
           'name': name, 
           'editable': True}
        response = client.post('/importer-api/data-layers/1/configure/', data=json.dumps(payload), content_type='application/json')
        self.assertEqual(response.status_code, 200)
        first_layer = Layer.objects.all()[0]
    ...
    def test_api_import(self):
        ...
        payload = {'index': 0, 
           ...
           'configureTime': True, 
           'editable': True}
        self.assertIsInstance(UploadLayer.objects.first().configuration_options, dict)
    ...
    def test_numeric_overflow(self):
        ...
        """
        self.generic_import('Walmart.zip', configs=[
         {'configureTime': False, 
            'convert_to_date': [
                              'W1_OPENDAT'], 
            'editable': True, 
    ...
    def test_utf8(self):
        ...
        sql = ("select NAME_CH from {0} where NAME_PY = 'An Zhou'").format(layer.name)
    ...
    def test_non_converted_date(self):
        ...
        """
        results = self.generic_import('TM_WORLD_BORDERS_2005.zip', configs=[
         {'index': 0, 
            'start_date': 'Year', 
            'configureTime': True}])
    ...
    def test_fid_field(self):
        ...
        """
        self.generic_import('noaa_paleoclimate.zip', configs=[
         {'index': 0}])
    ...
    def test_csv_with_wkb_geometry(self):
        ...
        for filename in filenames:
            self.generic_import(filename, {'configureTime': True, 
               'convert_to_date': [
                                 'date_time'], 
               'editable': True, 
               'index': 0, 
