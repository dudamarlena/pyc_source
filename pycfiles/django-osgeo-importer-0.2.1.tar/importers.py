...
# [GCC 8.4.0]
# Embedded file name: /mnt/importer/osgeo_importer/importers.py
...
class Import(object):
    ...
    def _initialize_handlers(self):
        self._import_handlers = [ load_handler(handler, self) for handler in self.enabled_handlers
                                ]
    ...
    def import_file(self, *args, **kwargs):
        ...
        for layer_configuration in configuration_options:
            if 'layer_name' in layer_configuration:
                ...
                if 'index' in layer_configuration:
                    lookup = 'index'
                else:
                    lookup = None
                ...
                for i in range(0, layer.GetFeatureCount()):
                    ...
                    if feature and feature.geometry():
                        if not layer.GetFIDColumn():
                            feature.SetFID(-1)
                        if feature.geometry().GetGeometryType() != target_layer.GetGeomType() and target_layer.GetGeomType() in range(4, 7):
