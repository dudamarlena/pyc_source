...
# [GCC 8.4.0]
# Embedded file name: /Volumes/Bucket/projects/tinymce/gu-django-tinymce/tinymce/compressor.py
# Compiled at: 2016-04-21 01:35:08
"""
...
def gzip_compressor(request):
    ...
    if not isJS:
        response.write(render_to_string('tinymce/tiny_mce_gzip.js', {'base_url': tinymce.settings.JS_BASE_URL}, context_instance=RequestContext(request)))
        return response
    else:
        ...
        if cacheData is not None:
            ...
            if 'Last-Modified' in cacheData:
                ...
                if if_modified_since == cacheData['Last-Modified']:
                    ...
                    response['Content-Length'] = '0'
                    return response
        tinyMCEPreInit = {'base': tinymce.settings.JS_BASE_URL, 
           'suffix': ''}
        ...
        content.append(('tinymce.each("{!s}".split(","), function(f){{tinymce.ScriptLoader.markDone(tinyMCE.baseURL+"/"+f+".js");}});').format((',').join(files)))
        unicode_content = []
