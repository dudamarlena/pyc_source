...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/rcquerybuilder/pipeline.py
# Compiled at: 2016-03-07 05:07:42
"""Pipeline module.
...
class PipelineExpr(Expr):
    ...
    def operator(self, operator, value_or_expression):
        if isinstance(value_or_expression, (list, tuple)):
            expression = [ _get_query(voe) for voe in value_or_expression ]
        else:
    ...
    def filter(self, input, name, cond):
        return self.operator('$filter', {'input': input, 
           'as': name, 
    ...
    def date_to_string(self, date, date_format='%Y-%m-%d %H:%M:%S'):
        return self.operator('$dateToString', {'format': date_format, 
           'date': date})

    def cond(self, if_cond, then_cond, else_cond):
        return self.operator('$cond', {'if': _get_query(if_cond), 
...
class GroupStage(PipelineStage):
    ...
    def id(self, *args, **kwargs):
        return self.field('_id').equals(_get_query(args[0] if args else kwargs))
