...
# [GCC 8.4.0]
# Embedded file name: fido/prepare.py
# Compiled at: 2019-11-25 15:48:20
...
def do_any_all_bitmasks(chars, i, predicate, littleendian):
    ...
    bitmask = ord(byt)
    regex = ('({})').format(('|').join([ '\\x' + hex(byte)[2:].zfill(2) for byte in range(256) if predicate(byte, bitmask)
                                       ]))
    return (regex, inc + 1)
