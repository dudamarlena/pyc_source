...
# [GCC 8.4.0]
# Embedded file name: /Users/kensuke-mi/Desktop/analysis_work/wiki_node_disambiguation/tests/test_interface.py
...
import configparser, unittest, os
...
class TestInterface(unittest.TestCase):
    ...
    def test_compute_wiki_node_probability_test1(self):
        ...
        sequence_score_objects = interface.compute_wiki_node_probability(seq_wiki_article_name=seq_wikipedia_article_object, entity_vector_model=self.model_object, is_use_cache=True)
    ...
    def test_predict_japanese_wiki_names_partial(self):
        connector = initialize_mysql_connector.initialize_pymysql_connector(hostname=self.config_obj.get('Mysql', 'host'), user_name=self.config_obj.get('Mysql', 'user_name'), password=self.config_obj.get('Mysql', 'password'), dbname=self.config_obj.get('Mysql', 'database_name'))
        test_input = [
         'お笑い', 'タレント', 'ロバート', '山本博', 'エンタ', 'の', '神様', '日テレ', '踊る!さんま御殿!!']
        sequence_score_objects = interface.predict_japanese_wiki_names_with_wikidump(input_tokens=test_input, wikipedia_db_connector=connector, entity_vector_model=self.model_object, is_use_cache=True, is_sort_object=True)
        self.assertTrue(isinstance(sequence_score_objects, list))
        for seq_obj in sequence_score_objects:
            self.assertTrue(isinstance(seq_obj, SequenceScore))
            import pprint
    ...
    def test_predict_japanese_wiki_names_complete(self):
        connector = initialize_mysql_connector.initialize_pymysql_connector(hostname=self.config_obj.get('Mysql', 'host'), user_name=self.config_obj.get('Mysql', 'user_name'), password=self.config_obj.get('Mysql', 'password'), dbname=self.config_obj.get('Mysql', 'database_name'))
        test_input = [
         'ヤマハ', 'バイク', 'スズキ', 'オーバーテイク', 'ホンダ', '優勝']
        sequence_score_objects = interface.predict_japanese_wiki_names_with_wikidump(input_tokens=test_input, wikipedia_db_connector=connector, entity_vector_model=self.model_object, is_use_cache=True, is_sort_object=True, search_method='complete')
        self.assertTrue(isinstance(sequence_score_objects, list))
        for seq_obj in sequence_score_objects:
            self.assertTrue(isinstance(seq_obj, SequenceScore))
            import pprint
