...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/trytond/modules/lims/notebook.py
# Compiled at: 2019-01-16 09:41:15
# Size of source mod 2**32: 196269 bytes
import operator
from datetime import datetime
import dateutil.relativedelta as relativedelta
from sql import Literal, Join
from trytond.model import ModelView, ModelSQL, fields
from trytond.wizard import Wizard, StateTransition, StateView, StateAction, StateReport, Button
...
__all__ = [
 'Notebook', 'NotebookLine', 'NotebookLineAllFields',
...
class Notebook(ModelSQL, ModelView):
    __doc__ = 'Laboratory Notebook'
    __name__ = 'lims.notebook'
    fraction = fields.Many2One('lims.fraction', 'Fraction', required=True, readonly=True,
      ondelete='CASCADE',
      select=True)
    lines = fields.One2Many('lims.notebook.line', 'notebook', 'Lines')
    product_type = fields.Function((fields.Many2One('lims.product.type', 'Product type')),
      'get_sample_field', searcher='search_sample_field')
    matrix = fields.Function((fields.Many2One('lims.matrix', 'Matrix')), 'get_sample_field',
      searcher='search_sample_field')
    party = fields.Function((fields.Many2One('party.party', 'Party')), 'get_sample_field',
      searcher='search_sample_field')
    party_code = fields.Function((fields.Char('Party')), 'get_party_code', searcher='search_party_code')
    label = fields.Function((fields.Char('Label')), 'get_sample_field', searcher='search_sample_field')
    date = fields.Function((fields.DateTime('Date')), 'get_sample_field', searcher='search_sample_field')
    date2 = fields.Function((fields.Date('Date')), 'get_sample_field', searcher='search_sample_field')
    ...
    fraction_comments = fields.Function(fields.Text('Fraction Comments'), 'get_fraction_field')
    shared = fields.Function((fields.Boolean('Shared')), 'get_fraction_field', searcher='search_fraction_field')
    ...
    divided_report = fields.Function(fields.Boolean('Divided report'), 'get_divided_report')
 ...
 L. 137         0  BUILD_MAP_0           0 
                2  STORE_FAST               'result'

 L. 138         4  SETUP_LOOP          174  'to 174'
                6  LOAD_FAST                'names'
                8  GET_ITER         
               10  FOR_ITER            172  'to 172'
               12  STORE_FAST               'name'

 L. 139        14  BUILD_MAP_0           0 
               16  LOAD_FAST                'result'
               18  LOAD_FAST                'name'
               20  STORE_SUBSCR     

 L. 140        22  LOAD_FAST                'name'
               24  LOAD_STR                 'fraction_type'
               26  COMPARE_OP               ==
               28  POP_JUMP_IF_FALSE    84  'to 84'

 L. 141        30  SETUP_LOOP          170  'to 170'
               32  LOAD_FAST                'notebooks'
               34  GET_ITER         
               36  FOR_ITER             80  'to 80'
               38  STORE_FAST               'n'

 L. 142        40  LOAD_GLOBAL              getattr
               42  LOAD_FAST                'n'
 ...
 L. 144        84  LOAD_FAST                'name'
               86  LOAD_STR                 'fraction_comments'
               88  COMPARE_OP               ==
               90  POP_JUMP_IF_FALSE   132  'to 132'

 L. 145        92  SETUP_LOOP          170  'to 170'
               94  LOAD_FAST                'notebooks'
               96  GET_ITER         
               98  FOR_ITER            128  'to 128'
              100  STORE_FAST               'n'

 L. 146       102  LOAD_GLOBAL              getattr
              104  LOAD_FAST                'n'
              106  LOAD_ATTR                fraction
              108  LOAD_STR                 'comments'
              110  LOAD_CONST               None
 ...
 L. 148       132  SETUP_LOOP          170  'to 170'
              134  LOAD_FAST                'notebooks'
              136  GET_ITER         
              138  FOR_ITER            168  'to 168'
              140  STORE_FAST               'n'

 L. 149       142  LOAD_GLOBAL              getattr
              144  LOAD_FAST                'n'
              ...
              162  LOAD_ATTR                id
              164  STORE_SUBSCR     
              166  JUMP_BACK           138  'to 138'
              168  POP_BLOCK        
            170_0  COME_FROM_LOOP      132  '132'
            170_1  COME_FROM_LOOP       92  '92'
            170_2  COME_FROM_LOOP       30  '30'
              170  JUMP_BACK            10  'to 10'
              172  POP_BLOCK        
            174_0  COME_FROM_LOOP        4  '4'

 L. 150       174  LOAD_FAST                'result'
              176  RETURN_VALUE     
               -1  RETURN_LAST      

Parse error at or near `COME_FROM_LOOP' instruction at offset 170_1
    ...
    def get_divided_report(self, name):
        return self.fraction and self.fraction.services or False
    ...
    def search_current_location(cls, name, domain=None):
        ...
        processed_lines = [{'fraction':fraction,  'current_location':location} for fraction, location in current_locations]
        record_ids = [line['notebook'] for line in processed_lines if _search_current_location_eval_domain(line, domain)]
...
class NotebookLine(ModelSQL, ModelView):
    __doc__ = 'Laboratory Notebook Line'
    __name__ = 'lims.notebook.line'
    notebook = fields.Many2One('lims.notebook', 'Laboratory notebook', ondelete='CASCADE',
      select=True,
      required=True)
    analysis_detail = fields.Many2One('lims.entry.detail.analysis', 'Analysis detail',
      select=True)
    service = fields.Many2One('lims.service', 'Service', readonly=True, ondelete='CASCADE',
      select=True)
    analysis = fields.Many2One('lims.analysis', 'Analysis', required=True, readonly=True)
    repetition = fields.Integer('Repetition', readonly=True)
    start_date = fields.Date('Start date', readonly=True)
    end_date = fields.Date('End date', states={'readonly': Or(~Bool(Eval('start_date')), Bool(Eval('accepted')))},
      depends=[
     'start_date', 'accepted'])
    ...
    laboratory_professionals = fields.Many2Many('lims.notebook.line-laboratory.professional', 'notebook_line', 'professional', 'Preparation professionals')
    initial_unit = fields.Many2One('product.uom', 'Initial unit', domain=[
     ('category.lims_only_available', '=', True)],
      states={'readonly': Bool(Eval('accepted'))},
      depends=['accepted'])
    final_unit = fields.Many2One('product.uom', 'Final unit', domain=[
     ('category.lims_only_available', '=', True)],
      states={'readonly': Bool(Eval('accepted'))},
      depends=['accepted'])
    result_modifier = fields.Selection([
     ...
     ('pre', 'Presence')],
      'Result modifier',
      sort=False, states={'readonly': Bool(Eval('accepted'))},
      depends=['accepted'])
    ...
    converted_result_modifier = fields.Selection([
     ...
     ('neg', 'Negative'),
     ('ni', 'ni')],
      'Converted result modifier',
      sort=False, states={'readonly': Bool(Eval('accepted'))},
      depends=['accepted'])
    converted_result_modifier_string = converted_result_modifier.translated('converted_result_modifier')
    result = fields.Char('Result', states={'readonly': Bool(Eval('accepted'))},
      depends=['accepted'])
    converted_result = fields.Char('Converted result', states={'readonly': Bool(Eval('accepted'))},
      depends=['accepted'])
    detection_limit = fields.Char('Detection limit', states={'readonly': Bool(Eval('accepted'))},
      depends=['accepted'])
    quantification_limit = fields.Char('Quantification limit', states={'readonly': Bool(Eval('accepted'))},
      depends=['accepted'])
    check_result_limits = fields.Function(fields.Boolean('Validate limits directly on the result'), 'get_typification_field')
    chromatogram = fields.Char('Chromatogram')
    professionals = fields.One2Many('lims.notebook.line.professional', 'notebook_line', 'Analytic professionals')
    ...
    literal_result = fields.Char('Literal result', translate=True, states={'readonly': Bool(Eval('accepted'))},
      depends=['accepted'])
    ...
    analysis_order = fields.Function(fields.Integer('Order'), 'get_analysis_order')
    dilution_factor = fields.Float('Dilution factor')
    accepted = fields.Boolean('Accepted')
    acceptance_date = fields.DateTime('Acceptance date', states={'readonly': True})
    not_accepted_message = fields.Text('Message', readonly=True, states={'invisible': Not(Bool(Eval('not_accepted_message')))})
    annulled = fields.Boolean('Annulled', states={'readonly': True})
    annulment_date = fields.DateTime('Annulment date', states={'readonly': True})
    results_report = fields.Many2One('lims.results_report', 'Results Report', readonly=True)
    planification = fields.Many2One('lims.planification', 'Planification', readonly=True)
    urgent = fields.Function((fields.Boolean('Urgent')), 'get_service_field', searcher='search_service_field')
    ...
    report_date = fields.Function((fields.Date('Date agreed for result')), 'get_service_field',
      searcher='search_service_field')
    fraction = fields.Function((fields.Many2One('lims.fraction', 'Fraction')), 'get_service_field',
      searcher='search_service_field')
    fraction_type = fields.Function((fields.Many2One('lims.fraction.type', 'Fraction type')),
      'get_fraction_field', searcher='search_fraction_field')
    ...
    date = fields.Function((fields.DateTime('Date')), 'get_sample_field', searcher='search_sample_field')
    date2 = fields.Function((fields.Date('Date')), 'get_sample_field', searcher='search_sample_field')
    report_type = fields.Function((fields.Char('Report type')), 'get_typification_field',
      searcher='search_typification_field')
    report_result_type = fields.Function((fields.Char('Result type')), 'get_typification_field',
      searcher='search_typification_field')
    results_estimated_waiting = fields.Integer('Estimated number of days for results',
      states={'readonly': True})
    results_estimated_date = fields.Function(fields.Date('Estimated date of result'), 'get_results_estimated_date')
    department = fields.Many2One('company.department', 'Department', readonly=True)
    icon = fields.Function(fields.Char('Icon'), 'get_icon')
    planning_comments = fields.Function(fields.Text('Planification comments'), 'get_planning_comments')
    controls = fields.Many2Many('lims.notebook.line-fraction', 'notebook_line', 'fraction', 'Controls')
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update({'end_date':'The end date cannot be lower than start date', 
         ...
         'not_accepted_7':'The Converted result / Converted result modifier is invalid', 
        ...
        if accepted:
            analysis_details = EntryDetailAnalysis.search([
             (
              'id', 'in', details)])
            if analysis_details:
                EntryDetailAnalysis.write(analysis_details, {'state': 'done'})
        else:
            analysis_details = EntryDetailAnalysis.search([
             (
              'id', 'in', details),
    ...
    def update_detail_report(lines):
        ...
        for d in analysis_details:
            cursor.execute('SELECT report FROM "' + NotebookLine._table + '" WHERE analysis_detail = %s ORDER BY id DESC LIMIT 1', (
             d.id,))
    ...
    def search_fraction_field(cls, name, clause):
        ...
        return [
         (
          'service.fraction.' + name,) + tuple(clause[1:])]
    ...
    def fields_view_get(cls, view_id=None, view_type='form'):
        ...
        result = super(NotebookLine, cls).fields_view_get(view_id=view_id, view_type=view_type)
        if view_id:
            if UiView(view_id).name == 'notebook_line_all_list':
                return result
        ...
        if view_type == 'tree':
            ...
            for field in ('report_date', 'result', 'converted_result', 'result_modifier',
                          'converted_result_modifier', 'literal_result', 'backup',
                          'verification', 'uncertainty', 'accepted', 'acceptance_date',
                          'end_date', 'report', 'annulled', 'annulment_date', 'icon'):
                fields.add(field)
    ...
    @fields.depends('result', 'converted_result', 'converted_result_modifier', 'backup', 'verification', 'uncertainty', 'end_date')
    ...
    @fields.depends('accepted', 'report', 'annulled', 'result', 'converted_result', 'literal_result', 'result_modifier', 'converted_result_modifier', 'end_date', 'acceptance_date')
    def on_change_accepted(self):
        self.not_accepted_message = ''
        if self.accepted:
            if not self.report:
                self.accepted = False
                self.not_accepted_message = self.raise_user_error('not_accepted_1',
                  raise_exception=False)
            else:
                if self.annulled:
                    self.accepted = False
                    self.not_accepted_message = self.raise_user_error('not_accepted_2',
                      raise_exception=False)
                else:
                    if not self.end_date:
                        self.accepted = False
                        self.not_accepted_message = self.raise_user_error('not_accepted_3',
                          raise_exception=False)
                    else:
                        if not self.result:
                            if not self.converted_result:
                                if not self.literal_result:
                                    if not self.result_modifier in ('nd', 'pos', 'neg',
                                                                    'ni', 'abs',
                                                                    'pre'):
                                        if not self.converted_result_modifier in ('nd',
                                                                                  'pos',
                                                                                  'neg',
                                                                                  'ni',
                                                                                  'abs',
                                                                                  'pre'):
                                            self.accepted = False
                                            self.not_accepted_message = self.raise_user_error('not_accepted_4',
                                              raise_exception=False)
                if self.converted_result and self.converted_result_modifier not in ('ni',
                                                                                    'eq',
                                                                                    'low'):
                    self.accepted = False
                    self.not_accepted_message = self.raise_user_error('not_accepted_5',
                      raise_exception=False)
                else:
                    if self.result and self.result_modifier not in ('ni', 'eq', 'low'):
                        self.accepted = False
                        self.not_accepted_message = self.raise_user_error('not_accepted_6',
                          raise_exception=False)
                    else:
                        if not (self.result_modifier == 'ni'):
                            if self.converted_result_modifier != 'nd':
                                self.accepted = False
                                self.not_accepted_message = self.raise_user_error('not_accepted_7',
                                  raise_exception=False)
                            else:
    ...
    def on_change_result_modifier(self):
        if self.result_modifier == 'na':
            self.annulled = self.annulled or True
        ...
        for nl in notebook_lines:
            typifications = Typification.search([
             ...
             (
              'method', '=', nl.method.id),
             ('valid', '=', True)])
            typification = typifications[0] if len(typifications) == 1 else None
            for name in names:
                if typification:
    ...
    def search_typification_field(cls, name, clause):
        ...
        cursor.execute('SELECT nl.id FROM "' + cls._table + '" nl INNER JOIN "' + Notebook._table + '" n ON nl.notebook = n.id INNER JOIN "' + Fraction._table + '" f ON n.fraction = f.id INNER JOIN "' + Sample._table + '" s ON f.sample = s.id INNER JOIN "' + Typification._table + '" t ON (nl.analysis = t.analysis AND nl.method = t.method AND s.product_type = t.product_type AND t.matrix = t.matrix) WHERE t.valid = TRUE AND t.' + name + ' ' + operator_ + ' %s', clause[2:3])
    ...
    def get_results_estimated_date(cls, notebook_lines, name):
        ...
        for nl in notebook_lines:
            ...
            if not estimated_waiting:
                continue
            result[nl.id] = cls._get_results_estimated_date(confirmation_date, estimated_waiting)

        return result

    @staticmethod
    def _get_results_estimated_date(confirmation_date, estimated_waiting):
        date = confirmation_date + relativedelta(days=estimated_waiting)
        return date
    ...
    def on_change_with_device_domain(self, name=None):
        ...
        if self.analysis:
            ...
            cursor.execute('SELECT DISTINCT(device) FROM "' + AnalysisDevice._table + '" WHERE analysis = %s  AND laboratory = %s', (
...
class NotebookLineAllFields(ModelSQL, ModelView):
    ...
    fraction_type = fields.Many2One('lims.fraction.type', 'Fraction type', readonly=True)
    ...
    laboratory_professionals = fields.Function((fields.Many2Many('lims.laboratory.professional', None, None, 'Preparation professionals')),
      'get_line_field', searcher='search_line_field')
    initial_unit = fields.Many2One('product.uom', 'Initial unit', readonly=True)
    ...
    professionals = fields.Function((fields.One2Many('lims.notebook.line.professional', None, 'Analytic professionals')),
      'get_line_field', searcher='search_line_field')
    comments = fields.Text('Entry comments', readonly=True)
    theoretical_concentration = fields.Char('Theoretical concentration', readonly=True)
    concentration_level = fields.Many2One('lims.concentration.level', 'Concentration level',
      readonly=True)
    ...
    results_estimated_waiting = fields.Integer('Estimated number of days for results')
    results_estimated_date = fields.Function(fields.Date('Estimated date of result'), 'get_line_field')
    department = fields.Many2One('company.department', 'Department', readonly=True)
...
class NotebookLineLaboratoryProfessional(ModelSQL):
    __doc__ = 'Laboratory Notebook Line - Laboratory Professional'
...
class NotebookLineProfessional(ModelSQL, ModelView):
    __doc__ = 'Laboratory Notebook Line Professional'
...
class NotebookInitialConcentrationCalcStart(ModelView):
    __doc__ = 'Initial Concentration Calculation'
...
class NotebookInitialConcentrationCalc(Wizard):
    ...
    start_state = 'ok'
    start = StateView('lims.notebook.initial_concentration_calc.start', 'lims.lims_notebook_initial_concentration_calc_view_form', [
        ...
        for notebook_line in notebook_lines:
            if notebook_line.accepted:
                ...
                if not ic:
                    ...
                    analysis_code = ic[1:]
                    result = self._get_analysis_result(analysis_code, notebook_line.notebook)
                    if result is not None:
                        notebook_line.initial_concentration = str(result)
                        lines_to_save.append(notebook_line)
                elif ic[0] == 'R':
                    analysis_code = ic[1:]
                    result = self._get_relation_result(analysis_code, notebook_line.notebook)
                    if result is not None:
                        notebook_line.initial_concentration = str(result)
                        lines_to_save.append(notebook_line)
                continue
    ...
    def _get_variables(self, formula, notebook):
        ...
        for var in variables.keys():
            if var[0] == 'A':
                analysis_code = var[1:]
                result = self._get_analysis_result(analysis_code, notebook, round_=True)
                if result is not None:
...
class NotebookLineInitialConcentrationCalc(NotebookInitialConcentrationCalc):
    ...
    def transition_ok(self):
        NotebookLine = Pool().get('lims.notebook.line')
        notebook_lines = NotebookLine.browse(Transaction().context['active_ids'])
        if not notebook_lines:
            return 'end'
...
class NotebookResultsConversion(Wizard):
    ...
    start_state = 'ok'
    start = StateView('lims.notebook.results_conversion.start', 'lims.lims_notebook_results_conversion_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Ok', 'ok', 'tryton-ok', default=True)])
    ok = StateTransition()
    ...
    def lines_results_conversion(self, notebook_lines):
        ...
        for notebook_line in notebook_lines:
            if notebook_line.accepted:
                continue
            if notebook_line.converted_result or notebook_line.result:
                if notebook_line.result_modifier != 'eq':
                    ...
                    variables = self._get_variables(formula, notebook_line, initial_uom_volume, final_uom_volume)
                    parser = FormulaParser(formula, variables)
                    formula_result = parser.getValue()
                    if initial_uom_volume:
                        if final_uom_volume:
                            ...
                            converted_result = result * (fc / ic) * (d_fc / d_ic) * formula_result
                            notebook_line.converted_result = str(converted_result)
                            notebook_line.converted_result_modifier = 'eq'
                            lines_to_save.append(notebook_line)
    ...
    def _get_variables(self, formula, notebook_line, initial_uom_volume=False, final_uom_volume=False):
        VolumeConversion = Pool().get('lims.volume.conversion')
        variables = {}
        for var in ('DI', ):
            while True:
...
class NotebookLimitsValidation(Wizard):
    ...
    start_state = 'ok'
    start = StateView('lims.notebook.limits_validation.start', 'lims.lims_notebook_limits_validation_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Ok', 'ok', 'tryton-ok', default=True)])
    ok = StateTransition()
    ...
    def lines_limits_validation(self, notebook_lines):
        ...
        for notebook_line in notebook_lines:
            ...
            if notebook_line.result and not notebook_line.check_result_limits:
                if notebook_line.converted_result or notebook_line.result_modifier != 'eq':
                    continue
                else:
                    try:
...
class NotebookInternalRelationsCalc1Start(ModelView):
    __doc__ = 'Internal Relations Calculation'
...
class NotebookInternalRelationsCalc1Relation(ModelSQL):
    ...
    variables = fields.One2Many('lims.notebook.internal_relations_calc_1.variable', 'relation', 'Variables')
    session_id = fields.Integer('Session ID')

    @classmethod
    def __register__(cls, module_name):
        super(NotebookInternalRelationsCalc1Relation, cls).__register__(module_name)
        cursor = Transaction().connection.cursor()
...
class NotebookInternalRelationsCalc1Variable(ModelSQL):
    ...
    relation = fields.Many2One('lims.notebook.internal_relations_calc_1.relation',
...
class NotebookInternalRelationsCalc1(Wizard):
    ...
    start_state = 'search'
    start = StateView('lims.notebook.internal_relations_calc_1.start', 'lims.lims_notebook_internal_relations_calc_1_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
    ...
    def get_relations(self, notebook_lines):
        NotebookInternalRelationsCalc1Relation = Pool().get('lims.notebook.internal_relations_calc_1.relation')
        relations = {}
        for notebook_line in notebook_lines:
            if notebook_line.accepted:
                continue
            ...
            if analysis_code:
                ...
                formulas += '+' + notebook_line.analysis.converted_result_formula
                for i in (' ', '\t', '\n', '\r'):
                    formulas = formulas.replace(i, '')

                variables = self._get_variables_list(formulas, notebook_line.notebook, {})
                if not variables:
                    continue
                ...
                if not has_repetition_zero:
                    continue
                relations[notebook_line.analysis.id] = {'notebook':notebook_line.notebook.id,  'internal_relation':notebook_line.analysis.id, 
                 'variables':[
                  (
                   'create', variables)], 
        ...
        if relations:
            NotebookInternalRelationsCalc1Relation.create([ir for ir in relations.values()])
            return True
        return False
    ...
    def transition_confirm(self):
        ...
        for relation in relations:
            ...
            result = self._get_relation_result(analysis_code, relation.notebook, analysis_code)
            converted_result = self._get_relation_result(analysis_code, (relation.notebook),
              analysis_code, converted=True)
    ...
    def _get_analysis_result(self, analysis_code, notebook, relation_code, converted=False):
        NotebookInternalRelationsCalc1Variable = Pool().get('lims.notebook.internal_relations_calc_1.variable')
    ...
    def _get_relation_result(self, analysis_code, notebook, relation_code, converted=False, round_=False):
        pool = Pool()
        Analysis = pool.get('lims.analysis')
        NotebookLine = pool.get('lims.notebook.line')
        internal_relations = Analysis.search([
        ...
        variables = self._get_variables(formula, notebook, relation_code, converted)
        if not variables:
            return
        else:
            parser = FormulaParser(formula, variables)
    ...
    def _get_variables(self, formula, notebook, relation_code, converted=False):
        VolumeConversion = Pool().get('lims.volume.conversion')
        variables = {}
        for prefix in ('A', 'D', 'T', 'Y', 'R'):
            while True:
        ...
        for var in variables.keys():
            if var[0] == 'A':
                analysis_code = var[1:]
                result = self._get_analysis_result(analysis_code, notebook, relation_code, converted)
                if result is not None:
                    variables[var] = result
            else:
                if var[0] == 'D':
...
class NotebookInternalRelationsCalc2Result(ModelView):
    ...
    relations = fields.Many2Many('lims.notebook.internal_relations_calc_2.relation', None, None, 'Relation')
...
class NotebookInternalRelationsCalc2Relation(ModelSQL, ModelView):
    ...
    notebook = fields.Many2One('lims.notebook', 'Laboratory notebook')
    internal_relation = fields.Many2One('lims.analysis', 'Internal relation')
    variables = fields.One2Many('lims.notebook.internal_relations_calc_2.variable', 'relation', 'Variables')
    session_id = fields.Integer('Session ID')

    @classmethod
    def __register__(cls, module_name):
        super(NotebookInternalRelationsCalc2Relation, cls).__register__(module_name)
        cursor = Transaction().connection.cursor()
        cursor.execute('DELETE FROM "' + cls._table + '"')
...
class NotebookInternalRelationsCalc2Variable(ModelSQL, ModelView):
    ...
    relation = fields.Many2One('lims.notebook.internal_relations_calc_2.relation',
      'Relation', ondelete='CASCADE',
      readonly=True)
    line = fields.Many2One('lims.notebook.line', 'Line')
    analysis = fields.Many2One('lims.analysis', 'Analysis', readonly=True)
    ...
    result_modifier = fields.Function(fields.Selection([
     ...
     ('ni', 'ni'),
     ('abs', 'Absence'),
     ('pre', 'Presence')], 'Result modifier'), 'get_line_field')
    result = fields.Function(fields.Char('Result'), 'get_line_field')
    initial_unit = fields.Function(fields.Many2One('product.uom', 'Initial unit'), 'get_line_field')
    initial_concentration = fields.Function(fields.Char('Initial concentration'), 'get_line_field')
    converted_result_modifier = fields.Function(fields.Selection([
     ...
     ('pos', 'Positive'),
     ('neg', 'Negative'),
     ('ni', 'ni')], 'Converted result modifier'), 'get_line_field')
    converted_result = fields.Function(fields.Char('Converted result'), 'get_line_field')
    final_unit = fields.Function(fields.Many2One('product.uom', 'Final unit'), 'get_line_field')
    final_concentration = fields.Function(fields.Char('Final concentration'), 'get_line_field')
    use = fields.Boolean('Use')
...
class NotebookInternalRelationsCalc2Process(ModelView):
    ...
    notebook = fields.Many2One('lims.notebook', 'Laboratory notebook', readonly=True)
    internal_relation = fields.Many2One('lims.analysis', 'Internal relation', readonly=True)
    variables = fields.One2Many('lims.notebook.internal_relations_calc_2.variable', None, 'Variables')
...
class NotebookInternalRelationsCalc2(Wizard):
    ...
    start_state = 'search'
    start = StateView('lims.notebook.internal_relations_calc_2.start', 'lims.lims_notebook_internal_relations_calc_2_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Search', 'search', 'tryton-forward', default=True)])
    search = StateTransition()
    result = StateView('lims.notebook.internal_relations_calc_2.result', 'lims.lims_notebook_internal_relations_calc_2_result_view_form', [])
    next_ = StateTransition()
    process = StateView('lims.notebook.internal_relations_calc_2.process', 'lims.lims_notebook_internal_relations_calc_2_process_view_form', [
    ...
    def get_relations(self, notebook_lines):
        NotebookInternalRelationsCalc2Relation = Pool().get('lims.notebook.internal_relations_calc_2.relation')
        relations = {}
        for notebook_line in notebook_lines:
            if notebook_line.accepted:
                continue
        ...
        if relations:
            res_lines = NotebookInternalRelationsCalc2Relation.create([ir for ir in relations.values()])
    ...
    def default_process(self, fields):
        NotebookInternalRelationsCalc2Variable = Pool().get('lims.notebook.internal_relations_calc_2.variable')
...
class NotebookLoadResultsFormulaStart(ModelView):
    __doc__ = 'Load Results by Formula'
    __name__ = 'lims.notebook.load_results_formula.start'
    analysis = fields.Many2One('lims.analysis', 'Analysis', domain=[
     ('state', '=', 'active'), ('formula', '!=', None)])
...
class NotebookLoadResultsFormulaResult(ModelView):
    ...
    lines = fields.Many2Many('lims.notebook.load_results_formula.line', None, None, 'Lines')
    total = fields.Integer('Total')
    index = fields.Integer('Index')
...
class NotebookLoadResultsFormulaAction(ModelSQL):
    ...
    variables = fields.One2Many('lims.notebook.load_results_formula.variable', 'action', 'Variables')
    session_id = fields.Integer('Session ID')

    @classmethod
    def __register__(cls, module_name):
        super(NotebookLoadResultsFormulaAction, cls).__register__(module_name)
        cursor = Transaction().connection.cursor()
        cursor.execute('DELETE FROM "' + cls._table + '"')
...
class NotebookLoadResultsFormulaProcess(ModelView):
    ...
    formula_formula = fields.Function(fields.Char('Formula'), 'on_change_with_formula_formula')
    variables = fields.One2Many('lims.notebook.load_results_formula.variable', None, 'Variables')
    ...
    def on_change_with_result(self, name=None):
        return self.formula and self.variables or None
...
class NotebookLoadResultsFormulaVariable(ModelSQL, ModelView):
    ...
    action = fields.Many2One('lims.notebook.load_results_formula.action', 'Action',
      ondelete='CASCADE')
...
class NotebookLoadResultsFormulaSit2(ModelView):
    ...
    details = fields.One2Many('lims.notebook.load_results_formula.sit2.detail', None, 'Supervisors')
...
class NotebookLoadResultsFormulaSit2Detail(ModelSQL, ModelView):
    ...
    supervisor = fields.Many2One('lims.laboratory.professional', 'Supervisor',
      depends=['supervisor_domain'], domain=[
     (
      'id', 'in', Eval('supervisor_domain'))])
    supervisor_domain = fields.Function(fields.Many2Many('lims.laboratory.professional', None, None, 'Supervisor domain'), 'get_supervisor_domain')
    lines = fields.Many2Many('lims.notebook.load_results_formula.sit2.detail.line', 'load_results', 'notebook_line', 'Lines')
    session_id = fields.Integer('Session ID')

    @classmethod
    def __register__(cls, module_name):
        super(NotebookLoadResultsFormulaSit2Detail, cls).__register__(module_name)
        cursor = Transaction().connection.cursor()
        cursor.execute('DELETE FROM "' + cls._table + '"')
...
class NotebookLoadResultsFormulaSit2DetailLine(ModelSQL):
    ...
    load_results = fields.Many2One('lims.notebook.load_results_formula.sit2.detail',
      'Load Results', ondelete='CASCADE',
      select=True,
      required=True)
    notebook_line = fields.Many2One('lims.notebook.line', 'Notebook Line', ondelete='CASCADE',
      select=True,
...
class NotebookLoadResultsFormula(Wizard):
    ...
    start = StateView('lims.notebook.load_results_formula.start', 'lims.lims_notebook_load_results_formula_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Search', 'search', 'tryton-forward', default=True)])
    search = StateTransition()
    empty = StateView('lims.notebook.load_results_formula.empty', 'lims.lims_notebook_load_results_formula_empty_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Search again', 'start', 'tryton-forward', default=True)])
    result = StateView('lims.notebook.load_results_formula.result', 'lims.lims_notebook_load_results_formula_result_view_form', [])
    next_ = StateTransition()
    prev_ = StateTransition()
    process = StateView('lims.notebook.load_results_formula.process', 'lims.lims_notebook_load_results_formula_process_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Previous', 'prev_', 'tryton-back'),
     Button('Next', 'next_', 'ttryton-forward', default=True)])
    beginning = StateView('lims.notebook.load_results_formula.beginning', 'lims.lims_notebook_load_results_formula_beginning_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Next', 'next_', 'tryton-forward', default=True)])
    confirm = StateView('lims.notebook.load_results_formula.confirm', 'lims.lims_notebook_load_results_formula_confirm_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Previous', 'prev_', 'tryton-back'),
     Button('Confirm', 'check_professional', 'tryton-ok', default=True)])
    check_professional = StateTransition()
    sit1 = StateView('lims.notebook.load_results_formula.sit1', 'lims.lims_notebook_load_results_formula_sit1_view_form', [
     Button('Cancel', 'end', 'tryton-cancel', default=True)])
    sit2 = StateView('lims.notebook.load_results_formula.sit2', 'lims.lims_notebook_load_results_formula_sit2_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
    ...
    def transition_search(self):
        pool = Pool()
        NotebookLine = pool.get('lims.notebook.line')
        NotebookLoadResultsFormulaLine = pool.get('lims.notebook.load_results_formula.line')
        clause = [
         (
    ...
    def transition_next_(self):
        pool = Pool()
        NotebookLoadResultsFormulaAction = pool.get('lims.notebook.load_results_formula.action')
        NotebookLoadResultsFormulaLine = pool.get('lims.notebook.load_results_formula.line')
        LaboratoryProfessional = pool.get('lims.laboratory.professional')
        has_prev = hasattr(self.process, 'line') and getattr(self.process, 'line')
        if has_prev:
            defaults = {'session_id':self._session_id,  'line':self.process.line.id, 
             ...
             'formula':self.process.formula.id if self.process.formula else None}
            variables = []
            for var in self.process.variables:
                variables.append({'number':var.number, 
            ...
            if action:
                defaults['variables'] = [('delete', [v.id for a in action for v in a.variables])] + defaults['variables']
        ...
        if self.result.index <= self.result.total:
            ...
            if action:
                ...
                self.process.initial_concentration = action[0].initial_concentration
    ...
    def default_process(self, fields):
        if not self.process.line:
            ...
            default['product_type'] = self.process.line.notebook.product_type.id
            default['matrix'] = self.process.line.notebook.matrix.id
            if hasattr(self.process, 'formula') and getattr(self.process, 'formula'):
                ...
                default['initial_concentration'] = self.process.initial_concentration
            ...
            for field in ('initial_concentration', 'comments'):
                if hasattr(self.process, field + '_copy') and getattr(self.process, field + '_copy'):
                    default[field] = getattr(self.process, field)
                    default[field + '_copy'] = getattr(self.process, field + '_copy')
                else:
                    default[field] = getattr(self.process.line, field)

            for field in ('professional', ):
                if hasattr(self.process, field) and getattr(self.process, field):
    ...
    def transition_check_professional(self):
        ...
        NotebookLoadResultsFormulaSit2Detail = pool.get('lims.notebook.load_results_formula.sit2.detail')
        ...
        for data in actions:
            key = (
             data.professional.id, data.line.method.id)
        ...
        for key, sit in situations.items():
            if sit == 2:
                situation_2.append({'session_id':self._session_id, 
                 'professional':key[0], 
                 'method':key[1], 
                 'lines':[
                  (
        ...
        if situation_2:
            details = NotebookLoadResultsFormulaSit2Detail.create(situation_2)
    ...
    def transition_confirm_(self):
        ...
        NotebookLine = pool.get('lims.notebook.line')
        LabProfessionalMethod = pool.get('lims.lab.professional.method')
        LabProfessionalMethodRequalification = pool.get('lims.lab.professional.method.requalification')
        Date = pool.get('ir.date')
        actions = NotebookLoadResultsFormulaAction.search([
         (
          'session_id', '=', self._session_id)])
        for data in actions:
            ...
            if not notebook_line:
                continue
            notebook_line_write = {'result':data.result, 
        ...
        if hasattr(self.sit2, 'details'):
            for detail in self.sit2.details:
                ...
                supervisor_lines[detail.supervisor.id].extend([l.id for l in detail.lines])
        ...
        for prof_id, lines in supervisor_lines.items():
            ...
            if notebook_lines:
                ...
                notebook_line_write = {'professionals': [('create', professionals)]}
        ...
        if hasattr(self.sit2, 'details'):
            for detail in self.sit2.details:
                key = (
                 detail.supervisor.id, detail.method.id)
                if key not in all_prof:
                    all_prof[key] = []
                key = (
                 detail.professional.id, detail.method.id)
        ...
        for key, sup in all_prof.items():
            ...
            if professional_method.state == 'training':
                history = LabProfessionalMethodRequalification.search([
                 ...
                 ('type', '=', 'training')])
                if history:
                    prev_supervisors = [s.supervisor.id for s in history[0].supervisors]
                    supervisors = [{'supervisor': s} for s in sup if s not in prev_supervisors]
                    LabProfessionalMethodRequalification.write(history, {'last_execution_date':today, 
                     'supervisors':[
                      (
                       'create', supervisors)]})
                else:
                ...
                if history:
                    ...
                    to_create = [
                     {'professional_method':professional_method.id,  'type':'qualification', 
                      'date':today, 
                      'last_execution_date':today}]
                    LabProfessionalMethodRequalification.create(to_create)
            else:
                ...
                if history:
                    ...
                    to_create = [
                     {'professional_method':professional_method.id,  'type':'requalification', 
                      'date':today, 
                      'last_execution_date':today}]
                    LabProfessionalMethodRequalification.create(to_create)
...
class NotebookLoadResultsManualResult(ModelView):
    ...
    professional = fields.Many2One('lims.laboratory.professional', 'Laboratory professional',
      required=True, readonly=True)
    lines = fields.One2Many('lims.notebook.load_results_manual.line', None, 'Lines')
...
class NotebookLoadResultsManualLine(ModelSQL, ModelView):
    ...
    @classmethod
    def __register__(cls, module_name):
        super(NotebookLoadResultsManualLine, cls).__register__(module_name)
        cursor = Transaction().connection.cursor()
        cursor.execute('DELETE FROM "' + cls._table + '"')
    ...
    def on_change_with_end_date(self):
        ...
        if self.result or self.literal_result or self.result_modifier not in ('eq',
                                                                              'low'):
            return Date.today()
...
class NotebookLoadResultsManualSit2(ModelView):
    ...
    supervisor = fields.Many2One('lims.laboratory.professional', 'Supervisor',
     ...
     (
      'id', 'in', Eval('supervisor_domain'))],
      required=True)
    supervisor_domain = fields.Many2Many('lims.laboratory.professional', None, None, 'Supervisor domain')
...
class NotebookLoadResultsManual(Wizard):
    ...
    start = StateView('lims.notebook.load_results_manual.start', 'lims.lims_notebook_load_results_manual_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Search', 'search', 'tryton-forward', default=True)])
    search = StateTransition()
    empty = StateView('lims.notebook.load_results_manual.empty', 'lims.lims_notebook_load_results_manual_empty_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Search again', 'start', 'tryton-forward', default=True)])
    result = StateView('lims.notebook.load_results_manual.result', 'lims.lims_notebook_load_results_manual_result_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Confirm', 'check_professional', 'tryton-ok', default=True)])
    check_professional = StateTransition()
    sit1 = StateView('lims.notebook.load_results_manual.sit1', 'lims.lims_notebook_load_results_manual_sit1_view_form', [
     Button('Cancel', 'end', 'tryton-cancel', default=True)])
    sit2 = StateView('lims.notebook.load_results_manual.sit2', 'lims.lims_notebook_load_results_manual_sit2_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
    ...
    def transition_search(self):
        ...
        NotebookLine = pool.get('lims.notebook.line')
        LaboratoryProfessional = pool.get('lims.laboratory.professional')
        NotebookLoadResultsManualLine = pool.get('lims.notebook.load_results_manual.line')
        clause = [
         ('end_date', '=', None),
         (
            ...
            for line in lines:
                res_line, = NotebookLoadResultsManualLine.create([
                 {'session_id':self._session_id, 
                  ...
                  'initial_unit':line.initial_unit.id if line.initial_unit else None, 
    ...
    def default_result(self, fields):
        ...
        default['professional'] = self.result.professional.id if self.result.professional else None
    ...
    def default_sit2(self, fields):
        ...
        if self.sit2.supervisor_domain:
            defaults['supervisor_domain'] = [p.id for p in self.sit2.supervisor_domain]
        if self.sit2.lines:
    ...
    def transition_confirm_(self):
        ...
        for data in actions:
            ...
            notebook_line_write = {'result':data.result, 
             ...
             'end_date':data.end_date, 
             'chromatogram':data.chromatogram, 
             'initial_unit':data.initial_unit.id if data.initial_unit else None, 
             'comments':data.comments, 
            ...
            if data.result_modifier == 'na':
                ...
                notebook_line_write['report'] = False
            professionals = [
             {'professional': self.result.professional.id}]
            notebook_line_write['professionals'] = [
             (
              'delete', [p.id for p in notebook_line.professionals])] + [
             (
        ...
        if hasattr(self.sit2, 'supervisor'):
            supervisor_lines[self.sit2.supervisor.id] = [l.id for l in self.sit2.lines]
        for prof_id, lines in supervisor_lines.items():
            notebook_lines = NotebookLine.search([
             (
              'id', 'in', lines)])
        ...
        if hasattr(self.sit2, 'supervisor'):
            for detail in self.sit2.lines:
                key = (
                 self.sit2.supervisor.id, detail.method.id)
                if key not in all_prof:
                    all_prof[key] = []
                key = (
                 self.result.professional.id, detail.method.id)
...
class NotebookAddInternalRelationsStart(ModelView):
    __doc__ = 'Add Internal Relations'
    __name__ = 'lims.notebook.add_internal_relations.start'
    analysis = fields.Many2Many('lims.analysis', None, None, 'Internal relations',
      required=True, domain=[
     (
      'id', 'in', Eval('analysis_domain'))],
      depends=[
     'analysis_domain'])
    analysis_domain = fields.One2Many('lims.analysis', None, 'Internal relations domain')


class NotebookAddInternalRelations(Wizard):
    __doc__ = 'Add Internal Relations'
    __name__ = 'lims.notebook.add_internal_relations'
    start = StateView('lims.notebook.add_internal_relations.start', 'lims.lims_notebook_add_internal_relations_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
    ...
    def default_start(self, fields):
        ...
        if not notebook.lines:
            ...
            notebook_analysis_codes = "', '".join((str(a) for a in notebook_analysis))
            cursor.execute('SELECT DISTINCT(a.id, a.result_formula) FROM "' + Analysis._table + '" a INNER JOIN "' + Typification._table + '" t ON a.id = t.analysis WHERE t.product_type = %s AND t.matrix = %s AND t.valid AND a.behavior = \'internal_relation\' AND a.code NOT IN (\'' + notebook_analysis_codes + "')", (
             notebook.fraction.product_type.id,
             notebook.fraction.matrix.id))
            internal_relations = cursor.fetchall()
            return internal_relations or default
    ...
    def _create_service(self, analysis, fraction):
        ...
        laboratory_id = analysis.laboratories[0].laboratory.id if analysis.laboratories else None
        typifications = Typification.search([
         (
        ...
        if report_grouper != 0:
            EntryDetailAnalysis.write(analysis_detail, {'report_grouper': report_grouper})
    ...
    def _get_report_grouper(self, analysis):
        ...
        for v in variables:
            if v in notebook_analysis:
                ...
                report_grouper = notebook_analysis[v].analysis_detail.report_grouper
                break

        return (
         divide, report_grouper)


class NotebookLineRepeatAnalysisStart(ModelView):
    __doc__ = 'Repeat Analysis'
...
class NotebookLineRepeatAnalysis(Wizard):
    ...
    start = StateView('lims.notebook.line.repeat_analysis.start', 'lims.lims_notebook_line_repeat_analysis_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
    ...
    def default_start(self, fields):
        ...
        default = {'analysis_domain': notebook_analysis}
    ...
    def transition_repeat(self):
        ...
        if analysis.type == 'analysis':
            ...
            analysis_to_repeat = Analysis.get_included_analysis_analysis(analysis.id)
        notebook_line = NotebookLine(Transaction().context['active_id'])
        notebook = Notebook(notebook_line.notebook.id)
        rm_type = notebook.fraction.special_type == 'rm'
        if rm_type:
            config = Config(1)
            rm_start_uom = config.rm_start_uom.id if config.rm_start_uom else None
        to_create = []
        details_to_update = []
        for analysis_id in analysis_to_repeat:
            ...
            if not nlines:
                ...
                defaults = {'analysis_detail':detail_id, 
                 ...
                 'device':nline_to_repeat.device.id if nline_to_repeat.device else None, 
                 ...
                 'concentration_level':nline_to_repeat.concentration_level.id if nline_to_repeat.concentration_level else None, 
                 'results_estimated_waiting':nline_to_repeat.results_estimated_waiting, 
                ...
                if rm_type:
                    ...
                    defaults['final_concentration'] = nline_to_repeat.final_concentration
                defaults['initial_unit'] = nline_to_repeat.initial_unit.id if nline_to_repeat.initial_unit else None
                defaults['final_unit'] = nline_to_repeat.final_unit.id if nline_to_repeat.final_unit else None
                defaults['detection_limit'] = nline_to_repeat.detection_limit
                defaults['quantification_limit'] = nline_to_repeat.quantification_limit
        ...
        if details:
            EntryDetailAnalysis.write(details, {'state': 'unplanned'})
        return 'end'


class NotebookAcceptLinesStart(ModelView):
    __doc__ = 'Accept Lines'
...
class NotebookAcceptLines(Wizard):
    ...
    start_state = 'ok'
    start = StateView('lims.notebook.accept_lines.start', 'lims.lims_notebook_accept_lines_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Ok', 'ok', 'tryton-ok', default=True)])
    ok = StateTransition()
    ...
    def transition_ok(self):
        ...
        for active_id in Transaction().context['active_ids']:
            ...
            notebook_lines = [l for l in notebook.lines if l.analysis.id not in repeated_analysis]
    ...
    def lines_accept(self, notebook_lines):
        ...
        for notebook_line in notebook_lines:
            ...
            if not notebook_line.end_date:
                continue
            if not notebook_line.result:
                if not notebook_line.converted_result:
                    if not notebook_line.literal_result:
                        if not notebook_line.result_modifier in ('nd', 'pos', 'neg',
                                                                 'ni', 'abs', 'pre'):
                            if not notebook_line.converted_result_modifier in ('nd',
                                                                               'pos',
                                                                               'neg',
                                                                               'ni',
                                                                               'abs',
                                                                               'pre'):
                                continue
                elif notebook_line.converted_result and notebook_line.converted_result_modifier not in ('ni',
                                                                                                        'eq',
                                                                                                        'low'):
                    continue
                if notebook_line.result:
                    if notebook_line.result_modifier not in ('ni', 'eq', 'low'):
                        continue
                ...
                if notebook_id not in accepted_analysis:
                    accepted_lines = NotebookLine.search([
                     ...
                     ('accepted', '=', True)])
                    accepted_analysis[notebook_id] = [l.analysis.id for l in accepted_lines]
        ...
        if lines_to_write:
            ...
            NotebookLine.write(lines_to_write, {'accepted':True, 
...
class NotebookLineUnacceptLines(Wizard):
    ...
    def lines_unaccept(self, notebook_lines):
        ...
        if lines_to_write:
            NotebookLine.write(lines_to_write, {'accepted':False, 
             'acceptance_date':None})


class NotebookResultsVerificationStart(ModelView):
    __doc__ = 'Results Verification'
    __name__ = 'lims.notebook.results_verification.start'
    range_type = fields.Many2One('lims.range.type', 'Origin', required=True, domain=[
...
class NotebookResultsVerification(Wizard):
    ...
    start = StateView('lims.notebook.results_verification.start', 'lims.lims_notebook_results_verification_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Ok', 'ok', 'tryton-ok', default=True)])
    ok = StateTransition()
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update({'ok':'OK', 
         'ok*':'OK*', 
         'out':'Out of Range'})
                        ...
                        if iu == fu and ic == fc:
                            ...
                            if iu != fu and ic == fc:
                                ...
                                if iu == fu and ic != fc:
                                    ...
                                    parser = FormulaParser(formula, variables)
                                    formula_result = parser.getValue()
                                    if conversions[0].initial_uom_volume and conversions[0].final_uom_volume:
                                        d_ic = VolumeConversion.brixToDensity(ic)
                                        d_fc = VolumeConversion.brixToDensity(fc)
                                        converted_result = result * (fc / ic) * (d_fc / d_ic) * formula_result
                                    else:
    ...
    def _verificate_result(self, result, range_):
        if range_.min95 and range_.max95:
            if result < range_.min:
                return 'out'
            if result < range_.min95:
                return 'ok*'
            if result <= range_.max95:
                return 'ok'
            if result <= range_.max:
                return 'ok*'
            return 'out'
        else:
            if range_.min:
    ...
    def _get_verifications(self):
        ...
        with Transaction().set_context(language=(lang.code)):
            verifications['ok'] = self.raise_user_error('ok', raise_exception=False)
            verifications['ok*'] = self.raise_user_error('ok*', raise_exception=False)
            verifications['out'] = self.raise_user_error('out', raise_exception=False)
...
class UncertaintyCalcStart(ModelView):
    __doc__ = 'Uncertainty Calculation'
...
class UncertaintyCalc(Wizard):
    ...
    start = StateView('lims.notebook.uncertainty_calc.start', 'lims.lims_notebook_uncertainty_calc_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Ok', 'ok', 'tryton-ok', default=True)])
    ok = StateTransition()
    ...
    def lines_uncertainty_calc(self, notebook_lines):
        ...
        for notebook_line in notebook_lines:
            ...
            if not ranges:
                continue
            uncertainty = self._get_uncertainty(result, notebook_line, ranges[0])
    ...
    def _get_uncertainty(self, result, notebook_line, range_):
        ...
        if dilution_factor:
            ...
            try:
                ...
                uncertainty = 0.0
                if not (range_.low_level_value):
                    if diluted_result > low_level:
                        uncertainty = range_.low_level_value
                    else:
                        if range_.low_level_value:
                            if range_.middle_level_value:
                                if range_.high_level_value:
                                    if low_level <= diluted_result and diluted_result < middle_level:
                                        uncertainty = range_.low_level_value
                                    else:
                                        if middle_level <= diluted_result and diluted_result < high_level:
                                            uncertainty = range_.middle_level_value
                                        else:
                                            if diluted_result >= high_level:
                                                uncertainty = range_.high_level_value
                    if range_.low_level_value and range_.middle_level_value:
                        if not range_.high_level_value:
                            if low_level <= diluted_result and diluted_result < middle_level:
                                uncertainty = range_.low_level_value
        else:
            if middle_level <= diluted_result:
                if diluted_result < high_level:
                    uncertainty = range_.middle_level_value
            if uncertainty > 0.0:
...
class NotebookPrecisionControl(Wizard):
    ...
    start_state = 'check'
    check = StateTransition()
    start = StateView('lims.notebook.precision_control.start', 'lims.lims_notebook_precision_control_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Ok', 'ok', 'tryton-ok', default=True)])
    ok = StateTransition()
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update({'acceptable':'Acceptable / Factor: %s - CV: %s', 
    ...
    def default_start(self, fields):
        ...
        RangeType = pool.get('lims.range.type')
        notebook = Notebook(Transaction().context['active_id'])
        default = {'product_type':notebook.product_type.id, 
        ...
        for notebook_line in notebook_lines:
            if notebook_line.verification:
                ...
                if concentration_level.code == 'NC':
                    ...
                    if concentration_level.code == 'NM':
                        cv = ranges[0].middle_level_coefficient_variation
                    else:
                        if concentration_level.code == 'NA':
                        ...
                        try:
                            if notebook_line.repetition == 0:
                                ...
                                rep_1 = float(self._get_repetition_result(notebook_line, 1))
                            else:
                                if notebook_line.repetition == 1:
                                    rep_0 = float(self._get_repetition_result(notebook_line, 0))
...
class NotebookLinePrecisionControl(NotebookPrecisionControl):
    ...
    def default_start(self, fields):
        ...
        with Transaction().set_context(active_id=(reference_line.notebook.id)):
            return super(NotebookLinePrecisionControl, self).default_start(fields)

    def transition_ok(self):
        NotebookLine = Pool().get('lims.notebook.line')
        reference_line = NotebookLine(Transaction().context['active_id'])
        notebook_lines = NotebookLine.browse(Transaction().context['active_ids'])
        notebook_lines = [l for l in notebook_lines if l.notebook.id == reference_line.notebook.id]
        if not notebook_lines:
            return 'end'
...
class OpenNotebookLines(Wizard):
    ...
    def do_open_(self, action):
        ...
        action['name'] = '%s - %s - %s - %s - %s' % (notebook.fraction.number,
...
class ChangeEstimatedDaysForResultsStart(ModelView):
    __doc__ = 'Change Estimated Days For Results'
    ...
    methods = fields.Many2Many('lims.lab.method', None,
      None, 'Methods', required=True)
...
class ChangeEstimatedDaysForResults(Wizard):
    ...
    start = StateView('lims.change_results_estimated_waiting.start', 'lims.change_results_estimated_waiting_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
    ...
    def transition_change(self):
        ...
        if notebook_lines:
            lines_to_save = []
            for line in notebook_lines:
                line.results_estimated_waiting = (line.results_estimated_waiting or 0) + self.start.results_estimated_waiting
                lines_to_save.append(line)
...
class PrintAnalysisPendingInformStart(ModelView):
    __doc__ = 'Analysis Pending of Inform'
    ...
    party = fields.Many2One('party.party', 'Party')
    include_comments_of_fraction = fields.Boolean('Include comments of the fraction')


class PrintAnalysisPendingInform(Wizard):
    __doc__ = 'Analysis Pending of Inform'
    __name__ = 'lims.print_analysis_pending_inform'
    start = StateView('lims.print_analysis_pending_inform.start', 'lims.print_analysis_pending_inform_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Print', 'print_', 'tryton-print', default=True)])
    print_ = StateReport('lims.analysis_pending_inform')

    def do_print_(self, action):
        data = {'date_from':self.start.date_from, 
         'date_to':self.start.date_to, 
         'laboratory':self.start.laboratory.id, 
         'party':self.start.party and self.start.party.id or None, 
         'include_comments_of_fraction':self.start.include_comments_of_fraction}
        return (
         action, data)
...
class AnalysisPendingInform(Report):
    ...
    def get_context(cls, records, data):
        ...
        report_context = super(AnalysisPendingInform, cls).get_context(records, data)
        ...
        report_context['include_comments_of_fraction'] = data['include_comments_of_fraction']
        objects = cls._get_report_records(data['date_from'], data['date_to'], data['laboratory'], data['party'])
    ...
    def _get_report_records(cls, date_from, date_to, laboratory, party):
        ...
        NotebookLine = pool.get('lims.notebook.line')
        res = []
        excluded_notebooks = cls._get_excluded_notebooks(date_from, date_to, laboratory, party)
...
class PrintAnalysisCheckedPendingInformStart(ModelView):
    __doc__ = 'Analysis checked pending of Inform'
...
class PrintAnalysisCheckedPendingInform(Wizard):
    ...
    start = StateView('lims.print_analysis_checked_pending_inform.start', 'lims.print_analysis_checked_pending_inform_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Print', 'print_', 'tryton-print', default=True)])
...
class AnalysisCheckedPendingInform(Report):
    ...
    def get_context(cls, records, data):
        ...
        Laboratory = pool.get('lims.laboratory')
        Party = pool.get('party.party')
        report_context = super(AnalysisCheckedPendingInform, cls).get_context(records, data)
        report_context['company'] = report_context['user'].company
        report_context['date_from'] = data['date_from']
        report_context['date_to'] = data['date_to']
        report_context['laboratory'] = Laboratory(data['laboratory']).rec_name
    ...
    def _get_report_records(cls, date_from, date_to, laboratory, party):
        ...
        NotebookLine = pool.get('lims.notebook.line')
        res = []
        included_notebooks = cls._get_included_notebooks(date_from, date_to, laboratory, party)
    ...
    def _get_included_notebooks(cls, date_from, date_to, laboratory, party):
        ...
        if party:
            party_clause = 'AND e.party = ' + str(party)
        cursor.execute('SELECT nl.notebook, nl.analysis FROM "' + NotebookLine._table + '" nl INNER JOIN "' + Notebook._table + '" n ON n.id = nl.notebook INNER JOIN "' + Fraction._table + '" f ON f.id = n.fraction INNER JOIN "' + Sample._table + '" s ON s.id = f.sample INNER JOIN "' + Entry._table + '" e ON e.id = s.entry INNER JOIN "' + FractionType._table + '" ft ON ft.id = f.type WHERE ft.report = TRUE AND s.date::date >= %s::date AND s.date::date <= %s::date AND nl.laboratory = %s AND nl.report = TRUE AND nl.accepted = TRUE AND nl.results_report IS NULL AND nl.annulled = FALSE ' + party_clause, (
         date_from, date_to, laboratory))
        notebook_lines = cursor.fetchall()
