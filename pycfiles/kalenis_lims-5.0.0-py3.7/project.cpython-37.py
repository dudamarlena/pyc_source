...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/trytond/modules/lims_project_water/project.py
...
__all__ = [
 'Project', 'Entry', 'Sample', 'CreateSampleStart', 'CreateSample']
STATES = {'required': Bool(Equal(Eval('type'), 'water'))}
DEPENDS = [
 'type']
...
class Project(metaclass=PoolMeta):
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update({'not_water': 'Please, select a "Water sampling" Project to print this report'})
...
class Sample(metaclass=PoolMeta):
    ...
    sampling_point = fields.Char('Sampling point', states={'invisible': Not(Bool(Equal(Eval('project_type'), 'water')))},
      depends=[
     'project_type'])
    gps_coordinates = fields.Char('GPS coordinates', states={'invisible': Not(Bool(Equal(Eval('project_type'), 'water')))},
      depends=[
     'project_type'])
    sampling_datetime = fields.DateTime('Sampling date and time', states={'invisible': Not(Bool(Equal(Eval('project_type'), 'water')))},
      depends=[
     'project_type'])
    sampling_responsible = fields.Many2One('party.party', 'Sampling responsible',
      states={'invisible': Not(Bool(Equal(Eval('project_type'), 'water')))},
      depends=[
     'project_type'])
...
class CreateSample(metaclass=PoolMeta):
    ...
    def _get_samples_defaults(self, entry_id):
        samples_defaults = super(CreateSample, self)._get_samples_defaults(entry_id)
        sampling_point = hasattr(self.start, 'sampling_point') and getattr(self.start, 'sampling_point') or None
        gps_coordinates = hasattr(self.start, 'gps_coordinates') and getattr(self.start, 'gps_coordinates') or None
        sampling_responsible_id = None
        if hasattr(self.start, 'sampling_responsible'):
            if getattr(self.start, 'sampling_responsible'):
                sampling_responsible_id = getattr(self.start, 'sampling_responsible').id
...
class ProjectWaterSampling(Report):
    __doc__ = 'Project Water Sampling report'
    ...
    def get_context(cls, records, data):
        ...
        report_context = super(ProjectWaterSampling, cls).get_context(records, data)
        project = Project(data['id'])
        entry = Entry.search([
         (
          'project', '=', project.id)],
        ...
        if entry:
            ...
            report_context['contact_results'] = ', '.join([r.name for r in cls.get_contact_results(project.id)])
            report_context['contact_invoice'] = ', '.join([r.name for r in cls.get_contact_invoice(project.id)])
        ...
        for fraction in fractions:
            objects.append({'number':fraction.get_formated_number('pt-m-sy-sn-fn'), 
             ...
             'sampling_point':fraction.sample.sampling_point if fraction.sample.sampling_point else '', 
             'gps_coordinates':fraction.sample.gps_coordinates if fraction.sample.gps_coordinates else '', 
             'sampling_responsible':fraction.sample.sampling_responsible if fraction.sample.sampling_responsible else '', 
             'entry_date':fraction.sample.date2, 
             'countersample_location':fraction.countersample_location.code if fraction.countersample_location else '', 
    ...
    def get_results_insitu(cls, fraction_id):
        ...
        slike = '%in situ%'
        cursor.execute('SELECT DISTINCT(a.description) ,  nl.result, p.symbol AS initial_unit FROM "' + NotebookLine._table + '" nl INNER JOIN "' + Service._table + '" s ON nl.service = s.id INNER JOIN "' + Analysis._table + '" a ON a.id = nl.analysis INNER JOIN "' + ProductUom._table + '" p ON nl.initial_unit = p.id WHERE s.fraction = %s AND a.description LIKE %s ORDER BY a.description ASC ', (
         fraction_id, slike))
        res = {}
        res = cursor.fetchall()
        line_result = []
    ...
    def get_contact_results(project_id):
        ...
        cursor.execute('SELECT  rc.contact FROM "' + Entry._table + '" e INNER JOIN "' + Party._table + '" p ON e.party = p.id INNER JOIN "' + EntryReportContact._table + '" rc ON e.id = rc.entry WHERE e.project = %s ', (
         project_id,))
    ...
    def get_contact_invoice(project_id):
        ...
        cursor.execute('SELECT  ic.contact FROM "' + Entry._table + '" e INNER JOIN "' + Party._table + '" p ON e.party = p.id INNER JOIN "' + EntryInvoiceContact._table + '" ic ON e.id = ic.entry WHERE e.project = %s ', (
         project_id,))
        return PartyAddress.search([
         (
          'id', 'in', cursor.fetchall())])