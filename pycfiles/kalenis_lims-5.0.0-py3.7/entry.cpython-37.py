...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/trytond/modules/lims/entry.py
# Compiled at: 2019-01-16 09:41:18
# Size of source mod 2**32: 58464 bytes
import logging
...
from trytond.wizard import Wizard, StateTransition, StateView, StateReport, Button
...
__all__ = [
 'Entry', 'EntryInvoiceContact', 'EntryReportContact',
...
class Entry(Workflow, ModelSQL, ModelView):
    ...
    party = fields.Many2One('party.party', 'Party', required=True, states={'readonly': Eval('state') != 'draft'},
      depends=['state'])
    invoice_party = fields.Many2One('party.party', 'Invoice party', domain=[
     (
      'id', 'in', Eval('invoice_party_domain'))],
      depends=[
     'invoice_party_domain', 'state'],
      required=True,
      states={'readonly': Eval('state') != 'draft'})
    invoice_party_view = fields.Function((fields.Many2One('party.party', 'Invoice party')),
      'get_views_field', searcher='search_views_field')
    invoice_party_domain = fields.Function(fields.Many2Many('party.party', None, None, 'Invoice party domain'), 'on_change_with_invoice_party_domain')
    invoice_contacts = fields.One2Many('lims.entry.invoice_contacts', 'entry', 'Invoice contacts')
    report_contacts = fields.One2Many('lims.entry.report_contacts', 'entry', 'Report contacts')
    acknowledgment_contacts = fields.One2Many('lims.entry.acknowledgment_contacts', 'entry', 'Acknowledgment contacts')
    ...
    no_acknowledgment_of_receipt = fields.Boolean('No acknowledgment of receipt')
    samples = fields.One2Many('lims.sample', 'entry', 'Samples', context={'entry':Eval('id'), 
     'party':Eval('party')},
      depends=[
     'party'])
    ...
    pending_reason = fields.Many2One('lims.entry.suspension.reason', 'Pending reason',
      states={'invisible':Not(Bool(Equal(Eval('state'), 'pending'))), 
    ...
    ack_report_cache = fields.Binary('Acknowledgment report cache', readonly=True,
      file_id='ack_report_cache_id',
      store_prefix='ack_report')
    ack_report_cache_id = fields.Char('Acknowledgment report cache ID', readonly=True)
    ack_report_format = fields.Char('Acknowledgment report format', readonly=True)
    ...
    def __setup__(cls):
        ...
        cls._transitions |= set((('draft', 'ongoing'), ('draft', 'pending'), ('pending', 'ongoing'),
                                 ('ongoing', 'closed')))
        cls._buttons.update({'create_sample':{'invisible': ~Eval('state').in_(['draft'])}, 
         'confirm':{'invisible': ~Eval('state').in_(['draft', 'pending'])}, 
         'on_hold':{'invisible': ~Eval('state').in_(['draft'])}})
        cls._error_messages.update({'no_entry_sequence':'There is no entry sequence for the work year "%s".', 
         'delete_entry':'You can not delete entry "%s" because it is not in draft state', 
         'not_fraction':'You can not confirm entry "%s" because has not fractions', 
         'missing_entry_contacts':'Missing contacts in entry "%s"', 
         'enac_acredited':'The analysis marked with * are not covered by the Accreditation.', 
         'english_report':'Do not forget to load the translations into English'})
    ...
    @fields.depends('party', 'invoice_party', 'invoice_contacts', 'report_contacts', 'acknowledgment_contacts')
    def on_change_party(self):
        ...
        if self.party:
            ...
            if self.party.addresses:
                for c in self.party.addresses:
                    if c.report_contact_default:
                        if c not in a_report_contacts:
                            value = ReportContacts(**ReportContacts.default_get(list(ReportContacts._fields.keys())))
                            value.contact = c
                            report_contacts.append(value)
                        if c.acknowledgment_contact_default and c not in a_acknowledgment_contacts:
                            value = AcknowledgmentContacts(**AcknowledgmentContacts.default_get(list(AcknowledgmentContacts._fields.keys())))
                            value.contact = c
        ...
        if self.party:
            invoice_party_domain = self.invoice_party or self.on_change_with_invoice_party_domain()
        ...
        if self.invoice_party:
            if self.invoice_party.addresses:
                for c in self.invoice_party.addresses:
                    if c.invoice_contact_default and c not in a_invoice_contacts:
                        value = InvoiceContacts(**InvoiceContacts.default_get(list(InvoiceContacts._fields.keys())))
                        value.contact = c
    ...
    def on_change_with_invoice_party_domain(self, name=None):
        ...
        if self.party:
            ...
            if config_.invoice_party_relation_type:
                parties.extend([r.to.id for r in self.party.relations if r.type == config_.invoice_party_relation_type])
        return parties
    ...
    def copy(cls, entries, default=None):
        ...
        for entry in entries:
            ...
            new_entry, = super(Entry, cls).copy([entry], default=current_default)
    ...
    def cron_acknowledgment_of_receipt(cls):
        ...
        """
        logging.getLogger('lims').info('Cron - Acknowledgment Of Receipt (Samples):INIT')
        pool = Pool()
        ForwardAcknowledgmentOfReceipt = pool.get('lims.entry.acknowledgment.forward',
          type='wizard')
        ...
        with Transaction().set_context(active_ids=[entry.id for entry in entries]):
            data = acknowledgment_forward.transition_start()
        if data:
            logging.getLogger('lims').info('data:%s' % data)
        logging.getLogger('lims').info('Cron - Acknowledgment Of Receipt (Samples):END')
    ...
    def on_hold(cls, entries):
        ...
        cls.write(entries, {'state':'pending', 
         'pending_reason':default_pending_reason})
        cls.pending_reason.states['required'] = Bool(Equal(Eval('state'), 'pending'))
    ...
    def check_contacts(self):
        if not (self.invoice_contacts and self.report_contacts and self.acknowledgment_contacts):
            self.raise_user_error('missing_entry_contacts', (self.rec_name,))

    def warn_english_report(self):
        if self.english_report:
            self.raise_user_warning('lims_english_report@%s' % self.number, 'english_report')

    def print_report(self):
        if self.ack_report_cache:
            return
        AcknowledgmentOfReceipt = Pool().get('lims.entry.acknowledgment.report',
          type='report')
        success = False
        try:
            ...
            logging.getLogger('lims').error('Unable to print report Acknowledgment of receipt for Entry:%s' % self.number)
    ...
    def mail_acknowledgment_of_receipt(self):
        if not self.ack_report_cache:
            ...
            return from_addr and to_addrs or None
        subject, body = self.subject_body()
        attachment_data = self.attachment()
        msg = self.create_msg(from_addr, to_addrs, subject, body, attachment_data)
    ...
    def attachment(self):
        data = {'content':self.ack_report_cache, 
         'format':self.ack_report_format, 
         'mimetype':self.ack_report_format == 'pdf' and 'pdf' or 'vnd.oasis.opendocument.text', 
         'filename':str(self.number) + '.' + str(self.ack_report_format), 
    ...
    def create_msg(self, from_addr, to_addrs, subject, body, attachment_data):
        ...
        attachment.add_header('content-disposition', 'attachment', filename=(
         'utf-8', '', attachment_data['filename']))
    ...
    def send_msg(self, from_addr, to_addrs, msg):
        ...
        try:
            ...
            logging.getLogger('lims').error('Unable to deliver mail for entry %s' % self.number)
...
class EntryInvoiceContact(ModelSQL, ModelView):
    __doc__ = 'Entry Invoice Contact'
    __name__ = 'lims.entry.invoice_contacts'
    entry = fields.Many2One('lims.entry', 'Entry', ondelete='CASCADE',
      select=True,
      required=True)
    contact = fields.Many2One('party.address', 'Contact', required=True, domain=[
     (
      'party', 'in',
      [Eval('_parent_entry', {}).get('party'),
       Eval('_parent_entry', {}).get('invoice_party')]),
     ('invoice_contact', '=', True)])


class EntryReportContact(ModelSQL, ModelView):
    __doc__ = 'Entry Report Contact'
...
class EntryAcknowledgmentContact(ModelSQL, ModelView):
    __doc__ = 'Entry Acknowledgment Contact'
...
class EntrySuspensionReason(ModelSQL, ModelView):
    __doc__ = 'Entry Suspension Reason'
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update({'default_suspension_reason': 'There is already a default suspension reason'})
...
class EntryDetailAnalysis(ModelSQL, ModelView):
    __doc__ = 'Entry Detail Analysis'
    __name__ = 'lims.entry.detail.analysis'
    service = fields.Many2One('lims.service', 'Service', required=True, ondelete='CASCADE',
      select=True,
      readonly=True)
    service_view = fields.Function(fields.Many2One('lims.service', 'Service',
      states={'invisible': Not(Bool(Eval('_parent_service')))}), 'on_change_with_service_view')
    create_date2 = fields.Function((fields.DateTime('Create Date')), 'get_create_date2',
      searcher='search_create_date2')
    fraction = fields.Function((fields.Many2One('lims.fraction', 'Fraction')), 'get_service_field',
      searcher='search_service_field')
    sample = fields.Function((fields.Many2One('lims.sample', 'Sample')), 'get_service_field',
      searcher='search_service_field')
    entry = fields.Function((fields.Many2One('lims.entry', 'Entry')), 'get_service_field',
      searcher='search_service_field')
    party = fields.Function((fields.Many2One('party.party', 'Party')), 'get_service_field',
      searcher='search_service_field')
    analysis = fields.Many2One('lims.analysis', 'Analysis', required=True, states={'readonly': True})
    ...
    laboratory = fields.Many2One('lims.laboratory', 'Laboratory', states={'readonly': True})
    method = fields.Many2One('lims.lab.method', 'Method', states={'readonly': True})
    device = fields.Many2One('lims.lab.device', 'Device', states={'readonly': True})
    analysis_origin = fields.Char('Analysis origin', states={'readonly': True})
    confirmation_date = fields.Date('Confirmation date', readonly=True)
    report_grouper = fields.Integer('Report Grouper')
    results_report = fields.Function(fields.Many2One('lims.results_report', 'Results Report'), 'get_results_report')
    ...
    cie_fraction_type = fields.Function(fields.Boolean('Blind Sample'), 'get_cie_fraction_type')

    @classmethod
    def __setup__(cls):
        super(EntryDetailAnalysis, cls).__setup__()
        cls._order.insert(0, ('service', 'DESC'))
        cls._error_messages.update({'delete_detail': 'You can not delete the analysis detail because its fraction is confirmed'})
    ...
    def copy(cls, details, default=None):
        ...
        return super(EntryDetailAnalysis, cls).copy(details, default=current_default)
    ...
    def create_notebook_lines(cls, details, fraction):
        ...
        lines_create = []
        for detail in details:
            cursor.execute('SELECT default_repetitions, initial_concentration, final_concentration, start_uom, end_uom, detection_limit, quantification_limit, calc_decimals, report FROM "' + Typification._table + '" WHERE product_type = %s AND matrix = %s AND analysis = %s AND method = %s AND valid', (
            ...
            if typification:
                ...
                decimals = 2
                report = False
            cursor.execute('SELECT results_estimated_waiting FROM "' + Method._table + '" WHERE id = %s', (
             detail.method.id,))
            res = cursor.fetchone()
            results_estimated_waiting = res and res[0] or None
            cursor.execute('SELECT department FROM "' + AnalysisLaboratory._table + '" WHERE analysis = %s AND laboratory = %s', (
             detail.analysis.id, detail.laboratory.id))
            res = cursor.fetchone()
            department = res and res[0] or None
            for i in range(0, repetitions + 1):
                notebook_line = {'analysis_detail':detail.id, 
        ...
        companies = lines_create or Company.search([])
        ...
        with Transaction().set_user(0):
            ...
            Notebook.write(notebook, {'lines': [('create', lines_create)]})
    ...
    def _order_service_field(name):
        ...
        def order_field(tables):
            ...
            if service_tables is None:
                ...
                service_tables = {None: (service, service.id == table.service)}
    ...
    def get_results_report(cls, details, name):
        ...
        result = {}
        for d in details:
            cursor.execute('SELECT results_report FROM "' + NotebookLine._table + '" WHERE analysis_detail = %s AND results_report IS NOT NULL ORDER BY id ASC LIMIT 1', (
             d.id,))
    ...
    def get_cie_fraction_type(self, name=None):
        if self.service:
            if self.service.fraction:
                if self.service.fraction.cie_fraction_type:
                    if not self.service.fraction.cie_original_fraction:
                        return True
        return False
    ...
    def write(cls, *args):
        ...
        for details, vals in zip(actions, actions):
            ...
            if change_cie_data:
                for detail in details:
                    if detail.service and detail.service.fraction and detail.service.fraction.confirmed:
    ...
    def update_cie_data(self):
        ...
        if nlines:
            ...
            if blind_samples:
                BlindSample.write(blind_samples, {'min_value':self.cie_min_value, 
                 'max_value':self.cie_max_value})


class ForwardAcknowledgmentOfReceipt(Wizard):
    __doc__ = 'Forward Acknowledgment of Samples Receipt'
    ...
    def transition_start(self):
        ...
        for active_id in Transaction().context['active_ids']:
            ...
            if entry.state != 'ongoing':
                continue
            printable = entry.no_acknowledgment_of_receipt or False
            cie_entry = False
...
class ChangeInvoicePartyStart(ModelView):
    __doc__ = 'Change Invoice Party'
    __name__ = 'lims.entry.change_invoice_party.start'
    invoice_party_domain = fields.Many2Many('party.party', None, None, 'Invoice party domain')
    invoice_party = fields.Many2One('party.party', 'Invoice party', domain=[
     (
      'id', 'in', Eval('invoice_party_domain'))],
      depends=[
     'invoice_party_domain'],
      required=True)
...
class ChangeInvoiceParty(Wizard):
    ...
    error = StateView('lims.entry.change_invoice_party.error', 'lims.lims_change_invoice_party_error_view_form', [
     Button('Cancel', 'end', 'tryton-cancel', default=True)])
    start = StateView('lims.entry.change_invoice_party.start', 'lims.lims_change_invoice_party_start_view_form', [
    ...
    def default_start(self, fields):
        ...
        return {'invoice_party_domain':invoice_party_domain,  'invoice_party':invoice_party}
...
class PrintAcknowledgmentOfReceipt(Wizard):
    __doc__ = 'Print Acknowledgment of Samples Receipt'
        ...
        while len(data_ids) > 0:
            ...
            if entry.state == 'ongoing':
                ...
                for sample in entry.samples:
                    ...
                    for fraction in sample.fractions:
                        if fraction.confirmed and fraction.services:
                            printable = fraction.cie_fraction_type or True
                            break
...
class AcknowledgmentOfReceipt(Report):
    __doc__ = 'Acknowledgment of Samples Receipt'
    ...
    def get_context(cls, records, data):
        ...
        Service = pool.get('lims.service')
        Entry = pool.get('lims.entry')
        report_context = super(AcknowledgmentOfReceipt, cls).get_context(records, data)
        if 'id' in data:
            entry = Entry(data['id'])
        else:
            entry = records[0]
        ...
        samples = []
        record = {'party':entry.party.rec_name, 
         'samples':samples}
        for sample in entry.samples:
            if not sample.fractions:
                continue
            ...
            for fraction in sample.fractions:
                if fraction.confirmed and fraction.services:
                    confirmed = fraction.cie_fraction_type or True
                    break
            ...
            sample_data = {'label':sample.label, 
             'producer':sample.producer.rec_name if sample.producer else '', 
             'date':sample.date, 
            ...
            for service in services_obj:
                if service.analysis.type == 'analysis':
                    if not cls.get_analysis_reportable(sample.product_type, sample.matrix, service.analysis, service.method):
                        continue
                    if service.analysis.id not in services:
                        s_methods = {}
                        services[service.analysis.id] = {'code':service.analysis.code,  'name':service.analysis.description, 
                         'methods':s_methods}
                        if service.analysis.type == 'analysis':
                            s_methods[service.method.id] = {'method':service.method,  'analysis':[],  'enac':False}
                            acredited = cls.get_accreditation(sample.product_type, sample.matrix, service.analysis, service.method)
                            if acredited:
                                s_methods[service.method.id]['enac'] = True
                        else:
                            ia_methods = cls.get_included_analysis(service.analysis.id, service.fraction.id)
                            for ia in ia_methods:
                                if ia['method_id'] not in s_methods:
                                    s_methods[ia['method_id']] = {'method':ia['method'],  'analysis':[],  'enac':False}
                                s_methods[ia['method_id']]['analysis'].append({'acredited':ia['acredited'], 
                                 'analysis':ia['analysis']})
                                if s_methods[ia['method_id']]['enac'] or ia['acredited']:
                                    s_methods[ia['method_id']]['enac'] = True

                        for v in s_methods.values():
                            if v['enac']:
                                v['enac_label'] = Entry.raise_user_error('enac_acredited',
                                  raise_exception=False)
                            else:
                                v['enac_label'] = ''
                            sorted_analysis = sorted((v['analysis']), key=(lambda x: x['analysis'].description))
        ...
        for analysis in included_analysis:
            if analysis.included_analysis.type == 'analysis':
                ...
                if analysis_detail:
                    ...
                    if cls.get_analysis_reportable(analysis_detail.sample.product_type, analysis_detail.sample.matrix, analysis_detail.analysis, analysis_detail.method):
                        childs.append({'method_id':analysis_detail.method.id, 
                         'method':analysis_detail.method, 
                         'analysis':analysis.included_analysis, 
                         'acredited':cls.get_accreditation(analysis_detail.sample.product_type, analysis_detail.sample.matrix, analysis_detail.analysis, analysis_detail.method)})
            childs.extend(cls.get_included_analysis(analysis.included_analysis.id, fraction_id))
