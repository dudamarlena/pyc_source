...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/trytond/modules/lims/sample.py
# Compiled at: 2019-01-16 09:41:14
# Size of source mod 2**32: 193910 bytes
import sys, logging, operator
...
__all__ = [
 'Zone', 'Variety', 'MatrixVariety', 'PackagingIntegrity',
...
class Variety(ModelSQL, ModelView):
    ...
    code = fields.Char('Code', required=True)
    description = fields.Char('Description', required=True)
    varieties = fields.One2Many('lims.matrix.variety', 'variety', 'Product Type - Matrix')
...
class MatrixVariety(ModelSQL, ModelView):
    __doc__ = 'Product Type - Matrix - Variety'
...
class FractionType(ModelSQL, ModelView):
    ...
    default_package_type = fields.Many2One('lims.packaging.type', 'Default Package type')
    default_fraction_state = fields.Many2One('lims.packaging.integrity', 'Default Fraction state')
...
class Service(ModelSQL, ModelView):
    ...
    fraction = fields.Many2One('lims.fraction', 'Fraction', required=True, ondelete='CASCADE',
      select=True,
      depends=['number'],
      states={'readonly': Or(Bool(Eval('number')), Bool(Eval('context', {}).get('readonly', True)))})
    fraction_view = fields.Function(fields.Many2One('lims.fraction', 'Fraction',
      states={'invisible': Not(Bool(Eval('_parent_fraction')))}), 'on_change_with_fraction_view')
    sample = fields.Function((fields.Many2One('lims.sample', 'Sample')), 'get_fraction_field',
      searcher='search_fraction_field')
    entry = fields.Function((fields.Many2One('lims.entry', 'Entry')), 'get_fraction_field',
      searcher='search_fraction_field')
    party = fields.Function((fields.Many2One('party.party', 'Party')), 'get_fraction_field',
      searcher='search_fraction_field')
    analysis = fields.Many2One('lims.analysis', 'Analysis/Set/Group', required=True,
    ...
    analysis_view = fields.Function((fields.Many2One('lims.analysis', 'Analysis/Set/Group')),
    ...
    typification_domain = fields.Function(fields.Many2Many('lims.typification', None, None, 'Typification domain'), 'on_change_with_typification_domain')
    analysis_type = fields.Function(fields.Selection([
     ...
     ('group', 'Group')],
      ...
      searcher='search_analysis_field')
    urgent = fields.Boolean('Urgent', states={'readonly': Bool(Eval('context', {}).get('readonly', True))})
    priority = fields.Integer('Priority', states={'readonly': Bool(Eval('context', {}).get('readonly', True))})
    ...
    laboratory = fields.Many2One('lims.laboratory', 'Laboratory', domain=[
     ...
     'readonly':Bool(Eval('context', {}).get('readonly', True))},
      depends=[
     'laboratory_domain'])
    laboratory_view = fields.Function(fields.Many2One('lims.laboratory', 'Laboratory'), 'get_views_field')
    ...
    method = fields.Many2One('lims.lab.method', 'Method', domain=[
     'OR', ('id', '=', Eval('method')),
     (
    ...
    device = fields.Many2One('lims.lab.device', 'Device', domain=[
     (
     ...
     'readonly':Bool(Eval('context', {}).get('readonly', True))},
      depends=[
     'device_domain'])
    device_view = fields.Function(fields.Many2One('lims.lab.device', 'Device'), 'get_views_field')
    device_domain = fields.Function(fields.Many2Many('lims.lab.device', None, None, 'Device domain'), 'on_change_with_device_domain')
    comments = fields.Text('Comments', states={'readonly': Bool(Eval('context', {}).get('readonly', True))})
    analysis_detail = fields.One2Many('lims.entry.detail.analysis', 'service', 'Analysis detail')
    confirmed = fields.Function((fields.Boolean('Confirmed')), 'get_confirmed', searcher='search_confirmed')
    confirmation_date = fields.Date('Confirmation date', readonly=True)
    divide = fields.Boolean('Divide')
    has_results_report = fields.Function(fields.Boolean('Results Report'), 'get_has_results_report')
    manage_service_available = fields.Function(fields.Boolean('Available for Manage services'), 'get_manage_service_available')
    icon = fields.Function(fields.Char('Icon'), 'get_icon')
    planned = fields.Function((fields.Boolean('Planned')), 'get_planned', searcher='search_planned')

    @classmethod
    def __setup__(cls):
        super(Service, cls).__setup__()
        cls._order.insert(0, ('number', 'DESC'))
        cls._error_messages.update({'no_service_sequence':'There is no service sequence for the work year "%s".', 
         'delete_service':'You can not delete service "%s" because its fraction is confirmed', 
         'duplicated_analysis':'The analysis "%(analysis)s" is assigned more than once to the fraction "%(fraction)s"'})
        ...
        if services:
            ...
            for service in services:
                if service.analysis:
                    ...
                    analysis_ids.extend(Analysis.get_included_analysis(service.analysis.id))

            new_analysis_ids = [
             self.analysis.id]
            new_analysis_ids.extend(Analysis.get_included_analysis(self.analysis.id))
            for a_id in new_analysis_ids:
                if a_id in analysis_ids:
                    analysis = Analysis(a_id)
                    self.raise_user_error('duplicated_analysis', {'analysis':analysis.rec_name, 
    ...
    def create(cls, vlist):
        ...
        Transaction().context.get('copying', False) or cls.update_analysis_detail(services)
        ...
        if aditional_services:
            if Transaction().context.get('manage_service', False):
                ...
                if analysis_detail:
                    ...
                    EntryDetailAnalysis.create_notebook_lines(analysis_detail, fraction)
                    EntryDetailAnalysis.write(analysis_detail, {'state': 'unplanned'})
        ...
        for service in services:
            ...
            if to_delete:
                ...
                if service.analysis.behavior == 'additional':
                    continue
                to_create = []
                service_context = {'product_type':service.fraction.product_type.id,  'matrix':service.fraction.matrix.id}
                analysis_data = []
                if service.analysis.type == 'analysis':
                    ...
                    analysis_data.append({'id':service.analysis.id, 
                     'origin':service.analysis.code, 
                     'laboratory':laboratory_id, 
                     'method':method_id, 
                     'device':device_id})
                else:
                    analysis_data.extend(Service._get_included_analysis(service.analysis, service.analysis.code, service_context))
            if analysis_data:
    ...
    @staticmethod
    def _get_included_analysis(analysis, analysis_origin='', service_context=None):
        Typification = Pool().get('lims.typification')
        childs = []
        if analysis.included_analysis:
            for included in analysis.included_analysis:
                if analysis.type == 'set' and included.included_analysis.type == 'analysis':
                    origin = analysis_origin
                else:
                    origin = analysis_origin + ' > ' + included.included_analysis.code
                if included.included_analysis.type == 'analysis':
                    ...
                    typifications = Typification.search([
                     ...
                     ('by_default', '=', True),
                     ('valid', '=', True)])
                    method_id = typifications[0].method.id if typifications else None
                    device_id = None
                    if included.included_analysis.devices:
                        for d in included.included_analysis.devices:
                            if d.laboratory.id == laboratory_id and d.by_default is True:
                                device_id = d.device.id

                    childs.append({'id':included.included_analysis.id, 
                     'origin':origin, 
                     'laboratory':laboratory_id, 
                     'method':method_id, 
                     'device':device_id})
                childs.extend(Service._get_included_analysis(included.included_analysis, origin, service_context))
        ...
        for service in services:
            ...
            for detail in entry_details:
                ...
                if typification.additional:
                    ...
                    if typification.additional.id not in aditional_services[service.fraction.id]:
                        aditional_services[service.fraction.id][typification.additional.id] = {'laboratory':None, 
                         'method':None, 
                         'device':None}
                    if typification.additionals:
                        ...
                        for additional in typification.additionals:
                            if additional.id not in aditional_services[service.fraction.id]:
                                cursor.execute('SELECT laboratory FROM "' + AnalysisLaboratory._table + '" WHERE analysis = %s', (
                                 additional.id,))
                                res = cursor.fetchone()
                                laboratory_id = res and res[0] or None
                                cursor.execute('SELECT method FROM "' + Typification._table + '" WHERE product_type = %s AND matrix = %s AND analysis = %s AND valid IS TRUE AND by_default IS TRUE', (
                                 service.fraction.product_type.id,
                                 service.fraction.matrix.id, additional.id))
                                res = cursor.fetchone()
                                method_id = res and res[0] or None
                                cursor.execute('SELECT device FROM "' + AnalysisDevice._table + '" WHERE analysis = %s AND laboratory = %s AND by_default IS TRUE', (
                                 additional.id, laboratory_id))
                                res = cursor.fetchone()
                                device_id = res and res[0] or None
                                aditional_services[service.fraction.id][additional.id] = {'laboratory':laboratory_id, 
                                 'method':method_id, 
                                 'device':device_id}
                    ...
                    if not Service.search([
                     ...
                     (
                      'analysis', '=', analysis_id)]):
                        services_default.append({'fraction':fraction_id, 
                         'analysis':analysis_id, 
    ...
    def _get_analysis_included_labs(cls, analysis):
        ...
        if analysis.included_analysis:
            for included in analysis.included_analysis:
                ...
                childs.extend(cls._get_analysis_included_labs(included.included_analysis))
    ...
    def copy(cls, services, default=None):
        ...
        for service in sorted(services, key=(lambda x: x.number)):
            with Transaction().set_context(copying=True):
                new_service, = super(Service, cls).copy([service], default=current_default)
            detail_default['service'] = new_service.id
            EntryDetailAnalysis.copy((service.analysis_detail), default=detail_default)
    ...
    def set_confirmation_date(services, confirmation_date=None):
        ...
        Service.write(services, {'confirmation_date': confirmation_date})
        analysis_details = EntryDetailAnalysis.search([
         (
          'service', 'in', [s.id for s in services])])
        if analysis_details:
            EntryDetailAnalysis.write(analysis_details, {'confirmation_date': confirmation_date})

    @fields.depends('analysis', 'fraction', 'typification_domain', 'laboratory')
    def on_change_analysis(self):
        ...
        if self.analysis:
            ...
            if len(methods) == 1:
                method = methods[0]
            devices = self._on_change_with_device_domain(self.analysis, Laboratory(laboratory), True)
            if len(devices) == 1:
                device = devices[0]
    ...
    def on_change_laboratory(self):
        ...
        if self.analysis:
            if self.laboratory:
                devices = self._on_change_with_device_domain(self.analysis, self.laboratory, True)
                if len(devices) == 1:
                    device = devices[0]
        self.device = device
    ...
    def on_change_with_laboratory_domain(self, name=None):
        ...
        if not self.analysis:
            ...
            cursor.execute('SELECT DISTINCT(laboratory) FROM "' + AnalysisLaboratory._table + '" WHERE analysis = %s', (
             self.analysis.id,))
            res = cursor.fetchall()
            return res or []
    ...
    def on_change_with_method_domain(self, name=None):
        ...
        if not self.analysis:
            ...
            typification_ids = ', '.join((str(t) for t in self.on_change_with_typification_domain()))
            if not typification_ids:
                return []
            cursor.execute('SELECT DISTINCT(method) FROM "' + Typification._table + '" WHERE id IN (' + typification_ids + ') AND analysis = %s', (
             self.analysis.id,))
            res = cursor.fetchall()
            return res or []
        return [x[0] for x in res]
    ...
    def on_change_with_device_domain(self, name=None):
        return self._on_change_with_device_domain(self.analysis, self.laboratory)

    @staticmethod
    def _on_change_with_device_domain(analysis=None, laboratory=None, by_default=False):
        cursor = Transaction().connection.cursor()
        AnalysisDevice = Pool().get('lims.analysis.device')
        if analysis:
            return laboratory or []
        else:
            if by_default:
                by_default_clause = 'AND by_default = TRUE'
            else:
                by_default_clause = ''
            cursor.execute('SELECT DISTINCT(device) FROM "' + AnalysisDevice._table + '" WHERE analysis = %s  AND laboratory = %s ' + by_default_clause, (
    ...
    def _order_analysis_field(name):
        ...
        def order_field(tables):
            ...
            if analysis_tables is None:
                ...
                analysis_tables = {None: (analysis, analysis.id == table.analysis)}
    ...
    def _order_fraction_field(name):
        ...
        def order_field(tables):
            ...
            if fraction_tables is None:
                ...
                fraction_tables = {None: (fraction, fraction.id == table.fraction)}
    ...
    def get_has_results_report(cls, services, names):
        ...
        for name in names:
            result[name] = {}
            for s in services:
                cursor.execute('SELECT service FROM "' + NotebookLine._table + '" WHERE service = %s AND results_report IS NOT NULL', (
                 s.id,))
                value = False
                if cursor.fetchone():
                    value = True
    ...
    def get_icon(self, name):
        if self.has_results_report:
            ...
            return self.confirmed or 'lims-red'

    def get_planned(self, name):
        if not self.analysis_detail:
            return False
        for ad in self.analysis_detail:
            if ad.state in ('draft', 'unplanned') and ad.analysis.behavior != 'internal_relation':
                return False
    ...
    def search_planned(cls, name, clause):
        ...
        if (op, operand) in (('=', True), ('!=', False)):
            return [
             (
              'id', 'not in', not_planned_ids)]
        if (
         op, operand) in (('=', False), ('!=', True)):
            return [
             (
              'id', 'in', not_planned_ids)]
        return []
...
class Fraction(ModelSQL, ModelView):
    ...
    create_date2 = fields.Function((fields.DateTime('Create Date')), 'get_create_date2',
      searcher='search_create_date2')
    sample = fields.Many2One('lims.sample', 'Sample', required=True, ondelete='CASCADE',
      select=True,
      depends=['number'],
      states={'readonly': Bool(Eval('number'))})
    sample_view = fields.Function(fields.Many2One('lims.sample', 'Sample', states={'invisible': Not(Bool(Eval('_parent_sample')))}), 'on_change_with_sample_view')
    entry = fields.Function((fields.Many2One('lims.entry', 'Entry')), 'get_sample_field',
      searcher='search_sample_field')
    party = fields.Function((fields.Many2One('party.party', 'Party')), 'get_sample_field',
      searcher='search_sample_field')
    label = fields.Function((fields.Char('Label')), 'get_sample_field', searcher='search_sample_field')
    type = fields.Many2One('lims.fraction.type', 'Fraction type', required=True)
    ...
    weight_uom = fields.Many2One('product.uom', 'Weight UoM', domain=[
     ('category.lims_only_available', '=', True)])
    packages_quantity = fields.Integer('Packages quantity', required=True)
    package_type = fields.Many2One('lims.packaging.type', 'Package type', required=True)
    size = fields.Float('Size')
    size_uom = fields.Many2One('product.uom', 'Size UoM', domain=[
     ('category.lims_only_available', '=', True)])
    expiry_date = fields.Date('Expiry date', states={'readonly': True})
    discharge_date = fields.Date('Discharge date')
    countersample_location = fields.Many2One('stock.location', 'Countersample location',
      readonly=True)
    countersample_date = fields.Date('Countersample date', readonly=True)
    fraction_state = fields.Many2One('lims.packaging.integrity', 'Fraction state',
      required=True)
    services = fields.One2Many('lims.service', 'fraction', 'Services', states={'readonly': Bool(Eval('button_manage_services_available'))},
      context={'analysis_domain':Eval('analysis_domain'), 
     'typification_domain':Eval('typification_domain'), 
     'product_type':Eval('product_type'), 
     'matrix':Eval('matrix'),  'fraction':Eval('id'), 
     'sample':Eval('sample'),  'entry':Eval('entry'), 
     'party':Eval('party'),  'readonly':False},
      depends=[
     'button_manage_services_available', 'analysis_domain',
    ...
    analysis_domain = fields.Function(fields.Many2Many('lims.analysis', None, None, 'Analysis domain'), 'on_change_with_analysis_domain')
    typification_domain = fields.Function(fields.Many2Many('lims.typification', None, None, 'Typification domain'), 'on_change_with_typification_domain')
    product_type = fields.Function(fields.Many2One('lims.product.type', 'Product type'), 'on_change_with_product_type')
    matrix = fields.Function(fields.Many2One('lims.matrix', 'Matrix'), 'on_change_with_matrix')
    button_manage_services_available = fields.Function(fields.Boolean('Button manage services available'), 'on_change_with_button_manage_services_available')
    confirmed = fields.Boolean('Confirmed')
    button_confirm_available = fields.Function(fields.Boolean('Button confirm available'), 'on_change_with_button_confirm_available')
    current_location = fields.Function((fields.Many2One('stock.location', 'Current Location')),
      'get_current_location', searcher='search_current_location')
    duplicated_analysis_message = fields.Text('Message', readonly=True, states={'invisible': Not(Bool(Eval('duplicated_analysis_message')))})
    has_results_report = fields.Function((fields.Boolean('Results Report')), 'get_has_results_report',
      searcher='search_has_results_report')
    has_all_results_reported = fields.Function(fields.Boolean('All results reported'), 'get_has_all_results_reported')
    ...
    entry_state = fields.Function((fields.Selection([
     ...
     ('closed', 'Closed')], 'Entry State')),
      'get_entry_state', searcher='search_entry_state')
    icon = fields.Function(fields.Char('Icon'), 'get_icon')
    special_type = fields.Function((fields.Char('Fraction type')), 'on_change_with_special_type',
    ...
    con_type = fields.Selection([
     ...
     ('itl', 'ITL')],
      'Type',
      sort=False)
    con_original_fraction = fields.Many2One('lims.fraction', 'Original fraction')
    ...
    bmz_original_fraction = fields.Many2One('lims.fraction', 'Original fraction')
    ...
    rm_original_fraction = fields.Many2One('lims.fraction', 'Original fraction')
    bre_product_type = fields.Many2One('lims.product.type', 'Product type')
    bre_matrix = fields.Many2One('lims.matrix', 'Matrix')
    bre_reagents = fields.One2Many('lims.fraction.reagent', 'fraction', 'Reagents')
    mrt_product_type = fields.Many2One('lims.product.type', 'Product type')
    mrt_matrix = fields.Many2One('lims.matrix', 'Matrix')
    cie_fraction_type_available = fields.Function(fields.Boolean('Available for Blind Sample'), 'on_change_with_cie_fraction_type_available')
    cie_fraction_type = fields.Boolean('Blind Sample', states={'readonly': Not(Bool(Eval('cie_fraction_type_available')))},
      depends=[
     'cie_fraction_type_available'])
    cie_original_fraction = fields.Many2One('lims.fraction', 'Original fraction',
      states={'readonly': Bool(Eval('confirmed'))}, depends=[
     'confirmed'])
    ...
    def __setup__(cls):
        ...
        cls._order.insert(0, ('number', 'DESC'))
        cls._buttons.update({'manage_services':{'invisible': ~Eval('button_manage_services_available')}, 
         'complete_services':{'invisible': ~Eval('button_manage_services_available')}, 
         'confirm':{'invisible': ~Eval('button_confirm_available')}, 
         'load_services':{'invisible': Or(Bool(Eval('button_manage_services_available')), Bool(Eval('services')))}})
        cls._error_messages.update({'missing_fraction_product':'Missing "Fraction product" on Lims configuration', 
         'delete_fraction':'You can not delete fraction "%s" because it is confirmed', 
         'duplicated_analysis':'The analysis "%s" is assigned more than once', 
         'not_services':'You can not confirm fraction "%s" because has not services', 
         'not_divided':'You can not confirm fraction because is not yet divided'})
    ...
    def create(cls, vlist):
        ...
        f_count = {}
        for values in vlist:
            if values['sample'] not in f_count:
                f_count[values['sample']] = 0
            f_count[values['sample']] += 1
            values['number'] = cls.get_next_number(values['sample'], f_count[values['sample']])
    ...
    def copy(cls, fractions, default=None):
        ...
        with Transaction().set_context(_check_access=False):
            ...
            for fraction in sorted(fractions, key=(lambda x: x.number)):
                ...
                new_fraction, = super(Fraction, cls).copy([fraction], default=current_default)
    ...
    def on_change_with_storage_time(self, name=None):
        if self.type:
            if self.type.max_storage_time:
                return self.type.max_storage_time
        if self.storage_location:
            if self.storage_location.storage_time:
    ...
    def on_change_with_special_type(self, name=None):
        ...
        if self.type:
            ...
            if self.type == config.mcl_fraction_type:
                return 'mcl'
            if self.type == config.con_fraction_type:
                return 'con'
            if self.type == config.bmz_fraction_type:
                return 'bmz'
            if self.type == config.rm_fraction_type:
                return 'rm'
            if self.type == config.bre_fraction_type:
                return 'bre'
            if self.type == config.mrt_fraction_type:
                return 'mrt'
            if self.type == config.coi_fraction_type:
                return 'coi'
            if self.type == config.mrc_fraction_type:
                return 'mrc'
            if self.type == config.sla_fraction_type:
                return 'sla'
            if self.type == config.itc_fraction_type:
                return 'itc'
            if self.type == config.itl_fraction_type:
                return 'itl'
        return ''
    ...
    def search_special_type(cls, name, clause):
        ...
        if types:
            ...
            if clause[1] in ('=', '!='):
                return [
                 (
                  'type', clause[1], res_type[0])]
            if clause[1] in ('in', 'not in'):
                return [
                 (
                  'type', clause[1], res_type)]
        return []
    ...
    @fields.depends('sample', 'special_type', 'con_original_fraction', 'services', 'create_date')
    ...
    @classmethod
    def get_sample_field--- This code section failed: ---

 L.1706         0  BUILD_MAP_0           0 
                2  STORE_FAST               'result'

 L.1707         4  SETUP_LOOP          188  'to 188'
                6  LOAD_FAST                'names'
                8  GET_ITER         
               10  FOR_ITER            186  'to 186'
               12  STORE_FAST               'name'

 L.1708        14  BUILD_MAP_0           0 
               16  LOAD_FAST                'result'
               18  LOAD_FAST                'name'
               20  STORE_SUBSCR     

 L.1709        22  LOAD_FAST                'name'
               24  LOAD_CONST               ('label', 'size')
               26  COMPARE_OP               in
               28  POP_JUMP_IF_FALSE    70  'to 70'

 L.1710        30  SETUP_LOOP          184  'to 184'
               32  LOAD_FAST                'fractions'
               34  GET_ITER         
               36  FOR_ITER             66  'to 66'
               38  STORE_FAST               'f'

 L.1711        40  LOAD_GLOBAL              getattr
               42  LOAD_FAST                'f'
               44  LOAD_ATTR                sample
               46  LOAD_FAST                'name'
               48  LOAD_CONST               None
 ...
 L.1712        70  LOAD_FAST                'name'
               72  LOAD_STR                 'fraction_state'
               74  COMPARE_OP               ==
               76  POP_JUMP_IF_FALSE   132  'to 132'

 L.1713        78  SETUP_LOOP          184  'to 184'
               80  LOAD_FAST                'fractions'
               82  GET_ITER         
               84  FOR_ITER            128  'to 128'
               86  STORE_FAST               'f'

 L.1714        88  LOAD_GLOBAL              getattr
               90  LOAD_FAST                'f'
               92  LOAD_ATTR                sample
               94  LOAD_STR                 'package_state'
               96  LOAD_CONST               None
               98  CALL_FUNCTION_3       3  '3 positional arguments'
              100  STORE_FAST               'field'

 L.1715       102  LOAD_FAST                'field'
 ...
 L.1717       132  SETUP_LOOP          184  'to 184'
              134  LOAD_FAST                'fractions'
              136  GET_ITER         
              138  FOR_ITER            182  'to 182'
              140  STORE_FAST               'f'

 L.1718       142  LOAD_GLOBAL              getattr
              144  LOAD_FAST                'f'
              146  LOAD_ATTR                sample
              148  LOAD_FAST                'name'
              150  LOAD_CONST               None
              152  CALL_FUNCTION_3       3  '3 positional arguments'
              154  STORE_FAST               'field'

 L.1719       156  LOAD_FAST                'field'
...
Parse error at or near `COME_FROM_LOOP' instruction at offset 184_1
    ...
    def on_change_with_cie_fraction_type_available(self, name=None):
        if not self.confirmed:
            if self.type:
                if self.type.cie_fraction_type:
                    return True
        return False
    ...
    def on_change_with_button_confirm_available(self, name=None):
        if not self.confirmed:
            if self.sample:
                if self.sample.entry:
                    if self.sample.entry.state == 'ongoing':
                        return True
        return False
    ...
    def create_laboratory_notebook(self):
        ...
        with Transaction().set_user(0):
            notebook = Notebook(fraction=(self.id))
            notebook.save()
        ...
        if self.services:
            ...
            for service in self.services:
                if service.analysis:
                    new_analysis_ids = [
                     service.analysis.id]
                    new_analysis_ids.extend(Analysis.get_included_analysis(service.analysis.id))
                    for a_id in new_analysis_ids:
                        if a_id in analysis_ids:
                            analysis = Analysis(a_id)
                            self.duplicated_analysis_message = self.raise_user_error('duplicated_analysis', (
                             analysis.rec_name,),
    ...
    def on_change_with_expiry_date(self, name=None):
        if self.countersample_date:
            return self.countersample_date + relativedelta(months=(self.storage_time))
    ...
    def get_current_location(cls, fractions, name=None):
        ...
        result = {}
        for f in fractions:
            cursor.execute('SELECT to_location FROM "' + Move._table + '" WHERE fraction = %s AND state IN (\'assigned\', \'done\') ORDER BY effective_date DESC, id DESC LIMIT 1', (
             f.id,))
            ...
            if domain and domain[1] == 'ilike':
                ...
                if not locations:
                    ...
                    if not locations:
                        return []
                domain = (
                 'current_location', 'in', [l.id for l in locations])
        cursor.execute('SELECT f.id, last_move.to_location FROM "' + Fraction._table + '" f INNER JOIN ( SELECT DISTINCT ON (fraction) fraction, to_location FROM "' + Move._table + '" WHERE state IN (\'assigned\', \'done\') ORDER BY fraction, effective_date DESC, id DESC ) last_move ON f.id = last_move.fraction')
        processed_lines = [{'fraction':x[0],  'current_location':x[1]} for x in cursor.fetchall()]
        record_ids = [line['fraction'] for line in processed_lines if _search_current_location_eval_domain(line, domain)]
    ...
    def _order_sample_field(name):
        ...
        def order_field(tables):
            ...
            if sample_tables is None:
                ...
                sample_tables = {None: (sample, sample.id == table.sample)}
    ...
    def get_has_results_report(cls, fractions, names):
        ...
        for name in names:
            result[name] = {}
            for f in fractions:
                cursor.execute('SELECT s.fraction FROM "' + Service._table + '" s INNER JOIN "' + NotebookLine._table + '" nl ON s.id = nl.service WHERE s.fraction = %s AND nl.results_report IS NOT NULL', (
                 f.id,))
                value = False
                if cursor.fetchone():
                    value = True
    ...
    def search_has_results_report(cls, name, clause):
        ...
        Service = pool.get('lims.service')
        NotebookLine = pool.get('lims.notebook.line')
        cursor.execute('SELECT DISTINCT(s.fraction) FROM "' + Service._table + '" s INNER JOIN "' + NotebookLine._table + '" nl ON s.id = nl.service WHERE nl.results_report IS NOT NULL')
        has_results_report = [x[0] for x in cursor.fetchall()]
        field, op, operand = clause
        if (op, operand) in (('=', True), ('!=', False)):
            return [
             (
              'id', 'in', has_results_report)]
        if (
         op, operand) in (('=', False), ('!=', True)):
            return [
             (
              'id', 'not in', has_results_report)]
        return []
    ...
    def get_has_all_results_reported(self, name=None):
        ...
        for nl in notebook_lines:
            if not nl.accepted:
                return False
                return nl.results_report or False
    ...
    def get_formated_number(self, format):
        ...
        if len(number_parts2) < 2:
            ...
            fraction_number = number_parts2[1]
        if format == 'sn-sy-fn':
            formated_number = sample_number + '-' + sample_year + '-' + fraction_number
        else:
            if format == 'sy-sn-fn':
                formated_number = sample_year + '-' + sample_number + '-' + fraction_number
            else:
                if format == 'pt-m-sn-sy-fn':
                    formated_number = self.product_type.code + '-' + self.matrix.code + '-' + sample_number + '-' + sample_year + '-' + fraction_number
                else:
                    if format == 'pt-m-sy-sn-fn':
                        formated_number = self.product_type.code + '-' + self.matrix.code + '-' + sample_year + '-' + sample_number + '-' + fraction_number
    ...
    def get_stored_fractions():
        ...
        Move = pool.get('stock.move')
        Location = pool.get('stock.location')
        cursor.execute('SELECT DISTINCT ON (m.fraction) m.fraction, l.type FROM "' + Move._table + '" m INNER JOIN "' + Location._table + '" l ON m.to_location = l.id WHERE m.fraction IS NOT NULL AND m.state IN (\'assigned\', \'done\') ORDER BY m.fraction ASC, m.effective_date DESC, m.id DESC')
...
class Sample(ModelSQL, ModelView):
    ...
    date2 = fields.Function((fields.Date('Date')), 'get_date', searcher='search_date')
    entry = fields.Many2One('lims.entry', 'Entry', required=True, ondelete='CASCADE',
      select=True,
      depends=['number'],
      states={'readonly': Bool(Eval('number'))})
    entry_view = fields.Function(fields.Many2One('lims.entry', 'Entry', states={'invisible': Not(Bool(Eval('_parent_entry')))}), 'on_change_with_entry_view')
    party = fields.Function((fields.Many2One('party.party', 'Party')), 'get_entry_field',
      searcher='search_entry_field')
    producer = fields.Many2One('lims.sample.producer', 'Producer company', domain=[
     (
      'party', '=', Eval('party'))],
      depends=['party'])
    label = fields.Char('Label', translate=True)
    sample_client_description = fields.Char('Product described by the client', translate=True)
    product_type = fields.Many2One('lims.product.type', 'Product type', states={'readonly': Bool(Eval('product_type_matrix_readonly'))},
      required=True,
      domain=[
     (
      'id', 'in', Eval('product_type_domain'))],
      depends=[
     'product_type_domain', 'product_type_matrix_readonly'])
    product_type_view = fields.Function((fields.Many2One('lims.product.type', 'Product type')),
      'get_views_field', searcher='search_views_field')
    ...
    matrix = fields.Many2One('lims.matrix', 'Matrix', required=True, states={'readonly': Bool(Eval('product_type_matrix_readonly'))},
      domain=[
     (
      'id', 'in', Eval('matrix_domain'))],
      depends=[
     'matrix_domain', 'product_type_matrix_readonly'])
    matrix_view = fields.Function((fields.Many2One('lims.matrix', 'Matrix')),
      'get_views_field', searcher='search_views_field')
    matrix_domain = fields.Function(fields.Many2Many('lims.matrix', None, None, 'Matrix domain'), 'on_change_with_matrix_domain')
    product_type_matrix_readonly = fields.Function(fields.Boolean('Product type and Matrix readonly'), 'get_product_type_matrix_readonly')
    package_state = fields.Many2One('lims.packaging.integrity', 'Package state')
    package_type = fields.Many2One('lims.packaging.type', 'Package type')
    packages_quantity = fields.Integer('Packages quantity', required=True)
    size = fields.Float('Size')
    size_uom = fields.Many2One('product.uom', 'Size UoM', domain=[
     ('category.lims_only_available', '=', True)])
    restricted_entry = fields.Boolean('Restricted entry', states={'readonly': True})
    zone = fields.Many2One('lims.zone', 'Zone', required=True)
    trace_report = fields.Boolean('Trace report')
    fractions = fields.One2Many('lims.fraction', 'sample', 'Fractions', context={'analysis_domain':Eval('analysis_domain'), 
     'typification_domain':Eval('typification_domain'), 
     'product_type':Eval('product_type'), 
     'matrix':Eval('matrix'),  'sample':Eval('id'), 
     'entry':Eval('entry'),  'party':Eval('party'), 
     'label':Eval('label'),  'package_type':Eval('package_type'), 
     'size':Eval('size'),  'size_uom':Eval('size_uom'), 
     'fraction_state':Eval('package_state')},
      depends=[
     'analysis_domain', 'typification_domain', 'entry',
     'party', 'label'])
    report_comments = fields.Text('Report comments', translate=True)
    comments = fields.Text('Comments')
    variety = fields.Many2One('lims.variety', 'Variety', domain=[
     (
      'varieties.matrix', '=', Eval('matrix'))],
      depends=[
     'matrix'])
    analysis_domain = fields.Function(fields.Many2Many('lims.analysis', None, None, 'Analysis domain'), 'on_change_with_analysis_domain')
    typification_domain = fields.Function(fields.Many2Many('lims.typification', None, None, 'Typification domain'), 'on_change_with_typification_domain')
    ...
    def __setup__(cls):
        ...
        cls._order.insert(0, ('number', 'DESC'))
        cls._error_messages.update({'no_sample_sequence':'There is no sample sequence for the work year "%s".', 
         'duplicated_label':'The label "%s" is already present in another sample', 
         'delete_sample':'You can not delete sample "%s" because its entry is not in draft state'})
    ...
    def copy(cls, samples, default=None):
        ...
        for sample in sorted(samples, key=(lambda x: x.number)):
            new_sample, = super(Sample, cls).copy([sample], default=default)
    ...
    def on_change_with_restricted_entry(self, name=None):
        return self.product_type and self.product_type.restricted_entry and self.matrix and self.matrix.restricted_entry and self.zone and self.zone.restricted_entry
    ...
    def on_change_with_analysis_domain(self, name=None):
        ...
        if self.product_type:
            ...
            return typified_analysis or []
        cursor.execute('SELECT id FROM "' + Analysis._table + '" WHERE type = \'analysis\' AND behavior IN (\'normal\', \'internal_relation\') AND disable_as_individual IS TRUE AND state = \'active\'')
        disabled_analysis = [a[0] for a in cursor.fetchall()]
        if disabled_analysis:
            typified_analysis = list(set(typified_analysis) - set(disabled_analysis))
        cursor.execute('SELECT DISTINCT(analysis) FROM "' + CalculatedTypification._table + '" WHERE product_type = %s AND matrix = %s', (
         self.product_type.id, self.matrix.id))
        typified_sets_groups = [a[0] for a in cursor.fetchall()]
        cursor.execute('SELECT id FROM "' + Analysis._table + '" WHERE behavior = \'additional\' AND state = \'active\'')
    ...
    def on_change_with_typification_domain(self, name=None):
        ...
        if self.product_type:
            ...
            cursor.execute('SELECT id FROM "' + Typification._table + '" WHERE product_type = %s AND matrix = %s AND valid', (
             self.product_type.id, self.matrix.id))
            res = cursor.fetchall()
            return res or []
        return [x[0] for x in res]
    ...
    def order_product_type_view(tables):
        ...
        if product_type_tables is None:
            ...
            product_type_tables = {None: (product_type, product_type.id == table.product_type)}
    ...
    def order_matrix_view(tables):
        ...
        if matrix_tables is None:
            ...
            matrix_tables = {None: (matrix, matrix.id == table.matrix)}
    ...
    def order_party(tables):
        ...
        if entry_tables is None:
            ...
            entry_tables = {None: (entry, entry.id == table.entry)}
    ...
    def get_has_results_report(cls, samples, names):
        ...
        for name in names:
            result[name] = {}
            for s in samples:
                cursor.execute('SELECT f.sample FROM "' + Fraction._table + '" f INNER JOIN "' + Service._table + '" s ON f.id = s.fraction INNER JOIN "' + NotebookLine._table + '" nl ON s.id = nl.service WHERE f.sample = %s AND nl.results_report IS NOT NULL', (
                 s.id,))
                value = False
                if cursor.fetchone():
                    value = True
...
class DuplicateSample(Wizard):
    ...
    start = StateView('lims.sample.duplicate.start', 'lims.lims_duplicate_sample_start_view_form', [
    ...
    def transition_duplicate(self):
        ...
        for label in labels_list:
            Sample.copy([sample], default={'label':label, 
             'date':date})
...
class DuplicateSampleFromEntryStart(ModelView):
    ...
    sample = fields.Many2One('lims.sample', 'Sample', required=True, domain=[
     (
      'entry', '=', Eval('entry'))],
      depends=['entry'])
    date = fields.DateTime('Date', required=True)
...
class DuplicateSampleFromEntry(Wizard):
    ...
    start = StateView('lims.entry.duplicate_sample.start', 'lims.lims_duplicate_sample_from_entry_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Copy', 'duplicate', 'tryton-ok', default=True)])
    duplicate = StateTransition()

    def default_start(self, fields):
        return {'entry': Transaction().context['active_id']}
...
class ManageServices(Wizard):
    __doc__ = 'Manage Services'
    __name__ = 'lims.manage_services'
    start_state = 'check'
    check = StateTransition()
    start = StateView('lims.fraction', 'lims.lims_manage_services_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update({'counter_sample_date': 'Reverse counter sample storage to enter the service '})
    ...
    def default_start(self, fields):
        ...
        if not fraction:
            return {}
        not_planned_services_ids = [s.id for s in fraction.services if s.manage_service_available]
    ...
    def transition_ok(self):
        ...
        fraction = Fraction(Transaction().context['active_id'])
        original_services = [s for s in fraction.services if s.manage_service_available]
            ...
            for actual_service in actual_services:
                if original_service == actual_service:
                    for field in self._get_comparison_fields():
                        if getattr(original_service, field) != getattr(actual_service, field):
                            self.update_service(original_service, actual_service, fraction, field)
                            delete_ack_report_cache = True
                            break
    ...
    def create_service(self, service, fraction):
        ...
        service_create = [
         {'fraction':fraction.id, 
          ...
          'laboratory':service.laboratory.id if service.laboratory else None, 
    ...
    def update_service(self, original_service, actual_service, fraction, field_changed):
        pool = Pool()
        Service = pool.get('lims.service')
        NotebookLine = pool.get('lims.notebook.line')
        EntryDetailAnalysis = pool.get('lims.entry.detail.analysis')
        ...
        update_details = True if field_changed in ('analysis', 'laboratory', 'method',
                                                   'device') else False
        if update_details:
            notebook_lines = NotebookLine.search([
             (
    ...
    def _get_comparison_fields(self):
        return ('analysis', 'laboratory', 'method', 'device', 'urgent', 'priority',
                'report_date', 'comments', 'divide')


class CompleteServices(Wizard):
    __doc__ = 'Complete Services'
    ...
    def complete_analysis_detail(self, service):
        ...
        if to_create:
            ...
            if analysis_detail:
                EntryDetailAnalysis.create_notebook_lines(analysis_detail, service.fraction)


class FractionsByLocationsStart(ModelView):
    __doc__ = 'Fractions by Locations'
...
class FractionsByLocations(Wizard):
    ...
    start = StateView('lims.fractions_by_locations.start', 'lims.fractions_by_locations_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
...
class CountersampleStorageStart(ModelView):
    __doc__ = 'Countersamples Storage'
    ...
    location_origin = fields.Many2One('stock.location', 'Origin Location', required=True,
      domain=[('type', '=', 'storage')])
...
class CountersampleStorageResult(ModelView):
    ...
    location_destination = fields.Many2One('stock.location', 'Destination Location',
      required=True, domain=[
     ('type', '=', 'storage')])
    countersample_date = fields.Date('Storage date', required=True)
    fractions = fields.Many2Many('lims.fraction', None, None, 'Fractions',
      required=True, domain=[
     (
      'id', 'in', Eval('fraction_domain'))],
      depends=[
     'fraction_domain'])
    fraction_domain = fields.One2Many('lims.fraction', None, 'Fractions domain')
...
class CountersampleStorage(Wizard):
    ...
    start = StateView('lims.countersample.storage.start', 'lims.lims_countersample_storage_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Search', 'search', 'tryton-forward', default=True)])
    search = StateTransition()
    empty = StateView('lims.countersample.storage.empty', 'lims.lims_countersample_storage_empty_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Search again', 'start', 'tryton-forward', default=True)])
    result = StateView('lims.countersample.storage.result', 'lims.lims_countersample_storage_result_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update({'reference': 'Countersamples Storage'})

    def default_start(self, fields):
        res = {}
        for field in ('report_date_from', 'report_date_to', 'date_from', 'date_to'):
    ...
    def transition_search(self):
        ...
        if self.start.storage_force is True:
            ...
            if fractions:
                for f in fractions:
                    notebook_lines_ids = self._get_fraction_notebook_lines_storage_force(f.id)
            ...
            if fractions:
                for f in fractions:
                    notebook_lines_ids = self._get_fraction_notebook_lines(f.id)
                    if not notebook_lines_ids:
                        continue
                    notebook_lines = NotebookLine.search([
                     (
                    ...
                    for nl in notebook_lines:
                        ...
                        if not nl.results_report:
                            ...
                            break
                        if not self._get_line_reported(nl, self.start.report_date_from, self.start.report_date_to):
                            all_results_reported = False
                            break
    ...
    def _get_fraction_notebook_lines(self, fraction_id):
        ...
        EntryDetailAnalysis = pool.get('lims.entry.detail.analysis')
        Service = pool.get('lims.service')
        cursor.execute('SELECT nl.id FROM "' + NotebookLine._table + '" nl INNER JOIN "' + EntryDetailAnalysis._table + '" ad ON ad.id = nl.analysis_detail INNER JOIN "' + Service._table + '" srv ON srv.id = nl.service WHERE srv.fraction = %s AND nl.report = TRUE AND nl.annulled = FALSE', (
         fraction_id,))
    ...
    def _get_line_reported(self, nl, date_from, date_to):
        cursor = Transaction().connection.cursor()
        pool = Pool()
        ReportVersionDetailLine = pool.get('lims.results_report.version.detail.line')
        ReportVersionDetail = pool.get('lims.results_report.version.detail')
        ReportVersion = pool.get('lims.results_report.version')
        cursor.execute('SELECT rvdl.id FROM "' + ReportVersionDetailLine._table + '" rvdl INNER JOIN "' + ReportVersionDetail._table + '" rvd ON rvd.id = rvdl.report_version_detail INNER JOIN "' + ReportVersion._table + '" rv ON rv.id = rvd.report_version WHERE rvdl.notebook_line = %s AND rv.results_report = %s AND DATE(COALESCE(rvd.write_date, rvd.create_date)) BETWEEN %s::date AND %s::date', (
    ...
    def transition_storage(self):
        ...
        for fraction in self.result.fractions:
            ...
            fraction.expiry_date = countersample_date + relativedelta(months=(fraction.storage_time))
            fractions_to_save.append(fraction)
    ...
    def _get_internal_shipment(self):
        ...
        shipment.reference = CountersampleStorage.raise_user_error('reference',
          raise_exception=False)
    ...
    def _get_fraction_notebook_lines_storage_force(self, fraction_id):
        ...
        EntryDetailAnalysis = pool.get('lims.entry.detail.analysis')
        Service = pool.get('lims.service')
        cursor.execute('SELECT nl.id FROM "' + NotebookLine._table + '" nl INNER JOIN "' + EntryDetailAnalysis._table + '" ad ON ad.id = nl.analysis_detail INNER JOIN "' + Service._table + '" srv ON srv.id = nl.service WHERE srv.fraction = %s ', (
         fraction_id,))
        return [x[0] for x in cursor.fetchall()]


class CountersampleStorageRevertStart(ModelView):
    __doc__ = 'Revert Countersamples Storage'
...
class CountersampleStorageRevert(Wizard):
    ...
    start = StateView('lims.countersample.storage_revert.start', 'lims.lims_countersample_storage_revert_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Search', 'search', 'tryton-forward', default=True)])
    search = StateTransition()
    empty = StateView('lims.countersample.storage_revert.empty', 'lims.lims_countersample_storage_revert_empty_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Search again', 'start', 'tryton-forward', default=True)])
    result = StateView('lims.countersample.storage_revert.result', 'lims.lims_countersample_storage_revert_result_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update({'reference': 'Countersamples Storage Reversion'})
    ...
    def _get_internal_shipment(self):
        ...
        with Transaction().set_user(0, set_context=True):
            shipment = ShipmentInternal()
        shipment.reference = CountersampleStorageRevert.raise_user_error('reference',
          raise_exception=False)
...
class CountersampleDischargeStart(ModelView):
    __doc__ = 'Countersamples Discharge'
...
class CountersampleDischarge(Wizard):
    ...
    start = StateView('lims.countersample.discharge.start', 'lims.lims_countersample_discharge_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Search', 'search', 'tryton-forward', default=True)])
    search = StateTransition()
    empty = StateView('lims.countersample.discharge.empty', 'lims.lims_countersample_discharge_empty_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Search again', 'start', 'tryton-forward', default=True)])
    result = StateView('lims.countersample.discharge.result', 'lims.lims_countersample_discharge_result_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update({'reference': 'Countersamples Discharge'})

    def default_start(self, fields):
        res = {}
        for field in ('expiry_date_from', 'expiry_date_to', 'date_from', 'date_to'):
            if hasattr(self.start, field) and getattr(self.start, field):
                res[field] = getattr(self.start, field)
    ...
    def _get_internal_shipment(self):
        ...
        with Transaction().set_user(0, set_context=True):
            shipment = ShipmentInternal()
        shipment.reference = CountersampleDischarge.raise_user_error('reference',
          raise_exception=False)
        shipment.planned_date = planned_date
        shipment.planned_start_date = planned_date
        shipment.company = company
...
class FractionDischarge(Wizard):
    ...
    start = StateView('lims.fraction.discharge.start', 'lims.lims_fraction_discharge_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Search', 'search', 'tryton-forward', default=True)])
    search = StateTransition()
    empty = StateView('lims.fraction.discharge.empty', 'lims.lims_fraction_discharge_empty_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Search again', 'start', 'tryton-forward', default=True)])
    result = StateView('lims.fraction.discharge.result', 'lims.lims_fraction_discharge_result_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Discharge', 'discharge', 'tryton-ok', default=True)])
    discharge = StateTransition()
    open = StateAction('stock.act_shipment_internal_form')
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update({'reference': 'Fractions Discharge'})
    ...
    def _get_internal_shipment(self):
        ...
        with Transaction().set_user(0, set_context=True):
            shipment = ShipmentInternal()
        shipment.reference = FractionDischarge.raise_user_error('reference',
          raise_exception=False)
        shipment.planned_date = planned_date
        shipment.planned_start_date = planned_date
        shipment.company = company
...
class FractionDischargeRevertStart(ModelView):
    __doc__ = 'Revert Fractions Discharge'
...
class FractionDischargeRevert(Wizard):
    ...
    start = StateView('lims.fraction.discharge_revert.start', 'lims.lims_fraction_discharge_revert_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Search', 'search', 'tryton-forward', default=True)])
    search = StateTransition()
    empty = StateView('lims.fraction.discharge_revert.empty', 'lims.lims_fraction_discharge_revert_empty_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Search again', 'start', 'tryton-forward', default=True)])
    result = StateView('lims.fraction.discharge_revert.result', 'lims.lims_fraction_discharge_revert_result_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Revert', 'revert', 'tryton-ok', default=True)])
    revert = StateTransition()
    open = StateAction('stock.act_shipment_internal_form')
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update({'reference': 'Fractions Discharge Reversion'})
    ...
    def _get_internal_shipment(self):
        ...
        with Transaction().set_user(0, set_context=True):
            shipment = ShipmentInternal()
        shipment.reference = FractionDischargeRevert.raise_user_error('reference',
          raise_exception=False)
        shipment.planned_date = today
        shipment.planned_start_date = today
        shipment.company = company
...
class CreateSampleStart(ModelView):
    ...
    sample_client_description = fields.Char('Product described by the client',
      required=True)
    sample_client_description_eng = fields.Char('Product described by the client (English)')
    product_type = fields.Many2One('lims.product.type', 'Product type', required=True,
     ...
     (
      'id', 'in', Eval('product_type_domain'))],
      depends=[
     'product_type_domain', 'services'])
    product_type_domain = fields.Many2Many('lims.product.type', None, None, 'Product type domain')
    matrix = fields.Many2One('lims.matrix', 'Matrix', required=True, states={'readonly': Bool(Eval('services'))},
      domain=[
     (
      'id', 'in', Eval('matrix_domain'))],
      depends=[
     'matrix_domain', 'services'])
    matrix_domain = fields.Many2Many('lims.matrix', None, None, 'Matrix domain')
    fraction_state = fields.Many2One('lims.packaging.integrity', 'Package state',
      required=True)
    package_type = fields.Many2One('lims.packaging.type', 'Package type', required=True)
    packages_quantity = fields.Integer('Packages quantity', required=True)
    size = fields.Float('Size')
    ...
    variety = fields.Many2One('lims.variety', 'Variety', domain=[
     ...
     'matrix'])
    labels = fields.Text('Labels')
    fraction_type = fields.Many2One('lims.fraction.type', 'Fraction type', required=True)
    storage_location = fields.Many2One('stock.location', 'Storage location', required=True,
      domain=[('type', '=', 'storage')])
    storage_time = fields.Integer('Storage time (in months)', required=True)
    weight = fields.Float('Weight')
    ...
    shared = fields.Boolean('Shared')
    analysis_domain = fields.Function(fields.Many2Many('lims.analysis', None, None, 'Analysis domain'), 'on_change_with_analysis_domain')
    services = fields.One2Many('lims.create_sample.service', None, 'Services', required=True,
...
class CreateSampleService(ModelView):
    ...
    laboratory = fields.Many2One('lims.laboratory', 'Laboratory', domain=[
     (
      ...
      depends=[
     'laboratory_domain'])
    laboratory_domain = fields.Many2Many('lims.laboratory', None, None, 'Laboratory domain')
    method = fields.Many2One('lims.lab.method', 'Method', domain=[
     (
      'id', 'in', Eval('method_domain'))],
      states={'required': Bool(Eval('method_domain'))},
      depends=[
     'method_domain'])
    method_domain = fields.Many2Many('lims.lab.method', None, None, 'Method domain')
    device = fields.Many2One('lims.lab.device', 'Device', domain=[
     (
      'id', 'in', Eval('device_domain'))],
      states={'required': Bool(Eval('device_domain'))},
      depends=[
     'device_domain'])
    device_domain = fields.Many2Many('lims.lab.device', None, None, 'Device domain')
    ...
    def on_change_analysis(self):
        ...
        if analysis_id:
            ...
            method_domain = self._get_method_domain(analysis_id, product_type_id, matrix_id)
            if len(method_domain) == 1:
                method_id = method_domain[0]
            if laboratory_id:
                device_domain = self._get_device_domain(analysis_id, laboratory_id)
    ...
    def _get_method_domain(analysis_id, product_type_id, matrix_id):
        cursor = Transaction().connection.cursor()
        Typification = Pool().get('lims.typification')
        cursor.execute('SELECT DISTINCT(method) FROM "' + Typification._table + '" WHERE product_type = %s AND matrix = %s AND analysis = %s AND valid', (
    ...
    def _get_device_domain(analysis_id, laboratory_id):
        cursor = Transaction().connection.cursor()
        AnalysisDevice = Pool().get('lims.analysis.device')
        cursor.execute('SELECT DISTINCT(device) FROM "' + AnalysisDevice._table + '" WHERE analysis = %s  AND laboratory = %s AND by_default = TRUE', (
...
class CreateSample(Wizard):
    ...
    start = StateView('lims.create_sample.start', 'lims.lims_create_sample_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
    ...
    def transition_create_(self):
        ...
        if hasattr(self.start, 'sample_client_description_eng'):
            if getattr(self.start, 'sample_client_description_eng'):
                with Transaction().set_context(language='en'):
                    sample_eng = Sample(sample.id)
                    sample_eng.sample_client_description = self.start.sample_client_description_eng
                    sample_eng.save()
        labels_list = self._get_labels_list(self.start.labels)
    ...
    def _get_samples_defaults(self, entry_id):
        producer_id = None
        if hasattr(self.start, 'producer'):
            if getattr(self.start, 'producer'):
                producer_id = getattr(self.start, 'producer').id
        size = hasattr(self.start, 'size') and getattr(self.start, 'size') or None
        size_uom_id = None
        if hasattr(self.start, 'size_uom'):
            if getattr(self.start, 'size_uom'):
                size_uom_id = getattr(self.start, 'size_uom').id
        zone_id = None
        if hasattr(self.start, 'zone'):
            if getattr(self.start, 'zone'):
                zone_id = getattr(self.start, 'zone').id
        restricted_entry = hasattr(self.start, 'restricted_entry') and getattr(self.start, 'restricted_entry') or False
        variety_id = None
        if hasattr(self.start, 'variety'):
            if getattr(self.start, 'variety'):
                variety_id = getattr(self.start, 'variety').id
        weight = hasattr(self.start, 'weight') and getattr(self.start, 'weight') or None
        weight_uom_id = None
        if hasattr(self.start, 'weight_uom'):
            if getattr(self.start, 'weight_uom'):
                weight_uom_id = getattr(self.start, 'weight_uom').id
        shared = hasattr(self.start, 'shared') and getattr(self.start, 'shared') or False
        trace_report = hasattr(self.start, 'trace_report') and getattr(self.start, 'trace_report') or False
        report_comments = hasattr(self.start, 'report_comments') and getattr(self.start, 'report_comments') or None
        comments = hasattr(self.start, 'comments') and getattr(self.start, 'comments') or None
        services_defaults = []
        for service in self.start.services:
            service_defaults = {'analysis':service.analysis.id, 
             'laboratory':service.laboratory.id if service.laboratory else None, 
             'method':service.method.id if service.method else None, 
             'device':service.device.id if service.device else None, 
             'urgent':service.urgent, 
        ...
        labels_list = self._get_labels_list(self.start.labels)
        for label in labels_list[:1]:
            fraction_defaults = {'type':self.start.fraction_type.id, 
             ...
             'services':[
              (
               'create', services_defaults)]}
            sample_defaults = {'entry':entry_id, 
             'date':self.start.date, 
             'producer':producer_id, 
             'sample_client_description':self.start.sample_client_description, 
...
class CountersampleStoragePrintStart(ModelView):
    __doc__ = 'Countersamples Storage Report'
...
class CountersampleStoragePrint(Wizard):
    ...
    start = StateView('lims.countersample.storage.print.start', 'lims.lims_countersample_storage_print_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Print', 'print_', 'tryton-print', default=True)])
    print_ = StateAction('lims.report_countersample_storage')

    def do_print_(self, action):
        data = {'report_date_from':self.start.report_date_from, 
...
class CountersampleStorageReport(Report):
    ...
    def get_context(cls, records, data):
        ...
        Fraction = pool.get('lims.fraction')
        NotebookLine = pool.get('lims.notebook.line')
        report_context = super(CountersampleStorageReport, cls).get_context(records, data)
        ...
        for f in fractions:
            ...
            for nl in notebook_lines:
                ...
                if not nl.results_report:
                    all_results_reported = False
                    break
                if not cls._get_line_reported(nl, data['report_date_from'], data['report_date_to']):
                    all_results_reported = False
                    break
        ...
        for fraction in f_list:
            if fraction.current_location.id not in objects:
                objects[fraction.current_location.id] = {'location':fraction.current_location.rec_name,  'fractions':[]}
            objects[fraction.current_location.id]['fractions'].append({'number':fraction.get_formated_number('pt-m-sy-sn-fn'), 
             'type':fraction.type.code, 
             'packages':'%s %s' % (fraction.packages_quantity or '',
              fraction.package_type.description if fraction.package_type else ''), 
             'entry_date':fraction.sample.date2, 
             'results_reports':cls.get_fraction_results_reports(fraction.id), 
             'stp_number':fraction.sample.entry.project.code if fraction.sample.entry.project else '', 
             'comments':fraction.comments if fraction.comments else ''})

        ordered_objects = sorted((list(objects.values())), key=(lambda x: x['location']))
    ...
    def get_fraction_results_reports(cls, fraction_id):
        ...
        cursor.execute('SELECT DISTINCT(nl.results_report) FROM "' + Service._table + '" s INNER JOIN "' + NotebookLine._table + '" nl ON s.id = nl.service WHERE s.fraction = %s AND nl.results_report IS NOT NULL', (
         fraction_id,))
        res = cursor.fetchall()
        if not res:
            return ''
...
class CountersampleDischargePrintStart(ModelView):
    __doc__ = 'Countersamples Discharge Report'
...
class CountersampleDischargePrint(Wizard):
    ...
    start = StateView('lims.countersample.discharge.print.start', 'lims.lims_countersample_discharge_print_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Print', 'print_', 'tryton-print', default=True)])
    print_ = StateAction('lims.report_countersample_discharge')

    def do_print_(self, action):
        data = {'expiry_date_from':self.start.expiry_date_from, 
...
class CountersampleDischargeReport(Report):
    ...
    def get_context(cls, records, data):
        pool = Pool()
        Fraction = pool.get('lims.fraction')
        report_context = super(CountersampleDischargeReport, cls).get_context(records, data)
        report_context['company'] = report_context['user'].company
