...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/trytond/modules/lims/certification.py
# Compiled at: 2018-04-28 14:59:09
# Size of source mod 2**32: 8001 bytes
...
__all__ = [
 'CertificationType', 'TechnicalScope', 'TechnicalScopeVersion',
...
class TechnicalScope(ModelSQL, ModelView):
    __doc__ = 'Technical Scope'
    __name__ = 'lims.technical.scope'
    party = fields.Many2One('party.party', 'Party', required=True)
    certification_type = fields.Many2One('lims.certification.type', 'Certification type')
    versions = fields.One2Many('lims.technical.scope.version', 'technical_scope', 'Versions')
...
class TechnicalScopeVersion(ModelSQL, ModelView):
    __doc__ = 'Technical Scope Version'
    __name__ = 'lims.technical.scope.version'
    _rec_name = 'number'
    technical_scope = fields.Many2One('lims.technical.scope', 'Technical scope',
      required=True)
    number = fields.Char('Number', required=True)
    date = fields.Date('Date', required=True)
    expiration_date = fields.Date('Expiration date')
    version_lines = fields.Many2Many('lims.technical.scope.version.line', 'version', 'typification', 'Typifications')
    valid = fields.Boolean('Active')

    @classmethod
    def __setup__(cls):
        super(TechnicalScopeVersion, cls).__setup__()
        cls._error_messages.update({'active_version': 'Only one version can be active for each technical scope'})
        cls._buttons.update({'open_typifications':{},  'add_typifications':{'invisible': ~Eval('valid')}, 
         'remove_typifications':{'invisible': ~Eval('valid')}})
...
class TechnicalScopeVersionLine(ModelSQL):
    __doc__ = 'Technical Scope Version Line'
    __name__ = 'lims.technical.scope.version.line'
    version = fields.Many2One('lims.technical.scope.version', 'Technical scope version',
...
class AnalysisFamily(ModelSQL, ModelView):
    __doc__ = 'Analysis Family'
    __name__ = 'lims.analysis.family'
    _rec_name = 'description'
    code = fields.Char('Code', required=True)
    description = fields.Char('Description', required=True)
    party = fields.Many2One('party.party', 'Certificant party')
    certificants = fields.One2Many('lims.analysis.family.certificant', 'family', 'Product Type - Matrix')
...
class DuplicateAnalysisFamilyStart(ModelView):
    __doc__ = 'Duplicate Analysis Family'
...
class DuplicateAnalysisFamily(Wizard):
    ...
    start = StateView('lims.analysis.family.duplicate.start', 'lims.lims_duplicate_analysis_family_start_view_form', [
    ...
    def default_start(self, fields):
        ...
        return {'family_origin':family_origin.id, 
         'code':family_origin.code, 
         'description':family_origin.description}

    def transition_duplicate(self):
        AnalysisFamily = Pool().get('lims.analysis.family')
        AnalysisFamily.copy([self.start.family_origin], default={'code':self.start.code, 
