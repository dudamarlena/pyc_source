...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/trytond/modules/lims_instrument_generic_service/generic_service_xls.py
# Compiled at: 2018-12-06 14:11:57
# Size of source mod 2**32: 6559 bytes
import xlrd
    ...
    for worksheet_name in worksheets:
        ...
        if worksheet.nrows == 0:
            ...
            if row2nd[COL['E']].ctype == xlrd.XL_CELL_TEXT:
                ...
                if row2nd[COL['E']].ctype == xlrd.XL_CELL_DATE:
                    dt = xlrd.xldate_as_tuple(end_date_raw, workbook.datemode)
                    end_date = date(dt[0], dt[1], dt[2])
                else:
                    end_date = None
            chromatogram = row2nd[COL['G']].value if row2nd[COL['G']].ctype == xlrd.XL_CELL_TEXT else None
            ...
            if row3nd[COL['E']].ctype == xlrd.XL_CELL_TEXT:
                ...
                if row3nd[COL['E']].ctype == xlrd.XL_CELL_DATE:
                    it = xlrd.xldate_as_tuple(inj_date_raw, workbook.datemode)
                    inj_date = date(it[0], it[1], it[2])
                else:
                    inj_date = None
        row4th = worksheet.row(3)
        sample = int(row4th[COL['E']].value) if row4th[COL['E']].ctype == xlrd.XL_CELL_NUMBER else None
        year = int(row4th[COL['F']].value) if row4th[COL['F']].ctype == xlrd.XL_CELL_NUMBER else None
        workyear = LabWorkYear.search([
         'code', '=', str(year)])
        padding = None
        if workyear and workyear[0]:
        ...
        if padding:
            if sample:
                ...
                continue
            fraction = int(row4th[COL['G']].value) if row4th[COL['G']].ctype == xlrd.XL_CELL_NUMBER else None
            if fraction:
                if year:
                    fraction = str(year) + '/' + sample + '-' + str(fraction)
            repetition = int(row4th[COL['H']].value) if row4th[COL['H']].ctype == xlrd.XL_CELL_NUMBER else None
            professionals = row4th[COL['D']].value if row4th[COL['D']].ctype == xlrd.XL_CELL_TEXT else None
            dilution_factor = int(row4th[COL['K']].value) if row4th[COL['K']].ctype == xlrd.XL_CELL_NUMBER else None
                ...
                if analysis_code:
                    if fraction:
                        if repetition is None:
                            continue
                        values = {}
                        result = row[COL['D']].value if row[COL['D']].ctype == xlrd.XL_CELL_NUMBER else None
                        rm_correction_formula = row[COL['O']].value if row[COL['O']].ctype == xlrd.XL_CELL_TEXT else None
                        literal_result = row[COL['P']].value if row[COL['P']].ctype == xlrd.XL_CELL_TEXT else None
                        device = None
                        if row[COL['J']].ctype == xlrd.XL_CELL_NUMBER:
                            device = str(int(row[COL['J']].value))
                        else:
                            if row[COL['J']].ctype == xlrd.XL_CELL_TEXT:
