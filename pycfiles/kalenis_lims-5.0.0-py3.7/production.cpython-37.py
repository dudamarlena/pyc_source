...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/trytond/modules/lims_production/production.py
...
__all__ = [
 'BOM', 'Production', 'FamilyEquivalentReport']
...
class Production(metaclass=PoolMeta):
    ...
    concentration = fields.Char('Concentration', depends=[
     'salable_product', 'state'],
      states={'invisible':Bool(Eval('salable_product')), 
     'readonly':~Eval('state').in_(['request', 'draft'])})
    preparation_date = fields.Date('Preparation date', depends=[
     'salable_product', 'state'],
      states={'invisible':Bool(Eval('salable_product')), 
     'readonly':~Eval('state').in_(['request', 'draft'])})
    expiration_date = fields.Date('Expiration date', depends=[
     'state'],
      states={'readonly': ~Eval('state').in_(['request', 'draft'])})
    technician = fields.Many2One('lims.laboratory.professional', 'Technician',
      depends=['state'], states={'readonly': ~Eval('state').in_(['request', 'draft'])})
    solvent = fields.Function(fields.Many2One('product.product', 'Solvent',
      depends=['salable_product', 'state'], states={'invisible':Bool(Eval('salable_product')), 
     'readonly':~Eval('state').in_(['request', 'draft'])}), 'on_change_with_solvent')
    salable_product = fields.Function(fields.Boolean('Salable', depends=[
     'product']), 'on_change_with_salable_product')
    comments = fields.Text('Comments')

    @classmethod
    def __setup__(cls):
        super(Production, cls).__setup__()
        cls._error_messages.update({'quantity_multiple_required': 'Quantity multiple of output bom required.'})
    ...
    def on_change_with_solvent(self, name=None):
        ...
        for input_ in self.inputs:
            if input_.product and input_.product.account_category and input_.product.account_category.id in solvent_domain:
    ...
    def explode_bom(self):
        ...
        if self.bom:
            if not (self.product and self.uom):
                return
        else:
            return self.bom.divide_lots or None
        if self.quantity:
            quantity = 0.0
        ...
        factor = self.bom.compute_factor(self.product, self.quantity or 0, self.uom)
        ...
        for output in self.bom.outputs:
            ...
            for q in range(lines):
                move = self._explode_move_values(self.location, storage_location, self.company, output, line_qty)
                if move:
                    move.unit_price = Decimal(0)
                    if output.product == move.product:
                        if quantity:
                            move.unit_price = Decimal(self.cost / Decimal(str(quantity))).quantize(Decimal(str(10 ** (-digits[1]))))
                    outputs.append(move)
    ...
    def update_outputs_costs(cls, productions):
        ...
        for production in productions:
            if production.bom:
                ...
                factor = production.bom.compute_factor(production.product, production.quantity or 0, production.uom)
            else:
                divide_lots = False
            ...
            for output in production.outputs:
                ...
                if quantity:
                    ...
                    output.unit_price = Decimal(cost_price / Decimal(str(quantity))).quantize(Decimal(str(10 ** (-digits[1]))))
                    output.save()
        ...
        for production in productions:
            for move in production.outputs:
                ...
                lot = Lot(number=number,
                  ...
                  technician=(production.technician.id if production.technician else None),
                  solvent=(production.solvent.id if production.solvent else None))
                if lot:
                    lot.save()
                    Move.write([move], {'lot': lot})

    def _explode_move_values(self, from_location, to_location, company, bom_io, quantity):
        ...
        return super(Production, self)._explode_move_values(from_location, to_location, company, bom_io, quantity)
...
class FamilyEquivalentReport(Report):
    ...
    def get_context(cls, records, data):
        ...
        report_context = super(FamilyEquivalentReport, cls).get_context(records, data)
