...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/trytond/modules/lims/planification.py
# Compiled at: 2019-01-16 09:41:17
# Size of source mod 2**32: 280821 bytes
import logging
...
class Planification(Workflow, ModelSQL, ModelView):
    ...
    analysis = fields.Many2Many('lims.planification-analysis', 'planification',
      'analysis', 'Analysis/Sets/Groups', states={'readonly': Not(Bool(Equal(Eval('state'), 'draft')))},
      context={'date_from':Eval('date_from'), 
     'date_to':Eval('date_to'),  'calculate':Bool(Equal(Eval('state'), 'draft'))},
      domain=[
     'OR', ('id', 'in', Eval('analysis')),
     [
      (
       'id', 'in', Eval('analysis_domain'))]],
      depends=[
     'state', 'date_from', 'date_to', 'analysis_domain'])
    analysis_domain = fields.Function(fields.Many2Many('lims.analysis', None, None, 'Analysis domain'), 'on_change_with_analysis_domain')
    technicians = fields.One2Many('lims.planification.technician', 'planification',
      'Technicians', depends=['method_domain',
     'technicians_domain'])
    date_from = fields.Date('Date from', depends=['state'], required=True, states={'readonly': Not(Bool(Equal(Eval('state'), 'draft')))})
    date_to = fields.Date('Date to', depends=['state'], required=True, states={'readonly': Not(Bool(Equal(Eval('state'), 'draft')))})
    start_date = fields.Date('Start date', depends=['state'], states={'readonly': Bool(Equal(Eval('state'), 'confirmed'))})
    details = fields.One2Many('lims.planification.detail', 'planification',
      'Fractions to plan', states={'readonly': Not(Bool(Equal(Eval('state'), 'draft')))},
      depends=[
     'state'])
    controls = fields.Many2Many('lims.planification-fraction', 'planification',
      'fraction', 'Controls', readonly=True)
    ...
    method_domain = fields.Function(fields.One2Many('lims.lab.method', None, 'Method domain'), 'on_change_with_method_domain')
    technicians_domain = fields.Function(fields.One2Many('lims.laboratory.professional', None, 'Technicians domain'), 'on_change_with_technicians_domain')
    ...
    def __setup__(cls):
        ...
        cls._transitions |= set((('draft', 'preplanned'), ('preplanned', 'confirmed'),
                                 ('confirmed', 'not_executed')))
        cls._buttons.update({'add_analysis':{'readonly': Eval('state') != 'draft'}, 
         'search_fractions':{'readonly': Eval('state') != 'draft'}, 
         'search_planned_fractions':{'readonly': Eval('state') != 'draft'}, 
         'preplan':{'invisible': Eval('state') != 'draft'}, 
         'confirm':{'invisible': Eval('state') != 'preplanned'}, 
         'release_controls':{'invisible': Eval('state') != 'confirmed'}, 
         'relate_technicians':{'readonly': Eval('state') != 'preplanned'}, 
         'unlink_technicians':{'readonly': Eval('state') != 'preplanned'}, 
         'replace_technician':{'readonly': Eval('state') != 'confirmed'}, 
         'add_fraction_con':{'readonly': Eval('state') != 'preplanned'}, 
         'add_fraction_rm_bmz':{'readonly': Eval('state') != 'preplanned'}, 
         'add_fraction_bre':{'readonly': Eval('state') != 'preplanned'}, 
         'add_fraction_mrt':{'readonly': Eval('state') != 'preplanned'}, 
         'remove_control':{'readonly': Eval('state') != 'preplanned'}})
        cls._error_messages.update({'not_start_date':'The planification must have a start date', 
         'invalid_start_date':'The start date must be after "%s"', 
         'no_technician':'The following fractions have not a responsible technician:%s', 
         'waiting_process':'Planification "%s" is still waiting for processing', 
         'delete_planification':'You can not delete planification "%s" because it is not in draft or pre-planned state', 
    ...
    def _get_analysis_domain(laboratory):
        ...
        if not laboratory:
            return []
        cursor.execute('SELECT al.analysis FROM "' + AnalysisLaboratory._table + '" al INNER JOIN "' + Analysis._table + '" a ON a.id = al.analysis WHERE al.laboratory = %s AND a.behavior != \'internal_relation\'', (
         laboratory.id,))
        analysis_sets_list = [a[0] for a in cursor.fetchall()]
        groups_list = []
        cursor.execute('SELECT id FROM "' + Analysis._table + '" WHERE type = \'group\'')
        groups_list_ids = [g[0] for g in cursor.fetchall()]
        for group_id in groups_list_ids:
            if Planification._get_group_available(group_id, analysis_sets_list):
    ...
    def _get_group_available(group_id, analysis_sets_list):
        ...
        Analysis = pool.get('lims.analysis')
        cursor.execute('SELECT ia.included_analysis, a.type FROM "' + AnalysisIncluded._table + '" ia INNER JOIN "' + Analysis._table + '" a ON a.id = ia.included_analysis WHERE ia.analysis = %s AND a.behavior != \'internal_relation\'', (
         group_id,))
        ...
        for analysis in included_analysis:
            if analysis[1] != 'group':
                if analysis[0] not in analysis_sets_list:
                    return False
            if analysis[1] == 'group':
                return Planification._get_group_available(analysis[0], analysis_sets_list) or False
    ...
    def create(cls, vlist):
        ...
        config = Config(1)
        for values in vlist:
            values['code'] = Sequence.get_id(config.planification_sequence.id)
    ...
    def check_technicians(self):
        ...
        for detail in self.details:
            for service_detail in detail.details:
                key = service_detail.staff_responsible or (
                 detail.fraction.id,
    ...
    def update_laboratory_notebook(self):
        ...
        for service_detail in service_details:
            notebook_line = NotebookLine(service_detail.notebook_line.id)
            notebook_line.start_date = self.start_date
            notebook_line.laboratory_professionals = [p.id for p in service_detail.staff_responsible]
            notebook_line.planification = self.id
            if not service_detail.is_control:
                notebook_line.controls = [f.id for f in self.controls]
    ...
    def update_analysis_detail(self):
        ...
        for service_detail in service_details:
            analysis_detail_ids.append(service_detail.notebook_line.analysis_detail.id)
    ...
    def re_update_analysis_detail(self):
        ...
        for detail in self.details:
            for service_detail in detail.details:
                if service_detail.is_control and service_detail.notebook_line and service_detail.notebook_line.analysis_detail:
                    analysis_detail_ids.append(service_detail.notebook_line.analysis_detail.id)
        ...
        analysis_details = EntryDetailAnalysis.search([
         (
          'id', 'in', analysis_detail_ids)])
        if analysis_details:
            EntryDetailAnalysis.write(analysis_details, {'state': 'unplanned'})
...
class PlanificationTechnician(ModelSQL, ModelView):
    ...
    planification = fields.Many2One('lims.planification', 'Planification', ondelete='CASCADE',
      select=True,
      required=True)
    laboratory_professional = fields.Many2One('lims.laboratory.professional', 'Laboratory professional',
      required=True, domain=[
     (
      'id', 'in', Eval('_parent_planification', {}).get('technicians_domain'))])
    details = fields.Function(fields.One2Many('lims.planification.technician.detail', 'technician', 'Fractions to plan'), 'get_details')
    ...
    def get_details(self, name=None):
        cursor = Transaction().connection.cursor()
        pool = Pool()
        PlanificationDetailProfessional = pool.get('lims.planification.service_detail-laboratory.professional')
        PlanificationServiceDetail = pool.get('lims.planification.service_detail')
        PlanificationDetail = pool.get('lims.planification.detail')
        NotebookLine = pool.get('lims.notebook.line')
        Fraction = pool.get('lims.fraction')
        LabMethod = pool.get('lims.lab.method')
        cursor.execute('SELECT DISTINCT(f.number, nl.analysis_origin, m.code||\' - \'||m.name) FROM "' + PlanificationDetailProfessional._table + '" sdp INNER JOIN "' + PlanificationServiceDetail._table + '" sd ON sd.id = sdp.detail INNER JOIN "' + PlanificationDetail._table + '" d ON d.id = sd.detail INNER JOIN "' + NotebookLine._table + '" nl ON nl.id = sd.notebook_line INNER JOIN "' + LabMethod._table + '" m ON m.id = nl.method INNER JOIN "' + Fraction._table + '" f ON f.id = d.fraction WHERE sdp.professional = %s AND d.planification = %s', (
         self.laboratory_professional.id, self.planification.id))
        fractions = []
        for d in cursor.fetchall():
            r = d[0].split(',')
            fractions.append({'fraction':str(r[0][1:]), 
...
class PlanificationDetail(ModelSQL, ModelView):
    __doc__ = 'Fraction to Plan'
    __name__ = 'lims.planification.detail'
    planification = fields.Many2One('lims.planification', 'Planification', ondelete='CASCADE',
      select=True,
      required=True)
    fraction = fields.Many2One('lims.fraction', 'Fraction', required=True)
    service_analysis = fields.Many2One('lims.analysis', 'Service', required=True)
    fraction_type = fields.Function((fields.Many2One('lims.fraction.type', 'Fraction type')),
      'get_fraction_field', searcher='search_fraction_field')
    label = fields.Function((fields.Char('Label')), 'get_fraction_field', searcher='search_fraction_field')
    product_type = fields.Function(fields.Many2One('lims.product.type', 'Product type'), 'get_fraction_field')
    matrix = fields.Function(fields.Many2One('lims.matrix', 'Matrix'), 'get_fraction_field')
    details = fields.One2Many('lims.planification.service_detail', 'detail',
      'Planification detail', states={'readonly': True})
    ...
    def search_fraction_field(cls, name, clause):
        ...
        return [
         (
          'fraction.' + name,) + tuple(clause[1:])]

    @classmethod
    def get_service_field--- This code section failed: ---

 L. 685         0  BUILD_MAP_0           0 
                2  STORE_FAST               'result'

 L. 686         4  SETUP_LOOP          130  'to 130'
                6  LOAD_FAST                'names'
                8  GET_ITER         
               10  FOR_ITER            128  'to 128'
               12  STORE_FAST               'name'

 L. 687        14  BUILD_MAP_0           0 
               16  LOAD_FAST                'result'
               18  LOAD_FAST                'name'
               20  STORE_SUBSCR     

 L. 688        22  LOAD_FAST                'name'
               24  LOAD_STR                 'urgent'
               26  COMPARE_OP               ==
               28  POP_JUMP_IF_FALSE    60  'to 60'

 L. 689        30  SETUP_LOOP          126  'to 126'
               32  LOAD_FAST                'details'
               34  GET_ITER         
               36  FOR_ITER             56  'to 56'
               38  STORE_FAST               'd'

 L. 690        40  LOAD_CONST               False
               42  LOAD_FAST                'result'
               44  LOAD_FAST                'name'
               46  BINARY_SUBSCR    
               48  LOAD_FAST                'd'
               50  LOAD_ATTR                id
               52  STORE_SUBSCR     
               54  JUMP_BACK            36  'to 36'
               56  POP_BLOCK        
               58  JUMP_BACK            10  'to 10'
             60_0  COME_FROM            28  '28'

 L. 691        60  LOAD_FAST                'name'
               62  LOAD_STR                 'priority'
               64  COMPARE_OP               ==
               66  POP_JUMP_IF_FALSE    98  'to 98'

 L. 692        68  SETUP_LOOP          126  'to 126'
               70  LOAD_FAST                'details'
               72  GET_ITER         
               74  FOR_ITER             94  'to 94'
               76  STORE_FAST               'd'

 L. 693        78  LOAD_CONST               0
               80  LOAD_FAST                'result'
               82  LOAD_FAST                'name'
               84  BINARY_SUBSCR    
               86  LOAD_FAST                'd'
 ...
 L. 695        98  SETUP_LOOP          126  'to 126'
              100  LOAD_FAST                'details'
              102  GET_ITER         
              104  FOR_ITER            124  'to 124'
              106  STORE_FAST               'd'

 L. 696       108  LOAD_CONST               None
              ...
              124  POP_BLOCK        
            126_0  COME_FROM_LOOP       98  '98'
            126_1  COME_FROM_LOOP       68  '68'
            126_2  COME_FROM_LOOP       30  '30'
              126  JUMP_BACK            10  'to 10'
              128  POP_BLOCK        
            130_0  COME_FROM_LOOP        4  '4'

 L. 697       130  SETUP_LOOP          220  'to 220'
              132  LOAD_FAST                'details'
              134  GET_ITER         
            136_0  COME_FROM           150  '150'
            136_1  COME_FROM           144  '144'
              136  FOR_ITER            218  'to 218'
              138  STORE_FAST               'd'

 L. 698       140  LOAD_FAST                'd'
              142  LOAD_ATTR                fraction
              144  POP_JUMP_IF_FALSE   136  'to 136'
              146  LOAD_FAST                'd'
              148  LOAD_ATTR                service_analysis
              150  POP_JUMP_IF_FALSE   136  'to 136'

 L. 699       152  SETUP_LOOP          216  'to 216'
              154  LOAD_FAST                'd'
              156  LOAD_ATTR                fraction
              158  LOAD_ATTR                services
              160  GET_ITER         
            162_0  COME_FROM           176  '176'
              162  FOR_ITER            214  'to 214'
              164  STORE_FAST               'service'

 L. 700       166  LOAD_FAST                'service'
              168  LOAD_ATTR                analysis
              170  LOAD_FAST                'd'
              172  LOAD_ATTR                service_analysis
              174  COMPARE_OP               ==
              176  POP_JUMP_IF_FALSE   162  'to 162'

 L. 701       178  SETUP_LOOP          212  'to 212'
              180  LOAD_FAST                'names'
              182  GET_ITER         
              184  FOR_ITER            210  'to 210'
              186  STORE_FAST               'name'

 L. 702       188  LOAD_GLOBAL              getattr
              190  LOAD_FAST                'service'
              192  LOAD_FAST                'name'
              ...
              204  LOAD_ATTR                id
              206  STORE_SUBSCR     
              208  JUMP_BACK           184  'to 184'
              210  POP_BLOCK        
            212_0  COME_FROM_LOOP      178  '178'
              212  JUMP_BACK           162  'to 162'
              214  POP_BLOCK        
            216_0  COME_FROM_LOOP      152  '152'
              216  JUMP_BACK           136  'to 136'
              218  POP_BLOCK        
            220_0  COME_FROM_LOOP      130  '130'

 L. 703       220  LOAD_FAST                'result'
              222  RETURN_VALUE     
               -1  RETURN_LAST      
...
class PlanificationServiceDetail(ModelSQL, ModelView):
    __doc__ = 'Planification Detail'
    __name__ = 'lims.planification.service_detail'
    detail = fields.Many2One('lims.planification.detail', 'Planification', ondelete='CASCADE',
      select=True,
      required=True)
    planification = fields.Function((fields.Many2One('lims.planification', 'Planification')),
      'get_planification', searcher='search_planification')
    notebook_line = fields.Many2One('lims.notebook.line', 'Notebook line', required=True)
    staff_responsible = fields.Many2Many('lims.planification.service_detail-laboratory.professional', 'detail', 'professional', 'Laboratory professionals')
    ...
    repetition = fields.Function(fields.Integer('Repetition'), 'get_repetition')
...
class PlanificationServiceDetailLaboratoryProfessional(ModelSQL):
    __doc__ = 'Planification Detail - Laboratory Professional'
    __name__ = 'lims.planification.service_detail-laboratory.professional'
    _table = 'lims_plan_service_d-laboratory_professional'
    detail = fields.Many2One('lims.planification.service_detail', 'Planification detail',
      ondelete='CASCADE', select=True, required=True)
    professional = fields.Many2One('lims.laboratory.professional', 'Laboratory professional',
      ondelete='CASCADE', select=True, required=True)


class PlanificationAnalysis(ModelSQL):
    __doc__ = 'Planification - Analysis'
...
class PlanificationFraction(ModelSQL):
    __doc__ = 'Planification - Fraction'
    __name__ = 'lims.planification-fraction'
    planification = fields.Many2One('lims.planification', 'Planification', ondelete='CASCADE',
      select=True,
      required=True)
    fraction = fields.Many2One('lims.fraction', 'Fraction', ondelete='CASCADE',
      select=True,
      required=True)


class NotebookLineFraction(ModelSQL):
    __doc__ = 'Laboratory Notebook Line - Fraction'
    __name__ = 'lims.notebook.line-fraction'
    notebook_line = fields.Many2One('lims.notebook.line', 'Notebook Line', ondelete='CASCADE',
      select=True,
      required=True)
    fraction = fields.Many2One('lims.fraction', 'Fraction', ondelete='CASCADE',
      select=True,
...
class FractionReagent(ModelSQL, ModelView):
    __doc__ = 'Fraction Reagent'
    __name__ = 'lims.fraction.reagent'
    fraction = fields.Many2One('lims.fraction', 'Fraction', required=True, ondelete='CASCADE',
      select=True)
    product = fields.Many2One('product.product', 'Reagent', required=True, domain=[
     (
      'account_category', 'in', Eval('reagent_domain'))],
      depends=[
     'reagent_domain'])
    reagent_domain = fields.Function(fields.Many2Many('product.category', None, None, 'Reagent domain'), 'get_reagent_domain')
    lot = fields.Many2One('stock.lot', 'Lot', domain=[
     (
      'product', '=', Eval('product'))],
      depends=[
     'product'])
    quantity = fields.Function(fields.Float('Current quantity'), 'on_change_with_quantity')
    default_uom = fields.Function(fields.Many2One('product.uom', 'Default UOM'), 'on_change_with_default_uom')
    ...
    def on_change_with_quantity(self, name=None):
        ...
        with Transaction().set_context(locations=[l.id for l in locations], stock_date_end=(Date.today())):
...
class LabProfessionalMethod(ModelSQL, ModelView):
    __doc__ = 'Laboratory Professional Method'
    __name__ = 'lims.lab.professional.method'
    professional = fields.Many2One('lims.laboratory.professional', 'Professional',
      required=True)
    ...
    type = fields.Selection([
     ...
     ('analytical', 'Analytical')],
      'Type',
      sort=False)
    requalification_history = fields.One2Many('lims.lab.professional.method.requalification', 'professional_method', 'Trainings/Qualifications/Requalifications')
    determination = fields.Function((fields.Char('Determination')), 'get_determination',
      searcher='search_determination')
...
class LabProfessionalMethodRequalification(ModelSQL, ModelView):
    __doc__ = 'Laboratory Professional Method Requalification'
    __name__ = 'lims.lab.professional.method.requalification'
    _table = 'lims_lab_pro_method_req'
    professional_method = fields.Many2One('lims.lab.professional.method', 'Professional Method',
      ondelete='CASCADE', select=True, required=True)
    type = fields.Selection([
     ('training', 'Training'),
    ...
    supervisors = fields.One2Many('lims.lab.professional.method.requalification.supervisor', 'method_requalification', 'Supervisors')
    controls = fields.One2Many('lims.lab.professional.method.requalification.control', 'method_requalification', 'Controls')


class LabProfessionalMethodRequalificationSupervisor(ModelSQL, ModelView):
    __doc__ = 'Laboratory Professional Method Requalification Supervisor'
    __name__ = 'lims.lab.professional.method.requalification.supervisor'
    _table = 'lims_lab_pro_method_req_supervisor'
    method_requalification = fields.Many2One('lims.lab.professional.method.requalification',
      'Professional method requalification',
      ondelete='CASCADE', select=True,
      required=True)
    supervisor = fields.Many2One('lims.laboratory.professional', 'Supervisor', required=True)


class LabProfessionalMethodRequalificationControl(ModelSQL, ModelView):
    __doc__ = 'Laboratory Professional Method Requalification Control'
    ...
    method_requalification = fields.Many2One('lims.lab.professional.method.requalification',
      ...
      ondelete='CASCADE', select=True,
      required=True)
    control = fields.Many2One('lims.fraction', 'Control', required=True)


class BlindSample(ModelSQL, ModelView):
    __doc__ = 'Blind Sample'
    __name__ = 'lims.blind_sample'
    line = fields.Many2One('lims.notebook.line', 'Line', required=True, readonly=True,
      ondelete='CASCADE',
      select=True)
    ...
    original_sample = fields.Many2One('lims.sample', 'Original sample', readonly=True)
    original_fraction = fields.Many2One('lims.fraction', 'Original fraction', readonly=True)
...
class RelateTechniciansResult(ModelView):
    ...
    technicians = fields.Many2Many('lims.laboratory.professional', None,
      None, 'Technicians', required=True, domain=[
     (
      'id', 'in', Eval('technicians_domain'))],
      depends=[
     'technicians_domain'])
    technicians_domain = fields.One2Many('lims.laboratory.professional', None, 'Technicians domain')
    grouping = fields.Selection([
     ('none', 'None'),
     ('origin_method', 'Analysis origin and Method'),
     ('origin', 'Analysis origin')],
    ...
    details1 = fields.Many2Many('lims.planification.relate_technicians.detail1',
      None, None, 'Fractions to plan',
      domain=[('id', 'in', Eval('details1_domain'))], states={'invisible': Not(Bool(Equal(Eval('grouping'), 'none')))},
      depends=[
     'details1_domain', 'grouping'])
    details1_domain = fields.One2Many('lims.planification.relate_technicians.detail1', None, 'Fractions domain')
    details2 = fields.Many2Many('lims.planification.relate_technicians.detail2',
      None, None, 'Fractions to plan',
      domain=[('id', 'in', Eval('details2_domain'))], states={'invisible': Not(Bool(Equal(Eval('grouping'), 'origin_method')))},
      depends=[
     'details2_domain', 'grouping'])
    details2_domain = fields.One2Many('lims.planification.relate_technicians.detail2', None, 'Fractions domain')
    details3 = fields.Many2Many('lims.planification.relate_technicians.detail3',
      None, None, 'Fractions to plan',
      domain=[('id', 'in', Eval('details3_domain'))], states={'invisible': Not(Bool(Equal(Eval('grouping'), 'origin')))},
      depends=[
     'details3_domain', 'grouping'])
    details3_domain = fields.One2Many('lims.planification.relate_technicians.detail3', None, 'Fractions domain')
...
class RelateTechniciansDetail1(ModelSQL, ModelView):
    ...
    fraction_type = fields.Function(fields.Many2One('lims.fraction.type', 'Fraction type'), 'get_fraction_field')
    label = fields.Function(fields.Char('Label'), 'get_fraction_field')
    session_id = fields.Integer('Session ID')

    @classmethod
    def __register__(cls, module_name):
        super(RelateTechniciansDetail1, cls).__register__(module_name)
        cursor = Transaction().connection.cursor()
    ...
    @classmethod
    def get_fraction_field--- This code section failed: ---

 L.1048         0  BUILD_MAP_0           0 
                2  STORE_FAST               'result'

 L.1049         4  SETUP_LOOP          188  'to 188'
                6  LOAD_FAST                'names'
                8  GET_ITER         
               10  FOR_ITER            186  'to 186'
               12  STORE_FAST               'name'

 L.1050        14  BUILD_MAP_0           0 
               16  LOAD_FAST                'result'
               18  LOAD_FAST                'name'
               20  STORE_SUBSCR     

 L.1051        22  LOAD_FAST                'name'
               24  LOAD_STR                 'label'
               26  COMPARE_OP               ==
               28  POP_JUMP_IF_FALSE    70  'to 70'

 L.1052        30  SETUP_LOOP          184  'to 184'
               32  LOAD_FAST                'details'
               34  GET_ITER         
               36  FOR_ITER             66  'to 66'
               38  STORE_FAST               'd'

 L.1053        40  LOAD_GLOBAL              getattr
               42  LOAD_FAST                'd'
               44  LOAD_ATTR                fraction
               46  LOAD_FAST                'name'
               48  LOAD_CONST               None
               50  CALL_FUNCTION_3       3  '3 positional arguments'
               52  LOAD_FAST                'result'
               54  LOAD_FAST                'name'
               56  BINARY_SUBSCR    
               58  LOAD_FAST                'd'
 ...
 L.1054        70  LOAD_FAST                'name'
               72  LOAD_STR                 'fraction_type'
               74  COMPARE_OP               ==
               76  POP_JUMP_IF_FALSE   132  'to 132'

 L.1055        78  SETUP_LOOP          184  'to 184'
               80  LOAD_FAST                'details'
               82  GET_ITER         
               84  FOR_ITER            128  'to 128'
               86  STORE_FAST               'd'

 L.1056        88  LOAD_GLOBAL              getattr
               90  LOAD_FAST                'd'
               92  LOAD_ATTR                fraction
               94  LOAD_STR                 'type'
               96  LOAD_CONST               None
               98  CALL_FUNCTION_3       3  '3 positional arguments'
              100  STORE_FAST               'field'

 L.1057       102  LOAD_FAST                'field'
            ...
            114_0  COME_FROM           110  '110'
              ...
              128  POP_BLOCK        
              130  JUMP_BACK            10  'to 10'
            132_0  COME_FROM            76  '76'

 L.1059       132  SETUP_LOOP          184  'to 184'
              134  LOAD_FAST                'details'
              136  GET_ITER         
              138  FOR_ITER            182  'to 182'
              140  STORE_FAST               'd'

 L.1060       142  LOAD_GLOBAL              getattr
              144  LOAD_FAST                'd'
              146  LOAD_ATTR                fraction
              148  LOAD_FAST                'name'
              150  LOAD_CONST               None
              152  CALL_FUNCTION_3       3  '3 positional arguments'
              154  STORE_FAST               'field'

 L.1061       156  LOAD_FAST                'field'
            ...
            168_0  COME_FROM           164  '164'
              ...
              182  POP_BLOCK        
            184_0  COME_FROM_LOOP      132  '132'
            184_1  COME_FROM_LOOP       78  '78'
            184_2  COME_FROM_LOOP       30  '30'
              184  JUMP_BACK            10  'to 10'
              186  POP_BLOCK        
            188_0  COME_FROM_LOOP        4  '4'

 L.1062       188  LOAD_FAST                'result'
              190  RETURN_VALUE     
               -1  RETURN_LAST      

Parse error at or near `COME_FROM_LOOP' instruction at offset 184_1
...
class RelateTechniciansDetail2(ModelSQL, ModelView):
    ...
    @classmethod
    def __register__(cls, module_name):
        super(RelateTechniciansDetail2, cls).__register__(module_name)
        cursor = Transaction().connection.cursor()
        cursor.execute('DELETE FROM "' + cls._table + '"')
...
class RelateTechniciansDetail3(ModelSQL, ModelView):
    ...
    @classmethod
    def __register__(cls, module_name):
        super(RelateTechniciansDetail3, cls).__register__(module_name)
        cursor = Transaction().connection.cursor()
        cursor.execute('DELETE FROM "' + cls._table + '"')
...
class RelateTechnicians(Wizard):
    ...
    start = StateView('lims.planification.relate_technicians.start', 'lims.lims_relate_technicians_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Search', 'search', 'tryton-ok', default=True)])
    search = StateTransition()
    result = StateView('lims.planification.relate_technicians.result', 'lims.lims_relate_technicians_result_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Relate', 'relate', 'tryton-ok', default=True)])
    relate = StateTransition()

    def default_start(self, fields):
        return {'exclude_relateds':True, 
         'grouping':'none'}
    ...
    def transition_search(self):
        ...
        if self.start.grouping == 'none':
            self.result.details1_domain = self._view_details1(planification_id, self.start.exclude_relateds)
        else:
            if self.start.grouping == 'origin_method':
                self.result.details2_domain = self._view_details2(planification_id, self.start.exclude_relateds)
            else:
                if self.start.grouping == 'origin':
                    self.result.details3_domain = self._view_details3(planification_id, self.start.exclude_relateds)
        return 'result'
    ...
    def default_result(self, fields):
        ...
        return {'technicians_domain':[t.laboratory_professional.id for t in planification.technicians], 
    ...
    def _view_details1(self, planification_id, exclude_relateds):
        ...
        PlanificationDetail = pool.get('lims.planification.detail')
        PlanificationServiceDetail = pool.get('lims.planification.service_detail')
        ServiceDetailProfessional = pool.get('lims.planification.service_detail-laboratory.professional')
        RelateTechniciansDetail1 = pool.get('lims.planification.relate_technicians.detail1')
        exclude_relateds_clause = ''
        if exclude_relateds:
            exclude_relateds_clause = ' AND sd.id NOT IN (SELECT sdp.detail FROM "' + ServiceDetailProfessional._table + '" sdp INNER JOIN "' + PlanificationServiceDetail._table + '" sd ON sdp.detail = sd.id INNER JOIN "' + PlanificationDetail._table + '" d ON sd.detail = d.id WHERE d.planification = %s)' % planification_id
        details1 = {}
        cursor.execute('SELECT d.fraction, d.service_analysis, sd.id FROM "' + PlanificationDetail._table + '" d INNER JOIN "' + PlanificationServiceDetail._table + '" sd ON sd.detail = d.id WHERE d.planification = %s' + exclude_relateds_clause, (
         planification_id,))
        for x in cursor.fetchall():
            f, s = x[0], x[1]
            if (f, s) not in details1:
                details1[(f, s)] = {'fraction':f,  'service_analysis':s}
    ...
    def _view_details2(self, planification_id, exclude_relateds):
        ...
        ServiceDetailProfessional = pool.get('lims.planification.service_detail-laboratory.professional')
        NotebookLine = pool.get('lims.notebook.line')
        RelateTechniciansDetail2 = pool.get('lims.planification.relate_technicians.detail2')
        exclude_relateds_clause = ''
        if exclude_relateds:
            exclude_relateds_clause = ' AND sd.id NOT IN (SELECT sdp.detail FROM "' + ServiceDetailProfessional._table + '" sdp INNER JOIN "' + PlanificationServiceDetail._table + '" sd ON sdp.detail = sd.id INNER JOIN "' + PlanificationDetail._table + '" d ON sd.detail = d.id WHERE d.planification = %s)' % planification_id
        details2 = {}
        cursor.execute('SELECT d.fraction, nl.analysis_origin, nl.method, sd.id FROM "' + PlanificationDetail._table + '" d INNER JOIN "' + PlanificationServiceDetail._table + '" sd ON sd.detail = d.id INNER JOIN "' + NotebookLine._table + '" nl ON sd.notebook_line = nl.id WHERE d.planification = %s' + exclude_relateds_clause, (
         planification_id,))
        for x in cursor.fetchall():
            f, a, m = x[0], x[1], x[2]
            if (f, a, m) not in details2:
                details2[(f, a, m)] = {'fraction':f,  'analysis_origin':a, 
                 'method':m}
    ...
    def _view_details3(self, planification_id, exclude_relateds):
        ...
        ServiceDetailProfessional = pool.get('lims.planification.service_detail-laboratory.professional')
        NotebookLine = pool.get('lims.notebook.line')
        RelateTechniciansDetail3 = pool.get('lims.planification.relate_technicians.detail3')
        exclude_relateds_clause = ''
        if exclude_relateds:
            exclude_relateds_clause = ' AND sd.id NOT IN (SELECT sdp.detail FROM "' + ServiceDetailProfessional._table + '" sdp INNER JOIN "' + PlanificationServiceDetail._table + '" sd ON sdp.detail = sd.id INNER JOIN "' + PlanificationDetail._table + '" d ON sd.detail = d.id WHERE d.planification = %s)' % planification_id
        details3 = {}
        cursor.execute('SELECT d.fraction, nl.analysis_origin, sd.id FROM "' + PlanificationDetail._table + '" d INNER JOIN "' + PlanificationServiceDetail._table + '" sd ON sd.detail = d.id INNER JOIN "' + NotebookLine._table + '" nl ON sd.notebook_line = nl.id WHERE d.planification = %s' + exclude_relateds_clause, (
         planification_id,))
        for x in cursor.fetchall():
            f, a = x[0], x[1]
            if (f, a) not in details3:
                details3[(f, a)] = {'fraction':f,  'analysis_origin':a}
    ...
    def transition_relate(self):
        ...
        planification = Planification(Transaction().context['active_id'])
        if self.start.grouping == 'none':
            details = self._get_details1(planification.id, self.start.exclude_relateds)
        else:
            if self.start.grouping == 'origin_method':
                details = self._get_details2(planification.id, self.start.exclude_relateds)
            else:
                if self.start.grouping == 'origin':
                    details = self._get_details3(planification.id, self.start.exclude_relateds)
                else:
                    return 'end'
        PlanificationServiceDetail.write(details, {'staff_responsible': [
                               ('remove',
    ...
    def _get_details1(self, planification_id, exclude_relateds):
        ...
        for detail in self.result.details1:
            cursor.execute('SELECT sd.id FROM "' + PlanificationDetail._table + '" d INNER JOIN "' + PlanificationServiceDetail._table + '" sd ON sd.detail = d.id WHERE d.planification = %s AND d.fraction = %s AND d.service_analysis = %s' + exclude_relateds_clause, (
    ...
    def _get_details2(self, planification_id, exclude_relateds):
        ...
        for detail in self.result.details2:
            cursor.execute('SELECT sd.id FROM "' + PlanificationDetail._table + '" d INNER JOIN "' + PlanificationServiceDetail._table + '" sd ON sd.detail = d.id INNER JOIN "' + NotebookLine._table + '" nl ON sd.notebook_line = nl.id WHERE d.planification = %s AND d.fraction = %s AND nl.analysis_origin = %s AND nl.method = %s' + exclude_relateds_clause, (
             planification_id, detail.fraction.id,
    ...
    def _get_details3(self, planification_id, exclude_relateds):
        ...
        for detail in self.result.details3:
            cursor.execute('SELECT sd.id FROM "' + PlanificationDetail._table + '" d INNER JOIN "' + PlanificationServiceDetail._table + '" sd ON sd.detail = d.id INNER JOIN "' + NotebookLine._table + '" nl ON sd.notebook_line = nl.id WHERE d.planification = %s AND d.fraction = %s AND nl.analysis_origin = %s' + exclude_relateds_clause, (
             planification_id, detail.fraction.id,
...
class UnlinkTechniciansStart(ModelView):
    __doc__ = 'Unlink Technicians Start'
    ...
    technicians = fields.Many2Many('lims.laboratory.professional', None,
     ...
     'technicians_domain'])
    technicians_domain = fields.One2Many('lims.laboratory.professional', None, 'Technicians domain')
    details1 = fields.Many2Many('lims.planification.unlink_technicians.detail1',
      None, None, 'Assigned fractions',
      domain=[('id', 'in', Eval('details1_domain'))], depends=[
     'details1_domain'])
    details1_domain = fields.Many2Many('lims.planification.unlink_technicians.detail1', None, None, 'Fractions domain')


class UnlinkTechniciansDetail1(ModelSQL, ModelView):
    __doc__ = 'Fraction Detail Unlink'
    ...
    @classmethod
    def __register__(cls, module_name):
        super(UnlinkTechniciansDetail1, cls).__register__(module_name)
        cursor = Transaction().connection.cursor()
        cursor.execute('DELETE FROM "' + cls._table + '"')
 ...
 L.1540         0  BUILD_MAP_0           0 
                2  STORE_FAST               'result'

 L.1541         4  SETUP_LOOP          188  'to 188'
                6  LOAD_FAST                'names'
                8  GET_ITER         
               10  FOR_ITER            186  'to 186'
               12  STORE_FAST               'name'

 L.1542        14  BUILD_MAP_0           0 
               16  LOAD_FAST                'result'
               18  LOAD_FAST                'name'
               20  STORE_SUBSCR     

 L.1543        22  LOAD_FAST                'name'
               24  LOAD_STR                 'label'
               26  COMPARE_OP               ==
               28  POP_JUMP_IF_FALSE    70  'to 70'

 L.1544        30  SETUP_LOOP          184  'to 184'
               32  LOAD_FAST                'details'
               34  GET_ITER         
               36  FOR_ITER             66  'to 66'
               38  STORE_FAST               'd'

 L.1545        40  LOAD_GLOBAL              getattr
               42  LOAD_FAST                'd'
               44  LOAD_ATTR                fraction
               46  LOAD_FAST                'name'
               48  LOAD_CONST               None
 ...
 L.1546        70  LOAD_FAST                'name'
               72  LOAD_STR                 'fraction_type'
               74  COMPARE_OP               ==
               76  POP_JUMP_IF_FALSE   132  'to 132'

 L.1547        78  SETUP_LOOP          184  'to 184'
               80  LOAD_FAST                'details'
               82  GET_ITER         
               84  FOR_ITER            128  'to 128'
               86  STORE_FAST               'd'

 L.1548        88  LOAD_GLOBAL              getattr
               90  LOAD_FAST                'd'
               92  LOAD_ATTR                fraction
               94  LOAD_STR                 'type'
               96  LOAD_CONST               None
 ...
 L.1551       132  SETUP_LOOP          184  'to 184'
              134  LOAD_FAST                'details'
              136  GET_ITER         
              138  FOR_ITER            182  'to 182'
              140  STORE_FAST               'd'

 L.1552       142  LOAD_GLOBAL              getattr
              144  LOAD_FAST                'd'
              146  LOAD_ATTR                fraction
              148  LOAD_FAST                'name'
              150  LOAD_CONST               None
...
class UnlinkTechnicians(Wizard):
    __doc__ = 'Unlink Technicians'
    __name__ = 'lims.planification.unlink_technicians'
    start = StateView('lims.planification.unlink_technicians.start', 'lims.lims_unlink_technicians_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
    ...
    def _view_details(self, planification_id):
        ...
        PlanificationDetail = pool.get('lims.planification.detail')
        PlanificationServiceDetail = pool.get('lims.planification.service_detail')
        UnlinkTechniciansDetail1 = pool.get('lims.planification.unlink_technicians.detail1')
        details1 = {}
        cursor.execute('SELECT d.fraction, d.service_analysis, sd.id FROM "' + PlanificationDetail._table + '" d INNER JOIN "' + PlanificationServiceDetail._table + '" sd ON sd.detail = d.id WHERE d.planification = %s', (
         planification_id,))
        for x in cursor.fetchall():
            f, s = x[0], x[1]
            if (f, s) not in details1:
    ...
    def _get_details(self, planification_id):
        ...
        for detail in self.start.details1:
            cursor.execute('SELECT sd.id FROM "' + PlanificationDetail._table + '" d INNER JOIN "' + PlanificationServiceDetail._table + '" sd ON sd.detail = d.id WHERE d.planification = %s AND d.fraction = %s AND d.service_analysis = %s', (
             planification_id, detail.fraction.id,
             detail.service_analysis.id))
            for x in cursor.fetchall():
                details.append(x[0])
...
class AddFractionControlStart(ModelView):
    ...
    type = fields.Selection([
     ...
     ('itl', 'ITL')],
      'Control type',
      sort=False, required=True)
    original_fraction = fields.Many2One('lims.fraction', 'Original fraction', required=True,
      domain=[('id', 'in', Eval('fraction_domain'))],
      depends=[
     'fraction_domain'])
    fraction_domain = fields.Function(fields.One2Many('lims.fraction', None, 'Fraction domain'), 'on_change_with_fraction_domain')
    label = fields.Char('Label', depends=['type'], states={'readonly': Eval('type') == 'exist'})
    concentration_level = fields.Many2One('lims.concentration.level', 'Concentration level',
      states={'invisible': Bool(Eval('concentration_level_invisible'))},
      depends=[
     'concentration_level_invisible'])
    concentration_level_invisible = fields.Boolean('Concentration level invisible')
    generate_repetition = fields.Boolean('Generate repetition', states={'readonly': Eval('type') == 'exist'},
      depends=['type'])
        ...
        if not self.type:
            ...
            for p_analysis in self.planification.analysis:
                if p_analysis.type == 'analysis':
                    ...
                    p_analysis_ids.extend(Analysis.get_included_analysis_analysis(p_analysis.id))
            ...
            return notebook_lines or []
        notebook_lines_ids = ', '.join((str(nl.id) for nl in notebook_lines))
        cursor.execute('SELECT DISTINCT(n.fraction) FROM "' + Notebook._table + '" n INNER JOIN "' + NotebookLine._table + '" nl ON nl.notebook = n.id WHERE nl.id IN (' + notebook_lines_ids + ')')
    ...
    def on_change_with_label(self, name=None):
        ...
        if self.concentration_level:
            label += ' (' + self.concentration_level.description + ')'
...
class AddFractionControl(Wizard):
    ...
    start = StateView('lims.planification.add_fraction_con.start', 'lims.lims_add_fraction_con_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update({'no_entry_control':'There is no default entry control for this work year', 
         'no_con_fraction_type':'There is no Control fraction type configured', 
         'no_concentration_level':'Missing concentration level for this control type'})

    def default_start(self, fields):
        Config = Pool().get('lims.configuration')
        config = Config(1)
        defaults = {'planification':Transaction().context['active_id'], 
         'concentration_level_invisible':True}
        if config.con_fraction_type:
            if config.con_fraction_type.control_charts:
    ...
    def create_control(self):
        ...
        if fraction_type.control_charts:
            if not self.start.concentration_level:
        ...
        new_sample, = Sample.copy([original_sample], default={'entry':entry.id, 
         'date':datetime.now(), 
         'label':self.start.label, 
         'fractions':[]})
        new_fraction, = Fraction.copy([original_fraction], default={'sample':new_sample.id, 
         'type':fraction_type.id, 
         'services':[],  'con_type':self.start.type, 
        ...
        for service in services:
            if not Analysis.is_typified(service.analysis, new_sample.product_type, new_sample.matrix):
                continue
            ...
            if service.analysis.type == 'analysis':
                ...
                new_service, = Service.copy([service], default={'fraction':new_fraction.id, 
                 'method':method_id, 
                 'device':device_id})
        ...
        if fraction_type.control_charts:
            ...
            if notebook_lines:
                defaults = {'concentration_level': self.start.concentration_level.id}
    ...
    def generate_repetition(self, notebook_lines):
        ...
        for notebook_line in notebook_lines:
            ...
            if notebook_line.repetition > analysis_to_repeat[notebook_line.analysis.id].repetition:
                analysis_to_repeat[notebook_line.analysis.id] = notebook_line

        notebook = Notebook(notebook_lines[0].notebook.id)
        to_create = []
        for analysis_id, nline in analysis_to_repeat.items():
            to_create.append({'analysis_detail':nline.analysis_detail.id, 
             ...
             'initial_unit':nline.initial_unit.id if nline.initial_unit else None, 
             'final_unit':nline.final_unit.id if nline.final_unit else None, 
             'detection_limit':nline.detection_limit, 
             'quantification_limit':nline.quantification_limit, 
             'decimals':nline.decimals, 
             'report':nline.report, 
             'concentration_level':nline.concentration_level.id if nline.concentration_level else None})

        Notebook.write([notebook], {'lines': [('create', to_create)]})

    def add_control(self, fraction):
        Planification = Pool().get('lims.planification')
        Planification.write([self.start.planification], {'controls': [('add', [fraction.id])]})
    ...
    def add_planification_detail(self, fraction):
        ...
        if notebook_lines:
            ...
            for nl in notebook_lines:
                ...
                details_to_create[(f, s)].append({'notebook_line':nl.id, 
                 'planned_service':s, 
                 'is_control':True})
            ...
            if details_to_create:
                for k, v in details_to_create.items():
                    ...
                    if details:
                        PlanificationDetail.write([details[0]], {'details': [('create', v)]})
                    else:
...
class AddFractionRMBMZStart(ModelView):
    ...
    reference_fraction = fields.Many2One('lims.fraction', 'Reference fraction',
      depends=['fraction_domain', 'rm_bmz_type'], states={'readonly':Bool(Equal(Eval('rm_bmz_type'), 'noref')), 
     'required':Not(Bool(Equal(Eval('rm_bmz_type'), 'noref')))},
      domain=[
     (
      'id', 'in', Eval('fraction_domain'))])
    fraction_domain = fields.Function(fields.One2Many('lims.fraction', None, 'Fraction domain'), 'on_change_with_fraction_domain')
    product_type = fields.Many2One('lims.product.type', 'Product type', states={'readonly':Not(Bool(Equal(Eval('rm_bmz_type'), 'noref'))), 
     'required':Bool(Equal(Eval('rm_bmz_type'), 'noref'))},
      domain=[
     (
      'id', 'in', Eval('product_type_domain'))],
      depends=[
     'rm_bmz_type', 'product_type_domain'])
    product_type_domain = fields.Function(fields.Many2Many('lims.product.type', None, None, 'Product type domain'), 'on_change_with_product_type_domain')
    matrix = fields.Many2One('lims.matrix', 'Matrix', required=True, states={'readonly':Not(Bool(Equal(Eval('rm_bmz_type'), 'noref'))), 
     'required':Bool(Equal(Eval('rm_bmz_type'), 'noref'))},
      domain=[
     (
      'id', 'in', Eval('matrix_domain'))],
      depends=[
     'rm_bmz_type', 'matrix_domain'])
    matrix_domain = fields.Function(fields.Many2Many('lims.matrix', None, None, 'Matrix domain'), 'on_change_with_matrix_domain')
    repetitions = fields.Integer('Repetitions', states={'readonly': Or(Eval('type') == 'bmz', Eval('rm_bmz_type') == 'exist')},
      depends=[
     'type', 'rm_bmz_type'])
    label = fields.Char('Label', depends=['rm_bmz_type'], states={'readonly': Eval('rm_bmz_type') == 'exist'})
    concentration_level = fields.Many2One('lims.concentration.level', 'Concentration level',
      states={'invisible': Bool(Eval('concentration_level_invisible'))},
      depends=[
     'concentration_level_invisible'])
    ...
    def on_change_with_concentration_level_invisible(self, name=None):
        ...
        config = Config(1)
        if self.type == 'rm':
            if config.rm_fraction_type and config.rm_fraction_type.control_charts:
                return False
        elif self.type == 'bmz':
            if config.bmz_fraction_type:
                if config.bmz_fraction_type.control_charts:
                    return False
        return True
    ...
    def default_product_type_domain():
        ...
        cursor.execute('SELECT DISTINCT(product_type) FROM "' + Typification._table + '" WHERE valid')
        return [x[0] for x in cursor.fetchall()]
    ...
    def on_change_with_matrix_domain(self, name=None):
        ...
        if not self.product_type:
            return []
        cursor.execute('SELECT DISTINCT(matrix) FROM "' + Typification._table + '" WHERE product_type = %s AND valid', (
         self.product_type.id,))
        return [x[0] for x in cursor.fetchall()]

    @fields.depends('type', 'rm_bmz_type', 'reference_fraction', 'product_type', 'matrix', 'concentration_level')
    def on_change_with_label(self, name=None):
        Date = Pool().get('ir.date')
        if self.rm_bmz_type == 'exist':
            return ''
            ...
            if self.type == 'rm':
                ...
                if self.rm_bmz_type == 'sla':
                    if self.reference_fraction:
                        label += ' ' + self.reference_fraction.label
                    label += ' ' + str(Date.today())
...
class AddFractionRMBMZ(Wizard):
    ...
    start = StateView('lims.planification.add_fraction_rm_bmz.start', 'lims.lims_add_fraction_rm_bmz_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Add', 'add', 'tryton-ok', default=True)])
    add = StateTransition()
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update({'no_entry_control':'There is no default entry control for this work year', 
         'no_rm_fraction_type':'There is no RM fraction type configured', 
         'no_bmz_fraction_type':'There is no BMZ fraction type configured', 
         'no_rm_default_configuration':'Missing default configuration for RM fraction type', 
         'no_bmz_default_configuration':'Missing default configuration for BMZ fraction type', 
         'no_concentration_level':'Missing concentration level for this control type', 
         'not_typified':'The analysis "%(analysis)s" is not typified for product type "%(product_type)s" and matrix "%(matrix)s"'})
    ...
    def _create_control_sla(self):
        ...
        if self.start.type == 'rm':
            ...
            if self.start.type == 'bmz':
            ...
            elif fraction_type.control_charts:
                if not self.start.concentration_level:
                    self.raise_user_error('no_concentration_level')
                else:
                    workyear_id = LabWorkYear.find()
                    workyear = LabWorkYear(workyear_id)
                    workyear.default_entry_control or self.raise_user_error('no_entry_control')
                entry = Entry(workyear.default_entry_control.id)
                ...
                new_sample, = Sample.copy([original_sample], default={'entry':entry.id, 
                 ...
                 'label':self.start.label, 
                 'fractions':[]})
                fraction_default = {'sample':new_sample.id, 
                 'type':fraction_type.id, 
                 'con_type':'', 
                 'services':[]}
                if self.start.type == 'rm':
                ...
                new_fraction, = Fraction.copy([original_fraction], default=fraction_default)
                p_analysis_ids = []
                for p_analysis in self.start.planification.analysis:
                    if p_analysis.type == 'analysis':
                        p_analysis_ids.append(p_analysis.id)
                ...
                if self.start.type == 'rm':
                    notebook_lines = NotebookLine.search([
                     (
                      'notebook.fraction', '=', new_fraction.id)])
                    if notebook_lines:
                        defaults = {'final_concentration':None,  'final_unit':None, 
                ...
                if self.start.repetitions and self.start.repetitions > 0:
                    notebook_lines = NotebookLine.search([
                     (
                      'notebook.fraction', '=', new_fraction.id)])
                    if notebook_lines:
                        self.generate_repetition(notebook_lines, self.start.repetitions)
            return new_fraction
    ...
    def _create_control_noref(self):
        ...
        if self.start.type == 'rm':
            if not config.rm_fraction_type:
                self.raise_user_error('no_rm_fraction_type')
            fraction_type = config.rm_fraction_type
            fraction_type.default_package_type and fraction_type.default_fraction_state or self.raise_user_error('no_rm_default_configuration')
        else:
            if self.start.type == 'bmz':
                if not config.bmz_fraction_type:
                    self.raise_user_error('no_bmz_fraction_type')
                    ...
                    fraction_type = config.bmz_fraction_type
                    if fraction_type.default_package_type:
                        if not fraction_type.default_fraction_state:
                        ...
                        fraction_default['storage_time'] = laboratory.related_location.storage_time
                    else:
                    ...
                    services_default = []
                    for p_analysis in self.start.planification.analysis:
                        if not Analysis.is_typified(p_analysis, new_sample.product_type, new_sample.matrix):
                            self.raise_user_error('not_typified', {'analysis':p_analysis.rec_name, 
                             'product_type':new_sample.product_type.rec_name, 
                             'matrix':new_sample.matrix.rec_name})
                        laboratory_id = laboratory.id if p_analysis.type != 'group' else None
                        method_id = None
                        if new_sample.typification_domain:
                            for t in new_sample.typification_domain:
                                if t.analysis.id == p_analysis.id and t.by_default is True:
                                    method_id = t.method.id

                        device_id = None
                        if p_analysis.devices:
                            for d in p_analysis.devices:
                                if d.laboratory.id == laboratory.id and d.by_default is True:
                                    device_id = d.device.id

                        services_default.append({'fraction':new_fraction.id, 
                ...
                if self.start.type == 'rm':
                    ...
                    if notebook_lines:
                        ...
                        if config.rm_start_uom:
                            defaults['initial_unit'] = config.rm_start_uom.id
                        NotebookLine.write(notebook_lines, defaults)
            elif self.start.repetitions:
                if self.start.repetitions > 0:
                    notebook_lines = NotebookLine.search([
                     (
                      'notebook.fraction', '=', new_fraction.id)])
                    if notebook_lines:
...
class AddFractionBREStart(ModelView):
    ...
    type = fields.Selection([
     ...
     ('exist', 'Existing BRE')],
      'BRE Type',
      sort=False, required=True)
    bre_fraction = fields.Many2One('lims.fraction', 'BRE fraction',
      depends=['fraction_domain', 'type'], states={'readonly':Bool(Equal(Eval('type'), 'new')), 
     'required':Bool(Equal(Eval('type'), 'exist'))},
      domain=[
     (
      'id', 'in', Eval('fraction_domain'))])
    fraction_domain = fields.Function(fields.One2Many('lims.fraction', None, 'Fraction domain'), 'on_change_with_fraction_domain')
    product_type = fields.Many2One('lims.product.type', 'Product type', states={'required': Bool(Equal(Eval('type'), 'new'))},
      domain=[
     (
      'id', 'in', Eval('product_type_domain'))],
      depends=[
     'type', 'product_type_domain'])
    product_type_domain = fields.Function(fields.Many2Many('lims.product.type', None, None, 'Product type domain'), 'on_change_with_product_type_domain')
    matrix = fields.Many2One('lims.matrix', 'Matrix', required=True, states={'required': Bool(Equal(Eval('type'), 'new'))},
      domain=[
     (
      'id', 'in', Eval('matrix_domain'))],
      depends=[
     'type', 'matrix_domain'])
    matrix_domain = fields.Function(fields.Many2Many('lims.matrix', None, None, 'Matrix domain'), 'on_change_with_matrix_domain')
    reagents = fields.One2Many('lims.fraction.reagent', None, 'Reagents',
      states={'readonly': Bool(Equal(Eval('type'), 'exist'))},
      depends=[
     'type'],
      required=True)
    label = fields.Char('Label', depends=['type'], states={'readonly': Eval('type') == 'exist'})
...
class AddFractionBRE(Wizard):
    ...
    start = StateView('lims.planification.add_fraction_bre.start', 'lims.lims_add_fraction_bre_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Add', 'add', 'tryton-ok', default=True)])
    add = StateTransition()
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update({'no_entry_control':'There is no default entry control for this work year', 
         'no_bre_fraction_type':'There is no BRE fraction type configured', 
         'no_bre_default_configuration':'Missing default configuration for BRE fraction type', 
         'no_concentration_level':'Missing concentration level for this control type', 
         'not_typified':'The analysis "%(analysis)s" is not typified for product type "%(product_type)s" and matrix "%(matrix)s"'})
    ...
    def default_start(self, fields):
        ...
        if config.bre_fraction_type:
            if config.bre_fraction_type.control_charts:
                defaults['concentration_level_invisible'] = False
        return defaults
    ...
    def create_control(self):
        ...
        if not config.bre_fraction_type:
            ...
            if not (fraction_type.default_package_type and fraction_type.default_fraction_state):
            ...
            if not workyear.default_entry_control:
                ...
                new_sample, = Sample.create([
                 {'entry':entry.id, 
                  ...
                  'packages_quantity':1, 
                  'fractions':[]}])
                bre_reagents = [{'product':r.product.id,  'lot':r.lot.id if r.lot else None} for r in self.start.reagents]
                fraction_default = {'sample':new_sample.id, 
                 'type':fraction_type.id, 
                 'storage_location':laboratory.related_location.id, 
                 'packages_quantity':1, 
                 'package_type':fraction_type.default_package_type.id, 
                 'fraction_state':fraction_type.default_fraction_state.id, 
                 'services':[],  'bre_product_type':new_sample.product_type.id, 
                ...
                if fraction_type.max_storage_time:
                    ...
                    if laboratory.related_location.storage_time:
                        fraction_default['storage_time'] = laboratory.related_location.storage_time
                    else:
                        fraction_default['storage_time'] = 3
        new_fraction, = Fraction.create([fraction_default])
...
class AddFractionMRTStart(ModelView):
    ...
    type = fields.Selection([
     ...
     ('exist', 'Existing MRT')],
      'MRT Type',
      sort=False, required=True)
    mrt_fraction = fields.Many2One('lims.fraction', 'MRT fraction',
      depends=['fraction_domain', 'type'], states={'readonly':Bool(Equal(Eval('type'), 'new')), 
     'required':Bool(Equal(Eval('type'), 'exist'))},
      domain=[
     (
...
class AddFractionMRT(Wizard):
    ...
    start = StateView('lims.planification.add_fraction_mrt.start', 'lims.lims_add_fraction_mrt_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Add', 'add', 'tryton-ok', default=True)])
    add = StateTransition()
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update({'no_entry_control':'There is no default entry control for this work year', 
         'no_mrt_fraction_type':'There is no MRT fraction type configured', 
         'no_mrt_default_configuration':'Missing default configuration for MRT fraction type', 
         'no_concentration_level':'Missing concentration level for this control type', 
         'not_typified':'The analysis "%(analysis)s" is not typified for product type "%(product_type)s" and matrix "%(matrix)s"'})
    ...
    def default_start(self, fields):
        ...
        if config.mrt_fraction_type:
            if config.mrt_fraction_type.control_charts:
                defaults['concentration_level_invisible'] = False
        return defaults
        ...
        if not config.mrt_fraction_type:
            ...
            if not workyear.default_entry_control:
                ...
                fraction_default = {'sample':new_sample.id, 
                 ...
                 'package_type':fraction_type.default_package_type.id, 
                 'fraction_state':fraction_type.default_fraction_state.id, 
                 'services':[],  'mrt_product_type':new_sample.product_type.id, 
...
class RemoveControlStart(ModelView):
    __doc__ = 'Remove Control'
    __name__ = 'lims.planification.remove_control.start'
    controls = fields.Many2Many('lims.fraction', None, None, 'Controls', required=True,
      domain=[('id', 'in', Eval('controls_domain'))],
      depends=[
     'controls_domain'])
    controls_domain = fields.Many2Many('lims.fraction', None, None, 'Controls domain')


class RemoveControl(Wizard):
    __doc__ = 'Remove Control'
    __name__ = 'lims.planification.remove_control'
    start = StateView('lims.planification.remove_control.start', 'lims.lims_remove_control_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
    ...
    def default_start(self, fields):
        ...
        return {'controls_domain': controls_domain}
...
class AddAnalysisStart(ModelView):
    ...
    analysis = fields.Many2Many('lims.analysis', None, None, 'Analysis/Sets/Groups',
      required=True, domain=[
     (
      'id', 'in', Eval('analysis_domain'))],
      context={'date_from':Eval('date_from'), 
     'date_to':Eval('date_to')},
      depends=[
     'analysis_domain', 'date_from', 'date_to'])
    analysis_domain = fields.Many2Many('lims.analysis', None, None, 'Analysis domain')


class AddAnalysis(Wizard):
    __doc__ = 'Add Analysis'
    __name__ = 'lims.planification.add_analysis'
    start = StateView('lims.planification.add_analysis.start', 'lims.lims_add_analysis_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Add', 'add', 'tryton-ok', default=True)])
    add = StateTransition()
    ...
    def default_start(self, fields):
        Planification = Pool().get('lims.planification')
        planification = Planification(Transaction().context['active_id'])
        analysis_domain = AddAnalysis._get_analysis_domain(planification.laboratory, planification.date_from, planification.date_to)
        return {'laboratory':planification.laboratory.id, 
    ...
    def transition_add(self):
        Planification = Pool().get('lims.planification')
        planification = Planification(Transaction().context['active_id'])
        Planification.write([planification], {'analysis': [('remove', self.start.analysis)]})
        Planification.write([planification], {'analysis': [('add', self.start.analysis)]})
        return 'end'


class SearchFractionsNext(ModelView):
    __doc__ = 'Search Fractions'
    __name__ = 'lims.planification.search_fractions.next'
    details = fields.Many2Many('lims.planification.search_fractions.detail',
      None,
      None, 'Fractions to plan', depends=['details_domain'], domain=[
     (
      'id', 'in', Eval('details_domain'))],
      required=True)
    details_domain = fields.One2Many('lims.planification.search_fractions.detail', None, 'Fractions domain')
...
class SearchFractionsDetail(ModelSQL, ModelView):
    ...
    fraction = fields.Many2One('lims.fraction', 'Fraction', readonly=True)
    service_analysis = fields.Many2One('lims.analysis', 'Service', readonly=True)
    fraction_type = fields.Function(fields.Many2One('lims.fraction.type', 'Fraction type'), 'get_fraction_field')
    label = fields.Function(fields.Char('Label'), 'get_fraction_field')
    ...
    @classmethod
    def __register__(cls, module_name):
        super(SearchFractionsDetail, cls).__register__(module_name)
        cursor = Transaction().connection.cursor()
        cursor.execute('DELETE FROM "' + cls._table + '"')
 ...
 L.3642         0  BUILD_MAP_0           0 
                2  STORE_FAST               'result'

 L.3643         4  SETUP_LOOP          188  'to 188'
                6  LOAD_FAST                'names'
                8  GET_ITER         
               10  FOR_ITER            186  'to 186'
               12  STORE_FAST               'name'

 L.3644        14  BUILD_MAP_0           0 
               16  LOAD_FAST                'result'
               18  LOAD_FAST                'name'
               20  STORE_SUBSCR     

 L.3645        22  LOAD_FAST                'name'
               24  LOAD_STR                 'label'
               26  COMPARE_OP               ==
               28  POP_JUMP_IF_FALSE    70  'to 70'

 L.3646        30  SETUP_LOOP          184  'to 184'
               32  LOAD_FAST                'details'
               34  GET_ITER         
               36  FOR_ITER             66  'to 66'
               38  STORE_FAST               'd'

 L.3647        40  LOAD_GLOBAL              getattr
               42  LOAD_FAST                'd'
               44  LOAD_ATTR                fraction
               46  LOAD_FAST                'name'
               48  LOAD_CONST               None
 ...
 L.3648        70  LOAD_FAST                'name'
               72  LOAD_STR                 'fraction_type'
               74  COMPARE_OP               ==
               76  POP_JUMP_IF_FALSE   132  'to 132'

 L.3649        78  SETUP_LOOP          184  'to 184'
               80  LOAD_FAST                'details'
               82  GET_ITER         
               84  FOR_ITER            128  'to 128'
               86  STORE_FAST               'd'

 L.3650        88  LOAD_GLOBAL              getattr
               90  LOAD_FAST                'd'
               92  LOAD_ATTR                fraction
               94  LOAD_STR                 'type'
               96  LOAD_CONST               None
 ...
 L.3653       132  SETUP_LOOP          184  'to 184'
              134  LOAD_FAST                'details'
              136  GET_ITER         
              138  FOR_ITER            182  'to 182'
              140  STORE_FAST               'd'

 L.3654       142  LOAD_GLOBAL              getattr
              144  LOAD_FAST                'd'
              146  LOAD_ATTR                fraction
              148  LOAD_FAST                'name'
              150  LOAD_CONST               None
...
Parse error at or near `COME_FROM_LOOP' instruction at offset 184_1
 ...
 L.3664         0  BUILD_MAP_0           0 
                2  STORE_FAST               'result'

 L.3665         4  SETUP_LOOP          130  'to 130'
                6  LOAD_FAST                'names'
                8  GET_ITER         
               10  FOR_ITER            128  'to 128'
               12  STORE_FAST               'name'

 L.3666        14  BUILD_MAP_0           0 
               16  LOAD_FAST                'result'
               18  LOAD_FAST                'name'
               20  STORE_SUBSCR     

 L.3667        22  LOAD_FAST                'name'
               24  LOAD_STR                 'urgent'
               26  COMPARE_OP               ==
               28  POP_JUMP_IF_FALSE    60  'to 60'

 L.3668        30  SETUP_LOOP          126  'to 126'
               32  LOAD_FAST                'details'
               34  GET_ITER         
               36  FOR_ITER             56  'to 56'
               38  STORE_FAST               'd'

 L.3669        40  LOAD_CONST               False
               42  LOAD_FAST                'result'
               44  LOAD_FAST                'name'
               46  BINARY_SUBSCR    
               48  LOAD_FAST                'd'
 ...
 L.3670        60  LOAD_FAST                'name'
               62  LOAD_STR                 'priority'
               64  COMPARE_OP               ==
               66  POP_JUMP_IF_FALSE    98  'to 98'

 L.3671        68  SETUP_LOOP          126  'to 126'
               70  LOAD_FAST                'details'
               72  GET_ITER         
               74  FOR_ITER             94  'to 94'
               76  STORE_FAST               'd'

 L.3672        78  LOAD_CONST               0
               80  LOAD_FAST                'result'
               82  LOAD_FAST                'name'
               84  BINARY_SUBSCR    
               86  LOAD_FAST                'd'
 ...
 L.3674        98  SETUP_LOOP          126  'to 126'
              100  LOAD_FAST                'details'
              102  GET_ITER         
              104  FOR_ITER            124  'to 124'
              106  STORE_FAST               'd'

 L.3675       108  LOAD_CONST               None
              110  LOAD_FAST                'result'
              112  LOAD_FAST                'name'
              114  BINARY_SUBSCR    
              116  LOAD_FAST                'd'
 ...
 L.3676       130  SETUP_LOOP          220  'to 220'
              132  LOAD_FAST                'details'
              134  GET_ITER         
            136_0  COME_FROM           150  '150'
            136_1  COME_FROM           144  '144'
 ...
 L.3678       152  SETUP_LOOP          216  'to 216'
              154  LOAD_FAST                'd'
              156  LOAD_ATTR                fraction
              158  LOAD_ATTR                services
              160  GET_ITER         
 ...
 L.3680       178  SETUP_LOOP          212  'to 212'
              180  LOAD_FAST                'names'
              182  GET_ITER         
              184  FOR_ITER            210  'to 210'
              186  STORE_FAST               'name'

 L.3681       188  LOAD_GLOBAL              getattr
              190  LOAD_FAST                'service'
              192  LOAD_FAST                'name'
              194  CALL_FUNCTION_2       2  '2 positional arguments'
              196  LOAD_FAST                'result'
...
class SearchFractions(Wizard):
    ...
    start_state = 'search'
    search = StateTransition()
    next = StateView('lims.planification.search_fractions.next', 'lims.lims_search_fractions_next_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Add', 'add', 'tryton-ok', default=True)])
    add = StateTransition()
    ...
    def transition_search(self):
        ...
        PlanificationServiceDetail = pool.get('lims.planification.service_detail')
        PlanificationDetail = pool.get('lims.planification.detail')
        SearchFractionsDetail = pool.get('lims.planification.search_fractions.detail')
        planification = Planification(Transaction().context['active_id'])
    ...
    def default_next(self, fields):
        ...
        return {'details':[],  'details_domain':details}
    ...
    def transition_add(self):
        ...
        PlanificationDetail = pool.get('lims.planification.detail')
        planification = Planification(Transaction().context['active_id'])
        records_added = ['(%s,%s)' % (d.fraction.id, d.service_analysis.id) for d in self.next.details]
        records_ids_added = ', '.join((str(x) for x in ['(0,0)'] + records_added))
        extra_where = 'AND (nb.fraction, srv.analysis) IN (' + records_ids_added + ') '
        ...
        for k, v in data.items():
            ...
            if details:
                ...
                to_create.append({'planification':planification.id, 
                 'fraction':k[0], 
                 'service_analysis':k[1], 
                 'details':[
                  (
    ...
    def _get_service_details(self, planification, extra_where=''):
        ...
        planned_lines = [pd.notebook_line.id for pd in planification_details if pd.notebook_line]
        planned_lines_ids = ', '.join((str(x) for x in [0] + planned_lines))
        result = {}
        nlines_added = []
        for a in planification.analysis:
            analysis_id = a.id
            all_included_analysis = [
             analysis_id]
            all_included_analysis.extend(Analysis.get_included_analysis_analysis(analysis_id))
            all_included_analysis_ids = ', '.join((str(x) for x in all_included_analysis))
            service_where = 'AND ad.analysis IN (' + all_included_analysis_ids + ') '
            if extra_where:
                sample_select = ' '
                sample_from = ''
            else:
                sample_select = ', smp.product_type, smp.matrix '
                sample_from = 'INNER JOIN "' + Sample._table + '" smp ON smp.id = frc.sample '
            sql_select = 'SELECT nl.id, nb.fraction, srv.analysis' + sample_select
            sql_from = 'FROM "' + NotebookLine._table + '" nl INNER JOIN "' + Analysis._table + '" nla ON nla.id = nl.analysis INNER JOIN "' + Notebook._table + '" nb ON nb.id = nl.notebook INNER JOIN "' + Fraction._table + '" frc ON frc.id = nb.fraction INNER JOIN "' + FractionType._table + '" ft ON ft.id = frc.type INNER JOIN "' + EntryDetailAnalysis._table + '" ad ON ad.id = nl.analysis_detail INNER JOIN "' + Service._table + '" srv ON srv.id = nl.service ' + sample_from
            sql_where = 'WHERE nl.planification IS NULL AND nl.annulled = FALSE AND ft.plannable = TRUE AND nl.id NOT IN (' + planned_lines_ids + ") AND nl.laboratory = %s AND nla.behavior != 'internal_relation' AND ad.confirmation_date::date >= %s::date AND ad.confirmation_date::date <= %s::date " + service_where + extra_where
            sql_order = 'ORDER BY nb.fraction ASC, srv.analysis ASC'
                ...
                if extra_where:
                    for nl in notebook_lines:
                        ...
                        if nl[0] not in nlines_added:
                            ...
                            result[(f_, s_)].append({'notebook_line':nl[0], 
...
class SearchPlannedFractionsNext(ModelView):
    ...
    details = fields.Many2Many('lims.planification.search_planned_fractions.detail',
      None,
      None, 'Fractions to replan', depends=['details_domain'], domain=[
     (
      'id', 'in', Eval('details_domain'))],
      required=True)
    details_domain = fields.One2Many('lims.planification.search_planned_fractions.detail', None, 'Fractions domain')


class SearchPlannedFractionsDetail(ModelSQL, ModelView):
    __doc__ = 'Fraction to Replan'
    ...
    @classmethod
    def __register__(cls, module_name):
        super(SearchPlannedFractionsDetail, cls).__register__(module_name)
        cursor = Transaction().connection.cursor()
        cursor.execute('DELETE FROM "' + cls._table + '"')
 ...
 L.3961         0  BUILD_MAP_0           0 
                2  STORE_FAST               'result'

 L.3962         4  SETUP_LOOP          188  'to 188'
                6  LOAD_FAST                'names'
                8  GET_ITER         
               10  FOR_ITER            186  'to 186'
               12  STORE_FAST               'name'

 L.3963        14  BUILD_MAP_0           0 
               16  LOAD_FAST                'result'
               18  LOAD_FAST                'name'
               20  STORE_SUBSCR     

 L.3964        22  LOAD_FAST                'name'
               24  LOAD_STR                 'label'
               26  COMPARE_OP               ==
               28  POP_JUMP_IF_FALSE    70  'to 70'

 L.3965        30  SETUP_LOOP          184  'to 184'
               32  LOAD_FAST                'details'
               34  GET_ITER         
               36  FOR_ITER             66  'to 66'
               38  STORE_FAST               'd'

 L.3966        40  LOAD_GLOBAL              getattr
               42  LOAD_FAST                'd'
               44  LOAD_ATTR                fraction
               46  LOAD_FAST                'name'
               48  LOAD_CONST               None
 ...
 L.3967        70  LOAD_FAST                'name'
               72  LOAD_STR                 'fraction_type'
               74  COMPARE_OP               ==
               76  POP_JUMP_IF_FALSE   132  'to 132'

 L.3968        78  SETUP_LOOP          184  'to 184'
               80  LOAD_FAST                'details'
               82  GET_ITER         
               84  FOR_ITER            128  'to 128'
               86  STORE_FAST               'd'

 L.3969        88  LOAD_GLOBAL              getattr
               90  LOAD_FAST                'd'
               92  LOAD_ATTR                fraction
               94  LOAD_STR                 'type'
               96  LOAD_CONST               None
 ...
 L.3972       132  SETUP_LOOP          184  'to 184'
              134  LOAD_FAST                'details'
              136  GET_ITER         
              138  FOR_ITER            182  'to 182'
              140  STORE_FAST               'd'

 L.3973       142  LOAD_GLOBAL              getattr
              144  LOAD_FAST                'd'
              146  LOAD_ATTR                fraction
              148  LOAD_FAST                'name'
              150  LOAD_CONST               None
...
Parse error at or near `COME_FROM_LOOP' instruction at offset 184_1
 ...
 L.3983         0  BUILD_MAP_0           0 
                2  STORE_FAST               'result'

 L.3984         4  SETUP_LOOP          130  'to 130'
                6  LOAD_FAST                'names'
                8  GET_ITER         
               10  FOR_ITER            128  'to 128'
               12  STORE_FAST               'name'

 L.3985        14  BUILD_MAP_0           0 
               16  LOAD_FAST                'result'
               18  LOAD_FAST                'name'
               20  STORE_SUBSCR     

 L.3986        22  LOAD_FAST                'name'
               24  LOAD_STR                 'urgent'
               26  COMPARE_OP               ==
               28  POP_JUMP_IF_FALSE    60  'to 60'

 L.3987        30  SETUP_LOOP          126  'to 126'
               32  LOAD_FAST                'details'
               34  GET_ITER         
               36  FOR_ITER             56  'to 56'
               38  STORE_FAST               'd'

 L.3988        40  LOAD_CONST               False
               42  LOAD_FAST                'result'
               44  LOAD_FAST                'name'
               46  BINARY_SUBSCR    
               48  LOAD_FAST                'd'
 ...
 L.3989        60  LOAD_FAST                'name'
               62  LOAD_STR                 'priority'
               64  COMPARE_OP               ==
               66  POP_JUMP_IF_FALSE    98  'to 98'

 L.3990        68  SETUP_LOOP          126  'to 126'
               70  LOAD_FAST                'details'
               72  GET_ITER         
               74  FOR_ITER             94  'to 94'
               76  STORE_FAST               'd'

 L.3991        78  LOAD_CONST               0
               80  LOAD_FAST                'result'
               82  LOAD_FAST                'name'
               84  BINARY_SUBSCR    
               86  LOAD_FAST                'd'
 ...
 L.3993        98  SETUP_LOOP          126  'to 126'
              100  LOAD_FAST                'details'
              102  GET_ITER         
              104  FOR_ITER            124  'to 124'
              106  STORE_FAST               'd'

 L.3994       108  LOAD_CONST               None
              110  LOAD_FAST                'result'
              112  LOAD_FAST                'name'
              114  BINARY_SUBSCR    
              116  LOAD_FAST                'd'
 ...
 L.3995       130  SETUP_LOOP          220  'to 220'
              132  LOAD_FAST                'details'
              134  GET_ITER         
            136_0  COME_FROM           150  '150'
            136_1  COME_FROM           144  '144'
 ...
 L.3997       152  SETUP_LOOP          216  'to 216'
              154  LOAD_FAST                'd'
              156  LOAD_ATTR                fraction
              158  LOAD_ATTR                services
              160  GET_ITER         
 ...
 L.3999       178  SETUP_LOOP          212  'to 212'
              180  LOAD_FAST                'names'
              182  GET_ITER         
              184  FOR_ITER            210  'to 210'
              186  STORE_FAST               'name'

 L.4000       188  LOAD_GLOBAL              getattr
              190  LOAD_FAST                'service'
              192  LOAD_FAST                'name'
              194  CALL_FUNCTION_2       2  '2 positional arguments'
              196  LOAD_FAST                'result'
...
class SearchPlannedFractions(Wizard):
    ...
    start = StateView('lims.planification.search_planned_fractions.start', 'lims.lims_search_planned_fractions_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Search', 'search', 'tryton-forward', default=True)])
    search = StateTransition()
    next = StateView('lims.planification.search_planned_fractions.next', 'lims.lims_search_planned_fractions_next_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Add', 'add', 'tryton-ok', default=True)])
    add = StateTransition()
    ...
    def transition_search(self):
        pool = Pool()
        SearchPlannedFractionsDetail = pool.get('lims.planification.search_planned_fractions.detail')
    ...
    def _get_service_details(self, extra_where=''):
        ...
        for a in self.start.analysis:
            ...
            excluded_fractions = self.get_control_fractions_excluded(self.start.laboratory.id, service_clause)
            excluded_fractions_ids = ', '.join((str(x) for x in [0] + excluded_fractions))
            if extra_where:
                sample_select = ' '
                sample_from = ''
            else:
            ...
            sql_select = 'SELECT nl.id, nb.fraction, srv.analysis' + sample_select
            sql_from = 'FROM "' + NotebookLine._table + '" nl INNER JOIN "' + Analysis._table + '" nla ON nla.id = nl.analysis INNER JOIN "' + Notebook._table + '" nb ON nb.id = nl.notebook INNER JOIN "' + Fraction._table + '" frc ON frc.id = nb.fraction INNER JOIN "' + FractionType._table + '" ft ON ft.id = frc.type INNER JOIN "' + EntryDetailAnalysis._table + '" ad ON ad.id = nl.analysis_detail INNER JOIN "' + Service._table + '" srv ON srv.id = nl.service ' + sample_from
            sql_where = "WHERE nl.planification IS NOT NULL AND ft.plannable = TRUE AND nl.end_date IS NULL AND nl.laboratory = %s AND nla.behavior != 'internal_relation' AND nb.fraction NOT IN (" + excluded_fractions_ids + ') AND ad.confirmation_date::date >= %s::date AND ad.confirmation_date::date <= %s::date ' + service_where + extra_where
            sql_order = 'ORDER BY nb.fraction ASC, srv.analysis ASC'
            with Transaction().set_user(0):
                cursor.execute(sql_select + sql_from + sql_where + sql_order, (
    ...
    def get_control_fractions_excluded(self, laboratory, search_clause):
        ...
        notebook_lines = NotebookLine.search([
         ...
         ('notebook.fraction.special_type', 'in', ('rm', 'bmz', 'bre', 'mrt')),
         search_clause])
        if notebook_lines:
            analysis = []
            notebooks_id = []
...
class CreateFractionControlStart(ModelView):
    __doc__ = 'Create Fraction Control'
    ...
    product_type = fields.Many2One('lims.product.type', 'Product type', required=True,
     ...
     'product_type_domain'])
    product_type_domain = fields.Function(fields.Many2Many('lims.product.type', None, None, 'Product type domain'), 'on_change_with_product_type_domain')
    matrix = fields.Many2One('lims.matrix', 'Matrix', required=True, domain=[
     (
      'id', 'in', Eval('matrix_domain'))],
      depends=[
     'matrix_domain'])
    ...
    analysis = fields.Many2Many('lims.planification-analysis', 'planification',
     ...
     (
      ...
      depends=[
     'analysis_domain'])
    analysis_domain = fields.Function(fields.One2Many('lims.analysis', None, 'Analysis domain'), 'on_change_with_analysis_domain')
    ...
    def on_change_with_concentration_level_invisible(self, name=None):
        ...
        config = Config(1)
        if self.type == 'coi':
            if config.coi_fraction_type and config.coi_fraction_type.control_charts:
                return False
        elif self.type == 'mrc':
            if config.mrc_fraction_type and config.mrc_fraction_type.control_charts:
                return False
        elif self.type == 'sla':
            if config.sla_fraction_type:
                if config.sla_fraction_type.control_charts:
                    return False
        return True
    ...
    def on_change_with_analysis_domain(self, name=None):
        ...
        if not (self.laboratory and self.product_type and self.matrix):
            return []
            cursor.execute('SELECT DISTINCT(analysis) FROM "' + AnalysisLaboratory._table + '" WHERE laboratory = %s', (
             self.laboratory.id,))
            analysis_sets_list = [a[0] for a in cursor.fetchall()]
            if not analysis_sets_list:
                return []
            lab_analysis_ids = ', '.join((str(a) for a in analysis_sets_list))
            groups_list = []
            ...
            if groups:
                for group in groups:
                    available = True
                    ia = Analysis.get_included_analysis_analysis(group.id)
                    if not ia:
                        continue
                    included_ids = ', '.join((str(a) for a in ia))
                    cursor.execute('SELECT id FROM "' + Analysis._table + '" WHERE id IN (' + included_ids + ') AND id NOT IN (' + lab_analysis_ids + ')')
                    if cursor.fetchone():
                        available = False
                    if available:
        ...
        cursor.execute('SELECT DISTINCT(typ.analysis) FROM (SELECT t.analysis FROM "' + Typification._table + '" t WHERE t.product_type = %s AND t.matrix = %s AND t.valid UNION SELECT ct.analysis FROM "' + CalculatedTypification._table + '" ct WHERE ct.product_type = %s AND ct.matrix = %s) AS typ WHERE typ.analysis IN (' + analysis_domain_ids + ')', (
...
class CreateFractionControl(Wizard):
    ...
    start = StateView('lims.planification.create_fraction_con.start', 'lims.lims_create_fraction_con_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update({'no_entry_control':'There is no default entry control for this work year', 
         'no_coi_fraction_type':'There is no COI fraction type configured', 
         'no_mrc_fraction_type':'There is no MRC fraction type configured', 
         'no_sla_fraction_type':'There is no SLA fraction type configured', 
         'no_coi_default_configuration':'Missing default configuration for COI fraction type', 
         'no_mrc_default_configuration':'Missing default configuration for MRC fraction type', 
         'no_sla_default_configuration':'Missing default configuration for SLA fraction type', 
         'no_concentration_level':'Missing concentration level for this control type'})

    def default_start(self, fields):
        defaults = {'laboratory':Transaction().context.get('laboratory', None), 
         'concentration_level_invisible':True}
        return defaults
...
class ReleaseFractionResult(ModelView):
    ...
    fractions = fields.Many2Many('lims.planification.detail', None, None, 'Fractions',
      required=True, domain=[
     (
      'id', 'in', Eval('fractions_domain'))],
      depends=[
     'fractions_domain'])
    fractions_domain = fields.One2Many('lims.planification.detail', None, 'Fractions domain')


class ReleaseFraction(Wizard):
    __doc__ = 'Release Fraction'
    __name__ = 'lims.planification.release_fraction'
    start = StateView('lims.planification.release_fraction.start', 'lims.lims_planification_release_fraction_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Search', 'search', 'tryton-forward', default=True)])
    search = StateTransition()
    empty = StateView('lims.planification.release_fraction.empty', 'lims.lims_planification_release_fraction_empty_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Search again', 'start', 'tryton-forward', default=True)])
    result = StateView('lims.planification.release_fraction.result', 'lims.lims_planification_release_fraction_result_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
    ...
    def default_start(self, fields):
        ...
        if hasattr(self.start, 'laboratory') and getattr(self.start, 'laboratory'):
            res['laboratory'] = getattr(self.start, 'laboratory').id
        else:
            res['laboratory'] = Transaction().context.get('laboratory', None)
        return res

    def transition_search--- This code section failed: ---

 L.4653         0  LOAD_GLOBAL              Pool
                2  CALL_FUNCTION_0       0  '0 positional arguments'
                4  LOAD_METHOD              get
                6  LOAD_STR                 'lims.planification.detail'
                8  CALL_METHOD_1         1  '1 positional argument'
               10  STORE_FAST               'PlanificationDetail'

 L.4655        12  LOAD_FAST                'PlanificationDetail'
               14  LOAD_METHOD              search

 L.4656        16  LOAD_STR                 'planification.laboratory'
               18  LOAD_STR                 '='
               20  LOAD_FAST                'self'
               22  LOAD_ATTR                start
               24  LOAD_ATTR                laboratory
               26  LOAD_ATTR                id
               28  BUILD_TUPLE_3         3 

 L.4657        30  LOAD_STR                 'planification.start_date'
               32  LOAD_STR                 '>='
               34  LOAD_FAST                'self'
               36  LOAD_ATTR                start
               38  LOAD_ATTR                date_from
               40  BUILD_TUPLE_3         3 

 L.4658        42  LOAD_STR                 'planification.start_date'
               44  LOAD_STR                 '<='
               46  LOAD_FAST                'self'
               48  LOAD_ATTR                start
               50  LOAD_ATTR                date_to
               52  BUILD_TUPLE_3         3 

 L.4659        54  LOAD_CONST               ('planification.state', '!=', 'draft')
               56  BUILD_LIST_4          4 
               58  CALL_METHOD_1         1  '1 positional argument'
               60  STORE_FAST               'details'

 L.4661        62  LOAD_FAST                'details'
               64  POP_JUMP_IF_FALSE   198  'to 198'

 L.4662        66  BUILD_LIST_0          0 
               68  STORE_FAST               'fractions'

 L.4663        70  SETUP_LOOP          182  'to 182'
               72  LOAD_FAST                'details'
               74  GET_ITER         
             76_0  COME_FROM           166  '166'
               76  FOR_ITER            180  'to 180'
               78  STORE_FAST               'detail'

 L.4664        80  LOAD_CONST               True
               82  STORE_FAST               'available'

 L.4665        84  SETUP_LOOP          164  'to 164'
               86  LOAD_FAST                'detail'
               88  LOAD_ATTR                details
               90  GET_ITER         
             92_0  COME_FROM           152  '152'
               92  FOR_ITER            162  'to 162'
               94  STORE_FAST               'service_detail'

 L.4666        96  LOAD_FAST                'service_detail'
               98  LOAD_ATTR                is_control
              100  POP_JUMP_IF_FALSE   108  'to 108'

 L.4667       102  LOAD_CONST               False
              104  STORE_FAST               'available'

 L.4668       106  BREAK_LOOP       
            108_0  COME_FROM           100  '100'

 L.4669       108  LOAD_FAST                'service_detail'
              110  LOAD_ATTR                notebook_line
              112  STORE_FAST               'nline'

 L.4670       114  LOAD_FAST                'nline'
              116  POP_JUMP_IF_TRUE    124  'to 124'

 L.4671       118  LOAD_CONST               False
              120  STORE_FAST               'available'

 L.4672       122  BREAK_LOOP       
            124_0  COME_FROM           116  '116'

 L.4673       124  LOAD_FAST                'nline'
              126  LOAD_ATTR                result
              128  POP_JUMP_IF_TRUE    154  'to 154'
              130  LOAD_FAST                'nline'
              132  LOAD_ATTR                converted_result
              134  POP_JUMP_IF_TRUE    154  'to 154'

 L.4674       136  LOAD_FAST                'nline'
              138  LOAD_ATTR                literal_result
              140  POP_JUMP_IF_TRUE    154  'to 154'
              142  LOAD_FAST                'nline'
              144  LOAD_ATTR                end_date
              146  POP_JUMP_IF_TRUE    154  'to 154'

 L.4675       148  LOAD_FAST                'nline'
              150  LOAD_ATTR                annulment_date
              152  POP_JUMP_IF_FALSE    92  'to 92'
            154_0  COME_FROM           146  '146'
            154_1  COME_FROM           140  '140'
            154_2  COME_FROM           134  '134'
            154_3  COME_FROM           128  '128'

 L.4676       154  LOAD_CONST               False
              156  STORE_FAST               'available'

 L.4677       158  BREAK_LOOP       
              160  JUMP_BACK            92  'to 92'
              162  POP_BLOCK        
            164_0  COME_FROM_LOOP       84  '84'

 L.4678       164  LOAD_FAST                'available'
              166  POP_JUMP_IF_FALSE    76  'to 76'

 L.4679       168  LOAD_FAST                'fractions'
              170  LOAD_METHOD              append
              172  LOAD_FAST                'detail'
              174  CALL_METHOD_1         1  '1 positional argument'
              176  POP_TOP          
              178  JUMP_BACK            76  'to 76'
              180  POP_BLOCK        
            182_0  COME_FROM_LOOP       70  '70'

 L.4681       182  LOAD_FAST                'fractions'
              184  POP_JUMP_IF_FALSE   198  'to 198'

 L.4682       186  LOAD_FAST                'fractions'
              188  LOAD_FAST                'self'
              190  LOAD_ATTR                result
              192  STORE_ATTR               fractions

 L.4683       194  LOAD_STR                 'result'
              196  RETURN_VALUE     
            198_0  COME_FROM           184  '184'
            198_1  COME_FROM            64  '64'

 L.4684       198  LOAD_STR                 'empty'
              200  RETURN_VALUE     
               -1  RETURN_LAST      

Parse error at or near `POP_BLOCK' instruction at offset 162

    def default_result(self, fields):
        fractions = [f.id for f in self.result.fractions]
        self.result.fractions = None
        return {'fractions_domain': fractions}
    ...
    def _re_update_laboratory_notebook(self):
        ...
        for detail in self.result.fractions:
            for service_detail in detail.details:
                if service_detail.is_control or service_detail.notebook_line:
                    notebook_line = NotebookLine(service_detail.notebook_line.id)
                    notebook_line.start_date = None
                    notebook_line.laboratory_professionals = []
                    notebook_line.planification = None
    ...
    def _re_update_analysis_detail(self):
        ...
        for detail in self.result.fractions:
            for service_detail in detail.details:
                if service_detail.is_control or service_detail.notebook_line and service_detail.notebook_line.analysis_detail:
                    analysis_detail_ids.append(service_detail.notebook_line.analysis_detail.id)
...
class QualificationSituations(ModelView):
    __doc__ = 'Technicians Qualification'
    __name__ = 'lims.planification.qualification.situations'
    situations = fields.Many2Many('lims.planification.qualification.situation', None, None, 'Situations')
    total = fields.Integer('Total')
    index = fields.Integer('Index')


class QualificationSituation(ModelSQL, ModelView):
    __doc__ = 'Qualification Situation'
    ...
    @classmethod
    def __register__(cls, module_name):
        super(QualificationSituation, cls).__register__(module_name)
        cursor = Transaction().connection.cursor()
        cursor.execute('DELETE FROM "' + cls._table + '"')


class QualificationAction(ModelSQL):
    __doc__ = 'Qualification Action'
    ...
    @classmethod
    def __register__(cls, module_name):
        super(QualificationAction, cls).__register__(module_name)
        cursor = Transaction().connection.cursor()
        cursor.execute('DELETE FROM "' + cls._table + '"')
...
class TechniciansQualification(Wizard):
    ...
    situations = StateView('lims.planification.qualification.situations', 'lims.lims_qualification_situations_view_form', [])
    start = StateTransition()
    next_ = StateTransition()
    confirm = StateTransition()
    sit2 = StateView('lims.planification.qualification.situation.2', 'lims.lims_qualification_situation_2_view_form', [
    ...
    sit3 = StateView('lims.planification.qualification.situation.3', 'lims.lims_qualification_situation_3_view_form', [
     Button('Requalify', 'sit3_op1', 'tryton-ok', default=True),
     Button('Cancel', 'end', 'tryton-cancel')])
    sit3_op1 = StateTransition()
    sit4 = StateView('lims.planification.qualification.situation.4', 'lims.lims_qualification_situation_4_view_form', [
     Button('Cancel', 'end', 'tryton-cancel')])
    ...
    def transition_start(self):
        ...
        QualificationSituation = pool.get('lims.planification.qualification.situation')
        planification = Planification(Transaction().context['active_id'])
        planification_details = PlanificationServiceDetail.search([
         (
          'planification', '=', planification.id)])
        ...
        for detail in planification_details:
            method = detail.notebook_line.method if detail.notebook_line else None
            if method:
                for technician in detail.staff_responsible:
                    key = (
                     technician.id, method.id)
        ...
        if situations:
            self.situations.situations = QualificationSituation.create([{'session_id':self._session_id,  'professional':k[0],  'method':k[1],  'situation':v} for k, v in situations.items()])
    ...
    def situation_1(self, data):
        QualificationAction = Pool().get('lims.planification.qualification.action')
    ...
    def situation_3(self, data):
        ...
        Date = pool.get('ir.date')
        QualificationAction = pool.get('lims.planification.qualification.action')
        deadline = Date.today() - relativedelta(months=(data.method.requalification_months))
        ...
        for requalification in professional_method.requalification_history:
            if requalification.last_execution_date and last_execution < requalification.last_execution_date:
                ...
                for technician in detail.staff_responsible:
                    qualifications = LabProfessionalMethod.search([
                     ...
                     (
                      'method', '=', method.id),
                     ('type', '=', 'preparation')])
                    if qualifications and qualifications[0].state in ('qualified',
                                                                      'requalified'):
                        ...
                        actions = QualificationAction.search([
                         ...
                         (
                          'method', '=', method.id),
                         ('action', '=', 4)])
                        qualified = actions or True
                        break

                qualified or methods.append('[%s] %s' % (method.code, method.name))
    ...
    def action_1(self, data, controls, start_date):
        ...
        professional_method, = LabProfessionalMethod.create([
         {'professional':data.professional.id, 
          ...
          'requalification_history':[
           (
            'create', requalification_history)]}])
        ...
        requalification_history = [
         {'type':'qualification', 
          ...
          'controls':[
           (
            'create', controls)]}]
        LabProfessionalMethod.write([professional_method], {'state':'qualified', 
         'requalification_history':[
          (
           'create', requalification_history)]})
        ...
        requalification_history = [
         {'type':'training', 
          ...
          'controls':[
           (
            'create', controls)]}]
        LabProfessionalMethod.write([professional_method], {'requalification_history': [('create', requalification_history)]})
        ...
        requalification_history = [
         {'type':'requalification', 
          ...
          'controls':[
           (
            'create', controls)]}]
        LabProfessionalMethod.write([professional_method], {'state':'requalified', 
         'requalification_history':[
          (
           'create', requalification_history)]})
    ...
    def action_5(self, data, controls, start_date):
        ...
        supervisors = self._get_supervisors(data)
        requalification_history = [
         {'type':'qualification' if professional_method.state == 'qualified' else 'requalification', 
          'date':Date.today(), 
          'last_execution_date':start_date, 
          'supervisors':[
           (
            ...
            for technician in detail.staff_responsible:
                ...
                qualifications = LabProfessionalMethod.search([
                 ...
                 (
                  'method', '=', data.method.id),
                 ('type', '=', 'preparation')])
                if qualifications and qualifications[0].state in ('qualified', 'requalified'):
                    if not data.method.supervised_requalification:
                        ...
                        actions = QualificationAction.search([
                         ...
                         (
                          'method', '=', data.method.id),
                         ('action', '=', 4)])
                        actions or supervisors.append(technician.id)
...
class ReplaceTechnicianStart(ModelView):
    __doc__ = 'Replace Technician'
    __name__ = 'lims.planification.replace_technician.start'
    planification = fields.Many2One('lims.planification', 'Planification')
    technician_replaced = fields.Many2One('lims.laboratory.professional', 'Technician replaced',
      required=True, domain=[
     (
      'id', 'in', Eval('replaced_domain'))],
      depends=[
     'replaced_domain'])
    replaced_domain = fields.One2Many('lims.laboratory.professional', None, 'Replaced domain')
    technician_substitute = fields.Many2One('lims.laboratory.professional', 'Technician substitute',
      required=True, domain=[
     (
      'id', 'in', Eval('substitute_domain'))],
      depends=[
     'substitute_domain'])
    substitute_domain = fields.Function(fields.One2Many('lims.laboratory.professional', None, 'Substitute domain'), 'on_change_with_substitute_domain')
        ...
        for technician_id in substitute_domain:
            for method_id in methods_ids:
                qualifications = LabProfessionalMethod.search([
                 ...
                 ('type', '=', 'preparation')])
                if qualifications:
                    qualifications[0].state in ('qualified', 'requalified') or substitute_domain.remove(technician_id)
                    break
...
class ReplaceTechnician(Wizard):
    ...
    start = StateView('lims.planification.replace_technician.start', 'lims.lims_replace_technician_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
    ...
    def default_start(self, fields):
        Planification = Pool().get('lims.planification')
        planification = Planification(Transaction().context['active_id'])
        return {'planification':planification.id, 
         'replaced_domain':[t.laboratory_professional.id for t in planification.technicians], 
         'substitute_domain':[]}
    ...
    def transition_replace(self):
        ...
        self.update_planification_detail(planification_id, technician_replaced_id, technician_substitute_id)
        self.update_planification_technician(planification_id, technician_replaced_id, technician_substitute_id)
        self.update_laboratory_notebook(planification_id, technician_replaced_id, technician_substitute_id)
        return 'end'

    def update_planification_detail(self, planification_id, technician_replaced_id, technician_substitute_id):
        PlanificationServiceDetailProfessional = Pool().get('lims.planification.service_detail-laboratory.professional')
        ...
        PlanificationServiceDetailProfessional.write(details_professional, {'professional': technician_substitute_id})

    def update_planification_technician(self, planification_id, technician_replaced_id, technician_substitute_id):
    ...
    def update_laboratory_notebook(self, planification_id, technician_replaced_id, technician_substitute_id):
        NotebookLineProfessional = Pool().get('lims.notebook.line-laboratory.professional')
        notebook_line_professional = NotebookLineProfessional.search([
         (
          'notebook_line.planification', '=', planification_id),
         (
          'professional', '=', technician_replaced_id)])
        NotebookLineProfessional.write(notebook_line_professional, {'professional': technician_substitute_id})


class LoadServices(Wizard):
    __doc__ = 'Load Services'
    ...
    def transition_check(self):
        ...
        if not (fraction and fraction.cie_fraction_type and fraction.cie_original_fraction):
            return 'end'
        return 'load'
    ...
    def transition_load(self):
        ...
        services = Service.search([
         (
          'fraction', '=', original_fraction)])
        for service in services:
            if not Analysis.is_typified(service.analysis, new_fraction.product_type, new_fraction.matrix):
                continue
            new_service, = Service.copy([service], default={'fraction': new_fraction.id})
...
class PlanificationSequenceReport(Report):
    ...
    def get_context(cls, records, data):
        report_context = super(PlanificationSequenceReport, cls).get_context(records, data)
        ...
        for planification in records:
            ...
            if date not in objects:
                objects[date] = {'date':planification.start_date,  'methods':{}}
            for detail in planification.details:
                fraction = detail.fraction
                for service_detail in detail.details:
                    if service_detail.notebook_line.analysis.behavior == 'internal_relation':
                        continue
                    notebook_line = service_detail.notebook_line
                    method_id = notebook_line.method.id
                    if method_id not in objects[date]['methods']:
                        objects[date]['methods'][method_id] = {'method':notebook_line.method.code,  'lines':{}}
                    number = fraction.get_formated_number('sn-sy-fn')
                    number = number + '-' + str(notebook_line.repetition)
                    number_parts = number.split('-')
                    order = number_parts[1] + '-' + number_parts[0] + '-' + number_parts[2] + '-' + number_parts[3]
                    ...
                    sample_client_description = fraction.sample.sample_client_description
                    ...
                    if key not in objects[date]['methods'][method_id]['lines']:
                        objects[date]['methods'][method_id]['lines'][key] = {'order':order,  'number':number, 
                         ...
                         'urgent':urgent, 
                         'comments':comments, 
                         'sample_client_description':sample_client_description}

        for k1 in objects.keys():
            for k2, lines in objects[k1]['methods'].items():
                sorted_lines = sorted((list(lines['lines'].values())), key=(lambda x: x['order']))
...
class PlanificationWorksheetAnalysisReport(Report):
    __doc__ = 'Worksheet by Analysis'
    __name__ = 'lims.planification.worksheet_analysis.report'

    @classmethod
    def get_context(cls, records, data):
        report_context = super(PlanificationWorksheetAnalysisReport, cls).get_context(records, data)
        report_context['company'] = report_context['user'].company
        objects = {}
        for planification in records:
            if planification.state != 'confirmed':
            ...
            date = str(planification.start_date)
            if date not in objects:
                objects[date] = {'date':planification.start_date,  'professionals':{}}
            for detail in planification.details:
                fraction = detail.fraction
                for service_detail in detail.details:
                    ...
                    if p_key not in objects[date]['professionals']:
                        objects[date]['professionals'][p_key] = {'professional':p_names,  'analysis':{},  'total':0}
                    notebook_line = service_detail.notebook_line
                    key = (notebook_line.analysis.id, notebook_line.method.id)
                    if key not in objects[date]['professionals'][p_key]['analysis']:
                        objects[date]['professionals'][p_key]['analysis'][key] = {'order':notebook_line.analysis.order or 9999, 
                    ...
                    number = number + '-' + str(notebook_line.repetition)
                    concentration_level = notebook_line.concentration_level.description if notebook_line.concentration_level else ''
                    if number in objects[date]['professionals'][p_key]['analysis'][key]['lines']:
                        continue
                    number_parts = number.split('-')
                    order = number_parts[3] + '-' + number_parts[2] + '-' + number_parts[4] + '-' + number_parts[5]
                    ...
                    record = {'order':order, 
                     ...
                     'device':notebook_line.device.code if notebook_line.device else None, 
                     'urgent':'SI' if notebook_line.urgent else '', 
                     'comments':comments, 
                     'planification_code':planification.code}
                    objects[date]['professionals'][p_key]['analysis'][key]['lines'][number] = record
                    objects[date]['professionals'][p_key]['total'] += 1

        for k1 in objects.keys():
            for k2 in objects[k1]['professionals'].keys():
                sorted_analysis = sorted((list(objects[k1]['professionals'][k2]['analysis'].items())),
                  key=(lambda x: x[1]['order']))
                objects[k1]['professionals'][k2]['analysis'] = []
                for item in sorted_analysis:
                    sorted_lines = sorted((list(item[1]['lines'].items())), key=(lambda x: x[1]['order']))
                    item[1]['lines'] = [l[1] for l in sorted_lines]
                    objects[k1]['professionals'][k2]['analysis'].append(item[1])
...
class PlanificationWorksheetMethodReport(Report):
    __doc__ = 'Worksheet by Method'
    __name__ = 'lims.planification.worksheet_method.report'

    @classmethod
    def get_context(cls, records, data):
        report_context = super(PlanificationWorksheetMethodReport, cls).get_context(records, data)
        report_context['company'] = report_context['user'].company
        objects = {}
        for planification in records:
            if planification.state != 'confirmed':
            ...
            for detail in planification.details:
                ...
                for service_detail in detail.details:
                    ...
                    p_key = tuple(sorted(p_key))
                    if p_key not in objects[date]['professionals']:
                        objects[date]['professionals'][p_key] = {'professional':p_names,  'total':0, 
                         'lines':{}}
                    notebook_line = service_detail.notebook_line
                    number = fraction.get_formated_number('pt-m-sy-sn-fn')
                    number = number + '-' + str(notebook_line.repetition)
                    concentration_level = notebook_line.concentration_level.description if notebook_line.concentration_level else ''
                    if number not in objects[date]['professionals'][p_key]['lines']:
                        number_parts = number.split('-')
                        order = number_parts[3] + '-' + number_parts[2] + '-' + number_parts[4] + '-' + number_parts[5]
                        comments_planif = '. '.join(cls.get_planning_legend(fraction, planification))
                        if comments_planif:
                            comments = '%s - %s - %s - %s' % (
                            ...
                            comments = '%s - %s - %s' % (
                             planification.comments or '', notebook_line.service.comments or '',
                             notebook_line.service.fraction.comments or '')
                        if fraction.packages_quantity != 0.0:
                            pack_quant = str(fraction.packages_quantity)
                        else:
                            pack_quant = ''
                        record = {'order':order,  'number':number, 
                         'label':fraction.label, 
                         'sample_client_description':fraction.sample.sample_client_description, 
                         'party':fraction.party.code, 
                         'storage_location':fraction.storage_location.code, 
                         ...
                         'comments':comments, 
                         'planification_code':planification.code, 
                         'package_type':pack_quant + ' ' + fraction.package_type.description, 
                         'methods':{}}
                        objects[date]['professionals'][p_key]['lines'][number] = record
                        objects[date]['professionals'][p_key]['total'] += 1
                    if notebook_line.method.id not in objects[date]['professionals'][p_key]['lines'][number]['methods']:
                        objects[date]['professionals'][p_key]['lines'][number]['methods'][notebook_line.method.id] = notebook_line.method.rec_name
        ...
        for k1 in objects.keys():
            for k2 in objects[k1]['professionals'].keys():
                ...
                for fraction in fractions:
                    ...
                    if m_key not in objects[k1]['professionals'][k2]['methods']:
                        objects[k1]['professionals'][k2]['methods'][m_key] = {'methods':m_names,  'lines':[]}
                    objects[k1]['professionals'][k2]['methods'][m_key]['lines'].append(fraction)

                del objects[k1]['professionals'][k2]['lines']
                for m_key in objects[k1]['professionals'][k2]['methods'].keys():
                    sorted_lines = sorted((objects[k1]['professionals'][k2]['methods'][m_key]['lines']),
                      key=(lambda x: x['order']))
                    objects[k1]['professionals'][k2]['methods'][m_key]['lines'] = sorted_lines
    ...
    def get_planning_legend(cls, fraction, planification):
        ...
        PlanificationDetail = pool.get('lims.planification.detail')
        NotebookLine = pool.get('lims.notebook.line')
        cursor.execute('SELECT DISTINCT(a.planning_legend) FROM "' + PlanificationDetail._table + '" pd INNER JOIN "' + PlanificationServiceDetail._table + '" psd ON pd.id = psd.detail INNER JOIN "' + NotebookLine._table + '" nl ON nl.id = psd.notebook_line INNER JOIN "' + Analysis._table + '" a ON a.id = nl.analysis WHERE pd.planification = %s AND pd.fraction = %s AND a.planning_legend IS NOT NULL ', (
...
class PlanificationWorksheetReport(Report):
    ...
    @classmethod
    def get_context(cls, records, data):
        report_context = super(PlanificationWorksheetReport, cls).get_context(records, data)
        report_context['company'] = report_context['user'].company
        objects = {}
        for planification in records:
            if planification.state != 'confirmed':
            ...
            for detail in planification.details:
                ...
                for service_detail in detail.details:
                    ...
                    if key not in objects[date]['professionals'][p_key]['analysis']:
                        objects[date]['professionals'][p_key]['analysis'][key] = {'analysis':service_detail.planned_service.rec_name, 
                         'lines':{}}
                    number = fraction.get_formated_number('pt-m-sy-sn-fn')
                    number = number + '-' + str(notebook_line.repetition)
                    concentration_level = notebook_line.concentration_level.description if notebook_line.concentration_level else ''
                    if number not in objects[date]['professionals'][p_key]['analysis'][key]['lines']:
                        number_parts = number.split('-')
                        order = number_parts[3] + '-' + number_parts[2] + '-' + number_parts[4] + '-' + number_parts[5]
                        comments_planif = '. '.join(cls.get_planning_legend(fraction, planification))
                        if comments_planif:
                        ...
                        record = {'order':order,  'number':number, 
                         'label':fraction.label, 
                         'sample_client_description':fraction.sample.sample_client_description if fraction.sample.sample_client_description else '', 
                         'party':fraction.party.code, 
                         'storage_location':fraction.storage_location.code, 
                         'fraction_type':fraction.type.code, 
                         'concentration_level':concentration_level, 
                        ...
                        objects[date]['professionals'][p_key]['analysis'][key]['lines'][number] = record
                        objects[date]['professionals'][p_key]['total'] += 1
                    if notebook_line.method.id not in objects[date]['professionals'][p_key]['analysis'][key]['lines'][number]['methods']:
                        objects[date]['professionals'][p_key]['analysis'][key]['lines'][number]['methods'][notebook_line.method.id] = notebook_line.method.rec_name

        for k1 in objects.keys():
            for k2 in objects[k1]['professionals'].keys():
                for k3 in objects[k1]['professionals'][k2]['analysis'].keys():
                    objects[k1]['professionals'][k2]['analysis'][k3]['methods'] = {}
                    fractions = list(objects[k1]['professionals'][k2]['analysis'][k3]['lines'].values())
                    for fraction in fractions:
                        m_key = ()
                        m_names = []
                        for m_id, m_name in fraction['methods'].items():
                        ...
                        m_key = tuple(sorted(m_key))
                        if m_key not in objects[k1]['professionals'][k2]['analysis'][k3]['methods']:
                            objects[k1]['professionals'][k2]['analysis'][k3]['methods'][m_key] = {'methods':m_names, 
                             'lines':[]}
                        objects[k1]['professionals'][k2]['analysis'][k3]['methods'][m_key]['lines'].append(fraction)

                    del objects[k1]['professionals'][k2]['analysis'][k3]['lines']
                    for m_key in objects[k1]['professionals'][k2]['analysis'][k3]['methods'].keys():
                        sorted_lines = sorted((objects[k1]['professionals'][k2]['analysis'][k3]['methods'][m_key]['lines']),
                          key=(lambda x: x['order']))
                        objects[k1]['professionals'][k2]['analysis'][k3]['methods'][m_key]['lines'] = sorted_lines
...
class PrintPendingServicesUnplannedReportStart(ModelView):
    __doc__ = 'Print Pending Services Unplanned Report Start'
...
class PrintPendingServicesUnplannedReport(Wizard):
    __doc__ = 'Print Pending Services Unplanned Report'
    __name__ = 'lims.pending_services_unplanned'
    start = StateView('lims.pending_services_unplanned.start', 'lims.print_pending_services_unplanned_report_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Print', 'print_', 'tryton-print', default=True),
     Button('Save', 'save', 'tryton-save')])
    print_ = StateAction('lims.report_pending_services_unplanned')
    save = StateAction('lims.report_pending_services_unplanned_spreadsheet')

    def do_print_(self, action):
        data = {'start_date':self.start.start_date, 
...
class PendingServicesUnplannedReport(Report):
    __doc__ = 'Pending Services Unplanned'
    __name__ = 'lims.pending_services_unplanned.report'

    @classmethod
    def get_context(cls, records, data):
        report_context = super(PendingServicesUnplannedReport, cls).get_context(records, data)
        pool = Pool()
        Service = pool.get('lims.service')
        Laboratory = pool.get('lims.laboratory')
        if report_context['user'].laboratory:
            labs = [
             report_context['user'].laboratory.id]
        else:
        ...
        report_context['start_date'] = data['start_date'] if data['start_date'] else ''
        report_context['end_date'] = data['end_date'] if data['end_date'] else ''
        ...
        for service in pending_services:
            ...
            if laboratory.id not in objects:
                objects[laboratory.id] = {'laboratory':laboratory.rec_name,  'services':{},  'total':0}
            analysis = service.analysis
            if analysis.id not in objects[laboratory.id]['services']:
                objects[laboratory.id]['services'][analysis.id] = {'service':analysis.rec_name,  'parties':{},  'total':0}
            party = service.party
            if party.id not in objects[laboratory.id]['services'][analysis.id]['parties']:
                objects[laboratory.id]['services'][analysis.id]['parties'][party.id] = {'party':party.code, 
                 'lines':[],  'total':0}
            number = service.fraction.get_formated_number('pt-m-sn-sy-fn')
            number = number + '-' + str(service.sample.label)
            number_parts = number.split('-')
            order = number_parts[3] + '-' + number_parts[2] + '-' + number_parts[4]
            result_estimated = []
            ...
            if result_estimated:
                ...
                result_estimated_date = confirmation_date + relativedelta(days=c_days)
            if data['include_method']:
                ...
                for method in method_pending:
                    record = {'order':order,  'number':number, 
                     ...
                     'sample_client_description':service.sample.sample_client_description, 
                     'current_location':service.fraction.current_location.code if service.fraction.current_location else '', 
                     ...
                     'confirmation_date':service.confirmation_date if service.confirmation_date else '', 
                     'results_estimated_date':result_estimated_date if result_estimated_date else ''}
                    objects[laboratory.id]['services'][analysis.id]['parties'][party.id]['lines'].append(record)
                    objects[laboratory.id]['total'] += 1
                    objects[laboratory.id]['services'][analysis.id]['total'] += 1
                    objects[laboratory.id]['services'][analysis.id]['parties'][party.id]['total'] += 1
        ...
        for k1 in objects.keys():
            for k2 in objects[k1]['services'].keys():
                for k3, lines in objects[k1]['services'][k2]['parties'].items():
                    sorted_lines = sorted((lines['lines']), key=(lambda x: x['order']))
                    objects[k1]['services'][k2]['parties'][k3]['lines'] = sorted_lines
    ...
    def get_unplanned_services(cls):
        ...
        EntryDetailAnalysis = pool.get('lims.entry.detail.analysis')
        Analysis = pool.get('lims.analysis')
        cursor.execute('SELECT DISTINCT(d.service) FROM "' + EntryDetailAnalysis._table + '" d INNER JOIN "' + Analysis._table + '" a ON a.id = d.analysis WHERE d.state = \'unplanned\' AND a.behavior != \'internal_relation\'')
    ...
    def get_unplanned_method(cls, service_id):
        ...
        Analysis = pool.get('lims.analysis')
        LabMethod = pool.get('lims.lab.method')
        cursor.execute('SELECT DISTINCT(m.code) FROM "' + EntryDetailAnalysis._table + '" d INNER JOIN "' + Analysis._table + '" a ON a.id = d.analysis INNER JOIN"' + LabMethod._table + '" m ON d.method = m.id WHERE d.service = %s AND d.state = \'unplanned\' AND a.behavior != \'internal_relation\'', (
         service_id,))
    ...
    def get_service_laboratory(cls, service_id):
        ...
        Service = pool.get('lims.service')
        Laboratory = pool.get('lims.laboratory')
        cursor.execute('SELECT d.laboratory FROM "' + EntryDetailAnalysis._table + '" d INNER JOIN "' + Service._table + '" s ON s.id = d.service WHERE s.id = %s ORDER BY d.id ASC LIMIT 1', (
         service_id,))
    ...
    def get_results_estimated(cls, service_id):
        ...
        NotebookLine = pool.get('lims.notebook.line')
        Analysis = pool.get('lims.analysis')
        cursor.execute('SELECT  d.confirmation_date, n.results_estimated_waiting FROM "' + NotebookLine._table + '" n INNER JOIN "' + EntryDetailAnalysis._table + '" d ON (d.service = n.service  AND d.analysis = n.analysis) INNER JOIN "' + Analysis._table + '" a ON a.id = d.analysis WHERE d.state = \'unplanned\' AND d.service = %s AND n.results_estimated_waiting IS NOT Null AND a.behavior != \'internal_relation\'ORDER BY n.results_estimated_waiting ASC LIMIT 1', (
         service_id,))
...
class PendingServicesUnplannedSpreadsheet(Report):
    ...
    @classmethod
    def get_context(cls, records, data):
        report_context = super(PendingServicesUnplannedSpreadsheet, cls).get_context(records, data)
        pool = Pool()
        Service = pool.get('lims.service')
        Laboratory = pool.get('lims.laboratory')
        if report_context['user'].laboratory:
        ...
        for service in pending_services:
            ...
            if report_date:
                ...
                if data['include_method']:
                    method_pending = cls.get_unplanned_method(service.id)
                    for method in method_pending:
                        record = {'laboratory':laboratory.rec_name,  'service':service.analysis.rec_name, 
                         'method':method[0], 
                         ...
                         'confirmation_date':service.confirmation_date if service.confirmation_date else '', 
                         'results_estimated_date':result_estimated_date if result_estimated_date else '', 
                         'results_estimated_waiting':result_estimated_waiting if result_estimated_waiting else '', 
                         'notice':notice if notice else ''}
                        objects.append(record)

            else:
                record = {'laboratory':laboratory.rec_name, 
...
class PrintBlindSampleReport(Wizard):
    ...
    start = StateView('lims.print_blind_sample_report.start', 'lims.lims_print_blind_sample_report_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Print', 'print_', 'tryton-print', default=True)])
    ...
    def do_print_(self, action):
        ...
        blind_samples = BlindSample.search_count([
         ...
         ('line.result', '!=', None)])
        if blind_samples > 0:
            data = {'date_from':self.start.date_from,  'date_to':self.start.date_to}
            return (
             action, data)
...
class BlindSampleReport(Report):
    ...
    @classmethod
    def get_context(cls, records, data):
        report_context = super(BlindSampleReport, cls).get_context(records, data)
        BlindSample = Pool().get('lims.blind_sample')
        report_context['company'] = report_context['user'].company
        objects = []
        ...
        for bs in blind_samples:
            if bs.line.converted_result:
                ...
                if bs.line.result:
                    ...
                    unit = None
                    concentration = ''
            record = {'date':bs.date,  'fraction':bs.fraction.rec_name, 
             'report':bs.line.results_report.number if bs.line.results_report else '', 
            ...
            if bs.original_fraction:
                ...
                if bs.original_line:
                    if bs.original_line.results_report:
                        record['original_report'] = bs.original_line.results_report.number
                    elif bs.original_line.converted_result:
                        original_result = float(bs.original_line.converted_result)
                        original_unit = bs.original_line.final_unit
                        original_concentration = bs.original_line.final_concentration
                    else:
                        if bs.original_line.result:
                            original_result = float(bs.original_line.result)
                            original_unit = bs.original_line.initial_unit
                            original_concentration = bs.original_line.initial_concentration
                        else:
                            original_result = None
                    if original_result:
                        original_result = round(original_result, bs.original_line.decimals)
                        record['original_result'] = original_result
                        record['original_unit'] = original_unit.symbol if original_unit else ''
                        record['original_concentration'] = original_concentration
                        if unit == original_unit:
                            average = (result + original_result) / 2
                            difference = result - original_result
                            record['error'] = round(difference * 100 / average, 2)
                            try:
                                maximum_concentration = float(unit.maximum_concentration)
                            ...
                            if result <= maximum_concentration:
                                record['max_sd'] = round(average * rsd_horwitz, 2)
                                record['two_max_sd'] = round(record['max_sd'] * 2, 2)
...
class PlanificationSequenceAnalysisReport(Report):
    __doc__ = 'Sequence Analysis'
    __name__ = 'lims.planification.sequence.analysis.report'

    @classmethod
    def get_context(cls, records, data):
        report_context = super(PlanificationSequenceAnalysisReport, cls).get_context(records, data)
        report_context['company'] = report_context['user'].company
        objects = {}
        for planification in records:
            if planification.state != 'confirmed':
