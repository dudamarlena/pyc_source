...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/trytond/modules/lims/tests/tools.py
# Compiled at: 2018-04-28 19:32:54
# Size of source mod 2**32: 11232 bytes
import datetime
...
def create_workyear(company=None, today=None, config=None):
    ...
    workyear.save()
    default_entry_control = _create_default_entry_control(company, today, config)
...
def _create_default_entry_control(company=None, today=None, config=None):
    ...
    AcknowledgmentContact = Model.get('lims.entry.acknowledgment_contacts', config=config)
    default_entry_control = Entry()
    default_entry_control.date = datetime.datetime.combine(today, datetime.time.min)
    ...
    default_entry_control.acknowledgment_contacts.append(acknowledgment_contact)
...
def _create_base_entry_tables(config=None):
    ...
    EntrySuspensionReason = Model.get('lims.entry.suspension.reason', config=config)
    packaging_type = PackagingType(code='01',
      description='Package')
    packaging_type.save()
    packaging_integrity = PackagingIntegrity(code='OK',
      description='Ok')
    packaging_integrity.save()
    zone = Zone(code='N',
      description='North')
    zone.save()
    suspension_reason = EntrySuspensionReason(code='01',
...
def _create_base_laboratory_tables(config=None):
    ...
    LaboratoryProfessional = Model.get('lims.laboratory.professional', config=config)
    ...
    lims_user = User(1)
    party = Party(name='Laboratory Professional',
      is_lab_professional=True,
      lims_user=lims_user)
    party.save()
    professional = LaboratoryProfessional(party=party,
      code='LP',
      role='Responsible')
    professional.save()
    related_location, = Location.find([('code', '=', 'STO')])
    laboratory = Laboratory(code='SQ',
    ...
    lims_user.save()
    device_type = LabDeviceType(code='PH',
      description='pH Meters')
    device_type.save()
    device = LabDevice(code='PH01',
...
def _create_base_tables(config=None):
    ...
    product_type = ProductType(code='WINE',
      description='Wine')
    product_type.save()
    matrix = Matrix(code='GRAPE',
      description='Grape')
    matrix.save()
    method = LabMethod(code='002',
    ...
    typification = Typification(product_type=product_type,
      matrix=matrix,
      analysis=analysis,
      method=method)
    typification.save()
    fraction_type = FractionType(code='MCL',
