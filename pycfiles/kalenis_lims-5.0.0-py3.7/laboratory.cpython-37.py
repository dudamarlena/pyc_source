...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/trytond/modules/lims/laboratory.py
# Compiled at: 2018-11-25 12:10:02
# Size of source mod 2**32: 11142 bytes
from trytond.model import ModelView, ModelSQL, fields, Unique
from trytond.pool import Pool
from trytond.transaction import Transaction
__all__ = [
 'LaboratoryProfessional', 'Laboratory', 'LaboratoryCVCorrection',
...
class Laboratory(ModelSQL, ModelView):
    ...
    default_laboratory_professional = fields.Many2One('lims.laboratory.professional', 'Default professional')
    default_signer = fields.Many2One('lims.laboratory.professional', 'Default signer',
      required=True)
    related_location = fields.Many2One('stock.location', 'Related location', required=True,
      domain=[('type', '=', 'storage')])
    cv_corrections = fields.One2Many('lims.laboratory.cv_correction', 'laboratory',
      'CV Corrections', help='Corrections for Coefficients of Variation (Control Charts)')
...
class LaboratoryCVCorrection(ModelSQL, ModelView):
    ...
    min_cv_corr_fact = fields.Float('Correction factor for Minimum CV', help='Correction factor for CV between Min and Max')
    max_cv_corr_fact = fields.Float('Correction factor for Maximum CV', help='Correction factor for CV greater than Max')


class LaboratoryProfessional(ModelSQL, ModelView):
    __doc__ = 'Laboratory Professional'
    ...
    methods = fields.One2Many('lims.lab.professional.method', 'professional', 'Methods')
    ...
    def get_lab_professional(cls):
        ...
        cursor.execute('SELECT id FROM party_party WHERE is_lab_professional = true AND lims_user = %s LIMIT 1', (
         login_user_id,))
        party_id = cursor.fetchone()
        if not party_id:
            return
        cursor.execute('SELECT id FROM "' + cls._table + '" WHERE party = %s LIMIT 1', (
         party_id[0],))
...
class LabMethod(ModelSQL, ModelView):
    __doc__ = 'Laboratory Method'
    __name__ = 'lims.lab.method'
    code = fields.Char('Code', required=True)
    name = fields.Char('Name', required=True, translate=True)
    reference = fields.Char('Reference')
    determination = fields.Char('Determination', required=True)
    requalification_months = fields.Integer('Requalification months', required=True)
    ...
    def update_laboratory_notebook(cls, methods):
        ...
        for method in methods:
            notebook_lines = NotebookLine.search([
             ...
             ('accepted', '=', False)])
            if notebook_lines:
                NotebookLine.write(notebook_lines, {'results_estimated_waiting': method.results_estimated_waiting})


class LabDevice(ModelSQL, ModelView):
    __doc__ = 'Laboratory Device'
    __name__ = 'lims.lab.device'
    _rec_name = 'description'
    code = fields.Char('Code', required=True)
    description = fields.Char('Description', required=True)
    device_type = fields.Many2One('lims.lab.device.type', 'Device type', required=True)
    laboratories = fields.One2Many('lims.lab.device.laboratory', 'device', 'Laboratories',
      required=True)
...
class LabDeviceType(ModelSQL, ModelView):
    __doc__ = 'Laboratory Device Type'
    __name__ = 'lims.lab.device.type'
    _rec_name = 'description'
    code = fields.Char('Code', required=True)
    description = fields.Char('Description', required=True)
    methods = fields.Many2Many('lims.lab.device.type-lab.method', 'device_type', 'method', 'Methods')
...
class LabDeviceTypeLabMethod(ModelSQL):
    __doc__ = 'Laboratory Device Type - Laboratory Method'
    __name__ = 'lims.lab.device.type-lab.method'
    device_type = fields.Many2One('lims.lab.device.type', 'Device type', ondelete='CASCADE',
      select=True,
      required=True)
...
class LabDeviceLaboratory(ModelSQL, ModelView):
    __doc__ = 'Laboratory Device Laboratory'
    __name__ = 'lims.lab.device.laboratory'
    device = fields.Many2One('lims.lab.device', 'Device', required=True, ondelete='CASCADE',
      select=True)
    laboratory = fields.Many2One('lims.laboratory', 'Laboratory', required=True)
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update({'physically_elsewhere': 'This Device is physically in another Laboratory'})
