...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/trytond/modules/lims/control_tendency.py
# Compiled at: 2019-01-16 09:41:19
# Size of source mod 2**32: 55332 bytes
import logging
...
__all__ = [
 'RangeType', 'Range', 'ControlTendency', 'ControlTendencyDetail',
...
try:
    ...
    logging.getLogger(__name__).warning('Unable to import pandas. Plotting disabled.',
      exc_info=True)
...
class RangeType(ModelSQL, ModelView):
    ...
    resultrange_title = fields.Char('Column Title in Results report', translate=True,
      states={'invisible': Eval('use') != 'result_range'},
      depends=[
     'use'])
    resultrange_comments = fields.Char('Comments in Results report', translate=True,
      states={'invisible': Eval('use') != 'result_range'},
      depends=[
     'use'])
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update({'default_range_type': 'There is already a default origin for this use'})
...
class Range(ModelSQL, ModelView):
    ...
    low_level = fields.Float('Low level', digits=(16, 3), states={'required': Bool(Eval('low_level_value'))},
      depends=[
     'low_level_value'])
    middle_level = fields.Float('Middle level', digits=(16, 3), states={'required': Bool(Eval('middle_level_value'))},
      depends=[
     'middle_level_value'])
    high_level = fields.Float('High level', digits=(16, 3), states={'required': Bool(Eval('high_level_value'))},
      depends=[
     'high_level_value'])
    ...
    low_level_coefficient_variation = fields.Float('Low level coefficient of variation',
      digits=(16, 3))
    middle_level_coefficient_variation = fields.Float('Middle level coefficient of variation',
      digits=(16, 3))
    high_level_coefficient_variation = fields.Float('High level coefficient of variation',
      digits=(16, 3))


class ControlTendency(ModelSQL, ModelView):
    __doc__ = 'Control Chart Tendency'
    __name__ = 'lims.control.tendency'
    product_type = fields.Many2One('lims.product.type', 'Product type', required=True,
      states={'readonly': Bool(Eval('context', {}).get('readonly', False))})
    matrix = fields.Many2One('lims.matrix', 'Matrix', required=True, states={'readonly': Bool(Eval('context', {}).get('readonly', False))})
    fraction_type = fields.Many2One('lims.fraction.type', 'Fraction type', required=True,
      states={'readonly': Bool(Eval('context', {}).get('readonly', False))})
    analysis = fields.Many2One('lims.analysis', 'Analysis', required=True, states={'readonly': Bool(Eval('context', {}).get('readonly', False))})
    concentration_level = fields.Many2One('lims.concentration.level', 'Concentration level',
      states={'readonly': Bool(Eval('context', {}).get('readonly', False))})
    mean = fields.Float('Mean', required=True, states={'readonly': Bool(Eval('context', {}).get('readonly', False))},
      digits=(
     16, Eval('digits', 2)),
      depends=['digits'])
    deviation = fields.Float('Standard Deviation', required=True, digits=(
     16, Eval('digits', 2)),
      depends=['digits'])
    one_sd = fields.Function(fields.Float('1 SD', digits=(16,
     Eval('digits', 2)),
      depends=['deviation', 'digits']), 'get_one_sd')
    two_sd = fields.Function(fields.Float('2 SD', digits=(16,
     Eval('digits', 2)),
      depends=['deviation', 'digits']), 'get_two_sd')
    three_sd = fields.Function(fields.Float('3 SD', digits=(16,
     Eval('digits', 2)),
      depends=['deviation', 'digits']), 'get_three_sd')
    cv = fields.Function(fields.Float('CV (%)', digits=(16, Eval('digits', 2)), depends=[
     'deviation', 'mean', 'digits']), 'get_cv')
    min_cv = fields.Float('Minimum CV (%)', digits=(16, Eval('digits', 2)), depends=[
     'digits'])
    max_cv = fields.Float('Maximum CV (%)', digits=(16, Eval('digits', 2)), depends=[
     'digits'])
    min_cv_corr_fact = fields.Float('Correction factor for Minimum CV', digits=(
     16, Eval('digits', 2)),
      depends=['digits'])
    max_cv_corr_fact = fields.Float('Correction factor for Maximum CV', digits=(
     16, Eval('digits', 2)),
      depends=['digits'])
    one_sd_adj = fields.Function(fields.Float('1 SD Adjusted', digits=(
     16, Eval('digits', 2)),
      depends=['cv', 'one_sd', 'min_cv',
     'max_cv', 'min_cv_corr_fact', 'max_cv_corr_fact', 'digits']), 'get_one_sd_adj')
    two_sd_adj = fields.Function(fields.Float('2 SD Adjusted', digits=(
     16, Eval('digits', 2)),
      depends=['cv', 'two_sd', 'min_cv',
     'max_cv', 'min_cv_corr_fact', 'max_cv_corr_fact', 'digits']), 'get_two_sd_adj')
    three_sd_adj = fields.Function(fields.Float('3 SD Adjusted', digits=(
     16, Eval('digits', 2)),
      depends=['cv', 'three_sd', 'min_cv',
     'max_cv', 'min_cv_corr_fact', 'max_cv_corr_fact', 'digits']), 'get_three_sd_adj')
    ucl = fields.Function(fields.Float('UCL', digits=(16, Eval('digits', 2)), depends=[
     'mean', 'three_sd_adj', 'digits']), 'get_ucl')
    uwl = fields.Function(fields.Float('UWL', digits=(16, Eval('digits', 2)), depends=[
     'mean', 'two_sd_adj', 'digits']), 'get_uwl')
    upl = fields.Function(fields.Float('UPL', digits=(16, Eval('digits', 2)), depends=[
     'mean', 'one_sd_adj', 'digits']), 'get_upl')
    lcl = fields.Function(fields.Float('LCL', digits=(16, Eval('digits', 2)), depends=[
     'mean', 'three_sd_adj', 'digits']), 'get_lcl')
    lwl = fields.Function(fields.Float('LWL', digits=(16, Eval('digits', 2)), depends=[
     'mean', 'two_sd_adj', 'digits']), 'get_lwl')
    lpl = fields.Function(fields.Float('LPL', digits=(16, Eval('digits', 2)), depends=[
     'mean', 'one_sd_adj', 'digits']), 'get_lpl')
    cl = fields.Function(fields.Float('CL', digits=(16, Eval('digits', 2)), depends=[
     'mean', 'digits']), 'get_cl')
    details = fields.One2Many('lims.control.tendency.detail', 'tendency', 'Details',
      readonly=True)
    ...
    def get_three_sd_adj(self, name=None):
        if self.cv < self.min_cv:
            ...
            if self.cv < self.max_cv:
                if self.min_cv_corr_fact:
                    return round(self.three_sd / self.min_cv_corr_fact, self.digits)
        elif self.max_cv_corr_fact:
            return round(self.three_sd / self.max_cv_corr_fact, self.digits)
...
class ControlTendencyDetail(ModelSQL, ModelView):
    __doc__ = 'Control Chart Tendency Detail'
    __name__ = 'lims.control.tendency.detail'
    tendency = fields.Many2One('lims.control.tendency', 'Tendency', ondelete='CASCADE',
      select=True,
      required=True)
    ...
    rules = fields.One2Many('lims.control.tendency.detail.rule', 'detail', 'Rules')
    rules2 = fields.Function(fields.Char('Rules', depends=['rules']), 'get_rules2')
    ...
    def get_icon(self, name):
        if self.rule in ('1', '2', '3', '4'):
            return {'1':'lims-green', 
             '2':'lims-blue', 
             '3':'lims-yellow', 
             '4':'lims-red'}[self.rule]


class ControlTendencyDetailRule(ModelSQL):
    __doc__ = 'Control Chart Tendency Detail Rule'
    __name__ = 'lims.control.tendency.detail.rule'
    detail = fields.Many2One('lims.control.tendency.detail', 'Detail', ondelete='CASCADE',
      select=True,
      required=True)
    rule = fields.Char('Rule')
...
class MeansDeviationsCalcStart(ModelView):
    __doc__ = 'Calculation of Means and Deviations'
    ...
    product_type = fields.Many2One('lims.product.type', 'Product type', domain=[
     (
    ...
    matrix = fields.Many2One('lims.matrix', 'Matrix', domain=[
     (
    ...
    fraction_type = fields.Many2One('lims.fraction.type', 'Fraction type', domain=[
     ('control_charts', '=', True)],
      required=True)
    ...
    def on_change_with_product_type_domain(self, name=None):
        cursor = Transaction().connection.cursor()
        AnalysisFamilyCertificant = Pool().get('lims.analysis.family.certificant')
        if not self.family:
            return self.default_product_type_domain()
        cursor.execute('SELECT DISTINCT(product_type) FROM "' + AnalysisFamilyCertificant._table + '" WHERE family = %s', (
         self.family.id,))
        return [x[0] for x in cursor.fetchall()]
    ...
    def on_change_with_matrix_domain(self, name=None):
        ...
        if not self.family:
            ...
            return [x[0] for x in cursor.fetchall()]
        cursor.execute('SELECT DISTINCT(matrix) FROM "' + AnalysisFamilyCertificant._table + '" WHERE product_type = %s AND family = %s', (
...
class ControlResultLine(ModelSQL, ModelView):
    __doc__ = 'Control Chart Result Line'
    ...
    one_sd = fields.Function(fields.Float('1 SD', depends=['deviation']), 'get_one_sd')
    two_sd = fields.Function(fields.Float('2 SD', depends=['deviation']), 'get_two_sd')
    three_sd = fields.Function(fields.Float('3 SD', depends=['deviation']), 'get_three_sd')
    ...
    details = fields.One2Many('lims.control.result_line.detail', 'line', 'Details',
      readonly=True)
    ...
    def __register__(cls, module_name):
        super(ControlResultLine, cls).__register__(module_name)
        cursor = Transaction().connection.cursor()
...
class ControlResultLineDetail(ModelSQL, ModelView):
    __doc__ = 'Control Chart Result Line Detail'
    __name__ = 'lims.control.result_line.detail'
    line = fields.Many2One('lims.control.result_line', 'Line', ondelete='CASCADE',
      select=True,
      required=True)
    date = fields.Date('Date')
    fraction = fields.Many2One('lims.fraction', 'Fraction')
...
class MeansDeviationsCalcResult2(ModelView):
    ...
    tendencies = fields.One2Many('lims.control.tendency', None, 'Tendencies', readonly=True)


class MeansDeviationsCalc(Wizard):
    __doc__ = 'Calculation of Means and Deviations'
    __name__ = 'lims.control.means_deviations_calc'
    start = StateView('lims.control.means_deviations_calc.start', 'lims.lims_control_means_deviations_calc_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Search', 'search', 'tryton-forward', default=True)])
    search = StateTransition()
    empty = StateView('lims.control.means_deviations_calc.empty', 'lims.lims_control_means_deviations_calc_empty_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Search again', 'start', 'tryton-forward', default=True)])
    result = StateView('lims.control.means_deviations_calc.result', 'lims.lims_control_means_deviations_calc_result_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Update', 'update', 'tryton-forward', default=True)])
    update = StateTransition()
    result2 = StateView('lims.control.means_deviations_calc.result2', 'lims.lims_control_means_deviations_calc_result2_view_form', [])
    ...
    def default_start(self, fields):
        ...
        for field in ('family', 'laboratory', 'product_type', 'matrix', 'fraction_type'):
            if hasattr(self.start, field) and getattr(self.start, field):
    ...
    def transition_search(self):
        ...
        if self.start.family:
            check_family = True
            families = []
            cursor.execute('SELECT product_type, matrix FROM "' + AnalysisFamilyCertificant._table + '" WHERE family = %s', (
        ...
        if lines:
            ...
            for line in lines:
                ...
                concentration_level_id = line.concentration_level.id if line.concentration_level else None
                key = (
                 product_type_id, matrix_id, analysis_id,
                 concentration_level_id)
                if key not in records:
                    records[key] = {'product_type':product_type_id,  'matrix':matrix_id, 
                     'fraction_type':fraction_type_id, 
                     'analysis':analysis_id, 
                     'concentration_level':concentration_level_id, 
                     'details':{}}
                records[key]['details'][line.id] = {'date':line.end_date, 
        ...
        for line in res_lines:
            ...
            tendency = ControlTendency.search([
             ...
             (
              'concentration_level', '=', concentration_level_id)])
            if tendency:
                ControlTendency.write(tendency, {'mean':line.mean, 
    ...
    def do_open(self, action):
        ...
        action['pyson_context'] = PYSONEncoder().encode({'readonly': True})
        self.result2.tendencies = None
        return (action, {})


class TendenciesAnalysisStart(ModelView):
    __doc__ = 'Tendencies Analysis'
...
class TendenciesAnalysis(Wizard):
    ...
    start = StateView('lims.control.tendencies_analysis.start', 'lims.lims_control_tendencies_analysis_start_view_form', [
     Button('Cancel', 'end', 'tryton-cancel'),
     Button('Open', 'search', 'tryton-ok', True)])
    search = StateTransition()
    result = StateView('lims.control.tendencies_analysis.result', 'lims.lims_control_tendencies_analysis_result_view_form', [])
    ...
    def transition_search(self):
        ...
        if tendencies:
            ...
            for tendency in tendencies:
                if check_family:
                    family_key = (
                     tendency.product_type.id, tendency.matrix.id)
                    if family_key not in families:
                        continue
                ...
                concentration_level_id = tendency.concentration_level.id if tendency.concentration_level else None
                clause = [
                 (
                  'laboratory', '=', self.start.laboratory.id),
                 (
                ...
                if lines:
                    ...
                    if prevs > 0:
                        ...
                        if prev_lines:
                            for line in prev_lines:
                                result = float(line.result if line.result else None)
                    ...
                    for line in lines:
                        ...
                        record = {'notebook_line':line.id,  'tendency':tendency.id, 
                         'date':line.end_date, 
                         'fraction':line.notebook.fraction.id, 
                         'device':line.device.id if line.device else None, 
                         'result':result, 
                        ...
                        if rules_to_create:
                            record['rules'] = [
                             (
                              'create', rules_to_create)]
                        to_create.append(record)

                    ControlTendencyDetail.create(to_create)
                    tendency_result.append(tendency)
                ControlTendency.write([tendency], {'rule_1_count':rule_1_count, 
    ...
    def get_rules(self, results, tendency):
        ...
        occurrences = 1
        total = 1
        if self._check_rule(results, upper_parameter, lower_parameter, occurrences, total):
            rules.append('4')
    ...
    def _check_rule(self, results, upper_parameter, lower_parameter, occurrences, total):
        ...
        for result in reversed(results):
            ...
            if result > upper_parameter:
                upper_counter += 1
                if total_counter == total and not upper_counter >= occurrences:
                    if lower_counter >= occurrences:
                        return True
                    return False
                    lower_counter = 0
                elif result < lower_parameter:
                    lower_counter += 1
                    if total_counter == total and not lower_counter >= occurrences:
                        if upper_counter >= occurrences:
                            return True
                        return False
                        upper_counter = 0
            elif total_counter == total and not upper_counter >= occurrences:
                if lower_counter >= occurrences:
                    return True
                return False
                upper_counter = 0
...
class PrintControlChart(Wizard):
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update({'number':'Measurement', 
         'result':'Result', 
         'ucl':'UCL (M+3D)', 
         'uwl':'UWL (M+2D)', 
         'upl':'UPL (M+D)', 
...
class ControlChartReport(Report):
    ...
    def get_context(cls, reports, data):
        report_context = super(ControlChartReport, cls).get_context(reports, data)
        pool = Pool()
    ...
    def _get_objects(cls, tendency):
        pool = Pool()
        PrintControlChart = pool.get('lims.control_chart.print', type='wizard')
        objects = {'number':{'name':PrintControlChart.raise_user_error('number', raise_exception=False), 
          'order':1,  'recs':{}}, 
         'result':{'name':PrintControlChart.raise_user_error('result', raise_exception=False), 
          'order':2,  'recs':{}}, 
         'ucl':{'name':PrintControlChart.raise_user_error('ucl', raise_exception=False), 
          'order':3,  'recs':{}}, 
         'uwl':{'name':PrintControlChart.raise_user_error('uwl', raise_exception=False), 
          'order':4,  'recs':{}}, 
         'upl':{'name':PrintControlChart.raise_user_error('upl', raise_exception=False), 
          'order':5,  'recs':{}}, 
         'cl':{'name':PrintControlChart.raise_user_error('cl', raise_exception=False), 
          'order':6,  'recs':{}}, 
         'lpl':{'name':PrintControlChart.raise_user_error('lpl', raise_exception=False), 
          'order':7,  'recs':{}}, 
         'lwl':{'name':PrintControlChart.raise_user_error('lwl', raise_exception=False), 
          'order':8,  'recs':{}}, 
         'lcl':{'name':PrintControlChart.raise_user_error('lcl', raise_exception=False), 
          'order':9,  'recs':{}}, 
         'cv':{'name':PrintControlChart.raise_user_error('cv', raise_exception=False), 
          'order':10,  'recs':{}}}
        count = 1
    ...
    def _get_plot(cls, columns, records):
        ...
        try:
            ax = df[[
             PrintControlChart.raise_user_error('ucl', raise_exception=False)]].plot(kind='line',
              color='red',
              rot=45,
              fontsize=7,
              figsize=(10, 7.5),
              linestyle='-')
            ax = df[[
             PrintControlChart.raise_user_error('uwl', raise_exception=False)]].plot(kind='line',
              color='orange',
              rot=45,
              fontsize=7,
              figsize=(10, 7.5),
              ...
              ax=ax)
            ax = df[[
             PrintControlChart.raise_user_error('upl', raise_exception=False)]].plot(kind='line',
              color='yellow',
              rot=45,
              fontsize=7,
              figsize=(10, 7.5),
              ...
              ax=ax)
            ax = df[[
             PrintControlChart.raise_user_error('cl', raise_exception=False)]].plot(kind='line',
              color='green',
              rot=45,
              fontsize=7,
              figsize=(10, 7.5),
              ...
              ax=ax)
            ax = df[[
             PrintControlChart.raise_user_error('lpl', raise_exception=False)]].plot(kind='line',
              color='yellow',
              rot=45,
              fontsize=7,
              figsize=(10, 7.5),
              ...
              ax=ax)
            ax = df[[
             PrintControlChart.raise_user_error('lwl', raise_exception=False)]].plot(kind='line',
              color='orange',
              rot=45,
              fontsize=7,
              figsize=(10, 7.5),
              ...
              ax=ax)
            ax = df[[
             PrintControlChart.raise_user_error('lcl', raise_exception=False)]].plot(kind='line',
              color='red',
              rot=45,
              fontsize=7,
              figsize=(10, 7.5),
              ...
              ax=ax)
            ax = df[[
             PrintControlChart.raise_user_error('result', raise_exception=False)]].plot(kind='line',
              color='blue',
              rot=45,
              fontsize=7,
              figsize=(10, 7.5),
