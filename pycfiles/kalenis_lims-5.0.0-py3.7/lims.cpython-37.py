...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/trytond/modules/lims_digital_sign/lims.py
# Compiled at: 2019-01-16 09:41:20
# Size of source mod 2**32: 14802 bytes
import os, time
...
import trytond.config as tconfig
from .tokenclient import GetToken
__all__ = [
 'ResultsReportVersionDetail', 'ResultsReport',
...
class ResultsReport(metaclass=PoolMeta):
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update({'polisample': 'Polisample'})
    ...
    def cron_digital_signs(cls):
        ...
        Cron - Digital Signs
        """
        logging.getLogger('lims_digital_sign').info('Cron - Digital Signs:INIT')
        pool = Pool()
        ...
        with Transaction().set_context(active_ids=[results_report.id for results_report in results_reports]):
            digital_sign.transition_sign()
        logging.getLogger('lims_digital_sign').info('Cron - Digital Signs:END')
        return True
    ...
    def build_report(self, english_report=False):
        ...
        if not details:
            logging.getLogger('lims_digital_sign').info('No %s details cached to build results report %s' % (
    ...
    def sign_report(self, output):
        ...
        try:
            ...
            try:
                logging.getLogger('lims_digital_sign').error('Unable to digitally sign for results report %s' % self.number)
    ...
    def mail_acknowledgment_of_results_report(self, spanish_report=False, english_report=False):
            ...
            for version in self.versions:
                for detail in version.details:
                    for line in detail.notebook_lines:
                        to_addrs.extend([c.contact.email for c in line.notebook_line.fraction.entry.report_contacts if c.contact.report_contact if not c.entry.invoice_party.block_reports_automatic_sending])
                        entries.append(line.notebook_line.fraction.entry.number)

        logging.getLogger('lims_digital_sign').info('Cron - Digital Signs:results_report.number:%s:to_addrs:%s' % (
         self.number, to_addrs and ', '.join(list(set(to_addrs))) or 'NONE'))
        logging.getLogger('lims_digital_sign').info('Cron - Digital Signs:results_report.number:%s:Entries:%s' % (
         self.number, entries and ', '.join(list(set(entries))) or 'NONE'))
        ...
        if english_report:
            attachments_data.append(self.attachment(english_report=english_report))
        msg = self.create_msg(from_addr, to_addrs, subject, body, attachments_data)
    ...
    def subject_body(self):
        ...
        with Transaction().set_context(language=(lang.code)):
            if len(sample_list) == 1:
                ...
                label = self.raise_user_error('polisample', raise_exception=False)
            subject = str('%s %s (%s)' % (
    ...
    def attachment(self, english_report=False):
        ...
        data = {'content':english_report and self.report_cache_eng or self.report_cache, 
         'format':english_report and self.report_format_eng or self.report_format, 
         'mimetype':'pdf', 
    ...
    def send_msg(self, from_addr, to_addrs, msg):
        ...
        try:
            ...
            logging.getLogger('lims_digital_sign').error('Unable to deliver mail for results report %s' % self.number)
...
class ResultsReportAnnulation(metaclass=PoolMeta):
    ...
    def transition_annul(self):
        logging.getLogger('lims_digital_sign').info('transition_annul():INIT')
        super(ResultsReportAnnulation, self).transition_annul()
        logging.getLogger('lims_digital_sign').info('transition_annul():INHERIT')
        ResultsReportVersionDetail = Pool().get('lims.results_report.version.detail')
        ...
        logging.getLogger('lims_digital_sign').info('transition_annul():END')
        return 'end'