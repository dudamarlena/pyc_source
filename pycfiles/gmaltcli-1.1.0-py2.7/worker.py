...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gmaltcli/worker.py
# Compiled at: 2017-08-14 06:33:50
import os, threading, logging, time, zipfile, hashlib
try:
    import queue
...
class WorkerPool(object):
    ...
    def __init__(self, worker, size, *args, **kwargs):
        ...
        self.workers = []
        for i in range(size):
            self.workers.append(worker((i + 1), self.queue, self.counter, self.stop_event, *args, **kwargs))
...
class DownloadWorker(Worker):
    ...
    def _validate_downloaded_file(filepath, md5sum):
        ...
        with open(filepath, 'rb') as (fp):
            ...
            if md5sum != md5digest:
                raise InvalidCheckSumException(('File {} md5 checksum does not match {}').format(filepath, md5sum))
