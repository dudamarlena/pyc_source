...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gmaltcli/app.py
# Compiled at: 2017-07-12 08:13:25
import logging, sys, argparse, sqlalchemy.exc, gmalthgtparser as hgt, gmaltcli.tools as tools, gmaltcli.worker as worker, gmaltcli.database as database
...
def create_read_from_hgt_parser():
    ...
    """
    parser = argparse.ArgumentParser(description='Pass along the latitude/longitude of the point you want to know the latitude of and a HGT file. It will look for the elevation of your point into the file and return it.')
...
def create_get_hgt_parser():
    ...
    parser.add_argument('dataset', type=tools.dataset_file, action=tools.LoadDatasetAction, help='A dataset file provided by this package or the path to your own dataset file. Please read documentation to get dataset JSON format')
    parser.add_argument('folder', type=tools.writable_folder, help='Path to the folder where the HGT zip will be downloaded or where the HGT zip have already been downloaded.')
    parser.add_argument('--skip-download', dest='skip_download', action='store_true', help="Set this flag if you don't want to download the zip files.")
    parser.add_argument('--skip-unzip', dest='skip_unzip', action='store_true', help="Set this flag if you don't want to unzip the HGT zip files")
    parser.add_argument('-c', type=int, dest='concurrency', default=1, help='How many worker will attempt to download or unzip files in parallel')
...
def get_hgt():
    ...
    try:
        tools.download_hgt_zip_files(args.folder, args.dataset_files, args.concurrency, skip=args.skip_download)
...
def create_load_hgt_parser():
    ...
    parser.add_argument('folder', type=tools.existing_folder, help='Path to the folder where the HGT files are stored.')
    parser.add_argument('-c', type=int, dest='concurrency', default=1, help='How many worker will attempt to load files in parallel')
    parser.add_argument('-v', dest='verbose', action='store_true', help='increase verbosity level')
    ...
    db_group.add_argument('--type', type=str, dest='type', default='postgres', help='The type of your database (default : postgres)')
    db_group.add_argument('-H', '--host', type=str, dest='host', default='localhost', help='The hostname of the database')
    ...
    db_group.add_argument('-u', '--user', type=str, dest='username', required=True, help='The user to connect to the database')
    db_group.add_argument('-p', '--pass', type=str, dest='password', help='The password to connect to the database')
    db_group.add_argument('-t', '--table', type=str, dest='table', default='elevation', help='The table name to import data')
    gis_group = parser.add_argument_group('gis', 'GIS configuration')
    gis_group.add_argument('-r', '--raster', dest='use_raster', action='store_true', help='Use raster to import data. Your database must have GIS capabilities like PostGIS for PostgreSQL.')
    gis_group.add_argument('-s', '--sample', nargs=2, type=int, dest='sample', metavar=('LNG_SAMPLE',
                                                                                        'LAT_SAMPLE'), default=(None,
                                                                                                                None), help='Separate a HGT file in multiple rasters. Sample on lng axis and lat axis.')
    gis_group.add_argument('--skip-raster2pgsql-check', dest='check_raster2pgsql', default=True, action='store_false', help='Skip raster2pgsql presence check')
    return parser
...
def load_hgt():
    ...
    try:
        ...
        logging.error(('Unable to connect to database with these settings : {}').format(factory.engine.url), exc_info=traceback)
