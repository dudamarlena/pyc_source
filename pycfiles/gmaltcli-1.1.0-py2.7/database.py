...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gmaltcli/database.py
# Compiled at: 2017-07-12 08:13:25
import logging
from future.utils import with_metaclass
from sqlalchemy import create_engine as sqlalchemy_create_engine
import sqlalchemy.engine.url as sql_url, sqlalchemy.exc
...
class ManagerRegistry(type):
    ...
    def get_manager_class(db_driver, use_raster):
        ...
        """
        if (
         db_driver, use_raster) not in ManagerRegistry.REGISTRY:
...
class PostgresValueManager(with_metaclass(ManagerRegistry, BaseManager)):
    ...
    TYPE = 'postgres'
    USE_RASTER = False
    TABLE_EXISTS_QUERY = 'SELECT EXISTS(    SELECT  1    FROM    information_schema.tables    WHERE   table_name=%(table_name)s)'
    TABLE_CREATE_QUERY = 'CREATE TABLE "{table_name}" (    lat_min DOUBLE PRECISION,    lng_min DOUBLE PRECISION,    lat_max DOUBLE PRECISION,    lng_max DOUBLE PRECISION,    "value" SMALLINT,    PRIMARY KEY (lat_min, lng_min, lat_max, lng_max));'
    VALUE_EXIST_QUERY = 'SELECT 1 FROM   "{table_name}" WHERE  lat_min=%(lat_min)s       AND lng_min=%(lng_min)s       AND lat_max=%(lat_max)s       AND lng_max=%(lng_max)s;'
    VALUE_CREATE_QUERY = 'INSERT INTO "{table_name}" (lat_min, lng_min, lat_max, lng_max, "value") VALUES (%(lat_min)s, %(lng_min)s, %(lat_max)s, %(lng_max)s, %(value)s)'
    ...
    def prepare_params(self, data, parser):
        ...
        area_corners = data[3]
        elevation_value = data[4]
        return {'lat_min': min([ corner[0] for corner in area_corners ]), 
...
class PostgresRasterManager(with_metaclass(ManagerRegistry, BaseManager)):
    ...
    USE_RASTER = True
    TABLE_EXISTS_QUERY = 'SELECT EXISTS(    SELECT  1    FROM    information_schema.tables    WHERE   table_name=%(table_name)s)'
    TABLE_CREATE_QUERY = 'CREATE TABLE "{table_name}" ("rid" serial PRIMARY KEY,"rast" raster);CREATE INDEX "{table_name}_rast_gist_idx" ON           "{table_name}" USING gist   (st_convexhull("rast"));'
    POSTGIS_AVAILABLE_QUERY = "SELECT EXISTS(    SELECT  1    FROM    pg_extension    WHERE   extname='postgis')"
    VALUE_EXIST_QUERY = 'SELECT 1 FROM   "{table_name}" WHERE  ST_Envelope(rast) = ST_GeomFromText(\'POLYGON((%(minx)s %(miny)s, %(minx)s %(maxy)s, %(maxx)s %(maxy)s, %(maxx)s %(miny)s, %(minx)s %(miny)s))\', 4326);'
    VALUE_CREATE_QUERY = 'INSERT INTO "{table_name}" ("rast") VALUES (ST_SetValues(    ST_AddBand(        ST_MakeEmptyRaster(%(width)s, %(height)s, %(topleftx)s, %(toplefty)s, %(scalex)s, %(scaley)s, 0, 0, 4326),        \'16BSI\'::text, %(default_value)s, %(nodata_value)s    ),    1, 1, 1, %(elevation_values)s::double precision[][]));'
