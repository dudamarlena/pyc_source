...
# [GCC 8.4.0]
# Embedded file name: /media/psf/Home/Documents/workspace/rqalpha-mod-vnpy/rqalpha_mod_vnpy/ctp/api.py
...
ORDER_TYPE_MAPPING = {ORDER_TYPE.MARKET: defineDict['THOST_FTDC_OPT_AnyPrice'], 
   ORDER_TYPE.LIMIT: defineDict['THOST_FTDC_OPT_LimitPrice']}
SIDE_MAPPING = {SIDE.BUY: defineDict['THOST_FTDC_D_Buy'], 
   SIDE.SELL: defineDict['THOST_FTDC_D_Sell']}
POSITION_EFFECT_MAPPING = {POSITION_EFFECT.OPEN: defineDict['THOST_FTDC_OF_Open'], 
...
class CtpMdApi(MdApi):
    ...
    def login(self):
        ...
        if not self.logged_in:
            req = {'UserID': self.user_id, 'Password': self.password, 
...
class CtpTdApi(TdApi):
    ...
    def authenticate(self):
        ...
        if self.authenticated:
            req = {'UserID': self.user_id, 'BrokerID': self.broker_id, 
    ...
    def qryCommission(self, order_book_id):
        ...
        if ins_dict is None:
            ...
            req = {'InstrumentID': ins_dict.instrument_id, 
               'InvestorID': self.user_id, 
               'BrokerID': self.broker_id, 
