...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tipfyrecipes/hg.py
# Compiled at: 2011-08-05 19:08:05
...
class Mercurial(object):
    ...
    def __init__(self, buildout, name, options):
        options['directory'] = options.get('directory') or os.path.join(buildout['buildout']['parts-directory'], name)
        self.logger = logging.getLogger(name)
        self.repository = options.get('repository')
        self.directory = options['directory']
        self.newest = options.get('newest', buildout['buildout'].get('newest', 'true')).lower() != 'false'
    ...
    def pull(self):
        commands.pull(ui.ui(), hg.repository(ui.ui(), self.directory), self.repository, update=True)
