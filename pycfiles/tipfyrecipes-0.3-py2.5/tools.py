...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tipfyrecipes/gae/tools.py
...
class InstallGAETools(zc.recipe.egg.Scripts):
    ...
    def __init__(self, buildout, name, opts):
        ...
        join = os.path.join
        defaults = {'sdk-directory': join(self.parts_dir, 'google_appengine'), 
    ...
    def install(self):
        """Creates the scripts."""
        entry_points = [ '%s=tipfyrecipes.gae.scripts:%s' % (scriptname, function) for (function, scriptname) in self.scripts
                       ]
