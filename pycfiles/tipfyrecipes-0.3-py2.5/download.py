...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tipfyrecipes/download.py
# Compiled at: 2011-08-05 19:07:14
import logging, os.path, shutil, tempfile, urlparse, zc.buildout
from zc.buildout.download import ChecksumError, Download
import setuptools.archive_util
from tipfyrecipes.utils import get_bool_option, get_checksum

class Recipe(object):
    """
    Recipe for downloading packages from the net and
    extracting them on the filesystem.
    """
    ...
    def __init__(self, buildout, name, options):
        ...
        self.download_cache = buildout['buildout'].setdefault('download-cache', os.path.join(buildout['buildout']['directory'], 'downloads'))
        self.option_url = options.get('url')
        self.option_md5sum = options.get('md5sum')
        self.option_sha1sum = options.get('sha1sum')
        self.option_destination = options.setdefault('destination', os.path.join(buildout['buildout']['parts-directory'], self.name))
        self.option_clear_destination = get_bool_option(options.setdefault('clear-destination', 'false'))
        self.option_strip_top_level_dir = get_bool_option(options.setdefault('strip-top-level-dir', 'false'))
        self.option_download_only = get_bool_option(options.setdefault('download-only', 'false'))
        self.option_hash_name = get_bool_option(options.setdefault('hash-name', 'false'))
    ...
    def install(self):
        ...
        try:
            ...
            if self.option_download_only:
                ...
                try:
                    setuptools.archive_util.unpack_archive(cached_path, extract_dir)
                except setuptools.archive_util.UnrecognizedFormat:
                    self.logger.error('Unable to extract the package %s. Unknown format.', cached_path)
                    raise zc.buildout.UserError('Package extraction error')

                base = self.calculate_base(extract_dir)
                if not os.path.isdir(self.option_destination):
                    os.makedirs(self.option_destination)
                self.logger.info('Extracting package to %s' % self.option_destination)
                for filename in os.listdir(base):
                    ...
                    if os.path.exists(dest):
                        if self.option_clear_destination:
                            ...
                            self.logger.error('Target %s already exists. Either remove it or set ``clear-destination = true`` in your buildout.cfg to remove existing files and directories before moving downloaded files.', dest)
    ...
    def calculate_base(self, extract_dir):
        ...
        if self.option_strip_top_level_dir:
            if len(top_level_contents) != 1:
                self.logger.error('Unable to strip top level directory because there are more than one element in the root of the package.')
    ...
    def download(self):
        d = Download(self.buildout['buildout'], hash_name=self.option_hash_name)
        (cached_path, is_temp) = d(self.option_url, md5sum=self.option_md5sum)
        if self.option_sha1sum and self.option_sha1sum != get_checksum(cached_path):
            raise ChecksumError('SHA1 checksum mismatch for cached download from %r at %r' % (
             self.option_url, cached_path))
