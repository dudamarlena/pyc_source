...
# [GCC 8.4.0]
# Embedded file name: /Users/lucaswiman/opensource/django-predicate/predicate/predicate.py
# Compiled at: 2016-04-09 19:27:37
# Size of source mod 2**32: 16439 bytes
...
class P(Q):
    ...
    def eval(self, instance):
        ...
        ret = evaluator(c.eval(instance) for c in eval_wrapper(self.children, connector=self.connector))
        if self.negated:
            return not ret
        else:
            return ret
    ...
    def add(self, data, conn_type, squash=True):
        ...
        if self.connector == conn_type:
            if isinstance(data, Node) and not data.negated and (data.connector == conn_type or len(data) == 1):
                ...
                if isinstance(data, tuple) and any(isinstance(child, tuple) and child[0] == data[0] for child in self.children):
    ...
    def get(self, iterable):
        ...
        if len(filtered) == 0:
            ...
            if len(filtered) > 1:
                raise MultipleObjectsReturned('get() returned more than one object -- it returned %s!' % len(filtered))
            else:
                return filtered[0]
...
class LookupComponent(str):
    ...
    def __repr__(self):
        return '{self.__class__.__name__}({repr})'.format(self=self, repr=super(LookupComponent, self).__repr__())
...
class LookupNode(object):
    ...
    def iteritems(self, lookup_stack=None):
        ...
        for component, node in six.viewitems(self.children):
            if component == LookupComponent.EMPTY:
                yield (
                 LookupComponent(LOOKUP_SEP.join(lookup_stack)),
                 self.value)
            else:
    ...
    def eval(self, instance):
        ...
        for node in values:
            ...
            for lookup, value in node.iteritems():
                queries = self[lookup]
                if self.connector == Q.AND:
                    node_matches &= all(evaluator(value) for evaluator in queries.evaluators)
                else:
                    if self.connector == Q.OR:
                        node_matches |= any(evaluator(value) for evaluator in queries.evaluators)
                    else:
                        raise NotImplementedError(self.connector)
    ...
    def values(self, obj):
        ...
        children_iters = (zip(itertools.repeat(lookup), values) for lookup, values in lookup2values.items())
        results = []
...
def get_values_list(obj, *lookups, **kwargs):
    ...
    if flat:
        ...
        return [tuple(value_dict[lookup].value for lookup in lookups) for value_dict in value_dicts]
