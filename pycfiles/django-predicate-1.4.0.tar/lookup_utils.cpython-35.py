...
# [GCC 8.4.0]
# Embedded file name: /Users/lucaswiman/opensource/django-predicate/predicate/lookup_utils.py
# Compiled at: 2016-04-09 19:03:57
# Size of source mod 2**32: 5233 bytes
import datetime, operator, django, re
...
class IsNull(LookupQueryEvaluator):
    evaluators = (
     lambda lhs, rhs: (lhs is None) == rhs,)


class Contains(LookupQueryEvaluator):
    evaluators = (
     NOT_NULL, lambda lhs, rhs: rhs in lhs)


class Regex(LookupQueryEvaluator):
    evaluators = (
     NOT_NULL, lambda lhs, regex: bool(regex.search(lhs)))
    flags = 0
    escape = False
    template = '%s'
...
class StartsWith(LookupQueryEvaluator):
    evaluators = (
     NOT_NULL, lambda lhs, rhs: lhs.startswith(rhs))


class EndsWith(LookupQueryEvaluator):
    evaluators = (
     NOT_NULL, lambda lhs, rhs: lhs.endswith(rhs))
...
class GT(DateCastMixin, LookupQueryEvaluator):
    evaluators = (
     NOT_NULL, operator.gt)


class GTE(DateCastMixin, LookupQueryEvaluator):
    evaluators = (
     NOT_NULL, operator.ge)


class LT(DateCastMixin, LookupQueryEvaluator):
    evaluators = (
     NOT_NULL, operator.lt)


class LTE(DateCastMixin, LookupQueryEvaluator):
    evaluators = (
     NOT_NULL, operator.le)


class Day(LookupQueryEvaluator):
    evaluators = (
     NOT_NULL, lambda lhs, rhs: lhs.day == rhs)


class Month(LookupQueryEvaluator):
    evaluators = (
     NOT_NULL, lambda lhs, rhs: lhs.month == rhs)


class Year(LookupQueryEvaluator):
    evaluators = (
     NOT_NULL, lambda lhs, rhs: lhs.year == rhs)


class WeekDay(LookupQueryEvaluator):
    evaluators = (
     NOT_NULL, lambda lhs, rhs: lhs.isoweekday() % 7 + 1 == rhs)


class Range(LookupQueryEvaluator):
    evaluators = (
     NOT_NULL, lambda lhs, rhs: rhs[0] < lhs < rhs[1])


LOOKUP_TO_EVALUATOR = {'contains': Contains, 
 'day': Day, 
 'endswith': EndsWith, 
 'exact': Exact, 
 'gt': GT, 
...
def get_field_and_accessor(instance_or_model, lookup_part):
    ...
    return (
     field, lookup_part if direct else field.get_accessor_name())