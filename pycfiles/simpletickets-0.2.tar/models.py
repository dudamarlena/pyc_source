...
# [GCC 8.4.0]
# Embedded file name: /django/.envs/simpletickets/local/lib/python2.7/site-packages/simpletickets/models.py
...
from .settings import ST_ATTACHMENTS, ST_DELTA_CLOSE, ST_TCKT_TYPE, ST_TCKT_SEVERITY, ST_TCKT_STATE
...
class TicketManager(models.Manager):
    ...
    def all(self):
        ...
        state_8_delta = all_objects.filter(state=8, resolution_date__lt=now_minus_delta)
...
class Ticket(models.Model):
    ...
    staff = models.ForeignKey(User, limit_choices_to={'is_staff': True}, related_name='usrStaff', blank=True, null=True)
    ...
    description = models.TextField(ugl('Description'), default='...')
    attachment = models.FileField(upload_to=uploadAttachment, blank=True, null=True)
    resolution_text = models.TextField(ugl('Resolution text'), default='')
    creation_date = models.DateTimeField(_('Creation Date'), default=timezone.now)
    modification_date = models.DateTimeField(_('Last Modification Date'), blank=True, null=True)
    resolution_date = models.DateTimeField(_('Resolution date'), blank=True, null=True)
    resolution_delta = models.FloatField(_('delayed time in seconds'), blank=True, null=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.id:
            header_msg = self.changesChecker(Ticket.objects.get(id=self.id), self)
        else:
        ...
        if not self.ticket_number:
            self.ticket_number = str(self.creation_date)[2:4] + ('00000{id}').format(id=self.id)[-6:]
            self.save()
    ...
    def _mailSender(self):
        ...
        if self.state == 9:
            context = {}
            subject = _('Your ticket number {ticket_number} has been solved').format(ticket_number=self.ticket_number)
            body = _('We are glad we have solved your ticket number: {ticket_number}\n\nThe resolution time was {resolution_delta}.\n\nYou have {ST_DELTA_CLOSE} hours to reopen it, remember to carefully explain your reasons.\nThe Team').format(ticket_number=self.ticket_number, resolution_delta=self.resolution_delta, ST_DELTA_CLOSE=ST_DELTA_CLOSE)
            email = self.user.email
            self.sendEmail(context, subject, body, email)
        if self.state == 2:
            context = {}
            subject = _('Ticket {ticket_number} assigned.\n\nThe Team').format(ticket_number=self.ticket_number)
    ...
    def changesChecker(self, original, actual):
        ...
        for attribute in MONITORIZED:
            ...
            if attribute in ('ticket_type', 'severity', 'state'):
                value = getattr(actual, ('get_{attribute}_display').format(attribute=attribute))()
            if not attr_ori == attr_act and (attr_ori or attr_act):
                msg.append(('[{attrname}: {attrvalue}]').format(attrname=attribute.upper(), attrvalue=value))
    ...
    def sendEmail(self, context, subject, body, email):
        EmailManager(context, subject=subject, body=body, from_email=settings.EMAIL_HOST_USER, to=self.user.email, bcc=email, connection=None, attachments=None, headers=None, alternatives=None, cc=None, reply_to=None)
        return

    def __unicode__(self):
        return ('{ticket_number} {user}').format(ticket_number=self.ticket_number, user=self.user)
