...
# [GCC 8.4.0]
# Embedded file name: /django/.envs/simpletickets/local/lib/python2.7/site-packages/simpletickets/views.py
# Compiled at: 2016-10-01 10:34:52
...
from .settings import BASE_TEMPLATE, ST_FL_MNTR_OWNER, ST_FL_MNTR_STAFF, ST_SETT_NUMBERS_STAFF, ST_SETT_NUMBERS_OWNER, ST_SETT_TIMES_STAFF, ST_SETT_TIMES_OWNER, ST_SETT_MAIN_TASKBAR, ST_STAFF_GNAME, ST_ADMIN_GNAME
    ...
    def get_context_data(self, **kwargs):
        ...
        if stt_times:
            ...
            if all_tickets.filter(state__gt=7):
                context['fastest'] = all_tickets.filter(state__gt=7).order_by('resolution_delta')[0].humanized_delta()
                if n_solved:
                    context['media'] = timedelta(seconds=sum([ t.resolution_delta for t in all_tickets.filter(state__gt=7)
                                                             ]) / n_solved)
                else:
                    context['media'] = 'N/A'
        return context
...
class Login_required_mixin(View):
    ...
    def as_view(self, **kwargs):
        return login_required(super(Login_required_mixin, self).as_view(**kwargs))
...
class TicketMixin(object):
    ...
    def get_queryset(self):
        ...
        is_ticket_manager = user.groups.filter(name=self.staff_group.name).exists()
        is_ticket_admin = user.groups.filter(name=self.admin_group.name).exists()
        if is_ticket_admin:
            return Ticket.objects.filter(Q(state=1) | Q(state__lt=9))
        if is_ticket_manager:
            return Ticket.objects.filter(Q(state=1) | Q(staff=user, state__lt=9))
...
class TicketDelete(ContextMixin, Login_required_mixin, TicketMixin, DeleteView):
...
class TicketUpdate(ContextMixin, Login_required_mixin, TicketMixin, UpdateView):
    title = _('Edit ticket')
    model = Ticket
    success_url = reverse_lazy('home')
    ...
    def form_valid(self, form):
        ...
        header_msg = self.getHeader(ticket.creation_date, self.request.user.username)
