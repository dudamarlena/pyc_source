...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.6/site-packages/filabel/web.py
# Compiled at: 2018-12-29 18:37:27
# Size of source mod 2**32: 5400 bytes
import configparser, flask, hashlib, hmac, jinja2, os
...
def process_webhook_pr(payload):
    ...
    try:
        ...
        pr_url = pull_request['url'].split('/')
        owner = pr_url[(-4)]
        repo = pr_url[(-3)]
        reposlug = f"{owner}/{repo}"
        if action not in ('opened', 'synchronize'):
            flask.current_app.logger.info(f"Action {action} from {reposlug}#{pr_number} skipped")
            return ('Accepted but action not processed', 202)
        filabel.run_pr(owner, repo, pull_request)
        flask.current_app.logger.info(f"Action {action} from {reposlug}#{pr_number} processed")
        return ('PR successfully filabeled', 200)
    except (KeyError, IndexError):
        flask.current_app.logger.info(f"Incorrect data entity from IP {flask.request.remote_addr}")
        flask.abort(422, 'Missing required payload fields')
    except Exception:
        flask.current_app.logger.error(f"Error occurred while processing {repo}#{pr_number}")
...
def process_webhook_ping(payload):
    ...
    try:
        ...
        flask.current_app.logger.info(f"Accepting PING from {repo}#WH-{hook_id}")
        return ('PONG', 200)
    except KeyError:
        flask.current_app.logger.info(f"Incorrect data entity from IP {flask.request.remote_addr}")
...
webhook_processors = {'pull_request':process_webhook_pr, 
...
def create_app(*args, **kwargs):
    ...
    def index():
        ...
        Landing info page
        """
        return flask.render_template('infopage.html',
    ...
    def webhook_listener():
        ...
        if secret is not None:
            if not webhook_verify_signature(flask.request.data, signature, secret):
                flask.current_app.logger.warning(f"Attempt with bad secret from IP {flask.request.remote_addr}")
