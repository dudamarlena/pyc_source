...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-x86_64/egg/dprofiling/admin.py
# Compiled at: 2013-05-14 16:55:43
...
class SessionAdmin(admin.ModelAdmin):
    ...
    def stats_link(self, obj):
        return '<a href="%s">Stats</a>' % (
         reverse('admin:dprofiling_session_stats', args=(
          obj.pk,)),)
    ...
    def get_urls(self):
        ...
        session_urls = patterns('', url('^(.+)/stats/$', self.admin_site.admin_view(self.stats_view), name='%s_%s_stats' % info))
    ...
    def stats_view(self, request, object_id, extra_context=None):
        ...
        if not callable(getattr(backend, 'get_stats', None)):
            raise Exception('Current backend does not support getting aggregate stats')
        ...
        context = {'title': 'Stats for %s' % obj.name, 
           'adminform': adminform, 
           'object_id': object_id, 
           'original': obj, 
        ...
        return TemplateResponse(request, [
         'admin/dprofiling/session/stats_form.html'], context, current_app=self.admin_site.name)
