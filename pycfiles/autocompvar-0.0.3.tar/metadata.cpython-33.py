...
# [GCC 8.4.0]
# Embedded file name: C:\Users\shu\Documents\PythonWorkSpace\py3\py33_projects\autocompvar-project\autocompvar\metadata.py
# Compiled at: 2017-02-24 14:25:52
# Size of source mod 2**32: 12186 bytes
try:
    from . import templates
    from .name_convention import is_valid_class_name, is_valid_variable_name, is_valid_surfix, to_variable_name
    ...
    from autocompvar.name_convention import is_valid_class_name, is_valid_variable_name, is_valid_surfix, to_variable_name
    ...
    if isinstance(metadata, list):
        ...
        if isinstance(classname, string_types):
            if is_valid_class_name(classname):
                ...
                elif not class_def_dict[classname] == class_def:
                    raise ValidationError('definition of %r is incls_instistent: %s' % (
                     classname, class_def))
            else:
...
class ClassDef(object):
    ...
    def __repr__(self):
        return '%s(%s)' % (
         ...
         ', '.join(['%s=%r' % (attr, value) for attr, value in self.__dict__.items()]))
...
class ClassInstance(object):
    __doc__ = 'A data container for instance of a class.     \n    '
    ...
    def __init__(self, classname, attrs, keys=None, data=None, collection=None):
        ...
        if collection is None:
            ...
            self.collection = [self.__class__.load(metadata) for metadata in collection]
        return
    ...
    def new_instance_kwargs_text(self):
        ...
        if self.data is None:
            ...
            return ', '.join(['%s=%r' % (attr, value) for attr, value in self.data.items()])
            return
    ...
    def load(cls, metadata):
        ...
        """
        return cls(classname=metadata.get('classname'), attrs=metadata.get('attrs'), keys=metadata.get('keys'), data=metadata.get('data'), collection=metadata.get('collection'))

    def dump(self):
        """dump class data to dict.
        """
        metadata = {'classname': self.classname, 
         'attrs': self.attrs, 
         'keys': self.keys, 
         'data': self.data}
        if self.collection is None:
            ...
            metadata['collection'] = [cls_insttructor.dump() for cls_insttructor in self.collection]
        return metadata
...
def gen_code_inst_part(metadata, do_validate=True, code_snipet=None):
    ...
    if cls_inst.collection:
        for c_i in cls_inst.collection:
            gen_code_inst_part(c_i.dump(), do_validate=False, code_snipet=code_snipet)
            for key in c_i.keys:
...
def gen_code(metadata):
    return templates.t_code.render(def_part=gen_code_def_part(metadata), inst_part=gen_code_inst_part(metadata))


if __name__ == '__main__':
    from pprint import pprint
    metadata = {'classname': 'Food', 
     'attrs': [],  'collection': [
                    {'classname': 'FoodCategory', 
                     ...
                     'keys': [
                              'name'], 
                     'data': {'id': 1,  'name': 'Fruit'},  'collection': [
                                    {'classname': 'Fruit', 
                                     'attrs': [
                                               'id', 'name'], 
                                     'keys': [
                    ...
                    {'classname': 'FoodCategory', 
                     ...
                     'keys': [
                              'name'], 
                     'data': {'id': 2,  'name': 'Meat'},  'collection': [
                                    {'classname': 'Meat', 
                                     'attrs': [
                                               'id', 'name'], 
                                     'keys': [
