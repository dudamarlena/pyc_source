...
# Embedded file name: build/bdist.linux-i686/egg/collective/kss/flygui/browser/menu.py
    ...
    def getMenuItems(self, context, request):
        ...
        for action in workflowActions:
            if action['category'] != 'workflow':
                continue
            cssClass = context_css_class + ' ' + 'kssattr-action-%s' % action['id']
            ...
            if action['allowed']:
                results.append({'title': action['title'], 'description': description, 
                   'action': actionUrl, 
                   'selected': False, 
                   'icon': None, 
                   'extra': {'separator': None, 'class': cssClass}, 'submenu': None})
...
class WorkflowSubMenuItem(BaseWorkflowSubMenuItem):
    ...
    def extra(self):
        ...
        return {'id': 'content-menu-workflow-%s' % self.context.UID(), 
           'class': 'state-%s' % state, 
           'state': state, 
...
class WorkflowMenuProvider(Explicit):
    ...
    def menu(self):
        menu = getUtility(IBrowserMenu, name='flygui.content.menu')
