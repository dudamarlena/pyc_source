...
# Embedded file name: build\bdist.win-amd64\egg\arcadeplus\examples\particle_systems.py
# Compiled at: 2020-03-29 18:06:45
# Size of source mod 2**32: 29177 bytes
...
import pyglet, os, random, math
SCREEN_WIDTH = 800
SCREEN_HEIGHT = 600
SCREEN_TITLE = 'Particle System Examples'
QUIET_BETWEEN_SPAWNS = 0.25
EMITTER_TIMEOUT = 600
...
def emitter_12():
    ...
    e = arcadeplus.Emitter(center_xy=CENTER_POS,
      ...
      particle_factory=(lambda emitter: arcadeplus.EternalParticle(filename_or_texture=TEXTURE,
      change_xy=(arcadeplus.rand_in_circle((0.0, 0.0), PARTICLE_SPEED_FAST)),
      scale=DEFAULT_SCALE,
      alpha=DEFAULT_ALPHA)))
    return (
     emitter_12.__doc__, e)
...
def emitter_18():
    ...
    e = arcadeplus.Emitter(center_xy=CENTER_POS,
      ...
      alpha=DEFAULT_ALPHA)))
    return (
     emitter_18.__doc__, e)


def emitter_19():
    """Interval, emit on line"""
    e = arcadeplus.Emitter(center_xy=(0.0, 0.0),
      emit_controller=(arcadeplus.EmitterIntervalWithTime(DEFAULT_EMIT_INTERVAL, DEFAULT_EMIT_DURATION)),
      particle_factory=(lambda emitter: arcadeplus.LifetimeParticle(filename_or_texture=TEXTURE,
      change_xy=(arcadeplus.rand_in_circle((0.0, 0.0), PARTICLE_SPEED_SLOW)),
      lifetime=DEFAULT_PARTICLE_LIFETIME,
...
def emitter_28():
    ...
    e = arcadeplus.Emitter(center_xy=CENTER_POS,
      ...
      particle_factory=(lambda emitter: arcadeplus.LifetimeParticle(filename_or_texture=(random.choice((TEXTURE, TEXTURE2, TEXTURE3))),
      change_xy=(arcadeplus.rand_in_circle((0.0, 0.0), PARTICLE_SPEED_FAST)),
      lifetime=DEFAULT_PARTICLE_LIFETIME,
      scale=DEFAULT_SCALE)))
    return (
     emitter_28.__doc__, e)
...
def emitter_31():
    ...
    e = arcadeplus.Emitter(center_xy=CENTER_POS,
      emit_controller=(arcadeplus.EmitterIntervalWithTime(DEFAULT_EMIT_INTERVAL * 5, DEFAULT_EMIT_DURATION)),
      particle_factory=(lambda emitter: arcadeplus.LifetimeParticle(filename_or_texture=TEXTURE2,
      change_xy=(arcadeplus.rand_in_circle((0.0, 0.0), PARTICLE_SPEED_FAST)),
      lifetime=DEFAULT_PARTICLE_LIFETIME,
      angle=45,
      scale=DEFAULT_SCALE)))
    return (
     emitter_31.__doc__, e)
...
def emitter_33():
    ...
    e = arcadeplus.Emitter(center_xy=CENTER_POS,
      emit_controller=(arcadeplus.EmitterIntervalWithTime(DEFAULT_EMIT_INTERVAL, DEFAULT_EMIT_DURATION)),
      particle_factory=(lambda emitter: arcadeplus.FadeParticle(filename_or_texture=TEXTURE,
      change_xy=(arcadeplus.rand_in_circle((0.0, 0.0), PARTICLE_SPEED_FAST)),
      lifetime=DEFAULT_PARTICLE_LIFETIME,
      scale=DEFAULT_SCALE)))
    return (
     emitter_33.__doc__, e)
...
def emitter_34():
    ...
    e = arcadeplus.Emitter(center_xy=CENTER_POS,
      emit_controller=(arcadeplus.EmitBurst(BURST_PARTICLE_COUNT)),
      particle_factory=(lambda emitter: arcadeplus.FadeParticle(filename_or_texture=(random.choice(textures)),
      change_xy=(arcadeplus.rand_in_circle((0.0, 0.0), PARTICLE_SPEED_FAST)),
      lifetime=DEFAULT_PARTICLE_LIFETIME,
      scale=DEFAULT_SCALE)))
    return (
     emitter_34.__doc__, e)
...
def emitter_36():
    ...
    class MovingEmitter(arcadeplus.Emitter):
        ...
        def update(self):
            super().update()
            self.elapsed += 0.016666666666666666
    ...
    e = MovingEmitter(center_xy=CENTER_POS,
...
def emitter_37():
    ...
    e.change_angle = 10.0
    return (
     emitter_37.__doc__, e)


def emitter_38():
    """Use simple emitter interface to create a burst emitter"""
    e = arcadeplus.make_burst_emitter(center_xy=CENTER_POS,
      filenames_and_textures=(
     TEXTURE, TEXTURE3, TEXTURE4),
      particle_count=50,
      particle_speed=2.5,
      ...
      fade_particles=True)
    return (
     emitter_38.__doc__, e)


def emitter_39():
    """Use simple emitter interface to create an interval emitter"""
    e = arcadeplus.make_interval_emitter(center_xy=CENTER_POS,
      filenames_and_textures=(
     TEXTURE, TEXTURE3, TEXTURE4),
      emit_interval=0.01,
      emit_duration=2.0,
...
class MyGame(arcadeplus.Window):
    ...
    def on_draw(self):
        ...
        self.obj.draw()
        if self.label:
            arcadeplus.draw_text(('#{} {}'.format(self.emitter_factory_id, self.label)), (SCREEN_WIDTH / 2),
              (SCREEN_HEIGHT - 20), (arcadeplus.color.PALE_GOLD),
              20, width=SCREEN_WIDTH, align='center', anchor_x='center',
              anchor_y='center')
        if self.emitter:
            self.emitter.draw()
