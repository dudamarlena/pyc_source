...
# Embedded file name: build\bdist.win-amd64\egg\arcadeplus\drawing_support.py
# Compiled at: 2020-03-31 14:53:34
...
def get_points_for_thick_line(start_x: float, start_y: float, end_x: float, end_y: float, line_width: float):
    """
...
def rotate_point(x: float, y: float, cx: float, cy: float, angle_degrees: float) -> List[float]:
    """
...
def calculate_points(image):
    ...
    def _r(point, height, width):
        return (
         point[0] - width / 2, height - point[1] - height / 2)
    ...
    p1 = (
     left_border + top_left_corner_offset, top_border)
