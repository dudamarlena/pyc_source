...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dpl1_main/testing_app/session_util.py
# Compiled at: 2014-02-28 04:43:43
...
class TestSession(object):
    ...
    def update_results(self, post_dict, test_id, page_id):
        ...
        """
        form_dict = {key:post_dict.getlist(key) for key in post_dict if 'question' in key}
    ...
    def get_test_result(self, test_id):
        ...
        :param test_id: id of the testing_app.models.Test
        """
        answer_lists = (self.answers[question] for question in self.answers.keys() if 'question' in question)
        ...
        test_results = Result.objects.filter(max_points__lte=total_points, test__id=test_id).order_by('-max_points')
    ...
    def get_last_test_page(self):
        ...
        """
        return (
         self.session['test_id'], self.session['last_page_id'])
...
class TestPaginator(object):
    ...
    def _convert_to_bool(self, next_, previous, first, last):
        """Returns the values of the 4 strings to the correct bool
            representation
        :returns a tuple of bools
        """
        true_values = [
         'True', 'true', '1', True, 'true', 'True', '1']
    ...
    def goto_page(self, next_=False, previous=False, first=False, last=False):
        ...
        :return:
        """
        next_, previous, first, last = self._convert_to_bool(next_, previous, first, last)
