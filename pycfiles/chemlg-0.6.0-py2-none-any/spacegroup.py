...
# Embedded file name: /home/gabriele/workspace/chemlab/chemlab/core/spacegroup/spacegroup.py
# Compiled at: 2015-01-08 16:33:19
__doc__ = 'Definition of the Spacegroup class.\n\nThis module only depends on NumPy and the space group database.\n'
...
class Spacegroup(object):
    ...
    array([[ 0. ,  0. ,  0. ],
           ...
           [ 0.5,  0.5,  0. ]])
    """
    no = property(lambda self: self._no, doc='Space group number in International Tables of Crystallography.')
    symbol = property(lambda self: self._symbol, doc='Hermann-Mauguin (or international) symbol for the space group.')
    setting = property(lambda self: self._setting, doc='Space group setting. Either one or two.')
    lattice = property(lambda self: self._symbol[0], doc='Lattice type: \n            P      primitive\n            I      body centering, h+k+l=2n\n            F      face centering, h,k,l all odd or even\n            A,B,C  single face centering, k+l=2n, h+l=2n, h+k=2n\n            R      rhombohedral centering, -h+k+l=3n (obverse); h-k+l=3n (reverse)\n            ')
    centrosymmetric = property(lambda self: self._centrosymmetric, doc='Whether a center of symmetry exists.')
    scaled_primitive_cell = property(lambda self: self._scaled_primitive_cell, doc='Primitive cell in scaled coordinates as a matrix with the primitive vectors along the rows.')
    reciprocal_cell = property(lambda self: self._reciprocal_cell, doc='Tree Miller indices that span all kinematically non-forbidden reflections as a matrix with the Miller indices along the rows.')
    nsubtrans = property(lambda self: len(self._subtrans), doc='Number of cell-subtranslation vectors.')
    ...
    subtrans = property(lambda self: self._subtrans, doc='Translations vectors belonging to cell-sub-translations.')
    rotations = property(lambda self: self._rotations, doc='Symmetry rotation matrices. The invertions are not included for centrosymmetrical crystals.')
    translations = property(lambda self: self._translations, doc='Symmetry translations. The invertions are not included for centrosymmetrical crystals.')
    ...
    def equivalent_sites(self, scaled_positions, ondublicates='error', symprec=0.001):
            ...
            for rot, trans in self.get_symop():
                ...
                if np.any(mask):
                    ...
                    if kinds[ind] == kind:
                        ...
                        warnings.warn('scaled_positions %d and %d are equivalent' % (
                         kinds[ind], kind))
                    elif ondublicates == 'error':
                        raise SpacegroupValueError('scaled_positions %d and %d are equivalent' % (
                         kinds[ind], kind))
                    else:
                        raise SpacegroupValueError('Argument "ondublicates" must be one of: "keep", "replace", "warn" or "error".')
                else:
                    sites.append(site)
                    kinds.append(kind)

        return (
         np.array(sites), kinds)
    ...
    def unique_sites(self, scaled_positions, symprec=0.001, output_mask=False):
...
def _skip_to_blank(f, spacegroup, setting):
    ...
    while True:
        line = f.readline()
        if not line:
            raise SpacegroupNotFoundError('invalid spacegroup %s, setting %i not found in data base' % (
             spacegroup, setting))
        if not line.strip():
            break
...
def _read_datafile_entry(spg, no, symbol, setting, f):
    ...
    f.readline()
    spg._scaled_primitive_cell = np.array([ list(map(float, f.readline().split())) for i in range(3)
                                          ], dtype='float')
    f.readline()
    spg._reciprocal_cell = np.array([ list(map(int, f.readline().split())) for i in range(3)
                                    ], dtype=np.int)
    spg._nsubtrans = int(f.readline().split()[0])
    spg._subtrans = np.array([ list(map(float, f.readline().split())) for i in range(spg._nsubtrans)
                             ], dtype=np.float)
    nsym = int(f.readline().split()[0])
    symop = np.array([ list(map(float, f.readline().split())) for i in range(nsym) ], dtype=np.float)
    spg._nsymop = nsym
...
def _read_datafile(spg, spacegroup, setting, f):
    if isinstance(spacegroup, int):
        ...
        while True:
            ...
            _no = int(_no)
            if (isinstance(spacegroup, int) and _no == spacegroup or isinstance(spacegroup, str) and _symbol == spacegroup) and _setting == setting:
        ...
        for j, s in enumerate(sym.split(sep)):
            ...
            while s:
                ...
                if s[0] in 'xyz':
                    ...
                    raise SpacegroupValueError('Error parsing %r. Invalid site symmetry: %s' % (
                     s, sym))
...
def spacegroup_from_data(no=None, symbol=None, setting=1, centrosymmetric=None, scaled_primitive_cell=None, reciprocal_cell=None, subtrans=None, sitesym=None, rotations=None, translations=None, datafile=None):
