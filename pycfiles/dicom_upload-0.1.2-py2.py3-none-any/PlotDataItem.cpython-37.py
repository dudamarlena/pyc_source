    ...
    def getData(self):
        ...
        if self.xDisp is None:
            ...
            if self.opts['clipToView']:
                ...
                range = view is None or view.autoRangeEnabled()[0] or 
                if range is not None:
                    if len(x) > 1:
                        dx = float(x[(-1)] - x[0]) / (len(x) - 1)
