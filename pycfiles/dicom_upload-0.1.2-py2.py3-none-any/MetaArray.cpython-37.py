...
class MetaArray(object):
        ...
        if type(ind) is int:
            ...
            if MetaArray.isNameType(ind):
                if not numOk:
                    raise Exception('string and integer indexes may not follow named indexes')
                return (
                 pos, self._getIndex(pos, ind), False)
            if type(ind) is slice and not MetaArray.isNameType(ind.start):
    ...
    def _readHDF5(self, fileName, readAllData=None, writable=False, **kargs):
        if 'close' in kargs:
            ...
            self._info = meta
            self._data = writable or readAllData or 
            self._openFile = f
        else:
            self._data = f['data'][:]
            f.close()
 ...
 L.1145        34  LOAD_FAST                'root'
               ...
               52  CALL_FUNCTION_EX_KW     1  'keyword args'
               54  POP_TOP          
 ...
 L.1154       140  LOAD_FAST                'self'
              ...
              162  CALL_FUNCTION_EX_KW     1  'keyword args'
              164  POP_TOP          
 ...
 L.1159       220  LOAD_FAST                'self'
              ...
              234  CALL_FUNCTION_EX_KW     1  'keyword args'
              236  POP_TOP          
...
Parse error at or near `JUMP_FORWARD' instruction at offset 306
    ...
    def writeMa(self, fileName, appendAxis=None, newFile=False):
        ...
        if appendAxis is not None:
            ...
            if not newFile:
                newFile = not os.path.exists(fileName) or 
