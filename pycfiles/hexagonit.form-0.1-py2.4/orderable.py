...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hexagonit/form/orderable.py
...
class OrderableFields(Fields):
    ...
    def moveField(self, name, direction=None, position=None, before=None, after=None):
        ...
        if before is not None:
            if before not in self.__FormFields_byname__:
                raise ValueError, "Can not move field '%s' before field '%s'. Field '%s' does not exist." % (name, before)
            if before == name:
        ...
        if after is not None:
            if after not in self.__FormFields_byname__:
                raise ValueError, "Can not move field '%s' after field '%s'. Field '%s' does not exist." % (name, after)
            if after == name:
        ...
        raise TypeError, 'moveField() must to be given exactly one of the following parameters: direction, position, before or after.'
        return
