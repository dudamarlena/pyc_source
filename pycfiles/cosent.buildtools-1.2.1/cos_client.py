...
# Embedded file name: build/bdist.linux-x86_64/egg/coscmd/cos_client.py
# Compiled at: 2020-04-19 22:20:02
...
import time, requests, logging, sys, os, base64, datetime, pytz, yaml, fnmatch, copy
...
class CoscmdConfig(object):
    ...
    def __init__(self, appid, region, endpoint, bucket, secret_id, secret_key, token=None, part_size=1, max_thread=5, schema='https', anonymous=False, verify='md5', retry=2, timeout=60, *args, **kwargs):
        self._appid = appid
        self._region = region
        ...
        logger.debug(('config parameter-> appid: {appid}, region: {region}, endpoint: {endpoint}, bucket: {bucket}, part_size: {part_size}, max_thread: {max_thread}').format(appid=appid, region=region, endpoint=endpoint, bucket=bucket, part_size=part_size, max_thread=max_thread))

    def uri(self, path=None):
        if path:
            if self._region is not None:
                url = ('{schema}://{bucket}.cos.{region}.myqcloud.com/{path}').format(schema=self._schema, bucket=self._bucket, region=self._region, path=to_unicode(path))
            else:
                url = ('{schema}://{bucket}.{endpoint}/{path}').format(schema=self._schema, bucket=self._bucket, endpoint=self._endpoint, path=to_unicode(path))
        elif self._region is not None:
            url = ('{schema}://{bucket}.cos.{region}.myqcloud.com/').format(schema=self._schema, bucket=self._bucket, region=self._region)
        else:
            url = ('{schema}://{bucket}.{endpoint}/').format(schema=self._schema, bucket=self._bucket, endpoint=self._endpoint)
        url = url.replace('./', '.%2F')
        url = url.replace('+', '%2B')
        return url
...
class Interface(object):
    ...
    def __init__(self, conf, session=None):
        ...
        self._multiupload_threshold = 20972544
        self._multidownload_threshold = 20972544
        try:
            if conf._endpoint == '':
                sdk_config = qcloud_cos.CosConfig(Region=conf._region, SecretId=conf._secret_id, SecretKey=conf._secret_key, Token=conf._token, Scheme=conf._schema, Anonymous=conf._anonymous, UA=self._ua, Timeout=self._timeout)
            else:
                sdk_config = qcloud_cos.CosConfig(Endpoint=conf._endpoint, Region=conf._region, SecretId=conf._secret_id, SecretKey=conf._secret_key, Token=conf._token, Scheme=conf._schema, Anonymous=conf._anonymous, UA=self._ua, Timeout=self._timeout)
    ...
    def list_part(self, cos_path):
        ...
        try:
            while IsTruncated == 'true':
                url = self._conf.uri(path=quote(to_printable_str(cos_path)) + ('?uploadId={UploadId}&upload&max-parts=1000&part-number-marker={nextmarker}').format(UploadId=self._upload_id, nextmarker=NextMarker))
                ...
                if rt.status_code == 200:
                    ...
                    logger.debug(('list resp, status code: {code}, headers: {headers}, text: {text}').format(code=rt.status_code, headers=rt.headers, text=to_printable_str(rt.text)))
    ...
    def upload_folder(self, local_path, cos_path, _http_headers='{}', **kwargs):
        ...
        def upload_file_list(upload_filelist):
            ...
            for _path in upload_filelist:
                ...
                try:
                    ...
                    if file_size <= self._multiupload_threshold:
                        self._inner_threadpool.add_task(self.single_upload, _local_path, _cos_path, _http_headers, **kwargs)
                    else:
            ...
            for _local_path, _cos_path in multiupload_filelist:
                rt = self.multipart_upload(_local_path, _cos_path, _http_headers, **kwargs)
                if 0 == rt:
                    _success_num += 1
                elif -2 == rt:
                    _skip_num += 1
            ...
            return (
             _success_num, _skip_num, _fail_num)
        ...
        logger.info(('{success_files} files uploaded, {skip_files} files skipped, {fail_files} files failed').format(success_files=_success_num, skip_files=_skip_num, fail_files=_fail_num))
        if kwargs['sync'] and kwargs['delete']:
            if kwargs['force'] is False:
                if query_yes_no(("WARN: you are deleting some files in the '{cos_path}' cos_path, please make sure").format(cos_path=raw_cos_path)) is False:
                    return -3
            logger.info('Synchronizing delete, please wait.')
            try:
                ...
                if ret != 0:
                    ...
                    logger.info(('{succ_files} files sync deleted, {fail_files} files sync failed').format(succ_files=del_succ, fail_files=del_fail))
            except Exception as e:
                logger.warn(to_unicode(e))
    ...
    def local2remote_sync_check(self, local_path, cos_path, **kwargs):
        ...
        if not is_include or is_ignore:
            logger.debug(('Skip {local_path}').format(bucket=self._conf._bucket, local_path=local_path))
            return -2
        if kwargs['sync'] is True:
            try:
                rt = self._client.head_object(Bucket=self._conf._bucket, Key=cos_path)
            except Exception:
                return 0
            ...
            if kwargs['_size'] == _size:
                if kwargs['skipmd5'] or kwargs['_md5'] == _md5:
                    logger.debug(('Skip {local_path}   =>   cos://{bucket}/{cos_path}').format(bucket=self._conf._bucket, local_path=local_path, cos_path=cos_path))
                    return -2
        return 0
    ...
    def single_upload(self, local_path, cos_path, _http_headers='{}', **kwargs):
        ...
        if kwargs['skipmd5'] is False:
            if file_size > 5368709120:
                logger.info('MD5 is being calculated, please wait. If you do not need to calculate md5, you can use --skipmd5 to skip')
            _md5 = get_file_md5(local_path)
        ret = self.local2remote_sync_check(local_path, cos_path, _md5=_md5, _size=file_size, **kwargs)
        if 0 != ret:
            return ret
        logger.info(('Upload {local_path}   =>   cos://{bucket}/{cos_path}').format(bucket=self._conf._bucket, local_path=local_path, cos_path=cos_path))
        try:
        ...
        for j in range(self._retry):
            try:
                if j > 0:
                    logger.info(('Retry to upload {local_path}   =>   cos://{bucket}/{cos_path}').format(bucket=self._conf._bucket, local_path=local_path, cos_path=cos_path))
                http_header = _http_header
                http_header['x-cos-meta-md5'] = _md5
                rt = self._session.put(url=url, auth=CosS3Auth(self._conf), data=data, headers=http_header, timeout=self._timeout)
                if rt.status_code == 200:
                    return 0
    ...
    def multipart_upload(self, local_path, cos_path, _http_headers='{}', **kwargs):
        ...
        def init_multiupload():
            ...
            try:
                rt = self._client.create_multipart_upload(Bucket=self._conf._bucket, Key=cos_path, **http_headers)
        ...
        def multiupload_parts():
            ...
            def multiupload_parts_data(local_path, offset, length, parts_size, idx):
                ...
                for j in range(self._retry):
                    try:
                        http_header = _http_header
                        rt = self._session.put(url=url, auth=CosS3Auth(self._conf), data=data, headers=http_header, timeout=self._timeout)
                        logger.debug(('Multi part result: part: {part}, round: {round}, code: {code}, headers: {headers}, text: {text}').format(part=idx, round=j + 1, code=rt.status_code, headers=rt.headers, text=to_printable_str(rt.text)))
                        if rt.status_code == 200:
                        ...
                        self._session.close()
                        self._session = requests.session()
                        logger.warn(('Upload part failed, key: {key}, partnumber: {part}, retrytimes: {round}, exception: {error}').format(key=cos_path, part=idx, round=j + 1, error=str(e)))
                        time.sleep(2 ** j)
            ...
            chunk_size = 1048576 * self._conf._part_size
        ...
        def complete_multiupload():
            ...
            try:
                self._client.complete_multipart_upload(self._conf._bucket, cos_path, self._upload_id, {'Part': lst})
                try:
    ...
    def remote2remote_sync_check(self, copy_source, cos_path, **kwargs):
        ...
        if 0 != ret:
            logger.debug(('Skip cos://{src_bucket}/{src_path} => cos://{dst_bucket}/{dst_path}').format(src_bucket=copy_source['Bucket'], src_path=copy_source['Key'], dst_bucket=self._conf._bucket, dst_path=cos_path))
            return -2
        if kwargs['force'] is False:
            if kwargs['sync'] is True:
                try:
    ...
    def copy_folder(self, source_path, cos_path, _http_headers='{}', **kwargs):
        ...
        try:
            if self._conf._endpoint is not None:
                ...
                sdk_config_source = qcloud_cos.CosConfig(SecretId=self._conf._secret_id, SecretKey=self._conf._secret_key, Token=self._conf._token, Endpoint=source_endpoint, Scheme=self._conf._schema, Anonymous=self._conf._anonymous)
                ...
                sdk_config_source = qcloud_cos.CosConfig(Region=source_region, SecretId=self._conf._secret_id, SecretKey=self._conf._secret_key, Token=self._conf._token, Scheme=self._conf._schema, Anonymous=self._conf._anonymous)
                self._client_source = qcloud_cos.CosS3Client(sdk_config_source)
        except Exception as e:
            logger.warn(to_unicode(e))
            logger.warn(('CopySource is invalid: {copysource}').format(copysource=source_path))
            return -1
        ...
        while IsTruncated == 'true':
            for i in range(self._retry):
                try:
                    rt = self._client_source.list_objects(Bucket=source_bucket, Marker=NextMarker, MaxKeys=1000, Delimiter='', Prefix=source_path)
                    ...
                    if 'Contents' in rt:
                        for _file in rt['Contents']:
                            ...
                            self._inner_threadpool.add_task(self.copy_file, _source_path, _cos_path, _http_headers, **kwargs)
        ...
        if kwargs['move']:
            logger.info(('{success_files} files moved, {skip_files} files skipped, {fail_files} files failed').format(success_files=_success_num, skip_files=_skip_num, fail_files=_fail_num))
        else:
            logger.info(('{success_files} files copied, {skip_files} files skipped, {fail_files} files failed').format(success_files=_success_num, skip_files=_skip_num, fail_files=_fail_num))
        if kwargs['sync'] and kwargs['delete']:
            if kwargs['force'] is False:
                if query_yes_no(("WARN: you are deleting some files in the '{cos_path}' cos_path, please make sure").format(cos_path=raw_cos_path)) is False:
                    return -3
    ...
    def copy_file(self, source_path, cos_path, _http_headers='{}', **kwargs):
        ...
        try:
            if self._conf._endpoint is not None:
                ...
                sdk_config_source = qcloud_cos.CosConfig(SecretId=self._conf._secret_id, SecretKey=self._conf._secret_key, Token=self._conf._token, Endpoint=copy_source['Endpoint'], Scheme=self._conf._schema, Anonymous=self._conf._anonymous)
                ...
                sdk_config_source = qcloud_cos.CosConfig(Region=copy_source['Region'], SecretId=self._conf._secret_id, SecretKey=self._conf._secret_key, Token=self._conf._token, Scheme=self._conf._schema, Anonymous=self._conf._anonymous)
                _source_client = qcloud_cos.CosS3Client(sdk_config_source)
        except Exception as e:
            logger.warn(to_unicode(e))
            logger.warn(('CopySource is invalid: {copysource}').format(copysource=source_path))
        ...
        if 0 != rt:
            ...
            try:
                if _move:
                    logger.info(('Move cos://{src_bucket}/{src_path}   =>   cos://{dst_bucket}/{dst_path}').format(src_bucket=copy_source['Bucket'], src_path=copy_source['Key'], dst_bucket=self._conf._bucket, dst_path=cos_path))
                    self._client.copy(Bucket=self._conf._bucket, Key=cos_path, CopySource=copy_source, CopyStatus=_directive, PartSize=self._conf._part_size, MAXThread=self._conf._max_thread, **kwargs)
                    _source_client.delete_object(Bucket=copy_source['Bucket'], Key=copy_source['Key'])
                else:
                    logger.info(('Copy cos://{src_bucket}/{src_path}   =>   cos://{dst_bucket}/{dst_path}').format(src_bucket=copy_source['Bucket'], src_path=copy_source['Key'], dst_bucket=self._conf._bucket, dst_path=cos_path))
                    rt = self._client.copy(Bucket=self._conf._bucket, Key=cos_path, CopySource=copy_source, CopyStatus=_directive, PartSize=self._conf._part_size, MAXThread=self._conf._max_thread, **kwargs)
                return 0
            except Exception as e:
                logger.warn(to_unicode(e))
                return -1
    ...
    def delete_folder(self, cos_path, **kwargs):
        if kwargs['force'] is False:
            if query_yes_no(("WARN: you are deleting the file in the '{cos_path}' cos_path, please make sure").format(cos_path=cos_path)) is False:
                return -3
        _force = kwargs['force']
        ...
        if _versions:
            ...
            while IsTruncated == 'true':
                ...
                for i in range(self._retry):
                    if VersionIdMarker == 'null':
                        VersionIdMarker = ''
                    try:
                        rt = self._client.list_objects_versions(Bucket=self._conf._bucket, KeyMarker=KeyMarker, VersionIdMarker=VersionIdMarker, MaxKeys=1000, Prefix=cos_path)
                        break
                    except Exception as e:
                        time.sleep(1 << i)
                        logger.warn(to_unicode(e))
                ...
                if 'DeleteMarker' in rt:
                    for _file in rt['DeleteMarker']:
                        ...
                        _path = _file['Key']
                        deleteList['Object'].append({'Key': _path, 'VersionId': _versionid})
                ...
                if len(deleteList['Object']) > 0:
                    rt = self._client.delete_objects(Bucket=self._conf._bucket, Delete=deleteList)
                if 'Deleted' in rt:
                    self._have_finished += len(rt['Deleted'])
                    self._file_num += len(rt['Deleted'])
                    for file in rt['Deleted']:
                        logger.info(('Delete {file}, versionId: {versionId}').format(file=file['Key'], versionId=file['VersionId']))

                if 'Error' in rt:
                    self._file_num += len(rt['Error'])
                    for file in rt['Error']:
                        logger.info(('Delete {file}, versionId: {versionId} fail, code: {code}, msg: {msg}').format(file=file['Key'], versionId=file['VersionId'], code=file['Code'], msg=file['Message']))
            ...
            while IsTruncated == 'true':
                ...
                for i in range(self._retry):
                    try:
                        rt = self._client.list_objects(Bucket=self._conf._bucket, Marker=NextMarker, MaxKeys=1000, Prefix=cos_path)
                        break
                    except Exception as e:
                        time.sleep(1 << i)
                        logger.warn(to_unicode(e))
                ...
                try:
                    ...
                    if 'Deleted' in rt:
                        self._have_finished += len(rt['Deleted'])
                        for file in rt['Deleted']:
                            logger.info(('Delete {file}').format(file=file['Key']))

                    if 'Error' in rt:
                        for file in rt['Error']:
                            logger.info(('Delete {file} fail, code: {code}, msg: {msg}').format(file=file['Key'], code=file['Code'], msg=file['Message']))
        ...
        if self._file_num == 0:
            ...
            if not _versions:
                logger.info(('{files} files successful, {fail_files} files failed').format(files=self._have_finished, fail_files=self._fail_num))
        ...
        if _versions:
            ...
            while IsTruncated == 'true':
                ...
                if 'DeleteMarker' in rt:
                    for _file in rt['DeleteMarker']:
                        _versionid = _file['VersionId']
                        _path = _file['Key']
                        deleteList.append({'Key': _path, 'VersionId': _versionid})
                ...
                if len(deleteList) > 0:
                    for file in deleteList:
                        try:
                            self._client.delete_object(Bucket=self._conf._bucket, Key=file['Key'], VersionId=file['VersionId'])
                            self._have_finished += 1
                            logger.info(('Delete {file}, versionId: {versionId}').format(file=file['Key'], versionId=file['VersionId']))
                        except Exception:
                            logger.info(('Delete {file}, versionId: {versionId} fail').format(file=file['Key'], versionId=file['VersionId']))
            ...
            while IsTruncated == 'true':
                ...
                if len(deleteList) > 0:
                    for file in deleteList:
                        try:
                            self._client.delete_object(Bucket=self._conf._bucket, Key=file['Key'])
                            self._have_finished += 1
                            logger.info(('Delete {file}').format(file=file['Key']))
                        except Exception:
                            logger.info(('Delete {file} fail').format(file=file['Key']))
    ...
    def delete_file(self, cos_path, **kwargs):
        ...
        url = self._conf.uri(path=('{path}?versionId={versionId}').format(path=quote(to_printable_str(cos_path)), versionId=_versionId))
        try:
            rt = self._session.delete(url=url, auth=CosS3Auth(self._conf))
            logger.debug(('init resp, status code: {code}, headers: {headers}').format(code=rt.status_code, headers=rt.headers))
            if rt.status_code == 204 or rt.status_code == 200:
                if _versionId == '':
                    logger.info(('Delete cos://{bucket}/{cos_path}').format(bucket=self._conf._bucket, cos_path=cos_path))
                else:
                    logger.info(('Delete cos://{bucket}/{cos_path}?versionId={versionId}').format(bucket=self._conf._bucket, cos_path=cos_path, versionId=_versionId))
                return 0
    ...
    def list_multipart_uploads(self, cos_path):
        ...
        while IsTruncated == 'true':
            IsTruncated = 'false'
            try:
                rt = self._client.list_multipart_uploads(Bucket=self._conf._bucket, Prefix=cos_path, Delimiter='', KeyMarker=KeyMarker, UploadIdMarker=UploadIdMarker, MaxUploads=10)
                if 'NextKeyMarker' in rt:
                    KeyMarker = rt['NextKeyMarker']
                ...
                if 'Upload' in rt:
                    for upload in rt['Upload']:
                        ...
                        logger.info(('Key:{key}, UploadId:{uploadid}').format(key=to_unicode(_key), uploadid=_uploadid))

            except Exception as e:
                logger.warn(to_unicode(e))

        logger.info((' Parts num: {file_num}').format(file_num=str(part_num)))
    ...
    def abort_parts(self, cos_path):
        ...
        try:
            while IsTruncated == 'true':
                ...
                for i in range(self._retry):
                    try:
                        rt = self._client.list_multipart_uploads(Bucket=self._conf._bucket, KeyMarker=NextKeyMarker, UploadIdMarker=NextUploadIdMarker, MaxUploads=1000, Prefix=cos_path)
                        break
                    except Exception as e:
                        time.sleep(1 << i)
                        logger.warn(to_unicode(e))
                ...
                if 'Upload' in rt:
                    for _file in rt['Upload']:
                        ...
                        abortList.append({'Key': _path, 'UploadId': _uploadid})

                if len(abortList) > 0:
                    for file in abortList:
                        try:
                            rt = self._client.abort_multipart_upload(Bucket=self._conf._bucket, Key=file['Key'], UploadId=file['UploadId'])
                            _success_num += 1
                            logger.info(('Abort Key: {key}, UploadId: {uploadid}').format(key=file['Key'], uploadid=file['UploadId']))
                        except Exception as e:
                            logger.warn(to_unicode(e))
                            logger.info(('Abort Key: {key}, UploadId: {uploadid} fail').format(key=file['Key'], uploadid=file['UploadId']))
                            _fail_num += 1

            logger.info(('{files} files successful, {fail_files} files failed').format(files=_success_num, fail_files=_fail_num))
            if _fail_num == 0:
                return 0
            return -1
        except Exception as e:
    ...
    def list_objects(self, cos_path, **kwargs):
            ...
            while IsTruncated == 'true':
                ...
                for i in range(self._retry):
                    try:
                        if VersionIdMarker == 'null':
                            VersionIdMarker = ''
                        rt = self._client.list_objects_versions(Bucket=self._conf._bucket, Delimiter=Delimiter, KeyMarker=KeyMarker, VersionIdMarker=VersionIdMarker, MaxKeys=1000, Prefix=cos_path)
                        break
                    except Exception as e:
                        time.sleep(1 << i)
                        logger.warn(to_unicode(e))
        ...
        while IsTruncated == 'true':
            ...
            for i in range(self._retry):
                try:
                    rt = self._client.list_objects(Bucket=self._conf._bucket, Delimiter=Delimiter, Marker=NextMarker, MaxKeys=1000, Prefix=cos_path)
                    break
                except Exception as e:
                    time.sleep(1 << i)
                    logger.warn(to_unicode(e))
    ...
    def download_folder(self, cos_path, local_path, _http_headers='{}', **kwargs):
                ...
                if 'Contents' in rt:
                    for _file in rt['Contents']:
                        ...
                        if _size <= self._multidownload_threshold:
                            self._inner_threadpool.add_task(self.single_download, _cos_path, _local_path, _http_headers, **kwargs)
                        else:
        ...
        logger.info(('{success_files} files downloaded, {skip_files} files skipped, {fail_files} files failed').format(success_files=_success_num, skip_files=_skip_num, fail_files=_fail_num))
        if kwargs['sync'] and kwargs['delete']:
            if kwargs['force'] is False:
                if query_yes_no(("WARN: you are deleting the file in the '{local_path}' local_path, please make sure").format(local_path=raw_local_path)) is False:
                    return -3
            logger.info('Synchronizing delete, please wait.')
            try:
    ...
    def remote2local_sync_check(self, cos_path, local_path, **kwargs):
        ...
        if 0 != ret:
            logger.debug(('Skip cos://{bucket}/{cos_path} => {local_path}').format(bucket=self._conf._bucket, local_path=local_path, cos_path=cos_path))
            return -2
        if kwargs['force'] is False:
            if os.path.isfile(local_path) is True:
                if kwargs['sync'] is True:
                    ...
                    logger.warn(('The file {file} already exists, please use -f to overwrite the file').format(file=local_path))
                    return -1
        return 0
    ...
    def single_download(self, cos_path, local_path, _http_headers='{}', **kwargs):
        ...
        if 0 != rt:
            return rt
        logger.info(('Download cos://{bucket}/{cos_path}   =>   {local_path}').format(bucket=self._conf._bucket, local_path=local_path, cos_path=cos_path))
        ...
        try:
            rt = self._client.get_object(Bucket=self._conf._bucket, Key=cos_path, **http_headers)
    ...
    def multipart_download(self, cos_path, local_path, _http_headers='{}', **kwargs):
        ...
        def get_parts_data(local_path, offset, length, parts_size):
            for j in range(self._retry):
                try:
                    ...
                    rt = self._client.get_object(Bucket=self._conf._bucket, Key=cos_path, **http_header)
                    fstream = rt['Body'].get_raw_stream()
                    chunk_size = 1048576
        ...
        logger.info(('Downloading {file}').format(file=local_path))
        dir_path = os.path.dirname(local_path)
        if os.path.isdir(dir_path) is False and dir_path != '':
            try:
                os.makedirs(dir_path, 493)
        ...
        for i in range(parts_num):
            if i + 1 == parts_num:
                pool.add_task(get_parts_data, local_path, offset, file_size - offset, i + 1)
            else:
                pool.add_task(get_parts_data, local_path, offset, chunk_size, i + 1)
                offset += chunk_size
        ...
        if not result['success_all'] or _fail_num > 0:
            logger.info(('{fail_num} parts download fail').format(fail_num=str(_fail_num)))
            try:
    ...
    def restore_folder(self, cos_path, **kwargs):
        ...
        logger.info(('{success_files} files successful, {progress_files} files have in progress, {fail_files} files failed').format(success_files=_success_num, progress_files=_progress_num, fail_files=_fail_num))
        if _fail_num == 0:
            return 0
        else:
            return -1
    ...
    def restore_file(self, cos_path, **kwargs):
        ...
        try:
            logger.info(('Restore cos://{bucket}/{path}').format(bucket=self._conf._bucket, path=cos_path))
            self._client.restore_object(Bucket=self._conf._bucket, Key=cos_path, RestoreRequest=restore_request)
            return 0
        except CosServiceError as e:
            if e.get_status_code() == 409 and e.get_error_code() == 'RestoreAlreadyInProgress':
                logger.warn(('cos://{bucket}/{path} already in pogress').format(bucket=self._conf._bucket, path=cos_path))
                return -2
            logger.warn(to_unicode(e))
            return -1
    ...
    def put_object_acl(self, grant_read, grant_write, grant_full_control, cos_path):
        ...
        try:
            ...
            logger.debug(('put resp, status code: {code}, headers: {headers}').format(code=rt.status_code, headers=rt.headers))
            if rt.status_code == 200:
                return True
            logger.warn(response_info(rt))
            return False
    ...
    def get_object_acl(self, cos_path):
        ...
        try:
            rt = self._session.get(url=url, auth=CosS3Auth(self._conf))
            logger.debug(('get resp, status code: {code}, headers: {headers}').format(code=rt.status_code, headers=rt.headers))
            root = minidom.parseString(rt.content).documentElement
    ...
    def create_bucket(self):
        ...
        try:
            ...
            logger.debug(('put resp, status code: {code}, headers: {headers}, text: {text}').format(code=rt.status_code, headers=rt.headers, text=rt.text))
            if rt.status_code == 200:
                logger.info(('Create cos://{bucket}').format(bucket=self._conf._bucket))
                return True
            logger.warn(response_info(rt))
            return False
        except Exception as e:
    ...
    def delete_bucket(self, **kwargs):
        ...
        try:
            if _force:
                ...
                self.delete_folder('', **kwargs)
            rt = self._session.delete(url=url, auth=CosS3Auth(self._conf))
            logger.debug(('delete resp, status code: {code}, headers: {headers}, text: {text}').format(code=rt.status_code, headers=rt.headers, text=rt.text))
            if rt.status_code == 204:
                logger.info(('Delete cos://{bucket}').format(bucket=self._conf._bucket))
                return True
            logger.warn(response_info(rt))
            return False
        except Exception as e:
