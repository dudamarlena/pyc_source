...
# Embedded file name: build/bdist.linux-x86_64/egg/coscmd/cos_cmd.py
# Compiled at: 2020-04-19 22:20:02
from six.moves.configparser import SafeConfigParser
from six import text_type
from argparse import ArgumentParser
from logging.handlers import RotatingFileHandler
import sys, logging, os, json, requests
...
def compatible(region):
    if region is None:
        ...
        _dict = {'tj': 'ap-beijing-1', 'bj': 'ap-beijing', 'gz': 'ap-guangzhou', 'sh': 'ap-shanghai', 'cd': 'ap-chengdu', 
           'spg': 'ap-singapore', 'hk': 'ap-hongkong', 'ca': 'na-toronto', 'ger': 'eu-frankfurt', 'cn-south': 'ap-guangzhou', 
...
def load_conf():
    ...
    try:
        with open(conf_path, 'r') as (f):
            ...
            if pre_region != '':
                region = pre_region
            conf = CoscmdConfig(appid=appid, secret_id=secret_id, secret_key=cp.get('common', 'secret_key'), token=token, region=compatible(region), endpoint=endpoint, bucket=bucket, part_size=part_size, max_thread=max_thread, schema=schema, anonymous=anonymous, verify=verify, retry=retry, timeout=timeout)
            return conf
...
def version_check():
    try:
        ...
        if cur_spl[0] < lat_spl[0] or cur_spl[1] < lat_spl[1] or cur_spl[2] < lat_spl[2]:
            logger.info(("The current version of coscmd is {v1} and the latest version is {v2}. It is recommended to upgrade coscmd with the command'pip install coscmd -U'.").format(v1=cos_global.Version, v2=latest_version))
    except Exception as e:
        logger.debug(e)
