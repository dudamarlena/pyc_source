...
# Embedded file name: build/bdist.linux-x86_64/egg/coscmd/cos_sync.py
...
def delete_objects(src, deleteList):
    ...
    try:
        if len(deleteList['Object']) > 0:
            rt = src['Client'].delete_objects(Bucket=src['Bucket'], Delete=deleteList)
        if 'Deleted' in rt:
            success_num += len(rt['Deleted'])
            for file in rt['Deleted']:
                logger.info(('Delete cos://{bucket}/{file}').format(bucket=src['Bucket'], file=file['Key']))

        if 'Error' in rt:
            fail_num += len(rt['Error'])
            for file in rt['Error']:
                logger.info(('Delete cos://{bucket}/{file} fail, code: {code}, msg: {msg}').format(bucket=src['Bucket'], file=file['Key'], code=file['Code'], msg=file['Message']))

    except Exception as e:
        logger.warn(e)
        return [
         0, len(deleteList['Object'])]
...
def local2remote_sync_delete(src, dst, **kwargs):
    ...
    while IsTruncated == 'true':
        ...
        for i in range(kwargs['retry']):
            try:
                rt = dst['Client'].list_objects(Bucket=dst['Bucket'], Marker=NextMarker, MaxKeys=1000, Delimiter='', Prefix=dst['Path'])
        ...
        while not q.empty():
            ...
            for filename in dirlist:
                ...
                if os.path.isdir(filepath):
                    ...
                    try:
                        src['Client'].head_object(Bucket=src['Bucket'], Key=cos_path + filename)
                    except CosServiceError as e:
                        if e.get_status_code() == 404:
                            try:
                                os.remove(filepath)
                                logger.info(('Delete {file}').format(file=filepath))
                                success_num += 1
                            except Exception:
                                logger.info(('Delete {file} fail').format(file=filepath))
                                fail_num += 1
            ...
            try:
                ...
                if 'Contents' in rt:
                    for _file in rt['Contents']:
                        ...
                        try:
                            src['Client'].head_object(Bucket=src['Bucket'], Key=_source_path)
                        except CosServiceError as e:
                            if e.get_status_code() == 404:
                                deleteList['Object'].append({'Key': _cos_path})
