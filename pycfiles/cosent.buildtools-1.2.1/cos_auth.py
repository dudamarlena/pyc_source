...
# Embedded file name: build/bdist.linux-x86_64/egg/coscmd/cos_auth.py
# Compiled at: 2020-04-17 04:18:40
import hmac, time, urllib, hashlib, logging, requests, sys
...
class CosS3Auth(AuthBase):
    ...
    def __call__(self, r):
        ...
        format_str = ('{method}\n{host}\n{params}\n{headers}\n').format(method=method.lower(), host=unquote(rt.path), params=urlencode(uri_params), headers=('&').join(map(lambda p: (lambda x, y: '%s=%s' % (x, y))(*p), sorted(headers.items()))))
        ...
        r.headers = tmp_r
        r.headers['Authorization'] = sign_tpl.format(ak=self._access_id, sign_time=sign_time, key_time=sign_time, params=(';').join(sorted(map(lambda k: k.lower(), uri_params.keys()))), headers=(';').join(sorted(headers.keys())), sign=sign)
        if self._anonymous:
