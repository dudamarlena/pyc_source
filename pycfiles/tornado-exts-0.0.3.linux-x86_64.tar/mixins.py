...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/tornado_extensions/sockjs/mixins.py
...
class EmitMixin(object):
    """EmitMixin is a mixin that helps to send message to client socket
    ...
    def flush_emits(self):
        ...
        if self._emit_queue:
            self.send(json.dumps([ {'name': name, 'args': args} for name, args in self._emit_queue
                                 ]))
        self._emit_queue = []

    def emit(self, event, *args):
        """Emits a specific message to a socket. According to the message
        ...
        if hasattr(self, '_group_emits') and self._group_emits:
            ...
            self.send(json.dumps({'name': event, 
               'args': args}))


class BroadcastMixin(object):
    """BroadcastMixin is a mixin that helps to send message to set of client
     sockets according to comet protocol."""

    def emit_broadcast(self, clients, event, *args):
        """Emits a message to set of clients.
        ...
        self.broadcast(clients, json.dumps({'name': event, 
           'args': args}))

    def msg_broadcast(self, clients, ms, *args):
        """Sends chat | feed | notification message to client-side. Depending
    ...
    def exception_msg(self, ex_type):
        """Sends an exception to client of particular type.

        Parameters
        ----------
        ex_type : str
        """
        self.send(json.dumps({'name': 'exception', 
           'args': [{'type': ex_type}]}))
