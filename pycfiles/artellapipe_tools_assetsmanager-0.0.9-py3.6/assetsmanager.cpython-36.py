...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/tools/assetsmanager/widgets/assetsmanager.py
# Compiled at: 2020-05-13 19:10:07
...
class ArtellaAssetsManager(artellapipe.ToolWidget, object):
    ...
    def show_sequence_info(self, sequence_widget):
        ...
        if not sequence_info:
            LOGGER.warning('Sequence {} has not an SequenceInfo widget associated to it. Skipping ...!'.format(sequence_widget.get_name()))
            return
    ...
    def _on_sync_all_assets_of_type(self, asset_type, ask=True):
        ...
        if ask:
            result = qtutils.show_question(None, 'Synchronizing All {} Assets ({})'.format(asset_type, total_assets), 'Are you sure you want to synchronize all {} assets ({})? This can take lot of time!'.format(asset_type, total_assets))
    ...
    def _on_sync_all_types(self, ask=True):
        ...
        total_assets = len(assets_to_sync)
        if ask:
            result = qtutils.show_question(None, 'Synchronizing All Assets ({})'.format(total_assets), 'Are you sure you want to synchronize all assets ({})? This will take lot of time!'.format(total_assets))
            if result == QMessageBox.No:
                return
        for asset in assets_to_sync:
            asset.sync(sync_type=(defines.ArtellaFileStatus.ALL))
