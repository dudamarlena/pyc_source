...
# Embedded file name: build/bdist.linux-x86_64/egg/crwy/utils/selenium_api.py
...
try:
    ...
    raise CrwyImportException('You should install selenium first! suggestion: pip install selenium==3.6.0')
...
class SeleniumApi(Spider):
    ...
    def __init__(self, driver_type='chrome', hub_url='http://127.0.0.1:4444/wd/hub', proxy=None, user_agent=None, use_hub=True, logger=None):
    ...
    def _init_chrome_driver(self):
        ...
        if self.use_hub:
            driver = webdriver.Remote(command_executor=self.hub_url, desired_capabilities=desired_capabilities)
        else:
    ...
    def _init_firefox_driver(self):
        ...
        if self.use_hub:
            driver = webdriver.Remote(command_executor=self.hub_url, desired_capabilities=desired_capabilities, browser_profile=firefox_profile)
        else:
            driver = webdriver.Firefox(firefox_profile=firefox_profile, capabilities=desired_capabilities)
        return driver

    def init_driver(self):
        if self.driver_type.upper() == 'CHROME':
            return self._init_chrome_driver()
        if self.driver_type.upper() == 'FIREFOX':
    ...
    def get_img(self, screenshot, xpath):
        ...
        right = int(element.location['x'] + element.size['width']) * self.device_pixel_ratio
        bottom = int(element.location['y'] + element.size['height']) * self.device_pixel_ratio
        img = Image.open(screenshot)
        img = img.crop((left, top, right, bottom))
        return img

    def click_img(self, answer, height, identify_img_xpath1=None, identify_button_xpath=None):
        u"""
        根据打码返回的坐标进行点击操作
        仅适用与点击型验证码
        ...
        for point in points:
            x, y = eval(point)
            actions.move_to_element_with_offset(img, x, y - int(height / self.device_pixel_ratio))
            actions.click()
    ...
    def deal_normal_verification_code(self, captcha_obj, captcha_code, identify_img_xpath):
    ...
    def deal_click_verification_code(self, captcha_obj, captcha_code, identify_img_xpath, identify_img_xpath1, identify_button_xpath):
        u"""
        处理点击型验证码
        ...
        self.click_img(answer, img1.height, identify_img_xpath1=identify_img_xpath1, identify_button_xpath=identify_button_xpath)
        os.remove(screenshot_path)
        os.remove(check_img_path)

    def get_mobile_code(self, phone_obj, phone, phone_token, check_str='智联招聘', regexp='\\d+', retry_times=20, sleep_time=5):
        while retry_times > 0:
            msg = phone_obj.get_message(token=phone_token, phone=phone)
            if check_str in msg:
                code = re.findall(regexp, msg)[0]
                self.logger.info(('{}: get mobile code success. code is: {}').format(phone, code))
                return code
            self.logger.info(('{}: no more message received. sleep {}s').format(phone, sleep_time))
    ...
    def wait_element(self, by, by_value, timeout=5):
        try:
            WebDriverWait(self.driver, timeout).until(EC.presence_of_element_located((by, by_value)))
