...
# Embedded file name: build/bdist.linux-x86_64/egg/crwy/utils/scrapy_plugs/dupefilters.py
# Compiled at: 2020-02-03 23:11:43
__doc__ = '\n@author: wuyue\n@contact: wuyue92tree@163.com\n@software: PyCharm\n@file: dupefilters.py\n@create at: 2018-06-14 14:52\n\n这一行开始写关于本文件的说明与解释\n'
import logging, time, datetime, hashlib
...
class RedisRFPDupeFilter(BaseDupeFilter):
    ...
    def __init__(self, debug=False, server=None, bot_name=None, spider_name=None, duperliter_delay_day=None, do_hash=None):
        self.debug = debug
        self.logdupes = True
        self.server = server
    ...
    def from_settings(cls, settings):
        ...
        if not spider_name:
            raise NotConfigured('%s - "SPIDER_NAME" is not found.' % cls.__name__)
        return cls(debug=debug, server=server, bot_name=bot_name, spider_name=spider_name, duperliter_delay_day=duperliter_delay_day, do_hash=do_hash)
    ...
    def request_seen(self, request):
        if not request.meta.get('dupefilter_key', None):
            ...
            if len(request.meta.get('redirect_urls', [])) > 0:
                ...
                dupefilter_key = hashlib.sha1(dupefilter_key).hexdigest() if self.do_hash else dupefilter_key
                key = ('{bot_name}:{spider_name}').format(bot_name=self.bot_name, spider_name=self.spider_name)
                if request.meta.get('duperliter_delay_day', ''):
                    self.duperliter_delay_day = int(request.meta.get('duperliter_delay_day'))
                if self.duperliter_delay_day == 0:
                    s = RedisSet(key, server=self.server)
                    if s.sadd(dupefilter_key) is True:
                        return False
                    self.logger.info('Filtered dupefilter_key: %s' % dupefilter_key)
                    return True
                ...
                if not last_time:
                    ...
                    return False
                if (datetime.datetime.utcfromtimestamp(now) - datetime.datetime.utcfromtimestamp(last_time)).days >= self.duperliter_delay_day:
                    z.zadd(now, dupefilter_key)
                    return False
    ...
    def log(self, request, spider):
        if self.debug:
            ...
            self.logger.debug(msg, {'request': request.meta.get('dupefilter_key')}, extra={'spider': spider})
        elif self.logdupes:
...
class ReleaseDupefilterKey(object):
    ...
    def call(self, spider, dupefilter_key):
        ...
        dupefilter_key = hashlib.sha1(dupefilter_key).hexdigest() if obj.do_hash else dupefilter_key
        key = ('{bot_name}:{spider_name}').format(bot_name=obj.bot_name, spider_name=obj.spider_name)
        ...
        obj.logger.info(('dupefilter_key: {} released.').format(dupefilter_key))
