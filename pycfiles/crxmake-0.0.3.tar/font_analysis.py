...
# Embedded file name: build/bdist.linux-x86_64/egg/crwy/utils/html/font_analysis.py
...
class FontAnalysis(BaseSpider):
    ...
    def analysis(self, is_clean=True, debug=False):
        ...
        with open(self.xml_path, 'rb') as (xml):
            ...
            for ttglyph in ttglyph_lst:
                ...
                analysis_dict['html_name'] = ('&#x{};').format(map_dict.get(ttglyph.get('name'))[2:])
                ttglyph_value = []
                ...
                analysis_dict['font_hash'] = hashlib.sha1(analysis_dict['ttglyph_value'].encode('utf-8')).hexdigest()
                analysis_res.append(analysis_dict)
                if debug:
                    print (
                     analysis_dict['ttglyph_name'],
                     analysis_dict['html_name'],
                     analysis_dict['font_hash'],
    ...
    def get_real_font_mapping(analysis_res, font_mapping, debug=False):
        ...
        for item in analysis_res:
            real_font_mapping[item['html_name']] = font_mapping[item['font_hash']]
            if debug is True:
                print (
                 item['html_name'], font_mapping[item['font_hash']])
...
font_mapping = {'7fd63556d48347cd5a50007b3151e2735f93bed2': '', 
