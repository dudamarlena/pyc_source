...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dbtrolls/__main__.py
# Compiled at: 2017-11-01 13:50:00
# Size of source mod 2**32: 7291 bytes
from __future__ import absolute_import, print_function, unicode_literals
import os, fnmatch, yaml, subprocess
...
def run_single(parser, options, args):
    if cfg_exist():
        ...
        if options.fix:
            print('\n Fix SQL file...\n')
            for i in cfg['fix']:
                stdout, stderr = subprocess.Popen(i, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True).communicate()
                if not stdout:
...
def main():
    ...
    parser.add_option('-c', '--config', action='store_true', help='Create config file')
    parser.add_option('--database_source', action='store')
    parser.add_option('--database_target', action='store')
    parser.add_option('-s', '--single', action='store_true', help='Execute single SQL file.')
    parser.add_option('-m', '--multiple', action='store_true', help='Execute multiple SQL files.')
    parser.add_option('-f', '--fix', action='store_true', help='Set fix true.')
    parser.add_option('-p', '--preload', action='store_true', help='Preload and Execute extra SQL files. ')
