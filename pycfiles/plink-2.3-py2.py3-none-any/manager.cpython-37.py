...
# Size of source mod 2**32: 28315 bytes
...
class LinkManager:
    ...
    def _from_string(self, contents):
        ...
        if not first_line.startswith('% Link Projection'):
            first_line.startswith('% Virtual Link Projection') or tkMessageBox.showwarning('Bad file', 'This is not a SnapPea link projection file')
        else:
            try:
                ...
                tkMessageBox.showwarning('Bad file', 'Failed while parsing line %d' % (num_lines - len(lines)))
    ...
    def polylines(self, break_at_overcrossings=True):
        ...
        segments = {}
        for arrow in self.Arrows:
            arrows_segments = arrow.find_segments((self.Crossings),
                ...
                for ec in this_component:
                    ...
                    if crossing.DT_hit(count, ec):
                        if crossing.comp2 in components:
                            ...
                            if crossing.comp1 in components:
    ...
    def DT_code(self, alpha=False, signed=True, return_sizes=False):
        ...
        if signed:
            ...
            if prefix_ints[0] > 26:
                tkMessageBox.showwarning('Error', 'Alphabetical DT codes require fewer than 26 crossings.')
                return
            ...
            for arrow in component:
                ...
                for index, virtual_crossing in enumerate(virtual_crossings):
                    if arrow == virtual_crossing.under:
                        ...
                        if arrow == virtual_crossing.over:
