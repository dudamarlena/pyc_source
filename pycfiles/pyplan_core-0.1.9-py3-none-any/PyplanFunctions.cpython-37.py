...
# Compiled at: 2020-05-05 17:38:58
# Size of source mod 2**32: 58131 bytes
import pandas as pd, xarray as xr, numpy as np, re, os, importlib, subprocess
try:
...
class PyplanFunctions(object):
    ...
    def split_text(self, param1, separator, part=None):
        ...
        for _part in range(_max_q_separators + 1):
            _dataarray = self.apply_fn(param1, lambda x: x.split(separator)[_part])
    ...
    def linear_depreciation(self, investments, usefulLife, timeIndex, includeInCurrentMonth=False, timeIndexFormat='%Y.%m'):
        ...
        _ending = self.dataarray_from_pandas(_df,
          _da_dims, valueColumns='ending', defaultValue='')
        _depreciations = investments * 0.0
        for t in timeIndex:
            _ending_month = self.subscript(_ending, timeIndex, t)
            _depreciation_amount_t = self.subscript(_depreciation, timeIndex, t)
            if includeInCurrentMonth:
                _depreciacion_t = xr.where((self.to_dataarray(timeIndex) >= t) & (self.to_dataarray(timeIndex) < _ending_month), _depreciation_amount_t, 0.0)
            else:
                _depreciacion_t = xr.where((self.to_dataarray(timeIndex) > t) & (self.to_dataarray(timeIndex) <= _ending_month), _depreciation_amount_t, 0.0)
    ...
    def irr(self, flow, time_index):
        ...
        if len(_rest_of_indexes_labels) == 0:
            ...
            _cube = self.create_dataarray(0.0, _rest_of_indexes)
    ...
    def copy_as_values(self, source, targetId):
        ...
        if not isinstance(source, xr.DataArray):
            if not isinstance(source, pd.Index):
        ...
        if isinstance(source, float) or isinstance(source, int):
            ...
            if isinstance(source, xr.DataArray):
                ...
                _data = np.array2string((source.values), separator=',', precision=20, formatter={'float_kind': lambda x:                                if np.isnan(x):
...
'np.nan' # Avoid dead code: repr(x)}).replace('\n', '')
    ...
    def subscript(self, dataArray, indexes, values):
        ...
        if not isinstance(dataArray, xr.DataArray):
            ...
            if not isinstance(indexes, list):
                indexes = [
                 indexes]
            values = isinstance(values, list) or [
             values]
        res = dataArray
        filterDic = {}
    ...
    def index_from_excel(self, excel, sheetName=None, namedRange=None, cellRange=None, columnName=None, removeEmpty=True):
        ...
        if isinstance(excel, str) or 'openpyxl.workbook' in str(type(excel)):
            _df = self.pandas_from_excel(excel, sheetName, namedRange, cellRange)
    ...
    def dataarray_from_pandas(self, dataframe, domainDic, valueColumns, defaultValue=None, valueColumnsAsDim=True, sumDuplicateRecords=True):
        ...
        if isinstance(valueColumns, pd.Index):
            ...
            if isinstance(valueColumns, np.ndarray):
                valueColumns = valueColumns.tolist()
            else:
                if not isinstance(valueColumns, str):
                    if not isinstance(valueColumns, list):
                        ...
                        if valueColumnsAsDim is True:
                            if isinstance(_index_value_columns, pd.Index):
                    ...
                    if valueColumnsAsDim is True and isinstance(_index_value_columns, pd.Index):
                        _df = pd.melt(_df, id_vars=_index_cols, value_vars=_filtered_value_columns, var_name=_index_value_columns_name,
                          value_name='values')
    ...
    def dataarray_from_excel(self, excel, sheetName=None, namedRange=None, cellRange=None, indexes=None, valueColumns=None, indexColumnHeaders=None, replaceByIndex=None, defaultValue=0):
        ...
        """
        dataframe = self.pandas_from_excel(excel, sheetName, namedRange, cellRange)
        if len(dataframe) == 0:
            if not isinstance(indexes, list):
                indexes = [
                 indexes]
            ...
            if isinstance(valueColumns, str):
                ...
                if indexColumnHeaders is None:
                    ...
                    if valueIndex is not None:
                        ...
                        _full = dataframe.reset_index().melt(id_vars=indexColumnHeaders, value_vars=valueColumns,
                          var_name='data_index',
                    ...
                    if indexes is not None and indexColumnHeaders is not None:
                        if not isinstance(indexes, list):
                            indexes = [
                             indexes]
                        idxPos = 0
                        for cubeIndex in indexes:
                            ...
                            if idxPos <= len(indexColumnHeaders) - 1:
                                ...
                                if newIndexName not in _da.coords:
    ...
    def goal_seek(self, nodeIdX, nodeIdObjective, goal=0, startValue=1, matrixIndex=None):
        ...
        if self._exists_module('scipy'):
            ...
            if matrixIndex is None:
                ...
                return _res
            raise ValueError('Multidimensional goal seek has not Implemented')
        else:
    ...
    def fill_inf(self, dataArray, value=0):
        ...
        """
        return self.apply_fn(dataArray, lambda x:         if np.isinf(x):
value # Avoid dead code: x)
    ...
    def subindex(self, dataArray, targetValue, targetIndex, method='Last'):
        ...
        if method == 'Last':
            ...
            _matriz_max = xr.where(_matriz_1_0_acum == _max, self.to_dataarray(targetIndex), np.nan)
            return _matriz_max.max(targetIndex.name)
        if method == 'First':
            _matriz_min = xr.where(_matriz_1_0_acum == 1, self.to_dataarray(targetIndex), np.nan)
    ...
    def concat_rows(self, array_param, index_param):
        ...
        for i in index_param.values:
            _index = self.concat_index(_index, pd.Index(array_param.sel({index_param.name: i}, drop=True).values))
...
class Pandas_from_acc:
    ...
    TABLE_RE = re.compile('CREATE TABLE \\[([a-zA-Z_0-9 ]+)\\]\\s+\\((.*?\\));', re.MULTILINE | re.DOTALL)
    ...
    def list_tables(cls, rdb_file, encoding='latin-1'):
        ...
        """
        tables = cls._Pandas_from_acc__get_tables(rdb_file, encoding)
    ...
    def read_schema(cls, rdb_file, encoding='utf8'):
        ...
        schema_ddl = '\n'.join((l for l in lines if l if not l.startswith('-')))
        tables = cls._Pandas_from_acc__get_tables(rdb_file, encoding)
        schema = {}
        for table, defs in tables:
            schema[table] = cls._Pandas_from_acc__extract_defs(defs)
    ...
    def to_pandas_schema(cls, schema, implicit_string=True):
        ...
        for tbl, defs in schema.items():
            ...
            for column, data_type in defs.items():
                dtype = cls._Pandas_from_acc__extract_dtype(data_type)
    ...
    def read_table(cls, rdb_file, table_name, *args, **kwargs):
        ...
        if kwargs.pop('converters_from_schema', True):
            ...
            schemas = cls.to_pandas_schema(cls.read_schema(rdb_file, schema_encoding), kwargs.pop('implicit_string', True))
            dtypes = schemas[table_name]
            dtypes.update(specified_dtypes)
            if dtypes != {}:
                kwargs['dtype'] = dtypes
        cmd = [
         'mdb-export', rdb_file, table_name]
    ...
    def __extract_dtype(data_type):
        ...
        if data_type.startswith('double'):
            return np.float_
        if data_type.startswith('long'):
            return np.float_
        if data_type.startswith('bool'):
            return np.bool_
        if data_type.startswith('text') or data_type.startswith('memo'):
            return np.str_
