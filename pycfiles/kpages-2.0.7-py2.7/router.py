...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/kpages/router.py
# Compiled at: 2019-08-12 20:22:37
"""
    Request router for tornado
    author comger@gmail.com
"""
import os, sys, tornado.web
    ...
    def actual(handler):
        ...
        if not issubclass(handler, tornado.web.RequestHandler):
            ...
            if not hasattr(handler, '__urls__') or not handler.__urls__:
                handler.__urls__ = []
            p = pattern or ('/{0}/{1}').format(handler.__module__, handler.__name__).lower()
            p = p.replace('.', '/')
...
def _sorted_hanlders(handlers):
    """ sort handlers """
    handlers = [ (pattern, order, h) for h in handlers for pattern, order in h.__urls__
               ]
...
__all__ = [
 'reg_ui_method', 'url', 'load_handlers']