...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/kpages/context.py
# Compiled at: 2019-08-12 20:16:25
"""
    author comger@gmail.com
    logic context for db, cache
    db is mongodb,cache is redis
"""
import os, time, tornado.gen as gen
...
class LogicContext(object):
    ...
    def get_redis(cls):
        ...
        if not hasattr(cls, '__cache__') or cls.__cache__ and not cls.__cache__.ping():
            cls.__cache__ = Redis(host=h, port=int(p), socket_timeout=__conf__.SOCK_TIMEOUT)
    ...
    def session(self, key, val=None, expire=None):
        ...
        if val:
            return self.get_redis().setex(key, val, expire)
    ...
    def get_context(cls, **kwargs):
        return hasattr(cls._thread_local, 'contexts') and cls._thread_local.contexts and cls._thread_local.contexts[(-1)] or cls(mongo_host=kwargs.get('db_host'))
    ...
    def get_mongoclient(cls, name=None):
        ...
        if not hasattr(cls, '__mongoclient__'):
            cls.__mongoclient__ = MongoClient(host=__conf__.DB_HOST, socketTimeoutMS=__conf__.SOCK_TIMEOUT_MS)
...
__all__ = [
 'ContextHandler', 'LogicContext', 'get_context']