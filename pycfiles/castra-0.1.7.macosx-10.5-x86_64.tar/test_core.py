...
# [GCC 8.4.0]
# Embedded file name: /Users/jcrist/anaconda/envs/castra/lib/python2.7/site-packages/castra/tests/test_core.py
# Compiled at: 2015-09-30 01:59:48
import os, tempfile, pickle, shutil, pandas as pd, pandas.util.testing as tm, pytest, numpy as np
from castra import Castra
from castra.core import mkdir, select_partitions, _decategorize, _categorize
A = pd.DataFrame({'x': [1, 2], 'y': [
       1.0, 2.0]}, columns=[
 'x', 'y'], index=[
 1, 2])
B = pd.DataFrame({'x': [10, 20], 'y': [
       10.0, 20.0]}, columns=[
 'x', 'y'], index=[
 10, 20])
C = pd.DataFrame({'x': [10, 20], 'y': [
       10.0, 20.0], 
   'z': [
       0, 1]}, columns=[
 'x', 'y', 'z']).set_index('z')
...
def test_timeseries():
    indices = [ pd.DatetimeIndex(start=str(i), end=str(i + 1), freq='w') for i in range(2000, 2015)
              ]
    dfs = [ pd.DataFrame({'x': list(range(len(ind)))}, ind).iloc[:-1] for ind in indices
          ]
...
def test_text():
    df = pd.DataFrame({'name': ['Alice', 'Bob'], 'balance': [
                 100, 200]}, columns=['name', 'balance'])
...
def test_categorize():
    A = pd.DataFrame({'x': [1, 2, 3], 'y': ['A', None, 'A']}, columns=[
     'x', 'y'], index=[0, 10, 20])
    B = pd.DataFrame({'x': [4, 5, 6], 'y': ['C', None, 'A']}, columns=[
     'x', 'y'], index=[30, 40, 50])
...
def test_same_categories_when_already_categorized():
    A = pd.DataFrame({'x': [1, 2] * 1000, 'y': [
           1.0, 2.0] * 1000, 
       'z': np.random.choice(list('abc'), size=2000)}, columns=list('xyz'))
...
def test_category_dtype():
    A = pd.DataFrame({'x': [1, 2] * 3, 'y': [
           1.0, 2.0] * 3, 
       'z': list('abcabc')}, columns=list('xyz'))
    with Castra(template=A, categories=['z']) as (c):
        c.extend(A)
...
def test_first_index_is_timestamp():
    pytest.importorskip('dask.dataframe')
    df = pd.DataFrame({'x': [1, 2] * 3, 'y': [
           1.0, 2.0] * 3, 
       'z': list('abcabc')}, columns=list('xyz'), index=pd.date_range(start='20120101', periods=6))
    with Castra(template=df) as (c):
        c.extend(df)
...
def test_extend_sequence_freq():
    ...
    with Castra(template=df) as (c):
        ...
        tm.assert_frame_equal(c[:], df)
        parts = pd.date_range(start=df.index[59], freq='h', periods=16).insert(17, df.index[(-1)])
...
def test_categorical_index():
    df = pd.DataFrame({'x': [1, 2, 3]}, index=pd.CategoricalIndex(['a', 'a', 'b'], ordered=True, name='foo'))
    with Castra(template=df, categories=True) as (c):
        c.extend(df)
        result = c[:]
        tm.assert_frame_equal(c[:], df)
    A = pd.DataFrame({'x': [1, 2, 3]}, index=pd.Index(['a', 'a', 'b'], name='foo'))
    B = pd.DataFrame({'x': [4, 5, 6]}, index=pd.Index(['c', 'd', 'd'], name='foo'))
    path = tempfile.mkdtemp(prefix='castra-')
    try:
        with Castra(path=path, template=A, categories=['foo']) as (c):
            ...
            expected.index = pd.CategoricalIndex(expected.index, name=expected.index.name, ordered=True)
...
def test_categorical_index_with_dask_dataframe():
    ...
    A = pd.DataFrame({'x': [1, 2, 3, 4]}, index=pd.Index(['a', 'a', 'b', 'b'], name='foo'))
    B = pd.DataFrame({'x': [4, 5, 6]}, index=pd.Index(['c', 'd', 'd'], name='foo'))
    path = tempfile.mkdtemp(prefix='castra-')
    try:
        with Castra(path=path, template=A, categories=['foo']) as (c):
            ...
            tm.assert_frame_equal(df.loc['b'].compute(get=dask.async.get_sync), expected.loc['b'])
    finally:
        shutil.rmtree(path)
