...
# [GCC 8.4.0]
# Embedded file name: /Users/jcrist/anaconda/envs/castra/lib/python2.7/site-packages/castra/core.py
# Compiled at: 2016-03-11 15:27:13
...
import blosc, bloscpack, numpy as np, pandas as pd
...
class Castra(object):
    meta_fields = [
     'columns', 'dtypes', 'index_dtype', 'axis_names']
    ...
    def __init__(self, path=None, template=None, categories=None, readonly=False):
        ...
        if isdir(self.dirname('meta')):
            if template is not None:
                raise ValueError('Opening a castra with a template, yet this castra\nalready exists.  Filename: %s' % self.path)
            self.load_meta()
            ...
            if isinstance(categories, (list, tuple)):
                ...
                if categories is True:
                    self.categories = dict((col, []) for col in template.columns if template.dtypes[col] == 'object')
                ...
                if self.categories:
                    ...
                    if categories.difference(template_categories) - set(['.index']):
                        raise ValueError('passed in categories %s are not all contained in template dataframe columns %s' % (
                         categories, template_categories))
                template2 = _decategorize(self.categories, template)[2]
                self.columns, self.dtypes, self.index_dtype = list(template2.columns), template2.dtypes, template2.index.dtype
            ...
            raise ValueError("must specify a 'template' when creating a new Castra")
        return

    def _empty_dataframe(self):
        return pd.DataFrame([], columns=pd.Index(self.columns, name=self.axis_names[1]), index=pd.Index([], name=self.axis_names[0]))
        ...
        for col in list(self.columns) + ['.index']:
            ...
            if os.path.exists(fn):
                with open(fn, 'rb') as (f):
                    text = f.read()
                self.categories[col] = [ loads(x) for x in text.split(separator)[:-1] ]
    ...
    def extend(self, df):
        ...
        if len(self.partitions) and df.index[0] <= self.partitions.index[(-1)]:
            if is_trivial_index(df.index):
                ...
                new_index = pd.Index(np.arange(start, start + len(df)), name=df.index.name)
                df.index = new_index
            else:
        ...
        bloscpack.pack_ndarray_file(index, fn, bloscpack_args=bp_args, blosc_args=blosc_args(index.dtype))
    ...
    def load_partition(self, name, columns, categorize=True):
        ...
        df = pd.DataFrame(dict(zip(columns, arrays)), columns=pd.Index(columns, name=self.axis_names[1], tupleize_cols=False), index=self.load_index(name))
        if categorize:
            df = _categorize(self.categories, df)
        return df

    def load_index(self, name):
        return pd.Index(unpack_file(self.dirname(name, '.index')), dtype=self.index_dtype, name=self.axis_names[0], tupleize_cols=False)
    ...
    def __getitem__(self, key):
        ...
        if not names:
            ...
            data_frames = [ self.load_partition(name, columns, categorize=False) for name in names
                          ]
    ...
    def __getstate__(self):
        if not self._readonly:
            self.flush()
        return (
         self.path, self._explicitly_given_path, self._readonly)
    ...
    def to_dask(self, columns=None):
        ...
        divisions = [
         self.minimum] + self.partitions.index.tolist()
        if '.index' in self.categories:
            divisions = [
             self.categories['.index'][0]] + [ self.categories['.index'][(d + 1)] for d in divisions[1:-1] ] + [self.categories['.index'][(-1)]]
        key_parts = list(enumerate(self.partitions.values))
        dsk = dict(((name, i), (Castra.load_partition, self, part, columns)) for i, part in key_parts)
...
def pack_file(x, fn, encoding='utf8'):
    ...
    """
    if x.dtype != 'O':
        bloscpack.pack_ndarray_file(x, fn, bloscpack_args=bp_args, blosc_args=blosc_args(x.dtype))
    else:
...
def unpack_file(fn, encoding='utf8'):
    ...
    try:
        ...
        with open(fn, 'rb') as (f):
            data = msgpack.unpackb(blosc.decompress(f.read()), encoding=encoding)
...
def _categorize(categories, df):
    ...
    if isinstance(df, pd.Series):
        return pd.Series(make_categorical(df, categories), index=make_categorical(df.index, categories), name=df.name)
    else:
        return pd.DataFrame(dict((col, make_categorical(df[col], categories)) for col in df.columns), columns=df.columns, index=make_categorical(df.index, categories))
...
def partitionby_freq(freq, buf, new):
    ...
    if not df.index.is_monotonic_increasing:
        df = df.sort_index(inplace=False)
    start, end = pd.tseries.resample._get_range_edges(df.index[0], df.index[(-1)], freq)
    inds = [ df.index.searchsorted(i) for i in pd.date_range(start, end, freq=freq)[1:]
           ]
    slices = [ (inds[(i - 1)], inds[i]) if i else (0, inds[i]) for i in range(len(inds))
             ]
