...
# Size of source mod 2**32: 37288 bytes
...
class Rolling(_RollingAndExpanding):
    ...
    def __init__(self, kdf_or_kser, window, min_periods=None):
        ...
        if isinstance(kdf_or_kser, (DataFrame, Series)):
            ...
            self._window = Window.orderBy(self._index_scols).rowsBetween(Window.currentRow - (self._window_val - 1), Window.currentRow)
    ...
    def _apply_as_series_or_frame(self, func):
        ...
        if isinstance(self.kdf_or_kser, DataFrame):
            kdf = self.kdf_or_kser
            applied = []
            for column in kdf.columns:
                applied.append(getattr(kdf[column].rolling(self._window_val, self._min_periods), func.__name__)())
    ...
    def sum(self):
        ...
        def sum(scol):
            ...
            return F.when(F.lag(scol, self._window_val - 1).over(window) >= self._min_periods, F.sum(scol).over(self._window)).otherwise(F.lit(None))
    ...
    def min(self):
        ...
        def min(scol):
            window = Window.orderBy(self._index_scols)
            return F.when(F.lag(scol, self._window_val - 1).over(window) >= self._min_periods, F.min(scol).over(self._window)).otherwise(F.lit(None))
    ...
    def max(self):
        ...
        def max(scol):
            window = Window.orderBy(self._index_scols)
            return F.when(F.lag(scol, self._window_val - 1).over(window) >= self._min_periods, F.max(scol).over(self._window)).otherwise(F.lit(None))
    ...
    def mean(self):
        ...
        def mean(scol):
            window = Window.orderBy(self._index_scols)
            return F.when(F.lag(scol, self._window_val - 1).over(window) >= self._min_periods, F.mean(scol).over(self._window)).otherwise(F.lit(None))
...
class Expanding(_RollingAndExpanding):
    ...
    def __init__(self, kdf_or_kser, min_periods=1):
        ...
        self._window = Window.orderBy(index_scols).rowsBetween(Window.unboundedPreceding, Window.currentRow)
    ...
    def _apply_as_series_or_frame(self, func):
        ...
        if isinstance(self.kdf_or_kser, DataFrame):
            ...
            applied = []
            for column in kdf.columns:
                applied.append(getattr(kdf[column].expanding(self._min_periods), func.__name__)())
    ...
    def sum(self):
        ...
        def sum(scol):
            return F.when(F.row_number().over(self._window) >= self._min_periods, F.sum(scol).over(self._window)).otherwise(F.lit(None))
    ...
    def min(self):
        ...
        def min(scol):
            return F.when(F.row_number().over(self._window) >= self._min_periods, F.min(scol).over(self._window)).otherwise(F.lit(None))
    ...
    def max(self):
        ...
        def max(scol):
            return F.when(F.row_number().over(self._window) >= self._min_periods, F.max(scol).over(self._window)).otherwise(F.lit(None))
    ...
    def mean(self):
        ...
        def mean(scol):
            return F.when(F.row_number().over(self._window) >= self._min_periods, F.mean(scol).over(self._window)).otherwise(F.lit(None))
