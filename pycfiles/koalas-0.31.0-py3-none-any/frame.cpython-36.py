...
# [GCC 8.4.0]
# Embedded file name: ./build/lib/databricks/koalas/frame.py
# Compiled at: 2019-11-07 14:25:58
# Size of source mod 2**32: 292943 bytes
"""
A wrapper class for Spark DataFrame to behave similar to pandas DataFrame.
"""
from collections import OrderedDict
from distutils.version import LooseVersion
import re, warnings, inspect, json
...
from pyspark.sql.types import BooleanType, ByteType, DecimalType, DoubleType, FloatType, IntegerType, LongType, NumericType, ShortType
...
class DataFrame(_Frame, Generic[T]):
    ...
    def _map_series_op(self, op, other):
        ...
        if not isinstance(other, DataFrame):
            if isinstance(other, IndexOpsMixin) or is_sequence(other):
                raise ValueError('%s with a sequence is currently not supported; however, got %s.' % (
                 op, type(other)))
        if isinstance(other, DataFrame) and self is not other:
            ...
            sdf = self._sdf.select(self._internal.index_scols + [c._scol for c in applied])
            internal = self._internal.copy(sdf=sdf, column_index=[c._internal.column_index[0] for c in applied],
    ...
    add.__doc__ = _flex_doc_FRAME.format(desc='Addition',
      op_name='+',
    ...
    radd.__doc__ = _flex_doc_FRAME.format(desc='Addition',
      op_name='+',
    ...
    div.__doc__ = _flex_doc_FRAME.format(desc='Floating division',
      op_name='/',
      equiv='dataframe / other',
      reverse='rdiv')
    divide = div
    ...
    rdiv.__doc__ = _flex_doc_FRAME.format(desc='Floating division',
      op_name='/',
      equiv='other / dataframe',
      reverse='div')
    ...
    mul.__doc__ = _flex_doc_FRAME.format(desc='Multiplication',
      op_name='*',
      equiv='dataframe * other',
      reverse='rmul')
    multiply = mul
    ...
    rmul.__doc__ = _flex_doc_FRAME.format(desc='Multiplication',
      op_name='*',
      equiv='other * dataframe',
      reverse='mul')
    ...
    sub.__doc__ = _flex_doc_FRAME.format(desc='Subtraction',
      op_name='-',
      equiv='dataframe - other',
      reverse='rsub')
    subtract = sub
    ...
    rsub.__doc__ = _flex_doc_FRAME.format(desc='Subtraction',
      op_name='-',
      equiv='other - dataframe',
      reverse='sub')
    ...
    mod.__doc__ = _flex_doc_FRAME.format(desc='Modulo',
      op_name='%',
      equiv='dataframe % other',
      reverse='rmod')
    ...
    rmod.__doc__ = _flex_doc_FRAME.format(desc='Modulo',
      op_name='%',
      equiv='other % dataframe',
      reverse='mod')
    ...
    rpow.__doc__ = _flex_doc_FRAME.format(desc='Exponential power',
      op_name='**',
      equiv='other ** dataframe',
      reverse='pow')
    ...
    def aggregate(self, func: Union[(List[str], Dict[(str, List[str])])]):
        ...
        if not isinstance(func, dict) or not all(isinstance(key, str) and (isinstance(value, str) or isinstance(value, list) and all(isinstance(v, str) for v in value)) for key, value in func.items()):
            raise ValueError('aggs must be a dict mapping from column name (string) to aggregate functions (list of strings).')
    ...
    def transpose(self):
        ...
        **Non-square DataFrame with mixed dtypes**
        ...
        if max_compute_count is not None:
            ...
            pairs = F.explode((F.array)(*[F.struct([F.lit(col).alias(SPARK_INDEX_NAME_FORMAT(i)) for i, col in enumerate(idx)] + [self[idx]._scol.alias('value')]) for idx in self._internal.column_index]))
            exploded_df = self._sdf.withColumn('pairs', pairs).select([
             F.to_json(F.struct(F.array([scol.cast('string') for scol in self._internal.index_scols]).alias('a'))).alias('index'),
             F.col('pairs.*')])
            internal_index_columns = [SPARK_INDEX_NAME_FORMAT(i) for i in range(self._internal.column_index_level)]
            pivoted_df = exploded_df.groupBy(internal_index_columns).pivot('index')
            transposed_df = pivoted_df.agg(F.first(F.col('value')))
            new_data_columns = list(filter(lambda x: x not in internal_index_columns, transposed_df.columns))
            internal = self._internal.copy(sdf=transposed_df,
    ...
    def transform(self, func):
        ...
        if not callable(func):
            raise AssertionError('the first argument should be a callable function.')
        else:
            ...
            if should_infer_schema:
                ...
                if len(pdf) <= limit:
                    return kdf
                applied = []
                for input_idx, output_idx in zip(self._internal.column_index, kdf._internal.column_index):
    ...
    def xs(self, key, axis=0, level=None):
        ...
        if not isinstance(key, (str, tuple)):
            ...
            if not all(isinstance(index, str) for index in key):
                raise ValueError("'key' should have index names as only strings or a tuple that contain index names as only strings")
            else:
                if axis not in (0, 'index'):
                    ...
                    if len(key) > len(self._internal.index_scols):
                        raise KeyError('Key length ({}) exceeds index depth ({})'.format(len(key), len(self._internal.index_scols)))
                if level is None:
                    level = 0
            ...
            sdf = self._sdf.select(scols).where(reduce(lambda x, y: x & y, rows))
            if len(key) == len(self._internal.index_scols):
                result = _col(DataFrame(_InternalFrame(sdf=sdf)).T)
                result.name = key
            else:
                internal = self._internal.copy(sdf=sdf,
                  index_map=(self._internal.index_map[:level] + self._internal.index_map[level + len(key):]))
    ...
    def set_index(self, keys, drop=True, append=False, inplace=False):
        ...
        if append:
            index_map = self._internal.index_map + [(self._internal.column_name_for(idx), idx) for idx in keys]
        else:
            index_map = [(self._internal.column_name_for(idx), idx) for idx in keys]
        internal = self._internal.copy(index_map=index_map, column_index=column_index,
    ...
    def reset_index(self, level=None, drop=False, inplace=False, col_level=0, col_fill=''):
        ...
        if level is None:
            new_index_map = [(column, name if name is not None else rename(i)) for i, (column, name) in enumerate(self._internal.index_map)]
            index_map = []
        else:
            ...
            if all(isinstance(l, int) for l in level):
                for lev in level:
                    if lev >= len(self._internal.index_map):
                        raise IndexError('Too many levels: Index has only {} level, not {}'.format(len(self._internal.index_map), lev + 1))

                idx = level
            else:
                if all(isinstance(lev, str) for lev in level):
                    idx = []
                    for l in level:
                        try:
                            ...
                            if multi_index:
                                ...
                                raise KeyError('Level unknown must be same as name ({})'.format(self._internal.index_columns[0]))
            ...
            new_data_scols = [self._internal.scol_for(column).alias(name_like_string(name)) for column, name in new_index_map]
            if len(index_map) > 0:
                index_scols = [scol_for(self._sdf, column) for column, _ in index_map]
                sdf = self._sdf.select(index_scols + new_data_scols + self._internal.column_scols)
            else:
                ...
                index_map = [
                 (
                  SPARK_INDEX_NAME_FORMAT(0), None)]
        ...
        if drop:
            ...
            internal = self._internal.copy(sdf=sdf,
              index_map=index_map,
              column_index=None,
              column_scols=([scol_for(sdf, name_like_string(name)) for _, name in new_index_map] + self._internal.column_scols))
            if self._internal.column_index_level > 1:
                ...
                columns = pd.MultiIndex.from_tuples([tuple([col_fill] * col_level + list(name) + [col_fill] * (column_depth - (len(name) + col_level))) for _, name in new_index_map] + self._internal.column_index)
            else:
    ...
    def nunique(self, axis: Union[(int, str)]=0, dropna: bool=True, approx: bool=False, rsd: float=0.05) -> pd.Series:
        """
        ...
        if axis not in (0, 'index'):
            ...
            res = self._sdf.select([self[idx]._nunique(dropna, approx, rsd) for idx in self._internal.column_index]).toPandas()
            if self._internal.column_index_level == 1:
    ...
    def round(self, decimals=0):
        ...
        if isinstance(decimals, ks.Series):
            decimals_list = [(k if isinstance(k, tuple) else (k,), v) for k, v in decimals._to_internal_pandas().items()]
        else:
            if isinstance(decimals, dict):
                decimals_list = [(k if isinstance(k, tuple) else (k,), v) for k, v in decimals.items()]
            else:
        ...
        return DataFrame(self._internal.copy(sdf=sdf, column_scols=[scol_for(sdf, col) for col in self._internal.data_columns]))
    ...
    def duplicated(self, subset=None, keep='first'):
        ...
        if len(diff) > 0:
            ...
            if column == index_column:
                ...
                sdf = sdf.select([self._internal.index_scols[0].alias(index_column)] + self._internal.data_scols)
            if keep == 'first' or keep == 'last':
                if keep == 'first':
                    ord_func = spark.functions.asc
                else:
                    ord_func = spark.functions.desc
                window = Window.partitionBy(group_cols).orderBy(ord_func(index_column)).rowsBetween(Window.unboundedPreceding, Window.currentRow)
                sdf = sdf.withColumn(column, F.row_number().over(window) > 1)
            else:
                if not keep:
                    window = Window.partitionBy(group_cols).orderBy(scol_for(sdf, index_column).desc()).rowsBetween(Window.unboundedPreceding, Window.unboundedFollowing)
        ...
        return _col(DataFrame(_InternalFrame(sdf=sdf, index_map=[
         (
          index_column,
    ...
    def to_table(self, name: str, format: Optional[str]=None, mode: str='error', partition_cols: Union[(str, List[str], None)]=None, **options):
        """
        ...
        """
        (self.to_spark().write.saveAsTable)(name=name, format=format, mode=mode, partitionBy=partition_cols, **options)

    def to_delta(self, path: str, mode: str='error', partition_cols: Union[(str, List[str], None)]=None, **options):
        """
        ...
        """
        (self.to_spark_io)(path=path, 
         mode=mode, format='delta', partition_cols=partition_cols, **options)

    def to_parquet(self, path: str, mode: str='error', partition_cols: Union[(str, List[str], None)]=None, compression: Optional[str]=None):
        """
        ...
        """
        self.to_spark().write.parquet(path=path,
          mode=mode,
          partitionBy=partition_cols,
          compression=compression)

    def to_spark_io(self, path: Optional[str]=None, format: Optional[str]=None, mode: str='error', partition_cols: Union[(str, List[str], None)]=None, **options):
        ...
        """
        (self.to_spark().write.save)(path=path, 
         format=format, mode=mode, partitionBy=partition_cols, **options)
    ...
    def to_spark(self, index_col: Optional[Union[(str, List[str])]]=None):
        ...
        if index_col is None:
            ...
            if isinstance(index_col, str):
                ...
                data_column_names = []
                data_columns = []
                data_columns_column_index = zip(self._internal.data_columns, self._internal.column_index)
                ...
                if len(index_col) != len(old_index_scols):
                    raise ValueError("length of index columns is %s; however, the length of the given 'index_col' is %s." % (
                     len(old_index_scols), len(index_col)))
    ...
    def _assign(self, kwargs):
        ...
        for k, v in kwargs.items():
            if not (isinstance(v, (Series, spark.Column)) or callable(v) or pd.api.types.is_scalar(v)):
                raise TypeError("Column assignment doesn't support type {0}".format(type(v).__name__))
            if callable(v):
                kwargs[k] = v(self)

        pairs = {(k if isinstance(k, tuple) else (k,)):(v._scol if isinstance(v, Series) else v if isinstance(v, spark.Column) else F.lit(v)) for k, v in kwargs.items()}
        scols = []
        for idx in self._internal.column_index:
        ...
        adding_column_index = [tuple(list(idx) + [''] * (level - len(idx))) for idx in adding_column_index]
        internal = self._internal.copy(sdf=sdf,
          column_index=(self._internal.column_index + adding_column_index),
          column_scols=[scol_for(sdf, col) for col in self._internal.data_columns + adding_data_columns])
        return DataFrame(internal)

    @staticmethod
    def from_records(data: Union[(np.array, List[tuple], dict, pd.DataFrame)], index: Union[(str, list, np.array)]=None, exclude: list=None, columns: list=None, coerce_float: bool=False, nrows: int=None) -> 'DataFrame':
        """
    ...
    def to_records(self, index=True, convert_datetime64=None, column_dtypes=None, index_dtypes=None):
        """
        ...
        args = locals()
        kdf = self
        return validate_arguments_and_invoke_function(kdf._to_internal_pandas(), self.to_records, pd.DataFrame.to_records, args)
    ...
    def dropna(self, axis=0, how='any', thresh=None, subset=None, inplace=False):
        ...
        if axis == 0 or axis == 'index':
            if subset is not None:
                ...
                invalids = [idx for idx in idxes if idx not in self._internal.column_index]
                if len(invalids) > 0:
                    raise KeyError(invalids)
                else:
                    idxes = self._internal.column_index
                cnt = reduce(lambda x, y: x + y, [F.when(self[idx].notna()._scol, 1).otherwise(0) for idx in idxes], F.lit(0))
    ...
    def fillna(self, value=None, method=None, axis=None, inplace=False, limit=None):
        ...
        if value is not None:
            ...
            internal = self._internal.copy(sdf=sdf, column_scols=[scol_for(sdf, col) for col in self._internal.data_columns])
        else:
            if method is None:
                raise ValueError("Must specify a fillna 'value' or 'method' parameter.")
            applied = []
            for idx in self._internal.column_index:
                applied.append(self[idx].fillna(value=value, method=method, axis=axis, inplace=False,
                  limit=limit))
    ...
    def replace(self, to_replace=None, value=None, subset=None, inplace=False, limit=None, regex=False, method='pad'):
        """
            ...
            if limit is not None:
                ...
                if value is not None:
                    ...
                    if isinstance(to_replace, dict):
                        if value is None and not any(isinstance(i, dict) for i in to_replace.values()):
                    ...
                    for name, replacement in to_replace.items():
                        if isinstance(name, str):
                            ...
                            if isinstance(replacement, dict):
                                ...
                                sdf = sdf.withColumn(df_column, F.when(scol_for(sdf, df_column) == replacement, value).otherwise(scol_for(sdf, df_column)))
    ...
    def clip(self, lower: Union[(float, int)]=None, upper: Union[(float, int)]=None) -> 'DataFrame':
        ...
        if is_list_like(lower) or is_list_like(upper):
            ...
            numeric_types = (
             ...
             ShortType)
            numeric_columns = [(c, self._internal.scol_for(c)) for c in self.columns if isinstance(self._internal.spark_type_for(c), numeric_types)]
            if lower is not None:
                numeric_columns = [(c, F.when(scol < lower, lower).otherwise(scol).alias(c)) for c, scol in numeric_columns]
            if upper is not None:
                numeric_columns = [(c, F.when(scol > upper, upper).otherwise(scol).alias(c)) for c, scol in numeric_columns]
        nonnumeric_columns = [self._internal.scol_for(c) for c in self.columns if not isinstance(self._internal.spark_type_for(c), numeric_types)]
            ...
            if not isinstance(values, (str, tuple)):
                if not isinstance(values, list):
                    ...
                    if not isinstance(aggfunc, str):
                        if not isinstance(aggfunc, dict) or not all(isinstance(key, (str, tuple)) and isinstance(value, str) for key, value in aggfunc.items()):
                            raise ValueError('aggfunc must be a dict mapping from column name (string or tuple) to aggregate functions (string).')
                        else:
                            if isinstance(aggfunc, dict):
                                if index is None:
                                    raise NotImplementedError("pivot_table doesn't support aggfunc as dict and without index.")
                            else:
                        ...
                        if isinstance(aggfunc, str):
                            if isinstance(values, list):
                                agg_cols = [F.expr('{1}(`{0}`) as `{0}`'.format(self._internal.column_name_for(value), aggfunc)) for value in values]
                            else:
                                agg_cols = [
                                 F.expr('{1}(`{0}`) as `{0}`'.format(self._internal.column_name_for(values), aggfunc))]
                        elif isinstance(aggfunc, dict):
                            aggfunc = {key if isinstance(key, tuple) else (key,):value for key, value in aggfunc.items()}
                            agg_cols = [F.expr('{1}(`{0}`) as `{0}`'.format(self._internal.column_name_for(key), value)) for key, value in aggfunc.items()]
                    ...
                    if index is None:
                        sdf = (self._sdf.groupBy().pivot(pivot_col=(self._internal.column_name_for(columns))).agg)(*agg_cols)
                    else:
                        if isinstance(index, list):
                            index = [idx if isinstance(idx, tuple) else (idx,) for idx in index]
                            sdf = (self._sdf.groupBy([self._internal.scol_for(idx) for idx in index]).pivot(pivot_col=(self._internal.column_name_for(columns))).agg)(*agg_cols)
                        else:
            ...
            if index is not None:
                if isinstance(values, list):
                    ...
                    if len(values) > 1:
                        ...
                        column_name_to_index = dict(zip(self._internal.data_columns, self._internal.column_index))
                        column_index = [tuple(list(column_name_to_index[name.split('_')[1]]) + [name.split('_')[0]]) for name in data_columns]
                        ...
                        internal = _InternalFrame(sdf=sdf, index_map=index_map,
    ...
    def pivot(self, index=None, columns=None, values=None):
        ...
        if should_use_existing_index:
            ...
            internal = df._internal.copy(index_map=[(index_column, None) for index_column in index_columns])
            return DataFrame(internal)
    ...
    def columns(self, columns):
        if isinstance(columns, pd.MultiIndex):
            ...
            sdf = self._sdf.select(self._internal.index_scols + [self._internal.scol_for(idx).alias(name) for idx, name in zip(self._internal.column_index, data_columns)])
            column_scols = [scol_for(sdf, col) for col in data_columns]
            self._internal = self._internal.copy(sdf=sdf, column_index=column_index,
    ...
    def dtypes(self):
        ...
        dtype: object
        """
        return pd.Series([self[idx].dtype for idx in self._internal.column_index], index=(pd.Index([idx if len(idx) > 1 else idx[0] for idx in self._internal.column_index])))
    ...
    def select_dtypes(self, include=None, exclude=None):
        ...
        * To select datetimes, use ``np.datetime64``, ``'datetime'`` or
        ...
        if set(include).intersection(set(exclude)):
            raise ValueError('include and exclude overlap on {inc_ex}'.format(inc_ex=(set(include).intersection(set(exclude)))))
        include_spark_type = []
        for inc in include:
            try:
        ...
        for idx in self._internal.column_index:
            if len(include) > 0:
                should_include = infer_dtype_from_object(self[idx].dtype.name) in include_numpy_type or self._internal.spark_type_for(idx) in include_spark_type
            else:
                should_include = not (infer_dtype_from_object(self[idx].dtype.name) in exclude_numpy_type or self._internal.spark_type_for(idx) in exclude_spark_type)
            if should_include:
                columns.append(self._internal.column_name_for(idx))
                column_index.append(idx)

        sdf = self._sdf.select(self._internal.index_scols + [self._internal.scol_for(col) for col in columns])
    ...
    def count(self, axis=None):
        ...
        return self._reduce_for_stat_function((_Frame._count_expr),
          name='count', axis=axis, numeric_only=False)

    def drop(self, labels=None, axis=1, columns: Union[(str, Tuple[(str, ...)], List[str], List[Tuple[(str, ...)]])]=None):
        """
        ...
        if labels is not None:
            ...
            if columns is not None:
                ...
                drop_column_index = set(idx for idx in self._internal.column_index for col in columns)
                if len(drop_column_index) == 0:
                    raise KeyError(columns)
                cols, idxes = zip(*((column, idx) for column, idx in zip(self._internal.data_columns, self._internal.column_index) if idx not in drop_column_index))
                sdf = self._sdf.select(self._internal.index_scols + [self._internal.scol_for(idx) for idx in idxes])
                internal = self._internal.copy(sdf=sdf,
    ...
    def _sort(self, by: List[Column], ascending: Union[(bool, List[bool])], inplace: bool, na_position: str):
        if isinstance(ascending, bool):
            ascending = [
             ascending] * len(by)
        else:
            if len(ascending) != len(by):
                raise ValueError('Length of ascending ({}) != length of by ({})'.format(len(ascending), len(by)))
            if na_position not in ('first', 'last'):
                raise ValueError("invalid na_position: '{}'".format(na_position))
        mapper = {(True, 'first'): lambda x: Column(getattr(x._jc, 'asc_nulls_first')()), 
    ...
    def sort_values(self, by: Union[(str, List[str])], ascending: Union[(bool, List[bool])]=True, inplace: bool=False, na_position: str='last') -> Optional['DataFrame']:
        """
    ...
    def sort_index(self, axis: int=0, level: Optional[Union[(int, List[int])]]=None, ascending: bool=True, inplace: bool=False, kind: str=None, na_position: str='last') -> Optional['DataFrame']:
        """
        ...
        if axis != 0:
            ...
            if level is None or is_list_like(level) and len(level) == 0:
                ...
                if is_list_like(level):
                    by = [self._internal.index_scols[l] for l in level]
                else:
                    by = [
    ...
    def nlargest(self, n: int, columns: 'Any') -> 'DataFrame':
        ...
        """
        kdf = self.sort_values(by=columns, ascending=False)
    ...
    def nsmallest(self, n: int, columns: 'Any') -> 'DataFrame':
        ...
        2  3.0   8
        """
        kdf = self.sort_values(by=columns, ascending=True)
        assert kdf is not None
        return kdf.head(n=n)
    ...
    def isin(self, values):
        ...
        if isinstance(values, (pd.DataFrame, pd.Series)):
            ...
            if not set(values.keys()).issubset(self.columns):
                raise AttributeError("'DataFrame' object has no attribute %s" % set(values.keys()).difference(self.columns))
        else:
            _select_columns = self._internal.index_columns.copy()
            if isinstance(values, dict):
                for col in self.columns:
                    if col in values:
                        _select_columns.append(self._internal.scol_for(col).isin(values[col]).alias(col))
                    else:
                        _select_columns.append(F.lit(False).alias(col))

            else:
                if is_list_like(values):
                    _select_columns += [self._internal.scol_for(col).isin(list(values)).alias(col) for col in self.columns]
                else:
    ...
    def merge(self, right: 'DataFrame', how: str='inner', on: Union[(str, List[str], Tuple[(str, ...)], List[Tuple[(str, ...)]])]=None, left_on: Union[(str, List[str], Tuple[(str, ...)], List[Tuple[(str, ...)]])]=None, right_on: Union[(str, List[str], Tuple[(str, ...)], List[Tuple[(str, ...)]])]=None, left_index: bool=False, right_index: bool=False, suffixes: Tuple[(str, str)]=('_x', '_y')) -> 'DataFrame':
        """
        ...
        As described in #263, joining string columns currently returns None for missing values
            instead of NaN.
        """
        _to_list = --- This code section failed: ---

 L.5583         0  LOAD_FAST                'os'
                2  LOAD_CONST               None
                4  COMPARE_OP               is
                6  POP_JUMP_IF_FALSE    12  'to 12'
                8  LOAD_FAST                'os'
               10  RETURN_END_IF_LAMBDA
             12_0  COME_FROM             6  '6'

 L.5584        12  LOAD_GLOBAL              isinstance
               14  LOAD_FAST                'os'
 ...
 L.5585        28  LOAD_GLOBAL              isinstance
               30  LOAD_FAST                'os'
               32  LOAD_GLOBAL              str
 ...
 L.5586        46  LOAD_LISTCOMP            '<code_object <listcomp>>'
               48  LOAD_STR                 'DataFrame.merge.<locals>.<lambda>.<locals>.<listcomp>'
               50  MAKE_FUNCTION_0          'Neither defaults, keyword-only args, annotations, nor closures'

 L.5587        52  LOAD_FAST                'os'
               54  GET_ITER         
               56  CALL_FUNCTION_1       1  '1 positional argument'
               58  RETURN_VALUE_LAMBDA
               -1  LAMBDA_MARKER    

Parse error at or near `RETURN_END_IF_LAMBDA' instruction at offset 26
        ...
        if on:
            ...
            if not left_keys:
                if not right_keys:
                    ...
                    if len(common) == 0:
                        raise ValueError('No common columns to perform merge on. Merge options: left_on=None, right_on=None, left_index=False, right_index=False')
                    left_keys = _to_list(common)
                    right_keys = _to_list(common)
        if len(left_keys) != len(right_keys):
            raise ValueError('len(left_keys) must equal len(right_keys)')
        if how == 'full':
            warnings.warn("Warning: While Koalas will accept 'full', you should use 'outer' instead to be compatible with the pandas merge API", UserWarning)
        if how == 'outer':
            how = 'full'
        if how not in ('inner', 'left', 'right', 'full'):
            raise ValueError("The 'how' parameter has to be amongst the following values: ", "['inner', 'left', 'right', 'outer']")
        ...
        left_key_columns = [left_scol_for(idx) for idx in left_keys]
        right_key_columns = [right_scol_for(idx) for idx in right_keys]
        join_condition = reduce(lambda x, y: x & y, [lkey == rkey for lkey, rkey in zip(left_key_columns, right_key_columns)])
        joined_table = left_table.join(right_table, join_condition, how=how)
        left_suffix = suffixes[0]
        right_suffix = suffixes[1]
        duplicate_columns = set(self._internal.column_index) & set(right._internal.column_index)
        exprs = []
        data_columns = []
        column_index = []
        for idx in self._internal.column_index:
            ...
            if idx in duplicate_columns:
                if idx in left_keys and idx in right_keys:
        ...
        if left_index:
            if right_index:
                if how in ('inner', 'left'):
                    ...
                    if how == 'right':
                        ...
                        index_map = []
                        for (col, name), left_scol, right_scol in zip(self._internal.index_map, left_index_scols, right_index_scols):
            ...
            internal = _InternalFrame(sdf=selected_columns, index_map=(index_map if index_map else None),
              column_index=column_index,
              column_scols=[scol_for(selected_columns, col) for col in data_columns])
            return DataFrame(internal)

    def join(self, right: 'DataFrame', on: Optional[Union[(str, List[str], Tuple[(str, ...)], List[Tuple[(str, ...)]])]]=None, how: str='left', lsuffix: str='', rsuffix: str='') -> 'DataFrame':
        """
        ...
        if len(common) > 0:
            if not lsuffix:
                if not rsuffix:
                    raise ValueError('columns overlap but no suffix specified: {rename}'.format(rename=common))
        else:
            if on:
                self = self.set_index(on)
                join_kdf = self.merge(right, left_index=True, right_index=True, how=how, suffixes=(
                 lsuffix, rsuffix)).reset_index()
            else:
                join_kdf = self.merge(right, left_index=True, right_index=True, how=how, suffixes=(
                 lsuffix, rsuffix))
        return join_kdf
    ...
    def append(self, other: 'DataFrame', ignore_index: bool=False, verify_integrity: bool=False, sort: bool=False) -> 'DataFrame':
        ...
        if not ignore_index:
            ...
            if verify_integrity:
                if len(index_scols) > 0:
                    if self._sdf.select(index_scols).intersect(other._sdf.select(other._internal.index_scols)).count() > 0:
    ...
    def update(self, other: 'DataFrame', join: str='left', overwrite: bool=True):
        ...
        update_columns = list(set(self._internal.column_index).intersection(set(other._internal.column_index)))
        ...
        for column_index in update_columns:
            ...
            if overwrite:
                update_sdf = update_sdf.withColumn(column_name, F.when(new_col.isNull(), old_col).otherwise(new_col))
            else:
                update_sdf = update_sdf.withColumn(column_name, F.when(old_col.isNull(), new_col).otherwise(old_col))

        sdf = update_sdf.select([scol_for(update_sdf, col) for col in self._internal.columns])
        internal = self._internal.copy(sdf=sdf, column_scols=[scol_for(sdf, col) for col in self._internal.data_columns])
        self._internal = internal
    ...
    def sample(self, n: Optional[int]=None, frac: Optional[float]=None, replace: bool=False, random_state: Optional[int]=None) -> 'DataFrame':
        ...
        """
        if n is not None:
            raise NotImplementedError('Function sample currently does not support specifying exact number of items to return. Use frac instead.')
        if frac is None:
    ...
    def astype(self, dtype) -> 'DataFrame':
        ...
        sdf = self._sdf.select(self._internal.index_scols + list(map(lambda ser: ser._scol, results)))
        return DataFrame(self._internal.copy(sdf=sdf, column_scols=[scol_for(sdf, col) for col in self._internal.data_columns]))
    ...
    def add_prefix(self, prefix):
        ...
        data_columns = [prefix + self._internal.column_name_for(idx) for idx in self._internal.column_index]
        sdf = self._sdf.select(self._internal.index_scols + [self._internal.scol_for(idx).alias(name) for idx, name in zip(self._internal.column_index, data_columns)])
        column_index = [tuple([prefix + i for i in idx]) for idx in self._internal.column_index]
        internal = self._internal.copy(sdf=sdf, column_index=column_index,
          column_scols=[scol_for(sdf, col) for col in data_columns])
        return DataFrame(internal)
    ...
    def add_suffix(self, suffix):
        ...
        data_columns = [self._internal.column_name_for(idx) + suffix for idx in self._internal.column_index]
        sdf = self._sdf.select(self._internal.index_scols + [self._internal.scol_for(idx).alias(name) for idx, name in zip(self._internal.column_index, data_columns)])
    ...
    def describe(self, percentiles: Optional[List[float]]=None) -> 'DataFrame':
        ...
        internal = _InternalFrame(sdf=sdf, index_map=[
         ('summary', None)],
          column_scols=[scol_for(sdf, col) for col in data_columns])
        ...
        if subset is None:
            ...
            if isinstance(subset, str):
                ...
                if isinstance(subset, tuple):
                    ...
                    subset = [sub if isinstance(sub, tuple) else (sub,) for sub in subset]
            sdf = self._sdf.drop_duplicates(subset=[self._internal.column_name_for(idx) for idx in subset])
            internal = self._internal.copy(sdf=sdf)
            if inplace:
                self._internal = internal
            else:
    ...
    def reindex(self, labels: Optional[Any]=None, index: Optional[Any]=None, columns: Optional[Any]=None, axis: Optional[Union[(int, str)]]=None, copy: Optional[bool]=True, fill_value: Optional[Any]=None) -> 'DataFrame':
        """
    ...
    def _reindex_columns(self, columns):
        ...
        for col in label_columns:
            if len(col) != level:
                raise ValueError("shape (1,{}) doesn't match the shape (1,{})".format(len(col), level))
        ...
        sdf = self._sdf.select(self._internal.index_scols + scols)
        return self._internal.copy(sdf=sdf, column_index=idx,
          column_scols=[scol_for(sdf, col) for col in columns])
        ...
        if var_name is None:
            if self._internal.column_index_names is not None:
                ...
                if self._internal.column_index_level == 1:
                    ...
                    var_name = ['variable_{}'.format(i) for i in range(self._internal.column_index_level)]
        else:
            if isinstance(var_name, str):
                var_name = [
                 var_name]
            pairs = F.explode((F.array)(*[(F.struct)(*[F.lit(c).alias(name) for c, name in zip(idx, var_name)] + [self._internal.scol_for(idx).alias(value_name)]) for idx in column_index if idx in value_vars]))
            columns = [self._internal.scol_for(idx).alias(name_like_string(idx)) for idx in id_vars] + [F.col('pairs.%s' % name) for name in var_name[:self._internal.column_index_level]] + [
    ...
    def all(self, axis: Union[(int, str)]=0) -> bool:
        ...
        for idx, applied_col in zip(column_index, applied):
            cols.append(F.struct([F.lit(col).alias(SPARK_INDEX_NAME_FORMAT(i)) for i, col in enumerate(idx)] + [
        ...
        index_column_name = lambda i: None if self._internal.column_index_names is None else (
         self._internal.column_index_names[i],)
        internal = self._internal.copy(sdf=sdf,
          index_map=[(SPARK_INDEX_NAME_FORMAT(i), index_column_name(i)) for i in range(self._internal.column_index_level)],
          column_index=None,
          column_scols=[
    ...
    def filter(self, items=None, like=None, regex=None, axis=None):
        ...
        elif axis not in ('index', 0, 'columns', 1, None):
            raise ValueError('No axis named %s for object type %s.' % (axis, type(axis)))
        else:
            index_scols = self._internal.index_scols
            sdf = self._sdf
                ...
                if like is not None:
                    ...
                    if regex is not None:
                        ...
                        if axis in ('columns', 1, None):
                            ...
                            output_idx = [idx for idx in column_index if any(matcher.search(i) is not None for i in idx)]
                            return self[output_idx]
                    else:
                        raise TypeError('Must pass either `items`, `like`, or `regex`')

    def rename(self, mapper=None, index=None, columns=None, axis='index', inplace=False, level=None, errors='ignore'):
        """
        Alter axes labels.
        ...
        if mapper:
            if axis == 'index' or axis == 0:
                ...
                if axis == 'columns' or axis == 1:
                    ...
                    raise ValueError('argument axis should be either the axis name (‘index’, ‘columns’) or number (0, 1)')
        else:
            if index:
        ...
        if columns_mapper_fn:
            ...
            new_data_scols = [scol_for(internal.sdf, old_col_name).alias(new_col_name) for old_col_name, new_col_name in zip(internal.data_columns, new_data_columns)]
            sdf = (internal.sdf.select)(*internal.index_scols + new_data_scols)
            internal = internal.copy(sdf=sdf, column_index=new_column_index, column_scols=[scol_for(sdf, col) for col in new_data_columns])
        if inplace:
            self._internal = internal
            return self
        else:
    ...
    def quantile(self, q=0.5, axis=0, numeric_only=True, accuracy=10000):
        ...
        for column in self._internal.data_columns:
            sdf = self._sdf
            percentile_cols.append(F.expr('approx_percentile(`%s`, array(%s), %s)' % (column, args, accuracy)).alias(column))
        ...
        internal = self._internal.copy(sdf=sdf,
          ...
          index_map=[
         (
          internal_index_column, None)],
          column_index=(self._internal.column_index),
          column_index_names=None)
        if not result_as_series:
            return DataFrame(internal)
        else:
            return DataFrame(internal).T[key]
        ...
        if all(len(idx) > 0 and idx[0] == '' for _, idx in indexes):
            ...
            if all(len(idx) == 0 for _, idx in indexes):
                try:
                    ...
                    assert len(idxes) == 1
                    index = list(idxes)[0]
                    kdf_or_ser = Series(self._internal.copy(scol=(self._internal.scol_for(index)), column_index=[
                     index]),
                      anchor=self)
                except AnalysisException:
                    raise KeyError(key)

            kdf_or_ser = DataFrame(self._internal.copy(column_index=[idx for _, idx in indexes], column_scols=[self._internal.scol_for(idx) for idx, _ in indexes], column_index_names=column_index_names))
        if recursive:
        ...
        if max_display_count is None:
            ...
            if pdf_length > max_display_count:
                ...
                if match is not None:
                    ...
                    footer = '\n\n[Showing only the first {nrows} rows x {ncols} columns]'.format(nrows=nrows,
                      ncols=ncols)
    ...
    def __setitem__(self, key, value):
        from databricks.koalas.series import Series
        if isinstance(value, Series) and value._kdf is not self or isinstance(value, DataFrame) and value is not self:
            level = self._internal.column_index_level
            ...
            if value_level > level:
                value.columns = pd.MultiIndex.from_tuples([idx[level:] for idx in value._internal.column_index])
            elif value_level < level:
                value.columns = pd.MultiIndex.from_tuples([tuple(list(idx) + [''] * (level - value_level)) for idx in value._internal.column_index])
            else:
                if isinstance(key, str):
                    key = [
                     (
            ...
            def assign_columns(kdf, this_column_index, that_column_index):
                ...
                for k, this_idx, that_idx in zip_longest(key, this_column_index, that_column_index):
                    yield (
                     kdf[that_idx], tuple(['that', *k]))
                    if this_idx is not None and this_idx[1:] != k:
                        yield (
                         kdf[this_idx], this_idx)
...
def _reduce_spark_multi(sdf, aggs):
    ...
    """
    if not isinstance(sdf, spark.DataFrame):
        raise AssertionError
    else:
        sdf0 = (sdf.agg)(*aggs)
        l = sdf0.head(2)
