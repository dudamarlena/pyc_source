...
# [GCC 8.4.0]
# Embedded file name: ./build/lib/databricks/koalas/plot.py
# Compiled at: 2019-10-28 13:14:51
# Size of source mod 2**32: 65480 bytes
...
if LooseVersion(pd.__version__) < LooseVersion('0.25'):
    from pandas.plotting._core import _all_kinds, BarPlot, BoxPlot, HistPlot, MPLPlot, PiePlot, AreaPlot, LinePlot, BarhPlot, ScatterPlot, KdePlot
else:
    from pandas.plotting._core import PlotAccessor
    from pandas.plotting._matplotlib import BarPlot, BoxPlot, HistPlot, PiePlot, AreaPlot, LinePlot, BarhPlot, ScatterPlot, KdePlot
...
class TopNPlot:
    ...
    def set_result_text(self, ax):
        ...
        if self.partial:
            ax.text(1, 1, ('showing top {} elements only'.format(max_rows)), size=6,
              ha='right',
              va='bottom',
...
class SampledPlot:
    ...
    def set_result_text(self, ax):
        ...
        if self.fraction < 1:
            ax.text(1,
              1, ('showing the sampled result by fraction %s' % self.fraction), size=6,
              ha='right',
              va='bottom',
              transform=(ax.transAxes))
...
class KoalasBoxPlot(BoxPlot):
    ...
    def boxplot(self, ax, bxpstats, notch=None, sym=None, vert=None, whis=None, positions=None, widths=None, patch_artist=None, bootstrap=None, usermedians=None, conf_intervals=None, meanline=None, showmeans=None, showcaps=None, showbox=None, showfliers=None, boxprops=None, labels=None, flierprops=None, medianprops=None, meanprops=None, capprops=None, whiskerprops=None, manage_xticks=True, autorange=False, zorder=None, precision=None):
        ...
        def _update_dict(dictionary, rc_name, properties):
            ...
            for prop_dict in properties:
                dictionary.setdefault(prop_dict, matplotlib.rcParams[rc_str.format(rc_name, prop_dict)])

            return dictionary

        flier_props = [
         'color', 'marker', 'markerfacecolor', 'markeredgecolor',
    ...
    @staticmethod
    def rc_defaults(notch=None, vert=None, whis=None, patch_artist=None, bootstrap=None, meanline=None, showmeans=None, showcaps=None, showbox=None, showfliers=None, **kwargs):
        if whis is None:
        ...
        return dict(whis=whis, bootstrap=bootstrap, notch=notch, vert=vert, patch_artist=patch_artist,
          meanline=meanline,
          showmeans=showmeans,
          showcaps=showcaps,
          showbox=showbox,
    ...
    def _compute_stats(data, colname, whis, precision):
        pdf = (data._kdf._sdf.agg)(*[F.expr('approx_percentile({}, {}, {})'.format(colname, q, 1.0 / precision)).alias('{}_{}%'.format(colname, int(q * 100))) for q in (0.25,
                                                                                                                                                                         0.5,
                                                                                                                                                                         0.75)], *(
        ...
        return (
         stats, (lfence.values[0], ufence.values[0]))
    ...
    def _calc_whiskers(colname, outliers):
        minmax = outliers.filter('not __{}_outlier'.format(colname)).agg(F.min(colname).alias('min'), F.max(colname).alias('max')).toPandas()
        return minmax.iloc[0][['min', 'max']].values

    @staticmethod
    def _get_fliers(colname, outliers):
        fliers_df = outliers.filter('__{}_outlier'.format(colname))
        fliers = fliers_df.select(F.abs(F.col('`{}`'.format(colname))).alias(colname)).orderBy(F.desc('`{}`'.format(colname))).limit(1001).toPandas()[colname].values
        return fliers
...
class KoalasHistPlot(HistPlot):
    ...
    def _compute_plot_data(self):
        ...
        if len(numeric_data.columns) == 0:
            raise TypeError('Empty {0!r}: no numeric data to plot'.format(numeric_data.__class__.__name__))
    ...
    def _make_plot(self):
        ...
        for i, idx in enumerate(self.data._internal.column_index):
            ...
            if style is not None:
                kwds['style'] = style
            y = KoalasHistPlot._compute_hist(y, self.bins)
    ...
    def _get_bins(sdf, bins):
        ...
        if boundaries[0] == boundaries[1]:
            boundaries = (
             boundaries[0] - 0.5, boundaries[1] + 0.5)
    ...
    def _compute_hist(sdf, bins):
        ...
        bucketizer = Bucketizer(splits=bins, inputCol=colname,
          outputCol=bucket_name,
          handleInvalid='skip')
        result = bucketizer.transform(sdf).select(bucket_name).groupby(bucket_name).agg(F.count('*').alias('count')).toPandas().sort_values(by=bucket_name)
...
def plot_series(data, kind='line', ax=None, figsize=None, use_index=True, title=None, grid=None, legend=False, style=None, logx=False, logy=False, loglog=False, xticks=None, yticks=None, xlim=None, ylim=None, rot=None, fontsize=None, colormap=None, table=False, yerr=None, xerr=None, label=None, secondary_y=False, **kwds):
    """
    ...
    **kwds : keywords
    ...
    return _plot(data, kind=kind, ax=ax, figsize=figsize, 
     ...
     fontsize=fontsize, colormap=colormap, table=table, yerr=yerr, 
     xerr=xerr, label=label, 
     secondary_y=secondary_y, **kwds)


def plot_frame(data, x=None, y=None, kind='line', ax=None, subplots=None, sharex=None, sharey=False, layout=None, figsize=None, use_index=True, title=None, grid=None, legend=True, style=None, logx=False, logy=False, loglog=False, xticks=None, yticks=None, xlim=None, ylim=None, rot=None, fontsize=None, colormap=None, table=False, yerr=None, xerr=None, secondary_y=False, sort_columns=False, **kwds):
    """
    ...
    **kwds : keywords
    ...
    - If `kind` = 'bar' or 'barh', you can specify relative alignments
      ...
      From 0 (left/bottom-end) to 1 (right/top-end). Default is 0.5 (center)
    """
    return _plot(data, kind=kind, x=x, y=y, ax=ax, figsize=figsize, 
     use_index=use_index, title=title, grid=grid, 
     legend=legend, subplots=subplots, style=style, 
     logx=logx, logy=logy, loglog=loglog, xticks=xticks, 
     yticks=yticks, xlim=xlim, ylim=ylim, rot=rot, 
     fontsize=fontsize, colormap=colormap, table=table, yerr=yerr, 
     xerr=xerr, sharex=sharex, sharey=sharey, secondary_y=secondary_y, 
     layout=layout, sort_columns=sort_columns, **kwds)
...
class KoalasSeriesPlotMethods(PandasObject):
    ...
    def __call__(self, kind='line', ax=None, figsize=None, use_index=True, title=None, grid=None, legend=False, style=None, logx=False, logy=False, loglog=False, xticks=None, yticks=None, xlim=None, ylim=None, rot=None, fontsize=None, colormap=None, table=False, yerr=None, xerr=None, label=None, secondary_y=False, **kwds):
        return plot_series(self.data, kind=kind, ax=ax, figsize=figsize, use_index=use_index, 
         ...
         table=table, yerr=yerr, xerr=xerr, 
         label=label, secondary_y=secondary_y, **kwds)
...
class KoalasFramePlotMethods(PandasObject):
    ...
    def __call__(self, x=None, y=None, kind='line', ax=None, subplots=None, sharex=None, sharey=False, layout=None, figsize=None, use_index=True, title=None, grid=None, legend=True, style=None, logx=False, logy=False, loglog=False, xticks=None, yticks=None, xlim=None, ylim=None, rot=None, fontsize=None, colormap=None, table=False, yerr=None, xerr=None, secondary_y=False, sort_columns=False, **kwds):
        return plot_frame(self.data, x=x, y=y, kind=kind, ax=ax, subplots=subplots, 
         sharex=sharex, sharey=sharey, layout=layout, figsize=figsize, 
         use_index=use_index, title=title, grid=grid, legend=legend, 
         style=style, logx=logx, logy=logy, loglog=loglog, 
         xticks=xticks, yticks=yticks, xlim=xlim, ylim=ylim, 
         rot=rot, fontsize=fontsize, colormap=colormap, table=table, 
         yerr=yerr, xerr=xerr, secondary_y=secondary_y, sort_columns=sort_columns, **kwds)
    ...
    def pie(self, y=None, **kwds):
        ...
        **kwds
        ...
        if isinstance(self.data, DataFrame):
            if y is None:
                if not kwds.get('subplots', False):
