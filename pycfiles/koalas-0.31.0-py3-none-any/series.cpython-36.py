...
# [GCC 8.4.0]
# Embedded file name: ./build/lib/databricks/koalas/series.py
# Compiled at: 2019-11-07 14:25:58
# Size of source mod 2**32: 131995 bytes
"""
...
from databricks.koalas.utils import validate_arguments_and_invoke_function, scol_for, combine_frames, name_like_string
...
class Series(_Frame, IndexOpsMixin, Generic[T]):
    ...
    add.__doc__ = _flex_doc_SERIES.format(desc='Addition',
      op_name='+',
      equiv='series + other',
      reverse='radd',
    ...
    radd.__doc__ = _flex_doc_SERIES.format(desc='Reverse Addition',
      op_name='+',
      equiv='other + series',
      reverse='add',
      series_examples=_add_example_SERIES)
    ...
    div.__doc__ = _flex_doc_SERIES.format(desc='Floating division',
      op_name='/',
      equiv='series / other',
      reverse='rdiv',
    ...
    rtruediv.__doc__ = _flex_doc_SERIES.format(desc='Reverse Floating division',
      op_name='/',
      equiv='other / series',
      reverse='truediv',
      series_examples=_div_example_SERIES)
    ...
    mul.__doc__ = _flex_doc_SERIES.format(desc='Multiplication',
      op_name='*',
      equiv='series * other',
      reverse='rmul',
    ...
    sub.__doc__ = _flex_doc_SERIES.format(desc='Subtraction',
      op_name='-',
      equiv='series - other',
      reverse='rsub',
    ...
    mod.__doc__ = _flex_doc_SERIES.format(desc='Modulo',
      op_name='%',
      equiv='series % other',
      reverse='rmod',
    ...
    rmod.__doc__ = _flex_doc_SERIES.format(desc='Reverse Modulo',
      op_name='%',
      equiv='other % series',
      reverse='mod',
      series_examples=_mod_example_SERIES)
    ...
    pow.__doc__ = _flex_doc_SERIES.format(desc='Exponential power of series',
      op_name='**',
      equiv='series ** other',
      reverse='rpow',
    ...
    rfloordiv.__doc__ = _flex_doc_SERIES.format(desc='Reverse Integer division',
      op_name='//',
      equiv='other // series',
      reverse='floordiv',
      series_examples=_floordiv_example_SERIES)
    ...
    def rename(self, index: Union[(str, Tuple[(str, ...)])]=None, **kwargs):
        ...
        internal = self._internal.copy(scol=scol,
          column_index=[
    ...
    def is_unique(self):
        ...
        """
        scol = self._scol
        return self._internal._sdf.select((F.count(scol) == F.countDistinct(scol)) & (F.count(F.when(scol.isNull(), 1).otherwise(None)) <= 1)).collect()[0][0]
    ...
    def to_frame(self, name: Union[(str, Tuple[(str, ...)])]=None) -> spark.DataFrame:
        ...
        internal = _InternalFrame(sdf=sdf, index_map=(renamed._internal.index_map),
          column_index=column_index,
          column_scols=[
         scol_for(sdf, sdf.columns[(-1)])],
            ...
            if not (axis == 0 or axis == 'index'):
                ...
                if value is None:
                    ...
                    if method is not None:
                        if method not in ('ffill', 'pad', 'backfill', 'bfill'):
                            raise ValueError("Expecting 'pad', 'ffill', 'backfill' or 'bfill'.")
                elif self.isnull().sum() == 0:
                    if inplace:
                ...
                window = (Window.partitionBy)(*part_cols).orderBy(self._internal.index_scols).rowsBetween(begin, end)
    ...
    def drop(self, labels=None, index: Union[(str, Tuple[(str, ...)], List[str], List[Tuple[(str, ...)]])]=None, level=None):
        """
            ...
            if not isinstance(index, (str, tuple, list)):
                ...
                if isinstance(index, str):
                    ...
                    if isinstance(index, tuple):
                        ...
                        if not (all(isinstance(idxes, str) for idxes in index) or all(isinstance(idxes, tuple) for idxes in index)):
                            raise ValueError('If the given index is a list, it should only contains names as strings, or a list of tuples that contain index names as strings')
                index = [idxes if isinstance(idxes, tuple) else (idxes,) for idxes in index]
            drop_index_scols = []
            for idxes in index:
                try:
                    index_scols = [self._internal.index_scols[lvl] == idx for lvl, idx in enumerate(idxes, level)]
                except IndexError:
                    if level_param is None:
                        raise KeyError('Key length ({}) exceeds index depth ({})'.format(len(self._internal.index_scols), len(idxes)))
                    else:
                        return self
    ...
    def unique(self):
        ...
        internal = _InternalFrame(sdf=sdf, column_index=[
         self._internal.column_index[0]],
          column_scols=[
         scol_for(sdf, self._internal.data_columns[0])],
    ...
    def _nunique(self, dropna=True, approx=False, rsd=0.05):
        ...
        if dropna:
            ...
            return (count_fn(self._scol) + F.when(F.count(F.when(self._scol.isNull(), 1).otherwise(None)) >= 1, 1).otherwise(0)).alias(colname)
    ...
    def sort_values(self, ascending: bool=True, inplace: bool=False, na_position: str='last') -> Union[('Series', None)]:
        ...
        Name: 0, dtype: object
        """
        kseries = _col(self.to_dataframe().sort_values(by=(self.name), ascending=ascending, na_position=na_position))
        if inplace:
            self._internal = kseries._internal
            self._kdf = kseries._kdf
            return
    ...
    def sort_index(self, axis: int=0, level: Optional[Union[(int, List[int])]]=None, ascending: bool=True, inplace: bool=False, kind: str=None, na_position: str='last') -> Optional['Series']:
        """
        ...
        Name: 0, dtype: int64
        """
        kseries = _col(self.to_dataframe().sort_index(axis=axis, level=level, ascending=ascending, kind=kind,
    ...
    def add_prefix(self, prefix):
        ...
        sdf = internal.sdf
        sdf = sdf.select([F.concat(F.lit(prefix), scol_for(sdf, index_column)).alias(index_column) for index_column in internal.index_columns] + internal.column_scols)
    ...
    def add_suffix(self, suffix):
        ...
        internal = kdf._internal
        sdf = internal.sdf
        sdf = sdf.select([F.concat(scol_for(sdf, index_column), F.lit(suffix)).alias(index_column) for index_column in internal.index_columns] + internal.column_scols)
        kdf._internal = internal.copy(sdf=sdf)
        return _col(kdf)
    ...
    def append(self, to_append: 'Series', ignore_index: bool=False, verify_integrity: bool=False) -> 'Series':
        ...
        Name: 0, dtype: int64
        """
        return _col(self.to_dataframe().append(to_append.to_dataframe(), ignore_index, verify_integrity))

    def sample(self, n: Optional[int]=None, frac: Optional[float]=None, replace: bool=False, random_state: Optional[int]=None) -> 'Series':
        return _col(self.to_dataframe().sample(n=n,
          frac=frac,
          replace=replace,
    ...
    def transform(self, func, *args, **kwargs):
        ...
        **kwargs
        ...
        if isinstance(func, list):
            ...
            sdf = self._internal._sdf.select(self._internal.index_scols + [c._scol for c in applied])
            internal = self.to_dataframe()._internal.copy(sdf=sdf,
    ...
    def quantile(self, q=0.5, accuracy=10000):
        ...
        for v in q if isinstance(q, list) else [q]:
            if not isinstance(v, float):
                raise ValueError('q must be a float of an array of floats; however, [%s] found.' % type(v))
            if v < 0.0 or v > 1.0:
        ...
        if isinstance(q, list):
            ...
            percentile_col = F.expr('approx_percentile(`%s`, array(%s), %s)' % (self.name, args, accuracy))
            ...
            return self._reduce_for_stat_function((lambda _: F.expr('approx_percentile(`%s`, %s, %s)' % (self.name, q, accuracy))),
              name='median')
    ...
    def _rank(self, method='average', ascending=True, part_cols=()):
        ...
        if method == 'first':
            window = (Window.orderBy(asc_func(column_name), asc_func(index_column)).partitionBy)(*part_cols).rowsBetween(Window.unboundedPreceding, Window.currentRow)
            scol = F.row_number().over(window)
        else:
            if method == 'dense':
                window = (Window.orderBy(asc_func(column_name)).partitionBy)(*part_cols).rowsBetween(Window.unboundedPreceding, Window.currentRow)
                ...
                if method == 'average':
                    ...
                    if method == 'min':
                        ...
                        if method == 'max':
                            stat_func = F.max
                        window1 = (Window.orderBy(asc_func(column_name)).partitionBy)(*part_cols).rowsBetween(Window.unboundedPreceding, Window.currentRow)
                        window2 = (Window.partitionBy)(*[
                         column_name] + list(part_cols)).rowsBetween(Window.unboundedPreceding, Window.unboundedFollowing)
    ...
    def truncate(self, before=None, after=None, copy=True):
        ...
        if not indexes_increasing:
            if not indexes.is_monotonic_decreasing:
    ...
    def mode(self, dropna=True) -> 'Series':
        ...
        sdf = sdf_most_value.select(F.col(SPARK_INDEX_NAME_FORMAT(0)).alias('0'))
        internal = _InternalFrame(sdf=sdf)
    ...
    def first_valid_index(self):
        ...
        first_valid_idx = tuple(first_valid_row[idx_col] for idx_col in self._internal.index_columns)
    ...
    def update(self, other):
        ...
        this_col = '__this_%s' % str(self._internal.column_name_for(self._internal.column_index[0]))
        that_col = '__that_%s' % str(self._internal.column_name_for(other._internal.column_index[0]))
        cond = F.when(scol_for(combined_sdf, that_col).isNotNull(), scol_for(combined_sdf, that_col)).otherwise(combined_sdf[this_col]).alias(str(self._internal.column_name_for(self._internal.column_index[0])))
        internal = _InternalFrame(sdf=(combined_sdf.select(index_scol_names + [cond])),
    ...
    def where(self, cond, other=np.nan):
        ...
        index_column = self._internal.index_columns[0]
        condition = F.when(sdf['__tmp_cond_col__'], sdf[data_col_name]).otherwise(sdf['__tmp_other_col__']).alias(data_col_name)
    ...
    def xs(self, key, level=None):
        ...
        if not isinstance(key, tuple):
            ...
            if level is None:
                level = 0
            cols = self._internal.index_scols[:level] + self._internal.index_scols[level + len(key):] + [
    ...
    def _cum(self, func, skipna, part_cols=()):
        ...
        window = (Window.orderBy(index_columns).partitionBy)(*part_cols).rowsBetween(Window.unboundedPreceding, Window.currentRow)
        if skipna:
            scol = F.when(self._scol.isNull(), F.lit(None)).otherwise(func(self._scol).over(window))
        else:
            scol = F.when(F.max(self._scol.isNull()).over(window), F.lit(None)).otherwise(func(self._scol).over(window))
    ...
    def _reduce_for_stat_function(self, sfun, name, axis=None, numeric_only=None):
        ...
        if axis in ('columns', 1):
            ...
            if isinstance(col_type, BooleanType):
                if sfun.__name__ not in ('min', 'max'):
    ...
    def __getitem__(self, key):
        if not isinstance(key, tuple):
            key = (
             key,)
        if len(self._internal._index_map) < len(key):
            raise KeyError('Key length ({}) exceeds index depth ({})'.format(len(key), len(self._internal.index_map)))
        cols = self._internal.index_scols[len(key):] + [
         self._internal.scol_for(self._internal.column_index[0])]
        rows = [self._internal.scols[level] == index for level, index in enumerate(key)]
        sdf = self._internal.sdf.select(cols).where(reduce(lambda x, y: x & y, rows))
        if len(self._internal._index_map) == len(key):
            pdf = sdf.limit(2).toPandas()
            length = len(pdf)
        ...
        if max_display_count is None:
            ...
            if pser_length > max_display_count:
                ...
                if match is not None:
                    ...
                    footer = '\nName: {name}, dtype: {dtype}\nShowing only the first {length}'.format(length=length,
                      name=(self.name),
                      dtype=(pprint_thing(name)))
                    return rest + footer
