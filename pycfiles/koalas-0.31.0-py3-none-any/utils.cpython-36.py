...
# Size of source mod 2**32: 16476 bytes
"""
...
def combine_frames(this, *args, how='full'):
    ...
    if get_option('compute.ops_on_diff_frames'):
        ...
        joined_df = this._sdf.alias('this').join((that._sdf.alias('that')),
          on=join_scols, how=how)
        joined_df = joined_df.select(merged_index_scols + [this[idx]._scol.alias('__this_%s' % this._internal.column_name_for(idx)) for idx in this._internal.column_index] + [that[idx]._scol.alias('__that_%s' % that._internal.column_name_for(idx)) for idx in that._internal.column_index])
        ...
        column_index = [tuple(['this'] + [''] * (level - len(idx)) + list(idx)) for idx in this._internal.column_index] + [tuple(['that'] + [''] * (level - len(idx)) + list(idx)) for idx in that._internal.column_index]
        column_index_names = [None] * (1 + level - len(this._internal.column_index_level)) + this._internal.column_index_names if this._internal.column_index_names is not None else None
        return DataFrame(this._internal.copy(sdf=joined_df, column_index=column_index,
    ...
    if not how == 'full':
        ...
        for combined_idx in combined_column_index:
            for common_idx in common_column_index:
                ...
                if how == 'left':
                    if combined_idx in [tuple(['that', *idx]) for idx in that_column_index]:
        ...
        if len(this_columns_to_apply) > 0 or len(that_columns_to_apply) > 0:
            kser_set, column_index_applied = zip(*resolve_func(combined, this_columns_to_apply, that_columns_to_apply))
...
def align_diff_series(func, this_series, *args, how='full'):
    ...
    that_columns = [combined['that'][arg._internal.column_index[0]]._scol if isinstance(arg, IndexOpsMixin) else arg for arg in args]
    scol = func(combined['this'][this_series._internal.column_index[0]]._scol, *that_columns)
    return Series(combined._internal.copy(scol=scol, column_index=(this_series._internal.column_index)),
      anchor=combined)
    ...
    for param in koalas_params.values():
        if param.name not in pandas_params:
            if args[param.name] == param.default:
                ...
                raise TypeError("The pandas version [%s] available does not support parameter '%s' for function '%s'." % (pd.__version__, param.name, pandas_func.__name__))
