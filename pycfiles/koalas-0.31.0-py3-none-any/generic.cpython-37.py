...
# [GCC 8.4.0]
# Embedded file name: ./build/lib/databricks/koalas/generic.py
# Compiled at: 2020-04-01 14:53:32
# Size of source mod 2**32: 64342 bytes
"""
...
from databricks.koalas.utils import validate_arguments_and_invoke_function, scol_for, validate_axis, align_diff_frames, name_like_string
...
class _Frame(object):
    ...
    def cummin(self, skipna: bool=True):
        ...
        """
        return self._apply_series_op(lambda kser: kser._cum(F.min, skipna))
    ...
    def cummax(self, skipna: bool=True):
        ...
        """
        return self._apply_series_op(lambda kser: kser._cum(F.max, skipna))
    ...
    def cumsum(self, skipna: bool=True):
        ...
        Name: A, dtype: float64
        """
        return self._apply_series_op(lambda kser: kser._cum(F.sum, skipna))
    ...
    def cumprod(self, skipna: bool=True):
        ...
        """
        return self._apply_series_op(lambda kser: kser._cumprod(skipna))
    ...
    def to_csv(self, path=None, sep=',', na_rep='', columns=None, header=True, quotechar='"', date_format=None, escapechar=None, num_files=None, index_col: Optional[Union[(str, List[str])]]=None, **options):
        r"""
        ...
        if path is None:
            kdf_or_ser = self
            if LooseVersion('0.24') > LooseVersion(pd.__version__):
                if isinstance(self, ks.Series):
                    return kdf_or_ser.to_pandas().to_csv(None,
                      sep=sep,
                      na_rep=na_rep,
                      header=header,
                ...
                if columns is None:
                    ...
                    if isinstance(columns, str):
                        ...
                        if isinstance(columns, tuple):
                            ...
                            column_labels = [lb if isinstance(lb, tuple) else (lb,) for lb in columns]
        ...
        if header is True and kdf._internal.column_labels_level > 1:
            ...
            if isinstance(header, list):
                ...
                sdf = sdf.select([scol_for(sdf, name_like_string(label)) for label in index_cols] + [self._internal.spark_column_for(label).alias(new_name) for label, new_name in zip(column_labels, header)])
                header = True
            else:
                sdf = kdf.to_spark(index_col)
                sdf = sdf.select([scol_for(sdf, name_like_string(label)) for label in index_cols] + [kdf._internal.spark_column_for(label) for label in column_labels])
        if num_files is not None:
            sdf = sdf.repartition(num_files)
        builder = sdf.write.mode('overwrite')
        OptionUtils._set_opts(builder,
          path=path,
          sep=sep,
          nullValue=na_rep,
          header=header,
    ...
    def to_json(self, path=None, compression='uncompressed', num_files=None, index_col: Optional[Union[(str, List[str])]]=None, **options):
        """
    ...
    def mean(self, axis=None, numeric_only=True):
        ...
        return self._reduce_for_stat_function((F.mean),
          name='mean', numeric_only=numeric_only, axis=axis)
    ...
    def sum(self, axis=None, numeric_only=True):
        ...
        return self._reduce_for_stat_function((F.sum),
          name='sum', numeric_only=numeric_only, axis=axis)
    ...
    def skew(self, axis=None, numeric_only=True):
        ...
        return self._reduce_for_stat_function((F.skewness),
          name='skew', numeric_only=numeric_only, axis=axis)
    ...
    def kurtosis(self, axis=None, numeric_only=True):
        ...
        return self._reduce_for_stat_function((F.kurtosis),
          name='kurtosis', numeric_only=numeric_only, axis=axis)
    ...
    def min(self, axis=None, numeric_only=None):
        ...
        return self._reduce_for_stat_function((F.min),
          name='min', numeric_only=numeric_only, axis=axis)
    ...
    def max(self, axis=None, numeric_only=None):
        ...
        return self._reduce_for_stat_function((F.max),
          name='max', numeric_only=numeric_only, axis=axis)
    ...
    def std(self, axis=None, numeric_only=True):
        ...
        return self._reduce_for_stat_function((F.stddev),
          name='std', numeric_only=numeric_only, axis=axis)
    ...
    def var(self, axis=None, numeric_only=True):
        ...
        return self._reduce_for_stat_function((F.variance),
          name='var', numeric_only=numeric_only, axis=axis)
    ...
    def abs(self):
        ...
        3  7  40   50
        """
        return self._apply_series_op(lambda kser: kser._with_new_scol(F.abs(kser._scol)).rename(kser.name))
    ...
    def groupby(self, by, axis=0, as_index: bool=True):
        ...
        if isinstance(df_or_s, ks.DataFrame):
            df = df_or_s
            should_drop_index = any((isinstance(col_or_s, ks.Series) and df is not col_or_s._kdf for col_or_s in by))
            kdf, col_by = self._resolve_grouping_series(by)
            return DataFrameGroupBy(kdf,
              col_by, as_index=as_index, should_drop_index=should_drop_index)
        if isinstance(df_or_s, ks.Series):
            col = df_or_s
            _, col_by = self._resolve_grouping_series(by)
            return SeriesGroupBy(col, col_by, as_index=as_index)
        raise TypeError('Constructor expects DataFrame or Series; however, got [%s]' % (df_or_s,))
    ...
    def first_valid_index(self):
        ...
        first_valid_idx = tuple((first_valid_row[idx_col] for idx_col in self._internal.index_spark_column_names))
    ...
    def median(self, accuracy=10000):
        ...
        if isinstance(kdf_or_kser, Series):
            ...
            return kser._reduce_for_stat_function((lambda _: F.expr('approx_percentile(`%s`, 0.5, %s)' % (
        ...
        return DataFrame(kdf._internal.copy(spark_frame=sdf,
          index_map=(OrderedDict({'__DUMMY__': None})),
          data_spark_columns=[scol_for(sdf, col) for col in kdf._internal.data_spark_column_names]))._to_internal_pandas().transpose().iloc[:,
         0]
