...
# Size of source mod 2**32: 66363 bytes
...
class KoalasBoxPlot(BoxPlot):
    ...
    def _compute_plot_data(self):
        ...
        if showfliers:
            ...
            fliers = []
        stats = []
        item = {'mean':col_stats['mean'],  'med':col_stats['med'], 
         'q1':col_stats['q1'], 
         'q3':col_stats['q3'], 
         'whislo':whiskers[0], 
         'whishi':whiskers[1], 
    ...
    def _compute_stats(data, colname, whis, precision):
        pdf = (data._kdf._sdf.agg)(*[F.expr('approx_percentile({}, {}, {})'.format(colname, q, int(1.0 / precision))).alias('{}_{}%'.format(colname, int(q * 100))) for q in (0.25,
                                                                                                                                                                              0.5,
                                                                                                                                                                              0.75)], *(
...
class KoalasHistPlot(HistPlot):
    ...
    def _compute_plot_data(self):
        ...
        numeric_data = data.select_dtypes(include=[
         'byte', 'decimal', 'integer', 'float', 'long', 'double', np.datetime64])
