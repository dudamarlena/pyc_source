...
# [GCC 8.4.0]
# Embedded file name: ./build/lib/databricks/koalas/typedef.py
# Compiled at: 2020-03-04 21:51:39
# Size of source mod 2**32: 15448 bytes
"""
...
__all__ = [
 'pandas_wraps', 'as_spark_type', 'as_python_type', 'infer_pd_series_spark_type']
...
class _DataFrame(object):
    ...
    def __init__(self, tpe):
        self.tpe = types.StructType([types.StructField('c%s' % i, tpe[i]) for i in range(len(tpe))])
...
def _to_stype(tpe) -> X:
    if hasattr(tpe, '__origin__'):
        if tpe.__origin__ == ks.Series:
            inner = as_spark_type(tpe.__args__[0])
            return _Series(inner)
    if hasattr(tpe, '__origin__'):
        if tpe.__origin__ == ks.DataFrame:
...
_base = {types.StringType(): [str, 'str', 'string'], 
...
def _build_type_dict():
    return dict([(other_type, spark_type) for spark_type, l in _base.items() for other_type in l] + [(spark_type, spark_type) for spark_type, _ in _base.items()])
...
def _make_fun(f: typing.Callable, return_type: types.DataType, *args, **kwargs) -> 'ks.Series':
    ...
    series = kser._with_new_scol(scol=col)
...
def _get_kser(args, kwargs):
    from databricks.koalas.series import Series
    all_cols = [arg for arg in args if isinstance(arg, Series)] + [arg for arg in kwargs.values() if isinstance(arg, Series)]
    assert all_cols
    return all_cols[0]
    ...
    def function_wrapper(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            if not isinstance(sig_return, _Series):
                raise ValueError('Expected the return type of this function to be of type column, but found type {}'.format(sig_return))
    ...
    if not return_col:
        if not return_sig:
            if not return_scalar:
                raise ValueError('Missing type information. It should either be provided as an argument to pandas_wraps, or as a python typing hint')
