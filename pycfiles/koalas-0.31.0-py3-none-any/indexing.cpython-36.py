...
# Size of source mod 2**32: 28233 bytes
"""
    ...
    def __getitem__(self, key):
        if self._ks is None:
            ...
            if len(self._kdf._internal.index_columns) != 1:
                ...
                if column not in self._kdf._internal.data_columns:
...
class LocIndexer(object):
    ...
    def __getitem__(self, key):
        ...
        if isinstance(rows_sel, Series):
            ...
            assert isinstance(rows_sel, slice) and len(self._kdf._internal.index_columns) > 0
                    ...
                    if isinstance(cols_sel, spark.Column):
                        ...
                        if all(isinstance(key, Series) for key in cols_sel):
                            ...
                            if all(isinstance(key, spark.Column) for key in cols_sel):
                                ...
                                if any(isinstance(key, tuple) for key in cols_sel):
                        ...
                        try:
                            ...
                            internal = _InternalFrame(sdf=sdf, index_map=(self._kdf._internal.index_map), column_index=column_index,
        ...
        if isinstance(rows_sel, Index):
            ...
            if isinstance(rows_sel, slice):
                if rows_sel == slice(None):
                    ...
                    if rows_sel.start is not None or rows_sel.step is not None:
            ...
            if isinstance(cols_sel, Series):
                ...
                if isinstance(cols_sel, int):
                    columns = [
                     self._kdf._internal.column_scols[cols_sel]]
                else:
