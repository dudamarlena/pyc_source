...
# [GCC 8.4.0]
# Embedded file name: ./build/lib/databricks/koalas/utils.py
# Compiled at: 2020-03-12 18:31:56
# Size of source mod 2**32: 19119 bytes
"""
...
def combine_frames(this, *args, how='full'):
    ...
    if all((isinstance(arg, Series) for arg in args)):
        assert all((arg._kdf is args[0]._kdf for arg in args)), 'Currently only one different DataFrame (from given Series) is supported'
    ...
    if get_option('compute.ops_on_diff_frames'):
        ...
        for (this_column, this_name), (that_column, that_name) in this_and_that_index_map:
            if this_name == that_name:
                ...
                merged_index_scols.append(F.when(this_scol.isNotNull(), this_scol).otherwise(that_scol).alias(this_column))
            else:
        ...
        joined_df = joined_df.select(merged_index_scols + [this[label]._scol.alias('__this_%s' % this._internal.spark_column_name_for(label)) for label in this._internal.column_labels] + [that[label]._scol.alias('__that_%s' % that._internal.spark_column_name_for(label)) for label in that._internal.column_labels])
        ...
        column_labels = [tuple(['this'] + [''] * (level - len(label)) + list(label)) for label in this._internal.column_labels] + [tuple(['that'] + [''] * (level - len(label)) + list(label)) for label in that._internal.column_labels]
        column_label_names = [None] * (1 + level - len(this._internal.column_labels_level)) + this._internal.column_label_names if this._internal.column_label_names is not None else None
        return DataFrame(this._internal.copy(spark_frame=joined_df,
          column_labels=column_labels,
          data_spark_columns=[scol_for(joined_df, col) for col in new_data_columns],
          column_label_names=column_label_names))
    raise ValueError("Cannot combine the series or dataframe because it comes from a different dataframe. In order to allow this operation, enable 'compute.ops_on_diff_frames' option.")
        ...
        for combined_label in combined_column_labels:
            for common_label in common_column_labels:
                if combined_label == tuple(['this', *common_label]):
                    ...
                    if combined_label == tuple(['that', *common_label]):
                        that_columns_to_apply.append(combined_label)
                        break
            else:
                if how == 'left':
                    if combined_label in [tuple(['that', *label]) for label in that_column_labels]:
...
def align_diff_series(func, this_series, *args, how='full'):
    ...
    scol = func(combined['this']._internal.data_spark_columns[0], *combined['that']._internal.data_spark_columns)
    return Series(combined._internal.copy(spark_column=scol,
...
def validate_arguments_and_invoke_function(pobj: Union[(pd.DataFrame, pd.Series)], koalas_func: Callable, pandas_func: Callable, input_args: Dict):
    """
    ...
    for param in koalas_params.values():
        if param.name not in pandas_params:
            if args[param.name] == param.default:
                ...
                raise TypeError("The pandas version [%s] available does not support parameter '%s' for function '%s'." % (
...
def validate_bool_kwarg(value, arg_name):
    ...
    if not isinstance(value, bool):
        if not value is None:
            raise ValueError('For argument "{}" expected type bool, received type {}.'.format(arg_name, type(value).__name__))
    return value
...
def verify_temp_column_name(sdf: spark.DataFrame, column_name: str) -> str:
    ...
    """
    assert column_name not in sdf.columns, 'The give column name `{}` already exists in the Spark DataFrame: {}'.format(column_name, sdf.columns)
    return column_name
