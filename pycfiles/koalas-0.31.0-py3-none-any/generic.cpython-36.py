...
# Size of source mod 2**32: 52822 bytes
"""
...
class _Frame(object):
    ...
    def cumprod(self, skipna: bool=True):
        ...
        def cumprod(scol):
            ...
            def negative_check(s):
                if not len(s) == 0:
                    if not ((s > 0) | s.isnull()).all():
                        raise AssertionError('values should be bigger than 0: %s' % s)
                return s

            return F.sum(F.log(negative_check(scol)))

        return self._cum(cumprod, skipna)
    ...
    def to_csv(self, path=None, sep=',', na_rep='', columns=None, header=True, quotechar='"', date_format=None, escapechar=None, num_files=None, **options):
        r"""
        ...
        if path is None:
            ...
            if LooseVersion('0.24') > LooseVersion(pd.__version__):
                if isinstance(self, ks.Series):
                    return kdf_or_ser.to_pandas().to_csv(None,
                      sep=sep, na_rep=na_rep, header=header, date_format=date_format,
                      index=False)
                return kdf_or_ser.to_pandas().to_csv(None,
                  sep=sep, na_rep=na_rep, columns=columns, header=header,
                  quotechar=quotechar,
            ...
            if kdf._internal.column_index_level > 1:
                raise ValueError('to_csv only support one-level index column now')
        else:
            if isinstance(header, list):
                sdf = kdf._sdf.select([self._internal.scol_for(idx).alias(new_name) for idx, new_name in zip(column_index, header)])
                header = True
            else:
    ...
    def to_excel(self, excel_writer, sheet_name='Sheet1', na_rep='', float_format=None, columns=None, header=True, index=True, index_label=None, startrow=0, startcol=0, engine=None, merge_cells=True, encoding=None, inf_rep='inf', verbose=True, freeze_panes=None):
        ...
        if isinstance(self, ks.DataFrame):
            ...
            if isinstance(self, ks.Series):
                ...
                raise TypeError('Constructor expects DataFrame or Series; however, got [%s]' % (
                 self,))
    ...
    def bool(self):
        ...
        if isinstance(self, ks.DataFrame):
            ...
            if isinstance(self, ks.Series):
                ...
                raise TypeError('bool() expects DataFrame or Series; however, got [%s]' % (
                 self,))
    ...
    def median(self, accuracy=10000):
        ...
        if isinstance(kdf_or_kser, Series):
            ...
            return DataFrame(kdf._internal.copy(sdf=sdf,
              index_map=[
             ('__DUMMY__', None)],
              column_scols=[scol_for(sdf, col) for col in kdf._internal.data_columns]))._to_internal_pandas().transpose().iloc[:,
             0]
...
def _resolve_col(kdf, col_like):
    if isinstance(col_like, ks.Series):
        if kdf is not col_like._kdf:
            raise ValueError("Cannot combine the series because it comes from a different dataframe. In order to allow this operation, enable 'compute.ops_on_diff_frames' option.")
        return col_like
    if isinstance(col_like, tuple):
        return kdf[col_like]
