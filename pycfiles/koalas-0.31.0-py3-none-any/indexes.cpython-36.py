...
# Size of source mod 2**32: 23840 bytes
"""
...
class Index(IndexOpsMixin):
    ...
    def __init__(self, data: Union[(DataFrame, list)], dtype=None, name=None, scol: Optional[spark.Column]=None) -> None:
        ...
        internal = kdf._internal.copy(scol=scol, column_index=(kdf._internal.index_names),
    ...
    def _validate_index_level(self, level):
        ...
        if isinstance(level, int):
            if level < 0:
                if level != -1:
                    raise IndexError('Too many levels: Index has only 1 level, %d is not a valid level number' % (
                     level,))
            if level > 0:
...
class MultiIndex(Index):
    ...
    def from_tuples(tuples, sortorder=None, names=None):
        ...
        MultiIndex([(1,  'red'),
                   ...
                   names=['number', 'color'])
        """
        return DataFrame(index=pd.MultiIndex.from_tuples(tuples=tuples,
          sortorder=sortorder,
          names=names)).index
    ...
    def from_arrays(arrays, sortorder=None, names=None):
        ...
        MultiIndex([(1,  'red'),
                   ...
                   names=['number', 'color'])
        """
        return DataFrame(index=pd.MultiIndex.from_arrays(arrays=arrays,
          sortorder=sortorder,
          names=names)).index
