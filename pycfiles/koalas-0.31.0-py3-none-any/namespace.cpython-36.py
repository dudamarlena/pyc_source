...
# Size of source mod 2**32: 71055 bytes
"""
...
def read_csv(path, sep=',', header='infer', names=None, index_col=None, usecols=None, squeeze=False, mangle_dupe_cols=True, dtype=None, parse_dates=False, quotechar=None, escapechar=None, comment=None, **options):
        ...
        if usecols is not None:
            ...
            if usecols is None or callable(usecols) or len(usecols) > 0:
                ...
                if isinstance(names, str):
                    ...
                    sdf = reader.csv(path)
                if header is None:
                    sdf = (sdf.selectExpr)(*['`%s` as `%s`' % (field.name, i) for i, field in enumerate(sdf.schema)])
                if isinstance(names, list):
                    names = list(names)
                    if len(set(names)) != len(names):
                        raise ValueError('Found non-unique column index')
                    if len(names) != len(sdf.schema):
                        raise ValueError('The number of names [%s] does not match the number of columns [%d]. Try names by a Spark SQL DDL-formatted string.' % (
                         len(sdf.schema), len(names)))
                    sdf = (sdf.selectExpr)(*['`%s` as `%s`' % (field.name, name) for field, name in zip(sdf.schema, names)])
                if usecols is not None:
                    if callable(usecols):
                        ...
                        if all(isinstance(col, int) for col in usecols):
                            ...
                            missing = [col for col in usecols if col >= len(sdf.schema) or sdf.schema[col].name not in cols]
                        else:
...
def read_excel(io, sheet_name=0, header=0, names=None, index_col=None, usecols=None, squeeze=False, dtype=None, engine=None, converters=None, true_values=None, false_values=None, skiprows=None, nrows=None, na_values=None, keep_default_na=True, verbose=False, parse_dates=False, date_parser=None, thousands=None, comment=None, skipfooter=0, convert_float=True, mangle_dupe_cols=True, **kwds):
    ...
    **kwds : optional
    ...
    pdfs = (pd.read_excel)(io=io, 
     sheet_name=sheet_name, header=header, names=names, index_col=index_col, usecols=usecols, 
     ...
     convert_float=convert_float, mangle_dupe_cols=mangle_dupe_cols, **kwds)
    ...
    if sparse is not False:
        ...
        if isinstance(data, Series):
            ...
            if columns is None:
                ...
                if any(isinstance(col, str) for col in columns):
                    if any(isinstance(col, tuple) for col in columns):
                        raise ValueError('Expected tuple, got str')
                    else:
                        column_index = [idx for key in columns if idx == key or idx[0] == key for idx in kdf._internal]
                elif len(column_index) == 0:
                    return kdf
                if prefix is None:
                    prefix = [str(idx) if len(idx) > 1 else idx[0] for idx in column_index]
                column_index_set = set(column_index)
                remaining_columns = [kdf[idx].rename(name_like_string(idx)) for idx in kdf._internal.column_index if idx not in column_index_set]
        ...
        if prefix is not None:
            if len(column_index) != len(prefix):
                raise ValueError("Length of 'prefix' ({}) did not match the length of the columns being encoded ({}).".format(len(prefix), len(column_index)))
...
def concat(objs, axis=0, join='outer', ignore_index=False):
    ...
    if isinstance(objs, (DataFrame, IndexOpsMixin)) or not isinstance(objs, Iterable):
            ...
            if join == 'inner':
                ...
                if join == 'outer':
                    ...
                    for kdf in objs:
                        ...
                        data_columns = kdf._internal.data_columns + [name_like_string(idx) for idx in columns_to_add]
        ...
        if ignore_index:
            ...
            sdfs = [kdf._sdf.select(kdf._internal.index_scols + kdf._internal.column_scols) for kdf in kdfs]
...
def merge(obj, right: 'DataFrame', how: str='inner', on: Union[(str, List[str], Tuple[(str, ...)], List[Tuple[(str, ...)]])]=None, left_on: Union[(str, List[str], Tuple[(str, ...)], List[Tuple[(str, ...)]])]=None, right_on: Union[(str, List[str], Tuple[(str, ...)], List[Tuple[(str, ...)]])]=None, left_index: bool=False, right_index: bool=False, suffixes: Tuple[(str, str)]=('_x', '_y')) -> 'DataFrame':
    ...
    return obj.merge(right,
      how=how, on=on, left_on=left_on, right_on=right_on, left_index=left_index,
