...
# Embedded file name: build/bdist.linux-i686/egg/pyjon/versionning/base.py
# Compiled at: 2010-10-04 09:22:43
import os
...
class Repository(object):
    ...
    def __init__(self, repo_folder, default_user='process'):
    ...
    def store_all(self):
        ...
        commands.commit(self.repo_ui, self.repo, message='initial import', user=self.default_user)
    ...
    def get_obj_name(self, item):
        if hasattr(item, 'name'):
            return unicodedata.normalize('NFKD', item.name).encode('ascii', 'ignore').lower().replace(' ', '_')
        else:
            if hasattr(item, 'display_name'):
                return unicodedata.normalize('NFKD', item.display_name).encode('ascii', 'ignore').lower().replace(' ', '_')
            return str(item.id)
    ...
    def store_item(self, item, user=None):
        self.__store_item(item, commit=True, user=user or self.default_user)

    def delete_item(self, item, user=None):
        file_path = self.get_filepath(item)
        if os.path.exists(file_path):
            commands.remove(self.repo_ui, self.repo, file_path)
            commands.commit(self.repo_ui, self.repo, file_path, message='deleting %s "%s"' % (
             self.get_entityname(item),
             self.get_obj_name(item)), user=user or self.default_user)
    ...
    def __store_item(self, item, commit=False, user=None):
        ...
        if commit is True:
            try:
                ...
                commands.commit(self.repo_ui, self.repo, file_path, message=str(msg), user=str(user))
            except util.Abort, message:
                if 'file not tracked' in unicode(message):
                    msg = 'creating %s - %s' % (
                     self.get_entityname(item),
                     self.get_obj_name(item))
                    commands.add(self.repo_ui, self.repo, file_path)
                    commands.commit(self.repo_ui, self.repo, str(file_path), message=str(msg), user=str(user))
                else:
                    raise