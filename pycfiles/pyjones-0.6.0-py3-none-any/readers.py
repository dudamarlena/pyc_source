...
# Embedded file name: build/bdist.linux-x86_64/egg/pyjon/descriptors/readers.py
# Compiled at: 2015-01-20 10:19:14
__doc__ = 'defines specialized readers that will be used by the pyjon descriptors\n'
__all__ = ['CSVReader', 'FixedLengthReader']
import ast, csv, logging
...
class ItemFactory(object):
    ...
    def __init__(self, schemaroot, encoding, caster):
        ...
        for field in schema_fields:
            ...
            type_dict = dict(name=type_node.text.lower().strip(), items=dict(type_node.items()))
            field_dict = dict(name=field.get('name'), type=type_dict, index=int(field.find('source').text) - 1)
            ...
            if mandatory is not None:
                ...
                msg = "Attribute 'mandatory' is missing from the field node %s" % field_dict['name']
                raise KeyError(msg)
            ...
            if prerenderer_node is not None:
                ...
                if prerenderer_type not in ('eval', 'function'):
                    raise ValueError('prerenderer type value can be "eval" or "function", not "%s"' % prerenderer_type)
                ...
                if field_dict['prerenderer_type'] == 'function':
                    ...
                    field_dict['prerenderer_function'] = evaldict[code_func_name]
                ...
                if 'prerenderer_value' in field:
                    if field['prerenderer_type'] == 'eval':
                        ...
                        try:
                            ...
                            raise ValueError('Error during evaluating expression %s : %s' % (
                             expression, str(e)))
...
class CSVReader(object):
    ...
    def __init__(self, encoding, schema_root, keep_source_ref=True):
        ...
        self.startline = int(self.schema_root.find('header/format/startline').text)
...
class XMLReader(object):
    ...
    def __init__(self, encoding, schema_root):
        ...
        for field in schema_fields:
            ...
            setattr(f, 'mandatory', fm == 'true' or fm == 'True')
            setattr(f, 'name', field.attrib['name'])
            setattr(f, 'source', field.find('source').text.strip())
            setattr(f, 'type', dict(name=field.find('type').text.strip(), items=field.find('type').attrib))
        ...
        for action, elem in context:
            ...
            if action == 'end' and in_datazone and elem.tag == dataitem:
                ...
                for field in self.fields:
                    ...
                    setattr(input_item, field.name, self.caster.to_python(child_value, field.type))
...
class FixedLengthReader(object):
    ...
    def __init__(self, encoding, schema_root, keep_source_ref=True, buffersize=16384):
        """@param encoding: the encoding that was declared by the stream in its
        header. This will be used by our readers to handle characters.
        @type encoding: string
    ...
    def __read(self, source):
        ...
        if splitlength is not None:
            ...
            source = self.split_source(source, splitlength_value)
