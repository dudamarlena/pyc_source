...
# Embedded file name: build/bdist.linux-x86_64/egg/pyjon/utils/base_converter.py
# Compiled at: 2014-10-10 11:21:37
__doc__ = 'Convert numbers from any base to any other base.\n\nThis is based on the work by wanyewon (PSF license):\n    http://code.activestate.com/recipes/65212/#c9\n'

def decimal_to_n(num, base_to, numerals='0123456789abcdefghijklmnopqrstuvwxyz'):
...
def n_to_m(num, base_from, base_to, numerals_to='0123456789abcdefghijklmnopqrstuvwxyz'):
    ...
    if not 2 <= base_from <= 36:
        raise ValueError('The base_from parameter must be between 2 and 36')
    if not 2 <= base_to <= len(numerals_to):
        raise ValueError('The base_to parameter must be between 2 and %s' % len(numerals_to))
