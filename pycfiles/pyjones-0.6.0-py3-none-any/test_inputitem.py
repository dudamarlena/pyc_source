...
# Embedded file name: build/bdist.linux-x86_64/egg/pyjon/descriptors/tests/test_inputitem.py
# Compiled at: 2015-01-20 10:16:16
__doc__ = 'test the input item mechanism\n'
...
class TestInputItem(unittest.TestCase):
    ...
    def test_items_with_private_callable(self):
        i = InputItem()
        i.update(dict(__a=lambda a: a ** 2))
        for k, v in i.items():
            self.assertEquals(k, 'a')
            self.assertEquals(v, 1)
