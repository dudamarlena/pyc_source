...
# Embedded file name: build/bdist.linux-x86_64/egg/pyjon/utils/main.py
# Compiled at: 2014-10-10 12:42:43
__doc__ = 'utilities for pyjon\n'
...
def indent(elem, root_level=0):
    ...
    if len(elem):
        ...
        if not elem.tail or not elem.tail.strip():
            elem.tail = i
    elif root_level and (not elem.tail or not elem.tail.strip()):
        elem.tail = i
        ...
        if 'caller_args_count' in opt:
            ...
            for i in range(options['caller_args_count']):
                try:
                    ...
                    raise TypeError('function takes exactly %d arguments (%d given)' % (
...
def substitute(string, callback, default='', regex='\\{([^{}]+?)\\}', sub_if_none=True):
    ...
    try:
        if not isinstance(default, basestring):
            raise ValueError('The default value must be a basestring, not a %s' % type(default))
    except NameError:
    ...
    for match in pattern.finditer(string):
        ...
        if rep is not None and not rep_is_base:
            raise ValueError('The callback must return a basestring, not a %s' % type(rep))
