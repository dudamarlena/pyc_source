...
# Embedded file name: build/bdist.linux-x86_64/egg/pyjon/reports/factory.py
# Compiled at: 2010-07-12 10:29:01
...
class ReportFactory(object):
    ...
    def __init__(self, template_path=None, auto_reload=True):
        ...
        if self.template_path:
            self.templates_loader = TemplateLoader(self.template_path, auto_reload=auto_reload)
        return
    ...
    def get_template(self, template_string=None, template_file=None, **kwargs):
        if not template_string:
            if template_file:
                template_file = self.templates_loader or open(template_file, 'r')
                template_string = template_file.read()
                template_file.close()
                template = ReportTemplate(template_string)
            else:
                template = self.templates_loader.load(template_file, cls=ReportTemplate)
        elif template_string and not template_file:
            template = ReportTemplate(template_string, loader=self.templates_loader)
        else:
            raise ValueError('You should provide either a template string' + ' or a template file.')
        return template

    def render_template(self, template_string=None, template_file=None, **kwargs):
        template = self.get_template(template_string=template_string, template_file=template_file, **kwargs)
        temp_filename = self.create_tempfile()
        template.render(temp_filename, **kwargs)
        self.check_last_tempfile()

    def render_template_flow(self, template_string=None, template_file=None, **kwargs):
        template = self.get_template(template_string=template_string, template_file=template_file, **kwargs)
        temp_filename = self.create_tempfile()
