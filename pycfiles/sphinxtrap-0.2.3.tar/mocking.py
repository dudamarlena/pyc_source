...
# [GCC 8.4.0]
# Embedded file name: /home/jose/Documents/projects/sphinxtrap/repo/tests/mocking.py
...
class TestSphinxtrap(unittest.TestCase):
    ...
    def test_parser(self):
        ...
        with open(os.path.join(path, 'test.html')) as (fp):
            pl.feed(fp.read())
        self.assertEqual(pl.links[7]['childs'][0]['href'], '#compression-algorithms')

    def test_build_raw_toctree(self):
        links = build_raw_toctree(builder, 'test', False, False)
        self.assertEqual(links[7]['childs'][0]['href'], '#compression-algorithms')

    def test_icon_role(self):
        icon = icon_role('foo', ':icon:`foo`', 'foo', 100, inliner, options={'classes': []}, content=[])
        self.assertTrue(isinstance(icon[0][0], emphasis))

    def test_btn_role(self):
        l_str = ':btn:`Foo <http://foo.org/>,btn-success,icon-globe`'
        btn = btn_role('foo', l_str, l_str, 100, inliner, options={'classes': []})
