...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_saltstack/sharepoint/views.py
...
class TenantViewSet(structure_views.BaseOnlineResourceViewSet):
    ...
    def perform_provision(self, serializer):
        ...
        backend.provision(tenant, site_name=site_name, site_description=site_description, template_uuid=template.uuid.hex, phone=phone, notify=notify)
        return
    ...
    def change_quotas(self, request, pk=None, **kwargs):
        ...
        event_logger.sharepoint_tenant.info('Sharepoint tenant {tenant_name} quota has been updated from %s to %s.' % (old_quota.limit, new_quota_limit), event_type='sharepoint_tenant_quota_update', event_context={'tenant': tenant})
...
class UserViewSet(BasePropertyViewSet):
    ...
    def password(self, request, pk=None, **kwargs):
        ...
        if serializer.validated_data.get('notify'):
            sms_user_password(user)
        event_logger.sharepoint_user.info('Sharepoint user {affected_user_name} ({affected_user_email}) password has been reset.', event_type='sharepoint_user_password_reset', event_context={'affected_user': user})
        return response.Response(serializer.data, status=HTTP_200_OK)


class SiteCollectionViewSet(mixins.CreateModelMixin, mixins.RetrieveModelMixin, mixins.DestroyModelMixin, mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    def perform_create(self, serializer):
        ...
        try:
            storage = serializer.validated_data.pop('storage')
            backend_site = backend.site_collections.create(admin_id=user.admin_id, template_code=template.code, site_url=serializer.validated_data['site_url'], name=serializer.validated_data['name'], description=serializer.validated_data['description'], storage=storage)
    ...
    def perform_destroy(self, site_collection):
        if not site_collection.deletable:
            raise exceptions.PermissionDenied('It is impossible to delete default tenant site collections.')
    ...
    def change_quotas(self, request, pk=None, **kwargs):
        ...
        backend = tenant.get_backend()
        serializer = serializers.SiteCollectionQuotaSerializer(data=request.data, context={'site_collection': site_collection})
        serializer.is_valid(raise_exception=True)
        if site_collection.type == models.SiteCollection.Types.PERSONAL and site_collection.template is None:
            try:
                ...
                return response.Response({'detail': 'User personal site collection is not initialized'}, status=HTTP_409_CONFLICT)

            site_collection.template = models.Template.objects.filter(code=check_result.template_code, settings=tenant.service_project_link.service.settings).first()
            site_collection.save()
        ...
        event_logger.sharepoint_site_collection.info('Sharepoint site collection {site_collection_name} quota has been updated from %s to %s.' % (
         old_quota.limit, storage), event_type='sharepoint_site_collection_quota_update', event_context={'site_collection': site_collection})
