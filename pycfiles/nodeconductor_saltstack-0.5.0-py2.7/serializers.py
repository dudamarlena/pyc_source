...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_saltstack/sharepoint/serializers.py
...
class SiteCollectionSerializer(serializers.HyperlinkedModelSerializer):
    ...
    template = serializers.HyperlinkedRelatedField(view_name='sharepoint-templates-detail', queryset=Template.objects.all(), lookup_field='uuid')
    ...
    class Meta(object):
        ...
        fields = ('url', 'uuid', 'template', 'template_code', 'template_name', 'user',
                  'storage', 'name', 'description', 'type', 'quotas', 'site_url',
                  'access_url', 'deletable')
        extra_kwargs = {'url': {'lookup_field': 'uuid'}, 'user': {'lookup_field': 'uuid', 'view_name': 'sharepoint-users-detail'}, 'tenant': {'lookup_field': 'uuid', 'view_name': 'sharepoint-tenants-detail'}}
        read_only_fields = ('uuid', 'access_url', 'type')
        protected_fields = ('template', )

    def validate_user(self, user):
        if user.tenant.state != SharepointTenant.States.ONLINE:
            raise serializers.ValidationError('It is impossible to create site collection if user tenant is not online.')
        return user
    ...
    def validate(self, attrs):
        ...
        if storage_quota.is_exceeded(delta=attrs['storage']):
            ...
            raise serializers.ValidationError('Storage quota is over limit. Site collection cannot be greater then %s MB' % max_storage)
        if SiteCollection.objects.filter(name=attrs['name'], user__tenant=tenant).exists():
            raise serializers.ValidationError('Site collection with name "%s" already exists in such tenant' % attrs['name'])
        return attrs
...
class UserSerializer(AugmentedSerializerMixin, PhoneValidationMixin, serializers.HyperlinkedModelSerializer):
    ...
    class Meta(object):
        ...
        fields = ('url', 'uuid', 'tenant', 'tenant_uuid', 'tenant_domain', 'name',
                  'email', 'first_name', 'last_name', 'username', 'password', 'phone',
                  'notify', 'personal_site_collection')
        read_only_fields = ('uuid', 'password')
        protected_fields = ('tenant', 'notify')
        extra_kwargs = {'url': {'lookup_field': 'uuid'}, 'tenant': {'lookup_field': 'uuid', 'view_name': 'sharepoint-tenants-detail'}}
        related_paths = {'tenant': ('uuid', 'domain')}
...
class TenantSerializer(PhoneValidationMixin, structure_serializers.PublishableResourceSerializer):
    ...
    service = serializers.HyperlinkedRelatedField(source='service_project_link.service', view_name='saltstack-detail', read_only=True, lookup_field='uuid')
    service_project_link = serializers.HyperlinkedRelatedField(view_name='saltstack-spl-detail', queryset=SaltStackServiceProjectLink.objects.all())
    storage = serializers.IntegerField(min_value=MINIMUM_TENANT_STORAGE_SIZE, write_only=True, initial=5120)
    ...
    template = serializers.HyperlinkedRelatedField(view_name='sharepoint-templates-detail', queryset=Template.objects.all(), lookup_field='uuid', write_only=True)
    ...
    class Meta(structure_serializers.PublishableResourceSerializer.Meta):
        ...
        fields = structure_serializers.PublishableResourceSerializer.Meta.fields + ('domain',
                                                                                    'quotas',
                                                                                    'storage',
                                                                                    'management_ip',
                                                                                    'site_name',
                                                                                    ...
                                                                                    'notify',
                                                                                    'access_url')
        protected_fields = structure_serializers.PublishableResourceSerializer.Meta.protected_fields + ('domain',
                                                                                                        'storage',
                                                                                                        'site_name',
                                                                                                        'site_description',
                                                                                                        'template',
    ...
    def validate(self, attrs):
        ...
        if not self.instance:
            ...
            if spl_storage_quota.is_exceeded(delta=attrs.get('storage')):
                ...
                raise serializers.ValidationError({'storage': 'Service project link quota exceeded: Total tenant storage size should be lower than %s MB' % storage_left})
...
class TenantQuotaSerializer(serializers.Serializer):
    storage = serializers.FloatField(min_value=TenantSerializer.MINIMUM_TENANT_STORAGE_SIZE, write_only=True, help_text='Maximum storage size, MB')
    ...
    def validate_storage(self, value):
        ...
        if diff > 0:
            ...
            if spl_storage_quota.is_exceeded(delta=diff):
                storage_left = spl_storage_quota.limit - spl_storage_quota.usage
                raise serializers.ValidationError('Service project link quota exceeded: Tenant size cannot be increased by more than %s MB' % storage_left)
        return value
