...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_saltstack/sharepoint/admin.py
...
class SharepointTenantAdmin(structure_admin.PublishableResourceAdmin):
    inlines = [
     QuotaInline]
    actions = [
     'sync_users', 'sync_site_collections']
    ...
    def sync_site_collections(self, request, queryset):
        ...
        message = ungettext('One tenant site collections scheduled for sync', '%(tasks_scheduled)d enant site collections scheduled for sync', tasks_scheduled)
    ...
    def sync_users(self, request, queryset):
        ...
        message = ungettext('One tenant scheduled for users sync', '%(tasks_scheduled)d tenants scheduled for users sync', tasks_scheduled)
        message = message % {'tasks_scheduled': tasks_scheduled}
        self.message_user(request, message)
