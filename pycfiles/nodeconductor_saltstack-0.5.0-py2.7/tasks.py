...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_saltstack/sharepoint/tasks.py
...
def provision(tenant_uuid, **kwargs):
    provision_tenant.si(tenant_uuid, **kwargs).apply_async(link=set_online.si(tenant_uuid), link_error=set_erred.si(tenant_uuid))
...
def destroy(tenant_uuid, force=False):
    if force:
        ...
        tenant.save()
    schedule_deletion.apply_async(args=(
     tenant_uuid,), link=delete.si(tenant_uuid), link_error=error_callback)
...
@save_error_message
def provision_tenant(tenant_uuid, transition_entity=None, site_name=None, site_description=None, template_uuid=None, phone=None, notify=None, **kwargs):
    tenant = transition_entity
    backend = tenant.get_backend()
    tenant_backend_id = 'NC_%s' % binascii.b2a_hex(os.urandom(5)).upper()
    backend.tenants.create(backend_id=tenant_backend_id, domain=tenant.domain)
    ...
    admin = User.objects.create(tenant=tenant, backend_id=backend_admin.id, admin_id=backend_admin.admin_id, password=backend_admin.password, phone=(phone or ''), **admin_data)
    tenant.admin = admin
    ...
    backend_collections_details = backend.site_collections.create_main(admin_id=admin.admin_id, name=site_name, description=site_description, template_code=template.code, storage=SiteCollection.Defaults.main_site_collection['storage'])
    main_sc = SiteCollection.objects.create(name=site_name, description=site_description, template=template, access_url=backend_collections_details.main_site_collection_url, user=admin, type=SiteCollection.Types.MAIN)
    ...
    admin_sc = SiteCollection.objects.create(name=SiteCollection.Defaults.admin_site_collection['name'], description=SiteCollection.Defaults.admin_site_collection['description'], access_url=backend_collections_details.admin_site_collection_url, user=admin, template=Template.objects.filter(code=template_code, settings=tenant.service_project_link.service.settings).first(), type=SiteCollection.Types.ADMIN)
...
def sync_tenant_users(tenant_uuid):
    ...
    for user in added_users:
        ...
        new_user.update({'backend_id': new_user.pop('id'), 
           'tenant': tenant})
