...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/pygp/gp/var_gp.py
# Compiled at: 2013-07-02 10:09:00
"""
...
class VarGP(GP):
    ...
    def _LML_Xm(self, hyperparams):
        ...
        correction_term = 1.0 / (2.0 * scipy.power(self.jitter, 2)) * scipy.sum(self.covar.Kdiag(hyperparams['covar'], self._get_x()) - scipy.diag(Q, 0))
        ...
        import pdb
        pdb.set_trace()
        rv = -0.5 * y.shape[0] * scipy.log(2 * scipy.pi)
        rv -= determinant
        rv -= 0.5 * scipy.dot(y.T, linalg.solve(Q, y))
    ...
    def _compute_sparsified_covariance(self, hyperparams):
        Xm = hyperparams['Xm']
        cross_covariance = self.covar.K(hyperparams['covar'], self._get_x(), Xm)
        sparse_covariance = self.covar.K(hyperparams['covar'], Xm)
