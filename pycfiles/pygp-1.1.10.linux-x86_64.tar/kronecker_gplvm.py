...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/pygp/gp/kronecker_gplvm.py
...
import pygp.gp.gplvm as GPLVM, pygp.optimize as OPT, pygp.priors.lnpriors as lnpriors, pygp.likelihood as lik, logging as LG, string, pickle, scipy as SP, numpy as N, copy, scipy.stats as st, pdb, copy
...
def check_dist(A, B):
    assert SP.absolute((A - B) / A).max() < 0.0001, 'outch'
...
class KroneckerGPLVM(GPLVM.GPLVM):
    ...
    \x0becY ~ GP(0, Kx \\otimes Kd + \\sigma^2 \\unit)
    ...
    def __init__(self, covar_func=None, covar_func_r=None, covar_func_c=None, likelihood=None, **kw_args):
        r"""initialize gplvm object, covar_func/covar_func_r: row covarince, covar_func_c: column covariance
        ...
        """
        assert ~(likelihood is None) and isinstance(likelihood, lik.GaussLikISO), 'only Gaussian likleihood models are supported'
    ...
    def get_covariances(self, hyperparams):
        ...
        if self._is_cached(hyperparams) and not self._active_set_indices_changed:
            ...
            Si = 1.0 / (krondiag(CC['Sr'], CC['Sc']) + Knoise[0])
            YSi = y_rot * Si
            CC['Knoise'] = Knoise
            CC['Si'] = Si
            CC['y_rot'] = y_rot
            ...
            if VERBOSE:
                ...
                SS = SS[iSS]
                UU = UU[:, iSS]
                assert SP.absolute(K - SP.dot(UU, SP.dot(SP.diag(SS), UU.T))).max() < 0.0001, 'no valid SVD'
                assert SP.absolute(SP.dot(UU, UU.T) - SP.eye(K.shape[0])).max() < 0.0001
                assert SP.absolute(SP.dot(UU.T, UU) - SP.eye(K.shape[0])).max() < 0.0001
    ...
    def _LML_covar(self, hyperparams):
        ...
        if VERBOSE:
            ...
            LMLd_ = 1.0 * SP.log(SP.linalg.cholesky(K).diagonal()).sum()
    ...
    def _LMLgrad_x(self, hyperparams):
        ...
        if 'x_r' in hyperparams:
            ...
            for i in xrange(len(self.gplvm_dimensions_r)):
                ...
                dlMl[:, i] = 0.5 * (dgradLogdetX + dgradQuadrFormX)
