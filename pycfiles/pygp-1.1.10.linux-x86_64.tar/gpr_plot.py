...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/pygp/plot/gpr_plot.py
...
"""
import pylab as PL, scipy as S_glu, matplotlib

def plot_training_data(x, y, shift=None, replicate_indices=None, format_data={'alpha': 0.5, 'marker': '.', 
   'linestyle': '--', 
   'lw': 1, 
   'markersize': 9}, draw_arrows=0, plot_old=False):
    """
    ...
    **Parameters:**
    ...
    x = S_glu.array(x)
    y = S_glu.array(y)
    x_shift = S_glu.array(x.copy())
    if shift is not None and replicate_indices is not None:
        assert len(shift) == len(S_glu.unique(replicate_indices)), 'Need one shift per replicate to plot properly'
        _format_data = format_data.copy()
        if format_data.has_key('alpha'):
            _format_data['alpha'] = 0.2 * format_data['alpha']
        else:
            _format_data['alpha'] = 0.2
        number_of_groups = len(S_glu.unique(replicate_indices))
        for i in S_glu.unique(replicate_indices):
            x_shift[(replicate_indices == i)] -= shift[i]

        for i in S_glu.unique(replicate_indices):
            col = matplotlib.cm.jet(i / (2.0 * number_of_groups))
            _format_data['color'] = col
            if plot_old:
                PL.plot(x[(replicate_indices == i)], y[(replicate_indices == i)], **_format_data)
            if draw_arrows:
                arrow_range = S_glu.where(replicate_indices == i)[0]
                for n in S_glu.arange(arrow_range[0], arrow_range[(-1)], max(1, round(len(arrow_range) / draw_arrows))):
                    offset = round((len(arrow_range) - 1) / draw_arrows)
                    n += max(int((i + 1) * offset / number_of_groups), 1)
                    PL.text((x_shift[n] + x[n]) / 2.0, y[n], '%.2f' % -shift[i], ha='center', va='center', fontsize=10)
                    PL.annotate('', xy=(x_shift[n], y[n]), xytext=(
                     x[n], y[n]), va='center', arrowprops=dict(facecolor=col, alpha=0.2, shrink=0.01, frac=0.2, headwidth=11, width=11))
...
def plot_sausage(X, mean, std, alpha=None, format_fill={'alpha': 0.1, 'facecolor': 'k'}, format_line=dict(alpha=1, color='g', lw=3, ls='dashed'), ax=None):
    """
    ...
    **Parameters:**
    ...
    if alpha is not None:
        ...
        for i, a in enumerate(alpha[:-2]):
            ...
            hf = ax.fill_between(X[i:i + 2], Y1[i:i + 2], Y2[i:i + 2], lw=0, **format_fill)

        i += 1
        hf = ax.fill_between(X[i:], Y1[i:], Y2[i:], lw=0, **format_fill)
    else:
        hf = ax.fill_between(X, Y1, Y2, **format_fill)
    hp = ax.plot(X, mean, **format_line)
    return [hf, hp]
