...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/colibri/RSD.py
# Compiled at: 2020-04-14 11:56:34
import colibri.constants as const, colibri.cosmology as cc, colibri.halo as hc, colibri.galaxy as gc, numpy as np, scipy, colibri.nonlinear as NL
...
class RSD(gc.galaxy):
    ...
    def __init__(self, z, k, mu, k_par, k_perp, code='camb', BAO_smearing=False, cosmology=cc.cosmo()):
        gc.galaxy.__init__(self, z=z, k=k, code=code, BAO_smearing=BAO_smearing, cosmology=cosmology)
        self.mu = mu
        self.k_par = k_par
        self.k_perp = k_perp
    ...
    def damping(self, x, kind='Lorentzian'):
        ...
        raise ValueError("Damping function for Fingers of God not known. Choose between 'Gaussian' and 'Lorentzian'.")

    def galaxy_RSD_Pk(self, model='nonlinear', bias=1.0, growth_rate=0.5, velocity_dispersion=300.0, fingers_of_god='Lorentzian', kind_central=None, kwargs_central={}, kind_satellite=None, kwargs_satellite={}, kwargs_mass_function={}, kwargs_concentration={}):
        r"""
        ...
        sigma = np.atleast_1d(velocity_dispersion) * (1.0 + self.z) / self.cosmology.H(self.z) / self.cosmology.h
        if model in hod_model + halo_model:
            things_to_check = [
             growth_rate, sigma]
        else:
            things_to_check = [
             bias, growth_rate, sigma]
        ...
        GROWTH_RATE = np.transpose(np.tile(growth_rate, (self.nk, self.nmu)).reshape((self.nmu, self.nk, self.nz)), (2,
                                                                                                                     1,
                                                                                                                     0))
        SIGMA = np.transpose(np.tile(sigma, (self.nk, self.nmu)).reshape((self.nmu, self.nk, self.nz)), (2,
                                                                                                         1,
                                                                                                         0))
        FoG = self.damping(x=(1.0 + ZZ) * SIGMA * KK * MU, kind=fingers_of_god)
        P_K_MU = np.zeros_like(ZZ)
        if model in linear_model:
            BIAS = np.transpose(np.tile(bias, (self.nk, self.nmu)).reshape((self.nmu, self.nk, self.nz)), (2,
                                                                                                           1,
                                                                                                           0))
            PK_BASE = np.transpose(np.tile(self.Pk['matter']['linear'], self.nmu).reshape((self.nz, self.nmu, self.nk)), (0,
                                                                                                                          2,
                                                                                                                          1))
            P_K_MU = (BIAS + GROWTH_RATE * MU ** 2.0) ** 2.0 * PK_BASE * FoG
        elif model in nonlinear_model:
            if self.k.min() > 0.001 or self.k.max() < 10.0 or len(self.k) < 100:
            ...
            BIAS = np.transpose(np.tile(bias, (self.nk, self.nmu)).reshape((self.nmu, self.nk, self.nz)), (2,
                                                                                                           1,
                                                                                                           0))
            do_nonlinear = NL.halofit_operator(z=self.z, k=self.k, pk=self.Pk['matter']['linear'], field='cb', BAO_smearing=self.BAO_smearing, cosmology=self.cosmology)
            pk_hf = do_nonlinear.pk_nl
            PK_BASE = np.transpose(np.tile(pk_hf, self.nmu).reshape((self.nz, self.nmu, self.nk)), (0,
                                                                                                    2,
                                                                                                    1))
            P_K_MU = (BIAS + GROWTH_RATE * MU ** 2.0) ** 2.0 * FoG * PK_BASE
        elif model in hod_model:
            if self.k.min() > 0.05:
                raise ValueError('To use this model you should set a self.k array such that min(k) <= 0.05 for a correct determination of galaxy bias.')
            self.load_HOD(kind_central=kind_central, kwargs_central=kwargs_central, kind_satellite=kind_satellite, kwargs_satellite=kwargs_satellite)
            ...
            bias = (self.Pk['galaxies']['real space']['total halo'] / self.Pk['matter']['total halo']) ** 0.5
            BIAS = np.transpose(np.tile(bias[:, 0], (self.nk, self.nmu)).reshape((self.nmu, self.nk, self.nz)), (2,
                                                                                                                 1,
                                                                                                                 0))
            PK_BASE = np.transpose(np.tile(self.Pk['galaxies']['real space']['total halo'], self.nmu).reshape((self.nz, self.nmu, self.nk)), (0,
                                                                                                                                              2,
                                                                                                                                              1))
            P_K_MU = (1.0 + GROWTH_RATE / BIAS * MU ** 2.0) ** 2.0 * FoG * PK_BASE
            ...
            if fingers_of_god == 'Gaussian':
                for im in xrange(self.nm):
                    for imu in xrange(self.nmu):
                        PCS[:, im, :, imu] = UNFW[:, im, :] * self.damping(x=((1.0 + ZZ) * SIGMA * KK * MU)[:, :, imu], kind=fingers_of_god)

                PSS = PCS ** 2.0
            else:
                if fingers_of_god == 'Lorentzian':
                    for im in xrange(self.nm):
                        for imu in xrange(self.nmu):
                            PCS[:, im, :, imu] = UNFW[:, im, :] * self.damping(x=((1.0 + ZZ) * SIGMA * KK * MU)[:, :, imu], kind=fingers_of_god)
                            PSS[:, im, :, imu] = UNFW[:, im, :] * self.damping(x=np.sqrt(2.0) * ((1.0 + ZZ) * SIGMA * KK * MU)[:, :, imu], kind=fingers_of_god)
                ...
                k_star = 0.01 * (1.0 + self.z)
                P_K_MU = np.zeros_like(PK_BASE)
                for iz in xrange(self.nz):
                    for ik in xrange(self.nk):
                        for im in xrange(self.nmu):
                            integrand_1h = 1.0 / n_avg[iz] ** 2.0 * dndM[iz] * (2.0 * Nc[iz] * Ns[iz] * PCS[iz, :, ik, im] + Ns[iz] ** 2.0 * PSS[iz, :, ik, im]) * M
                            integrand_2h = 1.0 / n_avg[iz] * dndM[iz] * (bias[iz] + growth_rate[iz] * self.mu[im] ** 2.0) * (Nc[iz] + Ns[iz] * PCS[iz, :, ik, im]) * M
                            P_g_1h = np.trapz(integrand_1h, dx=dlnM) * (1 - np.exp(-(self.k[ik] / k_star[iz]) ** 2.0))
                            P_g_2h = np.trapz(integrand_2h, dx=dlnM) ** 2.0 * normalization[iz] ** 2 * PK_BASE[(iz, ik, im)]
    ...
    def galaxy_RSD_Pk_multipoles(self, l=[
 0, 2, 4], model='nonlinear', bias=1.0, growth_rate=0.5, velocity_dispersion=300.0, fingers_of_god='Lorentzian', kind_central=None, kwargs_central={}, kind_satellite=None, kwargs_satellite={}, kwargs_mass_function={}, kwargs_concentration={}):
        r"""
        ...
        GROWTH_RATE = np.transpose(np.tile(growth_rate, (self.nk, nmu)).reshape((nmu, self.nk, self.nz)), (2,
                                                                                                           1,
                                                                                                           0))
        SIGMA = np.transpose(np.tile(sigma, (self.nk, nmu)).reshape((nmu, self.nk, self.nz)), (2,
                                                                                               1,
                                                                                               0))
        FoG = self.damping(x=(1.0 + ZZ) * SIGMA * KK * MU, kind=fingers_of_god)
        P_K_MU = np.zeros_like(ZZ)
        if model in linear_model:
            BIAS = np.transpose(np.tile(bias, (self.nk, nmu)).reshape((nmu, self.nk, self.nz)), (2,
                                                                                                 1,
                                                                                                 0))
            PK_BASE = np.transpose(np.tile(self.Pk['matter']['linear'], nmu).reshape((self.nz, nmu, self.nk)), (0,
                                                                                                                2,
                                                                                                                1))
            P_K_MU = (BIAS + GROWTH_RATE * MU ** 2.0) ** 2.0 * FoG * PK_BASE
        else:
            if model in nonlinear_model:
                if self.k.min() > 0.001 or self.k.max() < 10.0 or len(self.k) < 100:
                    raise ValueError('To use this model you should set a self.k array such that min(k) <= 0.001, max(k) >= 10 and with length greater than 100')
                BIAS = np.transpose(np.tile(bias, (self.nk, nmu)).reshape((nmu, self.nk, self.nz)), (2,
                                                                                                     1,
                                                                                                     0))
                do_nonlinear = NL.halofit_operator(z=self.z, k=self.k, pk=self.Pk['matter']['linear'], field='cb', BAO_smearing=False, cosmology=self.cosmology)
                pk_hf = do_nonlinear.pk_nl
                PK_BASE = np.transpose(np.tile(pk_hf, nmu).reshape((self.nz, nmu, self.nk)), (0,
                                                                                              2,
                                                                                              1))
                P_K_MU = (BIAS + GROWTH_RATE * MU ** 2.0) ** 2.0 * FoG * PK_BASE
            elif model in hod_model:
                ...
                self.halo_Pk(kwargs_mass_function=kwargs_mass_function, kwargs_concentration=kwargs_concentration)
                bias = (self.Pk['galaxies']['real space']['total halo'] / self.Pk['matter']['total halo']) ** 0.5
                BIAS = np.transpose(np.tile(bias[:, 0], (self.nk, nmu)).reshape((nmu, self.nk, self.nz)), (2,
                                                                                                           1,
                                                                                                           0))
                PK_BASE = np.transpose(np.tile(self.Pk['galaxies']['real space']['total halo'], nmu).reshape((self.nz, nmu, self.nk)), (0,
                                                                                                                                        2,
                                                                                                                                        1))
                P_K_MU = (1.0 + GROWTH_RATE / BIAS * MU ** 2.0) ** 2.0 * FoG * PK_BASE
            elif model in halo_model:
                ...
                DAMP_CS = self.damping(x=(1.0 + ZZ) * SIGMA * KK * MU, kind=fingers_of_god)
                if fingers_of_god == 'Gaussian':
                    for im in xrange(self.nm):
                    ...
                    if fingers_of_god == 'Lorentzian':
                        DAMP_SS = self.damping(x=np.sqrt(2.0) * ((1.0 + ZZ) * SIGMA * KK * MU), kind=fingers_of_god)
                        for im in xrange(self.nm):
                            for imu in xrange(nmu):
                                PCS[:, im, :, imu] = UNFW[:, im, :] * DAMP_CS[:, :, imu]
                    ...
                    for iz in xrange(self.nz):
                        for ik in xrange(self.nk):
                            for imu in xrange(nmu):
                                integrand_1h = 1.0 / n_avg[iz] ** 2.0 * dndM[iz] * (2.0 * Nc[iz] * Ns[iz] * PCS[iz, :, ik, imu] + Ns[iz] ** 2.0 * PSS[iz, :, ik, imu]) * M
                                integrand_2h = 1.0 / n_avg[iz] * dndM[iz] * (bias[iz] + growth_rate[iz] * mu[imu] ** 2.0) * (Nc[iz] + Ns[iz] * PCS[iz, :, ik, imu]) * M
                                P_g_1h = np.trapz(integrand_1h, dx=dlnM) * (1 - np.exp(-(self.k[ik] / k_star[iz]) ** 2.0))
                                P_g_2h = np.trapz(integrand_2h, dx=dlnM) ** 2.0 * normalization[iz] ** 2 * PK_BASE[(iz, ik, imu)]
            ...
            for il in xrange(nl):
                ...
                for iz in xrange(self.nz):
                    for ik in xrange(self.nk):
                        P_ell[(iz, il, ik)] = (2.0 * l[il] + 1.0) / 2.0 * scipy.integrate.simps(P_K_MU[iz, ik, :] * leg, x=mu)
    ...
    def galaxy_RSD_Pk_2D(self, model='nonlinear', bias=1.0, growth_rate=0.5, velocity_dispersion=300.0, fingers_of_god='Lorentzian', kind_central=None, kwargs_central={}, kind_satellite=None, kwargs_satellite={}, kwargs_mass_function={}, kwargs_concentration={}):
        r"""
        ...
        KK = np.sqrt(KPAR ** 2.0 + KPERP ** 2.0)
        MU = KPAR * 1.0 / KK
        GROWTH_RATE = np.transpose(np.tile(growth_rate, (self.nk_par, self.nk_perp)).reshape((self.nk_perp, self.nk_par, self.nz)), (2,
                                                                                                                                     1,
                                                                                                                                     0))
        SIGMA = np.transpose(np.tile(sigma, (self.nk_par, self.nk_perp)).reshape((self.nk_perp, self.nk_par, self.nz)), (2,
                                                                                                                         1,
                                                                                                                         0))
        FoG = self.damping(x=(1.0 + ZZ) * SIGMA * KPAR, kind=fingers_of_god)
        P_KPAR_KPERP = np.zeros_like(ZZ)
        if model in linear_model:
            BIAS = np.transpose(np.tile(bias, (self.nk_par, self.nk_perp)).reshape((self.nk_perp, self.nk_par, self.nz)), (2,
                                                                                                                           1,
                                                                                                                           0))
            ...
            P_KPAR_KPERP = (BIAS + GROWTH_RATE * MU ** 2.0) ** 2.0 * FoG * PK_BASE
        elif model in nonlinear_model:
            if self.k.min() > 0.001 or self.k.max() < 10.0 or len(self.k) < 100:
                raise ValueError('To use this model you should set a self.k array such that min(k) <= 0.001, max(k) >= 10 and with length greater than 100')
            BIAS = np.transpose(np.tile(bias, (self.nk_par, self.nk_perp)).reshape((self.nk_perp, self.nk_par, self.nz)), (2,
            ...
            self.halo_Pk(kwargs_mass_function=kwargs_mass_function, kwargs_concentration=kwargs_concentration)
            bias = (self.Pk['galaxies']['real space']['total halo'] / self.Pk['matter']['total halo']) ** 0.5
            BIAS = np.transpose(np.tile(bias[:, 0], (self.nk_par, self.nk_perp)).reshape((self.nk_perp, self.nk_par, self.nz)), (2,
                                                                                                                                 1,
                                                                                                                                 0))
            PK_BASE = np.zeros_like(ZZ)
            for iz in xrange(self.nz):
            ...
            P_KPAR_KPERP = (1.0 + GROWTH_RATE / BIAS * MU ** 2.0) ** 2.0 * FoG * PK_BASE
        elif model in halo_model:
            if self.k.min() > 0.05:
                raise ValueError('To use this model you should set a self.k array such that min(k) <= 0.05 for a correct determination of galaxy bias.')
            self.load_HOD(kind_central=kind_central, kwargs_central=kwargs_central, kind_satellite=kind_satellite, kwargs_satellite=kwargs_satellite)
                ...
                for iz in xrange(self.nz):
                    for ipar in xrange(self.nk_par):
                        for iperp in xrange(self.nk_perp):
                            integrand_1h = 1.0 / n_avg[iz] ** 2.0 * dndM[iz] * (2.0 * Nc[iz] * Ns[iz] * PCS[iz, :, ipar, iperp] + Ns[iz] ** 2.0 * PSS[iz, :, ipar, iperp]) * M
                            integrand_2h = 1.0 / n_avg[iz] * dndM[iz] * (bias[iz] + growth_rate[iz] * MU[(iz, ipar, iperp)] ** 2.0) * (Nc[iz] + Ns[iz] * PCS[iz, :, ipar, iperp]) * M
                            P_g_1h = np.trapz(integrand_1h, dx=dlnM) * (1 - np.exp(-(self.k[ik] / k_star[iz]) ** 2.0))
                            P_g_2h = np.trapz(integrand_2h, dx=dlnM) ** 2.0 * normalization[iz] ** 2 * PK_BASE[(iz, ipar, iperp)]
