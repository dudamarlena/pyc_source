...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/imageaug/functional.py
# Compiled at: 2020-01-05 20:33:00
# Size of source mod 2**32: 8856 bytes
...
def rotated_crop_distance(img, angle, crop_size):
    ...
    dw = img.width / 2.0 - cw_r / 2
    dh = img.height / 2.0 - ch_r / 2
    return (dw, dh)
...
def random_rotated_crop(img, crop_size, mean, std, downscale=0, resample=PIL.Image.NEAREST, fillcolor=None):
    ...
    rx = out.width / 2
    ry = out.height / 2
    dx = cx * cos(angle / 180.0 * pi) + cy * sin(angle / 180.0 * pi)
    dy = cy * cos(angle / 180.0 * pi) - cx * sin(angle / 180.0 * pi)
    rx += dx
    ry += dy
...
def yuv2ych(x):
    ...
    out[:, 2, :, :] = torch.atan2(x[:, 2, :, :], x[:, 1, :, :]) / pi / 2.0
    return out
...
colorspace_functions = {('rgb', 'yuv'):rgb2yuv, 
 ('rgb', 'ych'):rgb2ych, 
 ('yuv', 'ych'):yuv2ych, 
 ('yuv', 'rgb'):yuv2rgb, 
 ('ych', 'yuv'):ych2yuv, 
...
def convert(x, input_colorspace, output_colorspace, clip=True):
    if clip:
        if output_colorspace == 'rgb' or output_colorspace == 'rgba':
...
def scaled_gaussian_noise(tensor, std, mean=0.0, scale=2.0, mode='bilinear'):
    ...
    if len(tensor.shape) == 3:
        ...
        unsqueezed = False
    noise = F.interpolate(tensor, size=(int(tensor.shape[(-2)] / scale), int(tensor.shape[(-1)] / scale)), mode=mode,
      align_corners=False).normal_(mean, std)
    noise = F.interpolate(noise, size=(tensor.shape[(-2)], tensor.shape[(-1)]), mode=mode,
      align_corners=False)
    out = tensor + noise
    if unsqueezed:
        out = out.squeeze(0)
