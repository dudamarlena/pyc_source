...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dzenstatus.py
# Compiled at: 2009-11-03 12:56:53
"""
    DzenStatus
...
def load_plugins():
    ...
    (distributions, errors) = working_set.find_plugins(Environment(config.get('general', 'plugin_dir')))
...
def main_run_dzen():
    from subprocess import Popen, PIPE
    dzen_cmd = [
     config.get('general', 'dzen_path')] + config.get('general', 'dzen_opts').split()
...
def plugin_static(config):
    ...
    """
    return (
     config['name'], UPDATE_ONCE(), lambda : config['contents'])
...
def plugin_mpd(config):
    ...
    try:
        ...
        def mpd_state():
            ...
            data = {'vol_color': config['vol_color'], 'vol_icon': config['vol_icon'], 'playlist_color': config['playlist_color'], 
               'song_color': config['song_color']}
