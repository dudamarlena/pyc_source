...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/oplogutils/_trimmer.py
# Compiled at: 2010-05-30 00:51:42
import calendar
...
class Trimmer(_core.Command):
    ...
    def get_options(self):
        opts = OptionParser()
        opts.add_option('', '--remove-after', action='store', default=None, dest='remove_after', help='The time after which to delete oplog entries. (YYYY-MM-DD HH:MM:SS, in UTC).')
        opts.add_option('-y', '', action='store_true', dest='always_yes', default=None, help='Answer yes for all prompts.')
    ...
    def run(self):
        ...
        print warning
        print 'This will remove %d events from the end of the oplog after %s (UTC).' % (
    ...
    def trim(self):
        ...
        for (i, op) in izip(count(), db.tmp_oplog.find({'ts': {'$gte': ts}})):
            ...
            db.tmp_oplog.update({'ts': op['ts']}, {'$set': {'op': 'n', 'ns': ''}}, safe=True)

        conn.admin.command('renameCollection', 'local.tmp_oplog', to='local.oplog.$main')
        print 'done.'