...
# [GCC 8.4.0]
# Embedded file name: /home/theo/miniconda/lib/python2.7/site-packages/gmm_lbd/gmm.py
# Compiled at: 2015-09-21 16:55:41
...
class LbdGMM(mixture.GMM):
    ...
    def __init__(self, n_components=1, covariance_type='full', random_state=None, thresh=None, tol=0.001, min_covar=0.001, n_iter=100, n_init=1, params='wmc', init_params='wmc', verbose=0):
        super(LbdGMM, self).__init__(n_components=n_components, covariance_type=covariance_type, random_state=random_state, thresh=thresh, tol=tol, min_covar=min_covar, n_iter=n_iter, n_init=n_init, params=params, init_params=init_params)
    ...
    def bic(self, X):
        ...
        """
        return -2 * self.score(X).sum() + self._n_parameters() * 3 * np.log(X.shape[0])
    ...
    def conditional_distribution(self, x, indices=np.array([0])):
        ...
        for i, (mean, covar, weight) in enumerate(zip(self.means_, self.covars_, self.weights_)):
            ...
            expected_weights[i] = weight * multivariate_normal.pdf(x, mean=mean[indices], cov=covar[np.ix_(indices, indices)])

        expected_weights /= expected_weights.sum()
        return (
         expected_means, expected_covars, expected_weights)
    ...
    def regression(self, X=None, indices=np.array([0])):
        ...
        return (
         X[:, 0], approximed_means, approximed_covars)
