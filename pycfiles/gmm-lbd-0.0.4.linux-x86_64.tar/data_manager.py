...
# [GCC 8.4.0]
# Embedded file name: /home/theo/miniconda/lib/python2.7/site-packages/gmm_lbd/data_manager.py
# Compiled at: 2015-09-21 17:34:14
...
class GmmManager(object):
    ...
    def __init__(self, cv_types=[
 'full'], n_components_range=range(1, 30), criteria='aic'):
    ...
    def plot_regression(self, dataset_name=None, ax=None):
        ...
        ax.set_ylabel(('angles of {}').format(dataset_name))
        X = np.linspace(min(self.datasets[dataset_name][:, 0]), max(self.datasets[dataset_name][:, 0]), 500)
    ...
    def plot_bics(self, dataset_name=None, ax=None):
        ...
        for i, (cv_type, color) in enumerate(zip(self.cv_types, color_iter)):
            xpos = np.array(self.n_components_range) + 0.2 * (i - 2)
            bars.append(plt.bar(xpos, bic[i * len(self.n_components_range):(i + 1) * len(self.n_components_range)], width=0.2, color=color))

        plt.xticks(self.n_components_range)
        plt.ylim([bic.min() * 1.01 - 0.01 * bic.max(), bic.max()])
        plt.title(('{} score per model for {}').format(self.criteria.upper(), dataset_name))
        xpos = np.mod(bic.argmin(), len(self.n_components_range)) + 0.65 + 0.2 * np.floor(bic.argmin() / len(self.n_components_range))
...
class SanitizeRecordsForGmm(object):
    ...
    def plot_gmm(self, ax=None):
        ...
        for i, (mean, covar, color) in enumerate(zip(self.gmm.means_, self.gmm._get_covars(), color_iter)):
            v, w = linalg.eigh(covar)
            u = w[0] / linalg.norm(w[0])
            if not np.any(Y_ == i):
                continue
