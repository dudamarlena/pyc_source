...
# [GCC 8.4.0]
# Embedded file name: extensions/resinter.py
...
def process_residue_set(residueSet, routines, output, clean=False, neutraln=False, neutralc=False, ligand=None, assign_only=False, chain=False, debump=True, opt=True):
...
def write_all_residue_interaction_energies_combinations(routines, output, options, all_residue_combinations=False):
    ...
    for residueSet in combinationGenerator:
        count += 1
        process_residue_set(residueSet, routines, output, clean=options.clean, neutraln=options.neutraln, neutralc=options.neutralc, ligand=options.ligand, assign_only=options.assign_only, chain=options.chain, debump=options.debump, opt=options.opt)
...
def create_resinter_output(routines, outfile, options, residue_combinations=False, all_residue_combinations=False):
    """
    ...
    if residue_combinations or all_residue_combinations:
        write_all_residue_interaction_energies_combinations(routines, output, options, all_residue_combinations=all_residue_combinations)
    else:
        save_residue_interaction_energies(routines.protein.getResidues(), output)
...
def run_extension(routines, outroot, options):
    ...
    with open(outname, 'w') as (outfile):
        create_resinter_output(routines, outfile, options, residue_combinations=options.residue_combinations, all_residue_combinations=options.all_residue_combinations)