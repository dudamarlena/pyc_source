...
# [GCC 8.4.0]
# Embedded file name: /mnt/c/Users/lv_we/data/codes/tools_lib/cpmd_cube_tools/release/cpmd_cube_tools/core.py
# Compiled at: 2020-04-23 10:58:43
# Size of source mod 2**32: 15125 bytes
import os, time, scipy, pandas as pd, numpy as np
...
class cube:
    ...
    def get_mol_idx(self, inpf, alists):
        if alists == 'mol0':
            ...
            if alists.split(',')[0] != 'mol':
                return [int(i) for i in alists.split(',')]
        else:
            try:
        ...
        if alists == 'mol':
            return olp.set_index('mmid').loc[list(range(31))]['qmid'].values
        if alists.split(',')[0] == 'mol':
    ...
    def read_cube(self, fname):
        ...
        with open(fname, 'r') as (fin):
            self.filename = fname
            self.comment1 = fin.readline()
            self.comment2 = fin.readline()
            nOrigin = fin.readline().split()
            self.natoms = int(nOrigin[0])
            self.origin = np.array([float(nOrigin[1]), float(nOrigin[2]), float(nOrigin[3])])
    ...
    def vdd_analysis(self, inpf, alists):
        ...
        atomXYZ = np.array(self.atomsXYZ) + self.origin
        ...
        vddcharge += np.array([self.proton2ne(x) for x in np.array(self.atoms).astype(int)])
    ...
    def cube2vdd(self, gero, inpf):
        self.correct_cube(gero)
        vddpd, alist, vddcube = self.vdd_analysis(inpf, 'mol')
        compaidx = self.get_mol_idx(inpf, 'mol')
        print(pd.to_numeric(vddpd.loc[compaidx].charge).values.sum())
        return (pd.to_numeric(vddpd.loc[compaidx].charge).values, vddcube)

    def cube2dvdd(self, gero, inpf):
        self.correct_cube(gero)
