...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tom_lt/lt.py
# Compiled at: 2020-03-04 14:42:38
# Size of source mod 2**32: 23930 bytes
import time
...
try:
    ...
    LT_SETTINGS = {'proposalIDs':(('proposal ID1', ''), ('proposal ID2', '')), 
     'username':'', 
     'password':'', 
     'LT_HOST':'', 
     'LT_PORT':'', 
...
class LTObservationForm(GenericObservationForm):
    ...
    startdate = forms.CharField(label='Start Date', widget=forms.TextInput(attrs={'type': 'date'}))
    starttime = forms.CharField(label='Time', widget=forms.TextInput(attrs={'type': 'time'}),
      initial='12:00')
    enddate = forms.CharField(label='End Date', widget=forms.TextInput(attrs={'type': 'date'}))
    endtime = forms.CharField(label='Time', widget=forms.TextInput(attrs={'type': 'time'}),
      initial='12:00')
    max_airmass = forms.FloatField(min_value=1, max_value=3, initial=2, label='Constraints',
      widget=forms.NumberInput(attrs={'step': '0.1'}))
    max_seeing = forms.FloatField(min_value=1, max_value=5, initial=1.2, widget=forms.NumberInput(attrs={'step': '0.1'}),
      label='')
    max_skybri = forms.FloatField(min_value=0, max_value=10, initial=1, widget=forms.NumberInput(attrs={'step': '0.1'}),
      label='Sky Brightness Maximum')
    photometric = forms.ChoiceField(choices=[('clear', 'Yes'), ('light', 'No')], initial='light')

    def __init__(self, *args, **kwargs):
        (super().__init__)(*args, **kwargs)
        self.helper.layout = Layout(self.common_layout, self.layout(), self.extra_layout())
    ...
    def layout(self):
        return Div(Div(Div('project',
          css_class='form-row'),
          Div('startdate',
          'starttime', css_class='form-row'),
          Div('enddate',
          'endtime', css_class='form-row'),
          css_class='col-md-16'),
          Div(Div(css_class='col-md-2'),
          Div((PrependedText('max_airmass', 'Airmass <')),
          (PrependedAppendedText('max_seeing', 'Seeing <', 'arcsec')),
          (PrependedAppendedText('max_skybri', 'Dark + ', 'mag/arcsec²')),
    ...
    def _build_prolog(self):
        ...
        return etree.Element('RTML', {schemaLocation: LT_SCHEMA_LOCATION}, xmlns=LT_XML_NS, mode='request',
          uid=uid,
          version='3.1a',
          nsmap=namespaces)
    ...
    def _build_constraints(self):
        ...
        return [
         airmass_const, sky_const, seeing_const, photom_const, date_const]
...
class LT_IOO_ObservationForm(LTObservationForm):
    binning = forms.ChoiceField(choices=[('1x1', '1x1'), ('2x2', '2x2')], initial=('2x2',
                                                                                   '2x2'), help_text='2x2 binning is usual, giving 0.3 arcsec/pixel,                                 faster readout and lower readout noise. 1x1 binning should                                 only be selected if specifically required.')

    def __init__(self, *args, **kwargs):
        (super().__init__)(*args, **kwargs)
        self.filters = ('U', 'R', 'G', 'I', 'Z', 'B', 'V', 'Halpha6566', 'Halpha6634',
                        'Halpha6705', 'Halpha6755', 'Halpha6822')
        for filter in self.filters:
            if filter == self.filters[0]:
                self.fields['exp_time_' + filter] = forms.FloatField(min_value=0, initial=120,
                  label='Integration Time')
                self.fields['exp_count_' + filter] = forms.IntegerField(min_value=0, initial=0,
    ...
    def extra_layout(self):
        return Div(Div(Div((HTML('<br><h5>Sloan</h5>')), css_class='form_row'),
          Div(Div((PrependedAppendedText('exp_time_U', "u'", 's')), (PrependedAppendedText('exp_time_G', "g'", 's')),
          (PrependedAppendedText('exp_time_R', "r'", 's')),
          (PrependedAppendedText('exp_time_I', "i'", 's')),
          (PrependedAppendedText('exp_time_Z', "z'", 's')),
          css_class='col-md-6'),
          Div('exp_count_U', 'exp_count_G',
          'exp_count_R',
          'exp_count_I',
          'exp_count_Z',
          css_class='col-md-6'),
          ...
          Div(Div((PrependedAppendedText('exp_time_B', 'B', 's')), (PrependedAppendedText('exp_time_V', 'V', 's')),
          css_class='col-md-6'),
          Div('exp_count_B', 'exp_count_V',
          css_class='col-md-6'),
          css_class='form-row'),
          Div((HTML('<br><h5>H-alpha</h5>')), css_class='form_row'),
          Div(Div((PrependedAppendedText('exp_time_Halpha6566', '6566', 's')), (PrependedAppendedText('exp_time_Halpha6634', '6634', 's')),
          ...
          css_class='col-md-6'),
          Div('exp_count_Halpha6566', 'exp_count_Halpha6634',
          ...
          Div(Div('binning', css_class='col-md-6'),
          css_class='col'),
          css_class='form-row')
...
class LT_IOI_ObservationForm(LTObservationForm):
    exp_time = forms.FloatField(min_value=0, initial=120, label='Integration time', widget=forms.NumberInput(attrs={'step': '0.1'}))
    ...
    def extra_layout(self):
        return Div(Div(Div(Div((PrependedAppendedText('exp_time', 'H', 's')), css_class='col-md-6'),
...
class LT_SPRAT_ObservationForm(LTObservationForm):
    ...
    def extra_layout(self):
        return Div(Div((PrependedAppendedText('exp_time', 'SPRAT', 's')), css_class='col'),
...
class LT_FRODO_ObservationForm(LTObservationForm):
    exp_time_blue = forms.FloatField(min_value=0, initial=120, label='Integration time', widget=forms.NumberInput(attrs={'step': '0.1'}))
    ...
    exp_time_red = forms.FloatField(min_value=0, initial=120, label='', widget=forms.NumberInput(attrs={'step': '0.1'}))
    ...
    def extra_layout(self):
        return Div(Div((PrependedAppendedText('exp_time_blue', 'Blue Arm', 's')), (PrependedAppendedText('exp_time_red', 'Red Arm', 's')),
          css_class='col'),
    ...
    def _build_inst_schedule(self, payload):
        payload.append(self._build_schedule('FrodoSpec-Blue', str(self.cleaned_data['res_blue']), str(self.cleaned_data['exp_count_blue']), str(self.cleaned_data['exp_time_blue'])))
        payload.append(self._build_schedule('FrodoSpec-Red', str(self.cleaned_data['res_red']), str(self.cleaned_data['exp_count_red']), str(self.cleaned_data['exp_time_red'])))
...
class LTFacility(GenericObservationFacility):
    ...
    def get_form(self, observation_type):
        ...
        if observation_type == 'IOI':
            return LT_IOI_ObservationForm
        if observation_type == 'SPRAT':
            return LT_SPRAT_ObservationForm
        if observation_type == 'FRODO':
    ...
    def submit_observation(self, observation_payload):
        if LT_SETTINGS['DEBUG']:
            ...
            f.close()
            return [0]
        headers = {'Username':LT_SETTINGS['username'], 
         'Password':LT_SETTINGS['password']}
        url = '{0}://{1}:{2}/node_agent2/node_agent?wsdl'.format('http', LT_SETTINGS['LT_HOST'], LT_SETTINGS['LT_PORT'])
    ...
    def validate_observation(self, observation_payload):
        if LT_SETTINGS['DEBUG']:
            return []
        headers = {'Username':LT_SETTINGS['username'],  'Password':LT_SETTINGS['password']}
        url = '{0}://{1}:{2}/node_agent2/node_agent?wsdl'.format('http', LT_SETTINGS['LT_HOST'], LT_SETTINGS['LT_PORT'])
        client = Client(url=url, headers=headers)
        ...
        try:
            ...
            if response_rtml.get('mode') == 'offer':
                return []
            if response_rtml.get('mode') == 'reject':
                return [
