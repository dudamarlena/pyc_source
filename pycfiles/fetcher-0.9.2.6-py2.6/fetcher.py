...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\fetcher\fetcher.py
# Compiled at: 2011-11-15 00:43:21
__created__ = '2009/09/27'
__author__ = 'xlty.0512@gmail.com'
__author__ = '牧唐 杭州'
import urllib, urllib2, urlparse, cookielib, re, time, sys, os, gzip
from StringIO import StringIO
import mimetypes, logging, copy
...
class FetcherUtil:
    ...
    def get_content(res):
        ...
        len_cl = res.headers.get('Content-Length')
        if len_cl:
            len_cl = int(len_cl.strip())
        rcv = res.read()
        assert len_cl == len(rcv), '返回长度和http的协议头的声明不等'
        if ce and 'zip' in ce:
            return gzip.GzipFile(fileobj=StringIO(rcv)).read()
        return rcv

    @staticmethod
    def decode(content, to='utf-8', enc=None, default=sys.getdefaultencoding()):
        u"""
...
class FetchedData:
    ...
    def get_response_charset(self):
        ct = self.headers['Content-Type']
        print ct
        if ct:
            reps = dict([ kv.lower().strip().split('=') for kv in ct.split(';') if '=' in kv ])
            print reps
            if 'charset' in reps:
                return reps['charset']
...
class LazyIO(object):
    ...
    def get_buff_len(self, buff):
        ...
        if hasattr(buff, 'len') and hasattr(buff, 'read'):
            return buff.len
    ...
    def read(self, size_to_read=1099511627776):
...
class Fetcher(object):
    ...
    def __init__(self, timeout=3, debug=None):
    ...
    def fetch(self, url, body=None, ext_headers={}):
        ...
        try:
            ...
            ex = sys.exc_value
            if isinstance(ex, urllib2.HTTPError):
                return FetchedData(url, ex.code, hasattr(ex, 'msg') and ex.msg or '', None, None)
            return FetchedData(url, 1000, hasattr(ex, 'msg') and ex.msg or '', None, None)
...
class ImageFetcher(Fetcher):
    ...
    def fetch(self, url, body=None, ext_headers={}):
        ...
        if d.code == 200:
            title = re.findall('title.*?>([^<>]*?)</title', d.content, re.I | re.S | re.M)
            if title:
                title = title[0]
                try:
                    cs = d.get_response_charset()
                    title = FetcherUtil.decode(title, 'utf-8', cs)
                except:
                    (cs, _) = FetcherUtil.charset(d.content)
                    if cs:
                        title = title.decode(cs).encode('utf-8')

            else:
                title = None
            imgs = re.findall('img.*?src=\\s*[\'"]([^\'"<> ]*?)[\'"]', d.content, re.I | re.S | re.M)
            return (
             title, map(FetcherUtil._fix_url(url), filter(lambda u: self.p.match(u), imgs)))
        else:
            return (
             None, [])
            return
...
if __name__ == '__main__':
    ...
    d = fetcher.fetchImage('http://img.topit.me/l/201102/04/12967988733458.jpg')
    data = d.getvalue()
    f = open('x.jpg', 'wb')
    f.write(data)
    f.close()