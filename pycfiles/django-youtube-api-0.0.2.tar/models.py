...
# [GCC 8.4.0]
# Embedded file name: /Users/ramusus/workspace/movister/env/src/django-youtube-api/youtube_api/models.py
# Compiled at: 2015-09-09 17:16:38
import logging, re
...
class YoutubeModel(models.Model):
    ...
    def parse(self):
        ...
        for key, value in self._resource.items():
            if key == '_api':
                continue
            key = re.sub('([A-Z])', lambda m: '_' + m.group(0).lower(), key)
            try:
...
class YoutubeBaseModel(YoutubeModel):
    _resource = None
    fetched_at = models.DateTimeField('Fetched', blank=True)
...
class Video(YoutubeBaseModel):
    ...
    remote = VideoManager(remote_pk='video_id', methods={'get': 'videos', 
       'search': 'search'})
    ...
    def thumbnail_urls(self):
        return {'default': 'https://i.ytimg.com/vi/%s/default.jpg' % self.video_id, 
    ...
    def thumbnail_dimensions(self):
        return {'default': (120, 90), 
           'medium': (320, 180), 
           'high': (480, 360)}

    @property
    def slug(self):
        return 'watch?v=%s' % self.video_id
