...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/adminlteui/templatetags/adminlte_list.py
# Compiled at: 2020-05-05 22:21:28
# Size of source mod 2**32: 6084 bytes
import itertools
...
def adminlte_paginator_number(cl, i):
    ...
    """
    if i == DOT:
        return format_html('<li class="paginate_button"><a href="javascript:void(0);" aria-controls="example2" data-dt-idx="3" tabindex="0">… </a></li>')
    else:
        if i == cl.page_num:
            return format_html('<li class="paginate_button active"><a href="javascript:void(0);" aria-controls="example2" data-dt-idx="3" tabindex="0">{}</a></li>', i + 1)
        return format_html('<li class="paginate_button "><a href="{}" {} aria-controls="example2" data-dt-idx="3" tabindex="0">{}</a></li>', cl.get_query_string({PAGE_VAR: i}), mark_safe(' class="end"' if i == cl.paginator.num_pages - 1 else ''), i + 1)
    ...
    for choice in choices:
        ...
        for key in query_parts.keys():
            if key == field_key:
                ...
                if key.startswith(field_key + '__') or '__' + field_key + '__' in key:
                    value = query_parts[key][0]
                    matched_key = key
            if value:
...
def results(cl):
    ...
    if cl.formset:
        ...
        for res, form in zip(new_result_list, new_forms):
            yield (
             res.pk, get_parent_id(res), res.get_depth(),
...
def adminlte_result_tree(context, cl, request):
    ...
    headers.insert(1 if needs_checkboxes(context) else 0, {'text':'+', 
     'sortable':True, 
     'url':request.path, 
     'tooltip':_('Return to ordered tree'), 
     'class_attrib':mark_safe(' class="oder-grabber"')})
    return {'filtered':not check_empty_dict(request.GET), 
