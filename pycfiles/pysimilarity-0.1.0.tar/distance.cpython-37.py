...
# [GCC 8.4.0]
# Embedded file name: C:\Users\jcorporan\Documents\pysimilarity\pysimilarity\pysimilarity\distance.py
# Compiled at: 2019-09-30 16:07:49
...
class DistanceSimilarity(BaseSimilarity):
    __doc__ = 'Compute Similarity between two matrices by using the average\n       pairwise distances between their data points.\n    '
        ...
        if isinstance(distance_metric, str):
            if distance_metric != 'auto':
                if distance_metric not in METRIC_MAPPING:
                    raise ValueError(f"If distance metric is string, must be auto or one of:{str(METRIC_MAPPING)}")
    ...
    def transform(self, Y):
        ...
        print(f"Diving data into {comparison_data.shape[0] // self.batch_size + 1} batches of {self.batch_size} samples each.")
        for i in range(0, comparison_data.shape[0], self.batch_size):
            distances = cdist(comparison_data[i:i + self.batch_size], self.X, self.distance_metric)
    ...
    def _get_best_distance_metric(self, X):
        ...
        if is_unit_interval and len(unique_count) == 2:
            ...
            if is_unit_interval and sparsity_level > 0.05:
                best_metric = 'cosine'
            else:
