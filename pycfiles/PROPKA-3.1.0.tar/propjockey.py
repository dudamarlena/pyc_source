...
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/propjockey/propjockey.py
# Compiled at: 2017-11-22 17:16:41
...
def set_test_config():
    ...
    app.config['CLIENTS'] = {k:{'database': 'propjockey_test', 'collection': k} for k in ['votes', 'entries', 'workflows']}
    app.config['PASSWORDLESS']['tokenstore_client'] = {'database': 'propjockey_test', 
...
def connect_collections():
    ...
    b = Bunch()
    b.clients = {name:mongoconnect(clients_config[name]) for name in clients_config}
...
def tablerow_data((votedoc, entry, w_id), prop_missing=True):
    entry['description'] = econf['describe_entry'](entry, econf.get('description_fields', []))
    ...
    if votedoc:
        ...
        if 'user' in session:
            votedoc['votedfor'] = vconf['user_voted'](session['user'], prefilter=False, votes_doc=votedoc)
...
def find_votes(completed=False, user_only=False, sortdir=DESCENDING):
    ...
    return db.votes.find(filt, votedoc_projection(), sort=[
     (
      vconf['nvotes'], sortdir)])
...
def entries_by_filter(entry_filter, sort=None, skip=0, limit=0):
    db = get_collections()
    return db.entries.find(entry_filter, entry_projection(), sort=sort, skip=skip, limit=limit)
...
def format_rows(data):
    ...
    if fmt == 'json':
        return jsonify(data)
    if fmt != 'html':
        return ("error: Unknown response format: {}. Please choose 'json' or 'html'.").format(fmt)
    params = _rows_params()
    ...
    return render_template('index.html', prop_displayname=econf['prop_displayname'], rows=rows, params=params, extrasort_label=extrasort_label, no_more_rows=data.get('nomore'))


def _rows_params():
    user_only = request.args.get('useronly', 'false') == 'true'
    primary_sort_dir = DESCENDING if request.args.get('psort', 'decr') == 'decr' else ASCENDING
    secondary_sort_dir = ASCENDING if request.args.get('ssort', 'incr') == 'incr' else DESCENDING
    user_filter = None
    if request.args.get('filter'):
        user_filter = econf['filter']['transform'](request.args.get('filter'))
    which = set(request.args.getlist('which') or [
     'active', 'inactive_missing', 'inactive_has'])
    pagesize = request.args.get('psize', econf['rows_per_page'], type=int)
    pagenum = request.args.get('pnum', 0, type=int)
    skip = pagenum * pagesize
    return dict(user_only=user_only, primary_sort_dir=primary_sort_dir, secondary_sort_dir=secondary_sort_dir, user_filter=user_filter, which=which, pagesize=pagesize, pagenum=pagenum, skip=skip)
...
def rows():
    ...
    skip = params['skip']
    active_votedocs, active_entry_ids = votedocs_and_eids(completed=False, user_only=user_only, sortdir=primary_sort_dir)
    result = []
    if 'active' in which:
        rows = rows_active(active_votedocs, active_entry_ids, primary_sort_dir, secondary_sort_dir, user_filter=user_filter, user_only=user_only)
        if skip < len(rows):
            result += rows[skip:]
            skip = 0
        else:
    ...
    if len(result) > pagesize:
        ...
        if user_filter is None and not user_only or len(result) == 1 and user_filter and econf['e_id'] in user_filter:
            return format_rows({'rows': result, 'nomore': True})
        else:
            sort = [
             (
              econf['extrasort']['field'], secondary_sort_dir)]
            deficit = pagesize - len(result)
            limit = deficit + 1
            if user_only:
                _, completed_entry_ids = votedocs_and_eids(completed=True, user_only=user_only, sortdir=primary_sort_dir)
                e_id_constraint = {'$in': completed_entry_ids}
                prop_missing = False
                cursor = entries_inactive(e_id_constraint, user_filter, prop_missing=prop_missing, sort=sort, skip=skip, limit=limit)
            ...
            if 'inactive_missing' in which:
                prop_missing = True
                cursor = entries_inactive(e_id_constraint, user_filter, prop_missing=prop_missing, sort=sort, skip=0, limit=limit)
...
def votedocs_and_eids(completed=False, user_only=False, sortdir=DESCENDING):
    votedocs = list(find_votes(completed=completed, user_only=user_only, sortdir=sortdir))
    entry_ids = [ d[vconf['entry_id']] for d in votedocs ]
    return (
     votedocs, entry_ids)


def rows_active(active_votedocs, active_entry_ids, primary_sort_dir, secondary_sort_dir, user_filter=None, user_only=False):
    ...
    votedocs = [ d for d in active_votedocs if d[vconf['entry_id']] in entry_ids_set
               ]
    result = [ tablerow_data(z) for z in zip(votedocs, entries, workflow_ids) ]
    result.sort(key=itemgetter('extrasort'), reverse=secondary_sort_dir is DESCENDING)
    result.sort(key=itemgetter('nvotes'), reverse=primary_sort_dir is DESCENDING)
    return result
...
def rows_inactive(entries, prop_missing=True):
    if not entries:
        ...
        return [ tablerow_data(z, prop_missing=prop_missing) for z in zip(nones, entries, nones)
               ]


def entries_inactive(e_id_constraint, user_filter, prop_missing=True, sort=None, skip=0, limit=0):
...
def login():
    ...
    return render_template('login.html', remote_app={'name': pconf['remote_app_name'], 'uri': pconf['remote_app_uri']})
...
def authtoken():
    ...
    if app_id != pconf['remote_app_id'] or app_secret != pconf['remote_app_secret']:
        abort(401)
...
def _vote(user, eid, how):
    ...
    if how == 'up':
        ...
        if num_active_user_voted >= max_active:
            return (
             ('There is a maximum of {} active votes per user from this interface. Consider revoking votes for your least favorite entries.').format(max_active), ERROR)
    filt = {vconf['entry_id']: eid}
    filt.update(vconf['filter_active'])
    active_doc = db.votes.find_one(filt)
    filt_for_update = {vconf['entry_id']: eid, vconf['prop_field']: vconf['prop_value']}
    if active_doc:
        user_voted = vconf['user_voted'](user, prefilter=False, votes_doc=active_doc)
        if how == 'up' and user_voted:
            return ('cannot upvote twice', ERROR)
        if how == 'up':
            return (
             vconf['record_vote'](user, active_doc, db.votes, 'up', filt_for_update), SUCCESS)
        if how == 'down' and not user_voted:
            return ('can only downvote after upvote', ERROR)
        if how == 'down':
            return (
             vconf['record_vote'](user, active_doc, db.votes, 'down', filt_for_update), SUCCESS)
        return (
         'unknown voting operation on active entry', ERROR)
    else:
        filt = {vconf['entry_id']: eid}
        ...
        return (
         vconf['record_vote'](user, {}, db.votes, 'up', filt_for_update), SUCCESS)
...
def make_test_db():
    ...
    tdb.workflows.drop()
    tdb.workflows.insert_many([ {'eid': eid, 'wid': wid} for eid, wid in zip(entry_ids, wids) if wid
                              ])
