...
# [GCC 8.4.0]
# Embedded file name: /home/ggg/dev/repos/django-instant/instant/templatetags/instant_tags.py
# Compiled at: 2018-09-30 14:26:51
...
try:
    ...
    raise ImproperlyConfigured('The Centrifugo secret key must be set in settings.py with CENTRIFUGO_SECRET_KEY')
...
def _get_channels_for_role(path, role):
    if role == 'all':
        role_chans = CHANNELS['public'] + CHANNELS['users'] + CHANNELS['staff'] + CHANNELS['superuser']
    else:
...
def get_handlers_url(chan):
    if chan in HANDLERS:
        ...
        if DEFAULT_HANDLER is None:
            raise NoHandlerException('No handler found for channel ' + 'and no default handler is set. ' + 'Please set a default handler or ' + 'a handler for channel ' + chan)
        else:
            url = DEFAULT_HANDLER
    name = chan.replace('$', '')
    return [url, name]