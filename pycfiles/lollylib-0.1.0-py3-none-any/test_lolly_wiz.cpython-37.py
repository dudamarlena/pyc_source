...
# [GCC 8.4.0]
# Embedded file name: /Volumes/WIN/ASSETS/PROG_PYTHON/2019/lollylib/lollylib/tests/test_lolly_wiz.py
# Compiled at: 2019-06-22 02:33:06
...
class TestLollyWiz(TestCase):
    ...
    def test___init(self):
        print("***** Starting lolly_wiz tests. This will produce some expected error messages, please don't worry...")
        self._TestLollyWiz__create_test_dir_if_not_exists()
    ...
    def test_condition_processing(self):
        ...
        result = wiz._process_conditional_directives(data, 'test_data_3')
        if not (wiz.error or result == 'start.val2.end'):
            raise AssertionError
        wiz.set_definitions(['cond3'])
        result = wiz._process_conditional_directives(data, 'test_data_3')
        if not (wiz.error or result == 'start.val3.end'):
            raise AssertionError
        ...
        data = 'start.\n[##if cond1##]val1[##endif##]\n.end'
        result = wiz._process_conditional_directives(data, 'test_data_1')
        if not (wiz.error or result == 'start.\n.end'):
            raise AssertionError
        data = 'start.\r[##if cond1##]val1[##endif##]\r\n.end'
        result = wiz._process_conditional_directives(data, 'test_data_1')
        if not (wiz.error or result == 'start.\r.end'):
            raise AssertionError
    ...
    def test_check_version(self):
        ...
        str_list = [
         'LOLLYWIZ_TEXTFILE_VERSION = 1.1.0', 'test']
        wiz._check_version(str_list)
        assert wiz.error == 'version'
        str_list = [
         'LOLLYWIZ_TEXTFILE_VERSION = ', 'test']
        wiz._check_version(str_list)
        assert wiz.error == 'version'
        str_list = [
         'LOLLYWIZ_TEXTFILE_VERSION', 'test']
        ...
        assert wiz.error == 'version'
        str_list = [
         'LOLLYWIZ_TEXTFILE_VERSION = 0', 'test']
        wiz._check_version(str_list)
        assert wiz.error == 'version'
        str_list = [
         'test', 'LOLLYWIZ_TEXTFILE_VERSION = 0.0.24']
        result = wiz._check_version(str_list)
        if not (wiz.error or result == ['test']):
            raise AssertionError
        wiz.VERSION = '0.1.2'
    ...
    def test_common_replacements(self):
        ...
        if not (wiz._instr_file_data[0][0] == '2' and wiz._instr_file_data[0][1] == '0'):
            raise AssertionError
    ...
    def test_zzz_cleanup(self):
        self._TestLollyWiz__remove_test_dir()
