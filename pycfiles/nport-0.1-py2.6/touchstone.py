...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/nport/touchstone.py
# Compiled at: 2010-11-18 12:41:24
import re, os
from datetime import datetime
import numpy as np, nport, parameter
REAL_IMAG = 'RI'
MAG_ANGLE = 'MA'
DB_ANGLE = 'DB'
keys = {}
...
def read(file_path, verbose=False):
    ...
    """
    re_filename = re.compile('\\.s(?P<ports>\\d+)+p', re.I)
    m = re_filename.search(file_path)
    ports = int(m.group('ports'))
    if verbose:
        print "File '%s'" % file_path
        print '  Number of ports (based on file extension) = %d' % ports
...
def _parse_option_line(file, verbose=False):
    ...
    regex = {'unit': re.compile('(#|\\s+)(?P<unit>G|M|K|)Hz(\\s+|$)', re.I), 
       'parameter': re.compile('(#|\\s+)(?P<parameter>S|Y|Z|H|G)(\\s+|$)', re.I), 
       'format': re.compile('(#|\\s+)(?P<format>DB|MA|RI)(\\s+|$)', re.I), 
       'reference': re.compile('(#|\\s+)R\\s+(?P<reference>\\d+)(\\s+|$)', re.I)}
    prefix = {'': 1, 
       'K': 1000.0, 
       'M': 1000000.0, 
       'G': 1000000000.0}
    frequnit = 1000000000.0
...
def _get_next_line_data(file):
    ...
    """
    line = '!'
    while not _re_comment.search(line):
        if _re_options.search(line) or _re_empty.search(line):
            line = file.readline()
            raise line or EOFError
    ...
    while True:
        for i in range(len(data) / 2):
            ...
            try:
                ...
                raise Exception('more ports than reported in the file extension')
...
def write(instance, file_path, format=REAL_IMAG):
    ...
    for (i, freq) in enumerate(instance.freqs):
        ...
        for (i, element) in enumerate(parameters):
            if i != 0 and (threeport and i % 3 == 0 or not threeport and i % 4 == 0):
                sample += '\n\t\t'
            if format == REAL_IMAG:
