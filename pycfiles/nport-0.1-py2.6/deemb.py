...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/nport/deemb.py
# Compiled at: 2010-11-18 12:41:24
import nport, numpy as np
...
class Vandamme01(Deembedder):
    ...
    *IEEE Transactions on Electron Devices*, vol. 48, no. 4, pp. 737-742, 2001
    ...
    def __init__(self, open, short1, short2, through):
        ...
        self.g3 = (-y12_open.convert(nport.Z) + y12_through.convert(nport.Z)).convert(nport.Y)
    ...
    def deembed(self, measurement):
        ...
        y_a = y_meas - (np.asarray([[1, 0], [0, 0]]) * self.g1 + np.asarray([[0, 0], [0, 1]]) * self.g2)
        z_a = y_a.convert(nport.Z)
        z_b = z_a - (np.asarray([[1.0, 0], [0, 0]]) * self.z1 + np.asarray([[0, 0], [0, 1.0]]) * self.z2 + np.ones((2,
                                                                                                                    2)) * self.z3)
        y_b = z_b.convert(nport.Y)
        y_dut = y_b - np.asarray([[1.0, -1.0], [-1.0, 1.0]]) * self.g3
...
class Kolding00(Deembedder):
    ...
    def __init__(self, simple_open, simple_short, open, short1, short2, alpha=0.0):
        ...
        if kol00_asym:
            ...
            self.zi_plus_z1 = np.asarray([[1, 0], [0, 0]]) * zi1_plus_z11 + np.asarray([[0, 0], [0, 1]]) * zi2_plus_z12
            z31 = z21_o__ + z11_o__ - 2.0 * z21 - zi1_plus_z11
            z32 = z12_o__ + z22_o__ - 2.0 * z22 - zi2_plus_z12
            self.y3 = np.asarray([[1, 0], [0, 0]]) * z31.convert(nport.Y) + np.asarray([[0, 0], [0, 1]]) * z32.convert(nport.Y)
            zf1 = z31 * (z31 / (z21_o__ - z21) - 2)
            zf2 = z32 * (z32 / (z12_o__ - z22) - 2)
            self.yf = 0.5 * np.asarray([[0, 1], [1, 0]]) * (zf1 + zf2).convert(nport.Y)
        else:
    ...
    def deembed(self, measurement):
        ...
        if kol00_asym:
            ...
            z___ = z__ - (self.zi_plus_z1 * np.identity(2) + self.z2 * np.ones((2,
                                                                                2)))
            y___ = z___.convert(nport.Y)
            y_dut = y___ - (self.y3 * np.identity(2) + -self.yf * np.asarray([[0, 1], [1, 0]]))
        return y_dut.convert(nport.S)
