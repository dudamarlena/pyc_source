...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/nport/nport.py
# Compiled at: 2010-11-18 12:41:24
from __future__ import division
import numpy as np
from scipy import interpolate
import parameter, twonport
...
class NPortMatrixBase(np.ndarray):
    ...
    def __z0test__(cls, mtype, z0):
        if mtype in (S, T):
            ...
            raise ValueError('the specified n-port representation (%s) does not require a reference impedance' % mtype)
        else:
            return
        return
...
class NPortBase(NPortMatrixBase):
        ...
        def make_op(name, func):
            ...
            def op(this, other):
                if issubclass(type(other), NPortBase):
                    ...
                    result_freqs.sort()
                    result_freqs = np.array(result_freqs[result_freqs.index(min_freq):result_freqs.index(max_freq) + 1])
                    ...
                    result_z0 = this.z0
                subclass = type(this)
                return subclass(result_freqs, result_matrices, result_type, result_z0)

            op.__name__ = name
            return op

        for (name, func) in np.ndarray.__dict__.items():
            if callable(func) and name in ('__add__', '__sub__', '__mul__', '__div__',
    ...
    def __new__(cls, freqs, matrices, mtype, z0=None):
        if len(freqs) != len(matrices):
            raise ValueError('the list of frequencies and the list of matrices should have equal lenghts')
    ...
    def __getitem__(self, arg):
        if type(arg) == int:
            return NPortMatrix(np.asarray(self).__getitem__(arg), self.type, self.z0)
        else:
    ...
    def get_element(self, port1, port2):
        ...
        """
        subclass = type(self)
        return subclass(self.freqs, np.asarray(self[:, (port1 - 1,), :][:, :, (port2 - 1,)]), self.type, self.z0)
...
class NPortMatrix(NPortMatrixBase):
    ...
    def twonportmatrix(self, inports=None, outports=None):
        ...
        """
        if self.ports % 2 != 0:
            raise TypeError("this NPortMatrix' number of ports is not a multiple of 2")
        n = int(self.ports / 2)
    ...
    def convert(self, mtype, z0=None):
        ...
        if self.type == SCATTERING:
            if mtype == IMPEDANCE:
                result = self.z0 * (2 * np.linalg.inv(identity - self) - identity)
            elif mtype == ADMITTANCE:
                result = 1 / self.z0 * np.dot(np.linalg.inv(identity + self), identity - self)
            ...
            if mtype == SCATTERING:
                result = np.dot(identity - z0 * self, np.linalg.inv(identity + z0 * self))
            elif mtype == IMPEDANCE:
                result = np.linalg.inv(self)
            elif mtype == ADMITTANCE:
                raise NotImplementedError
        ...
        * port 4 is original port 6, but with reversed polarity
        ...
        for (i, ports) in enumerate(portsets):
            ...
            try:
                ...
                raise IndexError('specified port number is higher than number of ports')
...
class NPort(NPortBase):
    ...
    def twonport(self, inports=None, outports=None):
        ...
        for matrix in self:
            ...
            twonportmatrices.append(nportmatrix.twonportmatrix(inports, outports))

        return twonport.TwoNPort(self.freqs, twonportmatrices, self.type, self.z0)
...
def dot(arg1, arg2):
    ...
    if type(arg1) == NPort:
        if type(arg2) == NPort:
            ...
            result_matrices = np.asarray([ np.dot(a, b) for (a, b) in zip(arg1_matrices, arg2_matrices)
                                         ])
        else:
            result_freqs = arg1.freqs
            result_matrices = np.array([ np.dot(matrix, other) for matrix in arg1.matrices
                                       ])
        return NPort(result_freqs, result_matrices, arg1.type, arg1.z0)
    if type(arg1) == twonport.TwoNPort:
        if type(arg2) == twonport.TwoNPort:
            ...
            arg1_matrices = arg1.at(result_freqs)
            arg2_matrices = arg2.at(result_freqs)
            result_matrices = np.asarray([ twonport_dot(a, b) for (a, b) in zip(arg1_matrices, arg2_matrices)
                                         ])
        else:
            raise NotImplementedError
        return twonport.TwoNPort(result_freqs, result_matrices, arg1.type, arg1.z0)
    if type(arg2) == NPort:
        raise NotImplementedError
    raise NotImplementedError