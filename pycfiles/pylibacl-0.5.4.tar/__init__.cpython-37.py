...
# Embedded file name: /Users/sbendavi/src/pylib3/pylib3/__init__.py
# Compiled at: 2019-09-02 17:36:27
...
class ColoredStreamHandler(logging.StreamHandler):
    ...
    def format(self, record):
        ...
        level_to_color_mapper = {'INFO':'white', 
         'DEBUG':'blue', 
         'WARNING':'yellow', 
         'ERROR':'red', 
         'CRITICAL':'red'}
        color = level_to_color_mapper.get(record.levelname) or 
...
def init_logging(log_file, verbose=False, console=False):
    ...
    file_log_formatter = logging.Formatter('%(asctime)s [%(module)s] [%(levelname)s] %(message)s')
    console_log_formatter = logging.Formatter('%(asctime)s [%(process)d] [%(module)s] [%(levelname)s] %(message)s')
    if log_file:
...
def timer(func):
    ...
    def wrapper_function(*args, **kwargs):
        ...
        if elapsed_time >= 3600:
            elapsed_time /= 60
            tword = 'hours'
        print('==> {0} run {1:.2f} {2}'.format(func.__name__, elapsed_time, tword))
        return result
