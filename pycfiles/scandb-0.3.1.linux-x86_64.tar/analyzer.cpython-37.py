...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.7/site-packages/scandb/analyzer.py
...
def get_host_list_by_both(db, tcp, udp):
    conn = sqlite3.connect(db)
    cur = conn.cursor()
    sql = "SELECT distinct address FROM port WHERE port in ({seq_tcp}) and protocol = 'tcp' or port in ({seq_udp})  and protocol ='udp'".format(seq_tcp=(','.join(['?'] * len(tcp))),
...
def get_host_list_by_udp(db, udp):
    conn = sqlite3.connect(db)
    cur = conn.cursor()
    sql = "SELECT distinct address FROM port where port in ({seq_tcp}) and protocol = 'udp'".format(seq_tcp=(','.join(['?'] * len(udp))))
    cur.execute(sql, udp)
    rows = cur.fetchall()
    ips = [x[0] for x in rows]
    conn.close()
...
def get_host_list_by_tcp(db, tcp):
    conn = sqlite3.connect(db)
    cur = conn.cursor()
    sql = "SELECT distinct address FROM port where port in ({seq_tcp}) and protocol = 'tcp'".format(seq_tcp=(','.join(['?'] * len(tcp))))
    cur.execute(sql, tcp)
    rows = cur.fetchall()
    ips = [x[0] for x in rows]
    conn.close()
...
def analyzer():
    ...
    parser.add_argument('--statistics', required=False, action='store_true', default=False, help='Print statistics for each scan')
    parser.add_argument('-t', '--tcp', metavar='PORTS', required=False, type=str, default=None, help='TCP ports')
    parser.add_argument('-u', '--udp', metavar='PORTS', required=False, type=str, default=None, help='UDP ports')
    parser.add_argument('-o', '--operation', metavar='UNION|INTERSECTION', required=False, type=str, default='UNION', help='Operation to combine the sets of TCP and UDP ports (default: UNION)')
    parser.add_argument('--list', required=False, action='store_true', default=False, help='Generate a target list')
    parser.add_argument('-d', '--list-delimiter', required=False, default='\n', help='Delimiter used to separate hosts in the list output')
    parser.add_argument('--list-file', metavar='FILE', required=False, type=str, default=None, help='Generate a file with the targets instead of printing them to stdout')
