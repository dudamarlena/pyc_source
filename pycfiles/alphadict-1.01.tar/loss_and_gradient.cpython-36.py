...
# Embedded file name: /home/tom/.local/miniconda/lib/python3.6/site-packages/alphacsc/loss_and_gradient.py
...
def _assert_dtw():
    if _sdtw is None:
        raise NotImplementedError("Could not import alphacsc.other.sdtw. This module must be compiled with cython to make loss='dtw' available.")


def compute_objective(X=None, X_hat=None, z_hat=None, D=None, constants=None, reg=None, loss='l2', loss_params=dict()):
...
def compute_X_and_objective_multi(X, z_hat, D_hat=None, reg=None, loss='l2', loss_params=dict(), feasible_evaluation=True, uv_constraint='joint', return_X_hat=False):
    ...
    if feasible_evaluation:
        if D_hat.ndim == 2:
            ...
            D_hat, norm = prox_uv(D_hat, uv_constraint=uv_constraint, n_channels=n_channels,
              return_norm=True)
        else:
            D_hat = D_hat.copy()
    ...
    cost = compute_objective(X=X, X_hat=X_hat, z_hat=z_hat, reg=reg, loss=loss, loss_params=loss_params)
    if return_X_hat:
        return (cost, X_hat)
    else:
        return cost
...
def compute_gradient_norm(X, z_hat, D_hat, reg, loss='l2', loss_params=dict(), rank1=False, sample_weights=None):
    if X.ndim == 2:
        X = X[:, None, :]
        D_hat = D_hat[:, None, :]
    else:
        if rank1:
            grad_d = gradient_uv(uv=D_hat, X=X, z=z_hat, constants=None, loss=loss,
              loss_params=loss_params)
        else:
            grad_d = gradient_d(D=D_hat, X=X, z=z_hat, constants=None, loss=loss,
              loss_params=loss_params)
    grad_norm_z = 0
    for i in range(X.shape[0]):
        grad_zi = gradient_zi((X[i]), (z_hat[i]), D=D_hat, reg=reg, loss=loss,
          loss_params=loss_params)
...
def gradient_uv(uv, X=None, z=None, constants=None, reg=None, loss='l2', loss_params=dict(), return_func=False, flatten=False):
    ...
    if is_list_of_lil(z):
        ...
        if loss == 'l2':
            ...
            if loss == 'dtw':
                ...
                if loss == 'whitening':
                    cost, grad_d = _whitening_gradient_d(D=uv, X=X, z=z, loss_params=loss_params)
                else:
                    raise NotImplementedError('loss {} is not implemented.'.format(loss))
...
def gradient_zi(Xi, zi, D=None, constants=None, reg=None, loss='l2', loss_params=dict(), return_func=False, flatten=False):
    n_atoms = D.shape[0]
    if flatten:
        ...
        if loss == 'l2':
            ...
            if loss == 'dtw':
                ...
                if loss == 'whitening':
                    cost, grad = _whitening_gradient_zi(Xi, zi, D=D, loss_params=loss_params,
...
def gradient_d(D=None, X=None, z=None, constants=None, reg=None, loss='l2', loss_params=dict(), return_func=False, flatten=False):
        ...
        if is_list_of_lil(z):
            ...
            if loss == 'l2':
                ...
                if loss == 'dtw':
                    ...
                    if loss == 'whitening':
                        cost, grad_d = _whitening_gradient_d(D=D, X=X, z=z, loss_params=loss_params)
                    else:
                        raise NotImplementedError('loss {} is not implemented.'.format(loss))
        if flatten:
            grad_d = grad_d.ravel()
...
def _dtw_gradient_d(D, X=None, z=None, loss_params={}):
    ...
    return (
     cost, _dense_transpose_convolve_z(grad_X_hat, z))


def _dtw_gradient_zi(Xi, z_i, D=None, loss_params={}):
    n_channels = Xi.shape[0]
    cost_i, grad_Xi_hat = _dtw_gradient((Xi[None]), (z_i[None]), D=D, loss_params=loss_params)
    return (
     cost_i,
     _dense_transpose_convolve_d((grad_Xi_hat[0]),
       D=D, n_channels=n_channels))
...
def _l2_gradient_d(D, X=None, z=None, constants=None):
    if constants:
        ...
        return (
         None, g - constants['ztX'])
    else:
        n_channels = X.shape[1]
        residual = construct_X_multi(z, D=D, n_channels=n_channels) - X
        return (
         None, _dense_transpose_convolve_z(residual, z))
...
def _l2_objective(X=None, X_hat=None, D=None, constants=None):
    if constants:
        ...
        if D.ndim == 2:
            n_channels = constants['n_channels']
            grad_d = 0.5 * numpy_convolve_uv(constants['ztz'], D)
            grad_d -= constants['ztX']
            cost = (grad_d * D[:, None, n_channels:]).sum(axis=2)
            cost = np.dot(cost.ravel(), D[:, :n_channels].ravel())
        else:
            grad_d = 0.5 * tensordot_convolve(constants['ztz'], D)
            grad_d -= constants['ztX']
            cost = (D * grad_d).sum()
        cost += 0.5 * constants['XtX']
        return cost
    else:
...
def _whitening_gradient_zi(Xi, z_i, D, loss_params, return_func=False):
    ...
    hTh_res, func = _whitening_gradient((Xi[None]), Xi_hat, loss_params, return_func=return_func)
...
def _whitening_gradient_d(D, X, z, loss_params):
    n_channels = X.shape[1]
    X_hat = construct_X_multi(z, D=D, n_channels=n_channels)
    hTh_res, func = _whitening_gradient(X, X_hat, loss_params, return_func=False)
...
def _dense_transpose_convolve_z(residual, z):
    ...
    if is_list_of_lil(z):
        raise NotImplementedError()
    return np.sum([[[np.convolve(res_ip, (z_ik[::-1]), mode='valid') for res_ip in res_i] for z_ik in z_i] for z_i, res_i in zip(z, residual)],
      axis=0)
...
def _dense_transpose_convolve_d(residual_i, D=None, n_channels=None):
    ...
    if D.ndim == 2:
        ...
        return np.array([np.convolve(uR_ik, v_k[::-1], 'valid') for uR_ik, v_k in zip(uR_i, D[:, n_channels:])])
    else:
        return np.sum([[np.correlate(res_ip, d_kp, mode='valid') for res_ip, d_kp in zip(residual_i, d_k)] for d_k in D],
          axis=1)