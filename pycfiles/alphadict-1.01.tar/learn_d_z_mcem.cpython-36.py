...
# Embedded file name: /home/tom/.local/miniconda/lib/python3.6/site-packages/alphacsc/learn_d_z_mcem.py
...
def learn_d_z_weighted(X, n_atoms, n_times_atom, func_d=update_d_block, reg=0.1, alpha=1.9, lmbd_max='fixed', n_iter_global=10, init_tau=False, n_iter_optim=10, n_iter_mcmc=10, n_burnin_mcmc=0, random_state=None, n_jobs=1, solver_z='l-bfgs', solver_d_kwargs=dict(), solver_z_kwargs=dict(), ds_init=None, ds_init_params=dict(), verbose=0, callback=None):
    ...
    for ii in range(n_iter_global):
        if verbose > 0:
            print(('Global Iter: %d/%d\t' % (ii, n_iter_global)), end='', flush=True)
        else:
            if ii == 0:
                X_hat = np.zeros_like(X)
            else:
                X_hat = construct_X(z_hat, d_hat)
        phi, tau, loglk_mcmc = estimate_phi_mh(X,
          X_hat, alpha, phi, n_iter_mcmc, n_burnin_mcmc, random_state=rng, return_loglk=True,
          verbose=verbose)
        pobj, times, d_hat, z_hat, reg = learn_d_z(X,
          n_atoms, n_times_atom, func_d, reg=reg, lmbd_max=lmbd_max, n_iter=n_iter_optim,
          random_state=rng,
          sample_weights=(2 * tau),
          ds_init=d_hat,
          ds_init_params=ds_init_params,
