...
# Embedded file name: /home/tom/.local/miniconda/lib/python3.6/site-packages/alphacsc/utils/lil.py
...
def get_nnz_and_size(z_hat):
    if is_list_of_lil(z_hat):
        z_nnz = np.array([[len(d) for d in z.data] for z in z_hat]).sum(axis=0)
...
def init_zeros(use_sparse_z, n_trials, n_atoms, n_times_valid):
    if use_sparse_z:
        ...
        _assert_cython()
        z_hat = [sparse.lil_matrix((n_atoms, n_times_valid)) for _ in range(n_trials)]
    else:
