...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/safetynet.py
# Compiled at: 2015-03-09 13:25:35
import re, inspect, collections, abc
__all__ = [
 'typecheck',
 'List',
 'Dict',
  ...
  """
    ...
    def ListMembersOfInterest(cls, dct):
        for name, member in dct.items():
            if inspect.isfunction(member) and (not name.endswith('__') or name == '__init__'):
                yield (
                 name, member)
...
class InterfaceMeta(TypecheckMeta):
  ...
  """
    ...
    def __new__(cls, class_name, parents, dct):
        ...
        for name, member in cls.ListMembersOfInterest(dct):
            ...
            if typecheck_parent and name != '__init__':
                cls.CheckOverridenArgumentNames(class_name, member, parent_member, typecheck_parent)
                cls.CheckUndefinedPublicMethod(class_name, name, typecheck_parent.__name__, parent_member)
            dct[name] = cls.Decorate(class_name, member, parent_member)

        return abc.ABCMeta.__new__(cls, class_name, parents, dct)

    @classmethod
    def CheckOverridenArgumentNames(cls, class_name, member, parent_member, typecheck_parent):
    ...
    @classmethod
    def CheckUndefinedPublicMethod(cls, class_name, member_name, parent_name, parent_member):
...
class OptionalChecker(TypeChecker):
    ...
    def __repr__(self):
        return 'Optional[%s]' % (_FormatTypeCheck(self.subtype) if self.subtype else '')
        ...
        if not isinstance(value, collections.Mapping):
            ...
            if self.key_type is not None and self.value_type is not None:
                for key, item in value.items():
                    if self.key_type is not None and not _ValidateValue(key, self.key_type):
                        return False
                    if self.value_type is not None and not _ValidateValue(item, self.value_type):
                        return False
...
def _CollectTypeChecks(function, parent_type_check_dict, stack_location, self_name):
...
def _TypecheckFunction(function, parent_type_check_dict, stack_location, self_name):
    ...
    type_check_dict = _CollectTypeChecks(function, parent_type_check_dict, stack_location + 1, self_name)
...
def _TypecheckDecorator(subject=None, **kwargs):
  ...
  """
    if subject is None:
        ...
        if inspect.isfunction(subject) or inspect.ismethod(subject):
