...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/friendlists/__init__.py
# Compiled at: 2013-01-13 09:29:28
...
def get_fb_client(**kwargs):
    ...
    return FBClient(app.config['FB_APP_ID'], app.config['FB_APP_SECRET'], **kwargs)
...
def index(friendlist_id=None):
    ...
    listed = []
    if friendlist_id:
        listed = list(get_paged_results(fb, '%s/members' % friendlist_id))
        if request.method == 'POST':
            ...
            if removed:
                fb.graph_request('%s/members' % friendlist_id, method='delete', params=dict(members=(',').join(removed)))
            if added:
                fb.graph_request('%s/members' % friendlist_id, method='post', params=dict(members=(',').join(added)))
            return 'ok'
    friends = list(get_paged_results(fb, 'me/friends'))
    data = dict(friendlist_id=friendlist_id, friendlists=list(get_paged_results(fb, 'me/friendlists')), friends=friends, listed=listed, unlisted=[ f for f in friends if f not in listed ])
