...
# Embedded file name: c:\users\christian\documents\github\arduino_helpers\arduino_helpers\upload.py
# Compiled at: 2015-10-14 10:47:29
from serial_device import get_serial_ports
from .context import auto_context, Board, Uploader, ArduinoContext

def upload_firmware(firmware_path, board_name, port=None, arduino_install_home=None, **kwargs):
    """
    ...
    if port is None:
        if len(available_ports) == 1:
            ...
            raise IOError('No serial port was specified.  Please select one of the following ports: %s' % available_ports)
    return uploader.upload(firmware_path, port, **kwargs)


def upload(board_name, get_firmware, port=None, arduino_install_home=None, **kwargs):
    """
