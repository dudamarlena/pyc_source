...
class Item(Resource):
    __doc__ = 'Wrapper for the rdflib.resource.Resource class that allows getting property values from resources.'
...
class BurstConverter(object):
    __doc__ = 'The actual converter, that processes the chunk of lines from the CSV file, and uses the instructions from the ``schema`` graph to produce RDF.'
            ...
            for c in self.columns:
                ...
                try:
                    ...
                    if csvw_virtual == 'true':
                        ...
                        elif c.csvw_valueUrl is not None:
