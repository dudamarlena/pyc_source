...
# [GCC 8.4.0]
# Embedded file name: /home/marfcg/codes/FluVigilanciaBR/fludashboard/fludashboard/libs/views.py
# Compiled at: 2017-10-24 05:01:39
...
def compose_data_url(variables: str):
    ...
    :return:
    """
    url_var = {'data': '/data/<string:dataset>/<string:scale>', 
     'year': '<int:year>', 
     'epiweek': '<int:epiweek>', 
...
def index():
    ...
    return render_template('index.html', current_epiweek=epiweek, list_of_years=sorted(list_of_years, reverse=True), last_year=year, last_week_years=last_week_years)
...
def get_data(dataset: str, scale: str, year: int, territory_type: str):
    """
    ...
    df = df[(df.tipo == ('Estado' if territory_type == 'state' else 'Regional'))]
...
def data__weekly_incidence_curve(dataset: str, scale: str, year: int, state: str='Brasil'):
    """
    ...
    df = flu_data.get_data(dataset=dataset, scale=scale, year=year, state_name=state)
    try:
...
def data__incidence_levels(dataset: str, scale: str, year: int, epiweek: int=None,
                           state_name: str='Brasil'):
    """
    ...
    if not year > 0:
        return '[]'
    df = flu_data.get_data(dataset=dataset, scale=scale, year=year, state_name=state_name, epiweek=epiweek)
    ...
    se = pd.Series({'l0': df[(df.alert == 1)].count().l0, 
...
@app.route(compose_data_url('year/epiweek/territory_type/state_name/data-table'))
def data__data_table(dataset: str, scale: str, year: int, epiweek: int=None,
    ...
    df.sort_values(by=[
     'type_unit', 'unidade_da_federacao', 'epiyear', 'epiweek'], inplace=True)
    df.reset_index(drop=True, inplace=True)
    df.drop('type_unit', axis=1, inplace=True)
    if df.shape[0]:
        if epiweek:
            df.srag = df[['50%', '2.5%', '97.5%', 'situation']].apply(lambda row: flu_data.report_incidence(row['50%'], row['situation'], row['2.5%'], row['97.5%']), axis=1)
        else:
            df.srag = df[['srag', 'situation']].apply(lambda row: flu_data.report_incidence(row['srag'], row['situation']), axis=1)
        ...
        df.situation = df.situation.map(lambda x: situation_dict[x] if x else '')
    return '{"data": %s}' % df[ks].round({'srag': 2}).to_json(orient='records')
...
@cross_domain(origin='*')
def data__age_distribution(dataset: str, scale: str, year: int, epiweek: int=None,
                           state: str=None):
    """
    ...
    if not state:
        state = 'Brasil'
    df = pd.DataFrame(flu_data.get_data_age_sex(dataset=dataset, scale=scale, year=year, state_name=state, epiweek=epiweek)).round(2)
    df.rename(index={'0_4_anos': '0-4 anos', 
     '5_9_anos': '5-9 anos', 
