...
# [GCC 8.4.0]
# Embedded file name: /storage/Marcelo/codes/FluVigilanciaBR/fludashboard/fludashboard/libs/views.py
# Compiled at: 2018-06-21 10:51:14
# Size of source mod 2**32: 14881 bytes
from flask import render_template, Flask, Response
from ..settings import APP_AVAILABLE
from .calc_flu_alert import apply_filter_alert_by_epiweek, calc_alert_rank_whole_year
...
def compose_data_url(variables: str):
    ...
    :return:
    """
    url_var = {'data':'/data/<string:view_name>/<int:dataset_id>/<int:scale_id>', 
     'year':'<int:year>', 
     'epiweek':'<int:epiweek>', 
     'territory_type_id':'<int:territory_type_id>', 
     'territory_id':'<int:territory_id>', 
     'territory_name':'<string:territory_name>'}
    url = [url_var[v] if v in url_var else v for v in ['data'] + variables.split('/')]
    return '/'.join(url)
...
def index():
    ...
    if not APP_AVAILABLE:
        ...
        df = fluDB.read_data(table_name='current_estimated_values',
          dataset_id=1,
          scale_id=1,
          territory_id=0)
...
def get_data(view_name: str, dataset_id: int, scale_id: str, year: int, territory_type_id: int):
    """
    :param view_name:
    :param dataset_id:
    :param scale_id:
    ...
    :return:
    """
    df = fluDB.get_data(dataset_id=dataset_id,
      scale_id=scale_id,
      year=year,
      territory_type_id=territory_type_id,
      show_historical_weeks=False)
    return apply_filter_alert_by_epiweek(df, view_name).to_json(orient='records')


@app.route(compose_data_url('year/epiweek/weekly-incidence-curve'))
@app.route(compose_data_url('year/epiweek/territory_name/weekly-incidence-curve'))
def data__weekly_incidence_curve(view_name: str, dataset_id: int, scale_id: int, year: int, epiweek: int, territory_name: str='Brasil'):
    """
    :param view_name:
    :param dataset_id:
    :param scale_id:
    ...
    if not year > 0:
        ...
        df['typical_very_high'] = df[[
         'very_high_threshold', 'ci_upper', 'value', 'typical_very_high']].max().max() * 1.1
...
def data__incidence_levels(view_name: str, dataset_id: int, scale_id: int, year: int, epiweek: int=None, territory_name: str='Brasil'):
    """
    ...
    if not year > 0:
        ...
        se = pd.Series({'low_level':low_level, 
...
@app.route(compose_data_url('year/epiweek/territory_type_id' + '/territory_name/data-table'))
def data__data_table(view_name: str, dataset_id: str, scale_id: str, year: int, epiweek: int=None, territory_type_id: str=None, territory_name: str=None):
    """
    ...
    if not year > 0:
        ...
        df.sort_values(by=[
         'type_unit', 'territory_name', 'epiyear', 'epiweek'],
          inplace=True)
        ...
        if df.shape[0]:
            if epiweek:
                k = [
                 'estimated_cases', 'ci_lower', 'ci_upper', 'situation_id']
                df.value = df[k].apply((lambda row: fluDB.report_incidence(row['estimated_cases'], row['situation_id'], row['ci_lower'], row['ci_upper'])),
                  axis=1)
            else:
                df.value = df[['value', 'situation_id']].apply((lambda row: fluDB.report_incidence(row['value'], row['situation_id'])),
                  axis=1)
            ...
            df.situation_name = df.situation_name.map(lambda x: situation_dict[x] if x else '')
        return '{"data": %s}' % df[ks].round({'value': 2}).to_json(orient='records')
...
@cross_domain(origin='*')
def data__age_distribution(view_name: str, dataset_id: str, scale_id: str, year: int, epiweek: int=None, territory_name: str=None):
    """
    ...
    if not year > 0:
        ...
        if not territory_name:
            ...
            territory_id = fluDB.get_territory_id_from_name(territory_name)
            df = pd.DataFrame(fluDB.get_data_age_sex(dataset_id=dataset_id,
              scale_id=scale_id,
              year=year,
              week=epiweek,
              territory_id=territory_id)).round(2)
            df.rename(index={'years_5_9':'5-9 anos', 
...
def etiological_agents(view_name: str, dataset_id: str, scale_id: str, year: int, epiweek: int=None, territory_name: str=None):
    """
    ...
    territory_id = fluDB.get_territory_id_from_name(territory_name)
    scale_id = 2
    df = FluDB().get_etiological_data(dataset_id=dataset_id,
      scale_id=scale_id,
      year=year,
      week=epiweek,
      territory_id=territory_id)
...
def opportunities_boxplot_view(view_name: str, dataset_id: str, scale_id: str, year: int, epiweek: int=None, territory_name: str=None):
    """
    ...
    df = FluDB().get_opportunities(dataset_id=dataset_id,
      scale_id=scale_id,
      year=year,
      week=epiweek,
      territory_id=territory_id,
...
def static_data_br_states_view():
    ...
    :return:
    """
    path_states_json = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data', 'br-states.json')
