...
# [GCC 8.4.0]
# Embedded file name: /home/marfcg/codes/FluVigilanciaBR/fludashboard/fludashboard/libs/flu_data.py
...
def get_season_situation(df):
    ...
    def _fn(se):
        return df[((df.uf == se.uf) & (df.epiyear == se.epiyear))].situation.unique()[0]
...
def group_data_by_season(df, df_age_dist=None, season=None):
    ...
    l_incomplete = [
     'unknown', 'estimated', 'incomplete', 'Incompleto']
    ...
    if df_age_dist is not None:
        df_by_season = df_age_dist[[
         ...
         '40_49_anos', '50_59_anos', '60+_anos']].groupby([
         'uf', 'unidade_da_federacao', 'epiyear', 'sexo'], as_index=False).sum()
    else:
        df_by_season = df_tmp[season_basic_cols].groupby([
         'uf', 'unidade_da_federacao', 'epiyear'], as_index=False).sum()
    df_by_season['situation'] = df_by_season.apply(get_season_situation(df_tmp), axis=1)
    df_by_season_level = pd.crosstab([
     df_tmp.uf, df_tmp.unidade_da_federacao, df_tmp.epiyear], df_tmp.level).reset_index()
    df_by_season_level.columns.name = None
    for i in range(4):
        _l = 'l%s' % i
        if _l not in df_by_season_level.keys():
            df_by_season_level[_l] = 0

    df_by_season['level'] = df_by_season_level[list(level_dict.keys())].apply(get_season_level, axis=1)
...
def read_data(dataset: str='srag',
              scale: str='incidence'):
    """
    ...
    return pd.read_csv(os.path.join(path_data, 'current_estimated_values.csv'))


def prepare_data(dataset: str, scale: str, year: int=None) -> {str: pd.DataFrame}:
    """
    ...
    path_data = os.path.join(path_root, 'data')
    df_incidence = pd.read_csv(os.path.join(path_data, 'current_estimated_values.csv'))
    df_typical = pd.read_csv(os.path.join(path_data, 'mem-typical.csv'))
    df_thresholds = pd.read_csv(os.path.join(path_data, 'mem-report.csv'))
    df_incidence.escala.replace({'incidência': 'incidence', 
     'casos': 'cases'}, inplace=True)
    df_incidence = df_incidence[((df_incidence.dado == dataset) & (df_incidence.escala == scale))]
    df_typical.escala.replace({'incidência': 'incidence', 
     'casos': 'cases'}, inplace=True)
    df_typical = df_typical[((df_typical.dado == dataset) & (df_typical.escala == scale))]
    df_thresholds.escala.replace({'incidência': 'incidence', 
     'casos': 'cases'}, inplace=True)
    df_thresholds = df_thresholds[((df_thresholds.dado == dataset) & (df_thresholds.escala == scale))]
    ...
    df = pd.merge(df_incidence, df_typical, on=['uf', 'epiweek'], how='outer').merge(df_thresholds.drop(['unidade_da_federacao'], axis=1), on='uf', how='outer')
...
def get_data(dataset: str, scale: str, year: int, state_name: str=None,
             epiweek: int=0):
    """
...
def get_data_age_sex(dataset: str, scale: str, year: int, state_name: str=None,
                     epiweek: int=0):
    """
    ...
    if not state_name:
        state_name = 'Brasil'
    df_age_dist = pd.read_csv(os.path.join(path_data, 'clean_data_epiweek-weekly-incidence_w_situation.csv'), low_memory=False, encoding='utf-8')
    prepare_keys_name(df_age_dist)
    df_age_dist = df_age_dist[((df_age_dist.epiyear == season) & (df_age_dist.unidade_da_federacao == state_name))]
    if epiweek is not None and epiweek > 0:
        ...
        df = prepare_data(dataset=dataset, scale=scale, year=year)['df']
        df = df[(df.unidade_da_federacao == state_name)]
        df = group_data_by_season(df=df, df_age_dist=df_age_dist, season=season)
