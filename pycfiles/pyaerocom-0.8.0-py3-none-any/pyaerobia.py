...
# Embedded file name: /Users/chemikadze/programming/pyaerobia/.venv/lib/python2.7/site-packages/pyaerobia.py
# Compiled at: 2014-01-03 07:34:09
...
class Workout(object):
    ...
    def __repr__(self):
        return (('Workout(' + '%(id)s, %(name)s, %(date)s, %(duration)s, ' + '%(length)s, %(type)s' + ')') % self.__dict__).encode('utf-8')


class Aerobia(object):
    _CHEAT_HEADERS = {'X-Requested-With': 'XMLHttpRequest', 
       'Accept': '*/*'}
    ...
    def _workouts_url(self, user_id, page=None):
        base = urljoin(self.__root, 'users/%(user_id)s/workouts?view=list' % locals())
        if page is not None:
    ...
    def _export_url(self, workout_id, fmt):
        return urljoin(self.__root, 'export/workouts/%(workout_id)s/%(fmt)s' % locals())

    def _import_form_url(self):
        return urljoin(self.__root, 'import/files/new')

    def _import_file_url(self):
        return urljoin(self.__root, 'import/files')
    ...
    def _do_auth(self, user, password, token):
        ...
        data = urlencode({'user[email]': user, 
    ...
    def _workout_page(self, page, user_id=None):
        ...
        request = Request(url=self._workouts_url(user_id, page), headers=self._CHEAT_HEADERS)
        response = self._opener.open(request)
        soup = BeautifulSoup(response.read())
        ...
        for tr in workout_rows:
            ...
            datetime_str = ('').join(self._flatten_strings(datetime_spans[0])).strip()
            datetime_re = re.compile('(\\d+)\\s+(\\S+)\\s+(\\d+)(\\D+(\\d+)\\:(\\d+))?')
            ...
            duration_re = re.compile('((\\d+)\\D+)?(\\d+)\\D+(\\d+)')
            _, d_h, d_m, d_s = duration_re.match(duration_str).groups()
            duration = timedelta(hours=int(d_h or 0), minutes=int(d_m), seconds=int(d_s))
            length_re = re.compile('([0-9.]+)\\s+(km|км)')
            length_tds = tr.findAll(lambda el: length_re.match(unicode(el.string)))
            if len(length_tds):
    ...
    def import_workout(self, file):
        ...
        print cookies
        response = requests.post(self._import_file_url(), headers=headers, cookies=cookies, files={'workout_file[file][]': content, 
           'authenticity_token': token})
        assert response.status_code / 100 == 2
        uploaded_json = response.json()
        continue_response = self._opener.open(urljoin(self.__root, uploaded_json['continue_path']))
