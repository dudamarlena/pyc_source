...
# Embedded file name: /home/runner/work/capture-the-flag/capture-the-flag/ctf/api.py
# Compiled at: 2020-04-26 14:02:35
# Size of source mod 2**32: 17970 bytes
__doc__ = 'Catpure The Flag (Ctf) state machine and game logic.'
import numpy as np
...
class Ctf(object):
    ...
    def key(self):
        ...
        """
        key = {vars(v):k for k, v in self._key.items() if v != 'EMPTY' if v != 'EMPTY'}
        key[0] = 'EMPTY'
        return key
    ...
    def _legal_moves(self, unit):
        ...
        for move in legal_moves:
            if move == 'N':
                ...
                if isinstance(self._key[self._board[pos]], Unit):
    ...
    def legal_moves(self):
        ...
        if not hasattr(self, '_key'):
            raise error.GameNotFoundError('Call .new_game() prior to this method.')
        moves = {}
    ...
    def move(self, unit, direction):
        ...
        if not hasattr(self, '_key'):
            ...
            if unit not in self.need_to_move:
                raise error.OutOfTurnError('Unit {} is not allowed to move, only one of {} are allowed\n                to move.\n                '.format(unit, self.need_to_move))
            direction = direction.upper()
            if direction not in self.legal_moves()[unit]:
                raise error.IllegalMoveError("Direction {} is illegal for unit {}, unit {}'s legal moves\n                are {}.\n                ".format(direction, unit, unit, self.legal_moves()[unit]))
    ...
    def render(self):
        ...
        self._renderer.draw_scoreboard_logs(dims=(self._dimensions),
          score=(self._score),
