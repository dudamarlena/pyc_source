...
# Embedded file name: build/bdist.linux-x86_64/egg/caspredict/castyping.py
        ...
        if len(tmpX) >= 3:
            if best_score <= 5:
                ...
                if len(zzz) > 1:
                    ...
                    prediction = best_type
            elif sum(type_scores == best_score) > 1:
                prediction = 'Ambiguous'
                best_type = list(type_scores.index.values[(type_scores.values == np.amax(type_scores.values))])
            else:
                prediction = best_type
            ...
            if accept:
                ...
                prediction = 'False'
        outdict = {'Contig':list(tmp['Acc'])[0],  'Operon':operon, 
         'Start':start_operon, 
         'End':end_operon, 
