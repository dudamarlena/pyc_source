...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/hanzo/httptools/messaging.py
# Compiled at: 2012-01-21 22:14:19
        ...
        if text and self.mode == 'headers':
            ...
            if self.mode == 'body':
                if not self.header.has_body():
                    self.mode = 'end'
                elif self.header.body_is_chunked():
...
class HTTPHeader(object):
    STRIP_HEADERS = ('Content-Length', 'Transfer-Encoding', 'Content-Encoding', 'TE',
                     'Expect', 'Trailer')
