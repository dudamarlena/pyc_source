...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/hep_spt/adbin.py
# Compiled at: 2019-11-15 13:28:41
"""
...
class AdBin(object):
    ...
    def __init__(self, arr, range=None, weights=None):
        ...
        :param range: range of the histogram in each dimension. As         [(xmin, ymin), (xmax, ymax)].
    ...
    def contains(self, arr):
        ...
        :type arr: numpy.ndarray
        :returns: Whether the values in the input array are inside this bin or         not.
    ...
    def divide(self, ndiv=2):
        ...
        :param ndiv: number of divisions to create. For large values, this         algorithm will ask for having a low sum of weights for the first         bin, which will translate in having a long thin bin.
        :type ndiv: int
        :returns: Two new bins, supposed to contain half the sum of weights of         the parent.
        :rtype: AdBin, AdBin
        ...
        if self.array is None:
            raise RuntimeError('Attempt to call AdBin.divide on a bin whose pointers have been freed')
        ...
        bounds = np.array([ np.nextafter(sarr[(p, i)], np.infty) for i, p in enumerate(p)
                          ])
        ...
        if min_dim == np.infty:
            warnings.warn('All possible divisions have size equal to zero; not dividing', RuntimeWarning)
            return [
             self]
        else:
    ...
    def size(self):
        ...
        :returns: Size of this bin calculated as the product of         the individual sizes in each dimension.
        :rtype: float
        """
    ...
    def sw_unc(self, arr, weights=None):
        ...
        :param weights: possible weights.
        :type weights: numpy.ndarray
        :returns: Uncertainty of the sum of weights in the bin. If "weights" is         provided, this magnitude is equal to the square root of the sum of         weights in the bin. Otherwise poissonian errors are considered.
        :rtype: float
        """
        true = self.contains(arr)
        if weights is not None:
...
def adbin_hist1d(arr, nbins=100, range=None, weights=None, is_sorted=False, reduce_bias=False, **kwargs):
    ...
    :param arr: array of data.
    :type arr: numpy.ndarray
    :param nbins: number of bins. Must be greater than the length of the     input array.
    :type nbins: int
    ...
    :type is_sorted: bool
    :param reduce_bias: if set to True, the array will be processed twice     (backward and forward) to reduce any kind of bias produced.     See :func:`adbin_hist1d_edges` for more details.
    :type reduce_bias: bool
    :param kwargs: any other argument to be passed to     :func:`plotting.errorbar_hist`.
    :type kwargs: dict
    :returns: Values, edges, the distance between the center and the bounds     of the bins in X and Y errors. The shape of the errors in Y depends on     the uncertainty type passed to :func:`errorbar_hist` in "kwargs".
...
def adbin_hist1d_edges(arr, nbins=100, range=None, weights=None, is_sorted=False, reduce_bias=False):
    ...
    :param is_sorted: whether the input sample is already sorted or not.
    :type is_sorted: bool
    :param reduce_bias: if set to True, the array will be processed twice     (backward and forward) to reduce any kind of bias produced.     If no weights are provided, this is unnecessary, since the process is     deterministic.
    :returns: Edges of the histogram, with size (nbins + 1).
    :rtype: numpy.ndarray
    :raises ValueError: If the number of bins specified is greater than the     length of the input array.
    ...
    """
    if nbins > len(arr):
        raise ValueError('Number of bins can not be greater than the length of the input array')
    arr, range, weights = _proc_hist_input_1d(arr, range, weights)
    if not is_sorted:
        srt = arr.argsort()
        arr = arr[srt]
...
def adbin_hist2d_rectangles(bins, smp, range=None, weights=None, cmap=None, fill='sw', color=True, **kwargs):
    """
    ...
    :param cmap: optional color map. If "None", the default from     matplotlib.pyplot is used.
    ...
    :type color: bool
    :param kwargs: any other argument to the constructor of     :class:`matplotlib.patches.Rectangle`.
    :type kwargs: dict
    :returns: Rectangles and contents.
    :rtype: numpy.ndarray, numpy.ndarray
    """
    if fill not in ('sw', 'dens'):
        raise ValueError(('Unknown fill option "{}", choose between ("sw", "dens")').format(fill))
...
def adbin_hist(arr, nbins=100, range=None, weights=None, ndiv=2, free_memory=True):
    ...
    :type arr: numpy.ndarray
    :param range: range of the histogram in each dimension. For example,     [(xmin, ymin), (xmax, ymax)] in the 2-dimensional case.
    :type range: None or numpy.ndarray((float, float, ...), (float, float, ...)).
    :param nbins: number of bins. In this algorithm, divisions will be made     till the real number of bins is equal or greater than "nbins". If this     number is a power of "ndiv", then the real number of bins will match     "nbins".
    :type nbins: int
    :param weights: optional array of weights.
    :type weights: numpy.ndarray
    :param ndiv: see :meth:`AdBin.divide`.
    :type ndiv: int
    :param free_memory: whether to free the pointers pointing to the arrays of     data and weights in the bins.
...
def _proc_hist_input_1d(arr, range=None, weights=None):
    ...
    :param weights: optional array of weights.
    :type weights: numpy.ndarray
    :returns: Processed array of data, weights, and the minimum     and maximum values.
...
def _proc_hist_input(arr, range=None, weights=None):
    ...
    :param weights: optional array of weights.
    :type weights: numpy.ndarray
    :returns: Processed array of data, weights, and the minimum     and maximum values for each dimension.
