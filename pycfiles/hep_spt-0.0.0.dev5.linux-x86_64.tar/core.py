...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/hep_spt/stats/core.py
# Compiled at: 2019-11-15 13:34:06
"""
...
class FlatDistTransform(object):
    ...
    def __init__(self, points, values=None, kind='cubic'):
        ...
        :param kind: kind of interpolation to use. For more details see         :func:`scipy.interpolate.interp1d`.
        :type kind: str or int
        """
        srt = points.argsort()
        points = points[srt]
        if values is None:
            c = np.linspace(1.0 / len(points), 1.0, len(points))
        else:
            c = np.cumsum(values[srt])
            c *= 1.0 / c[(-1)]
        self._trans = interp1d(points, c, copy=False, kind=kind, bounds_error=False, fill_value=(0,
                                                                                                 1))
        return
...
def stat_values(arr, axis=None, weights=None):
    ...
    :type weights: None or numpy.ndarray
    :returns: Mean, variance, standard deviation, variance of the mean and     standard deviation of the mean.
    :rtype: numpy.ndarray, numpy.ndarray, numpy.ndarray, numpy.ndarray, numpy.ndarray
    """
    keepdims = axis is not None

    def asum(a):
        return np.sum(a, axis=axis, keepdims=keepdims, dtype=float)
    ...
    if weights is None:
        ...
        if keepdims:
            ...
            lgth = arr.size
        var = asum((arr - mean) ** 2) / (lgth - 1.0)
        var_mean = var / lgth
    else:
        sw = asum(weights)
        nzw = asum(weights != 0)
        mean = asum(weights * arr) / sw
        var = asum(weights * (arr - mean) ** 2) * nzw / (sw * (nzw - 1.0))
        var_mean = var / nzw
    std = np.sqrt(var)
