...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/hep_spt/stats/poisson.py
# Compiled at: 2019-11-15 13:33:31
"""
...
def calc_poisson_fu(m, cl=one_sigma):
    ...
    if m < 1:
        ...
        def fleft(l):
            return 1.0 - (poisson.cdf(m, l) - poisson.pmf(m, l)) - alpha

        lw = fsolve(fleft, il)[0]

    def fright(l):
        return poisson.cdf(m, l) - alpha
...
def calc_poisson_llu(m, cl=one_sigma):
    ...
    def nll(x):
        return -2.0 * np.log(poisson.pmf(m, x))
...
def _process_poisson_unc(m, lw, up):
    ...
    if any(s < 0 for s in (s_lw, s_up)):
        warnings.warn('Poisson uncertainties have been incorrectly calculated')
    return (
     float(s_lw), float(s_up))
