...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/hep_spt/plotting.py
# Compiled at: 2019-11-15 13:30:21
"""
...
import math, matplotlib, matplotlib.pyplot as plt, matplotlib.ticker as ticker, numpy as np, os, warnings
__all__ = [
 'available_styles',
 'corr_hist2d',
 'modified_format',
...
def available_styles():
    ...
    :rtype: list(str)
    """
    available_styles = list(map(lambda s: s[:s.find('.mplstyle')], os.listdir(PATH_TO_STYLES)))
...
def corr_hist2d(matrix, titles, frmt='{:.2f}', vmin=None, vmax=None, cax=None):
    ...
    :param frmt: format to display the correlation value in each bin. By     default it is assumed that the values go between :math:`[0, 1]`, so     three significant figures are considered. If "frmt" is None, then     no text is displayed in the bins.
    :type frmt: str
...
def modified_format(kwargs):
    ...
    :param kwargs: dictionary with the format to be applied to     :any:`matplotlib.rcParams`.
    :type kwargs: dict
    """
...
def samples_cycler(smps, *args, **kwargs):
    ...
    nc = len(cyc)
    if ns > nc:
        warnings.warn('Not enough plotting styles in cycler, some samples might have the same style.', RuntimeWarning)
        l = ns // nc + bool(ns % nc)
        re_cyc = (l * cyc)[:ns]
    else:
        re_cyc = cyc[:ns]
...
def text_in_rectangles(recs, txt, cax=None, **kwargs):
    ...
    :type txt: list(str)
    :param cax: axes where the rectangles are being drawn. If None, then the     current axes are taken.
    :type cax: matplotlib.axes.Axes
