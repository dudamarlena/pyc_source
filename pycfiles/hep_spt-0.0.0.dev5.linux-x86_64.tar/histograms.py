...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/hep_spt/histograms.py
# Compiled at: 2019-11-15 13:29:32
"""
...
def errorbar_hist(arr, bins=20, range=None, weights=None, norm=False, norm_type='range', uncert=None):
    ...
    :param norm: if True, normalize the histogram. If it is set to a number,     the histogram is normalized and multiplied by that number.
    :type norm: bool, int or float
    :param norm_type: the normalization type to apply. To be chosen between     'range' and 'all'. In the first case (default), only the points lying     inside the histogram range are considered in the normalization. In the     later, all the points are considered. In this case, the sum of the     values returned by this function will not sum the value provided in "norm".
    :type norm_type: str
    :param uncert: type of uncertainty to consider. If None, the square root     of the sum of squared weights is considered. The possibilities     are:     - "freq": frequentist uncertainties.     - "dll": uncertainty based on the difference on the logarithm of     likelihood.     - "sw2": sum of square of weights. In case of non-weighted data, the     uncertainties will be equal to the square root of the entries in the bin.
    :type uncert: str or None
    :returns: Values, edges, the spacing between bins in X the Y errors.     In the non-weighted case, errors in Y are returned as two arrays, with the     lower and upper uncertainties.
    ...
    :raises TypeError: if the uncertainty is "freq" or "dll" and "weights" is     not of integer type.
...
def process_range(arr, range=None):
    ...
    :param range: range of the histogram. It must contain tuple(min, max),     where "min" and "max" can be either floats (1D case) or collections     (ND case).
...
def profile(x, y, bins=20, range=None, weights=None, std_type='mean'):
    ...
    :type weights: None or numpy.ndarray(value-type)
    :param std_type: type of standard deviation to be returned. The default     ('mean') makes this function return the standard deviation of the mean.     If set to 'sample', it will return the standard deviation of the sample     instead. The definitions can be seen in :func:`stat_values`.
    :type std_type: str
...
def pull(vals, err, ref, ref_err=None):
    ...
    :param err: array of errors. Both symmetric and asymmetric errors     can be provided. In the latter case, they must be provided as a     (2, n) array.
    ...
    :returns: Pull of the values with respect to the reference and     associated errors. In case asymmetric errors have been provided,     the returning array has shape (2, n).
    :rtype: numpy.ndarray, numpy.ndarray
...
def residual(vals, err, ref, ref_err=None):
    ...
    :param ref_err: possible errors for the reference.
    :type ref_err: numpy.ndarray
    :returns: Residual of the values with respect to the reference and     associated errors. In case asymmetric errors have been provided,     the returning array has shape (2, n).
    :rtype: numpy.ndarray, numpy.ndarray
    :raises TypeError: if any of the error arrays does not have shape (2, n) or (n,).
