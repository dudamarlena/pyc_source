...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/hep_spt/stats/binomial.py
# Compiled at: 2019-11-15 13:30:43
"""
...
__all__ = [
 'clopper_pearson_int', 'clopper_pearson_unc',
 'wald_int', 'wald_unc',
...
def clopper_pearson_int(k, N, cl=one_sigma):
    ...
    if k.ndim == 0:
        ...
        if pcl.ndim == 0:
            ...
            upcl = pcl[cd]
        uk, uN = k[cd], N[cd]
        up[cd] = beta(uk + 1, uN - uk).ppf(1.0 - upcl)
    return (lw, up)
...
def wilson_int(k, N, cl=one_sigma):
    ...
    z2 = z * z
    c = p + z2 * 0.5 * iN
    s = z * iN * np.sqrt(p * (1.0 - p) * N + 0.25 * z2)
    d = 1.0 + z2 * iN
    p_l = (c - s) / d
    p_u = (c + s) / d
    zero = np.isclose(p, 0.0)
    one = np.isclose(p, 1.0)
    if p_l.ndim != 0:
        p_l[zero] = 0.0
    elif zero:
        p_l = 0.0
