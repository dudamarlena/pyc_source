...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/hep_spt/cls.py
# Compiled at: 2019-11-15 13:29:02
"""
...
class CLsTS_continuous(CLsTS):
    ...
    def _cl(self, hyp, tv, size):
        ...
        srt = ts.argsort()
        ts = ts[srt]
        cs = np.linspace(1.0 / size, 1.0, size)
        idx = np.searchsorted(ts, tv)
        if idx.ndim != 0:
            idx[(idx == size)] -= 1
        elif idx == len(values):
...
class CLsHypo(object):
    ...
    def __init__(self, pf):
        ...
        :ivar func: probability density/mass function representing the         hypothesis.
