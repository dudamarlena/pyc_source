...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/schevo/store/file_storage.py
# Compiled at: 2007-03-21 14:34:41
"""
$URL: svn+ssh://svn.mems-exchange.org/repos/trunk/durus/file_storage.py $
$Id: file_storage.py 2633 2006-10-10 15:35:33Z mscott $
"""
import sys
...
if os.name == 'posix':
    ...
    def lock_file(fp):
        win32file.LockFileEx(win32file._get_osfhandle(fp.fileno()), win32con.LOCKFILE_EXCLUSIVE_LOCK | win32con.LOCKFILE_FAIL_IMMEDIATELY, 0, -65536, pywintypes.OVERLAPPED())


    def unlock_file(fp):
        win32file.UnlockFileEx(win32file._get_osfhandle(fp.fileno()), 0, -65536, pywintypes.OVERLAPPED())
...
class FileStorage(Storage):
    ...
    def __init__(self, filename=None, readonly=False, repair=False, fp=None):
        ...
        if fp is not None:
            ...
            if not filename:
                raise ValueError('A filename is required for a readonly storage.')
            if repair:
            ...
            elif os.path.exists(self.filename) and os.stat(self.filename).st_size > 0:
    ...
    def end(self, handle_invalidations=None):
        ...
        if self.fp is None:
            raise IOError, 'storage is closed'
        index = {}
        for z in self._write_transaction(self.fp, self._generate_pending_records(), index):
            pass
    ...
    def _packer(self):
        if self.filename:
            ...
            packed = NamedTemporaryFile(suffix='.durus', mode='w+b')
        lock_file(packed)
        ...
        index = {}
        for z in self._write_transaction(packed, gen_reachable_records(), index):
            yield None
...
class FileStorage1(FileStorage):
            ...
            try:
                ...
                if self.fp.tell() > transaction_offset:
                    if not self.repair:
                        raise
                    if self.fp.mode == 'r':
                        raise RuntimeError("Can't repair readonly file.\n%s" % exc)
    ...
    def load(self, oid):
        return FileStorage.load(self, oid)[8:]
