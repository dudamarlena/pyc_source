...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/schevo/query.py
...
class Exact(Param):
    ...
    def __unicode__(self):
        ...
        if criteria:
            ...
            criteria = [ (label(field_spec[name]), unicode(self.f[name])) for name in criteria ]
            criteria = (', ').join(('%s == %s' % (field_label, value_label) for (field_label, value_label) in criteria))
            return '%s where (%s)' % (plural(extent), criteria)
        else:
            return 'all %s' % plural(extent)
...
class Links(Query):
    ...
    def __call__(self):
        return results(self._entity.sys.links(self._other_extent, self._other_field_name))
...
o_any = MatchOperator('any', 'is anything')
o_assigned = MatchOperator('assigned', 'has a value')
o_unassigned = MatchOperator('unassigned', 'has no value')
o_eq = MatchOperator('eq', '==', operator.eq)
o_in = MatchOperator('in', 'in', operator.contains)
o_le = MatchOperator('le', '<=', operator.le)
o_lt = MatchOperator('lt', '<', operator.lt)
o_ge = MatchOperator('ge', '>=', operator.ge)
o_gt = MatchOperator('gt', '>', operator.gt)
o_ne = MatchOperator('ne', '!=', operator.ne)
...
o_contains = MatchOperator('contains', 'contains', _contains)
...
o_startswith = MatchOperator('startswith', 'starts with', _startswith)
o_aliases = {'==': o_eq, '<=': o_le, '<': o_lt, '>=': o_ge, '>': o_gt, '!=': o_ne, 'eq': o_eq, 'le': o_le, 'lt': o_lt, 'ge': o_ge, 'gt': o_gt, 'ne': o_ne, 'any': o_any, 'assigned': o_assigned, 'contains': o_contains, 'in': o_in, 'startswith': o_startswith, 'unassigned': o_unassigned}

class Match(Query):
    """Field match query."""
    __module__ = __name__

    def __init__(self, on, field_name, operator=o_eq, value=None, FieldClass=None):
    ...
    def __call__(self):
        ...
        if operator is o_in:
            ...
            return results((obj for obj in on if getattr(obj, field_name) in value))
        else:
            if operator is o_any:
                return results(on)
            elif operator is o_assigned:
                return results((obj for obj in on if getattr(obj, field_name) is not UNASSIGNED))
            elif operator is o_unassigned:
                if isinstance(on, base.Extent):
                    kw = {field_name: UNASSIGNED}
                    return results(on.find(**kw))
                else:
                    return results((obj for obj in on if getattr(obj, field_name) is UNASSIGNED))
    ...
    def __unicode__(self):
        ...
        if isinstance(on, base.Extent):
            ...
            on_label = unicode(on)
        s = '%s where %s %s' % (on_label, label(field), label(self.operator))
...
class Intersection(Query):
    ...
    def __unicode__(self):
        ...
        last_on = None
        for query in self.queries:
            if isinstance(query, Match) and isinstance(query.on, base.Extent) and (query.on is last_on or not last_on) and query.operator is o_any:
                last_on = query.on
                continue
            return 'the intersection of (%s)' % (', ').join((unicode(query) for query in self.queries))

        return 'all %s' % plural(last_on)
    ...
    def remove_match(self, field_name):
        ...
        raise schevo.error.FieldDoesNotExist('%r not found in %r' % (field_name, self))
...
class Union(Query):
    ...
    def __unicode__(self):
        return 'the union of (%s)' % (', ').join((unicode(query) for query in self.queries))
...
class Group(Query):
    ...
    def __unicode__(self):
        ...
        return '%s, grouped by %s' % (self.query, label(field))
...
class Min(Query):
    ...
    def __unicode__(self):
        field = self.FieldClass(self, self.field_name)
        return 'results that have the minimum %s in each (%s)' % (label(field), self.query)
...
class Max(Query):
    ...
    def __unicode__(self):
        field = self.FieldClass(self, self.field_name)
        return 'results that have the maximum %s in each (%s)' % (label(field), self.query)
