...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/schevo/store/connection.py
# Compiled at: 2007-03-21 14:34:41
"""$URL: svn+ssh://svn.mems-exchange.org/repos/trunk/durus/connection.py $
$Id: connection.py 2636 2006-10-10 15:37:06Z mscott $
"""
import sys
...
class Cache(object):
    ...
    def shrink(self, connection):
        ...
        if current <= self.size:
            log(10, '[%s] cache size %s recent %s', getpid(), current, len(self.recent_objects))
            return
        start_time = time()
        ...
        log(10, '[%s] shrink %fs removed %s ghosted %s size %s recent %s', getpid(), time() - start_time, current - len(self.objects), num_ghosted, len(self.objects), len(self.recent_objects))
        return
