...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/schevo/transaction.py
...
from schevo.error import DatabaseMismatch, DeleteRestricted, TransactionExpired, TransactionFieldsNotChanged, TransactionNotExecuted
...
class Transaction(base.Transaction):
    ...
    def _undo(self):
        ...
        if not self._executed:
            raise TransactionNotExecuted('A transaction must be executed before its undo transaction is requested.')
        return Inverse(self)
...
class Create(Transaction):
    ...
    def _execute(self, db):
        ...
        for (field_name, field) in self._field_map.iteritems():
            entity = field._value
            if isinstance(entity, base.Entity):
                if entity.sys.db is not db:
                    resolved = False
                    if entity._default_key is not None:
                        ...
                        if hasattr(db, extent_name):
                            ...
                            criteria = dict([ (name, getattr(entity, name)) for name in entity._default_key ])
                            value = extent.findone(**criteria)
                            if value is not None:
                                field._value = value
                                resolved = True
                    raise resolved or DatabaseMismatch(msg % (field_name, entity))
...
class Delete(Transaction):
    ...
    def _execute(self, db):
        ...
        if entity._rev != self._rev:
            raise TransactionExpired('Original entity revision was %i, is now %i' % (self._rev, entity._rev))
        ...
        for ((e_name, f_name), others) in entity.sys.links().iteritems():
            ...
            for other in others:
                e_o = (
                 EntityClass, other.sys.oid)
                if e_o in traversed or e_o in deletes:
                    continue
                traversed.add(e_o)
                field = getattr(other.f, f_name)
                on_delete = field.on_delete.get(entity.__class__.__name__, field.on_delete_default)
                if on_delete is CASCADE:
                    ...
                    if e_o in self._known_deletes:
                        continue
                    raise DeleteRestricted('%r cannot be deleted; it is referred to by %r.%s' % (entity, other, f_name))
...
class Inverse(Transaction):
    ...
    def __init__(self, original_tx):
        ...
        if not original_tx._executed:
            raise TransactionNotExecuted('Transaction must be executed before inverse can be determined.')
        self._original_tx = original_tx
        self._label = 'Inverse of %s' % label(original_tx)
...
class _Populate(Transaction):
    ...
    def _execute(self, db):
        ...
        def process_data(extent):
            ...
            for values in data:
                value_map = {}
                for (field_name, value, FieldClass) in zip(field_names, values, field_classes):
                    if value is not DEFAULT:
                        value = resolve(field_name, value, FieldClass, field_names)
        ...
        def resolve(field_name, value, FieldClass, field_names):
            if issubclass(FieldClass, field.Entity) and not isinstance(value, base.Entity) and value is not UNASSIGNED:
                allow = FieldClass.allow
                if len(allow) > 1:
                ...
                if isinstance(value, dict):
                    ...
                    for key_field_name in default_key:
                        ...
                        v = resolve(key_field_name, kw[key_field_name], FClass, default_key)
                        kw[key_field_name] = v

                else:
                    msg = 'value %r is not valid for field %r in %r' % (value, field_name, field_names)
                    raise TypeError(msg)
        ...
        if self._extent_names:
            extents = [ extent for extent in db.extents() if extent.name in self._extent_names ]
        else:
            extents = db.extents()
        self._extents = extents
        priority_extents = reversed(sorted(((getattr(extent._EntityClass, priority_attr, 0), extent) for extent in extents)))
