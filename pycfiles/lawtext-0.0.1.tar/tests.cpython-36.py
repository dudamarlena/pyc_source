...
# [GCC 8.4.0]
# Embedded file name: c:\Users\m\dev\lawtext\lawtext\tests.py
# Compiled at: 2017-12-14 09:31:46
# Size of source mod 2**32: 23152 bytes
...
class AbstractTest(unittest.TestCase, metaclass=ABCMeta):
    LAWS = [dict(zip(['law_id', 'law_num', 'law_name'], values)) for values in (('359AC0000000086', '昭和五十九年法律第八十六号', '電気通信事業法'),
    ...
    def _compare_xml(self, lines_1, lines_2, lines, law_num, filename1='reference', filename2='output'):
        ...
        if conc1 == conc2:
            return
        conc1 = re.sub(' Num="[\\d:_]+?"', '', conc1)
        conc2 = re.sub(' Num="[\\d:_]+?"', '', conc2)
        if conc1 == conc2:
            out_text = '\n'.join(lines).rstrip()
            print(f"###\n### [Num] mismatch\n###\n\n*** {filename1}\n--- {filename2}\n***************\n\n{out_text}\n")
            return
        ...
        if conc1 == conc2:
            ...
            print(f"###\n### [Type] mismatch\n###\n\n*** {filename1}\n--- {filename2}\n***************\n\n{out_text}\n")
            return
        conc2 = re.sub('<AppdxTableTitle(?P<appdx_table_attr>[^>]*?)>(?P<appdx_table_text>.*)</AppdxTableTitle>.*?<RelatedArticleNum>(?P<related_article_num_text>.*)</RelatedArticleNum>',
          '<AppdxTableTitle\\g<appdx_table_attr>>\\g<appdx_table_text>\\g<related_article_num_text></AppdxTableTitle>',
          conc2,
          flags=(re.DOTALL))
        if conc1 == conc2:
            out_text = '\n'.join(lines).rstrip()
            ...
            return
        conc1 = re.sub('</Column>\\s*<Column[^>]*?>', '\u3000', conc1)
        conc1 = re.sub('。</Sentence>\\s*<Sentence[^>]*?>', '。', conc1)
        conc1 = re.sub('</Sentence>\\s*<Sentence[^>]*?>', '\u3000', conc1)
        conc2 = re.sub('</Column>\\s*<Column[^>]*?>', '\u3000', conc2)
        conc2 = re.sub('。</Sentence>\\s*<Sentence[^>]*?>', '。', conc2)
        conc2 = re.sub('</Sentence>\\s*<Sentence[^>]*?>', '\u3000', conc2)
        if conc1 == conc2:
            out_text = '\n'.join(lines).rstrip()
    ...
    def _request(self, url):
        import requests
        response = requests.get(url, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/62.0.3202.94 Safari/537.36'})
    ...
    def _fail_diff_out_text(self, law_name, law_num, out_text, filename1='reference', filename2='output'):
        self.fail(f"Mismatching Output:\n{law_name}\n（{law_num}）\n\n*** {filename1}\n--- {filename2}\n***************\n\n{out_text}")
    ...
    def test_render_htmlfragment_with_elaws(self):
        ...
        for law in self.LAWS:
            ...
            with (self.subTest)(**law):
                ...
                rendered_text = re.sub('\\[.+?\\]', '[別画面で表示]', rendered_text)
                print('  Comparing ...', file=(sys.stderr))
                out_text = self._get_diff_out_text((self._get_reference_text(law_id).split()),
                  (rendered_text.split()),
                  law_num,
                  type_='text')
                if out_text:
    ...
    def test_render_docx_with_elaws(self):
        ...
        for law in self.LAWS:
            ...
            with (self.subTest)(**law):
                ...
                rendered_document = re.sub('<(?P<slash>/?)w:(?P<tag>\\S+?)(?P<attr>[^>]*?)>', '<\\g<slash>w_\\g<tag>\\g<attr>>', rendered_document)
                ...
                rendered_texts = [subtext for el in rendered_el.findall('.//w_t') for subtext in el.text.split()]
                rendered_text = '\n'.join(rendered_texts)
                rendered_text = re.sub('（\\s*(?P<body>\\S+)\\s*）', '（\\g<body>）', rendered_text)
                rendered_text = re.sub('\\[.+?\\]', '[別画面で表示]', rendered_text)
                self.assertNotEqual(rendered_text, '')
                print('  Comparing ...', file=(sys.stderr))
                out_text = self._get_diff_out_text((self._get_reference_text(law_id).split()),
    ...
    def test_render_lawtext_with_elaws(self):
        ...
        for law in self.LAWS:
            ...
            with (self.subTest)(**law):
                ...
                rendered_text = re.sub('^\\s*(?:\\* )?- (?:\\[\\S+?\\])*(.*)$',
                  '\\1',
                  rendered_text,
                  flags=(re.M))
                rendered_text = re.sub('(?:</?Ruby>)|(?:</?Rt>)',
                  '',
                  rendered_text,
                  flags=(re.M))
                rendered_text = re.sub('^(\\s*)\\.\\.(?:\\s*)figure::(?:\\s*)(?:.+)$',
                  '\\1[別画面で表示]',
                  rendered_text,
                  flags=(re.M))
                self.assertNotEqual(rendered_text, '')
    ...
    def test_parse_lawtext_with_elaws(self):
        ...
        for law in self.LAWS:
            ...
            with (self.subTest)(**law):
                ...
                print(f"    parse:    {(t1 - t0) * 1000000 / lines_count:3,.0f} μs/line  =  {(t1 - t0) * 1000:5,.0f} ms / {lines_count:,} lines")
                print(f"    decorate: {(t2 - t1) * 1000000 / lines_count:3,.0f} μs/line  =  {(t2 - t1) * 1000:5,.0f} ms / {lines_count:,} lines")
                ...
                with io.StringIO() as (buffer):
                    sys.stdout = buffer
                    out_text = self._get_diff_out_text((web_outtext.splitlines()),
                ...
                if warning_outtext:
                    ...
                    print('    ### Some warning detected !!!', file=(sys.stderr))
                    print(f'    ### Please see "{str(warning_out_path)}"', file=(sys.stderr))
                    print('    ###', file=(sys.stderr))
                    warning_out_path.write_text(warning_outtext, encoding='utf-8')
                if out_text:
                    self._fail_diff_out_text(law_name,
                      law_num, out_text, filename1=(str(web_out_path)),
                      filename2=(str(parsed_out_path)))