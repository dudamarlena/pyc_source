...
# [GCC 8.4.0]
# Embedded file name: C:\Users\m\dev\lawtext\lawtext\htmlapp.py
# Compiled at: 2017-12-24 11:04:00
# Size of source mod 2**32: 3471 bytes
import argparse, locale, os, shutil, subprocess, sys
...
def main(outdir, preserve_compiled_files, ie=False):
    ...
    if outdir.exists():
        rmtree(str(outdir))
    copy_tree(str(Path(__file__).parent.resolve() / 'htmlapp_templates'), str(outdir))
    shutil.copyfile(str(Path(__file__).parent.resolve() / 'static' / 'law.css'), str(Path(outdir / 'src' / 'law.css')))
    ...
    if ie:
        ...
        pd_text = pd_text.replace('new RegExp(pattern, jsFlags)', 'new RegExp(pattern, jsFlags.replace("u", ""))')
