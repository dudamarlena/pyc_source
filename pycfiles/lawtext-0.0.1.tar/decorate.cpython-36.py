...
# [GCC 8.4.0]
# Embedded file name: c:\Users\m\dev\lawtext\lawtext\decorate.py
# Compiled at: 2017-12-13 18:14:42
# Size of source mod 2**32: 10554 bytes
import re
re_JpNum = re.compile('^(?P<s1000>(?P<d1000>\\S*)千)?(?P<s100>(?P<d100>\\S*)百)?(?P<s10>(?P<d10>\\S*)十)?(?P<d1>\\S*)?$')
jpnum_digits = {'一':1, 
 '二':2,  '三':3,  '四':4,  '五':5,  '六':6, 
 '七':7,  '八':8,  '九':9}
...
re_LawTypes = [
 (
  re.compile('^法律$'), 'Act'),
 (
  re.compile('^政令$'), 'CabinetOrder'),
 (
  re.compile('^勅令$'), 'ImperialOrder'),
 (
  re.compile('^\\S*[^政勅]令$'), 'MinisterialOrdinance'),
 (
  re.compile('^\\S*規則$'), 'Rule')]
re_LawNum = re.compile('(?P<era>\\S+?)(?P<year>[一二三四五六七八九十]+)年(?P<law_type>\\S+?)第(?P<num>[一二三四五六七八九十百千]+)号')
...
re_NamedNum = re.compile('^(?P<circle>○?)第?(?P<num>[一二三四五六七八九十百千]+)\\S*?(?P<branch>の\\S+)?$')
IROHA_CHARS = 'イロハニホヘトチリヌルヲワカヨタレソツネナラムウヰノオクヤマケフコエテアサキユメミシヱヒモセスン'
re_ItemNum = re.compile('^\\D*(?P<num>\\d+)\\D*$')
...
def parse_named_num(text):
    ...
    for subtext in text.replace('及び', '、').replace('から', '、').replace('まで', '').replace('～', '、').replace('・', '、').split('、'):
        ...
        if match:
            nums = [
             parse_jpnum(match.group(2))]
            if match.group(3):
                nums.extend([parse_jpnum(b) for b in match.group(3).split('の') if b])
...
def decorate_toc_article_group(el):
    ...
    for subel in el['children']:
        if subel['tag'] in ('PartTitle', 'ChapterTitle', 'SectionTitle', 'SubsectionTitle',
                            'DivisionTitle', 'ArticleTitle') and len(subel['children']):
...
def decorate_article(el):
    ...
    el['attr']['Hide'] = 'false'
    for subel in el['children']:
        if subel['tag'] == 'ArticleTitle' and len(subel['children']):
            body = subel['children'][0]
            num = parse_named_num(body.split()[0])
            if num:
                el['attr']['Num'] = num
...
def decorate_item(el):
    ...
    el['attr']['Hide'] = 'false'
    for subel in el['children']:
        if subel['tag'] in ('ItemTitle', 'Subitem1Title', 'Subitem2Title', 'Subitem3Title',
...
def decorate_column_sentence_group(el):
    column_sentences = []
    for subel in el['children']:
        if subel['tag'] in ('Column', 'Sentence'):
    ...
    for i, subel in enumerate(column_sentences):
        ...
        if subel['tag'] == 'Sentence' and len(subel['children']) and (subel['children'][0].startswith('ただし、') or subel['children'][0].startswith('但し、')):
...
def decorate(el):
    if el['tag'] == 'Law':
        ...
        if el['tag'] in ('TOCPart', 'TOCChapter', 'TOCSection', 'TOCSubsection', 'TOCDivision',
                         'TOCArticle'):
                ...
                if el['tag'] == 'Article':
                    ...
                    if el['tag'] == 'Paragraph':
                        ...
                        if el['tag'] == 'Sentence':
                            ...
                            if el['tag'] in ('Item', 'Subitem1', 'Subitem2', 'Subitem3',
                                             'Subitem4', 'Subitem5', 'Subitem6',
                                             'Subitem7', 'Subitem8', 'Subitem9',
                                             'Subitem10'):
                                decorate_item(el)
                            else:
                                if el['tag'] in ('ParagraphSentence', 'ItemSentence',
                                                 'Subitem1Sentence', 'Subitem2Sentence',
                                                 'Subitem3Sentence', 'Subitem4Sentence',
                                                 'Subitem5Sentence', 'Subitem6Sentence',
                                                 'Subitem7Sentence', 'Subitem8Sentence',
