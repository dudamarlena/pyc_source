...
# [GCC 8.4.0]
# Embedded file name: c:\Users\m\dev\lawtext\lawtext\parse.py
# Compiled at: 2017-12-10 08:07:47
# Size of source mod 2**32: 43365 bytes
import re
DEFAULT_INDENT = '\u3000'
...
class LawtextParseError(Exception):
    ...
    def __str__(self):
        lines = [f"{'==>' if lineno == self.lineno else '   '}{lineno:5}|{self.lines[lineno]}\n" for lineno in range(max(0, self.lineno - 2), min(len(self.lines), self.lineno + 2 + 1))]
...
re_DETECT_INDENT = re.compile('^(?P<indent>[ \\t\\f\\v]+)\\S.*$')
...
re_TC1stLine = re.compile('^(?P<indents>\\s*)\\* - (?P<tcbody>.*)$')
re_TCnthLine = re.compile('^(?P<indents>\\s*)  - (?P<tcbody>.*)$')
re_FigLine = re.compile('^(?P<indents>\\s*)\\.\\.(?:\\s+)figure::(?:\\s+)(?P<figbody>.*)$')
re_DefaultLine = re.compile('^(?P<indents>\\s*)(?P<linebody>\\S.*)$')
...
re_Arg = re.compile('^\\[(?P<name>\\S+?)="?(?P<value>\\S+?)"?\\]')
...
PARENTHESIS_PAIRS = [
 ...
 (
  re.compile('「'), re.compile('」'))]
re_FORCE_EXIT_PARENTHESIS = re.compile('」')
SENTENCE_DELIMITERS = [
 re.compile('。')]
...
def get_sentences(text):
    ...
    if len(text_split) >= 2:
        for column in text_split:
            els.append({'tag':'Column', 
             'attr':{},  'children':[{'tag':'Sentence',  'attr':{},  'children':[sentence]} for sentence in split_sentences(column)]})

    else:
        els.extend([{'tag':'Sentence',  'attr':{},  'children':[sentence]} for sentence in split_sentences(text)])
    return els


re_LawNum = re.compile('^[(（](?P<body>.+?)[）)](?:\\s*)$')
re_ArticleRange = re.compile('(?P<body>[(（].+?[）)])(?:\\s*)$')
re_ArticleCaption = re.compile('^[(（](?P<body>.+?)[）)](?:\\s*)$')
re_ArticleBody = re.compile('^(?P<title>\\S+)\\s+(?P<body>\\S.*)(?:\\s*)$')
re_ParagraphCaption = re.compile('^[(（](?P<body>.+?)[）)](?:\\s*)$')
re_ParagraphItemBody = re.compile('^(?P<title>\\S+)\\s+(?P<body>\\S.*)(?:\\s*)$')
re_SupplProvisionLabel = re.compile('^(?P<title>.+?)\\s*(?:[(（](?P<amend_law_num>.+?)[）)])?(?:\\s+(?P<extract>抄?))?(?:\\s*)$')
re_AppdxTableLabel = re.compile('^(?P<title>.+?)\\s*(?:(?P<related_article_num>[(（].+?[）)]))?(?:\\s*)$')
re_AppdxStyleLabel = re.compile('^(?P<title>.+?)\\s*(?:(?P<related_article_num>[(（].+?[）)]))?(?:\\s*)$')
...
class Parser:
    ...
    def process_title(self):
        ...
        if line_type == 'DefaultLine':
            if indent == 0:
                if len(body_split) == 1:
                    if next_line_type == 'BlankLine' or next_line_type == 'DefaultLine' and next_indent == 0:
                        law_title = {'tag':'LawTitle', 
                         'attr':{},  'children':[
                          data['body']]}
                        self.forward()
                        if self.continuing():
                            ...
                            if line_type == 'DefaultLine':
                                ...
                                law_num = {'tag':'LawNum',  'attr':{},  'children':[
                                  match.group(1)]}
                                self.forward()
        return (
         law_title, law_num)
    ...
    def process_enact_statement(self):
        ...
        if indent == 0:
            if len(body_split) == 1:
                if next_line_type == 'BlankLine':
                    enact_statement = {'tag':'EnactStatement', 
                     'attr':{},  'children':[
                      data['body']]}
                    self.forward()
    ...
    def process_toc_group(self):
        ...
        line_type, indent, data = self.here()
        next_line_type, next_indent, next_data = self.next()
        body_split = re.split('\\s+', re.split('の', data['body'])[0])
        if line_type == 'DefaultLine':
            toc_group_children = []
            toc_group_char = body_split[0][(len(body_split[0]) - 1)]
            title = re_ArticleRange.sub('', data['body'])
            title_tag = {'編':'PartTitle', 
             '章':'ChapterTitle',  '節':'SectionTitle',  '款':'SubsectionTitle', 
             '目':'DivisionTitle',  '条':'ArticleTitle',  '則':'SupplProvisionLabel'}[toc_group_char]
            toc_group_children.append({'tag':title_tag, 
             'attr':{},  'children':[
              title]})
            match = re_ArticleRange.search(data['body'])
            if match:
                toc_group_children.append({'tag':'ArticleRange', 
                 'attr':{},  'children':[
                  match.group(1)]})
            self.forward()
            ...
            toc_tag = {'編':'TOCPart', 
             '章':'TOCChapter',  '節':'TOCSection',  '款':'TOCSubsection', 
             '目':'TOCDivision',  '条':'TOCArticle',  '則':'TOCSupplProvision'}[toc_group_char]
            toc_group = {'tag':toc_tag, 
             'attr':{},  'children':toc_group_children}
        return toc_group
    ...
    def process_toc(self):
        ...
        if indent == 0:
            if len(body_split) == 1:
                if next_line_type == 'DefaultLine':
                    toc_children = []
                    toc_children.append({'tag':'TOCLabel', 
                     'attr':{},  'children':[
                      data['body']]})
                    self.forward()
                    toc_base_indent = indent
    ...
    def process_main_provision(self):
        ...
        if len(main_provision_children):
            main_provision = {'tag':'MainProvision',  'attr':{},  'children':main_provision_children}
    ...
    def process_article_group(self, parent_group_tag=''):
        ...
        article_group_tag_rank = {'':0, 
         'Part':1,  'Chapter':2,  'Section':3,  'Subsection':4, 
         'Division':5}
        article_group_tags = {'編':'Part', 
         '章':'Chapter',  '節':'Section',  '款':'Subsection', 
         '目':'Division'}
        if indent != 0 and article_group_char in article_group_tags and article_group_tag_rank[article_group_tags[article_group_char]] > article_group_tag_rank[parent_group_tag] and next_line_type == 'BlankLine':
            article_group_tag = article_group_tags[article_group_char]
            article_group_title_tag = {'編':'PartTitle', 
             '章':'ChapterTitle',  '節':'SectionTitle',  '款':'SubsectionTitle', 
             '目':'DivisionTitle',  '条':'ArticleTitle'}[article_group_char]
            article_group_children = []
            article_group_children.append({'tag':article_group_title_tag, 
             'attr':{},  'children':[
              data['body']]})
            self.forward()
            self.skip_blank_lines()
            ...
            if len(article_group_children):
                article_group = {'tag':article_group_tag,  'attr':{},  'children':article_group_children}
    ...
    def process_article(self):
        ...
        if line_type == 'DefaultLine' and indent == 1 and re_ArticleCaption.match(data['body']) and next_line_type == 'DefaultLine' and next_indent == 0 and not (next2_data['body'].startswith('別表') if len(next2_data) else False) and (re_ArticleBody.match(next_data['body']) or len(next_body_split) == 1 and next2_line_type == 'DefaultLine' and next2_indent == 0 and re_ParagraphItemBody.match(next2_data['body'])):
            article_children.append({'tag':'ArticleCaption', 
             'attr':{},  'children':[
              data['body']]})
            self.forward()
            line_type, indent, data = self.here()
            ...
            if line_type == 'DefaultLine':
                if indent == 0:
                    if not data['body'].startswith('別表'):
                        if match or len(body_split) == 1 and next_line_type == 'DefaultLine' and next_indent == 0 and re_ParagraphItemBody.match(next_data['body']):
                            if match:
                                article_title, _ = match.group(1), match.group(2)
                            else:
                                article_title = data['body']
                            article_children.append({'tag':'ArticleTitle', 
                             'attr':{},  'children':[
                              article_title]})
                            first_paragraph_in_article = True
                            if not match:
                                article_children.append({'tag':'Paragraph', 
                                 'attr':{},  'children':[
                                  {'tag':'ParagraphNum', 
                                   'attr':{},  'children':[]},
                            ...
                            article = {'tag':'Article', 
                             'attr':{},  'children':article_children}
        return article
    ...
    def process_paragraph_item(self, current_indent, first_paragraph_in_article=False):
        ...
        if not first_paragraph_in_article:
            if line_type == 'DefaultLine':
                if indent == current_indent + 1:
                    if re_ParagraphCaption.match(data['body']):
                        if next_line_type == 'DefaultLine':
                            if next_indent == current_indent:
                                if next_match:
                                    if '条' not in next_match.group(1):
                                        if '附' not in next_match.group(1):
                                            if '付' not in next_match.group(1):
                                                if not next_match.group(1).startswith('別表'):
                                                    paragraph_item_children.append({'tag':'ParagraphCaption', 
                                                     'attr':{},  'children':[
                                                      data['body']]})
                                                    self.forward()
                                                    line_type, indent, data = self.here()
            ...
            paragraph_item_tags = {0:'Paragraph', 
             1:'Item',  2:'Subitem1', 
             3:'Subitem2',  4:'Subitem3',  5:'Subitem4', 
             6:'Subitem5',  7:'Subitem6',  8:'Subitem7', 
             9:'Subitem8',  10:'Subitem9',  11:'Subitem10'}
            paragraph_item_title_tags = {0:'ParagraphNum', 
             1:'ItemTitle',  2:'Subitem1Title', 
             3:'Subitem2Title',  4:'Subitem3Title',  5:'Subitem4Title', 
             6:'Subitem5Title',  7:'Subitem6Title',  8:'Subitem7Title', 
             9:'Subitem8Title',  10:'Subitem9Title',  11:'Subitem10Title'}
            paragraph_item_sentence_tags = {0:'ParagraphSentence', 
             1:'ItemSentence',  2:'Subitem1Sentence', 
             3:'Subitem2Sentence',  4:'Subitem3Sentence',  5:'Subitem4Sentence', 
             6:'Subitem5Sentence',  7:'Subitem6Sentence',  8:'Subitem7Sentence', 
             9:'Subitem8Sentence',  10:'Subitem9Sentence',  11:'Subitem10Sentence'}
            match = re_ParagraphItemBody.match(data['body'])
            if line_type == 'DefaultLine':
                if indent == current_indent:
                    if match and '附' not in match.group(1) and '付' not in match.group(1) and '備考' not in match.group(1) and (first_paragraph_in_article or '条' not in match.group(1) and not match.group(1).startswith('別表')) or indent == 0 and len(split(data['body'])) == 1 and not data['body'].startswith('別表'):
                        if first_paragraph_in_article or not match:
                            paragraph_item_children.append({'tag':'ParagraphNum', 
                             'attr':{},  'children':[]})
                        else:
                            paragraph_item_title = match.group(1)
                            paragraph_item_title_tag = paragraph_item_title_tags[current_indent]
                            paragraph_item_children.append({'tag':paragraph_item_title_tag, 
                             'attr':{},  'children':[
                        ...
                        paragraph_item_children.append({'tag':paragraph_item_sentence_tag, 
                         'attr':{},  'children':paragraph_item_sentence_children})
                        self.forward()
                        ...
                        paragraph_item = {'tag':paragraph_item_tag, 
                         'attr':{},  'children':paragraph_item_children}
            ...
            if indent == current_indent:
                ...
                if len(table_columns):
                    ...
                    table_row = {'tag':'TableRow', 
                     'attr':{},  'children':table_row_children}
        return table_row

    def process_table_columns--- This code section failed: ---

 L. 891         0  BUILD_LIST_0          0 
                2  STORE_FAST               'table_columns'

 L. 893         4  LOAD_CONST               False
                6  STORE_FAST               'first_column_processed'

 L. 895         8  SETUP_LOOP          102  'to 102'
               10  LOAD_FAST                'self'
               12  LOAD_ATTR                continuing
               14  CALL_FUNCTION_0       0  '0 positional arguments'
               16  POP_JUMP_IF_FALSE   100  'to 100'

 L. 896        18  LOAD_FAST                'self'
               20  LOAD_ATTR                here
               22  CALL_FUNCTION_0       0  '0 positional arguments'
               24  UNPACK_SEQUENCE_3     3 
               26  STORE_FAST               'line_type'
               28  STORE_FAST               'indent'
               30  STORE_FAST               'data'

 L. 899        32  LOAD_FAST                'first_column_processed'
               34  UNARY_NOT        
               36  POP_JUMP_IF_FALSE    46  'to 46'

 L. 900        38  LOAD_FAST                'line_type'
               40  LOAD_STR                 'TC1stLine'
               42  COMPARE_OP               ==
             44_0  COME_FROM            36  '36'
               44  POP_JUMP_IF_TRUE     58  'to 58'

 L. 902        46  LOAD_FAST                'first_column_processed'
               48  POP_JUMP_IF_FALSE    96  'to 96'

 L. 903        50  LOAD_FAST                'line_type'
               52  LOAD_STR                 'TCnthLine'
               54  COMPARE_OP               ==
             56_0  COME_FROM            48  '48'
             56_1  COME_FROM            44  '44'
               56  POP_JUMP_IF_FALSE    96  'to 96'

 L. 906        58  LOAD_FAST                'self'
               60  LOAD_ATTR                process_table_column
               62  LOAD_FAST                'current_indent'
               64  CALL_FUNCTION_1       1  '1 positional argument'
               66  STORE_FAST               'table_column'

 L. 907        68  LOAD_FAST                'table_column'
               70  POP_JUMP_IF_FALSE    92  'to 92'

 L. 908        72  LOAD_FAST                'table_columns'
               74  LOAD_ATTR                append
               76  LOAD_FAST                'table_column'
               78  CALL_FUNCTION_1       1  '1 positional argument'
               80  POP_TOP          

 L. 909        82  LOAD_FAST                'first_column_processed'
               84  POP_JUMP_IF_TRUE     94  'to 94'

 L. 910        86  LOAD_CONST               True
               88  STORE_FAST               'first_column_processed'
               90  JUMP_ABSOLUTE        98  'to 98'
               92  ELSE                     '94'
 ...
 L. 914        96  BREAK_LOOP       
              ...
              100  POP_BLOCK        
            102_0  COME_FROM_LOOP        8  '8'

 L. 916       102  LOAD_FAST                'table_columns'
              104  RETURN_VALUE     
               -1  RETURN_LAST      

Parse error at or near `ELSE' instruction at offset 96
    ...
    def process_table_column(self, current_indent):
        ...
        line_type, indent, data = self.here()
        next_line_type, next_indent, next_data = self.next()
        if line_type in ('TC1stLine', 'TCnthLine'):
            if indent == current_indent:
                table_column_children.append({'tag':'Sentence', 
                 'attr':{},  'children':[
                  data['body']]})
                self.forward()
                ...
                while self.continuing():
                    line_type, indent, data = self.here()
                    next_line_type, next_indent, next_data = self.next()
                    if line_type == 'DefaultLine' and indent == current_indent + 2:
                        table_column_children.append({'tag':'Sentence', 
                         'attr':{},  'children':[
                          data['body']]})
                        self.forward()
    ...
    def process_list(self, current_indent):
        ...
        if line_type == 'DefaultLine':
            if indent == current_indent:
                if body_split[0][(len(body_split[0]) - 1)] not in ('編', '章', '節', '款',
                                                                   '目', '条', '附',
                                                                   '付', '則'):
                    list_ = {'tag':'List', 
                     'attr':{},  'children':[
                      {'tag':'ListSentence', 
                       'attr':{},  'children':get_sentences(data['body'])}]}
                    self.forward()
        return list_
    ...
    def process_suppl_provision(self):
        ...
        titlejoin = re.sub('\\s+', '', match.group(1)) if match else ''
        if line_type == 'DefaultLine' and indent != 0 and match and (titlejoin.startswith('付則') or titlejoin.startswith('附則')):
            suppl_provision_attr = {}
            if match.group(3):
                suppl_provision_attr['Extract'] = 'true'
            if match.group(2):
                suppl_provision_attr['AmendLawNum'] = match.group(2)
            suppl_provision_children.append({'tag':'SupplProvisionLabel', 
             'attr':{},  'children':[
              match.group(1)]})
            self.forward()
            ...
            line_type, indent, data = self.here()
            if not articles_or_paragraph_items_processed:
                if line_type == 'DefaultLine':
                    if indent == 0:
                        suppl_provision_children.append({'tag':'Paragraph', 
                         'attr':{},  'children':[
                          {'tag':'ParagraphNum', 
                           'attr':{},  'children':[]},
                          {'tag':'ParagraphSentence', 
            ...
            if len(suppl_provision_children):
                suppl_provision = {'tag':'SupplProvision',  'attr':suppl_provision_attr, 
    ...
    def process_appdx_table(self):
        ...
        titlejoin = re.sub('\\s+', '', match.group(1)) if match else ''
        if line_type == 'DefaultLine' and indent == 0 and match and titlejoin.startswith('別表'):
            appdx_table_children.append({'tag':'AppdxTableTitle', 
             'attr':{},  'children':[
              match.group(1)]})
            if match.group(2):
                appdx_table_children.append({'tag':'RelatedArticleNum', 
                 'attr':{},  'children':[
                  match.group(2)]})
            self.forward()
            ...
            appdx_table = {'tag':'AppdxTable', 
             'attr':{},  'children':appdx_table_children}
        return appdx_table
    ...
    def process_appdx_style(self):
        ...
        body_split = re.split('\\s+', (data['body']), maxsplit=1) if len(data) else ['']
        if line_type == 'DefaultLine' and indent == 0 and body_split[0].startswith('様式'):
            appdx_style_children.append({'tag':'AppdxStyleTitle', 
             'attr':{},  'children':[
              match.group(1)]})
            if match.group(2):
                appdx_style_children.append({'tag':'RelatedArticleNum', 
                 'attr':{},  'children':[
                  match.group(2)]})
            self.forward()
            current_indent = indent
            ...
            if len(style_struct_children):
                appdx_style_children.append({'tag':'StyleStruct', 
                 'attr':{},  'children':style_struct_children})
            appdx_style = {'tag':'AppdxStyle', 
             'attr':{},  'children':appdx_style_children}
        return appdx_style
    ...
    def process_remarks(self, current_indent):
        ...
        if line_type == 'DefaultLine':
            if indent == current_indent:
                if body_split[0].startswith('備考'):
                    remarks_children.append({'tag':'RemarksLabel', 
                     'attr':{},  'children':[
                      body_split[0]]})
                    if len(body_split) > 1:
                    ...
                    remarks = {'tag':'Remarks', 
                     'attr':{},  'children':remarks_children}
        return remarks
    ...
    def process_law(self):
        ...
        law_children.append({'tag':'LawBody', 
         'attr':{},  'children':law_body_children})
        return {'tag':'Law', 
         'attr':{},  'children':law_children}
