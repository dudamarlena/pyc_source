...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/advancedcaching/hildon_plugins.py
# Compiled at: 2011-04-23 08:59:57
import geocaching, gtk, hildon, pango, threadpool, logging, geo
    ...
    def _on_show_search(self, widget, data):
        ...
        while True:
            ...
            if response == RESPONSE_RESET:
                ...
                return
            if response == RESPONSE_LAST_RESULTS:
                if self.old_search_window == None:
                    return
        ...
        def add_tiles(source, zoom):
            ...
            if zoom + 1 <= max_zoom_step:
                for add_x in (0, 1):
                    for add_y in (0, 1):
                        tile = (
                         source[0] * 2 + add_x, source[1] * 2 + add_y)
...
class HildonToolsDialog(object):
    ...
    NUMERAL_MAP = zip((1000, 900, 500, 400, 100, 90, 50, 40, 10, 9, 5, 4, 1), ('M',
                                                                               'CM',
                                                                               'D',
                                                                               'CD',
                                                                               'C',
