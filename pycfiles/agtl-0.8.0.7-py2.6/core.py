...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/advancedcaching/core.py
# Compiled at: 2011-05-10 14:41:08
from __future__ import with_statement
VERSION = '0.8.0.7'
import logging
logging.basicConfig(level=logging.WARNING, format='%(relativeCreated)6d %(levelname)10s %(name)-20s %(message)s')
...
import downloader, geocaching, gpsreader
from os import path, mkdir, extsep, remove, walk
import provider
from threading import Thread
import cachedownloader, fieldnotesuploader
from actors.tts import TTS
import connection, gobject
if len(argv) == 1:
    import cli
    print cli.usage % {'name': argv[0]}
    exit()
if '-v' in argv:
    import colorer, logging.handlers
...
class Core(gobject.GObject):
    __gsignals__ = {'map-marks-changed': (
       ...
       'fieldnotes-changed': (
                            gobject.SIGNAL_RUN_FIRST, gobject.TYPE_NONE, ()), 
       'good-fix': (
                  gobject.SIGNAL_RUN_FIRST, gobject.TYPE_NONE, (gobject.TYPE_PYOBJECT, gobject.TYPE_PYOBJECT, gobject.TYPE_PYOBJECT)), 
       'no-fix': (
    ...
    UPDATE_MODULES = [
     cachedownloader, fieldnotesuploader]
    DEFAULT_SETTINGS = {'download_visible': True, 
    ...
    def format_file_size(size):
        if size < 1024:
            ...
            if size < 1073741824:
                return '%d MiB' % (size / 1048576)
            return '%d GiB' % (size / 1073741824)
    ...
    def __on_save_settings(self, caller):
        ...
        if self.current_target != None:
            settings = {'last_target_lat': self.current_target.lat, 'last_target_lon': self.current_target.lon}
    ...
    def get_route(self, c1, c2, r):
        ...
        for i in range(len(route)):
            ...
            if len(together) < MAX_TOGETHER and i < len(route) - 1 and (abs(brg - 90) < TOL or abs(brg + 90) < TOL or abs(brg) < TOL or abs(brg - 180) < TOL) and route[i].distance_to(route[(i + 1)]) < r * 1000 * 2:
                ...
                new_c2 = c2.transform(135, r * 1000 * sqrt(2))
