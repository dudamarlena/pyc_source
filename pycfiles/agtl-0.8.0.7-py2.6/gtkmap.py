...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/advancedcaching/gtkmap.py
...
import cairo, geo, geocaching, gobject, gtk, openstreetmap, pango, threadpool
...
class Map(gtk.DrawingArea, AbstractMap):
    ...
    def __configure_event(self, widget, event):
        ...
        self.cr_drawing_area = self.pixmap.cairo_create()
    ...
    def __drag_draw(self):
        ...
        self.window.clear()
        self.window.draw_drawable(gc=self.xgc, src=self.pixmap, xsrc=0, ysrc=0, xdest=self.draw_at_x - self.drag_offset_x, ydest=self.draw_at_y - self.drag_offset_y, width=width, height=height)
        return True
...
class MarksLayer(AbstractMarksLayer):
    ...
    def draw(self):
        ...
        if self.gps_data != None and self.gps_has_fix:
            position = (
             self.map.map_width - self.OSD_BORDER_LEFTRIGHT, self.OSD_BORDER_TOPBOTTOM)
