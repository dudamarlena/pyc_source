...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/advancedcaching/qt/mapwidget.py
...
import geo, geocaching, threadpool
...
class QtMap(QGLWidget, AbstractMap):
    ...
    def __get_id_string(self, tile, display_zoom, undersample):
        return (
         self.tile_loader.PREFIX, tile[0], tile[1], display_zoom, 1 if undersample else 0)
...
class QtSingleMarkLayer(AbstractQtLayer):
    ...
    def draw(self):
        ...
        p.drawLines(QLineF(t[0] - radius_o, t[1], t[0] - radius_i, t[1]), QLineF(t[0] + radius_o, t[1], t[0] + radius_i, t[1]), QLineF(t[0], t[1] + radius_o, t[0], t[1] + radius_i), QLineF(t[0], t[1] - radius_o, t[0], t[1] - radius_i))
        p.drawArc(QRectF(t[0] - radius_c / 2, t[1] - radius_c / 2, radius_c, radius_c), 0, 5760)
...
class QtOsdLayer(AbstractQtLayer):
    ...
    def clicked_screen(self, screenpoint):
        if self.click_zoom_in == None or self.click_zoom_out == None:
            ...
            if self.click_zoom_in.contains(*screenpoint):
                self.map.zoom_in()
                return False
            if self.click_zoom_out.contains(*screenpoint):
                self.map.zoom_out()
                return False
            return
    ...
    def draw(self):
        position = (
         self.OSD_BORDER_LEFTRIGHT, self.map.map_height - 10 - self.OSD_BORDER_TOPBOTTOM)
        ...
        final_length_meters = round(first_length_meters, int(-math.floor(math.log(first_length_meters, 10) + 1e-05)))
    ...
    def draw(self):
        ...
        if self.map.get_zoom() < self.CACHES_ZOOM_LOWER_BOUND:
            ...
            for c in coords:
                ...
                p.drawLines(QLineF(loc[0] - s, loc[1] - s, loc[0] + s, loc[1] + s), QLineF(loc[0] - s, loc[1] + s, loc[0] + s, loc[1] - s))
                if c.was_downloaded():
                    width = 6
                    dist = 3
                    count = 3
            ...
            if self.current_cache != None and self.current_cache.get_waypoints() != None:
                ...
                for w in self.current_cache.get_waypoints():
                    if w['lat'] != -1 and w['lon'] != -1:
                        ...
                        p.drawArc(QRectF(loc[0] - radius - 1, loc[1] - radius - 1, radius * 2 + 1, radius * 2 + 1), 0, 5760)
    ...
    def draw(self):
        ...
        if loc != None and self.map.point_in_screen(loc):
            ...
            if self.gps_has_fix:
                ...
                p.setBrush(QBrush(Qt.transparent))
                p.drawArc(loc[0] - radius_pixels / 2.0 - 1, loc[1] - radius_pixels / 2.0 - 1, radius_pixels, radius_pixels, 0, 5760)
                if self.gps_data.speed > 2.5:
                    ...
                    p.drawArc(loc[0] - self.RADIUS_STANDING / 2.0, loc[1] - self.RADIUS_STANDING / 2.0 - 1, self.RADIUS_STANDING + 1, self.RADIUS_STANDING + 1, 0, 5760)
            if self.gps_has_fix:
                p.setBrush(self.BRUSH_CURRENT_POSITION)
            else:
                p.setBrush(self.BRUSH_CURRENT_POSITION_NO_FIX)
            p.setPen(QPen(Qt.transparent))
            p.drawPie(loc[0] - self.SIZE_CURRENT_POSITION / 2.0, loc[1] - self.SIZE_CURRENT_POSITION / 2.0, self.SIZE_CURRENT_POSITION + 1, self.SIZE_CURRENT_POSITION + 1, 0, 5760)
