...
# Embedded file name: build\bdist.win-amd64\egg\aigpy\ffmpegHelper.py
# Compiled at: 2019-10-26 00:11:28
# Size of source mod 2**32: 12262 bytes
...
class FFmpegTool(object):
    ...
    def mergerByM3u8_Multithreading2(self, url, filepath, showprogress=False, showshell=False):
        try:
            urllist = self._FFmpegTool__parseM3u8(url)
            if len(urllist) <= 0:
                return False
        ...
        try:
            ...
            for i, item in enumerate(urllist):
                ...
                if os.path.exists(path):
                    os.remove(path)
                self.thread.start(self._FFmpegTool__thradfunc_dl, item, path, 3)
    ...
    def mergerByM3u8(self, url, filepath, showshell=False):
        ...
        try:
            ...
            res = self._FFmpegTool__process(cmd, 3, showshell, filepath)
        except:
            pass
    ...
    def mergerByTs(self, srcDir, filepath, showshell=False):
        ...
        ret = self._FFmpegTool__process(cmd, 3, showshell, tmppath)
        if ret is True:
            cmd = 'ffmpeg -i "' + tmppath + '" -c copy "' + filepath + '"'
            ret = self._FFmpegTool__process(cmd, 3, showshell, filepath)
    ...
    def mergerByTsfiles(self, srcfilepaths, filepath, showshell=False):
        ...
        for item in array:
            ...
            ret = self._FFmpegTool__process(cmd, 3, showshell, tmppath2)
            if ret is False:
                break
            pathHelper.remove(tmppath)
    ...
    def covertFile(self, srcfile, descfile, showshell=False):
        try:
            ...
            ret = self._FFmpegTool__process(cmd, 3, showshell, filepath, False)
            return ret
        except:
            return False
    ...
    def test(self):
        self._FFmpegTool__process('dir', 3, False, 'e:\\7\\Video\\1.ts')