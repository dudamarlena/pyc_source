...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/DigiPathAI/new_Segmentation.py
# Compiled at: 2019-12-05 07:10:21
# Size of source mod 2**32: 27063 bytes
...
import matplotlib.pyplot as plt, openslide, numpy as np, tensorflow as tf
...
import gc, pandas as pd, tifffile, skimage.io as io, DigiPathAI
...
class WSIStridedPatchDataset(Dataset):
    ...
    def __init__(self, wsi_path, mask_path, label_path=None, image_size=256, normalize=True, flip='NONE', rotate='NONE', level=5, sampling_stride=16, roi_masking=True):
        """
    ...
    def _preprocess(self):
        ...
        if self._mask_path is not None:
            ...
            if mask_file_name.endswith('.tiff'):
                ...
                self._mask = np.array(mask_obj.read_region((0, 0), self._level, mask_obj.level_dimensions[self._level]).convert('L')).T
        ...
        if X_slide // X_mask != Y_slide // Y_mask:
            raise Exception('Slide/Mask dimension does not match , X_slide / X_mask : {} / {}, Y_slide / Y_mask : {} / {}'.format(X_slide, X_mask, Y_slide, Y_mask))
        self._resolution = np.round(X_slide * 1.0 / X_mask)
        if not np.log2(self._resolution).is_integer():
            raise Exception('Resolution (X_slide / X_mask) is not power of 2 : {}'.format(self._resolution))
...
def getSegmentation(img_path, patch_size=256, stride_size=128, batch_size=32, quick=True, tta_list=None, crf=False, status=None):
    """
    ...
    mask_path = None
    start_time = time.time()
    dataset_obj = WSIStridedPatchDataset(wsi_path, mask_path=None, label_path=None, image_size=patch_size, normalize=True, flip=None, rotate=None, level=level, sampling_stride=scale_sampling_stride, roi_masking=True)
