...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/DigiPathAI/Segmentation.py
# Compiled at: 2019-12-06 11:18:55
# Size of source mod 2**32: 10383 bytes
from __future__ import absolute_import
from __future__ import division
from __future__ import print_function
from datetime import datetime
import os, glob, random, imgaug
from imgaug import augmenters as iaa
from PIL import Image
import matplotlib.pyplot as plt, numpy as np, tensorflow as tf, pandas as pd
...
def get_prediction(wsi_path, mask_path=None, label_path=None, batch_size=8, models=None, tta_list=None, num_workers=8, verbose=0, patch_size=256, stride_size=256, status=None):
    """
            patch based segmentor
        """
    dataset_obj = WSIStridedPatchDataset(wsi_path, mask_path, label_path, image_size=patch_size, normalize=True, flip=None, rotate=None, sampling_stride=stride_size, roi_masking=True)
    ...
    for i, (image_patches, x_coords, y_coords, label_patches) in enumerate(dataloader):
        ...
        for j, model_name in enumerate(models.keys()):
            for tta_ in tta_list:
                ...
                prediction = models[model_name].predict(image_patches, batch_size=batch_size, verbose=verbose, steps=None)
                for i in range(batch_size):
                    prediction_trans = transform_prob(prediction[i], tta_) / (1.0 * len(tta_list))
...
def getSegmentation(img_path, patch_size=256, stride_size=128, batch_size=32, quick=True, tta_list=None, crf=False, save_path='../Results', status=None, mode='colon'):
    """
            args:
    ...
    print('==================================================')
    print(mode)
    if mode not in ('colon', 'liver', 'breast'):
        raise ValueError("Unknown mode found, allowed fields are: ['colon', 'liver', 'breast']")
    if mode == 'colon':
        path = os.path.join(home, '.DigiPathAI/digestpath_models')
        if not os.path.exists(os.path.join(path, 'digestpath_inception.h5')) or not os.path.exists(os.path.join(path, 'digestpath_deeplabv3.h5')) or not os.path.exists(os.path.join(path, 'digestpath_densenet.h5')):
            if status is not None:
                status['status'] = 'Downloading Trained Models'
            download_digestpath()
            ...
            if status is not None:
                status['status'] = 'Found Trained Models, Skipping download'
            model_path_inception = os.path.join(path, 'digestpath_inception.h5')
            model_path_deeplabv3 = os.path.join(path, 'digestpath_deeplabv3.h5')
            model_path_densenet = os.path.join(path, 'digestpath_densenet.h5')
    else:
        if mode == 'liver':
            ...
            if not os.path.exists(os.path.join(path, 'paip_inception.h5')) or not os.path.exists(os.path.join(path, 'paip_deeplabv3.h5')) or not os.path.exists(os.path.join(path, 'paip_densenet.h5')):
                if status is not None:
                    status['status'] = 'Downloading Trained Models'
                download_paip()
            ...
            if mode == 'breast':
                ...
                if not os.path.exists(os.path.join(path, 'camelyon_inception.h5')) or not os.path.exists(os.path.join(path, 'camelyon_deeplabv3.h5')) or not os.path.exists(os.path.join(path, 'camelyon_densenet.h5')):
                    if status is not None:
                        status['status'] = 'Downloading Trained Models'
                    download_camelyon()
                    ...
                    model_path_densenet = os.path.join(path, 'camelyon_densenet.h5')
                if status is not None:
                    status['status'] = 'Loading Trained weights'
        ...
        for i, model_name in enumerate(models_to_consider.keys()):
            models[model_name] = load_trained_models(model_name, models_to_consider[model_name])

        threshold = 0.3
        if status is not None:
            status['status'] = 'Running segmentation'
        img, probs_map, count_map, tissue_mask, label_mask = get_prediction(img_path, mask_path=None, label_path=None, batch_size=batch_size, tta_list=tta_list, models=models, patch_size=patch_size, stride_size=stride_size, status=status)
        count_map[count_map == 0] = 1
        mean_probs, uncertanity = get_mean_img(probs_map.values(), count_map)
        mean_probs = mean_probs[(Ellipsis, None)]
        if crf:
