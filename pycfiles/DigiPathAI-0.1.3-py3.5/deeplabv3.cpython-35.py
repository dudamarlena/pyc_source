...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/DigiPathAI/models/deeplabv3.py
# Compiled at: 2019-11-09 07:15:25
# Size of source mod 2**32: 22662 bytes
...
def SepConv_BN(x, filters, prefix, stride=1, kernel_size=3, rate=1, depth_activation=False, epsilon=0.001):
...
def _conv2d_same(x, filters, prefix, stride=1, kernel_size=3, rate=1):
    ...
    if stride == 1:
        return Conv2D(filters, (
         ...
         stride, stride), padding='same', use_bias=False, dilation_rate=(
        ...
        return Conv2D(filters, (
         kernel_size, kernel_size), strides=(
         stride, stride), padding='valid', use_bias=False, dilation_rate=(
         rate, rate), name=prefix)(x)
...
def _inverted_res_block(inputs, expansion, stride, alpha, filters, block_id, skip_connection, rate=1):
    ...
    if block_id:
        ...
        x = BatchNormalization(epsilon=0.001, momentum=0.999, name=prefix + 'expand_BN')(x)
    ...
    x = BatchNormalization(epsilon=0.001, momentum=0.999, name=prefix + 'depthwise_BN')(x)
    x = Activation(relu6, name=prefix + 'depthwise_relu')(x)
    x = Conv2D(pointwise_filters, kernel_size=1, padding='same', use_bias=False, activation=None, name=prefix + 'project')(x)
    x = BatchNormalization(epsilon=0.001, momentum=0.999, name=prefix + 'project_BN')(x)
    if skip_connection:
        return Add(name=prefix + 'add')([inputs, x])
    return x


def Deeplabv3(weights='came', input_tensor=None, input_shape=(512, 512, 3), classes=21, backbone='mobile2', OS=16, alpha=1.0, activation=None):
    ...
    """
    if weights not in {None, 'cityscapes', 'pascal_voc'}:
    ...
    if backbone not in {'mobilenetv2', 'xception'}:
    ...
    if backbone == 'xception':
        ...
        x = BatchNormalization(epsilon=0.001, momentum=0.999, name='Conv_BN')(x)
    ...
    b4 = BatchNormalization(name='image_pooling_BN', epsilon=1e-05)(b4)
    ...
    b0 = BatchNormalization(name='aspp0_BN', epsilon=1e-05)(b0)
    b0 = Activation('relu', name='aspp0_activation')(b0)
    if backbone == 'xception':
        b1 = SepConv_BN(x, 256, 'aspp1', rate=atrous_rates[0], depth_activation=True, epsilon=1e-05)
        b2 = SepConv_BN(x, 256, 'aspp2', rate=atrous_rates[1], depth_activation=True, epsilon=1e-05)
        b3 = SepConv_BN(x, 256, 'aspp3', rate=atrous_rates[2], depth_activation=True, epsilon=1e-05)
        x = Concatenate()([b4, b0, b1, b2, b3])
    else:
        x = Concatenate()([b4, b0])
    x = Conv2D(256, (1, 1), padding='same', use_bias=False, name='concat_projection')(x)
    x = BatchNormalization(name='concat_projection_BN', epsilon=1e-05)(x)
    x = Activation('relu')(x)
    x = Dropout(0.1)(x)
    if backbone == 'xception':
        size_before2 = tf.keras.backend.int_shape(x)
        x = Lambda(lambda xx: tf.compat.v1.image.resize(xx, size_before2[1:3] * tf.constant(OS // 4), method='bilinear', align_corners=True))(x)
        dec_skip1 = Conv2D(48, (1, 1), padding='same', use_bias=False, name='feature_projection0')(skip1)
        dec_skip1 = BatchNormalization(name='feature_projection0_BN', epsilon=1e-05)(dec_skip1)
        dec_skip1 = Activation('relu')(dec_skip1)
        x = Concatenate()([x, dec_skip1])
        x = SepConv_BN(x, 256, 'decoder_conv0', depth_activation=True, epsilon=1e-05)
        x = SepConv_BN(x, 256, 'decoder_conv1', depth_activation=True, epsilon=1e-05)
