...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/DigiPathAI/main_server.py
# Compiled at: 2019-12-06 11:05:22
# Size of source mod 2**32: 9902 bytes
        ...
        for name in sorted(os.listdir(os.path.join(basedir, relpath))):
            ...
            if os.path.isdir(cur_path):
                ...
                if cur_dir.children:
                    ...
                    if OpenSlide.detect_format(cur_path) and 'slide' in os.path.basename(cur_path):
...
def run_segmentation(status, getSegmentation):
    ...
    getSegmentation(img_path=status['slide_path'], save_path=get_mask_path(status['slide_path']), status=status, mode=status['tissuetype'])
    time.sleep(0.1)
...
def get_slide_properties(slide_path):
    ...
    if slide_area / 1000000.0 != 0:
        properties['Area'] = '%d million pixels' % int(slide_dims[1] * slide_dims[0] / 1000000.0)
    else:
        if slide_area / 1000.0 != 0:
            properties['Area'] = '%d thousand pixels' % int(slide_dims[1] * slide_dims[0] / 10000.0)
        else:
...
def slide(path):
    ...
    print(slide_url)
    print(VIEWER_ONLY)
    return render_template('viewer.html', slide_url=slide_url, mask_status=mask_status, viewer_only=VIEWER_ONLY, properties=properties, slide_filename=slide.filename, slide_mpp=slide.mpp, root_dir=_Directory(app.basedir))
...
def main():
    ...
    parser.add_option('-p', '--port', metavar='PORT', dest='port', type='int', default=8080, help='port to listen on [8080]')
    ...
    parser.add_option('--viewer-only', action='store_true', dest='viewer_only', help='disable segmentation')
