...
# [GCC 8.4.0]
# Embedded file name: /Users/ramusus/workspace/manufacture/env/src/django-vkontakte-groups/vkontakte_groups/models.py
# Compiled at: 2016-03-02 14:07:43
import logging
        ...
        def check_members_count(count):
            if check_count and group.members_count and count > 0:
                ...
                if 0.98 > division or 1.01 < division:
                    raise CheckMembersCountFailed('Suspicious ammount of members fetched for group %s. Actual ammount is %d, fetched %d, division is %s' % (
...
class Group(PhotableModelMixin, VideoableModelMixin, UserableModelMixin, VkontaktePKModel):
    ...
    screen_name = models.CharField('Короткое имя группы', max_length=50, db_index=True)
    is_closed = models.NullBooleanField('Флаг закрытой группы')
    is_admin = models.NullBooleanField('Пользователь является администратором')
    members_count = models.IntegerField('Всего участников', null=True)
    verified = models.NullBooleanField('Флаг официальной группы')
    type = models.CharField('Тип объекта', max_length=10, choices=GROUP_TYPE_CHOICES)
    photo = models.URLField()
    photo_big = models.URLField()
    photo_medium = models.URLField()
    remote = GroupRemoteManager(remote_pk=('remote_id', ), methods_namespace='groups', version=5.28, methods={'get': 'getById', 
       'search': 'search', 
