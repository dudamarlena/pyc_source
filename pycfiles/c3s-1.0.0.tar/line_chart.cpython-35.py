...
# Embedded file name: /Users/ben/Documents/Projects/C3PyO/c3pyo/line_chart.py
# Compiled at: 2016-10-21 03:50:50
...
class LineChart(C3Chart):
        ...
        if isinstance(data, dict):
            ...
            if is_iterable(data):
                ...
                if all([is_iterable(x) for x in data]):
                    ...
                    if not any(is_iterable_of_iterables):
                        self.y_data.append(data)
                    else:
    ...
    def get_points_for_json(self):
        return {'show': self.show_points}

    def get_axis_for_json(self):
        if self.x_is_datetimes:
            return {'x': {'type': 'timeseries', 
                   'tick': {'format': DATETIME_FORMAT}}}
        else:
            if self.x_is_dates:
                return {'x': {'type': 'timeseries', 
                       'tick': {'format': DATE_FORMAT}}}
            return {}
