...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/gerald/schema.py
...
class Schema(object):
    ...
    def compare(self, other_schema):
        ...
        elif self.schema.keys() != other_schema.schema.keys():
...
class View(object):
    ...
    def dump(self):
        ...
        for column in cols:
            ...
            outputs.append('%7s' % self.__class__.calc_precision(column['type'], column['length'], column['precision'], column['scale']))
            if not column['nullable']:
                outputs.append(' NOT NULL')
            outputs.append('\n')
...
class CodeObject(object):
    ...
    def __init__(self, code_object_name, code_object_type, cursor=None, schema=None):
        """
