...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/gerald/tests/test_oracle_schema.py
...
class TestSchemaAddTable(object):
    ...
    def test_new_table(self):
        ...
        new_table.columns['col1'] = {'sequence': 1, 'name': 'col1', 'type': 'VARCHAR2', 
           'length': 20, 'nullable': False, 'default': "'Default value'"}
...
class TestNewTableParent(object):
    ...
    def setup_class(self):
        ...
        self.column = {'sequence': 1, 'name': self.column_name, 'type': 'VARCHAR2', 'length': 20, 
           'nullable': False}
...
class TestTableAndFkFromDb(object):
    ...
    def setup_method(self, method):
        self.child_table = Table(self.child_table_name, self.cursor, self.schema_name)
        self.parent_table = Table(self.parent_table_name, self.cursor, self.schema_name)
...
class TestCodeObjectFromDb(TestCodeObjectParent):
    ...
    def test_get_co_from_db(self):
        ...
        proc = CodeObject(self.proc_name, self.proc_type, self.cursor, self.schema)
