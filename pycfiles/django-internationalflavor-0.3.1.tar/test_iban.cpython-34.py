...
class IBANTestCase(TestCase):
    valid = {'GB82WeST12345698765432': 'GB82WEST12345698765432', 
    ...
    invalid = {'GB82WEST1234569876543': [
                               'This IBAN does not match the requirements for GB.'], 
     'CA34CIBC123425345': [
                           'CA IBANs are not allowed in this field.'], 
     'GB29ÉWBK60161331926819': [
     ...
     'SA0380000000608019167519': [
                                  'This IBAN does not have a valid checksum.'], 
     'EE012200221111099080': [
                              'This IBAN does not have a valid checksum.'], 
     'NL91ABNB0417164300': [
                            'This IBAN does not have a valid checksum.'], 
     'MU17BOMM0101101030300200000MUR12345': [
                                             'This IBAN does not match the requirements for MU.',
                                             'Ensure this value has at most 34 characters (it has 35).'], 
     'EG1100006001880800100014553': [
                                     'EG IBANs are not allowed in this field.']}
    ...
    def test_validator(self):
        ...
        self.assertRaisesMessage(ValidationError, 'This IBAN does not start with a country code and checksum, or contains invalid characters.', validator, 'NL02 ABNA 0123 4567 89')
    ...
    include_countries_valid = {'NL02ABNA0123456789': 'NL02ABNA0123456789', 
     'BE68539007547034': 'BE68539007547034', 
     'LU280019400644750000': 'LU280019400644750000'}
    include_countries_invalid = {'GB82WEST12345698765432': [
                                'GB IBANs are not allowed in this field.']}

    def test_include_countries_form_field(self):
        self.assertFieldOutput(IBANFormField, field_kwargs={'countries': self.include_countries}, valid=self.include_countries_valid, invalid=self.include_countries_invalid)
...
class BICTestCase(TestCase):
    ...
    invalid = {'NEDSZAJJXX': [
                    'This Bank Identifier Code (BIC) is invalid.'], 
     'CIBCJJH2': [
                  'JJ is not a valid country code.'], 
     'DÉUTDEFF': [
                  'This Bank Identifier Code (BIC) is invalid.']}
