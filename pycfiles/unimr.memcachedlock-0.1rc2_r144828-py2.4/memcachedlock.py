...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/unimr/memcachedlock/memcachedlock.py
# Compiled at: 2010-09-23 08:17:22
import os, md5, time
try:
...
class MemcachedRLock(object):
    ...
    def __repr__(self):
        return '<%s(owner: %s, count: %d)>' % (self.__class__.__name__, self.__owner, self.__count)
...
def lock_getter(key, timeout=30, intervall=0.05):
    ...
    client = MemcachedClient(servers=servers, defaultNS=unicode(PROJECTNAME), defaultAge=timeout, trackKeys=True)
