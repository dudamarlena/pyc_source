...
# [GCC 8.4.0]
# Embedded file name: /Users/yhoorneman/Git/opsgenielib/opsgenielib/opsgenielib.py
# Compiled at: 2020-01-04 12:48:32
# Size of source mod 2**32: 30933 bytes
"""
...
"""
import re, logging, json, urllib.parse
...
class Team:
    ...
    def integrations(self):
        ...
        url = f"{self._server._base_url}/v2/integrations?teamName={self.name}"
        self._logger.debug('Making a call to "%s"', url)
        response = self._server._session.get(url)
        if not response.ok:
...
class Integration:
    __doc__ = 'Models the integration.'
...
class Opsgenie:
    ...
    def _authenticate(self, api_key):
        session = Session()
        session.headers.update({'Authorization':f"GenieKey {api_key}",  'Content-Type':'application/json'})
    ...
    def set_maintenance_schedule(self, team_id, start_date, end_date, rules_type, description, state, rules_id):
        """Creation of a maintenance policy for a specified schedule. Returns the json."""
        url = f"{self._base_url}/v1/maintenance"
        payload = {'teamId':f"{team_id}", 
    ...
    def set_maintenance_hours(self, team_id, hours, rules_type, description, state, rules_id):
    ...
    def query_alerts(self, query):
        ...
        results = response.json()['data']
        while 'next' in response.json()['paging']:
            if not bool(re.search('2\\d\\d\\d', response.json()['paging']['next'])):
                url = response.json()['paging']['next']
                self._logger.debug('Making a call to "%s"', url)
                response = self._session.get(url)
                results += response.json()['data']
    ...
    def create_alert_policy(self, name, filter, policy_description, team_id, enabled=False):
        """POST request to create an alert policy with simplified logic."""
        payload = {'type':'alert', 
         'name':f"{name}", 
         'enabled':f"{enabled}", 
         'description':f"{policy_description}", 
         'filter':{'type':'match-any-condition', 
          'conditions':[
           {'field':'description', 
            'operation':'matches', 
            'expectedValue':f"{filter}"},
    ...
    def list_schedule_timeline_by_team_name(self, team_name):
    ...
    def set_override_scheduled(self, team_name, start_date, end_date, user):
        ...
        start_date_utc = start.astimezone(pytz.utc).strftime('%Y-%m-%dT%H:%M:%SZ')
        end_date_utc = end.astimezone(pytz.utc).strftime('%Y-%m-%dT%H:%M:%SZ')
        url = f"{self._schedules_url}/{team_name}/overrides?scheduleIdentifierType=name"
        payload = {'user':{'type':'user', 
          'username':f"{user}"}, 
