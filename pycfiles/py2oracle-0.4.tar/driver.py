...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/py2opencl/driver.py
...
class Py2OpenCL(object):
    ...
    def __init__(self, lmb, prompt=False, user_dev_selection=None, bindings=None):
        ...
        if prompt:
            self.user_dev_selection = None if Py2OpenCL.only_one_device() else self.init()
        self.ctx = cl.create_some_context(interactive=False, answers=self.user_dev_selection) if self.user_dev_selection else cl.create_some_context()
    ...
    def bind(self, *arrays, **kw):
        ...
        for a in self.arrays:
            ...
            if length is None:
                length = len(a)
            elif not len(a) == length:
                raise AssertionError

        for t in sorted(set(self.types)):
            try:
                assert self.ctx.devices[0].__getattribute__('preferred_vector_width_' + nptyp_to_cl[t]), 'unsupported type on this platform:: numpy:%s openCL:%s' % (t, nptyp_to_cl[t])
            except AttributeError:
                pass

        self.argnames, self.kernel, cl_return_typ = function_to_kernel(self.lmb, self.types, self.shape, bindings=self.bindings, return_type=kw.get('return_type'))
    ...
    def apply(self, *new_arrays):
        ...
        buffs = [ cl.Buffer(self.ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=arr) for arr in self.arrays
                ]
