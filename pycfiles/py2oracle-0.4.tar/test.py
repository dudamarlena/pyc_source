...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/py2opencl/test/test.py
# Compiled at: 2014-11-11 10:50:58
__doc__ = '\nsimple test of py2opencl\n'
import numpy as np, time, os.path
...
def main():
    ...
    try:
        s.map((100 * np.random.rand(int(1000.0))).astype('int64'))
    except TypeError:
        pass
    else:
    ...
    arr = np.random.rand(int(10000.0)).astype('float32')
    ...
    for size in (10000.0, 100000.0, 1000000.0, 10000000.0):
        a, b = np.random.rand(int(10000000.0)).astype(np.float32), np.random.rand(int(10000000.0)).astype(np.float32)
        before = time.time()
        res = two.map(a, b)
