...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\dao\builtins\tests\testparser.py
...
class TestParameterize:
    ...
    def test_chars(self):
        ...
        eq_(eval(let([(chars, function(((x, cs), and_p(char(x), contain(cs, x)))))], parse_text(chars(x, 'a'), 'a'))), True)
...
class Testterminal:
    ...
    def test_number(self):
        ...
        eq_(eval(begin(parse_text(number(z), '0232'), z)), 154)
...
class Testrule:
    ...
    def test_two_rule(self):
        eq_(eval(letr([(f, function(((), char('a')), ((), char('b'))))], parse_text(f(), 'a'), parse_text(f(), 'b'))), 'b')

    def test_right_recursive1(self):
        function1 = function(((), and_p(char('a'), f())), (
         (), char('b')))
        eq_(eval(letr([(f, function1)], parse_text(f(), 'b'), parse_text(f(), 'ab'), parse_text(f(), 'aab'))), 'b')
    ...
    def test_right_recursive2(self):
        ...
        function1 = [
         (p,
          function(((), and_p(char(x), p())), (
           (), char(x))))]
        eq_(eval(letr(function1, parse_text(p(), 'a'), parse_text(p(), 'ab'), parse_text(p(), 'abc'))), 'c')
...
class TestKleeneByfunction:
    ...
    def testKleene4(self):
        (x, _, c, s, kleene) = (
         Var('x'), DummyVar('_'), Var('c'), Var('s'), Var('kleene'))
...
class testIndentUndent:
    ...
    def testIndentUndent(self):
        (_, n, s, line) = (
         DummyVar('_'), Var('n'), Var('s'), Var('line'))
        space = char(' ')
        ruleList = [
         (s,
...
class TestExpression:
    ...
    def testRecursiveReturnValue1(self):
        (E, F, G, e, e1) = (
         Var('E'), Var('F'), Var('G'), Var('e'), Var('e1'))
