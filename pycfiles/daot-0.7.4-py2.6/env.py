...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\dao\env.py
# Compiled at: 2011-11-06 08:10:58
    ...
    try:
        ...
        if cont is stop_cont:
            if next_cont is None:
                return cont
            return next_cont
        return unwind(cont.cont, form_value, tag, stop_cont, solver, next_cont)
...
class BlockEnvironment(ExtendEnvironment):
    ...
    def _repr(self):
        return 'BENV %s:%s' % (self.label, self.bindings)
...
class ModuleEnvironment(ExtendEnvironment):
    ...
    def lookup(self, var):
        try:
            ...
            raise NotExistVariable(var)

    def _repr(self):
        return 'MEnv%s' % self.bindings