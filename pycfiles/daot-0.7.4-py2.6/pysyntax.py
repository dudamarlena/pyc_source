...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\dao\pysyntax.py
...
def lead_class(klass):
    ...
    for (a, value) in klass.__dict__.items():
        if not a.startswith('__init__') and isinstance(value, type(lead_class)):
...
def _lead_function(klass, function):
    return lambda self, *args, **kw: function(klass(self.__form_name__, self.__form_grammar__), *args, **kw)
...
(__lt__, __le__, __eq__, __ne__, __gt__, __ge__, __getattr__, __call__, __getitem__, __iter__, __add__, __sub__, __mul__, __floordiv__, __div__, __truediv__, __mod__, __pow__, __lshift__, __rshift__, __and__, __xor__, __or__, __neg__, __pos__, __abs__, __invert__) = range(27)
names = ('__lt__, __le__, __eq__, __ne__, __gt__, __ge__, __getattr__, __call__, __getitem__, __iter__, __add__, __sub__, __mul__, __floordiv__, __div__, __truediv__, __mod__, __pow__, __lshift__, __rshift__, __and__, __xor__, __or__, __neg__, __pos__, __abs__, __invert__').split(', ')
...
class FormTraveller(object):
    ...
    def __lt__(self, other):
        self.__operator_data__.append((__lt__, other))
        return self

    def __le__(self, other):
        self.__operator_data__.append((__le__, other))
        return self

    def __eq__(self, other):
        self.__operator_data__.append((__eq__, other))
        return self

    def __ne__(self, other):
        self.__operator_data__.append((__ne__, other))
        return self

    def __gt__(self, other):
        self.__operator_data__.append((__gt__, other))
        return self

    def __ge__(self, other):
        self.__operator_data__.append((__ge__, other))
        return self

    def __getattr__(self, name):
        self.__operator_data__.append((__getattr__, name))
        return self
    ...
    def __getitem__(self, key):
        self.__operator_data__.append((__getitem__, key))
        return self

    def __add__(self, other):
        self.__operator_data__.append((__add__, other))
        return self

    def __sub__(self, other):
        self.__operator_data__.append((__sub__, other))
        return self

    def __mul__(self, other):
        self.__operator_data__.append((__mul__, other))
        return self

    def __floordiv__(self, other):
        self.__operator_data__.append((__floordiv__, other))
        return self
    ...
    def __pow__(self, other):
        self.__operator_data__.append((__pow__, other))
        return self

    def __lshift__(self, other):
        self.__operator_data__.append((__lshift__, other))
        return self

    def __rshift__(self, other):
        self.__operator_data__.append((__rshift__, other))
        return self

    def __and__(self, other):
        self.__operator_data__.append((__and__, other))
        return self
    ...
    def __or__(self, other):
        self.__operator_data__.append((__or__, other))
        return self

    def __iter__(self):
        self.__operator_data__.append(__iter__)
        return self

    def __neg__(self):
        self.__operator_data__.append(__neg__)
        return self

    def __pos__(self):
        self.__operator_data__.append(__pos__)
        return self

    def __abs__(self):
        self.__operator_data__.append(__abs__)
        return self

    def __invert__(self):
        self.__operator_data__.append(__invert__)
        return self
    ...
    def ____repr____(self):
        ...
        for x in self.__operator_data__:
            if x == __neg__:
                ...
                result += '>=%s' % dao_repr(x[1])
            elif x[0] == __getattr__:
                result += '.%s' % dao_repr(x[1])
            elif x[0] == __call__:
                result += '(%s)' % dao_repr(x[1])
            elif x[0] == __getitem__:
                result += '[%s]' % dao_repr(x[1])
            elif x[0] == __iter__:
                ...
                result += '*%s' % dao_repr(x[1])
            elif x[0] == __floordiv__:
                result += '//%s' % dao_repr(x[1])
            elif x[0] == __div__:
                result += '/%s' % dao_repr(x[1])
            elif x[0] == __truediv__:
                result += '/%s' % dao_repr(x[1])
            elif x[0] == __mod__:
                result += '%%%s' % dao_repr(x[1])
            elif x[0] == __pow__:
...
def binary(attr):
    ...
    def func(solver, cont, argument=None):
        ...
        if pos == len(syntax_result):
            return
        else:
            try:
                if syntax_result[pos][0] != attr:
                    return
            except:
                return
            else:
...
def call(solver, cont, args=None, kwargs=None):
    ...
    if pos == len(syntax_result):
        ...
        try:
            if syntax_result[pos][0] != __call__:
                return
        except:
            return
        else:
...
pos = unary(__pos__)()
neg = unary(__neg__)()
invert = unary(__invert__)()
abs = unary(__abs__)()
pow = binary(__pow__)
getattr = binary(__getattr__)
getitem = binary(__getitem__)
...
def words(text):
    return [ getattr(w.strip()) for w in text.split(',') ]


def attr_item(name):
    return lambda arg: and_p(getattr(name), getitem(arg))


def attr_call(name):
    return lambda *args: and_p(getattr(name), call(*args))
