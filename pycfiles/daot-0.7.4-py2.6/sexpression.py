...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\samples\sexpression.py
...
sexpression_rules = [
 ...
 (
  ...
  function((
   [
    Result],
   and_p(char('{'), sexpression(Expr2), char('}'), setvalue(Result, eval_(pycall(cons2tuple, Expr2))))), (
   [
 ...
 (
  ...
  function((
   [
    Result],
   and_p(sexpression(Expr2), eoi, is_(Result, eval_(pycall(cons2tuple, Expr2)))))))]
