...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\dao\builtins\terminal.py
...
def lead_chars(solver, cont, chars):
    ...
    if last_char_(solver.parse_state) not in chars:
        return
    yield (
     cont, True)
...
def not_lead_chars(solver, cont, chars):
    chars = deref(chars, solver.env)
    assert isinstance(chars, str)
    if last_char_(solver.parse_state) in chars:
        return
    yield (
     cont, True)
...
def follow_chars(solver, cont, chars):
    chars = deref(chars, solver.env)
    assert isinstance(chars, str)
    if not eoi_(solver.parse_state) and next_char_(solver.parse_state) not in chars:
        return
    yield (
     cont, True)
...
def lead_string(solver, cont, string):
    ...
    if not parsed_(solver.parse_state).endwith(strArgument):
        return
    yield (
     cont, True)
...
def not_lead_string(solver, cont, string):
    ...
    if parsed_(solver.parse_state).endwith(string):
        return
    solver.value = True
...
def follow_string(solver, cont, strArgument):
    ...
    if not left_(solver.parse_state).startswith(strArgument.name):
        raise UnifyFail
    solver.value = True
...
def not_follow_string(solver, cont, string):
    string = string.deref(solver.env)
    if left_(solver.parse_state).startswith(string.name):
        return
    solver.value = True
...
def char_between(lower, upper):
    return char_on_predicate(lambda char: lower <= char <= upper, 'charbetween <%s-%s>' % (lower, upper))


def char_in(string, repr_string=''):
    return char_on_predicate(lambda char: char in string, repr_string or 'char_in ' + string)
...
uletter = char_on_predicate(lambda char: char == '_' or 'a' <= char <= 'z' or 'A' <= char <= 'Z', '_letter')
_letter_digit_test = lambda char: char == '_' or '0' <= char <= '9' or 'a' <= char <= 'z' or 'A' <= char <= 'Z'
