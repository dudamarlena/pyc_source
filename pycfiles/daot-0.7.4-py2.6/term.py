...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\dao\term.py
...
def unify_rule_head(value, head, env, subst):
    if isinstance(head, Var):
        ...
        try:
            ...
            if (isinstance(head, list) or isinstance(head, tuple)) and (isinstance(value, list) or isinstance(value, tuple)):
...
def unify_list_rule_head(values, args, env, subst):
    for _ in apply_generators(tuple(unify_rule_head(x, y, env, subst) for (x, y) in zip(values, args))):
        yield True
...
class Command(object):
    ...
    def run(self, solver, cont, values):
        ...
        if solver.parse_state is None or not self.memorable:
            ...
            sign_state = (
             (
              self, signatures), hash_parse_state(solver.parse_state))
            ...
            if memo_results is not None:
                for (_, c) in sign_state2cont:
                    if c.cont_order > cont.cont_order:
                        continue
...
class Var(Command):
    ...
    def unify(self, other, env, occurs_check=False):
        ...
        if isinstance(self, Var):
            if isinstance(other, Var) and other is self:
                yield True
                return
            if occurs_check and contain_var(other, self):
                return
            ...
            if occurs_check and contain_var(self, other):
                return
    ...
    def free(self, env):
        return isinstance(self.deref(env), Var)
...
class DummyVar(Var):
    ...
    def __init__(self, name='_v', index=0):
...
def vars(names):
    return [ var(x.strip()) for x in names.split(',') ]


def dummies(names):
    return [ dummy(x.strip()) for x in names.split(',') ]
...
class ClosureVar(Var):
    ...
    def getvalue(self, env):
        return getvalue(self.value, env)
    ...
    def closure(self, env):
        ...
        if value is self.var:
            ...
            return ClosureVar(self.var, value)

    def free(self, env):
        return isinstance(self.value, Var)

    def __repr__(self):
        return '(%s:%s)' % (self.var, self.value)

    def __eq__(self, other):
        return self.var is other
...
class CommandCall(Command):
    ...
    def to_sexpression(self):
        return (
         to_sexpression(self.operator),) + tuple(to_sexpression(x) for x in self.operand)
