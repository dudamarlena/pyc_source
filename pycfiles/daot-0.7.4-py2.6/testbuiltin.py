...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\dao\builtins\tests\testbuiltin.py
...
class Testfindall:
    ...
    def test_findall(self):
        ...
        eq_(eval(let([(f, function(((), 2), ((), 3)))], findall(is_(x, f()), x, y), y)), [2, 3])
...
class TestRule:
    ...
    def test_assert(self):
        ...
        eq_(eval(let([(f, function(([1], 1)))], assert_(f, [2], [2]), f(2))), 2)

    def test_asserta(self):
        from dao.builtins.rule import asserta
        eq_(eval(let([(f, function(([1], 1)))], asserta(f, [2], [2]), f(2))), 2)

    def test_replace(self):
        from dao.builtins.rule import replace
        eq_(eval(let([(f, function(([1], 1), ([2], 2)))], replace(f, [2], [3]), f(2))), 3)
...
class TestQuasiquote:
    ...
    def test_unquote_slice(self):
        eq_(eval(quasiquote((unquote(add(1, 1)), unquote_slice(quote((3, 4)))))), (2,
                                                                                   3,
                                                                                   4))
