...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\dao\special.py
...
class set_list(SpecialForm):
    ...
    def __eq__(self, other):
        return self.vars == other.vars and self.exp == other.exp

    def __repr__(self):
        return 'set(%s %s)' % (self.vars, self.exp)
...
class set_outer(SpecialForm):
    ...
    def __repr__(self):
        return 'set_outer(%s, %s)' % (self.var, self.exp)
...
class get_global(SpecialForm):
    ...
    def __repr__(self):
        return 'get_global(%s, %s)' % (self.var, self.exp)
...
class set_global(SpecialForm):
    ...
    def __repr__(self):
        return 'set_global(%s, %s)' % (self.var, self.exp)
...
def make_if_cont(then, els, cont):
    ...
    def if_cont(value, solver):
        if value:
            yield (solver.cont(then, cont), value)
        elif els is not None:
            yield (solver.cont(els, cont), value)
        else:
            yield (
             cont, None)
        return
...
def make_iff_cont(then, clauses, els, cont):
    ...
    def iff_cont(value, solver):
        if value:
            ...
            if len(clauses) == 1:
                ifcont = make_if_cont(clauses[0][1], els, cont)
            else:
                ifcont = make_iff_cont(clauses[0][1], clauses[1:], els, cont)
            yield (
             solver.cont(clauses[0][0], ifcont), value)
...
class CaseForm(SpecialForm):
    ...
    def cont(self, cont, solver):
        ...
        def case_cont(value, solver):
            try:
                exps = self.cases[value]
            except:
                exps = self.els

            yield (
             solver.exps_cont(exps, cont), value)
...
class EachForm(RepeatForm):
    ...
    def tag_loop_label(self, tagger):
        ...
        return set(iterator, make_iter(quote(self.iterator))) + block(label, pytry(setvar, DaoStopIteration, exit_block(label)), *(body + [continue_block(label)]))
...
class exit(ParserForm):
    ...
    def __eq__(self, other):
        return self.value == other.value and self.type == other.type and self.level == other.level and self.label == other.label
...
class next(ParserForm):
    ...
    def __eq__(self, other):
        return self.type == other.type and self.level == other.level and self.label == other.label
    ...
    def apply(self, solver, cont, values, signatures):
        ...
        if arity == 0:
            ...
            for signature in signatures:
                if signature == (signature[0], Var):
                    continue
                else:
                    var_sign = (
                     signature[0], Var)
                    index_set &= sign2index.get(var_sign, pyset()) | sign2index.get(signature, pyset())
...
class UserFunction(Rules, Function):
    ...
    def __repr__(self):
        return 'fun(%s)' % repr(self.arity2rules)
...
class UserMacro(Rules, Macro):
    ...
    def __repr__(self):
        return 'macro(%s)' % repr(self.arity2rules)
...
class exit_block(SpecialForm):
    ...
    def __repr__(self):
        return 'exit_block(%s)' % self.label
...
def lookup(cont, tag, stop_cont, solver):
    try:
        return cont.lookup(cont, tag, stop_cont, solver)
...
def label_cont_lookup(cont, tag, stop_cont, solver):
    if tag == cont.tag:
        ...
        return lookup(cont, tag, stop_cont, solver)
...
class throw(SpecialForm):
    ...
    def __repr__(self):
        return 'throw(%s,%s)' % (repr(self.tag), repr(self.form))
...
class unwind_protect(SpecialForm):
    ...
    def to_sexpression(self):
        return (
         unwind_protect, to_sexpression(self.form)) + to_sexpression(self.cleanup)
