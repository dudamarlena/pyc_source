...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\dao\dinpy\dexpr.py
...
class _SymbolExpression:
    ...
    def __rlt__(self, other):
        return _lt(other, self)

    def __le__(self, other):
        return _le(self, other)

    def __rle__(self, other):
        return _le(other, self)
...
class _DummyVarSymbol(_VarSymbol):
    ...
    def __repr__(self):
        return 'DummySymbol(%s)' % self.name
...
def _get_assign_vars_chain(exp):
    if isinstance(exp, _VarSymbol):
        return (varcache(exp.name),)
    if isinstance(exp, _lshift):
...
class _neg(_Unary):
    ...
    def ___parse___(self, parser):
        if isinstance(self.x, _Unary) and isinstance(self.x.x, _VarSymbol):
            ...
            return arith.neg(parser.parse(self.x))
...
class _call(_SymbolExpression):
    ...
    def ____repr____(self):
        return '%s(%s,%s)' % ((',').join([ dao_repr(a) for a in self.args ]),
         ', ' + (',').join([ '%s=%s' % (dao_reprk, dao_repr(a)) for (k, a) in self.kwargs ]) if self.kwargs else '')