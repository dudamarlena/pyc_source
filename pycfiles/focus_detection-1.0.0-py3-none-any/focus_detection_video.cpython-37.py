...
# [GCC 8.4.0]
# Embedded file name: c:\users\jmd0421\dev\focus-detection\focus_detection\focus_detection_video.py
# Compiled at: 2019-10-17 15:45:14
...
class App(QMainWindow):
    ...
    def __init__(self, camera, threshold):
        ...
        os.makedirs('focus-detection-out/blurry', exist_ok=True)
        os.makedirs('focus-detection-out/not-blurry', exist_ok=True)
        self.title = 'Focus Detection'
        ...
        while self.is_running:
            ...
            if time() - self._last_check >= 0.5:
                ...
                if self._save_as_blurry.is_set():
                    ...
                    cv2.imwrite(f"focus-detection-out/blurry/{time()}.jpg", frame)
                    self._save_as_blurry.clear()
                if self._save_as_not_blurry.is_set():
                    print('Saving as not blurry!')
                    cv2.imwrite(f"focus-detection-out/not-blurry/{time()}.jpg", frame)
...
def main():
    parser = argparse.ArgumentParser(description='Focus Detection')
