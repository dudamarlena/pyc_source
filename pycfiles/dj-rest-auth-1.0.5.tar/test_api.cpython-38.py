...
# [GCC 8.4.0]
# Embedded file name: /Users/michael/Projects/dj-rest-auth/dj_rest_auth/tests/test_api.py
# Compiled at: 2020-04-21 21:33:28
# Size of source mod 2**32: 23011 bytes
import json
import allauth.account as account_app_settings
...
try:
    ...
    class APIBasicTests(TestsMixin, TestCase):
        ...
        BASIC_USER_DATA = {'first_name':'John', 
         'last_name':'Smith', 
         'email':EMAIL}
        ...
        def test_login_by_email(self):
            ...
            payload = {'email':self.EMAIL.lower(), 
             'password':self.PASS}
            self.post((self.login_url), data=payload, status_code=400)
            self.post((self.password_change_url), status_code=403)
        ...
        def test_password_change(self):
            ...
            self.token = self.response.json['key']
            new_password_payload = {'new_password1':'new_person', 
             'new_password2':'new_person'}
            self.post((self.password_change_url),
            ...
            self.post((self.login_url), data=login_payload, status_code=200)
            new_password_payload = {'new_password1':'new_person1', 
             'new_password2':'new_person'}
            self.post((self.password_change_url),
              data=new_password_payload,
              status_code=400)
        ...
        def test_password_change_with_old_password(self):
            ...
            self.post((self.login_url), data=login_payload, status_code=200)
            self.token = self.response.json['key']
            new_password_payload = {'old_password':'%s!' % self.PASS, 
             'new_password1':'new_person', 
             'new_password2':'new_person'}
            self.post((self.password_change_url),
              data=new_password_payload,
              status_code=400)
            new_password_payload = {'old_password':self.PASS, 
             'new_password1':'new_person', 
             'new_password2':'new_person'}
            self.post((self.password_change_url),
              data=new_password_payload,
        ...
        def test_password_reset(self):
            ...
            data = {'new_password1':self.NEW_PASS, 
        ...
        def test_registration_with_email_verification(self):
            ...
            self.post((self.verify_email_url),
        ...
        @override_settings(REST_USE_JWT=True)
        @override_settings(JWT_AUTH_COOKIE='jwt-auth')
        @override_settings(REST_FRAMEWORK=dict(DEFAULT_AUTHENTICATION_CLASSES=[
        ...
        def test_blacklisting_not_installed(self):
            ...
            self.assertEqual(resp.data['detail'], 'Neither cookies or blacklist are enabled, so the token has not been deleted server side. Please make sure the token is deleted client side.')
        ...
        def test_login_csrf(self):
            ...
            request = factory.post(self.login_url, {'username':self.USERNAME, 
             'password':self.PASS})
