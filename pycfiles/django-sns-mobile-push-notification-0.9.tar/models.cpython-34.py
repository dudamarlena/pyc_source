...
# [GCC 8.4.0]
# Embedded file name: /Users/rvaziri/django-sns-mobile-push-notification/sns_mobile_push_notification/models.py
# Compiled at: 2018-04-29 16:11:10
...
class Device(models.Model):
    ...
    def send(self, notification_type, text, data, title):
        ...
        :return: response from SNS
        """
        log = Log(device=self, notification_type=notification_type)
        log.save()
        client = Client()
        if self.is_android:
            message, response = client.publish_to_android(arn=self.arn, text=text, title=title, notification_type=notification_type, data=data, id=log.id)
        elif self.is_ios:
            message, response = client.publish_to_ios(arn=self.arn, text=text, title=title, notification_type=notification_type, data=data, id=log.id)
        log.message = message
