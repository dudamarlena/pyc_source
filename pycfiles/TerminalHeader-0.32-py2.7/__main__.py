...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\theader\__main__.py
# Compiled at: 2016-01-27 21:32:37
import os, sys
...
def can_add(path):
    ...
    def can_add_to_dir(path):
        return enable_add_to_hidden_dir or not enable_add_to_hidden_dir and not is_hidden(path)

    if not os.path.exists(path):
        return False
    enable_add_to_hidden_dir = Settings().get('enable_add_header_to_hidden_dir')
    enable_add_to_hidden_file = Settings().get('enable_add_header_to_hidden_file')
    if os.path.isfile(path):
        if can_add_to_dir(os.path.dirname(path)):
            if enable_add_to_hidden_file or not enable_add_to_hidden_file and not is_hidden(path):
                return True
...
def get_args(syntax_type, options={}):
    ...
    format = get_strftime()
    args.update({'create_time': datetime.now().strftime(format), 
    ...
    for option in options:
        ...
        if not os.path.exists(filepath):
            ...
            with open(filepath, 'r') as (f):
                if option == 'header':
                    contents = header + f.read().replace('\\r', '')
                elif option == 'body':
                    contents = f.read().replace('\\r', '') + header
            with open(filepath, 'w') as (f):
                f.write(contents)
