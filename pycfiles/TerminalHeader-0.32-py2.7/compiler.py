...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\theader\jinja2\compiler.py
...
from jinja2._compat import range_type, text_type, string_types, iteritems, NativeStringIO, imap
operators = {'eq': '==', 
   'ne': '!=', 
   'gt': '>', 
   'gteq': '>=', 
...
unoptimize_before_dead_code = bool(getattr(unoptimize_before_dead_code(), '__closure__', None))
...
class CodeGenerator(NodeVisitor):
    ...
    def function_scoping(self, node, frame, children=None, find_special=True):
        ...
        overridden_closure_vars = func_frame.identifiers.undeclared & func_frame.identifiers.declared & (func_frame.identifiers.declared_locally | func_frame.identifiers.declared_parameter)
        if overridden_closure_vars:
            self.fail("It's not possible to set and access variables derived from an outer scope! (affects: %s)" % (', ').join(sorted(overridden_closure_vars)), node.lineno)
    ...
    def visit_FromImport(self, node, frame):
        ...
        for name in node.names:
            if isinstance(name, tuple):
                ...
                alias = name
            self.writeline('l_%s = getattr(included_template, %r, missing)' % (
             alias, name))
            self.writeline('if l_%s is missing:' % alias)
            self.indent()
            self.writeline('l_%s = environment.undefined(%r %% included_template.__name__, name=%r)' % (
             alias,
    ...
    def visit_For(self, node, frame):
        ...
        if 'loop' not in aliases and 'loop' in find_undeclared(node.iter_child_nodes(only=('else_',
                                                                                           'test')), ('loop', )):
