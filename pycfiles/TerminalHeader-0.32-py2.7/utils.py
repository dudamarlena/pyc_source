...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\theader\jinja2\utils.py
...
def urlize(text, trim_url_limit=None, nofollow=False):
    ...
    for i, word in enumerate(words):
        ...
        if match:
            ...
            if middle.startswith('www.') or '@' not in middle and not middle.startswith('http://') and not middle.startswith('https://') and len(middle) > 0 and middle[0] in _letters + _digits and (middle.endswith('.org') or middle.endswith('.net') or middle.endswith('.com')):
