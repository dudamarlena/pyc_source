...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\theader\jinja2\parser.py
...
class Parser(object):
    ...
    def _fail_ut_eof(self, name, end_token_stack, lineno):
        ...
        if name is None:
            ...
            message = [
             "Encountered unknown tag '%s'." % name]
    ...
    def parse_test(self, node):
        ...
        if self.stream.current.type == 'lparen':
            args, kwargs, dyn_args, dyn_kwargs = self.parse_call(None)
                                          ...
                                          'lbracket', 'lbrace') and not self.stream.current.test_any('name:else', 'name:or', 'name:and'):
