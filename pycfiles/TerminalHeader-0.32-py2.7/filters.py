...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\theader\jinja2\filters.py
...
def make_attrgetter(environment, attribute):
    ...
    """
    if not isinstance(attribute, string_types) or '.' not in attribute and not attribute.isdigit():
...
def do_urlencode(value):
    ...
    if itemiter is None:
        ...
        return ('&').join(unicode_urlencode(k) + '=' + unicode_urlencode(v) for k, v in itemiter)
...
def do_xmlattr(_eval_ctx, d, autospace=True):
    ...
    """
    rv = (' ').join('%s="%s"' % (escape(key), escape(value)) for key, value in iteritems(d) if value is not None and not isinstance(value, Undefined))
    if autospace and rv:
        rv = ' ' + rv
...
def do_title(s):
    ...
    """
    rv = []
    for item in re.compile('([-\\s]+)(?u)').split(soft_unicode(s)):
        if not item:
            continue
...
FILTERS = {'attr': do_attr, 'replace': do_replace, 
   'upper': do_upper, 
   'lower': do_lower, 
   'escape': escape, 
   'e': escape, 
