...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\theader\jinja2\lexer.py
...
try:
    ...
    name_re = re.compile('[%s][%s]*' % (_stringdefs.xid_start,
...
def _describe_token_type(token_type):
    ...
    return {TOKEN_COMMENT_BEGIN: 'begin of comment', TOKEN_COMMENT_END: 'end of comment', 
...
class TokenStream(object):
    r"""A token stream is an iterable that yields :class:`Token`\s.  The
    ...
    def expect(self, expr):
        ...
        if not self.current.test(expr):
            ...
            raise TemplateSyntaxError('expected token %r, got %r' % (
             expr, describe_token(self.current)), self.current.lineno, self.name, self.filename)
        try:
            return self.current
        finally:
            next(self)
...
class Lexer(object):
    ...
    def __init__(self, environment):
        ...
        if environment.lstrip_blocks:
            no_lstrip_re = e('+')
            block_diff = c('^%s(.*)' % e(environment.block_start_string))
            m = block_diff.match(environment.comment_start_string)
            no_lstrip_re += m and '|%s' % e(m.group(1)) or ''
            m = block_diff.match(environment.variable_start_string)
            no_lstrip_re += m and '|%s' % e(m.group(1)) or ''
            comment_diff = c('^%s(.*)' % e(environment.comment_start_string))
            m = comment_diff.match(environment.variable_start_string)
            no_variable_re = m and '(?!%s)' % e(m.group(1)) or ''
            lstrip_re = '^[ \\t]*'
            block_prefix_re = '%s%s(?!%s)|%s\\+?' % (
             lstrip_re,
             e(environment.block_start_string),
             no_lstrip_re,
             e(environment.block_start_string))
            comment_prefix_re = '%s%s%s|%s\\+?' % (
             lstrip_re,
             e(environment.comment_start_string),
             no_variable_re,
             e(environment.comment_start_string))
                  ...
                  (
                   c('(.*?)(?:%s)' % ('|').join([
                    '(?P<raw_begin>(?:\\s*%s\\-|%s)\\s*raw\\s*(?:\\-%s\\s*|%s))' % (
                     ...
                     e(environment.block_end_string))] + [ '(?P<%s_begin>\\s*%s\\-|%s)' % (n, r, prefix_re.get(n, r)) for n, r in root_tag_rules
                                                         ])),
                   (
           ...
           TOKEN_BLOCK_BEGIN: [
                             (
                              c('(?:\\-%s\\s*|%s)%s' % (
                               ...
                               e(environment.block_end_string),
                               block_suffix_re)),
                              TOKEN_BLOCK_END, '#pop')] + tag_rules, 
           TOKEN_VARIABLE_BEGIN: [
                                (
                                 c('\\-%s\\s*|%s' % (
                                  e(environment.variable_end_string),
                                  e(environment.variable_end_string))),
                                 TOKEN_VARIABLE_END, '#pop')] + tag_rules, 
           TOKEN_RAW_BEGIN: [
                           (
                        ...
                        if data == '{':
                            ...
                            if expected_op != data:
                                raise TemplateSyntaxError("unexpected '%s', expected '%s'" % (
                                 data, expected_op), lineno, name, filename)
