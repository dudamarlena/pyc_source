...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.5/site-packages/PySquashfsImage/PySquashfsImage.py
# Compiled at: 2016-10-11 06:35:00
# Size of source mod 2**32: 33633 bytes
"""
...
SQUASHFS_INVALID = 281474976710655
SQUASHFS_INVALID_FRAG = 4294967295
SQUASHFS_INVALID_XATTR = 4294967295
SQUASHFS_INVALID_BLK = 18446744073709551615
SQUASHFS_USED_BLK = SQUASHFS_INVALID_BLK - 1
SQUASHFS_DIR_TYPE = 1
...
def SQUASHFS_MKINODE(A, B):
    return (A << 16) + B & 18446744073709551615


def SQUASHFS_MK_VFS_INODE(a, b):
    return (a << 8) + (b >> 2) + 1 & 4294967295
...
_compressors = (
 _Compressor(), _ZlibCompressor(), _XZCompressor())
...
class _Squashfs_commons:
    ...
    def makeBufInteger(self, buf, start, lenght):
        ...
        for i in range(start, start + lenght):
            if pyVersionTwo:
                ret += (ord(buf[i]) & 255) * pwr
            else:
                ret += (int(buf[i]) & 255) * pwr
            pwr *= 256
    ...
    def autoMakeBufInteger(self, buf, start, length):
        """ Assemble multibyte integer """
        return (
         self.makeBufInteger(buf, start, length), start + length)
...
class _Xattr_id(_Squashfs_commons):
...
class SquashFsImage(_Squashfs_commons):
    ...
    def initialize(self, myfile):
        self._SquashFsImage__read_super(myfile)
    ...
    def read_block(self, myfile, start):
        ...
        if SQUASHFS_COMPRESSED(c_byte):
            ...
            return (
             block, start + offset + c_byte, c_byte)
        else:
            myfile.seek(self.offset + start + offset)
            c_byte = SQUASHFS_COMPRESSED_SIZE(c_byte)
    ...
    def read_inode(self, start_block, offset):
        ...
        if header.inode_type == SQUASHFS_DIR_TYPE:
            ...
            if header.inode_type == SQUASHFS_LDIR_TYPE:
                ...
                if header.inode_type == SQUASHFS_FILE_TYPE:
                    ...
                    if header.inode_type == SQUASHFS_LREG_TYPE:
                        ...
                        if header.inode_type == SQUASHFS_SYMLINK_TYPE or header.inode_type == SQUASHFS_LSYMLINK_TYPE:
                            ...
                            i.symlink[header.symlink_size] = '\x00'
                            ...
                            if header.inode_type == SQUASHFS_BLKDEV_TYPE or header.inode_type == SQUASHFS_CHRDEV_TYPE:
                                header.dev_header(self.inode_table, block_ptr)
                                i.data = header.rdev
                                i.xattr = SQUASHFS_INVALID_XATTR
                            else:
                                if header.inode_type == SQUASHFS_LBLKDEV_TYPE or header.inode_type == SQUASHFS_LCHRDEV_TYPE:
                                    header.ldev_header(self.inode_table, block_ptr)
                                    i.data = header.rdev
                                    i.xattr = header.xattr
                                else:
                                    if header.inode_type == SQUASHFS_FIFO_TYPE or header.inode_type == SQUASHFS_SOCKET_TYPE:
                                        i.data = 0
                                        i.xattr = SQUASHFS_INVALID_XATTR
                                    else:
                                        if header.inode_type == SQUASHFS_LFIFO_TYPE or header.inode_type == SQUASHFS_LSOCKET_TYPE:
