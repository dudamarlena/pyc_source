...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/tg/ext/silverplate/controller.py
# Compiled at: 2008-07-24 23:28:00
"""
controller Module
...
from tg.ext.silverplate.widgets import registration_form, user_form, users_table, profile_form, groups_table, group_edit_form, group_add_form, permissions_table, permission_add_form, permission_edit_form
from tg.ext.silverplate.config import config
__all__ = [
 'BaseController', 'SilverPlate']
...
class SilverPlate(BaseController):
    ...
    def groups(self, **kw):
        ...
        pylons.c.groups = groups_table
        pylons.c.form = group_add_form
        return dict(value=value, formValue=kw)

    @expose(config.group_template)
    def group(self, group_id):
        data = self.groupSessionConfig.get_value(dict(group_id=group_id))
        pylons.c.edit = group_edit_form
        return dict(value=data)

    @expose()
    @validate(form=group_edit_form, error_handler=group)
    def edit_group(self, **kw):
        params = pylons.request.params.copy()
        self.provider.create_relationships(config.groups_table, params)
        self.provider.edit(table_name=config.groups_table, values=kw)
        redirect('groups')

    @expose()
    @validate(form=group_add_form, error_handler=groups)
    ...
    def permissions(self, **kw):
        ...
        pylons.c.permissions = permissions_table
        pylons.c.form = permission_add_form
        return dict(value=value, formValue=kw)

    @expose(config.permission_template)
    def permission(self, permission_id):
        data = self.permissionSessionConfig.get_value(dict(permission_id=permission_id))
        pylons.c.edit = permission_edit_form
        return dict(value=data)

    @expose()
    @validate(form=permission_add_form, error_handler=permissions)
    ...
    @expose()
    @validate(form=permission_edit_form, error_handler=permission)
    def edit_permission(self, **kw):
        params = pylons.request.params.copy()
        self.provider.create_relationships(config.permissions_table, params)
        self.provider.edit(table_name=config.permissions_table, values=kw)
        redirect('permissions')
    ...
    def users(self):
        ...
        pylons.c.users_table = users_table
        return dict(value=value)

    @expose(config.user_template)
    def user(self, user_id):
        user_data = self.userSessionConfig.get_value(dict(user_id=user_id))
        pylons.c.edit = user_form
    ...
    def edit_user(self, **kw):
        params = pylons.request.params.copy()
        self.provider.create_relationships(config.users_table, params)
        self.provider.edit(table_name=config.users_table, values=kw)
        redirect('users')
    ...
    def create_relationships(self, table_name, params):
        if table_name in self.provider.getManyToManyTables():
            return
        pk = self.provider.get_pPrimaryKeys(table_name)
        assert len(pk) == 1
        id = params[pk[0]]
        relationships = {}
        ...
        for (key, value) in relationships.iteritems():
            self.provider.setManyToMany(table_name, id, key, value)
...
class SilverPlateProfile(BaseController):
    ...
    def registration(self, **kw):
        pylons.c.form = registration_form
        return dict(value=kw)

    @expose()
    @validate(form=registration_form, error_handler=registration)
