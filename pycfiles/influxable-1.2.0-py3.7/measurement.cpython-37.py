...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/influxable/measurement.py
# Compiled at: 2019-12-13 03:39:16
# Size of source mod 2**32: 9113 bytes
import os
from pathlib import Path
from jinja2 import Environment, FileSystemLoader
from .attributes import BaseAttribute, GenericFieldAttribute, TagFieldAttribute, TimestampFieldAttribute
...
class Measurement(object, metaclass=MeasurementMeta):
    ...
    def check_attribute_values(self, **kwargs):
        ...
        required_attributes = list(filter(filter_required_attributes, attributes))
        required_attributes_names = [r.attribute_name for r in required_attributes]
        for key in required_attributes_names:
            if key not in kwargs:
                raise InfluxDBAttributeValueError("The attribute '{}' cannot be nullable".format(key))
    ...
    def get_prep_value(self):
        ...
        attributes = self.get_attributes()
        field_attributes = list(filter(factory_filter_func(GenericFieldAttribute), attributes))
        tag_attributes = list(filter(factory_filter_func(TagFieldAttribute), attributes))
        timestamp_attributes = list(filter(factory_filter_func(TimestampFieldAttribute), attributes))
        prep_value_groups = []
        attributes_groups = [
         tag_attributes, field_attributes, timestamp_attributes]
    ...
    def bulk_save(points):
        ...
        for point in points:
            if not isinstance(point, Measurement):
                raise InfluxDBAttributeValueError('type of point must be Measurement')
...
def SimpleMeasurement(measurement_name, field_names, tag_names=[]):
    ...
    j2_env = Environment(loader=(FileSystemLoader(template_folder_path)),
      trim_blocks=True)
    rendered_template = j2_env.get_template(TEMPLATE_FILE_NAME).render(measurement_name=measurement_name,
