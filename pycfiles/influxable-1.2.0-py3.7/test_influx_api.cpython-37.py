...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/test_influx_api.py
...
class TestInfluxApi:
    ...
    def test_get_debug_requests_success(self):
        ...
        res = InfluxDBApi.get_debug_requests((instance.connection.request),
          seconds=10)
    ...
    def test_execute_query_bad_method_fail(self):
        with pytest.raises(requests.exceptions.HTTPError):
            query = 'SHOW DATABASES'
            instance = self.get_instance()
            InfluxDBApi.execute_query((instance.connection.request),
              query,
              method='delete')
    ...
    def test_write_points_success(self):
        ...
        instance = self.get_instance()
        res = InfluxDBApi.write_points(instance.connection.request, points, precision)
        assert res is True
    ...
    def test_write_points_with_policy_success(self):
        ...
        instance = self.get_instance()
        res = InfluxDBApi.write_points((instance.connection.request),
          points,
    ...
    def test_write_points_bad_timestamp_fail(self):
        with pytest.raises(exceptions.InfluxDBInvalidTimestampError):
            ...
            instance = self.get_instance()
            InfluxDBApi.write_points(instance.connection.request, points)