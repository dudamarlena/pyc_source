...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/influxable/attributes.py
# Compiled at: 2019-12-03 04:28:49
...
TIMESTAMP_CONVERT_RATIO = {TimestampPrecision.HOURS: inv(3600), 
 TimestampPrecision.MICROSECONDS: 1000000, 
 TimestampPrecision.MILLISECONDS: 1000, 
 TimestampPrecision.MINUTES: inv(60), 
 TimestampPrecision.NANOSECONDS: 1000000000, 
    ...
    def validate(self, value):
        ...
        if value is not None:
            if self.min_value is not None:
                if self.to_python(value) < self.min_value:
                    raise InfluxDBAttributeValueError('The value must be greater than the min_value')
        if value is not None:
            if self.max_value is not None:
                if self.to_python(value) > self.max_value:
                    raise InfluxDBAttributeValueError('The value must be lower than the max_value')

    def validate_options(self):
        super(IntegerFieldAttribute, self).validate_options()
        if self.min_value is not None:
            if not isinstance(self.min_value, int):
                raise InfluxDBAttributeValueError('min_value must be integer')
        if self.max_value is not None:
            if not isinstance(self.max_value, int):
                raise InfluxDBAttributeValueError('max_value must be integer')
...
class FloatFieldAttribute(IntegerFieldAttribute):
    ...
    def validate_options(self):
        ...
        if self.max_nb_decimals is not None:
            if not isinstance(self.max_nb_decimals, int):
                raise InfluxDBAttributeValueError('max_nb_decimals must be integer')
        if self.max_nb_decimals is not None:
            if self.max_nb_decimals <= 0:
                raise InfluxDBAttributeValueError('max_nb_decimals must be positive')
...
class StringFieldAttribute(GenericFieldAttribute):
    ...
    def validate(self, value):
        ...
        if self.choices is not None:
            if value not in self.choices:
                raise InfluxDBAttributeValueError('The value is not refered in choices')
        if self.max_length is not None:
            if len(str(value)) > self.max_length:
                raise InfluxDBAttributeValueError('the string length must be lower than the max_length')
    ...
    def validate_options(self):
        ...
        if self.choices is not None:
            if not any([isinstance(c, str) for c in self.choices]):
                raise InfluxDBAttributeValueError('choices items must be a string')
        if self.max_length is not None:
            if not isinstance(self.max_length, int):
                raise InfluxDBAttributeValueError('max_length must be integer')
        if self.max_length is not None:
            if self.max_length <= 0:
                raise InfluxDBAttributeValueError('max_length must be positive')
...
class TimestampFieldAttribute(BaseAttribute):
    ...
    def validate_options(self):
        ...
        if self.precision not in TIMESTAMP_CONVERT_RATIO:
            raise InfluxDBAttributeValueError('precision must be one of [ns,u,ms,s,m,h]')
