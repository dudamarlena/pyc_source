...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/rdisq/service.py
# Compiled at: 2020-03-07 02:12:19
# Size of source mod 2**32: 7564 bytes
__author__ = 'smackware'
import time, uuid
MISSING_DISPATCHER_ERROR_TEXT = "Service class must have a 'redis_dispatcher' attribute pointing to a RedisDispatcher instance"
MISSING_SERVICE_NAME_IN_MAIN_ERROR_TEXT = 'When instantiating a service in __main__, the service_name class attribute must be set'
...
class RdisqService(object):
    ...
    _RdisqService__go = True
    _RdisqService__sync_consumer = None
    _RdisqService__async_consumer = None
    ...
    def __init__(self, uid=None):
        ...
        self._RdisqService__uid = uid or str(uuid.uuid4())
        self._RdisqService__queue_to_callable = None
        self._RdisqService__map_exposed_methods_to_queues()

    @property
    def rdisq_go(self):
        return self._RdisqService__go

    def rdisq_process_one(self):
        self._RdisqService__process_one()
    ...
    def __map_exposed_methods_to_queues(self):
        self._RdisqService__queue_to_callable = self.get_queue_name_to_exposed_method_mapping()
        if not self._RdisqService__queue_to_callable:
    ...
    def get_queue_name_to_exposed_method_mapping(self):
        ...
        for attr in dir(self):
            ...
            method_queue_name = self.get_queue_name_for_method(stub_method_name, self._RdisqService__uid)
            mapping[method_queue_name] = call
    ...
    def get_consumer(cls):
        if cls._RdisqService__sync_consumer is None:
            cls._RdisqService__sync_consumer = RdisqWaitingConsumer(cls)
        return cls._RdisqService__sync_consumer

    @classmethod
    def get_async_consumer(cls):
        if cls._RdisqService__async_consumer is None:
            cls._RdisqService__async_consumer = RdisqAsyncConsumer(cls)
        return cls._RdisqService__async_consumer
    ...
    def __process_one(self, timeout=0):
        ...
        redis_result = redis_con.brpop((self._RdisqService__queue_to_callable.keys()), timeout=timeout)
        if redis_result is None:
            return
        method_queue_name, task_id = redis_result
        request_key = get_request_key(task_id.decode())
        call = self._RdisqService__queue_to_callable[method_queue_name.decode()]
        ...
        try:
            ...
            result = None
            raised_exception = ex
            if self.log_returned_exceptions:
                if self.logger:
                    self.logger.exception(ex)
            self.on_exception(ex)

        duration_seconds = time.time() - time_start
        response_payload = ResponsePayload(returned_value=result,
    ...
    def process(self):
        self.on_start()
        redis_con = self.get_redis()
        while self._RdisqService__go:
            self._RdisqService__process_one(1)
            redis_con.hset(self.get_service_uid_list_key(), self._RdisqService__uid, time.time())

    def stop(self):
        self._RdisqService__go = False