...
# Embedded file name: build/bdist.linux-i686/egg/couchish/couchish_formish_jsonbuilder.py
# Compiled at: 2010-02-11 16:50:55
...
class FileUpload(formish.FileUpload):
    ...
    def __init__(self, filestore=filestore, show_file_preview=True, show_download_link=False, show_image_thumbnail=False, url_base=None, css_class=None, image_thumbnail_default=None, url_ident_factory=None, identify_size=False):
        formish.FileUpload.__init__(self, filestore=filestore, show_file_preview=show_file_preview, show_download_link=show_download_link, show_image_thumbnail=show_image_thumbnail, url_base=url_base, css_class=css_class, image_thumbnail_default=image_thumbnail_default, url_ident_factory=url_ident_factory)
...
class SelectChoiceFacetTreeCouchDB(widgets.Widget):
    ...
    def __init__(self, options, **k):
        ...
        self.options = [ (key, indented_label(key, value['data']['label'])) for (key, value) in options
                       ]
...
class RefInput(formish.Input):
    ...
    def to_request_data(self, field, data):
        if data is None:
            ...
            additional_fields = [
             '_ref'] + self.additional_fields
            return [
             ('|').join(data.get(attr, '') for attr in additional_fields)]
...
class WidgetRegistry(FormishWidgetRegistry):
    ...
    def fileupload_factory(self, spec, k):
        ...
        return FileUpload(filestore=filestore.CachedTempFilestore(), url_base=url_base, image_thumbnail_default=image_thumbnail_default, show_download_link=show_download_link, show_file_preview=show_file_preview, show_image_thumbnail=show_image_thumbnail, url_ident_factory=url_ident_factory, identify_size=identify_size, **k)
