...
# Embedded file name: build/bdist.linux-i686/egg/couchish/tests/test_store.py
# Compiled at: 2010-03-01 05:07:19
from __future__ import with_statement
import os.path, time, unittest, couchdb
...
class Test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.S = store.CouchishStore(self.db, config.Config.from_yaml(dict((name, type_filename(name)) for name in ['book', 'author', 'post', 'dvd']), data_filename('test_couchish_views.yaml')))
        self.S.sync_views()
    ...
    def test_make_refs(self):
        ...
        assert refs == {matt_id: {'_ref': matt_id, 'first_name': 'Matt', 'last_name': 'Goodall'}, tim_id: {'_ref': tim_id, 'first_name': 'Tim', 'last_name': 'Parkin'}}
    ...
    def test_simple_reference(self):
        ...
        tim = {'model_type': 'author', 'first_name': 'Tim', 'last_name': 'Parkin'}
        tim_id = sess.create(tim)
        book = {'model_type': 'book', 'title': 'Title', 'writtenby': {'_ref': matt_id, 'first_name': 'Matt', 'last_name': 'Goodall'}, 'coauthored': {'_ref': tim_id, 'last_name': 'Parkin'}}
        ...
        assert book == {'model_type': 'book', 'title': 'Title', 'writtenby': {'_ref': matt_id, 'first_name': 'Matt', 'last_name': 'Woodall'}, 'coauthored': {'_ref': tim_id, 'last_name': 'Parkin'}}
    ...
    def test_simple_reference_addingdictionary(self):
        ...
        assert book == {'model_type': 'book', 'title': 'Title', 'writtenby': {'_ref': matt_id, 'first_name': 'Matt', 'last_name': {'firstpart': 'Woo', 'lastpart': 'dall'}}, 'coauthored': {'_ref': tim_id, 'last_name': 'Parkin'}}

    def test_multiple_changes(self):
        sess = self.S.session()
        matt = {'model_type': 'author', 'first_name': 'Matt', 'last_name': 'Goodall'}
        matt_id = sess.create(matt)
        book = {'model_type': 'book', 'title': 'Title', 'writtenby': {'_ref': matt_id, 'first_name': 'Matt', 'last_name': 'Goodall'}, 'coauthored': {'_ref': matt_id, 'last_name': 'Goodall'}}
        book_id = sess.create(book)
        sess.flush()
        sess = self.S.session()
        matt = sess.doc_by_id(matt_id)
        ...
        book = strip_id_rev_meta(self.db[book_id])
        assert matt == {'model_type': 'author', 'first_name': 'Matt', 'last_name': 'Woodall'}
        assert book == {'model_type': 'book', 'title': 'Title', 'writtenby': {'_ref': matt_id, 'first_name': 'Matt', 'last_name': 'Woodall'}, 'coauthored': {'_ref': matt_id, 'last_name': 'Woodall'}}
...
class TestDeep(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if db_name in server:
            del server[db_name]
        self.db = server.create(db_name)
        self.S = store.CouchishStore(self.db, config.Config.from_yaml(dict((name, type_filename(name, 'deepref')) for name in ['book', 'author']), type_filename('views', 'deepref')))
        self.S.sync_views()
    ...
    def test_simple(self):
        ...
        tim = {'model_type': 'author', 'first_name': 'Tim', 'last_name': 'Parkin'}
        tim_id = sess.create(tim)
        book = {'model_type': 'book', 'title': 'Title', 'metadata': {'writtenby': {'_ref': matt_id, 'first_name': 'Matt', 'last_name': 'Goodall'}, 'coauthored': {'_ref': tim_id, 'last_name': 'Parkin'}}}
        book_id = sess.create(book)
        sess.flush()
        sess = self.S.session()
        matt = sess.doc_by_id(matt_id)
        ...
        book = strip_id_rev_meta(self.db[book_id])
        assert matt == {'model_type': 'author', 'first_name': 'Matt', 'last_name': 'Woodall'}
        assert book == {'model_type': 'book', 'title': 'Title', 'metadata': {'writtenby': {'_ref': matt_id, 'first_name': 'Matt', 'last_name': 'Woodall'}, 'coauthored': {'_ref': tim_id, 'last_name': 'Parkin'}}}
...
class TestDeep2(util.TempDatabaseMixin, unittest.TestCase):
    ...
    def test_missing_ref_container(self):
        ...
        """
        cfg = config.Config({'author': {'fields': [{'name': 'name'}]}, 'book': {'fields': [{'name': 'title'}, {'name': 'author', 'type': 'Reference()', 'refersto': 'test/author_summary'}, {'name': 'authors', 'type': 'Sequence(Reference())', 'refersto': 'test/author_summary'}]}}, [{'name': 'author_summary', 'designdoc': 'test', 'uses': ['author.name']}])
        ...
        book_id = S.create({'model_type': 'book', 'title': 'My First Colouring Book', 'author': {'_ref': author_id, 'name': 'Matt'}})
        S.flush()
        S = couchish_store.session()
...
class TestRefsInSequences(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if db_name in server:
            del server[db_name]
        self.db = server.create(db_name)
        self.S = store.CouchishStore(self.db, config.Config.from_yaml(dict((name, type_filename(name, 'refinseq')) for name in ['book', 'author']), type_filename('views', 'refinseq')))
        self.S.sync_views()
    ...
    def test_simple(self):
        ...
        tim = {'model_type': 'author', 'first_name': 'Tim', 'last_name': 'Parkin'}
        tim_id = sess.create(tim)
        book = {'model_type': 'book', 'title': 'Title', 'authors': [{'_ref': matt_id, 'first_name': 'Matt', 'last_name': 'Goodall'}, {'_ref': tim_id, 'last_name': 'Parkin'}]}
        book_id = sess.create(book)
        sess.flush()
        sess = self.S.session()
        matt = sess.doc_by_id(matt_id)
...
class TestNestedRefsInSequences(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if db_name in server:
            del server[db_name]
        self.db = server.create(db_name)
        self.S = store.CouchishStore(self.db, config.Config.from_yaml(dict((name, type_filename(name, 'nestedrefinseq')) for name in ['book', 'author']), type_filename('views', 'nestedrefinseq')))
        self.S.sync_views()
    ...
    def test_simple(self):
        ...
        tim = {'model_type': 'author', 'first_name': 'Tim', 'last_name': 'Parkin'}
        tim_id = sess.create(tim)
        book = {'model_type': 'book', 'title': 'Title', 'authors': [{'nested': {'_ref': matt_id, 'first_name': 'Matt', 'last_name': 'Goodall'}}, {'nested': {'_ref': tim_id, 'last_name': 'Parkin'}}]}
        book_id = sess.create(book)
        sess.flush()
        sess = self.S.session()
        matt = sess.doc_by_id(matt_id)
    ...
    def test_twoentries(self):
        ...
        matt = {'model_type': 'author', 'first_name': 'Matt', 'last_name': 'Goodall'}
        matt_id = sess.create(matt)
        book = {'model_type': 'book', 'title': 'Title', 'authors': [{'nested': {'_ref': matt_id, 'first_name': 'Matt', 'last_name': 'Goodall'}}, {'nested': {'_ref': matt_id, 'first_name': 'Matt', 'last_name': 'Goodall'}}]}
        book_id = sess.create(book)
        sess.flush()
        sess = self.S.session()
        matt = sess.doc_by_id(matt_id)
...
class TestNestedRefsInNestedSequences(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if db_name in server:
            del server[db_name]
        self.db = server.create(db_name)
        self.S = store.CouchishStore(self.db, config.Config.from_yaml(dict((name, type_filename(name, 'nestedrefinnestedseq')) for name in ['book', 'author']), type_filename('views', 'nestedrefinnestedseq')))
        self.S.sync_views()
    ...
    def test_simple(self):
        ...
        tim = {'model_type': 'author', 'first_name': 'Tim', 'last_name': 'Parkin'}
        tim_id = sess.create(tim)
        book = {'model_type': 'book', 'title': 'Title', 'people': [{'authors': [{'nested': {'_ref': matt_id, 'first_name': 'Matt', 'last_name': 'Goodall'}}, {'nested': {'_ref': tim_id, 'last_name': 'Parkin'}}]}]}
        book_id = sess.create(book)
        sess.flush()
        sess = self.S.session()
        matt = sess.doc_by_id(matt_id)
    ...
    def test_twoentries(self):
        ...
        matt = {'model_type': 'author', 'first_name': 'Matt', 'last_name': 'Goodall'}
        matt_id = sess.create(matt)
        book = {'model_type': 'book', 'title': 'Title', 'people': [{'authors': [{'nested': {'_ref': matt_id, 'first_name': 'Matt', 'last_name': 'Goodall'}}, {'nested': {'_ref': matt_id, 'first_name': 'Matt', 'last_name': 'Goodall'}}]}]}
        book_id = sess.create(book)
        sess.flush()
        sess = self.S.session()
        matt = sess.doc_by_id(matt_id)
