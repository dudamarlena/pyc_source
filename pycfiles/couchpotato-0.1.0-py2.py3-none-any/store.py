...
# Embedded file name: build/bdist.linux-i686/egg/couchish/store.py
# Compiled at: 2010-02-26 07:36:22
...
class CouchishStoreSession(object):
    ...
    def __init__(self, store):
        self.store = store
        self.session = Session(store.db, pre_flush_hook=self._pre_flush_hook, post_flush_hook=self._post_flush_hook, encode_doc=jsonutil.encode_to_dict, decode_doc=lambda d: jsonutil.decode_from_dict(d, self))
        self.file_additions = {}
        self.file_deletions = {}
    ...
    def doc_by_view(self, view, key=None):
        if key is not None:
            results = self.session.view(view, startkey=key, endkey=key, limit=2, include_docs=True)
        else:
    ...
    def docs_by_id(self, ids, remove_rows_with_missing_doc=False, **options):
        ...
        """
        options['keys'] = ids
        return self.docs_by_view('_all_docs', remove_rows_with_missing_doc=remove_rows_with_missing_doc, **options)

    def docs_by_type(self, type, remove_rows_with_missing_doc=False, **options):
        """
        ...
        if not view:
            view = '%s/all' % type
        return self.docs_by_view(view, remove_rows_with_missing_doc=remove_rows_with_missing_doc, **options)

    def docs_by_view(self, view, remove_rows_with_missing_doc=False, **options):
    ...
    def _pre_flush_hook(self, session, deletions, additions, changes):
        deletions, additions, changes = list(deletions), list(additions), list(changes)
    ...
    def _post_flush_hook(self, session, deletions, additions, changes):
        ...
        for (doc, actions) in changes:
            ...
            edited = set(('.').join([doc_type, ('.').join(str(p) for p in action['path'])]) for action in actions if action['action'] == 'edit')
            views = set()
            for attr in edited:
