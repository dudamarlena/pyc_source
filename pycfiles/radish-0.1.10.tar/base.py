...
# [GCC 8.4.0]
# Embedded file name: /Users/claudiob/Projects/legendary/env/src/radish/radish/features/base.py
# Compiled at: 2011-07-05 21:00:41
from lettuce import *
...
@step('there is no (.+?)(?:| with (.+))$')
def no_instances(step, model_name, options):
    model = ContentType.objects.get(model=model_name.lower()).model_class()
    kwargs = eval(options or '{}')
    model.objects.filter(**kwargs).delete()
    if model_name in world.instances:
        del world.instances[model_name]


@step('there is an instance of (.+?)(?:| with (.+))$')
...
@step('there should be (?:many|(\\d+)) instances of (.+)')
...
@step('I fill the field labell?ed "(.*)" with "(.*)"')
...
@step('I fill the "(.*)" field with "(.*)" and "(.*)"')
...
@step('the "(.+)" field should have value "(.+)"')
...
@step('I select that instance of (.+?) from "(.+)"')
...
@step('I check the field of that (.+)$')
...
@step('I navigate (?:|to )the page of that (.+)$')
...
@step('I navigate (?:|to )the page of the (.+?) of that (.+)$')
...
@step('I click on the (.+?) image of that (.+)$')
...
@step('I have the "(.*)" image stored at "(.*)"')
def i_have_the_image(step, url, filename):
    if not exists(filename):
        urlretrieve(url, filename)


@step('that image should in the "(.*)" section')
...
@step('I should see the (.+?) of that (.+?) (as|in) the "(.*)"(| element)$')
...
@step('I should see "(.*)" (in|as) the "(.*?)"(?:| tag)')
def i_should_see_the_text_in_the_tag(step, text, in_as, tag_name):
    ...
    elif not (in_as == 'in' and text in page_text):
        raise AssertionError


@step('I should see "(.*)" (in|as) the "(.*?)" element')
...
@step('an email should be sent to "(.*)"')
