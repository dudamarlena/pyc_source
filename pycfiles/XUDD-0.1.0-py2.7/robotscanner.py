...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/xudd/demos/robotscanner.py
# Compiled at: 2013-07-27 20:44:55
...
class Overseer(Actor):
    ...
    def __init__(self, hive, id):
        ...
        self.message_routing.update({'init_world': self.init_world, 'transmission': self.transmission})
    ...
    def init_world(self, message):
        ...
        for clean_droids, infected_droids in ROOM_STRUCTURE:
            ...
            if last_room:
                self.hive.send_message(to=last_room, directive='set_next_room', body={'id': room})
                self.hive.send_message(to=room, directive='set_previous_room', body={'id': last_room})
            for droid_num in range(clean_droids):
                droid = self.hive.create_actor(Droid, infected=False, room=room)
                _log.debug('New droid created')
                yield self.wait_on_message(to=droid, directive='register_with_room')
                _log.debug('I guess this never really happens')

            for droid_num in range(infected_droids):
                droid = self.hive.create_actor(Droid, infected=True, room=room)
                yield self.wait_on_message(to=droid, directive='register_with_room')
        ...
        self.hive.send_message(to=security_robot, directive='begin_mission', body={'starting_room': first_room})
        return
...
class WarehouseRoom(Actor):
    ...
    def __init__(self, hive, id):
        ...
        self.message_routing.update({'set_next_room': self.set_next_room, 'set_previous_room': self.set_previous_room, 
    ...
    def get_next_room(self, message):
        message.reply({'id': self.next_room})

    def get_previous_room(self, message):
        message.reply({'id': self.previous_room})

    def list_droids(self, message):
        message.reply({'droid_ids': self.droids})
...
class Droid(Actor):
    ...
    def __init__(self, hive, id, room, infected=False):
        ...
        self.hp = 50
        self.room = room
        self.message_routing.update({'infection_expose': self.infection_expose, 'get_shot': self.get_shot, 
    ...
    def register_with_room(self, message):
        ...
        yield self.wait_on_message(to=self.room, directive='register_droid', body={'droid_id': self.id})
        _log.debug(('Registered droid {0}!').format(self.id))

    def infection_expose(self, message):
        message.reply({'is_infected': self.infected})

    def get_shot(self, message):
        damage = random.randrange(0, 60)
        self.hp -= damage
        alive = self.hp > 0
        message.reply(body={'hp_left': self.hp, 
           'damage_taken': damage, 
           'alive': alive})
        if not alive:
...
class SecurityRobot(Actor):
    ...
    def __init__(self, hive, id):
        ...
        self.room = None
        self.message_routing.update({'begin_mission': self.begin_mission})
        return
    ...
    def begin_mission(self, message):
        ...
        while True:
            self.hive.send_message(to='overseer', directive='transmission', body={'message': 'Entering room %s...' % self.room})
            response = yield self.wait_on_message(to=self.room, directive='list_droids')
            for droid_id in response.body['droid_ids']:
                response = yield self.wait_on_message(to=droid_id, directive='infection_expose')
                if not response.body['is_infected']:
                    transmission = '%s is clean... moving on.' % droid_id
                    self.hive.send_message(to='overseer', directive='transmission', body={'message': transmission})
                    continue
                transmission = '%s found to be infected... taking out' % droid_id
                self.hive.send_message(to='overseer', directive='transmission', body={'message': transmission})
                infected_droid_alive = True
                while infected_droid_alive:
                    response = yield self.wait_on_message(to=droid_id, directive='get_shot')
                    droid_status = self.__droid_status_format(response)
                    self.hive.send_message(to='overseer', directive='transmission', body={'message': droid_status})
                    infected_droid_alive = response.body['alive']

            response = yield self.wait_on_message(to=self.room, directive='get_next_room')
        ...
        self.hive.send_message(to='overseer', directive='transmission', body={'message': 'Mission accomplished.'})
