...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/xudd/message.py
...
class Message(object):
    ...
    **Args:**
    ...
    def __init__(self, to, directive, from_id, id, body=None, in_reply_to=None, wants_reply=False, hive_proxy=None):
        self.to = to
    ...
    def __repr__(self):
        return ('<{cls} #{id} {inreply}to={to} from={from_id}>').format(cls=self.__class__.__name__, inreply=('in-reply-to={in_reply_to} ').format(in_reply_to=self.in_reply_to) if self.in_reply_to else '', to=self.to, from_id=self.from_id, id=self.id)

    def reply(self, body=None, directive='reply', wants_reply=False):
        """
        ...
        self.hive_proxy.send_message(to=self.from_id, directive=directive, wants_reply=wants_reply, in_reply_to=self.id, body=body)
        self.replied = True
        return
