...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/windtalker-project/windtalker/cipher.py
# Compiled at: 2020-03-04 17:52:47
...
class BaseCipher(object):
    ...
    def encrypt_file(self, path, output_path=None, overwrite=False, stream=True, enable_verbose=True, **kwargs):
        """
        ...
        """
        path, output_path = files.process_dst_overwrite_args(src=path,
          dst=output_path,
          overwrite=overwrite,
          src_to_dst_func=(files.get_encrpyted_path))
        self._show(("Encrypt '%s' ..." % path), enable_verbose=enable_verbose)
        st = time.clock()
        files.transform(path, output_path, converter=(self.encrypt), overwrite=overwrite,
          stream=stream,
          chunksize=(self._encrypt_chunk_size))
        self._show(('    Finished! Elapse %.6f seconds' % (time.clock() - st,)), enable_verbose=enable_verbose)
        return output_path

    def decrypt_file(self, path, output_path=None, overwrite=False, stream=True, enable_verbose=True, **kwargs):
        """
        ...
        st = time.clock()
        files.transform(path, output_path, converter=(self.decrypt), overwrite=overwrite,
          stream=stream,
    ...
    def encrypt_dir(self, path, output_path=None, overwrite=False, stream=True, enable_verbose=True):
        ...
        path, output_path = files.process_dst_overwrite_args(src=path,
          ...
          overwrite=overwrite,
          src_to_dst_func=(files.get_encrpyted_path))
        self._show(("--- Encrypt directory '%s' ---" % path), enable_verbose=enable_verbose)
        st = time.clock()
        for current_dir, _, file_list in os.walk(path):
            ...
            for basename in file_list:
                ...
                self.encrypt_file(old_path, new_path, overwrite=overwrite,
                  stream=stream,
                  enable_verbose=enable_verbose)

        self._show(('Complete! Elapse %.6f seconds' % (time.clock() - st,)), enable_verbose=enable_verbose)
        return output_path

    def decrypt_dir(self, path, output_path=None, overwrite=False, stream=True, enable_verbose=True):
        """
        ...
        for current_dir, _, file_list in os.walk(path):
            ...
            for basename in file_list:
                old_path = os.path.join(current_dir, basename)
                new_path = os.path.join(new_dir, basename)
                self.decrypt_file(old_path, new_path, overwrite=overwrite,
                  stream=stream,
                  enable_verbose=enable_verbose)
