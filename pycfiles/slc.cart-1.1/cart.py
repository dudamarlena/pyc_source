...
# [GCC 8.4.0]
# Embedded file name: /Users/zupo/work/slc.cart/src/slc/cart/browser/cart.py
# Compiled at: 2012-11-12 07:02:10
...
class Cart(grok.View):
    ...
    def item_count(self):
        ...
        if self.request.get('HTTP_X_REQUESTED_WITH', None) == 'XMLHttpRequest':
            response_dict = {'status': STATUS.OK, 'body': len(self.cart), 'err_info': None}
    ...
    def clear(self):
        ...
        api.portal.show_message(message='Cart cleared.', type='info', request=self.request)
...
class AddToCart(grok.View):
    ...
    def render(self):
        ...
        if len(cart) >= limit:
            ...
            err_info = dict(msg=message, level=ERR_LEVEL.WARNING, label='Error')
        else:
            ...
            err_info = None
        if self.request.get('HTTP_X_REQUESTED_WITH', None) == 'XMLHttpRequest':
            response_dict = {'status': status, 'body': body, 
               'err_info': err_info}
            return json.dumps(response_dict)
        else:
            if err_info:
                api.portal.show_message(message=err_info['msg'], type='error', request=self.request)
...
class RemoveFromCart(grok.View):
    ...
    def render(self):
        ...
        if self.request.get('HTTP_X_REQUESTED_WITH', None) == 'XMLHttpRequest':
            response_dict = {'status': STATUS.OK, 'body': len(cart), 'err_info': None}
            return json.dumps(response_dict)
        else:
            self.request.response.redirect(portal.absolute_url() + '/@@cart')
            return
...
class IsInCart(grok.View):
    ...
    def render(self):
        ...
        if self.request.get('HTTP_X_REQUESTED_WITH', None) == 'XMLHttpRequest':
            response_dict = {'status': STATUS.OK, 'body': is_in_cart, 'err_info': None}
            return json.dumps(response_dict)
        else:
            return is_in_cart
            return