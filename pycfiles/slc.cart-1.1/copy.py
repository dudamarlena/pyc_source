...
# [GCC 8.4.0]
# Embedded file name: /Users/zupo/work/slc.cart/src/slc/cart/actions/copy.py
# Compiled at: 2012-11-05 02:13:15
...
class CopyAction(grok.Adapter):
    ...
    def run(self):
        ...
        cp_data = (
         0, obj_list)
        cp_data = _cb_encode(cp_data)
        response = request.response
        path = ('{0}').format(cookie_path(request))
        response.setCookie('__cp', cp_data, path=path)
        request['__cp'] = cp_data
        api.portal.show_message(message=('{0} item(s) copied.').format(len(obj_list)), request=request, type='info')
