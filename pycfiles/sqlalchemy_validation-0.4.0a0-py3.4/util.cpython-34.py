...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sqlalchemy_validation/util.py
...
def convert_model_to_dict(model, *args, **kwargs):
    ...
    if has_default:
        if args:
            ret = dict((key, model_vars.get(key, default)) for key in args)
            for key, val in kwargs.items():
                ret[key] = model_vars.get(key, val)

            return ret
        else:
            return dict((key, model_vars.get(key, kwargs.get(key, default))) for key in column_names)
    else:
        if remove_empty:
            if args:
                ret = dict((key, model_vars[key]) for key in args if key in model_vars)
                for key, val in kwargs.items():
                    ret[key] = model_vars.get(key, val)
