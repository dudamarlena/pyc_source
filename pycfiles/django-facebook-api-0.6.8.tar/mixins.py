...
# [GCC 8.4.0]
# Embedded file name: /Users/ramusus/workspace/manufacture/env/src/django-facebook-api/facebook_api/mixins.py
# Compiled at: 2015-12-25 15:20:44
"""
...
class OwnerableModelMixin(models.Model):
    owner_content_type = models.ForeignKey(ContentType, null=True, related_name='content_type_owners_%(app_label)s_%(class)ss')
...
class AuthorableModelMixin(models.Model):
    ...
    author_content_type = models.ForeignKey(ContentType, null=True, related_name='content_type_authors_%(app_label)s_%(class)ss')
...
class ActionableModelMixin(models.Model):
    ...
    def save(self, *args, **kwargs):
        self.actions_count = sum([ getattr(self, field, None) or 0 for field in [
...
class LikableModelMixin(models.Model):
    ...
    def fetch_likes(self, limit=1000, **kwargs):
        ...
        return (
         User.objects.filter(pk__in=ids), response)
...
class ShareableModelMixin(models.Model):
    ...
    def fetch_shares(self, limit=1000, **kwargs):
        ...
        """
        from facebook_api.models import MASTER_DATABASE
        ids = []
        response = api_call(('%s/sharedposts' % self.graph_id), **kwargs)
        if response:
            timestamps = dict([ (int(post['from']['id']), datetime_parse(post['created_time'])) for post in response['data'] ])
            ...
            field_name = [ f.attname for f in m2m_model._meta.local_fields if isinstance(f, models.ForeignKey) and f.name != 'user'
                         ][0]
            self.shares_users.get_query_set_through().filter(time_from=None).delete()
            self.shares_users.get_query_set_through().filter(**{field_name: self.pk, 'user_id__in': map(lambda i: i[1], ids_add_pairs)}).delete()
            get_share_date = lambda id: timestamps[id] if id in timestamps else self.created_time
            m2m_model.objects.bulk_create([ m2m_model(**{field_name: self.pk, 'user_id': pk, 'time_from': get_share_date(graph_id)}) for graph_id, pk in ids_add_pairs
                                          ])
