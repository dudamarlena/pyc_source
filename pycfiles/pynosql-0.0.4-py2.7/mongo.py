...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pynosql/credentials/mongo.py
...
class MongoDBSSLCredentials(BaseCredentials):
    ...
    def check_credentials(self):
        ...
        """
        if not self.certificate_file or not self.certificate_requirements or not self.ca_certificates:
...
class MongoDBCredentials(BaseCredentials):
    ...
    def __init__(self, username=None, password=None, source=None, mechanism=None, ssl_obj=None):
        """ MongoDB Credentials Provider
    ...
    def check_credentials(self):
        ...
        if self.ssl_detail:
            if not isinstance(self.ssl_detail, MongoDBSSLCredentials):
                raise InvalidCredentials('SSL is not an instance of MongoDBSSLCredentials.')
