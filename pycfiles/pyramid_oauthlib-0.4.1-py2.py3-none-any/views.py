...
# Embedded file name: build/bdist.macosx-10.9-intel/egg/pyramid_oauth2_provider/views.py
...
def require_https(handler):
    ...
    def wrapped(request):
        if request.scheme != 'https' and oauth2_settings('require_ssl', default=True):
            log.info('rejected request due to unsupported scheme: %s' % request.scheme)
            return HTTPBadRequest(InvalidRequest(error_description='Oauth2 requires all requests to be made via HTTPS.'))
...
@view_config(route_name='oauth2_provider_authorize', renderer='json', permission=Authenticated)
@require_https
def oauth2_authorize(request):
    ...
    * In the case of a 'code' authorize request a GET or POST is made
    ...
    client = db.query(Oauth2Client).filter_by(client_id=request.client_id).first()
    if not client:
        log.info('received invalid client credentials')
        return HTTPBadRequest(InvalidRequest(error_description='Invalid client credentials'))
    else:
        redirect_uri = request.params.get('redirect_uri')
        redirection_uri = None
        if len(client.redirect_uris) == 1 and (not redirect_uri or redirect_uri == client.redirect_uris[0]):
            redirection_uri = client.redirect_uris[0]
        else:
            if len(client.redirect_uris) > 0:
                redirection_uri = db.query(Oauth2RedirectUri).filter_by(client_id=client.id, uri=redirect_uri).first()
            if redirection_uri is None:
                return HTTPBadRequest(InvalidRequest(error_description='Redirection URI validation failed'))
        resp = None
...
@view_config(route_name='oauth2_provider_token', renderer='json', permission=NO_PERMISSION_REQUIRED)
@require_https
    ...
    * In the case of a token refresh request a POST with the following
    ...
    if request.method != 'POST':
        ...
        if grant_type == 'password':
            ...
            return HTTPBadRequest(UnsupportedGrantType(error_description='Only password and refresh_token grant types are supported by this authentication server'))
...
def handle_password(request, client):
    if 'username' not in request.POST or 'password' not in request.POST:
        ...
        return HTTPBadRequest(InvalidRequest(error_description='Both username and password are required to obtain a password based grant.'))
    auth_check = request.registry.queryUtility(IAuthCheck)
    user_id = auth_check().checkauth(request.POST.get('username'), request.POST.get('password'))
    if not user_id:
...
def handle_refresh_token(request, client):
    ...
    auth_token = db.query(Oauth2Token).filter_by(refresh_token=request.POST.get('refresh_token')).first()
    if not auth_token:
    ...
    if str(auth_token.user_id) != request.POST.get('user_id'):
        ...
        return HTTPBadRequest(InvalidClient(error_description='The given user_id does not match the given refresh_token.'))
