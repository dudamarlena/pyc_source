...
# Embedded file name: build/bdist.macosx-10.9-intel/egg/pyramid_oauth2_provider/tests.py
# Compiled at: 2013-03-11 22:09:01
import base64, unittest, transaction
...
class TestAuthorizeEndpoint(TestCase):
    ...
    def _validate_authcode_response(self, response):
        ...
        dbauthcodes = DBSession.query(Oauth2Code).filter_by(authcode=params.get('code')).all()
    ...
    def testRetainRedirectQueryComponent(self):
        ...
        with transaction.manager:
            redirect_uri = Oauth2RedirectUri(self.client, uri)
            DBSession.add(redirect_uri)
...
class TestTokenEndpoint(TestCase):
    ...
    def _create_request(self):
        headers = self.getAuthHeader(self.client.client_id, self.client.client_secret)
    ...
    def _validate_token(self, token):
        ...
        dbtoken = DBSession.query(Oauth2Token).filter_by(access_token=token.get('access_token')).first()
    ...
    def testInvalidClientCreds(self):
        self.request.headers = self.getAuthHeader(self.client.client_id, 'abcde')
        token = self._process_view()
        self.failUnless(isinstance(token, jsonerrors.HTTPBadRequest))
    ...
    def testCacheHeaders(self):
        self._process_view()
        self.failUnlessEqual(self.request.response.headers.get('Cache-Control'), 'no-store')
        self.failUnlessEqual(self.request.response.headers.get('Pragma'), 'no-cache')
    ...
    def testRefreshToken(self):
        token = self._process_view()
        self._validate_token(token)
        self.request = self._create_refresh_token_request(token.get('refresh_token'), token.get('user_id'))
        token = self._process_view()
        self._validate_token(token)
    ...
    def testInvalidRefreshToken(self):
        token = self._process_view()
        self._validate_token(token)
        self.request = self._create_refresh_token_request('abcd', token.get('user_id'))
        token = self._process_view()
        self.failUnless(isinstance(token, jsonerrors.HTTPUnauthorized))
    ...
    def testRefreshInvalidClientId(self):
        ...
        self._validate_token(token)
        self.request = self._create_refresh_token_request(token.get('refresh_token'), token.get('user_id'))
        self.request.headers = self.getAuthHeader('1234', self.client.client_secret)
        token = self._process_view()
        self.failUnless(isinstance(token, jsonerrors.HTTPBadRequest))
    ...
    def testUserIdMissmatch(self):
        token = self._process_view()
        self._validate_token(token)
        self.request = self._create_refresh_token_request(token.get('refresh_token'), '2')
        token = self._process_view()
        self.failUnless(isinstance(token, jsonerrors.HTTPBadRequest))
