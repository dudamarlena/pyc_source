...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/yoton/__init__.py
# Compiled at: 2016-03-26 04:09:49
...
class YoTon(object):
    ...
    def __init__(self, cache_servers, connection_factory_cls=SimpleConnectionFactory, serializer=cPickle):
        """Initializer
    ...
    def cache(self, key_pattern, expire_seconds, key_formatter=None, database=None):
        ...
        def inner(func):
            return CacheWrapper(self, func, key_pattern=key_pattern, expire_seconds=expire_seconds, key_formatter=key_formatter, database=database)
...
class CacheWrapper(object):
    ...
    def __init__(self, yoton, func, key_pattern, expire_seconds, key_formatter=None, database=None):
        """Initializer
    ...
    def __call__(self, *args, **kwargs):
        ...
        connection = self.yoton.connection_factory.get_connection(cache_key, self.database)
        ...
        if data is not None:
            ...
            connection.setex(cache_key, time=self.expire_seconds, value=serialized_data)
        return return_val
    ...
    def _get_cache_key(self, *args, **kwargs):
        if not (inspect.ismethod(self.func) or inspect.isfunction(self.func)):
            raise TypeError(('staticmethod and classmethod are not supported now,please check your func {}').format(self.func))
        if self.out_caller:
            parameters = inspect.getcallargs(self.func, self.out_caller, *args, **kwargs)
        else:
