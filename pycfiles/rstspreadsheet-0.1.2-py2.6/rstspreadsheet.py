...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/rstspreadsheet.py
# Compiled at: 2011-06-10 14:54:22
"""
============================================
...
_RE_EQ_LHS = re.compile('\\{(?P<col>[^\\:\\}]*)(\\:(?P<cond>[^\\:\\}]*))?\\} *= *(?P<eq>.*) *')
...
class SpreadSheet(RSTTable):
    option_spec = {'eq': parse_equations, 'setup': ScopeSetup, 
       'format': parse_format}
    ...
    def run(self):
        ...
        if 'eq' not in self.options:
            ...
            if setup.fail:
                return [table_node] + message + error_with_tb(setup.tb, self.state.reporter.error, 'An error occurs while executing `:setup:`')
            ...
            for tbody in table_node.traverse(nodes.tbody):
                ...
                for (irow, row) in enumerate(tbody.traverse(nodes.row)):
                    coldata = [ None if len(entry) == 0 else str(entry[0][0]) for entry in row.traverse(nodes.entry)
                              ]
                    if irow == 0:
                    ...
                    for (i, cond, eq) in deflist:
                        _scope = setup.with_default(i=irow, col=cols[i], cols=cols, last=last)
                        if cond is not None:
                            (cond_result, cond_tb) = safe_eval(cond, _scope, _mix=False)
                            if cond_tb is not None:
                                message += error_with_tb(cond_tb, self.state.reporter.error, ("Following error occurs while validating the condition of the cell in {0}-th row, {1}-th col: '{2}'").format(irow, i, cond))
                        else:
                            cond_result = True
                        if cond_result:
                            (result, tb) = safe_eval(eq.format(*coldata), _scope, _mix=False)
                            if tb is None:
                                if i in formatdict:
                                ...
                                message += error_with_tb(tb, self.state.reporter.error, ("Following error occurs while validating the equation of the cell in {0}-th row, {1}-th col: '{2}'").format(irow, i, eq))

                    for (i, c) in enumerate(coldata):
                        cols[i].append(c)

                    for (entry, col) in zip(row.traverse(nodes.entry), coldata):
...
def main():
    ...
    description = ('Generates document with <writer>.  Choose a writer from [{0}].  The default writer is "html". ').format(('|').join(_WRITER_LIST)) + default_description + '  This generation tool includes "spreadsheet" directive.'
    if len(sys.argv) < 2 or sys.argv[1] not in _WRITER_LIST:
        ...
        argv = sys.argv[2:]
    register_directive()
    publish_cmdline(writer_name=writer_name, description=description, usage=_USAGE, argv=argv)
