...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ohppipeline/utils/imageprocessing.py
# Compiled at: 2020-02-06 10:53:22
...
def makemasterflat(masterbias, listofflat, unit=u.adu):
    ...
    scaling_func = lambda arr: 1.0 / mamedian(arr)
...
def crosscorrelalign(listofimage, returncombiner=True):
    ...
    correlations = [fftconvolve(images[0], image[::-1, ::-1], mode='same') for image in images[1:]]
    shift_indices = [unravel_index(argmax(corr_array, axis=None), corr_array.shape) for corr_array in correlations]
    deltas = [(ind[0] - int(nX / 2), ind[1] - int(nY / 2)) for ind in shift_indices]
    realigned_images = [roll(image, deltas[i], axis=(0, 1)) for i, image in enumerate(images[1:])]
    res = [
     listofimage[0]] + [CCDData(im, meta=fit.header, unit=u.adu) for im, fit in zip(realigned_images, listofimage[1:])]
    if returncombiner:
        comb = Combiner(res)
        scaling_func = lambda arr: 1.0 / mamedian(arr)
        comb.scaling = scaling_func
