...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ohppipeline/spectral/calibration.py
# Compiled at: 2020-02-21 13:06:53
...
def _gauss(x, *p):
    A, mu, sigma = p
    return A * exp(-(x - mu) ** 2 / (2.0 * sigma ** 2))


class Calibrator(object):
    __doc__ = 'Class in order to realise the wavelength calibration'
    ...
    def _lookForRays(self, nrays=None, threshold=None):
        ...
        while True:
            if nrays is not None and self.rayCount >= nrays:
                break
            if threshold is not None and self.maskedspectrum.max() < threshold * self._calib.max():
                break
    ...
    def visualizeRays(self, ax):
        ...
        for i in range(self.rayCount):
            ax.text(self.rayTable.loc[(i, 'centroid')], self.rayTable.loc[(i, 'Amplitude')], '%d' % i, horizontalalignment='center', verticalalignment='bottom')
    ...
    def adjustFromFirstGuess(self, polycoeff1guess, threshold=1.0, degcalib=2):
