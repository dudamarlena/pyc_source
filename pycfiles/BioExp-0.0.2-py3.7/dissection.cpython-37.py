...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/BioExp/spatial/dissection.py
# Compiled at: 2019-11-15 02:16:49
# Size of source mod 2**32: 9721 bytes
import pdb, os, cv2, keras, random, numpy as np
from glob import glob
import SimpleITK as sitk, pandas as pd
from ..helpers.utils import *
from keras.models import Model
import matplotlib.pyplot as plt
import skimage.transform as imresize
...
class Dissector:
    ...
    def get_threshold_maps(self, dataset_path, save_path, percentile):
        """
        ...
        if os.path.exists(os.path.join(save_path, 'ModelDissection_layer_fmaps_{}.npy'.format(self.layer_name))):
            ...
            for i in range(len(input_paths) if len(input_paths) < 500 else 500):
                ...
                input_, label_ = load_numpy_slice(os.path.join(dataset_path, input_paths[i]), os.path.join(dataset_path, input_paths[i]).replace('mask', 'label').replace('labels', 'masks'), self.seq)
                output = np.squeeze(self.model.predict(input_[(None, Ellipsis)]))
                fmaps.append(output)
    ...
    def apply_threshold(self, image, threshold_maps, nfeatures=None, save_path=None, post_process_threshold=80, ROI=None):
        """
        ...
        """
        fmaps = np.squeeze(self.model.predict(image[(None, Ellipsis)]))
        ...
        for i in range(nfeatures):
            ...
            try:
                post_processed_img = self._perform_postprocessing(resized_img, threshold=post_process_threshold)
            except:
    ...
    def quantify_gt_features(self, image, gt, threshold_maps, nclasses, nfeatures, save_path, save_fmaps=False, post_process_threshold=80, ROI=None):
        """
        ...
        for i in range(nfeatures):
            ...
            for class_ in nclasses.keys():
                ...
                class_dice = (np.sum(mask * (eroded_img > 0)) + 1e-05) * 2.0 / (np.sum(mask * 1.0) + np.sum((eroded_img > 0) * 1.0) + 1e-05)
