...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/BioExp/uncertainty/uncertanity.py
# Compiled at: 2019-11-15 10:42:01
# Size of source mod 2**32: 3861 bytes
import keras, random, numpy as np
from glob import glob
from keras.models import Model
from keras.utils import np_utils
from keras.models import load_model
import os, imgaug as ia
...
class uncertainty:
    ...
    def save(self, mean, var):
        ...
        im = ax.imshow((gt.reshape((240, 240))), vmin=0.0, vmax=3.0)
        ax.set_xticklabels([])
        ax.set_yticklabels([])
        ax.set_aspect('equal')
        ax.tick_params(bottom='off', top='off', labelbottom='off', right='off', left='off', labelleft='off')
        ax = plt.subplot(gs[(0, 1)])
        im = ax.imshow((np.argmax(mean, axis=(-1)).reshape((240, 240))), vmin=0.0, vmax=3.0)
        ax.set_xticklabels([])
        ax.set_yticklabels([])
        ax.set_aspect('equal')
        ax.tick_params(bottom='off', top='off', labelbottom='off', right='off', left='off', labelleft='off')
    ...
    def epistemic(self, model, iterations=1000):
        ...
        for i in range(iterations):
            ...
            predictions.append(model.predict(aug_image[(None, Ellipsis)]))
    ...
    def aleatoric(self, model, iterations=1000, dropout=0.5):
        ...
        predictions = []
        for i in range(iterations):
            predictions.append(model.predict(self.test_image[(None, Ellipsis)]))

        predictions = np.array(predictions)
        mean = np.mean(predictions, axis=0)
        var = np.var(predictions, axis=0)
        if np.sum(var) == 0:
            raise ValueError('Model trained without dropouts')
        return (
         mean, var)
