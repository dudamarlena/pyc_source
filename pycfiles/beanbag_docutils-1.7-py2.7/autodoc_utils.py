...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/beanbag_docutils/sphinx/ext/autodoc_utils.py
...
class BeanbagDocstring(GoogleDocstring):
    ...
    * Parsing improvements for arguments to allow for wrapping across lines,
    ...
    """
    partial_typed_arg_start_re = re.compile(b'\\s*(.+?)\\s*\\(\\s*(.*[^\\s]+)\\s*[^:)]*$')
    partial_typed_arg_end_re = re.compile(b'\\s*(.+?)\\s*\\):$')
    ...
    def register_returns_section(self, keyword, label):
        ...
        """
        self._sections[keyword] = lambda *args: self._format_fields(label, self._consume_returns_section())
    ...
    def register_fields_section(self, keyword, label):
        ...
        Args:
            ...
            label (unicode):
                The label outputted in the section.
        """
        self._sections[keyword] = lambda *args: self._format_fields(label, self._consume_fields())
    ...
    def register_version_info_section(self, keyword, admonition):
        ...
        """
        self._sections[keyword] = lambda *args: self._format_admonition_with_params(admonition, self._consume_to_next_section())
    ...
    def _format_admonition_with_params(self, admonition, lines):
        ...
        if lines:
            ...
            if param_line.endswith(b':'):
                param_line = param_line[:-1]
            return [
             b'.. %s:: %s' % (admonition, param_line), b''] + self._indent(self._dedent(lines[1:], 3)) + [b'']
        return [
         b'.. %s::' % admonition, b'']
    ...
    def _consume_field(self, parse_type=True, *args, **kwargs):
                    ...
                    if m:
                        ...
                        for line in lines:
                            ...
                            if norm_line and prev_line is not None:
                                if norm_line.startswith(b'.') or prev_line.endswith(b'.'):
                                    line = norm_line
                                else:
                                    parts.append(b' ')
                            parts.append(line)
        ...
        return super(BeanbagDocstring, self)._consume_field(parse_type, *args, **kwargs)
...
def _filter_members(app, what, name, obj, skip, options):
    ...
    """
    module_name = app.env.temp_data[b'autodoc:module']
    module = sys.modules[module_name]
    module_excludes = set(getattr(module, b'__autodoc_excludes__', []))
    if name in module_excludes:
        return True
    global_excludes = app.config[b'autodoc_excludes']
    for key in (what, b'*'):
        if key in global_excludes and name in global_excludes.get(key, []):
            return True

    if name in getattr(module, b'__deprecated__', []):
        return True
    return skip
...
def _process_docstring(app, what, name, obj, options, lines):
    ...
    """
    if app.config[b'napoleon_beanbag_docstring']:
        docstring = BeanbagDocstring(lines, app.config, app, what, name, obj, options)
...
def setup(app):
    ...
    Args:
        app (sphinx.application.Sphinx):
            ...
            events on.
    """
    app.add_config_value(b'autodoc_excludes', {}, True)
    app.add_config_value(b'napoleon_beanbag_docstring', False, True)
