...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/beanbag_docutils/sphinx/ext/collect_files.py
...
def collect_files(app, env):
    ...
    """
    collect_patterns = app.config[b'collect_file_patterns']
    src_dir = app.builder.srcdir
    out_dir = app.builder.outdir
    for root, dirs, files in os.walk(src_dir):
        if root == src_dir:
            try:
                dirs.remove(b'_build')
            except ValueError:
                pass
        ...
        for filename in files:
            for pattern in collect_patterns:
                if fnmatch(filename, pattern):
                    shutil.copy(os.path.join(root, filename), os.path.join(out_dir, os.path.relpath(root, src_dir), filename))
                    break
...
def setup(app):
    ...
    return {b'version': b'1.0', 
       b'parallel_read_safe': True}