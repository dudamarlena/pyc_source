...
# [GCC 8.4.0]
# Embedded file name: /Users/neil/Scripts/tweetcal/build/lib/tweetcal/tweetcal.py
# Compiled at: 2015-10-02 20:10:30
...
def get_since_id(cal, since_id=None):
    ...
    since_id = since_id or cal.get('X-MAX-TWEET-ID') or max(int(x.get('X-TWEET-ID') or 0) for x in cal.subcomponents + [{'X-TWEET-ID': 0}]) or None
    logging.getLogger('tweetcal').debug('Setting since_id: {}'.format(since_id))
    if since_id:
        return {'since_id': str(since_id)}
    else:
        return {}
...
def get_tweets(consumer_key, consumer_secret, key, secret, **kwargs):
    ...
    api = tweepy.API(auth)
    logging.getLogger('tweetcal').debug('Setting up connection to Twitter API (since_id: {})'.format(kwargs.get('since_id')))
...
def tweetcal(screen_name, config, **kwargs):
    ...
    cursor = get_tweets(consumer_key=keys['consumer_key'], consumer_secret=keys['consumer_secret'], key=keys['key'], secret=keys['secret'], **since)
