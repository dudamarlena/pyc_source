...
# Embedded file name: build\bdist.win32\egg\pyevolve\Mutators.py
...
def G1DBinaryStringMutatorSwap(genome, **args):
    ...
    for it in xrange(int(round(mutations))):
        Util.listSwapElement(genome, rand_randint(0, stringLength - 1), rand_randint(0, stringLength - 1))
   ...
   """
    ...
    if mutations < 1.0:
        ...
        for it in xrange(listSize):
            if Util.randomFlipCoin(args['pmut']):
                genome[it] = rand_randint(genome.getParam('rangemin', Consts.CDefRangeMin), genome.getParam('rangemax', Consts.CDefRangeMax))
                mutations += 1

    for it in xrange(int(round(mutations))):
        which_gene = rand_randint(0, listSize - 1)
        genome[which_gene] = rand_randint(genome.getParam('rangemin', Consts.CDefRangeMin), genome.getParam('rangemax', Consts.CDefRangeMax))
...
def G1DListMutatorRealRange(genome, **args):
   ...
   """
    ...
    if mutations < 1.0:
        ...
        for it in xrange(listSize):
            if Util.randomFlipCoin(args['pmut']):
                genome[it] = rand_uniform(genome.getParam('rangemin', Consts.CDefRangeMin), genome.getParam('rangemax', Consts.CDefRangeMax))
                mutations += 1

    for it in xrange(int(round(mutations))):
        which_gene = rand_randint(0, listSize - 1)
        genome[which_gene] = rand_uniform(genome.getParam('rangemin', Consts.CDefRangeMin), genome.getParam('rangemax', Consts.CDefRangeMax))
    ...
    if mutations < 1.0:
        ...
        for i in xrange(height):
            for j in xrange(width):
                if Util.randomFlipCoin(args['pmut']):
                    index_b = (
                     rand_randint(0, height - 1), rand_randint(0, width - 1))
