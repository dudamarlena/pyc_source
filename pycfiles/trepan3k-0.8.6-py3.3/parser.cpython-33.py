...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/trepan/processor/parse/parser.py
...
DEFAULT_DEBUG = {'rules': False,  'transition': False,  'reduce': False,  'errorstack': None, 
 'dups': False, 
 'local_print': False}
...
class LocationParser(GenericASTBuilder):
    ...
    def error(self, tokens, index):
        ...
        if self.debug.get('local_print', False):
            ...
            if 'context' in self.debug:
                if self.debug['context']:
                    super(LocationParser, self).error(tokens, index)
        raise LocationError(self.text, ' ' * (token.offset + len(str(token.value))) + '^')
    ...
    def nonterminal(self, nt, args):
        ...
        if has_len and len(args) == 1 and hasattr(args[0], '__len__') and len(args[0]) == 1:
...
def parse_location(start_symbol, text, out=sys.stdout, show_tokens=False, parser_debug=DEFAULT_DEBUG):
