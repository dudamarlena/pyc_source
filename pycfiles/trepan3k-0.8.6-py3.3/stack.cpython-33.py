...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/trepan/lib/stack.py
# Compiled at: 2018-02-01 07:53:21
# Size of source mod 2**32: 11442 bytes
...
import reprlib as Mrepr, inspect
...
def format_stack_entry(dbg_obj, frame_lineno, lprefix=': ', include_location=True, color='plain'):
    ...
    if '<module>' == funcname and ([], None, None) == (args, varargs, varkw):
        ...
        if is_exec_stmt(frame):
            ...
            s += ' %s(%s)' % (
             format_token(Mformat.Function, fn_name, highlight=color), source_text)
        else:
...
def frame2filesize(frame):
    ...
    if bc_path:
        version, timestamp, magic_int, co, is_pypy, bc_source_size = xdis.load_module(bc_path, fast_load=True, get_code=False)
        return (
         fs_size, bc_source_size)
    else:
...
def print_dict(s, obj, title):
    if hasattr(obj, '__dict__'):
        d = obj.__dict__
        if isinstance(d, types.DictType) or isinstance(d, types.DictProxyType):
            keys = list(d.keys())
            if len(keys) == 0:
                s += '\n  No %s' % title
            else:
