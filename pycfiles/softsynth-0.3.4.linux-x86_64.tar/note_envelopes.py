...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/synth/note_envelopes.py
# Compiled at: 2015-07-03 15:39:42
...
class MidiTrackNoteEnvelope(object):
    ...
    def get_notes_for_range(self, options, phase, nr_of_samples):
        result = []
        while self.event_pointer < self.nr_of_events and self.events[self.event_pointer].start_time + self.nr_of_loops * self.track_length <= phase + nr_of_samples:
            ...
            if event.start_time + offset >= phase:
                ...
                result.append(NoteEvent(event.start_time + offset, event.stop_time + offset, event.param1, event.param2))
