...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/rc/util.py
# Compiled at: 2020-04-24 01:10:26
...
def run(cmd: Union[(str, List[str])], *, shell=[
 '/bin/sh', '-c'], input=None, timeout=None, text=True):
...
def running(cmd: Union[(str, List[str])], *, shell=[
 '/bin/sh', '-c'], input=None, text=True):
    if type(cmd) is list:
        cmd = convert_list_command_to_str(cmd)
    try:
        if not shell:
            shell = []
        p = subprocess.Popen([*shell, cmd], stdin=(subprocess.PIPE if input else None), stdout=(subprocess.PIPE),
...
def run_stream(cmd: Union[(str, List[str])], *, shell=[
 '/bin/sh', '-c'], input=None, text=True):
    p = running(cmd, shell=shell, input=input, text=text)
    q = Queue()
...
def print_stream(q, *, prefix):
    return handle_stream(q, stdout_handler=(lambda line: print(prefix, 'STDOUT |', line, end='')), stderr_handler=(lambda line: print(prefix,
      'STDERR |', line, end='')),
    ...
    with open(os.path.join(path, name + '.stdout'), 'w') as (stdout):
        with open(os.path.join(path, name + '.stderr'), 'w') as (stderr):
            with open(os.path.join(path, name + '.exitcode'), 'w') as (exitcode):
                return handle_stream(q, stdout_handler=(lambda line: stdout.write(line)), stderr_handler=(lambda line: stderr.write(line)),
