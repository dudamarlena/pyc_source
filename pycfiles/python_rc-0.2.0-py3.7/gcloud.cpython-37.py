...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/rc/provider/gcloud.py
# Compiled at: 2020-04-24 01:13:03
...
from rc.exception import MachineCreationException, MachineNotRunningException, MachineShutdownException, MachineDeletionException, MachineChangeTypeException, MachineNotReadyException, SaveImageException, DeleteImageException, FirewallRuleCreationException, FirewallRuleDeleteionException, MachineBootupException, RcException
...
def list():
    ...
    for line in lines:
        zone, name, ip = re.split('\\s+', line)
        result.append(Machine(provider=gcloud_provider, name=name, zone=zone,
          ip=ip,
...
def get(name):
    ...
    if p.stdout.strip('\n') == '':
        return
    zone, name, ip = re.split('\\s+', p.stdout.strip('\n'))
    return Machine(provider=gcloud_provider, name=name, zone=zone,
      ip=ip,
      username=(_get_username()),
      ssh_key_path=SSH_KEY_PATH)
...
def create(name, *, project=None, machine_type, disk_size, image_project, image_family=None, image=None, zone, min_cpu_platform=None, preemptible=False, firewall_allows=None, reserve_ip=True, firewalls=None, disk_type=None):
    args = [name]
            ...
            if not preemptible:
                if reserve_ip:
                    ...
                    if p.returncode != 0:
                        ...
                        raise MachineCreationException(p.stderr)
        machine = Machine(provider=gcloud_provider, name=name, zone=zone, ip=ip,
          username=(_get_username()),
          ssh_key_path=SSH_KEY_PATH)
        machine.wait_ssh()
        return machine
