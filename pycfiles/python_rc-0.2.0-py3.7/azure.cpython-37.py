...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/rc/provider/azure.py
# Compiled at: 2019-12-26 14:31:46
# Size of source mod 2**32: 2845 bytes
from rc.util import run
from rc.exception import MachineCreationException, MachineDeletionException, MachineShutdownException, MachineBootupException, SaveImageException
...
def create(*, name, machine_size, disk_size_gb=None, image, location):
    ...
    machine = Machine(provider=azure_provider, name=name, zone=None, ip=ip,
...
def get(name):
    ...
    if p.returncode == 0:
        ...
        if not res:
            return
        ip_address = res[0]['virtualMachine']['network']['publicIpAddresses'][0]['ipAddress']
        return Machine(provider=azure_provider, name=name, zone=None, ip=ip_address,
          username=(os.getlogin()),
          ssh_key_path=SSH_KEY_PATH)
    return
