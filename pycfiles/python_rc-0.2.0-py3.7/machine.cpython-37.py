...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/rc/machine.py
# Compiled at: 2020-04-24 02:43:55
...
from rc.util import run, run_stream, convert_list_command_to_str, bash, sudo, python, python2, python3, running, kill, ok
...
class Machine:
    ...
    def __eq__(self, other):
        return (
         self.provider, self.name, self.zone, self.ip, self.username, self.ssh_key_path) == (other.provider, other.name, other.zone, other.ip, other.username, other.ssh_key_path)

    def __repr__(self):
        return 'Machine(provider={provider}, name={name}, zone={zone}, ip={ip}, username={username}, ssh_key_path={ssh_key_path})'.format(provider=(self.provider),
          name=(self.name),
          zone=(self.zone),
          ip=(self.ip),
    ...
    def upload(self, local_path, machine_path, switch_user=None, su=None, user=None):
        ...
        if user:
            ...
            rsync = ''
        p = run(f"\nrsync -e 'ssh -o StrictHostKeyChecking=no -i {self.ssh_key_path}' -r     {rsync} --progress {local_path} {self.username}@{self.ip}:{machine_path}\n")
        if p.returncode != 0:
    ...
    def download(self, machine_path, local_path, sudo=True):
        if sudo:
            ...
            rsync = ''
        p = run(f"\nrsync -e 'ssh -o StrictHostKeyChecking=no -i {self.ssh_key_path}' -r     {rsync} --progress {self.username}@{self.ip}:{machine_path} {local_path}\n")
        if p.returncode != 0:
    ...
    def sudo(self, script, *, shell=None, user='root', timeout=None, flag='set -euo pipefail'):
        return sudo(script, shell=shell, user=user, timeout=timeout, flag=flag, run_shell=(self._ssh_shell()))

    def bash(self, script, *, timeout=None, flag='set -euo pipefail', login=False, interactive=False):
        return bash(script, timeout=timeout, login=login, interactive=interactive, flag=flag, run_shell=(self._ssh_shell()))
    ...
    def run_bg(self, script, *, cmd='bash', stdout='/tmp/python-rc.log', stderr='/tmp/python-rc.log', exitcode='/tmp/python-rc.exitcode', pid='/tmp/python-rc.pid', user=None):
        ts = datetime.datetime.strftime(datetime.datetime.utcnow(), '%Y%m%d_%H%M%S')
        ok(self.ensure_dir('/tmp/python-rc'))
    ...
    def ensure_user(self, username, *, sudo=True, pubkey=True):
        ...
        if pubkey is True:
            pubkey = run(f"ssh-keygen -y -f {self.ssh_key_path}").stdout.strip()
        if pubkey:
