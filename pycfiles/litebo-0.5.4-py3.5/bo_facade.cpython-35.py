...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/litebo/facade/bo_facade.py
# Compiled at: 2020-05-03 04:57:58
# Size of source mod 2**32: 4934 bytes
import abc, logging, numpy as np
...
class BayesianOptimization(BaseFacade):
    ...
    def __init__(self, objective_function, config_space, time_limit_per_trial=180, max_runs=200, task_id=None, rng=None):
        ...
        self.optimizer = InterleavedLocalAndRandomSearch(acquisition_function=self.acquisition_function, config_space=self.config_space, rng=np.random.RandomState(seed=rng.randint(MAXINT)), max_steps=self.sls_max_steps, n_steps_plateau_walk=self.sls_n_steps_plateau_walk)
        self._random_search = RandomSearch(self.acquisition_function, self.config_space, rng)
    ...
    def choose_next(self, X: np.ndarray, Y: np.ndarray):
        ...
        challengers = self.optimizer.maximize(runhistory=self.history_container, num_points=1000, random_configuration_chooser=self.random_configuration_chooser)
