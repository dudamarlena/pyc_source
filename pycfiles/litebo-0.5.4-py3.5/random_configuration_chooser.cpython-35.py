...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/litebo/optimizer/random_configuration_chooser.py
...
class ChooserNoCoolDown(RandomConfigurationChooser):
    __doc__ = 'Interleave a random configuration after a constant number of configurations found by Bayesian optimization.\n\n    Parameters\n    ----------\n    modulus : float\n        Every modulus-th configuration will be at random.\n\n    '
...
class ChooserLinearCoolDown(RandomConfigurationChooser):
    ...
    def __init__(self, start_modulus: float=2.0,
                 modulus_increment: float=0.3, end_modulus: float=np.inf):
...
class ChooserCosineAnnealing(RandomConfigurationChooser):
    __doc__ = 'Interleave a random configuration according to a given probability which is decreased according to a cosine\n    annealing schedule.\n\n    Parameters\n    ----------\n    prob_max : float\n        Initial probility of a random configuration\n    prob_min : float\n        Lowest probility of a random configuration\n    restart_iteration : int\n        Restart the annealing schedule every ``restart_iteration`` iterations.\n    rng : np.random.RandomState\n        Random state\n    '

    def __init__(self, prob_max: float, prob_min: float, restart_iteration: int, rng: np.random.RandomState):
        self.logger = logging.getLogger(self.__module__ + '.' + self.__class__.__name__)
    ...
    def next_smbo_iteration(self) -> None:
        self.prob = self.prob_min + 0.5 * (self.prob_max - self.prob_min) * (1 + np.cos(self.iteration * np.pi / self.restart_iteration))
