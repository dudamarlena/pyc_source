...
# Embedded file name: build/bdist.macosx-10.6-universal/egg/comicvine_api.py
# Compiled at: 2010-09-02 08:41:59
__doc__ = "Simple-to-use Python interface to Comic Vine's API (www.comicvine.com)\nModified from http://github.com/dbr/tvdb_api\n\nExample usage:\n\n>>> from comicvine_api import Comicvine\n>>> c = Comicvine()\n>>> c['Y: The Last Man'][1]['issuename']\n'Unmanned'\n"
__author__ = 'swc/Steve'
__version__ = '1.04'
import os, re, sys, urllib, urllib2, StringIO, tempfile, warnings, logging, datetime
try:
...
from comicvine_exceptions import comicvine_error, comicvine_userabort, comicvine_seriesnotfound, comicvine_issuenotfound, comicvine_attributenotfound
lastTimeout = None
...
class Issue(dict):
    ...
    def __repr__(self):
        issno = float(self.get('issue_number', 0))
        issname = self.get('issuename')
...
class Comicvine():
    ...
    def __init__(self, interactive=False, select_first=False, debug=False, cache=True, credits=False, custom_ui=None, apikey=None, forceConnect=False):
        ...
        self.series = SeriesContainer()
        self.corrections = {}
        self.config = {}
        if apikey is not None:
            self.config['apikey'] = apikey
        else:
            self.config['apikey'] = '630af825c6c615476f9f89cac4ba0d232c434e90'
        ...
        if self.config['debug_enabled']:
            warnings.warn('The debug argument to comicvine_api.__init__ will be removed in the next version. To enable debug messages, use the following code before importing: import logging; logging.basicConfig(level=logging.DEBUG)')
            logging.basicConfig(level=logging.DEBUG)
        self.config['base_url'] = 'http://api.comicvine.com'
        self.config['url_getSeries'] = '%(base_url)s/search/?api_key=%(apikey)s&query=%%s&resources=volume&offset=%%s&field_list=name,id' % self.config
        self.config['url_issInfo'] = '%(base_url)s/issue/%%s/?api_key=%(apikey)s' % self.config
        self.config['url_seriesInfo'] = '%(base_url)s/volume/%%s/?api_key=%(apikey)s' % self.config
        self.config['url_siteDetail'] = '%%s?sort=issue_number&page=%%s' % self.config
        return
