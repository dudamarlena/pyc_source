...
# [GCC 8.4.0]
# Embedded file name: /elicit/ui.py
# Compiled at: 2019-11-20 00:02:26
# Size of source mod 2**32: 15814 bytes
"""
User Interface base classes and themes.
"""
__all__ = [
 'UserInterface', 'FormatWrapper']
...
class UserInterface:
    __doc__ = 'An ANSI terminal user interface for CLIs.  '
    ...
    def choose(self, somelist, defidx=0, prompt=None):
        return simpleui.choose(somelist, defidx,
          (self._get_prompt('PS3', prompt)),
          input=(self._io.input),
          error=(self.error))

    def choose_value(self, somemap, default=None, prompt=None):
        return simpleui.choose_value(somemap, default,
          (self._get_prompt('PS3', prompt)),
          input=(self._io.input),
          error=(self.error))

    def choose_key(self, somemap, default=None, prompt=None):
        return simpleui.choose_key(somemap, default,
          (self._get_prompt('PS3', prompt)),
          input=(self._io.input),
          error=(self.error))

    def choose_multiple(self, somelist, chosen=None, prompt=None):
        return simpleui.choose_multiple(somelist, chosen,
          (self._get_prompt('PS3', prompt)),
          input=(self._io.input),
          error=(self.error))

    def choose_multiple_from_map(self, somemap, chosen=None, prompt=None):
        return simpleui.choose_multiple_from_map(somemap, chosen,
          (self._get_prompt('PS3', prompt)),
          input=(self._io.input),
          error=(self.error))

    def get_text(self, msg=None):
        return simpleui.get_text((self._get_prompt('PS4')), msg, input=(self._io.input))

    def get_value(self, prompt, default=None):
        return simpleui.get_input(self.prompt_format(prompt), default, self._io.input)
    ...
    def get_int(self, prompt='', default=None):
        return simpleui.get_int(prompt, default, input=(self._io.input), error=(self.error))

    def get_float(self, prompt='', default=None):
        return simpleui.get_float(prompt, default, input=(self._io.input), error=(self.error))

    def get_bool(self, prompt='', default=None):
        return simpleui.get_bool(prompt, default, input=(self._io.input),
          error=(self.error))

    def yes_no(self, prompt, default=True):
        while True:
            yesno = simpleui.get_input(self.prompt_format(prompt), 'Y' if default else 'N', self._io.input)
            yesno = yesno.upper()
    ...
    def _initfsm(self):
        theme = self._theme
        self._PROMPT_EXPANSIONS = {'I':PROMPT_START_IGNORE + theme.BRIGHT + PROMPT_END_IGNORE, 
         ...
         't':self._time, 
         'T':self._date}
        self._FORMAT_EXPANSIONS = {'I':theme.BRIGHT, 
         'N':theme.NORMAL, 
         'D':theme.DEFAULT, 
         'R':theme.BRIGHTRED, 
         'G':theme.BRIGHTGREEN, 
    ...
    def _error(self, input_symbol, fsm):
        self._io.errlog('Prompt string error: {}\n{!r}'.format(input_symbol))
        fsm.reset()
