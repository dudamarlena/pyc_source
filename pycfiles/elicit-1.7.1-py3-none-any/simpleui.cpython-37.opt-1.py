...
# [GCC 8.4.0]
# Embedded file name: /elicit/simpleui.py
# Compiled at: 2018-08-13 00:06:39
# Size of source mod 2**32: 11917 bytes
"""
...
def choose(somelist, defidx=0, prompt='choose', input=input, error=default_error, lines=LINES, columns=COLUMNS):
    ...
    defidx = int(defidx)
    if not (defidx >= 0 and defidx < len(somelist)):
        raise AssertionError('default index out of range.')
    while True:
        try:
            ri = get_input(prompt, defidx + 1, input)
        except EOFError:
            return
        else:
            try:
...
def choose_multiple(somelist, chosen=None, prompt='choose multiple', input=input, error=default_error, lines=LINES, columns=COLUMNS):
    somelist = somelist[:]
    if chosen is None:
        chosen = []
    while 1:
        ...
        try:
            ri = get_input(prompt, None, input)
        except EOFError:
            return chosen
        else:
            if not ri:
...
def choose_value(somemap, default=None, prompt='choose', input=input, error=default_error, lines=LINES, columns=COLUMNS):
...
def choose_key(somemap, default=0, prompt='choose', input=input, error=default_error, lines=LINES, columns=COLUMNS):
...
def choose_multiple_from_map(somemap, chosen=None, prompt='choose multiple', input=input, error=default_error, lines=LINES, columns=COLUMNS):
