...
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/tests/test_compare.py
# Compiled at: 2019-03-09 01:11:08
import mock, pytest, types
...
unicode_query_results = [{'a': 'one', 'b': 'two', 'c': 'three'}, {'a': 'four', 'b': 'five', 'c': 'six'}]
...
def test_comparatorset():
    ...
    with pytest.raises(InvalidCompSetException):
        ComparatorSet([
         sp2, sp2], comps=[
         comps.LEN_COMP, comps.FIRST_COMP], names='Shirley')
    names = ['Shirley', 'Eugene']
    with pytest.raises(InvalidCompSetException):
        ComparatorSet([
         sp2, sp2], comps=comps.COMPS.get(comps.LEN_COMP), names=names)
    with pytest.raises(InvalidCompSetException):
        ComparatorSet([
         sp2, sp2], comps=[
         comps.LEN_COMP, 'nope'], names=names)
    cmps = [[comps.LEN_COMP], [comps.FIRST_COMP]]
    cs = ComparatorSet([
     sp2, sp2], comps=cmps, names=names)
