...
# Embedded file name: /home/olek/Wszystkie_aggrescamy_swiata/aggrescan3d/a3d_gui/views.py
# Compiled at: 2018-11-27 07:29:30
...
class MyForm(Form):
    name = StringField('Project name', validators=[
     Length(min=4, max=50), optional()], description='Project name', default='')
    input_pdb_code = StringField('PDB code', validators=[
     pdb_input_validator,
     pdb_code_validator], description='1YWO', default='')
    chain = StringField('Chain', validators=[
     Length(min=1, max=5),
     optional(), single_chain_validator], description='A', default='')
    input_file = FileField('Local PDB file', validators=[
     pdb_file_validator,
     non_empty_file])
    jid = HiddenField(default=unique_id())
    foldx = RadioField('Stability calculations', validators=[
     foldx_validator], choices=[
     ('1', 'Yes'), ('0', 'No')], default='1')
    mutation = RadioField('Mutate residues', choices=[
     ('1', 'Yes'), ('0', 'No')], validators=[
     foldx_and_m_validator], default='0')
    auto_mutation = RadioField('Enhance protein solubility', choices=[
     ('1', 'Yes'), ('0', 'No')], validators=[
     auto_mut_validator], default='0')
    dynamic = RadioField('Dynamic mode', choices=[
     ('1', 'Yes'), ('0', 'No')], default='0')
    distance = RadioField('Distance of aggregation analysis: ', choices=[
     (
      '5', ('5Å').decode('utf-8')),
     (
      '10', ('10Å').decode('utf-8'))], default='10')
...
def add_init_data_to_db(form):
    ...
    if mutate == '1':
        ...
        query_db('INSERT INTO project_details (jid, chain_numbering, chain_sequence,                  dynamic, mutate, distance, chains, foldx)                  VALUES(?,?,?,?,?,?,?,?)', [
...
def job_status(jid):
    try:
        ...
        flash("Your job tried to access files that don't exists. Perhaps a job with status done had its files removed? Or you tried to set done on a job that misses aggrescan files.  Changed status to error.")
        flash('Your error: %s' % str(e))
        query_db("UPDATE user_queue SET status='error'                  WHERE jid=?", [jid], insert=True)
        return job_status(jid)

    aa_t = aminoacids()
    return render_template('job_info.html', jid=jid, aa_dict=aa_t, info=all_info)


@app.route('/mutate/<jid>', methods=['GET', 'POST'])
def mutate(jid):
    q = query_db('SELECT chain_numbering FROM project_details                   WHERE jid=?', [jid], one=True)
    chains = json.loads(q['chain_numbering'])
    aa_t = aminoacids()
    return render_template('mutate.html', sequence=chains, jid=jid, aa_dict=aa_t)
...
def auto_mutate(jid):
    ...
    chains = json.loads(q['chain_numbering'])
    aa_t = aminoacids()
    return render_template('auto_mutate.html', sequence=chains, jid=jid, aa_dict=aa_t)
...
def up_remutation():
    if request.method == 'POST':
        ...
        old_queue = query_db('SELECT project_name, working_dir FROM                 user_queue WHERE jid=?', [old_jid], one=True)
        ...
        shutil.copy(os.path.join(old_directory, 'output.pdb'), os.path.join(dest_directory, 'input.pdb'))
        ...
        query_db('INSERT INTO project_details(jid, dynamic, mutate, mutation, chains, foldx, distance, chain_sequence, chain_numbering) VALUES(?,?,?,?,?,?,?,?,?)', [
...
def up_mutation():
    if request.method == 'POST':
        ...
        mut = (', ').join(request.form.getlist('toreplace[]'))
        details = query_db('SELECT project_name, working_dir FROM user_queue WHERE                 jid=?', [jid], one=True)
        proname = details['project_name'] + ' [mutate: ' + mut + ']'
        query_db("UPDATE user_queue SET status='running',                 project_name=? WHERE jid=?", [proname, jid], insert=True)
...
def get_job_status(jid):
    response = {}
    details = query_db('SELECT working_dir,status,jid FROM user_queue WHERE             jid=?', [jid], one=True)
    try:
...
def queue_page(page=1):
    ...
    if request.method == 'GET':
        ...
        if search != '':
            ...
            q = query_db("SELECT project_name, jid, status,                     started datet FROM user_queue                     WHERE status!='mut_wait' AND (project_name LIKE ? OR                     jid=?) ORDER BY started DESC LIMIT ?,?", [
             '%' + search + '%', search, before,
             app.config['PAGINATION']])
            q_all = query_db("SELECT status FROM user_queue WHERE                     status!='mut_wait' AND (project_name LIKE ? OR jid=?)                     ORDER BY started DESC", ['%' + search + '%', search])
            out = parse_out(q)
            if len(out) == 0:
    ...
    qall = query_db("SELECT status FROM user_queue WHERE             status!='mut_wait' ORDER BY started DESC", [])
    q = query_db("SELECT jid, project_name, status, started datet                   FROM user_queue WHERE status != 'mut_wait'                   ORDER BY started DESC LIMIT ?,?", [before, app.config['PAGINATION']])
