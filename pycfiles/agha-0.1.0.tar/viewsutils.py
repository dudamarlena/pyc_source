...
# Embedded file name: /home/olek/Wszystkie_aggrescamy_swiata/aggrescan3d/a3d_gui/viewsutils.py
# Compiled at: 2018-11-27 08:08:24
...
def page_not_found(error):
    ...
    return (
     render_template('page_not_found.html', code='404'), 404)
...
def page_error(error):
    app.logger.warning('Page not found (IP: ' + request.remote_addr + ') ' + request.url)
    return (
     render_template('page_not_found.html', code='500'), 500)
...
def ustatic_folder(jobid, folder, filename):
    data_dir = query_db('SELECT working_dir FROM user_queue WHERE             jid=?', [jobid], one=True)[0]
...
def save_uri(jobid):
    try:
        ...
        query_db('INSERT INTO pictures (jid, filename, type, model)                  VALUES(?,?,?,?)', [
         jobid, filename, pic_type, model], insert=True)
        return Response('%s : %s' % (url_for('serve_pic', filename=filename), filename), status=200, mimetype='text/csv')
...
def serve_pic(filename):
    ...
    if os.path.exists(path):
        ...
        return Response('Requested file %s does not exist in pictures folder' % filename, status=400, mimetype='text/plain')
...
def delete_pic():
    ...
    if os.path.exists(path):
        try:
            ...
            return Response("Failed to delete the file. Perhaps you don't have necessary permissions?" % filename, status=400, mimetype='text/plain')
...
def get_pics(jobid):
    ...
    if query is None:
        ...
        results = []
        for row in query:
            results.append({'path': url_for('serve_pic', filename=row['filename']), 'model': row['model'], 
               'type': row['type'], 'filename': row['filename']})
...
def get_mut_plot(jobid):
    ...
    try:
        ...
        return Response('No plots were generated as the Mutations.csv file is missing', status=400, mimetype='text/plain')
