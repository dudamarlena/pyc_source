...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sfws/products.py
# Compiled at: 2020-05-05 16:07:20
...
class Products:
    ...
    def shop_product_search(self, access_token, fields, phrase, url_params, order=None, api='/s/MasAbrazos_AR/dw/shop/'):
        if self.client:
            params = {'query':{'text_query': {'fields':fields, 
    ...
    def product_search(self, access_token, fields, phrase, order=None, api='/s/-/dw/data/'):
        if self.client:
            params = {'query':{'text_query': {'_type':'text_query', 
                             'fields':fields, 
                             'search_phrase':phrase}}, 
             'expand':[
              'prices'], 
    ...
    def get_product(self, access_token, _id, api='/s/-/dw/data/'):
        if self.client:
            params = {'expand': ['availability, images, prices, variations']}
    ...
    def update_product_inventory_record(self, access_token, parameters, api='/s/-/dw/data/'):
        if self.client:
            params = {'allocation': {'amount': parameters['amount']}}
            if 'single_inventory' in parameters:
                if 'single_product_id' in parameters:
                    params = {'allocation': {'amount': parameters['amount']}}
                ...
                for product in products:
                    ...
                    self.client.set_api(api)
                    self.client.set_request_path(request_end_point, 'PATCH')
                    params = {'allocation': {'amount': current_product_amount}}
                    ocapi_response = self.client.do_request(access_token, params)
