...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/yars.py
# Compiled at: 2010-08-14 12:58:38
"""
...
class YARS(object):
    ...
    def add_feed(self, caption, url):
        ...
        self.db.mset({self.prefix + '/feed/' + str(n) + '/url': url, self.prefix + '/feed/' + str(n) + '/caption': caption, 
    ...
    def remove_feed(self, feed_number):
        ...
        if self.db.srem(self.prefix + '/feeds', n):
            for k in [self.prefix + '/feed/' + n + '/url', self.prefix + '/feed/' + n + '/caption',
    ...
    def remove_item(self, item_number):
        ...
        if self.db.srem(self.prefix + '/wanted', n):
            for k in [self.prefix + '/wanted/' + n + '/caption', self.prefix + '/wanted/' + n + '/search_words',
    ...
    def _get_all_new_feed_entries(self, feeds, dry_run=False):
        ...
        for n in feeds:
            ...
            getter.append(g)
            if self.verbose:
                print 'Starting fetching for feed %s.' % feeds[n]['caption']
            g.start()
    ...
    def _find_desired_items_in(self, all_items):
        ...
        for n in desired:
            for item in all_items:
                if string_contains_all(item['title'], desired[n]['search_words']) and not string_contains_any(item['title'], desired[n]['exclude_words']):
