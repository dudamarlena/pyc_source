...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\hnc_forms_ext\controls.py
# Compiled at: 2013-10-30 09:52:54
...
class SlideShareUrl(formencode.validators.String):
    messages = dict(tooLong='Enter a value not more than %(max)i characters long', tooShort='Enter a value %(min)i characters long or more', notSlideshareUrl='Please add a valid slideshare url. You can find it in the sharing options of the presentation.')

    def _to_python(self, value, state):
        result = super(SlideShareUrl, self)._to_python(value, state)
        if not getSlideshareMeta(state, value):
            raise formencode.api.Invalid(self.message('notSlideshareUrl', state, max=self.max), value, state)
        else:
            return result
...
class VideoUrl(formencode.validators.String):
    messages = dict(tooLong='Enter a value not more than %(max)i characters long', tooShort='Enter a value %(min)i characters long or more', notVideoUrl='Please add a valid youtube or vimeo url. You can find it in the browser address bar when watching the video.')

    def _to_python(self, value, state):
        result = super(VideoUrl, self)._to_python(value, state)
        meta = getYoutubeMeta(state, value) or getVimeoMeta(state, value)
        if not meta:
            raise formencode.api.Invalid(self.message('notVideoUrl', state, max=self.max), value, state)
        else:
            return result
...
class LongitudeLatitudePicker(Field):
    ...
    def getValidator(self, request):
        return deep_dict(self.name, formencode.Schema(**{self.latName: formencode.validators.Number(**self.getValidatorArgs()), 
