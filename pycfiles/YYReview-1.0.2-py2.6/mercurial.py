...
class MercurialClient(SCMClient):
    ...
    def __init__(self, **kwargs):
        ...
        self._hg_root = ''
        self._remote_path = ()
        self._hg_env = {'HGRCPATH': os.devnull, 
           'HGPLAIN': '1'}
        self._remote_path_candidates = [
         'reviewboard', 'origin', 'parent',
         'default']
    ...
    def get_repository_info(self):
        if not check_install('hg --help'):
            ...
            if not svn_info.startswith('abort:') and not svn_info.startswith('hg: unknown command') and not svn_info.lower().startswith('not a child of'):
    ...
    def _calculate_remote_path(self):
        for candidate in self._remote_path_candidates:
            ...
            if not self._remote_path and self.hgrc.get(rc_key):
                self._remote_path = (
                 candidate, self.hgrc.get(rc_key))
                logging.debug('Using candidate path %r: %r' % self._remote_path)
                return

    def _calculate_hgsubversion_repository_info(self, svn_info):
        self._type = 'svn'
        m = re.search('^Repository Root: (.+)$', svn_info, re.M)
        if not m:
            return None
        else:
            path = m.group(1)
            m2 = re.match('^(svn\\+ssh|http|https|svn)://([-a-zA-Z0-9.]*@)(.*)$', path)
            if m2:
                path = '%s://%s' % (m2.group(1), m2.group(3))
            m = re.search('^URL: (.+)$', svn_info, re.M)
            if not m:
                return None
    ...
    def hg_root(self):
        if not self._hg_root:
            root = execute(['hg', 'root'], env=self._hg_env, ignore_errors=True)
    ...
    def extract_summary(self, revision):
        ...
        return execute(['hg', 'log', '-r%s' % revision, '--template',
         '{desc|firstline}'], env=self._hg_env)
    ...
    def _get_outgoing_diff(self, files):
        ...
        outgoing_changesets = self._get_outgoing_changesets(current_branch, remote)
        if outgoing_changesets:
            (top_rev, bottom_rev) = self._get_top_and_bottom_outgoing_revs(outgoing_changesets)
        else:
            top_rev = None
            bottom_rev = None
        if self.options.guess_summary and not self.options.summary:
        ...
        if self.options.guess_description and not self.options.description:
            self.options.description = self.extract_description(bottom_rev, top_rev)
        if bottom_rev is not None and top_rev is not None:
            full_command = [
             'hg', 'diff', '-r', str(bottom_rev), '-r',
             str(top_rev)] + files
            return (
             execute(full_command, env=self._hg_env), None)
        else:
            return ('', None)
            return
    ...
    def _get_outgoing_changesets(self, current_branch, remote):
        ...
        raw_outgoing = execute(['hg', '-q', 'outgoing', '--template',
         'b:{branches}\nr:{rev}\n\n', remote], env=self._hg_env, extra_ignore_errors=(1, ))
    ...
    def _get_top_and_bottom_outgoing_revs(self, outgoing_changesets):
        ...
        for rev in reversed(outgoing_changesets):
            parents = execute(['hg', 'log', '-r', str(rev),
             '--template', '{parents}'], env=self._hg_env)
    ...
    def diff_between_revisions(self, revision_range, args, repository_info):
        ...
        return (
         execute(['hg', 'diff', '-r', r1, '-r', r2], env=self._hg_env), None)
