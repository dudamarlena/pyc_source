...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\rbtools\clients\git.py
...
class GitClient(SCMClient):
    ...
    def get_repository_info(self):
        if not check_install('git --help'):
            if sys.platform.startswith('win') and check_install('git.cmd --help'):
                ...
                return
        git_dir = execute([self.git, 'rev-parse', '--git-dir'], ignore_errors=True).rstrip('\n')
        if git_dir.startswith('fatal:') or not os.path.isdir(git_dir):
            ...
            if not self.bare:
                git_top = execute([self.git, 'rev-parse', '--show-toplevel'], ignore_errors=True).rstrip('\n')
            ...
            if not self.options.repository_url and os.path.isdir(git_svn_dir) and len(os.listdir(git_svn_dir)) > 0:
                    ...
                    if m:
                        ...
                        if m:
                            ...
                            if self.options.parent_branch:
                                ...
                                m = re.search('^Remote Branch:\\s*(.+)$', data, re.M)
                                if m:
                                    self.upstream_branch = m.group(1)
                                else:
                                    sys.stderr.write('Failed to determine SVN tracking branch. Defaultingto "master"\n')
                                    self.upstream_branch = 'master'
                            return SVNRepositoryInfo(path=path, base_path=base_path, uuid=uuid, supports_parent_diffs=True)
                else:
                    version = execute([self.git, 'svn', '--version'], ignore_errors=True)
                    version_parts = re.search('version (\\d+)\\.(\\d+)\\.(\\d+)', version)
                    svn_remote = execute([self.git, 'config', '--get',
                     'svn-remote.svn.url'], ignore_errors=True)
                    if version_parts and not self.is_valid_version((int(version_parts.group(1)),
                     int(version_parts.group(2)),
                     int(version_parts.group(3))), (1, 5, 4)) and svn_remote:
                        die('Your installation of git-svn must be upgraded to version 1.5.4 or later')
            self.upstream_branch = ''
            if self.head_ref:
                short_head = self._strip_heads_prefix(self.head_ref)
                merge = execute([self.git, 'config', '--get',
                 'branch.%s.merge' % short_head], ignore_errors=True).strip()
                remote = execute([self.git, 'config', '--get',
                 'branch.%s.remote' % short_head], ignore_errors=True).strip()
            ...
            if self.options.repository_url:
                ...
                (self.upstream_branch, origin_url) = self.get_origin(self.upstream_branch, True)
            ...
            if url:
                self.type = 'git'
                return RepositoryInfo(path=url, base_path='', supports_parent_diffs=True)
            return
    ...
    def get_origin(self, default_upstream_branch=None, ignore_errors=False):
        ...
        """
        upstream_branch = self.options.tracking or default_upstream_branch or 'origin/master'
        upstream_remote = upstream_branch.split('/')[0]
        origin_url = execute([self.git, 'config', '--get',
         'remote.%s.url' % upstream_remote], ignore_errors=True).rstrip('\n')
    ...
    def scan_for_server(self, repository_info):
        ...
        if server_url:
            ...
            url = execute([self.git, 'config', '--get', 'reviewboard.url'], ignore_errors=True).strip()
            if url:
                return url
            if self.type == 'svn':
    ...
    def diff(self, args):
        ...
        if self.options.guess_summary and not self.options.summary:
            s = execute([self.git, 'log', '--pretty=format:%s', 'HEAD^..'], ignore_errors=True)
        ...
        if self.options.guess_description and not self.options.description:
            self.options.description = execute([
             ...
             (parent_branch or self.merge_base) + '..'], ignore_errors=True).strip()
    ...
    def diff_between_revisions(self, revision_range, args, repository_info):
        ...
        if ':' not in revision_range:
            ...
            if self.options.guess_description and not self.options.description:
                self.options.description = execute([
                 self.git, 'log', '--pretty=format:%s%n%n%b',
                 revision_range + '..'], ignore_errors=True).strip()
            return (
             self.make_diff(revision_range), parent_diff_lines)
        else:
            ...
            if self.options.guess_description and not self.options.description:
                self.options.description = execute([
                 self.git, 'log', '--pretty=format:%s%n%n%b',
                 '%s..%s' % (r1, r2)], ignore_errors=True).strip()
