...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\rbtools\clients\plastic.py
...
class PlasticClient(SCMClient):
    ...
    def get_repository_info(self):
        if not check_install('cm version'):
            ...
            self.workspacedir = execute(['cm', 'gwp', '.', '--format={1}'], split_lines=False, ignore_errors=True).strip()
            logging.debug('Workspace is %s' % self.workspacedir)
            split = execute(['cm', 'ls', self.workspacedir, '--format={8}'], split_lines=True, ignore_errors=True)
            split = filter(None, split)
            m = re.search('^rep:(.+)$', split[0], re.M)
            if not m:
                return
            path = m.group(1)
            return RepositoryInfo(path, supports_changesets=True, supports_parent_diffs=False)
    ...
    def diff(self, args):
        ...
        if changenum is None:
            ...
            return (
             self.changenum_diff(changenum), None)
            return
    ...
    def branch_diff(self, args):
        ...
        diff_entries = execute(['cm', 'diff', branch,
         '--format={status} {path} rev:revid:{revid} rev:revid:{parentrevid} src:{srccmpath} dst:{dstcmpath}{newline}'], split_lines=True)
    ...
    def process_diffs(self, my_diff_entries):
        ...
        for f in my_diff_entries:
            ...
            if not f:
                continue
            m = re.search('(?P<type>[ACMD]) (?P<file>.*) (?P<revspec>rev:revid:[-\\d]+) (?P<parentrevspec>rev:revid:[-\\d]+) src:(?P<srcpath>.*) dst:(?P<dstpath>.*)$', f)
            if not m:
            ...
            if changetype == 'M':
                ...
                dl = self.diff_files(tmp_diff_from_filename, empty_filename, oldfilename, 'rev:revid:-1', oldspec, changetype)
                diff_lines += dl
                self.write_file(newfilename, newspec, tmp_diff_to_filename)
                dl = self.diff_files(empty_filename, tmp_diff_to_filename, newfilename, newspec, 'rev:revid:-1', changetype)
                diff_lines += dl
            else:
    ...
    def diff_files(self, old_file, new_file, filename, newrevspec, parentrevspec, changetype):
        """
