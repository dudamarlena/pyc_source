...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\rbtools\postreview.py
# Compiled at: 2012-09-04 05:45:40
import base64, cookielib, getpass, logging, mimetools, os, re, sys, urllib2
...
options = None
configs = []
ADD_REPOSITORY_DOCS_URL = 'http://www.reviewboard.org/docs/manual/dev/admin/configuration/repositories/'
...
class PresetHTTPAuthHandler(urllib2.BaseHandler):
    ...
    def http_request(self, request):
        if options.username and not self.used:
            (username, password) = self.password_mgr.find_user_password('Web API', self.url)
            raw = '%s:%s' % (username, password)
            request.add_header(urllib2.HTTPBasicAuthHandler.auth_header, 'Basic %s' % base64.b64encode(raw).strip())
            self.used = True
        return request
...
class ReviewBoardHTTPBasicAuthHandler(urllib2.HTTPBasicAuthHandler):
    ...
    def retry_http_basic_auth(self, *args, **kwargs):
        ...
        if not self._retried:
            self._retried = True
            self.retried = 0
            response = urllib2.HTTPBasicAuthHandler.retry_http_basic_auth(self, *args, **kwargs)
...
class ReviewBoardServer(object):
    ...
    def __init__(self, url, info, cookie_file):
        ...
        password_mgr = ReviewBoardHTTPPasswordMgr(self.url, options.username, options.password)
    ...
    def login(self, force=False):
        ...
        """
        if options.diff_filename == '-':
            if not (self.has_valid_cookie() or options.username and options.password):
                die('Authentication information needs to be provided on the command line when using --diff-filename=-')
            ...
            try:
                self.api_post('api/json/accounts/login/', {'username': username, 
                   'password': password})
            except APIError, e:
    ...
    def new_review_request(self, changenum, submit_as=None):
        ...
        if isinstance(self.info.path, list):
            ...
            if isinstance(self.info.path, list):
                ...
                sys.stderr.write('\n')
                sys.stderr.write('There was no matching repository pathfound on the server.\n')
        ...
        try:
            ...
            if e.error_code == 204:
                ...
                if options.diff_only:
                    ...
                    self.update_review_request_from_changenum(changenum, rsp['review_request'])
            elif e.error_code == 206:
                sys.stderr.write('\n')
                sys.stderr.write('There was an error creating this review request.\n')
                sys.stderr.write('\n')
    ...
    def update_review_request_from_changenum(self, changenum, review_request):
        if self.deprecated_api:
            self.api_post('api/json/reviewrequests/%s/update_from_changenum/' % review_request['id'])
        else:
            self.api_put(review_request['links']['self']['href'], {'changenum': review_request['changenum']})
    ...
    def set_review_request_field(self, review_request, field, value):
        ...
        debug("Attempting to set field '%s' to '%s' for review request '%s'" % (
         field, value, rid))
        if self.deprecated_api:
            self.api_post('api/json/reviewrequests/%s/draft/set/' % rid, {field: value})
        else:
            self.api_put(review_request['links']['draft']['href'], {field: value})
    ...
    def get_repositories(self):
        ...
        if self.deprecated_api:
            ...
            rsp = self.api_get(self.root_resource['links']['repositories']['href'])
            repositories = rsp['repositories']
    ...
    def get_repository_info(self, rid):
        ...
        if self.deprecated_api:
            ...
            rsp = self.api_get('%s%s/' % (self.root_resource['links']['repositories']['href'],
             rid))
    ...
    def save_draft(self, review_request):
        ...
        """
        if self.deprecated_api:
            self.api_post('api/json/reviewrequests/%s/draft/save/' % review_request['id'])
        else:
            self.api_put(review_request['links']['draft']['href'], {'public': 1})
    ...
    def upload_diff(self, review_request, diff_content, parent_diff_content):
        ...
        files['path'] = {'filename': 'diff', 
           'content': diff_content}
        if parent_diff_content:
            files['parent_diff_path'] = {'filename': 'parent_diff', 'content': parent_diff_content}
        if self.deprecated_api:
            self.api_post('api/json/reviewrequests/%s/diff/new/' % review_request['id'], fields, files)
        else:
            self.api_post(review_request['links']['diffs']['href'], fields, files)
    ...
    def reopen(self, review_request):
        ...
        debug('Reopening')
        if self.deprecated_api:
            self.api_post('api/json/reviewrequests/%s/reopen/' % review_request['id'])
        else:
            self.api_put(review_request['links']['self']['href'], {'status': 'pending'})
    ...
    def publish(self, review_request):
        ...
        debug('Publishing')
        if self.deprecated_api:
            self.api_post('api/json/reviewrequests/%s/publish/' % review_request['id'])
        else:
            self.api_put(review_request['links']['draft']['href'], {'public': 1})
    ...
    def process_error(self, http_status, data):
        ...
        try:
            ...
            raise APIError(http_status, rsp['err']['code'], rsp, rsp['err']['msg'])
        except ValueError:
...
def tempt_fate(server, tool, changenum, diff_content=None, parent_diff_content=None, submit_as=None, retries=3):
    """
    ...
    try:
        if options.rid:
            ...
            if status == 'submitted':
                die('Review request %s is marked as %s. In order to update it, please reopen the request using the web interface and try again.' % (
                 options.rid, status))
        else:
            review_request = server.new_review_request(changenum, submit_as)
        if options.target_groups:
            server.set_review_request_field(review_request, 'target_groups', options.target_groups)
        if options.target_people:
            server.set_review_request_field(review_request, 'target_people', options.target_people)
        if options.summary:
            server.set_review_request_field(review_request, 'summary', options.summary)
        if options.branch:
            server.set_review_request_field(review_request, 'branch', options.branch)
        if options.bugs_closed:
            options.bugs_closed = options.bugs_closed.strip(', ')
            bug_set = set(re.split('[, ]+', options.bugs_closed)) | set(review_request['bugs_closed'])
            options.bugs_closed = (',').join(bug_set)
            server.set_review_request_field(review_request, 'bugs_closed', options.bugs_closed)
        if options.description:
            server.set_review_request_field(review_request, 'description', options.description)
        if options.testing_done:
            server.set_review_request_field(review_request, 'testing_done', options.testing_done)
        if options.change_description:
            server.set_review_request_field(review_request, 'changedescription', options.change_description)
    except APIError, e:
        if e.error_code == 103:
            retries = retries - 1
            if retries >= 0:
                server.login(force=True)
                return tempt_fate(server, tool, changenum, diff_content, parent_diff_content, submit_as, retries=retries)
        if options.rid:
...
def parse_options(args):
    parser = OptionParser(usage='%prog [-pond] [-r review_id] [changenum]', version='YYReview ' + get_version_string())
    parser.add_option('-p', '--publish', dest='publish', action='store_true', default=get_config_value(configs, 'PUBLISH', False), help='publish the review request immediately after submitting')
    parser.add_option('-r', '--review-request-id', dest='rid', metavar='ID', default=None, help='existing review request ID to update')
    parser.add_option('-o', '--open', dest='open_browser', action='store_true', default=get_config_value(configs, 'OPEN_BROWSER', False), help='open a web browser to the review request page')
    parser.add_option('-n', '--output-diff', dest='output_diff_only', action='store_true', default=False, help='outputs a diff to the console and exits. Does not post')
    parser.add_option('--server', dest='server', default=get_config_value(configs, 'REVIEWBOARD_URL'), metavar='SERVER', help='specify a different Review Board server to use')
    parser.add_option('--disable-proxy', action='store_true', dest='disable_proxy', default=not get_config_value(configs, 'ENABLE_PROXY', True), help='prevents requests from going through a proxy server')
    parser.add_option('--diff-only', dest='diff_only', action='store_true', default=False, help='uploads a new diff, but does not update info from changelist')
    parser.add_option('--reopen', dest='reopen', action='store_true', default=False, help='reopen discarded review request after update')
    parser.add_option('--target-groups', dest='target_groups', default=get_config_value(configs, 'TARGET_GROUPS'), help='names of the groups who will perform the review')
    parser.add_option('--target-people', dest='target_people', default=get_config_value(configs, 'TARGET_PEOPLE'), help='names of the people who will perform the review')
    parser.add_option('--summary', dest='summary', default=None, help='summary of the review ')
    parser.add_option('--description', dest='description', default=None, help='description of the review ')
    parser.add_option('--description-file', dest='description_file', default=None, help='text file containing a description of the review')
    parser.add_option('-g', '--guess-fields', dest='guess_fields', action='store_true', default=get_config_value(configs, 'GUESS_FIELDS', False), help='equivalent to --guess-summary --guess-description')
    parser.add_option('--guess-summary', dest='guess_summary', action='store_true', default=get_config_value(configs, 'GUESS_SUMMARY', False), help='guess summary from the latest commit (git/hg/hgsubversion only)')
    parser.add_option('--guess-description', dest='guess_description', action='store_true', default=get_config_value(configs, 'GUESS_DESCRIPTION', False), help='guess description based on commits on this branch (git/hg/hgsubversion only)')
    parser.add_option('--testing-done', dest='testing_done', default=None, help='details of testing done ')
    parser.add_option('--testing-done-file', dest='testing_file', default=None, help='text file containing details of testing done ')
    parser.add_option('--branch', dest='branch', default=get_config_value(configs, 'BRANCH'), help='affected branch ')
    parser.add_option('--bugs-closed', dest='bugs_closed', default=None, help='list of bugs closed ')
    parser.add_option('--change-description', default=None, help='description of what changed in this revision of the review request when updating an existing request')
    parser.add_option('--revision-range', dest='revision_range', default=None, help='generate the diff for review based on given revision range')
    parser.add_option('--submit-as', dest='submit_as', default=get_config_value(configs, 'SUBMIT_AS'), metavar='USERNAME', help='user name to be recorded as the author of the review request, instead of the logged in user')
    parser.add_option('--username', dest='username', default=get_config_value(configs, 'USERNAME'), metavar='USERNAME', help='user name to be supplied to the reviewboard server')
    parser.add_option('--password', dest='password', default=get_config_value(configs, 'PASSWORD'), metavar='PASSWORD', help='password to be supplied to the reviewboard server')
    parser.add_option('--change-only', dest='change_only', action='store_true', default=False, help='updates info from changelist, but does not upload a new diff (only available if your repository supports changesets)')
    parser.add_option('--parent', dest='parent_branch', default=get_config_value(configs, 'PARENT_BRANCH'), metavar='PARENT_BRANCH', help='the parent branch this diff should be against (only available if your repository supports parent diffs)')
    parser.add_option('--tracking-branch', dest='tracking', default=get_config_value(configs, 'TRACKING_BRANCH'), metavar='TRACKING', help='Tracking branch from which your branch is derived (git only, defaults to origin/master)')
    parser.add_option('--p4-client', dest='p4_client', default=get_config_value(configs, 'P4_CLIENT'), help='the Perforce client name that the review is in')
    parser.add_option('--p4-port', dest='p4_port', default=get_config_value(configs, 'P4_PORT'), help='the Perforce servers IP address that the review is on')
    parser.add_option('--p4-passwd', dest='p4_passwd', default=get_config_value(configs, 'P4_PASSWD'), help='the Perforce password or ticket of the user in the P4USER environment variable')
    parser.add_option('--svn-changelist', dest='svn_changelist', default=None, help='generate the diff for review based on a local SVN changelist')
    parser.add_option('--repository-url', dest='repository_url', default=get_config_value(configs, 'REPOSITORY'), help='the url for a repository for creating a diff outside of a working copy (currently only supported by Subversion with --revision-range or --diff-filename and ClearCase with relative paths outside the view). For git, this specifiesthe origin url of the current repository, overriding the origin url supplied by the git client.')
    parser.add_option('-d', '--debug', action='store_true', dest='debug', default=get_config_value(configs, 'DEBUG', False), help='display debug output')
    parser.add_option('--diff-filename', dest='diff_filename', default=None, help='upload an existing diff file, instead of generating a new diff')
    parser.add_option('--http-username', dest='http_username', default=get_config_value(configs, 'HTTP_USERNAME'), metavar='USERNAME', help='username for HTTP Basic authentication')
    parser.add_option('--http-password', dest='http_password', default=get_config_value(configs, 'HTTP_PASSWORD'), metavar='PASSWORD', help='password for HTTP Basic authentication')
    (globals()['options'], args) = parser.parse_args(args)
    if options.debug:
        logging.getLogger().setLevel(logging.DEBUG)
    if options.description and options.description_file:
        sys.stderr.write('The --description and --description-file options are mutually exclusive.\n')
        sys.exit(1)
    ...
    if options.testing_done and options.testing_file:
        sys.stderr.write('The --testing-done and --testing-done-file options are mutually exclusive.\n')
        sys.exit(1)
    ...
    if options.reopen and not options.rid:
        sys.stderr.write('The --reopen option requires --review-request-id option.\n')
        sys.exit(1)
    if options.change_description and not options.rid:
        sys.stderr.write('--change-description may only be used when updating an existing review-request\n')
        sys.exit(1)
    return args
...
def main():
    ...
    if options.revision_range:
        (diff, parent_diff) = tool.diff_between_revisions(options.revision_range, args, repository_info)
    ...
    elif isinstance(tool, SVNClient):
        (diff, parent_diff) = tool.diff(args, server.info)
    else:
        (diff, parent_diff) = tool.diff(args)
    if len(diff) == 0:
        die("There don't seem to be any diffs!")
    if (isinstance(tool, PerforceClient) or isinstance(tool, PlasticClient)) and changenum is not None:
        changenum = tool.sanitize_changenum(changenum)
        if parse_version(server.rb_version) >= parse_version('1.5.2') and parse_version(server.rb_version) <= parse_version('1.5.3.1'):
            debug('Using changenums on Review Board %s, which is broken. Falling back to the deprecated 1.0 API' % server.rb_version)
            server.deprecated_api = True
    if options.output_diff_only:
        print diff,
        sys.exit(0)
    server.login()
    review_url = tempt_fate(server, tool, changenum, diff_content=diff, parent_diff_content=parent_diff, submit_as=options.submit_as)
