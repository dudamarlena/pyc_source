...
class ClearCaseClient(SCMClient):
            ...
            for line in property_lines:
                ...
                if properties[0] == 'Properties:':
                    if 'webview' in properties:
                        die('Webviews are not supported. You can use post-review only in dynamic or snapshot view.')
    ...
    def check_options(self):
        if (self.options.revision_range or self.options.tracking) and self.viewtype != 'dynamic':
            die('To generate diff using parent branch or by passing revision ranges, you must use a dynamic view.')
    ...
    def get_branch_changeset(self, branch):
        ...
        output = execute([
         ...
         'brtype(%s)' % branch,
         '-exec',
         'cleartool descr -fmt "%En\\t%PVn\\t%Vn\\n" ' + CLEARCASE_XPN], extra_ignore_errors=(1, ), with_errors=False)
        if output:
            changeset = self._construct_changeset(output)
    ...
    def find_server_repository_info(self, server):
        ...
        for repository in repositories:
            ...
            logging.debug('Matching repository uuid:%s with path:%s' % (uuid,
             info['repopath']))
            return ClearCaseRepositoryInfo(info['repopath'], info['repopath'], uuid)
