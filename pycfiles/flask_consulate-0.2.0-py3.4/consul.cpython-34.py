...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/flask_consulate/consul.py
# Compiled at: 2017-04-16 13:46:21
# Size of source mod 2**32: 3908 bytes
import os, json, consulate
...
class Consul(object):
    ...
    def apply_remote_config(self, namespace=None):
        ...
        if namespace is None:
            namespace = 'config/{service}/{environment}/'.format(service=os.environ.get('SERVICE', 'generic_service'), environment=os.environ.get('ENVIRONMENT', 'generic_environment'))
        for k, v in iteritems(self.session.kv.find(namespace)):
            ...
            msg = "Set {k}={v} from consul kv '{ns}'".format(k=k, v=v, ns=namespace)
