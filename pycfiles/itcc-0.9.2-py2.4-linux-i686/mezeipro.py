...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/ccs2/mezeipro.py
...
def _wrappyramid(atmidx, coords, dismat):
    u"""_wrappyramid((i1, i2, i3, i4), coords, dismat)
...
def propyramida(atmidx, coords, dismat):
    ...
    for i3coord in _wrappyramid((i3, i1, i4, i5), coords, dismat):
        ...
        i2coords = tuple(_wrappyramid((i2, i1, i3, i4), newcoords, dismat))
...
class R6sub:
    ...
    def __call__(self, angle):
        ...
        newcoords[i5] = p5
        resultsa = propyramida((i5, i4, i3, i2, i1), newcoords, self.dismat)
...
def R6a(coords, atmidx, dismat):
    ...
    d46ref = dismat[(i4, i6)]
    d46mat = [ [ getd46rela(ps[1], ps[3], d46ref) for ps in result ] for result in results ]
    results = []
    for i in range(steps):
        for j in range(16):
            if d46mat[i][j] is not None and d46mat[(i + 1)][j] is not None and d46mat[i][j] * d46mat[(i + 1)][j] <= 0:
                try:
                    angle = rtbis.rtbis(d46_Resolver(r6sub, j, d46ref), i * stepsize, (i + 1) * stepsize, math.radians(1.0))
                    result = tuple(r6sub(angle))[j]
                    yield {i3: result[0], i4: result[1], i5: result[2], i6: result[3], i7: result[4]}
                except rtbis.Error:
                    pass
