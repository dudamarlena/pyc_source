...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/molecule/utils.py
...
def detailcmp():
    ...
    bonds_data = []
    for (i, j) in r1.bonds:
        if atoms is not None and (i not in atoms or j not in atoms):
            continue
        l1 = mol1.calclen(i, j)
        l2 = mol2.calclen(i, j)
    ...
    for (i, j, k) in r1.angles:
        if atoms is not None and (i not in atoms or j not in atoms or k not in atoms):
            continue
    ...
    for (i, j, k, l) in r1.torsions:
        if atoms is not None and (i not in atoms or j not in atoms or k not in atoms or l not in atoms):
            continue
...
def sub_pyramid_check(fname, atoms):
    ...
    for atom in atoms:
        ...
        if len(neis) != 4:
            continue
        if is_pyramid(mol.coords[atom], mol.coords[neis[0]], mol.coords[neis[1]], mol.coords[neis[2]], mol.coords[neis[3]]):
            res.append(atom)
