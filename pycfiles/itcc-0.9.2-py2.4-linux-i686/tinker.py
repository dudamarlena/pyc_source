...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/tinker/tinker.py
# Compiled at: 2008-04-20 13:19:45
__revision__ = '$Rev$'
import os, sys, os.path, tempfile, itertools, subprocess, shutil
from itcc.molecule import read, relalist, write
debug = False
curdir = False
TNK_ROOT = os.getenv('TNK_ROOT', '')
if TNK_ROOT:
...
def _which(cmdname):
    if subprocess.call(['which', cmdname], stdout=file('/dev/null', 'w')) == 0:
        return True
    else:
        return False
...
def analyze(mol, forcefield):
    ...
    lines = subprocess.Popen([cmdname, molfname, forcefield, 'E'], stdout=subprocess.PIPE).communicate()[0].splitlines()
    for line in lines:
        if 'Total' in line:
            try:
...
def optimize_minimize_mol(cmdname, mol, forcefield, converge=0.01, prefix=None):
    ...
    ofile.flush()
    ofname = ifname + '_2'
    lines = subprocess.Popen([progpath, ifname, forcefield, str(converge)], stdout=subprocess.PIPE).communicate()[0].splitlines()
    result = None
    for line in lines:
...
def optimize_minimize_file(cmdname, ifname, forcefield, converge=0.01):
    ofname = ifname + '_2'
    lines = subprocess.Popen([cmdname, ifname, forcefield, str(converge)], stdout=subprocess.PIPE).communicate()[0].splitlines()
    result = None
    for line in lines:
        if line.find('Function') != -1:
            result = float(line.split()[(-1)])
...
def newton_file(ifname, forcefield, converge=0.01):
    ofname = ifname + '_2'
    lines = subprocess.Popen(['newton', ifname, forcefield, 'A', 'A', str(converge)], stdout=subprocess.PIPE).communicate()[0].splitlines()
    result = None
    for line in lines:
        if line.find('Function') != -1:
            result = float(line.split()[(-1)])
...
def vibratemol(mol, forcefield):
    ...
    if cmdname is None:
        raise Error()
    forcefield = getparam(forcefield)
    lines = subprocess.Popen([cmdname, molfname, forcefield, '0'], stdout=subprocess.PIPE).communicate()[0].splitlines()
...
class NewtonMol(object):
    ...
    def newton_mol(self, mol, forcefield, converge=0.01, prefix=None):
        ...
        ofile.flush()
        ofname = ifname + '_2'
        ifile = subprocess.Popen(['newton', ifname, forcefield, 'A', 'A', str(converge)], stdout=subprocess.PIPE).communicate()[0]
        result = None
