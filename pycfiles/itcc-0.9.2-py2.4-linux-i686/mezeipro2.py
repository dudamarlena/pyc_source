...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/ccs2/mezeipro2.py
...
class Mezeipro2:
    ...
    def __call__(self):
        ...
        for i in range(self.steps):
            p2 = self.p2o + self.p2x * math.cos(stepsize * i) + self.p2y * math.sin(stepsize * i)
    ...
    def _theta_to_p2(self, theta):
        return self.p2o + self.p2x * math.cos(theta) + self.p2y * math.sin(theta)
    ...
    def get_p3(self, p2):
        (results, error) = pyramid.pyramid(self._coord(0), self._coord(1), p2, self._dismatrix(0, 3), self._dismatrix(1, 3), self._dismatrix(2, 3))
    ...
    def get_p6(self, p3):
        (results, error) = pyramid.pyramid(p3, self._coord(8), self._coord(9), self._dismatrix(6, 3), self._dismatrix(6, 8), self._dismatrix(6, 9))
        if error >= self.threshold:
    ...
    def get_p4(self, p2, p3, p6):
        (results, error) = pyramid.pyramid(p2, p3, p6, self._dismatrix(4, 2), self._dismatrix(4, 3), self._dismatrix(4, 6))
        if error >= self.threshold:
            for result in results:
                yield result
    ...
    def get_p5(self, p3, p4, p6):
        (results, error) = pyramid.pyramid(p3, p4, p6, self._dismatrix(5, 3), self._dismatrix(5, 4), self._dismatrix(5, 6))
        if error >= self.threshold:
            yield results[0]
            yield results[1]
        else:
    ...
    def get_p7(self, p6):
        (results, error) = pyramid.pyramid(p6, self._coord(8), self._coord(9), self._dismatrix(7, 6), self._dismatrix(7, 8), self._dismatrix(7, 9))
        if error >= self.threshold:
            yield results[0]
            yield results[1]
        else:
