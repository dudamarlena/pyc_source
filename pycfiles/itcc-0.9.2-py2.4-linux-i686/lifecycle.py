...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/torsionfit/lifecycle.py
...
def life_cycle(organisms, elitism, nb_children, nb_organisms, replaceratio=0.0, neworganismgen=None):
...
"""
    ...
    for child in children:
        if child not in organisms:
            organisms.append(child)
            if len(organisms) >= nb_organisms:
                break
...
def run(organisms, elitism=1, nb_generation=10, nb_children=100, nb_organisms=10, dump_generation=0, replaceratio=0.0, neworganismgen=None):
...
If dump_generation is true, dumps all generation. Else, prints only the final best organisms."""
    for i in xrange(nb_generation):
        ...
        organisms = life_cycle(organisms, elitism, nb_children, nb_organisms, replaceratio, neworganismgen)
        if dump_generation:
            dump(organisms)
