...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/ccs2/tors.py
...
class Tors(object):
    ...
    def __eq__(self, other):
        ...
        for i in range(n):
            if max(abs(doubledata[i:i + n] - odata)) < criter or max(abs(doubledata[i:i + n] + odata)) < criter or max(abs(doubledata[i + n:i:-1] - odata)) < criter or max(abs(doubledata[i + n:i:-1] + odata)) < criter:
                return True
