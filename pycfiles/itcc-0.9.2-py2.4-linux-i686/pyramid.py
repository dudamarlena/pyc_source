...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/ccs2/pyramid.py
...
def construct_transform_matrixA(O, Ox, Oy, Oz):
    u"""建坐标变换矩阵，新的坐标系的原点位于O, x轴在Ox上，
    ..., 坐标变换从frame->WC"""
    tensor = numpy.array([[Ox[0], Oy[0], Oz[0]], [Ox[1], Oy[1], Oz[1]], [Ox[2], Oy[2], Oz[2]]])
    vector = O
    return RotationTranslation(tensor, vector)


def construct_both_transform_matrix(A, B, C):
    u"""建坐标变换矩阵，新的坐标系的原点位于A,
    x轴在AB上，z轴垂直于ABC平面,
    result[1] = frame->WC
    result[0] = WC->frame"""
