...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/tools/tordiff.py
# Compiled at: 2008-04-20 13:19:45
import sys, math, os.path, numpy
...
def tordiff(data1, data2, idxs, step=None):
    if step is None:
        ...
        data2s = [ data2.take(tuple(range(i, len(data2)) + range(i)), axis=0) for i in range(0, len(data2), step) ]
    return math.degrees(min([ math.pi - min(abs(abs(data1 - x) - math.pi)) for x in data2s ]))
...
def main():
    ...
    if tor is None or file_list is None and len(args) != 2 or file_list is not None and len(args) != 0:
        usage(sys.stderr)
        sys.exit(1)
    tor_ifile = sys.stdin
    if tor != '-':
