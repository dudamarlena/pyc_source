...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/ccs2/solvent_caflisch.py
...
def init_caflisch():
    global caflisch_dat
    if caflisch_dat is not None:
        return
    caflisch_dat = read_caflisch(pkg_resources.resource_stream(__name__, 'caflisch.dat'))
    return
...
def solvent_caflisch(mol, forcefield, debug=0):
    ...
    for i in range(len(mol)):
        if mol.atoms[i].type not in ff:
            raise RuntimeError('no corresponding caflisch type for type %i of %s' % (mol.atoms[i].type, forcefield))
    ...
    for i in range(len(data)):
        ...
        for j in range(len(data)):
            ...
            if rijsq >= max_r * max_r:
                continue
            rij = math.sqrt(rijsq)
            bij = math.pi * (ri + r_probe) * (max_r - rij) * (1 + (rj - ri) / rij)
