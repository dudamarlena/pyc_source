...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/ccs2/loopclosure.py
# Compiled at: 2008-04-20 13:19:45
import math, sys, os, os.path, bisect, heapq, pprint, time, random, tempfile, shutil, cPickle, logging, socket
...
class LoopClosure(object):
    ...
    S_NONE = 0
    S_INITED = 1
    config_keys = {'DEFAULT': (('moltypekey', str, ''), ('cmptorsfile', str, ''), ('loopfile', str, ''), ('chiral_index_file', str, ''), ('molfname', str, ''), ('is_chain', bool, False), ('log_iter', bool, False), ('check_energy_before_minimization', bool, True), ('is_achiral', str, 'auto'), ('check_minimal', bool, False), ('out_mtxyz', bool, True), ('out_mtxyz_fname', str, '%(molfname)s-out'), ('maxsteps', int, -1), ('dump_steps', int, 1000), ('np', int, 1), ('head_tail', str, 'auto'), ('loopstep', str, 'auto'), ('log_level', int, 1), ('keeprange', float, -1.0), ('searchrange', float, -1.0), ('eneerror', float, 0.00015), ('legal_min_ene', float, -100000), ('torerror', float, 10.0), ('minimal_invalid_energy_before_minimization', float, 100000.0), ('min_method', str, 'newton')), 'tinker': (('forcefield', str, 'mm2'), ('solvate', str, ''), ('tinker_key_file', str, ''), ('minconverge', float, 0.001))}
    ...
    def __init__(self, config):
        self.config = config
        methods = {str: self.config.get, bool: self.config.getboolean, int: self.config.getint, float: self.config.getfloat}
        ...
        self.loopatoms = None
        self.r6s = None
        min_func_dict = {'newton': tinker.newton_mol, 'minimize': tinker.minimize_mol, 'optimize': tinker.optimize_mol}
    ...
    def _run_single_thread(self):
        ...
        while self.maxsteps < 0 or self._step_count < self.maxsteps:
    ...
    def _run_multi_thread(self):
        ...
        some_threads_finished_condition = threading.Condition(self.mutex)
        ...
        while True:
            ...
            clear_threads()
            if not threads and not self.taskheap or self.maxsteps >= 0 and self._step_count >= self.maxsteps:
    ...
    def init_mol(self):
        self.seedmol = read.readxyz(file(os.path.join(self.olddir, self.molfname)))
        return True
    ...
    def init_loop(self):
        if self.loopatoms is None:
            if self.loopfile:
                self.loopatoms = sum([ [ int(x) - 1 for x in line.split() if line.strip() if line[0] != '#' ] for line in file(os.path.join(self.olddir, self.loopfile)).readlines() ], [])
        if self.loopatoms is None:
        ...
        if len(self.loopatoms) < 6:
            self.log("your ring is %s-member, we can't deal with ring less than 6-member.\n" % len(self.loopatoms))
            return False
    ...
    def init_check(self):
        if self.chiral_idxs is None:
            if self.chiral_index_file:
                self.chiral_idxs = sum([ [ int(x) - 1 for x in line.split() if line.strip() if line[0] != '#' ] for line in file(os.path.join(self.olddir, self.chiral_index_file)).readlines()
                                       ], [])
        if self.chiral_idxs:
        ...
        if self.cmptors is None:
            if self.cmptorsfile:
                self.cmptors = [ [ int(x) - 1 for x in line.split() if line.strip() if line[0] != '#' ] for line in file(self.cmptorsfile).readlines() ]
        if self.cmptors is None:
            if self.is_chain:
    ...
    def init(self):
        ...
        self.print_config()
        if not (self.init_env() and self.init_mol() and self.init_loop() and self.init_r6() and self.init_sidechain() and self.init_check() and self.init_tinker() and self.init_task()):
            return False
        self.print_params()
    ...
    def eneidx(self, mol, ene):
        ...
        res = None
        self.mutex.acquire()
        if self.keeprange >= 0 and self.searchrange >= 0:
            if ene > max(self.keepbound, self.searchbound):
                res = -2
        if res is None:
            for idx in range(bisect.bisect_left(self.enes, (ene - self.eneerror,)), bisect.bisect_right(self.enes, (ene + self.eneerror,))):
    ...
    def addtask(self, mol, ene):
        ...
        self.log('    Potential Surface Map       Minimum %6i %21.4f\n' % (taskidx + 1, ene))
    ...
    def reorganizeresults(self):
        ...
        newidxs = [ ene[1] for ene in self.enes if self.keeprange < 0.0 or ene[0] <= self.keepbound ]
        ...
        newmolnametmp = os.path.splitext(self.molfname)[0] + '.%0' + str(len(str(len(newidxs)))) + 'i.xyz'
        for (oldidx, oldmol) in enumerate(oldmols.read_mol_as_string()):
            try:
                newidx = newidxs.index(oldidx)
            except ValueError:
...
def getshakedata(mol, loop):
    ...
    for (idx, atomidx) in enumerate(loop):
        refidxs = [
         atomidx, dloop[(idx - 1)], dloop[(idx + 1)]]
