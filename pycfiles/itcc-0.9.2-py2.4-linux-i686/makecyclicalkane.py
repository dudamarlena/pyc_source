...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/tools/makecyclicalkane.py
...
def makecyclicalkane(n):
    ...
    for i in range(n):
        ...
        coords.append([r * math.cos(angle), r * math.sin(angle), random() * 0.1])
    ...
    for i in range(n):
        mol.addatom(H, mol.coords[i] + numpy.array([random() * 0.5, random() * 0.5, random() * 0.5]))
        mol.addatom(H, mol.coords[i] + numpy.array([random() * 0.5, random() * 0.5, random() * 0.5]))
