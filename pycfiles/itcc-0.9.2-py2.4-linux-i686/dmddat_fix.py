...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/tools/dmddat_fix.py
# Compiled at: 2008-04-20 13:19:45
__revision__ = '$Rev$'
import sys, struct, copy, os.path
...
class Olddmddat_Header:
    ...
    def pack(self):
        if self.format == 'old':
            return struct.pack(self.header_fmt, self.beadnum, self.framenum, int(self.boxsize[0] * 1000), int(self.boxsize[1] * 1000), int(self.boxsize[2] * 1000), 0)
        else:
            return struct.pack(self.header_fmt, 2, self.beadnum, self.framenum, int(self.boxsize[0] * 1000), int(self.boxsize[1] * 1000), int(self.boxsize[2] * 1000))

    def __eq__(self, other):
        return self.beadnum == other.beadnum and self.framenum == other.framenum and self.boxsize == other.boxsize
...
def change_boxsize(header):
    while True:
        ans = raw_input('old boxsize is %s, please input new boxsize, enter means not change, seperate by space: ' % (header.boxsize,))
        if not ans:
            return
        try:
            words = ans.split()
