...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/molecule/rmsd.py
...
def rmsd2(mol1, mol2, atoms1=None, atoms2=None, mirror=False):
    (coords1, coords2) = rmsd_common(mol1, mol2, atoms1, atoms2)
    if mirror:
        return min(_rmsd.rmsd2(coords1, coords2), _rmsd.rmsd2(coords1, -coords2))
    else:
...
def main_common(rmsd_func):
    ...
    parser.add_option('-H', '--no-h', dest='no_h', action='store_true', help='does not include hydrogen')
    parser.add_option('-a', '--atoms', dest='atoms', help='only compare selected atoms, 1-based', metavar='STRING')
    parser.add_option('-A', '--atomsfile', dest='atomsfile', help='read the selected atoms from file', metavar='FILE')
    parser.add_option('-b', '--atoms1', dest='atoms1', help='the selected atoms for molecule 1', metavar='STRING')
    parser.add_option('-B', '--atoms1file', dest='atoms1file', help='read the selected atoms from file', metavar='FILE')
    parser.add_option('-c', '--atoms2', dest='atoms2', help='the selected atoms for molecule 2', metavar='STRING')
    parser.add_option('-C', '--atoms2file', dest='atoms2file', help='read the selected atoms from file', metavar='FILE')
    parser.add_option('-F', '--files', dest='files', help='read the compare file lists from FILE', metavar='FILE')
    parser.add_option('-s', '--loop-step', dest='loop_step', type='int', help='logical symmetry: loop step')
    parser.add_option('-m', '--mirror', dest='mirror', action='store_true', help='also consider the mirror molecule')
    ...
    if options.atoms and options.atomsfile or options.atoms1 and options.atoms1file or options.atoms2 and options.atoms2file:
    ...
    for filepair in filelists:
        ...
        if options.no_h:
            ...
            atoms2_new = atoms2
        print rmsd_func(mol1, mol2, atoms1, atoms2_new, options.mirror, options.loop_step)
