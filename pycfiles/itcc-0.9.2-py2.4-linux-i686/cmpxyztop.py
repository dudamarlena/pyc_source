...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/molecule/cmpxyztop.py
...
def cmpatoms(mol1, mol2):
    if len(mol1) != len(mol2):
        print 'Atom numbers is not equal: %i vs %i' % (len(mol1), len(mol2))
        return False
    result = True
    for idx in range(len(mol1)):
        ...
        if atom1.no != atom2.no:
            print 'the %i atom is different: %s vs %s' % (idx + 1, atom.atomchr(atom1.no), atom.atomchr(atom2.no))
            result = False
        elif atom1.type != atom2.type:
            print "the %i atom's type is different: %i vs %i" % (idx + 1, atom1.type, atom2.type)
            result = False
    ...
    for j in range(len(connect1)):
        for i in range(j):
            if connect1[(i, j)] != connect2[(i, j)]:
                print 'the connect condition of atom %i and %i isdiffrent: %s vs %s' % (i + 1, j + 1, bool(connect1[(i, j)]), bool(connect2[(i, j)]))
                result = False
...
def main():
    ...
    parser.add_option('-c', '--chiral', action='store_true', dest='chiral', help='also check chiral')
