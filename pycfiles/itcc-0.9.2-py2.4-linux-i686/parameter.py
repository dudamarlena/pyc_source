...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/tinker/parameter.py
...
def readtorsionprm(prmfname, i, j, k, l):
    ...
    for line in ifile:
        words = line.split()
        if len(words) in [14, 17, 20, 23] and words[0] == 'torsion' and molparam.torsion_uni([ int(x) for x in words[1:5] ]) == toridx:
            fold = (len(words) - 5) // 3
            result = []
