...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/ccs2/neighbour.py
...
__all__ = [
 'calctor', 'Neighbour', 'NeighbourI', 'NeighbourII']

def calctor(coords, loopatoms):
    """return in degrees"""
    doubleloop = loopatoms * 2
    return tuple([ math.degrees(torsionangle(*tuple([ coords[j] for j in doubleloop[i:i + 4] ]))) for i in range(len(loopatoms)) ])
...
class Neighbour:
    ...
    def init(self):
        ...
        for R6idx in self.R6s:
            self.R6results[R6idx] = R6.R6_in_vivo([ self.coords[idx] for idx in R6idx ])
