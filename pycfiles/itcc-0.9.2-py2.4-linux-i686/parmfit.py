...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/torsionfit/parmfit.py
...
def printprm(param, types, folds):
    ...
    idx = 0
    for (typ, fold) in zip(types, folds):
        sys.stdout.write(str(parameter.Torsionparameter(list(typ) + param[idx:idx + fold])))
        idx += fold
...
def main():
    if len(sys.argv) != 4:
        import os.path
        print >> sys.stderr, 'Usage: %s datfname idxfname param' % os.path.basename(sys.argv[0])
        sys.exit(1)
