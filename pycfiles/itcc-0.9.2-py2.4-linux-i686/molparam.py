...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/tinker/molparam.py
...
def readprm(prmfile):
    ifile = file(prmfile)
    keys = ['forcefield', 'vdwtype', 'radiusrule', 'radiustype', 'radiussize', 'epsilonrule', 'torsionunit', 'vdw-14-scale', 'chg-14-scale', 'dielectric', 'atom', 'vdw', 'bond', 'angle', 'imptors', 'torsion', 'charge']
    result = Param()
    result.param['head'] = ''
    for line in ifile:
        words = line.split()
