...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/ccs2/mezei.py
...
class Mezei(mezeibase.MezeiBase):
    ...
    def r6_base(self, points, len1, len2, error=0.1):
        ...
        """
        assert len(points) == 4 and len(len1) == 4 and len(len2) == 5
        (p0, p1, p5, p6) = tuple(points)
        (d12, d23, d34, d45) = tuple(len1)
        (d02, d13, d24, d35, d46) = tuple(len2)
        ...
        for i in range(4):
            ...
            for j in range(steps):
                if d24s[i][j] and d24s[i][(j + 1)] and (d24s[i][j] - d24) * (d24s[i][(j + 1)] - d24) <= 0:
                    try:
