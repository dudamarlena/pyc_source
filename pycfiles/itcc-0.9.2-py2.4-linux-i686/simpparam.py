...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/tinker/simpparam.py
...
def main():
    if len(sys.argv) != 3:
        import os.path
        print >> sys.stderr, 'Usage: %s mol param' % os.path.basename(sys.argv[0])
        sys.exit(1)
    subparam(sys.argv[1], sys.argv[2])


key2key = {'atom': checkatomtypes, 'vdw': checkatomtypes, 'strbnd': checkatomtypes, 'angang': checkatomtypes, 'charge': checkatomtypes, 'piatom': checkatomtypes, 'vdwpr': checktypepairs, 'hbond': checktypepairs, 'bond': checkbondtypes, 'bond3': checkbondtypes, 'bond4': checkbondtypes, 'bond5': checkbondtypes, 'opbend': checkbondtypes, 'strtors': checkbondtypes, 'dipole': checkbondtypes, 'dipole3': checkbondtypes, 'dipole4': checkbondtypes, 'dipole5': checkbondtypes, 'pibond': checkbondtypes, 'pibond4': checkbondtypes, 'pibond5': checkbondtypes, 'angle': checkangletypes, 'angle3': checkangletypes, 'angle4': checkangletypes, 'angle5': checkangletypes, 'electneg': checkangletypes, 'torsion': checktorsiontypes, 'torsion4': checktorsiontypes, 'torsion5': checktorsiontypes}
