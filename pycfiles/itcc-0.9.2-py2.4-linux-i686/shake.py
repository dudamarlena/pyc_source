...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/tools/shake.py
...
class DatRecord:
    ...
    def __init__(self, line):
        ...
        self.idx1, self.idx2 = min(self.idx1, self.idx2), max(self.idx1, self.idx2)
        if len(words) == 5:
...
def random_vec3(scale):
    return molecule.CoordType(random.uniform(-scale, scale), random.uniform(-scale, scale), random.uniform(-scale, scale))
    ...
    while True:
        ...
        for dat in err_dats:
            if not dat.check(newmol):
                finished = False
                if dat.type == 'BOND':
                    (newmol.coords[dat.idx1], newmol.coords[dat.idx2]) = shake3(newmol.coords[dat.idx1], newmol.coords[dat.idx2], random.uniform(dat.min, dat.max))
                else:
                    (newmol.coords[dat.idx1], newmol.coords[dat.idx2]) = shake3(newmol.coords[dat.idx1], newmol.coords[dat.idx2], random.uniform(dat.min, dat.min + 2.0))
