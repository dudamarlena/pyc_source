...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/torsionfit/cmpxyz.py
...
def cmpxyz(ifname1, ifname2):
    ...
    Dds1 = [ tools.torsionangle(xyz1[x[0]], xyz1[x[1]], xyz1[x[2]], xyz1[x[3]]) for x in Ds ]
    Dds2 = [ tools.torsionangle(xyz2[x[0]], xyz2[x[1]], xyz2[x[2]], xyz2[x[3]]) for x in Ds ]
    ...
    M = maxb(dA)
    print 'max angle diff: %f, A(%i, %i, %i)' % (dA[M], As[M][0] + 1, As[M][1] + 1, As[M][2] + 1)
    M = maxb(dD)
    print 'max torsion diff: %f, D(%i, %i, %i, %i)' % (dD[M], Ds[M][0] + 1, Ds[M][1] + 1, Ds[M][2] + 1, Ds[M][3] + 1)
