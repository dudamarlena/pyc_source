...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/itcc/torsionfit/parmeval.py
...
def parmeval(datfname, param):
    ...
    writer.writerow([])
    writer.writerow(['Weighted Averrage of E_diff', tools.weightedmean(E_diffs, weights)])
    writer.writerow(['Weighted SD of E_diff', tools.weightedsd(E_diffs, weights)])
...
def main():
    if len(sys.argv) != 3:
        import os.path
        print >> sys.stderr, 'Usage: %s datfname param' % os.path.basename(sys.argv[0])
        sys.exit(1)
