...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\vbox\api\storage.py
...
class Storage(base.Child):
    kwargName = 'storage'
    expectedKwargs = {'hdd': lambda cnt: True, 
...
class HDD(Medium):
    kwargName = 'hdd'
    expectedKwargs = {'size': 1, 
       'name': (0, 1)}
    defaultKwargs = {'name': None}
    idx = property(lambda s: s.parent._getHddId(s))

    def befoureSetup(self, kwargs):
        myImage = self._findMedia()
        if myImage is None:
            createKw = {'size': kwargs['size']}
            if 'name' in kwargs:
                createKw.update({'autoname': False, 
...
class Removable(Medium):
    expectedKwargs = {'target': (0, 1)}
    defaultKwargs = {'target': lambda : None}
