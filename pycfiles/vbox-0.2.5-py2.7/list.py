...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\vbox\cli\list.py
...
class List(subCmd.Base):
    ...
    PlainOutputs = ('systemproperties', 'hostinfo')
    GroupOutputs = ('runningvms', 'hostdvds', 'hostfloppies', 'bridgedifs', 'hostonlyifs',
                    'dhcpservers', 'hdds', 'dvds', 'floppies')
    _all = None

    def all(self):
        rc, cmd, outp = self._all or self.call([])
        if not rc == 1:
            raise AssertionError(cmd)
            outp = ('').join(line.strip() for line in outp.splitlines())
            match = re.search('(\\w+(?:\\|\\w+)+)', outp)
    ...
    def _vmList(self, cmd):
        ...
        return dict((el[1], el[2]) for el in util.iterMatches('^\\s*([\'\\"])(.*?)\\1\\s+{(.*?)}\\s*$', txt))
