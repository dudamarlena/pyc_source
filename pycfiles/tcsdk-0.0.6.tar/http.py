...
# [GCC 8.4.0]
# Embedded file name: /var/workspace/workspace/tcsdk_deploy/tcsdk/tcsdk/network/http.py
...
class Session(object):
    ...
    def do_request(self, req, timeout):
        try:
            logger.debug(('send request : method {}, url {}, params {}, headers {}, timeout {}, data {}').format(req.method, req.url, req.params, req.headers, timeout, req.data))
            response = Response(self.session.request(req.method, req.url, json=req.data, params=req.params, headers=req.headers, timeout=timeout))
...
class Request(object):
    ...
    def __init__(self, method, url, data=None, params=None, headers=None):
        ...
        logger.debug(('init request : method {}, url {}, params {}, headers {}, data {}').format(self.method, self.url, self.params, self.headers, self.data))
        return
...
class Response(object):
    ...
    def __init__(self, response):
        ...
        logger.debug(('Get response headers, req-id:{0}, status: {1}, headers: {2}').format(self.request_id, self.status, self.headers))
