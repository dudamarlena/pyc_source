...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/dccs/maya/core/rivet.py
...
class Rivet(object):
    ...
    def _create_surface(self):
        ...
        edge_to_curve_1 = maya.cmds.createNode('polyEdgeToCurve',
          n=(tp.Dcc.find_unique_name('rivetCurve1_{}'.format(self._name))))
        maya.cmds.setAttr(('{}.inputComponents'.format(edge_to_curve_1)),
          2, ('vtx[{}]'.format(vert_ids[0])),
          ('vtx[{}]'.format(vert_ids[1])), type='componentList')
        vert_iterator = api.IterateEdges(shape)
        vert_ids = vert_iterator.get_connected_vertices(edge_index_2)
        edge_to_curve_2 = maya.cmds.createNode('polyEdgeToCurve',
          n=(tp.Dcc.find_unique_name('rivetCurve2_{}'.format(self._name))))
    ...
    def _create_point_on_surface(self):
        self._point_on_surface = maya.cmds.createNode('pointOnSurfaceInfo',
          n=(tp.Dcc.find_unique_name('pointOnSurface_{}'.format(self._surface))))
    ...
    def _create_aim_constraint(self):
        self._aim_constraint = maya.cmds.createNode('aimConstraint',
          n=(tp.Dcc.find_unique_name('aimConstraint_{}'.format(self._surface))))
