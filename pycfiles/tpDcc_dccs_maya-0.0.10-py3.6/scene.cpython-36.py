...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/dccs/maya/core/scene.py
# Compiled at: 2020-05-13 19:28:33
# Size of source mod 2**32: 17607 bytes
"""
Module that contains functions and classes related with attributes
"""
from __future__ import print_function, division, absolute_import
import os, string, logging, traceback, contextlib
...
def get_scene_cameras(include_default_cameras=True, include_non_default_cameras=True, get_transforms=False):
    ...
    startup_cameras = [camera for camera in cameras if maya.cmds.camera((maya.cmds.listRelatives(camera,
      parent=True)[0]),
...
def get_top_dag_nodes_in_list(list_of_transforms):
    ...
    for xform in list_of_transforms:
        ...
        if long_name and long_name[0].count('|') == 1:
            found.append(xform)
...
def get_all_scene_nodes():
    ...
    from tpDcc.dccs.maya.core import node
    obj_types = (
     maya.OpenMaya.MFn.kDagNode, maya.OpenMaya.MFn.kCharacter, maya.OpenMaya.MFn.kDependencyNode)
...
def get_sets():
    ...
    for obj_set in sets:
        if obj_set == 'defaultObjectSet':
            ...
            outputs = maya.cmds.listConnections(obj_set,
              plugs=False, connections=False, destination=True, source=False, skipConversionNodes=True)
            if not outputs:
