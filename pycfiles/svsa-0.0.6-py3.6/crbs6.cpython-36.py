...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/svsa/crbs6.py
# Compiled at: 2019-05-21 21:50:25
...
def crbs6(y, rlx_int, eukenf, c_int, c_tr, xi):
    ...
    def crbs6_solver(xi):
        ...
        i1011 = i1110
        a[:, 0] = -j020 * np.array([i0000, i0001, i0011, i0010, 0, 0]) + np.array([complex(0.0, 1.0), 0, 0, 0, 0, 0])
        a[:, 1] = -j020 * np.array([i0100, i0101, i0111, i0110, 0, 0]) + np.array([0, complex(0.0, 1.0), 0, 0, 0, 0])
        a[:, 2] = (j020 - j110) * np.array([i1100, i1101, i1111, i1110, 0, 0]) + j011110 * np.array([0, 0, 0, 0, -i0100, -i0101]) + np.array([0, 0, complex(-0.0, -1.0), 0, 0, 0])
        a[:, 3] = (j020 - j100) * np.array([i1000, i1001, i1011, i1010, 0, 0]) + j100001 * np.array([0, 0, 0, 0, -i0000, -i0001]) + np.array([0, 0, 0, complex(-0.0, -1.0), 0, 0])
        a[:, 4] = j100001 * np.array([i1000, i1001, i1011, i1010, 0, 0]) + (j001 - j020) * np.array([0, 0, 0, 0, i0000, i0001]) + np.array([0, 0, 0, 0, complex(0.0, 1.0), 0])
        a[:, 5] = j011110 * np.array([i1100, i1101, i1111, i1110, 0, 0]) + (j011 - j020) * np.array([0, 0, 0, 0, i0100, i0101]) + np.array([0, 0, 0, 0, 0, complex(0.0, 1.0)])
