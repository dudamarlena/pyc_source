...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/svsa/support_vector_spectrum.py
# Compiled at: 2019-08-28 16:19:19
...
class SupportVectorSpectrum:
    ...
    def fit(self, eps=1e-15, nu=None, gamma=1, K=5):
        ...
        if nu is not None:
            self.mods = [fitu((self.params), (self.u[:, k]), mdl=NuSVR, nu=(self.nu), gamma=gamma) for k in range(0, K)]
        else:
            self.mods = [fitu((self.params), (self.u[:, k]), mdl=SVR, epsilon=eps, gamma=gamma) for k in range(0, K)]
