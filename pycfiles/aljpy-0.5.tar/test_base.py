...
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/opensearchsdk/tests/apiclient/test_base.py
# Compiled at: 2015-12-07 06:45:30
...
RESOURCE_URL = '/index'
SPEC_URL = ''
HEADERS = {'User-Agent': 'ali-opensearch-python-client', 'Content-Type': 'application/x-www-form-urlencoded'}
BODY = {'a': '1'}
SIGN = 'sign'
...
FAKE_RESP = {'status_code': 200, 
   'text': BODY}
FAKE_INVALID_RESP = {'status_code': 200, 
   'text': BODY, 
   'raise_e': ValueError}
...
class HTTPClientTest(base.TestCase):
    ...
    def setUp(self):
        ...
        requests.request = mock.Mock(return_value=base.TestResponse(FAKE_RESP))
        self.http_client = api_base.HTTPClient(URL)

    def test_request_get(self):
        resp = self.http_client.request(GET, RESOURCE_URL)
        requests.request.assert_called_with(GET, URL + RESOURCE_URL, headers=HEADERS)
        self.assertEqual(BODY, resp)

    def test_request_post(self):
        resp = self.http_client.request(POST, RESOURCE_URL)
        requests.request.assert_called_with(POST, URL + RESOURCE_URL, headers=HEADERS)
        self.assertEqual(BODY, resp)

    @mock.patch.object(requests, 'request', return_value=base.TestResponse(FAKE_INVALID_RESP))
    def test_invalid_response(self, mock_req):
        self.assertRaises(exceptions.InvalidResponse, self.http_client.request, POST, RESOURCE_URL)

    @mock.patch.object(requests, 'request', return_value=base.TestResponse(RESP_404))
    def test_request_404(self, mock_req):
        self.assertEqual('NotFoundException: NotFoundException, app', str(exceptions.NotFoundException(details='app')))
        self.assertRaises(exceptions.NotFoundException, self.http_client.request, POST, RESOURCE_URL)

    @mock.patch.object(requests, 'request', return_value=base.TestResponse(RESP_400))
    def test_request_400(self, mock_req):
        self.assertRaises(exceptions.HttpException, self.http_client.request, POST, RESOURCE_URL)
...
class ManagerTest(base.TestCase):
    ...
    def test_request(self):
        ...
        api_base.HTTPClient.request.assert_called_with(GET, RESOURCE_URL + SPEC_URL, data=SIGNED_BODY)
