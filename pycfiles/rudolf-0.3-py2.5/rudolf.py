...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/rudolf.py
# Compiled at: 2007-12-22 13:45:06
...
##############################################################################
"""
import binascii, doctest, os, re, sys, time, traceback, unittest, warnings, nose.config, nose.core, nose.plugins, nose.util
__version__ = '0.3'
...
GRAY_STEPS = (8, 18, 28, 38, 48, 58, 68, 78, 88, 98,
 108, 118, 128, 132, 148, 158, 168, 178, 188, 198, 208,
 218, 228, 238)
...
def rgb_from_xterm(n):
    ...
    return (
     GRAY_STEPS[(n - GRAY_START)],) * 3


RGB_FROM_XTERM_COLOR = [ rgb_from_xterm(xi) for xi in range(TABLE_START, TABLE_END) ]
...
def xterm_from_rgb(rgb):
    ...
    for index in range(0, TABLE_END - TABLE_START):
        ...
        dist = (rc[0] - rgb[0]) ** 2 + (rc[1] - rgb[1]) ** 2 + (rc[2] - rgb[2]) ** 2
...
class Ansi16Color(object):
    ...
    def terminal_code(self):
        ...
        if self._bright is None:
            ...
            prefix_code = '0;'
        return '\x1b[%s%sm' % (prefix_code, fg_code)
...
def parse_color(color_text):
    ...
    colorcodes = {'default': None, 'normal': None, 'black': 0, 
       'red': 1, 
       'green': 2, 
       'brown': 3, 
       'yellow': 3, 'blue': 4, 
...
class ColorfulOutputFormatter(object):
    ...
    separator2 = '-' * 70
    doctest_template = '\nFile "%s", line %s, in %s\n\n%s\nWant:\n%s\nGot:\n%s\n'
    diff_color = {'-': 'expected-output', '+': 'actual-output', 
       '?': 'character-diffs', 
       '@': 'diff-chunk', 
       '*': 'diff-chunk', 
       '!': 'actual-output'}

    def __init__(self, verbosity, descriptions, colorscheme, stream=sys.stdout, clean_tracebacks=False, base_dir=False):
        self._stream = stream
    ...
    def start_test(self, test):
        if self._show_all:
            self._stream.write(self.colorize('normal', self.get_description(test)))
    ...
    def _format_seconds(self, n_seconds, normal='normal'):
        ...
        if n_seconds >= 60:
            ...
            return '%s seconds' % self.colorize('number', '%.3f' % n_seconds, normal)
    ...
    def print_doctest_failure(self, formatted_failure):
        ...
        for line in lines:
            if line.startswith('File '):
                m = re.match('File "(.*)", line (\\d*), in (.*)$', line)
                if m:
                    (filename, lineno, test) = m.groups()
                    if self._clean_tracebacks:
                        (filename, lineno) = elide_foreign_path_and_line_nr(self._base_dir, filename, lineno)
                ...
                if colorize_diff and len(line) > 4:
                    ...
                    print >> self._stream, self.colorize(color_of_indented_text, line)
            else:
                colorize_diff = False
                if colorize_exception:
                    self.print_colorized_traceback(('\n').join(exc_lines), indent_level=1)
                    colorize_exception = False
                    exc_lines = []
                if line.startswith('Failed example'):
                    ...
                    if line in ('Differences (ndiff with -expected +actual):', 'Differences (unified diff with -expected +actual):'):
                        line = ('').join([
        ...
        for line in formatted_traceback.splitlines():
            if line.startswith('  File'):
                ...
                if m:
                    ...
                    if self._clean_tracebacks:
                        (filename, lineno) = elide_foreign_path_and_line_nr(self._base_dir, filename, lineno)
                    tb_lines = [self.color('normal'), '  File "',
                     self.color('filename'), filename,
                     self.color('normal'), '", line ',
                     self.color('lineno'), lineno]
                    if test:
                ...
                print >> self._stream, self.colorize('failed-example', indentation + line)
            elif line.startswith('Traceback (most recent call last)'):
                print >> self._stream, indentation + line
            else:
                print >> self._stream, self.colorize('exception', indentation + line)
...
class ColorOutputPlugin(nose.plugins.Plugin):
    ...
    base_dir = None
    default_colorscheme = {'normal': 'normal', 'pass': 'green', 
       'failure': 'magenta', 
       'error': 'brightred', 
       'number': 'green', 
       'ok-number': 'green', 
       ...
       'diff-chunk': 'magenta', 
       'exception': 'red'}
    default_colorscheme = dict(((name, parse_color(color)) for (name, color) in default_colorscheme.iteritems()))
    score = 50
    ...
    def options(self, parser, env=os.environ):
        ...
        parser.add_option('--no-color', action='store_false', dest='enable_plugin_color', help="Don't output in color")
        action = sys.stdout.isatty() and 'store_true' or 'store_false'
        parser.add_option('--auto-color', action=action, dest='enable_plugin_color', help='Output in color only if stdout is a terminal')
        env_opt = 'NOSE_COLORS'
        parser.add_option('--colors', action='store', type='string', dest='colors', default=env.get(env_opt, ''), help="Colour scheme for --with-color terminal output, listing colours to be used for each named part of the output.  Format is name1=color1,name2=color2 . Colours can be specified as xterm 256 colour codes (e.g. '45'), RGB colours (e.g. 'rgb(00ff00)'), ANSI 16 colour names (e.g. 'red' or 'brightred'), and the special colour 'normal'.  Example: --colors='fail=red,pass=rgb(00ff00),error=45' " + '[%s]' % env_opt)
    ...
    def configure(self, options, conf):
        ...
        try:
            ...
            warnings.warn('Bad colorscheme string (from --colors or one of %s): %s' % (
             (', ').join(filenames), exc), RuntimeWarning)
            user_colorscheme = {}

        unknown_names = set(user_colorscheme.keys()) - set(cs.keys())
        if unknown_names:
            warnings.warn('Invalid colorscheme names: %s' % (', ').join(unknown_names))
    ...
    def setOutputStream(self, stream):
        self._stream = stream
        self._formatter = self.formatter_class(self._verbosity, True, self._colorscheme, self._stream, clean_tracebacks=self.clean_tracebacks, base_dir=self.base_dir)
    ...
    def report(self, stream):
        self._print_errors()
        self._print_summary(self._result.__start_time, time.time())
        self._result = None
        return
    ...
    def _print_summary(self, start, stop):
        ...
        if not success:
            (summary['failures'], summary['errors']) = map(len, [self._result.__failures, self._result.__errors])
            for cls in self._result.errorClasses.keys():
                (storage, label, isfail) = self._result.errorClasses[cls]
                if not isfail:
                    continue
        ...
        self._formatter.print_summary(success, summary, self._result.__tests_run, start, stop)
