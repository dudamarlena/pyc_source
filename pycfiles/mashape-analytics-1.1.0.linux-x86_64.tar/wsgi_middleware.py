...
# [GCC 8.4.0]
# Embedded file name: /root/.virtualenvs/mashape-test/lib/python2.7/site-packages/mashapeanalytics/middleware/wsgi_middleware.py
# Compiled at: 2016-05-17 13:38:35
import os, re, socket
...
class WsgiMiddleware(object):
    ...
    def host(self, env):
        if env.get('HTTP_X_FORWARDED_HOST', False):
            ...
            if env['wsgi.url_scheme'] == 'http' and env['SERVER_PORT'] == '80' or env['wsgi.url_scheme'] == 'https' and env['SERVER_PORT'] == '443':
    ...
    def wrap_start_response(self, env, start_response):
        ...
        def wrapped_start_response(status, response_headers, exc_info=None):
            ...
            def wrapped_write(body):
                write(self.count_response_content_size(env, body))
    ...
    def __call__(self, env, start_response):
        ...
        try:
            ...
            entry = {'startedDateTime': env['MashapeAnalytics.startedDateTime'].isoformat() + 'Z', 
               ...
               'request': {'method': env['REQUEST_METHOD'], 
                           ...
                           'cookies': [], 'queryString': requestQueryString, 
                           'headers': requestHeaders, 
                           'headersSize': requestHeaderSize, 
                           'bodySize': requestContentSize}, 
               'response': {'status': env['MashapeAnalytics.responseStatusCode'], 
                            'statusText': env['MashapeAnalytics.responseReasonPhrase'], 
                            'httpVersion': 'HTTP/1.1', 
                            'cookies': [], 'headers': responseHeaders, 
                            'headersSize': responseHeadersSize, 
                            'content': {'size': env['MashapeAnalytics.responseContentSize'], 
            ...
            if 'CONTENT_LENGTH' in env and env['CONTENT_LENGTH'] != '0':
                entry['request']['content'] = {'size': requestContentSize, 'mimeType': env['CONTENT_TYPE'] or 'application/octet-stream'}
            alf.addEntry(entry)
