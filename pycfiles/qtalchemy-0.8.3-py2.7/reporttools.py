...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/qtalchemy/ext/reporttools.py
# Compiled at: 2013-08-31 09:49:56
...
def ReportLabSize(str_pagesize):
    ...
    if size.upper() not in ('LETTER', 'LEGAL', 'ELEVENSEVENTEEN') + tuple(('A{0}').format(i) for i in range(7)) + tuple(('B{0}').format(i) for i in range(7)):
        raise ValueError(('the pagesize {0} is not valid').format(size))
    if mod.lower() not in ('landscape', 'portrait', ''):
...
def thousands(x, sep=',', dot='.'):
    ...
    num = re.sub('(\\d{3})(?=\\d)', '\\1' + sep, num[::-1])[::-1]
    if frac:
        num += dot + frac
    return num
...
def ColumnWidthRatios(cols, pageSize, marginLeft, marginRight):
    ...
    if detailRatios is None:
        detailRatios = [
         (pageSize[0] - totalMargins) / len(cols)] * len(cols)
    else:
...
def GeraldoElements(cols, pageSize=None, marginLeft=0, marginRight=0):
    ...
    for i in range(len(cols)):
        ...
        if cols[i].type_ is decimal.Decimal:
            ...
            style['wordWrap'] = True
        det_elements.append(ObjectValue(attribute_name=cols[i].attr, get_value=wf(cols[i].attr), top=0, left=marginLeft + sum([ detailRatios[j] for j in range(i) ]), width=detailRatios[i] - padRight, style=style))
...
def GeraldoHeaders(cols, pageSize=None, marginLeft=0, marginRight=0):
    ...
    det_elements = []
    for i in range(len(cols)):
        det_elements.append(SystemField(expression=cols[i].label, top=0, left=marginLeft + sum([ detailRatios[j] for j in range(i) ]), width=detailRatios[i]))
