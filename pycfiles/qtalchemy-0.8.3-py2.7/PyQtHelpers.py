...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/qtalchemy/PyQtHelpers.py
...
def fromQType(v, suggested=None):
    ...
    if hasattr(v, 'toPyObject'):
        if v.isNull():
            return
        v = v.toPyObject()
    elif hasattr(v, 'toPyDateTime'):
        if v.isNull():
            return
        v = v.toPyDateTime()
    elif hasattr(v, 'toPyDate'):
        if v.isNull():
            return
        v = v.toPyDate()
    elif hasattr(v, 'toPyTime'):
        if v.isNull():
            return
        v = v.toPyTime()
...
def readTableColumnGeo(table, name):
    ...
    for i in range(table.model().columnCount(None)):
        ...
        elif defaults is not None and defaults[i] is not None:
...
def message_excepthook(type, value, tb):
    ...
    """
    try:
        import traceback, qtalchemy.xplatform as xp
