...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/qtalchemy/commands.py
# Compiled at: 2013-09-24 14:36:03
"""
...
class CommandMenu(object):
    ...
    def command(self, descr, key=None, statusTip=None, type='action', iconFile=None, sort=500, default=False, requireSelection=False, viewRelated=False, needs_reload=True):
        ...
        def decorator(f):
            x = Command(descr, key=key, type=type, statusTip=statusTip, iconFile=iconFile, sort=sort, default=default, viewRelated=viewRelated, requireSelection=requireSelection, needs_reload=needs_reload)
            x.func = f
...
class BoundCommandMenuItem(object):
    ...
    def activatedProxy(self, index):
        ...
        elif self.cmd.viewRelated and self.model is not None:
            handle = index
            if handle is not None:
