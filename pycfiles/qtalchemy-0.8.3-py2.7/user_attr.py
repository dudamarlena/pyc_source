...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/qtalchemy/user_attr.py
...
class UserAttr(property):
    ...
    def fget(self, row):
        ...
        inst.getting -= 1
        if hasextendedattr(row, self.storage):
            return getextendedattr(row, self.storage)
        else:
            return default_for_type(self.base_type)
