...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/qtalchemy/PyQtModels.py
# Compiled at: 2014-02-03 08:04:35
"""
...
"""
import datetime, decimal, sqlalchemy
...
def ClassAttributeYoke(classMember):
    ...
    if type_ == datetime.date:
        ...
        if type_ == datetime.time:
            ...
            if type_ == int:
                return 'int'
            if type_ in (decimal.Decimal, float):
                return 'float'
            return 'line'
...
def AttrWidgetFactory(userattr, parent=None, index=None):
    if hasattr(userattr, 'WidgetFactory'):
        ...
        if myAttrType == int:
            ...
            return w
        if myAttrType in (decimal.Decimal, float):
            w = QtGui.QLineEdit(parent)
            ...
            if role == QtCore.Qt.DisplayRole:
                ...
                if role == QtCore.Qt.CheckStateRole:
                    ...
                    if role == QtCore.Qt.TextAlignmentRole:
                        ...
                        return int({'left': QtCore.Qt.AlignLeft, 'hcenter': QtCore.Qt.AlignHCenter, 'right': QtCore.Qt.AlignRight}[hori] | {'top': QtCore.Qt.AlignTop, 'vcenter': QtCore.Qt.AlignVCenter, 'bottom': QtCore.Qt.AlignBottom}[vert])
                    if role == QtCore.Qt.FontRole:
                        ...
                        if not (role >= QtCore.Qt.UserRole and QtCore.Qt.UserRole <= role < QtCore.Qt.UserRole + len(self.columns)):
...
def modelMimeRectangle(model, selected):
    ...
    if len(selected):
        ...
        rows.sort()
        textrows = [
         ('\t').join([ model.headerData(x, QtCore.Qt.Horizontal, QtCore.Qt.DisplayRole) for x in cols ])]
        for y in rows:
