...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/aws_secretsmanager_caching/cache/items.py
# Compiled at: 2019-05-13 16:05:47
# Size of source mod 2**32: 8927 bytes
"""Secret cache items"""
import threading
...
class SecretCacheObject:
    __doc__ = 'Secret cache object that handles the common refresh logic.'
    ...
    def get_secret_value(self, version_stage=None):
        ...
        with self._lock:
            self._SecretCacheObject__refresh()
...
class SecretCacheItem(SecretCacheObject):
    __doc__ = 'The secret cache item that maintains a cache of secret versions.'
    ...
    def _get_version(self, version_stage):
        ...
        self._versions.put_if_absent(version_id, SecretCacheVersion(self._config, self._client, self._secret_id, version_id))
        return self._versions.get(version_id).get_secret_value()


class SecretCacheVersion(SecretCacheObject):
    __doc__ = 'Secret cache object for a secret version.'
