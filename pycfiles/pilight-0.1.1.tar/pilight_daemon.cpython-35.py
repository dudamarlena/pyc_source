...
# Compiled at: 2016-10-11 04:30:19
...
PORT = 5005
SEND_DELAY = 0.1
FAKE_DATA = {'origin': 'receiver', 
 'repeats': 1, 
 'message': {'protocol': ['kaku_switch'], 
             'id': 0, 
             'unit': 0, 
             'off': 1}}

class PilightDaemon(object):
    __doc__ = 'Provide a pilight-daemon in with-statement.'
...
class PilightDeamonSim(threading.Thread):
    __doc__ = 'Simulate the pilight-daemon API for testing.'
    ...
    def _send_codes(self):
        if self.send_codes and (datetime.datetime.now() - self.last_send).total_seconds() > SEND_DELAY and len(self.client_sockets) > 0:
            for i in range(10):
