...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/qndex/nlp/nlp.py
# Compiled at: 2017-04-13 04:52:31
# Size of source mod 2**32: 1658 bytes
import argtyp, numpy as np, qnd
__all__ = [
 'NULL_INDEX',
 'UNKNOWN_INDEX',
 'def_chars',
...
def def_word_array():
    ...
    def word_array():
        ...
        word_array = np.zeros([len(get_words()),
         min(max(len(word) for word in qnd.FLAGS.words), qnd.FLAGS.word_length)], np.int32)
        for i, word in enumerate(qnd.FLAGS.words):
            for j, char in enumerate(word[:qnd.FLAGS.word_length]):
                word_array[(i, j)] = chars.index(char) if char in chars else UNKNOWN_INDEX
        ...
        if qnd.FLAGS.save_word_array_file:
            np.savetxt((qnd.FLAGS.save_word_array_file), word_array,
              fmt='%d',
              delimiter=',')
        return word_array
