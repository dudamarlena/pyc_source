...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/xbus/file_emitter/file_emitter.py
# Compiled at: 2015-05-12 09:20:59
# Size of source mod 2**32: 5184 bytes
"""Generic Xbus file emitter."""
from aiozmq import rpc
import asyncio, datetime
from decimal import Decimal
import msgpack, pyjon.descriptors as pjdesc
...
class FileEmitter(object):
    __doc__ = 'Generic Xbus file emitter.'

    def __init__(self, broker_front_url, login, password, descriptors, loop=None):
    ...
    def login(self):
        ...
        :raise FileEmitterException.
        """
        self._client = yield from rpc.connect_rpc(connect=self._broker_front_url, loop=self._loop)
    ...
    def send_files(self, files, encoding='utf-8'):
        ...
        for file, desc_name in files:
            ...
            try:
                yield from self.send_file(envelope_id, event_type, file, descriptor)
            except pjdesc.exceptions.DescriptorError as e:
                raise FileEmitterException('%s - %s' % (type(e), e))

        yield from self._client.call.end_envelope(self._token, envelope_id)
        return envelope_id
    ...
    def send_file(self, envelope_id, event_type, data_file, descriptor):
        ...
        event_id = yield from self._client.call.start_event(self._token, envelope_id, event_type, num_lines)
        if not event_id:
        ...
        for data_item in data_iter:
            ...
            yield from self._client.call.send_item(self._token, envelope_id, event_id, msgpack.packb(data, use_bin_type=True))

        yield from self._client.call.end_event(self._token, envelope_id, event_id)