...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/MICA/bin/utils.py
# Compiled at: 2019-10-04 13:04:58
# Size of source mod 2**32: 22618 bytes
"""
This module contains helper functions, essential to the execution of MICA (Mutual Information-based clustering algorithm)
"""
import sys, time, numpy as np, pandas as pd, matplotlib
...
def slice_file(df_file, out_file_name, slice_size='1000'):
    ...
    for i in range(b):
        ...
        slice_ = pd.DataFrame(data=(df.iloc[start:end, :].values),
          index=(df.index[start:end]),
          columns=(df.columns))
        slice_.to_hdf(out_file_name + '.sliced.h5', 'slice_' + slice_name)

    pd.DataFrame(data=(np.array(df.shape + (b,))), index=['row', 'col', 'slice']).to_hdf(out_file_name + '.sliced.h5', 'attr')
    pd.DataFrame(data=(df.columns), index=(df.columns)).to_hdf(out_file_name + '.sliced.h5', 'cols')
    pd.DataFrame(data=(df.index), index=(df.index)).to_hdf(out_file_name + '.sliced.h5', 'rows')
...
def patch_file(df_file, out_file_name):
    ...
    pd.DataFrame(data=(np.array(df.shape + (1, ))), index=['row', 'col', 'slice']).to_hdf(out_file_name + '.whole.h5', 'attr')
    pd.DataFrame(data=(df.columns), index=(df.columns)).to_hdf(out_file_name + '.whole.h5', 'cols')
    pd.DataFrame(data=(df.index), index=(df.index)).to_hdf(out_file_name + '.whole.h5', 'rows')
...
def calc_prep(in_file, project_name):
    ...
    """
    in_ = pd.HDFStore(in_file, 'r')
    bins = int(np.floor(in_['attr'].loc['row'] ** 0.3333333333333333))
    ...
    for i in range(b):
        ...
        for j in range(i, b):
            ...
            mat_tmp = project_name + '_' + name + '.h5.tmp'
            mat1.to_hdf(mat_tmp, key1)
            mat2.to_hdf(mat_tmp, key2)
            pd.DataFrame(data=(np.array([key1, key2, bins, m, name, project_name, b])), index=[
             'key1', 'key2',
             'num_bins', 'n_genes',
...
def calc_distance_mat(mat1, mat2, paras, method):
    ...
    if method == 'mi':
        ...
        start = time.time()
        for c in mat2.index:
            df.loc[(mat1.index, c)] = mat1.apply(calc_mi,
              axis=1, args=(mat2.loc[c, :], bins, m))
    ...
    df.to_hdf(out_file_name, str(key))
...
def merge_dist_mats(mi_slices, in_common_name, metrics):
    ...
    n_slice = int(mi_0['params'].loc[('num_slices', 0)])
...
def norm_mi_mat(in_mat_file, out_file_name):
    ...
    if in_mat_file == out_file_name + '_dist.h5':
        ...
        (df / np.sqrt(np.matmul(diag.T, diag))).to_hdf(out_file_name + '_dist.h5', 'norm_mi')
    hdf.close()


def tsne(data, max_dim, out_file_name, tag, perplexity=30, plot='True'):
    embed = manifold.TSNE(n_components=2,
      n_iter=5000,
      learning_rate=200,
...
def umap(data, max_dim, min_dist=0.25):
    embed = ump.UMAP(random_state=30,
      metric='euclidean',
      n_neighbors=10,
...
def mds(in_mat_file, max_dim, out_file_name, perplexity=30, print_plot='True', dist_method='mi'):
    ...
    V = evecs[:, evals_pos]
    Y = pd.DataFrame(data=(V.dot(L)),
      index=(df.index),
      columns=['mds_' + str(x) for x in np.arange(1, L.shape[0] + 1)])
    Y.to_hdf(out_file_name + '_reduced.h5', 'mds')
    if print_plot == 'True':
        vis = tsne(Y, max_dim, out_file_name, 'mds', perplexity, print_plot)
        vis.to_hdf(out_file_name + '_reduced', 'mds_tsne')
...
def lpl(in_mat_file, max_dim, out_file_name, perplexity=30, plot='True', dist_method='mi'):
    ...
    laplacian = manifold.SpectralEmbedding(n_components=n,
      eigen_solver='lobpcg',
      random_state=10)
    Y = pd.DataFrame(data=(laplacian.fit_transform(df)),
      index=(df.index),
...
def pca(in_mat_file, max_dim, out_file_name, perplexity=30, plot='True', dist_method='mi'):
    ...
    Y = pd.DataFrame(data=(np.transpose(pca_.fit(df).components_)),
      index=(df.index),
...
def kmeans(in_mat, n_clusters, project_name, dim, bootstrap_id):
    ...
    km_res = pd.DataFrame(data=(np.transpose(km.fit_predict(in_mat.iloc[:, 0:dim]))),
      index=(in_mat.index),
      columns=[
     'label'])
...
def aggregate(km_results, n_clusters, common_name):
    ...
    for i in range(n_iter):
        ...
        dff_div = pd.DataFrame(data=(np.array((np.diag(dff),) * dff.shape[0]).T),
          index=(dff.index),
          columns=(dff.columns))
        mem_mat = pd.DataFrame(data=(dff / dff_div == 1),
          index=(dff.index),
          columns=(dff.columns),
          dtype=(np.float32))
    ...
    clust = cluster.AgglomerativeClustering(linkage='ward',
...
def visualization(agg_mtx, reduced_mi_file, transformation, out_file_name, max_dim=0, visualize='umap', min_dist=0.25, perplexity=30):
    cclust = agg_mtx
...
def heatmap(df, linkage_, matrix, out_dir, out_file_name, tag, dendro='True'):
    ...
    subgrid = gs.GridSpecFromSubplotSpec(2,
      2,
      subplot_spec=(grid[0]),
      wspace=0.01,
      hspace=0.01,
...
def heatmap2(data, labels, out_dir, out_file_name, tag):
    ...
    cax = ax2.matshow((data.loc[(sorted_labels.index, sorted_labels.index)]),
      cmap='YlOrRd',
      interpolation=None,
      aspect='auto')
    ax2.set_xticks([])
