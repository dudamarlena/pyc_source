...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jmbo/tests/test_api.py
...
class APITestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.image.image.save(os.path.basename(IMAGE_PATH), ContentFile(open(IMAGE_PATH, 'rb').read()))
        cls.mbi1 = ModelBaseImage.objects.create(modelbase=cls.obj1, image=cls.image)
        cls.mbi2 = ModelBaseImage.objects.create(modelbase=cls.obj2, image=cls.image)
    ...
    def test_modelbase_scales(self):
        ...
        as_json = json.loads(response.content)
        self.assertEqual(response.status_code, 200)
        self.failUnless('http://testserver/jmbo/image-scale-url/%s/thumbnail/' % self.image.pk in as_json)
    ...
    def test_create_image(self):
        ...
        data = {'title': 'title', 
           'image': fp}
        response = self.client.post('/api/v1/jmbo-image/', data, format='multipart')
        self.assertEqual(response.status_code, 403)
        self.login()
        fp.seek(0)
        response = self.client.post('/api/v1/jmbo-image/', data, format='multipart')
    ...
    def test_create_modelbaseimage(self):
        ...
        data = {'modelbase': 'http://testserver/api/v1/jmbo-modelbase/%s/' % self.obj1.pk, 
           'image': 'http://testserver/api/v1/jmbo-image/%s/' % self.image.pk}
        response = self.client.post('/api/v1/jmbo-modelbaseimage/', data)
        self.assertEqual(response.status_code, 403)
        self.login()
        response = self.client.post('/api/v1/jmbo-modelbaseimage/', data)
        self.assertEqual(response.status_code, 201)
