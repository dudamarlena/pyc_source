...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jmbo/tests/test_modelbase.py
# Compiled at: 2017-09-28 08:49:42
import os
...
def set_image(obj):
    ...
    image.image.save(os.path.basename(IMAGE_PATH), ContentFile(open(IMAGE_PATH, 'rb').read()))
    ModelBaseImage.objects.create(modelbase=obj, image=image)


def set_image_override(image):
    override = ImageOverride.objects.create(image=image, photo_size=PhotoSize.objects.get(name='thumbnail'))
    override.replacement.save(os.path.basename(IMAGE_OVERRIDE_PATH), ContentFile(open(IMAGE_OVERRIDE_PATH, 'rb').read()))
...
class ModelBaseTestCase(TestCase):
    ...
    def test_as_leaf_class(self):
        ...
        extra_leaf.save()
        self.failUnless(TrunkModel.objects.get(slug=leaf.slug).as_leaf_class() == leaf)
        self.failUnless(BranchModel.objects.get(slug=leaf.slug).as_leaf_class() == leaf)
        self.failUnless(LeafModel.objects.get(slug=leaf.slug).as_leaf_class() == leaf)
        self.failUnless(ModelBase.objects.get(slug=extra_leaf.slug).as_leaf_class() == extra_leaf)
        branch = BranchModel(title='title')
        branch.save()
        self.failUnless(TrunkModel.objects.get(slug=branch.slug).as_leaf_class() == branch)
        self.failUnless(BranchModel.objects.get(slug=branch.slug).as_leaf_class() == branch)
    ...
    def test_publishing_timezone_unaware(self):
        obj_naive = ModelBase.objects.create(title='Obj1', publish_on=timezone.datetime.now() - timezone.timedelta(hours=1))
    ...
    def test_publishing_timezone_aware(self):
        obj_aware = ModelBase.objects.create(title='Obj2', publish_on=timezone.now() - timezone.timedelta(hours=1))
        call_command('jmbo_publish')
    ...
    def test_unicode(self):
        ...
        obj.publish()
        self.assertEqual(unicode(obj), 'Title (all sites)')
        obj = TestModel(title='Title')
        obj.save()
        obj.sites = [1]
        obj.publish()
        self.assertEqual(unicode(obj), 'Title (testserver)')

    def test_get_absolute_url(self):
        leaf = LeafModel.objects.create(title='title')
        extra_leaf = ExtraLeafModel.objects.create(title='title')
        self.assertEqual(leaf.get_absolute_url(), '/tests/detail/%s/' % leaf.slug)
        self.assertEqual(extra_leaf.get_absolute_url(), '/jmbo/detail/%s/' % extra_leaf.slug)

    def test_get_absolute_url_categorized(self):
        category = Category.objects.create(title='cat1', slug='cat1')
        leaf = LeafModel.objects.create(title='title', primary_category=category)
        extra_leaf = ExtraLeafModel.objects.create(title='title', primary_category=category)
        self.assertEqual(leaf.get_absolute_url_categorized(), '/tests/detail/%s/%s/' % (category.slug, leaf.slug))
        self.assertEqual(extra_leaf.get_absolute_url_categorized(), '/jmbo/detail/%s/%s/' % (category.slug, extra_leaf.slug))
    ...
    def test_get_related_items(self):
        ...
        Relation.objects.create(source=obj3, target=obj1, name='multi')
    ...
    def test_image_detail_url(self):
        ...
        self.assertTrue(leaf.image_detail_url.startswith('photologue/photos/cache/image_'))
        self.assertTrue(leaf.image_detail_url.endswith('_tests_branchmodel_detail.jpg'))
        self.assertTrue(extra_leaf.image_detail_url.startswith('photologue/photos/cache/image_'))
        self.assertTrue(extra_leaf.image_detail_url.endswith('_jmbo_modelbase_detail.jpg'))
    ...
    def test_image_list_url(self):
        ...
        extra_leaf = ExtraLeafModel.objects.create(title='title')
        set_image(extra_leaf)
        self.assertTrue(leaf.image_list_url.startswith('photologue/photos/cache/image_'))
        self.assertTrue(leaf.image_list_url.endswith('_tests_branchmodel_list.jpg'))
        self.assertTrue(extra_leaf.image_list_url.startswith('photologue/photos/cache/image_'))
        self.assertTrue(extra_leaf.image_list_url.endswith('_jmbo_modelbase_list.jpg'))

    def test_image_override_url(self):
        leaf = LeafModel.objects.create(title='title')
        set_image(leaf)
        set_image_override(leaf.image)
        self.assertTrue(leaf.image.get_thumbnail_url().endswith('override.jpg'))