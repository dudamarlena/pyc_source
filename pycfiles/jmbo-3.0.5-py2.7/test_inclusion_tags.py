...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jmbo/tests/test_inclusion_tags.py
...
class InclusionTagsTestCase(TestCase):
    ...
    def test_render_object_tag(self):
        ...
        t = template.Template('{% load jmbo_inclusion_tags %}{% render_object object "test_block" %}')
        result = t.render(self.context)
        expected = 'TestModel block\n'
        self.failUnlessEqual(result, expected)
        self.context = template.Context({'object': self.obj2})
        t = template.Template('{% load jmbo_inclusion_tags %}{% render_object object "test_block" %}')
        result = t.render(self.context)
        expected = 'BranchModel block\n'
        ...
        t = template.Template('{% load jmbo_inclusion_tags %}{% render_object object "test_block" %}')
        result = t.render(self.context)
        expected = 'ExtraLeafModel block\n'
        self.failUnlessEqual(result, expected)
        t = template.Template('{% load jmbo_inclusion_tags %}{% render_object object "foobar" %}')
        result = t.render(self.context)
        expected_result = ''
    ...
    def test_header_tag(self):
        ...
        t = template.Template('{% load jmbo_inclusion_tags %}{% object_header object %}')
        result = t.render(self.context)
    ...
    def test_footer_tag(self):
        self.context = template.Context({'object': self.obj1, 'request': self.request})
        t = template.Template('{% load jmbo_inclusion_tags %}{% object_footer object %}')
        result = t.render(self.context)
        self.failUnless('footer-inclusion' in result)

    def test_comments_tag(self):
        self.context = template.Context({'object': self.obj1, 'request': self.request})
        t = template.Template('{% load jmbo_inclusion_tags %}{% object_comments object %}')
        result = t.render(self.context)
    ...
    def test_render_tag_inherits_context(self):
        ...
        self.context = template.Context({'object': self.obj1, 'request': self.request})
        t = template.Template('{% load jmbo_inclusion_tags tests_template_tags %}{% inject_foo %}{% object_header object %}')
        result = t.render(self.context)
        self.failUnless('foo = bar' in result)

    def test_image_url_tag(self):
        self.context = template.Context({'object': self.obj1})
        t = template.Template('{% load jmbo_inclusion_tags %}{% image_url object "detail" %}')
        result = t.render(self.context)
