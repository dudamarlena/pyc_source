...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jmbo/signals.py
...
def check_slug(sender, instance, **kwargs):
    ...
    if isinstance(instance, jmbo.models.ModelBase) and kwargs['action'] == 'post_add' and sender.__name__.endswith('_sites'):
        for site in instance.sites.all():
            q = jmbo.models.ModelBase.objects.filter(slug=instance.slug, sites=site).exclude(id=instance.id)
            if q.exists():
                raise IntegrityError('The slug %s is already in use for site %s by %s' % (
