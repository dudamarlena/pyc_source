...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/idioms/models/AsyncSubprocess.py
# Compiled at: 2019-05-09 20:28:26
# Size of source mod 2**32: 10965 bytes
from collections import OrderedDict
import signal, subprocess, time, shlex, datetime
from bitstring import BitArray
import logging, json
from copy import copy
import gemeinsprache.utils as pp
...
class AsyncSubprocess(threading.Thread):
    ...
    def __init__(self, group=None, target=None, name=None, args=(), loglevel='ERROR', silent=False, **kwargs):
        super(AsyncSubprocess, self).__init__(group=None, target=None,
          name=None)
        self.args = args
        self.kwargs = kwargs
    ...
    def start(self):
        ...
        self.process = None
        self.log('info', f"Initialized new AsyncSubprocess thread with cmd: '{self.cmd}'")
    ...
    def run(self):
        ...
        if self._is_event_loop:
            ...
            while keep_going:
                if has_timeout:
                    if datetime.datetime.now() - self.time_started > max_wait:
                        self._aborted = True
                        self.kill()
    ...
    def is_running(self) -> bool:
        return bool(self._aborted is False and self.is_initialized and self.process and self.process.poll() is None)
    ...
    def read(self):
        ...
        try:
            ...
            while self.is_running:
                if i % 10000 == 0:
                    self.log('info', f"Waiting for thread {self.name} to finish running...")
                i += 1
                time.sleep(duration)
    ...
    def to_dict(self):
        ...
        unserializable_attrs = [
         '_stderr', '_started', 'logger', 'process', 'self']
