...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/jws/tests.py
# Compiled at: 2015-03-10 10:18:32
import unittest, jws, ecdsa, hashlib, Crypto.PublicKey.RSA as rsa
...
class TestJWS_helpers(unittest.TestCase):
    ...
    def test_default_algorithm_finding(self):
        names = [
         (
          'ES256', jws.algos.ECDSA), ('ES384', jws.algos.ECDSA), ('ES512', jws.algos.ECDSA),
         (
        ...
        class F7U12(jws.algos.AlgorithmBase):
            ...
            def verify(self, msg, sig, key):
                ...
                if sys.version < '3':
                    if sig == 'u mad?' + key:
                        return '<trollface>'
                elif sig == 'u mad?' + bytes(key, 'UTF-8'):
                    return '<trollface>'
...
class TestJWS_ecdsa(unittest.TestCase):
    ...
    sk512 = ecdsa.SigningKey.generate(ecdsa.NIST521p)

    def setUp(self):
        self.payload = {'whine': {'luke': 'But I was going into Tosche station to pick up some power converters!'}, 'rebuttal': {'owen': "You can waste time with your friends when you're done with your chores."}}
