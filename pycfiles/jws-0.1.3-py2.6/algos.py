...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/jws/algos.py
# Compiled at: 2015-03-10 10:22:20
...
class RSABase(HasherBase):
    ...
    def sign(self, msg, key):
        ...
        return self.padder.new(key).sign(self.hashm)
    ...
    def verify(self, msg, crypto, key):
        ...
        if not self.padder.new(private_key).verify(self.hashm, crypto):
            raise SignatureError('Could not validate signature')
        return True
...
DEFAULT = (
 (
  '^HS(?P<bits>256|384|512)$', HMAC),
 (
  '^RS(?P<bits>256|384|512)$', RSA_PKCS1_5),
 (
  '^PS(?P<bits>256|384|512)$', RSA_PSS),
 (
  '^ES(?P<bits>256|384|512)$', ECDSA))
CUSTOM = []