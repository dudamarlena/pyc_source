...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/pwserver/main.py
# Compiled at: 2010-06-18 06:47:08
...
class PasswordProtocol(LineReceiver):
    ...
    def doGenerate(self, params):
        ...
        password = pwgen.generate(randomBits=int(params.get('randombits', '47')), maxLength=params.get('maxlength', None))
    ...
    def doCheck(self, password, params):
        ...
        if username is not None:
            del params['username']
        result = pwcheck.checkPassword(password, old_password=old_password, username=username, personal=params)
        if not result:
            self.transport.write('200 OK\r\n')
...
def main():
    ...
    parser = OptionParser()
    parser.add_option('-c', '--config', dest='config_file', default='/etc/pwserverd.cfg', help='use the specified configuration file')
