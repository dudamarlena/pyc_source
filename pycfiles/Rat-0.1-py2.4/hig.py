...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/rat/hig.py
...
__all__ = ('HigProgress', 'RunDialog', 'SetupDialog', 'SetupLabel', 'SetupScrolledWindow',
           'WidgetCostumizer', 'dialog_error', 'dialog_ok_cancel', 'dialog_warn',
           'dialog_alert', 'humanize_seconds', 'save_changes', 'hig_alert')
import gobject, gtk, datetime
from gettext import gettext as _
from gettext import ngettext as N_
dialog_error = lambda primary_text, secondary_text, **kwargs: hig_alert(primary_text, secondary_text, stock=gtk.STOCK_DIALOG_ERROR, buttons=(gtk.STOCK_CLOSE, gtk.RESPONSE_OK), **kwargs)
dialog_warn = lambda primary_text, secondary_text, **kwargs: hig_alert(primary_text, secondary_text, stock=gtk.STOCK_DIALOG_WARNING, buttons=(gtk.STOCK_CLOSE, gtk.RESPONSE_OK), **kwargs)
dialog_ok_cancel = lambda primary_text, secondary_text, ok_button=gtk.STOCK_OK, run=True, **kwargs: hig_alert(primary_text, secondary_text, stock=gtk.STOCK_DIALOG_WARNING, buttons=(gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL, ok_button, gtk.RESPONSE_OK), **kwargs)
dialog_info = lambda primary_text, secondary_text, **kwargs: hig_alert(primary_text, secondary_text, stock=gtk.STOCK_DIALOG_INFO, buttons=(gtk.STOCK_CLOSE, gtk.RESPONSE_OK), **kwargs)
...
class SetupDialog(WidgetCostumizer):
    ...
    def _run(self, dialog, container):
        ...
        align = gtk.Alignment()
        align.set_padding(padding_top=0, padding_bottom=7, padding_left=0, padding_right=0)
...
class _TimeUpdater:
    ...
    def get_text(self):
        ...
        secondary_text = _("If you don't save, changes from the last %s will be permanently lost.")
...
def save_changes(files, last_save=None, parent=None, document=_('document'), documents=_('documents'), **kwargs):
    ...
    if len(files) == 1:
        ...
        if last_save is None:
            secondary_text = _("If you don't save, your changes will be permanently lost.")
        else:
            updater = _TimeUpdater(last_save)
            secondary_text = updater.get_text()
        if 'title' in kwargs:
            kwargs = {'title': kwargs['title']}
        dialog = hig_alert(primary_text, secondary_text, parent=parent, stock=gtk.STOCK_DIALOG_WARNING, buttons=(_('Close without saving'), gtk.RESPONSE_CLOSE, gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL, gtk.STOCK_SAVE, gtk.RESPONSE_OK), run=False, **kwargs)
        ...
        primary_text = _('There are %(total)s %(documents)s with unsaved changes. Save changes before closing?') % {'total': len(files), 'documents': documents}
        secondary_text = _("If you don't save, all your changes will be permanently lost.")
        dlg = hig_alert(primary_text, secondary_text, parent=parent, stock=gtk.STOCK_DIALOG_WARNING, buttons=(_('Close without saving'), gtk.RESPONSE_CLOSE, gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL, gtk.STOCK_SAVE, gtk.RESPONSE_OK), run=False, files=files, documents=documents, _setup_alert=SetupFileListSaveAlert, **kwargs)
        response = dlg.run()
        dlg.destroy()
...
def list_dialog(primary_text, secondary_text, parent=None, items=(), *args, **kwargs):
    return hig_alert(primary_text, secondary_text, parent=parent, _setup_alert=SetupListAlert, items=items, **kwargs)
