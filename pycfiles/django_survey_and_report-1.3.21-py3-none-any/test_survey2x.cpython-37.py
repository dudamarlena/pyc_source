...
# [GCC 8.4.0]
# Embedded file name: /home/pierre/workspace/django-survey/survey/tests/exporter/test_survey2x.py
...
class TestSurvey2X(TestManagement):
    ...
    def test_initially_need_update(self):
        self.assertTrue(self.actual_survey2x.need_update(), 'No file exported and the survey contain response : we should need an update. {}'.format(self.get_fail_info(self.actual_survey2x)))
    ...
    def test_need_update_after_generation(self):
        self.assertFalse(self.actual_survey2x.need_update(), 'We exported the file and there is no new response, we should not need an update. {}'.format(self.get_fail_info(self.actual_survey2x)))
    ...
    def test_need_update_after_new_response(self):
        self.assertTrue(self.actual_survey2x.need_update(), 'We exported the file but there is a new response, we should need an update. {}'.format(self.get_fail_info(self.actual_survey2x)))