...
# [GCC 8.4.0]
# Embedded file name: /home/pierre/workspace/django-survey/survey/exporter/survey2x.py
...
class Survey2X:
    __doc__ = ' Abstract class for Survey exporter. '

    def __init__(self, survey=None):
        self._check_survey(survey)
        self.survey = survey
        self._Survey2X__directory = None
    ...
    def need_update(self):
        ...
        if no_response_at_all:
            return not self._Survey2X__generation_done_once()
        file_modification_time = self.file_modification_time
        LOGGER.debug('We %sneed an update of <%s> because latest_answer_date=%s >= file_modification_time=%s is %s \n', '' if latest_answer_date > file_modification_time else 'do not ', self.survey.name, latest_answer_date, file_modification_time, latest_answer_date > file_modification_time)
