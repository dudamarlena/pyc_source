...
# Embedded file name: build/bdist.linux-x86_64/egg/test/test_orm/test_dict.py
...
class TestDeepSet(TestMetaData):
    ...
    def test_to_deep_m2o(self):
        ...
        self.assertEqual(t2.to_dict(deep={'tbl1': {}}), {'t2id': 1, 'name': 'test2', 'tbl1_t1id': 1, 'tbl1': {'name': 'test1'}})

    def test_to_deep_m2o_none(self):
        with self.session.begin():
            t2 = self.Table2(t2id=1, name='test2')
        assert t2.to_dict(deep={'tbl1': {}}) == {'t2id': 1, 'name': 'test2', 'tbl1_t1id': None, 'tbl1': None}
        return

    def test_to_deep_o2m_empty(self):
        with self.session.begin():
            t1 = self.Table1(t1id=51, name='test2')
        assert t1.to_dict(deep={'tbl2s': {}}) == {'t1id': 51, 'name': 'test2', 'tbl2s': []}
    ...
    def test_to_deep_o2m(self):
        with self.session.begin():
            ...
            t1.tbl2s.append(t2)
        assert t1.to_dict(deep={'tbl2s': {}}) == {'t1id': 52, 'name': 'test3', 
    ...
    def test_to_deep_o2o(self):
        ...
        assert t1.to_dict(deep={'tbl3': {}}) == {'t1id': 53, 'name': 'test2', 
    ...
    def test_to_deep_nested(self):
        with self.session.begin():
            ...
            t2 = self.Table2(t2id=1, name='test2', tbl1=t1)
        assert t2.to_dict(deep={'tbl1': {'tbl3': {}}}) == {'t2id': 1, 'name': 'test2', 
           'tbl1_t1id': 1, 
           'tbl1': {'name': 'test1', 'tbl3': {'t3id': 1, 'name': 'test3'}}}
