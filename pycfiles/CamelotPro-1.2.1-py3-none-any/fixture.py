...
# Embedded file name: build/bdist.linux-x86_64/egg/camelot/model/fixture.py
...
class Fixture(Entity):
    ...
    def find_fixture_reference(cls, entity, fixture_key, fixture_class=None):
        ...
        return Session().query(cls).filter_by(model=unicode(entity_name), fixture_key=fixture_key, fixture_class=fixture_class).first()
    ...
    def find_fixture(cls, entity, fixture_key, fixture_class=None):
        ...
        reference = cls.find_fixture_reference(entity, fixture_key, fixture_class)
        if reference:
    ...
    def find_fixture_key(cls, entity, primary_key):
        ...
        """
        entity_name = unicode(entity.__name__)
        fixture = Session().query(cls).filter_by(model=entity_name, primary_key=primary_key).first()
        if fixture:
    ...
    def find_fixture_key_and_class(cls, obj):
        ...
        fixture = Session().query(cls).filter_by(model=entity_name, primary_key=obj.id).first()
        if fixture:
    ...
    def find_fixture_keys_and_classes(cls, entity):
        ...
        return dict((f.primary_key, (f.fixture_key, f.fixture_class)) for f in fixtures)

    @classmethod
    def insert_or_update_fixture(cls, entity, fixture_key, values, fixture_class=None):
...
class FixtureVersion(Entity):
    ...
    fixture_class = Column(Unicode(256), index=True, nullable=True, unique=True)
