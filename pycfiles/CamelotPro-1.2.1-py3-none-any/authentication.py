...
# Embedded file name: build/bdist.linux-x86_64/egg/camelot/model/authentication.py
# Compiled at: 2013-04-11 17:47:52
__doc__ = 'Set of classes to store authentication and permissions\n'
...
def get_current_authentication(_obj=None):
    ...
    if not hasattr(_current_authentication_, 'mechanism') or not _current_authentication_.mechanism or not orm.object_session(_current_authentication_.mechanism):
        import getpass
...
def update_last_login(initial_group_name=None, initial_group_roles=[]):
...
class AuthenticationMechanism(Entity):
    ...
    authentication_type = Column(camelot.types.Enumeration([(1, 'operating_system'),
     (2, 'database')]), nullable=False, index=True, default='operating_system')
...
class AuthenticationGroup(Entity):
    ...
    members = ManyToMany(AuthenticationMechanism, tablename='authentication_group_member', backref='groups')
...
class AuthenticationGroupRole(Entity):
    ...
    group_id = Column(Integer(), ForeignKey('authentication_group.id', onupdate='cascade', ondelete='cascade'), nullable=False, primary_key=True)


AuthenticationGroup.roles = orm.relationship(AuthenticationGroupRole, cascade='all, delete, delete-orphan')