...
# Embedded file name: build/bdist.linux-x86_64/egg/camelot/view/filters.py
...
class Filter(object):
    ...
    def get_filter_data(self, admin):
        ...
        options = [
         filter_option(name=_('All'), value=Filter.All, decorator=lambda q: q)]
        for value in session.execute(query):
            ...
            options.append(filter_option(name=option_name, value=value[0], decorator=create_decorator(col, attributes, value[0], joins)))

        return filter_data(name=filter_names[0], options=options, default=self.default)
...
class EditorFilter(Filter):
    ...
    def __init__(self, field_name, verbose_name=None, default_operator=None, default_value_1=None, default_value_2=None):
    ...
    def render(self, filter_data, parent):
        ...
        return FilterOperator(entity, field_name, field_attributes, self._default_operator, self._default_value_1, self._default_value_2, parent)

    def get_filter_data(self, admin):
        field_attributes = admin.get_field_attributes(self._field_name)
        name = self._verbose_name or field_attributes['name']
        return (
         name, (admin.entity, self._field_name, field_attributes))
...
class ValidDateFilter(Filter):
    ...
    def __init__(self, from_attribute='from_date', thru_attribute='thru_date', verbose_name=_('Valid at'), default=datetime.date.today):
        """
    ...
    def get_filter_data(self, admin):
        ...
        def query_decorator(query, date):
            e = admin.entity
            if date:
                return query.filter(and_(getattr(e, self._from_attribute) <= date, getattr(e, self._thru_attribute) >= date))
            return query

        option = filter_option(name=None, value=None, decorator=query_decorator)
        return filter_data(name=self._verbose_name, options=[
