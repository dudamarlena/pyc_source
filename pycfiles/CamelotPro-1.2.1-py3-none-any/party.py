...
# Embedded file name: build/bdist.linux-x86_64/egg/camelot/model/party.py
# Compiled at: 2013-04-11 17:47:52
__doc__ = "Set of classes to store persons, organizations, relationships and\ncontact mechanisms\n\nThese structures are modeled like described in 'The Data Model Resource Book'\nby Len Silverston, Chapter 2\n"
import datetime
...
from camelot.core.orm import Entity, using_options, Field, ManyToMany, ManyToOne, OneToMany, ColumnProperty
...
class Country(GeographicBoundary):
    ...
    geographicboundary_id = Field(Integer, ForeignKey('geographic_boundary.id'), primary_key=True)
...
class Address(Entity):
    ...
    city = ManyToOne(City, required=True, ondelete='cascade', onupdate='cascade', lazy='subquery')
    party_addresses = OneToMany('PartyAddress')

    def name(self):
        return sql.select([self.street1 + ', ' + GeographicBoundary.full_name], whereclause=GeographicBoundary.id == self.city_geographicboundary_id)
    ...
    def __unicode__(self):
        return '%s, %s' % (self.street1 or '', self.city or '')
...
class PartyContactMechanismAdmin(EntityAdmin):
    ...
    field_attributes = {'party_name': {'minimal_column_width': 25, 'editable': False}, 'mechanism': {'minimal_column_width': 25, 'editable': True, 
                     'nullable': False, 
                     'name': _('Mechanism'), 
...
class Party(Entity):
    ...
    contact_mechanisms = OneToMany('PartyContactMechanism', lazy='select', cascade='all, delete, delete-orphan')
    ...
    status = Status()
    categories = ManyToMany('PartyCategory', tablename='party_category_party', remote_colname='party_category_id', local_colname='party_id')
    row_type = schema.Column(Unicode(40), nullable=False)
    __mapper_args__ = {'polymorphic_on': row_type}
    ...
    def email(self):
        return self._get_contact_mechanism('email')

    @email.setter
    def email_setter(self, value):
        return self._set_contact_mechanism('email', value)
    ...
    @hybrid.hybrid_property
    def phone(self):
        return self._get_contact_mechanism('phone')

    @phone.setter
    def phone_setter(self, value):
        return self._set_contact_mechanism('phone', value)
    ...
    @hybrid.hybrid_property
    def fax(self):
        return self._get_contact_mechanism('fax')

    @fax.setter
    def fax_setter(self, value):
        return self._set_contact_mechanism('fax', value)
    ...
    @hybrid.hybrid_property
    def street1(self):
        return self._get_address_field('street1')

    @street1.setter
    def street1_setter(self, value):
        return self._set_address_field('street1', value)

    @hybrid.hybrid_property
    def street2(self):
        return self._get_address_field('street2')

    @street2.setter
    def street2_setter(self, value):
        return self._set_address_field('street2', value)

    @hybrid.hybrid_property
    def city(self):
        return self._get_address_field('city')

    @city.setter
    def city_setter(self, value):
        return self._set_address_field('city', value)
...
class EmployerEmployee(PartyRelationship):
    ...
    partyrelationship_id = Field(Integer, ForeignKey('party_relationship.id'), primary_key=True)
    ...
    def __unicode__(self):
        return '%s %s %s' % (unicode(self.established_to), _('Employed by'), unicode(self.established_from))
...
class DirectedDirector(PartyRelationship):
    ...
    class Admin(PartyRelationship.Admin):
        ...
        field_attributes = {'established_from': {'name': _('Organization')}, 'established_to': {'name': _('Director')}}
...
class SharedShareholder(PartyRelationship):
    ...
    class Admin(PartyRelationship.Admin):
        ...
        list_search = ['established_from.full_name', 'established_to.full_name']
        field_attributes = {'established_from': {'name': _('Organization')}, 'established_to': {'name': _('Shareholder')}}
...
class PartyAddress(Entity):
    ...
    party = ManyToOne(Party, required=True, ondelete='cascade', onupdate='cascade', lazy='subquery')
    address = ManyToOne(Address, required=True, ondelete='cascade', onupdate='cascade', lazy='subquery')
    from_date = Field(Date(), default=datetime.date.today, required=True, index=True)
    thru_date = Field(Date(), default=end_of_times, required=True, index=True)
    ...
    def party_name(self):
        return sql.select([sql.func.coalesce(Party.full_name, '')], whereclause=Party.id == self.party_id)
...
class AddressAdmin(PartyAddress.Admin):
    ...
    field_attributes = dict(street1=dict(name=_('Street'), editable=True, nullable=False), street2=dict(name=_('Street Extra'), editable=True), city=dict(name=_('City'), editable=True, nullable=False, delegate=delegates.Many2OneDelegate, target=City))
...
class ContactMechanism(Entity):
    ...
    def __unicode__(self):
        if self.mechanism:
            return '%s : %s' % (self.mechanism[0], self.mechanism[1])
...
class PartyContactMechanism(Entity):
    ...
    def party_name(self):
        return sql.select([Party.full_name], whereclause=Party.id == self.party_id)
...
class PartyCategory(Entity):
    ...
    parties = ManyToMany('Party', lazy=True, tablename='party_category_party', remote_colname='party_id', local_colname='party_category_id')
...
class PartyAdmin(EntityAdmin):
    ...
    list_display = ['name', 'email', 'contact_mechanisms_phone']
    list_search = ['full_name']
    list_filter = ['categories.name']
    form_display = ['addresses', 'contact_mechanisms', 'shares', 'directed_organizations']
    form_size = (700, 700)
    field_attributes = dict(addresses={'admin': AddressAdmin}, contact_mechanisms={'admin': PartyPartyContactMechanismAdmin}, suppliers={'admin': SupplierCustomer.SupplierAdmin}, customers={'admin': SupplierCustomer.CustomerAdmin}, employers={'admin': EmployerEmployee.EmployerAdmin}, employees={'admin': EmployerEmployee.EmployeeAdmin}, directed_organizations={'admin': DirectedDirector.DirectedAdmin}, directors={'admin': DirectedDirector.DirectorAdmin}, shares={'admin': SharedShareholder.SharedAdmin}, shareholders={'admin': SharedShareholder.ShareholderAdmin}, sex=dict(choices=[('M', _('male')), ('F', _('female'))]), name=dict(minimal_column_width=50), street1=dict(editable=True, minimal_column_width=50), street2=dict(editable=True, minimal_column_width=50), city=dict(editable=True, delegate=delegates.Many2OneDelegate, target=City), email=dict(editable=True, minimal_column_width=20, address_type='email', from_string=lambda s: (
     'email', s), delegate=delegates.VirtualAddressDelegate), phone=dict(editable=True, minimal_column_width=20, address_type='phone', from_string=lambda s: (
     'phone', s), delegate=delegates.VirtualAddressDelegate), fax=dict(editable=True, minimal_column_width=20, address_type='fax', from_string=lambda s: (
     'fax', s), delegate=delegates.VirtualAddressDelegate))
...
class PersonAdmin(Party.Admin):
    ...
    form_display = TabForm([
     ...
     (
      _('Official'),
      Form(['birthdate', 'social_security_number', 'passport_number',
