...
# Embedded file name: build/bdist.linux-x86_64/egg/camelot/view/database_selection.py
...
from PyQt4.QtGui import QBoxLayout, QDialog, QFont, QGridLayout, QHBoxLayout, QLabel, QLineEdit, QPushButton, QFileDialog, QComboBox, QWidget, QVBoxLayout
...
from camelot.core.dbprofiles import fetch_profiles, use_chosen_profile, store_profiles, get_network_proxy, last_used_profile, connection_string_from_profile
...
allow all languages
    ...
    def create_labels_and_widgets(self):
        ...
        self.not_accessible_media_path_label = QLabel(_('Media location path is not accessible.'))
        self.not_accessible_media_path_label.setStyleSheet('color: red')
        self.not_writable_media_path_label = QLabel(_('Media location path is not writable.'))
        ...
        self.language_editor = LanguageEditor(languages=self.languages, parent=self)
    ...
    def set_widgets_values(self):
        ...
        if self.dialects:
            ...
            dialects = [ name for _importer, name, is_package in pkgutil.iter_modules(sqlalchemy.dialects.__path__) if is_package ]
    ...
    def set_tab_order(self):
        all_widgets = [
         self.profile_editor, self.dialect_editor,
    ...
    def is_connection_valid(self):
        ...
        mt.post(lambda : self.test_connection(info), progress.finished, progress.exception)
        progress.exec_()
    ...
    def test_connection(self, profile):
        ...
        try:
            ...
            self._connection_valid = False
            raise UserException(_('Could not connect to database, please check host and port'), resolution=_('Verify driver, host and port or contact your system administrator'), detail=unicode(e))
    ...
    def new_network_request(self):
        ...
        if self.proxy_host_editor.text():
            proxy = QtNetwork.QNetworkProxy(QtNetwork.QNetworkProxy.HttpProxy, self.proxy_host_editor.text(), int(str(self.proxy_port_editor.text())))
    ...
    def fill_media_location(self):
        ...
        if not info.isReadable():
            self.main_widget().layout().addWidget(self.not_accessible_media_path_label, 13, 1, 1, 4)
            return
        if not info.isWritable():
            self.main_widget().layout().addWidget(self.not_writable_media_path_label, 13, 1, 1, 4)
            return
