...
from camelot.view.model_thread import object_thread, model_function, post
...
class DelayedProxy(object):
    ...
    def __call__(self):
        return CollectionProxy(self._admin, self._collection_getter, self._columns_getter)

    def __unicode__(self):
        collection = self._collection_getter()
        if collection:
            try:
                return (',').join(list(unicode(o) or '' for o, _i in zip(collection, range(3))))
        ...
        try:
            try:
                ...
                if field_attributes['python_type'] == list:
                    field_value = DelayedProxy(field_attributes['admin'], create_collection_getter(obj, col[0]), field_attributes['admin'].get_columns)
                else:
        ...
        try:
            try:
                ...
                if 'unicode_format' in static_attributes:
                    ...
                    unicode_data = ('.').join([ unicode(e) for e in field_data ])
                ...
                log_programming_error(logger, "Could not get view data for field '%s' with of object of type %s" % (static_attributes['name'],
                 obj.__class__.__name__), exc_info=e)
...
class CollectionProxy(QtGui.QProxyModel):
    ...
    * header_icon : the icon to be used in the vertical header
    ...
    def __init__(self, admin, collection_getter, columns_getter, max_number_of_rows=10, flush_changes=True, cache_collection_proxy=None):
        """
...
"""
        ...
        self.iconSize = QtCore.QSize(vertical_header_font_height, vertical_header_font_height)
        if self.header_icon:
    ...
    def setColumns(self, columns):
        ...
        for i, c in enumerate(columns):
            ...
            if settings_width:
                header_item.setData(QtCore.QVariant(QtCore.QSize(settings_width, self._horizontal_header_height)), Qt.SizeHintRole)
            else:
                header_item.setData(QtCore.QVariant(QtCore.QSize(max(minimal_widths), self._horizontal_header_height)), Qt.SizeHintRole)
    ...
    def setHeaderData(self, section, orientation, value, role):
        if not object_thread(self):
            raise AssertionError
            if orientation == Qt.Horizontal and role == Qt.SizeHintRole:
                width = value.width()
                self.settings.beginGroup('column_width')
                self.settings.beginGroup('0')
                self.settings.setValue(self._columns[section][1]['field_name'], width)
                self.settings.endGroup()
                self.settings.endGroup()
        ...
        if orientation == Qt.Vertical:
            if role == Qt.SizeHintRole:
                if self.header_icon != None:
                    return QtCore.QVariant(QtCore.QSize(self.iconSize.width() + 10, self._vertical_header_height))
                else:
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if not object_thread(self):
            raise AssertionError
            if not index.isValid() or not 0 <= index.row() <= self.rowCount(index) or not 0 <= index.column() <= self.columnCount():
                return QtCore.QVariant()
            ...
            for flushed, column, value in request_group:
                ...
                if value_changed:
                    try:
                        ...
                        self.logger.error("Can't set attribute %s to %s" % (attribute, unicode(new_value)), exc_info=e)
                    except TypeError:
                        pass
