...
# Embedded file name: build/bdist.linux-x86_64/egg/camelot/model/type_and_status.py
...
class StatusHistoryAdmin(EntityAdmin):
    ...
    def __unicode__(self):
        return unicode(self.classified_by or '')
...
class Status(Property):
    ...
    def attach(self, entity, name):
        ...
        status_history_admin = type(entity.__name__ + 'StatusHistoryAdmin', (
         StatusHistoryAdmin,), {'verbose_name': _(entity.__name__ + ' Status'), 'verbose_name_plural': _(entity.__name__ + ' Statuses')})
        if self.enumeration == None:
            status_type_admin = type(entity.__name__ + 'StatusType', (
             StatusTypeAdmin,), {'verbose_name': _(entity.__name__ + ' Status'), 'verbose_name_plural': _(entity.__name__ + ' Statuses')})
            status_type = type(entity.__name__ + 'StatusType', (
             StatusType, entity._descriptor.entity_base), {'__tablename__': status_type_name, 'Admin': status_type_admin})
            foreign_key = schema.ForeignKey(status_type.id, ondelete='cascade', onupdate='cascade')
            status_history = type(entity.__name__ + 'StatusHistory', (
             StatusHistory, entity._descriptor.entity_base), {'__tablename__': status_name, 'classified_by_id': schema.Column(types.Integer(), foreign_key, nullable=False), 
            ...
            status_history = type(entity.__name__ + 'StatusHistory', (
             StatusHistory, entity._descriptor.entity_base), {'__tablename__': status_name, 'classified_by': schema.Column(Enumeration(self.enumeration), nullable=False, index=True), 
               'Admin': status_history_admin})
    ...
    def create_non_pk_cols(self):
        ...
        for col in table.primary_key.columns:
            col_name = 'status_for_%s' % col.name
            if not hasattr(self.status_history, col_name):
                constraint = schema.ForeignKey(col, ondelete='cascade', onupdate='cascade')
...
class StatusMixin(object):
    ...
    def current_status_query(status_history, status_class):
        ...
        """
        return sql.select([status_history.classified_by], whereclause=sql.and_(status_history.status_for_id == status_class.id, status_history.status_from_date <= sql.functions.current_date(), status_history.status_thru_date >= sql.functions.current_date()), from_obj=[
         status_history.table]).order_by(status_history.id.desc()).limit(1)
    ...
    def change_status(self, new_status, status_from_date=None, status_thru_date=end_of_times()):
        ...
        old_status_filter = sql.and_(history_type.status_for == self, history_type.status_from_date <= status_from_date, history_type.status_thru_date >= status_from_date)
        ...
        new_status = history_type(status_for=self, classified_by=new_status, status_from_date=status_from_date, status_thru_date=status_thru_date, from_date=datetime.date.today(), thru_date=end_of_times())
        session.flush()
        return
