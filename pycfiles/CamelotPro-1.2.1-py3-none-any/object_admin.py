...
# Embedded file name: build/bdist.linux-x86_64/egg/camelot/admin/object_admin.py
# Compiled at: 2013-04-11 17:47:52
__doc__ = 'Admin class for Plain Old Python Object'
import logging
...
.. attribute:: name
    ...
    def get_verbose_name(self):
        return unicode(self.verbose_name or _(self.entity.__name__.capitalize()))

    def get_verbose_name_plural(self):
        return unicode(self.verbose_name_plural or self.get_verbose_name() + 's')
    ...
    def get_verbose_identifier(self, obj):
        ...
        """
        return '%s : %s' % (self.get_verbose_name(), unicode(obj))
    ...
    def get_related_toolbar_actions(self, toolbar_area, direction):
        ...
        """
        app_admin = self.get_application_admin()
        return self.related_toolbar_actions or app_admin.get_related_toolbar_actions(toolbar_area, direction)
                ...
                if callable(value):
                    ...
                    try:
                        try:
                            ...
                            logger.error('error in field_attribute function of %s' % name, exc_info=exc)
    ...
    def get_columns(self):
        ...
        return [ (field, self.get_field_attributes(field)) for field in table.get_fields()
               ]
    ...
    def get_validator(self, model=None, initial_validation=False):
        ...
        """
        return self.validator(self, model=model, initial_validation=initial_validation)
                ...
                if has_default:
                    ...
                    if isinstance(default, ColumnDefault):
                        ...
                        try:
                            ...
                            logger.error('Programming Error : could not set attribute %s to %s on %s' % (
                             field,
                             default_value,
                             object_instance.__class__.__name__), exc_info=exc)
    ...
    def create_object_form_view(self, title, object_getter, parent=None):
        ...
        model = self.model(self, create_collection_getter(object_getter, []), self.get_fields)
    ...
    def create_new_view(admin, related_collection_proxy=None, parent=None):
        ...
        model = NewObjectCollectionProxy(related_collection_proxy, admin, None, admin.get_fields, max_number_of_rows=1)
        ...
        class NewView(FormView):
            ...
            def __init__(self, parent):
                super(NewView, self).__init__(title=_('New'), admin=admin, model=model, index=0)
