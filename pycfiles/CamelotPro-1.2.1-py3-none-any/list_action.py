...
# Embedded file name: build/bdist.linux-x86_64/egg/camelot/admin/action/list_action.py
...
class CallMethod(Action):
    ...
    def model_run(self, model_context):
        from camelot.view.action_steps import UpdateProgress, FlushSession, UpdateObject
...
class OpenFormView(ListContextAction):
    ...
    def gui_run(self, gui_context):
        ...
        if isinstance(related_model, QueryTableProxy):
            model = QueryTableProxy(gui_context.admin, related_model.get_query_getter(), gui_context.admin.get_fields, max_number_of_rows=1, cache_collection_proxy=related_model)
        else:
            model = CollectionProxy(gui_context.admin, related_model.get_collection, gui_context.admin.get_fields, max_number_of_rows=1)
            row = related_model.map_to_source(row)
        formview = gui_context.admin.create_form_view(' ', model, row)
...
class OpenNewView(EditAction):
    ...
    def gui_run(self, gui_context):
        ...
        form = admin.create_new_view(related_collection_proxy=model, parent=None)
...
class DeleteSelection(EditAction):
    ...
    def model_run(self, model_context):
        ...
        if model_context.admin.get_delete_mode() == 'on_confirm':
            step = action_steps.MessageBox(_('Please confirm'), admin.get_delete_message(None), QtGui.QMessageBox.Yes, QtGui.QMessageBox.No)
            response = yield step
        ...
        for i, obj in enumerate(objects_to_remove):
            yield action_steps.UpdateProgress(i, model_context.selection_count, _('Removing'))
            try:
...
class ExportSpreadsheet(ListContextAction):
    ...
    def model_run(self, model_context):
        ...
        from camelot.view.import_utils import RowData, ColumnMapping, ColumnSelectionAdmin
        ...
        field_choices = [ (f, entity_fa['name']) for f, entity_fa in all_fields.items()
                        ]
        row_data = RowData(1, [None] * len(all_fields))
        mapping = ColumnMapping(len(all_fields), [
         row_data], admin, [ field for field, _fa in admin.get_columns() ])
        mapping_admin = ColumnSelectionAdmin(len(all_fields), admin, field_choices)
        ...
        def get_style(font_specs=dict(), border_specs=dict(), pattern=None, num_format_str=None):
            style_key = (
             freeze(font_specs),
             freeze(border_specs),
             pattern,
        ...
        title_style = get_style(dict(font_name=self.font_name, bold=True, height=240))
        ...
        for i, (name, field_attributes) in enumerate(columns):
            ...
            border_specs = dict(top=1)
            name = unicode(name)
            if i == 0:
                border_specs['left'] = 1
            elif i == len(columns) - 1:
                border_specs['right'] = 1
        ...
        for j, obj in enumerate(model_context.get_collection(yield_per=100)):
            dynamic_attributes = admin.get_dynamic_field_attributes(obj, field_names)
            row = offset + j
            if j % 100 == 0:
            ...
            for i, ((_name, attributes), delta_attributes) in enumerate(zip(columns, dynamic_attributes)):
                ...
                if row - offset + 1 == model_context.collection_count:
                    border_specs['bottom'] = 1
                style = get_style(font_specs, border_specs, None, format_string)
...
class PrintPreview(ListContextAction):
    ...
    def model_run(self, model_context):
        ...
        context = {'title': model_context.admin.get_verbose_name_plural(), 'table': table, 
           'columns': [ field_attributes['name'] for _field, field_attributes in columns ]}
        yield action_steps.PrintJinjaTemplate(template='list.html', context=context)
...
class ImportFromFile(EditAction):
    ...
    def model_run(self, model_context):
        import os.path, chardet
        from camelot.view import action_steps
        from camelot.view.import_utils import UnicodeReader, RowData, RowDataAdmin, XlsReader, ColumnMapping, ColumnMappingAdmin
        ...
        default_fields = [ field for field, fa in admin.get_columns() if fa.get('editable', True)
                         ]
        column_mapping = ColumnMapping(columns, collection, admin, default_fields)
        field_choices = [ (f, entity_fa['name']) for f, entity_fa in admin.get_all_fields_and_attributes().items() if entity_fa.get('editable', True)
                        ]
        column_mapping_admin = ColumnMappingAdmin(columns, admin, field_choices)
        ...
        yield action_steps.MessageBox(icon=QtGui.QMessageBox.Warning, title=_('Proceed with import'), text=_('Importing data cannot be undone,\nare you sure you want to continue'))
        with model_context.session.begin():
            for i, row in enumerate(collection):
                ...
                for field_name, attributes in row_data_admin.get_columns():
                    ...
                    setattr(new_entity_instance, attributes['original_field'], from_string(getattr(row, field_name)))
...
class AddNewObject(OpenNewView):
    ...
    def gui_run(self, gui_context):
        create_inline = gui_context.field_attributes.get('create_inline', False)
