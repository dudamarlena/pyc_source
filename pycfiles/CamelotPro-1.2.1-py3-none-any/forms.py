...
# Embedded file name: build/bdist.linux-x86_64/egg/camelot/view/forms.py
# Compiled at: 2013-04-11 17:47:52
__doc__ = 'Classes to layout fields on a form.  These are mostly used for specifying the\nform_display attribute in Admin classes, but they can be used on their own as\nwell.  Form classes can be used recursive.\n'
import logging
...
"""
    ...
    def __unicode__(self):
        return 'Form(%s)' % (',').join(unicode(c) for c in self)
    ...
    def render(self, widgets, parent=None, toplevel=False):
        ...
        if toplevel or has_vertical_expanding_row:
            form_widget.setSizePolicy(QtGui.QSizePolicy.Expanding, QtGui.QSizePolicy.Expanding)
...
.. image:: /_static/form/tab_form.png
    ...
    def __init__(self, tabs, position=NORTH):
        ...
        super(TabForm, self).__init__(sum((tab_form.get_fields() for tab_label, tab_form in self.tabs), []))
...
class HBoxForm(Form):
  ...
  """
    ...
    def __init__(self, columns, scrollbars=False):
        ...
        super(HBoxForm, self).__init__(sum((column_form.get_fields() for column_form in self.columns), []), scrollbars=scrollbars)
