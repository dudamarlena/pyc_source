...
# Embedded file name: build/bdist.linux-x86_64/egg/camelot/view/import_utils.py
# Compiled at: 2013-04-11 17:47:52
__doc__ = 'Utility classes to import files into Camelot'
from PyQt4 import QtCore
import csv, codecs, itertools, logging, string, re
...
class ColumnMapping(object):
    ...
    def __init__(self, columns, rows, admin, default_fields):
        ...
        self.show_row = 0
        for i, field in itertools.izip(range(self.columns), default_fields):
    ...
    def match_names(self):
        ...
        field_choices = [ (f, entity_fa['name']) for f, entity_fa in self.admin.get_all_fields_and_attributes().items() if entity_fa.get('editable', True)
                        ]
        matches = dict((unicode(verbose_name).lower(), fn) for fn, verbose_name in field_choices)
        matches.update(dict((fn.lower().replace('_', ''), fn) for fn, _verbose_name in field_choices))
        for i in range(self.columns):
...
class ColumnMappingAdmin(ObjectAdmin):
    ...
    field_attributes = {'show_row': {'editable': True, 'calculator': False, 
                    'delegate': delegates.IntegerDelegate}}
    form_actions = [
     CallMethod(_('Show next'), ColumnMapping.show_next),
    ...
    def get_field_attributes(self, field_name):
        ...
        if match != None:
            ...
            fa.update({'delegate': delegates.ComboBoxDelegate, 'editable': True, 
               'name': self.verbose_name_for_column(column), 
               'choices': [
                         (None, '')] + self.field_choices})
        return fa

    def get_form_display(self):
        columns = self.columns
        rows = [ ['column_%i_value' % i, 'column_%i_field' % i] for i in range(columns)
               ]
...
class ColumnSelectionAdmin(ColumnMappingAdmin):
    ...
    def get_form_display(self):
        columns = self.columns
        return forms.Form([ 'column_%i_field' % i for i in range(columns) ], scrollbars=True)
...
class UnicodeReader(object):
    ...
    def next(self):
        ...
        try:
            ...
            raise UserException(text=ugettext('This file contains unexpected characters'), resolution=ugettext('Recreate the file with %s encoding') % self.encoding, detail=ugettext('Exception occured at line %s : ') % self.line + unicode(exception))
...
class XlsReader(object):
    ...
    def __init__(self, filename):
        ...
        workbook = xlrd.open_workbook(filename, formatting_info=formatting_info)
            ...
            for column in range(self.sheet.ncols):
                ...
                if ctype in (xlrd.XL_CELL_EMPTY,
                 ...
                 xlrd.XL_CELL_BLANK):
                    ...
                    value = unicode(self.locale.toString(cell.value, format='f', precision=precision))
                elif ctype == xlrd.XL_CELL_DATE:
                    date_tuple = xlrd.xldate_as_tuple(cell.value, self.datemode)
    ...
    def get_columns(self):
        if self._columns:
            ...
            for i in range(self.column_mapping.columns):
                ...
                original_field = getattr(self.column_mapping, field_name + '_field')
