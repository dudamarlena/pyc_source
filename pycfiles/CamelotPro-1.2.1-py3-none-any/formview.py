...
# Embedded file name: build/bdist.linux-x86_64/egg/camelot/view/controls/formview.py
...
class FormEditors(object):
    ...
    def create_editor(self, field_name, parent):
        ...
        widget_editor = delegate.createEditor(parent, self.option, model_index)
...
class FormWidget(QtGui.QWidget):
    ...
    def set_model(self, model):
        ...
        def get_columns_and_form():
            return (
             self._model.getColumns(), self._admin.get_form_display())
...
class FormView(AbstractView):
    ...
    def __init__(self, title, admin, model, index, parent=None):
        ...
        post(functools.update_wrapper(functools.partial(get_toolbar_actions, Qt.TopToolBarArea), get_toolbar_actions), self.set_toolbar_actions)
        return
