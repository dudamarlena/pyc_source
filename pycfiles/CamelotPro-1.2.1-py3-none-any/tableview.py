...
# Embedded file name: build/bdist.linux-x86_64/egg/camelot/view/controls/tableview.py
    ...
    def _current_index_changed(self, current_index):
        ...
        for tab_index, (first_column, last_column) in self.groups.items():
            for column_index in range(first_column, last_column):
                self.table_widget.setColumnHidden(column_index, tab_index != current_index)
...
"""
    ...
    def __init__(self, lines_per_row=1, parent=None):
        ...
        self.setEditTriggers(QtGui.QAbstractItemView.SelectedClicked | QtGui.QAbstractItemView.DoubleClicked | QtGui.QAbstractItemView.CurrentChanged)
        ...
        self.horizontalHeader().sectionClicked.connect(self.horizontal_section_clicked)
    ...
    def timerEvent(self, event):
        ...
        for logical_index, new_width in self._columns_changed.items():
            if self.horizontalHeader().isSectionHidden(logical_index):
                continue
            old_size = variant_to_pyobject(self.model().headerData(logical_index, Qt.Horizontal, Qt.SizeHintRole))
            if old_size.width() != new_width:
                new_size = QtCore.QSize(new_width, old_size.height())
                self.model().setHeaderData(logical_index, Qt.Horizontal, new_size, Qt.SizeHintRole)
    ...
    def horizontal_section_clicked(self, logical_index):
        ...
        if not object_thread(self):
            ...
            header.isSortIndicatorShown() or header.setSortIndicatorShown(True)
    ...
    def close_editor(self):
        ...
        if not object_thread(self):
            ...
            return current_index.isValid() or None
    ...
    def activated(self, selectedIndex, previousSelectedIndex):
        ...
        new_size = self.itemDelegate(selectedIndex).sizeHint(option, selectedIndex)
        ...
        self.setRowHeight(row, max(new_size.height(), self._minimal_row_height))
...
class AdminTableWidget(QtGui.QWidget):
    ...
    def __init__(self, admin, parent=None):
        ...
        assert object_thread(self)
        self._admin = admin
        table_widget = TableWidget(parent=self, lines_per_row=admin.lines_per_row)
    ...
    def _fill_expanded_search_options(self, columns):
        ...
        for i, (field, attributes) in enumerate(columns):
            if 'operators' in attributes and attributes['operators']:
                ...
                widget = FilterOperator(self._admin.entity, field, attributes, box)
    ...
    def expand_search_options(self):
        if not object_thread(self):
            ...
            if self._expanded_search.isHidden():
                self._expanded_filters_created or post(self._admin.get_expanded_search_fields, self._fill_expanded_search_options)
...
class TableView(AbstractView):
  ...
  """
    ...
    def create_table_model(self, admin):
        ...
        return self.table_model(admin, None, admin.get_columns)
    ...
    def set_admin(self, admin):
        ...
        def get_filters_and_actions():
            return (
             admin.get_filters(), admin.get_list_actions())
