...
# Embedded file name: build/bdist.linux-x86_64/egg/camelot/view/controls/editors/many2oneeditor.py
...
class Many2OneEditor(CustomEditor):
    ...
    def __init__(self, admin=None, parent=None, editable=True, field_name='manytoone', **kwargs):
    ...
    def textEdited(self, text):
        ...
        post(create_search_completion(unicode(text)), self.display_search_completions)
    ...
    def search_completions(self, text):
        ...
        if search_decorator:
            sresult = [ (unicode(e), create_constant_function(e)) for e in search_decorator(self.admin.entity.query).limit(20)
                      ]
            return (
             text, sresult)
        return (text, [])
    ...
    def show_form_view(self, admin_and_title):
        ...
        model = CollectionProxy(admin, create_collection_getter(self.entity_instance_getter), admin.get_fields)
    ...
    def setEntity(self, entity_instance_getter, propagate=True):
        ...
        post(update_wrapper(partial(get_instance_representation, entity_instance_getter, propagate), get_instance_representation), self.set_instance_representation)
