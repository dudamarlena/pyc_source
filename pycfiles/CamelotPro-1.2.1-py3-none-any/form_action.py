...
# Embedded file name: build/bdist.linux-x86_64/egg/camelot/admin/action/form_action.py
...
class ShowHistory(Action):
    ...
    def model_run(self, model_context):
        ...
        class ChangeAdmin(ObjectAdmin):
            ...
            field_attributes = {'at': {'delegate': delegates.DateTimeDelegate}, 'memento_type': {'delegate': delegates.ComboBoxDelegate, 'choices': memento.memento_types, 
                                'name': _('Type')}}
        ...
        if obj != None:
            ...
            if None not in primary_key:
                changes = list(memento.get_changes(model=unicode(model_context.admin.entity.__name__), primary_key=primary_key, current_attributes={}))
...
class CloseForm(Action):
    ...
    def model_run(self, model_context):
        ...
        if valid:
            ...
            message = action_steps.MessageBox(('\n').join(messages), QtGui.QMessageBox.Warning, _('Invalid form'), QtGui.QMessageBox.Ok | QtGui.QMessageBox.Discard)
            reply = yield message
