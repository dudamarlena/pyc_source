...
# Embedded file name: build/bdist.linux-x86_64/egg/camelot/model/batch_job.py
# Compiled at: 2013-04-11 17:47:52
__doc__ = 'Most applications need to perform some scheduled jobs to process information.\nUsers need to be able to monitor the functioning of those scheduled jobs.\n\nThese classes provide the means to store the result of batch jobs to enable the \nuser to review or plan them.\n'
import logging, sys, sqlalchemy.types
...
class BatchJob(Entity, type_and_status.StatusMixin):
    ...
    def add_exception_to_message(self, exc_type=None, exc_val=None, exc_tb=None):
        ...
        traceback.print_exception(exc_type or sys.exc_type, exc_val or sys.exc_value, exc_tb or sys.exc_traceback, None, sio)
        traceback_print = sio.getvalue()
        sio.close()
        self.add_strings_to_message([unicode(exc_type or sys.exc_type)], color='red')
    ...
    def add_strings_to_message(self, strings, color=None):
        ...
        if color:
            strings = [
             '<font color="%s">' % color] + strings + ['</font>']
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if exc_type != None:
            ...
            LOGGER.info('batch job closed with exception', exc_info=(
    ...
    class Admin(EntityAdmin):
        ...
        form_display = forms.TabForm([(_('Job'), list_display + ['message']),
         (
          _('History'), ['status'])])
        form_actions = [
         type_and_status.ChangeStatus('canceled', _('Cancel'))]