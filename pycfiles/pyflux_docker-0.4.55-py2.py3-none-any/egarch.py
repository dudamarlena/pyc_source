...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/garch/egarch.py
    ...
    **** BETA-t-EGARCH MODELS ****
    ...
    def _create_latent_variables(self):
        ...
        for p_term in range(self.p):
            ...
            if p_term == 0:
                self.latent_variables.z_list[(-1)].start = 3.2
            else:
    ...
    def _sim_prediction_bayes(self, h, simulations):
        ...
        for n in range(0, simulations):
            ...
            for t in range(0, h):
                ...
                if self.leverage is True:
                    new_value += t_z[(1 + self.p + self.q)] * np.sign(-(Y_exp[(-1)] - t_z[(-1)])) * (scores_exp[(-1)] + 1)
                lmda_exp = np.append(lmda_exp, [new_value])
                scores_exp = np.append(scores_exp, scores[np.random.randint(scores.shape[0])])
                Y_exp = np.append(Y_exp, Y[np.random.randint(Y.shape[0])])
    ...
    def mb_neg_loglik(self, beta, mini_batch):
        ...
        The negative logliklihood of the model
        """
        lmda, Y, ___ = self._mb_model(beta, mini_batch)
        return -np.sum(ss.t.logpdf(x=Y, df=self.latent_variables.z_list[(-2)].prior.transform(beta[(-2)]), loc=np.ones(lmda.shape[0]) * self.latent_variables.z_list[(-1)].prior.transform(beta[(-1)]), scale=np.exp(lmda / 2.0)))
