...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/gas/gasreg.py
...
class GASReg(tsm.TSM):
    ...
    **** GENERALIZED AUTOREGRESSIVE SCORE (GAS) REGRESSION MODELS ****
    ...
    def _cythonized_model(self, beta):
        ...
        return (
         np.array(theta[:-1]), self.model_Y, self.model_scores, coefficients)
    ...
    def _cythonized_mb_model(self, beta, mini_batch):
        ...
        return (
         np.array(theta[:-1]), Y, self.model_scores, coefficients)
    ...
    def _uncythonized_model(self, beta):
        ...
        return (
         theta[:-1], self.model_Y, self.model_scores, coefficients)
    ...
    def _uncythonized_model(self, beta, mini_batch):
        ...
        theta = np.zeros(Y.shape[0] + 1)
        model_scale, model_shape, model_skewness = self._get_scale_and_shape(parm)
        theta, self.model_scores, coefficients = gas_reg_recursion(parm, theta, X, coefficients, self.model_scores, Y, Y.shape[0], self.family.reg_score_function, self.link, model_scale, model_shape, model_skewness, self.max_lag)
        return (
         theta[:-1], Y, self.model_scores, coefficients)
        ...
        if self.latent_variables.estimated is False:
            ...
            if intervals == True:
                ...
                for count in range(9):
                    plt.fill_between(date_index[-h - 1:], error_bars[count], error_bars[(-count)], alpha=alpha[count])
