...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/ssm/nllt.py
...
class NLLT(tsm.TSM):
    ...
    **** NON-GAUSSIAN LOCAL LINEAR TREND MODEL ****
    ...
    def _animate_bbvi(self, stored_parameters, stored_predictive_likelihood):
        ...
        anim = FuncAnimation(fig, ud, frames=np.arange(stored_parameters.shape[0]), init_func=ud.init, interval=10, blit=True)
        plt.plot(self.data)
        plt.xlabel('Time')
...
class BBVINLLTAnimate(object):
    ...
    def init(self):
        self.line.set_data(range(int((self.means[0].shape[0] - self.start_index) / 2)), self.link(self.means[0][self.start_index:-((self.means[0].shape[0] - self.start_index) / 2)]))
        return (self.line,)

    def __call__(self, i):
        if i == 0:
            return self.init()
        self.line.set_data(range(int((self.means[0].shape[0] - self.start_index) / 2)), self.link(self.means[i][self.start_index:-((self.means[0].shape[0] - self.start_index) / 2)]))
        return (self.line,)