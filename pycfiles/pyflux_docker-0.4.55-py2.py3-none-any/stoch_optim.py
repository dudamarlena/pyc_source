...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/inference/stoch_optim.py
...
class RMSProp(object):
    ...
    *** RMSProp *** 
    ...
    def update(self, gradient):
        ...
        if self.t > 5:
            self.parameters += (self.learning_rate + self.learning_rate * 15.0 * 0.99 ** self.t) * (gradient / np.sqrt(self.variance + self.epsilon))
        self.t += 1
...
class ADAM(object):
    ...
    *** Adaptive Moment Estimation (ADAM) *** 
    ...
    def update(self, gradient):
        ...
        if self.t > 5:
            self.parameters += (self.learning_rate + self.learning_rate * 15.0 * 0.99 ** self.t) * (f_gradient_hat / (np.sqrt(variance_hat) + self.epsilon))
        self.t += 1
        return self.parameters