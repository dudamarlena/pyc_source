...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/ensembles/mixture_of_experts.py
...
class Aggregate:
    ...
    def add_model(self, model):
        ...
        if not self.model_list:
            ...
            if model.model_type in ('EGARCH', 'EGARCHM', 'EGARCHMReg', 'GARCH', 'LMEGARCH',
                                    'LMSEGARCH', 'SEGARCH', 'SEGARCHM'):
                self.data = np.abs(model.data)
            else:
                self.data = model.data
            self.index = model.index
        elif model.model_type in ('EGARCH', 'EGARCHM', 'EGARCHMReg', 'GARCH', 'LMEGARCH',
                                  'LMSEGARCH', 'SEGARCH', 'SEGARCHM'):
        ...
        elif np.isclose(np.abs(model.data[-self.match_window:] - self.data[-self.match_window:]).sum(), 0.0) or model.model_type == 'GPNARX':
            self.model_list.append(model)
        else:
            raise ValueError('Data entered is deemed different based on %s last values!' % s)
    ...
    def _model_predict(self, h, recalculate=False, fit_once=True):
        ...
        if len(self.model_predictions) == 0 or h != self.h or recalculate is True:
            ...
            self.h = h
            return (
             result, self.predict_index)
        else:
            return (
             self.model_predictions, self.predict_index)
    ...
    def run(self, h, recalculate=False):
        ...
        return (
         normalized_weights, self._construct_losses(data, predictions, ensemble_prediction), ensemble_prediction)
