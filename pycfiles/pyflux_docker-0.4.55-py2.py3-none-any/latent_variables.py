...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/latent_variables.py
...
class LatentVariables(object):
    ...
    def __str__(self):
        ...
        for z in range(len(self.z_list)):
            z_row.append({'z_index': z, 'z_name': z_names[z], 'z_prior': prior_names[z], 
               'z_hyper': prior_z_names[z], 'z_vardist': vardist_names[z], 
    ...
    def adjust_prior(self, index, prior):
        ...
        elif index < 0 or index > len(self.z_list) - 1 or not isinstance(index, int):
    ...
    def get_z_priors_names(self):
        ...
        for prior in priors:
            if isinstance(prior, Normal):
                ...
                prior_z_names.append('loc0: ' + str(np.round(prior.loc0, 4)) + ', scale0: ' + str(np.round(prior.scale0, 4)) + ', df0: ' + str(np.round(prior.df0, 4)))
            elif isinstance(prior, Skewt):
                prior_names.append('Skewt')
                prior_z_names.append('loc0: ' + str(np.round(prior.loc0, 4)) + ', scale0: ' + str(np.round(prior.scale0, 4)) + ', df0: ' + str(np.round(prior.df0, 4)) + ', gamma0: ' + str(np.round(prior.gamma0, 4)))
    ...
    def plot_z(self, indices=None, figsize=(15, 5), loc=1):
        import matplotlib.pyplot as plt, matplotlib.mlab as mlab, seaborn as sns
        plt.figure(figsize=figsize)
        for z in range(1, len(self.z_list) + 1):
            if indices is not None and z - 1 not in indices:
                continue
            elif hasattr(self.z_list[(z - 1)], 'sample'):
