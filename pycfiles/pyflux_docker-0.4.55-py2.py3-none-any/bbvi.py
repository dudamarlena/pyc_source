...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/inference/bbvi.py
...
class BBVI(object):
    ...
    def __init__(self, neg_posterior, q, sims, optimizer='RMSProp', iterations=1000, learning_rate=0.001, record_elbo=False, quiet_progress=False):
    ...
    def get_means_and_scales(self):
        ...
        """
        return (
         self.optim.parameters[::2], np.exp(self.optim.parameters[1::2]))
    ...
    def run(self):
        ...
        return (
         self.q, final_means, final_ses, elbo_records)
    ...
    def run_and_store(self):
        ...
        if not self.quiet_progress:
            ...
            print 'Final model ELBO is ' + str(-self.neg_posterior(final_means) - self.create_normal_logq(final_means))
        return (
         self.q, final_means, final_ses, stored_means, stored_predictive_likelihood, elbo_records)


class CBBVI(BBVI):

    def __init__(self, neg_posterior, log_p_blanket, q, sims, optimizer='RMSProp', iterations=300000, learning_rate=0.001, record_elbo=False, quiet_progress=False):
...
class BBVIM(BBVI):
    ...
    def __init__(self, neg_posterior, full_neg_posterior, q, sims, optimizer='RMSProp', iterations=1000, learning_rate=0.001, mini_batch=2, record_elbo=False, quiet_progress=False):
        self.neg_posterior = neg_posterior
