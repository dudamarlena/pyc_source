...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/results.py
...
class MLEResults(Results):
    ...
    def __init__(self, data_name, X_names, model_name, model_type, latent_variables, results, data, index, multivariate_model, objective_object, method, z_hide, max_lag, ihessian=None, signal=None, scores=None, states=None, states_var=None):
    ...
    def summary_with_hessian(self, transformed=True):
        ...
        for i in range(len(self.z.z_list) - int(self.z_hide)):
            if self.z.z_list[i].prior.transform == np.array:
                data.append({'z_name': self.z.z_list[i].name, 
            ...
            elif transformed is True:
                data.append({'z_name': self.z.z_list[i].name, 
                   'z_value': np.round(self.z.z_list[i].prior.transform(self.z_values[i]), self.rounding_points)})
            else:
                data.append({'z_name': self.z.z_list[i].prior.itransform_name + '(' + self.z.z_list[i].name + ')', 
        ...
        model_details.append({'model_details': 'Dependent Variable: ' + str(self.data_name), 'model_results': 'Method: ' + str(self.method)})
        model_details.append({'model_details': 'Start Date: ' + str(self.index[self.max_lag]), 'model_results': obj_desc})
        model_details.append({'model_details': 'End Date: ' + str(self.index[(-1)]), 'model_results': 'AIC: ' + str(np.round(2 * len(self.z_values) + 2 * self.objective_object(self.z_values), 4))})
        model_details.append({'model_details': 'Number of observations: ' + str(self.data_length), 'model_results': 'BIC: ' + str(np.round(2 * self.objective_object(self.z_values) + len(self.z_values) * np.log(self.data_length), 4))})
    ...
    def summary_without_hessian(self):
        ...
        model_details.append({'model_details': 'Dependent Variable: ' + self.data_name, 'model_results': 'Method: ' + str(self.method)})
        model_details.append({'model_details': 'Start Date: ' + str(self.index[self.max_lag]), 'model_results': obj_desc})
        model_details.append({'model_details': 'End Date: ' + str(self.index[(-1)]), 'model_results': 'AIC: ' + str(self.aic)})
        model_details.append({'model_details': 'Number of observations: ' + str(self.data_length), 'model_results': 'BIC: ' + str(self.bic)})
        print TablePrinter(model_fmt, ul='=')(model_details)
        print '=' * 106
        print TablePrinter(fmt, ul='=')(data)
        print '=' * 106
...
class BBVIResults(Results):
    ...
    def __init__(self, data_name, X_names, model_name, model_type, latent_variables, data, index, multivariate_model, objective_object, method, z_hide, max_lag, ses, signal=None, scores=None, elbo_records=None, states=None, states_var=None):
        self.data_name = data_name
        self.X_names = X_names
        self.max_lag = max_lag
        self.model_name = model_name
...
class BBVISSResults(Results):
    ...
    def __init__(self, data_name, X_names, model_name, model_type, latent_variables, data, index, multivariate_model, objective, method, z_hide, max_lag, ses, signal=None, scores=None, states=None, states_var=None, elbo_records=None):
        self.data_name = data_name
        self.X_names = X_names
        self.max_lag = max_lag
        self.model_name = model_name
...
class LaplaceResults(Results):
    ...
    def __init__(self, data_name, X_names, model_name, model_type, latent_variables, data, index, multivariate_model, objective_object, method, z_hide, max_lag, ihessian, signal=None, scores=None, states=None, states_var=None):
        self.data_name = data_name
        self.X_names = X_names
        self.max_lag = max_lag
        self.model_name = model_name
...
class MCMCResults(Results):
    ...
    def __init__(self, data_name, X_names, model_name, model_type, latent_variables, data, index, multivariate_model, objective_object, method, z_hide, max_lag, samples, mean_est, median_est, lower_95_est, upper_95_est, signal=None, scores=None, states=None, states_var=None):
        self.data_name = data_name
        self.X_names = X_names
        self.max_lag = max_lag
        self.model_name = model_name
