...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/garch/segarch.py
...
class SEGARCH(tsm.TSM):
    ...
    **** skew BETA-t-EGARCH MODELS ****
    ...
    def neg_loglik(self, beta):
        ...
        return -np.sum(logpdf(Y, self.latent_variables.z_list[(-2)].prior.transform(beta[(-2)]), loc=theta, scale=np.exp(lmda / 2.0), skewness=self.latent_variables.z_list[(-3)].prior.transform(beta[(-3)])))
    ...
    def sample(self, nsims=1000):
        ...
        if self.latent_variables.estimation_method not in ('BBVI', 'M-H'):
            ...
            data_draws = np.array([ fam.Skewt.draw_variable(loc=self.latent_variables.z_list[(-1)].prior.transform(lv_draws[(-1, i)]), shape=self.latent_variables.z_list[(-2)].prior.transform(lv_draws[(-2, i)]), skewness=self.latent_variables.z_list[(-3)].prior.transform(lv_draws[(-3, i)]), nsims=len(sigmas[i]), scale=np.exp(sigmas[i] / 2.0)) for i in range(nsims)
                                  ])
            return data_draws
