...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/ssm/ndynlin.py
...
class NDynReg(tsm.TSM):
    ...
    **** NON-GAUSSIAN DYNAMIC REGRESSION MODELS ****
    ...
    def neg_loglik(self, beta):
        ...
        return self.state_likelihood(beta, states) + self.family.neg_loglikelihood(self.data, self.link(np.sum(self.X * states.T, axis=1)), scale, shape, skewness)
    ...
    def likelihood_markov_blanket(self, beta):
        ...
        parm = np.array([ self.latent_variables.z_list[k].prior.transform(beta[k]) for k in range(self.z_no) ])
        scale, shape, skewness = self._get_scale_and_shape(parm)
        return self.family.markov_blanket(self.data, self.link(np.sum(self.X * states.T, axis=1)), scale, shape, skewness)
    ...
    def fit(self, optimizer='RMSProp', iterations=1000, print_progress=True, start_diffuse=False, **kwargs):
        ...
        BBVI fit object
        """
        return self._bbvi_fit(optimizer=optimizer, print_progress=print_progress, start_diffuse=start_diffuse, iterations=iterations, **kwargs)
    ...
    def initialize_approx_dist(self, phi, start_diffuse, gaussian_latents):
        ...
        H, mu = self.family.approximating_model_reg(phi, T, Z, R, Q, gaussian_latents.get_z_values(transformed=True)[0], self.data, self.X, self.state_no)
    ...
    def _bbvi_fit(self, optimizer='RMSProp', iterations=1000, print_progress=True, start_diffuse=False, **kwargs):
        ...
        gaussian_latents = self._preoptimize_model()
