...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/garch/egarchmreg.py
    ...
    **** BETA-t-EGARCH IN MEAN REGRESSION MODELS ****
    ...
    def _mean_prediction(self, lmda, Y, scores, h, t_params, X_oos):
        ...
        for t in range(0, h):
            ...
            lmda_exp = np.append(lmda_exp, [new_lambda_value])
    ...
    def sample(self, nsims=1000):
        ...
        if self.latent_variables.estimation_method not in ('BBVI', 'M-H'):
            ...
            data_draws = np.array([ ss.t.rvs(loc=self.latent_variables.z_list[(-len(self.X_names) * 2 - 1)].prior.transform(lv_draws[(-len(self.X_names) * 2 - 1, i)]), df=self.latent_variables.z_list[(-len(self.X_names) * 2 - 2)].prior.transform(lv_draws[(-len(self.X_names) * 2 - 2, i)]), scale=np.exp(sigmas[i] / 2.0)) for i in range(nsims)
                                  ])
            return data_draws
