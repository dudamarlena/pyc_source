...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/arma/tests/test_arimax_poisson.py
# Compiled at: 2018-02-01 11:59:15
import numpy as np, pandas as pd
from pyflux.arma import ARIMAX
from pyflux.families import Poisson
countdata = np.random.poisson(3, 150)
x1 = np.random.normal(0, 1, 150) + countdata * 0.1
...
def test_no_terms():
    ...
    """
    model = ARIMAX(formula='y ~ x1', data=data, ar=0, ma=0, family=Poisson())
    x = model.fit()
...
def test_couple_terms_integ():
    ...
    values are not nan
    """
    model = ARIMAX(formula='y ~ x1', data=data, ar=1, ma=1, integ=1, family=Poisson())
    x = model.fit()
    assert len(model.latent_variables.z_list) == 4
    lvs = np.array([ i.value for i in model.latent_variables.z_list ])
    assert len(lvs[np.isnan(lvs)]) == 0
...
def test_2_no_terms():
    ...
    are not nan
    """
    model = ARIMAX(formula='y ~ x1 + x2', data=data, ar=0, ma=0, family=Poisson())
    x = model.fit()
...
def test_2_couple_terms():
    ...
    are not nan
    """
    model = ARIMAX(formula='y ~ x1 + x2', data=data, ar=1, ma=1, family=Poisson())
    x = model.fit()
...
def test_2_predict_length():
    ...
    Tests that the length of the predict dataframe is equal to no of steps h
    """
    model = ARIMAX(formula='y ~ x1 + x2', data=data, ar=2, ma=2, family=Poisson())
    x = model.fit()
    x.summary()
    assert model.predict(h=5, oos_data=data_oos).shape[0] == 5
