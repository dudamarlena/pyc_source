...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/garch/egarchm.py
...
class EGARCHM(tsm.TSM):
    ...
    **** BETA-t-EGARCH IN MEAN MODELS ****
    ...
    def neg_loglik(self, beta):
        ...
        return -np.sum(ss.t.logpdf(x=Y, df=self.latent_variables.z_list[(-3)].prior.transform(beta[(-3)]), loc=loc, scale=np.exp(lmda / 2.0)))
    ...
    def sample(self, nsims=1000):
        ...
        if self.latent_variables.estimation_method not in ('BBVI', 'M-H'):
            ...
            data_draws = np.array([ ss.t.rvs(loc=self.latent_variables.z_list[(-2)].prior.transform(lv_draws[(-2, i)]), df=self.latent_variables.z_list[(-3)].prior.transform(lv_draws[(-3, i)]), scale=np.exp(sigmas[i] / 2.0)) for i in range(nsims)
                                  ])
            return data_draws
