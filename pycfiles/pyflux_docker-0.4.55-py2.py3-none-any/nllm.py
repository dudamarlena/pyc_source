...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-n5kV9S/pyflux/pyflux/ssm/nllm.py
...
class NLLEV(tsm.TSM):
    ...
    **** NON-GAUSSIAN LOCAL LEVEL MODEL ****
    ...
    def neg_loglik(self, beta):
        ...
        return self.state_likelihood(beta, states) + self.family.neg_loglikelihood(self.data, self.link(states[0, :]), scale, shape, skewness)
    ...
    def _animate_bbvi(self, stored_latent_variables, stored_predictive_likelihood):
        ...
        anim = FuncAnimation(fig, ud, frames=np.arange(stored_latent_variables.shape[0]), init_func=ud.init, interval=10, blit=True)
        plt.plot(self.data)
        plt.xlabel('Time')
