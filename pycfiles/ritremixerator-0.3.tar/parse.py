...
# [GCC 8.4.0]
# Embedded file name: /home/eitan/Documents/code/RITRemixerator/dorrie/../dorrie/comps/parse.py
# Compiled at: 2012-02-01 17:08:09
import os, pytz, urllib, json
...
def build_ks(id):
    ...
    for line in lines:
        if line.startswith('%include'):
            continue
        if line.startswith('part / ') and lines[(lines.index(line) + 1)].startswith('part / '):
            continue
        fd.write(line)
...
def analyze_log(spin):
    ...
    for line in reversed(fd.readlines()):
        ...
        if line.find('done, estimate finish') is not -1:
            try:
        ...
        if line.find('Parallel mksquashfs') is not -1:
            return ('Create Squash filesystem', 80)
        if line.find('e2fsck') is not -1 or line.find('resize2fs') is not -1 or line.find('e2image') is not -1:
            return ('Check and resize filesystem', 70)
        if line.find('password') is not -1:
            return ('Changing root password', 65)
        if line.find('Installing:') is not -1:
            try:
                estimate = int(float(line[line.find('[') + 1:line.find('/')]) / float(line[line.find('/') + 1:line.find(']')]) * 50)
            except:
                return (None, None)

            return ('Installing packages%s' % ('.' * (estimate / 10)), 15 + estimate)
        if line.find('Retrieving') is not -1:
            return ('Retrieving repodata', 10)
        if line.find('mke2fs') is not -1:
