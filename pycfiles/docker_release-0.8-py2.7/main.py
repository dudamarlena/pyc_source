...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/docker_release/main.py
# Compiled at: 2016-10-28 02:04:30
from os import path
...
def main():
    ...
    parser.add_argument('--force', '-f', action='store_true', help='Release even if it is already released.')
    parser.add_argument('--dry-run', '-d', action='store_true', help='Just build and tag locally. Do not push to docker hub.')
    parser.add_argument('--no-build', '-B', action='store_true', help='Do not build images. Tag and push current :latest')
    parser.add_argument('--tag-once', '-T', action='store_true', help='Tag the git repository once with release-<version> rather than per-image')
    parser.add_argument('--yes', '-y', action='store_true', help='Accept suggested answers for all the questions. Noninteractive mode.')
    parser.add_argument('--verbose', '-v', action='store_true', help='Make ouput more verbose (print output from docker daemon).')
    parser.add_argument('docker_image_dir', metavar='dir', nargs='+', help='Location of docker image directory with Dockerfile to be released')
    ...
    type_group.add_argument('--snapshot', '-s', action='store_true', help="Release a snapshot version (push tags with git sha and updates 'latest').")
