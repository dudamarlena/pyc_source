...
# [GCC 8.4.0]
# Embedded file name: /Users/Alister/miniconda3/envs/obsea/lib/python3.7/site-packages/obsea/processing.py
...
def decompose(xarr):
    ...
    mrl = np.abs(np.mean((Hn * np.exp(complex(0.0, 1.0) * theta)), axis=(-1)) * (2 * np.pi))
...
def svd_filter(xarr):
    ...
    u, s, vh = np.linalg.svd(((xarr - xarr.mean(dim='time')).values),
      full_matrices=False)
    knee = find_knee(s)
