...
# [GCC 8.4.0]
# Embedded file name: /Users/Alister/miniconda3/envs/obsea/lib/python3.7/site-packages/obsea/core.py
# Compiled at: 2019-11-19 06:09:36
# Size of source mod 2**32: 13910 bytes
"""
Core module.
...
def trace2xarr(tr):
    ...
    """
    xarr = xr.DataArray(data=(tr.data.astype('float')),
...
def stft(tr, nperseg, step, water_level=None):
    ...
    result = result.rolling(time=nperseg, center=True).construct('frequency',
      stride=step)
    ...
    if water_level:
        response = tr.stats.response.get_evalresp_response_for_frequencies(result.coords['frequency'])
        w = np.abs(response).max() * 10.0 ** (-water_level / 20.0)
        mask = np.where(np.abs(response) < w)
        response[mask] = w * np.exp(complex(0.0, 1.0) * np.angle(response[mask]))
        response = xr.DataArray(data=response,
          coords={'frequency': result.coords['frequency']},
          dims='frequency')
        result = result / response
    dims = [dim for dim in result.dims if dim not in ('frequency', 'time')] + [
     'frequency', 'time']
...
def azigram(z, nperseg, step, method='intensity', mode='net'):
    ...
    if method == 'intensity':
        ...
        result = result['vx'] + complex(0.0, 1.0) * result['vy']
        result /= np.abs(result)
        double_angle = False
    else:
        if method == 'polarization':
            if mode == 'net':
                result = z[['vx', 'vy']].to_array(dim='component').transpose('frequency', 'time', 'component')
                x = np.stack((result.real.values, result.imag.values), axis=(-1))
                u, s, vh = np.linalg.svd(x)
                r = u[(Ellipsis, 0, 0)] + complex(0.0, 1.0) * u[(Ellipsis, 0, 1)]
                result = xr.DataArray(data=r,
                  coords={'time':z['time'].values, 
                ...
                result = result ** 2
                double_angle = True
        result = result.rolling(time=nperseg, center=True).construct('w',
          stride=step)
...
def time_azimuth(r, nperseg, step, bins, sigma=None, fmin=None, fmax=None):
    ...
    rot = np.exp(complex(0.0, 1.0) * np.pi / bins)
    result = r * rot
    result = result.rolling(time=nperseg,
      center=True).construct('h',
      stride=step)
    data = result.values.swapaxes(-3, -2)
    data = data.reshape(data.shape[:-2] + (data.shape[(-2)] * data.shape[(-1)],))

    def histogram(z):
        h = np.histogram((np.arctan2(z.real, z.imag) % (2 * np.pi)), bins=bin_edges,
          density=False,
          weights=(np.abs(z)))[0]
...
def orientation_frequency(r, track, bins, sigma=None, fmin=None, fmax=None):
    ...
    track /= np.abs(track)
    result = r.conj() * track * np.exp(complex(0.0, 1.0) * np.pi / bins)
    bin_edges = np.linspace(0, 2 * np.pi, bins + 1)

    def histogram(z):
        h = np.histogram((-np.arctan2(z.imag, z.real) % (2 * np.pi)), bins=bin_edges,
          density=False,
          weights=(np.abs(z)))[0]
        return h * (h.size / z.size) / (2 * np.pi)
    ...
    if sigma:
        data = gaussian_filter1d(data, sigma=(sigma * bins / 360), mode='wrap')
    coords = {'orientation':np.linspace(0, 360, bins, endpoint=False),  'frequency':result.coords['frequency']}
    result = xr.DataArray(data=data, coords=coords, dims=[
