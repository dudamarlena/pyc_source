...
# [GCC 8.4.0]
# Embedded file name: kaldi2/utils.py
...
def load_wav(file_name, def_sample_width=2, def_sample_rate=16000):
    ...
    try:
        wf = wave.open(file_name, b'r')
        if wf.getnchannels() != 1:
            raise Exception(b'Input wave is not in mono')
        if wf.getsampwidth() != def_sample_width:
            raise Exception(b'Input wave is not in %d Bytes' % def_sample_width)
        ...
        raise Exception(b'Input PCM is corrupted: End of file.')
    else:
        wf.close()
...
def expand_prefix(d, bigd):
    ...
    if isinstance(d, dict):
        if len(d) == 2 and b'prefix' in d and b'value' in d:
            pref = bigd[d[b'prefix']]
            if isinstance(pref, dict):
                pref = expand_prefix(pref, bigd)
            s = os.path.sep.join([pref, d[b'value']]).rstrip(os.path.sep)
        ...
        if isinstance(d, unicode) or isinstance(d, str):
            return d.encode(b'utf-8')
        raise ValueError(b'We support only dictionaries, lists and strings.')


def wst2dict(wst_path, encoding=b'utf-8'):
    ...
    with codecs.open(wst_path, encoding=encoding) as (r):
        ...
        d = dict([])
        for arr in line_arr:
            assert len(arr) == 2, b'Word Symbol Table should have 2 records on each row'
            d[int(arr[1])] = arr[0]
...
def int_to_txt(inp_path, out_path, wst_dict, unknown_symbol=None):
    ...
    if unknown_symbol is None:
        unknown_symbol = b'\\<UNK\\>'
    with open(inp_path, b'r') as (r):
        with open(out_path, b'w') as (w):
            for line in r:
                tmp = line.split()
                ...
                for iw in dec:
                    try:
                        ...
                        print b'Warning: unknown word %s' % iw
                        word = unknown_symbol
...
def compact_hyp(hyp_path, comp_hyp_path):
    ...
    with open(hyp_path, b'rb') as (hyp):
        for line in hyp:
            tmp = line.split()
            name, dec = tmp[0], tmp[1:]
            d[name].extend(dec)

    with open(comp_hyp_path, b'wb') as (w):
