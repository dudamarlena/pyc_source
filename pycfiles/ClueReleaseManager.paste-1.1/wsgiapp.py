...
# Embedded file name: build/bdist.linux-i686/egg/clue/relmgr/wsgiapp.py
# Compiled at: 2009-09-18 12:06:12
import os, re, xmlrpclib, werkzeug
from werkzeug import exceptions as werkexc
from repoze.who import middleware as whomiddleware, config as whoconfig, classifiers
from repoze.who.plugins import basicauth, sql
from werkzeug import routing
import jinja2
from docutils import core as docutilscore
from clue.relmgr import utils, pypi
import pkg_resources
__version__ = pkg_resources.get_distribution('ClueReleaseManager').version
template_loader = jinja2.Environment(loader=jinja2.PackageLoader('clue.relmgr', 'templates'))
...
class NoSuchDistroError(werkexc.NotFound, CommonError):
    ...
    def __init__(self, distro_id):
        ...
        super(NoSuchDistroError, self).__init__('No such distro "%s"' % distro_id)
...
class AbstractPyPiApp(object):
    ...
    def __call__(self, environ, start_response):
        ...
        try:
            ...
            res = werkzeug.Response(tmpl.render(exc=exc, url_root=req.url_root, version=__version__), content_type='text/html; charset=UTF-8', status=404)
            return res(environ, start_response)
...
class SimpleIndexApp(AbstractPyPiApp):
    ...
    def respond_index(self, req):
        ...
        for distro in self.pypi.get_distros():
            yield '<li><a href="%s/">%s</a></li>' % (distro.distro_id,
             distro.name)
...
class PyPiInnerApp(AbstractPyPiApp):
    ...
    urlmap.add(routing.Rule('/d/<distro_id>/i/<indexname>', endpoint='customindex'))
    ...
    def respond_file(self, req, distro_id, filename):
        ...
        res.response = open(self.pypi.get_file(distro_id, filename), 'rb')
        return res
    ...
    def respond_index(self, req, distro_id=None):
        ...
        yield tmpl.render(page=page, latest_changes=page, version=__version__, **self.globs(req.environ))
    ...
    def respond_search(self, req):
        ...
        yield tmpl.render(s=s, items=page, page=page, version=__version__, **self.globs(req.environ))
    ...
    def respond_distro(self, req, distro_id=None):
        ...
        for x in self.pypi.get_indexes(distro_id):
            indexes.append({'indexname': x, 'url': 'i/' + x})

        files = self.pypi.get_files(distro_id)
        kwargs = self.globs(req.environ, distro=self.pypi.get_distro(distro_id), indexes=indexes, files=[ {'filename': os.path.basename(x), 'url': '%sd/%s/f/%s' % (url_root, distro_id, os.path.basename(x))} for x in files
                                                                                                        ], url_root=url_root, rst=self.rst_format)
        return tmpl.render(version=__version__, **kwargs)

    def rst_format(self, s):
        published = docutilscore.publish_parts(s, settings_overrides={'halt_level': 10}, writer_name='html')
        return '<div class="rst">' + published['html_body'] + '</div>'

    def __call__(self, environ, start_response):
        pypi.active_info.username = environ.get('REMOTE_USER', None)
        self.logger.debug('Handling request as: %s' % pypi.active_info.username)
...
class PyPiApp(object):
    ...
    def __init__(self, basefiledir, baseurl=None, security_config=None, sqluri='sqlite:///cluerelmgr.db', self_register=False, backup_pypis=[], logger=utils.logger, securelogger=utils.securelogger):
        self.logger = logger
    ...
    def pypi(self):
        return self.pypi_factory(self.basefiledir, self.sqluri, self.self_register)
    ...
    def app(self):
        ...
        if self.security_config is None:
            identifiers = [
             (
              'basicauth', basicauth.BasicAuthPlugin('pypi'))]
            ...
            authenticators = [
             (
              'sqlauth',
              sql.SQLAuthenticatorPlugin('SELECT username, password FROM users WHERE username=:login', factory, None))]
        ...
        app = whomiddleware.PluggableAuthenticationMiddleware(innerapp, identifiers, authenticators, challengers, mdproviders, classifiers.default_request_classifier, classifiers.default_challenge_decider, log_stream=self.securelogger, log_level=self.securelogger.getEffectiveLevel())
        if self.baseurl:
            app = VirtualHostMiddleware(self.baseurl, app)
        app = werkzeug.SharedDataMiddleware(app, {'/static': ('clue.relmgr', 'static')})
        innerapp.top = app
        return app
