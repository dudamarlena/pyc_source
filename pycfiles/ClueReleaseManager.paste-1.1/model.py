...
# Embedded file name: build/bdist.linux-i686/egg/clue/relmgr/model.py
...
users_groups_table = sa.Table('users_groups', metadata, sa.Column('username', sa.String, sa.ForeignKey('users.username'), primary_key=True), sa.Column('groupname', sa.String, sa.ForeignKey('groups.groupname'), primary_key=True))
...
class SQLRoleMapping(Base):
    ...
    def __init__(self, role=None, distro_id=None, username=None, groupname=None):
        if role is not None:
            self.role = role
        if distro_id is not None:
            self.distro_id = distro_id
    ...
    distro_id = sa.Column(sa.String, sa.ForeignKey('distros.distro_id'), primary_key=True)
    username = sa.Column(sa.String, sa.ForeignKey('users.username'), primary_key=True)
    groupname = sa.Column(sa.String, sa.ForeignKey('groups.groupname'), primary_key=True)
    user = orm.relation(SQLUser, primaryjoin=username == SQLUser.username)
    distro = orm.relation(SQLDistro, primaryjoin=distro_id == SQLDistro.distro_id)
    group = orm.relation(SQLGroup, primaryjoin=groupname == SQLGroup.groupname)
...
class SecurityManager(object):
    ...
    def get_roles(self, username, distro_id=None, also_global=False):
        ...
        if not username or username == 'anonymous':
            ...
            self._populate_roles(ses, roles, 'anonymous', distro_id, also_global)
        return roles

    def _populate_roles(self, ses, roles, username, distro_id, also_global):
        distro = distro_id
        rows = ses.execute('select groupname from users_groups where username=:username', {'username': username})
        ...
        if isinstance(distro, basestring):
            ...
            if distro is None:
                raise ValueError('no such distro, "%s"' % str(distro_id))
        if username == distro.owner:
            roles.add(OWNER_ROLE)
        for check in checks:
            for x in query.filter_by(distro_id=distro.distro_id, **check):
                roles.add(x.role)
    ...
    def update_roles(self, distro_id='', username='', groupname='', roles=[]):
        ses = self.sessionmaker()
    ...
    def get_groups(self, username):
        ...
        s = sql.select([users_groups_table.c.groupname], users_groups_table.c.username == username)
        groups = set()
    ...
    def update_users_groups(self, username, groups):
        ...
        g2 = set(groups)
        if g1 == g2:
            self.logger.info('Groups for user "%s" unchanged since no changes needed' % username)
            return
        self.logger.info('original: ' + (', ').join(g1))
        self.logger.info('new: ' + (', ').join(g2))
        ses.execute(users_groups_table.delete().where(users_groups_table.c.username == username))
        keys = groupdict.keys()
        for x in keys:
            if x not in groups:
                del groupdict[x]
        ...
        for (gname, group) in groupdict.items():
            s = users_groups_table.insert().values(username=username, groupname=gname)
            ses.execute(s)
    ...
    def _update_roles(self, ses, distro_id='', username='', groupname='', roles=[]):
        if not username:
            assert groupname
            kwargs = {'distro_id': distro_id, 'groupname': groupname, 'username': username}
            assert username and not groupname
...
class SQLIndexItem(Base):
    ...
    distro_id = sa.Column(sa.String, sa.ForeignKey('distros.distro_id'), primary_key=True)
    target_distro_id = sa.Column(sa.String, sa.ForeignKey('distros.distro_id'), primary_key=True)
...
class IndexManager(object):
    ...
    def get_indexes(self, distro_id):
        ...
        for x in indexq.filter_by(distro_id=distro_id):
            ...
            if d is None:
                raise ValueError('Cannot find target distro: %s' % x.target_distro_id)
    ...
    def add_index_item(self, distro_id, indexname, target_distro_id, target_version):
        ses = self.sessionmaker()
        entry = SQLIndexItem()
    ...
    def del_index_item(self, distro_id, indexname, target_distro_id):
        ses = self.sessionmaker()
        q = ses.query(SQLIndexItem)
        for item in q.filter_by(distro_id=distro_id, indexname=indexname, target_distro_id=target_distro_id):
            ses.delete(item)
