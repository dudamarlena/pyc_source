...
# Embedded file name: build/bdist.linux-i686/egg/clue/chatter/svnnotify.py
# Compiled at: 2008-06-05 18:00:27
import os, sys, subprocess, xmlrpclib, optparse
...
class Revision(object):
    ...
    def _svnlook(self, cmd):
        cmdargs = [
         'svnlook', cmd, self.repo, '-r', self.rev]
        output = subprocess.Popen(cmdargs, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0]
        return output.strip()


class SVNBlabber(object):
    COMMIT_MSG = 'SVN [%(reponame)s] %(author)s r%(rev)s : %(num_modified)i modified : %(extra)s%(msg)s\n  %(url)s'

    def __init__(self, repo, network, channel, bothost='localhost', botport=7766, rev_url=None):
        self.repo = repo
    ...
    def report_commit(self, revnum):
        ...
        if self.rev_url:
            ...
            url = ''
        msg = self.COMMIT_MSG % {'author': rev.author, 'rev': rev.rev, 'extra': extra, 
...
def main(argv=sys.argv[1:]):
    ...
    parser.add_option('-a', '--address', action='store', dest='address', help='IP address of IRC bot')
    parser.add_option('-p', '--port', action='store', dest='port', type='int', help='Port of IRC bot')
    parser.add_option('-r', '--rev_url', action='store', dest='rev_url', help='Revision URL to use (ie looks like http://somehost.com/%(repo)s/%(rev)s)')
