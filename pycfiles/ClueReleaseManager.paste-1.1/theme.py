...
# Embedded file name: build/bdist.linux-i686/egg/clue/themer/theme.py
# Compiled at: 2008-07-04 21:38:36
import os
...
class Theme(object):
    ...
    def __init__(self, themeid, name, path=None, rule_uri=None, theme_uri=None):
    ...
    def copy(self, newthemeid=None, newname=None):
        return Theme(newthemeid or self.themeid, newname or self.name, self.path, self.rule_uri, self.theme_uri)

    def link(self, newthemeid=None, newname=None):
        return LinkedTheme(newthemeid or self.themeid, newname or self.name, self)
...
class LinkedTheme(Theme):
    ...
    def link(self, newthemeid=None, newname=None):
        return LinkedTheme(newthemeid or self.themeid, newname or self.name, self.srctheme)


DEFAULT_CM_THEME = Theme(themeid='$DEFAULT_CM_THEME', name='Default ClueMapper Theme', rule_uri='/.defaulttheme/rules/rules.xml', theme_uri='/.defaulttheme/static/theme.html')
DEFAULT_CONFIG_THEME = DEFAULT_CM_THEME.link('$DEFAULT_CONFIG_THEME', 'Default Configured Theme')
...
class ThemeManager(object):
    ...
    def _setup_themes(self):
        ...
        themes[DEFAULT_CONFIG_THEME.themeid] = srctheme.link(DEFAULT_CONFIG_THEME.themeid, DEFAULT_CONFIG_THEME.name)
...
class ThemeFilter(object):
    ...
    def _act_on_theme(self, environ, query=None):
        ...
        newthemeid = query.get('__clue_set_themeid', environ.get('cluemapper.themeid', None))
        tmanager = self._tmanager_factory(environ[WORKINGDIR], environ[CONFIG])
        if not newthemeid:
...
def make_filter(app, global_conf, *args, **kwargs):
    ...
    """
    deliver = wsgimiddleware.make_filter(app, global_conf, theme_uri=DEFAULT_CONFIG_THEME.theme_uri, rule_uri=DEFAULT_CONFIG_THEME.rule_uri)
...
def switch_theme(tmanager, environ, themeid):
    ...
    return (
     theme.rule_uri, theme.theme_uri)