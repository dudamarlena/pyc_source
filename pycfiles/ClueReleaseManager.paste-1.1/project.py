...
# Embedded file name: build/bdist.linux-i686/egg/clue/app/project.py
# Compiled at: 2008-06-27 12:03:50
import string, os
from trac import env
from trac import perm
import tempfile, logging, subprocess, pkg_resources, clue.app
...
class ProjectManager(object):
    ...
    def _cluemapperconfig(self):
        return self._environ.get('cluemapper.config', self._global_conf.get('cluemapper.config', None))

    @property
    def _workingdir(self):
        return self._environ.get('cluemapper.workingdir', self._global_conf.get('cluemapper.workingdir', None))
    ...
    def _gen_unique_id(self, name):
        ...
        if 'project:' + newname in self._cluemapperconfig.keys():
            raise ProjectError('Could not generate new unique id for %r' % name)
        return newname
    ...
    def _get_paths(self, prjid):
        ...
        tracadminpath = base.get('tracadminpath', pb(os.path.join(self._workingdir, 'bin', 'trac-admin')))
        if not os.path.exists(tracadminpath):
            tracadminpath = self._cached_system_trac or self._find_tracadminpath()
            self._cached_system_trac = tracadminpath
        return (
         tracadminpath, tracenvpath, svnrepopath)

    def create_project(self, prj_name, themeid=None):
        if not self._can_manage():
            raise PermissionError('Current user %r has not been assigned as a manager' % self._username)
        base = self._cluemapperconfig['base']
    ...
    def upgrade_project(self, prjid):
        (tracadminpath, tracenvpath, svnrepopath) = self._get_paths(prjid)
        cmd = [tracadminpath, tracenvpath, 'upgrade']
        self._execute(cmd)
        cmd = [tracadminpath, tracenvpath, 'resync']
        self._execute(cmd)
        logger.info('Project %s upgraded' % prjid)
    ...
    def _execute(self, cmd):
        ...
        try:
            try:
                ...
                raise ProjectError('The cmd %r could not be executed, full cmd args are: %r' % (
                 cmd[0], (' ').join(cmd)))
    ...
    DEFAULT_PERMS = {'anonymous': [], 'authenticated': [], 'member': [
                'BROWSER_VIEW',
                'CHANGESET_VIEW',
                'FILE_VIEW',
                'LOG_VIEW',
    ...
    def reset_perms(self, prjid):
        ...
        for (user, perms) in self.DEFAULT_PERMS.items():
            ...
            if ret != 0:
                raise ProjectError('Got error code %r when running permission' % ret)
            if len(perms) == 0:
                continue
            args = [
             tracadminpath, tracenvpath, 'permission', 'add', user]
            args.extend(perms)
        ...
        for (user, v) in permmap.items():
            ...
            ret = self._execute(args)
            if ret != 0:
                raise ProjectError('Got error code %r when running permission: %r' % (
                 ret, args))
    ...
    def fix_paths(self, prjid):
        ...
        pb = utils.PathBuilder(self._workingdir)
        tracconfig['account-manager']['password_file'] = pb(clueconfig['base']['auth_file'])
        tracconfig['trac']['repository_dir'] = pb(os.path.join(clueconfig['base']['svnrepo_base'], prjid))
        tracconfig['cluemapper']['database'] = 'sqlite:' + pb(os.path.join('etc', 'cluemapper', 'cluemapper.db'))
        tracconfig.save()
    ...
    def _setup_default_trac(self, prjid):
        ...
        tracconffile = os.path.join(tracenvpath, 'conf', 'trac.ini')
        tracconfig = config.Config(tracconffile)
        templatename = os.path.join('skel', 'trac.ini.template')
        data = pkg_resources.resource_string(__name__, templatename)
        pb = utils.PathBuilder(self._workingdir)
        dbpath = pb(os.path.join('etc', 'cluemapper', 'cluemapper.db'))
        ini = data % {'auth_file': self._cluemapperconfig['base']['auth_file'], 'shared_db': 'sqlite:%s' % dbpath, 
           'svnauthzfile': self._cluemapperconfig['base']['svnauthzfile']}
