...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/xbmc_client/xbmc_client.py
# Compiled at: 2016-08-17 15:36:44
...
class XbmcClient(object):
    ...
    def execute(self):
        ...
        if self.options.scan is not None:
            ...
            if self.options.clean == 'video':
                ...
                if self.options.clean == 'music':
                    res = self.xbmc.AudioLibrary.Clean()
                else:
                    print("Unsupported library type : '%s'" % self.options.clean)
        if res is not None:
            success = False
            if 'result' in res and (res['result'] == 'OK' or res['result'] == True):
                success = True
        else:
            if self.options.volume is not None and 'result' in res and type(res['result']) == int:
                success = True
            else:
                if 'result' in res and res['result'] == 'pong':
                    success = True
                else:
                    if self.options.playpause and 'result' in res:
                        success = True
                    else:
                        if 'result' in res and res['result'] == False and self.options.unmute:
                            success = True
                        else:
    ...
    def playYoutubeVideo(self, url):
        match = re.match('.*youtube.com/watch\\?v=(.{11})', url)
        if match is None:
            print('Error, could not find valid youtube url.')
            exit(-1)
        video_id = match.group(1)
        self.xbmc.Player.Open(item={'file': 'plugin://plugin.video.youtube/?action=play_video&videoid=' + video_id})
...
def main():
    ...
    parser.add_option('--youtube', action='store', type='string', dest='youtube_url', help='Open a youtube url')
