...
class Interact(object):
    ...
    def __init__(self, grammar_or_parser, application='cly', prompt=None, user_context=None, with_context=None, history_file=None, history_length=500, completion_key='tab', completion_delimiters=' \t', help_key='?', inhibit_exceptions=False, with_backtrace=False):
        if prompt is None:
            prompt = application + '> '
    ...
    def error_at_cursor(self, context, text):
        ...
        indent = ' ' * (context.cursor % term_width + len(Interact.prompt))
...
def interact(grammar_or_parser, inhibit_exceptions=False, with_backtrace=False, *args, **kwargs):
    """Start an interactive session with the given grammar or parser object."""
    interact = Interact(grammar_or_parser, *args, **kwargs)
    interact.loop(inhibit_exceptions=inhibit_exceptions, with_backtrace=with_backtrace)