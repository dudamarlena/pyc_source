...
# Embedded file name: build/bdist.linux-i686/egg/cly/builder.py
# Compiled at: 2007-12-09 10:39:19
__doc__ = 'Classes for constructing CLY grammars.'
import re, os, posixpath
from xml.dom import minidom
from inspect import isclass
from cly.exceptions import *
__all__ = [
 'Node', 'Alias', 'Group', 'Action', 'Variable', 'Grammar', 'Help',
 'LazyHelp', 'Word', 'String', 'URI', 'LDAPDN', 'Integer', 'Float', 'IP',
 'Hostname', 'Host', 'EMail', 'File', 'Boolean']
__docformat__ = 'restructuredtext en'

class Node(object):
    r"""The base class for all grammar nodes.
    ...
    def __iter__(self):
        ...
        [<Node:/three>, <Node:/two>]
        """
        children = sorted(self._children.values(), key=lambda i: (
         i.group, i.order, i.name))
    ...
    def match(self, context):
        ...
        if match:
            if not self._separator.match(context.command, context.cursor + len(match.group())):
                return
            if self.match_candidates and match.group() + ' ' not in self.candidates(context, match.group()):
                return
            return match
        return
    ...
    def valid(self, context):
        ...
        return not self.traversals or context.traversed(self) < self.traversals
...
class Alias(Node):
    ...
    def __init__(self, target, *args, **kwargs):
        Node.__init__(self, ('<alias for "%s">' % target), *args, **kwargs)
        self._target = target
...
class Action(Node):
    ...
    def terminal(self, context):
        if self.with_user_context or self.with_user_context is None and self.parser.with_user_context:
...
class Variable(Node):
    ...
    def valid(self, context):
        if self.traversals != 1 or self.traversals == 1 and self.name not in context.vars or len(context.vars.get(self.name, [])) < self.traversals:
            return Node.valid(self, context)
        return False

    def selected(self, context, match):
        r"""Convert the match to a value with self.parse(), then add
        ...
        try:
            ...
            raise ValidationError(context, token=match.group(), exception=unicode(e))
    ...
    def parse(self, context, match):
        r"""Parse the match and return a value. Value can be of any type: tuple,
        list, object, etc.
...
class Grammar(Node):
    ...
    def from_xml(cls, xml, extra_nodes=None, **locals):
        ...
        arg_types = {'traversals': int, 
           'group': int, 
           'order': int, 
           'match_candidates': boolean}
        arg_types.update(dict.fromkeys(('children follow selected next match advance visible terminal depth path candidates find valid callback').split(), evaluate))
        node_types = dict([ (v.__name__.lower(), v) for v in globals().values() + extra_nodes if isclass(v) if issubclass(v, Node)
                          ])
        ...
        def parse(parent, xnode):
            ...
            if xnode.nodeType == minidom.Node.ELEMENT_NODE:
                ...
                attributes = dict([ (str(k), v) for (k, v) in xnode.attributes.items()
                                  ])
        ...
        if dom.firstChild.localName != 'grammar':
            raise XMLParseError('Invalid root element "%s", expected "grammar"' % dom.firstChild.localName)
...
class LazyHelp(Help):
    ...
    def __call__(self, context):
        ...
        if self.node.name == self.node.pattern:
            yield (
             self.node.name, self.text)
        else:
            yield (
             '<%s>' % self.node.name, self.text)
...
class Hostname(Variable):
    ...
    ('www', 'example', 'com')
    """
    pattern = '(?i)([A-Z0-9][A-Z0-9_-]*)(?:\\.([A-Z0-9][A-Z0-9_-]*))*'
...
class Host(Variable):
    ...
    (123, 34, 67, 89)
    """
    pattern = '(?i)(%s)|(%s)' % (IP.pattern, Hostname.pattern)
...
class File(Variable):
    ...
    def candidates(self, context, text):
        ...
        def get_candidates(dir, file):
            return [ f for f in os.listdir(dir) if f.startswith(file) if self.match_file(os.path.join(dir, f))
                   ]
        ...
        if len(candidates) == 1:
            if os.path.isdir(os.path.join(dir, candidates[0])):
                ...
                return [
                 clean(os.path.join(dir, candidates[0] + ' '))]
        return [ clean(os.path.join(dir, f)) for f in candidates if self.allow_dotfiles or f[0] != '.' ]
