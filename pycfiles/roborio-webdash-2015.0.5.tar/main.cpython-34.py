...
# [GCC 8.4.0]
# Embedded file name: /home/christian/PycharmProjects/RoboRIO-webdash/webdash/main.py
# Compiled at: 2015-02-10 09:48:12
...
def main():
    if len(sys.argv) <= 1:
        ...
        if sys.argv[1] == 'install-initfile':
            for tgt in INSTALL_LOCATIONS:
                if not exists(dirname(tgt)):
                    print('ERROR: Installation path {} does not exist. (Are you sure you are installing this on an os with sysvinit?)'.format(dirname(tgt)))
                    exit(-1)
