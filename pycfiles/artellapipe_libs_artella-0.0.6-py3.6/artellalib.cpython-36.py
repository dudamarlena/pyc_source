...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/libs/artella/core/artellalib.py
# Compiled at: 2020-04-25 12:41:52
# Size of source mod 2**32: 44787 bytes
"""
...
import os, re, sys, json, socket, logging, traceback, threading
try:
...
class AbstractArtella(object):
    ...
    def getCmsUri(broken_path):
        path_parts = re.split('[/\\\\]', broken_path)
...
def update_artella_paths():
    ...
    if artella_folder is not None:
        if os.path.exists(artella_folder):
...
def get_artella_data_folder():
    ...
    if osplatform.is_mac():
        ...
        if artella_app_version is not None:
            ...
            artella_folder = [os.path.join(artella_folder, name) for name in os.listdir(artella_folder) if os.path.isdir(os.path.join(artella_folder, name)) if name != 'ui']
        ...
        if not os.path.exists(artella_folder):
            qtutils.show_info(None, 'Artella Folder not found!', 'Artella App Folder {} does not exists! Make sure that Artella is installed in your computer!')
...
def launch_artella_app():
    ...
    if os.name == 'mac':
        ...
        qtutils.show_info(None, 'Not supported in MAC', 'Artella Pipeline do not support automatically Artella Launch for Mac. Please close Maya, launch Artella manually, and start Maya again!')
...
def spigot_listen(cli, app_id, handler):
    ...
    spigot_thread = threading.Thread(target=(cli._pullMessages),
      args=(
     app_id, handler))
...
def get_metadata():
    ...
    rsp = json.loads(rsp)
    metadata = artellaclasses.ArtellaAppMetaData(cms_web_root=(rsp['cms_web_root']),
...
def split_version(name, next_version=False):
    ...
    if next_version:
        ...
        return (
         new_string_version, new_int_version, new_int_version_formatted)
    else:
        return (
         string_version, int_version, int_version_formatted)
    ...
    if rsp:
        if isinstance(rsp, artellaclasses.ArtellaDirectoryMetaData):
            ...
            if rsp.header.status != 'OK':
                ...
                return is_updated
        if isinstance(rsp, artellaclasses.ArtellaReferencesMetaData):
    ...
    if not file_path:
        ...
        if not can_write:
            if is_locked_by_me:
                ...
                if artellapipe.project:
                    artellapipe.project.message(msg)
        if in_edit_mode is None:
            if is_locked_by_me is None:
        ...
        if force or not in_edit_mode:
            ...
            if not force:
                if not in_edit_mode:
                    ...
                    if artellapipe.project:
                        artellapipe.project.message(msg)
                    result = tp.Dcc.confirm_dialog(title='Artella Pipeline - Lock File',
                      message=msg,
                      button=['Yes', 'No'],
                      cancel_button='No',
                      dismiss_string='No')
...
def upload_file(file_path, comment):
    ...
    if rsp.get('status', {}).get('meta', {}).get('status') != 'OK':
        ...
        LOGGER.info(msg)
        tp.Dcc.warning(msg)
        tp.Dcc.confirm_dialog(title='Artella Pipeline -  Failed to Upload Bug. Restart DCC please!',
          message=msg,
          button=['OK'])
        return False
    else:
    ...
    if not file_path:
        ...
        if file_path is not None and file_path != '':
            ...
            if artellapipe.project:
                artellapipe.project.message(msg)
            if comment is None:
                result = tp.Dcc.confirm_dialog(title='Artella Pipeline - Save New Version on Artella Server',
                  message=msg,
                  button=['Save', 'Cancel'],
            ...
            if isinstance(rsp, (unicode, str)):
                rsp = json.loads(rsp)
            if 'status' in rsp:
                if 'meta' in rsp['status']:
                    if rsp['status']['meta']['status'] != 'OK':
    ...
    if rsp:
        if isinstance(rsp, artellaclasses.ArtellaDirectoryMetaData):
            ...
            if isinstance(rsp, artellaclasses.ArtellaReferencesMetaData):
                ...
                if isinstance(rsp, artellaclasses.ArtellaAssetMetaData):
