...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/local_worker/core/task_manager.py
  ...
  """
    ...
    def __init__(self, sample_path, avs, test_method, window, max_retries, store_logcat_snapshot):
        ...
        for av in avs:
            ...
            task_test_data = {'detection_method': test_method, 
    ...
    """
        result = {'sample': get_hashes(self.sample_path), 'test_count': int(self.ready_queue.qsize()), 
           'success_count': 0, 
           'tests': []}
            ...
            try:
             ...
             virtualdevice.AVDStartTimeOut) as e:
                ...
                if task.num_tries == self.max_retries:
                    ...
                    task_id = task.test['test_module_name'] + ', on ' + task.test['avd_name'] + ', with ' + task.sample_path + ' for ' + task.test['detection_method']
                    result['tests'].append(task.failure(e.__class__.__name__, 'Task %s failed after %d attempts' % (task_id, self.max_retries)))
                    continue
    ...
    def _get_antivirus_info(self, avd_name):
        ...
        av_file.close()
        return {'engine_version': av['engine_version'], 
           'name': av['name'], 
