...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\jinja\parser.py
# Compiled at: 2007-11-17 17:39:58
"""
    jinja.parser
    ~~~~~~~~~~~~
...
__all__ = [
 'Parser']
end_of_block = StateTest.expect_token('block_end', msg='expected end of block tag')
end_of_variable = StateTest.expect_token('variable_end', msg='expected end of variable')
end_of_comment = StateTest.expect_token('comment_end', msg='expected end of comment')
...
class Parser(object):
    ...
    def __init__(self, environment, source, filename=None):
        ...
        self.closed = False
        self.blocks = set()
        self.directives = {'raw': self.parse_raw_directive, 
    ...
    def parse_raw_directive(self):
        ...
        """
        if self.stream:
            raise TemplateSyntaxError('raw directive does not support any arguments.', self.stream.lineno, self.filename)
        raise TemplateSyntaxError('missing end tag for raw directive.', self.stream.lineno, self.filename)

    def parse_extends_directive(self):
        """
        Handle the extends directive used for inheritance.
        """
        raise TemplateSyntaxError('mispositioned extends tag. extends must be the first tag of a template.', self.stream.lineno, self.filename)
    ...
    def parse_for_loop(self):
        ...
        if not item.allows_assignments():
            raise TemplateSyntaxError('cannot assign to expression', token.lineno, self.filename)
        ...
        if self.stream.current.type == 'else':
            ...
            else_ = None
        self.stream.expect('block_end')
        return nodes.ForLoop(item, seq, body, else_, recursive, token.lineno, self.filename)
    ...
    def parse_set_directive(self):
        ...
        if self.stream.current.type == 'bang':
            ...
            scope_local = True
        self.stream.expect('block_end')
        return nodes.Set(name.value, value, scope_local, token.lineno, self.filename)
    ...
    def parse_macro_directive(self):
        ...
        self.stream.expect('block_end')
        return nodes.Macro(macro_name.value, args, body, token.lineno, self.filename)
    ...
    def parse_block_directive(self):
        ...
        if name in self.blocks:
            raise TemplateSyntaxError('block %r defined twice' % name, token.lineno, self.filename)
    ...
    def parse_trans_directive(self):
        ...
        if self.stream.current.type == 'string':
            ...
            self.stream.expect('block_end')
            return nodes.Trans(text.value, None, None, None, trans_token.lineno, self.filename)
        replacements = {}
        plural_var = None
        while self.stream.current.type != 'block_end':
            if replacements:
            ...
            if self.stream.current.type == 'assign':
                ...
                value = nodes.NameExpression(name.value, name.lineno, self.filename)
            if name.value in replacements:
                raise TemplateSyntaxError('translation variable %r is defined twice' % name.value, name.lineno, self.filename)
        ...
        def process_variable():
            ...
            if var_name.value not in replacements:
                raise TemplateSyntaxError("unregistered translation variable '%s'." % var_name.value, var_name.lineno, self.filename)
        ...
        while True:
            ...
            if token.type == 'data':
                ...
                if plural is None and self.stream.current.type == 'pluralize':
                    ...
                    raise TemplateSyntaxError('blocks are not allowed in trans tags', self.stream.lineno, self.filename)
                self.stream.expect('block_end')
            else:
        ...
        if plural is not None:
            plural = ('').join(plural)
        return nodes.Trans(singular, plural, plural_var, replacements, trans_token.lineno, self.filename)
    ...
    def parse_conditional_expression(self):
        ...
        while self.stream.current.type == 'if':
            ...
            expr1 = nodes.ConditionalExpression(expr2, expr1, expr3, lineno, self.filename)
            lineno = self.stream.lineno
    ...
    def parse_compare_expression(self):
        ...
        while True:
            ...
            elif self.stream.current.type == 'not' and self.stream.look().type == 'in':
                self.stream.skip(2)
    ...
    def parse_primary_expression(self, parse_postfix=True):
        ...
        if current.type == 'name':
            ...
            raise TemplateSyntaxError("unexpected token '%s'" % self.stream.current, self.stream.current.lineno, self.filename)
        if parse_postfix:
    ...
    def parse_tuple_expression(self, enforce=False, simplified=False):
        ...
        if not is_tuple and args:
            if enforce:
                raise TemplateSyntaxError('tuple expected', lineno, self.filename)
            return args[0]
    ...
    def parse_bool_expression(self):
        ...
        if token.value == 'true':
            ...
            raise TemplateSyntaxError('expected boolean literal', token.lineno, self.filename)
    ...
    def parse_number_expression(self):
        ...
        if token.type not in ('integer', 'float'):
            raise TemplateSyntaxError('integer or float literal expected', token.lineno, self.filename)
        self.stream.next()
    ...
    def parse_subscript_expression(self, node):
        ...
        if self.stream.current.type == 'dot':
            ...
            if token.type in ('name', 'integer'):
                arg = nodes.ConstantExpression(token.value, token.lineno, self.filename)
            else:
                raise TemplateSyntaxError('expected name or number', token.lineno, self.filename)
            self.stream.next()
            ...
            raise TemplateSyntaxError('expected subscript expression', self.lineno, self.filename)
    ...
    def parse_call_expression(self, node=None):
        ...
        def ensure(expr):
            if not expr:
                raise TemplateSyntaxError('invalid syntax for function call expression', token.lineno, self.filename)
        ...
        self.stream.expect('rparen')
        return nodes.CallExpression(node, args, kwargs, dyn_args, dyn_kwargs, token.lineno, self.filename)
    ...
    def parse_test_expression(self, node):
        ...
        if self.stream.current.type == 'lparen':
            ...
            self.stream.expect('rparen')
        elif self.stream.current.type in ('name', 'string', 'integer', 'float', 'lparen',
                                          'lbracket', 'lbrace', 'regex'):
            args.append(self.parse_expression())
        node = nodes.TestExpression(node, name, args, token.lineno, self.filename)
        if negated:
    ...
    def test_name(self, name):
        ...
        if name in ('true', 'false', 'none', 'undefined', '_'):
            raise TemplateSyntaxError('expected name not special constant', self.stream.lineno, self.filename)
    ...
    def subparse(self, test, drop_needle=False):
        ...
        def push_buffer():
            ...
            result.append(nodes.Text(('').join(template), variables, buffer[0][1].lineno, self.filename))
            del buffer[:]
            ...
            if token_type == 'variable_begin':
                ...
                if handler is None:
                    if self.no_variable_block:
                        ...
                        raise TemplateSyntaxError('unexpected directive %r.' % self.stream.current.type, lineno, self.filename)
                    else:
                        name = self.stream.current.value
                        raise TemplateSyntaxError('unknown directive %r.' % name, lineno, self.filename)
                else:
                    node = handler()
                    if node is not None:
                        push_node(node)
        ...
        if test is not None:
            ...
            raise TemplateSyntaxError('unexpected end of stream' + msg, self.stream.lineno, self.filename)
        return assemble_list()
    ...
    def _sanitize_tree(self, nodelist, stack, extends, body):
        ...
        for node in nodelist:
            if extends is not None and node.__class__ is nodes.Block and stack[(-1)] is not body:
                for n in stack:
                    if n.__class__ is nodes.Block:
                        break
                else:
                    raise TemplateSyntaxError('misplaced block %r, blocks in child templates must be either top level or located in a block tag.' % node.name, node.lineno, self.filename)
            stack.append(node)
    ...
    def parse(self):
        ...
        try:
            ...
            if self.stream.current.type == 'block_begin' and self.stream.look().type == 'extends':
                self.stream.skip(2)
    ...
    def close(self):
        """Clean up soon."""
        self.closed = True
        self.stream = self.directives = self.stream = self.blocks = self.environment = None
        return