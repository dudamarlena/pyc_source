...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\jinja\datastructure.py
# Compiled at: 2007-11-16 08:23:02
"""
    jinja.datastructure
    ~~~~~~~~~~~~~~~~~~~
...
class SilentUndefinedType(AbstractUndefinedType):
    ...
    __sub__ = __mul__ = __div__ = __rsub__ = __rmul__ = __div__ = __mod__ = __radd__ = __rmod__ = __add__
...
class Context(BaseContext):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(Context, self).__init__(environment.undefined_singleton, environment.globals, base)
...
class LoopContext(object):
    ...
    """
    jinja_allowed_attributes = [
     'index', 'index0', 'length', 'parent',
    ...
    def __call__(self, seq):
        ...
        raise TemplateRuntimeError('In order to make loops callable you have to define them with the "recursive" modifier.')
        return
...
class StateTest(object):
    ...
    def expect_token(*types, **kw):
        ...
        if msg is None:
            msg = 'expected one of %s' % (', ').join([ "'%s'" % type for type in types
                                                     ])
...
class TokenStream(object):
    ...
    def read_whitespace(self):
        ...
        buf = []
        while self.current.type == 'data' and not self.current.value.strip():
            buf.append(self.current.value)
            self.next()

        if buf:
            return Token(lineno, 'data', ('').join(buf))
    ...
    def expect(self, token_type, token_value=_missing):
        ...
        if self.current.type != token_type:
            raise TemplateSyntaxError('expected token %r, got %r' % (
             token_type, self.current.type), self.current.lineno, self.filename)
        elif token_value is not _missing and self.current.value != token_value:
            raise TemplateSyntaxError('expected %r, got %r' % (
             token_value, self.current.value), self.current.lineno, self.filename)
        try:
            return self.current
        finally:
            self.next()
