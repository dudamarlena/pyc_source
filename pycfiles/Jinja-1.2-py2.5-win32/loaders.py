...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\jinja\loaders.py
# Compiled at: 2007-11-10 18:31:20
"""
    jinja.loaders
    ~~~~~~~~~~~~~
...
from jinja.exceptions import TemplateNotFound, TemplateSyntaxError, TemplateIncludeError
from jinja.utils import CacheDict
__all__ = [
 'FileSystemLoader', 'PackageLoader', 'DictLoader', 'ChoiceLoader',
...
def get_template_filename(searchpath, name):
    ...
    """
    return path.join(searchpath, *[ p for p in name.split('/') if p if p[0] != '.'
                                  ])
...
def get_cachename(cachepath, name, salt=None):
    ...
    """
    return path.join(cachepath, 'jinja_%s.cache' % sha1('jinja(%s|%s)tmpl' % (
...
class ControlledLoader(LoaderWrapper):
    ...
    def get_controlled_loader(self):
        raise TypeError('Cannot get new controlled loader from an already controlled loader.')
    ...
    def _controlled(method):
        ...
        def new_method(self, name, *args, **kw):
            if name in self._stack:
                raise TemplateIncludeError('Circular imports/extends detected.  %r appeared twice.' % name)
...
class MemcachedLoaderMixin(object):
    ...
    def __init__(self, use_memcache, memcache_time=604800, memcache_host=None, item_prefix='template/', client=None):
        ...
        if use_memcache:
            if client is None:
                try:
                    ...
                    raise RuntimeError('the %r loader requires an installed memcache module' % self.__class__.__name__)
                else:
    ...
    def load(self, environment, name, translator):
        ...
        try:
            if translator is not PythonTranslator:
                return super(MemcachedLoaderMixin, self).load(environment, name, translator)
            tmpl = None
            push_to_memory = False
            if self.__memcache is not None:
                ...
                if bytecode:
                    ...
                    push_to_memory = True
            if tmpl is None:
                tmpl = super(MemcachedLoaderMixin, self).load(environment, name, translator)
            if push_to_memory:
                self.__memcache.set(self.__item_prefix + name, tmpl.dump(), self.__memcache_time)
            return tmpl
        finally:
            self.__lock.release()
...
class FileSystemLoader(CachedLoaderMixin, BaseFileSystemLoader):
    ...
    def __init__(self, searchpath, use_memcache=False, memcache_size=40, cache_folder=None, auto_reload=True, cache_salt=None):
        BaseFileSystemLoader.__init__(self, searchpath)
        if cache_salt is None:
            cache_salt = self.searchpath
        CachedLoaderMixin.__init__(self, use_memcache, memcache_size, cache_folder, auto_reload, cache_salt)
        return
...
class MemcachedFileSystemLoader(MemcachedLoaderMixin, BaseFileSystemLoader):
    ...
    def __init__(self, searchpath, use_memcache=True, memcache_time=604800, memcache_host=None, item_prefix='template/'):
        BaseFileSystemLoader.__init__(self, searchpath)
        MemcachedLoaderMixin.__init__(self, use_memcache, memcache_time, memcache_host, item_prefix)
...
class BasePackageLoader(BaseLoader):
    ...
    def _get_load_func(self):
        ...
        try:
            ...
            def load_func(name):
                filename = path.join(basepath, *(self.package_path.split('/') + [ p for p in name.split('/') if p != '..' ]))
                if path.exists(filename):
                    f = file(filename)
                    try:
                        return f.read()
            ...
            def load_func(name):
                path = ('/').join([self.package_path] + [ p for p in name.split('/') if p != '..'
                                                        ])
...
class PackageLoader(CachedLoaderMixin, BasePackageLoader):
    ...
    def __init__(self, package_name, package_path, use_memcache=False, memcache_size=40, cache_folder=None, auto_reload=True, cache_salt=None):
    ...
    def check_source_changed(self, environment, name):
        ...
        fn = resource_filename(self.package_name, ('/').join([self.package_path] + [ p for p in name.split('/') if p if p[0] != '.' ]))
...
class FunctionLoader(CachedLoaderMixin, BaseFunctionLoader):
    ...
    def __init__(self, loader_func, getmtime_func=None, use_memcache=False, memcache_size=40, cache_folder=None, auto_reload=True, cache_salt=None):
