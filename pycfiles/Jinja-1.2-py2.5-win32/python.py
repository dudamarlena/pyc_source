...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\jinja\translators\python.py
# Compiled at: 2007-11-10 18:34:42
"""
...
_debug_re = re.compile('^\\s*\\# DEBUG\\(filename=(?P<filename>.*?), lineno=(?P<lineno>\\d+)\\)$')
try:
...
class Template(object):
    ...
    def _debug(self, ctx, exc_type, exc_value, traceback):
        ...
        if self.environment.friendly_traceback:
            ...
            (exc_type, exc_value, traceback) = translate_exception(self, ctx, exc_type, exc_value, traceback)
        print_exception(exc_type, exc_value, traceback)
...
class PythonTranslator(Translator):
    ...
    def __init__(self, environment, node, source):
        ...
        self.handlers = {nodes.Template: self.handle_template, 
    ...
    def handle_template(self, node):
        ...
        while node.extends is not None:
            requirements += [ child for child in node.body.get_child_nodes() if child.__class__ not in (nodes.Text,
             nodes.Block)
                            ]
            parent = self.loader.parse(node.extends, node.filename)
        ...
        lines = [
         '# Essential imports', 'from __future__ import division']
        if self.used_utils:
            lines.append('from jinja.utils import %s' % (', ').join(tuple(self.used_utils)))
        if self.require_runtime_error:
            lines.append('from jinja.exceptions import TemplateRuntimeError')
        if self.used_data_structures:
            lines.append('from jinja.datastructure import %s' % (', ').join(self.used_data_structures))
        ...
        lines.append('\n# Aliases for some speedup\n%s\n\n# Marker for Jinja templates\n__jinja_template__ = True\n\n# Name for disabled debugging\n__name__ = %r\n\ndef generate(context):\n    assert environment is context.environment' % (
         ('\n').join([ '%s = environment.%s' % (item, item) for item in self.used_shortcuts
            ]),
         outer_filename))
        if requirements:
        ...
        for (idx, line) in enumerate(sourcelines):
            ...
            if m is not None:
                ...
                if filename in file_mapping:
                    ...
                    file_id = file_mapping[filename] = 'F%d' % len(file_mapping)
                this = (
                 file_id, int(d['lineno']))
                if this != last:
    ...
    def handle_template_text(self, node):
        ...
        if len(node.variables) == 1 and node.text == '%s':
            return self.indent(self.nodeinfo(node)) + '\n' + self.indent('yield finish_var(%s, context)' % self.handle_node(node.variables[0]))
        buf = []
    ...
    def handle_for_loop(self, node):
        ...
        if node.recursive:
            write('def loop(seq):')
            self.indention += 1
            write("for %s in context['loop'].push(seq):" % self.handle_node(node.item))
        else:
            write("context['loop'] = loop = LoopContext(%s, context['loop'], None)" % self.handle_node(node.seq))
            write('for %s in loop:' % self.handle_node(node.item))
        self.indention += 1
        ...
        if node.recursive:
            ...
            self.indention -= 1
            write("context['loop'] = LoopContext(None, context['loop'], buffereater(loop))")
            self.used_utils.add('buffereater')
    ...
    def handle_print(self, node):
        ...
        """
        self.used_shortcuts.add('finish_var')
        return self.indent(self.nodeinfo(node)) + '\n' + self.indent('yield finish_var(%s, context)' % self.handle_node(node.expr))
    ...
    def handle_macro(self, node):
        ...
        if have_conditional_expr:
            arg_tmpl = "'%(name)s': args[%(pos)d] if argcount > %(pos)d else %(default)s"
        else:
            arg_tmpl = "'%(name)s': (argcount > %(pos)d and (args[%(pos)d],) or (%(default)s,))[0]"
        if node.arguments:
            varargs_init = "'varargs': args[%d:]" % len(node.arguments)
            write('argcount = len(args)')
            for (idx, (name, n)) in enumerate(node.arguments):
                arg_items.append(arg_tmpl % {'name': name, 
                   'pos': idx, 
                   'default': n is None and 'undefined_singleton' or self.handle_node(n)})
                if name == 'caller':
        ...
        write('context.push({%s})' % (',\n              ').join([ idx and self.indent(item) or item for (idx, item) in enumerate(arg_items)
                                                                ]))
        write('if kw:')
        self.indention += 1
        write("raise TemplateRuntimeError('%s got an unexpected keyword argument %%r' %% iter(kw).next())" % node.name)
        ...
        write('if 0: yield None')
        self.indention -= 1
        buf.append(self.indent('context[%r] = buffereater(macro, True)' % node.name))
        self.used_utils.add('buffereater')
        return ('\n').join(buf)
    ...
    def handle_call(self, node):
        ...
        write('if 0: yield None')
        self.indention -= 1
        write('yield ' + self.handle_call_expr(node.expr, {'caller': 'buffereater(call)'}))
        self.used_utils.add('buffereater')
        return ('\n').join(buf)
    ...
    def handle_filter(self, node):
        ...
        write('yield apply_filters(buffereater(filtered)(), context, %s)' % self.to_tuple([ '(%r, %s)' % (name, self.to_tuple(map(self.handle_node, args))) for (name, args) in node.filters
                                                                                          ]))
        self.used_utils.add('buffereater')
        return ('\n').join(buf)
    ...
    def handle_include(self, node):
        ...
        """
        tmpl = self.loader.parse(node.template, node.filename)
        try:
    ...
    def handle_filter_expr(self, node):
        ...
        return 'apply_filters(%s, context, %s)' % (
         self.handle_node(node.node),
         self.to_tuple([ '(%r, %s)' % (name, self.to_tuple(map(self.handle_node, args))) for (name, args) in node.filters
              ]))
    ...
    def handle_concat(self, node):
        ...
        return "u''.join(%s)" % self.to_tuple([ 'to_unicode(%s)' % self.handle_node(arg) for arg in node.args
                                              ])
    ...
    def handle_dict(self, node):
        ...
        """
        return '{%s}' % (', ').join([ '%s: %s' % (self.handle_node(key), self.handle_node(value)) for (key, value) in node.items
                                    ])
    ...
    def handle_set_expr(self, node):
        ...
        return 'set([%s])' % (', ').join([ self.handle_node(n) for n in node.items
                                         ])
    ...
    def handle_list(self, node):
        ...
        """
        return '[%s]' % (', ').join([ self.handle_node(n) for n in node.items ])
    ...
    def handle_conditional_expr(self, node):
        ...
        return tmpl % {'test': self.handle_node(node.test), 
