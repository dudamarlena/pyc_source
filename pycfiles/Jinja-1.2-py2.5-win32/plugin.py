...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\jinja\plugin.py
    ...
    def __init__(self, extra_vars_func=None, options=None):
        if 'jinja.environment' in options:
            ...
            if 'searchpath' in opt:
                opt['loader'] = FileSystemLoader(opt.pop('searchpath'), use_memcache, memcache_size, cache_folder, auto_reload)
            elif 'package' in opt:
                opt['loader'] = PackageLoader(opt.pop('package'), opt.pop('package_path', ''), use_memcache, memcache_size, cache_folder, auto_reload)
            elif loader_func is not None:
                opt['loader'] = FunctionLoader(loader_func, getmtime_func, use_memcache, memcache_size, cache_folder, auto_reload)
...
def jinja_plugin_factory(options):
    ...
    =================== =================================================
    """
    warn(DeprecationWarning('general plugin interface implementation deprecated because not an accepted standard.'))
    if 'environment' in options:
        env = options['environment']
        if not len(options) == 1:
            raise TypeError('if environment provided no other arguments are allowed')
    else:
        ...
        if 'searchpath' in options:
            options['loader'] = FileSystemLoader(options.pop('searchpath'), use_memcache, memcache_size, cache_folder, auto_reload)
        elif 'package' in options:
            options['loader'] = PackageLoader(options.pop('package'), options.pop('package_path', ''), use_memcache, memcache_size, cache_folder, auto_reload)
        elif loader_func is not None:
            options['loader'] = FunctionLoader(loader_func, getmtime_func, use_memcache, memcache_size, cache_folder, auto_reload)
