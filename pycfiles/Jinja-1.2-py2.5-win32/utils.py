...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\jinja\utils.py
# Compiled at: 2007-11-10 07:57:38
"""
    jinja.utils
    ~~~~~~~~~~~
...
_punctuation_re = re.compile('^(?P<lead>(?:%s)*)(?P<middle>.*?)(?P<trail>(?:%s)*)$' % (
...
def urlize(text, trim_url_limit=None, nofollow=False):
    ...
    for (i, word) in enumerate(words):
        ...
        if match:
            ...
            if middle.startswith('www.') or '@' not in middle and not middle.startswith('http://') and len(middle) > 0 and middle[0] in string.letters + string.digits and (middle.endswith('.org') or middle.endswith('.net') or middle.endswith('.com')):
...
def get_attribute(obj, name):
    ...
    if getattr(obj, '__class__', None) in callable_types and name.startswith('func_') or name.startswith('im_'):
...
try:
    ...
    if e.args and e.args[0] is _test_singleton:
        capture_generator = ('').join
        RUNTIME_EXCEPTION_OFFSET = 1
    else:
        capture_generator = lambda gen: ('').join(tuple(gen))
...
def collect_translations(ast):
    ...
    while todo:
        ...
        elif node.__class__ is nodes.CallExpression and node.node.__class__ is nodes.NameExpression and node.node.name == '_':
            if len(node.args) == 1 and not node.kwargs and not node.dyn_args and not node.dyn_kwargs and node.args[0].__class__ is nodes.ConstantExpression:
