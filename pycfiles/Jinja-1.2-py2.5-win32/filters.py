...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\jinja\filters.py
# Compiled at: 2007-10-21 16:14:46
"""
    jinja.filters
    ~~~~~~~~~~~~~
...
from jinja.utils import urlize, escape, reversed, sorted, groupby, get_attribute, pformat
...
def do_replace(s, old, new, count=None):
    ...
    .. sourcecode:: jinja
        ...
        {{ "aaaaargh"|replace("a", "d'oh, ", 2) }}
            -> d'oh, d'oh, aaargh
    """
    if not isinstance(old, basestring) or not isinstance(new, basestring):
        raise FilterArgumentError('the replace filter requires string replacement arguments')
    if count is None:
        return s.replace(old, new)
    if not isinstance(count, (int, long)):
        raise FilterArgumentError('the count parameter of the replace filter requires an integer')
...
def do_dictsort(case_sensitive=False, by='key'):
    ...
    def wrapped(env, context, value):
        items = value.items()
        items.sort(lambda a, b: cmp(sort_func(a[pos], env), sort_func(b[pos], env)))
        return items

    return wrapped


def do_default(default_value='', boolean=False):
    """
...
def do_round(precision=0, method='common'):
    ...
    *new in Jinja 1.1*
    ...
    if precision < 0:
        raise FilterArgumentError('precision must be a postive integer or zero.')
...
def do_groupby(attribute):
    ...
    *New in Jinja 1.2*
    ...
    def wrapped(env, context, value):
        ...
        return sorted([ {'grouper': a, 'list': list(b)} for (a, b) in groupby(sorted(value, key=expr), expr)
