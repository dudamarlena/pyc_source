...
# [GCC 8.4.0]
# Embedded file name: /Users/scottblevins/git/old impression/impression/mixin.py
# Compiled at: 2016-07-20 17:11:58
...
from impression.utils import json_dumps, camelcase_to_underscore, underscore_to_camelcase, uuid
...
def paginate(query_object, current_page, pagesize):
    ...
    result = query_object.limit(pagesize).offset((current_page - 1) * pagesize).all()
    return (
     result, max_pages)
...
def property_to_dict(model_property, **kwargs):
    ...
    elif isinstance(model_property, OurMixin):
...
class OurMixin(object):
    ...
    def doc_dict(self, camel_case=False, columns=None):
        ...
        for key in the_dict:
            cc_key = (camel_case or underscore_to_camelcase)(key) if 1 else key
