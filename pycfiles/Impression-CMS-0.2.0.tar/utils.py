...
# [GCC 8.4.0]
# Embedded file name: /Users/scottblevins/git/old impression/impression/utils.py
# Compiled at: 2016-07-20 14:22:29
import re, simplejson, hashlib, random
...
def camelcase_to_underscore(name):
    ...
    """
    return re.sub('([a-z0-9])([A-Z])', '\\1_\\2', re.sub('(.)([A-Z][a-z]+)', '\\1_\\2', name)).lower()


def json_dumps(python_object, dents=None, if_err=None):
    r"""Takes a value (list, dictionary, etc) that is JSON serializable, and
    ...
    def to_json(an_object):
        ...
        """
        from datetime import date, datetime, time
        if isinstance(an_object, date) or isinstance(an_object, datetime) or isinstance(an_object, time):
