...
# [GCC 8.4.0]
# Embedded file name: /Users/scottblevins/git/impression/impression/controllers/file.py
# Compiled at: 2016-07-22 11:00:29
import os, json
from flask import Blueprint, flash, request, redirect, url_for, send_from_directory, jsonify, g
...
def upload_file(payload, ufile):
    ...
    if ufile and allowed_file(ufile.filename):
        ...
        if not os.path.isfile(path):
            ...
            afile = File(name=filename, path=path, thumbnail_name=thumbnail_name, thumbnail_path=thumbnail_path, user_id=payload.get('user_id'), size=os.path.getsize(path), width=width, height=height, mimetype=ufile.mimetype)
            afile.insert()
            safe_commit()
            return afile.id
    return
