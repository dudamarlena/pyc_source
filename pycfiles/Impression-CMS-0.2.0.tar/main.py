...
# [GCC 8.4.0]
# Embedded file name: /Users/scottblevins/git/old impression/impression/controllers/main.py
# Compiled at: 2016-07-20 22:32:07
import json
...
from impression.controls import get_payload, render, get_setting, get_menu_items, render_admin
...
def blog_index(page=1, tag=None):
    ...
    posts = Content.filter(Content.published == True).filter(Content.type == 'post').order_by(Content.published_on.desc())
    if tag:
        posts = posts.filter(Content.tags.contains(tag))
    posts, max_pages = paginate(posts, page, limit)
    return render('index.html', user=import_user(), posts=posts, current_page=page, max_pages=max_pages, tag_chunks=tag_chunks, tag=tag, menu_items=get_menu_items())
...
def search_page():
    ...
    contents = Content.filter(or_(Content.body.ilike(('%{}%').format(search)), Content.tags.ilike(('%{}%').format(search)), Content.title.ilike(('%{}%').format(search)))).filter(Content.published == True).all()
    return render('search.html', user=import_user(), contents=contents, menu_items=get_menu_items())
...
def render_page(content_id):
    ...
    return render(content.template, user=import_user(), content=content, menu_items=get_menu_items())
...
def retrieve_content():
    ...
    if content_id:
        ...
        contents = Content.filter(Content.type == content_type).filter(Content.published == published).order_by(Content.published_on.desc())
    ...
    if user_count > 0:
        ...
        if request.method == 'POST':
            ...
            if payload.get('email') and payload.get('password'):
                user = User(username=payload.get('email'), password=payload.get('password'), firstname=payload.get('firstname'), lastname=payload.get('lastname'))
                my_role = Role(name='admin')
                my_role.add_abilities('create_users', 'delete_users', 'create_content', 'delete_content', 'upload_files', 'delete_files', 'change_settings')
                ...
                themes = [
                 'Stock Bootstrap 3', 'amelia', 'cerulean', 'cosmo', 'cyborg', 'darkly', 'flatly',
                ...
                Setting(name='allowed-extensions', vartype='list', system=True, value="['txt', 'pdf', 'png', 'jpg', 'jpeg', 'gif', 'tiff']").insert()
                Setting(name='upload-directory', vartype='str', system=True, value='uploads/').insert()
                Setting(name='max-file-size', vartype='int', system=True, value=16777216).insert()
                safe_commit()
...
def create_user():
    return ("You can only see this if you are logged in with 'create_users' permissions!",
            200)