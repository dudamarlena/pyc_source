...
# [GCC 8.4.0]
# Embedded file name: /Users/scottblevins/git/old impression/impression/controllers/admin.py
# Compiled at: 2016-07-20 22:21:04
import os, json
from datetime import datetime
from dateutil import parser
from flask import Blueprint, render_template, flash, request, redirect, jsonify, url_for, g
...
def create_user():
    ...
    user = User(username=payload.get('email'), password=payload.get('password'), name=payload.get('name'))
    valid = user.validate()
    if valid['success']:
        user.insert()
        safe_commit()
...
def admin_pages_add():
    ...
    print themes
    return render_admin('content.html', user=g.user, content_type='Page', action='Add', content=content, themes=themes)
...
def admin_pages_edit(content_id):
    ...
    themes = [ t.identifier for t in get_themes_list() if t.identifier != 'admin' ]
    return render_admin('content.html', user=g.user, content_type=content.type, action='Edit', content=content, themes=themes)
