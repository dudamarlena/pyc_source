...
# [GCC 8.4.0]
# Embedded file name: /home/travis/virtualenv/python2.7.9/lib/python2.7/site-packages/distributor/__init__.py
# Compiled at: 2016-03-22 07:07:24
try:
...
try:
    from ConfigParser import SafeConfigParser as ConfigParser, Error as parseError
except ImportError:
...
import dns.message, dns.rdatatype, dns.resolver, dns.tsig, dns.tsigkeyring
from dns import query, name as dns_name
from jinja2 import Environment, PackageLoader
from jinja2.exceptions import TemplateNotFound
from pyparsing import Literal, White, Word, alphanums, CharsNotIn, Optional, Forward, Group, ZeroOrMore, OneOrMore, QuotedString, restOfLine
from requests import Session, get as rget
from requests.exceptions import SSLError
__all__ = [
 'Distributor', 'create_html']
...
class NginxParser(object):
    ...
    def __init__(self):
        ...
        value = ZeroOrMore(CharsNotIn('{};#"\'') | space | QuotedString("'", escChar='\\', multiline=True) | QuotedString('"', escChar='\\', multiline=True))
        modifier = Literal('=') | Literal('~*') | Literal('~') | Literal('^~')
        comment = Literal('#').suppress() + Optional(restOfLine)
        assignment = Group((key | value) + value + semicolon + Optional(space + comment))
        block = Forward()
        block << Group(Group(key + Optional(space + modifier) + Optional(space) + Optional(value) + Optional(space + value)) + left_bracket + Group(ZeroOrMore(assignment | block | comment.suppress())) + right_bracket)
    ...
    def parse(self, s):
        ...
        def to_dict(o):
            ...
            for i in o:
                try:
                    d[(i[0].strip() if isinstance(i[0], (str, int)) else (' ').join(i[0]).strip())].append(i[1].strip() if isinstance(i[1], (str, int)) else to_dict(i[1]))
                except:
                    continue
...
def check_dns(domain, ns):
    ns_s = list(filter(len, map(lambda _s: _s.strip(), ns.replace('"', '').split(';'))))
    if len(ns_s) < 1:
        return {}
    mess = dns.message.make_query(dns_name.from_text(domain), dns.rdatatype.SOA)
    result = {}
    for ns in ns_s:
        try:
...
def check_txt(domain, status):
    spf, dmarc = (1, 1)
    if 'Не делегирован' in status:
        return (spf, dmarc)
    else:
        try:
            mess = dns.resolver.query(dns_name.from_text(domain), dns.rdatatype.TXT)
        except:
            spf = 1
        else:
            ...
            if not spfs:
                spf = 1
            elif '+all' in (' ').join(spfs):
                spf = 2
            else:
                spf = 0
            try:
                mess = dns.resolver.query(dns_name.from_text('_dmarc.' + domain), dns.rdatatype.TXT)
            except:
                return (
                 spf, dmarc)
...
class Distributor(object):
    ...
    def __init__(self, configs_dir, settings):
        self.services = defaultdict(lambda : defaultdict(lambda : defaultdict(set)))
        self._api = dict()
        self.changed = []
        self.authors = defaultdict(set)
        self.re_haproxy = compile('^listen\\s+?([\\w\\d\\.:\\-_]+)\\s+?(?:(?:\\s+?bind\\s+?)?((?:\\d{1,3}\\.){3}\\d{1,3}:\\d+)\\s+?)+', DOTALL | MULTILINE)
    ...
    def index(self):
        ...
        return self.tpl.get_template('index.html').render(last_sync=last_sync, categories=self.get_cats(), bind_zones=zones)
    ...
    def parse_dns(self, conf):
        ...
        for server in servers:
            if '_domainkey' in server[0]:
                continue
            priority, class_path, list_type, destination = server[1].split(' ', 3)
            self.services[cat][server[0]] = {'   Priority': [
                             priority], 
               '  Class': [
                         class_path], 
               ' Type': [
    ...
    def parse_nic(self, conf):
        ...
        for domain in domains:
            self.services['NIC'][domain['domain']] = {'    Status': [domain['status']], '   TXT': domain['txt'], 
               '  NS': domain['ns'].items(), 
               ' Till': [
                       domain['till']], 
               'Auto-renew': [
    ...
    def parse_nginx(self, conf):
        ...
        for server in servers['http']:
            ...
            ports = list(filter(lambda p: p != '80' and len(p), map(lambda l: l.split()[0].split(':')[1] if len(l.split()[0].split(':')) > 1 else '', listeners)))
            if server['server_name'] and '_' not in server['server_name'][0]:
                urls = map(lambda l: l + ':' + (',').join(ports) if len(ports) else l, server['server_name'][0].split())
            else:
                continue
            ...
            for url in urls:
                ...
                self._api['web']['servers'][url] = {'author': author, 
                   'log': log_path, 
    ...
    def parse_haproxy(self, conf):
        listeners = [ (i[0].replace('cluster.', ''), i[1]) for i in self.re_haproxy.findall(open(conf).read())
                    ]
        ...
        for listener in listeners:
            ...
            if fl:
                for pattern in ['ssh', 'sql', 'rdp', 'mail', 'http', 'ldap', 'sms']:
    ...
    def write(self, cat):
        ...
        services1 = {0: [], 1: []}
        for service in self.services[cat].keys():
            c = 0 if len(self.services[cat][service].keys()) > 1 and 'DNS' not in cat and 'NIC' not in cat else 1
            services1[c].append((
             service,
             None if c < 1 else servers.index(next(iter(self.services[cat][service].keys())))))
        ...
        services = sorted(services1[0], key=skip_www) + sorted(services1[1], key=skip_www)
        result_services = []
        for service, zone in services:
            ...
            skipped_cat = False
            try:
                for candidate in self.settings.get('git', 'skipped').split(','):
                        ...
                        if r.status_code != 200:
                            ...
                            for head_name, header in r.headers.items():
                                if 'set-cookie' in head_name:
                                    continue
                                h = list(map(lambda s: s.strip(), header.split(',')))
                                if len(h) > len(set(h)):
            ...
            for server in servers:
                ...
                if 'NIC' in cat and 'NS' in server:
                    temp_result = []
                    for ip, stat in sorted(self.services[cat][service][server]):
                    ...
                    result_service['servers'].append([ {'ip': ip} for ip in sorted(self.services[cat][service][server])
                                                     ])
                if 'NIC' in cat and 'TXT' in server:
                    if 'Не делегирован' not in ('').join(self.services[cat][service]['    Status']):
                        spf, dmarc = self.services[cat][service][server]
                        result_service['servers'].append([{'ip': 'spf ', 'spf': spf}, {'ip': 'dmarc ', 'dmarc': dmarc}])
                        if spf or dmarc:
                            errors |= True

            if cat == 'web' or cat == 'promo' or cat == 'stream':
                result_service['author'] = (' ').join(filter(len, self.authors.get(service, {})))
        ...
        return template.render(services=result_services, columns=columns, servers=servers, cat=cat)
    ...
    def fetch_nic(self):
        try:
            s = Session()
            login = s.post('https://www.nic.ru/login/manager/', data={'login': self.settings.get('nic', 'login'), 
               'client_type': 'NIC-D', 
               'password': self.settings.get('nic', 'password'), 
               'password_type': 'adm'})
            if login.status_code == 200:
                csv = s.get('https://www.nic.ru/manager/my_domains.cgi?step=srv.my_domains&view.format=csv', verify=False)
                if csv.status_code == 200:
                    csv = csv.text
                    csv = list(map(lambda info: {'domain': info[0], 'ns': check_dns(info[1], info[2]), 
                       'txt': check_txt(info[1], info[5]), 
                       'status': info[5], 
                       'sost': info[6], 
                       'till': info[7]}, map(lambda l: l.split(','), filter(len, csv.split('\n')[2:]))))
                    json.dump(csv, open(pjoin(self.configs, 'nic'), 'w'))
        except:
            logging.error("can't fetch from NIC.ru by login: %s" % self.settings.get('nic', 'login'))

    def fetch_dns(self):
        try:
            name_server = self.settings.get('dns', 'server')
            keyring = dns.tsigkeyring.from_text({self.settings.get('dns', 'tsig_name'): self.settings.get('dns', 'tsig_key')})
            for domain in self.settings.get('dns', 'domains').split(','):
                try:
                    responses = query.xfr(name_server, dns_name.from_text(domain), keyring=keyring, keyname=self.settings.get('dns', 'tsig_name'), keyalgorithm=dns.tsig.HMAC_SHA512)
                    with open(pjoin(self.configs, 'dns.' + domain), 'w') as (config):
            ...
            logging.error("can't fetch from DNS: %s" % self.settings.get('dns', 'server'))

    def fetch_git(self):
        try:
            servers = self.settings.get('git', 'servers').split(',')
            auth = {'PRIVATE-TOKEN': self.settings.get('git', 'token')}
            url = 'https://%s/api/v3/projects' % self.settings.get('git', 'host')
            for server in servers:
                try:
                    id_proj = rget('%s/%s%%2F%s' % (
                     url, self.settings.get('git', 'group'), server), headers=auth).json()['id']
                except:
                    logging.warning('Repository %s does not exists' % server)

                sha = rget('%s/%s/repository/commits' % (url, id_proj), headers=auth).json()[0]['id']
                for filepath in ['usr/local/etc/nginx/nginx.conf',
                 ...
                 'nginx.conf']:
                    try:
                        ...
                        main_file = rget('%s/%s/repository/blobs/%s' % (
                         url, id_proj, sha), params=params, headers=auth)
                        if main_file.status_code != 200:
                            continue
                        with open(pjoin(self.configs, '%s.%s.all' % (
                         splitext(basename(filepath))[0], server)), 'w') as (config):
                            main_file = main_file.text
                            for incl in findall('(?:^i|^[ \\t]+i)nclude (.+?);$', main_file, REM):
                                try:
                                    params = {'filepath': pjoin(dirname(filepath), incl)}
                                    include_file = rget('%s/%s/repository/blobs/%s' % (
                                     url, id_proj, sha), params=params, headers=auth)
                                    if include_file.status_code == 200:
                                        main_file = main_file.replace('include ' + incl + ';', include_file.text)
                                except:
                                    pass
            ...
            logging.error("can't fetch from GitLab: %s" % self.settings.get('git', 'host'))