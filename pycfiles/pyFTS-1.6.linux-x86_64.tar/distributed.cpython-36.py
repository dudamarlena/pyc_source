...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyFTS/tests/distributed.py
# Compiled at: 2018-03-03 08:52:05
...
def evaluate_individual_model(model, partitioner, train, test, window_size, time_displacement):
    ...
    return {'model':model.shortname,  'partitions':partitioner.partitions,  'order':model.order,  'rmse':_rmse, 
     'mape':_mape,  'u':_u}
...
for ct, train, test in cUtil.sliding_window(data, 300):
    for partition in np.arange(5, 100, 1):
        ...
        partitioner = partitioners.PolynomialNonStationaryPartitioner(train, tmp_partitioner, window_size=35,
          degree=1)
        for model in models:
