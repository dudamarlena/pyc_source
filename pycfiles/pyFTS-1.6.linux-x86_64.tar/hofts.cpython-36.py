...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyFTS/models/hofts.py
# Compiled at: 2019-04-24 17:51:01
...
class HighOrderFLRG(flrg.FLRG):
    __doc__ = 'Conventional High Order Fuzzy Logical Relationship Group'
...
class WeightedHighOrderFLRG(flrg.FLRG):
    __doc__ = 'Weighted High Order Fuzzy Logical Relationship Group'
...
class HighOrderFTS(fts.FTS):
    __doc__ = 'Conventional High Order Fuzzy Time Series'
    ...
    def generate_lhs_flrg(self, sample, explain=False):
        nsample = [self.partitioner.fuzzyfy(k, mode='sets', alpha_cut=(self.alpha_cut)) for k in sample]
        if explain:
        ...
        if l < self.max_lag:
            ...
            for k in np.arange(self.max_lag, l):
                ...
                for flrg in flrgs:
                    if flrg.get_key() not in self.flrgs:
                        if len(flrg.LHS) > 0:
                            ...
                            if explain:
                                self.append_log('Rule Matching', '{} -> {} (Naïve) Midpoint: {}'.format(str(flrg.LHS), flrg.LHS[(-1)], mp))
                            else:
                ...
                if mode == 'mean' or fuzzyfied:
                    ...
                    if explain:
                        self.append_log('Deffuzyfication', 'By Mean: {}'.format(final))
                else:
                    final = np.dot(midpoints, memberships) / np.nansum(memberships)
                if explain:
                    self.append_log('Deffuzyfication', 'By Memberships: {}'.format(final))
                ret.append(final)
...
class WeightedHighOrderFTS(HighOrderFTS):
    __doc__ = 'Weighted High Order Fuzzy Time Series'
