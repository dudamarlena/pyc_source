...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyFTS/models/nonstationary/nsfts.py
# Compiled at: 2018-08-06 14:39:57
# Size of source mod 2**32: 10013 bytes
import numpy as np
...
class ConventionalNonStationaryFLRG(flrg.NonStationaryFLRG):
    __doc__ = 'First Order NonStationary Fuzzy Logical Relationship Group'
...
class NonStationaryFTS(fts.FTS):
    __doc__ = 'NonStationaryFTS Fuzzy Time Series'
    ...
    def _smooth(self, a):
        return 0.1 * a[0] + 0.3 * a[1] + 0.6 * a[2]
    ...
    def train(self, data, **kwargs):
        ...
        if self.method == 'conditional':
            ...
            self.variance_residual = np.var(self.residuals)
    ...
    def _affected_sets(self, sample, perturb):
        if self.method == 'conditional':
            affected_sets = [[ct, self.sets[self._fsset_key(ct)].membership(sample, perturb[ct])] for ct in np.arange(len(self.partitioner.sets)) if self.sets[self._fsset_key(ct)].membership(sample, perturb[ct]) > 0.0]
            ...
            affected_sets = [[ct, self.sets[self._fsset_key(ct)].membership(sample, perturb)] for ct in np.arange(len(self.partitioner.sets)) if self.sets[self._fsset_key(ct)].membership(sample, perturb) > 0.0]
            if len(affected_sets) == 0:
    ...
    def forecast_interval(self, ndata, **kwargs):
        ...
        for k in np.arange(0, l):
            ...
            affected_sets = [[self.sets[key], self.sets[key].membership(ndata[k], tdisp)] for key in self.partitioner.ordered_sets if self.sets[key].membership(ndata[k], tdisp) > 0.0]
            if len(affected_sets) == 0:
