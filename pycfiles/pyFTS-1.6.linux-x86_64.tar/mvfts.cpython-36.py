...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyFTS/models/multivariate/mvfts.py
# Compiled at: 2019-04-22 09:00:28
...
class MVFTS(fts.FTS):
    ...
    def apply_transformations(self, data, params=None, updateUoD=False, **kwargs):
        ...
        for var in self.explanatory_variables:
            try:
                values = ndata[var.data_label].values
                if self.uod_clip:
                    if var.partitioner.type == 'common':
                        ndata[var.data_label] = np.clip(values, var.partitioner.min, var.partitioner.max)
    ...
    def forecast(self, data, **kwargs):
        ...
        ret = self.target_variable.apply_inverse_transformations(ret, params=(data[self.target_variable.data_label].values))
        return ret

    def forecast_ahead(self, data, steps, **kwargs):
        generators = kwargs.get('generators', None)
        if generators is None:
            raise Exception("You must provide parameter 'generators'! generators is a dict where the keys are the dataframe column names (except the target_variable) and the values are lambda functions that accept one value (the actual value of the variable)  and return the next value or trained FTS models that accept the actual values and forecast new ones.")
        ndata = self.apply_transformations(data)
        ret = []
