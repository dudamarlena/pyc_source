...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyFTS/tests/multivariate.py
# Compiled at: 2019-04-30 22:43:02
...
vhour = variable.Variable('Hour', data_label='date', partitioner=(seasonal.TimeGridPartitioner), npart=24, data=train_mv,
  partitioner_specific=sp,
  alpha_cut=0.3)
vtemp = variable.Variable('Temperature', data_label='temperature', alias='temp', partitioner=(Grid.GridPartitioner),
  npart=15,
  func=(Membership.gaussmf),
  data=train_mv,
  alpha_cut=0.3)
vload = variable.Variable('Load', data_label='load', alias='load', partitioner=(Grid.GridPartitioner),
  npart=20,
  func=(Membership.gaussmf),
  data=train_mv,
  alpha_cut=0.3)
...
for ct, train, test in cUtil.sliding_window(df, windowsize=32000, train=0.98, inc=0.05):
    print('Window {}'.format(ct))
    for order in (1, 2, 3):
        for knn in (1, 2, 3):
            model = granular.GranularWMVFTS(explanatory_variables=[vhour, vtemp, vload], target_variable=vload, order=order,
              knn=knn)
            model.fit(train)
            forecasts1 = model.predict(test, type='multivariate')
            forecasts2 = model.predict(test, type='multivariate', generators={'date': time_generator}, steps_ahead=100)
            for var in ('temperature', 'load'):
                row = [order, knn, var, len(model)]
                for horizon in (1, 25, 50, 75, 100):
                    if horizon == 1:
                        row.append(Measures.mape(test[var].values[model.order:model.order + 10], forecasts1[var].values[:10]))
                    else:
                        row.append(Measures.mape(test[var].values[:horizon], forecasts2[var].values[:horizon]))
