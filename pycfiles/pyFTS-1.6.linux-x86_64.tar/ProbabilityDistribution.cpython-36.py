...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pyFTS/probabilistic/ProbabilityDistribution.py
# Compiled at: 2018-10-29 15:49:11
# Size of source mod 2**32: 10491 bytes
...
class ProbabilityDistribution(object):
    ...
    def __init__(self, type='KDE', **kwargs):
        ...
        if self.type == 'KDE':
            ...
            _min = np.nanmin(data)
            _min = _min * 0.7 if _min > 0 else _min * 1.3
            _max = np.nanmax(data)
            _max = _max * 1.3 if _max > 0 else _max * 0.7
            self.uod = [_min, _max]
    ...
    def density(self, values):
        ...
        for k in values:
            if self.type == 'histogram':
                v = self.bin_index.find_ge(k)
                ret.append(self.distribution[v] / (self.count + 1e-05))
            elif self.type == 'KDE':
    ...
    def entropy(self):
        ...
        """
        h = -sum([self.distribution[k] * np.log(self.distribution[k]) if self.distribution[k] > 0 else 0 for k in self.bins])
        return h
    ...
    def crossentropy(self, q):
        ...
        """
        h = -sum([self.distribution[k] * np.log(q.distribution[k]) if self.distribution[k] > 0 else 0 for k in self.bins])
        return h
    ...
    def kullbackleiblerdivergence(self, q):
        ...
        """
        h = sum([self.distribution[k] * np.log(self.distribution[k] / q.distribution[k]) if self.distribution[k] > 0 else 0 for k in self.bins])
        return h
