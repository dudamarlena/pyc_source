...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/kwl.py
# Compiled at: 2016-04-23 06:47:21
"""A standalone KWL interpretor."""
from kwl2text import kwl2text
from kwl2text.generator import Generator
import kwl2text.semantics as semantics, kasahorow as k, data, logging, re, sys, text2kwl.parse_text as t2k, voice as v
...
def extract_kwl(text, tag='kwl'):
    stories = re.findall('(<%(tag)s.*?%(tag)s>)' % {'tag': tag}, text, re.S | re.M | re.DOTALL)
    return stories
...
def localize(kwl, language, lexicon={}, voice=None):
    ...
    tg = Generator(get_lc(language), language, grammar_model, ast=parse_kwl(kwl), lexicon=lexicon)
