...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/kwl2text/generator.py
# Compiled at: 2016-05-08 19:13:45
import libconjugate as lib, l10n, logging
...
class Token():
    ...
    def __str__(self):
        return 'TOKEN(%s:%s)' % (self.pos, self.fragment)
...
class Generator():
    ...
    def generate(self):
        ...
        surface = self.getHumanFromAST(self.ast)
        ...
        surface = self.make_substitutions(surface, 'punctuation')
        surface = self.single_spacing(surface)
    ...
    def command_fn(self, surface):
        return '%s!' % self.capfirst_fn(surface)
    ...
    def conjugate(self, surface, person, tense):
        ...
        try:
            ...
            cjn = 'CONJUGATION FAIL: %s with error: %s' % (surface, e)
    ...
    def two_part_fn(self, ast, parts):
        ...
        if parts[0][-5:] in ('v_p', 'tdy_p', 'ydy_p', 'tmw_p'):
            self.setSubject('')
    ...
    def noun_fn(self, noun, number=1, pos='noun', nclass=None):
        if self.language in ('english', 'inglish'):
            ...
            if number == 1:
                human = singular if '-noun' not in singular else '[%s]' % noun
            else:
    ...
    def lookup(self, word, pos, number=1, nclass=None):
        ...
        if number == 1:
            try:
                ...
                human = singular if '-%s' % pos not in singular else '[%s]' % word
            else:
        ...
        if self.language in ('english', 'inglish'):
            ...
            if number == 1:
                try:
                    ...
                    human = singular if '-adjective' not in singular else '[%s]' % adjective
    ...
    def in_p_fn(self, surface):
        new_surface = '%s<kwb>%s' % (self.preposition_fn('in'), surface)
    ...
    def like_fn(self, surface):
        """Represent like(X). """
        new_surface = '%s<kwb>%s' % (self.preposition_fn('like'), surface)
        return self.tuple_fn(new_surface, 'in_p')

    def of_fn(self, surface):
        """Join two noun phrases with the preposition 'of'. """
        words = surface.split('<kwb>')
        order = self.reorder_tuple(words, 'of_p')
    ...
    def at_fn(self, surface):
        """Represent at(X). """
        new_surface = '%s<kwb>%s' % (self.preposition_fn('at'), surface)
        return self.tuple_fn(new_surface, 'in_p')

    def to_fn(self, surface):
        """Represent to(X). """
        new_surface = '%s<kwb>%s' % (self.preposition_fn('to'), surface)
        return self.tuple_fn(new_surface, 'in_p')
    ...
    def reorder_triple(self, triple, ptype='statement'):
        ...
        if 'english' == self.language:
            if triple[0] == 'who' and ptype == 'question':
                ptype = 'who_subject_question'
            elif triple[2] == 'who' and ptype == 'question':
                ptype = 'who_object_question'
        try:
    ...
    def when_fn(self, surface):
        ...
        if len(parts) < 2:
            ...
            human = parts[0] + ' ' + sep + ' ' + (' ').join(parts[1:])
            subject = self.single_spacing(human)
        self.setSubject(subject)
        return subject
    ...
    def getPartOfSpeech(self, pos):
        POS = {'object': self.object, 
           'subject': self.subject, 
           'verb': self.verb}
        if pos in POS:
            return POS[pos]
        return '<%s not implemented>' % pos
    ...
    def get_en_date(self, number_str, cat):
        ...
        day_map = {'1': 'Sunday', 
           ...
           '7': 'Saturday'}
        if cat == 'month':
            if number_str in month_map:
                return month_map[number_str]
            return number_str
        if cat == 'day':
            if number_str in day_map:
                return day_map[number_str]
            return number_str
    ...
    def getHumanFromAST(self, ast):
        ...
        if not ast:
            ...
            if 't' in ast and 'v' in ast:
                self.operators.append(ast['t'])
                if ast['t'] in ('alpha', 'number', 'raw'):
                    _l10n = ast['v']
                elif ast['t'] == 'date':
    ...
    def translate(self, lexicon_key):
        ...
        if lexicon_key and str(lexicon_key) in TM.keys():
            translated_text = TM[lexicon_key]
        else:
            translated_text = '[%s]' % human_text
