...
# Embedded file name: build\bdist.win32\egg\pyrrd\backend\external.py
# Compiled at: 2014-01-12 21:42:12
import sys
...
def _cmd(command, args=''):
    ...
    process = Popen(command, shell=True, stdout=PIPE, stderr=PIPE, close_fds=close_fds)
...
def last(filename):
    ...
    '920806200'
    """
    return _cmd('last', filename).strip()
...
def prepareObject(function, obj):
    ...
    if function == 'update':
        ...
        if obj.values[FIRST_VALUE][DATA]:
            data = [ '%s:%s' % (time, values) for (time, values) in obj.values ]
        else:
