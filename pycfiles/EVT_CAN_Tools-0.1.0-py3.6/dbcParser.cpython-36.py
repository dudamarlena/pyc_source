...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/evtcantools/dbcparser.py
# Compiled at: 2018-03-16 13:34:34
...
__regex_pattern__ = re.compile(' SG_ (?P<name>.*) : (?P<start_bit>[0-9]{1,2})\\|(?P<length>[0-9]{1,2})@(?P<format>[0-1])(?P<type>[+-]) \\((?P<factor>.*),(?P<offset>.*)\\) \\[(?P<min>.*)\\|(?P<max>.*)\\] "(?P<unit>.*)" (?P<rx_nodes>.*)')
MOTOROLA = 0
INTEL = 1
...
class CANDatabase:
    ...
    def __init__(self, fileName):
        ...
        """
        self._dbcPath = ResourceLocator.get_locator('config/edsfiles').fetch_file_path(fileName)
        ...
        try:
            ...
            for line in file:
                ...
                if line.startswith('BU_:'):
                    ...
                    new_value = (
                     new_value_name, new_value_canID, list())
                    ...
                    for i in range(0, len(pairs), 2):
                        try:
                            ...
                            print('Invalid value: ' + new_value_name + '. Found on line ' + str(line_number) + '.')
                            return
                    ...
                    if line.startswith('BA_DEF_ BO_'):
                        ...
                        new_attr = (
                         attr_name, attr_type, attr_min, attr_max)
    ...
    def _parseSignalEntry(self, line):
        ...
        return CANSignal(name, sig_type, sig_format, start_bit, length, offset, factor, minimum, maximum, unit)
...
class CANSignal:
    ...
    def __init__(self, name, sigtype, sigformat, startbit, length, offset, factor, minVal, maxVal, unit):
        """
        Constructor.
        """
        self._name = name
