...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/redjack/web.py
# Compiled at: 2011-01-24 00:44:09
import os, re, logging, socket, BaseHTTPServer
...
class RjSrvHTTPRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def get_rest_root(self):
        ...
        if 'text/html' in self.headers['Accept']:
            ...
            data = dict(machines_groups_uri='/rest/machines_groups', projects_uri='/rest/projects')
        ...
        if 'text/html' in self.headers['Accept']:
            ...
            for mg_name in self.server.cfg.machines.get_groups():
                ...
                for m_name in self.server.cfg.machines.get_machines(mg_name):
                    ...
                    machine = dict(uri='/rest/machines_groups/%s/machines/%s' % (mg_name, m_name), name=m_name, address=m['address'], port=m['port'])
                    machines.append(machine)

                group = dict(uri='/rest/machines_groups/%s' % mg_name, name=mg_name, machines=machines)
                data.append(group)
    ...
    def get_rest_machines_group(self, name):
        ...
        if 'text/html' in self.headers['Accept']:
            ...
            for m_name in self.server.cfg.machines.get_machines(group):
                m = self.server.cfg.machines.get_machine(group, m_name)
                machine = dict(uri='/rest/machines_groups/%s/machines/%s' % (name, m_name), name=m_name, address=m['address'], port=m['port'])
                machines.append(machine)

            data = dict(uri='/rest/machines_groups/%s' % name, name=mg_name, machines=machines)
            body = json.dumps(data)
        log.info('BODY: ' + body)
        return (body, 'text/html')
    ...
    def get_rest_machine(self, group_name, name):
        ...
        if 'text/html' in self.headers['Accept']:
            ...
            data = dict(uri='/rest/machines_groups/%s/machines/%s' % (group_name, m_name), name=name, group_uri='/rest/machines_groups/%s' % group_name, group_name=group_name, address=machine['address'], port=machine['port'])
            body = json.dumps(data)
        return (
         body, 'text/html')
    ...
    def get_rest_build_lines(self):
        ...
        if 'text/html' in self.headers['Accept']:
            ...
            data = []
            for bl_name in self.server.cfg.build_lines.get_build_lines():
                data.append(dict(uri='/rest/build_lines/%s' % bl_name, name=bl_name))
    ...
    def get_root(self):
        menu = [
         ('BUILD LINES', '/build_lines'), ('SETTINGS', '/settings'),
    ...
    def get_machines_group(self, group):
        ...
        return self._handle_page('machines_group.html', "Machines' group: " + group, [], action_menu)
