...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/redjack/server.py
# Compiled at: 2011-01-23 17:31:28
from __future__ import print_function, division
from future_builtins import *
import sys, os, re, killableprocess, datetime, time, yaml, httplib, sqlite3, pprint, codecs, logging, logging.handlers, smtplib, socket, collections, threading, pickle, json
...
handler = logging.handlers.RotatingFileHandler('rjserver.log', maxBytes=1024 * 1024, backupCount=5)
...
class Job(object):
    ...
    def _pick_machine(self):
        if 'machine' not in self.job_info:
...
class Project(object):
    ...
    def __init__(self, name, initial_label, interval, quiet_period, cfg):
        ...
        crsr.execute('CREATE TABLE IF NOT EXISTS project (field TEXT UNIQUE, value TEXT)')
    ...
    def get_builds(self, crsr):
        ...
        for (build_id, version, label, start_date, end_date, state) in crsr.fetchall():
            blds.append(dict(build_id=build_id, version=version, label=label, start_date=start_date, end_date=end_date, state=state))
    ...
    def get_build(self, crsr, build):
        ...
        return dict(build_id=build_id, version=version, label=label, start_date=start_date, end_date=end_date, state=state)
    ...
    def get_build_changes(self, crsr, build):
        ...
        return dict(build_id=build_id, triggers=pickle.loads(str(triggers)))
    ...
    def get_state(self):
        data = dict(name=self.name, paused=self.paused, version=self.version, label=self.label, building=len(self._build_steps) > 0)
        return data
...
class Config(object):
    ...
    def load_config(self):
        ...
        if 'any' not in cfg['machines'] and 'localhost' not in cfg['machines']['any']:
...
class Machines(object):
    ...
    def __init__(self, connection, dblock):
        ...
        try:
            ...
            for machine in crsr.fetchall():
                self.groups[machine[4]][machine[1]] = {'id': machine[0], 'name': machine[1], 'address': machine[2], 'port': machine[3]}

        finally:
            crsr.close()
            self.dblock.release()

        if 'any' not in self.groups:
            self.add_group('any')
    ...
    def add_machine(self, group, machine):
        if group in self.groups and machine['name'] not in self.groups[group]:
            self.dblock.acquire()
            crsr = self.conn.cursor()
            try:
    ...
    def remove_machine(self, group, name):
        if group in self.groups and name not in self.groups[group]:
