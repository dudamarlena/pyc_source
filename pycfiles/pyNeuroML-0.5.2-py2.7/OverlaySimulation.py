...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyneuroml/povray/OverlaySimulation.py
# Compiled at: 2017-01-10 08:31:40
import os.path
...
def process_args():
    ...
    parser.add_argument('prefix', type=str, metavar='<network prefix>', help='Prefix for files in PovRay, e.g. use PREFIX is files are PREFIX.pov, PREFIX_net.inc, etc.')
    parser.add_argument('lems_file_name', type=str, metavar='<lems file>', help="LEMS file describing simulatin & what's recorded")
    parser.add_argument('-maxV', type=float, metavar='<maxV>', default=50.0, help='Max voltage for colour scale in mV')
    parser.add_argument('-minV', type=float, metavar='<minV>', default=-90.0, help='Min voltage for colour scale in mV')
    parser.add_argument('-startTime', type=float, metavar='<startTime>', default=0, help='Time in ms at which to start overlaying the simulation activity')
    parser.add_argument('-endTime', type=float, metavar='<endTime>', default=100, help='End time of simulation activity in ms')
    parser.add_argument('-rotations', type=float, metavar='<rotations>', default=0.5, help='Number of rotations to complete in movie')
    parser.add_argument('-skip', type=int, metavar='<skip>', default=49, help='Number of time points to skip before generating the next frame')
    parser.add_argument('-singlecell', type=str, metavar='<reference_n>', default=None, help='If this is specified, visualise activity in a single cell; dat files for each segment should be present: reference_n.0.dat, reference_n.1.dat, etc.')
    parser.add_argument('-rainbow', action='store_true', default=False, help='If this is specified, use a rainbow based colouring of the cell activity (still in development...)')
    parser.add_argument('-povrayOptions', type=str, metavar='<povrayOptions>', default=povArgs, help='Set more specific arguments for the povray command.')
...
def main(argv):
    ...
    for fi in frame_indices:
        ...
        if not args.singlecell:
            ...
            ind = maxind[0:len(maxind) - len(str(index))]
