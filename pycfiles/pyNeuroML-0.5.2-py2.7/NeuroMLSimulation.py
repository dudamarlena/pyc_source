...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyneuroml/tune/NeuroMLSimulation.py
# Compiled at: 2017-07-07 12:30:24
"""
...
class NeuroMLSimulation(object):
    ...
    def __init__(self, reference, neuroml_file, target, sim_time=1000, dt=0.05, simulator='jNeuroML', generate_dir='./', cleanup=True, nml_doc=None):
    ...
    def go(self):
        ...
        generate_lems_file_for_neuroml(self.reference, self.neuroml_file, self.target, self.sim_time, self.dt, lems_file_name=lems_file_name, target_dir=self.generate_dir, nml_doc=self.nml_doc)
        ...
        if self.simulator == 'jNeuroML':
            results = pynml.run_lems_with_jneuroml(lems_file_name, nogui=True, load_saved_data=True, plot=False, exec_in_dir=self.generate_dir, verbose=False, cleanup=self.cleanup)
        else:
            if self.simulator == 'jNeuroML_NEURON':
                results = pynml.run_lems_with_jneuroml_neuron(lems_file_name, nogui=True, load_saved_data=True, plot=False, exec_in_dir=self.generate_dir, verbose=False, cleanup=self.cleanup)
            else:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 2 and sys.argv[1] == '-net':
        sim = NeuroMLSimulation('TestNet', '../../examples/test_data/simplenet.nml', 'simplenet', sim_time, dt, 'jNeuroML', 'temp/')
        sim.go()
        sim.show()
    else:
        sim = NeuroMLSimulation('TestHH', '../../examples/test_data/HHCellNetwork.net.nml', 'HHCellNetwork', sim_time, dt, 'jNeuroML', 'temp')
        sim.go()
        sim.show()