...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyneuroml/tune/NeuroMLController.py
...
"""
import os.path, os, sys, time, shutil
from collections import OrderedDict
import pyneuroml.pynml, pprint
pp = pprint.PrettyPrinter(indent=4)

class NeuroMLController:

    def __init__(self, ref, neuroml_file, target, sim_time=1000, dt=0.05, simulator='jNeuroML', generate_dir='./', num_parallel_evaluations=1, cleanup=True):
        self.ref = ref
        ...
        if int(num_parallel_evaluations) != num_parallel_evaluations or num_parallel_evaluations < 1:
            raise Exception('Error with num_parallel_evaluations = %s\nPlease use an integer value greater than 1.' % num_parallel_evaluations)
        self.nml_doc = pyneuroml.pynml.read_neuroml2_file(neuroml_file, include_includes=True, verbose=True, already_included=[], check_validity_pre_include=True)
        self.still_included = []
        ...
        if self.num_parallel_evaluations == 1:
            ...
            for candidate_i in range(len(candidates)):
                ...
                vars = (
                 ...
                 self.dt,
                 self.simulator)
                job = job_server.submit(run_individual, vars, (), ('pyneuroml.pynml',
    ...
    def run_individual(self, sim_var, show=False, cleanup=None):
        return run_individual(sim_var, self.ref, self.neuroml_file, self.nml_doc, self.still_included, self.generate_dir, self.target, self.sim_time, self.dt, self.simulator, cleanup=self.cleanup if cleanup == None else cleanup, show=show)
...
def run_individual(sim_var, reference, neuroml_file, nml_doc, still_included, generate_dir, target, sim_time, dt, simulator, cleanup=True, show=False):
    ...
    sim = NeuroMLSimulation(reference, neuroml_file=new_neuroml_file, target=target, sim_time=sim_time, dt=dt, simulator=simulator, generate_dir=generate_dir, cleanup=cleanup, nml_doc=nml_doc)
    sim.go()
    if show:
        sim.show()
...
if __name__ == '__main__':
    ...
    cont = NeuroMLController('TestHH', '../../examples/test_data/HHCellNetwork.net.nml', 'HHCellNetwork', sim_time, dt, 'jNeuroML_NEURON', 'temp/', num_parallel_evaluations=1)
    num_cands = 4
    if '-many' in sys.argv:
        candidates = []
        ...
        data_analysis = analysis.NetworkAnalysis(v, t, analysis_var, start_analysis=0, end_analysis=sim_time)
