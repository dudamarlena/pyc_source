...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyneuroml/analysis/NML2ChannelAnalysis.py
# Compiled at: 2020-03-27 07:18:50
import sys, os, os.path, argparse, pprint, glob, re, neuroml.loaders as loaders
from pyneuroml.pynml import run_lems_with_jneuroml, print_comment, print_comment_v, read_neuroml2_file
import airspeed, matplotlib.pyplot as plt
...
def process_args():
    ...
    parser.add_argument('channelFiles', type=str, nargs='+', metavar='<NeuroML 2 Channel file>', help='Name of the NeuroML 2 file(s)')
    parser.add_argument('-v', action='store_true', default=DEFAULTS['v'], help='Verbose output')
    parser.add_argument('-minV', type=int, metavar='<min v>', default=DEFAULTS['minV'], help='Minimum voltage to test (integer, mV), default: %smV' % DEFAULTS['minV'])
    parser.add_argument('-maxV', type=int, metavar='<max v>', default=DEFAULTS['maxV'], help='Maximum voltage to test (integer, mV), default: %smV' % DEFAULTS['maxV'])
    parser.add_argument('-temperature', type=float, metavar='<temperature>', default=DEFAULTS['temperature'], help='Temperature (float, celsius), default: %sdegC' % DEFAULTS['temperature'])
    parser.add_argument('-duration', type=float, metavar='<duration>', default=DEFAULTS['duration'], help='Duration of simulation in ms, default: %sms' % DEFAULTS['duration'])
    parser.add_argument('-clampDelay', type=float, metavar='<clamp delay>', default=DEFAULTS['clampDelay'], help='Delay before voltage clamp is activated in ms, default: %sms' % DEFAULTS['clampDelay'])
    parser.add_argument('-clampDuration', type=float, metavar='<clamp duration>', default=DEFAULTS['clampDuration'], help='Duration of voltage clamp in ms, default: %sms' % DEFAULTS['clampDuration'])
    parser.add_argument('-clampBaseVoltage', type=float, metavar='<clamp base voltage>', default=DEFAULTS['clampBaseVoltage'], help='Clamp base (starting/finishing) voltage in mV, default: %smV' % DEFAULTS['clampBaseVoltage'])
    parser.add_argument('-stepTargetVoltage', type=float, metavar='<step target voltage>', default=DEFAULTS['stepTargetVoltage'], help='Voltage in mV through which to step voltage clamps, default: %smV' % DEFAULTS['stepTargetVoltage'])
    parser.add_argument('-erev', type=float, metavar='<reversal potential>', default=DEFAULTS['erev'], help='Reversal potential of channel for currents, default: %smV' % DEFAULTS['erev'])
    parser.add_argument('-scaleDt', type=float, metavar='<scale dt in generated LEMS>', default=DEFAULTS['scaleDt'], help='Scale dt in generated LEMS, default: %s' % DEFAULTS['scaleDt'])
    parser.add_argument('-caConc', type=float, metavar='<Ca2+ concentration>', default=DEFAULTS['caConc'], help='Internal concentration of Ca2+ (float, concentration in mM), default: %smM' % DEFAULTS['caConc'])
    parser.add_argument('-datSuffix', type=str, metavar='<dat suffix>', default=DEFAULTS['datSuffix'], help='String to add to dat file names (before .dat)')
    parser.add_argument('-norun', action='store_true', default=DEFAULTS['norun'], help="If used, just generate the LEMS file, don't run it")
    parser.add_argument('-nogui', action='store_true', default=DEFAULTS['nogui'], help='Supress plotting of variables and only save data to file')
    parser.add_argument('-html', action='store_true', default=DEFAULTS['html'], help='Generate a HTML page featuring the plots for the channel')
    parser.add_argument('-md', action='store_true', default=DEFAULTS['md'], help='Generate a (GitHub flavoured) Markdown page featuring the plots for the channel')
    parser.add_argument('-ivCurve', action='store_true', default=DEFAULTS['ivCurve'], help='Save currents through voltage clamp at each level & plot current vs voltage for ion channel')
...
def generate_lems_channel_analyser(channel_file, channel, min_target_voltage, step_target_voltage, max_target_voltage, clamp_delay, clamp_duration, clamp_base_voltage, duration, erev, gates, temperature, ca_conc, iv_curve, scale_dt=1, dat_suffix='', verbose=True):
    print_comment('Generating LEMS file to investigate %s in %s, %smV->%smV, %sdegC' % (
     channel, channel_file, min_target_voltage,
    ...
    model = {'channel_file': channel_file, 'includes': includes_relative, 
       'channel': channel, 
...
def make_lems_file(channel, a):
    ...
    lems_content = generate_lems_channel_analyser(channel.file, channel.id, a.min_v, a.step_target_voltage, a.max_v, a.clamp_delay, a.clamp_duration, a.clamp_base_voltage, a.duration, a.erev, gates, a.temperature, a.ca_conc, a.iv_curve, scale_dt=a.scale_dt, dat_suffix=a.dat_suffix, verbose=a.v)
...
def run_lems_file(lems_file, verbose):
    results = run_lems_with_jneuroml(lems_file, nogui=True, load_saved_data=True, plot=False, verbose=verbose)
    return results
...
def plot_kinetics(channel, a, results, grid=True):
    fig = plt.figure()
    fig.canvas.set_window_title('Time Course(s) of activation variables of %s from %s at %s degC' % (
    ...
    for g in gates:
        ...
        plt.plot([ v * 1000 for v in results[V] ], [ t * 1000 for t in results[g_tau] ], color=col, linestyle='-', label='%s %s tau' % (channel.id, g))
...
def plot_steady_state(channel, a, results, grid=True):
    fig = plt.figure()
    fig.canvas.set_window_title('Steady state(s) of activation variables of %s from %s at %s degC' % (
     channel.id, channel.file, a.temperature))
    plt.xlabel('Membrane potential (mV)')
    ...
    for g in gates:
        g_inf = 'rampCellPop0[0]/test/%s/%s/inf' % (
         channel.id, g)
        col = get_state_color(g)
        plt.plot([ v * 1000 for v in results[V] ], results[g_inf], color=col, linestyle='-', label='%s %s inf' % (channel.id, g))
...
def compute_iv_curve(channel, a, results, grid=True):
    ...
    iv_file.close()
    items = [
     'hold_v', 'times', 'currents', 'i_steady', 'i_peak']
    locals_ = locals().copy()
...
def plot_iv_curves(channel, a, iv_data, grid=True):
    ...
    plot_iv_curve(a, x['hold_v'], x['i_steady'], grid=grid, label='Steady state currents')


def plot_iv_curve_vm(channel, a, hold_v, times, currents, grid=True):
    fig = plt.figure()
    ax = plt.subplot(111)
    fig.canvas.set_window_title('Currents through voltage clamp for %s from %s at %s degC, erev: %s V' % (
    ...
    for v in hold_v:
        ...
        plt.plot([ t * 1000 for t in times[v] ], [ i * 1000000000000.0 for i in currents[v] ], color=col, linestyle='-', label='%s mV' % (v * 1000))
...
def make_iv_curve_fig(a, grid=True):
    fig = plt.figure()
    fig.canvas.set_window_title('Currents vs. holding potentials at erev = %s V' % a.erev)
    plt.xlabel('Membrane potential (mV)')
    plt.ylabel('Current (pA)')
    plt.grid('on' if grid else 'off')
...
def plot_iv_curve(a, hold_v, i, *plt_args, **plt_kwargs):
    ...
    plt.plot([ v * 1000.0 for v in hold_v ], [ ii * 1000000000000.0 for ii in i ], *plt_args, **plt_kwargs)
    plt.legend(loc=2)
...
def run(a=None, **kwargs):
    ...
    info = {'info': 'Channel information at: T = %s degC, E_rev = %s mV, [Ca2+] = %s mM' % (
