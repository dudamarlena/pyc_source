...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyneuroml/neuron/analysis/HHanalyse.py
# Compiled at: 2018-09-10 05:44:50
import argparse, re, neuron
print '\n\n'
...
def process_args():
    ...
    parser.add_argument('channel', type=str, metavar='<channel name>', help='Name of the channel as used by NEURON (i.e. in SUFFIX statement)')
    parser.add_argument('-v', action='store_true', default=False, help='Verbose output')
    parser.add_argument('-nogui', action='store_true', default=False, help='Supress plotting of variables and only save to file')
    parser.add_argument('-minV', type=int, metavar='<min v>', default=-100, help='Minimum voltage to test (integer, mV)')
    parser.add_argument('-maxV', type=int, metavar='<max v>', default=100, help='Maximum voltage to test (integer, mV)')
    parser.add_argument('-stepV', type=int, metavar='<step v>', default=10, help='Voltage step to use (integer, mV)')
    parser.add_argument('-dt', type=float, metavar='<time step>', default=0.01, help='Timestep for simulations, dt, in ms')
    parser.add_argument('-duration', type=float, metavar='<duration>', default=10000, help='Maximum duration of simulations, in ms')
    parser.add_argument('-temperature', type=str, metavar='<temperature>', default=6.3, help='Temperature (float or list, e.g. [22,34], celsius)')
    parser.add_argument('-caConc', type=float, metavar='<Ca2+ concentration>', default=5e-05, help='Internal concentration of Ca2+ (float, concentration in mM)')
    parser.add_argument('-modFile', type=str, metavar='<name of mod file>', help='Name of the mod file containing the channel')
    return parser.parse_args()


def remove_comments(txt):
    clear_txt = re.sub('(:.*)', '', txt)
    return clear_txt


def get_states(txt):
    clear_txt = remove_comments(txt)
    state_grp = re.search('(?<=STATE)\\s*\\{(?P<st_txt>[^}]+)(?=\\})', clear_txt)
    state_txt = state_grp.group('st_txt')
    state_list = []
    for state in re.finditer('(\\w+)', state_txt):
...
def get_suffix(txt):
    clear_txt = remove_comments(txt)
    nrn_str = re.search('NEURON\\s*\\{(\\s*[\\w+,]\\s*)*\\s\\}?', clear_txt).group()
    try:
        sfx_str = re.search('(?<=SUFFIX)\\s*(\\w+)', nrn_str).group(1)
...
def main():
    ...
    if not chanToTest == get_suffix(modFileTxt):
        raise AssertionError('Channel name could be wrong')
        ...
        for temperature in temperatures:
            ...
            for vh in volts:
                ...
                foundInf = []
                for s in states:
                    lastCheckVal[s] = -1e-09
                    initSlopeVal[s] = 1000000000.0
                        ...
                        for s in states:
                            ...
                            if s not in foundInf:
                                if abs((lastCheckVal[s] - val) / val) > tolerance:
                                    ...
                                    if verbose:
                                        print '  State %s has passed at %f; lastCheckVal[s] = %f; fract = %f; tolerance = %f' % (s, val, lastCheckVal[s], (lastCheckVal[s] - val) / val, tolerance)
                                    foundInf.append(s)
                                lastCheckVal[s] = val

                print '    Finished run; t: %f, v: %f, vhold: %f, initSlopeVal: %s, timeCourses: %s ---  \n' % (h.t, sec(0.5).v, vh, str(initSlopeVal), str(timeCourseVals))
                if verbose:
                    plV.plot(tRec, vRec, solid_joinstyle='round', solid_capstyle='round', color='#000000', linestyle='-', marker='None')
                for s in states:
                    col = get_state_color(s)
                    if verbose:
                        plR.plot(tRec, rateRec[s], solid_joinstyle='round', solid_capstyle='round', color=col, linestyle='-', marker='None')
