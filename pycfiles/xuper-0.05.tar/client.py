...
# [GCC 8.4.0]
# Embedded file name: /root/pythonsdk/xuper/client.py
# Compiled at: 2019-11-28 21:08:18
import time, json, random, requests, ecdsa
from ecdsa.util import sigencode_der, sigdecode_der
import base64, hashlib, binascii, codecs
...
class XuperSDK(object):
    ...
    def __format_obj(self, obj):
        ...
        for k, v in obj.items():
            if type(v) is list:
                ...
                self.__format_obj(v)
            elif k in ('txid', 'blockid', 'ref_txid', 'pre_hash', 'proposer', 'tip_blockid',
                       'root_blockid'):
    ...
    def post_tx(self, tx):
        ...
        """
        payload = {'bcname': self.bcname, 
           'header': {'logid': 'pysdk_post_tx' + str(int(time.time() * 1000000.0))}, 'txid': tx['txid'], 
           'tx': tx}
    ...
    def preexec(self, contract, method, args, module='wasm'):
        ...
        """
        payload = {'bcname': self.bcname, 
           'header': {'logid': 'pysdk_preexec' + str(int(time.time()) * 1000000.0)}, 'requests': [
                      OrderedDict([
                       (
                        'module_name', module),
                       (
    ...
    def query_tx(self, txid):
        ...
        """
        payload = {'bcname': self.bcname, 
           'header': {'logid': 'pysdk_query_tx' + str(int(time.time() * 1000000.0))}, 'txid': codecs.encode(codecs.decode(txid, 'hex'), 'base64').decode()}
    ...
    def get_block(self, blockid):
        ...
        """
        payload = {'bcname': self.bcname, 
           'header': {'logid': 'pysdk_get_block' + str(int(time.time() * 1000000.0))}, 'blockid': codecs.encode(codecs.decode(blockid, 'hex'), 'base64').decode(), 
           'need_content': True}
    ...
    def system_status(self):
        ...
        get system status
        """
        payload = {'header': {'logid': 'pysdk_system_status' + str(int(time.time() * 1000000.0))}}
    ...
    def get_block_by_height(self, height):
        ...
        """
        payload = {'bcname': self.bcname, 
           'header': {'logid': 'pysdk_get_block_by_height' + str(int(time.time() * 1000000.0))}, 'height': height}
    ...
    def deploy(self, account_name, contract_name, code, init_args, runtime='c'):
        ...
        runtime_desc = {'c': '\n\x01c', 
           'go': '\n\x02go'}
        js_init_args = go_style_dumps(OrderedDict([ (k, base64.b64encode(init_args[k]).decode()) for k in sorted(init_args.keys()) ]))
        args = {'account_name': account_name.encode(), 
    ...
    def balance(self, address=None):
        ...
        payload = {'bcs': [{'bcname': self.bcname}], 'address': address}
    ...
    def transfer(self, to_address, amount, desc='', contract_info=None):
        ...
        payload = {'bcname': self.bcname, 
           ...
           'header': {'logid': 'pysdk_' + str(int(time.time() * 1000000.0))}, 'needLock': False}
        ...
        tx['tx_outputs'].append({'amount': base64.b64encode(to_bytes(amount).lstrip('\x00')).decode(), 
           'to_addr': base64.b64encode(to_address.encode('ascii')).decode()})
        if output_return > 0:
            tx['tx_outputs'].append({'amount': base64.b64encode(to_bytes(output_return).lstrip('\x00')).decode(), 
               'to_addr': base64.b64encode(self.__my_address().encode()).decode()})
        tx['desc'] = base64.b64encode(desc.encode()).decode()
        tx['nonce'] = str(int(time.time() * 1000000.0))
        tx['timestamp'] = int(time.time() * 1000000.0)
        tx['initiator'] = self.__my_address()
        tx['auth_require'].append(self.__my_auth_require())
        tx['initiator_signs'].append({'PublicKey': self.public_key_js, 
           'Sign': ''})
        tx['auth_require_signs'].append({'PublicKey': self.public_key_js, 
           'Sign': ''})
