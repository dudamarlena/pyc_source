...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kaa/display/lcdproc.py
# Compiled at: 2009-05-21 20:15:23
__all__ = [
 'LCD']
import os, socket, logging, kaa
...
class Screen(object):
    ...
    def widget_set(self, type, id, *args):
        ...
        """
        args = list(args)
        if type == 'scroller' and len(args[(-1)]) > self.width and args[(-1)][(-1)] not in ' -_':
            args[(-1)] += '     '
...
class LCD(object):
    ...
    def __init__(self, server='127.0.0.1', port=13666):
        self.signals = {'connected': kaa.Signal()}
