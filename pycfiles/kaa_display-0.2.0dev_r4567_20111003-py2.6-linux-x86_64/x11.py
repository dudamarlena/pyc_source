...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kaa/display/x11.py
# Compiled at: 2011-08-08 09:55:16
import weakref, struct, logging, traceback, kaa
from kaa.utils import property
import _X11
log = logging.getLogger('display.x11')
_default_x11_display = None
_keysym_names = {338: 'up', 
   340: 'down', 
   337: 'left', 
   339: 'right', 
   446: 'F1', 
    ...
    def _handle_error(self, exc):
        if len(self.signals['error']) == 0:
            ...
            if isinstance(exc, X11Error):
                log.error('An untrapped X error was received: %s (serial=%ld error=%d request=%d minor=%d)', exc.strerror, exc.serial, exc.error_code, exc.request_code, exc.minor_code)
            else:
        ...
        for (event, data) in self._display.handle_events():
            ...
            if wid and wid in self._windows:
                ...
                if event == X11Display.XEVENT_CONFIGURE_NOTIFY:
                    window_events[wid] = [ x for x in window_events[wid] if x[0] != X11Display.XEVENT_CONFIGURE_NOTIFY
                                         ]
...
class X11Window(object):
    ...
    def __init__(self, display=None, window=None, **kwargs):
        ...
        self.signals = kaa.Signals('key_press_event', 'key_release_event', 'button_press_event', 'button_release_event', 'focus_in_event', 'focus_out_event', 'expose_event', 'map_event', 'unmap_event', 'resize_event', 'delete_event', 'configure_event')
        return
    ...
    def render_imlib2_image(self, i, dst_pos=(0, 0), src_pos=(0, 0), size=(-1, -1), dither=True, blend=False):
        return _X11.render_imlib2_image(self._window, i._image, dst_pos, src_pos, size, dither, blend)
    ...
    def set_fullscreen(self, fs=True):
        if not fs:
            if self._fs_size_save:
                self.resize(force=True, *self._fs_size_save)
            ...
            self.resize(force=True, *display_size)
            return
    ...
    def get_properties(self):
        ...
        for (name, type, format, n_items, data) in self._window.get_properties():
            struct_format = {'INTEGER': 'i', 
               'CARDINAL': 'L', 
               'WINDOW': 'L', 
               'STRING': 's', 
               'UTF8_STRING': 's'}.get(type, None)
            if struct_format:
