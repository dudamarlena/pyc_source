...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/wlfilebrowser/templatetags/fb_tags.py
# Compiled at: 2016-04-17 01:57:39
from __future__ import unicode_literals
from future.builtins import str
from django import template
from django.utils.http import urlquote
from wlfilebrowser.settings import SELECT_FORMATS
register = template.Library()

@register.inclusion_tag(b'wlfilebrowser/include/_response.html', takes_context=True)
...
def get_query_string(p, new_params=None, remove=None):
    ...
    return b'?' + (b'&').join([ b'%s=%s' % (urlquote(k), urlquote(v)) for k, v in p.items() ])
...
def string_to_dict(string):
    ...
    if string:
        ...
        if b',' not in string:
            string += b','
        for arg in string.split(b','):
            arg = arg.strip()
            if arg == b'':
                continue
            kw, val = arg.split(b'=', 1)
            kwargs[kw] = val
...
class SelectableNode(template.Node):
    ...
    def render(self, context):
        ...
        if filetype and format and filetype in SELECT_FORMATS[format]:
            ...
            selectable = True
        context[b'selectable'] = selectable
        return b''
...
def selectable(parser, token):
    try:
        ...
        raise TemplateSyntaxError(b'%s tag requires 2 arguments' % token.contents.split()[0])
