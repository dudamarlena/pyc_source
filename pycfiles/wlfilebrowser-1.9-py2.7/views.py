...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/wlfilebrowser/views.py
...
from wlfilebrowser.functions import get_path, get_breadcrumbs, get_filterdate, get_settings_var, get_directory, convert_filename
from wlfilebrowser.templatetags.fb_tags import query_helper
from wlfilebrowser.base import FileObject
from wlfilebrowser.decorators import flash_login_required
...
mixin_class_name = b'wlfilebrowser.storage.%sMixin' % storage_class_name
if django_settings.DEFAULT_FILE_STORAGE == b's3_folder_storage.s3.DefaultStorage':
    mixin_class_name = b'wlfilebrowser.storage.S3BotoStorageMixin'
try:
...
def remove_thumbnails(file_path):
    """
    Cleans up previous wenlincms thumbnail directories when
...
def browse(request):
    ...
    if path is None:
        ...
        return render_to_response(b'wlfilebrowser/index.html', {b'dir': path, 
           b'p': p, 
           b'page': page, 
...
wlfilebrowser_pre_createdir = Signal(providing_args=[b'path', b'dirname'])
wlfilebrowser_post_createdir = Signal(providing_args=[b'path', b'dirname'])

def mkdir(request):
    """
    Make Directory.
    """
    from wlfilebrowser.forms import MakeDirForm
    query = request.GET
    path = get_path(query.get(b'dir', b''))
    if path is None:
        msg = _(b'The requested Folder does not exist.')
        ...
        if request.method == b'POST':
            ...
            if form.is_valid():
                ...
                try:
                    wlfilebrowser_pre_createdir.send(sender=request, path=path, dirname=form.cleaned_data[b'dir_name'])
                    default_storage.makedirs(server_path)
                    wlfilebrowser_post_createdir.send(sender=request, path=path, dirname=form.cleaned_data[b'dir_name'])
        ...
        return render_to_response(b'wlfilebrowser/makedir.html', {b'form': form, 
           b'query': query, 
...
def upload(request):
    ...
    if path is None:
        ...
        return render_to_response(b'wlfilebrowser/upload.html', {b'query': query, 
...
wlfilebrowser_pre_upload = Signal(providing_args=[b'path', b'file'])
wlfilebrowser_post_upload = Signal(providing_args=[b'path', b'file'])
...
def _upload_file(request):
    ...
    Implement unicode handlers - https://github.com/sehmaschine/django-wlfilebrowser/blob/master/wlfilebrowser/sites.py#L471
    """
    if request.method == b'POST':
        folder = request.POST.get(b'folder')
        fb_uploadurl_re = re.compile(b'^.*(%s)' % reverse(b'fb_upload'))
        ...
        if request.FILES:
            ...
            wlfilebrowser_pre_upload.send(sender=request, path=request.POST.get(b'folder'), file=filedata)
            ...
            wlfilebrowser_post_upload.send(sender=request, path=request.POST.get(b'folder'), file=FileObject(smart_text(file_path)))
...
wlfilebrowser_pre_delete = Signal(providing_args=[b'path', b'filename'])
wlfilebrowser_post_delete = Signal(providing_args=[b'path', b'filename'])
...
def delete(request):
    ...
    if request.method != b'POST':
        ...
        if request.GET.get(b'filetype') != b'Folder':
            ...
            try:
                wlfilebrowser_pre_delete.send(sender=request, path=path, filename=filename)
                default_storage.delete(os.path.join(abs_path, filename))
                wlfilebrowser_post_delete.send(sender=request, path=path, filename=filename)
...
wlfilebrowser_pre_rename = Signal(providing_args=[b'path', b'filename', b'new_filename'])
wlfilebrowser_post_rename = Signal(providing_args=[b'path', b'filename', b'new_filename'])
...
def rename(request):
    ...
    """
    from wlfilebrowser.forms import RenameForm
    query = request.GET
    path = get_path(query.get(b'dir', b''))
    filename = query.get(b'filename', b'')
    if path is None or filename is None:
        ...
        if request.method == b'POST':
            ...
            if form.is_valid():
                ...
                try:
                    wlfilebrowser_pre_rename.send(sender=request, path=path, filename=filename, new_filename=new_filename)
                    remove_thumbnails(new_relative_server_path)
                    default_storage.move(relative_server_path, new_relative_server_path)
                    wlfilebrowser_post_rename.send(sender=request, path=path, filename=filename, new_filename=new_filename)
        ...
        return render_to_response(b'wlfilebrowser/rename.html', {b'form': form, 
           b'query': query, 
