...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/cmdopt.py
# Compiled at: 2011-12-13 08:53:49
import sys, os, re
...
class Schema(object):
    ...
    def add(self, options, desc, validator=None, handler=None):
        ...
        item = SchemaItem(short=short, long=long, arg=arg, required=required, attr=attr, desc=desc, options=options, validator=validator, handler=handler)
        self._register(item)
        return item

    _rexp1 = re.compile('^ *-(\\w), *--(\\w[-\\w]*)(?:=(\\S+)|\\[=(\\S+)\\])?$')
    _rexp2 = re.compile('^ *-(\\w)(?: +(\\S+)|\\[(\\S+)\\])?$')
    _rexp3 = re.compile('^ *--(\\w[-\\w]*)(?:=(\\S+)|\\[=(\\S+)\\])?$')
    ...
    def _parse_options(self, options):
        ...
        return (
         short, long, arg or arg2, bool(arg))

    def _register(self, item):
        self.items.append(item)
        if item.short:
            self._option2item['-' + item.short] = item
        if item.long:
            self._option2item['--' + item.long] = item
...
class SchemaItem(object):
    ...
    def __init__(self, short=None, long=None, arg=None, required=None, attr=None, desc=None, options=None, validator=None, handler=None):
        self.short = short
        self.long = long
        self.arg = arg
    ...
    def parse(self, args, defaults=None):
        ...
        while args:
            ...
            if arg == '--':
                ...
                self._parse_long(arg, opts)
            elif arg.startswith('-'):
                self._parse_short(arg, args, opts)
            else:
                args.insert(0, arg)
                break
