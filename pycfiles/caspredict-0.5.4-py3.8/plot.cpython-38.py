...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/caspredict/plot.py
# Compiled at: 2020-04-15 17:15:17
# Size of source mod 2**32: 14829 bytes
...
class Map(object):
    ...
    def draw_gene(self, start, end, strand, name, n, z, col):
        ...
        if isinstance(name, str):
            ...
            name = str(name)
        if z % 2 == 1:
            self.draw.text((self.scale / 50 * start + 5, n * 20 * self.scale - 3 * self.scale), (name.title()), (0,
                                                                                                                 0,
                                                                                                                 0), font=(self.font))
        else:
            self.draw.text((self.scale / 50 * start + 5, n * 20 * self.scale + 5 * self.scale), (name.title()), (0,
                                                                                                                 0,
                                                                                                                 0), font=(self.font))
    ...
    def get_longest(self, crisO, casO, criscas, crisA):
        ...
        if len(casO) > 0:
            casO_M = max(casO['End'] - casO['Start'])
        if len(criscas) > 0:
            cc1_M = max(self.criscas_len(criscas, crisA))
        ...
        try:
            ...
            if not self.noplot:
                if total > 0:
                    ...
                    y_start = self.nogrid or 8 * self.scale
