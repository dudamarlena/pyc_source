...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/store/run.py
...
import json, urllib, uuid, cbor, kvlayer, yakonfig
...
class App(yakonfig.cmd.ArgParseCmd):
    ...
    def args_load(self, p):
        p.add_argument('chunk_files', nargs='+', help='One or more feature collection chunk files.')
        p.add_argument('--id-feature', default=None, help='The name of the feature containing an id.')
        p.add_argument('--id-feature-prefix', default='', help='Add a prefix to the corresponding id.')
        p.add_argument('--batch-size', default=30, type=int, help='The number of FCs to insert at a time.')
        return

    def do_load(self, args):
        get_content_id = partial(self.get_content_id, args.id_feature_prefix, args.id_feature)
    ...
    def args_ids(self, p):
        p.add_argument('--count-features', action='store_true', default=False, help='show count of features')
        p.add_argument('--show-features', action='store_true', default=False, help='show strings from features')
        p.add_argument('--prefix-filter', help='prefix required of all content_ids returned')

    def do_ids(self, args):
        if args.show_features:
            for cid, fc in self.store.scan():
                if args.prefix_filter and not cid.startswith(args.prefix_filter):
                    continue
    ...
    def args_get(self, p):
        p.add_argument('content_id', type=str, help='The `content_id` of the feature collection to show.')
        p.add_argument('--feature-name', type=str, help='Name of a particular feature to show.')
    ...
    def args_find(self, p):
        p.add_argument('idx_name', type=str, help='The name of the feautre index.')
        p.add_argument('query', type=str, help='The `query` to find in a feature index collection to show.')
        p.add_argument('--show-features', action='store_true', default=False, help='show strings from features')
...
def pretty_string(fc):
    ...
    for fname, feature in sorted(fc.items()):
        if isinstance(feature, StringCounter):
            feature = [ '%s: %d' % (k, v) for k, v in feature.most_common() ]
            feature = '\n\t' + ('\n\t').join(feature)
        s.append(fname + ': ' + feature)
