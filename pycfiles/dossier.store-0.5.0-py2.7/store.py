...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/store/store.py
...
class Store(object):
    ...
    **Methods for indexing:**
    ...
    def __new__(cls, kvlclient, impl=None, feature_indexes=None):
        if impl is None:
            return super(Store, cls).__new__(cls, kvlclient, feature_indexes=feature_indexes)
        else:
    ...
    def put(self, items, indexes=True):
        ...
        """
        items = list(items)
        self.kvl.put(self.TABLE, *imap(lambda (cid, fc): ((cid,), fc_dumps(fc)), items))
        if indexes:
    ...
    def scan(self, *key_ranges):
        ...
        return imap(lambda (cid, fc): (cid[0], fc_loads(fc)), self.kvl.scan(self.TABLE, *key_ranges))
    ...
    def scan_prefix_ids(self, prefix):
        ...
        :rtype: generator of ``content_id``
        """
        return self.scan_ids((prefix, prefix + b'\xff'))
    ...
    def index_scan_prefix(self, idx_name, val_prefix):
        ...
        :raises: :exc:`~exceptions.KeyError`
        """
        return self._index_scan_prefix_impl(idx_name, val_prefix, lambda k: k[2])
    ...
    def index_scan_prefix_and_return_key(self, idx_name, val_prefix):
        ...
        """
        return self._index_scan_prefix_impl(idx_name, val_prefix, lambda k: (k[0], k[2]))
        ...
        for cid_fc in ids_and_fcs:
            ...
            for index_value in icreate(itrans, cid_fc):
                if index_value and index_value not in seen_values:
                    yield (
                     index_value, idx_name, content_id)
    ...
    def _index(self, name):
        ...
        try:
            ...
            raise KeyError('Index "%s" has not been registered with this FC store.' % name)
