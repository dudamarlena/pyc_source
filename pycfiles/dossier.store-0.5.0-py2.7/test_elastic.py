...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/store/tests/test_elastic.py
...
def fcs():
    return [
     ...
     (
      'patti',
      FC({'NAME': {'Patti Scialfa': 1}, 
         'boNAME': {'patti': 10, 
                    'scialfa': 1}})),
     (
      'big-man',
      FC({'NAME': {'Clarence Clemons': 8, 
                  'The Big Man': 1}, 
         'boNAME': {'clarence': 8, 
                    'clemons': 8, 
                    'the': 1, 
     ...
     ('boss',
      FC({'NAME': {'Bruce Springsteen': 2, 
         ...
         'boNAME': {'bruce': 2, 
                    ...
                    'the': 1, 
                    'boss': 1}, 
         'body': {"The screen door slams, Mary's dress sways": 1}, 'body2': {'Like a vision she dances across the porch': 1}})),
     (
      'patti',
      FC({'NAME': {'Patti Scialfa': 1}, 
         'boNAME': {'patti': 10, 
                    'scialfa': 1}, 
         'body': {'I come from down in the valley': 1}})),
     (
      'big-man',
      FC({'NAME': {'Clarence Clemons': 8, 
                  'The Big Man': 1}, 
         'boNAME': {'clarence': 8, 
                    ...
                    'big': 1, 
                    'man': 1}, 
         'body': {'Drinking warm beer in the soft summer rain': 1}}))]


def create_test_store(host, namespace):
    return ElasticStoreSync(hosts=host, namespace=NAMESPACE, type=namespace, fulltext_indexes=[
     'body'], feature_indexes=[{'NAME': {'es_index_type': 'string', 'feature_names': ['NAME']}}, {'boNAME': {'es_index_type': 'string', 'feature_names': ['boNAME']}}])
...
def test_scan_all(store, fcs):
    ...
    assert list(store.scan_ids()) == list(['big-man', 'boss', 'patti'])
...
def test_scan_all_random(store):
    ...
    ids = []
    for _ in xrange(100):
        s = ('').join([ chr(random.randrange(ord('a'), ord('z') + 1)) for _ in xrange(random.randrange(2, 20))
                      ])
        ids.append(s)
...
def test_scan_prefix(store, fcs):
    ...
    assert_set_eq(store.scan_prefix('b'), [
     ...
     (
      'big-man', fcget(fcs, 'big-man'))])
    assert frozenset(store.scan_prefix_ids('b')) == frozenset(['boss', 'big-man'])
...
def test_get_many_missing(store):
    assert frozenset(store.get_many(['boss', 'patti'])) == frozenset([('boss', None), ('patti', None)])
    return
...
def test_keyword_scan(store, fcs):
    store.put(fcs)
    assert frozenset(store.keyword_scan_ids('boss')) == frozenset(['big-man'])
...
def test_keyword_scan_partial(store, fcs):
    ...
    assert_set_eq(store.keyword_scan('boss'), [
     (
      'big-man', fcget(fcs, 'big-man'))])
    expected = FC({'NAME': {'Clarence Clemons': 8, 'The Big Man': 1}})
...
def test_keyword_scan_emphemeral(store, fcs):
    ...
    assert frozenset(store.keyword_scan_ids(query_id, query_fc)) == frozenset()
    query_fc['boNAME']['patti'] += 1
    query_fc['boNAME']['mayonnaise'] += 1
    assert frozenset(store.keyword_scan_ids(query_id, query_fc)) == frozenset(['patti'])


def test_optional_indexing(store, fcs):
    store.put(fcs)
    foo1 = FC({'NAME': {'Foo Bar': 1}, 'boNAME': {'bruce': 1, 'patti': 1, 'foo': 1, 'bar': 1}})
    foo2 = FC({'NAME': {'Foo Baz': 1}, 'boNAME': {'foo': 1, 'baz': 1}})
    store.put([('foo1', foo1), ('foo2', foo2)], indexes=False)
    assert store.get('foo1') == foo1
    assert store.get('foo2') == foo2
    assert frozenset(store.keyword_scan_ids('foo1')) == frozenset(['boss', 'patti'])
    assert frozenset(store.index_scan_ids('boNAME', 'patti')) == frozenset(['patti'])


def test_index_scan_ids(store, fcs):
    store.put(fcs)
    assert frozenset(store.index_scan_ids('boNAME', 'the')) == frozenset(['boss', 'big-man'])


def test_byte_keys(store):
    fc = FC({'NAME': {'Foo Bar': 1}})
    store.put([(b'\x00\xff\xf4', fc)])
...
def test_index_mapping_keyword(elastic_address, namespace_string, fcs):
    store = ElasticStoreSync(hosts=elastic_address, namespace=NAMESPACE, type=namespace_string, feature_indexes=[
     {'NAME': {'es_index_type': 'string', 
                 'feature_names': [
                                 'NAME', 'boNAME']}}, {'boNAME': {'es_index_type': 'string', 'feature_names': []}}])
    try:
        store.put(fcs)
        query = FC({'NAME': {'The Boss': 1, 'clarence': 1}})
        assert frozenset(store.keyword_scan_ids('ephemeral', query)) == frozenset(['boss', 'big-man'])
    finally:
        store.delete_all()
...
def test_index_mapping_raw_scan(elastic_address, namespace_string, fcs):
    ...
    try:
        store.put(fcs)
        assert frozenset(store.index_scan_ids('NAME', 'The Boss')) == frozenset(['boss'])
        assert frozenset(store.index_scan_ids('NAME', 'clarence')) == frozenset(['big-man'])
    finally:
        store.delete_all()
...
def test_fulltext_scan(store, fcs_texts):
    store.put(fcs_texts)
    query = FC({'body': {'valley': 1}})
    assert frozenset(map(itemgetter(1), store.fulltext_scan_ids(query_fc=query))) == frozenset(['patti'])
    query = FC({'body': {'in': 1}})
    assert frozenset(map(itemgetter(1), store.fulltext_scan_ids(query_fc=query))) == frozenset(['patti', 'big-man'])
    query = FC({'body': {"mary's": 1}})
    assert frozenset(map(itemgetter(1), store.fulltext_scan_ids(query_fc=query))) == frozenset(['boss'])


def test_fulltext_mapping_keyword(elastic_address, namespace_string, fcs_texts):
    store = ElasticStoreSync(hosts=elastic_address, namespace=NAMESPACE, type=namespace_string, fulltext_indexes=[
     {'body': [
               'body2']}])
    try:
        store.put(fcs_texts)
