...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/store/elastic.py
...
class ElasticStore(object):
    ...
    **Low-level**
    ...
    def __init__(self, hosts=None, namespace=None, type='fc', feature_indexes=None, shards=10, replicas=0, fulltext_indexes=None):
        ...
        """
        if hosts is None:
            raise yakonfig.ProgrammerError('ElasticStore needs at least one host specified.')
        if namespace is None:
            raise yakonfig.ProgrammerError('ElasticStore needs a namespace defined.')
        if type is None:
        ...
        if not self.conn.indices.exists(index=self.index):
            self._create_index()
        mapping = self.conn.indices.get_mapping(index=self.index, doc_type=self.type)
        if len(mapping) == 0:
    ...
    def get(self, content_id, feature_names=None):
        ...
        """
        try:
            resp = self.conn.get(index=self.index, doc_type=self.type, id=eid(content_id), _source=self._source(feature_names))
    ...
    def get_many(self, content_ids, feature_names=None):
        ...
        """
        try:
            resp = self.conn.mget(index=self.index, doc_type=self.type, _source=self._source(feature_names), body={'ids': map(eid, content_ids)})
    ...
    def put(self, items, indexes=True):
        ...
        for cid, fc in items:
            ...
            actions.append({'_index': self.index, 
               '_type': self.type, 
               '_id': eid(cid), 
               '_op_type': 'index', 
               '_source': dict(idxs, **{'fc': self.fc_to_dict(fc)})})
    ...
    def delete(self, content_id):
        ...
        """
        try:
            self.conn.delete(index=self.index, doc_type=self.type, id=eid(content_id))
        except NotFoundError:
            pass
    ...
    def delete_all(self):
        ...
        try:
            ...
            logger.warn('type %r in index %r already deleted', self.index, self.type, exc_info=True)
    ...
    def scan(self, *key_ranges, **kwargs):
        ...
        for hit in self._scan(*key_ranges, **kwargs):
            yield (
             did(hit['_id']), self.fc_from_dict(hit['_source']['fc']))
    ...
    def fulltext_scan(self, query_id=None, query_fc=None, feature_names=None, preserve_order=True):
        """Fulltext search.
        ...
        """
        it = self._fulltext_scan(query_id, query_fc, feature_names=feature_names, preserve_order=preserve_order)
        for hit in it:
            fc = self.fc_from_dict(hit['_source']['fc'])
            yield (hit['_score'], did(hit['_id']), fc)

    def fulltext_scan_ids(self, query_id=None, query_fc=None, preserve_order=True):
        ...
        """
        it = self._fulltext_scan(query_id, query_fc, feature_names=False, preserve_order=preserve_order)
        for hit in it:
            yield (
             hit['_score'], did(hit['_id']))
    ...
    def keyword_scan(self, query_id=None, query_fc=None, feature_names=None):
        ...
        :rtype: Iterable of ``(content_id, FC)``
        """
        it = self._keyword_scan(query_id, query_fc, feature_names=feature_names)
        for hit in it:
            fc = self.fc_from_dict(hit['_source']['fc'])
    ...
    def index_scan_ids(self, fname, val):
        ...
        hits = scan(self.conn, index=self.index, doc_type=self.type, query={'_source': False, 
           'query': query})
        for hit in hits:
            yield did(hit['_id'])
    ...
    def _fulltext_scan(self, query_id, query_fc, preserve_order=True, feature_names=None):
        ...
        for fname, features in self.fulltext_indexes.iteritems():
            ...
            if len(qvals) == 0:
                continue
            query = {'filtered': {'query': {'multi_match': {'query': (' ').join(qvals), 
                                                      'fields': map(fname_to_full_idx_name, features)}}, 
                            'filter': {'not': {'ids': {'values': list(ids)}}}}}
            logger.info('fulltext scanning index: %s, query: %r', fname, qvals)
            hits = scan(self.conn, index=self.index, doc_type=self.type, preserve_order=preserve_order, query={'_source': self._source(feature_names), 
               'query': query})
            for hit in hits:
                ids.add(eid(hit['_id']))
                yield hit
    ...
    def _keyword_scan(self, query_id, query_fc, feature_names=None):
        ...
        for fname in self.indexes:
            ...
            query = {'constant_score': {'filter': {'and': [
                                                   {'not': {'ids': {'values': list(ids)}}},
                                                   {'or': term_disj}]}}}
            logger.info('keyword scanning index: %s', fname)
            hits = scan(self.conn, index=self.index, doc_type=self.type, query={'_source': self._source(feature_names), 
               'query': query})
            for hit in hits:
                ids.add(eid(hit['_id']))
                yield hit
    ...
    def _scan(self, *key_ranges, **kwargs):
        ...
        return scan(self.conn, index=self.index, doc_type=self.type, _source=self._source(feature_names), preserve_order=True, query={'sort': {'_uid': {'order': 'asc'}}, 'query': {'constant_score': {'filter': {'and': range_filters}}}})

    def _scan_prefix(self, prefix, feature_names=None):
        query = {'constant_score': {'filter': {'and': [
                                               {'prefix': {'_id': eid(prefix)}}]}}}
        return scan(self.conn, index=self.index, doc_type=self.type, _source=self._source(feature_names), preserve_order=True, query={'sort': {'_uid': {'order': 'asc'}}, 'query': query})
    ...
    def _create_index(self):
        """Create the index"""
        try:
            self.conn.indices.create(index=self.index, timeout=60, request_timeout=60, body={'settings': {'number_of_shards': self.shards, 
    ...
    def _create_mappings(self):
        ...
        self.conn.indices.put_mapping(index=self.index, doc_type=self.type, timeout=60, request_timeout=60, body={self.type: {'dynamic_templates': [
                                           {'default_no_analyze_fc': {'match': 'fc.*', 
                                                                        'mapping': {'index': 'no'}}}], 
                       '_all': {'enabled': False}, 
    ...
    def _get_index_mappings(self):
        ...
        for fname in self.indexed_features:
            ...
            print(fname, config)
            maps[fname_to_idx_name(fname)] = {'type': config.get('es_index_type', 'integer'), 
               'store': False, 
               'index': 'not_analyzed'}

        for fname in self.fulltext_indexed_features:
            maps[fname_to_full_idx_name(fname)] = {'type': 'string', 'store': False, 
               'index': 'analyzed'}
    ...
    def _normalize_feature_indexes(self, feature_indexes):
        for x in feature_indexes or []:
            if isinstance(x, Mapping):
                ...
                features = [x]
                index_type = 'integer'
            self.indexes[name] = {'feature_names': features, 'es_index_type': index_type}
            for fname in features:
    ...
    def get_query_fc(self, query_id, query_fc):
        if query_fc is None:
            if query_id is None:
                raise ValueError('one of query_id or query_fc must not be None')
...
def fname_to_idx_name(fname):
    return 'idx_%s' % fname.decode('utf-8')
...
def fname_to_full_idx_name(fname):
    return 'full_idx_%s' % fname.decode('utf-8')