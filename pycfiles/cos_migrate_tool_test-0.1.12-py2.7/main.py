...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/migrate_tool/main.py
# Compiled at: 2017-03-31 09:36:01
...
log_config = {'version': 1, 
   ...
   'formatters': {'standard': {'format': '%(asctime)s [%(levelname)s] %(name)s: %(message)s'}, 
                  'error': {'format': '%(asctime)s\t%(message)s'}}, 
   'handlers': {'default': {'level': 'DEBUG', 
                            'formatter': 'standard', 
                            'class': 'logging.StreamHandler'}, 
                'error_file': {'level': 'DEBUG', 
                               'formatter': 'error', 
   ...
   'loggers': {'': {'handlers': [
                    ...
                    'level': 'DEBUG', 
                    'propagate': True}, 
               'migrate_tool.fail_file': {'handlers': [
                                                     'error_file'], 
                                          'level': 'DEBUG', 
                                          'propagate': False}, 
               'requests.packages': {'handlers': [
                                                'default'], 
                                     'level': 'DEBUG', 
                                     'propagate': True}}}
services_ = {}
