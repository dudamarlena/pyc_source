...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/migrate_tool/services/cosv4.py
    ...
    def exists(self, task):
        ...
        if ret['code'] != 0:
            ...
            if ret['data']['filelen'] != ret['data']['filesize']:
                logger.warn(('file is broken, filelen: {len}, filesize: {size}').format(len=ret['data']['filelen'], size=ret['data']['filesize']))
                return False
            if _size is not None and ret['data']['filelen'] != _size:
                return False
            if ret['data']['filelen'] != _size:
                return False
            return True