...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\getfontname\fontfile.py
# Compiled at: 2016-08-21 21:55:23
# Size of source mod 2**32: 2179 bytes
from getfontname.fontname import *
import platform, os, locale
...
class FontMapMate(type):
    ...
    def _loadFontMap():
        ...
        if platform.uname().system.upper() == 'WINDOWS':
            ...
            fontdir = [
             '/usr/share/fonts', 'usr/share/fonts/truetype', '~/.fonts']
            lang = en_us
        fonts = []
        for fdir in fontdir:
    ...
    def getFontFile(fontname, style='Regular'):
        ...
        fn = FontMapMate._FontMap.get(fn)
        if style == 'Bold':
            if not fn:
                fn = FontMapMate._FontMap.get('%s-Regular' % fontname)
        return fn
...
if __name__ == '__main__':
    ...
    ff1 = get_font_file('宋体')
    ff2 = get_font_file('SimSun')
    print('font file', ff, ff1, ff2)