...
# [GCC 8.4.0]
# Embedded file name: /var/makina/pasteStage/pasteFunBot/Paste-1.7.2-py2.6.egg/paste/urlmap.py
...
class URLMap(DictMixin):
    ...
    def not_found_app(self, environ, start_response):
        ...
        app = httpexceptions.HTTPNotFound(environ['PATH_INFO'], comment=extra).wsgi_application
    ...
    def normalize_url(self, url, trim=True):
        if isinstance(url, (list, tuple)):
            ...
            assert not url or url.startswith('/') or self.domain_url_re.search(url), 'URL fragments must start with / or http:// (you gave %r)' % url
    ...
    def __getitem__(self, url):
        ...
        raise KeyError('No application with the url %r (domain: %r; existing: %s)' % (
...
class PathProxyURLMap(object):
    ...
    def __setitem__(self, url, app):
        ...
        url = (
         url[0] or self.base_paste_url[0],
