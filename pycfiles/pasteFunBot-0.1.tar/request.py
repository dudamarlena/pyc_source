...
# [GCC 8.4.0]
# Embedded file name: /var/makina/pasteStage/pasteFunBot/Paste-1.7.2-py2.6.egg/paste/request.py
...
__all__ = [
 'get_cookies', 'get_cookie_dict', 'parse_querystring',
...
def parse_headers(environ):
    ...
    for (cgi_var, value) in environ.iteritems():
        if cgi_var in _parse_headers_special:
            yield (
             _parse_headers_special[cgi_var], value)
        elif cgi_var.startswith('HTTP_'):
            yield (
             cgi_var[5:].title().replace('_', '-'), value)
