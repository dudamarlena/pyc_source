...
# [GCC 8.4.0]
# Embedded file name: /var/makina/pasteStage/pasteFunBot/Paste-1.7.2-py2.6.egg/paste/evalexception/middleware.py
...
import pprint, itertools, time, re
...
def simplecatcher(application):
    ...
    def simplecatcher_app(environ, start_response):
        try:
            ...
            res = out.getvalue()
            return [
             '<h3>Error</h3><pre>%s</pre>' % html_quote(res)]
...
class EvalException(object):
    ...
    def debug(self, environ, start_response):
        ...
        if not method:
            exc = httpexceptions.HTTPNotFound('%r not found when parsing %r' % (
    ...
    def view(self, environ, start_response):
        ...
        if id not in self.debug_infos:
            ...
            return [
             'Traceback by id %s does not exist (maybe the server has been restarted?)' % id]
...
class DebugInfo(object):
    ...
    def eval_javascript(self):
        ...
        return '<script type="text/javascript" src="%s/mochikit/MochiKit.js"></script>\n<script type="text/javascript" src="%s/media/debug.js"></script>\n<script type="text/javascript">\ndebug_base = %r;\ndebug_count = %r;\n</script>\n' % (
...
class EvalHTMLFormatter(formatter.HTMLFormatter):
    ...
    def format_source_line(self, filename, frame):
        ...
        return line + '  <a href="#" class="switch_source" tbid="%s" onClick="return showFrame(this)">&nbsp; &nbsp; <img src="%s/_debug/media/plus.jpg" border=0 width=9 height=9> &nbsp; &nbsp;</a>' % (
