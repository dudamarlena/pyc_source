...
# [GCC 8.4.0]
# Embedded file name: /var/makina/pasteStage/pasteFunBot/Paste-1.7.2-py2.6.egg/paste/errordocument.py
    ...
    def __call__(self, environ, start_response):
        ...
        def change_response(status, headers, exc_info=None):
            ...
            if not (new_url == None or isinstance(new_url, str)):
                raise TypeError('Expected the url to internally redirect to in the StatusBasedForward mapperto be a string or None, not %s' % repr(new_url))
            if new_url:
...
class _StatusBasedRedirect(object):
    ...
    def __call__(self, environ, start_response):
        ...
        try:
            ...
            return [
             self.fallback_template % {'message': message, 'code': code}]
        else:
            if url:
                url_ = url[0]
                new_environ = {}
                ...
                try:
                    ...
                    return [
                     self.fallback_template % {'message': message, 
                        'code': code}]
                else:
