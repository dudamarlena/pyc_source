...
# [GCC 8.4.0]
# Embedded file name: /var/makina/pasteStage/pasteFunBot/Paste-1.7.2-py2.6.egg/paste/auth/cookie.py
...
class AuthCookieSigner(object):
    ...
    def sign(self, content):
        ...
        """
        cookie = base64.encodestring(hmac.new(self.secret, content, sha1).digest() + make_time(time.time() + 60 * self.timeout) + content)[:-1]
            ...
            for k in scanlist:
                ...
                if v is not None:
                    if type(v) is not str:
                        raise ValueError('The value of the environmental variable %r is not a str (only str is allowed; got %r)' % (
                         k, v))
...
__all__ = [
 'AuthCookieHandler', 'AuthCookieSigner', 'AuthCookieEnviron']
