...
# [GCC 8.4.0]
# Embedded file name: /var/makina/pasteStage/pasteFunBot/Paste-1.7.2-py2.6.egg/paste/urlparser.py
# Compiled at: 2009-07-20 09:44:04
"""
WSGI applications that parse the URL and dispatch to on-disk resources
"""
import os, sys, imp, mimetypes
try:
...
__all__ = [
 'URLParser', 'StaticURLParser', 'PkgResourcesParser']
...
class URLParser(object):
    ...
    def __init__(self, global_conf, directory, base_python_name, index_names=NoDefault, hide_extensions=NoDefault, ignore_extensions=NoDefault, constructors=None, **constructor_conf):
        ...
        if index_names is NoDefault:
            index_names = global_conf.get('index_names', ('index', 'Index', 'main',
                                                          'Main'))
        ...
        for (name, value) in constructor_conf.items():
            if not name.startswith('constructor '):
                raise ValueError("Only extra configuration keys allowed are 'constructor .ext = import_expr'; you gave %r (=%r)" % (
                 name, value))
    ...
    def __call__(self, environ, start_response):
        ...
        if not path_info:
            ...
            if self.init_module:
                if getattr(self.init_module, 'urlparser_hook', None):
                ...
                if application or self.init_module and getattr(self.init_module, 'not_found_hook', None) and environ.get('paste.urlparser.not_found_parser') is not self:
                ...
                if filename is None:
                    ...
                    if not name:
                        name = 'one of %s' % (', ').join(self.index_names or [
                         '(no index_names defined)'])
                    return self.not_found(environ, start_response, 'Tried to load %s from directory %s' % (
                     name, self.directory))
    ...
    def find_application(self, environ):
        if self.init_module and getattr(self.init_module, 'application', None) and not environ.get('paste.urlparser.init_application') == environ['SCRIPT_NAME']:
            ...
            return (
             self.init_module.application, None)
        else:
            (name, rest_of_path) = request.path_info_split(environ['PATH_INFO'])
    ...
    def not_found(self, environ, start_response, debug_message=None):
        exc = httpexceptions.HTTPNotFound('The resource at %s could not be found' % request.construct_url(environ), comment='SCRIPT_NAME=%r; PATH_INFO=%r; looking in %r; debug: %s' % (
    ...
    def add_slash(self, environ, start_response):
        ...
        exc = httpexceptions.HTTPMovedPermanently('The resource has moved to %s - you should be redirected automatically.' % url, headers=[
         (
          'location', url)])
        return exc.wsgi_application(environ, start_response)
    ...
    def register_constructor(cls, extension, constructor):
        ...
        assert not d.has_key(extension), 'A constructor already exists for the extension %r (%r) when attemption to register constructor %r' % (
...
class StaticURLParser(object):
    ...
    def __init__(self, directory, root_directory=None, cache_max_age=None):
        ...
        self.root_directory = os.path.normcase(os.path.normpath(os.path.abspath(self.root_directory)))
    ...
    def __call__(self, environ, start_response):
        ...
        if not path_info:
            ...
            full = os.path.normcase(os.path.normpath(os.path.join(self.directory, filename)))
            if os.path.sep != '/':
            ...
            if os.path.isdir(full):
                child_root = self.root_directory is not None and self.root_directory or self.directory
                return self.__class__(full, root_directory=child_root, cache_max_age=self.cache_max_age)(environ, start_response)
...
class PkgResourcesParser(StaticURLParser):
    ...
    def not_found(self, environ, start_response, debug_message=None):
        exc = httpexceptions.HTTPNotFound('The resource at %s could not be found' % request.construct_url(environ), comment='SCRIPT_NAME=%r; PATH_INFO=%r; looking in egg:%s#%r; debug: %s' % (
         environ.get('SCRIPT_NAME'), environ.get('PATH_INFO'),
