...
# [GCC 8.4.0]
# Embedded file name: smart_mirror/zomato.py
# Compiled at: 2018-09-21 08:41:58
...
class Zomato:
    ...
    def setLocation(self, location):
        ...
        if type(self).user_location:
            ...
            if 'location_suggestions' not in response:
                type(self).zomato_location = {'entity_type':'subzone', 
                 'entity_id':3410,  'title':'Kothrud, Pune',  'latitude':18.50634,  'longitude':73.806706,  'city_id':5,  'city_name':'Pune',  'country_id':1,  'country_name':'India'}
                return
            else:
                if not len(response['location_suggestions']) > 0:
                    self.location_suggestions = {'entity_type':'subzone', 
                     'entity_id':3410,  'title':'Kothrud, Pune',  'latitude':18.50634,  'longitude':73.806706,  'city_id':5,  'city_name':'Pune',  'country_id':1,  'country_name':'India'}
                    return
    ...
    def getBestRestaurants(self):
        querystring = {'entity_id':self.getLocation()['entity_id'], 
         'entity_type':self.getLocation()['entity_type']}
    ...
    def getRestaurants(self, startFrom=0):
        querystring = {'entity_id':self.getLocation()['entity_id'], 
         'entity_type':self.getLocation()['entity_type'],  'q':type(self).search_query,  'start':20 * startFrom}
    ...
    def getRestaurants_by_category(self, category, startFrom=0):
        querystring = {'entity_id':self.getLocation()['entity_id'], 
         'entity_type':self.getLocation()['entity_type'],  'q':type(self).search_query,  'category':category,  'start':20 * startFrom}
        print(querystring)
        response = requests.request('GET', (url + 'search'), headers=headers, params=querystring)
        restaurants = response.json()['restaurants']
        print(restaurants)
    ...
    def getRestaurants_by_establishment(self, establishment, startFrom=0):
        querystring = {'entity_id':self.getLocation()['entity_id'], 
         'entity_type':self.getLocation()['entity_type'],  'q':type(self).search_query,  'establishment_type':establishment,  'start':20 * startFrom}
        print(querystring)
        response = requests.request('GET', (url + 'search'), headers=headers, params=querystring)
        restaurants = response.json()['restaurants']
        return restaurants
    ...
    def getRestaurants_by_cuisine(self, cuisine, startFrom=0):
        querystring = {'entity_id':self.getLocation()['entity_id'], 
         'entity_type':self.getLocation()['entity_type'],  'q':type(self).search_query,  'cuisines':cuisine,  'start':20 * startFrom}
        print(querystring)
        response = requests.request('GET', (url + 'search'), headers=headers, params=querystring)
        restaurants = response.json()['restaurants']
        return restaurants
    ...
    def getRestaurants_by_collection(self, collection, startFrom=0):
        querystring = {'entity_id':self.getLocation()['entity_id'], 
         'entity_type':self.getLocation()['entity_type'],  'q':type(self).search_query,  'collection_id':collection,  'start':20 * startFrom}
        print(querystring)
        response = requests.request('GET', (url + 'search'), headers=headers, params=querystring)
        restaurants = response.json()['restaurants']
        return restaurants