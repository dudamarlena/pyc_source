...
from Products.PluggableAuthService.plugins.CookieAuthHelper import CookieAuthHelper as BasePlugin
from Products.PluggableAuthService.utils import classImplements
from Products.PluggableAuthService.interfaces.authservice import IPluggableAuthService
from Products.PluggableAuthService.interfaces.plugins import IRolesPlugin, IExtractionPlugin, IAuthenticationPlugin, IPropertiesPlugin, IUserEnumerationPlugin
from Products.CMFPlone.utils import log
from Products.AnonPAS.config import *

def manage_addAnonCookiePlugin(self, id, title='', RESPONSE=None, **kw):
...
class AnonCookiePlugin(BasePlugin):
    ...
    """
    __module__ = __name__
    _properties = ({'id': 'title', 'label': 'Title', 'type': 'string', 'mode': 'w'}, {'id': 'cookie_name', 'label': 'Cookie Name', 'type': 'string', 'mode': 'w'}, {'id': 'anon_roles', 'label': 'Anonymous Roles', 'type': 'lines', 'mode': 'w'})
        ...
        if exact_match and userid:
            if userid.startswith('anon_'):
                results = [
                 userid]
        for usr in results:
            val = {'id': usr, 'login': usr, 'pluginid': self.getId(), 'fullname': usr}
            return_list += (val,)

        return return_list


classImplements(AnonCookiePlugin, IRolesPlugin, IExtractionPlugin, IAuthenticationPlugin, IPropertiesPlugin, IUserEnumerationPlugin)
