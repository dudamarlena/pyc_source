...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/dhcpkit_vpp/vpp_papi.py
# Compiled at: 2017-06-20 18:34:39
# Size of source mod 2**32: 23442 bytes
from __future__ import print_function
import atexit, collections, glob, json, logging, struct, sys, threading
if sys.version[0] == '2':
...
class VPP:
    ...
    class ContextId(object):
        __doc__ = 'Thread-safe provider of unique context IDs.'
    ...
    def __struct(self, t, n=None, e=-1, vl=None):
        ...
        if t in base_types:
            ...
            if not vl:
                ...
                if e > 0:
                    return [
                     e, struct.Struct('>' + base_types[t])]
                if e == 0:
                    return [
                     -1, struct.Struct('>' + base_types[t])]
            else:
                if t == 'u8':
                    return [vl, struct.Struct('>s')]
                else:
                    return [
        ...
        if t in self.messages:
            if e > 0 and not vl:
                return [e,
                 lambda self, encode, buf, offset, args: self._VPP__struct_type(encode, self.messages[t], buf, offset, args)]
            if vl:
                return [vl,
                 lambda self, encode, buf, offset, args: self._VPP__struct_type(encode, self.messages[t], buf, offset, args)]
            if e == 0:
                raise NotImplementedError(1, 'No support for compound types ' + t)
            return lambda self, encode, buf, offset, args: self._VPP__struct_type(encode, self.messages[t], buf, offset, args)
    ...
    def __struct_type(self, encode, msgdef, buf, offset, kwargs):
        ...
        if encode:
            return self._VPP__struct_type_encode(msgdef, buf, offset, kwargs)
        else:
            return self._VPP__struct_type_decode(msgdef, buf, offset)
            ...
            if k in kwargs:
                if type(v) is list:
                    if callable(v[1]):
                        ...
                        size = 0
                        for i in range(e):
                            size += v[1](self, True, buf, off + size, kwargs[k][i])
                ...
                size = v.size if type(v) is not list else 0
    ...
    def encode(self, msgdef, kwargs):
        ...
        offset = 0
        size = self._VPP__struct_type(True, msgdef, buf, offset, kwargs)
        return buf[:offset + size]

    def decode(self, msgdef, buf):
        return self._VPP__struct_type(False, msgdef, buf, 0, None)[1]
    ...
    def __struct_type_decode(self, msgdef, buf, offset):
        ...
        return (
         off + size - offset, msgdef['return_tuple']._make(res))
    ...
    def add_message(self, name, msgdef, typeonly=False):
        ...
        for i, f in enumerate(msgdef):
            if type(f) is dict:
                ...
                field_name = f[1]
                if len(f) == 3:
                    if f[2] == 0 and i != len(msgdef) - 2:
                        raise ValueError('Variable Length Array must be last: ' + name)
                args[field_name] = self._VPP__struct(*f)
        ...
        msg['return_tuple'] = collections.namedtuple(name, fields, rename=True)
        ...
        for name, msgdef in vpp_iterator(self.messages):
            ...
            if i > 0:
                ...
                if hasattr(self, name):
                    raise NameError(3, "Conflicting name in JSON definition: `%s'" % name)
                setattr(self, name, f)
            else:
    ...
    def connect_internal(self, name, msg_handler, chroot_prefix, rx_qlen, async):
        rv = vpp_api.vac_connect(name.encode(), chroot_prefix.encode(), msg_handler, rx_qlen)
        if rv != 0:
        ...
        self.control_ping_index = vpp_api.vac_get_msg_index(('control_ping_' + crc[2:]).encode())
    ...
    def connect(self, name, chroot_prefix=ffi.NULL, async=False, rx_qlen=32):
        ...
        return self.connect_internal(name, msg_handler, chroot_prefix, rx_qlen, async)
    ...
    def connect_sync(self, name, chroot_prefix=ffi.NULL, rx_qlen=32):
        ...
        client and server.
        """
        return self.connect_internal(name, ffi.NULL, chroot_prefix, rx_qlen, async=False)
    ...
    def _control_ping(self, context):
        """Send a ping command."""
        self._call_vpp_async(self.control_ping_index, self.control_ping_msgdef, context=context)
