...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/dhcpkit_vpp/tests/protocols/test_layer2.py
# Compiled at: 2017-06-08 14:12:48
# Size of source mod 2**32: 4710 bytes
"""
...
class Layer2FrameTestCase(FrameTestCase):
    ...
    def setUp(self):
        ...
        self.packet_fixture = bytes.fromhex('0123456789abcdef0123456786dd6efbcdef001811fd200109e0000000000000000000032002200109e0000400320212004500320222123456780018a695') + b'Demo UDP packet!'
        self.message_fixture = Ethernet(destination=bytes.fromhex('0123456789ab'), source=bytes.fromhex('cdef01234567'), ethertype=int('86dd', 16), payload=IPv6(traffic_class=239, flow_label=773615, next_header=17, hop_limit=253, source=IPv6Address('2001:9e0::3:2002'), destination=IPv6Address('2001:9e0:4:32:212:45:32:0222'), payload=UDP(source_port=4660, destination_port=22136, checksum=42645, payload=b'Demo UDP packet!')))
        self.parse_packet()
    ...
    def test_ethernet_length(self):
        with self.assertRaisesRegex(ValueError, '14 bytes'):
            Ethernet.parse(bytes.fromhex('00000000000000000000000000'))
        Ethernet.parse(bytes.fromhex('0000000000000000000000000000'))

    def test_l3_payload_type(self):
        message = Ethernet(payload=UnknownLayer3Packet(b'1234'))
        packet = bytes.fromhex('000000000000000000000000000031323334')
    ...
    def test_unknown_payload_type(self):
        message = Ethernet(payload=UnknownProtocolElement(b'1234'))
        packet = bytes.fromhex('000000000000000000000000000031323334')
        self.assertEqual(message.save(), packet)
