...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\VisionEgg\TCPController.py
# Compiled at: 2009-07-21 15:02:00
"""
...
"""
import VisionEgg, VisionEgg.Core, VisionEgg.FlowControl, VisionEgg.ParameterTypes as ve_types, socket, select, re, string, types, numpy.oldnumeric as Numeric, math
try:
    import Tkinter
except:
    pass
...
class TCPServer():
    ...
    def __init__(self, hostname='', port=7834, single_socket_but_reconnect_ok=0, dialog_ok=1, confirm_address_with_gui=1):
        """Bind to hostname and port, but don't listen yet.

        """
        server_address = (
         socket.getfqdn(hostname), port)
        ...
        class GetServerAddressWindow(Tkinter.Frame):
            ...
            def __init__(self, server_address, **kw):
                ...
                self.port.set(port)
                row = 0
                Tkinter.Label(self, text='Please enter the hostname and port you would like to listen for connections on.').grid(row=row, columnspan=2)
                row += 1
                Tkinter.Label(self, text='Hostname (blank means localhost):').grid(row=row, column=0, sticky=Tkinter.E)
                ...
                row += 1
                b = Tkinter.Button(self, text='Bind port and listen for connections', command=self.click_ok)
    ...
    def create_listener_once_connected(self, eval_frequency=None):
        ...
        if self.dialog_ok:
            ...
            class WaitingDialog(Tkinter.Frame):
                ...
                def __init__(self, server_socket=None, **kw):
                    ...
                    spacer.pack()
                    Tkinter.Label(spacer, text='Awaiting connection to TCP Server at "%s", port %d' % (fqdn, port)).pack()
        ...
        if self.single_socket_but_reconnect_ok:
            return SocketListenController(client, disconnect_ok=1, server_socket=self.server_socket)
        else:
...
class SocketListenController(VisionEgg.FlowControl.Controller):
    ...
    """
    help_string = '    TCP commands (sent over network socket):\n\n    close -- close the connection\n    exit -- close the connection\n    quit -- quit the server program\n    help -- print this message\n    <name> -- show the value of the controller of <name>\n    <name>=const(<args>) -- assign a new ConstantController to <name>\n    <name>=eval_str(<args>) -- assign a new EvalStringController to <name>\n    <name>=exec_str(<args>) -- assign a new ExecStringController to <name>\n    <name>=exec_str(*<args>) -- assign a new unrestricted namespace ExecStringController to <name>\n\n    TCP commands are always on a single line.  (Newlines in string\n    literals can be specified by using "\\n" without the quotes.)\n\n    The assignment commands share common behavior:\n\n    <name> -- value passed as argument "tcp_name" to method create_tcp_controller\n    <args> -- during_go [, between_go [, eval_frequency [, temporal_variables [, return_type ]]]]\n    '
    _re_line = re.compile('^(.*)\\n', re.MULTILINE)
    _re_x_finder = re.compile('\\A|\\Wx\\s?=[^=]')
    _re_const = re.compile('^const\\(\\s?(.*)\\s?\\)$', re.DOTALL)
    _re_eval_str = re.compile('^eval_str\\(\\s?(.*)\\s?\\)$', re.DOTALL)
    _re_exec_str = re.compile('^exec_str\\(\\s?(\\*)?\\s?(.*)\\s?\\)$', re.DOTALL)
    _parse_args_globals = {'types': types}
    _parse_args_locals = VisionEgg.FlowControl.Controller.flag_dictionary

    def __init__(self, socket, disconnect_ok=0, server_socket=None, temporal_variables=VisionEgg.FlowControl.Controller.TIME_INDEPENDENT, eval_frequency=VisionEgg.FlowControl.Controller.EVERY_FRAME):
        """Instantiated by TCPServer."""
        VisionEgg.FlowControl.Controller.__init__(self, return_type=types.NoneType, temporal_variables=temporal_variables, eval_frequency=eval_frequency)
        self.socket = socket
    ...
    def __check_socket(self):
        if self.socket is not None:
            ...
            if new_info:
                self.buffer = string.replace(self.buffer, chr(13), '')
                self.buffer = string.replace(self.buffer, chr(10), '\n')
    ...
    def create_tcp_controller(self, tcp_name=None, initial_controller=None, require_type=None):
        ...
        if initial_controller is None:
            initial_controller = VisionEgg.FlowControl.ConstantController(during_go_value=1.0, between_go_value=0.0)
        elif not isinstance(initial_controller, VisionEgg.FlowControl.Controller):
        ...
        self.last_command[tcp_name] = None
        controller = TCPController(tcp_name=tcp_name, initial_controller=initial_controller)
        name_re_str = re.compile('^' + tcp_name + '\\s*=\\s*(.*)\\s*\\n$', re.MULTILINE)
    ...
    def __get_five_args(self, arg_string):
        ...
        if num_args == 0:
            ...
            args = (
             args[0], args[1], args[2], None, None)
        elif num_args == 4:
            args = (
        ...
        if match is not None:
            ...
            if match is not None:
                try:
                    match_groups = self.__get_five_args(match.group(1))
                    kw_args = {}
                    kw_args['during_go_eval_string'] = string.replace(match_groups[0], '\\n', '\n')
                    if match_groups[1] is not None:
                        kw_args['between_go_eval_string'] = string.replace(match_groups[1], '\\n', '\n')
                    self.__process_common_args(kw_args, match_groups)
                    kw_args.setdefault('return_type', require_type)
                ...
                if match is not None:
                    try:
                        ...
                        tmp = string.replace(match_groups[0], '\\n', '\n')
                        ...
                        if match_groups[1] is not None:
                            tmp = string.replace(match_groups[1], '\\n', '\n')
                            if not SocketListenController._re_x_finder.match(tmp):
