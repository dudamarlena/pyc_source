...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\VisionEgg\SphereMap.py
# Compiled at: 2009-07-07 11:29:42
"""
Stimuli on spheres, including texture maps.

"""
import math, types, logging, VisionEgg.Core, VisionEgg.Textures, VisionEgg.Text, VisionEgg.Gratings, VisionEgg.ThreeDeeMath, VisionEgg.ParameterTypes as ve_types, numpy, numpy.oldnumeric as Numeric, Image, VisionEgg.GL as gl
...
class AzElGrid(VisionEgg.Core.Stimulus):
    ...
    parameters_and_defaults = {'on': (
       ...
       'anti_aliasing': (
                       True,
                       ve_types.Boolean)}
    constant_parameters_and_defaults = {'use_text': (
                  True,
                  ve_types.Boolean), 
       'radius': (
                1.0,
       ...
       'text_anchor': (
                     'lowerleft',
                     ve_types.String)}
    __slots__ = ('cached_minor_lines_display_list', 'cached_major_lines_display_list',
                 'text_viewport', 'text_viewport_orig', '_gave_alpha_warning', 'labels',
                 'labels_xyz')

    def __init__(self, **kw):
        VisionEgg.Core.Stimulus.__init__(self, **kw)
        self.cached_minor_lines_display_list = gl.glGenLists(1)
        self.cached_major_lines_display_list = gl.glGenLists(1)
        ...
        if cp.use_text:
            ...
            for el in els_major:
                for az in azs_major:
                    ...
                    self.labels.append(VisionEgg.Text.Text(text='%.0f, %.0f' % (az, el), font_size=cp.font_size, color=cp.text_color, anchor=cp.text_anchor))
                    if el == -90 or el == 90:
        ...
        if p.on:
            ...
            if p.anti_aliasing:
                if len(p.minor_line_color) == 4 and not self._gave_alpha_warning:
                    if p.minor_line_color[3] != 1.0:
                        ...
                        logger.warning('The parameter anti_aliasing is set to true in the AzElGrid stimulus class, but the color parameter specifies an alpha value other than 1.0.  To acheive the best anti-aliasing, ensure that the alpha value for the color parameter is 1.0.')
            ...
            if cp.use_text:
                ...
                if self.text_viewport is None or self.text_viewport_orig != my_view:
                    ...
                    self.text_viewport = VisionEgg.Core.Viewport(screen=vp.screen, position=vp.position, size=vp.size, anchor=vp.anchor)
                ...
                try:
                    ...
                    for i in range(len(self.labels)):
                        ...
                        label.parameters.position = (
                         this_pos[0] + p.text_offset[0],
...
class SphereMap(VisionEgg.Textures.TextureStimulusBaseClass):
    ...
    parameters_and_defaults = {'on': (
       ...
       'stacks': (
                30,
                ve_types.UnsignedInteger)}
    __slots__ = ('cached_display_list', '_cached_radius', '_cached_slices', '_cached_stacks')

    def __init__(self, **kw):
        VisionEgg.Textures.TextureStimulusBaseClass.__init__(self, **kw)
        self.cached_display_list = gl.glGenLists(1)
...
class SphereGrating(VisionEgg.Gratings.LuminanceGratingCommon):
    ...
    parameters_and_defaults = {'on': (
       ...
       'orientation': (
                     0.0,
                     ve_types.Real), 
       'grating_center_azimuth': (
                                0.0,
                                ve_types.Real), 
       'grating_center_elevation': (
                                  0.0,
                                  ve_types.Real), 
       'check_texture_size': (
                            True,
       ...
       'stacks': (
                30,
                ve_types.UnsignedInteger)}
    __slots__ = ('texture_object_id', 'cached_display_list_id', '_cached_num_samples',
                 '_cached_radius', '_cached_slices', '_cached_stacks')
    ...
    def __init__(self, **kw):
        ...
        self.cached_display_list_id = gl.glGenLists(1)
        self.__rebuild_display_list()
        return
    ...
    def __rebuild_texture_object(self):
        ...
        while this_mipmap_level_num_samples >= 1:
            inc = 360.0 / float(this_mipmap_level_num_samples)
            ...
            if p.check_texture_size:
                gl.glTexImage1D(gl.GL_PROXY_TEXTURE_1D, mipmap_level, self.gl_internal_format, this_mipmap_level_num_samples, 0, self.format, self.gl_type, texel_data)
            ...
            gl.glTexImage1D(gl.GL_TEXTURE_1D, mipmap_level, self.gl_internal_format, this_mipmap_level_num_samples, 0, self.format, self.gl_type, texel_data)
    ...
    def draw(self):
        ...
        if p.on:
            ...
            while this_mipmap_level_num_samples >= 1:
                ...
                gl.glTexSubImage1D(gl.GL_TEXTURE_1D, mipmap_level, 0, this_mipmap_level_num_samples, self.format, self.gl_type, texel_data)
                this_mipmap_level_num_samples = this_mipmap_level_num_samples / 2
                mipmap_level += 1
...
class SphereWindow(VisionEgg.Gratings.LuminanceGratingCommon):
    ...
    parameters_and_defaults = {'on': (
            True,
            ve_types.Boolean), 
       'window_center_elevation': (
                                 0.0,
                                 ve_types.Real), 
       'window_center_azimuth': (
                               0.0,
                               ve_types.Real), 
       'opaque_color': (
                      (0.5, 0.5, 0.5, 0.0),
       ...
       'stacks': (
                30,
                ve_types.UnsignedInteger)}
    __slots__ = ('texture_object_id', 'windowed_display_list_id', 'opaque_display_list_id',
                 '_cached_window_shape', '_cached_shape_radius_parameter', '_cached_shape_parameter2',
                 '_cached_num_s_samples', '_cached_num_t_samples', '_cached_radius',
                 '_cached_slices', '_cached_stacks', '_texture_s_is_azimuth')
    ...
    def __init__(self, **kw):
        ...
        self.texture_object_id = gl.glGenTextures(1)
        self.__rebuild_texture_object()
        self.windowed_display_list_id = gl.glGenLists(1)
        self.opaque_display_list_id = gl.glGenLists(1)
        self.__rebuild_display_lists()
    ...
    def __rebuild_texture_object(self):
        ...
        if p.window_shape == 'circle':
            ...
            angle_deg = min(180, p.window_shape_radius_parameter)
            ...
            angle_deg = min(90, p.window_shape_parameter2 * 0.5)
        ...
        gl.glTexImage2D(gl.GL_PROXY_TEXTURE_2D, 0, self.gl_internal_format, p.num_s_samples, p.num_t_samples, 0, self.format, self.gl_type, texel_data)
        ...
        gl.glTexImage2D(gl.GL_TEXTURE_2D, 0, self.gl_internal_format, p.num_s_samples, p.num_t_samples, 0, self.format, self.gl_type, texel_data)
    ...
    def __rebuild_display_lists(self):
        ...
        for stack in range(p.stacks):
            ...
            y_lower = p.radius * math.cos(theta_lower)
            w_lower = p.radius * math.sin(theta_lower)
            for slice in range(p.slices / 2, p.slices):
                slice_start_frac = float(slice) / p.slices
                slice_stop_frac = float(slice + 1) / p.slices
                phi_start = slice_start_frac * 2 * math.pi
                phi_stop = slice_stop_frac * 2 * math.pi
