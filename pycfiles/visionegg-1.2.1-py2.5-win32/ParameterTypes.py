...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\VisionEgg\ParameterTypes.py
# Compiled at: 2009-07-07 11:29:42
"""
Type checking for the Vision Egg.

"""
import VisionEgg, types, warnings, numpy, numpy.oldnumeric as numpyNumeric
array_types = [
 numpy.ndarray]
try:
    import numarray
...
def is_parameter_type_def(item_type):
    if type(item_type) == types.ClassType:
        ...
        return True
    elif issubclass(item_type, ParameterTypeDef):
        return True
    elif item_type == types.NoneType:
        warnings.warn('types.NoneType will stop being a supported type ' + 'argument.  Please call ' + 'VisionEgg.ParameterTypes.get_type(None) to get the ' + 'supported value', DeprecationWarning, stacklevel=2)
        return True
    else:
        return False
    ...
    if isinstance(value, bool):
        ...
        if is_sequence:
            if len(value) == 4:
                ...
                if is_sequence4x4:
                    sequence4x4_type = get_type(value[0][0])
                    return Sequence4x4(sequence4x4_type)
            if type(value) == str:
                return String
            lcd_type = get_type(value[0])
            for i in range(len(value)):
                ...
                if lcd_type != this_type:
                    if lcd_type.verify(this_type):
                        ...
                        lcd_type = AnyOf(lcd_type, this_type)
