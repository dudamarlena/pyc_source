...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\VisionEgg\Textures.py
# Compiled at: 2009-07-21 14:10:44
"""
Texture (images mapped onto polygons) stimuli.

"""
import logging, VisionEgg, VisionEgg.Core, VisionEgg.ParameterTypes as ve_types, Image, ImageDraw, pygame.surface, pygame.image, math, types, os, numpy, numpy.oldnumeric as numpyNumeric, numpy.oldnumeric.mlab as MLab, VisionEgg.GL as gl, OpenGL.GLU as glu, _imaging, ImageFile, ImageFileIO, BmpImagePlugin, JpegImagePlugin, PngImagePlugin
...
class Texture(object):
    ...
    __slots__ = ('texels', 'texture_object', 'size', '_filename', '_file_stream', 'buf_lf',
                 'buf_rf', 'buf_bf', 'buf_tf', '_buf_l', '_buf_r', '_buf_b', '_buf_t')
    ...
    def __init__(self, texels=None, size=None):
        ...
        if texels is None:
            if size is None:
                size = (256, 256)
            texels = Image.new('RGB', size, (255, 255, 255))
        if type(texels) == types.FileType:
            texels = Image.open(texels)
        ...
        if isinstance(texels, Image.Image):
            if texels.mode == 'P':
                texels = texels.convert('RGBX')
            self.size = texels.size
        elif isinstance(texels, pygame.surface.Surface):
            self.size = texels.get_size()
        elif isinstance(texels, numpy.ndarray):
    ...
    def get_texels_as_image(self):
        ...
        if isinstance(self.texels, numpy.ndarray):
            ...
            raise NotImplementedError("Don't know how to convert texel data %s to PIL image" % (
             self.texels,))
    ...
    def get_texels_as_array(self):
        ...
        if isinstance(self.texels, numpy.ndarray):
            ...
            if self.texels.mode == 'P':
                texel_data = self.texels.convert('RGBA')
            ...
            raise NotImplementedError("Don't know how to convert texel data %s to numpy array" % (
             self.texels,))

    def get_pixels_as_image(self):
        logger = logging.getLogger('VisionEgg.Textures')
        logger.warning('Using deprecated method get_pixels_as_image(). Use get_texels_as_image() instead.')
        return self.get_texels_as_image()

    def load(self, texture_object, build_mipmaps=True, rescale_original_to_fill_texture_object=False, internal_format=gl.GL_RGB):
        ...
        if width != width_pow2 or height != height_pow2:
            if isinstance(self.texels, numpy.ndarray):
                ...
                buffer = pygame.surface.Surface((width_pow2, height_pow2), self.texels.get_flags(), self.texels.get_bitsize())
        ...
        texture_object.put_new_image(buffer, internal_format=internal_format, mipmap_level=0)
        if not isinstance(self.texels, Image.Image):
            raise NotImplementedError('Building of mipmaps not implemented for this texel ' + 'data type. (Use PIL Images or set parameter ' + 'mipmaps_enabled = False.)')
        ...
        while biggest_dim > 1:
            ...
            texture_object.put_new_image(im, mipmap_level=mipmap_level, internal_format=internal_format, check_opengl_errors=False)
            mipmap_level += 1
            biggest_dim = max(this_width, this_height)
...
class TextureFromFile(Texture):
    ...
    def __init__(self, filename):
        logger = logging.getLogger('VisionEgg.Textures')
        logger.warning('class TextureFromFile deprecated, use class Texture instead.')
...
class TextureObject(object):
    ...
    __slots__ = ('min_filter', 'mag_filter', 'wrap_mode_r', 'wrap_mode_s', 'wrap_mode_t',
                 'border_color', 'target', 'dimensions', 'gl_id', '__gl_module__')
    _cube_map_side_names = [
    ...
    def put_new_image(self, texel_data, mipmap_level=0, border=0, check_opengl_errors=True, internal_format=gl.GL_RGB, data_format=None, data_type=None, cube_side=None):
        ...
        if isinstance(texel_data, numpy.ndarray):
            if self.dimensions != 'cube':
                ...
                if not data_dimensions == self.dimensions:
                    assert data_dimensions == self.dimensions + 1
                else:
            ...
            if data_format is None:
                if isinstance(texel_data, numpy.ndarray):
                    ...
                    if texel_data.mode == 'L':
                        ...
                        texel_data = texel_data.convert('RGBA')
                        data_format = gl.GL_RGBA
                    else:
            ...
            if data_type == gl.GL_UNSIGNED_BYTE:
                if isinstance(texel_data, numpy.ndarray):
                    texel_data = texel_data.astype(numpy.uint8)
            else:
                raise NotImplementedError('Only data_type GL_UNSIGNED_BYTE currently supported')
            if self.dimensions == 1:
                width = texel_data.shape[0]
                raise is_power_of_2(width) or ValueError('texel_data does not have all dimensions == n^2')
        else:
            if isinstance(texel_data, numpy.ndarray):
        ...
        if check_opengl_errors:
            ...
            if self.dimensions == 1:
                gl.glTexImage1Dub(gl.GL_PROXY_TEXTURE_1D, mipmap_level, internal_format, border, data_format, texel_data)
                ...
                gl.glTexImage2D(target, mipmap_level, internal_format, width, height, border, data_format, data_type, raw_data)
                ...
                gl.glTexImage3Dub(gl.GL_PROXY_TEXTURE_3D, mipmap_level, internal_format, border, data_format, texel_data)
        ...
        if self.dimensions == 1:
            gl.glTexImage1Dub(gl.GL_TEXTURE_1D, mipmap_level, internal_format, border, data_format, texel_data)
        elif self.dimensions in (2, 'cube'):
            if self.dimensions == 2:
            ...
            gl.glTexImage3Dub(gl.GL_TEXTURE_3D, mipmap_level, internal_format, border, data_format, texel_data)
        else:
            raise RuntimeError('Unknown number of dimensions.')
        return

    def put_new_image_build_mipmaps(self, texel_data, internal_format=gl.GL_RGB, data_format=None, data_type=None, cube_side=None):
        ...
        args = (
         ...
         data_type)
        print 'args', args
        glu.gluBuild2DMipmaps(self.target, internal_format, width, height, data_format, data_type, raw_data)
        return

    def put_sub_image(self, texel_data, mipmap_level=0, offset_tuple=None, data_format=None, data_type=None, cube_side=None):
        ...
        if self.dimensions == 1:
            ...
            gl.glTexSubImage1D(gl.GL_TEXTURE_1D, mipmap_level, x_offset, width, data_format, data_type, raw_data)
        elif self.dimensions in (2, 'cube'):
            if self.dimensions == 2:
                target = gl.GL_TEXTURE_2D
            else:
            ...
            if isinstance(texel_data, numpy.ndarray):
                ...
                if texel_data.get_alpha():
                    ...
                    raw_data = pygame.image.tostring(texel_data, 'RGB', 1)
            gl.glTexSubImage2D(target, mipmap_level, x_offset, y_offset, width, height, data_format, data_type, raw_data)
        elif self.dimensions == 3:
    ...
    def put_new_framebuffer(self, buffer='back', mipmap_level=0, border=0, framebuffer_lowerleft=None, size=None, internal_format=gl.GL_RGB, cube_side=None):
        ...
        if not is_power_of_2(height):
            raise ValueError('texel_data does not have all dimensions == n^2')
        target = gl.GL_TEXTURE_2D
        gl.glCopyTexImage2D(target, mipmap_level, internal_format, x, y, width, height, border)
        return
...
class TextureStimulusBaseClass(VisionEgg.Core.Stimulus):
    ...
    shrink_texture_ok -- Allow automatic shrinking of texture if too big? (Boolean)
                         Default: False
    """
    parameters_and_defaults = {'texture': (
                 None,
       ...
       'texture_wrap_t': (
                        ...
                        'OpenGL texture wrap enum',
                        VisionEgg.ParameterDefinition.OPENGL_ENUM)}
    constant_parameters_and_defaults = {'internal_format': (
                         gl.GL_RGB,
                         ve_types.Integer,
    ...
    __slots__ = ('texture_object', '_using_texture')
    _mipmap_modes = [
     gl.GL_LINEAR_MIPMAP_LINEAR, gl.GL_LINEAR_MIPMAP_NEAREST,
    ...
    def _reload_texture(self):
        ...
        if not self.constant_parameters.shrink_texture_ok:
            p.texture.load(self.texture_object, internal_format=self.constant_parameters.internal_format, build_mipmaps=self.constant_parameters.mipmaps_enabled)
        else:
            max_dim = gl.glGetIntegerv(gl.GL_MAX_TEXTURE_SIZE)
            resized = 0
...
class Mask2D(VisionEgg.ClassWithParameters):
    ...
    radius_parameter -- radius for circle, sigma for gaussian (Real)
                        Default: 25.0
    """
    constant_parameters_and_defaults = {'function': (
                  'gaussian',
                  ve_types.String,
                  "'gaussian' or 'circle'"), 
       'radius_parameter': (
    ...
    def __init__(self, **kw):
        ...
        if cp.function == 'gaussian':
            xx = numpyNumeric.outerproduct(numpyNumeric.ones((1, cp.num_samples[1])), numpyNumeric.arange(0, cp.num_samples[0], 1.0) - cp.num_samples[0] / 2)
            yy = numpyNumeric.outerproduct(numpyNumeric.arange(0, cp.num_samples[1], 1.0) - cp.num_samples[1] / 2, numpyNumeric.ones((1, cp.num_samples[0])))
            ...
            for x_offset in x_offsets:
                for y_offset in y_offsets:
                    xx = numpyNumeric.outerproduct(numpyNumeric.ones((1, cp.num_samples[1])), numpyNumeric.arange(0, cp.num_samples[0], 1.0) - cp.num_samples[0] / 2 + 0.5) + x_offset
                    yy = numpyNumeric.outerproduct(numpyNumeric.arange(0, cp.num_samples[1], 1.0) - cp.num_samples[1] / 2 + 0.5, numpyNumeric.ones((1, cp.num_samples[0]))) + y_offset
                    dist_from_center = numpyNumeric.sqrt(xx ** 2 + yy ** 2)
        ...
        self.texture_object.put_new_image(data, data_format=gl.GL_ALPHA, internal_format=gl.GL_ALPHA)
        self.texture_object.set_min_filter(gl.GL_LINEAR)
    ...
    def draw(self):
        ...
        if p.lowerleft != None:
            if not hasattr(VisionEgg.config, '_GAVE_LOWERLEFT_DEPRECATION'):
                logger = logging.getLogger('VisionEgg.Textures')
                logger.warning("Specifying texture by 'lowerleft' deprecated parameter deprecated.  Use 'position' parameter instead.  (Allows use of 'anchor' parameter to set to other values.)")
        ...
        if p.on:
            ...
            try:
                ...
                if len(translate_vector) == 2:
                    translate_vector = (
                     translate_vector[0], translate_vector[1], 0)
                ...
                tex.update()
                if p.mask:
                    p.mask.draw_masked_quad(tex.buf_lf, tex.buf_rf, tex.buf_bf, tex.buf_tf, l, r, b, t, 0.0)
                else:
                    gl.glBegin(gl.GL_QUADS)
...
class SpinningDrum(TextureStimulusBaseClass):
    ...
    parameters_and_defaults = {'on': (
       ...
       'position': (
                  (0.0, 0.0, 0.0),
                  ve_types.AnyOf(ve_types.Sequence2(ve_types.Real), ve_types.Sequence3(ve_types.Real)),
       ...
       'orientation': (
                     ...
                     ve_types.Real,
                     '0=right, 90=up')}
    __slots__ = ('cached_display_list_normal', 'cached_display_list_mirror', 'cached_display_list_num_sides',
                 'cached_display_list_radius', 'cached_display_list_height', 'texture_stimulus')

    def __init__(self, **kw):
        TextureStimulusBaseClass.__init__(self, **kw)
        self.cached_display_list_normal = gl.glGenLists(1)
        self.cached_display_list_mirror = gl.glGenLists(1)
    ...
    def draw(self):
        ...
        if p.on:
            ...
            gl.glPushMatrix()
            try:
                gl.glColor4f(0.5, 0.5, 0.5, p.contrast)
                if not self.constant_parameters.mipmaps_enabled:
                    if p.texture_min_filter in TextureStimulusBaseClass._mipmap_modes:
                        raise RuntimeError('Specified a mipmap mode in texture_min_filter, but mipmaps not enabled.')
                self.texture_object.set_min_filter(p.texture_min_filter)
                ...
                if p.flat:
                    ...
                    (w, h) = p.texture.size
                    tex_phase = p.angular_position / 360.0 + 0.5
                    tex_phase = tex_phase % 1.0
                    TINY = 1e-10
                    tex = p.texture
                    tex.update()
                    ...
                    if p.num_sides != self.cached_display_list_num_sides or p.radius != self.cached_display_list_radius or p.height != self.cached_display_list_height:
                        self.rebuild_display_list()
                    if not p.flip_image:
...
class FixationCross(VisionEgg.Core.Stimulus):
    ...
    parameters_and_defaults = {'on': (
       ...
       'size': (
              (64, 64),
              ve_types.Sequence2(ve_types.Real))}
    constant_parameters_and_defaults = {'texture_size': (
                      (64, 64),
                      ve_types.Sequence2(ve_types.Real))}
    __slots__ = ('texture_stimulus', )
    ...
    def __init__(self, **kw):
        ...
        self.texture_stimulus = TextureStimulus(texture=Texture(texels), position=self.parameters.position, anchor='center', size=self.parameters.size, internal_format=gl.GL_RGBA, mipmaps_enabled=False, texture_min_filter=gl.GL_NEAREST, texture_mag_filter=gl.GL_NEAREST)

    def draw(self):
        contained = self.texture_stimulus.parameters
        my = self.parameters
