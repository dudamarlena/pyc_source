...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\VisionEgg\PyroApps\EPhysGUI.py
# Compiled at: 2009-07-15 18:56:28
import VisionEgg, sys, socket, re, time, string, types, os, parser, symbol, token, compiler, pickle, random, math, threading, Tkinter, tkMessageBox, tkSimpleDialog, tkFileDialog, StringIO, Pyro, numpy, VisionEgg, VisionEgg.PyroClient, VisionEgg.PyroApps.ScreenPositionGUI, VisionEgg.GUI, VisionEgg.ParameterTypes as ve_types, VisionEgg.PyroApps.TargetGUI, VisionEgg.PyroApps.MouseTargetGUI, VisionEgg.PyroApps.FlatGratingGUI, VisionEgg.PyroApps.SphereGratingGUI, VisionEgg.PyroApps.SpinningDrumGUI, VisionEgg.PyroApps.GridGUI, VisionEgg.PyroApps.ColorCalGUI, VisionEgg.PyroApps.DropinGUI, VisionEgg.PyroApps.AST_ext as AST_ext, VisionEgg.PyroApps.VarTypes as VarTypes
client_list = []
...
class ScrollListFrame(Tkinter.Frame):
    ...
    def __init__(self, master=None, list_of_contained_objects=None, contained_objectbject_maker=None, container_class=ContainedObjectBase, **cnf):
        ...
        self.frame.title = Tkinter.Listbox(self.frame, relief=Tkinter.FLAT, font=('courier',
                                                                                  10,
                                                                                  'bold'), height=1, exportselection=0)
        self.frame.title.insert(Tkinter.END, self.container_class.header)
        self.frame.list = Tkinter.Listbox(self.frame, relief=Tkinter.SUNKEN, font=('courier',
                                                                                   10,
                                                                                   'normal'), width=40, height=10, selectbackground='#eed5b7', selectborderwidth=0, selectmode=Tkinter.BROWSE, xscroll=self.frame.hscroll.set, yscroll=self.frame.vscroll.set, exportselection=0)
    ...
    def edit_selected(self, dummy_arg=None):
        ...
        if selected is not None:
            if len(self.list[main_list_index]) == 1:
                ...
                if modified_contained_object is not None:
        ...
        if selected is not None:
            ...
            if loop_list_index == 0:
                if main_list_index > 0:
                    ...
                    merge_error = 3
            elif loop_list_index == len(selected_loop_list) - 1:
                del self.list[main_list_index][loop_list_index]
...
class LoopParamDialog(tkSimpleDialog.Dialog):
    ...
    def body(self, master):
        Tkinter.Label(master, text='Add sequence of automatic variable values', font=('Helvetica',
                                                                                      12,
                                                                                      'bold')).grid(row=0, column=0, columnspan=2)
        var_frame = Tkinter.Frame(master, relief=Tkinter.GROOVE, borderwidth=2)
        ...
        num_cols = int(math.ceil(len(loopable_variables) / 10.0))
        var_frame_row = 0
        Tkinter.Label(var_frame, text='Select a variable', font=('Helvetica', 12, 'bold')).grid(row=var_frame_row, column=0, columnspan=num_cols)
        self.var_name = Tkinter.StringVar()
        self.var_name.set('<repeat>')
        var_names = loopable_variables[:]
        var_names.sort()
        var_frame_row += 1
        Tkinter.Radiobutton(var_frame, text='Repeat (Average)', variable=self.var_name, value='<repeat>', anchor=Tkinter.W).grid(row=var_frame_row, column=0, sticky='w')
        var_frame_row += 1
        for var_name in var_names:
            use_row = var_frame_row % 10 + 1
            Tkinter.Radiobutton(var_frame, text=var_name, variable=self.var_name, value=var_name, anchor=Tkinter.W).grid(row=use_row, column=int(math.floor(var_frame_row / 10.0)), sticky='w')
            var_frame_row += 1

        seq_row = 0
        Tkinter.Label(sequence_frame, text='Sequence values', font=('Helvetica', 12,
                                                                    'bold')).grid(row=seq_row, column=0, columnspan=2)
        seq_row += 1
        self.sequence_type = Tkinter.StringVar()
        self.sequence_type.set('manual')
        Tkinter.Radiobutton(sequence_frame, text='Manual:', variable=self.sequence_type, value='manual', anchor=Tkinter.W).grid(row=seq_row, column=0, sticky='w')
        self.sequence_manual_string = Tkinter.StringVar()
        self.sequence_manual_string.set('[1,2,3]')
        Tkinter.Entry(sequence_frame, textvariable=self.sequence_manual_string).grid(row=seq_row, column=1)
        seq_row += 1
        Tkinter.Radiobutton(sequence_frame, text='Linear:', variable=self.sequence_type, value='linear', anchor=Tkinter.W).grid(row=seq_row, column=0, sticky='w')
        ...
        seq_row += 1
        Tkinter.Radiobutton(sequence_frame, text='Log:', variable=self.sequence_type, value='log', anchor=Tkinter.W).grid(row=seq_row, column=0, sticky='w')
        ...
        seq_row += 1
        Tkinter.Radiobutton(sequence_frame, text='Log:', variable=self.sequence_type, value='logb', anchor=Tkinter.W).grid(row=seq_row, column=0, sticky='w')
        ...
        Tkinter.Label(rest_dur_frame, text='Other sequence parameters', font=('Helvetica',
                                                                              12,
                                                                              'bold')).grid(row=0, column=0, columnspan=2)
        Tkinter.Label(rest_dur_frame, text='Interval duration (seconds)').grid(row=1, column=0)
        self.rest_dur = Tkinter.DoubleVar()
        self.rest_dur.set(0.5)
        Tkinter.Entry(rest_dur_frame, textvariable=self.rest_dur, width=10).grid(row=1, column=1)
        self.shuffle_tk_var = Tkinter.BooleanVar()
        self.shuffle_tk_var.set(0)
        Tkinter.Checkbutton(rest_dur_frame, text='Shuffle sequence order', variable=self.shuffle_tk_var).grid(row=2, column=0, columnspan=2)
    ...
    def validate(self):
        if self.sequence_type.get() == 'manual':
            try:
                ...
                tkMessageBox.showwarning('Invalid sequence parameters', 'Manual sequence entry: %s' % (str(x),), parent=self)
                return 0
            else:
                if type(seq) != types.ListType:
                    tkMessageBox.showwarning('Invalid sequence parameters', 'Manual sequence entry: Not a list', parent=self)
                    return 0
            ...
            n = self.lin_n_tk.get()
            if n < 2:
                tkMessageBox.showwarning('Invalid sequence parameters', 'Must have n >= 2.', parent=self)
                return 0
            ...
            tkMessageBox.showwarning('Invalid sequence parameters', 'Invalid sequence type.', parent=self)
            return 0
        rest_dur_sec = self.rest_dur.get()
        if self.shuffle_tk_var.get():
            random.shuffle(seq)
        self.result = Loop(variable=self.var_name.get(), sequence=seq, rest_duration_sec=rest_dur_sec)
        return 1
...
def get_server(hostname='', port=7766):
    ...
    class ConnectWindow(Tkinter.Frame):
        ...
        def __init__(self, master=None, hostname='', port=7766, **kw):
            ...
            current_row = 0
            Tkinter.Message(self, text='Welcome to the "EPhys GUI" of the Vision Egg!\n\n' + 'Please enter the hostname ' + 'and port number ' + 'of the computer on which you have the ' + '"EPhys server" running.').grid(row=current_row, column=0, columnspan=2)
            hostname = 'localhost'
...
class GammaFrame(Tkinter.Frame):
    ...
    def __init__(self, master=None, ephys_server=None, **kw):
        Tkinter.Frame.__init__(self, master, **kw)
        self.winfo_toplevel().title('Gamma - Vision Egg')
        self.ephys_server = ephys_server
        self.columnconfigure(0, weight=1)
        row = 0
        Tkinter.Label(self, font=('Helvetica', 12, 'bold'), text='Load Gamma Table').grid(row=row)
        row += 1
        Tkinter.Button(self, text='Set from .ve_gamma file...', command=self.set_from_file).grid(row=row, sticky='w')
        row += 1
        Tkinter.Button(self, text='Set to monitor default (linear gamma table)', command=self.set_monitor_default).grid(row=row, sticky='w')
        row += 1
        invert_frame = Tkinter.Frame(self)
        invert_frame.grid(row=row, sticky='we')
        Tkinter.Button(invert_frame, text='Linearize luminance for gammas', command=self.linearize).grid(row=0, column=0)
        Tkinter.Label(invert_frame, text='Red:').grid(row=0, column=1)
        self.red_gamma = Tkinter.DoubleVar()
        self.red_gamma.set(2.2)
        Tkinter.Entry(invert_frame, textvariable=self.red_gamma, width=5).grid(row=0, column=2)
        Tkinter.Label(invert_frame, text='Green:').grid(row=0, column=3)
        self.green_gamma = Tkinter.DoubleVar()
        self.green_gamma.set(2.2)
        Tkinter.Entry(invert_frame, textvariable=self.green_gamma, width=5).grid(row=0, column=4)
        Tkinter.Label(invert_frame, text='Blue:').grid(row=0, column=5)
        self.blue_gamma = Tkinter.DoubleVar()
        self.blue_gamma.set(2.2)
        Tkinter.Entry(invert_frame, textvariable=self.blue_gamma, width=5).grid(row=0, column=6)
        row += 1
    ...
    def set_monitor_default(self, dummy_arg=None):
        self.success_label.configure(text='Setting...')
        try:
            red = self.get_corrected_gamma_table(1.0)
        except:
            self.success_label.configure(text='Calculation error')
            raise
    ...
    def set_from_file(self):
        self.success_label.configure(text='Setting...')
        filename = tkFileDialog.askopenfilename(parent=self, defaultextension='.ve_gamma', filetypes=[
         ('Configuration file', '*.ve_gamma')], initialdir=VisionEgg.config.VISIONEGG_USER_DIR)
        if not filename:
...
class ImageSequenceLauncher(Tkinter.Toplevel):
    ...
    def do_it(self):
        ...
        self.ephys_server.save_image_sequence(fps=fps, filename_base=filename_base, filename_suffix=filename_suffix, save_dir=server_save_dir)
        self.destroy()


class AppWindow(Tkinter.Frame):

    def __init__(self, master=None, client_list=None, server_hostname='', server_port=7766, **cnf):
        ...
        self.bar.file_menu.add_checkbutton(label='Quit server too', variable=self.quit_server_too)
        ...
        for (maybe_stimkey, maybe_control_frame, maybe_title) in self.client_list:
            if maybe_title != 'Vision Egg Script':
                self.bar.stimuli_menu.add_radiobutton(label=maybe_title, command=self.change_stimulus, variable=self.stimulus_tk_var, value=maybe_stimkey)

        self.bar.calibration_menu = Tkinter.Menu(self.bar, name='calibration_menu')
        self.bar.add_cascade(label='Configure/Calibrate', menu=self.bar.calibration_menu)
        ...
        self.bar.calibration_menu.add_checkbutton(label='Warn on frame skip', variable=self.notify_on_dropped_frames)
        self.override_t_abs_sec = Tkinter.StringVar()
        self.override_t_abs_sec.set('0.0')
        self.override_t_abs_on = Tkinter.BooleanVar()
        self.override_t_abs_on.set(0)
        self.bar.calibration_menu.add_checkbutton(label='Override server absolute time (CAUTION)', variable=self.override_t_abs_on)
        row = 0
        self.stim_frame_cnf = {'row': row, 'column': 0, 
           'columnspan': 2, 
           'sticky': 'nwes'}
        row += 1
        Tkinter.Label(self, text='Sequence information', font=('Helvetica', 12, 'bold')).grid(row=row, column=0)
        row += 1
        self.loop_frame_cnf = {'row': row, 'column': 0, 
           'sticky': 'nwes'}
        row -= 1
        Tkinter.Label(self, text='Parameter Save Options', font=('Helvetica', 12, 'bold')).grid(row=row, column=1)
        row += 1
        self.auto_save_frame = Tkinter.Frame(self)
        asf = self.auto_save_frame
        asf.grid(row=row, column=1, sticky='nwes')
        asf.columnconfigure(1, weight=1)
        asf.grid_row = 0
        self.autosave = Tkinter.BooleanVar()
        self.autosave.set(1)
        self.auto_save_button = Tkinter.Checkbutton(asf, text='Auto save trial parameters', variable=self.autosave)
        ...
        filetype_bar = Tkinter.Menubutton(asf, textvariable=self.param_file_type_tk_var, relief=Tkinter.RAISED)
        filetype_bar.grid(row=asf.grid_row, column=2)
        filetype_bar.menu = Tkinter.Menu(filetype_bar, tearoff=0)
        filetype_bar.menu.add_radiobutton(label='Python format', value='Python format', variable=self.param_file_type_tk_var)
        filetype_bar.menu.add_radiobutton(label='Matlab format', value='Matlab format', variable=self.param_file_type_tk_var)
        filetype_bar['menu'] = filetype_bar.menu
        asf.grid_row += 1
        Tkinter.Label(asf, text='Parameter file directory:').grid(row=asf.grid_row, column=0, sticky='e')
        Tkinter.Entry(asf, textvariable=self.autosave_dir).grid(row=asf.grid_row, column=1, sticky='we')
        Tkinter.Button(asf, text='Set...', command=self.set_autosave_dir).grid(row=asf.grid_row, column=2)
        asf.grid_row += 1
        Tkinter.Label(asf, text='Parameter file basename:').grid(row=asf.grid_row, column=0, sticky='e')
        Tkinter.Entry(asf, textvariable=self.autosave_basename).grid(row=asf.grid_row, column=1, sticky='we')
        Tkinter.Button(asf, text='Reset', command=self.reset_autosave_basename).grid(row=asf.grid_row, column=2)
        row += 1
        ...
        row += 1
        self.progress = VisionEgg.GUI.ProgressBar(self, width=300, relief='sunken', doLabel=0, labelFormat='%s')
    ...
    def switch_to_stimkey(self, stimkey):
        ...
        if hasattr(self, 'loop_frame'):
            ...
            del self.loop_frame
        self.loop_frame = ScrollListFrame(master=self, container_class=LoopContainedObject)
        self.loop_frame.grid(**self.loop_frame_cnf)
        self.autosave_basename.set(self.stim_frame.get_shortname())
        self.stimulus_tk_var.set(self.stim_frame.get_shortname())
    ...
    def save_config(self):
        self.stim_frame.send_values()
        if self.config_dir is not None:
            initialdir = self.config_dir
        else:
            initialdir = VisionEgg.config.VISIONEGG_USER_DIR
        filename = tkFileDialog.asksaveasfilename(parent=self, defaultextension='.ve_cfg', filetypes=[
         ('Configuration file', '*.ve_cfg')], initialdir=initialdir)
        if not filename:
            return
        fd = open(filename, 'wb')
        save_dict = {'stim_type': self.stim_frame.get_shortname(), 'loop_list': self.loop_frame.list, 
        ...
        self.config_dir = os.path.split(filename)[0]
        return
    ...
    def load_config(self):
        if self.config_dir is not None:
            ...
            initialdir = VisionEgg.config.VISIONEGG_USER_DIR
        filename = tkFileDialog.askopenfilename(parent=self, defaultextension='.ve_cfg', filetypes=[
         ('Configuration file', '*.ve_cfg')], initialdir=initialdir)
        if not filename:
            return
        self.config_dir = os.path.split(filename)[0]
    ...
    def load_params(self, orig_load_dict={}):
        filename = tkFileDialog.askopenfilename(parent=self, defaultextension='.py', filetypes=[
         ('Auto-saved parameter file', '*.py')])
        if not filename:
            return
        locals = {}
        load_dict = orig_load_dict.copy()
        execfile(filename, locals, load_dict)
        if load_dict['stim_type'] != self.stim_frame.get_shortname():
            self.change_stimulus(new_stimkey=load_dict['stim_type'] + '_server')
        self.loop_frame.list = []
        self.loop_frame.update_now()
        new_params = {}
        exception_info = []
        for param_name in dir(self.stim_frame.meta_params):
            if param_name[:2] != '__':
                try:
                    ...
                    exception_info.append(sys.exc_info())
                else:
        ...
        try:
            ...
            tkMessageBox.showwarning('No absolute time parameter', 'No absolute time parameter when loading file', parent=self)
        else:
            if not self.override_t_abs_on.get():
                self.override_t_abs_on.set(1)
                tkMessageBox.showwarning('Overriding absolute time parameter', 'Overriding absolute time parameter on server. ' + 'Remember to turn this option off (in Configure ' + '/ Calibrate menu) when done.', parent=self)
            self.override_t_abs_sec.set(repr(override_t_abs_sec))

        self.stim_frame.update_tk_vars()
        return load_dict

    def load_demoscript(self):
        self.demoscript_filename = tkFileDialog.askopenfilename(parent=self, defaultextension='.py', filetypes=[
         ('Vision Egg Demo Script', '*.py')])
    ...
    def launch_screen_pos(self, dummy_arg=None):
        ...
        frame = VisionEgg.PyroApps.ScreenPositionGUI.ScreenPositionControlFrame(dialog, auto_connect=1, server_hostname=self.server_hostname, server_port=self.server_port)
    ...
    def launch_stim_onset_cal(self, dummy_arg=None):
        ...
        Tkinter.Label(frame, font=('Helvetica', 12, 'bold'), text='Stimulus onset timing').grid(row=0, column=0)
        Tkinter.Label(frame, text='Use a light detector to verify the onset of a trial.').grid(row=1, column=0)
        Tkinter.Checkbutton(frame, text='Black box (always) with white box (during trial)', variable=self.stim_onset_cal_tk_var, command=self.update_stim_onset_cal).grid(row=2, column=0)
        ...
        Tkinter.Button(frame, text='update position and size', command=self.set_stim_onset_cal_position).grid(row=4, column=0)
        self.set_stim_onset_cal_position()
        frame.pack(expand=1, fill=Tkinter.BOTH)

    def launch_gamma_panel(self, dummy_arg=None):
        dialog = Tkinter.Toplevel(self)
        frame = GammaFrame(dialog, self.ephys_server)
        frame.pack(expand=1, fill=Tkinter.BOTH)
        ...
        def process_loops(depth):
            ...
            class LoopInfoFrame(Tkinter.Frame):
                ...
                def __init__(self, master=None, **kw):
                    Tkinter.Frame.__init__(self, master, **kw)
                    Tkinter.Label(self, text='Doing sequence').grid(row=0, column=0)
                    self.status_tk_var = Tkinter.StringVar()
                    Tkinter.Label(self, textvariable=self.status_tk_var).grid(row=1, column=0)
                    self.cancel_asap = 0
                    Tkinter.Button(self, text='Cancel', command=self.cancel).grid(row=2, column=0)
                    self.focus_set()
                    self.grab_set()
    ...
    def do_single_trial_work(self):
        ...
        try:
            ...
            if duration_sec > 0:
                ...
                while self.ephys_server.is_in_go_loop():
                    time.sleep(0.1)

                if self.notify_on_dropped_frames.get():
                    if self.ephys_server.were_frames_dropped_in_last_go_loop():
                        tkMessageBox.showwarning('Dropped frame(s)', 'During the last trial, at least 1 frame was dropped.', parent=self)
            elif self.stim_frame.send_values():
...
if __name__ == '__main__':
    ...
    if result:
        ...
        app_window = AppWindow(client_list=client_list, server_hostname=hostname, server_port=port)
