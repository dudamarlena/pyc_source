...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\VisionEgg\GUI.py
# Compiled at: 2009-07-07 11:29:42
"""
Graphical user interface classes and functions.

"""
import VisionEgg, logging, VisionEgg, os, sys
...
class ProgressBar(Tkinter.Frame):
    ...
    def __init__(self, master=None, orientation='horizontal', min=0, max=100, width=100, height=18, doLabel=1, fillColor='LightSteelBlue1', background='gray', labelColor='black', labelFont='Helvetica', labelText='', labelFormat='%d%%', value=50, **cnf):
        ...
        self.value = value
        self.canvas = Tkinter.Canvas(self, height=height, width=width, bd=0, highlightthickness=0, background=background)
...
class GraphicsConfigurationWindow(Tkinter.Frame):
    ...
    def __init__(self, master=None, **cnf):
        ...
        self.clicked_ok = 0
        row = 0
        Tkinter.Label(self, text='Vision Egg - Graphics configuration', font=('Helvetica',
                                                                              14,
                                                                              'bold')).grid(row=row, columnspan=2)
        row += 1
        topframe = Tkinter.Frame(self)
        topframe.grid(row=row, column=0, columnspan=2)
        topframe_row = 0
        Tkinter.Label(topframe, text=self.format_string('The default value for these variables and the presence of this dialog window can be controlled via the Vision Egg config file. If this file exists in the Vision Egg user directory, that file is used.  Otherwise, the configuration file found in the Vision Egg system directory is used.')).grid(row=topframe_row, column=1, columnspan=2, sticky=Tkinter.W)
        topframe_row += 1
        try:
            import _imaging, _imagingtk, ImageFile, ImageFileIO, BmpImagePlugin, JpegImagePlugin, Image, ImageTk
            ...
            logger.info('No Vision Egg logo :( because of error while trying to display image in GUI.GraphicsConfigurationWindow: %s: %s' % (
        ...
        file_row = 0
        Tkinter.Label(file_frame, text='This script:').grid(row=file_row, column=0, sticky=Tkinter.E)
        Tkinter.Label(file_frame, text='%s' % (os.path.abspath(sys.argv[0]),)).grid(row=file_row, column=1, sticky=Tkinter.W)
        file_row += 1
        Tkinter.Label(file_frame, text='Vision Egg system directory:').grid(row=file_row, column=0, sticky=Tkinter.E)
        Tkinter.Label(file_frame, text='%s' % (os.path.abspath(VisionEgg.config.VISIONEGG_SYSTEM_DIR),)).grid(row=file_row, column=1, sticky=Tkinter.W)
        file_row += 1
        Tkinter.Label(file_frame, text='Vision Egg user directory:').grid(row=file_row, column=0, sticky=Tkinter.E)
        Tkinter.Label(file_frame, text='%s' % (os.path.abspath(VisionEgg.config.VISIONEGG_USER_DIR),)).grid(row=file_row, column=1, sticky=Tkinter.W)
        file_row += 1
        Tkinter.Label(file_frame, text='Config file location:').grid(row=file_row, column=0, sticky=Tkinter.E)
        if VisionEgg.config.VISIONEGG_CONFIG_FILE:
            Tkinter.Label(file_frame, text='%s' % (os.path.abspath(VisionEgg.config.VISIONEGG_CONFIG_FILE),)).grid(row=file_row, column=1, sticky=Tkinter.W)
        else:
            Tkinter.Label(file_frame, text='(None)').grid(row=file_row, column=1, sticky=Tkinter.W)
        file_row += 1
        Tkinter.Label(file_frame, text='Log file location:').grid(row=file_row, column=0, sticky=Tkinter.E)
        if VisionEgg.config.VISIONEGG_LOG_FILE:
            Tkinter.Label(file_frame, text='%s' % (os.path.abspath(VisionEgg.config.VISIONEGG_LOG_FILE),)).grid(row=file_row, column=1, sticky=Tkinter.W)
        else:
            Tkinter.Label(file_frame, text='(stderr console)').grid(row=file_row, column=1, sticky=Tkinter.W)
        row += 1
        ...
        Tkinter.Checkbutton(cf, text='Fullscreen', variable=self.fullscreen, relief=Tkinter.FLAT).grid(row=cf_row, column=0, sticky=Tkinter.W)
        cf_row += 1
        ...
        if self.show_synclync_option:
            Tkinter.Checkbutton(cf, text='SyncLync device present', variable=self.synclync_present, relief=Tkinter.FLAT).grid(row=cf_row, column=0, sticky=Tkinter.W)
        cf_row += 1
        self.maxpriority = Tkinter.BooleanVar()
        self.maxpriority.set(VisionEgg.config.VISIONEGG_MAXPRIORITY)
        Tkinter.Checkbutton(cf, text='Maximum priority (use with caution)', variable=self.maxpriority, relief=Tkinter.FLAT).grid(row=cf_row, column=0, sticky=Tkinter.W)
        cf_row += 1
        if sys.platform == 'darwin':
            ...
            Tkinter.Button(cf, text='Maximum priority options...', command=self.darwin_maxpriority_tune).grid(row=cf_row, column=0)
            cf_row += 1
        self.sync_swap = Tkinter.BooleanVar()
        self.sync_swap.set(VisionEgg.config.VISIONEGG_SYNC_SWAP)
        Tkinter.Checkbutton(cf, text='Attempt vsync', variable=self.sync_swap, relief=Tkinter.FLAT).grid(row=cf_row, column=0, sticky=Tkinter.W)
        cf_row += 1
        self.frameless = Tkinter.BooleanVar()
        self.frameless.set(VisionEgg.config.VISIONEGG_FRAMELESS_WINDOW)
        Tkinter.Checkbutton(cf, text='No frame around window', variable=self.frameless, relief=Tkinter.FLAT).grid(row=cf_row, column=0, sticky=Tkinter.W)
        cf_row += 1
        self.mouse_visible = Tkinter.BooleanVar()
        self.mouse_visible.set(not VisionEgg.config.VISIONEGG_HIDE_MOUSE)
        Tkinter.Checkbutton(cf, text='Mouse cursor visible', variable=self.mouse_visible, relief=Tkinter.FLAT).grid(row=cf_row, column=0, sticky=Tkinter.W)
        cf_row += 1
        self.stereo = Tkinter.BooleanVar()
        self.stereo.set(VisionEgg.config.VISIONEGG_REQUEST_STEREO)
        Tkinter.Checkbutton(cf, text='Stereo', variable=self.stereo, relief=Tkinter.FLAT).grid(row=cf_row, column=0, sticky=Tkinter.W)
        cf_row += 1
        if sys.platform == 'darwin':
        ...
        self.gamma_source.set(str(VisionEgg.config.VISIONEGG_GAMMA_SOURCE).lower())
        Tkinter.Label(gamma_frame, text='Gamma:').grid(row=0, column=0)
        Tkinter.Radiobutton(gamma_frame, text='Native', value='none', variable=self.gamma_source).grid(row=0, column=1, padx=1)
        Tkinter.Radiobutton(gamma_frame, text='Quick', value='invert', variable=self.gamma_source).grid(row=0, column=2)
        Tkinter.Label(gamma_frame, text='R:').grid(row=0, column=3)
        self.gamma_invert_red = Tkinter.DoubleVar()
        self.gamma_invert_red.set(VisionEgg.config.VISIONEGG_GAMMA_INVERT_RED)
        Tkinter.Entry(gamma_frame, textvariable=self.gamma_invert_red, width=3).grid(row=0, column=4)
        Tkinter.Label(gamma_frame, text='G:').grid(row=0, column=5)
        self.gamma_invert_green = Tkinter.DoubleVar()
        self.gamma_invert_green.set(VisionEgg.config.VISIONEGG_GAMMA_INVERT_GREEN)
        Tkinter.Entry(gamma_frame, textvariable=self.gamma_invert_green, width=3).grid(row=0, column=6)
        Tkinter.Label(gamma_frame, text='B:').grid(row=0, column=7)
        self.gamma_invert_blue = Tkinter.DoubleVar()
        self.gamma_invert_blue.set(VisionEgg.config.VISIONEGG_GAMMA_INVERT_BLUE)
        Tkinter.Entry(gamma_frame, textvariable=self.gamma_invert_blue, width=3).grid(row=0, column=8)
        Tkinter.Radiobutton(gamma_frame, text='Custom:', value='file', variable=self.gamma_source).grid(row=0, column=9)
        ...
        if os.path.isfile(VisionEgg.config.VISIONEGG_GAMMA_FILE):
            ...
            self.gamma_file.set('')
        Tkinter.Entry(gamma_frame, textvariable=self.gamma_file, width=15).grid(row=0, column=10)
        Tkinter.Button(gamma_frame, command=self.set_gamma_file, text='Set...').grid(row=0, column=11)
        row += 1
    ...
    def darwin_maxpriority_tune(self):
        ...
        class DarwinFineTuneDialog(ToplevelDialog):
            ...
            def __init__(self, parent, **cnf):
                ...
                row = 2
                Tkinter.Radiobutton(f, text='Realtime task method', variable=parent.darwin_conventional, value=0).grid(row=row, column=2, columnspan=2)
                row += 1
...
class GetKeypressDialog(ToplevelDialog):
    ...
    def __init__(self, title='Press a key', text='Press a key', key_list=[], **kw):
