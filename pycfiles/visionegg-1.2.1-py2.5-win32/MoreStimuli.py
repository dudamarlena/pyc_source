...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\VisionEgg\MoreStimuli.py
# Compiled at: 2009-07-07 11:29:42
"""
Assorted stimuli.

"""
import logging, VisionEgg, VisionEgg.Core, VisionEgg.ParameterTypes as ve_types, numpy.oldnumeric as Numeric, math, VisionEgg.GL as gl
    ...
    def draw(self):
        ...
        if p.center is not None:
            if not hasattr(VisionEgg.config, '_GAVE_CENTER_DEPRECATION'):
                ...
                logger.warning("Specifying Target2D by deprecated 'center' parameter deprecated.  Use 'position' parameter instead.  (Allows use of 'anchor' parameter to set to other values.)")
            ...
            if p.anti_aliasing:
                if not self._gave_alpha_warning:
                    if len(p.color) > 3 and p.color[3] != 1.0:
                        logger = logging.getLogger('VisionEgg.MoreStimuli')
                        logger.warning('The parameter anti_aliasing is set to true in the Target2D stimulus class, but the color parameter specifies an alpha value other than 1.0.  To acheive anti-aliasing, ensure that the alpha value for the color parameter is 1.0.')
            ...
            if p.anti_aliasing:
                if not self._gave_alpha_warning:
                    if len(p.color) > 3 and p.color[3] != 1.0:
                        ...
                        logger.warning('The parameter anti_aliasing is set to true in the Arrow stimulus class, but the color parameter specifies an alpha value other than 1.0.  To acheive anti-aliasing, ensure that the alpha value for the color parameter is 1.0.')
                        self._gave_alpha_warning = 1
                gl.glEnable(gl.GL_LINE_SMOOTH)
                gl.glBlendFunc(gl.GL_SRC_ALPHA, gl.GL_ONE_MINUS_SRC_ALPHA)
                gl.glEnable(gl.GL_BLEND)
