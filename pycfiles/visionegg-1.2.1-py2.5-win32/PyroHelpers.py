...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\VisionEgg\PyroHelpers.py
...
though!"""
import VisionEgg, VisionEgg.Core, VisionEgg.FlowControl, VisionEgg.ParameterTypes as ve_types, Pyro.core, Pyro.errors
...
class PyroLocalDictController(VisionEgg.FlowControl.EncapsulatedController, Pyro.core.ObjBase):
    ...
    def __init__(self, dict=None, key=None, **kw):
        if dict is None:
            self.dict = {}
            initial_controller = VisionEgg.FlowControl.ConstantController(during_go_value=0, between_go_value=0, eval_frequency=VisionEgg.FlowControl.Controller.NEVER)
        else:
            self.dict = dict
        if key is None:
