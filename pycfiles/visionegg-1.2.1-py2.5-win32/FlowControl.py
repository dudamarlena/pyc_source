...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\VisionEgg\FlowControl.py
# Compiled at: 2009-07-07 11:29:42
"""
Flow control for the Vision Egg.

"""
import logging, logging.handlers, VisionEgg, VisionEgg.GL as gl, VisionEgg.ParameterTypes as ve_types, numpy.oldnumeric as Numeric, math, types, pygame
...
class Presentation(VisionEgg.ClassWithParameters):
    ...
    parameters_and_defaults = {'viewports': (
       ...
       'go_duration': (
                     (5.0, 'seconds'),
                     ve_types.Sequence(ve_types.AnyOf(ve_types.Real, ve_types.String)),
    ...
    __slots__ = ('controllers', 'num_frame_controllers', 'frame_draw_times', 'time_sec_absolute',
                 'frames_absolute', 'in_go_loop', 'frames_dropped_in_last_go_loop',
                 'last_go_loop_start_time_absolute_sec', 'time_sec_since_go', 'frames_since_go')
    ...
    def remove_controller(self, class_with_parameters, parameter_name, controller=None):
        ...
        """
        if class_with_parameters is None:
            if parameter_name is None:
                if not isinstance(controller, Controller) and controller != None:
                    raise TypeError('When deleting a controller, specify an instance of VisionEgg.FlowControl.Controller class!')
                if controller == None:
                    self.controllers = []
                i = 0
                ...
                return
            if controller is None and (class_with_parameters is None or parameter_name is None):
    ...
    def __call_controllers(self, go_started=None, doing_transition=None):
        done_once = []
        for (parameters_instance, parameter_name, controller) in self.controllers:
            ...
            if evaluate:
                ...
                elif not controller.eval_frequency & NOT_BETWEEN_GO:
                    if controller.temporal_variables & TIME_SEC_SINCE_GO:
    ...
    def go(self):
        ...
        self.in_go_loop = 1
        swap_buffers = VisionEgg.Core.swap_buffers
        self.frames_dropped_in_last_go_loop = False
        p = self.parameters
        ...
        synclync_connection = VisionEgg.config._SYNCLYNC_CONNECTION
        if synclync_connection:
            import synclync
            synclync_connection.next_control_packet.action_flags += synclync.SL_CLEAR_VSYNC_COUNT + synclync.SL_CLEAR_NOTIFY_SWAPPED_COUNT + synclync.SL_CLEAR_FRAMESKIP_COUNT
            synclync_hack_done_once = 0
        self.__call_controllers(go_started=1, doing_transition=1)
        ...
        while current_duration_value < p.go_duration[0]:
            ...
            self.__call_controllers(go_started=1, doing_transition=0)
            for viewport in p.viewports:
                viewport.draw()

            if synclync_connection:
                if not synclync_hack_done_once:
                    synclync_connection.next_control_packet.action_flags += synclync.SL_NOTIFY_SWAPPED_BUFFERS + synclync.SL_NOTIFY_IN_GO_LOOP
        ...
        self.__call_controllers(go_started=0, doing_transition=1)
        if synclync_connection:
            synclync_connection.send_control_packet()
        try:
        ...
        if self.num_frame_controllers:
            impossibly_fast_frame_rate = 210.0
            if calculated_fps > impossibly_fast_frame_rate:
                logger = logging.getLogger('VisionEgg.FlowControl')
                logger.error('Frame by frame control desired, but average frame rate was %.2f frames per second-- faster than any display device (that I know of).  Set your drivers to sync buffer swapping to vertical retrace. (platform/driver dependent)' % calculated_fps)
        if abs(calculated_fps - VisionEgg.config.VISIONEGG_MONITOR_REFRESH_HZ) / float(VisionEgg.config.VISIONEGG_MONITOR_REFRESH_HZ) > self.parameters.warn_mean_fps_threshold:
            logger = logging.getLogger('VisionEgg.FlowControl')
            logger.warning('Calculated frames per second was %.3f, while the VISIONEGG_MONITOR_REFRESH_HZ variable is %s.' % (
             calculated_fps,
        ...
        if p.collect_timing_info:
            ...
            if longest_frame_draw_time_sec is not None:
                ...
                if longest_frame_draw_time_sec >= frame_skip_fraction * inter_frame_inteval:
                    ...
                    logger.warning('One or more frames took %.1f msec, which is signficantly longer than the expected inter frame interval of %.1f msec for your frame rate (%.1f Hz).' % (
                     longest_frame_draw_time_sec * 1000.0,
                     inter_frame_inteval * 1000.0,
                     VisionEgg.config.VISIONEGG_MONITOR_REFRESH_HZ))
                else:
                    logger.debug('Longest frame update was %.1f msec. Your expected inter frame interval is %f msec.' % (
                     longest_frame_draw_time_sec * 1000.0,
                     inter_frame_inteval * 1000.0))
    ...
    def export_movie_go(self, frames_per_sec=12.0, filename_suffix='.tif', filename_base='visionegg_movie', path='.'):
        ...
        import VisionEgg.Core, Image, os
        p = self.parameters
        self.time_sec_absolute = VisionEgg.time_func()
        ...
        if scp.red_bits is not None:
            ...
            if warn_about_movie_depth:
                logger.warning('Only saved 8 bit per pixel movie, even though your framebuffer supports more!')
    ...
    def between_presentations(self):
        ...
        import VisionEgg.Core
        self.time_sec_absolute = VisionEgg.time_func()
        self.__call_controllers(go_started=0, doing_transition=0)
...
class Controller(object):
    ...
    \x07ttribute{temporal_variables}.
    ...
    NOT_DURING_GO = 8
    NOT_BETWEEN_GO = 16
    flag_dictionary = {'TIME_INDEPENDENT': TIME_INDEPENDENT, 
    ...
    def __init__(self, eval_frequency=EVERY_FRAME, temporal_variables=TIME_SEC_SINCE_GO, return_type=None):
...
class ConstantController(Controller):
    ...
    def __init__(self, during_go_value=None, between_go_value=None, **kw):
...
class EvalStringController(Controller):
    ...
    def __init__(self, during_go_eval_string=None, between_go_eval_string=None, **kw):
        import VisionEgg.Core
        self.eval_globals = {}
...
class ExecStringController(Controller):
    ...
    def __init__(self, during_go_exec_string=None, between_go_exec_string=None, restricted_namespace=1, **kw):
        import VisionEgg.Core
        self.eval_globals = {}
...
class FunctionController(Controller):
    ...
    def __init__(self, during_go_func=None, between_go_func=None, **kw):
        ...
        kw.setdefault('temporal_variables', TIME_SEC_SINCE_GO)
        if kw.get('return_type', None) is None:
            logger = logging.getLogger('VisionEgg.FlowControl')
...
class EncapsulatedController(Controller):
    ...
    def between_go_eval(self):
        """Called by Presentation. Overrides method in Controller base class."""
        import VisionEgg.Core, VisionEgg.FlowControl
        if self.temporal_variables & TIME_SEC_ABSOLUTE:
            self.contained_controller.time_sec_absolute = self.time_sec_absolute
        if self.temporal_variables & FRAMES_ABSOLUTE:
            self.contained_controller.frames_absolute = self.frames_absolute
