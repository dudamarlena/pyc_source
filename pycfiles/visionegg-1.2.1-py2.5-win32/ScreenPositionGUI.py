...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\VisionEgg\PyroApps\ScreenPositionGUI.py
# Compiled at: 2009-07-15 18:56:28
"""Handle 3D perspective projection (client-side)"""
import sys, os, pickle, math, string, Tkinter, tkFileDialog, Pyro.core, VisionEgg.PyroClient, StringIO
...
class ScreenPositionControlFrame(Tkinter.Frame):
    ...
    def __init__(self, master=None, auto_connect=0, server_hostname='', server_port=7766, **kw):
        ...
        row = 0
        Tkinter.Label(self, text='3D Perspective Calibration', font=('Helvetica', 12,
                                                                     'bold')).grid(row=row, column=0, columnspan=2)
        row += 1
        Tkinter.Label(self, text='This dialog allows you to enter acheive ' + 'the proper perspective distortion for 3D scenes.').grid(row=row, column=0, columnspan=2)
        if not auto_connect:
            row += 1
        ...
        ff_row = 0
        Tkinter.Label(frustum_frame, text='Viewing volume size', font=('Helvetica',
                                                                       12, 'bold')).grid(row=ff_row, column=0, columnspan=3, ipady=5)
        ff_row += 1
        ...
        CallbackEntry(frustum_frame, self.send_values, width=self.entry_width, textvariable=self.left_tk_var).grid(row=ff_row, column=1)
        ff_row += 1
        ...
        CallbackEntry(frustum_frame, self.send_values, width=self.entry_width, textvariable=self.right_tk_var).grid(row=ff_row, column=1)
        ff_row += 1
        ...
        CallbackEntry(frustum_frame, self.send_values, width=self.entry_width, textvariable=self.top_tk_var).grid(row=ff_row, column=1)
        ff_row += 1
        ...
        CallbackEntry(frustum_frame, self.send_values, width=self.entry_width, textvariable=self.bottom_tk_var).grid(row=ff_row, column=1)
        ff_row += 1
        ...
        CallbackEntry(frustum_frame, self.send_values, width=self.entry_width, textvariable=self.near_tk_var).grid(row=ff_row, column=1)
        ff_row += 1
        ...
        CallbackEntry(frustum_frame, self.send_values, width=self.entry_width, textvariable=self.far_tk_var).grid(row=ff_row, column=1)
        ...
        la_row = 0
        Tkinter.Label(lookat_frame, text='Viewing volume orientation', font=('Helvetica',
                                                                             12,
                                                                             'bold')).grid(row=la_row, column=0, columnspan=3, ipady=5)
        la_row += 1
        ...
        CallbackEntry(lookat_frame, self.send_values, width=self.entry_width, textvariable=self.eye_x_tk_var).grid(row=la_row, column=1)
        la_row += 1
        ...
        CallbackEntry(lookat_frame, self.send_values, width=self.entry_width, textvariable=self.eye_y_tk_var).grid(row=la_row, column=1)
        la_row += 1
        ...
        CallbackEntry(lookat_frame, self.send_values, width=self.entry_width, textvariable=self.eye_z_tk_var).grid(row=la_row, column=1)
        la_row += 1
        ...
        CallbackEntry(lookat_frame, self.send_values, width=self.entry_width, textvariable=self.center_x_tk_var).grid(row=la_row, column=1)
        ...
        CallbackEntry(lookat_frame, self.send_values, width=self.entry_width, textvariable=self.center_y_tk_var).grid(row=la_row, column=1)
        la_row += 1
        ...
        CallbackEntry(lookat_frame, self.send_values, width=self.entry_width, textvariable=self.center_z_tk_var).grid(row=la_row, column=1)
        la_row += 1
        ...
        CallbackEntry(lookat_frame, self.send_values, width=self.entry_width, textvariable=self.up_x_tk_var).grid(row=la_row, column=1)
        ...
        CallbackEntry(lookat_frame, self.send_values, width=self.entry_width, textvariable=self.up_y_tk_var).grid(row=la_row, column=1)
        la_row += 1
        ...
        CallbackEntry(lookat_frame, self.send_values, width=self.entry_width, textvariable=self.up_z_tk_var).grid(row=la_row, column=1)
        row += 1
        button_row_frame = Tkinter.Frame(self)
        button_row_frame.grid(row=row, column=0, ipady=5)
        Tkinter.Label(button_row_frame, text='File operations', font=('Helvetica',
                                                                      12, 'bold')).grid(row=0, column=0, columnspan=2, ipady=5)
    ...
    def az_increase(self, dummy_arg=None):
        (az, el) = self.get_az_el(self.meta_params.center)
        az = az + 5.0
        self.look_at_az_str.set('%.1f' % az)
    ...
    def az_decrease(self, dummy_arg=None):
        (az, el) = self.get_az_el(self.meta_params.center)
        az = az - 5.0
        self.look_at_az_str.set('%.1f' % az)
        (x, y, z) = self.get_xyz((az, el))
    ...
    def el_increase(self, dummy_arg=None):
        (az, el) = self.get_az_el(self.meta_params.center)
        el = el + 5.0
        el = min(el, 90.0)
        self.look_at_el_str.set('%.1f' % el)
    ...
    def el_decrease(self, dummy_arg=None):
        (az, el) = self.get_az_el(self.meta_params.center)
        el = el - 5.0
        el = max(el, -90.0)
        self.look_at_el_str.set('%.1f' % el)
    ...
    def up_az_increase(self, dummy_arg=None):
        (az, el) = self.get_az_el(self.meta_params.up)
        az = az + 5.0
        self.up_az_str.set('%.1f' % az)
        (x, y, z) = self.get_xyz((az, el))
    ...
    def up_az_decrease(self, dummy_arg=None):
        (az, el) = self.get_az_el(self.meta_params.up)
        az = az - 5.0
        self.up_az_str.set('%.1f' % az)
        (x, y, z) = self.get_xyz((az, el))
    ...
    def up_el_increase(self, dummy_arg=None):
        (az, el) = self.get_az_el(self.meta_params.up)
        el = el + 5.0
        el = min(el, 90.0)
        self.up_el_str.set('%.1f' % el)
    ...
    def up_el_decrease(self, dummy_arg=None):
        (az, el) = self.get_az_el(self.meta_params.up)
        el = el - 5.0
        el = max(el, -90.0)
        self.up_el_str.set('%.1f' % el)
