...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\VisionEgg\DaqLPT.py
...
"""
import VisionEgg, VisionEgg.Core, VisionEgg.FlowControl, VisionEgg.Daq, VisionEgg.ParameterTypes as ve_types, sys
...
class LPTChannel(VisionEgg.Daq.Channel):
    ...
    def __init__(self, **kw):
        if 'raw_lpt_module' not in globals().keys():
...
class LPTDevice(VisionEgg.Daq.Device):
    ...
    def __init__(self, base_address=888, **kw):
        if 'raw_lpt_module' not in globals().keys():
            raise RuntimeError('LPT output not supported on this platform.')
...
class LPTTriggerOutController(VisionEgg.FlowControl.Controller):
    ...
    def __init__(self, lpt_device=None):
        if 'raw_lpt_module' not in globals().keys():
            raise RuntimeError('LPT output not supported on this platform.')
        VisionEgg.FlowControl.Controller.__init__(self, return_type=ve_types.NoneType, eval_frequency=VisionEgg.FlowControl.Controller.EVERY_FRAME)
        self.trigger_out_channel = LPTChannel(signal_type=VisionEgg.Daq.Digital(), daq_mode=VisionEgg.Daq.Immediate(), functionality=LPTOutput())
...
class LPTTriggerInController(VisionEgg.FlowControl.Controller):
    ...
    def __init__(self, lpt_device=None, pin=13):
        ...
        self.trigger_in_channel = LPTChannel(signal_type=VisionEgg.Daq.Digital(), daq_mode=VisionEgg.Daq.Immediate(), functionality=LPTInput())
        if lpt_device is None:
            self.device = LPTDevice()
        else:
            if not isinstance(lpt_device, LPTDevice):
