...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\VisionEgg\PyroApps\EPhysGUIUtils.py
# Compiled at: 2009-07-15 18:56:28
import sys, os, time, types, socket, Tkinter, Pyro.core, VisionEgg.PyroClient

class StimulusControlFrame(Tkinter.Frame):

    def __init__(self, master=None, suppress_go_buttons=0, title='Stimulus Control', meta_params_class=None, **kw):
        ...
        Tkinter.Label(self, text=title, font=('Helvetica', 12, 'bold')).pack(expand=Tkinter.YES, fill=Tkinter.X)
        if not suppress_go_buttons:
            connected_frame = Tkinter.Frame(self)
            connected_frame.pack(expand=Tkinter.YES, fill=Tkinter.X)
            ...
            Tkinter.Label(connected_frame, text='Server hostname').grid(row=0, column=0)
            Tkinter.Entry(connected_frame, textvariable=self.server_hostname).grid(row=0, column=1, columnspan=2)
            Tkinter.Label(connected_frame, textvariable=self.connected_text).grid(row=1, column=0)
            Tkinter.Button(connected_frame, text='Connect', command=self.standalone_connect).grid(row=1, column=1)
        ...
        if not suppress_go_buttons:
            Tkinter.Button(self, text='Begin Trial', command=self.go).pack()
        return
    ...
    def set_parameters_dict(self, dict):
        for key in dict.keys():
            if key not in dir(self.meta_params):
    ...
    def connect(self, server_hostname, server_port):
        ...
        self.connected = 1
        if hasattr(self, 'connected_text'):
