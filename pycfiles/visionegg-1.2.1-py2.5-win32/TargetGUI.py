...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\VisionEgg\PyroApps\TargetGUI.py
# Compiled at: 2009-07-15 18:56:28
"""Handle small target stimulus (client-side)"""
import VisionEgg, sys, os, math, Tkinter, VisionEgg.PyroApps.EPhysGUIUtils as client_utils

def get_control_list():
    return [
     (
      'target_server', TargetControlFrame, TargetControlFrame.title)]
...
class TargetControlFrame(client_utils.StimulusControlFrame):
    ...
    def __init__(self, master=None, suppress_go_buttons=0, **kw):
        client_utils.StimulusControlFrame.__init__(self, master, suppress_go_buttons, TargetControlFrame.title, TargetMetaParameters, **kw)
        ...
        bar.menu.add_radiobutton(label='white on black', value='white on black', variable=self.color_tk_var, command=self.send_values)
        bar.menu.add_radiobutton(label='black on white', value='black on white', variable=self.color_tk_var, command=self.send_values)
        bar['menu'] = bar.menu
        pf_row += 1
        ...
        manual = Tkinter.Radiobutton(param_frame, text='Manual', variable=self.ortho_tk_var, value='manual')
        ...
        manual_entry = Tkinter.Entry(param_frame, textvariable=self.orient_tk_var, width=self.entry_width)
        manual_entry.grid(row=pf_row, column=2)
        ortho = Tkinter.Radiobutton(param_frame, text='Orthogonal to motion', variable=self.ortho_tk_var, value='ortho')
    ...
    def update_tk_vars(self):
        ...
        self.stim_dur_tk_var.set(self.meta_params.stim_sec)
        self.poststim_dur_tk_var.set(self.meta_params.post_stim_sec)
        if self.meta_params.color == (0.0, 0.0, 0.0, 1.0) and self.meta_params.bgcolor == (1.0,
                                                                                           1.0,
                                                                                           1.0,
                                                                                           0.0):
            self.color_tk_var.set('black on white')
        elif self.meta_params.color == (1.0, 1.0, 1.0, 1.0) and self.meta_params.bgcolor == (0.0,
                                                                                             0.0,
                                                                                             0.0,
                                                                                             0.0):
