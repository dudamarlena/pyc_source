...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\VisionEgg\Configuration.py
...
"""
import VisionEgg, re, os, errno, sys, ConfigParser
defaults = {'VISIONEGG_ALWAYS_START_LOGGING': 0, 
...
extra_darwin_defaults = {'VISIONEGG_DARWIN_MAXPRIORITY_CONVENTIONAL_NOT_REALTIME': 1, 
...
class Config:
    ...
    def __init__(self):
        ...
        self._delayed_configuration_log_warnings = []
        for option in general_options:
            name = option.upper()
            if name not in defaults.keys():
                self._delayed_configuration_log_warnings.append('While reading %s: The variable "%s" is not (anymore) a Vision Egg variable.' % (os.path.abspath(configFile), option))
                continue
...
def save_settings():
    ...
    re_setting_finder = re.compile('^\\s?((?:VISIONEGG_[A-Z_]*)|(?:SYNCLYNC_[A-Z_]*))\\s?=\\s?(\\S*)\\s?$', re.IGNORECASE)
