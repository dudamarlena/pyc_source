...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\VisionEgg\PyroApps\SphereGratingGUI.py
...
def get_control_list():
    return [
     (
      'sphere_grating_server', SphereGratingControlFrame, SphereGratingControlFrame.title)]
...
class SphereGratingControlFrame(client_utils.StimulusControlFrame):
    ...
    def __init__(self, master=None, suppress_go_buttons=0, **kw):
        client_utils.StimulusControlFrame.__init__(self, master, suppress_go_buttons, SphereGratingControlFrame.title, SphereGratingMetaParameters, **kw)
        ...
        bar.menu.add_radiobutton(label='circle', value='circle', variable=self.window_func_tk_var, command=self.send_values)
        bar.menu.add_radiobutton(label='gaussian', value='gaussian', variable=self.window_func_tk_var, command=self.send_values)
        bar['menu'] = bar.menu
        pf_row += 1
