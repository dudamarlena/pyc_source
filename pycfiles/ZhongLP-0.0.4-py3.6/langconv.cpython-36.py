...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ZhongLP/converter_pk/langconv.py
# Compiled at: 2019-12-20 10:06:13
# Size of source mod 2**32: 8034 bytes
        ...
        if node.have_child:
            ...
            if cond == ERROR:
                ...
                if cond == TAIL:
                    ...
                    if self.state == START or self.state == WAIT_TAIL:
                        if cond == MATCHED_SWITCH:
                            ...
                            if cond == UNMATCHED_SWITCH or cond == CONNECTOR:
                                if self.state == START:
                                    new = self.clone(node.from_word)
                                    self.final += node.to_word
                                    self.len += 1
                        ...
                        if self.state == END:
                            ...
                            raise StatesMachineException('Translate States Machine have error with input data %s' % node)
        return new
...
class Converter(object):
    ...
    def end(self):
        self.machines = [fsm for fsm in self.machines if fsm.state == FAIL or fsm.state == END]
        self._clean()
