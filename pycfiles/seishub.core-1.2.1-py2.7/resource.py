...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\core\xmldb\resource.py
...
from seishub.core.xmldb.defaults import resource_tab, document_tab, document_meta_tab
...
class XmlDocument(Serializable):
    ...
    db_mapping = {'_id': 'id', 'revision': 'revision', 
    ...
    data = db_property(getData, setData, 'Raw xml data as a string', attr='_data')
...
class Resource(Serializable):
    ...
    db_mapping = {'_id': 'id', 'resourcetype': Relation(ResourceTypeWrapper, 'resourcetype_id', lazy=False), 
       'name': 'name', 
       'document': Relation(XmlDocument, 'resource_id', lazy=False, relation_type='to-many', cascading_delete=True)}

    def __init__(self, resourcetype=ResourceTypeWrapper(), id=None, document=None, name=None):
    ...
    document = db_property(getDocument, setDocument, 'xml document', attr='_document')
