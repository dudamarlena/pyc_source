...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\core\registry\package.py
# Compiled at: 2010-12-23 17:42:44
from seishub.core.db.orm import Serializable, Relation, db_property
from seishub.core.registry.defaults import schema_tab, stylesheet_tab, alias_tab, packages_tab, resourcetypes_tab
from seishub.core.util.text import validate_id, to_uri
from seishub.core.util.xmlwrapper import XmlSchema, XmlStylesheet
from seishub.core.xmldb.interfaces import IResource
import seishub.core.xmldb.resource

class PackageWrapper(Serializable):
    """Wrapped around packages for storage in database."""
    db_table = packages_tab
    db_mapping = {'package_id': 'name', 'version': 'version', 
       '_id': 'id'}
    ...
    package_id = property(getPackage_id, setPackage_id, 'unique package id (string)')
...
class ResourceTypeWrapper(Serializable):
    ...
    db_mapping = {'resourcetype_id': 'name', 'package': Relation(PackageWrapper, 'package_id', lazy=False), 
       'version': 'version', 
       'version_control': 'version_control', 
       '_id': 'id'}

    def __init__(self, resourcetype_id=None, package=PackageWrapper(), version='', version_control=False):
    ...
    def __str__(self):
        return str(self.package.package_id) + '/' + str(self.resourcetype_id) + '/' + self.version
    ...
    resourcetype_id = property(getResourcetype_id, setResourcetype_id, 'unique package id (string)')
    ...
    version_control = property(getVersion_control, setVersion_control, 'Boolean indicating if version control is enabled')
...
class Alias(Serializable):
    ...
    def setURI(self, data):
        if data is not None and not isinstance(data, basestring):
            raise TypeError('Invalid alias URI, String expected: %s' % data)
        self._uri = data
        return

    uri = property(getURI, setURI, 'URI of the alias')
    ...
    def setExpr(self, data):
        if data is not None and not isinstance(data, basestring):
            raise TypeError('Invalid alias expression, String expected: %s' % data)
        self._expr = data
        return
...
class DocBase(Serializable):
    db_mapping = {'resourcetype': Relation(ResourceTypeWrapper, 'resourcetype_id'), 
       'package': Relation(PackageWrapper, 'package_id'), 
       'type': 'type', 
       'document_id': 'document_id'}

    def __init__(self, package=PackageWrapper(), resourcetype=ResourceTypeWrapper(), type=None, document_id=None):
        super(Serializable, self).__init__()
        self.package = package
    ...
    def __str__(self):
        return to_uri(self.package.package_id, self.resourcetype.resourcetype_id) + '/' + self.type
    ...
    resourcetype = db_property(getResourceType, setResourceType, 'Resource type', attr='_resourcetype')
    ...
    document_id = property(getDocument_id, setDocument_id, 'Unique document identifier (integer)')
...
class Schema(DocBase):
    ...
    def validate(self, resource):
        if not self._parsed_doc:
            self._parsed_doc = XmlSchema(self.resource.document.data, self._type)
