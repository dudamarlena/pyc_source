...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\core\auth.py
...
from seishub.core.exceptions import NotFoundError, DuplicateObjectError, SeisHubError
...
class User(Base):
    ...
    def __init__(self, id, name, password, uid=1000, institution='', email='', permissions=755):
        self.id = id
        self.uid = uid
        self.name = name
...
class AuthenticationManager(object):
    ...
    """
    IntOption('seishub', 'min_password_length', MIN_PASSWORD_LENGTH, 'Minimum password length.')
    ...
    def __init__(self, env):
        self.env = env
        uri = self.env.config.get('seishub', 'auth_uri') or 'sqlite:///' + os.path.join(self.env.getInstancePath(), 'db', 'auth.db')
        engine = create_engine(uri, encoding='utf-8', convert_unicode=True)
        Base.metadata.create_all(engine, checkfirst=True)
        self.Session = sessionmaker(bind=engine)
        self.refresh()
        if not self.users:
            self.env.log.warn("An administrative account with both username and passwort 'admin' has been automatically created. You should change the password as soon as possible.")
            self.addUser(id='admin', name='Administrator', password='admin', uid=100, permissions=777, checkPassword=False)
            self.addUser(id='anonymous', name='Anonymous', password='', uid=101, permissions=755, checkPassword=False)
        elif self.checkPassword('admin', 'admin'):
            self.env.log.warn('The administrative account is accessible via the standard password! Please change this as soon as possible!')
    ...
    def addUser(self, id, name, password, uid=1000, institution='', email='', permissions=755, checkPassword=True):
        """
        Adds an user.
        """
        ...
        user = User(id=id, uid=uid, name=name, password=hash(password), institution=institution, email=email, permissions=permissions)
    ...
    def updateUser(self, id, name='', password='', uid=1000, institution='', email='', permissions=755):
        """
