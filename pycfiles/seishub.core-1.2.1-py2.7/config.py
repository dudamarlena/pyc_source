...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\core\config.py
...
import os
__all__ = [
 'Configuration', 'Option', 'BoolOption', 'IntOption', 'ListOption',
 'ConfigurationError']
_TRUE_VALUES = (
 'yes', 'true', 'on', '1', 1, True, 'enabled')
CRLF = '\r\n'
    ...
    def save(self):
        ...
        for section in self.sections():
            ...
            for option in self[section]:
                current = self.parser.has_option(section, option) and self.parser.get(section, option)
            ...
            for section, options in sections:
                ...
                for key, val in options:
                    if key in self[section].overridden:
                        print >> fileobj, '# %s = <set in global seishub.ini>' % key
                    else:
...
class ListOption(Option):
    ...
    def __init__(self, section, name, default=None, sep=',', keep_empty=False, doc=''):
