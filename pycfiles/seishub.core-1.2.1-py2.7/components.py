...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\core\packages\admin\web\components.py
...
class SchemasPanel(Component):
            ...
            if 'file' in args.keys():
                ...
                if type and package_id in packages:
                    if resourcetype_id in resourcetypes.get(package_id, []):
                        data.update(self._addSchema(package_id, resourcetype_id, type, file))
    ...
    def _addSchema(self, package_id, resourcetype_id, type, file):
        try:
            self.registry.schemas.register(package_id, resourcetype_id, type, file)
...
class StylesheetsPanel(Component):
    ...
    def render(self, request):
        ...
        if request.method == 'POST':
            ...
            if 'file' in args.keys():
                ...
                if type and package_id in packages:
                    data.update(self._addStylesheet(package_id, resourcetype_id, type, file))
    ...
    def _addStylesheet(self, package_id, resourcetype_id, type, file):
        try:
            self.registry.stylesheets.register(package_id, resourcetype_id, type, file)
        except Exception as e:
...
class BrowserPanel(Component):
    ...
    panel_ids = ('components', 'Components', 'browse-components', 'Component Browser')
...
class IndexesPanel(Component):
    ...
    def _getIndexes(self, data):
        data['indexes'] = self.catalog.getIndexes(package_id=data['package_id'], resourcetype_id=data['resourcetype_id'])
        return data
    ...
    def _reindexResourceType(self, data):
        try:
            self.env.catalog.reindexResourceType(package_id=data['package_id'], resourcetype_id=data['resourcetype_id'])
        except Exception as e:
    ...
    def _addIndex(self, data):
        try:
            self.catalog.registerIndex(data['package_id'], data['resourcetype_id'], data['label'], data['xpath'], data['type_id'], options=data['options'])
        except Exception as e:
...
class FileSystemPanel(Component):
    ...
    panel_ids = ('components', 'Components', 'edit-filesystem', 'FileSystem')
