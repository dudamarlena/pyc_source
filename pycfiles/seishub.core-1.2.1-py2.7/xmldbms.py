...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\core\xmldb\xmldbms.py
# Compiled at: 2010-12-23 17:42:44
from seishub.core.db.orm import DbStorage, DbError, DB_LIMIT
from seishub.core.exceptions import DuplicateObjectError, NotFoundError, InvalidParameterError
from seishub.core.xmldb.defaults import resource_tab
from seishub.core.xmldb.resource import XmlDocument, Resource
...
class XmlDbManager(DbStorage):
    ...
    def addResource(self, resource=Resource()):
        ...
        try:
            ...
            msg = 'Error adding a resource: A resource with the given ' + 'parameters already exists.'
    ...
    def modifyResource(self, old_resource, resource, uid=None):
        ...
        if not old_resource.resourcetype._id == resource.resourcetype._id:
            msg = 'Error modifying a resource: Resourcetypes of old and ' + 'new resource do not match. %s != %s'
    ...
    def renameResource(self, resource, new_name):
        ...
        try:
            ...
            msg = 'Error renaming a resource: A resource with the given ' + 'parameters already exists. (%s)'
            raise DuplicateObjectError(msg % str(resource), e)

    def _getResource(self, package_id=None, resourcetype_id=None, name=None, revision=None, id=None):
        if not revision:
        ...
        if name:
            name = str(name)
        try:
            res = self.pickup(Resource, resourcetype={'package': {'package_id': package_id}, 'resourcetype_id': resourcetype_id}, name=name, _id=id, document=document)[0]
        except IndexError:
            self._raise_not_found(package_id, resourcetype_id, name, id)

        return res

    def getResource(self, package_id=None, resourcetype_id=None, name=None, revision=None, document_id=None, id=None):
        """
        ...
        """
        if not (package_id and resourcetype_id and name or id or document_id):
            raise TypeError('getResource(): Invalid number of arguments.')
        if document_id:
            try:
                res = self.pickup(Resource, document={'_id': document_id})[0]
            except IndexError as e:
                raise NotFoundError("Resource not found. ('%s')" % document_id, e)

            return res
        res = self._getResource(package_id, resourcetype_id, name, revision, id)
        return res

    def getRevisions(self, package_id=None, resourcetype_id=None, name=None, id=None):
        """
        ...
        if name:
            name = str(name)
        try:
            res = self.pickup(Resource, _order_by={'document': {'revision': 'asc'}}, resourcetype={'package': {'package_id': package_id}, 'resourcetype_id': resourcetype_id}, name=name, _id=id)[0]
        except IndexError:
            self._raise_not_found(package_id, resourcetype_id, name, id)
    ...
    def getAllResources(self, package_id, resourcetype_id=None):
        ...
        """
        res = self.pickup(Resource, resourcetype={'package': {'package_id': package_id}, 'resourcetype_id': resourcetype_id}, document=DB_LIMIT('revision', 'max'))
        return res
    ...
    def deleteAllResources(self, package_id, resourcetype_id=None):
        ...
        """
        self.drop(Resource, resourcetype={'package': {'package_id': package_id}, 'resourcetype_id': resourcetype_id})

    def revertResource(self, package_id=None, resourcetype_id=None, name=None, revision=None, id=None):
        """
        Reverts the specified revision for the given resource by removing 
        all newer revisions than the specified one.
        """
        if not (revision and (package_id and resourcetype_id and name or id)):
    ...
    def getAllResourceNames(self, resourcetype, limit=100, ordered=False):
        ...
        query = query.where(resource_tab.c['resourcetype_id'] == resourcetype._id)
        if ordered:
