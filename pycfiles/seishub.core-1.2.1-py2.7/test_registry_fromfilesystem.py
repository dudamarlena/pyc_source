...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\core\registry\tests\test_registry_fromfilesystem.py
...
class APackage(Component):
    ...
    registerStylesheet('data' + os.sep + 'resourcelist_json.xslt', 'resourcelist')
...
class PackageRegistryFilesystemTest(SeisHubEnvironmentTestCase):
    ...
    def test_registerSchema(self):
        ...
        p2 = os.path.join(self.env.getPackagePath(), 'seishub', 'core', 'registry', 'tests', 'path/to/file2')
        assert {'filename': p2, 'type': 'xhtml'} in FooBar._registry_schemas
    ...
    def test_registerStylesheet(self):
        ...
        p1 = os.path.join(self.env.getPackagePath(), 'seishub', 'core', 'registry', 'tests', 'path/to/file1')
        p2 = os.path.join(self.env.getPackagePath(), 'seishub', 'core', 'registry', 'tests', 'path/to/file2')
        assert {'filename': p1, 'type': 'xhtml'} in Bar._registry_stylesheets
        assert {'filename': p2, 'type': 'xhtml'} in FooBar._registry_stylesheets
    ...
    def test_autoInstaller(self):
        ...
        stylesheet = self.registry.stylesheets.get('atestpackage', 'aresourcetype', 'aformat')
        self.assertEqual(len(stylesheet), 1)
        p = os.path.join(self.env.getPackagePath(), 'seishub', 'core', 'registry', 'tests', 'data', 'resourcelist_json.xslt')
        self.assertEqual(stylesheet[0].resource.document.data, file(p).read())
        stylesheet = self.registry.stylesheets.get('atestpackage', None, 'resourcelist')
        self.assertEqual(len(stylesheet), 1)
        p = os.path.join(self.env.getPackagePath(), 'seishub', 'core', 'registry', 'tests', 'data', 'resourcelist_json.xslt')
        self.assertEqual(stylesheet[0].resource.document.data, file(p).read())
        self.registry.stylesheets.delete(stylesheet[0].package.package_id, stylesheet[0].resourcetype.resourcetype_id, stylesheet[0].type)
