...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\core\db\manager.py
# Compiled at: 2011-01-03 17:15:11
"""
The database manager.
"""
from seishub.core.config import Option, IntOption
from seishub.core.db import DEFAULT_MAX_OVERFLOW, DEFAULT_POOL_SIZE, DEFAULT_DB_URI
...
class DatabaseManager(object):
    ...
    IntOption('db', 'max_overflow', DEFAULT_MAX_OVERFLOW, "The number of connections to allow in connection pool 'overflow', that is connections that can be opened above and beyond the " + 'pool_size setting, which defaults to five.')
    IntOption('db', 'pool_size', DEFAULT_POOL_SIZE, 'The number of connections to keep open inside the connection pool.')
    ...
    def __init__(self, env):
        ...
        self.max_overflow = self.env.config.getint('db', 'max_overflow') or DEFAULT_MAX_OVERFLOW
        self.pool_size = self.env.config.getint('db', 'pool_size') or DEFAULT_POOL_SIZE
    ...
    def _initDb(self):
        ...
        except sa.exceptions.DBAPIError as e:
    ...
    def _getEngine(self):
        ...
        if self.uri.startswith('sqlite:///'):
            ...
            if len(filepart) == 1:
                self.uri = 'sqlite:///' + os.path.join(self.env.getInstancePath(), 'db', filename)
                return self._getSQLiteEngine()
            if len(filepart) == 2 and filepart[0] == 'db':
                self.uri = 'sqlite:///' + os.path.join(self.env.getInstancePath(), filename)
                return self._getSQLiteEngine()
            ...
            return self._getSQLiteEngine()
        return sa.create_engine(self.uri, echo=self.echo, encoding='utf-8', convert_unicode=True, max_overflow=self.max_overflow, pool_size=self.pool_size)

    def _getSQLiteEngine(self):
        """
        Return a SQLite engine without a connection pool.
        """
        self.env.log.warn('A SQLite database should never be used in a productive environment!')
        return sa.create_engine(self.uri, echo=self.echo, encoding='utf-8', convert_unicode=True)
