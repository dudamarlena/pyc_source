...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\core\packages\admin\web\general.py
...
class BasicPanel(Component):
    ...
    def render(self, request):
        data = {'log_levels': dict([ (v, k) for k, v in LOG_LEVELS.iteritems() ]), 
        ...
        data.update({'instance': self.config.path, 
           ...
           'clear_logs_on_startup': self.config.getbool('seishub', 'clear_logs_on_startup')})
        return data
...
class DatabasePanel(Component):
    ...
    def render(self, request):
        ...
        if db.engine.name == 'sqlite':
            data['info'] = ('SQLite Database enabled!', "A SQLite database should never be used in a productive environment!<br />Instead try to use any supported database listed at <a href='http://www.sqlalchemy.org/trac/wiki/DatabaseNotes'>http://www.sqlalchemy.org/trac/wiki/DatabaseNotes</a>.")
        if request.method == 'POST':
            uri = request.args0.get('uri', '')
            pool_size = request.args0.get('pool_size', DEFAULT_POOL_SIZE)
            max_overflow = request.args0.get('max_overflow', DEFAULT_MAX_OVERFLOW)
            ...
            try:
                ...
                data['error'] = (
                 'Could not connect to database %s' % uri,
                 'Please make sure the database URI has ' + 'the correct syntax: dialect://user:' + 'password@host:port/dbname.')
            else:
                self.config.set('db', 'uri', uri)
                data['info'] = ('Connection to database was successful',
                 'You have to restart SeisHub in order to ' + 'see any changes at the database settings.')
...
class LogsPanel(Component):
    ...
    def render(self, request):
        log_file = os.path.join(self.env.getInstancePath(), 'logs', 'seishub.log')
        try:
            fh = open(log_file, 'r')
            logs = fh.readlines()
            fh.close()
...
class UsersPanel(Component):
    ...
    def _addUser(self, args):
        ...
        if not id:
            ...
            try:
                self.auth.addUser(id=id, name=name, password=password, uid=uid, email=email, institution=institution, permissions=permissions)
    ...
    def _editUser(self, args):
        ...
        if not id:
            ...
            try:
                self.auth.updateUser(id=id, name=name, password=password, uid=uid, email=email, institution=institution, permissions=permissions)
            except SeisHubError as e:
                data['error'] = str(e)
            except Exception as e:
...
class PluginsPanel(Component):
    ...
    def _updatePlugins(self, request):
        ...
        for component in ComponentMeta._components:
            ...
            if classname in enabled or classname in DEFAULT_COMPONENTS or modulename in DEFAULT_COMPONENTS:
    ...
    def _viewPlugins(self, request, error=None):
        ...
        for component in ComponentMeta._components:
            ...
            plugin = {'name': component.__name__, 
               ...
               'required': classname in DEFAULT_COMPONENTS or modulename in DEFAULT_COMPONENTS}
            packagename = ('.').join(modulename.split('.')[0:3])
            plugins.setdefault(packagename, {})
            plugins[packagename].setdefault(modulename, {})
            plugins[packagename][modulename].setdefault('plugins', []).append(plugin)
            plugins[packagename][modulename]['description'] = description

        data = {'sorted_plugins': sorted(plugins), 'plugins': plugins, 
           'error': error}
        return data
...
class ServicesPanel(Component):
    ...
    def render(self, request):
        ...
        data = {'services': service.IServiceCollection(self.env.app)}
        return data
    ...
    def _changeServices(self, request):
        ...
        for srv in service.IServiceCollection(self.env.app):
            if srv.running and srv.service_id not in serviceList:
