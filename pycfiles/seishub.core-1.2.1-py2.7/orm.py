...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\core\db\orm.py
...
from zope.interface import implements, Interface, directlyProvides, implementedBy, Attribute
...
class DbStorage(DbEnabled):
    ...
    def _to_kwargs(self, o):
        ...
        for field, col in o.db_mapping.iteritems():
            ...
            if IRelation.providedBy(col) and col.relation_type == 'to-one':
                if value:
                    if not isinstance(value, col.cls):
                        raise AssertionError
                        raise (value._id or DbError)('A related object could not be ' + 'located in the database. %s: %s' % (
            ...
            if IRelation.providedBy(col):
                if isinstance(val, dict):
                    try:
                        ...
                        raise DbError('A related object could not be ' + 'located in the database. %s: %s' % (
                         col.cls, str(val)))
    ...
    def _generate_query(self, q, table, mapping, params, joins=list(), order_by=dict()):
        ...
        for attr, col in mapping.iteritems():
            ...
            if IRelation.providedBy(col):
                if col.relation_type == 'to-one':
                    ...
                    colname = 'id'
                    relname = col.name
            if IRelation.providedBy(col) and value is not DB_NULL:
                ...
                if col.lazy and value and '_id' in value.keys() and attr not in order_by.keys():
                    ...
                    if isinstance(value, DB_LIMIT):
                        ...
                        value = None
                    q, joins = self._generate_query(q, rel_tab, col.cls.db_mapping, value, joins, rel_order_by)
            elif value == DB_NULL:
                q = q.where(table.c[colname] == None)
            elif value:
                q = q.where(table.c[colname] == value)
            if not ILazyAttribute.providedBy(col) and not (IRelation.providedBy(col) and col.relation_type == 'to-many'):
                ...
                if IRelation.providedBy(col):
                    if col.relation_type == 'to-one':
                        ...
                        if col.lazy:
                            values[attr] = col.cls()
                            if rel_id:
                                values[attr] = DbObjectProxy(self, col.cls, _id=rel_id)
                        else:
                    ...
                    values[attr] = DbAttributeProxy(self, col, table, {'id': result[(str(table) + '_id')]})
                else:
    ...
    def _order_by(self, q, table, mapping, order_by=dict()):
        for col, direction in order_by.iteritems():
            if isinstance(direction, dict):
                q = self._order_by(q, mapping[col].cls.db_table, mapping[col].cls.db_mapping, direction)
                continue
    ...
    def pickup(self, cls, **keys):
        ...
        q, join_list = self._generate_query(q, table, map, keys, order_by=order_by, joins=list())
        if join_list and len(join_list) >= 2:
            left_tab = join_list[0][1][0]
            joins = left_tab.outerjoin(join_list[1][0], onclause=or_(*join_list[1][1]))
            del join_list[:2]
            for j in join_list:
    ...
    def drop(self, cls, **keys):
        ...
        if hasattr(self, 'debug') and self.debug:
            print 'DBUTIL: Deletion completed in %s seconds.' % (time.time() - start)
        return ret
...
class Serializable(object):
    ...
    def _setId(self, id):
        if id and not (isinstance(id, int) or isinstance(id, long)):
            raise TypeError('Id has to be integer or long. Got a %s.' % str(type(id)))
...
class DbObjectProxy(object):
    ...
    def get(self):
        try:
            ...
            raise DbError('A related object could not be located in ' + 'the database. %s: %s' % (self.cls, self.kwargs))
...
class DbAttributeProxy(object):
    ...
    def get(self):
        ...
        try:
            ...
            raise DbError('An error occurred while getting related object ' + 'data "%s": %s' % (self.attr_name, self.keyargs))
...
class db_property(property):
    ...
    def __get__(self, obj, objtype):
        ...
        if IDbObjectProxy.providedBy(data) or IDbAttributeProxy.providedBy(data):
...
class Relation(object):
    ...
    def __init__(self, cls, name, lazy=True, cascading_delete=False, relation_type='to-one'):
        self.cls = cls
        self.name = name
        self.lazy = lazy
