...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\core\services\sftp.py
# Compiled at: 2010-12-23 17:42:43
"""
A SFTP server.
"""
from seishub.core.config import IntOption, Option, BoolOption
from seishub.core.defaults import SFTP_AUTOSTART, SFTP_PORT, SFTP_PRIVATE_KEY, SFTP_PUBLIC_KEY
from seishub.core.exceptions import InternalServerError, ForbiddenError, NotFoundError, SeisHubError
from seishub.core.processor import Processor, PUT, DELETE, GET, MOVE, HEAD, getChildForRequest
from seishub.core.processor.interfaces import IFileSystemResource, IStatical, IResource, IScriptResource, IRESTResource, IXMLIndex, IAdminResource
...
from twisted.conch.ssh.filetransfer import SFTPError, FX_FILE_ALREADY_EXISTS, FX_FAILURE, FX_OP_UNSUPPORTED, FXF_READ, FXF_WRITE, FX_NO_SUCH_FILE
...
class SFTPProcessor(Processor):
    ...
    def render(self):
        ...
        if IStatical.providedBy(child):
            return child.render(self)
        if IScriptResource.providedBy(child):
            msg = 'Script resources may not be called via SFTP.'
            raise ForbiddenError(msg)
        else:
            if IRESTResource.providedBy(child):
                return child.render(self)
...
class SFTPServiceProtocol():
    ...
    def renameFile(self, oldpath, newpath):
        ...
        try:
            proc.run(MOVE, oldpath, received_headers={'Destination': destination})
        except ForbiddenError as e:
            raise SFTPError(FX_OP_UNSUPPORTED, e.message)
        except SeisHubError as e:
            raise SFTPError(FX_FAILURE, e.message)
...
class DirList():
    ...
    def next(self):
        ...
        s = st()
        attrs['permissions'] = s.st_mode = attrs.get('permissions', 16877)
        attrs['uid'] = s.st_uid = attrs.get('uid', 0)
        attrs['gid'] = s.st_gid = attrs.get('gid', DEFAULT_GID)
        attrs['size'] = s.st_size = attrs.get('size', 0)
        attrs['atime'] = s.st_atime = float(attrs.get('atime', self.env.startup_time))
        attrs['mtime'] = s.st_mtime = float(attrs.get('mtime', self.env.startup_time))
...
class SFTPServiceRealm():
    ...
    def requestAvatar(self, avatarId, mind, *interfaces):
        if IConchUser in interfaces:
            logout = lambda : None
            return (
             IConchUser, SFTPServiceAvatar(avatarId, self.env), logout)
...
class SFTPServiceFactory(SSHFactory):
    ...
    def __init__(self, env):
        self.env = env
        self.portal = portal.Portal(SFTPServiceRealm(env), env.auth.getCheckers())
