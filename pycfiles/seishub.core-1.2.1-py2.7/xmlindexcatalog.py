...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\core\xmldb\xmlindexcatalog.py
# Compiled at: 2010-12-23 17:42:44
from seishub.core.db import util
from seishub.core.db.orm import DbStorage, DbError
from seishub.core.exceptions import InvalidParameterError, SeisHubError, NotFoundError, InvalidObjectError, DuplicateObjectError
from seishub.core.registry.defaults import resourcetypes_tab, packages_tab
from seishub.core.xmldb.defaults import document_tab, resource_tab, document_meta_tab
from seishub.core.xmldb.index import XmlIndex, type_classes
...
class _IndexView(object):
    ...
    def updateIndexView(self, resourcetype):
        ...
        xmlindex_list = self.getIndexes(package_id=package_id, resourcetype_id=resourcetype_id)
    ...
    def _createIndexView(self, xmlindex_list, compact=False):
        ...
        oncl = sql.and_(resourcetypes_tab.c['id'] == resource_tab.c['resourcetype_id'], resourcetypes_tab.c['id'] == xmlindex.resourcetype._id)
...
class _QueryProcessor(object):
    ...
    def findIndex(self, package_id, resourcetype_id, expr):
        ...
        """
        if '/' not in expr:
            idx = self.getIndexes(package_id=package_id, resourcetype_id=resourcetype_id, label=expr)
            if idx:
                return idx[0]
        expr = '/' + expr
        if '*' in expr:
        ...
        idx = self.getIndexes(package_id=package_id, resourcetype_id=resourcetype_id, xpath=expr)
        if idx:
            return idx[0]
    ...
    def _join_on_index(self, idx, joins=None, method='outerjoin', complement=False):
        if joins == None:
            joins = document_tab
    ...
    def _joinIndexes(self, xmlindex_list, q, joins=None):
        ...
        for idx in xmlindex_list:
            ...
            if not grouping or idx_gp not in gp_paths:
                oncl = sql.and_(idx_tab.c['document_id'] == document_tab.c['id'], idx_tab.c['index_id'] == idx_id)
                if grouping:
                    gp_paths[idx_gp] = idx_tab
            else:
                oncl = sql.and_(idx_tab.c['document_id'] == document_tab.c['id'], idx_tab.c['index_id'] == idx_id, idx_tab.c['group_pos'] == gp_paths[idx_gp].c['group_pos'])
            joins = join(idx_tab, onclause=oncl)
    ...
    def _process_predicates(self, p, query, joins=None, complement=False):
        ...
        if len(p) == 3:
            ...
            if op in XPathQuery._relational_ops:
                ...
                joins, ltab = self._join_on_index(lidx, joins, complement=complement)
                if isinstance(r, list):
                    ridx = self.findIndex(r[0], r[1], r[2])
                    joins, rtab = self._join_on_index(ridx, joins, complement=complement)
                    w = ltab.c['keyval'] == rtab.c['keyval']
                else:
                    w = self._applyOp(op, ltab.c['keyval'], lidx.prepareKey(r))
            else:
                query, joins, lw = self._process_predicates(l, query, joins, complement=complement)
                query, joins, rw = self._process_predicates(r, query, joins, complement=complement)
            ...
            joins, idx_tab = self._join_on_index(idx, joins, complement=complement)
    ...
    def _process_order_by(self, order_by, query, joins=None):
        for ob in order_by:
            ...
            idx_name = str(idx)
            if idx_name not in query.columns:
            ...
            if id not in ordered:
                ...
                for key, val in idx_values.iteritems():
                    if isinstance(results[id][key], list):
                        if val not in results[id][key]:
                            results[id][key].append(val)
                    elif not results[id][key] == val:
                        results[id][key] = [
                         results[id][key]]
                        results[id][key].append(val)
    ...
    def query(self, xpath):
        ...
        if predicates:
            query, joins, w = self._process_predicates(predicates, query, joins)
            if w is not None:
                query = query.where(w)
        if order_by:
...
class XmlIndexCatalog(DbStorage, _QueryProcessor, _IndexView):
    ...
    def dumpIndexByResource(self, resource):
        ...
        """
        xmlindexes = self.getIndexes(package_id=resource.package._package_id, resourcetype_id=resource.resourcetype._resourcetype_id)
        elements = list()
        for xmlindex in xmlindexes:
            el = self.pickup(xmlindex._getElementCls(), document={'_id': resource.document._id}, index=xmlindex)
            if el:
                elements.extend(el)
    ...
    def flushResource(self, resource):
        ...
        for element_cls in type_classes.values():
            self.drop(element_cls, document={'_id': resource.document._id})
    ...
    def reindexIndexes(self, xmlindex_list):
        ...
        if resourcetype.version_control:
            rmax_query = sql.select([document_tab.c['resource_id'],
             sql.func.max(document_tab.c['revision']).label('max_revision')])
            rmax_query = rmax_query.group_by(document_tab.c['resource_id'])
            b = rmax_query.alias('b')
            j = b.join(a, sql.and_(a.c['resource_id'] == b.c['resource_id'], a.c['revision'] == b.c['max_revision']))
            query = query.select_from(j)
        query = query.with_only_columns([a.c['id'],
         a.c['data'],
         a.c['revision']])
        query = query.where(sql.and_(a.c['resource_id'] == resource_tab.c['id'], resource_tab.c['resourcetype_id'] == resourcetype._id))
