...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\core\xmldb\xpath.py
...
class RestrictedXPathQueryParser(object):
    ...
    def evalPath(self, s, loc, tokens):
        ...
        if tokens[0] == self.SEP:
            return [
             [
              tokens[1], tokens[2], self.SEP.join(tokens[3:])]]
        steps = 0
        for t in tokens:
    ...
    def createParser(self):
        ...
        """
        xmlNameStartChar = pp.alphas + ':' + '_' + pp.srange('[\\u00C0-\\u00D6]') + pp.srange('[\\u00D8-\\u00F6]') + pp.srange('[\\u00F8-\\u02FF]') + pp.srange('[\\u0370-\\u037D]') + pp.srange('[\\u037F-\\u1FFF]') + pp.srange('[\\u200C-\\u200D]') + pp.srange('[\\u2070-\\u218F]') + pp.srange('[\\u2C00-\\u2FEF]') + pp.srange('[\\u3001-\\uD7FF]') + pp.srange('[\\uF900-\\uFDCF]') + pp.srange('[\\uFDF0-\\uFFFD]') + pp.srange('[\\u10000-\\uEFFFF]')
        xmlNameChar = xmlNameStartChar + '-' + '.' + pp.nums + unichr(183) + pp.srange('[\\u0300-\\u036F]') + pp.srange('[\\u203F-\\u2040]')
        wildcard = pp.Literal(self.WILDCARD)
        sep = pp.Literal(self.SEP)
        selfNd = pp.Literal('.').suppress()
        parentNd = pp.Literal(self.PARENT)
        lpar = pp.Literal('(').suppress()
        rpar = pp.Literal(')').suppress()
        pstart = pp.Literal('[').suppress()
        pend = pp.Literal(']').suppress()
        ncPrefix = pp.Word(xmlNameStartChar, xmlNameChar) + ':'
        ndName = pp.Combine(pp.Optional('@') + pp.Optional(ncPrefix) + pp.Word(xmlNameStartChar, xmlNameChar))
        node = wildcard | parentNd | selfNd | ndName
        literalValue = pp.Literal('"').suppress() + pp.CharsNotIn('"') + pp.Literal('"').suppress() | pp.Literal("'").suppress() + pp.CharsNotIn("'") + pp.Literal("'").suppress()
        numericValue = pp.Combine(pp.Optional('-') + pp.Word(pp.nums) + pp.Optional('.' + pp.Word(pp.nums)))
        ...
        eqOp = pp.Literal('==').setParseAction(pp.replaceWith('=')) | pp.Literal('=')
        ltOp = pp.Literal('<')
        gtOp = pp.Literal('>')
        leOp = pp.Literal('<=')
        geOp = pp.Literal('>=')
        ...
        package_id = (pp.Word(pp.alphanums + '-') | wildcard).setResultsName('package_id').setParseAction(self.evalPackage_id).suppress()
        resourcetype_id = (pp.Word(pp.alphanums + '-') | wildcard).setResultsName('resourcetype_id').setParseAction(self.evalResourcetype_id).suppress()
        locationStep = (sep.suppress() + (ndName | wildcard)).setResultsName('locationStep', True)
        location = (sep.suppress() + package_id + sep.suppress() + resourcetype_id + pp.ZeroOrMore(locationStep)).setParseAction(self.evalLocationSteps)
        pexpr = pp.Forward().setParseAction(self.remove_list)
        pathExpr = (pp.Optional(sep) + node + pp.ZeroOrMore(sep.suppress() + node)).setParseAction(self.evalPath)
        valueExpr = literalValue | numericValue
        relExpr = pathExpr + pp.Optional(relOp + (valueExpr | pathExpr))
        parExpr = pp.Group(lpar + pexpr + rpar)
        notExpr = pp.Group(notFunc + parExpr)
        pexpr << (notExpr | pp.Group(relExpr) | parExpr) + pp.Optional(logOp + (pp.Group(pexpr) | parExpr))
        obItem = (pathExpr + pp.Optional(asc | desc, 'asc')).setResultsName('order_by', listAllMatches=True)
        orderByExpr = orderBy + pp.delimitedList(obItem, ',')
        limitExpr = limit + pp.Word(pp.nums).setResultsName('limit') + pp.Optional(',' + pp.Word(pp.nums).setResultsName('offset'))
        offsetExpr = offset + pp.Word(pp.nums).setResultsName('offset')
        predicates = (pstart + pexpr + pend).setResultsName('predicates')
        query = pp.StringStart() + location + pp.Optional(predicates) + pp.Optional(orderByExpr) + pp.Optional(limitExpr) + pp.Optional(offsetExpr) + pp.StringEnd()
