...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\core\packages\admin\web\catalog.py
...
LIMITS = {'10': 10, '100': 100, '1000': 1000, '10000': 10000, 'unlimited': None}
...
class ResourcesPanel(Component):
    ...
    def _getResources(self, data):
        ...
        t1 = time.time()
        result = self.catalog.getAllResourceNames(data['package_id'], data['resourcetype_id'], limit)
        t2 = time.time()
    ...
    def _addResource(self, data):
        try:
            self.catalog.addResource(package_id=data['package_id'], resourcetype_id=data['resourcetype_id'], xml_data=data['file'])
        except InvalidParameterError as e:
            data['error'] = (
             'Please choose a non-empty XML document', e)
...
class CatalogQueryPanel(Component):
    ...
    def render(self, request):
        data = {'query': '/seishub/stylesheet order by media-type ASC ' + 'limit 2 offset 5', 
           'result': '', 
           'rows': '', 
           'clock': '0.000000'}
        args = request.args
