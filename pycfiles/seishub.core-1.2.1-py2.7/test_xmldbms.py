...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\core\xmldb\tests\test_xmldbms.py
# Compiled at: 2010-12-23 17:42:44
from seishub.core.db.orm import DbAttributeProxy
from seishub.core.exceptions import InvalidObjectError, InvalidParameterError, DuplicateObjectError, NotFoundError
from seishub.core.test import SeisHubEnvironmentTestCase
from seishub.core.util.text import hash
from seishub.core.xmldb.resource import XML_DECLARATION_LENGTH, XmlDocument, Resource, newXMLDocument
...
class XmlDbManagerTest(SeisHubEnvironmentTestCase):
    ...
    def setUp(self):
        ...
        self.test_resourcetype = self.env.registry.db_getResourceType('test', 'testml')
        if not self.test_resourcetype:
            self.test_resourcetype = self.env.registry.db_registerResourceType('test', 'testml')
        self.vc_resourcetype = self.env.registry.db_getResourceType('test', 'vcresource')
        if not self.vc_resourcetype:
            self.vc_resourcetype = self.env.registry.db_registerResourceType('test', 'vcresource', version_control=True)

    def tearDown(self):
        try:
            self.env.registry.db_deleteResourceType(self.test_package.package_id, self.test_resourcetype.resourcetype_id)
        except:
            print 'Warning: Resourcetype %s could not be deleted during tear down.' % self.test_resourcetype.resourcetype_id

        try:
            self.env.registry.db_deleteResourceType(self.test_package.package_id, self.vc_resourcetype.resourcetype_id)
        except:
            print 'Warning: Resourcetype %s could not be deleted during tear down.' % self.vc_resourcetype.resourcetype_id

        try:
            self.env.registry.db_deletePackage(self.test_package.package_id)
        except:
            print 'Warning: Package %s could not be deleted during tear down.' % self.test_package.package_id
    ...
    def testUnversionedResource(self):
        ...
        res1 = Resource(self.test_resourcetype, document=newXMLDocument(self.test_data, uid='testuser'))
        otherpackage = self.env.registry.db_registerPackage('otherpackage')
        othertype = self.env.registry.db_registerResourceType('otherpackage', 'testml')
        res2 = Resource(othertype, document=newXMLDocument(self.test_data))
        self.xmldbm.addResource(res1)
        self.xmldbm.addResource(res2)
        res3 = Resource(self.test_resourcetype, document=newXMLDocument(self.test_data), id=res2.id)
        ...
        self.assertEquals(result.document.meta.size, len(self.test_data) + XML_DECLARATION_LENGTH)
        self.assertEquals(result.document.meta.hash, hash(self.test_data))
        self.assertEquals(result.document.meta.uid, 'testuser')
        self.assertEquals(result.package.package_id, self.test_package.package_id)
        self.assertEquals(result.resourcetype.resourcetype_id, self.test_resourcetype.resourcetype_id)
        ...
        self.assertEquals(result.resourcetype.resourcetype_id, othertype.resourcetype_id)
        self.assertEquals(result.resourcetype.version_control, False)
        modres = Resource(res1.resourcetype, res1.id, document=newXMLDocument(self.test_data_mod))
        self.xmldbm.modifyResource(res1, modres)
        result = self.xmldbm.getResource(res1.package.package_id, res1.resourcetype.resourcetype_id, id=res1.id)
        ...
        self.env.registry.db_deleteResourceType(otherpackage.package_id, othertype.resourcetype_id)
        self.env.registry.db_deletePackage(otherpackage.package_id)
        return

    def testVersionControlledResource(self):
        testres = Resource(self.vc_resourcetype, document=newXMLDocument(self.test_data))
        self.xmldbm.addResource(testres)
        result = self.xmldbm.getResource(id=testres.id)
        self.assertEquals(result.document.data, self.test_data)
        self.assertEquals(result.package.package_id, self.test_package.package_id)
        self.assertEquals(result.resourcetype.resourcetype_id, self.vc_resourcetype.resourcetype_id)
        self.assertEquals(result.resourcetype.version_control, True)
        self.assertEquals(result.document.revision, 1)
        testres_v2 = Resource(self.vc_resourcetype, document=newXMLDocument(self.test_data % 'r2'), id=result.id)
        self.xmldbm.modifyResource(testres, testres_v2)
        rev2 = self.xmldbm.getResource(testres.package.package_id, testres.resourcetype.resourcetype_id, id=testres.id)
        ...
        rev1 = self.xmldbm.getResource(testres.package.package_id, testres.resourcetype.resourcetype_id, id=testres.id, revision=1)
        ...
        testres_v3 = Resource(self.vc_resourcetype, document=newXMLDocument(self.test_data % 'r3'))
        ...
        self.assertRaises(NotFoundError, self.xmldbm.getResource, testres.package.package_id, testres.resourcetype.resourcetype_id, None, 2, None, testres.id)
        res = self.xmldbm.getRevisions(id=testres.id)
        ...
        self.assertRaises(NotFoundError, self.xmldbm.getResource, id=testres.id)
        return

    def testGetResourceList(self):
        res1 = Resource(self.test_resourcetype, document=newXMLDocument(self.test_data))
        res2 = Resource(self.test_resourcetype, document=newXMLDocument(self.test_data))
        self.xmldbm.addResource(res1)
        self.xmldbm.addResource(res2)
        l = self.xmldbm.getAllResources(self.test_package.package_id)
        assert len(l) == 2
        for res in l:
            self.assertEqual(res.package.package_id, self.test_package.package_id)
            self.assertEqual(res.resourcetype.resourcetype_id, self.test_resourcetype.resourcetype_id)
