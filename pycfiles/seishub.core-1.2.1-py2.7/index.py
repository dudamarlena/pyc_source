...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\core\xmldb\index.py
...
class XmlIndex(Serializable):
    ...
    db_mapping = {'resourcetype': Relation(ResourceTypeWrapper, 'resourcetype_id', lazy=True), 
       'xpath': 'xpath', 
    ...
    def __init__(self, resourcetype=None, xpath=None, type=TEXT_INDEX, options=None, group_path=None, label=None):
    ...
    def __str__(self):
        return '/' + self.resourcetype.package.package_id + '/' + self.resourcetype.resourcetype_id + self.xpath
    ...
    relative_xpath = property(getRelative_xpath, setRelative_xpath, 'relative xpath')
    ...
    def _eval(self, xml_doc, env):
        ...
        try:
            if self.group_path:
                ...
                return [ node.evalXPath(self.relative_xpath) or [None] for node in nodes
                       ]
            else:
                return [
                 parsed_doc.evalXPath(self.xpath) or [None]]
...
class KeyIndexElement(Serializable):
    ...
    document = db_property(getDocument, setDocument, 'XmlDocument', attr='_document')
