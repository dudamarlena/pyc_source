...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\core\xmldb\xmlcatalog.py
# Compiled at: 2010-12-23 17:42:44
from seishub.core.exceptions import InvalidParameterError, NotFoundError, InvalidObjectError
...
class XmlCatalog(object):
    ...
    def addResource(self, package_id, resourcetype_id, xml_data, uid=None, name=None):
        """
        ...
        """
        _, resourcetype = self.env.registry.objects_from_id(package_id, resourcetype_id)
        res = Resource(resourcetype, document=newXMLDocument(xml_data, uid=uid), name=name)
    ...
    def modifyResource(self, resource, xml_data, uid=None):
        ...
        """
        new_resource = Resource(resourcetype=resource.resourcetype, document=newXMLDocument(xml_data), name=resource.name)
    ...
    def deleteResource(self, resource=None, resource_id=None):
        ...
        if self.env.config.getbool('seishub', 'use_trash_folder', False):
            ...
            path = os.path.join(self.env.getInstancePath(), 'data', 'trash', resource.package.package_id, resource.resourcetype.resourcetype_id)
        ...
        if not res:
            msg = 'Error deleting a resource: No resource was found with ' + 'the given parameters.'
    ...
    def getResource(self, package_id=None, resourcetype_id=None, name=None, revision=None, document_id=None, id=None):
        ...
        @param revision: revision of related document (if no revision is given,
            ...
            use getRevisions()
        """
        return self.xmldb.getResource(package_id=package_id, resourcetype_id=resourcetype_id, name=name, revision=revision, document_id=document_id, id=id)
    ...
    def registerIndex(self, package_id=None, resourcetype_id=None, label=None, xpath=None, type='text', options=None):
        """
        Register an index.
        ...
        _, resourcetype = self.env.registry.objects_from_id(package_id, resourcetype_id)
        xmlindex = XmlIndex(resourcetype=resourcetype, label=label, xpath=xpath, type=type, options=options, group_path=group_path)
    ...
    def getIndexes(self, package_id=None, resourcetype_id=None, xpath=None, group_path=None, type=None, options=None, _id=None, label=None):
        """
        ...
        return self.index_catalog.getIndexes(package_id=package_id, resourcetype_id=resourcetype_id, xpath=xpath, group_path=group_path, type=type, options=options, _id=_id, label=label)
    ...
    def query(self, query, full=False):
        ...
        if not full:
            return results
        return [ self.xmldb.getResource(document_id=id) for id in results['ordered'] ]
    ...
    def getAllResourceNames(self, package_id, resourcetype_id, limit=100, ordered=False):
        """
