...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\core\tests\test_core_twisted_compatibility.py
# Compiled at: 2010-12-23 17:42:44
"""
Test cases for Twisted component architecture, adopted from 
twisted.python.test.test_components. But this time we use Interface and 
implements declaration from seishub.core.
"""
...
class InterfacesTestCase(twisted_unittest.TestCase):
    """Test interfaces."""
    pass
...
class ComponentizedTestCase(twisted_unittest.TestCase):
    ...
    def test_getComponentDefaults(self):
        ...
        default = object()
        self.assertIdentical(componentized.getComponent(ITest, default), default)
        self.assertIdentical(componentized.getComponent(ITest, default=default), default)
        self.assertIdentical(componentized.getComponent(ITest), None)
        return


class AdapterTestCase(twisted_unittest.TestCase):
    """Test adapters."""
...
class TestMetaInterface(twisted_unittest.TestCase):
...
class RegistrationTestCase(twisted_unittest.TestCase):
    """
    ...
    def _registerAdapterForClassOrInterface(self, original):
        ...
        self.assertIdentical(components.getAdapterFactory(original, TheInterface, None), adapter)
        return
    ...
    def _duplicateAdapterForClassOrInterface(self, original):
        ...
        self.assertRaises(ValueError, components.registerAdapter, secondAdapter, original, TheInterface)
        self.assertIdentical(components.getAdapterFactory(original, TheInterface, None), firstAdapter)
        return
    ...
    def _multipleInterfacesForClassOrInterface(self, original):
        ...
        self.assertIdentical(components.getAdapterFactory(original, FirstInterface, None), adapter)
        self.assertIdentical(components.getAdapterFactory(original, SecondInterface, None), adapter)
        return
    ...
    def _subclassAdapterRegistrationForClassOrInterface(self, original):
        ...
        self.assertIdentical(components.getAdapterFactory(original, TheInterface, None), firstAdapter)
        self.assertIdentical(components.getAdapterFactory(TheSubclass, TheInterface, None), secondAdapter)
        return
...
class ProxyForInterfaceTests(twisted_unittest.TestCase):
    """
    ...
    def test_proxyName(self):
        ...
        self.assertEquals(proxy.__name__.split('.')[(-1)], 'IProxiedInterface)')
