...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\core\log.py
# Compiled at: 2010-12-23 17:42:44
from glob import iglob
from seishub.core.config import Option, BoolOption
from seishub.core.core import ERROR, WARN, INFO, DEBUG
from twisted.python import log, logfile
import os, traceback
LOG_LEVELS = {'OFF': 0, 'ERROR': ERROR, 
   'WARN': WARN, 
   'INFO': INFO, 
   'DEBUG': DEBUG}
...
class Logger(object):
    ...
    emit log messages.
    """
    Option('seishub', 'log_level', 'DEBUG', 'Level of verbosity in log.\n\n        Should be one of (`ERROR`, `WARN`, `INFO`, `DEBUG`).')
    BoolOption('seishub', 'clear_logs_on_startup', False, 'Clears logs at startup of SeisHub.')
    ...
    def __init__(self, env, log_file):
        ...
        if log_file:
            ...
            env.app.setComponent(log.ILogObserver, log.FileLogObserver(daily_log_file).emit)
        self.start()
