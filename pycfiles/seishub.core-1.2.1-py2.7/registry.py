...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\core\registry\registry.py
# Compiled at: 2010-12-23 17:42:44
from seishub.core.core import PackageManager
from seishub.core.db.orm import DbStorage, DB_NULL
from seishub.core.exceptions import SeisHubError, DuplicateObjectError, InvalidParameterError
from seishub.core.packages.interfaces import IPackage, IResourceType, IProcessorIndex, IMapper, ISQLView, IResourceFormater
from seishub.core.registry.package import Alias, Schema, Stylesheet, PackageWrapper, ResourceTypeWrapper
from seishub.core.registry.util import RegistryListProxy
...
class ComponentRegistry(DbStorage):
    ...
    def getComponents(self, interface, package_id=None):
        ...
        package_id.
        """
        components = PackageManager.getComponents(interface, package_id, self.env)
        return components
    ...
    def getPackageIds(self):
        ...
        enabled = [ id for id in all if self.env.isComponentEnabled(PackageManager.getClasses(IPackage, id)[0])
                  ]
    ...
    def getAllPackagesAndResourceTypes(self):
        ...
        for id in ids:
            ...
            resourcetypes[id] = [ cls.resourcetype_id for cls in all if self.env.isComponentEnabled(cls)
                                ]
    ...
    def db_deletePackage(self, package_id):
        if not self._is_package_deletable(package_id):
            raise SeisHubError(("Package with id '%s' cannot be deleted due " + 'to other objects depending on it.') % str(package_id))
        self.drop(PackageWrapper, package_id=package_id)

    def db_registerResourceType(self, package_id, resourcetype_id, version='', version_control=False):
        try:
            package = self.db_getPackages(package_id)[0]
        except IndexError:
            raise SeisHubError('Package not present in database: %s' % str(package_id))

        o = ResourceTypeWrapper(resourcetype_id, package, version, version_control)
        self.store(o)
        return o
    ...
    def db_deleteResourceType(self, package_id, resourcetype_id):
        if not self._is_resourcetype_deletable(package_id, resourcetype_id):
            raise SeisHubError(("Resourcetype with id '%s' in package '%s' " + 'cannot be deleted due to other objects ' + 'depending on it.') % (
...
class RegistryBase(DbStorage, list):
    ...
    def _split_uri(self, uri):
        ...
        return (
         package_id, resourcetype_id, type)
    ...
    def register(self, package_id, resourcetype_id, type, xml_data, name=None):
        ...
        res = self.catalog.addResource(self.package_id, self.resourcetype_id, xml_data, name=name)
        try:
    ...
    def get(self, package_id=None, resourcetype_id=None, type=None, document_id=None, uri=None):
        if uri:
            package_id, resourcetype_id, type = self._split_uri(uri)
        keys = {'package': None, 'resourcetype': None, 'type': type}
        if document_id:
    ...
    def delete(self, package_id=None, resourcetype_id=None, type=None, document_id=None, uri=None):
        o = self.get(package_id, resourcetype_id, type, uri=uri, document_id=document_id)
        if len(o) > 1:
            raise SeisHubError('Error deleting a schema or stylesheet: ' + 'Unexpected result set length.')
        if len(o) == 0:
            raise SeisHubError('Error deleting a schema or stylesheet: ' + 'No objects found with the given parameters.')
...
class SchemaRegistry(RegistryBase):
    ...
    def register(self, package_id, resourcetype_id, type, xml_data, name=None):
        ...
        return RegistryBase.register(self, package_id, resourcetype_id, type, xml_data, name)
    ...
    def get(self, package_id=None, resourcetype_id=None, type=None, document_id=None, uri=None):
        ...
        The following combinations return multiple schemas:
         ...
         - get() -> all stylesheets
        """
        return RegistryBase.get(self, package_id, resourcetype_id, type, document_id, uri)
    ...
    def delete(self, package_id=None, resourcetype_id=None, type=None, document_id=None, uri=None):
        ...
        parameter combinations are allowed:
         ...
         - delete(uri = ...)
        """
        return RegistryBase.delete(self, package_id, resourcetype_id, type, document_id, uri)
    ...
    def update(self):
        ...
        for cls in all:
            ...
            if not hasattr(cls, 'mapping_url'):
                msg = 'Class %s has a wrong implementation of %s. ' + 'Attribute mapping_url is missing.'
...
class ProcessorIndexRegistry(object):
    ...
    def register(self, cls):
        ...
        rt = self.env.registry.db_getResourceType(cls.package_id, cls.resourcetype_id)
        clsname = cls.__module__ + '.' + cls.__name__
        idx = index.XmlIndex(resourcetype=rt, xpath=cls.label, type=index.PROCESSOR_INDEX, options=clsname, label=cls.label)
...
class ResourceFormaterRegistry(object):
    ...
    def update(self):
        ...
        for cls in all:
            ...
            if not hasattr(cls, 'package_id'):
                msg = 'Class %s has a wrong implementation of %s. ' + 'Attribute package_id is missing.'
                self.env.log.warn(msg % (cls, IResourceFormater))
                continue
            if not hasattr(cls, 'resourcetype_id'):
                msg = 'Class %s has a wrong implementation of %s. ' + 'Attribute resourcetype_id is missing.'
                self.env.log.warn(msg % (cls, IResourceFormater))
                continue
            if not hasattr(cls, 'format_id'):
                msg = 'Class %s has a wrong implementation of %s. ' + 'Attribute format_id is missing.'
                self.env.log.warn(msg % (cls, IResourceFormater))
                continue
            ...
            if not isinstance(cls.format_id, list):
                msg = 'Class %s has a wrong implementation of %s. ' + 'Attribute format_id must be a list or string instance.'
                self.env.log.warn(msg % (cls, IResourceFormater))
                continue
            for format in formats:
                ids = (
                 cls.package_id, cls.resourcetype_id, format)
                self._dict[ids] = cls
    ...
    def get(self, package_id, resourcetype_id, format):
        ...
        """
        ids = (
         package_id, resourcetype_id, format)
