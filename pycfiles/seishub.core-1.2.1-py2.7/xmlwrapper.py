...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\core\util\xmlwrapper.py
...
class XmlNode(object):
    ...
    def getStrContent(self):
        if isinstance(self._node_obj, basestring):
            ...
            str = etree.tostring(self._node_obj, encoding=self.encoding)
        return str
    ...
    def evalXPath(self, expr):
        ...
        try:
            ...
            raise InvalidXPathExpression(('Error evaluating a XPath ' + 'expression: %s') % str(expr), e)
...
class XmlStylesheet(object):
    ...
    def __init__(self, stylesheet_data):
        ...
        self.content_type = root.xpath('.//xsl:output/@media-type', namespaces=root.nsmap)
...
def toString(xml_obj, method='xml'):
    ...
    """
    return etree.tostring(xml_obj, method=method, xml_declaration=True, pretty_print=True, encoding='utf-8')