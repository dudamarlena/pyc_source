...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/test_extensions/testrunners/xmloutput.py
# Compiled at: 2011-06-22 14:43:03
    ...
    def stopTest(self, test):
        ...
        for assertion in test._extraAssertions:
            if not seen.has_key(assertion):
                self._addAssertion(assertion[:110])
                seen[assertion] = True
...
def _cleanHTML(whut):
    return whut.replace('"', '&quot;').replace('<', '&lt;').replace('>', '&gt;')