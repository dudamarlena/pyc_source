...
# [GCC 8.4.0]
# Embedded file name: /private/var/folders/l0/ss9rqt5j7xbg0x2bpvmjx_k40000gp/T/pip-install-n2xwwglt/workstation/workstation/drivers/smoothie_drivers/v2_0_0/driver.py
# Compiled at: 2019-09-24 08:15:55
# Size of source mod 2**32: 24641 bytes
import json, math, time
from threading import Event
from workstation.util.log import get_logger
from workstation.util.vector import Vector
from workstation.drivers.smoothie_drivers import VirtualSmoothie, SmoothieDriver
...
class SmoothieDriver_2_0_0(SmoothieDriver):
    ...
    CONFIG_STEPS_PER_MM = {'x':'alpha_steps_per_mm', 
     'y':'beta_steps_per_mm', 
    ...
    def wait_for_ok(self):
        ...
        if res != 'ok':
            raise RuntimeError('{0}: {1}'.format(self.SMOOTHIE_ERROR, res))
    ...
    def is_simulating(self):
        return bool(isinstance(self.connection.device(), VirtualSmoothie))
    ...
    def send_command(self, command, read_after=True, timeout=25, m400=False, **kwargs):
        """
    ...
    def move(self, mode='absolute', **kwargs):
        ...
        self.set_speed()
        current = self.get_head_position()['target']
        target_point = {axis:kwargs.get(axis, 0 if mode == 'relative' else current[axis]) for axis in 'xyz'}
        flipped_vector = self.flip_coordinates(Vector(target_point), mode)
        for axis in 'xyz':
            kwargs[axis] = flipped_vector[axis]

        args = {axis.upper():kwargs.get(axis) for axis in 'xyzab' if axis in kwargs}
        ...
        while attempts <= max_attempts:
            ...
            try:
                ...
                try:
                    ...
                    log.exception('Failed to send MOVE command. Will reconnect and try again in {} seconds'.format(reconnect_delay))
        ...
        if not move_sent:
            raise RuntimeWarning('Failed to complete move command after {} tries'.format(attempts))
        arguments = {'name':'move-finished', 
         'position':{'head':self.get_head_position()['current'], 
    ...
    def home(self, *axis):
        ...
        try:
            ...
            raise RuntimeWarning('HOMING ERROR: Check switches are being pressed and connected')
    ...
    def wait(self, delay_time):
        ...
        def _sleep(delay_time):
            ...
            if self.is_simulating():
                ...
                self.send_command('{} S{} P{}'.format(self.DWELL, int(delay_time), round(delay_time % 1, 2)))
            else:
                time.sleep(delay_time)

        end_time = _current_time() + delay_time
        trace.EventBroker.get_instance().notify({'name':'delay-start', 
         'time':delay_time})
        while end_time > _current_time():
            self.check_paused_stopped()
            _sleep(min(1, end_time - _current_time()))
            trace.EventBroker.get_instance().notify({'name':'countdown', 
             'countdown':int(end_time - time.time())})

        trace.EventBroker.get_instance().notify({'name': 'delay-finish'})
    ...
    def get_plunger_positions(self):
        ...
        for state in ('current', 'target'):
            plunger_coords[state] = {axis:coords[state].get(axis, 0) for axis in 'ab'}

        return plunger_coords

    def get_position(self):
        return {'current':self.get_current_position(), 
         'target':self.get_target_position()}
    ...
    def set_acceleration(self, **kwargs):
        axis = {ax.upper():val for ax, val in kwargs.items() if ax.upper() in 'XYZABC' if ax.upper() in 'XYZABC'}
    ...
    def set_speed(self, *args, **kwargs):
        ...
        if self.is_connected():
            kwargs = {key.upper():int(val / 60) for key, val in self.speeds.items()}
    ...
    def get_endstop_switches(self):
        ...
        return {endstop[0].lower():bool(int(endstop.split(':')[1])) for endstop in endstop_values}
    ...
    def set_mosfet(self, mosfet_index, state):
        try:
            ...
            raise IndexError('Smoothie mosfet not at index {}'.format(mosfet_index))
    ...
    def read_sd_file(self, filename, timeout=30):
        self.send_command(('cat /sd/{}'.format(filename)),
          read_after=False)
        file_string = ''
    ...
    def _parse_axis_values(self, string):
        try:
            ...
            return {s.split(':')[0].lower():float(s.split(':')[1]) for s in parsed_values}
