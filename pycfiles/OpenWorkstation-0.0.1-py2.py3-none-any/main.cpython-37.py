...
# [GCC 8.4.0]
# Embedded file name: /private/var/folders/l0/ss9rqt5j7xbg0x2bpvmjx_k40000gp/T/pip-install-n2xwwglt/workstation/workstation/server/main.py
# Compiled at: 2019-09-24 08:15:55
# Size of source mod 2**32: 26364 bytes
import json, logging, os, sys, threading, time, argparse, flask
from flask import Flask, send_from_directory, request
from flask_socketio import SocketIO
from flask_cors import CORS
from workstation import robot, robot2, Robot, Robot2, instruments, containers
from workstation.util import trace, environment, state as robot_state
from workstation.util.vector import VectorEncoder
from workstation.drivers.smoothie_drivers.v2_0_0 import player
sys.path.insert(0, os.path.abspath('..'))
from workstation.server import helpers
from workstation.server.process_manager import run_once
STATIC_FOLDER = os.path.join(helpers.get_frozen_root() or '', 'templates')
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('path',
      nargs='?', default=STATIC_FOLDER,
...
def get_protocol_locals():
    from workstation import robot, containers, instruments
    return locals()
...
def upload():
    ...
    if not file:
        ...
        if extension == 'py':
            commands, error_msg = helpers.run_protocol((app.robot),
              (app.code), mode='null')
            if error_msg:
    ...
    return flask.jsonify({'status':status, 
     'data':{'errors':api_response['errors'], 
...
def emit_notifications(notifications, _type):
    for notification in notifications:
        socketio.emit('event', {'name':'notification', 
         'text':notification, 
         'type':_type})
...
def run():
    ...
    def _run():
        socketio.emit('event', {'caller':'ui', 
         'mode':'live', 
         'name':'command-run', 
         'command_description':'Protocol run initiated. Simulating...'})
        commands, error_msg = helpers.run_protocol((app.robot),
          (app.code), mode='null')
        app.robot.cmds_total = len(commands) + 1
        app.robot._commands = []
        app.robot.resume()
        ...
        end_time = time.time()
        if error:
            helpers.run_protocol((app.robot),
              (app.code), mode='null')
...
def _detached_progress():
    while not exit_threads.is_set():
        ...
        progress_data = 'Protocol {} Complete - Elapsed Time {}:{}:{}'.format(percentage, h, m, s)
...
def _run_detached():
    try:
        ...
        d = {'caller':'ui', 
         'mode':'live',  'name':'command-run'}
        d.update({'command_description': 'Simulating, please wait...'})
        notify(d)
        app.robot.smoothie_drivers['simulate_switches'].record_start(p)
        commands, error_msg = helpers.run_protocol((app.robot),
          (app.code), mode='simulate_switches')
        app.robot.smoothie_drivers['simulate_switches'].record_stop()
        if error_msg:
            raise RuntimeError(error_msg)
        d.update({'command_description': 'Saving file to robot, please wait...'})
        notify(d)
        app.robot._driver.play(p)
        d.update({'command_description': 'Protocol running, unplug USB at any time.'})
        notify(d)
        d.update({'command_description': 'To stop, unplug USB and power robot OFF'})
        notify(d)
        _detached_progress()
    except Exception as e:
        try:
...
def halt():
    ...
    emit_notifications([
     'Robot halted suddenly, please HOME ALL before running again'], 'info')
    return flask.jsonify({'status':'success', 
     'data':result})
...
def script_loader(filename):
    ...
    return flask.send_from_directory(scripts_root_path,
      filename, mimetype='application/javascript')


@app.route('/robot/serial/list')
def get_serial_ports_list():
    return flask.jsonify({'ports': app.robot.get_serial_ports_list()})


@app.route('/robot/serial/is_connected')
def is_connected():
    return flask.jsonify({'is_connected':app.robot.is_connected(), 
     'port':app.robot.get_connected_port()})


@app.route('/robot/get_coordinates')
def get_coordinates():
    return flask.jsonify({'coords': app.robot._driver.get_position().get('target')})


@app.route('/robot/diagnostics')
def diagnostics():
    return flask.jsonify({'diagnostics': app.robot.diagnostics()})


@app.route('/robot/versions')
def get_versions():
    return flask.jsonify({'versions': app.robot.versions()})


@app.route('/app_version')
def app_version():
    return flask.jsonify({'version': os.environ.get('appVersion')})
    ...
    try:
        ...
        try:
            ...
            if 'versions are incompatible' in data:
                data += '. To upgrade, go to docs.workstation.com'
...
def _start_connection_watcher():
    connection_state_watcher, watcher_should_run = BACKGROUND_TASKS.get('CONNECTION_STATE_WATCHER', (None,
                                                                                                     None))
    if connection_state_watcher:
        if watcher_should_run:
            watcher_should_run.set()
    watcher_should_run = threading.Event()

    def watch_connection_state(should_run):
        while not should_run.is_set():
            socketio.emit('event', {'type':'connection_status', 
             'is_connected':app.robot.is_connected()})
            socketio.sleep(1.5)

    connection_state_watcher = socketio.start_background_task(watch_connection_state, watcher_should_run)
...
def home(axis):
    ...
    result = ''
    try:
        if not axis == 'undefined':
            if axis == '' or axis.lower() == 'all':
...
def move_to_container():
    ...
    try:
        ...
        well_x, well_y, well_z = tuple(instrument.calibrator.convert(container[0], container[0].bottom()[1]))
        _, _, robot_max_z = app.robot._driver.get_dimensions()
        app.robot.move_head(z=robot_max_z)
...
def pick_up_tip():
    try:
        ...
        tip_x, tip_y, tip_z = tuple(instrument.calibrator.convert(tip_placeable, tip_placeable.bottom()[1]))
        _, _, robot_max_z = app.robot._driver.get_dimensions()
        app.robot.move_head(z=robot_max_z)
...
def _calibrate_placeable(container_name, parent_slot, axis_name):
    ...
    if not this_container:
        raise ValueError('Container {0} not found in slot {1}'.format(container_name, parent_slot))
...
def log_before_request():
    ...
    log_msg = '[BR] {method} {url} | {data}'.format(method=(request.method),
      url=(request.url),
      data=(request.data))
    logger.info(log_msg)
...
def start():
    ...
    from workstation.server import log
    lg = logging.getLogger('workstation-app')
    lg.info('Starting Flask Server')
    [app.logger.addHandler(handler) for handler in lg.handlers]
    print('workstation API server is serving UI from: ' + STATIC_FOLDER)
    socketio.run(app,
      debug=False,
      logger=False,
      use_reloader=False,
