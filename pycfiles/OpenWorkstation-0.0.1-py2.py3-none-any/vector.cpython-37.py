...
# [GCC 8.4.0]
# Embedded file name: /private/var/folders/l0/ss9rqt5j7xbg0x2bpvmjx_k40000gp/T/pip-install-n2xwwglt/workstation/workstation/util/vector.py
# Compiled at: 2019-09-24 08:15:55
# Size of source mod 2**32: 5985 bytes
import math, json
...
class Vector(object):
    ...
    def coordinates_from_iterable(cls, iterable):
        return value_type(iterable[0], iterable[1], iterable[2])
    ...
    def length(self):
        return math.sqrt(pow(self.coordinates.x, 2) + pow(self.coordinates.y, 2) + pow(self.coordinates.z, 2))
    ...
    def __init__(self, *args, **kwargs):
        ...
        if args_len == 1:
            ...
            if isinstance(arg, dict):
                ...
                raise ValueError('One argument supplied expected to be dict or iterable, received {}'.format(type(arg)))
        elif args_len == 3:
    ...
    def __eq__(self, other):
        if isinstance(other, Vector):
            return all([isclose(a, b, rel_tol=1e-05) for a, b in zip(self, other)])
        if isinstance(other, dict):
            return self == Vector(other)
        if self.is_iterable(other):
            return self == Vector(other)
        raise ValueError('Expected operand to be dict, iterable or vector')

    def __add__(self, other):
        other = other
        return Vector(self.coordinates.x + other[0], self.coordinates.y + other[1], self.coordinates.z + other[2])
    ...
    def __str__(self):
        return '(x={:.2f}, y={:.2f}, z={:.2f})'.format(self.coordinates.x, self.coordinates.y, self.coordinates.z)
