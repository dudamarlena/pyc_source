...
def are_instrument_positions_calibrated(instrument):
    ...
    if len(positions.values()) != 4:
        return False
    return not any([str(val).endswith('.0101') for val in positions.values()])
...
def get_all_pipettes(robot):
    ...
    """
    pipettes = [inst for _, inst in robot.get_instruments() if isinstance(inst, pipette.Pipette)]
...
def sort_containers(container_list):
    ...
    tipracks = sorted(tiprack_containers,
      key=(lambda c: c.get_name().lower()))
    other = sorted(other_containers,
      key=(lambda c: c.get_name().lower()))
