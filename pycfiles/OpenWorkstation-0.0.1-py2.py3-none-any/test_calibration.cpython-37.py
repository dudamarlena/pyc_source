...
# [GCC 8.4.0]
# Embedded file name: /private/var/folders/l0/ss9rqt5j7xbg0x2bpvmjx_k40000gp/T/pip-install-n2xwwglt/workstation/workstation/server/tests/test_calibration.py
...
class CalibrationTestCase(unittest.TestCase):
    ...
    def test_move_to_slot(self):
        arguments = {'slot': 'A1'}
        response = self.app.post('/move_to_slot',
    ...
    def test_aspirate_dispense(self):
        ...
        self.robot._instruments['B'].calibrate_plunger(top=0,
          bottom=5,
          blow_out=6,
          drop_tip=7)
        self.robot.move_head(z=0)
        arguments = {'axis': 'b'}
        response = self.app.post('/aspirate',
          data=(json.dumps(dict(arguments))),
          content_type='application/json')
        status = json.loads(response.data.decode())['status']
        self.assertEqual(status, 'success')
        arguments = {'axis': 'b'}
        response = self.app.post('/dispense',
          data=(json.dumps(dict(arguments))),
          content_type='application/json')
        status = json.loads(response.data.decode())['status']
        self.assertEqual(status, 'success')
        arguments = {'axis':'b', 
         'volume':12}
        response = self.app.post('/set_max_volume',
          data=(json.dumps(dict(arguments))),
          content_type='application/json')
        status = json.loads(response.data.decode())['status']
        self.assertEqual(status, 'success')
    ...
    def test_move_to_plunger_position(self):
        ...
        arguments = {'position':'bottom', 
         'axis':'b'}
        response = self.app.post('/move_to_plunger_position',
          data=(json.dumps(dict(arguments))),
          content_type='application/json')
        self.assertEqual(status, 'success')
        current_pos = self.robot._driver.get_plunger_positions()['target']
    ...
    def test_move_to_container(self):
        ...
        arguments = {'label':'test-plate', 
         'slot':'B2', 
         'axis':'b'}
        response = self.app.post('/move_to_container',
          data=(json.dumps(dict(arguments))),
          content_type='application/json')
        status = json.loads(response.data.decode())['status']
        self.assertEqual(status, 'success')
    ...
    def test_calibrate_placeable(self):
        ...
        arguments = {'label':'test-plate', 
         'axis':'b', 
         'slot':'B2'}
        response = self.app.post('/calibrate_placeable',
          data=(json.dumps(dict(arguments))),
          content_type='application/json')
    ...
    def test_calibrate_plunger(self):
        ...
        response = self.app.post('/calibrate_plunger',
          data=(json.dumps(dict(arguments))),
          content_type='application/json')
        status = json.loads(response.data.decode())['status']
        self.assertEqual(status, 'success')
