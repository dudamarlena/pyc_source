...
# [GCC 8.4.0]
# Embedded file name: /Users/kghose/.venvs/benten/lib/python3.7/site-packages/benten/langserver/lspobjects.py
...
def to_dict(v):
    if isinstance(v, LSPObject):
        return {k:to_dict(_v) for k, _v in v.__dict__.items() if _v is not None if _v is not None}
...
class Diagnostic(LSPObject):
    ...
    def __init__(self, _range: Range, message: str, severity: DiagnosticSeverity=None, code: str=None, source: str=None):
        self.range = _range
...
class CompletionItem(LSPObject):
    ...
    def __init__(self, label: str, text_edit: TextEdit=None, additional_text_edits: [TextEdit]=None, kind: CompletionItemKind=CompletionItemKind.Text, insert_text_format: InsertTextFormat=None, detail: str=None, documentation: str=None, preselect: bool=None, sort_text: str=None, filter_text: str=None):
        self.label = label
        self.kind = kind
        self.detail = detail
    ...
    def from_snippet(cls, snippet: dict):
        ...
        snippet['text_edit'] = TextEdit(_range=Range(start=Position(line=0, character=0),
...
class Hover(LSPObject):
    ...
    def __init__(self, contents, _range=None, wrap_as_code_block=False, is_markdown=False):
        if wrap_as_code_block:
            self.contents = MarkupContent(kind='markdown',
              value=('```\n' + contents + '\n```'))
        else:
            if is_markdown:
                self.contents = MarkupContent(kind='markdown',
...
class MarkupContent(LSPObject):

    def __init__(self, value, kind='markdown'):
        self.kind = kind
        self.value = value