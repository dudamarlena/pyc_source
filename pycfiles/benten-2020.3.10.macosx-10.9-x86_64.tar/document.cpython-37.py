...
# [GCC 8.4.0]
# Embedded file name: /Users/kghose/.venvs/benten/lib/python3.7/site-packages/benten/code/document.py
# Compiled at: 2020-02-05 20:32:14
...
class Document:
    ...
    def __init__(self, doc_uri: str, scratch_path: pathlib.Path, text: str, version: int, type_dicts: dict):
    ...
    def parse(self, cwl):
        ...
        if cwl_v not in self.type_dicts:
            logger.error(f"No language model for cwl version {str(cwl_v)}. Using {latest_published_cwl_version}")
            cwl_v = latest_published_cwl_version
        lm = self.type_dicts.get(cwl_v)
        inferred_type = infer_type(node=cwl,
          allowed_types=[lm.get(t) for t in process_types])
        inferred_type.parse(doc_uri=(self.doc_uri),
          node=cwl,
