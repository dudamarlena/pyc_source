...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/hackercodecs/__init__.py
# Compiled at: 2015-02-15 14:53:13
r"""This package provides codecs useful for hacking and hacking related CTFs. 
...
def blocks(data, size):
    assert len(data) % size == 0, 'Cannot divide into blocks of size %s' % size
...
def bin_decode(input, errors='strict'):
    """print 8 bits of whatever int goes in"""
    output = ''
    assert len(input) % 8 == 0, 'Wrong number of bits, %s is not divisible by 8' % len(input)
...
def entity_decode_hex(input, errors='strict'):
    ...
    preamble_regex = re.compile('&#x', flags=re.I)
...
def aba_track_2_encode(input, errors='strict'):
    output = ''
    assert all(map(lambda x: 63 >= ord(x) >= 48, input)), 'Characters found out of range 0x30 - 0x3f'
    len_in = len(input)
    assert len_in <= 37, 'No room for sentinel and LRC. Input must be 37 characters or under'
    input = ';' + input + '?'
    out = []
    for c in input:
        c = ord(c) - 48
...
def aba_track_2_decode(input, errors='strict'):
    ...
    assert not len_in > 200, 'String too long: cannot be ABA Track 2'
...
CODECS_IN_FILE = {'morse': CodecInfo(name='morse', encode=morse_encode, decode=morse_decode), 
   'bin': CodecInfo(name='bin', encode=bin_encode, decode=bin_decode), 
   'url': CodecInfo(name='url', encode=url_encode, decode=url_decode), 
   'entity': CodecInfo(name='entity', encode=entity_encode, decode=entity_decode), 
   'entityhex': CodecInfo(name='entityhex', encode=entity_encode_hex, decode=entity_decode_hex), 
   'ascii85': CodecInfo(name='ascii85', encode=ascii85_encode, decode=ascii85_decode), 
   'yenc': CodecInfo(name='yenc', encode=y_encode, decode=y_decode)}
