...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.6/site-packages/rawkit/raw.py
# Compiled at: 2017-07-08 15:12:43
# Size of source mod 2**32: 13986 bytes
""":mod:`rawkit.raw` --- High-level raw file API
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
"""
import ctypes, os, random, string, tempfile, warnings
...
output_file_types = namedtuple('OutputFileType', ['ppm', 'tiff'])('ppm', 'tiff')
...
class Raw(object):
    ...
    def save(self, filename=None, filetype=None):
        ...
        if filename is None:
            ...
            if filetype not in output_file_types:
                raise InvalidFileType('Output filetype must be in raw.output_file_types')
        self.data.contents.params.output_tiff = filetype == output_file_types.tiff
        self.unpack()
        self.process()
    ...
    def color_description(self):
        ...
        """
        return self.data.contents.idata.cdesc
    ...
    def raw_image(self, include_margin=False):
        ...
        if not bool(image):
            ...
            if sizes.pixel_aspect != 1:
                warnings.warn('The pixel aspect is not unity, it is: ' + str(sizes.pixel_aspect))
            if sizes.flip != 0:
    ...
    def bayer_data(self, include_margin=False):
        ...
        """
        return (
         self.raw_image(include_margin), self.color_filter_array)
    ...
    def to_buffer(self):
        ...
        processed_image = self.libraw.libraw_dcraw_make_mem_image(self.data, ctypes.cast(ctypes.addressof(status), ctypes.POINTER(ctypes.c_int)))
        raise_if_error(status.value)
        data_pointer = ctypes.cast(processed_image.contents.data, ctypes.POINTER(ctypes.c_byte * processed_image.contents.data_size))
    ...
    def thumbnail_to_buffer(self):
        ...
        self.unpack_thumb()
        status = ctypes.c_int(0)
        processed_image = self.libraw.libraw_dcraw_make_mem_thumb(self.data, ctypes.cast(ctypes.addressof(status), ctypes.POINTER(ctypes.c_int)))
        raise_if_error(status.value)
        data_pointer = ctypes.cast(processed_image.contents.data, ctypes.POINTER(ctypes.c_byte * processed_image.contents.data_size))
        data = bytearray(data_pointer.contents)
        self.libraw.libraw_dcraw_clear_mem(processed_image)
    ...
    def metadata(self):
        ...
        """
        return Metadata(aperture=(self.data.contents.other.aperture),
...
class DarkFrame(Raw):
    ...
    def __init__(self, filename=None):
        ...
        self.options = Options({'auto_brightness':False, 
         'brightness':1.0, 
         'auto_stretch':True, 
         'bps':16, 
         'gamma':(1, 1), 
         'rotation':0})
        self._tmp = os.path.join(tempfile.gettempdir(), '{prefix}{rand}'.format(prefix=(tempfile.gettempprefix()),
