...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\otsapi.py
# Compiled at: 2019-03-15 05:28:34
...
class otsapi(object):
    ...
    def update_row(self, table_name, k, v, function='PUT'):
        ...
        """
        primary_key = k
        update_of_attribute_columns = {function: v}
        row = Row(primary_key, update_of_attribute_columns)
        condition = Condition(RowExistenceExpectation.IGNORE, SingleColumnCondition(k[0][0], k[0][1], ComparatorType.EQUAL))
    ...
    def put_row(self, table_name, primary_key, attribute_columns):
        ...
        condition = Condition(RowExistenceExpectation.EXPECT_NOT_EXIST, SingleColumnCondition(primary_key[0][0], primary_key[0][1], ComparatorType.EQUAL))
    ...
    def create_table(self, table_name, schema_of_primary_key, max_time_deviation=1892160000):
        """
        创建OTS 表
        :param table_name: 表名称
        :param schema_of_primary_key = [('uid', 'STRING')] 主键
        :return:
        """
        table_meta = TableMeta(table_name, schema_of_primary_key)
        table_option = TableOptions(-1, 1, max_time_deviation)
    ...
    def list_search_index(self, table_name):
        tablesearchindexdict = {}
        for table, index_name in self.client.list_search_index(table_name):
            tablesearchindexdict[table] = index_name

        return tablesearchindexdict

    def delete_search_index(self, table_name, index_name):
        try:
            self.client.delete_search_index(table_name, index_name)
        except:
            raise Exception('delete search index error')
