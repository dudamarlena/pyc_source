...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/txbonjour/discovery.py
# Compiled at: 2014-10-30 16:37:51
"""
...
from txbonjour.service import BonjourService, BonjourReader, IBroadcastProtocol, IDiscoverProtocol
...
def _dispatch(protocol, resolving=False, resolve_ip=True, *args):
    ...
    """
    if len(args) == 6:
        (sdref, flags, interface_index, service_name, registry_type, reply_domain) = args
        error_code = pybonjour.kDNSServiceErr_NoError
        return reactor.callLater(0, protocol.registerReceived, *args)
    (sdref, flags, interface_index, error_code, service_name, registry_type, reply_domain) = args
    ...
    def _call(*args):
        ...
        if error_code != pybonjour.kDNSServiceErr_NoError:
            ...
            reactor.callLater(0, protocol.removeService, args[4], args[5], args[6], args[2], args[1])
        elif flags & pybonjour.kDNSServiceFlagsAdd:
            reactor.callLater(0, protocol.addService, args[4], args[5], args[6], args[2], args[1])
    ...
    d.addErrback(lambda res: reactor.callLater(0, protocol.resolveError, res, *resolve_args))
...
def broadcast(protocol, regtype, port, name, record=None, _do_start=True):
    ...
    sdref = pybonjour.DNSServiceRegister(regtype=regtype, port=port, callBack=cb, txtRecord=record, name=name)
...
def connectBonjour(*args, **kwargs):
    ...
    """
    reader = broadcast(_do_start=False, *args, **kwargs)
...
def listenBonjour(*args, **kwargs):
    ...
    @see: http://twistedmatrix.com/documents/12.2.0/core/howto/application.html 
    """
    reader = discover(_do_start=False, *args, **kwargs)
    return BonjourService(reader)
