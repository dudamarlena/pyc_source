...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/txbonjour/tap.py
# Compiled at: 2014-10-30 17:13:36
...
class Options(usage.Options):
    optFlags = [
     [
      'resolve-domains', 'n',
      'Resolve FQDM to ip addresses before reporting, acceptable false values are: %s' % (
       falses,)]]
    optParameters = [
     [
      'port', 'p', 9898, 'The port to broadcast to bonjour clients to connect to you',
      int],
     [
      'registration', 'r', '_examples._tcp', 'The mDNS registry for bonjour, ie. <domain>.<transport>'],
     [
      'service-name', 's', 'Example-Service', 'The name bonjour clients will see for your service']]
...
class ServiceMaker(object):
    ...
    def makeService(self, options, broadcast_protocol=None, discovery_protocol=None):
        ...
        discover_service = discovery.listenBonjour(discovery_protocol, registry, resolve_ip=resolve)
        discover_service.setName('discovery')
        discover_service.setServiceParent(s)

        def broadcast():
            broadcast_service = discovery.connectBonjour(broadcast_protocol, registry, port, service_name)
