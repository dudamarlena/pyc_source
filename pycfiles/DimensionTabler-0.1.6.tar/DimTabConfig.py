...
# [GCC 8.4.0]
# Embedded file name: h:\dev\dimensiontabler\DimensionTabler\DimTabConfig.py
...
class DimTabConfig(object):
    ...
    def IntermediateTable(self, value):
        if type(value) is not DimTabConfig.IntermediateTableConfig and value is not None:
    ...
    class DimensionConfig(object):
        ...
        def __init__(self, description, timeSec, granularitySec):
            ...
            if type(timeSec) is not int and not timeSec == DimTabConfig.DIMENSION_TIMESEC_PAST:
    ...
    def WaitSecondsBeforeCumulating(self, value):
        if type(value) is int:
            ...
            raise Exception('Value must be seconds.')
    ...
    def WaitSecondsBeforeMemCleanup(self, value):
        if type(value) is int:
            ...
            raise Exception('Value must be seconds. Defaults to WaitSecondsBeforeCumulating * 10.')
