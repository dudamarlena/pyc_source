...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tcdt/tcdt.py
# Compiled at: 2020-01-05 04:40:12
# Size of source mod 2**32: 15750 bytes
...
def count_change2(larray, vindex, findex, rlindex):
    ...
    for idx in rlindex:
        if valid_idx(idx, findex, tmp):
            ...
            idx2 = idx - 1
            while idx2 >= 0:
                if not valid_idx(idx2, findex, tmp):
                    if vindex[idx2]:
                        if larray[idx] != larray[idx2]:
                            nfchange += 1
                        break
...
class Node:
    ...
    def __init__(self, fname, fvalueL, fvalueR, left, right, ipath, depth, data, datamask, availFeatures, changeCount, fnamevalue, timeCol, labelCol):
        self.fname = fname
    ...
    def __str__(self):
        if self.left == None:
            ...
            for i in range(0, len(self.timeSeries) - 1, 2):
                ...
                tstr += '{} -- {}: {}; '.format(row1[self.timeCol], row2[self.timeCol], row1[self.labelCol])
...
class NodeSplitter:
    ...
    def split_node(self, node, rlindex, fvindex, labelCol, featureCol, timeCol):
        fname, fvalue, leftcount, rightcount, availFeatures, findex = self.find_bestsplit_hier(node, rlindex, fvindex, labelCol)
        if fname == '':
            ...
            node.left = Node('', [], [], None, None, lpath, newdepth, dataleft, maskleft, fleft, leftcount, node.fnamevalue, timeCol, labelCol)
            node.right = Node('', [], [], None, None, rpath, newdepth, dataright, maskright, fright, rightcount, node.fnamevalue, timeCol, labelCol)
            node.data = None
            return True
    ...
    def find_bestsplit_hier(self, node, rlindex, fvindex, labelCol):
        ...
        for availFlatFeatures in hier_features(newAvailFeatures):
            fname, fvalue, leftcount, rightcount, findex = self.find_bestsplit(node, availFlatFeatures, rlindex, fvindex, labelCol)
            if fname != '':
...
class TCDT:
    ...
    def fit(self, trainData, trainFeatures, labelCol, timeCol):
        ...
        self.root = Node('', [], [], None, None, [], 0, trainData, datamask, trainFeatures, changecount, set(), timeCol, labelCol)
        self.nodecnt = 1
        nodequeue = deque()
    ...
    def get_leaf(self, case):
        ...
        while True:
            if root.fname == '':
                return (
                 root, depth)
            if case[root.fname] in root.fvalueL:
                root = root.left
                depth += 1
            else:
                root = root.right
        ...
        def print_tree_recurse(node, report):
            ...
            if node.depth <= max_depth:
                ...
                if node.left != None:
                    assert node.right != None
                    report += indent
                    report += '{} == {}'.format(self.featureNames[node.fname], node.fvalueL[0])
