...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\tg2exe\command.py
# Compiled at: 2006-11-14 20:58:26
import os, sys, pkg_resources, shutil, glob, py_compile, modulefinder
try:
    import win32api
except:
...
class Packager(object):
    ...
    def get_basic_stuff(self):
        ...
        pydll = win32api.GetModuleFileName(win32api.LoadLibrary('python24.dll'))
        crtdll = win32api.GetModuleFileName(win32api.LoadLibrary('msvcr71.dll'))
        pycomdll = win32api.GetModuleFileName(win32api.LoadLibrary('pythoncom24.dll'))
        pywintypesdll = win32api.GetModuleFileName(win32api.LoadLibrary('pywintypes24.dll'))
        ...
        shutil.copy(os.path.join(self.py_sitepackdir, 'win32', 'PythonService.exe'), w32_dest)
        shutil.copy(os.path.join(self.py_libdir, 'site.py'), self.libdir)
        shutil.copytree(os.path.join(self.py_basedir, 'DLLs'), os.path.join(self.distdir, 'DLLs'))
    ...
    def get_eggs(self, name):
        ...
        for egg in eggs:
            dest = os.path.join(self.sitepackdir, os.path.basename(egg.location))
    ...
    def get_dependencies(self, py):
        ...
        print '*** %s out of %s were new, unique dependencies ***' % (copied, len(mf.modules))
    ...
    def copy_module(self, module):
        if str(module.__file__).find('site-packages') != -1:
            ...
            return
        if module.__path__:
            dest = os.path.join(destbase, *module.__name__.split('.'))
    ...
    def is_new_dependency(self, module):
        if module.__name__ in self.unique_dependencies:
            return False
    ...
    def copy_special_files(self):
        specials = {'.': ['*.dll', 'prod.cfg', 'pgvl.tar', '*.js'], self.distname + '\\config': ['*.py', '*.cfg'], self.distname + '\\static\\css': ['*.css'], self.distname + '\\static\\images': ['*.gif', '*.png', '*.jpg', '*.ico'], self.distname + '\\static\\javascript': ['*.js'], self.distname + '\\templates': ['*.py', '*.html', '*.kid', '*.tmpl']}
