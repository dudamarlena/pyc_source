...
# [GCC 8.4.0]
# Embedded file name: /Users/xaralis/Workspace/elladev/ella/test_ella/test_core/test_listing.py
...
class TestListing(TestCase):
    ...
    def test_get_listing_empty(self):
        c = Category.objects.create(title='third nested category', description='category nested in case.category_nested_second', tree_parent=self.category_nested_second, site_id=self.site_id, slug='third-nested-category')
    ...
    def test_get_listing_with_immediate_children_no_duplicates(self):
        expected = [ listing for listing in self.listings if listing.category in (self.category, self.category_nested) ]
        listing = Listing.objects.create(publishable=expected[0].publishable, category=expected[0].category, publish_from=now() - timedelta(days=2))
        expected[0] = listing
        l = Listing.objects.get_listing(category=self.category, children=ListingHandler.IMMEDIATE)
        tools.assert_equals(expected, l)

    def test_get_listing_with_all_children_no_duplicates(self):
        listing = Listing.objects.create(publishable=self.publishables[0], category=self.category_nested_second, publish_from=now() - timedelta(days=2))
