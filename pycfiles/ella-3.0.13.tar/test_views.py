...
# [GCC 8.4.0]
# Embedded file name: /Users/xaralis/Workspace/elladev/ella/test_ella/test_core/test_views.py
...
class TestObjectDetail(ViewsTestCase):
    ...
    def test_object_detail(self):
        ...
        tools.assert_equals(ContentType.objects.get_for_model(self.publishable), response.context['content_type'])
        tools.assert_true('content_type_name' in response.context)
        tools.assert_equals(slugify(self.publishable._meta.verbose_name_plural), response.context['content_type_name'])
    ...
    def test_static_object_detail_redirects_to_correct_url_on_wrong_slug(self):
        ...
        tools.assert_equals('http://testserver/nested-category/%d-first-article/' % self.publishable.id, response['Location'])
    ...
    def test_static_redirects_preserve_custom_url_remainder(self):
        ...
        tools.assert_equals(301, response.status_code)
        tools.assert_equals('http://testserver/nested-category/%d-first-article/some/custom/url/action/' % self.publishable.id, response['Location'])
...
class TestGetTemplates(ViewsTestCase):
    ...
    def test_homepage_uses_only_path(self):
        tools.assert_equals([
         'page/category/ni-hao-category/category.html', 'page/category.html'], get_templates('category.html', category=self.category))

    def test_first_nested_uses_only_path(self):
        tools.assert_equals([
         'page/category/nested-category/category.html', 'page/category.html'], get_templates('category.html', category=self.category_nested))

    def test_more_nested_uses_fallback_to_parents(self):
        tools.assert_equals([
         'page/category/nested-category/second-nested-category/category.html', 'page/category/nested-category/category.html', 'page/category.html'], get_templates('category.html', category=self.category_nested_second))