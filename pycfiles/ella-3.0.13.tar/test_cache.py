...
# [GCC 8.4.0]
# Embedded file name: /Users/xaralis/Workspace/elladev/ella/test_ella/test_core/test_cache.py
# Compiled at: 2013-11-25 05:58:07
import time
...
from test_ella.test_core import create_basic_categories, create_and_place_a_publishable, create_and_place_more_publishables, list_all_publishables_in_category_by_hour
...
class TestCacheUtils(CacheTestCase):
    ...
    def test_get_article_uses_the_publishable_key_and_0_for_version(self):
        tools.assert_equals((':').join((utils.KEY_PREFIX, str(ContentType.objects.get_for_model(Publishable).pk), '123', '0')), utils._get_key(utils.KEY_PREFIX, ContentType.objects.get_for_model(Article), pk=123))
    ...
    def test_get_article_uses_the_publishable_key_and_version_from_cache(self):
        ...
        tools.assert_equals((':').join((utils.KEY_PREFIX, str(ContentType.objects.get_for_model(Publishable).pk), '123', '3')), utils._get_key(utils.KEY_PREFIX, ContentType.objects.get_for_model(Article), pk=123))
...
class TestRedisListings(TestCase):
    ...
    def test_listing_gets_removed_when_publishable_goes_unpublished(self):
        ...
        tools.assert_equals(set([
         ...
         'listing:d:2',
         'listing:d:3',
         'listing:ct:%d' % ct_id]), set(redis.client.keys()))
    ...
    def test_redis_lh_slicing(self):
        ...
        for offset, count in [(0, 10), (0, 1), (0, 2), (1, 2), (2, 3), (3, 3)]:
            ...
            tools.assert_equals([ l.publishable for l in partial ], [ l.publishable for l in self.listings[offset:offset + count] ])
...
class TestAuthorLH(TestCase):
    ...
    def test_listing_save_adds_itself_to_relevant_zsets(self):
        ...
        tools.assert_equals(set([
         ...
         'listing:ct:%d' % ct_id]), set(redis.client.keys()))
        tools.assert_equals(['%d:1' % ct_id, '%d:2' % ct_id, '%d:3' % ct_id], redis.client.zrange('listing:a:1', 0, 100))
    ...
    def test_gets_added_when_first_listing_is_added(self):
        ct_id = self.publishables[0].content_type_id
        tools.ok_('%d:1' % ct_id not in redis.client.zrange('listing:a:1', 0, 100))
        Listing.objects.get_or_create(publishable=self.publishables[0], category=self.publishables[0].category, publish_from=self.publishables[0].publish_from)[0]
        tools.ok_('%d:1' % ct_id in redis.client.zrange('listing:a:1', 0, 100))
...
class TestSlidingListings(TestCase):
    ...
    def test_regenerate_removes_old_slots(self):
        redis.client.zadd('sliding:WINDOWS', **{'sliding:1:20101010': 20101010, 
        ...
        tools.assert_equals([
         ('sliding:1:20101007', 20101007),
         ('sliding:1:20101009', 20101009),
         ('sliding:1:20101010', 20101010)], redis.client.zrange('sliding:WINDOWS', 0, -1, withscores=True))
