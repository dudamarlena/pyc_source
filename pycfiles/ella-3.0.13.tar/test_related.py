...
# [GCC 8.4.0]
# Embedded file name: /Users/xaralis/Workspace/elladev/ella/test_ella/test_core/test_related.py
...
class TestDefaultRelatedFinder(GetRelatedTestCase):
    ...
    def test_returns_unique_objects_or_shorter_list_if_not_available(self):
        ...
        tools.assert_equals(expected, [ p.pk for p in Related.objects.get_related_for_object(self.publishable, len(expected) * 3) ])

    def test_returns_publishables_listed_in_same_cat_if_no_related(self):
        expected = map(lambda x: x.pk, reversed(self.publishables))
        tools.assert_equals(expected, [ p.pk for p in Related.objects.get_related_for_object(self.publishable, len(expected)) ])

    def test_returns_at_most_count_objects(self):
        tools.assert_equals([
         self.publishables[(-1)].pk], [ p.pk for p in Related.objects.get_related_for_object(self.publishable, 1) ])
    ...
    def test_returns_manual_objects_of_correct_model_type_first(self):
        ...
        r.save()
        tools.assert_equals([
         self.publishables[0].pk, self.publishables[(-1)].pk], [ p.pk for p in Related.objects.get_related_for_object(self.publishable, 2, mods=[self.publishable.__class__]) ])
    ...
    def test_returns_only_manual_objects_when_direct_finder_specified(self):
        ...
        r.save()
        tools.assert_equals([
         self.publishables[0].pk], [ p.pk for p in Related.objects.get_related_for_object(self.publishable, 2, finder='directly') ])
    ...
    def test_returns_unique_objects(self):
        ...
        r.save()
        tools.assert_equals([
         self.publishables[(-2)].pk, self.publishables[(-1)].pk, self.publishables[(-3)].pk], [ p.pk for p in Related.objects.get_related_for_object(self.publishable, 3) ])
