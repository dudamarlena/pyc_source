...
# Embedded file name: build/bdist.macosx-10.10-intel/egg/circonus/client.py
# Compiled at: 2015-01-29 15:23:48
__doc__ = '\n\ncirconus.client\n~~~~~~~~~~~~~~~\n\n'
...
def log_http_error(f):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            log.error('%s: %s (%s. %s)', e.response.status_code, e.response.json()['code'], e.response.json()['message'], e.response.json()['explanation'])
            raise
...
class CirconusClient(object):
    ...
    def create_collectd_graphs(self, check_bundle, interface_names=None, mount_dirs=None, titles=None):
        ...
        return (
         responses and all([ status_codes.OK == r.status_code for r in responses ]), responses)