...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyenergy/interactive_graph.py
# Compiled at: 2015-02-20 08:29:01
...
import collections, numpy as np, scipy, scipy.stats, pyenergy
...
class Graph(QMainWindow):
    ...
    def updatesliders(self):
        self.tinterval = self.timeslider.value() / 1000.0
    ...
    def setup_graphs(self):
        self.data = {'1': {'xdata': [], 'idata': [], 'pdata': [], 'vdata': []}, '2': {'xdata': [], 'idata': [], 'pdata': [], 'vdata': []}, '3': {'xdata': [], 'idata': [], 'pdata': [], 'vdata': []}, 'Self': {'xdata': [], 'idata': [], 'pdata': [], 'vdata': []}}
        self.state = {'1': [
    ...
    def update(self):
        ...
        if disabled_tref is not None and first_on is not None:
            self.toffset += (measurements[self.tref][4] - measurements[first_on][4]) * 2.0 / 168000000 * 2.0
            self.tref = first_on
        if self.tref is not None:
            base_t = measurements[self.tref][4] * 2.0 / 168000000 * 2.0 + self.toffset
        else:
            base_t = 0
        problem = False
        for mp, m in measurements.items():
            ...
            v = float(vref) / 4096.0 * m[2] * 2
            c = float(vref) / gain / res / 4096.0 * m[3]
            p = v * c
            t = base_t
