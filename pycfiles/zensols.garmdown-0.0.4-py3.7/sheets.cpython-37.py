...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/garmdown/sheets.py
# Compiled at: 2020-03-02 18:14:59
# Size of source mod 2**32: 8182 bytes
import logging
from pathlib import Path
from datetime import datetime
import itertools as it, httplib2 as hl
...
class CompletedEntry(object):
    __doc__ = 'Represents a row of training data (bike/swim/run) in the Google training\n    spreadsheet.\n\n    '

    def __init__(self, idx, row_offset, datestr, swim=None, bike=None, run=None, strength=None):
        """Initialize.
    ...
    def row(self):
        ...
        row = tuple(map(lambda x:         if x == 0:
None # Avoid dead code: x, row))
        return row
    ...
    def __str__(self):
        return f"{self.rowidx}: date={self.date}: exist={self.exists}, " + f"s={self.swim},  b={self.bike}, r={self.run}, " + f"w={self.strength}"
...
class SheetUpdater(object):
    __doc__ = 'Updates a Google Sheets spreadsheet with activity data from the activity\n    database.\n\n    '
    ...
    def __init__(self, config):
        ...
        self.act_char_to_col_type = config.fetch_config.get_options(section='activity_sheet')
    ...
    def service(self):
        ...
        logger.info(f"getting last update with {cred_path} " + f"with file {token_path}")
        ...
        return gd.build((self.params.api), (self.params.version), http=(creds.authorize(hl.Http())),
    ...
    def _get_completed_entries(self):
        ...
        return map(lambda x: CompletedEntry(x[0], self.row_offset, x[1][0][0], *x[1][1]), data)
    ...
    def _get_update_range_entries(self, last_idx=None, end_date=None):
        ...
        return filter(lambda x: x.idx > last_idx and x.date < end_date, entries)
    ...
    def _upload_row_data(self, entries):
        ...
        range = self._get_completed_cell_range(entries[0].rowidx, entries[(-1)].rowidx)
