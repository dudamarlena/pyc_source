...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/garmdown/cli.py
...
from zensols.garmdown import Manager, Backuper, SheetUpdater, Reporter, AppConfig
...
class ReporterCli(object):
    ...
    def __init__(self, config, format, datestr=None):
        ...
        if format not in self.FORMAT_TYPES:
            ...
            raise ValueError(f"unknown format type '{format}' not one of {fopts}")
...
class ConfAppCommandLine(OneConfPerActionOptionsCliEnv):
    ...
    def __init__(self):
        detail_op = [
         ...
         {'dest':'detail', 
          'action':'store_true', 
          'default':False,  'help':'report details of missing data'}]
        ...
        date_op = ['-a', '--date', False,
         {'dest':'datestr', 
          'metavar':'mm/dd/yyyy',  'help':'the date to report, which defaults to today'}]
        ...
        super(ConfAppCommandLine, self).__init__(cnf,
          config_env_name='garmdownrc', pkg_dist='zensols.garmdown', config_type=AppConfig,
          default_action='sync')
