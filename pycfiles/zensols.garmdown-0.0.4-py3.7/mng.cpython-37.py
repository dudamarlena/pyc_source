...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/garmdown/mng.py
# Compiled at: 2020-03-02 18:14:59
# Size of source mod 2**32: 5919 bytes
import logging, sys
from pathlib import Path
import shutil
from zensols.actioncli import PersistedWork, persisted
from zensols.garmdown import Backuper, Persister, Fetcher
logger = logging.getLogger(__name__)

class Manager(object):
    __doc__ = 'Manages downloading and database work.  This includes downloading data from\n    the Garmin connect website and persisting status data in an SQLite\n    database.\n\n    '
    ...
    def sync_tcx(self, limit=None):
        ...
        for act in acts:
            ...
            if dl_path.exists():
                logger.warning(f"activity {act.id} is downloaded " + 'but not marked--marking now')
            else:
                ...
                if sr.st_size < self.download.min_size:
                    m = f"downloaded file {dl_path} has size " + f"{sr.st_size} < {self.download.min_size}"
                    raise ValueError(m)
    ...
    def import_tcx(self, limit=None):
        ...
        for act in acts:
            ...
            if import_path.exists():
                logger.warning(f"activity {act.id} is imported " + 'but not marked--marking now')
            else:
    ...
    def write_not_downloaded(self, detail=False, limit=None, writer=sys.stdout):
