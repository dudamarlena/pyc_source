...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/garmdown/db.py
# Compiled at: 2020-03-02 18:14:59
# Size of source mod 2**32: 6901 bytes
import logging, sys
from pathlib import Path
from datetime import datetime
import json, sqlite3
from zensols.actioncli import persisted, resource
from zensols.garmdown import ActivityFactory, Backup
...
class Persister(object):
    __doc__ = 'CRUDs activities in the SQLite database.\n\n    '
    ...
    def get_missing_downloaded(self, conn, limit=None):
        ...
        return tuple(self._thaw_activity(conn, self.sql.missing_downloads, limit))
    ...
    def get_missing_imported(self, conn, limit=None):
        ...
        if limit is None:
            limit = sys.maxsize
        return tuple(self._thaw_activity(conn, self.sql.missing_imported, limit))
    ...
    def get_activities_by_date(self, conn, datetime):
        ...
        return tuple(self._thaw_activity(conn, self.sql.activity_by_date, datestr))