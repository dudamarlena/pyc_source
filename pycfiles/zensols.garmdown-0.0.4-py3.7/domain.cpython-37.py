...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/garmdown/domain.py
...
class Activity(object):
    ...
    def write(self, writer=sys.stdout, detail=True):
        if detail:
            ...
            writer.write(f"{self.id}: start: {self.start_time}, type: {self.type}\n")
...
class ActivityFactory(object):
    ...
    def __init__(self, config):
        self.type_to_char = config.fetch_config.get_options(section='activity_type')
        self.char_to_name = config.fetch_config.get_options(section='activity_name')
