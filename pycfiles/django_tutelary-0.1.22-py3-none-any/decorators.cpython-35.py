...
# [GCC 8.4.0]
# Embedded file name: /Volumes/Work/dev/cadasta/django-tutelary/tutelary/decorators.py
    ...
    def retfn(obj, action):
        if action in perms_objs:
            if perms_objs[action] is None:
                ...
                return get_perms_object(getattr(obj, perms_objs[action]), action)
        else:
...
def permissioned_model(cls, perm_type=None, path_fields=None, actions=None):
    ...
    if not issubclass(cls, models.Model):
        raise DecoratorException('permissioned_model', "class '" + cls.__name__ + "' is not a Django model")
    added = False
    try:
        if not hasattr(cls, 'TutelaryMeta'):
            if perm_type is None or path_fields is None or actions is None:
                raise DecoratorException('permissioned_model', 'missing argument: all of perm_type, path_fields and ' + 'actions must be supplied')
            added = True
            cls.TutelaryMeta = type('TutelaryMeta', (object,), dict(perm_type=perm_type, path_fields=path_fields, actions=actions))
        cls.TutelaryMeta.pfs = [
         cls.TutelaryMeta.perm_type] + get_path_fields(cls)
        perms_objs = {}
...
def action_error_message(actions, req_actions, default=None):
    for req in req_actions:
        for a in actions:
            if isinstance(a, tuple) and a[0] == req and 'error_message' in a[1]:
                return (
                 a[1]['error_message'],)
