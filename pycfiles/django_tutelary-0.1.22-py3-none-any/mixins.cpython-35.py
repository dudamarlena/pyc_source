...
# [GCC 8.4.0]
# Embedded file name: /Volumes/Work/dev/cadasta/django-tutelary/tutelary/mixins.py
# Compiled at: 2017-09-07 05:44:10
...
class BasePermissionRequiredMixin:
    ...
    def get_permission_required(self):
        if not hasattr(self, 'permission_required') or self.permission_required is None:
    ...
    def get_permission_denied_message(self, default=None):
        ...
        if hasattr(self, 'model') and hasattr(self.model, 'TutelaryMeta'):
            return action_error_message(self.model.TutelaryMeta.actions, self.get_permission_required(), default)
...
class LoginPermissionRequiredMixin(PermissionRequiredMixin, LoginRequiredMixin):
...
class PermissionsFilterMixin:
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if permissions:
            ...
            if hasattr(self, 'permission_filter_queryset') and isinstance(self.permission_filter_queryset, Sequence):
                actions += tuple(self.permission_filter_queryset)
...
class APIPermissionRequiredMixin(BasePermissionRequiredMixin):
    __doc__ = 'Permission checking mixin for Django Rest Framework -- works just\n    like the ``PermissionRequiredMixin`` in the default Django\n    authentication system.\n\n    '
    ...
    def check_permissions(self, request):
        ...
        if hasattr(self, 'permission_filter_queryset') and self.permission_filter_queryset is not False and self.request.method == 'GET':
            ...
            has_perm = check_perms(self.request.user, self.get_permission_required(), objs, self.request.method)
        if not has_perm:
            msg = self.get_permission_denied_message(default='Permission denied.')
