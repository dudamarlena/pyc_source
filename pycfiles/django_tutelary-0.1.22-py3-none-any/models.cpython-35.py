...
# [GCC 8.4.0]
# Embedded file name: /Volumes/Work/dev/cadasta/django-tutelary/tutelary/models.py
# Compiled at: 2017-12-04 07:19:48
# Size of source mod 2**32: 12880 bytes
...
class Policy(models.Model):
    __doc__ = 'An individual policy has a name and a JSON policy body.  Changes to\n    policies are audited.\n\n    '
    ...
    def variable_names(self):
        pat = re.compile('\\$(?:([_a-z][_a-z0-9]*)|{([_a-z][_a-z0-9]*)})')
...
class RolePolicyAssign(models.Model):
    __doc__ = 'A role is defined by an ordered sequence of policies.\n\n    '
...
class RoleManager(models.Manager):
    __doc__ = "Custom manager for roles: ensures that role variable assignments\n    cover all the variables used in the role's policies.\n\n    "
...
class Role(models.Model):
    __doc__ = 'A policy role has a name, a sequence of policies and a set of\n    variable assignments.  Changes to roles are audited.\n\n    '
    name = models.CharField(max_length=200)
...
class PolicyInstance(models.Model):
    ...
    def __str__(self):
        return 'policy={} pset={} index={} role={} variables={}'.format(self.policy.name, self.pset.pk, self.index, self.role.name if self.role else 'NULL', self.variables)
...
class PermissionSetManager(models.Manager):
    __doc__ = 'Permission sets have a custom manager that folds all instances with\n    the same set of policy instances together in the database.\n\n    '
        ...
        for psetid in _policy_psets(canonpols):
            ...
            if len(pis) == len(canonpols):
                ...
                for pi, canonpol in zip(pis, canonpols):
                    if pi.variables != canonpol[1] or canonpol[2] is not None and pi.role != canonpol[0] or canonpol[2] is None and pi.policy != canonpol[0]:
                        same = False
                        break
...
class PermissionSet(models.Model):
    ...
    def tree(self):
        ...
        if cached is None:
            ptree = engine.PermissionTree(policies=[engine.PolicyBody(json=pi.policy.body, variables=json.loads(pi.variables)) for pi in PolicyInstance.objects.select_related('policy').filter(pset=self)])
            cache.set(key, ptree)
            cached = ptree
        return cached
...
def user_cache_key(user):
    if user is not None:
        return 'tutelary:user:' + str(user.pk)
    return 'anon'
...
def user_assigned_policies(user):
    ...
    for pi in PolicyInstance.objects.select_related('policy', 'role').filter(pset=pset):
