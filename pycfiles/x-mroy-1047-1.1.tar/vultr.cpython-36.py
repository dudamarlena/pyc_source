...
# [GCC 8.4.0]
# Embedded file name: /Users/dr/Documents/code/Python/vps-manager/web/vultr.py
# Compiled at: 2019-05-28 23:31:28
# Size of source mod 2**32: 17468 bytes
import requests
from termcolor import cprint, colored
import json, os, re, cmd, random, getpass, base64
    ...
    def do_create_script(self, file):
        ...
        if os.path.exists(file):
            with open(file) as (fp):
                t = fp.read()
        con = requests.post((APIURL + 'startupscript/create'), data={'name':'script', 
         'script':t},
        ...
        if not self.SCRIPTID:
            ...
            if yn in ('', 'y', 'Y'):
                ...
                while True:
                    ...
                    elif re.match('^\\d+$', c) and c in ids:
                        self.SCRIPTID = c
                        break
                    else:
                        cprint('try again !', 'red')

        create_one((self.api_key), DCID=self.DCID, 
         OSID=self.OSID, 
