...
# [GCC 8.4.0]
# Embedded file name: /Users/dr/Documents/code/Python/vps-manager/web/test_route.py
# Compiled at: 2019-05-28 23:31:28
# Size of source mod 2**32: 13573 bytes
import json, os, time, sys, atexit, signal, asyncio, time
...
def sync(token=None, my_ip=None, **options):
    ...
    if not token:
        print('no token')
        return
    res = requests.get('https://api.vultr.com/v1/server/list', headers={'API-Key': token}).json().values()
    ss = {}
    ...
    for i in res:
        if my_ip:
            if i['main_ip'] == my_ip:
                continue
        s = Host(host=(i['main_ip']), passwd=(i['default_password']),
          port='22',
          user='root',
          location=(i['location']),
          os=(i['os']),
...
def main():
    ...
    if args.gen:
        if os.path.exists('/etc/supervisor'):
            if not os.path.exists('/etc/supervisor/conf.d/x-test.conf'):
                gen_supervisor()
        else:
