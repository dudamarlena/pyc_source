...
# [GCC 8.4.0]
# Embedded file name: /Users/dr/Documents/code/Python/vps-manager/web/controller.py
# Compiled at: 2019-05-31 05:04:55
# Size of source mod 2**32: 32829 bytes
import os, re, json, time, random, tornado, tornado.web, functools, asyncio, aiofiles, concurrent
...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def get_current_user(self):
        return (
         self.get_cookie('user'), self.get_cookie('passwd'))
...
class Get_ipHandler(BaseHandler):
    ...
    def query_hosts(self):
        ...
        for d in self.db.query(Host):
            geojsonFeature = {'type':'Feature',  'properties':{'name':'Coors Field', 
            ...
            if not ip == '0.0.0.0':
                if ip.startswith('127.0') or ip.startswith('192.168') or ip.startswith('172.16'):
                    pass
                else:
                    ...
                    while geo_mark in geo_ll:
                        geo = [
                         str(float(geo[0]) + random.random()), str(float(geo[1]) + random.random())]
                        geo_mark = '-'.join(geo)
...
class AsyncremoteapiHandler(BaseHandler):
                    ...
                    if post_args['op'] == 'mark':
                        ...
                        if post_args['op'] == 'destroy':
                            ...
                            if post_args['op'] == 'update':
                                ...
                                if post_args['op'] == 'settoken':
                                    api = post_args['token']
                                    t = Token(token=api)
                                    t.save(self.db)
                                    self.write(json.dumps({'msg': 'ok'}))
                                else:
                                    if post_args['op'] == 'create_server':
                                        msg = post_args['msg']
                                        osn = post_args['os']
                                        label = post_args['label']
                                        print(post_args)
                                        ...
                                        await back_request('https://api.vultr.com/v1/startupscript/list', after_get_scriptid, method='get',
                                          headers={'API-Key': token},
                                          loop=tloop)
                                    else:
                                        if post_args['op'] == 'add-hosts':
                                            ip = post_args.get('ip')
                                            port = post_args.get('port')
                                            pwd = post_args.get('password')
                                            await self.back_add_hosts(ip, port, pwd)
                                        else:
                                            if post_args['op'] == 'getpass':
                                                hh = self.db.query_one(Host, host=(post_args['ip']))
                                                self.write(json.dumps({'data': {'passwd':hh.passwd, 
                                                          'createTime':hh.getTime(), 
                                                          'host':hh.host}}))
                                            else:
                                                if post_args['op'] == 'base':
                                                    h = post_args['ip']
                                                    host = self.db.query_one(Host, host=h)
                                                    ...
                                                    if post_args['op'] == 'qr':
                                                        ...
                                                        if post_args['op'] == 'set-proxy':
                                                            ...
                                                            if post_args['op'] == 'link':
                                                                ...
                                                                if post_args['op'] == 'set-mode':
                                                                    ip = post_args['ip']
                                                                    mode = post_args['mode'][0]
                                                                    try:
                                                                        mode = int(mode)
                                                                    except ValueError:
                                                                        self.write(json.dumps({'res': 'mode: must int 1,2,3'}))
                                                                    ...
                                                                    if os.path.exists(f):
                                                                        ...
                                                                        self.write(json.dumps({'res': 'no ss conf to set!!'}))
                                                                else:
                                                                    if post_args['op'] == 'reset-routes':
                                                                        ip = post_args['ip']
                                                                        ...
                                                                        if post_args['op'] == 'build-routes':
                                                                            try:
                                                                                ...
                                                                                self.write(json.dumps({'res': str(e)}))

        if post_args['op'] == 'all_links':
            h = post_args['ip']
    ...
    def all_links(self, ip):
        ...
        if os.path.exists(path):
            ...
            if b'routes' in routes:
                ips = [{'ip':i['server'],  'geo':self.get_ip(i['server'])} for i in json.loads(routes.decode())['routes'] if self.get_ip(i['server']) if i['server'] != ip]
                return ips
        return []
    ...
    async def async_link(self, f_ip, to_ip, use_time=600):
        ...
        if isinstance(res, bytes):
            ...
            if not res:
                self.write(json.dumps({'res':'link failed', 
                 'log':'bad'}))
            else:
                if 'Ich liebe dich' in res:
                    self.write(json.dumps({'res':'link ok', 
                     'log':'good'}))
                else:
                    self.write(json.dumps({'res':res, 
                     'log':'bad'}))
