...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-intel/egg/hadoop_mock/hadoop_mock.py
# Compiled at: 2015-04-20 10:25:39
import sys, os, subprocess, argparse, glob, datetime, shutil, pipes
...
def mock(args):
    ...
    for row in files.split('\n'):
        ...
        if not os.path.exists(dst_dir):
            os.makedirs(dst_dir)
        if dst:
            calls.append(dict(args='./hadoop_ssh fs -cat %s | head -%d > %s' % (fname, args.head, dst), shell=True))
