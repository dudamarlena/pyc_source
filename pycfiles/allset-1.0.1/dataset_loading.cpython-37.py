...
# Embedded file name: build/bdist.linux-x86_64/egg/allrank/data/dataset_loading.py
# Compiled at: 2020-03-04 08:08:01
# Size of source mod 2**32: 10154 bytes
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        x, y, indices = sample
        return (
         torch.from_numpy(x).type(torch.float32), torch.from_numpy(y).type(torch.float32), torch.from_numpy(indices).type(torch.long))
...
class FixLength(object):
    ...
    def __call__(self, sample):
        ...
        return (
         fixed_len_x, fixed_len_y, indices)
...
class LibSVMDataset(Dataset):
    ...
    def from_svm_file(cls, svm_file_path, transform=None):
        ...
        logger.info('loaded dataset from {} and got x shape {}, y shape {} and query_ids shape {}'.format(svm_file_path, x.shape, y.shape, query_ids.shape))
    ...
    def shape(self):
        ...
        return [
         batch_dim, document_dim, features_dim]
