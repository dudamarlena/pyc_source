...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/pybasic/basic_yacc.py
# Compiled at: 2019-04-20 06:35:12
...
precedence = (('left', 'COMMA'), ('left', 'AND', 'OR'), ('left', 'EQUALS', 'NOT_EQUAL', 'GREATER_THAN', 'LESS_THAN', 'EQUAL_GREATER_THAN', 'EQUAL_LESS_THAN'),
              ('left', 'AS'), ('left', 'PLUS', 'MINUS'), ('left', 'TIMES', 'DIVIDE', 'MOD'),
              ('left', 'EXP'), ('right', 'UMINUS', 'NOT'))
...
def p_for_block_begin(p):
    ...
    p[0].add_group([
     ASTNode(type='id', value=(p[2])),
     p[4],
     p[6],
     ASTNode(type='number', value=1) if len(p) == 7 else p[8],
     ASTNode.BlockNode()])
...
def p_return(p):
    ...
    func_node = current_root.parent
    if len(p) == 2:
        if func_node.value not in ('<FUNCTION>', '<SUB>'):
            raise BasicError('RETURN without FUNCTION or SUB')
    if len(p) == 3:
        if func_node.value != '<FUNCTION>':
