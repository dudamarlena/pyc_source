...
# Embedded file name: build/bdist.linux-i686/egg/alfanous/Support/whoosh/reading.py
# Compiled at: 2015-06-30 06:52:38
__doc__ = 'This module contains classes that allow reading from an index.\n'
...
class IndexReader(ClosableMixin):
    ...
    def most_frequent_terms(self, fieldid, number=5, prefix=''):
        ...
        """
        return nlargest(number, ((tf, token) for token, _, tf in self.iter_prefix(fieldid, prefix)))
    ...
    def most_distinctive_terms(self, fieldid, number=5, prefix=None):
        ...
        """
        return nlargest(number, ((tf * (1.0 / df), token) for token, df, tf in self.iter_prefix(fieldid, prefix)))
