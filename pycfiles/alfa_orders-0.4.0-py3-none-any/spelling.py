...
# Embedded file name: build/bdist.linux-i686/egg/alfanous/Support/whoosh/spelling.py
# Compiled at: 2015-06-30 06:52:38
__doc__ = 'This module contains functions/classes using a Whoosh index as a backend for\na spell-checking engine.\n'
...
class SpellChecker(object):
    ...
    def __init__(self, storage, indexname='SPELL', booststart=2.0, boostend=1.0, mingram=3, maxgram=4, minscore=0.5):
        """
    ...
    def suggestions_and_scores(self, text, weighting=None):
        ...
        try:
            result = s.search(q)
            return [ (fs['word'], fs['score'], result.score(i)) for i, fs in enumerate(result) if fs['word'] != text
                   ]
        finally:
            s.close()
