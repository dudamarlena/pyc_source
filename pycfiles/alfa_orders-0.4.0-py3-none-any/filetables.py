...
# Embedded file name: build/bdist.linux-i686/egg/alfanous/Support/whoosh/filedb/filetables.py
# Compiled at: 2015-06-30 06:52:38
__doc__ = "This module defines writer and reader classes for a fast, immutable\non-disk key-value database format. The current format is identical\nto D. J. Bernstein's CDB format (http://cr.yp.to/cdb.html).\n"
...
def decode_termkey(key):
    return (
     unpackushort(key[:_USHORT_SIZE]), utf8decode(key[_USHORT_SIZE:])[0])
...
class FileHashReader(object):
    ...
    def items(self):
        ...
        for keypos, keylen, datapos, datalen in self._ranges():
            yield (
             read(keypos, keylen), read(datapos, datalen))
...
class OrderedHashReader(FileHashReader):
    ...
    def __init__(self, dbfile):
        ...
        lastpos, lastnum = self.read2ints(2040)
