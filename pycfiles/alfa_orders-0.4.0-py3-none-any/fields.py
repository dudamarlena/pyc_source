...
# Embedded file name: build/bdist.linux-i686/egg/alfanous/Support/whoosh/fields.py
# Compiled at: 2015-06-30 06:52:38
__doc__ = ' Contains functions and classes related to fields.\n'
import datetime, re, struct
from analysis import IDAnalyzer, RegexAnalyzer, KeywordAnalyzer, StandardAnalyzer, NgramAnalyzer
...
class FieldType(object):
    ...
    * unique (boolean): whether this field's value is unique to each document.
    ...
    parse_query = None
    indexed = True
    __inittypes__ = dict(format=Format, vector=Format, scorable=bool, stored=bool, unique=bool)

    def __init__(self, format, vector=None, scorable=False, stored=False, unique=False):
        self.format = format
        self.vector = vector
...
class IDLIST(FieldType):
    ...
    def __init__(self, stored=False, unique=False, expression=None, field_boost=1.0):
        ...
        """
        expression = expression or re.compile('[^\\r\\n\\t ,;]+')
...
class KEYWORD(FieldType):
    ...
    """
    __inittypes__ = dict(stored=bool, lowercase=bool, commas=bool, scorable=bool, unique=bool, field_boost=float)

    def __init__(self, stored=False, lowercase=False, commas=False, scorable=False, unique=False, field_boost=1.0):
        """
...
class TEXT(FieldType):
    ...
    """
    __inittypes__ = dict(analyzer=object, phrase=bool, vector=Format, stored=bool, field_boost=float)

    def __init__(self, analyzer=None, phrase=True, vector=None, stored=False, field_boost=1.0):
        """
...
class NGRAM(FieldType):
    ...
    def __init__(self, minsize=2, maxsize=4, stored=False, field_boost=1.0):
        ...
        """
        self.format = Frequency(analyzer=NgramAnalyzer(minsize, maxsize), field_boost=field_boost)
        self.scorable = True
        self.stored = stored
