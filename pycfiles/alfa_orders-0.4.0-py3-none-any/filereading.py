...
# Embedded file name: build/bdist.linux-i686/egg/alfanous/Support/whoosh/filedb/filereading.py
...
from alfanous.Support.whoosh.filedb.filetables import FileTableReader, FileRecordReader, FileListReader, encode_termkey, decode_termkey, encode_vectorkey, decode_vectorkey, decode_terminfo, depickle, unpackint
...
def open_terms(storage, segment):
    ...
    return FileTableReader(termfile, keycoder=encode_termkey, keydecoder=decode_termkey, valuedecoder=decode_terminfo)
...
def open_storedfields(storage, segment, storedfieldnames):
    ...
    return FileListReader(listfile, segment.doc_count_all(), valuedecoder=dictifier)


def open_vectors(storage, segment):
    vectorfile = storage.open_file(segment.vector_filename)
    return FileTableReader(vectorfile, keycoder=encode_vectorkey, keydecoder=decode_vectorkey, valuedecoder=unpackint)
...
class SegmentReader(IndexReader):
    ...
    def __init__(self, storage, segment, schema):
        ...
        self._fieldnum_to_scorable_pos = dict((fnum, i) for i, fnum in enumerate(self._scorable_fields))
    ...
    def __contains__(self, term):
        return (
         self.schema.to_number(term[0]), term[1]) in self.termtable
    ...
    def postings(self, fieldid, text, exclude_docs=frozenset()):
        ...
        try:
            totalfreq, offset, postcount = self.termtable[(fieldnum, text)]
        except KeyError:
