...
# Embedded file name: build/bdist.linux-x86_64/egg/tests/t_matlab.py
# Compiled at: 2013-10-24 05:23:45
import unittest, sys, pymatlab, numpy
...
class MatlabTestCase(unittest.TestCase):
    ...
    def getvalue_complex(self):
        for datatype in [
         'single',
         'double']:
            a = eye(4, 5, dtype=datatype) + eye(4, 5, dtype=datatype) * complex(0.0, 1.0)
    ...
    def putvalue_complex(self):
        for datatype in [
         'complex128',
         'complex64']:
            ar = randn(2, 3) * 10 + randn(2, 3) * complex(0.0, 10.0)
            a = ar.astype(datatype)
            self.session.putvalue('a', a)
            self.session.run('a')
            buf = self.session.buf.value
    ...
    def getput_complex(self):
        for datatype in [
         'complex64',
         'complex128']:
            ar = randn(2, 4, 3) + randn(2, 4, 3) * complex(0.0, 1.0)
            a = ar.astype(datatype)
            self.session.putvalue('A', a)
            b = self.session.getvalue('A')
            self.assertEqual(a.dtype, b.dtype)
