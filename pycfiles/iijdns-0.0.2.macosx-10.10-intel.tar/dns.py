...
# [GCC 8.4.0]
# Embedded file name: /Users/murakamitig/.virtualenvs/dns/lib/python2.7/site-packages/iijdns/dns.py
# Compiled at: 2015-02-10 02:10:09
API_VER = '20140601'
...
def initialize(access_key, secret_key, service_code, alias=DEFAULT_ALIAS):
    ...
    if alias in _secret:
        ...
        _secret[alias] = {ACCESS_KEY: access_key, 
           SECRET_KEY: secret_key, 
           SERVICE_CODE: service_code}
...
def base_req(http_method, uniq_uri, expire_time, alias=DEFAULT_ALIAS, **kwargs):
    ...
    """
    import urllib3, urllib, hmac, hashlib, base64
    secret = get_secret(alias=alias)
    access_key = secret[ACCESS_KEY]
    secret_key = secret[SECRET_KEY]
    service_code = secret[SERVICE_CODE]
    request_path = '/r/%s/%s/%s' % (API_VER, service_code, uniq_uri)
    ...
    headers = {'x-iijapi-Expire': expire_time.strftime('%Y-%m-%dT%H:%M:%SZ'), 
