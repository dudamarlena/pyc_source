...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/napalm_brocade/brocade.py
# Compiled at: 2016-10-10 18:32:11
"""
...
from napalm_base.exceptions import ConnectionException, MergeConfigException, ReplaceConfigException, SessionLockedException, CommandErrorException
import os, re
...
class BrocadeDriver(NetworkDriver):
    ...
    def open(self):
        ...
        try:
            self.device = ConnectHandler(device_type='brocade_vdx', ip=self.hostname, port=self.port, username=self.username, password=self.password, timeout=self.timeout, verbose=True)
        except Exception:
    ...
    def get_environment(self):
        ...
        environment['power']['invalid'] = {'status': True, 
           'output': -1.0, 
           'capacity': -1.0}
        environment.setdefault('fans', {})
        environment['fans']['invalid'] = {'status': True}
        environment.setdefault('temperature', {})
        environment['temperature']['invalid'] = {'is_alert': False, 
           'is_critical': False, 
           'temperature': -1.0}
        return environment
    ...
    def get_interfaces(self):
        ...
        for line in output:
            ...
            interface_list[interface] = {'is_up': is_up, 
    ...
    def get_interfaces_counters(self):
        ...
        for line in lines:
            if len(line) == 0:
                return {}
            if len(line.split()) == 9:
                interface_type, interface, pkts_rx, pkts_tx, err_rx, err_tx, discards_rx, discards_tx, crc_rx = line.split()
    ...
    def get_mac_address_table(self):
        ...
        for line in lines:
            if len(line) == 0:
                return {}
            if len(line.split()) == 7:
                vlanid, tt, mac_address, type, state, port_type, port = line.split()
                entry = {'vlanid': vlanid, 
