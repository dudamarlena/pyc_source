...
# [GCC 8.4.0]
# Embedded file name: /Users/luoxi/Project/yunify/qingcloud-cli/qingcloud/cli/qs_client/actions/bucket.py
# Compiled at: 2017-03-01 04:19:20
...
class ListObjectsAction(BaseAction):
    ...
    usage = '%(prog)s -b <bucket> [-p <prefix> -d <delimiter> -m <marker> -l ' + '<limit> -f <conf_file>]'

    @classmethod
    def add_ext_arguments(cls, parser):
        parser.add_argument('-b', '--bucket', dest='bucket', required=True, help='The bucket name')
        parser.add_argument('-p', '--prefix', dest='prefix', help='The specified prefix that returned keys should start with')
        parser.add_argument('-d', '--delimiter', dest='delimiter', help='Which character to use for grouping the keys')
        parser.add_argument('-m', '--marker', dest='marker', help='The key to start with when listing objects in the bucket')
        parser.add_argument('-l', '--limit', dest='limit', type=int, default=20, help='The maximum number of keys returned')
        return parser
...
class SetBucketAclAction(BaseAction):
    ...
    def add_ext_arguments(cls, parser):
        parser.add_argument('-b', '--bucket', dest='bucket', required=True, help='The bucket name')
        parser.add_argument('-A', '--acl', required=True, nargs='*', help='ACL entries, each entry is in format type,id|name,permission. permission can be READ, WRITE or FULL_CONTROL. Multiple entries are separated by spaces')
        return parser
    ...
    def send_request(cls, options):
        ...
        for pairs in options.acl:
            ...
            if t == 'user':
                ...
                exit(1)
            acl.append({'grantee': grantee, 
               'permission': perm})

        resp = cls.conn.make_request('PUT', options.bucket, params=params, data=json_dumps({'acl': acl}))
        if resp.status != HTTP_OK:
            prints_body(resp)
        return
