...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/componentized/builtins/joiners.py
# Compiled at: 2011-07-12 09:15:45
from pyf.dataflow.components import zip_merging, ordered_key_merging, linear_merging, izip_longest
...
import operator, itertools, time, logging
...
class LinearJoiner(DataJoiner):
    ...
    * simple: takes values from all sources, and stops when any of the sources is finished.
    ...
    def intelligent_izip_longest(self, input_ports, sources, metadata=None, fillvalue=Ellipsis, clear_each=50, threshold=30):
        ...
        while continue_sources:
            ...
            for (source_name, source) in input_flows:
                ...
                try:
                    ...
                    if source_name not in burnt_sources:
...
class SequencialJoiner(DataJoiner):
    ...
    def __get_source_names(self):
        ...
        if len(source_nodes) > 0:
            return dict([ (node.get('name'), node.get('priority', 0)) for node in source_nodes
                        ])
        else:
            return
            return
    ...
    def launch(self, sources, out, ports=None):
        ...
        if source_names is not None and ports is not None:
            ...
            ordered_sources = map(operator.itemgetter(1), sorted(ordered_sources, key=operator.itemgetter(0)))
        else:
...
class OrderedKeyJoiner(DataJoiner):
    ...
    def get_keys(self):
        if len(self.config_node.findall('key')) > 0:
            keys = [ (k.text.strip(), k.get('type', 'attribute'), k.get('source', None)) for k in self.config_node.findall('key')
                   ]
            if len(keys) > 1:
                return dict([ (source, self.prepare_key(value, type, source)) for (value, type, source) in keys
                            ])
    ...
    def launch(self, sources, out, ports=None):
        sources = list(sources)
        self.do_debug = self.get_config_key('debug', 'false').lower() in ('true', 'on',
                                                                          'yes')
        keys = self.get_keys()
        ...
        if output_type == 'packet':
            for row in merger:
                yield Packet(dict([ (ports[num], item) for (num, item) in enumerate(row)
                                  ]))
...
class OrderedKeyMerger(OrderedKeyJoiner):
    ...
    def get_keys(self):
        if len(self.config_node.findall('source')) > 1:
            keys = [ (k.text.strip(), k.get('type', 'attribute'), k.get('key'), k.get('source')) for k in self.config_node.findall('source')
                   ]
            return dict([ (source, (name, self.prepare_key(value, type, source))) for (name, type, value, source) in keys
                        ])
        raise ValueError, 'To use the ordered key object merger, please set a key with a distinct name per input flow'
...
class NeutralJoiner(DataJoiner):
    ...
    def launch(self, sources, out, ports=None):
        sources = list(sources)
        yield Packet(dict([ (ports[num], source) for (num, source) in enumerate(sources)
                          ]))