...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/componentized/components/base.py
...
from pyf.componentized.configuration.keys import SimpleKey, ComplexCompoundKey, BooleanKey
...
class ComponentMetaClass(type):
    ...
    def __new__(metaclass, classname, bases, class_dict):
        ...
        def document_key(key, level):
            v = '- %s: %s' % (
             key['label'] and '"**%s**" `(label: %s)`' % (key['key'], key['label']) or '"**%s**" `(label: %s)`' % (key['key'], key['key'].title()),
             key['type_info'])
            if key.get('default'):
        ...
        get_recursive_bases = lambda dd: reduce(operator.add, [ get_bases(cd) and [cd] + get_recursive_bases(get_bases(cd)) or [cd] for cd in dd
                     ])
        base_list = reversed(get_recursive_bases(bases))
        configuration_list = list()
        for (base_name, base) in [ (b.__name__, b.__dict__) for b in base_list
                                 ] + [(classname, class_dict)]:
            if base.get('configuration'):
                for key in base.get('configuration'):
                    if key.key in map(operator.attrgetter('key'), configuration_list):
                        configuration_list.remove(filter(lambda x: x.key == key.key, configuration_list)[0])
...
class Component(object):
    ...
    configuration = [
     ComplexCompoundKey('advanced', content=[
      BooleanKey('separate_process', label='Separate Process')], collapsible=True, collapsed=True, label='Advanced'),
    ...
    def get_config_key(self, key, default=None):
        ...
        if node is None:
            ...
            if self.has_configuration_protocol and self.configuration_keys.get(key):
                if hasattr(self.configuration_keys.get(key), 'gets_node_list') and self.configuration_keys.get(key).gets_node_list:
                    return self.configuration_keys[key].from_xml(self.get_config_key_nodes(key), default=default)
                else:
    ...
    def get_config_key_node(self, key):
        node_current = self.config_node.find(key)
        if node_current is None and hasattr(self, 'process_config_node'):
            node_current = self.process_config_node.find(key)
        return node_current

    def get_config_key_nodes(self, key):
        nodes_current = self.config_node.findall(key)
        if (nodes_current is None or len(nodes_current) == 0) and hasattr(self, 'process_config_node'):
...
class CodeComponent(Component):
    ...
    def introspect_function(self):
        ...
        """
        self.code_func_name = compiler.parse(self.code_text, 'exec').node.nodes[(-1)].name

    @component('IN', 'OUT')
    def launch_function(self, values, out):
        evaldict = dict(out=out, get_config_key=self.get_config_key, get_config_key_node=self.get_config_key_node, get_config_key_nodes=self.get_config_key_nodes, config_node=self.config_node, message_callback=self.message_callback, param=self.params)
    ...
    def launch_producer(self, progression_callback=None, message_callback=None, params=None):
        evaldict = dict(progression_callback=progression_callback, message_callback=message_callback, params=params, get_config_key=self.get_config_key, get_config_key_nodes=self.get_config_key_nodes, get_config_key_node=self.get_config_key_node)
        exec self.code in evaldict
