...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/aiogcd/connector/service_account_token.py
...
import asyncio, aiohttp, datetime, json, jwt, logging, time, typing
ScopeList = typing.List[str]
JWT_GRANT_TYPE = 'urn:ietf:params:oauth:grant-type:jwt-bearer'
GCLOUD_TOKEN_DURATION = 3600
MISMATCH = "Project name passed to Token does not match service_file's project_id."
    ...
    async def ensure_token(self):
        if self.acquiring:
            ...
            if not self.access_token:
                self.acquiring = asyncio.ensure_future(self._acquire_access_token())
                await self.acquiring
            else:
...
async def _smoke_test(project, service_file, scopes=None):
    ...
    with aiohttp.ClientSession() as (session):
        token = ServiceAccountToken(project,
          service_file,
          session=session,
          scopes=scopes)
