...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/aiogcd/connector/pathelement.py
...
def path_element_from_decoder(decoder):
    ...
    if not (kind is not None and name_or_id is not None):
        raise AssertionError('Expecting a path element with a kind and name/id.')
...
class PathElement:
    ...
    def __init__(self, kind, name_or_id):
        if not name_or_id is None:
            assert isinstance(name_or_id, (int, str)), 'Expecting a str or int type but got: {}'.format(type(name_or_id))
        self.kind = kind
        self.id = name_or_id
