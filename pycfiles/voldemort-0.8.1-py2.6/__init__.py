...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-intel/egg/voldemort/__init__.py
# Compiled at: 2014-01-24 01:50:14
import os, sys, logging, datetime, shutil, urllib
from optparse import OptionParser
import BaseHTTPServer
from SimpleHTTPServer import SimpleHTTPRequestHandler
import template, config, util, paginator
...
class Voldemort(object):
    ...
    def __init__(self, work_dir, conf):
        ...
        self.ignored_items = [
         ...
         os.path.join(self.work_dir, '.DS_Store')] + self.config.layout_dirs
        log.debug('The following list of directories/files will be ignored: %s' % (', ').join(self.ignored_items))
        self.posts = []
        self.tags = {}
        self.pages = []
    ...
    def get_page_name_for_site(self, filename, extn='.html'):
        ...
        if ext not in self.template_extensions or ext in self.preserved_extensions:
            return filename
    ...
    def parse_meta_data(self):
        ...
        for post in os.listdir(self.config.posts_dir):
            ...
            post_meta['date'] = datetime.datetime.strptime(post_meta['date'], self.date_format)
            post_url = os.path.join('/', post_meta['date'].strftime(self.config.post_url), os.path.splitext(post_meta['filename'].split(self.config.posts_dir)[1][1:])[0])
        ...
        for (post_num, post) in enumerate(self.posts):
            ...
            for tagname in post.get('tags', []):
                tag_url = os.path.join('/', 'tag', urllib.quote_plus(tagname.lower()))
                post_tags.append({'name': tagname, 'url': tag_url, 
        ...
        for (tagname, post) in self.tags.iteritems():
            tag_url = os.path.join('/', 'tag', urllib.quote_plus(tagname.lower()))
            tags_info.append({'name': tagname, 'url': tag_url, 
               'posts': self.tags[tagname]})
        ...
        template.env.globals.update({'posts': self.posts, 'site': site, 
           'tags': tags_info})
        return
    ...
    def paginate(self, filename, page_meta):
        ...
        for pgr in self.paginator:
            ...
            html = template.render(page_meta['raw'], {'page': page_meta, 'paginator': pgr})
            if pgr.current_page == 1:
                paginator_path = os.path.join(self.config.site_dir, filename.split(self.work_dir)[1][1:])
                log.debug('Generating page %s' % paginator_path)
                write_html(paginator_path, html)
            current_page = 'page%s' % pgr.current_page
            (site_path, ext) = os.path.splitext(filename.split(self.work_dir)[1][1:])
            if site_path == 'index':
                site_path = ''
            paginator_path = os.path.join(self.config.site_dir, site_path, current_page, 'index.html')
            log.debug('Generating page %s' % paginator_path)
            write_html(paginator_path, html)
    ...
    def generate_posts(self):
        ...
        for post in self.posts:
            html = template.render(post['raw'], {'post': post, 'page': post})
        ...
        for (root, dirs, files) in os.walk(self.work_dir):
            ...
            for filename in files:
                ...
                if extn not in self.template_extensions:
                    dest = os.path.join(self.config.site_dir, filename.split(self.work_dir)[1][1:])
                    move_to_site(filename, dest)
                    continue
                page_meta = template.get_meta_data(filename)
                page_url = os.path.join('/', os.path.splitext(page_meta['filename'].split(self.work_dir)[1][1:])[0])
                ...
                page_path = os.path.join(self.config.site_dir, filename.split(self.work_dir)[1][1:])
    ...
    def generate_tags(self):
        ...
        tag_template_meta = template.get_meta_data(os.path.join(self.work_dir, self.tag_template))
        for (tagname, posts) in self.tags.iteritems():
            render_vars = {'tag': {'name': tagname, 'posts': posts}, 'page': tag_template_meta}
            html = template.render(tag_template_meta['raw'], render_vars)
            tag_page_path = os.path.join(self.config.site_dir, 'tag', urllib.quote_plus(tagname.lower()), 'index.html')
    ...
    def generate_sitemap(self, filename='sitemap.xml'):
        ...
        sitemap = template.render(SITE_MAP, {'posts': self.posts, 'pages': self.pages})
...
def main():
    ...
    parser.add_option('-s', '--serve', action='store_true', help='Start the HTTP Server', default=False)
    parser.add_option('-p', '--port', help='Port inwhich the HTTPServer should run', type='int', default=8080)
    parser.add_option('-d', '--deploy', action='store_true', help='Deploy this website', default=False)
    parser.add_option('-u', '--user', help='Login name for server', default=conf.deploy.get('user'))
    parser.add_option('-a', '--at', help='Server address to deploy the site', default=conf.deploy.get('at'))
    parser.add_option('-t', '--to', help='Deployment directory', default=conf.deploy.get('to'))
    parser.add_option('--skip-blog', action='store_true', help='Skip blog posts generation', default=False)
    parser.add_option('--skip-pages', action='store_true', help='Skip pages generation', default=False)
    parser.add_option('--skip-tags', action='store_true', help='Skip tags generation', default=False)
    parser.add_option('--skip-feeds', action='store_true', help='Skip Atom feed generation', default=False)
    parser.add_option('--skip-sitemap', action='store_true', help='Skip sitemap generation', default=False)
