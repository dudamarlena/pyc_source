...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-intel/egg/voldemort/template.py
# Compiled at: 2013-08-02 16:32:03
import os, io, re, logging, markdown
...
POST_RE = re.compile('---(?P<meta>.*)---(?P<body>.*)', re.DOTALL)
...
class MarkdownExtension(Extension):
    ...
    def __init__(self, environment):
        ...
        environment.extend(markdowner=markdown.Markdown(extensions=['codehilite']))

    def parse(self, parser):
        lineno = parser.stream.next().lineno
        body = parser.parse_statements([
         'name:endmarkdown'], drop_needle=True)
        return nodes.CallBlock(self.call_method('_markdown_support'), [], [], body).set_lineno(lineno)
