...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sipvicious/svcrack.py
# Compiled at: 2020-02-18 00:36:42
# Size of source mod 2**32: 25340 bytes
...
import base64, logging, dbm, random, select, socket, time, os, pickle, traceback
from sys import exit
from datetime import datetime
from optparse import OptionParser
from libs.pptable import to_string
from libs.svhelper import __version__, mysendto, reportBugToAuthor, numericbrute, dictionaryattack, packetcounter, check_ipv6, createTag, makeRequest, getAuthHeader, getNonce, getOpaque, getAlgorithm, getQop, getCID, getRealm, getCredentials, getRange, standardscanneroptions, standardoptions, calcloglevel, resumeFrom
__prog__ = 'svcrack'

class ASipOfRedWine:

    def __init__(self, host='localhost', bindingip='', localport=5060, port=5060, externalip=None, username=None, crackmode=1, crackargs=None, realm=None, sessionpath=None, selecttime=0.005, compact=False, reusenonce=False, extension=None, maxlastrecvtime=10, domain=None, requesturi=None, method='REGISTER', ipv6=False):
        ...
        if ipv6:
            ...
            if self.sessionpath is not None:
                self.resultpasswd = dbm.open(os.path.join(self.sessionpath, 'resultpasswd'), 'c')
                try:
    ...
    def getResponse(self):
        ...
        if buff.startswith(self.PROXYAUTHREQ):
            ...
            if buff.startswith(self.AUTHREQ):
            ...
            if buff.startswith(self.OKEY):
                ...
                if buff.startswith(self.NOTFOUND):
                    self.log.warn('User not found')
                    self.noauth = True
                else:
                    if buff.startswith(self.INVALIDPASS):
                        pass
                ...
                try:
                    ...
                    if self.sessionpath is not None:
                        if next(self.packetcount):
                            try:
                                if self.crackmode == 1:
                                    pickle.dump(self.previouspassword, open(os.path.join(self.sessionpath, 'lastpasswd.pkl'), 'wb+'))
                                    self.log.debug('logged last extension %s' % self.previouspassword)
                                else:
                                    if self.crackmode == 2:
                                        pickle.dump(self.crackargs.tell(), open(os.path.join(self.sessionpath, 'lastpasswd.pkl'), 'wb+'))
...
def main():
    ...
    parser.add_option('-u', '--username', dest='username', help='username to try crack',
      metavar='USERNAME')
    parser.add_option('-d', '--dictionary', dest='dictionary', type='string', help='specify a dictionary file with passwords',
      metavar='DICTIONARY')
    parser.add_option('-r', '--range', dest='range', default='100-999', help='specify a range of numbers. example: 100-200,300-310,400',
      metavar='RANGE')
    parser.add_option('-e', '--extension', dest='extension', help='Extension to crack. Only specify this when the extension is different from the username.',
      metavar='EXTENSION')
    parser.add_option('-z', '--zeropadding', dest='zeropadding', type='int', default=0, help='the number of zeros used to padd the password.\n                  the options "-r 1-9999 -z 4" would give 0001 0002 0003 ... 9999',
      metavar='PADDING')
    parser.add_option('-n', '--reusenonce', dest='reusenonce', default=False, help="Reuse nonce. Some SIP devices don't mind you reusing the nonce (making them vulnerable to replay attacks). Speeds up the cracking.",
      action='store_true')
    ...
    parser.add_option('--requesturi', dest='requesturi', help='force the first line URI to a specific value; e.g. sip:999@example.org')
    ...
    if options.resume is not None:
        ...
        if options.save is not None:
            ...
            if options.save is not None:
                if sipvicious.previouspassword is not None:
                    lastextensiondst = os.path.join(exportpath, 'lastpasswd.pkl')
                    logging.debug('saving state to %s' % lastextensiondst)
                    try:
                        if crackmode == 1:
                            pickle.dump(sipvicious.previouspassword, open(os.path.join(exportpath, 'lastpasswd.pkl'), 'wb+'))
                            logging.debug('logged last password %s' % sipvicious.previouspassword)
                        else:
                            if crackmode == 2:
                                pickle.dump(sipvicious.crackargs.tell(), open(os.path.join(exportpath, 'lastpasswd.pkl'), 'wb+'))
