...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sipvicious/svcrash.py
# Compiled at: 2020-02-26 03:08:06
# Size of source mod 2**32: 7171 bytes
...
import re, sys, time, logging, optparse, os.path, random, select, socket, warnings
try:
...
def getArgs():
    ...
    parser.add_option('--auto', help='Automatically send responses to attacks', dest='auto',
      default=False,
      action='store_true')
    parser.add_option('--astlog', help='Path for the asterisk full logfile', dest='astlog')
    parser.add_option('-d', help="specify attacker's ip address", dest='ipaddr')
    parser.add_option('-p', help="specify attacker's port", dest='port', type='int',
      default=5060)
    parser.add_option('-b', help="bruteforce the attacker's port", dest='bruteforceport', default=False,
      action='store_true')
    options, args = parser.parse_args()
    if not options.auto:
        options.astlog or options.ipaddr or parser.error('When auto or astlog is not specified, you need to pass an IP address')
    else:
        if options.auto:
            if scapyversion == 0:
                parser.error('You should have scapy installed for spoofing the packets: python3 -m pip install scapy.')
            else:
                if options.astlog:
                    if not os.path.exists(options.astlog):
                        parser.error('Could not read %s' % options.astlog)
        else:
            if not (scapyversion == 0 or options.auto):
                try:
...
class sniffnsend:
    ...
    def checknsend(self, pkt):
        ...
        try:
            ...
            src = (
             ipaddr, port)
            if src not in self.mytimer:
                self.mytimer[src] = time.time() - 2
            if time.time() - self.mytimer[src] > 2:
                if time.time() - self.lastsent > 0.5:
                    if ('User-Agent: friendly-scanner' in data or 'User-Agent: Asterisk PBX') in data:
                        if 'CSeq: 1 REGISTER' in data:
                            if 'REGISTER ' in data:
    ...
    def start(self):
        try:
            sniff(prn=(self.checknsend), filter=('udp port %s' % self.port),
              store=0)
        except KeyboardInterrupt:
            print('goodbye')
