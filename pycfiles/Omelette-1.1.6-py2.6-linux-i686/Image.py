...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/PIL/Image.py
...
class Image():
    ...
    def save(self, fp, format=None, **params):
        ...
        if isStringType(fp):
            filename = fp
        elif hasattr(fp, 'name') and isStringType(fp.name):
            filename = fp.name
        else:
            filename = ''
        self.load()
...
def fromarray(obj, mode=None):
    ...
    if mode is None:
        typestr = arr['typestr']
        if not (typestr[0] == '|' or typestr[0] == _ENDIAN or typestr[1:] not in ('u1',
                                                                                  'b1',
                                                                                  'i4',
                                                                                  'f4')):
