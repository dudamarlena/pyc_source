...
# [GCC 8.4.0]
# Embedded file name: /opt/work/lib/pyenv/versions/3.6.1/envs/maestro/lib/python3.6/site-packages/rest_framework/forms/models.py
...
from rest_framework.forms.fields import CharField, DateTimeField, IntegerField, BooleanField, FloatField, DateField, TimeField, UUIDField, ChoiceField
...
MODEL_FORM_FIELD_MAPPINGS = {models.CharField: CharField, 
...
POP_KWARGS = ('required', 'default', 'null', 'min_length', 'max_length', 'min_value',
              'max_value', 'validators')

def get_form_field(model_field, **kwargs):
    defaults = {'required':not model_field.default not in EMPTY_VALUES or model_field.null, 
    ...
    if getattr(model_field, 'unique', False):
        ...
        validator = UniqueValidator(queryset=(model_field.model_class.select()),
...
class ModelFormMetaclass(DeclarativeFieldsMetaclass):
    ...
    def __new__(mcs, name, bases, attrs):
        ...
        if bases == (BaseModelForm,):
            ...
            if form_opts.model:
                if form_opts.fields is None:
                    if form_opts.exclude is None:
                    ...
                    if isinstance(form_opts.fields, str):
                        if form_opts.fields.lower() == ALL_FIELDS:
                            form_opts.fields = None
                    model_fields = fields_for_model(model=(form_opts.model),
...
class BaseModelForm(BaseForm):
    ...
    def __init__(self, request=None, data=None, files=None, initial=None, empty_permitted=False, instance=None):
        opts = self._meta
        ...
        super(BaseModelForm, self).__init__(request=request,
          data=data,
          files=files,
          initial=object_data,
    ...
    def validators(self):
        ...
        field_names = {f.source for f in self.fields.values() if f.source != '*' if '.' not in f.source if '.' not in f.source}
        validators = []
        for unique_together in model_unique_together_list:
            if field_names.issuperset(unique_together):
                validator = UniqueTogetherValidator(queryset=(self.Meta.model.select()),
