...
# [GCC 8.4.0]
# Embedded file name: /opt/work/lib/pyenv/versions/3.6.1/envs/maestro/lib/python3.6/site-packages/rest_framework/core/views.py
# Compiled at: 2018-10-29 02:29:05
...
class RequestHandler(BaseRequestHandler, metaclass=HandlerMethodType):
    ...
    def _parse_query_arguments(self):
        ...
        return {k:v for k, v in query_arguments if v if v}
    ...
    async def prepare(self):
        ...
        if not content_type or method == b'get':
            ...
            return
        parser = self._RequestHandler__select_parser(content_type)
        if not parser:
            error_detail = _('Unsupported media type `%s` in request') % content_type
            raise APIException(detail=error_detail,
              code='MediaTypeError',
    ...
    async def dispatch_request(self, *args, **kwargs):
        ...
        try:
            ...
            try:
                ...
                error_content = {'message':_('Internal Server Error'), 
                 'code':'Error'}
    ...
    def _handle_request_exception(self, e):
        ...
        if isinstance(e, HTTPError):
            ...
            error_content = {'message':http_code_detail.description if http_code_detail else _('Internal Server Error'), 
             'code':'Error'}
            return self.write_error(error_content, status_code)
        else:
            error_content = {'message':traceback.format_exc(), 
             'code':'Error'}
...
class WebSocketHandler(BaseRequestHandler):
    ...
    async def prepare(self, message):
        ...
        if message_context:
            return json_decode(message_context)
        else:
            return {}
