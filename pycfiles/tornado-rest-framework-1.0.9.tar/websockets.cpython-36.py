...
# [GCC 8.4.0]
# Embedded file name: /opt/work/lib/pyenv/versions/maestro/lib/python3.6/site-packages/rest_framework/core/websockets.py
# Compiled at: 2018-10-21 03:00:37
# Size of source mod 2**32: 5517 bytes
import enum, typing
...
class WebSocket(Mapping):
    ...
    async def receive(self) -> Message:
        ...
        if self.client_state == WebSocketState.CONNECTED:
            message = await self._receive()
            message_type = message['type']
            assert message_type in frozenset({'websocket.disconnect', 'websocket.receive'})
    ...
    async def send(self, message: Message) -> None:
        if self.application_state == WebSocketState.CONNECTING:
            message_type = message['type']
            assert message_type in frozenset({'websocket.accept', 'websocket.close'})
            ...
            if self.application_state == WebSocketState.CONNECTED:
                message_type = message['type']
                assert message_type in frozenset({'websocket.send', 'websocket.close'})
                if message_type == 'websocket.close':
                    self.application_state = WebSocketState.DISCONNECTED
                await self._send(message)
            else:
