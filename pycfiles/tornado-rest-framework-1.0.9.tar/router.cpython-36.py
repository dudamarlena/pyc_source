...
# [GCC 8.4.0]
# Embedded file name: /opt/work/lib/pyenv/versions/maestro/lib/python3.6/site-packages/rest_framework/core/router.py
# Compiled at: 2018-10-21 03:53:27
# Size of source mod 2**32: 8453 bytes
...
class PatternParser:
    ...
    CAST = {str: lambda x: x.decode('utf-8'), 
     int: lambda x: int(x), 
    ...
    def validate_param_name(cls, name: bytes):
        if b':' in name:
            raise RouteConfigurationError('Special characters are not allowed in param name. Use type hints in function parameters to cast the variable or regexes with named groups to ensure only a specific URL matches.')
    ...
    def extract_params(cls, pattern: bytes) -> tuple:
        ...
        return (
         re.compile(new_pattern), params, simplified_pattern)
...
class Router:
    ...
    def __init__(self):
        ...
        self.cache = LRUCache(max_size=1048576)
    ...
    def add_route(self, route: 'Route', check_slashes: bool=True):
        ...
        if all(conditions):
            pattern = route.pattern[:-1] if route.pattern.endswith(b'/') else route.pattern + b'/'
    ...
    def get_route(self, request: Request or WebSocket) -> 'Route':
        try:
            ...
            logger.error(f"get route error, url:{request.url} method: {request.method}", exc_info=True)
...
class Route:
    ...
    def __init__(self, pattern: bytes, handler, methods=None, parent=None, app=None, dynamic=None, name: str=None):
    ...
    def clone(self, pattern=None, name=None, handler=None, methods=None, dynamic=None):
        return Route(pattern=(pattern or self.pattern), handler=(handler or self.handler), methods=(methods or self.methods),
          parent=(self.parent),
          app=(self.app),
