...
# [GCC 8.4.0]
# Embedded file name: /opt/work/lib/pyenv/versions/3.6.1/envs/maestro/lib/python3.6/site-packages/rest_framework/lib/orm/peewee.py
# Compiled at: 2018-10-12 04:41:52
# Size of source mod 2**32: 170177 bytes
import re, time, uuid, weakref, calendar, datetime, decimal, hashlib, itertools, logging, operator, threading
...
MYSQL_DATE_TRUNC_MAPPING = {'year':'%Y', 
 'month':'%Y-%m', 
 'day':'%Y-%m-%d', 
 'hour':'%Y-%m-%d %H', 
 'minute':'%Y-%m-%d %H:%i', 
...
class Node(object):
    __doc__ = 'Base-class for any part of a query which shall be composable.'
    c = _CDescriptor()
    _node_type = 'node'
...
class SQL(Node):
    __doc__ = 'An unescaped SQL string, with optional parameters.'
    _node_type = 'sql'
...
class Entity(Node):
    __doc__ = 'A quoted-name or entity, e.g. "table"."column".'
    _node_type = 'entity'
...
class Func(Node):
    __doc__ = 'An arbitrary SQL function call.'
    _node_type = 'func'
    ...
    def over(self, partition_by=None, order_by=None, start=None, end=None, window=None):
        if isinstance(partition_by, Window):
            if window is None:
                window = partition_by
            if start is not None:
                if not isinstance(start, SQL):
                    start = SQL(*start)
        else:
            if end is not None:
                if not isinstance(end, SQL):
                    end = SQL(*end)
            if window is None:
                sql = Window(partition_by=partition_by, order_by=order_by, start=start,
                  end=end).__sql__()
            else:
...
class Expression(Node):
    __doc__ = 'A binary expression, e.g `foo + 1` or `bar < 7`.'
...
class Clause(Node):
    __doc__ = 'A SQL clause, one or more Node objects joined by spaces.'
    _node_type = 'clause'
    glue = ' '
    parens = False
...
class CommaClause(Clause):
    __doc__ = 'One or more Node objects joined by commas, no parens.'
    glue = ', '


class EnclosedClause(CommaClause):
    __doc__ = 'One or more Node objects joined by commas and enclosed in parens.'
    parens = True
...
class Window(Node):
    ...
    def __sql__(self):
        ...
        if self.order_by:
            over_clauses.append(Clause(SQL('ORDER BY'), CommaClause(*self.order_by)))
        if self.start is not None:
            if self.end is not None:
                over_clauses.append(Clause(SQL('RANGE BETWEEN'), self.start, SQL('AND'), self.end))
        if self.start is not None:
...
class DQ(Node):
    __doc__ = 'A "django-style" filter expression, e.g. {\'foo__eq\': \'x\'}.'
...
JoinMetadata = namedtuple('JoinMetadata', ('src_model', 'dest_model', 'src', 'dest',
                                           'attr', 'primary_key', 'foreign_key',
                                           'is_backref', 'alias', 'is_self_join',
                                           'is_expression'))
...
class Join(namedtuple('_Join', ('src', 'dest', 'join_type', 'on'))):
    ...
    def model_from_alias(self, model_or_alias):
        if isinstance(model_or_alias, ModelAlias):
            ...
            if isinstance(model_or_alias, SelectQuery):
                return model_or_alias.model_class
            ...
            if fk_field is None:
                if self.on is not None:
                    ...
                    if fk_field is not None:
                        ...
                        primary_key = None
                    if join_alias or fk_field is not None:
                        if is_backref:
                ...
                target_attr = None
        return JoinMetadata(src_model=src,
          dest_model=dest,
          src=(self.src),
          dest=(self.dest),
...
class Field(Node):
    __doc__ = 'A column on a table.'
    _field_counter = 0
    _order = 0
    _node_type = 'field'
    db_field = 'unknown'

    def __init__(self, null=False, index=False, unique=False, verbose_name=None, help_text=None, db_column=None, default=None, choices=None, primary_key=False, sequence=None, constraints=None, schema=None, undeclared=False):
        self.null = null
        self.index = index
        self.unique = unique
    ...
    def clone_base(self, **kwargs):
        inst = (type(self))(null=self.null, 
         ...
         schema=self.schema, 
         undeclared=self.undeclared, **kwargs)
        if self._is_bound:
            inst.name = self.name
    ...
    def __ddl_column__(self, column_type):
        ...
        if modifiers:
            return SQL('%s(%s)' % (column_type, ', '.join(map(str, modifiers))))
        else:
    ...
    def __ddl__(self, column_type):
        ...
        ddl = [
         self.as_entity(), self.__ddl_column__(column_type)]
        if not self.null:
...
class _AutoPrimaryKeyField(PrimaryKeyField):
    ...
    def __init__(self, *args, **kwargs):
        if 'undeclared' in kwargs:
            if not kwargs['undeclared']:
...
class DecimalField(Field):
    ...
    def clone_base(self, **kwargs):
        return (super(DecimalField, self).clone_base)(max_digits=self.max_digits, 
         decimal_places=self.decimal_places, 
         auto_round=self.auto_round, 
         rounding=self.rounding, **kwargs)
    ...
    def db_value(self, value):
        ...
        if not value:
            ...
            if self.auto_round or not isinstance(value, D):
                value = D(str(value))
                if value.is_normal():
                    if self.auto_round:
...
class CharField(_StringField):
    ...
    def clone_base(self, **kwargs):
        return (super(CharField, self).clone_base)(max_length=self.max_length, **kwargs)
...
class _BaseFormattedField(Field):
    ...
    def clone_base(self, **kwargs):
        return (super(_BaseFormattedField, self).clone_base)(formats=self.formats, **kwargs)
...
class TimestampField(IntegerField):
    ...
    def get_db_field(self):
        if self.resolution == 1:
            ...
            return BigIntegerField.db_field
...
class RelationDescriptor(FieldDescriptor):
    __doc__ = 'Foreign-key abstraction to replace a related PK with a related model.'
    ...
    def __set__(self, instance, value):
        if isinstance(value, self.rel_model):
            ...
            instance._data[self.att_name] = value
        if orig_value != value:
            if self.att_name in instance._obj_cache:
                del instance._obj_cache[self.att_name]
        instance._dirty.add(self.att_name)


class ReverseRelationDescriptor(object):
    __doc__ = 'Back-reference to expose related objects as a `SelectQuery`.'
...
class ObjectIdDescriptor(object):
    __doc__ = 'Gives direct access to the underlying id'
...
class ForeignKeyField(IntegerField):
    ...
    def __init__(self, rel_model, related_name=None, on_delete=None, on_update=None, extra=None, to_field=None, object_id_name=None, *args, **kwargs):
        if rel_model != 'self':
            if not isinstance(rel_model, (Proxy, DeferredRelation)):
                if not issubclass(rel_model, Model):
                    raise TypeError('Unexpected value for `rel_model`.  Expected `Model`, `Proxy`, `DeferredRelation`, or "self"')
        self.rel_model = rel_model
    ...
    def clone_base(self, **kwargs):
        return (super(ForeignKeyField, self).clone_base)(rel_model=self.rel_model, 
         related_name=self._get_related_name(), 
         on_delete=self.on_delete, 
         on_update=self.on_update, 
         extra=self.extra, 
         to_field=self.to_field, 
         object_id_name=self.object_id_name, **kwargs)
    ...
    def _get_related_name(self):
        if self._related_name:
            if isinstance(self._related_name, Callable):
    ...
    def add_to_class(self, model_class, name):
        if isinstance(self.rel_model, Proxy):
            ...
            if not obj_id_name:
                ...
                raise ValueError('Cannot set a foreign key object_id_name to the same name as the field itself.')
            else:
                if not self.verbose_name:
                    self.verbose_name = re.sub('_+', ' ', name).title()
                model_class._meta.add_field(self)
            ...
            if model_class._meta.validate_backrefs:
                ...
                def invalid(msg, **context):
                    context.update(field=('%s.%s' % (model_class._meta.name, name)),
                ...
                if self.related_name in self.rel_model._meta.fields:
                    invalid('The related_name of %(field)s ("%(backref)s") conflicts with a field of the same name.')
                else:
                    if self.related_name in self.rel_model._meta.reverse_rel:
                        invalid('The related_name of %(field)s ("%(backref)s") is already in use by another foreign key.')
                if obj_id_name in model_class._meta.fields:
                    invalid('The object id descriptor of %(field)s conflicts with a field named %(obj_id_name)s')
                elif obj_id_name in model_class.__dict__:
                    invalid('Model attribute "%(obj_id_name)s" would be shadowed by the object id descriptor of %(field)s.')
        setattr(model_class, name, self._get_descriptor())
        setattr(model_class, obj_id_name, self._get_id_descriptor())
        setattr(self.rel_model, self.related_name, self._get_backref_descriptor())
        self._is_bound = True
...
class CompositeKey(object):
    __doc__ = 'A primary key composed of multiple columns.'
    ...
    def __get__(self, instance, instance_type=None):
        if instance is not None:
            return tuple([getattr(instance, field_name) for field_name in self.field_names])
        else:
            return self
    ...
    def __eq__(self, other):
        expressions = [self.model_class._meta.fields[field] == value for field, value in zip(self.field_names, other)]
...
class QueryCompiler(object):
    ...
    op_map = {OP.EQ: '=', 
     ...
     OP.REGEXP: 'REGEXP', 
     OP.CONCAT: '||'}
    join_map = {JOIN.INNER: 'INNER JOIN', 
    ...
    def get_parse_map(self):
        return {'expression':self._parse_expression, 
    ...
    def _parse_passthrough(self, node, alias_map, conv):
        if node.adapt:
            ...
            return (
             self.interpolation, [node.value])

    def _parse_param(self, node, alias_map, conv):
        if node.adapt:
            if conv:
                if conv.db_value is node.adapt:
                    conv = None
    ...
    def _parse_clause(self, node, alias_map, conv):
        sql, params = self.parse_node_list(node.nodes, alias_map, conv, node.glue)
        if node.parens:
    ...
    def _parse_entity(self, node, alias_map, conv):
        return (
         '.'.join(map(self.quote, node.path)), [])

    def _parse_sql(self, node, alias_map, conv):
        return (
         node.value, list(node.params))
    ...
    def _parse_compound_select_query(self, node, alias_map, conv):
        ...
        sql2, p2 = self.generate_select(rhs, self.calculate_alias_map(rhs, new_map))
    ...
    def _parse_select_query(self, node, alias_map, conv):
        ...
        if not node._explicit_selection:
            if conv:
                if isinstance(conv, ForeignKeyField):
                    clone._select = (
                     conv.to_field,)
            else:
    ...
    def _parse(self, node, alias_map, conv):
        ...
        if node_type in self._parse_map:
            ...
            unknown = node_type in self._unknown_types and node.adapt is None and conv is None
        else:
            if isinstance(node, (list, tuple, set)):
                ...
                if isinstance(node, Model):
                    ...
                    if isclass(node) and issubclass(node, Model) or isinstance(node, ModelAlias):
    ...
    def parse_node(self, node, alias_map=None, conv=None):
        ...
        if params:
            if any(isinstance(p, Node) for p in params):
                clean_params = []
                clean_sql = []
    ...
    def generate_select(self, query, alias_map=None):
        ...
        if query._windows is not None:
            ...
            clauses.append(CommaClause(*[Clause(SQL(window._alias), SQL('AS'), window.__sql__()) for window in query._windows]))
        if query._order_by:
    ...
    def generate_update(self, query):
        ...
        if query._on_conflict:
            ...
            statement = 'UPDATE'
        clauses = [
         SQL(statement), model.as_entity(), SQL('SET')]
        update = []
        for field, value in self._sorted_fields(query._update):
            ...
            update.append(Expression(field.as_entity(with_table=False),
              (OP.EQ),
              value,
              flat=True))
    ...
    def _get_field_clause(self, fields, clause_type=EnclosedClause):
        return clause_type(*[field.as_entity(with_table=False) for field in fields])
        ...
        if query._query is not None:
            ...
            if query._rows is not None:
                ...
                for row_dict in query._iter_rows():
                    if not have_fields:
                        fields = sorted((row_dict.keys()),
                          key=(operator.attrgetter('_sort_key')))
                        have_fields = True
                    values = []
                    for field in fields:
                ...
                if fields:
                    ...
                    clauses.append(query.database.default_insert_clause(query.model_class))
            if query._returning is not None:
                returning_clause = Clause(*query._returning)
                returning_clause.glue = ', '
                clauses.extend([SQL('RETURNING'), returning_clause])
                ...
                clauses.extend([
                 SQL('RETURNING'),
                 self._get_field_clause((meta.get_primary_key_fields()),
    ...
    def _create_foreign_key(self, model_class, field, constraint=None):
        ...
        return Clause(SQL('ALTER TABLE'), model_class.as_entity(), SQL('ADD CONSTRAINT'), Entity(constraint), *fk_clause.nodes)
    ...
    def _truncate_table(self, model_class, restart_identity=False, cascade=False):
        ddl = [
         SQL('TRUNCATE TABLE'), model_class.as_entity()]
        if restart_identity:
    ...
    def index_name(self, table, columns):
        ...
        if len(index) > 64:
            ...
            index = '%s_%s' % (table[:55], index_hash[:8])
        return index
    ...
    def _create_index(self, model_class, fields, unique, *extra):
        ...
        return Clause(SQL(statement), Entity(index_name), SQL('ON'), model_class.as_entity(), EnclosedClause(*[field.as_entity() for field in fields]), *extra)
...
class TuplesQueryResultWrapper(ExtQueryResultWrapper):
    ...
    def process_row(self, row):
        return tuple([col if self.conv[i][2] is None else self.conv[i][2](col) for i, col in enumerate(row)])
...
class NamedTupleQueryResultWrapper(ExtQueryResultWrapper):
    ...
    def process_row(self, row):
        return (self.constructor)(*[f(row[i]) if f is not None else row[i] for i, _, f in self.conv])
...
class ModelQueryResultWrapper(QueryResultWrapper):
    ...
    def initialize(self, description):
        ...
        self._col_set = set(col for col in self.column_meta if isinstance(col, Field))
                ...
                for join in joins[current]:
                    ...
                    if metadata.dest in models or metadata.dest_model in models:
                        if metadata.foreign_key is not None:
                            ...
                            check = metadata.foreign_key.null and (fk_present or pk_present)
                        else:
...
class AggregateQueryResultWrapper(ModelQueryResultWrapper):
    ...
    def initialize(self, description):
        ...
        for metadata, _, _, _ in self.join_list:
            ...
            self.source_to_dest[metadata.src][metadata.dest] = JoinCache(metadata=metadata,
...
class Query(Node):
    __doc__ = 'Base class representing a database query on one or more tables.'
    require_commit = True
    ...
    def _clone_joins(self):
        return dict((mc, list(j)) for mc, j in self._joins.items())
    ...
    def orwhere(self, *expressions):
        self._where = self._add_query_clauses(self._where, expressions, operator.or_)

    @returns_clone
    def join(self, dest, join_type=None, on=None):
        src = self._query_ctx
        if on is None:
            require_join_condition = join_type != JOIN.CROSS and (isinstance(dest, SelectQuery) or isclass(dest) and not src._meta.rel_exists(dest))
    ...
    def convert_dict_to_node(self, qdict):
        ...
        joins = []
        relationship = (
         ForeignKeyField, ReverseRelationDescriptor)
        for key, value in sorted(qdict.items()):
            curr = self.model_class
            if '__' in key:
                if key.rsplit('__', 1)[1] in DJANGO_MAP:
                    key, op = key.rsplit('__', 1)
                    op_group = DJANGO_MAP[op]
                    op, value = (op_group[0], op_group[1] % value) if len(op_group) == 2 else (op_group[0], value)
            if value is None:
                op = OP.IS
            else:
                op = OP.EQ
    ...
    def _filter_or_exclude(self, negate, *args, **kwargs):
        ...
        for field in dq_joins:
            if isinstance(field, ForeignKeyField):
                ...
                if isinstance(field, ReverseRelationDescriptor):
...
class SelectQuery(Query):
    ...
    def __init__(self, model_class, *selection):
        ...
        (self._SelectQuery__select)(*selection)
        self._from = None
        self._group_by = None
        self._having = None
        self._order_by = None
    ...
    def __xor__(self, rhs):
        wrapped_rhs = self.model_class.select(SQL('*')).from_(EnclosedClause(self & rhs).alias('_')).order_by()
    ...
    select = returns_clone(_SelectQuery__select)
    ...
    def for_update(self, for_update=True, nowait=False):
        self._for_update = 'FOR UPDATE NOWAIT' if (for_update and nowait) else ('FOR UPDATE' if for_update else None)
    ...
    def verify_naive(self):
        ...
        for node in self._select:
            if isinstance(node, Field):
                if node.model_class != model_class:
                    return False
                elif isinstance(node, Node):
                    if node._bind_to is not None:
    ...
    def requires_returning(method):
        ...
        def inner(self, *args, **kwargs):
            ...
            if not db.returning_clause:
                raise ValueError('RETURNING is not supported by your database: %s' % type(db))
...
class Database(object):
    ...
    exceptions = {'ConstraintError':IntegrityError, 
    ...
    def get_result_wrapper(self, wrapper_type):
        if wrapper_type == RESULTS_NAIVE:
            if self.use_speedups:
                return _ModelQueryResultWrapper
            ...
            if wrapper_type == RESULTS_MODELS:
                ...
                if wrapper_type == RESULTS_TUPLES:
                    if self.use_speedups:
                        return _TuplesQueryResultWrapper
                    else:
                        return TuplesQueryResultWrapper
                else:
                    if wrapper_type == RESULTS_DICTS:
                        if self.use_speedups:
                            return _DictQueryResultWrapper
                        else:
                            return DictQueryResultWrapper
                    if wrapper_type == RESULTS_NAMEDTUPLES:
    ...
    def create_foreign_key(self, model_class, field, constraint=None):
        qc = self.compiler()
        return (self.execute_sql)(*qc.create_foreign_key(model_class, field, constraint))
    ...
    def drop_table(self, model_class, fail_silently=False, cascade=False):
        qc = self.compiler()
        if cascade:
            if not self.drop_cascade:
                raise ValueError('Database does not support DROP TABLE..CASCADE.')
        return (self.execute_sql)(*qc.drop_table(model_class, fail_silently, cascade))
    ...
    def truncate_table(self, model_class, restart_identity=False, cascade=False):
        qc = self.compiler()
        return (self.execute_sql)(*qc.truncate_table(model_class, restart_identity, cascade))
...
class MySQLDatabase(Database):
    ...
    op_overrides = {OP.LIKE: 'LIKE BINARY', 
     OP.ILIKE: 'LIKE', 
     OP.XOR: 'XOR'}
    quote_char = '`'
    ...
    def get_columns(self, table, schema=None):
        ...
        return [ColumnMetadata(name, dt, null == 'YES', name in pks, table) for name, null, dt in cursor.fetchall()]
    ...
    def default_insert_clause(self, model_class):
        return Clause(EnclosedClause(model_class._meta.primary_key), SQL('VALUES (DEFAULT)'))
    ...
    def __enter__(self):
        with self.database._conn_lock:
            ...
            self.connection = (self.database._connect)(
             (self.database.database), **(self.database).connect_kwargs)
...
class ModelOptions(object):
    ...
    def __init__(self, cls, database=None, db_table=None, db_table_func=None, indexes=None, order_by=None, primary_key=None, table_alias=None, constraints=None, schema=None, validate_backrefs=True, only_save_dirty=False, depends_on=None, **kwargs):
    ...
    def _update_field_lists(self):
        ...
        self.valid_fields = set(self.fields.keys()) | set(self.fields.values()) | set((self.primary_key,))
        self.declared_fields = [field for field in self.sorted_fields if not field.undeclared]
    ...
    def get_primary_key_fields(self):
        if self.composite_key:
            return [self.fields[field_name] for field_name in self.primary_key.field_names]
        else:
            return [
             self.primary_key]
    ...
    def __new__(cls, name, bases, attrs):
        if name == _METACLASS_ or bases[0].__name__ == _METACLASS_:
                    ...
                    for k, v in b.__dict__.items():
                        if k in attrs:
                            ...
                            if isinstance(v, FieldDescriptor):
                                attrs[k] = v.field.primary_key or deepcopy(v.field)
            ...
            if model_pk is not False:
                ...
                cls._meta.auto_increment = isinstance(model_pk, PrimaryKeyField) or bool(model_pk.sequence)
...
class Model(with_metaclass(BaseModel)):
    ...
    @classmethod
    def update(cls, _Model__data=None, **update):
        fdict = _Model__data or {}
        fdict.update([(cls._meta.fields[f], update[f]) for f in update])
        return UpdateQuery(cls, fdict)

    @classmethod
    def insert(cls, _Model__data=None, **insert):
        fdict = _Model__data or {}
    ...
    @classmethod
    def _index_data(cls):
        return itertools.chain([((field,), field.unique) for field in cls._fields_to_index()], cls._meta.indexes or ())
    ...
    def sqlall(cls):
        ...
        pk = cls._meta.primary_key
        if cls._meta.database.sequences:
            if pk.sequence:
    ...
    def _populate_unsaved_relations(self, field_dict):
        for key in self._meta.rel:
            conditions = key in self._dirty and key in field_dict and field_dict[key] is None and self._obj_cache.get(key) is not None
            if conditions:
    ...
    def __eq__(self, other):
        return other.__class__ == self.__class__ and self._get_pk_value() is not None and other._get_pk_value() == self._get_pk_value()
    ...
    for i, subquery in enumerate(subqueries):
        ...
        for j in reversed(range(i + 1)):
            ...
            if rels:
                ...
                backrefs = last_model._meta.rel_for_model(subquery_model,
                  multi=True)
            if fks or backrefs:
                if target_model is last_model or target_model is None:
                    break

        if not (fks or backrefs):
            tgt_err = ' using %s' % target_model if target_model else ''
            raise AttributeError('Error: unable to find foreign key for query: %s%s' % (
             subquery, tgt_err))
        if fks:
            ...
            if backrefs:
                expr = reduce(operator.or_, [backref.to_field << last_query.select(backref) for backref in backrefs])
                subquery = subquery.where(expr)
...
__prefetched = namedtuple('__prefetched', ('query', 'fields', 'backref', 'rel_models',
                                           'field_to_name', 'model'))
...
class PrefetchResult(__prefetched):
    ...
    def __new__(cls, query, fields=None, backref=None, rel_models=None, field_to_name=None, model=None):
        ...
        model = query.model_class
        return super(PrefetchResult, cls).__new__(cls, query, fields, backref, rel_models, field_to_name, model)
