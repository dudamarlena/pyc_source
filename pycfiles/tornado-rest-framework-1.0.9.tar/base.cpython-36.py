...
# [GCC 8.4.0]
# Embedded file name: /opt/work/lib/pyenv/versions/3.6.1/envs/maestro/lib/python3.6/site-packages/rest_framework/core/script/base.py
# Compiled at: 2018-10-12 04:41:52
# Size of source mod 2**32: 9451 bytes
import os, re, sys, types, logging, argparse
...
def add_help(parser, help_args):
    if not help_args:
        return
    (parser.add_argument)(*help_args, action='help', 
...
class Manager(object):
    ...
    def handle(self, prog, args=None):
        ...
        if not func_stack:
            ...
            if remaining_args:
                if not getattr(last_func, 'capture_all_args', False):
                    app_parser.error('too many arguments')
            args = []
            for handle in func_stack:
                config_keys = [action.dest for action in handle.parser._actions if handle is last_func or action.__class__ in safe_actions]
                config = dict((k, v) for k, v in iteritems(kwargs) if k in config_keys)
                kwargs = dict((k, v) for k, v in iteritems(kwargs) if k not in config_keys)
                if handle is last_func:
                    if getattr(last_func, 'capture_all_args', False):
                        args.append(remaining_args)
                try:
                    res = handle(*args, **config)
                except TypeError as err:
                    err.args = (
                     '{}: {}'.format(handle, str(err)),)
                    raise
