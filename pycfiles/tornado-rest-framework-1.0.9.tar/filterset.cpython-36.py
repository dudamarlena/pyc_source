...
# [GCC 8.4.0]
# Embedded file name: /opt/work/lib/pyenv/versions/3.6.1/envs/maestro/lib/python3.6/site-packages/rest_framework/filters/filterset.py
...
class FilterSetMetaclass(type):
    ...
    def get_declared_filters(bases, attributes):
        ...
        :return:
        """
        filter_list = [(filter_name, attributes.pop(filter_name)) for filter_name, obj in list(attributes.items()) if isinstance(obj, filters.Filter)]
        ...
        for base in reversed(bases):
            if hasattr(base, 'declared_filters'):
                filter_list = [(name, f) for name, f in base.declared_filters.items() if name not in attributes] + filter_list

        return OrderedDict(filter_list)


FILTER_FOR_DBFIELD_DEFAULTS = {models.CharField: {'filter_class': filters.CharFilter}, 
...
class BaseFilterSet:
    ...
    def get_fields(cls):
        ...
        if fields is None:
            if exclude is None:
                raise ImproperlyConfigured("Creating a FilterSet without either the 'fields' attribute or the 'exclude' attribute is prohibited; filter %s needs updating." % cls.__name__)
    ...
    def get_filters(cls):
        ...
        if not model_class:
            ...
            if undefined:
                raise TypeError("'Meta.fields' contains fields that are not defined on this FilterSet: %s" % ', '.join(undefined))
    ...
    def filter_for_lookup(cls, field, lookup_expr):
        ...
        if not filter_class:
            ...
            if lookup_expr == 'exact':
                if field.choices:
                    return (
                     filters.ChoiceFilter, {'choices': field.choices})
            return (
             filter_class, params)
