...
# [GCC 8.4.0]
# Embedded file name: /opt/work/lib/pyenv/versions/3.6.1/envs/maestro/lib/python3.6/site-packages/rest_framework/filters/filters.py
# Compiled at: 2018-03-20 03:54:00
# Size of source mod 2**32: 9169 bytes
...
from rest_framework.filters.fields import Lookup, RangeField, DateRangeField, DateTimeRangeField, TimeRangeField
__all__ = [
 'Filter',
 'CharFilter',
 'BooleanFilter',
    ...
    def method(self):
        ...
        if callable(instance.method):
            ...
            if not hasattr(instance, 'parent'):
                raise AssertionError("Filter '%s' must have a parent FilterSet to find '.%s()'" % (
...
class Filter(object):
    ...
    def __init__(self, field_name=None, lookup_expr='exact', method=None, distinct=False, exclude=False, source=None, **kwargs):
        """
...
class RangeFilter(Filter):
    ...
    def filter(self, qs, value):
        ...
        if not value or value.start is None and value.stop is None:
            ...
            if field is None:
                error_msg = '字段{field_name}无法在queryset.model_class或queryset.join的model找到'.format(field_name=field_name)
                raise ImproperlyConfigured(error_msg)
            expressions = None
            if value.start is not None:
                if value.stop is not None:
                    expressions = [models.Expression(field, models.OP.BETWEEN, models.Clause(value.start, models.R('AND'), value.stop))]
            if value.start is not None:
                expressions = [
