...
# [GCC 8.4.0]
# Embedded file name: /Users/sam/Dropbox/Projects/videogrep/build/lib/videogrep/videogrep.py
# Compiled at: 2016-09-30 17:38:13
import os, re, random, gc, subprocess
from collections import OrderedDict
import pattern, searcher, audiogrep
...
def get_fps(filename):
    ...
    fps = re.findall('\\d+ fps', output, flags=re.MULTILINE)
    try:
...
def make_edl_segment(n, time_in, time_out, rec_in, rec_out, full_name, filename, fps=25):
    ...
    out = template.format(n, full_name, Timecode(fps, start_seconds=time_in), Timecode(fps, start_seconds=time_out), Timecode(fps, start_seconds=rec_in), Timecode(fps, start_seconds=rec_out), filename, full_name, reel)
    return out
        ...
        for s in audiogrep.convert_timestamps(files):
            for w in s['words']:
                if search_line(w[0], search, searchtype):
                    seg = {'file': s['file'].replace('.transcription.txt', ''), 'line': w[0], 
                       'start': float(w[1]), 
                       'end': float(w[2])}
                    final_segments.append(seg)
...
def videogrep(inputfile, outputfile, search, searchtype, maxclips=0, padding=0, test=False, randomize=False, sync=0, use_transcript=False):
    ...
    if len(composition) == 0:
        ...
        if test is True:
            demo_supercut(composition, padding)
        elif os.path.splitext(outputfile)[1].lower() == '.edl':
