...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.7/site-packages/smmc/__init__.py
# Compiled at: 2019-07-31 06:30:48
# Size of source mod 2**32: 4085 bytes
import json, base64, os.path

class SMMC(object):

    def __init__(self, message_limiter=[
 '---BEGIN-SCANNERMANAGER-MESSAGE---', '---END-SCANNERMANAGER-MESSAGE---']):
        """
    ...
    def send_status(self, status, details={}):
        ...
        self._SMMC__send_message(message)
    ...
    def __send_output(self, filename, encoding, payload, description=None):
        ...
        message = {'message_type':'output', 
         'encoding':encoding,  'filename':filename,  'payload':payload}
        if description:
    ...
    def send_output_string(self, filename, payload, description=None):
        ...
        self._SMMC__send_output(filename=filename, encoding='plaintext', payload=payload,
    ...
    def send_output_json(self, filename, payload, description=None):
        ...
        description [str]: description for the file if its not clear how to interpret the content
        """
        self._SMMC__send_output(filename=filename, encoding='json', payload=payload,
          description=description)
    ...
    def send_output_file(self, path_to_file, filename=None, description=None):
        ...
        self._SMMC__send_output(filename=filename, payload=payload, description=description,
          encoding='base64')
