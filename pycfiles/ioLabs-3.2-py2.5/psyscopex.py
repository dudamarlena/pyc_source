...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/psyscopex.py
...
try:
    from hid.osx import IOObjectRelease, find_usb_devices, COMObjectRef, IOCreatePlugInInterfaceForService, CFUUIDGetConstantUUIDWithBytes, IOCFPlugInInterfaceStruct, SInt32, UInt32, UInt8, kIOCFPlugInInterfaceID, IUNKNOWN_C_GUTS, CFUUIDGetUUIDBytes, io_iterator_t, IOIteratorNext, kIOReturnSuccess
    on_osx = True
except:
    on_osx = False
...
if on_osx:
    ...
    kIOReturnExclusiveAccess = iokit_common_err(709)
    kIOUSBFindInterfaceDontCare = 65535
    kIOUSBDeviceUserClientTypeID = CFUUIDGetConstantUUIDWithBytes(None, 157, 199, 183, 128, 158, 192, 17, 212, 165, 79, 0, 10, 39, 5, 40, 97)
    kIOUSBInterfaceUserClientTypeID = CFUUIDGetConstantUUIDWithBytes(None, 45, 151, 134, 198, 158, 243, 17, 212, 173, 81, 0, 10, 39, 5, 40, 97)
    kIOUSBInterfaceInterfaceID = CFUUIDGetConstantUUIDWithBytes(None, 115, 201, 122, 232, 158, 243, 17, 212, 177, 208, 0, 10, 39, 5, 40, 97)
    kIOUSBDeviceInterfaceID = CFUUIDGetConstantUUIDWithBytes(None, 92, 129, 135, 208, 158, 243, 17, 212, 139, 69, 0, 10, 39, 5, 40, 97)
    ...
    class IOUSBInterfaceInterface182(Structure):
        _fields_ = IUNKNOWN_C_GUTS + [
         ...
         parse('IOReturn (*ReadIsochPipeAsync)(void *self, UInt8 pipeRef, void *buf, UInt64 frameStart, UInt32 numFrames, IOUSBIsocFrame *frameList,IOAsyncCallback1 callback, void *refcon)').cstruct,
         parse('IOReturn (*WriteIsochPipeAsync)(void *self, UInt8 pipeRef, void *buf, UInt64 frameStart, UInt32 numFrames, IOUSBIsocFrame *frameList,IOAsyncCallback1 callback, void *refcon)').cstruct,
...
class PsyScopeXUSBDevice(HIDDevice):
    ...
    def open(self):
        ...
        score = SInt32()
        err = IOCreatePlugInInterfaceForService(self._usbDevice, kIOUSBDeviceUserClientTypeID, kIOCFPlugInInterfaceID, byref(plugInInterface.ref), byref(score))
    ...
    def _open_interface(self, interface):
        plugInInterface = COMObjectRef(POINTER(POINTER(IOCFPlugInInterfaceStruct))())
        score = SInt32()
        err = IOCreatePlugInInterfaceForService(interface, kIOUSBInterfaceUserClientTypeID, kIOCFPlugInInterfaceID, byref(plugInInterface.ref), byref(score))
