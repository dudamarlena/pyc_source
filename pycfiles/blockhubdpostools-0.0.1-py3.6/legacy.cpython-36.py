...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/legacy.py
# Compiled at: 2017-12-21 18:46:07
# Size of source mod 2**32: 34806 bytes
...
import binascii, datetime, logging, requests.exceptions as rq
if c.LOGGING['USE']:
...
def set_sender(default_share=0, cover_fees=False, share_percentage_exceptions=None, timestamp_brackets=None, min_payout_daily=0, min_payout_weekly=0, min_payout_monthly=0, day_weekly_payout=5, day_monthly_payout=10, payoutsender_test=True, sender_exception=None, wait_time_day=0, wait_time_week=0, wait_time_month=0, smartbridge=''):
...
class Address:
    ...
    def payout(address):
        ...
        Transaction = namedtuple('transaction', 'id amount timestamp recipientId senderId rawasset type fee')
        named_transactions = []
        for i in qry:
    ...
    def votes(address):
        ...
        Vote = namedtuple('vote', 'direction delegate timestamp')
        res = []
        for i in qry:
            if i[0][0] == '+':
                direction = True
    ...
    def balance_over_time(address):
        ...
        balance = 0
        block = 0
        Balance = namedtuple('balance', 'timestamp amount')
        for tx in txhistory:
            if forged_blocks:
...
class Delegate:
    ...
    def delegates():
        ...
        Delegate = namedtuple('delegate', 'username pubkey timestamp address transactionId')
        res = []
        for i in qry:
            registration = Delegate(username=(i[0]),
    ...
    def lastpayout(delegate_address, blacklist=None):
        ...
        result = []
        Payout = namedtuple('payout', 'address id timestamp')
        for i in qry:
            payout = Payout(address=(i[0]),
              id=(i[1]),
              timestamp=(i[2]))
    ...
    def votes(delegate_pubkey):
        ...
        Voter = namedtuple('voter', 'address timestamp')
        voters = []
        for i in qry:
            voter = Voter(address=(i[0]),
              timestamp=(i[1]))
            voters.append(voter)
    ...
    def blocks(delegate_pubkey=None, max_timestamp=None):
        ...
        if not delegate_pubkey:
            ...
            if max_timestamp:
                ...
                max_timestamp_sql = ''
        qry = DbCursor().execute_and_fetchall('\n             SELECT blocks."timestamp", blocks."height", blocks."id", blocks."totalFee", blocks."reward"\n             FROM blocks\n             WHERE {0} blocks."generatorPublicKey" = \'\\x{1}\'\n             ORDER BY blocks."timestamp" \n             ASC'.format(max_timestamp_sql, delegate_pubkey))
        Block = namedtuple('block', 'timestamp height id totalFee reward')
        block_list = []
        for block in qry:
            block_value = Block(timestamp=(block[0]), height=(block[1]),
              id=(block[2]),
              totalFee=(block[3]),
              reward=(block[4]))
    ...
    def share(passphrase=None, last_payout=None, start_block=0, del_pubkey=None, del_address=None):
        ...
        if del_pubkey:
            ...
            transactions = get_transactionlist(delegate_pubkey=delegate_pubkey)
            votes = Delegate.votes(delegate_pubkey)
            voter_dict = {}
            for voter in votes:
            ...
            last_payout = last_payout or Delegate.lastpayout(delegate_address)
        ...
        blocks = Delegate.blocks(max_timestamp=max_timestamp, delegate_pubkey=delegate_pubkey)
            ...
            for tx in transactions:
                while tx.timestamp > blocks[block_nr].timestamp:
                    ...
                    for i in voter_dict:
                        ...
                        if voter_dict[i]['status']:
                            if voter_dict[i]['last_payout'] < blocks[block_nr].timestamp:
                                if c.CALCULATION_SETTINGS['SHARE_FEES']:
                                    share = balance / poolbalance * (blocks[block_nr].reward + blocks[block_nr].totalFee)
                                else:
        ...
        for i in voter_dict:
            logger.info('{0}  {1}  {2}  {3}  {4}'.format(i, voter_dict[i]['share'], voter_dict[i]['status'], voter_dict[i]['last_payout'], voter_dict[i]['vote_timestamp']))

        return (
         voter_dict, max_timestamp)
