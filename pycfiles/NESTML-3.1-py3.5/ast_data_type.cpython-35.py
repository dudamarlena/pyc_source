...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pynestml/meta_model/ast_data_type.py
...
class ASTDataType(ASTNode):
    ...
    def __init__(self, is_integer=False, is_real=False, is_string=False, is_boolean=False, is_void=False, unit_type=None, source_position=None):
        """
    ...
    def set_type_symbol(self, type_symbol):
        ...
        assert type_symbol is not None and isinstance(type_symbol, TypeSymbol), '(PyNestML.AST.DataType) No or wrong type of type symbol provided (%s)!' % type(type_symbol)
    ...
    def equals(self, other):
        ...
        if not isinstance(other, ASTDataType):
            return False
        if not (self.is_integer == other.is_integer and self.is_real == other.is_real and self.is_string == other.is_string and self.is_boolean == other.is_boolean and self.is_void == other.is_void):
            return False
