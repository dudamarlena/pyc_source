...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pynestml/utils/error_strings.py
# Compiled at: 2020-03-05 05:49:41
# Size of source mod 2**32: 17565 bytes
...
class ErrorStrings(object):
    ...
    def message_implicit_magnitude_conversion(cls, origin, parent_node):
        ...
        assert parent_node is not None and (isinstance(parent_node, ASTExpression) or isinstance(parent_node, ASTAssignment))
        target_expression = None
        target_unit = None
        convertee_expression = None
        convertee_unit = None
        operation = None
        if isinstance(parent_node, ASTExpression) and parent_node.get_binary_operator() is not None:
        ...
        if isinstance(parent_node, ASTAssignment):
            lhs_variable_symbol = parent_node.get_scope().resolve_to_symbol(parent_node.get_variable().get_complete_name(), SymbolKind.VARIABLE)
            operation = '='
            ...
            convertee_unit = convertee_expression.type.astropy_unit
        assert target_expression is not None and convertee_expression is not None and operation is not None, 'Only call this on an addition/subtraction  or assignment after an implicit conversion wrt unit magnitudes has already been determined'
        error_msg_format = "Non-matching unit types at '" + str(parent_node)
        error_msg_format += "'. Implicit conversion of rhs to lhs"
        error_msg_format += ' (units: ' + str(convertee_unit) + ' and ' + str(target_unit) + ' )'
        error_msg_format += " implicitly replaced by '" + str(target_expression) + operation + convertee_expression.printImplicitVersion() + "'"
        return cls.code(origin) + cls.SEPARATOR + error_msg_format + '(' + str(parent_node.get_source_position()) + ')'
    ...
    @classmethod
    def message_add_sub_type_mismatch(cls, origin, lhs_type_text, rhs_type_text, result_type_text, source_position):
        """
        ...
        :rtype: str
        """
        error_msg_format = 'Addition/subtraction of ' + lhs_type_text + ' and ' + rhs_type_text + '. Assuming: ' + result_type_text + '.'
        return cls.code(origin) + cls.SEPARATOR + error_msg_format + '(' + str(source_position) + ')'
    ...
    def message_ternary_mismatch(cls, origin, if_true_text, if_not_text, source_position):
        ...
        :rtype: str
        """
        error_msg_format = 'Mismatched conditional alternatives ' + if_true_text + ' and ' + if_not_text + '-> Assuming real.'
        return cls.code(origin) + cls.SEPARATOR + error_msg_format + '(' + str(source_position) + ')'
