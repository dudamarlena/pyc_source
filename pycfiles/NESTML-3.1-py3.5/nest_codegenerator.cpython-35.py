...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pynestml/codegeneration/nest_codegenerator.py
# Compiled at: 2020-03-05 05:49:41
# Size of source mod 2**32: 25961 bytes
...
from pynestml.solver.solution_transformers import integrate_exact_solution, functional_shapes_to_odes, integrate_delta_solution
...
class NESTCodeGenerator(CodeGenerator):
    _variable_matching_template = '(\\b)({})(\\b)'
    ...
    def generate_module_code(self, neurons):
        ...
        with open(str(os.path.join(FrontendConfiguration.get_target_path(), FrontendConfiguration.get_module_name())) + '.h', 'w+') as (f):
            f.write(str(self._template_module_header.render(namespace)))
        with open(str(os.path.join(FrontendConfiguration.get_target_path(), FrontendConfiguration.get_module_name())) + '.cpp', 'w+') as (f):
            f.write(str(self._template_module_class.render(namespace)))
        with open(str(os.path.join(FrontendConfiguration.get_target_path(), 'CMakeLists')) + '.txt', 'w+') as (f):
        ...
        with open(str(os.path.join(FrontendConfiguration.get_target_path(), 'sli', FrontendConfiguration.get_module_name() + '-init')) + '.sli', 'w+') as (f):
    ...
    def define_solver_type(self, neuron, namespace):
        ...
        if neuron.get_equations_block() is not None and len(neuron.get_equations_block().get_declarations()) > 0 and (not self.is_functional_shape_present(neuron.get_equations_block().get_ode_shapes()) or len(neuron.get_equations_block().get_ode_equations()) > 1):
    ...
    def transform_shapes_and_odes(self, neuron, shape_to_buffers):
        ...
        if len(equations_block.get_ode_shapes()) == 0:
            ...
            Logger.log_message(neuron, code, message, neuron.get_source_position(), LoggingLevel.INFO)
            return neuron
        if len(equations_block.get_ode_shapes()) == 1 and str(equations_block.get_ode_shapes()[0].get_expression()).strip().startswith('delta'):
            shape = equations_block.get_ode_shapes()[0]
            integrate_delta_solution(equations_block, neuron, shape, shape_to_buffers)
            return neuron
        else:
            if len(equations_block.get_ode_equations()) == 1:
                ...
                if solver_result['solver'] is 'numeric' and self.is_functional_shape_present(equations_block.get_ode_shapes()):
    ...
    def apply_spikes_from_buffers(self, neuron, shape_to_buffers):
        ...
        for declaration in initial_values.get_declarations():
            ...
            for shape in shape_to_buffers:
                matcher_computed_shape_odes = re.compile(shape + '(__d)*')
                if re.fullmatch(matcher_computed_shape_odes, str(variable)):
                    buffer_type = neuron.get_scope().resolve_to_symbol(shape_to_buffers[shape], SymbolKind.VARIABLE).get_type_symbol()
                    assignment_string = variable.get_complete_name() + ' += (' + shape_to_buffers[shape] + '/' + buffer_type.print_nestml_type() + ') * ' + self._printer.print_expression(declaration.get_expression())
    ...
    def transform_ode_and_shapes_to_json(self, equations_block):
        ...
        for shape in equations_block.get_ode_shapes():
            if shape.get_variable().get_differential_order() == 0:
                ...
                if extracted_shape_name not in ode_shape_names:
        ...
        for shape_name in ode_shape_names:
            ...
            while True:
                ...
                if shape_name_symbol is not None:
                    shape_name_to_initial_values[shape_name].append(self._printer.print_expression(shape_name_symbol.get_declaring_expression()))
                    shape_name_to_shape_definition[shape_name] = self._printer.print_expression(shape_name_symbol.get_ode_definition())
                else:
                    break
                order = order + 1
    ...
    def store_transformed_model(self, ast):
        if FrontendConfiguration.store_log:
            with open(str(os.path.join(FrontendConfiguration.get_target_path(), '..', 'report', ast.get_name())) + '.txt', 'w+') as (f):
                f.write(str(ast))