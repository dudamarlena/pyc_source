...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pynestml/visitors/ast_symbol_table_visitor.py
# Compiled at: 2020-03-05 05:49:41
# Size of source mod 2**32: 37670 bytes
...
class ASTSymbolTableVisitor(ASTVisitor):
    ...
    def visit_neuron(self, node):
        ...
        Logger.log_message(neuron=node, code=code, error_position=node.get_source_position(), message=message, log_level=LoggingLevel.INFO)
    ...
    def endvisit_neuron(self, node):
        ...
        if node.get_input_blocks() is not None and node.get_equations_blocks() is not None and len(node.get_equations_blocks().get_declarations()) > 0:
    ...
    def visit_function(self, node):
        ...
        symbol = FunctionSymbol(scope=node.get_scope(), element_reference=node, param_types=list(), name=node.get_name(), is_predefined=False, return_type=None)
        self.symbol_stack.push(symbol)
        symbol.set_comment(node.get_comment())
        node.get_scope().add_symbol(symbol)
        scope = Scope(scope_type=ScopeType.FUNCTION, enclosing_scope=node.get_scope(), source_position=node.get_source_position())
    ...
    def endvisit_function(self, node):
        ...
        for arg in node.get_parameters():
            ...
            var_symbol = VariableSymbol(element_reference=arg, scope=scope, name=arg.get_name(), block_type=BlockType.LOCAL, is_predefined=False, is_function=False, is_recordable=False, type_symbol=PredefinedTypes.get_type(type_name), variable_type=VariableType.VARIABLE)
        ...
        node.get_scope().delete_scope(scope)
    ...
    def visit_update_block(self, node):
        ...
        """
        self.block_type_stack.push(BlockType.LOCAL)
        scope = Scope(scope_type=ScopeType.UPDATE, enclosing_scope=node.get_scope(), source_position=node.get_source_position())
        node.get_scope().add_scope(scope)
        node.get_block().update_scope(scope)
    ...
    def visit_declaration(self, node):
        ...
        is_recordable = node.is_recordable or self.block_type_stack.top() == BlockType.STATE or self.block_type_stack.top() == BlockType.INITIAL_VALUES
        ...
        for var in node.get_variables():
            ...
            symbol = VariableSymbol(element_reference=node, scope=node.get_scope(), name=var.get_complete_name(), block_type=self.block_type_stack.top(), declaring_expression=expression, is_predefined=False, is_function=node.is_function, is_recordable=is_recordable, type_symbol=type_symbol, initial_value=init_value, vector_parameter=vector_parameter, variable_type=VariableType.VARIABLE)
            symbol.set_comment(node.get_comment())
            node.get_scope().add_symbol(symbol)
    ...
    def visit_unit_type(self, node):
        ...
        if node.is_pow:
            ...
            if node.is_encapsulated:
                ...
                if isinstance(node.lhs, ASTUnitType):
    ...
    def visit_ode_function(self, node):
        ...
        symbol = VariableSymbol(element_reference=node, scope=node.get_scope(), name=node.get_variable_name(), block_type=BlockType.EQUATION, declaring_expression=node.get_expression(), is_predefined=False, is_function=True, is_recordable=node.is_recordable, type_symbol=type_symbol, variable_type=VariableType.VARIABLE)
        symbol.set_comment(node.get_comment())
        node.get_scope().add_symbol(symbol)
        node.get_data_type().update_scope(node.get_scope())
        node.get_expression().update_scope(node.get_scope())
    ...
    def visit_ode_shape(self, node):
        ...
        """
        if node.get_variable().get_differential_order() == 0 and node.get_scope().resolve_to_symbol(node.get_variable().get_complete_name(), SymbolKind.VARIABLE) is None:
            symbol = VariableSymbol(element_reference=node, scope=node.get_scope(), name=node.get_variable().get_name(), block_type=BlockType.EQUATION, declaring_expression=node.get_expression(), is_predefined=False, is_function=False, is_recordable=True, type_symbol=PredefinedTypes.get_real_type(), variable_type=VariableType.SHAPE)
            symbol.set_comment(node.get_comment())
            node.get_scope().add_symbol(symbol)
        node.get_variable().update_scope(node.get_scope())
        node.get_expression().update_scope(node.get_scope())
    ...
    def visit_block_with_variables(self, node):
        ...
        """
        self.block_type_stack.push(BlockType.STATE if node.is_state else BlockType.INTERNALS if node.is_internals else BlockType.PARAMETERS if node.is_parameters else BlockType.INITIAL_VALUES)
    ...
    def endvisit_input_port(self, node):
        ...
        type_symbol.is_buffer = True
        symbol = VariableSymbol(element_reference=node, scope=node.get_scope(), name=node.get_name(), block_type=buffer_type, vector_parameter=node.get_index_parameter(), is_predefined=False, is_function=False, is_recordable=False, type_symbol=type_symbol, variable_type=VariableType.BUFFER)
        symbol.set_comment(node.get_comment())
        node.get_scope().add_symbol(symbol)
...
def make_trivial_assignment(var, order, equations_block, is_shape=False):
    ...
    lhs_variable = ASTNodeFactory.create_ast_variable(name=var.get_name(), differential_order=order + 1, source_position=ASTSourceLocation.get_added_source_position())
    rhs_variable = ASTNodeFactory.create_ast_variable(name=convert_variable_name_to_generator_notation(var).get_name(), differential_order=order, source_position=ASTSourceLocation.get_added_source_position())
    expression = ASTNodeFactory.create_ast_simple_expression(variable=rhs_variable, source_position=ASTSourceLocation.get_added_source_position())
            ...
            if isinstance(declaration, ASTOdeShape) and declaration.get_variable().get_differential_order() > 0:
                ...
                for i in range(1, order):
                    ...
                    for eq in equations_block.get_ode_shapes():
                        if eq.get_variable().get_name() == declaration.get_variable().get_name() and eq.get_variable().get_differential_order() == i:
                            found = True
                            break

                    if not found:
                        checked.append(make_trivial_assignment(declaration.get_variable(), i, equations_block, True))
                ...
                if isinstance(declaration, ASTOdeEquation):
                    ...
                    for i in range(1, order):
                        ...
                        for ode in equations_block.get_ode_equations():
                            if ode.get_lhs().get_name() == declaration.get_lhs().get_name() and ode.get_lhs().get_differential_order() == i:
                                found = True
                                break
...
def mark_conductance_based_buffers(input_ports):
    ...
    for bufferDeclaration in input_ports:
        if bufferDeclaration.is_spike():
            symbol = bufferDeclaration.get_scope().resolve_to_symbol(bufferDeclaration.get_name(), SymbolKind.VARIABLE)
...
def add_ode_to_variable(ode_equation):
    ...
    existing_symbol = ode_equation.get_scope().resolve_to_symbol(ode_equation.get_lhs().get_name() + "'" * diff_order, SymbolKind.VARIABLE)
    if existing_symbol is not None:
        ...
        Logger.log_message(error_position=existing_symbol.get_referenced_object().get_source_position(), code=code, message=message, log_level=LoggingLevel.INFO)
    else:
        code, message = Messages.get_no_variable_found(ode_equation.get_lhs().get_name_of_lhs())
        Logger.log_message(code=code, message=message, error_position=ode_equation.get_source_position(), log_level=LoggingLevel.ERROR)
...
def add_ode_shape_to_variable(ode_shape):
    ...
    if ode_shape.get_variable().get_differential_order() == 0:
        return
    existing_symbol = ode_shape.get_scope().resolve_to_symbol(ode_shape.get_variable().get_name_of_lhs(), SymbolKind.VARIABLE)
    if existing_symbol is not None:
        ...
        Logger.log_message(code=code, message=message, error_position=ode_shape.get_source_position(), log_level=LoggingLevel.ERROR)
