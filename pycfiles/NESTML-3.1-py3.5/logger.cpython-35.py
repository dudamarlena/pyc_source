...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pynestml/utils/logger.py
...
class Logger(object):
    ...
    def log_message(cls, neuron=None, code=None, message=None, error_position=None, log_level=None):
        ...
        if not neuron is None:
            assert isinstance(neuron, ASTNeuron), '(PyNestML.Logger) Wrong type of neuron provided (%s)!' % type(neuron)
            if not error_position is None:
                assert isinstance(error_position, ASTSourceLocation), '(PyNestML.Logger) Wrong type of error position provided (%s)!' % type(error_position)
                if isinstance(neuron, ASTNeuron):
                    cls.log[cls.curr_message] = (neuron.get_artifact_name(), neuron, log_level, code, error_position, message)
        elif cls.current_neuron is not None:
            cls.log[cls.curr_message] = (
             cls.current_neuron.get_artifact_name(), cls.current_neuron,
        ...
        if cls.logging_level.value <= log_level.value:
            ...
            to_print = to_print + (neuron.get_name() + ', ' if neuron is not None else cls.current_neuron.get_name() + ', ' if cls.current_neuron is not None else 'GLOBAL, ')
    ...
    def get_all_messages_of_level_and_or_neuron(cls, neuron, level):
        ...
        for artifactName, neuron_i, logLevel, code, errorPosition, message in cls.log.values():
            if (level == logLevel if level is not None else True) and (neuron if neuron is not None else True) and (neuron.get_artifact_name() == artifactName if neuron is not None else True):
    ...
    def get_all_messages_of_neuron(cls, neuron):
        ...
        ret = list()
        for artifactName, neuron_i, logLevel, code, errorPosition, message in cls.log.values():
            if (neuron_i == neuron if neuron is not None else True) and (neuron.get_artifact_name() == artifactName if neuron is not None else True):
                ret.append((neuron, logLevel, message))
    ...
    def get_json_format(cls):
        ...
        for messageNr in cls.log.keys():
            ...
            ret += '{"filename":"' + artifactName + '", ' + '"neuronName":"' + (neuron.get_name() if neuron is not None else 'GLOBAL') + '", ' + '"severity":"' + str(logLevel.name) + '", '
            if code is not None:
                ret += '"code":"' + code.name + '", '
            ret += '"row":"' + (str(errorPosition.get_start_line()) if errorPosition is not None else '') + '", ' + '"col":"' + (str(errorPosition.get_start_column()) if errorPosition is not None else '') + '", ' + '"message":"' + str(message).replace('"', "'") + '"}'
            ret += ','
