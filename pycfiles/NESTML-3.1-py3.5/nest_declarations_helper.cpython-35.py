...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pynestml/codegeneration/nest_declarations_helper.py
...
class NestDeclarationsHelper(object):
    ...
    def get_variables(cls, ast_declaration):
        ...
        """
        assert isinstance(ast_declaration, ASTDeclaration), '(PyNestML.CodeGeneration.DeclarationsHelper) No or wrong type of declaration provided (%s)!' % type(ast_declaration)
        ret = list()
        for var in ast_declaration.get_variables():
            ...
            if symbol is not None:
                ...
                Logger.log_message(code=code, message=message, error_position=ast_declaration.get_source_position(), log_level=LoggingLevel.ERROR)
            return ret
