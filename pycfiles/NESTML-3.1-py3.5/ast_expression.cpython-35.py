...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pynestml/meta_model/ast_expression.py
...
class ASTExpression(ASTExpressionNode):
    ...
    def __init__(self, is_encapsulated=False, unary_operator=None, is_logical_not=False, expression=None, lhs=None, binary_operator=None, rhs=None, condition=None, if_true=None, if_not=None, source_position=None):
        """
        ...
        """
        if not binary_operator is None:
            if not isinstance(binary_operator, ASTArithmeticOperator):
                if not isinstance(binary_operator, ASTBitOperator):
                    if not isinstance(binary_operator, ASTLogicalOperator):
                        assert isinstance(binary_operator, ASTComparisonOperator), '(PyNestML.AST.Expression) Wrong type of binary operator provided (%s)!' % type(binary_operator)
    ...
    def equals(self, other):
        ...
        if self.is_unary_operator() + other.is_unary_operator() == 1:
            return False
        if self.is_unary_operator() and other.is_unary_operator() and not self.get_unary_operator().equals(other.get_unary_operator()):
            return False
        ...
        if self.is_compound_expression() + other.is_compound_expression() == 1:
            return False
        if self.is_compound_expression() and other.is_compound_expression() and not (self.get_lhs().equals(other.get_lhs()) and self.get_rhs().equals(other.get_rhs()) and self.get_binary_operator().equals(other.get_binary_operator())):
            return False
        if self.is_ternary_operator() + other.is_ternary_operator() == 1:
            return False
        if self.is_ternary_operator() and other.is_ternary_operator() and not (self.get_condition().equals(other.get_condition()) and self.get_if_true().equals(other.get_if_true()) and self.get_if_not().equals(other.get_if_not())):
            return False
        return True