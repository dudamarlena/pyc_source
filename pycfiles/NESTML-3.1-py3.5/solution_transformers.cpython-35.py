...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pynestml/solver/solution_transformers.py
# Compiled at: 2020-03-05 05:49:41
# Size of source mod 2**32: 9063 bytes
import re
from sympy import simplify, diff, Symbol, exp
from sympy.parsing.sympy_parser import parse_expr
from pynestml.meta_model.ast_neuron import ASTNeuron
from pynestml.meta_model.ast_ode_shape import ASTOdeShape
from pynestml.solver.transformer_base import add_declarations_to_internals, compute_state_shape_variables_declarations, add_declarations_to_initial_values, compute_state_shape_variables_updates, replace_integrate_call, add_state_updates
...
def integrate_delta_solution(equations_block, neuron, shape, shape_to_buffers):
    ...
    const_input = simplify(1 / diff(ode_rhs, parse_expr(shape_name)) * (ode_rhs - diff(ode_rhs, ode_lhs) * ode_lhs) - parse_expr(shape_name))
...
def functional_shapes_to_odes(neuron, transformed_shapes):
    ...
    """
    shape_names, shape_name_to_initial_values, shape_name_to_shape_state_variables, shape_state_variable_to_ode = _convert_to_shape_specific(transformed_shapes)
    shapes_to_delete = []
    for declaration in neuron.get_equations_block().get_declarations():
        if isinstance(declaration, ASTOdeShape) and declaration.get_variable().get_name() in shape_names:
    ...
    for shape_name in shape_names:
        for variable, initial_value in zip(shape_name_to_shape_state_variables[shape_name], shape_name_to_initial_values[shape_name]):
            state_shape_variables_declarations[variable] = initial_value

        for variable, shape_state_ode in zip(shape_name_to_shape_state_variables[shape_name], shape_state_variable_to_ode[shape_name]):
...
def _convert_to_shape_specific(transformed_shapes):
    ...
    for shape_name in shape_names:
        matcher_shape = re.compile(shape_name + '(__\\d+)?')
        initial_values = []
        shape_state_variables = []
        shape_state_odes = []
        while working_index_variables < len(transformed_shapes['shape_state_variables']) and matcher_shape.match(transformed_shapes['shape_state_variables'][working_index_variables]):
            initial_values.append(transformed_shapes['shape_initial_values'][working_index_variables])
            shape_state_variables = [
             transformed_shapes['shape_state_variables'][working_index_variables]] + shape_state_variables
