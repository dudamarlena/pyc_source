...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pynestml/codegeneration/nest_printer.py
# Compiled at: 2020-03-05 05:49:41
# Size of source mod 2**32: 14938 bytes
...
class NestPrinter(object):
    ...
    def print_step(cls, for_stmt):
        ...
        :rtype: str
        """
        assert isinstance(for_stmt, ASTForStmt), '(PyNestML.CodeGenerator.Printer) No or wrong type of for-stmt provided (%s)!' % type(for_stmt)
    ...
    def print_origin(cls, variable_symbol):
        ...
        :rtype: str
        """
        assert isinstance(variable_symbol, VariableSymbol), '(PyNestML.CodeGenerator.Printer) No or wrong type of variable symbol provided (%s)!' % type(variable_symbol)
    ...
    def print_output_event(cls, ast_body):
        ...
        :rtype: str
        """
        assert ast_body is not None and isinstance(ast_body, ASTBody), '(PyNestML.CodeGeneration.Printer) No or wrong type of body provided (%s)!' % type(ast_body)
    ...
    def print_function_declaration(cls, ast_function):
        ...
        from pynestml.meta_model.ast_function import ASTFunction
        from pynestml.symbols.symbol import SymbolKind
        assert ast_function is not None and isinstance(ast_function, ASTFunction), '(PyNestML.CodeGeneration.Printer) No or wrong type of ast_function provided (%s)!' % type(ast_function)
    ...
    def print_function_definition(cls, ast_function, namespace):
        ...
        :rtype: str
        """
        assert isinstance(ast_function, ASTFunction), '(PyNestML.CodeGeneration.Printer) No or wrong type of ast_function provided (%s)!' % type(ast_function)
        assert isinstance(namespace, str), '(PyNestML.CodeGeneration.Printer) No or wrong type of namespace provided (%s)!' % type(namespace)
        function_symbol = ast_function.get_scope().resolve_to_symbol(ast_function.get_name(), SymbolKind.FUNCTION)
        if function_symbol is None:
            raise RuntimeError('Cannot resolve the method ' + ast_function.get_name())
        params = list()
        ...
        declaration += ast_function.get_name() + '('
        for typeSym in function_symbol.get_parameter_types():
            declaration += PyNestml2NestTypeConverter.convert(typeSym) + ' ' + params[function_symbol.get_parameter_types().index(typeSym)]
            if function_symbol.get_parameter_types().index(typeSym) < len(function_symbol.get_parameter_types()) - 1:
                declaration += ', '
    ...
    def print_buffer_array_getter(self, ast_buffer):
        ...
        :rtype: str
        """
        assert ast_buffer is not None and isinstance(ast_buffer, VariableSymbol), '(PyNestML.CodeGeneration.Printer) No or wrong type of ast_buffer symbol provided (%s)!' % type(ast_buffer)
        if ast_buffer.is_spike_buffer() and ast_buffer.is_inhibitory() and ast_buffer.is_excitatory():
            return 'inline ' + PyNestml2NestTypeConverter.convert(ast_buffer.get_type_symbol()) + '&' + ' get_' + ast_buffer.get_symbol_name() + '() {' + '  return spike_inputs_[' + ast_buffer.get_symbol_name().upper() + ' - 1]; }'
        else:
    ...
    def print_buffer_getter(cls, ast_buffer, is_in_struct=False):
        ...
        """
        assert ast_buffer is not None and isinstance(ast_buffer, VariableSymbol), '(PyNestMl.CodeGeneration.Printer) No or wrong type of ast_buffer symbol provided (%s)!' % type(ast_buffer)
        assert is_in_struct is not None and isinstance(is_in_struct, bool), '(PyNestMl.CodeGeneration.Printer) No or wrong type of is-in-struct provided (%s)!' % type(is_in_struct)
        declaration = 'inline '
    ...
    def print_buffer_declaration_value(cls, ast_buffer):
        ...
        :rtype: str
        """
        assert isinstance(ast_buffer, VariableSymbol), '(PyNestML.CodeGeneration.Printer) No or wrong type of ast_buffer symbol provided (%s)!' % type(ast_buffer)
        if ast_buffer.has_vector_parameter():
