...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pynestml/cocos/co_co_illegal_expression.py
# Compiled at: 2020-03-05 05:49:41
# Size of source mod 2**32: 11222 bytes
...
class CorrectExpressionVisitor(ASTVisitor):
    ...
    def visit_declaration(self, node):
        ...
        if node.has_expression():
            ...
            if isinstance(rhs_type, ErrorTypeSymbol):
                ...
                return
            if self._CorrectExpressionVisitor__types_do_not_match(lhs_type, rhs_type):
    ...
    def visit_assignment(self, node):
        ...
        if node.get_source_position().equals(ASTSourceLocation.get_added_source_position()):
            return
        if node.is_direct_assignment:
            self.handle_simple_assignment(node)
        else:
            self.handle_complex_assignment(node)
    ...
    def handle_complex_assignment(self, node):
        ...
        if isinstance(rhs_type_symbol, ErrorTypeSymbol):
            LoggingHelper.drop_missing_type_error(node)
            return
        if self._CorrectExpressionVisitor__types_do_not_match(lhs_variable_symbol.get_type_symbol(), rhs_type_symbol):
            TypeCaster.try_to_recover_or_error(lhs_variable_symbol.get_type_symbol(), rhs_type_symbol, node.get_expression())
    ...
    def handle_simple_assignment(self, node):
        ...
        lhs_variable_symbol = node.get_scope().resolve_to_symbol(node.get_variable().get_complete_name(), SymbolKind.VARIABLE)
        rhs_type_symbol = node.get_expression().type
        if isinstance(rhs_type_symbol, ErrorTypeSymbol):
            LoggingHelper.drop_missing_type_error(node)
            return
        if lhs_variable_symbol is not None and self._CorrectExpressionVisitor__types_do_not_match(lhs_variable_symbol.get_type_symbol(), rhs_type_symbol):
            TypeCaster.try_to_recover_or_error(lhs_variable_symbol.get_type_symbol(), rhs_type_symbol, node.get_expression())
    ...
    def visit_if_clause(self, node):
        ...
        if isinstance(cond_type, ErrorTypeSymbol):
            ...
            Logger.log_message(code=code, message=message, error_position=node.get_condition().get_source_position(), log_level=LoggingLevel.ERROR)
        elif not cond_type.equals(PredefinedTypes.get_boolean_type()):
            code, message = Messages.get_type_different_from_expected(PredefinedTypes.get_boolean_type(), cond_type)
            Logger.log_message(code=code, message=message, error_position=node.get_condition().get_source_position(), log_level=LoggingLevel.ERROR)
    ...
    def visit_for_stmt(self, node):
        ...
        if isinstance(from_type, ErrorTypeSymbol):
            ...
            Logger.log_message(code=code, message=message, error_position=node.get_start_from().get_source_position(), log_level=LoggingLevel.ERROR)
        elif not (from_type.equals(PredefinedTypes.get_integer_type()) or from_type.equals(PredefinedTypes.get_real_type())):
            code, message = Messages.get_type_different_from_expected(PredefinedTypes.get_integer_type(), from_type)
            Logger.log_message(code=code, message=message, error_position=node.get_start_from().get_source_position(), log_level=LoggingLevel.ERROR)
        ...
        if isinstance(to_type, ErrorTypeSymbol):
            ...
            Logger.log_message(code=code, message=message, error_position=node.get_end_at().get_source_position(), log_level=LoggingLevel.ERROR)
        elif not (to_type.equals(PredefinedTypes.get_integer_type()) or to_type.equals(PredefinedTypes.get_real_type())):
