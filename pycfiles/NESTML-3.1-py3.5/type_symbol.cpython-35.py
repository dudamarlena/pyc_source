...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pynestml/symbols/type_symbol.py
...
class TypeSymbol(Symbol):
    ...
    def __init__(self, name):
        ...
        super(TypeSymbol, self).__init__(element_reference=None, scope=None, name=name, symbol_kind=SymbolKind.TYPE)
    ...
    def differs_only_in_magnitude(self, other_type):
        ...
        if (isinstance(unit_a, units.Unit) or isinstance(unit_a, units.PrefixUnit) or isinstance(unit_a, units.CompositeUnit)) and (isinstance(unit_b, units.Unit) or isinstance(unit_b, units.PrefixUnit) or isinstance(unit_b, units.CompositeUnit)) and unit_a.physical_type == unit_b.physical_type:
            return True
        return False
    ...
    def binary_operation_not_defined_error(self, _operator, _other):
        ...
        Logger.log_message(code=code, message=message, error_position=self.referenced_object.get_source_position(), log_level=LoggingLevel.ERROR)
        return result

    def unary_operation_not_defined_error(self, _operator):
        from pynestml.symbols.error_type_symbol import ErrorTypeSymbol
        result = ErrorTypeSymbol()
        code, message = Messages.get_unary_operation_not_defined(_operator, self.print_symbol())
        Logger.log_message(code=code, message=message, error_position=self.referenced_object.get_source_position(), log_level=LoggingLevel.ERROR)
        return result
    ...
    def warn_implicit_cast_from_to(self, _from, _to):
        ...
        Logger.log_message(code=code, message=message, error_position=self.get_referenced_object().get_source_position(), log_level=LoggingLevel.WARNING)
        return _to