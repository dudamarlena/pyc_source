...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pynestml/utils/messages.py
# Compiled at: 2020-03-05 05:49:41
# Size of source mod 2**32: 45216 bytes
...
class Messages(object):
    ...
    def get_implicit_magnitude_conversion(cls, lhs, rhs, conversion_factor):
        message = 'Non-matching unit types at %s +/- %s! Implicitly replaced by %s +/- %s * %s.' % (
    ...
    def get_start_building_symbol_table(cls):
        ...
        """
        return (
         MessageCode.START_SYMBOL_TABLE_BUILDING, 'Start building symbol table!')
    ...
    def get_function_call_implicit_cast(cls, arg_nr, function_call, expected_type, got_type, castable=False):
        ...
        """
        if not castable:
            message = str(arg_nr) + ". argument of function-call '%s' at is wrongly typed! Expected '%s', found '%s'!" % (
             function_call.get_name(), got_type.print_symbol(),
             expected_type.print_symbol())
        else:
            message = str(arg_nr) + ". argument of function-call '%s' is wrongly typed! Implicit cast from '%s' to '%s'." % (
             function_call.get_name(),
             got_type.print_symbol(),
             expected_type.print_symbol())
        return (
         MessageCode.FUNCTION_CALL_TYPE_ERROR, message)
    ...
    def get_type_different_from_expected(cls, expected_type, got_type):
        ...
        assert expected_type is not None and isinstance(expected_type, TypeSymbol), '(PyNestML.Utils.Message) Not a type symbol provided (%s)!' % type(expected_type)
        assert got_type is not None and isinstance(got_type, TypeSymbol), '(PyNestML.Utils.Message) Not a type symbol provided (%s)!' % type(got_type)
    ...
    def get_buffer_set_to_conductance_based(cls, buffer):
        ...
        :rtype: (MessageCode,str)
        """
        assert buffer is not None and isinstance(buffer, str), '(PyNestML.Utils.Message) Not a string provided (%s)!' % type(buffer)
    ...
    def get_ode_updated(cls, variable_name):
        ...
        :rtype: (MessageCode,str)
        """
        assert variable_name is not None and isinstance(variable_name, str), '(PyNestML.Utils.Message) Not a string provided (%s)!' % type(variable_name)
    ...
    def get_buffer_type_not_defined(cls, buffer_name):
        ...
        :rtype: (MessageCode,str)
        """
        assert buffer_name is not None and isinstance(buffer_name, str), '(PyNestML.Utils.Message) Not a string provided (%s)!' % type(buffer_name)
    ...
    def get_neuron_contains_errors(cls, neuron_name):
        ...
        :rtype: (MessageCode,str)
        """
        assert neuron_name is not None and isinstance(neuron_name, str), '(PyNestML.Utils.Message) Not a string provided (%s)!' % type(neuron_name)
    ...
    def get_code_generated(cls, neuron_name, path):
        ...
        """
        assert neuron_name is not None and isinstance(neuron_name, str), '(PyNestML.Utils.Message) Not a string provided (%s)!' % type(neuron_name)
        assert path is not None and isinstance(path, str), '(PyNestML.Utils.Message) Not a string provided (%s)!' % type(path)
    ...
    def get_first_arg_not_shape_or_equation(cls, func_name):
        ...
        :rtype: (MessageCode,str)
        """
        assert func_name is not None and isinstance(func_name, str), '(PyNestML.Utils.Message) Not a string provided (%s)!' % type(func_name)
    ...
    def get_wrong_numerator(cls, unit):
        ...
        :rtype: (MessageCode,str)
        """
        assert unit is not None and isinstance(unit, str), '(PyNestML.Utils.Message) Not a string provided (%s)!' % type(unit)
    ...
    def get_order_not_declared(cls, lhs):
        ...
        :rtype: (MessageCode,str)
        """
        assert lhs is not None and isinstance(lhs, str), '(PyNestML.Utils.Message) Not a string provided (%s)!' % lhs
    ...
    def get_current_buffer_specified(cls, name, keyword):
        ...
        :rtype: (MessageCode,str)
        """
        assert name is not None and isinstance(name, str), '(PyNestML.Utils.Message) Not a string provided (%s)!' % name
    ...
    def get_block_not_defined_correctly(cls, block, missing):
        ...
        :rtype: (MessageCode,str)
        """
        assert block is not None and isinstance(block, str), '(PyNestML.Utils.Message) Not a string provided (%s)!' % type(block)
        assert missing is not None and isinstance(missing, bool), '(PyNestML.Utils.Message) Not a bool provided (%s)!' % type(missing)
        if missing:
            message = block + ' block not defined, model not correct!'
        else:
            message = block + ' block not unique, model not correct!!'
        return (
         MessageCode.BLOCK_NOT_CORRECT, message)
    ...
    def get_wrong_number_of_args(cls, function_call, expected, got):
        ...
        :rtype: (MessageCode,str)
        """
        assert function_call is not None and isinstance(function_call, str), '(PyNestML.Utils.Message) Not a string provided (%s)!' % type(function_call)
        assert expected is not None and isinstance(expected, int), '(PyNestML.Utils.Message) Not a int provided (%s)!' % type(expected)
        assert got is not None and isinstance(got, int), '(PyNestML.Utils.Message) Not a string provided (%s)!' % type(got)
    ...
    def get_no_rhs(cls, name):
        ...
        :rtype: (MessageCode,str)
        """
        assert name is not None and isinstance(name, str), '(PyNestML.Utils.Message) Not a string provided (%s)!' % type(name)
    ...
    def get_several_lhs(cls, names):
        ...
        :rtype: (MessageCode,str)
        """
        assert names is not None and isinstance(names, list), '(PyNestML.Utils.Message) Not a list provided (%s)!' % type(names)
    ...
    def get_function_redeclared(cls, name, predefined):
        ...
        return (
         MessageCode.FUNCTION_REDECLARED, message)
    ...
    def get_compilation_unit_name_collision(cls, name, art1, art2):
        ...
        """
        assert name is not None and isinstance(name, str), '(PyNestML.Utils.Message) Not a string provided (%s)!' % type(name)
        assert art1 is not None and isinstance(art1, str), '(PyNestML.Utils.Message) Not a string provided (%s)!' % type(art1)
        assert art2 is not None and isinstance(art2, str), '(PyNestML.Utils.Message) Not a string provided (%s)!' % type(art2)
    ...
    def get_multiple_keywords(cls, keyword):
        ...
        :rtype: (MessageCode,str)
        """
        assert keyword is not None and isinstance(keyword, str), '(PyNestML.Utils.Message) Not a string provided (%s)!' % type(keyword)
    ...
    def get_vector_in_non_vector(cls, vector, non_vector):
        ...
        :rtype: (MessageCode,str)
        """
        assert vector is not None and isinstance(vector, str), '(PyNestML.Utils.Message) Not a string provided (%s)!' % type(vector)
        assert non_vector is not None and isinstance(non_vector, list), '(PyNestML.Utils.Message) Not a string provided (%s)!' % type(non_vector)
    ...
    def get_variable_redeclared(cls, name, predefined):
        ...
        """
        assert name is not None and isinstance(name, str), '(PyNestML.Utils.Message) Not a string provided (%s)!' % type(name)
        assert predefined is not None and isinstance(predefined, bool), '(PyNestML.Utils.Message) Not a bool provided (%s)!' % type(predefined)
        if predefined:
            message = "Predefined variable '%s' redeclared!" % name
        else:
            message = "Variable '%s' redeclared !" % name
        return (
         MessageCode.VARIABLE_REDECLARED, message)
