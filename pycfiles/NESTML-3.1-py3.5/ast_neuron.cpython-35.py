...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pynestml/meta_model/ast_neuron.py
# Compiled at: 2020-03-05 05:49:41
# Size of source mod 2**32: 27652 bytes
...
class ASTNeuron(ASTNode):
    ...
    def __init__(self, name, body, source_position=None, artifact_name=None):
        ...
        """
        assert isinstance(name, str), '(PyNestML.AST.Neuron) No  or wrong type of neuron name provided (%s)!' % type(name)
        assert isinstance(body, ASTBody), '(PyNestML.AST.Neuron) No or wrong type of neuron body provided (%s)!' % type(body)
        assert artifact_name is not None and isinstance(artifact_name, str), '(PyNestML.AST.Neuron) No or wrong type of artifact name provided (%s)!' % type(artifact_name)
    ...
    def get_input_buffers(self):
        ...
        for symbol in symbols:
            if isinstance(symbol, VariableSymbol) and (symbol.block_type == BlockType.INPUT_BUFFER_SPIKE or symbol.block_type == BlockType.INPUT_BUFFER_CURRENT):
                ret.append(symbol)
    ...
    def get_parameter_symbols(self):
        ...
        ret = list()
        for symbol in symbols:
            if isinstance(symbol, VariableSymbol) and symbol.block_type == BlockType.PARAMETERS and not symbol.is_predefined:
                ret.append(symbol)
    ...
    def get_state_symbols(self):
        ...
        ret = list()
        for symbol in symbols:
            if isinstance(symbol, VariableSymbol) and symbol.block_type == BlockType.STATE and not symbol.is_predefined:
                ret.append(symbol)
    ...
    def get_internal_symbols(self):
        ...
        ret = list()
        for symbol in symbols:
            if isinstance(symbol, VariableSymbol) and symbol.block_type == BlockType.INTERNALS and not symbol.is_predefined:
                ret.append(symbol)
    ...
    def get_ode_aliases(self):
        ...
        ret = list()
        for symbol in symbols:
            if isinstance(symbol, VariableSymbol) and symbol.block_type == BlockType.EQUATION and symbol.is_function:
                ret.append(symbol)
        ...
        for iBuffer in self.get_spike_buffers():
            if iBuffer.is_excitatory() and iBuffer.is_inhibitory():
                if iBuffer is not None:
                    ...
                    Logger.log_message(message=message, code=code, error_position=iBuffer.get_source_position(), log_level=LoggingLevel.ERROR)
    ...
    def get_initial_values_symbols(self):
        ...
        ret = list()
        for symbol in symbols:
            if isinstance(symbol, VariableSymbol) and symbol.block_type == BlockType.INITIAL_VALUES and not symbol.is_predefined:
                ret.append(symbol)
    ...
    def get_ode_defined_symbols(self):
        ...
        ret = list()
        for symbol in symbols:
            if isinstance(symbol, VariableSymbol) and symbol.block_type == BlockType.INITIAL_VALUES and symbol.is_ode_defined() and not symbol.is_predefined:
                ret.append(symbol)
    ...
    def get_state_symbols_without_ode(self):
        ...
        ret = list()
        for symbol in symbols:
            if isinstance(symbol, VariableSymbol) and symbol.block_type == BlockType.STATE and not symbol.is_ode_defined() and not symbol.is_predefined:
                ret.append(symbol)
