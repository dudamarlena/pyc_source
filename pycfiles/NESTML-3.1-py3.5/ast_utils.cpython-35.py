...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pynestml/utils/ast_utils.py
# Compiled at: 2020-03-05 05:49:41
# Size of source mod 2**32: 18631 bytes
...
class ASTUtils(object):
    ...
    @classmethod
    def deconstruct_assignment(cls, lhs=None, is_plus=False, is_minus=False, is_times=False, is_divide=False, _rhs=None):
        """
        ...
        assert is_plus + is_minus + is_times + is_divide == 1, '(PyNestML.CodeGeneration.Utils) Type of assignment not correctly specified!'
        if is_plus:
            op = ASTNodeFactory.create_ast_arithmetic_operator(is_plus_op=True, source_position=_rhs.get_source_position())
        else:
            if is_minus:
                op = ASTNodeFactory.create_ast_arithmetic_operator(is_minus_op=True, source_position=_rhs.get_source_position())
            else:
                if is_times:
                    op = ASTNodeFactory.create_ast_arithmetic_operator(is_times_op=True, source_position=_rhs.get_source_position())
                else:
                    op = ASTNodeFactory.create_ast_arithmetic_operator(is_div_op=True, source_position=_rhs.get_source_position())
        var_expr = ASTNodeFactory.create_ast_simple_expression(variable=lhs, source_position=lhs.get_source_position())
        var_expr.update_scope(lhs.get_scope())
        op.update_scope(lhs.get_scope())
        rhs_in_brackets = ASTNodeFactory.create_ast_expression(is_encapsulated=True, expression=_rhs, source_position=_rhs.get_source_position())
        rhs_in_brackets.update_scope(_rhs.get_scope())
        expr = ASTNodeFactory.create_ast_compound_expression(lhs=var_expr, binary_operator=op, rhs=rhs_in_brackets, source_position=_rhs.get_source_position())
    ...
    def is_castable_to(cls, type_a, type_b):
        ...
        if type_a.is_unit and type_b.is_real:
            ...
            if type_a.is_boolean and type_b.is_real:
                return True
            if type_a.is_real and type_b.is_boolean:
                return True
            if type_a.is_integer and type_b.is_real:
                return True
            if type_a.is_real and type_b.is_integer:
                return True
            return False
    ...
    def get_vectorized_variable(cls, ast, scope):
        ...
        from pynestml.meta_model.ast_variable import ASTVariable
        from pynestml.symbols.symbol import SymbolKind
        variables = (var for var in cls.get_all(ast, ASTVariable) if scope.resolve_to_symbol(var.get_complete_name(), SymbolKind.VARIABLE))
    ...
    def create_internal_block(cls, neuron):
        ...
        if neuron.get_internals_blocks() is None:
            internal = ASTNodeFactory.create_ast_block_with_variables(False, False, True, False, list(), ASTSourceLocation.get_added_source_position())
    ...
    def create_state_block(cls, neuron):
        ...
        from pynestml.meta_model.ast_node_factory import ASTNodeFactory
        if neuron.get_internals_blocks() is None:
            state = ASTNodeFactory.create_ast_block_with_variables(True, False, False, False, list(), ASTSourceLocation.get_added_source_position())
    ...
    def create_initial_values_block(cls, neuron):
        ...
        if neuron.get_initial_blocks() is None:
            initial_values = ASTNodeFactory.create_ast_block_with_variables(False, False, False, True, list(), ASTSourceLocation.get_added_source_position())
    ...
    def contains_sum_call(cls, variable):
        ...
        for func in variable.get_declaring_expression().get_function_calls():
            if func.get_name() == PredefinedFunctions.CONVOLVE or func.get_name() == PredefinedFunctions.CURR_SUM or func.get_name() == PredefinedFunctions.COND_SUM:
                return True
