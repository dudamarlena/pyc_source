...
# [GCC 8.4.0]
# Embedded file name: /home/maikel/Projects/Trionyx/trionyx/quickstart/__init__.py
# Compiled at: 2020-02-12 11:54:24
...
:license: GPLv3
"""
import os, shutil, subprocess, yaml, trionyx
from trionyx import utils

class Quickstart:
    __doc__ = 'Class for creating Trionyx project and apps'
    ...
    def create_project(self, project_path):
        ...
        self.update_file(project_path, 'requirements.txt', {'trionyx_version': trionyx.__version__})
        self.update_file(project_path, 'environment.json', {'secret_key': utils.random_string(32)})
        app_name = os.path.basename(project_path).capitalize()
        self.update_file(project_path, 'README.rst', {'title': '{name}\n{heading}'.format(name=app_name,
                    heading=('=' * len(app_name)))})
        self.update_file(project_path, 'config/settings/base.py', {'app_name':app_name, 
    ...
    def create_app(self, apps_path, name):
        ...
        self.update_file(app_path, '__init__.py', {'name': name.lower()})
        self.update_file(app_path, 'apps.py', {'name':name.lower(), 
    ...
    def create_ansible(self, project_path, domain, repo):
        ...
        self.update_file(project_path, 'production', {'domain': domain})
        ...
        config = {'ansible_ssh_user':'ansible', 
         ...
         'smtp_password':''}
        self.update_file(project_path, 'group_vars/all.yml', {'config': yaml.dump(config, default_flow_style=False, allow_unicode=True)})
