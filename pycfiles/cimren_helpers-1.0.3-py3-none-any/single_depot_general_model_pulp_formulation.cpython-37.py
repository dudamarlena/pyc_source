...
# Embedded file name: build\bdist.win-amd64\egg\cvrptw_optimization\src\single_depot_general_model_pulp_formulation.py
# Compiled at: 2020-04-05 02:46:52
# Size of source mod 2**32: 11263 bytes
__doc__ = '\nInput data sets\nCVRPTW formulation\n'
from pulp import *
import pandas as pd
...
class ModelFormulation:
    ...
    def __init__(self, time_variables_dict, assignment_variables_dict, vertices_dict, vehicles_dict, customers_dict, transit_dict, transit_starting_customers_dict, depot_name):
    ...
    def formulate_problem(self, bigm=100000000):
        """
        Formulate problem
        :param bigm:
        :return:
        ...
        self.assignment_var = pulp.LpVariable.dicts('Assign', self.assignment_variables_dict.keys(), 0, 1, pulp.LpBinary)
        self.model = pulp.LpProblem('CVRPTW', pulp.LpMinimize)
        print('objective function')
        self.model += pulp.lpSum((self.transit_dict['TRANSPORTATION_COST'][(from_loc, to_loc)] * self.assignment_var[(from_loc, to_loc, vehicle)] for from_loc, to_loc, vehicle in self.assignment_variables_dict.keys()))
        ...
        for customer in self.customers_dict['DEMAND'].keys():
            ...
            self.model += (
             pulp.lpSum([self.assignment_var[(customer, to_loc, vehicle)] for to_loc in outgoing_arcs for vehicle in self.vehicles_dict['CAPACITY'].keys()]) == 1,
             'customerVisit' + str(customer) + 'k')
        ...
        for vehicle in self.vehicles_dict['CAPACITY'].keys():
            self.model += (
             pulp.lpSum([self.assignment_var[(depot_leave, customer, vehicle)] for customer in self.customers_dict['DEMAND'].keys()]) == 1,
             'entryDepotConnection' + str(vehicle))
        ...
        for customer in self.customers_dict['DEMAND'].keys():
            for vehicle in self.vehicles_dict['CAPACITY'].keys():
                ...
                self.model += (
                 pulp.lpSum([self.assignment_var[(from_loc, customer, vehicle)] for from_loc in incoming_arcs]) - pulp.lpSum([self.assignment_var[(customer, to_loc, vehicle)] for to_loc in outgoing_arcs]) == 0,
                 'forTrip' + str(customer) + 'k' + str(vehicle))

        print('Each vehicle should enter a depot')
        depot_enter = self.depot_name + '_ENTER'
        for vehicle in self.vehicles_dict['CAPACITY'].keys():
            self.model += (
             pulp.lpSum([self.assignment_var[(customer, depot_enter, vehicle)] for customer in self.customers_dict['DEMAND'].keys()]) == 1,
             'exitDepotConnection' + str(vehicle))

        print('vehicle Capacity')
        for vehicle in self.vehicles_dict['CAPACITY'].keys():
            self.model += (
             pulp.lpSum([float(self.customers_dict['DEMAND'][from_loc]) * self.assignment_var[(from_loc, to_loc, vehicle)] for from_loc, to_loc in self.transit_starting_customers_dict['DRIVE_MINUTES'].keys()]) <= float(self.vehicles_dict['CAPACITY'][vehicle]), 'Capacity' + str(vehicle))
        ...
        for from_loc, to_loc, vehicle in self.assignment_variables_dict.keys():
            ...
            self.model += (
             self.time_var[(to_loc, vehicle)] - self.time_var[(from_loc, vehicle)] >= self.transit_dict['DRIVE_MINUTES'][(from_loc, to_loc)] + stop_time + bigm * self.assignment_var[(from_loc, to_loc, vehicle)] - bigm,
             'timewindow' + str(vehicle) + 'p' + str(from_loc) + 'p' + str(to_loc))

        print('time windows')
        for vertex, vehicle in self.time_variables_dict.keys():
            self.time_var[(vertex, vehicle)].bounds(float(self.vertices_dict['TIME_WINDOW_START'][vertex]), float(self.vertices_dict['TIME_WINDOW_END'][vertex]))

    def solve_model(self, mip_gap=0.001, solver_time_limit_minutes=10, enable_solution_messaging=1, solver_type='PULP_CBC_CMD'):
        """
        Solve model
        :param mip_gap:
        ...
        if solver_type == 'PULP_CBC_CMD':
            self.model.solve(PULP_CBC_CMD(msg=enable_solution_messaging,
        ...
        if self.model.status == 1:
            ...
            for loc, vehicle in self.time_var.keys():
                if self.time_var[(loc, vehicle)].value() > 0:
                    ...
                    solution_time.append({'LOCATION_NAME':loc,  'VEHICLE':vehicle, 
