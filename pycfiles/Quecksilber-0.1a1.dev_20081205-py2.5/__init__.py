...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/quecksilber/__init__.py
...
__all__ = [
 'blacklist_changeset', 'one_head', 'branch_name',
...
def blacklist_changeset(ui, repo, node, **kwargs):
    ...
    """
    blacklist = ui.config(SECTION_CHANGESET, SECTION_CHANGESET_ENTRY_BLACKLIST).split()
    broken = False
    for this_node in (hex(cctx.node()) for cctx in _all_changectxs(repo, node)):
        if this_node in blacklist:
            if not broken:
                ui.warn('You are trying to commit (a) blacklisted changeset(s):\n')
            ui.warn('  - %s\n' % this_node)
            broken = True
...
def one_head(ui, repo, node, **kwargs):
    ...
    """
    if len(repo.heads()) > 1:
        ui.warn('Action will result in multiple heads. This is not allowed.\n')
        return True
    return False
...
def file_content_check(ui, repo, node, **kwargs):
    ...
    broken = False
    pattern = {}
    for ext in ui.config(SECTION_FILE_CHECK, SECTION_FILE_CHECK_ENTRY_EXTENSIONS).split():
        pattern[ext] = re.compile(eval(ui.config(SECTION_FILE_CHECK, ext)), re.MULTILINE)
    ...
    for (file_, cctx) in _all_files_and_changectxs(repo, node):
        ...
        if file_ext in pattern.keys():
            if pattern[file_ext].match(cctx[file_].data()):
                if not broken:
                    ui.warn('Some files do match the bad file content regular expression:\n')
