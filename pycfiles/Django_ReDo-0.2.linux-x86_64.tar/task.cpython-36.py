...
# [GCC 8.4.0]
# Embedded file name: /hdd/dev/os/django-do/.env/lib/python3.6/site-packages/django_redo/task.py
# Compiled at: 2018-10-05 04:31:08
# Size of source mod 2**32: 7387 bytes
import time, json, redis, importlib
...
class Task(object):
    ...
    def __str__(self):
        ...
        return '{}{}{}(*{}, **{})'.format(fn['m'], '.'.join([''] + fn['n']) if fn['n'] else '', '.' + fn['f'], self.args, self.kwargs)
    ...
    def serialize(self):
        ...
        :return str:
        """
        return json.dumps({'f':self.serialize_function(self.fn), 
         'a':self.args, 
         'k':self.kwargs})
...
class RedisQueue(object):
    ...
    def _redis_connect(cls, conf):
        ...
        if 'unix_socket_path' in conf:
            return redis.Redis(db=(conf.get('DB', 0)),
