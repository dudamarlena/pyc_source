...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/postpy/dml.py
# Compiled at: 2017-10-24 10:28:33
...
def create_insert_statement(qualified_name, column_names, table_alias='', param_style=PYFORMAT):
    column_string = ', '.join(column_names)
    param_func = PARAM_STYLES.get(param_style)
    value_string = param_func(column_names)
    if table_alias:
        table_alias = ' AS %s' % table_alias
    return 'INSERT INTO {0}{1} ({2}) VALUES ({3})'.format(qualified_name, table_alias, column_string, value_string)
...
def insert_many(conn, tablename, column_names, records, chunksize=2500):
    ...
    insert_template = 'INSERT INTO {table} ({columns}) VALUES {values}'.format(table=tablename, columns=column_str, values='{0}')
    with conn:
...
def format_upsert(qualified_name, column_names, constraint, clause='', table_alias='current', param_style=PYFORMAT):
    insert_template = create_insert_statement(qualified_name, column_names, table_alias=table_alias, param_style=param_style)
    statement = format_upsert_expert(insert_template, column_names, constraint, clause, table_alias)
    return statement


def format_upsert_expert(insert_template, column_names, constraint, clause='', table_alias='current'):
    ...
    if non_key_columns:
        ...
        action = ' DO UPDATE SET ({non_key_columns}) = ({excluded}) {clause}'.format(non_key_columns=non_key_column_str, excluded=excluded_str, clause=clause, table_alias=table_alias)
    else:
        action = ' DO NOTHING'
    statement = '{insert_template} ON CONFLICT ({constraint}){action}'.format(insert_template=insert_template, constraint=constraint_str, action=action)
    return statement


def format_upsert_primary_key(qualified_name, column_names, primary_key_names, param_style=PYFORMAT):
    warning = 'Deprecation Warning. Function will be removed as of version 0.1.0.'
    warnings.warn(warning, DeprecationWarning)
    query = format_upsert(qualified_name, column_names, primary_key_names, param_style=param_style)
    return query


class DeleteManyPrimaryKey:
    __doc__ = 'Deletes subset of table rows.\n\n    Uses DELETE FROM in conjunction with a where clause\n    through a temporary table reference containing primary keys\n    to delete.\n    '
    ...
    def __call__(self, conn, records, chunksize=2500):
        with conn:
            ...
            insert_many(conn, self.delete_table.name, self.delete_table.column_names, records, chunksize=chunksize)
            delete_from_statement = delete_joined_table_sql(self.table.qualified_name, self.delete_table.name, self.table.primary_key.column_names)
...
class UpsertPrimaryKey:
    ...
    def __init__(self, qualified_name, column_names, primary_key_names):
        self.query = format_upsert(qualified_name, column_names, primary_key_names)
...
def delete_joined_table_sql(qualified_name, removing_qualified_name, primary_key):
    ...
    where_clause = ' AND '.join(condition_template.format(pkey, pkey) for pkey in primary_key)
    delete_statement = 'DELETE FROM {table} t USING {delete_table} d WHERE {where_clause}'.format(table=qualified_name, delete_table=removing_qualified_name, where_clause=where_clause)
...
class CopyFrom(CopyFromCsvBase):
    __doc__ = 'Copy from CSV file object.'
...
class CopyFromUpsert(BulkDmlPrimaryKey):
    __doc__ = 'Upsert subset of table rows contained in a file stream.\n\n    Upsert rows based on same composite primary key.\n    '
    TEMP_PREFIX = 'tmp_bulk_upsert'
    _INSERT_TEMPLATE = 'INSERT INTO {table} ({columns})\n  SELECT {columns} FROM {temp_table}\n'

    def make_dml_query(self):
        query = self._INSERT_TEMPLATE.format(table=self.table.qualified_name, columns=self.column_str, temp_table=self.copy_table.name)
        query = format_upsert_expert(query, self.table.column_names, self.table.primary_key_columns)
        return query


class CopyFromDelete(BulkDmlPrimaryKey):
    __doc__ = 'Deletes subset of table rows contained in a file stream.\n\n    Deletes rows with matching composite primary key.\n    '
    TEMP_PREFIX = 'delete_from'

    def make_dml_query(self):
        delete_from_statement = delete_joined_table_sql(self.table.qualified_name, self.copy_table.name, self.table.primary_key.column_names)
        return delete_from_statement


def copy_from_csv(conn, file, qualified_name: str, delimiter=',',
                  encoding='utf8', null_str='',
                  header=True, escape_str='\\', quote_char='"', force_not_null=None,
                  force_null=None):
    ...
    """
    copy_sql = copy_from_csv_sql(qualified_name, delimiter, encoding, null_str=null_str, header=header, escape_str=escape_str, quote_char=quote_char, force_not_null=force_not_null, force_null=force_null)
    with conn:
        with conn.cursor() as (cursor):
