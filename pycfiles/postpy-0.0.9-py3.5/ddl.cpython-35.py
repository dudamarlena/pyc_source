...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/postpy/ddl.py
...
def compile_create_table(qualified_name: str, column_statement: str, primary_key_statement: str) -> str:
    """Postgresql Create Table statement formatter."""
    statement = '\n                CREATE TABLE {table} ({columns} {primary_keys});\n                '.format(table=qualified_name, columns=column_statement, primary_keys=primary_key_statement)
    return statement


def compile_create_temporary_table(table_name: str, column_statement: str, primary_key_statement: str) -> str:
    """Postgresql Create Temporary Table statement formatter."""
    statement = '\n                CREATE TEMPORARY TABLE {table} ({columns} {primary_keys});\n                '.format(table=table_name, columns=column_statement, primary_keys=primary_key_statement)
    return statement
...
class CreateTableAs:
    ...
    def compile(self):
        statement = '{create} ({select} {clause})'.format(create=self._create_statement(), select=self._select_statement(), clause=self._clause_statement())
        return statement

    def compile_with_cte(self, common_table_expression):
        statement = '{create} (WITH {cte} {select} {clause})'.format(create=self._create_statement(), cte=common_table_expression, select=self._select_statement(), clause=self._clause_statement())
        return statement
    ...
    def _select_statement(self):
        return '\n  SELECT {column_str} \n  FROM {parent_table}'.format(column_str=self.column_str, parent_table=self.parent_table)
...
class MaterializedView:
    __doc__ = 'Postgres materialized view declaration formatter.'
