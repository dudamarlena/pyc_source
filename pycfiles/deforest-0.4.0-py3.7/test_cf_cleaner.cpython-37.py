...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/deforest/tests/test_cf_cleaner.py
...
class TestCFCleaner(unittest.TestCase):
    ...
    def test_clean_cf_removed(self):
        self.result = {'AWSTemplateFormatVersion':'2010-09-09', 
         'Description':'a sample template',  'Resources':{'MyRestApi': {'Type':'AWS::ApiGateway::RestApi',  'Properties':{'Body': {'swagger':'2.0', 
                                  'info':{'title':'a title',  'version':'0.1'}}}}}}
        sut = cleaner.CloudFormationCleaner(self)
        sut.clean()
    ...
    def test_clean_cf_two_apis(self):
        self.result = {'AWSTemplateFormatVersion':'2010-09-09', 
         'Description':'a sample template',  'Resources':{'MyRestApi':{'Type':'AWS::ApiGateway::RestApi',  'Properties':{'Body': {'swagger':'2.0', 
                     'info':{'title':'a title',  'version':'0.1'}}}}, 
          'MySecondsRestApi':{'Type':'AWS::ApiGateway::RestApi', 
           'Properties':{'Body': {'swagger':'2.0',  'info':{'title': 'second api'}}}}}}
        sut = cleaner.CloudFormationCleaner(self)
        sut.clean()
    ...
    def test_clean_cf_ignore_invalid_api(self):
        self.result = {'AWSTemplateFormatVersion':'2010-09-09', 
         'Description':'a sample template',  'Resources':{'MyRestApi':{'Type':'AWS::ApiGateway::RestApi',  'Properties':{'Body': {'swagger':'2.0', 
                     'info':{'title':'a title',  'version':'0.1'}}}}, 
          'MySecondsRestApi':{'NotType':'AWS::ApiGateway::RestApi', 
           'Properties':{'Body': {'swagger':'2.0',  'info':{'title': 'second api'}}}}}}
        sut = cleaner.CloudFormationCleaner(self)
        sut.clean()
        assert len(self.result) == 1
    ...
    def test_clean_cf_serverless_api(self):
        self.result = {'AWSTemplateFormatVersion':'2010-09-09', 
         'Description':'a sample template',  'Resources':{'MyRestApi': {'Type':'AWS::Serverless::Api',  'Properties':{'DefinitionBody': {'swagger':'2.0', 
                                            'info':{'title':'a title',  'version':'0.1'}}}}}}
        sut = cleaner.CloudFormationCleaner(self)
        sut.clean()
        assert 'AWSTemplateFormatVersion' not in self.result[0]
    ...
    def test_clean_cf_missing_body(self):
        self.result = {'AWSTemplateFormatVersion':'2010-09-09', 
         'Description':'a sample template',  'Resources':{'MyRestApi': {'Type':'AWS::Serverless::Api',  'Properties':{'InvalidBody': {'swagger':'2.0', 
                                         'info':{'title':'a title',  'version':'0.1'}}}}}}
        sut = cleaner.CloudFormationCleaner(self)
    ...
    def test_clean_cf_and_srvless_apis(self):
        self.result = {'AWSTemplateFormatVersion':'2010-09-09',  'Description':'a sample template',  'Resources':{'MyRestApi':{'Type':'AWS::ApiGateway::RestApi',  'Properties':{'Body': {'swagger':'2.0', 
                     'info':{'title':'a title',  'version':'0.1'}}}}, 
          'MySecondsRestApi':{'Type':'AWS::Serverless::Api', 
           'Properties':{'DefinitionBody': {'swagger':'2.0',  'info':{'title': 'second api'}}}}}}
        sut = cleaner.CloudFormationCleaner(self)
        sut.clean()
        assert len(self.result) == 2
        assert 'AWSTemplateFormatVersion' not in self.result[0]
