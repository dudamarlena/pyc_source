...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/deforest/filecreator.py
# Compiled at: 2020-03-17 13:35:56
...
class FileCreator:
    ...
    def write_to_file(self):
        logging.debug('writing {} results to file, format setting: {}'.format(len(self.data), self.format))
        if len(self.data) > 1:
            if self.filename is not None:
                logging.warning('using default deforest filenaming conventions since the output constists of more than one file')
                self.filename = None
        for d in self.data:
            fname = self.filename
            if self.filename is None:
                logging.debug('setting filename {}'.format(self._specify_filename(d)))
