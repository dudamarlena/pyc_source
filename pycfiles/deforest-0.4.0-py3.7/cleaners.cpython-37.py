...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/deforest/cleaners.py
...
class CloudFormationCleaner:
    ...
    def clean(self):
        ...
        if CloudFormationCleaner.KEY_CF_FIELD not in self.caller.result:
            self.caller.result = [
             self.caller.result]
            return
        rval = []
        ...
        for k in res:
            if isinstance(res[k], dict):
                if self._is_apigw(res[k]):
                    logging.info("identified resource '{}' as an API Gateway".format(k))
                    if self._has_body(res[k]):
                        key = CloudFormationCleaner.KEY_CF_BODY
                    else:
                        if self._has_definition_body(res[k]):
                            key = CloudFormationCleaner.KEY_SRVLESS_BODY
                        else:
                            raise InvalidBodyProperty('could not find a valid body property')
                rval.append(res[k][CloudFormationCleaner.KEY_CF_PROPERTIES][key])
