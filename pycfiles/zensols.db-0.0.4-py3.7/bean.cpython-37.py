...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/db/bean.py
# Compiled at: 2019-09-28 20:01:03
# Size of source mod 2**32: 18405 bytes
...
from zensols.actioncli import resource, Stash, ConfigFactory
from zensols.db import DynamicDataParser
logger = logging.getLogger(__name__)

class connection(resource):
    __doc__ = 'Annotation used to create and dispose of DB-API connections.\n\n    '

    def __init__(self):
        super(connection, self).__init__('_create_connection', '_dispose_connection')


class ConnectionManager(ABC):
    __doc__ = 'Instance DB-API connection lifecycle.\n\n    '
...
class DbPersister(object):
    __doc__ = 'CRUDs data to/from a DB-API connection.\n\n    '
    ...
    def execute_by_name(self, name, params=(), row_factory='tuple', map_fn=None):
...
class Bean(ABC):
    __doc__ = 'A container class like a Java *bean*.\n\n    '
    ...
    def __str__(self):
        return ', '.join(map(lambda x: f"{x}: {getattr(self, x)}", self.get_attr_names()))
...
class ReadOnlyBeanDbPersister(DbPersister):
    __doc__ = 'A read-only persister that CRUDs data based on predefined SQL given in the\n    configuration.  The class optionally works with instances of ``Bean`` when\n    ``row_factory`` is set to the target bean class.\n\n    '

    def __init__(self, *args, select_name=None, select_by_id=None, select_exists=None, row_factory='tuple', **kwargs):
        """Initialize.
    ...
    def get(self) -> list:
        ...
        """
        return self.execute_by_name((self.select_name),
    ...
    def get_by_id(self, id: int):
        ...
        """
        rows = self.execute_by_name((self.select_by_id),
          params=(id,), row_factory=(self.row_factory))
        if len(rows) > 0:
            return rows[0]
    ...
    def exists(self, id: int) -> bool:
        ...
        cnt = self.execute_by_name((self.select_exists),
          params=(id,), row_factory='tuple')
        return cnt[0][0] == 1


class InsertableBeanDbPersister(ReadOnlyBeanDbPersister):
    __doc__ = 'A class that contains insert funtionality.\n\n    '

    def __init__(self, *args, insert_name=None, **kwargs):
        """Initialize.
    ...
    @connection()
    def insert_rows(self, conn, rows: list, errors='raise', set_id_fn=None, map_fn=None) -> int:
        ...
        self._check_entry(entry_name)
        sql = self.sql_entries[entry_name]
        return self.conn_manager.insert_rows(conn, sql, rows, errors, set_id_fn, map_fn)
...
class UpdatableBeanDbPersister(InsertableBeanDbPersister):
    __doc__ = "A class that contains the remaining CRUD funtionality the super class\n    doesn't have.\n\n    "

    def __init__(self, *args, update_name=None, delete_name=None, **kwargs):
        """Initialize.
...
class BeanDbPersister(UpdatableBeanDbPersister):
    ...
    def __init__(self, *args, keys_name=None, count_name=None, **kwargs):
        """Initialize.
...
class BeanStash(Stash):
    __doc__ = 'A stash that uses a backing DB-API backed ``BeanDbPersister``.\n\n    '
...
class DbPersisterFactory(ConfigFactory):
    ...
    def _class_name_params(self, name):
        class_name, params = super(DbPersisterFactory, self)._class_name_params(name)
