...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/easyBay/__init__.py
# Compiled at: 2006-03-18 18:56:52
...
class eBay(object):
    ...
    class eBayError(Exception):
        ...
        def __repr__(self):
            return '(%s) %s: %s' % (str(self.ErrorCode), str(self.SeverityCode), str(self.LongMessage))
    ...
    def __init__(self, dev=None, app=None, cert=None, token=None, url='http://api.ebay.com/ws/api.dll', config=None):
        if config:
    ...
    def __repr__(self):
        return '<API.eBay dev=%r, app=%r, cert=%r, token=%r, server=%r>' % (self._dev, self._app, self._cert, str(self._token)[:15] + (str(self._token)[15:] and '...'), self._server)
    ...
    def _call(self, verb):
        ...
        def _request(*args, **kwargs):
            req = Element(verb + 'Request', xmlns='urn:ebay:apis:eBLBaseComponents')
            ...
            conn.request('POST', self._command, reqString, self._headers(verb, kwargs))
    ...
    def _headers(self, verb, kwargs):
        return {'X-EBAY-API-COMPATIBILITY-LEVEL': str(self._default('Version', self._callDefaults, 435)), 'X-EBAY-API-SESSION-CERTIFICATE': '%s;%s;%s' % (self._dev, self._app, self._cert), 'X-EBAY-API-DEV-NAME': self._dev, 'X-EBAY-API-APP-NAME': self._app, 'X-EBAY-API-CERT-NAME': self._cert, 'X-EBAY-API-CALL-NAME': verb, 'X-EBAY-API-SITEID': str(self._default('SiteId', kwargs, self._callDefaults, 0)), 'Content-Type': 'text/xml'}
...
class Node(object):
    ...
    def _repr(self, pre=''):
        ...
        if not self:
            return repr(None)
        return ('\n').join([ Node.safeCreate(e)._repr(pre + self._tag) for e in self._elem.getchildren() ])
