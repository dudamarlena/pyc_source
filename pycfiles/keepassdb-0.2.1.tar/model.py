...
# [GCC 8.4.0]
# Embedded file name: /home/hans/workspace/keepassdb/keepassdb/model.py
...
import abc, logging, base64
from keepassdb import const, util
from keepassdb.structs import GroupStruct, EntryStruct
__authors__ = [
 'Karsten-Kai König <kkoenig@posteo.de>', 'Hans Lellelid <hans@xmpl.org>']
...
class Group(BaseModel):
    ...
    def __init__(self, id=None, title=None, icon=None, level=None, created=None, modified=None, accessed=None, expires=None, flags=None, parent=None, db=None):
        """
    ...
    def __repr__(self):
        return ('<Group title={0} id={1} level={2}>').format(self.title, self.id, self.level)
    ...
    def to_dict(self, hierarchy=True, hide_passwords=False):
        d = dict(id=self.id, title=self.title, icon=self.icon, level=self.level, created=self.created if self.created != const.NEVER else None, modified=self.modified if self.modified != const.NEVER else None, accessed=self.accessed if self.accessed != const.NEVER else None, expires=self.expires if self.expires != const.NEVER else None, flags=self.flags)
...
class Entry(BaseModel):
    ...
    def __init__(self, uuid=None, group_id=None, group=None, icon=None, title=None, url=None, username=None, password=None, notes=None, created=None, modified=None, accessed=None, expires=None, binary_desc=None, binary=None):
        """
    ...
    def __repr__(self):
        return ('<Entry title={0} username={1}>').format(self.title, self.username)
    ...
    def to_dict(self, hide_passwords=False):
        d = dict(uuid=self.uuid, group_id=self.group_id, icon=self.icon, title=self.title, url=self.url, username=self.username, password=(hide_passwords or self).password if 1 else '********', notes=self.notes, created=self.created if self.created != const.NEVER else None, modified=self.modified if self.modified != const.NEVER else None, expires=self.expires if self.expires != const.NEVER else None, binary_desc=self.binary_desc, binary=base64.b64encode(self.binary) if self.binary is not None else '')
        return d