...
# [GCC 8.4.0]
# Embedded file name: /home/hans/workspace/keepassdb/keepassdb/db.py
...
"""
import binascii, logging, os, os.path, hashlib
...
__authors__ = [
 'Karsten-Kai König <kkoenig@posteo.de>', 'Hans Lellelid <hans@xmpl.org>', 'Brett Viren <brett.viren@gmail.com>']
...
class Database(object):
    ...
    def create_default_group(self):
        ...
        return self.create_group('Internet', icon=1)
    ...
    def load_from_buffer(self, buf, password=None, keyfile=None, readonly=False):
        ...
        final_key = util.derive_key(seed_key=self.header.seed_key, seed_rand=self.header.seed_rand, rounds=self.header.key_enc_rounds, password=password, keyfile=keyfile)
        ...
        if len(decrypted_content) > const.DB_MAX_CONTENT_LEN or len(decrypted_content) == 0 and self.header.ngroups > 0:
    ...
    def save(self, dbfile=None, password=None, keyfile=None):
        ...
        final_key = util.derive_key(seed_key=header.seed_key, seed_rand=header.seed_rand, rounds=header.key_enc_rounds, password=password, keyfile=keyfile)
    ...
    def create_group(self, title, parent=None, icon=1, expires=None):
        ...
        if self.groups:
            ...
            group_id = 1
        group = Group(id=group_id, title=title, icon=icon, db=self, created=util.now(), modified=util.now(), accessed=util.now(), expires=expires)
        if parent is None:
