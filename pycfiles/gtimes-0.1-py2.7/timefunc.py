...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gtimes/timefunc.py
# Compiled at: 2016-01-20 04:25:04
"""
...
"""
import time, math, calendar, datetime, pytz, numpy as np
...
def TimefromYearf(yearf, string=None):
    ...
    doy = int(math.floor(dayf))
    fofday = dayf - doy
    Hour = int(math.floor(fofday * 24))
    Min = int(math.floor(fofday * 24 * 60 % 60))
    fsec = fofday * 24 * 60 * 60 % 60
    Sec = int(math.floor(fsec))
    musec = int(math.floor((fsec - Sec) * 1000000))
...
def gpsWeekDay(days=0, refday=currDate(), fromYearf=False):
    ...
    return gpsFromUTC(hour=12, min=0, sec=0, *tmp)[0:3:2]
...
def shlyear(yyyy=currYear(), change=True):
    ...
    if len(str(abs(yyyy))) <= 2 and change == True:
...
def toDatetimel(dStrlist, fStr):
    ...
    if type(dStrlist) is not list:
        dStrlist = [
         dStrlist]
    dStrlist = [ toDatetime(dStr, fStr) for dStr in dStrlist ]
    if len(dStrlist) == 1:
        return dStrlist[0]
    else:
        return dStrlist
