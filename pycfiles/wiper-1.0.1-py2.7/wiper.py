...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/wiper/wiper.py
# Compiled at: 2015-09-10 20:50:10
"""
...
import ConfigParser, logging, re, socket, sys, time, paramiko
from paramikoe import SSHClientInteraction
from transitions import Machine

class WiperApicInteract(SSHClientInteraction):

    def __init__(self, client, timeout=60, newline='\r', buffer_size=1024, display=False, type=''):
        self.type = type
        SSHClientInteraction.__init__(self, client, timeout, newline, buffer_size, display)
...
class ProvisionApic(Machine):
    ...
    def __init__(self, opts):
        ...
        self.states = [{'name': 'start'}, {'name': 'connect_cimc', 'on_enter': 'on_enter_connect_cimc'}, {'name': 'check_sol', 'on_enter': 'on_enter_check_sol'}, {'name': 'configure_sol', 'on_enter': 'on_enter_configure_sol'}, {'name': 'disconnect_cimc', 'on_enter': 'on_enter_disconnect_cimc'}, {'name': 'cycle_host', 'on_enter': 'on_enter_cycle_host'}, {'name': 'connect_apic', 'on_enter': 'on_enter_connect_apic'}, {'name': 'logout_apic', 'on_enter': 'on_enter_logout_apic'}, {'name': 'login_apic', 'on_enter': 'on_enter_login_apic'}, {'name': 'password_login_apic', 'on_enter': 'on_enter_password_login_apic'}, {'name': 'eraseconfig', 'on_enter': 'on_enter_eraseconfig'}, {'name': 'press_any_key', 'on_enter': 'on_enter_press_any_key'}, {'name': 'provide_fabric_name', 'on_enter': 'on_enter_provide_fabric_name'}, {'name': 'provide_number_ctrlrs', 'on_enter': 'on_enter_provide_number_ctrlrs'}, {'name': 'provide_ctrlr_id', 'on_enter': 'on_enter_provide_ctrlr_id'}, {'name': 'provide_ctrlr_name', 'on_enter': 'on_enter_provide_ctrlr_name'}, {'name': 'provide_tep_addr_pool', 'on_enter': 'on_enter_provide_tep_addr_pool'}, {'name': 'provide_infra_vlan_id', 'on_enter': 'on_enter_provide_infra_vlan_id'}, {'name': 'provide_bd_mc_addr_pool', 'on_enter': 'on_enter_provide_bd_mc_addr_pool'}, {'name': 'provide_oob_address', 'on_enter': 'on_enter_provide_oob_address'}, {'name': 'provide_oob_def_gw', 'on_enter': 'on_enter_provide_oob_def_gw'}, {'name': 'provide_int_speed', 'on_enter': 'on_enter_provide_int_speed'}, {'name': 'provide_strong_passwd', 'on_enter': 'on_enter_provide_strong_passwd'}, {'name': 'provide_admin_passwd', 'on_enter': 'on_enter_provide_admin_passwd'}, {'name': 'provide_modify_config', 'on_enter': 'on_enter_provide_modify_config'}]
        Machine.__init__(self, states=self.states, initial='start')
        self.add_transition(trigger='start', source='start', dest='connect_cimc')
        self.add_transition(trigger='cimc_prompt_detected', source='connect_cimc', dest='check_sol')
        self.add_transition(trigger='sol_not_configured', source='check_sol', dest='configure_sol')
        self.add_transition(trigger='sol_config_committed', source='configure_sol', dest='check_sol')
        self.add_transition(trigger='cycle_host', source='connect_apic', dest='cycle_host')
        self.add_transition(trigger='connect_to_apic', source=[
         'check_sol', 'logout_apic'], dest='connect_apic')
        self.add_transition(trigger='apic_prompt_detected', source='connect_apic', dest='logout_apic')
        self.add_transition(trigger='apic_login_detected', source=[
         'connect_apic', 'logout_apic', 'password_login_apic',
         'cycle_host'], dest='login_apic')
        self.add_transition(trigger='apic_password_detected', source=[
         'connect_apic', 'cycle_host'], dest='password_login_apic')
        self.add_transition(trigger='apic_prompt_detected', source=[
         'login_apic', 'cycle_host'], dest='eraseconfig')
        self.add_transition(trigger='press_any_key', source=[
         'connect_apic', 'eraseconfig', 'cycle_host'], dest='press_any_key')
        self.add_transition(trigger='enter_fabric_name', source=[
         'connect_apic', 'provide_modify_config', 'press_any_key'], dest='provide_fabric_name')
        self.add_transition(trigger='enter_num_ctrlrs', source=[
         'connect_apic', 'provide_fabric_name'], dest='provide_number_ctrlrs')
        self.add_transition(trigger='enter_ctrlr_id', source=[
         'connect_apic', 'provide_number_ctrlrs'], dest='provide_ctrlr_id')
        self.add_transition(trigger='enter_ctrlr_name', source=[
         'connect_apic', 'provide_ctrlr_id'], dest='provide_ctrlr_name')
        self.add_transition(trigger='enter_tep_addr_pool', source=[
         'connect_apic', 'provide_ctrlr_name'], dest='provide_tep_addr_pool')
        self.add_transition(trigger='enter_infra_vlan_id', source=[
         'connect_apic', 'provide_tep_addr_pool'], dest='provide_infra_vlan_id')
        self.add_transition(trigger='enter_bd_mc_addr_pool', source=[
         'connect_apic', 'provide_infra_vlan_id'], dest='provide_bd_mc_addr_pool')
        self.add_transition(trigger='enter_oob_ip_addr', source=[
         'connect_apic',
         'provide_bd_mc_addr_pool',
         'provide_infra_vlan_id'], dest='provide_oob_address')
        self.add_transition(trigger='enter_oob_def_gw', source=[
         'connect_apic', 'provide_oob_address'], dest='provide_oob_def_gw')
        self.add_transition(trigger='enter_int_speed', source=[
         'connect_apic', 'provide_oob_def_gw'], dest='provide_int_speed')
        self.add_transition(trigger='enter_strong_passwd', source=[
         'connect_apic', 'provide_int_speed'], dest='provide_strong_passwd')
        self.add_transition(trigger='enter_admin_passwd', source=[
         'connect_apic', 'provide_strong_passwd'], dest='provide_admin_passwd')
        self.add_transition(trigger='reenter_admin_passwd', source=[
         'connect_apic', 'provide_admin_passwd'], dest='provide_admin_passwd')
        self.add_transition(trigger='enter_edit_cfg', source=[
         'connect_apic', 'provide_admin_passwd', 'provide_int_speed'], dest='provide_modify_config')
        self.add_transition(trigger='restart_setup', source='provide_modify_config', dest='provide_fabric_name')
        return
    ...
    def on_enter_connect_cimc(self):
        ...
        try:
            self.log(('Connecting to {0} as user {1} for CIMC control.').format(self.cimc, self.cimc_username), print_only=True)
            self.cimc_client.connect(hostname=self.cimc, username=self.cimc_username, password=self.cimc_password, look_for_keys=False)
            self.log(('Connecting to {0} as user {1} for APIC control.').format(self.cimc, self.cimc_username), print_only=True)
            self.apic_client.connect(hostname=self.cimc, username=self.cimc_username, password=self.cimc_password, look_for_keys=False)
        ...
        self.cimc_interact = WiperApicInteract(self.cimc_client, timeout=10, display=self.verbose, type='cimc')
        self.cimc_interact.send('\n')
        self.apic_interact = WiperApicInteract(self.apic_client, timeout=10, display=self.verbose, type='apic')
    ...
    def on_enter_check_sol(self):
        ...
        while True:
            ...
            try:
                sol_list = re.split('\\s*', self.cimc_interact.current_output_clean.split('\n')[2])
                ...
                if 'yes' not in sol_enabled or '115200' not in sol_baud or 'com0' not in sol_com:
                    ...
                    time.sleep(3)
                    self.log('Serial Over LAN is not configured, moving to configure it.', print_only=True)
                    self.sol_not_configured()
                else:
                    self.log('Serial Over LAN is configured.', print_only=True)
                    return
            except (KeyError, IndexError):
                self.log("The command output for 'show sol' was not valid, trying again.", print_only=True)

    def on_enter_configure_sol(self):
        sol_prompt = 'C220-.* /sol # '
        sol_needs_commit_prompt = 'C220-.* /sol \\*# '
        top_prompt = 'C220-.*# '
        cmds = list()
    ...
    def on_enter_connect_apic(self):
        transitions = {'.*login:.*': self.apic_login_detected, 
           '.*Password:.*': self.apic_password_detected, 
           '.*:~> ': self.apic_prompt_detected, 
           '.*Press any key to continue....*': self.press_any_key, 
           '.*Enter the fabric name \\[.*\\]:.*': self.enter_fabric_name, 
           '.*Enter the number of controllers in the fabric \\(1-9\\) \\[[0-9]+]:.*': self.enter_num_ctrlrs, 
           '.*Enter the controller ID \\(1-3\\) \\[[0-9]+\\]:.*': self.enter_ctrlr_id, 
           '.*Enter the controller name \\[.*\\]:.*': self.enter_ctrlr_name, 
           '.*Enter address pool for TEP addresses \\[.*\\]:.*': self.enter_tep_addr_pool, 
           '.*Enter the VLAN ID for infra network \\(1-4094\\).*:.*': self.enter_infra_vlan_id, 
           '.*Enter address pool for BD multicast addresses \\(GIPO\\) \\[.*\\]:.*': self.enter_bd_mc_addr_pool, 
           '.*Enter the IP address \\[.*\\].*': self.enter_oob_ip_addr, 
           '.*Enter the IP address of the default gateway \\[.*\\]:.*': self.enter_oob_def_gw, 
           '.*Enter the interface speed/duplex mode \\[.*\\]:.*': self.enter_int_speed, 
           '.*Enable strong passwords\\? \\[.*\\]:.*': self.enter_strong_passwd, 
           '.*Enter the password for admin:.*': self.enter_admin_passwd, 
           '.*Reenter the password for admin:.*': self.reenter_admin_passwd, 
           '.*Would you like to edit the configuration\\? \\(y/n\\) \\[.*\\].*': self.enter_edit_cfg}
        try:
            self.log('Trying to connect to the APIC console via Serial Over LAN, ' + 'using a timeout of 10 seconds.', print_only=True)
            index = self.do_cmd('connect host\n', transitions.keys(), self.apic_interact, clear_outputs=True)
    ...
    def on_enter_cycle_host(self):
        chassis_prompt = 'C220-.* /chassis # '
        power_cycle_prompt = '.*Do you want to continue\\?\\[.*\\].*'
        top_prompt = 'C220-.*# '
        cmds = list()
    ...
    def on_enter_disconnect_cimc(self):
        self.log('Disconnecting from both CIMC and the APIC by closing the connections.', print_only=True)
    ...
    def on_enter_login_apic(self):
        ...
        if index == 0:
            prompt = '.*~> .*'
            self.log('Rescue-user was prompted for a password, sending the APIC admin password.', print_only=True)
    ...
    def on_enter_eraseconfig(self):
        prompt = '.*Do you want to cleanup the initial setup data\\? The system will be ' + 'REBOOTED. \\(Y/n\\):.*'
        ...
        self.log("Sending 'Y' to continue with the eraseconfig setup, will wait for the reboot, " + 'timeout is 600 seconds.', print_only=True)
        self.do_cmd('Y', prompt, self.apic_interact, timeout=600)
        self.press_any_key()

    def on_enter_press_any_key(self):
        prompt = '.*Enter the fabric name \\[.*\\]:.*'
    ...
    def on_enter_provide_fabric_name(self):
        prompt = '.*Enter the number of controllers in the fabric \\(1-9\\) \\[[0-9]+]:.*'
        self.log(("Setting the fabric name to '{0}' on the APIC.").format(self.fabric_name), print_only=True)
    ...
    def on_enter_provide_number_ctrlrs(self):
        prompt = '.*Enter the controller ID \\(1-3\\) \\[[0-9]+\\]:.*'
        self.log(("Setting number of controllers to '{0}' on the APIC.").format(self.num_controllers), print_only=True)
    ...
    def on_enter_provide_ctrlr_id(self):
        ...
        self.log(("Setting the controller id to '{0}' on the APIC.").format(self.controller_id), print_only=True)
        self.do_cmd(self.controller_id, prompt, self.apic_interact)
        self.enter_ctrlr_name()

    def on_enter_provide_ctrlr_name(self):
        prompt = '.*Enter address pool for TEP addresses \\[.*\\]:.*'
        self.log(("Setting the controller name to '{0}' on the APIC.").format(self.controller_name), print_only=True)
    ...
    def on_enter_provide_tep_addr_pool(self):
        prompt = '.*Enter the VLAN ID for infra network \\(1-4094\\).*:.*'
        self.log(("Setting the TEP Address Pool to '{0}' on the APIC.").format(self.tep_address_pool), print_only=True)
    ...
    def on_enter_provide_infra_vlan_id(self):
        ...
        self.log(("Setting the infra VLAN ID to '{0}' on the APIC.").format(self.infra_vlan_id), print_only=True)
    ...
    def on_enter_provide_bd_mc_addr_pool(self):
        ...
        self.log(("Setting the BD Multicast Address Pool to '{0}' on the APIC").format(self.bd_mc_address_pool), print_only=True)
        self.do_cmd(self.bd_mc_address_pool, prompt, self.apic_interact)
        self.enter_oob_ip_addr()

    def on_enter_provide_oob_address(self):
        prompt = '.*Enter the IP address of the default gateway \\[.*\\]:.*'
        self.log(('Setting the Out Of Band IP address to {0} on the APIC.').format(self.oob_ip_addr), print_only=True)
        self.do_cmd(self.oob_ip_addr, prompt, self.apic_interact)
        self.enter_oob_def_gw()

    def on_enter_provide_oob_def_gw(self):
        prompt = '.*Enter the interface speed/duplex mode \\[.*\\]:.*'
        self.log(('Setting the Out Of Band default gateway to {0} ').format(self.oob_def_gw) + 'on the APIC', print_only=True)
    ...
    def on_enter_provide_int_speed(self):
        prompts = [
         ...
         '.*Would you like to edit the configuration\\? \\(y/n\\) \\[.*\\].*']
        self.log(('Setting the Out Of Band interface speed/duplex to {0} ').format(self.int_speed) + 'on the APIC.', print_only=True)
    ...
    def on_enter_provide_strong_passwd(self):
        ...
        self.log(("Sending '{0}' for enabling strong passwords ").format(self.strong_passwd) + 'on the APIC', print_only=True)
    ...
    def on_enter_provide_admin_passwd(self):
        ...
        if index == 0:
            prompt = '.*Would you like to edit the configuration\\? \\(y/n\\) \\[.*\\].*'
    ...
    def on_enter_provide_modify_config(self):
        if self.provided_fabric_name:
            self.log('Completed a full setup script attempt, waiting for the APIC login prompt ' + 'for up to 60 seconds.', print_only=True)
            self.do_cmd('n', '.*login:.*', self.apic_interact, timeout=60)
        else:
            prompt = '.*Enter the fabric name \\[.*\\]:.*'
            self.log(("Setting the fabric name to '{0}' on the APIC.").format(self.fabric_name), print_only=True)
    ...
    def do_cmds(self, cmd_list, interact, clear_outputs=True):
        ...
        for cmd_prompt in cmd_list:
            if len(cmd_prompt) == 2:
                self.do_cmd(cmd_prompt[0], cmd_prompt[1], interact, clear_outputs=clear_outputs)
            elif len(cmd_prompt) == 3:
                self.do_cmd(cmd_prompt[0], cmd_prompt[1], interact, clear_outputs=cmd_prompt[2])
            elif len(cmd_prompt) == 4:
                self.do_cmd(cmd_prompt[0], cmd_prompt[1], interact, clear_outputs=cmd_prompt[2], timeout=cmd_prompt[3])
            else:
                raise ValueError(('Invalid command tuple do_cmds {0}').format(cmd_prompt))
    ...
    def do_cmd(self, cmd, prompt, interact, clear_outputs=True, timeout=10):
        ...
        try:
            self.log(("Expecting prompt: '{0}' with a timeout of {1} seconds").format(prompt, timeout), debug_only=True)
...
def parse_args():
    ...
    parser.add_argument('-ap', '--apic_admin_password', required=False, default=None, help='The APIC admin user password to enter into the APIC setup script.')
    parser.add_argument('-b', '--bd-mc-addresses', required=False, default=None, help='The Bridge Domain Multicast address range to enter into the APIC ' + 'setup script.')
    parser.add_argument('cimc_ip', help='CIMC hostname or IP address used to ssh to CIMC')
    parser.add_argument('-cna', '--controller-name', required=False, default=None, help='The controller name to enter into the APIC setup script.')
    parser.add_argument('-cnu', '--controller_number', required=False, type=str, default=None, help='The controller number (id) to enter into the APIC setup script.')
    parser.add_argument('-cp', '--cimc_password', required=False, default=None, help='CIMC password')
    parser.add_argument('-cu', '--cimc_username', required=False, default=None, help='CIMC username')
    parser.add_argument('-i', '--ini-file', required=False, default='wiper.ini', help='Use an ini file to find parameters to provision an APIC.')
    parser.add_argument('-is', '--int-speed', required=False, default=None, choices=[
     'auto',
     '10baseT/Half',
     '10baseT/Full',
     '100baseT/Half',
     '100baseT/Full',
     '1000baseT/Full'])
    parser.add_argument('-iv', '--infra-vlan-id', required=False, default=None, help='The infra vlan id to enter into the APIC setup script.')
    parser.add_argument('-f', '--fabric-name', required=False, default=None, help='The fabric name to enter into the APIC setup script.')
    parser.add_argument('-nc', '--number-of-controllers', required=False, type=str, default=None, help='The number of controllers to enter into the APIC setup script.')
    parser.add_argument('-od', '--oob-default-gateway', required=False, default=None, help='The APIC Out-Of-Band default gateway to enter into the APIC setup ' + 'script.')
    parser.add_argument('-oi', '--oob-ip-address', required=False, default=None, help='The APIC Out-Of-Band IP address to enter into the APIC setup script.')
    parser.add_argument('-q', '--quiet', required=False, default='False', action='store_const', const='True', help='Be quiet, do not provide status messages')
    parser.add_argument('-sim', '--simulator', required=False, action='store_const', const='True', default='False', help='This flag identifies the APIC as a simulator.')
    parser.add_argument('-sp', '--strong-passwords', required=False, default=None, choices=[
     'Y', 'n'], help='Strong password option to enter into the APIC setup script.')
    parser.add_argument('-t', '--tep-address-pool', required=False, default=None, help='The TEP address pool to enter into the APIC setup script.')
    parser.add_argument('-v', '--verbose', required=False, default='False', action='store_const', const='True', help='Enable debugging and be verbose.')
    ...
    for option_name in required_options:
        try:
            ...
            print ('Unable to complete provisioning.  Missing --{0} option').format(option_name.replace('_', '-'))
            print ''
