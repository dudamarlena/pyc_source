...
# [GCC 8.4.0]
# Embedded file name: /Users/twaldear/Sites/secure-headers-root/flask_secure_headers/core.py
...
class Secure_Headers:
    ...
    def __init__(self):
        ...
        self.defaultPolicies = {'CSP': {'default-src': [
                                 'self'], 
                   'script-src': [], 'img-src': [], 'object-src': [], 'plugin-src': [], 'style-src': [], 'media-src': [], 'child-src': [], 'connect-src': [], 'base-uri': [], 'font-src': [], 'form-action': [], 'frame-ancestors': [], 'plugin-types': [], 'referrer': [], 'reflected-xss': [], 'sandbox': [], 'report-uri': [
                                '/csp_report']}, 
           'HSTS': {'max-age': 31536000, 
                    'includeSubDomains': True, 
                    'preload': False}, 
           'HPKP': {'max-age': 5184000, 
                    'includeSubDomains': True, 
                    'report-uri': '/hpkp_report', 
                    'pins': []}, 
           'X_Frame_Options': {'value': 'sameorigin'}, 
           'X_XSS_Protection': {'value': 1, 
                                'mode': 'block'}, 
    ...
    def _getHeaders(self, updateParams=None):
        ...
        return [ globals()[k](v).create_header() for k, v in policies.items() if v is not None
               ]
