...
# [GCC 8.4.0]
# Embedded file name: /Users/twaldear/Sites/secure-headers-root/flask_secure_headers/tests/core_test.py
# Compiled at: 2015-05-20 19:20:29
import unittest
...
class TestAppUseCase(unittest.TestCase):
    ...
    def test_update_function(self):
        ...
        self.sh.update({'X_Permitted_Cross_Domain_Policies': {'value': 'all'}, 'CSP': {'script-src': ['self', 'code.jquery.com']}, 'HPKP': {'pins': [{'sha256': 'test123'}, {'sha256': 'test2256'}]}})
    ...
    def test_rewrite_function(self):
        ...
        self.sh.rewrite({'CSP': {'default-src': ['none']}, 'HPKP': {'pins': [{'sha256': 'test123'}]}})
    ...
    def test_wrapper_update_function(self):
        ...
        @self.app.route('/')
        @self.sh.wrapper({'CSP': {'script-src': ['self', 'code.jquery.com']}, 'X_Permitted_Cross_Domain_Policies': {'value': 'none'}, 'X-XSS-Protection': {'value': 1, 'mode': False}, 'HPKP': {'pins': [{'sha256': 'test2256'}]}})
        def index():
            return 'hi'
