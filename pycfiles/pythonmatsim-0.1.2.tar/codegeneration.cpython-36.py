...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/matsim-eth/python-matsim/buildutils/codegeneration.py
# Compiled at: 2019-08-26 14:55:37
# Size of source mod 2**32: 8377 bytes
import xml.etree.ElementTree as et, os, subprocess, atexit, signal
from typing import List, Tuple
import logging, pkg_resources, setuptools, shutil, tempfile
...
class JvmConfig:
    ...
    def add_dependency(self, artifact_id: str, version: str='${matsim.version}', group_id: str='org.matsim.contrib'):
    ...
    def add_repository(self, id: str, url: str):
            ...
            with open(pom_file, 'w') as (out):
                for template_line in template:
                    ...
                    if '<!-- PROPERTIES HERE -->' in template_line:
                        ...
                        if '<!-- ADDITIONAL DEPENDENCIES HERE -->' in template_line:
                            lines = self._dependencies
                        else:
                            if '<!-- ADDITIONAL REPOSITORIES HERE -->' in template_line:
                                lines = self._repositories
                    out.writelines(lines)

    def build_and_start_jvm(self, maven_dir: str, code_dir: str, root_package: str, jvm_path=None):
        import jpype
        if jvm_path is None:
        ...
        maven_completion = subprocess.run(['mvn', '-DskipTests=true', 'assembly:assembly'], cwd=maven_dir)
...
class JavaAdapterCodeGenerationCommand(setuptools.Command):
    ...
    def run(self):
        ...
        with tempfile.TemporaryDirectory() as (tmp):
            ...
            os.mkdir(maven_dir)
            build_and_start_jvm(maven_dir, java_gen_dir, 'javawrappers')
            ...
            for f in os.listdir(target_dir):
                if f.endswith('jar-with-dependencies.jar'):
                    shutil.copy(os.path.join(target_dir, f), 'javaresources')