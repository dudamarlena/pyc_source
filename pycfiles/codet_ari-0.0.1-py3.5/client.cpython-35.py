...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ari/client.py
# Compiled at: 2016-11-21 07:55:49
# Size of source mod 2**32: 11623 bytes
"""ARI client library.
"""
import json, urllib.parse, swaggerpy.client
from ari.model import *
log = logging.getLogger(__name__)

class Client(object):
    __doc__ = 'ARI Client object.\n\n    :param base_url: Base URL for accessing Asterisk.\n    :param http_client: HTTP client interface.\n    '
    ...
    def __init__(self, base_url, http_client):
        ...
        self.repositories = {name:Repository(self, name, api) for name, api in list(self.swagger.resources.items())}
        events = [api['api_declaration'] for api in self.swagger.api_docs['apis'] if api['name'] == 'events']
        if events:
        ...
        self.exception_handler = lambda ex: log.exception('Event listener threw exception')
    ...
    def on_event(self, event_type, event_cb, *args, **kwargs):
        ...
        callback_obj = (
         event_cb, args, kwargs)
        listeners.append(callback_obj)
        client = self

        class EventUnsubscriber(object):
            __doc__ = 'Class to allow events to be unsubscribed.\n            '
    ...
    def on_object_event(self, event_type, event_cb, factory_fn, model_id, *args, **kwargs):
        ...
        obj_fields = [k for k, v in list(event_model['properties'].items()) if v['type'] == model_id]
        if not obj_fields:
        ...
        def extract_objects(event, *args, **kwargs):
            ...
            :param kwargs: Keyword arguments to pass to the event
                                      callback
            """
            obj = {obj_field:factory_fn(self, event[obj_field]) for obj_field in obj_fields if event.get(obj_field)}
            if len(obj_fields) == 1:
                if obj:
    ...
    def on_live_recording_event(self, event_type, fn, *args, **kwargs):
        ...
        :param kwargs: Keyword arguments to pass to fn
        """
        return self.on_object_event(event_type, fn, LiveRecording, 'LiveRecording', *args, **kwargs)
    ...
    def on_stored_recording_event(self, event_type, fn, *args, **kwargs):
        ...
        :param kwargs: Keyword arguments to pass to fn
        """
        return self.on_object_event(event_type, fn, StoredRecording, 'StoredRecording', *args, **kwargs)
    ...
    def on_device_state_event(self, event_type, fn, *args, **kwargs):
        ...
        :param kwargs: Keyword arguments to pass to fn
        """
        return self.on_object_event(event_type, fn, DeviceState, 'DeviceState', *args, **kwargs)
