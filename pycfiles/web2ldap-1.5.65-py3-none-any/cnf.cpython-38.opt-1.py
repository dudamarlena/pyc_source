...
# Size of source mod 2**32: 7062 bytes
"""
...
class Web2LDAPConfig(LogHelper):
    ...
    def clone(self, **params):
        ...
        """
        old_params = dict([(
         param_name, getattr(self, param_name)) for param_name in VALID_CFG_PARAM_NAMES if hasattr(self, param_name)])
        new = Web2LDAPConfig(**old_params)
        new.update(params)
        self.log(logging.DEBUG, 'Cloned config %s with %d parameters to %s with %d new params %s', id(self), len(old_params), id(new), len(params), params)
        return new
