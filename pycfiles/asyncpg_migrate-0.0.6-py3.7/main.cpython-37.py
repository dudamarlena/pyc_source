...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/asyncpg_migrate/main.py
# Compiled at: 2020-01-13 09:51:39
# Size of source mod 2**32: 4689 bytes
import asyncio
from pathlib import Path
import sys, typing as t, asyncpg, click
...
try:
    ...
    def async_run(coro: t.Awaitable[_T]) -> _T:
...
@click.pass_context
def db(ctx: click.Context, config: Path, verbose: int) -> None:
    ...
    logger.debug('Flags are config={config}, verbose={verbose}',
      config=config,
      verbose=verbose)
...
def upgrade_cmd(ctx: click.Context, revision: str) -> None:
    ...
    async def _runner():
        ...
        return await upgrade.run(config=config,
...
def downgrade_cmd(ctx: click.Context, revision: str) -> None:
    ...
    async def _runner():
        config = loader.load_configuration(ctx.obj['configuration_file_path'])
        return await downgrade.run(config=config,
          target_revision=revision,
          connection=(await asyncpg.connect(dsn=(config.database_dsn))))
...
def revision_cmd(ctx: click.Context) -> None:
    ...
    async def _runner():
        config = loader.load_configuration(ctx.obj['configuration_file_path'])
        return await migration.latest_revision(connection=(await asyncpg.connect(dsn=(config.database_dsn))))

    db_revision = async_run(_runner())
    if db_revision is None:
        raise click.ClickException('No revisions found, you might want to run some migrations first :)')
    else:
...
def history(ctx: click.Context) -> None:
    ...
    async def _runner(cfg: model.Config) -> model.MigrationHistory:
        return await migration.list(connection=(await asyncpg.connect(dsn=(cfg.database_dsn))))
    ...
    if not mig_history:
        ...
        click.echo(tabulate([[m.revision, m.label, m.timestamp, m.direction] for m in mig_history],
          headers=[
         'No.', 'Revision', 'Lable', 'TS', 'Direction'],
          showindex='1'))
