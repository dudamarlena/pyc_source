...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python2.7/site-packages/landlab/utils/structured_grid.py
# Compiled at: 2015-02-11 19:25:27
import numpy as np, itertools
from ..grid.base import CORE_NODE, FIXED_VALUE_BOUNDARY, FIXED_GRADIENT_BOUNDARY, TRACKS_CELL_BOUNDARY, CLOSED_BOUNDARY, BAD_INDEX_VALUE
...
def boundary_iter(shape):
    ...
    """
    return itertools.chain(bottom_index_iter(shape), left_right_iter(shape, 1, shape[0] - 1), top_index_iter(shape))
...
def node_index_at_link_ends(shape):
    ...
    node_ids.shape = shape
    return (
     node_index_at_link_head(node_ids),
...
def face_index_at_links(shape, actives=None, inactive_link_index=BAD_INDEX_VALUE):
    """
...
def active_links(shape, node_status_array=None, link_nodes=None):
    ...
    *node_status_array* keyword to specify the status for each of the grid's nodes. If not given, each of the
    ...
    active_links = (from_node_status == CORE_NODE) & ~(to_node_status == CLOSED_BOUNDARY) | (to_node_status == CORE_NODE) & ~(from_node_status == CLOSED_BOUNDARY)
...
def horizontal_link_ids(shape):
    ...
    link_ids = np.arange(link_id_offset, link_id_offset + horizontal_link_count(shape), dtype=np.int)
...
def horizontal_active_link_ids(shape, node_status=None):
    if node_status is None:
        ...
        link_ids = np.arange(link_id_offset, link_id_offset + horizontal_active_link_count(shape), dtype=np.int)
    else:
        ...
        link_ids = np.empty(inactive_links.size)
        link_ids[inactive_links] = -1
        link_ids[~inactive_links] = np.arange(link_id_offset, link_id_offset + active_link_count, dtype=np.int)
    link_ids.shape = (
     shape[0] - 2, shape[1] - 1)
    return link_ids
...
def active_west_links(shape, node_status=None):
    ...
    try:
        active_west_links[1:-1, 1:] = horizontal_active_link_ids(shape, node_status=node_status)
    except ValueError:
        pass
...
def active_east_links(shape, node_status=None):
    ...
    try:
        active_east_links[1:-1, :-1] = horizontal_active_link_ids(shape, node_status=node_status)
    except ValueError:
        pass
...
def node_index_with_halo(shape, halo_indices=BAD_INDEX_VALUE):
    ...
    interiors, boundaries = interior_nodes(shape_with_halo), boundary_nodes(shape_with_halo)
...
def _neighbor_node_ids(ids_with_halo):
    shape = (
     ids_with_halo.shape[0] - 2, ids_with_halo.shape[1] - 2)
...
def _centered_node_ids(ids_with_halo):
    shape = (
     ids_with_halo.shape[0] - 2, ids_with_halo.shape[1] - 2)
    kwds = {'strides': ids_with_halo.strides, 'buffer': ids_with_halo, 
       'dtype': ids_with_halo.dtype}
...
def linked_neighbor_node_ids(shape, closed_boundary_nodes, open_boundary_nodes=[], inactive=BAD_INDEX_VALUE):
...
def _set_open_boundary_neighbors(neighbors, open_boundary_nodes, value):
    ...
    is_open_boundary_neighbor = _find_open_boundary_neighbors(neighbors, open_boundary_nodes)
...
def diagonal_node_array(shape, out_of_bounds=BAD_INDEX_VALUE, contiguous=True, boundary_node_mask=None):
    """
...
def diagonal_array_slow(shape, out_of_bounds=BAD_INDEX_VALUE):
    ...
    for r in xrange(1, nrows - 1):
        for c in xrange(1, ncols - 1):
            cell_id = r * ncols + c
            diagonal_cells[(cell_id, 2)] = cell_id - ncols - 1
            diagonal_cells[(cell_id, 0)] = cell_id + ncols + 1
            diagonal_cells[(cell_id, 3)] = cell_id - ncols + 1
            diagonal_cells[(cell_id, 1)] = cell_id + ncols - 1

    return diagonal_cells


def has_boundary_neighbor(neighbors, diagonals, out_of_bounds=BAD_INDEX_VALUE):
    """
...
def nodes_around_points_on_unit_grid(shape, coords, mode='raise'):
    ...
    try:
        return np.ravel_multi_index(((rows, rows + 1, rows + 1, rows),
         (
          cols, cols, cols + 1, cols + 1)), shape, mode=mode).T
...
def nodes_around_points(shape, coords, spacing=(1.0, 1.0), origin=(0.0, 0.0)):
    ...
    *spacing*, and *origin*.
    ...
    """
    try:
        return nodes_around_points_on_unit_grid(shape, (
