...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python2.7/site-packages/landlab/grid/structured_quad/rectilinear.py
...
class RectilinearGrid(StructuredQuadGrid):
    ...
    def __init__(self, coord):
        ...
        shape = (
         len(coord[0]),
         len(coord[1]))
        node_coord = np.meshgrid(indexing='ij', *coord)
...
class RasterGrid(UniformRectilinearGrid):
    ...
    def __init__(self, shape, spacing=1.0, origin=0.0):
        ...
        origin : tuple, optional
            Coordinates of grid origin.
        """
        super(RasterGrid, self).__init__(shape, origin=origin, spacing=(
         spacing,) * len(shape))