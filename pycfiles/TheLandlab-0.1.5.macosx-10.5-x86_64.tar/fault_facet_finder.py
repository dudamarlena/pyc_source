...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python2.7/site-packages/landlab/utils/fault_facet_finder.py
...
class find_facets(object):
    ...
    def __init__(self, grid, elev_field='topographic_elevation', fault_azimuth=None):
        """
    ...
    def define_aspect_node_subset(self, angle_tolerance=5.0):
        ...
        *angle_tolerance* is the angle in degrees that the aspect must be within
        ...
        five_degrees = np.pi / 180.0 * angle_tolerance
        ...
        if (perp_to_az + np.pi + five_degrees) // (2.0 * np.pi):
    ...
    def define_aspect_node_subset_local(self, dist_tolerance=4.0, angle_tolerance=15.0, dip_dir='E'):
        """
        """
        grid = self.grid
        try:
        ...
        for i in self.ft_trace_node_ids:
            grid.get_distances_of_nodes_to_point((grid.node_x[i], grid.node_y[i]), node_subset=grid.core_nodes[subset], get_az='angles', out_distance=new_distance_to_ft, out_azimuth=new_angle_to_ft)
        ...
        condition = np.less(np.fabs(divergence_at_ft), grid.node_spacing * dist_tolerance)
        diff_angles = np.min([np.fabs(angle_to_ft - self.aspect[subset]), np.fabs(np.fabs(angle_to_ft - self.aspect[subset]) - 2.0 * np.pi)], axis=0)
        ...
        figure(6)
        gridshow.imshow_core_node_grid(self.grid, np.where(self.diff_angles < 100000.0, self.diff_angles, -1.0))
    ...
    def find_coherent_facet_patches(self, tolerance=3.0, threshold_num_px=12):
        ...
        for i in self.possible_core_nodes:
            ...
            while 1:
                possible_neighbors = np.union1d(self.grid.get_neighbor_list(nodes_in_patch).flat, self.possible_core_nodes)
    ...
    def find_slope_lines(self, tolerance=1.0):
        ...
        unique_starting_pts = np.unique(self.closest_ft_node[pcn])
        profile_ft_node_id = []
        profile_ft_node_x = []
        profile_ft_node_y = []
        profile_ft_node_z = []
        ...
        for i in unique_starting_pts:
            ...
            ft_pt_distances_to_node = self.grid.get_distances_of_nodes_to_point((grid.node_x[i], grid.node_y[i]), node_subset=self.ft_trace_node_ids)
            close_ft_nodes = np.less(ft_pt_distances_to_node, 5.0 * grid.node_spacing)
            ...
            if nodes_possible.size > 10.0:
                ...
                if patch_size > 10.0:
                    ...
                    end_pt_of_cluster = locs_of_large_diffs[(biggest_interval_loc + 1)] + 1
