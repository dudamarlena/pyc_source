...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python2.7/site-packages/landlab/io/tests/test_write_esri_ascii.py
...
def test_grid_with_no_fields():
    grid = RasterModelGrid(4, 5, dx=2.0)
    with cdtemp() as (_):
...
def test_names_keyword_multiple_names():
    ...
    grid.add_field('node', 'land_surface__elevation', np.arange(20.0))
    with cdtemp() as (_):
        files = write_esri_ascii('test.asc', grid, names=[
         'air__temperature',
...
def test_names_keyword_with_bad_name():
    ...
    grid.add_field('node', 'air__temperature', np.arange(20.0))
    with cdtemp() as (_):
        assert_raises(ValueError, write_esri_ascii, 'test.asc', grid, names='not_a_name')
...
def test_clobber_keyword():
    ...
    with cdtemp() as (_):
        ...
        assert_raises(ValueError, write_esri_ascii, 'test.asc', grid)
        assert_raises(ValueError, write_esri_ascii, 'test.asc', grid, clobber=False)
