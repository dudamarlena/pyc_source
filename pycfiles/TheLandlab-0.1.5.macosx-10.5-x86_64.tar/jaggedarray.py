...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python2.7/site-packages/landlab/utils/jaggedarray.py
...
class JaggedArray(object):
    ...
    def __init__(self, *args):
        ...
        """
        if len(args) == 1:
            values, values_per_row = np.concatenate(args[0]), [ len(row) for row in args[0] ]
        else:
    ...
    def empty_like(jagged, dtype=None):
        return JaggedArray(np.empty_like(jagged.array, dtype=dtype), np.diff(jagged.offset))
