...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python2.7/site-packages/landlab/components/flexure/funcs.py
...
def get_flexure_parameter(h, E, n_dim):
    ...
    if n_dim == 2:
        alpha = pow(D / (rho_m * _GRAVITY), 0.25)
    else:
        alpha = pow(4.0 * D / (rho_m * _GRAVITY), 0.25)
    return alpha
...
def _calculate_deflections(load, locs, coords, alpha, out=None):
    c = -load / (2.0 * np.pi * _RHO_MANTLE * _GRAVITY * pow(alpha, 2.0))
...
def subside_point_load(load, loc, coords, params=None, out=None):
    ...
    *x* and *y* are the x and y coordinates of each node of the solution
    ...
    5.315e-07
    """
    params = params or dict(eet=6500.0, youngs=70000000000.0)
    eet, youngs = params['eet'], params['youngs']
    assert len(loc) in (1, 2)
    assert len(coords) == len(loc)
    if not len(coords[0].shape) == 1:
        raise AssertionError
        load = isinstance(load, (int, float, np.ndarray)) or np.array(load)
    if out is None:
    ...
    if len(loc) == 2:
        ...
        c = load / (2.0 * alpha * _RHO_MANTLE * _GRAVITY)
...
def subside_point_loads(loads, locs, coords, params=None, deflection=None, n_procs=1):
    ...
    if n_procs > 1:
        _subside_in_parallel(deflection, loads, locs, coords, eet, youngs, n_procs=n_procs)
    else:
        for index in loads.nonzero()[0]:
            loc = [ dim.flat[index] for dim in locs ]
            deflection += subside_point_load(loads.flat[index], loc, coords, eet, youngs)
...
def _subside_in_parallel(dz, loads, locs, coords, eet, youngs, n_procs=4):
    ...
    for index in loads.nonzero()[0]:
        loc = (
         locs[0].flat[index], locs[1].flat[index])
