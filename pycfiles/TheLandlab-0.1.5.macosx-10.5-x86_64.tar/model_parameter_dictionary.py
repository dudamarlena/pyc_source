...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python2.7/site-packages/landlab/core/model_parameter_dictionary.py
...
"""
import warnings, types, scipy.io
from numpy import *
import numpy as np
...
def _to_bool(string):
    ...
    if upper in _VALID_TRUE_VALUES:
        return True
    if upper in _VALID_FALSE_VALUES:
        return False
    raise ValueError('invalid literal for _to_bool()')


_CONVERT_FROM_STR = {'float': float, 
   'int': int, 
   'str': str, 
   'bool': _to_bool}
...
class ModelParameterDictionary(dict):
    ...
    def get(self, key, *args, **kwds):
        ...
        *bool*. This will not work as the Python *bool* function does not
        ...
        assert len(kwds) == 0
        if ptype is bool:
            warnings.warn("Using bool function to convert value. This probably isn't what you want. Use ptype='bool' instead.", RuntimeWarning)
    ...
    def read_int(self, key, *args):
        ...
        not an integer.
        """
        return self.get(key, ptype=int, *args)
    ...
    def read_bool(self, key):
        ...
        if my_value.upper() in _VALID_TRUE_VALUES:
            return True
        if my_value.upper() in _VALID_FALSE_VALUES:
            return False
