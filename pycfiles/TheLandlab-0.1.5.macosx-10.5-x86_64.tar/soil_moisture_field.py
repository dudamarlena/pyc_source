...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python2.7/site-packages/landlab/components/soil_moisture/soil_moisture_field.py
...
class SoilMoisture(Component):
    ...
    def __init__(self, grid, **kwds):
        ...
        self._interception_cap = kwds.pop('INTERCEPT_CAP', 1.0)
    ...
    def update(self, current_time, **kwds):
        ...
        for cell in range(0, self.grid.number_of_cells):
            s = self._SO[cell]
            Inf_cap = self._soil_Ib * (1 - self._vegcover[cell]) + self._soil_Iv * self._vegcover[cell]
            Int_cap = min(self._vegcover[cell] * self._interception_cap, P)
            ...
            if sini >= fc:
                ...
                if Tb < tfc:
                    s = abs(sini - 1 / beta * np.log(((nu - mu + mu * np.exp(beta * (sini - fc))) * np.exp(beta * (nu - mu) * Tb) - mu * np.exp(beta * (sini - fc))) / (nu - mu)))
                ...
                self._D[cell] = 0
                self._ETA[cell] = 1000 * ZR * pc * (sini - s)
            self._water_stress[cell] = min(max(((sc - (s + sini) / 2.0) / (sc - wp)) ** 4.0, 0.001), 1.0)
            self._S[cell] = s
            self._SO[cell] = s
