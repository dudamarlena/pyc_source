...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python2.7/site-packages/landlab/grid/unstructured/base.py
...
class BaseGrid(object):
    ...
    def __init__(self, nodes, axis_name=None, axis_units=None, node_status=None, links=None, cells=None):
        ...
        if links is not None and node_status is not None:
            ...
            self._active_link_grid = BaseGrid.create_active_link_grid(self.status_at_node, links, self.number_of_nodes)
        return

    @staticmethod
    def create_active_link_grid(node_status, links, number_of_nodes):
        active_link_ids = find_active_links(node_status, links)
        return LinkGrid((links[0][active_link_ids], links[1][active_link_ids]), number_of_nodes, link_ids=active_link_ids)
    ...
    def status_at_node(self, status):
        ...
        self._active_link_grid = BaseGrid.create_active_link_grid(self.status_at_node, (self.node_at_link_start,
    ...
    def node_to_node_distance(self, node0, node1, out=None):
        ...
        array([ 0.,  3.,  4.,  5.])
        """
        return point_to_point_distance(self._get_coord_at_node(node0), self._get_coord_at_node(node1), out=out)
        node0, node1 = np.broadcast_arrays(node0, node1)
        return np.sqrt(np.sum((self.coord_at_node[:, node1] - self.coord_at_node[:, node0]) ** 2, axis=0))
