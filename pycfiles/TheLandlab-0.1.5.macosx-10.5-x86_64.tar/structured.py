...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python2.7/site-packages/landlab/grid/structured_quad/structured.py
...
class StructuredQuadGrid(BaseGrid):
    ...
    def __init__(self, node_coord, shape=None, axis_name=None, axis_units=None, links=True, cells=True, node_status=None):
        """
        Parameters
        ----------
        node_coord : tuple
        ...
        if node_status is None:
            self._status = nodes.status_with_perimeter_as_boundary(self.shape, status_on_perimeter=FIXED_VALUE_BOUNDARY)
        else:
        ...
        BaseGrid.__init__(self, (node_coord[0].flatten(),
         node_coord[1].flatten()), links=link_grid, cells=cell_grid)
