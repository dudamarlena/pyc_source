...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python2.7/site-packages/landlab/components/diff_2d.py
# Compiled at: 2014-09-23 12:37:24
import numpy as np
from scipy import ndimage
from landlab import Implements
from landlab.framework.interfaces import BmiBase, BmiUniformRectilinear, BadVarNameError, GRID_TYPE_UNIFORM
...
class Diff2D(object):
    ...
    def _set_bc(data):
        ...
        data[-1, :] = 0.25 * top_column ** 2 - (np.arange(top_column + 1) - 0.5 * top_column) ** 2
    ...
    def initialize(self, file_name=None):
        ...
        if file_name is not None:
            ...
            self._time_step, row_count, column_count = float(vals[0]), int(vals[1]), int(vals[2])
            self._shape = (
        ...
        self._stencil = np.array([[0.0, column_spacing ** 2, 0.0], [row_spacing ** 2, 0.0, row_spacing ** 2], [0.0, column_spacing ** 2, 0.0]]) / (2.0 * (row_spacing ** 2 + column_spacing ** 2)) * self._time_step
        return
    ...
    def update(self):
        ...
        """
        ndimage.convolve(self._elevation, self._stencil, output=self._temp_elevation)
