...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python2.7/site-packages/landlab/components/transport_limited_fluvial/tl_fluvial_monodirectional.py
...
class TransportLimitedEroder(object):
        ...
        ***Parameters for input file***
        ...
        try:
            ...
            try:
                ...
                self.shields_prefactor_noD = 1.0 / ((self.sed_sensity - self.fluid_density) * self.g)
    ...
    def sed_capacity_equation(self, grid, shields_stress, slopes_at_nodes=None, areas_at_node=None):
        ...
        if self.Qc == 'MPM':
            ...
            try:
                capacity = self.MPM_prefactor * (shields_stress - thresh).clip(0.0) ** 1.5
            except AttributeError:
                capacity = 8.0 * self.C_MPM * np.sqrt(self.relative_weight * self.Dchar * self.Dchar * self.Dchar) * (shields_stress - thresh).clip(0.0) ** 1.5
        ...
        capacity *= self.k_w * areas_at_node ** self._b * 31557600.0
        return capacity
    ...
    def sed_capacity_equation_alt_method(self, grid, shear_stress, slopes_at_nodes=None, areas_at_node=None):
        ...
        if self.lamb_flag:
            ...
            thresh = self.shields_crit
        if self.Qc == 'MPM':
            capacity = self.MPM_prefactor_alt * (shear_stress - self.excess_SG * self.Dchar * self.rho_g * thresh).clip(0.0)
        elif type(self.Qc) == str:
            try:
                capacity = grid.at_node[self.Qc]
            except:
    ...
    def weave_iter_sed_trp_balance(self, grid, capacity, r, s, dt):
        ...
        sed_into_cell = np.zeros(capacity.size, dtype=np.float64)
        ...
        try:
            ...
            for donor in s[::-1]:
                rcvr = r[donor]
                dz[donor] = (sed_into_cell[donor] - capacity[rcvr] * dt) / cell_areas[donor]
                if donor != rcvr:
                    sed_into_cell[rcvr] += capacity[donor]

        return dz

    def erode(self, grid, dt, node_drainage_areas='drainage_area', slopes_at_nodes=None, link_slopes=None, link_node_mapping='links_to_flow_receiver', receiver='flow_receiver', upstream_order='upstream_ID_order', slopes_from_elevs=None, W_if_used=None, Q_if_used=None, Dchar_if_used=None, io=None):
        """
        ...
        *Dchar_if_used* must be set as a grid field string or nnoodes-long array
        ...
        if slopes_at_nodes == None:
            ...
            try:
                s_in = np.delete(s_in, np.where(np.in1d(s_in, np.where(capacity.mask), assume_unique=True)))
            except AttributeError:
        ...
        return (
         grid, grid.at_node[elev_name], capacity)