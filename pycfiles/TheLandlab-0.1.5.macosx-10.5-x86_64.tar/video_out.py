...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python2.7/site-packages/landlab/plot/video_out.py
...
class VideoPlotter(object):
    ...
    def initialize(self, grid, data_centering, start, stop, step):
        ...
        *data_centering* controls the type of data the video will be plotting.
        ...
        self.fig = plt.figure(self.randomized_name)
        return
    ...
    def add_frame(self, grid, data, elapsed_t, **kwds):
        ...
        if self.step_control_tuple[0] <= elapsed_t < self.step_control_tuple[1]:
            if not self.step_control_tuple[2]:
    ...
    def produce_video(self, interval=200, repeat_delay=2000, filename='video_output.gif', override_min_max=None):
        ...
        self.min_limit = override_min_max or np.amin(self.data_list[0])
        self.max_limit = np.amax(self.data_list[0])
        if not len(self.data_list) > 1:
            raise AssertionError('You must include at least two frames to make an animation!')
    ...
    def _yield_image(self):
        ...
        for i in self.data_list:
            yield (
             i, self.plotfunc, (self.min_limit, self.max_limit), self.grid, self.kwds)
