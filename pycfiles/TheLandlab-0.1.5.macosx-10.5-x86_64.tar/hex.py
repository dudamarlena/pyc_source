...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python2.7/site-packages/landlab/grid/hex.py
...
class HexModelGrid(VoronoiDelaunayGrid):
    ...
    def __init__(self, base_num_rows=0, base_num_cols=0, dx=1.0, orientation='horizontal', shape='hex', reorient_links=False, **kwds):
        ...
        if base_num_rows * base_num_cols > 0:
            self._initialize(base_num_rows, base_num_cols, dx, orientation, shape, reorient_links)
        super(HexModelGrid, self).__init__(**kwds)

    def _initialize(self, base_num_rows, base_num_cols, dx, orientation, shape, reorient_links=False):
        r"""
        ...
        if self._DEBUG_TRACK_METHODS:
            print 'HexModelGrid._initialize(' + str(base_num_rows) + ', ' + str(base_num_cols) + ', ' + str(dx) + ')'
        assert orientation[0].lower() == 'h' or orientation[0].lower() == 'v', 'orientation must be either "horizontal" (default) or "vertical"'
        assert shape[0].lower() == 'h' or shape[0].lower() == 'r', 'shape must be either "hex" (default) or "rect"'
    ...
    def _setup_cell_areas_array(self):
        ...
        .. math::
            ...
            A = 3 dx^2 / 2 \\sqrt{3} \x07pprox 0.866 dx^2
        """
    ...
    def configure_hexplot(self, data, data_label=None, color_map=None):
        ...
        if self.node_y[0] == self.node_y[1]:
