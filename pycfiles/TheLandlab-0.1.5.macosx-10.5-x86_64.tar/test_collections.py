...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python2.7/site-packages/landlab/tests/test_collections.py
...
try:
    from nose.tools import assert_dict_equal, assert_tuple_equal, assert_list_equal
except ImportError:
    from landlab.testing.tools import assert_dict_equal, assert_tuple_equal, assert_list_equal
...
def test_2_components_find_connections():
    palette = Palette(one=Sample1, two=Sample2)
    connections = {'one': {'deposition__rate': ['two']}, 'two': {'air__temperature': ['one'], 'surface__elevation': [
                                    'one']}}
