...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python2.7/site-packages/landlab/components/radiation/radiation_field.py
...
class Radiation(Component):
    ...
    def __init__(self, grid, **kwds):
        ...
        self._slope, self._aspect = grid.calculate_slope_aspect_at_nodes_Burrough(vals='Elevation')
        self._cell_values['Slope'] = self._slope
        self._cell_values['Aspect'] = self._aspect

    def update(self, current_time, **kwds):
        self._t = kwds.pop('Hour', 12.0)
        ...
        self._julian = np.floor((current_time - np.floor(current_time)) * 365.25)
        self._phi = np.radians(self._latitude)
        self._delta = 23.45 * np.radians(np.cos(2 * np.pi / 365 * (172 - self._julian)))
        self._tau = (self._t + 12.0) * np.pi / 12.0
        self._alpha = np.arcsin(np.sin(self._delta) * np.sin(self._phi) + np.cos(self._delta) * np.cos(self._phi) * np.cos(self._tau))
        if self._alpha <= 0.25 * np.pi / 180.0:
            self._alpha = 0.25 * np.pi / 180.0
        self._Rgl = self._Io * np.exp(-1 * self._n * (0.128 - 0.054 * np.log10(1 / np.sin(self._alpha))) * (1 / np.sin(self._alpha)))
        self._phisun = np.arctan(-np.sin(self._tau) / (np.tan(self._delta) * np.cos(self._phi) - np.sin(self._phi) * np.cos(self._tau)))
        if self._phisun >= 0 and -np.sin(self._tau) <= 0:
            self._phisun = self._phisun + np.pi
        elif self._phisun <= 0 and -np.sin(self._tau) >= 0:
            self._phisun = self._phisun + np.pi
        self._flat = np.cos(np.arctan(0)) * np.sin(self._alpha) + np.sin(np.arctan(0)) * np.cos(self._alpha) * np.cos(self._phisun - 0)
        self._Rsflat = self._Rgl * self._flat
        self._Rnetflat = (1 - self._A) * (1 - 0.65 * self._N ** 2) * self._Rsflat
        self._sloped = np.cos(self._slope) * np.sin(self._alpha) + np.sin(self._slope) * np.cos(self._alpha) * np.cos(self._phisun - self._aspect)
