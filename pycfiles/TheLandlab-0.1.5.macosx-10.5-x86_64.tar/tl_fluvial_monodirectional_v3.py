...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python2.7/site-packages/landlab/components/transport_limited_fluvial/tl_fluvial_monodirectional_v3.py
    ...
    def erode(self, grid, dt=None, node_elevs='topographic_elevation', node_drainage_areas='drainage_area', node_receiving_flow='flow_receiver', node_order_upstream='upstream_ID_order', node_slope='steepest_slope', steepest_link='links_to_flow_receiver', runoff_rate_if_used=None, stability_condition='loose', Dchar_if_used=None, io=None):
        ...
        ***Note the time units are SECONDS in these fields***
        ...
        if runoff_rate_if_used != None:
            ...
            square_link_length = np.square(link_length)
            try:
            ...
            while 1:
                ...
                if stability_condition == 'tight':
                    ...
                    tstep_each_node = 10.0 * square_link_length / mock_diffusivities
                    delta_t_internal = np.nanmin(tstep_each_node)
                    if delta_t_internal == np.inf:
                ...
                if stability_condition == 'loose':
                    ...
                    most_flattened_nodes = np.take(grid.core_nodes, most_flattened_nodes)
                    most_flattened_val = np.take(node_flattening, most_flattened_nodes)
                    if most_flattened_val >= 0.0:
                        break_flag = True
                    else:
        ...
        if self.return_ch_props:
            ...
            H = Q ** (0.6 * (1.0 - self._b)) * (self.mannings_n / self.k_w) ** 0.6 * node_S ** (-0.3)
