...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python2.7/site-packages/landlab/grid/unstructured/links.py
...
def _split_link_ends(link_ends):
    ...
    if len(link_ends) < 2:
        ...
        try:
            ...
            return (
             np.array(start).ravel(), np.array(end).ravel())
...
def link_is_active(status_at_link_ends):
    ...
    return (status_at_link_start == CORE_NODE) & ~(status_at_link_end == CLOSED_BOUNDARY) | (status_at_link_end == CORE_NODE) & ~(status_at_link_start == CLOSED_BOUNDARY)
...
def in_link_ids_at_node(node_at_link_ends, link_ids=None, number_of_nodes=None):
    ...
    links_per_node = in_link_count_per_node(node_at_link_ends, number_of_nodes=number_of_nodes)
...
def out_link_ids_at_node(node_at_link_ends, link_ids=None, number_of_nodes=None):
    ...
    links_per_node = out_link_count_per_node(node_at_link_ends, number_of_nodes=number_of_nodes)
    return (link_ids, links_per_node)
...
def link_ids_at_node(node_at_link_ends, number_of_nodes=None):
    ...
    array([1, 1, 1, 2, 2, 2, 1, 1, 1])
    """
    links_per_node = link_count_per_node(node_at_link_ends, number_of_nodes=number_of_nodes)
    in_links = JaggedArray(*in_link_ids_at_node(node_at_link_ends, number_of_nodes=number_of_nodes))
    out_links = JaggedArray(*out_link_ids_at_node(node_at_link_ends, number_of_nodes=number_of_nodes))
...
class LinkGrid(object):
    ...
    def __init__(self, link_ends, number_of_nodes, link_ids=None, node_status=None):
        ...
        array([0, 2])
        """
        self._in_link_at_node = JaggedArray(*in_link_ids_at_node(link_ends, link_ids=link_ids, number_of_nodes=number_of_nodes))
        self._out_link_at_node = JaggedArray(*out_link_ids_at_node(link_ends, link_ids=link_ids, number_of_nodes=number_of_nodes))
    ...
    def number_of_links_at_node(self, node):
        ...
        [2, 2, 2, 2]
        """
        return self.number_of_in_links_at_node(node) + self.number_of_out_links_at_node(node)
