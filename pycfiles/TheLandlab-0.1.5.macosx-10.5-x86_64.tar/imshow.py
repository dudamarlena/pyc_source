...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python2.7/site-packages/landlab/plot/imshow.py
...
def imshow_node_grid(grid, values, **kwds):
    ...
    assert_array_size_matches(values, grid.number_of_nodes, 'number of values does not match number of nodes')
    data = values.view()
...
def imshow_active_node_grid(grid, values, other_node_val='min', **kwds):
    ...
    try:
        assert_array_size_matches(values, active_nodes.size, 'number of values does not match number of active nodes')
    except ValueError:
        assert_array_size_matches(values[active_nodes], active_nodes.size, 'number of values does not match number of active nodes')
...
def imshow_cell_grid(grid, values, **kwds):
    ...
    try:
        assert_array_size_matches(values, cells.size, 'number of values does not match number of cells')
    except ValueError:
        assert_array_size_matches(values[cells], cells.size, 'number of values does not match number of cells')
...
def imshow_active_cell_grid(grid, values, other_node_val='min', **kwds):
    ...
    try:
        assert_array_size_matches(values, active_cells.size, 'number of values does not match number of active cells')
    except ValueError:
        assert_array_size_matches(values[active_cells], active_cells.size, 'number of values does not match number of active cells')
...
def _imshow_grid_values(grid, values, var_name=None, var_units=None, grid_units=(None, None), symmetric_cbar=False, cmap='pink', limits=None, allow_colorbar=True, norm=None):
    ...
    if RasterModelGrid in gridtypes:
        ...
        y = np.arange(values.shape[0] + 1) - grid.dx * 0.5
        x = np.arange(values.shape[1] + 1) - grid.dx * 0.5
        kwds = dict(cmap=cmap)
        if limits is None:
            if symmetric_cbar:
        ...
        import matplotlib.colors as colors, matplotlib.cm as cmx
        ...
        for order in grid.vor.point_region:
            ...
            if -1 not in region:
                ...
                plt.fill(color=colortouse, *zip(*polygon))
...
def imshow_field(field, name, **kwds):
    ...
    imshow_grid(field, field.field_values(values_at, name), var_name=name, var_units=field.field_units(values_at, name), **kwds)
...
def imshow_active_cells(grid, values, var_name=None, var_units=None, grid_units=(None, None), symmetric_cbar=False, cmap='pink'):
    ...
    y = np.arange(data.shape[0]) - grid.dx * 0.5
    x = np.arange(data.shape[1]) - grid.dx * 0.5
    if symmetric_cbar:
