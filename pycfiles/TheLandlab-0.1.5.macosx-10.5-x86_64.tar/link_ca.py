...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python2.7/site-packages/landlab/components/linkca/link_ca.py
...
import landlab, numpy, pylab as plt, time
_NEVER = 1000000000000.0
_DEBUG = False
_TEST = False
...
class LinkCellularAutomaton():
    ...
    def __init__(self, model_grid, node_state_dict, transition_list, initial_node_states, orientation_matters=False):
        ...
        for t in transition_list:
            try:
                assert t.from_state < self.num_link_states, 'Transition from_state out of range'
                assert t.to_state < self.num_link_states, 'Transition to_state out of range'
                this_type = int
            except:
                ...
                for i in t.from_state[:-1]:
                    assert i < self.num_node_states, 'Transition from_state out of range'

                for i in t.to_state[:-1]:
                    assert i < self.num_node_states, 'Transition to_state out of range'

                assert t.from_state[(-1)] < 2, 'Encoding for horizontal/vertical in from_state must be 0 or 1.'
                assert t.to_state[(-1)] < 2, 'Encoding for horizontal/vertical in to_state must be 0 or 1.'
                this_type = tuple

            assert last_type == this_type or last_type == None, 'All transition types must be either int IDs, or all tuples.'
            last_type = this_type
        ...
        self.number_of_vertical_links = self.grid.number_of_node_columns * (self.grid.number_of_node_rows - 1)
    ...
    def set_node_state_grid(self, node_states):
        ...
        """
        assert type(node_states) is numpy.ndarray, 'initial_node_states must be a Numpy array'
        assert len(node_states) == self.grid.number_of_nodes, 'length of initial_node_states must equal number of nodes in grid'
    ...
    def assign_link_states_from_node_types(self):
        ...
        to link states.
        """
        self.link_state = numpy.zeros(self.grid.number_of_active_links, dtype=int)
    ...
    def get_next_event(self, link, current_state, current_time):
        ...
        """
        assert self.n_xn[current_state] > 0, 'must have at least one potential transition'
    ...
    def update_link_state(self, link, new_link_state, current_time):
        ...
        if self.grid.node_status[fn] != landlab.grid.base.CORE_NODE or self.grid.node_status[tn] != landlab.grid.base.CORE_NODE:
    ...
    def do_transition(self, event, current_time, plot_each_transition=False, plotter=None):
        """
        ...
        if event.time == self.next_update[event.link]:
            ...
            from_changed, to_changed = self.update_node_states(fromnode, tonode, event.xn_to)
            ...
            print self.next_update[event.link], 'so event will be ignored'
        return
    ...
    def update_component_data(self, new_node_state_array, nodes_added, translation_vector, changed_uplift=False):
        ...
        *changed_uplift* is a flag which tells the method to refresh its 
        ...
        if self.translation_index is False:
            ...
            if translation_vector[0] > 0:
                ...
                for i in xrange(max_iter):
                    ...
                    on_grid_nodes = numpy.logical_and(numpy.greater(hoz_iter, i), numpy.greater(vert_iter, i))
        ...
        self.next_update[self.links_refreshed] = 0.0
    ...
    def run(self, run_duration, node_state_grid=None, plot_each_transition=False, plotter=None):
...
def setup_transition_list():
    ...
    """
    xn_list = []
    xn_list.append(Transition(1, 3, 1.0, 'weathering'))
    xn_list.append(Transition(2, 3, 1.0, 'weathering'))
    if False and _DEBUG:
        print
...
def setup_transition_list2():
    ...
    """
    xn_list = []
    xn_list.append(Transition(1, 2, 1.0, 'mobilization'))
    xn_list.append(Transition(3, 6, 1.0, 'mobilization'))
    xn_list.append(Transition(10, 11, 1.0, 'mobilization'))
    xn_list.append(Transition(12, 15, 1.0, 'mobilization'))
    xn_list.append(Transition(2, 6, 10.0, 'left motion'))
    xn_list.append(Transition(6, 2, 10.0, 'right motion'))
    xn_list.append(Transition(11, 15, 1000.0, 'downward motion'))
