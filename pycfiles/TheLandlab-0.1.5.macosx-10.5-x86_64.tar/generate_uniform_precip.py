...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python2.7/site-packages/landlab/components/uniform_precip/generate_uniform_precip.py
...
_DEFAULT_INPUT_FILE = os.path.join(os.path.dirname(__file__), 'preciptest.in')
...
class PrecipitationDistribution(Component):
    ...
    def yield_storm_interstorm_duration_intensity(self, subdivide_interstorms=False):
        ...
        assert delta_t == None and subdivide_interstorms == False, 'You specified you wanted storm subdivision, but did not provide a DELTA_T to allow this!'
        self._elapsed_time = 0.0
        while self._elapsed_time < self.run_time:
            ...
            yield (
             storm_duration - step_time, intensity)
            ...
            if subdivide_interstorms:
                ...
                yield (
                 interstorm_duration - step_time, 0.0)
            else:
                yield (
                 interstorm_duration, 0.0)
