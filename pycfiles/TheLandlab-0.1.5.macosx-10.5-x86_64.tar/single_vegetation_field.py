...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python2.7/site-packages/landlab/components/single_vegetation/single_vegetation_field.py
...
class Vegetation(Component):
    ...
    def __init__(self, grid, **kwds):
        ...
        self._LAI_max = kwds.pop('LAI_MAX', 2.0)
        ...
        self._Blive_init = kwds.pop('BLIVE_INI', 102.0)
        self._Bdead_init = kwds.pop('BDEAD_INI', 450.0)
    ...
    def update(self, **kwds):
        ...
        for cell in range(0, self.grid.number_of_cells):
            ...
            LAIdead = min(self._cd * self._Bdead_ini[cell], self._LAI_max - LAIlive)
            if PET[cell] > PETthreshold:
                NPP = max(ActualET[cell] / (Tb + Tr) * self._WUE * 24.0 * 0.55 * 1000.0, 0.001)
                Bmax = (self._LAI_max - LAIdead) / self._cb
                Yconst = 1.0 / (1.0 / Bmax + (self._kws * Water_stress[cell] + self._ksg) / NPP)
                Blive = (self._Blive_ini[cell] - Yconst) * np.exp(-(NPP / Yconst) * ((Tb + Tr) / 24.0)) + Yconst
                Bdead = (self._Bdead_ini[cell] + (Blive - max(Blive * np.exp(-self._ksg * Tb / 24), 1e-05))) * np.exp(-self._kdd * min(PET[cell] / 10.0, 1.0) * Tb / 24.0)
            else:
                Blive = max(self._Blive_ini[cell] * np.exp(-2 * self._ksg * Tb / 24.0), 1.0)
                Bdead = max((self._Bdead_ini[cell] + (self._Blive_ini[cell] - max(self._Blive_ini[cell] * np.exp(-2 * self._ksg * Tb / 24.0), 1e-06))) * np.exp(-1 * self._kdd * min(PET[cell] / 10.0, 1.0) * Tb / 24.0), 0.0)
