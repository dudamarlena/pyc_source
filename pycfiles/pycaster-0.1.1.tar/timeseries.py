...
# Embedded file name: /Library/Python/2.7/site-packages/pycast/common/timeseries.py
# Compiled at: 2015-05-28 05:27:39
import time, random, os
_STR_EPOCHS = 'UNIX-epochs'
os.environ['TZ'] = 'GMT'
NormalizationLevels = {'second': 1, 
   'minute': 1 * 60, 
   'hour': 1 * 60 * 60, 
   'day': 1 * 60 * 60 * 24, 
   'week': 1 * 60 * 60 * 24 * 7, 
...
FusionMethods = {'mean': lambda l: sum(l) / float(len(l)), 
   'median': lambda l: sorted(l)[(len(l) // 2)], 
   'sum': lambda l: sum(l)}
from helper import linear_interpolation
InterpolationMethods = {'linear': linear_interpolation}
