...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/ngramdb/app.py
...
import itertools as it, csv
from multiprocessing import Pool
import zensols.actioncli.time as time
from zensols.actioncli import persisted, StashFactory
...
class CreateDatabase(PersisterContainer):
    ...
    def __init__(self, config, year_limit=None):
        self.config = config
        self.n_workers = self.config.get_option_int('n_workers',
    ...
    def load(self, clear=True, batches=sys.maxsize):
        ...
        logger.info(f"starting {len(path_sets)} groups importing " + f"on/after {self.year_limit}")
