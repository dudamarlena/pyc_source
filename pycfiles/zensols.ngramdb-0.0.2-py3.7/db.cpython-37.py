...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/ngramdb/db.py
...
from zensols.actioncli import persisted, PersistedWork, StashFactory, DelegateStash
from zensols.db import Bean, BeanDbPersister, DbPersisterFactory
import zensols.dbpg
...
class NgramBean(Bean):
    ...
    def __init__(self, grams: str, yr: int, match_count: int, page_count: int, volume_count: int, id: int):
        self.grams = grams
        self.yr = yr
...
class NgramPersister(BeanDbPersister):
    ...
    def __init__(self, lang, n_gram, prefix, *args, **kwargs):
        (super(NgramPersister, self).__init__)(args, row_factory=NgramBean, **kwargs)
        self.lang = lang
        self.n_gram = n_gram
        self.prefix = prefix

    def get_by_grams(self, grams) -> list:
        return self.execute_by_name((self.prefix + '_select_by_ngram'),
          params=(
         grams,),
    ...
    def get_match_pattern_count(self, grams) -> int:
        row = self.execute_singleton_by_name('ngram_agg_select_aggregate_match_count_by_grams',
          params=(
         grams,))
        if row is not None:
            return row[0]
    ...
    def get_top_nth_count(self, n) -> int:
        return self.execute_singleton_by_name('ngram_agg_top_n_count',
          row_factory='dict',
          params=(
         n,))

    def grams_exist(self, grams) -> bool:
        res = self.execute_singleton_by_name((self.prefix + '_select_entry_exists_by_id'),
          params=(
         grams,))
        if res is not None:
            return res[0] > 0
    ...
    def get_grams(self) -> list:
        return self.execute_by_name((self.prefix + '_select_distinct_grams'),
          map_fn=(lambda x: x[0]))
...
class NgramStash(DelegateStash):
    ...
    def __init__(self, config, persister, year_limit=None):
        ...
        data_dir = config.get_option('data_dir',
          section=(AppConfig.NGRAM_SECTION), expect=False)
