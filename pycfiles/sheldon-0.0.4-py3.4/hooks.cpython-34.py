...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/sheldon/hooks.py
# Compiled at: 2015-11-23 12:25:17
...
def find_hooks(plugin_module):
    ...
    for name in plugin_module.__dict__.keys():
        ...
        if hasattr(item, '_sheldon_hook') and item._sheldon_hook.type != 'interval':
            hooks.append(item._sheldon_hook)
        elif hasattr(item, '_sheldon_hook') and item._sheldon_hook.type == 'interval':
