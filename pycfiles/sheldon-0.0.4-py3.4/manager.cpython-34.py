...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/sheldon/manager.py
# Compiled at: 2015-11-23 11:21:12
...
class PluginsManager:
    ...
    def load_plugin(self, plugin_name):
        ...
        if not plugin_module:
            logger.error_message("'{}' plugin didn't load".format(plugin_name))
            return
        plugin_config = parse_config(plugin_module)
        hooks, interval_hooks = find_hooks(plugin_module)
        plugin = Plugin(plugin_name, plugin_module, plugin_config, hooks, interval_hooks)
