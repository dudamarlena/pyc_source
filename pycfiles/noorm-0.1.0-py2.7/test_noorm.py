...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noorm/tests/test_noorm.py
...
class TestNoORM(unittest.TestCase):
        ...
        def compare_dicts(d1, d2):
            ...
            for key in d1:
                if isinstance(d1[key], dict):
                    ...
                    raise AssertionError(('unexpected data: {}, {}').format(type(d1[key]), type(d2[key])))

        compare_dicts(normalize(res1), normalize(res2))

    def test_compare_results(self):
        testdata1 = {'foo': [{'stuff': 'things', 'bar': 'baz', 'id': 1}, {'stuff': 'yay', 'bar': 'wooot', 'id': 2}], 'bar': [{'stuff': 'yoo!', 'bar': 'foo', 'id': 1}, {'stuff': '...', 'bar': ':-(', 'id': 2}]}
        same_as_testdata1_but_2nd_list_has_different_order = {'foo': [{'stuff': 'things', 'bar': 'baz', 'id': 1}, {'stuff': 'yay', 'bar': 'wooot', 'id': 2}], 'bar': [{'stuff': '...', 'bar': ':-(', 'id': 2}, {'stuff': 'yoo!', 'bar': 'foo', 'id': 1}]}
        slightly_different_dict = {'foo': [{'bar': 'baz', 'id': 1}, {'stuff': 'yay', 'bar': 'wooot', 'id': 2}], 'bar': [{'stuff': 'yoo!', 'bar': 'foo', 'id': 1}, {'stuff': '...', 'bar': ':-(', 'id': 2}]}
        ...
        assert success, 'comparation succeeded falsely for testdata1, slightly_different_dict'
        success = False
        try:
        ...
        assert success, 'comparation succeeded falsely for same_as_testdata1, slightly_different_dict'
    ...
    def test_update(self):
        with NoORM(':memory:') as (ssql):
            ...
            self.compare_results(ssql.dump(), {'foo': [{'stuff': 'things', 'bar': 'updated', 'id': 1}]})

    def test_faulty_insert_with_id(self):
        with NoORM(':memory:') as (ssql):
            try:
                ssql['foo'].insert({'stuff': 'things', 'bar': 'updated', 'id': 1})
    ...
    def test_id_query(self):
        with NoORM(':memory:') as (ssql):
            ...
            expected_result = {'id': 1, 'bar': 'baz', 'stuff': 'things'}
    ...
    def test_slice_query(self):
        with NoORM(':memory:') as (ssql):
            ...
            assert len(result) == 1
            result = result[0]
            assert result['stuff'] == 'yay'
            assert result['bar'] == 'wooot'
            assert result['id'] == 2
    ...
    def test_get_all(self):
        with NoORM(':memory:') as (ssql):
            ...
            tbl.insert({'bar': 'wooot', 'stuff': 'yay'})
            result = tbl['*']
            expected_result = [{'stuff': 'things', 'bar': 'baz', 'id': 1}, {'stuff': 'yay', 'bar': 'wooot', 'id': 2}]
    ...
    def test_dump(self):
        with NoORM(':memory:') as (ssql):
            ...
            result = ssql.dump()
            expected_result = {'foo': [{'stuff': 'things', 'bar': 'baz', 'id': 1}, {'stuff': 'yay', 'bar': 'wooot', 'id': 2}], 'bar': [{'stuff': 'yoo!', 'bar': 'foo', 'id': 1}, {'stuff': '...', 'bar': ':-(', 'id': 2}]}
            self.compare_results(result, expected_result)
    ...
    def test_dict_query(self):
        with NoORM(':memory:') as (ssql):
            ...
            expected_result = [{'stuff': 'yoo!', 'bar': 'foo', 'id': 1}]
            self.compare_results(result, expected_result)
    ...
    def test_raname_table(self):
        with NoORM(':memory:') as (ssql):
            ...
            expected_result = [{'stuff': 'things', 'bar': 'baz', 'id': 1}]
            self.compare_results(result, expected_result)
    ...
    def test_table_info(self):
        with NoORM(':memory:') as (ssql):
            ...
            assert len(tbl_info['foo']) == 3
            assert tbl_info['foo']['id'] == 0
            assert tbl_info['foo']['stuff'] == 1
            assert tbl_info['foo']['bar'] == 2