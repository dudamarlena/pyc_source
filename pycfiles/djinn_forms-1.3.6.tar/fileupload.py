...
# [GCC 8.4.0]
# Embedded file name: /home/bouma/gitprojects/provgroningen/buildout/src/djinn_forms/djinn_forms/views/fileupload.py
# Compiled at: 2014-05-22 04:46:26
import json, magic, os
from tempfile import mkstemp
import uuid, logging as log, mimetypes
...
class UploadView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        file_name = None
        attachments = []
        attachment_type = request.REQUEST.get('attachment_type', 'djinn_contenttypes.DocumentAttachment')
        if attachment_type in ('image', 'avatar'):
            ...
            for filename in request.FILES.keys():
                ...
                mime_type = request.FILES[filename].content_type or magic.Magic(mime=True).from_file(temp_file) or mimetypes.guess_type(file_name)[0]
                log.debug('Uploaded file mime type: %s' % mime_type)
                relative_file_name = self.generate_relative_file_path(file_name, mime_type, attachment_type)
                if attachment_type in ('image', 'avatar'):
                    if not self.is_image(mime_type):
                        log.error('Upload is not an image.')
                        response = HttpResponse(json.dumps({'success': False, 'msg': 'Upload is not an image'}))
                        return response
                    if attachment_id:
                        attachment = model.objects.get(pk=attachment_id)
                        attachment.original_filename = file_name
                    else:
                        attachment = model(mime_type=mime_type, title=file_name, original_filename=file_name)
                    ...
                    if attachment_id:
                        ...
                        attachment = model.objects.create(file=store_path, mime_type=mime_type, title=file_name, original_filename=file_name)
                    attachment.save()
                else:
        ...
        context['html'] = render_to_string(template, {'attachments': attachments})
    ...
    def generate_relative_file_path(self, file_name, mime_type, attachment_type):
        """ Check on file type, and generate path accordingly """
        fn, file_extension = os.path.splitext(file_name)
        file_id = str(uuid.uuid4())
        file_name = os.path.join(file_id[:2], '%s%s' % (file_id, file_extension))
        return file_name
