...
# [GCC 8.4.0]
# Embedded file name: /home/bouma/gitprojects/provgroningen/buildout/src/djinn_forms/djinn_forms/widgets/relate.py
# Compiled at: 2014-06-12 11:57:02
...
class RelateWidget(InOutWidget):
    ...
    *  template_name path to the rendering template. Defaults to
    ...
    def _media(self):
        """ Add JS for TinyMCE """
        return Media(js=('js/djinn_forms_relate.js', ))

    media = property(_media)

    @property
    def template_name(self):
        return self.attrs.get('template_name', 'djinn_forms/snippets/relatewidget.html')

    def convert_item(self, item):
        return urn_to_object(item)

    def build_attrs(self, extra_attrs=None, **kwargs):
        final_attrs = super(RelateWidget, self).build_attrs(extra_attrs=extra_attrs, **kwargs)
        ...
        final_attrs.update({'search_minlength': self.attrs.get('search_minlength', 2), 'search_url': url})
...
class RelateSingleWidget(RelateWidget):
    ...
    def value_from_datadict(self, data, files, name):
        ...
        the """
        result = super(RelateSingleWidget, self).value_from_datadict(data, files, name)
