...
# [GCC 8.4.0]
# Embedded file name: /home/bouma/gitprojects/provgroningen/buildout/src/djinn_forms/djinn_forms/fields/role.py
# Compiled at: 2014-10-06 05:47:30
...
class UpdateRoles(UpdateRelations):
    ...
    def update(self):
        ...
        for profile in self.rms:
            self.instance.rm_local_role(role, _profile_to_user_or_group(profile))

        for profile in self.adds:
            self.instance.add_local_role(role, _profile_to_user_or_group(profile))
...
class UpdateRole(UpdateRelation):
    ...
    def update(self):
        ...
        if self.tgt:
            ...
            self.instance.add_local_role(role, _profile_to_user_or_group(self.tgt))
...
class LocalRoleField(RelateField):
    ...
    def __init__(self, role_id, content_types, *args, **kwargs):
        self.role = role_id
        super(LocalRoleField, self).__init__(self.role, content_types, *args, **kwargs)
    ...
    def prepare_value(self, data):
        ...
        """
        if data and 'add' not in data.keys():
            return data
        if not data:
            data = {}
        ...
        try:
            users_or_groups = filter(lambda x: x not in data['rm'], users_or_groups)
        except:
            pass
        ...
        return [ {'label': unicode(profile), 'value': object_to_urn(profile)} for profile in users_or_groups ]
