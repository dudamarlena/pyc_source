...
# Embedded file name: build/bdist.linux-x86_64/egg/cvu/metis.py
# Compiled at: 2013-03-14 11:55:07
...
REALTYPEWIDTH = os.getenv('METIS_REALTYPEWIDTH', '64')
if IDXTYPEWIDTH == '32':
...
del value
del obj
_opt_types = {METIS_OPTION_PTYPE: mptype_et, 
...
if not _dll_filename:
    truffl = '/local_mount/space/truffles/1/users/rlaplant/metis/lib/libmetis.so'
    if os.path.exists(truffl):
        _dll_filename = truffl
    else:
        try:
...
def _result_errcheck(result, func, args):
    ...
    if result != METIS_OK:
        ...
        raise RuntimeError('Error raising error: Bad error.')
    return result
...
@_wrapdll(P(idx_t), P(idx_t), P(idx_t), P(idx_t), P(idx_t), P(idx_t), P(idx_t), P(idx_t), P(real_t), P(real_t), P(idx_t), P(idx_t), P(idx_t))
def _METIS_PartGraphKway(nvtxs, ncon, xadj, adjncy, vwgt, vsize, adjwgt, nparts, tpwgts, ubvec, options, objval, part):
    """
    Called by `part_graph`
    """
    return _METIS_PartGraphKway.call(nvtxs, ncon, xadj, adjncy, vwgt, vsize, adjwgt, nparts, tpwgts, ubvec, options, objval, part)


@_wrapdll(P(idx_t), P(idx_t), P(idx_t), P(idx_t), P(idx_t), P(idx_t), P(idx_t), P(idx_t), P(real_t), P(real_t), P(idx_t), P(idx_t), P(idx_t))
def _METIS_PartGraphRecursive(nvtxs, ncon, xadj, adjncy, vwgt, vsize, adjwgt, nparts, tpwgts, ubvec, options, objval, part):
    """
    Called by `part_graph` 
    """
    return _METIS_PartGraphRecursive.call(nvtxs, ncon, xadj, adjncy, vwgt, vsize, adjwgt, nparts, tpwgts, ubvec, options, objval, part)


def part_graph(graph, nparts=2, tpwgts=None, ubvec=None, recursive=False, **opts):
    """
    ...
    if tpwgts:
        assert len(tpgwgts) == nparts * graph.ncon
        assert ubvec and len(ubvec) == graph.ncon
    ...
    return (
     objval.value, list(partition))


def example_adjlist():
    return [
     [
      1, 2, 3, 4], [0], [0], [0], [0, 5], [4, 6], [13, 5, 7],
     [
