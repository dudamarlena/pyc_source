...
# Embedded file name: build/bdist.linux-x86_64/egg/cvu/tractography.py
# Compiled at: 2014-05-19 10:08:16
from __future__ import division
import os, struct, numpy as np, scipy.linalg as lin, nibabel as nib, random
...
def rescale(x, y, z, invert_y=True, **surf_properties_kwargs):
    import volume
    surf_xmin, surf_ymin, surf_zmin, surf_xmax, surf_ymax, surf_zmax = volume.surf_properties(**surf_properties_kwargs)
    pad_nr = 2
    surf_xmin += pad_nr
    surf_xmax -= pad_nr
    surf_ymin += pad_nr
...
def apply_cmp_affines(s, b0vol_fname, trk_fname, fs_subj_name, fs_subjs_dir, fsenvsrc=None):
...
def apply_affines_carefully(s, b0vol_fname, trk_fname, fs_subj_name, fs_subj_dir, fsenvsrc=None):
    if fsenvsrc:
        import cvu_utils
        cvu_utils.tcsh_env_interpreter(fsenvsrc)
    a1 = fetch_b0_to_tkr(b0vol_fname, trk_fname)
...
def fetch_ras_to_tkr(b0vol_fname, fs_subj_name, fs_subjects_dir, fsenvsrc=None, out_fname=None):
    ...
    cmd = "bbregister --s %s --mov %s --init-fsl --reg '%s' --bold --tol1d 1e-3" % (
     fs_subj_name, b0vol_fname, tmpfname + '.reg')
    ...
    if proc.wait() != 0:
        ...
        raise cvu_utils.CVUError('bbregister crashed with error %i' % proc.returncode)
