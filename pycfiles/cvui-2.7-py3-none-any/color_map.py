...
# Embedded file name: build/bdist.linux-x86_64/egg/cvu/color_map.py
# Compiled at: 2015-07-13 11:28:18
import numpy as np
from matplotlib.colors import LinearSegmentedColormap
from mayavi.core import lut_manager
from pylab import get_cmap
from traits.api import HasTraits, Enum, Bool, File, Range, Str, Instance, Property, Event, on_trait_change
...
class CustomColormap(HasTraits):
    ...
    def _cmap_default(self):
        if self.map_type == 'scalar':
            return 'BuGn'
        else:
            if self.map_type == 'activation':
                return 'YlOrRd'
            if self.map_type == 'connmat':
                return 'RdYlBu'
            return 'cool'
    ...
    def _label_default(self):
        if self.map_type == 'scalar':
            return 'Scalars Colormap'
        else:
            if self.map_type == 'activation':
                return 'Conns Colormap'
            if self.map_type == 'connmat':
                return 'Matrix Colormap'
...
def gen_heatmap(t=0.2, two_tailed=True, reverse=False):
...
two-tailed -- if false, only the upper (red) part of the heatmap is shown"""
    ...
    if two_tailed:
        cdict = {'red': revif((swapif((0.0, 0.0, 0.6)), swapif((t, 0.2, 0.4)),
                 swapif((1 - t, 0.4, 1.0)),
                 swapif((1.0, 1.0, 0.0)))), 
        ...
        cdict = {'red': revif((swapif((0.0, 0.0, 0.4)),
                 swapif((1 - t, 0.4, 1.0)),
                 swapif((1.0, 1.0, 0.0)))), 'green': revif((swapif((0.0, 0.0, 0.4)),
                   swapif((1 - t, 0.4, 0.0)),
                   swapif((1.0, 1.0, 0.0)))), 
...
def set_color_range(mayavi_obj, scalars, use_vector_lut=False):
    ...
    module_manager.data_range = (
     np.min(scalars), np.max(scalars))