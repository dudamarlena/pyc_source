...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/tom_education/tests.py
# Compiled at: 2020-01-06 08:52:00
# Size of source mod 2**32: 79492 bytes
...
from tom_education.models import ASYNC_STATUS_CREATED, ASYNC_STATUS_FAILED, ASYNC_STATUS_PENDING, AsyncError, AsyncProcess, crop_image, InvalidPipelineError, ObservationAlert, ObservationTemplate, PipelineProcess, PipelineOutput, TIMELAPSE_GIF, TIMELAPSE_MP4, TIMELAPSE_WEBM, TimelapsePipeline
...
class ObservationTemplateTestCase(TomEducationTestCase):
    ...
    def test_existing_templates_shown(self, mock):
        template = ObservationTemplate.objects.create(name='mytemplate',
          target=(self.target),
          facility=(self.facility),
          fields='{"one": "two"}')
        template2 = ObservationTemplate.objects.create(name='other-template',
          target=(self.target),
    ...
    def test_create_template(self, mock):
        ...
        fields = {'test_input':'some-name', 
         'extra_field':'this is some extra text', 
         'another_extra_field':4, 
         'target_id':self.target.pk, 
         'facility':self.facility, 
         'observation_type':''}
        post_params = dict(fields, **{'create-template': 'yes please'})
        self.client.force_login(self.non_staff)
    ...
    def test_instantiate_template(self, dt_mock, _):
        dt_mock.now.return_value = datetime(year=2019,
          month=1,
          day=2,
          hour=3,
          minute=4,
        ...
        template = ObservationTemplate.objects.create(name='mytemplate',
          target=(self.target),
          facility=(self.facility),
          fields='{"test_input": "mytemplate", "extra_field": "someextravalue", "another_extra_field": 5}')
...
class InvalidObservationTemplateNameTestCase(SimpleTestCase):
    ...
    def test_invalid_template_name(self, mock):
        ...
        template = ObservationTemplate.objects.create(name='cool-template-name',
          target=target,
          facility='TemplateFake',
          fields='...')
        url = reverse('tom_education:create_obs', kwargs={'facility': 'TemplateFake'})
        response = self.client.post(url, {'test_input':'cool-template-name', 
         'extra_field':'blah', 
         'another_extra_field':4, 
         'target_id':target.pk, 
         'facility':'TemplateFake', 
...
class DataProductTestCase(TomEducationTestCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.target = Target.objects.create(name='my target')
        cls.observation_record = ObservingRecordFactory.create(target_id=(cls.target.id),
        ...
        for i, date in enumerate(dates):
            ...
            prod = DataProduct.objects.create(product_id=product_id,
              target=(cls.target),
...
class TargetDataViewTestCase(DataProductTestCase):
    ...
    def test_null_product_id(self):
        ...
        response = self.client.post((self.url), data={'action': 'view_gallery', 
         dp1.pk: 'on', 
         dp2.pk: 'on'})
...
@override_settings(TOM_EDUCATION_TIMELAPSE_SETTINGS={'format':'gif', 
 'fps':10,  'size':500,  'crop_scale':0.5})
class TimelapseTestCase(DataProductTestCase):
    ...
    def create_timelapse_pipeline(self, products, **kwargs):
        pipeline = (TimelapsePipeline.objects.create)(identifier='test_{}'.format(datetime.now().isoformat()), 
    ...
    def assert_webm_data(self, data):
        data.seek(0)
        self.assertEqual(data.read(4), b'\x1aE\xdf\xa3')
    ...
    def test_create_timelapse_form(self, dt_mock):
        ...
        response2 = self.client.post(url, {'action': 'pipeline', 
    ...
    def test_fits_file_sorting(self):
        correct_order = [
         self.prods[0], self.prods[1], self.prods[3], self.prods[2]]
    ...
    def test_multiple_observations(self):
        ...
        """
        other_obs = ObservingRecordFactory.create(target_id=(self.target.id),
          facility=(FakeFacility.name),
          parameters='{}')
        other_obs_prod = DataProduct.objects.create(product_id='different observation',
          target=(self.target),
    ...
    @override_settings(TOM_EDUCATION_TIMELAPSE_SETTINGS={'format':TIMELAPSE_GIF,  'fps':16}, TOM_EDUCATION_TIMELAPSE_GROUP_NAME='timelapsey')
...
class GalleryTestCase(TomEducationTestCase):
    ...
    def test_post(self):
        ...
        response = self.client.post(self.url, {'product_pks': ','.join([str(p.pk) for p in self.prods]), 
         'group': mygroup.pk, 
         self.pk0: 'on', 
         self.pk1: 'on'})
...
class AsyncStatusApiTestCase(TomEducationTestCase):
    ...
    def test_api(self, views_dt_mock, models_dt_mock, django_mock):
        ...
        self.assertEqual(response1.json(), {'timestamp':current_timestamp, 
         'processes':[
          failed_proc_dict, dict(proc_dict, status='pending')]})
        proc.status = ASYNC_STATUS_CREATED
        proc.save()
        response2 = self.client.get(url)
        self.assertEqual(response2.status_code, 200)
        self.assertEqual(response2.json(), {'timestamp':current_timestamp, 
         'processes':[
          failed_proc_dict,
        ...
        self.assertEqual(response3.status_code, 200)
        self.assertEqual(response3.json(), {'timestamp':current_timestamp, 
         'processes':[
          failed_proc_dict,
          dict(proc_dict, status='failed', terminal_timestamp=terminal_timestamp, failure_message=None)]})
...
class FakePipelineWithFlags(FakePipeline):
    flags = {'myflag':{'default':False, 
      'long_name':'myflag'}, 
     'default_true':{'default':True, 
      'long_name':'Default True'}, 
     'default_false':{'default':False, 
      'long_name':'Default False'}}
...
class PipelineTestCase(TomEducationTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.prods = [DataProduct.objects.create(product_id=f"test_{i}", target=(cls.target)) for i in range(4)]
    ...
    def test_save_outputs(self, dt_mock):
        dt_mock.now.return_value = datetime(year=1970,
          month=1,
          day=1,
          hour=0,
          minute=0,
    ...
    def test_api(self, async_mock, django_mock):
        async_mock.now.return_value = datetime(year=1970,
          month=1,
          day=1,
          hour=0,
          minute=6,
          second=0,
          microsecond=0)
        django_mock.return_value = datetime(year=1970,
          month=1,
          day=1,
          hour=0,
          minute=5,
          second=0,
          microsecond=0)
        proc = FakePipeline.objects.create(target=(self.target),
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), {'identifier':'someprocess', 
         'created':300, 
         'status':'somestatus', 
         'logs':'', 
         'terminal_timestamp':None, 
        ...
        response2 = self.client.get(url)
        self.assertEqual(response2.status_code, 200)
        self.assertEqual(response2.json(), {'identifier':'someprocess', 
         'created':300, 
        ...
        response3 = self.client.get(url)
        self.assertEqual(response3.status_code, 200)
        self.assertEqual(response3.json(), {'identifier':'someprocess', 
         'created':300, 
         'status':ASYNC_STATUS_FAILED, 
         'terminal_timestamp':360, 
    ...
    def test_form(self, dt_mock, flags_mock):
        ...
        url = reverse('tom_education:target_data', kwargs={'pk': self.target.pk})
        test_settings = {'mypip':'tom_education.tests.FakePipeline', 
        ...
        with self.settings(TOM_EDUCATION_PIPELINES=test_settings):
            ...
            self.assertEqual(response.context['pipeline_flags'], {'withflags': FakePipelineWithFlags.flags})
            expect_400_data = [
             {'action': 'pipeline'},
             {'action':'pipeline', 
              'pipeline_name':'blah'}]
            ...
            response2 = self.client.post(url, {'action': 'pipeline', 'pipeline_name': 'mypip', self.pks[0]: 'on'})
            self.assertEqual(response2.status_code, 200)
            ...
            response3 = self.client.post(url, {'action': 'pipeline', 'pipeline_name': 'withflags', self.pks[0]: 'on', 
             'pipeline_flag_myflag': 'on', 
             'pipeline_flag_default_false': 'on', 
             'pipeline_flag_bogus': 'on'})
            expected_flags = {'myflag':True, 
             'default_true':False,  'default_false':True}
    ...
    def test_allowed_suffixes(self):
        ...
        with patch('tom_education.tests.FakePipeline.allowed_suffixes', ['.tar', '.7z']):
            ...
            self.assertIn('_file.tar.gz', err_msg)
            self.assertIn('.tar, .7z', err_msg)
...
class TargetDetailApiTestCase(TomEducationTestCase):
    ...
    def setUp(self):
        ...
        self.target = Target(name=(self.target_name),
          ra=1.2345)
        ...
        other_pipeline = PipelineProcess.objects.create(identifier='this_is_not_a_timelapse',
          target=(self.target))
        ...
        tl_gif_pipeline = TimelapsePipeline.objects.create(identifier='gif_tl',
          target=(self.target))
        ...
        tl_webm_pipeline = TimelapsePipeline.objects.create(identifier='webm_tl',
          target=(self.target))
        ...
        tl_failed = TimelapsePipeline.objects.create(identifier='tl_failed',
          target=(self.target),
          status=ASYNC_STATUS_FAILED)
        tl_pending = TimelapsePipeline.objects.create(identifier='tl_pending',
          target=(self.target),
    ...
    def test_api(self):
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), {'target':{'name':self.target_name, 
...
class ObservationAlertApiTestCase(TomEducationTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.target = Target.objects.create(name='my target')
        self.template = ObservationTemplate.objects.create(name='mytemplate',
          target=(self.target),
          facility='TemplateFake',
    ...
    def test_create(self, dt_mock, _mock):
        ...
        response = self.client.post(url, {'target':self.target.pk, 
        ...
        self.assertEqual(json.loads(ob.parameters), {'target_id':self.target.pk, 
         'facility':'TemplateFake', 
    ...
    def test_invalid_target(self, _mock):
        url = reverse('tom_education:observe_api')
        response = self.client.post(url, {'target':10000000000, 
         'template_name':self.template.name, 
         'facility':'TemplateFake', 
         'email':'someone@somesite.org'},
          content_type='application/json')
    ...
    def test_invalid_facility(self, _mock):
        ...
        response = self.client.post(url, {'target':self.target.pk, 
         ...
         'email':'someone@somesite.org'},
          content_type='application/json')
        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.json(), {'detail': 'Facility not found.'})
    ...
    def test_invalid_template(self, _mock):
        ...
        response = self.client.post(url, {'target':self.target.pk, 
         ...
         'email':'someone@somesite.org'},
          content_type='application/json')
        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.json(), {'detail': "Template 'tempo' not found for target 'my target' and facility 'TemplateFake'"})
    ...
    def test_invalid_form(self, _mock):
        ...
        self.assertEqual(response.json(), {'another_extra_field': ['Enter a whole number.']})
...
class ProcessObservationAlertsTestCase(TomEducationTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.target = Target.objects.create(name='my target')
        cls.ob = ObservingRecordFactory.create(target_id=(cls.target.pk),
    ...
    def test_non_alert_observation_not_updated(self, save_dp_mock):
        non_alert_ob = ObservingRecordFactory.create(target_id=(self.target.pk),
          facility=(FakeTemplateFacility.name),
          status='not even started')
        call_command('process_observation_alerts')
    ...
    @patch('tom_education.models.TimelapsePipeline.create_timestamped', wraps=(TimelapsePipeline.create_timestamped))
    def test_timelapse_created(self, pipeline_mock, save_dp_mock):
        alert = ObservationAlert.objects.create(observation=(self.ob),
          email='someone@somesite.org')
        call_command('process_observation_alerts')
    ...
    def test_old_timelapses_deleted(self, save_dp_mock):
        alert = ObservationAlert.objects.create(observation=(self.ob), email='someone@somesite.org')
        tl = DataProduct.objects.create(target=(self.target),
          product_id='mytimelapse',
          data_product_type=(settings.DATA_PRODUCT_TYPES['timelapse'][0]))
        other_dp = DataProduct.objects.create(target=(self.target),
          product_id='notatimelapse')
        call_command('process_observation_alerts')
...
class DataProductDeleteMultipleViewTestCase(DataProductTestCase):
    ...
    def test_delete(self):
        base_url = reverse('tom_education:delete_dataproducts')
        response = self.client.post(base_url, {'next':'mycoolwebsite.net', 
...
def mock_instruments(_self):
    return {'myinstr': {'type':'IMAGE', 
                 'class':'2M0', 
                 'name':'test instrument', 
                 'optical_elements':{'filters': [
                              {'code':'redfilter', 
                               'name':'RED',  'schedulable':True},
                              {'code':'greenfilter', 
                               'name':'GREEN',  'schedulable':True},
                              {'code':'bluefilter', 
                               'name':'BLUE',  'schedulable':True}]}}}


def mock_proposals(_self):
    return [
     ('myprop', 'some proposal')]
...
class EducationLCOFacilityTestCase(TomEducationTestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(username='test', email='test@example.com')
        self.client.force_login(self.user)
        self.base_form_data = {'target_id':self.target.pk, 
         'facility':'LCO', 
         'name':'someobs', 
         'proposal':'myprop', 
         'ipp_value':'1.05', 
    ...
    def test_multiple_instrument_configurations(self, _validate_mock, submit_mock):
        response = self.client.post((self.url), data={**(self.base_form_data), **{'redfilter_exposure_count':'1', 
        ...
        red_instr_config = {'exposure_count':1, 
         'exposure_time':2, 
         'optical_elements':{'filter': 'redfilter'}}
        blue_instr_config = {'exposure_count':3, 
         'exposure_time':4, 
    ...
    def test_exposure_settings(self, _validate_mock, submit_mock):
        ...
        response = self.client.post((self.url), data={**(self.base_form_data), **{'redfilter_exposure_time':'2', 
         'bluefilter_exposure_count':'3', 
    ...
    def test_instrument_filter_info(self, _validate_mock, _submit_mock):
        instr_response = {'instr1':{'optical_elements': {'filters':[
                                {'code':'a1', 
                                 'schedulable':False},
                                {'code':'a2', 
                                 'schedulable':True}], 
