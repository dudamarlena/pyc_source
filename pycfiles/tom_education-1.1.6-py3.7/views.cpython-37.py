...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/tom_education/views.py
# Compiled at: 2020-04-30 08:35:11
# Size of source mod 2**32: 19219 bytes
...
from tom_targets.models import Target, GLOBAL_TARGET_FIELDS, REQUIRED_NON_SIDEREAL_FIELDS, REQUIRED_NON_SIDEREAL_FIELDS_PER_SCHEME
...
from tom_education.models import AsyncProcess, ASYNC_STATUS_CREATED, ObservationAlert, ObservationTemplate, PipelineProcess, TimelapsePipeline
from tom_education.serializers import AsyncProcessSerializer, ObservationAlertSerializer, PipelineProcessSerializer, TargetDetailSerializer, TimestampField
...
class TemplatedObservationCreateView(ObservationCreateView):
    ...
    def get_initial(self):
        ...
        if template_id:
            template = ObservationTemplate.objects.filter(target=(self.get_target()),
              facility=facility).get(pk=template_id)
    ...
    def form_valid(self, form):
        ...
        if self.get_form_class().new_template_action[0] in form.data:
            ...
            try:
                template = ObservationTemplate.objects.create(name=name,
...
class TargetDetailApiView(RetrieveAPIView):
    ...
    def get_object(self):
        ...
        tl_pipelines = TimelapsePipeline.objects.filter(target=target,
          group__dataproduct__target__isnull=False,
          status=ASYNC_STATUS_CREATED,
          process_type='TimelapsePipeline').order_by('-terminal_timestamp')
...
class ObservationAlertApiCreateView(CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            template = ObservationTemplate.objects.get(target=target,
            ...
            err = "Template '{}' not found for target '{}' and facility '{}'".format(data['template_name'], target.name, data['facility'])
            raise NotFound(detail=err)

        form_data = {'target_id':target.pk, 
        ...
        form = facility_class.get_form(None)(form_data)
        if not form.is_valid():
            raise serializers.ValidationError(form.errors)
        observation_ids = facility_class().submit_observation(form.observation_payload())
        assert len(observation_ids) == 1, 'Submittion created multiple observation IDs: {}'.format(observation_ids)
        ob = ObservationRecord.objects.create(target=target,
          facility=(facility_class.name),
...
class NonSiderealFieldsMixin:
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if self.get_target_type() == Target.NON_SIDEREAL:
            ...
            context['non_sidereal_fields'] = json.dumps({'base_fields':base, 
