...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/tom_education/templatetags/tom_education_extras.py
# Compiled at: 2020-01-06 06:46:44
...
def dataproduct_thumbs_for_target(target):
    return {'products':target.dataproduct_set.filter(data__contains='fits').order_by('-created')[:6], 
     'target':target}
...
def dataproduct_other_for_target(target):
    timelapses = target.dataproduct_set.filter(data_product_type='timelapse')
    return {'timelapse':timelapses.latest() if timelapses else None, 
     'photometry':target.dataproduct_set.filter(data_product_type='photometry'), 
     'target':target}
