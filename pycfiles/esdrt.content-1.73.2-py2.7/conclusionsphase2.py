...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/esdrt/content/conclusionsphase2.py
# Compiled at: 2020-01-10 09:04:56
...
class IConclusionsPhase2(form.Schema, IImageScaleTraversable):
    ...
    """
    closing_reason = schema.Choice(title=_('Final Status of Observation'), vocabulary='esdrt.content.conclusionphase2reasons', required=True)
    text = schema.Text(title=_('Recommendation for Draft Review Report (not visible to MS)'), required=True, default=DEFAULTCONCLUSIONTEXT)
    remarks = schema.Text(title=_('Concluding remark'), description=_('(visible to MS when observation finalised)'), required=False)
...
class ConclusionsPhase2(dexterity.Container):
    ...
    def reason_value(self):
        return self._vocabulary_value('esdrt.content.conclusionphase2reasons', self.closing_reason)
...
class AddForm(dexterity.AddForm):
    ...
    def updateFields(self):
        ...
        conclusion_fields = field.Fields(IConclusionsPhase2).select('closing_reason', 'text', 'remarks')
