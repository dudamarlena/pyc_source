...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/esdrt/content/browser/carryover.py
...
def reopen_with_qa(workflows, obj, actor):
    ...
    if question:
        add_to_wh(workflows['question'], question, action_question, new_state_question, actor)
    if in_phase2:
...
class CarryOverView(BrowserView):
    ...
    def start(self, action, xls):
        ...
        workflows = dict(tool=wft, observation=wf_obs, question=wf_question, conclusion=wf_conclusion, conclusion2=wf_conclusion2)
        copy_func = partial(actions[action], context, catalog, workflows, obj_from_url)
