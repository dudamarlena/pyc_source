...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/esdrt/content/subscriptionconfiguration.py
...
import copy
ROLE_TRANSLATOR = {'ReviewerPhase1': 'Sector Expert (phase 1)', 
...
NOTIFICATIONS_PER_ROLE = {'ReviewerPhase1': {'observation_finalisation_denied': True, 
                      'observation_finalised': True, 
                      'question_answered': True, 
                      'question_to_ms': True}, 
   'ReviewerPhase2': {'observation_finalisation_denied': True, 
                      'observation_finalised': True, 
                      'observation_to_phase2': True, 
                      'question_answered': True, 
                      'question_to_ms': True}, 
   'QualityExpert': {'conclusion_to_comment': True, 
                     'observation_finalisation_request': True, 
                     'question_answered': True, 
                     'question_ready_for_approval': True, 
                     'question_to_counterpart': True}, 
   'LeadReviewer': {'conclusion_to_comment': True, 
                    'observation_finalisation_request': True, 
                    'observation_to_phase2': True, 
                    'question_answered': True, 
                    'question_ready_for_approval': True, 
                    'question_to_counterpart': True}, 
   'MSAuthority': {'answer_acknowledged': True, 
                   'observation_finalised': True, 
                   'question_to_ms': True}, 
   'CounterPart': {'conclusion_to_comment': True, 
                   'question_to_counterpart': True}, 
   'MSExpert': {'answer_to_msexperts': True, 
                'question_answered': True}}
NOTIFICATION_NAMES = {'ReviewerPhase1': {'observation_finalisation_denied': 'Observation finalisation denied by QE', 
   ...
   'ReviewerPhase2': {'observation_finalisation_denied': 'Observation finalisation denied by LR', 
   ...
   'QualityExpert': {'conclusion_to_comment': 'Conclusion to comment by you as QE', 
   ...
   'LeadReviewer': {'conclusion_to_comment': 'Conclusion to comment by you as LR', 
   ...
   'MSAuthority': {'answer_acknowledged': 'Answer acknowledged by sector expert', 
                   'observation_finalised': 'Observation finished by Quality Expert', 
                   'question_to_ms': 'Question to be answered by your country'}, 
   'CounterPart': {'conclusion_to_comment': 'Conclusion to comment by you as counterpart', 
                   'question_to_counterpart': 'Question to comment by you as counterpart'}, 
   'MSExpert': {'answer_to_msexperts': 'New question to comment by you as MS expert', 
                'question_answered': 'Question answered by MS'}}
...
class SubscriptionConfigurationMixin(grok.View):
    ...
    def user_roles(self, context=None, translated_roles=True):
        ...
        for role in api.user.get_roles(user=user, obj=context):
            ...
            elif role in ROLE_TRANSLATOR.keys():
                roles.append(role)
    ...
    def get_unsubscribed_notifications(self, context=None):
        if context is None:
            context = self.context
        if not (IReviewFolder.providedBy(context) or IObservation.providedBy(context)):
            return {}
        else:
            user = self.user()
    ...
    def notification_name(self, rolename, notification_name):
        return NOTIFICATION_NAMES.get(rolename, {}).get(notification_name, notification_name)
