...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fssa/fssa.py
...
import numpy as np, numpy.ma as ma, scipy.optimize
...
def scaledata(l, rho, a, da, rho_c, nu, zeta):
    ...
    x = np.power(l_mesh, 1.0 / nu) * (rho_mesh - rho_c)
...
def _wls_linearfit_predict(x, w, wx, wy, wxx, wxy, select):
    ...
    kxy = wxy[select].sum()
    delta = k * kxx - kx ** 2
    m = 1.0 / delta * (k * kxy - kx * ky)
    b = 1.0 / delta * (kxx * ky - kx * kxy)
    b_var = kxx / delta
    m_var = k / delta
    bm_covar = -kx / delta
    y = b + m * x
...
def _jprimes(x, i, x_bounds=None):
    ...
    for i_prime in range(k):
        if i_prime == i:
            ...
            continue
            jprimes = np.searchsorted(x[i_prime], x[i], side='right').astype(float) - 1
            jprimes[np.logical_or(jprimes < x_lower[i_prime], jprimes >= x_upper[i_prime])] = np.nan
...
def quality(x, y, dy, x_bounds=None):
    ...
    for i in range(k):
        ...
        for j in range(n):
            ...
            if not select.any():
                continue
            master_y[(i, j)], master_dy2[(i, j)] = _wls_linearfit_predict(x=x[(i, j)], w=w, wx=wx, wy=wy, wxx=wxx, wxy=wxy, select=select)

    return np.nanmean(np.nanmean((y - master_y) ** 2 / (dy ** 2 + master_dy2), axis=1))
...
def _neldermead_errors(sim, fsim, fun):
    ...
    x = 0.5 * (sim[np.mgrid[0:n + 1, 0:n + 1]][1] + sim[np.mgrid[0:n + 1, 0:n + 1]][0])
...
def autoscale(l, rho, a, da, rho_c0, nu0, zeta0, x_bounds=None, **kwargs):
    ...
    def goal_function(x):
        my_x, my_y, my_dy = scaledata(rho=rho, l=l, a=a, da=da, nu=x[1], zeta=x[2], rho_c=x[0])
        return quality(my_x, my_y, my_dy, x_bounds=x_bounds)

    ret = scipy.optimize.minimize(goal_function, [
     rho_c0, nu0, zeta0], method=_minimize_neldermead, options={'xtol': 0.01, 
     'ftol': 0.01})
    errors, varco = _neldermead_errors(sim=ret['final_simplex'][0], fsim=ret['final_simplex'][1], fun=goal_function)
    ret['varco'] = varco
    ret['errors'] = errors
