...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/wapiti/api.py
# Compiled at: 2014-10-27 04:33:47
"""
...
__version__ = '0.1'
import re, os, six, ctypes, logging, multiprocessing
from ctypes.util import find_library
_libwapiti_root_path = os.path.join(os.path.abspath(os.path.dirname(__file__)), '../')
_wapiti = ctypes.CDLL(os.path.join(_libwapiti_root_path, list(filter(lambda x: re.search('^libwapiti.*?\\.so$', x), os.listdir(_libwapiti_root_path))).pop()))
...
class ModelType(ctypes.Structure):
    _fields_ = [
     (
      'opt', ctypes.POINTER(OptType)),
     (
      'nlbl', ctypes.c_size_t),
     (
      'nobs', ctypes.c_size_t),
     (
      'nftr', ctypes.c_size_t),
     (
      'kind', ctypes.c_char_p),
     (
      'uoff', ctypes.c_void_p),
     (
      'boff', ctypes.c_void_p),
     (
      'theta', ctypes.c_void_p),
     (
      'train', ctypes.c_void_p),
     (
      'devel', ctypes.c_void_p),
     (
      'reader', ctypes.c_void_p),
     (
      'werr', ctypes.c_void_p),
     (
      'wcnt', ctypes.c_int),
     (
      'wpos', ctypes.c_int),
...
_wapiti.api_new_model.argtypes = [
 ctypes.POINTER(OptType), ctypes.c_char_p]
...
api_logs[0] = LogFunc(fatal)
api_logs[1] = LogFunc(fatal)
api_logs[2] = LogFunc(logger.warning)
api_logs[3] = LogFunc(logger.info)
...
class Model:
    ...
    def __init__(self, patterns=None, encoding='utf8', **options):
        ...
        if self.options.model:
            self._model = _wapiti.api_load_model(self.options.model, ctypes.pointer(self.options))
        else:
            ...
            self._model = _wapiti.api_new_model(ctypes.pointer(self.options), self.patterns)
