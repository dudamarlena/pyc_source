...
# Compiled at: 2020-05-01 15:56:41
# Size of source mod 2**32: 54872 bytes
...
class Airfoil:
    ...
    def __init__(self, name=None, coordinates=None, CL_function=None, CDp_function=None, Cm_function=None, repanel=False):
        """
    ...
    def populate_sectional_functions_from_xfoil_fits(self, parallel=True, verbose=True):
        if not self.has_xfoil_data(raise_exception_if_absent=False):
            self.get_xfoil_data(parallel=parallel,
              verbose=verbose)
        self.AirfoilFitter = AirfoilFitter(airfoil=self,
          parallel=parallel,
          verbose=verbose)
        self.AirfoilFitter.fit_xfoil_data_Cl(plot_fit=False)
        self.AirfoilFitter.fit_xfoil_data_Cd(plot_fit=False)

        def CL_function(alpha, Re, mach=0, deflection=0, fitter=self.AirfoilFitter):
            return fitter.Cl_function(alpha=alpha,
              Re=Re)

        def CDp_function(alpha, Re, mach=0, deflection=0, fitter=self.AirfoilFitter):
            return fitter.Cd_function(alpha=alpha,
              Re=Re)

        def Cm_function(alpha, Re, mach=0, deflection=0, fitter=self.AirfoilFitter):
            return alpha * 0
    ...
    def has_sectional_functions(self, raise_exception_if_absent=True):
        ...
        """
        data_present = hasattr(self, 'CL_function') and callable(self.CL_function) and hasattr(self, 'CDp_function') and callable(self.CDp_function) and hasattr(self, 'Cm_function') and callable(self.Cm_function)
        if not data_present:
            if raise_exception_if_absent:
                raise Exception("This Airfoil %s does not yet have sectional functions,\n                so you can't run the function you've called.\n                To get sectional functions, first call:\n                    Airfoil.populate_sectional_functions_from_xfoil_fits()\n                which will perform an in-place update that\n                provides the data." % self.name)
        return data_present
    ...
    def repanel(self, n_points_per_side=80, inplace=False):
        """
        ...
        x_coors = interp1d(distances_from_TE_normalized,
          (self.coordinates[:, 0]),
          kind='cubic')(s)
        y_coors = interp1d(distances_from_TE_normalized,
          (self.coordinates[:, 1]),
          kind='cubic')(s)
    ...
    def repanel_legacy(self, n_points_per_side=100, inplace=False):
        """
        Returns a repaneled version of the airfoil with cosine-spaced coordinates on the upper and lower surfaces.
        :param n_points_per_side: Number of points per side (upper and lower) of the airfoil [int]
            Notes: The number of points defining the final airfoil will be n_points_per_side*2-1,
        ...
        x_upper = interp1d(s, upper_distances_from_TE_normalized, upper_original_coors[:, 0])
        y_upper = interp1d(s, upper_distances_from_TE_normalized, upper_original_coors[:, 1])
        x_lower = interp1d(s, lower_distances_from_LE_normalized, lower_original_coors[:, 0])
        y_lower = interp1d(s, lower_distances_from_LE_normalized, lower_original_coors[:, 1])
    ...
    def get_xfoil_data(self, a_start=-6, a_end=12, a_step=0.5, a_init=0, Re_start=10000.0, Re_end=10000000.0, n_Res=30, mach=0, max_iter=20, repanel=False, parallel=True, verbose=True):
    ...
    def has_xfoil_data(self, raise_exception_if_absent=True):
        ...
        if not data_present:
            if raise_exception_if_absent:
                raise Exception("This Airfoil %s does not yet have XFoil data,\n                so you can't run the function you've called.\n                To get XFoil data, first call:\n                    Airfoil.get_xfoil_data()\n                which will perform an in-place update that\n                provides the data." % self.name)
        return data_present
    ...
    def plot_xfoil_data_all_polars(self, n_lines_max=20, Cd_plot_max=0.04):
        """
    ...
    def plot_xfoil_data_polar(self, Res, Cd_plot_max=0.04, repanel=False, parallel=True, max_iter=40, verbose=True):
        """
        ...
        def get_xfoil_data_at_Re(Re):
            import numpy as np
            xfoil_data = self.xfoil_aseq(a_start=0,
              a_end=15,
              a_step=0.25,
              Re=Re,
              M=0,
        ...
        for i, Re in enumerate(Res):
            plt.plot((runs_data[i]['Cd'] * 10000.0),
              (runs_data[i]['Cl']),
              label=('Re = %s' % eng_string(Re)),
              color=(colors[i]))
