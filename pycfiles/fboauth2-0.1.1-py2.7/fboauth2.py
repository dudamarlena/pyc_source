...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/fboauth2.py
# Compiled at: 2012-12-09 22:08:43
...
class FBClient(object):
    ...
    def get_auth_url(self, scope=None, redirect_uri=None, state=None):
        if redirect_uri:
            self.redirect_uri = redirect_uri
        params = dict(client_id=self.client_id, redirect_uri=self.redirect_uri, scope=scope or self.scope or '')
        if state:
            params['state'] = state
        return self.auth_uri + '?' + urllib.urlencode(params)

    def get_access_token(self, code):
        params = dict(client_id=self.client_id, client_secret=self.client_secret, redirect_uri=self.redirect_uri, code=code)
