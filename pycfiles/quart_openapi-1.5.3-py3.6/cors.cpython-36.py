...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/quart_openapi/cors.py
...
def crossdomain(origin: str=None, methods: Iterable[str]=None, headers: Union[(str, Iterable[str])]=None, expose_headers: Union[(str, Iterable[str])]=None, max_age: Union[(int, timedelta)]=21600, attach_to_all: bool=True, automatic_options: bool=True, credentials: bool=False) -> Callable:
    ...
    if methods is not None:
        ...
        if headers is not None:
            ...
            if expose_headers is not None:
                if not isinstance(expose_headers, str):
                    expose_headers = ', '.join(x.upper() for x in expose_headers)
            origin = isinstance(origin, str) or ', '.join(origin)
