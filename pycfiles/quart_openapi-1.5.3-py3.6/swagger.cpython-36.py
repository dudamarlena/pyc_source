...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/quart_openapi/swagger.py
# Compiled at: 2020-04-13 15:04:18
# Size of source mod 2**32: 19254 bytes
"""swagger.py
...
def _clean_header(header: HeaderType) -> Dict[(str, Any)]:
    ...
    if isinstance(header, str):
        ...
        if typedef in PY_TYPES:
            ...
            if len(typedef) == 1:
                if typedef[0] in PY_TYPES:
                    header['type'] = 'array'
                    header['items'] = {'type': PY_TYPES[typedef[0]]}
        else:
            if hasattr(typedef, '__schema__'):
        ...
        if not converter:
            ...
            if converter in PATH_TYPES:
                ...
                if converter == 'uuid':
                    ...
                    if converter in RouteMap.default_converters:
                        param['schema']['type'] = 'string'
                    else:
...
class Swagger:
    __doc__ = 'Class for generating a openapi.json from the resources and information defined with\n    :class:`~factset.quart_openapi.Pint`'
    ...
    def as_dict(self) -> Dict[(str, Any)]:
        ...
        infos = {'title':self.api.title or 'OpenApi Rest Documentation', 
         'version':self.api.version or '1.0'}
        if self.api.description:
            infos['description'] = self.api.description
        if self.api.contact:
            if self.api.contact_email or self.api.contact_url:
                infos['contact'] = not_none({'name':self.api.contact, 
        ...
        scheme = self.api.config.get('PREFERRED_URL_SCHEME', 'http' if not self.api.config.get('PREFER_SECURE_URLS', False) else 'https')
        spec = {'openapi':'3.0.0', 
         'info':infos, 
         'servers':[
    ...
    def parameters_for(self, doc: Dict[(str, Any)]) -> Iterable[Dict[(str, Any)]]:
        ...
        for name, param in doc['params'].items():
            if 'ref' in param:
                if isinstance(param['ref'], str):
                    if param['ref'].startswith('#/components/'):
    ...
    def operation_id_for(self, doc: Dict[(str, Any)], method: str) -> str:
        ...
        """
        if 'id' in doc[method]:
            return doc[method]['id']
        else:
            return self.api.default_id(doc['name'], method)
    ...
    @staticmethod
    def process_headers(response: Dict[(str, Any)], doc: Dict[(str, Any)], method: Optional[str]=None, headers: Optional[Dict[(str, Union[(str, Dict[(str, Any)])])]]=None) -> Dict[(str, Any)]:
        ...
        if 'headers' in doc or 'headers' in method_doc or headers:
            response['headers'] = dict((k, _clean_header(v)) for k, v in chain(doc.get('headers', {}).items(), method_doc.get('headers', {}).items(), (headers or {}).items()))
        return response
        ...
        if isinstance(validator, (list, tuple)):
            ...
            if isinstance(validator, Draft4Validator):
                ...
                if isinstance(validator, str):
                    ...
                    return validator.schema
                if isinstance(validator, (type, type(None))):
                    if validator in PY_TYPES:
                        return {'type': PY_TYPES[validator]}
            return {}

    def serialize_resource(self, resource: Union[(Resource, Callable)], path: str, methods: Iterable[str]) -> Dict[(str, Any)]:
    ...
    def serialize_operation(self, doc: Mapping[(str, Any)], method: str) -> Dict[(str, Any)]:
        ...
        """
        operation = {'summary':doc[method]['docstring']['summary'], 
        ...
        if doc is False:
            ...
            for method in [m.lower() for m in resource.methods or []]:
                ...
                if hasattr(method_impl, 'im_func'):
                    ...
                    if hasattr(method_impl, '__func__'):
        ...
        if 'expect' not in doc:
            ...
            for expect in doc.get('expect', []):
                ...
                if isinstance(validator, str):
                    ...
                    if not isinstance(validator, Draft4Validator):
                        continue
                schema = self.serialize_schema(validator)
                if '$ref' in schema:
                    if '/components/requestBodies/' in schema['$ref']:
                        return schema
                params[content_type] = not_none(dict(
                 {'schema': self.serialize_schema(validator)}, **kwargs))
