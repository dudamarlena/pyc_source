...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/asterisk_dialplan/dialplan.py
# Compiled at: 2015-03-14 04:48:37
""" Dialplan Functions """
...
def parse_diaplan_context(dialplan_string):
  ...
  the extension, and the arguments are priority followed by the command """
    ...
    exten = None
    priority = None
    grammar = re.compile('^(exten\\s*?\\=\\>?\\s*(?P<exten>.+)\\s*,\\s*(?P<priority>[0-9]+|n)\\s*,\\s*(?P<command>.+))|(same\\s*?\\=\\>?\\s*(?P<priority_same>[0-9]+|n)\\s*,\\s*(?P<command_same>.+))')
    for row in rows:
...
def convert_dialplan_to_range(dialplan):
    ...
    for key, value in dialplan.iteritems():
        if key[0] == '_':
            dialplan_obj = {'exten_range': pattern_to_ranges(key), 'args': value}
        else:
            dialplan_obj = {'exten': key, 'args': value}
