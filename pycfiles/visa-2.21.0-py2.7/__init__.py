...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/visa/__init__.py
# Compiled at: 2019-02-27 03:29:13
import base64, logging, platform, sys, urllib, textwrap
try:
...
if not _json_loaded:
    try:
        ...
        if _json_loaded is None:
            raise ImportError("VISA requires a JSON library, which you do not appear to have.  Please install the simplejson library.  HINT: Try installing the python simplejson library via 'easy_install simplejson', or contact support@visa.com with questions.")
        else:
            raise ImportError("VISA requires a JSON library with the same interface as the Python 2.6 'json' library.  You appear to have a 'json' library with a different interface.  Please install the simplejson library.  HINT: Try installing the python simplejson library via 'easy_install simplejson', or contact support@visa.com with questions.")
...
if not _httplib:
    raise ImportError("VISA requires either pycurl or Google App Engine's urlfetch.  If you are on a platform where neither of these libraries are available, please let us know at support@visa.com.")
VERSION = '1.0.0'
logger = logging.getLogger('visa')
logger.addHandler(logging.StreamHandler(sys.stderr))
logger.setLevel(logging.ERROR)
api_key = None
api_base = 'https://api.visa.com/v1'

class VISAError(Exception):
    pass


class APIError(VISAError):
    pass


class APIConnectionError(VISAError):
    pass


class CardError(VISAError):
...
class InvalidRequestError(VISAError):
...
class AuthenticationError(VISAError):
    pass


def convertToVISAObject(resp, api_key):
    types = {'charge': Charge, 'customer': Customer, 'invoice': Invoice, 
       'invoice_item': InvoiceItem}
    if isinstance(resp, list):
        return [ convertToVISAObject(i, api_key) for i in resp ]
    else:
        if isinstance(resp, dict):
            resp = resp.copy()
            klass_name = resp.get('object')
            klass = types.get(klass_name, VISAObject)
...
class APIRequestor(object):
    ...
    def request(self, meth, url, params={}):
        ...
        if my_api_key is None:
            raise AuthenticationError('No API key provided.  (HINT: set your API key using "visa.api_key = <API-KEY>".  You can generate API keys from the VISA web interface.  See https://visa.com/api for details, or email support@visa.com if you have any questions.')
        abs_url = self.apiUrl(url)
        params = params.copy()
        self._objects_to_ids(params)
        ua = {'bindings_version': VERSION, 
           'lang': 'python', 
           'publisher': 'visa'}
        ...
        headers = {'X-VISA-Client-User-Agent': json.dumps(ua), 
           'User-Agent': 'VISA/v1 PythonBindings/%s' % (VERSION,), 
        ...
        if _httplib == 'pycurl':
            ...
            if _httplib == 'urlfetch':
                ...
                raise VISAError('VISA bug discovered: invalid httplib %s.  Please report to support@visa.com' % (_httplib,))
            try:
        ...
        logger.info('API request to %s returned (response code, response object) of (%d, %r)' % (
         abs_url, rcode, resp))
    ...
    def handleCurlError(self, e):
        if e[0] in [pycurl.E_COULDNT_CONNECT,
         ...
         pycurl.E_OPERATION_TIMEOUTED]:
            msg = "Could not connect to VISA (%s).  Please check your internet connection and try again.  If this problem persists, you should check VISA's service status at https://twitter.com/visa, or let us know at support@visa.com." % api_base
        elif e[0] == pycurl.E_SSL_CACERT:
            msg = "Could not verify VISA's SSL certificate.  Please make sure that your network is not intercepting certificates.  (Try going to https://api.visa.com in your browser.)  If this problem persists, let us know at support@visa.com."
        else:
            msg = 'Unexpected error communicating with VISA.  If this problem persists, let us know at support@visa.com.'
    ...
    def urlfetchRequest(self, meth, abs_url, headers, params):
        ...
        return (
         result.content, result.status_code)

    def handleUrlfetchError(self, e, abs_url):
        if isinstance(self, urlfetch.InvalidURLError):
            msg = 'The VISA library attempted to fetch an invalid URL (%r).  This is likely due to a bug in the VISA Python bindings.  Please let us know at support@visa.com.' % (
             abs_url,)
        elif isinstance(self, urlfetch.DownloadError):
            msg = 'There were a problem retrieving data from VISA.'
        elif isinstance(self, urlfetch.ResponseTooLargeError):
            msg = 'There was a problem receiving all of your data from VISA.  This is likely due to a bug in VISA.  Please let us know at support@visa.com.'
        else:
            msg = 'Unexpected error communicating with VISA.  If this problem persists, let us know at support@visa.com.'
        msg = textwrap.fill(msg) + '\n\n(Network error: ' + str(e) + ')'
        raise APIConnectionError(msg)
...
class VISAObject(object):
    ...
    def __getattr__(self, k):
        ...
        if k in self._transient_values:
            raise AttributeError("%r object has no attribute %r.  HINT: The %r attribute was set in the past, however.  It was then wiped when refreshing the object with the result returned by VISA's API, probably as a result of a save().  The attributes currently available on this object are: %s" % (
    ...
    def __getitem__(self, k):
        if k in self._values:
            return self.__dict__[k]
        if k in self._transient_values:
            raise KeyError("%r.  HINT: The %r attribute was set in the past, however.  It was then wiped when refreshing the object with the result returned by VISA's API, probably as a result of a save().  The attributes currently available on this object are: %s" % (k, k, (', ').join(self._values)))
        else:
            raise KeyError(k)
    ...
    def refreshFrom(self, values, api_key, partial=False):
        ...
        for k, v in values.iteritems():
            if k in self._permanent_attributes:
                continue
            self.__dict__[k] = convertToVISAObject(v, api_key)
            self._values.add(k)
    ...
    def __repr__(self, nested=False):
        ...
        if ident:
            ...
            ident = ''
        if nested:
            return '<visa.%s%s ...>' % (type(self).__name__, ident)
        values_str = []
        for k in sorted(self._values):
            if k in self._ignored_attributes:
                continue
            v = getattr(self, k)
            if isinstance(v, VISAObject):
                v = v.__repr__(True)
        ...
        return '<visa.%s%s %s>' % (type(self).__name__, ident, (', ').join(values_str))

    def __str__(self):
        return repr(self)


class APIResource(VISAObject):
...
class ListableAPIResource(APIResource):
    ...
    def all(cls, api_key=None, **params):
        ...
        return convertToVISAObject(response, api_key)
