...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/fitsne/cywrap.py
# Compiled at: 2020-01-07 16:11:12
# Size of source mod 2**32: 5696 bytes
from .cppwrap import _TSNErun
import numpy as np

def FItSNE(X: np.ndarray, no_dims: int=2, perplexity: float=30.0, sigma: float=-30.0, K: int=-1, initialization: np.ndarray=None, theta: float=0.5, rand_seed: int=-1, max_iter: int=1000, stop_early_exag_iter: int=250, fft_not_bh: bool=True, ann_not_vptree: bool=True, early_exag_coeff: float=12.0, no_momentum_during_exag: bool=False, start_late_exag_iter: int=-1, late_exag_coeff: float=-1, mom_switch_iter: int=250, momentum: float=0.5, final_momentum: float=0.8, learning_rate: int=200, n_trees: int=50, search_k: int=-1, nterms: int=3, intervals_per_integer: float=1, min_num_intervals: int=50, load_affinities: int=0, nthreads: int=0, df: float=1.0) -> np.ndarray:
    """
   ...
   df: float, default=0 
    ...
    _TSNErun(X, N, D, Y, no_dims, perplexity, theta, rand_seed, skip_random_init, max_iter, stop_early_exag_iter, mom_switch_iter, momentum, final_momentum, learning_rate, K, sigma, nbody_algo, knn_algo, early_exag_coeff, costs, no_momentum_during_exag_i, start_late_exag_iter, late_exag_coeff, n_trees, search_k, nterms, intervals_per_integer, min_num_intervals, nthreads, df)
    return Y