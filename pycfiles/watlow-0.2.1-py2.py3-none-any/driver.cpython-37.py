...
# [GCC 8.4.0]
# Embedded file name: /Users/jamesjeffryes/Documents/NuMat/watlow/build/lib/watlow/driver.py
...
class TemperatureController(object):
    ...
    responses = {'actual':re.compile('^55ff060010000b8802030104010108([0-9a-f]{8})([0-9a-f]{4})$'), 
     'setpoint':re.compile('^55ff060010000b8802030107010108([0-9a-f]{8})([0-9a-f]{4})$'), 
     'set':re.compile('^55ff060010000a76020407010108([0-9a-f]{8})([0-9a-f]{4})$')}
    ...
    def get(self):
        ...
        output = {'actual':None, 
         'setpoint':None}
        for key in output:
            output[key] = self._write_and_read(request=(self.commands[key]),
              length=21,
    ...
    def set(self, setpoint):
        ...
        if not 10 <= setpoint <= 200:
            raise ValueError('Setpoint must be between 10°C and 200°C.')
        body = self.commands['set']['body'] + struct.pack('>f', c_to_f(setpoint))
        checksum = struct.pack('<H', ~crc(body) & 65535)
        response = self._write_and_read(request=(self.commands['set']['header'] + body + checksum),
          length=20,
          check=(self.responses['set']))
        if round(setpoint, 2) != round(response, 2):
            raise IOError(f"Could not change setpoint from {response:.2f}°C to {setpoint:.2f}°C.")
...
class Gateway(AsyncioModbusClient):
    ...
    async def set_setpoint(self, zone: int, setpoint: float):
        ...
        if not self.setpoint_range[0] < setpoint < self.setpoint_range[1]:
            raise ValueError(f"Setpoint ({setpoint}) in not in the valid range from {self.setpoint_range[0]} to {self.setpoint_range[1]}")
