...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/globus_search_cli/commands/ingest.py
# Compiled at: 2018-12-11 21:16:05
import copy, json, click
...
@globus_cmd('ingest', help='Send DOCUMENTS to Globus Search to index. DOCUMENTS are paths to files, with "-" taken to mean stdin')
@click.option('--source-type', type=click.Choice(('gingest', 'gmetalist')), default='gingest', show_default=True, help='How to consume each given document -- are they GIngest documents to send verbatim to the service, or are they GMetaLists which we will break into batches to send?')
@click.option('--batch-size', type=int, default=100, show_default=True, help='When source-type=GMetaList, how many entries to send at a time')
@index_argument
