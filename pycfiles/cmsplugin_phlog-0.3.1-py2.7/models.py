...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-x86_64/egg/cmsplugin_phlog/models.py
...
class OrderedGallery(models.Model):
    ...
    photos = models.ManyToManyField(Photo, related_name='ordered_galleries', verbose_name=_('photos'), null=True, blank=True, through='PhotoOrdering')
...
class PhlogGalleryPlugin(CMSPlugin):
    gallery = models.ForeignKey(OrderedGallery)
    order = models.CharField(max_length=32, default='gallery', choices=(
     ('date', 'By Date'),
     ('gallery', 'Gallery Order'),
     ('random', 'Random order')))
    limit = models.PositiveIntegerField(default=0, help_text='0 means no limit')
    public = models.BooleanField(default=True)
    template = models.CharField(max_length=255, blank=True, help_text='Enter a template to us instead of the standard templates')
...
class GalleryPlugin(CMSPlugin):
    template = models.CharField(max_length=255, blank=True, help_text='Enter a template to use instead of the standard templates')
