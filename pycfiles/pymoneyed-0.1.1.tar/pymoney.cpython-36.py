...
# Embedded file name: build\bdist.win-amd64\egg\pymoney.py
# Compiled at: 2018-12-01 00:11:16
...
class connect(object):
    ...
    def new(self, user):
        user = str(user)
        if self.database == 'sqlite':
            self.conn.execute('insert into user (user, money) values (?,?)', (user, '000'))
        else:
            self.cursor.execute('insert into user (user, money) values (%s,%s)', (user, '000'))
        self.conn.commit()
