...
# Embedded file name: build/bdist.macosx-10.4-x86_64/egg/abl/jquery/ui/widgets.py
# Compiled at: 2011-02-18 13:00:51
from simplejson import dumps
from tw.api import JSLink, js_callback, js_function, Widget
from tw.forms import TextField
from abl.jquery.core import jQuery, jquery_js
from abl.utils.odict import OrderedDict
from abl.jquery.core.widgets import update_content_js
from abl.jquery.ui import ui_autocomplete_js, ui_autocomplete_callbacks_js, ui_tabs_js, ui_dialog_js, effects_core_js
...
class JQueryUITabs(Widget):
    ...
    params = dict(tabs_id="HTML id of the tabs container. Default is 'tabs'", title_list="To give each widget a tab name. If not given or incomplete, the tabs named 'Tab <index>'", default_field='A optional widget which is displayed on all tabs.', tabs_options='A dictionary with extra option for jQuery tabs. See http://docs.jquery.com/UI/Tabs')
    tabs_id = 'tabs'
    tabs_options = dict()
    default_field = None
    title_list = []
...
ui_dialog_content_js = JSLink(modname=__name__, filename='static/javascript/ui.dialog.content.js', javascript=[
 ui_dialog_js, effects_core_js])
...
class DialogContentWidget(Widget):
    ...
    params = dict(url='optional content url', title='The header of the dialog, optional', config='Configuration of the dialog, optional')
    url = ''
    title = ''
    config = dict()
    default_config = dict(width=426, resizable=False, position=[
     'center', 70])
    ...
    def _get_ordered_config(cls, config):
        ...
        for (key, value) in config.iteritems():
            ...
            elif isinstance(value, js_callback):
                value = unicode(value)
            else:
                try:
                    value = dumps(value)
...
ui_dialog_confirm_js = JSLink(modname=__name__, filename='static/javascript/ui.dialog.confirm.js', javascript=[
 ui_dialog_content_js])
...
class DialogConfirmWidget(DialogContentWidget):
    ...
    """
    javascript = [
     ui_dialog_confirm_js]
    params = dict(action_url='URL that will be called when the user confirms. Its only used if confirm_callback is not defined.', cancel_callback='optional define a js_callback. default closes the dialog', cancel_button='Name of the Cancel button, default is Cancel', confirm_callback="Optional define a js_callback. Default makes a 'window.location.href = action_url'", confirm_button='Name of the confirm button, default is Ok')
    action_url = ''
    cancel_callback = js_callback('close')
    confirm_callback = ''
    cancel_button = 'Cancel'
    confirm_button = 'Ok'
    default_config = dict(modal=True, width=426, resizable=False, position=[
     'center', 70])
    ...
    def perform_call(self, d):
        if not d.config.has_key('buttons'):
            ...
            if d.confirm_callback:
                ...
                buttons[d.confirm_button] = js_callback("function() {abljqui_confirm('%s');}" % d.action_url)
            buttons[d.cancel_button] = d.cancel_callback
            d.config['buttons'] = buttons
        super(DialogConfirmWidget, self).perform_call(d)


ui_dialog_form_js = JSLink(modname=__name__, filename='static/javascript/ui.dialog.form.js', javascript=[
 form_widget_js,
 ui_dialog_content_js,
 update_content_js])
...
class DialogFormWidget(DialogContentWidget):
    ...
    javascript = [
     ui_dialog_form_js]
    params = dict(cancel_callback='optional define a js_callback. default closes the dialog', cancel_button='Name of the Cancel button, default is Cancel', submit_callback='Optional define a js_callback. Default makes an ajax submit', submit_button='Name of the confirm button, default is Submit')
    cancel_callback = js_callback('close')
...
MINLENGTH = (3, )
DELAY = (1, )
POSITION = dict(my='left top', at='left bottom', collision='none', offset='0, -1px;')
...
class AutoCompleteTextField(TextField):
    ...
    """
    params = dict(options='A dictionary with options.', submit_on_select='Auto-submit if a suggestion was selected by click or enter-key, Default is True')
    submit_on_select = True
    options = dict()
    default_options = dict(source=[], minLength=MINLENGTH, position=POSITION, delay=DELAY)
...
class InstantSearchTextField(AutoCompleteTextField):
    ...
    """
    params = dict(options='A dictionary with options.', instant_url="The URL that retrieves both, suggestions and instant results. Default is ''")
    instant_url = ''
    ...
    default_options = dict(source=source_callback, open=open_callback, close=close_callback, select=select_callback, minLength=MINLENGTH, position=POSITION, delay=DELAY, instant_url=instant_url)
...
class MultipleValuesAutocompleteTextField(AutoCompleteTextField):
    ...
    """
    params = dict(options='A dictionary with options.', suggestion_list='A list of strings used for local autocompletion', max_results='The maximum amount of results displayed as suggestion. Works only or a suggestion_list not for remote suggestion. Default is 5')
    options = dict()
    ...
    default_options = dict(minLength=1, position=POSITION, delay=DELAY, select=select_callback, focus=focus_callback)

    def update_params(self, d):
        super(AutoCompleteTextField, self).update_params(d)
        if d.suggestion_list and not self.options.has_key('source'):
            self.default_options['source'] = js_callback('function(request, response) {\n                      response($.ui.autocomplete.filter(%s,\n                                                        multiple_autocomplete_extract_last(request.term)).slice(0, %i));\n                    }' % (dumps(d.suggestion_list),
             d.max_results))
        options = self.default_options.copy()
        options.update(d.options)
