...
# [GCC 8.4.0]
# Embedded file name: /Applications/MAMP/htdocs/jongos1/jongos.py
# Compiled at: 2013-07-22 04:33:05
import json, time, random, string, hashlib, lockfile
...
class jongos():
    ...
    def load(self, dump=None):
        ...
        try:
            ...
            self.elapsed = time.time() - start
            if dump != None:
                self.set_status(500, 'Cant load %s as storage!' % str(self.storage))
    ...
    def insert(self, baris):
        self.refresh()
        if type(self.barisan) == type(list()):
            return self.insert_list(baris)
        if type(self.barisan) == type(dict()):
            return self.insert_dict(baris)
    ...
    def insert_dict(self, baris):
        ...
        if udah_ada == True:
            ...
            if self.changes >= self.saveTrigger:
                self.save()
            self.elapsed = time.time() - start
            return True
                ...
                for k, v in qw.iteritems():
                    ...
                    if type(v) == type(dict()):
                        ...
                        for w, x in v.iteritems():
                            if w == '$eq':
                                if k == x:
                                    s_terpenuhi = s_terpenuhi + 1
                            if w == '$ne':
                                if k != x:
                                    s_terpenuhi = s_terpenuhi + 1
                            if w == '$like':
                                try:
                                    if k.lower().find(x.lower()) > -1:
                                        s_terpenuhi = s_terpenuhi + 1
                                except:
                                    pass
                            ...
                            if w == '$likes' and type(x) == type(list()):
                                ...
                                for xt in x:
                                    try:
                                        if k.find(xt.lower()) > -1:
                                            t_terpenuhi = t_terpenuhi + 1
                                    except:
                                        pass
        ...
        if fields == None:
            ...
            for res in self.results:
                ...
                for t in fields:
                    t = str(t)
                    try:
                        r = self._baca_dotted_field(self.results, t)
                    except:
                        r = t
