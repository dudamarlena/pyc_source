...
# [GCC 8.4.0]
# Embedded file name: /home/zhemin/software/GrapeTree/module/views.py
# Compiled at: 2019-03-07 09:05:39
...
def generate_tree():
    try:
        ...
        tree = backend(profile=params['profile'], method=params['method'], checkEnv=params['checkEnv'])
...
def sendToMicroReact(debug=None):
    ...
    try:
        ...
        metadata = reviseField(metadata, 'latitude|^lat$', 'latitude', '')
        metadata = reviseField(metadata, 'longitude|^lon$', 'longitude', '')
        metadata = reviseField(metadata, 'year', 'year', '1')
        metadata = reviseField(metadata, 'month', 'month', '1')
        metadata = reviseField(metadata, 'day', 'day', '1')
        for c in ('year', 'month', 'day'):
            d = metadata[c].values
            sites = metadata[c].str.extract('(-*\\d+)').astype(float).values.flatten()
            sites[np.isnan(sites)] = -1
            d[sites < 0] = ''
            metadata[c] = d

        columns = list(filter(lambda c: c.find('__') < 0, metadata.columns))
        geo_columns = list(filter(lambda c: re.match('village|town|city|locat|state|region|district|^site$|geograph', c.lower()), columns))
        country_column = list(filter(lambda c: re.match('country', c.lower()), columns))
