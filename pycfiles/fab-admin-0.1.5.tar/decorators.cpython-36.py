...
# [GCC 8.4.0]
# Embedded file name: e:\opt\private\cw1427\fab-admin\fab_admin\fab_manager_overwrite\flask_appbuilder/security/decorators.py
# Compiled at: 2020-02-17 01:47:55
...
def has_access_api(f):
    ...
    def wraps(self, *args, **kwargs):
        ...
        if self.appbuilder.sm.has_access(permission_str, self.__class__.__name__):
            ...
            log.warning(LOGMSG_ERR_SEC_ACCESS_DENIED.format(permission_str, self.__class__.__name__))
            response = make_response(jsonify({'message':str(FLAMSG_ERR_SEC_ACCESS_DENIED),  'severity':'danger'}), 403 if current_user.is_authenticated else 401)
