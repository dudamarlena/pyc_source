...
# [GCC 8.4.0]
# Embedded file name: e:\opt\private\cw1427\fab-admin\fab_admin\fab_manager_overwrite\rq_scheduler_dashboard/web.py
# Compiled at: 2020-02-17 05:01:56
# Size of source mod 2**32: 7764 bytes
"""RQ Scheduler Dashboard Flask Blueprint.
...
blueprint = Blueprint('rq_scheduler_dashboard',
  __name__,
...
def setup_rq_connection():
    ...
    if redis_url:
        ...
        if redis_sentinels:
            ...
            sentinel_hosts = [tuple(sentinel.split(':', 1)) for sentinel in redis_sentinels.split(',')]
            sentinel = Sentinel(sentinel_hosts, db=db, password=password)
            current_app.redis_conn = sentinel.master_for(redis_master)
        else:
            current_app.redis_conn = Redis(host=(current_app.config.get('REDIS_HOST')),
...
def remove_none_values(input_dict):
    return {k:v for k, v in input_dict.items() if v is not None if v is not None}
...
def serialize_queues(queues):
    return [dict(name=(q.name), count=(q.count), url=url_for('.overview', queue_name=(q.name))) for q in queues]
...
def overview(queue_name, page):
    ...
    return render_template('rq_scheduler_dashboard/dashboard.html',
      queue=queue,
      page=page,
      queues=(Queue.all()),
...
def list_jobs(queue_name, page):
    ...
    pages_in_window = [dict(number=p, url=url_for('.overview', queue_name=queue_name, page=p)) for p in pages_numbers_in_window]
    last_page = int(ceil(total_items / float(per_page)))
    prev_page = None
    if current_page > 1:
        prev_page = dict(url=url_for('.overview',
          queue_name=queue_name, page=(current_page - 1)))
    next_page = None
    if current_page < last_page:
        next_page = dict(url=url_for('.overview',
          queue_name=queue_name, page=(current_page + 1)))
    pagination = remove_none_values(dict(pages_in_window=pages_in_window,
      next_page=next_page,
      prev_page=prev_page))
...
def list_queues():
    ...
    return dict(queues=[dict(name=queue_name, count=count, url=url_for('.overview', queue_name=queue_name)) for queue_name, count in queues.items()])
...
def scheduler_status():
    scheduler = Scheduler()
    return dict(running=(scheduler.connection.exists(scheduler.scheduler_key) and not scheduler.connection.hexists(scheduler.scheduler_key, 'death')))
