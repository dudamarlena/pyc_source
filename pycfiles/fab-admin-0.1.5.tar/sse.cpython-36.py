...
# [GCC 8.4.0]
# Embedded file name: e:\opt\private\cw1427\fab-admin\fab_admin\addon\sse\sse.py
# Compiled at: 2020-02-10 10:03:47
# Size of source mod 2**32: 6635 bytes
"""
Created on {* now *}
...
class Message(object):
    ...
    def __repr__(self):
        ...
        kwargs_repr = ''.join(', {key}={value!r}'.format(key=key, value=value) for key, value in kwargs.items())
        return '{classname}({data!r}{kwargs})'.format(classname=(self.__class__.__name__),
          data=(self.data),
          kwargs=kwargs_repr)
...
class ServerSentEventsBlueprint(object):
    ...
    def stream(self, channel=None):
        ...
        return current_app.response_class((generator()),
          mimetype='text/event-stream',
          headers={'X-Accel-Buffering':'no', 
         'chunked_transfer_encoding':'off',  'Cache-Control':'no-cache'})
