...
# [GCC 8.4.0]
# Embedded file name: e:\opt\private\cw1427\fab-admin\fab_admin\addon\confcenter\views_confcenter.py
# Compiled at: 2020-02-15 09:40:34
...
class ConfCenterView(BaseView):
    __doc__ = 'The Config center fab view module.'
    route_base = '/conf'
    ...
    def add_project(self, project=None):
        ...
        if project:
            ...
            return (
             jsonify({'message': 'project parameter wrong'}), 400)
            ...
            if self._check_conf_permission(project, 'write'):
                ...
                if data.get('name') and data.get('value'):
                    if not self._check_conf_exists(project, data['name']):
                        ...
                        pipe.execute()
                        log.info('Successfully add config {c} for project {p} by {u}'.format(c=(data.get('name')), p=project,
                          u=(g.user.username)))
                        return jsonify({'message': 'success'})
                    else:
                        return (
                         jsonify({'message': 'Key exists'}), 400)
                else:
                    return (
                     jsonify({'message': 'Wrong parameter data'}), 400)
            else:
                return (
                 jsonify({'message': "doesn't have permission"}), 403)
        else:
            return (
             jsonify({'message': 'project does not exists'}), 400)
    ...
    def update_configs_byproject(self, project):
        ...
        if redis_slave.sismember(CONF_PROJECTS_KEY_FORMAT, project.strip()):
                ...
                if data.get('name') and data.get('value'):
                    if self._check_conf_exists(project, data['name']):
                        ...
                        if redis_slave.exists(CONF_KEY_HISTORY_LIST_FORMAT.format(p=project, k=(data.get('name')))):
                            redis_master.jsonarrinsert(CONF_KEY_HISTORY_LIST_FORMAT.format(p=project, k=(data.get('name'))), '.', 0, ori_item)
                        else:
                            redis_master.jsonset(CONF_KEY_HISTORY_LIST_FORMAT.format(p=project, k=(data.get('name'))), '.', [ori_item])
                        try:
                            value = json.loads(data.get('value'))
                        except Exception:
                            value = data.get('value')
                        ...
                        pipe.jsonset(CONF_KEY_FORMAT.format(p=project, k=(data.get('name'))), '.changed_by', g.user.username)
                        pipe.execute()
                        log.info('Successfully update config {c} for project {p} by {u}'.format(c=(data.get('name')),
                          p=project,
                          u=(g.user.username)))
                        return jsonify({'message': 'success'})
                    else:
                        return (
                         jsonify({'message': 'Key does not exists'}), 400)
                else:
                    return (
                     jsonify({'message': 'Wrong parameter data'}), 400)
            else:
            ...
            return (
             jsonify({'message': "project doesn't exists"}), 400)
    ...
    def get_config_byproject(self, project, name):
        """
        {
            "desc": "RESTFul api for config retrieve by transfer project name the key name in the url <br/>                      1. {project} parameter is the config project name, <br/>                      2. {name} parameter is the key name <br/>                      3. The response data is what the config data setup, please refer the {Sample data} Tag <br/>                      4. It will support for the json data subnode(json path) searching soon",
            ...
            if self._check_conf_permission(project, 'read'):
                try:
                    ...
                    if value:
                        ...
                        return (
                         jsonify({'message': 'key does not exists'}), 404)
        ...
        if redis_slave.sismember(CONF_PROJECTS_KEY_FORMAT, project.strip()):
            ...
            if data.get('perm') and data.get('users'):
                if data.get('perm') == 'write':
                    ...
                    if data.get('perm') == 'read':
                        conf_proj_perm_key = CONF_PROJECT_READ_PERM_FORMAT
                    else:
                        return (
                         jsonify({'message': 'wrong permission params'}), 400)
                pipe = redis_master.pipeline()
                pipe.delete(conf_proj_perm_key.format(p=project))
                (pipe.sadd)(conf_proj_perm_key.format(p=project), *data.get('users'))
                pipe.execute()
                log.info('Successfully update project permission for project {p} by {u}'.format(p=project, u=(g.user.username)))
                return jsonify({'message': 'success'})
            else:
                return (
                 jsonify({'message': 'wrong parameters'}), 400)
        else:
            return (
             jsonify({'message': "project doesn't exists"}), 400)
    ...
    def del_config_byproject(self, project, name):
        ...
        if redis_slave.sismember(CONF_PROJECTS_KEY_FORMAT, project.strip()):
            if self._check_conf_permission(project, 'write'):
                try:
                    del_keys = [
                     CONF_KEY_FORMAT.format(p=project, k=name),
                    ...
                    pipe.execute()
                    log.info('Successfully delete project config for project {p} by {u}'.format(p=project, u=(g.user.username)))
                    return jsonify({'message': 'success'})
                except Exception as e:
                    log.error(e)
            ...
            if self._check_conf_permission(project, 'write'):
                try:
                    ...
                    for name in keys:
                        del_keys = [
                         CONF_KEY_FORMAT.format(p=project, k=name),
                         CONF_KEY_HISTORY_LIST_FORMAT.format(p=project,
                           k=name)]
                        (redis_master.delete)(*del_keys)

                    redis_master.delete(CONF_PROJECT_KEYSET_FORMAT.format(p=project), CONF_PROJECT_WRITE_PERM_FORMAT.format(p=project), CONF_PROJECT_READ_PERM_FORMAT.format(p=project))
                    redis_master.srem(CONF_PROJECTS_KEY_FORMAT, project)
                    log.info('Successfully delete project byname for project {p} by {u}'.format(p=project, u=(g.user.username)))
                    return jsonify({'message': 'success'})
                except Exception as e:
                    log.error(e)
                    return (jsonify({'message': 'delete item  error= {0}'.format(e)}), 500)
