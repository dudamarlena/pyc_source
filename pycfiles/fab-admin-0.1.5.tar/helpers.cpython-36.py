...
# [GCC 8.4.0]
# Embedded file name: e:\opt\private\cw1427\fab-admin\fab_admin\fab_manager_overwrite\rq/cli/helpers.py
# Compiled at: 2019-06-01 02:57:09
...
def get_redis_from_config(settings, connection_class=Redis):
    ...
    if settings.get('REDIS_URL') is not None:
        ...
        if settings.get('SENTINEL') is not None:
        ...
        kwargs = {'host':settings.get('REDIS_HOST', 'localhost'), 
...
def state_symbol(state):
    symbols = {WorkerStatus.BUSY: red('busy'), 
...
class CliConfig(object):
    __doc__ = 'A helper class to be used with click commands, to handle shared options'

    def __init__(self, url=None, config=None, worker_class=DEFAULT_WORKER_CLASS, job_class=DEFAULT_JOB_CLASS, queue_class=DEFAULT_QUEUE_CLASS, connection_class=DEFAULT_CONNECTION_CLASS, path=None, client=None, *args, **kwargs):
        ...
        if client:
            try:
                ...
                raise click.BadParameter((str(exc)), param_hint='--client')
