...
# [GCC 8.4.0]
# Embedded file name: e:\opt\private\cw1427\fab-admin\fab_admin\addon\queue\views_queues.py
# Compiled at: 2020-02-17 02:47:36
...
import rq_dashboard, rq_scheduler_dashboard
...
class QueuesView(BaseView):
    __doc__ = 'Queues Viewmodel class'
    route_base = '/queue'
    ...
    def at_requests(self, at):
        """
        {
            "desc": "AT request tasks entrance point. <br/>             It would provide at (linux command) feature to help you reserve a request event by your parameter.<br/>             It needs a json data have (url,method,auth,header,data) properties. <br/>             The auth and header are optional property. <br/>             Parameter:  at=20190502133000 it means when at that time the task would occurred. <br/>             The at parameter format should be: %Y%m%d%H%M%S",
        ...
        try:
            ...
            (schedule_requests_task.schedule)(at, data['url'], data['method'], basic_auth, header, job_id=job_id, **data['data'])
        except Exception as e:
            log.error(e)
...
def check_auth():
    ...
    if not current_user.is_authenticated:
        ...
        return (
         render_template('403.html', base_template=(appbuilder.base_template), appbuilder=appbuilder), 403)
