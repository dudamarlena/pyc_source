...
# [GCC 8.4.0]
# Embedded file name: e:\opt\private\cw1427\fab-admin\fab_admin\fab_manager_overwrite\rq/cli/cli.py
# Compiled at: 2019-06-01 02:55:42
...
from rq.cli.helpers import read_config_file, refresh, setup_loghandlers_from_args, show_both, show_queues, show_workers, CliConfig
from rq.contrib.legacy import cleanup_ghosts
from rq.defaults import DEFAULT_CONNECTION_CLASS, DEFAULT_JOB_CLASS, DEFAULT_QUEUE_CLASS, DEFAULT_WORKER_CLASS, DEFAULT_RESULT_TTL, DEFAULT_WORKER_TTL, DEFAULT_JOB_MONITORING_INTERVAL, DEFAULT_LOGGING_FORMAT, DEFAULT_LOGGING_DATE_FORMAT
...
shared_options = [
 click.option('--url', '-u', envvar='RQ_REDIS_URL',
   help='URL describing Redis connection details.'),
 click.option('--config', '-c', envvar='RQ_CONFIG',
   help='Module containing RQ settings.'),
 click.option('--worker-class', '-w', envvar='RQ_WORKER_CLASS',
   default=DEFAULT_WORKER_CLASS,
   help='RQ Worker class to use'),
 click.option('--job-class', '-j', envvar='RQ_JOB_CLASS',
   default=DEFAULT_JOB_CLASS,
   help='RQ Job class to use'),
 click.option('--queue-class', envvar='RQ_QUEUE_CLASS',
   default=DEFAULT_QUEUE_CLASS,
   help='RQ Queue class to use'),
 click.option('--connection-class', envvar='RQ_CONNECTION_CLASS',
   default=DEFAULT_CONNECTION_CLASS,
   help='Redis client class to use'),
 click.option('--path', '-P', default='.',
   help='Specify the import path.',
   multiple=True),
 click.option('--client', '-C', envvar='RQ_CLIENT',
   help='Redis client obj.')]
...
def empty(cli_config, all, queues, **options):
    """Empty given queues."""
    if all:
        queues = cli_config.queue_class.all(connection=(cli_config.connection), job_class=(cli_config.job_class))
    else:
        queues = [cli_config.queue_class(queue, connection=(cli_config.connection), job_class=(cli_config.job_class)) for queue in queues]
    if not queues:
...
def requeue(cli_config, queue, all, job_class, job_ids, **options):
    """Requeue failed jobs."""
    failed_job_registry = FailedJobRegistry(queue, connection=(cli_config.connection))
    if all:
...
def info(cli_config, interval, raw, only_queues, only_workers, by_queue, queues, **options):
    ...
    try:
        with Connection(cli_config.connection):
            refresh(interval, func, queues, raw, by_queue, cli_config.queue_class, cli_config.worker_class)
...
@click.argument('queues', nargs=(-1))
@pass_cli_config
def worker(cli_config, burst, logging_level, name, results_ttl, worker_ttl, job_monitoring_interval, verbose, quiet, sentry_dsn, exception_handler, pid, disable_default_exception_handler, queues, log_format, date_format, **options):
    """Starts an RQ worker."""
    ...
    try:
        ...
        worker = cli_config.worker_class(queues,
          name=name, connection=(cli_config.connection), default_worker_ttl=worker_ttl,
