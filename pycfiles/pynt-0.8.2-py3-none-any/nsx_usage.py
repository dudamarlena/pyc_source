...
# Embedded file name: /Users/yfauser/Documents/Development/vmware-org/pynsxv/pynsxv/library/nsx_usage.py
# Compiled at: 2016-08-09 03:34:59
__author__ = 'yfauser'
...
def host_prep_state(session):
    ...
    for cluster in enabled_clusters:
        ...
        dfw_enabled = [ feature['enabled'] for feature in feature_status if feature['featureId'] == 'com.vmware.vshield.firewall'
                      ][0]
    ...
    for cluster_moid, cluster_name, dfw_enabled in clusters:
        ...
        hosts.extend([ (host['resource']['name'], host['resource']['scope']['name'], host['resource']['objectId'], host['resource']['scope']['id'], dfw_enabled) for host in enabled_hosts
                     ])

    prepared_hosts_count = len(hosts)
    dfw_enabled_hosts_count = len([ host for host in hosts if host[4] == 'true' ])
    return (
     prepared_hosts_count, dfw_enabled_hosts_count, hosts)
...
def calculate_socket_usage(host_list, host_info):
    ...
    return (
     nsx_socket_count, dfw_scocket_count)
...
def ls_state(session):
    ...
    return (
     len(ls_list), ls_list, len(uls_list), uls_list, len(hwgwls_list), hwgwls_list)
...
def edge_state(session):
    ...
    return (
     len(esg_list), esg_list, len(dlr_list), dlr_list)
...
def contruct_parser(subparsers):
    parser = subparsers.add_parser('usage', description='Functions to retrieve NSX-v usage statistics', help='Functions to retrieve NSX-v usage statistics')
...
def _usage_main(args):
    ...
    print 'Done'
    if args.verbose:
        print tabulate(host_list, headers=['Host name', 'Cluster name', 'Host moid', 'Cluster moid', 'DFW enabled'], tablefmt='psql'), '\n'
    ...
    if args.verbose:
        print tabulate(edge_feature_list, headers=[
         'Edge service gw name', 'Edge service gw Id', 'Loadbalancer',
