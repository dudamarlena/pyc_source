...
# Embedded file name: /Users/yfauser/Documents/Development/vmware-org/pynsxv/pynsxv/library/nsx_esg.py
# Compiled at: 2016-08-09 03:13:50
__author__ = 'yfauser'
...
def esg_create(client_session, esg_name, esg_pwd, esg_size, datacentermoid, datastoremoid, resourcepoolid, default_pg, esg_username=None, esg_remote_access=None):
    """
    ...
    esg_create_dict['edge']['cliSettings'] = {'password': esg_pwd, 'remoteAccess': esg_remote_access, 'userName': esg_username}
...
def _esg_create(client_session, vccontent, **kwargs):
    needed_params = [
     'esg_name', 'esg_size', 'esg_pwd', 'datacenter_name', 'edge_datastore', 'edge_cluster',
     'portgroup']
    if not check_for_parameters(needed_params, kwargs):
        ...
        esg_id, esg_params = esg_create(client_session, kwargs['esg_name'], kwargs['esg_pwd'], kwargs['esg_size'], datacentermoid, datastoremoid, resourcepoolid, portgroupmoid, esg_remote_access=kwargs['esg_remote_access'])
...
def esg_cfg_interface(client_session, esg_name, ifindex, ipaddr=None, netmask=None, prefixlen=None, name=None, mtu=None, is_connected=None, portgroup_id=None, vnic_type=None, enable_send_redirects=None, enable_proxy_arp=None, secondary_ips=None):
    """
    ...
    if not esg_id:
        ...
        cfg_result = client_session.update('vnic', uri_parameters={'index': ifindex, 'edgeId': esg_id}, request_body_dict=vnic_config)
...
def _esg_cfg_interface(client_session, vccontent, **kwargs):
    ...
    if not check_for_parameters(needed_params, kwargs):
        ...
        result = esg_cfg_interface(client_session, kwargs['esg_name'], kwargs['vnic_index'], name=kwargs['vnic_name'], vnic_type=kwargs['vnic_type'], portgroup_id=portgroup, is_connected=kwargs['vnic_state'], ipaddr=kwargs['vnic_ip'], netmask=netmask, prefixlen=prefixlen, secondary_ips=secondary_ips)
        if result:
...
def _esg_clear_interface(client_session, **kwargs):
    ...
    if not check_for_parameters(needed_params, kwargs):
        ...
        if result:
            print ('Edge Services Router {} vnic{} configuration has been cleared').format(kwargs['esg_name'], kwargs['vnic_index'])
        else:
            print ('Edge Services Router {} vnic{} configuration failed').format(kwargs['esg_name'], kwargs['vnic_index'])
        return
...
def esg_dgw_set(client_session, esg_name, dgw_ip, vnic, mtu=None, admin_distance=None):
    ...
    if not esg_id:
        ...
        rtg_cfg['staticRouting']['defaultRoute'] = {'vnic': vnic, 'gatewayAddress': dgw_ip, 'adminDistance': admin_distance, 
           'mtu': mtu}
        cfg_result = client_session.update('routingConfigStatic', uri_parameters={'edgeId': esg_id}, request_body_dict=rtg_cfg)
        if cfg_result['status'] == 204:
            return True
        return False
...
def esg_dgw_read(client_session, esg_name):
    ...
    return (
     dgw_cfg_tpl, rtg_cfg['staticRouting']['defaultRoute'])
...
def esg_route_add(client_session, esg_name, network, next_hop, vnic=None, mtu=None, admin_distance=None, description=None):
    """
    ...
    if not esg_id:
        ...
        if rtg_cfg['staticRouting']['staticRoutes']:
            ...
            routes = []
        new_route = {'vnic': vnic, 'network': network, 'nextHop': next_hop, 'adminDistance': admin_distance, 'mtu': mtu, 
...
def _esg_route_add(client_session, **kwargs):
    needed_params = [
     'esg_name', 'next_hop', 'route_net']
    if not check_for_parameters(needed_params, kwargs):
        return
    else:
        result = esg_route_add(client_session, kwargs['esg_name'], kwargs['route_net'], kwargs['next_hop'], vnic=kwargs['vnic_index'])
        if result:
...
def _esg_route_del(client_session, **kwargs):
    ...
    if not check_for_parameters(needed_params, kwargs):
        ...
        if result:
            print ('Deletion of route {} on Edge Services Router {} succeeded').format(kwargs['route_net'], kwargs['esg_name'])
        else:
...
def esg_route_list(client_session, esg_name):
    ...
    for route in routes_api:
        if 'vnic' in route.keys():
            ...
            vnic = ''
        add_route = (
         route['network'], route['nextHop'], vnic, route['adminDistance'], route['mtu'])
        routes.append(add_route)

    return (
     routes, rtg_cfg['staticRouting']['staticRoutes'])
...
def esg_fw_default_set(client_session, esg_name, def_action, logging_enabled=None):
    ...
    if not esg_id:
        ...
        cfg_result = client_session.update('defaultFirewallPolicy', uri_parameters={'edgeId': esg_id}, request_body_dict=def_policy_body)
        if cfg_result['status'] == 204:
            return True
        return False
...
def _esg_fw_default_set(client_session, **kwargs):
    ...
    if not check_for_parameters(needed_params, kwargs):
        ...
        if result:
            print ('Default firewall policy on Edge Services Router {} set to {}').format(kwargs['esg_name'], kwargs['fw_default'])
        else:
...
def contruct_parser(subparsers):
    parser = subparsers.add_parser('esg', description="nsxv function for edge services gateway'%(prog)s @params.conf'.", help='Functions for edge services gateways', formatter_class=RawTextHelpFormatter)
    ...
    parser.add_argument('-n', '--esg_name', help='ESG name')
    parser.add_argument('-p', '--esg_password', help="ESG admin password, default is 'VMware1!VMware1!'", default='VMware1!VMware1!')
    parser.add_argument('-s', '--esg_size', help='ESG size (compact, large, quadlarge, xlarge), default is compact', default='compact')
    parser.add_argument('-r', '--esg_remote_access', help='ESG state of remote access (SSH) (True = enabled, False = disabled), default is False', default='false')
    parser.add_argument('-pg', '--portgroup', help='ESG portgroup name for vnic related operations and for the first vnic pg of esg creation')
    parser.add_argument('-ls', '--logical_switch', help='ESG logical switch for vnic related operations and for the first vnic of esg creation')
    parser.add_argument('-vi', '--vnic_index', help='vnic Id when configuring Vnic interfaces (0 to 9)')
    parser.add_argument('-vt', '--vnic_type', help='vnic type when configuring Vnic interfaces (internal or uplink)')
    parser.add_argument('-vn', '--vnic_name', help='vnic name when configuring Vnic interfaces (internal or uplink)')
    parser.add_argument('-vs', '--vnic_state', help='vnic connection state (true/false) Default is true', default='true')
    parser.add_argument('-ip', '--vnic_ip', help='vnic IP Address')
    parser.add_argument('-sip', '--vnic_secondary_ips', help="vnic secondary IP Addresses (comma separated list of IPs, e.g. '192.168.178.10,192.168.178.10')")
    parser.add_argument('-m', '--vnic_mask', help='vnic subnet mask or size (e.g. 255.255.255.0 or 24)')
    parser.add_argument('-gw', '--next_hop', help='ESG default gateway or static route next hop')
    parser.add_argument('-rt', '--route_net', help='Network the static route points to in the x.x.x.x/yy format, e.g. 192.168.1.0/24')
    parser.add_argument('-fw', '--fw_default', help='ESG firewall default rule action (accept/deny)')
    parser.add_argument('-dc', '--datacenter_name', help='vCenter DC name to deploy ESGs in, default is taken from INI File')
    parser.add_argument('-ds', '--edge_datastore', help='Datastore name to deploy ESGs in, default is taken from INI File')
    parser.add_argument('-cl', '--edge_cluster', help='vCenter Cluster or Ressource Pool to deploy ESGs in, default is taken from INI File')
...
def _esg_main(args):
    ...
    vccontent = connect_to_vc(config.get('vcenter', 'vcenter'), config.get('vcenter', 'vcenter_user'), config.get('vcenter', 'vcenter_passwd'))
    ...
    try:
        command_selector = {'list': _esg_list_print, 'create': _esg_create, 
           'delete': _esg_delete, 
           'read': _esg_read, 
        ...
        command_selector[args.command](client_session, vccontent=vccontent, esg_name=args.esg_name, esg_pwd=args.esg_password, esg_size=args.esg_size, datacenter_name=datacenter_name, edge_datastore=edge_datastore, edge_cluster=edge_cluster, next_hop=args.next_hop, portgroup=args.portgroup, logical_switch=args.logical_switch, vnic_index=args.vnic_index, vnic_type=args.vnic_type, vnic_name=args.vnic_name, vnic_state=args.vnic_state, vnic_ip=args.vnic_ip, vnic_mask=args.vnic_mask, route_net=args.route_net, fw_default=args.fw_default, esg_remote_access=args.esg_remote_access, vnic_secondary_ips=args.vnic_secondary_ips, verbose=args.verbose)
