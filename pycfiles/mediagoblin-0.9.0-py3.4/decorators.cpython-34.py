...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mediagoblin/decorators.py
# Compiled at: 2016-03-29 15:18:42
# Size of source mod 2**32: 14320 bytes
...
from mediagoblin.db.models import MediaEntry, LocalUser, TextComment, AccessToken, Comment
from mediagoblin.tools.response import redirect, render_404, render_user_banned, json_response
...
def require_active_login(controller):
    ...
    def new_controller_func(request, *args, **kwargs):
        if request.user and not request.user.has_privilege('active'):
            return redirect(request, 'mediagoblin.user_pages.user_home', user=request.user.username)
        if not request.user or not request.user.has_privilege('active'):
...
def active_user_from_url(controller):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        return controller(request, url_user=user, *args, **kwargs)
...
def user_may_delete_media(controller):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        if not (request.user.has_privilege('admin') or request.user.id == uploader_id):
            raise Forbidden()
        return controller(request, *args, **kwargs)
...
def user_may_alter_collection(controller):
    ...
    @wraps(controller)
    def wrapper(request, *args, **kwargs):
        creator_id = request.db.LocalUser.query.filter_by(username=request.matchdict['user']).first().id
        if not (request.user.has_privilege('admin') or request.user.id == creator_id):
            raise Forbidden()
        return controller(request, *args, **kwargs)
...
def get_user_media_entry(controller):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        if media_slug.startswith('id:'):
            try:
                media = MediaEntry.query.filter_by(id=int(media_slug[3:]), state='processed', actor=user.id).first()
            except ValueError:
                raise NotFound()

        else:
            media = MediaEntry.query.filter_by(slug=media_slug, state='processed', actor=user.id).first()
        if not media:
            raise NotFound()
...
def get_user_collection(controller):
    ...
    @wraps(controller)
    def wrapper(request, *args, **kwargs):
        user = request.db.LocalUser.query.filter_by(username=request.matchdict['user']).first()
        if not user:
            return render_404(request)
        collection = request.db.Collection.query.filter_by(slug=request.matchdict['collection'], actor=user.id).first()
        if not collection:
            return render_404(request)
...
def get_user_collection_item(controller):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        if not user:
            return render_404(request)
        collection_item = request.db.CollectionItem.query.filter_by(id=request.matchdict['collection_item']).first()
...
def get_media_entry_by_id(controller):
    ...
    @wraps(controller)
    def wrapper(request, *args, **kwargs):
        media = MediaEntry.query.filter_by(id=request.matchdict['media_id'], state='processed').first()
        if not media:
            return render_404(request)
...
def get_workbench(func):
    ...
    def new_func(*args, **kwargs):
        with mgg.workbench_manager.create() as (workbench):
            return func(workbench=workbench, *args, **kwargs)
...
def allow_registration(controller):
    ...
    def wrapper(request, *args, **kwargs):
        if not mgg.app_config['allow_registration']:
            messages.add_message(request, messages.WARNING, _('Sorry, registration is disabled on this instance.'))
...
def allow_reporting(controller):
    ...
    def wrapper(request, *args, **kwargs):
        if not mgg.app_config['allow_reporting']:
            messages.add_message(request, messages.WARNING, _('Sorry, reporting is disabled on this instance.'))
            return redirect(request, 'index')
        return controller(request, *args, **kwargs)
...
def get_optional_media_comment_by_id(controller):
    ...
    def wrapper(request, *args, **kwargs):
        if 'comment' in request.matchdict:
            comment = Comment.query.filter_by(id=request.matchdict['comment']).first()
            if comment is None:
                return render_404(request)
...
def auth_enabled(controller):
    ...
    def wrapper(request, *args, **kwargs):
        if not mgg.app.auth:
            messages.add_message(request, messages.WARNING, _('Sorry, authentication is disabled on this instance.'))
            return redirect(request, 'index')
        return controller(request, *args, **kwargs)
...
def require_admin_or_moderator_login(controller):
    ...
    @wraps(controller)
    def new_controller_func(request, *args, **kwargs):
        if request.user and not (request.user.has_privilege('admin') or request.user.has_privilege('moderator')):
            raise Forbidden()
...
def oauth_required(controller):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        valid, r = resource_endpoint.validate_protected_resource_request(uri=request.url, http_method=request.method, body=request.data, headers=dict(request.headers))
        if not valid:
