...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mediagoblin/tests/test_reporting.py
# Compiled at: 2016-03-29 15:18:42
# Size of source mod 2**32: 6542 bytes
import pytest, six
from mediagoblin.tools import template
from mediagoblin.tests.tools import fixture_add_user, fixture_media_entry, fixture_add_comment, fixture_add_comment_report
...
class TestReportFiling:
    ...
    def _setup(self, test_app):
        ...
        fixture_add_user('allie', privileges=[
         'reporter', 'active'])
    ...
    def query_for_users(self):
        return (
         LocalUser.query.filter(LocalUser.username == 'allie').first(),
    ...
    def testMediaReports(self):
        ...
        media_entry = fixture_media_entry(uploader=natalie_user.id, state='processed')
        mid = media_entry.id
        media_uri_slug = '/u/{0}/m/{1}/'.format(natalie_user.username, media_entry.slug)
        response = self.test_app.get(media_uri_slug + 'report/')
        assert response.status == '200 OK'
        response, context = self.do_post({'report_reason': 'Testing Media Report',  'reporter_id': six.text_type(allie_id)}, url=media_uri_slug + 'report/')
    ...
    def testCommentReports(self):
        ...
        comment_uri_slug = '/u/{0}/m/{1}/c/{2}/'.format(natalie_user.username, media_entry.slug, comment.id)
        response = self.test_app.get(comment_uri_slug + 'report/')
        assert response.status == '200 OK'
        response, context = self.do_post({'report_reason': 'Testing Comment Report', 
    ...
    def testArchivingReports(self):
        ...
        fixture_add_comment(author=allie_user.id, comment='Comment will be removed')
        test_comment = TextComment.query.filter(TextComment.actor == allie_user.id).first()
        fixture_add_comment_report(comment=test_comment, reported_user=allie_user, report_content='Testing Archived Reports #1', reporter=natalie_user)
        comment_report = Report.query.filter(Report.reported_user == allie_user).first()
        assert comment_report.report_content == 'Testing Archived Reports #1'
        response, context = self.do_post({'action_to_resolve': ['userban', 'delete'],  'targeted_user': allie_user.id, 
         'resolution_content': 'This is a test of archiving reports.'}, url='/mod/reports/{0}/'.format(comment_report.id))
        assert response.status == '302 FOUND'
        allie_user, natalie_user = self.query_for_users()
        archived_report = Report.query.filter(Report.reported_user == allie_user).first()
