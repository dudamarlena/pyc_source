...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mediagoblin/tests/test_storage.py
# Compiled at: 2016-03-29 15:18:42
# Size of source mod 2**32: 10962 bytes
import os, tempfile, pytest, six
from werkzeug.utils import secure_filename
from mediagoblin import storage

def test_clean_listy_filepath():
    expected = [
     'dir1', 'dir2', 'linooks.jpg']
    assert storage.clean_listy_filepath([
     'dir1', 'dir2', 'linooks.jpg']) == expected
    expected = [
     'dir1', 'foo_.._nasty', 'linooks.jpg']
    assert storage.clean_listy_filepath([
...
class FakeRemoteStorage(storage.filestorage.BasicFileStorage):
    ...
    def copy_local_to_storage(self, *args, **kwargs):
        return storage.StorageInterface.copy_local_to_storage(self, *args, **kwargs)


def test_storage_system_from_config():
    this_storage = storage.storage_system_from_config({'base_url': 'http://example.org/moodia/',  'base_dir': '/tmp/', 
    ...
    this_storage = storage.storage_system_from_config({'foobie': 'eiboof',  'blech': 'hcelb', 
     'garbage_arg': 'garbage_arg', 
     'storage_class': 'mediagoblin.tests.test_storage:FakeStorageSystem'})
    assert this_storage.foobie == 'eiboof'
    assert this_storage.blech == 'hcelb'
    assert six.text_type(this_storage.__class__) == "<class 'mediagoblin.tests.test_storage.FakeStorageSystem'>"
...
def test_basic_storage__resolve_filepath():
    ...
    assert result == os.path.join(tmpdir, 'dir1/dir2/filename.jpg')
    result = this_storage._resolve_filepath(['../../etc/', 'passwd'])
    assert result == os.path.join(tmpdir, 'etc/passwd')
    pytest.raises(storage.InvalidFilepath, this_storage._resolve_filepath, [
     '../../', 'etc', 'passwd'])
...
def test_basic_storage_get_file():
    ...
    manually_written_file = os.makedirs(os.path.join(tmpdir, 'testydir'))
...
def test_basic_storage_delete_file():
    tmpdir, this_storage = get_tmp_filestorage()
    assert not os.path.exists(os.path.join(tmpdir, 'dir1/dir2/ourfile.txt'))
    filepath = [
     'dir1', 'dir2', 'ourfile.txt']
    with this_storage.get_file(filepath, 'w') as (our_file):
...
def test_basic_storage_url_for_file():
    tmpdir, this_storage = get_tmp_filestorage()
    pytest.raises(storage.NoWebServing, this_storage.file_url, [
     'dir1', 'dir2', 'filename.txt'])
    cleanup_storage(this_storage, tmpdir)
    ...
    cleanup_storage(this_storage, tmpdir)
    tmpdir, this_storage = get_tmp_filestorage('http://media.example.org/ourmedia/')
    result = this_storage.file_url([
     'dir1', 'dir2', 'filename.txt'])
...
def _test_copy_local_to_storage_works(tmpdir, this_storage):
    ...
    with open(local_filename, 'w') as (tmpfile):
        tmpfile.write('haha')
    this_storage.copy_local_to_storage(local_filename, ['dir1', 'dir2', 'copiedto.txt'])
    os.remove(local_filename)
    assert open(os.path.join(tmpdir, 'dir1/dir2/copiedto.txt'), 'r').read() == 'haha'
