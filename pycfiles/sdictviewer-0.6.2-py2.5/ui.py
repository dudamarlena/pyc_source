...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sdictviewer/ui.py
# Compiled at: 2008-05-07 23:04:01
"""
...
import pygtk
pygtk.require('2.0')
import gtk, gobject, pango, dictutil, dictinfo, articleformat, xml.sax.saxutils, re, time, webbrowser
from appstate import *
from threading import Thread
from Queue import Queue
from math import fabs
from sdictviewer import detect_format, ucollator
from collections import defaultdict
from gtk.gdk import _2BUTTON_PRESS, _3BUTTON_PRESS, BUTTON_PRESS, MOTION_NOTIFY, BUTTON_RELEASE, NO_EXPOSE, EXPOSE, VISIBILITY_NOTIFY
...
class SDictViewer(object):
    ...
    def is_link_click(self, widget, event, reference):
        if event.type == BUTTON_PRESS:
            widget.armed_link = (
             reference, event.get_coords(), event.get_time())
            ...
            if event.type == BUTTON_RELEASE and widget.armed_link:
                ...
                result = dt <= 200 and dt >= 50 and fabs(x - x0) <= 3 and fabs(y - y0) <= 3 and armed_ref is reference
                return result
        return
    ...
    def do_lookup(self, start_word, to_select):
        ...
        return (
         lang_word_list, interrupted)

    def sort_key(self, word_lookup):
        return ucollator.sort_key(str(word_lookup), maxlevel=3)
    ...
    def get_word_from_wordlookup(self, treeviewcolumn, cell_renderer, model, iter):
        cell_renderer.set_property('text', str(model[iter][0]))

    def create_word_input(self):
        word_input = gtk.ComboBoxEntry(gtk.TreeStore(str, str))
        try:
            word_input.child.set_property('hildon-input-mode', 'full')
        except:
            print 'Hildon Input Mode is not available'
