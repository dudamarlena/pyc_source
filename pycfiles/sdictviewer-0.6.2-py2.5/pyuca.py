...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sdictviewer/pyuca.py
...
"""
import unicodedata, types, sys, os
    ...
    def find_prefix(self, key):
        ...
        if remainder == key:
            return (
             None, remainder[1:])
        return (
         curr_node[0], remainder)


class Collator:

    def __init__(self, filename, normalize='NFC'):
        self.table = Trie()
        self.load(self.findfile(filename))
        self.normalize = normalize

    def findfile(self, filename):
        for dirname in [os.path.dirname(__file__)] + sys.path:
    ...
    def sort_key(self, string, encoding='utf-8', maxlevel=0):
        if type(string) is types.StringType:
            string = string.decode(encoding)
        string = unicodedata.normalize(self.normalize, string)
