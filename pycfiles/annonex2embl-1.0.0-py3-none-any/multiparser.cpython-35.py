...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/multiparser.py
# Compiled at: 2020-01-22 12:06:14
# Size of source mod 2**32: 18259 bytes
...
class Multiparser(object):
        ...
        for folder in os.listdir(ref_folder):
            ...
            if '_folder' in folder:
                ...
                if ref_feature == '':
                    ...
                    if ref_feature == '_fasta':
                        if datas[0].endswith('.fa'):
                            prefix = datas[0][:-3]
                        else:
                            if datas[0].endswith('.fna'):
                                prefix = datas[0][:-4]
                    ...
                    for tar in os.listdir(tar_folder):
                        if tar.endswith('.fa') or tar.endswith('.fna') or tar.endswith('.fasta'):
                            filename = '.'.join(tar.split('.')[:-1])
                            for file_ in files:
                                if filename == file_:
                                    self.helper.merge_file(os.path.join(tar_folder, tar), os.path.join(tar_folder, self.tmp_fa))
                                    change = True

                    if change:
                        change = False
                        shutil.move(os.path.join(tar_folder, self.tmp_fa), os.path.join(tar_merge, prefix + '.fa'))
                        ...
                        if tar.endswith('.wig') and file_ == filename[(-1)][:-4]:
                            for lib in libs:
                                if filename[0] in lib and lib[(-1)] == '+':
                                    self.helper.merge_file(os.path.join(tar_folder, tar), os.path.join(tar_folder, self.tmp_wig_forward))
                                    change_f = True
                                elif filename[0] in lib and lib[(-1)] == '-':
                                    self.helper.merge_file(os.path.join(tar_folder, tar), os.path.join(tar_folder, self.tmp_wig_reverse))
                                    change_r = True

                if change_f and change_r:
                    change_f = False
                    change_r = False
                    shutil.move(os.path.join(tar_folder, self.tmp_wig_forward), os.path.join(tar_merge, prefix + '_forward.wig'))
                    shutil.move(os.path.join(tar_folder, self.tmp_wig_reverse), os.path.join(tar_merge, prefix + '_reverse.wig'))
                else:
                    print('Error: comparing input files of {0} failed. Please check the seq IDs of all gff, fasta and wig files, they should be the same.\nPlease also check the wiggle files which should contain forward and reverse files.'.format(prefix))
                    sys.exit()
                    ...
                    for tar in os.listdir(tar_folder):
                        for file_ in files:
                            if '.gff' in tar and file_ + tar_feature == tar[:-4]:
                                self.helper.merge_file(os.path.join(tar_folder, tar), os.path.join(tar_folder, self.tmp_gff))
                                change = True

                    if change:
                        change = False
                        shutil.move(os.path.join(tar_folder, self.tmp_gff), os.path.join(tar_folder, 'merge_tmp', prefix + tar_feature + '.gff'))
    ...
    def parser_fasta(self, fastas):
        ...
        if not detect:
            print('Error: there are folders which conatin no fasta files! The files should end with .fa or .fna or .fasta!')
            sys.exit()
        for fasta in os.listdir(fastas):
            if '_folder' not in fasta and 'tmp' != fasta and (fasta.endswith('.fa') or fasta.endswith('.fna') or fasta.endswith('.fasta')):
                ...
                with open(os.path.join(fastas, fasta), 'r') as (f_f):
                    for line in f_f:
                        if line[0] == '>':
                            ...
                            out_t = open(os.path.join(par_tmp, name + '.fa'), 'w')
                            out.write('>' + name + '\n')
                            out_t.write('>' + name + '\n')
                        else:
                            out.write(line)
    ...
    def parser_gff(self, gff_folder, feature):
        ...
        for filename in os.listdir(gff_folder):
            ...
            if '_folder' not in filename and 'tmp' != filename:
                ...
                if '.gff' in filename:
                    ...
                    self.helper.check_make_folder(out_path)
                    self.helper.sort_gff(os.path.join(gff_folder, filename), os.path.join(gff_folder, 'tmp.gff'))
                    f_h = open(os.path.join(gff_folder, 'tmp.gff'), 'r')
                    for row in csv.reader(f_h, delimiter='\t'):
                        if row[0].startswith('#'):
                            continue
                        else:
                            if '|' in row[0] and len(row[0].split('|')) > 4:
                                strain = row[0].split('|')
                                name = strain[3]
                            else:
                                name = row[0]
                            if pre_seq_id == name:
                                ...
                                if first:
                                    ...
                                    out.close()
                                    out_t.close()
                                out = open(os.path.join(out_path, name + feature + '.gff'), 'w')
                                out_t = open(os.path.join(par_tmp, name + feature + '.gff'), 'w')
                                pre_seq_id = name
                                out.write('\t'.join([name] + row[1:]) + '\n')
                                out_t.write('\t'.join([name] + row[1:]) + '\n')
        ...
        if not detect:
            print('Error: There are folders which contain no gff3 files! The files should end with .gff!')
            sys.exit()
    ...
    def parser_wig(self, wig_folder):
                        ...
                        for line in w_f:
                            if not line.startswith('#') and len(line) != 0:
                                ...
                                if line[0] == 'variableStep' or line[0] == 'fixedStep':
                                    chrom = line[1].split('=')
                                    if '|' in chrom[1] and len(chrom[1].split('|')) > 4:
                                    ...
                                    out = open(''.join([
                                     ...
                                     '_STRAIN_', name, '.wig']), 'w')
                                    out_t = open(''.join([
                                     os.path.join(wig_folder, 'tmp', filename[:-4]),
                                     '_STRAIN_', name, '.wig']), 'w')
                                    if track_info != '':
        ...
        if not detect:
            print('Error: There are folders which contain no wig files! The files should end with .wig!')
            sys.exit()
        if out is not None:
            out.close()
        if out_t is not None:
