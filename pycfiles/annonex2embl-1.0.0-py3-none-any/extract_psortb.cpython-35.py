...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/extract_psortb.py
...
def get_results(line, scores, psortbs, out_p, seq_name, fuzzy):
    ...
    if len(line) == 0:
        ...
        if '(This protein may have multiple localization sites.)' in line:
            results = line.split(' ')
...
def get_information(psortb_table, out_p, fuzzy):
    ...
    with open(psortb_table, 'r') as (p_h):
        for line in p_h:
            line = line.strip()
            if line.startswith('--') or line.startswith('Secondary localization(s):'):
                detects['result'] = False
            if detects['score'] and 'Final Prediction:' not in line:
                datas = line.split(' ')
                scores.append({'local': datas[0], 
                 'score': float(datas[(-1)])})
            if detects['result']:
                local_name, local_score = get_results(line, scores, psortbs, out_p, seq_name, fuzzy)
...
def print_gff(gffs, psortbs, out_m):
    for gff in gffs:
        ...
        for psortb in psortbs:
            if gff.feature == 'CDS' and gff.start == psortb['start'] and gff.end == psortb['end'] and gff.strand == psortb['strand']:
                if 'protein_id' in gff.attributes.keys():
                    ...
                    if 'locus_tag' in gff.attributes.keys():
                        if gff.attributes['locus_tag'] == psortb['protein_id']:
                            detect = True
                            break
                    elif gff.attributes['ID'] == psortb['protein_id']:
                        detect = True
                        break
