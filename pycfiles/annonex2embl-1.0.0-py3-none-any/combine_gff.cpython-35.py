...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/combine_gff.py
...
def del_attributes(entry):
    ...
    if entry.feature == 'CDS' or entry.feature == 'tRNA' or entry.feature == 'rRNA' or entry.feature == 'exon':
        pass
...
def compare_tran(datas, tran, out):
    ...
    for data in datas:
        del_attributes(data)
        if data.seq_id == tran.seq_id and data.strand == tran.strand and data.start >= tran.start and data.end <= tran.end:
...
def compare_tran_term(term, tran, out, fuzzy_term):
    ...
    if term.seq_id == tran.seq_id and term.strand == tran.strand:
        if term.start >= tran.start and term.end <= tran.end:
            print_file(term, tran, out)
        else:
            if term.strand == '+':
                if term.start - fuzzy_term <= tran.end and term.end + fuzzy_term >= tran.end:
                    print_file(term, tran, out)
                elif term.start <= tran.end and term.end >= tran.end:
                    print_file(term, tran, out)
            elif term.end + fuzzy_term >= tran.start and term.start - fuzzy_term <= tran.start:
                print_file(term, tran, out)
    elif term.start <= tran.start and term.end >= tran.start:
        print_file(term, tran, out)


def combine_gff(gff_file, ta_file, tss_file, utr5_file, utr3_file, term_file, fuzzy_tss, fuzzy_term, out_file):
        ...
        if tss_file is not None:
            for tss in tsss:
                ...
                if tss.seq_id == tran.seq_id and tss.strand == tran.strand:
                    if tss.strand == '+':
                        if tss.start + fuzzy_tss >= tran.start and tss.start <= tran.end:
                            print_file(tss, tran, out)
                        else:
                            if tss.start >= tran.start and tss.end - fuzzy_tss <= tran.end:
                                print_file(tss, tran, out)
