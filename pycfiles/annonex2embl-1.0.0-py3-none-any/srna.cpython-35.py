...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/srna.py
# Compiled at: 2019-06-26 11:51:40
# Size of source mod 2**32: 55112 bytes
import os, gc, sys, shutil, time
...
class sRNADetection(object):
    ...
    def __init__(self, args_srna):
        ...
        self.prefixs = {'merge': os.path.join(args_srna.out_folder, 'tmp_merge'), 
         ...
         'merge_table': os.path.join(args_srna.out_folder, 'tmp_merge_table'), 
         
         'utr_table': os.path.join(args_srna.out_folder, 'tmp_utrsrna_table'), 
         
         'normal_table': os.path.join(args_srna.out_folder, 'tmp_normal_table'), 
         
         'in_cds_table': os.path.join(args_srna.out_folder, 'tmp_incds_table'), 
        ...
        self.table_output = os.path.join(args_srna.out_folder, 'tables')
        self.stat_path = os.path.join(args_srna.out_folder, 'statistics')
        self.all_best = {'all_gff': os.path.join(self.gff_output, 'all_candidates'), 
         
         'best_gff': os.path.join(self.gff_output, 'best_candidates'), 
         
         'all_table': os.path.join(self.table_output, 'all_candidates'), 
         
         'best_table': os.path.join(self.table_output, 'best_candidates')}
    ...
    def _formatdb(self, database, type_, out_folder, blastdb, database_type, log):
        ...
        err.close()
        if database.endswith('.fa') or database.endswith('.fna') or database.endswith('.fasta'):
    ...
    def _merge_frag_tex_file(self, files, args_srna):
        ...
        if args_srna.frag_wigs is not None and args_srna.tex_wigs is not None:
    ...
    def _run_normal(self, prefix, gff, tran, fuzzy_tss, args_srna, log):
        ...
        if self.tss_path is not None:
            if 'TSS_classes' in os.listdir(args_srna.out_folder) and not args_srna.source:
                tss = os.path.join(args_srna.out_folder, 'TSS_classes', prefix + '_TSS.gff')
            else:
                tss = self.helper.get_correct_file(self.tss_path, '_TSS.gff', prefix, None, None)
        else:
            tss = None
        if self.pro_path is not None:
            pro = self.helper.get_correct_file(self.pro_path, '_processing.gff', prefix, None, None)
        else:
            pro = None
        if args_srna.frag_wigs is not None:
            files['frag_gff'] = os.path.join(args_srna.out_folder, '_'.join(['tmp_frag', prefix]))
            files['frag_csv'] = os.path.join(args_srna.out_folder, '_'.join(['tmp_frag_table', prefix]))
            args_srna = self.args_container.container_intersrna('frag', files, args_srna, prefix, os.path.join(args_srna.gffs, gff), tran, tss, pro, fuzzy_tss)
            frag_datas = self._read_lib_wig(args_srna)
            log.write('Running sRNA_intergenic.py to detecting intergenic sRNA for {0} based on fragmented libs.\n'.format(prefix))
            intergenic_srna(args_srna, frag_datas[0], frag_datas[1], frag_datas[2], frag_datas[3], tss)
        if args_srna.tex_wigs is not None:
            files['tex_gff'] = os.path.join(args_srna.out_folder, '_'.join(['tmp_tex', prefix]))
            files['tex_csv'] = os.path.join(args_srna.out_folder, '_'.join(['tmp_tex_table', prefix]))
            args_srna = self.args_container.container_intersrna('tex', files, args_srna, prefix, os.path.join(args_srna.gffs, gff), tran, tss, pro, fuzzy_tss)
            tex_datas = self._read_lib_wig(args_srna)
            log.write('Running sRNA_intergenic.py to detecting intergenic sRNA for {0} based on dRNA-Seq libs.\n'.format(prefix))
            intergenic_srna(args_srna, tex_datas[0], tex_datas[1], tex_datas[2], tex_datas[3], tss)
        ...
        return (
         tss, frag_datas, tex_datas)

    def _run_utrsrna(self, gff, tran, prefix, tss, pro, args_srna, frag_datas, tex_datas, log):
        ...
        files = {'frag_gff': None, 'frag_csv': None, 
         ...
         'merge_gff': None, 'merge_csv': None}
        if args_srna.tex_wigs is not None:
            files['tex_gff'] = os.path.join(args_srna.out_folder, '_'.join(['tmp_utr_tex', prefix]))
            files['tex_csv'] = os.path.join(args_srna.out_folder, '_'.join(['tmp_utr_tex_table', prefix]))
            args_srna = self.args_container.container_utrsrna(os.path.join(args_srna.gffs, gff), tran, tss, files, pro, os.path.join(self.fasta_path, prefix + '.fa'), 'tex', prefix, args_srna)
            log.write('Running sRNA_utr_derived.py to detect UTR-derived sRNAs for {0} based on dRNA-Seq data.\n'.format(prefix))
            utr_derived_srna(args_srna, tex_datas[0], tex_datas[1], tex_datas[2], tex_datas[3])
        if args_srna.frag_wigs is not None:
            files['frag_gff'] = os.path.join(args_srna.out_folder, '_'.join(['tmp_utr_frag', prefix]))
            files['frag_csv'] = os.path.join(args_srna.out_folder, '_'.join(['tmp_utr_frag_table', prefix]))
            args_srna = self.args_container.container_utrsrna(os.path.join(args_srna.gffs, gff), tran, tss, files, pro, os.path.join(self.fasta_path, prefix + '.fa'), 'frag', prefix, args_srna)
            log.write('Running sRNA_utr_derived.py to detect UTR-derived sRNAs for {0} based on fragmented libs.\n'.format(prefix))
            utr_derived_srna(args_srna, frag_datas[0], frag_datas[1], frag_datas[2], frag_datas[3])
        files['merge_csv'] = '_'.join([self.prefixs['utr_table'], prefix])
        files['merge_gff'] = '_'.join([self.prefixs['utr'], prefix])
        self._merge_frag_tex_file(files, args_srna)
        log.write('Running sRNA_filter_min_utr.py to filter out the UTR-derived sRNAs which length is too short.\n')
    ...
    def _check_database(self, formatdb, database):
        if formatdb:
            ...
            for fasta in os.listdir(folder):
                if (fasta.endswith('.fa') or fasta.endswith('.fna') or fasta.endswith('.fasta')) and '.'.join(fasta.split('.')[:-1]) == filename:
                    database = os.path.join(folder, fasta)
                    return database

        else:
            return database
        print('Error: The nr database or sRNA database is not in fasta format or the file name does not end with .fa or .fna or .fasta!')
        sys.exit()

    def _check_necessary_file(self, args_srna, log):
        if args_srna.gffs is None or args_srna.trans is None or args_srna.tex_wigs is None and args_srna.frag_wigs is None:
            print('Error: Lack required files!')
            log.write('The annotation gff files, transcipt files, or wiggle files do not be assigned.\n')
            sys.exit()
        if args_srna.utr_srna:
            if args_srna.tss_folder is None:
                print('Error: Lack required TSS files for UTR derived sRNA detection!')
                log.write('TSS files are required for detecting UTR-derived sRNAs.\n')
                sys.exit()
            ...
            args_srna.nr_database = self._check_database(args_srna.nr_format, args_srna.nr_database)
            args_srna.srna_database = self._check_database(args_srna.srna_format, args_srna.srna_database)
            if args_srna.tss_folder is not None:
                ...
                self.multiparser.combine_gff(args_srna.gffs, self.tss_path, None, 'TSS')
            if args_srna.pro_folder is not None:
                ...
                self.multiparser.combine_gff(args_srna.gffs, self.pro_path, None, 'processing')
            if args_srna.sorf_file is not None:
                ...
                self.multiparser.combine_gff(args_srna.gffs, self.sorf_path, None, 'sORF')
            if args_srna.import_info is not None and (args_srna.utr_srna or 'sec_str' in args_srna.import_info or args_srna.nr_database is not None or args_srna.srna_database is not None) and args_srna.fastas is None:
            ...
            if args_srna.terms is not None:
                ...
                self.multiparser.combine_gff(args_srna.gffs, self.term_path, None, 'term')
        else:
            self.term_path = None
    ...
    def _run_program(self, args_srna, log):
        ...
        for gff in os.listdir(args_srna.gffs):
            if gff.endswith('.gff'):
                ...
                tran = self.helper.get_correct_file(self.tran_path, '_transcript.gff', prefix, None, None)
                ...
                if not args_srna.source:
                    ...
                    upstream(os.path.join(self.tss_path, prefix + '_TSS.gff'), None, os.path.join(args_srna.gffs, prefix + '.gff'), os.path.join(args_srna.out_folder, 'TSS_classes', '_'.join([prefix, 'TSS.gff'])), args_srna, prefix)
                print('Running sRNA detection of {0}'.format(prefix))
                tss, frag_datas, tex_datas = self._run_normal(prefix, gff, tran, args_srna.fuzzy_tsss['inter'], args_srna, log)
                if args_srna.utr_srna:
                    ...
                    if self.pro_path is not None:
                        ...
                        pro = None
                    if tss is not None:
                        self._run_utrsrna(gff, tran, prefix, tss, pro, args_srna, frag_datas, tex_datas, log)
                    tex_datas = self._merge_tex_frag_datas(tex_datas, frag_datas)
                    del frag_datas
                    gc.collect()
                    self._merge_srna(args_srna, gffs, csvs, prefix, os.path.join(args_srna.gffs, gff), tss, tex_datas)
                    del tex_datas
                    filter_frag(csvs['merge'], gffs['merge'])
                    self.helper.sort_gff(gffs['merge'], '_'.join([self.prefixs['basic'], prefix]))
                    log.write('\t' + '_'.join([self.prefixs['basic'], prefix]) + ' is generated to temporary store sRNA candidates.\n')
                    log.write('\t' + csvs['merge'] + ' is generated to temporary store the detail information of sRNA candidates.\n')

        return prefixs

    def _merge_srna(self, args_srna, gffs, csvs, prefix, gff_file, tss, tex_datas):
        print('Merging data of sRNA')
        merge_srna_gff(gffs, args_srna.in_cds, args_srna.cutoff_overlap, gff_file, args_srna.ex_srna)
        merge_srna_table(gffs['merge'], csvs, tex_datas[2], tex_datas[3], tss, args_srna)
    ...
    def _get_seq_sec(self, fasta_path, out_folder, prefix, sec_path, dot_path, rnafold, log):
        """extract the sec str energy"""
        detect = False
        for fasta in os.listdir(fasta_path):
            if fasta.endswith('.fa') and '.'.join(fasta.split('.')[:-1]) == prefix:
                detect = True
                break
        ...
        if detect:
            ...
            index_file = os.path.join(out_folder, '_'.join([
             'sRNA_index', prefix]))
            log.write('Running helper.py to get the sequences of sRNA for {0}.\n'.format(prefix))
            self.helper.get_seq('_'.join([self.prefixs['basic'], prefix]), os.path.join(fasta_path, fasta), index_file)
        ...
        log.write('Running extract_sRNA_info.py to extract the energy information for {0}.\n'.format(prefix))
        extract_energy(os.path.join(main_path, '_'.join([self.prefixs['basic'], prefix])), sec_file, os.path.join(main_path, '_'.join([self.prefixs['energy'], prefix])))
        log.write('\t' + os.path.join(main_path, '_'.join([self.prefixs['energy'], prefix])) + ' is generated to temporary store energy information.\n')
    ...
    def _replot_sec(self, relplot_pl, tmp_paths, prefix, log):
        ...
        for file_ in os.listdir(os.getcwd()):
            if file_.endswith('ss.ps'):
                ...
                self._run_replot(relplot_pl, tmp_paths, file_, dot_file, rel_file, log)

        log.write('Done!\n')
        os.mkdir(os.path.join(tmp_paths['sec'], prefix))
        os.mkdir(os.path.join(tmp_paths['dot'], prefix))
        self.helper.move_all_content(tmp_paths['tmp'], os.path.join(tmp_paths['sec'], prefix), [
         'rss.ps'])
        self.helper.move_all_content(tmp_paths['tmp'], os.path.join(tmp_paths['dot'], prefix), [
         'dp.ps'])
    ...
    def _plot_mountain(self, mountain, moun_path, tmp_paths, prefix, mountain_pl, log):
        if mountain:
            ...
            for dot_file in os.listdir(dot_path):
                if dot_file.endswith('dp.ps'):
                    ...
                    shutil.move(moun_file, os.path.join(tmp_moun_path, prefix, moun_file))
                    out.close()
                    os.remove(moun_txt)

            log.write('Done!\n')
            log.write('All plots are stored in {0}.'.format(os.path.join(tmp_moun_path, prefix)))

    def _compute_2d_and_energy(self, args_srna, prefixs, log):
        print('Running energy calculation')
        moun_path = os.path.join(args_srna.out_folder, 'figs', 'mountain_plots')
        sec_path = os.path.join(args_srna.out_folder, 'figs', 'sec_plots')
        dot_path = os.path.join(args_srna.out_folder, 'figs', 'dot_plots')
        ...
        for prefix in prefixs:
            tmp_paths = self._get_seq_sec(self.fasta_path, args_srna.out_folder, prefix, sec_path, dot_path, args_srna.rnafold, log)
            self._replot_sec(args_srna.relplot_pl, tmp_paths, prefix, log)
            self._plot_mountain(args_srna.mountain, moun_path, tmp_paths, prefix, args_srna.mountain_pl, log)
            os.chdir(tmp_paths['main'])
            shutil.move('_'.join([self.prefixs['energy'], prefix]), '_'.join([self.prefixs['basic'], prefix]))
            log.write('_'.join([self.prefixs['basic'], prefix]) + ' is updated, and ' + '_'.join([self.prefixs['energy'], prefix]) + ' is deleted.\n')
            shutil.rmtree(os.path.join(args_srna.out_folder, 'tmp_srna'))

    def _run_blast(self, program, database, e, seq_file, blast_file, strand, para_num, processes, log):
        if para_num == 1:
    ...
    def _run_para_blast(self, program, database, e, seq_file, blast_file, strand, paras, log):
        srnas = {}
        ...
        if file_num == 0 or paras == 1:
            self._run_blast(program, database, e, seq_file, blast_file, strand, 1, processes, log)
        else:
            cur_para = 0
            line_num = 0
            first = True
            seq_files = []
            log.write('{0} is splited to {1} subset files.\n'.format(seq_file, paras))
            for name, seq in srnas.items():
                if line_num >= file_num or first:
                    ...
                    first = False
                    if cur_para < paras:
                        out = open('_'.join([seq_file, str(cur_para)]), 'w')
            ...
            for para in range(paras):
                self._run_blast(program, database, e, '_'.join([seq_file, str(para)]), '_'.join([blast_file, strand, str(para)]), strand, paras, processes, log)
    ...
    def _blast(self, database, database_format, data_type, args_srna, prefixs, program, database_type, e, filters, log):
        if database is None:
            log.write(' No database was assigned!\n')
            print('Error: No database was assigned!')
        else:
            if database_format:
                database = self._formatdb(database, data_type, args_srna.out_folder, args_srna.blastdb, database_type, log)
            for prefix in prefixs:
                blast_file = os.path.join(args_srna.out_folder, 'blast_results_and_misc', '_'.join([database_type, 'blast', prefix + '.txt']))
                if os.path.exists(blast_file):
                    os.remove(blast_file)
                srna_file = '_'.join([self.prefixs['basic'], prefix])
                out_file = os.path.join(args_srna.out_folder, '_'.join(['tmp', database_type, prefix]))
                print('Running Blast of {0} in {1}'.format(prefix, database))
                seq_file = os.path.join(args_srna.out_folder, '_'.join(['sRNA_seq', prefix]))
                if seq_file not in os.listdir(args_srna.out_folder) or database_type == 'nr' and 'sec_str' in filters:
                    log.write('Running helper.py to extract the sequences of sRNAs.\n')
                    self.helper.get_seq(srna_file, os.path.join(self.fasta_path, prefix + '.fa'), seq_file)
                    log.write('\t' + seq_file + ' is generated.\n')
                if database_type == 'nr':
                    log.write('Running BLAST+ for nr database for {0}.'.format(prefix))
                    log.write('Make sure the version of BLAST+ is at least 2.2.28+.\n')
                    tmp_plus, tmp_minus = self._get_strand_fasta(seq_file, args_srna.out_folder)
                    tmp_blast = os.path.join(args_srna.out_folder, 'blast_results_and_misc', 'tmp_blast.txt')
                    if os.path.exists(tmp_blast):
                        os.remove(tmp_blast)
                    self._run_para_blast(program, database, e, tmp_plus, tmp_blast, 'plus', args_srna.para_blast, log)
                    self._run_para_blast(program, database, e, tmp_minus, blast_file, 'minus', args_srna.para_blast, log)
                    self.helper.merge_file(tmp_blast, blast_file)
                    os.remove(tmp_plus)
                    os.remove(tmp_minus)
                else:
                    log.write('Running BLAST+ for sRNA database for {0}.'.format(prefix))
                    log.write('Make sure the version of BLAST+ is at least 2.2.28+.\n')
                    self._run_para_blast(program, database, e, seq_file, blast_file, 'both', args_srna.para_blast, log)
                log.write('Running extract_sRNA_info.py to extract BLAST information.\n')
                extract_blast(blast_file, srna_file, out_file, out_file + '.csv', database_type, args_srna.blast_score_s, args_srna.blast_score_n)
    ...
    def _class_srna(self, prefixs, args_srna, log):
        ...
        if args_srna.import_info is not None or args_srna.srna_database is not None or args_srna.nr_database is not None or self.sorf_path is not None or self.tss_path is not None or self.term_path is not None or args_srna.promoter_table is not None:
            log.write('Running sRNA_class.py to classify sRNAs based on input files and --filter_info.\n')
            ...
            for prefix in prefixs:
                ...
                out_stat = os.path.join(self.stat_path, '_'.join([
                 'stat_sRNA_class', prefix + '.csv']))
                classify_srna(os.path.join(self.all_best['all_gff'], '_'.join([prefix, 'sRNA.gff'])), class_gff, out_stat, args_srna)
                log.write('\t' + out_stat + '\n')
                for srna in os.listdir(class_gff):
                    out_table = os.path.join(class_table, srna.replace('.gff', '.csv'))
                    gen_srna_table(os.path.join(class_gff, srna), '_'.join([self.prefixs['merge_table'], prefix]), '_'.join([self.tmps['nr'], prefix + '.csv']), '_'.join([self.tmps['srna'], prefix + '.csv']), args_srna, out_table, self.term_path)
    ...
    def _get_best_result(self, prefixs, args_srna, log):
        ...
        log.write('The following files are generated:\n')
        for prefix in prefixs:
            best_gff = os.path.join(self.all_best['best_gff'], '_'.join([prefix, 'sRNA.gff']))
            best_table = os.path.join(self.all_best['best_table'], '_'.join([prefix, 'sRNA.csv']))
            gen_best_srna(os.path.join(self.all_best['all_gff'], '_'.join([prefix, 'sRNA.gff'])), best_gff, args_srna)
            gen_srna_table(os.path.join(self.all_best['best_gff'], '_'.join([prefix, 'sRNA.gff'])), '_'.join([self.prefixs['merge_table'], prefix]), '_'.join([self.tmps['nr'], prefix + '.csv']), '_'.join([self.tmps['srna'], prefix + '.csv']), args_srna, best_table, self.term_path)
            log.write('\t' + best_gff + '\n')
    ...
    def _filter_srna(self, args_srna, prefixs, log):
        ...
        if args_srna.nr_database is not None:
            self._blast(args_srna.nr_database, args_srna.nr_format, 'prot', args_srna, prefixs, args_srna.blastx, 'nr', args_srna.e_nr, args_srna.import_info, log)
        if self.sorf_path is not None:
            for prefix in prefixs:
                if '_'.join([prefix, 'sORF.gff']) in os.listdir(self.sorf_path):
                    tmp_srna = os.path.join(args_srna.out_folder, ''.join(['tmp_srna_sorf', prefix]))
                    tmp_sorf = os.path.join(args_srna.out_folder, ''.join(['tmp_sorf_srna', prefix]))
                    log.write('Running compare_sRNA_sORF.py to compare sRNAs and sORFs.\n')
                    srna_sorf_comparison('_'.join([self.prefixs['basic'], prefix]), os.path.join(self.sorf_path, '_'.join([prefix, 'sORF.gff'])), tmp_srna, tmp_sorf)
                    os.remove(tmp_sorf)
        ...
        if args_srna.srna_database is not None:
            self._blast(args_srna.srna_database, args_srna.srna_format, 'nucl', args_srna, prefixs, args_srna.blastn, 'sRNA', args_srna.e_srna, args_srna.import_info, log)
    ...
    def _gen_table(self, prefixs, args_srna, log):
        ...
        for prefix in prefixs:
            ...
            out_table = os.path.join(self.all_best['all_table'], '_'.join([prefix, 'sRNA.csv']))
            gen_srna_table(os.path.join(self.all_best['all_gff'], '_'.join([prefix, 'sRNA.gff'])), '_'.join([self.prefixs['merge_table'], prefix]), '_'.join([self.tmps['nr'], prefix + '.csv']), '_'.join([self.tmps['srna'], prefix + '.csv']), args_srna, out_table, self.term_path)
            log.write('\t' + out_table + '\n')
    ...
    def _print_rank_all(self, prefixs, log):
        ...
        for prefix in prefixs:
            all_table = os.path.join(self.all_best['all_table'], '_'.join([prefix, 'sRNA.csv']))
            best_table = os.path.join(self.all_best['best_table'], '_'.join([prefix, 'sRNA.csv']))
    ...
    def _filter_min_utr(self, prefixs, min_utr):
        ...
        for prefix in prefixs:
            filter_utr(os.path.join(self.all_best['all_gff'], '_'.join([prefix, 'sRNA.gff'])), os.path.join(self.all_best['all_table'], '_'.join([prefix, 'sRNA.csv'])), min_utr)
    ...
    def _antisense(self, gffs, prefixs):
        ...
        for prefix in prefixs:
            all_table = os.path.join(self.all_best['all_table'], '_'.join([prefix, 'sRNA.csv']))
            best_table = os.path.join(self.all_best['best_table'], '_'.join([prefix, 'sRNA.csv']))
            all_gff = os.path.join(self.all_best['all_gff'], '_'.join([prefix, 'sRNA.gff']))
            best_gff = os.path.join(self.all_best['best_gff'], '_'.join([prefix, 'sRNA.gff']))
            srna_antisense(all_gff, all_table, os.path.join(gffs, prefix + '.gff'))
            srna_antisense(best_gff, best_table, os.path.join(gffs, prefix + '.gff'))
    ...
    def _blast_stat(self, stat_path, srna_tables, log):
        ...
        for srna_table in os.listdir(os.path.join(srna_tables, 'best_candidates')):
            out_srna_blast = os.path.join(stat_path, 'stat_' + srna_table.replace('.csv', '_blast.csv'))
            blast_class(os.path.join(srna_tables, 'best_candidates', srna_table), out_srna_blast)
    ...
    def _compare_term_promoter(self, out_table, prefix, args_srna, log):
        ...
        if self.term_path is not None:
            log.write('Running compare_srna_term.py to compare sRNAs with terminators.\n')
            compare_srna_term(os.path.join(self.all_best['all_gff'], '_'.join([prefix, 'sRNA.gff'])), out_table, os.path.join(self.term_path, '_'.join([prefix, 'term.gff'])), args_srna.fuzzy_b, args_srna.fuzzy_a)
            log.write(os.path.join(self.all_best['all_gff'], '_'.join([prefix, 'sRNA.gff'])) + ' is updated.\n')
            log.write(out_table + ' is updated.\n')
        if args_srna.promoter_table is not None:
            log.write('Running compare_srna_term.py to compare sRNAs with promoters.\n')
            compare_srna_promoter(os.path.join(self.all_best['all_gff'], '_'.join([prefix, 'sRNA.gff'])), out_table, args_srna)
            log.write(os.path.join(self.all_best['all_gff'], '_'.join([prefix, 'sRNA.gff'])) + ' is updated.\n')
            log.write(out_table + ' is updated.\n')

    def _get_poly_u(self, prefixs, args_srna, log):
        print('Searching poly U tail ...')
        log.write('Running get_srna_poly_u.py to seach the poly U tails of sRNAs.\n')
        for prefix in prefixs:
            get_srna_poly_u('_'.join([self.prefixs['basic'], prefix]), os.path.join(self.fasta_path, prefix + '.fa'), '_'.join([self.prefixs['merge_table'], prefix]), args_srna)
    ...
    def _re_table(self, args_srna, prefixs, log):
        ...
        for type_ in ['all_candidates', 'best_candidates']:
            for prefix in prefixs:
                srna_table = os.path.join(args_srna.out_folder, 'tables', type_, '_'.join([
                 prefix, 'sRNA.csv']))
                reorganize_table(args_srna.libs, args_srna.merge_wigs, 'Track/Coverage', srna_table)
                log.write('\t' + srna_table + '\n')

        for c_table in os.listdir(os.path.join(args_srna.out_folder, 'tables', 'for_classes', prefix)):
            for prefix in prefixs:
                srna_table_c = os.path.join(args_srna.out_folder, 'tables', 'for_classes', prefix, c_table)
                reorganize_table(args_srna.libs, args_srna.merge_wigs, 'Track/Coverage', srna_table_c)
                log.write('\t' + srna_table_c + '\n')

    def _check_overlap_cds(self, args_srna, prefixs, log):
        log.write('Running check_srna_overlap.py to compare sRNAs with genome annotations.\n')
        log.write('The following files are updated:\n')
        for type_ in ['all_candidates', 'best_candidates']:
            for prefix in prefixs:
                srna_table = os.path.join(args_srna.out_folder, 'tables', type_, '_'.join([
    ...
    def run_srna_detection(self, args_srna, log):
        ...
        self.multiparser.combine_gff(args_srna.gffs, self.tran_path, None, 'transcript')
        self.multiparser.parser_fasta(args_srna.fastas)
        self.multiparser.combine_fasta(args_srna.gffs, self.fasta_path, None)
        ...
        for prefix in prefixs:
            shutil.copyfile('_'.join([self.prefixs['basic'], prefix]), os.path.join(self.all_best['all_gff'], '_'.join([prefix, 'sRNA.gff'])))
            log.write('\t' + os.path.join(self.all_best['all_gff'], '_'.join([prefix, 'sRNA.gff'])) + ' is generated, and _'.join([self.prefixs['basic'], prefix]) + ' is deleted.\n')
        ...
        if args_srna.srna_database is not None and 'blast_srna' in args_srna.import_info:
