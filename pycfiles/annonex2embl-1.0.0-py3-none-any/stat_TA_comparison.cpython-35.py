...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/stat_TA_comparison.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 18040 bytes
import os, shutil
...
def assign_tss(tss, tran):
    ...
    if 'Parent' not in tss.attributes.keys():
        ...
        tss.attributes['Parent'] = ','.join([tss.attributes['Parent'], tran_id])
...
def compare_tran_tss(trans, tsss, fuzzy, stat, out):
    ...
    for tran in trans:
        ...
        check = [0, 0, 0]
        for tss in tsss:
            if tss.strand == tran.strand and tss.seq_id == tran.seq_id:
                if tss.strand == '+':
                    if tss.start + int(fuzzy) >= tran.start and tss.start <= tran.end:
                        if check[0] != 1:
                            stat['with_TSS'] += 1
                        assign_tss(tss, tran)
                        check[0] = 1
                ...
                elif tss.end - int(fuzzy) <= tran.end and tss.end >= tran.start:
                    if check[0] != 1:
                        stat['with_TSS'] += 1
                    assign_tss(tss, tran)
                    check[0] = 1
        ...
        tran.attribute_string = ';'.join(['='.join(items) for items in tran.attributes.items()])
        if out is not None:
...
def print_tas_stat(stat, out):
    ...
    out.write('\tTranscript starts or overlap with TSS:{0} ({1})\n'.format(stat['with_TSS'], float(stat['with_TSS']) / float(total_tran)))
    out.write('\tTranscript has no relationship with TSS:{0} ({1})\n'.format(stat['no_TSS'], float(stat['no_TSS']) / float(total_tran)))
    out.write('\tTSS starts or overlap with transcript:{0} ({1})\n'.format(stat['TSS_with_tran'], float(stat['TSS_with_tran']) / float(total_TSS)))
    out.write('\tTSS has no relationship with transcript:{0} ({1})\n'.format(stat['TSS_no_tran'], float(stat['TSS_no_tran']) / float(total_TSS)))
        ...
        for strain_tss in tsss_uni.keys():
            for strain_ta in tas_uni.keys():
                if strain_tss == strain_ta:
                    ...
                    stats = detect_tas_region(tsss_uni[strain_tss], sort_tas, None, None, fuzzy)
                    print_tas_stat(stats, out_stat)
...
def assign_parent(gff, tran, feature):
    if 'Parent' not in gff.attributes.keys():
        ...
        gff.attributes['Parent'] = ','.join([gff.attributes['Parent'], tran.attributes['ID']])
    if '_'.join(['associated', feature]) not in tran.attributes.keys():
        if 'locus_tag' in gff.attributes.keys():
            tran.attributes['_'.join(['associated', feature])] = gff.attributes['locus_tag']
        else:
            if 'protein_id' in gff.attributes.keys():
                tran.attributes['_'.join(['associated', feature])] = gff.attributes['protein_id']
            else:
                if 'Name' in gff.attributes.keys():
                    tran.attributes['_'.join(['associated', feature])] = gff.attributes['Name']
                else:
                    strand = Helper().get_strand_name(gff.strand)
                    tran.attributes['_'.join(['associated', feature])] = ''.join([gff.feature, ':', str(gff.start),
                     '-', str(gff.end), '_', strand])
    else:
        if 'locus_tag' in gff.attributes.keys():
            tran.attributes['_'.join(['associated', feature])] = ','.join([tran.attributes['_'.join(['associated', feature])],
...
def compare_ta_gff(gffs, tran, check, tran_type, detect, stats, c_feature):
    for gff in gffs:
        if gff.feature == c_feature and gff.strand == tran.strand and gff.seq_id == tran.seq_id:
            if gff.start < tran.start and gff.end > tran.end:
                if check[0] != 1:
                ...
                if gff.start >= tran.start and gff.end <= tran.end:
                    ...
                    if gff.start >= tran.start and gff.end > tran.end and gff.start < tran.end:
                        if check[1] != 1:
                        ...
                        check[1] = 1
                        detect = True
                    elif gff.start < tran.start and gff.end <= tran.end and gff.end > tran.start:
                        if check[2] != 1:
...
def detect_tag_region(gffs, trans, stats, out_t, out_g, c_feature, region):
    ...
    for tran in trans:
        ...
        tran_type = []
        tran_type_string = ''
        detect = compare_ta_gff(gffs, tran, check, tran_type, detect, stats, c_feature)
        if not detect:
        ...
        attribute_string = ';'.join(['='.join(items) for items in tran.attributes.items()])
    ...
    for gff in gffs:
        attribute_string = ';'.join(['='.join(items) for items in gff.attributes.items()])
...
def detect_express_gene(gffs, c_feature, strain):
    express_gene = 0
    for gff in gffs:
        if gff.feature == c_feature and (strain == 'all' or gff.seq_id == strain) and 'Parent' in gff.attributes.keys() and 'tran' in gff.attributes['Parent'].lower():
            express_gene += 1
...
def print_tag_stat(stats, out, express_gene, c_feature):
    total = stats['bsae'] + stats['bsbe'] + stats['asae'] + stats['asbe'] + stats['other']
    out.write('\t\tTranscript starts before and ends after {0}:{1} ({2})\n'.format(c_feature, str(stats['asbe']), str(float(stats['asbe']) / float(total))))
    out.write('\t\tTranscript starts after and ends before {0}:{1} ({2})\n'.format(c_feature, str(stats['bsae']), str(float(stats['bsae']) / float(total))))
    out.write('\t\tTranscript starts before and ends within {0}:{1} ({2})\n'.format(c_feature, str(stats['asae']), str(float(stats['asae']) / float(total))))
    out.write('\t\tTranscript starts within and ends after {0}:{1} ({2})\n'.format(c_feature, str(stats['bsbe']), str(float(stats['bsbe']) / float(total))))
    out.write('\t\tTranscript has no overlap of {0}:{1} ({2})\n'.format(c_feature, str(stats['other']), str(float(stats['other']) / float(total))))
    out.write('\t\tTotal {0}s which have expression:{1} ({2})\n'.format(c_feature, str(express_gene), str(float(express_gene) / float(stats['gene']))))
...
def read_tag_file(gff_file, ta_file, c_feature):
        ...
        if entry.feature == c_feature:
            ...
            if 'Parent' in entry.attributes.keys():
                ...
                if entry.seq_id in stats.keys():
                    ...
                    stats['All']['gene'] += 1
                if entry.feature.lower() != 'region' and entry.feature.lower() != 'source' and entry.feature.lower() != 'remark':
                    gffs.append(entry)
                else:
                    region = entry
    ...
    tas = sorted(tas, key=lambda k: (k.seq_id, k.start, k.end, k.strand))
    return (
     gffs, tas, stats, region)


def stat_ta_gff(ta_file, gff_file, stat_file, out_ta_file, out_gff_file, c_feature):
    ...
    for feature in c_feature:
        ...
        gffs, tas, stats, region = read_tag_file(tmp_gff_file, tmp_ta_file, feature)
        ...
        if len(stats) > 2:
            for strain in stats.keys():
                if strain != 'All':
                    ...
                    out_stat.write('\tThe transcript information compares with {0}:\n'.format(feature))
                    print_tag_stat(stats[strain], out_stat, express_gene, feature)
