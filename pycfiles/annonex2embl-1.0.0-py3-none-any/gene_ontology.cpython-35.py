...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/gene_ontology.py
...
def compare_cds_tran(gffs, trans):
    ...
    for gff in gffs:
        for ta in trans:
            if gff.seq_id == ta.seq_id and gff.strand == ta.strand and (gff.end < ta.end and gff.end > ta.start and gff.start <= ta.start or gff.start > ta.start and gff.start < ta.end and gff.end >= ta.end or gff.end >= ta.end and gff.start <= ta.start or gff.end <= ta.end and gff.start >= ta.start):
                new_gffs.append(gff)
                break
...
def get_go_id(gffs, id_, uni_lines, gos):
    ...
    detect = False
    for gff in gffs:
        if 'Name' in gff.attributes.keys() and id_ == gff.attributes['Name']:
            detect = True
        if 'protein_id' in gff.attributes.keys() and id_ == gff.attributes['protein_id']:
            detect = True
        if detect:
            detect = False
...
def print_go(gos, out):
    ...
    for go in gos:
        if go != pre_go and pre_go is not None:
            if go['strain'] == pre_go['strain'] and go['strand'] == pre_go['strand'] and go['start'] == pre_go['start'] and go['end'] == pre_go['end'] and go['protein_id'] == pre_go['protein_id']:
                go_ids = []
...
def retrieve_uniprot(database_file, gff_file, out_file, tran_file, type_):
    ...
    for entry in Gff3Parser().entries(open(gff_file)):
        if entry.feature == 'CDS':
            if 'Name' in entry.attributes.keys() and 'protein_id' in entry.attributes.keys():
                ...
                if 'Name' in entry.attributes.keys():
                    import_uniprot_data(entry, name_list, 'Name')
...
def plot(total_nums, strain, filename, total, out_folder):
    ...
    elif filename == 'molecular_function' or filename == 'cellular_component' or filename == 'biological_process':
...
def print_file(classes, total_nums, out_folder, stat):
    ...
    for strain, datas in classes.items():
        ...
        if printed and total_nums[strain]['total'] != 0:
            plot(total_nums[strain], strain, 'three_roots', total_nums[strain]['total'], out_folder)
            out_stat.write('{0}:\n'.format(strain))
            for origin, types in datas.items():
                plot(types, strain, origin, total_nums[strain][origin], out_folder)
                out_stat.write('\t{0}: {1}(percentage in total: {2})\n'.format(origin, total_nums[strain][origin], float(total_nums[strain][origin]) / float(total_nums[strain]['total'])))
                for type_, num in types.items():
                    out_stat.write('\t\t{0}: {1}(percentage in {2}: {3})\n'.format(type_, num, origin, float(num) / float(total_nums[strain][origin])))
                        ...
                        if target_term == term_obo['id']:
                            if 'is_a' in term_obo.keys():
                                for is_a in term_obo['is_a']:
                                    go_a = is_a.split(' ! ')
                                    if go_a[1] != 'biological_process' and go_a[1] != 'cellular_component' and go_a[1] != 'molecular_function':
                                        target_terms.append(go_a[0])

                            else:
                                if 'is_obsolete' in term_obo.keys() and term_obo['is_obsolete'] == 'true':
                                    break
                                for slim_obo in slim_obos:
                                    for target_term in target_terms:
                                        if target_term == slim_obo['id'] and target_term not in pro_list:
                                            detect = True
                                            import_class(slim_obo, classes, strain)
                                            import_class(slim_obo, classes, 'All_genome')
                                            import_total(slim_obo, total_nums, strain)
                                            import_total(slim_obo, total_nums, 'All_genome')
