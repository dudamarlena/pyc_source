...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/coverage_detection.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 8083 bytes
import copy
...
def coverage_comparison(cover, cover_sets, poss, first, strand, cover_pos):
    ...
    if first:
        ...
        poss['high'] = cover_pos
        poss['low'] = cover_pos
    elif cover_sets['high'] < cover:
        cover_sets['high'] = cover
        poss['high'] = cover_pos
        poss['low'] = cover_pos
        cover_sets['low'] = cover
    if strand == '+' and poss['low'] >= poss['high'] or strand == '-' and poss['low'] <= poss['high']:
        if cover_sets['low'] > cover:
            cover_sets['low'] = cover
            poss['low'] = cover_pos
    elif strand == '+' and poss['low'] < poss['high'] or strand == '-' and poss['low'] > poss['high']:
        poss['low'] = cover_pos
        cover_sets['low'] = cover
    return first
        ...
        if len(texs) != 0:
            for keys in texs.keys():
                ...
                if cover['track'] == tracks[0]:
                    if cover['avg'] > cutoff:
                        return True
                elif cover['track'] == tracks[1] and cover['avg'] > notex:
                    return True

        elif cover['avg'] > cutoff:
            return True
    elif cover['avg'] > cutoff:
        return True


def run_tex(cover, texs, check_texs, tex_notex, type_, detect_num, poss, target_datas):
            ...
            if cover['track'] in key:
                ...
                if texs[key] >= tex_notex:
                    if type_ == 'sRNA_utr_derived':
                        ...
                        if tex_notex != 1 and check_texs[key][0] not in target_datas:
...
def check_tex(template_texs, covers, target_datas, notex, type_, poss, median, coverages, utr_type, cutoff_coverage, tex_notex):
        ...
        if type_ == 'sRNA_utr_derived':
            ...
            if type_ == 'sORF':
                ...
                if type_ == 'terminator':
                    ...
                    if type_ == 'normal':
                        run_check_tex = check_notex(cover, texs, cutoff_coverage, notex)
                    elif cover['avg'] > cutoff_coverage:
                        run_check_tex = True
        if run_check_tex:
            detect_num = run_tex(cover, texs, check_texs, tex_notex, type_, detect_num, poss, target_datas)
...
def replicate_comparison(args_srna, srna_covers, strand, type_, median, coverages, utr_type, notex, cutoff_coverage, texs):
    ...
    for cond, covers in srna_covers.items():
        detect_num = check_tex(texs, covers, srna_datas['detail'], notex, type_, tmp_poss, median, coverages, utr_type, cutoff_coverage, args_srna.tex_notex)
        if 'texnotex' in cond:
        ...
        if detect:
            ...
            if type_ == 'sRNA_utr_derived':
                ...
                if strand == '+':
                    ...
                    sort_datas = sorted(srna_datas['detail'], key=lambda k: k['pos'], reverse=True)
