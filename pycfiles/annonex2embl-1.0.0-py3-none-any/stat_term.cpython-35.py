...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/stat_term.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 10977 bytes
import csv
...
def print_method(nums, method_name, method, express, detect, only, out):
    ...
    out.write('\tTotal {0} terminators = {1}\n'.format(method_name, nums[method] + nums['frhp']))
    print_percent(out, float(nums['total']), float(nums[method] + nums['frhp']), '')
    ...
    print_percent(out, float(nums[method] + nums['frhp']), float(nums[method]), method_name + ' ')
    ...
    print_percent(out, float(nums[method] + nums['frhp']), float(nums[express]), method_name + ' ')
    ...
    print_percent(out, float(nums[method] + nums['frhp']), float(nums[detect]), method_name + ' ')
    print_express(out, float(nums['total_ex']), float(nums[detect]), '')
    print_express(out, float(nums[express]), float(nums[detect]), method_name + ' ')
    ...
    print_decrease(out, float(nums[detect]), float(nums[only]), method_name + ' ')
    out.write('\n')
...
def classify_terms(terms, nums, out_d, out_e, out_n, pre_strain):
    for term in terms:
        ...
        if term.attributes['method'] == 'gene_converged':
            ...
            if term.attributes['method'] == 'TransTermHP':
                plus_num(nums, strain, 'total')
...
def stat_term(term_gff, term_table, stat, output_decrease, output_expression, output_non):
    terms = []
    nums = {}
    ...
    for entry in Gff3Parser().entries(gh):
        detect = check_repeat(checks, entry.seq_id, entry.start, entry.end, entry.strand)
        if detect:
            terms.append(entry)
