...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/get_inter_seq.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 12764 bytes
...
def get_terminal(genes, inters, gene_len, type_, file_type):
    ...
    if type_ == 'start':
        for gene in genes:
            if gene.strand == '-':
                inters.append(import_data(gene.strand, gene.seq_id, 1, gene.start, file_type))
                break

    elif type_ == 'end':
        for gene in reversed(genes):
            if gene.strand == '+':
                inters.append(import_data(gene.strand, gene.seq_id, gene.end, gene_len, file_type))
                break
...
def get_inter(features, seq, file_type):
    ...
    for feature1 in sort_features:
        if pre_strain != feature1.seq_id:
            if feature1.strand == '-':
                inters.append(import_data('terminal', feature1, 1, feature1.start, file_type))
            if not first:
                if pre_feature1.strand == '+':
                    inters.append(import_data(pre_feature1, 'terminal', pre_feature1.start, len(seq[pre_feature1.seq_id]), file_type))
                if first:
                    first = False
                for feature2 in sort_features:
                    if feature1.seq_id == feature2.seq_id and feature1.end < feature2.end and feature1.end >= feature2.start and feature1.strand == feature2.strand:
                        break
                    elif feature1.seq_id == feature2.seq_id and feature1.end <= feature2.start and feature1.strand == '+':
                        if feature1.strand == feature2.strand:
                            break
                        else:
                            inters.append(import_data(feature1, feature2, feature1.end, feature2.start, file_type))
                            break
...
def get_overlap_inters(inter1, inter2, merges, id_):
    if inter1['end'] < inter2['end'] and inter1['end'] > inter2['start'] and inter1['start'] <= inter2['start']:
        merges.append(import_merge(id_, inter1['strain'], inter2['start'], inter1['end'], inter2['parent_p'], inter1['parent_m'], inter2['p_pos'], inter1['m_pos']))
        inter1['print'] = True
        inter2['print'] = True
        id_ += 1
    else:
        if inter1['start'] > inter2['start'] and inter1['start'] < inter2['end'] and inter1['end'] >= inter2['end']:
            merges.append(import_merge(id_, inter1['strain'], inter1['start'], inter2['end'], inter1['parent_p'], inter2['parent_m'], inter1['p_pos'], inter2['m_pos']))
            inter1['print'] = True
            inter2['print'] = True
            id_ += 1
        else:
            if inter1['end'] >= inter2['end'] and inter1['start'] <= inter2['start']:
                merges.append(import_merge(id_, inter2['strain'], inter2['start'], inter2['end'], inter2['parent_p'], inter2['parent_m'], inter2['p_pos'], inter2['m_pos']))
                inter1['print'] = True
                inter2['print'] = True
                id_ += 1
            elif inter1['end'] <= inter2['end'] and inter1['start'] >= inter2['start']:
                merges.append(import_merge(id_, inter1['strain'], inter1['start'], inter1['end'], inter1['parent_p'], inter1['parent_m'], inter1['p_pos'], inter1['m_pos']))
                inter1['print'] = True
                inter2['print'] = True
                id_ += 1
    return id_
...
def detect_confliction(gc, genes, seq):
    ...
    if 'tran' in gc['parent_p']:
        if gc['start'] > 80:
            for gene in genes:
                if gc['start'] - 80 > gene.start and gc['start'] - 80 < gene.end and gene.strand == '+':
        ...
        if gc['start'] > 30:
            ...
            tmp_start = 1
        corr_merges.append(import_merge(gc['ID'], gc['strain'], tmp_start, gc['end'], gc['parent_p'], gc['parent_m'], gc['p_pos'], gc['m_pos']))
        corr_merges[(-1)]['strand'] = '+'
        if 'tran' in gc['parent_m']:
            if gc['end'] < len(seq[gc['strain']]) - 80:
                for gene in genes:
                    if gc['end'] + 80 > gene.start and gc['end'] + 80 < gene.end and gene.strand == '-':
            ...
            if gc['end'] < len(seq[gc['strain']]) - 30:
                ...
                tmp_end = len(seq[gc['strain']])
    corr_merges.append(import_merge(gc['ID'], gc['strain'], gc['start'], tmp_end, gc['parent_p'], gc['parent_m'], gc['p_pos'], gc['m_pos']))
    corr_merges[(-1)]['strand'] = '-'
    return corr_merges
...
def get_fasta(seq, merge, num, strand, args_term, out, out_i):
    if merge['end'] - merge['start'] > args_term.window:
        ...
        for start in range(merge['start'], merge['end'] + 1, args_term.shift):
            ...
            inter_seq = Helper().extract_gene(seq[merge['strain']], start, end, strand)
        ...
        inter_seq = Helper().extract_gene(seq[merge['strain']], merge['start'], merge['end'], strand)
        out_i.write('>' + '|'.join(['inter_' + str(num), str(merge['start']), str(merge['end']), merge['strain'], merge['parent_p'], merge['parent_m'], merge['p_pos'], merge['m_pos'], strand]) + '\n')
        out.write('>inter_' + str(num) + '\n')
        out.write(inter_seq + '\n')
        num += 1
    return num
    ...
    for inter in inter_tas:
        for gene in genes:
            if inter['strain'] == gene.seq_id:
                if gene.end > inter['start'] and gene.end < inter['end'] and gene.strand == '+':
                    inter['start'] = gene.end
                    inter['parent_p'], inter['p_pos'] = get_feature(gene, 'gene')
                elif gene.start > inter['start'] and gene.start < inter['end'] and gene.strand == '-':
                    inter['end'] = gene.start
                    inter['parent_m'], inter['m_pos'] = get_feature(gene, 'gene')
                    break


def intergenic_seq(seq_file, tran_file, gff_file, out_file, index_file, args_term):
    """get intergenic seq"""
    out = open(out_file, 'w')
    ...
    for tmp_merge in merges:
        ...
        for merge in corr_merges:
            if merge['start'] < merge['end']:
                if merge['strand'] == '+':
                    num = get_fasta(seq, merge, num, '+', args_term, out, out_i)
                else:
                    num = get_fasta(seq, merge, num, '-', args_term, out, out_i)