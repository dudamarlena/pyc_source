...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/optimize.py
...
def optimize_tss(args_ops, log):
        ...
        for strain in args_ops.strain_lengths.keys():
            ...
            for man in os.listdir(manual_path):
                if strain == man.replace('.gff', ''):
                    detect = True
                    log.write('The manually-curated set is found - {0}\n'.format(os.path.join(manual_path, man)))

            if not detect:
                log.write('The manually-curated set of {0} is not found.\n'.format(strain))
                print('Error: There are genomes in --genome_lengths which is not contained in manually-detected TSS gff files!')
                sys.exit()
    ...
    for man in os.listdir(manual_path):
        ...
        if prefix in args_ops.strain_lengths.keys():
            ...
            length = 'all'
            run = True
        log.write('The comparing sequence region of {0} is '.format(prefix, length))
        if run:
            gff_file = None
            ...
            if gff_file is None:
                print('Error: No corresponding genome annotation gff file of {0} can be found!'.format(prefix))
                sys.exit()
            Helper().check_uni_attributes(gff_file)
            log.write('Running optimize_TSSpredator.py for optimization.\n')
            optimization(wig_path, fasta_file, gff_file, args_ops, man_file, length, prefix, log)
            Helper().remove_all_content(os.path.join(args_ops.output_folder, 'optimized_TSSpredator'), 'config', 'file')
            Helper().remove_all_content(os.path.join(args_ops.output_folder, 'optimized_TSSpredator'), 'Master', 'dir')
