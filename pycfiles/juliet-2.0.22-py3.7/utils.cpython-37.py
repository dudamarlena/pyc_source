...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/juliet/utils.py
# Compiled at: 2020-02-28 13:46:38
# Size of source mod 2**32: 26724 bytes
import numpy as np
from astropy.io import fits
import pickle, batman, radvel
try:
    import catwoman
    have_catwoman = True
except:
...
def get_TESS_data(filename, fluxtype='PDCSAP_FLUX'):
    ...
    idx = np.where((data[fluxtype] != 0.0) & ~np.isnan(data[fluxtype]))[0]
    return (
     data['TIME'][idx], data[fluxtype][idx] / np.median(data[fluxtype][idx]),
...
def get_all_TESS_data(object_name, radius='.02 deg', get_PDC=True, get_all=False):
    ...
    for i in range(len(obs_table['dataURL'])):
        if 's_lc.fits' in obs_table['dataURL'][i]:
            ...
            if len(metadata) == 5:
                ...
                idx_goodpdc = np.where((f != 0.0) & ~np.isnan(f))[0]
                idx_goodsap = np.where((fs != 0.0) & ~np.isnan(fs))[0]
    ...
    if get_all:
        return (
         out_dict, times, fluxes, fluxes_errors)
...
def convert_ld_coeffs(ld_law, coeff1, coeff2):
    if ld_law == 'quadratic':
        q1 = (coeff1 + coeff2) ** 2
        q2 = coeff1 / (2.0 * (coeff1 + coeff2))
    else:
        if ld_law == 'squareroot':
            q1 = (coeff1 + coeff2) ** 2
            q2 = coeff2 / (2.0 * (coeff1 + coeff2))
        else:
            if ld_law == 'logarithmic':
                q1 = (1 - coeff2) ** 2
                q2 = (1.0 - coeff1) / (1.0 - coeff2)
    return (
     q1, q2)
...
def reverse_bp(r1, r2, pl, pu):
    ...
    for i in range(nsamples):
        if r1[i] > Ar:
            b[i], p[i] = (1 + pl) * (1.0 + (r1[i] - 1.0) / (1.0 - Ar)), (1 - r2[i]) * pl + r2[i] * pu
        else:
            b[i], p[i] = 1.0 + pl + np.sqrt(r1[i] / Ar) * r2[i] * (pu - pl), pu + (pl - pu) * np.sqrt(r1[i] / Ar) * (1.0 - r2[i])
...
def read_data(fname):
    ...
    return (
     ts, fs, ferrs, instruments, indexes, len(instrument_names), instrument_names, lm_boolean, lm_arguments)
        ...
        if line != '':
            if line[0] != '#':
                ...
                if pvector[0] == 'K':
                    ...
                    n_rv += 1
                elif prior_name.lower() == 'fixed':
                    priors[parameter]['distribution'] = input_dict or prior_name.lower()
                    ...
                    priors[parameter]['cvalue'] = 0.0
        else:
            break
        if input_dict and counter == n_allkeys - 1:
            break

    if not input_dict:
        return (
         priors, n_transit, n_rv, numbering_transit.astype('int'), numbering_rv.astype('int'), n_params)
...
def get_quantiles(dist, alpha=0.68, method='median'):
    ...
    if method == 'median':
        ...
        if nsamples % 2 == 0.0:
            ...
            param = (ordered_dist[med_idx_up] + ordered_dist[med_idx_down]) / 2.0
...
def bin_data(x, y, n_bin):
    ...
    return (
     np.array(x_bins), np.array(y_bins), np.array(y_err_bins))
    ...
    for pname in posteriors['posterior_samples'].keys():
        if pname != 'unnamed':
            if pname != 'loglike':
                ...
                if pname.split('_')[0] == 'r2':
                    ...
                    if 'ecosomega_' + planet in posteriors['posterior_samples']:
                        ...
                        omega = np.arctan2(posteriors['posterior_samples'][('esinomega_p' + str(iplanet))], posteriors['posterior_samples'][('ecosomega_p' + str(iplanet))])
                    else:
                        if 'secosomega_' + planet in posteriors['posterior_samples']:
                            iplanet = planet[1:]
                            ecc = posteriors['posterior_samples'][('secosomega_p' + str(iplanet))] ** 2 + posteriors['posterior_samples'][('sesinomega_p' + str(iplanet))] ** 2
                            omega = np.arctan2(posteriors['posterior_samples'][('sesinomega_p' + str(iplanet))], posteriors['posterior_samples'][('secosomega_p' + str(iplanet))])
                        else:
                            if 'ecc_' + planet in posteriors['posterior_samples']:
                                ecc = posteriors['posterior_samples'][('ecc_' + planet)]
                                omega = posteriors['posterior_samples'][('omega_' + planet)] * np.pi / 180.0
                            else:
                                ecc = 0.0
                                omega = 90.0
                    ecc_factor = (1.0 + ecc * np.sin(omega)) / (1.0 - ecc ** 2)
                    if 'rho' in posteriors['posterior_samples']:
                        G = 6.67408e-11
                        if 'P_' + planet in posteriors['posterior_samples']:
                            a = (posteriors['posterior_samples']['rho'] * G * (posteriors['posterior_samples'][('P_' + planet)] * 24.0 * 3600.0) ** 2 / (3.0 * np.pi)) ** 0.3333333333333333
                        else:
                            a = (posteriors['posterior_samples']['rho'] * G * (priors[('P_' + planet)]['hyperparameters'] * 24.0 * 3600.0) ** 2 / (3.0 * np.pi)) ** 0.3333333333333333
                    else:
                ...
                if pname.split('_')[0] == 'ecosomega':
                    ...
                    iplanet = planet[1:]
                    ecc = np.sqrt(posteriors['posterior_samples'][('ecosomega_p' + str(iplanet))] ** 2 + posteriors['posterior_samples'][('esinomega_p' + str(iplanet))] ** 2)
                    omega = np.arctan2(posteriors['posterior_samples'][('esinomega_p' + str(iplanet))], posteriors['posterior_samples'][('ecosomega_p' + str(iplanet))]) * (180 / np.pi)
                    val, valup, valdown = get_quantiles(ecc)
                    usigma = valup - val
                    dsigma = val - valdown
                    fout.write('{0:18} \t \t {1:.10f} \t \t {2:.10f} \t \t {3:.10f}\n'.format('ecc_' + planet, val, usigma, dsigma))
                    idx = np.where(omega > 0.0)[0]
            ...
            if pname.split('_')[0] == 'secosomega':
                ...
                iplanet = planet[1:]
                ecc = posteriors['posterior_samples'][('secosomega_p' + str(iplanet))] ** 2 + posteriors['posterior_samples'][('sesinomega_p' + str(iplanet))] ** 2
                omega = np.arctan2(posteriors['posterior_samples'][('sesinomega_p' + str(iplanet))], posteriors['posterior_samples'][('secosomega_p' + str(iplanet))]) * (180 / np.pi)
                val, valup, valdown = get_quantiles(ecc)
                usigma = valup - val
                dsigma = val - valdown
                fout.write('{0:18} \t \t {1:.10f} \t \t {2:.10f} \t \t {3:.10f}\n'.format('ecc_' + planet, val, usigma, dsigma))
...
import astropy.time as APYTime
