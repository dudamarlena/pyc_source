...
# [GCC 8.4.0]
# Embedded file name: /home/hedley/django/instances/django-link/link/forms.py
    ...
    def clean(self):
        ...
        for fieldname in ['view_name', 'target_content_type', 'url']:
            if cleaned_data.get(fieldname):
                if field_count:
                    raise forms.ValidationError('You may set at most one of view_name, target or URL.')
                field_count += 1

        if not field_count:
            raise forms.ValidationError('You may set at least one of view_name, target or URL.')
        return cleaned_data