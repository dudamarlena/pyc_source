...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/clue/bzrserver/wsgi.py
...
class SecuredBzrApp(object):
    ...
    def __init__(self, sourcedir, passwdfile=None, configfile=None, logger=utils.logger):
        self.logger = logger
        ...
        if self.security is None:
            if configfile:
                self.logger.info('Config file at "%s" does not exist, skipping' % configfile)
        ...
        if passwdfile:
            if os.path.exists(passwdfile):
                if pwdplugin is not None:
                    ...
                    self.logger.warn('No authenticator plugin has been configured to accept the passwd argument')
            else:
                self.logger.warn('Could not find passwd file, %r' % passwdfile)
        if pwdplugin is not None:
            self.logger.info('Config passwd file: %s' % pwdplugin.filename)
        self.app = whoconfig.PluggableAuthenticationMiddleware(BzrServerApp(sourcedir, logger, security=self.security), parser.identifiers, parser.authenticators, parser.challengers, parser.mdproviders, parser.request_classifier, parser.challenge_decider, utils.security_logger, None, parser.remote_user_key)
        return
...
class BzrServerApp(object):
    ...
    def __init__(self, sourcedir, logger, security=None):
        ...
        self.bzrapp = make_bzrlib_app(security=security, root=sourcedir, prefix='', path_var='PATH_INFO', readonly=False, load_plugins=True, enable_logging=False)
...
def make_secured_app(global_conf, sourcedir, passwdfile=None, configfile=None, logger=utils.logger):
    return SecuredBzrApp(sourcedir, passwdfile, configfile, logger)
...
class ACLTransport(chroot.ChrootTransport):
    ...
    def _call(self, methodname, relpath, *args):
        ...
        if methodname not in self.read and methodname not in self.write:
            raise errors.BzrError('Operation not supported: %s' % methodname)
        if relpath.find('/.bzr/branch/') > -1:
            branch = relpath[:relpath.find('/.bzr')]
            print 'User: %s;  branch: %s' % (_request_info.username, branch)
            if self.backing_transport.has(os.path.join(branch, '.bzr', 'branch')):
                if methodname in self.read and not self.security.can_read(branch):
                    _request_info.error = errors.PermissionDenied('No read access to branch: %s' % branch)
                    raise _request_info.error
                if methodname in self.write and not self.security.can_write(branch):
                    _request_info.error = errors.PermissionDenied('No write access to branch: %s' % branch)
                    raise _request_info.error
...
def make_bzrlib_app(security, root, prefix, path_var='REQUEST_URI', readonly=True, load_plugins=True, enable_logging=True):
