...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/aws_switchrole_links/__main__.py
# Compiled at: 2018-07-05 05:43:00
# Size of source mod 2**32: 2774 bytes
import argparse, json, logging, configparser, os, sys
...
def main():
    ...
    parser.add_argument('--config', type=str, default=AWS_CLI_CONFIG, help='path to aws config file')
    parser.add_argument('--format', type=str, choices=OUTPUT_FORMATS, default=TEXT, help='output format')
    ...
    for section in aws_config.sections():
        ...
        if section == 'default':
            ...
            if profile.get('role_arn') is None:
                ...
                item = {'signinUrl': signin_url, 
                 'displayName': display_name, 
                 'parameters': parameters}
                links.append(item)

    final_result = {'signinLinks': links}
    if args.format == JSON:
