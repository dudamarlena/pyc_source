...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/bird/_bird.py
# Compiled at: 2014-10-11 15:37:19
...
def _single_mp_run(x, Phi, bound, max_iter, verbose=False, pad=0, random_state=None, memory=Memory(None)):
    ...
    while (current_lambda > bound) & (it_number < max_iter):
        ...
        for scale_idx, size in enumerate(Phi.sizes):
            ...
            coeffs[(scale_idx * n):((scale_idx + 1) * n)] = mdct(residual[shift:shift + n], size).ravel()
...
def _single_multichannel_mp_run(X, Phi, bound, selection_rule, stop_crit, max_iter, verbose=False, pad=0, random_state=None, memory=Memory(None)):
    ...
    while (current_lambda > bound) & (it_number < max_iter):
        ...
        for s_idx, L in enumerate(Phi.sizes):
            ...
            for c_idx in range(n_channels):
                coeffs[c_idx, s_idx * n_samples:(s_idx + 1) * n_samples] = mdct(residual[c_idx, shift:shift + n_samples], L).ravel()
        ...
        for c_idx in range(n_channels):
            ...
            residual[c_idx, pos:pos + L] -= coeffs[(c_idx, idx)] * mdct_wf(L, freq_bin)
            X_est[c_idx, pos:pos + L] += coeffs[(c_idx, idx)] * mdct_wf(L, freq_bin)
            err_mse[c_idx].append(linalg.norm(residual[c_idx, :]))
            current_lambda_array[c_idx] = np.sqrt(1.0 - err_mse[c_idx][(-1)] / err_mse[c_idx][(-2)])
...
def _denoise(seeds, x, dico, sup_bound, n_atoms, verbose=False, indep=True, stop_crit=None, selection_rule=None, pad=0, memory=Memory(None)):
    ...
    for seed in seeds:
        ...
        if indep:
            approx.append(_single_mp_run(x, dico, sup_bound, n_atoms, verbose=verbose, pad=pad, random_state=seed, memory=memory)[0])
        else:
            approx.append(_single_multichannel_mp_run(x, dico, sup_bound, selection_rule, stop_crit, n_atoms, verbose=verbose, pad=pad, random_state=seed, memory=memory)[0])

    return approx


def _bird_core(X, scales, n_runs, Lambda_W, max_iter=100, stop_crit=np.mean, selection_rule=np.sum, n_jobs=1, indep=True, random_state=None, memory=Memory(None), verbose=False):
    ...
    approx = []
    rng = check_random_state(random_state)
    seeds = rng.randint(4294967295, size=n_runs)
    if n_jobs <= 0:
        n_cores = multiprocessing.cpu_count()
        n_jobs = min(n_cores + n_jobs + 1, n_cores)
    if indep:
        for r, x in zip(X_denoise, X):
            this_approx = Parallel(n_jobs=n_jobs)(delayed(_denoise)(this_seeds, x, Phi, Lambda_W, max_iter, pad=pad, verbose=verbose, indep=True, memory=memory) for this_seeds in np.array_split(seeds, n_jobs))
            this_approx = sum(this_approx[1:], this_approx[0])
            r[:] = sum([ a[pad:-pad] for a in this_approx ])
            approx.append(this_approx)

    else:
        this_approx = Parallel(n_jobs=n_jobs)(delayed(_denoise)(this_seeds, X, Phi, Lambda_W, max_iter, pad=pad, verbose=verbose, selection_rule=selection_rule, indep=False, memory=memory, stop_crit=stop_crit) for this_seeds in np.array_split(seeds, n_jobs))
...
def bird(X, scales, n_runs, p_above, max_iter=100, random_state=None, n_jobs=1, memory=Memory(None), verbose=False):
    ...
    print 'Starting BIRD with MDCT dictionary of %d Atoms. Lambda_W=%1.3f, n_runs=%d' % (
     M, Lambda_W, n_runs)
    X_denoised = _bird_core(X, scales, n_runs, Lambda_W, verbose=verbose, max_iter=max_iter, indep=True, n_jobs=n_jobs, random_state=random_state, memory=memory)
...
def s_bird(X, scales, n_runs, p_above, p_active=1, max_iter=100, random_state=None, n_jobs=1, memory=Memory(None), verbose=False):
    ...
    this_stop_crit = partial(stop_crit, lint=lint)
    this_selection_rule = partial(selection_rule, lint=lint)
    print 'Starting S-BIRD with MDCT dictionary of %d Atoms. Lambda_W=%1.3f, n_runs=%d, p_active=%1.1f' % (
     M, Lambda_W,
     n_runs, p_active)
