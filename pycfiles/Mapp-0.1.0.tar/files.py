...
# [GCC 8.4.0]
# Embedded file name: /home/user/workspace/mapp/mapp/dataset/files.py
# Compiled at: 2014-02-03 06:06:14
"""
...
from mapp.utils.common import fastalist_from_string, MappFile, AminoAcids, parse_mutations
...
class Dataset(object):
    ...
    def __init__(self, path, separator=','):
        ...
        with open(path, 'r') as (dataset):
            lines = [ line.rstrip('\n').split(separator) for line in dataset.readlines() ]
        self.header = lines[0]
        self.datalines = lines[1:]
...
class Settings(AnalysisFile):
    ...
    def create_from_default(default, fileid, newsettings):
        ...
        if not config.read(default):
            raise MappError('Creating settings Error', ('{0} file cannot be readed').format(default))
...
class Alignment(AnalysisFile):
    ...
    def __init__(self, dataid, path):
        ...
        try:
            ...
            raise MappError('Alignment object error', ("seqid {0} hasn't been founded in alignment {1}").format(dataid, path))
...
class Mapp(AnalysisFile):
    ...
    def get_values(self, mutation):
        ...
        """
        mutation = correct_mutations(self.id, [mutation], self._msa.get_contentstring())[0]
