...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/svnpuppy.py
# Compiled at: 2009-03-31 09:23:25
    ...
    def backup(self):
        ...
        for repo in check_list:
            ...
            if svn_instance.verify_repo() == False:
                ...
                continue
            try:
                dest_filename = self.incremental(basename(repo), self.dest, self.incCount, self.arc_type)
...
if __name__ == '__main__':
    ...
    parser.add_option('-R', '--repository', dest='src', help='Top level directory containing SVN repositories', default='/var/lib/svn')
    parser.add_option('-B', '--backup-directory/file', dest='dest', help='Top level directory under which backup will be stored', default='/var/backups/svn')
    parser.add_option('-c', '--compress', dest='compress', default='bz2')
    parser.add_option('-I', '--incremental', dest='inc', type='int', default=0, help='Increments value for incremental backup')
