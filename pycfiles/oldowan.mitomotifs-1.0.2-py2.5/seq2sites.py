...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-ppc/egg/oldowan/mitomotifs/seq2sites.py
# Compiled at: 2008-08-26 13:31:06
import re
...
class MatchingRange(object):
    ...
    def intersect(self, other):
        ...
        if (self.query_start in other_query_range or self.query_end in other_query_range) and (self.target_start in other_target_range or self.target_end in other_target_range):
            return True
        return False
...
def align(query, reference, word_size=15, mismatch_cutoff=0.7):
    ...
    if mismatch is not None:
        ...
        mismatch.target_end = mismatch.target_start + mismatch.query_end - mismatch.query_start
        mismatches.append(mismatch)
        mismatch = None
    polymorphisms = []
    merged_matches = []
    ...
    for mm in merged_matches:
        ...
        alignments = biopython.pairwise2.align.globalms(slice1, slice2, 3, -1, -3, -2)
        aln_polymorphisms = []
