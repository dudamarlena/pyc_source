...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/caserec/utils/split_database.py
# Compiled at: 2019-08-20 09:47:28
# Size of source mod 2**32: 5792 bytes
""""
...
class SplitDatabase(ReadFile):
    ...
    def __init__(self, input_file, dir_folds=None, n_splits=10, sep_read='\t', sep_write='\t', header=None, names=None, as_binary=False, binary_col=None, write_mode='w'):
        """
        ...
        """
        super(SplitDatabase, self).__init__(input_file, sep=sep_read, header=header, names=names, as_binary=as_binary, binary_col=binary_col)
    ...
    def write_files(self, trained_model):
        ...
        for train_index, test_index in trained_model:
            if self.dir_folds is not None:
                ...
                WriteFile(train_file, sep=(self.sep_write), mode=(self.write_mode)).write_with_pandas(df_train.sort_values(by=[0, 1]))
                WriteFile(test_file, sep=(self.sep_write), mode=(self.write_mode)).write_with_pandas(df_test.sort_values(by=[0, 1]))
                fold += 1
