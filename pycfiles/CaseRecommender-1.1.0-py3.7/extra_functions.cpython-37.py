...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/caserec/utils/extra_functions.py
...
class ComputeBui(object):
    ...
    def compute_bi(self):
        ...
        for item in self.training_set['items']:
            ...
            for user in self.training_set['users_viewed_item'][item]:
                self.bi[item] = self.bi.get(item, 0) + float(self.training_set['feedback'][user][item]) - self.training_set['mean_value'] - self.bu.get(user, 0)
                cont += 1
    ...
    def compute_bu(self):
        ...
        for user in self.training_set['users']:
            ...
            for item in self.training_set['items_seen_by_user'][user]:
                self.bu[user] = self.bu.get(user, 0) + float(self.training_set['feedback'][user][item]) - self.training_set['mean_value'] - self.bi.get(item, 0)
                cont += 1
    ...
    def compute_bui(self):
        for user in self.training_set['users']:
            for item in self.training_set['items']:
                try:
                    self.bui.setdefault(user, {}).update({item: self.training_set['mean_value'] + self.bu[user] + self.bi[item]})
                except KeyError:
