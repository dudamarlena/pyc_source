...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/caserec/recommenders/rating_prediction/base_rating_prediction.py
# Compiled at: 2019-08-20 09:47:28
# Size of source mod 2**32: 7503 bytes
""""
...
class BaseRatingPrediction(object):
    ...
    def __init__(self, train_file, test_file, output_file=None, similarity_metric='cosine', sep='\t', output_sep='\t'):
        """
    ...
    def create_matrix(self):
        ...
        for user in self.train_set['users']:
            for item in self.train_set['feedback'][user]:
                self.matrix[self.user_to_user_id[user]][self.item_to_item_id[item]] = self.train_set['feedback'][user][item]
    ...
    def evaluate(self, metrics, verbose=True, as_table=False, table_sep='\t'):
        ...
        results = RatingPredictionEvaluation(verbose=verbose, as_table=as_table, table_sep=table_sep, metrics=metrics).evaluate_recommender(predictions=(self.predictions),
    ...
    def compute(self, verbose=True):
        ...
        if verbose:
            test_info = None
            main_info = {'title':'Rating Prediction > ' + self.recommender_name, 
            ...
            if self.test_file is not None:
                test_info = {'n_users':len(self.test_set['users']),  'n_items':len(self.test_set['items']), 
