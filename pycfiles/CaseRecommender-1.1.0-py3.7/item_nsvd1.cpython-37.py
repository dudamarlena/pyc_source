...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/caserec/recommenders/rating_prediction/item_nsvd1.py
...
class ItemNSVD1(BaseNSVD1):
    ...
    def __init__(self, train_file=None, test_file=None, metadata_file=None, output_file=None, epochs=30, learn_rate=0.01, delta=0.015, factors=10, init_mean=0, init_stdev=0.1, stop_criteria=0.001, batch=False, n2=10, learn_rate2=0.01, delta2=0.015, sep='\t', output_sep='\t', metadata_sep='\t', metadata_as_binary=False, random_seed=None):
        ...
        """
        super(ItemNSVD1, self).__init__(train_file=train_file, test_file=test_file, output_file=output_file, factors=factors,
          init_mean=init_mean,
                ...
                for _ in range(self.n2):
                    for i, item in enumerate(self.items):
                        ...
                        for l in self.non_zero_x[i]:
                            self.w[l] += self.learn_rate2 * (self.d[i] * np.dot(self.x[i][l], e.T) - self.w[l] * self.delta2)
