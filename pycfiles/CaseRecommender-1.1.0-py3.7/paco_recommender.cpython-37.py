...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/caserec/recommenders/item_recommendation/paco_recommender.py
...
class PaCoRecommender(object):
    ...
    def __init__(self, train_file, test_file=None, output_file=None, k_row=None, l_col=None, density_low=0.01, as_binary=True, min_density=0.3):
        """
    ...
    def recommender(self):
        for n, k in enumerate(self.co_clustering.list_row):
            ...
            for u_idx in k:
                ...
                unseen_items = set()
                for l in cols:
                    if self.co_clustering.density[(n, l)] != 0 and self.co_clustering.density[(n, l)] != 1 and self.co_clustering.density[(n, l)] >= self.min_density:
    ...
    def compute(self, verbose=True, metrics=list(['PREC', 'RECALL', 'MAP', 'NDCG']), verbose_evaluation=True, as_table=False, table_sep='\t'):
        if verbose:
            print('[Case Recommender: Item Recommendation > %s]\n' % self.recommender_name)
        self.run_co_clustering()
        self.recommender()
        if self.test_file is not None:
            ItemRecommendationEvaluation(metrics=metrics, as_table=as_table, table_sep=table_sep, verbose=verbose_evaluation).evaluate_recommender(self.predictions, self.test_set)