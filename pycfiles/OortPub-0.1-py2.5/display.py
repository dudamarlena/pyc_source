...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/oort/display.py
# Compiled at: 2007-09-30 15:55:47
"""
"""
import os.path
    ...
    def generate_query_items(self, graph, resource, lang):
        ...
        if self.queries:
            for (name, query) in self.queries.items():
                yield (
                 name, query(graph, lang, resource))
...
class Aspect(TemplateAspectBase):
    ...
    doctypes = dict(((key.lower().replace('_', '-'), item) for (key, item) in DocType.__dict__.items() if key.isupper() if isinstance(item, tuple)))
    ...
    def make_result_generator(self, graph, resource, lang, data=None):
        ...
        def generator(format, encoding):
            ...
            yield tplt.generate(**allData).render(format, doctype=doctype, encoding=encoding)
...
class TpltAspect(TemplateAspectBase):
    ...
    def post_init_configure(self, cfg):
        self.__super.post_init_configure(cfg)
        (self.engine, self.path) = self.engineManager.get_engine_and_path(self.templateBase + '.' + self.templateName, self.defaultEngine)
