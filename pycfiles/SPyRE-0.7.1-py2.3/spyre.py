...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\spyre.py
# Compiled at: 2006-02-24 20:00:32
"""
...
class OrthoCam(object):
    ...
    def viewProj(self):
        ...
        ogl.glLoadIdentity()
        ogl.glOrtho(self.left, self.right, self.bottom, self.top, self.near, self.far)

    def viewMV(self):
        """Sets eye and center position in OpenGL """
        ogl.glMatrixMode(ogl.GL_MODELVIEW)
        ogl.glLoadIdentity()
        oglu.gluLookAt(self.eye[0], self.eye[1], self.eye[2], self.center[0], self.center[1], self.center[2], self.up[0], self.up[1], self.up[2])
...
class FrustumCam(OrthoCam):
    ...
    def viewProj(self):
        ...
        ogl.glMatrixMode(ogl.GL_PROJECTION)
        ogl.glLoadIdentity()
        ogl.glFrustum(self.left, self.right, self.bottom, self.top, self.near, self.far)
...
class Camera(object):
    ...
    def __init__(self, engine, eye, center, up, left, right, bottom, top, near, far):
    ...
    def saveInit(self):
        ...
        self.__saveinit = (
         self._eye, self._center, self.left, self.right, self.bottom, self.top, self.near, self.far)
    ...
    def restoreInit(self):
        ...
        (self.left, self.right, self.bottom, self.top, self.near, self.far) = stuff[2:]
    ...
    def setViewport(self, left, top, right, bottom):
        ...
        self.viewport = (int(left * w), int(top * h), int((right - left) * w), int((bottom - top) * h))
...
class BasicCamera(Camera):
    ...
    def __init__(self, engine, eye, center=(
 0, 0, 0), up=(0, 1, 0), left=None, right=None, bottom=None, top=None, near=None, far=None):
        if left is None:
            left = -5
        if right is None:
            right = 5
        ...
        if far is None:
            far = 11
        Camera.__init__(self, engine, eye, center, up, left, right, bottom, top, near, far)
        self.saveInit()
        return
...
class PrecessingCamera(BasicCamera):
    ...
    def __init__(self, engine, distance, height, rate, center=(
 0, 0, 0), up=(0, 0, 1)):
    ...
    def calc(self):
        ...
        self.eye = (
         self.distance * math.cos(self.angle), self.distance * math.sin(self.angle), self.height)
...
class MobileCamera(BasicCamera):
    ...
    def __init__(self, engine, eyeRho, center=(
 0, 0, 0), up=(0, 0, 1)):
        """Initialize new camera.
        @param engine: the engine
        ...
        if type(eyeRho) == type((0,)):
            eyeVec = la.Vector(eyeRho[0] - center[0], eyeRho[1] - center[1], eyeRho[2] - center[2])
            rho = eyeVec.norm()
    ...
    def _setEye(self, eye):
        ...
        ctr = self.center
        if type(eye) == type((1,)):
            (self.rho, self.theta, self.phi) = cartesianToSpherical(eye[0] - ctr[0], eye[1] - ctr[1], eye[2] - ctr[2])
            self.calc()
            self.dirty = True
    ...
    def calc(self):
        ...
        self._eye = (there[0] + ctr[0], there[1] + ctr[1], there[2] + ctr[2])
...
class RovingCamera(BasicCamera):
 ...
 2, 2, 2), center=(0, 0, 0), up=(0, 0, 1), elev=None):
        """Initialize new camera.
        @param engine: the engine
        ...
        if elev == None:
            elev = lambda x, y: 0
        self.rho = math.sqrt((center[0] - eye[0]) ** 2 + (center[1] - eye[1]) ** 2)
...
class RovingCameraY(BasicCamera):
 ...
 2, 2, 2), center=(0, 0, 0), up=(0, 0, 1), elev=None):
        ...
        if elev == None:
            elev = lambda x, y: 0
        self.rho = math.sqrt((center[0] - eye[0]) ** 2 + (center[2] - eye[2]) ** 2)
        self.theta = math.acos((center[0] - eye[0]) / self.rho)
        if eye[2] < center[2]:
            self.theta = twoPi - self.theta
...
class PivotingInterface(BasicInterface):
    ...
    def mouseDrag(self, button, loc):
        ...
        abT = (1.0 * loc[0] / self.engine.width * cam.right, -1.0 * loc[1] / self.engine.height * cam.top)
        abS = (
         1.0 * self.mouseMark[0] / self.engine.width * cam.right, -1.0 * self.mouseMark[1] / self.engine.height * cam.top)
        deltaX = -self.sensitivity * (abT[0] - abS[0])
        deltaY = -self.sensitivity * (abT[1] - abS[1])
        eyeVec = la.Vector(cam.eye[0] - cam.center[0], cam.eye[1] - cam.center[1], cam.eye[2] - cam.center[2])
        ...
        (cam.rho, cam.theta, cam.phi) = cartesianToSpherical(eyeVec.x, eyeVec.y, eyeVec.z)
        self.mouseMark = loc
        cam.refresh()
        self.engine.redisplay()
...
class CursorKeyInterface(PanningInterface):
    ...
    def pivotUp(self, empty=None):
        ...
        event.post(ev)
        ev = event.Event(MOUSEMOTION, pos=(self.mouseLoc[0], self.mouseLoc[1] + self.increment), rel=(0, 0), buttons=1)
        event.post(ev)
    ...
    def pivotLeft(self, empty=None):
        ev = event.Event(MOUSEBUTTONDOWN, pos=(0, 0), button=1)
        event.post(ev)
        ev = event.Event(MOUSEMOTION, pos=(self.mouseLoc[0] - self.increment, self.mouseLoc[1]), rel=(0, 0), buttons=1)
        event.post(ev)
        ev = event.Event(MOUSEBUTTONUP, pos=(0, 0), button=1)
        event.post(ev)
...
class Light(object):
    ...
    __module__ = __name__
    available_lights = [
     ogl.GL_LIGHT7, ogl.GL_LIGHT6, ogl.GL_LIGHT5, ogl.GL_LIGHT4, ogl.GL_LIGHT3, ogl.GL_LIGHT2, ogl.GL_LIGHT1, ogl.GL_LIGHT0]
...
class Spot(Bulb):
    ...
    def __init__(self, cutoff, direction, ambient=None, diffuse=None, specular=None, position=None):
        """Initialize new instance. 
...
class NullStudio(object):
    ...
    def depthCueing(self, status=True, fmode=None, density=None, near=None, far=None):
...
class StudioColorMat(NullStudio):
    ...
    def __init__(self, engine, ambient=(
 0.15, 0.15, 0.15, 1.0)):
        """Initialize new instance.
        @param engine: the engine
...
class TimeKeeper(Object):
    ...
    def __init__(self, fps=24):
        ...
        self.startTime = None
        self.display_interval = 1.0 / fps
        self.play = 1
        self.timePeriod = 2
        (self.frameMark, self.timeMark) = (0, time.time())
        self.frameRate = 0.0
    ...
    def shutdown(self):
        """Turn timer off."""
        pytime.set_timer(self.timerEventID, 0)

    def set_timer(self):
        """Enable timekeeper."""
        pytime.set_timer(self.timerEventID, int(self.display_interval * 1000))
...
class Engine(object):
 ...
 320, 320)):
    ...
    def init(self):
        ...
        self.window = display.set_mode((self.width, self.height), self.mode)
