...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\zoe_objects.py
# Compiled at: 2005-12-18 22:32:47
"""
...
__license__ = 'LGPL'
import OpenGL.GL as ogl, spyre, pygame, types
...
class StatusObject(spyre.Object):
    ...
    def drawText(self, textString):
        ...
        ogl.glDrawPixels(textSurface.get_width(), textSurface.get_height(), ogl.GL_RGBA, ogl.GL_UNSIGNED_BYTE, textData)
        return
...
class FrameRateCounter(StatusObject):
    ...
    def __init__(self, engine, x=10, y=10, style=0):
    ...
    def display(self):
        """ draw the object """
        self.displayText(self.x, self.y, self.style, 'frame %d rate %.2f' % (spyre.Object.runTurn, self.engine.runTimer.frameRate))
...
class RotateTransform(Transform):
    ...
    def __init__(self, angle, ray=(
 0, 0, 1)):
        self.angle = angle
        self.ray = ray

    def apply(self):
        ogl.glRotated(self.angle * spyre.radiansToDegrees, self.ray[0], self.ray[1], self.ray[2])
...
class RotatingGroup(TransformGroup):
    ...
    def __init__(self, angularSpeed, ray=(
 0, 0, 1), objects=None):
...
class NewtonianParticle(Particle):
 ...
 0, 0, 0), vel=(0, 0, 0)):
    ...
    def impulse(self, deltavee):
        ...
        self.vel = (
         self.vel[0] + deltavee[0], self.vel[1] + deltavee[1], self.vel[2] + deltavee[2])
