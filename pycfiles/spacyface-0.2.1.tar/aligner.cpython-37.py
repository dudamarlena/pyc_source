...
# Compiled at: 2020-01-16 16:50:21
# Size of source mod 2**32: 8807 bytes
from typing import List, Iterable, Union
import spacy
import spacy.tokens.token as SpacyToken
import spacy.tokens.doc as SpacyDoc
import torch, regex as re
...
def MakeAligner(pretrained_tokenizer, spacy_language_model):
    ...
    class Aligner(pretrained_tokenizer):
        ...
        def sentence_to_input(self, sentence: str):
            ...
            model_input = {k:torch.tensor(v).unsqueeze(0) for k, v in raw_model_input.items() if isinstance(v, List) if isinstance(v, List)}
            meta_input = self.prepare_for_model(meta_tokens)['input_ids']
            new_meta = list(map(self._maybe_conv_to_token, meta_input))
            return (
             model_input, new_meta)
