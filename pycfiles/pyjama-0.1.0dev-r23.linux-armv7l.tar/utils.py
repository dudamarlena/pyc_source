...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.5/site-packages/pyjama/gui/hildon/utils.py
...
from Queue import *
from pyjama.gui.gtk.utils import LOGO_PIXBUF
...
class PyjamaStackableWindow(hildon.StackableWindow):
    """
...
class AboutView(PyjamaStackableWindow):
    """
    ...
    def init_center_view(self, centerview):
        ...
        centerview.add(gtk.Label('Pyjama - ' + version.getInstance().getVersion()))
        centerview.add(gtk.Label('Pyjama is a python project bootstraper for Maemo5 development'))
        centerview.add(gtk.Label('by Thierry Bressure - http://blog.pyjama.bressure.net'))
    ...
    def on_blog_clicked_event(self, widget, data):
        call_handled_method(open_browser_on, 'http://blog.pyjama.bressure.net')

    def on_site_clicked_event(self, widget, data):
        call_handled_method(open_browser_on, 'http://pyjama.bressure.net')

    def on_group_clicked_event(self, widget, data):
        call_handled_method(open_browser_on, 'http://group.pyjama.bressure.net')


class BugReportView(PyjamaStackableWindow):
    """
    ...
    def __init__(self, type, value, traceback):
        ...
        PyjamaStackableWindow.__init__(self, title='Bug reporting')
    ...
    def init_center_view(self, centerview):
        ...
        return PyjamaStackableWindow.init_center_view(self, centerview)

    def init_bottom_button(self, bottomButtons):
        post = self.create_button('Post issue', None)
        post.connect('clicked', self.on_post_button_clicked, self)
        self.add_button(post)
        return PyjamaStackableWindow.init_bottom_button(self, bottomButtons)
        ...
        try:
            ...
            if response == gtk.RESPONSE_OK:
                try:
                    ...
                    issues_client.client_login(username, password, 'pyjama', 'code')
...
class AsyncTask(threading.Thread):
    ...
    def __init__(self, window, stop_hook, method, *argv, **kwarg):
        self.task_name = 'Pyjama-AsyncTask'
