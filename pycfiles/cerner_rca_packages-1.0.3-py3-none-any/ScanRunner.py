...
# Embedded file name: build/bdist.macosx-10.6-intel/egg/ScanEvent/ScanRunner.py
# Compiled at: 2012-04-25 05:20:11
...
class ScanRunner:
    ...
    def __init__(self, conf, scanEvent):
        ...
        self.reportMgr = ReportManager(self.scanEvent.getName(), self.scanEvent.getScansToKeep(), self.conf['archivePath'])
        ...
        for emailItem in self.scanEvent.getEmailItems():
            ...
            for report in emailItem.getReportList():
                if report not in self.finalReports:
                    ...
                    self.finalReports[report] = ReportFramework.writeSRReport(self.rf, outputPath, report)
    ...
    def launch(self):
        uuid = self.nessusComm.performScan_By_PolicyName(self.scanEvent.getName(), self.scanEvent.getTargets(), self.scanEvent.getPolicy())
