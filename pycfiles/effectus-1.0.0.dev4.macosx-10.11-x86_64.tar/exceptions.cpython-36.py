...
# [GCC 8.4.0]
# Embedded file name: /Users/benjaminweb/22_Projekte/Pareto/python/venv/lib/python3.6/site-packages/effectus/exceptions.py
# Compiled at: 2017-01-30 15:56:59
...
class StringFound(ValueError):
    __doc__ = 'Raised if a string is found where only values are allowed.'


class InvalidFrequencyCount(ValueError):
    __doc__ = 'Raised if number of values does not equal that of frequencies.'


class InvalidFrequencyValue(ValueError):
    __doc__ = 'Raised if frequency is not None or not an integer.'


class MixedPrefixes(ValueError):
    __doc__ = 'Raised if values contain negative and positive ones.'


class ExcelNotOpen(OSError):
    __doc__ = 'Raised if Excel is not open.'


class ExcelNoValuesSelected(OSError):
    __doc__ = 'Raised if no selection has been made in Excel.'


class InvalidLimit(ValueError):
    __doc__ = 'Raised if limit is not between 0 and 1 (including).'
...
def check_vafreqs(vafreqs):
    ...
    Raises:
        StringFound: No value must be a string.
...
def check_and_unfold_vafreqs(vafreqs):
    ...
    try:
        ...
        raise UsageError('no value must be a string - but at least one does.')
    except MixedPrefixes:
        raise UsageError('observations may be either negative or positive only.')
    except InvalidFrequencyCount:
        raise UsageError('each value must have a frequency.')
    except InvalidFrequencyValue:
        raise UsageError('each frequency has to be a positive integer.')

    effect_values = unfold_values(vafreqs)
    if len(effect_values) == 0:
        raise UsageError('Could not extract any value. Check file. If your column separator is not comma (,), specify it with `--delimiter` option.')
    return effect_values


def no_string(vafreqs):
    """Returns False if a string is in vafreq."""
    for vafreq in vafreqs:
        if isinstance(vafreq, tuple):
            value, frequency = vafreq
            if isinstance(value, str) or isinstance(frequency, str):
                return False
        else:
            value = vafreq
