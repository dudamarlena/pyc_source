...
# [GCC 8.4.0]
# Embedded file name: /Users/benjaminweb/22_Projekte/Pareto/python/venv/lib/python3.6/site-packages/effectus/helpers.py
...
def frac_gcd(frac1, frac2):
    ...
    return (
     the_gcd, int(new_frac1_cd), int(new_frac2_cd))
...
def make_relations(effect_values):
    ...
    for ordinal, value in enumerate(sort_effects(effect_values), 1):
        ...
        relations.append((Fraction(ordinal, count),
         Fraction(Fraction.from_float(runner), Fraction.from_float(total))))
...
def get_majors(relations):
    ...
    for given_causes, given_effects in relations:
        if given_causes - Fraction(2, 100) <= Fraction(1, 2) and given_effects + Fraction(2, 100) >= Fraction(1, 2):
...
def format_summary(summary, precision=3):
    ...
    """
    if summary['causes'].denominator > 100 or summary['effects'].denominator > 100:
        causes = round(float(summary['causes']), precision)
        effects = round(float(summary['effects']), precision)
    else:
        causes = '{}/{}'.format(summary['causes'].numerator, summary['causes'].denominator)
        effects = '{}/{}'.format(summary['effects'].numerator, summary['effects'].denominator)
    return (
     causes, effects)
