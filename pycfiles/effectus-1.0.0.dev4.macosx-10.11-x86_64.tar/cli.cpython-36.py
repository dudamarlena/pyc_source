...
# [GCC 8.4.0]
# Embedded file name: /Users/benjaminweb/22_Projekte/Pareto/python/venv/lib/python3.6/site-packages/effectus/cli.py
# Compiled at: 2017-01-31 19:05:29
...
"""
import csv, click, xlwings as xw
from effectus.helpers import get_values, sort_effects, format_summary, make_relations
from effectus.exceptions import check_and_unfold_vafreqs, check_valid_limit, InvalidLimit, ExcelNotOpen, ExcelNoValuesSelected
from effectus.core import make_summary, attain_effects, attain_causes, separate_causes, separate_effects
...
def check_cli_limit(limit):
    ...
    try:
        ...
        raise click.UsageError('Please provide a limit between 0 and 1 (including).')


def skip_blanks(xl_vafreqs):
    """Skip blank entries from Excel selection."""
    return list(filter(lambda x: x is not None and x != (None, None), xl_vafreqs))
...
def print_summary(summary):
    ...
    if summary:
        ...
        click.echo('{}{:.2f}'.format('Variability'.ljust(15), summary['variability']))


@click.command()
@click.argument('filename')
@click.option('-d', '--delimiter', default=',', help="Column delimiter (default: ',')")
@click.option('--observations', default=1, type=(click.INT), help='Column number of observations (default: 1)')
@click.option('--frequencies', default=2, type=(click.INT), help='Column number of frequencies (default: 2)')
@click.option('-p', '--precision', default=2, help='Number of decimals to show (default: 2)')
def cli_make_summary(filename, delimiter, observations, frequencies, precision):
    ...
    with open(filename) as (csvfile):
        ...
        summary['causes'], summary['effects'] = format_summary(summary, precision)
...
def cli_make_summary_xl():
    ...
    summary.update(ratio=(ratio(effect_values)))
    summary.update(pareto=(summary['ratio'] <= 1))
    summary['causes'], summary['effects'] = format_summary(summary, precision=3)
    print_summary(summary)


@click.command()
@click.argument('filename')
@click.option('-l', '--limit', default=0.8, type=(click.FLOAT), help='Effects ratio between 0 and 1 to attain')
@click.option('-d', '--delimiter', default=',', help="Column delimiter (default: ',')")
@click.option('-p', '--precision', default=3, help='Number of decimals to show (default: 3)')
@click.option('--observations', default=1, type=(click.INT), help='Column number of observations (default: 1)')
@click.option('--frequencies', default=2, type=(click.INT), help='Column number of frequencies (default: 2)')
def cli_attain_effects(filename, limit, delimiter, precision, observations, frequencies):
...
@click.command()
@click.argument('filename')
@click.option('-l', '--limit', default=0.2, type=(click.FLOAT), help='Causes ratio between 0 and 1 to attain')
@click.option('-d', '--delimiter', default=',', help="Column delimiter (default: ',')")
@click.option('-p', '--precision', default=3, help='Number of decimals to show (default: 3)')
@click.option('--observations', default=1, type=(click.INT), help='Column number of observations (default: 1)')
@click.option('--frequencies', default=2, type=(click.INT), help='Column number of frequencies (default: 2)')
def cli_attain_causes(filename, limit, delimiter, precision, observations, frequencies):
...
@click.command()
@click.argument('filename')
@click.option('-l', '--limit', default=0.2, type=(click.FLOAT), help='Causes ratio between 0 and 1 to attain (default: 0.2)')
@click.option('-d', '--delimiter', default=',', help="Column delimiter (default: ',')")
@click.option('--observations', default=1, type=(click.INT), help='Column number of observations (default: 1)')
@click.option('--frequencies', default=2, type=(click.INT), help='Column number of frequencies (default: 2)')
def cli_separate_causes(filename, limit, delimiter, observations, frequencies):
...
@click.command()
@click.option('-l', '--limit', default=0.8, type=(click.FLOAT), help='Effects ratio between 0 and 1 to attain (default: 0.8)')
...
@click.option('--observations', default=1, type=(click.INT), help='Column number of observations (default: 1)')
@click.option('--frequencies', default=2, type=(click.INT), help='Column number of frequencies (default: 2)')
def cli_separate_effects(filename, limit, delimiter, observations, frequencies):
    ...
    with open(filename) as (csvfile):
        ...
        effect_values = check_and_unfold_vafreqs(vafreqs)
        sorted_effects = sort_effects(effect_values)
        separator, equal, total_equals = separate_effects(sorted_effects, limit)
        click.echo('{}{}'.format('Separator'.ljust(15), separator))
        if equal < total_equals:
            click.echo('{}{}'.format('Element'.ljust(15), equal))
            click.echo('{}{}'.format('of'.ljust(15), total_equals))
