...
# [GCC 8.4.0]
# Embedded file name: /Users/benjaminweb/22_Projekte/Pareto/python/venv/lib/python3.6/site-packages/effectus/core.py
# Compiled at: 2017-01-31 19:05:29
# Size of source mod 2**32: 6600 bytes
"""
    effectus.core
    ~~~~~~~~~~~~~
...
from effectus.helpers import get_majors, get_relevant, pick_best, get_gcd
...
def attain_effects(relations, required_effects=Fraction(8, 10), precision=3):
...
def separate_causes(sorted_effects, required_causes, precision=3):
    ...
    for ordinal, current_effect_value in enumerate(sorted_effects):
        ...
        if ordinal > 0:
            ...
            if causes_cml_ge:
                total_equal_count = sum(1 for x in sorted_effects if x == current_effect_value)
...
def separate_effects(sorted_effects, required_effects, precision=3):
    ...
    for ordinal, current_effect_value in enumerate(sorted_effects):
        ...
        effects_cml_perc = Fraction(effects, Fraction.from_float(total_effects))
        if ordinal > 0:
            previous_effect_value = sorted_effects[(ordinal - 1)]
            if previous_effect_value == current_effect_value:
                equals_count += 1
