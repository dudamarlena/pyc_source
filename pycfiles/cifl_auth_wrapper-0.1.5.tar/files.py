...
# Embedded file name: build/bdist.linux-i686/egg/cifitlib/files.py
...
import re, os, sys, stat, pwd, grp, tempfile, popen2, time, logging, filecmp, shutil
...
class executor(object):
    ...
    def run(self, command):
        ...
        output += [ line[:-1] for line in runpipe.fromchild.readlines() if line
                  ]
        return (
         cmdstat, output)
...
def calcPerms(perms):
    """This compares ondisk permissions with specified ones
        ...
        """
    pdisp = [{1: stat.S_ISVTX, 2: stat.S_ISGID, 4: stat.S_ISUID}, {1: stat.S_IXUSR, 2: stat.S_IWUSR, 4: stat.S_IRUSR}, {1: stat.S_IXGRP, 2: stat.S_IWGRP, 4: stat.S_IRGRP}, {1: stat.S_IXOTH, 2: stat.S_IWOTH, 4: stat.S_IROTH}]
...
def getOctPerms(filename):
    ...
    pmap = [{stat.S_IRUSR: 4, stat.S_IWUSR: 2, stat.S_IXUSR: 1}, {stat.S_IRGRP: 4, stat.S_IWGRP: 2, stat.S_IXGRP: 1}, {stat.S_IROTH: 4, stat.S_IWOTH: 2, stat.S_IXOTH: 1}]
    pdigits = [
     0, 0, 0]
