...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/panstamps/downloader.py
...
class downloader(object):
    ...
    **Usage**
    ...
    def __init__(self, log, downloadDirectory=False, settings=False, fits=True, jpeg=False, arcsecSize=60, filterSet='gri', color=True, singleFilters=True, ra=False, dec=False, imageType='stack', mjdStart=False, mjdEnd=False, window=False):
        self.log = log
    ...
    def get(self):
        ...
        **Return**
        ...
        if 'No PS1 3PI images were found' in str(content):
            self.log.warning("No images found. PS1 3Pi has not covered this area of the sky. Here's the requested URL:\n%(url)s" % locals())
            return ([], [], [])
        allStacks, allWarps, colorImage = self.parse_html_for_image_urls_and_metadata(content=content)
        ...
        if self.singleFilters:
            for images in [allStacks, allWarps]:
                ...
                fitsFilenames = []
                if self.fits:
                    fitsFilenames[:] = [ t + '.fits' for t in images['filenames'] ]
                    urls += images['fits']
                fitsPaths += self._download_images(urls=urls, filenames=fitsFilenames, downloadDirectory=downloadDirectory)
                urls = []
                jpegFilenames = []
                if self.jpeg:
                    jpegFilenames[:] = [ t + '.jpeg' for t in images['filenames'] ]
                    urls += images['jpegs']
                jpegPaths += self._download_images(urls=urls, filenames=jpegFilenames, downloadDirectory=downloadDirectory)

        if self.color:
            theseFilenames = []
            theseFilenames[:] = [ t + '.jpeg' for t in colorImage['filename'] ]
            colorPath += self._download_images(urls=colorImage['jpeg'], filenames=theseFilenames, downloadDirectory=downloadDirectory)
        self.log.debug('completed the ``get`` method')
        if len(fitsPaths + jpegPaths + colorPath) == 0:
            self.log.warning("No images found. Your options may not be set correctly. Here's the requested URL:\n%(url)s" % locals())
    ...
    def get_html_content(self):
        ...
        **Usage**
        ...
        if jpegSize < 1200:
            jpegSize = 1200
        try:
            response = requests.get(url='http://plpsipp1v.stsci.edu/cgi-bin/ps1cutouts', params={'pos': pos, 
               'filter': filterSet, 
    ...
    def parse_html_for_image_urls_and_metadata(self, content):
        """
        ...
        **Return**
        ...
        """
        self.log.debug('completed the ````parse_html_for_image_urls_and_metadata`` method')
        stackFitsUrls = []
        warpFitsUrls = []
        stackJpegUrls = []
        warpJpegUrls = []
        ...
        warpJpegFilename = []
        colorJpegFilename = []
        allStacks = {'jpegs': [], 'fits': [], 'filenames': [], 'filters': []}
        allWarps = {'jpegs': [], 'fits': [], 'filenames': []}
        colorImage = {'jpeg': [], 'filename': []}
        reFitscutouts = re.compile('<th>(?P<imagetype>\\w+)\\s+(?P<skycellid>\\d+.\\d+)\\s+(?P<ffilter>[\\w\\\\]+)(\\s+(?P<mjd>\\d+\\.\\d+))?(\\s<a.*\\(warning\\)</a>)?<br.*?href="(http:)?//plpsipp1v.*?Display</a>.*?Fits cutout" href="(?P<fiturl>(http:)?//plpsipp1v.*?\\.fits)".*?</th>', re.I)
        ...
        reJpegs = re.compile('<img src="(?P<jpegUrl>(http:)?//plp.*?skycell.*?)\\"', re.I)
        ...
        for item in thisIter:
            ...
            if 'red' in jpegUrl and 'blue' in jpegUrl:
                ...
                self.log.warning("We are not downloading this jpeg: '%(jpegUrl)s'" % locals())

        reFitsMeta = re.compile('http?.*?\\?.*?skycell\\.(?P<skycell>\\d+\\.\\d+).*?x=(?P<ra>\\d+\\.\\d+).*?y=(?P<dec>[+|-]?\\d+\\.\\d+).*?size=(?P<pixels>\\d+).*?stk\\.(?P<ffilter>\\w+).*?fits', re.S | re.I)
        filterMjd = --- This code section failed: ---

 L. 448         0  LOAD_DEREF            0  'self'
                3  LOAD_ATTR             0  'mjdStart'
                6  UNARY_NOT        
                7  POP_JUMP_IF_TRUE     52  'to 52'
               10  LOAD_GLOBAL           1  'float'

 L. 449        13  LOAD_FAST             0  'x'
               16  CALL_FUNCTION_1       1  None
               19  LOAD_DEREF            0  'self'
               22  LOAD_ATTR             2  'mjdEnd'
               25  COMPARE_OP            0  <
               28  POP_JUMP_IF_FALSE    56  'to 56'
               31  LOAD_GLOBAL           1  'float'
               34  LOAD_FAST             0  'x'
               37  CALL_FUNCTION_1       1  None
               40  LOAD_DEREF            0  'self'
               43  LOAD_ATTR             0  'mjdStart'
               46  COMPARE_OP            4  >
             49_0  COME_FROM            28  '28'
             49_1  COME_FROM             7  '7'
               49  POP_JUMP_IF_FALSE    56  'to 56'
               52  LOAD_GLOBAL           3  'True'
               55  RETURN_VALUE_LAMBDA
             56_0  COME_FROM            49  '49'
               56  LOAD_GLOBAL           4  'False'
               59  RETURN_VALUE_LAMBDA
               -1  LAMBDA_MARKER    
...
Parse error at or near `COME_FROM' instruction at offset 56_0
        ...
        reFitsMeta = re.compile('http?.*?\\?.*?skycell\\.(?P<skycell>\\d+\\.\\d+).*?x=(?P<ra>\\d+\\.\\d+).*?y=(?P<dec>[+|-]?\\d+\\.\\d+).*?size=(?P<pixels>\\d+).*?wrp\\.(?P<ffilter>\\w+)\\.(?P<mjd>\\d+\\.\\d+).*?fits', re.S | re.I)
        ...
        if len(colorJpegUrl):
            reColorMeta = re.compile('(?P<color>\\w+)=(?P<datapath>/data.*?)&', re.S | re.I)
            thisIter = reColorMeta.finditer(colorJpegUrl[0])
            ffilter = ''
            for item in thisIter:
                fits = item.group('datapath').replace('%3A', ':').split('/')[(-1)]
                for j, f, n, b in zip(allStacks['jpegs'], allStacks['fits'], allStacks['filenames'], allStacks['filters']):
                    if fits in f:
                        ffilter += b
                        filename = n

            filename = 'color_' + ffilter + '_' + ('_').join(filename.split('_')[2:])
            colorImage['jpeg'].append(colorJpegUrl[0])
            colorImage['filename'].append(filename)
        self.log.debug('completed the ``parse_html_for_image_urls_and_metadata`` method')
        return (
         allStacks, allWarps, colorImage)

    def _download_images(self, urls=[], filenames=[], downloadDirectory=False):
        """
        *download images*
        ...
        **Return**
        ...
        localUrls = multiobject_download(urlList=urls, downloadDirectory=downloadDirectory, log=self.log, timeStamp=0, timeout=180, concurrentDownloads=10, resetFilename=filenames, credentials=False, longTime=False, indexFilenames=False)
