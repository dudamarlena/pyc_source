...
# [GCC 8.4.0]
# Embedded file name: \data\p\python\paps-settings\env\Lib\site-packages\paps_settings\settable_plugin.py
# Compiled at: 2016-04-01 13:39:14
...
__copyright__ = b'Copyright (C) 2016, Florian JUNG'
__license__ = b'MIT'
__version__ = b'0.1.0'
__date__ = b'2016-04-01'
import hashlib
...
class SettablePlugin(Plugin):
    ...
    def __init__(self, settings=None):
        ...
        self._resource_path = settings.get(b'resource_path')
        self._resource_file_types = settings.get(b'resource_file_types', [
         b'html', b'js', b'css'])
        self._resource_file_types = [ s.lower() for s in self._resource_file_types ]
        self._resources = {}
    ...
    def resource_get_list(self):
        ...
        with self._resource_lock:
            for key in self._resources:
                res.append((key, self._resources[key][b'hash']))
    ...
    def resource_update_list(self, reset=False):
        ...
        if not self._resource_path:
            raise PluginException(b'No resource path set')
        if not os.path.isdir(self._resource_path):
            raise PluginException((b"Resource path directory '{}' not found").format(self._resource_path))
        res = []
        with self._resource_lock:
            if reset:
                self._resources = {}
            ...
            for dirname, dirnames, filenames in os.walk(self._resource_path):
                for file_name in filenames:
                    ...
                    if file_ext not in self._resource_file_types:
                        self.debug((b"Skipping '{}'").format(file_name))
                        continue
                    file_path = os.path.join(dirname, file_name)
                    try:
                        file_hash = get_file_hash(file_path)
                    except:
                        self.exception((b"Failed to hash '{}'").format(file_path))
                        continue

                    self._resources[file_name] = {b'name': file_name, 
                       b'path': file_path, 
                       b'hash': file_hash, 
                       b'checked': datetime.datetime.utcnow()}

            for key in self._resources:
                resource = self._resources[key]
                if key not in old or old[key][b'hash'] != resource[b'hash']:
                    res.append((key, resource[b'hash']))
