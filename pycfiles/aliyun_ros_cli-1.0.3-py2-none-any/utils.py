...
# [GCC 8.4.0]
# Embedded file name: /root/Documents/ros-cli/ros/apps/utils.py
# Compiled at: 2017-09-26 22:37:59
from aliyunsdkcore.acs_exception.exceptions import ClientException
from aliyunsdkcore.acs_exception import error_code, error_msg
import ros.apps.config as connect, sys, json, urllib2
reload(sys)
...
def deal_resp(status, headers, body, print_response):
    ...
    try:
        data = json.loads(body) if body != '' else 'Response status: %s' % status
        if type(data) == dict:
            if 200 <= status < 300:
                print '[Succeed]'
                print_response(data)
