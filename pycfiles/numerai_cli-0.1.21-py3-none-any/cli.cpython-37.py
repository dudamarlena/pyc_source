...
# [GCC 8.4.0]
# Embedded file name: /Users/jparyani/workspace/numerai-compute-cli/numerai_compute/cli.py
# Compiled at: 2019-08-13 14:19:49
# Size of source mod 2**32: 30447 bytes
from pathlib import Path
import os
from os import path
import configparser, shutil, subprocess, base64, sys, platform
from datetime import datetime
import click, boto3, numerapi
...
def load_keys():
    ...
    return KeyConfig(aws_public=(config['default']['AWS_ACCESS_KEY_ID']), aws_secret=(config['default']['AWS_SECRET_ACCESS_KEY']),
    ...
    try:
        ...
        try:
            if 'NotSignedUp' in str(e):
                raise exception_with_msg('Your AWS keys are valid, but the account is not finished signing up. You either need to update your credit card in AWS at https://portal.aws.amazon.com/billing/signup?type=resubscribe#/resubscribed, or wait up to 24 hours for their verification process to complete.')
            raise exception_with_msg('AWS keys seem to be invalid. Make sure you\'ve entered them correctly and that your user has the "AdministratorAccess" policy.')
        finally:
            e = None
            del e
...
def setup_keys():
    ...
    config = configparser.ConfigParser()
    config['default'] = {'AWS_ACCESS_KEY_ID':aws_public, 
...
def read_docker_repo_file():
    ...
    if not path.exists(docker_file):
        raise exception_with_msg('docker repo not found. Make sure you have run `numerai setup` successfully before running this command.')
...
def read_submission_url_file():
    ...
    if not path.exists(f):
        raise exception_with_msg('submission url file not found. Make sure you have run `numerai setup` successfully before running this command.')
    with open(f, 'r') as (f):
        return f.read().strip()
...
def copy_terraform():
    ...
    shutil.copytree((path.join(get_code_dir(), 'terraform')), (get_project_numerai_dir()),
...
def copy_file(directory, filename, verbose, force):
    ...
    if os.path.exists(filename):
        overwrite = click.prompt(filename + ' already exists. Overwrite? [y]/n').strip()
        if overwrite != '':
            if overwrite != 'y':
                if overwrite != 'yes':
                    return
...
def terraform_setup(verbose, cpu, memory):
    if path.abspath(os.getcwd()) == path.abspath(str(Path.home())):
        raise exception_with_msg('`numerai setup` cannot be run from your $HOME directory. Please create another directory and run this again.')
    else:
        ...
        if cpu is not None or memory is not None:
            ...
            with open(path.join(get_code_dir(), 'terraform_variables_template.tf'), 'r') as (template):
                with open(path.join(get_project_numerai_dir(), 'variables.tf'), 'w') as (out):
                    out.write(template.read().format(fargate_cpu=cpu, fargate_memory=memory))
        numerai_dir = format_path_if_mingw(numerai_dir)
        keys = load_or_setup_keys()
        c = ('docker run --rm -it -v {numerai_dir}:/opt/plan -w /opt/plan hashicorp/terraform:light init'.format)(**locals())
        if verbose:
        ...
        if res.returncode != 0:
            if b'is not recognized as an internal or external command' in res.stderr:
                ...
                elif b'command not found' in res.stderr:
                    if sys.platform == 'darwin':
                        raise exception_with_msg('Docker does not appear to be installed. You can install it with `brew cask install docker` or from https://hub.docker.com/editions/community/docker-ce-desktop-mac')
                    else:
                        raise exception_with_msg('docker command not found. Please install docker and make sure that the `docker` command is in your $PATH')
                if b'This error may also indicate that the docker daemon is not running' in res.stderr or b'Is the docker daemon running' in res.stderr:
                    if sys.platform == 'darwin':
                        raise exception_with_msg('Docker daemon not running. Make sure you\'ve started "Docker Desktop" and then run this command again.')
            elif sys.platform == 'linux2':
                raise exception_with_msg('Docker daemon not running or this user cannot acccess the docker socket. Make sure docker is running and that your user has permissions to run docker. On most systems, you can add your user to the docker group like so: `sudo groupadd docker; sudo usermod -aG docker $USER` and then restarting your computer.')
            else:
                if sys.platform == 'win32':
                    if 'DOCKER_TOOLBOX_INSTALL_PATH' in os.environ:
                        raise exception_with_msg('Docker daemon not running. Make sure you\'ve started "Docker Quickstart Terminal" and then run this command again.')
                    else:
                        raise exception_with_msg('Docker daemon not running. Make sure you\'ve started "Docker Desktop" and then run this command again.')
                if b'invalid mode: /opt/plan' in res.stderr and sys.platform == 'win32':
                    raise exception_with_msg('It appears that you\'re running Docker Toolbox, but you\'re not using the "Docker Quickstart Terminal". Please re-run `numerai setup` from that terminal.')
            if b'Drive has not been shared' in res.stderr:
                raise exception_with_msg("It appears that you're running from a directory that isn't shared to your docker Daemon. Make sure your directory is shared through Docker Desktop: https://docs.docker.com/docker-for-windows/#shared-drives")
            print((res.stderr.decode('utf8')), file=(sys.stderr))
        res.check_returncode()
        click.echo('succesfully setup .numerai with terraform')
        c = ('docker run -e "AWS_ACCESS_KEY_ID={keys.aws_public}" -e "AWS_SECRET_ACCESS_KEY={keys.aws_secret}" --rm -it -v {numerai_dir}:/opt/plan -w /opt/plan hashicorp/terraform:light apply -auto-approve'.format)(**locals())
        if verbose:
            click.echo('running: ' + keys.sanitize_message(c))
        if sys.platform == 'win32':
            if 'DOCKER_TOOLBOX_INSTALL_PATH' in os.environ:
                click.echo('running aws setup through terraform. this can take a few minutes')
                res = subprocess.run(c, shell=True,
                  stdout=(subprocess.PIPE),
                  stderr=(subprocess.PIPE))
                if res.returncode != 0:
                    if b'No configuration files' in res.stdout:
                        raise exception_with_msg("It appears that you're running from a directory that isn't shared to your docker Daemon. Try running from a directory under your HOME, e.g. C:\\Users\\$YOUR_NAME\\$ANY_FOLDER")
        ...
        res = subprocess.run(c, shell=True)
    res.check_returncode()
    c = ('docker run --rm -it -v {numerai_dir}:/opt/plan -w /opt/plan hashicorp/terraform:light output docker_repo'.format)(**locals())
    if verbose:
        click.echo('running: ' + keys.sanitize_message(c))
    ...
    c = ('docker run --rm -it -v {numerai_dir}:/opt/plan -w /opt/plan hashicorp/terraform:light output submission_url'.format)(**locals())
    if verbose:
        click.echo('running: ' + keys.sanitize_message(c))
    res = subprocess.run(c, shell=True, stdout=(subprocess.PIPE))
...
def terraform_destroy(verbose):
    ...
    numerai_dir = format_path_if_mingw(numerai_dir)
    keys = load_or_setup_keys()
    c = ('docker run -e "AWS_ACCESS_KEY_ID={keys.aws_public}" -e "AWS_SECRET_ACCESS_KEY={keys.aws_secret}" --rm -it -v {numerai_dir}:/opt/plan -w /opt/plan hashicorp/terraform:light destroy -auto-approve'.format)(**locals())
    if verbose:
        click.echo('running: ' + keys.sanitize_message(c))
    res = subprocess.run(c, shell=True)
    res.check_returncode()
...
@click.option('--cpu',
  '-c',
  help='the cpu to use in the compute container (defaults to 1024). See https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task-cpu-memory-error.html for possible settings',
  type=int)
@click.option('--memory',
  '-m',
  help='the memory to use in the compute container (defaults to 8192). See https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task-cpu-memory-error.html for possible settings',
  type=int)
...
def docker_build(verbose):
    ...
    keys = load_keys()
    c = ('docker build -t {docker_repo} --build-arg NUMERAI_PUBLIC_ID={keys.numerai_public} --build-arg NUMERAI_SECRET_KEY={keys.numerai_secret} .'.format)(**locals())
    if verbose:
        click.echo('running: ' + keys.sanitize_message(c))
    res = subprocess.run(c, shell=True)
    res.check_returncode()
...
def train(command, verbose):
    ...
    c = ('docker run --rm -it -v {cur_dir}:/opt/app -w /opt/app {docker_repo} {command}'.format)(**locals())
    if verbose:
...
def run(verbose):
    ...
    docker_repo = read_docker_repo_file()
    cur_dir = format_path_if_mingw(os.getcwd())
    c = ('docker run --rm -it -v {cur_dir}:/opt/app -w /opt/app {docker_repo} '.format)(**locals())
    if verbose:
        click.echo('running: ' + c)
    res = subprocess.run(c, shell=True)
    res.check_returncode()
...
def docker_cleanup(verbose):
    ...
    resp = ecr_client.list_images(repositoryName='numerai-submission', filter={'tagStatus': 'UNTAGGED'})
    imageIds = resp['imageIds']
    if len(imageIds) == 0:
        return
    resp = ecr_client.batch_delete_image(repositoryName='numerai-submission',
      imageIds=imageIds)
    imageIds = resp['imageIds']
    if len(imageIds) > 0:
        click.echo(Fore.YELLOW + 'deleted ' + str(len(imageIds)) + ' old image(s) from remote docker repo')
...
def deploy(verbose):
    ...
    res = subprocess.run(('docker login -u {username} -p {password} {docker_repo}'.format)(**locals()),
      stdout=(subprocess.PIPE),
      stderr=(subprocess.PIPE),
      shell=True)
    res.check_returncode()
...
def test_webhook(quiet):
    ...
    if not quiet:
        ...
        click.echo(req.json())
        click.echo(Fore.YELLOW + 'You can now run `numerai compute status` or `numerai compute logs -f` to see your compute node running.')
...
def get_latest_task(keys, verbose):
    ecs_client = boto3.client('ecs', region_name='us-east-1',
      aws_access_key_id=(keys.aws_public),
      aws_secret_access_key=(keys.aws_secret))
    tasks = ecs_client.list_tasks(cluster='numerai-submission-ecs-cluster', desiredStatus='RUNNING')
    if len(tasks['taskArns']) == 0:
        tasks = ecs_client.list_tasks(cluster='numerai-submission-ecs-cluster', desiredStatus='STOPPED')
    if len(tasks['taskArns']) == 0:
        return
    tasks = ecs_client.describe_tasks(cluster='numerai-submission-ecs-cluster', tasks=[
     tasks['taskArns'][0]])
...
def status(verbose):
    ...
    if task is None:
        raise exception_with_msg('No tasks in the PENDING/RUNNING/STOPPED state found. This may mean that your task has been finished for a long time, and no longer exists. Check `numerai compute logs` and `numerai compute logs -l lambda` to see what happened.')
...
def logs(verbose, num_lines, log_type, follow_tail):
    ...
    def print_logs(events, limit):
        ...
        for event in events['events']:
            click.echo(str(datetime.fromtimestamp(event['timestamp'] / 1000)) + ':' + event['message'])

    def get_name_and_print_logs(logs_client, family, limit, nextToken=None, raise_on_error=True):
        streams = logs_client.describe_log_streams(logGroupName=family, orderBy='LastEventTime',
          descending=True)
        if len(streams['logStreams']) == 0:
            if not raise_on_error:
                return False
            raise exception_with_msg("No logs found. Make sure the webhook has triggered (check 'numerai compute logs -l lambda'). If it has, then check `numerai compute status` and make sure it's in the RUNNING state (this can take a few minutes). Also, make sure your webhook has triggered at least once by running 'curl `cat .numerai/submission_url.txt`'")
    ...
    if log_type == 'fargate':
        ...
        if log_type == 'lambda':
            ...
            return
        raise exception_with_msg("Unknown log type, expected 'fargate' or 'lambda': got " + log_type)
        ...
        if task is None:
            ...
            if task['desiredStatus'] == 'RUNNING':
                if task['lastStatus'] != 'RUNNING':
                    click.echo(Fore.RED + 'there is another task in the PENDING state. Check its status with `numerai compute status` and then rerun the logs command once it is RUNNING.')

    def task_status(task):
        ecs_client = boto3.client('ecs', region_name='us-east-1',
          aws_access_key_id=(keys.aws_public),
          aws_secret_access_key=(keys.aws_secret))
        resp = ecs_client.describe_tasks(cluster='numerai-submission-ecs-cluster',
          tasks=[task['taskArn']])
        tasks = resp['tasks']
        if len(tasks) == 0:
            return
    ...
    task = get_latest_task(keys, verbose)
    events = logs_client.get_log_events(logGroupName=family, logStreamName=name,
      limit=num_lines)
    click.echo('log for ' + family + ':' + name + ':')
    ...
    if follow_tail:
        while 1:
            ...
            if status != 'RUNNING':
                click.echo(Fore.YELLOW + 'Task is now in the ' + status + ' state')
                return
