...
# [GCC 8.4.0]
# Embedded file name: /home/tim/projects/wholebaked-site/venv/lib/python2.7/site-packages/richtext_blog/views.py
# Compiled at: 2012-04-15 07:51:04
...
class PostListView(list.ListView):
    ...
    def get_queryset(self):
        ...
        if 'month' in self.kwargs:
            objects = Post.objects.filter(created__year=self.kwargs['year'], created__month=self.kwargs['month'])
...
class TagView(list.ListView):
    ...
    def get_queryset(self):
        ...
        """
        return Post.objects.filter(tags__slug=self.kwargs['slug']).order_by('-created')
...
class PostView(edit.ProcessFormView, detail.DetailView, edit.FormMixin):
    ...
    def form_valid(self, form):
        ...
        Comment.objects.create(post=self.get_object(), author=name, auth_user=user, email=form_data['email'], comment=form_data['comment'])
