...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/archipelagentvmcasting/__init__.py
# Compiled at: 2013-03-20 13:50:16
import hypervisorrepomanager, vmappliancemanager
...
def make_archipel_plugin(configuration, entity, group):
    ...
    if group == 'archipel.plugin.virtualmachine':
        return [
         {'info': vmappliancemanager.TNVMApplianceManager.plugin_info(), 'plugin': vmappliancemanager.TNVMApplianceManager(configuration, entity, group)}]
    if group == 'archipel.plugin.hypervisor':
        return [
         {'info': hypervisorrepomanager.TNHypervisorRepoManager.plugin_info(), 'plugin': hypervisorrepomanager.TNHypervisorRepoManager(configuration, entity, group)}]
...
def version():
    ...
    return (
     __name__, pkg_resources.get_distribution('archipel-agent-vmcasting').version, [vmappliancemanager.TNVMApplianceManager.plugin_info(), hypervisorrepomanager.TNHypervisorRepoManager.plugin_info()])