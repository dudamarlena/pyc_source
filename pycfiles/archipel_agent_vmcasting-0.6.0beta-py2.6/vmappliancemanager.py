...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/archipelagentvmcasting/vmappliancemanager.py
# Compiled at: 2013-03-20 13:50:16
import os, sqlite3, xmpp
...
import archipel.archipelLibvirtEntity, appliancecompresser, appliancedecompresser
...
class TNVMApplianceManager(TNArchipelPlugin):
    ...
    def __init__(self, configuration, entity, entry_point_group):
        ...
        registrar_items = [
         {'commands': ['list appliances'], 'parameters': [], 'method': self.message_get, 
            'permissions': [
                          'appliance_get'], 
            'description': 'List all appliances'},
         {'commands': ['attach appliance'], 'parameters': [{'name': 'identifier', 'description': 'The identifer of the appliance, UUID or name'}], 'method': self.message_attach, 
            'permissions': [
                          'appliance_attach'], 
            'description': 'Attach to the given appliance'},
         {'commands': ['detach appliance'], 'parameters': [], 'method': self.message_detach, 
            'permissions': [
                          'appliance_detach'], 
    ...
    def process_iq(self, conn, iq):
        ...
        if self.entity.is_migrating and action not in 'get':
    ...
    def iq_package(self, iq):
        ...
        try:
            ...
            if package_should_gzip:
                package_should_gzip = package_should_gzip.lower() in ('true', '1',
                                                                      'y', 'yes')
            ...
            def perform_packaging(conn, resp):
                ...
                compressor = appliancecompresser.TNApplianceCompresser(package_name, paths, self.entity.definition, snapshots, working_dir, self.entity.folder, self.hypervisor_repo_path, self.finish_packaging, self.error_packaging, self.entity, package_should_gzip)
                self.is_installing = True
                compressor.start()
