...
# [GCC 8.4.0]
# Embedded file name: c:\work\python\retryz\build\lib\retryz\__init__.py
# Compiled at: 2015-12-21 04:58:32
import inspect, numbers, threading
...
def retry(on_error=None, on_errors=None, unless_error=None, unless_errors=None, error_call_back=None, on_return=None, on_returns=None, unless_return=None, unless_returns=None, return_call_back=None, limit=None, wait=None, timeout=None):
    ...
    def check_exclusive(list1, list2):
        if len(list1) and len(list2):
            raise ValueError('positive and negative criteria cannot be specified at the same time.')
    ...
    def get_wait(inst, retry_count):
        if wait is None or retry_count == 0:
            ...
            raise ValueError('wait should be a number or a callback of try count.')
        return ret
