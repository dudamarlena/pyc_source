...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/xoc_utils/kafka/KConsumer.py
# Compiled at: 2018-05-31 10:55:44
...
class KConsumer(metaclass=Singleton):
    ...
    def __init__(self, server_origin, group_id, service_name, poll_timeout=3.0, auto_commit_enable=False, auto_commit_interval=0, dead_letter_topic='xoc.dead-letter-queue', retries=3):
        ...
        self.k_consumer = Consumer({'bootstrap.servers':server_origin, 
         'group.id':group_id, 
        ...
        try:
            ...
            if self.retries == 0:
                ...
                if msg.topic().endswith('-retry'):
                    ...
                    new_msg_obj = {'message':msg_obj, 
                     'retry':{'service':self.service_name, 
                      'topic':msg.topic(), 
                      'errors':[
                       error]}}
    ...
    def _consume_message(self, msg):
        ...
        try:
            if msg.error():
                ...
                print('Received message: {}, partition: {}, offset: {}'.format(msg.value().decode('utf-8'), msg.partition(), msg.offset()))
    ...
    def consume(self):
        ...
        """
        self.k_consumer.subscribe((list(self.handlers.keys())),
          on_assign=(self._log_assignment))
        try:
            while True:
                if self.stopped:
                    break
