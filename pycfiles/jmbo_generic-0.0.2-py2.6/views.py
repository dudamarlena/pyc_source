...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/generic/views.py
# Compiled at: 2011-09-27 09:16:58
...
class CategoryURL(object):
    ...
    def __call__(self, obj=None):
        if self.category and obj:
            return reverse('category_object_detail', kwargs={'category_slug': self.category.slug, 'slug': obj.slug})
        else:
            if obj:
...
class CategoryObjectDetailView(DetailView):
    ...
    def get_template_names(self):
        return [
         'category/%s_detail.html' % self.category.slug, 'category/detail.html'] + super(CategoryObjectDetailView, self).get_template_names()
...
class CategoryObjectListView(ListView):
    ...
    def get_template_names(self):
        return [
         'category/%s_list.html' % self.category.slug, 'category/list.html'] + super(CategoryObjectListView, self).get_template_names()
