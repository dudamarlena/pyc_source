...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/generic/models.py
# Compiled at: 2011-09-27 09:30:35
import inspect
...
class Link(models.Model):
    ...
    view_name = models.CharField(max_length=256, help_text='View name to which this link will redirect. This takes precedence over Category and URL fields below.', blank=True, null=True)
    category = models.ForeignKey('category.Category', help_text='Category to which this link will redirect. This takes precedence over URL field below.', blank=True, null=True)
    url = models.CharField(max_length=256, help_text='URL to which this menu link will redirect.', blank=True, null=True)
...
class RegistrationPreferences(Preferences):
    __module__ = 'preferences.models'
    raw_display_fields = models.CharField('Display fields', max_length=32, default='', help_text=_('Fields to display on the registration form.'))
    raw_required_fields = models.CharField('Required fields', max_length=32, default='', blank=True, help_text=_('Set fields which are not required by default as required on the registration form.'))
    raw_unique_fields = models.CharField('Unique fields', max_length=32, default='', blank=True, help_text=_('Set fields which must be unique on the registration form.'))
...
class LoginPreferences(Preferences):
    ...
    raw_login_fields = models.CharField('Login fields', max_length=32, default='username', choices=(
     ...
     (
      'username,email', _('Username or email address'))), help_text=_('Users may log in with more than one identifier.'))
...
class ElementOption(models.Model):
    ...
    title = models.CharField(max_length=256, help_text='A short descriptive title.')
    content = models.ManyToManyField('jmbo.ModelBase', help_text='Content to display, takes preference over Category field.', blank=True, null=True)
    category = models.ForeignKey('category.Category', help_text='Category for which to collect objects.', blank=True, null=True)
    count = models.IntegerField(help_text='Number of content objects to display.')
    style = models.CharField(choices=((style[0], style[0]) for style in inspect.getmembers(element_styles, inspect.isclass)), max_length=64)
    position = models.IntegerField()