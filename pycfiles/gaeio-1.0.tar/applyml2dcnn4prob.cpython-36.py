...
class applyml2dcnn4prob(object):
    ...
    def clickBtnApplyMl2DCnn4Prob(self):
        self.refreshMsgBox()
        if self.checkSurvInfo() is False:
            vis_msg.print('ERROR in ApplyMl2DCnn4Prob: No seismic survey available', type='error')
        ...
        if ml_tfm.checkCNNModel(self.modelpath, self.modelname) is False:
            vis_msg.print('ERROR in ApplyMl2DCnn4Prob: No CNN network found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 2D-CNN', 'No CNN network found')
            return
        _featurelist = self.modelinfo['feature_list']
        for f in _featurelist:
            if self.checkSeisData(f) is False:
                vis_msg.print(("ERROR in ApplyMl2DCnn4Prob: Feature '%s' not found in seismic data" % f), type='error')
        ...
        if _image_height_old is False or _image_width_old is False or _image_height_new is False or _image_width_new is False:
            vis_msg.print('ERROR in ApplyMl2DCnn4Prob: Non-integer feature size', type='error')
        ...
        if _image_height_old < 2 or _image_width_old < 2 or _image_height_new < 2 or _image_width_new < 2:
            vis_msg.print('ERROR in ApplyMl2DCnn4Prob: Features are not 2D', type='error')
        ...
        if _batch is False or _batch < 1:
            vis_msg.print('ERROR in ApplyMl2DCnn4Prob: Non-positive batch size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 2D-CNN', 'Non-positive batch size')
            return
        if len(self.ldtsave.text()) < 1:
            vis_msg.print('ERROR in ApplyMl2DCnn4Prob: No prefix specified', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 2D-CNN', 'No prefix specified')
            return
        if len(self.lwgtarget.selectedItems()) < 1:
            vis_msg.print('ERROR in ApplyMl2DCnn4Prob: No target label specified', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 2D-CNN', 'No target label specified')
            return
        ...
        for i in range(_nloop):
            ...
            _result[idxlist, :] = ml_cnn.probabilityFromCNNClassifier(_dict, cnnpath=(self.modelpath),
