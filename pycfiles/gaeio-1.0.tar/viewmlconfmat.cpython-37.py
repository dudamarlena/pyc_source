...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\cognitivegeo\src\gui\viewmlconfmat.py
...
class viewmlconfmat(object):
    ...
    def retranslateGUI(self, ViewMlConfMat):
        ...
        if np.ndim(self.confmat) == 2:
            if np.shape(self.confmat)[0] > 1:
                if np.shape(self.confmat)[1] > 1:
