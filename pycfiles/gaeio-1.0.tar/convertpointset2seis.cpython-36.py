...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\gaeio\src\gui\convertpointset2seis.py
# Compiled at: 2020-04-25 14:58:02
# Size of source mod 2**32: 13082 bytes
...
class convertpointset2seis(object):
    ...
    def clickBtnApply(self):
        ...
        if len(_pointlist) < 1:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Convert PointSet to Seismic', 'No pointset selected for conversion')
            return
        ...
        if len(_attriblist) < 1:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Convert PointSet to Seismic', 'No property selected for conversion')
            return
        if basic_data.str2float(self.ldtvalue.text()) is False:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Convert PointSet to Seismic', 'Non-float filling-up value')
            return
        if checkSurvInfo(self.survinfo) is False:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Convert PointSet to Seismic', 'No seismic survey found for conversion')
            return
        for _f in _attriblist:
            if _f in self.seisdata.keys() and checkSeisData(self.seisdata[_f], self.survinfo):
                reply = QtWidgets.QMessageBox.question(self.msgbox, 'Convert PointSet to Seismic', _f + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                if reply == QtWidgets.QMessageBox.No:
                    return

        _survinfo = self.survinfo
        for _f in _attriblist:
            _seisdata = np.zeros([_survinfo['ZNum'], _survinfo['XLNum'], _survinfo['ILNum']]) + basic_data.str2float(self.ldtvalue.text())
            for _p in _pointlist:
                _pointsetdata = basic_mdt.exportMatDict(self.pointsetdata[_p], ['Inline', 'Crossline', 'Z', _f])
                _pointsetdata = seis_ays.removeOutofSurveySample(_pointsetdata, inlstart=(_survinfo['ILStart']),
                  inlend=(_survinfo['ILEnd']),
                  xlstart=(_survinfo['XLStart']),
                  xlend=(_survinfo['XLEnd']),
                  zstart=(_survinfo['ZStart']),
                  zend=(_survinfo['ZEnd']))
                if np.shape(_pointsetdata)[0] < 1:
                    pass
                else:
                    _ijk = seis_ays.convertIXZToIJK(_pointsetdata, inlstart=(_survinfo['ILStart']),
                      inlstep=(_survinfo['ILStep']),
                      xlstart=(_survinfo['XLStart']),
                      xlstep=(_survinfo['XLStep']),
                      zstart=(_survinfo['ZStart']),
                      zstep=(_survinfo['ZStep']))
                    for _i in range(np.shape(_ijk)[0]):
                        ...
                        if float(_data) == basic_data.str2float(self.ldtvalue.text()) or np.isnan(_data) and np.isnan(basic_data.str2float(self.ldtvalue.text())):
        ...
        QtWidgets.QMessageBox.information(self.msgbox, 'Convert PointSet to Seismic', 'PointSet converted successfully')
