...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\cognitivegeo\src\gui\exportpointsetnpy.py
# Compiled at: 2020-03-24 10:20:01
...
class exportpointsetnpy(object):
    ...
    def clickBtnSave(self):
        ...
        _file = _dialog.getSaveFileName(None, 'Select PointSet NumPy', (self.rootpath), filter='PointSet NumPy files (*.pts.npy);; All files (*.*)')
        if len(_file[0]) > 0:
    ...
    def clickBtnExportPointSetNpy(self):
        ...
        if len(_pointlist) < 1:
            vis_msg.print('ERROR in ExportPointSetNpy; No pointset selected for export', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Export PointSet NumPy', 'No pointset selected for export')
            return
        if len(self.ldtsave.text()) < 1:
            vis_msg.print('ERROR in ExportPointSetNpy: No name specified for NumPy', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Export PointSet NumPy', 'No name specified for NumPy')
            return
        ...
        if len(_pointlist) > 1:
            if self.cbbtype.currentIndex() >= 1:
                reply = QtWidgets.QMessageBox.question(self.msgbox, 'Export PointSet Numpy', 'Warning: For exporting >=2 pointset, property name used as file name. Continue?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                if reply == QtWidgets.QMessageBox.No:
                    return
        if self.cbbtype.currentIndex() == 0:
            _npydata = {}
            for i in range(len(_pointlist)):
                _npydata[_pointlist[i].text()] = self.pointsetdata[_pointlist[i].text()]

            np.save(os.path.join(_savepath, _savename), _npydata)
        if self.cbbtype.currentIndex() == 1:
            for i in range(len(_pointlist)):
                _data = self.pointsetdata[_pointlist[i].text()]
                if len(_pointlist) > 1:
                    _savename = _pointlist[i].text() + '.pts.npy'
                np.save(os.path.join(_savepath, _savename), _data)

        if self.cbbtype.currentIndex() == 2:
            reply = QtWidgets.QMessageBox.question(self.msgbox, 'Export PointSet Numpy', 'Warning: property header not saved in numpy matrix. Continue?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
            if reply == QtWidgets.QMessageBox.No:
                return
            for i in range(len(_pointlist)):
                _data = basic_mdt.exportMatDict(self.pointsetdata[_pointlist[i].text()], [
                 'Inline', 'Crossline', 'Z'])
                for j in self.pointsetdata[_pointlist[i].text()].keys():
                    if j != 'Inline' and j != 'Crossline' and j != 'Z':
                        _data = np.concatenate((_data, self.pointsetdata[_pointlist[i].text()][j]), axis=1)

                if len(_pointlist) > 1:
                    _savename = _pointlist[i].text() + '.pts.npy'
                np.save(os.path.join(_savepath, _savename), _data)

        QtWidgets.QMessageBox.information(self.msgbox, 'Export PointSet NumPy', str(len(_pointlist)) + ' pointsets exported as NumPy successfully')
