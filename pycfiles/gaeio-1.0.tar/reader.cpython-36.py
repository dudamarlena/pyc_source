...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\gaeio\src\segpy\reader.py
# Compiled at: 2020-04-25 14:33:32
# Size of source mod 2**32: 25835 bytes
from __future__ import print_function
import os, sys
sys.path.append(os.path.dirname(__file__)[:-6][:-4][:-6])
from gaeio.src.segpy.encoding import ASCII
from gaeio.src.segpy.packer import HeaderPacker
from gaeio.src.segpy.trace_header import TraceHeaderRev1
from gaeio.src.segpy.util import file_length, filename_from_handle
from gaeio.src.segpy.datatypes import DATA_SAMPLE_FORMAT_TO_SEG_Y_TYPE, SEG_Y_TYPE_DESCRIPTION, SEG_Y_TYPE_TO_CTYPE, size_in_bytes
from gaeio.src.segpy.toolkit import extract_revision, bytes_per_sample, read_binary_reel_header, read_trace_header, catalog_traces, read_binary_values, REEL_HEADER_NUM_BYTES, TRACE_HEADER_NUM_BYTES, read_textual_reel_header, read_extended_textual_headers, guess_textual_header_encoding
...
def create_reader(fh, encoding=None, trace_header_format=TraceHeaderRev1, endian='>', progress=None):
    ...
    if line_catalog is not None:
        return SegYReader3D(fh, textual_reel_header, binary_reel_header, extended_textual_header, trace_offset_catalog, trace_length_catalog, line_catalog, trace_header_format, encoding, endian)
    else:
        if cdp_catalog is not None:
            return SegYReader2D(fh, textual_reel_header, binary_reel_header, extended_textual_header, trace_offset_catalog, trace_length_catalog, cdp_catalog, trace_header_format, encoding, endian)
        return SegYReader(fh, textual_reel_header, binary_reel_header, extended_textual_header, trace_offset_catalog, trace_length_catalog, trace_header_format, encoding, endian)


class SegYReader(object):
    __doc__ = 'A basic SEG Y reader.\n\n    Use to obtain read the reel header, the trace_samples headers or trace_samples\n    values. Traces can be accessed only by trace_samples index.\n    '

    def __init__(self, fh, textual_reel_header, binary_reel_header, extended_textual_headers, trace_offset_catalog, trace_length_catalog, trace_header_format, encoding, endian='>'):
        ...
        self._bytes_per_sample = bytes_per_sample(self._binary_reel_header, self.revision)
    ...
    def trace_samples(self, trace_index, start=None, stop=None):
        ...
        if not 0 <= trace_index < self.num_traces():
            ...
            if not 0 <= stop_sample <= num_samples_in_trace:
                raise ValueError('trace_samples(): stop value {} out of range 0 to {}'.format(stop, num_samples_in_trace))
            raise 0 <= start_sample <= stop_sample or ValueError('trace_samples(): start value {} out of range 0 to {}'.format(start, stop_sample))
        dsf = self._binary_reel_header.data_sample_format
        seg_y_type = DATA_SAMPLE_FORMAT_TO_SEG_Y_TYPE[dsf]
        start_pos = self._trace_offset_catalog[trace_index] + TRACE_HEADER_NUM_BYTES + start_sample * size_in_bytes(SEG_Y_TYPE_TO_CTYPE[seg_y_type])
        num_samples_to_read = stop_sample - start_sample
        trace_values = read_binary_values(self._fh, start_pos, seg_y_type, num_samples_to_read, self._endian)
        return trace_values
...
class SegYReader3D(SegYReader):
    __doc__ = 'A reader for 3D seismic data.\n\n    In addition to the capabilities provided by the SegYReader base\n    class, this reader provides an index to facilitate random access\n    to individual traces via crossline and inline co-ordinates.\n    '

    def __init__(self, fh, textual_reel_header, binary_reel_header, extended_textual_headers, trace_offset_catalog, trace_length_catalog, line_catalog, trace_header_format, encoding, endian='>'):
        ...
        Args:
            ...
            endian: '>' for big-endian data (the standard and default), '<' for
                little-endian (non-standard)
        """
        super(SegYReader3D, self).__init__(fh, textual_reel_header, binary_reel_header, extended_textual_headers, trace_offset_catalog, trace_length_catalog, trace_header_format, encoding, endian)
...
class SegYReader2D(SegYReader):
    ...
    def __init__(self, fh, textual_reel_header, binary_reel_header, extended_textual_headers, trace_offset_catalog, trace_length_catalog, cdp_catalog, trace_header_format, encoding, endian='>'):
        ...
        Args:
            ...
            endian: '>' for big-endian data (the standard and default), '<' for
                little-endian (non-standard)
        """
        super(SegYReader2D, self).__init__(fh, textual_reel_header, binary_reel_header, extended_textual_headers, trace_offset_catalog, trace_length_catalog, trace_header_format, encoding, endian)
