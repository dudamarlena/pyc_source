...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\cognitivegeo\src\gui\plotml2dcnnconvmask.py
...
class plotml2dcnnconvmask(gui_core):
    modelpath = ''
    modelname = ''
    ...
    def setupGUI(self, PlotMl2DCnnConvMask):
        gui_core.title = 'Plot 2D-CNN Conv. Mask'
        gui_core.icon = 'mask.png'
        gui_core.imagestyle = self.maskstyle
        gui_core.imagelist = self.getConvMaskList()
        gui_core.setupGUI(self, PlotMl2DCnnConvMask)

    def clickBtnApply(self):
        self.refreshMsgBox()
        if ml_tfm.checkCNNModel(self.modelpath, self.modelname) is False:
            vis_msg.print('ERROR in PlotMl2DCnnConvMask: No CNN network found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Plot 2D-CNN Conv. Mask', 'No CNN network found')
            return
...
if __name__ == '__main__':
    ...
    PlotMl2DCnnConvMask = QtWidgets.QWidget()
    gui = plotml2dcnnconvmask()
    gui.setupGUI(PlotMl2DCnnConvMask)
    PlotMl2DCnnConvMask.show()
    sys.exit(app.exec_())