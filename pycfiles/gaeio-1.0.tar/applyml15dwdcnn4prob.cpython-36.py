...
class applyml15dwdcnn4prob(object):
    ...
    def clickBtnApplyMl15DWdcnn4Prob(self):
        self.refreshMsgBox()
        if self.checkSurvInfo() is False:
            vis_msg.print('ERROR in ApplyMl15DWdcnn4Prob: No seismic survey available', type='error')
        ...
        if ml_tfm.check15DWDCNNModel(self.modelpath, self.modelname) is False:
            vis_msg.print('ERROR in ApplyMl15DWdcnn4Prob: No pre-WDCNN network found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 1.5D-WDCNN', 'No pre-WDCNN network found')
            return
        for f in self.modelinfo['feature_list']:
            if self.checkSeisData(f) is False:
                vis_msg.print(("ERROR in ApplyMl15DWdcnn4Prob: Feature '%s' not found in seismic data" % f), type='error')
        ...
        if _image_height is False or _image_width is False or _image_height_new is False or _image_width_new is False:
            vis_msg.print('ERROR in ApplyMl15DWdcnn4Prob: Non-integer feature size', type='error')
        ...
        if _image_height < 2 or _image_width < 2 or _image_height_new < 2 or _image_width_new < 2:
            vis_msg.print('ERROR in ApplyMl15DWdcnn4Prob: Features are not 2D', type='error')
        ...
        if _batch is False or _batch < 1:
            vis_msg.print('ERROR in ApplyMl15DWdcnn4Prob: Non-positive batch size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 1.5D-WDCNN', 'Non-positive batch size')
            return
        if len(self.ldtsave.text()) < 1:
            vis_msg.print('ERROR in ApplyMl15DWdcnn4Prob: No prefix specified', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 1.5D-WDCNN', 'No prefix specified')
            return
        if len(self.lwgtarget.selectedItems()) < 1:
            vis_msg.print('ERROR in ApplyMl15DWdcnn4Prob: No target class specified', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 1.5D-WDCNN', 'No target class specified')
            return
        ...
        _result = np.zeros([_nsample, len(_classlist), _image_height])
        idxstart = 0
        for i in range(_nloop):
            ...
            _rst = ml_wdcnn15d.probabilityFrom15DWDCNNSegmentor(_dict, imageheight=_image_height_new,
            ...
            if _image_height_new != _image_height:
                _rst = np.reshape(_rst, [np.shape(_rst)[0] * len(_classlist), _image_height_new])
                _rst = basic_curve.changeCurveSize(_rst, length=_image_height_new,
                  length_new=_image_height,
                  kind='linear')
            _result[idxlist, :, :] = np.reshape(_rst, [-1, len(_classlist), _image_height])
            _pgsdlg.setValue(i + 1)

        print('Done')
        for i in range(len(_classlist)):
            if self.cbbornt.currentIndex() == 0 or self.cbbornt.currentIndex() == 1:
                _rst = np.reshape(_result[:, i, :], [_seisinfo['ILNum'], _seisinfo['XLNum'], _seisinfo['ZNum']])
                _rst = np.transpose(_rst, [2, 1, 0])
            if self.cbbornt.currentIndex() == 2:
                _rst = np.reshape(_result[:, i, :], [_seisinfo['ILNum'], _seisinfo['ZNum'], _seisinfo['XLNum']])
