...
from cognitivegeo.src.gui.viewml15dwdcnn import viewml15dwdcnn as gui_viewml15dwdcnn
...
class extractml15dwdcnn(object):
    ...
    def clickBtnExtractMl15DWdcnn(self):
        self.refreshMsgBox()
        if self.checkSurvInfo() is False:
            vis_msg.print('ERROR in ExtractMl15DWdcnn: No seismic survey available', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Extract 1.5D-WDCNN', 'No seismic survey available')
            return
        if ml_tfm.check15DWDCNNModel(self.modelpath, self.modelname) is False:
            vis_msg.print('ERROR in ExtractMl15DWdcnn: No pre-WDCNN network found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Extract 1.5D-WDCNN', 'No pre-WDCNN network found')
            return
        for f in self.modelinfo['feature_list']:
            if self.checkSeisData(f) is False:
                vis_msg.print(("ERROR in ExtractMl15DWdcnn: Feature '%s' not found in seismic data" % f), type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Extract 1.5D-WDCNN', "Feature '" + f + "' not found in seismic data")
                return
        ...
        if _image_height is False or _image_width is False or _image_height_new is False or _image_width_new is False:
            vis_msg.print('ERROR in ExtractMl15DWdcnn: Non-integer feature size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Extract 1.5D-WDCNN', 'Non-integer feature size')
            return
        if _image_height < 2 or _image_width < 2 or _image_height_new < 2 or _image_width_new < 2:
            vis_msg.print('ERROR in ExtractMl15DWdcnn: Features are not 2D', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Extract 1.5D-WDCNN', 'Features are not 2D')
            return
        _batch = basic_data.str2int(self.ldtbatchsize.text())
        if _batch is False or _batch < 1:
            vis_msg.print('ERROR in ExtractMl15DWdcnn: Non-positive batch size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Extract 1.5D-WDCNN', 'Non-positive batch size')
            return
        if len(self.ldtsave.text()) < 1:
            vis_msg.print('ERROR in ExtractMl15DWdcnn: No prefix specified', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Extract 1.5D-WDCNN', 'No prefix specified')
            return
        if self.ldtsave.text() + '1' in self.seisdata.keys():
            if self.checkSeisData(self.ldtsave.text() + '1'):
                reply = QtWidgets.QMessageBox.question(self.msgbox, 'Extract 1.5D-WDCNN', 'Prefix ' + self.ldtsave.text() + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                if reply == QtWidgets.QMessageBox.No:
                    return
        print('ExtractMl15DWdcnn: Step 1 - Reformat images')
        _seisdict = {}
        for f in _features:
            _seisdict[f] = self.reformatImage(f)

        print('ExtractMl15DWdcnn: Step 2 - Interpolate images if necessary: (%d, %d) --> (%d, %d)' % (
        ...
        if _image_height_new != _image_height or _image_width_new != _image_width:
            for f in _features:
                _seisdict[f] = basic_image.changeImageSize((_seisdict[f]), image_height=_image_height,
                  image_width=_image_width,
                  image_height_new=_image_height_new,
                  image_width_new=_image_width_new)

        print('ExtractMl15DWdcnn: Step 3 - Extract')
        _nsample = np.shape(_seisdict[_features[0]])[0]
        ...
        _pgsdlg.setWindowTitle('Extract 1.5D-WDCNN')
        ...
        for i in range(_nloop):
            ...
            sys.stdout.write('\r>>> Extract 1.5D-WDCNN, proceeding %.1f%% ' % (float(i) / float(_nloop) * 100.0))
            sys.stdout.flush()
            ...
            _rst = ml_wdcnn15d.extract15DWDCNNConvFeature(_dict, imageheight=_image_height_new,
        ...
        QtWidgets.QMessageBox.information(self.msgbox, 'Extract 1.5D-WDCNN', str(_nfeature) + ' WDCNN features extracted successfully')
