...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\cognitivegeo\src\gui\exportpsseisnpy.py
# Compiled at: 2020-03-24 11:16:34
# Size of source mod 2**32: 9884 bytes
...
class exportpsseisnpy(object):
    ...
    def retranslateGUI(self, ExportPsSeisNpy):
        ...
        self.cbbtype.addItems(['Dictionary (all in one)', 'Dictionary (individual)', '3-D numpy matrix'])
    ...
    def clickBtnSave(self):
        ...
        _file = _dialog.getSaveFileName(None, 'Select Pre-stack Seismic NumPy', (self.rootpath), filter='Pre-stack Seismic NumPy files (*.psseis.npy);; All files (*.*)')
        if len(_file[0]) > 0:
    ...
    def clickBtnExportPsSeisNpy(self):
        ...
        if len(_psseislist) < 1:
            vis_msg.print('ERROR in ExportPsSeisNpy; No pre-stack seismic selected for export', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Export Pre-stack Seismic NumPy', 'No pre-stack seismic selected for export')
            return
        if len(self.ldtsave.text()) < 1:
            vis_msg.print('ERROR in ExportPsSeisNpy: No name specified for NumPy', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Export Pre-stack Seismic NumPy', 'No name specified for NumPy')
            return
        ...
        if len(_psseislist) > 1:
            if self.cbbtype.currentIndex() >= 1:
                reply = QtWidgets.QMessageBox.question(self.msgbox, 'Export Pre-stack Seismic Numpy', 'Warning: For exporting >=2 pre-stack, property name used as file name. Continue?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
        ...
        if self.cbbtype.currentIndex() == 1:
            for i in range(len(_psseislist)):
                ...
                if len(_psseislist) > 1:
                    _savename = _psseislist[i].text()
                for j in _data.keys():
                    if 'ShotData' in _data[j].keys():
                        np.save(os.path.join(_savepath, _savename + '_shot_' + j + '.psseis.npy'), _data[j])

        if self.cbbtype.currentIndex() == 2:
            reply = QtWidgets.QMessageBox.question(self.msgbox, 'Export Pre-stack Seismic Numpy', 'Warning: Shot information not saved in numpy matrix. Continue?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
            if reply == QtWidgets.QMessageBox.No:
                return
        ...
        QtWidgets.QMessageBox.information(self.msgbox, 'Export Pre-stack Seismic NumPy', str(len(_psseislist)) + ' pre-stack seismic exported as NumPy successfully')
