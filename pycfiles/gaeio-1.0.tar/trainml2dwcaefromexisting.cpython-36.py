...
class trainml2dwcaefromexisting(object):
    ...
    def clickBtnTrainMl2DWcaeFromExisting(self):
        self.refreshMsgBox()
        if len(self.lwgfeature.selectedItems()) < 1:
            vis_msg.print('ERROR in TrainMl2DWcaeFromExisting: No feature selected for training', tyep='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 2D-WCAE', 'No feature selected for training')
            return
        ...
        if _image_height is False or _image_width is False or _image_height_new is False or _image_width_new is False:
            vis_msg.print('ERROR in TrainMl2DWcaeFromExisting: Non-integer feature size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 2D-WCAE', 'Non-integer feature size')
            return
        if _image_height < 2 or _image_width < 2 or _image_height_new < 2 or _image_width_new < 2:
            vis_msg.print('ERROR in TrainMl2DWcaeFromExisting: Features are not 2D', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 2D-WCAE', 'Features are not 2D')
            return
        ...
        if len(self.ldtexisting.text()) < 1:
            vis_msg.print('ERROR in TrainMl2DWcaeFromExisting: No name specified for pre-trained network', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 2D-WCAE', 'No name specified for pre-trained network')
            return
        ...
        if ml_tfm.getConvModelNChannel(_precnnpath, _precnnname) != len(_features):
            vis_msg.print('ERROR in TrainMl2DWcaeFromExisting: Feature channel number not match with pre-trained network', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 2D-WCAE', 'Feature channel number not match with pre-trained network')
            return
        ...
        if _nconvblock is False or _nconvblock <= 0:
            vis_msg.print('ERROR in TrainMl2DWcaeFromExisting: Non-positive convolutional block number', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 2D-WCAE', 'Non-positive convolutional block number')
            return
        for _i in _nconvlayer:
            if _i is False or _i < 1:
                vis_msg.print('ERROR in TrainMl2DWcaeFromExisting: Non-positive convolutional layer number', type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Train 2D-WCAE', 'Non-positive convolutional layer number')
                return

        for _i in _nconvfeature:
            if _i is False or _i < 1:
                vis_msg.print('ERROR in TrainMl2DWcaeFromExisting: Non-positive convolutional feature number', type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Train 2D-WCAE', 'Non-positive convolutional feature number')
                return

        if _n1x1layer is False or _n1x1layer <= 0:
            vis_msg.print('ERROR in TrainMl2DWcaeFromExisting: Non-positive 1x1 convolutional layer number', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 2D-WCAE', 'Non-positive 1x1 convolutional layer number')
            return
        for _i in _n1x1feature:
            if _i is False or _i < 1:
                vis_msg.print('ERROR in TrainMl2DWcaeFromExisting: Non-positive 1x1 convolutional feature number', type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Train 2D-WCAE', 'Non-positive 1x1 convolutional feature number')
                return

        if _patch_height is False or _patch_width is False or _patch_height < 1 or _patch_width < 1:
            vis_msg.print('ERROR in TrainMl2DWcaeFromExisting: Non-positive convolutional patch size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 2D-WCAE', 'Non-positive convolutional patch size')
            return
        if _pool_height is False or _pool_width is False or _pool_height < 1 or _pool_width < 1:
            vis_msg.print('ERROR in TrainMl2DWcaeFromExisting: Non-positive pooling size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 2D-WCAE', 'Non-positive pooling size')
            return
        if _nepoch is False or _nepoch <= 0:
            vis_msg.print('ERROR in TrainMl2DWcaeFromExisting: Non-positive epoch number', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 2D-WCAE', 'Non-positive epoch number')
            return
        if _batchsize is False or _batchsize <= 0:
            vis_msg.print('ERROR in TrainMl2DWcaeFromExisting: Non-positive batch size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 2D-WCAE', 'Non-positive batch size')
            return
        if _learning_rate is False or _learning_rate <= 0:
            vis_msg.print('ERROR in TrainMl2DWcaeFromExisting: Non-positive learning rate', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 2D-WCAE', 'Non-positive learning rate')
            return
        if _dropout_prob is False or _dropout_prob <= 0:
            vis_msg.print('ERROR in TrainMl2DWcaeFromExisting: Negative dropout rate', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 2D-WCAE', 'Negative dropout rate')
            return
        if len(self.ldtsave.text()) < 1:
            vis_msg.print('ERROR in TrainMl2DWcaeFromExisting: No name specified for CAE network', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 2D-WCAE', 'No name specified for CAE network')
            return
        ...
        print('TrainMl2DWcaeFromExisting: Step 1 - Step 1 - Get training samples:')
        ...
        if np.shape(_traindata)[0] <= 0:
            vis_msg.print('ERROR in TrainMl2DWcaeFromExisting: No training sample found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 2D-WCAE', 'No training sample found')
            return
        print('TrainMl2DWcaeFromExisting: Step 2 - Retrieve and interpolate images if necessary: (%d, %d) --> (%d, %d)' % (
        ...
        if self.traindataconfig['RemoveInvariantFeature_Checked']:
            for f in _features:
                ...
                if basic_mdt.maxDictConstantRow(_traindict) <= 0:
                    vis_msg.print('ERROR in TrainMl2DWcaeFromExisting: No training sample found', type='error')
                    QtWidgets.QMessageBox.critical(self.msgbox, 'Train 2D-CAE', 'No training sample found')
                    return
        ...
        print('TrainMl2DWcaeFromExisting: A total of %d valid training samples' % basic_mdt.maxDictConstantRow(_traindict))
        print('TrainMl2DWcaeFromExisting: Step 3 - Start training')
        _pgsdlg = QtWidgets.QProgressDialog()
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(os.path.join(self.iconpath, 'icons/new.png')), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        _pgsdlg.setWindowIcon(icon)
        _pgsdlg.setWindowTitle('Train 2D-CAE')
        _pgsdlg.setCancelButton(None)
        _pgsdlg.setWindowFlags(QtCore.Qt.WindowStaysOnTopHint)
        _pgsdlg.forceShow()
        _pgsdlg.setFixedWidth(400)
        _caelog = ml_wcae.createWCAEReconstructorFromExisting(_traindict, imageheight=_image_height_new,
        ...
        QtWidgets.QMessageBox.information(self.msgbox, 'Train 2D-WCAE', 'WCAE trained successfully')
        reply = QtWidgets.QMessageBox.question(self.msgbox, 'Train 2D-WCAE', 'View learning matrix?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.Yes)
