...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\gaeio\src\gui\convertpointset2horizon.py
...
class convertpointset2horizon(object):
    ...
    def clickBtnApply(self):
        ...
        if len(_pointlist) < 1:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Convert PointSet to Horizon', 'No pointset selected for conversion')
            return
        ...
        if basic_data.str2float(self.ldtvalue.text()) is False:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Convert PointSet to Horizon', 'Non-float filling-up value')
            return
        for _f in _pointlist:
            if _f in self.horizondata.keys() and checkHorizonData(self.horizondata[_f]):
                reply = QtWidgets.QMessageBox.question(self.msgbox, 'Convert PointSet to Horizon', _f + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                if reply == QtWidgets.QMessageBox.No:
                    return

        for _pts in _pointlist:
            self.horizondata[_pts] = point_ays.convertPointSet2Horizon((self.pointsetdata[_pts]), property_list=_attriblist, filling_up_value=(basic_data.str2float(self.ldtvalue.text())))

        QtWidgets.QMessageBox.information(self.msgbox, 'Convert PointSet to Horizon', 'PointSet converted successfully')
