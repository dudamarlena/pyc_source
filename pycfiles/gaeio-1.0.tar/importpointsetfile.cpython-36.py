...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\gaeio\src\gui\importpointsetfile.py
# Compiled at: 2020-04-25 15:07:54
# Size of source mod 2**32: 15382 bytes
...
class importpointsetfile(object):
    ...
    def clickBtnFile(self):
        ...
        _file = _dialog.getOpenFileNames(None, 'Select PointSet File(s)', (self.rootpath), filter='All files (*.*)')
        if len(_file[0]) > 0:
    ...
    def clickBtnImportPointSetFile(self):
        ...
        if _undefined_value is False:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import PointSet from File', 'Nom-float undefined value')
            return
        ...
        for i in range(_nfile):
            ...
            _pointsetdata[_filenamemain] = point_io.readPointSetFromAscii(_filename, comment=_comment,
        ...
        for key in _pointsetdata.keys():
            if key in self.pointsetdata.keys():
                if checkPointSetData(self.pointsetdata[key]):
                    reply = QtWidgets.QMessageBox.question(self.msgbox, 'Import PointSet from File', key + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
