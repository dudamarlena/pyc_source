...
class evaluateml15ddcnn(object):
    ...
    def clickBtnEvaluateMl15DDcnn(self):
        self.refreshMsgBox()
        if self.checkSurvInfo() is False:
            vis_msg.print('ERROR in EvaluateMl15DDcnn: No seismic survey available', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Evaluate 1.5D-DCNN', 'No seismic survey available')
            return
        if ml_tfm.check15DDCNNModel(self.modelpath, self.modelname) is False:
            vis_msg.print('ERROR in EvaluateMl15DDcnn: No pre-DCNN network found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Evaluate 1.5D-DCNN', 'No pre-DCNN network found')
            return
        for f in self.modelinfo['feature_list']:
            if self.checkSeisData(f) is False:
                vis_msg.print(("ERROR in EvaluateMl15DDcnn: Feature '%s' not found in seismic data" % f), type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Evaluate 1.5D-DCNN', "Feature '" + f + "' not found in seismic data")
                return

        if self.modelinfo['target'] not in self.seisdata.keys():
            vis_msg.print(("ERROR in EvaluateMl15DDcnn: Target label '%s' not found in seismic data" % self.modelinfo['target']),
              type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Evaluate 1.5D-DCNN', "Target label '" + self.modelinfo['target'] + ' not found in seismic data')
            return
        ...
        if _image_height is False or _image_width is False or _image_height_new is False or _image_width_new is False:
            vis_msg.print('ERROR in EvaluateMl15DDcnn: Non-integer feature size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Evaluate 1.5D-DCNN', 'Non-integer feature size')
            return
        if _image_height < 2 or _image_width < 2 or _image_height_new < 2 or _image_width_new < 2:
            vis_msg.print('ERROR in EvaluateMl15DDcnn: Features are not 2D', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Evaluate 1.5D-DCNN', 'Features are not 2D')
            return
        _batch = basic_data.str2int(self.ldtbatchsize.text())
        if _batch is False or _batch < 1:
            vis_msg.print('ERROR in EvaluateMl15DDcnn: Non-positive batch size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Evaluate 1.5D-DCNN', 'Non-positive batch size')
            return
        ...
        for i in range(_nloop):
            ...
            sys.stdout.write('\r>>> Evaluate 1.5D-DCNN, proceeding %.1f%% ' % (float(i) / float(_nloop) * 100.0))
            sys.stdout.flush()
            ...
            if _image_height_new != _image_height:
                if _target not in _features:
                    _dict[_target] = basic_curve.changeCurveSize((_dict[_target]), length=_image_height,
                      ...
                      kind='linear')
            _confmatrix = ml_dcnn15d.evaluate15DDCNNSegmentor(_dict, imageheight=_image_height_new,
