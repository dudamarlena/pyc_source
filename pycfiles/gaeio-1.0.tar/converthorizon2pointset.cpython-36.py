...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\gaeio\src\gui\converthorizon2pointset.py
...
class converthorizon2pointset(object):
    ...
    def clickBtnApply(self):
        ...
        if len(_horizonlist) < 1:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Convert Horizon to PointSet', 'No horizon selected for conversion')
            return
        ...
        if basic_data.str2float(self.ldtvalue.text()) is False:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Convert Horizon to PointSet', 'Non-float undefined value')
            return
        for _f in _horizonlist:
            if _f in self.pointsetdata.keys():
                reply = QtWidgets.QMessageBox.question(self.msgbox, 'Convert Horizon to PointSet', _f + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                if reply == QtWidgets.QMessageBox.No:
                    return

        for _hrz in _horizonlist:
            self.pointsetdata[_hrz] = horizon_ays.convertHorizon2PointSet((self.horizondata[_hrz]), property_list=_attriblist, undefined_value=(basic_data.str2float(self.ldtvalue.text())))

        QtWidgets.QMessageBox.information(self.msgbox, 'Convert Horizon to PointSet', 'Horizon converted successfully')
