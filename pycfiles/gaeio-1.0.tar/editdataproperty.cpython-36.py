...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\gaeio\src\gui\editdataproperty.py
    ...
    def clickBtnEditDataProperty(self):
        ...
        if self.cbbaction.currentIndex() != 3:
            if len(_attriblist) < 1:
                ...
                QtWidgets.QMessageBox.critical(self.msgbox, 'Edit Property', 'No property selected for editing')
                return
        ...
        if self.cbbaction.currentIndex() == 1:
            if len(self.ldtrename.text()) < 1:
                ...
                QtWidgets.QMessageBox.critical(self.msgbox, 'Edit Property', 'No name specified for rename')
                return
            if self.ldtrename.text() in self.dataproperty.keys():
                reply = QtWidgets.QMessageBox.question(self.msgbox, 'Edit Property', self.ldtrename.text() + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
