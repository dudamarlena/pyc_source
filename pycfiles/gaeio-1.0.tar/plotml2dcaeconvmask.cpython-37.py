...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\cognitivegeo\src\gui\plotml2dcaeconvmask.py
# Compiled at: 2019-12-16 00:14:22
# Size of source mod 2**32: 4660 bytes
...
class plotml2dcaeconvmask(gui_core):
    modelpath = ''
    modelname = ''
    ...
    def setupGUI(self, PlotMl2DCaeConvMask):
        gui_core.title = 'Plot 2D-CAE Conv. Mask'
        gui_core.icon = 'mask.png'
        gui_core.imagestyle = self.maskstyle
        gui_core.imagelist = self.getConvMaskList()
        gui_core.setupGUI(self, PlotMl2DCaeConvMask)

    def clickBtnApply(self):
        self.refreshMsgBox()
        if ml_tfm.checkCAEModel(self.modelpath, self.modelname) is False:
            vis_msg.print('ERROR in PlotMl2DCaeConvMask: No CAE network found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Plot 2D-CAE Conv. Mask', 'No CAE network found')
            return
        ...
        ml_cae.plotCAEConvMask(caepath=(self.modelpath), caename=(self.modelname), blockidx=_blockidx,
          layeridx=_layeridx,
          ncol=_ncol,
          cmapname=_colormap,
          flipcmap=_flipped,
...
if __name__ == '__main__':
    ...
    PlotMl2DCaeConvMask = QtWidgets.QWidget()
    gui = plotml2dcaeconvmask()
    gui.setupGUI(PlotMl2DCaeConvMask)
    PlotMl2DCaeConvMask.show()
    sys.exit(app.exec_())