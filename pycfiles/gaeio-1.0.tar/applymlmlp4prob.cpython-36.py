...
class applymlmlp4prob(object):
    ...
    def clickBtnApplyMlMlp4Prob(self):
        self.refreshMsgBox()
        if self.checkSurvInfo() is False:
            vis_msg.print('ERROR in ApplyMlMlp4Prob: No seismic survey available', type='error')
        ...
        if ml_tfm.checkFNNModel(self.modelpath, self.modelname) is False:
            vis_msg.print('ERROR in ApplyMlMlp4Prob: No MLP network found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply MLP', 'No MLP network found')
            return
        _featurelist = self.modelinfo['feature_list']
        for f in _featurelist:
            if self.checkSeisData(f) is False:
                vis_msg.print(("ERROR in ApplyMlMlp4Prob: Feature '%s' not found in seismic data" % f), type='error')
        ...
        if self.ldtoldlength.text() != self.ldtnewlength.text() or self.ldtoldtotal.text() != self.ldtnewtotal.text():
            vis_msg.print('ERROR in ApplyMlMlp4Prob: Feature length not match', type='error')
        ...
        if _patch_height is False or _patch_width is False or _patch_depth is False or _patch_height < 1 or _patch_width < 1 or _patch_depth < 1:
            vis_msg.print('ERROR in ApplyMlMlp4Prob: Non-positive feature size', type='error')
        ...
        if _batch is False or _batch < 1:
            vis_msg.print('ERROR in ApplyMlMlp4Prob: Non-positive batch size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply MLP', 'Non-positive batch size')
            return
        if len(self.ldtsave.text()) < 1:
            vis_msg.print('ERROR in ApplyMlMlp4Prob: No prefix specified', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply MLP', 'No prefix specified')
            return
        if len(self.lwgtarget.selectedItems()) < 1:
            vis_msg.print('ERROR in ApplyMlMlp4Prob: No target label specified', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply MLP', 'No target label specified')
            return
        ...
        for i in range(_nloop):
            ...
            _result[idxlist, :] = ml_fnn.probabilityFromFNNClassifier(_dict, fnnpath=(self.modelpath),
