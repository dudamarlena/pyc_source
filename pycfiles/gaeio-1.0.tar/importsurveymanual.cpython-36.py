...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\gaeio\src\gui\importsurveymanual.py
# Compiled at: 2020-04-25 15:07:54
# Size of source mod 2**32: 10314 bytes
...
class importsurveymanual(object):
    ...
    def clickBtnImportSurveyManual(self):
        ...
        if _inlstart is False or _xlstart is False or _zstart is False or _inlstep is False or _xlstep is False or _zstep is False or _inlnum is False or _xlnum is False or _znum is False:
            vis_msg.print('ERROR in ImportSurveyManual: Non-integer survey information', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Create Survey', 'Non-integer survey information')
            return
        if _inlnum <= 0 or _xlnum <= 0 or _znum <= 0:
            vis_msg.print('ERROR in ImportSurveyManual: Zero survey dimension', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Create Survey', 'Zero survey dimension')
            return
        if _inlstep <= 0 or _xlstep <= 0 or _zstep == 0:
            vis_msg.print('ERROR in ImportSurveyManual: Zero survey step', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Create Survey', 'Zero survey step')
            return
        self.survinfo = seis_ays.createSeisInfoFrom3DMat(seis3dmat=(np.zeros([_znum, _xlnum, _inlnum])), inlstart=_inlstart,
          inlstep=_inlstep,
          xlstart=_xlstart,
          xlstep=_xlstep,
          zstart=_zstart,
          zstep=_zstep)
        QtWidgets.QMessageBox.information(self.msgbox, 'Create Survey', 'Survey created successfully')
