...
class evaluateml2dwdcnn(object):
    ...
    def clickBtnEvaluateMl2DWdcnn(self):
        self.refreshMsgBox()
        if self.checkSurvInfo() is False:
            vis_msg.print('ERROR in EvaluateMl2DWdcnn: No seismic survey available', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Evaluate 2D-WDCNN', 'No seismic survey available')
            return
        if ml_tfm.checkWDCNNModel(self.modelpath, self.modelname) is False:
            vis_msg.print('ERROR in EvaluateMl2DWdcnn: No pre-WDCNN network found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Evaluate 2D-WDCNN', 'No pre-WDCNN network found')
            return
        for f in self.modelinfo['feature_list']:
            if self.checkSeisData(f) is False:
                vis_msg.print(("ERROR in EvaluateMl2DWdcnn: Feature '%s' not found in seismic data" % f), type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Evaluate 2D-WDCNN', "Feature '" + f + "' not found in seismic data")
                return

        if self.modelinfo['target'] not in self.seisdata.keys():
            vis_msg.print(("ERROR in EvaluateMl2DWdcnn: Target key '%s' not found in seismic data" % self.modelinfo['target']),
              type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Evaluate 2D-WDCNN', "Target key '" + self.modelinfo['target'] + ' not found in seismic data')
            return
        ...
        if _image_height is False or _image_width is False or _image_height_new is False or _image_width_new is False:
            vis_msg.print('ERROR in EvaluateMl2DWdcnn: Non-integer feature size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Evaluate 2D-WDCNN', 'Non-integer feature size')
            return
        if _image_height < 2 or _image_width < 2 or _image_height_new < 2 or _image_width_new < 2:
            vis_msg.print('ERROR in EvaluateMl2DWdcnn: Features are not 2D', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Evaluate 2D-WDCNN', 'Features are not 2D')
            return
        _batch = basic_data.str2int(self.ldtbatchsize.text())
        if _batch is False or _batch < 1:
            vis_msg.print('ERROR in EvaluateMl2DWdcnn: Non-positive batch size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Evaluate 2D-WDCNN', 'Non-positive batch size')
            return
        ...
        for i in range(_nloop):
            ...
            sys.stdout.write('\r>>> Evaluate 2D-WDCNN, proceeding %.1f%% ' % (float(i) / float(_nloop) * 100.0))
            sys.stdout.flush()
            ...
            _confmatrix = ml_wdcnn.evaluateWDCNNSegmentor(_dict, imageheight=_image_height_new,
