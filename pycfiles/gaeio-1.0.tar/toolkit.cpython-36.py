...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\gaeio\src\segpy\toolkit.py
# Compiled at: 2020-04-25 14:33:31
# Size of source mod 2**32: 30737 bytes
from __future__ import print_function
from array import array
from collections import OrderedDict
from itertools import zip_longest, count
import os, struct, re, logging, sys
sys.path.append(os.path.dirname(__file__)[:-6][:-4][:-6])
import gaeio.src.segpy.textual_reel_header
from gaeio.src.segpy.binary_reel_header import BinaryReelHeader
from gaeio.src.segpy.catalog import CatalogBuilder
from gaeio.src.segpy.datatypes import SEG_Y_TYPE_TO_CTYPE, size_in_bytes, DATA_SAMPLE_FORMAT_TO_SEG_Y_TYPE, CTYPE_TO_SIZE
from gaeio.src.segpy.encoding import guess_encoding, is_supported_encoding, UnsupportedEncodingError
from gaeio.src.segpy.ibm_float import IBMFloat
from gaeio.src.segpy.packer import HeaderPacker
from gaeio.src.segpy.revisions import canonicalize_revision
from gaeio.src.segpy.trace_header import TraceHeaderRev1
from gaeio.src.segpy.util import file_length, batched, pad, complementary_intervals, NATIVE_ENDIANNESS, EMPTY_BYTE_STRING
HEADER_NEWLINE = '\r\n'
CARD_LENGTH = 80
CARDS_PER_HEADER = 40
...
def read_textual_reel_header(fh, encoding):
    ...
    if num_bytes_read < TEXTUAL_HEADER_NUM_BYTES:
        raise EOFError('Only {} bytes of {} byte textual reel header could be read'.format(num_bytes_read, TEXTUAL_HEADER_NUM_BYTES))
...
def catalog_traces(fh, bps, trace_header_format=TraceHeaderRev1, endian='>', progress=None):
    ...
    for trace_number in count():
        ...
        line_catalog_builder.add((trace_header.inline_number,
         trace_header.crossline_number), trace_number)
        alt_line_catalog_builder.add((trace_header.file_sequence_num,
         trace_header.ensemble_num), trace_number)
...
c
    ...
    if len(buf) < block_size:
        ...
        values = unpack_ibm_floats(buf, count) if ctype == 'ibm' else unpack_values(buf, count, ctype, endian)
...
def format_standard_textual_header(revision, **kwargs):
    ...
    if len(kwargs) > 0:
        raise TypeError('The following keyword arguments did not correspond to template placeholders: {!r}'.format(list(kwargs.keys())))
    concatenation = ''.join(chunks)
    lines = concatenation.splitlines(keepends=False)
    return lines[1:]


_TEMPLATE_PATTERN = '\\{\\s*(\\w*)\\s*\\}'
...
def write_textual_reel_header(fh, lines, encoding):
    ...
    if not is_supported_encoding(encoding):
        ...
        fh.seek(0)
        padded_lines = [line.encode(encoding).ljust(CARD_LENGTH, ' '.encode(encoding))[:CARD_LENGTH] for line in pad(lines, padding='', size=CARDS_PER_HEADER)]
...
def format_extended_textual_header(text, encoding, include_text_stop=False):
    ...
    for original_line in original_lines:
        padded_lines = (pad_and_terminate_header_line(original_line[i:i + width], width) for i in range(0, len(original_line), width))
    ...
    for page_index, page in enumerate(pages):
        ...
        for line_index, line in enumerate(page):
            ...
            if num_encoded_bytes != CARD_LENGTH:
                raise ValueError('Extended textual header line {} of page {} at {} bytes is not {} bytes'.format(line_index, page_index, num_encoded_bytes, CARD_LENGTH))
            encoded_page.append(encoded_line)

        num_encoded_lines = len(encoded_page)
        if num_encoded_lines != CARDS_PER_HEADER:
            raise ValueError('Extended textual header page {} number of lines {} is not {}'.format(num_encoded_lines, CARDS_PER_HEADER))
...
def write_binary_values(fh, values, ctype, pos=None, endian='>'):
    ...
    if pos is not None:
        fh.seek(pos, os.SEEK_SET)
    buf = pack_ibm_floats(values) if fmt == 'ibm' else pack_values(values, fmt, endian)
    fh.write(buf)
