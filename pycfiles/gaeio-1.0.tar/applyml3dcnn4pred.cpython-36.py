...
class applyml3dcnn4pred(object):
    ...
    def clickBtnApplyMl3DCnn4Pred(self):
        self.refreshMsgBox()
        if self.checkSurvInfo() is False:
            vis_msg.print('ERROR in ApplyMl3DCnn4Pred: No seismic survey available', type='error')
        ...
        if ml_tfm.check3DCNNModel(self.modelpath, self.modelname) is False:
            vis_msg.print('ERROR in ApplyMl3DCnn4Pred: No CNN network found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 3D-CNN', 'No CNN network found')
            return
        _featurelist = self.modelinfo['feature_list']
        for f in _featurelist:
            if self.checkSeisData(f) is False:
                vis_msg.print(("ERROR in ApplyMl3DCnn4Pred: Feature '%s' not found in seismic data" % f), type='error')
        ...
        if _video_height_old is False or _video_width_old is False or _video_depth_old is False or _video_height_new is False or _video_width_new is False or _video_depth_new is False:
            vis_msg.print('ERROR in ApplyMl3DCnn4Pred: Non-integer feature size', type='error')
        ...
        if _batch is False or _batch < 1:
            vis_msg.print('ERROR in ApplyMl3DCnn4Pred: Non-positive batch size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 3D-CNN', 'Non-positive batch size')
            return
        if len(self.ldtsave.text()) < 1:
            vis_msg.print('ERROR in ApplyMl3DCnn4Pred: No name specified', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 3D-CNN', 'No name specified')
            return
        ...
        for i in range(_nloop):
            ...
            _result[idxlist, 0:1] = ml_cnn3d.predictionFrom3DCNNClassifier(_dict, cnn3dpath=(self.modelpath),
