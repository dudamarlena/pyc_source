...
class extractml2ddcnn(object):
    ...
    def clickBtnExtractMl2DDcnn(self):
        self.refreshMsgBox()
        if self.checkSurvInfo() is False:
            vis_msg.print('ERROR in ExtractMl2DDcnn: No seismic survey available', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Extract 2D-DCNN', 'No seismic survey available')
            return
        if ml_tfm.checkDCNNModel(self.modelpath, self.modelname) is False:
            vis_msg.print('ERROR in ExtractMl2DDcnn: No pre-DCNN network found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Extract 2D-DCNN', 'No pre-DCNN network found')
            return
        for f in self.modelinfo['feature_list']:
            if self.checkSeisData(f) is False:
                vis_msg.print(("ERROR in ExtractMl2DDcnn: Feature '%s' not found in seismic data" % f), type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Extract 2D-DCNN', "Feature '" + f + "' not found in seismic data")
                return
        ...
        if _image_height is False or _image_width is False or _image_height_new is False or _image_width_new is False:
            vis_msg.print('ERROR in ExtractMl2DDcnn: Non-integer feature size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Extract 2D-DCNN', 'Non-integer feature size')
            return
        if _image_height < 2 or _image_width < 2 or _image_height_new < 2 or _image_width_new < 2:
            vis_msg.print('ERROR in ExtractMl2DDcnn: Features are not 2D', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Extract 2D-DCNN', 'Features are not 2D')
            return
        _batch = basic_data.str2int(self.ldtbatchsize.text())
        if _batch is False or _batch < 1:
            vis_msg.print('ERROR in ExtractMl2DDcnn: Non-positive batch size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Extract 2D-DCNN', 'Non-postive batch size')
            return
        if len(self.ldtsave.text()) < 1:
            vis_msg.print('ERROR in ExtractMl2DDcnn: No prefix specified', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Extract 2D-DCNN', 'No prefix specified')
            return
        if self.ldtsave.text() + '1' in self.seisdata.keys():
            if self.checkSeisData(self.ldtsave.text() + '1'):
                reply = QtWidgets.QMessageBox.question(self.msgbox, 'Extract 2D-DCNN', 'Prefix ' + self.ldtsave.text() + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
        ...
        _pgsdlg.setWindowTitle('Extract 2D-DCNN')
        ...
        for i in range(_nloop):
            ...
            sys.stdout.write('\r>>> Extract 2D-DCNN, proceeding %.1f%% ' % (float(i) / float(_nloop) * 100.0))
            sys.stdout.flush()
            ...
            _rst = ml_dcnn.extractDCNNConvFeature(_dict, imageheight=_image_height_new,
        ...
        QtWidgets.QMessageBox.information(self.msgbox, 'Extract 2D-DCNN', str(_nfeature) + ' DCNN features extracted successfully')
