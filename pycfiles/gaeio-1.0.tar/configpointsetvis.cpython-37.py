...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\cognitivegeo\src\gui\configpointsetvis.py
# Compiled at: 2020-03-23 08:59:53
...
class configpointsetvis(object):
    ...
    def retranslateGUI(self, ConfigPointSetVis):
        ...
        for point in sorted(self.pointsetvisconfig.keys()):
            ...
            item.setCurrentIndex(list.index(self.MarkerStyleList, self.pointsetvisconfig[point]['Marker']))
            self.twgpoint.setCellWidget(_idx, 1, item)
            item = QtWidgets.QComboBox()
            item.addItems(self.MarkerColorList)
            for _i in range(len(self.MarkerColorList)):
                item.setItemIcon(_i, QtGui.QIcon(QtGui.QPixmap(os.path.join(self.iconpath, 'icons/color_' + self.MarkerColorList[_i] + '.png')).scaled(80, 40)))

            item.setCurrentIndex(list.index(self.MarkerColorList, self.pointsetvisconfig[point]['Color']))
            self.twgpoint.setCellWidget(_idx, 2, item)
            item = QtWidgets.QComboBox()
            item.addItems([str(_i) for _i in self.MarkerSizeList])
            item.setCurrentIndex(list.index(self.MarkerSizeList, self.pointsetvisconfig[point]['Size']))
