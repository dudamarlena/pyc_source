...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\gaeio\src\gui\edithorizon.py
# Compiled at: 2020-04-25 14:58:02
# Size of source mod 2**32: 9712 bytes
...
class edithorizon(object):
    ...
    def clickBtnEditHorizon(self):
        ...
        if len(_horizonlist) < 1:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Edit Horizon', 'No horizon selected for editing')
            return
        ...
        if self.cbbaction.currentIndex() == 2:
            if len(self.ldtrename.text()) < 1:
                ...
                QtWidgets.QMessageBox.critical(self.msgbox, 'Edit Horizon', 'No name specified for rename')
                return
            if self.ldtrename.text() in self.horizondata.keys():
                reply = QtWidgets.QMessageBox.question(self.msgbox, 'Edit Horizon', self.ldtrename.text() + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
        ...
        if self.cbbaction.currentIndex() == 3:
            ...
            _gui.dataproperty = horizon_ays.convertHorizon2PointSet(horizon=_hrz, property_list=(list(_hrz['HorizonData'].keys())))
            _gui.rootpath = self.rootpath
            _gui.setupGUI(_editproperty)
            _editproperty.exec()
            _pts = _gui.dataproperty
            self.horizondata[_horizonlist[0].text()] = horizon_ays.convertPointSet2Horizon(pointset=_pts, property_list=(list(_pts.keys())))
            _editproperty.show()
        self.refreshLwgHorizon()
