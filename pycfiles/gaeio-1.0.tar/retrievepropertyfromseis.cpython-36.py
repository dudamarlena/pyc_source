...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\gaeio\src\gui\retrievepropertyfromseis.py
# Compiled at: 2020-04-25 15:12:01
# Size of source mod 2**32: 14413 bytes
...
class retrievepropertyfromseis(object):
    ...
    def retranslateGUI(self, RetrievePropertyFromSeis):
        ...
        RetrievePropertyFromSeis.setWindowTitle(_translate('RetrievePropertyFromSeis', 'Retrieve Data Property from Seismic'))
    ...
    def changeLdtFrom(self):
        ...
        try:
            ...
            if 'SeisInfo' not in self.npydata.keys() or seis_ays.checkSeisInfo(self.npydata['SeisInfo']) is False:
                vis_msg.print('ERROR in RetrievePropertyFromSeis: NumPy dictionary contains no information about seismic survey', type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Retrieve Data Property from Seismic', 'NumPy dictionary contains no information about seismic')
                return
            ...
            if 'SeisData' in self.npydata.keys():
                if type(self.npydata['SeisData']) is not dict:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Retrieve Data Property from Seismic', 'NumPy dictionary expected')
            return
        else:
            if seis_ays.checkSeisInfo(self.npyinfo) is False:
                ...
                QtWidgets.QMessageBox.critical(self.msgbox, 'Retrieve Data Property from Seismic', 'Selected not seismic numpy')
                return
            _firstattrib = None
    ...
    def clickBtnFrom(self):
        ...
        _file = _dialog.getOpenFileName(None, 'Select Seismic NumPy Dictionary', (self.rootpath), filter='NumPy files (*.seis.npy);; All files (*.*)')
        if len(_file[0]) > 0:
    ...
    def clickBtnRetrievePropertyFromSeis(self):
        ...
        if len(_attriblist) < 1:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Retrieve Data Property from Seismic', 'No property selected for export')
            return
        if self.checkPointSetData() is False:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Retrieve Data Property from Seismic', 'No point loaded for retrieval')
            return
        ...
        if _wdinl is False or _wdxl is False or _wdz is False:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Retrieve Data Property from Seismic', 'Non-integer retrieval window')
            return
        if _wdinl < 0 or _wdxl < 0 or _wdz < 0:
            vis_msg.print('ERROR in RetrievePropertyFromSeis: Non-positive retrieval window', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Retrieve Data Property from Seismic', 'Non-positive retrieval window')
            return
        ...
        for i in range(len(_attriblist)):
            ...
            _data = seis_ays.retrieveSeisWindowFrom3DMat(_npydata, _targetdata, seisinfo=(self.npyinfo), wdinl=_wdinl,
              wdxl=_wdxl,
              wdz=_wdz,
              verbose=False,
              qpgsdlg=_pgsdlg)
            ...
            if _attriblist[i].text() in self.pointsetdata.keys():
                reply = QtWidgets.QMessageBox.question(self.msgbox, 'Retrieve Data Property from Seismic', _attriblist[i].text() + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                if reply == QtWidgets.QMessageBox.No:
                    return
            self.pointsetdata[_attriblist[i].text()] = np.reshape(_data, [-1, 1])

        QtWidgets.QMessageBox.information(self.msgbox, 'Retrieve Data Property from Seismic', str(len(_attriblist)) + ' properties retrieved successfully')
