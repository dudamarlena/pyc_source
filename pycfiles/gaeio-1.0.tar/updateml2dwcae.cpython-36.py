...
class updateml2dwcae(object):
    ...
    def clickBtnUpdateMl2DWcae(self):
        self.refreshMsgBox()
        if self.checkSurvInfo() is False:
            vis_msg.print('ERROR in UpdateMl2DWcae: No seismic data available', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WCAE', 'No seismic data available')
            return
        if ml_tfm.checkWCAEModel(self.modelpath, self.modelname) is False:
            vis_msg.print('ERROR in UpdateMl2DWcae: No pre-WCAE network found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WCAE', 'No pre-WCAE network found')
            return
        for f in self.modelinfo['feature_list']:
            if self.checkSeisData(f) is False:
                vis_msg.print(("ERROR in UpdateMl2DWcae: Feature '%s' not found in seismic data" % f), type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WCAE', "Feature '" + f + "' not found in seismic data")
                return
        ...
        if _image_height is False or _image_width is False or _image_height_new is False or _image_width_new is False:
            vis_msg.print('ERROR in UpdateMl2DWcae: Non-integer feature size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WCAE', 'Non-integer feature size')
            return
        if _image_height < 2 or _image_width < 2 or _image_height_new < 2 or _image_width_new < 2:
            vis_msg.print('ERROR in UpdateMl2DWcae: Features are not 2D', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WCAE', 'Features are not 2D')
            return
        ...
        if self.modelinfo['target'] not in self.seisdata.keys():
            vis_msg.print(("ERROR in UpdateMl2DWcae: Target '%s' not found in seismic data" % self.modelinfo['target']),
              type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WCAE', "Target '" + self.modelinfo['target'] + ' not found in seismic data')
            return
        if self.modelinfo['weight'] not in self.seisdata.keys():
            vis_msg.print(("ERROR in UpdateMl2DWcae: Weight '%s' not found in seismic data" % self.modelinfo['weight']),
              type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WCAE', "Weight '" + self.modelinfo['target'] + ' not found in seismic data')
            return
        ...
        if _nepoch is False or _nepoch <= 0:
            vis_msg.print('ERROR in UpdateMl2DWcae: Non-positive epoch number', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WCAE', 'Non-positive epoch number')
            return
        if _batchsize is False or _batchsize <= 0:
            vis_msg.print('ERROR in UpdateMl2DWcae: Non-positive batch size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WCAE', 'Non-positive batch size')
            return
        if _learning_rate is False or _learning_rate <= 0:
            vis_msg.print('ERROR in UpdateMl2DWcae: Non-positive learning rate', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WCAE', 'Non-positive learning rate')
            return
        if _dropout_prob is False or _dropout_prob <= 0:
            vis_msg.print('ERROR in UpdateMl2DWcae: Negative dropout rate', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WCAE', 'Negative dropout rate')
            return
        if len(self.ldtsave.text()) < 1:
            vis_msg.print('ERROR in UpdateMl2DWcae: No name specified for new-WCAE', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2DW-CAE', 'No name specified for new-CAE')
            return
        ...
        print('UpdateMl2DWcae: Step 1 - Get training samples:')
        ...
        if np.shape(_traindata)[0] <= 0:
            vis_msg.print('ERROR in UpdateMl2DWcae: No training sample found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WCAE', 'No training sample found')
            return
        print('UpdateMl2DWcae: Step 2 - Retrieve and interpolate images if necessary: (%d, %d) --> (%d, %d)' % (
        ...
        print('UpdateMl2DWcae: A total of %d valid training samples' % basic_mdt.maxDictConstantRow(_traindict))
        print('UpdateMl2DWcae: Step 3 - Start training')
        ...
        _caelog = ml_wcae.updateWCAEReconstructor(_traindict, imageheight=_image_height_new,
          ...
          savename=_savename,
          qpgsdlg=_pgsdlg)
        QtWidgets.QMessageBox.information(self.msgbox, 'Update 2D-WCAE', 'WCAE updated successfully')
        reply = QtWidgets.QMessageBox.question(self.msgbox, 'Update 2D-WCAE', 'View learning matrix?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.Yes)
