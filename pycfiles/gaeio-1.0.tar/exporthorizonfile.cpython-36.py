...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\gaeio\src\gui\exporthorizonfile.py
...
class exporthorizonfile(object):
    ...
    def clickBtnSave(self):
        ...
        _file = _dialog.getSaveFileName(None, 'Select Horizon File', (self.rootpath), filter='Horizon Ascii files (*.txt);; All files (*.*)')
        if len(_file[0]) > 0:
    ...
    def clickBtnExportHorizonFile(self):
        ...
        if len(_horizonlist) < 1:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Export Horizon FIle', 'No horizon selected for export')
            return
        if len(self.ldtsave.text()) < 1:
            vis_msg.print('ERROR in ExportHorizonFile: No name specified for export', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Export Horizon File', 'No name specified for export')
            return
        ...
        if len(_horizonlist) > 1:
            reply = QtWidgets.QMessageBox.question(self.msgbox, 'Export Horizon File', 'Warning: For exporting >=2 horizon, property name used as file name. Continue?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
            if reply == QtWidgets.QMessageBox.No:
                return
        for i in range(len(_horizonlist)):
            if len(_horizonlist) > 1:
                _savename = _horizonlist[i].text()
            horizon_io.writeHorizonToAscii(horizon=(self.horizondata[_horizonlist[i].text()]), asciifile=(os.path.join(_savepath, _savename)),
              property_list=(list(self.horizondata[_horizonlist[i].text()].keys())))

        QtWidgets.QMessageBox.information(self.msgbox, 'Export Horizon File', str(len(_horizonlist)) + ' horizons exported as Ascii File successfully')
