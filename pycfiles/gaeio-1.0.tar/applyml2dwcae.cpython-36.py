...
class applyml2dwcae(object):
    ...
    def clickBtnApplyMl2DWcae(self):
        self.refreshMsgBox()
        if self.checkSurvInfo() is False:
            vis_msg.print('ERROR in ApplyMl2DWcae: No seismic survey available', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 2D-WCAE', 'No seismic survey available')
            return
        if ml_tfm.checkWCAEModel(self.modelpath, self.modelname) is False:
            vis_msg.print('ERROR in ApplyMl2DWcae: No pre-WCAE network found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 2D-WCAE', 'No pre-WCAE network found')
            return
        for f in self.modelinfo['feature_list']:
            if self.checkSeisData(f) is False:
                vis_msg.print(("ERROR in ApplyMl2DWcae: Feature '%s' not found in seismic data" % f), type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 2D-WCAE', "Feature '" + f + "' not found in seismic data")
                return
        ...
        if _image_height is False or _image_width is False or _image_height_new is False or _image_width_new is False:
            vis_msg.print('ERROR in ApplyMl2DWcae: Non-integer feature size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 2D-WCAE', 'Non-integer feature size')
            return
        if _image_height < 2 or _image_width < 2 or _image_height_new < 2 or _image_width_new < 2:
            vis_msg.print('ERROR in ApplyMl2DWcae: Features are not 2D', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 2D-WCAE', 'Features are not 2D')
            return
        _batch = basic_data.str2int(self.ldtbatchsize.text())
        if _batch is False or _batch < 1:
            vis_msg.print('ERROR in ApplyMl2DWcae: Non-positive batch size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 2D-WCAE', 'Non-positive batch size')
            return
        if len(self.ldtsave.text()) < 1:
            vis_msg.print('ERROR in ApplyMl2DWcae: No name specified', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 2D-WCAE', 'No name specified')
            return
        if self.ldtsave.text() in self.seisdata.keys():
            if self.checkSeisData(self.ldtsave.text()):
                reply = QtWidgets.QMessageBox.question(self.msgbox, 'Apply 2D-WCAE', self.ldtsave.text() + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
        ...
        _pgsdlg.setWindowTitle('Apply 2D-WCAE')
        ...
        for i in range(_nloop):
            ...
            sys.stdout.write('\r>>> Apply 2D-WCAE, proceeding %.1f%% ' % (float(i) / float(_nloop) * 100.0))
            sys.stdout.flush()
            ...
            _rst = ml_wcae.reconstructFromWCAEReconstructor(_dict, imageheight=_image_height_new,
        ...
        QtWidgets.QMessageBox.information(self.msgbox, 'Apply 2D-WCAE', 'WCAE applied successfully')
