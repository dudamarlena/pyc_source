...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\gaeio\src\gui\convertseis2psseis.py
# Compiled at: 2020-04-25 14:58:02
# Size of source mod 2**32: 23688 bytes
...
class convertseis2psseis(object):
    ...
    def clickBtnApply(self):
        ...
        if self.ldtsave.text() in self.psseisdata.keys():
            reply = QtWidgets.QMessageBox.question(self.msgbox, 'Convert Seismic to Pre-stack', self.ldtsave.text() + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
            if reply == QtWidgets.QMessageBox.No:
                return
        if self.checkSurvInfo() is False:
            vis_msg.print('ERROR in ConvertSeis2PsSeis: No seismic survey found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Convert Seismic to Pre-stack', 'No seismic survey found')
            return
        if len(self.lwgattrib.selectedItems()) < 1:
            vis_msg.print('ERROR in ConvertSeis2PsSeis: No seismic property selected', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Convert Seismic to Pre-stack', 'No seismic property selected')
            return
        if self.rdbsrvpart.isChecked():
            ...
            if _inlstart is False or _xlstart is False or _zstart is False or _inlend is False or _xlend is False or _zend is False:
                vis_msg.print('ERROR in ConvertSeis2PsSeis: Non-integer survey selection', type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Convert Seismic to Pre-stack', 'Non-integer survey selection')
                return
        _survinfo = self.survinfo
        if self.rdbsrvall.isChecked():
        ...
        for _idx in range(len(_proplist)):
            ...
            if self.rdbsrvpart.isChecked():
                ...
                _data = seis_ays.retrieveSeisSampleFrom3DMat(_data, seisinfo=(self.survinfo), verbose=False,
                  samples=_pts)
            ...
            self.psseisdata[self.ldtsave.text()][_proplist[_idx]]['ShotInfo'] = psseis_ays.createShotInfoFromShotData(_data, zstart=(_info['ZStart']),
              zstep=(_info['ZStep']),
              xlstart=(_info['XLStart']),
              xlstep=(_info['XLStep']),
              inlstart=(_info['ILStart']),
              inlstep=(_info['ILStep']))

        _pgsdlg.setValue(len(_proplist))
        QtWidgets.QMessageBox.information(self.msgbox, 'Convert Seismic to Pre-stack', str(len(_proplist)) + ' properties converted successfully')
