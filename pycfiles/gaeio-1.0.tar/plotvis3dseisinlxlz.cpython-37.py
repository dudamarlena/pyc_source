...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\cognitivegeo\src\gui\plotvis3dseisinlxlz.py
# Compiled at: 2020-03-18 15:05:47
# Size of source mod 2**32: 24244 bytes
...
import cognitivegeo.src.gui.configplayer as gui_configplayer
...
class plotvis3dseisinlxlz(object):
    ...
    def setupGUI(self, PlotVis3DSeisInlXlZ):
        ...
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(os.path.join(self.iconpath, 'icons/video.png')), QtGui.QIcon.Normal, QtGui.QIcon.Off)
    ...
    def retranslateGUI(self, PlotVis3DSeisInlXlZ):
        ...
        if self.checkSurvInfo() is True:
            if self.lwgattrib.currentItem() is not None:
                if self.checkSeisData(self.lwgattrib.currentItem().text()) is True:
    ...
    def changeLdtInlSlice(self):
        if len(self.ldtinlslice.text()) > 0:
            ...
            if _val >= self.slbinlslice.minimum():
                if _val <= self.slbinlslice.maximum():
    ...
    def changeLdtXlSlice(self):
        if len(self.ldtxlslice.text()) > 0:
            ...
            if _val >= self.slbxlslice.minimum():
                if _val <= self.slbxlslice.maximum():
    ...
    def changeLdtZSlice(self):
        if len(self.ldtzslice.text()) > 0:
            ...
            if _val >= self.slbzslice.minimum():
                if _val <= self.slbzslice.maximum():
    ...
    def clickBtnPlotSlice(self):
        ...
        if len(_attriblist) < 1:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, '3D Window: Seismic', 'No property selected for plot')
            return
        _inlsls = None
        if self.cbxinlslice.isChecked():
        ...
        if _min is False or _max is False:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, '3D Window: Seismic', 'Non-float range specified for plot')
            return
        ...
        if _zsls is False or _zscale <= 0.0:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, '3D Window: Seismic', 'z-scale > 0.0 required')
            return
        ...
        for i in range(len(_attriblist)):
            ...
            seis_vis.plotSeisILXLZSlicePlayerFrom3DMat(_data, seisinfo=(self.survinfo), initinlsl=_inlsls,
              initxlsl=_xlsls,
              initzsl=_zsls,
              valuemax=_max,
              valuemin=_min,
    ...
    def getAttribRange(self, f):
        ...
        _max = 1
        if self.checkSurvInfo() is True:
            if f in self.seisdata.keys():
                if self.checkSeisData(f) is True:
