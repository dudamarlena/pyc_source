...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\gaeio\src\gui\exportseisnpy.py
# Compiled at: 2020-04-25 14:58:02
# Size of source mod 2**32: 10973 bytes
...
class exportseisnpy(object):
    ...
    def retranslateGUI(self, ExportSeisNpy):
        ...
        self.cbbtype.addItems(['Dictionary (all in one)', 'Dictionary (individual)',
         '2-D numpy matrix', '3-D numpy matrix'])
        ...
        self.cbbtype.setItemIcon(3, QtGui.QIcon(os.path.join(self.iconpath, 'icons/vis3d.png')))
        self.cbbtype.setCurrentIndex(3)
    ...
    def clickBtnSave(self):
        ...
        _file = _dialog.getSaveFileName(None, 'Select Seismic NumPy', (self.rootpath), filter='Seismic NumPy files (*.seis.npy);; All files (*.*)')
        if len(_file[0]) > 0:
    ...
    def clickBtnExportSeisNpy(self):
        ...
        if len(_attriblist) < 1:
            vis_msg.print('ERROR in ExportSeisNpy; No property selected for export', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Export Seismic NumPy', 'No property selected for export')
            return
        if len(self.ldtsave.text()) < 1:
            vis_msg.print('ERROR in ExportSeisNpy: No name specified for NumPy', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Export Seismic NumPy', 'No name specified for NumPy')
            return
        ...
        if len(_attriblist) > 1 and self.cbbtype.currentIndex() >= 1:
            reply = QtWidgets.QMessageBox.question(self.msgbox, 'Export Seismic Numpy', 'Warning: For exporting >=2 seismic, property name used as file name. Continue?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
            if reply == QtWidgets.QMessageBox.No:
                return
        if self.cbbtype.currentIndex() == 0:
            _npydata = {}
            _npydata['SeisInfo'] = self.survinfo
            _npydata['SeisData'] = {}
            for i in range(len(_attriblist)):
                _npydata['SeisData'][_attriblist[i].text()] = self.seisdata[_attriblist[i].text()]
        ...
        if self.cbbtype.currentIndex() == 1:
            for i in range(len(_attriblist)):
                _npydata = {}
                _npydata['SeisInfo'] = self.survinfo
                _npydata['SeisData'] = self.seisdata[_attriblist[i].text()]
                if len(_attriblist) > 1:
        ...
        if self.cbbtype.currentIndex() == 2:
            ...
            for i in range(len(_attriblist)):
                ...
                _data = np.reshape(np.transpose(_data, [2, 1, 0]), [-1, 1])
                _data = np.concatenate((_npydata, _data), axis=1)
                if len(_attriblist) > 1:
                    _savename = _attriblist[i].text() + '.seis.npy'
                np.save(os.path.join(_savepath, _savename), _data)

        if self.cbbtype.currentIndex() == 3:
            reply = QtWidgets.QMessageBox.question(self.msgbox, 'Export Seismic Numpy', 'Warning: survey information not saved in 3D numpy matrix. Continue?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
            if reply == QtWidgets.QMessageBox.No:
                return
            for i in range(len(_attriblist)):
                _data = self.seisdata[_attriblist[i].text()]
        ...
        QtWidgets.QMessageBox.information(self.msgbox, 'Export Seismic NumPy', str(len(_attriblist)) + ' properties exported as NumPy successfully')
