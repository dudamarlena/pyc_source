...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\gaeio\src\gui\cropsurvey.py
# Compiled at: 2020-04-25 14:58:02
# Size of source mod 2**32: 16436 bytes
...
class cropsurvey(object):
    ...
    def clickBtnApply(self):
        ...
        if self.checkSurvInfo() is False:
            vis_msg.print('ERROR in CropSurvey: No seismic survey found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Crop Survey', 'No seismic survey found')
            return
        ...
        if _inlstart is False or _xlstart is False or _zstart is False or _inlend is False or _xlend is False or _zend is False:
            vis_msg.print('ERROR in CropSurvey: Non-integer survey selection', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Crop Survey', 'Non-integer survey selection')
            return
        _survinfo = self.survinfo
        ...
        _survinfo = seis_ays.createSeisInfoFrom3DMat((np.transpose(_idx, [2, 1, 0])), inlstart=(_inlstart_idx * self.survinfo['ILStep'] + self.survinfo['ILStart']),
          inlstep=((self.cbbinlitvl.currentIndex() + 1) * self.survinfo['ILStep']),
          xlstart=(_xlstart_idx * self.survinfo['XLStep'] + self.survinfo['XLStart']),
          xlstep=((self.cbbxlitvl.currentIndex() + 1) * self.survinfo['XLStep']),
          zstart=(_zstart_idx * self.survinfo['ZStep'] + self.survinfo['ZStart']),
          zstep=((self.cbbzitvl.currentIndex() + 1) * self.survinfo['ZStep']))
        for i in range(len(_inlidx)):
            for j in range(len(_xlidx)):
                _idx[i, j, :] = _zidx + _xlidx[j] * self.survinfo['ZNum'] + _inlidx[i] * self.survinfo['XLNum'] * self.survinfo['ZNum']

        _idx = np.reshape(_idx, [len(_zidx) * len(_xlidx) * len(_inlidx)])
        for key in self.seisdata.keys():
            if self.checkSeisData(key):
                _dict = {}
                _dict[key] = np.reshape(np.transpose(self.seisdata[key], [2, 1, 0]), [-1, 1])
                self.seisdata[key] = np.transpose(np.reshape(basic_mdt.retrieveDictByIndex(_dict, _idx)[key], [
                 _survinfo['ILNum'],
                 _survinfo['XLNum'],
                 _survinfo['ZNum']]), [
                 2, 1, 0])
        ...
        QtWidgets.QMessageBox.information(self.msgbox, 'Crop Survey', 'Survey cropped successfully')
        self.dialog.close()
