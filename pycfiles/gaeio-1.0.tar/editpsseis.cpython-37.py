...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\cognitivegeo\src\gui\editpsseis.py
# Compiled at: 2020-03-24 12:38:17
...
import cognitivegeo.src.gui.viewpsseis as gui_viewpsseis
...
class editpsseis(object):
    ...
    def retranslateGUI(self, EditPsSeis):
        ...
        self.cbbaction.addItems(['Copy', 'Delete', 'Rename', 'Group shots', 'View', 'Gather-plot'])
        ...
        self.cbbaction.setItemIcon(3, QtGui.QIcon(os.path.join(self.iconpath, 'icons/merge.png')))
        self.cbbaction.setItemIcon(4, QtGui.QIcon(os.path.join(self.iconpath, 'icons/view.png')))
        self.cbbaction.setItemIcon(5, QtGui.QIcon(os.path.join(self.iconpath, 'icons/gather.png')))
    ...
    def changeCbbAction(self):
        if self.cbbaction.currentIndex() == 2 or self.cbbaction.currentIndex() == 3:
            ...
            self.ldtrename.setVisible(False)
        if self.cbbaction.currentIndex() == 1 or self.cbbaction.currentIndex() == 3:
            self.lwgpsseis.setSelectionMode(QtWidgets.QAbstractItemView.MultiSelection)
        else:
            self.lwgpsseis.setSelectionMode(QtWidgets.QAbstractItemView.SingleSelection)
    ...
    def clickBtnEditPsSeis(self):
        ...
        if len(_psseislist) < 1:
            vis_msg.print('ERROR in EditPsSeis: No pre-stack seismic selected for editing', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Edit Pre-stack Seismic', 'No pre-stack seismic selected for editing')
            return
        ...
        if self.cbbaction.currentIndex() == 1:
            for _i in range(len(_psseislist)):
                self.psseisdata.pop(_psseislist[_i].text())

        if self.cbbaction.currentIndex() == 2:
            if len(self.ldtrename.text()) < 1:
                vis_msg.print('ERROR in EditPsSeis: No name specified for rename', type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Edit Pre-stack Seismic', 'No name specified for rename')
                return
            if self.ldtrename.text() in self.psseisdata.keys():
                reply = QtWidgets.QMessageBox.question(self.msgbox, 'Edit Pre-stack Seismic', self.ldtrename.text() + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
        ...
        if self.cbbaction.currentIndex() == 3:
            if len(self.ldtrename.text()) < 1:
                vis_msg.print('ERROR in EditPsSeis: No name specified for grouped shots', type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Edit Pre-stack Seismic', 'No name specified for grouped shots')
                return
            if self.ldtrename.text() in self.psseisdata.keys():
                reply = QtWidgets.QMessageBox.question(self.msgbox, 'Edit Pre-stack Seismic', self.ldtrename.text() + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                if reply == QtWidgets.QMessageBox.No:
                    return
            self.psseisdata[self.ldtrename.text()] = {}
            _shot = 0
            for _i in range(len(_psseislist)):
                _data = self.psseisdata[_psseislist[_i].text()]
                for _j in range(len(_data.keys())):
                    self.psseisdata[self.ldtrename.text()][str(_shot)] = _data[list(_data.keys())[_j]]
                    _shot += 1
