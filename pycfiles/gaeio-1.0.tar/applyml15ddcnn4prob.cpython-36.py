...
class applyml15ddcnn4prob(object):
    ...
    def clickBtnApplyMl15DDcnn4Prob(self):
        self.refreshMsgBox()
        if self.checkSurvInfo() is False:
            vis_msg.print('ERROR in ApplyMl15DDcnn4Prob: No seismic survey available', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 1.5D-DCNN', 'No seismic survey available')
            return
        if ml_tfm.check15DDCNNModel(self.modelpath, self.modelname) is False:
            vis_msg.print('ERROR in ApplyMl15DDcnn4Prob: No pre-DCNN network found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 1.5D-DCNN', 'No pre-DCNN network found')
            return
        for f in self.modelinfo['feature_list']:
            if self.checkSeisData(f) is False:
                vis_msg.print(("ERROR in ApplyMl15DDcnn4Prob: Feature '%s' not found in seismic data" % f), type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 1.5D-DCNN', "Feature '" + f + "' not found in seismic data")
                return
        ...
        if _image_height is False or _image_width is False or _image_height_new is False or _image_width_new is False:
            vis_msg.print('ERROR in ApplyMl15DDcnn4Prob: Non-integer feature size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 1.5D-DCNN', 'Non-integer feature size')
            return
        if _image_height < 2 or _image_width < 2 or _image_height_new < 2 or _image_width_new < 2:
            vis_msg.print('ERROR in ApplyMl15DDcnn4Prob: Features are not 2D', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 1.5D-DCNN', 'Features are not 2D')
            return
        _batch = basic_data.str2int(self.ldtbatchsize.text())
        if _batch is False or _batch < 1:
            vis_msg.print('ERROR in ApplyMl15DDcnn4Prob: Non-positive batch size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 1.5D-DCNN', 'Non-positive batch size')
            return
        if len(self.ldtsave.text()) < 1:
            vis_msg.print('ERROR in ApplyMl15DDcnn4Prob: No prefix specified', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 1.5D-DCNN', 'No prefix specified')
            return
        if len(self.lwgtarget.selectedItems()) < 1:
            vis_msg.print('ERROR in ApplyMl15DDcnn4Prob: No target class specified', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 1.5D-DCNN', 'No target class specified')
            return
        ...
        for _class in _classlist:
            if self.ldtsave.text() + str(_class) in self.seisdata.keys():
                if self.checkSeisData(self.ldtsave.text() + str(_class)):
                    reply = QtWidgets.QMessageBox.question(self.msgbox, 'Apply 1.5D-DCNN', self.ldtsave.text() + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
        ...
        _pgsdlg.setWindowTitle('Apply 1.5D-DCNN')
        ...
        for i in range(_nloop):
            ...
            sys.stdout.write('\r>>> Apply 1.5D-DCNN, proceeding %.1f%% ' % (float(i) / float(_nloop) * 100.0))
            sys.stdout.flush()
            ...
            _rst = ml_dcnn15d.probabilityFrom15DDCNNSegmentor(_dict, imageheight=_image_height_new,
        ...
        QtWidgets.QMessageBox.information(self.msgbox, 'Apply 1.5D-DCNN', 'DCNN applied successfully')
