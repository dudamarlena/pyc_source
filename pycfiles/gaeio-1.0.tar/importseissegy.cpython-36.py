...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\gaeio\src\gui\importseissegy.py
# Compiled at: 2020-04-25 15:07:54
# Size of source mod 2**32: 15930 bytes
...
from gaeio.src.gui.viewsegytextualheader import viewsegytextualheader as gui_viewsegytextualheader
from gaeio.src.gui.viewsegybinaryheader import viewsegybinaryheader as gui_viewsegybinaryheader
from gaeio.src.gui.viewsegytraceheader import viewsegytraceheader as gui_viewsegytraceheader
...
class importseissegy(object):
    ...
    def clickBtnSegy(self):
        ...
        _file = _dialog.getOpenFileNames(None, 'Select Seismic SEG-Y(s)', (self.rootpath), filter='SEG-Y files (*.segy; *.sgy);;All files (*.*)')
        if len(_file[0]) > 0:
    ...
    def clickBtnTextualHeader(self):
        ...
        if _nfile <= 0:
            vis_msg.print('ERROR in ImportSeisSegy: No SEG-Y selected for viewing textual header', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import SEG-Y', 'No SEG-Y selected for viewing textual header')
            return
    ...
    def clickBtnBinaryHeader(self):
        ...
        _nfile = len(self.segylist)
        if _nfile <= 0:
            vis_msg.print('ERROR in ImportSeisSegy: No SEG-Y selected for viewing binary header', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import Seismic SEG-Y', 'No SEG-Y selected for viewing binary header')
            return
        _viewheader = QtWidgets.QDialog()
    ...
    def clickBtnTraceHeader(self):
        ...
        _nfile = len(self.segylist)
        if _nfile <= 0:
            vis_msg.print('ERROR in ImportSeisSegy: No SEG-Y selected for viewing trace header', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import Seismic SEG-Y', 'No SEG-Y selected for viewing trace header')
            return
        _viewheader = QtWidgets.QDialog()
    ...
    def clickBtnImport(self):
        ...
        _nfile = len(self.segylist)
        if _nfile <= 0:
            vis_msg.print('ERROR in ImportSeisSegy: No SEG-Y selected for import', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import Seismic SEG-Y', 'No SEG-Y selected for import')
            return
        _traceheaderformat = seis_io.defSegyTraceHeaderFormat(x_byte=(self.cbbx.currentIndex() + 1), y_byte=(self.cbby.currentIndex() + 1),
        ...
        _seisdata = {}
        print('Retrieve SEG-Y survey information from ' + os.path.basename(self.segylist[0]))
        _survinfo = seis_io.readSeisInfoFromSegy((self.segylist[0]), traceheaderformat=_traceheaderformat)
        ...
        for i in range(_nfile):
            ...
            print('ImportSeisSegy: Import %d of %d SEG-Y: %s' % (i + 1, _nfile, os.path.basename(_filename)))
            _segydata = seis_io.readSeis3DMatFromSegyWithInfo(_filename, seisinfo=_survinfo, traceheaderformat=_traceheaderformat,
              qpgsdlg=_pgsdlg)
            _segydata = _segydata
        ...
        for key in _seisdata.keys():
            if key in self.seisdata.keys():
                if checkSeisData(self.seisdata[key], self.survinfo):
                    reply = QtWidgets.QMessageBox.question(self.msgbox, 'Import Seismic SEG-Y', key + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                    if reply == QtWidgets.QMessageBox.No:
                        return
            self.seisdata[key] = _seisdata[key]

        QtWidgets.QMessageBox.information(self.msgbox, 'Import Seismic SEG-Y', str(_nfile) + ' SEG-Y imported successfully')
