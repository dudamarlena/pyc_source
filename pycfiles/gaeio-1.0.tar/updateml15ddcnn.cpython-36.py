...
from cognitivegeo.src.gui.viewml15ddcnn import viewml15ddcnn as gui_viewml15ddcnn
...
class updateml15ddcnn(object):
    ...
    def clickBtnUpdateMl15DDcnn(self):
        self.refreshMsgBox()
        if self.checkSurvInfo() is False:
            vis_msg.print('ERROR in UpdateMl15DDcnn: No seismic survey available', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 1.5D-DCNN', 'No seismic survey available')
            return
        if ml_tfm.check15DDCNNModel(self.modelpath, self.modelname) is False:
            vis_msg.print('ERROR in UpdateMl15DDcnn: No pre-DCNN network found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 1.5D-DCNN', 'No pre-DCNN network found')
            return
        for f in self.modelinfo['feature_list']:
            if self.checkSeisData(f) is False:
                vis_msg.print(("ERROR in UpdateMl15DDcnn: Feature '%s' not found in seismic data" % f), type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Update 1.5D-DCNN', "Feature '" + f + "' not found in seismic data")
                return
        ...
        if _image_height is False or _image_width is False or _image_height_new is False or _image_width_new is False:
            vis_msg.print('ERROR in UpdateMl15DDcnn: Non-integer feature size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 1.5D-DCNN', 'Non-integer feature size')
            return
        if _image_height < 2 or _image_width < 2 or _image_height_new < 2 or _image_width_new < 2:
            vis_msg.print('ERROR in UpdateMl15DDcnn: Features are not 2D', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 1.5D-DCNN', 'Features are not 2D')
            return
        ...
        if self.modelinfo['target'] not in self.seisdata.keys():
            vis_msg.print(("ERROR in UpdateMl15DDcnn: Target label '%s' not found in seismic data" % self.modelinfo['target']),
              type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 1.5D-DCNN', "Target label '" + self.modelinfo['target'] + ' not found in seismic data')
            return
        ...
        if _nepoch is False or _nepoch <= 0:
            vis_msg.print('ERROR in UpdateMl15DDcnn: Non-positive epoch number', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 1.5D-DCNN', 'Non-positive epoch number')
            return
        if _batchsize is False or _batchsize <= 0:
            vis_msg.print('ERROR in UpdateMl15DDcnn: Non-positive batch size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 1.5D-DCNN', 'Non-positive batch size')
            return
        if _learning_rate is False or _learning_rate <= 0:
            vis_msg.print('ERROR in UpdateMl15DDcnn: Non-positive learning rate', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 1.5D-DCNN', 'Non-positive learning rate')
            return
        if _dropout_prob is False or _dropout_prob <= 0:
            vis_msg.print('ERROR in UpdateMl15DDcnn: Negative dropout rate', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 1.5D-DCNN', 'Negative dropout rate')
            return
        if len(self.ldtsave.text()) < 1:
            vis_msg.print('ERROR in UpdateMl15DDcnn: No name specified for DCNN network', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 1.5D-DCNN', 'No name specified for DCNN network')
            return
        ...
        print('UpdateMl15DDcnn: Step 1 - Get training samples:')
        ...
        if np.shape(_traindata)[0] <= 0:
            vis_msg.print('ERROR in UpdateMl15DDcnn: No training sample found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-DCNN', 'No training sample found')
            return
        print('UpdateMl15DDcnn: Step 2 - Retrieve and interpolate images if necessary: (%d, %d) --> (%d, %d)' % (
        ...
        if self.traindataconfig['RemoveInvariantFeature_Checked']:
            for f in _features:
                ...
                if basic_mdt.maxDictConstantRow(_traindict) <= 0:
                    vis_msg.print('ERROR in UpdateMl15DDcnn: No training sample found', type='error')
                    QtWidgets.QMessageBox.critical(self.msgbox, 'Update 1.5D-DCNN', 'No training sample found')
                    return
        ...
        print('UpdateMl15DDcnn: A total of %d valid training samples' % basic_mdt.maxDictConstantRow(_traindict))
        print('UpdateMl15DDcnn: Step 3 - Start training')
        _pgsdlg = QtWidgets.QProgressDialog()
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(os.path.join(self.iconpath, 'icons/update.png')), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        _pgsdlg.setWindowIcon(icon)
        _pgsdlg.setWindowTitle('Update 1.5D-DCNN')
        _pgsdlg.setCancelButton(None)
        _pgsdlg.setWindowFlags(QtCore.Qt.WindowStaysOnTopHint)
        _pgsdlg.forceShow()
        _pgsdlg.setFixedWidth(400)
        _dccnlog = ml_dcnn15d.update15DDCNNSegmentor(_traindict, imageheight=_image_height_new,
          ...
          savename=_savename,
          qpgsdlg=_pgsdlg)
        QtWidgets.QMessageBox.information(self.msgbox, 'Update 1.5D-DCNN', 'DCNN updated successfully')
        reply = QtWidgets.QMessageBox.question(self.msgbox, 'Update 1.5D-DCNN', 'View learning matrix?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.Yes)
