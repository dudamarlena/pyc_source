...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\gaeio\src\gui\gui_main.py
# Compiled at: 2020-04-25 15:12:17
# Size of source mod 2**32: 231300 bytes
from PyQt5 import QtCore, QtGui, QtWidgets
import numpy as np, os, sys, shutil, webbrowser, vispy.io as vispy_io
...
class mainwindow(object):
    ...
    def setupGUI(self, MainWindow):
        ...
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(os.path.join(self.iconpath, 'icons/pydict.png')), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        ...
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(os.path.join(self.iconpath, 'icons/surface.png')), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        ...
        self.lblseisicon.setPixmap(QtGui.QPixmap(os.path.join(self.iconpath, 'icons/seismic.png')).scaled(20, 20, QtCore.Qt.KeepAspectRatio))
        ...
        self.lblhorizonicon.setPixmap(QtGui.QPixmap(os.path.join(self.iconpath, 'icons/surface.png')).scaled(20, 20, QtCore.Qt.KeepAspectRatio))
        ...
        self.lblpointseticon.setPixmap(QtGui.QPixmap(os.path.join(self.iconpath, 'icons/point.png')).scaled(20, 20, QtCore.Qt.KeepAspectRatio))
    ...
    def retranslateGUI(self, MainWindow):
        ...
        self.canvas = scene.SceneCanvas(keys='interactive', title='Canvas', bgcolor=[0.5, 0.5, 0.5], size=(1130,
                                                                                                           850),
          app='pyqt5',
        ...
        try:
            ...
            if 'survinfo' in _proj.keys():
                ...
                if os.path.exists(os.path.join(self.projpath, self.projname + '.proj.data/Survey/' + 'survey' + '.srv.npy')):
                    try:
                        _survinfo = np.load((os.path.join(self.projpath, self.projname + '.proj.data/Survey/' + 'survey' + '.srv.npy')),
                ...
                if 'survinfo' in _proj.keys():
                        ...
                        for key in _proj['psseisdata'].keys():
                            ...
                            for shot in _proj['psseisdata'][key].keys():
                                ...
                                if os.path.exists(os.path.join(self.projpath, self.projname + '.proj.data/PsSeismic/' + key + '_shot_' + shot + '.psseis.npy')):
                                    try:
                                        _psseisdata[shot] = np.load((os.path.join(self.projpath, self.projname + '.proj.data/PsSeismic/' + key + '_shot_' + shot + '.psseis.npy')),
                                          allow_pickle=True).item()
                                    except ValueError:
                                        _psseisdata[shot] = {}
                    ...
                    if 'horizondata' in _proj.keys():
                        ...
                        for _key in _proj['horizondata'].keys():
                            if os.path.exists(os.path.join(self.projpath, self.projname + '.proj.data/Horizon/' + _key + '.hrz.npy')):
                                try:
                                    _horizondata = np.load((os.path.join(self.projpath, self.projname + '.proj.data/Horizon/' + _key + '.hrz.npy')),
                                      allow_pickle=True).item()
                                except ValueError:
                                    _horizondata = {}
                    ...
                    if 'faultpatchdata' in _proj.keys():
                        ...
                        for _key in _proj['faultpatchdata'].keys():
                            if os.path.exists(os.path.join(self.projpath, self.projname + '.proj.data/FaultPatch/' + _key + '.ftp.npy')):
                                try:
                                    _faultdata = np.load((os.path.join(self.projpath, self.projname + '.proj.data/FaultPatch/' + _key + '.ftp.npy')),
                                      allow_pickle=True).item()
                                except ValueError:
                                    _faultdata = {}
                ...
                if 'welllogdata' in _proj.keys():
                    ...
                    for key in _proj['welllogdata'].keys():
                        if os.path.exists(os.path.join(self.projpath, self.projname + '.proj.data/WellLog/' + key + '.wlg.npy')):
                            try:
                                _welllogdata = np.load((os.path.join(self.projpath, self.projname + '.proj.data/WellLog/' + key + '.wlg.npy')),
                                  allow_pickle=True).item()
                            except ValueError:
                                _welllogdata = {}
            ...
            if 'settings' in _proj.keys():
                if os.path.exists(os.path.join(self.projpath, self.projname + '.proj.data/Settings/' + 'settings' + '.npy')):
                    try:
                        _settings = np.load((os.path.join(self.projpath, self.projname + '.proj.data/Settings/' + 'settings' + '.npy')),
                          allow_pickle=True).item()
                    except ValueError:
                        _settings = {}
    ...
    def doImportSeisNpyDictAll(self):
        self.refreshMsgBox()
        _dialog = QtWidgets.QFileDialog()
        _file = _dialog.getOpenFileName(None, 'Select Seismic NumPy Dictionary (all in one)', (self.settings['General']['RootPath']),
        ...
        try:
            ...
            if 'SeisInfo' not in _npydata.keys() or seis_ays.checkSeisInfo(_npydata['SeisInfo']) is False:
                vis_msg.print('ERROR in ImportSeisNpyDictAll: NumPy dictionary contains no information about seismic survey', type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Import Seismic NumPy Dictionary (all in one)', 'NumPy dictionary contains no information about seismic')
                return
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import Seismic NumPy Dictionary (all in one)', 'NumPy dictionary expected')
            return
        else:
            if _survinfo['ZStep'] >= 0:
                reply = QtWidgets.QMessageBox.question(self.msgbox, 'Import Seismic Numpy Dictionary (all in one)', 'Warning: positive z wii be reversed. Continue?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
            ...
            for key in _seisdata.keys():
                if key in self.seisdata.keys():
                    if checkSeisData(self.seisdata[key], self.survinfo):
                        reply = QtWidgets.QMessageBox.question(self.msgbox, 'Import Seismic NumPy Dictionary (all in one)', key + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                        if reply == QtWidgets.QMessageBox.No:
                            return
                self.seisdata[key] = _seisdata[key]

            QtWidgets.QMessageBox.information(self.msgbox, 'Import Seismic NumPy Dictionary (all in one)', 'NumPy dictionary imported successfully')
            self.updateSurveyVis()
            self.updateSeisVis()
    ...
    def doImportSeisNpyDictOne(self):
        self.refreshMsgBox()
        _dialog = QtWidgets.QFileDialog()
        _file = _dialog.getOpenFileNames(None, 'Select Seismic NumPy Dictionary (individual)', (self.settings['General']['RootPath']),
          filter='Seismic NumPy files (*.seis.npy);; All files (*.*)')
        if len(_file[0]) < 1:
        ...
        try:
            ...
            for _i in range(len(_file[0])):
                ...
                if 'SeisInfo' not in _npydata.keys() or seis_ays.checkSeisInfo(_npydata['SeisInfo']) is False:
                    vis_msg.print('ERROR in ImportSeisNpyDict: NumPy dictionary contains no information about seismic survey',
                      type='error')
                    QtWidgets.QMessageBox.critical(self.msgbox, 'Import Seismic NumPy Dictionary (individual)', 'NumPy dictionary contains no information about seismic')
                    return
                _survinfo = _npydata['SeisInfo']
                if 'SeisData' in _npydata.keys() and checkSeisData(_npydata['SeisData'], _survinfo):
                    _seisdata[_filename] = _npydata['SeisData']

        except ValueError:
            vis_msg.print('ERROR in ImportSeisNpyDict: NumPy dictionary expected', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import Seismic NumPy Dictionary (individual)', 'NumPy dictionary expected')
            return
        else:
            if _survinfo['ZStep'] >= 0:
                reply = QtWidgets.QMessageBox.question(self.msgbox, 'Import Seismic Numpy Dictionary (individual)', 'Warning: positive z wii be reversed. Continue?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                if reply == QtWidgets.QMessageBox.No:
                    return
                _survinfo['ZStart'] = -_survinfo['ZStart']
                _survinfo['ZStep'] = -_survinfo['ZStep']
            ...
            for key in _seisdata.keys():
                if key in self.seisdata.keys():
                    if checkSeisData(self.seisdata[key], self.survinfo):
                        reply = QtWidgets.QMessageBox.question(self.msgbox, 'Import Seismic NumPy Dictionary', key + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                        if reply == QtWidgets.QMessageBox.No:
                            return
                self.seisdata[key] = _seisdata[key]

            QtWidgets.QMessageBox.information(self.msgbox, 'Import Seismic NumPy Dictionary', 'NumPy dictionary imported successfully')
            self.updateSurveyVis()
            self.updateSeisVis()
    ...
    def doImportSeisNpyMat(self):
        self.refreshMsgBox()
        _dialog = QtWidgets.QFileDialog()
        _file = _dialog.getOpenFileNames(None, 'Select Seismic NumPy 2/3D Matrix', (self.settings['General']['RootPath']),
          filter='Seismic NumPy files (*.seis.npy);; All files (*.*)')
        if len(_file[0]) < 1:
        ...
        try:
            ...
            for _i in range(len(_file[0])):
                ...
                if np.ndim(_npydata) <= 1 or np.ndim(_npydata) >= 4:
                    ...
                    QtWidgets.QMessageBox.critical(self.msgbox, 'Import Seismic NumPy 2/3D Matrix', 'NumPy matrix shall be 2D or 3D')
                    return
                if np.ndim(_npydata) == 2:
                    if np.shape(_npydata)[1] < 4:
                        vis_msg.print('ERROR in ImportSeisNpyMat: 2D NumPy matrix shall contain at least 4 columns', type='error')
                        QtWidgets.QMessageBox.critical(self.msgbox, 'Import Seismic NumPy 2/3D Matrix', '2D NumPy matrix shall contain at least 4 columns')
                        return
                ...
                if np.ndim(_npydata) == 3:
                    if checkSurvInfo(self.survinfo):
                        if self.survinfo['ZNum'] == np.shape(_npydata)[0]:
                            if self.survinfo['XLNum'] == np.shape(_npydata)[1]:
                                if self.survinfo['ILNum'] == np.shape(_npydata)[2]:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import Seismic NumPy 2/3D Matrix', 'NumPy matrix expected')
            return
        else:
            if _survinfo['ZStep'] >= 0:
                reply = QtWidgets.QMessageBox.question(self.msgbox, 'Import Seismic Numpy 2/3D Matrix', 'Warning: positive z wii be reversed. Continue?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                if reply == QtWidgets.QMessageBox.No:
                    return
                _survinfo['ZStart'] = -_survinfo['ZStart']
                _survinfo['ZStep'] = -_survinfo['ZStep']
            ...
            for key in _seisdata.keys():
                if key in self.seisdata.keys():
                    if checkSeisData(self.seisdata[key], self.survinfo):
                        reply = QtWidgets.QMessageBox.question(self.msgbox, 'Import Seismic NumPy 2/3D Matrix', key + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                        if reply == QtWidgets.QMessageBox.No:
                            return
                self.seisdata[key] = _seisdata[key]

            QtWidgets.QMessageBox.information(self.msgbox, 'Import Seismic NumPy 2/3D Matrix', str(len(_file[0])) + ' NumPy matrix imported successfully')
            self.updateSurveyVis()
            self.updateSeisVis()
    ...
    def doImportPsSeisNpyDictAll(self):
        self.refreshMsgBox()
        _dialog = QtWidgets.QFileDialog()
        _file = _dialog.getOpenFileName(None, 'Select Pre-stack Seismic NumPy Dictionary (all in one)', (self.settings['General']['RootPath']), filter='Pre-stack Seismic NumPy files (*.psseis.npy);; All files (*.*)')
        if os.path.exists(_file[0]) is False:
        ...
        try:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import Pre-stack Seismic NumPy Dictionary (all in one)', 'NumPy dictionary expected')
            return
        else:
            ...
            QtWidgets.QMessageBox.information(self.msgbox, 'Import Pre-stack Seismic NumPy Dictionary (all in one)', 'NumPy dictionary imported successfully')

    def doImportPsSeisNpyDictOne(self):
        self.refreshMsgBox()
        _dialog = QtWidgets.QFileDialog()
        _file = _dialog.getOpenFileNames(None, 'Select Pre-stack Seismic NumPy Dictionary (individual)', (self.settings['General']['RootPath']),
        ...
        try:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import Pre-stack Seismic NumPy Dictionary (individual)', 'NumPy dictionary expected')
            return
        else:
            for key in _psseisdata.keys():
                if key in self.psseisdata.keys():
                    if checkPsSeisData(self.psseisdata[key]):
                        reply = QtWidgets.QMessageBox.question(self.msgbox, 'Import Pre-stack Seismic NumPy Dictionary (individual)', key + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                        if reply == QtWidgets.QMessageBox.No:
                            return
                self.psseisdata[key] = _psseisdata[key]

            QtWidgets.QMessageBox.information(self.msgbox, 'Import Pre-stack Seismic NumPy Dictionary (individual)', str(len(_file[0])) + ' NumPy dictionary imported successfully')

    def doImportPsSeisNpyMat(self):
        self.refreshMsgBox()
        _dialog = QtWidgets.QFileDialog()
        _file = _dialog.getOpenFileNames(None, 'Select Pre-stack Seismic NumPy 2/3D Matrix', (self.settings['General']['RootPath']),
          filter='Pre-stack Seismic NumPy files (*.psseis.npy);; All files (*.*)')
        if len(_file[0]) < 1:
        ...
        try:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import Pre-stack Seismic NumPy 2/3D Matrix', 'NumPy matrix expected')
            return
        else:
            for key in _psseisdata.keys():
                if key in self.psseisdata.keys():
                    if checkPsSeisData(self.psseisdata[key]):
                        reply = QtWidgets.QMessageBox.question(self.msgbox, 'Import Pre-stack Seismic NumPy 2/3D Matrix', key + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                        if reply == QtWidgets.QMessageBox.No:
                            return
                self.psseisdata[key] = _psseisdata[key]

            QtWidgets.QMessageBox.information(self.msgbox, 'Import Pre-stack Seismic NumPy 2/3D Matrix', str(len(_file[0])) + ' NumPy matrix imported successfully')
    ...
    def doImportHorizonNpyDictAll(self):
        self.refreshMsgBox()
        _dialog = QtWidgets.QFileDialog()
        _file = _dialog.getOpenFileName(None, 'Select Horizon NumPy Dictionary (all in one)', (self.settings['General']['RootPath']),
        ...
        try:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import HOrizon NumPy Dictionary (all in one)', 'NumPy dictionary expected')
            return
        else:
            for key in _horizondata.keys():
                if key in self.horizondata.keys() and checkHorizonData(_horizondata[key]):
                    reply = QtWidgets.QMessageBox.question(self.msgbox, 'Import Horizon NumPy', key + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                    if reply == QtWidgets.QMessageBox.No:
                        return
                self.horizondata[key] = _horizondata[key]

            QtWidgets.QMessageBox.information(self.msgbox, 'Import Horizon NumPy Dictionary (all in one)', 'NumPy dictionary (all in one) imported successfully')
            self.updateHorizonVis()

    def doImportHorizonNpyDictOne(self):
        self.refreshMsgBox()
        _dialog = QtWidgets.QFileDialog()
        _file = _dialog.getOpenFileNames(None, 'Select Horizon NumPy Dictionary (individual)', (self.settings['General']['RootPath']),
          filter='Horizon NumPy files (*.hrz.npy);; All files (*.*)')
        if len(_file[0]) < 1:
        ...
        try:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import Horizon NumPy Dictionary (individual)', 'NumPy dictionary expected')
            return
        else:
            for key in _horizondata.keys():
                if key in self.horizondata.keys():
                    if checkHorizonData(_horizondata[key]):
                        reply = QtWidgets.QMessageBox.question(self.msgbox, 'Import Horizon NumPy Dictionary (individual)', key + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                        if reply == QtWidgets.QMessageBox.No:
                            return
                self.horizondata[key] = _horizondata[key]

            QtWidgets.QMessageBox.information(self.msgbox, 'Import Horizon NumPy Dictionary', str(len(_file[0])) + ' NumPy dictionary imported successfully')
            self.updateHorizonVis()

    def doImportHorizonNpyMat(self):
        self.refreshMsgBox()
        _dialog = QtWidgets.QFileDialog()
        _file = _dialog.getOpenFileNames(None, 'Select Horizon NumPy 2D Matrix', (self.settings['General']['RootPath']),
          filter='Horizon NumPy files (*.hrz.npy);; All files (*.*)')
        if len(_file[0]) < 1:
        ...
        try:
            ...
            for _i in range(len(_file[0])):
                ...
                if np.ndim(_npydata) != 2:
                    ...
                    QtWidgets.QMessageBox.critical(self.msgbox, 'Import Horizon NumPy 2D matrix', 'NumPy matrix shall be 2D')
                    return
                _ncol = np.shape(_npydata)[1]
                if _ncol < 3:
                    vis_msg.print('ERROR in ImportHorizonNpyMat: 2D NumPy matrix shall contain at least 3 columns', type='error')
                    QtWidgets.QMessageBox.critical(self.msgbox, 'Import Horizon NumPy 2D matrix', '2D NumPy matrix shall contain at least 3 columns')
                    return
                _pointsetdata = {}
                ...
                _horizondata[_filename] = point_ays.convertPointSet2Horizon(pointset=_pointsetdata, property_list=(list(_pointsetdata.keys())))

        except ValueError:
            vis_msg.print('ERROR in ImportHorizonNpyMat: NumPy Matrix expected', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import Horizon NumPy 2D Matrix', 'NumPy matrix expected')
            return
        else:
            for key in _horizondata.keys():
                if key in self.horizondata.keys():
                    if checkHorizonData(_horizondata[key]):
                        reply = QtWidgets.QMessageBox.question(self.msgbox, 'Import Horizon NumPy 2D Matrix', key + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                        if reply == QtWidgets.QMessageBox.No:
                            return
                self.horizondata[key] = _horizondata[key]

            QtWidgets.QMessageBox.information(self.msgbox, 'Import Horizon NumPy 2D Matrix', str(len(_file[0])) + ' NumPy matrix imported successfully')
            self.updateHorizonVis()
    ...
    def doImportPointSetNpyDictAll(self):
        self.refreshMsgBox()
        _dialog = QtWidgets.QFileDialog()
        _file = _dialog.getOpenFileName(None, 'Select PointSet NumPy Dictionary (all in one)', (self.settings['General']['RootPath']), filter='PointSet NumPy files (*.pts.npy);; All files (*.*)')
        if os.path.exists(_file[0]) is False:
        ...
        try:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import PointSet NumPy Dictionary (all in one)', 'NumPy dictionary expected')
            return
        else:
            ...
            QtWidgets.QMessageBox.information(self.msgbox, 'Import PointSet NumPy Dictionary (all in one)', 'NumPy dictionary (all in one) imported successfully')
            self.updatePointSetVis()

    def doImportPointSetNpyDictOne(self):
        self.refreshMsgBox()
        _dialog = QtWidgets.QFileDialog()
        _file = _dialog.getOpenFileNames(None, 'Select PointSet NumPy Dictionary (individual)', (self.settings['General']['RootPath']),
        ...
        try:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import PointSet NumPy Dictionary (individual)', 'NumPy dictionary expected')
            return
        else:
            for key in _pointsetdata.keys():
                if key in self.pointsetdata.keys():
                    if checkPointSetData(_pointsetdata[key]):
                        reply = QtWidgets.QMessageBox.question(self.msgbox, 'Import PointSet NumPy Dictionary (individual)', key + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                        if reply == QtWidgets.QMessageBox.No:
                            return
                self.pointsetdata[key] = _pointsetdata[key]

            QtWidgets.QMessageBox.information(self.msgbox, 'Import PointSet NumPy Dictionary', str(len(_file[0])) + ' NumPy dictionary imported successfully')
            self.updatePointSetVis()

    def doImportPointSetNpyMat(self):
        self.refreshMsgBox()
        _dialog = QtWidgets.QFileDialog()
        _file = _dialog.getOpenFileNames(None, 'Select PointSet NumPy 2D Matrix', (self.settings['General']['RootPath']), filter='PointSet NumPy files (*.pts.npy);; All files (*.*)')
        if len(_file[0]) < 1:
        ...
        try:
            ...
            for _i in range(len(_file[0])):
                ...
                if np.ndim(_npydata) != 2:
                    ...
                    QtWidgets.QMessageBox.critical(self.msgbox, 'Import PointSet NumPy 2D matrix', 'NumPy matrix shall be 2D')
                    return
                _ncol = np.shape(_npydata)[1]
                if _ncol < 3:
                    vis_msg.print('ERROR in ImportPointSetNpyMat: 2D NumPy matrix shall contain at least 3 columns', type='error')
                    QtWidgets.QMessageBox.critical(self.msgbox, 'Import PointSet NumPy 2D matrix', '2D NumPy matrix shall contain at least 3 columns')
                    return
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import PointSet NumPy 2D Matrix', 'NumPy matrix expected')
            return
        else:
            for key in _pointsetdata.keys():
                if key in self.pointsetdata.keys():
                    if checkPointSetData(_pointsetdata[key]):
                        reply = QtWidgets.QMessageBox.question(self.msgbox, 'Import PointSet NumPy 2D Matrix', key + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                        if reply == QtWidgets.QMessageBox.No:
                            return
                self.pointsetdata[key] = _pointsetdata[key]

            QtWidgets.QMessageBox.information(self.msgbox, 'Import PointSet NumPy 2D Matrix', str(len(_file[0])) + ' NumPy matrix imported successfully')
            self.updatePointSetVis()
    ...
    def doQuit(self):
        self.refreshMsgBox()
        reply = QtWidgets.QMessageBox.question(self.msgbox, 'GAEIO', 'Are you sure to quit GAEIO?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
    ...
    def updateSeisVis(self):
        ...
        if type(self.seisdata) is dict and len(self.seisdata.keys()) > 0:
            ...
            if type(self.canvascomponents) is dict:
                if 'Seismic' in self.canvascomponents.keys():
                    if type(self.canvasproperties) is dict:
                        if 'Seismic' in self.canvasproperties.keys():
                            _idx_pop = []
                            for _idx in range(len(self.canvasproperties['Seismic'])):
                                if self.canvasproperties['Seismic'][_idx]['Name'] not in _all_available_seis:
                                    ...
                                    _visual = self.createVisualSeis(self.canvasproperties['Seismic'][_idx]['Name'], self.canvasproperties['Seismic'][_idx]['Orientation'], self.canvasproperties['Seismic'][_idx]['Number'])
                                if _visual is None:
                                    _idx_pop.append(_idx)
                                else:
        ...
        if type(self.horizondata) is dict and len(self.horizondata.keys()) > 0:
            for _hrz in sorted(self.horizondata.keys()):
                if checkHorizonData(self.horizondata[_hrz]):
                    ...
                    _config[_hrz] = {}
                    if type(self.horizonvisconfig) is dict:
                        if _hrz in self.horizonvisconfig.keys():
            ...
            if type(self.canvascomponents) is dict:
                if 'Horizon' in self.canvascomponents.keys():
                    if type(self.canvasproperties) is dict:
                        if 'Horizon' in self.canvasproperties.keys():
                            _idx_pop = []
                            for _idx in range(len(self.canvasproperties['Horizon'])):
                                if self.canvasproperties['Horizon'][_idx]['Name'] not in _all_available_horizon:
                                    ...
                                    _visual = self.createVisualHorizon(self.canvasproperties['Horizon'][_idx]['Name'], self.canvasproperties['Horizon'][_idx]['Property'])
                                if _visual is None:
                                    _idx_pop.append(_idx)
                                else:
    ...
    def updatePointSetVis(self):
        ...
        if type(self.pointsetdata) is dict:
            ...
            if type(self.canvascomponents) is dict:
                if 'PointSet' in self.canvascomponents.keys():
                    if type(self.canvasproperties) is dict:
                        if 'PointSet' in self.canvasproperties.keys():
                            _idx_pop = []
    ...
    def updateSurveyVis(self):
        if self.canvas is None:
            ...
            self.view = self.canvas.central_widget.add_view()
            self.view.camera = scene.TurntableCamera(elevation=30, azimuth=135)
        elif 'Survey_Box' in self.canvascomponents.keys():
            ...
            if 'Survey_Box' not in self.canvasproperties.keys():
                ...
                if len(self.canvascomponents['Survey_Box']) == 12:
                    ...
                    self.canvasproperties['Survey_Box'] = True
                if 'XYZ_Axis' in self.canvascomponents.keys():
    ...
    def clickBtnAddSeis2Canvas(self):
        ...
        if len(_all_seis) > 0 and checkSeisData(self.seisdata[_all_seis[self.cbblistseis.currentIndex()]], self.survinfo):
            _seis = {}
            _seis['Visible'] = False
    ...
    def clickBtnConfigHorizonVis(self):
        ...
        for _i in range(len(self.canvascomponents['Horizon'])):
            ...
            _vis = self.createVisualHorizon(self.canvasproperties['Horizon'][_i]['Name'], self.canvasproperties['Horizon'][_i]['Property'])
    ...
    def clickBtnAddHorizon2Canvas(self):
        ...
        if len(_all_horizon) > 0 and checkHorizonData(self.horizondata[_all_horizon[self.cbblisthorizon.currentIndex()]]):
            _horizon = {}
    ...
    def changeCbbPropHorizonOnCanvas(self, idx):
        if idx < len(self.canvasproperties['Horizon']):
            ...
            _all_property.insert(0, '---')
            self.canvasproperties['Horizon'][idx]['Property'] = _all_property[self.twghorizononcanvas.cellWidget(idx, 2).currentIndex()]
            self.canvascomponents['Horizon'][idx].parent = None
            _vis = self.createVisualHorizon(self.canvasproperties['Horizon'][idx]['Name'], self.canvasproperties['Horizon'][idx]['Property'])
    ...
    def clickBtnAddPointSet2Canvas(self):
        ...
        if len(_all_pointset) > 0 and checkPointSetData(self.pointsetdata[_all_pointset[self.cbblistpointset.currentIndex()]]):
            _point = {}
    ...
    def updateTwgPointSetOnCanvas(self):
        ...
        for _i in range(len(_point)):
            ...
            _item = QtWidgets.QComboBox()
            _item.addItems(_all_property)
            _item.setCurrentIndex(list.index(_all_property, _point[_i]['Property']))
            _item.setEnabled(False)
                    ...
                    if ort != 'Inline':
                        if ort != 'Crossline':
                            if ort != 'Z':
                                ...
                                if ort == 'Inline':
                                    if no not in _inlrange:
                                        return
                        else:
                            if ort == 'Crossline':
                                if no not in _xlrange:
                                    return
                            if ort == 'Z':
                                if no not in _zrange:
                                    return
                    else:
                        _data = np.zeros([2, 2])
    ...
    def createVisualHorizon(self, horizon, property):
        if type(self.horizondata) is not dict or horizon not in self.horizondata.keys():
            ...
            if checkHorizonData(self.horizondata[horizon]) is False:
                ...
                if property != '---':
                    if property not in self.horizondata[horizon]['HorizonData'].keys():
                        return
                if property != '---':
                    if property not in self.horizonvisconfig[horizon]['Property'].keys():
                        return
                ...
                _cmap = vis_cmap.makeColorMap(cmapname='Seismic', flip=False, opacity=(self.horizonvisconfig[horizon]['Opacity']),
                  return_array=True)
                ...
                if property == '---':
                    ...
                    _cmap = vis_cmap.makeColorMap(cmapname=(self.horizonvisconfig[horizon]['Property'][property]['Colormap']), flip=(self.horizonvisconfig[horizon]['Property'][property]['Flip']),
            ...
            _c = vis_cmap.applyColorMap(values=(np.reshape(_values, [-1])), cmapmat=_cmap, vmin=_vmin,
              vmax=_vmax)
...
class qt_mainwindow(QtWidgets.QMainWindow):
    ...
    def closeEvent(self, event):
        reply = QtWidgets.QMessageBox.question(self, 'GAEIO', 'Are you sure to quit GAEIO?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
        if reply == QtWidgets.QMessageBox.Yes:
            sys.exit()
        else:
            event.ignore()
...
def checkSettings(setting):
    if type(setting) is not dict or len(setting.keys()) < 1:
        ...
        if 'Gui' not in setting.keys() or 'General' not in setting.keys() or 'Visual' not in setting.keys() or 'Viewer' not in setting.keys():
            return False
        if core_set.checkSettings(gui=(setting['Gui']), general=(setting['General']), visual=(setting['Visual']),
          viewer=(setting['Viewer'])) is False:
            return False
        return True


def saveProject(survinfo={}, seisdata={}, psseisdata={}, horizondata={}, faultpatchdata={}, pointsetdata={}, welllogdata={}, settings={}, savepath='', savename='gpy'):
    _proj = {}
    ...
    for key in psseisdata.keys():
        ...
        for shot in psseisdata[key].keys():
            ...
            np.save(os.path.join(savepath, savename + '.proj.data/PsSeismic/' + key + '_shot_' + shot + '.psseis.npy'), psseisdata[key][shot])
