...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\gaeio\src\gui\exporthorizonnpy.py
# Compiled at: 2020-04-25 14:58:02
# Size of source mod 2**32: 10101 bytes
...
class exporthorizonnpy(object):
    ...
    def clickBtnSave(self):
        ...
        _file = _dialog.getSaveFileName(None, 'Select Horizon NumPy', (self.rootpath), filter='Horizon NumPy files (*.hrz.npy);; All files (*.*)')
        if len(_file[0]) > 0:
    ...
    def clickBtnExportHorionNpy(self):
        ...
        if len(_horizonlist) < 1:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Export Horizon NumPy', 'No horizon selected for export')
            return
        if len(self.ldtsave.text()) < 1:
            vis_msg.print('ERROR in ExportHorionNpy: No name specified for NumPy', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Export Horizon NumPy', 'No name specified for NumPy')
            return
        ...
        if len(_horizonlist) > 1 and self.cbbtype.currentIndex() >= 1:
            reply = QtWidgets.QMessageBox.question(self.msgbox, 'Export Horizon Numpy', 'Warning: For exporting >=2 horizon, property name used as file name. Continue?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
        ...
        if self.cbbtype.currentIndex() == 2:
            reply = QtWidgets.QMessageBox.question(self.msgbox, 'Export Horizon Numpy', 'Warning: property header not saved in numpy matrix. Continue?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
            if reply == QtWidgets.QMessageBox.No:
                return
            for i in range(len(_horizonlist)):
                _pointsetdata = horizon_ays.convertHorizon2PointSet(horizon=(self.horizondata[_horizonlist[i].text()]), property_list=(list(self.horizondata[_horizonlist[i].text()]['HorizonData'].keys())))
        ...
        QtWidgets.QMessageBox.information(self.msgbox, 'Export Horizon NumPy', str(len(_horizonlist)) + ' horizons exported as NumPy successfully')
