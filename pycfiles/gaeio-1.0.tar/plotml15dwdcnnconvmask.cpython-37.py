...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\cognitivegeo\src\gui\plotml15dwdcnnconvmask.py
...
class plotml15dwdcnnconvmask(gui_core):
    modelpath = ''
    modelname = ''
    ...
    def setupGUI(self, PlotMl15DWdcnnConvMask):
        gui_core.title = 'Plot 1.5D-WDCNN Conv. Mask'
        gui_core.icon = 'mask.png'
        gui_core.imagestyle = self.maskstyle
        gui_core.imagelist = self.getConvMaskList()
        gui_core.setupGUI(self, PlotMl15DWdcnnConvMask)

    def clickBtnApply(self):
        self.refreshMsgBox()
        if ml_tfm.check15DWDCNNModel(self.modelpath, self.modelname) is False:
            vis_msg.print('ERROR in PlotMl15DWdcnnConvMaks: No DCNN network found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Plot 1.5D-WDCNN Conv. Mask', 'No 1.5D-WDCNN network found')
            return
        ...
        ml_wdcnn15d.plot15DWDCNNConvMask(wdcnnpath=(self.modelpath), wdcnnname=(self.modelname), blockidx=_blockidx,
          layeridx=_layeridx,
          ncol=_ncol,
          cmapname=_colormap,
          flipcmap=_flipped,
...
if __name__ == '__main__':
    ...
    PlotMl15DWdcnnConvMask = QtWidgets.QWidget()
    gui = plotml15dwdcnnconvmask()
    gui.setupGUI(PlotMl15DWdcnnConvMask)
    PlotMl15DWdcnnConvMask.show()
    sys.exit(app.exec_())