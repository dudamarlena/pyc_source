...
class updateml2dwdcnn(object):
    ...
    def clickBtnUpdateMl2DWdcnn(self):
        self.refreshMsgBox()
        if self.checkSurvInfo() is False:
            vis_msg.print('ERROR in UpdateMl2DWdcnn: No seismic survey available', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WDCNN', 'No seismic survey available')
            return
        if ml_tfm.checkWDCNNModel(self.modelpath, self.modelname) is False:
            vis_msg.print('ERROR in UpdateMl2DWdcnn: No pre-WDCNN network found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WDCNN', 'No pre-WDCNN network found')
            return
        for f in self.modelinfo['feature_list']:
            if self.checkSeisData(f) is False:
                vis_msg.print(("ERROR in UpdateMl2DWdcnn: Feature '%s' not found in seismic data" % f), type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WDCNN', "Feature '" + f + "' not found in seismic data")
                return
        ...
        if _image_height is False or _image_width is False or _image_height_new is False or _image_width_new is False:
            vis_msg.print('ERROR in UpdateMl2DWdcnn: Non-integer feature size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WDCNN', 'Non-integer feature size')
            return
        if _image_height < 2 or _image_width < 2 or _image_height_new < 2 or _image_width_new < 2:
            vis_msg.print('ERROR in UpdateMl2DWdcnn: Features are not 2D', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WDCNN', 'Features are not 2D')
            return
        ...
        if self.modelinfo['target'] not in self.seisdata.keys():
            vis_msg.print(("ERROR in UpdateMl2DWdcnn: Target '%s' not found in seismic data" % self.modelinfo['target']),
              type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WDCNN', "Target '" + self.modelinfo['target'] + ' not found in seismic data')
            return
        if self.modelinfo['weight'] not in self.seisdata.keys():
            vis_msg.print(("ERROR in UpdateMl2DWdcnn: Weight '%s' not found in seismic data" % self.modelinfo['target']),
              type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WDCNN', "Weight '" + self.modelinfo['target'] + ' not found in seismic data')
            return
        ...
        if _nepoch is False or _nepoch <= 0:
            vis_msg.print('ERROR in UpdateMl2DWdcnn: Non-positive epoch number', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WDCNN', 'Non-positive epoch number')
            return
        if _batchsize is False or _batchsize <= 0:
            vis_msg.print('ERROR in UpdateMl2DWdcnn: Non-positive batch size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WDCNN', 'Non-positive batch size')
            return
        if _learning_rate is False or _learning_rate <= 0:
            vis_msg.print('ERROR in UpdateMl2DWdcnn: Non-positive learning rate', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WDCNN', 'Non-positive learning rate')
            return
        if _dropout_prob is False or _dropout_prob <= 0:
            vis_msg.print('ERROR in UpdateMl2DWdcnn: Negative dropout rate', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WDCNN', 'Negative dropout rate')
            return
        if len(self.ldtsave.text()) < 1:
            vis_msg.print('ERROR in UpdateMl2DWdcnn: No name specified for new-WDCNN', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WDCNN', 'No name specified for new-WDCNN')
            return
        ...
        print('UpdateMl2DWdcnn: Step 1 - Get training samples:')
        ...
        if np.shape(_traindata)[0] <= 0:
            vis_msg.print('ERROR in UpdateMl2DWdcnn: No training sample found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-WDCNN', 'No training sample found')
            return
        print('UpdateMl2DWdcnn: Step 2 - Retrieve and interpolate images if necessary: (%d, %d) --> (%d, %d)' % (
        ...
        print('UpdateMl2DWdcnn: A total of %d valid training samples' % basic_mdt.maxDictConstantRow(_traindict))
        print('UpdateMl2DWdcnn: Step 3 - Start training')
        _pgsdlg = QtWidgets.QProgressDialog()
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(os.path.join(self.iconpath, 'icons/update.png')), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        _pgsdlg.setWindowIcon(icon)
        _pgsdlg.setWindowTitle('Update 2D-WDCNN')
        _pgsdlg.setCancelButton(None)
        _pgsdlg.setWindowFlags(QtCore.Qt.WindowStaysOnTopHint)
        _pgsdlg.forceShow()
        _pgsdlg.setFixedWidth(400)
        _dccnlog = ml_wdcnn.updateWDCNNSegmentor(_traindict, imageheight=_image_height_new,
          ...
          savename=_savename,
          qpgsdlg=_pgsdlg)
        QtWidgets.QMessageBox.information(self.msgbox, 'Update 2D-WDCNN', 'WDCNN updated successfully')
        reply = QtWidgets.QMessageBox.question(self.msgbox, 'Update 2D-DCNN', 'View learning matrix?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.Yes)
