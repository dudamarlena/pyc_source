...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\gaeio\src\gui\confighorizonvis.py
    ...
    def retranslateGUI(self, ConfigHorizonVis):
        ...
        for hrz in sorted(self.horizonvisconfig.keys()):
            ...
            for _i in range(len(self.ColorList)):
                item.setItemIcon(_i, QtGui.QIcon(QtGui.QPixmap(os.path.join(self.iconpath, 'icons/color_' + self.ColorList[_i] + '.png')).scaled(80, 40)))

            item.setCurrentIndex(list.index(self.ColorList, self.horizonvisconfig[hrz]['Color']))
            self.twghorizon.setCellWidget(_idx, 1, item)
            item = QtWidgets.QComboBox()
            item.addItems(self.OpacityList)
            item.setCurrentIndex(list.index(self.OpacityList, self.horizonvisconfig[hrz]['Opacity']))
        ...
        for _i in range(len(_all_property)):
            ...
            for _i in range(len(self.ColorMapList)):
                item.setItemIcon(_i, QtGui.QIcon(QtGui.QPixmap(os.path.join(self.iconpath, 'icons/cmap_' + self.ColorMapList[_i] + '.png')).scaled(80, 40)))

            item.setCurrentIndex(list.index(self.ColorMapList, self.horizonvisconfig[_hrz]['Property'][_prop]['Colormap']))
