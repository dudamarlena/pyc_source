...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\cognitivegeo\src\gui\plotml2dwdcnnconvmask.py
...
class plotml2dwdcnnconvmask(gui_core):
    modelpath = ''
    modelname = ''
    ...
    def setupGUI(self, PlotMl2DWdcnnConvMask):
        gui_core.title = 'Plot 2D-WDCNN Conv. Mask'
        gui_core.icon = 'mask.png'
        gui_core.imagestyle = self.maskstyle
        gui_core.imagelist = self.getConvMaskList()
        gui_core.setupGUI(self, PlotMl2DWdcnnConvMask)

    def clickBtnApply(self):
        self.refreshMsgBox()
        if ml_tfm.checkWDCNNModel(self.modelpath, self.modelname) is False:
            vis_msg.print('ERROR in PlotMl2DWdcnnCOnvMask: No WDCNN network found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Plot 2D-WDCNN Conv. Mask', 'No WDCNN network found')
            return
        ...
        ml_wdcnn.plotWDCNNConvMask(wdcnnpath=(self.modelpath), wdcnnname=(self.modelname), blockidx=_blockidx,
          layeridx=_layeridx,
          ncol=_ncol,
          cmapname=_colormap,
          flipcmap=_flipped,
...
if __name__ == '__main__':
    ...
    PlotMl2DWdcnnConvMask = QtWidgets.QWidget()
    gui = plotml2dwdcnnconvmask()
    gui.setupGUI(PlotMl2DWdcnnConvMask)
    PlotMl2DWdcnnConvMask.show()
    sys.exit(app.exec_())