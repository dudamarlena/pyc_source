...
class applyml2ddcnn4pred(object):
    ...
    def clickBtnApplyMl2DDcnn4Pred(self):
        self.refreshMsgBox()
        if self.checkSurvInfo() is False:
            vis_msg.print('ERROR in ApplyMl2DDcnn4Pred: No seismic survey available', type='error')
        ...
        if ml_tfm.checkDCNNModel(self.modelpath, self.modelname) is False:
            vis_msg.print('ERROR in ApplyMl2DDcnn4Pred: No pre-DCNN network found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 2D-DCNN', 'No pre-DCNN network found')
            return
        for f in self.modelinfo['feature_list']:
            if self.checkSeisData(f) is False:
                vis_msg.print(("ERROR in ApplyMl2DDcnn4Pred: Feature '%s' not found in seismic data" % f), type='error')
        ...
        if _image_height is False or _image_width is False or _image_height_new is False or _image_width_new is False:
            vis_msg.print('ERROR in ApplyMl2DDcnn4Pred: Non-integer feature size', type='error')
        ...
        if _image_height < 2 or _image_width < 2 or _image_height_new < 2 or _image_width_new < 2:
            vis_msg.print('ERROR in ApplyMl2DDcnn4Pred: Features are not 2D', type='error')
        ...
        if _batch is False or _batch < 1:
            vis_msg.print('ERROR in ApplyMl2DDcnn4Pred: Non-positive batch size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 2D-DCNN', 'Non-positive batch size')
            return
        if len(self.ldtsave.text()) < 1:
            vis_msg.print('ERROR in ApplyMl2DDcnn4Pred: No name specified', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 2D-DCNN', 'No name specified')
            return
        ...
        for i in range(_nloop):
            ...
            _rst = ml_dcnn.predictionFromDCNNSegmentor(_dict, imageheight=_image_height_new,
