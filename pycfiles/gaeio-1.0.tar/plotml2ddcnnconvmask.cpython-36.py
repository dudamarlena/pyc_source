...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\cognitivegeo\src\gui\plotml2ddcnnconvmask.py
...
class plotml2ddcnnconvmask(gui_core):
    modelpath = ''
    modelname = ''
    ...
    def setupGUI(self, PlotMl2DDcnnConvMask):
        gui_core.title = 'Plot 2D-DCNN Conv. Mask'
        gui_core.icon = 'mask.png'
        gui_core.imagestyle = self.maskstyle
        gui_core.imagelist = self.getConvMaskList()
        gui_core.setupGUI(self, PlotMl2DDcnnConvMask)

    def clickBtnApply(self):
        self.refreshMsgBox()
        if ml_tfm.checkDCNNModel(self.modelpath, self.modelname) is False:
            vis_msg.print('ERROR in PlotMl2DDcnnConvMask: No DCNN network found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Plot 2D-DCNN Conv. Mask', 'No DCNN network found')
            return
        ...
        ml_dcnn.plotDCNNConvMask(dcnnpath=(self.modelpath), dcnnname=(self.modelname), blockidx=_blockidx,
          layeridx=_layeridx,
          ncol=_ncol,
          cmapname=_colormap,
          flipcmap=_flipped,
...
if __name__ == '__main__':
    ...
    PlotMl2DDcnnConvMask = QtWidgets.QWidget()
    gui = plotml2ddcnnconvmask()
    gui.setupGUI(PlotMl2DDcnnConvMask)
    PlotMl2DDcnnConvMask.show()
    sys.exit(app.exec_())