...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\cognitivegeo\src\gui\plotvis1dseisz.py
# Compiled at: 2020-03-18 15:05:46
# Size of source mod 2**32: 18128 bytes
...
class plotvis1dseisz(object):
    ...
    def changeLdtInl(self):
        if len(self.ldtinl.text()) > 0:
            ...
            if _val >= self.slbinl.minimum():
                if _val <= self.slbinl.maximum():
    ...
    def changeLdtXl(self):
        if len(self.ldtxl.text()) > 0:
            ...
            if _val >= self.slbxl.minimum():
                if _val <= self.slbxl.maximum():
    ...
    def clickBtnPlot(self):
        ...
        if len(_attriblist) < 1:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, '1D Window: Seismic Waveform', 'No property selected for plot')
            return
        ...
        if _min is False or _max is False:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, '1D Window: Seismic Waveform', 'Non-float range specified for plot')
            return
        for i in range(len(_attriblist)):
            ...
            seis_vis.plotSeisZTracePlayerFrom3DMat(_data, initinltc=_inls, initxltc=_xls, seisinfo=(self.survinfo), titlesurf=(_attriblist[i].text() + ' at '),
              valuemin=_min,
              valuemax=_max,
              color=_color,
