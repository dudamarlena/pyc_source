...
class updateml2ddcnn(object):
    ...
    def clickBtnUpdateMl2DDcnn(self):
        self.refreshMsgBox()
        if self.checkSurvInfo() is False:
            vis_msg.print('ERROR in UpdateMl2DDcnn: No seismic survey available', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-DCNN', 'No seismic survey available')
            return
        if ml_tfm.checkDCNNModel(self.modelpath, self.modelname) is False:
            vis_msg.print('ERROR in UpdateMl2DDcnn: No pre-DCNN network found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-DCNN', 'No pre-DCNN network found')
            return
        for f in self.modelinfo['feature_list']:
            if self.checkSeisData(f) is False:
                vis_msg.print(("ERRRO in UpdateMl2DDcnn: Feature '%s' not found in seismic data" % f), type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-DCNN', "Feature '" + f + "' not found in seismic data")
                return
        ...
        if _image_height is False or _image_width is False or _image_height_new is False or _image_width_new is False:
            vis_msg.print('ERROR in UpdateMl2DDcnn: Non-integer feature size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-DCNN', 'Non-integer feature size')
            return
        if _image_height < 2 or _image_width < 2 or _image_height_new < 2 or _image_width_new < 2:
            vis_msg.print('ERROR in UpdateMl2DDcnn: Features are not 2D', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-DCNN', 'Features are not 2D')
            return
        ...
        if self.modelinfo['target'] not in self.seisdata.keys():
            vis_msg.print(("ERROR in UpdateMl2DDcnn: Target label '%s' not found in seismic data" % self.modelinfo['target']),
              type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-DCNN', "Target label '" + self.modelinfo['target'] + ' not found in seismic data')
            return
        ...
        if _nepoch is False or _nepoch <= 0:
            vis_msg.print('ERROR in UpdateMl2DDcnn: Non-positive epoch number', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-DCNN', 'Non-positive epoch number')
            return
        if _batchsize is False or _batchsize <= 0:
            vis_msg.print('ERROR in UpdateMl2DDcnn: Non-positive batch size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-DCNN', 'Non-positive batch size')
            return
        if _learning_rate is False or _learning_rate <= 0:
            vis_msg.print('ERROR in UpdateMl2DDcnn: Non-positive learning rate', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-DCNN', 'Non-positive learning rate')
            return
        if _dropout_prob is False or _dropout_prob <= 0:
            vis_msg.print('ERROR in UpdateMl2DDcnn: Negative dropout rate', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-DCNN', 'Negative dropout rate')
            return
        if len(self.ldtsave.text()) < 1:
            vis_msg.print('ERROR in UpdateMl2DDcnn: No name specified for new-DCNN', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-DCNN', 'No name specified for new-DCNN')
            return
        ...
        print('UpdateMl2DDcnn: Step 1 - Get training samples:')
        ...
        if np.shape(_traindata)[0] <= 0:
            vis_msg.print('ERROR in UpdateMl2DDcnn: No training sample found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Update 2D-DCNN', 'No training sample found')
            return
        print('UpdateMl2DDcnn: Step 2 - Retrieve and interpolate images if necessary: (%d, %d) --> (%d, %d)' % (
        ...
        print('UpdateMl2DDcnn: A total of %d valid training samples' % basic_mdt.maxDictConstantRow(_traindict))
        print('UpdateMl2DDcnn: Step 3 - Start training')
        ...
        _dccnlog = ml_dcnn.updateDCNNSegmentor(_traindict, imageheight=_image_height_new,
          ...
          savename=_savename,
          qpgsdlg=_pgsdlg)
        QtWidgets.QMessageBox.information(self.msgbox, 'Update 2D-DCNN', 'DCNN updated successfully')
