...
class trainml15dwdcnnfromscratch(object):
    ...
    def clickBtnTrainMl15DWdcnnFromScratch(self):
        self.refreshMsgBox()
        if len(self.lwgfeature.selectedItems()) < 1:
            vis_msg.print('ERROR in TrainMl15DWdcnnFromScratch: No feature selected for training', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 1.5D-WDCNN', 'No feature selected for training')
            return
        ...
        if _image_height is False or _image_width is False or _image_height_new is False or _image_width_new is False:
            vis_msg.print('ERROR in TrainMl15DWdcnnFromScratch: Non-integer feature size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 1.5D-WDCNN', 'Non-integer feature size')
            return
        if _image_height < 2 or _image_width < 2 or _image_height_new < 2 or _image_width_new < 2:
            vis_msg.print('ERROR in TrainMl15DWdcnnFromScratch: Features are not 2D', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 1.5D-WDCNN', 'Features are not 2D')
            return
        ...
        if _target in _features:
            vis_msg.print('ERROR in TrainMl15DWdcnnFromScratch: Target also used as features', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 1.5D-WDCNN', 'Target also used as features')
            return
        if _weight in _features:
            vis_msg.print('ERROR in TrainMl15DWdcnnFromScratch: Weight also used as features', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 1.5D-WDCNN', 'Weight also used as features')
            return
        ...
        if _nconvblock is False or _nconvblock <= 0:
            vis_msg.print('ERROR in TrainMl15DWdcnnFromScratch: Non-positive convolutional block number', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 1.5D-WDCNN', 'Non-positive convolutional block number')
            return
        for _i in _nconvlayer:
            if _i is False or _i < 1:
                vis_msg.print('ERROR in TrainMl15DWdcnnFromScratch: Non-positive convolutional layer number', type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Train 1.5D-WDCNN', 'Non-positive convolutional layer number')
                return

        for _i in _nconvfeature:
            if _i is False or _i < 1:
                vis_msg.print('ERROR in TrainMl15DWdcnnFromScratch: Non-positive convolutional feature number', type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Train 1.5D-WDCNN', 'Non-positive convolutional feature number')
                return

        if _n1x1layer is False or _n1x1layer <= 0:
            vis_msg.print('ERROR in TrainMl15DWdcnnFromScratch: Non-positive 1x1 convolutional layer number', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 1.5D-WDCNN', 'Non-positive 1x1 convolutional layer number')
            return
        for _i in _n1x1feature:
            if _i is False or _i < 1:
                vis_msg.print('ERROR in TrainMl15DWdcnnFromScratch: Non-positive 1x1 convolutional feature number', type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Train 1.5D-WDCNN', 'Non-positive 1x1 convolutional feature number')
                return

        if _patch_height is False or _patch_width is False or _patch_height < 1 or _patch_width < 1:
            vis_msg.print('ERROR in TrainMl15DWdcnnFromScratch: Non-positive convolutional patch size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 1.5D-WDCNN', 'Non-positive convolutional patch size')
            return
        if _pool_height is False or _pool_width is False or _pool_height < 1 or _pool_width < 1:
            vis_msg.print('ERROR in TrainMl15DWdcnnFromScratch: Non-positive pooling size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 1.5D-WDCNN', 'Non-positive pooling size')
            return
        if _nepoch is False or _nepoch <= 0:
            vis_msg.print('ERROR in TrainMl15DWdcnnFromScratch: Non-positive epoch number', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 1.5D-WDCNN', 'Non-positive epoch number')
            return
        if _batchsize is False or _batchsize <= 0:
            vis_msg.print('ERROR in TrainMl15DWdcnnFromScratch: Non-positive batch size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 1.5D-WDCNN', 'Non-positive batch size')
            return
        if _learning_rate is False or _learning_rate <= 0:
            vis_msg.print('ERROR in TrainMl15DWdcnnFromScratch: Non-positive learning rate', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 1.5D-WDCNN', 'Non-positive learning rate')
            return
        if _dropout_prob is False or _dropout_prob <= 0:
            vis_msg.print('ERROR in TrainMl15DWdcnnFromScratch: Negative dropout rate', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 1.5D-WDCNN', 'Negative dropout rate')
            return
        if len(self.ldtsave.text()) < 1:
            vis_msg.print('ERROR in TrainMl15DWdcnnFromScratch: No name specified for WDCNN network', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 1.5D-WDCNN', 'No name specified for WDCNN network')
            return
        ...
        print('TrainMl15DWdcnnFromScratch: Step 1 - Get training samples:')
        ...
        if np.shape(_traindata)[0] <= 0:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Train 1.5D-WDCNN', 'No training sample found')
            return
        print('TrainMl15DWdcnnFromScratch: Step 2 - Retrieve and interpolate images if necessary: (%d, %d) --> (%d, %d)' % (
        ...
        if self.traindataconfig['RemoveInvariantFeature_Checked']:
            for f in _features:
                ...
                if basic_mdt.maxDictConstantRow(_traindict) <= 0:
                    vis_msg.print('ERROR in TrainMl15DWdcnnFromScratch: No training sample found', type='error')
                    QtWidgets.QMessageBox.critical(self.msgbox, 'Train 1.5D-WDCNN', 'No training sample found')
                    return
        ...
        print('TrainMl15DWdcnnFromScratch: A total of %d valid training samples' % basic_mdt.maxDictConstantRow(_traindict))
        print('TrainMl15DWdcnnFromScratch: Step 3 - Start training')
        _pgsdlg = QtWidgets.QProgressDialog()
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(os.path.join(self.iconpath, 'icons/new.png')), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        _pgsdlg.setWindowIcon(icon)
        _pgsdlg.setWindowTitle('Train 1.5D-WDCNN')
        _pgsdlg.setCancelButton(None)
        _pgsdlg.setWindowFlags(QtCore.Qt.WindowStaysOnTopHint)
        _pgsdlg.forceShow()
        _pgsdlg.setFixedWidth(400)
        _dcnnlog = ml_wdcnn15d.create15DWDCNNSegmentor(_traindict, imageheight=_image_height_new,
          ...
          savename=_savename,
          qpgsdlg=_pgsdlg)
        QtWidgets.QMessageBox.information(self.msgbox, 'Train 1.5D-WDCNN', 'WDCNN trained successfully')
        reply = QtWidgets.QMessageBox.question(self.msgbox, 'Train 1.5D-WDCNN', 'View learning matrix?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.Yes)
