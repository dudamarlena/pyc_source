...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\cognitivegeo\src\gui\rtrvseisprop.py
# Compiled at: 2020-03-26 14:03:18
# Size of source mod 2**32: 14023 bytes
...
class rtrvseisprop(object):
    ...
    def changeLdtFrom(self):
        ...
        try:
            ...
            if 'SeisInfo' not in self.npydata.keys() or seis_ays.checkSeisInfo(self.npydata['SeisInfo']) is False:
                vis_msg.print('ERROR in RtrvSeisProp: NumPy dictionary contains no information about seismic survey', type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Retrieve Seismic Property', 'NumPy dictionary contains no inforamtion about seismic')
                return
            ...
            if np.ndim(_npydata) <= 1 or np.ndim(_npydata) >= 4:
                ...
                QtWidgets.QMessageBox.critical(self.msgbox, 'Retrieve Seismic Property', 'NumPy matrix shall be 2D or 3D')
                return
            if np.ndim(_npydata) == 2:
                if np.shape(_npydata)[1] < 4:
                    vis_msg.print('ERROR in RtrvSeisProp: 2D NumPy matrix shall contain at least 4 columns', type='error')
                    QtWidgets.QMessageBox.critical(self.msgbox, 'Retrieve Seismic Property', '2D NumPy matrix shall contain at least 4 columns')
                    return
        ...
        if seis_ays.checkSeisInfo(self.npyinfo) is False:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Retrieve Seismic Property', 'Selected not seismic numpy')
            return
        _firstattrib = None
    ...
    def clickBtnFrom(self):
        ...
        _file = _dialog.getOpenFileName(None, 'Select Seismic NumPy', (self.rootpath), filter='NumPy files (*.npy);; All files (*.*)')
        if len(_file[0]) > 0:
    ...
    def clickBtnRtrvSeisProp(self):
        ...
        if len(_attriblist) < 1:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Retrieve Seismic Property', 'No property selected for export')
            return
        if self.checkPointSetData() is False:
            vis_msg.print('ERROR in RtrvSeisProp: No point loaded for retrieval', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Retrieve Seismic Property', 'No point loaded for retrieval')
            return
        ...
        if _wdinl is False or _wdxl is False or _wdz is False:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Retrieve Seismic Property', 'Non-integer retrieval window')
            return
        if _wdinl < 0 or _wdxl < 0 or _wdz < 0:
            vis_msg.print('ERROR in RtrvSeisProp: Non-positive retrieval window', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Retrieve Seismic Property', 'Non-positive retrieval window')
            return
        ...
        QtWidgets.QMessageBox.information(self.msgbox, 'Retrieve Seismic Property', str(len(_attriblist)) + ' properties retrieved successfully')
