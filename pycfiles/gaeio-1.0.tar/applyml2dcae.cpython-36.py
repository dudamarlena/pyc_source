...
class applyml2dcae(object):
    ...
    def clickBtnApplyMl2DCae(self):
        self.refreshMsgBox()
        if self.checkSurvInfo() is False:
            vis_msg.print('ERROR in ApplyMl2DCae: No seismic survey available', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 2D-CAE', 'No seismic survey available')
            return
        if ml_tfm.checkCAEModel(self.modelpath, self.modelname) is False:
            vis_msg.print('ERROR in ApplyMl2DCae: No pre-CAE network found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 2D-CAE', 'No pre-CAE network found')
            return
        for f in self.modelinfo['feature_list']:
            if self.checkSeisData(f) is False:
                vis_msg.print(("ERROR in ApplyMl2DCae: Feature '%s' not found in seismic data" % f), type='error')
                QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 2D-CAE', "Feature '" + f + "' not found in seismic data")
                return
        ...
        if _image_height is False or _image_width is False or _image_height_new is False or _image_width_new is False:
            vis_msg.print('ERROR in ApplyMl2DCae: Non-integer feature size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 2D-CAE', 'Non-integer feature size')
            return
        if _image_height < 2 or _image_width < 2 or _image_height_new < 2 or _image_width_new < 2:
            vis_msg.print('ERROR in ApplyMl2DCae: Features are not 2D', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 2D-CAE', 'Features are not 2D')
            return
        _batch = basic_data.str2int(self.ldtbatchsize.text())
        if _batch is False or _batch < 1:
            vis_msg.print('ERROR in ApplyMl2DCae: Non-positive batch size', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 2D-CAE', 'Non-positive batch size')
            return
        if len(self.ldtsave.text()) < 1:
            vis_msg.print('ERROR in ApplyMl2DCae: No name specified', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Apply 2D-CAE', 'No name specified')
            return
        if self.ldtsave.text() in self.seisdata.keys():
            if self.checkSeisData(self.ldtsave.text()):
                reply = QtWidgets.QMessageBox.question(self.msgbox, 'Apply 2D-CAE', self.ldtsave.text() + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
        ...
        _pgsdlg.setWindowTitle('Apply 2D-CAE')
        ...
        for i in range(_nloop):
            ...
            sys.stdout.write('\r>>> Apply 2D-CAE, proceeding %.1f%% ' % (float(i) / float(_nloop) * 100.0))
            sys.stdout.flush()
            ...
            _rst = ml_cae.reconstructFromCAEReconstructor(_dict, imageheight=_image_height_new,
        ...
        QtWidgets.QMessageBox.information(self.msgbox, 'Apply 2D-CAE', 'CAE applied successfully')
