...
# [GCC 8.4.0]
# Embedded file name: C:\Users\HDi\Google Drive\ProgramCodes\PythonCodes\gaeio\src\vis\video.py
# Compiled at: 2020-04-25 14:33:31
# Size of source mod 2**32: 9573 bytes
import sys, os, numpy as np, matplotlib.pyplot as plt, matplotlib.animation as animation
sys.path.append(os.path.dirname(__file__)[:-4][:-4][:-6])
from gaeio.src.vis.colormap import colormap as vis_cmap
__all__ = [
 'video']

def animateVideo(video, video_height, video_width, video_depth, frame_list=[], cmapname='Red-White-Blue', flipcmap=False, fontsize=12, savepath='./', savename='video.mp4', verbose=True):
    """
    Generate animation of a single video (by matplotlib.animation)
    ...
    def animate(i):
        ...
        ims = []
        im = plt.imshow((video[:, :, frame_list[i]]), interpolation='bicubic',
          cmap=(vis_cmap.makeColorMap(cmapname, flipcmap)))
        plt.axis('off')
        ims.append(im)
        return ims

    anim = animation.FuncAnimation(fig, animate, frames=(len(frame_list)), interval=500, blit=True,
      init_func=None)
...
def animateVideoGallery(video, video_height, video_width, video_depth, frame_list=[], ncol=5, cmapname='red_white_blue', flipcmap=False, spacing=False, maxfigsize=10, fontsize=12, savepath='./', savename='video.mp4', verbose=True):
    """
    Generate animation of multiple videos (by matplotlib.animation)
    ...
    def init():
        ...
        for k in range(nrow * ncol):
            ...
            im = plt.imshow((np.zeros([video_height, video_width])), interpolation='bicubic',
              cmap=(vis_cmap.makeColorMap(cmapname, flipcmap)))
            plt.axis('off')
            ims.append(im)
    ...
    def animate(i):
        ...
        for k in range(nrow * ncol):
            plt.subplot(nrow, ncol, k + 1)
            im = plt.imshow((video[k, :, :, frame_list[i]]), interpolation='bicubic',
              cmap=(vis_cmap.makeColorMap(cmapname, flipcmap)))
            plt.axis('off')
            ims.append(im)
