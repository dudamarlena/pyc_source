...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/print_fortran_routines/__init__.py
# Compiled at: 2020-03-27 13:02:48
# Size of source mod 2**32: 17793 bytes
import glob
from collections import Counter
from pathlib import Path
import subprocess, filecmp

def modify_fortran_file(input_file, output_file=None, ignore_functions=[], write=True):
    """
    Input:
    ...
    fortran_types = ('real', 'type', 'use', 'comment', 'integer', 'logical', '!', ' ',
                     'include', '9', 'character', 'implicit', 'interface', 'procedure',
                     'optional', 'double', 'save', 'equivalence', 'select case',
                     'complex', 'data', '*', '>', 'import', 'class', 'parameter')
    ...
    def read_initial_file(filename):
        ...
        f_nospaces = [l.strip() for l in original_file]
        return (
         original_file, f_nospaces, f_short)
    ...
    def get_subroutine_data(f_nospaces, f_short):
        ...
        is_subroutine = [True if 'subroutine' in l else False for l in first_lines]
        last_lines = [get_lastline(l, sub) for l, sub in zip(first_lines, is_subroutine)]
        routines = [l.split('(', 1)[0] for l in first_lines]
        start_texts = [' '.join(['      write(*,*)', "'", 'start --', r, '--', f_short, "'\n"]) for r in routines]
        finish_texts = [' '.join(['      write(*,*)', "'", 'finsh --', r, '--', f_short, "'\n"]) for r in routines]
        return [
         first_lines, last_lines, start_texts, finish_texts]
    ...
    def correct_startline(file_contents, start_index, end_index):
        ...
        ends_and = [True] + ['&' in l or  for l in routine][:-1]
        is_blank = [True if y == '' else False for y in routine]
        incase = [True if (any((x.startswith('select case') for x in routine[:i])) and any((x.startswith('end select') for x in routine[i:]))) else False for i, val in enumerate(routine)]
        is_header = [True if any((a, b, c, d)) else False for a, b, c, d in zip(type, ends_and, is_blank, incase)]
        return next((j for j, header in zip(line_indices, is_header) if not header), line_indices[(-1)])
    ...
    def write_one_subroutine(modified_file, nospaces, a, b, stext, etext):
        ...
        if b.lower() not in nospaces[istart:]:
            if 'subroutine' in b:
                b = 'end subroutine'
        try:
    ...
    def modify_original_file(original, f_nospaces, f_short):
        ...
        for a, b, stext, etext in zip(first_lines, last_lines, stexts, etexts):
            modified_file = write_one_subroutine(modified_file, nospaces, a, b, stext, etext)
...
def modify_mesa_terminal_output(input_file, output_file, i_ignore=2, files=[]):
    """
    Input:
    ...
    def modify_original_file(original_file):
        ...
        starting = ('start', 'finsh')
        modified = [x for x in modified if x.startswith(starting)]
        modified = [m.strip() for m in modified]
        modified = remove_ignore(modified)
        if files:
            modified = [x for x in modified if x.endswith(tuple(files))]
        finish_names = ['finsh' + x[5:] if x[:5] == 'start' else False for x in modified]
        indexlist = [get_finish_index(modified, i, x) for i, x in enumerate(finish_names)]
...
def star_mesa_f_files(mesa_dir):
    ...
    """
    other_files = [
     '/star/private/evolve.f90', '/star/job/run_star.f',
     '/include/standard_run_star_extras.inc']
    short_files = [
     'adjust_mass', 'eps_grav', 'micro',
...
def write_mesa_routines(mesa_dir, mesa_dir_print, files='main', reset=True, ignore_functions=[
 'subroutine check']):
    """
    Input:
    ...
    def modify_specific(files_to_modify):
        ...
        output_filenames = [mesa_dir_print + x.split(mesa_dir)[(-1)] for x in input_filenames]
        all_files = all_mesa_f_files(mesa_dir)
        all_files_print = [mesa_dir_print + x.split(mesa_dir)[(-1)] for x in all_files]
        for i, o in zip(input_filenames, output_filenames):
            with open(o, 'r+') as (f):
                original_file = f.readlines()
                modified_original = modify_fortran_file(i,
        ...
        if reset:
            for i, o in zip(all_files, all_files_print):
                if o not in output_filenames:
                    filecmp.cmp(i, o) or subprocess.call(['cp', i, o])
