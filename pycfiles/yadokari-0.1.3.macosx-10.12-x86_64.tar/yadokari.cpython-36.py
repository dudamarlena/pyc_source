...
# [GCC 8.4.0]
# Embedded file name: /Users/suyamar/github/yadokari.py/venv/lib/python3.6/site-packages/yadokari.py
# Compiled at: 2018-03-13 06:43:46
# Size of source mod 2**32: 3897 bytes
import json, pprint, click, requests
...
def cal(context):
    ...
    if res.status_code == 404:
        ...
        if reservations:
            click.echo('\n'.join('{}: {} - {}'.format(r['schedule'], r['started_on'], r['finished_on']) for r in reservations))
        else:
...
def reserve(context):
    ...
    if not context:
        ...
        res = requests.post((API_URL + 'api/v1/yados/{}/reservations'.format(context)), data={'reservation[{}]'.format(k):v for k, v in info.items()})
...
def _ask_confirm(info):
    """ユーザーに宿の予約情報の確認を行う """
    click.echo(RESERVATION_INFO.format('\n'.join('{}. {}: {}'.format(i, t[1], info[t[0]]) for i, t in enumerate(VALUE_TO_PROMPT))))
