...
# [GCC 8.4.0]
# Embedded file name: \Python34\Lib\site-packages\autohdl\webdav.py
# Compiled at: 2015-05-17 03:10:32
# Size of source mod 2**32: 4855 bytes
import getpass, locale, os
...
class Client(object):
    ...
    def check_auth(self, default=True):
        ...
        if self.session.request(method='head', url=self.path_to_check_auth, auth=auth).status_code == 200:
    ...
    def dump_auth(self):
        ...
        try:
            ...
            alog.warning(e)
            new_content = 'machine {host}\nlogin {user}\npassword {password}'.format(host=self.host, user=self.username, password=self.password)
...
if __name__ == '__main__':
    alog.info('ping')
    i = 'русишь нихт nicht'
    print('default enc: {}\npreferred enc: {}\nstdin enc: {}'.format(sys.getdefaultencoding(), locale.getpreferredencoding(), sys.stdin.encoding))
