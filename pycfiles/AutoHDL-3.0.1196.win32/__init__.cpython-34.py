...
# [GCC 8.4.0]
# Embedded file name: \Python34\Lib\site-packages\autohdl\__init__.py
# Compiled at: 2015-05-17 13:47:45
# Size of source mod 2**32: 2120 bytes
import logging.config as lc, os
...
LOGGING_DICT = {'version': 1, 
 ...
 'formatters': {'verbose': {'format': '%(asctime)s:%(filename)s:%(lineno)d:%(levelname)s:\n%(message)s'}, 
                'simple': {'format': '%(levelname)s: %(message)s'}}, 
 'handlers': {'cmd_debug': {'class': 'logging.StreamHandler', 
                            'level': 'DEBUG', 
                            'formatter': 'verbose'}, 
              'cmd_info': {'class': 'logging.StreamHandler', 
                           'level': 'INFO', 
                           'formatter': 'simple'}, 
              'file_debug': {'class': 'logging.handlers.RotatingFileHandler', 
                             'level': 'DEBUG', 
                             'formatter': 'simple', 
