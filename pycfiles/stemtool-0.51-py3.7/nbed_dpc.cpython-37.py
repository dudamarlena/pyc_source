...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/stemtool/dpc/nbed_dpc.py
# Compiled at: 2020-05-06 17:53:01
# Size of source mod 2**32: 13892 bytes
from scipy import ndimage as scnd
from scipy import optimize as sio
import numpy as np, numba, warnings, stemtool as st
...
def strain_and_disk(data4D, disk_size, pixel_list_xy, disk_list, ROI=1, med_factor=50):
    ...
    return (
     e_xx, e_xy, e_th, e_yy, disk_x, disk_y, COM_x, COM_y)
...
def dpc_central_disk(data4D, disk_size, position, ROI=1, med_val=20):
    ...
    for ii in numba.prange(int(no_points)):
        ...
        fitted_disk_list = st.util.fit_gaussian2D_mask(corr_image, pos_p, pos_q, disk_size)
        ...
        fitted_circle = st.util.make_circle(np.asarray(cbed_image.shape), fitted_center[0], fitted_center[1], disk_size)
...
def integrate_dpc(xshift, yshift, fourier_calibration=1):
    ...
    qx = np.mean(np.diff(np.arange(-size_array[1], size_array[1], 1) / (2 * fourier_calibration * size_array[1])))
    qy = np.mean(np.diff(np.arange(-size_array[0], size_array[0], 1) / (2 * fourier_calibration * size_array[0])))
    mirr_ft = (x_mirr_ft + complex(0.0, 1.0) * y_mirr_ft) / (qx + complex(0.0, 1.0) * qy)
