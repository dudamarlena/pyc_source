...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/stemtool/dpc/atomic_dpc.py
# Compiled at: 2020-05-06 17:53:53
# Size of source mod 2**32: 2049 bytes
from scipy import ndimage as scnd
from scipy import optimize as sio
import numpy as np, stemtool as st
...
def calculate_dpc(data4D):
    ...
    com_y = np.sum((np.multiply(data4D, diff_y)), axis=(2, 3)) / np.sum(data4D, axis=(2,
                                                                                      3))
    com_x = np.sum((np.multiply(data4D, diff_x)), axis=(2, 3)) / np.sum(data4D, axis=(2,
                                                                                      3))
