...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/stemtool/nbed/nbed_strain.py
# Compiled at: 2020-05-06 18:00:44
# Size of source mod 2**32: 40006 bytes
...
import stemtool as st, warnings
...
def fit_nbed_disks(corr_image, disk_size, positions, diff_spots, nan_cutoff=0):
    ...
    if nancount == no_pos:
        ...
        if center.shape[0] > 0:
            ...
            lcbed = np.nan
    return (
     fitted_disk_list, center_position, fit_deviation, lcbed)


@numba.jit
def strain_in_ROI(data4D, ROI, center_disk, disk_list, pos_list, reference_axes=0, med_factor=10, gauss_val=3, hybrid_cc=0.1, nan_cutoff=0.5):
    """
...
@numba.jit
def strain_log(data4D_ROI, center_disk, disk_list, pos_list, reference_axes=0, med_factor=10):
    warnings.filterwarnings('ignore')
    no_of_disks = data4D_ROI.shape[(-1)]
    ...
    return (
     e_xx_log, e_xy_log, e_th_log, e_yy_log)
...
def strain_oldstyle(data4D_ROI, center_disk, disk_list, pos_list, reference_axes=0):
    ...
    return (
     e_xx_ROI, e_xy_ROI, e_th_ROI, e_yy_ROI)
...
def spectra_finder(data4D, yvals, xvals):
    spectra_data = np.ravel(np.mean((data4D[:, :, yvals[0]:yvals[1], xvals[0]:xvals[1]]), axis=(-1,
                                                                                                -2), dtype=(np.float64)))
...
@numba.jit
def strain4D_general(data4D, disk_radius, ROI=0, disk_center=np.nan, rotangle=0, med_factor=30, gauss_val=3, hybrid_cc=0.2):
    """
