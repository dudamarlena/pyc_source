...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/stemtool/util/image_utils.py
# Compiled at: 2020-05-06 17:43:22
# Size of source mod 2**32: 21873 bytes
...
def move_by_phase(image_to_move, x_pixels, y_pixels):
    ...
    move_phase = np.exp(-2 * np.pi * complex(0.0, 1.0) * move_matrix)
...
def phase_color(phase_image):
    ...
    gray_im = 0.2989 * r + 0.587 * g + 0.114 * b
    gray_im = gray_im
...
def cross_corr(image_1, image_2, hybridizer=0, normal=True):
    ...
    .. math::
         G_c = \x0crac{G_1         imes G_2^*}{\\mid G_1    imes G_2^* \\mid}
    
    Thus, we can now define a hybrid cross-correlation where
    .. math::
         G_c = \x0crac{G_1         imes G_2^*}{\\mid G_1    imes G_2^* \\mid ^n}
