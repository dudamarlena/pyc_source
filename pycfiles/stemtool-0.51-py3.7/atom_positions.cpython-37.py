...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/stemtool/afit/atom_positions.py
# Compiled at: 2020-05-06 17:48:57
# Size of source mod 2**32: 27147 bytes
...
import warnings, stemtool as st
...
@numba.jit
def mpfit(main_image, initial_peaks, peak_runs=16, cut_point=0.6666666666666666, tol_val=0.01, peakparams=False):
    """
    ...
    for jj in np.arange(len(initial_peaks)):
        ...
        for ii in np.arange(peak_runs):
            ...
            popt, _ = spo.curve_fit((st.util.gaussian_2D_function), xy, zgaus, initial_guess, bounds=(
    ...
    if peakparams:
        return (
         mpfit_peaks, peak_vals)
    return mpfit_peaks


@numba.jit
def mpfit_voronoi(main_image, initial_peaks, peak_runs=16, cut_point=0.6666666666666666, tol_val=0.01, blur_factor=0.25):
    """
    Multi-Gaussian Peak Refinement (mpfit) 
...
def get_origin(image, peak_pos, coords):
    ...
    def origin_function(xyCenter, input_data=(
 peak_pos, coords)):
        peaks = input_data[0]
        coords = input_data[1]
        ...
        average_deviation = (np.mean(np.abs(atom_coords[:, 3] - atom_coords[:, 5])) ** 2 + np.mean(np.abs(atom_coords[:, 2] - atom_coords[:, 4])) ** 2) ** 0.5
...
def relative_strain(n_list, coords):
    ...
    return (
     cell_center, e_yy, e_xx, e_xy, e_th)
