...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/stemtool/proc/sobel_canny.py
# Compiled at: 2020-05-06 18:01:15
# Size of source mod 2**32: 12266 bytes
import numpy as np, numba, warnings
from scipy import signal as scisig
from scipy import ndimage as scnd
import stemtool as st, math
...
def sobel(im, order=3):
    ...
    if order == 3:
        ...
        k_x = np.asarray(((-1, -2, 0, 2, 1), (-4, -8, 0, 8, 4), (-6, -12, 0, 12, 6),
                          (-4, -8, 0, 8, 4), (-1, -2, 0, 2, 1)), dtype=(np.float64))
        k_y = np.asarray(((1, 4, 6, 4, 1), (2, 8, 12, 8, 2), (0, 0, 0, 0, 0), (-2, -8, -12, -8, -2),
...
def edge_thinner(sobel_mag, sobel_angle):
    ...
    for ii in range(1, matrix_size[0] - 1):
        for jj in range(1, matrix_size[1] - 1):
            q = 1
            r = 1
            if not 0 <= sobel_degree[(ii, jj)] < 22.5:
                if 157.5 <= sobel_degree[(ii, jj)] <= 180:
                    q = sobel_mag[(ii, jj + 1)]
                    r = sobel_mag[(ii, jj - 1)]
            if 22.5 <= sobel_degree[(ii, jj)] < 67.5:
                q = sobel_mag[(ii + 1, jj - 1)]
                r = sobel_mag[(ii - 1, jj + 1)]
            else:
                if 67.5 <= sobel_degree[(ii, jj)] < 112.5:
                    q = sobel_mag[(ii + 1, jj)]
                    r = sobel_mag[(ii - 1, jj)]
                else:
                    if 112.5 <= sobel_degree[(ii, jj)] < 157.5:
                        q = sobel_mag[(ii - 1, jj - 1)]
                        r = sobel_mag[(ii + 1, jj + 1)]
...
def edge_joining(thresholded, lowThreshold, highThreshold):
    ...
    for ii in range(1, matrix_size[0] - 1):
        for jj in range(1, matrix_size[1] - 1):
            if not thresholded[(ii, jj)] == lowThreshold or thresholded[(ii - 1, jj - 1)] == highThreshold or thresholded[(ii - 1, jj)] == highThreshold or thresholded[(ii - 1, jj + 1)] == highThreshold or thresholded[(ii, jj - 1)] == highThreshold or thresholded[(ii, jj + 1)] == highThreshold or thresholded[(ii + 1, jj - 1)] == highThreshold or thresholded[(ii + 1, jj)] == highThreshold or thresholded[(ii + 1, jj + 1)] == highThreshold:
