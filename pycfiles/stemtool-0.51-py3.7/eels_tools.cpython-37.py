...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/stemtool/eels/eels_tools.py
# Compiled at: 2020-05-03 18:27:13
# Size of source mod 2**32: 11266 bytes
import numpy as np, pywt, numba
...
def powerlaw_plot(xdata, ydata, xrange, figtitle, showdata=True, font={'family':'sans-serif', 
 'weight':'bold', 
 'size':25}):
    (mpl.rc)(*('font', ), **font)
...
def eels_3D(eels_dict, fit_range, peak_range, LBA_radius=3):
    ...
    return (
     peak_values, elemental_subtracted)
    ...
    for pp in range(eels_array.shape[(-2)]):
        for qq in range(eels_array.shape[(-1)]):
            for rr in range(no_elements):
                ...
                lbi = (yy - ii) ** 2 + (xx - jj) ** 2 <= LBA_radius ** 2
                eels_lbi = np.mean((eels_array[:, lbi]), axis=(-1))
                popt, _ = spo.curve_fit(lcpl, (xdata[star_val:stop_val]),
                  (eels_lbi[star_val:stop_val]),
                  bounds=(
                 lower_bound[:, rr], upper_bound[:, rr]),
                  ftol=0.0001,
                  xtol=0.0001)
