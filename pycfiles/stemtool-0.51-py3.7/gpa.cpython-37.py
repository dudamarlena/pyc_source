...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/stemtool/gpa/gpa.py
...
def find_diffraction_spots(image, circ_0, circ_1, circ_2):
    """
...
def define_reference(image, A, B, C, D):
    """
    ...
    c_DA = D[1] - m_DA * D[0]
    ref_reg = np.logical_and(np.logical_and(yy > m_AB * xx + c_AB, (yy - c_BC) / m_BC > xx), np.logical_and(yy < m_CD * xx + c_CD, (yy - c_DA) / m_DA < xx))
...
def phase_diff(angle_image):
    ...
    Debangshu Mukherjee <mukherjeed@ornl.gov>
    """
    imaginary_image = np.exp(complex(0.0, 1.0) * angle_image)
...
def phase_subtract(matrix_1, matrix_2):
    return np.angle(np.exp(complex(0.0, 1.0) * (matrix_1 - matrix_2)))
