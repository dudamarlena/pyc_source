...
# Embedded file name: build/bdist.linux-i686/egg/py3o/renderers/pyuno/office.py
# Compiled at: 2014-11-11 09:15:39
from __future__ import print_function
import glob, logging, os, socket, subprocess, sys, time
...
class OfficeBase(object):
    ...
    SEARCHPATHS_UNIX = glob.glob('/usr/lib*/libreoffice*') + glob.glob('/usr/lib*/openoffice*') + glob.glob('/usr/lib*/ooo*') + glob.glob('/opt/libreoffice*') + glob.glob('/opt/openoffice*') + glob.glob('/opt/ooo*') + glob.glob('/usr/local/libreoffice*') + glob.glob('/usr/local/openoffice*') + glob.glob('/usr/local/ooo*') + glob.glob('/usr/local/lib/libreoffice*')
    SEARCHPATHS_MAC = glob.glob('/Applications/*Office.app/Contents/')

    def __init__(self, binary_name=None, office_path=None, uno_path=None, python_home=None, python=None):
        """
    ...
    def find_office_path(self):
        ...
        if self.office_path:
            ...
            log.debug('%s not found in office path %s. Now searching for %s in the default paths.' % (
             office_exe, self.office_path,
             office_exe))
        ...
        raise OfficeException('Office path not found in %s' % self.searchpaths)
    ...
    def find_office_executable(self):
        ...
        raise OfficeException('%s executable not found in %s' % (self.OFFICE_EXE,
         self.searchpaths))
    ...
    def find_uno_path(self):
        if self.uno_path:
            ...
            log.debug('%s not found in uno path %s. Now searching for %s in the default paths.' % (
             self.pyunolib, self.uno_path,
             self.pyunolib))
        for path in self.searchpaths:
        ...
        raise OfficeException('%s not found in %s' % (self.pyunolib, self.searchpaths))

    def find_python_home(self):
        if self.python_home:
            if os.path.exists(self.python_home):
                return self.python_home
            log.debug('python home %s does not exist. Now searching for python home in default paths' % self.python_home)
        officepath = self.find_office_path()
        if self.is_mac():
            paths = glob.glob(os.path.join(officepath, '*OfficePython.framework'))
        else:
    ...
    def find_python_executable(self):
        if self.python:
            if os.path.isfile(self.python):
                return self.python
            log.debug('python binary %s does not exist. Now searching for python binary in default paths' % self.python)
        for path in self.searchpaths:
            opath = os.path.join(path, self.OFFICE_PROGAM_PATH)
            for exe in [os.path.join(opath, 'python')] + glob.glob(os.path.join(opath, 'python-*')):
                if os.path.isfile(exe):
                    return exe

        log.debug('No office specific python binary found. Using default %s' % sys.executable)
...
class OfficeServer(OfficeBase):
    ...
    """
    OFFICE_ARG_ACCEPT = '-accept=socket,host=%(host)s,port=%(port)s;urp;StarOffice.ServiceManager'
    OFFICE_ARGS = [
     '-norestore',
     '-nofirststartwizard',
     '-nologo',
    ...
    def is_running(self):
        try:
            csocket = socket.create_connection((self.host, self.port), self.timeout)
            csocket.close()
    ...
    def start(self):
        ...
        log.debug('executing command: ' + (' ').join([
         exe] + self.OFFICE_ARGS + [
         self.OFFICE_ARG_ACCEPT % dict(host=self.host, port=self.port)]))
        proc = subprocess.Popen([
         exe] + self.OFFICE_ARGS + [
         self.OFFICE_ARG_ACCEPT % dict(host=self.host, port=self.port)], env=newenv, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        self.oopid = proc.pid
        waited = 0
        started = False
        while waited < 10:
            if proc.poll():
                ...
                raise OfficeException('OfficeServer has already terminated. %s' % stderr)
    ...
    def die(self):
        if self.oopid:
            log.debug('Shutting down OfficeServer with pid: %s' % self.oopid)
            try:
                os.kill(self.oopid, 15)
                state = (0, 0)
                waited = 0
                while 1:
                    if state == (0, 0) and waited < 15:
                        log.debug('Waiting for OfficeServer with pid %s to disappear.' % self.oopid)
...
class OfficeClient(OfficeBase):
    ...
    def convert(self, infilename, outfilename, format=None):
        ...
        command = [
         python, '-B', py_file, self.host, str(self.port),
        ...
        if out:
            log.debug('Spawned client output was:\n%s' % out)
        else:
            log.debug('Spawned client returned without output')
        if proc.returncode != 0:
            if proc.returncode > 0:
                raise OfficeException('Spawned client had an error: %s' % err)
            else:
                log.error('Spawned client has exited due to signal %s' % abs(proc.returncode))
        return
    ...
    def get_office_env(self):
        ...
        env['URE_BOOTSTRAP'] = self.UNO_PATHNAME + ':' + os.path.join(uno_path, 'fundamentalrc')
        if pythonhome:
...
class OfficeClientSpawned(object):
    ...
    UNO_RESOLVE = 'uno:socket,host=%(host)s,port=%(port)s;urp;' + 'StarOffice.ComponentContext'
    ...
    def connect(self):
        ...
        resolver = localctx.ServiceManager.createInstanceWithContext(self.UNO_RESOLVER, localctx)
        try:
            ctx = resolver.resolve(self.UNO_RESOLVE % dict(host=self.host, port=self.port))
        except NoConnectException:
            raise OfficeException('Could not connect to OfficeServer %s on port %s' % (
             self.host, self.port))
    ...
    def load_document(self, infilename, inputprops=None):
        ...
        document = self.desktop.loadComponentFromURL(in_url, '_blank', 0, inputprops())
        if not document:
            raise OfficeException("Input document %s couldn't be loaded" % in_url)
        return document
