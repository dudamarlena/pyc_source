...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cary/caryapp.py
# Compiled at: 2015-08-04 06:07:26
...
class CaryApp:
    ...
    def dispatch_command(self, parsed_msg):
        if parsed_msg.subject in self.command_table:
            ...
            self.last_response = self.help_response(self.last_subject, parsed_msg.from_address)
    ...
    @property
    def help(self):
        return "Hi!  I'm Cary.  Send me a message with one of the following in the title, and I'll do my best to respond usefully:\n\n\t" + '\n\t'.join('{0} - {1}'.format(name, self.command_table[name].description) for name in self.command_table.keys())
    ...
    def send_response(self, subject, to_address, response):
        ...
        """
        logging.debug('Sending mail using SMTP host {0}, return {1}, to {2}'.format(self.smtp_host, self.return_address, to_address))
        logging.debug('Response:\n{0}'.format(response))
        s = smtplib.SMTP(self.smtp_host)
        logging.debug('Types: {0}, {1}, {2}'.format(type(self.return_address), type(to_address), type(response)))
