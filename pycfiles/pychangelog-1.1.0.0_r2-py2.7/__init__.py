...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pychangelog\__init__.py
# Compiled at: 2014-05-20 20:14:10
"""
The toplevel module for the pychangelog package.
"""
from docit import *
import re, collections, types, datetime, threading, abc
release_header_re = re.compile('^rel\\s+(?P<rel>[0-9]+)\\s+(?:-\\s+)?v(?P<vers>[\\.0-9]+)\\s+(?:-\\s+)(?P<year>[0-9]{4})-(?P<month>[0-9]{1,2})-(?P<day>[0-9]{1,2})\\s*$', re.I)
prerelease_header_re = re.compile('^pre[ -]?rel(?:ease)?\\s+(?P<rel>[0-9]+)(?:\\s*$|\\s+(?:-\\s+)?v(?:(?P<vers>[\\.0-9]+)|[\\.0-9\\?]+)\\s+(?:-\\s+)(?:(?P<year>[0-9]{4})|\\?\\?\\?\\?)-(?:(?P<month>[0-9]{1,2}|\\?\\?))-(?:(?P<day>[0-9]{1,2})|\\?\\?)\\s*$)', re.I)
        ...
        if state == 0:
            if len(line):
                if not line[0].isspace():
                    ...
                    if mobj is not None:
                        release_info = ReleaseInfo(release_num=mobj.group('rel'), version_numbers=mobj.group('vers'), year=mobj.group('year'), month=mobj.group('month'), day=mobj.group('day'))
                        change_line = None
                        indent_1 = None
                        indent_2 = None
                        state = 1
                        ...
                        if mobj is not None:
                            release_info = ReleaseInfo(release_num=mobj.group('rel'), version_numbers=mobj.group('vers'), year=mobj.group('year'), month=mobj.group('month'), day=mobj.group('day'), pre_release=True)
                            change_line = None
                            indent_1 = None
                            indent_2 = None
                            state = 1
