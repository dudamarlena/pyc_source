...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/tests/performance/recursive_model_serializer.py
# Compiled at: 2020-05-13 07:46:01
import os, django, timeit, sys
...
def main():
    ...
    object = models.CategoryHierarchy(top_category, categories=[
     models.CategoryHierarchy(child_movies, categories=[
      models.CategoryHierarchy(cartoons, films=[ models.Film.objects.create(name='Mickey Mouse', year=1966, uploaded_by=user, category=cartoons) for _ in range(10)
                                         ])]),
     models.CategoryHierarchy(serious_stuff, categories=[
      models.CategoryHierarchy(anime, films=[ models.Film.objects.create(name='Ghost in the shell', year=1989, uploaded_by=user, category=anime) for _ in range(10)
                                      ])])])
    ...
    print ('Model with recursion serializer: ',
     timeit.timeit('simple_serialize()', globals={'simple_serialize': simple_serialize}, number=500))
    print ('Cached model with recursion serializer: ',
     timeit.timeit('cached_serialize()', globals={'cached_serialize': cached_serialize}, number=500))
