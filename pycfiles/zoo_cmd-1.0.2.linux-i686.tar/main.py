...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/zoo_cmd/main.py
# Compiled at: 2016-11-05 22:58:47
from cmd import Cmd
import sys
...
class ZooCmd(Cmd):
    ...
    def do_ls(self, line=None, *args):
        for pname in sorted(self.zoo.ls(line)):
            ...
            _pname = '+ %-40s%s' % (pname, time_str) if len(self.zoo.ls('%s/%s' % (line, pname))) else '- %-40s%s' % (pname, time_str)
            print _pname
