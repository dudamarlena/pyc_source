...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/zoo_cmd/zk/zk_opers.py
# Compiled at: 2016-11-05 22:57:06
...
class ZkOpers(object):
    ...
    def _relative_path_cd(self, path):
        ...
        list(map(lambda x: _pathlist.pop() if x == '..' else _pathlist.append(x), list(filter(lambda x: x, path.split('/')))))
    ...
    def cd(self, path=None):
        if not path:
            return
        _prefix_path = path if path.startswith('/') else self._relative_path_cd(path)
    ...
    def recursion_cat(self, path):
        ...
        while i < layer_num:
            for prefix in prefixes:
                ...
                paths_selects = list(filter(lambda x: re.search(paths[i], x), paths_pool))
                next_prefixes = []
