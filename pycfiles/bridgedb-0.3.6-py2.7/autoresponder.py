...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bridgedb/email/autoresponder.py
...
def createResponseBody(lines, context, client, lang=b'en'):
    ...
    try:
        ...
        if not bridgeRequest.isValid():
            raise EmailRequestedHelp(b"Email request from '%s' was invalid." % str(client))
        ...
        logging.info(b'Got a mail too frequently: %s.' % error)
        return templates.buildSpamWarning(translator, client)
    except (IgnoreEmail, addr.BadEmail) as error:
        logging.info(error)
        return

    answer = b'(no bridges currently available)\r\n'
    if bridges:
        transport = bridgeRequest.justOnePTType()
        answer = (b'').join(b'  %s\r\n' % b.getBridgeLine(bridgeRequest, context.includeFingerprints) for b in bridges)
    return templates.buildAnswerMessage(translator, client, answer)
    return


def generateResponse(fromAddress, client, body, subject=None, messageID=None, gpgSignFunc=None):
    ...
    response.to = client
    response.writeHeaders(fromAddress.encode(b'utf-8'), str(client), subject, inReplyTo=messageID)
    response.writeBody(body.encode(b'utf-8'))
    if not safelog.safe_logging:
        contents = response.readContents()
        logging.debug(b'Email contents:\n%s' % str(contents))
    else:
        logging.debug(b'Email text for %r created.' % str(client))
    response.rewind()
    return response
...
class EmailResponse(object):
    ...
    def close(self):
        ...
        logging.debug(b'Closing %s.mailfile...' % self.__class__.__name__)
        self.mailfile.close()
        self.closed = True
    ...
    def read(self, size=None):
        ...
        """
        contents = b''
        logging.debug(b'Reading%s from %s.mailfile...' % (
         (b' {0} bytes').format(size) if size else b'',
         self.__class__.__name__))
        try:
            if size is not None:
                contents = self.mailfile.read(int(size)).encode(b'utf-8')
            else:
                contents = self.mailfile.read().encode(b'utf-8')
    ...
    def rewind(self):
        ...
        logging.debug(b'Rewinding %s.mailfile...' % self.__class__.__name__)
        self.mailfile.seek(0)
    ...
    def write(self, line):
        ...
        """
        if line.find(b'\r\n') != -1:
            logging.debug(b'Found newlines in %r. Calling writelines().' % line)
            self.writelines(line)
        else:
            line += self.delimiter
            self.mailfile.write(self._buff(line.encode(b'utf8')))
            self.mailfile.flush()

    def writelines(self, lines):
        r"""Calls :meth:`write` for each line in **lines**.
    ...
    def writeHeaders(self, fromAddress, toAddress, subject=None, inReplyTo=None, includeMessageID=True, contentType=b'text/plain; charset="utf-8"', **kwargs):
        ...
        """
        self.write(b'From: %s' % fromAddress)
        self.write(b'To: %s' % toAddress)
        if includeMessageID:
            self.write(b'Message-ID: %s' % smtp.messageid().encode(b'utf-8'))
        if inReplyTo:
            self.write(b'In-Reply-To: %s' % inReplyTo.encode(b'utf-8'))
        self.write(b'Content-Type: %s' % contentType.encode(b'utf-8'))
        self.write(b'Date: %s' % smtp.rfc822date().encode(b'utf-8'))
        if not subject:
            subject = b'[no subject]'
        if not subject.lower().startswith(b're'):
            subject = b'Re: ' + subject
        self.write(b'Subject: %s' % subject.encode(b'utf-8'))
        if kwargs:
            for headerName, headerValue in kwargs.items():
                headerName = headerName.capitalize()
                headerName = headerName.replace(b' ', b'-')
                headerName = headerName.replace(b'_', b'-')
                header = b'%s: %s' % (headerName, headerValue)
                self.write(header.encode(b'utf-8'))
    ...
    def writeBody(self, body):
        ...
        """
        logging.info(b'Writing email body...')
        if self.gpgSign:
            logging.info(b'Attempting to sign email...')
...
class SMTPAutoresponder(smtp.SMTPClient):
    ...
    def getMailData(self):
        r"""Gather all the data for building the response to the client.
        ...
        if not clients:
            ...
            client = clients[0]
            if not safelog.safe_logging:
                logging.debug(b'Incoming email was from %s ...' % client)
            if not self.runChecks(client):
                return
            recipient = self.getMailFrom()
            lang = translations.getLocaleFromPlusAddr(recipient)
            logging.info(b'Client requested email translation: %s' % lang)
            body = createResponseBody(self.incoming.lines, self.incoming.context, client, lang)
            if not body:
                return
            messageID = self.incoming.message.getheader(b'Message-ID', None)
            subject = self.incoming.message.getheader(b'Subject', None)
            response = generateResponse(recipient, client, body, subject, messageID, self.incoming.context.gpgSignFunc)
            return response
    ...
    def getMailTo(self):
        ...
        addrHeader = None
        try:
            fromAddr = self.incoming.message.getaddr(b'From')[1]
        ...
        if not addrHeader:
            logging.warn(b'No From header on incoming mail.')
            try:
                senderHeader = self.incoming.message.getaddr(b'Sender')[1]
            except (IndexError, TypeError, AttributeError):
                pass
            else:
        ...
        if not addrHeader:
            logging.warn(b'No Sender header on incoming mail.')
            return clients
        else:
            client = None
            try:
                if addrHeader in self.incoming.context.whitelist.keys():
                    logging.debug(b'Email address was whitelisted: %s.' % addrHeader)
                    client = smtp.Address(addrHeader)
                else:
                    normalized = addr.normalizeEmail(addrHeader, self.incoming.context.domainMap, self.incoming.context.domainRules)
    ...
    def getMailFrom(self):
        ...
        """
        logging.debug(b"Searching for our email address in 'To:' header...")
        ours = None
        try:
            ourAddress = smtp.Address(self.incoming.context.fromAddr)
            allRecipients = self.incoming.message.getaddrlist(b'To')
            for _, addr in allRecipients:
                recipient = smtp.Address(addr)
                if ourAddress.domain not in recipient.domain:
                    logging.debug(b'Not our domain (%s) or subdomain, skipping email address: %s' % (
                     ourAddress.domain, str(recipient)))
                    continue
                if not recipient.local.startswith(ourAddress.local):
                    logging.debug(b"Username doesn't begin with ours, skipping email address: %s" % str(recipient))
                    continue
            ...
            logging.error(b"Couldn't find our email address in incoming email headers: %r" % error)
            ours = self.incoming.context.fromAddr

        logging.debug(b'Found our email address: %s.' % ours)
        return ours
    ...
    def sentMail(self, success):
        ...
        for address, code, resp in addresses:
            logging.info(b'Sent reply to %s' % address)
            logging.debug(b'SMTP server response: %d %s' % (code, resp))
    ...
    def sendError(self, fail):
        ...
        """
        logging.debug(b'called with %r' % fail)
        if isinstance(fail, failure.Failure):
            error = fail.getTraceback() or b'Unknown'
        else:
    ...
    def reply(self):
        ...
        :returns: A ``Deferred`` which will callback when the response has
            ...
            sending the email.
        """
        logging.info(b'Got an email; deciding whether to reply.')
    ...
    def runChecks(self, client):
        ...
        if not self.incoming.canonicalFromSMTP:
            logging.warn(b"SMTP 'MAIL FROM' wasn't from a canonical domain for email from %s" % str(client))
            return False
        if str(client) in self.incoming.context.whitelist.keys():
            self.incoming.canonicalFromEmail = client.domain
            logging.info(b"'From:' header contained whitelisted address: %s" % str(client))
        else:
            if str(client) in self.incoming.context.blacklist:
                logging.info(b"'From:' header contained blacklisted address: %s")
                return False
            logging.debug(b'Canonicalizing client email domain...')
            try:
                self.incoming.canonicalFromEmail = canonicalizeEmailDomain(client.domain, self.incoming.canon)
                logging.debug(b'Canonical email domain: %s' % self.incoming.canonicalFromEmail)
            except addr.UnsupportedDomain as error:
                logging.info(b"Domain couldn't be canonicalized: %s" % safelog.logSafely(client.domain))
                return False

            if self.incoming.canonicalFromSMTP != self.incoming.canonicalFromEmail:
                logging.error(b'SMTP/Email canonical domain mismatch!')
                logging.debug(b'Canonical domain mismatch: %s != %s' % (
        ...
        if self.incoming.context.fuzzyMatch != 0:
            for blacklistedAddress in self.incoming.context.blacklist:
                ...
                if distance <= self.incoming.context.fuzzyMatch:
                    logging.info(b'Fuzzy-matched %s to blacklisted address %s!' % (
                     self.incoming.canonicalFromEmail,
                     blacklistedAddress))
                    return False
    ...
    def send(self, response, retries=0, timeout=30, reaktor=reactor):
        ...
        """
        logging.info(b'Sending reply to %s ...' % str(response.to))
        factory = smtp.SMTPSenderFactory(self.incoming.context.smtpFromAddr, str(response.to), response, self.deferred, retries=retries, timeout=timeout)
        factory.domain = smtp.DNSNAME
        reaktor.connectTCP(self.incoming.context.smtpServerIP, self.incoming.context.smtpServerPort, factory)
        return self.deferred