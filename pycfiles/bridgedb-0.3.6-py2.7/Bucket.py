...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bridgedb/Bucket.py
...
"""
import logging, time, bridgedb.Storage, bridgedb.Bridges, binascii, sqlite3
...
class BucketManager(object):
    ...
    def assignBridgesToBuckets(self):
        ...
        while self.unallocated_available and len(self.bucketList) > 0:
            logging.debug("We have %d unallocated bridges and %d buckets to fill. Let's do it." % (
            ...
            for d in self.bucketList:
                if d.allocated < d.needed:
                    try:
                        ...
                        logging.warn("Couldn't assign unallocated bridge to %s: %s" % (
                         dist, e))
        ...
        with bridgedb.Storage.getDB() as (db):
            for b in bridges:
                if self.cfg.COLLECT_TIMESTAMPS:
                    ...
                    if bh:
                        bridgeHistories.append(bh)
                    bridgeHistories.sort(lambda x, y: cmp(x.weightedFractionalUptime, y.weightedFractionalUptime))

            try:
                f = open(filename, 'w')
                if self.cfg.COLLECT_TIMESTAMPS:
                    for bh in bridgeHistories:
                        days = bh.tosa / long(86400)
                        line = '%s:%s\t(%d days at this address)' % (
                         bh.ip, bh.port, days)
                        if str(bh.fingerprint) in blocklist.keys():
                            line = line + '\t(Might be blocked): (%s)' % (',').join(blocklist[bh.fingerprint])
                        f.write(line + '\n')
