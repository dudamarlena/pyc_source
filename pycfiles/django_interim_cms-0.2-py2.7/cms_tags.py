...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/interim_cms/templatetags/cms_tags.py
...
class GetLeftDashboardNode(template.Node):
    ...
    def render(self, context):
        dashboard = _get_dashboard_cls(getattr(settings, 'CMS_LEFT_DASHBOARD', 'interim_cms.dashboard.LeftDashboard'), context)()
...
class GetCenterDashboardNode(template.Node):
    ...
    def render(self, context):
        dashboard = _get_dashboard_cls(getattr(settings, 'CMS_CENTER_DASHBOARD', 'interim_cms.dashboard.CenterDashboard'), context)()
...
class GetRightDashboardNode(template.Node):
    ...
    def render(self, context):
        dashboard = _get_dashboard_cls(getattr(settings, 'CMS_RIGHT_DASHBOARD', 'interim_cms.dashboard.RightDashboard'), context)()
