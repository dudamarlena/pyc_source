...
# [GCC 8.4.0]
# Embedded file name: /local/hd1/home1/data/acue/rd/p-open-deploy/xmlschema_acue/components/xmlschema_acue/tests/xmlschema_acue_tests/libs/validators/test_validators/Case010/CallCase.py
# Compiled at: 2019-05-19 13:15:11
"""
This module runs tests concerning the validation/decoding/encoding of XML files.
"""
import unittest, pdb, os, sys, pickle
from decimal import Decimal
import base64, warnings
from elementpath import datatypes
import xmlschema_acue as xmlschema
from xmlschema_acue import XMLSchemaEncodeError, XMLSchemaValidationError, XMLSchema, ParkerConverter, BadgerFishConverter, AbderaConverter, JsonMLConverter
from xmlschema_acue.compat import unicode_type, ordered_dict_class
from xmlschema_acue.etree import etree_element, etree_tostring, is_etree_element, ElementTree, etree_elements_assert_equal, lxml_etree, lxml_etree_element
from xmlschema_acue.helpers import local_name
from xmlschema_acue.qnames import XSI_TYPE
from xmlschema_acue.resources import fetch_namespaces
from tests.xmlschema_acue_tests.testtools.test_factory import tests_factory
from tests.xmlschema_acue_tests.testtools.TestCaseXMLSchema import XMLSchemaTestCase
from xmlschema_acue.validators import XMLSchema11
_VEHICLES_DICT = {'@xmlns:vh': 'http://example.com/vehicles', 
   '@xmlns:xsi': 'http://www.w3.org/2001/XMLSchema-instance', 
   '@xsi:schemaLocation': 'http://example.com/vehicles vehicles.xsd', 
   'vh:cars': {'vh:car': [{'@make': 'Porsche', '@model': '911'}, {'@make': 'Porsche', '@model': '911'}]}, 'vh:bikes': {'vh:bike': [{'@make': 'Harley-Davidson', '@model': 'WL'}, {'@make': 'Yamaha', '@model': 'XS650'}]}}
_VEHICLES_DICT_ALT = [{'vh:cars': [{'vh:car': None, '@make': 'Porsche', '@model': '911'}, {'vh:car': None, '@make': 'Porsche', '@model': '911'}]}, {'vh:bikes': [{'vh:bike': None, '@make': 'Harley-Davidson', '@model': 'WL'}, {'vh:bike': None, '@make': 'Yamaha', '@model': 'XS650'}]}, {'@xsi:schemaLocation': 'http://example.com/vehicles vehicles.xsd'}]
_COLLECTION_DICT = {'@xmlns:col': 'http://example.com/ns/collection', 
   '@xmlns:xsi': 'http://www.w3.org/2001/XMLSchema-instance', 
   ...
   'object': [
            ...
            {'@available': True, 
               ...
               'title': None, 
               'year': '1925'}]}
_COLLECTION_PARKER = {'object': [
            {'author': {'born': '1841-02-25', 'dead': '1919-12-03', 
                          'name': 'Pierre-Auguste Renoir', 
                          'qualification': 'painter'}, 
               'estimation': 10000.0, 
               'position': 1, 
               'title': 'The Umbrellas', 
               'year': '1886'},
            {'author': {'born': '1893-04-20', 'dead': '1983-12-25', 
                          'name': 'Joan Miró', 
                          'qualification': 'painter, sculptor and ceramicist'}, 
               'position': 2, 
               'title': None, 
               'year': '1925'}]}
_COLLECTION_PARKER_ROOT = {'col:collection': {'object': [
                               {'author': {'born': '1841-02-25', 'dead': '1919-12-03', 
                                             'name': 'Pierre-Auguste Renoir', 
                                             'qualification': 'painter'}, 
                                  'estimation': 10000.0, 
                               ...
                               {'author': {'born': '1893-04-20', 'dead': '1983-12-25', 
                                  ...
                                  'title': None, 
                                  'year': '1925'}]}}
_COLLECTION_BADGERFISH = {'@xmlns': {'col': 'http://example.com/ns/collection', 
              'xsi': 'http://www.w3.org/2001/XMLSchema-instance'}, 
   'col:collection': {'@xsi:schemaLocation': 'http://example.com/ns/collection collection.xsd', 
                      'object': [
                               {'@available': True, 
                                  '@id': 'b0836217462', 
                                  'author': {'@id': 'PAR', 
                                             'born': {'$': '1841-02-25'}, 'dead': {'$': '1919-12-03'}, 'name': {'$': 'Pierre-Auguste Renoir'}, 'qualification': {'$': 'painter'}}, 
                                  'estimation': {'$': 10000.0}, 'position': {'$': 1}, 'title': {'$': 'The Umbrellas'}, 'year': {'$': '1886'}},
                               {'@available': True, 
                                  '@id': 'b0836217463', 
                                  'author': {'@id': 'JM', 
                                             'born': {'$': '1893-04-20'}, 'dead': {'$': '1983-12-25'}, 'name': {'$': 'Joan Miró'}, 'qualification': {'$': 'painter, sculptor and ceramicist'}}, 
                                  'position': {'$': 2}, 'title': {}, 'year': {'$': '1925'}}]}}
_COLLECTION_ABDERA = {'attributes': {'xsi:schemaLocation': 'http://example.com/ns/collection collection.xsd'}, 
   'children': [
              {'object': [
                          {'attributes': {'available': True, 'id': 'b0836217462'}, 'children': [
                                        {'author': {'attributes': {'id': 'PAR'}, 'children': [
                                                                 {'born': '1841-02-25', 
                                                                    'dead': '1919-12-03', 
                                                                    'name': 'Pierre-Auguste Renoir', 
                                                                    'qualification': 'painter'}]}, 
                                           ...
                                           'title': 'The Umbrellas', 
                                           'year': '1886'}]},
                          {'attributes': {'available': True, 'id': 'b0836217463'}, 'children': [
                                        {'author': {'attributes': {'id': 'JM'}, 'children': [
                                                                 {'born': '1893-04-20', 
                                                                    'dead': '1983-12-25', 
                                                                    'name': 'Joan Miró', 
                                                                    'qualification': 'painter, sculptor and ceramicist'}]}, 
...
_COLLECTION_JSON_ML = [
 'col:collection',
 {'xmlns:col': 'http://example.com/ns/collection', 'xmlns:xsi': 'http://www.w3.org/2001/XMLSchema-instance', 
    'xsi:schemaLocation': 'http://example.com/ns/collection collection.xsd'},
 [
  'object', {'available': True, 'id': 'b0836217462'},
  [
   'position', 1],
  [
   'title', 'The Umbrellas'],
  ...
  [
   ...
   Decimal('10000.00')]],
 [
  'object', {'available': True, 'id': 'b0836217463'},
  [
   'position', 2],
  [
   'title'],
  ...
  [
   ...
   [
    'qualification', 'painter, sculptor and ceramicist']]]]
_DATA_DICT = {'@xmlns:ns': 'ns', 
   '@xmlns:xsi': 'http://www.w3.org/2001/XMLSchema-instance', 
   '@xsi:schemaLocation': 'ns ./simple-types.xsd', 
   'certification': [{'$': 'ISO-9001', '@Year': 1999}, {'$': 'ISO-27001', '@Year': 2009}], 'decimal_value': [
                   Decimal('1')], 
   'menù': 'baccalà mantecato', 
   'complex_boolean': [{'$': True, '@Type': 2}, {'$': False, '@Type': 1}, True, False], 'simple_boolean': [
                    True, False]}
...
def make_validator_test_class(test_file, test_args, test_num, schema_class, check_with_lxml):
    ...
    class TestValidator(XMLSchemaTestCase):
        ...
        def check_encoding_with_element_tree(self):
            ...
            self.check_etree_encode(root, cdata_prefix='#', **options)
        ...
        def check_decoding_and_encoding_with_lxml(self):
            ...
            if not errors:
                ...
                options = {'etree_element_class': lxml_etree_element, 
        ...
        def check_validate_and_is_valid_api(self):
            if expected_errors:
                ...
                self.assertEqual(self.schema.validate(xml_file), None, msg_tmpl % 'file without errors not validated')
            return

        def check_iter_errors(self):
            self.assertEqual(len(list(self.schema.iter_errors(xml_file))), expected_errors, msg_tmpl % 'wrong number of errors (%d expected)' % expected_errors)

        def check_lxml_validation(self):
            try:
                schema = lxml_etree.XMLSchema(self.lxml_schema.getroot())
            except lxml_etree.XMLSchemaParseError:
                print ('\nSkip lxml.etree.XMLSchema validation test for {!r} ({})').format(xml_file, self.__name__)
...
class TestValidation11(TestValidation):
    ...
    def test_default_attributes(self):
        ...
        self.assertTrue(xs.is_valid("<tree xmlns='ns'>   <node node-id='1'>alpha</node>   <node node-id='2' colour='red'>beta</node></tree>"))
        self.assertFalse(xs.is_valid("<tree xmlns='ns'>   <node>alpha</node>   <node node-id='2' colour='red'>beta</node></tree>"))
...
class TestDecoding(XMLSchemaTestCase):
    ...
    def check_decode(self, xsd_component, data, expected, **kwargs):
        if isinstance(expected, type) and issubclass(expected, Exception):
            ...
            if isinstance(obj, tuple) and len(obj) == 2 and isinstance(obj[1], list) and isinstance(obj[1][0], Exception):
    ...
    def test_validation_strict(self):
        self.assertRaises(xmlschema.XMLSchemaValidationError, self.vh_schema.to_dict, ElementTree.parse(self.casepath('examples/vehicles/vehicles-2_errors.xml')), validation='strict', namespaces=self.vh_namespaces)
    ...
    def test_datetime_types(self):
        ...
        self.assertEqual(xs.decode('<ns:dt xmlns:ns="ns">2019-01-01T13:40:00</ns:dt>', datetime_types=True), datatypes.DateTime10.fromstring('2019-01-01T13:40:00'))
        xs = self.get_schema('<element name="dt" type="date"/>')
        self.assertEqual(xs.decode('<ns:dt xmlns:ns="ns">2001-04-15</ns:dt>'), '2001-04-15')
        self.assertEqual(xs.decode('<ns:dt xmlns:ns="ns">2001-04-15</ns:dt>', datetime_types=True), datatypes.Date10.fromstring('2001-04-15'))
    ...
    def test_duration_type(self):
        ...
        self.assertEqual(xs.decode('<ns:td xmlns:ns="ns">P5Y3MT60H30.001S</ns:td>', datetime_types=True), datatypes.Duration.fromstring('P5Y3M2DT12H30.001S'))
    ...
    def test_converters(self):
        ...
        parker_dict_root = self.col_schema.to_dict(filename, converter=xmlschema.ParkerConverter(preserve_root=True), decimal_type=float)
        self.assertTrue(parker_dict_root == _COLLECTION_PARKER_ROOT)
        badgerfish_dict = self.col_schema.to_dict(filename, converter=xmlschema.BadgerFishConverter, decimal_type=float)
        self.assertTrue(badgerfish_dict == _COLLECTION_BADGERFISH)
        abdera_dict = self.col_schema.to_dict(filename, converter=xmlschema.AbderaConverter, decimal_type=float, dict_class=dict)
    ...
    def test_dict_granularity(self):
        ...
        self.assertTrue(isinstance(xml_data_1['bar'], type(xml_data_2['bar'])), msg='XSD with an array that return a single element from xml must still yield a list.')
    ...
    def test_any_type(self):
        ...
        self.assertEqual(any_type.decode(xml_data_2), (None, [], []))
        return
    ...
    def test_choice_model_decoding(self):
        ...
        self.assertEqual(data, {'@xmlns:xsi': 'http://www.w3.org/2001/XMLSchema-instance', 
    ...
    def test_cdata_decoding(self):
        ...
        self.assertEqual(schema.decode(xml_file, dict_class=ordered_dict_class, cdata_prefix='#'), ordered_dict_class([
         ('@xmlns:xsi', 'http://www.w3.org/2001/XMLSchema-instance'),
    ...
    def test_binary_data_facets(self):
        ...
        self.check_decode(base64_code_type, base64.b64encode('ok'), XMLSchemaValidationError)
        base64_value = base64.b64encode('hello')
        self.check_decode(base64_code_type, base64_value, base64_value.decode('utf-8'))
        self.check_decode(base64_code_type, base64.b64encode('abcefgh'), 'YWJjZWZnaA==')
        self.check_decode(base64_code_type, ' Y  W J j ZWZ\t\tn\na A= =', 'Y W J j ZWZ n a A= =')
        self.check_decode(base64_code_type, ' Y  W J j ZWZ\t\tn\na A= =', 'Y W J j ZWZ n a A= =')
        self.check_decode(base64_code_type, base64.b64encode('abcefghi'), 'YWJjZWZnaGk=')
        ...
        self.check_decode(base64_length4_type, base64.b64encode('abc'), XMLSchemaValidationError)
        self.check_decode(base64_length4_type, base64.b64encode('abce'), 'YWJjZQ==')
        self.check_decode(base64_length4_type, base64.b64encode('abcef'), XMLSchemaValidationError)
        base64_length5_type = self.st_schema.types['base64Length5']
        self.check_decode(base64_length5_type, base64.b64encode('1234'), XMLSchemaValidationError)
        self.check_decode(base64_length5_type, base64.b64encode('12345'), 'MTIzNDU=')
        self.check_decode(base64_length5_type, base64.b64encode('123456'), XMLSchemaValidationError)
    ...
    def test_default_namespace(self):
        ...
        self.assertEqual(xs.to_dict('<foo xmlns="http://example.com/foo">bar</foo>', path='/foo', namespaces={'': 'http://example.com/foo'}), 'bar')
        self.assertEqual(xs.to_dict('<foo>bar</foo>', path='/foo', namespaces={'': 'http://example.com/foo'}), None)
        return
...
class TestDecoding11(TestDecoding):
    ...
    def test_datetime_types(self):
        xs = self.get_schema('<element name="dt" type="dateTime"/>')
        self.assertEqual(xs.decode('<ns:dt xmlns:ns="ns">2019-01-01T13:40:00</ns:dt>'), '2019-01-01T13:40:00')
        self.assertEqual(xs.decode('<ns:dt xmlns:ns="ns">2019-01-01T13:40:00</ns:dt>', datetime_types=True), datatypes.DateTime.fromstring('2019-01-01T13:40:00'))
        xs = self.get_schema('<element name="dt" type="date"/>')
        self.assertEqual(xs.decode('<ns:dt xmlns:ns="ns">2001-04-15</ns:dt>'), '2001-04-15')
        self.assertEqual(xs.decode('<ns:dt xmlns:ns="ns">2001-04-15</ns:dt>', datetime_types=True), datatypes.Date.fromstring('2001-04-15'))
    ...
    def test_derived_duration_types(self):
        ...
        self.assertEqual(xs.decode('<ns:td xmlns:ns="ns">P2Y10M</ns:td>', datetime_types=True), datatypes.Duration.fromstring('P2Y10M'))
        ...
        self.assertEqual(xs.decode('<ns:td xmlns:ns="ns">P2DT6H30M30.001S</ns:td>', datetime_types=True), datatypes.Duration.fromstring('P2DT6H30M30.001S'))
...
class TestEncoding(XMLSchemaTestCase):
    ...
    def test_decode_encode(self):
        ...
        self.assertEqual(len([ e for e in elem.iter() ]), 20, msg='The encoded tree must have 20 elements as the origin.')
        self.assertTrue(all([ local_name(e1.tag) == local_name(e2.tag) for e1, e2 in zip(elem.iter(), xt.getroot().iter())
                            ]))
    ...
    def test_list_types(self):
        ...
        self.check_encode(list_of_strings, (10, 25, 40), '', validation='lax')
        ...
        self.check_encode(list_of_booleans, [True, False, 40.0], 'true false 40.0', validation='skip')
    ...
    def test_complex_elements(self):
        ...
        self.check_encode(schema.elements['A'], data={'@a1': 10, '@a2': -1, '$': 'simple '}, expected='<ns:A xmlns:ns="ns" a1="10" a2="-1">simple </ns:A>')
        self.check_encode(schema.elements['A'], {'@a1': 10, '@a2': -1, '$': 'simple '}, ElementTree.fromstring('<A xmlns="ns" a1="10" a2="-1">simple </A>'))
        self.check_encode(schema.elements['A'], {'@a1': 10, '@a2': -1}, ElementTree.fromstring('<A xmlns="ns" a1="10" a2="-1"/>'))
        self.check_encode(schema.elements['A'], {'@a1': 10, '$': 'simple '}, ElementTree.fromstring('<A xmlns="ns" a1="10">simple </A>'))
        ...
        self.check_encode(xsd_component=schema.elements['A'], data=ordered_dict_class([('B1', 'abc'), ('B2', 10), ('B3', False)]), expected='<ns:A xmlns:ns="ns">\n<B1>abc</B1>\n<B2>10</B2>\n<B3>false</B3>\n</ns:A>', indent=0)
        self.check_encode(schema.elements['A'], {'B1': 'abc', 'B2': 10, 'B4': False}, XMLSchemaValidationError)
        self.check_encode(xsd_component=schema.elements['A'], data=ordered_dict_class([('B1', 'abc'), ('B2', 10), ('#1', 'hello'), ('B3', True)]), expected='<ns:A xmlns:ns="ns">\n<B1>abc</B1>\n<B2>10</B2>\nhello\n<B3>true</B3>\n</ns:A>', indent=0, cdata_prefix='#')
        self.check_encode(xsd_component=schema.elements['A'], data=ordered_dict_class([('B1', 'abc'), ('B2', 10), ('#1', 'hello')]), expected=XMLSchemaValidationError, indent=0, cdata_prefix='#')

    def test_encode_datetime(self):
        xs = self.get_schema('<element name="dt" type="dateTime"/>')
        dt = xs.decode('<ns:dt xmlns:ns="ns">2019-01-01T13:40:00</ns:dt>', datetime_types=True)
        self.assertEqual(etree_tostring(xs.encode(dt)), '<ns:dt xmlns:ns="ns">2019-01-01T13:40:00</ns:dt>')

    def test_encode_date(self):
        xs = self.get_schema('<element name="dt" type="date"/>')
        date = xs.decode('<ns:dt xmlns:ns="ns">2001-04-15</ns:dt>', datetime_types=True)
        self.assertEqual(etree_tostring(xs.encode(date)), '<ns:dt xmlns:ns="ns">2001-04-15</ns:dt>')

    def test_duration(self):
        xs = self.get_schema('<element name="td" type="duration"/>')
        duration = xs.decode('<ns:td xmlns:ns="ns">P5Y3MT60H30.001S</ns:td>', datetime_types=True)
        self.assertEqual(etree_tostring(xs.encode(duration)), '<ns:td xmlns:ns="ns">P5Y3M2DT12H30.001S</ns:td>')
    ...
    def test_gregorian_year(self):
        ...
        self.assertEqual(etree_tostring(xs.encode(gyear)), '<ns:td xmlns:ns="ns">2000</ns:td>')
    ...
    def test_gregorian_yearmonth(self):
        ...
        self.assertEqual(etree_tostring(xs.encode(gyear_month)), '<ns:td xmlns:ns="ns">2000-12</ns:td>')
