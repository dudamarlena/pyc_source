...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ancypwn_backend_wsl2\backend.py
# Compiled at: 2019-08-02 23:45:32
import docker, appdirs, os, win32.servicemanager as servicemanager, win32api, win32serviceutil, win32service, win32event, multiprocessing, struct
...
def _figure_volumes(directory):
    r"""figures out the volumes to be binded
...
def _run_container(url, image_name, volumes, privileged):
    ...
    running = container.run(image_name, '/bin/bash', cap_add=[
     'SYS_ADMIN', 'SYS_PTRACE'], detach=True, tty=True, volumes=volumes, privileged=privileged, network_mode='host', remove=True)
