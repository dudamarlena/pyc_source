...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/rhubarbtart/threadobject.py
# Compiled at: 2006-01-27 01:56:27
...
class FakeObject(object):
    ...
    def _FAKE_getobj(self, attr=None):
        if not hasattr(self._FAKE_dispatcher, 'object'):
            if attr:
                raise AttributeError('Cannot access .%s: _FAKE_attach on object %s has not been called yet to attach a root object' % (attr, self._FAKE_description))
            else:
                raise AttributeError('_FAKE_attach on object %s has not been called yet to attach a root object' % self._FAKE_description)
    ...
    def _FAKE_attach(self, object):
        assert object is not self, 'Attempting to attach a threadlocal object to itself!'
    ...
    def _FAKE_detach(self, ignore_missing=False):
        if not hasattr(self._FAKE_dispatcher, 'object'):
            if ignore_missing:
                return
            raise AttributeError('You cannot detach the root object when no object has been attached yet')
...
for attr in ['__getitem__', '__setitem__', '__delitem__', '__contains__', '__iter__', '__len__', 'get', '__str__', '__add__']:
...
class FakeString(FakeObject):
    ...
    def __radd__(self, other):
        if isinstance(other, (str, unicode, self.__class__)):
            ...
            raise TypeError("I don't know how to add a %r to a string" % other)