...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/italianskin/tool/content/ItalianSkinTool.py
...
schema = Schema((StringField(name='validator_url', default='http://validator.w3.org/check', widget=StringWidget(label='URL validator', label_msgid='ItalianSkin_label_validator_url', description='URL del validatore che si vuole utilizzare (default: w3c)', description_msgid='ItalianSkin_help_validator_url', i18n_domain='ItalianSkin'), required=1), BooleanField(name='validation_action_enabled', default=True, accessor='isValidationActionEnabled', widget=BooleanWidget(label='Attiva la validazione integrata del codice', label_msgid='ItalianSkin_label_validationactionenabled', description='Attivando questa opzione avrai a disposizione uno strumento per la validazione integrata del codice', description_msgid='ItalianSkin_help_validationactionenabled', i18n_domain='ItalianSkin')), LinesField(name='class_titles', default=('Normal Text', 'Small Text', 'Large Text',
                                         'Plone Default (contrast improved)', 'High Contrast (background black)'), widget=LinesWidget(label='Class titles', label_msgid='italianskin.tool_label_class_titles', description='Add your CSS class titles', description_msgid='italianskin.tool_help_class_titles', i18n_domain='italianskin.tool'), required=1), LinesField(name='css_classes', default=('normalText', 'smallText', 'largeText', 'HighContrast',
                                        'HighContrastBlack'), widget=LinesWidget(label='CSS Classes', label_msgid='italianskin.tool_label_css_classes', description='Add your CSS class ids', description_msgid='italianskin.tool_help_css_classes', i18n_domain='italianskin.tool'), required=1)))
ItalianSkinTool_schema = BaseSchema.copy() + schema.copy()
...
class ItalianSkinTool(UniqueObject, BaseContent):
    ...
    actions = ({'action': 'string:${object_url}/is_tool_view', 'category': 'object', 'id': 'view', 'name': 'View', 'permissions': ('View', ), 'condition': 'python:0'}, {'id': 'metadata', 'name': 'Properties', 'action': 'string:${object_url}/base_metadata', 'permissions': (permissions.ModifyPortalContent,), 'condition': 'python:0'}, {'id': 'edit', 'name': 'Edit', 'action': 'string:${object_url}/is_tool_view', 'permissions': (permissions.ModifyPortalContent,), 'condition': 'python:0'})
    ...
    def getValidationResults(self, xhtml):
        results = {'m:errors': {'m:errorcount': 0, 'm:errorlist': []}, 'm:warnings': {'m:warningcount': 0, 'm:warninglist': []}, 'm:validity': None}
    ...
    def getStyles(self):
        """ Return a list of dictionary like {'title':'My Class Title', 'class':'my_class'}"""
