...
# Embedded file name: build/bdist.linux-x86_64/egg/chombopy/xarray.py
# Compiled at: 2020-05-04 14:00:23
# Size of source mod 2**32: 13147 bytes
import xarray as xr
...
import matplotlib as mpl, logging, warnings
...
class AMRDataset(object):
    ...
    def get_amr_data(self, component, levels=None, valid_only=True):
        ...
        ds_levs = [self.get_level_data(component, level, valid_only) for level in levels]
        return ds_levs
    ...
    def get_outline(self, level):
        ...
        for box in self.get_boxes(level):
            ...
            end_points = [[lo_vals[i] - self.get_dx(level) / 2, hi_vals[i] + self.get_dx(level) / 2] for i in range(self.space_dim)]
    ...
    def plot(self, component, levels=None):
        ...
        for level in levels:
            ...
            ax.pcolormesh((coords[dims[0]]), (coords[dims[1]]), (self.get_level_data(component, level)),
            ...
            if level > 0:
                ...
                outline.plot(ax=ax, edgecolor=(colors[(level - 1)]), facecolor=[1, 1, 1, 0], linewidth=3.0)
