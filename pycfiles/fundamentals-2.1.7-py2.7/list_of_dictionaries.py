...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fundamentals/renderer/list_of_dictionaries.py
...
import sys, os, io, re, codecs, copy, json, yaml
    ...
    **Usage**
    ...
    def json(self, filepath=None):
        ...
        **Usage**
        ...
        renderedData = json.dumps(dataCopy, separators=(',', ': '), sort_keys=True, indent=4)
        if filepath and len(self.listOfDictionaries):
            if not os.path.exists(os.path.dirname(filepath)):
                os.makedirs(os.path.dirname(filepath))
            writeFile = codecs.open(filepath, encoding='utf-8', mode='w')
    ...
    def mysql(self, tableName, filepath=None, createStatement=None):
        ...
        **Usage**
        ...
        if createStatement and 'create table if not exists' not in createStatement.lower():
            regex = re.compile('^\\s*CREATE TABLE ', re.I | re.S)
            createStatement = regex.sub('CREATE TABLE IF NOT EXISTS ', createStatement)
        renderedData = self._list_of_dictionaries_to_mysql_inserts(tableName=tableName, createStatement=createStatement)
        if filepath and len(self.listOfDictionaries):
            if not os.path.exists(os.path.dirname(filepath)):
                os.makedirs(os.path.dirname(filepath))
    ...
    def _list_of_dictionaries_to_csv(self, csvType='human'):
        ...
        **Return**
        ...
        if not len(self.listOfDictionaries):
            ...
            columnWidths = []
            columnWidths[:] = [ len(tableColumnNames[i]) for i in range(len(tableColumnNames))
                              ]
            output = io.BytesIO()
            if csvType == 'machine':
                delimiter = ','
                ...
                if csvType in ('human', 'markdown'):
                    ...
                    if csvType in ('markdown', ):
                        writer = csv.writer(output, delimiter=delimiter, quoting=csv.QUOTE_NONE, doublequote=False, quotechar='"', escapechar='\\', lineterminator='\n')
                    else:
                        writer = csv.writer(output, dialect='excel', delimiter=delimiter, quotechar='"', quoting=csv.QUOTE_MINIMAL, lineterminator='\n')
                    if csvType in ('markdown', ):
                        dividerWriter = csv.writer(output, delimiter='|', quoting=csv.QUOTE_NONE, doublequote=False, quotechar='"', escapechar='\\', lineterminator='\n')
                    else:
                        dividerWriter = csv.writer(output, dialect='excel', delimiter='+', quotechar='"', quoting=csv.QUOTE_MINIMAL, lineterminator='\n')
                    header = []
                    divider = []
                    rstDivider = []
                    allRows = []
                ...
                for row in dataCopy:
                    ...
                    for i, c in enumerate(tableColumnNames):
                        if csvType in ('human', 'markdown', 'reST'):
                            if row[c] == None:
                                row[c] = ''
                            row[c] = str(str(row[c]).ljust(columnWidths[i] + 2).rjust(columnWidths[i] + 3))
