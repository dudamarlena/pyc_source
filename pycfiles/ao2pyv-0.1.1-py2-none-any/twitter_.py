...
# Embedded file name: build/bdist.linux-x86_64/egg/ao/social/twitter_.py
# Compiled at: 2010-04-14 10:08:44
import urllib
...
class TwitterClient(OAuthClient):
    ...
    _default_config = {'request_token_url': 'https://twitter.com/oauth/request_token', 
    ...
    def post(self, text, token='', secret=''):
        ...
        url = self._config['update_url'] % {'format': 'json'}
        data = {'status': text.encode('utf-8')}
        data = urllib.urlencode(data)
        token = oauth.OAuthToken(token, secret)
        request = oauth.OAuthRequest.from_consumer_and_token(self._consumer, token=token, http_url=url, http_method='POST')
        request.set_parameter('status', text)
        request.sign_request(self._signature_method, self._consumer, token)
        response = urlfetch.fetch(url, payload=data, method=urlfetch.POST, headers=request.to_header())
        return json.loads(response.content)