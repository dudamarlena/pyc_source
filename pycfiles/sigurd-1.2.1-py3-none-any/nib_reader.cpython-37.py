...
nib_directories = [
 'nib_a_00', 'nib_b_00', 'nib_c_00', 'nib_n_00']
...
def read_tei(main_link):
    ...
    segments = [[[segment.text for segment in line.findall('.//n:seg', namespaces=NAMESPACES)] for line in chapter.findall('.//n:l', namespaces=NAMESPACES)] for chapter in chapters]
    return segments
...
def read_places():
    ...
    places = [line.split(':') for line in lines if len(line) > 0 if not line.startswith('#')]
    places = {place[0]:[p for p in place[1].split(' ') if p] for place in places if len(place) == 2 if len(place) == 2}
    return places
...
def read_names():
    with codecs.open((os.path.join(PACKDIR, 'nib_augsburg', 'annotations', 'names.txt')), encoding='utf-8') as (f):
        lines = f.read().strip().split(os.linesep)
    names = [line.split(':') for line in lines if len(line) > 0 if not line.startswith('#')]
    names = {name[0]:[n.strip() for n in name[1].split(' ') if n] for name in names if len(name) == 2 if len(name) == 2}
    return names