...
# [GCC 8.4.0]
# Embedded file name: /home/bouma/projects/pythonunited/provgroningen/buildout/src/djinn_core/djinn_core/utils.py
# Compiled at: 2015-09-10 04:33:25
...
def _class_implements(clazz, superclazz, check_self=True):
    ...
    if check_self and clazz == superclazz:
        return True
    if isinstance(superclazz, basestring) and superclazz in map(lambda x: '%s.%s' % (
...
def object_to_urn(object):
    ...
    return URN_SCHEMA % {'object_app': app_label, 'object_ctype': ct_name, 
       'object_id': object.id}
...
class HTMLTruncate(HTMLParser):
    ...
    def _flatten_attrs(self, attrs):
        if attrs:
            return ' ' + (' ').join([ '%s="%s"' % (name, val) for name, val in attrs ])
        else:
            return ''
