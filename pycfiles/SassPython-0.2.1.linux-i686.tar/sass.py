...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/sass.py
# Compiled at: 2012-08-08 03:38:16
from ctypes import *
...
class Context(Structure):
    ...
    def __str__(self):
        return ('<context source="{source_string} output="{output_string}" status="{error_status}" error="{error_message}>"').format(source_string=self.source_string, output_string=self.output_string, error_status=self.error_status, error_message=self.error_message)
...
def compile(scss):
    ...
    if ctx.error_status:
        ...
        return (
         True, to_string(ctx.output_string))
...
def compile_path(path):
    ...
    if fctx.error_status:
        ...
        return (
         True, to_string(fctx.output_string))
...
def compile_folder(path):
    ...
    if dctx.error_status:
        ...
        return (
         True, to_string(dctx.output_string))
...
def build_arg_parser():
    ...
    group.add_argument('-f', '--file', type=str, default=None, dest='file_path', help='file to convert')
    group.add_argument('-d', '--dir', type=str, default=None, dest='dir_path', help='directory to convert')
    return parser
