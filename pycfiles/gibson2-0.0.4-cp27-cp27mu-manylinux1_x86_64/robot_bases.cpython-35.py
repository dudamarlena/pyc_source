...
# [GCC 8.4.0]
# Embedded file name: /home/fei/Development/gibsonv2/gibson2/core/physics/robot_bases.py
# Compiled at: 2020-04-07 16:11:37
# Size of source mod 2**32: 14367 bytes
import pybullet as p, gym, gym.spaces, gym.utils, numpy as np, os, inspect, pybullet_data
...
class BaseRobot:
    ...
    def load(self):
        ...
        if self.model_type == 'URDF':
            self.robot_ids = (
             p.loadURDF(os.path.join(self.physics_model_dir, self.model_file), globalScaling=self.scale, flags=flags),)
...
class BodyPart:
    ...
    def get_linear_velocity(self):
        if self.body_part_index == -1:
            ...
            _, _, _, _, _, _, (vx, vy, vz), _ = p.getLinkState(self.bodies[self.body_index], self.body_part_index, computeLinkVelocity=1)
    ...
    def get_angular_velocity(self):
        if self.body_part_index == -1:
            ...
            _, _, _, _, _, _, _, (vr, vp, vyaw) = p.getLinkState(self.bodies[self.body_index], self.body_part_index, computeLinkVelocity=1)
...
class Joint:
    ...
    def __init__(self, joint_name, bodies, body_index, joint_index):
        ...
        _, _, self.joint_type, _, _, _, _, _, self.lower_limit, self.upper_limit, self.max_torque, self.max_velocity, _, _, _, _, _ = p.getJointInfo(self.bodies[self.body_index], self.joint_index)
    ...
    def set_position(self, position):
        ...
        p.setJointMotorControl2(self.bodies[self.body_index], self.joint_index, p.POSITION_CONTROL, targetPosition=position)

    def set_velocity(self, velocity):
        """Set velocity of joint"""
        velocity = np.clip(velocity, -self.max_velocity, self.max_velocity)
        p.setJointMotorControl2(self.bodies[self.body_index], self.joint_index, p.VELOCITY_CONTROL, targetVelocity=velocity)

    def set_torque(self, torque):
        torque = np.clip(torque, -self.max_torque, self.max_torque)
        p.setJointMotorControl2(bodyIndex=self.bodies[self.body_index], jointIndex=self.joint_index, controlMode=p.TORQUE_CONTROL, force=torque)
    ...
    def disable_motor(self):
        p.setJointMotorControl2(self.bodies[self.body_index], self.joint_index, controlMode=p.POSITION_CONTROL, targetPosition=0, targetVelocity=0, positionGain=0.1, velocityGain=0.1, force=0)

    def get_joint_relative_state(self):
    ...
    def reset_joint_state(self, position, velocity):
