...
# [GCC 8.4.0]
# Embedded file name: /home/fei/Development/gibsonv2/gibson2/core/simulator.py
...
import pybullet as p, gibson2, os, numpy as np

class Simulator:

    def __init__(self, gravity=9.8, timestep=0.004166666666666667, use_fisheye=False, mode='gui', image_width=128, image_height=128, vertical_fov=90, device_idx=0, render_to_tensor=False):
        """
    ...
    def load(self):
        ...
        if self.render_to_tensor:
            self.renderer = MeshRendererG2G(width=self.image_width, height=self.image_height, vertical_fov=self.vertical_fov, device_idx=self.device_idx, use_fisheye=self.use_fisheye)
        else:
            self.renderer = MeshRenderer(width=self.image_width, height=self.image_height, vertical_fov=self.vertical_fov, device_idx=self.device_idx, use_fisheye=self.use_fisheye)
        if self.mode == 'gui':
            ...
            for shape in p.getVisualShapeData(new_object):
                ...
                if type == p.GEOM_MESH:
                    ...
                    if filename not in self.visual_objects.keys():
                        self.renderer.load_object(filename, texture_scale=texture_scale, load_texture=load_texture)
                ...
                if visual_object is not None:
                    self.renderer.add_instance(visual_object, pybullet_uuid=new_object, class_id=class_id)
        ...
        for shape in p.getVisualShapeData(new_object):
            ...
            if type == p.GEOM_MESH:
                ...
                if type == p.GEOM_SPHERE:
                    ...
                    self.renderer.load_object(filename, transform_orn=rel_orn, transform_pos=rel_pos, input_kd=color[:3], scale=[
                     dimensions[0] / 0.5, dimensions[0] / 0.5, dimensions[0] / 0.5])
                    visual_object = len(self.renderer.visual_objects) - 1
                else:
                    if type == p.GEOM_CAPSULE or type == p.GEOM_CYLINDER:
                        ...
                        visual_object = len(self.renderer.visual_objects) - 1
            if visual_object is not None:
                self.renderer.add_instance(visual_object, pybullet_uuid=new_object, class_id=class_id, dynamic=True, softbody=softbody)
    ...
    def import_robot(self, robot, class_id=None):
        ...
        self.renderer.add_robot(object_ids=visual_objects, link_ids=link_ids, pybullet_uuid=ids[0], class_id=class_id, poses_rot=poses_rot, poses_trans=poses_trans, dynamic=True, robot=robot)
        return ids
    ...
    def import_articulated_object(self, obj, class_id=None):
        ...
        self.renderer.add_instance_group(object_ids=visual_objects, link_ids=link_ids, pybullet_uuid=ids, class_id=class_id, poses_rot=poses_rot, poses_trans=poses_trans, dynamic=True, robot=None)
        return ids
