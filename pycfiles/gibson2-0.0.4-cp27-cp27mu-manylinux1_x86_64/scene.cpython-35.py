...
# [GCC 8.4.0]
# Embedded file name: /home/fei/Development/gibsonv2/gibson2/core/physics/scene.py
# Compiled at: 2020-04-07 16:11:37
# Size of source mod 2**32: 16875 bytes
import pybullet as p, os, inspect
...
class BuildingScene(Scene):
    ...
    def __init__(self, model_id, trav_map_resolution=0.1, trav_map_erosion=2, build_graph=True, is_interactive=False, num_waypoints=10, waypoint_resolution=0.2, pybullet_load_texture=False):
        """
    ...
    def load_floor_planes(self):
        if self.is_interactive:
            for f in range(len(self.floors)):
                ...
                collision_id = p.createCollisionShape(p.GEOM_MESH, fileName=plane_name)
                visual_id = p.createVisualShape(p.GEOM_MESH, fileName=plane_name)
                texture_filename = get_texture_file(plane_name)
                texture_id = p.loadTexture(texture_filename)
                floor_body_id = p.createMultiBody(baseCollisionShapeIndex=collision_id, baseVisualShapeIndex=visual_id)
                p.changeVisualShape(floor_body_id, -1, textureUniqueId=texture_id)
                floor_height = self.floors[f]
                p.resetBasePositionAndOrientation(floor_body_id, posObj=[
                 0, 0, floor_height], ornObj=[
                 0, 0, 0, 1])
    ...
    def load_trav_map(self):
        ...
        self.floor_graph = []
        for f in range(len(self.floors)):
            trav_map = np.array(Image.open(os.path.join(get_model_path(self.model_id), 'floor_trav_{}.png'.format(f))))
            obstacle_map = np.array(Image.open(os.path.join(get_model_path(self.model_id), 'floor_{}.png'.format(f))))
    ...
    def load(self):
        ...
        return [
         self.mesh_body_id] + self.floor_body_ids
    ...
    def reset_floor(self, floor=0, additional_elevation=0.02, height=None):
        ...
        p.resetBasePositionAndOrientation(self.floor_body_ids[0], posObj=[
         0, 0, height], ornObj=[
         0, 0, 0, 1])
