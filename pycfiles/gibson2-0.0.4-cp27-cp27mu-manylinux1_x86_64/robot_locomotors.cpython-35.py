...
# [GCC 8.4.0]
# Embedded file name: /home/fei/Development/gibsonv2/gibson2/core/physics/robot_locomotors.py
# Compiled at: 2020-04-07 16:11:37
# Size of source mod 2**32: 30955 bytes
from gibson2.core.physics.robot_bases import BaseRobot
from gibson2.utils.utils import rotate_vector_3d
import numpy as np, pybullet as p, os, gym, gym.spaces
from transforms3d.euler import euler2quat, euler2mat
from transforms3d.quaternions import quat2mat, qmult
import transforms3d.quaternions as quat, sys
from gibson2.external.pybullet_tools.utils import set_base_values, joint_from_name, set_joint_position, set_joint_positions, add_data_path, connect, plan_base_motion, plan_joint_motion, enable_gravity, joint_controller, dump_body, load_model, joints_from_names, user_input, disconnect, get_joint_positions, get_link_pose, link_from_name, HideOutput, get_pose, wait_for_user, dump_world, plan_nonholonomic_motion, set_point, create_box, stable_z, control_joints, get_max_limits, get_min_limits, get_base_values

class LocomotorRobot(BaseRobot):
    __doc__ = ' Built on top of BaseRobot\n    '

    def __init__(self, filename, action_dim, base_name=None, scale=1.0, control='torque', is_discrete=True, torque_coef=1.0, velocity_coef=1.0, self_collision=False):
        ...
        if self.control == 'torque':
            ...
            if self.control == 'velocity':
                ...
                if self.control == 'position':
                    ...
                    if self.control == 'differential_drive':
                        ...
                        if type(self.control) is list or type(self.control) is tuple:
    ...
    def policy_action_to_robot_action(self, action):
        if self.is_discrete:
            if isinstance(action, (list, np.ndarray)):
                assert len(action) == 1 and isinstance(action[0], (np.int64, int)), 'discrete action has incorrect format'
                action = action[0]
            real_action = self.action_list[action]
        else:
            action = np.clip(action, self.action_space.low, self.action_space.high)
            real_action = (self.action_high - self.action_low) / 2.0 * action + (self.action_high + self.action_low) / 2.0
        return real_action
...
class Ant(LocomotorRobot):
    ...
    def __init__(self, config):
        ...
        LocomotorRobot.__init__(self, 'ant/ant.xml', action_dim=8, torque_coef=2.5, scale=config.get('robot_scale', 1.0), is_discrete=config.get('is_discrete', False), control='torque')
...
class Humanoid(LocomotorRobot):
    ...
    def __init__(self, config):
        ...
        LocomotorRobot.__init__(self, 'humanoid/humanoid.xml', action_dim=17, torque_coef=0.41, scale=config.get('robot_scale', 1.0), is_discrete=config.get('is_discrete', False), control='torque', self_collision=True)
    ...
    def set_up_discrete_action_space(self):
        self.action_list = [
         [
          self.torque] * self.action_dim, [0.0] * self.action_dim]
    ...
    def robot_specific_reset(self):
        ...
        if self.glass_id is None:
            ...
            cid = p.createConstraint(humanoidId, -1, self.glass_id, -1, p.JOINT_FIXED, jointAxis=[
             0, 0, 0], parentFramePosition=[
             0, 0, self.glass_offset], childFramePosition=[
             0, 0, 0])
...
class Husky(LocomotorRobot):
    ...
    def __init__(self, config):
        ...
        LocomotorRobot.__init__(self, 'husky/husky.urdf', action_dim=4, torque_coef=2.5, scale=config.get('robot_scale', 1.0), is_discrete=config.get('is_discrete', False), control='torque')
...
class Quadrotor(LocomotorRobot):
    ...
    def __init__(self, config):
        ...
        LocomotorRobot.__init__(self, 'quadrotor/quadrotor.urdf', action_dim=6, torque_coef=2.5, scale=config.get('robot_scale', 1.0), is_discrete=config.get('is_discrete', False), control='torque')
    ...
    def set_up_discrete_action_space(self):
        self.action_list = [
         [
          self.torque, 0, 0, 0, 0, 0], [-self.torque, 0, 0, 0, 0, 0],
         [
...
class Turtlebot(LocomotorRobot):
    ...
    def __init__(self, config):
        ...
        LocomotorRobot.__init__(self, 'turtlebot/turtlebot.urdf', action_dim=2, scale=config.get('robot_scale', 1.0), is_discrete=config.get('is_discrete', False), control='velocity')
    ...
    def set_up_discrete_action_space(self):
        self.action_list = [
         [
          self.velocity, self.velocity], [-self.velocity, -self.velocity],
         [
...
class Freight(LocomotorRobot):
    ...
    def __init__(self, config):
        self.config = config
        self.velocity = config.get('velocity', 1.0)
        LocomotorRobot.__init__(self, 'fetch/freight.urdf', action_dim=2, scale=config.get('robot_scale', 1.0), is_discrete=config.get('is_discrete', False), control='velocity')
...
class Fetch(LocomotorRobot):
    ...
    def __init__(self, config):
        ...
        self.arm_dim = 7
        LocomotorRobot.__init__(self, 'fetch/fetch.urdf', action_dim=self.wheel_dim + self.torso_lift_dim + self.arm_dim, scale=config.get('robot_scale', 1.0), is_discrete=config.get('is_discrete', False), control='velocity', self_collision=True)

    def set_up_continuous_action_space(self):
        self.action_high = np.array([
         self.wheel_velocity] * self.wheel_dim + [self.torso_lift_velocity] * self.torso_lift_dim + [
    ...
    def robot_specific_reset(self):
        ...
        arm_joints = joints_from_names(robot_id, [
         ...
         'wrist_roll_joint'])
        rest_position = (
         0.02, np.pi / 2.0 - 0.4, np.pi / 2.0 - 0.1, -0.4, np.pi / 2.0 + 0.1, 0.0, np.pi / 2.0, 0.0)
...
class JR2(LocomotorRobot):
    ...
    def __init__(self, config):
        self.config = config
        self.velocity = config.get('velocity', 1.0)
        LocomotorRobot.__init__(self, 'jr2_urdf/jr2.urdf', action_dim=4, scale=config.get('robot_scale', 1.0), is_discrete=config.get('is_discrete', True), control='velocity')
...
class JR2_Kinova(LocomotorRobot):
    ...
    def __init__(self, config):
        ...
        self.arm_velocity = config.get('arm_velocity', 1.0)
        self.arm_dim = 5
        LocomotorRobot.__init__(self, 'jr2_urdf/jr2_kinova.urdf', action_dim=self.wheel_dim + self.arm_dim, scale=config.get('robot_scale', 1.0), is_discrete=config.get('is_discrete', False), control='velocity', self_collision=True)
...
class Locobot(LocomotorRobot):
    ...
    def __init__(self, config):
        ...
        self.wheel_dim = 2
        self.wheel_axle_half = 0.115
        self.wheel_radius = 0.038
        LocomotorRobot.__init__(self, 'locobot/locobot.urdf', base_name='base_link', action_dim=self.wheel_dim, scale=config.get('robot_scale', 1.0), is_discrete=config.get('is_discrete', False), control='differential_drive')
