...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/easyshop/kss/customers/address.py
...
class AddressKSSView(PloneKSSView):
    ...
    def showEditAddressForm(self, form):
        ...
        brains = catalog.searchResults(UID=form.get('uid', ''))
        try:
    ...
    def addAddress(self, form):
        ...
        kss_zope.refreshViewlet(selector, manager='easyshop.manager.addresses', name='easyshop.addresses')
    ...
    def _getAddAddressForm(self):
        """
        """
        html = '<h1>%s</h1>' % _('Add Address')
        html += '<div class="description">%s</div>' % _('Please enter a new address and click save.')
