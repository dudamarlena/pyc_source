...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/elaphe/datamatrix.py
...
class DataMatrix(Barcode):
    ...
    class _Renderer(MatrixCodeRenderer):
        default_options = dict(MatrixCodeRenderer.default_options, rows=0, columns=0, prefix='', encoding='ascii', raw=False, parse=False, parsefnc=False)
        ...
        def _code_bbox(self, codestring):
            ...
            if encoding == 'raw':
                ...
                if encoding == 'ascii':
                    if parsefnc == True:
                        cw_length += len(re.findall('(?<!\\^)\\^FNC1', codestring))
                        codestring = re.sub('(?<!\\^)\\^FNC1', '', codestring)
                        codestring.replace('^^', '^')
                    residue = codestring
                    while residue:
                    ...
                    enc_props = dict(c40=dict(mode=230, eightbits=True, charmap=''), text=dict(mode=239, eightbits=True, charmap=''), x12=dict(mode=238, eightbits=False, parsefnc=False, charmap=''))
                    ...
                    if parsefnc == True:
                        cw_length += 2 * len(re.findall('(?<!\\^)\\^FNC1', codestring))
                        codestring = re.sub('(?<!\\^)\\^FNC1', '', codestring)
                        codestring.replace('^^', '^')
                    residue = codestring
                    while residue:
                        ...
                        if 0 < ord(ch) < 128:
                            ...
                            if m_ord != 255:
                                ...
                                raise ValueError('Invalid character in X12 encoding')
            ...
            return (
             0, 0, DPI * cols / 72.0 * 2, DPI * rows / 72.0 * 2)
        ...
        def build_params(self, codestring):
            ...
            params['bbox'] = '%d %d %d %d' % self._boundingbox(self._code_bbox(codestring), self._code_bbox(codestring))
            return params
