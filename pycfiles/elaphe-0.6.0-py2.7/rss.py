...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/elaphe/rss.py
...
class Rss14(Barcode):
    ...
    class _Renderer(LinearCodeRenderer):
        default_options = dict(LinearCodeRenderer.default_options, height=33 / 72.0, textyoffset=-7, linkage=False)
        _symbol_length = 97
        ...
        def _text_bbox(self, codestring):
            ...
            if self.lookup_option('includetext'):
                ...
                return [
                 textminx, textyoffset, textmaxx, textyoffset + textsize]
            else:
        ...
        def build_params(self, codestring):
            ...
            params['bbox'] = '%d %d %d %d' % self._boundingbox(self._code_bbox(codestring), self._text_bbox(codestring))
            return params
...
class RssLimited(Barcode):
    ...
    aliases = ('rsslimited', 'rss limited', 'rss_limited', 'rss-limited', 'rss14limited',
               'rss14 limited', 'rss14_limited', 'rss14-limited')

    class _Renderer(Rss14._Renderer):
        default_options = dict(LinearCodeRenderer.default_options, textyoffset=-7, height=10 / 72.0)
        _symbol_length = 74
...
class RssExpanded(Barcode):
    ...
    aliases = ('rssexpanded', 'rss expanded', 'rss_expanded', 'rss-expanded', 'rss14expanded',
               'rss14 expanded', 'rss14_expanded', 'rss14-expanded')

    class _Renderer(Rss14._Renderer):
        default_options = dict(LinearCodeRenderer.default_options, height=34 / 72.0)
