...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/elaphe/onecode.py
...
class OneCode(Barcode):
    ...
    class _Renderer(LinearCodeRenderer):
        default_options = dict(LinearCodeRenderer.default_options, height=0.175, textyoffset=-7, textsize=12)

        @property
        def code_bbox(self):
            height = self.lookup_option('height')
            return [0, 0, 64 * 3.3120000000000003 + 1.44, height * DPI]
        ...
        def text_bbox(self):
            ...
            if self.lookup_option('includetext'):
                return [0, textyoffset, 85 + textsize * 0.6, textyoffset + textsize]
            else:
