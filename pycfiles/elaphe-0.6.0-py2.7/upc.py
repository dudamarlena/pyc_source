...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/elaphe/upc.py
...
class UpcA(Barcode):
    ...
    class _Renderer(LinearCodeRenderer):
        ...
        def code_bbox(self):
            ...
            return [0, 0, 3 + 84 + 3 + 5, DPI * height]
...
class UpcE(Barcode):
    ...
    class _Renderer(LinearCodeRenderer):
        default_options = dict(LinearCodeRenderer.default_options, textsize=12, textyoffset=-4, height=1)

        @property
        def code_bbox(self):
            height = self.lookup_option('height')
            return [0, 0, 4 + 42 + 7, DPI * height]
        ...
        def text_bbox(self):
            ...
            textmaxh = textyoffset + textsize
            if self.lookup_option('includetext'):
                return [-7, textyoffset, 53 + textsize * 0.6, textmaxh]
            else:
                return self.code_bbox
