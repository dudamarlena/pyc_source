...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/elaphe/azteccode.py
...
class AztecCode(Barcode):
    ...
    class _Renderer(MatrixCodeRenderer):
        default_options = dict(MatrixCodeRenderer.default_options, readerinit=False, layers=-1, eclevel=23, ecaddchars=3, format=None, parse=False, raw=False, dontdraw=False)
        ...
        def _code_bbox(self, codestring):
            ...
            for frmt, mlyr, icap, ncws, bpcw in AZTEC_CODE_METRICS:
                ...
                raise ValueError('No appropreate mode.')
            ...
            if format_ == 'full':
                ...
                size = 9 + layers * 4 + 2
            return (
             0, 0, DPI * size * 2 / 72.0, DPI * size * 2 / 72.0)
