...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/elaphe/util.py
...
_cap_escape_re = re.compile('^\\^\\d\\d\\d')
...
def dict_to_optstring(d, none=lambda x: '<>', empty=lambda x: '<>', raw_none=lambda x: '()', raw_empty=lambda x: '()', raw=True):
    r"""Converts dictionary into ps string in barcode.ps specific format.
    ...
    if d is None:
        ...
        if d:
            ret = (' ').join(key + {True: '', False: '=%s' % to_ps(value)}[(value is True)] for key, value in d.items() if value is not False)
            if raw:
                return '(' + ret + ')'
