...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/elaphe/qrcode.py
# Compiled at: 2013-12-05 09:10:25
import codecs, itertools
from base import Barcode, MatrixCodeRenderer, DPI
SYMBOL_CAPACITIES = dict(fullcaps=dict(numeric=dict(L=[
 ...
 4417, 4686, 4965, 5253, 5529, 5836, 6153, 6479, 6743, 7089], M=[
 ...
 3486, 3693, 3909, 4134, 4343, 4588, 4775, 5039, 5313, 5596], Q=[
 ...
 2473, 2670, 2805, 2949, 3081, 3244, 3417, 3599, 3791, 3993], H=[
 ...
 1897, 2022, 2157, 2301, 2361, 2524, 2625, 2735, 2927, 3057]), alphanumeric=dict(L=[
 ...
 2677, 2840, 3009, 3183, 3351, 3537, 3729, 3927, 4087, 4296], M=[
 ...
 2113, 2238, 2369, 2506, 2632, 2780, 2894, 3054, 3220, 3391], Q=[
 16, 29, 47, 67, 87, 108, 125, 157, 189, 221,
 259, 296, 352, 376, 426, 470, 531, 574, 644, 702,
 742, 823, 890, 963, 1041, 1094, 1172, 1263, 1322, 1429,
 1499, 1618, 1700, 1787, 1867, 1966, 2071, 2181, 2298, 2420], H=[
 10, 20, 35, 50, 64, 84, 93, 122, 143, 174,
 200, 227, 259, 283, 321, 365, 408, 452, 493, 557,
 587, 640, 672, 744, 779, 864, 910, 958, 1016, 1080,
 1150, 1226, 1307, 1394, 1431, 1530, 1591, 1658, 1774, 1852]), byte=dict(L=[
 17, 32, 53, 78, 106, 134, 154, 192, 230, 271,
 321, 367, 425, 458, 520, 586, 644, 718, 792, 858,
 929, 1003, 1091, 1171, 1273, 1367, 1465, 1528, 1628, 1732,
 1840, 1952, 2068, 2188, 2303, 2431, 2563, 2699, 2809, 2953], M=[
 14, 26, 42, 62, 84, 106, 122, 152, 180, 213,
 251, 287, 331, 362, 412, 450, 504, 560, 624, 666,
 711, 779, 857, 911, 997, 1059, 1125, 1190, 1264, 1370,
 1452, 1538, 1628, 1722, 1809, 1911, 1989, 2099, 2213, 2331], Q=[
 11, 20, 32, 46, 60, 74, 86, 108, 130, 151,
 177, 203, 241, 258, 292, 322, 364, 394, 442, 482,
 509, 565, 611, 661, 715, 751, 805, 868, 908, 982,
 1030, 1112, 1168, 1228, 1283, 1351, 1423, 1499, 1579, 1663], H=[
 7, 14, 24, 34, 44, 58, 64, 84, 98, 119,
 137, 155, 177, 194, 220, 250, 280, 310, 338, 382,
 403, 439, 461, 511, 535, 593, 625, 658, 698, 742,
 790, 842, 898, 958, 983, 1051, 1093, 1139, 1219, 1273]), kanji=dict(L=[
 10, 20, 32, 48, 65, 82, 95, 118, 141, 167,
 198, 226, 262, 282, 320, 361, 397, 442, 488, 528,
 572, 618, 672, 721, 784, 842, 902, 940, 1002, 1066,
 1132, 1201, 1273, 1347, 1417, 1496, 1577, 1661, 1729, 1817], M=[
 8, 16, 26, 38, 52, 65, 75, 93, 111, 131,
 155, 177, 204, 223, 254, 277, 310, 345, 384, 410,
 438, 480, 528, 561, 614, 652, 692, 732, 778, 843,
 894, 947, 1002, 1060, 1113, 1176, 1224, 1292, 1362, 1435], Q=[
 7, 12, 20, 28, 37, 45, 53, 66, 80, 93,
 109, 125, 149, 159, 180, 198, 224, 243, 272, 297,
 314, 348, 376, 407, 440, 462, 496, 534, 559, 604,
 634, 684, 719, 756, 790, 832, 876, 923, 972, 1024], H=[
 4, 8, 15, 21, 27, 36, 39, 52, 60, 74,
 85, 96, 109, 120, 136, 154, 173, 191, 208, 235,
 248, 270, 284, 315, 330, 365, 385, 405, 430, 457,
 486, 518, 553, 590, 605, 647, 673, 701, 750, 784])), microcaps=dict(numeric=dict(L=[
 5, 10, 23, 35], M=[-1, 8, 18, 30], Q=[
 -1, -1, -1, 21], H=[-1, -1, -1, -1]), alphanumeric=dict(L=[
 -1, 6, 14, 21], M=[-1, 5, 11, 18], Q=[
 -1, -1, -1, 13], H=[-1, -1, -1, -1]), byte=dict(L=[
 -1, -1, 9, 15], M=[-1, -1, 7, 13], Q=[
 -1, -1, -1, 9], H=[-1, -1, -1, -1]), kanji=dict(L=[
 -1, -1, 6, 9], M=[-1, -1, 4, 8], Q=[
 -1, -1, -1, 5], H=[-1, -1, -1, -1])))
QRCODE_METRIC = [
 [
...
def qrcode_metric(msgbits, encoding=None, format_='full', eclevel=None, version=None):
    ...
    if encoding == 'raw':
        ...
        caps_for_enc = SYMBOL_CAPACITIES.get('fullcaps' if format_ == 'full' else 'microcaps').get(encoding)
        if version is None:
            ...
            if eclevel == 'Q' and n_chars <= caps_for_enc['H'][version_idx]:
                eclevel = 'H'
            encvals = dict(numeric=0, alphanumeric=1, byte=2, kanji=3)
            mids = dict(M1=[
             '', -1, -1, -1], M2=[
             '0', '1', -1, -1], M3=[
             '00', '01', '10', '11'], M4=[
        ...
        raise ValueError('No appropriate mode for %sbits enc=%s/fmt=%s/eclv=%s/ver=%s' % (
...
def decode_n_base(s, digits=1, mode='digits'):
    ...
    while True:
        if mode == 'alnum':
            ...
            if len(head) == 2:
                yield (
                 11, ALNUM.find(head[0]) * 45 + ALNUM.find(head[1]))
            elif len(head) == 1:
                yield (
                 6, ALNUM.find(head[0]))
            else:
...
class QrCode(Barcode):
    ...
    >>> # _.show()
    """
    _sample_kanji_string = ('あいうえお').encode('sjis')
    codetype = 'qrcode'
    ...
    class _Renderer(MatrixCodeRenderer):
        ...
        >>>
        """
        default_options = dict(MatrixCodeRenderer.default_options, encoding=None, raw=False, format='full', version=None, eclevel=None)
