...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/elaphe/pdf417.py
...
class Pdf417(Barcode):
    ...
    class _Renderer(MatrixCodeRenderer):
        default_options = dict(MatrixCodeRenderer.default_options, dontdraw=False, compact=False, eclevel=-1, columns=2, rows=0, rowmult=3, ccc=False, raw=False, parse=False)
        ...
        def _code_bbox(self, codestring):
            ...
            if eclevel == -1:
                if m <= 40:
                    ...
                    eclevel = 5
            maxeclevel = int(math.log(927 - m) / math.log(2)) - 1
            ...
            if 1 <= columns <= 30:
                ...
                raise ValueError('Column out of bound: %d.' % columns)
            ...
            return (
             0, 0, DPI * rwid / 72.0, DPI * (r / 72.0) * rowmult)
