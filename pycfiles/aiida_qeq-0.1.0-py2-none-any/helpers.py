...
def get_computer(name=LOCALHOST_NAME, workdir=None):
    ...
    try:
        ...
        computer = Computer(name=name, description='localhost computer set up by aiida_diff tests', hostname=name, workdir=workdir, transport_type='local', scheduler_type='direct')
        computer.store()
        computer.configure()
...
def get_code(entry_point, computer):
    ...
    try:
        ...
        raise KeyError(('Entry point {} not recognized. Allowed values: {}').format(entry_point, list(executables.keys())))

    try:
        code = Code.get_from_string(('{}@{}').format(executable, computer.get_name()))
    except NotExistent:
        path = get_path_to_executable(executable)
        code = Code(input_plugin_name=entry_point, remote_computer_exec=[
         computer, path])
