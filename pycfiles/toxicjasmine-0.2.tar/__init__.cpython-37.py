...
# [GCC 8.4.0]
# Embedded file name: /home/juca/mysrc/cerberus_auth/toxicjasmine/__init__.py
# Compiled at: 2020-02-09 16:08:29
# Size of source mod 2**32: 7824 bytes
import argparse, mimetypes, os, pkg_resources, sys, cherrypy
...
class ToxicjasmineCore(Core):
    ...
    def css_files(cls, theme='dark'):
        ...
        return cls._uniq(sorted(filter(lambda x: css_filter in x, pkg_resources.resource_listdir(cls.css_package(), ''))))
...
class ToxicjasmineConfig(Config):
    ...
    def stylesheet_urls(self):
        jasmine_css = ['/__jasmine__/{0}'.format(core_css) for core_css in ToxicjasmineCore.css_files(theme=(self.theme))]
        src_css = ['/__src__/{0}'.format(css_file) for css_file in self.stylesheets()]
    ...
    def script_urls(self):
        ...
        jasmine_js = ['/__jasmine__/{0}'.format(core_js) for core_js in core_js_files]
        src_js = [self._prefix_src_underscored(src_file) for src_file in self.src_files()]
        helpers = ['/__spec__/{0}'.format(helper) for helper in self.helpers()]
        specs = ['/__spec__/{0}'.format(spec_file) for spec_file in self.spec_files()]
        return jasmine_js + src_js + helpers + specs


class ToxicjasmineCommand(Command):
    __doc__ = 'Jasmine with a new other css theme and new ci options to\n    pass to the browser.'
    ...
    def __init__(self, app, ci_runner):
        ...
        server = subcommands.add_parser('server',
          help='Jasmine server', description='run a server hosting your Jasmine specs')
        server.add_argument('-p', '--port', type=int, default=8888, help='The port of the Jasmine html runner')
        server.add_argument('-o', '--host', type=str, default='127.0.0.1', help='The host of the Jasmine html runner')
        server.add_argument('-c', '--config', type=str, help='Custom path to jasmine.yml')
        server.add_argument('-r', '--root', type=str, default=None, help='The root dir for the jasmine server process')
        server.set_defaults(func=(self.server))
        ci = subcommands.add_parser('ci',
          help='Jasmine CI', description='execute your specs in a browser')
        ci.add_argument('-b', '--browser', type=str, help='The selenium driver to utilize')
        ci.add_argument('-l', '--logs', action='store_true', help='Displays browser logs')
        ci.add_argument('-s', '--seed', type=str, help='Seed for random spec order')
        ci.add_argument('-c', '--config', type=str, help='Custom path to jasmine.yml')
        ci.add_argument('-t', '--theme', type=str, default='clear', help="The theme's name for the html report")
        ci.add_argument('-i', '--options', type=str, help='Browser options')
    ...
    def _load_config(self, custom_config_path):
        ...
        return ToxicjasmineConfig(config_file, project_path=project_path, theme=(self._args.theme),
...
class ToxicjasmineFile(object):
    ...
    def index(self, path):
        if path == 'jasmine_favicon':
            return cherrypy.lib.static.serve_fileobj((pkg_resources.resource_stream('jasmine_core.images', 'jasmine_favicon.png')),
              content_type='image/png')
        mime_type, _ = mimetypes.guess_type(path)
        return cherrypy.lib.static.serve_fileobj((pkg_resources.resource_stream('toxicjasmine', path)),
