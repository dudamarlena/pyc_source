...
# [GCC 8.4.0]
# Embedded file name: /Users/parente/projects/nbviewer/nbviewer/providers/base.py
# Compiled at: 2016-11-20 16:26:32
# Size of source mod 2**32: 22750 bytes
import hashlib, pickle, socket, time, statsd
...
from nbformat import current_nbformat, reads
...
class BaseHandler(web.RequestHandler):
    __doc__ = 'Base Handler class with common utilities'
    ...
    def redirect(self, url, *args, **kwargs):
        ...
        eurl = urlunparse((
         purl.scheme,
         purl.netloc,
         '/'.join([url_escape(url_unescape(p), plus=False) for p in purl.path.split('/')]),
         purl.params,
         purl.query,
         purl.fragment))
        return super(BaseHandler, self).redirect(eurl, *args, **kwargs)
    ...
    def prepare(self):
        ...
        if self.hub_api_url or self.hub_api_token or self.hub_base_url:
            ...
            def redirect_to_login():
                self.redirect(url_path_join(self.hub_base_url, '/hub/login') + '?' + urlencode({'next': self.request.path}))

            encrypted_cookie = self.get_cookie(self.hub_cookie_name)
            if not encrypted_cookie:
                raise gen.Return(redirect_to_login())
            try:
                yield self.http_client.fetch(url_path_join(self.hub_api_url, 'authorizations/cookie', self.hub_cookie_name, quote(encrypted_cookie, safe='')), headers={'Authorization': 'token ' + self.hub_api_token})
    ...
    def statsd(self):
        if hasattr(self, '_statsd'):
            ...
            if self.settings['statsd_host']:
                print(self.settings)
                self._statsd = statsd.StatsClient(self.settings['statsd_host'], self.settings['statsd_port'], self.settings['statsd_prefix'] + '.' + type(self).__name__)
                return self._statsd
    ...
    def template_namespace(self):
        return {'mathjax_url': self.mathjax_url, 
    ...
    def breadcrumbs(self, path, base_url):
        ...
        for name in path.split('/'):
            ...
            breadcrumbs.append({'url': base_url, 
             'name': name})
    ...
    def cache_and_finish(self, content=''):
        ...
        cache_data = pickle.dumps({'headers': self.cache_headers, 
         'body': content}, pickle.HIGHEST_PROTOCOL)
...
def cached(method):
    ...
    def cached_method(self, *args, **kwargs):
        ...
        if pending_future:
            ...
            toc = loop.time()
            app_log.info('Waited %.3fs for concurrent request at %s', toc - tic, short_url)
        try:
...
class RenderingHandler(BaseHandler):
    __doc__ = 'Base for handlers that render notebooks'
    ...
    @gen.coroutine
    def finish_notebook(self, json_notebook, download_url, provider_url=None, provider_icon=None, provider_label=None, msg=None, breadcrumbs=None, public=False, format=None, request=None):
        ...
        try:
            ...
            with time_block('Rendered %s' % download_url):
                ...
                nbhtml, config = yield self.pool.submit(render_notebook, self.formats[format], nb, download_url, config=self.config)
                render_time.stop()
...
class FilesRedirectHandler(BaseHandler):
    __doc__ = 'redirect files URLs without files prefix\n\n    matches behavior of old app, currently unused.\n    '
...
class AddSlashHandler(BaseHandler):
    __doc__ = 'redirector for URLs that should always have trailing slash'
...
class RemoveSlashHandler(BaseHandler):
    __doc__ = 'redirector for URLs that should never have trailing slash'
