...
class URLHandler(RenderingHandler):
    ...
    def get(self, secure, netloc, url):
        ...
        refer_url = url.endswith('.ipynb') or self.request.headers.get('Referer', '').split('://')[(-1)]
        ...
        yield self.finish_notebook(nbjson, download_url=remote_url, msg='file from url: %s' % remote_url, public=public, request=self.request, format=self.format)
