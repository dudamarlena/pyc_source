...
# [GCC 8.4.0]
# Embedded file name: ./build/lib/supervisor/tests/test_options.py
# Compiled at: 2019-06-16 14:41:06
# Size of source mod 2**32: 139317 bytes
"""Test suite for supervisor.options"""
import os, sys, tempfile, socket, unittest, signal, shutil, errno, platform
...
class OptionTests(unittest.TestCase):
    ...
    def _makeOptions(self, read_error=False):
        ...
        options.add(name='anoption', confname='anoption', short='o',
          long='option',
          default='default')
        options.add(name='other', confname='other', env='OTHER', short='p:',
          long='other=',
          handler=integer)
        return options
    ...
    def test_config_reload_do_usage_false(self):
        ...
        self.assertRaises(ValueError, options.process_config, False)
...
class ClientOptionsTests(unittest.TestCase):
    ...
    def test_options_with_environment_expansions(self):
        ...
        fp = StringIO(s)
        instance = self._makeOne()
        instance.environ_expansions = {'ENV_HOME':tempfile.gettempdir(),  'ENV_USER':'johndoe', 
    ...
    def test_options_supervisorctl_section_expands_here(self):
        ...
        options = instance.configroot.supervisorctl
        self.assertEqual(options.history_file, os.path.join(here, 'sc_history'))
        self.assertEqual(options.serverurl, 'unix://' + os.path.join(here, 'supervisord.sock'))
    ...
    def test_read_config_no_supervisord_section_raises_valueerror(self):
        ...
        try:
            ...
            try:
                self.assertEqual(exc.args[0], '.ini file does not include supervisorctl section')
            finally:
                exc = None
                del exc
...
class ServerOptionsTests(unittest.TestCase):
    ...
    def test_options(self):
        ...
        self.assertEqual(options.umask, 18)
        self.assertEqual(options.logfile, 'supervisord.log')
        self.assertEqual(options.logfile_maxbytes, 1048576000)
        ...
        self.assertEqual(proc1.stdout_logfile_maxbytes, datatypes.byte_size('50MB'))
        ...
        self.assertEqual(proc3.stdout_logfile_maxbytes, datatypes.byte_size('50MB'))
        ...
        self.assertEqual(proc4_a.autorestart, datatypes.RestartWhenExitUnexpected)
        self.assertEqual(proc4_a.uid, None)
        self.assertEqual(proc4_a.stdout_logfile, datatypes.Automatic)
        self.assertEqual(proc4_a.stdout_logfile_maxbytes, datatypes.byte_size('50MB'))
        ...
        self.assertEqual(proc4_b.autorestart, datatypes.RestartWhenExitUnexpected)
        self.assertEqual(proc4_b.uid, None)
        self.assertEqual(proc4_b.stdout_logfile, datatypes.Automatic)
        self.assertEqual(proc4_b.stdout_logfile_maxbytes, datatypes.byte_size('50MB'))
        ...
        self.assertEqual(instance.umask, 18)
        self.assertEqual(instance.logfile, os.path.join(here, 'supervisord.log'))
        self.assertEqual(instance.logfile_maxbytes, 1048576000)
    ...
    def test_reload(self):
        ...
        self.assertTrue(section.process_group_configs is instance.process_group_configs)
        text = lstrip('        [supervisord]\n        user=root\n\n        [program:one]\n        command = /bin/cat\n\n        [program:three]\n        command = /bin/pig\n\n        [group:thegroup]\n        programs = three\n        ')
        instance.configfile = StringIO(text)
    ...
    def test_read_config_no_supervisord_section_raises_valueerror(self):
        ...
        try:
            ...
            try:
                self.assertEqual(exc.args[0], '.ini file does not include supervisord section')
            finally:
                exc = None
                del exc
    ...
    def test_read_config_include_with_no_files_raises_valueerror(self):
        ...
        try:
            ...
            try:
                self.assertEqual(exc.args[0], '.ini file has [include] section, but no files setting')
            finally:
                exc = None
                del exc
    ...
    def test_read_config_include_reads_files_in_sorted_order(self):
        ...
        for letter in reversed(a_z):
            ...
            for letter in sorted(a_z):
                filename = os.path.join(conf_d, '%s.conf' % letter)
                expected_msgs.append('Included extra file "%s" during parsing' % filename)
            else:
    ...
    def test_read_config_include_expands_here(self):
        conf = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'fixtures', 'include.conf')
    ...
    def test_check_execv_args_cant_find_command(self):
        ...
        self.assertRaises(NotFound, instance.check_execv_args, '/not/there', None, None)

    def test_check_execv_args_notexecutable(self):
        instance = self._makeOne()
        from supervisor.options import NotExecutable
        self.assertRaises(NotExecutable, instance.check_execv_args, '/etc/passwd', [
         'etc/passwd'], os.stat('/etc/passwd'))

    def test_check_execv_args_isdir(self):
        instance = self._makeOne()
        from supervisor.options import NotExecutable
        self.assertRaises(NotExecutable, instance.check_execv_args, '/', [
         '/'], os.stat('/'))
    ...
    def test_realize_positional_args_not_supported(self):
        ...
        self.assertEqual(recorder[0], 'positional arguments are not supported: %s' % args)
    ...
    def test_options_afunix(self):
        ...
        self.assertEqual(options.server_configs[0]['chmod'], 448)
        self.assertEqual(options.server_configs[0]['chown'], (-1, -1))
    ...
    def test_options_afunix_chmod_bad(self):
        ...
        try:
            ...
            try:
                self.assertEqual(exc.args[0], 'Invalid chmod value NaN')
            finally:
                exc = None
                del exc
    ...
    def test_options_afunix_chown_bad(self):
        ...
        try:
            ...
            try:
                self.assertEqual(exc.args[0], 'Invalid sockchown value thisisnotavaliduser')
            finally:
                exc = None
                del exc
    ...
    def test_options_afunix_no_file(self):
        ...
        try:
            ...
            try:
                self.assertEqual(exc.args[0], 'section [unix_http_server] has no file value')
            finally:
                exc = None
                del exc
    ...
    def test_options_afunix_username_without_password(self):
        ...
        try:
            ...
            try:
                self.assertEqual(exc.args[0], 'Section [unix_http_server] contains incomplete authentication: If a username or a password is specified, both the username and password must be specified')
            finally:
                exc = None
                del exc
    ...
    def test_options_afunix_file_expands_here(self):
        ...
        self.assertEqual(serverconf['file'], os.path.join(here, 'supervisord.sock'))
    ...
    def test_options_afinet_username_without_password(self):
        ...
        try:
            ...
            try:
                self.assertEqual(exc.args[0], 'Section [inet_http_server] contains incomplete authentication: If a username or a password is specified, both the username and password must be specified')
            finally:
                exc = None
                del exc
    ...
    def test_options_afinet_no_port(self):
        ...
        try:
            ...
            try:
                self.assertEqual(exc.args[0], 'section [inet_http_server] has no port value')
            finally:
                exc = None
                del exc
    ...
    def test_cleanup_afunix_ignores_oserror_enoent(self):
        ...
        try:
            ...
            instance.httpservers = [
             (
              {'family':socket.AF_UNIX, 
               'file':notfound}, Server()),
             (
              {'family':socket.AF_UNIX, 
               'file':socketname}, Server())]
            instance.pidfile = ''
            instance.cleanup()
    ...
    def test_close_parent_pipes(self):
        ...
        pipes = {'stdin':0,  'stdout':1,  'stderr':2,  'child_stdin':3, 
         'child_stdout':4,  'child_stderr':5}
    ...
    def test_processes_from_section(self):
        ...
        self.assertEqual(pconfig.environment, {'KEY1':'val1', 
         'KEY2':'val2',  'KEY3':'0'})
    ...
    def test_processes_from_section_process_num_expansion(self):
        ...
        for num in (0, 1):
            ...
            self.assertEqual(pconfigs[num].stderr_logfile, '/tmp/foo_%d_stderr' % num)
            self.assertEqual(pconfigs[num].stdout_logfile, '/tmp/foo_%d_stdout' % num)
    ...
    def test_processes_from_section_redirect_stderr_with_filename(self):
        ...
        config.read_string(text)
        pconfigs = instance.processes_from_section(config, 'program:foo', 'bar')
        self.assertEqual(instance.parse_warnings[0], 'For [program:foo], redirect_stderr=true but stderr_logfile has also been set to a filename, the filename has been ignored')
    ...
    def test_options_with_environment_expansions(self):
        ...
        instance.read_config(StringIO(text))
        instance.realize(args=[])
        self.assertEqual(instance.logfile, '%(ENV_HOME)s/supervisord.log' % config.expansions)
        self.assertEqual(instance.identifier, 'supervisor_%(ENV_USER)s' % config.expansions)
        ...
        self.assertEqual(proc1.stopasgroup, False)
        self.assertEqual(proc1.killasgroup, False)
        self.assertEqual(proc1.stdout_logfile_maxbytes, datatypes.byte_size('78KB'))
    ...
    def test_options_supervisord_section_expands_here(self):
        ...
        self.assertEqual(instance.childlogdir, os.path.join(here))
        self.assertEqual(instance.directory, os.path.join(here))
        self.assertEqual(instance.logfile, os.path.join(here, 'supervisord.log'))
        self.assertEqual(instance.pidfile, os.path.join(here, 'supervisord.pid'))
    ...
    def test_options_program_section_expands_here(self):
        ...
        self.assertEqual(proc.directory, os.path.join(here, 'thedirectory'))
        self.assertEqual(proc.command, os.path.join(here, 'bin/cat'))
        self.assertEqual(proc.environment, {'FOO': os.path.join(here, 'foo')})
        self.assertEqual(proc.serverurl, 'unix://' + os.path.join(here, 'supervisord.sock'))
        self.assertEqual(proc.stdout_logfile, os.path.join(here, 'stdout.log'))
        self.assertEqual(proc.stderr_logfile, os.path.join(here, 'stderr.log'))
    ...
    def test_options_eventlistener_section_expands_here(self):
        ...
        proc = group.process_configs[0]
        self.assertEqual(proc.directory, os.path.join(here, 'thedirectory'))
        self.assertEqual(proc.command, os.path.join(here, 'bin/memmon'))
        self.assertEqual(proc.environment, {'FOO': os.path.join(here, 'foo')})
        self.assertEqual(proc.serverurl, 'unix://' + os.path.join(here, 'supervisord.sock'))
        self.assertEqual(proc.stdout_logfile, os.path.join(here, 'stdout.log'))
        self.assertEqual(proc.stderr_logfile, os.path.join(here, 'stderr.log'))
    ...
    def test_options_expands_combined_expansions(self):
        ...
        text += 'command = %(here)s/bin/cat --foo=%(ENV_FOO)s --num=%(process_num)d --node=%(host_node_name)s'
        here = tempfile.mkdtemp()
        supervisord_conf = os.path.join(here, 'supervisord.conf')
        with open(supervisord_conf, 'w') as (f):
            f.write(text)
        try:
            instance.environ_expansions = {'ENV_LOGNAME':'mainlog', 
             'ENV_FOO':'bar'}
            instance.configfile = supervisord_conf
            instance.realize(args=[])
        finally:
        ...
        section = instance.configroot.supervisord
        self.assertEqual(section.logfile, os.path.join(here, 'mainlog.log'))
    ...
    def test_options_error_handler_shows_main_filename(self):
        ...
        try:
            ...
            try:
                ...
                try:
                    self.assertEqual(str(e.args[0]), "value 'NOTASIGNAL' is not a valid signal name in section 'program:cat' (file: %r)" % supervisord_conf)
                finally:
                    e = None
                    del e
    ...
    def test_options_error_handler_shows_included_filename(self):
        ...
        try:
            ...
            try:
                ...
                try:
                    self.assertEqual(str(e.args[0]), "value 'NOTASIGNAL' is not a valid signal name in section 'program:cat' (file: %r)" % included_conf)
                finally:
                    e = None
                    del e
    ...
    def test_processes_from_section_bad_program_name_spaces(self):
        instance = self._makeOne()
        text = lstrip('        [program:spaces are bad]\n        ')
        from supervisor.options import UnhosedConfigParser
        config = UnhosedConfigParser()
        config.read_string(text)
        self.assertRaises(ValueError, instance.processes_from_section, config, 'program:spaces are bad', None)

    def test_processes_from_section_bad_program_name_colons(self):
        instance = self._makeOne()
        text = lstrip('        [program:colons:are:bad]\n        ')
        from supervisor.options import UnhosedConfigParser
        config = UnhosedConfigParser()
        config.read_string(text)
        self.assertRaises(ValueError, instance.processes_from_section, config, 'program:colons:are:bad', None)
    ...
    def test_processes_from_section_no_procnum_in_processname(self):
        ...
        config = UnhosedConfigParser()
        config.read_string(text)
        self.assertRaises(ValueError, instance.processes_from_section, config, 'program:foo', None)
    ...
    def test_processes_from_section_no_command(self):
        ...
        try:
            ...
            try:
                self.assertTrue(exc.args[0].startswith('program section program:foo does not specify a command'))
            finally:
                exc = None
                del exc
    ...
    def test_processes_from_section_unexpected_end_of_key_value_pairs(self):
        ...
        try:
            ...
            try:
                self.assertTrue("Unexpected end of key/value pairs in value 'KEY1=val1,KEY2=val2,KEY3' in section 'program:foo'" in str(e))
            finally:
                e = None
                del e

        else:
            self.fail('instance.processes_from_section should raise a ValueError')
    ...
    def test_processes_from_section_shows_conf_filename_on_valueerror(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w+') as (f):
            try:
                ...
                try:
                    self.assertEqual(e.args[0], "program section program:foo does not specify a command in section 'program:foo' (file: %r)" % f.name)
                finally:
                    e = None
                    del e
    ...
    def test_processes_from_section_autolog_without_rollover(self):
        ...
        config.read_string(text)
        instance.processes_from_section(config, 'program:foo', None)
        self.assertEqual(instance.parse_warnings[0], 'For [program:foo], AUTO logging used for stdout_logfile without rollover, set maxbytes > 0 to avoid filling up filesystem unintentionally')
        self.assertEqual(instance.parse_warnings[1], 'For [program:foo], AUTO logging used for stderr_logfile without rollover, set maxbytes > 0 to avoid filling up filesystem unintentionally')

    def test_homogeneous_process_groups_from_parser(self):
        text = lstrip('        [program:many]\n        process_name = %(program_name)s_%(process_num)s\n        command = /bin/cat\n        numprocs = 2\n        priority = 1\n        ')
        from supervisor.options import UnhosedConfigParser
        config = UnhosedConfigParser()
        config.read_string(text)
        instance = self._makeOne()
    ...
    def test_event_listener_pool_disallows_buffer_size_zero(self):
        ...
        try:
            ...
            try:
                self.assertEqual(exc.args[0], '[eventlistener:dog] section sets invalid buffer_size (0)')
            finally:
                exc = None
                del exc
    ...
    def test_event_listener_pool_disallows_redirect_stderr(self):
        ...
        try:
            ...
            try:
                self.assertEqual(exc.args[0], '[eventlistener:dog] section sets redirect_stderr=true but this is not allowed because it will interfere with the eventlistener protocol')
            finally:
                exc = None
                del exc
    ...
    def test_event_listener_pool_result_handler_unimportable(self):
        ...
        try:
            ...
            try:
                self.assertEqual(exc.args[0], 'supervisor.tests.base:nonexistent cannot be resolved within [eventlistener:cat]')
            finally:
                exc = None
                del exc
    ...
    def test_fcgi_programs_from_parser(self):
        ...
        self.assertEqual(gconf_foo.socket_config.url, 'unix:///tmp/foo.sock')
        self.assertEqual(exp_owner, gconf_foo.socket_config.get_owner())
        self.assertEqual(438, gconf_foo.socket_config.get_mode())
    ...
    def test_fcgi_programs_from_parser_with_environment_expansions(self):
        ...
        from supervisor.options import UnhosedConfigParser
        instance = self._makeOne()
        instance.environ_expansions = {'ENV_HOME':'/tmp',  'ENV_SERVER_PORT':'9210', 
        ...
        self.assertEqual(gconf_foo.name, 'foo')
        self.assertEqual(gconf_foo.priority, 1)
        self.assertEqual(gconf_foo.socket_config.url, 'unix:///tmp/foo.usock')
        self.assertEqual(exp_owner, gconf_foo.socket_config.get_owner())
        self.assertEqual(438, gconf_foo.socket_config.get_mode())
        self.assertEqual(32676, gconf_foo.socket_config.get_backlog())
        self.assertEqual(len(gconf_foo.process_configs), 2)
    ...
    def test_rpcinterfaces_from_parser(self):
        text = lstrip('        [rpcinterface:dummy]\n        supervisor.rpcinterface_factory = %s\n        foo = bar\n        baz = qux\n        ' % __name__)
        from supervisor.options import UnhosedConfigParser
        config = UnhosedConfigParser()
        config.read_string(text)
        instance = self._makeOne()
        factories = instance.get_plugins(config, 'supervisor.rpcinterface_factory', 'rpcinterface:')
    ...
    def test_rpcinterfaces_from_parser_factory_missing(self):
        ...
        instance = self._makeOne()
        try:
            instance.get_plugins(config, 'supervisor.rpcinterface_factory', 'rpcinterface:')
            self.fail('nothing raised')
        except ValueError as exc:
            try:
                self.assertEqual(exc.args[0], 'section [rpcinterface:dummy] does not specify a supervisor.rpcinterface_factory')
            finally:
                exc = None
                del exc
    ...
    def test_rpcinterfaces_from_parser_factory_not_importable(self):
        ...
        try:
            ...
            try:
                self.assertEqual(exc.args[0], 'nonexistent cannot be resolved within [rpcinterface:dummy]')
            finally:
                exc = None
                del exc
    ...
    def test_openhttpservers_reports_socket_error_with_errno(self):
        ...
        instance.openhttpservers(supervisord)
        self.assertEqual(len(recorder), 1)
        expected = 'Cannot open an HTTP server: socket.error reported errno.EPERM (%d)' % errno.EPERM
    ...
    def test_openhttpservers_reports_other_socket_errors(self):
        ...
        instance.openhttpservers(supervisord)
        self.assertEqual(len(recorder), 1)
        expected = 'Cannot open an HTTP server: socket.error reported uh oh'
        self.assertEqual(recorder[0], expected)
    ...
    def test_openhttpservers_does_not_catch_other_exception_types(self):
        ...
        instance.make_http_servers = make_http_servers
        self.assertRaises(OverflowError, instance.openhttpservers, supervisord)
...
class ProcessConfigTests(unittest.TestCase):
    ...
    def _makeOne(self, *arg, **kw):
        defaults = {}
        for name in ('name', 'command', 'directory', 'umask', 'priority', 'autostart',
                     'autorestart', 'startsecs', 'startretries', 'uid', 'stdout_logfile',
                     'stdout_capture_maxbytes', 'stdout_events_enabled', 'stdout_syslog',
                     'stderr_logfile', 'stderr_capture_maxbytes', 'stderr_events_enabled',
                     'stderr_syslog', 'stopsignal', 'stopwaitsecs', 'stopasgroup',
                     'killasgroup', 'exitcodes', 'redirect_stderr', 'environment'):
            defaults[name] = name
        else:
            for name in ('stdout_logfile_backups', 'stdout_logfile_maxbytes', 'stderr_logfile_backups',
    ...
    def test_make_dispatchers_stderr_not_redirected(self):
        ...
        with tempfile.NamedTemporaryFile() as (stdout_logfile):
            with tempfile.NamedTemporaryFile() as (stderr_logfile):
                ...
                self.assertEqual(dispatchers[5].event_type, ProcessCommunicationStdoutEvent)
                ...
                self.assertEqual(dispatchers[7].event_type, ProcessCommunicationStderrEvent)
    ...
    def test_make_dispatchers(self):
        ...
        with tempfile.NamedTemporaryFile() as (stdout_logfile):
            with tempfile.NamedTemporaryFile() as (stderr_logfile):
                ...
                self.assertEqual(dispatchers[5].process.listener_state, EventListenerStates.ACKNOWLEDGED)
                self.assertEqual(pipes['stdout'], 5)
                self.assertEqual(dispatchers[7].channel, 'stderr')
                from supervisor.events import ProcessCommunicationStderrEvent
                self.assertEqual(dispatchers[7].event_type, ProcessCommunicationStderrEvent)
...
class EventListenerPoolConfigTests(unittest.TestCase):
    ...
    def _makeOne(self, options, name, priority, process_configs, buffer_size, pool_events, result_handler):
        return self._getTargetClass()(options, name, priority, process_configs, buffer_size, pool_events, result_handler)
...
class UnhosedConfigParserTests(unittest.TestCase):
    ...
    def test_saneget_no_default(self):
        ...
        self.assertRaises(ConfigParser.NoOptionError, parser.saneget, 'supervisord', 'missing')
    ...
    def test_saneget_expands_arg_expansions(self):
        ...
        parser.expansions = {'pet': 'dog'}
        parser.read_string('[supervisord]\nfoo=%(pet)s\n')
        result = parser.saneget('supervisord', 'foo', expansions={'pet': 'cat'})
