...
# [GCC 8.4.0]
# Embedded file name: ./build/lib/supervisor/tests/test_http.py
# Compiled at: 2019-04-05 17:19:18
# Size of source mod 2**32: 25345 bytes
import base64, os, stat, sys, socket, tempfile, unittest
...
class LogtailHandlerTests(HandlerTests, unittest.TestCase):
    ...
    def test_handle_request(self):
        with tempfile.NamedTemporaryFile() as (f):
            ...
            self.assertEqual(request.headers['Last-Modified'], http_date.build_http_date(os.stat(t)[stat.ST_MTIME]))
...
class TailFProducerTests(unittest.TestCase):
    ...
    def test_handle_more(self):
        ...
        f.write(b'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
        f.flush()
        producer = self._makeOne(request, f.name, 80)
        result = producer.more()
        self.assertEqual(result, b'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
        f.write(as_bytes(b'wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww'))
        f.flush()
        result = producer.more()
        self.assertEqual(result, b'wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww')
        result = producer.more()
    ...
    def test_handle_more_fd_closed(self):
        request = DummyRequest('/logtail/foo', None, None, None)
        with tempfile.NamedTemporaryFile() as (f):
            f.write(as_bytes('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'))
            f.flush()
            producer = self._makeOne(request, f.name, 80)
            producer.file.close()
            result = producer.more()
    ...
    def test_handle_more_follow_file_recreated(self):
        ...
        f2 = open(f.name, 'wb')
        try:
            f2.write(as_bytes(b'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb'))
            f2.close()
            result = producer.more()
        finally:
            os.unlink(f2.name)

        self.assertEqual(result, b'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb')
    ...
    def test_handle_more_follow_file_gone(self):
        ...
        self.assertEqual(result, b'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
        with open(filename, 'wb') as (f):
            f.write(as_bytes(b'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb'))
        try:
            result = producer.more()
            self.assertEqual(result, b'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb')
        finally:
...
class DeferringGlobbingProducerTests(unittest.TestCase):
    ...
    def _makeOne(self, producer, buffer_size=65536):
...
class DeferringHttpRequestTests(unittest.TestCase):
    ...
    def _makeOne(self, channel=None, req='GET / HTTP/1.0', command='GET', uri='/', version='1.0', header=()):
        return self._getTargetClass()(channel, req, command, uri, version, header)
...
class DeferringHttpChannelTests(unittest.TestCase):
    ...
    def _makeOne(self):
        return self._getTargetClass()(server=None,
          conn=None,
          addr=None)
...
class SupervisorAuthHandlerTests(unittest.TestCase):
    ...
    def test_ctor(self):
        ...
        from supervisor.http import encrypted_dictionary_authorizer
        self.assertEqual(handler.authorizer.__class__, encrypted_dictionary_authorizer)
...
class TopLevelFunctionTests(unittest.TestCase):
    ...
    def test_make_http_servers_socket_type_error(self):
        config = {'family':999, 
         'host':'localhost',  'port':17735,  'username':None, 
         'password':None,  'section':'inet_http_server'}
        try:
    ...
    def test_make_http_servers_noauth(self):
        ...
        inet = {'family':socket.AF_INET,  'host':'localhost',  'port':17735,  'username':None, 
         'password':None,  'section':'inet_http_server'}
        unix = {'family':socket.AF_UNIX,  'file':socketfile,  'chmod':448,  'chown':(-1, -1), 
         'username':None,  'password':None,  'section':'unix_http_server'}
    ...
    def test_make_http_servers_withauth(self):
        socketfile = tempfile.mktemp()
        inet = {'family':socket.AF_INET,  'host':'localhost',  'port':17736,  'username':'username', 
         'password':'password',  'section':'inet_http_server'}
        unix = {'family':socket.AF_UNIX,  'file':socketfile,  'chmod':448,  'chown':(-1, -1), 
         'username':'username',  'password':'password',  'section':'unix_http_server'}
        servers = self._make_http_servers([inet, unix])
        self.assertEqual(len(servers), 2)
        from supervisor.http import supervisor_auth_handler
        for config, server in servers:
            for handler in server.handlers:
                self.assertTrue(isinstance(handler, supervisor_auth_handler), handler)
