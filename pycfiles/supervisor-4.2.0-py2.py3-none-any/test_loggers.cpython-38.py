...
# [GCC 8.4.0]
# Embedded file name: ./build/lib/supervisor/tests/test_loggers.py
# Compiled at: 2019-10-19 13:06:49
# Size of source mod 2**32: 21442 bytes
...
class RotatingFileHandlerTests(FileHandlerTests):
    ...
    def test_emit_does_rollover(self):
        ...
        record = self._makeLogRecord(b'aaaa')
        handler.emit(record)
        ...
        with open(self.filename + '.1', 'rb') as (f):
            self.assertEqual(f.read(), b'aaaaaaaaaaaa')
...
class SyslogHandlerTests(HandlerTests, unittest.TestCase):
    ...
    if PY2:
        ...
        def test_emit_unicode_noerror(self):
            ...
            handler.emit(record)
            syslog.syslog.assert_called_with('fiÃ\xad')
        ...
        def test_emit_unicode_witherror(self):
            ...
            handler.emit(record)
            self.assertEqual(called, [as_string('fiÃ\xad')])
