...
# [GCC 8.4.0]
# Embedded file name: ./build/lib/supervisor/tests/test_supervisorctl.py
# Compiled at: 2019-06-16 13:45:15
# Size of source mod 2**32: 82077 bytes
import sys, unittest
...
class ControllerTests(unittest.TestCase):
    ...
    def test__upcheck_wrong_server_version(self):
        ...
        self.assertEqual(controller.stdout.getvalue(), 'Sorry, this version of supervisorctl expects to talk to a server with API version 3.0, but the remote version is 1.0.\n')
    ...
    def test__upcheck_unknown_method(self):
        ...
        result = controller.upcheck()
        self.assertEqual(result, False)
        self.assertEqual(controller.stdout.getvalue(), 'Sorry, supervisord responded but did not recognize the supervisor namespace commands that supervisorctl uses to control it.  Please check that the [rpcinterface:supervisor] section is enabled in the configuration file (see sample.conf).\n')
    ...
    def test_onecmd_bad_command_error(self):
        ...
        self.assertEqual(controller.stdout.getvalue(), '*** Unknown syntax: badcmd\n')
        self.assertEqual(controller.exitstatus, LSBInitExitStatuses.GENERIC)
...
class TestDefaultControllerPlugin(unittest.TestCase):
    ...
    def test_status_table_process_column_expands(self):
        ...
        def f(*arg, **kw):
            ...
            return [
             {'name':'foofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoofoo',  'group':'foo', 
              'pid':11, 
    ...
    def test_status_bad_process_name_with_group(self):
        ...
        self.assertEqual(result, None)
        value = plugin.ctl.stdout.getvalue().split('\n')
        self.assertEqual(value[0], 'badgroup:badprocess: ERROR (no such process)')
        self.assertEqual(plugin.ctl.exitstatus, LSBStatusExitStatuses.UNKNOWN)
    ...
    def test_start_badname(self):
        ...
        self.assertEqual(result, None)
        self.assertEqual(plugin.ctl.stdout.getvalue(), 'BAD_NAME: ERROR (no such process)\n')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.GENERIC)
    ...
    def test_start_no_file(self):
        ...
        self.assertEqual(result, None)
        self.assertEqual(plugin.ctl.stdout.getvalue(), 'NO_FILE: ERROR (no such file)\n')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.GENERIC)
    ...
    def test_start_not_executable(self):
        ...
        self.assertEqual(result, None)
        self.assertEqual(plugin.ctl.stdout.getvalue(), 'NOT_EXECUTABLE: ERROR (file is not executable)\n')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.GENERIC)
    ...
    def test_start_alreadystarted(self):
        ...
        self.assertEqual(result, None)
        self.assertEqual(plugin.ctl.stdout.getvalue(), 'ALREADY_STARTED: ERROR (already started)\n')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.SUCCESS)
    ...
    def test_start_spawnerror(self):
        ...
        self.assertEqual(result, None)
        self.assertEqual(plugin.ctl.stdout.getvalue(), 'SPAWN_ERROR: ERROR (spawn error)\n')
    ...
    def test_start_group(self):
        ...
        result = plugin.do_start('foo:')
        self.assertEqual(result, None)
        self.assertEqual(plugin.ctl.stdout.getvalue(), 'foo:foo_00: started\nfoo:foo_01: started\n')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.SUCCESS)
    ...
    def test_start_group_bad_name(self):
        ...
        result = plugin.do_start('BAD_NAME:')
        self.assertEqual(result, None)
        self.assertEqual(plugin.ctl.stdout.getvalue(), 'BAD_NAME: ERROR (no such group)\n')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.INVALID_ARGS)
    ...
    def test_stop_fail(self):
        ...
        self.assertEqual(result, None)
        self.assertEqual(plugin.ctl.stdout.getvalue().split('\n')[0], 'Error: stop requires a process name')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.GENERIC)
    ...
    def test_stop_notrunning(self):
        ...
        self.assertEqual(result, None)
        self.assertEqual(plugin.ctl.stdout.getvalue(), 'NOT_RUNNING: ERROR (not running)\n')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.SUCCESS)
    ...
    def test_stop_group(self):
        ...
        result = plugin.do_stop('foo:')
        self.assertEqual(result, None)
        self.assertEqual(plugin.ctl.stdout.getvalue(), 'foo:foo_00: stopped\nfoo:foo_01: stopped\n')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.SUCCESS)
    ...
    def test_signal_bad_signal(self):
        ...
        self.assertEqual(result, None)
        self.assertEqual(plugin.ctl.stdout.getvalue(), 'foo: ERROR (bad signal name)\n')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.GENERIC)
    ...
    def test_signal_group(self):
        ...
        result = plugin.do_signal('HUP foo:')
        self.assertEqual(result, None)
        self.assertEqual(plugin.ctl.stdout.getvalue(), 'foo:foo_00: signalled\nfoo:foo_01: signalled\n')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.SUCCESS)
    ...
    def test_restart_fail(self):
        ...
        self.assertEqual(result, None)
        self.assertEqual(plugin.ctl.stdout.getvalue().split('\n')[0], 'Error: restart requires a process name')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.GENERIC)
    ...
    def test_clear_fail(self):
        ...
        self.assertEqual(result, None)
        self.assertEqual(plugin.ctl.stdout.getvalue().split('\n')[0], 'Error: clear requires a process name')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.GENERIC)
    ...
    def test_open_fail(self):
        ...
        self.assertEqual(result, None)
        self.assertEqual(plugin.ctl.stdout.getvalue(), 'ERROR: url must be http:// or unix://\n')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.GENERIC)
    ...
    def test_shutdown_catches_xmlrpc_fault_shutdown_state(self):
        ...
        result = plugin.do_shutdown('')
        self.assertEqual(result, None)
        self.assertEqual(plugin.ctl.stdout.getvalue(), 'ERROR: already shutting down\n')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.SUCCESS)
    ...
    def test_shutdown_reraises_other_xmlrpc_faults(self):
        ...
        plugin.ctl.options._server.supervisor.shutdown = raise_fault
        self.assertRaises(xmlrpclib.Fault, plugin.do_shutdown, '')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.GENERIC)
    ...
    def test_shutdown_reraises_other_socket_errors(self):
        ...
        plugin.ctl.options._server.supervisor.shutdown = raise_fault
        self.assertRaises(socket.error, plugin.do_shutdown, '')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.GENERIC)
    ...
    def test_reread_cant_reread(self):
        ...
        self.assertEqual(plugin.ctl.stdout.getvalue(), 'ERROR: cant\n')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.GENERIC)
    ...
    def test_reread_shutdown_state(self):
        ...
        plugin.ctl.options._server.supervisor.reloadConfig = reloadConfig
        plugin.do_reread(None)
        self.assertEqual(plugin.ctl.stdout.getvalue(), 'ERROR: supervisor shutting down\n')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.GENERIC)
    ...
    def test__formatConfigInfo(self):
        ...
        info = {'group':'group1',  'name':'process1', 
         'inuse':False, 
         'autostart':False, 
         'process_prio':999, 
         'group_prio':999}
    ...
    def test_avail(self):
        ...
        class FakeSupervisor(object):
            ...
            def getAllConfigInfo(self):
                return [
                 {'group':'group1', 
                  'name':'process1',  'inuse':False, 
                  'autostart':False,  'process_prio':999, 
                  'group_prio':999}]
    ...
    def test_add_already_added(self):
        ...
        self.assertEqual(result, None)
        self.assertEqual(plugin.ctl.stdout.getvalue(), 'ERROR: process group already active\n')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.SUCCESS)
    ...
    def test_add_bad_name(self):
        ...
        self.assertEqual(result, None)
        self.assertEqual(plugin.ctl.stdout.getvalue(), 'ERROR: no such process/group: BAD_NAME\n')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.GENERIC)
    ...
    def test_add_shutdown_state(self):
        ...
        self.assertEqual(result, None)
        self.assertEqual(plugin.ctl.stdout.getvalue(), 'ERROR: shutting down\n')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.GENERIC)
    ...
    def test_remove_still_running(self):
        ...
        self.assertEqual(result, None)
        self.assertEqual(plugin.ctl.stdout.getvalue(), 'ERROR: process/group still running: STILL_RUNNING\n')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.GENERIC)
    ...
    def test_update_with_gname(self):
        ...
        def reloadConfig():
            return [
             [
              [
               'added1', 'added2'], ['changed'], ['removed']]]

        supervisor.reloadConfig = reloadConfig
        supervisor.processes = ['changed', 'removed']
        plugin.do_update('changed')
        self.assertEqual(sorted(supervisor.processes), sorted(['changed', 'removed']))
        plugin.do_update('added1 added2')
        self.assertEqual(sorted(supervisor.processes), sorted(['changed', 'removed', 'added1', 'added2']))
        plugin.do_update('removed')
        self.assertEqual(sorted(supervisor.processes), sorted(['changed', 'added1', 'added2']))
        supervisor.processes = [
         'changed', 'removed']
        plugin.do_update('removed added1')
        self.assertEqual(sorted(supervisor.processes), sorted(['changed', 'added1']))
        supervisor.processes = [
         'changed', 'removed']
    ...
    def test_update_changed_procs(self):
        ...
        supervisor.addProcessGroup('changed_group')
        results = [
         {'name':'changed_process',  'group':'changed_group', 
        ...
        calls[:] = []
        results[:] = [
         {'name':'changed_process1',  'group':'changed_group', 
    ...
    def _dont_test_maintail_dashf(self):
        ...
        error = errors[0]
        self.assertEqual(plugin.listener.closed, 'http://localhost:65532/mainlogtail')
        self.assertEqual(error[0], 'http://localhost:65532/mainlogtail')
    ...
    def test_maintail_readlog_error_nofile(self):
        ...
        result = plugin.do_maintail('-100')
        self.assertEqual(result, None)
        self.assertEqual(plugin.ctl.stdout.getvalue(), 'supervisord: ERROR (no log file)\n')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.GENERIC)
    ...
    def test_maintail_readlog_error_failed(self):
        ...
        result = plugin.do_maintail('-100')
        self.assertEqual(result, None)
        self.assertEqual(plugin.ctl.stdout.getvalue(), 'supervisord: ERROR (unknown error reading log)\n')
        self.assertEqual(plugin.ctl.exitstatus, LSBInitExitStatuses.GENERIC)
