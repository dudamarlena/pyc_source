...
# [GCC 8.4.0]
# Embedded file name: ./build/lib/supervisor/events.py
# Compiled at: 2019-04-05 17:19:18
# Size of source mod 2**32: 7235 bytes
...
class ProcessStateEvent(Event):
    __doc__ = ' Abstract class, never raised directly '
    frm = None
    to = None
    ...
    def payload(self):
        ...
        L = [
         (
          'processname', self.process.config.name), ('groupname', groupname),
         (
...
class ProcessStateStartingOrBackoffEvent(ProcessStateEvent):
    ...
    def get_extra_values(self):
        return [
         (
          'tries', int(self.process.backoff))]
...
class ProcessStateExitedEvent(ProcessStateEvent):
    ...
    def get_extra_values(self):
        return [
         (
          'expected', int(self.expected)), ('pid', self.process.pid)]
...
TICK_EVENTS = [
 Tick5Event, Tick60Event, Tick3600Event]
