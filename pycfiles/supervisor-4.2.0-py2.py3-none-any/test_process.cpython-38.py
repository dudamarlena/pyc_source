...
# [GCC 8.4.0]
# Embedded file name: ./build/lib/supervisor/tests/test_process.py
# Compiled at: 2019-04-10 17:22:49
# Size of source mod 2**32: 99926 bytes
import errno, os, signal, tempfile, time, unittest
...
class SubprocessTests(unittest.TestCase):
    ...
    def test_ctor(self):
        ...
        config = DummyPConfig(options, 'cat', 'bin/cat', stdout_logfile='/tmp/temp123.log',
    ...
    def test_drain(self):
        options = DummyOptions()
        config = DummyPConfig(options, 'test', '/test', stdout_logfile='/tmp/foo',
    ...
    def test_cmp_bypriority(self):
        ...
        config = DummyPConfig(options, 'notthere', '/notthere', stdout_logfile='/tmp/foo',
          priority=1)
        instance = self._makeOne(config)
        config = DummyPConfig(options, 'notthere1', '/notthere', stdout_logfile='/tmp/foo',
          priority=2)
        instance1 = self._makeOne(config)
        config = DummyPConfig(options, 'notthere2', '/notthere', stdout_logfile='/tmp/foo',
          priority=3)
    ...
    def test_transition_starting_to_running_laststart_in_future(self):
        ...
        options = DummyOptions()
        test_startsecs = 2
        pconfig = DummyPConfig(options, 'process', 'process', '/bin/process', startsecs=test_startsecs)
        process = self._makeOne(pconfig)
        process.backoff = 1
        process.delay = 1
        process.system_stop = False
        ...
        self.assertEqual(options.logger.data[0], 'success: process entered RUNNING state, process has stayed up for > than {} seconds (startsecs)'.format(test_startsecs))
        self.assertEqual(len(L), 1)
        event = L[0]
        self.assertEqual(event.__class__, events.ProcessStateRunningEvent)
...
class FastCGIProcessGroupTests(unittest.TestCase):
    ...
    def test___init__with_socket_error(self):
        ...
        self.assertRaises(ValueError,
          (self._makeOne),
          gconfig, socketManager=DummySocketManager)
...
class EventListenerPoolTests(ProcessGroupBaseTests):
    ...
    def test_dispatch_pipe_error(self):
        ...
        self.assertEqual(options.logger.data[1], 'rebuffering event abc for pool whatever (buf size=0, max=10)')
