...
# [GCC 8.4.0]
# Embedded file name: ./build/lib/supervisor/loggers.py
...
class Handler:
    ...
    def emit(self, record):
        try:
            binary = self.fmt == '%(message)s' and isinstance(record.msg, bytes) and (not record.kw or record.kw == {'exc_info': None})
...
class FileHandler(Handler):
    __doc__ = 'File handler which supports reopening of logs.\n    '
...
class RotatingFileHandler(FileHandler):
    ...
    def __init__(self, filename, mode='ab', maxBytes=536870912, backupCount=10):
        """
    ...
    def doRollover(self):
        ...
        if self.maxBytes <= 0:
            ...
            return self.stream.tell() >= self.maxBytes or None
        self.stream.close()
