...
# [GCC 8.4.0]
# Embedded file name: ./build/lib/supervisor/tests/test_datatypes.py
# Compiled at: 2019-04-05 17:19:18
# Size of source mod 2**32: 27244 bytes
"""Test suite for supervisor.datatypes"""
import os, signal, socket, tempfile, unittest
...
class DictOfKeyValuePairsTests(unittest.TestCase):
    ...
    def test_handles_unquoted_non_alphanum(self):
        actual = self._callFUT('HOME=/home/auser,FOO=/.foo+(1.2)-_/,SUPERVISOR_SERVER_URL=http://127.0.0.1:9001')
        expected = {'HOME':'/home/auser', 
         'FOO':'/.foo+(1.2)-_/',  'SUPERVISOR_SERVER_URL':'http://127.0.0.1:9001'}
        self.assertEqual(actual, expected)
...
class ExistingDirpathTests(unittest.TestCase):
    ...
    def test_raises_if_dir_does_not_exist(self):
        ...
        try:
            ...
            try:
                expected = 'The directory named as part of the path %s does not exist' % path
                self.assertEqual(e.args[0], expected)
            finally:
                e = None
                del e
    ...
    def test_raises_if_exists_but_not_a_dir(self):
        path = os.path.join(os.path.dirname(__file__), os.path.basename(__file__), 'foo')
        try:
            self._callFUT(path)
            self.fail()
        except ValueError as e:
...
class ColonSeparatedUserGroupTests(unittest.TestCase):
    ...
    def test_missinguser_username(self):
        self.assertRaises(ValueError, self._callFUT, 'godihopethisuserdoesntexist')

    def test_missinguser_username_and_groupname(self):
        self.assertRaises(ValueError, self._callFUT, 'godihopethisuserdoesntexist:foo')
