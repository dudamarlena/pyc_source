...
# [GCC 8.4.0]
# Embedded file name: ./build/lib/supervisor/tests/test_supervisord.py
# Compiled at: 2019-04-05 17:19:18
# Size of source mod 2**32: 33035 bytes
import unittest, time, signal, sys, os, tempfile, shutil
...
try:
    ...
    class EntryPointTests(unittest.TestCase):
        ...
        def test_main_noprofile(self):
            ...
            conf = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'fixtures', 'donothing.conf')
            new_stdout = StringIO()
    ...
    class SupervisordTests(unittest.TestCase):
        ...
        def test_handle_sigterm(self):
            ...
            self.assertEqual(options.logger.data[0], 'received SIGTERM indicating exit request')
        ...
        def test_handle_sigint(self):
            ...
            supervisord.handle_signal()
            self.assertEqual(supervisord.options.mood, SupervisorStates.SHUTDOWN)
            self.assertEqual(options.logger.data[0], 'received SIGINT indicating exit request')
        ...
        def test_handle_sigquit(self):
            ...
            supervisord.handle_signal()
            self.assertEqual(supervisord.options.mood, SupervisorStates.SHUTDOWN)
            self.assertEqual(options.logger.data[0], 'received SIGQUIT indicating exit request')
        ...
        def test_handle_sighup_in_running_state(self):
            ...
            self.assertEqual(options.logger.data[0], 'received SIGHUP indicating restart request')
        ...
        def test_handle_sighup_in_shutdown_state(self):
            ...
            supervisord.handle_signal()
            self.assertEqual(supervisord.options.mood, SupervisorStates.SHUTDOWN)
            self.assertEqual(options.logger.data[0], 'ignored SIGHUP indicating restart request (shutdown in progress)')
        ...
        def test_handle_sigchld(self):
            ...
            supervisord.handle_signal()
            self.assertEqual(supervisord.options.mood, SupervisorStates.RUNNING)
            msgs = ('received SIGCHLD indicating a child quit', 'received SIGCLD indicating a child quit')
        ...
        def test_handle_sigusr2(self):
            ...
            self.assertEqual(options.logger.data[0], 'received SIGUSR2 indicating log reopen request')
        ...
        def test_handle_unknown_signal(self):
            ...
            supervisord.handle_signal()
            self.assertEqual(supervisord.options.mood, SupervisorStates.RUNNING)
            self.assertEqual(options.logger.data[0], 'received SIGUSR1 indicating nothing')
        ...
        def test_diff_changed(self):
            ...
            def make_pconfig(name, command, **params):
                result = {'name':name, 
                 'command':command,  'directory':None, 
                 'umask':None,  'priority':999,  'autostart':True,  'autorestart':True, 
                 'startsecs':10,  'startretries':999,  'uid':None, 
                 'stdout_logfile':None,  'stdout_capture_maxbytes':0,  'stdout_events_enabled':False, 
                 'stdout_logfile_backups':0, 
                 'stdout_logfile_maxbytes':0,  'stdout_syslog':False, 
                 'stderr_logfile':None, 
                 'stderr_capture_maxbytes':0,  'stderr_events_enabled':False, 
                 'stderr_logfile_backups':0, 
                 'stderr_logfile_maxbytes':0,  'stderr_syslog':False, 
                 'redirect_stderr':False, 
                 'stopsignal':None, 
                 'stopwaitsecs':10,  'stopasgroup':False, 
                 'killasgroup':False, 
                 'exitcodes':(0, ), 
