...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/mixed.py
# Compiled at: 2008-03-04 16:18:26
...
class Mixed(int):
    ...
    for d in range(8, 1, -1):
        for n in range(d - 1, 0, -1):
            knownFractions[int(round(float(n) / float(d) * 10 ** float_precision))] = (
             n, d)

    reader = re.compile('(\\-)?(\\s*(\\d+)[\\s\\-]+)?(\\d+)\\s*/\\s*(\\d+)')
