...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/esdrt/content/utilities/roles.py
...
QUERY_LDAP_ROLES = ldap_utils.format_or('cn', (
 LDAP_SR + '-sector*-*',
 LDAP_QE + '-sector*',
 LDAP_RE + '-sector*-*',
 LDAP_LR + '-*',
...
def setup_reviewfolder_roles(folder):
    ...
    grant = chain(product([ROLE_RP1], filter(f_start_sr, groups)), product([ROLE_QE], filter(f_start_qe, groups)), product([ROLE_RP2], filter(f_start_re, groups)), product([ROLE_LR], filter(f_start_lr, groups)), product([ROLE_MSA], filter(f_start_msa, groups)))
