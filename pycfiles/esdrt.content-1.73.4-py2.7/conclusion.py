...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/esdrt/content/conclusion.py
...
class IConclusion(form.Schema, IImageScaleTraversable):
    ...
    """
    closing_reason = schema.Choice(title=_('Status of observation'), vocabulary='esdrt.content.conclusionreasons', required=True)
    text = schema.Text(title=_('Internal note for expert/reviewers'), required=True)
...
class Conclusion(dexterity.Container):
    ...
    def reason_value(self):
        return self._vocabulary_value('esdrt.content.conclusionreasons', self.closing_reason)
...
class AddForm(dexterity.AddForm):
    ...
    def updateFields(self):
        ...
        conclusion_fields = field.Fields(IConclusion).select('closing_reason', 'text', 'remarks')
    ...
    def create(self, data={}):
        ...
        api.content.transition(obj=self.context, transition='phase1-draft-conclusions')
