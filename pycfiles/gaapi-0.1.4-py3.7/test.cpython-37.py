...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/gaapi/test.py
# Compiled at: 2019-11-12 13:26:22
...
def _decamelify(obj):
    func = lambda string: re.sub('([a-z0-9])([A-Z])', '\\1_\\2', string).lower()
    return modify_object(obj, func)


def _camelify(obj):
    func = lambda string: re.sub('_(\\w)', lambda match: match.group(1).upper(), string)
    return modify_object(obj, func)
...
def modify_object(obj, func):
    if isinstance(obj, dict):
        ...
        if isinstance(obj, str):
            return obj.isupper() or func(obj)
    return obj
...
class Client(object):
    ...
    def batch_get(self, query, cache_ttl=None):
    ...
    def key_fn(self, a, k):
        return hashlib.md5(pickle.dumps((self.request_query, a, k))).hexdigest()
