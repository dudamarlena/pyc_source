...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyquchk/arbitraries/sequences.py
# Compiled at: 2013-09-29 04:58:21
from collections import namedtuple
from itertools import islice
from .arbitrary import Arbitrary, MappedArbitrary, FilteredArbitrary, get_arbitrary, with_samples, add_spec_processor
from .utils import filter_possible, unique, product
from .numbers import int_
from .primitives import char, letter
__all__ = [
 'tuple_', 'namedtuple_', 'dict_fixkeys',
...
class tuple_(Arbitrary):
    ...
    def could_generate(self, x):
        return all(arb.could_generate(y) for arb, y in zip(self.arbitraries, x))

    def next_random(self):
        return tuple(arb.next_random() for arb in self.arbitraries)

    def gen_serial(self):
        return product(*[ arb.gen_serial() for arb in self.arbitraries
                        ])
...
class dict_fixkeys(MappedArbitrary):
    ...
    def __init__(self, items_lst=None, **items_dct):
        self.inner_arb = namedtuple_('DictItems', items_lst or items_dct.items())
...
class list_(Arbitrary):
    ...
    def could_generate(self, x):
        return self.length.could_generate(len(x)) and all(self.elements.could_generate(el) for el in x)
...
class list_ordered(MappedArbitrary):
    ...
    @Arbitrary.args_for_sample(length=int_(0, 10), elements=int_(-100, 100))
    def __init__(self, length=int_(0, 20), elements=int_, key=lambda x: x, reverse=False):
...
class dict_(MappedArbitrary):
    ...
    def __init__(self, length=int_(0, 20), keys=int_, values=int_):
