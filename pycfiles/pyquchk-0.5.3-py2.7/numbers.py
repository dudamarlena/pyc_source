...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyquchk/arbitraries/numbers.py
# Compiled at: 2013-09-21 05:29:08
import random
...
class int_(Arbitrary):
    ...
    def __init__(self, low=-4294967296, high=4294967295, uniform=False):
        self.low = low
        self.high = high
    ...
    def gen_serial(self):
        ...
        elif abs(self.low) < abs(self.high):
    ...
    def shrink(self, x):
        ...
        xabs = abs(x)
        yield xsign * (xabs // 2)
        yield xsign * (xabs // 100000 * 100000)
        yield xsign * (xabs // 100 * 100)
        if x < 0:
            yield xabs
...
class float_(Arbitrary):
    ...
    def __init__(self, low=-100, high=100, uniform=False):
        ...
        _, min_exp = frexp(min(abs(low), abs(high))) if len(signs) == 1 else None, max_exp - 40
        self.exp = int_(min_exp, max_exp)
        self.sig = int_(0, 9007199254740992, uniform=True)
        return
    ...
    def gen_serial(self):
        ...
        yield self.low
        yield self.high
        for sign, sig, exp in product(self.sign.gen_serial(), self.sig.gen_serial(), self.exp.gen_serial()):
            sig_p2 = 2 ** sig.bit_length()
            sig = 1.0 * sig / sig_p2
