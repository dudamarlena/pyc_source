...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/notifications/channels.py
...
class BasicWebSocketChannel(BaseNotificationChannel):
    __doc__ = 'It creates a RabbitMQ user for each user (based on their username).'

    def _connect(self):
        """Connect to the RabbitMQ Server."""
        rabbit_mq_url = settings.NOTIFICATIONS_RABBIT_MQ_URL
        connection = pika.BlockingConnection(pika.connection.URLParameters(rabbit_mq_url))
        return connection
    ...
    def notify(self, message):
        ...
        channel.basic_publish(exchange='',
          routing_key=(recipient.username),
