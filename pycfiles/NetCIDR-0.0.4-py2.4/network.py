...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/netcidr/network.py
# Compiled at: 2007-12-23 01:29:51
...
def validateVLSM(vlsm):
    ...
    if 0 > int(vlsm) or int(vlsm) > 32:
        raise ValueError, 'The variable length subnet mask, or ' + '"prefix length" must be between 0 and 32, inclusive.'


def getDirection(src_cidr_obj, dst_cider_obj, inside_nets):
    """
    ...
    >>> inside_nets = Networks([inside_block1, inside_block2, inside_block3])

    # Define some hosts
    >>> laptop = CIDR('192.168.4.100')
    >>> laptop in inside_nets
    True
    >>> webserver = CIDR('10.2.5.10')
    >>> webserver in inside_nets
    True
    >>> fileserver = CIDR('172.16.9.50')
    >>> fileserver in inside_nets
    True
    >>> google_www = CIDR('64.233.167.147')
    >>> google_www in inside_nets
    False
    >>> yahoo_mail = CIDR('216.136.224.155')
    >>> yahoo_mail in inside_nets
    False

    >>> getDirection(laptop, webserver, inside_nets)
    >>> getDirection(fileserver, webserver, inside_nets)
    >>> getDirection(laptop, yahoo_mail, inside_nets)
    'outbound'
    >>> getDirection(fileserver, google_www, inside_nets)
    'outbound'
    >>> getDirection(webserver, google_www, inside_nets)
    'outbound'
    >>> getDirection(google_www, webserver, inside_nets)
    'inbound'
    >>> getDirection(yahoo_mail, laptop, inside_nets)
    'inbound'
    """
    if dst_cider_obj in inside_nets and src_cidr_obj not in inside_nets:
        return 'inbound'
    elif src_cidr_obj in inside_nets and dst_cider_obj not in inside_nets:
        return 'outbound'
...
class CIDR(object):
    ...
    def __init__(self, cidr_string):
        ...
        if len(net) == 2:
            ...
            raise ValueError, "There appear to be too many '/' in " + 'your network notation.'
        else:
            net = net[0]
        self.vlsm = vlsm
        self.net = net
...
class Networks(list):
    ...
    def isInRange(self, cidr_obj, cidr_tuple=(), cidr_netblock=None):
        ...
        if not (cidr_tuple or cidr_netblock):
            raise ValueError, 'You must provide either a tuple of ' + 'CIDR objects or a CIDR object that represents a ' + 'netblock.'
        if cidr_tuple:
            (start, end) = cidr_tuple
        else:
            (start, end) = cidr_netblock.getHostRange()
        if start.getOctetTuple() < cidr_obj.getOctetTuple() < end.getOctetTuple():
            return True
        return False
