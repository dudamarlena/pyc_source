...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/netcidr/ipmath.py
# Compiled at: 2007-12-30 20:25:37
minOct = 0
maxOct = 2 ** 8 - 1
maxTotal = 256 ** 4 - 1
...
def octets2int(octs):
    ...
    for octet in octs:
        if not minOct <= octet <= maxOct:
            msg = 'Each octect must have a value between 0 and 2**8-1, ' + 'inclusive.'
            raise ValueError, msg
        num <<= 8
        num += octet
