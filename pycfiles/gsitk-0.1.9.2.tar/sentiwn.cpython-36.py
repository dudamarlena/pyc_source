...
# [GCC 8.4.0]
# Embedded file name: /home/oaraque/git/gsi/gsitk/gsitk/features/sentiwn.py
# Compiled at: 2017-02-23 11:38:01
...
class SentiWordNet(object):
    ...
    def __init__(self, swn_file):
        ...
        self.pos_synset = self._SentiWordNet__parse_swn_file()
    ...
    def __parse_swn_file(self):
        ...
        head_less_swn_data = filter(lambda line: not re.search('^\\s*#', line), swn_data)
        for data in head_less_swn_data:
            fields = data.strip().split('\t')
            try:
                pos, syn_set_id, pos_score, neg_score, syn_set_score, gloss = fields
            except:
                print('Found data without all details')

            if pos and syn_set_score:
                pos_synset_hash[(pos, int(syn_set_id))] = (
                 float(pos_score),
                 float(neg_score))
    ...
    def get_score(self, word, pos=None):
        ...
        for synset in synsets:
            if (
             synset.pos(), synset.offset()) in self.pos_synset.keys():
