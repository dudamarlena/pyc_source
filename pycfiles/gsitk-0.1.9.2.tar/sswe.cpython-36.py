...
# [GCC 8.4.0]
# Embedded file name: /home/oaraque/git/gsi/gsitk/gsitk/features/sswe.py
...
"""
import os, yaml, logging, csv, numpy as np
...
class SSWE(Embedding, TransformerMixin):
 ...
 1, 0, 0], download=True):
    ...
    def _download_model(self):
        if self._download:
            dataset_utils._maybe_download(self.info['name'], self.info['url'], self.info['filename'], self.info['expected_bytes'], self.info['sha256'])

    def load_sswe(self, path=config.DATA_PATH):
        data_path = os.path.join(path, self.info['name'])
