...
# [GCC 8.4.0]
# Embedded file name: /home/oaraque/git/gsi/gsitk/gsitk/features/simon.py
# Compiled at: 2018-07-13 09:44:39
...
class Simon(TransformerMixin, BaseEstimator):
    ...
    def __init__(self, lexicon, n_lexicon_words=250, embedding=None, wordnet_metric=None, wordnet_cache=False, pooling=np.max, weighting=False, remove_stopwords=False, lex_values=None):
    ...
    def _prepare_lexicon(self):
        ...
        for lexicon_split in self._lexicon_split:
            lex_words = sorted([(word_i, count[word_i]) for word_i in lexicon_split], key=(lambda x: x[1]),
              reverse=True)
        ...
        if self.embedding is None:
            ...
            for i, lex_word_i in enumerate(self.lex_words):
                if lex_word_i not in all_lemmas:
                    pass
                else:
    ...
    def _generate_wordnet_cache(self):
        ...
        for index, vocab_word in enumerate(self.all_lemmas):
            cache[index] = np.array([self.wns.word_similarity(vocab_word, lex_word) for lex_word in self.lex_words])
    ...
    def _compute_with_wordnet(self, x):
        ...
        for i, x_i in enumerate(x):
            ...
            for j, word in enumerate(x_i):
                m_i_j = [self.wns.word_similarity(word, lex_word, self.wordnet_metric) for lex_word in self.lex_words]
