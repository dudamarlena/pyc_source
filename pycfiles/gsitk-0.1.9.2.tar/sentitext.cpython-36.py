...
# [GCC 8.4.0]
# Embedded file name: /home/oaraque/git/gsi/gsitk/gsitk/features/sentitext.py
# Compiled at: 2018-02-19 13:31:10
# Size of source mod 2**32: 12302 bytes
from nltk.corpus import stopwords
from gsitk.features.sentiwn import SentiWordNet
import re, nltk, csv, sys, unicodedata, tarfile, logging, string, pandas as pd, numpy as np, nltk
...
def extract_ngrams(texts):
    pos_dictionary = {'JJR':'JJ', 
     'JJS':'JJ',  'NNS':'NN',  'NNP':'NN',  'NNPS':'NN',  'PRP$':'PRP', 
     'RBD':'RB',  'RBS':'RB',  'RBP':'RB',  'VBZ':'VB',  'VBD':'VB', 
     'VBP':'VB',  'VBN':'VB',  'VBG':'VB'}
    ...
    return (
     unigrams_words, unigrams_lemmas, pos_tagged)
...
def clean_pos(pos):
    pos_tags = {'NN':'NN', 
     'NNP':'NN',  'NNP-LOC':'NN',  'NNS':'NN',  'JJ':'JJ',  'JJR':'JJ',  'JJS':'JJ',  'RB':'RB',  'RBR':'RB',  'RBS':'RB', 
     'VB':'VB',  'VBD':'VB',  'VGB':'VB',  'VBN':'VB',  'VBP':'VB',  'VBZ':'VB'}
    if pos in pos_tags:
        pos = pos_tags[pos]
    return pos
...
def load_nrc_dict(folder=config.DATA_PATH):
    nrc = pd.read_csv((os.path.join(folder, 'NRC-emotion-lexicon-wordlevel-alphabetized-v0.92.txt')),
      delimiter='\t',
      header=None,
      names=[
     'word', 'emotion', 'value'])
...
def sentiscore(tokens):
    ...
    okays = ['ok', 'okay']
    for word_pos in words_pos:
        if not word_pos[0] in string.punctuation:
            if word_pos[0] in okays:
                pass
            else:
    ...
    for word in tokens:
        for synset in tokens[word]:
            if synset is None:
                ...
                info = synset.name().split('.'.replace(' ', ' '))
...
def get_score(word, pos=None):
    ...
    if word == 'd' or pos == 'd' or pos not in list_pos:
        ...
        for synset in synsets:
            if (
             synset.pos(), synset.offset()) in swn.pos_synset:
                pos_val, neg_val = swn.pos_synset[(synset.pos(), synset.offset())]
                senti_scores.append({'pos':pos_val,  'neg':neg_val,  'obj':1.0 - pos_val - neg_val, 
                 'synset':synset})
...
def extract_features(bag_of_words, bag_of_lemmas, text):
    ...
    feature_set['words_all_caps'] = sum([1 for token in text.split() if token.isupper() if len(token) >= 3])
