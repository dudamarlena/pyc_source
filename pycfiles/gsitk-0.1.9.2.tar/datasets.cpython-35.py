...
# [GCC 8.4.0]
# Embedded file name: /home/oaraque/git/gsi/gsitk/gsitk/datasets/datasets.py
# Compiled at: 2017-03-21 05:56:45
# Size of source mod 2**32: 6705 bytes
"""
Access operations to the available datasets.
"""
import os, inspect, logging, yaml, glob, importlib, hashlib, zipfile
...
class Dataset:
    ...
    def maybe_download(self):
        utils._maybe_download(data_name=self.name, url=self.info['properties']['url'], filename=self.info['properties']['filename'], expected_bytes=self.info['properties']['expected_bytes'], sha256=self.info['properties']['sha256'])
    ...
    def prepare_data(self, download=True):
        ...
        if download:
            if self.info['properties'].get('download') is None or self.info['properties'].get('download') is True:
                self.maybe_download()
            else:
...
class DatasetManager:
    ...
    def get_datasets(self):
        ...
        path = os.path.dirname(os.path.abspath(__file__))
        info_files = [dataset for dataset in glob.glob(os.path.join(path, '*.yaml'))]
        info_files.extend([dataset for dataset in glob.glob(os.path.join(path, '*.yml'))])
        infos = dict()
        objs = dict()
        for info_name in info_files:
            ...
            for data_name, data_class in inspect.getmembers(dataset_module):
                if inspect.isclass(data_class) and data_name.lower() == name.lower():
                    found = True
                    obj = data_class(info)
                    objs[name] = obj
                    break
            ...
            if not found:
                raise ImportError('Module gsitk.datasets.{} does not contain the desired class.'.format(name))
