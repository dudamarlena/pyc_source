...
# [GCC 8.4.0]
# Embedded file name: /home/oaraque/git/gsi/gsitk/gsitk/evaluation/evaluation.py
# Compiled at: 2019-04-03 05:46:49
# Size of source mod 2**32: 14042 bytes
"""
Evaluation of classifiers on datasets.
"""
import logging
from collections import namedtuple
import itertools, copy, numpy as np, pandas as pd
...
class Evaluation:
    ...
    def __init__(self, tuples=None, datasets=None, pipelines=None, features=None):
        ...
        self.pipelines = self._pipelines_setup(pipelines) if pipelines is not None else pipelines
        self.tuples = tuples if tuples is not None else self.generate_tuples()
        ...
        if metric_name == 'accuracy':
            ...
            if metric_name == 'recall':
                ...
                if metric_name == 'recall_macro':
                    return lambda labels, preds: recall_score(labels,
                      preds, average='macro')
                else:
                    if metric_name == 'precision':
                        ...
                        if metric_name == 'precision_macro':
                            return lambda labels, preds: precision_score(labels,
                              preds, average='macro')
                        if metric_name == 'f1':
                            return f1_score
                    if metric_name == 'f1_weighted':
                        return lambda labels, preds: f1_score(labels,
    ...
    def generate_tuples(self):
        ...
        for combination in itertools.product(self.datasets.keys(), self.pipelines.keys()):
            tuples.append(EvalPipeline(self.pipelines[combination[1]], combination[0]))
    ...
    def evaluate_model(self, features_name, model):
        ...
        prediction = self.Prediction('{}__{}'.format(model.name, features_name), _features.dataset, _features.name, model.name, predictions)
    ...
    def evaluate_pipeline(self, tuple):
        ...
        logger.debug('Pipeline {} predicting from dataset {}'.format(tuple.pipeline.named_steps, tuple.dataset))
        if tuple.features is not None:
        ...
        prediction = Prediction('{}__{}'.format(tuple.name, input_name), tuple.dataset, None, tuple.name, predictions, description)
    ...
    def cross_evaluate_model(self, cross_eval_tuple, multi_core):
        ...
        logger.debug('Cross evaluating: model={}, features={}, folds={}, labels={}'.format(model, _features, folds, _labels))
        n_jobs = -1 if multi_core else 1
        logger.debug('Using n_jobs={}'.format(n_jobs))
        metrics = [
         None] * len(self.metrics_defs)
        for metric in cross_eval_tuple.metrics:
            args = [
             model, features.values, labels]
    ...
    def evaluate(self, multi_core=True):
        ...
        for tuple in self.tuples:
            if isinstance(tuple, EvalTuple):
                ...
                if isinstance(tuple, CrossEvalTuple):
                    self.cross_evaluate_model(tuple, multi_core)
                else:
                    if isinstance(tuple, _EvalPipeline):
        ...
        for prediction in self.predictions.values():
            ...
            i = self._next_i()
            result = [
             test_dataset_name, features_name, model_name, False]
            result.extend(scores)
            self.results.loc[i, :] = result
            try:
