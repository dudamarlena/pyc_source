...
# Compiled at: 2017-12-13 11:15:59
# Size of source mod 2**32: 8425 bytes
"""
Access operations to the available datasets.
"""
import os, inspect, logging, yaml, glob, hashlib, zipfile
...
class Dataset:
    ...
    def maybe_download(self, move=False):
        utils._maybe_download(data_path=(self.config.DATA_PATH), resources_path=(self.config.RESOURCES_PATH),
    ...
    def prepare_data(self, download=True):
        ...
        processed_path = os.path.join(self.data_path, self.info['properties']['processed_file'])
        file_path = os.path.exists(data_file) or os.path.join(self.data_path, self.info['properties']['filename'])
...
class DatasetManager:
    ...
    def find_datasets(self, path=None):
        ...
        for ext in extensions:
            info_files.extend([dataset for dataset in glob.glob(os.path.join(path, '*.{}'.format(ext)))])
        ...
        if not isinstance(info, dict):
            ...
            for data_name, data_class in inspect.getmembers(dataset_module):
                if inspect.isclass(data_class):
                    if data_name.lower() == module_name.lower():
                        obj = data_class(info, config)
                        break

            raise obj or ImportError('Module {} not found the desired class.'.format(name))
        return obj
