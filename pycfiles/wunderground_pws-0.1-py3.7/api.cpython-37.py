...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/wunderground_pws/api.py
# Compiled at: 2019-05-19 14:09:35
# Size of source mod 2**32: 5626 bytes
import datetime, simplejson, requests
...
class WUndergroundAPI:
    ...
    def __init__(self, api_key: str, default_station_id: str=None, units: str=units.METRIC_SI_UNITS, fmt: str='json'):
        ...
        :param str default_station_id: Default station ID.         Can be None, if not specified you must pass it every time you use PWS methods
        :param str units: The unit of measure for the response. The following values are supported:         `wundeground_pws.ENGLISH_UNITS`, `wundeground_pws.METRIC_UNITS`,        `wundeground_pws.HYBRID_UNITS`, `wundeground_pws.METRIC_SI_UNITS`
    ...
    def _request(self, path: tuple, **kwargs):
        ...
        r = requests.get((self._base_url + '/'.join(path)), params={**{'apiKey':self.api_key, 
         'units':self.units, 
    ...
    def current(self, station_id: str=None):
        ...
        """
        if not self.default_station_id:
            if not station_id:
                raise StationIDNotSuppliedError('You must supply a station ID in __init__() or fill the station_id parameter in every method!')
        return self._request(('pws', 'observations', 'current'), stationId=(station_id or self.default_station_id))
    ...
    def hourly(self, station_id: str=None):
        ...
        if not self.default_station_id:
            if not station_id:
                raise StationIDNotSuppliedError('You must supply a station ID in __init__() or fill the station_id parameter in every method!')
        return self._request(('pws', 'observations', 'hourly', '7day'), stationId=(station_id or self.default_station_id))
    ...
    def summary(self, station_id: str=None):
        ...
        if not self.default_station_id:
            if not station_id:
                raise StationIDNotSuppliedError('You must supply a station ID in __init__() or fill the station_id parameter in every method!')
        return self._request(('pws', 'dailysummary', '7day'), stationId=(station_id or self.default_station_id))
    ...
    def history(self, date: datetime.date, granularity: str='hourly', station_id: str=None):
        ...
        if not self.default_station_id:
            if not station_id:
                raise StationIDNotSuppliedError('You must supply a station ID in __init__() or fill the station_id parameter in every method!')
        return self._request(('pws', 'history', granularity), date=(date.strftime('%Y%m%d')),
