...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pagseguro_xml\consultas\v3\classes\detalhes.py
...
class CreditorFees(XMLAPI):
    ...
    def __init__(self):
        ...
        self.installmentFeeAmount = TagDecimal(nome=b'installmentFeeAmount', raiz=b'//transaction/creditorFees', tamanho_max=10, decimal_digitos=2, obrigatorio=False)
        self.operationalFeeAmount = TagDecimal(nome=b'operationalFeeAmount', raiz=b'//transaction/creditorFees', tamanho_max=10, decimal_digitos=2, obrigatorio=False)
        self.intermediationRateAmount = TagDecimal(nome=b'intermediationRateAmount', raiz=b'//transaction/creditorFees', tamanho_max=10, decimal_digitos=2, obrigatorio=False)
        self.intermediationFeeAmount = TagDecimal(nome=b'intermediationFeeAmount', raiz=b'//transaction/creditorFees', tamanho_max=10, decimal_digitos=2, obrigatorio=False)
...
class ClasseTransacaoDetalhes(classe_v2.ClasseTransacaoDetalhes):
    ...
    def set_xml(self, arquivo):
        if self._le_xml(arquivo):
            ...
            self.items = self.le_grupo(b'/transaction/items/item', Item)
