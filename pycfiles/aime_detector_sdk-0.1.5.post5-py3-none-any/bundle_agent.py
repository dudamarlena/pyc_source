...
# Embedded file name: /home/grad03/fengl/AIMES_project/aimes.bundle/src/aimes/bundle/agent/bundle_agent.py
# Compiled at: 2015-09-21 10:55:53
import Queue, threading, logging, socket, datetime, time, os, xml.etree.ElementTree as ET, re, json, paramiko
...
class RemoteBundleAgent(threading.Thread):
    ...
    def setup_ssh_connection(self, credential):
        ...
        self.ssh.connect(hostname=self._login_server, port=credential.get('port', 22), username=credential.get('username'), password=credential.get('password'), key_filename=credential.get('key_filename'))
        self.cmd_prefix = ''
...
backdoor_cluster_num_nodes = {'stampede.tacc.xsede.org': 6400, 
   'trestles.sdsc.xsede.org': 324, 
   'gordon.sdsc.xsede.org': 1024, 
   'hopper.sdsc.xsede.org': 1024, 
   'blacklight.psc.xsede.org': 256, 
   'hopper.nersc.gov': 6384}
backdoor_trestles_properties_queues = {'exclusive': [
               'normal'], 
   'exclusive,work': [
                    'normal'], 
   'shared': [
   ...
   'shared,abaqus': [
                   'shared']}
backdoor_trestles_queue_limits = {'num_procs_limit': 32 * 32, 
   'max_walltime': 48 * 3600}
backdoor_gordon_interested_queues = [
 'normal', 'vsmp']
backdoor_gordon_queue_num_procs_limits = {'normal': 64 * 16, 
   'vsmp': 1 * 16}
backdoor_gordon_properties_queues = {'native,flash': [
                  'normal'], 
   'native,noflash': [
                    'normal'], 
   'native,bigflash': [
...
backdoor_hopper_queue_jobs_limits = {'debug': 2, 
   'regular': 32, 
   'low': 12, 
   'thruput': 500, 
   'scavenger': 2, 
   'premium': 1}
backdoor_hopper_queue_num_procs_limits = {'debug': 512 * 24, 
   'regular': 4096 * 24, 
   'low': 683 * 24, 
   'thruput': 2 * 24, 
   'scavenger': 683 * 24, 
...
class PbsAgent(RemoteBundleAgent, threading.Thread):
    ...
    def get_queue_config(self, flag=0):
        ...
        if exit_status != 0:
            err_msg = ("'qstat -Q -f' return ({}): stdout({}), stderr({})").format(exit_status, stdout, stderr)
            self.error(err_msg)
            return
        _queue_info = {}
            ...
            if line_str.find('Queue') == 0:
                ...
                if queue_attr_k == 'acl_user_enable':
                    ...
                    if queue_attr_v.count(':') == 2:
                        ...
                        _queue_info[queue_name]['max_walltime'] = 86400 * d + 3600 * h + 60 * m + s
                    ...
                    _queue_info[queue_name]['num_queueing_jobs'] = int(queue_attr_v[pos + len('Queued:'):queue_attr_v.find(' ', pos + len('Queued:'))])
                    pos = queue_attr_v.find('Running:')
                    _queue_info[queue_name]['num_running_jobs'] = int(queue_attr_v[pos + len('Running:'):queue_attr_v.find(' ', pos + len('Running:'))])

        for k, v in _queue_info.items():
            if 'gordon' in self.hostname and v['queue_name'] not in backdoor_gordon_interested_queues or 'blacklight' in self.hostname and v['queue_name'] not in backdoor_blacklight_interested_queues or 'hopper' in self.hostname and v['queue_name'] not in backdoor_hopper_interested_queues or 'restricted_access' in v and v['restricted_access'] == True or 'enabled' in v and v['enabled'] != True or 'started' in v and v['started'] != True:
                del _queue_info[k]
                continue
            if 'enabled' in v:
    ...
    def qstat_x(self):
        ...
        if exit_status != 0:
            err_msg = ("'qstat -a -x' return ({}): stdout({}), stderr({})").format(exit_status, stdout, stderr)
            self.error(err_msg)
            return
        else:
            queue_nodes = {}
            ...
            for job in root.findall('Job'):
                ...
                if _queue in _interested_queues:
                    if _queue not in queue_nodes:
                        queue_nodes[_queue] = {'num_busy_procs': 0, 'num_queueing_jobs': 0, 
                           'num_running_jobs': 0}
    ...
    def get_workload_hopper(self):
        ...
        if queue_nodes:
            ...
            for _queue in backdoor_hopper_interested_queues:
                workload[_queue] = {'alive_nodes': backdoor_cluster_num_nodes['hopper.nersc.gov'], 
    ...
    def get_workload_blacklight(self):
        ...
        queue_nodes = self.qstat_x()
        if queue_nodes:
            workload = {'batch': {'alive_nodes': 256, 
                         'alive_procs': 4096, 
                         'busy_nodes': 0, 
                         'busy_procs': 0, 
                         'free_nodes': 256, 
    ...
    def pbsnodes(self):
        ...
        if exit_status != 0:
            err_msg = ("'pbsnodes -a -x' return ({}): stdout({}), stderr({})").format(exit_status, stdout, stderr)
            self.error(err_msg)
            return
        else:
            root = ET.fromstring(stdout)
            ...
            for node in root.findall('Node'):
                try:
                    node_list[node.find('name').text] = [node.find('state').text.strip(),
                        ...
                        if properties is not None:
                            if properties in properties_queues:
                                for _q in properties_queues[properties]:
                                    ...
                                    elif state == 'job-exclusive' or state == 'free' and jobs != None:
...
backdoor_stampede_queue_proc_per_node = {'normal': 16, 
   'development': 16, 
   'largemem': 32, 
   'serial': 16, 
   'large': 16}
backdoor_stampede_queue_num_procs_limits = {'normal': 256 * 16, 
   'development': 16 * 16, 
   'largemem': 4 * 32, 
   'serial': 1 * 16, 
   'large': 1024 * 16}
backdoor_stampede_queue_jobs_limits = {'normal': 50, 
   'development': 1, 
   'largemem': 4, 
   'serial': 8, 
   'large': 50}
backdoor_stampede_properties_queues = {'native,flash': [
                  'normal'], 
   'native,noflash': [
                    'normal'], 
   'native,bigflash': [
...
class SlurmAgent(RemoteBundleAgent, threading.Thread):
    ...
    def get_queue_config(self, flag=0):
        ...
        if exit_status != 0:
            err_msg = ("'sinfo' return ({}): stdout({}), stderr({})").format(exit_status, stdout, stderr)
            self.error(err_msg)
            return
        else:
            _queue_info = {}
            for line_str in stdout.splitlines()[1:]:
                ...
                if flag == 0:
                    ...
                    _timelimit = 86400 * d + 3600 * h + 60 * m + s
                    if 'stampede' in self.hostname and _partition in backdoor_stampede_interested_queues:
                        _queue_info[_partition] = {'queue_name': _partition, 'max_walltime': _timelimit, 'num_procs_limit': backdoor_stampede_queue_num_procs_limits[_partition], 'queued_jobs_limit': backdoor_stampede_queue_jobs_limits[_partition]}
                elif 'stampede' in self.hostname and _partition in backdoor_stampede_interested_queues:
                    _active_nodes, _idle_nodes, _other_nodes, _total_nodes = [ int(n) for n in _nodes.split('/') ]
                    _queue_info[_partition] = {'queue_name': _partition, 'active_nodes': _active_nodes, 'idle_nodes': _idle_nodes, 'other_nodes': _other_nodes, 'total_nodes': _total_nodes}
    ...
    def get_workload(self):
        ...
        if exit_status != 0:
            err_msg = ("'squeue' return ({}): stdout({}), stderr({})").format(exit_status, stdout, stderr)
            self.error(err_msg)
            return
        else:
            workload = {}
            if 'stampede' in self.hostname:
                for k, v in self.queue_info.items():
                    workload[k] = {'alive_nodes': v['total_nodes'] - v['other_nodes'], 
...
class CondorAgent(RemoteBundleAgent):
    ...
    def _query_site_node_slot(self):
        ...
        """
        exit_status, stdout, stderr = self.run_cmd("condor_status -pool osg-flock.grid.iu.edu -state -format '%s' GLIDEIN_Site -format ' %s' Machine -format ' %s\n' State | sort | uniq -c")
        if exit_status != 0:
            print ('CondorAgent _query_site_node_slot failed:\n{}\n{}\n{}').format(exit_status, stdout, stderr)
            return
        else:
            try:
                site_node_slot = {'num_nodes': 0, 
                   'busy_jobslots': 0, 
                   'idle_jobslots': 0, 
                   'site_info': {}}
                for l in stdout.splitlines():
                    ...
                    if len(l_token) == 4:
                        ...
                        if _site not in site_node_slot['site_info']:
                            site_node_slot['site_info'][_site] = {'site_name': _site, 'node_list': [], 'num_nodes': 0, 
                               'busy_jobslots': 0, 
                               'idle_jobslots': 0}
                ...
                print ('CondorAgent _query_site_node_slot failed:\n{}\n{}\n{}\n{}').format(stdout, stderr, str(e.__class__), str(e))
                return
    ...
    def _query_jobs(self):
        """Query the total number of running/idle/held jobs.
           ...
           Return:
               ...
               Failure - None
        """
        exit_status, stdout, stderr = self.run_cmd('condor_status -pool osg-flock.grid.iu.edu -schedd -total')
        if exit_status != 0:
            print ('CondorAgent _query_total_jobs failed:\n{}\n{}\n{}').format(exit_status, stdout, stderr)
            return
        else:
            try:
                return [ int(n) for n in stdout.splitlines()[(-1)].strip().split()[1:3] ]
            except Exception as e:
    ...
    def _query_job_slots(self, site=None):
        ...
        Benchmarking         8     0         8       0          0       0
           ...
           Return:
               ...
               Failure - None
        """
        exit_status, stdout, stderr = self.run_cmd('condor_status -pool osg-flock.grid.iu.edu -state -total')
        if exit_status != 0:
            ...
            try:
                ...
                return [
                 BusyClaimed + IdleUnclaimed + IdleClaimed,
...
supported_types = {'moab': MoabAgent, 
   'pbs': PbsAgent, 
   'slurm': SlurmAgent, 
   'condor': CondorAgent}
...
def create(resource_config, db_dir=None, verbosity=0):
    try:
        if resource_config['cluster_type'].lower() == 'moab':
            ...
            return MoabAgent(resource_config, verbosity, db_dir)
        if resource_config['cluster_type'].lower() == 'pbs':
            return PbsAgent(resource_config, verbosity, db_dir)
        if resource_config['cluster_type'].lower() == 'slurm':
            if resource_config['login_server'].lower().startswith('stampede'):
                return SlurmAgent(resource_config, verbosity, db_dir)
