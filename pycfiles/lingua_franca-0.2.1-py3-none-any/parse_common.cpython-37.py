...
# [GCC 8.4.0]
# Embedded file name: /home/ake/projects/python/lingua-franca/lingua_franca/lang/parse_common.py
...
class Normalizer:
    ...
    @property
    def accents(self):
        return self.config.get('accents', {'á':'a', 
         'à':'a',  'ã':'a',  'â':'a',  'é':'e', 
         'è':'e',  'ê':'e',  'ẽ':'e',  'í':'i', 
         'ì':'i',  'î':'i',  'ĩ':'i',  'ò':'o', 
         'ó':'o',  'ô':'o',  'õ':'o',  'ú':'u', 
...
def tokenize(text):
    ...
    """
    return [Token(word, index) for index, word in enumerate(Normalizer.tokenize(text))]
...
def look_for_fractions(split_list):
    ...
    """
    if len(split_list) == 2:
        if is_numeric(split_list[0]):
            if is_numeric(split_list[1]):
                return True
    return False
