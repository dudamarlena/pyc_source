...
# [GCC 8.4.0]
# Embedded file name: /home/ake/projects/python/lingua-franca/lingua_franca/parse.py
# Compiled at: 2020-01-13 02:47:00
# Size of source mod 2**32: 12972 bytes
...
def match_one(query, choices):
    ...
    best = (
     _choices[0], fuzzy_match(query, _choices[0]))
    for c in _choices[1:]:
...
def extract_number(text, short_scale=True, ordinals=False, lang=None):
    ...
    if lang_code == 'it':
        return extractnumber_it(text, short_scale=short_scale, ordinals=ordinals)
    if lang_code == 'fr':
    ...
    if lang_code == 'es':
        return extract_numbers_es(text, short_scale, ordinals)
    if lang_code == 'nl':
        return extractnumber_nl(text, short_scale=short_scale, ordinals=ordinals)
    _log_unsupported_language(lang_code, [
     'en', 'es', 'pt', 'it', 'fr',
     'sv', 'de', 'da', 'nl'])
    return text
