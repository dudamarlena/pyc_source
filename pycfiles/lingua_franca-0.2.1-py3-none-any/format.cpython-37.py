...
# [GCC 8.4.0]
# Embedded file name: /home/ake/projects/python/lingua-franca/lingua_franca/format.py
# Compiled at: 2019-12-13 09:22:48
# Size of source mod 2**32: 23337 bytes
...
NUMBER_TUPLE = namedtuple('number', 'x, xx, x0, x_in_x0, xxx, x00, x_in_x00, xx00, xx_in_xx00, x000, x_in_x000, x0_in_x000, x_in_0x00')
...
class DateTimeFormat:
    ...
    def cache(self, lang):
        if lang not in self.lang_config:
            try:
                with open(self.config_path + '/' + lang + '/date_time.json', 'r') as (lang_config_file):
                    self.lang_config[lang] = json.loads(lang_config_file.read())
            except FileNotFoundError:
                with open(self.config_path + '/en-us/date_time.json', 'r') as (lang_config_file):
                    self.lang_config[lang] = json.loads(lang_config_file.read())

            for x in ('decade_format', 'hundreds_format', 'thousand_format', 'year_format'):
                i = 1
                while self.lang_config[lang][x].get(str(i)):
                    self.lang_config[lang][x][str(i)]['re'] = re.compile(self.lang_config[lang][x][str(i)]['match'])
                    i = i + 1

    def _number_strings(self, number, lang):
        x = self.lang_config[lang]['number'].get(str(number % 10)) or str(number % 10)
        xx = self.lang_config[lang]['number'].get(str(number % 100)) or str(number % 100)
        x_in_x0 = self.lang_config[lang]['number'].get(str(int(number % 100 / 10))) or str(int(number % 100 / 10))
        x0 = self.lang_config[lang]['number'].get(str(int(number % 100 / 10) * 10)) or str(int(number % 100 / 10) * 10)
        xxx = self.lang_config[lang]['number'].get(str(number % 1000)) or str(number % 1000)
        x00 = self.lang_config[lang]['number'].get(str(int(number % 1000 / 100) * 100)) or str(int(number % 1000 / 100) * 100)
        x_in_x00 = self.lang_config[lang]['number'].get(str(int(number % 1000 / 100))) or str(int(number % 1000 / 100))
        xx00 = self.lang_config[lang]['number'].get(str(int(number % 10000 / 100) * 100)) or str(int(number % 10000 / 100) * 100)
        xx_in_xx00 = self.lang_config[lang]['number'].get(str(int(number % 10000 / 100))) or str(int(number % 10000 / 100))
        x000 = self.lang_config[lang]['number'].get(str(int(number % 10000 / 1000) * 1000)) or str(int(number % 10000 / 1000) * 1000)
        x_in_x000 = self.lang_config[lang]['number'].get(str(int(number % 10000 / 1000))) or str(int(number % 10000 / 1000))
        x0_in_x000 = self.lang_config[lang]['number'].get(str(int(number % 10000 / 1000) * 10)) or str(int(number % 10000 / 1000) * 10)
        x_in_0x00 = self.lang_config[lang]['number'].get(str(int(number % 1000 / 100)) or str(int(number % 1000 / 100)))
        return NUMBER_TUPLE(x, xx, x0, x_in_x0, xxx, x00, x_in_x00, xx00, xx_in_xx00, x000, x_in_x000, x0_in_x000, x_in_0x00)
    ...
    def _decade_format(self, number, number_tuple, lang):
        ...
        return s.format(x=(number_tuple.x), xx=(number_tuple.xx), x0=(number_tuple.x0),
          x_in_x0=(number_tuple.x_in_x0),
    ...
    def _number_format_hundreds(self, number, number_tuple, lang, formatted_decade):
        ...
        return s.format(xxx=(number_tuple.xxx), x00=(number_tuple.x00), x_in_x00=(number_tuple.x_in_x00),
    ...
    def _number_format_thousand(self, number, number_tuple, lang, formatted_decade, formatted_hundreds):
        ...
        return s.format(x_in_x00=(number_tuple.x_in_x00), xx00=(number_tuple.xx00),
            ...
            if dt.year == now.year:
                ...
                if tomorrow.date() == dt.date():
                    ...
                    if now.date() == dt.date():
                        ...
                        if yesterday.date() == dt.date():
                            format_str = 'yesterday'
        return self.lang_config[lang]['date_format'][format_str].format(weekday=(self.lang_config[lang]['weekday'][str(dt.weekday())]),
    ...
    def date_time_format(self, dt, lang, now, use_24hour, use_ampm):
        ...
        return self.lang_config[lang]['date_time_format']['date_time'].format(formatted_date=date_str,
    ...
    def year_format(self, dt, lang, bc):
        ...
        formatted_decade = self._decade_format(dt.year, number_tuple, lang)
        formatted_hundreds = self._number_format_hundreds(dt.year, number_tuple, lang, formatted_decade)
        formatted_thousand = self._number_format_thousand(dt.year, number_tuple, lang, formatted_decade, formatted_hundreds)
        s = self._format_string(dt.year, 'year_format', lang)
        return re.sub(' +', ' ', s.format(year=(str(dt.year)),
...
date_time_format = DateTimeFormat(os.path.join(os.path.dirname(__file__), 'res/text'))
...
def pronounce_number(number, lang=None, places=2, short_scale=True, scientific=False, ordinals=False):
    """
    ...
    lang_code = get_primary_lang_code(lang)
    if lang_code == 'en':
        return pronounce_number_en(number, places=places, short_scale=short_scale,
          scientific=scientific,
          ordinals=ordinals)
    if lang_code == 'it':
        return pronounce_number_it(number, places=places, short_scale=short_scale,
          scientific=scientific)
    if lang_code == 'es':
...
def nice_date_time(dt, lang=None, now=None, use_24hour=False, use_ampm=False):
    ...
    full_code = get_full_lang_code(lang)
    date_time_format.cache(full_code)
    return date_time_format.date_time_format(dt, full_code, now, use_24hour, use_ampm)
...
def join_list(items, connector, sep=None, lang=None):
    ...
    if not items:
        ...
        sep += ' '
    return sep.join((str(item) for item in items[:-1])) + ' ' + _translate_word(connector, lang) + ' ' + items[(-1)]
...
def expand_options(parentheses_line: str) -> list:
    ...
    """
    options = expand_parentheses(re.split('([(|)])', parentheses_line))
    return [re.sub('\\s+', ' ', ' '.join(i)).strip() for i in options]