...
# [GCC 8.4.0]
# Embedded file name: /home/ake/projects/python/lingua-franca/lingua_franca/lang/format_da.py
# Compiled at: 2020-01-13 02:46:59
# Size of source mod 2**32: 12325 bytes
...
def pronounce_number_da(num, places=2):
    ...
    def pronounce_triplet_da(num):
        ...
        if num > 99:
            ...
            if hundreds > 0:
                if hundreds == 1:
                    result += 'ethundrede' + EXTRA_SPACE
                else:
                    result += NUM_STRING_DA[hundreds] + 'hundrede' + EXTRA_SPACE
                    num -= hundreds * 100
        if num == 0:
            result += ''
        else:
    ...
    def pronounce_whole_number_da(num, scale_level=0):
        ...
        num = floor(num / 1000)
        scale_level += 1
        return pronounce_whole_number_da(num, scale_level) + result + EXTRA_SPACE
...
def pronounce_ordinal_da(num):
    ordinals = [
     'nulte', 'første', 'anden', 'tredie', 'fjerde', 'femte',
        ...
        if word[-1:] == '.':
            if word[:-1].isdecimal():
                if wordNext.lower() in months:
                    ...
                    if wordPrev.lower() in ('om', 'den', 'fra', 'til', '(fra', '(om',
                                            'til'):
                        word += 'n'
                    else:
                        if wordPrev.lower() not in ('den', ):
                            word += 'r'
                    words[idx] = word
