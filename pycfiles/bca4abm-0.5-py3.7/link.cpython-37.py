...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\bca4abm\processors\link.py
# Compiled at: 2020-02-14 01:16:04
# Size of source mod 2**32: 7807 bytes
import logging, os, pandas as pd, numpy as np, openmatrix as omx
...
def eval_link_spec(link_spec, link_file_names, data_dir, link_file_column_map, link_index_fields, model_settings, trace_tag=None, trace_od=None):
    ...
    for scenario in ('base', 'build'):
        ...
        for suffix, link_file_name in list(link_file_names.items()):
            df = read_csv_file(data_dir=(os.path.join(data_dir, link_data_subdir)), file_name=link_file_name,
        ...
        if link_index_fields:
            ...
            trace_rows = None
        summary, trace_results, trace_assigned_locals = bca.eval_and_sum(link_spec, links_df,
          locals_dict,
          df_alias='links',
        ...
        if trace_results is not None:
            ...
            tracing.write_csv(trace_results, file_name=('%s_results_%s' % (trace_tag, scenario)),
              index_label='index',
              column_labels=[
             'label', 'link'])
...
def link_processor(link_spec, data_dir):
    ...
    for row in link_manifest.itertuples(index=True):
        ...
        row_results = eval_link_spec(link_spec, (row.link_file_name),
          data_dir,
...
def link_daily_processor(link_daily_spec, data_dir, trace_od):
    ...
    if 'link_daily_file_names' in model_settings:
        ...
        if 'link_daily_file_name' in model_settings:
    ...
    results = eval_link_spec(link_daily_spec, link_daily_file_names,
      data_dir,
