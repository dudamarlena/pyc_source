...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\bca4abm\processors\aggregate_trips.py
...
@inject.step()
def aggregate_trips_processor(aggregate_trips_spec, settings, data_dir):
    """
    ...
    for row in aggregate_trips_manifest.itertuples(index=True):
        ...
        locals_dict['build_trips'] = get_omx_matrix(matrix_dir, row.trip_file_name, row.trip_table_name)
        ...
        row_results = bca.scalar_assign_variables(assignment_expressions=aggregate_trips_spec, locals_dict=locals_dict)
