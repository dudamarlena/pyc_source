...
# [GCC 8.4.0]
# Embedded file name: /Users/dpwrussell/Checkout/OME/forms/omero_forms/utils.py
# Compiled at: 2020-01-18 13:24:24
# Size of source mod 2**32: 28247 bytes
...
def add_form_version(conn, master_user_id, form_id, schema, ui_schema, author, timestamp, message, obj_types=[]):
    """
    ...
    json_data = json.dumps({'id':form_id, 
     'schema':schema, 
     'uiSchema':ui_schema, 
     'author':author, 
    ...
    if anno is not None:
        ...
        mapAnn.setValue([
         ...
         [
          'owner', str(author)]] + [['objType', obj_type] for obj_type in obj_types])
        mapAnn.save()
...
def list_forms(conn, master_user_id, group_id=None, obj_type=None):
    ...
    _assignments = None
    if group_id is not None:
        _assignments = _build_assignment_lookup(_get_assignments(conn, master_user_id, group_id))
    else:
        params = omero.sys.ParametersI()
        params.addLong('mid', master_user_id)
        q = '\n        SELECT anno\n        FROM Experimenter user\n        JOIN user.annotationLinks links\n        JOIN links.child anno\n        WHERE anno.class = MapAnnotation\n        AND user.id = :mid\n        '
        if group_id is None:
            ...
            if _assignments is not None and len(_assignments) > 0:
                namespace = 'hms.harvard.edu/omero/forms/schema'
                namespaces = [namespace + '/%s' % assignment for assignment in list(_assignments.keys())]
...
def get_form_version(conn, user_conn, master_user_id, form_id, timestamp=None):
    ...
    for kv in kvs:
        if kv.name == 'id':
            ...
            if kv.name == timestamp:
                _json_data = kv.value
        if timestamp is None and _json_data is None:
            _json_data = kv.value
...
def assign_form(conn, master_user_id, form_id, add_group_ids=[], remove_group_ids=[]):
    ...
    new_add_group_ids = [group_id for group_id in add_group_ids if group_id not in _assignments]
    new_remove_group_ids = [group_id for group_id in remove_group_ids if group_id in _assignments]
    ...
    if len(new_remove_group_ids) > 0:
        ...
        namespaces = ['%s/%d' % (namespace, group_id) for group_id in new_remove_group_ids]
        params = omero.sys.ParametersI()
        params.addLong('mid', master_user_id)
        params.add('nss', omero.rtypes.wrap(namespaces))
        qs = conn.getQueryService()
        ...
        if len(annos) > 0:
            handle = conn.deleteObjects('MapAnnotation', [anno.id.val for anno in annos])
            cb = omero.callbacks.CmdCallbackI(conn.c, handle)
            while not cb.block(500):
                pass
...
def list_form_data_orphans(conn, master_user_id):
    ...
    q = '\n        SELECT anno\n        FROM Experimenter user\n        JOIN user.annotationLinks links\n        JOIN links.child anno\n        WHERE anno.class = MapAnnotation\n        AND user.id = :mid\n        AND anno.ns LIKE :ns\n        '
    rows = qs.projection(q, params, conn.SERVICE_OPTS)
    ore = re.compile('hms.harvard.edu/omero/forms/data/(Project|Dataset|Image|Screen|Plate)([0-9]+)/([A-z0-9]+)')
    obj_types_ids = {}
    obj_form_ids = {}
    for row in rows:
    ...
    for obj, form_ids in obj_form_ids.items():
        for form_id in form_ids:
            yield {'formId':form_id, 
             'objType':obj[0],  'objId':obj[1]}


def add_form_data(conn, master_user_id, form_id, form_timestamp, message, obj_type, obj_id, data, changed_by, changed_at):
    """
    ...
    namespace = 'hms.harvard.edu/omero/forms/data/%s/%s/%s' % (
     ...
     obj_id,
     form_id)
    json_data = json.dumps({'formId':form_id, 
...
def get_form_data(conn, master_user_id, form_id, obj_type, obj_id):
    ...
    specified object
    """
    return next(get_form_data_history(conn, master_user_id, form_id, obj_type, obj_id), None)
