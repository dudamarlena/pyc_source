...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-x86_64/egg/xml2data/xml2data.py
# Compiled at: 2013-03-01 16:08:08
import urllib2, re
...
def urlload(url, template, param=None):
    ...
    if ct is not None:
        m = re.search(';\\s*charset=[\\\'\\"]?(?P<encoding>\\S+?)[\\\'\\"]?\\s*$', ct)
...
class Parser:
    ...
    def _parse(cls, content, xml=None):
        ...
        c = content.lstrip()
        for f in (
         cls._parse_num, cls._parse_str) + (lambda c_: cls._parse_dict(c_, xml),) + ((
    ...
    def _parse_str(cls, content):
        c = content.lstrip()
        q = "'" if c[0] == "'" else '"' if c[0] == '"' else None
        if q is None:
            raise Xml2DataSyntaxError()
        s = 1
        while True:
