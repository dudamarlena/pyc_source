...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\b3\parsers\csgo.py
...
class CsgoParser(Parser):
    ...
    @ger.gameEvent('^//', '^server cvars start', '^server cvars end', '^\\[basechat\\.smx\\] .*', '^\\[META\\] Loaded \\d+ plugins \\(\\d+ already loaded\\)$', '^Log file closed.$', '^\\[META\\] Loaded \\d+ plugin.$')
    ...
    @ger.gameEvent('^"(?P<cvar_name>\\S+)" = "(?P<cvar_value>\\S*)"$', '^server_cvar: "(?P<cvar_name>\\S+)" "(?P<cvar_value>\\S*)"$')
    ...
    @ger.gameEvent('^World triggered "(?P<event_name>\\S*)"(?P<properties>.*)$')
    ...
    def on_player_action(self, name, cid, guid, team, event_name, properties):
        ...
        if event_name in ('Got_The_Bomb', 'Dropped_The_Bomb', 'Planted_The_Bomb', 'Begin_Bomb_Defuse_Without_Kit',
                          ...
                          'round_mvp', 'Rescued_A_Hostage', 'Touched_A_Hostage',
                          'Escaped_As_VIP', 'Became_VIP', 'Killed_A_Hostage'):
    ...
    @ger.gameEvent('^Log file started (?P<properties>.*)$')
