...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\b3\fake.py
...
"""
__version__ = '1.18'
import b3.events, b3.output, b3.parser, b3.parsers.punkbuster, logging, re, StringIO, sys, time, traceback
...
class FakeClient(b3.clients.Client):
    ...
    def kills(self, victim, weapon=1, hit_location=1):
        ...
        if self == victim:
            self.suicides()
            return
        if self.team != b3.TEAM_UNKNOWN and self.team == victim.team:
