...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\tests\core\parsers\test_bfh.py
...
class Test_getServerInfo(BFHTestCase):
    ...
    def test_getServerInfo(self):
        ...
        self.assertDictEqual({'punkBusterVersion': '', 'team2score': None, 'numPlayers': '0', 'maxPlayers': '14', 'targetScore': '0', 'closestPingSite': 'ams', 
           'blazeGameState': 'IN_GAME', 'onlineState': '', 'serverName': '[WASP] Hotwire all maps -- Working kick on kills', 
           'gamemode': 'Hotwire0', 'hasPunkbuster': 'true', 
           'hasPassword': 'false', 'numTeams': '0', 'team1score': None, 'roundTime': '6019', 'blazePlayerCount': '0', 
           'isRanked': 'true', 'roundsPlayed': '0', 'serverUptime': '428710', 'team4score': None, 
           'level': 'MP_Eastside', 'country': 'NL', 'region': 'EU', 'joinQueueEnabled': 'true', 'roundsTotal': '2', 
           'gameIpAndPort': '108.61.98.177:40000', 'team3score': None}, self.parser.game.serverinfo)
        return
...
class Test_punkbuster_events(BFHTestCase):
    ...
    def test_PB_SV_BanList(self):
        ...
        self.assertEquals('Event<EVT_PUNKBUSTER_UNKNOWN>([\'PunkBuster Server: 1   (UnBanned) b59ffffffffffffffffffffffffffc7d {15/15} "Cucurbitaceae" "87.45.14.2:3659" retest" ""\'], None, None)', str(self.pb('PunkBuster Server: 1   (UnBanned) b59ffffffffffffffffffffffffffc7d {15/15} "Cucurbitaceae" "87.45.14.2:3659" retest" ""')))
    ...
    def test_PB_Screenshot_received_message(self):
        ...
        self.assertEqual("Event<EVT_PUNKBUSTER_SCREENSHOT_RECEIVED>({'slot': '19', 'name': 'Jaffar', 'ip': '111.22.33.111', 'pbid': 'da876546546546546546546546547673', 'imgpath': 'C:\\\\games\\\\bf3\\\\173_199_73_213_25200\\\\862147\\\\bf3\\\\pb\\\\svss\\\\pb000709.png', 'port': '3659', 'md5': '4576546546546546546546546543E1E1'}, None, None)", str(result))
    ...
    def test_misc(self):
        self.assertEqual("Event<EVT_PUNKBUSTER_LOST_PLAYER>({'slot': '1', 'ip': 'x.x.x.x', 'port': '3659', 'name': 'joe', 'pbuid': '0837c128293d42aaaaaaaaaaaaaaaaa'}, None, None)", str(self.pb('PunkBuster Server: Lost Connection (slot #1) x.x.x.x:3659 0837c128293d42aaaaaaaaaaaaaaaaa(-) joe')))
...
class Test_bfh_maps(BFHTestCase):
    ...
    def test_getGamemodeSoundingLike(self):
        ...
        def assertSoundsLike(expected_id, map_id, input_gamemode):
            self.assertEqual(expected_id, self.parser.getGamemodeSoundingLike(map_id, input_gamemode), '%s:%r' % (map_id, input_gamemode))

        def assertSoundsLikeList(expected_list, map_id, input_gamemode):
            self.assertListEqual(expected_list, self.parser.getGamemodeSoundingLike(map_id, input_gamemode), '%s:%r' % (map_id, input_gamemode))
...
class Test_patch_b3_admin_plugin(BFHTestCase):
    ...
    def setUp(self):
        ...
        with logging_disabled():
            ...
            adminPlugin_conf.loadFromString(dedent('\n                [commands]\n                map: 20\n            '))
    ...
    def test_map_known_on_correct_gamemode(self):
        ...
        self.assertListEqual([call('mp_downtown', gamemode_id='TurfwarLarge0', number_of_rounds=2)], changeMap_mock.mock_calls)
        self.assertListEqual([], self.joe.message_history)

    def test_map_InvalidGameModeOnMap(self):
        self.parser.game.gameType = 'Hotwire0'
        when(self.parser).changeMap('mp_bank', gamemode_id='Hotwire0', number_of_rounds=2).thenRaise(CommandFailedError(['InvalidGameModeOnMap']))
        self.joe.says('!map bank')
        self.assertListEqual([
         'Bank Job cannot be played with gamemode Hotwire',
         'supported gamemodes are : Conquest Large, Conquest Small, Heist, Blood Money, Crosshair, Rescue, Team Deathmatch'], self.joe.message_history)

    def test_map_InvalidRoundsPerMap(self):
        when(self.parser).changeMap('mp_bank', gamemode_id='TurfWarSmall0', number_of_rounds=2).thenRaise(CommandFailedError(['InvalidRoundsPerMap']))
        self.joe.says('!map bank')
...
class Test_getClient(BFHTestCase):
    ...
    def test_team_red_player(self):
        when(self.parser).write(('admin.listPlayers', 'player', 'Foobar')).thenReturn(self.build_listPlayer_response('Foobar', '1', '0'))
        player = self.parser.getClient('Foobar')
        self.assertEqual(b3.TEAM_RED, player.team)

    def test_team_blue_player(self):
        when(self.parser).write(('admin.listPlayers', 'player', 'Foobar')).thenReturn(self.build_listPlayer_response('Foobar', '2', '0'))
        player = self.parser.getClient('Foobar')
        self.assertEqual(b3.TEAM_BLUE, player.team)

    def test_team_red_commander(self):
        when(self.parser).write(('admin.listPlayers', 'player', 'Foobar')).thenReturn(self.build_listPlayer_response('Foobar', '1', '2'))
        player = self.parser.getClient('Foobar')
        self.assertEqual(b3.TEAM_RED, player.team)

    def test_team_blue_commander(self):
        when(self.parser).write(('admin.listPlayers', 'player', 'Foobar')).thenReturn(self.build_listPlayer_response('Foobar', '2', '2'))
        player = self.parser.getClient('Foobar')
        self.assertEqual(b3.TEAM_BLUE, player.team)

    def test_team_spectator(self):
        when(self.parser).write(('admin.listPlayers', 'player', 'Foobar')).thenReturn(self.build_listPlayer_response('Foobar', '0', '1'))
        player = self.parser.getClient('Foobar')
