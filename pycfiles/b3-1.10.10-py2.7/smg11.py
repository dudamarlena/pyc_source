...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\b3\parsers\smg11.py
...
class Smg11Parser(AbstractParser):
    ...
    _lineFormats = (
     re.compile('^\\d+:\\s+(?P<data>client:(?P<cid>\\d+)\\s+health:(?P<health>\\d+)\\s+damage:(?P<damage>[.\\d]+)\\s+where:(?P<hitloc>[^\\s]+)\\s+from:(?P<aweap>[^\\s]+)\\s+by:(?P<acid>\\d+))$', re.IGNORECASE),
            ...
            if client:
                ...
                if 'guid' in bclient:
                    ...
                    if 'skill' in bclient:
                        ...
                        self.clients.newClient(cid, name=bclient['name'], ip='0.0.0.0', state=b3.STATE_ALIVE, guid=guid, data={'guid': guid}, bot=True, money=20)
                    else:
                ...
                if 'ip' in bclient:
                    self.clients.newClient(cid, name=bclient['name'], ip=bclient['ip'], state=b3.STATE_ALIVE, guid=guid, data={'guid': guid}, bot=False, money=20)
                else:
    ...
    def OnItem(self, action, data, match=None):
        ...
        if client:
            if 'pickup_money' in item:
                re_pickup_money = re.compile('^pickup_money \\((?P<amount>\\d+)\\) picked up \\(\\$(?P<totalmoney>\\d+)\\)$')
            ...
            if 'bought' in item:
                re_bought = re.compile('^(?P<item>.+) bought \\(\\$(?P<cost>\\d+)/\\$(?P<totalmoney>\\d+)\\)$')
                ...
                if m is not None:
                    ...
                    if cost is not None and totalmoney is not None:
                        ...
                        self.verbose('%s has now $%s' % (client.name, client.money))
                        return self.getEvent('EVT_CLIENT_SPEND_MONEY', {'item': what, 'cost': cost, 
