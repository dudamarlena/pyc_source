...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\b3\tools\documentationBuilder.py
...
__version__ = '1.2.9'
import datetime, os, re, StringIO, time
...
class DocBuilder:
    _supportedExportType = [
     'xml', 'html', 'oldhtml', 'htmltable', 'json']
    _console = None
    _adminPlugin = None
    _outputType = 'html'
    ...
    def __init__(self, console):
        ...
        if self._console.config.has_section('autodoc'):
            if self._console.config.has_option('autodoc', 'destination'):
                ...
                elif dest.startswith('ftp://') or dest.startswith('file://'):
    ...
    def save(self):
        ...
        if self._outputType not in self._supportedExportType:
            ...
            if self._outputType == 'xml':
                ...
                self._write(doc_template.safe_substitute(commandsTable=self.getHtmlTable(), dateUpdated=time.asctime(), server=self._console._publicIp + ':' + str(self._console._port)))
    ...
    def get_json(self, indent=None):
        import json
        output = {'commands': self._getCommandsDict(), 
           'updated': datetime.datetime.now().isoformat(), 
           'level_info': {}, 'server_info': {'ip': self._console._publicIp, 
                           'port': self._console._port}}
        for g in self._console.storage.getGroups():
            output['level_info'].update({g.level: {'name': g.name, 
                         'keyword': g.keyword, 
                         'level': g.level}})
    ...
    def _getCommandsDict(self):
        ...
        def commands_compare(x, y):
            if x['plugin'] < y['plugin']:
                ...
                if x['plugin'] > y['plugin']:
                    return 1
                if int(x['minlevel']) < int(y['minlevel']):
                    return -1
                if int(x['minlevel']) > int(y['minlevel']):
                    return 1
                if x['name'] < y['name']:
