...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\b3\tools\debug\runsnakerun\squaremap.py
# Compiled at: 2016-03-08 18:42:10
import wx, sys, os, logging, wx.lib.newevent
...
class SquareMap(wx.Panel):
    ...
    def __init__(self, parent=None, id=-1, pos=wx.DefaultPosition, size=wx.DefaultSize, style=wx.TAB_TRAVERSAL | wx.NO_BORDER | wx.FULL_REPAINT_ON_RESIZE, name='SquareMap', model=None, adapter=None, labels=True, highlight=True, padding=2):
    ...
    def OnKeyUp(self, event):
        ...
        if event.KeyCode == wx.WXK_DOWN:
            ...
            wx.PostEvent(self, SquareActivationEvent(node=self.selectedNode, map=self))
    ...
    def DrawBox(self, dc, node, x, y, w, h, hot_map, depth=0):
        ...
        log.debug('Draw: %s to (%s,%s,%s,%s) depth %s', node, x, y, w, h, depth)
