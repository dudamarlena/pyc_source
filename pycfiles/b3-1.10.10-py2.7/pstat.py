...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\b3\tools\debug\statlib\pstat.py
...
"""
    ...
    for addon in args:
        ...
        if len(addon) < len(source):
            if len(source) % len(addon) == 0:
                repeats = len(source) / len(addon)
                origadd = copy.deepcopy(addon)
                for i in range(repeats - 1):
                    addon = addon + origadd

            else:
                repeats = len(source) / len(addon) + 1
                origadd = copy.deepcopy(addon)
                for i in range(repeats - 1):
                    addon = addon + origadd
            ...
            if len(addon) % len(source) == 0:
                repeats = len(addon) / len(source)
                origsour = copy.deepcopy(source)
                for i in range(repeats - 1):
                    source = source + origsour

            else:
                repeats = len(addon) / len(source) + 1
                origsour = copy.deepcopy(source)
                for i in range(repeats - 1):
                    source = source + origsour
...
"""
    ...
    elif type(addon[0]) not in [ListType, TupleType]:
        for i in range(minlen):
            list[i] = source[i] + [addon[i]]
...
"""
    global index
    column = 0
    if type(cnums) in [ListType, TupleType]:
        index = cnums[0]
...
try:
    ...
    """
        ...
        if len(columnlist) == 1 and len(valuelist) > 1:
            ...
            if len(valuelist) == 1 and len(columnlist) > 1:
