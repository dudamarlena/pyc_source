...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\b3\clients.py
# Compiled at: 2016-03-08 18:42:09
import b3, b3.events, functions, re, string, sys, threading, time, traceback
...
class Client(object):
    ...
    def _set_team(self, team):
        if self._team != team:
            ...
            if self.console:
                ...
                self.console.queueEvent(self.console.getEvent('EVT_CLIENT_TEAM_CHANGE2', {'previous': previous_team, 'new': self.team}, self))
    ...
    def _set_name(self, name):
        ...
        if self._name == newName:
            ...
            return
        if self.cid == '-1' or self.cid == 'Server':
            if self.console:
    ...
    def warn(self, duration, warning, keyword=None, admin=None, data=''):
        ...
        if self.id:
            ...
            if self.console:
                self.console.queueEvent(self.console.getEvent('EVT_CLIENT_WARN', data={'reason': warn.reason, 
                   'duration': warn.duration, 
                   'data': warn.data, 
                   'admin': admin, 
                   'timeExpire': warn.timeExpire}, client=self))
            return warn
    ...
    def notice(self, notice, spare, admin=None):
        ...
        if self.id:
            ...
            if self.console:
                self.console.queueEvent(self.console.getEvent('EVT_CLIENT_NOTICE', data={'notice': notice, 
                   'admin': admin, 
                   'timeAdd': notice_object.timeAdd}, client=self))
...
class Clients(dict):
    ...
    def getClientsByName(self, name):
        ...
        """
        clist = []
        needle = re.sub('\\s', '', name.lower())
        for cid, c in self.items():
            cleanname = re.sub('\\s', '', c.name.lower())
        ...
        try:
            ...
            for cid, c in self.items():
                if c.guid and c.guid == guid:
                    ...
                    return c
                if functions.fuzzyGuidMatch(c.guid, guid):
                    return c
