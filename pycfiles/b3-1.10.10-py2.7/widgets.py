...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\b3\gui\widgets.py
# Compiled at: 2016-03-08 18:42:09
import b3, feedparser, logging, os, re
...
GEOMETRY = {'nt': {'MAIN_TABLE_HEIGHT': 260, 
...
class MainTable(QTableWidget):
    ...
    def process_delete(self, process):
        ...
        msgbox.layout().addItem(QSpacerItem(300, 0, QSizePolicy.Minimum, QSizePolicy.Expanding), msgbox.layout().rowCount(), 0, 1, msgbox.layout().columnCount())
        msgbox.exec_()
    ...
    def process_log(self, process):
        ...
        try:
            ...
            msgbox.setStandardButtons(QMessageBox.Ok)
            msgbox.layout().addItem(QSpacerItem(400, 0, QSizePolicy.Minimum, QSizePolicy.Expanding), msgbox.layout().rowCount(), 0, 1, msgbox.layout().columnCount())
            msgbox.exec_()
        else:
...
class MainWindow(QMainWindow):
    ...
    system_tray_balloon = {'win32': "B3 is still running! If you want to quit B3, right click this icon and select 'Quit'.", 
       'darwin': 'B3 is still running! If you want to quit B3, close the application using the Dock launcher.'}
    ...
    def closeEvent(self, event):
        ...
        elif B3App.Instance().shutdown_requested:
            B3App.Instance().shutdown()
        else:
            event.ignore()
    ...
    def minimize_in_system_tray(self):
        ...
        if b3.getPlatform() != 'linux':
            ...
            if not self.system_tray_minimized:
                ...
                self.system_tray.showMessage('B3 is still running!', self.system_tray_balloon[b3.getPlatform()], QSystemTrayIcon.Information, 20000)
        ...
        if path:
            try:
                ...
                if analysis:
                    ...
                    msgbox.setStandardButtons(QMessageBox.Ok)
                    msgbox.exec_()
                elif not B3.exists(config.fileName):
    ...
    def open_extplugins_directory(self):
        ...
        if not os.path.isdir(extplugins_dir):
            try:
                ...
                msgbox.setDetailedText('B3 could not create missing 3rd party plugins directory (%s). Please make sure B3 has writing permissions on "%s"' % (
                 extplugins_dir,
