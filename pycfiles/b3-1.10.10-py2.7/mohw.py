...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\b3\parsers\mohw.py
# Compiled at: 2016-03-08 18:42:10
import b3, b3.events, traceback, sys
...
GAME_MODES_NAMES = {'CombatMission': 'Combat Mission', 
   'Sport': 'Home Run', 
...
MAP_NAME_BY_ID = {'MP_03': 'Somalia Stronghold', 
...
GAME_MODES_BY_MAP_ID = {'MP_03': (
           'CombatMission', 'Sport', 'SectorControl', 'TeamDeathMatch', 'BombSquad'), 
   'MP_05': (
           'CombatMission', 'SectorControl', 'TeamDeathMatch', 'BombSquad'), 
   'MP_10': (
           'CombatMission', 'Sport', 'SectorControl', 'TeamDeathMatch', 'BombSquad'), 
   'MP_12': (
           'CombatMission', 'Sport', 'SectorControl', 'TeamDeathMatch'), 
   'MP_13': (
           'CombatMission', 'SectorControl', 'TeamDeathMatch', 'BombSquad'), 
   'MP_16': (
           'CombatMission', 'Sport', 'SectorControl', 'TeamDeathMatch', 'BombSquad'), 
...
class MohwParser(AbstractParser):
    gameName = 'mohw'
    _gamePort = None
    _commands = {'message': ('admin.say', '%(message)s', 'team', '%(teamId)s'), 
    ...
    _gameServerVars = ('3pCam', 'autoBalance', 'bannerUrl', 'buddyOutline', 'bulletDamage',
                       'friendlyFire', 'gameModeCounter', 'gamePassword', 'hardcoreBulletDamageOverride',
                       'hudBuddyInfo', 'hudClassAbility', 'hudCrosshair', 'hudEnemyTag',
                       'hudExplosiveIcons', 'hudGameMode', 'hudHealthAmmo', 'hudMinimap',
                       'hudObiturary', 'hudPointsTracker', 'hudUnlocks', 'idleBanRounds',
                       'idleTimeout', 'killCam', 'maxPlayers', 'playerManDownTime',
                       'playerRespawnTime', 'playlist', 'ranked', 'regenerateHealth',
                       'roundRestartPlayerCount', 'roundStartPlayerCount', 'roundsPerMap',
                       'serverDescription', 'serverName', 'soldierHealth', 'teamKillCountForKick',
                       'teamKillKickForBan', 'teamKillValueDecreasePerSecond', 'teamKillValueForKick',
                       'teamKillValueIncrease')
    _gamemode_aliases = {'cm': 'Combat Mission', 
       'hr': 'Home Run', 
       'sc': 'Sector Control', 
    ...
    def checkVersion(self):
        ...
        if int(version[1]) < MOHW_REQUIRED_VERSION:
            raise Exception('the MOHW parser can only work with Medal of Honor Warfighter server version %s and above. You are trying to connect to %s v%s' % (
             MOHW_REQUIRED_VERSION, version[0], version[1]))
...
class NewMapListBlock(b3.parsers.frostbite2.util.MapListBlock):
    ...
    def append(self, data):
        ...
        if len(data) != 3 + num_maps * num_words:
            raise MapListBlockError('invalid data: the total number of elements is not coherent with the number of maps declared. %s != (2 + %s * %s)' % (
        ...
        for i in range(num_maps):
            ...
            try:
                ...
                raise MapListBlockError('invalid data: %sth element should be a integer, got %r' % (
                 base_index + 2, data[(base_index + 2)]))
