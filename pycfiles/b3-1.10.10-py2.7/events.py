...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\b3\events.py
        ...
        if self.console.log.isEnabledFor(VERBOSE):
            for plugin_name, plugin_timers in self._handling_timers.iteritems():
                for event_name, event_timers in plugin_timers.iteritems():
                    ...
                    if len(event_timers):
                        self.console.verbose('%s %s : (ms) min(%0.1f), max(%0.1f), mean(%0.1f), stddev(%0.1f)', plugin_name, event_name, min(event_timers), max(event_timers), mean, stdv)

        if self.console.log.isEnabledFor(DEBUG):
            mean, stdv = meanstdv(self._queue_wait)
            if len(self._queue_wait):
                self.console.debug('Events waiting in queue stats : (ms) min(%0.1f), max(%0.1f), mean(%0.1f), stddev(%0.1f)', min(self._queue_wait), max(self._queue_wait), mean, stdv)
