...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\b3\config.py
# Compiled at: 2016-03-08 18:42:09
__author__ = 'ThorN, Courgette, Fenix'
__version__ = '1.7.9'
import os, re, time, b3, b3.functions, b3.exceptions, b3.storage, ConfigParser
try:
...
class MainConfig(B3ConfigParserMixin):
    ...
    def _init_plugins_from_xml(self):
        ...
        for p in self._config_parser.get('plugins/plugin'):
            x = p.get('disabled')
            self._plugins.append({'name': p.get('name'), 
               'conf': p.get('config'), 
               'path': p.get('path'), 
    ...
    def _init_plugins_from_cfg(self):
        ...
        if self._config_parser.has_section('plugins'):
            for name in self._config_parser.options('plugins'):
                self._plugins.append({'name': name, 
    ...
    def analyze(self):
        ...
        if self.has_option('b3', 'database'):
            ...
            if not dsndict:
                ...
                analysis.append('invalid storage protocol specified in b3::database (%s) : valid protocols are : %s' % (
