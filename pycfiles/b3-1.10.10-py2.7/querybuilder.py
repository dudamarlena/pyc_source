...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\b3\querybuilder.py
...
class QueryBuilder(object):
    ...
    def WhereClause(self, fields=None, values=None, concat=' and '):
        ...
        elif fields is not None and not isinstance(fields, tuple) and values is not None and not isinstance(values, tuple):
    ...
    def SelectQuery(self, fields, table, where='', orderby='', limit=0, offset='', groupby='', having='', **keywords):
        ...
        """
        sql = [
         'SELECT %s FROM %s' % (self.fieldStr(fields), table)]
        if where:
