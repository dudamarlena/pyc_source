...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\b3\functions.py
# Compiled at: 2016-03-08 18:42:09
__author__ = 'ThorN, xlr8or, courgette'
__version__ = '1.23'
import collections, os, re, sys, shutil, string, urllib2, zipfile
...
def splitDSN(url):
    ...
    """
    m = re.match('^(?:(?P<protocol>[a-z]+)://)?(?:(?P<user>[^:]+)(?::(?P<password>[^@]*?))?@)?(?P<host>[^/:]+)?(?::(?P<port>\\d+))?(?P<path>.*)', url)
    if not m:
        return
    else:
        g = m.groupdict()
...
def vars2printf(inputstr):
    if inputstr is not None and inputstr != '':
        return re.sub('\\$([a-zA-Z_]+)', '%(\\1)s', inputstr)
    else:
        return ''
        return
...
def sanitizeMe(s):
    ...
    """
    if s:
        return re.sub('[\\x00-\\x1F]|[\\x7F-\\xff]', '?', s)
    return ''
...
def split_extension(filename):
    ...
    """
    r = re.compile('^(?P<filename>.+)\\.(?P<extension>.*)$')
    m = r.match(filename)
...
def getBytes(size):
    ...
    """
    size = str(size).upper()
    r = re.compile('^(?P<size>\\d+)\\s*(?P<mult>KB|MB|GB|TB|K|M|G|T?)$')
    m = r.match(size)
    if not m:
        raise TypeError('invalid input given: %s' % size)
    multipliers = {'K': 1024, 
       'KB': 1024, 'M': 1048576, 
       'MB': 1048576, 'G': 1073741824, 
       'GB': 1073741824, 'T': 1099511627776, 
       'TB': 1099511627776}
    try:
