...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\b3\plugin.py
# Compiled at: 2016-03-08 18:42:10
__author__ = 'ThorN, Courgette'
__version__ = '1.13'
import re, b3.clients, b3.config, b3.events, b3.functions
...
class Plugin(object):
    ...
    def __init__(self, console, config=None):
        ...
        if isinstance(config, b3.config.XmlConfigParser) or isinstance(config, b3.config.CfgConfigParser):
            ...
            try:
                ...
                self.critical('TIP: make use of a text editor with syntax highlighting to modify your config files: it makes easy to spot errors')
                raise e
    ...
    def getSetting(self, section, option, value_type=b3.STRING, default=None, validate=None):
        ...
        handlers = {b3.STRING: _get_string, 
        ...
        try:
            ...
            try:
                ...
                val = default
                self.warning('could not convert %s::%s : invalid value type specified (%s) : expecting one of (%s), using default : %s', section, option, value_type, (', ').join(map(str, handlers.keys())), default)
            else:
                try:
                    val = func(val)
    ...
    def getMessage(self, msg, *args):
        ...
        if len(args) == 1 and type(args[0]) is dict:
            try:
                ...
                self.error('failed to format message %r (%r) with parameters %r: missing value for %s', msg, _msg, args, err)
                raise
    ...
    def registerEventHook(self, event_id, hook):
        ...
        if event_id not in self.eventmap:
