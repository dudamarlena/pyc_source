...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\b3\tools\debug\statlib\stats.py
...
def lmedian(inlist, numbins=1000):
    ...
    """
    hist, smallest, binsize, extras = histogram(inlist, numbins, [min(inlist), max(inlist)])
    cumhist = cumsum(hist)
    ...
    freq = float(hist[cfbin])
    median = LRL + (len(inlist) / 2.0 - cfbelow) / float(freq) * binsize
    return median
...
def lhistogram(inlist, numbins=10, defaultreallimits=None, printextras=0):
    ...
    return (
     bins, lowerreallimit, binsize, extrapoints)
...
def llincc(x, y):
    ...
    Returns: Lin's CC
    """
    covar = lcov(x, y) * (len(x) - 1) / float(len(x))
    xvar = lvar(x) * (len(x) - 1) / float(len(x))
    yvar = lvar(y) * (len(y) - 1) / float(len(y))
...
def lttest_1samp(a, popmean, printit=0, name='Sample', writemode='a'):
    ...
    if printit != 0:
        ...
        outputpairedstats(printit, writemode, 'Population', '--', popmean, 0, 0, 0, name, n, x, v, min(a), max(a), statname, t, prob)
    return (
     t, prob)
...
def lttest_ind(a, b, printit=0, name1='Samp1', name2='Samp2', writemode='a'):
    ...
    if printit != 0:
        ...
        outputpairedstats(printit, writemode, name1, n1, x1, v1, min(a), max(a), name2, n2, x2, v2, min(b), max(b), statname, t, prob)
    return (
     t, prob)
...
def lttest_rel(a, b, printit=0, name1='Sample1', name2='Sample2', writemode='a'):
    ...
    if printit != 0:
        ...
        outputpairedstats(printit, writemode, name1, n, x1, v1, min(a), max(a), name2, n, x2, v2, min(b), max(b), statname, t, prob)
    return (
     t, prob)
...
def lksprob(alam):
    ...
    for j in range(1, 201):
        ...
        sum = sum + term
        if math.fabs(term) <= 0.001 * termbf or math.fabs(term) < 1e-08 * sum:
            return sum
        fac = -fac
...
def ageometricmean(inarray, dimension=None, keepdims=0):
    ...
    if dimension == None:
        ...
        if type(dimension) in [IntType, FloatType]:
...
def aharmonicmean(inarray, dimension=None, keepdims=0):
    ...
    if dimension == None:
        ...
        tinarray = N.transpose(inarray, nondims + dims)
        idx = [0] * len(nondims)
        if idx == []:
...
def amedian(inarray, numbins=1000):
    ...
    cumhist = N.cumsum(hist)
    otherbins = N.greater_equal(cumhist, len(inarray) / 2.0)
    otherbins = list(otherbins)
    cfbin = otherbins.index(1)
    LRL = smallest + binsize * cfbin
    cfbelow = N.add.reduce(hist[0:cfbin])
    freq = hist[cfbin]
    median = LRL + (len(inarray) / 2.0 - cfbelow) / float(freq) * binsize
    return median
...
def amedianscore(inarray, dimension=None):
    ...
    if inarray.shape[dimension] % 2 == 0:
        indx = inarray.shape[dimension] / 2
...
def amode(a, dimension=None):
    ...
    if dimension == None:
        a = N.ravel(a)
        dimension = 0
    scores = pstat.aunique(N.ravel(a))
...
def ahistogram(inarray, numbins=10, defaultlimits=None, printextras=1):
    ...
    if defaultlimits != None:
        ...
        estbinwidth = float(Max - Min) / float(numbins) + 1e-06
...
def acov(x, y, dimension=None, keepdims=0):
    ...
    if dimension == None:
        ...
        y = N.ravel(y)
        dimension = 0
    xmn = amean(x, dimension, 1)
    xdeviations = x - xmn
    ymn = amean(y, dimension, 1)
    ydeviations = y - ymn
    if type(dimension) == ListType:
        n = 1
        for d in dimension:
...
def alincc(x, y):
    ...
    x = N.ravel(x)
    y = N.ravel(y)
    covar = acov(x, y) * (len(x) - 1) / float(len(x))
    xvar = avar(x) * (len(x) - 1) / float(len(x))
    yvar = avar(y) * (len(y) - 1) / float(len(y))
    lincc = 2 * covar / (xvar + yvar + (amean(x) - amean(y)) ** 2)
    return lincc
...
def attest_1samp(a, popmean, printit=0, name='Sample', writemode='a'):
    ...
    if printit != 0:
        statname = 'Single-sample T-test.'
        outputpairedstats(printit, writemode, 'Population', '--', popmean, 0, 0, 0, name, n, x, v, N.minimum.reduce(N.ravel(a)), N.maximum.reduce(N.ravel(a)), statname, t, prob)
    return (
     t, prob)
...
def attest_ind(a, b, dimension=None, printit=0, name1='Samp1', name2='Samp2', writemode='a'):
    ...
    svar = N.where(zerodivproblem, 1, svar)
    t = (x1 - x2) / N.sqrt(svar * (1.0 / n1 + 1.0 / n2))
    t = N.where(zerodivproblem, 1.0, t)
    ...
    if printit != 0:
        ...
        if type(probs) == N.ndarray:
            probs = probs[0]
        statname = 'Independent samples T-test.'
        outputpairedstats(printit, writemode, name1, n1, x1, v1, N.minimum.reduce(N.ravel(a)), N.maximum.reduce(N.ravel(a)), name2, n2, x2, v2, N.minimum.reduce(N.ravel(b)), N.maximum.reduce(N.ravel(b)), statname, t, probs)
        return
    else:
        return (
         t, probs)
...
def attest_rel(a, b, dimension=None, printit=0, name1='Samp1', name2='Samp2', writemode='a'):
    ...
    t = N.add.reduce(d, dimension) / denom
    t = N.where(zerodivproblem, 1.0, t)
    probs = abetai(0.5 * df, 0.5, float(df) / (df + t * t))
    if type(t) == N.ndarray:
        probs = N.reshape(probs, t.shape)
    ...
    if printit != 0:
        statname = 'Related samples T-test.'
        outputpairedstats(printit, writemode, name1, n, x1, v1, N.minimum.reduce(N.ravel(a)), N.maximum.reduce(N.ravel(a)), name2, n, x2, v2, N.minimum.reduce(N.ravel(b)), N.maximum.reduce(N.ravel(b)), statname, t, probs)
        return
    else:
        return (
         t, probs)
...
def awilcoxont(x, y):
    ...
    d = x - y
    d = N.compress(N.not_equal(d, 0), d)
    count = len(d)
    absd = abs(d)
...
def achisqprob(chisq, df):
    ...
    if df < 1:
        ...
        probs = N.where(N.less_equal(chisq, 0), 1.0, probs)
        a = 0.5 * chisq
        if df > 1:
            y = ex(-a)
        if df % 2 == 0:
        ...
        if df > 2:
            ...
            while asum(mask) != totalelements:
                ...
                z = z + 1.0
                newmask = N.greater(z, chisq)
                a_notbig_frozen = N.where(newmask * N.equal(mask, 0) * (1 - a_big), c * y + s2, a_notbig_frozen)
                mask = N.clip(newmask + mask, 0, 1)

            probs = N.where(N.equal(probs, 1), 1, N.where(N.greater(a, BIG), a_big_frozen, a_notbig_frozen))
            return probs
        return s
...
def azprob(z):
    ...
    def wfunc(w):
        x = ((((((((0.000124818987 * w - 0.001075204047) * w + 0.005198775019) * w - 0.019198292004) * w + 0.059054035642) * w - 0.151968751364) * w + 0.319152932694) * w - 0.5319230073) * w + 0.797884560593) * N.sqrt(w) * 2.0
        return x

    Z_MAX = 6.0
    x = N.zeros(z.shape, N.float_)
    y = 0.5 * N.fabs(z)
    x = N.where(N.less(y, 1.0), wfunc(y * y), yfunc(y - 2.0))
    x = N.where(N.greater(y, Z_MAX * 0.5), 1.0, x)
...
def aksprob(alam):
   ...
   """
    ...
    for j in range(1, 201):
        ...
        sum = sum + term
        newmask = N.where(N.less_equal(abs(term), 0.001 * termbf) + N.less(abs(term), 1e-08 * sum), 1, 0)
    ...
    if arrayflag:
        return N.where(N.equal(frozen, -1), 1.0, frozen)
    else:
        return N.where(N.equal(frozen, -1), 1.0, frozen)[0]
...
def abetacf(a, b, x, verbose=1):
    ...
    arrayflag = 1
    if type(x) == N.ndarray:
        frozen = N.ones(x.shape, N.float_) * -1
    else:
        arrayflag = 0
        frozen = N.array([-1])
        x = N.array([x])
...
def abetai(a, b, x, verbose=1):
    ...
    bt = N.exp(exponents)
    if type(x) == N.ndarray:
        ans = N.where(N.less(x, (a + 1) / (a + b + 2.0)), bt * abetacf(a, b, x, verbose) / float(a), 1.0 - bt * abetacf(b, a, 1.0 - x, verbose) / float(b))
    elif x < (a + 1) / (a + b + 2.0):
...
def aglm(data, para):
    ...
    b = N.dot(N.dot(LA.inv(N.dot(N.transpose(x), x)), N.transpose(x)), data)
    diffs = data - N.dot(x, b)
    s_sq = 1.0 / (n - len(p)) * N.dot(N.transpose(diffs), diffs)
    if len(p) == 2:
        c = N.array([1, -1])
        df = n - 2
        fact = asum(1.0 / asum(x, 0))
...
def outputfstats(Enum, Eden, dfnum, dfden, f, prob):
    ...
    title = [
     [
      'EF/ER', 'DF', 'Mean Square', 'F-value', 'prob', '']]
...
def afindwithin(data):
    ...
    for col in range(1, numfact + 1):
        ...
        if len(pstat.unique(pstat.colex(rows, 0))) < len(rows):
            withinvec[col - 1] = 1
...
try:
    import numpy as N, numpy.linalg as LA, operator
    geometricmean = Dispatch((lgeometricmean, (ListType, TupleType)), (
