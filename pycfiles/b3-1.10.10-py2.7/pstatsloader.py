...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\b3\tools\debug\runsnakerun\pstatsloader.py
...
class PStatsLoader(object):
    ...
    def find_root(self, rows):
        ...
        if len(roots) > 1:
            root = PStatGroup(directory='*', filename='*', name=_('<profiling run>'), children=roots)
            root.finalize()
...
class PStatRow(BaseStat):
    ...
    def __init__(self, key, raw):
        ...
        self.calls, self.recursive, self.local, self.localPer, self.cummulative, self.cummulativePer, self.directory, self.filename, self.name, self.lineno = (
         nc,
         cc,
         tt,
         tt / (cc or 1e-14),
