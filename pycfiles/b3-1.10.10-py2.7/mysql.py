...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\b3\storage\mysql.py
...
class MysqlStorage(DatabaseStorage):
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    mysqldriver = None
                    raise ImportError("missing MySQL connector driver. You need to install one of the following MySQL connectors: 'pymysql', 'python-mysql.connector', 'MySQL-python': look for 'dependencies' in B3 documentation.")
    ...
    def connect(self):
        ...
        if time() - self._lastConnectAttempt < self._reconnectDelay:
            self.db = None
            self.console.bot('New MySQL database connection requested but last connection attempt failed less than %s seconds ago: exiting...' % self._reconnectDelay)
        else:
            self.shutdown()
            self.console.bot('Connecting to MySQL database: %(protocol)s://%(user)s:******@%(host)s:%(port)s%(path)s...', self.dsnDict)
            try:
                self.db = self.__driver.connect(host=self.dsnDict['host'], port=self.dsnDict['port'], user=self.dsnDict['user'], passwd=self.dsnDict['password'], db=self.dsnDict['path'][1:], charset='utf8')
                ...
                if not self.getTables():
                    try:
                        ...
                        self.shutdown()
                        self.console.critical('Missing MySQL database tables. You need to create the necessary tables for B3 to work. You can do so by importing the following SQL script into your database: %s. An attempt of creating tables automatically just failed: %s' % (
