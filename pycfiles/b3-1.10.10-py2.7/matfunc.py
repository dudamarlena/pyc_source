...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\b3\tools\debug\statlib\matfunc.py
...
class Matrix(Table):
    ...
    def qr(self, ROnly=0):
        ...
        self.qr = lambda r=0, c=`self`: not r and c == `self` and (Q, R) or Matrix.qr(self, r)
...
if __name__ == '__main__':
    ...
    points = xvec, yvec = funToVec(lambda x: math.sin(x) + 2 * math.cos(0.7 * x + 0.1), low=0, high=3, EqualSpacing=1)
    ...
    points = xvec, yvec = funToVec(lambda x: math.sin(x) + 2 * math.cos(0.7 * x + 0.1), low=0, high=3)
    ...
    print 'eigs:'
    print A.eigs()
    print 'Should be:', Vec([11.6158, 5.0, -3.6158])
    print 'det(A)'
    print A.det()
    c = Mat([[1, 2, 30], [4, 5, 10], [10, 80, 9]])
    print 'C:\n', c
    print c.eigs()
    print 'Should be:', Vec([-8.9554, 43.2497, -19.2943])
    A = Mat([[1, 2, 3, 4], [4, 5, 6, 7], [2, 1, 5, 0], [4, 2, 1, 0]])
    print 'A:\n', A
    print A.eigs()
