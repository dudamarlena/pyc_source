...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\tests\core\parsers\test_iourt42.py
# Compiled at: 2016-03-08 18:42:10
import b3, logging, unittest2 as unittest
...
class Test_log_lines_parsing(Iourt42TestCase):
    ...
    def test_Radio(self):
        self.joe.connects('0')
        self.assertEvent('Radio: 0 - 7 - 2 - "New Alley" - "I\'m going for the flag"', event_type='EVT_CLIENT_RADIO', event_client=self.joe, event_data={'msg_group': '7', 'msg_id': '2', 'location': 'New Alley', 'text': "I'm going for the flag"})

    def test_botStatusAfterInitRound(self):
        self.bot.connects('0')
        self.assertEqual(self.bot.team, b3.TEAM_RED)
        self.assertTrue(self.bot.bot)
        self.console.parseLine('InitRound: \\sv_allowdownload\x00\\g_matchmode\x00\\g_gametype\x04\\sv_maxclients\x0e\\sv_floodprotect\x01\\g_warmup\x05\\capturelimit\x00')
        self.assertEqual(self.bot.team, b3.TEAM_RED)
        self.assertTrue(self.bot.bot)

    def test_Hotpotato(self):
        self.assertEvent('Hotpotato:', event_type='EVT_GAME_FLAG_HOTPOTATO')

    def test_Callvote(self):
        self.joe.connects('1')
        self.assertEvent('Callvote: 1 - "map dressingroom"', event_type='EVT_CLIENT_CALLVOTE', event_client=self.joe, event_data='map dressingroom')

    def test_Vote(self):
        self.joe.connects('0')
        self.assertEvent('Vote: 0 - 2', event_type='EVT_CLIENT_VOTE', event_client=self.joe, event_data='2')

    def test_Votepassed(self):
        self.assertEvent('VotePassed: 1 - 0 - "reload"', event_type='EVT_VOTE_PASSED', event_data={'yes': 1, 'no': 0, 'what': 'reload'})

    def test_Votefailed(self):
        self.assertEvent('VoteFailed: 1 - 1 - "restart"', event_type='EVT_VOTE_FAILED', event_data={'yes': 1, 'no': 1, 'what': 'restart'})

    def test_Flagcapturetime(self):
        patate = FakeClient(self.console, name='Patate', guid='Patate_guid')
        patate.connects('0')
        self.assertEvent('FlagCaptureTime: 0: 1234567890', event_type='EVT_FLAG_CAPTURE_TIME', event_client=patate, event_data=1234567890)
    ...
    def test_Hit_1(self):
        ...
        fatmatic.connects('3')
        d4dou.connects('6')
        self.assertEvent("Hit: 6 3 5 8: Fat'Matic hit [FR]d4dou in the Torso", event_type='EVT_CLIENT_DAMAGE', event_client=fatmatic, event_target=d4dou, event_data=(17,
                                                                                                                                                                      '19',
                                                                                                                                                                      '5'))
    ...
    def test_Hit_2(self):
        ...
        fatmatic.connects('3')
        d4dou.connects('6')
        self.assertEvent("Hit: 3 6 9 17: [FR]d4dou hit Fat'Matic in the Legs", event_type='EVT_CLIENT_DAMAGE', event_client=d4dou, event_target=fatmatic, event_data=(13,
                                                                                                                                                                      '36',
                                                                                                                                                                      '9'))
    ...
    def test_Hit_unkown_location(self):
        ...
        with patch.object(self.console, 'warning') as (warning_mock):
            self.assertEvent("Hit: 6 3 321 8: Fat'Matic hit [FR]d4dou in the Pinky", event_type='EVT_CLIENT_DAMAGE', event_client=fatmatic, event_target=d4dou, event_data=(15,
                                                                                                                                                                            '19',
                                                                                                                                                                            '321'))
        self.assertListEqual([call('_getDamagePoints(19, 321) cannot find value : list index out of range')], warning_mock.mock_calls)
    ...
    def test_Kill(self):
        ...
        patate.connects('0')
        psyp.connects('1')
        self.assertEvent('Kill: 0 1 38: Patate killed psyp by UT_MOD_GLOCK', event_type='EVT_CLIENT_KILL', event_client=patate, event_target=psyp, event_data=(100,
                                                                                                                                                               '38',
                                                                                                                                                               'body',
                                                                                                                                                               'UT_MOD_GLOCK'))
    ...
    def test_say(self):
        ...
        marcel.connects('6')
        self.assertEvent('say: 6 ^5Marcel^2[^6CZARMY^2]: !help', event_type='EVT_CLIENT_SAY', event_client=marcel, event_data='!help')

    def test_ClientJumpRunStarted(self):
        marcel = FakeClient(self.console, name='^5Marcel^2[^6CZARMY^2]', guid='11111111111111')
        marcel.connects('0')
        self.assertEvent('ClientJumpRunStarted: 0 - way: 1', event_type='EVT_CLIENT_JUMP_RUN_START', event_client=marcel, event_data={'way_id': '1', 'attempt_num': None, 'attempt_max': None})
        return

    def test_ClientJumpRunStarted_with_attempt(self):
        marcel = FakeClient(self.console, name='^5Marcel^2[^6CZARMY^2]', guid='11111111111111')
        marcel.connects('0')
        self.assertEvent('ClientJumpRunStarted: 0 - way: 1 - attempt: 1 of 5', event_type='EVT_CLIENT_JUMP_RUN_START', event_client=marcel, event_data={'way_id': '1', 'attempt_num': '1', 'attempt_max': '5'})

    def test_ClientJumpRunStopped(self):
        marcel = FakeClient(self.console, name='^5Marcel^2[^6CZARMY^2]', guid='11111111111111')
        marcel.connects('0')
        self.assertEvent('ClientJumpRunStopped: 0 - way: 1 - time: 12345', event_type='EVT_CLIENT_JUMP_RUN_STOP', event_client=marcel, event_data={'way_id': '1', 'way_time': '12345', 'attempt_max': None, 'attempt_num': None})
        return

    def test_ClientJumpRunStopped_with_attempt(self):
        marcel = FakeClient(self.console, name='^5Marcel^2[^6CZARMY^2]', guid='11111111111111')
        marcel.connects('0')
        self.assertEvent('ClientJumpRunStopped: 0 - way: 1 - time: 12345 - attempt: 1 of 5', event_type='EVT_CLIENT_JUMP_RUN_STOP', event_client=marcel, event_data={'way_id': '1', 'way_time': '12345', 'attempt_max': '5', 'attempt_num': '1'})

    def test_ClientJumpRunCancelled(self):
        marcel = FakeClient(self.console, name='^5Marcel^2[^6CZARMY^2]', guid='11111111111111')
        marcel.connects('0')
        self.assertEvent('ClientJumpRunCanceled: 0 - way: 1', event_type='EVT_CLIENT_JUMP_RUN_CANCEL', event_client=marcel, event_data={'way_id': '1', 'attempt_max': None, 'attempt_num': None})
        return

    def test_ClientJumpRunCancelled_with_attempt(self):
        marcel = FakeClient(self.console, name='^5Marcel^2[^6CZARMY^2]', guid='11111111111111')
        marcel.connects('0')
        self.assertEvent('ClientJumpRunCanceled: 0 - way: 1 - attempt: 1 of 5', event_type='EVT_CLIENT_JUMP_RUN_CANCEL', event_client=marcel, event_data={'way_id': '1', 'attempt_max': '5', 'attempt_num': '1'})

    def test_ClientSavePosition(self):
        marcel = FakeClient(self.console, name='^5Marcel^2[^6CZARMY^2]', guid='11111111111111')
        marcel.connects('0')
        self.assertEvent('ClientSavePosition: 0 - 335.384887 - 67.469154 - -23.875000', event_type='EVT_CLIENT_POS_SAVE', event_client=marcel, event_data={'position': (335.384887, 67.469154, -23.875)})

    def test_ClientLoadPosition(self):
        marcel = FakeClient(self.console, name='^5Marcel^2[^6CZARMY^2]', guid='11111111111111')
        marcel.connects('0')
        self.assertEvent('ClientLoadPosition: 0 - 335.384887 - 67.469154 - -23.875000', event_type='EVT_CLIENT_POS_LOAD', event_client=marcel, event_data={'position': (335.384887, 67.469154, -23.875)})
    ...
    def test_ClientGoto(self):
        ...
        patate.connects('0')
        psyp.connects('1')
        self.assertEvent('ClientGoto: 0 - 1 - 335.384887 - 67.469154 - -23.875000', event_type='EVT_CLIENT_GOTO', event_client=patate, event_target=psyp, event_data={'position': (335.384887, 67.469154, -23.875)})

    def test_ClientSpawn(self):
        patate = FakeClient(self.console, name='Patate', guid='Patate_guid')
        patate.connects('0')
        self.assertEvent('ClientSpawn: 0', event_type='EVT_CLIENT_SPAWN', event_client=patate, event_data=None)
        return
    ...
    def test_client_freeze(self):
        ...
        bob.connects('1')
        self.assertEvent('Freeze: 0 1 16: Alice froze Bob by UT_MOD_SPAS', event_type='EVT_CLIENT_FREEZE', event_client=alice, event_target=bob, event_data=Iourt42Parser.UT_MOD_SPAS)
    ...
    def test_client_thawout_started(self):
        ...
        bob = FakeClient(self.console, name='Bob', guid='bobguid')
        bob.connects('1')
        self.assertEvent('ThawOutStarted: 0 1: Alice started thawing out Biddle', event_type='EVT_CLIENT_THAWOUT_STARTED', event_client=alice, event_target=bob)
    ...
    def test_client_thawout_finished(self):
        ...
        bob = FakeClient(self.console, name='Bob', guid='bobguid')
        bob.connects('1')
        self.assertEvent('ThawOutFinished: 0 1: Fenix thawed out Biddle', event_type='EVT_CLIENT_THAWOUT_FINISHED', event_client=alice, event_target=bob)

    def test_client_melted(self):
        alice = FakeClient(self.console, name='Alice', guid='aliceguid')
        alice.connects('0')
        self.assertEvent('ClientMelted: 0', event_type='EVT_CLIENT_MELTED', event_client=alice)

    def test_SurvivorWinner_player(self):
        marcel = FakeClient(self.console, name='^5Marcel^2[^6CZARMY^2]', guid='11111111111111')
        marcel.connects('0')
        self.assertEvent('SurvivorWinner: 0', event_type='EVT_CLIENT_SURVIVOR_WINNER', event_client=marcel)

    def test_SurvivorWinner_team(self):
        self.assertEvent('SurvivorWinner: Red', event_type='EVT_SURVIVOR_WIN', event_data='Red')
        self.assertEvent('SurvivorWinner: Blue', event_type='EVT_SURVIVOR_WIN', event_data='Blue')

    def test_bomb_related(self):
        self.joe.connects('2')
        self.assertEvent('Bomb was tossed by 2', event_type='EVT_CLIENT_ACTION', event_data='bomb_tossed', event_client=self.joe)
        self.assertEvent('Bomb was planted by 2', event_type='EVT_CLIENT_ACTION', event_data='bomb_planted', event_client=self.joe)
        self.assertEvent('Bomb was defused by 2!', event_type='EVT_CLIENT_ACTION', event_data='bomb_defused', event_client=self.joe)
        self.assertEvent('Bomb has been collected by 2', event_type='EVT_CLIENT_ACTION', event_data='bomb_collected', event_client=self.joe)
        self.assertEvent('Pop!', event_type='EVT_BOMB_EXPLODED', event_data=None, event_client=None)
        return

    def test_say_after_player_changed_name(self):

        def assert_new_name_and_text_does_not_break_auth(new_name, text='!help'):
            self.console.parseLine('777:16 ClientUserinfoChanged: 2 n\\%s\\t\\3\\r\\1\\tl\\0\\f0\\\\f1\\\\f2\\\\a0\\0\\a1\\255\\a2\\0' % new_name)
            self.console.parseLine('777:16 AccountValidated: 2 - louk - 6 - "basic"')
            self.console.parseLine('777:16 ClientUserinfo: 2 \\name\\%s\\ip\\49.111.22.33:27960\\password\\xxxxxx\\racered\\0\\raceblue\\0\\rate\\16000\\ut_timenudge\\0\\cg_rgb\\0 255 0\\funred\\ninja,caprd,bartsor\\funblue\\ninja,gasmask,capbl\\cg_physics\\1\\snaps\\20\\color1\\4\\color2\\5\\handicap\\100\\sex\\male\\cg_autoPickup\\-1\\cg_ghost\\0\\cl_time\\n34|0610q5qH=t<a\\racefree\\1\\gear\\GZAAVWT\\authc\\2708\\cl_guid\\AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\\weapmodes\\01000110220000020002000' % new_name)
            self.console.parseLine('777:16 ClientUserinfoChanged: 2 n\\%s\\t\\3\\r\\1\\tl\\0\\f0\\\\f1\\\\f2\\\\a0\\0\\a1\\255\\a2\\0' % new_name)
            self.assertEvent('777:18 say: 2 %s: %s' % (new_name, text), event_type='EVT_CLIENT_SAY', event_client=player, event_data=text.lstrip())
...
class Test_kill_mods(Test_log_lines_parsing):
    ...
    def test_mod_water(self):
        self.assertEvent('0:56 Kill: 1022 0 1: <world> killed Joe by MOD_WATER', event_type='EVT_CLIENT_SUICIDE', event_client=self.joe, event_target=self.joe, event_data=(
         100, self.console.MOD_WATER, 'body', 'MOD_WATER'))

    def test_nuke(self):
        self.assertEvent('5:19 Kill: 0 0 35: Joe killed Joe by UT_MOD_NUKED', event_type='EVT_CLIENT_KILL', event_client=self.world, event_target=self.joe, event_data=(
         100, self.console.UT_MOD_NUKED, 'body', 'UT_MOD_NUKED'))

    def test_lava(self):
        self.assertEvent('5:19 Kill: 1022 0 3: <world> killed Joe by MOD_LAVA', event_type='EVT_CLIENT_SUICIDE', event_client=self.joe, event_target=self.joe, event_data=(
         100, self.console.MOD_LAVA, 'body', 'MOD_LAVA'))

    def test_falling(self):
        self.assertEvent('0:32 Kill: 1022 0 6: <world> killed Joe by MOD_FALLING', event_type='EVT_CLIENT_SUICIDE', event_client=self.joe, event_target=self.joe, event_data=(
         100, self.console.MOD_FALLING, 'body', 'MOD_FALLING'))

    def test_unknown_kill_mode(self):
        self.assertEvent('5:19 Kill: 0 0 1234: Joe killed Joe by MOD_F00', event_type='EVT_CLIENT_SUICIDE', event_client=self.joe, event_target=self.joe, event_data=(100,
                                                                                                                                                                      '1234',
                                                                                                                                                                      'body',
                                                                                                                                                                      'MOD_F00'))
    ...
    def test_constants(self):
        ...
        def assert_mod(kill_mod_number, kill_mod_name):
            ...
            with patch.object(self.console, 'queueEvent') as (queueEvent):
                self.console.parseLine('Kill: 0 1 %s: Joe killed Bob by %s' % (kill_mod_number, kill_mod_name))
                assert queueEvent.called, 'No event was fired'
                args = queueEvent.call_args
...
class Test_OnClientuserinfo(Iourt42TestCase):
    ...
    def test_ioclient(self):
        ...
        self.assertEqual(self.console.queryClientFrozenSandAccount.call_count, 2)
        return
    ...
    def test_bot(self):
        infoline = '0 \\gear\\GMIORAA\\team\\blue\\skill\\5.000000\\characterfile\\bots/ut_chicken_c.c\\color\\4\\sex\\male\\race\\2\\snaps\\20\\rate\\25000\\name\\InviteYourFriends!'
...
class Test_OnClientuserinfochanged(Iourt42TestCase):
    ...
    def test_ioclient(self):
        ...
        self.assertEqual('laCourge', client.name)
        self.assertEqual('00000000011111111122222223333333', client.guid)
        infoline = '2 n\\UnnamedOne\\t\\3\\r\\2\\tl\\0\\f0\\\\f1\\\\f2\\\\a0\\0\\a1\\0\\a2\\0'
...
class Test_queryClientFrozenSandAccount(Iourt42TestCase):
    ...
    def test_authed(self):
        when(self.console).write('auth-whois 0').thenReturn('auth: id: 0 - name: ^7laCourge - login: courgette - notoriety: serious - level: -1')
    ...
    def test_not_active(self):
        when(self.console).write('auth-whois 3').thenReturn('Client 3 is not active.')
        data = self.console.queryClientFrozenSandAccount('3')
        self.assertDictEqual({}, data)

    def test_no_account(self):
        when(self.console).write('auth-whois 3').thenReturn('auth: id: 3 - name: ^7laCourge - login:  - notoriety: 0 - level: 0  - ^7no account')
        data = self.console.queryClientFrozenSandAccount('3')
        self.assertDictEqual({'cid': '3', 'name': 'laCourge', 'login': '', 'notoriety': '0', 'level': '0', 'extra': '^7no account'}, data)

    def test_all(self):
        when(self.console).write('auth-whois all', maxRetries=anything()).thenReturn('No player found for "all".\nauth: id: 0 - name: ^7laCourge - login: courgette - notoriety: serious - level: -1\nauth: id: 1 - name: ^7f00 - login:  - notoriety: 0 - level: 0  - ^7no account\nauth: id: 2 - name: ^7Qant - login: qant - notoriety: basic - level: -1\n')
        data = self.console.queryAllFrozenSandAccount()
        self.assertDictEqual({'0': {'cid': '0', 'extra': None, 'level': '-1', 'login': 'courgette', 'name': 'laCourge', 'notoriety': 'serious'}, '1': {'cid': '1', 'extra': '^7no account', 'level': '0', 'login': '', 'name': 'f00', 'notoriety': '0'}, '2': {'cid': '2', 'extra': None, 'level': '-1', 'login': 'qant', 'name': 'Qant', 'notoriety': 'basic'}}, data)
        return
...
class Test_auth_without_FSA(Iourt42TestCase):
    ...
    def setUp(self):
        Iourt42TestCase.setUp(self)
        when(self.console).write('auth-whois 3').thenReturn('auth: id: 3 - name: ^7Joe - login:  - notoriety: 0 - level: 0  - ^7no account')
...
class Test_auth_with_unknown_FSA(Iourt42TestCase):
    ...
    def setUp(self):
        Iourt42TestCase.setUp(self)
        when(self.console).write('auth-whois 3').thenReturn('auth: id: 3 - name: ^7Joe - login: joe_fsa - notoriety: serious - level: -1')
...
class Test_parser_API(Iourt42TestCase):
    ...
    def test_getPlayerList(self):
        when(self.console).write('status', maxRetries=anything()).thenReturn('map: ut4_casa\nnum score ping name            lastmsg address               qport rate\n--- ----- ---- --------------- ------- --------------------- ----- -----\n  4     0  141 theName^7              0 11.22.33.44:27961     38410  8000\n  5     0   48 theName2^7             0 11.22.33.45:27961     38410  8000\n')
        rv = self.console.getPlayerList()
        self.assertDictContainsSubset({'5': {'slot': '5', 'last': '0', 'name': 'theName2^7', 'ip': '11.22.33.45', 'ping': '48', 'pbid': None, 'qport': '38410', 'rate': '8000', 'score': '0', 'port': '27961'}, '4': {'slot': '4', 'last': '0', 'name': 'theName^7', 'ip': '11.22.33.44', 'ping': '141', 'pbid': None, 'qport': '38410', 'rate': '8000', 'score': '0', 'port': '27961'}}, rv)
        return

    def test_sync(self):
        when(self.console).write('status', maxRetries=anything()).thenReturn('map: ut4_casa\nnum score ping name            lastmsg address               qport rate\n--- ----- ---- --------------- ------- --------------------- ----- -----\n  0     0   48 laCourge^7              0 11.22.33.44:27961  13492 16000\n')
        when(self.console).write('dumpuser 0').thenReturn('userinfo\n--------\nip                  11.22.33.44:27961\nname                laCourge\ncl_guid             00000000000000014111111111111111\n')
        when(self.console).write('auth-whois 0').thenReturn('auth: id: 0 - name: ^7laCourge - login: courgette - notoriety: serious - level: -1\n')
    ...
    def test_getMap(self):
        when(self.console).write('status').thenReturn('map: ut4_casa\nnum score ping name            lastmsg address               qport rate\n--- ----- ---- --------------- ------- --------------------- ----- -----\n')
