...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\b3\plugins\poweradminurt\iourt41.py
# Compiled at: 2016-03-08 18:42:10
import b3, b3.config, b3.events, b3.plugin, b3.cron, time, thread, threading, re, os, random, string
...
class Poweradminurt41Plugin(b3.plugin.Plugin):
    _reClean = re.compile('(\\^.)|[\\x00-\\x20]|[\\x7E-\\xff]', re.I)
    _adminPlugin = None
    _ignoreTill = 0
    _checkdupes = True
    _checkunknown = True
    ...
    def loadTeamBalancer(self):
        ...
        self._autobalance_gametypes_array = re.split('[\\s,]+', self._autobalance_gametypes)
    ...
    def onAction(self, event):
        ...
        """
        if event.data in ('flag_captured', 'flag_dropped', 'flag_returned', 'bomb_planted',
                          'bomb_defused'):
    ...
    def _saveTeamvars(self, client):
        for var in ('kills', 'deaths', 'teamkills', 'headhits', 'helmethits', 'flag_captured',
                    'flag_returned', 'bomb_planted', 'bomb_defused'):
    ...
    def _getScores(self, clients, usexlrstats=True):
        ...
        maxstats = {}
        minstats = {}
        keys = ('hsratio', 'killratio', 'teamcontrib', 'xhsratio', 'xkillratio', 'flagperf',
                'bombperf')
        now = self.console.time()
        for c in clients:
        ...
        for c in clients:
            score = 0.0
            tm = min(1.0, playerstats[c.id]['age'] / 5.0)
            msg = []
            for key in keys:
            ...
            for _ in xrange(moves):
                ...
                if newteam and scores:
                    ...
                    if scores[c.id] == bestscore:
                        messages = ['You were moved because %n team needs more noobs.',
                        ...
                        messages = ['%n team needs your help! Try not to die too many times...',
    ...
    def _randTeams(self, times, slack, maxmovesperc=None):
        ...
        sbestdiff = None
        bestnumdiff = None
        bestblue = bestred = None
        sbestblue = sbestred = None
        epsilon = 0.0001
        ...
        if sbestdiff is not None:
            ...
            return (
             olddiff, sbestdiff, sbestblue, sbestred, scores)
        else:
            return (
             olddiff, bestdiff, bestblue, bestred, scores)
    ...
    def cmd_paskuffle(self, data=None, client=None, cmd=None):
        ...
        if client and client.maxLevel < 20 and self.ignoreCheck() and sincelast < 60 * self._minbalinterval:
            ...
            if client:
                if client.team == b3.TEAM_BLUE and client.cid not in [ c.cid for c in blue ] or client.team == b3.TEAM_RED and client.cid not in [ c.cid for c in red ]:
                    blue, red = red, blue
            moves = 0
            if bestdiff is not None:
    ...
    def cmd_paautoskuffle(self, data, client, cmd=None):
        ...
        """
        modes = [
         '0-none', '1-advise', '2-autobalance', '3-autoskuffle']
        if not data:
    ...
    def cmd_paforce(self, data, client, cmd=None):
        ...
        if team == 'free':
            ...
            return
        if lock:
            sclient.setvar(self, 'paforced', team)
        else:
            sclient.delvar(self, 'paforced')
        ...
        if not data or data not in ('on', 'off'):
            ...
            if data == 'on':
                ...
                if newpassword is None:
                    client.message('^1ERROR: ^7could not set public mode off because                                 there is no password specified in the config file')
                    return
    ...
    def cmd_pagear(self, data, client=None, cmd=None):
        ...
        if not data:
            ...
            return
        if data[:5] not in ('all', 'none', 'reset', '+nade', '+snip', '+spas', '+pist',
                            '+auto', '+nege', '-nade', '-snip', '-spas', '-pist',
                            '-auto', '-nege'):
            if client:
        ...
        if self._team_change_force_balance_enable and not self._matchmode:
            ...
            if self.isEnabled() and not self._balancing:
                ...
                if self.console:
                    ...
                    self.console.queueEvent(self.console.getEvent('EVT_CLIENT_SUICIDE', (100,
                                                                                         'penalty',
                                                                                         'body',
                                                                                         'Team_Switch_Penalty'), client, client))
                    self.console.queueEvent(self.console.getEvent('EVT_CLIENT_SUICIDE', (100,
                                                                                         'penalty',
                                                                                         'body',
                                                                                         'Team_Switch_Penalty'), client, client))
                    ...
                    if plugin:
                        client.message('^7Switching made teams ^1UNFAIR^7! Points where deducted from your stats as a penalty!')
    ...
    def setupVars(self, client):
        if not client.isvar(self, 'totalhits'):
            client.setvar(self, 'totalhits', 0.0)
        if not client.isvar(self, 'totalhitted'):
            client.setvar(self, 'totalhitted', 0.0)
        if not client.isvar(self, 'headhits'):
            client.setvar(self, 'headhits', 0.0)
        if not client.isvar(self, 'headhitted'):
            client.setvar(self, 'headhitted', 0.0)
        if not client.isvar(self, 'helmethits'):
            client.setvar(self, 'helmethits', 0.0)
        if not client.isvar(self, 'torsohitted'):
            client.setvar(self, 'torsohitted', 0.0)
        ...
        if self.isEnabled() and self._hsenable:
            ...
            for c in clients:
                if c.isvar(self, 'hitvars'):
                    c.setvar(self, 'totalhits', 0.0)
                    c.setvar(self, 'totalhitted', 0.0)
                    c.setvar(self, 'headhits', 0.0)
                    c.setvar(self, 'headhitted', 0.0)
                    c.setvar(self, 'helmethits', 0.0)
                    c.setvar(self, 'torsohitted', 0.0)
    ...
    def headshotcounter(self, event):
        ...
        if self.isEnabled() and self._hsenable and attacker.isvar(self, 'hitvars') and victim.isvar(self, 'hitvars') and not self._matchmode:
            ...
            if self._hswarnhelmet and victim.connections < 20 and victim.var(self, 'headhitted').value == self._hswarnhelmetnr and hitloc == self._hitlocations['HL_HEAD']:
                victim.message('You were hit in the head %s times! Consider wearing a helmet!' % self._hswarnhelmetnr)
            if self._hswarnkevlar and victim.connections < 20 and victim.var(self, 'torsohitted').value == self._hswarnkevlarnr and hitloc == self._hitlocations['HL_TORSO']:
                victim.message('You were hit in the torso %s times! Wearing kevlar vest will reduce                                 your number of deaths!' % self._hswarnkevlarnr)
