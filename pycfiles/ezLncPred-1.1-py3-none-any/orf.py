...
# [GCC 8.4.0]
# Embedded file name: /mnt/zxh/LncrnaPackage/models/CPAT/lib/cpmodule/orf.py
            ...
            if c in start_codens or not start_codens and c not in stop_codens:
                ...
                while True:
                    ...
                    if end:
                        ...
                        if L == self.winner and orf_start < self.result[2]:
                            self.result = (direction, frame_number + 1, orf_start, orf_end, L)
                        break

    def longest_orf(self, direction, start_coden=[
 'ATG'], stop_coden=['TAG', 'TAA', 'TGA']):
        if direction == '+':
            for frame in range(3):
                self.run_one(frame, direction, start_coden, stop_coden)

            return (self.result[4], self.result[1], self.seq[self.result[2]:self.result[3]])
        if direction == '-':
