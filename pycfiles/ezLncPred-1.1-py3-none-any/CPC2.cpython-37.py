...
# [GCC 8.4.0]
# Embedded file name: /mnt/zxh/Ezlncpred_LS_test/Ezlncpred/models/CPC2/bin/CPC2.py
# Compiled at: 2019-11-21 07:35:41
# Size of source mod 2**32: 11667 bytes
...
class FindCDS:
    ...
    def get_codons(self, frame_number):
        ...
        while coordinate + 3 <= len(self.seq):
            yield (
             self.seq[coordinate:coordinate + 3], coordinate)
            coordinate += 3
    ...
    def longest_orf(self, direction, start_codon={'ATG': None}, stop_codon={'TAG':None,  'TAA':None,  'TGA':None}):
        ...
        if direction == '-':
            ...
            if self.result[0] == '-':
                ...
                orf_integrity = self.result[4]
        return (
         return_orf, start_coordinate, strand_direction, orf_integrity)
...
class Fickett:
    ...
    def __init__(self):
        ...
        self.position_parameter = [
         1.9, 1.8, 1.7, 1.6, 1.5, 1.4, 1.3, 1.2, 1.1, 0.0]
        self.content_parameter = [0.33, 0.31, 0.29, 0.27, 0.25, 0.23, 0.21, 0.19, 0.17, 0]
        self.position_probability = {'A':[
          0.51, 0.55, 0.57, 0.52, 0.48, 0.58, 0.57, 0.54, 0.5, 0.36], 
         'C':[
          0.29, 0.44, 0.55, 0.49, 0.52, 0.6, 0.6, 0.56, 0.51, 0.38], 
         'G':[
          0.62, 0.67, 0.74, 0.65, 0.61, 0.62, 0.52, 0.41, 0.31, 0.17], 
         'T':[
          0.51, 0.6, 0.69, 0.64, 0.62, 0.67, 0.58, 0.48, 0.39, 0.24]}
        self.position_weight = {'A':0.062, 
         'C':0.093,  'G':0.205,  'T':0.154}
        self.content_probability = {'A':[
          0.4, 0.55, 0.58, 0.58, 0.52, 0.48, 0.45, 0.45, 0.38, 0.19], 
         'C':[
          0.5, 0.63, 0.59, 0.5, 0.46, 0.45, 0.47, 0.56, 0.59, 0.33], 
         'G':[
          0.21, 0.4, 0.47, 0.5, 0.52, 0.56, 0.57, 0.52, 0.44, 0.23], 
         'T':[
          0.3, 0.49, 0.56, 0.53, 0.48, 0.48, 0.52, 0.57, 0.6, 0.51]}
        self.content_weight = {'A':0.084, 
         'C':0.076,  'G':0.081,  'T':0.055}
...
def calculate_potential(fasta, strand, outfile):
    ...
    data_dir = script_dir + '/models/CPC2/data/'
    lib_dir = script_dir + '/models/CPC2/../tools/'
    app_svm_scale = lib_dir + 'libsvm/svm-scale'
    app_svm_predict = lib_dir + 'libsvm/svm-predict'
    print('svm path is')
    print(app_svm_scale, app_svm_predict)
    ...
    cmd = cmd + app_svm_predict + ' -b 1 ' + outfile + '.tmp.2 ' + data_dir + 'cpc2.model ' + outfile + '.tmp.1 &&'
