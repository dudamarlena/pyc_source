...
# [GCC 8.4.0]
# Embedded file name: /mnt/zxh/ezLncPred/ezLncPred/models/PredLnc_GFStack/src/get_features_module/ORF.py
...
class ExtractORF:
    ...
    def codons(self, frame):
        ...
        while start_coord + 3 <= len(self.seq):
            yield (
             self.seq[start_coord:start_coord + 3], start_coord)
            start_coord += 3
                    ...
                    if end:
                        ...
                        if ORF_Length == self.longest:
                            if ORF_start < self.result[1]:
                                self.result = (
                                 integrity, ORF_start, ORF_end, ORF_Length)
                        break

    def longest_ORF(self, start=[
 'ATG'], stop=['TAA', 'TAG', 'TGA']):
        orf_seq = ''
