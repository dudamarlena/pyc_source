...
# [GCC 8.4.0]
# Embedded file name: /mnt/zxh/ezLncPred/ezLncPred/models/PredLnc_GFStack/src/PredLnc_GFStack.py
# Compiled at: 2020-01-13 01:51:02
# Size of source mod 2**32: 13059 bytes
import numpy as np, pandas as pd, os, random, time
...
def main(args):
    ...
    saveDir = script_dir + '/'
    if getModel(str('GFStack_' + args.species), saveDir):
        args.input = args.fasta
        ...
        if args.input:
            if args.output:
                if args.species:
                    ...
                    label, All_proba = predict(model)
                    print('[INFO] Done!')
                    print('[INFO] Saving labels...')
                    save_result(label, output_file, seq_id, All_proba)
                    print('[INFO] Finish!')
        else:
...
def getModel(model, saveDir):
    print(Fore.CYAN + Style.BRIGHT + '------------------------------Start Loading %s models----------------------------\n' % model)
    modelAbsPath = saveDir + model
    if os.path.exists(modelAbsPath):
        print(Fore.CYAN + Style.BRIGHT + '-------------------------------------Loading Completed--------------------------------------\n')
        return True
    else:
        choice = input(Fore.GREEN + Style.BRIGHT + 'Loading Failed! Do you want to download ' + model + ' models from remote repository ? [y/n]' + '\x1b[39m')
        if choice == 'y':
            downURL = 'http://120.27.208.220/lncRNAPredModels/' + model + '.tgz'
...
def Get_ORF_features(input_file):
    transcript_length, first_orf_len, first_orf_Rlen, longest_orf_len, longest_orf_Rlen, integrity, ORF_frame_score, reading_frame_1, reading_frame_2, reading_frame_3, ORF_kmers = ([], [], [], [], [], [], [], [], [], [], [])
    seq_id = []
    names = [
     'A', 'G', 'C', 'T', 'AA', 'AAA', 'AAC', 'AAG', 'AAT', 'AC', 'ACA', 'ACC', 'ACG', 'ACT', 'AG', 'AGA',
    ...
    for seq in SeqIO.parse(input_file, 'fasta'):
        ...
        transcript_length.append(a)
        first_orf_len.append(b)
        first_orf_Rlen.append(c)
        longest_orf_len.append(d)
        longest_orf_Rlen.append(e)
        integrity.append(f)
        ORF_frame_score.append(g)
        (
         reading_frame_1.append(h), reading_frame_2.append(i), reading_frame_3.append(j))
        ORF_kmers.append(k)
    ...
    for i in range(len(reading_frame_1)):
        a = GC1(reading_frame_1[i])
        b = GC1(reading_frame_2[i])
        c = GC1(reading_frame_3[i])
        GC1_frame_score.append((a - b) ** 2 + (a - c) ** 2 + (b - c) ** 2 / 2)
        a = GC2(reading_frame_1[i])
        b = GC2(reading_frame_2[i])
        c = GC2(reading_frame_3[i])
        GC2_frame_score.append((a - b) ** 2 + (a - c) ** 2 + (b - c) ** 2 / 2)
        a = GC3(reading_frame_1[i])
        b = GC3(reading_frame_2[i])
        c = GC3(reading_frame_3[i])
    ...
    tmp = {'transcript_length':transcript_length,  'first_orf_len':first_orf_len,  'first_orf_Rlen':first_orf_Rlen,  'longest_orf_len':longest_orf_len, 
     'longest_orf_Rlen':longest_orf_Rlen,  'integrity':integrity,  'ORF_frame_score':ORF_frame_score,  'GC1_frame_score':GC1_frame_score, 
     'GC2_frame_score':GC2_frame_score,  'GC3_frame_score':GC3_frame_score,  'pI_Mw_frame_score':pI_Mw_frame_score}
...
def get_sequence_features(input_file, model):
    script_dir = os.path.split(os.path.realpath(__file__))[0]
    if model == 'mouse':
        hex_file = script_dir + '/GFStack_mouse/Mouse_features_Hexamer.tsv'
    else:
        hex_file = script_dir + '/GFStack_human/Human_features_Hexamer.tsv'
    coding, noncoding = coding_nocoding_potential(hex_file)
    hexamer, Instability_index, pI, Gravy, fickett_score, STOP_Codon_Count, STOP_Codon_Frequency, PI_Mw, Mw, transcript_GC1, transcript_GC2, transcript_GC3, transcript_GC, snr, seq_id = ([], [], [], [], [], [], [], [], [], [], [], [], [], [], [])
    for seq in SeqIO.parse(input_file, 'fasta'):
        ...
        Mw.append(tmp[3])
        Instability_index.append(tmp[0])
        pI.append(tmp[1])
        Gravy.append(tmp[2])
        PI_Mw.append(tmp[4])
    ...
    tmp = {'hexamer_score':hexamer,  'Instability_index':Instability_index,  'pI':pI, 
     'Gravy':Gravy,  'fickett_score':fickett_score,  'Mw':Mw,  'PI_Mw':PI_Mw, 
     'STOP_Codon_Count':STOP_Codon_Count,  'STOP_Codon_Frequency':STOP_Codon_Frequency,  'transcript_GC1':transcript_GC1, 
     'transcript_GC2':transcript_GC2,  'transcript_GC3':transcript_GC3,  'transcript_GC':transcript_GC,  'snr':snr}
    result = pd.DataFrame(tmp)
...
def get_special_features(input_file, transcript_length):
    ...
    for seq in SeqIO.parse(input_file, 'fasta'):
        ...
        os.system(script_dir + '/txCdsPredict ' + tmp_fa + ' ' + tmp_cds)
    ...
    tmp = {'CDS_length':CDS_length, 
     'CDS_percentage':CDS_percentage, 
     'txCdsPredict_score':txCdsPredict_score}
    result = pd.DataFrame(tmp)
...
def predict(model):
    ...
    Selected_features = []
    if model == 'mouse':
        temp = script_dir + '/GFStack_mouse/'
        filename = temp + 'mouse' + '_selected_features.csv'
    else:
        temp = script_dir + '/GFStack_human/'
        filename = temp + 'human' + '_selected_features.csv'
...
def save_result(label, output_file, seq_id, ncPotential):
    with open(output_file, 'w') as (out):
        out.write('Sequence\tClass\tLncRNA Probability\n')
        count = 0
        for i in label:
            if i == 1:
                out.write(seq_id[count] + '\t' + 'lncRNA' + '\t' + str(ncPotential[count]) + '\n')
            else:
                out.write(seq_id[count] + '\t' + 'pct' + '\t' + str(ncPotential[count]) + '\n')
            count += 1
