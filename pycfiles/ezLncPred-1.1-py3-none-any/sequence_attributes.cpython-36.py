...
# [GCC 8.4.0]
# Embedded file name: /mnt/zxh/ezLncPred/ezLncPred/models/longdist/bin/sequence_attributes.py
# Compiled at: 2019-12-04 10:19:56
# Size of source mod 2**32: 10356 bytes
...
class SequenceAttributes:
    ...
    def process(self, patterns=ALL_PATTERNS):
        if self.use_intermediate_file and os.path.exists(self.LONGDIST_NPY % self.fasta_file):
            ...
            dt = np.dtype([
             (
              'id', np.str_, 32), ('class', np.int_), ('length', np.int_)] + [(pattern, np.float64) for pattern in patterns])
