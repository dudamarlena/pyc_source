...
# [GCC 8.4.0]
# Embedded file name: /mnt/zxh/ezLncPred/ezLncPred/models/CPPred/bin/CPPred.py
# Compiled at: 2019-12-09 21:53:28
# Size of source mod 2**32: 4673 bytes
import os, sys
from ezLncPred.models.CPPred.bin.feamodule import CTD
from ezLncPred.models.CPPred.bin.feamodule import ProtParam as PP
from ezLncPred.models.CPPred.bin.feamodule import ORF_length as len
import Bio.SeqIO as Seq
from ezLncPred.models.CPPred.bin.feamodule import fickett
from ezLncPred.models.CPPred.bin.feamodule import FrameKmer
...
def output_feature(seq_file, hex_file, species):
    ...
    for seq in Seq.parse(seq_file, 'fasta'):
        ...
        if species == 'Human':
            tem = [
             inte_fe, Cov, insta_fe, T2, C0, PI_fe, Len, AC, T0, G0, C2, A4, G2, TG, A0, TC, G1, C3, T3, A1, GC, T1, G4, C1, G3, A3, gra_fe, hexamer, C4, AG, fickett_fe, A2, T4, C, G, A, T]
        ...
        if species == 'Integrated':
            tem = [
             Cov, inte_fe, GC, insta_fe, Len, T0, fickett_fe, G2, C3, PI_fe, A3, C1, G3, hexamer, TG, G1, TC, A0, A1, AC, C2, G0, T4, C0, A4, G, A2, T, T3, G4, C4, gra_fe, T2, AG, AT, T1, A, C]
...
def predict(range_file, model_file):
    ...
    os.system(script_dir + '/../../tools/libsvm/svm-scale -r ' + range_file + ' test.f_svm  > test.scaled ')
    os.system(script_dir + '/../../tools/libsvm/svm-predict -b 1 test.scaled ' + model_file + ' tmp.txt >  tmp2.txt')
...
def main(args):
    script_dir = os.path.split(os.path.realpath(__file__))[0]
    args.hexmar = script_dir + '/../Hexamer/' + args.species + '_Hexamer.tsv'
    args.range = script_dir + '/../' + args.species + '_Model/' + args.species + '.range'
    args.whichmodel = script_dir + '/../' + args.species + '_Model/' + args.species + '.model'
