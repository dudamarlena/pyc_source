...
# [GCC 8.4.0]
# Embedded file name: /Users/ricard/develop/my_django_tweaks/my_django_tweaks/serializers.py
...
from rest_framework.fields import api_settings, DjangoValidationError, empty, OrderedDict, set_value, SkipField, ValidationError
    ...
    def get_fields_for_serialization(self, fields_name):
        ...
        if fields_name in self.context:
            ...
            if 'request' in self.context:
                if fields_name in self.context['request'].query_params:
    ...
    def check_if_needs_serialization(self, field_name, fields, include_fields, on_demand_fields):
        if fields:
            if field_name not in fields:
                return False
        elif field_name in on_demand_fields:
