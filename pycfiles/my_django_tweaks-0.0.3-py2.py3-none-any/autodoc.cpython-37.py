...
# [GCC 8.4.0]
# Embedded file name: /Users/ricard/develop/my_django_tweaks/my_django_tweaks/autodoc.py
...
class AutodocBase(object):
    __doc__ = ' base class for autodoc '
...
class PaginationAutodoc(AutodocBase):
    __doc__ = ' Autodoc for pagination - applied only when pagination is present. Please not that pagination is present by\n        default, so to avoid having pagination params in retrieve-type generics, you have to explicitly put there\n        pagination_class = None '
    applies_to = ('get', )
...
class PermissionsAutodoc(AutodocBase):
    __doc__ = ' Autodoc for permission classes - shows permissions + docstrings for them '
    applies_to = ('get', 'post', 'put', 'patch', 'delete')
    ...
    def _generate_text(cls, documented_cls, method_name):
        text = ''
        if hasattr(documented_cls, 'permission_classes'):
            if documented_cls.permission_classes:
...
class VersioningAutodoc(AutodocBase):
    __doc__ = ' autodoc for versioning - applied only when ApiVersionMixin is present and\n        rest_framework.versioning.AcceptHeaderVersioning '
...
class OrderingAndFilteringAutodoc(AutodocBase):
    __doc__ = ' Adding ordering & filtering informations '
    applies_to = ('get', )
...
class BaseInfoAutodoc(AutodocBase):
    __doc__ = ' insert the base docstring to each method - this will be displayed on the swagger folded list '
...
if hasattr(settings, 'AUTODOC_DEFAULT_CLASSESS'):
    ...
    DEFAULT_CLASSESS = (
     BaseInfoAutodoc, PermissionsAutodoc, OrderingAndFilteringAutodoc, PaginationAutodoc,
...
def autodoc(base_doc='', classess=DEFAULT_CLASSESS, add_classess=None, skip_classess=None):
    ...
    def wrapped(cls):
        applies_to = set([])
        classess_to_apply = [c for c in classess if skip_classess if c not in skip_classess]
        if add_classess:
            classess_to_apply += [c for c in add_classess if skip_classess if c not in skip_classess]
        ...
        for autodoc_class in classess_to_apply:
            for method_name in autodoc_class.applies_to:
                method = getattr(cls, method_name, None)
                if method:
                    six.get_unbound_function(method).__doc__ = autodoc_class.update_docstring(cls, base_doc, six.get_unbound_function(method).__doc__, method_name)
