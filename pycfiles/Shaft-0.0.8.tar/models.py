...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/shaft/shaft/contrib/auth/models.py
# Compiled at: 2017-02-11 20:25:46
import datetime, arrow
from . import exceptions
from flask_login import UserMixin
from shaft import db, utils, send_mail, _
...
class AuthUser(db.Model, UserMixin):
    ...
    def login_email_verfied(self):
        user_login = self.get_login('email')
        if user_login and user_login.email_verified:
            return True
        return False
    ...
    def has_any_roles(self, *roles):
        ...
        if AuthRole.query().join(AuthUser).filter(AuthRole.name.in_(roles)).filter(AuthUser.id == self.id).count():
            return True
        return False
    ...
    def send_email(self, template, **kwargs):
        ...
        :return:
        """
        return send_mail(template=template, to=self.contact_email, user=self.email_data, **kwargs)
    ...
    def sign_data(self, data, expires_in=None, url_safe=True):
        ...
        """
        if url_safe:
            return utils.sign_url_safe(data, secret_key=self.secret_key, salt=self.secret_salt, expires_in=expires_in)
        else:
            return utils.sign_data(data, secret_key=self.secret_key, salt=self.secret_salt, expires_in=expires_in)
    ...
    def unsign_data(self, data, url_safe=True):
        ...
        """
        if url_safe:
            return utils.unsign_url_safe(data, secret_key=self.secret_key, salt=self.secret_salt)
        else:
            return utils.unsign_data(data, secret_key=self.secret_key, salt=self.secret_salt)
...
class AuthUserLogin(db.Model):
    ...
    @classmethod
    def new(cls, email=None, username=None, password=None, provider='email', name=None, social_token=None, social_id=None, random_password=True, role=None, user=None):
        ...
        if not user:
            ...
            user = AuthUser(name=name, registration_method=data['provider'].lower(), active=True, role=role)
        data['user'] = user
    ...
    def get_by_social(cls, provider, social_id):
        return cls.query().filter(cls.provider == provider).filter(cls.social_id == social_id).first()
    ...
    def change_password(self, password):
        ...
        self.update(password_hash=self.encrypt_password(password), require_password_change=False)
