...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/shaft/shaft/contrib/views/auth.py
# Compiled at: 2017-02-12 23:55:00
import datetime, uuid, arrow, exceptions
from flask_login import fresh_login_required
import logging, exceptions
from flask_login import LoginManager, login_user, current_user, fresh_login_required
from shaft import Shaft, _, models, page_meta, request, redirect, flash_success, flash_error, flash_info, abort, recaptcha, storage, decorators as shaft_deco, exceptions as shaft_exc, exceptions, utils, paginate, views
from shaft.contrib.auth import is_authenticated, not_authenticated, send_password_reset_email, send_email_verification_email, send_registration_welcome_email, session_set_require_password_change, visible_to_managers, get_user_token, login, register, exceptions, change_password, change_login, update_info, __options__ as auth_options, login_manager, decorators as deco
    ...
    def login(self):
        if request.method == 'POST':
            ...
            try:
                ...
                user = login(email=email, password=password, require_verified_email=__options__.get('verify_email'))
                if not user:
...
class Account(Shaft):
    ...
    def update_info(self):
        ...
        try:
            ...
            kwargs = {}
            if file:
                my_photo = storage.upload(file, name=utils.guid(), prefix='profile-images/', allowed_extensions=[
                 'jpg', 'jpeg', 'png', 'gif'])
                if my_photo:
                    kwargs['profile_image'] = {'object_name': my_photo.name, 'url': my_photo.url, 
                       'path': my_photo.path, 
...
class Admin(Shaft):
    ...
    def _user_roles_options(cls):
        _r = models.AuthRole.query().filter(models.AuthRole.level <= current_user.role.level).order_by(models.AuthRole.level.desc())
    ...
    def create(self):
        try:
            ...
            if not name:
                ...
                if not password or password != password_confirm:
                    raise shaft_exc.AppError("Passwords don't match")
                user = register(email=email, password=password, name=name, role_name=role.name)
                if user:
        ...
        if request.method == 'POST':
            try:
                ...
                if name and level:
                    ...
                    elif models.AuthRole.get_by_level(level):
