...
# Compiled at: 2017-02-13 03:35:36
"""
Shaft

"""
import re, os, sys, arrow, jinja2, inspect, logging, functools, pkg_resources, logging.config, active_alchemy
...
def to_local_datetime(utcdatetime, timezone=None):
    """
    ...
    :return:
    """
    timezone = timezone or get_config('DATETIME_LOCAL_TIMEZONE', 'US/Eastern')
    if isinstance(utcdatetime, arrow.Arrow):
        return utcdatetime.to(timezone)
...
class Shaft(object):
    ...
    _global = dict(__NAME__=__title__, __VERSION__=__version__, __YEAR__=utc_now().year, __META__=_default_page_meta)
