...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/automata/fuzzystring.py
# Compiled at: 2012-12-20 01:24:56
import automata, os, timeit
            ...
            for e in range(k + 1):
                ...
                if e < k:
                    if 'substitute' in operations:
                        nfa.add_transition((i, e), automata.NFA.ANY, (i + 1, e + 1))
                    if 'insert' in operations:
                        nfa.add_transition((i, e), automata.NFA.ANY, (i, e + 1))
                    if 'delete' in operations:
                        nfa.add_transition((i, e), automata.NFA.EPSILON, (i + 1, e + 1))
...
def LevAutomata(term, k, operations=[
 'insert', 'delete', 'substitute'], search=False):
...
if __name__ == '__main__':
    ...
    haystack = eterm[2:] + eterm * 10 * 1048576 + 'F'
    for i in range(10):
        print 'executing against %f MB string' % (float(len(haystack)) / 1048576)
