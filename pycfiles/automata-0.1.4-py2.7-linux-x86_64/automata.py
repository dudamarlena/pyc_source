...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/automata/automata.py
# Compiled at: 2013-01-04 18:15:19
import copy, VM
...
class NFA(object):
    ...
    def is_tagged(self, transition, src, dst):
        return (
         transition, src, dst) in self._tag_assocs
    ...
    def _transitions_to_dfa_bytecode(self, sources, trn, cached_tcode, debug=False, compiled_states=None, gathered_epsilons=None, cached_transitions=None, reltags_cache=None):
        key = (
         trn, tuple(sources))
        if key in cached_tcode:
            return cached_tcode[key]
        sb = self._states_to_dfa_bytecode(sources, tran=trn, debug=debug, compiled_states=compiled_states, gathered_epsilons=gathered_epsilons, cached_transitions=cached_transitions, cached_tcode=cached_tcode, reltags_cache=reltags_cache)
    ...
    def _states_to_dfa_bytecode(self, states, tran=None, debug=False, compiled_states=None, gathered_epsilons=None, cached_transitions=None, cached_tcode=None, reltags_cache=None):
