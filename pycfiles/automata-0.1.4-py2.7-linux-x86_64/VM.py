...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/automata/VM.py
# Compiled at: 2012-12-20 01:24:56
import copy, avm, ctypes, util
...
class CodeContext(object):
    ...
    def get_blocks(self, entry_point=None):
        if entry_point != None:
            yield (
             entry_point, self._blocks[entry_point])
        for name in self._blocks:
            if name == entry_point:
                continue
            yield (
             name, self._blocks[name])
