...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/libs/alembic/maya/alembic.py
# Compiled at: 2020-05-13 18:49:44
# Size of source mod 2**32: 16443 bytes
"""
...
if tp.is_maya():
    ...
    from tpDcc.dccs.maya.core import geometrycache
...
class MayaAlembic(alembic.AbstractAlembic):
        ...
        if user_attr is None:
            ...
            if attr is None:
                ...
                if root is None:
                        ...
                        if frame_range is None:
                            ...
                            if user_attr_prefix is None:
                                user_attr_prefix = [
                                 'ABC_']
                            job_arg = ''
                            booleans = {'eulerFilter':euler_filter, 
                            ...
                            single_arguments = {'dataFormat':data_format, 
                             'step':step, 
                    ...
                    export_args = {'dontSkipUnwrittenFrames':dont_skip_unwritten_frames, 
                     'verbose':verbose, 
    ...
    def import_alembic(self, alembic_file, mode='import', nodes=None, parent=None, fix_path=False, namespace=None, reference=False, as_gpu_cache=True, unique_namespace=True):
        """
        ...
        if not os.path.exists(alembic_file):
            ...
            self.load_alembic_plugin()
            LOGGER.debug('Import Alembic File (%s) with job arguments:\n\t(alembic_file) %s\n\t(nodes) %s', mode, alembic_file, nodes)
            res = None
            try:
                if fix_path:
                ...
                if as_gpu_cache:
                    ...
                    if reference:
                        if namespace:
                            res = maya.cmds.file(abc_file, type='Alembic', reference=True, returnNewNodes=True)
                        else:
                            res = maya.cmds.file(abc_file, type='Alembic', reference=True, returnNewNodes=True)
                    else:
                        if nodes:
            ...
            if reference:
                LOGGER.info('Alembic File %s referenced successfully!', os.path.basename(alembic_file))
            else:
