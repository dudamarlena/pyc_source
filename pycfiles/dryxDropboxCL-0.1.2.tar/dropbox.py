...
# [GCC 8.4.0]
# Embedded file name: /Users/Dave/git_repos/dryxDropboxCL/dryxDropboxCL/original/dropbox.py
# Compiled at: 2016-03-16 05:54:03
from __future__ import with_statement
import errno, locale, optparse, os, platform, shutil, socket, StringIO, subprocess, sys, tarfile, tempfile, threading, thread, time, traceback, urllib2
try:
    import gpgme
except ImportError:
    gpgme = None
...
if GUI_AVAILABLE:
    ...
    def download():
        ...
        import pygtk
        pygtk.require('2.0')
        import gtk, gobject, pango, webbrowser
        gtk.gdk.threads_init()
        load_serialized_images()

        def FatalVisibleError(s):
            error = gtk.MessageDialog(parent=None, flags=gtk.DIALOG_MODAL, type=gtk.MESSAGE_ERROR, buttons=gtk.BUTTONS_OK, message_format=s)
        ...
        class DownloadDialog(gtk.Dialog):
            ...
            def handle_ok(self, button):
                ...
                self.task = GeneratorTask(self.download.copy_data, download_progress, finished, error).start()
                return
            ...
            def unpack_dropbox(self):
                ...
                self.task = GeneratorTask(self.download.unpack, unpack_progress, finished, error).start()
            ...
            def label_motion(self, widget, event):
                ...
                index = layout.xy_to_index(int((offx + event.x) * pango.SCALE), int((offy + event.y) * pango.SCALE))[0]
            ...
            def __init__(self):
                super(DownloadDialog, self).__init__(parent=None, title='Dropbox Installation')
                self.download = None
                self.hovering = False
                self.clicked_link = False
                ...
                GPG_WARNING_MSG = (gpgme or '\n\n') + GPG_WARNING if 1 else ''
    ...
    def download():
        ...
        console_print('%s %s\n' % (INFO, LINK))
        if not gpgme:
            GPG_WARNING_MSG = '\n%s' % GPG_WARNING if 1 else ''
            return yes_no_question('%s%s' % (WARNING, GPG_WARNING_MSG)) or None
        else:
...
class DropboxCommand(object):
    ...
    def send_command(self, name, args):
        ...
        self.f.write(('\n').encode('utf8'))
        self.f.writelines((('\t').join([k] + (list(v) if hasattr(v, '__iter__') else [v])) + '\n').encode('utf8') for k, v in args.iteritems())
        self.f.write(('done\n').encode('utf8'))
        self.f.flush()
...
def start_dropbox():
    db_path = os.path.expanduser('~/.dropbox-dist/dropboxd').encode(sys.getfilesystemencoding())
    if os.access(db_path, os.X_OK):
        f = open('/dev/null', 'w')
        a = subprocess.Popen([db_path], preexec_fn=os.setsid, cwd=os.path.expanduser('~'), stderr=sys.stderr, stdout=f, close_fds=True)
        interval = 0.5
        wait_for = 60
...
def columnize(list, display_list=None, display_width=None):
    ...
    if non_unicode:
        raise TypeError, 'list[i] not a string for i in %s' % (', ').join(map(unicode, non_unicode))
            ...
            if options.list:
                if len(args) == 0:
                    ...
                    def path_to_string(file_path):
                        ...
                        supports_color = sys.stderr.isatty() and (env_term.startswith('vt') or env_term.startswith('linux') or 'xterm' in env_term or 'color' in env_term)
                ...
                if len(args) == 0:
                    args = [ name for name in sorted(os.listdir('.'), key=methodcaller('lower')) if type(name) == unicode ]
                if len(args) == 0:
                    console_print('<empty>')
                    return
