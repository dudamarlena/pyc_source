...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/ezplone/inst.py
# Compiled at: 2009-04-06 12:24:30
"""
...
class EasyInstaller(_BaseEasyClass):
    ...
    def install_tool(self, tool_cls):
        ...
        if not hasattr(self.context, tool_cls.id):
            ...
            portal.manage_addProduct[self.name].manage_addTool(tool_cls.meta_type, None)
            self.log('Installed tool %s.' % tool_cls.meta_type)
        else:
            self.log("Couldn't add tool %s as it was already present." % tool_cls.meta_type)
        return
    ...
    def install_workflow(self, workflow_id=None):
        ...
        theWorkflowTool.manage_addWorkflow(id=self.workflow_id, workflow_type='%s (%s Workflow)' % (self.workflow_id, self.name))
...
class EasyUninstaller(_BaseEasyClass):
    ...
    def uninstall_configlets(self, configlet_list):
        """
                ...
                """
        configTool = getToolByName(self.context, 'portal_controlpanel', None)
        assert configTool, "can't find portal_controlpanel for removing configlets"
        for conf in configlet_list:
