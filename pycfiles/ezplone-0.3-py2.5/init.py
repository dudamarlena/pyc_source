...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/ezplone/init.py
# Compiled at: 2009-04-06 12:15:50
"""
...
class EasyInitialiser(object):
    ...
    def init_content(self, perm):
        """
                ...
                """
        (content_types, constructors, ftis) = process_types(listTypes(self.name), self.name)
        utils.ContentInit(self.name + ' Content', content_types=content_types, permission=perm, extra_constructors=constructors, fti=ftis).initialize(self.context)
    ...
    def init_tools(self, tool_classes, tool_icon):
        """
                ...
                """
        utils.ToolInit(self.name + ' Tool', tools=tool_classes, icon=tool_icon).initialize(self.context)
