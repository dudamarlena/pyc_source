...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_paas_oracle/backend.py
...
class OracleBackend(ServiceBackend):
    ...
    def __init__(self, settings):
        ...
        if not self.templates or 'provision' not in self.templates:
            raise OracleBackendError('Improperly configured: ORACLE_TICKET_TEMPLATES should be defined')
    ...
    def sync(self):
        try:
            self.project = Project.objects.get(backend_id=self.settings.token, available_for_all=True)
    ...
    def _jira_ticket(self, name, deployment, request, **kwargs):
        ...
        message = unicode(template['details']).format(deployment=deployment, customer=deployment.service_project_link.service.customer, project=deployment.service_project_link.project, **kwargs)
        payload = {'project': reverse('jira-projects-detail', kwargs={'uuid': self.project.uuid.hex}), 
