...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_paas_oracle/models.py
...
class OracleService(structure_models.Service):
    projects = models.ManyToManyField(structure_models.Project, related_name='oracle_services', through='OracleServiceProjectLink')
...
class Deployment(structure_models.Resource):
    ...
    service_project_link = models.ForeignKey(OracleServiceProjectLink, related_name='deployments', on_delete=models.PROTECT)
    ...
    db_size = models.PositiveIntegerField(validators=[
     MinValueValidator(10), MaxValueValidator(2048)], help_text='Data storage size in GB')
    db_arch_size = models.PositiveIntegerField(validators=[
     MinValueValidator(10), MaxValueValidator(2048)], help_text='Archive storage size in GB', null=True, blank=True)
