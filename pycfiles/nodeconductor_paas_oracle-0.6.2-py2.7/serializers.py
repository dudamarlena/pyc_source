...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_paas_oracle/serializers.py
...
class ServiceSerializer(structure_serializers.BaseServiceSerializer):
    SERVICE_ACCOUNT_FIELDS = {'token': "JIRA project key (e.g. 'GM')"}
...
class ServiceProjectLinkSerializer(structure_serializers.BaseServiceProjectLinkSerializer):
    ...
    class Meta(structure_serializers.BaseServiceProjectLinkSerializer.Meta):
        ...
        extra_kwargs = {'service': {'lookup_field': 'uuid', 'view_name': 'oracle-detail'}}
...
class DeploymentSerializer(structure_serializers.BaseResourceSerializer):
    service = serializers.HyperlinkedRelatedField(source='service_project_link.service', view_name='oracle-detail', read_only=True, lookup_field='uuid')
    service_project_link = serializers.HyperlinkedRelatedField(view_name='oracle-spl-detail', queryset=models.OracleServiceProjectLink.objects.all())
    tenant = serializers.HyperlinkedRelatedField(view_name='openstack-tenant-detail', lookup_field='uuid', queryset=openstack_models.Tenant.objects.all())
    tenant_name = serializers.ReadOnlyField(source='tenant.name')
    flavor = serializers.HyperlinkedRelatedField(view_name='oracle-flavors-detail', lookup_field='uuid', queryset=models.Flavor.objects.all(), write_only=True)
    support_requests = SupportRequestSerializer(read_only=True, many=True)
    ssh_public_key = serializers.HyperlinkedRelatedField(view_name='sshpublickey-detail', lookup_field='uuid', queryset=core_models.SshPublicKey.objects.all(), required=False, write_only=True)
    ...
    class Meta(structure_serializers.BaseResourceSerializer.Meta):
        ...
        fields = structure_serializers.BaseResourceSerializer.Meta.fields + ('tenant',
                                                                             ...
                                                                             'key_name',
                                                                             'key_fingerprint')
        protected_fields = structure_serializers.BaseResourceSerializer.Meta.protected_fields + ('tenant',
                                                                                                 'flavor',
                                                                                                 'user_data',
                                                                                                 'ssh_public_key',
                                                                                                 'db_name',
    ...
    def get_fields(self):
        ...
        if 'ssh_public_key' in fields:
            fields['ssh_public_key'].queryset = fields['ssh_public_key'].queryset.filter(user=self.context['request'].user)
        if self.instance:
    ...
    def get_jira_issue_uuid(self, obj):
        try:
            ...
            return next(issue.uuid.hex for issue in obj.support_requests.all() if issue.summary == backend.templates['provision']['summary'])
    ...
    def get_jira_issue_key(self, obj):
        try:
            backend = obj.get_backend()
            return next(issue.key for issue in obj.support_requests.all() if issue.summary == backend.templates['provision']['summary'])
        except StopIteration:
            return
