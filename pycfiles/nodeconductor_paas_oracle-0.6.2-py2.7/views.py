...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_paas_oracle/views.py
...
class DeploymentViewSet(structure_views.BaseResourceViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if service_project_link.service.settings.state == core_models.SynchronizationStates.ERRED:
            raise core_exceptions.IncorrectStateException(detail='Cannot create resource if its service is in erred state.')
        try:
    ...
    def perform_provision(self, serializer):
        ...
        try:
            ticket = backend.provision(resource, self.request, ssh_key=serializer.validated_data.get('ssh_public_key'))
            event_logger.resource.info('{resource_full_name} creation has been scheduled (%s).' % ticket.key, event_type='resource_creation_scheduled', event_context={'resource': serializer.instance})
        except OracleBackendError as e:
            resource.error_message = unicode(e)
            resource.set_erred()
            resource.save(update_fields=['state', 'error_message'])
...
DBNAME='TST12DB'"
        ...
        if report:
            ...
            event_logger.oracle_deployment.info('Oracle deployment {deployment_name} report has been updated.', event_type='oracle_deployment_report_updated', event_context={'deployment': resource})
...
Thanks."
        ...
        event_logger.oracle_deployment.info('Support for Oracle deployment {deployment_name} has been requested ({jira_issue_key}).', event_type='oracle_deployment_support_requested', event_context={'deployment': resource, 
           'jira_issue_key': ticket.key})
        return response.Response({'detail': 'Support request accepted', 'jira_issue_uuid': ticket.uuid.hex, 'jira_issue_key': ticket.key}, status=status.HTTP_202_ACCEPTED)
    ...
    def resize(self, request, resource, uuid=None):
        ...
        if resource.state == States.RESIZING:
            ...
            resource.save(update_fields=['state'])
            event_logger.oracle_deployment.info('Resize request for Oracle deployment {deployment_name} has been fulfilled.', event_type='oracle_deployment_resize_succeeded', event_context={'deployment': resource})
        ...
        resource.save(update_fields=['state'])
        event_logger.oracle_deployment.info('Resize request for Oracle deployment {deployment_name} has been submitted ({jira_issue_key}).', event_type='oracle_deployment_resize_requested', event_context={'deployment': resource, 
           'jira_issue_key': ticket.key})
        return response.Response({'detail': 'Resizing scheduled', 'jira_issue_uuid': ticket.uuid.hex, 'jira_issue_key': ticket.key}, status=status.HTTP_202_ACCEPTED)
    ...
    def destroy(self, request, resource, uuid=None):
        ...
        resource.save(update_fields=['state'])
        return response.Response({'detail': 'Deletion scheduled', 'jira_issue_uuid': ticket.uuid.hex, 'jira_issue_key': ticket.key}, status=status.HTTP_202_ACCEPTED)
    ...
    def start(self, request, resource, uuid=None):
        ...
        if resource.state == States.STARTING:
            if not self.request.user.is_staff:
                ...
                resource.set_online()
                resource.save(update_fields=['state'])
                event_logger.oracle_deployment.info('Start request for Oracle deployment {deployment_name} has been fulfilled.', event_type='oracle_deployment_start_succeeded', event_context={'deployment': resource})
        ...
        resource.save(update_fields=['state'])
        event_logger.oracle_deployment.info('Start request for Oracle deployment {deployment_name} has been submitted ({jira_issue_key}).', event_type='oracle_deployment_start_requested', event_context={'deployment': resource, 
           'jira_issue_key': ticket.key})
        return response.Response({'detail': 'Starting scheduled', 'jira_issue_uuid': ticket.uuid.hex, 'jira_issue_key': ticket.key}, status=status.HTTP_202_ACCEPTED)
    ...
    def stop(self, request, resource, uuid=None):
        ...
        if resource.state == States.STOPPING:
            if not self.request.user.is_staff:
                ...
                resource.save(update_fields=['state'])
                event_logger.oracle_deployment.info('Stop request for Oracle deployment {deployment_name} has been fulfilled.', event_type='oracle_deployment_stop_succeeded', event_context={'deployment': resource})
        ...
        resource.save(update_fields=['state'])
        event_logger.oracle_deployment.info('Stop request for Oracle deployment {deployment_name} has been submitted ({jira_issue_key}).', event_type='oracle_deployment_stop_requested', event_context={'deployment': resource, 
           'jira_issue_key': ticket.key})
        return response.Response({'detail': 'Stopping scheduled', 'jira_issue_uuid': ticket.uuid.hex, 'jira_issue_key': ticket.key}, status=status.HTTP_202_ACCEPTED)
    ...
    def restart(self, request, resource, uuid=None):
        ...
        if resource.state == States.RESTARTING:
            if not self.request.user.is_staff:
                ...
                resource.set_online()
                resource.save(update_fields=['state'])
                event_logger.oracle_deployment.info('Restart request for Oracle deployment {deployment_name} has been fulfilled.', event_type='oracle_deployment_restart_succeeded', event_context={'deployment': resource})
        ...
        resource.save(update_fields=['state'])
        event_logger.oracle_deployment.info('Restart request for Oracle deployment {deployment_name} has been submitted ({jira_issue_key}).', event_type='oracle_deployment_restart_requested', event_context={'deployment': resource, 
           'jira_issue_key': ticket.key})
        return response.Response({'detail': 'Restarting scheduled', 'jira_issue_uuid': ticket.uuid.hex, 'jira_issue_key': ticket.key}, status=status.HTTP_202_ACCEPTED)