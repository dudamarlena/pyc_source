...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/autobahntestsuite/fuzzing.py
# Compiled at: 2018-12-17 11:51:20
__all__ = [
 'startClient', 'startServer', 'WS_COMPRESSION_TESTDATA']
...
from autobahn.twisted.websocket import WebSocketServerFactory, WebSocketServerProtocol, WebSocketClientFactory, WebSocketClientProtocol
from case import Case, Cases, CaseCategories, CaseSubCategories, CaseSetname, CaseBasename
...
class FuzzingProtocol():
   ...
   """
    ...
    def connectionMade(self):
        attrs = [
         'case', 'runCase', 'caseAgent', 'caseStarted', 'connectionWasOpen', 'shutdownOnComplete']
        for attr in attrs:
    ...
    def connectionLost(self, reason):
        if self.runCase:
            ...
            caseResult = {'case': self.case, 'id': self.factory.CaseSet.caseClasstoId(self.Case), 
               'description': self.Case.DESCRIPTION, 
               'expectation': self.Case.EXPECTATION, 
               'agent': self.caseAgent, 
               'started': self.caseStarted, 
               'duration': int(round(1000.0 * (self.caseEnd - self.caseStart))), 
               'reportTime': self.runCase.reportTime, 
    ...
    def onOpen(self):
        ...
        if self.runCase:
            ...
            self.sendMessage(json.dumps({'id': self.factory.CaseSet.caseClasstoId(self.Case), 
    ...
    def onMessage(self, msg, binary):
        if self.runCase:
            ...
            if obj[0] == 'sendframe':
                ...
                self.sendFrame(opcode=obj[1]['opcode'], payload=pl.encode('UTF-8'), fin=obj[1].get('fin', True), rsv=obj[1].get('rsv', 0), mask=obj[1].get('mask', None), payload_len=obj[1].get('payload_len', None), chopsize=obj[1].get('chopsize', None), sync=obj[1].get('sync', False))
            elif obj[0] == 'sendframes':
                frames = obj[1]
                for frame in frames:
                    pl = frame.get('payload', '')
                    self.sendFrame(opcode=frame['opcode'], payload=pl.encode('UTF-8'), fin=frame.get('fin', True), rsv=frame.get('rsv', 0), mask=frame.get('mask', None), payload_len=frame.get('payload_len', None), chopsize=frame.get('chopsize', None), sync=frame.get('sync', False))
   ...
   """
    ...
    def logCase(self, caseResults):
      ...
      """
        ...
        if (
         agent, case) in self.resultListeners:
      ...
      """
        ...
        if case['behavior'] == Case.OK:
            ...
            if case['behavior'] == Case.NON_STRICT:
                ...
                if case['behavior'] == Case.INFORMATIONAL:
                    style = 'case_info'
                    text = 'Informational'
                else:
                    style = 'case_failed'
            ...
            for t in wl:
                ...
                if t[0] in ('RO', 'TO', 'RF', 'TF'):
                    ...
                    if t[0] in ('RO', 'TO'):
                        ...
                        for ll in lines:
                            f.write('         <pre class="%s">%s%s</pre>\n' % (css_class, (6 + len(prefix)) * ' ', ll.encode('utf8')))
...
class FuzzingServerFactory(FuzzingFactory, WebSocketServerFactory):
    ...
    def __init__(self, spec, debug=False):
        ...
        self.logOctets = True
        self.logFrames = True
        self.setSessionParameters(url=spec['url'], protocols=spec.get('protocols', []), server='AutobahnTestSuite/%s-%s' % (autobahntestsuite.version, autobahn.version))
...
class FuzzingClientFactory(FuzzingFactory, WebSocketClientFactory):
    ...
    def nextServer(self):
        ...
        if self.currServer < len(self.spec['servers']):
            ...
            self.setSessionParameters(url=server['url'], origin=server.get('origin', None), protocols=server.get('protocols', []), useragent='AutobahnTestSuite/%s-%s' % (autobahntestsuite.version, autobahn.version))
            self.resetProtocolOptions()
            self.setProtocolOptions(failByDrop=False)
            self.setProtocolOptions(**self.spec.get('options', {}))
