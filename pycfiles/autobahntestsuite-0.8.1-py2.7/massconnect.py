...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/autobahntestsuite/massconnect.py
...
class MassConnectFactory(WebSocketClientFactory):
    ...
    def clientConnectionFailed(self, connector, reason):
        if self.test.onFailed():
            reactor.callLater(float(self.retrydelay) / 1000.0, connector.connect)
...
class MassConnect:
    ...
    def onConnected(self):
        ...
        if self.actual == self.targetCnt:
            ...
            result = {'name': self.name, 'uri': self.uri, 
    ...
    def connectBunch(self):
        ...
        if redo:
            reactor.callLater(float(self.batchdelay) / 1000.0, self.connectBunch)
...
class MassConnectTest:
    ...
    def run(self):
        ...
        for s in self.spec['servers']:
            t = MassConnect(s['name'], s['uri'], self.spec['options']['connections'], self.spec['options']['batchsize'], self.spec['options']['batchdelay'], self.spec['options']['retrydelay'])
            r = yield t.run()
            res.append(r)
