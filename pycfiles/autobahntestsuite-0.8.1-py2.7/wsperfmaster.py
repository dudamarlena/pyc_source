...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/autobahntestsuite/wsperfmaster.py
...
from autobahn.websocket.http import HttpException, BAD_REQUEST
...
class WsPerfMasterProtocol(WebSocketServerProtocol):
    ...
    def runCase(self, workerRunId, caseDef):
        test = {'uri': caseDef['uri'].encode('utf8'), 'name': 'foobar', 
           'count': caseDef['count'], 
        ...
        if not binary:
            if msg is not None:
                try:
                    ...
                    elif o['type'] == 'test_complete':
                        workerRunId = o['token']
                        self.factory.caseCompleted(workerRunId)
                    elif o['type'] == 'test_welcome':
...
class WsPerfMasterFactory(WebSocketServerFactory):
    ...
    def runCase(self, caseDef):
      ...
      """
        ...
        for proto in self.protoToSlaves:
            ...
            self.runs[runId][workerRunId] = {'slaveId': slaveId, 'results': []}
    ...
    def caseResult(self, workerRunId, result):
        ...
        self.uiFactory.caseResult(runId, run['slaveId'], run['workerId'], result)
...
class WsPerfMasterUiFactory(WampServerFactory):
    ...
    def slaveConnected(self, id, host, port, version, num_workers, ident):
        self._dispatchEvent(URI_EVENT + 'slaveConnected', {'id': id, 'host': host, 
           'port': port, 
           'version': version, 
    ...
    def caseResult(self, runId, slaveId, workerId, result):
        event = {'runId': runId, 'slaveId': slaveId, 
           'workerId': workerId, 
           'result': result}
