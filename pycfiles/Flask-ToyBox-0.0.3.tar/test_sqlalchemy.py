...
# [GCC 8.4.0]
# Embedded file name: /home/drdaeman/Development/flask-toybox/tests/test_sqlalchemy.py
# Compiled at: 2012-12-09 15:05:53
import unittest
...
class User(Base, SAModelMixin):
    ...
    def __repr__(self):
        return ('<{0}: {1}, {2}>').format(self.__class__.__name__, self.username, self.fullname)
...
class SQLAlchemyModelTestCase(unittest.TestCase):
    ...
    def test_get_collection(self):
        ...
        self.assertEqual(response.status_code, 200, response.status)
        reference = [{'username': 'spam', 'fullname': 'Spam'}, {'username': 'ham', 'fullname': 'Ham'}, {'username': 'eggs', 'fullname': 'Eggs'}]
        data = json.loads(response.data)
    ...
    def test_patch(self):
        response = self.app.get('/users/eggs', headers={'Accept': 'application/json'})
        ...
        etag = response.headers.get('ETag', None)
        self.assertTrue(etag is not None)
        response = self.app.patch('/users/eggs', headers={'Accept': 'application/json'}, data=json.dumps({'fullname': 'Python Eggs'}), content_type='application/json')
        self.assertEqual(response.status_code, 428, response.status)
        response = self.app.patch('/users/eggs', headers={'Accept': 'application/json', 
           'If-Match': etag}, data=json.dumps({'fullname': 'Python Eggs'}), content_type='application/json')
    ...
    def test_patch_non_writeable(self):
        ...
        self.assertTrue(etag is not None)
        response = self.app.patch('/users/eggs', headers={'Accept': 'application/json', 
           'If-Match': etag}, data=json.dumps({'username': 'eggs2'}), content_type='application/json')
