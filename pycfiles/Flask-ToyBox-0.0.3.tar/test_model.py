...
# [GCC 8.4.0]
# Embedded file name: /home/drdaeman/Development/flask-toybox/tests/test_model.py
# Compiled at: 2012-12-09 11:44:34
import unittest
...
class SimpleModelTestCase(unittest.TestCase):
    ...
    def test_patch(self):
        ...
        etag = response.headers.get('ETag', None)
        self.assertTrue(etag is not None)
        response = self.app.patch('/test', headers={'Accept': 'application/json', 
           'If-Match': etag}, data=json.dumps({'spam': 'Spam Spam Spam'}), content_type='application/json')
    ...
    def test_base_is_readonly(self):
        for method in (self.app.post, self.app.patch, self.app.put, self.app.delete):
            ...
            self.assertEqual(response.status_code, 405, ('BaseModelView allowed {0}').format(method.__name__.upper()))