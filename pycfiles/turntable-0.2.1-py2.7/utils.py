...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/turntable/utils.py
# Compiled at: 2015-06-01 14:30:51
"""The utils module provides a collection of methods used across the package or of general utility.

"""
import os, re, sys, shutil, errno, fnmatch, numpy as np, pandas as pd
try:
...
class Timer:
    ...
    def loop(self):
        ...
        if self.verbose:
            displayAll(self.elapsed, self.display_amt, self.est_end, self.nLoops, self.count, self.numPrints)
...
def displayAll(elapsed, display_amt, est_end, nLoops, count, numPrints):
    ...
    if count % display_amt == 0:
        ...
        disp_elapsed, disp_avg, disp_est = timeUnit(int(round(elapsed)), int(round(avg)), int(round(est_end)))
