...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/turntable/press.py
# Compiled at: 2015-06-11 21:07:58
"""The press module is used to create Record Collections.

"""
import shutil, sys, os, pandas as pd, turntable.utils, traceback, turntable
...
class RecordPress(object):
    ...
    def __getattribute__(*args):
        ignore_args_list = [
         '__dict__', '__members__', '__methods__',
         '__class__', 'trait_names', '_getAttributeNames']
        if any([ attr in args[1] for attr in [
         'pickle', 'class_path', 'pickles'] + ignore_args_list
               ]):
            ...
            if self.pickle & (name in self.pickles):
                return turntable.utils.from_pickle(filename=self.class_path + name + '.pkl', clean_disk=False)
            return object.__getattribute__(*args)
    ...
    def __setattr__(self, name, value):
        ...
        if self.pickle:
            if any([ attr in name for attr in ['pickle', 'class_path', 'pickles'] ]) == False:
                return_value = turntable.utils.to_pickle(obj=value, filename=self.class_path + name + '.pkl', clean_memory=True)
    ...
    def __setattr__(self, name, value):
        ...
        if any([ attr in name for attr in blacklisted_keys ]) == False:
            if 'series' in self.__dict__.keys():
                ...
                self.series = pd.Series(dict([ (key, value) for key, value in self.__dict__.items() if key not in blacklisted_keys ]))
