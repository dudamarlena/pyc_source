...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/libmetric/cli.py
# Compiled at: 2017-11-20 07:48:53
import click
from libmetric.query_range import InfluxRangeQuery, GraphiteRangeQuery, PrometheusRangeQuery, RrdRangeQuery
from libmetric.query_instant import InfluxInstantQuery, GraphiteInstantQuery, PrometheusInstantQuery, RrdInstantQuery
...
@click.command()
@click.option('--engine', default='prometheus', help='Engine [prometheus, elasticsearch, influxdb]')
...
def _range_metric(engine, url, user, password, partition, query, start, end, step):
    data = {'queries': [
                 query], 
       'url': url, 
       'user': user, 
...
@click.option('--end', default=None, help='Time range end')
@click.option('--step', default=None, help='Query resolution step width')
@click.option('--alarm-threshold', default=None, help='Threshold value for the alarm')
@click.option('--alarm-operator', default=None, help='Arithmetic operator for alarm evaluation')
@click.option('--alarm-series', default=None, help='Series to perform alarm against')
@click.option('--aggregation', default=None, help='Aggregation function for the given time-series')
def _range_alarm(engine, url, user, password, partition, query, start, end, step, alarm_operator, alarm_threshold, alarm_series, aggregation):
    data = {'queries': [
                 query], 
       'url': url, 
       'user': user, 
    ...
    alarm = {'alarm_operator': alarm_operator, 
...
@click.option('--alarm-operator', default=None, help='Arithmetic operator for alarm evaluation')
@click.option('--alarm-series', default=None, help='Series to perform alarm against')
def _instant_alarm(engine, url, user, password, partition, query, moment, alarm_operator, alarm_threshold, alarm_series):
    data = {'queries': [
                 query], 
       'url': url, 
       'user': user, 
...
@click.option('--start', default=None, help='Time range start')
@click.option('--end', default=None, help='Time range end')
def _search_metrics(engine, url, user, password, partition, search, start, end):
    data = {'search': [
                search], 
       'url': url, 
       'user': user, 
