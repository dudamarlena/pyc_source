...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/test_encode.py
# Compiled at: 2010-12-03 00:35:48
from unittest import TestCase
import mimetypes, poster.encode, StringIO, sys
...
class TestEncode_String(TestCase):
    ...
    def test_simple(self):
        ...
        self.assertEqual(expected, poster.encode.encode_string('XXXXXXXXX', 'foo', 'bar'))

    def test_quote_name_space(self):
        expected = unix2dos('--XXXXXXXXX\nContent-Disposition: form-data; name="foo baz"\nContent-Type: text/plain; charset=utf-8\n\nbar\n')
        self.assertEqual(expected, poster.encode.encode_string('XXXXXXXXX', 'foo baz', 'bar'))

    def test_quote_name_phparray(self):
        expected = unix2dos('--XXXXXXXXX\nContent-Disposition: form-data; name="files[]"\nContent-Type: text/plain; charset=utf-8\n\nbar\n')
        self.assertEqual(expected, poster.encode.encode_string('XXXXXXXXX', 'files[]', 'bar'))

    def test_quote_unicode_name(self):
        expected = unix2dos('--XXXXXXXXX\nContent-Disposition: form-data; name="=?utf-8?b?4piD?="\nContent-Type: text/plain; charset=utf-8\n\nbar\n')
        self.assertEqual(expected, poster.encode.encode_string('XXXXXXXXX', '☃', 'bar'))

    def test_quote_value(self):
        expected = unix2dos('--XXXXXXXXX\nContent-Disposition: form-data; name="foo"\nContent-Type: text/plain; charset=utf-8\n\nbar baz@bat\n')
        self.assertEqual(expected, poster.encode.encode_string('XXXXXXXXX', 'foo', 'bar baz@bat'))

    def test_boundary(self):
        expected = unix2dos('--ABC+DEF\nContent-Disposition: form-data; name="foo"\nContent-Type: text/plain; charset=utf-8\n\nbar\n')
        self.assertEqual(expected, poster.encode.encode_string('ABC DEF', 'foo', 'bar'))

    def test_unicode(self):
        expected = unix2dos('--XXXXXXXXX\nContent-Disposition: form-data; name="foo"\nContent-Type: text/plain; charset=utf-8\n\nbár\n')
        self.assertEqual(expected, poster.encode.encode_string('XXXXXXXXX', 'foo', 'bár'))
...
class TestEncode_File(TestCase):
    ...
    def test_simple(self):
        expected = unix2dos('--XXXXXXXXX\nContent-Disposition: form-data; name="foo"\nContent-Type: text/plain; charset=utf-8\n\n')
        self.assertEqual(expected, poster.encode.encode_file_header('XXXXXXXXX', 'foo', 42))

    def test_content_type(self):
        expected = unix2dos('--XXXXXXXXX\nContent-Disposition: form-data; name="foo"\nContent-Type: text/html\n\n')
        self.assertEqual(expected, poster.encode.encode_file_header('XXXXXXXXX', 'foo', 42, filetype='text/html'))

    def test_filename_simple(self):
        expected = unix2dos('--XXXXXXXXX\nContent-Disposition: form-data; name="foo"; filename="test.txt"\nContent-Type: text/plain; charset=utf-8\n\n')
        self.assertEqual(expected, poster.encode.encode_file_header('XXXXXXXXX', 'foo', 42, 'test.txt'))

    def test_quote_filename(self):
        expected = unix2dos('--XXXXXXXXX\nContent-Disposition: form-data; name="foo"; filename="test file.txt"\nContent-Type: text/plain; charset=utf-8\n\n')
        self.assertEqual(expected, poster.encode.encode_file_header('XXXXXXXXX', 'foo', 42, 'test file.txt'))
        expected = unix2dos('--XXXXXXXXX\nContent-Disposition: form-data; name="foo"; filename="test\\"file.txt"\nContent-Type: text/plain; charset=utf-8\n\n')
        self.assertEqual(expected, poster.encode.encode_file_header('XXXXXXXXX', 'foo', 42, 'test"file.txt'))

    def test_unicode_filename(self):
        expected = unix2dos('--XXXXXXXXX\nContent-Disposition: form-data; name="foo"; filename="&#9731;.txt"\nContent-Type: text/plain; charset=utf-8\n\n')
        self.assertEqual(expected, poster.encode.encode_file_header('XXXXXXXXX', 'foo', 42, '☃.txt'))
...
class TestEncodeAndQuote(TestCase):
    ...
    def test(self):
        ...
        self.assertEqual('%28%C2%A9%29+2008', poster.encode.encode_and_quote('(©) 2008'))
...
class TestMultiparam(TestCase):
    ...
    def test_from_params(self):
        ...
        expected = [poster.encode.MultipartParam('foo', 'bar'), poster.encode.MultipartParam('baz', fileobj=fp, filename=fp.name, filetype=mimetypes.guess_type(fp.name)[0])]
        self.assertEqual(poster.encode.MultipartParam.from_params([('foo', 'bar'), ('baz', fp)]), expected)
        self.assertEqual(poster.encode.MultipartParam.from_params((('foo', 'bar'), ('baz', fp))), expected)
        self.assertEqual(poster.encode.MultipartParam.from_params({'foo': 'bar', 'baz': fp}), expected)
        self.assertEqual(poster.encode.MultipartParam.from_params([expected[0], expected[1]]), expected)

    def test_from_params_dict(self):
        p = poster.encode.MultipartParam('file', fileobj=open('tests/test_encode.py'))
        params = {'foo': 'bar', 'file': p}
        expected = [
         poster.encode.MultipartParam('foo', 'bar'), p]
    ...
    def test_from_params_assertion(self):
        ...
        self.assertRaises(AssertionError, poster.encode.MultipartParam.from_params, params)
    ...
    def test_simple(self):
        ...
        self.assertEqual(poster.encode.get_body_size([p], boundary), len(expected))
        self.assertEqual(poster.encode.get_headers([p], boundary), {'Content-Length': str(len(expected)), 'Content-Type': 'multipart/form-data; boundary=%s' % boundary})
        (datagen, headers) = poster.encode.multipart_encode([p], boundary)
        self.assertEqual(headers, {'Content-Length': str(len(expected)), 'Content-Type': 'multipart/form-data; boundary=%s' % boundary})
        self.assertEqual(('').join(datagen), expected)

    def test_multiple_keys(self):
        params = poster.encode.MultipartParam.from_params([('key', 'value1'), ('key', 'value2')])
        boundary = 'XYZXYZXYZ'
    ...
    def test_MultipartParam_file_cb(self):
        ...
        p = poster.encode.MultipartParam('foo', fileobj=open('tests/test_encode.py'), cb=cb)
