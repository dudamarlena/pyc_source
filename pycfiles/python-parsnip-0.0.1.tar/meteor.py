...
# [GCC 8.4.0]
# Embedded file name: /Users/pastylegs/Dropbox/GitHub/python-parsnip/parsnip/operators/meteor.py
...
class Meteor(BaseWebtexter):
    ...
    def login(self):
        response = self.connection.send(self.LOGIN_POST, data={'username': self.phone_number, 
           'userpass': self.pin, 
           'x': '19', 
           'y': '13', 
           'returnTo': '/'})
    ...
    def _do_send(self, message_chunk, recipients_chunk):
        ...
        for r in recipients_chunk:
            response = self.connection.send('%s?event=smsAjax&func=addEnteredMsisdns' % self.SEND_POST, data={'ajaxRequest': 'addEnteredMSISDNs', 
               'remove': '-', 
               'add': '0|%s' % r})
            if 'appendRecipients(' not in response:
                raise exceptions.MessageSendingError("Couldn't add recipient", webtexter=self)

        response = self.connection.send('%s?event=smsAjax&func=sendSMS' % self.SEND_POST, data={'ajaxRequest': 'sendSMS', 
