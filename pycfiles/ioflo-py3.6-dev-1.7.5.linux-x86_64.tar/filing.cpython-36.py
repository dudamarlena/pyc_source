...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.6/site-packages/ioflo/aid/filing.py
...
def dump(data, filepath):
    ...
    """
    if ' ' in filepath:
        raise IOError("Invalid filepath '{0}' contains space".format(filepath))
    else:
        ...
        if ext == '.json':
            ...
            if not msgpack:
                raise IOError("Invalid filepath ext '{0}' needs msgpack installed".format(filepath))
            with ocfn(filepath, 'w+b', binary=True) as (f):
                msgpack.dump(data, f)
                f.flush()
                os.fsync(f.fileno())
        else:
            raise IOError("Invalid filepath ext '{0}' not '.json' or '.msgpack'".format(filepath))
