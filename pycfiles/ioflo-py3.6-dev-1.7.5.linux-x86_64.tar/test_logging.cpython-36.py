...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.6/site-packages/ioflo/base/test/test_logging.py
# Compiled at: 2017-12-17 08:35:26
# Size of source mod 2**32: 61510 bytes
"""
Unit Test Template
"""
import sys
...
class LoggerTestCase(testing.LoggerIofloTestCase):
    ...
    def testLogger(self):
        ...
        self.assertTrue(self.logger.runner)
        ...
        log = logging.Log(name='test', store=(self.store),
          kind='text',
          baseFilename='',
        ...
        log.prepare()
        self.assertEqual(log.formats, {'_time':'%s',  'heading':odict([('value', '\t%s')]), 
    ...
    def testLogAlways(self):
        ...
        status = self.logger.runner.send(globaling.START)
        self.assertTrue(self.logger.path.startswith('/tmp/log/ioflo/HouseTest/LoggerTest_'))
        self.assertTrue(log.path.startswith(self.logger.path))
        self.assertTrue(log.path.endswith(log.baseFilename + '.txt'))
        self.assertTrue(log.file)
        ...
        self.store.advanceStamp(0.125)
        status = self.logger.runner.send(globaling.STOP)
        log.reopen()
        log.file.seek(0)
        line = log.file.readline()
        self.assertEqual(line, 'text\tAlways\ttest\n')
    ...
    def testLogUpdateFields(self):
        ...
        self.assertIs(log.stamp, None)
        status = self.logger.runner.send(globaling.START)
        self.assertEqual(log.formats, {'_time':'%s',  'ned':odict([('north', '\t%s'), ('east', '\t%s')])})
        self.store.advanceStamp(0.125)
        status = self.logger.runner.send(globaling.RUN)
        self.store.advanceStamp(0.125)
        self.assertEqual(self.store.stamp, 0.25)
        ned.update(north=0.0)
        status = self.logger.runner.send(globaling.RUN)
        self.store.advanceStamp(0.125)
        ned.update(down=0.0)
        status = self.logger.runner.send(globaling.RUN)
    ...
    def testLogDeck(self):
        ...
        self.store.advanceStamp(0.125)
        self.assertEqual(self.store.stamp, 0.25)
        ned.push(odict(north=9.0, east=6.0, down=5.0))
        status = self.logger.runner.send(globaling.RUN)
        self.store.advanceStamp(0.125)
        status = self.logger.runner.send(globaling.RUN)
        self.store.advanceStamp(0.125)
        self.assertEqual(self.store.stamp, 0.5)
        ned.push(odict(north=10.0, east=7.0, down=6.0))
        status = self.logger.runner.send(globaling.RUN)
        self.store.advanceStamp(0.125)
        status = self.logger.runner.send(globaling.STOP)
        log.reopen()
...
class HouseTestCase(testing.HouseIofloTestCase):
    ...
    def testSetup(self):
        ...
        cycle = 0.0
        keep = 0
        logger = logging.Logger(name='LoggerTest', store=(self.store),
        ...
        status = logger.runner.send(globaling.START)
    ...
    def testCycle(self):
        ...
        self.store.advanceStamp(0.125)
        heading.value += 1.0
        status = logger.runner.send(globaling.STOP)
