...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.6/site-packages/ioflo/base/test/_testmeta.py
...
def Run(fileName=None, period=0.2, real=False, verbose=0, profiling=False):
    """Run once """
    import ioflo.base.skedding as skedding
    if not fileName:
        fileName = '../../app/plan/meta.flo'
    print('Building ...')
    skedder = tasking.Skedder(name='TestTasker', period=period,
      real=real,
      filepath=fileName)
    if skedder.build():
...
if __name__ == '__main__':
    ...
    try:
        opts, pargs = getopt.getopt(sys.argv[1:], 'hrv:f:p:')
