...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.6/site-packages/ioflo/base/test/_testArbiting.py
...
def Test():
    ...
    arbiter = ArbiterSwitch(name='Switch', store=store, output=output, group=group,
      inputs=inputs)
    arbiter._expose()
    store.expose()
    arbiter.update()
    ...
    arbiter = ArbiterPriority(name='Priority', store=store, output=output, group=group,
      inputs=inputs)
    arbiter._expose()
    store.expose()
    arbiter.update()
    ...
    arbiter = ArbiterTrusted(name='Trusted', store=store, output=output, group=group,
      inputs=inputs)
    arbiter._expose()
    store.expose()
    arbiter.update()
    ...
    arbiter = ArbiterWeighted(name='Weighted', store=store, output=output, group=group,
      inputs=inputs)
    arbiter._expose()
    store.expose()
    arbiter.update()
