...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.6/site-packages/ioflo/aid/classing.py
...
class NonStringIterable:
    __doc__ = ' Allows isinstance check for iterable that is not a string\n    '

    @classmethod
    def __subclasshook__(cls, C):
        if cls is NonStringIterable:
            if not issubclass(C, basestring):
                if issubclass(C, Iterable):
                    return True
...
class NonStringSequence:
    __doc__ = ' Allows isinstance check for sequence that is not a string\n    '
...
if sys.version_info >= (3, 6):
    ...
    def attributize(genfunc):
        ...
        def wrapper(*args, **kwargs):
            ...
            tdict = {'__iter__':__iter__, 
             'send':send,  'throw':throw}
            ...
            if fargs:
                ...
                gen = genfunc(ag, *args, **kwargs)
            for attr in ('__next__', 'close', 'send', 'throw', 'gi_code', 'gi_frame',
