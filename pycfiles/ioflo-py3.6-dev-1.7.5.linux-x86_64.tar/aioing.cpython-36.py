...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.6/site-packages/ioflo/aio/aioing.py
...
def normalizeHost(host):
    ...
    if host == '':
        host = '0.0.0.0'
    try:
        info = socket.getaddrinfo(host, None, socket.AF_INET, socket.SOCK_DGRAM, socket.IPPROTO_IP, 0)
    except socket.gaierror as ex:
        if host in ('', '0.0.0.0'):
            host = '::'
        info = socket.getaddrinfo(host, None, socket.AF_INET6, socket.SOCK_DGRAM, socket.IPPROTO_IP, 0)
...
def arpCreate(ether, host, interface='en0', temp=True):
    ...
    console.terse('{0}: Creating {1} arp entry for {2} at {3} on {4}\n'.format(datetime.datetime.utcnow().isoformat(), temp, ether, host, interface))
    console.flush()
    try:
...
def arpDelete(host, interface='en0'):
    ...
    """
    console.terse('{0}: Deleting arp entry at {1} on {2}\n'.format(datetime.datetime.utcnow().isoformat(), host, interface))
    console.flush()
    try:
        process = subprocess.run(['sudo',
         '/usr/sbin/arp',
