...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.6/site-packages/ioflo/aid/test/test_eventing.py
# Compiled at: 2017-12-17 08:35:26
# Size of source mod 2**32: 7589 bytes
"""
Unit Test Template
"""
...
class BasicTestCase(unittest.TestCase):
    ...
    def testEventify(self):
        ...
        event = eventing.eventify(tag=eventing.tagify(head='exchange', tail='started'), stamp=stamp)
        self.assertEqual(event['tag'], 'exchange.started')
        self.assertEqual(event['stamp'], stamp)
        event = eventing.eventify(tag=eventing.tagify(tail='started', head='exchange'), stamp=stamp,
        ...
        self.assertEqual(event, {'tag':'process.started',  'stamp':'2015-08-10T19:26:47.194736', 
         'data':{'name': 'Jill'}})
        event = eventing.eventify(tag='with uid', stamp=stamp, uid='abcde')
        self.assertEqual(event, {'data':{},  'stamp':'2015-08-10T19:26:47.194736', 
         'tag':'with uid', 
         'uid':'abcde'})
        console.reinit(verbosity=(console.Wordage.concise))
    ...
    def testEventize(self):
        ...
        event = eventing.eventize(tag=eventing.tagify(head='exchange', tail='started'), stamp=True,
          uid=True,
          data=True,
        ...
        self.assertEqual(event['route'], odict([('src', (None, None, None)),
         ('dst', (None, None, None))]))
        event = eventing.eventize(tag=eventing.tagify(head='exchange', tail='started'), stamp=stamp,
          uid=tuid,
          data=odict(name='John'),
          route=(odict([('src', (None, None, None)),
         ('dst', (None, None, None))])))
