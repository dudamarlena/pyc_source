...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.6/site-packages/ioflo/trim/interior/plain/simulating.py
# Compiled at: 2017-12-17 08:35:26
# Size of source mod 2**32: 58563 bytes
...
class SimulatorMotionUuv(SimulatorBase):
    __doc__ = 'UUV motion simulator class\n    '
    Ioinits = odict(group='simulator.motion.uuv',
      speed='state.speed',
      speedRate='state.speedRate',
      velocity='state.velocity',
      depth='state.depth',
      depthRate='state.depthRate',
      pitch='state.pitch',
      pitchRate='state.pitchRate',
      altitude='state.altitude',
      heading='state.heading',
      headingRate='state.headingRate',
      ...
      parms=dict(rpmLimit=1200.0, sternLimit=20.0, rudderLimit=20.0, gs=0.0022,
      gpr=(-0.4),
      gpp=0.0,
      gdb=(-0.1),
      ghr=(-0.4)))
    ...
    def __init__(self, **kw):
        ...
        self.ionames = dict(speed=None, speedRate=None, bottom=None, pitch=None,
          pitchRate=None,
          depth=None,
          depthRate=None,
          altitude=None,
    ...
    def _prepio(self, group, speed, speedRate, velocity, depth, depthRate, pitch, pitchRate, altitude, heading, headingRate, position, location, rpm, stern, rudder, current, bottom, onset, origin, parms=None, **kw):
        ...
        if not parms:
            parms = dict(rpmLimit=1200.0, sternLimit=20.0, rudderLimit=20.0, gs=0.002,
              gpr=(-0.4),
              gpp=0.0,
              gdb=(-0.1),
              ghr=(-0.4))
    ...
    def action(self, **kw):
        ...
        headingLast = self.heading.value
        ...
        deltaNorth = horizontal * math.cos(DEGTORAD * headAvg) + self.current.data.north * self.lapse
        deltaEast = horizontal * math.sin(DEGTORAD * headAvg) + self.current.data.east * self.lapse
        self.velocity.update(north=(deltaNorth / self.lapse), east=(deltaEast / self.lapse))
...
class SimulatorMotionUsv(SimulatorBase):
    __doc__ = 'USV motion simulator class\n    '
    Ioinits = odict(group='simulator.motion.usv',
      speed='state.speed',
      speedRate='state.speedRate',
      velocity='state.velocity',
      heading='state.heading',
      ...
      current='scenario.current',
      onset='scenario.onset',
      parms=dict(rpmLimit=3000.0, rudderLimit=20.0, gs=0.0025,
      ghr=(-0.25)))

    def __init__(self, **kw):
        (super(SimulatorMotionUsv, self).__init__)(**kw)
        self.ionames = dict(speed=None, speedRate=None, heading=None,
          headingRate=None)

    def _prepio(self, group, speed, speedRate, velocity, heading, headingRate, position, rpm, rudder, current, onset, parms=None, **kw):
        """ Override since legacy interface
    ...
    def action(self, **kw):
        (super(SimulatorMotionUsv, self).action)(**kw)
        self.elapsed.value = self.lapse
        if self.lapse <= 0.0:
            return
        rpm = self.rpm.value
...
class SimulatorSensorGps(SimulatorBase):
    __doc__ = 'GPS sensor simulator class\n    '
    Ioinits = odict(group='simulator.sensor.gps',
      positionOut='gps.position',
      velocityOut='gps.velocity',
      error='gps.error',
      heading='heading.output',
      speed='state.speed',
      positionIn='state.position',
      velocityIn='state.velocity',
    ...
    def _prepio(self, group, positionOut, velocityOut, error, heading, speed, positionIn, velocityIn, scenario, parms=None, **kw):
        """ Override since legacy interface
    ...
    def action(self, **kw):
        (super(SimulatorSensorGps, self).action)(**kw)
        self.elapsed.value = self.lapse
        if self.lapse <= 0.0:
            return
        if self.scenario.data.dropout:
            return
        ...
        self.positionOut.update(north=(self.positionIn.data.north + en), east=(self.positionIn.data.east + ee))
        self.velocityOut.update(north=(self.velocityIn.data.north + evn), east=(self.velocityIn.data.east + eve))
...
class SimulatorSensorDvl(SimulatorBase):
    __doc__ = 'DVLSensorSimulator DeedLapse Deed Class\n       DVL sensor simulator class\n    '
    Ioinits = odict(group='simulator.sensor.dvl',
      velocity='dvl.velocity',
      currentOut='dvl.current',
    ...
    def _prepio(self, group, velocity, currentOut, altitude, heading, speed, currentIn, bottom, scenario, parms=None, **kw):
        """ Override since legacy interface
        ...
        self.currentIn.create(north=0.0)
    ...
    def action(self, **kw):
        (super(SimulatorSensorDvl, self).action)(**kw)
        self.elapsed.value = self.lapse
        if self.lapse <= 0.0:
            return
        if self.scenario.data.dropout:
...
class SimulatorSensorCompass(SimulatorBase):
    __doc__ = 'compass sensor simulator class\n    '
    Ioinits = odict(group='simulator.sensor.compass',
      output='compass',
      input='state.heading',
      depth='state.depth',
    ...
    def action(self, **kw):
        (super(SimulatorSensorCompass, self).action)(**kw)
        self.elapsed.value = self.lapse
        if self.lapse <= 0.0:
            return
        trueHeading = self.input.value
        ...
        heading = trueHeading - declination + error
        heading %= 360.0
        if heading < 0.0:
            heading += 360.0
...
class SimulatorSalinityLinear(SimulatorBase):
    __doc__ = 'linear salinity simulator class\n    '
    Ioinits = odict(group='simulator.salinity.linear',
      output='ctdsim',
      depth='state.depth',
      input='state.position',
      parms=dict(track=0.0, north=0.0, east=0.0, middle=32.0,
      spread=4.0,
      rising=True,
      width=500.0,
      layer=20.0,
    ...
    def _prepio(self, group, output, input, depth, parms=None, **kw):
        ...
        self.parm = self.store.create(group + '.parm')
        if not parms:
            parms = dict(track=15, north=0.0, east=0.0, middle=32.0, spread=3.0,
              rising=True,
              width=1000,
              layer=20.0,
              shift=2.0)
    ...
    def action(self, **kw):
        (super(SimulatorSalinityLinear, self).action)(**kw)
        self.elapsed.value = self.lapse
        pn = self.input.data.north
        pe = self.input.data.east
        track = self.parm.data.track
        north = self.parm.data.north
        east = self.parm.data.east
        middle = self.parm.data.middle
        ...
        d = max(-width / 2.0, min(width / 2.0, d))
        if rising:
            s = 1.0
        else:
            s = -1.0
...
class SimulatorSalinitySinusoid(SimulatorBase):
    __doc__ = 'salinity sensor simulator class\n    '
    Ioinits = odict(group='simulator.salinity.sinusoid',
      output='ctdsim',
      input='state.position',
      depth='state.depth',
      parms=dict(track=0.0, north=0.0, east=0.0, middle=32.0,
    ...
    def action(self, **kw):
        (super(SimulatorSalinitySinusoid, self).action)(**kw)
        self.elapsed.value = self.lapse
        pn = self.input.data.north
        pe = self.input.data.east
        track = self.parm.data.track
...
class SimulatorGradient(SimulatorBase):
    __doc__ = 'gradient simulator class\n    '
    Ioinits = odict(group='simulator.gradient.depth',
      output='ctdsim',
      field='depth',
      position='state.position',
      depth='state.depth',
    ...
    def action(self, **kw):
        (super(SimulatorGradient, self).action)(**kw)
        self.elapsed.value = self.lapse
        pn = self.position.data.north
        ...
        layer = self.parm.data.layer
        shift = self.parm.data.shift
        span = self.parm.data.span
        height = self.parm.data.height
        duct = self.parm.data.duct
        dh = navigating.DistancePointToTrack2D([east, north], track, [pe, pn])
        dv = self.depth.value - layer
        dh -= dv * shift
        dh = max(-width / 2.0, min(width / 2.0, dh))
        dv = max(-height / 2.0, min(height / 2.0, dv))
        if duct == 0:
            dv = abs(dv)
        else:
            if duct == -1:
...
SimulatorGradient.__register__('SimulatorGradientTemperature', ioinits=odict(group='simulator.gradient.temperature',
  output='ctdsim',
  field='temperature',
  position='state.position',
  depth='state.depth',
  ...
  height=20.0,
  duct=0)))
SimulatorGradient.__register__('SimulatorGradientSalinity', ioinits=odict(group='simulator.gradient.salinity',
  output='ctdsim',
  field='salinity',
  position='state.position',
  depth='state.depth',
