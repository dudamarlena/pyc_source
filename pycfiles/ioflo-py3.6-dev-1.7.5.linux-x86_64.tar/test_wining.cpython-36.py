...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.6/site-packages/ioflo/aio/win/test/test_wining.py
...
from ioflo.aio.win import wining
console = getConsole()
...
class BasicTestCase(unittest.TestCase):
    ...
    def testWinMailslotNb(self):
        """
        Test Class SocketUxdNb
        """
        console.terse('{0}\n'.format(self.testWinMailslotNb.__doc__))
        ...
        try:
            ...
            tempDirpath = tempfile.mkdtemp(prefix='test', suffix='wms', dir=userDirpath)
            sockDirpath = os.path.join(tempDirpath, 'wms')
            ...
            result = wireLog.reopen(prefix='alpha', midfix='wms')
            ha = os.path.join(sockDirpath, 'alpha.wms')
            alpha = wining.WinMailslotNb(ha=ha, wlog=wireLog)
            result = alpha.reopen()
            self.assertIs(result, True)
            self.assertEqual(alpha.ha, ha)
            ha = os.path.join(sockDirpath, 'beta.wms')
            beta = uxding.SocketUxdNb(ha=ha, umask=119)
            result = beta.reopen()
            self.assertIs(result, True)
            self.assertEqual(beta.ha, ha)
            ha = os.path.join(sockDirpath, 'gamma.wms')
            gamma = uxding.SocketUxdNb(ha=ha, umask=119)
            result = gamma.reopen()
            self.assertIs(result, True)
            ...
            pairs = [
             (
              alpha, beta), (alpha, gamma), (alpha, alpha),
             (
