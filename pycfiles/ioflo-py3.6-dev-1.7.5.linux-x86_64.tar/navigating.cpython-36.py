...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.6/site-packages/ioflo/aid/navigating.py
# Compiled at: 2017-12-17 08:35:26
# Size of source mod 2**32: 20755 bytes
"""
...
def MoveToHSD(north=0.0, east=0.0, heading=0.0, speed=1.0, duration=0.0):
    """
...
def AlongCrossTrack(track=0.0, north=0.0, east=0.0, mag=None, heading=None):
    """
...
def CrabSpeed(track=0.0, speed=2.0, north=0.0, east=0.0, mag=None, heading=None):
    """
    ...
    crab = track + delta
    B = speed * (math.sin(DEGTORAD * crab) * math.sin(DEGTORAD * track) + math.cos(DEGTORAD * crab) * math.cos(DEGTORAD * track))
...
def DistancePointToTrack2D(a, track, b):
    """Computes the signed distance between point b and  the track ray defined by
       ...
       left right defined as shortest angle (< 180)
    """
    dx = math.sin(DEGTORAD * track)
    dy = math.cos(DEGTORAD * track)
...
def HumanLLToFracDeg(hdm):
    """Converts  a coordinate  in human friendly string of form   DegXMin to
       ...
       Does not handle wrapping over poles or past halfway round
    """
    dm = REO_LatLonNE.findall(hdm)
    if dm:
        deg = float(dm[0][0])
        min_ = float(dm[0][1])
        return deg + min_ / 60.0
    dm = REO_LatLonSW.findall(hdm)
    if dm:
        deg = float(dm[0][0])
        min_ = float(dm[0][1])
        return -(deg + min_ / 60.0)
...
def Midpoint(latDM0, lonDM0, latDM1, lonDM1):
    ...
    dn = dn / 2.0
    de = de / 2.0
    lat1, lon1 = SphereLLByDNDEToLL(lat0, lon0, dn, de)
