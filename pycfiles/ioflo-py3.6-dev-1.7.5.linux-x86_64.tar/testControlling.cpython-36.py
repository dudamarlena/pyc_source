...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.6/site-packages/ioflo/trim/interior/plain/test/testControlling.py
...
def TestPID():
    ...
    controller = ControllerPid(name='controllerPIDTest', store=store, group='controller.pid.test',
      output='goal.testoutput',
...
def TestMotion():
    ...
    controller = MotionController(name='controllerMotionTest', store=store, group='controller.motion.test',
      speed='state.speed',
      ...
      parms=dict(rpmLimit=1000.0, sternLimit=20.0, rudderLimit=20.0, gs=0.0025,
      gpr=(-0.5),
      ghr=(-0.5)))
    store.expose()
