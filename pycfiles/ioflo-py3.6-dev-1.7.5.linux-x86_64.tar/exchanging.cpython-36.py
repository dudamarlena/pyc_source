...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.6/site-packages/ioflo/aio/proto/exchanging.py
# Compiled at: 2017-12-17 08:35:26
# Size of source mod 2**32: 12349 bytes
"""
...
class Exchange(MixIn):
    ...
    def __init__(self, stack, uid=None, name=None, device=None, timeout=None, redoTimout=None, tx=None, rx=None):
        """
    ...
    def process(self):
        ...
        if self.timeout > 0.0:
            if self.timer.expired:
                console.verbose('{0}. Timed out with {1} at {2}\n'.format(self.stack.name, self.device.name, round(self.stack.stamper.stamp, 3)))
                self.fail()
                return
        if self.redoTimeout > 0.0:
            if self.redoTimer.expired:
                self.redoTimer.restart()
                console.verbose('{0}: Redoing {1} with {2} at {3}\n'.format(self.stack.name, self.name, self.device.name, round(self.stack.stamper.stamp, 3)))
    ...
    def finish(self):
        ...
        """
        self.prepFinish()
        console.verbose('{0}: Finished {1} with {2} as {3} at {4}\n'.format(self.stack.name, self.name, self.device.name, 'FAILURE' if self.failed else 'SUCCESS', round(self.stack.stamper.stamp, 3)))
...
class Exchangent(Exchange):
    ...
    def respond(self, rx=None):
        ...
        if rx is not None:
            self.rx = rx
        if self.rx is None:
            raise ValueError('{0}: Cannot respond {1} no .rxreceived.\n'.format(self.stack.name, self.name))
        self.finish()