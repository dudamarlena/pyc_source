...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.6/site-packages/ioflo/aio/proto/stacking.py
# Compiled at: 2017-12-17 08:35:26
# Size of source mod 2**32: 76539 bytes
"""
...
import enum, time, random, errno, socket
...
class Stack(MixIn):
    ...
    def __init__(self, stamper=None, version=None, puid=None, local=None, uid=None, name=None, ha=None, kind=None, handler=None, rxbs=None, rxPkts=None, rxMsgs=None, txbs=None, txPkts=None, txMsgs=None, remotes=None, nameRemotes=None, haRemotes=None, stats=None, **kwa):
        """
        ...
        if getattr(self, 'aha', None) is None:
            self.aha = ha if ha is not None else ''
        if getattr(self, 'puid', None) is None:
            self.puid = puid if puid is not None else self.Uid
        self.local = local if local is not None else devicing.LocalDevice(stack=self, name=name,
          uid=uid,
          ha=ha,
          kind=kind)
        ...
        if self.handler:
            if not self.handler.reopen():
                raise ValueError("Stack '{0}': Failed opening handler at '{1}'\n".format(self.name, self.handler.ha))
            self.aha = self.handler.ha
            console.verbose("Stack '{0}': Opened handler at '{1}'\n".format(self.name, self.aha))
        self.stats = stats if stats is not None else odict()
    ...
    def _serviceOneTxPkt(self):
        ...
        console.profuse('{0}: sent\n    0x{1}\n'.format(self.name, hexlify(self.txbs[:count]).decode('ascii')))
    ...
    def _serviceOneReceived(self):
        ...
        if packet is not None:
            console.profuse('{0}: received\n    0x{1}\n'.format(self.name, hexlify(self.rxbs[:packet.size]).decode('ascii')))
...
class RemoteStack(Stack):
    ...
    def __init__(self, remotes=None, nameRemotes=None, haRemotes=None, **kwa):
        """
        Setup Stack instance
    ...
    def _serviceOneTxMsg(self):
        ...
        """
        msg, remote = self.txMsgs.popleft()
        console.verbose('{0} sending to {1}\n{2}\n'.format(self.name, remote.name, msg))
    ...
    def messagize(self, pkt, ha):
        ...
        try:
            ...
            console.verbose("{0}: Dropping packet received from unknown remote ha '{1}'.\n{2}\n".format(self.name, ha, pkt.packed))
            return (None, None)
...
class KeepStack(RemoteStack):
    ...
    def __init__(self, puid=None, clean=False, cleanlocal=False, cleanremote=False, keep=None, dirpath='', basedirpath='', local=None, name='', uid=None, ha=None, **kwa):
        """
        Setup Stack instance
        """
        if getattr(self, 'puid', None) is None:
            ...
            self.keep = keep or keeping.LotKeep(dirpath=dirpath, basedirpath=basedirpath,
              stackname=name)
            if clean or cleanlocal:
                self.clearLocalKeep()
            local = self.restoreLocal() or local or lotting.Lot(stack=self, name=name,
              uid=uid,
              ha=ha)
            local.stack = self
            (super(KeepStack, self).__init__)(puid=puid, local=local, **kwa)
    ...
    def restoreLocal(self):
        ...
        if data:
            if self.keep.verifyLocalData(data):
                local = lotting.Lot(stack=self, uid=(data['uid']),
                  name=(data['name']),
                  ha=(data['ha']),
                  sid=(data['sid']))
                self.local = local
    ...
    def restoreRemote(self, name):
        ...
        if data:
            if self.keep.verifyRemoteData(data):
                remote = lotting.Lot(stack=self, uid=(data['uid']),
                  name=(data['name']),
                  ha=(data['ha']),
                  sid=(data['sid']))
...
class IpStack(Stack):
    ...
    def __init__(self, puid=None, local=None, uid=None, name=None, ha=None, kind=None, host=None, port=None, **kwa):
        """
        Setup Stack instance
        ...
        if getattr(self, 'puid', None) is None:
            self.puid = puid if puid is not None else self.Uid
        local = local if local is not None else devicing.IpLocalDevice(stack=self, uid=uid,
          name=name,
          ha=ha,
          kind=kind)
...
class TcpServerStack(RemoteStack, IpStack):
    ...
    def __init__(self, host='', eha=None, bufsize=1048576, **kwa):
        """
        Setup Stack instance
    ...
    def createHandler(self, ha):
        ...
        """
        handler = serving.Server(ha=ha, eha=(self.eha),
          bufsize=(self.bufsize))
        self.eha = handler.eha
        return handler
    ...
    def _serviceOneTxPkt(self):
        ...
        try:
            ...
            console.profuse('{0}: Error sending to {1}\n{2}\n'.format(self.name, ca, ex))
            raise

        console.profuse('{0}: sent to {1}\n    0x{2}\n'.format(self.name, ca, hexlify(pkt.packed).decode('ascii')))
        return True
    ...
    def _serviceOneReceived(self, ix, ca):
        ...
        if not ix.rxbs:
            ...
            if packet is None:
                return False
            console.profuse('{0}: received\n    0x{1}\n'.format(self.name, hexlify(ix.rxbs[:packet.size]).decode('ascii')))
            del ix.rxbs[:packet.size]
            self.rxPkts.append((packet, ca))
            return True
...
class TcpClientStack(ClientStreamStack, IpStack):
    ...
    def __init__(self, puid=None, local=None, uid=None, name=None, ha=None, kind=None, host=None, port=None, bufsize=16192, timeout=None, remote=None, **kwa):
        """
        Setup Stack instance
        ...
        local = local if local is not None else devicing.IpLocalDevice(stack=self, uid=uid,
          name=name,
          kind=kind)
        remote = remote if remote is not None else devicing.IpSingleRemoteDevice(stack=self, ha=ha,
          uids=(
         local.uid,))
        (super(TcpClientStack, self).__init__)(local=local, remote=remote, ha=ha, **kwa)
        if getattr(self, 'remote', None) is None:
            self.remote = remote
        self.local.ha = self.handler.ca
    ...
    def createHandler(self, ha):
        ...
        """
        handler = clienting.Client(ha=ha, store=(self.stamper),
          bufsize=(self.bufsize),
          rxbs=(self.rxbs),
    ...
    def _serviceOneReceived(self):
        ...
        if not received:
            ...
            packet = self.parserize(self.rxbs[:])
            if packet is not None:
                console.profuse('{0}: received from {1}\n    0x{2}\n'.format(self.name, self.remote.ha, hexlify(self.rxbs[:packet.size]).decode('ascii')))
                del self.rxbs[:packet.size]
                self.rxPkts.append(packet)
            return True
    ...
    def serviceConnect(self):
        ...
        if self.handler.cutoff:
            if self.handler.reconnectable:
                if self.handler.timeout > 0.0:
                    if self.handler.timer.expired:
                        self.handler.reopen()
                        self.handler.refresh()
        else:
            self.handler.connected or self.handler.serviceConnect()
        if self.handler.connected:
            self.local.ha = self.handler.ca
...
class GramStack(Stack):
    ...
    def _serviceOneTxPkt(self, laters, blockeds):
        ...
        if ha in blockeds:
            ...
            try:
                count = self.handler.send(pkt.packed, ha)
            except socket.error as ex:
            ...
            console.profuse('{0}: sent to {1}\n    0x{2}\n'.format(self.name, ha, hexlify(pkt.packed).decode('ascii')))
            return True
    ...
    def _serviceOneReceived(self):
        ...
        if not raw:
            ...
            if packet is not None:
                console.profuse('{0}: received\n    0x{1}\n'.format(self.name, hexlify(raw).decode('ascii')))
                self.rxPkts.append((packet, ha))
            return True
...
class UdpStack(GramStack, RemoteStack, IpStack):
    ...
    def __init__(self, bufcnt=2, **kwa):
        """
        Setup Stack instance
    ...
    def createHandler(self, ha):
        ...
        Create local listening server for stack
        """
        handler = udping.SocketUdpNb(ha=ha, bufsize=(udping.UDP_MAX_PACKET_SIZE * self.bufcnt))
        return handler
