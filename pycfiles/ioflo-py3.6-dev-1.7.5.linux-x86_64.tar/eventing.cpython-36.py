...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.6/site-packages/ioflo/aid/eventing.py
...
def tagify(tail='', head='', sep='.'):
    ...
    if isinstance(head, (list, tuple, Set)):
        ...
        parts = [
         head]
    if isinstance(tail, (list, tuple, Set)):
        parts.extend(tail)
    else:
        parts.append(tail)
...
def eventify(tag, data=None, stamp=None, uid=None):
    ...
    event = odict([('tag', tag),
     (
      'stamp', stamp if stamp is not None else iso8601())])
    if uid is not None:
        event['uid'] = uid
