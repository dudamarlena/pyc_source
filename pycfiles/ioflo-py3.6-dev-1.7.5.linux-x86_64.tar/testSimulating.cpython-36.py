...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.6/site-packages/ioflo/trim/interior/plain/test/testSimulating.py
...
def TestSalinity():
    ...
    sim = SalinitySensorSimulator(name='simulatorSensorSalinity', store=store, group='simulator.sensor.salinity',
      output='ctd',
      input='state.position',
      parms=dict(track=45.0, north=0.0, east=0.0, middle=32.0,
      spread=4.0,
      rising=True,
      width=500.0))
...
def TestMotion():
    ...
    simulator = SimulatorMotionUuv(name='simulatorMotionTest', store=store, group='simulator.motion.test',
      speed='state.speed',
      ...
      parms=dict(rpmLimit=1500.0, sternLimit=20.0, rudderLimit=20.0, gs=0.0025,
      gpr=(-0.5),
      ghr=(-0.5)))
    store.expose()
