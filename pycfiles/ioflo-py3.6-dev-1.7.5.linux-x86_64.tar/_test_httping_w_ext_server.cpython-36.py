...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.6/site-packages/ioflo/aio/test/_test_httping_w_ext_server.py
# Compiled at: 2017-12-17 08:35:26
# Size of source mod 2**32: 23684 bytes
"""
...
class BasicTestCase(unittest.TestCase):
    ...
    def testNonBlockingRequestEcho(self):
        ...
        headers = odict([('Accept', 'application/json')])
        request = httping.Requester(hostname=host, port=port,
          method=method,
          path=path,
          headers=headers)
    ...
    def testNonBlockingRequestMultipart(self):
        ...
        self.assertEqual(response.data, {'action':None,  'content':None, 
         'form':[
          ...
          [
           'html', '<html><body></body><html>']], 
         'query':{},  'url':'http://127.0.0.1:8080/echo', 
         'verb':'POST'})
        self.assertEqual(len(beta.rxbs), 0)
        beta.close()
        wireLogBeta.close()
