...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.6/site-packages/ioflo/aid/optimizing.py
        ...
        try:
            ...
            while i < codelen:
                ...
                if not newtuple:
                    ...
                    newcode[i - reljump + 1] = reljump - 3 & 255
            ...
            codeobj = type(co)(co.co_argcount, co.co_nlocals, co.co_stacksize, co.co_flags, codestr, tuple(newconsts), co.co_names, co.co_varnames, co.co_filename, co.co_name, co.co_firstlineno, co.co_lnotab, co.co_freevars, co.co_cellvars)
    ...
    try:
        ...
        for k, v in d.items():
            if k in stoplist:
                ...
                if hasattr(v, '__do_not_optimize__'):
                    if v.__do_not_optimize__:
                        continue
            if type(v) is FunctionType:
                newv = _make_constants(v, builtin_only, stoplist, verbose)
                setattr(mc, k, newv)
            else:
                if type(v) in (type, ClassType):
