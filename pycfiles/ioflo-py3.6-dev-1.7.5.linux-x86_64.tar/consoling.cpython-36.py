...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.6/site-packages/ioflo/base/consoling.py
...
class Console(object):
            ...
            try:
                ...
                if not os.path.exists(dirpath):
                    try:
                        ...
                        msg = 'Error creating console file directory {0}. {1}\n'.format(dirpath, ex)
                        sys.stderr.write(msg)
                        return False

                self._file = Console.ocfn(self._path, 'a+')
    ...
    def close(self):
        ...
        if self._file:
            if not self._file.closed:
                if self._file.name not in ('<stdout>', '<stderr>'):
                    try:
                        self._file.close()
    ...
    def ocfn(filename, openMode='r+'):
        ...
        """
        try:
            newfd = os.open(filename, os.O_EXCL | os.O_CREAT | os.O_RDWR, 436)
