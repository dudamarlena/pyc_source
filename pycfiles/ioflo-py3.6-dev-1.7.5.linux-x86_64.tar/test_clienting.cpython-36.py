...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.6/site-packages/ioflo/aio/http/test/test_clienting.py
# Compiled at: 2017-12-17 08:35:26
# Size of source mod 2**32: 174466 bytes
"""
...
class BasicTestCase(unittest.TestCase):
    ...
    def testNonBlockingRequestEcho(self):
        ...
        request = clienting.Requester(hostname=host, port=port,
          method=method,
          path=path,
          headers=headers)
        ...
        response.dictify()
        self.assertEqual(bytes(response.body), bytearray(b'{"content": null, "query": {"name": "fame"}, "verb": "GET", "url": "http://127.0.0.1:8080/echo?name=fame", "action": null}'))
        self.assertEqual(response.data, {'action':None,  'content':None, 
         'query':{'name': 'fame'}, 
    ...
    def testNonBlockingRequestStreamFancyJson(self):
        ...
        port = 6061
        method = 'GET'
        path = '/fancy?idify=true&jsonify=true'
        console.terse('{0} from  {1}:{2}{3} ...\n'.format(method, host, port, path))
        headers = odict([('Accept', 'application/json')])
        request = clienting.Requester(hostname=host, port=port,
          method=method,
    ...
    def testNonBlockingRequestEchoTLS(self):
        ...
        alpha = tcp.ServerTls(host='localhost', port=6101,
          bufsize=131072,
          wlog=wireLogAlpha,
          context=None,
          version=None,
        ...
        beta = tcp.ClientTls(ha=(alpha.ha), bufsize=131072,
          wlog=wireLogBeta,
          context=None,
          version=None,
          certify=None,
    ...
    def testPatronRequestEcho(self):
        ...
        path = '/echo?name=fame'
        headers = odict([('Accept', 'application/json')])
        beta = clienting.Patron(bufsize=131072, wlog=wireLogBeta,
          hostname=host,
          port=port,
          method=method,
          path=path,
        ...
        while True:
            ...
            alpha.serviceConnects()
            if beta.connector.connected:
                if beta.connector.ca in alpha.ixes:
                    break
            time.sleep(0.05)
        ...
        self.assertEqual(bytes(beta.respondent.body), bytearray(b'{"content": null, "query": {"name": "fame"}, "verb": "GET", "url": "http://127.0.0.1:8080/echo?name=fame", "action": null}'))
        self.assertEqual(beta.respondent.data, {'action':None,  'content':None, 
         'query':{'name': 'fame'}, 
         'url':'http://127.0.0.1:8080/echo?name=fame', 
         'verb':'GET'})
    ...
    def testPatronPipelineEcho(self):
        ...
        console.terse('Beta requests to Alpha\n')
        console.terse('from {0}:{1}, {2} {3} ...\n'.format(beta.connector.ha[0], beta.connector.ha[1], request['method'], request['path']))
        ...
        self.assertEqual(response, {'version':(1, 1),  'status':200, 
         'reason':'OK', 
    ...
    def testPatronPipelineEchoSimple(self):
        ...
        self.assertEqual(response, clienting.Response(version=(1, 1),
          status=200,
          reason='OK',
    ...
    def testPatronEchoSimpleFirst(self):
        ...
        self.assertEqual(response, {'version':(1, 1),  'status':200, 
         ...
         'request':{'host':'127.0.0.1', 
          ...
          'data':None, 
          'fargs':None}})
        beta.request(method='GET', path='/echo?name=fame',
          headers=(odict([('Accept', 'application/json')])))
        while not alpha.ixes or beta.requests or beta.connector.txes or not beta.respondent.ended:
            self.mockEchoService(alpha)
            time.sleep(0.05)
    ...
    def testPatronPipelineEchoSimpleSecure(self):
        ...
        serverCafilepath = self.certdirpath + '/server.pem'
        serverCertCommonName = 'localhost'
        alpha = tcp.ServerTls(host=serverCertCommonName, port=6101,
          bufsize=131072,
          wlog=wireLogAlpha,
          context=None,
          version=None,
        ...
        host = alpha.eha[0]
        port = alpha.eha[1]
        beta = clienting.Patron(hostname=serverCertCommonName, port=(alpha.eha[1]),
          bufsize=131072,
          wlog=wireLogBeta,
          scheme='https',
          reconnectable=True,
    ...
    def testPatronPipelineEchoSimpleSecurePath(self):
        ...
        beta = clienting.Patron(path=path, bufsize=131072,
          wlog=wireLogBeta,
          reconnectable=True,
          keypath=clientKeypath,
          certpath=clientCertpath,
    ...
    def testPatronRedirectComplexSecure(self):
        ...
        wireLogGamma = wiring.WireLog(buffify=True, same=True)
        result = wireLogGamma.reopen()
        gamma = tcp.ServerTls(host=serverCertCommonName, port=6103,
          bufsize=131072,
          wlog=wireLogGamma,
          context=None,
          version=None,
    ...
    def testQueryQuoting(self):
        ...
        if sep:
            ...
            location = path
        self.assertEqual(location, 'https://api.twitter.com/1.1/account/verify_credentials.json?oauth_consumer_key=meWtb1jEOCQciCgqheqiQoU&oauth_nonce=eb616fe02004000&oauth_signature_method=HMAC-SHA1&oauth_timestamp=1437580412&oauth_token=1048104-WpGhCC4Fbj9Bp5PaTTuN0laSqD4vxCb2B7xh62YD&oauth_version=1.0&oauth_signature=KBD3DdNVZBjyOd0fqQ9X17ack%3D')
        splits = urlsplit(location)
        query = splits.query
        self.assertEqual(query, 'oauth_consumer_key=meWtb1jEOCQciCgqheqiQoU&oauth_nonce=eb616fe02004000&oauth_signature_method=HMAC-SHA1&oauth_timestamp=1437580412&oauth_token=1048104-WpGhCC4Fbj9Bp5PaTTuN0laSqD4vxCb2B7xh62YD&oauth_version=1.0&oauth_signature=KBD3DdNVZBjyOd0fqQ9X17ack%3D')
        querySplits = query.split('&')
        self.assertEqual(querySplits, ['oauth_consumer_key=meWtb1jEOCQciCgqheqiQoU',
        ...
        self.assertEqual(qargs, {'oauth_consumer_key':'meWtb1jEOCQciCgqheqiQoU',  'oauth_nonce':'eb616fe02004000', 
         'oauth_signature_method':'HMAC-SHA1', 
         'oauth_timestamp':'1437580412', 
         'oauth_token':'1048104-WpGhCC4Fbj9Bp5PaTTuN0laSqD4vxCb2B7xh62YD', 
         'oauth_version':'1.0', 
         'oauth_signature':'KBD3DdNVZBjyOd0fqQ9X17ack='})
        qargParts = ['{0}={1}'.format(key, quote_plus(str(val))) for key, val in qargs.items()]
        newQuery = '&'.join(qargParts)
        self.assertEqual(newQuery, 'oauth_consumer_key=meWtb1jEOCQciCgqheqiQoU&oauth_nonce=eb616fe02004000&oauth_signature_method=HMAC-SHA1&oauth_timestamp=1437580412&oauth_token=1048104-WpGhCC4Fbj9Bp5PaTTuN0laSqD4vxCb2B7xh62YD&oauth_version=1.0&oauth_signature=KBD3DdNVZBjyOd0fqQ9X17ack%3D')
