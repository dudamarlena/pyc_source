...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.6/site-packages/ioflo/aid/vectoring.py
...
from .sixing import *
from ..base.globaling import Pxy, Pxyz, Pne, Pned, Pfs, Pfsb
...
def add(u, v):
    ...
    """
    if len(u) != len(v):
        raise ValueError('Vectors lengths differ {} != {}'.format(len(u), len(v)))
...
def wind(p, vs):
    ...
    if p in vs:
        ...
        for i in range(l):
            j = (i + 1) % l
            if tween2(p, vs[i], vs[j]):
                return 0
            x, y = vs[i][:2]
            u, v = vs[j][:2]
            if y <= py:
                if v > py and right(sub(p, vs[i]), sub(vs[j], vs[i])):
                    w += 1
            else:
                if v <= py:
                    if left(sub(p, vs[i]), sub(vs[j], vs[i])):
                        w -= 1
