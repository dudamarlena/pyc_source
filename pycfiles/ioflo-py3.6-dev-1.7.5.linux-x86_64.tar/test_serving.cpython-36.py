...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python3.6/site-packages/ioflo/aio/http/test/test_serving.py
# Compiled at: 2017-12-17 08:35:26
# Size of source mod 2**32: 53565 bytes
"""
...
class BasicTestCase(unittest.TestCase):
    ...
    def testHttpError(self):
        ...
        self.assertEqual(error.headers, odict())
        body = error.render()
        self.assertEqual(body, b'400 Bad Request\n\n\n')
        body = error.render(jsonify=True)
        self.assertEqual(body, b'{\n  "status": 400,\n  "reason": "Bad Request",\n  "title": "",\n  "detail": "",\n  "fault": null\n}')
        error = httping.HTTPError(status=700, title='Validation Error',
          detail='Bad mojo',
          fault=50,
        ...
        self.assertEqual(error.headers, odict(Accept='application/json'))
        body = error.render()
    ...
    def testPorterServiceEcho(self):
        ...
        alpha = serving.Porter(port=6101, bufsize=131072,
          wlog=wireLogAlpha,
          store=store)
        ...
        request = odict([('method', 'GET'),
         ...
         (
          'headers',
          odict([('Accept', 'application/json'),
           ('Content-Length', 0)]))])
        beta.requests.append(request)
        while beta.requests or beta.connector.txes or not beta.responses or not alpha.servant.ixes or not alpha.idle():
            alpha.serviceAll()
            time.sleep(0.05)
            beta.serviceAll()
            time.sleep(0.05)
        ...
        self.assertEqual(requestant.headers, {'accept':'application/json',  'accept-encoding':'identity', 
         'content-length':'0', 
         'host':'localhost:6101'})
        self.assertEqual(len(beta.responses), 1)
        response = beta.responses.popleft()
        self.assertEqual(response['data'], {'body':'',  'data':None, 
         'fragment':'', 
    ...
    def testValetServiceBasic(self):
        ...
        wireLogAlpha = wiring.WireLog(buffify=True, same=True)
        result = wireLogAlpha.reopen()
        alpha = serving.Valet(port=6101, bufsize=131072,
          wlog=wireLogAlpha,
          store=store,
          app=wsgiApp)
        self.assertIs(alpha.servant.reopen(), True)
        ...
        request = odict([('method', 'GET'),
         ...
         (
          ...
          odict([('Accept', 'application/json'),
           ('Content-Length', 0)]))])
        beta.requests.append(request)
        while beta.requests or beta.connector.txes or not beta.responses or not alpha.idle():
            alpha.serviceAll()
            time.sleep(0.05)
            beta.serviceAll()
            time.sleep(0.05)
    ...
    def testValetServiceBottle(self):
        ...
        def echoGet(action=None):
            ...
            data = odict(verb=(bottle.request.method), url=(bottle.request.url),
              action=action,
              query=query,
              form=form,
              content=body)
        ...
        self.assertEqual(response['body'], bytearray(b'{"verb": "GET", "url": "http://localhost:6101/echo?name=fame", "action": null, "query": {"name": "fame"}, "form": {}, "content": null}'))
        self.assertEqual(response['data'], {'action':None,  'content':None, 
         'form':{},  'query':{'name': 'fame'}, 
    ...
    def testValetServiceBottleSecure(self):
        ...
        self.assertEqual(response['status'], 200)
        self.assertEqual(response['reason'], 'OK')
        self.assertEqual(response['body'], bytearray(b'{"verb": "GET", "url": "https://localhost:6101/echo?name=fame", "action": null, "query": {"name": "fame"}, "form": {}, "content": null}'))
        self.assertEqual(response['data'], {'action':None,  'content':None, 
         'form':{},  'query':{'name': 'fame'}, 
