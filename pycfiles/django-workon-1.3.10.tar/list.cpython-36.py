...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/site-packages/workon/views/list.py
# Compiled at: 2018-11-07 02:57:45
# Size of source mod 2**32: 16996 bytes
import datetime, workon.utils, workon.conf
...
__all__ = [
 'ListColumn', 'ListCol', 'ListFilter', 'ListAction', 'List', 'TableList']
...
class ListAction:
    ...
    def __init__(self, name, label=None, col=2):
        self.name = name
        self.meta = {'class':f"col s12 m{col}", 
         'label':label or name}


class ListFilter:

    def __init__(self, name, label=None, col=2, choices=None, classes='', multiple=False, default=None, field_class=None, field_kwargs=None, linebreaks=False):
        self.name = name
...
class List(generic.FormView):
    ...
    def __get_vomr(self, name):
        ...
        if workon.utils.is_lambda(value):
            ...
            if workon.utils.is_method(value):
                return value()
        ...
        if isinstance(fi, ListFilter):
            ...
            if isinstance(fi, dict):
                ...
                if isinstance(fi, tuple):
                    ...
                    if isinstance(fi, six.string_types):
    ...
    def get_form_kwargs(self):
        ...
        if self.request.method in 'GET':
            kwargs.update({'initial': {k.lstrip(f"{prefix}-"):v for k, v in self.request.GET.items()}})
    ...
    def get_row(self, obj):
        ...
        return {'object':obj,  'id':self._List__get_vomr_obj('row_id', obj), 
         'attrs':self._List__get_vomr_obj('row_attrs', obj), 
         'class':self._List__get_vomr_obj('row_class', obj), 
         'style':self._List__get_vomr_obj('row_style', obj), 
         'cells':cells, 
         'update_method':self._List__get_vomr_obj('row_update_method', obj), 
         'delete_method':self._List__get_vomr_obj('row_delete_method', obj), 
         'view_method':self._List__get_vomr_obj('row_view_method', obj), 
    ...
    def render_row(cls, request, obj):
        ...
        self.get_form()
        return workon.render_content(request, self._List__get_vomr('row_template_name'), {'row': self.get_row(obj)})
    ...
    def render_cell(self, column_instance, obj):
        ...
        if column_instance.value is not None:
            if workon.is_lambda(column_instance.value):
                ...
                if workon.is_method(column_instance.value):
                    value = column_instance.value(self, obj)
                else:
                    value = column_instance.value
        else:
            value = self._List__get_vomr_obj_value(name, obj)
        if value is None or value is '':
            value = '<i class="icon disabled">block</i>'
        else:
            if isinstance(value, bool):
                ...
                if isinstance(value, datetime.date):
                    ...
                    if isinstance(value, six.string_types):
                        value = value
        data['value'] = value
        classes = self._List__get_vomr_obj(f"{name}_class", obj)
        if classes:
            data['classes'] += classes
        attrs = self._List__get_vomr_obj(f"{name}_attrs", obj)
        if attrs:
            data['attrs'] += attrs
        return data
    ...
    def form_valid(self, form):
        ...
        if action:
            if hasattr(self, f"action_{action}"):
                ...
                self.queryset = response or self.get_queryset()
            else:
    ...
    def paginate(self):
        ...
        self.queryset = workon.utils.DiggPaginator((self.queryset),
          (self.paginate_by),
          body=(self.paginate_body),
          padding=(self.paginate_padding)).get_queryset_for_page(page)
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['create_method'] = self._List__get_vomr('create_method')
        ctx['create_url'] = self._List__get_vomr('create_url')
        ctx['list_results_template'] = self._List__get_vomr('results_template_name')
        ctx['list_row_template'] = self._List__get_vomr('row_template_name')
        ctx['floating_actions_template'] = self._List__get_vomr('floating_actions_template_name')
        ctx['layout_template'] = self._List__get_vomr('layout_template_name')
        ctx['list_id'] = self._List__get_vomr('list_id')
        ctx['list_class'] = self._List__get_vomr('list_class')
