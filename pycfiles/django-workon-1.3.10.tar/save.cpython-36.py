...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/site-packages/workon/views/save.py
# Compiled at: 2018-11-07 10:40:10
...
import workon.utils, workon.conf
__all__ = [
 'Save', 'ModalSave', 'JsonSave', 'SaveField']
...
class Save(generic.UpdateView):
    ...
    def get_form(self, *args, **kwargs):
        ...
        for field in self.form.fields.values():
            field.meta = {'class':'col s12',  'label':field.label}

        if self.fields:
            for field in self.fields:
                if isinstance(field, SaveField) and self.form.fields.get(field.name):
    ...
    def render_form(self, **kwargs):
        return workon.render_content(self.request, self.get_template_names(), (self.get_context_data)(form=self.form, **kwargs))

    def render_row(self, obj):
        return workon.render_content(self.request, self.result_template_name, {'row': {'object':obj, 
                 'id':f"{obj}_{obj.pk}"}})
...
class JsonSave(Save):
    ...
    def get_valid_json(self, obj, **kwargs):
        ...
        json = {'formAction':kwargs['save_url'], 
         'notice':{'content':success_message, 
          'classes':'success'}}
