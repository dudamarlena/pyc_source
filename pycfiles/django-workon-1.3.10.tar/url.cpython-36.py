...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/site-packages/workon/utils/url.py
# Compiled at: 2018-07-30 04:19:06
...
def urls_to_html(urls, reverse=True, target='_blank', hide_protocol=True, classname=None, divider='<br />'):
    if not urls:
        ...
        urls = ['<a %shref="%s" %s/>%s</a>' % ('target="%s" ' % target if target else '', url, 'class="%s" ' % classname if classname else '', (url.replace('https://', '').replace('http://', '') if hide_protocol else url).strip('/')) for url in urls]
        if reverse:
            urls.reverse()
...
def replace_urls_to_href(text, target='_blank', hide_protocol=True):
    if not text:
        ...
        text = _url_regex_multiline.sub('<a href="http://\\1" %s rel="nofollow">\\1</a>' % ('target="%s" ' % target if target else ''), text)
...
def build_absolute_url(url='', request=None):
    ...
    return '{0}://{1}{2}'.format(getattr(settings, 'DEFAULT_HTTP_PROTOCOL', 'http'), domain.split('//')[(-1)], url).strip('/')
...
def canonical_url(url, domain_check=False):
    ...
    if domain_check:
        ...
        if url_parts.subdomains()[-2:] != current_site_parts.subdomains()[-2:]:
            raise ValueError("Suspicious domain '%s' that differs from the current Site one '%s'" % (
             url_parts.domain(), current_site_parts.domain()))
    return url
...
def canonical_url_static(url, domain_check=False):
    """
...
def default_redirect(request, fallback_url, **kwargs):
    ...
    is_safe = functools.partial(ensure_safe_url,
