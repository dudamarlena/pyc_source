...
# [GCC 8.4.0]
# Embedded file name: /home/dalou/www/DJANGO-WORKON/workon/templates/template.py
# Compiled at: 2018-01-29 03:12:22
...
class Library(template.Library):
    ...
    def as_tag(self, tag_func):
        ...
        """
        package = tag_func.__module__.split(b'.')[0]
        if DJANGO_VERSION >= (1, 9) and package != b'mezzanine':
            warnings.warn(b"The `as_tag` template tag builder is deprecated in favour of Django's built-in `simple_tag`, which supports variable assignment in Django 1.9 and above.", DeprecationWarning, stacklevel=2)
    ...
    def to_end_tag(self, tag_func):
        ...
        def tag_wrapper(parser, token):
            ...
            class ToEndTagNode(template.Node):
                ...
                def __init__(self):
                    end_name = b'end%s' % tag_func.__name__
