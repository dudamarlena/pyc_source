...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/site-packages/workon/utils/email.py
# Compiled at: 2018-07-30 04:15:50
...
try:
    ...
    def premailer(s, *args, **kwargs):
        return s


_email_regex = re.compile("([a-z0-9!#$%&'*+\\/=?^_`{|}~-]+(?:\\.[a-z0-9!#$%&'*+\\/=?^_`{|}~-]+)*(@|\\sat\\s)(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?(\\.|\\sdot\\s))+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?)")
__all__ = [
 'EmailStagingBackend',
...
def emails_to_html(emails, reverse=True, classname=None, divider='<br />'):
    emails = ['<a href="mailto:%s" %s/>%s</a>' % (email, 'class="%s" ' % classname if classname else '', email.strip('/')) for email in emails]
    if reverse:
        emails.reverse()
...
def make_email(subject, sender, receivers, content=None, html=None, template=None, context={}, content_type=None, files=[], **kwargs):
 ...
 subject, html, sender, receivers, context, files=files, **kwargs)
        ...
        if html:
            ...
            if content:
                message = ContentEmail(subject, content, sender, receivers, content_type=content_type, 
                 files=files, **kwargs)
    return message
