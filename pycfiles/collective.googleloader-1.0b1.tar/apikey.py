...
# Embedded file name: /home/toutpt/workspace/collective.googlelibraries/collective/googlelibraries/apikey.py
...
class GoogleAPIKeyWidget(ASCIIWidget):
    ...
    def hasInput(self):
        return self.name + '.url' in self.request.form and self.name + '.key' in self.request.form
    ...
    def __call__(self):
        ...
        if len(value) == 2:
            value = (
             value[0].strip(), value[1].strip())
        else:
            value = ('', '')
        url = renderElement(self.tag, type=self.type, name=self.name + '.url', id=self.name + '.url', value=value[0], cssClass=self.cssClass, size=85, extra=self.extra)
        key = renderElement(self.tag, type=self.type, name=self.name + '.key', id=self.name + '.key', value=value[1], cssClass=self.cssClass, size=85, extra=self.extra)
        return '%s %s' % (url, key)
