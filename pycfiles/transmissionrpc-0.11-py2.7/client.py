...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/transmissionrpc/client.py
# Compiled at: 2013-08-31 14:40:16
import re, time, operator, warnings, os, base64, json
...
def debug_httperror(error):
    ...
    LOGGER.debug(json.dumps({'response': {'url': error.url, 
                    'code': error.code, 
                    'msg': error.message, 
...
class Client(object):
    ...
    def __init__(self, address='localhost', port=DEFAULT_PORT, user=None, password=None, http_handler=None, timeout=None):
        ...
        elif hasattr(http_handler, 'set_authentication') and hasattr(http_handler, 'request'):
    ...
    def rpc_version(self):
        ...
        if self.protocol_version is None:
            if self.server_version and self.server_version[0] == 2 and self.server_version[1] in (20,
                                                                                                  21,
                                                                                                  22):
    ...
    def add(self, data, timeout=None, **kwargs):
        ...
        if data:
            ...
            if 'metainfo' not in kwargs and 'filename' not in kwargs:
    ...
    def remove_torrent(self, ids, delete_data=False, timeout=None):
        ...
        self._request('torrent-remove', {'delete-local-data': rpc_bool(delete_data)}, ids, True, timeout=timeout)
    ...
    def get_files(self, ids=None, timeout=None):
        ...
        """
        fields = [
         'id', 'name', 'hashString', 'files', 'priorities', 'wanted']
