...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/transmissionrpc/utils.py
# Compiled at: 2013-03-05 20:21:51
...
def argument_value_convert(method, argument, value, rpc_version):
    ...
    if argument in args:
        ...
        while invalid_version:
            ...
            if invalid_version:
                if replacement:
                    LOGGER.warning('Replacing requested argument "%s" with "%s".' % (
                     argument, replacement))
                    argument = replacement
                    info = args[argument]
                else:
                    raise ValueError('Method "%s" Argument "%s" does not exist in version %d.' % (
                     method, argument, rpc_version))

        return (
         argument, TR_TYPE_MAP[info[0]](value))
    else:
...
def get_arguments(method, rpc_version):
    ...
    if method in ('torrent-add', 'torrent-get', 'torrent-set'):
        ...
        if method in ('session-get', 'session-set'):
            args = constants.SESSION_ARGS[method[-3:]]
        else:
            return ValueError('Method "%s" not supported' % method)
        accessible = []
