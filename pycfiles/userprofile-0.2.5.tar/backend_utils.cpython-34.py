...
# [GCC 8.4.0]
# Embedded file name: /Users/aneumeier/github/userprofile/userprofile/templatetags/backend_utils.py
# Compiled at: 2015-08-15 01:46:34
# Size of source mod 2**32: 2110 bytes
import re
from django import template
from social.backends.oauth import OAuthAuth
register = template.Library()
name_re = re.compile('([^O])Auth')
...
def social_backends(backends):
    backends = [(name, backend) for name, backend in backends.items() if name not in ('username',
                                                                                      'email')]
...
def legacy_backends(backends):
    backends = [(name, backend) for name, backend in backends.items() if name in ('username',
                                                                                  'email')]
    backends.sort(key=lambda b: b[0])
    return backends
