...
# [GCC 8.4.0]
# Embedded file name: /home/tonis/Projects/lino/src/react/lino_react/react/views.py
# Compiled at: 2020-01-15 03:50:42
# Size of source mod 2**32: 35858 bytes
...
def find(itter, target, key=None):
    ...
    if not key == None:
        assert callable(key), "key shold be a function that takes the itter's item and returns that wanted matched item"
    ...
    def get(self, request, app_label=None, actor=None):
        ...
        if fmt == constants.URL_FORMAT_JSON:
            ...
            for row in ar.create_phantom_rows():
                if not ar.limit is None:
                    if len(rows) + 1 < ar.limit or ar.limit == total_count + 1:
...
def choices_for_field(ar, holder, field):
    ...
    if not holder.get_view_permission(ar.request.user.user_type):
        raise Exception('{user} has no permission for {holder}'.format(user=(ar.request.user),
          holder=holder))
    else:
        ...
        if chooser:
            ...
            if not isiterable(qs):
                ...
                if chooser.instance_values:

                    def row2dict(obj, d):
                        d[constants.CHOICES_TEXT_FIELD] = holder.get_choices_text(obj, ar.request, field)
