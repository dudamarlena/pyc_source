...
# [GCC 8.4.0]
# Embedded file name: /Users/faustman/Sites/projects/django-frontend/djfrontend/templatetags/djfrontend.py
# Compiled at: 2016-07-20 12:30:27
...
def djfrontend_normalize(version=None):
    ...
    return format_html('<link rel="stylesheet" href="{0}djfrontend/css/normalize/{1}/normalize.css">', _static_url, version)
...
def djfrontend_fontawesome(version=None):
    ...
    return format_html('<link rel="stylesheet" href="{0}djfrontend/css/fontawesome/{1}/font-awesome{2}.css">', _static_url, version, _min)
...
def djfrontend_twbs_theme_css(version=None):
    ...
    return format_html('<link rel="stylesheet" href="{static}djfrontend/css/twbs/{v}/bootstrap-theme{min}.css">', static=_static_url, v=version, min=_min)
...
def djfrontend_twbs_js(version=None, files=None):
    ...
    if files:
        ...
        files = 'all'
    if files == 'all':
        return format_html('<script src="//cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/{v}/js/bootstrap.min.js"></script>\n<script>window.jQuery.fn.scrollspy || document.write(\'<script src="{static}djfrontend/js/twbs/{v}/bootstrap.min.js"><\\/script>\')</script>', v=version, static=_static_url)
    else:
        if 'popover' in files and 'tooltip' not in files:
            files.append('tooltip')
        for file in files:
            file = [ '<script src="%sdjfrontend/js/twbs/%s/%s.js"></script>' % (_static_url, version, file) for file in files
                   ]
