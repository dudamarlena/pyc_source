...
# [GCC 8.4.0]
# Embedded file name: build/lib.linux-i686-2.6/m_lib/net/ftp/ftpscan.py
# Compiled at: 2017-04-23 16:41:24
"""Recursive FTP scanners"""
from __future__ import print_function
import codecs, ftplib, sys
from .ftpparse import ftpparse

class FtpScanError(Exception):
    pass


ftpscan_error_mark = object()
...
class GetFiles:
    ...
    def filter(self, file_type):
        return filter(lambda e, file_type=file_type: e.file_type == file_type, self.entries)
...
class ReconnectingFTPWrapper:
    ...
    def __init__(self, ftp, ftp_server, ftp_port=0, login=None, password=None, ftp_dir='/', tree=None):
        ...
        ftp_dir = [''] + [ name for name in ftp_dir.split('/') if name ]
    ...
    def cwd(self, new_cwd, do_ftp=True):
        ...
        self._ftp_dir = ('/').join(ftp_dir)
        if do_ftp:
            self._wrap(self._ftp.cwd)(new_cwd)
...
def ftpscan1(ftp_server, ftp_port=0, login=None, password=None, ftp_dir='/', passive=None, FTPClass=ftplib.FTP, reconnect=False, ReconnectingFTPWrapperClass=ReconnectingFTPWrapper):
...
def ftpscanrecursive(ftp_server, ftp_port=0, login=None, password=None, ftp_dir='/', passive=None, FTPClass=ftplib.FTP, reconnect=False):
    """
   ...
   """
    ...
    for line in lines:
        if line:
            if line[(-1)] == ':' and not line.startswith('-rw-'):
...
def ftpscan(ftp_server, ftp_port=0, login=None, password=None, ftp_dir='/', passive=None, FTPClass=ftplib.FTP):
    try:
