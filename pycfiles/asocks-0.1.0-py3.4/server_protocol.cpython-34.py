...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/server/server_protocol.py
# Compiled at: 2017-06-08 19:23:40
# Size of source mod 2**32: 7647 bytes
import sys, os, asyncio, errno, struct, logging, socket, functools, auth
from networking import SOCK_PROTOCOL_VERSION, AddressType, ConnectionStatus as Status
...
class Socks5ProtocolState:
    ...
    def state_name(cls, state):
        if cls.StateMapping is None:
            cls.StateMapping = {getattr(cls, attr):attr for attr in dir(cls) if not callable(getattr(cls, attr)) and not attr.startswith('__') and attr != 'StateMapping'}
        return cls.StateMapping.get(state)


class ServerRemoteProtocol(asyncio.Protocol):
    __doc__ = 'Protocol for streaming to remote host'
...
class ServerClientProtocol(asyncio.Protocol):
    ...
    def connection_made(self, transport):
        ...
        self.logger.info('Received connection from client {}.'.format(peername))
    ...
    def data_received(self, data):
        if self.state == Socks5ProtocolState.INIT:
            ...
            if self.state == Socks5ProtocolState.NEGOTIATED:
                raise NotImplementedError
            else:
                if self.state == Socks5ProtocolState.AUTHORIZED:
    ...
    def _accept_connect(self, data):
        ...
        if atype == AddressType.IPv4:
            ...
            if atype == AddressType.DomainName:
                ...
                host = ':'.join([str(struct.unpack('>H', data[idx:idx + 2])[0]) for idx in range(4, 20, 2)])
                port = data[20:22]
            port = struct.unpack('>H', port)[0]
            self.logger.info('Connecting to remote server at {}:{}.'.format(host, port))
    ...
    def _connect_to_remote(self, host, port, waiter):
        exception = None
        try:
            transport, protocol = yield from self._loop.create_connection(functools.partial(ServerRemoteProtocol, transport_to_client=self.transport_to_client), host, port)
            ...
            self.logger.error('Connecting to {}:{} failed: '.format(host, port, os.strerror(err.errno)))
