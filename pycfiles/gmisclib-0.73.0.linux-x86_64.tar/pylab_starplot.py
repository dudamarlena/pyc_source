...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/gmisclib/pylab_starplot.py
# Compiled at: 2009-12-18 16:00:31
...
class errorbar_maker(object):
    ...
    def __init__(self, cx, cy, n, **kwargs):
        ...
        self.cy = cy
        self.n = n
        self.kwargs = {'marker': 'o', 'linestyle': '-', 'mec': 'r', 
           'color': 'k', 'zorder': 2}
    ...
    def make(self, i, xl, xc, xh, **kwargs):
        ...
        pylab.plot(x4, y4, marker=kwa['marker'], linestyle='', mec=mcolor, mew=0.0, color=mcolor, zorder=zorder)
...
def plot(xlch, **kwargs):
    ...
    if n == 0:
        ...
        for x, llch in xlch:
            star(x, 0.0, [ lch * f for lch in llch if lch is not None ], **kwargs)
...
if __name__ == '__main__':
    plot([(0.0, [errorbar(0, 1, 2), errorbar(0, 2, 3), errorbar(0, 2, 3), errorbar(0, 1, 2)]),
     ...
     (
      2.0, [errorbar(-1, 1, 2, dim=True), errorbar(-1, 0, 1, dim=True), errorbar(-1, -0.5, 0), errorbar(-1, -0.5, -0.25)])], lcolormap=[
     'r', 'g', 'b', 'c', 'm', 'k'])
    pylab.show()