...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/gmisclib/HTK_HMM_io.py
# Compiled at: 2010-09-29 07:46:23
"""This reads in HMM files produced by HTK.
"""
import re, sys, numpy, string
...
class vec(object):
    ...
    def write(self):
        o = [
         '<%s> %d' % (self.cname[self.code], self.v.shape[0])]
        n = self.v.shape[0]
...
class ref(object):
 ...
 L. 114         0  LOAD_FAST             1  'thing'
                3  LOAD_ATTR             0  'code'
                6  LOAD_CONST               None
                9  COMPARE_OP            9  is-not
               12  POP_JUMP_IF_FALSE    30  'to 30'
               15  LOAD_FAST             1  'thing'
               18  LOAD_ATTR             2  'name'
               21  LOAD_CONST               None
               24  COMPARE_OP            9  is-not
             27_0  COME_FROM            12  '12'
               27  POP_JUMP_IF_TRUE     49  'to 49'
               30  LOAD_ASSERT              AssertionError
               33  LOAD_CONST               'thing=%s'
               36  LOAD_GLOBAL           4  'repr'
               39  LOAD_FAST             1  'thing'
               42  CALL_FUNCTION_1       1  None
               45  BINARY_MODULO    
               46  RAISE_VARARGS_2       2  None

 L. 115        49  LOAD_FAST             1  'thing'
               52  LOAD_FAST             0  'self'
               55  STORE_ATTR            5  'thing'
               58  LOAD_CONST               None
               61  RETURN_VALUE     
...
Parse error at or near `LOAD_CONST' instruction at offset 58
    ...
    def write(self):
        return [
         I * 4 + '~%s "%s"' % (self.thing.code, self.thing.name)]
...
class hmm(object):
    ...
    def write(self):
        ...
        return [
         '~h "%s"' % self.name, '<BeginHMM>', I * 1 + '<NumStates> %d' % nstates] + o + tr + ['<EndHMM>']
...
def HTKformat(s):
    ...
    if not isinstance(s, str):
        raise AssertionError, 's=%s' % repr(s)
        raise (s or NoPhonemeToFormat)("lbl='%s'" % s)
    s = _q.sub('qq', s)
