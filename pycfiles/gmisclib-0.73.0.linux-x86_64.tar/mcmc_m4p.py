...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/gmisclib/mcmc_m4p.py
...
class stepper(MCH.stepper):
    ...
    def communicate_hook(self, id):
        ...
        if size() > 1:
            ...
            nv, nlp, nid = mpi.sendrecv(sendobj=(v, lp, id), dest=(r + 1) % s, sendtag=self.MPIID, source=(r + s - 1) % s, recvtag=self.MPIID)
