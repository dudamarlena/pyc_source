...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/gmisclib/mcmc_logger.py
# Compiled at: 2011-05-11 15:12:59
import re, numpy
...
class logger_c(mcmcP.logger_template):
    ...
    def add(self, stepperInstance, iter):
        ...
        elif iter >= self.last_logged_iter + self.iterstep and self.erg > self.last_logged_erg + self.ergstep:
...
class logger_A(logger_c):
    ...
    def __init__(self, logwriter, iterstep=100, ergstep=0.1):
        ...
        self.HUGEp = 10000000000.0
        self.HUGEl = 10000000000.0
    ...
    def Add(self, stepperInstance, iter, reason=None):
        ...
        tmp = {'T': stepperInstance.acceptable.T(), 'resetid': stepperInstance.reset_id(), 
           'iter': iter}
...
class logger_N(logger_c):
    ...
    def Add(self, stepperInstance, iter, reason=None):
        ...
        self.last_logged_erg = self.erg
        self.last_logged_iter = iter
        current = stepperInstance.current()
        idxr = current.pd.idxr
        self.lg.add('UID', current.prms(), idxr.map, current.logp_nocompute(), iter, extra={'T': stepperInstance.acceptable.T(), 'resetid': stepperInstance.reset_id()}, reason=reason)
        if hasattr(idxr, 'get_fixed'):
            fixed = stepperInstance.current().pd.idxr.get_fixed()
            if fixed:
                mp = {}
                pc = numpy.zeros((len(fixed),))
                for i, (k, v) in enumerate(sorted(fixed.items())):
                    pc.itemset(i, v)
                    mp[k] = i

                self.lg.add('fixed', pc, mp, stepperInstance.current().logp_nocompute(), iter, extra={'T': stepperInstance.acceptable.T(), 'resetid': stepperInstance.reset_id()}, reason=reason)
        self.lg.flush()


BadFormatError = fiatio.BadFormatError

def read_currentlist(fname, trigger=None):
    """Starting from a file name, read in a log file.
        ...
        @rtype: C{tuple(list(dict(str:str)), dict(str:str))}
        @return: a list of data lines and header information.
                A data line is a dictionary, containing parameters and other information from one iteration.
        @note: this is very similar to L{newstem2.newlogger.read_currentlist} except that returns a more
                complicated data structure that has one more level of indirection.
        """
...
class lti_c(object):
    ...
    def parse(self, d):
        """This constructs an L{newstem2.indexclass.index} object from a line from a log file.
                @param d: a line from a log file specifying information for an iteration.
                @type d: dict(str:str)
                @return: the parameters for that iteration.
                @rtype: L{newstem2.indexclass.index}
                """
        ...
        for k, v in d.items():
            if k.startswith(PREFIX):
                prms.itemset(self.h.get_image(unfmt(k[lp:])), float(v))
                n += 1
...
def read_multisample(fname, Nsamp=10, tail=0.0, trigger=None, reader=read_currentlist):
    """Read in a log file: it gives you multiple samples
        ...
        @param reader: A function that sucks in the data from the log file, yielding a list of
                iterations and an overall header.
        @type reader: function(str, trigger) -> (list(dict(str:str)), dict(str:str))
        @return:
        @rtype: (dict(str:str), list(indexer), list(float))
        @except NoDatError: when the log file is empty.
        @except BadFormatError: when a parameter appears mid-way through the optimization or an iteration cannot be read.
        """
    currentlist, lasthdr = reader(fname, trigger=trigger)
    if len(currentlist) == 0:
        raise NoDataError, fname
    lc = len(currentlist)
    tail = min(tail, 1.0 - 0.5 / lc)
    nsamp = min(Nsamp, lc - int(lc * tail))
    assert nsamp > 0
    lti = lti_c()
    indexers = []
    logps = []
    ...
    except nice_hash.NotInHash as x:
