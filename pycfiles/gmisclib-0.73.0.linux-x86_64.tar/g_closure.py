...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/gmisclib/g_closure.py
# Compiled at: 2007-10-13 12:27:49
...
class Closure:
    ...
    def __repr__(self):
        return '%s(%s)' % (str(self.fcn),
         (',').join([ str(q) for q in self.args ] + [ '%s=%s' % kv for kv in self.argd.items() ]))
