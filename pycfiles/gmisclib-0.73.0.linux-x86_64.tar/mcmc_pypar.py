...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/gmisclib/mcmc_pypar.py
# Compiled at: 2011-05-11 15:12:52
...
"""
import sys, pypar as mpi, operator, random
...
class stepper(MCH.stepper):
    ...
    def _not_at_bottom(self, xchanged, nchg, es, dotchanged, ndot, update_T):
        mytmp = Num.sometrue(Num.less(xchanged, nchg)) or es < 1.0 or dotchanged < ndot or update_T and self.x.T > 1.5
        self.note('_nab pre')
        ntrue = Reduce(int(mytmp), 0)
        self.note('_nab post')
...
def test():
    ...
    x = mcmc.bootstepper(test_logp, Num.array([0.0, 2.0]), Num.array([[1.0, 0], [0, 2.0]]))
    print 'rank=', mpi.rank()
    thr = stepper(x)
    for i in range(2):
        print 'RTC'
