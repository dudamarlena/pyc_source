...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/gmisclib/mcmc_indexclass.py
# Compiled at: 2011-05-13 03:20:37
...
"""
import re, os, numpy, die, dictops, g_encode
Debug = False
...
class index_base(object):
 ...
 L.  44         0  LOAD_GLOBAL           0  'isinstance'
                3  LOAD_FAST             1  'keymap'
                6  LOAD_GLOBAL           1  'dict'
                9  CALL_FUNCTION_2       2  None
               12  POP_JUMP_IF_TRUE     40  'to 40'
               15  LOAD_ASSERT              AssertionError
               18  LOAD_CONST               'Keymap must be a dict, not %s: %s'
               21  LOAD_GLOBAL           3  'type'
               24  LOAD_FAST             1  'keymap'
               27  CALL_FUNCTION_1       1  None
               30  LOAD_FAST             1  'keymap'
               33  BUILD_TUPLE_2         2 
               36  BINARY_MODULO    
               37  RAISE_VARARGS_2       2  None

 L.  45        40  LOAD_FAST             1  'keymap'
               43  LOAD_FAST             0  'self'
               46  STORE_ATTR            4  'map'

 L.  46        49  BUILD_MAP_0           0  None
               52  LOAD_FAST             0  'self'
               55  STORE_ATTR            5  '_p'

 L.  47        58  LOAD_FAST             2  'name'
               61  LOAD_FAST             0  'self'
               64  STORE_ATTR            6  'name'

 L.  48        67  LOAD_CONST               0
               70  LOAD_FAST             0  'self'
               73  STORE_ATTR            7  'modified'
...
Parse error at or near `LOAD_FAST' instruction at offset 70
    ...
    def pN(self, num, *key):
        return [ self.p(*(key + (str(i),))) for i in range(num)
               ]

    def pNr(self, num, low, high, *key):
        return [ self.p_range(low, high, *(key + (str(i),))) for i in range(num)
               ]

    def pNl(self, num, low, high, *key):
        return [ self.p_lower(low, high, *(key + (str(i),))) for i in range(num)
               ]
    ...
    def _fmt(cls, key):
        for kq in key:
            if not isinstance(kq, str):
                raise AssertionError, 'Key is not entirely composed of strings: %s' % repr(key)
...
class guess(index_base):
    ...
    def __init__(self, guesses, fp=None, name=''):
        ...
        try:
            for pat, sampler in guesses:
                ...
                if not hasattr(sampler, '__call__'):
                    raise AssertionError, 'Must be a callable object (e.g. function)'
    ...
    def _write(self, key, extra_stackdrop=0):
        if self.fp is not None:
            ...
            tmp = {'key': fmtd, 'index': self.map[key], 'stack': (',').join(stack), 'name': self.name}
    ...
    def p_range--- This code section failed: ---

 L. 375         0  LOAD_DEREF            1  'ub'
                3  LOAD_DEREF            0  'lb'
                6  COMPARE_OP            4  >
                9  POP_JUMP_IF_TRUE     52  'to 52'
               12  LOAD_ASSERT              AssertionError
               15  LOAD_CONST               'Bad range: %s<%s for %s'
               18  LOAD_GLOBAL           1  'str'
               21  LOAD_DEREF            1  'ub'
               24  CALL_FUNCTION_1       1  None
               27  LOAD_GLOBAL           1  'str'
               30  LOAD_DEREF            0  'lb'
 ...
 L. 377        52  LOAD_CLOSURE          1  'ub'
               55  LOAD_CLOSURE          0  'lb'
               61  LOAD_CODE                <code_object fix_range>
               64  MAKE_CLOSURE_0        0  None
               67  STORE_FAST            4  'fix_range'

 L. 383        70  LOAD_CONST               'range from %g to %g'
               73  LOAD_DEREF            0  'lb'
               76  LOAD_DEREF            1  'ub'
               79  BUILD_TUPLE_2         2 
               82  BINARY_MODULO    
               83  LOAD_FAST             4  'fix_range'
               86  STORE_ATTR            2  '__doc__'

 L. 384        89  LOAD_FAST             0  'self'
               92  LOAD_ATTR             3  '_compute'
               95  LOAD_FAST             3  'key'
               98  LOAD_FAST             4  'fix_range'
              101  LOAD_CONST               0
              104  CALL_FUNCTION_3       3  None
              107  RETURN_VALUE     
               -1  RETURN_LAST      

Parse error at or near `RETURN_VALUE' instruction at offset 107
...
def default_mapfp(nmprefix='m'):
    ...
    fp = open(os.path.join('MAPS', '%s%s' % (nmprefix, fname)), 'w')
    return fp
