...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/gmisclib/gpk_rlsq.py
# Compiled at: 2010-12-12 10:37:22
import numpy
...
class w_linear_least_squares(gpk_lsq.linear_least_squares):
    ...
    def __init__(self, a, y, w, minsv=None, minsvr=None, copy=True):
        ...
        gpk_lsq.linear_least_squares.__init__(self, aw, None, minsv=minsv, minsvr=minsvr, copy=False)
...
def test_w1():
    ...
    w = numpy.array([1, 3.2, 1, 1, 5, 1, 1, 1, 0.0001, 1])
    ...
    assert numpy.absolute(w - soln.w).sum() < 1e-06
...
def test_w1b():
    ...
    w = numpy.array([0.2, 1, 0.2, 5, 1, 0.2, 1, 2, 0.001, 0.01])
    soln = w_linear_least_squares(basis, y, w)
    print 'soln.x', soln.x()
...
def test_r1():
    ...
    assert numpy.absolute(numpy.dot(basis, soln.x()) - soln.fit()).sum() < 1e-05
