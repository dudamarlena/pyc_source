...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/gmisclib/cache.py
# Compiled at: 2011-05-13 03:18:39
import os, sys, stat, types, hashlib, cPickle, threading, die, gpkmisc
...
def modFileInfo(fname, *other):
    ...
    if fname is None:
        ...
        return (
         os.path.abspath(fname), s[stat.ST_SIZE]) + other
...
class cache_info(object):
    ...
    def addinfo(self, *s, **kv):
        """
                ...
                """
        return cache_info(root=self.root, info=self.info + s, fname=kv.get('fname', None), modname=kv.get('modname', None), mod=kv.get('mod', None))
    ...
    def load(self):
        ...
        li = len(self.info)
        assert li > 0
        if isinstance(x, tuple) and len(x) == 3 and len(x[0]) > 0 and len(x[2]) > 0 and len(x[0]) + len(x[2]) == li + 1 and x[0][(-1)] == x[2][0] and x[0] == self.info[:(1 + li) // 2] and x[2] == self.info[(li - 1) // 2:]:
    ...
    Errors = (
     IOError, EOFError, UnpicklingError, BadFileFormat)
