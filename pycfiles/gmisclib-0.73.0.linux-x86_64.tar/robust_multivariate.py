...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/gmisclib/robust_multivariate.py
...
def integrate(xlow, xhigh, fcn, c=None, nstart=5, tol=0.0001):
...
def covariance(veclist, emax=1.0):
    ...
    E = echg * emax
    nvec, vecdim = veclist.shape
    e0 = integrate(0, 3.0 + 0.5 * vecdim, wtdim, c=(0.0, vecdim, 0), nstart=vecdim * 2)
    e0qq = integrate(0, 3.0 + 0.5 * vecdim, wtdim, c=(0.0, vecdim, 2), nstart=vecdim * 2)
    ...
    while ipass < 1000:
        ...
        qlast = q
        e = integrate(0, 3.0 + 0.5 * vecdim, wtdim, c=(E, vecdim, 0), nstart=vecdim * 2)
        eqq = integrate(0, 3.0 + 0.5 * vecdim, wtdim, c=(E, vecdim, 2), nstart=vecdim * 2)
        efac = eqq / e0qq / (e / e0)
        print 'efac=', efac
