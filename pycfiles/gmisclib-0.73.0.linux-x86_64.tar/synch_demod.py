...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/gmisclib/synch_demod.py
# Compiled at: 2007-08-13 06:22:59
import Num, math, djt_spectrum
...
def pwrs(dt, harmonics, f0, signal, sig_dt, bandwidth):
    ...
    for h in harmonics:
        mod = Num.exp(complex(0.0, 1.0) * h * phase) * signal
...
def stepped_ampl(dt, harmonics, f0, signal, sig_dt, subwindow=0.01, nsubw=4):
    ...
    for h in harmonics:
        mod = Num.exp(complex(0.0, 1.0) * h * phase) * signal
        m = sharp_energy.stepped_median(dt / sig_dt, subwindow / sig_dt, mod, nsubw)
        o.append(m)
