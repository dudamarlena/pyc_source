...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/gmisclib/gpkequations.py
# Compiled at: 2008-11-27 11:03:25
import Num, types, sbd_array
...
class eqn_c:
    ...
    def __init__(self, terms, sum, wt, index_ctr):
        ...
        self.terms = map(lambda x, w=wt, c=index_ctr: (x[0] + c, x[1] * w), terms)
        self.terms.sort()
        self.sum = sum * wt
...
def test_solve():
    ...
    a, b = build_matrices(e)
    x = solve_eqns(a, b)
    assert abs(x[(0, 0)]) < 1e-06
    assert abs(x[(0, 1)] - 1) < 1e-06
    e = []
    e.append(eqn_c([(0, 1)], 1.0, 2.0, 0))
    e.append(eqn_c([(0, 1)], 2.0, 1.0, 0))
    a, b = build_matrices(e)
    x = solve_eqns(a, b)
    assert abs(x[(0, 0)] - 6.0 / 5.0) < 1e-06
...
def test_solve2():
    ...
    x = solve_eqns(a, b)
    assert x.shape == (1, 4)
    assert abs(x[(0, 0)] - 1) < 1e-06
    assert abs(x[(0, 1)] - 2) < 1e-06
    assert abs(x[(0, 2)] - 3) < 1e-06
    assert abs(x[(0, 3)] - 4) < 1e-06
