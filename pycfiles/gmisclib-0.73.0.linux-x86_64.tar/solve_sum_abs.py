...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/gmisclib/solve_sum_abs.py
...
def solve_fit(x, y, epsx=1e-07, epsm=1e-07):
    ...
    mhat = math.tan(root.root(to_be_zeroed, -mp2, mp2, (x, y), epsm, xtol))
    return (
     mhat, b(x, y, mhat) + ybar - mhat * xbar)


def solve_fit_wt(x, y, wt, epsx=1e-07, epsm=1e-07):
    """Solves for the line yhat = m*x + b that minimizes
        sum(abs(y - yhat)).   In other words, it's a fit to a straight line,
        but a highly robust fit that will not be disturbed by outliers.
        The algorithm is from Numerical Recipes, Volume 2.
    ...
    if not epsx > 0.0:
        ...
        sw = numpy.sum(wt)
        raise sw > 0.0 or ValueError, 'Weights are NaN or all zero'
    x = numpy.asarray(x)
    ...
    mhat = math.tan(root.root(to_be_zeroed, -mp2, mp2, (x, y, wt), epsm, xtol))
    return (
     mhat, b(x, y, wt, mhat) + ybar - mhat * xbar)
