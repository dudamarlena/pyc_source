...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/gmisclib/kl_dist.py
...
"""
import Num, mcmc, mcmc_helper, gpkavg, gpkmisc, math, random
...
def kldist_vec(p, q, N=None, Fp=1.0, Fq=1.0, Clip=0.01):
    ...
    xq = mcmc.bootstepper(multinomial_logp, qstart, qV, c=(
     q, Fq), fixer=multinomial_fixer)
    xp = mcmc.bootstepper(multinomial_logp, pstart, pV, c=(
     p, Fp), fixer=multinomial_fixer)
...
def solve_for_pi(p):
    ...
    while iter < MAXITERS:
        ...
        npi = nxt / nxtnorm
        if Num.square(pi - npi).sum() < 1e-08:
            break
        pi = npi
        iter += 1
...
def kl_nonzero_tr_probs(pp, qq):
    ...
    for i, pi_i in enumerate(pi):
        assert pi_i > EPS and abs(Num.sum(pp[i, :]) - 1) < 0.0001
        if not abs(Num.sum(qq[i, :]) - 1) < 0.0001:
