...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/gmisclib/read_dicom.py
# Compiled at: 2007-08-13 06:22:59
import g_exec, re, Num
...
def read_header(f):
    pass1 = re.compile('[(]([0-9a-fA-F]+),([0-9a-fA-F]+)[)]\\s+[A-Z]+.[0-9].\\s+([a-zA-Z0-9_]+):\\s*(.*)\\s*[(]\\s*[0-9]+ bytes\\s*[)]$')
    pass2gel = re.compile('([a-zA-Z0-9_]+)\\s+[(][0-9]+[)]:\\s*([x0-9a-fA-F]+)')
    pass2e = re.compile('(.*)\\s*:\\s*(.*)')
    gnpat = re.compile('(.*)\\s+-\\s+Group\\s+([x0-9a-fA-F]+)')
    defer = {}
    gnames = {}
    caught = set()
    ipass = 0
