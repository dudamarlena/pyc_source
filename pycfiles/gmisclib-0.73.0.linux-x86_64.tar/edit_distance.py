...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/gmisclib/edit_distance.py
# Compiled at: 2010-04-21 17:38:35
...
def distf1(s, t, csub):
    ...
    for j in range(1, n + 1):
        for i in range(1, m + 1):
            d[(i, j)] = min(d[(i - 1, j)] + csub(s[(i - 1)], None), d[(i, j - 1)] + csub(None, t[(j - 1)]), d[(i - 1, j - 1)] + csub(s[(i - 1)], t[(j - 1)]))
...
def distf2(s, t, csub):
    ...
    for j in range(1, n + 1):
        ...
        for i in range(1, m + 1):
            dj[i] = min(dj[(i - 1)] + csub(s[(i - 1)], None), djm1[i] + csub(None, t[(j - 1)]), djm1[(i - 1)] + csub(s[(i - 1)], t[(j - 1)]))
...
def test2():
    ...
    b = list(a)
    b[-4] = 'fir'
    assert abs(distf(a, b, text_cost(frac_exp=1.0)) - 1.0 / 3.0) < 0.001
