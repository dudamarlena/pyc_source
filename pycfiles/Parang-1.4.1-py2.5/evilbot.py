...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/parang/evilbot.py
...
class EvilBot(DumbBot):
    ...
    def __init__(self, **kwargs):
        ...
        try:
            ...
            if self.games.has_key(game):
                shared = self.games[game]
            else:
                self.games[game] = shared = self.shared_info()
        finally:
        ...
        self.shared = shared
        if self.power:
            shared.friends.add(self.power.key)

    def handle_HLO(self, message):
        self.log_debug(1, 'EvilBot takes over %s, passcode %d!', message[2], message[5].value())
        self.shared.friends.add(message[2])
        super(EvilBot, self).handle_HLO(message)

    def friendly(self, nation):
        return nation.key in self.shared.friends
    ...
    def generate_movement_orders(self, values):
        ...
        try:
            ...
            if turn and turn == now.key:
                ...
                orders = self.dumb_movement(values, OrderSet(), [ u for u in self.map.units if self.friendly(u.nation) ])
    ...
    def handle_SCO(self, message):
        ...
        for ally in self.shared.friends:
            if self.map.powers[ally].units:
                self.attitude[ally] = -0.1
            else:
                self.attitude[ally] = 2

        self.draws = [
         self.shared.friends & set(self.map.current_powers())]
