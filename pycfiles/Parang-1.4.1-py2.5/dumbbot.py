...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/parang/dumbbot.py
...
class DumbBot(Player):
    ...
    def __init__(self, values=None, **kwargs):
        ...
        self.log_debug(9, '%s (%s); started at %s', self.name, self.version, ctime())
    ...
    def handle_SCO(self, message):
        ...
        for (power_counter, power) in self.map.powers.iteritems():
            ...
            self.power_size[power_counter] = (self.vals.size_square_coefficient * size * size + self.vals.size_coefficient * size + self.vals.size_constant) * self.attitude[power_counter]
    ...
    def generate_orders(self):
        ...
        try:
            ...
            if not (self.in_game and self.missing_orders()):
                return
            ...
            if phase == turn.move_phase:
                orders = self.generate_movement_orders(values)
            elif phase == turn.retreat_phase:
                orders = self.generate_retreat_orders(values)
                ...
                self.log_debug(1, 'Unknown phase %s', phase)
            if orders:
                self.submit_set(orders)
            if self.options.print_csv:
                self.generate_csv(values)
        except:
        ...
        for province_counter in self.map.spaces.itervalues():
            if province_counter.is_supply():
                if self.friendly(province_counter.owner):
                    ...
                    if owner:
                        ...
                        values.attack_value[province_counter.key] = self.vals.size_constant
            ...
            for province_coast_iterator in province_counter.coasts:
                values.proximity_map[province_coast_iterator.key] = [
                 values.attack_value[province_counter.key] * proximity_attack_weight + values.defence_value[province_counter.key] * proximity_defence_weight]
            ...
            for proximity_iterator in self.map.coasts.itervalues():
                ...
                for coast_iterator in proximity_iterator.borders_out:
                    ...
                    if provs_seen.has_key(key):
                        if weight > provs_seen[key]:
                            provs_seen[key] = weight
                    else:
                        provs_seen[key] = weight

                values.proximity_map[proximity_iterator.key].append(sum(provs_seen.values(), values.proximity_map[proximity_iterator.key][proximity_counter]) // 5)
    ...
    def calculate_defence_value(self, province):
        ...
        for prov in province.borders_in:
            for unit in self.map.spaces[prov].units:
                ...
                if not self.friendly(unit.nation) and self.power_size[unit.nation.key] > defence_value and unit.can_move_to(province):
        ...
        if self.power.key in province.homes:
            if defence_value:
                defence_value *= self.vals.home_defence_weight
            else:
                defence_value = -self.vals.home_vacation_weight
    ...
    def generate_csv(self, values):
        ...
        try:
            ...
            self.log_debug(1, "Couldn't open csv file " + filename)
        else:
    ...
    def friendly(self, nation):
        return nation == self.power
    ...
    def check_for_wasted_holds(self, orders, values):
        ...
        for order in orders:
            if order.is_moving():
                destination_map[order.destination.province.key] = (
                 order,
                 values.destination_value[order.destination.key] * values.competition_value[order.destination.province.key])
            else:
                destination_map[order.unit.coast.province.key] = (
                 order,
                 values.destination_value[order.unit.coast.key] * (values.competition_value[order.unit.coast.province.key] - 1))
                if order.is_holding():
        ...
        for order in holding:
            ...
            for adjacent_province in [ self.map.coasts[coast].province for coast in unit.coast.borders_out
                                     ]:
            ...
            if source:
                self.log_debug(11, '  Overriding hold order in %s with support to %s', unit.coast.province, source.unit)
                ...
                if source.is_moving():
                    ...
                    orders.add(SupportHoldOrder(unit, source.unit), unit.nation)
    ...
    def order_unit(self, unit, orders, values, waiting):
        ...
        destination_map = [
         unit.coast.key] + [ key for key in unit.coast.borders_out if key[1] not in waiters
                           ]
        while True:
    ...
    def consider_convoy(self, fleet, orders, values, dest=None):
        if self.vals.convoy_weight > 0 and fleet.can_convoy():
            ...
            if dest:
                ...
                if key:
                    beach = self.map.coasts[key]
                else:
                    self.log_debug(13, '   Selected location not coastal.')
            else:
                ...
                for border in fleet.coast.borders_out:
                    ...
                    if not orders.moving_into(prov):
                        for unit in prov.units:
                            if not (self.friendly(unit.nation) and not orders.is_moving(unit)):
                                key = prov.is_coastal()
                                if key:
                                    possible.append(key)
                ...
                if possible:
                    ...
                    self.log_debug(13, '   No available landing sites.')
            if beach:
                armies = [ unit.coast.key for unit in self.map.units if unit.can_be_convoyed() if self.friendly(unit.nation) if fleet.can_move_to(unit.coast.province) if not orders.get_order(unit) if not unit.can_move_to(beach.province.key)
                         ]
                if armies:
                    ...
                    if convoy_best != self.weighted_choice(first, second):
                        ...
                        self.log_debug(13, '   Convoy rejected')
                else:
                    self.log_debug(13, '   Nobody available to convoy')
        return
    ...
    def generate_retreat_orders(self, values):
        ...
        for unit in our_units:
            ...
            destination_map = [ key for key in unit.coast.borders_out if key[1] in unit.retreats
                              ]
    ...
    def generate_remove_orders(self, remove_count, values):
        ...
        while remove_count and remove_map:
            unit = remove_map.pop(self.random_selection([ -values.destination_value[u.coast.key] for u in remove_map
                                                        ]))
    ...
    def generate_build_orders(self, build_count, values):
        ...
        build_map = [ coast.key for prov in [ self.map.spaces[p] for p in self.power.homes ] if prov.owner == self.power if not prov.units for coast in prov.coasts
                    ]
    ...
    def random_destination(self, coast_keys, values):
        ...
        """
        return self.map.coasts[coast_keys[self.random_selection([ values.destination_value[key] for key in coast_keys ])]]
    ...
    def random_selection(self, choice_values):
        ...
        while iterator > 0:
            iterator -= 1
            if self.weighted_choice(sorted_values[best][0], sorted_values[iterator][0]):
                current = iterator
            else:
                break
    ...
    def weighted_choice(self, best_choice, second_choice):
        ...
        use_next = False
        if best_choice:
            best_chance = (best_choice - second_choice) * self.vals.alternative_difference_modifier // abs(best_choice)
        else:
            best_chance = 0
        ...
        if random() * 100 < chance:
            ...
            choice = 'First'
        self.log_debug(14, '     %s choice selected (%d%% chance of %d over %d)', choice, chance, second_choice, best_choice)
        return use_next
