...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/parang/teddybot.py
# Compiled at: 2009-08-22 22:50:09
"""TeddyBot - A Diplomacy bot that attempts to choose targets.
...
class TeddyBot(Player):
    """A bot that attempts to choose targets carefully.
    """
    ...
    def process_map(self):
        """Performs supplemental processing on the map.
            For TeddyBot, this involves distance and centrality calculations.
            Returns whether to accept the MAP message.
        """
        self.distance = cache('Teddy.distance.' + self.map.name, self.calc_distances)
        self.centrality = cache('Teddy.centrality.' + self.map.name, self.calc_centrality, self.distance)
        return True
    ...
    def calc_centrality(self, distance):
        ...
        coasts = self.map.coasts
        for source in coasts:
            closeness[source] = sum((2 ** (-distance[(source, sink)]) for sink in coasts if sink != source))

        spaces = self.map.spaces
        for source in spaces:
            closeness[source] = sum((2 ** (-distance[(source, sink)]) for sink in spaces if sink.is_supply()))
