...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/parang/neurotic.py
...
class Brain(VerboseObject):
    ...
    def sort_lists(self, board):
        ...
        self.coastals = [ board.spaces[token].is_coastal() for token in self.provinces if token.is_coastal()
                        ]
        self.coastlines = dict(((token, [ coast.key for coast in sorted(province.coasts) ]) for (token, province) in board.spaces.iteritems()))
        self.borders = dict(((token, sorted(province.borders_out)) for (token, province) in board.spaces.iteritems()))

    def create_net(self, board):
        self.input_headers = map(str, self.seasons)
        for coast in self.coasts:
            if coast[2]:
                self.input_headers.extend(('%s %s %s: %s' % (coast[0], coast[1], coast[2], power) for power in self.powers))
            else:
                self.input_headers.extend(('%s %s: %s' % (coast[0], coast[1], power) for power in self.powers))

        for center in self.centers:
            self.input_headers.extend(('%s: %s' % (center, power) for power in self.powers))
        ...
        for token in self.provinces:
            self.output_headers.extend(('%s: From %s' % (token, prov) for prov in self.provinces))
            self.output_headers.extend(('%s: To %s' % (token, dest(coast)) for coast in self.coastlines[token]))
            self.output_headers.extend(('%s: To %s' % (token, dest(coast)) for prov in self.borders[token] for coast in self.coastlines[prov]))
            self.output_headers.extend(('%s: CTO %s' % (token, dest(coast)) for coast in self.coastals))
    ...
    def collect_inputs(self, board):
        ...
        inputs = [
         0] * len(self.input_headers)
        inputs[board.current_turn.index] = 1
        index = len(self.seasons)
        for coast in self.coasts:
            units = board.coasts[coast].province.units
            for power in self.powers:
                inputs[index] = len([ unit for unit in units if unit.coast.key == coast if unit.nation == power
                                    ])
                index += 1
    ...
    def parse_outputs(self, board, outputs):
            ...
            if units:
                ...
                for prov in self.provinces:
                    if prov == token:
                        ...
                        order_values.extend(((unit_value, (unit.dislodged and DisbandOrder or RemoveOrder)(unit)) for unit in units))
                ...
                for coast in self.coastlines[token]:
                    ...
                    order_values.extend(((hold_value, HoldOrder(unit)) for unit in units if coast == unit.coast.key))
                    index += 1
                ...
                for prov in self.borders[token]:
                    for key in self.coastlines[prov]:
                        ...
                        order_values.extend(((value, (unit.dislodged and RetreatOrder or MoveOrder)(unit, dest)) for unit in units if unit.can_move_to(dest)))
                        for (start, start_value) in starting.iteritems():
                            for mover in board.spaces[start].units:
                                if mover.can_move_to(dest):
                                    ...
                                    order_values.extend(((value, SupportMoveOrder(unit, mover, dest)) for unit in units))
                                ...
                                for mover in board.spaces[start].units:
                                    if mover.can_be_convoyed():
                                        ...
                                        order_values.append((value,
                                         ConvoyingOrder(unit, mover, dest)))
                ...
                index += sum((len(self.coastlines[prov]) for prov in self.borders[token]))
        ...
        self.log_debug(7, 'Outputs: %d; Index: %d; Orders: %d', len(outputs), index, len(order_values))
            ...
            if prov_orders.has_key(token):
                ...
                for prov in self.provinces:
                    if prov == token:
                        ...
                        if isinstance(order, (SupportOrder, ConvoyingOrder)) and order.supported.coast.province == token:
                            outputs[index] = 1
                    index += 1

                for coast in self.coastlines[token]:
                    if isinstance(order, HoldOrder) and coast == order.unit.coast.key:
                        outputs[index] = 1
                    index += 1
                ...
                for prov in self.borders[token]:
                    for key in self.coastlines[prov]:
                        if isinstance(order, (RetreatOrder,
                         MoveOrder, SupportMoveOrder)) and order.destination.key == key:
                            outputs[index] = 1
                        index += 1

                for key in self.coastals:
                    if isinstance(order, (ConvoyedOrder, ConvoyingOrder)) and order.destination.key == key:
                        outputs[index] = 1
                    index += 1
...
class Neurotic(Player):
        ...
        try:
            ...
            while missing and values:
                ...
                if order.unit and orders.has_order(order.unit.coast.province):
                    self.log_debug(11, 'Already has an order for %s', order.unit.coast.province)
                    continue
                elif note != MBV:
    ...
    def handle_ORD(self, message):
        ...
        if turn != self.map.current_turn:
            self.log_debug(1, 'Order for %s in %s', turn, self.map.current_turn)
    ...
    def handle_NOW(self, message):
        ...
        if last_turn:
            self.brain.learn(self.inputs[last_turn], self.learned[last_turn], self.map)
