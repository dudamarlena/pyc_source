...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/parang/combobot.py
...
class ComboBot(DumbBot):
    ...
    def generate_movement_orders(self, values):
        ...
        for orders in combo_sets:
            self.log_debug(11, '    %d - %s', *orders)

        best = combo_sets.pop()
        using = best[1]
        while combo_sets:
            second = combo_sets.pop()
            if self.weighted_choice(best[0], second[0]):
                using = second[1]
            else:
                break
    ...
    def generate_hold_combos(self):
        return [ OrderCombo(self, [HoldOrder(unit)]) for unit in self.power.units
               ]
    ...
    def generate_move_combos(self):
        ...
        for unit in self.power.units:
            for key in unit.coast.borders_out:
                ...
                if not any((other.nation == self.power for other in dest.province.units)):
                    adj_provs[key[1]].append(MoveOrder(unit, dest))

        return [ OrderCombo(self, combo) for value in adj_provs.values() for combo in sublists(value) if combo ]

    def generate_convoy_combos(self):
        return sum([ self.get_convoys(unit) for unit in self.power.units if unit.can_convoy()
                   ], [])
    ...
    def get_convoys(self, fleet):
        ...
        for border in fleet.coast.borders_out:
            ...
            if key:
                beaches.append(coasts[key])

        if not armies:
            return []
        return [ OrderCombo(self, [ConvoyingOrder(fleet, army, beach), ConvoyedOrder(army, beach, [fleet])]) for beach in beaches for army in armies ] + sum([ self.get_multi_convoys(next, [fleet, next], armies) for next in fleets
                                                                                                                                                             ], [])

    def get_multi_convoys(self, fleet, path, armies):
        self.log_debug(11, 'Considering convoys through %s.', ('-').join([ unit.coast.province.key.text for unit in path ]))
        fleets = []
        beaches = []
        coasts = self.map.coasts
        for border in fleet.coast.borders_out:
        ...
        return [ OrderCombo(self, [ConvoyingOrder(fleet, army, beach), ConvoyedOrder(army, beach, path)]) for beach in beaches for army in armies
               ] + sum([ self.get_multi_convoys(next, path + [next], armies) for next in fleets if next not in path
                       ], [])

    def combine_combos(self, combos, base=None):
        """ Gathers a list of complete order sets from the combos."""
        if base is None:
            base = ComboSet(self.power)
        results = []
        roots = list(combos)
        while roots:
            combo = roots.pop()
            compatibility = self.unfilled_orders(base, combo)
            self.log_debug(13, ' Compatibility of %s with %s: %d', combo, base, compatibility)
            if compatibility >= 0:
...
class OrderCombo(object):
    ...
    def get_children(self, new_orders):
        ...
        def attackers(province, friendly=self.player.power.key, spaces=self.player.map.spaces):
            """ Compiles a list of enemy units that can enter the province."""
            bordering = [ spaces[prov].units for prov in province.borders_in ]
            return [ u for u in sum(bordering, []) if u if u.nation != friendly if u.can_move_to(province)
                   ]
        ...
        unordered = [ u for u in self.player.power.units if not self.unit_orders.get_order(u)
                    ]
        holding = [ order.unit for order in self.unit_orders if order.is_holding()
                  ]
        if not (unordered or holding):
            return []
        sub_orders = []
        enter_provs = []
        for order in new_orders:
            if order.is_moving():
                if occupied(order.destination.province) or len(attackers(order.destination.province)) > 0:
                    sub_orders.extend([ SupportMoveOrder(u, order.unit, order.destination) for u in unordered + holding if u.can_move_to(order.destination.province)
                                      ])
                enter_provs.append(order.unit.coast.province.key)
            else:
                enemies = attackers(order.unit.coast.province)
                if len(enemies) > 1:
                    sub_orders.extend([ SupportHoldOrder(u, order.unit) for u in unordered + holding if u.can_move_to(order.unit.coast.province)
                                      ])
                    enter_provs.extend([ u.coast.province.key for u in enemies ])
                if order.is_supporting() and order.supported.coast != order.destination and occupied(order.destination.province):
                    enter_provs.extend([ u.coast.province.key for u in attackers(order.destination.province)
                                       ])

        sub_orders.extend([ MoveOrder(u, self.player.map.coasts[key]) for u in unordered for key in u.coast.borders_out if key[1] in enter_provs
                          ])
        sub_orders.extend(self.convoys_to(enter_provs, unordered, holding))
        return [ OrderCombo(self, order_list) for order_list in sublists(sub_orders) if valid(order_list)
               ]
    ...
    def convoys_to(self, prov_list, unordered, holding):
        ...
        for fleet in convoyers:
            ...
            for coast in [ coasts[key] for key in fleet.coast.borders_out ]:
                ...
                if key:
                    army = coasts[key]
                    if coast.province in prov_list:
                        beaches.append(army)
                    elif army in unordered:
                        armies.append(army)
                elif coast in convoyers:
                    fleets.append(coast)
        ...
        while new_paths:
            ...
            new_paths = [ route + [fleet] for route in new_paths for fleet in all_fleets[route[(-1)].key] if fleet not in route
                        ]

        return sum([ [ConvoyedOrder(army, beach, route)] + [ ConvoyingOrder(fleet, army, beach) for fleet in route ] for route in paths for army in all_armies[route[0].key] for beach in all_beaches[route[(-1)].key]
                   ], [])
        results = []
        for route in paths:
            for army in all_armies[route[0].key]:
                for beach in all_beaches[route[(-1)].key]:
                    ...
                    results.extend([ ConvoyingOrder(fleet, army, beach) for fleet in route
                                   ])
...
class ComboSet(object):
    ...
    def enemies(self, province, nation, adjacent_units):
        return [ unit for (power, unit_list) in adjacent_units[province.key].iteritems() if power.key not in (nation.key, self.power.key) for unit in unit_list if unit.coast.province.key != province.key
               ]

    def friends(self, province, adjacent_units):
        return [ unit for unit in adjacent_units[province.key][self.power.key] if unit.coast.province.key != province.key
               ]

    def stay_prob(self, unit, bias, adjacent_units):
        """ How likely a unit is to stay in the province indicated."""
        result = (1 - self.dislodge_prob(unit, not bias, adjacent_units)) * (1 - self.move_prob(unit, not bias, adjacent_units))
        return result
            ...
            for unit in self.friends(province, adjacent_units):
                ...
                if order.is_moving():
                    if order.destination.province.key == province.key:
                        entering = True
                elif order.is_supporting() and order.destination.province.key == province.key and order.supported.coast.province.key != province.key:
                    supports.append(1 - self.success_prob(order, bias, adjacent_units))

            if force == 1:
                result = entering and 1 or 0
            elif len(supports) >= force - 1:
                result = 1 - (bias and min or max)([ reduce(mul, sl, 1) for sl in sublists(supports) if len(sl) == force - 1
                                                   ])
        if bias:
            enemies = self.enemies(province, nation, adjacent_units)
            if len(enemies) >= force:
                result = 1
        return result
    ...
    def dislodge_prob(self, unit, bias, adjacent_units):
        ...
        if len(enemies) <= 1:
            return 0
        if all((self.attacked_prob(enemy.coast.province, enemy.nation, 1, not bias, adjacent_units) == 1 for enemy in enemies)):
            return 0
        strength = 1
        return self.attacked_prob(unit.coast.province, unit.nation, strength, bias, adjacent_units)
    ...
    def move_prob(self, unit, bias, adjacent_units):
        ...
        if order:
            if order.is_moving():
                result = self.success_prob(order, bias, adjacent_units)
            else:
                result = 0
        else:
            result = bias and 1 or 0
        return result
    ...
    def success_prob(self, order, bias, adjacent_units):
        ...
        if order.is_moving():
            ...
            result = 1 - self.attacked_prob(order.unit.coast.province, order.unit.nation, 1, not bias, adjacent_units)
        else:
            result = bias and 1 or 0
        return result
