...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/parang/blabberbot.py
...
from parlance.language import IntegerToken, Message, StringToken, Token, protocol
from parlance.tokens import BWX, ECS, HUH, NCS, REJ, SCS, TRY, WCS, YES
...
class BlabberBot(DumbBot):
    ...
    def run(self):
        if self.countries:
            self.send_press(choice(self.countries), self.random_expression('press_message'))
    ...
    def random_expression(self, expression):
        ...
        try:
            ...
            if options:
                items = self.syntax[expression] = [ syntax for (level, syntax) in options if level <= self.game_opts.LVL
                                                  ]
            else:
                items = self.syntax[expression] = ()
    ...
    def random_category(self, category):
        ...
        if category == 'Powers':
            ...
            result = StringToken(choice("'abcdefghijklmnopqrstuvwxyz ABCDEFGHIJKLMNOPQRSTUVWXYZ +-*/0123456789.?"))
        elif category.endswith('SC'):
            cat = category.replace('_', ' ')
            provs = [ prov for prov in self.map.spaces.keys() if prov.category_name() == cat
                    ]
            if provs:
                result = choice(provs)
        elif category == 'Token':
