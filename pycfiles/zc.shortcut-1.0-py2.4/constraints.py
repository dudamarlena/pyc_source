...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/zc/shortcut/constraints.py
...
class ShortcutTypePrecondition(object):
    ...
    def __init__(self, *target_types, **kwargs):
        ...
        if kwargs:
            raise TypeError("ShortcutTypePrecondition.__init__ got an unexpected keyword argument '%s'" % (kwargs.iterkeys().next(),))
...
def contains(*types, **kwargs):
    ...
    if not (f_locals is not f_globals and f_locals.get('__module__') and f_locals.get('__module__') == f_globals.get('__name__')):
