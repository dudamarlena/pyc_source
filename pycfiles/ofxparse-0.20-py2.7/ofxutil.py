...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/ofxparse/ofxutil.py
# Compiled at: 2018-07-30 14:59:10
from __future__ import absolute_import, with_statement
import os, collections, xml.etree.ElementTree as ET, six
...
class OfxData(object):
    ...
    def add_tag(self, name):
        ...
        if not isinstance(self.nodes[name], list):
            self.nodes[name] = [
             self.nodes[name], OfxData(name.upper())]
        else:
    ...
    def __setattr__(self, name, value):
        if name in self.__dict__ or name in ('nodes', 'tag', 'data', '                headers',
                                             'xml'):
    ...
    def __getattr__(self, name):
        if name in self.__dict__:
            ...
            if name in self.__dict__['nodes']:
        ...
        if self.data or not self.nodes:
            if self.tag.upper() == 'OFX':
                return [
                 [
                  '<%s>%s</%s>' % (self.tag,
                   self.data if self.data else '', self.tag), 0]]
