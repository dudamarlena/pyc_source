...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/ofxparse/ofxparse.py
# Compiled at: 2018-11-30 23:32:47
from __future__ import absolute_import
import sys, decimal, datetime, codecs, re, collections, contextlib
try:
...
class OfxFile(object):
    ...
    def handle_encoding(self):
        ...
        """
        ascii_headers = odict.OrderedDict((key.decode('ascii', 'replace'), value.decode('ascii', 'replace')) for key, value in six.iteritems(self.headers))
        ...
        self.headers = odict.OrderedDict((key.decode(encoding), value.decode(encoding)) for key, value in six.iteritems(self.headers))
...
class OfxPreprocessedFile(OfxFile):
    ...
    def __init__(self, fh):
        ...
        if self.fh is None:
            ...
            closing_tags = [ t.upper() for t in re.findall('(?i)</([a-z0-9_\\.]+)>', ofx_string)
                           ]
            last_open_tag = None
            tokens = re.split('(?i)(</?[a-z0-9_\\.]+>)', ofx_string)
            new_fh = StringIO()
            for token in tokens:
                ...
                if is_tag:
                    if last_open_tag is not None:
                        ...
                        last_open_tag = None
                if is_open_tag:
                    tag_name = re.findall('(?i)<([a-z0-9_\\.]+)>', token)[0]
...
class OfxParser(object):
    ...
    def parse(cls, file_handle, fail_fast=True, custom_date_format=None):
        ...
        if not hasattr(file_handle, 'seek'):
            raise TypeError(six.u('parse() accepts a seek-able file handle                            , not %s' % type(file_handle).__name__))
        ofx_obj = Ofx()
        ...
        if stmttrnrs:
            ...
            if stmttrnrs_status:
                ofx_obj.status = {}
                ofx_obj.status['code'] = int(stmttrnrs_status.find('code').contents[0].strip())
                ofx_obj.status['severity'] = stmttrnrs_status.find('severity').contents[0].strip()
                message = stmttrnrs_status.find('message')
                ofx_obj.status['message'] = message.contents[0].strip() if message else None
        ...
        if ccstmttrnrs:
            ...
            if ccstmttrnrs_status:
                ofx_obj.status = {}
                ofx_obj.status['code'] = int(ccstmttrnrs_status.find('code').contents[0].strip())
                ofx_obj.status['severity'] = ccstmttrnrs_status.find('severity').contents[0].strip()
        ...
        if ccstmtrs_ofx:
            ofx_obj.accounts += cls.parseStmtrs(ccstmtrs_ofx, AccountType.CreditCard)
    ...
    def parseOfxDateTime(cls, ofxDateTime):
        ...
        try:
            ...
            if ofxDateTime[:8] == '00000000':
                ...
                return datetime.datetime.strptime(ofxDateTime[:8], cls.custom_date_format) - timeZoneOffset + msec
        ...
        for invstmtrs_ofx in invstmtrs_list:
            ...
            if hasattr(acctid_tag, 'contents'):
                try:
                    ...
                    account.warnings.append(six.u('Empty acctid tag for %s') % invstmtrs_ofx)
                    if cls.fail_fast:
                        raise
            ...
            if hasattr(brokerid_tag, 'contents'):
                try:
                    ...
                    account.warnings.append(six.u('Empty brokerid tag for %s') % invstmtrs_ofx)
                    if cls.fail_fast:
                        raise

            account.type = AccountType.Investment
            if invstmtrs_ofx:
                account.statement = cls.parseInvestmentStatement(invstmtrs_ofx)
            ret.append(account)
    ...
    def parseSeclist(cls, seclist_ofx):
        ...
        for secinfo_ofx in seclist_ofx.findAll('secinfo'):
            ...
            if uniqueid_tag and name_tag:
                ...
                securityList.append(Security(uniqueid_tag.contents[0].strip(), name_tag.contents[0].strip(), ticker, memo))
    ...
    def parseInvestmentTransaction(cls, ofx):
        ...
        if hasattr(tag, 'contents'):
            try:
                transaction.tradeDate = cls.parseOfxDateTime(tag.contents[0].strip())
            except ValueError:
                raise
        ...
        if hasattr(tag, 'contents'):
            try:
                transaction.settleDate = cls.parseOfxDateTime(tag.contents[0].strip())
            except ValueError:
                raise
    ...
    def parseInvestmentStatement(cls, invstmtrs_ofx):
        ...
        if invtranlist_ofx is not None:
            ...
            if hasattr(tag, 'contents'):
                try:
                    statement.start_date = cls.parseOfxDateTime(tag.contents[0].strip())
                except IndexError:
            ...
            if hasattr(tag, 'contents'):
                try:
                    statement.end_date = cls.parseOfxDateTime(tag.contents[0].strip())
                except IndexError:
         ...
         'posdebt']:
            try:
                for investment_ofx in invstmtrs_ofx.findAll(transaction_type):
                    statement.positions.append(cls.parseInvestmentPosition(investment_ofx))
             ...
             TypeError):
                ...
                if cls.fail_fast:
                    raise
                statement.discarded_entries.append({six.u('error'): six.u('Error parsing positions:                         ') + str(e), 
                   six.u('content'): investment_ofx})

        for transaction_type in InvestmentTransaction.AGGREGATE_TYPES:
            try:
                for investment_ofx in invstmtrs_ofx.findAll(transaction_type):
                    statement.transactions.append(cls.parseInvestmentTransaction(investment_ofx))

            except (ValueError, IndexError, decimal.InvalidOperation):
                e = sys.exc_info()[1]
                if cls.fail_fast:
                    raise
                statement.discarded_entries.append({six.u('error'): transaction_type + ': ' + str(e), six.u('content'): investment_ofx})

        for transaction_ofx in invstmtrs_ofx.findAll('invbanktran'):
            for stmt_ofx in transaction_ofx.findAll('stmttrn'):
                try:
                    statement.transactions.append(cls.parseTransaction(stmt_ofx))
        ...
        if hasattr(bal_tag, 'contents'):
            ...
            if hasattr(balamt_tag, 'contents'):
                try:
                    ...
                    statement.warnings.append(six.u('%s balance amount was empty for                             %s') % (bal_type_string, stmt_ofx))
                    if cls.fail_fast:
                        raise OfxParserException('Empty %s balance                            ' % bal_type_string)

            if hasattr(dtasof_tag, 'contents'):
                try:
                    setattr(statement, bal_date_attr, cls.parseOfxDateTime(dtasof_tag.contents[0].strip()))
                except IndexError:
                    statement.warnings.append(six.u('%s balance date was empty for %s                            ') % (bal_type_string, stmt_ofx))
                    if cls.fail_fast:
                        raise
                except ValueError:
                    statement.warnings.append(six.u('%s balance date was not allowed for                             %s') % (bal_type_string, stmt_ofx))
                    if cls.fail_fast:
                        raise
    ...
    def parseStatement(cls, stmt_ofx):
        ...
        if hasattr(dtstart_tag, 'contents'):
            try:
                statement.start_date = cls.parseOfxDateTime(dtstart_tag.contents[0].strip())
            except IndexError:
                statement.warnings.append(six.u('Statement start date was empty for %s') % stmt_ofx)
                if cls.fail_fast:
                    raise
            except ValueError:
                statement.warnings.append(six.u('Statement start date was not allowed for                         %s') % stmt_ofx)
                if cls.fail_fast:
                    raise
        ...
        if hasattr(dtend_tag, 'contents'):
            try:
                statement.end_date = cls.parseOfxDateTime(dtend_tag.contents[0].strip())
            except IndexError:
                statement.warnings.append(six.u('Statement start date was empty for %s') % stmt_ofx)
                if cls.fail_fast:
                    raise
            except ValueError:
                msg = six.u('Statement start date was not formatted correctly for %s')
        ...
        if hasattr(currency_tag, 'contents'):
            try:
                ...
                statement.warnings.append(six.u('Currency definition was empty for %s') % stmt_ofx)
                if cls.fail_fast:
                    raise

        cls.parseBalance(statement, stmt_ofx, 'ledgerbal', 'balance', 'balance_date', 'ledger')
        cls.parseBalance(statement, stmt_ofx, 'availbal', 'available_balance', 'available_balance_date', 'ledger')
        for transaction_ofx in stmt_ofx.findAll('stmttrn'):
            try:
                statement.transactions.append(cls.parseTransaction(transaction_ofx))
            except OfxParserException:
                ofxError = sys.exc_info()[1]
                statement.discarded_entries.append({'error': str(ofxError), 'content': transaction_ofx})
                if cls.fail_fast:
    ...
    def parseTransaction(cls, txn_ofx):
        ...
        if hasattr(type_tag, 'contents'):
            try:
                ...
                raise OfxParserException(six.u('No Transaction type (a required field)'))
        ...
        if hasattr(name_tag, 'contents'):
            try:
                ...
                raise OfxParserException(six.u('No Transaction name (a required field)'))
        ...
        if hasattr(amt_tag, 'contents'):
            try:
                ...
                if amt_tag.contents[0].strip() in ('null', '-null'):
                    ...
                    raise OfxParserException(six.u("Invalid Transaction Amount: '%s'") % amt_tag.contents[0])
            except TypeError:
                raise OfxParserException(six.u('No Transaction Amount (a required field)'))

        else:
            raise OfxParserException(six.u('Missing Transaction Amount (a required field)'))
        date_tag = txn_ofx.find('dtposted')
        if hasattr(date_tag, 'contents'):
            try:
                transaction.date = cls.parseOfxDateTime(date_tag.contents[0].strip())
            except IndexError:
                raise OfxParserException('Invalid Transaction Date')
            except ValueError:
                ve = sys.exc_info()[1]
                ...
                raise OfxParserException(six.u('No Transaction Date (a required field)'))

        else:
            raise OfxParserException(six.u('Missing Transaction Date (a required field)'))
