...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/ofxparse/ofxprinter.py
# Compiled at: 2018-07-30 14:59:10
import six
...
class OfxPrinter:
    ...
    def writeLine(self, data, tabs=0, term=None):
        ...
        return self.out_handle.write(('{0}{1}{2}').format(tabbing, data, term))
    ...
    def writeTrn(self, trn, tabs=5):
        ...
        self.writeLine(('<DTPOSTED>{}').format(self.printDate(trn.date)), tabs=tabs)
        ...
        if len(str(trn.checknum)) > 0:
            self.writeLine(('<CHECKNUM>{}').format(trn.checknum), tabs=tabs)
    ...
    def writeLedgerBal(self, statement, tabs=4):
        ...
        if bal and baldt:
            ...
            self.writeLine(('<DTASOF>{0}').format(self.printDate(baldt)), tabs=tabs + 1)
            self.writeLine('</LEDGERBAL>', tabs=tabs)
        return
    ...
    def writeStmTrs(self, tabs=3):
        for acct in self.ofx.accounts:
            ...
            tabs += 1
            if acct.curdef:
                self.writeLine(('<CURDEF>{0}').format(acct.curdef), tabs=tabs)
            if acct.routing_number or acct.account_id or acct.account_type:
                ...
                if acct.routing_number:
                    self.writeLine(('<BANKID>{0}').format(acct.routing_number), tabs=tabs + 1)
                if acct.account_id:
                    self.writeLine(('<ACCTID>{0}').format(acct.account_id), tabs=tabs + 1)
                if acct.account_type:
                    self.writeLine(('<ACCTTYPE>{0}').format(acct.account_type), tabs=tabs + 1)
                self.writeLine('</BANKACCTFROM>', tabs=tabs)
            self.writeLine('<BANKTRANLIST>', tabs=tabs)
            tabs += 1
            self.writeLine(('<DTSTART>{0}').format(self.printDate(acct.statement.start_date)), tabs=tabs)
            self.writeLine(('<DTEND>{0}').format(self.printDate(acct.statement.end_date)), tabs=tabs)
    ...
    def writeBankMsgsRsv1(self, tabs=1):
        ...
        if self.ofx.trnuid is not None:
            self.writeLine(('<TRNUID>{0}').format(self.ofx.trnuid), tabs=tabs)
        if self.ofx.status:
            self.writeLine('<STATUS>', tabs=tabs)
            self.writeLine(('<CODE>{0}').format(self.ofx.status['code']), tabs=tabs + 1)
            self.writeLine(('<SEVERITY>{0}').format(self.ofx.status['severity']), tabs=tabs + 1)
            self.writeLine('</STATUS>', tabs=tabs)
