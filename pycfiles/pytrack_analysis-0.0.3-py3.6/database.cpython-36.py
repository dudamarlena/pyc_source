...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/pytrack_analysis/database.py
# Compiled at: 2017-07-27 07:09:47
# Size of source mod 2**32: 11148 bytes
import os, sys, time, numpy as np, pandas as pd, yaml, json
...
def check_meta(_dict, _dir):
    ...
    for key, val in _dict.items():
        for expkeys in val.keys():
            if expkeys not in os.listdir(_dir):
                flag = 1
...
def check_data(_dict, _dir):
    ...
    for key, val in _dict.items():
        for expkeys, v2 in val.items():
            for session in v2:
                if session not in os.listdir(_dir):
                    flag = 1
...
class Node(object):
    __doc__ = ' Node object for GraphDict '
...
class Session(object):
    ...
    def load(self, load_as='pd'):
        ...
        if load_as == 'pd':
            ...
            data = data.rename(columns={' body_x': 'body_x'})
        else:
            if load_as == 'np':
