...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/dynamodb_meta_store/meta_store.py
# Compiled at: 2020-03-21 08:12:33
# Size of source mod 2**32: 9683 bytes
from boto3.dynamodb.conditions import Key
from dynamodb_meta_store.exceptions import TableNotReadyException, MisconfiguredSchemaException, ItemNotFound
import logging, boto3, decimal
log = logging.getLogger(__name__)

class DynamoDBMetaStore(object):
    __doc__ = ' DynamoDB Config Store instance '

    def __init__(self, table_name, store_name, aws_region=None, connection=None, store_key='_store', option_key='_option', create_table=False, read_units=1, write_units=1):
            ...
            if status not in ('ACTIVE', 'UPDATING'):
                ...
                for key in schema:
                    if key['AttributeName'] == self.store_key:
                        if key['KeyType'] == 'HASH':
                            hash_found = True
                    if key['AttributeName'] == self.option_key and key['KeyType'] == 'RANGE':
                        range_found = True

                raise hash_found and range_found or MisconfiguredSchemaException
    ...
    def _create_table(self):
        ...
        """
        table = self.connection.create_table(TableName=(self.table_name),
          KeySchema=[
         ...
         {'AttributeName':self.option_key, 
          'AttributeType':'S'}],
          ProvisionedThroughput={'ReadCapacityUnits':self.read_units, 
    ...
    def get_option(self, option, keys=None):
        ...
        :returns: dict -- Dictionary with all data; {"key": "value"}
        """
        response = self.table.get_item(Key={self.store_key: self.store_name, 
    ...
    def query(self, partition_filter, total_items=None, start_key=None):
        """
        ...
        if not start_key:
            response = self.table.query(KeyConditionExpression=(Key(pk).eq(pkv)))
        else:
            response = self.table.query(KeyConditionExpression=(Key(pk).eq(pkv)),
        ...
        if response.get('LastEvaluatedKey'):
            ...
            return_items = self.query_item(partition_key=partition_filter,
    ...
    def __del__(self):
        self.connection.meta.client._endpoint.http_session.close()
