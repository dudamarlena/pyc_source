...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kvndb/cmdclient.py
# Compiled at: 2016-11-17 09:23:11
...
class KVNDBCmdClient(cmd.Cmd):
    ...
    def do_get(self, cmd):
        ...
        key = cmd
        try:
            value = threads.blockingCallFromThread(self.reactor, self.proto.get, key)
        except KeyError:
    ...
    def do_getkeys(self, cmd):
        ...
        if cmd:
            ...
            cols = 2
        try:
            keys = threads.blockingCallFromThread(self.reactor, self.proto.getkeys)
        except Exception as e:
            self.stdout.write(('Error: {e}\n').format(e=e))
            return
