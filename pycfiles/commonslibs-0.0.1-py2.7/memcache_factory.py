...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/commonslib/memcache_factory.py
    ...
    def get_instance(cls, session, runmod=None):
        ...
        if memcache_factory is not None:
            ...
            if memcache_config is not None:
                memcache_factory = MemCacheFactory(memcache_config.get('servers'), behaviors=memcache_config.get('behaviors'), binary=memcache_config.get('binary'), username=memcache_config.get('username'), password=memcache_config.get('password'))
            ...
            if factory is None:
                ...
                if len(args) == 1 and isinstance(args[0], list):
                    ...
                    if len(cache_keys) > len(cache_obj_dict):
                        ...
                        no_cached_ids = [ long(no_cache_key) for no_cache_key in list(set(str_keys).difference(set(cached_keys))) ]
