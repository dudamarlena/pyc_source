...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/commonslib/oss_factory.py
...
class OssFactory(object):
    ...
    def download_site(name, runmod='development'):
        ...
        oss_file_config = OssFactory.oss_file_config.get(name)
    ...
    def get_instance(name, runmod='development'):
        ...
        oss_file_config = OssFactory.oss_file_config.get(name)
        if oss_file_config:
            return OssFactory(regional_node=oss_file_config.get('regional_node'), id=oss_file_config.get('id'), key=oss_file_config.get('key'), bucket=oss_file_config.get('bucket'))
        else:
            return
            return

    def __init__(self, regional_node=None, id=None, key=None, bucket='eking-test1', max_idle_time=25200):
