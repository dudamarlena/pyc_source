...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/qoalai/object_detector/yolo.py
# Compiled at: 2020-03-21 07:20:53
# Size of source mod 2**32: 14648 bytes
"""
    File name: yolo.py
...
"""
import json, random, tensorflow as tf
from qoalai.tensor_operations import *
from qoalai.object_detector.detector_utils import *
...
class Yolo(ObjectDetector):
 ...
 (10, 13), (16, 30), (33, 23), (30, 61), (62, 45), (59, 119), (116, 90), (156, 198), (373, 326)], dropout_rate=0.8, leaky_relu_alpha=0.1, convert_to_tflite=False):
        """[summary]
        ...
        """
        super(Yolo, self).__init__(num_of_class=num_of_class, input_height=input_height,
    ...
    def build_net(self, input_tensor, network_type='big', is_training=False):
        """[summary]
    ...
    def batch_generator(self, batch_size, dataset_path, message):
        ...
        while True:
            ...
            yield (
             np.array(x_batch), [np.array(y_pred1), np.array(y_pred2), np.array(y_pred3)])
    ...
    def optimize(self, subdivisions, iterations, best_loss, train_generator, val_generator, save_path):
        ...
        for i in range(iterations):
            ...
            print('eph: ', i, 'ttl loss: ', round(total, 2), 'obj loss: ', round(obj * 100, 2), 'noobj loss: ', round(noobj * 100, 2), 'ctr loss: ', round(ctr * 100, 2), 'size loss: ', round(size * 100, 2), round(class_l * 100, 2), sign)
    ...
    def check_val_data(self, val_generator):
        ...
        total, obj, noobj, ctr, size, class_l, iou_avg_val, obj_acc_val, noobj_acc_val, class_acc_val = self.session.run([self.all_losses,
         ...
         self.noobj_acc_avg,
         self.class_acc_avg], val_feed_dict)
        print('> iou: ', round(iou_avg_val * 100, 3), 'obj acc: ', round(obj_acc_val * 100, 3), 'noobj acc: ', round(noobj_acc_val * 100, 3), 'class acc: ', round(class_acc_val * 100, 3))
        print('ttl loss: ', round(total, 2), 'obj loss: ', round(obj * 100, 2), 'noobj loss: ', round(noobj * 100, 2), 'ctr loss: ', round(ctr * 100, 2), 'size loss: ', round(size * 100, 2), round(class_l * 100, 2))