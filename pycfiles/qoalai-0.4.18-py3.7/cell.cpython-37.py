...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/qoalai/networks/cell.py
# Compiled at: 2019-12-06 02:20:09
...
class NASBaseCell(object):
    __doc__ = "NASNet Cell class that is used as a 'layer' in image architectures.\n  Args:\n    num_conv_filters: The number of filters for each convolution operation.\n    operations: List of operations that are performed in the NASNet Cell in\n      order.\n    used_hiddenstates: Binary array that signals if the hiddenstate was used\n      within the cell. This is used to determine what outputs of the cell\n      should be concatenated together.\n    hiddenstate_indices: Determines what hiddenstates should be combined\n      together with the specified operations to create the NASNet cell.\n  "

    def __init__(self, num_conv_filters, operations, used_hiddenstates, hiddenstate_indices, drop_path_keep_prob, total_num_cells, total_training_steps):
            ...
            for i in range(int(len(self._operations) / 2)):
                with tf.variable_scope('comb_iter_{}'.format(i)):
                    ...
                    with tf.variable_scope('left'):
                        h1 = self._apply_operation(h1, self._operations[(i * 2)], stride, self._hiddenstate_indices[(i * 2)] < 2)
                    with tf.variable_scope('right'):
                        h2 = self._apply_operation(h2, self._operations[(i * 2 + 1)], stride, self._hiddenstate_indices[(i * 2 + 1)] < 2)
        ...
        if 'separable' in operation:
            ...
            for layer_num in range(num_layers):
                ...
                net = slim.batch_norm(net,
                  scope=('bn_sep_{0}x{0}_{1}'.format(kernel_size, layer_num + 1)))
                stride = 1

        else:
            if operation in ('none', ) and not stride > 1:
...
class PNASCell(NASBaseCell):
    __doc__ = 'PNASNet Cell.'

    def __init__(self, num_conv_filters, drop_path_keep_prob, total_num_cells, total_training_steps):
        operations = [
        ...
        super(PNASCell, self).__init__(num_conv_filters, operations, used_hiddenstates, hiddenstate_indices, drop_path_keep_prob, total_num_cells, total_training_steps)