...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/qoalai/networks/modified_densenet.py
# Compiled at: 2019-10-16 04:54:07
...
def _conv(inputs, num_filters, kernel_size, stride=1, dropout_rate=None, scope=None, outputs_collections=None):
    with tf.variable_scope(scope, 'xx', [inputs]) as (sc):
        ...
        net = tf.nn.relu(net)
        net = slim.conv2d(net, num_filters, kernel_size, stride)
        if dropout_rate:
...
@slim.add_arg_scope
def _transition_block(inputs, num_filters, compression=1.0, scope=None, outputs_collections=None, code=0):
    ...
    with tf.variable_scope(scope, 'transition_blockx', [inputs]) as (sc):
        ...
        if code == 0:
            ...
            s = 2
        net = _conv(net, num_filters, 1, s, scope='blk')
        net = slim.utils.collect_named_outputs(outputs_collections, sc.name, net)
    return (net, num_filters)
