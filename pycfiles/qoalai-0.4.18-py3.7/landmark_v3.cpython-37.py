...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/qoalai/landmarks/landmark_v3.py
# Compiled at: 2019-10-10 07:18:26
# Size of source mod 2**32: 15175 bytes
...
class Landmark(object):
    ...
    def __init__(self, num_landmark_point, input_height=300, input_width=300, input_channel=3, grid_num=3, threshold=0.5):
        """Constructor
    ...
    def build_densenet_base(self, input_tensor, dropout_rate, is_training, top_layer_depth):
        ...
        out = new_conv2d_layer(input=out, filter_shape=[
         2, 2, out.get_shape().as_list()[(-1)], top_layer_depth],
         ...
         1, 2, 2, 1],
          ...
          use_bias=False,
          use_batchnorm=False)
        self.point_map_nonlogit = new_conv2d_layer(input=out, filter_shape=[
         2, 2, out.get_shape().as_list()[(-1)], self.num_landmark_point],
        ...
        out_x = new_fc_layer(out, num_inputs=(int(out.get_shape().as_list()[(-1)])),
          num_outputs=1000,
          dropout_val=(1.0 - dropout_rate),
          activation='NONE',
          is_training=is_training,
          use_bias=False)
        out_x = new_fc_layer(out_x, num_inputs=(int(out_x.get_shape().as_list()[(-1)])),
          num_outputs=(self.num_landmark_point),
          dropout_val=1.0,
          activation='sigmoid',
          is_training=is_training,
          use_bias=False)
        out_y = new_fc_layer(out, num_inputs=(int(out.get_shape().as_list()[(-1)])),
          num_outputs=1000,
          dropout_val=(1.0 - dropout_rate),
          activation='NONE',
          is_training=is_training,
          use_bias=False)
        out_y = new_fc_layer(out_y, num_inputs=(int(out_y.get_shape().as_list()[(-1)])),
          num_outputs=(self.num_landmark_point),
          dropout_val=1.0,
          activation='sigmoid',
          is_training=is_training,
    ...
    def optimize(self, iteration, subdivition, cost_tensor, optimizer_tensor, session, saver, train_generator, val_generator, best_loss, path_tosave_model='model/model1'):
        ...
        for i in range(iteration):
            ...
            print('>> epoch: ', i, 'train loss: ', round(t_loss, 3), 'val loss: ', round(loss, 3), 'val map acc: ', round(map_acc, 3), round(pml, 3), round(ppl, 3), sign)