...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/qoalai/green_assesment/greenness.py
# Compiled at: 2019-04-29 06:46:59
# Size of source mod 2**32: 5856 bytes
import cv2, numpy as np

class GreenLevelAssesement:

    def __init__(self, resize_height, resize_width, grid_height, grid_width):
        """Constructor
        ...
        self.kernel = np.ones((3, 3), np.float32) / 9.0
        ...
        self.green_class_dict['a'] = np.full((self.grid_height, self.grid_width, 3), (19,
                                                                                      81,
                                                                                      27)).astype(np.uint8)
        self.green_class_dict['b'] = np.full((self.grid_height, self.grid_width, 3), (31,
                                                                                      135,
                                                                                      44)).astype(np.uint8)
        self.green_class_dict['c'] = np.full((self.grid_height, self.grid_width, 3), (95,
                                                                                      196,
                                                                                      45)).astype(np.uint8)
        self.green_class_dict['d'] = np.full((self.grid_height, self.grid_width, 3), (75,
                                                                                      242,
                                                                                      110)).astype(np.uint8)
        self.green_class_dict['e'] = np.full((self.grid_height, self.grid_width, 3), (76,
                                                                                      242,
                                                                                      177)).astype(np.uint8)
        self.green_class_dict['f'] = np.full((self.grid_height, self.grid_width, 3), (76,
                                                                                      200,
                                                                                      200)).astype(np.uint8)
        self.green_class_dict['g'] = np.full((self.grid_height, self.grid_width, 3), (76,
                                                                                      200,
                                                                                      255)).astype(np.uint8)
        self.green_class_dict['h'] = np.full((self.grid_height, self.grid_width, 3), (76,
                                                                                      165,
                                                                                      255)).astype(np.uint8)
        self.green_class_dict['i'] = np.full((self.grid_height, self.grid_width, 3), (76,
                                                                                      135,
                                                                                      255)).astype(np.uint8)
        self.green_class_dict['j'] = np.full((self.grid_height, self.grid_width, 3), (76,
                                                                                      100,
                                                                                      255)).astype(np.uint8)

    def get_green_index(self, img, alpha=0, beta=0, method=cv2.NORM_MINMAX, dtype=cv2.CV_32F):
        ...
        norm_img = cv2.normalize(img, None,
          alpha=alpha,
          beta=beta,
          norm_type=method,
          dtype=dtype)
