...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/qoalai/landmarks/landmark_v5.py
# Compiled at: 2019-10-23 10:56:42
# Size of source mod 2**32: 13970 bytes
...
class Landmark(object):
    ...
    def __init__(self, num_landmark_point, input_height=300, input_width=300, input_channel=3, grid_num=19, threshold=0.6):
        """Constructor
    ...
    def build_densenet_base(self, input_tensor, dropout_rate, is_training, top_layer_depth):
        ...
        out = new_conv2d_layer(input=out, filter_shape=[
         2, 2, out.get_shape().as_list()[(-1)], self.num_landmark_point + 2],
    ...
    def calculate_loss(self):
        ...
        label_map = 1.0 - 2 * tf.sqrt(tf.square(label_deltax) + tf.square(label_deltay))
        ...
        for k in range(self.num_landmark_point):
            for i in range(self.grid_num):
                for j in range(self.grid_num):
                    ...
                    tmp = 1.0 - 2 * math.sqrt(tmpx * tmpx + tmpy * tmpy)
                    if tmp > biggest_val:
                        biggest_val = tmp
                        nearest_point = k
    ...
    def optimize(self, iteration, subdivition, cost_tensor, optimizer_tensor, session, saver, train_generator, val_generator, best_loss, path_tosave_model='model/model1'):
        ...
        for i in range(iteration):
            ...
            print('>> epoch: ', i, 'train loss: ', round(t_loss, 3), 'val loss: ', round(loss, 3), 'val map acc: ', round(map_acc, 3), 'val map loss: ', round(pml, 3), 'val poss loss: ', round(ppl, 3), sign)