...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/qoalai/segmentations/deeplab_inceptionresnetv2.py
# Compiled at: 2019-09-05 02:12:21
...
class DeepLab(InceptionV4ResnetV2):
    ...
    def __init__(self, num_classes, input_tensor=None, input_shape=(None, 300, 300, 3), output_stride=16, is_training=True):
        """[summary]
        ...
        if self.num_classes == 1:
            output_shape = (
             None, self.input_height, self.input_width, 1)
        else:
            output_shape = (
             None, self.input_height, self.input_width, 3)
    ...
    def atrous_spatial_pyramid_pooling(self, inputs, output_stride):
        ...
        conv_1x1_1 = self.conv_block(inputs=inputs, filters=128,
          kernel_size=(1, 1),
          strides=(1, 1))
        input_depth = inputs.get_shape().as_list()[(-1)]
        kernel = np.random.random((3, 3, input_depth, 128)).astype(np.float32)
        conv_3x3_1 = tf.nn.atrous_conv2d(value=inputs, filters=kernel,
          rate=(atrous_rates[0]),
          padding='SAME',
          name='atrous1')
        conv_3x3_2 = tf.nn.atrous_conv2d(value=inputs, filters=kernel,
          rate=(atrous_rates[1]),
          padding='SAME',
          name='atrous2')
        conv_3x3_3 = tf.nn.atrous_conv2d(value=inputs, filters=kernel,
          rate=(atrous_rates[2]),
          padding='SAME',
          name='atrous3')
        concat_1 = tf.concat([conv_1x1_1, conv_3x3_1, conv_3x3_2, conv_3x3_3], -1)
        conv_1x1_2 = self.conv_block(inputs=concat_1, filters=128,
          kernel_size=(1, 1),
          strides=(1, 1))
    ...
    def create_deeplab(self, inputs):
        ...
        with tf.variable_scope('deeplab'):
            ...
            conv_1x1 = self.conv_block(inputs=inputs, filters=128,
              kernel_size=(1, 1),
              strides=(1, 1),
              padding='same')
            atrous_output = self.atrous_spatial_pyramid_pooling(inputs=inputs, output_stride=16)
            concat_1 = tf.concat([atrous_output, conv_1x1], -1)
            if self.num_classes == 1:
                filters = 1
            else:
                filters = 3
            conv_3x3 = self.conv_block(inputs=concat_1, filters=filters,
              kernel_size=(1, 1),
              strides=(1, 1),
              activation=None)
