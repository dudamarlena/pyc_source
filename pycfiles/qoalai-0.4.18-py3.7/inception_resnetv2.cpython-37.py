...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/qoalai/networks/inception_resnetv2.py
# Compiled at: 2019-09-05 02:09:43
# Size of source mod 2**32: 18012 bytes
...
class InceptionV4ResnetV2(object):
    ...
    def conv_block(self, inputs, filters, kernel_size, strides=(2, 2), padding='valid', dropout_rate=0.2, activation='relu'):
        """[summary]
        ...
        Returns:
            [type] -- [description]
        """
        conv = tf.layers.conv2d(inputs=inputs, filters=filters,
          kernel_size=kernel_size,
          strides=strides,
          padding=padding,
          activation=activation)
        conv = tf.layers.batch_normalization(inputs=conv, training=(self.is_training))
        conv = tf.layers.batch_normalization(inputs=conv, axis=3, momentum=0.9, epsilon=1e-05, scale=True, training=(self.is_training))
    ...
    def stem_block(self, input_tensor):
        ...
        Returns:
            [type] -- [description]
        """
        conv_1 = self.conv_block(inputs=input_tensor, filters=32,
          kernel_size=(3, 3),
          strides=(2, 2))
        conv_2 = self.conv_block(inputs=conv_1, filters=32,
          kernel_size=(3, 3),
          strides=(1, 1))
        conv_3 = self.conv_block(inputs=conv_2, filters=64,
          kernel_size=(3, 3),
          padding='same',
          strides=(1, 1))
        conv_4 = self.conv_block(inputs=conv_3, filters=96,
          kernel_size=(3, 3),
          strides=(2, 2))
        max_pool_1 = tf.layers.max_pooling2d(inputs=conv_3, pool_size=(3, 3), strides=(2,
                                                                                       2))
        concat_1 = tf.concat([conv_4, max_pool_1], -1)
        conv_5 = self.conv_block(inputs=concat_1, filters=64,
          kernel_size=(3, 3),
          strides=(1, 1))
        conv_6 = self.conv_block(inputs=conv_5, filters=64,
          kernel_size=(7, 1),
          padding='same',
          strides=(1, 1))
        conv_7 = self.conv_block(inputs=conv_6, filters=64,
          kernel_size=(1, 7),
          padding='same',
          strides=(1, 1))
        conv_8 = self.conv_block(inputs=conv_7, filters=96,
          kernel_size=(3, 3),
          padding='same',
          strides=(1, 1))
        conv_9 = self.conv_block(inputs=concat_1, filters=64,
          kernel_size=(1, 1),
          strides=(1, 1))
        conv_10 = self.conv_block(inputs=conv_9, filters=96,
          kernel_size=(3, 3),
          strides=(1, 1))
        concat_2 = tf.concat([conv_8, conv_10], -1)
        max_pool_2 = tf.layers.max_pooling2d(inputs=concat_2, pool_size=(3, 3), strides=(2,
                                                                                         2))
        conv_11 = self.conv_block(inputs=concat_2, filters=192,
          kernel_size=(3, 3),
          strides=(2, 2))
        concat_3 = tf.concat([max_pool_2, conv_11], -1)
        return concat_3

    def inception_resnet_a(self, inputs, drop_out=0.8, activation='NONE', use_bias=True, use_batchnorm=True):
        """[summary]
