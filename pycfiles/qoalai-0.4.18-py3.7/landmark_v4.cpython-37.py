...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/qoalai/landmarks/landmark_v4.py
# Compiled at: 2019-11-01 07:05:43
# Size of source mod 2**32: 15539 bytes
import json, cv2, math, random, numpy as np, tensorflow as tf
...
class Landmark(object):
    ...
    def __init__(self, num_landmark_point, input_height=300, input_width=300, input_channel=3, grid_num=19, threshold=0.7, pos_loss_alpha=1.0, neg_loss_alpha=1.2, position_loss_alpha=1.0):
        """Constructor
        ...
        self.grid_size_relative = 1.0 / self.grid_num
        self.threshold = threshold
        self.pos_loss_alpha = pos_loss_alpha
        self.neg_loss_alpha = neg_loss_alpha
        self.position_loss_alpha = position_loss_alpha
    ...
    def build_densenet_base(self, input_tensor, dropout_rate, is_training, top_layer_depth):
        """[summary]
        ...
        with slim.arg_scope(arg_scoope):
            out = densenet.densenet121(inputs=input_tensor, num_classes=1,
              is_training=is_training)
            base_var_list = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)
        print('===========', out)
        out = new_conv2d_layer(input=out, filter_shape=[
         2, 2, out.get_shape().as_list()[(-1)], 3],
          dropout_val=(1.0 - dropout_rate),
          activation='NONE',
          padding='SAME',
          strides=[
         1, 1, 1, 1],
          ...
          use_batchnorm=False)
        print('===>>>', out)
        self.point_map = tf.nn.sigmoid(out[:, :, :, :1])
        self.point_position = tf.nn.tanh(out[:, :, :, 1:])
        print('--------------------INFO---------------------')
    ...
    def calculate_loss(self):
        ...
        Returns:
            [type] -- [description]
        """
        pred_mapx = self.point_position[:, :, :, :1] * self.grid_size_relative
        pred_mapy = self.point_position[:, :, :, 1:] * self.grid_size_relative
        label_mapx = self.point_position_placeholder[:, :, :, :1] * self.grid_size_relative
        label_mapy = self.point_position_placeholder[:, :, :, 1:] * self.grid_size_relative
        label_deltax = label_mapx - pred_mapx
        label_deltay = label_mapy - pred_mapy
        label_map = 1.0 - tf.sqrt(tf.square(label_deltax) + tf.square(label_deltay))
        label_map_pos_tmp = tf.math.greater(label_map, tf.convert_to_tensor(np.array(self.threshold), tf.float32))
        label_map_pos_tmp = tf.cast(label_map_pos_tmp, tf.float32)
        label_map_pos = label_map * label_map_pos_tmp
        pred_map_pos = self.point_map * label_map_pos_tmp
        self.point_map_loss_pos = self.pos_loss_alpha * mse_loss_mean(pred_map_pos, label_map_pos)
        label_map_neg_tmp = 1.0 - label_map_pos_tmp
        label_map_neg = label_map * label_map_neg_tmp
        pred_map_neg = self.point_map * label_map_neg_tmp
        self.point_map_loss_neg = self.neg_loss_alpha * mse_loss_mean(pred_map_neg, label_map_neg)
        ...
        self.point_position_loss = self.position_loss_alpha * mse_loss_sum(tmp_point_position, tmp_point_position_placeholder) / tf.math.reduce_sum(tmp_point_map)
        self.total_loss = self.point_map_loss_pos + self.point_map_loss_neg + self.point_position_loss
    ...
    def read_target(self, file_path):
        point_map_lable = np.zeros((self.grid_num, self.grid_num, 3), dtype=(np.float32))
        x_points = []
        y_points = []
        ...
        for i in range(self.grid_num):
            for j in range(self.grid_num):
                ...
                for k in range(self.num_landmark_point):
                    ...
                    tmp = 1.0 - math.sqrt(tmpx * tmpx + tmpy * tmpy)
                    if tmp > biggest_val:
                        biggest_val = tmp
                        nearest_point = k
        ...
        return (
         point_map_lable[:, :, :1], point_map_lable[:, :, 1:])
    ...
    def batch_generator(self, batch_size, dataset_path, message):
        ...
        while True:
            ...
            yield (
             np.array(x_batch), np.array(y_pred_map), np.array(y_pred_position))

    def optimize(self, iteration, subdivition, cost_tensor, optimizer_tensor, session, saver, train_generator, val_generator, best_loss, path_tosave_model='model/model1'):
        """[summary]
        ...
        for i in range(iteration):
            ...
            loss = session.run(cost_tensor, feed_dict)
            map_acc, pml_pos, pml_neg, ppl = session.run([self.point_map_acc, self.point_map_loss_pos, self.point_map_loss_neg, self.point_position_loss], feed_dict)
            ...
            print('>> epoch: ', i, 'train loss: ', round(t_loss, 3), 'val loss: ', round(loss, 3), 'val map acc: ', round(map_acc, 3), 'val map pos loss: ', round(pml_pos, 3), 'val map neg loss: ', round(pml_neg, 3), 'val poss loss: ', round(ppl, 3), sign)