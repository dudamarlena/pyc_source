...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/qoalai/transfer_learning/image_recognition.py
...
"""
import json, cv2, random, numpy as np, tensorflow as tf
...
class ImageRecognition(object):
    ...
    def __init__(self, classes, input_height=400, input_width=400, input_channel=3):
        """Constructor
    ...
    def build_resnetv2(self, input_tensor, is_training, top_layer_depth=128):
        """[summary]
        ...
        with slim.arg_scope(resnet_arg_scope()):
            out, end_points = resnet_v2_101(inputs=input_tensor, num_classes=1001,
        ...
        with tf.variable_scope('resnet_v2_101'):
            ...
            out = new_fc_layer(out, num_inputs=depth,
    ...
    def build_inceptionv4_basenet(self, input_tensor, is_training=False, final_endpoint='Mixed_7d', top_layer_depth=128):
        ...
        with slim.arg_scope(arg_scope):
            out, end_points = inception_v4(input_tensor, num_classes=1,
        ...
        with tf.variable_scope('InceptionV4'):
            ...
            while True:
                if size == 1:
                    break
                out = new_conv2d_layer(out, filter_shape=[
                 3, 3, out.get_shape().as_list()[(-1)], top_layer_depth],
                  name='cv1',
                  dropout_val=0.85,
                  activation='LRELU',
                  lrelu_alpha=0.2,
    ...
    def build_densenet_base(self, input_tensor, dropout_rate, is_training, top_layer_depth=128):
        """[summary]
    ...
    def build_pnasnet_large_base(self, input_tensor, is_training):
        out, _ = build_pnasnet_large(images=input_tensor, num_classes=(len(self.classes)),
          is_training=is_training,
          final_endpoint=None,
          config=None)
        base_var_list = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)
    ...
    def build_vgg19_base(self, input_tensor, dropout_rate, is_training):
        """[summary]
        ...
        with slim.arg_scope(arg_scope):
            out, _ = vgg_19(inputs=input_tensor, dropout_keep_prob=(1.0 - dropout_rate),
              is_training=is_training,
            ...
            for i in self.classes:
                for j in range(perclass_sample):
                    ...
                    try:
                        ...
                        tmp_y[self.classes.index(i)] = 1.0
    ...
    def optimize(self, iteration, subdivition, cost_tensor, optimizer_tensor, out_tensor, session, saver, train_generator, val_generator, best_loss, path_tosave_model='model/model1'):
        """[summary]
