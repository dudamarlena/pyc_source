...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/qoalai/landmarks/inferencing.py
# Compiled at: 2019-11-06 03:08:14
...
def nms_phone(map, position, grid_size=1 / GRID):
    ...
    if len(result.keys()) == 4:
        for i in result:
            ...
            if gradien_x >= 0:
                if gradien_y >= 0:
                    if 1 not in sorted_result:
                        sorted_result[1] = (
                         result[i][0], ctr_x, ctr_y, gradien_x)
                    elif gradien_x > sorted_result[1][3]:
            ...
            elif 3 not in sorted_result:
                sorted_result[3] = (
                 result[i][0], ctr_x, ctr_y, gradien_x)
            elif gradien_x > sorted_result[3][3]:
