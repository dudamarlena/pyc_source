...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/qoalai/similarity/similarity.py
# Compiled at: 2020-03-21 07:20:53
# Size of source mod 2**32: 10728 bytes
import cv2, math, random, numpy as np, tensorflow as tf
...
class Similarity:
 ...
 'normal', 'hard_damage', 'other'], input_height=300, input_width=300, input_channel=3, num_feature=128):
    ...
    def calculate_loss(self, feature1, feature2, feature3, feature4):
        ...
        self.loss = self.pos_loss - 1.3 * self.neg_loss - self.other_loss + 10.0
    ...
    def batch_generator(self, batch_size, dataset_path, message):
        ...
        while True:
                ...
                if i != 'other':
                    for j in range(perclass_sample):
                        ...
                        try:
                            ...
                            tmp_x_pos = tmp_x_pos.astype(np.float32) / 255.0
                            ...
                            tmp_x_neg = tmp_x_neg.astype(np.float32) / 255.0
                            ...
                            tmp_x_other = tmp_x_other.astype(np.float32) / 255.0
            ...
            random.shuffle(c)
            x_batch, x_pos, x_neg, x_other = zip(*c)
            yield (np.array(x_batch), np.array(x_pos), np.array(x_neg), np.array(x_other))
    ...
    def optimize(self, iteration, subdivition, cost_tensor, optimizer_tensor, session, saver, train_generator, val_generator, best_loss, path_tosave_model='model/model1'):
        ...
        for i in range(iteration):
            ...
            print('>> epoch: ', i, 'train loss: ', round(t_loss, 3), 'val loss: ', round(loss, 3), 'p loss: ', round(p_loss, 3), 'n loss: ', round(n_loss, 3), 'o loss: ', round(o_loss, 3), sign)