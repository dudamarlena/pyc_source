...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/cloudbackup/client/agents.py
# Compiled at: 2018-11-01 16:23:11
"""
Rackspace Cloud Backup Agent API
"""
from __future__ import print_function
import datetime, gzip, hashlib, json, logging, os, requests, time, threading, six
...
def _keep_agent_awake_thread_fn(my_notifier=None, userid=None, usertype=None, credentials=None, method=None, rse_app=None, rse_version=None, rse_agentkey=None, rse_log=None, rse_apihost=None, rse_period=None, apihost=None, agent_id=None, api_version=None, project_id=None):
    """
    ...
    if api_version > 1:
        if project_id is None:
            raise RuntimeError(('Invalid parameters. api_version = {0} and project_id is missing.').format(api_version))
    log = logging.getLogger(__name__)
    import cloudbackup.client.auth, cloudbackup.client.rse
    data = threading.local()
    data.thread_id = threading.current_thread().ident
    data.log_prefix = ('RSE Wakeup Thread[{0:}] Log').format(data.thread_id)
    data.auth_engine = cloudbackup.client.auth.Authentication(userid, credentials, usertype=usertype, method=method)
    data.agent_engine = cloudbackup.client.agents.Agents(True, data.auth_engine, apihost, api_version, project_id)
    data.logfile = None
    ...
    data.rse_engine = cloudbackup.client.rse.Rse(rse_app, rse_version, data.auth_engine, data.agent_engine, rse_agentkey, logfile=data.logfile, apihost=rse_apihost, api_version=api_version, project_id=project_id)
...
class AgentLogLevel(Command):
    ...
    def GetLogLevel(self, machine_agent_id):
        ...
        if self.api_version == 1:
            self.ReInit(self.sslenabled, ('/v1.0/agent/logging/{0}').format(machine_agent_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            self.headers['Content-Type'] = 'application/json; charset=utf-8'
        else:
            self.ReInit(self.sslenabled, ('/v{0}/{1}/agents/{2}').format(self.api_version, self.project_id, machine_agent_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            self.headers['Content-Type'] = 'application/json; charset=utf-8'
    ...
    def SetLogLevel(self, machine_agent_id, level):
        ...
        if self.api_version == 1:
            ...
            if level not in ('Fatal', 'Error', 'Warn', 'Info', 'Debug', 'Trace', 'All',
                             1, 2, 3, 4, 5, 6, 7):
...
class AgentDetails(object):
    ...
    def __init__(self, details, version=1):
        ...
        if self.version == 1:
            for prop in ('MachineAgentId', 'AgentVersion', 'Architecture', 'Flavor',
                         'BackupVaultSize', 'CleanupAllowed', 'Datacenter', 'IPAddress',
                         'IsDisabled', 'IsEncrypted', 'MachineName', 'OperatingSystem',
                         'OperatingSystemVersion', 'PublicKey', 'Status', 'TimeOfLastSuccessfulBackup',
                         'UseServiceNet', 'HostServerId'):
                x = details[prop]
    ...
    def IPAddress(self):
        ...
        if self.version == 1:
            ...
            return next(address for address in self._details['host']['addresses'] if address['version'] == 4)['addr']
    ...
    def GetVaultDbContainer(self, backup_name=None):
        ...
        if self.version == 1:
            ...
            if vault_info['use_internal']:
                vault_url = next(url for url in vault_info['links'] if url['rel'] == 'internalURL')['href']
            else:
                vault_url = next(url for url in vault_info['links'] if url['rel'] == 'publicURL')['href']
    ...
    def GetBundlePath(self, backup_name, bundle_id):
        ...
        try:
            if self.version == 1:
                ...
                vaultdburi = vault_db_url + '/BUNDLES/' + ('{0:010}').format(bundle_id)
                self.log.debug('VaultDB Path: ' + vaultdburi)
                return vaultdburi
...
class Agents(Command):
    ...
    def __init__(self, sslenabled, authenticator, apihost, api_version=1, project_id=None):
        ...
        self.loglevel = AgentLogLevel(sslenabled, authenticator, apihost, api_version, project_id)
        if type(api_version) is int:
            self.api_version = api_version
        else:
            self.api_version = 1
    ...
    def WakeAgents(self):
        ...
        if self.api_version == 1:
            ...
            self.ReInit(self.sslenabled, ('/v{0}/{1}/events').format(self.api_version, self.project_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            self.headers['Content-Type'] = 'application/json; charset=utf-8'
            self.headers['X-Project-Id'] = self.project_id
            self.o = {}
                ...
                if keep_agent_awake:
                    if wake_period is None:
                        ...
                        if self.api_version == 1:
                            ...
                            wake_period = rse_heartbeat_config['timeout_ms']['real_time'] / 1000
                        if wake_period > 6:
    ...
    def KeepAgentAwake(self, machine_agent_id, rse, period):
        ...
        for a_thread in self.wake_agent_threads:
            if a_thread['id'] == machine_agent_id:
                ...
                a_thread_kwargs = {'my_notifier': wake_agent_thread['terminator'], 
                ...
                a_thread['thread'] = threading.Thread(target=_keep_agent_awake_thread_fn, kwargs=a_thread_kwargs)
    ...
    def GetAgentDetails(self, machine_agent_id):
        ...
        self.agents = {}
        if self.api_version == 1:
            self.ReInit(self.sslenabled, ('/v1.0/agent/{0}').format(machine_agent_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            self.headers['Content-Type'] = 'application/json; charset=utf-8'
        else:
            self.ReInit(self.sslenabled, ('/v{0}/{1}/agents/{2}').format(self.api_version, self.project_id, machine_agent_id))
    ...
    def GetAgentsFromApi(self):
        ...
        if self.api_version == 1:
            ...
            self.ReInit(self.sslenabled, ('/v{0}/{1}/agents').format(self.api_version, self.project_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            self.headers['Content-Type'] = 'application/json; charset=utf-8'
            self.headers['X-Project-Id'] = self.project_id
            res = requests.get(self.Uri, headers=self.Headers)
    ...
    def GetAgentLogFile(self, machine_agent_id):
        ...
        """
        if self.api_version == 1:
            self.ReInit(self.sslenabled, ('/v1.0/agent/requestlog/{0}').format(machine_agent_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            self.headers['Content-Type'] = 'application/json; charset=utf-8'
            res = requests.post(self.Uri, headers=self.Headers)
            if res.status_code == 200:
            ...
            self.ReInit(self.sslenabled, ('/v{0}/{1}/agents/{2}/logfiles').format(self.api_version, self.project_id, machine_agent_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            self.headers['Content-Type'] = 'application/json; charset=utf-8'
            self.headers['X-Project-Id'] = self.project_id
            res = requests.post(self.Uri, headers=self.Headers)
    ...
    def GetExistingAgentLogFiles(self, machine_agent_id):
        ...
        """
        if self.api_version == 1:
            self.ReInit(self.sslenabled, ('/v1.0/agent/logfiles/{0}').format(machine_agent_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            self.headers['Content-Type'] = 'application/json; charset=utf-8'
        else:
            self.ReInit(self.sslenabled, ('/v{0}/{1}/agents/{2}/logfiles').format(self.api_version, self.project_id, machine_agent_id))
        ...
        if res.status_code == 200:
            if self.api_version == 1:
                for logfile_entry in res.json():
                    e = {'id': logfile_entry['DisplayName'], 
    ...
    def DownloadAgentLogFile(self, logfile_data, target_filename):
        try:
            try:
                headers = {'X-Auth-Token': self.authenticator.AuthToken}
                res = requests.get(logfile_data['link'], stream=True, headers=self.headers)
            ...
            if etag_match is not None:
                if etag_match.upper() != compressed_md5_hash.hexdigest().upper():
                    raise UserWarning(('Failed to download. {0} != {1}').format(etag_match.upper(), compressed_md5_hash.hexdigest().upper()))
    ...
    def GetAgentConfiguration(self, machine_agent_id):
        ...
        """
        if self.api_version == 1:
            self.ReInit(self.sslenabled, ('/v1.0/agent/configuration/{0}').format(machine_agent_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            self.headers['Content-Type'] = 'application/json; charset=utf-8'
        else:
            self.ReInit(self.sslenabled, ('/v{0}/{1}/agents/{2}/configuration').format(self.api_version, self.project_id, machine_agent_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            self.headers['Content-Type'] = 'application/json; charset=utf-8'
            self.headers['X-Project-Id'] = self.project_id
        res = requests.get(self.Uri, headers=self.Headers)
        if res.status_code == 200:
            self.configurations[machine_agent_id] = AgentConfiguration(configuration=res.json(), version=self.api_version)
            return True
        else:
    ...
    def GetAgentLatestActivity(self, machine_agent_id):
        ...
        agent_config = self.AgentConfiguration(machine_agent_id)
        if self.api_version == 1:
            self.ReInit(self.sslenabled, ('/v1.0/{0}/system/activity{1}').format(self.authenticator.AuthTenantId, machine_agent_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            self.headers['Content-Type'] = 'application/json; charset=utf-8'
        else:
            self.ReInit(self.sslenabled, ('/v{0}/{1}/agents/{2}/activities').format(self.api_version, self.project_id, machine_agent_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            self.headers['Content-Type'] = 'application/json; charset=utf-8'
            self.headers['X-Project-Id'] = self.project_id
        res = requests.get(self.Uri, headers=self.Headers)
            ...
            if self.api_version == 1:
                for activity in res.json():
                    ...
                    if activity['ParentId'] != 0:
                        try:
                            activity_name = ('{0} - {1}').format(activity['Type'], agent_config.GetBackupNameFromId(activity['ParentId']))
                        except:
                            activity_name = ('{0} - UNKNOWN({1})').format(activity['Type'], activity['ParentId'])

                    else:
                        activity_name = ('{0} - {1}').format(activity['Type'], activity['DisplayName'])
                    results.append({'id': activity['Id'], 
                       'name': activity_name, 
                       'type': activity['Type'], 
                ...
                for activity in activities:
                    ...
                    if 'configuration' in activity.keys():
                        try:
                            activity_name = ('{0} - {1}').format(activity['type'], agent_config.GetBackupNameFromId(activity['configuration']['id']))
                        except:
                            activity_name = ('{0} - UNKNOWN({1})').format(activity['type'], activity['configuration']['id'])
    ...
    def GetAgentEventsSince(self, machine_agent_id, last_event_id, event_limit=100, results={}):
        ...
        if self.api_version == 1:
            ...
            if last_event_id is None:
                ...
                self.ReInit(self.sslenabled, ('/v{0}/{1}/agents/{2}/events?marker={3}&limit={4}&sort_dir=asc').format(self.api_version, self.project_id, machine_agent_id, last_event_id, event_limit))
        ...
        if res.status_code == 200:
            ...
            if self.api_version == 1:
                ...
                if new_last_event_id is None:
                    new_last_event_id = 0
                if 'heartbeats' not in results.keys():
                    results['heartbeats'] = []
                for event in events:
                    ...
                    if 'event' in event.keys():
                        ...
                        if 'backup' in event.keys():
                            if 'id' in event['backup'].keys():
                                event_name = ('Backup {0}').format(event['backup']['id'])
                        elif 'restore' in event.keys():
                            if 'id' in event['restore'].keys():
                                event_name = ('Restore {0}').format(event['restore']['id'])
