...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/cloudbackup/database/sqlite.py
# Compiled at: 2017-05-12 15:59:18
"""
Rackspace Cloud Backup SQLite Database Functionality
"""
from __future__ import print_function
import base64, datetime, logging, os, os.path, random, sqlite3
...
class CloudBackupSqlite(object):
    ...
    def GetFileAddedInSnapshot(self, snapshotid, limit_lower=None, limit_higher=None):
        ...
        """
        conn = self.dbinstance.cursor()
        stmt = 'SELECT f.fileid, f.directoryid, d.path, f.filename, f.type, f.metadata FROM files f JOIN directories d  ON f.directoryid=d.directoryid WHERE addedinsnapshotid =: snapshotid ORDER BY d.path'
        stmt_dict = {'snapshotid': snapshotid}
        if limit_lower:
    ...
    def GetBackupConfigurations(self):
        ...
        for result in conn.execute('SELECT backupconfigurationid, legacyguid, externalid, cleanupdays, removed FROM backupconfigurations'):
            backupconfiguration = {'backupconfigurationid': int(result[0]), 
               'legacyguid': result[1], 
    ...
    def DetectUniqueConstraintViolations(self):
        ...
        for entry in conn.execute('SELECT COUNT(fileid), backupconfigurationid, directoryid, filename, addedinsnapshotid, lastsnapshotid FROM files WHERE lastsnapshotid=2000000000 GROUP BY filename, directoryid, backupconfigurationid HAVING COUNT(fileid) > 1'):
            ...
            self.log.debug(('Found entry (directoryid={0:}, filename={1:}, backupconfigurationid={2:}: added-in={3:}, count={4:}) with possible errors').format(info['directoryid'], info['filename'], info['backupconfigurationid'], info['addedinsnapshotid'], info['count']))
            first_round = True
            for row in conn2.execute('SELECT directoryid, fileid, filename, addedinsnapshotid, lastsnapshotid, backupconfigurationid FROM files WHERE directoryid = ? AND filename = ? AND backupconfigurationid = ? AND lastsnapshotid = ? ORDER BY addedinsnapshotid DESC', (
             info['directoryid'], info['filename'], info['backupconfigurationid'], info['lastsnapshotid'])):
                confirmed_info = {'directoryid': row[0], 
                   'fileid': row[1], 
                   'filename': row[2], 
                ...
                if not first_round:
                    self.log.debug(('Confirmed entry (directoryid={0:}, filename={1:}, backupconfigurationid={2:}) has errors').format(confirmed_info['directoryid'], confirmed_info['filename'], confirmed_info['backupconfigurationid']))
            ...
            for row in conn2.execute('SELECT directoryid, fileid, filename, addedinsnapshotid, lastsnapshotid, backupconfigurationid FROM files WHERE directoryid = ? AND filename = ? AND backupconfigurationid = ? AND lastsnapshotid = ? ORDER BY addedinsnapshotid DESC', (
             info['directoryid'], info['filename'], info['backupconfigurationid'], info['lastsnapshotid'])):
                confirmed_info = {'directoryid': row[0], 
                   ...
                   'backupconfigurationid': row[5]}
                if not first_round:
                    self.log.debug(('Fixing entry (directoryid={0:}, filename={1:}, backupconfigurationid={2:}) - lastsnapshotid ({3:} -> {4:})').format(confirmed_info['directoryid'], confirmed_info['filename'], confirmed_info['backupconfigurationid'], confirmed_info['lastsnapshotid'], confirmed_info['addedinsnapshotid']))
    ...
    def GetAgentNextCleanupsForConfigurations(self, cleanup_offset=None, adjustment=None):
        ...
        for backupconfig_result in results:
            ...
            data = (
             backupconfigid, next_cleanup_time[0], next_cleanup_time[1])
    ...
    def GetSnapshots(self, backupconfigurationids=None, states=None):
        ...
        for result in conn.execute('SELECT snapshotid, startdate, state, cleanupindex, backupconfigurationid FROM snapshots'):
            snapshot = {'snapshotid': int(result[0]), 
               'startdate': result[1], 
               'state': int(result[2]), 
    ...
    def BloatDatabase(self, table_suffix=None, granularity=1048576, minimum_compressed_size=5476083302.4):
        """
        ...
        def __bloat_db_find_test_compressed_size():
            ...
            file_sizes = (
             gzip_file_size, gzip_file_size / 1024, gzip_file_size / 1048576, gzip_file_size / 1073741824)
        ...
        return (
         original_compressed_size, new_compressed_size)