...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/cloudbackup/cmd/shell/interface.py
# Compiled at: 2017-05-12 15:59:56
from __future__ import print_function
import argparse, json, logging, logging.config, random, sys, time, six, cloudbackup.client.agents, cloudbackup.client.auth, cloudbackup.client.backup, cloudbackup.client.rse, cloudbackup.cmd.shell.exceptions, cloudbackup.cmd.shell.prompter as prompt_user, cloudbackup.utils.menus
...
class CloudBackupApiShell(object):
    ...
    def __init__(self, logger, auth_data_file, datacenter, use_servicenet=False):
        if datacenter not in ('ord', 'syd', 'hkg', 'iad', 'dfw', 'lon'):
            raise cloudbackup.cmd.shell.exceptions.CloudBackupApiBadParameters(('Invalid Datacenter - {0}').format(datacenter))
        self.api = {}
        self.auth_data = {}
        self.log = logger
        ...
        if self.auth_data['user_type'] not in ('user', 'tenantid', 'tenantname'):
            raise cloudbackup.cmd.shell.exceptions.CloudBackupApiBadAuthData(('invalid user type {0}').format(self.auth_data['user_type']))
        ...
        self.auth_engine = cloudbackup.client.auth.Authentication(self.auth_data['user'], self.auth_data['credentials'], usertype=self.auth_data['user_type'], method=self.auth_data['method'], datacenter=self.datacenter)
        self.api['uri'] = self.auth_engine.GetCloudBackupApiUri(self.datacenter, self.use_servicenet)
        self.log.debug(('Cloud Backup API URI: {0}').format(self.api['uri']))
        self.api['version'] = self.auth_engine.GetCloudBackupApiVersion(self.datacenter, self.use_servicenet)
        self.log.debug(('Cloud Backup API Version: {0}').format(self.api['version']))
        self.agents = cloudbackup.client.agents.Agents(True, self.auth_engine, self.api['uri'], self.api['version'], self.auth_engine.AuthTenantId)
        self.backup_engine = cloudbackup.client.backup.Backups(True, self.auth_engine, self.api['uri'], self.api['version'], self.auth_engine.AuthTenantId)
        self.rse = None
        self.snapshot_ids = {}
        return
    ...
    def GetAgentIdFromUser(self):
        ...
        for agent_id in self.GetAgentIds():
            agent_id_entry = {'index': len(agent_menu), 
               'text': ('{0}').format(agent_id), 
               'type': 'agent-id'}
            agent_menu.append(agent_id_entry)

        agent_menu.append({'index': len(agent_menu), 
           'text': 'Exit', 
           'type': 'EXIT'})
        selection = cloudbackup.utils.menus.promptSelection(agent_menu, 'Select Agent ID')
        return selection

    def doCreateV1BackupConfiguration(self, active_agent_id, config_data):
        DayOfWeekMapping = {'Sunday': 0, 
           'Monday': 1, 
           'Tuesday': 2, 
           'Wednesday': 3, 
           'Thursday': 4, 
        ...
        if config_data['schedule']['start-time']['hour'] == 0:
            ...
            if config_data['schedule']['start-time']['hour'] == 12:
                config_data['schedule']['start-time']['am-pm'] = 'PM'
            else:
                if config_data['schedule']['start-time']['hour'] in range(13, 24):
                ...
                if config_data['schedule']['day-of-week'] is not None:
    ...
    def doCreateV2BackupConfiguration(self, active_agent_id, config_data):
        DayOfWeekMapping = {'Sunday': 'SU', 'Monday': 'MO', 
           'Tuesday': 'TU', 
           'Wednesday': 'WE', 
           'Thursday': 'TH', 
           'Friday': 'FR', 
    ...
    def doCreateBackupConfiguration(self, active_agent_id):
        ...
        prompted_config_data = {'name': None, 
           'schedule': {'frequency': None, 
                        'day-of-week': None, 
                        'start-time': {'hour': None, 
                                       'minute': None, 
                                       'am-pm': None, 
                                       'timezone': tz.get_timezone(self.api['version'] == 1)}, 
                        'hourly-interval': None}, 
           'retention': None, 
           'notifications': {'e-mail-addresses': [], 'success': False, 
                             'failure': True}, 
    ...
    def doPrintBackupConfigurationDetails(self, active_agent_id, backup_id, backup_name):
        backup_config = self.agents.AgentConfiguration(active_agent_id).GetBackupConfigurationById(backup_id)
        if self.api['version'] == 1:
            self.doPrintBackupConfigurationV1Details(active_agent_id, backup_config)
        elif self.api['version'] == 2:
            self.doPrintBackupConfigurationV2Details(active_agent_id, backup_config)
        else:
            print('Unknown Cloud Backup API Version')

    def doPrintAgentGlobalConfiguration(self, active_agent_id, show_password=False):
        self.agents.GetAgentConfiguration(active_agent_id)
        agent_configuration = self.agents.AgentConfiguration(active_agent_id)
        ...
        for volume in agent_configuration.Volumes:
            for k, v in six.iteritems(volume):
                ...
                elif k.lower() == 'password' and show_password or k.lower() != 'password':
                    if k == 'links':
                        for href_link in v:
                            print(('\t\t\t{0} = {1}').format(href_link['rel'], href_link['href']))
    ...
    def GetBackupIds(self, active_agent_id):
        ...
        for backup_id in agent_configuration.GetBackupIds():
            backup_name = agent_configuration.GetBackupNameFromId(backup_id)
            info.append({'id': backup_id, 
               'name': backup_name})
    ...
    def doPrintLatestAgentActivity(self, active_agent_id, show_agent_id=True):
        ...
        if len(activities):
            for activity in activities:
                print(('\t{0} - {1}').format(activity['id'], activity['name']))
        ...
        if user_prompt_continue == 'Yes':
            ...
            while True:
                try:
                    ...
                    events, last_event_id = self.agents.GetAgentEventsSince(active_agent_id, last_event_id, 5, events)
                    if len(events):
                        for event_name, event_data in six.iteritems(events):
                            if event_name == 'heartbeats':
                                if show_heartbeat:
                                    for event_entry in event_data:
                                        print(('\tHeart Beat - {0}').format(event_entry['time']))
    ...
    def RetrieveBackupReports(self, active_agent_id, config_id):
        while True:
            all_backups = self.backup_engine.GetAllBackupsForConfiguration(active_agent_id, config_id)
            backup_config_menu = []
            for backup_entry in all_backups:
                backup_config_menu.append({'index': len(backup_config_menu), 
                   'text': ('{0} - {2} - {1}').format(backup_entry['id'], backup_entry['state'], backup_entry['updated_at']), 
                   'type': 'backup_id', 
            ...
            backup_config_selection = cloudbackup.utils.menus.promptSelection(backup_config_menu, 'Select Backup')
            if backup_config_selection['type'] == 'returnToPrevious':
                break
            elif backup_config_selection['type'] == 'backup_id':
                backup_report = self.backup_engine.GetBackupReport(backup_config_selection['backup_id'])
                report_data = json.dumps(backup_report, sort_keys=True, indent=4)
                print(report_data)
                cloudbackup.utils.menus.promptUserAnyKey()

    def RetrieveCleanupReports(self, active_agent_id):
        while True:
            all_cleanups = self.backup_engine.GetAllCleanupsForConfiguration(active_agent_id)
            cleanup_config_menu = []
            for cleanup_entry in all_cleanups:
                cleanup_config_menu.append({'index': len(cleanup_config_menu), 
                   'text': ('{0} - {2} - {1}').format(cleanup_entry['id'], cleanup_entry['state'], cleanup_entry['updated_at']), 
                   'type': 'cleanup_id', 
            ...
            cleanup_config_menu.append({'index': len(cleanup_config_menu), 
               'text': 'Return to previous menu', 
               'type': 'returnToPrevious'})
            cleanup_config_selection = cloudbackup.utils.menus.promptSelection(cleanup_config_menu, 'Select Cleanup')
            if cleanup_config_selection['type'] == 'returnToPrevious':
                break
            elif cleanup_config_selection['type'] == 'cleanup_id':
                cleanup_report = self.backup_engine.GetCleanupReport(cleanup_config_selection['cleanup_id'])
                report_data = json.dumps(cleanup_report, sort_keys=True, indent=4)
                print(report_data)
                cloudbackup.utils.menus.promptUserAnyKey()
    ...
    def EnableDisableSpecificAgentConfiguration(self, active_agent_id, config_id):
        while True:
            try:
                self.agents.GetAgentConfiguration(active_agent_id)
                backup_config = self.agents.AgentConfiguration(active_agent_id).GetBackupConfigurationById(config_id)
            except:
                backup_config = None
            ...
            enable_disable_menu = [
             config_state_entry, {'index': 1, 'text': 'Update Configuration State', 'type': 'updateState'}, {'index': 2, 'text': 'Return to previous menu', 'type': 'returnToPrevious'}]
            print(('Current Configuration State: {0}').format(current_state))
            selection = cloudbackup.utils.menus.promptSelection(enable_disable_menu, 'Select Action')
            if selection['type'] == 'returnToPrevious':
                return
            if selection['type'] == 'updateState':
                continue
            elif selection['type'] == 'stateChange':
                if self.backup_engine.EnableDisableBackupConfiguration(active_agent_id, config_id, selection['value']):
    ...
    def DeleteBackupConfiguration(self, active_agent_id, config_id):
        verify_delete = cloudbackup.utils.menus.promptYesNoCancel(('Confirm Delete Configuration {0} - {1}').format(config_id, config_name))
        if verify_delete == 'Yes':
            print('Deleting configuration...')
            if self.backup_engine.DeleteBackupConfiguration(config_id):
                return
    ...
    def WorkOnSpecificAgentConfiguration(self, active_agent_id, config_id, config_name):
        specific_config_menu = [{'index': 0, 'text': 'Run', 'type': 'actionRun'}, {'index': 1, 'text': 'Show', 'type': 'actionShow'}, {'index': 2, 'text': 'Check Status', 'type': 'actionCheckStatus'}, {'index': 3, 'text': 'Enable/Disable Configuration', 'type': 'actionEnableDisable'}, {'index': 4, 'text': 'Get Backup Reports', 'type': 'actionGetBackupReports'}, {'index': 5, 'text': 'Return to previous menu', 'type': 'returnToPrevious'}, {'index': 99, 'text': 'Delete', 'type': 'actionDelete'}]
        while True:
            print(('Agent ID: {0}').format(active_agent_id))
            ...
            selection = cloudbackup.utils.menus.promptSelection(specific_config_menu, 'Select Action')
            if selection['type'] == 'returnToPrevious':
                return
            if selection['type'] == 'actionRun':
                if active_agent_id not in self.snapshot_ids.keys():
                ...
                self.doPrintBackupConfigurationDetails(active_agent_id, config_id, config_name)
            elif selection['type'] == 'actionCheckStatus':
                snapshot_id = None
                if active_agent_id not in self.snapshot_ids.keys():
                    print('No known Backup is running.')
                    user_provides_snapshot_id = cloudbackup.utils.menus.promptYesNoCancel('Do you know of a Snapshot ID for this agent to check on?')
                    if user_provides_snapshot_id == 'Yes':
                        snapshot_id = cloudbackup.utils.menus.promptUserInputString('Snapshot ID', '')
                else:
                ...
                if snapshot_id is not None:
                    ...
                    try:
                        if self.api['version'] == 1:
                            snapshot_state = self.backup_engine.GetBackupProgressV1(snapshot_id)
                        else:
                            snapshot_state = self.backup_engine.GetBackupProgressV2
                        if active_agent_id not in self.snapshot_ids:
                ...
                self.RetrieveBackupReports(active_agent_id, config_id)
            elif selection['type'] == 'actionEnableDisable':
                self.EnableDisableSpecificAgentConfiguration(active_agent_id, config_id)
            elif selection['type'] == 'actionDelete':
                self.DeleteBackupConfiguration(active_agent_id, config_id)
    ...
    def WorkOnAgentConfiguration(self, active_agent_id):
        while True:
            print(('Agent ID: {0}').format(active_agent_id))
            backup_configuration_ids = self.GetBackupIds(active_agent_id)
            ...
            for info in backup_configuration_ids:
                ...
                agent_configuration_menu.append({'index': len(agent_configuration_menu), 
                   'text': ('{0} - {1}').format(backup_id, backup_name), 
                   'type': 'configuration', 
                   'id': backup_id, 
                   'name': backup_name})
            ...
            agent_configuration_menu.append({'index': len(agent_configuration_menu), 
               'text': 'Return to previous menu', 
               'type': 'returnToPrevious'})
            selection = cloudbackup.utils.menus.promptSelection(agent_configuration_menu, 'Select Action')
            if selection['type'] == 'returnToPrevious':
                return
            if selection['type'] == 'addConfiguration':
                self.doCreateBackupConfiguration(active_agent_id)
            elif selection['type'] == 'showAgentGlobalConfiguration':
                self.doPrintAgentGlobalConfiguration(active_agent_id)
            elif selection['type'] == 'configuration':
                self.WorkOnSpecificAgentConfiguration(active_agent_id, selection['id'], selection['name'])

    def WorkOnAgentLogFiles(self, active_agent_id):
        while True:
            print(('Agent ID: {0}').format(active_agent_id))
            available_log_files = self.agents.GetExistingAgentLogFiles(active_agent_id)
            agent_logfile_menu = []
            for log_file_entry in available_log_files:
                entry = {'index': len(agent_logfile_menu), 
                   'type': 'logfile', 
                   'text': ('Log File[{0}] - {1}').format(log_file_entry['date'], log_file_entry['status']), 
                   'value': log_file_entry}
                agent_logfile_menu.append(entry)

            agent_logfile_menu.append({'index': len(agent_logfile_menu), 
            ...
            agent_logfile_menu.append({'index': len(agent_logfile_menu), 
               ...
               'text': 'Return to previous menu', 
               'value': None})
            selection = cloudbackup.utils.menus.promptSelection(agent_logfile_menu, 'Select Action')
            if selection['type'] == 'actionUpdate':
                continue
            else:
                if selection['type'] == 'returnToPrevious':
                    return
                if selection['type'] == 'logfile':
                    if selection['value']['status'] in ('Finished', 'completed'):
                        download_output_filename = cloudbackup.utils.menus.promptUserInputString('Output Filename', '')
                        if download_output_filename is not None:
                            print(('Downloading file to {0}').format(download_output_filename))
                            successful_download = self.agents.DownloadAgentLogFile(selection['value'], download_output_filename)
    ...
    def WorkOnAgentLog(self, active_agent_id):
        agent_log_menu = [{'index': 0, 'text': 'Get Log Level', 'type': 'actionGetLogLevel'}, {'index': 1, 'text': 'Set Log Level', 'type': 'actionSetLogLevel'}, {'index': 2, 'text': 'Log Files', 'type': 'actionLogFiles'}, {'index': 3, 'text': 'Return to previous menu', 'type': 'returnToPrevious'}]
        agent_log_level_menu = [{'index': 0, 'text': 'All Levels', 'type': 'loglevel', 'value': 'All'}, {'index': 1, 'text': 'Trace', 'type': 'loglevel', 'value': 'Trace'}, {'index': 2, 'text': 'Debug', 'type': 'loglevel', 'value': 'Debug'}, {'index': 3, 'text': 'Info', 'type': 'loglevel', 'value': 'Info'}, {'index': 4, 'text': 'Warn', 'type': 'loglevel', 'value': 'Warn'}, {'index': 5, 'text': 'Error', 'type': 'loglevel', 'value': 'Error'}, {'index': 6, 'text': 'Fatal', 'type': 'loglevel', 'value': 'Fatal'}, {'index': 7, 'text': 'Cancel', 'type': 'cancel'}]
        while True:
            print(('Agent ID: {0}').format(active_agent_id))
            selection = cloudbackup.utils.menus.promptSelection(agent_log_menu, 'Selection Action')
            if selection['type'] == 'returnToPrevious':
                return
            if selection['type'] == 'actionGetLogLevel':
                current_log_level = self.agents.loglevel.GetLogLevel(active_agent_id)
                print(('\tActive Log Level: {0}').format(current_log_level))
            elif selection['type'] == 'actionSetLogLevel':
                log_level_selection = cloudbackup.utils.menus.promptSelection(agent_log_level_menu, 'Select Log Level')
                if log_level_selection['type'] == 'cancel':
                    pass
                elif log_level_selection['type'] == 'loglevel':
                    print(('Attempting to set log level to {0}').format(log_level_selection['text']))
                    if not self.agents.loglevel.SetLogLevel(active_agent_id, log_level_selection['value']):
    ...
    def WorkOnAgent(self, active_agent_id):
        if not self.agents.GetAgentDetails(active_agent_id):
            msg = ('Failed to retrieve agent details for agent id {0}').format(active_agent_id)
            self.log.error(msg)
            print(msg)
        else:
            woke_agent = False
            should_wake_agent = cloudbackup.utils.menus.promptYesNoCancel('Wake agent?')
            if should_wake_agent == 'Yes':
                print('Attempting to wake the agent...')
                self.agents.GetAgentConfiguration(active_agent_id)
                agent_configuration = self.agents.AgentConfiguration(active_agent_id)
                self.rse = cloudbackup.client.rse.Rse('cloudbackup-sdk-shell', '1.0', self.auth_engine, self.agents, agent_configuration.RseChannel, apihost=self.api['uri'], api_version=self.api['version'], project_id=self.auth_engine.AuthTenantId)
                self.agents.WakeSpecificAgent(active_agent_id, self.rse, 1000, keep_agent_awake=True)
                woke_agent = True
            ...
            while continue_specific_agent_config:
                ...
                agent_detail_menu = [{'index': 1, 'text': 'Show Details', 'type': 'details'}, {'index': 2, 'text': 'Log Configuration', 'type': 'actionLog'}, {'index': 3, 'text': 'Access Backup Configurations', 'type': 'configuration'}, {'index': 4, 'text': 'Check agent activity', 'type': 'actionCheckActivity'}, {'index': 5, 'text': 'Monitor Agent Events', 'type': 'actionMonitorAgent'}, {'index': 6, 'text': 'Get Cleanup Reports', 'type': 'actionGetCleanupReports'}, {'index': 7, 'text': 'Return to previous menu', 'type': 'returnToPrevious'}]
                selection = cloudbackup.utils.menus.promptSelection(agent_detail_menu, 'Selection Action')
                if selection['type'] == 'returnToPrevious':
