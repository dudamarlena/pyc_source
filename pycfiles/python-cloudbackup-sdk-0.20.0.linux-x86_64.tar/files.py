...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/cloudbackup/cloud/files.py
# Compiled at: 2018-11-01 16:23:11
"""
Rackspace Cloud Files
"""
import gzip, hashlib, logging, os.path, requests, time
...
class CloudFiles(Command):
    ...
    def DownloadVaultDb(self, container, vaultdb_data, localpath, decompress=True, maximum_file_size_supported=5368709120):
        """
        ...
        try:
            ...
            if res.status_code == 404:
                ...
                if meter['bytes-total'] > 5368709120:
    ...
    def UploadVaultDb(self, container, vaultdb_data, localpath, skip_md5_check=False, compress=True, maximum_file_size_supported=5368709120):
        """
        ...
        try:
            ...
            if os.path.getsize(gzip_file) > 5368709120:
                large_file_hashes = self.__GetLargeFileHashes(localpath)
                ...
                vaultdb_data['upload-split-boundary'] = 536870912
    ...
    def DownloadBundle(self, container, uripath, bundle_data, localpath):
        ...
        try:
            ...
            if res.status_code == 404:
                ...
                meter['bar-count'] = 50
                meter['bytes-per-bar'] = meter['bytes-remaining'] // meter['bar-count']
                meter['block-size'] = min(4194304, meter['bytes-per-bar'])
                meter['chunks-per-bar'] = meter['bytes-per-bar'] // meter['block-size']
                meter['chunks'] = 0
                meter['bars-remaining'] = meter['bar-count']
                meter['bars-completed'] = 0
