...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/cloudbackup/utils/menus.py
...
def promptYesNoCancel(prompt, prefix=''):
    ...
    :returns: string, menu entry text
    """
    menu = [{'index': 1, 'text': 'Yes', 'type': 'YES'}, {'index': 2, 'text': 'No', 'type': 'NO'}, {'index': 3, 'text': 'Cancel', 'type': 'EXIT'}]
    ...
    while user_result is None:
        try:
            ...
            if len(user_result) <= min_length:
                print(('Value too short. {0} <= {1}. Please try again...').format(len(user_result), min_length))
                user_result = None
            elif len(user_result) >= max_length:
                print(('Value too long. {0} >= {1}. Please try again...').format(len(user_result), max_length))
                user_result = None
...
def promptUserInputNumber(prompt, prefix='', min_value=0, max_value=99999, show_range=False):
    ...
    while user_result is None:
        try:
            print('Press CTRL+C to cancel')
            if show_range:
                print(('Acceptable Range: {0} < X < {1}').format(min_value, max_value))
            if six.PY2:
                user_result = raw_input(('{0}: ').format(prompt))
            else:
                user_result = input(('{0}: ').format(prompt))
            ...
            if match_result:
                ...
                if value <= min_value:
                    print(('Value is too small. {0} <= {1}. Please try again...').format(user_result, min_value))
                    user_result = None
                elif value >= max_value:
                    print(('Value is too large. {0} >= {1}. Please try again...').format(user_result, max_value))
                    user_result = None
            else:
