...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/cloudbackup/client/auth.py
# Compiled at: 2017-05-12 15:59:18
"""
Rackspace Authentication API
"""
import datetime, json, logging, requests, time, re
...
class Authentication(Command):
    ...
    def IsExpired(self, fuzz=0):
        ...
        def time_fuzzy_compare(timeval_older, timeval_newer, new_fuzz):
            ...
            return time_layer_compare(timeval_older.year, timeval_newer.year, {'call': time_layer_compare, 
               'older': timeval_older.month, 'newer': timeval_newer.month, 'next': {'call': time_layer_compare, 
                        'older': timeval_older.day, 'newer': timeval_newer.day, 'next': {'call': time_layer_compare, 
                                 'older': timeval_older.hour, 'newer': timeval_newer.hour, 'next': {'call': time_layer_compare, 
                                          'older': timeval_older.minute, 'newer': timeval_newer.minute, 'next': {'call': time_layer_compare, 
                                                   'older': timeval_older.second, 'newer': timeval_newer.second + new_fuzz, 'next': None}}}}})
    ...
    def InitialAuthCredentials(self):
        return {'userid': self.parameters['userid'], 
                ...
                if service['name'] == 'cloudBackup':
                    if len(service['endpoints']) == 1:
                        ...
                        if self._GetCloudBackupAPiVersion(dcuri) > 1:
                            dcuri = ('https://api-prod-global.drivesrvr.com/v2/{0}').format(self.AuthTenantId)
                    else:
                        for endpoint in service['endpoints']:
                            if endpoint['region'].lower() == dc.lower():
                                if useServiceNet:
    ...
    def _GetCloudBackupAPiVersion(self, cloudbackup_api_uri):
        ...
        self.log.debug(('Matching {0} with {1}').format(cloudbackup_api_uri, api_regex))
