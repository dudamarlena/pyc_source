...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/cloudbackup/client/backup.py
...
class BackupConfiguration(object):
    ...
    def from_dict(cls, config, source):
        ...
        try:
            ...
            if source in (None, 'agent-configuration'):
                ...
                keys_to_copy = [
                 'BackupConfigurationName', 'Exclusions', 'Inclusions']
            else:
                if source == 'backup-configuration':
    ...
    def to_creation_dict(self):
        ...
        if self.dict_source in (None, 'agent-configuration'):
            ...
            if self.dict_source == 'backup-configuration':
                keys_to_remove = [
                 'LastRunBackupReportId',
                 'LastRunTime']
            else:
    ...
    def AddFolder(self, absoluteFolderPath, excluded=False):
        ...
        elif not self.IsFolderIncluded(absoluteFolderPath):
    ...
    def AddFile(self, absoluteFilePath, excluded=False):
        ...
        elif not self.IsFileIncluded(absoluteFilePath):
            self.backup_config['Inclusions'].append(entry)
        else:
            self.log.debug(('File: {0:} is already in the inclusions').format(absoluteFilePath))
...
class BackupConfigurationV2(object):
    ...
    def __init__(self):
        ...
        """
        self.log = logging.getLogger(__name__)
        self.backup_config = BackupConfigurationV2.CreateBackupConfigurationTemplate()
        self.schedule = {'frequency': None, 'interval': 1, 
           'start_hour': None, 
           'start_minute': None, 
           'weekday': None, 
           'time_zone': 'UTC'}
    ...
    def _generate_schedule_string(self):
        if self.schedule['frequency'] == 'MANUALLY':
            ...
            if self.schedule['weekday']:
                schedule_string += (';BYDAY={0}').format(self.schedule['weekday'])
            if self.schedule['start_hour']:
                schedule_string += (';BYHOUR={0}').format(self.schedule['start_hour'])
            if self.schedule['start_minute']:
                schedule_string += (';BYMINUTE={0}').format(self.schedule['start_minute'])
            self.backup_config['schedule'] = {'recurrence': [schedule_string], 'time_zone': self.schedule['time_zone']}
        return
    ...
    def Frequency(self, frequency):
        ...
        if frequency in valid_values:
            ...
            raise ValueError(('frequency {0} is not in {1}').format(frequency, valid_values))
...
class Backups(Command):
    ...
    def CreateBackupConfiguration(self, backupinfo):
        ...
        if self.api_version == 1 and isinstance(backupinfo, BackupConfiguration):
            ...
            self.ReInit(self.sslenabled, ('/v{0}/{1}/configurations').format(self.api_version, self.project_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            ...
            raise TypeError('backup info is either not an instance of BackupConfiguration or not an appropriate instance for the API version')
    ...
    def EnableDisableBackupConfiguration(self, agent_id, backup_config_id, enabled):
        ...
        if self.api_version == 1:
            self.ReInit(self.sslenabled, ('/v1.0/{0}/backup-configuration/enable/{1}').format(self.authenticator.AuthTenantId, backup_config_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            self.headers['Content-Type'] = 'application/json'
            config_change = {'Enabled': enabled}
            ...
            self.ReInit(self.sslenabled, ('/v{0}/{1}/configurations/{2}').format(self.api_version, self.project_id, backup_config_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
    ...
    def StartBackup(self, backup_config_id, retry=20):
        ...
        if self.api_version == 1:
            ...
            self.ReInit(self.sslenabled, ('/v{0}/{1}/backups').format(self.api_version, self.project_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            self.headers['X-Project-Id'] = self.project_id
            self.headers['Content-Type'] = 'application/json; charset=utf-8'
            o = {}
    ...
    def GetBackupProgressV2(self, snapshot_id):
        ...
        """
        self.ReInit(self.sslenabled, ('/v{0}/{1}/backups/{2}').format(self.api_version, self.project_id, snapshot_id))
        self.headers['X-Auth-Token'] = self.authenticator.AuthToken
        self.headers['X-Project-Id'] = self.project_id
        self.headers['Content-Type'] = 'application/json; charset=utf-8'
        res = requests.get(self.Uri, headers=self.Headers)
    ...
    def MonitorBackupProgress(self, snapshot_id, timeoutMilliseconds, pausePeriod=5.0):
        ...
        if self.api_version == 1:
            stoplist = [
             'Completed', 'Skipped', 'Missed', 'Stopped', 'Failed', 'CompletedWithErrors']
            ...
            stoplist = [
             'completed', 'skipped', 'missed', 'stopped', 'failed',
    ...
    def GetAllBackupsForConfiguration(self, agent_id, backup_config_id):
        ...
        """
        if self.api_version == 1:
            self.ReInit(self.sslenabled, ('/v1.0/{0}/system/activity/{1}').format(self.authenticator.AuthTenantId, agent_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            res = requests.get(self.Uri, headers=self.Headers)
            if res.status_code is 200:
                backups = []
            ...
            self.ReInit(self.sslenabled, ('/v{0}/{1}/configurations/{2}/activities').format(self.api_version, self.project_id, backup_config_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            self.headers['X-Project-Id'] = self.project_id
            self.headers['Content-Type'] = 'application/json; charset=utf-8'
            res = requests.get(self.Uri, headers=self.Headers)
    ...
    def GetCompletedBackups(self, backup_config_id):
        ...
        """
        if self.api_version == 1:
            self.ReInit(self.sslenabled, ('/v1.0/backup/completed/{0}').format(backup_config_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            res = requests.get(self.Uri, headers=self.Headers)
            if res.status_code is 200:
                snapshots = res.json()
    ...
    def GetCompletedBackup(self, backup_config_id, snapshot_id):
        ...
        if self.api_version == 1:
            ...
            return False
        if len(snapshots) > 0:
            found = next((snapshot for snapshot in snapshots if snapshot['snapshot_id'] == snapshot_id), None)
            if found:
                return True
            self.log.error(('Unable to retrieve backup completion information for backup configuration id {0}').format(backup_config_id))
        return False
        return
    ...
    def GetBackupReport(self, backup_id):
        ...
        if self.api_version == 1:
            ...
            self.ReInit(self.sslenabled, ('/v{0}/{1}/backups/{2}').format(self.api_version, self.project_id, backup_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            self.headers['X-Project-Id'] = self.project_id
            self.headers['Content-Type'] = 'application/json'
            res = requests.get(self.Uri, headers=self.Headers)
            if res.status_code == 200:
                ...
                return BackupConfigurationV2.convert_backup_report_to_v1(json_data, error_data)
            msg = ('Unable to retrieve backup report for backup id ({0:}). RCBU API returned error status ({1:}) with text ({2:}) with reason ({3:})').format(backup_id, res.status_code, res.text, res.reason)
            self.log.error(msg)
            raise RuntimeError(msg)
        return
    ...
    def GetBackupErrors(self, backup_id):
        if self.api_version == 1:
            ...
            self.ReInit(self.sslenabled, ('/v{0}/{1}/backups/{2}/errors').format(self.api_version, self.project_id, backup_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            self.headers['X-Project-Id'] = self.project_id
            self.headers['Content-Type'] = 'application/json'
            res = requests.get(self.Uri, headers=self.Headers)
            if res.status_code == 200:
                return res.json()
            msg = ('Unable to retrieve backup errors for backup id ({0:}). RCBU API returned error status ({1:}) with text ({2:}) with reason ({3:})').format(backup_id, res.status_code, res.text, res.reason)
            self.log.error(msg)
            raise RuntimeError(msg)
    ...
    def StartBackupRetry(self, parameters):
        ...
        for retry in range(parameters['retry_attempts']):
            ...
            if output['agent_snapshotid'] == -1 or output['agent_snapshotid'] is None:
    ...
    def GetBackupsForRestore(self, machine_agent_id, backup_config_id):
        ...
        if res.status_code != 200:
            ...
            availForRestore['backups'] = [ BackupConfigurationV2.convert_listed_backup_to_v1(cur_backup) for cur_backup in list_of_backups
                                         ]
            if len(availForRestore['backups']) == 0:
                self.log.error(('Unable to find any backups to restore for agent {0}').format(machine_agent_id))
            return availForRestore
    ...
    def GetCleanupReport(self, cleanup_id):
        ...
        """
        if self.api_version == 1:
            self.ReInit(self.sslenabled, '/v1.0/cleanup/report/' + str(cleanup_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            self.headers['Content-Type'] = 'application/json'
            res = requests.get(self.Uri, headers=self.Headers)
            if res.status_code == 200:
                return res.json()
            msg = ('Unable to retrieve cleanup report for cleanup id ({0:}). Rackspace Cloud Backup API returned error status ({1:}) with text ({2:}) with reason ({3:})').format(cleanup_id, res.status_code, res.text, res.reason)
            self.log.error(msg)
            raise RuntimeError(msg)
        else:
            self.ReInit(self.sslenabled, ('/v{0}/{1}/cleanups/{2}').format(self.api_version, self.project_id, cleanup_id))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            self.headers['X-Project-Id'] = self.project_id
            self.headers['Content-Type'] = 'application/json'
            res = requests.get(self.Uri, headers=self.Headers)
            if res.status_code == 200:
                ...
                return res.json()
            msg = ('Unable to retrieve cleanup report for cleanup id ({0:}). RCBU API returned error status ({1:}) with text ({2:}) with reason ({3:})').format(cleanup_id, res.status_code, res.text, res.reason)
            self.log.error(msg)
            raise RuntimeError(msg)
        return
...
class RestoreConfigurationV2(object):
    ...
    def __init__(self):
        self.log = logging.getLogger(__name__)
        self.restore_config = RestoreConfigurationV2.CreateRestoreConfigurationTemplate()
    ...
    def add_folder(self, folder):
        data = {'type': 'folder', 'path': folder['name']}
...
class Restores(Command):
    ...
    def StartRestoreV2(self, restore_config, retry=20):
        self.ReInit(self.sslenabled, ('/v{0}/{1}/restores').format(self.api_version, self.project_id))
        self.headers['X-Auth-Token'] = self.authenticator.AuthToken
    ...
    def StartRestoreRetryV2(self, restore_config, parameters):
        ...
        for _ in range(parameters['retry_attempts']):
            self.MonitorRestoreProgress(self.restore_id, parameters['restore_timeout'], parameters['monitor_period'])
            output['restore_report'] = self.GetRestoreDetails(self.restore_id)
            if output['restore_report']['state'] != 'completed' and output['restore_report']['state'] != 'completed_' + 'with_errors':
    ...
    def GetRestoreDetails(self, restoreId):
        ...
        if self.api_version == 1:
            ...
            self.ReInit(self.sslenabled, ('/v{0}/{1}/restores/{2}').format(self.api_version, self.project_id, restoreId))
            self.headers['X-Auth-Token'] = self.authenticator.AuthToken
            res = requests.get(self.Uri, headers=self.Headers)
            if res.status_code is 200:
                return res.json()
    ...
    def MonitorRestoreProgress(self, restoreId, timeoutMilliseconds, pausePeriod=5.0):
        ...
        if self.api_version == 1:
            ...
            while int(round(time.time() * 1000)) < finish_time:
                ...
                self.log.info(('Current State: {0}').format(current_state))
