...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-aarch64/egg/deepstacks/macros.py
# Compiled at: 2017-07-18 07:05:50
...
def roll(f, n, d=1, inscale=1.0, outscale=1.0, w=1, h=1, m=None):
    ...
    first = True
    for i in range(n):
        res = ((0, (f, f ** i, w * h) if i != n - 1 else (int(f * inscale + 0.5), f ** i, w * h), 0, 0, 0, 0, {}),
         (
          0, f * d, 1, 1, 0, 0, {}),
         (
          0, f * d, 1, 1, 0, 0, {}),
         (
          0, int(f * outscale + 0.5) if first else f, 1, 1, 0, 0, m or {} if first else {})) + res
        first = False
