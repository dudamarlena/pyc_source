...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/neorg/web.py
# Compiled at: 2011-06-07 20:23:59
from __future__ import with_statement
import os, re
from sqlite3 import dbapi2 as sqlite3
from contextlib import closing
from flask import Flask, request, g, redirect, url_for, render_template, flash, send_from_directory
import jinja2
...
def find_temp_path(path):
    ...
    """
    temp_path_list = g.db.execute('select page_path from pages').fetchall()
...
def relpath_from_temp(page_path, temp_path):
    ...
    if page_path[:parent_len] != parent_path:
        raise ValueError("temp_path '%s' is not the correct template path of page_path '%s'." % (
         temp_path, page_path))
...
def filter_descendants(path, path_list):
    ...
    """
    lenpath = len(path)
    return [ remove_leading_slash(p[lenpath:]) for p in path_list if p.startswith(path)
           ]
...
def find_descendants(path):
    ...
    """
    path_list = [ row[0] for row in g.db.execute('select page_path from pages').fetchall()
                ]
...
def system_info():
    ...
    """
    with closing(connect_db()) as (db):
        sysinfo_current = db.execute('select *, max(updated) from system_info').fetchone()
    if sysinfo_current:
        return dict(zip([
         'version', 'updated'], sysinfo_current[:-1]))
...
def update_system_info():
    ...
    if oldver == curver:
        ...
        raise RuntimeError(('The old version ({0}) is newer than the version of the running version ({1}). Please install newer version').format(oldver, curver))
...
def update_search_index():
    ...
    doc_list = [ dict(zip(('page_path', 'page_text'), doc)) for doc in pages ]
    ix = get_search_index()
...
def get_page_text(page_path):
    row = g.db.execute('select page_text from pages where page_path = ?', [
     page_path]).fetchone()
    if row:
        return row[0]
    else:
...
def delete(page_path):
    if request.form.get('yes') == 'Yes':
        g.db.execute('delete from pages where page_path = ?', [
         page_path])
        g.db.execute('insert into page_history (page_path, page_text, page_exists) values (?, ?, 0)', [
         page_path, ''])
        g.db.commit()
...
def confirm_delete(page_path):
    ...
    return render_template('confirm_delete.html', title=path_as_title(page_path), page_path=page_path, page_html=page_html)
...
def save(page_path):
    if request.form.get('save') == 'Save':
        ...
        if get_page_text(page_path) == page_text:
            ...
            return redirect(url_for('page', page_path=page_path))
        g.db.execute('insert or replace into pages (page_path, page_text) values (?, ?)', [
         page_path, page_text])
        g.db.execute('insert into page_history (page_path, page_text) values (?, ?)', [
         page_path, page_text])
        g.db.commit()
    ...
    if request.form.get('preview') == 'Preview':
        ...
        return render_template('preview.html', title=path_as_title(page_path), page_path=page_path, page_text=page_text, page_html=page_html)
...
def edit(page_path):
    (page_text, page_html) = get_page_text_and_html(page_path)
    return render_template('edit.html', title=path_as_title(page_path), page_path=page_path, page_html=page_html, page_text=page_text if page_text else '')
...
def page(page_path):
    (page_text, page_html) = get_page_text_and_html(page_path)
    if page_text:
        return render_template('page.html', title=path_as_title(page_path), page_path=page_path, page_html=page_html)
    else:
        generated = gene_from_template(page_path)
        if generated:
            return generated
        if has_descendants(page_path):
            flash(('Page {0} does not exist. Showing sub-pages.').format(page_path))
            return redirect(url_for('descendants', page_path=page_path))
...
def gene_text_from_temp(page_path, temp_path, match):
    ...
    page_text = template.render({'path': page_path, 
...
def gene_from_template(page_path):
    ...
    if match:
        (page_text, tb_text) = gene_text_from_temp(page_path, temp_path, match, _mix=False, _debug=app.config['DEBUG'])
        if page_text is None:
            page_html = tb_text
        else:
            page_html = gene_html(page_text, page_path, _debug=app.config['DEBUG'])
        return render_template('page.html', title=path_as_title(page_path), temp_path=temp_path, page_path=page_path, page_html=page_html)
    else:
        return
...
def descendants(page_path):
    ...
    return render_template('descendants.html', title=path_as_title(page_path), link_list=link_list, page_path=page_path)


@app.route('/_history', defaults={'page_path': ''})
@app.route('/<path:page_path>/_history')
def history(page_path):
    page_history_list = g.db.execute('select history_id, page_text, updated from page_history where page_path = ?', [
     page_path]).fetchall()
    page_history_keys = ('history_id', 'page_text', 'updated')
    page_history = [ dict(zip(page_history_keys, row)) for row in reversed(page_history_list)
                   ]
    return render_template('history.html', title=path_as_title(page_path), page_path=page_path, page_history=page_history)


@app.route('/_old/<int:history_id>', defaults={'page_path': ''})
@app.route('/<path:page_path>/_old/<int:history_id>')
def old(page_path, history_id):
    page_text = g.db.execute('select page_text from page_history where history_id = ?', [
     history_id]).fetchone()
...
def favicon():
    ...
    if os.path.exists(favicon_path):
        return send_from_directory(app.config['NEORG_DIR'], 'favicon.ico')
    else:
...
def help(filename):
    ...
    if isinstance(helpdirpath, basestring) and os.path.exists(helpdirpath):
        return send_from_directory(helpdirpath, filename)
    else:
        return redirect(url_for('static', filename=os.path.join('help', filename)))
...
def search_results():
    ...
    if query:
        ...
        return render_template('search.html', title=query, search_query=query, results=list(results))
    else:
        return redirect(url_for('page', page_path=''))