...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/neorg/wiki.py
# Compiled at: 2011-06-07 22:56:15
"""
...
def convert_page_path_to_nodes(text, node_list=[]):
    ...
    if len(split) == 8:
        ...
        page_path = split[2]
        rest = ('').join(split[-2:])
        new_node_list = node_list + [nodes.Text(pre)] + list(gene_link(page_path))
...
_RE_PAGE_PATH = re.compile('(?P<head>^|[\\s%s])(?P<page_path>(/|(\\.{1,2}/)+)([a-zA-Z0-9][a-zA-Z0-9_\\-\\.\\+]*/)*)(?P<tail>$|[\\s%s])' % (
 '/' + re.escape(_PAGE_PATH_SE_SYMBOLS),
 '.' + re.escape(_PAGE_PATH_SE_SYMBOLS)))
...
def _path_tree(path_list):
    ...
    for page_path in path_list:
        ...
        if not page_path.endswith('/'):
            page_path += '/'
        sub_dict[None] = page_path

    return tree_dict


def _gene_link_tree_from_tree_dict(tree_dict, parent_key=None, bullet='*'):
    link_tree = []
    if None in tree_dict:
        if parent_key is None:
            parent_key = tree_dict[None]
        link_tree.append(with_children(nodes.paragraph, gene_link(parent_key + '/', tree_dict[None])))
    ...
    if children:
        ...
        for key in children:
            bullet_list += with_children(nodes.list_item, _gene_link_tree_from_tree_dict(tree_dict[key], key, bullet=bullet))
...
class ProcessDictDiff(Transform):
    ...
    def _replace_node(self, node):
        ...
        data_syspath_list = self._glob_list([ path.join(base_syspath, directives.uri(arg)) for arg in arguments
                                            ], glob_list_sorted)
        link = node.get('link', [])
        ...
        diff_data = data_table.diff(include=node.get('include'), exclude=node.get('exclude'))
        ...
        for data_syspath in data_table.names:
            ...
            link_magic = SafeMagic({'path': parent_relpath, 
               'relpath': path.relpath(parent_syspath, base_syspath)})
            from_base = path.relpath(data_syspath, base_syspath)
            table_row = [
             from_base] + [ diff_data[keystr].get(data_syspath, '') for keystr in keylist
                          ]
            if link:
                table_row.append(gene_links_in_paragraph(map(link_magic, link)))
            table_data.append(table_row)

        if node.hasattr('trans'):
            table_data = zip(*table_data)
        table_node = gene_table(table_data, title=title_from_path(arguments, node.get('base'), 'Diff of data found in: %s'))
...
def gene_entry(node_or_any):
    ...
    if isinstance(node_or_any, nodes.Node):
        entry += node_or_any
    elif isinstance(node_or_any, (list, tuple)) and all(isinstance(n, nodes.Node) for n in node_or_any):
        entry += node_or_any
    else:
...
def gene_table(list2d, title=None, colwidths=None, rows_highlight=[], classes=[], _baseclass='neorg-gene-table', rows_highlight_class='neorg-gene-table-rows-highlight'):
    """
...
def gene_link(name, uri=None):
    ...
    target = nodes.target(ids=[
     nodes.make_id(reference['name'])], names=[
     nodes.fully_normalize_name(reference['name'])], refuri=reference['refuri'])
    return (
     reference, target)
...
def gene_link_list(uri_list, bullet='*'):
    ...
    """
    bullet_list = nodes.bullet_list(bullet=bullet)
    bullet_list += [ nodes.list_item('', with_children(nodes.paragraph, gene_link(l))) for l in uri_list
                   ]
    return bullet_list
...
def transpose_dict(dict_of_dict):
    ...
    for subkey in subkeyset:
        transposed[subkey] = dict((key, dict_of_dict[key][subkey]) for key in dict_of_dict if subkey in dict_of_dict[key])
...
def title_from_path(pathlist, base=None, format='%s'):
    ...
    if len(pathlist) < 2:
        ...
        pathstr = path.join(base, '{%s}' % (', ').join([ arg.strip(path.sep) for arg in pathlist ]))
    ...
    if len(rowdata) > 0:
        if colwidths is not None:
            ...
            if len(rowdata[0]) > len(colwidths):
                messages.append(warning(('Not enough widths arguments for table-data: "{0}"').format(colwidthsstr)))
                colwidths = None
            elif len(rowdata[0]) < len(colwidths):
                messages.append(warning(('Too many widths arguments for table-data: "{0}"').format(colwidthsstr)))
                colwidths = None
        if len(rowdata[0]) < 1:
...
class TableData(Directive):
    ...
    option_spec = {'data': parse_text_list, 'base': directives.path, 
    ...
    def run(self):
        ...
        if path_order == 'sort_r':
            ...
            glob_list_sorted = sorted
        base_syspath = path.join(datadir, self.options.get('base', ''))
        data_syspath_list = self._glob_list([ path.join(base_syspath, directives.uri(arg)) for arg in self.arguments
                                            ], glob_list_sorted)
        from_base_list = [ path.relpath(x, base_syspath) for x in data_syspath_list ]
        data_keys = self.options.get('data', [])
        colwidths = self.options.get('widths')
        link = self.options.get('link')
        data_table = self._DictTable.from_path_list(data_syspath_list, from_base_list)
        ...
        for (data_syspath, row) in zip(data_syspath_list, rowdata[1:]):
            ...
            link_magic = SafeMagic({'path': parent_relpath, 
               'relpath': path.relpath(parent_syspath, base_syspath)})
            if link is not None:
                row.append(gene_links_in_paragraph(map(link_magic, link)))

        if 'trans' in self.options:
            rowdata = zip(*rowdata)
        (messages, colwidths) = check_rowdata_and_widths(rowdata, colwidths, self.state.reporter.warning)
        return [
         gene_table(rowdata, title=title_from_path(self.arguments, self.options.get('base'), 'Data found in: %s'), colwidths=colwidths)] + messages
...
class TableDataAndImage(Directive):
    ...
    optional_arguments = OPTIONAL_ARGUMENTS_INF
    final_argument_whitespace = True
    option_spec = {'data': parse_text_list, 'image': parse_text_list, 
    ...
    def run(self):
        ...
        for data_syspath in data_table.names:
            ...
            link_magic = SafeMagic({'path': parent_relpath, 
               'relpath': path.relpath(parent_syspath, base_syspath)})
            key_val_list = data_table.get_nested_fnmatch(data_syspath, data_keys)
            rows_highlight = [ i for (i, kv) in enumerate(key_val_list) if kv[0] in diffkeys
                             ]
            subtable = gene_table(key_val_list, title=path.relpath(data_syspath, base_syspath), rows_highlight=rows_highlight)
            col0 = [
             subtable]
            if link is not None:
                col0.append(gene_link_list(map(link_magic, link)))
            images = [ nodes.image(uri=path.join(parent_absurl, name), **image_options.get(i, {})) for (i, name) in enumerate(image_names)
                     ]
...
class DictDiff(Directive):
    ...
    optional_arguments = OPTIONAL_ARGUMENTS_INF
    final_argument_whitespace = True
    option_spec = {'base': directives.path, 'link': parse_text_list, 
    ...
    def run(self):
        return [
         dictdiff(rawsource=self.block_text, arguments=self.arguments, **self.options)]
...
class FindImages(Directive):
    ...
    def run(self):
        ...
        datadirurl = self._web.app.config['DATADIRURL']
        base_syspath = path.join(datadir, self.options.get('base', ''))
        image_syspath_list = self._glob_list([ path.join(base_syspath, directives.uri(arg)) for arg in self.arguments
                                             ])

        def gene_image(relpath):
            image_node = nodes.image(rawsource='', uri=path.join(datadirurl, relpath))
...
def gene_table_from_grid_dict(grid_dict, param, conv, title=None, **kwds):
    ...
    if len_axes > 1:
        ...
        list2d.append([
         ''] + [ gene_paragraph('%s=%s' % (param[1], y), classes=['neorg-grid-col']) for y in axes1
               ])
        for x in axes0:
            list1d = [gene_paragraph('%s=%s' % (param[0], x), classes=[
              'neorg-grid-row'])]
            for y in axes1:
                dct = grid_dict[(x, y)]
                if len_axes == 2:
                    ...
                    elem = gene_table_from_grid_dict(dct, param[2:], conv, **kwds)
                list1d.append(elem)
        ...
        axes0 = sorted(grid_dict.axes[0])
        list2d = [ [gene_paragraph('%s=%s' % (param[0], x), classes=['neorg-grid-row']), conv(grid_dict[x])] for x in axes0
                 ]
...
class GridImages(Directive):
    ...
    optional_arguments = OPTIONAL_ARGUMENTS_INF
    final_argument_whitespace = True
    option_spec = {'param': parse_text_list, 'image': parse_text_list, 
       'base': directives.path}
    has_content = False
    ...
    def run(self):
        ...
        def images_from_data_syspath(data_syspath):
            ...
            parent_relpath = path.dirname(data_relpath)
            parent_absurl = path.join(datadirurl, parent_relpath)
            return [ nodes.image(uri=path.join(parent_absurl, name), classes=['neorg-grid-images-image']) for (i, name) in enumerate(image_names)
                   ]
        ...
        title = title_from_path(self.arguments, self.options.get('base'), 'Comparing images of data found in: %s')
        return [
         gene_table_from_grid_dict(grid_dict, param, conv, title, classes=[
          'neorg-grid-images'])]
...
def gene_html(text, page_path=None, settings_overrides={}):
    ...
    new_settings_overrides.update(neorg_page_path=page_path, **settings_overrides)
    return publish_parts(text, writer=Writer(), reader=Reader(), settings_overrides=new_settings_overrides)['html_body']