...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/neorg/search.py
# Compiled at: 2011-06-07 16:44:27
import os
...
def get_schema():
    ...
    return Schema(page_path=ID(stored=True, unique=True), page_text=TEXT, hash=STORED)
...
def _update_doc(writer, searcher, page_path, page_text):
    ...
    if old_hash != new_hash:
        writer.update_document(page_path=unicode(page_path), page_text=page_text, hash=new_hash)
        return True
    else:
        return False
        return
