...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/neorg/tests/test_wiki.py
# Compiled at: 2011-06-06 14:22:20
import os
...
from neorg.tests.utils import MockWeb, MockDictTable, CheckData, trim, CaptureStdIO

def doctree_from_dict(data):
    if 'document' == data['node']:
        node = utils.new_document('<string>')
    else:
        node = getattr(nodes, data['node'])(rawsource='', **data.get('kwds', {}))
...
def dirtext(directive, contents, *args, **kwds):
    ...
    """
    lines = [
     ('.. {0}:: {1}').format(directive, (' ').join(args))]
...
class TestPagePathInlineMarkup(CheckData):
    ...
    data = [
     (
      trim('\n         This should become an /link/to/the/other/page/.\n         But this should not: not/the/link/to/the/other/page/.\n         '), ['/link/to/the/other/page/'], ['not/the/link/to/the/other/page/']), (trim('\n         /texts/in/the/literal/block/::\n\n             /will/not/be/a/link/\n         '), ['/texts/in/the/literal/block/'], ['/will/not/be/a/link/']), (trim('\n         */texts/in/the/inline/emphasis/literal/*\n         **/texts/in/the/inline/strong/emphasis/literal/**\n         '), ['/texts/in/the/inline/emphasis/literal/', '/texts/in/the/inline/strong/emphasis/literal/'], []), (trim('\n         ``/texts/in/the/inline/literal/``\n         '), [], ['/texts/in/the/inline/literal/']), (trim('\n         `/texts/in/the/hyper/link/ <link>`_`\n         '), [], ['/texts/in/the/hyper/link/']), (trim('\n         /texsts_with_a_-hyphen-after-the-under_score/\n         '), [], ['/texsts_with_a_-hyphen-after-the-under_score/']), (trim('\n         /texsts_with_a\\_-hyphen-after-the-under_score/\n         '), ['/texsts_with_a_-hyphen-after-the-under_score/'], [])] + [ ('%s/starts/with/' % s, ['/starts/with/'], []) for s in '/' + data_symbols ] + [ ('/ends/with/%s' % s, ['/ends/with/'], []) for s in '.' + data_symbols
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     ]
    ...
    def check(self, page_text, links, not_links):
        ...
        DictTable = object()
        setup_wiki(web=web, DictTable=DictTable)
        page_html = gene_html(page_text, page_path, settings_overrides={'report_level': 4})

        def html_link(link):
            return 'href="%(link)s">%(link)s</a>' % {'link': link}

        for l in links:
            assert html_link(l) in page_html, "link '%s' should be in `page_html`" % l

        for l in not_links:
            assert html_link(l) not in page_html, "link '%s' should NOT be in `page_html`" % l


class TestTableData(CheckData):
    data_file_tree_1 = {'ex/data_1/file.pickle': {'a': 1, 'b': 0}, 'ex/data_2/file.pickle': {'a': 2, 'b': 0}, 'ex/data_3/file.pickle': {'a': 3, 'b': 0}}
    data = [
     (
      {'args': ['ex/data*/file.pickle'], 'data': 'a b'},
      data_file_tree_1, []),
     (
      {'args': ['data*/file.pickle'], 'data': 'a b', 
         'base': 'ex', 
         'widths': '1 2 2', 
         'path-order': 'sort_r'},
      data_file_tree_1, []),
...
class TestTableDataAndImage(CheckData):
    ...
    def check(self, dirdata, file_tree, links, errors=[]):
        ...
        for key in dirdata['data'].split():
            ...
            eq_(num_td, num_data, ("there must be #{0} of '{2}'. #{1} exists").format(num_data, num_td, td_tag))
...
class TestFindImages(CheckData):
    ...
    def check(self, dirdata, files):
        ...
        glob_list.assert_called_with(map(lambda arg: os.path.join(base_syspath, arg), dirdata['args']))
...
class TestDictDiff(CheckData):
    ...
    data_file_tree = {'data_1/file.pickle': {'a': 1, 'b': 0}, 'data_2/file.pickle': {'a': 2, 'b': 0}, 'data_3/file.pickle': {'a': 3, 'b': 0}}
    data_dictdiff = {'a': {'data_1/file.pickle': 1, 'data_2/file.pickle': 2, 
...
class TestGridImages(CheckData):
    data_file_tree_1 = dict(('a_%d_b_%d_c_%d/file.pickle' % (a, b, c), dict(a=a, b=b, c=c)) for (a, b, c) in product([0, 1, 2], [3, 4, 5], [6, 7, 8]))
    data_file_tree_2 = dict((key, val) for (key, val) in data_file_tree_1.iteritems() if key not in [ 'a_%d_b_%d_c_%d/file.pickle' % abc for abc in [
     (0, 3, 6), (0, 3, 7)]
                                                                                                    ])
    ...
    def check(self, page_text, file_tree, stats):
        ...
        for (key, val) in stats.iteritems():
            eq_(page_html.count(key), val, "page_html must contains %d of '%s'" % (val, key))


class TestConvTexts(CheckData):
    textdir = os.path.join(os.path.dirname(__file__), 'texts')
    data = [ (os.path.relpath(abspath, textdir),) for abspath in glob(os.path.join(textdir, '*.txt'))
           ]

    def check(self, path):
        gene_html(file(os.path.join(self.textdir, path)).read(), settings_overrides={'report_level': 4})
...
def test_gene_html_no_fail():
    exception_message = '`gene_html` should not fail whatever happen during the html generation, provided DEBUG=False'
    page_text = trim('\n    .. dictdiff:: *\n\n    dictdiff is for raising Exception via DictTable\n    ')
    page_path = 'it does not depend on the page_path'
    web = MockWeb()
    DictTable = Mock()
    DictTable.from_path_list = Mock(side_effect=CheckException(exception_message))
    glob_list = Mock(return_value=[])
    setup_wiki(web=web, DictTable=DictTable, glob_list=glob_list)
    page_html = gene_html(page_text, page_path, _debug=False)
    assert exception_message in page_html
