...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/neorg/tests/test_data.py
# Compiled at: 2011-05-28 05:37:15
...
class TestGeneDict(CheckData):
    data = [
     (
      ('a', 'b', 'c.d', 'c.e'), {},
      {'a': 'val_a', 'b': 'val_b', 'c': {'d': 'val_c_d', 'e': 'val_c_e'}}),
     (
      ('a', 'b', 'c.d', 'c.e'), {'val_c_e': 'replaced'},
      {'a': 'val_a', 'b': 'val_b', 'c': {'d': 'val_c_d', 'e': 'replaced'}})]
...
class TestGeneDictFail(TestGeneDict):
    data = [
     (
      ('a', 'b', 'c.d', 'c.e'), {},
      {'a': 'val_a', 'b': 'val_b', 'c': {'d': 'val_c_d', 'e': 'WRONG'}}),
     (
      ('a', 'b', 'c.d', 'c.e'), {'val_c_e': 'replaced'},
      {'a': 'val_a', 'b': 'val_b', 'c': {'d': 'val_c_d', 'e': 'WRONG'}})]
...
class TestDictTableFilterByFnmatch(CheckData):
    ...
    dict_b_replaces = {'val_b': 'replaced_b', 'val_c_e': 'replaced_c_e'}
    ...
    data = [
     ...
     ({'name_a': dict_a},
      (
       [
        'c.*'],), {}, {'name_a': gene_dict('c.d', 'c.e')}),
     ({'name_a': dict_a, 'name_b': dict_b},
      (
       [
        'c.*'],), {},
      {'name_a': gene_dict('c.d', 'c.e'), 'name_b': gene_dict('c.d', 'c.e', **dict_b_replaces)}),
    ...
    data = [
     ...
     (
      [
       ...
       ((2, 2, 2), '222')],),
     (
      [ ((i, j * 0.1), None) for i in range(3) for j in range(4) if i != j
      ],),
     (
      [ ((i, j * 0.1, str(k)), None) for i in range(3) for j in range(4) for k in range(5) if i == j == k if not i == j + 1 == k + 2
      ],)]
