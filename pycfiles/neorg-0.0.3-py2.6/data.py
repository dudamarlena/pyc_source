...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/neorg/data.py
# Compiled at: 2011-06-02 15:55:25
import re
...
def load_any(path, ftype=None):
    ...
    def ftype_match(*types):
        return ftype in types or path.lower().endswith(tuple([ '.%s' % t for t in types ]))

    if ftype_match('pickle'):
        return pickle.load(file(path))
    if ftype_match('py', 'python'):
        return load_pyfile(path)
    if ftype_match('yaml', 'yml'):
...
class DictTable(object):
    ...
    def __eq__(self, other):
        return isinstance(other, self.__class__) and self._table == other._table and self._name == other._name and self._keys == other._keys
    ...
    def as_list(self, key_list=None, name_list=None, deficit=None, with_key=True, with_name=True, left_top='', as_str=repr):
        """
        ...
        def get(name, key):
            value = self._table.get(self.parse_key(key), {}).get(name, notfound)
    ...
    def filter_by_fnmatch(self, key_list, *args, **kwds):
        ...
        def filtered(name):
            return self._gene_dict(self.get_nested_fnmatch(name, key_list, *args, **kwds))
    ...
    def get_nested_fnmatch(self, name, key_list, *args, **kwds):
        data = []
        for key in key_list:
            data += get_nested_fnmatch(self.get_by_name(name), key, *args, **kwds)
    ...
    def _filter_by_re_list(keyiter, include=None, exclude=None):
        ...
        for key in keyiter:
            include_key = not include or any(inc.match(k) for inc in re_include for k in key)
            exclude_key = any(exc.match(k) for exc in re_exclude for k in key)
    ...
    def diff(self, include=None, exclude=None):
        ...
        for key in self._filter_by_re_list(self._table, include, exclude):
            ...
            if num_name != len(val_dict) or not self._identical(val_dict.itervalues()):
        ...
        for (i, key) in enumerate(diff_key_list):
            ...
            diffdict[keystr] = dict((name, val_dict[name]) for name in self._name if name in val_dict)
    ...
    def print_diff_as_table(self, include=None, exclude=None, deco_border=False, deco_header=False, deco_hlines=False, deco_vlines=False):
        diffdict = self.diff(include, exclude)
        if not diffdict:
            return
        ...
        table.add_rows([
         [
          ''] + self._name] + [ [keystr] + [ self._getrepr(diffdict[keystr], name) for name in self._name ] for keystr in sortedkey
                              ])
        print table.draw()
    ...
    def grid_dict(self, key_list):
        ...
        for name in self.names:
            value = tuple(self._table.get(self.parse_key(key), {}).get(name) for key in key_list)
...
def get_nested_fnmatch(dct, dictpath, sep='.'):
    ...
    """
    for (keylist, val) in sorted(_nested_fnmatch(dct, dictpath.split(sep))):
        yield (
         sep.join(map(str, keylist)), val)
...
class GridDict(object):
    ...
    def append(self, key, val):
        if not (isinstance(key, tuple) and len(key) == self.num):
            raise KeyError('key must be a tuple with length=%d' % self.num)
    ...
    def sorted_axes(self, reverse={}):
        if reverse == 'all':
            ...
            return [ sorted(a, reverse=reverse.get(i, False)) for (i, a) in enumerate(self.axes)
                   ]