...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/neorg/tests/test_web.py
# Compiled at: 2011-06-07 20:33:06
"""
...
"""
import os, re, tempfile, shutil, urllib
...
def setup_app():
    ...
    dirpath = tempfile.mkdtemp(prefix=TMP_PREFIX)
    ...
    return (
     app, db_fd, gene_html_with_encode)
...
class TestNEOrgWeb(TestNEOrgWebBase):
    ...
    def gene_pages(self, prefix='test', num=None):
        ...
        return zip(self.gene_page_paths(num, prefix), self.gene_page_texts(num, prefix))
    ...
    def check_save(self, page_path, page_text, page_html=None):
        ...
        response = self.app.post(save_path, data={'save': 'Save', 
           'page_text': page_text}, follow_redirects=True)
    ...
    def check_delete_yes(self, page_path, page_text):
        (save_response, page_html) = self.check_save(page_path, page_text)
        delete_path = urljoin('/', page_path, '_delete')
        response = self.app.post(delete_path, data={'yes': 'Yes'})
    ...
    def check_delete_no(self, page_path, page_text):
        (save_response, page_html) = self.check_save(page_path, page_text)
        delete_path = urljoin('/', page_path, '_delete')
        response = self.app.post(delete_path, data={'no': 'No'}, follow_redirects=True)
        assert page_html in response.data
    ...
    def test_delete_no(self):
        for (page_path, page_text) in self.gene_pages('TestDelNo'):
            yield (
             self.check_delete_no, page_path, page_text)
    ...
    def check_edit_existing_page(self, page_path, page_text):
        (response, page_html) = self.check_edit_new_page(page_path, page_text)
        assert page_html in response.data
    ...
    def test_edit_existing_page(self):
        for (page_path, page_text) in self.gene_pages('TestEditExisting'):
            yield (
             self.check_edit_existing_page, page_path, page_text)

    def check_preview(self, page_path, page_text, page_html=None):
        if page_html is None:
            page_html = self.gene_html(page_text)
        preview_path = urljoin('/', page_path, '_save')
        response = self.app.post(preview_path, data={'preview': 'Preview', 
           'page_text': page_text}, follow_redirects=True)
        assert page_html in response.data
        assert page_text in response.data
    ...
    def test_preview(self):
        for (page_path, page_text) in self.gene_pages('TestPreview'):
            yield (
             self.check_preview, page_path, page_text)
    ...
    def check_old(self, page_path, page_text, num_update=5):
        (hist_response, page_text_history) = self.check_history(page_path, page_text, num_update)
        old_links = re.findall('<a href="(.*_old/[0-9]*)">', hist_response.data)
        assert len(page_text_history) == len(old_links)
        for (link, old_text) in zip(old_links, reversed(page_text_history)):
    ...
    def test_help(self):
        ...
        response = self.app.get('/_help/index.html', follow_redirects=True)
        import neorg
    ...
    def check_temp(self, base_path, page_text, num):
        ...
        temp_text = page_text + '\n' + trim('\n        path: {{ path }}\n        relpath: {{ relpath }}\n        ') + '\n' + ('\n').join('args[%(i)d]: {{ args[%(i)d] }}' % {'i': i} for i in range(num))
    ...
    def test_temp(self):
        ...
        for (page_relpath, page_text) in self.gene_pages('TestTemp'):
            ...
            for num in range(1, num_max + 1):
                yield (
                 self.check_temp, page_path, page_text, num)

    def test_no_fail_page(self):

        def gene_page_with_error(debug):
            page_text = 'nonexistent_target_'
            page_html = self.gene_html(page_text, settings_overrides={'halt_level': 2}, _debug=debug)
            return page_html

        from docutils.utils import SystemMessage
        with CaptureStdIO() as (stdio):
            assert_raises(SystemMessage, gene_page_with_error, debug=True)
        stderr = stdio.read_stderr()
        assert '(ERROR/3) Unknown target name: "nonexistent_target".' in stderr
        with CaptureStdIO() as (stdio):
    ...
    def assert_page_path_in_search_result(page_path, response):
        ...
        assert page_path_link in response.data, "page_path '{0}' should be in the response"
    ...
    def test_search(self):
        num_test = 3
        for ((page_path, page_text), query) in zip(self.gene_pages('TestSearchMatch', num_test), map(('match{0} query{0}').format, range(num_test))):
            page_text = (' ').join([page_text, query])
            yield (self.check_search_match,
             page_path, page_text, query)

        for ((page_path, page_text), query) in zip(self.gene_pages('TestSearchNoMatch', num_test), map(('match{0} query{0}').format, range(num_test))):
    ...
    def test_search_deleted(self):
        ...
        self.check_search_match(page_path, page_text, query)
        self.check_delete_yes(page_path, page_text)
        response = self.get_search_response(query)
        assert_raises(AssertionError, self.assert_page_path_in_search_result, page_path, response)
    ...
    def test_jump_to_descendants(self):
        ...
        assert response.location == 'http://localhost/TestJumpToDesc/_descendants'
        response = self.app.get('TestJumpToDesc/NoSubPage/')
        assert response.location == 'http://localhost/TestJumpToDesc/NoSubPage/_edit'
...
class TestNEOrgWebWithEmptyDB(TestNEOrgWebSlow):
    ...
    def test_update_system_info_to_older(self):
        ...
        from neorg.web import update_system_info
        older_version = '0.0.0'
        assert older_version < neorg.__version__
