...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/Products/TemplateCacheManager/TemplateCacheManager.py
# Compiled at: 2008-07-03 18:43:14
"""
Page cache manager --
...
class TemplateCacheManager(CacheManager, SimpleItem):
    ...
    __ac_permissions__ = (('View management screens', ('getSettings', 'manage_main', 'manage_stats')), ('Change cache managers', ('manage_editProps', 'manage_invalidate', 'manage_purge'), ('Manager', )))
    ...
    def __init__(self, ob_id):
        self.id = ob_id
        self.title = ''
        self._settings = {'threshold': 500, 'cleanup_interval': 60, 'max_age': 3600, 'active': 'on_always'}
    ...
    manage_main = PageTemplateFile('www/editTCM', globals(), __name__='editTCM')
    ...
    def manage_editProps(self, title, settings=None, REQUEST=None):
        ...
        self._settings = {'threshold': int(settings['threshold']), 'cleanup_interval': int(settings['cleanup_interval']), 'max_age': int(settings['max_age']), 'active': settings.get('active', 'on_always')}
        cache = self.ZCacheManager_getCache()
        cache.initSettings(self._settings)
    ...
    manage_stats = PageTemplateFile('www/statsTCM', globals(), __name__='statsTCM')
...
manage_addTemplateCacheManagerForm = PageTemplateFile('www/addTCM', globals(), __name__='addTCM')
