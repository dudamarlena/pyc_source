...
# [GCC 8.4.0]
# Embedded file name: /Users/adam/code/claw/claw/html_quotations.py
# Compiled at: 2015-08-25 11:32:44
"""
...
def cut_microsoft_quote(html_message):
    ...
    splitter = html_message.xpath("//div[@style='border:none;border-top:solid #B5C4DF 1.0pt;padding:3.0pt 0cm 0cm 0cm']|//div[@style='padding-top: 5px; border-top-color: rgb(229, 229, 229); border-top-width: 1px; border-top-style: solid;']")
    if splitter:
        splitter = splitter[0]
        if splitter == splitter.getparent().getchildren()[0]:
            splitter = splitter.getparent()
    else:
        splitter = html_message.xpath("//div/div[@class='MsoNormal' and @align='center' and @style='text-align:center']/font/span/hr[@size='3' and @width='100%' and @align='center' and @tabindex='-1']")
        if len(splitter):
            splitter = splitter[0]
