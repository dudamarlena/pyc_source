...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ddshape\inc.py
# Compiled at: 2015-06-09 09:48:22
import math, os
...
def get_effective_radius(nums=1, d=0.001):
    ...
    default d=0.0001.
    """
    effective_radius = d * (3 * nums / math.pi / 4) ** (1.0 / 3)
...
def write_shape(point_max=(50, 50, 50), index=1, data='', precision=1):
    ...
    """
    replace_string = {'nx': point_max[0], 'ny': point_max[1], 
       'nz': point_max[2], 
       'total': index - 1, 
       'data': data}
...
def get_range(out_radius=25, cutoff=(0, 0, 0, 0, 0, 0), precision=1):
    ...
    """
    return (
     precision * (-1 * out_radius + cutoff[0]),
