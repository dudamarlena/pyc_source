...
# [GCC 8.4.0]
# Embedded file name: dnspodweb/app.py
# Compiled at: 2020-01-12 00:37:59
"""
...
def get_domainlist():
    if request.method == 'POST':
        if not request.form['token_id']:
            raise dnspod.DNSPodException('danger', '请输入Token ID。', -1)
        else:
            session['token_id'] = request.form['token_id']
        if not request.form['token_key']:
            raise dnspod.DNSPodException('danger', '请输入Token Key。', -1)
        else:
    ...
    for domain in response['domains']:
        ...
        list_sub = list_sub.replace('{{status_text}}', '启用' if domain['status'] == 'pause' else '暂停')
...
def post_domaincreate():
    if not request.form['domain']:
        raise dnspod.DNSPodException('danger', '参数错误。', -1)
    dnspod_api.api_call('Domain.Create', {'domain': request.form['domain']})
    raise dnspod.DNSPodException('success', '添加成功。', '/domainlist')
...
def get_domainstatus():
    ...
    if not request.args.get('status'):
        raise dnspod.DNSPodException('danger', '参数错误。', -1)
    dnspod_api.api_call('Domain.Status', {'domain_id': request.args.get('domain_id'), 'status': request.args.get('status')})
    raise dnspod.DNSPodException('success', ('启用' if request.args.get('status') == 'enable' else '暂停') + '成功。', '/domainlist')
...
def get_domainremove():
    ...
    raise dnspod.DNSPodException('success', '删除成功。', '/domainlist')
...
def get_recordlist():
    ...
    for record in response['records']:
        ...
        list_sub = list_sub.replace('{{enabled}}', '启用' if int(record['enabled']) else '暂停')
        list_sub = list_sub.replace('{{status_new}}', 'disable' if int(record['enabled']) else 'enable')
        list_sub = list_sub.replace('{{status_text}}', '暂停' if int(record['enabled']) else '启用')
    ...
    text = text.replace('{{title}}', '记录列表 - %s' % response['domain']['name'])
    text = text.replace('{{list}}', list_text)
...
def get_recordcreatef():
    ...
    if 'line_' + request.args.get('domain_id') not in session:
        response = dnspod_api.api_call('Record.Line', {'domain_id': request.args.get('domain_id'), 'domain_grade': request.args.get('grade')})
...
def post_recordcreate():
    ...
    if not request.form.get('value'):
        raise dnspod.DNSPodException('danger', '请输入记录值。', -1)
    mx = None
    ...
    if not request.form.get('ttl'):
        ttl = 600
    response = dnspod_api.api_call('Record.Create', {'domain_id': request.args.get('domain_id'), 'sub_domain': sub_domain or request.form['sub_domain'], 
    ...
    if request.form['remark'] != '':
        dnspod_api.api_call('Record.Remark', {'domain_id': request.args.get('domain_id'), 'record_id': response['record']['id'], 
           'remark': request.form['remark']})
    raise dnspod.DNSPodException('success', '添加成功。', '/recordlist?domain_id=%s' % request.args.get('domain_id'))
    return
...
def get_recordeditf():
    ...
    if not request.args.get('record_id'):
        raise dnspod.DNSPodException('danger', '参数错误。', -1)
    record = dnspod_api.api_call('Record.Info', {'domain_id': request.args.get('domain_id'), 'record_id': request.args.get('record_id')})
...
def post_recordedit():
    ...
    if not request.form.get('ttl'):
        ttl = 600
    dnspod_api.api_call('Record.Modify', {'domain_id': request.args.get('domain_id'), 'record_id': request.args.get('record_id'), 
    ...
    if request.form['remark'] != request.form['oremark']:
        dnspod_api.api_call('Record.Remark', {'domain_id': request.args.get('domain_id'), 'record_id': request.args.get('record_id'), 
           'remark': request.form['remark']})
    raise dnspod.DNSPodException('success', '修改成功。', '/recordlist?domain_id=%s' % request.args.get('domain_id'))
    return
...
def get_recordremove():
    ...
    if not request.args.get('record_id'):
        raise dnspod.DNSPodException('danger', '参数错误。', -1)
    dnspod_api.api_call('Record.Remove', {'domain_id': request.args.get('domain_id'), 'record_id': request.args.get('record_id')})
    raise dnspod.DNSPodException('success', '删除成功。', '/recordlist?domain_id=%s' % request.args.get('domain_id'))
...
def get_recordstatus():
    ...
    if not request.args.get('status'):
        raise dnspod.DNSPodException('danger', '参数错误。', -1)
    dnspod_api.api_call('Record.Status', {'domain_id': request.args.get('domain_id'), 'record_id': request.args.get('record_id'), 
       'status': request.args.get('status')})
    raise dnspod.DNSPodException('success', ('启用' if request.args.get('status') == 'enable' else '暂停') + '成功。', '/recordlist?domain_id=%s' % request.args.get('domain_id'))
