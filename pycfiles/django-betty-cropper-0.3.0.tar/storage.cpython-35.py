...
# [GCC 8.4.0]
# Embedded file name: /app/djbetty/storage.py
...
class BettyCropperStorage(Storage):
    ...
    def url(self, name, ratio='original', width=600, format='jpg', fixed=False):
        return '{base_url}/{id_string}/{ratio}/{width}.{format}'.format(base_url=self.build_base_url(fixed), id_string=self.build_id_string(name), ratio=ratio, width=width, format=format)

    def animated_url(self, name, format='gif', fixed=False):
        return '{base_url}/{id_string}/animated/original.{format}'.format(base_url=self.build_base_url(fixed), id_string=self.build_id_string(name), format=format)