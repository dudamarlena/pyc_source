...
# [GCC 8.4.0]
# Embedded file name: /app/djbetty/fields.py
...
class ImageDescriptor(FileDescriptor):
    ...
    def __get__(self, instance=None, owner=None):
        ...
        if isinstance(file, int) or file is None:
            ...
            if isinstance(file, File) and not isinstance(file, ImageFieldFile):
...
class ImageField(Field):
    ...
    def _check_unique(self):
        if self._unique_set_explicitly:
            return [
             checks.Error("'unique' is not a valid argument for a %s." % self.__class__.__name__, hint=None, obj=self, id='fields.E200')]
        else:
            return []
    ...
    def _check_primary_key(self):
        if self._primary_key_set_explicitly:
            return [
             checks.Error("'primary_key' is not a valid argument for a %s." % self.__class__.__name__, hint=None, obj=self, id='fields.E201')]
        else:
            return []
