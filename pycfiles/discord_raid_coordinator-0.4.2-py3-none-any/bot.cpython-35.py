...
# [GCC 8.4.0]
# Embedded file name: /home/ddesousa/git/discord-raid-coordinator/src/raid_coordinator/bot.py
# Compiled at: 2017-09-25 19:15:24
# Size of source mod 2**32: 29035 bytes
import asyncio, discord, re, traceback, pytz, urllib.parse
...
async def invite_user_to_raid(channel, user):
    ...
    await client.send_message(channel, '{}, you are now a member of this raid group.'.format(user.mention), embed=get_success_embed('{} has joined the raid!'.format(user.display_name)))
...
async def on_reaction_add(reaction, user):
    ...
    if reaction.emoji == get_join_emoji():
        ...
        if is_raid_channel(raid_channel) and raid_channel.overwrites_for(user).is_empty():
...
async def on_message(message):
    ...
    if perms.send_messages and message.content.startswith('$map'):
    ...
    if channel not in get_raid_channels(server) and is_raid_start_message(message):
        ...
        if raid_channel:
            ...
            raid_message = await client.edit_message(raid_message, '**{}**\n**in:** {}'.format(message.clean_content, raid_channel.mention), embed=get_raid_start_embed(user, started_dt, expiration_dt))
            await invite_user_to_raid(raid_channel, user)
            join_emoji = get_join_emoji()
        ...
        if is_raid_channel(channel) and message.content.startswith('$leaveraid'):
            await uninvite_user_from_raid(channel, user)
        else:
            if is_raid_channel(channel) and message.content.startswith('$listraid'):
...
def get_args():
    ...
    parser.add_argument('--raid-channel-regex', default='^raid-group-.+', help='Pattern which all raid channels must have. (default: %(default)s)')
    parser.add_argument('--raid-start-regex', default='^raid-(?!group).+', help='Regex for role mentions to trigger a raid. (default: %(default)s)')
    parser.add_argument('--raid-duration-seconds', type=int, default=7200, help='Time until a raid group expires, in seconds (default: %(default)s).')
    parser.add_argument('--raid-cleanup-interval-seconds', type=int, default=60, help='Time between checks for cleaning up raids (default: %(default)s)')
    parser.add_argument('--raid-viewer-role-name', default='raid-viewer', help='Role to user for users that can view active raids without participating (default: %(default)s)')
    parser.add_argument('--create-roles', default=False, action='store_true', help='If set, will create raid-organizer and raid-viewer roles.')
    parser.add_argument('--raid-organizer-role-name', default='raid-organizer', help='Role to use for users that can help organize raids (default: %(default)s)')
    parser.add_argument('--active-raids-channel-max-age-seconds', type=int, default=7200, help='Refreshes the active raids channel this often, even if there are no changes (default: %(default)s)')
    parser.add_argument('--active-raids-channel-name', default=None, help='Channel where active raids are listed (default: %(default)s)')
    parser.add_argument('--raid-join-emoji', default='👤', help='Emoji used for joining raids (default: %(default)s)')
    parser.add_argument('--raid-leave-emoji', default='🚪', help='Emoji used for leaving raids (default: %(default)s)')
    parser.add_argument('--raid-full-emoji', default='😟', help='Emoji used for full raid channels (default: %(default)s)')
