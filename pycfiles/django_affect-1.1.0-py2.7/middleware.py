...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/affect/middleware.py
# Compiled at: 2014-03-28 12:43:25
from django.core.cache import cache
from django.utils.encoding import smart_str
from .models import Criteria
from .utils import ALL_CRITERIA_KEY, CRITERIA_FLAGS_KEY, FLAG_CONFLICTS_KEY, cache_criteria, meets_criteria, settings
    ...
    def process_request(self, request):
        ...
        for name, flag in flags.items():
            ...
            if flag_conflicts is None:
                flag_conflicts = flag.conflicts.filter(active=True, priority__gte=flag.priority)
    ...
    def process_response(self, request, response):
        ...
        if hasattr(request, 'affected_tests'):
            for criteria, active in request.affected_tests.items():
                name = smart_str(settings.AFFECTED_TESTING_COOKIE % criteria.name)
                if criteria.max_cookie_age:
                    age = criteria.max_cookie_age
                else:
                    age = None
