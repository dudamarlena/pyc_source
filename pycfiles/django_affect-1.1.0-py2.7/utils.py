...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/affect/utils.py
...
settings.AFFECTED_TESTING_COOKIE = getattr(settings, 'AFFECTED_TESTING_COOKIE', 'dact_%s')
settings.AFFECTED_NONENTRY_DOMAINS = getattr(settings, 'AFFECTED_NONENTRY_DOMAINS', [])
...
def detect_device(request):
    ...
    if user_agent:
        if 'ipod' in user_agent or 'iphone' in user_agent or 'ipad' in user_agent or 'android' in user_agent or 'webos' in user_agent or 'windows phone' in user_agent:
            return Criteria.MOBILE_DEVICE
        if 'opera mini' in user_agent or 'application/vnd.wap.xhtml+xml' in request.META.get('HTTP_ACCEPT', ''):
...
def meets_criteria(request, criteria_name):
    ...
    if criteria.everyone:
        ...
        if criteria.persistent:
            criteria_cookie = request.COOKIES.get(settings.AFFECTED_COOKIE % criteria.name, '')
            if criteria_cookie:
        ...
        if criteria.referrer:
            ...
            if referrer in criteria_referrers:
                return True
        if criteria.entry_url and referrer != request.META.get('HTTP_HOST', ''):
            if referrer not in settings.AFFECTED_NONENTRY_DOMAINS:
...
def uncache_criteria(**kwargs):
    criteria = kwargs.get('instance')
    cache.set_many({CRITERIA_KEY % criteria.name: None, 
...
post_save.connect(uncache_criteria, sender=Criteria, dispatch_uid='save_criteria')
post_delete.connect(uncache_criteria, sender=Criteria, dispatch_uid='delete_criteria')
m2m_changed.connect(uncache_criteria, sender=Criteria.users.through, dispatch_uid='m2m_criteria_users')
m2m_changed.connect(uncache_criteria, sender=Criteria.groups.through, dispatch_uid='m2m_criteria_groups')
m2m_changed.connect(uncache_criteria, sender=Criteria.flags.through, dispatch_uid='m2m_criteria_flags')
...
m2m_changed.connect(uncache_flag, sender=Flag.conflicts.through, dispatch_uid='m2m_flag_conflicts')