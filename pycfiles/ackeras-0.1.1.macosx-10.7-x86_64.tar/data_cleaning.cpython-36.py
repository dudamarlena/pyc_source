...
# [GCC 8.4.0]
# Embedded file name: /anaconda3/lib/python3.6/site-packages/ackeras/data_cleaning.py
# Compiled at: 2018-09-06 08:37:37
# Size of source mod 2**32: 8298 bytes
import sys, pandas as pd, numpy as np
from datetime import datetime
import matplotlib.pyplot as plt, seaborn as sns, pdb
...
class AccuratPreprocess:
    ...
    def __init__(self, input_data=None, path=None):
        self.path = path
    ...
    def get_data(self, skip_bad=False, sep=',', encoding='utf_8'):
        ...
        if not isinstance(self.path, str):
            ...
            if detect_format == 'json':
                ...
                try:
                    df = pd.read_csv((self.path), error_bad_lines=skip_bad,
                      sep=sep)
                    self.raw_data = df
                    print('Data saved in class')
                    ...
                    print('Error parsing the csv. Try the correct encoding e.g. get_data(encodeing="ascii")')

            else:
                raise ImportError('Path did not lead to a understandable file type, did not import')

    def deal_na(self, deal='normal', thresh=0.5, deal_cat=True):
        if not isinstance(self.raw_data, pd.DataFrame):
            raise AssertionError
        else:
            raw_data = self.raw_data
            raw_data = raw_data.dropna(axis=1,
              thresh=(int(thresh * raw_data.shape[0])))
            ...
            def normal(df):
                ...
                for col in nan_cols:
                    ...
                    try:
                        ...
                        print(f'Skipping column {col} because not numerical, using ffill instead, deactivate with "deal_cat = False"')
                        if deal_cat:
    ...
    def data_encoding(self, categorical_features, label_encoder=False):
        ...
        for col in cat_columns:
            ...
            if len(list(col_data.unique())) > 2:
                ...
                renamed_columns = pd.Series(list(one_hot.labels)).apply(lambda x: ', '.join([str(col), str(x)]))
                one_hot_df = pd.DataFrame(one_hot_data,
                  columns=renamed_columns)
                cat_data = pd.concat([cat_data, one_hot_df], axis=1)
                cat_data = cat_data.drop((sorted(list(renamed_columns))[(-1)]),
                  axis=1)

        raw_data = raw_data.drop(cat_columns, axis=1)
        raw_data = pd.concat([raw_data, cat_data], axis=1)
        raw_data[cat_data.columns] = raw_data[cat_data.columns].astype(CategoricalDtype())
    ...
    def datetime_index(self, timecolumn, set_index=False):
        if isinstance(timecolumn, list):
            ...
            try:
                raw_data[timecolumn] = raw_data[timecolumn].apply(lambda x: pd.Timestamp(x))
                if set_index:
    ...
    def fit_transform(self, categorical_features=None, timecolumn=None, save=False, drop_rest=True, outputplot=False, extreme_drop=None):
        ...
        if drop_rest:
            self.data = self.data.drop((self.data.select_dtypes('object').columns),
              axis=1)
