...
# [GCC 8.4.0]
# Embedded file name: /anaconda3/lib/python3.6/site-packages/ackeras/regression.py
# Compiled at: 2018-09-05 08:54:05
...
class Regression:
    ...
    def __init__(self, data, y, problem=None):
        if not isinstance(data, pd.DataFrame):
            ...
            if not isinstance(y, str):
                raise AssertionError
            elif not problem in ('classification', 'regression', None):
                raise AssertionError
            else:
    ...
    def ridge_regression(self, opt_space=np.arange(0.1, 10.0, 0.5)):
    ...
    def logistic_regression(self, opt_space=[
 0.001, 0.01, 0.1, 1, 10, 100, 1000]):
        feautures, target = self.X_series, self.y_series
        parameters = {'C':opt_space, 
         'penalty':('l1', 'l1')}
    ...
    def fit_predict(self):
        ...
        if self.problem == 'classification':
            ...
            if self.problem == 'regression':
                ...
                raise TypeError('--FLAG-- : Problem not understood, no regression done. Rerun with "problem" not None')
        return (
         self.opt_regres, returning)