...
# [GCC 8.4.0]
# Embedded file name: /anaconda3/lib/python3.6/site-packages/ackeras/clustering.py
# Compiled at: 2018-09-05 06:43:50
# Size of source mod 2**32: 5190 bytes
import sklearn, pandas as pd, numpy as np, random, pdb
...
class Clustering:
    ...
    def __init__(self, data, categorical_features=None, pre_k=None):
        ...
        """
        self.data = data
        self.categorical_features = categorical_features if np.issubdtype(data[categorical_features].dtypes, np.int) else [data.columns.get_loc(c) for c in data.columns if c in categorical_features]
        self.is_cat = False
        self.check_df()
        self.pre_k = pre_k
    ...
    def check_df(self):
        ...
        for n, i in enumerate(range_data):
            if np.abs(i > 3):
                print(f"-- Flag --: the column {normalize_data.columns[n]} does not seem to be normalized")
        ...
        if self.categorical_features is not None:
            ...
            print('Did not found categorical variables, specify at "categorical_features"')
    ...
    def dbscan(self):
        ...
        pred_label = pd.Series(dbscan.fit_predict(cluster_data)).apply(lambda x: None if x == -1 else x)
        return pred_label
    ...
    def silouhette_analysis(self, cluster_data, pca=False, prototype=False, end_range=20):
        ...
        for n_cluster in range_n_cluster:
            ...
            clusterer = KMeans(n_clusters=n_cluster, random_state=(self.seed))
            train, test = train_test_split(cluster_data,
            ...
            test = self.pca_trans.transform(test) if self.pca_trans is not None else test
        ...
        print(f"The best cluster has silhoutte score of {np.max(sil_avg)} k={opt_k}")
        return opt_k
