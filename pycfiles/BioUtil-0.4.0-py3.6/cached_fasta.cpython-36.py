...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/BioUtil/cached_fasta.py
...
class cachedFasta:
    __doc__ = 'fasta seq fetcher, use buffer to improve performance\n        example: \n        fa = cachedFasta("hg19.fa")\n        seq = fa[\'chr1\'][2000:3000] \n    '
    ...
    def __init__(self, fa, trans=None, buffer_size=('-2k', '8k')):
        ...
        self._cachedFasta__set_trans(trans)
    ...
    def buffer_size(self):
        ...
        support both integer or number with suffix k,M
        """
        return self._cachedFasta__buffer_size

    @buffer_size.setter
    def buffer_size(self, size_tuple):
        self._cachedFasta__buffer_size = tuple(map(readable2num, size_tuple))
        self.buffer = _seqBuffer(self.fa, self.buffer_size)

    @property
    def trans(self):
        """transform function"""
        return self._cachedFasta__trans

    def __set_trans(self, func):
        if func is None:
            func = lambda x: x
        self._cachedFasta__trans = func
...
class _chrFetcher:
    __doc__ = 'fetch seq on one reference'
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, int):
            ...
            if not (self.chr == self.buffer.chr and key.start >= self.buffer.start and key.stop <= self.buffer.end):
