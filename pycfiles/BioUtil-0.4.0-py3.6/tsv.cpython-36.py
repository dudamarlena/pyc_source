...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/BioUtil/tsv.py
# Compiled at: 2018-11-26 22:39:24
...
import abc, shlex
__all__ = [
 'tsvFile', 'tsvRecord', '_read_sha_header', '_write_sha_header']
...
class tsvRecord(abc.ABC):
            ...
            if rec is None:
                ...
                if isinstance(rec, list) or isinstance(rec, tuple):
                    ...
                    if len(rec) != len(cls._fields):
                        raise ValueError('field number not match (%s provide, %s needed). ' % (len(rec), len(cls._fields)) + 'line:\n' + '\t'.join(map(shlex.quote, rec)))
    ...
    def to_str(self):
        return self._delim.join(self._fields_encoder.get(attr, str)(getattr(self, attr)) for attr in self._fields) + self._newline
