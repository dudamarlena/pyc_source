...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/mozilla/io/file.py
# Compiled at: 2009-11-15 20:26:48
import silme.io.file, silme.io.clients, os, silme.format
from silme.core import EntityList, Package, Blob, Structure
from mozilla.core.filter import *

@classmethod
def get_l10nobjectfilter(cls, path, source=False, code='default', parser=None, filter=None):
    """
  Method which invokes the right get_l10nobject classmethod and filters 
  the results.
  """
    l10nObject = cls.get_structure(path=path, source=source, code=code, parser=parser)
    if filter is not None:
        entitylist = l10nObject.get_entitylist()
        for id in entitylist.keys():
            if not filter['filter'].filter(filter['module'], filter['relativepath'], id):
...
@classmethod
def get_entitylistfilter(cls, path, source=False, code='default', parser=None, filter=None):
    """
  Method which invokes the right get_entitylist classmethod and filters 
  the results.
  """
    try:
        entityList = cls.get_entitylist(path=path, source=source, code=code, parser=parser)
    except Exception, e:
        if str(e).find('not utf_8 like expected, but utf_8_sig') != -1:
            entityList = EntityList()
            entityList.id = os.path.basename(path)
            entityList.log('ERROR', path + ': this File contains a Byte Order ' + 'Mark (BOM)! Parsing file aborted! The information ' + 'below may be incorrect...')
        else:
            raise Exception, e
...
@classmethod
def get_package(cls, path, code='default', object_type='l10nobject', source=False, ignore=[
 'CVS', '.svn', '.DS_Store', '.hg'], filter=None):
    """
  ...
  """
    ...
    for elem in l:
        ...
        if elem in ignore:
            continue
        if filter is not None:
            if not filter['filter'].filter(filter['module'], os.path.join(relativepath, elem)):
                continue
        if os.path.isdir(p2):
            if filter is not None:
                filter['relativepath'] = os.path.join(relativepath, elem)
            l10nPackage._packages[elem] = cls.get_package(p2, code=code, object_type=object_type, source=source, ignore=ignore, filter=filter)
        else:
            try:
                ...
                try:
                    if object_type == 'object':
                        ...
                        l10nPackage._structures[elem] = cls.get_entitylistfilter(p2, source=source, code=code, parser=parser, filter=filter)
                        if l10nPackage._structures[elem].encoding == 'utf_8_sig':
                            l10nPackage._structures[elem].log('ERROR', 'this file ' + 'starts with a UTF8-BOM, but the use of it is not allowed!')
                    else:
                        l10nPackage._structures[elem] = cls.get_l10nobjectfilter(p2, source=source, code=code, parser=parser, filter=filter)
                        if l10nPackage._structures[elem].encoding == 'utf_8_sig':
                            l10nPackage._structures[elem].log('ERROR', 'this file ' + 'starts with a UTF8-BOM, but the use of it is not allowed!')
