...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/mozilla/core/main.py
# Compiled at: 2009-11-15 19:20:53
...
class CompareInit(object):
  ...
  items will be done
  @param accesskeys: (optional) boolean value. If True: access keys test will be performed
  ...
  """
    ...
    def __init__(self, reference='en-US', inipath=None, l10nbase=None, locales=None, fp=None, turbo=False, accesskeys=False, mercurial=False, merge=None, testreference=False, output=0, inputtype='source', checkvalidity=False, verbose=5, compatibility=False, returnmode='dump', returnvalue='full', force=False, forcelocaledir=False):
...
def optionhelper(optionpack):
  ...
  """
    if optionpack.inputtype == 'source':
        ...
        try:
            optionpack.manifest = ManifestParser.get_locales(optionpack.xpi.read('chrome.manifest', 'utf_8'))
        except KeyError, e:
            raise KeyError('Could not find the chrome.manifest file in the ' + 'following archive: ' + optionpack.inipath + ' ! Incompatible extension? ')
        else:
        ...
        raise TypeError('There is not such an input type')
    ...
    optionpack.objectType = 'entitylist'
    return
...
def doCompare(optionpack):
  ...
  """
    ...
    for locale in optionpack.locales:
        ...
        if optionpack.inputtype == 'source':
            (localediffpackage, localel10npackage, referencel10npackage) = read_and_compare_source(optionpack, locale, localediffpackage, localel10npackage, referencel10npackage, l10npackagesreference)
        elif optionpack.inputtype == 'xpi':
            (localediffpackage, localel10npackage, referencel10npackage) = read_and_compare_xpi(optionpack, locale, localediffpackage, localel10npackage, referencel10npackage, l10npackagesreference)
        elif optionpack.inputtype == 'file':
            (localediffpackage, localel10npackage, referencel10npackage) = read_and_compare_file(optionpack, locale, localediffpackage, localel10npackage, referencel10npackage, l10npackagesreference)
        elif optionpack.inputtype == 'dir':
            (localediffpackage, localel10npackage, referencel10npackage) = read_and_compare_dir(optionpack, locale, localediffpackage, localel10npackage, referencel10npackage, l10npackagesreference)
        localediffpackage.id = locale
        ...
        if optionpack.checkvalidity:
            try:
                ...
                l10ndiffpackages.package(locale).log('WARNING', 'path: ' + os.path.join(optionpack.l10nbase, locale) + '; type: OSError: ' + str(e))
                print 'WARNING: path: ' + os.path.join(optionpack.l10nbase, locale) + '; type: OSError: ' + str(e)
                optionpack.checkvalidity = False
            except Exception, e:
                l10ndiffpackages.package(locale).log('WARNING', 'path: ' + os.path.join(optionpack.l10nbase, locale) + '; type: Exception' + str(e))
                print 'WARNING: path: ' + os.path.join(optionpack.l10nbase, locale) + '; type: Exception: ' + str(e)
                optionpack.checkvalidity = False

        statistics = Statistics(locale, optionpack.reference)
        resultset.details[locale] = optionpack.fp.make_l10npackagediff(l10ndiffpackages.package(locale), l10npackages.package(locale), l10npackagesreference, statistics=statistics, indent=0, optionpack=optionpack)
