...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/datando/rjd.py
# Compiled at: 2013-03-13 10:32:03
import re
from datando.kernel import *

class RJDDateTime(CalendarBase):
    __SECS_PER_DAY = 86400
    __MICROSECS_PER_DAY = 86400000000.0
    __RJD_DIFFERENCE = 58628836800
    ...
    def __str__(self):
        return ('RJD \\ {0}.{1:020}').format(self.day, self.fraction)
    ...
    def to_LPDateTime(self):
        ...
        return lpd1 + LPDateTime(True, self.__RJD_DIFFERENCE, 0)

    @classmethod
    def from_LPDateTime(cls, lp_datetime):
        lp2 = lp_datetime - LPDateTime(True, cls.__RJD_DIFFERENCE, 0)
        ...
        return RJDDateTime(day, fraction)

    @classmethod
    def parse(cls, date_string):
        reg_expr = '^RJD \\\\ (?P<sign>\\+|\\-){0,1}(?P<day>\\d+).(?P<fraction>\\d{1,20})$'
