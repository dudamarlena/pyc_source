...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/datando/gregorian.py
# Compiled at: 2013-06-15 11:32:43
import re
...
class GregorianDateTime(CalendarBase):
    ...
    def __init__(self, year=1, month=1, day=1, hour=0, minute=0, second=0, microsecond=0, style=None):
        if style not in (None, 'ab nativitate', 'ab incarnatione (Florence)', 'ab incarnatione (Pisa)',
                         'byzantine', 'venetian'):
            raise ValueError()
    ...
    def __str__(self):
        ...
        if self.style == None:
            ...
            if self.month < 3:
                y = self.year - 1
        return ('{0} \\ {1}-{2:02d}-{3:02d} T {4:02d}:{5:02d}:{6:02d}.{7:06d}').format(st_string, y, self.month, self.day, self.hour, self.minute, self.second, self.microsecond)
    ...
    def parse(cls, date_string):
        reg_expr = '^Gregorian \\\\ ((?P<style>[a-zA-Z_ ]*) \\\\)* (?P<sign>\\+|\\-){0,1}(?P<year>\\d*)\\-(?P<month>\\d{2})-(?P<day>\\d{2}) T (?P<hh>\\d{2}):(?P<mm>\\d{2}):(?P<ss>\\d{2})\\.(?P<ms>\\d{6})$'
