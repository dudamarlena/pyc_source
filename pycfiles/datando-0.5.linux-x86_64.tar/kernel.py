...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/datando/kernel.py
# Compiled at: 2013-06-22 11:30:58
import copy, re
...
class CalendarBase:
    ...
    def __gt__(self, other):
        ...
        if s.positive and not o.positive:
            ...
            if s.positive:
                if s.second > o.second or s.second == o.second and s.microsecond > o.microsecond:
                    return True
                return False
            else:
                return False
    ...
    def __neg__(self):
        ...
        if s == None:
            ...
            return LPDateTime(not s.positive, s.second, s.microsecond)
    ...
    def __add__(self, other):
        ...
        if s.positive:
            ...
            if o.positive:
                return LPDateTime(True, o.second, o.microsecond) + LPDateTime(False, s.second, s.microsecond)
            else:
                return -(LPDateTime(True, s.second, s.microsecond) + LPDateTime(True, o.second, o.microsecond))
...
class LPDateTime(CalendarBase):
    ...
    def parse(cls, date_string):
        reg_expr = '^(?P<sign>\\+|\\-){0,1}(?P<ss>\\d+).(?P<ms>\\d{1, 6})$'
    ...
    def from_float(cls, number):
        second = int(number)
        microsecond = int((number - second) * 1000000.0)
        positive = number >= 0
...
class LPInterval:
    ...
    def parse(cls, date_string):
        ...
        reg_expr = '^Interval \\\\ START (?P<start>.+) ; END (?P<end>.+) ; FUZZY START (?P<fuzzystart>.+) ; FUZZY END (?P<fuzzyend>.+) ; FUZZY FUNCTION (?P<fuzzyfun>.+)$'
        m = re.match(reg_expr, date_string)
        if m:
            gd = m.groupdict()
            str_start = gd['start']
    ...
    def fuzzy_membership(self, lp_datetime):
        if self.contains(lp_datetime):
            ...
            if not (self.fuzzy_start is None or self.fuzzy_function is None) and lp_datetime >= self.fuzzy_start and lp_datetime <= self.start:
