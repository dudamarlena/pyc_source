...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/datando/djd.py
# Compiled at: 2013-03-13 10:29:14
import re
from datando.kernel import *

class DJDDateTime(CalendarBase):
    __SECS_PER_DAY = 86400
    __MICROSECS_PER_DAY = 86400000000.0
    __DJD_DIFFERENCE = 59926564800
    ...
    def __str__(self):
        return ('DJD \\ {0}.{1:020}').format(self.day, self.fraction)
    ...
    def to_LPDateTime(self):
        ...
        return lpd1 + LPDateTime(True, self.__DJD_DIFFERENCE, 0)

    @classmethod
    def from_LPDateTime(cls, lp_datetime):
        lp2 = lp_datetime - LPDateTime(True, cls.__DJD_DIFFERENCE, 0)
        ...
        return DJDDateTime(day, fraction)

    @classmethod
    def parse(cls, date_string):
        reg_expr = '^DJD \\\\ (?P<sign>\\+|\\-){0,1}(?P<day>\\d+).(?P<fraction>\\d{1,20})$'
