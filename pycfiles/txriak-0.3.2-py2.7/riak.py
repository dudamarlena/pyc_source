...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/txriak/riak.py
...
"""
import random, base64, urllib, re, json, codecs
...
def getPageWithHeaders(contextFactory=None, scheme='http', host='127.0.0.1', port=80, path='/', *args, **kwargs):
    ...
    if 'postdata' in kwargs:
        ...
        body = None
    d = client.Agent(reactor).request(kwargs['method'], url, Headers(kwargs['headers']), body)
...
class RiakClient(object):
    ...
    def __init__(self, host='127.0.0.1', port=8098, prefix='riak', mapred_prefix='mapred', client_id=None, r_value=2, w_value=2, dw_value=0):
        """
        ...
        if client_id:
            ...
            self._client_id = 'py_' + base64.b64encode(str(random.randint(1, 1073741824)))
        self._r = r_value
        self._w = w_value
        self._dw = dw_value
        return
    ...
    def is_alive(self):
        ...
        """
        response = yield RiakUtils.http_request_deferred('GET', self._host, self._port, '/ping')
    ...
    def list_buckets(self):
        ...
        """
        params = {'buckets': 'true'}
        host, port, url = RiakUtils.build_rest_path(self, None, None, None, params)
...
class RiakMapReduce(object):
    ...
    def add_bucket_key_data(self, bucket, key, data):
        ...
        if self._input_mode == 'bucket' or self._input_mode == 'key_filter':
            raise Exception('Already added a %s, ' % self._input_mode + "can't add an object.")
        else:
    ...
    def map(self, function, options=None):
        ...
        if isinstance(function, list):
            ...
            language = 'javascript'
        mr = RiakMapReducePhase('map', function, RiakUtils.get_value('language', options, language), RiakUtils.get_value('keep', options, False), RiakUtils.get_value('arg', options, None))
    ...
    def reduce(self, function, options=None):
        ...
        if isinstance(function, list):
            ...
            language = 'javascript'
        mr = RiakMapReducePhase('reduce', function, RiakUtils.get_value('language', options, language), RiakUtils.get_value('keep', options, False), RiakUtils.get_value('arg', options, None))
        self._phases.append(mr)
        return self
    ...
    def run(self, timeout=None):
        ...
        response = yield RiakUtils.http_request_deferred('POST', host, port, url, {}, content)
        if response[0]['http_code'] > 299:
            raise Exception('Error running map/reduce job. Error: ' + response[1])
...
class RiakMapReducePhase(object):
    ...
    def _to_array(self):
        ...
        internally.
        """
        stepdef = {'keep': self._keep, 'language': self._language, 
           'arg': self._arg}
...
class RiakLinkPhase(object):
    ...
    def _to_array(self):
        ...
        internally.
        """
        stepdef = {'bucket': self._bucket, 'tag': self._tag, 
           'keep': self._keep}
        return {'link': stepdef}
...
class RiakBucket(object):
    ...
    def set_properties(self, props):
        ...
        response = yield RiakUtils.http_request_deferred('PUT', host, port, url, headers, content)
        if response == None:
...
class RiakObject(object):
    ...
    def store(self, w=None, dw=None):
        ...
        headers = {'Accept': 'text/plain, */*; q=0.5', 'Content-Type': self.get_content_type(), 
    ...
    def delete(self, dw=None):
        ...
        params = {'dw': dw}
        host, port, url = RiakUtils.build_rest_path(self._client, self._bucket, self._key, None, params)
        response = yield RiakUtils.http_request_deferred('DELETE', host, port, url)
    ...
    def _populate_links(self, link_headers):
        ...
        for link_header in link_headers.strip().split(','):
            ...
            matches = re.match('\\<\\/([^\\/]+)\\/([^\\/]+)\\/([^\\/]+)\\>; ?riaktag="([^\']+)"', link_header)
            if matches != None:
...
class RiakUtils(object):
    ...
    def build_rest_path(cls, client, bucket=None, key=None, spec=None, params=None):
        ...
        if params != None:
            ...
            path += '?' + s
        return (
         client._host, client._port, path)

    @classmethod
    def http_request_deferred(cls, method, host, port, path, headers=None, obj=''):
        """
        ...
        if headers == None:
            headers = {}
        return getPageWithHeaders(contextFactory=None, scheme='http', host=host, method=method, port=port, path=path, headers=headers, postdata=obj, timeout=20)
