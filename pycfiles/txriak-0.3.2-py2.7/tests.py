...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/txriak/tests.py
...
class RiakTestCase1(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        keys = [
         'foo', 'foo1', 'foo2', 'bar', 'baz', 'ba_foo1foo1',
         'foo2', 'foo3', 'blue_foo1']
    ...
    def test_list_keys(self):
        ...
        self.assertEqual(sorted(['foo1', 'foo2']), sorted(keys))
    ...
    def test_list_buckets(self):
        ...
        self.assertTrue('bucket1' in buckets)
        self.assertTrue('bucket2' in buckets)
        yield obj1.delete()
        yield obj2.delete()
    ...
    def test_javascript_source_map(self):
        ...
        yield obj.store()
        job = client.add('bucket', 'foo').map('function (v) { return [JSON.parse(v.values[0].data)]; }')
    ...
    def test_javascript_named_map(self):
        ...
        obj = bucket.new('foo', 2)
        yield obj.store()
        job = client.add('bucket', 'foo').map('Riak.mapValuesJson')
        result = yield job.run()
        self.assertEqual(result, [2])
    ...
    def test_javascript_source_map_reduce(self):
        ...
        job = client.add('bucket', 'foo').add('bucket', 'bar').add('bucket', 'baz').map('function (v) { return [1]; }').reduce('function(v) { return [v.length]; } ')
        result = yield job.run()
    ...
    def test_javascript_named_map_reduce(self):
        ...
        yield bucket.new('bar', 3).store()
        yield bucket.new('baz', 4).store()
        job = client.add('bucket', 'foo').add('bucket', 'bar').add('bucket', 'baz').map('Riak.mapValuesJson').reduce('Riak.reduceSum')
        result = yield job.run()
    ...
    def test_javascript_key_filter_map_reduce(self):
        ...
        yield bucket.new('bar', 3).store()
        yield bucket.new('baz', 4).store()
        job = client.add({'bucket': 'bucket', 'key_filters': [
                         [
                          'starts_with', 'ba']]}).map('function (v) { return [1]; }').reduce('function(v) { if(v.length) return [v.length]; else return []} ')
        result = yield job.run()
        self.assertEqual(result, [2])
    ...
    def test_javascript_bucket_map_reduce(self):
        ...
        job = client.add(bucket.get_name()).map('Riak.mapValuesJson').reduce('Riak.reduceSum')
        result = yield job.run()
        self.assertEqual(result, [9])
    ...
    def test_javascript_arg_map_reduce(self):
        ...
        obj = bucket.new('foo', 2)
        obj.store()
        job = client.add('bucket', 'foo', 5).add('bucket', 'foo', 10).add('bucket', 'foo', 15).add('bucket', 'foo', -15).add('bucket', 'foo', -5).map('function(v, arg) { return [arg]; }').reduce('Riak.reduceSum')
        result = yield job.run()
    ...
    def test_erlang_map_reduce(self):
        ...
        yield obj.store()
        job = client.add('bucket', 'foo').add('bucket', 'bar').add('bucket', 'baz').map([
    ...
    def test_store_and_get_links(self):
        ...
        client = riak.RiakClient(client_id=RIAK_CLIENT_ID)
        bucket = client.bucket('bucket')
        obj = bucket.new('foo', 2).add_link(bucket.new('foo1')).add_link(bucket.new('foo2'), 'tag').add_link(bucket.new('foo3'), 'tag2!@#%^&*)')
        yield obj.store()
        del obj
    ...
    def test_link_walking(self):
        ...
        yield obj_3.store()
        obj = bucket.new('foo', 2).add_link(obj_1).add_link(obj_2, 'tag').add_link(obj_3, 'tag2!@#%^&*)')
        yield obj.store()
        obj = yield bucket.get('foo')
        job = obj.link('bucket')
