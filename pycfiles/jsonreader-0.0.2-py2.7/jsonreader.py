...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jsonreader/jsonreader.py
# Compiled at: 2012-10-12 08:39:01
__author__ = 'the5fire'
__blog__ = 'http://www.the5fire.net'
__desc__ = '\n    工作时需要处理ajax的json数据，浏览器无法直接打开，就顺手写了这工具，可发送get和post请求。\n    有问题可到博客交流。\n'
import wx, json, urllib, urllib2, functools, operator, codecs
HEAD_START = 0.1
SECRET_LENGTH = 100
...
class MainFrame(wx.Frame):
    ...
    def init_get_panel(self):
        ...
        self.urlinput = urlinput = wx.TextCtrl(bkg, -1, '点击下面url或者手动输入', size=(400,
                                                                               25))
    ...
    def post_handler(self, event):
        ...
        try:
            req = urllib2.Request(url=url, data=urllib.urlencode(content), headers=headers)
    ...
    def add_tree_nodes(self, parent_item=None, items=None):
        for item in items:
            ...
            elif isinstance(items, dict) and not isinstance(items[item], (dict, list)):
...
class Client(wx.App):
    ...
    def OnInit(self):
        win = MainFrame(None, title='JsonReader by the5fire.net', size=(800, 900), pos=(400,
                                                                                        0))
        win.Show()
        return True
