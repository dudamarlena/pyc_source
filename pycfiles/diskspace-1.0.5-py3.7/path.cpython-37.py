...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\diskspace\path.py
# Compiled at: 2019-01-30 12:53:29
...
class ShowPath:
    ...
    def __init__(self, path='.', exclude=None, include_folder=True, human=False, include_stats=True, whitelist=False):
        self.files = {}
        self.path = path
    ...
    def getPathFiles(self):
        ...
        for i, file in enumerate(os.listdir(self.path)):
            ...
            if not self.include_folder:
                ...
                self.files[i] = FileInfo(file, size, self.readable(size), is_folder, self.creation_date(file))
    ...
    def pretty_print(self, spacing=11):
        ...
        files = '\n'.join([f"{g.human if self.human else g.bytes:<14} {g.name}" for g in sorted(allFiles, key=(lambda x: x.bytes), reverse=True)])
        output = files
