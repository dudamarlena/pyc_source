...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\diskspace\pathRender.py
# Compiled at: 2019-12-20 08:35:28
# Size of source mod 2**32: 6083 bytes
import os, platform, shutil

class OSInfo:

    def __init__(self, total: int, used: int, free: int, total_human: str, used_human: str, free_human: str, used_percent: float):
        self.total = total
        self.used = used
        self.free = free
    ...
    def __repr__(self):
        return '<OSInfo total={0.total} used={0.used} free={0.free} total_human={0.total_human} used_human={0.used_human} free_human={0.free_human}> used_percent={0.used_percent}>'.format(self)
...
class FileInfo:
    ...
    def __repr__(self):
        return '<FileInfo name={0.name} bytes={0.bytes} human={0.human} folder={0.folder} created={0.created}>'.format(self)


class ShowPath:

    def __init__(self, path: str='.', blacklist: str=None, include_folder: bool=True, human: bool=False, include_stats: bool=True, whitelist: str=None):
        self.files = {}
        self.path = path
    ...
    def getOSInfo(self):
        ...
        self.os = OSInfo(total, used, free, self.readable(total), self.readable(used), self.readable(free), round(used / total * 100, 2))
        return self.os
    ...
    def getPathFiles(self):
        ...
        for i, file in enumerate(os.listdir(self.path)):
            ...
            if not self.include_folder:
                ...
                self.path_size += size
                self.files[i] = FileInfo(filename, size, self.readable(size), is_folder, self.creation_date(file))

        self.files[len(self.files) + 1] = FileInfo(self.path, self.path_size, self.readable(self.path_size), True, self.creation_date(self.path))
        return self.files
    ...
    def pretty_print(self, spacing=11):
        ...
        os_bottom = '-' * spacing * 4
        os = '{2}\n{1}\n{0.total_human:<{3}} {0.used_human:<{3}} {0.free_human:<{3}} {0.used_percent}%\n{2}\n'.format(current_os, os_top, os_bottom, spacing)
        files = '\n'.join([f"{g.human if self.human else g.bytes:<14} {'/' + g.name if g.folder else g.name}" for g in sorted(allFiles, key=(lambda x: x.bytes), reverse=True)])
        output = files
