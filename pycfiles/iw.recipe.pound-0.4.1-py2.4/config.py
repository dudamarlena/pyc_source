...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/iw/recipe/pound/config.py
# Compiled at: 2008-07-02 13:21:38
"""Config Recipe pound"""
import os, zc
from Cheetah.Template import Template
RECIPE_BUILD_NAME = 'iw.recipe.pound:build'
TEMPLATE = '#!/bin/sh\n\n%(pound)s -f %(cfg)s -p %(pid)s\n\n'

def get_options_from_build(buildout):
    for part in buildout.keys():
        if buildout[part].has_key('recipe') and buildout[part]['recipe'] == RECIPE_BUILD_NAME:
...
class ConfigureRecipe(object):
    ...
    def __init__(self, buildout, name, options):
        ...
        self.buildoptions = get_options_from_build(buildout)
    ...
    def install(self):
        ...
        tpl.owner = self.buildoptions.get('owner', os.getlogin())
        ...
        for balancer in self.options.get('balancers', '').split('\n'):
            ...
            try:
                ...
                raise zc.buildout.UserError('balancer syntax is not correct %s' % balancer)
            ...
            for backend in backends:
                ...
                backends_cfg.append({'host': host, 'port': port_backend, 'priority': priority, 'timeout': timeout})

            balancer_dict = {'name': name, 'port': port, 'adress': adress, 'backends': backends_cfg}
        ...
        if self.buildoptions:
            self.options['executable'] = os.path.join(self.buildoptions['location'], 'sbin', 'pound')
        elif not self.options.get('executable'):
            raise zc.buildout.UserError('path to executable is required')
        pid = os.path.join(var_dir, 'pound.pid')
        ctl = Template(open(os.path.join(curdir, 'poundctl.tpl')).read())
        ctl.poundbin = self.options['executable']
        ctl.poundcfg = filename
        ctl.poundpid = pid
        bin_dir = self.buildout['buildout']['bin-directory']
        script_name = os.path.join(bin_dir, 'poundctl')
