...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/transit/writer.py
# Compiled at: 2017-12-12 16:52:26
import sys, msgpack, re
...
class Writer(object):
    ...
    def __init__(self, io, protocol='json', opts={'cache_enabled': True}):
        if protocol == 'json':
            ...
            raise ValueError("'" + protocol + "' is not a supported protocol. " + "Protocol must be 'json', 'json_verbose', or 'msgpack'.")
...
class Marshaler(object):
    ...
    def emit_nil(self, _, as_map_key, cache):
        if as_map_key:
            return self.emit_string(ESC, '_', None, True, cache)
        else:
            return self.emit_object(None)
    ...
    def emit_boolean(self, b, as_map_key, cache):
        if as_map_key:
            return self.emit_string(ESC, '?', b, True, cache)
        return self.emit_object(b)
        ...
        if len(tag) == 1:
            if isinstance(rep, pyversion.string_types):
                ...
                if isinstance(rep, pyversion.string_types):
                    ...
                    raise AssertionError('Cannot be encoded as string: ' + str({'tag': tag, 'rep': rep, 
                       'obj': obj}))
            else:
                self.emit_tagged(tag, rep, cache)
        elif as_map_key:
            raise AssertionError('Cannot be used as a map key: ' + str({'tag': tag, 'rep': rep, 
               'obj': obj}))
        else:
            self.emit_tagged(tag, rep, cache)
...
class MsgPackMarshaler(Marshaler):
    ...
    default_opts = {'prefer_strings': False, 'max_int': MSGPACK_MAX_INT, 
...
class JsonMarshaler(Marshaler):
    ...
    default_opts = {'prefer_strings': True, 'max_int': JSON_MAX_INT, 
    ...
    def emit_object(self, obj, as_map_key=False):
        ...
        if tp in pyversion.string_types:
            self.io.write('"')
            self.io.write(('').join([ ESCAPE_DCT[c] if c in ESCAPE_DCT else c for c in obj ]))
            self.io.write('"')
