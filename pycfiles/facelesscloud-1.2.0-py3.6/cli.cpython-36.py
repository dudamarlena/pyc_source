...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/facelesscloud/cli.py
# Compiled at: 2019-06-11 18:46:13
# Size of source mod 2**32: 15550 bytes
""" CLI functions """
import sys, json, aaargh, urllib3, requests, pyqrcode, simplejson
...
def makeconf():
    ...
    if sshkey:
        if not validate_ssh_key(sshkey):
...
def spawn(configfile, time, flavor, operating_system, region, currency, sshkey, kickstart, force):
...
def extend(subid, extendtime, currency):
    ...
    if not subid or not extendtime:
        ...
        data = {'subid':subid, 
         'expend_time':extendtime,  'currency':currency}
