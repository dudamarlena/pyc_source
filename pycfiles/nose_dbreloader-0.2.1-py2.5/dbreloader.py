...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/nose_dbreloader/dbreloader.py
# Compiled at: 2008-05-28 15:25:33
import logging, tempfile, os, sys, re, subprocess
from nose.plugins import Plugin
from nose.util import tolist
ENGINES = ('mysql', )
engine_re = '(' + ('|').join(ENGINES) + ')'
r = engine_re + '://(?:([\\w]+)(?::([\\w]+))?@)?([\\w.-]+)(?::([\\d]{1,5}))?'
regex = re.compile(r)
...
def split_dsn(dsn):
    ...
    if not m:
        ...
        return dict(zip(('engine', 'username', 'password', 'host', 'port'), m.groups()))
...
def make_option_maker(parser, cmdline_prefix='', env_prefix='', dest_prefix=''):
    ...
    def option_maker(name, action='store', help='', default=None):
        ...
        if default is None:
            ...
            _default = default
        parser.add_option('--' + cmdline_prefix + name.lower(), action=action, dest=dest_prefix + name.lower(), default=_default, help=help + ' [' + env_prefix + name.upper() + ']')
        return
...
class DatabaseSnapshotPlugin(Plugin):
    ...
    def options(self, parser, env=None):
        ...
        omaker = make_option_maker(parser, dest_prefix='dbdump', env_prefix='NOSE_DBDUMP', cmdline_prefix='dbdump')
