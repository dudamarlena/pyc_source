...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/udplog/udplog.py
...
import logging, socket, time, simplejson
...
def augmentWithFailure(eventDict, failure, why=None):
    ...
    eventDict['message'] = why or eventDict['excValue'] or eventDict['excType']
...
class UDPLogger(object):
    ...
    def __init__(self, host=DEFAULT_HOST, port=DEFAULT_PORT, defaultFields=None):
    ...
    def serialize(self, category, eventDict):
        ...
        """
        msg = simplejson.dumps(eventDict, default=lambda x: str(repr(x)), skipkeys=True)
    ...
    def serializeFailure(self, category, eventDict, size, failure, why):
        ...
        for key in ('logLevel', 'logName', 'excText', 'excType', 'excValue', 'lineno',
                    'filename', 'funcName'):
            if key in eventDict:
    ...
    def log(self, category, eventDict):
        ...
        try:
            ...
            data = self.serializeFailure(category, eventDict, len(data), failure, why)
            try:
                self.socket.send(data)
            except Exception:
                import sys
...
class UDPLogHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        """
        try:
            eventDict = {'category': self.category, 
            ...
            extra = {name:value for name, value in vars(record).iteritems() if name not in _DEFAULT_LOGGING_ATTRIBUTES if name not in _DEFAULT_LOGGING_ATTRIBUTES}
...
class ConfigurableUDPLogHandler(UDPLogHandler):
    ...
    def __init__(self, defaultFields=None, category='python_logging', host=DEFAULT_HOST, port=DEFAULT_PORT, includeHostname=True):
        """
