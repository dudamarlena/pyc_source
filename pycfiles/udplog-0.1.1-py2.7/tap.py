...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/udplog/tap.py
# Compiled at: 2013-09-26 18:33:00
"""
...
class Options(usage.Options):
    optParameters = [
     ...
     ('rabbitmq-queue-size', None, 2500, 'Maximum number of log events to buffer for RabbitMQ'),
...
def makeService(config):
    ...
    udplogServer = internet.UDPServer(port=config['udplog-port'], protocol=dispatcher, interface=config['udplog-interface'], maxPacketSize=65536)
    ...
    if config['rabbitmq-host']:
        ...
        factory = UDPLogClientFactory(rabbitmq.RabbitMQPublisher, dispatcher, vhost=config['rabbitmq-vhost'], exchange=config['rabbitmq-exchange'], queueSize=config['rabbitmq-queue-size'])
