...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/udplog/test/test_twisted.py
# Compiled at: 2013-09-04 05:30:50
"""
...
class UDPLogObserverTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.observer = twisted.UDPLogObserver(self.logger, defaultCategory='test')
    ...
    def test_emitErr(self):
        ...
        """
        self.publisher.msg(failure=failure.Failure(Exception('Test')), why='Something went wrong', isError=1)
        self.assertEqual(1, len(self.logger.logged))
        category, eventDict = self.logger.logged[(-1)]
    ...
    def test_emitErrNoWhy(self):
        ...
        """
        self.publisher.msg(failure=failure.Failure(Exception('Test')), why=None, isError=1)
        self.assertEqual(1, len(self.logger.logged))
        category, eventDict = self.logger.logged[(-1)]
        self.assertEqual('Test', eventDict['message'])
        self.assertNotIn('why', eventDict)
    ...
    def test_emitErrNoWhyNoArgs(self):
        ...
        """
        self.publisher.msg(failure=failure.Failure(Exception()), why=None, isError=1)
        self.assertEqual(1, len(self.logger.logged))
        category, eventDict = self.logger.logged[(-1)]
    ...
    def test_emitWarning(self):
        ...
        """
        self.publisher.msg(warning=UserWarning('Oops!'), category='exceptions.UserWarning', filename='test.py', lineno=342, format='%(filename)s:%(lineno)s: %(category)s: %(warning)s')
        self.assertEqual(1, len(self.logger.logged))
        category, eventDict = self.logger.logged[(-1)]
        self.assertEqual('Oops!', eventDict['message'])
        self.assertNotIn('warning', eventDict)
        self.assertEqual('exceptions.UserWarning', eventDict['warningCategory'])
...
class QueueProducerTest(unittest.TestCase):
    ...
    def test_putLimitedQueue(self):
        ...
        """
        self.producer = twisted.QueueProducer(self.callback, size=3, clock=self.clock)
        self.producer.resumeProducing()
        self.producer.pauseProducing()
        self.producer.put(1)
        self.producer.put(2)
