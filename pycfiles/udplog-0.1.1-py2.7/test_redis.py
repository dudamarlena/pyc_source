...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/udplog/test/test_redis.py
...
class RedisPublisherServiceTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.publisher = redis.RedisPublisher(self.dispatcher, self.client, 'test_list')
    ...
    def test_sendEvent(self):
        ...
        """
        event = {'category': 'test', 'message': 'test', 
    ...
    def test_sendEventUnserializable(self):
        ...
        event = {'category': 'test', 'message': Object(), 
           'timestamp': 1340634165}
        self.publisher.sendEvent(event)
        self.assertEqual(0, len(self.client.pushes))
    ...
    def test_sendEventNoClient(self):
        ...
        self.publisher.sendEvent(event)
        self.assertEqual(0, len(self.client.pushes))
        self.assertEqual(0, len(self.flushLoggedErrors()), 'Unexpected error logged')
...
class RedisPushMultiClientTest(unittest.TestCase):
    ...
    def test_lpushMultiple(self):
        ...
        d = defer.gatherResults([ multiClient.lpush('test_list', value) for i in xrange(50)
                                ])
        d.addCallback(cb)
        return d
...
class MakeServiceTest(unittest.TestCase):
    ...
    def test_services(self):
        ...
        """
        config = {'redis-hosts': set(['10.0.0.2', '10.0.0.3']), 'redis-port': 6379, 
           'redis-key': 'udplog'}
