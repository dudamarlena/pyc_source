...
# [GCC 8.4.0]
# Embedded file name: /home/bouma/gitprojects/provgroningen/buildout/src/djinn_contenttypes/djinn_contenttypes/models/publishable.py
...
class PublishableMixin(object):
    ...
    def is_published(self):
        ...
        """
        now = datetime.now()
        return (not self.publish_from or self.publish_from < now) and (not self.publish_to or self.publish_to > now)
    ...
    def is_public(self):
        ...
        """
        return super(PublishableMixin, self).is_public and not self.is_tmp and self.is_published


class PublishableContent(PublishableMixin, BaseContent):
    """ Anything that is publishable should extend this class """
    publish_from = models.DateTimeField(_('Publish from'), null=True, db_index=True, blank=True, default=None)
    publish_to = models.DateTimeField(_('Publish to'), null=True, db_index=True, blank=True, default=None)
    publish_notified = models.BooleanField(_('Event sent'), default=False, help_text=_('Publish event is sent'))
    unpublish_notified = models.BooleanField(_('Event sent'), default=False, help_text=_('Un-publish event is sent'))
    remove_after_publish_to = models.BooleanField(_('Remove the content ater publication to has past'), default=False)
