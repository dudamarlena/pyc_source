...
# [GCC 8.4.0]
# Embedded file name: /home/bouma/gitprojects/provgroningen/buildout/src/djinn_contenttypes/djinn_contenttypes/models/history.py
# Compiled at: 2016-01-28 12:43:52
...
UNPUBLISHED = 4
STATUS_MAP = [
 '', 'created', 'changed', 'published', 'unpublished']

class HistoryManager(models.Manager):

    def log(self, instance, status_flag, user=None, change_message=''):
        entry = self.model(user=user, object_id=instance.id, object_ct=ContentType.objects.get_for_model(instance), status_flag=status_flag, change_message=change_message)
        entry.save()

    def get_last(self, instance, *flags, **kwargs):
        _filter = {'object_id': instance.id, 
...
class History(models.Model):
    ...
    def __unicode__(self):
        ...
        try:
            ...
            return 'History(id=%d) %s' % (self.id, status)
