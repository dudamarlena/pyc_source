...
# [GCC 8.4.0]
# Embedded file name: /home/bouma/gitprojects/provgroningen/buildout/src/djinn_contenttypes/djinn_contenttypes/views/base.py
# Compiled at: 2016-06-28 11:54:57
...
from djinn_contenttypes.utils import get_object_by_ctype_id, has_permission, check_get_url
...
class TemplateResolverMixin(object):
    ...
    def get_template_names(self):
        ...
        if self.request.GET.get('modal', False) or self.request.is_ajax():
            ...
            modal = ''
        return [
         '%s/%s_%s%s.html' % (self.app_label, self.ct_name, self.mode,
          modal),
         'djinn_contenttypes/base_%s%s.html' % (self.mode, modal)]

    @property
    def delete_url(self):
        return reverse('%s_delete_%s' % (self.app_label, self.ct_name), kwargs={'pk': self.object.id})

    @property
    def add_url(self):
        return reverse('%s_add_%s' % (self.app_label, self.ct_name), kwargs=self.kwargs)

    @property
    def edit_url(self):
        return reverse('%s_edit_%s' % (self.app_label, self.ct_name), kwargs={'pk': self.object.id})
    ...
    def view_url(self):
        ...
        return reverse('%s_view_%s' % (self.app_label, self.ct_name), kwargs=kwargs)
...
class MimeTypeMixin(object):
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        return super(MimeTypeMixin, self).render_to_response(context, **response_kwargs)
...
class CTMixin(object):
    ...
    def get_object(self, queryset=None):
        ...
        return get_object_by_ctype_id(self.kwargs['ctype'], self.kwargs.get('id', self.kwargs.get('pk', None)))
...
class AcceptMixin(object):
    ...
    def render_to_response(self, context, **response_kwargs):
        if self.is_json:
            ...
            return HttpResponse(json.dumps(context, skipkeys=True, default=json_serializer), **response_kwargs)
        else:
            if self.is_text:
                try:
                    del response_kwargs['mimetype']
...
class DetailView(AbstractBaseView, BaseDetailView, HistoryMixin):
    ...
    def get_template_names(self):
        ...
        if self.request.is_ajax() and not self.request.REQUEST.get('modal', False):
            templates = ['%s/snippets/%s.html' % (
              self.app_label, self.ct_name)] + templates
        return templates
    ...
    @property
    def created(self):
        return hasattr(self.object, 'publish_from') and self.object.publish_from or self.object.created

    def get(self, request, *args, **kwargs):
        """ We need our own implementation of GET, to be able to do permission
        checks on the object """
        self.object = self.get_object()
        perm = CTRegistry.get(self.ct_name).get('view_permission', 'contenttypes.view')
...
class CreateView(TemplateResolverMixin, SwappableMixin, AcceptMixin, BaseCreateView, HistoryMixin):
    mode = 'add'
    fk_fields = []
    ...
    def get_object(self, queryset=None):
        if not getattr(self.real_model, 'create_tmp_object', False):
            ...
            if self.request.REQUEST.get('tmp_id'):
                obj = self.real_model.objects.get(id=self.request.REQUEST.get('tmp_id'))
            else:
                obj = self.real_model.objects.create(creator=self.request.user, is_tmp=True, changed_by=self.request.user)
                if self.get_initial():
    ...
    def get(self, request, *args, **kwargs):
        ...
        self.object = None
        perm = CTRegistry.get(self.ct_name).get('add_permission', 'contenttypes.add_contenttype')
...
class UpdateView(TemplateResolverMixin, SwappableMixin, AcceptMixin, BaseUpdateView, HistoryMixin):
    mode = 'edit'
    ...
    def get(self, request, *args, **kwargs):
        ...
        self.object = self.get_object()
        perm = CTRegistry.get(self.ct_name).get('edit_permission', 'contenttypes.change_contenttype')
        if not has_permission(perm, self.request.user, self.object):
            return HttpResponseForbidden()
...
class DeleteView(TemplateResolverMixin, SwappableMixin, AcceptMixin, BaseDeleteView, HistoryMixin):
    mode = 'delete'

    def delete(self, request, *args, **kwargs):
        self.object = self.get_object()
        perm = CTRegistry.get(self.ct_name).get('delete_permission', 'contenttypes.delete_contenttype')
        if not has_permission(perm, self.request.user, self.object):
            return HttpResponseForbidden()
        deleted = False
        try:
        ...
        if deleted:
            if self.is_json:
                return HttpResponse(json.dumps({'message': 'bye bye'}), content_type='application/json')
            else:
                if self.is_text:
            ...
            if self.is_json:
                return HttpResponse(json.dumps({'message': 'error'}), status=202, content_type='application/json')
            else:
                if self.is_text:
                    return HttpResponse('Error', status=202, content_type='text/plain')
