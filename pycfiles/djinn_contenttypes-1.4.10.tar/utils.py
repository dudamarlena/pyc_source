...
# [GCC 8.4.0]
# Embedded file name: /home/bouma/gitprojects/provgroningen/buildout/src/djinn_contenttypes/djinn_contenttypes/views/utils.py
# Compiled at: 2015-06-11 05:55:03
import logging
    ...
    for model in models:
        ...
        if kwargs.get('view', True):
            if name:
                ...
                viewname = None
            views.append(generate_view_url(model, modelname, modulename, name=viewname))
        if kwargs.get('edit', True):
            if name:
                editname = '%s_edit_%s' % name
            else:
                editname = None
            views.append(generate_edit_url(model, modelname, modulename, name=editname))
        if kwargs.get('add', True):
            if name:
                addname = '%s_add_%s' % name
            else:
                addname = None
            views.append(generate_add_url(model, modelname, modulename, name=addname))
        if kwargs.get('delete', True):
            if name:
                delname = '%s_delete_%s' % name
            else:
                delname = None
            views.append(generate_delete_url(model, modelname, modulename, name=delname))
...
def generate_view_url(model, modelname, modulename, name=None):
    ...
    if hasattr(real_model, 'slug'):
        expr = '^%s/(?P<pk>[\\d]+)/(?P<slug>[^\\/]+)/?$' % modelname
    else:
        expr = '^%s/(?P<pk>[\\d]+)/?$' % modelname
...
def generate_delete_url(model, modelname, modulename, name=None):
    expr = '^delete/%s/(?P<pk>[\\d]+)/?$' % modelname
...
def generate_add_url(model, modelname, modulename, name=None):
    ...
    if extends(model, FKContentMixin):
        kwargs['fk_fields'] = [ f.name for f in model._meta.fields if isinstance(f, models.ForeignKey)
                              ]
        expr = '^add/%s' % modelname
        ...
        expr += '/?$'
    view = find_view(model, modulename, suffix='CreateView', default=CreateView, **kwargs)
    return url(expr, view, name=name)
...
def generate_edit_url(model, modelname, modulename, name=None):
    ...
    keyword arguments """
    expr = '^edit/%s/(?P<pk>[\\d]+)/?$' % modelname
    name = name or '%s_edit_%s' % (modulename, modelname)
    form_class = find_form_class(model, modulename)
    kwargs = {}
    if form_class:
        kwargs['form_class'] = form_class
    view = find_view(model, modulename, suffix='UpdateView', default=UpdateView, **kwargs)
    return url(expr, view, name=name)
