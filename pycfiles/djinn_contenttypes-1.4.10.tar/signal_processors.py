...
# [GCC 8.4.0]
# Embedded file name: /home/bouma/gitprojects/provgroningen/buildout/src/djinn_contenttypes/djinn_contenttypes/models/signal_processors.py
...
from djinn_contenttypes.models.history import CREATED, CHANGED, PUBLISHED, UNPUBLISHED, History
    ...
    if implements(instance, PublishableContent):
        if instance.is_public:
            ...
            if not History.objects.has_been(instance, PUBLISHED, UNPUBLISHED):
                ...
                changed = True
            elif History.objects.get_last(instance, PUBLISHED, UNPUBLISHED, as_flag=True) == UNPUBLISHED:
                publish.send(sender, instance=instance)
                changed = True
            if changed:
                History.objects.log(instance, PUBLISHED, user=instance.changed_by)
                instance.__class__.objects.filter(pk=instance.pk).update(publish_notified=True, unpublish_notified=False)
        else:
            last_state = History.objects.get_last(instance, PUBLISHED, UNPUBLISHED, as_flag=True)
            if last_state == PUBLISHED:
                unpublish.send(sender, instance=instance)
                instance.__class__.objects.filter(pk=instance.pk).update(unpublish_notified=True)
                if not instance.is_deleted:
                    History.objects.log(instance, UNPUBLISHED, user=instance.changed_by)