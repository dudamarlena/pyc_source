...
# [GCC 8.4.0]
# Embedded file name: /Users/brianbruggeman/repos/mine/oogli/oogli/Program.py
# Compiled at: 2016-02-07 09:28:54
import ctypes
from collections import OrderedDict
from glfw import gl
import numpy as np
from .shaders import Shader, VertexShader, FragmentShader, GeometryShader, TessellationControlShader, TessellationEvaluationShader
...
class Program(object):
    ...
    def build(self):
        shaders = [ shader for shader in (
         self.vert, self.frag, self.tc, self.te, self.geo) if isinstance(shader, Shader)
                  ]
        program_id = self.program
        assert program_id != 0
        error_message = 'Both a vertex and fragment shader must be provided.'
        assert len(shaders) >= 2, error_message
        if not (self.vert is not None and self.frag is not None):
            ...
            assert log.strip() and result == gl.TRUE and log_length == 0, log
    ...
    def load(self, mode=gl.TRIANGLES, fill=gl.LINE, indices=[], data=[], bits=None, **kwds):
        ...
        if not self.loaded:
            ...
            if data_len == 0:
                ...
                self.buffer = np.zeros(data_len, dtype=[ (k, v.dtype, v.shape[(-1)]) for k, v in interleaved.items() ])
    ...
    def draw(self, mode=gl.TRIANGLES, fill=gl.LINE, indices=[], data=[], **kwds):
        ...
        for varindex, vardata in enumerate(self.inputs.items()):
            ...
            gl.bind_buffer(gl.ELEMENT_ARRAY_BUFFER, self.indices_id)
            gl.vertex_attrib_pointer(loc, self.buffer[varname].shape[(-1)], gl.GL_FLOAT, False, stride, offset)
