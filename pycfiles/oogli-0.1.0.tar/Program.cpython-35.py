...
class Program(object):
    ...
    def attach(self, shader):
        ...
        if isinstance(shader, VertexShader):
            ...
            if isinstance(shader, FragmentShader):
                ...
                if isinstance(shader, TessellationControlShader):
                    self.tc = shader
                else:
                    if isinstance(shader, TessellationEvaluationShader):
                        self.te = shader
    ...
    def build(self):
        shaders = [shader for shader in (self.vert, self.frag, self.tc, self.te, self.geo) if isinstance(shader, Shader)]
    ...
    def load(self, mode=gl.TRIANGLES, fill=gl.LINE, indices=[], data=[], **kwds):
        ...
        if not self.loaded:
            ...
            if data_len == 0:
                ...
                self.buffer = np.zeros(data_len, dtype=[(k, v.dtype, v.shape[(-1)]) for k, v in data.items()])
    ...
    def draw(self, mode=gl.TRIANGLES, fill=gl.LINE, indices=[], data=[], **kwds):
        ...
        for varindex, vardata in enumerate(self.bound_attributes.items()):
            ...
            gl.bind_buffer(gl.ELEMENT_ARRAY_BUFFER, self.indices_id)
            gl.vertex_attrib_pointer(varindex, self.buffer[varname].shape[(-1)], gl.GL_FLOAT, False, stride, offset)
