...
class Shader(object):
    ...
    def compile(self):
        ...
        shader_id = self.compiled or self.shader
    ...
    def parse(self, source):
        ...
        inputs2_pattern = '\\s*GLSL_TYPE\\s+((highp|mediump|lowp)\\s+)?(?P<vartype>\\w+)\\s+(?P<varname>\\w+)\\s*(\\[(?P<varsize>\\d+)\\])?(\\s*\\=\\s*(?P<vardefault>[0-9.]+))?\\s*;'
        inputs_pattern = '(?P<direction>(in|out|uniform))\\s+((highp|mediump|lowp)\\s+)?(?P<vartype>\\w+)\\s+(?P<varname>\\w+)\\s*(\\s*\\=\\s*(?P=vartype)?(?P<vardefault>(.+)))?\\;'
        version_eng = re.compile(version_pattern)
        self.version = major, minor = (3, 2)
        engines = [
         re.compile(inputs_pattern)] + [ re.compile(inputs2_pattern.replace('GLSL_TYPE', kind), flags=re.MULTILINE) for kind in ('uniform',
                                                                                                                                 'attribute',
                                                                                                                                 'varying',
                                                                                                                                 'const')
                                       ]
            ...
            for eng in engines:
                if eng.search(line):
                    ...
                    if direction == 'in':
                        ...
                        setattr(self, varname, vartype)
                        if default:
                            self.uniforms[varname] = (
                             'uniform', vartype, default)
                        else:
                            self.uniforms[varname] = (
                             'uniform', vartype)
                    break
