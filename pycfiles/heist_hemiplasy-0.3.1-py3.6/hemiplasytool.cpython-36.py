...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/heist/hemiplasytool.py
# Compiled at: 2020-04-19 12:04:38
# Size of source mod 2**32: 24503 bytes
import matplotlib.pyplot as plt, numpy as np, numpy.polynomial.polynomial as poly, logging as log, os, io, re, math, shlex
...
def summarize(results):
    ...
    return [
     c_disc_follow, c_conc_follow]


def add_branch_lengths(tree, const=1):
    """Adds dummy branch lengths to newick tree"""
    newTree = re.sub('(\\d+)', '\\1:1', tree)
    newTree = re.sub('(\\))', '\\1:1', newTree)
    newTree = newTree[:-2]
    return newTree
...
def write_output(summary, mutation_counts_c, mutation_counts_d, reduced, counts, speciesTree, admix, traits, min_mutations_required, filename, reps, conversions, oldTree, intercept, coef, newick_internals, coal_internals):
    ...
    for key, val in traits.items():
        if val == 1:
            ...
            tree = re.sub('\\b%s\\b' % str(key) + ':', str(key) + '*:', tree)
    ...
    for k in mutation_counts_keys:
        if k in mutation_counts_cc.keys():
            if k in mutation_counts_dd.keys():
                mutation_counts_comb[k] = mutation_counts_cc[k] + mutation_counts_dd[k]
            if k in mutation_counts_cc.keys() and k not in mutation_counts_dd.keys():
                mutation_counts_comb[k] = mutation_counts_cc[k]
            else:
                if k not in mutation_counts_cc.keys():
                    if k in mutation_counts_dd.keys():
    ...
    out1.write('With homoplasy only, ' + str(min_mutations_required) + ' mutations are required to explain this trait pattern (Fitch parsimony)' + '\n\n')
    for event in admix:
    ...
    try:
        out1.write('Combinations of hemiplasy and homoplasy (1 < # mutations < ' + str(min_mutations_required) + ') occur ' + str(mix) + ' time(s)\n\n')
        out2.write(str(mix) + '\n')
    except:
        out1.write('Combinations of hemiplasy and homoplasy (1 < # mutations < ' + str(min_mutations_required) + ') occur ' + str(0) + ' time(s)\n\n')
        out2.write(str(0) + '\n')

    out1.write('"True" homoplasy (>= ' + str(min_mutations_required) + ' mutations) occurs ' + str(true_homo) + ' time(s)\n\n')
...
def plot_mutations(mutation_counts_c, mutation_counts_d, filename):
    ...
    for k in mutation_counts_keys:
        if k in mutation_counts_cc.keys() and k in mutation_counts_dd.keys():
            ...
            if k not in mutation_counts_cc.keys() and k in mutation_counts_dd.keys():
                mutation_counts_comb[k] = mutation_counts_dd[k]
...
def subs2coal(newick_string):
    """
        ...
        units.
        """
    scfs = re.findall('\\)(.*?)\\:', newick_string)
    scfs = list(filter(None, scfs))
    coal_internals = []
    for i in range(len(scfs)):
        if float(scfs[i]) / 100 < 1 and float(scfs[i]) != 1:
            coal_estimate = -1 * (math.log(1.5) + math.log(1 - float(scfs[i]) / 100))
            if coal_estimate > 0:
    ...
    def branch_regression(newick_branches, coal_internals):
        """
                ...
                """
        for i in range(len(newick_branches)):
    ...
    newick_tips = []
    for i in range(len(tip_sections)):
    ...
    return (
     coal_newick_string, Tree(coal_newick_string, format=1), intercept, coef, n, c)
    ...
    for i, line in enumerate(f):
        if line.startswith('begin trees'):
            ...
            if l[0] == 'set derived taxon':
                ...
                if l[0] == 'set outgroup taxon':
                    outgroup = l[1]
                else:
                    if l[0].startswith('set introgression'):
                        l = line.replace('\n', '').split(' ')
                        sp1 = l[2].split('=')[1]
                        sp2 = l[3].split('=')[1]
                        ...
                        if l[0].startswith('set type coal'):
                            treeType = 'coal'

    return (
     tree, derived, admix, outgroup, treeType)
