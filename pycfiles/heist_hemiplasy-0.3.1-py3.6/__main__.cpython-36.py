...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/heist/__main__.py
...
Indiana University
"""
import argparse, time, sys, os, logging as log, subprocess
...
def newick2ms(*args):
    parser = argparse.ArgumentParser(description='Tool for converting                         a newick string to ms-style splits.                             Note that this only makes sense if the input                                tree is in coalescent units.')
...
def subs2coal(*args):
    parser = argparse.ArgumentParser(description='Tool for converting                         a newick string with branch lengths in subs/site                            to a neewick string with branch lengths in                                 coalescent units. Input requires gene                                      or site-concordancee factors as                                          branch labels')
    parser.add_argument('input',
      metavar='input',
      help='Input newick string file')
    args = parser.parse_args()
...
def heistMerge(*args):
    parser = argparse.ArgumentParser(description='Merge output files from multiple HeiST runs.             Useful for simulating large trees by running multiple batch jobs.')
    parser.add_argument('-d',
      help='Merge all files in a directory', action='store_true')
    ...
    print('Combinations of hemiplasy and homoplasy (1 < # mutations < ____) occur ' + str(data['#3']) + ' time(s)\n')
    print('"True" homoplasy (>= ______ mutations) occurs ' + str(data['#4']) + ' time(s)\n')
...
def main(*args):
    ...
    parser = argparse.ArgumentParser(description='Tool for characterising                         hemiplasy given traits mapped onto a species tree')
    ...
    parser.add_argument('-t',
      '--threads', metavar='', help='Number of threads for simulations', default=16)
    parser.add_argument('-p',
      '--mspath', metavar='', help='Path to ms (if not in user path)', default='ms')
    parser.add_argument('-g',
      '--seqgenpath', metavar='', help='Path to seq-gen (if not in user path)', default='seq-gen')
    ...
    if len(admix) != 0:
                ...
                for y in range(0, threads):
                    if y != threads - 1:
                        ...
                        for m, event in enumerate(admix):
                            ...
                            ms_call = hemiplasytool.splits_to_ms(splits, taxa, int(reps * float(event[3])), args.mspath, o, prefix, event)
                            m = hemiplasytool.call_programs(ms_call, '', 'trees.tmp', taxalist)
                            processes_ms.append(m)
        ...
        match_species_pattern, counts_by_tree = seqtools.readSeqs(prefix + '.seqs.tmp', len(taxalist), traits, len(splits), i, prefix, intro_start)
    ...
    hemiplasytool.write_output(summary, mutation_counts_c, mutation_counts_d, mutation_pat, counts_by_tree, str(treeSp), admix, traits, min_mutations_required, args.outputdir, reps, conversions, original_tree[0], intercept, coef, newick_internals, coal_internals)
