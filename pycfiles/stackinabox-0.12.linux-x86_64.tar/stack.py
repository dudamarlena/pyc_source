...
# [GCC 8.4.0]
# Embedded file name: /home/bmeyer/Devel/stackInABox/.tox/twine/lib/python2.7/site-packages/stackinabox/stack.py
# Compiled at: 2017-05-27 01:24:11
"""
Stack-In-A-Box: Stack Management
"""
import logging, re, threading, uuid, six
...
class StackInABox(object):
    ...
    def call_into(cls, method, request, uri, headers):
        ...
        return local_store.instance.call(method, request, uri, headers)
    ...
    def hold_onto(cls, name, obj):
        ...
        """
        logger.debug(('Holding on {0} of type {1} with id {2}').format(name, type(obj), id(obj)))
    ...
    def hold_out(cls, name):
        ...
        """
        logger.debug(('Retreiving {0} from hold').format(name))
        obj = local_store.instance.from_hold(name)
        logger.debug(('Retrieved {0} of type {1} with id {2} from hold').format(name, type(obj), id(obj)))
        return obj
    ...
    def get_services_url(url, base_url):
        ...
        result = url[length:]
        logger.debug(('{0} from {1} equals {2}').format(base_url, url, result))
        return result
    ...
    def base_url(self, value):
        ...
        logger.debug(('StackInABox({0}): Updating URL from {1} to {2}').format(self.__id, self.__base_url, value))
        self.__base_url = value
        for k, v in six.iteritems(self.services):
            matcher, service = v
            service.base_url = StackInABox.__get_service_url(value, service.name)
            logger.debug(('StackInABox({0}): Service {1} has url {2}').format(self.__id, service.name, service.base_url))

    def reset(self):
        """Reset StackInABox to a like-new state."""
        logger.debug(('StackInABox({0}): Resetting...').format(self.__id))
        for k, v in six.iteritems(self.services):
            matcher, service = v
            logger.debug(('StackInABox({0}): Resetting Service {1}').format(self.__id, service.name))
            service.reset()

        self.services = {}
        self.holds = {}
        logger.debug(('StackInABox({0}): Reset Complete').format(self.__id))
    ...
    def register(self, service):
        ...
        if service.name not in self.services.keys():
            logger.debug(('StackInABox({0}): Registering Service {1}').format(self.__id, service.name))
            regex = ('^/{0}/').format(service.name)
            self.services[service.name] = [
             re.compile(regex),
             service]
            service.base_url = StackInABox.__get_service_url(self.base_url, service.name)
            logger.debug(('StackInABox({0}): Service {1} has url {2}').format(self.__id, service.name, service.base_url))
        else:
            raise ServiceAlreadyRegisteredError(('Service {0} is already registered').format(service.name))
    ...
    def call(self, method, request, uri, headers):
        ...
        """
        logger.debug(('StackInABox({0}): Received call to {1} - {2}').format(self.__id, method, uri))
        service_uri = StackInABox.get_services_url(uri, self.base_url)
        for k, v in six.iteritems(self.services):
            matcher, service = v
            logger.debug(('StackInABox({0}): Checking if Service {1} handles...').format(self.__id, service.name))
            logger.debug(('StackInABox({0}): ...using regex pattern {1} against {2}').format(self.__id, matcher.pattern, service_uri))
            if matcher.match(service_uri):
                logger.debug(('StackInABox({0}): Trying Service {1} handler...').format(self.__id, service.name))
                try:
                    service_caller_uri = service_uri[len(service.name) + 1:]
                    return service.request(method, request, service_caller_uri, headers)
                except Exception as ex:
                    logger.exception(('StackInABox({0}): Service {1} - Internal Failure').format(self.__id, service.name))
                    return (596,
                     headers,
                     ('Service Handler had an error: {0}').format(ex))

        return (
         597, headers, ('Unknown service - {0}').format(service_uri))
    ...
    def from_hold(self, name):
        ...
        """
        logger.debug(('StackInABox({0}): Retreiving {1} from the hold').format(self.__id, name))
        obj = self.holds[name]
        logger.debug(('StackInABox({0}): Retrieved {1} of type {2} with id {3}').format(self.__id, name, type(obj), id(obj)))
        return obj
