...
# [GCC 8.4.0]
# Embedded file name: /home/bmeyer/Devel/stackInABox/.tox/twine/lib/python2.7/site-packages/stackinabox/tests/test_requests_mock_decorator.py
# Compiled at: 2017-11-01 23:25:05
"""
Stack-In-A-Box: Basic Test
"""
import collections, json, logging, types, unittest, requests, stackinabox.util.requests_mock.decorator as stack_decorator
...
class TestRequestsMockBasic(unittest.TestCase):
    ...
    @stack_decorator.stack_activate('localhost', HelloService(), 200, value='Hello')
    ...
    @stack_decorator.stack_activate('localhost', HelloService(), 200, value='Hello', access_services='stack')
    def test_basic_with_stack_acccess(self, response_code, value='alpha', stack=None):
        res = requests.get('http://localhost/hello/')
        self.assertEqual(res.status_code, response_code)
        self.assertEqual(res.text, value)
...
class TestRequestMockAdvanced(unittest.TestCase):
    ...
    @stack_decorator.stack_activate('localhost', AdvancedService(), session='session')
    def test_basic(self, session):
        ...
        res = session.get('http://localhost/advanced/g?bob=alice;alice=bob&joe=jane')
        self.assertEqual(res.status_code, 200)
...
class TestRequestsMockBasicWithDecoratorAndGenerator(unittest.TestCase):
    ...
    @stack_decorator.stack_activate('localhost', requests_mock_generator(), 200, value='Hello')
    def test_basic_with_parameters(self, response_code, value='alpha'):
        res = requests.get('http://localhost/hello/')
        self.assertEqual(res.status_code, response_code)
        self.assertEqual(res.text, value)

    @stack_decorator.stack_activate('localhost', requests_mock_generator(), 200, value='Hello', access_services='stack')
    def test_basic_with_stack_acccess(self, response_code, value='alpha', stack=None):
        res = requests.get('http://localhost/hello/')
        self.assertEqual(res.status_code, response_code)
        self.assertEqual(res.text, value)
...
class TestRequestsMockBasicWithDecoratorAndGenerator(unittest.TestCase):
    ...
    @stack_decorator.stack_activate('localhost', requests_mock_list(), 200, value='Hello')
    def test_basic_with_parameters(self, response_code, value='alpha'):
        res = requests.get('http://localhost/hello/')
        self.assertEqual(res.status_code, response_code)
        self.assertEqual(res.text, value)

    @stack_decorator.stack_activate('localhost', requests_mock_list(), 200, value='Hello', access_services='stack')
    def test_basic_with_stack_acccess(self, response_code, value='alpha', stack=None):
        res = requests.get('http://localhost/hello/')
        self.assertEqual(res.status_code, response_code)
        self.assertEqual(res.text, value)
