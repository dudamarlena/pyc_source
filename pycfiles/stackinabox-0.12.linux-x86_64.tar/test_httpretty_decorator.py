...
# [GCC 8.4.0]
# Embedded file name: /home/bmeyer/Devel/stackInABox/.tox/twine/lib/python2.7/site-packages/stackinabox/tests/test_httpretty_decorator.py
# Compiled at: 2017-11-01 23:25:05
"""
Stack-In-A-Box: Basic Test
"""
import collections, types, unittest, requests, stackinabox.util.httpretty.decorator as stack_decorator
...
class TestHttprettyBasicWithDecoratorAndGenerator(unittest.TestCase):
    ...
    @stack_decorator.stack_activate('localhost', httpretty_generator())
    def test_basic(self):
        res = requests.get('http://localhost/hello/')
        self.assertEqual(res.status_code, 200)
        self.assertEqual(res.text, 'Hello')

    @stack_decorator.stack_activate('localhost', httpretty_generator(), 200, value='Hello')
    def test_basic_with_parameters(self, response_code, value='alpha'):
        res = requests.get('http://localhost/hello/')
        self.assertEqual(res.status_code, response_code)
        self.assertEqual(res.text, value)

    @stack_decorator.stack_activate('localhost', httpretty_generator(), 200, value='Hello', access_services='stack')
    def test_basic_with_stack_acccess(self, response_code, value='alpha', stack=None):
        res = requests.get('http://localhost/hello/')
        self.assertEqual(res.status_code, response_code)
        self.assertEqual(res.text, value)
...
class TestHttprettyBasicWithDecoratorAndList(unittest.TestCase):
    ...
    @stack_decorator.stack_activate('localhost', httpretty_list())
    def test_basic(self):
        res = requests.get('http://localhost/hello/')
        self.assertEqual(res.status_code, 200)
        self.assertEqual(res.text, 'Hello')

    @stack_decorator.stack_activate('localhost', httpretty_list(), 200, value='Hello')
    def test_basic_with_parameters(self, response_code, value='alpha'):
        res = requests.get('http://localhost/hello/')
        self.assertEqual(res.status_code, response_code)
        self.assertEqual(res.text, value)

    @stack_decorator.stack_activate('localhost', httpretty_list(), 200, value='Hello', access_services='stack')
    def test_basic_with_stack_acccess(self, response_code, value='alpha', stack=None):
        res = requests.get('http://localhost/hello/')
        self.assertEqual(res.status_code, response_code)
        self.assertEqual(res.text, value)
