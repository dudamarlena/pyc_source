...
# [GCC 8.4.0]
# Embedded file name: /home/bmeyer/Devel/stackInABox/.tox/twine/lib/python2.7/site-packages/stackinabox/tests/test_service.py
# Compiled at: 2017-05-27 01:24:11
import re, unittest, httpretty, requests
...
class TestServiceRegex(unittest.TestCase):
    ...
    def test_instantiation(self):
        ...
        self.assertEqual(len(service.base_url), len(name) + 1)
    ...
    def test_stackinabox_service_regex(self):
        ...
        for case in case_regex:
            self.assertEqual(StackInABoxService.get_service_regex('base', case, False), case)

        for case in case_nonregex:
            case_regex = re.compile(('^{0}$').format(case))
            self.assertEqual(StackInABoxService.get_service_regex('base', case, False), case_regex)
...
class AnotherAdvancedService(StackInABoxService):
    ...
    def __init__(self):
        ...
        self.register(StackInABoxService.GET, '/', AnotherAdvancedService.first_handler)
...
class YetAnotherService(StackInABoxService):
    ...
    def __init__(self):
        ...
        self.register(StackInABoxService.GET, '^/french$', YetAnotherService.yaas_handler)
...
class TestServiceRouteRegistration(unittest.TestCase):
    ...
    def test_bad_registration(self):
        ...
        with self.assertRaises(RouteAlreadyRegisteredError):
            service.register(StackInABoxService.GET, '/', AnotherAdvancedService.second_handler)
