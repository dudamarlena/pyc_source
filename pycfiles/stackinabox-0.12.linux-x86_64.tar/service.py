...
# [GCC 8.4.0]
# Embedded file name: /home/bmeyer/Devel/stackInABox/.tox/twine/lib/python2.7/site-packages/stackinabox/services/service.py
# Compiled at: 2017-05-27 01:24:11
"""
Stack-In-A-Box: Service Interface
"""
import logging, re, uuid, six
...
class StackInABoxServiceRouter(object):
    ...
    def set_subservice(self, obj):
        ...
        """
        if self.obj is not None:
            raise RouteAlreadyRegisteredError(('Service Router ({0} - {1}): Route {2} already has a sub-service handler').format(id(self), self.service_name, self.uri))
        if len(self.methods):
            logger.debug(('WARNING: Service Router ({0} - {1}): Methods detected on Route {2}. Sub-Service {3} may be hidden.').format(id(self), self.service_name, self.uri, obj.name))
    ...
    def register_method(self, method, fn):
        ...
        if method not in self.methods.keys():
            logger.debug(('Service Router ({0} - {1}): Adding method {2} on route {3}').format(id(self), self.service_name, method, self.uri))
            self.methods[method] = fn
        else:
            raise RouteAlreadyRegisteredError(('Service Router ({0} - {1}): Method {2} already registered on Route {3}').format(id(self), self.service_name, method, self.uri))
    ...
    def __call__(self, method, request, uri, headers):
        ...
        if method in self.methods:
            logger.debug(('Service Router ({0} - {1}): Located Method {2} on Route {3}. Calling...').format(id(self), self.service_name, method, self.uri))
            return self.methods[method](self.parent_obj, request, uri, headers)
        else:
            if self.obj is not None:
                logger.debug(('Service Router ({0} - {1}): Located Subservice {2} on Route {3}. Calling...').format(id(self), self.service_name, self.obj.name, self.uri))
                return self.obj.sub_request(method, request, uri, headers)
            else:
                logger.debug(('Service Router ({0} - {1}): No Method handler for service').format(id(self), self.service_name))
                return (405,
                 headers,
                 ('{0} not supported. Supported Methods are {1}').format(method, self.methods))
...
class StackInABoxService(object):
    ...
    def validate_regex(regex, sub_service):
        ...
        if regex.pattern.endswith('$') is True and sub_service is True:
            logger.debug('StackInABoxService: Sub-Service RegEx Pattern must not end with $')
            raise InvalidRouteRegexError('Pattern must end with $')
    ...
    def get_service_regex(base_url, service_url, sub_service):
        ...
        if StackInABoxService.is_regex(service_url):
            logger.debug(('StackInABoxService: Received regex {0} for use...').format(service_url.pattern))
            StackInABoxService.validate_regex(service_url, sub_service)
            return service_url
        else:
            regex = ('^{0}{1}$').format('', service_url)
            logger.debug(('StackInABoxService: {0} + {1} -> {2}').format(base_url, service_url, regex))
    ...
    def base_url(self, value):
        ...
        """
        logger.debug(('StackInABoxService ({0}:{1}) Updating Base URL from {2} to {3}').format(self.__id, self.name, self.__base_url, value))
        self.__base_url = value
        for k, v in six.iteritems(self.routes):
            v['regex'] = StackInABoxService.get_service_regex(value, v['uri'], v['handlers'].is_subservice)
    ...
    def try_handle_route(self, route_uri, method, request, uri, headers):
        ...
        uri_path = route_uri
        if '?' in uri:
            logger.debug(('StackInABoxService ({0}:{1}): Found query string removing for match operation.').format(self.__id, self.name))
            uri_path, uri_qs = uri.split('?')
            logger.debug(('StackInABoxService ({0}:{1}): uri =  "{2}", query = "{3}"').format(self.__id, self.name, uri_path, uri_qs))
        for k, v in six.iteritems(self.routes):
            logger.debug(('StackInABoxService ({0}:{1}): Checking if route {2} handles...').format(self.__id, self.name, v['uri']))
            logger.debug(('StackInABoxService ({0}:{1}): ...using regex pattern {2} against {3}').format(self.__id, self.name, v['regex'].pattern, uri_path))
            if v['regex'].match(uri_path):
                logger.debug(('StackInABoxService ({0}:{1}): Checking if route {2} handles method {2}...').format(self.__id, self.name, v['uri'], method))
                return v['handlers'](method, request, uri, headers)
    ...
    def request(self, method, request, uri, headers):
        ...
        :returns: tuple - (int, dict, string) containing:
                          ...
                          string - http string response
        """
        logger.debug(('StackInABoxService ({0}:{1}): Request Received {2} - {3}').format(self.__id, self.name, method, uri))
    ...
    def sub_request(self, method, request, uri, headers):
        ...
        :returns: tuple - (int, dict, string) containing:
                          ...
                          string - http string response
        """
        logger.debug(('StackInABoxService ({0}:{1}): Sub-Request Received {2} - {3}').format(self.__id, self.name, method, uri))
    ...
    def create_route(self, uri, sub_service):
        ...
        if uri not in self.routes.keys():
            logger.debug(('Service ({0}): Creating routes').format(self.name))
            self.routes[uri] = {'regex': StackInABoxService.get_service_regex(self.base_url, uri, sub_service), 
               'uri': uri, 
               'handlers': StackInABoxServiceRouter(self.name, uri, None, self)}
        return
    ...
    def register(self, method, uri, call_back):
        ...
        self.routes[uri]['handlers'].register_method(method, call_back)
