...
# [GCC 8.4.0]
# Embedded file name: /Users/elaineangelino/tabular/tests/test_tabular.py
# Compiled at: 2013-01-26 17:14:28
"""
Run from tabular/

"""
import unittest, cPickle, os, shutil, types, numpy as np, tabular as tb, tabular.utils as utils, tabular.spreadsheet as spreadsheet
...
class TesterCore(unittest.TestCase):
    ...
    def setUp(self):
        self.D = tb.tabarray(array=np.random.rand(1050, 100))
        self.Root = 'big'
    ...
    def test_equals_copy(self):
        D = self.D.copy()
        self.assert_(eq(self.D, D) and self.D is not D)
...
class TestBasic(TesterCore):
    ...
    def setUp(self):
        self.D = tb.tabarray(array=[
         (2, 'a', 2, 'cc', 3.0), (2, 'b', 5, 'dcc', 2.0),
         (7, 'e', 2, 'cjc', 8.0), (2, 'e', 2, 'ccj', 3.0)], names=[
         'a', 'c', 'b', 'd', 'e'], formats='i4,|S1,i4,|S3,f8', coloring={'moo': ['a', 'b'], 'boo': ['a', 'd', 'e']})
        self.Root = 'basic'
    ...
    def test_aggregate_AggFunc(self):
        AggFunc = np.mean
        D1, s = self.D[['a', 'b', 'e']].aggregate(On=[
         'e'], AggFunc=AggFunc, returnsort=True)
    ...
    def test_aggregate1(self):
        AggFuncDict = {'d': (',').join}
        D1, s = self.D[['a', 'b', 'd']].aggregate(On=[
         'a'], AggFuncDict=AggFuncDict, returnsort=True)
    ...
    def test_aggregate2(self):
        ...
        D1, s = self.D[['a', 'c', 'b', 'd']].aggregate(On=[
         'a', 'b'], AggFuncDict=AggFuncDict, returnsort=True)
        ...
        D2 = tb.tabarray(columns=[[ x[0] for x in ab ], [ x[1] for x in ab ], c, d], names=[
         'a', 'b', 'c', 'd'], coloring=D1.coloring)
        self.assert_(eq(D1, D2))
...
class TestAddCols(unittest.TestCase):
    ...
    def setUp(self):
        ...
        Recs = [ (a, b, np.random.rand() * 100, np.random.randint(100000), np.random.rand(), 'Yes' if np.random.rand() < 0.5 else 'No') for a in V1 for b in V2
               ]
        self.Y = tb.tabarray(records=Recs, names=['Region', 'Sector', 'Amount',
         'Population', 'Importance', 'Modernized'])
        self.n1 = 'Importance'
        self.n2 = 'Modernized'
        oklist = [ o for o in self.Y.dtype.names if o not in [self.n1, self.n2]
                 ]
    ...
    def test_4(self):
        Y = self.X.addcols([self.Y[self.n1], self.Y[self.n2]], names=[
         self.n1, self.n2])
...
class TestJoin(unittest.TestCase):
    ...
    def test_join3(self):
        Recs1 = [
         ('North', 'Service', 80.81823782850684),
         ('North', 'Manufacturing', 67.06511482978966),
         ('North', 'Education', 31.043641435185123),
         ('North', 'Healthcare', 14.196823211749276),
         ('South', 'Service', 2.358379823491452)]
        Recs2 = [('North', 'Service', 33.0690224710869),
         ('North', 'Manufacturing', 63.155520758932305),
         ('North', 'Education', 70.80529023970098),
         ('North', 'Healthcare', 40.30123179857017),
        ...
        Z = spreadsheet.join([X1, X2], keycols=['Region', 'Sector'], Names=[
         'US', 'China'])
        Recs = [ (a, b, c, d) for (a, b, c), (x1, x2, d) in zip(Recs1, Recs2) ]
        X = tb.tabarray(records=Recs, names=[
         'Region', 'Sector', 'Amount_US', 'Amount_China'])
...
def test_bionumbers():
    ...
    names = ('ID', 'Property', 'Organism', 'Value', 'Units', 'Range', 'NumericalValue',
             'Version')
    assert_bio(X.dtype.names == names and len(X) == 4615 and eq(X, Y), fname)


class TestLoadSaveSV(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.X = tb.tabarray(records=Recs, names=[
         'Region', 'Sector', 'Amount', 'Population'], coloring={'zoo': ['Region', 'Sector'], 'york': [
                  'Population', 'Sector', 'Region']})
    ...
    def test_load_save_CSV_infer2(self):
        ...
        self.X.saveSV(fname, printmetadict=False, metadata=[
         'coloring', 'names'])
        X2 = tb.tabarray(SVfile=fname, metametadata={'coloring': 0, 'names': 1})
        self.assert_io(eq(self.X, X2), fname)
    ...
    def test_linefixer(self):
        ...
        X2 = tb.tabarray(SVfile=fname, linefixer=lambda x: x.replace('@', '\t'))
    ...
    def test_valuefixer(self):
        ...
        X1.saveSV(fname)
        X2 = tb.tabarray(SVfile=fname, valuefixer=lambda x: str(int(x) - 1) if x.isdigit() else x)
        print X1, X2
...
def TestExtract():
    D = tb.tabarray(array=np.random.rand(1050, 100))
...
def TestReplace():
    ...
    X2 = tb.tabarray(records=Recs2, names=[
     'Region', 'Sector', 'Amount', 'Population'])
...
def TestReplace2():
    ...
    X = tb.tabarray(records=Recs, names=[
     'Region', 'Sector', 'Amount', 'Population'])
    X2 = tb.tabarray(records=Recs, names=[
     'Region', 'Sector', 'Amount', 'Population'])
    X.replace('S', 'M')
    assert (X == X2).all()
...
def TestPivot1():
    ...
    Y = X.pivot('b', 'a')
    Z = tb.tabarray(records=[(0, 3, 3, 5, 1), (1, 3, 5, 6, 3)], names=[
     'b', 'x_c', 'y_c', 'x_d', 'y_d'])
    assert (Y == Z).all()


def TestPivot2():
    X = tb.tabarray(records=[('x', 1, 3, 6), ('y', 0, 3, 1), ('x', 0, 3, 5)], names=[
     'a', 'b', 'c', 'd'])
    Y = X.pivot('b', 'a')
    Z = tb.tabarray(records=[(0, 3, 3, 5, 1), (1, 3, 0, 6, 0)], names=[
     'b', 'x_c', 'y_c', 'x_d', 'y_d'])
    assert (Y == Z).all()
...
def TestPivot3():
    ...
    W = tb.tabarray(columns=[V1] + Cols, names=[
     'Region'] + [ b + '_Amount' for b in Z ])
    assert (W == Y).all()
...
def TestPivot4():
    ...
    V2.sort()
    Recs = [ (a, b, 100 * np.random.rand()) for a in V1 for b in V2 ]
    X = tb.tabarray(records=Recs[:-1], names=[
     'Region', 'Source', 'Amount'])
