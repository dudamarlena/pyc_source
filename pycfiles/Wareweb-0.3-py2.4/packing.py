...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/wareweb/packing.py
# Compiled at: 2006-10-22 17:17:11
import sys, inspect, xmlrpclib, traceback
try:
...
json = None
__all__ = [
 'unpack', 'unpack_xmlrpc', 'unpack_json']
...
def unpack_xmlrpc(func):
    ...
    def replacement_func(self):
        ...
        try:
            ...
            body = xmlrpclib.dumps(xmlrpclib.Fault(1, fault), encoding='utf-8')
        else:
            if not isinstance(result, tuple):
                result = (
                 result,)
            body = xmlrpclib.dumps(result, methodresponse=True, encoding='utf-8')
...
def make_rpc_exception(environ, exc_info):
    ...
    if rpc_exception not in (None, 'occurred', 'exception', 'traceback'):
        environ['wsgi.errors'].write("Bad 'rpc_exception' setting: %r\n" % rpc_exception)
        rpc_exception = None
...
def unpack_json(func):
    ...
    def replacement_func(self):
        ...
        try:
            ...
            response = {'result': None, 'error': body, 'id': id}
        else:
            response = {'result': result, 'error': None, 'id': id}
...
class FunctionArg(object):
    ...
    def __init__(self, name):
        ...
        while 1:
            if name.endswith('_int'):
                ...
                if self.name_parts:
                    raise TypeError('The _path extension must come last (use foo_int_path, not foo_path_int)')
                self.name_parts.append(name[-5:])
                name = name[:-5]
                self.from_path = True
            else:
    ...
    def coerce(self, value):
        try:
            ...
            raise HTTPBadRequest('Bad variable %r: %s' % (self.name, e))
...
class FunctionArgSpec(object):
    ...
    def __init__(self, func):
        (self.argnames, self.varargs, self.varkw, defaults) = inspect.getargspec(func)
        self.required_args = []
        self.required_path_args = []
        self.optional_path_args = []
        if self.argnames:
            if self.argnames[0] == 'self':
            ...
            for name in argnames:
                if argnames.count(name) > 1:
                    raise TypeError('Argument names must be unique. More than one found for %r' % name)
            ...
            self.required_args = defaults or funcargs
        else:
    ...
    def unpack_args(self, path_parts, fields):
        ...
        if len(self.required_path_args) > len(path_parts):
            raise HTTPBadRequest('Not enough parameters on the URL (expected %i more path segments)' % (len(self.required_path_args) - len(path_parts)))
        if not self.varargs and len(self.required_path_args) + len(self.optional_path_args) < len(path_parts):
            raise HTTPBadRequest('Too many parameters on the URL (expected %i less path segments)' % (len(path_parts) - len(self.required_path_args) - len(self.optional_path_args)))
