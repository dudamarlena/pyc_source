...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/wareweb/servlet_script.py
...
class ServletCommand(Command):
    ...
    max_args = 1
    default_verbosity = 1
    parser = Command.standard_parser(simulate=True, interactive=True, quiet=True)
    parser.add_option('--no-servlet', action='store_true', dest='no_servlet', help="Don't create a servlet; just template(s)")
    ...
    def command(self):
        ...
        vars = {'name': name, 'base_package': base_package}
        if not self.options.no_servlet:
            self.create_servlet(base_package, base, dir, name, vars)
        ...
        for blank in blanks:
            self.create_blank(blank, template_base, dir, name, vars)
    ...
    def create_servlet(self, base_package, base, dir, name, vars):
        ...
        if not os.path.exists(blank):
            blank = os.path.join(os.path.dirname(__file__), 'paster_templates', 'blank_servlet.py_tmpl')
        f = open(blank, 'r')
        content = f.read()
        f.close()
        if blank.endswith('_tmpl'):
            content = copydir.substitute_content(content, vars, filename=blank)
        dest = os.path.join(base, dir, '%s.py' % name)
        self.write_file(dest, content, source=blank)
    ...
    def web_dir(self):
        ...
        if not possible:
            raise BadCommand('No web package found (looked in %s)' % (', ').join(packages))
        if len(possible) > 1:
            raise BadCommand('Multiple web packages found (%s)' % possible)
        return possible[0]