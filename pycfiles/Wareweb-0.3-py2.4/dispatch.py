...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/wareweb/dispatch.py
...
class MethodDispatch(object):
    ...
    def __addtoclass__(self, attr, cls):
        if self.__class__ is MethodDispatch:
            raise NotImplementedError('MethodDispatch is an abstract class, and cannot be used directly (use one of its subclasses)')
    ...
    def get_method(self, servlet, action):
        if self.prefix:
            try:
                return (
                 self.prefix + action, getattr(servlet, self.prefix + action))
            except AttributeError:
                pass

        try:
            return (
             action, getattr(servlet, action))
        except AttributeError:
            pass
...
class ActionDispatch(MethodDispatch):
    ...
    def find_method(self, servlet, ret_value, **kw):
        ...
        if len(possible_actions) > 1:
            raise httpexceptions.HTTPBadRequest('More than one action received: %s' % (', ').join(map(repr, possible_actions)))
        action = possible_actions[0]
        (name, method) = self.get_method(servlet, action)
        if name is None:
            raise httpexceptions.HTTPForbidden('Action method not found: %r' % action)
        if not self.valid_method(name, method):
            raise httpexceptions.HTTPForbidden('Method not allowed: %r' % action)
        return method()
...
class PathDispatch(MethodDispatch):
    ...
    def find_method(self, servlet, ret_value, **kw):
        ...
        (name, method) = self.get_method(servlet, action)
        if name is None:
            raise httpexceptions.HTTPForbidden('Method not found: %r' % action)
        if not self.valid_method(name, method):
            raise httpexceptions.HTTPForbidden('Method not allowed: %r' % action)
        return method()