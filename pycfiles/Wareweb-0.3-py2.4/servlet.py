...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/wareweb/servlet.py
...
import cgifields, event, cookiewriter
__all__ = [
 'Servlet']
...
class Servlet(object):
    ...
    def __call__(self, environ, start_response):
        try:
            (status, headers, app_iter) = self._process(environ, start_response)
    ...
    def _process(self, environ, start_response):
        ...
        self.config = self.environ.get('paste.config', {})
        if self.config.get('app_name'):
            self.app_name = self.config['app_name']
        self._cached_output = []
        self.headers_out = {'content-type': 'text/html; charset=%s' % self.encoding}
        self.status = '200 OK'
        self._cookies_out = {}
        self.request_method = environ['REQUEST_METHOD'].upper()
        self.app_url = self.environ.get('%s.base_url' % self.app_name, '')
        self.app_static_url = self.config.get('static_url', self.app_url + '/static')
        self.path_info = self.environ.get('PATH_INFO', '')
        if self.path_info:
            self.path_parts = filter(None, self.path_info[1:].split('/'))
        else:
            self.path_parts = []
        self.fields = cgifields.Fields(wsgilib.parse_formvars(environ))
        self.cookies = {}
        ...
        return (
         self.status, headers, self._cached_output)
    ...
    def set_cookie(self, cookie_name, value, path='/', expires='ONCLOSE', secure=False):
        c = cookiewriter.Cookie(cookie_name, value, path=path, expires=expires, secure=secure)
    ...
    def write(self, *obj):
        for v in obj:
            if v is None:
                ...
                self._cached_output.append(v.encode(self.encoding))
            else:
                self._cached_output.append(unicode(v).encode(self.encoding))
    ...
    def redirect(self, url, **query_vars):
        ...
        raise httpexceptions.get_exception(status)('This resource has been redirected', headers=[('Location', url)])
