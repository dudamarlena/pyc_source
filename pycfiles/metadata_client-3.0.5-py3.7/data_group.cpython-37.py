...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/metadata_client/modules/data_group.py
# Compiled at: 2017-06-19 07:48:53
# Size of source mod 2**32: 5354 bytes
...
class DataGroup(DataGroupApi):

    def __init__(self, metadata_client, name, language, data_group_type_id, experiment_id, user_id, prefix_path, flg_available, flg_writing, flg_public, format, data_passport, removed_at, description='', runs_data_groups_attributes=None):
    ...
    def create(self):
        ...
        response = mdc_client.create_data_group_api(self._DataGroup__get_resource())
    ...
    def update(self):
        mdc_client = self.metadata_client
        response = mdc_client.update_data_group_api(self.id, self._DataGroup__get_resource())
    ...
    def create_from_dict(mdc_client, data_grp):
        new_data_group = DataGroup(metadata_client=mdc_client,
          name=(data_grp['name']),
          ...
          runs_data_groups_attributes=(data_grp['runs_data_groups_attributes']))
    ...
    def get_all_by_experiment_id(mdc_client, experiment_id):
        response = mdc_client.get_all_data_groups_by_experiment_id_api(experiment_id)
        Base.cal_debug(MODULE_NAME, 'get_all_by_experiment_id', response)
        return Base.format_response(response, GET, OK, MODULE_NAME)

    def __get_resource(self):
        data_group = {MODULE_NAME: {'name':self.name, 
