...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/metadata_client/modules/proposal.py
...
class Proposal(ProposalApi):

    def __init__(self, metadata_client, number, title, abstract, url, instrument_id, instrument_cycle_id, principal_investigator_id, main_proposer_id, begin_at, end_at, release_at, flg_beamtime_status, beamtime_start_at, beamtime_end_at, flg_proposal_system, flg_available, description=''):
    ...
    def create(self):
        ...
        response = mdc_client.create_proposal_api(self._Proposal__get_resource())
    ...
    def update(self):
        mdc_client = self.metadata_client
        response = mdc_client.update_proposal_api(self.id, self._Proposal__get_resource())
    ...
    def create_from_dict(mdc_client, proposal):
        new_proposal = Proposal(metadata_client=mdc_client,
    ...
    def get_all_by_number(mdc_client, number):
        ...
        Base.cal_debug(MODULE_NAME, 'get_all_by_number', response)
        return Base.format_response(response, GET, OK, MODULE_NAME)

    @staticmethod
    def get_by_number(mdc_client, number):
        response = mdc_client.get_proposal_by_number_api(number)
        Base.cal_debug(MODULE_NAME, 'get_proposal_by_number', response)
        return Base.format_response(response, GET, OK, MODULE_NAME)

    @staticmethod
    def get_runs_by_proposal_number(mdc_client, proposal_number, run_number=None):
        response = mdc_client.get_runs_by_proposal_number_api(proposal_number, run_number)
        return Base.format_response(response, GET, OK, MODULE_NAME)
    ...
    def get_active_proposal_by_instrument(mdc_client, instrument_id):
        ...
        Base.cal_debug(MODULE_NAME, 'get_active_proposal_by_instrument', resp)
        return Base.format_response(resp, GET, OK, MODULE_NAME)

    def __get_resource(self):
        proposal = {MODULE_NAME: {'number':self.number, 
                       'title':self.title, 
