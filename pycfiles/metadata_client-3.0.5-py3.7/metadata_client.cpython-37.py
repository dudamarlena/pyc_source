...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/metadata_client/metadata_client.py
# Compiled at: 2019-11-14 11:36:45
# Size of source mod 2**32: 34125 bytes
...
from modules.data_source_group import DataSourceGroup
from modules.data_source_group_version import DataSourceGroupVersion
from modules.data_type import DataType
...
class MetadataClient(ExperimentType, DataGroupType, DataGroupRepository, ParameterType, DataType, Parameter, Instrument, Proposal, User, Sample, Experiment, Repository, Run, DataGroup, DataFile, DataSourceGroup, DataSourceGroupVersion):
    ...
    @staticmethod
    def get_all_xfel_instruments(mdc_client):
        resp = Instrument.get_all_from_xfel(mdc_client)
        if not resp['success']:
    ...
    @staticmethod
    def get_all_data_group_types(mdc_client):
        resp = DataGroupType.get_all(mdc_client)
        if not resp['success']:
            error_msg = '{0} >> {1}'.format(resp['info'], resp['app_info'])
            logging.error(error_msg)
        return resp
    ...
    def get_active_proposal_by_instrument(mdc_client, instrument_id):
        resp = Proposal.get_active_proposal_by_instrument(mdc_client, instrument_id)
        if not resp['success']:
            error_msg = '{0} >> {1}'.format(resp['info'], resp['app_info'])
            logging.error(error_msg)
        return resp
    ...
    @staticmethod
    def get_proposal_info(mdc_client, proposal_number):
        logging.debug('proposal_number: {0}'.format(proposal_number))
        resp = Proposal.get_by_number(mdc_client, proposal_number)
        proposal_id = resp['data']['id']
        logging.debug('proposal_id: {0}'.format(proposal_id))
        return resp

    @staticmethod
    def get_repository_by_id(mdc_client, repository_id):
        logging.debug('repository_id: {0}'.format(repository_id))
        resp = Repository.get_by_id(mdc_client, repository_id)
        return resp

    @staticmethod
    def get_proposal_runs(mdc_client, proposal_number, run_number=None):
        logging.debug('proposal_number: {0}'.format(proposal_number))
        resp = Proposal.get_runs_by_proposal_number(mdc_client, proposal_number, run_number)
        number_of_runs = len(resp['data']['runs']) if 'runs' in resp['data'] else 0
        logging.debug('number of runs: {0}'.format(number_of_runs))
        return resp

    @staticmethod
    def get_proposal_experiments(mdc_client, proposal_number):
        logging.debug('proposal_number: {0}'.format(proposal_number))
        resp = Proposal.get_by_number(mdc_client, proposal_number)
        ...
        proposal_id = resp['data']['id']
        logging.debug('proposal_id: {0}'.format(proposal_id))
        resp = Experiment.get_all_by_proposal_id(mdc_client, proposal_id)
        return resp

    @staticmethod
    def get_proposal_samples(mdc_client, proposal_number):
        logging.debug('proposal_number: {0}'.format(proposal_number))
        resp = Proposal.get_by_number(mdc_client, proposal_number)
        if not (resp['success'] and resp['data']):
            error_msg = '{0} >> {1}'.format(resp['info'], resp['app_info'])
        ...
        proposal_id = resp['data']['id']
        logging.debug('proposal_id: {0}'.format(proposal_id))
        resp = Sample.get_all_by_proposal_id(mdc_client, proposal_id)
        return resp

    @staticmethod
    def register_run(mdc_client, experiment_id, sample_id, run_dict, data_grp_dict):
        ...
        dgr_mandatory_fields = [
         'data_group_type_id', 'creator_id',
         'prefix_path']
        ...
        if len(missing_mandatory) > 0:
            info_msg = 'The following fields are mandatory: {0}'.format(missing_mandatory)
            return {'info':info_msg, 
             'success':False, 
             'data':{},  'app_info':{}}
        resp = MetadataClient._MetadataClient__create_run_from_dict(mdc_client, experiment_id, sample_id, run_dict)
        if not resp['success']:
            error_msg = '{0} >> {1}'.format(resp['info'], resp['app_info'])
            logging.error(error_msg)
            return resp
        ...
        resp = MetadataClient._MetadataClient__create_data_group_from_dict(mdc_client, experiment_id, run_id, data_grp_dict)
        if not resp['success']:
            error_msg = '{0} >> {1}'.format(resp['info'], resp['app_info'])
            logging.error(error_msg)
            error_msg = 'Deleting Run with ID: "{0}"'.format(run_id)
            logging.error(error_msg)
            Run.delete_by_id(mdc_client, run_id)
            return resp
    ...
    @staticmethod
    def register_run_data(mdc_client, experiment_id, sample_id, run_id, data_group_id, data_group_dict, data_group_files_ar):
        logging.debug('experiment_id: {0}'.format(experiment_id))
        logging.debug('sample_id: {0}'.format(sample_id))
        logging.debug('run_id: {0}'.format(run_id))
        logging.debug('data_group_id: {0}'.format(data_group_id))
        ...
        if not data_group_dict is None:
            if not data_group_dict == {}:
                dgr_mandatory_fields = [
                 'data_group_type_id',
                 'creator_id']
                for field in dgr_mandatory_fields:
        ...
        elif data_group_files_ar is None or data_group_files_ar == []:
            missing_mandatory.append('data_group_files')
        else:
            dgr_files_mandatory_fields = [
        ...
        if len(missing_mandatory) > 0:
            ...
            return {'info':info_msg, 
             'success':False, 
             'data':{},  'app_info':{}}
        resp = data_group_dict is None or data_group_dict == {} or DataGroup.update_from_dict(mdc_client, data_group_id, data_group_dict)
        if not resp['success']:
            error_msg = '{0} >> {1}'.format(resp['info'], resp['app_info'])
            logging.error(error_msg)
            return resp
        resp = MetadataClient._MetadataClient__create_data_file_from_dict(mdc_client, data_group_id, data_group_files_ar)
        if not resp['success']:
            error_msg = '{0} >> {1}'.format(resp['info'], resp['app_info'])
            logging.error(error_msg)
            return resp
    ...
    @staticmethod
    def close_run(mdc_client, experiment_id, sample_id, run_id, data_group_id, run_dict, dg_params_dict):
        logging.debug('experiment_id: {0}'.format(experiment_id))
        logging.debug('sample_id: {0}'.format(sample_id))
        logging.debug('run_id: {0}'.format(run_id))
        logging.debug('data_group_id: {0}'.format(data_group_id))
        ...
        if not run_dict is None:
            if not run_dict == {}:
                run_mandatory_fields = [
                 'end_at', 'last_train']
                for field in run_mandatory_fields:
                    if field not in run_dict:
            ...
            if dg_params_dict is None or dg_params_dict == {}:
                ...
                if str(type(dg_parameters_ar)) != "<class 'list'>":
                    ...
                    if 'flg_status' in run_dict:
                        run_dict['flg_status'] = Util.get_opt_dict_val(run_dict, 'flg_status', 2)
                else:
                    run_dict['flg_status'] = 2
                if 'flg_available' in run_dict:
                    run_dict['flg_available'] = Util.get_opt_dict_val(run_dict, 'flg_available', 'true')
                resp = run_dict is None or run_dict == {} or Run.update_from_dict(mdc_client, run_id, run_dict)
                if not resp['success']:
                    error_msg = '{0} >> {1}'.format(resp['info'], resp['app_info'])
                    logging.error(error_msg)
                    return resp
                resp = MetadataClient._MetadataClient__create_parameters_from_dict(mdc_client, dg_params_dict)
                if not resp['success']:
                    error_msg = '{0} >> {1}'.format(resp['info'], resp['app_info'])
                    logging.error(error_msg)
                    return resp
    ...
    @staticmethod
    def register_run_data_and_results(mdc_client, experiment_id, sample_id, run_dict, data_group_dict, data_group_files_ar, dg_params_dict):
        logging.debug('experiment_id: {0}'.format(experiment_id))
        logging.debug('sample_id: {0}'.format(sample_id))
        logging.debug('run_dict: {0}'.format(run_dict))
        ...
        result_1 = MetadataClient.register_run(mdc_client, experiment_id, sample_id, run_dict, data_group_dict)
        if result_1['success']:
            ...
            if data_group_files_ar is None or data_group_files_ar == []:
                ...
                result_2 = MetadataClient.register_run_data(mdc_client, experiment_id, sample_id, run_id, data_group_id, data_group_dict, data_group_files_ar)
                if result_2['success']:
                    data_file_id = result_2['data']['data_file_id']
                else:
                    logging.error(result_2)
                    return result_2
            result_3 = MetadataClient.close_run(mdc_client, experiment_id, sample_id, run_id, data_group_id, run_dict, dg_params_dict)
            if result_3['success']:
                pass
            else:
    ...
    @staticmethod
    def register_run_replica(mdc_client, proposal_number, run_number, repository_identifier):
        replica_registration = True
        resp = MetadataClient._MetadataClient__manage_run_replica(mdc_client, proposal_number, run_number, repository_identifier, replica_registration)
        return resp

    @staticmethod
    def get_run_register_repositories(mdc_client, proposal_number, run_number, repository_identifier):
        replica_registration = False
        resp = MetadataClient._MetadataClient__manage_run_replica(mdc_client, proposal_number, run_number, repository_identifier, replica_registration)
        return resp

    @staticmethod
    def unregister_run_replica(mdc_client, proposal_number, run_number, repository_identifier):
        replica_registration = False
        resp = MetadataClient._MetadataClient__manage_run_replica(mdc_client, proposal_number, run_number, repository_identifier, replica_registration)
        return resp
    ...
    def search_data_files(mdc_client, run_number, proposal_number):
        resp = DataFile.search_data_files(mdc_client, run_number, proposal_number)
        if not resp['success']:
            error_msg = '{0} >> {1}'.format(resp['info'], resp['app_info'])
            logging.error(error_msg)
            return resp
        return {'info':'Search data files return successfully',  'success':True, 
         'data':resp['data'], 
         'app_info':{}}

    @staticmethod
    def get_data_source_group(mdc_client, data_source_group_name):
        resp = DataSourceGroup.get_by_name(mdc_client, data_source_group_name)
        if not resp['success']:
            error_msg = '{0} >> {1}'.format(resp['info'], resp['app_info'])
            logging.error(error_msg)
        return resp
    ...
    @staticmethod
    def get_data_source_group_version(mdc_client, data_source_group_id, version_name):
        resp = DataSourceGroupVersion.get(mdc_client, data_source_group_id, version_name)
        if not resp['success']:
            error_msg = '{0} >> {1}'.format(resp['info'], resp['app_info'])
            logging.error(error_msg)
        return resp
    ...
    @staticmethod
    def update_data_source_group_version(mdc_client, id, params):
        resp = DataSourceGroupVersion.update(mdc_client, id, params)
        if not resp['success']:
            error_msg = '{0} >> {1}'.format(resp['info'], resp['app_info'])
            logging.error(error_msg)
        return resp
    ...
    def get_data_source_group_version_by_name(self, group_name, version_name):
        resp = self.get_data_source_group(self, group_name)
        if not resp['success'] or len(resp['data']) == 0:
            return
        dsg = resp['data'][0]
        resp = self.get_data_source_group_version(self, dsg['id'], version_name)
        if not resp['success'] or len(resp['data']) == 0:
            return
        return resp['data'][0]

    def deploy_data_source_group_version(self, group_name, version_name):
        dsgv = self.get_data_source_group_version_by_name(group_name, version_name)
        if not dsgv:
            return {'info':'DSG version not found', 
             'success':False, 
             'data':{},  'app_info':{}}
        if dsgv['flg_status'] == DataSourceGroupVersion.FLG_STATUS_DEPLOYED:
            return {'info':'DSG version already marked as deployed. Nothing to do.', 
             'success':True, 
             'data':{},  'app_info':{}}
        resp = self.update_data_source_group_version(self, dsgv['id'], {'last_deployed_at':None, 
         'flg_status':DataSourceGroupVersion.FLG_STATUS_DEPLOYED})
        if not resp['success']:
            error_msg = '{0} >> {1}'.format(resp['info'], resp['app_info'])
            logging.error(error_msg)
            return resp
        return {'info':'DSG version deployed successfully',  'success':True, 
         'data':{},  'app_info':{}}

    @staticmethod
    def __create_run_from_dict(mdc_client, experiment_id, sample_id, run_dict):
        logging.debug('experiment_id: {0}'.format(experiment_id))
        logging.debug('sample_id: {0}'.format(sample_id))
        logging.debug('run: {0}'.format(run_dict))
        run_experiment_id = str(experiment_id)
        run_sample_id = Util.get_opt_val(sample_id)
        run_run_number = run_dict['run_number']
        run_run_alias = Util.get_opt_dict_val(run_dict, 'run_alias')
        run_begin_at = run_dict['begin_at']
        ...
        run_flg_avail = Util.get_opt_dict_val(run_dict, 'flg_available', 'true')
        ...
        mdc_run = {'run_number':run_run_number, 
        ...
        return Run.create_from_dict(mdc_client, mdc_run)

    @staticmethod
    def __create_data_group_from_dict(mdc_client, experiment_id, run_id, dgrp_dict):
        logging.debug('experiment_id: {0}'.format(experiment_id))
        logging.debug('run_id: {0}'.format(run_id))
        ...
        dgr_type_id = dgrp_dict['data_group_type_id']
        dgr_exp_id = experiment_id
        dgr_creator_id = dgrp_dict['creator_id']
        dgr_prefix_path = dgrp_dict['prefix_path']
        dgr_flg_avail = Util.get_opt_dict_val(dgrp_dict, 'flg_available', 'true')
        dgr_flg_writing = Util.get_opt_dict_val(dgrp_dict, 'flg_writing', 'false')
        ...
        return DataGroup.create_from_dict(mdc_client, mdc_data_group)

    @staticmethod
    def __create_data_file_from_dict(mdc_client, data_group_id, files_list):
        logging.debug('data_group_id: {0}'.format(data_group_id))
        logging.debug('data_group_files: {0}'.format(files_list))
        data_file_group_id = data_group_id
        data_file_files = str(files_list).replace(' ', '')
        mdc_data_file = {'data_group_id':data_file_group_id, 
         'files':data_file_files}
        logging.debug('Built DataFile: {0}'.format(mdc_data_file))
        return DataFile.create_from_dict(mdc_client, mdc_data_file)

    @staticmethod
    def __create_parameters_from_dict(mdc_client, parameters_list):
        logging.debug('parameters: {0}'.format(parameters_list))
        return Parameter.create_multiple_from_dict(mdc_client, parameters_list)

    @staticmethod
    def __set_data_group_repo_from_dict(mdc_client, data_group_id, repository_id, flg_available):
        logging.debug('data_group_id: {0}'.format(data_group_id))
        logging.debug('repository_id: {0}'.format(repository_id))
        logging.debug('flg_available: {0}'.format(flg_available))
        data_group_repository = {'data_group_id':data_group_id, 
         'repository_id':repository_id, 
         'flg_available':flg_available}
        msg = 'Built DataGroupRepository: {0}'.format(data_group_repository)
        logging.debug(msg)
        return DataGroupRepository.set_from_dict(mdc_client, data_group_repository)

    @staticmethod
    def __manage_run_replica(mdc_client, proposal_number, run_number, repository_identifier, replica_registration):
        ...
        resp = Repository.get_all_by_identifier(mdc_client, repository_identifier)
        if not resp['success']:
            error_msg = '{0} >> {1}'.format(resp['info'], resp['app_info'])
            logging.error(error_msg)
            return resp
        repository_id = resp['data'][0]['id']
        resp = Run.get_all_raw_data_groups(mdc_client, run_number, proposal_number)
        if not resp['success']:
            error_msg = '{0} >> {1}'.format(resp['info'], resp['app_info'])
            logging.error(error_msg)
            return resp
        ...
        for data_group_id in data_group_ids:
            resp = MetadataClient._MetadataClient__set_data_group_repo_from_dict(mdc_client, data_group_id, repository_id, flg_available)
            if not resp['success']:
                error_msg = '{0} >> {1}'.format(resp['info'], resp['app_info'])
                logging.error(error_msg)
                return resp
