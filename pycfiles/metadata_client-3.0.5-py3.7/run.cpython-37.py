...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/metadata_client/modules/run.py
# Compiled at: 2019-08-14 10:42:37
# Size of source mod 2**32: 6378 bytes
"""Run module class"""
...
class Run(RunApi):
    ...
    def __init__(self, metadata_client, run_number, run_alias, experiment_id, sample_id, begin_at, end_at, first_train, last_train, flg_available, flg_status, original_format, system_msg, description=''):
    ...
    def create(self):
        ...
        response = mdc_client.create_run_api(self._Run__get_resource())
    ...
    def update(self):
        mdc_client = self.metadata_client
        response = mdc_client.update_run_api(self.id, self._Run__get_resource())
    ...
    def create_from_dict(mdc_client, run):
        new_run = Run(metadata_client=mdc_client,
    ...
    @staticmethod
    def get_all_by_run_number_and_experiment_id(mdc_client, run_number, experiment_id):
        response = mdc_client.get_run_by_run_number_and_experiment_id_api(run_number, experiment_id)
        Base.cal_debug(MODULE_NAME, 'get_all_by_run_number', response)
        return Base.format_response(response, GET, OK, MODULE_NAME)

    @staticmethod
    def get_by_run_number_and_experiment_id(mdc_client, run_number, experiment_id):
        res = Run.get_all_by_run_number_and_experiment_id(mdc_client, run_number, experiment_id)
        if res['success']:
            res = Base.unique_key_format_result(res=res, module_name=MODULE_NAME)
        return res

    @staticmethod
    def get_all_by_run_number_and_proposal_number(mdc_client, run_number, proposal_number):
        response = mdc_client.get_all_runs_by_run_number_api(run_number)
        Base.cal_debug(MODULE_NAME, 'get_all_by_run_number', response)
        return Base.format_response(response, GET, OK, MODULE_NAME)

    @staticmethod
    def get_by_run_number_and_proposal_number(mdc_client, run_number, proposal_number):
        res = Run.get_all_by_run_number_and_proposal_number(mdc_client, run_number, proposal_number)
        if res['success']:
            res = Base.unique_key_format_result(res=res, module_name=MODULE_NAME)
        return res
    ...
    def get_all_raw_data_groups(mdc_client, run_number, proposal_number):
        response = mdc_client.get_run_all_raw_data_groups_ids_api(run_number, proposal_number)
        Base.cal_debug(MODULE_NAME, 'get_all_raw_data_groups', response)
        return Base.format_response(response, GET, OK, MODULE_NAME)

    def __get_resource(self):
        run = {MODULE_NAME: {'run_number':self.run_number, 
