...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/metadata_client/common/base.py
...
class Base:
    ...
    def format_response(response, action, success_code, module_name):
        ...
        if response.status_code == success_code and r_content != {}:
            ...
            if response.status_code == NO_CONTENT and r_content == {}:
                res = Base.response_success(module_name, action, r_content)
            else:
    ...
    def unique_key_format_result(res, module_name, unique_id=None):
        if len(res['data']) > 0:
            ...
            data = {}
            if unique_id:
                info = '{0} "{1}" could not be found!'.format(module_name, unique_id)
            else:
