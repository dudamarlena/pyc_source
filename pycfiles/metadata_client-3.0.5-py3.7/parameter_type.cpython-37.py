...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/metadata_client/modules/parameter_type.py
# Compiled at: 2017-06-19 07:48:53
# Size of source mod 2**32: 2748 bytes
"""ParameterType module class"""
from apis.parameter_type_api import ParameterTypeApi
from common.base import Base
from ..common.config import *
MODULE_NAME = PARAMETER_TYPE

class ParameterType(ParameterTypeApi):
    ...
    def create(self):
        ...
        response = mdc_client.create_parameter_type_api(self._ParameterType__get_resource())
    ...
    def delete(self):
        mdc_client = self.metadata_client
        response = mdc_client.delete_parameter_type_api(self.id)
        Base.cal_debug(MODULE_NAME, DELETE, response)
        return Base.format_response(response, DELETE, NO_CONTENT, MODULE_NAME)

    def update(self):
        mdc_client = self.metadata_client
        response = mdc_client.update_parameter_type_api(self.id, self._ParameterType__get_resource())
        Base.cal_debug(MODULE_NAME, UPDATE, response)
        return Base.format_response(response, UPDATE, OK, MODULE_NAME)

    @staticmethod
    def get_by_id(mdc_client, parameter_type_id):
        response = mdc_client.get_parameter_type_by_id_api(parameter_type_id)
    ...
    def get_all_by_name(mdc_client, name):
        response = mdc_client.get_all_parameter_types_by_name_api(name)
        Base.cal_debug(MODULE_NAME, 'get_all_by_name', response)
        return Base.format_response(response, GET, OK, MODULE_NAME)

    @staticmethod
    def get_by_name(mdc_client, name):
        res = ParameterType.get_all_by_name(mdc_client, name)
        if res['success']:
    ...
    def __get_resource(self):
        parameter_type = {MODULE_NAME: {'name':self.name, 
