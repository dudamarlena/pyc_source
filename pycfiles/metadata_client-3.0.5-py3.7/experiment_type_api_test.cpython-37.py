...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/metadata_client/tests/apis/experiment_type_api_test.py
# Compiled at: 2017-06-19 07:48:53
# Size of source mod 2**32: 5504 bytes
"""ExperimentTypeApiTest class"""
import unittest
...
class ExperimentTypeApiTest(ApiBase, unittest.TestCase):
    ...
    def test_create_experiment_type_api(self):
        _ExperimentTypeApiTest__unique_name = Generators.generate_unique_name('ExperimentTypeApi')
        _ExperimentTypeApiTest__unique_identifier = Generators.generate_unique_identifier()
        experiment_type = {'experiment_type': {'name':_ExperimentTypeApiTest__unique_name, 
                             'identifier':_ExperimentTypeApiTest__unique_identifier, 
                             'flg_available':'true', 
                             'description':'desc 01'}}
        expect = experiment_type['experiment_type']
        received = self._ExperimentTypeApiTest__create_entry_api(experiment_type, expect)
        experiment_type_id = received['id']
        experiment_type_name = received['name']
        self._ExperimentTypeApiTest__create_error_entry_uk_api(experiment_type)
        self._ExperimentTypeApiTest__get_all_entries_by_name_api(experiment_type_name, expect)
        self._ExperimentTypeApiTest__get_entry_by_id_api(experiment_type_id, expect)
        self._ExperimentTypeApiTest__update_entry_api(experiment_type_id, expect)
        self._ExperimentTypeApiTest__delete_entry_by_id_api(experiment_type_id)
    ...
    def __create_entry_api(self, entry_info, expect):
        response = self.client_api.create_experiment_type_api(entry_info)
    ...
    def __update_entry_api(self, entry_id, expect):
        ...
        experiment_type_upd = {'experiment_type': {'name':unique_name_upd, 
                             'identifier':unique_id_upd, 
                             'flg_available':'false', 
                             'description':'desc 01 updated!!!'}}
        response = self.client_api.update_experiment_type_api(entry_id, experiment_type_upd)
        resp_content = self.load_response_content(response)
        receive = resp_content
        expect_upd = experiment_type_upd['experiment_type']
