...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/metadata_client/tests/apis/data_type_api_test.py
# Compiled at: 2017-06-19 07:48:53
# Size of source mod 2**32: 5342 bytes
"""DataTypeApiTest class"""
import unittest
...
class DataTypeApiTest(ApiBase, unittest.TestCase):
    ...
    def test_create_data_type_api(self):
        _DataTypeApiTest__unique_name = Generators.generate_unique_name('DataTypeApi')
        _DataTypeApiTest__unique_identifier = Generators.generate_unique_identifier()
        data_type = {'data_type': {'name':_DataTypeApiTest__unique_name, 
                       'identifier':_DataTypeApiTest__unique_identifier, 
                       'flg_available':'true', 
                       'description':'desc 01'}}
        expect = data_type['data_type']
        received = self._DataTypeApiTest__create_entry_api(data_type, expect)
        data_type_id = received['id']
        data_type_name = received['name']
        self._DataTypeApiTest__create_error_entry_uk_api(data_type)
        self._DataTypeApiTest__get_all_entries_by_name_api(data_type_name, expect)
        self._DataTypeApiTest__get_entry_by_id_api(data_type_id, expect)
        self._DataTypeApiTest__update_entry_api(data_type_id, expect)
        self._DataTypeApiTest__delete_entry_by_id_api(data_type_id)
    ...
    def __create_entry_api(self, entry_info, expect):
        response = self.client_api.create_data_type_api(entry_info)
    ...
    def __update_entry_api(self, entry_id, expect):
        ...
        data_type_upd = {'data_type': {'name':unique_name_upd, 
                       'identifier':unique_id_upd, 
                       'flg_available':'false', 
                       'description':'desc 01 updated!!!'}}
        response = self.client_api.update_data_type_api(entry_id, data_type_upd)
        resp_content = self.load_response_content(response)
        receive = resp_content
        expect_upd = data_type_upd['data_type']
