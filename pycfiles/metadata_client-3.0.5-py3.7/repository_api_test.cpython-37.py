...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/metadata_client/tests/apis/repository_api_test.py
...
class RepositoryApiTest(ApiBase, unittest.TestCase):
    ...
    def test_create_repository_api(self):
        _RepositoryApiTest__unique_name = Generators.generate_unique_name('RepositoryApi')
        _RepositoryApiTest__unique_identifier = Generators.generate_unique_identifier()
        repository = {'repository': {'name':_RepositoryApiTest__unique_name, 
                        'identifier':_RepositoryApiTest__unique_identifier, 
                        'mount_point':_RepositoryApiTest__unique_identifier, 
        ...
        received = self._RepositoryApiTest__create_entry_api(repository, expect)
        repository_id = received['id']
        repository_name = received['name']
        self._RepositoryApiTest__create_error_entry_uk_api(repository)
        self._RepositoryApiTest__get_all_entries_by_name_api(repository_name, expect)
        self._RepositoryApiTest__get_entry_by_id_api(repository_id, expect)
        self._RepositoryApiTest__update_entry_api(repository_id, expect)
        self._RepositoryApiTest__delete_entry_by_id_api(repository_id)
    ...
    def __update_entry_api(self, entry_id, expect):
        ...
        repository_upd = {'repository': {'name':unique_name_upd, 
        ...
        response = self.client_api.update_repository_api(entry_id, repository_upd)
        resp_content = self.load_response_content(response)
        receive = resp_content
