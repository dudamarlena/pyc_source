...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/metadata_client/tests/apis/parameter_type_api_test.py
# Compiled at: 2017-06-19 07:48:53
# Size of source mod 2**32: 5477 bytes
"""ParameterTypeApiTest class"""
import unittest
...
class ParameterTypeApiTest(ApiBase, unittest.TestCase):
    ...
    def test_create_parameter_type_api(self):
        _ParameterTypeApiTest__unique_name = Generators.generate_unique_name('ParameterTypeApi')
        _ParameterTypeApiTest__unique_identifier = Generators.generate_unique_identifier()
        parameter_type = {'parameter_type': {'name':_ParameterTypeApiTest__unique_name, 
                            'identifier':_ParameterTypeApiTest__unique_identifier, 
                            'flg_available':'true', 
                            'description':'desc 01'}}
        expect = parameter_type['parameter_type']
        received = self._ParameterTypeApiTest__create_entry_api(parameter_type, expect)
        parameter_type_id = received['id']
        parameter_type_name = received['name']
        self._ParameterTypeApiTest__create_error_entry_uk_api(parameter_type)
        self._ParameterTypeApiTest__get_all_entries_by_name_api(parameter_type_name, expect)
        self._ParameterTypeApiTest__get_entry_by_id_api(parameter_type_id, expect)
        self._ParameterTypeApiTest__update_entry_api(parameter_type_id, expect)
        self._ParameterTypeApiTest__delete_entry_by_id_api(parameter_type_id)
    ...
    def __create_entry_api(self, entry_info, expect):
        response = self.client_api.create_parameter_type_api(entry_info)
    ...
    def __update_entry_api(self, entry_id, expect):
        ...
        parameter_type_upd = {'parameter_type': {'name':unique_name_upd, 
                            'identifier':unique_id_upd, 
                            'flg_available':'false', 
                            'description':'desc 01 updated!!!'}}
        response = self.client_api.update_parameter_type_api(entry_id, parameter_type_upd)
        resp_content = self.load_response_content(response)
        receive = resp_content
        expect_upd = parameter_type_upd['parameter_type']
