...
# [GCC 8.4.0]
# Embedded file name: /Users/hughdbrown/workspace/hughdbrown/autopylint/src/autopylint.py
...
import os.path, sys, re, logging
from collections import namedtuple, Counter
from operator import attrgetter
from sed.engine import StreamEditor, call_main, REPEAT, NEXT, CUT, ANY
from src.python.autopylint.table_regex import MODULE_NAME, PYLINT_ITEM, PYLINT_SEMI_ITEM, PYLINT_ERROR_ITEM
from src.python.autopylint.action_regex import STD_IMPORT, FROM_IMP, IF_STMT, CONTINUATION, HANGING
from src.python.autopylint.repair_regex import ASSIGNMENTS, COMPARISONS, START_BRACKETS, END_BRACKETS, WHITESPACE_TABLE
...
def item_maker(match):
    ...
    return Item(match['type'], int(match['where1']) - 1, int(match['where2']), match['desc'].rstrip(), match['error'].rstrip())
...
def line_split(s, length):
    ...
    if len(s) <= length:
        ...
        if m1 or m2:
            g, conj = (m1.groupdict(),
             ' or' if m1 else m2.groupdict(), ' and')
            result = [
             ind0 + 'if (',
             ind2 + g['first'] + conj,
             ind2 + g['second'],
...
def find_string(s):
    ...
    if result is not None:
        ...
        end = next(i for i, c in enumerate(s[start + 1:]) if c == quote_char and s[(i - 1)] != '\\')
        if end is not None:
...
def bad_continuation(editor, item):
    ...
    if m:
        ...
        if verb:
            repaired_line = error_text[count:] if verb == 'remove' else ' ' * count + error_text if verb == 'add' else None
...
def trailing_newline(editor, item):
    ...
    line_no = item.line_no
    loc = (
     next(x for x in reversed(range(line_no, len(editor.lines))) if not re.match('^\\s*$', editor.lines[x])) or line_no,
     len(editor.lines))
...
def unused_import(editor, item):
    ...
    if m:
        ...
        if not final_imports:
            ...
            return (
             line_no + 1, -1)
        repaired_line = ('from {0} import {1}').format(library, (', ').join(sorted(final_imports)))
        loc = (
         line_no, line_no + 1)
...
def unused_variable(editor, item):
    ...
    unused_re = re.compile("Unused variable '(?P<unused>.*)'")
    line_no = item.line_no
    error_text = editor.lines[line_no]
    m = unused_re.search(item.desc)
    unused_var = ('\\b{0[unused]}\\b').format(m.groupdict())
    if re.match(('.*except.*as\\s+{0}:').format(unused_var), error_text):
        repaired_line = re.sub(('\\s+as+{0}').format(unused_var), '', error_text)
    else:
...
def wrong_import_order(editor, item):
    ...
    if m:
        g = m.groupdict()
        before, after = (re.compile(('^{0}$').format(g[key])) for key in ('before',
                                                                          'after'))
...
def relative_import(editor, item):
    ...
    line_no = item.line_no
    error_text = editor.lines[line_no]
    regex = "Relative import '(?P<actual>[\\w\\d\\._]+)', should be '(?P<desired>[\\w\\d\\._]+)'"
    m = re.match(regex, item.desc)
    if m:
        g = m.groupdict()
        actual, desired = g['actual'], g['desired']
        regex = ('^(.*?){0}').format(actual)
        repl = ('\\1{0}').format(desired)
...
def dangerous_default_value(editor, item):
    ...
    line_no = item.line_no
    error_text = editor.lines[line_no]
    regex = 'Dangerous default value (?P<default_arg>.*?) as argument'
    m = re.match(regex, item.desc)
    if m:
        default_arg = m.groupdict()['default_arg']
        regex = ('^.*(?P<arg_name>[\\w\\d_]+)(?P<spacing>\\s*=\\s*){0}.*$').format(default_arg)
...
FN_TABLE = {'anomalous-backslash-in-string': anomalous_backslash_in_string, 
    ...
    def fix_pylint(filename, items):
        ...
        try:
            ...
            for item in sorted(items, reverse=True, key=lambda x: x.line_no):
                ...
                if distance:
                    item = Item(item.type, item.line_no + distance, item.line_offset, item.desc, item.error)
                item_assert(item)
