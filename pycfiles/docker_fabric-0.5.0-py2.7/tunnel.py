...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/dockerfabric/tunnel.py
# Compiled at: 2016-12-10 16:24:03
...
class LocalTunnel(object):
    ...
    def __init__(self, remote_port, remote_host=None, bind_port=None, bind_host=None, remote_cmd=None):
        ...
        self.remote_host = remote_host or b'localhost'
        self.bind_port = bind_port or remote_port
        self.bind_host = bind_host or b'localhost'
    ...
    def get_channel(self, transport, remote_addr, local_peer):
        channel = transport.open_channel(b'direct-tcpip', remote_addr, local_peer)
        if channel is None:
            raise Exception(b'Incoming request to %s:%d was rejected by the SSH server.' % remote_addr)
        return channel
    ...
    def connect(self):
        ...
        def accept(listen_sock, transport, remote_addr):
            ...
            handler = ThreadHandler(b'fwd', _forwarder, channel, accept_sock)
        ...
        self.listening_thread = ThreadHandler(b'local_bind', listener_thread_main, listening_socket, accept, connections[env.host_string].get_transport(), (
