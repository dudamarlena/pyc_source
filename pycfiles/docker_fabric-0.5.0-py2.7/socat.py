...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/dockerfabric/socat.py
# Compiled at: 2016-12-10 16:24:31
...
class SocketTunnels(ConnectionDict):
    ...
    def __getitem__(self, item):
        ...
        def _connect_socket_tunnel():
            ...
            svc = SocketTunnel(remote_socket, local_port, env.get(b'socat_quiet', True))
            svc.connect()
            return svc
...
class SocketTunnel(LocalTunnel):
    ...
    def __init__(self, remote_socket, local_port, quiet=True):
        dest = b'STDIO'
        src = (b'UNIX-CONNECT:{0}').format(remote_socket)
        self.quiet = quiet
        self._socat_cmd = (b' ').join((b'socat', dest, src))
        super(SocketTunnel, self).__init__(local_port)

    def get_channel(self, transport, remote_addr, local_peer):
        channel = transport.open_channel(b'session')
        if channel is None:
            raise Exception(b'Failed to open channel on the SSH server.')
        if not self.quiet:
