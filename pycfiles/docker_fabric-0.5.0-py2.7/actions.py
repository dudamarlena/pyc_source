...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/dockerfabric/actions.py
# Compiled at: 2017-09-18 09:06:48
...
def script(container, script_path, fail_nonzero=False, upload_dir=False, **kwargs):
    ...
    with temp_dir() as (remote_tmp):
        ...
        results = [ output.result for output in container_fabric().run_script(container, script_path=remote_script, **kwargs) if o.action_type == ContainerUtilAction.SCRIPT
                  ]
    for res in results:
        puts((b'Exit code: {0}').format(res[b'exit_code']))
        if res[b'exit_code'] == 0 or not fail_nonzero:
            puts(res[b'log'])
        else:
            error(res[b'log'])
...
def single_cmd(container, command, fail_nonzero=False, download_result=None, **kwargs):
    ...
    """
    with temp_dir() as (remote_tmp):
        kwargs.setdefault(b'command_format', [b'-c', command])
        results = [ output.result for output in container_fabric().run_script(container, script_path=remote_tmp, **kwargs) if o.action_type == ContainerUtilAction.SCRIPT
                  ]
        if download_result:
            get(posixpath.join(remote_tmp, b'*'), local_path=download_result)
    for res in results:
        puts((b'Exit code: {0}').format(res[b'exit_code']))
        if res[b'exit_code'] == 0 or not fail_nonzero:
            puts(res[b'log'])
