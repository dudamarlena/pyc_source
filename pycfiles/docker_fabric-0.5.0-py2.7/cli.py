...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/dockerfabric/cli.py
# Compiled at: 2017-12-27 15:57:16
from __future__ import unicode_literals
import os, posixpath
from fabric.api import cd, env, fastprint, get, put, run, settings, sudo
from fabric.network import needs_host
from dockermap.api import USE_HC_MERGE
from dockermap.client.cli import DockerCommandLineOutput, parse_containers_output, parse_inspect_output, parse_images_output, parse_version_output, parse_top_output, parse_networks_output, parse_volumes_output
...
def _find_image_id(output):
    for line in reversed(output.splitlines()):
        if line and line.startswith(b'Successfully built '):
            return line[19:]
...
class DockerCliClient(DockerUtilityMixin):
    ...
    def __init__(self, cmd_prefix=None, default_bin=None, base_url=None, tls=None, use_sudo=None, debug=None):
        ...
        base_url = base_url or env.get(b'docker_base_url')
        if base_url:
            cmd_args = [
             (b'-H {0}').format(base_url)]
        else:
            cmd_args = []
        if tls or tls is None and env.get(b'docker_tls'):
            cmd_args.append(b'--tls')
        self._out = DockerCommandLineOutput(cmd_prefix or env.get(b'docker_cli_prefix'), default_bin or env.get(b'docker_cli_bin', b'docker'), cmd_args or None)
        if use_sudo or use_sudo is None and env.get(b'docker_cli_sudo'):
            self._call_method = sudo
        else:
            self._call_method = run
        self._quiet = not (debug or debug is None and env.get(b'docker_cli_debug'))
    ...
    def create_container(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'create_container', *args, **kwargs)
        return {b'Id': self._call(cmd_str)}

    def start(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'start', *args, **kwargs)
        self._call(cmd_str)

    def restart(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'restart', *args, **kwargs)
        self._call(cmd_str)

    def stop(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'stop', *args, **kwargs)
        self._call(cmd_str)

    def remove_container(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'remove_container', *args, **kwargs)
        self._call(cmd_str)

    def remove_image(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'remove_image', *args, **kwargs)
        self._call(cmd_str)

    def kill(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'kill', *args, **kwargs)
        self._call(cmd_str)

    def wait(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'wait', *args, **kwargs)
        self._call(cmd_str)

    def containers(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'containers', *args, **kwargs)
        res = self._call(cmd_str, quiet=True)
        return parse_containers_output(res)

    def inspect_container(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'inspect_container', *args, **kwargs)
        res = self._call(cmd_str, quiet=True)
        return parse_inspect_output(res, b'container')

    def images(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'images', *args, **kwargs)
        res = self._call(cmd_str, quiet=True)
        return parse_images_output(res)

    def pull(self, repository, tag=None, **kwargs):
        repo_tag = (b'{0}:{1}').format(repository, tag) if tag else repository
        cmd_str = self._out.get_cmd(b'pull', repo_tag, **kwargs)
        self._call(cmd_str)

    def push(self, repository, tag=None, **kwargs):
        repo_tag = (b'{0}:{1}').format(repository, tag) if tag else repository
        cmd_str = self._out.get_cmd(b'push', repo_tag, **kwargs)
        self._call(cmd_str)

    def create_network(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'create_network', *args, **kwargs)
        return {b'Id': self._call(cmd_str)}

    def remove_network(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'remove_network', *args, **kwargs)
        self._call(cmd_str)

    def connect_container_to_network(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'connect_container_to_network', *args, **kwargs)
        self._call(cmd_str)

    def disconnect_container_from_network(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'disconnect_container_from_network', *args, **kwargs)
        self._call(cmd_str)

    def networks(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'networks', *args, **kwargs)
        res = self._call(cmd_str, quiet=True)
        return parse_networks_output(res)

    def inspect_network(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'inspect_network', *args, **kwargs)
        res = self._call(cmd_str, quiet=True)
        return parse_inspect_output(res, b'network')

    def create_volume(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'create_volume', *args, **kwargs)
        return {b'Name': self._call(cmd_str)}

    def remove_volume(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'remove_volume', *args, **kwargs)
        self._call(cmd_str)

    def volumes(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'volumes', *args, **kwargs)
        res = self._call(cmd_str, quiet=True)
        return {b'Volumes': parse_volumes_output(res), b'Warnings': None}

    def inspect_volume(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'inspect_volume', *args, **kwargs)
        res = self._call(cmd_str, quiet=True)
        return parse_inspect_output(res, b'volume')

    def exec_create(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'exec_create', *args, **kwargs)
        self._call(cmd_str)

    def exec_start(self, *args, **kwargs):
        cmd_str = self._out.get_cmd(b'exec_start', *args, **kwargs)
        self._call(cmd_str)

    def top(self, container, ps_args):
        if ps_args:
            cmd_str = self._out.get_cmd(b'top', container, ps_args)
        else:
            cmd_str = self._out.get_cmd(b'top', container)
        res = self._call(cmd_str, quiet=True)
        return parse_top_output(res)

    def tag(self, image, repository, tag=None, **kwargs):
        if tag:
            repo_tag = (b'{0}:{1}').format(repository, tag)
        else:
            repo_tag = repository
        cmd_str = self._out.get_cmd(b'tag', image, repo_tag, **kwargs)
        return self._call(cmd_str)

    def logs(self, *args, **kwargs):
        kwargs.pop(b'stream', None)
        cmd_str = self._out.get_cmd(b'logs', *args, **kwargs)
        ...
        for key, variable in [
         ...
         ('insecure_registry', 'insecure')]:
            if key not in kwargs:
                env_value = env.get((b'docker_registry_{0}').format(variable))
                if env_value:
                    kwargs[key] = env_value

        registry = kwargs.pop(b'registry', env.get(b'docker_registry_repository'))
        if registry:
            cmd_str = self._out.get_cmd(b'login', registry, **kwargs)
        else:
            cmd_str = self._out.get_cmd(b'login', **kwargs)
        res = self._call(cmd_str, quiet=True)
        lines = res.splitlines()
        fastprint(lines)
        return b'Login Succeeded' in lines

    def build(self, tag, add_latest_tag=False, add_tags=None, raise_on_error=True, **kwargs):
        try:
            context = kwargs.pop(b'fileobj')
        except KeyError:
            raise ValueError(b"'fileobj' needs to be provided. Using 'path' is currently not implemented.")

        for a in [b'custom_context', b'encoding']:
            kwargs.pop(a, None)

        with temp_dir() as (remote_tmp):
            remote_fn = posixpath.join(remote_tmp, b'context')
            put(context, remote_fn)
            cmd_str = self._out.get_cmd(b'build', b'- <', remote_fn, tag=tag, **kwargs)
    ...
    def version(self, **kwargs):
        kwargs.pop(b'api_version', None)
        cmd_str = self._out.get_cmd(b'version')
        res = self._call(cmd_str, quiet=True)
    ...
    def _update_api_version(self):
        if self.api_version and self.api_version != b'auto':
            return
        version_dict = self.version()
        if b'APIVersion' in version_dict:
            self.api_version = version_dict[b'APIVersion']
        elif b'ApiVersion' in version_dict:
            self.api_version = version_dict[b'ApiVersion']
...
def copy_resource(container, resource, local_filename, contents_only=True):
    ...
    with temp_dir() as (remote_tmp):
        ...
        copy_path = posixpath.join(remote_tmp, b'copy_tmp')
        run(mkdir(copy_path, check_if_exists=True))
        remote_name = posixpath.join(copy_path, base_name)
        archive_name = (b'container_{0}.tar.gz').format(container)
        archive_path = posixpath.join(remote_tmp, archive_name)
        run((b'docker cp {0}:{1} {2}').format(container, resource, copy_path), shell=False)
...
def copy_resources(src_container, src_resources, storage_dir, dst_directories=None, apply_chown=None, apply_chmod=None):
    ...
    def _copy_resource(resource):
        ...
        run((b'docker cp {0}:{1} {2}').format(src_container, resource, rel_path), shell=False)
        return

    directories = dst_directories or {}
    generic_path = directories.get(b'*')
...
def isolate_and_get(src_container, src_resources, local_dst_dir, **kwargs):
    ...
    with temp_dir() as (remote_tmp):
        copy_path = posixpath.join(remote_tmp, b'copy_tmp')
        archive_path = posixpath.join(remote_tmp, (b'container_{0}.tar.gz').format(src_container))
        copy_resources(src_container, src_resources, copy_path, **kwargs)
        with cd(copy_path):
            sudo(targz(archive_path, b'*'))
...
def isolate_to_image(src_container, src_resources, dst_image, **kwargs):
    ...
    with temp_dir() as (remote_tmp):
        ...
        with cd(remote_tmp):
            sudo((b'tar -cz * | docker import - {0}').format(dst_image))
...
def save_image(image, local_filename):
    ...
    """
    r_name, __, i_name = image.rpartition(b'/')
    i_name, __, __ = i_name.partition(b':')
    with temp_dir() as (remote_tmp):
        archive = posixpath.join(remote_tmp, (b'image_{0}.tar.gz').format(i_name))
        run((b'docker save {0} | gzip --stdout > {1}').format(image, archive), shell=False)
        get(archive, local_filename)


@needs_host
def flatten_image(image, dest_image=None, no_op_cmd=b'/bin/true', create_kwargs={}, start_kwargs={}):
    """
    ...
    with temp_container(image, no_op_cmd=no_op_cmd, create_kwargs=create_kwargs, start_kwargs=start_kwargs) as (c):
        run((b'docker export {0} | docker import - {1}').format(c, dest_image), shell=False)