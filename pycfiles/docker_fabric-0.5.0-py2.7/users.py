...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/dockerfabric/utils/users.py
# Compiled at: 2014-10-01 07:12:26
...
def get_group_id(groupname):
    ...
    :rtype: int
    """
    gid = single_line_stdout((b'id -g {0}').format(groupname), expected_errors=(1, ), shell=False)
...
def get_user_id(username):
    ...
    :rtype: int
    """
    uid = single_line_stdout((b'id -u {0}').format(username), expected_errors=(1, ), shell=False)
...
def get_user_groups(username):
    ...
    :rtype: list
    """
    out = single_line_stdout((b'groups {0}').format(username))
    if out:
        return out.split()[2:]
    else:
        return
...
def get_or_create_group(groupname, gid_preset, system=False, id_dependent=True):
    ...
    if gid is None:
        ...
        if id_dependent and gid != gid_preset:
            error((b"Present group id '{0}' does not match the required id of the environment '{1}'.").format(gid, gid_preset))
        return gid


def get_or_create_user(username, uid_preset, groupnames=[], system=False, no_password=False, no_login=True, gecos=None, id_dependent=True):
    """
    ...
    if id_dependent and gid is not None and gid != uid_preset:
        error((b"Present group id '{0}' does not match the required id of the environment '{1}'.").format(gid, uid_preset))
    if gid is None:
    ...
    if uid is None:
        ...
        return uid
    if id_dependent and uid != uid_preset:
        error((b"Present user id '{0}' does not match the required id of the environment '{1}'.").format(uid, uid_preset))
