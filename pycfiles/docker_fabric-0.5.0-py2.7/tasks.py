...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/dockerfabric/tasks.py
# Compiled at: 2017-09-26 12:18:35
...
def _format_output_table(data_dict, columns, full_ids=False, full_cmd=False, short_image=False):
    ...
    def _format_port(port_dict):
        if b'PublicPort' in port_dict and b'IP' in port_dict:
            return (b'{IP}:{PublicPort}->{PrivatePort}/{Type}').format(**port_dict)
        return (b'{PrivatePort}/{Type}').format(**port_dict)

    def _get_column(item, column):
        data = item.get(column, b'')
        ...
        if column == b'Command' and not full_cmd:
            return (data[:25],)
        if column == b'Image' and short_image:
            __, __, i_name = data.rpartition(b'/')
            return (
             i_name,)
        return (
         unicode(data),)
    ...
    row_format = (b'  ').join((b'{{{0}:{1}}}').format(i, l) for i, l in enumerate(col_lens))
    for row in rows:
        for c in itertools.izip_longest(fillvalue=b'', *row):
            fastprint(row_format.format(*c), end=b'\n', flush=False)

    fastprint(b'', flush=True)
...
def reset_socat(use_sudo=False):
    ...
    :type use_sudo: bool
    """
    output = stdout_result(b'ps -o pid -C socat', quiet=True)
    pids = output.split(b'\n')[1:]
    puts((b'Removing process(es) with id(s) {0}.').format((b', ').join(pids)))
    which = sudo if use_sudo else run
    which((b'kill {0}').format((b' ').join(pids)), quiet=True)
...
def version():
    ...
    for k, v in six.iteritems(output):
        fastprint((b'{0:{1}} {2}').format((b'').join((k, b':')), col_len, v), end=b'\n', flush=False)

    fastprint(b'', flush=True)


@task
def get_ip(interface_name=b'docker0'):
    """
...
@task
def get_ipv6(interface_name=b'docker0', expand=False):
    """
...
def list_volumes():
    ...
    """
    volumes = docker_fabric().volumes()[b'Volumes'] or ()
...
def cleanup_containers(**kwargs):
    ...
    if kwargs.get(b'list_only'):
        puts(b'Existing containers:')
        for c_id, c_name in containers:
            fastprint((b'{0}  {1}').format(c_id, c_name), end=b'\n')
...
def cleanup_images(remove_old=False, **kwargs):
    ...
    """
    keep_tags = env.get(b'docker_keep_tags')
    if keep_tags is not None:
        kwargs.setdefault(b'keep_tags', keep_tags)
    removed_images = docker_fabric().cleanup_images(remove_old=remove_old, **kwargs)
    if kwargs.get(b'list_only'):
        puts(b'Unused images:')
        for image_name in removed_images:
            fastprint(image_name, end=b'\n')
...
def save_image(image, filename=None):
    ...
    """
    local_name = filename or (b'{0}.tar.gz').format(image)
...
def load_image(filename, timeout=120):
    ...
    """
    c = docker_fabric()
    with open(expand_path(filename), b'r') as (f):
        _timeout = c._timeout
        c._timeout = timeout
        try:
            c.load_image(f)
