...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/dockerfabric/utils/files.py
# Compiled at: 2016-05-25 11:25:37
...
def _safe_name(tarinfo):
    return tarinfo.name[0] != b'/' and b'..' not in tarinfo.name
...
def get_remote_temp():
    ...
    :rtype: unicode
    """
    return single_line_stdout(b'mktemp -d')
...
def is_directory(path, use_sudo=False):
    ...
    :rtype: bool or ``None``
    """
    result = single_line_stdout((b'if [[ -f {0} ]]; then echo 0; elif [[ -d {0} ]]; then echo 1; else echo -1; fi').format(path), sudo=use_sudo, quiet=True)
    if result == b'0':
        return False
    else:
        if result == b'1':
...
def extract_tar(filename, dest_path, **kwargs):
    ...
    """
    with tarfile.open(filename, b'r', **kwargs) as (tf):
