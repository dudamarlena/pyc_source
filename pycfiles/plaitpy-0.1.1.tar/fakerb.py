...
# [GCC 8.4.0]
# Embedded file name: /home/okay/tonka/src/plait.py/src/fakerb.py
# Compiled at: 2018-01-25 11:35:46
import os, yaml, random, re, hashlib
...
def open_cache():
    ...
    for file in OPENED_DATA:
        ...
        h = hashlib.md5(data)
        d = h.hexdigest()
        if 'md5' not in OPENED_DATA[file] or d != OPENED_DATA[file]['md5']:
...
def fetch(key, fallback_base, lookup=False):
    ...
    if len(tokens) > 1:
        ...
        if not fields:
            ...
            if filename not in OPENED_DATA:
