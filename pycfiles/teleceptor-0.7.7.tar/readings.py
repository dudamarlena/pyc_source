...
# [GCC 8.4.0]
# Embedded file name: /home/esalazar/git/teleceptor/teleceptor/api/readings.py
...
class SensorReadings:
    exposed = True
    validFilterArgs = {'datastream': '^\\d+$', 
       'start': '^\\d+$', 
       'end': '^\\d+$'}
    validOperatorArgs = {'condense': '^true|false$', 
       'granularity': '^\\d+$'}
    ...
    def cleanInputs(self, params):
        ...
        for key, value in paramsCopy.iteritems():
            if key in self.validFilterArgs:
                ...
                if key in self.validOperatorArgs:
...
def incrementMean(readings):
    ...
    return [
     mid_time, sum([ float(x[1]) for x in readings ]) / len(readings)]


reductMethods = {'mean': incrementMean}