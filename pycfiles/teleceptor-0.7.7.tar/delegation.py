...
# [GCC 8.4.0]
# Embedded file name: /home/esalazar/git/teleceptor/teleceptor/api/delegation.py
                    ...
                    with sessionScope() as (s):
                        ...
                        if sensor in mote['info']['in']:
                            if foundSensor.message_queue is not None:
                                msgList = []
                                msgs = s.query(Message).filter(Message.message_queue_id == foundSensor.message_queue.id, Message.timeout >= time(), Message.read != True)
                                for msg in msgs:
                                    msg.read = True
