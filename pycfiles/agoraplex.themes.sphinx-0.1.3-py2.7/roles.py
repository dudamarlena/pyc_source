...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/agoraplex/themes/sphinx/roles.py
# Compiled at: 2013-02-08 08:41:25
"""
...
def github_project_role(name, rawtext, text, lineno, inliner, options={}, content=[]):
    ...
    try:
        ...
        msg = inliner.reporter.error('GitHub requires `account/project` or `account`; "%s" is invalid.' % text, line=lineno)
...
def make_github_link_node(rawtext, app, account, project, options):
    ...
    set_classes(options)
    node = nodes.reference(rawtext, pagename, refuri=ref, **options)
    return node
...
def make_pypi_link_node(rawtext, app, project, options):
    ...
    set_classes(options)
    node = nodes.reference(rawtext, project, refuri=ref, **options)
    return node
...
def wikipedia_role(name, rawtext, text, lineno, inliner, options={}, content=[]):
    ...
    article = text.strip()
    if not article:
        msg = inliner.reporter.error('Wikipedia requires an `article name`; "%s" is invalid.' % text, line=lineno)
        prb = inliner.problematic(rawtext, rawtext, msg)
        return (
         [
          prb], [msg])
...
def make_wikipedia_link_node(rawtext, app, article, options):
    ...
    set_classes(options)
    node = nodes.reference(rawtext, article, refuri=ref, **options)
    return node
