...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/awsglue/dynamicframe.py
# Compiled at: 2019-08-21 04:18:45
...
class DynamicFrame(object):
    ...
    def mapPartitionsWithIndex(self, f, preservesPartitioning=False, transformation_ctx='', info='', stageThreshold=0, totalThreshold=0):
        return DynamicFrame(self.glue_ctx._jvm.DynamicFrame.fromPythonRDD(self._jdf, PipelinedRDD(self._rdd, f, preservesPartitioning)._jrdd, self.glue_ctx._ssql_ctx, transformation_ctx, self.name, _call_site(self._sc, callsite(), info), long(stageThreshold), long(totalThreshold)), self.glue_ctx, self.name)
    ...
    def fromDF(cls, dataframe, glue_ctx, name):
        ...
        :return: DynamicFrame
        """
        return DynamicFrame(glue_ctx._jvm.DynamicFrame.apply(dataframe._jdf, glue_ctx._ssql_ctx), glue_ctx, name)
    ...
    def unbox(self, path, format, transformation_ctx='', info='', stageThreshold=0, totalThreshold=0, **options):
        ...
        """
        return DynamicFrame(self._jdf.unbox(path, format, json.dumps(options), transformation_ctx, _call_site(self._sc, callsite(), info), long(stageThreshold), long(totalThreshold)), self.glue_ctx, self.name)
    ...
    def drop_fields(self, paths, transformation_ctx='', info='', stageThreshold=0, totalThreshold=0):
        ...
        if isinstance(paths, basestring):
            paths = [
             paths]
        return DynamicFrame(self._jdf.dropFields(self.glue_ctx._jvm.PythonUtils.toSeq(paths), transformation_ctx, _call_site(self._sc, callsite(), info), long(stageThreshold), long(totalThreshold)), self.glue_ctx, self.name)
    ...
    def select_fields(self, paths, transformation_ctx='', info='', stageThreshold=0, totalThreshold=0):
        ...
        if isinstance(paths, basestring):
            paths = [
             paths]
        return DynamicFrame(self._jdf.selectFields(self.glue_ctx._jvm.PythonUtils.toSeq(paths), transformation_ctx, _call_site(self._sc, callsite(), info), long(stageThreshold), long(totalThreshold)), self.glue_ctx, self.name)
    ...
    def split_fields(self, paths, name1, name2, transformation_ctx='', info='', stageThreshold=0, totalThreshold=0):
        ...
        if isinstance(paths, basestring):
            paths = [
             paths]
        jdfs = _as_java_list(self._sc, self._jdf.splitFields(self.glue_ctx._jvm.PythonUtils.toSeq(paths), transformation_ctx, _call_site(self._sc, callsite(), info), long(stageThreshold), long(totalThreshold)))
    ...
    def split_rows(self, comparison_dict, name1, name2, transformation_ctx='', info='', stageThreshold=0, totalThreshold=0):
        ...
        jdfs = _as_java_list(self._sc, self._jdf.splitRows(self.glue_ctx._jvm.PythonUtils.toSeq(paths), self.glue_ctx._jvm.PythonUtils.toSeq(values), self.glue_ctx._jvm.PythonUtils.toSeq(operators), transformation_ctx, _call_site(self._sc, callsite(), info), long(stageThreshold), long(totalThreshold)))
        return DynamicFrameCollection({name1: DynamicFrame(jdfs[0], self.glue_ctx, name1), name2: DynamicFrame(jdfs[1], self.glue_ctx, name2)}, self.glue_ctx)
    ...
    def rename_field(self, oldName, newName, transformation_ctx='', info='', stageThreshold=0, totalThreshold=0):
        ...
        :return: DynamicFrame
        """
        return DynamicFrame(self._jdf.renameField(oldName, newName, transformation_ctx, _call_site(self._sc, callsite(), info), long(stageThreshold), long(totalThreshold)), self.glue_ctx, self.name)

    def write(self, connection_type, connection_options={}, format=None, format_options={}, accumulator_size=0):
        return self.glue_ctx.write_from_options(frame_or_dfc=self, connection_type=connection_type,
    ...
    def spigot(self, path, options={}, transformation_ctx='', info='', stageThreshold=0, totalThreshold=0):
        return DynamicFrame(self._jdf.spigot(path, makeOptions(self._sc, options), transformation_ctx, _call_site(self._sc, callsite(), info), long(stageThreshold), long(totalThreshold)), self.glue_ctx, self.name)
    ...
    def relationalize(self, root_table_name, staging_path, options={}, transformation_ctx='', info='', stageThreshold=0, totalThreshold=0):
        ...
        """
        _rFrames = _as_java_list(self._sc, self._jdf.relationalize(root_table_name, staging_path, makeOptions(self._sc, options), transformation_ctx, _call_site(self._sc, callsite(), info), long(stageThreshold), long(totalThreshold)))
    ...
    def applyMapping(self, *args, **kwargs):
        if len(args) > 0:
            if isinstance(args[0], DynamicFrame):
    ...
    def apply_mapping(self, mappings, case_sensitive=False, transformation_ctx='', info='', stageThreshold=0, totalThreshold=0):
        ...
        def _to_java_mapping(mapping_tup):
            if not isinstance(mapping_tup, tuple):
                raise TypeError('Mapping must be specified as a tuple. Got ' + mapping_tup)
            ...
            if len(mapping_tup) == 3:
                return java_cls.apply(tup3.apply(mapping_tup[0], mapping_tup[1], mapping_tup[2]))
            if len(mapping_tup) == 4:
                return java_cls.apply(tup4.apply(mapping_tup[0], mapping_tup[1], mapping_tup[2], mapping_tup[3]))
            raise ValueError('Mapping tuple must be of length 2, 3, or 4Got tuple of length ' + str(len(mapping_tup)))

        if isinstance(mappings, tuple):
            mappings = [
             mappings]
        mappings_list = [_to_java_mapping(m) for m in mappings]
        new_jdf = self._jdf.applyMapping(self.glue_ctx._jvm.PythonUtils.toSeq(mappings_list), case_sensitive, transformation_ctx, _call_site(self._sc, callsite(), info), long(stageThreshold), long(totalThreshold))
        return DynamicFrame(new_jdf, self.glue_ctx, self.name)

    def resolveChoice(self, specs=None, choice='', database=None, table_name=None, transformation_ctx='', info='', stageThreshold=0, totalThreshold=0):
        """
        ...
        new_jdf = self._jdf.resolveChoice(self.glue_ctx._jvm.PythonUtils.toSeq(specs_list), choice_option, database_option, table_name_option, transformation_ctx, _call_site(self._sc, callsite(), info), long(stageThreshold), long(totalThreshold))
        return DynamicFrame(new_jdf, self.glue_ctx, self.name)
...
class DynamicFrameCollection(object):
    ...
    def __init__(self, dynamic_frames, glue_ctx):
        ...
        if isinstance(dynamic_frames, list):
            ...
            if isinstance(dynamic_frames, dict):
    ...
    def map(self, callable, transformation_ctx=''):
        ...
        for k, v in self._df_dict.iteritems():
            ...
            if not isinstance(res, DynamicFrame):
                raise TypeError('callable must return a DynamicFrame. Got {}'.format(str(type(res))))
            new_dict[k] = res
    ...
    def flatmap(self, f, transformation_ctx=''):
        ...
        for frame in self._df_dict.itervalues():
            ...
            if isinstance(res, DynamicFrame):
                ...
                raise TypeError('Function argument to flatmap must return DynamicFrame or DynamicFrameCollection. Got {}'.format(str(type(res))))
...
class DynamicFrameWriter(object):
    ...
    def from_options(self, frame, connection_type, connection_options={}, format=None, format_options={}, transformation_ctx=''):
        """Creates a DynamicFrame with the specified connection and format.
        """
        return self._glue_context.write_dynamic_frame_from_options(frame, connection_type, connection_options, format, format_options, transformation_ctx)
    ...
    def from_jdbc_conf(self, frame, catalog_connection, connection_options={}, redshift_tmp_dir='', transformation_ctx=''):
        ...
        """
        return self._glue_context.write_dynamic_frame_from_jdbc_conf(frame, catalog_connection, connection_options, redshift_tmp_dir, transformation_ctx)