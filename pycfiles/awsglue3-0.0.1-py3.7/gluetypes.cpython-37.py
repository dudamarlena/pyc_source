...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/awsglue/gluetypes.py
# Compiled at: 2019-08-21 04:11:21
# Size of source mod 2**32: 14201 bytes
import json
...
class DecimalType(AtomicType):
    ...
    def __repr__(self):
        return 'DecimalType({}, {}, {})'.format(self.precision, self.scale, self.properties)
...
class ArrayType(DataType):
    ...
    def __init__(self, elementType=UnknownType(), properties={}):
        assert isinstance(elementType, DataType), 'elementType should be DataType. Got' + str(elementType.__class__)
    ...
    def fromJsonValue(cls, json_value):
        ...
        return cls(elementType=element_type, properties=(json_value.get('properties', {})))
...
class ChoiceType(DataType):
    ...
    def __repr__(self):
        sorted_values = sorted((self.choices.values()), key=(lambda x: x.typeName()))
    ...
    def add(self, new_choice):
        if new_choice.typeName() in self.choices:
            raise ValueError('Attempting to insert duplicate choice', new_choice)
    ...
    def jsonValue(self):
        return dict(super(ChoiceType, self).jsonValue().items() + [
         (
          'choices',
          [v.jsonValue() for v in self.choices.values()])])
...
class MapType(DataType):
    ...
    @classmethod
    def fromJsonValue(cls, json_value):
        return cls(valueType=(_deserialize_json_value(json_value['valueType'])), properties=(json_value.get('properties', {})))


class Field(object):

    def __init__(self, name, dataType, properties={}):
        assert isinstance(dataType, DataType), 'dataType should be DataType. Got ' + str(dataType.__class__)
    ...
    def __repr__(self):
        return 'Field({}, {}, {})'.format(self.name, self.dataType, self.properties)
    ...
    @classmethod
    def fromJsonValue(cls, json_value):
        return cls(json_value['name'], _deserialize_json_value(json_value['container']), json_value.get('properties', {}))
...
class StructType(DataType):
    ...
    def __init__(self, fields=[], properties={}):
        ...
        assert all((isinstance(f, Field) for f in fields)), 'fields should be a list of Field'
        self.fields = fields
    ...
    def __repr__(self):
        return 'StructType([{}], {})'.format(','.join([str(f) for f in self.fields]), self.properties)
    ...
    @classmethod
    def fromJsonValue(cls, json_value):
        return cls([Field.fromJsonValue(f) for f in json_value['fields']], json_value.get('properties', {}))
...
_complex_types = [
 ArrayType, ChoiceType, MapType, StructType, SetType]
...
def _make_choice(s1, s2):
    ...
    for typecode, datatype in left_types.iteritems():
        if typecode in right_types:
            right_types[typecode] = mergeDataTypes(datatype, right_types[typecode])
        else:
...
def mergeDataTypes(s1, s2):
    if isinstance(s1, UnknownType) or isinstance(s1, NullType):
        return s2
    if isinstance(s2, UnknownType) or isinstance(s2, NullType):
        return s1
    if isinstance(s1, ChoiceType) or isinstance(s2, ChoiceType):
    ...
    if isinstance(s1, StructType):
        ...
        for field in s1:
            if s2.hasField(field):
                new_fields.append(Field(field.name, mergeDataTypes(field.dataType, s2.getField(field).dataType), field.properties))
            else:
                new_fields.append(Field(field.name, field.dataType, field.properties))

        new_fields.extend([Field(field.name, field.dataType, field.properties) for field in s2 if not s1.hasField(field)])
        return StructType(new_fields, s1.properties)
    if isinstance(s1, ArrayType):
        return ArrayType(mergeDataTypes(s1.elementType, s2.elementType))
    if isinstance(s1, MapType):
        return MapType(mergeDataTypes(s1.valueType, s2.valueType))
    if isinstance(s1, EnumType):
...
def _revert_to_dict(dynamicRecord):
    ...
    if isinstance(dynamicRecord, list):
