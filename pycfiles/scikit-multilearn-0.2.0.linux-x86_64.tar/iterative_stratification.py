...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/skmultilearn/model_selection/iterative_stratification.py
# Compiled at: 2018-10-09 20:28:22
...
def _get_most_desired_combination(samples_with_combination):
    ...
    for combination, evidence in samples_with_combination.items():
        ...
        if support_size == 0:
            continue
        if currently_chosen is None or best_number_of_combinations < number_of_combinations and best_support_size > support_size:
...
class IterativeStratification(_BaseKFold):
    ...
    def _prepare_stratification(self, y):
        ...
        self.desired_samples_per_fold = np.array([ self.percentage_per_fold[i] * self.n_samples for i in range(self.n_splits)
                                                 ])
        ...
        self.desired_samples_per_combination_per_fold = {combination:np.array([ len(evidence_for_combination) * self.percentage_per_fold[j] for j in range(self.n_splits) ]) for combination, evidence_for_combination in samples_with_combination.items()}
        return (
    ...
    def _iter_test_indices(self, X, y=None, groups=None):
        ...
        rows, rows_used, all_combinations, per_row_combinations, samples_with_combination, folds = self._prepare_stratification(y)
