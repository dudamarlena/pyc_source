...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/skmultilearn/dataset.py
# Compiled at: 2018-09-03 19:16:42
import arff, bz2, pickle
from scipy import sparse
import hashlib, os, requests, shutil
...
def available_data_sets():
    ...
    if r.status_code != 200:
        ...
        for row in raw_data_list.split('\n'):
            ...
            variant_information[(set_name, variant)] = [
             md5, file_name]
...
def download_dataset(set_name, variant, data_home=None):
    ...
    if md5 != found_md5:
        raise Exception(('{}: MD5 mismatch {} vs {} - possible download error').format(name, md5, found_md5))
...
def load_from_arff(filename, label_count, label_location='end', input_feature_type='float', encode_nominal=True, load_sparse=False, return_attribute_definitions=False):
...
def save_to_arff(X, y, label_location='end', save_sparse=True, filename=None):
    ...
    dataset = {'description': 'traindata', 'relation': ('traindata: -C {}').format(y.shape[1] * relation_sign), 
