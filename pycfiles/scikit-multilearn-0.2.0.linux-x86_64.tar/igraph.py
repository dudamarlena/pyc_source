...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/skmultilearn/cluster/igraph.py
...
class IGraphLabelGraphClusterer(LabelGraphClustererBase):
    ...
    """
    _METHODS = {'fastgreedy': lambda graph, w=None: graph.community_fastgreedy(weights=w).as_clustering(), 
    ...
    def fit_predict(self, X, y):
        ...
        self.graph_ = Graph(edges=[ x for x in edge_map ], vertex_attrs=dict(name=list(range(1, y.shape[1] + 1))), edge_attrs=self.weights_)
        return np.array(IGraphLabelGraphClusterer._METHODS[self.method](self.graph_, self.weights_['weight']))