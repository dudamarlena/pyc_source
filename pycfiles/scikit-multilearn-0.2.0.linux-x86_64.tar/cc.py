...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/skmultilearn/problem_transform/cc.py
...
class ClassifierChain(ProblemTransformationBase):
    ...
    def fit(self, X, y, order=None):
        ...
        for label in self._order():
            ...
            self.classifiers_[label] = self.classifier.fit(self._ensure_input_format(X_extended), self._ensure_output_format(y_subset))
    ...
    def predict(self, X):
        ...
        X_extended = self._ensure_input_format(X, sparse_format='csc', enforce_sparse=True)
        for label in self._order():
            prediction = self.classifiers_[label].predict(self._ensure_input_format(X_extended))
    ...
    def predict_proba(self, X):
        ...
        for label in self._order():
            prediction = self.classifiers_[label].predict(self._ensure_input_format(X_extended))
            prediction = self._ensure_output_format(prediction, sparse_format='csc', enforce_sparse=True)
            prediction_proba = self.classifiers_[label].predict_proba(self._ensure_input_format(X_extended))
            prediction_proba = self._ensure_output_format(prediction_proba, sparse_format='csc', enforce_sparse=True)[:, 1]
