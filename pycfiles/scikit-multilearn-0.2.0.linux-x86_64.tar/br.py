...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/skmultilearn/problem_transform/br.py
...
class BinaryRelevance(ProblemTransformationBase):
    ...
    def fit(self, X, y):
        ...
        for i in range(self.model_count_):
            ...
            classifier.fit(self._ensure_input_format(X), self._ensure_output_format(y_subset))
    ...
    def predict(self, X):
        ...
        :mod:`scipy.sparse` matrix of `{0, 1}`, shape=(n_samples, n_labels)
            binary indicator matrix with label assignments
        """
        predictions = [ self._ensure_multi_label_from_single_class(self.classifiers_[label].predict(self._ensure_input_format(X))) for label in range(self.model_count_)
                      ]
    ...
    def predict_proba(self, X):
        ...
        for label_assignment, classifier in zip(self.partition_, self.classifiers_):
            if isinstance(self.classifier, MLClassifierBase):
                ...
                result[:, label_assignment] = self._ensure_multi_label_from_single_class(classifier.predict_proba(self._ensure_input_format(X)))[:,
                 1]
