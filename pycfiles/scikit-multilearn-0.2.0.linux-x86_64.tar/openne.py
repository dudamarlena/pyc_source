...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/skmultilearn/embedding/openne.py
...
import networkx as nx, numpy as np, tensorflow as tf, scipy.sparse as sp
...
class OpenNetworkEmbedder:
    ...
    """
    _EMBEDDINGS = {'GraphFactorization': (
                            GraphFactorization, 'rep_size'), 
       'GraRep': (
                GraRep, 'dim'), 
       'HOPE': (
              HOPE, 'd'), 
       'LaplacianEigenmaps': (
                            LaplacianEigenmaps, 'rep_size'), 
       'LINE': (
              LINE, 'rep_size'), 
       'LLE': (
             LLE, 'd')}
    _AGGREGATION_FUNCTIONS = {'add': np.add.reduce, 
    ...
    def __init__(self, graph_builder, embedding, dimension, aggregation_function, normalize_weights, param_dict=None):
        ...
        if aggregation_function in self._AGGREGATION_FUNCTIONS:
            ...
            raise ValueError(('Aggregation function must be callable or one of {}').format((', ').join(self._AGGREGATION_FUNCTIONS.keys())))
    ...
    def _embedd_y(self, y):
        ...
        if sp.issparse(y):
            return np.array([ self.aggregation_function([ self.embeddings_.vectors[node] for node in row ]) if len(row) > 0 else empty_vector for row in _iterate_over_sparse_matrix(y)
                            ]).astype('float64')
        return np.array([ self.aggregation_function([ self.embeddings_.vectors[node] for node, v in enumerate(row) if v > 0 ]) if len(row) > 0 else empty_vector for row in y.A if isinstance(y, np.matrix) else y
                        ]).astype('float64')
