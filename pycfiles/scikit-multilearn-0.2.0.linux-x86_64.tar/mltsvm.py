...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/skmultilearn/adapt/mltsvm.py
# Compiled at: 2018-12-09 09:08:18
...
class MLTSVM(MLClassifierBase):
    ...
    def __init__(self, c_k=0, sor_omega=1.0, threshold=1e-06, lambda_param=1.0, max_iteration=500):
    ...
    def predict(self, X):
        ...
        wk_norms_multiplicated = self.wk_norms[np.newaxis, :]
    ...
    def _successive_overrelaxation(self, omegaW, Q):
        ...
        D_inv = 1.0 / D
        small_l = Q.shape[1]
        oldnew_alpha = np.zeros([small_l, 1])
        is_not_enough = True
        ...
        while is_not_enough:
            ...
            if not was_going_down and last_alfa_norm_change > alfa_norm_change:
                was_going_down = True
            is_not_enough = alfa_norm_change > self.threshold and nr_iter < self.max_iteration and (not was_going_down or last_alfa_norm_change > alfa_norm_change)
