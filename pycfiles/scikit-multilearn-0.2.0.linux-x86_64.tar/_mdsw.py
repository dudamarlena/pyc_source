...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/skmultilearn/embedding/_mdsw.py
# Compiled at: 2018-12-10 01:50:15
"""
...
def _smacof_single_w(similarities, n_uq, uq_weight, metric=True, n_components=2, init=None, max_iter=300, verbose=0, eps=0.001, random_state=None):
    """
...
def _smacof_w(similarities, n_uq, uq_weight, metric=True, n_components=2, init=None, n_init=8, n_jobs=1, max_iter=300, verbose=0, eps=0.001, random_state=None, return_n_iter=False):
    """
    ...
    if n_jobs == 1:
        for it in range(n_init):
            pos, stress, n_iter_ = _smacof_single_w(similarities, n_uq, uq_weight, metric=metric, n_components=n_components, init=init, max_iter=max_iter, verbose=verbose, eps=eps, random_state=random_state)
        ...
        results = Parallel(n_jobs=n_jobs, verbose=max(verbose - 1, 0))(delayed(_smacof_single_w)(similarities, n_uq, uq_weight, metric=metric, n_components=n_components, init=init, max_iter=max_iter, verbose=verbose, eps=eps, random_state=seed) for seed in seeds)
...
class _MDSW(BaseEstimator):
    ...
    def __init__(self, n_components=2, n_uq=1, uq_weight=None, metric=True, n_init=4, max_iter=300, verbose=0, eps=0.001, n_jobs=1, random_state=None, dissimilarity='euclidean'):
    ...
    def fit_transform(self, X, y=None, init=None):
        ...
        self.embedding_, self.stress_, self.n_iter_ = _smacof_w(self.dissimilarity_matrix_, self.n_uq, self.uq_weight, metric=self.metric, n_components=self.n_components, init=init, n_init=self.n_init, n_jobs=self.n_jobs, max_iter=self.max_iter, verbose=self.verbose, eps=self.eps, random_state=self.random_state, return_n_iter=True)
