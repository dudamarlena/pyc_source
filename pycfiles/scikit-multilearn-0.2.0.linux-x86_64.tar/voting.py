...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/skmultilearn/ensemble/voting.py
...
class MajorityVotingClassifier(LabelSpacePartitioningClassifier):
    ...
    def __init__(self, classifier=None, clusterer=None, require_dense=None):
        super(MajorityVotingClassifier, self).__init__(classifier=classifier, clusterer=clusterer, require_dense=require_dense)
    ...
    def predict(self, X):
        ...
        """
        predictions = [ self._ensure_input_format(self._ensure_input_format(c.predict(X)), sparse_format='csc', enforce_sparse=True) for c in self.classifiers_
                      ]
