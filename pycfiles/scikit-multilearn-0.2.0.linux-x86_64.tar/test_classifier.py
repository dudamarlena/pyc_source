...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/skmultilearn/embedding/unit_tests/test_classifier.py
...
class EmbeddingTest(ClassifierBaseTest):
    ...
    def classifiers(self):
        ...
        param_dicts = {'GraphFactorization': dict(epoch=1), 
           'GraRep': dict(Kstep=2), 
           'HOPE': dict(), 
        ...
        if not (sys.version_info[0] == 2 or platform.architecture()[0] == '32bit'):
            for embedding in OpenNetworkEmbedder._EMBEDDINGS:
                if embedding == 'LLE':
                    ...
                    dimension = 4
                yield EmbeddingClassifier(OpenNetworkEmbedder(copy(graph_builder), embedding, dimension, 'add', True, param_dicts[embedding]), LinearRegression(), MLkNN(k=2))

        yield EmbeddingClassifier(SKLearnEmbedder(SpectralEmbedding(n_components=2)), LinearRegression(), MLkNN(k=2))
        EmbeddingClassifier(CLEMS(metrics.accuracy_score, True), LinearRegression(), MLkNN(k=2), True)
