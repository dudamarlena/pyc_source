...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/skmultilearn/cluster/random.py
...
class RandomLabelSpaceClusterer(LabelSpaceClustererBase):
    ...
    def fit_predict(self, X, y):
        ...
        if (self.cluster_count + 1) * self.cluster_size < y.shape[1]:
            raise ValueError(('Cannot include all of {} labels in {} clusters of {} labels').format(y.shape[1], self.cluster_count, self.cluster_size))
        ...
        while not all_labels_assigned_to_division:
            ...
            all_labels_assigned_to_division = all(any(label in subset for subset in label_sets) for label in range(y.shape[1]))
