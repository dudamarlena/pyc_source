...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/skmultilearn/model_selection/measures.py
...
def example_distribution(folds, desired_size):
    ...
    return np.sum(np.abs(len(fold) - desired_fold_size) for fold, desired_fold_size in zip(folds, desired_size)) / n_splits
...
def folds_label_combination_pairs_without_evidence(y, folds, order):
    u"""Fold - Label / Label Pair / Label Combination (FLZ, FLPZ, FLCZ)  pair count measure
...
def label_combination_distribution(y, folds, order):
    u"""Label / Label Pair / Label Combination Distribution (LD, LPD, LCZD) measure
    ...
    for combination in all_combinations:
        number_of_samples_with_combination = np.sum([ 1 for combinations_in_row in combinations_per_row if combination in combinations_in_row ])
        d = _get_proportion(number_of_samples, number_of_samples_with_combination)
        internal_sum = 0
        for fold in folds:
            S_i_j = np.sum([ 1 for combinations_in_row in combinations_per_row[fold] if combination in combinations_in_row ])
            fold_size = len(fold)
