...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/skmultilearn/cluster/graphtool.py
...
class StochasticBlockModel:
    ...
    def fit_predict(self, graph, weights):
        ...
        if self.weight_model:
            self.model_ = self._model_fit_function()(graph, deg_corr=self.use_degree_correlation, overlap=self.allow_overlap, state_args=dict(recs=[weights], rec_types=[
             self.weight_model]))
        else:
