...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/archipelcore/archipelEntity.py
...
"""
import datetime, socket, sys, time, traceback, xmpp
...
import archipelcore.archipelPermissionCenter, archipelcore.pubsub
...
ARCHIPEL_MESSAGING_HELP_MESSAGE = '\nYou can communicate with me using text commands, just like if you were chatting with your friends. I try to understand you as much as I can, but you have to be nice with me.Note that you can use more complex sentence than describe into the following list. For example, if you see in the command ["how are you"], I\'ll understand any sentence containing "how are you". Parameters (if any) are separated with spaces.\nNote that the text you enter must start with the command: "How are you" works, "Hello, how are you" don\'t.\n\nFor example, you can send command using the following form:\ncommand param1 param2 param3\n\n'
...
class TNArchipelEntity(object):
    ...
    def initialize_modules(self, group):
        ...
        if loading_module_policy not in ('restrictive', 'permissive'):
    ...
    def connect_xmpp(self):
        ...
        if self.configuration.has_option('LOGGING', 'xmpppy_debug') and self.configuration.getboolean('LOGGING', 'xmpppy_debug'):
            debug_mode = [
             'always', 'nodebuilder']
        self.xmppclient = xmpp.Client(self.jid.getDomain(), debug=debug_mode)
    ...
    def recover_pubsubs(self, origin, user_info, arguments):
        ...
        self.pubSubNodeEvent.configure({archipelcore.pubsub.XMPP_PUBSUB_VAR_ACCESS_MODEL: archipelcore.pubsub.XMPP_PUBSUB_VAR_ACCESS_MODEL_OPEN, 
           ...
           archipelcore.pubsub.XMPP_PUBSUB_VAR_SEND_LAST_PUBLISHED_ITEM: archipelcore.pubsub.XMPP_PUBSUB_VAR_SEND_LAST_PUBLISHED_ITEM_NEVER}, wait=True)
        ...
        self.pubSubNodeLog.configure({archipelcore.pubsub.XMPP_PUBSUB_VAR_ACCESS_MODEL: archipelcore.pubsub.XMPP_PUBSUB_VAR_ACCESS_MODEL_OPEN, 
           archipelcore.pubsub.XMPP_PUBSUB_VAR_DELIVER_NOTIFICATION: 1, 
    ...
    def process_presence(self, conn, presence):
        ...
        if presence.getFrom().getStripped() == self.jid.getStripped():
            raise xmpp.protocol.NodeProcessed
        if presence.getType() not in ('subscribe', 'unsubscribe'):
            raise xmpp.protocol.NodeProcessed
    ...
    def build_help(self, msg):
        ...
        for registrar_item in self.messages_registrar:
            if 'ignore' not in registrar_item:
                granted = True
                if 'permissions' in registrar_item:
                    granted = self.permission_center.check_permissions(msg.getFrom().getStripped(), registrar_item['permissions'])
                if granted:
        ...
        try:
            ...
            for perm in perms:
                ...
                if perm_type == 'role':
                    ...
                    elif not self.permission_center.revoke_permission_to_role(perm_name, perm_target):
                ...
                if perm_target not in perm_targets:
