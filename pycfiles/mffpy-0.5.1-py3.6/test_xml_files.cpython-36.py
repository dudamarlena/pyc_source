...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/mffpy/tests/test_xml_files.py
# Compiled at: 2020-02-14 18:48:18
# Size of source mod 2**32: 13788 bytes
"""
...
import pytz, numpy as np, pytest
...
def test_DataInfo_calibrations_GCAL(field, expected, data_info):
    ...
    if isinstance(expected, dict):
        ...
        raise AssertionError('F[%s] = %s [should be %s]' % (
         field, val, expected))
...
@pytest.mark.parametrize('prop,idx,expected', [
 (
  'sensors', 1,
  {'name':'None',  'number':1,  'type':0, 
   'x':415.0,  'y':147.0,  'z':0.0}),
 (
  'sensors', 258,
  {'name':'None',  'number':258,  'type':2,  'x':270.0, 
   'y':93.0,  'z':0.0,  'identifier':1002}),
 ('threads', 0, (1, 2)),
 ('threads', -3, (253, 254)),
 (
  'tilingSets', 0,
  list(map(int, '4 7 10 14 17 19 23 27 30 32 35 37 41 44         46 48 51 57 60 62 65 70 73 75 78 86 89 91 93 95 99 107 109 112 114         120 122 124 126 130 141 145 147 150 153 155 160 166 170 172 174 176         184 187 189 191 193 195 198 206 209 212 216 218 222 224 228 236 240         248 251 254 256'.split()))),
 (
...
def test_Coordinates(coordinates):
    for idx, expected in [
     (
      1,
      {'name':'None',  'number':1,  'type':0,  'x':np.float32(5.88478),  'y':np.float32(6.51941), 
       'z':np.float32(-0.247411)}),
     (
      258,
      {'name':'Nasion',  'number':258,  'type':2,  'x':0.0,  'y':np.float32(10.1822), 
       'z':np.float32(-1.98987),  'identifier':2002})]:
    ...
    if not coordinates.acqTime == expected:
...
@pytest.mark.parametrize('idx,expected', [
 ...
 (
  -2,
  {'beginTime':3323676000,  'endTime':3359904000,  'firstBlock':184, 
   'lastBlock':186})])
...
def test_EventTrack_to_xml():
    ...
    events = [
     {'beginTime':XML._parse_time_str('2003-04-17T13:35:22.032000-08:00'), 
      'duration':1000, 
     ...
     {'beginTime':XML._parse_time_str('2003-04-17T13:35:22.032000-08:00'), 
      ...
      'code':'REOG'}]
    track_dict = XML.todict('eventTrack', name=name, trackType=trackType, events=events)
    if not track_dict.pop('filename') == 'Events.xml':
        raise AssertionError
    else:
        xml = dict2xml(**track_dict)
        xml_stream = BytesIO()
        xml.write(xml_stream, encoding='UTF-8', xml_declaration=True,
          method='xml')
        xml_stream.seek(0)
...
def test_Categories(categories):
    if not all(k in categories for k in ('ULRN', 'LRND')):
        raise AssertionError
    else:
        if not len(categories['ULRN']) == 50:
            raise AssertionError
        else:
            assert len(categories['LRND']) == 19
            expected_ULRN0 = {'status':'bad', 
             'beginTime':0, 
             'endTime':1200000, 
             'evtBegin':201981, 
             'evtEnd':201981, 
        ...
        expected_LRND0 = {'status':'good',  'beginTime':3655704000, 
         'endTime':3656904000, 
         'evtBegin':3655907981, 
         'evtEnd':3655907981, 
         'channelStatus':[
...
def test_dipoleSet(dipoleSet):
    if not dipoleSet.name == 'SWS_003_IHM':
        raise AssertionError(dipoleSet.name)
    else:
        if not dipoleSet.type == 'Dense':
            raise AssertionError(dipoleSet.type)
        else:
            if not len(dipoleSet) == 4:
                raise AssertionError(f"found {len(dipoleSet)}")
            elif not dipoleSet.computationCoordinate == pytest.approx(np.array([
             [
              64, 120, 150],
             [
              68, 120, 150],
