...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/mffpy/tests/test_reader.py
...
def signals_3():
    ...
    csv_file = join(dirname(__file__), '..', 'resources', 'testing', 'example_3_signals.csv')
...
@pytest.mark.parametrize('prop,expected', [
 ...
 (
  'startdatetime',
  datetime(2003, 4, 17, 13, 35, 22, tzinfo=(timezone(timedelta(-1, 57600))))),
 (
  'durations', {'EEG': 16.6}),
 (
...
def test_get_physical_samples(t0, expected_eeg, expected_start, reader):
    ...
    eeg = eeg[0]
    if not start_time == expected_start:
        raise AssertionError
    elif not eeg == pytest.approx(expected_eeg, abs=1e-06):
        raise AssertionError
...
def test_get_physical_samples_multiple_bin_files(signals_3, mffpath_3):
    ...
    if not eeg_start == pns_start == 0.0:
        raise AssertionError
    else:
        assert eeg == pytest.approx(expected_eeg, abs=1e-06)
        assert pns == pytest.approx(expected_pns, abs=1e-06)
