...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/mffpy/xml_files.py
# Compiled at: 2020-02-14 18:47:56
# Size of source mod 2**32: 31530 bytes
"""
...
class XMLType(type):
    ...
    def todict(typ, xmltype, **kwargs) -> Dict[(str, Any)]:
        ...
        return {'content':T.content(**kwargs), 
...
class FileInfo(XML):
    ...
    def recordTime(self):
        ...
        if el is not None:
            return self._parse_time_str(el.text)

    @classmethod
    def content(cls, recordTime: datetime, mffVersion: str='3') -> dict:
        ...
        return {'mffVersion':{TEXT: mffVersion}, 
         'recordTime':{TEXT: cls._dump_datetime(recordTime)}}

    def get_content(self):
        """return mff version and time of recording start"""
        return {'mffVersion':self.version, 
...
class DataInfo(XML):
    _xmlns = '{http://www.egi.com/info_n_mff}'
    _xmlroottag = 'dataInfo'
    ...
    def _parse_calibration(self, cali):
        ...
        ans['channels'] = {int(el.get('n')):np.float32(el.text) for el in self.find('channels', cali)}
        return ans

    @classmethod
    def content(cls, fileDataType: str, dataTypeProps: dict=None, filters: List[dict]=None, calibrations: List[dict]=None) -> dict:
        ...
        *calibrations*: lists a number of calibrations
        ...
        filters = filters or []
        return {'generalInformation':{TEXT: {'fileDataType': {TEXT: {fileDataType: {TEXT: {k:{TEXT: v} for k, v in dataTypeProps.items()}}}}}}, 
         'filters':{'filter': [{'beginTime':{TEXT: f['beginTime']},  'method':{TEXT: f['method']},  'type':{TEXT: f['type']},  'cutoffFrequency':{TEXT: f['cutoffFrequency'], ATTR: {'units': 'Hz'}}} for f in filters]}, 
         'calibrations':{'calibration': [{'beginTime':{TEXT: cal['beginTime']},  'type':{TEXT: cal['type']},  'channels':{'ch':[{TEXT: str(v), ATTR: {'n': str(n)}}] for k, (v, n) in cal.items()}} for cal in calibrations]}}

    def get_content(self):
        """return info on the associated (data) .bin file"""
        return {'generalInformation':self.generalInformation, 
...
class Patient(XML):
    _xmlns = '{http://www.egi.com/subject_mff}'
    _xmlroottag = 'patient'
    _default_filename = 'subject.xml'
    _type_converter = {'string':str, 
     None:lambda x: x}
...
class SensorLayout(XML):
    _xmlns = '{http://www.egi.com/sensorLayout_mff}'
    _xmlroottag = 'sensorLayout'
    _default_filename = 'sensorLayout.xml'
    _type_converter = {'name':str, 
     'number':int, 
     'type':int, 
     'identifier':int, 
     'x':np.float32, 
    ...
    @cached_property
    def sensors(self):
        return dict([self._parse_sensor(sensor) for sensor in self.find('sensors')])
    ...
    def get_serializable_content(self):
        ...
        for field in ('sensors', 'neighbors'):
            content[field] = {str(key):value for key, value in content[field].items()}
...
class Coordinates(XML):
    _xmlns = '{http://www.egi.com/coordinates_mff}'
    _xmlroottag = 'coordinates'
    ...
    def sensors(self):
        ...
        return dict([self._parse_sensor(sensor) for sensor in self.find('sensors', sensorLayout)])
...
class Epochs(XML):
    _xmlns = '{http://www.egi.com/epochs_mff}'
    _xmlroottag = 'epochs'
    _default_filename = 'epochs.xml'
    _type_converter = {'beginTime':int, 
     'endTime':int, 
     'firstBlock':int, 
     'lastBlock':int}
    ...
    @cached_property
    def epochs(self):
        return [self._parse_epoch(epoch) for epoch in self.root]
    ...
    @classmethod
    def content(cls, epochs: List[Epoch]) -> dict:
        return {'epoch': [epoch.content for epoch in epochs]}
    ...
    def get_content(self):
        ...
        for epch in self.epochs:
            epochs.append({'beginTime':epch.beginTime, 
...
class EventTrack(XML):
    _xmlns = '{http://www.egi.com/event_mff}'
    _xmlroottag = 'eventTrack'
    _default_filename = 'Events.xml'
    _event_type_reverter = {'beginTime':XML._dump_datetime, 
     'duration':str, 
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._event_type_converter = {'beginTime':lambda e: self._parse_time_str(e.text), 
        ...
        self._key_type_converter = {'short':np.int16, 
         'string':str}
    ...
    @cached_property
    def events(self):
        return [self._parse_event(event) for event in self.findall('event')]
    ...
    def _parse_event(self, events_el):
        ...
        return {tag:self._event_type_converter[tag](el) for tag, el in map(lambda e: (self.nsstrip(e.tag), e), events_el)}

    def _parse_keys(self, keys_el):
        return dict([self._parse_key(key_el) for key_el in keys_el])
    ...
    @classmethod
    def content(cls, name: str, trackType: str, events: List[dict]) -> dict:
        ...
        for event in events:
            ...
            for k, v in event.items():
                ...
                formatted[k] = {TEXT: cls._event_type_reverter[k](v)}

            formatted_events.append({TEXT: formatted})

        return {'name':{TEXT: name},  'trackType':{TEXT: trackType}, 
...
class Categories(XML):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._channel_prop_converter = {'signalBin':int, 
         'exclusion':str}
    ...
    def _parse_channel_status(self, status_el):
        ...
        for channel_el in self.findall('channels', status_el):
            channel = {prop:converter(channel_el.get(prop)) for prop, converter in self._channel_prop_converter.items()}
...
class DipoleSet(XML):
    ...
    def dipoles(self) -> Dict[(str, np.ndarray)]:
        ...
        d_arrays = {tag:np.array(lists, dtype=(np.float32)) for tag, lists in dipoles.items()}
        shp = (
         len(dipole_tags), 3)
        assert all(v.shape == shp for v in d_arrays.values()), f"\n        Parsing dipoles result in broken shape.  Found {[(k, v.shape) for k, v in d_arrays.items()]}"
        return d_arrays
