...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/mffpy/reader.py
# Compiled at: 2020-02-14 18:48:18
# Size of source mod 2**32: 11180 bytes
"""
...
class Reader:
    ...
    def sampling_rates(self) -> Dict[(str, float)]:
        ...
        """
        return {fn:bin_file.sampling_rate for fn, bin_file in self._blobs.items()}
    ...
    def durations(self) -> Dict[(str, float)]:
        ...
        """
        return {fn:bin_file.duration for fn, bin_file in self._blobs.items()}
    ...
    def units(self) -> Dict[(str, str)]:
        ...
        """
        return {fn:bin_file.unit for fn, bin_file in self._blobs.items()}
    ...
    def num_channels(self) -> Dict[(str, int)]:
        ...
        """
        return {fn:bin_file.num_channels for fn, bin_file in self._blobs.items()}
    ...
    def _blobs(self) -> Dict[(str, bin_files.BinFile)]:
        ...
        for si in self.directory.signals_with_info():
            with self.directory.filepointer(si.info) as (fp):
                info = XML.from_file(fp)
            bf = bin_files.BinFile(si.signal, info, info.generalInformation['channel_type'])
            _Reader__blobs[bf.signal_type] = bf

        return _Reader__blobs
    ...
    def get_physical_samples(self, t0: float=0.0, dt: float=None, channels: List[str]=None, block_slice: slice=None) -> Dict[(str, Tuple[(np.ndarray, float)])]:
        ...
        return {typ:blob.get_physical_samples(t0, dt, block_slice=block_slice) for typ, blob in self._blobs.items() if typ in channels}

    def get_physical_samples_from_epoch(self, epoch: xml_files.Epoch, t0: float=0.0, dt: float=None, channels: List[str]=None) -> Dict[(str,
 Tuple[(np.ndarray, float)])]:
        ...
        **Example use**
        ...
        ```
        """
        if not isinstance(epoch, xml_files.Epoch):
            raise AssertionError(f"\n        argument epoch of type {type(epoch)} [requires {xml_files.Epoch}]")
        elif not t0 >= 0.0:
            raise AssertionError('Only non-negative `t0` allowed [%s]' % t0)
        dt = dt if dt is None or 0.0 < dt < epoch.dt - t0 else None
        return self.get_physical_samples(t0,
          dt, channels, block_slice=(epoch.block_slice))
                    ...
                    if obj.xml_root_tag == 'categories':
                        for category in content['categories'].values():
                            for segment in category:
                                t0 = segment['beginTime'] * 1e-06
                                dt = (segment['endTime'] - segment['beginTime']) * 1e-06
                                samples = self.get_physical_samples(t0=t0,
                                  dt=dt,
                                  channels=['EEG'])
