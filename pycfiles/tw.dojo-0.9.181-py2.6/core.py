...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/tw/dojo/core.py
...
class DojoLinkMixin(Link):
    params = {'basename': '(string) basename for the given file.  if you want dojotest-min.js, the base is dojotest/dojotest', 'prefix': '(string) "", min, beta-min, or beta.  Default is "min"', 
...
dijit_dir = DojoLinkMixin(basename='static/dijit/')
dojox_dir = DojoLinkMixin(basename='static/dojox/')
tundra_css = DojoCSSLink(basename='dijit/themes/tundra/tundra')
soria_css = DojoCSSLink(basename='dijit/themes/soria/soria')
nihilo_css = DojoCSSLink(basename='dijit/themes/nihilo/nihilo')
tundragrid_css = DojoCSSLink(basename='dojox/grid/resources/tundraGrid')
soriagrid_css = DojoCSSLink(basename='dojox/grid/resources/soriaGrid')
nihilogrid_css = DojoCSSLink(basename='static/dojox/grid/resources/nihiloGrid')
...
class DojoTheme(Widget):
    ...
    def __init__(self, themes=['tundra'], **kw):
        ...
        for th in themes:
            if themes_css.has_key(th):
                self.css.extend(themes_css[th])
