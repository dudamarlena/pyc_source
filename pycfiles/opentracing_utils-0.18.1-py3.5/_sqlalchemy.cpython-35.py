...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/opentracing_utils/libs/_sqlalchemy.py
...
def trace_sqlalchemy(operation_name=None, span_extractor=None, set_error_tag=False, skip_span=None):
    ...
    def trace_before_cursor_execute(conn, cursor, statement, parameters, context, executemany):
        ...
        if context:
            ...
            if context:
                query_span.set_tag(ot_tags.COMPONENT, 'sqlalchemy').set_tag('db.type', 'sql').set_tag('db.engine', context.dialect.name).set_tag('db.statement', statement)
