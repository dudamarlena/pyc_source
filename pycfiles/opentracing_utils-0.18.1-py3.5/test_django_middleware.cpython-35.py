...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/test_django/test_django_middleware.py
# Compiled at: 2019-02-13 08:31:21
# Size of source mod 2**32: 7606 bytes
import opentracing, pytest, six
...
@pytest.mark.skipif(six.PY2, reason='')
@pytest.mark.parametrize('url,method,status', (('/', 'GET', 200), ('/', 'POST', 200),
                                               ('/notfound', 'GET', 404)))
...
def test_request_propagated(monkeypatch, client):
    ...
    response = client.get('/', HTTP_X_TRACE_ID=123)
    assert response.content == b'TRACED'
