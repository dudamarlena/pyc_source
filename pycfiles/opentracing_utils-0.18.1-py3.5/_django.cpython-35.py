...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/opentracing_utils/libs/_django.py
...
class OpenTracingHttpMiddleware(MiddlewareMixin):
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        op_name = self._op_name_callable(request, view_func, view_args, view_kwargs) if self._op_name_callable else view_func.__name__
        span = None
        try:
    ...
    def _get_headers(self, request):
        ...
        for k, v in request.META.items():
            k = k.lower().replace('_', '-')
            k = k.replace('http-', '') if k.startswith('http-') else k
            headers[k] = v
    ...
    def _finish_tracing(self, request, response=None, exception=None):
        ...
        if response:
            ...
            current_span.set_tag('error', True)
            current_span.log_kv({'error.kind': str(exception), 
