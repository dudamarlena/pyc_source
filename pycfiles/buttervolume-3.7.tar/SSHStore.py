...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/buttersink/SSHStore.py
# Compiled at: 2018-06-26 18:40:50
__doc__ = ' Manage read-only snapshots in a remote filesystem over SSH.\n\nCopyright (c) 2014-2015 Ames Cornish.  All rights reserved.  Licensed under GPLv3.\n'
from progress import DisplayProgress
import ButterStore, Store, version, io, json, logging, os.path, platform, subprocess, sys, traceback, urllib
...
class _Obj2Arg:
    ...
    def diff(self, diff):
        if diff is None:
            ...
            return (
             self.vol(diff.toVol), self.vol(diff.fromVol))
            return
...
class _Obj2Dict:
    ...
    def vol(self, vol):
        if vol is None:
            ...
            return dict(uuid=vol.uuid, gen=vol.gen, size=vol.size, exclusiveSize=vol.exclusiveSize)
...
class SSHStore(Store.Store):
    ...
    def __unicode__(self):
        ...
        return 'ssh://%s%s' % (self.host, self.userPath)
...
class _Client(object):
    ...
    def _open(self):
        ...
        if self._process is not None:
            ...
            self._process = subprocess.Popen(cmd, stdin=subprocess.PIPE, stderr=sys.stderr, stdout=subprocess.PIPE)
    ...
    def _checkMode(self, name, mode):
        ...
        if requestedMode > allowedMode:
            raise Exception("%s connection does not allow %s method '%s'" % (
...
class StoreProxyServer(object):
    ...
    def _errorInfo(self, command, error):
        ...
        return dict(error=str(error), errorType=type(error).__name__, command=command, server=True, traceback=trace)
