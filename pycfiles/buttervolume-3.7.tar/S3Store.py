...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/buttersink/S3Store.py
# Compiled at: 2018-06-26 18:40:50
__doc__ = ' Back-end for AWS S3.\n\nCopyright (c) 2014 Ames Cornish.  All rights reserved.  Licensed under GPLv3.\n\n'
if True:
    if True:
        from util import humanize
        import progress, Store, util, boto, boto.s3.connection, collections, io, logging, os.path, re
    if True:
        theProgressCount = 50
        isEncrypted = True
        theInfoBufferSize = 10 * 1048576
...
class S3Store(Store.Store):
    ...
    def __init__(self, host, path, mode, dryrun):
        ...
        try:
            s3 = boto.s3.connection.S3Connection(calling_format=boto.s3.connection.SubdomainCallingFormat())
    ...
    def __unicode__(self):
        ...
        return 'S3 Bucket "%s"' % self.bucketName
    ...
    def _flushPartialUploads(self, dryrun):
        for upload in self.bucket.list_multipart_uploads():
            if not upload.key_name.startswith(self.userPath.lstrip('/')):
                continue
            if self._skipDryRun(logger, 'INFO', dryrun)('Delete old partial upload: %s (%d parts)', upload, len([ part for part in upload ])):
                continue
...
class _Downloader(io.RawIOBase):
    ...
    def read(self, n=-1):
        ...
        if self.progress is None:
            data = self.key.get_contents_as_string(headers)
        else:
            progress = _BotoProgress(n, 'Range', self.progress)
            with progress:
                data = self.key.get_contents_as_string(headers, **_BotoProgress.botoArgs(progress))
        size = len(data)
...
class _Uploader(io.RawIOBase):
    ...
    def open(self):
        if self.uploader is not None:
            logger.warning('Ignoring double open%s', _displayTraceBack())
            return
        else:
            ...
            self.uploader = self.bucket.initiate_multipart_upload(self.keyName, encrypt_key=isEncrypted, metadata=self.metadata)
    ...
    def skipChunk(self, chunkSize, checkSum, data=None):
        ...
        if part is None:
            ...
            if tag != checkSum:
                ...
                return False
            self.chunkCount += 1
            logger.info('Skipping already uploaded %s chunk #%d', humanize(chunkSize), self.chunkCount)
            return True
    ...
    def close(self):
        if self.uploader is None:
            logger.debug('Ignoring double close%s', _displayTraceBack())
            return
        else:
            logger.debug('Closing%s', _displayTraceBack())
    ...
    def upload(self, bytes):
        ...
        if self.progress is None:
            self.uploader.upload_part_from_file(fileObject, self.chunkCount)
        else:
            cb = _BotoProgress(size, 'Chunk #%d' % self.chunkCount, self.progress)
            with cb:
                self.uploader.upload_part_from_file(fileObject, self.chunkCount, **_BotoProgress.botoArgs(cb))
        return size