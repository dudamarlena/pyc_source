...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/buttersink/btrfs.py
# Compiled at: 2018-06-26 18:40:50
__doc__ = ' Pythonic wrapper around kernel ioctl-level access to btrfs. '
from ioctl import Structure, t
from util import pretty, humanize
import collections, ioctl, logging, os.path
...
btrfs_ioctl_ino_lookup_args = Structure((
 t.u64, 'treeid'), (
 t.u64, 'objectid'), (
 t.char, 'name', BTRFS_INO_LOOKUP_PATH_MAX, t.readString, t.writeString), packed=True)
btrfs_ioctl_search_key = Structure((
 ...
 t.u64, 'unused2'), (
 t.u64, 'unused3'), (
 t.u64, 'unused4'), packed=True)
btrfs_ioctl_search_header = Structure((
 t.u64, 'transid'), (
 t.u64, 'objectid'), (
 t.u64, 'offset'), (
 t.u32, 'type'), (
 t.u32, 'len'), packed=True)
btrfs_ioctl_dev_info_args = Structure((
 t.u64, 'devid'), (
 t.u8, 'uuid', BTRFS_UUID_SIZE, bytes2uuid, uuid2bytes), (
 t.u64, 'bytes_used'), (
 t.u64, 'total_bytes'), (
 t.u64, 'unused', 379, t.readBuffer), (
 t.u8, 'path', BTRFS_DEVICE_PATH_NAME_MAX, t.readString, t.writeString), packed=True)
btrfs_ioctl_fs_info_args = Structure((
 t.u64, 'max_id'), (
 t.u64, 'num_devices'), (
 t.u8, 'fsid', BTRFS_FSID_SIZE, bytes2uuid, uuid2bytes), (
 t.u64, 'reserved', 124, t.readBuffer), packed=True)
BTRFS_SEARCH_ARGS_BUFSIZE = 4096 - btrfs_ioctl_search_key.size
btrfs_ioctl_search_args = Structure((
 btrfs_ioctl_search_key, 'key'), (
 t.char, 'buf', BTRFS_SEARCH_ARGS_BUFSIZE), packed=True)
btrfs_disk_key = Structure((
 t.le64, 'objectid'), (
 t.u8, 'type'), (
 t.le64, 'offset'), packed=True)
btrfs_timespec = Structure((
 t.le64, 'sec'), (
 t.le32, 'nsec'), packed=True)
btrfs_inode_item = Structure((
 ...
 btrfs_timespec, 'otime'), packed=True)
...
btrfs_ioctl_vol_args_v2 = Structure((
 ...
 t.char, 'name', BTRFS_SUBVOL_NAME_MAX + 1), packed=True)
btrfs_ioctl_timespec = Structure((
 t.u64, 'sec'), (
 t.u32, 'nsec'), packed=True)
btrfs_ioctl_received_subvol_args = Structure((
 t.char, 'uuid', BTRFS_UUID_SIZE), (
 t.u64, 'stransid'), (
 t.u64, 'rtransid'), (
 btrfs_ioctl_timespec, 'stime'), (
 btrfs_ioctl_timespec, 'rtime'), (
 t.u64, 'flags'), (
 t.u64, 'reserved', 16), packed=True)
BTRFS_IOCTL_MAGIC = 148
objectTypeKeys = {'zero': 0, 
...
class _Volume(object):
    ...
    def _addLink(self, dirTree, dirID, dirSeq, dirPath, name):
        ...
        assert (
         dirTree, dirID, dirSeq) not in self.links, (dirTree, dirID, dirSeq)
    ...
    def copy(self, path):
        ...
        with self._snapshot() as (source):
            with destDir as (dest):
                dest.SNAP_CREATE_V2(flags=BTRFS_SUBVOL_RDONLY, name=str(os.path.basename(path)), fd=source.fd)
...
class FileSystem(ioctl.Device):
            ...
            with open('/proc/self/mountinfo') as (mtab):
                for line in mtab:
                    ...
                    if dev not in self.devices:
                        logger.debug('%s device (%s) not in %s devices (%s)', path, dev, self.path, self.devices)
                        continue
    ...
    Key.first = Key(objectid=0, type=0, offset=0)
    Key.last = Key(objectid=BTRFS_LAST_FREE_OBJECTID, type=t.max_u32, offset=t.max_u64)
    Key.next = lambda key: FileSystem.Key(key.objectid, key.type, key.offset + 1)

    def _walkTree(self, treeid):
        key = FileSystem.Key.first
        while True:
            result = self.TREE_SEARCH(key=dict(tree_id=treeid, min_type=key.type, max_type=FileSystem.Key.last.type, min_objectid=key.objectid, max_objectid=FileSystem.Key.last.objectid, min_offset=key.offset, max_offset=FileSystem.Key.last.offset, min_transid=0, max_transid=t.max_u64, nr_items=4096))
    ...
    def _getRoots(self):
        for header, buf in self._walkTree(BTRFS_ROOT_TREE_OBJECTID):
            if header.type == objectTypeKeys['BTRFS_ROOT_BACKREF_KEY']:
                ...
                self.volumes[header.objectid]._addLink(header.offset, info.dirid, info.sequence, directory.name, name)
                ...
                if header.objectid >= BTRFS_FIRST_FREE_OBJECTID and header.objectid <= BTRFS_LAST_FREE_OBJECTID or header.objectid == BTRFS_FS_TREE_OBJECTID:
                    assert header.objectid not in self.volumes, header.objectid
                    self.volumes[header.objectid] = _Volume(self, header.objectid, header.offset, info)
        ...
        for header, buf in self._walkTree(BTRFS_QUOTA_TREE_OBJECTID):
            if header.type == objectTypeKeys['BTRFS_QGROUP_INFO_KEY']:
                ...
                try:
                    ...
                    if data.referenced < 0 or data.exclusive < 0 or data.referenced < data.exclusive:
                        raise _BtrfsError('Btrfs returned corrupt size of %s (%s exclusive) for %s' % (
