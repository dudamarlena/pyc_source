...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/flask_s3.py
# Compiled at: 2017-02-23 15:44:46
# Size of source mod 2**32: 18740 bytes
import hashlib, json, logging, os, re, gzip, warnings
try:
...
import boto3, boto3.exceptions
from botocore.exceptions import ClientError
from flask import current_app
from flask import url_for as flask_url_for
import six
logger = logging.getLogger('flask_s3')
header_mapping = {'cache-control':'CacheControl', 
...
DEFAULT_SETTINGS = {'FLASKS3_USE_HTTPS':True, 
 ...
 'FLASKS3_HEADERS':{},  'FLASKS3_FILEPATH_HEADERS':{},  'FLASKS3_ONLY_MODIFIED':False, 
 'FLASKS3_URL_STYLE':'host', 
 'FLASKS3_GZIP':False, 
 'FLASKS3_GZIP_ONLY_EXTS':[],  'FLASKS3_FORCE_MIMETYPE':False, 
 'FLASKS3_PREFIX':''}
__version__ = (0, 3, 3)
...
def _get_bucket_name(**values):
    ...
    if url_style == 'host':
        ...
        if get_setting('FLASKS3_CDN_DOMAIN', app):
            ...
            bucket_path = url_format.format(bucket_name=(get_setting('FLASKS3_BUCKET_NAME', app)),
...
def url_for(endpoint, **values):
    ...
    if app.config.get('TESTING', False):
        if not app.config.get('FLASKS3_OVERRIDE_TESTING', True):
...
def _gather_files(app, hidden, filepath_filter_regex=None):
    ...
    dirs = [
     (
      six.u(app.static_folder), app.static_url_path)]
    ...
    for static_folder, static_url_loc in dirs:
        ...
        for root, _, files in os.walk(static_folder):
            relative_folder = re.sub('^/', '', root.replace(static_folder, ''))
            files = [os.path.join(root, x) for x in files if hidden or x[0] != '.' if filepath_filter_regex == None or re.search(filepath_filter_regex, os.path.join(relative_folder, x))]
            if files:
...
def _write_files(s3, app, static_url_loc, static_folder, files, bucket, ex_keys=None, hashes=None):
    ...
    for file_path in files:
        ...
        full_key_name = _static_folder_path(static_url_loc, static_folder_rel, asset_loc)
        ...
        if app.config.get('FLASKS3_ONLY_MODIFIED', False):
            ...
            if hashes:
                if hashes.get(full_key_name, None) == file_hash:
                    exclude = True
        if ex_keys and full_key_name in ex_keys or exclude:
            ...
            with open(file_path, file_mode) as (fp):
                ...
                if per_file_should_gzip:
                    ...
                    z = gzip.GzipFile(os.path.basename(file_path), 'wb', 9, compressed)
                    z.write(fp.read())
                    z.close()
...
def _upload_files(s3, app, files_, bucket, hashes=None):
    ...
    for (static_folder, static_url), names in six.iteritems(files_):
        ...
        new_hashes.extend(_write_files(s3, app, static_upload_url, static_folder, names,
...
def get_setting(name, app=None):
    ...
    if app:
        return app.config.get(name, default_value)
    else:
        return default_value


def create_all(app, user=None, password=None, bucket_name=None, location=None, include_hidden=False, filepath_filter_regex=None, put_bucket_acl=True):
    """
    ...
    .. _bucket restrictions: http://docs.amazonwebservices.com/AmazonS3    /latest/dev/BucketRestrictions.html
    ...
    if not bucket_name:
        ...
        if get_setting('FLASKS3_ONLY_MODIFIED', app):
            ...
            try:
                s3.put_object(Bucket=bucket_name, Key='.file-hashes',
...
class FlaskS3(object):
    ...
    def init_app(self, app):
        ...
        if app.debug:
            if not get_setting('FLASKS3_DEBUG', app):
                app.config['FLASKS3_ACTIVE'] = False
        if get_setting('FLASKS3_ACTIVE', app):
            app.jinja_env.globals['url_for'] = url_for
        if get_setting('FLASKS3_USE_CACHE_CONTROL', app):
            if app.config.get('FLASKS3_CACHE_CONTROL'):
