...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/importd/__init__.py
# Compiled at: 2015-10-24 07:12:41
...
__docformat__ = 'html'
import copy, inspect, os, sys, traceback
from datetime import datetime
from getpass import getuser
from platform import python_version
import dj_database_url, django.core.urlresolvers, django
...
try:
    import werkzeug, django_extensions
except ImportError:
...
class SmartReturnMiddleware(object):
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        if isinstance(res, tuple):
            ...
            res = render_to_response(template_name, context, RequestContext(request))
        else:
...
def env(key, default='', factory=None):
    ...
    if factory != NotSet:
        ...
        if factory:
            if factory == bool:
                if isinstance(val, basestring) and val.lower() in ('no', 'false', 'off',
                                                                   '0'):
                    val = False
            val = factory(val)
    return val
...
def esettings(request):
    return {'esettings': global_esettings}
...
class D(object):
    ...
    def openenv(self, path=None):
        ...
        if path:
            if not os.path.isabs(path):
                path = os.path.realpath(os.path.join(os.path.dirname(os.path.realpath(inspect.stack()[(-1)][1])), path))
        else:
    ...
    def _fix_coffin_pre(self):
        try:
            ...
            from django.template.base import add_to_builtins, import_library, Origin, InvalidTemplateLibrary, builtins, get_library
    ...
    def _fix_coffin_post(self):
        try:
            ...
            django.template.loaders.app_directories.app_template_dirs = get_app_template_dirs('templates')
        else:
    ...
    def _configure_django(self, **kw):
        ...
        if settings.configured:
            return
        self.APP_DIR, app_filename = os.path.split(os.path.realpath(inspect.stack()[2][1]))
        ...
        if not kw.get('dont_configure', False):
            ...
            if 'db' in kw:
                if isinstance(kw['db'], basestring):
                    kw['DATABASES'] = {'default': dj_database_url.parse(kw.pop('db'))}
                else:
                    db = kw.pop('db')
            ...
            if kw.pop('SMART_RETURN', True):
                ...
                if 'MIDDLEWARE_CLASSES' not in kw:
                    kw['MIDDLEWARE_CLASSES'] = global_settings.MIDDLEWARE_CLASSES
                kw['MIDDLEWARE_CLASSES'] = list(kw['MIDDLEWARE_CLASSES'])
                kw['MIDDLEWARE_CLASSES'].append('importd.SmartReturnMiddleware')
            ...
            if admin_url:
                ...
                if 'django.contrib.sessions' not in installed:
                    ...
                    kw['MIDDLEWARE_CLASSES'] = list(kw['MIDDLEWARE_CLASSES'])
                    kw['MIDDLEWARE_CLASSES'].insert(last_position, 'django.contrib.sessions.middleware.SessionMiddleware')
                if 'django.contrib.admin' not in installed:
                    installed.append('django.contrib.admin')
                    kw['MIDDLEWARE_CLASSES'].append('django.contrib.auth.middleware.AuthenticationMiddleware')
                ...
                if 'debug_toolbar' not in installed and debug_toolbar:
                    ...
                    kw['MIDDLEWARE_CLASSES'].insert(1, 'debug_toolbar.middleware.DebugToolbarMiddleware')
                    kw['DEBUG_TOOLBAR_PANELS'] = ('debug_toolbar.panels.versions.VersionsPanel',
            ...
            if DJANGO_JINJA:
                ...
                kw['TEMPLATE_LOADERS'] += ('django_jinja.loaders.AppLoader', 'django_jinja.loaders.FileSystemLoader')
            if COFFIN:
                installed.append('coffin')
                kw['TEMPLATE_LOADERS'] = list(kw.get('TEMPLATE_LOADERS', []))
                kw['TEMPLATE_LOADERS'] += ('coffin.contrib.loader.AppLoader', 'coffin.contrib.loader.FileSystemLoader')
        ...
        for namespace, meta in self.blueprints.items():
            ...
            self.register_blueprint(bp, url_prefix=meta.get('url_prefix', namespace + '/'), namespace=namespace, app_name=meta.get('app_name', ''))
    ...
    def _apply_blueprint(self, bp):
        ...
        url = self.surl(bp.url_prefix, include(bp.patterns, namespace=bp.namespace, app_name=bp.app_name))
        urlpatterns = self.get_urlpatterns()
