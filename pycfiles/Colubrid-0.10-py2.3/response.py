...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/colubrid/response.py
...
class HttpResponse(object):
    ...
    def set_cookie(self, key, value='', max_age=None, expires=None, path='/', domain=None, secure=None):
        ...
        if not expires is None:
            ...
            if not expires is None:
                ...
                day = [
                 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'][expires.tm_wday]
                date = '%02d-%s-%s' % (now.tm_mday, month, str(now.tm_year)[-2:])
                d = '%s, %s %02d:%02d:%02d GMT' % (day, date, now.tm_hour, now.tm_min, now.tm_sec)
    ...
    def __call__(self, request):
        if not 'Content-Type' in self.headers:
            self.headers['Content-Type'] = 'text/html; charset=%s' % str(request.charset)
        headers = self.headers.get()
        if not self._cookies is None:
            for morsel in self._cookies.values():
                headers.append(('Set-Cookie', morsel.output(header='')))

        status = '%d %s' % (self.status, HTTP_STATUS_CODES.get(self.status, 'UNKNOWN'))
