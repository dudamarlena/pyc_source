...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/colubrid/request.py
# Compiled at: 2006-09-10 12:33:06
"""
    Colubrid Request Object
    =======================
"""
from __future__ import generators
from colubrid.utils import MultiDict, MergedMultiDict, FieldStorage, get_full_url
from colubrid.response import HttpResponse
import posixpath, cgi, email
...
class Request(object):
    ...
    def __init__(self, environ, start_response, charset='utf-8'):
        ...
        url = ('').join([quote(self.environ['SCRIPT_NAME']), quote(self.environ.get('PATH_INFO', ''))])
    ...
    def _load_post_data(self):
        ...
        if self.environ['REQUEST_METHOD'] == 'POST':
            ...
            if self.environ.get('CONTENT_TYPE', '').startswith('multipart'):
                lines = [
                 'Content-Type: %s' % self.environ.get('CONTENT_TYPE', '')]
                ...
                for sub in msg.get_payload():
                    ...
                    if 'filename' in name_dict:
                        ...
                        if 'Content-Type' in sub:
                            ...
                            content_type = None
                        s = FieldStorage(name_dict['name'], filename, content_type, payload)
                ...
                if not self.charset is None:
                    for (key, value) in d.iteritems():
                        d[key] = [ i.decode(self.charset, 'ignore') for i in value ]
        ...
        if not hasattr(self, '_get'):
            ...
            if not self.charset is None:
                for (key, value) in query.iteritems():
                    query[key] = [ i.decode(self.charset, 'ignore') for i in value ]
