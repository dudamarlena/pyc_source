...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/wsgistate/tests/test_wsgistate.py
# Compiled at: 2007-10-22 17:47:48
"""Unit tests for wsgistate."""
import unittest, StringIO, copy, os, time, urlparse
...
class TestWsgiState(unittest.TestCase):
    ...
    def test_db_set_getmany(self):
        ...
        testcache.set('test', 'test')
        testcache.set('test2', 'test2')
        self.assertEqual(sorted(testcache.get_many(('test', 'test2'))), [
         'test', 'test2'])
    ...
    def test_wsgimemoize_user_info_sc(self):
        ...
        testc = simple.SimpleCache()
        env = {'PATH_INFO': '/', 'REQUEST_METHOD': 'GET', 'wsgi.input': StringIO.StringIO('num=12121&str1=test&state=NV&Submit=Submit')}
    ...
    def test_wsgimemoize_allowed_methods(self):
        """Tests memoizing with HTTP method as part of the key with MemCached."""
        testc = simple.SimpleCache()
        env = {'PATH_INFO': '/', 'REQUEST_METHOD': 'POST', 'wsgi.input': StringIO.StringIO('num=12121&str1=test&state=NV&Submit=Submit')}
