...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/grnhse/harvest/api.py
# Compiled at: 2018-11-10 02:54:32
from datetime import date, datetime
import requests, six
from grnhse.exceptions import InvalidAPIVersion, InvalidAPICallError, HTTPError, EndpointNotFound
...
class Harvest(object):
    ...
    def __getattr__(self, key):
        ...
        if uris is not None:
            ...
            return HarvestObject(endpoint, self._api_key, self._base_url, uris.get('list'), uris.get('retrieve'), related=related)
        else:
...
class HarvestObject(SessionAuthMixin):
    ...
    def __getattr__(self, key):
        ...
        if uris is not None:
            if self._object_id is None:
                raise InvalidAPICallError(('Cannot query related object {} without selecting an object id first').format(endpoint))
            list_uri = uris['list'].format(rel_id=self._object_id) if uris.get('list') else None
            retrieve_uri = uris['retrieve'].format(rel_id=self._object_id) if uris.get('retrieve') else None
            return HarvestObject(endpoint, self._api_key, self._base_url, list_uri, retrieve_uri)
        else:
            raise EndpointNotFound(endpoint)
            return
    ...
    def _set_params(self, **params):
        if params:
            params = {k:strf_dt(v) if isinstance(v, (date, datetime)) else v for k, v in six.iteritems(params)}
