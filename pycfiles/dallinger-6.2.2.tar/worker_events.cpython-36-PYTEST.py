...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/Dallinger/Dallinger/dallinger/experiment_server/worker_events.py
...
def worker_function(event_type, assignment_id, participant_id, node_id=None, details=None):
    ...
    try:
        ...
        db.logger.debug('rq: Received Queue Length: %d (%s)', len(q), ', '.join(q.job_ids))
    ...
    if event_type == 'TrackingEvent':
        ...
        if not node:
            ...
            if participant_id:
                ...
                if assignment_id:
                    participants = models.Participant.query.filter_by(assignment_id=assignment_id).all()
                    if participants:
                        participant = max(participants, key=(attrgetter('creation_time')))
                        participant_id = participant.id
            if not participant:
                exp.log('Warning: No participant associated with this TrackingEvent notification.', key)
                return
            nodes = participant.nodes()
            if not nodes:
                exp.log('Warning: No node associated with this TrackingEvent notification.', key)
                return
    ...
    if assignment_id is not None:
        ...
        if participant_id is not None:
            ...
            raise ValueError('Error: worker_function needs either an assignment_id or a participant_id, they cannot both be None')
        participant_id = participant.id
        runner = runner_cls(participant, assignment_id, exp, db.session, _config(), datetime.now())
        runner()
        db.session.commit()
...
class WorkerEvent(object):
    key = '-----'
    supported_event_types = ('AssignmentAccepted', 'AssignmentAbandoned', 'AssignmentReassigned',
                             'AssignmentReturned', 'AssignmentSubmitted', 'BotAssignmentSubmitted',
                             'BotAssignmentRejected', 'NotificationMissing')
...
class AssignmentSubmitted(WorkerEvent):
    ...
    def award_bonus(self, bonus):
        ...
        self.participant.recruiter.reward_bonus(self.assignment_id, bonus, self.experiment.bonus_reason())
