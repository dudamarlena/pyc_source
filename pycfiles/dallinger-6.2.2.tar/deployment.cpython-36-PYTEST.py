...
# Size of source mod 2**32: 24498 bytes
from __future__ import unicode_literals
import builtins as @py_builtins, _pytest.assertion.rewrite as @pytest_ar, os, pkg_resources, re, redis, requests, shutil, six, sys, tempfile, threading, time, webbrowser
...
def _handle_launch_data(url, error, delay=INITIAL_DELAY, attempts=MAX_ATTEMPTS):
    for remaining_attempt in sorted((range(attempts)), reverse=True):
        ...
        try:
            ...
            error('Error parsing response from {}, check web dyno logs for details: {}'.format(url, launch_request.text))
            raise

        if launch_request.ok:
            return launch_data
        error('Error accessing {} ({}):\n{}'.format(url, launch_request.status_code, launch_request.text))
