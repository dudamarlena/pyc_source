...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/Dallinger/Dallinger/dallinger/bots.py
# Compiled at: 2020-04-27 20:27:30
# Size of source mod 2**32: 11685 bytes
"""Bots."""
import builtins as @py_builtins, _pytest.assertion.rewrite as @pytest_ar, json, logging, random, tempfile, uuid
...
CAPABILITY_MAP = {'phantomjs':webdriver.DesiredCapabilities.PHANTOMJS, 
 'firefox':webdriver.DesiredCapabilities.FIREFOX, 
 'chrome':webdriver.DesiredCapabilities.CHROME}

class BotBase(object):
    __doc__ = 'A base class for bots that works with the built-in demos.\n\n    This kind of bot uses Selenium to interact with the experiment\n    using a real browser.\n    '
    ...
    def __init__(self, URL, assignment_id='', worker_id='', participant_id='', hit_id=''):
        if not URL:
            ...
            worker_id = worker_id or query.get('worker_id', [''])[0]
    ...
    def driver(self):
        ...
        if driver_url:
            ...
            if capabilities is None:
                raise ValueError('Unsupported remote webdriver_type: {}'.format(driver_type))
    ...
    def sign_up(self):
        ...
        try:
            ...
            begin = WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable((By.CLASS_NAME, 'btn-primary')))
            begin.click()
            ...
            consent = WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable((By.ID, 'consent')))
            consent.click()
            logger.info('Clicked consent button.')
            participate = WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable((By.CLASS_NAME, 'btn-success')))
            participate.click()
    ...
    def sign_off(self):
        ...
        try:
            ...
            feedback = WebDriverWait(self.driver, 20).until(EC.presence_of_element_located((By.ID, 'submit-questionnaire')))
...
class HighPerformanceBotBase(BotBase):
    ...
    def sign_up(self):
        ...
        while 1:
            url = '{host}/participant/{self.worker_id}/{self.hit_id}/{self.assignment_id}/debug?fingerprint_hash={hash}&recruiter=bots:{bot_name}'.format(host=(self.host),
              self=self,
              hash=(uuid.uuid4().hex),
    ...
    def complete_experiment(self, status):
        ...
        while 1:
            url = '{host}/{status}?participant_id={participant_id}'.format(host=(self.host),
    ...
    def complete_questionnaire(self):
        ...
        """
        while 1:
            data = {'question':'questionnaire', 
             'number':1, 
             'response':json.dumps(self.question_responses)}
            url = '{host}/question/{self.participant_id}'.format(host=(self.host),
              self=self)
            try:
