...
# [GCC 8.4.0]
# Embedded file name: /Users/ramusus/workspace/manufacture/env/src/django-m2m-history/m2m_history/descriptors.py
# Compiled at: 2016-04-02 14:49:42
    ...
    class ManyToManyHistoryThroughManager(baseManagerClass):
        ...
        def _prepare_queryset(self, qs, only_pk=False, unique=True):
            ...
            if not only_pk:
                ...
                qs = getattr(super(ManyToManyHistoryThroughManager, self), method_name)().using(self.db).filter(pk__in=qs)
            if unique:
                qs = qs.distinct()
            return qs
        ...
        def get_queryset_through(self):
            qs = self.through._default_manager.using(self.db).filter(**{self.source_field_name: self._fk_val})
            return qs
        ...
        def were_between(self, time_from, time_to, **kwargs):
            ...
            qs = self.get_queryset_through().filter(Q(time_from=None, time_to=None) | Q(time_from=None, time_to__gte=time_to) | Q(time_from__lte=time_from, time_to=None) | Q(time_from__gte=time_from, time_to__lte=time_to) | Q(time_from__lte=time_from, time_to__gte=time_to) | Q(time_from__lt=time_to, time_to__gt=time_from))
            return self._prepare_queryset(qs, **kwargs)
        ...
        def were_at(self, time, **kwargs):
            qs = self.get_queryset_through().filter(Q(time_from=None, time_to=None) | Q(time_from=None, time_to__gt=time) | Q(time_from__lte=time, time_to=None) | Q(time_from__lte=time, time_to__gt=time))
            return self._prepare_queryset(qs, **kwargs)
        ...
        def send_signal(self, source_field_name, action, ids):
            if self.reverse or source_field_name == self.source_field_name:
                signals.m2m_changed.send(sender=self.through, action=action, instance=self.instance, reverse=self.reverse, model=self.model, pk_set=ids, using=self.db)
                m2m_history_changed.send(sender=self.through, action=action, instance=self.instance, reverse=self.reverse, model=self.model, pk_set=ids, using=self.db, field_name=self.prefetch_cache_name, time=self.get_time())
        ...
        def _add_items(self, source_field_name, target_field_name, *objs):
            if objs:
                ...
                current_ids = self.through._default_manager.using(self.db).values_list(target_field_name, flat=True).filter(**{source_field_name: self._fk_val, 
                   '%s__in' % target_field_name: new_ids, 
                   'time_to': None})
                new_ids = new_ids.difference(set(current_ids))
                self.send_signal(source_field_name, 'pre_add', new_ids)
                self.through._default_manager.using(self.db).bulk_create([ self.through(**{'%s_id' % source_field_name: self._fk_val, '%s_id' % target_field_name: obj_id, 'time_from': self.get_time()}) for obj_id in new_ids
                                                                         ])
        ...
        def _remove_items(self, source_field_name, target_field_name, *objs):
            if objs:
                ...
                qs = self.through._default_manager.using(self.db).filter(**{source_field_name: self._fk_val, 
                   'time_to': None, 
...
class ReverseManyRelatedObjectsHistoryDescriptor(ReverseManyRelatedObjectsDescriptor):
    ...
    def related_manager_cls(self):
        ...
        """
        return create_many_related_history_manager(self.field.rel.to._default_manager.__class__, self.field.rel)
...
class ManyRelatedObjectsHistoryDescriptor(ManyRelatedObjectsDescriptor):
    ...
    def related_manager_cls(self):
        ...
        Difference from super method is return our own manager inherited from the build-in
        """
        return create_many_related_history_manager(self.related.model._default_manager.__class__, self.related.field.rel)
