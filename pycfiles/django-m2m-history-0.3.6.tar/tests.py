...
# [GCC 8.4.0]
# Embedded file name: /Users/ramusus/workspace/manufacture/env/src/django-m2m-history/m2m_history/tests.py
...
class ManyToManyHistoryTest(TransactionTestCase):
    ...
    def test_m2m_fields_and_methods(self):
        self.assertListEqual(sorted([ field.name for field in Article._meta.get_field('publications').rel.through._meta.local_fields ]), sorted(['id', 'time_from', 'time_to', 'article', 'publication']))
    ...
    def test_m2m_history_features(self):
        ...
        self.assertListEqual(sorted(list(article.publications.were_at(state_time4, only_pk=True))), sorted(list(map(lambda o: o.pk, article.publications.were_at(state_time4)))))
        ...
        article.publications_no_versions = [
         p1, p2]
        self.assertEqual(ManyToManyHistoryVersion.objects.count(), 6)
    ...
    def test_m2m_default_features(self):
        ...
        self.assertPublicationsEqual(Article.objects.filter(publications__title__startswith='Science').distinct(), [a2])
        ...
        self.assertPublicationsEqual(Publication.objects.filter(article__headline__startswith='NASA'), [p4, p3, p2, p1])
