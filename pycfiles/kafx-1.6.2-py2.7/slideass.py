...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\kafx\slideass.py
# Compiled at: 2012-06-04 22:24:44
"""
...
def Url2FileName(url):
    ...
    fname = os.path.join(DOWNLOAD_FOLDER, url.rsplit('/', 1)[(-1)])
...
def DownloadFromPicasa(gdc, album, username):
    ...
    photos = gdc.GetFeed('/data/feed/api/user/%s/albumid/%s?kind=photo' % (
...
def ConvertFolder():
    ...
    DOWNLOAD_FOLDER = raw_input('Folder: ')
    for entry in os.listdir(DOWNLOAD_FOLDER):
        if os.path.splitext(entry)[(-1)].lower() in ('.gif', '.jpg', '.jpeg'):
            fname = os.path.join(DOWNLOAD_FOLDER, entry)
            to = Url2FileName(entry)
            print 'Converting %s to %s' % (fname, to)
            im = Image.open(fname)
            im.save(to, 'PNG')
