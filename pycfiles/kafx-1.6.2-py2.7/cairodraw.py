...
def drawRoundedRectangle(cr, x, y, width, height, radius):
    ...
    cr.arc(x + width - radius, y + radius, radius, -90.0 * math.pi / 180.0, 0.0 * math.pi / 180.0)
    cr.line_to(x + width, y + height - radius)
    cr.arc(x + width - radius, y + height - radius, radius, 0.0 * math.pi / 180.0, 90.0 * math.pi / 180.0)
    cr.line_to(x + radius, y + height)
    cr.arc(x + radius, y + height - radius, radius, 90.0 * math.pi / 180.0, 180.0 * math.pi / 180.0)
    cr.line_to(x, y + radius)
    cr.arc(x + radius, y + radius, radius, 180.0 * math.pi / 180.0, 270.0 * math.pi / 180.0)
...
class ColorBox(gtk.VBox):
    ...
    def Pack(self, child, expand=True, fill=True, padding=0, onover=lambda *a: None, onout=lambda *a: None, onclick=lambda *a: None):
        evbox = gtk.EventBox()
        evbox.add(child)
...
class CairoWindow(gtk.Window):
    ...
    def __init__(self, w, h, script=None):
        ...
        for st, tt, cb in zip((
         ...
         gtk.STOCK_YES,
         gtk.STOCK_REFRESH,
         gtk.STOCK_QUIT), ('Open an existing CairoScript', 'Save this CairoScript',
                           'Write CairoScript to PNG/SVG images', 'Refresh the DrawingArea',
    ...
    def __updateGlobals(self, *args):
        self.__glbls.SetHelp((', ').join(g for g in CAIRODRAW_LIBRARY_GLOBALS if not g == '__builtins__'))

    def __openScript(self, *args):
        fc = gtk.FileChooserDialog('Select a CairoScript file...', self, buttons=(
    ...
    def __saveScript(self, *args):
        fc = gtk.FileChooserDialog('Save File As...', self, gtk.FILE_CHOOSER_ACTION_SAVE, (
    ...
    def __savePNG(self, *args):
        fc = gtk.FileChooserDialog('Save PNG/SVG Images As...', self, gtk.FILE_CHOOSER_ACTION_SAVE, (
         gtk.STOCK_CANCEL, 0, gtk.STOCK_SAVE, 1))
        if fc.run():
