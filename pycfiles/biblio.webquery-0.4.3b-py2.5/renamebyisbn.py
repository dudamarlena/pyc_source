...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/biblio/webquery/scripts/renamebyisbn.py
...
def parse_args():
    ...
    description = 'Extract an ISBN from a file name, look up the associated bibliographic information in a webservice and rename the file appropriately.'
    epilog = 'ISBNs are extracted from filenames by pure heuristics - obviously not all forms will be found. The new name is generated first before the various processing options are applied. In order, characters are stripped from the name, excess whitespace is collapsed and then the case conversion is applied. The file extension, if any, is removed before renaming and re-applied afterwards. We suggest you try a dryrun before renaming any files.'
    optparser = OptionParser(usage=usage, version=version, epilog=epilog, description=description)
    add_shared_options(optparser)
    (
     optparser.add_option('--case', '-c', dest='case', help='Case conversion of the new file name. Choices are %s.The default is %s. ' % (
      (', ').join(CASE_CHOICES), CASE_CHOICES[0]), choices=CASE_CHOICES, default=CASE_CHOICES[0]),)
    optparser.add_option('--leave_whitespace', action='store_true', dest='leave_whitespace', help='Leave excess whitespace. By default, consecutive spaces in names are compacted', default=False)
    optparser.add_option('--replace_whitespace', dest='replace_whitespace', help='Replace whitespace in the new name with this string.', default='')
    optparser.add_option('--strip_chars', dest='strip_chars', help="Remove these characters from the new name. By default this are '%s'." % DEF_STRIP_CHARS, default=DEF_STRIP_CHARS)
    optparser.add_option('--overwrite', action='store_true', dest='overwrite', help='Overwrite existing files.', default=False)
    optparser.add_option('--dryrun', action='store_true', dest='dryrun', help='Check function and without renaming files.', default=False)
    optparser.add_option('--template', dest='template', help="The form to use for renaming the file. The fields recognised are auth (primary authors family name), title (full title of the book), short_title (abbreviated title), isbn, year (year of publication). The default is '%s'." % DEF_NAME_FMT, default=DEF_NAME_FMT)
    optparser.add_option('--unknown', dest='unknown', help='Use this string if value is undefined.', default='unknown')
...
def generate_new_name(bibrec, options):
    ...
    return options.template % {'auth': auth_str, 
...
def main():
    ...
    try:
        ...
        for fpath in fpath_list:
            ...
            if isbn:
                try:
                    bibrec_list = webqry.query_bibdata_by_isbn(isbn, format='bibrecord')
                    if bibrec_list:
