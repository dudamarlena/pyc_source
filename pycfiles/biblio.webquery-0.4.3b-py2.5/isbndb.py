...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/biblio/webquery/isbndb.py
...
class IsbndbQuery(BaseKeyedWebQuery):
    ...
    def __init__(self, key, timeout=5.0, limits=None):
        """
                ...
                """
        BaseKeyedWebQuery.__init__(self, root_url=ISBNDB_ROOTURL, key=key, timeout=timeout, limits=limits)
    ...
    def query_service(self, index, value, results):
        ...
        sub_url = 'index1=%(indx)s&value1=%(val)s' % {'indx': index, 
           'val': value}
        if results:
...
def isbndb_xml_to_bibrecords(xml_txt):
    ...
    assert_or_raise(root.tag == 'ISBNdb', errors.ParseError, "ISBNdb document root should be 'ISBNdb', not '%s'" % root.tag)
    assert_or_raise(len(root) == 1, errors.ParseError, 'ISBNdb document root has wrong number of children (%s)' % len(root))
    booklist_elem = root[0]
    assert_or_raise(booklist_elem.tag == 'BookList', errors.ParseError, "ISBNdb document should contain 'BookList', not '%s'" % booklist_elem.tag)
    bibrecs = []
    for bdata in booklist_elem.findall('BookData'):
        newrec = BibRecord()
        newrec.pubtype = 'book'
        newrec.title = (bdata.findtext('TitleLong') or '').strip()
        newrec.abstract = (bdata.findtext('Summary') or '').strip()
        newrec.note = (bdata.findtext('Notes') or '').strip()
        newrec.keywords = [ e.text.strip() for e in bdata.findall('Subject') ]
        authors = []
        author_elem = bdata.find('Authors')
        if author_elem:
            author_strs = [ e.text.strip() for e in author_elem.findall('Person') ]
        ...
        if pub_elem:
            (newrec.publisher, newrec.city, newrec.year) = parse_publisher(pub_elem.text)
