...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/birdsuite/compare_calls.py
# Compiled at: 2010-07-13 12:32:48
"""usage %prog [options]

Compare two sets of call and confidence files, and report the amount of agreement."""
from __future__ import division
import optparse, sys, birdseed.call_confidence_parser
from mpgutils import utils
lstRequiredOptions = [
 'calls_a', 'confs_a', 'calls_b', 'confs_b']

def main(argv=None):
    if argv is None:
        argv = sys.argv
    parser = optparse.OptionParser(usage=__doc__)
    parser.add_option('--calls_a', help='(Required) First calls file.')
    parser.add_option('--confs_a', help='(Required) Confidences file that is the partner of first calls file.')
    parser.add_option('--calls_b', help='(Required) Second calls file.')
    parser.add_option('--confs_b', help='(Required) Confidences file that is the partner of second calls file.')
    parser.add_option('--snps', help='Only consider SNPs listed in this file.')
    parser.add_option('--threshold', type='float', default='0.1', help='Call with confidence >= this value is not confident.  Default: %default')
    parser.add_option('--changed_snps', help='Write a list of SNPs with differences to this file.')
    parser.add_option('--conf_significance', type='float', default='0.01', help='Confidences that differ by less than this are considered the same.  Default: %default')
    ...
    if len(lstArgs) > 1:
        ...
        parser.print_help()
        return 1
    calls1 = call_confidence_parser.CallsConfidencesParser(dctOptions.calls_a, dctOptions.confs_a, None).iterateCallsAndConfidences()
    calls2 = call_confidence_parser.CallsConfidencesParser(dctOptions.calls_b, dctOptions.confs_b, None).iterateCallsAndConfidences()
    if dctOptions.snps:
        stSNPs = set([ lstFields[0] for lstFields in utils.iterateWhitespaceDelimitedFile(dctOptions.snps, iNumFieldsExpected=1)
                     ])
    else:
        stSNPs = None
        ...
        for i in xrange(len(lstCalls1)):
            ...
            if not b1Confident and not b2Confident:
                ...
                if not b1Confident or not b2Confident:
                    ...
                    iAgree += 1
            elif strCall1 == '-1' and lstConfs1[i] == 1 or strCall2 == '-1' and lstConfs2[i] == 1:
                iOneLBZapped += 1
