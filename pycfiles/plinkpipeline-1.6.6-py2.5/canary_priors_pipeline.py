...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/birdsuitepriors/canary/canary_priors_pipeline.py
...
@author: nemesh
"""
import optparse, sys, mpgutils.utils as utils, os, birdsuite.cn_annotate_allele_summary, birdsuite.cn_locus_summarize, birdsuite.cn_probeset_summarize, birdsuitepriors.canary.prep_clusters
from mpgutils import mergeMatrixes
from mpgutils.RUtils import RscriptToPython

class EnvironmentSpec(object):

    def __init__(self, intensityFile, genotypeFile, probe_locus, tempDir, basename, outputFile, force_cn_summarization, cnpDefsFile, missingValueLabel, normalizationProbes, mapFile, offSetFile, minNumSamples, popFile, famFile, mapFileOut, bVerbose):
...
def alignGenotypesIntensityData(envSpec):
    ...
    methodName = 'align'
    dctArguments = {'genotypesFileIn': envSpec.genotypeFile(), 'intensityFileIn': envSpec.cnSummarizeOut(), 
...
def QCAlignedGenotypeIntensityData(envSpec):
    ...
    lstLibraries = [
     'broadgap.birdsuitepriors']
    methodName = 'reduceMap'
    dctArguments = {'genotypesFileIn': genotypes, 'intensityFileIn': intensity, 
...
def splitCanaryData(envSpec):
    ...
    if not os.path.exists(canaryTestDir):
        os.makedirs(canaryTestDir)
    lstLibraries = [
     'broadgap.birdsuitepriors']
    methodName = 'splitTrainingData'
    dctArguments = {'trainingData': envSpec.intensitiesAlignedQCed(), 'plateMap': envSpec.mapFile(), 
       'outDir': canaryTestDir, 
       'suffix': '.probeset_summary', 
       'minNumSamples': envSpec.minNumSamples()}
    RscriptToPython.callRscript(lstLibraries, methodName, dctArguments, captureOutput=False, bVerbose=True)
    dctArguments = {'trainingData': envSpec.genotypesAlignedQCed(), 'plateMap': envSpec.mapFile(), 
       'outDir': canaryTestDir, 
...
def fill_clusters_per_plate(plateName, envSpec):
    ...
    dctArguments = {'sourceFile': prepClusterSource, 'outputFile': outPriors}
    RscriptToPython.callRscript(lstLibraries, methodName, dctArguments, captureOutput=False, bVerbose=True)
...
def mergePerPlatePriors(envSpec):
    ...
    dctArguments = {'fileDirectory': envSpec.canaryTestDir(), 'suffix': '.priors', 
...
def runCanarySingle(inFile, envSpec):
    ...
    dctArguments = {'profileList': envSpec.canaryTestDir() + '/' + inFile, 'priorsFile': envSpec.priorsFileWithoutQC(), 
...
def mergeCanaryResults(fileType, envSpec):
    ...
    for f in files:
        if f.endswith(fileType):
            lstFiles.append(envSpec.canaryTestDir() + '/' + f)
...
def plotCanary(envSpec):
    ...
    dctArguments = {'intensityDataFile': intensityDataFile, 'assignmentDataFile': assignmentDataFile, 
...
def qcPriors(envSpec):
    ...
    dctArguments = {'referenceIntensityFile': referenceIntensityFile, 'referenceGenotypesFile': referenceGenotypesFile, 
...
def populationSpecificPriors(envSpec):
    ...
    dctArguments = {'popFile': popFile, 'famFile': famFile, 
...
def cleanupPriorsMap(envSpec):
    ...
    dctArguments = {'priorsFile': priorsFile, 'originalMapFile': originalMapFile, 
    ...
    if envSpec.offSetFile() is not None:
        ...
        dctArguments = {'priorsFile': priorsFile, 'inOffsetFile': offSetFile, 
...
def main(argv=None):
    ...
    parser.add_option('--normalization-probes', dest='normalizationProbes', help='List of probes for normalizing each sample.  For each sample, find the median of \nthese probes and divide the output intensity by this value.')
    parser.add_option('--mapFile', default=None, dest='mapFile', help='A map file that has the sample and plate information.  \nUsed to calculate per-plate priors which are then averaged together. \nNo header, tab separated file.\n                      \n                      Format:\n                      \n                      CEL_A    SAMPLE_A    PLATE_FOO\n                      CEL_B    SAMPLE_B    PLATE_FOO\n                      ')
    parser.add_option('--mapFileOut', default=None, dest='mapFileOut', help='A matching file for the output priors file.  \nThis is the original --mapFile entry, subselected for the CNVs that passed QC on this platform.\n\nSuggested naming scheme for mapFile is as follows:\nchipType + "." genome build + "." + canaryMapName + ".cnv_defs"\nexample:\nGenomeWideSNP_6.hg18.HM3.cnv_defs\nGenomeWideSNP_6.hg18.cnv_defs\n(where canaryMapName is the source for CNV map, such as HM3, HM2, Conrad, 1KG)\n')
    parser.add_option('--offSetFile', dest='offSetFile', default=None, help='(OPTIONAL) A list of CNVs that have offsets.  Canary works in a 0-4 CN workspace, so if a CNV has higher CN than that [example: CN2-6], you can "add" to the integer copy number call post canary using this file.  In this example, you would shift the CN by 2. File format has a header, and is tab separated:\n                      \n                      cnp_id  offset\n                      \n                      CNVR1.6   0\n                      \n                      CNVR17.1  1\n                      ')
    parser.add_option('--popFile', dest='popFile', help='Links samples to populations.  Used to make population specific priors.\nFile format is 2 columns tab separated with no header:\n                      \n                      GIH NA20847\n                      \n                      YRI NA18488\n                      \n                      YRI NA18519\n                      ')
    parser.add_option('--famFile', dest='famFile', help='Links samples to their pedigree info.  Used to make population specific priors.\nFile format is at least 4 columns tab separated with no header.  This is a plink standard format.\n                      \n                      NA20847 NA20847 0 0 2 -9\n                      \n                      Y001 NA18488 0 0 2 -9\n                      \n                      Y014 NA18519 0 0 1 -9\n                      ')
    parser.add_option('--minNumSamples', dest='minNumSamples', default=20, help='The minimum number of samples on a chemistry plate to consider it for priors.\nDefaults to %default.')
    parser.add_option('--tempDir', dest='tempDir', default=None, help='A temp directory to put intermediate files in.')
    parser.add_option('--first_step', type='int', default=1, help='What step to start with in birdseed priors generation process.  Default: %default')
    parser.add_option('--last_step', type='int', default=len(lstSteps), help='What step to end with in birdseed priors generation process (1-based, inclusive).  Default: %default')
    parser.add_option('--verbose', dest='bVerbose', default=False, action='store_true', help='Run programs in verbose mode whenever possible')
    lstRequiredOptions = [
     'intensityFile', 'genotypeFile', 'probe_locus', 'tempDir', 'basename', 'outputFile', 'cnpDefsFile', 'mapFileOut', 'popFile']
    ...
    envSpec = EnvironmentSpec(dctOptions.intensityFile, dctOptions.genotypeFile, dctOptions.probe_locus, tempDir, dctOptions.basename, dctOptions.outputFile, dctOptions.force_cn_summarization, dctOptions.cnpDefsFile, dctOptions.strMissingValueLabel, dctOptions.normalizationProbes, dctOptions.mapFile, dctOptions.offSetFile, dctOptions.minNumSamples, dctOptions.popFile, dctOptions.famFile, dctOptions.mapFileOut, dctOptions.bVerbose)
