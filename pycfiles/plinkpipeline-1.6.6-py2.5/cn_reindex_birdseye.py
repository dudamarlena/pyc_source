...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/birdsuite/cn_reindex_birdseye.py
...
import copy, fileinput, math, optparse, os, sys, subprocess, shutil
...
def main(argv=None):
    ...
    parser.add_option('--input_birdseye_calls', help='(Optional) The matrix of birdseye calls (often produced by post_birdseye --birdseye_calls). ')
    parser.add_option('-o', '--output', help='(Required) Output file. A reindexed birdseye file, with the samples in the same index order as the canary calls.')
    lstRequiredOptions = [
     'canary_calls', 'input_birdseye_calls', 'output']
