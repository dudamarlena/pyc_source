...
# [GCC 8.4.0]
# Embedded file name: c:\git\pydicom-clean\source\build\lib\dicom\values.py
# Compiled at: 2017-01-26 21:09:39
...
def convert_ATvalue(byte_string, is_little_endian, struct_format=None):
    ...
    if length % 4 != 0:
        logger.warn("Expected length to be multiple of 4 for VR 'AT', got length %d", length)
    return MultiValue(Tag, [convert_tag(byte_string, is_little_endian, offset=x) for x in range(0, length, 4)])
...
def convert_SQ(byte_string, is_implicit_VR, is_little_endian, encoding=default_encoding, offset=0):
    """Convert a sequence that has been read as bytes but not yet parsed."""
    fp = BytesIO(byte_string)
    seq = read_sequence(fp, is_implicit_VR, is_little_endian, len(byte_string), encoding, offset)
    return seq
...
def convert_UI(byte_string, is_little_endian, struct_format=None):
    ...
    if in_py3:
        byte_string = byte_string.decode(default_encoding)
    if byte_string and byte_string.endswith('\x00'):
        byte_string = byte_string[:-1]
    ...
    if VR == 'PN':
        ...
        if VR in text_VRs:
            ...
            if VR != 'SQ':
                ...
                value = convert_SQ(byte_string, is_implicit_VR, is_little_endian, encoding, raw_data_element.value_tell)
    return value
