...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/loot.py
# Compiled at: 2016-08-10 08:24:31
...
@command()
@option('--user', '-u', help="Specify github user' which you want to                                     check or download")
    ...
    if not user:
        ...
        for r in json_res:
            ...
            if len(file_result) > 0:
                single_result = {'id': id,  'description': description, 
                 'files': file_result}
            else:
                continue
...
def clone(gist_id, dir):
    ...
    for file in files:
        ...
        gists.append({'filename': filename, 
         'raw_url': raw_url})
