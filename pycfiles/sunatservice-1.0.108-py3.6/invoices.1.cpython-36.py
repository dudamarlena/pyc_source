...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sunatservice/invoices.1.py
# Compiled at: 2020-01-17 09:20:00
# Size of source mod 2**32: 23925 bytes
...
class Invoices:
    ...
    def fillDocument(self, XMLpath, fileName, data):
        ...
        Note.text = self.precioALiteral(data['totalVenta'])
        ...
        for AccountingCustomerParty in InvoiceNode.findall(self.getInvoiceNameSpace('cac') + 'AccountingCustomerParty'):
            ...
            if float(data['tributo']['IGV']['total_venta']) > 0:
                ...
                TaxableAmount.text = self.getGlobalTaxableAmountIGV(data['tributo'])
                TaxableAmount.set('currencyID', data['tipoMoneda'])
                TaxSubtotal.append(TaxableAmount)
                TaxAmount = etree.Element(self.getInvoiceNameSpace('cbc') + 'TaxAmount')
                TaxAmount.text = data['tributo']['IGV']['sumatoria']
                TaxAmount.set('currencyID', data['tipoMoneda'])
