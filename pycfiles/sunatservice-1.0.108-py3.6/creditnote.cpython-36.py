...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sunatservice/creditnote.py
# Compiled at: 2020-01-17 09:20:02
# Size of source mod 2**32: 25780 bytes
...
class Creditnote:
    ...
    def fillDocument(self, XMLpath, fileName, data):
        tree = etree.parse(XMLpath + '/XMLdocuments/1_unfilled/UNFILLED-credit-note.XML')
        ...
        CreditNoteNode = tree.getroot()
        documentID = str(data['serie']) + '-' + str(data['numero'])
        ID = CreditNoteNode.find(self.getInvoiceNameSpace('cbc') + 'ID')
        ID.text = documentID
        IssueDate = CreditNoteNode.find(self.getInvoiceNameSpace('cbc') + 'IssueDate')
        IssueDate.text = data['fechaEmision']
        IssueTime = CreditNoteNode.find(self.getInvoiceNameSpace('cbc') + 'IssueTime')
        IssueTime.text = data['horaEmision']
        Note = CreditNoteNode.find(self.getInvoiceNameSpace('cbc') + 'Note')
        Note.text = self.precioALiteral(data['totalVentaGravada'])
        Note.set('languageLocaleID', '1000')
        DocumentCurrencyCode = CreditNoteNode.find(self.getInvoiceNameSpace('cbc') + 'DocumentCurrencyCode')
        DocumentCurrencyCode.text = data['tipoMoneda']
        for DiscrepancyResponse in CreditNoteNode.findall(self.getInvoiceNameSpace('cac') + 'DiscrepancyResponse'):
        ...
        for BillingReference in CreditNoteNode.findall(self.getInvoiceNameSpace('cac') + 'BillingReference'):
        ...
        for Signature in CreditNoteNode.iter(self.getInvoiceNameSpace('cac') + 'Signature'):
        ...
        for AccountingSupplierParty in CreditNoteNode.findall(self.getInvoiceNameSpace('cac') + 'AccountingSupplierParty'):
        ...
        for AccountingCustomerParty in CreditNoteNode.findall(self.getInvoiceNameSpace('cac') + 'AccountingCustomerParty'):
        ...
        for tributo_name in tributos:
            ...
            for tributo_item_percent_differenced in tributo:
                ...
                if tagTaxTotalAdded == False:
                    CreditNoteNode.append(TaxTotal)
        ...
        CreditNoteNode.append(LegalMonetaryTotal)
        index = 0
        items = data['items']
        for item in items:
            CreditNoteLine = etree.Element(self.getInvoiceNameSpace('cac') + 'CreditNoteLine')
            ID = etree.Element(self.getInvoiceNameSpace('cbc') + 'ID')
            ID.text = str(index + 1)
            CreditNoteLine.append(ID)
            CreditedQuantity = etree.Element(self.getInvoiceNameSpace('cbc') + 'CreditedQuantity')
            CreditedQuantity.text = item['cantidad']
            CreditedQuantity.set('unitCode', item['medidaCantidad'])
            CreditedQuantity.set('unitCodeListID', 'UN/ECE rec 20')
            CreditedQuantity.set('unitCodeListAgencyName', 'United Nations Economic Commission for Europe')
            CreditNoteLine.append(CreditedQuantity)
            ...
            CreditNoteLine.append(LineExtensionAmount)
            ...
            CreditNoteLine.append(PricingReference)
            ...
            for tributo in tributos:
                ...
                TaxTotal.append(TaxSubtotal)
                if tagTaxTotalAdded == False:
                    CreditNoteLine.append(TaxTotal)
                    tagTaxTotalAdded = True
            ...
            CreditNoteLine.append(Price)
            CreditNoteNode.append(CreditNoteLine)
            index = index + 1
