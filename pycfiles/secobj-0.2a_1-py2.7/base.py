...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/secobj/provider/base.py
# Compiled at: 2012-08-27 10:07:54
import inspect, json, re, weakref
...
NAMEDRULES = re.compile('^(?P<section>[a-zA-Z0-9_]+(?:\\.[a-zA-Z0-9_]+)*)' + '#(?P<option>[a-zA-Z0-9_]+)$')
...
class SecurityProvider(object):
    ...
    def _getacl(self, resource):
        if not self.hasaccess(resource):
            raise error(SecurityError, self.log, _("Can't get acl on resource, which is not security controlled"))
        rules = list()
        name = ''
        if inspect.ismethod(resource):
            name = ('{modulename}.{classname}.{methodname}').format(modulename=resource.im_class.__module__, classname=resource.im_class.__name__, methodname=resource.__name__)
            ...
            if inspect.isfunction(resource):
                ...
                if hasattr(resource, '__acl_class__'):
                    name = ('{modulename}.{classname}.{methodname}').format(modulename=resource.__acl_class__.__module__, classname=resource.__acl_class__.__name__, methodname=resource.__name__)
                    rules.extend(resource.__acl_class__.__acl__)
                else:
                    name = ('{modulename}.{methodname}').format(modulename=resource.__module__, methodname=resource.__name__)
            elif inspect.isclass(resource):
                name = ('{modulename}.{classname}').format(modulename=resource.__module__, classname=resource.__name__)
                rules.extend(resource.__acl__)
            elif hasattr(resource, '__class__'):
                name = ('{modulename}.{classname}').format(modulename=resource.__class__.__module__, classname=resource.__class__.__name__)
    ...
    def _getnamedacl(self, name):
        ...
        if m is None:
            raise error(ValueError, self.log, _(('Invalid name for named rule: {name}').format(name=name)))
        section = 'rules:' + m.group('section')
        option = m.group('option')
        if not self.config.has_option(section, option):
            self.log.warning(_("Named rule '{option}' doesn't exist in section '{section}'").format(section=section, option=option))
            return tuple()
        else:
            text = self.config.get(section, option)
            if not text:
                self.log.warning(_("Named rule '{option}' in section '{section}' has no value").format(section=section, option=option))
                return tuple()
    ...
    def _getowner(self, resource):
        if not self.hasaccess(resource):
            raise error(SecurityError, self.log, _("Can't get owner on resource, which is not security controlled"))
        if inspect.ismethod(resource):
        ...
        if inspect.isfunction(resource):
            if hasattr(resource, '__acl_class__'):
                raise error(SecurityError, self.log, _("Can't get owner on unbound method function. Use method instead"))
    ...
    def _setowner(self, resource, principal=SYSTEM):
        if not isinstance(principal, Principal):
            ...
            raise (self.hasaccess(resource) or error)(SecurityError, self.log, _("Can't set owner on resource, which is not security controlled"))
        if inspect.ismethod(resource):
    ...
    def _getpermission(self, resource, default=None):
        if not self.hasaccess(resource):
            raise error(SecurityError, self.log, _("Can't set permission on resource, which is not security controlled"))
    ...
    def _getsubject(self, name):
        try:
            ...
            raise error(UnknownPrincipalError, self.log, _('Unknown subject: {name}').format(name=name))

    def _getprincipal(self, name):
        try:
            return Principal.get(name)
        except KeyError:
            raise error(UnknownPrincipalError, self.log, _('Unknown principal: {name}').format(name=name))
    ...
    def _runas(self, func, user, *args, **kwargs):
        ...
        if olduser != user:
            try:
                self.log.info(_('Starting run as for {newuser} by {olduser}').format(newuser=user.name, olduser=olduser.name))
                self._setcurrentuser(user)
                return func(*args, **kwargs)
            finally:
                self._setcurrentuser(olduser)
                self.log.info(_('Finished run as for {newuser} by {olduser}').format(newuser=user.name, olduser=olduser.name))
    ...
    def _check_getowner(self, resource):
        self.checkaccess(resource, permission='secobj.getowner', namedacl='secobj.provider#getowner')

    def _check_setowner(self, resource, principal):
        self.checkaccess(resource, permission='secobj.setowner', namedacl='secobj.provider#setowner')
    ...
    def _check_setcurrentuser(self, user):
        self.checkaccess(self, permission='secobj.runas', namedacl='secobj.provider#runas')
    ...
    def _checkrules(self, rules, resourcename, resource, args=None, kwargs=None):
        ...
        for rule in rules:
            if not rule.match(user, resource, args, kwargs):
                continue
            self.log.debug(_('Applied rule for {user} on {resource}: {rule}').format(user=user.name, resource=resourcename, rule=str(rule)))
            if rule.denied(user, resource):
                raise error(AccessDeniedError, self.log, _('Access denied for {user} on {resource}').format(user=user.name, resource=resourcename))
            else:
                if rule.allowed(user, resource):
                    return rule.getpermissions_for(resource)
                msg = _('Rule neither allowed nor denied access for {user} on {resource}').format(user=user.name, resource=resourcename)
                self.log.critical(msg)
                self.log.critical(_('Failed rule: {rule}').format(rule=str(rule)))
                raise SecurityError, msg

        raise error(NoMatchingRuleError, self.log, _('No rule matched for {name} on {resource}').format(name=user.name, resource=resourcename))
    ...
    def checkaccess(self, resource, permission=None, namedacl=None, args=None, kwargs=None):
        ...
        if permission not in permissions:
            raise error(AccessDeniedError, self.log, _("Access denied for {user} on {resource}, required: '{permission}', granted: {permissions}").format(user=self.getcurrentuser().name, resource=name, permission=permission.name, permissions=map(str, permissions)))
        else:
            self.log.info(_("Access allowed for {user} on {resource} with '{permission}'").format(user=self.getcurrentuser().name, resource=name, permission=permission.name, permissions=map(str, permissions)))
        return
    ...
    def getacl(self, resource):
        ...
        name, rules = self._getacl(resource)
        self.log.debug(_('Getting ACL on {name} for {user}').format(name=name, user=self.getcurrentuser().name))
        return rules
