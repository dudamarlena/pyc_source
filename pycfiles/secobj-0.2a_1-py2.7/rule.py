...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/secobj/rule.py
# Compiled at: 2012-08-27 09:04:42
...
class Rule(object):
    ...
    def __init__(self, action, principal, *permissions):
        ...
        if isinstance(principal, Principal):
            ...
            if Principal.has(principal):
                ...
                raise error(ValueError, 'rule', _('Invalid principal name: {name}').format(name=principal))
    ...
    def __str__(self):
        return ('({action}, {principal}, {permissions})').format(action=self._action, principal=self._principalname, permissions=map(str, self.permissions))
