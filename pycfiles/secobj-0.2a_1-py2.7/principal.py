...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/secobj/principal.py
# Compiled at: 2012-08-27 08:54:51
import collections, inspect, re
from abc import ABCMeta, abstractmethod
from secobj.localization import _
PREDICATE_NAME = re.compile('^&[a-zA-Z0-9][a-zA-Z0-9_\\.]*$')
USER_NAME = re.compile('^[a-zA-Z0-9][a-zA-Z0-9_\\.]*$')
GROUP_NAME = re.compile('^@[a-zA-Z0-9][a-zA-Z0-9_\\.]*$')
SUBJECT_NAME = re.compile('^[@]?[a-zA-Z0-9][a-zA-Z0-9_\\.]*$')
PRINCIPAL_NAME = re.compile('^[@&]?[a-zA-Z0-9][a-zA-Z0-9_\\.]*$')
...
class Principal(object):
    ...
    @classmethod
    def has(cls, name):
        return name in cls._principals

    @classmethod
    def get(cls, name):
        return cls._principals[name]
...
class Group(Subject):
    ...
    def make(cls, name, *args, **kwargs):
        ...
        return Principal.make(name, *args, **kwargs)
...
ANONYMOUS = User.make('Anonymous', instanceclass=Anonymous)
SYSTEM = User.make('System')
AUTHENTICATED = Principal.make('&Authenticated', instanceclass=Authenticated)
EVERYONE = Principal.make('&Everyone', instanceclass=Everyone)
OWNER = Principal.make('&Owner', instanceclass=Owner)