...
# [GCC 8.4.0]
# Embedded file name: /home/fin/Development/github/py-cloc-plot/cloc_plot/__init__.py
# Compiled at: 2019-06-19 14:09:56
...
def die(msg):
    print('\x1b[31:1m !!! \x1b[0m{}'.format(msg))
    sys.exit(1)


def info(msg):
    print('\x1b[32:1m *** \x1b[0m{}'.format(msg))
...
def main():
    ...
    for project in projects:
        ...
        stats.pop('SUM', None)
        stats = {'name':project, 
         'loc':[{'lang':key,  'lines':value['code']} for key, value in stats.items()],  'sum':sum(v['code'] for v in stats.values())}
    ...
    wedges, texts, autotexts = ax.pie(locs,
      radius=(1 - size),
      labels=languages,
      colors=colors,
      labeldistance=None,
    ...
    loc_sum = sum(locs)
    wedges, texts, autotexts = ax.pie([v['sum'] for v in project_stats],
      radius=1,
