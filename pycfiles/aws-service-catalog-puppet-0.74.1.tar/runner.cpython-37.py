...
# [GCC 8.4.0]
# Embedded file name: /codebuild/output/src613133936/src/github.com/awslabs/aws-service-catalog-puppet/servicecatalog_puppet/workflow/runner.py
# Compiled at: 2020-05-13 08:59:18
# Size of source mod 2**32: 10729 bytes
import json, os, sys, time
from glob import glob
from pathlib import Path
import click, colorclass, luigi, terminaltables, yaml
...
def run_tasks(tasks_to_run, num_workers, is_dry_run=False):
    ...
    if should_forward_failures_to_opscenter:
        with betterboto_client.ClientContextManager('ssm') as (ssm):
            ssm_client = ssm
    entries = []
    for type in ('failure', 'success', 'timeout', 'process_failure', 'processing_time',
                 'broken_task'):
        os.makedirs(Path(constants.RESULTS_DIRECTORY) / type)

    logger.info(f"About to run workflow with {num_workers} workers")
    tasks.print_stats()
    run_result = luigi.build(tasks_to_run,
      local_scheduler=True,
      detailed_summary=True,
      workers=num_workers,
      log_level='INFO')
    exit_status_codes = {LuigiStatusCode.SUCCESS: 0, 
    ...
    click.echo('Results')
    if is_dry_run:
        table_data = [['Result', 'Launch', 'Account', 'Region', 'Current Version', 'New Version', 'Notes']]
        ...
        for filename in glob('results/processing_time/*.json'):
            ...
            if should_use_eventbridge:
                entries.append({'Source':constants.SERVICE_CATALOG_PUPPET_EVENT_SOURCE, 
                 'Resources':[],  'DetailType':result.get('task_type'), 
        ...
        for filename in glob('results/failure/*.json'):
            ...
            if should_forward_failures_to_opscenter:
                ...
                for param_name, param in params.items():
                    operational_data[param_name] = {'Value':json.dumps(param, default=str),  'Type':'SearchableString'}

                description = '\n'.join(result.get('exception_stack_trace'))[-1024:]
                ssm_client.create_ops_item(Title=title,
        ...
        if should_use_eventbridge:
            ...
            with betterboto_client.ClientContextManager('events') as (events):
                for i in range(0, len(entries), constants.EVENTBRIDGE_MAX_EVENTS_PER_CALL):
                    events.put_events(Entries=(entries[i:i + constants.EVENTBRIDGE_MAX_EVENTS_PER_CALL]))
                    time.sleep(1)
...
def run_tasks_for_generate_shares(tasks_to_run):
    ...
    for region in config.get_regions():
        sharing_policies = {'accounts':[],  'organizations':[]}
        with betterboto_client.ClientContextManager('cloudformation', region_name=region) as (cloudformation):
            ...
            logger.info('Finished generating policies collection')
            template = config.env.get_template('policies.template.yaml.j2').render(sharing_policies=sharing_policies,
              VERSION=version)
            with betterboto_client.ClientContextManager('cloudformation', region_name=region) as (cloudformation):
                cloudformation.create_or_update(StackName='servicecatalog-puppet-policies',
                  TemplateBody=template,
                  NotificationARNs=([f"arn:aws:sns:{region}:{puppet_account_id}:servicecatalog-puppet-cloudformation-regional-events"] if should_use_sns else []))
