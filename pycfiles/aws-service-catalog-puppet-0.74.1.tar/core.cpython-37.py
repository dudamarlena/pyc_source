...
# [GCC 8.4.0]
# Embedded file name: /codebuild/output/src613133936/src/github.com/awslabs/aws-service-catalog-puppet/servicecatalog_puppet/core.py
# Compiled at: 2020-05-13 08:59:18
# Size of source mod 2**32: 40139 bytes
from pathlib import Path
import cfn_tools, requests
from colorclass import Color
import terminaltables, shutil, json
from threading import Thread
import pkg_resources, yaml, logging, os, click
...
import servicecatalog_puppet.workflow as management_tasks
import servicecatalog_puppet.workflow as portfoliomanagement_tasks
import servicecatalog_puppet.workflow as provisioning_tasks
...
def generate_shares(f):
    ...
    launch_tasks = manifest_utils_for_launches.generate_launch_tasks(manifest,
      puppet_account_id, False, False, include_expanded_from=False, single_account=None, is_dry_run=False)
    for launch_task in launch_tasks:
        t = manifest_utils_for_launches.generate_launch_task_defs_for_launch(launch_task.launch_name, launch_task.manifest, launch_task.puppet_account_id, launch_task.should_use_sns, launch_task.should_use_product_plans, launch_task.include_expanded_from, launch_task.single_account, launch_task.is_dry_run)
        tasks = launch_task.generate_provisions(t.get('task_defs'))
        for task_ in tasks:
            task = task_.param_kwargs
            a_id = task.get('account_id')
            tasks_to_run.append(portfoliomanagement_tasks.CreateShareForAccountLaunchRegion(puppet_account_id=puppet_account_id,
              account_id=a_id,
...
def reset_provisioned_product_owner(f):
    puppet_account_id = config.get_puppet_account_id()
    manifest = manifest_utils.load(f)
    task_defs = manifest_utils_for_launches.generate_launch_tasks(manifest, puppet_account_id, False, False)
    tasks_to_run = []
    for task in task_defs:
        task_status = task.get('status')
        if task_status == constants.PROVISIONED:
            tasks_to_run.append(provisioning_tasks.ResetProvisionedProductOwnerTask(launch_name=(task.get('launch_name')),
...
def generate_tasks(f, single_account=None, is_dry_run=False):
    ...
    tasks_to_run = manifest_utils_for_launches.generate_launch_tasks(manifest,
      puppet_account_id,
      should_use_sns,
...
def _do_bootstrap_spoke(puppet_account_id, cloudformation, puppet_version, permission_boundary):
    ...
    args = {'StackName':'{}-spoke'.format(constants.BOOTSTRAP_STACK_NAME), 
...
def bootstrap_spoke_as(puppet_account_id, iam_role_arns, permission_boundary):
    ...
    with betterboto_client.CrossMultipleAccountsClientContextManager('cloudformation', cross_accounts) as (cloudformation):
        _do_bootstrap_spoke(puppet_account_id, cloudformation, config.get_puppet_version(), permission_boundary)
...
def _do_bootstrap(puppet_version, with_manual_approvals, puppet_code_pipeline_role_permission_boundary, source_role_permissions_boundary, puppet_generate_role_permission_boundary, puppet_deploy_role_permission_boundary, puppet_provisioning_role_permissions_boundary, cloud_formation_deploy_role_permissions_boundary, deploy_environment_compute_type='BUILD_GENERAL1_SMALL', deploy_num_workers=10):
    ...
    with betterboto_client.MultiRegionClientContextManager('cloudformation', all_regions) as (clients):
        ...
        template = Template(template).render(VERSION=puppet_version)
        args = {'StackName':'{}-regional'.format(constants.BOOTSTRAP_STACK_NAME), 
         'TemplateBody':template, 
         'Capabilities':[
          'CAPABILITY_IAM'], 
         'Parameters':[
    ...
    with betterboto_client.ClientContextManager('codecommit') as (codecommit):
        ...
        clone_command = "git clone --config 'credential.helper=!aws codecommit credential-helper $@' --config 'credential.UseHttpPath=true' {}".format(clone_url)
        click.echo('You need to clone your newly created repo now and will then need to seed it: \n{}'.format(clone_command))
...
def bootstrap_branch(branch_name, with_manual_approvals, puppet_code_pipeline_role_permission_boundary, source_role_permissions_boundary, puppet_generate_role_permission_boundary, puppet_deploy_role_permission_boundary, puppet_provisioning_role_permissions_boundary, cloud_formation_deploy_role_permissions_boundary):
    _do_bootstrap('https://github.com/awslabs/aws-service-catalog-puppet/archive/{}.zip'.format(branch_name), with_manual_approvals, puppet_code_pipeline_role_permission_boundary, source_role_permissions_boundary, puppet_generate_role_permission_boundary, puppet_deploy_role_permission_boundary, puppet_provisioning_role_permissions_boundary, cloud_formation_deploy_role_permissions_boundary)
...
def seed(complexity, p):
    ...
    shutil.copy2(asset_helpers.resolve_from_site_packages(os.path.sep.join(['manifests', example])), os.path.sep.join([p, 'manifest.yaml']))
    ...
    if format == 'table':
        ...
        for account_id in account_ids:
            for region_name in all_regions:
                ...
                with betterboto_client.CrossAccountClientContextManager('servicecatalog',
                  role, ('sc-{}-{}'.format(account_id, region_name)), region_name=region_name) as (spoke_service_catalog):
                    ...
                    for portfolio in portfolios:
                        ...
                        for product_view_detail in response.get('ProductViewDetails', []):
                            ...
                            response = spoke_service_catalog.search_provisioned_products(Filters={'SearchQuery': ['productId:{}'.format(product_id)]})
                            for provisioned_product in response.get('ProvisionedProducts', []):
                                ...
                                provisioning_artifact_response = spoke_service_catalog.describe_provisioning_artifact(ProvisioningArtifactId=(provisioned_product.get('ProvisioningArtifactId')),
                                  ProductId=(provisioned_product.get('ProductId'))).get('ProvisioningArtifactDetail')
                                ...
                                deployments[account_id][constants.LAUNCHES][launch_name][region_name] = {'launch_name':launch_name,  'portfolio':portfolio.get('DisplayName'), 
                                ...
                                with open(output, 'w') as (f):
                                    f.write(json.dumps(provisioned_product,
                                      indent=4,
                                      default=str))
        ...
        for task in tasks:
            ...
            for region, regional_details in deployments[account_id][constants.LAUNCHES][launch_name].items():
                results[f"{account_id}_{region}_{launch_name}"] = {'account_id':account_id, 
                 'region':region, 
                 'launch':launch_name, 
...
def expand(f):
    ...
    if org_iam_role_arn is None:
        ...
        with betterboto_client.CrossAccountClientContextManager('organizations', org_iam_role_arn, 'org-iam-role') as (client):
...
def version():
    ...
    with betterboto_client.ClientContextManager('ssm') as (ssm):
        ...
        click.echo('regional stack version: {} for region: {}'.format(response.get('Parameter').get('Value'), response.get('Parameter').get('ARN').split(':')[3]))
...
def upload_config(config):
    with betterboto_client.ClientContextManager('ssm') as (ssm):
        ssm.put_parameter(Name=(constants.CONFIG_PARAM_NAME),
          Type='String',
...
def set_org_iam_role_arn(org_iam_role_arn):
    with betterboto_client.ClientContextManager('ssm') as (ssm):
        ssm.put_parameter(Name=(constants.CONFIG_PARAM_NAME_ORG_IAM_ROLE_ARN),
          Type='String',
...
def run(what, tail):
    ...
    click.echo(f"https://{os.environ.get('AWS_DEFAULT_REGION')}.console.aws.amazon.com/codesuite/codepipeline/pipelines/{pipeline_name}/executions/{pipeline_execution_id}/timeline")
...
def list_resources():
    ...
    for file in Path(__file__).parent.resolve().glob('*.template.yaml'):
        ...
        for logical_name, resource in template.get('Resources').items():
            ...
            name = '-'
            type_to_name = {'AWS::IAM::Role':'RoleName', 
...
def get_manifest():
    with betterboto_client.ClientContextManager('codecommit') as (codecommit):
        content = codecommit.get_file(repositoryName=(constants.SERVICE_CATALOG_PUPPET_REPO_NAME),
          filePath='manifest.yaml').get('fileContent')
        return yaml.safe_load(content)


def save_manifest(manifest):
    with betterboto_client.ClientContextManager('codecommit') as (codecommit):
        parent_commit_id = codecommit.get_branch(repositoryName=(constants.SERVICE_CATALOG_PUPPET_REPO_NAME),
          branchName='master').get('branch').get('commitId')
        codecommit.put_file(repositoryName=(constants.SERVICE_CATALOG_PUPPET_REPO_NAME),
          branchName='master',
...
def remove_from_accounts(account_id_or_ou_id_or_ou_path):
    ...
    for account in manifest.get('accounts', []):
        if account.get('account_id', '') == account_id_or_ou_id_or_ou_path:
            ...
            if account.get('ou', '') == account_id_or_ou_id_or_ou_path:
                manifest.get('accounts').remove(account)
                return save_manifest(manifest)
    ...
    if org_iam_role_arn is None:
        ...
        with betterboto_client.CrossAccountClientContextManager('organizations', org_iam_role_arn, 'org-iam-role') as (client):
            ...
            for spoke in response:
                tasks.append(management_tasks.BootstrapSpokeAsTask(puppet_account_id=puppet_account_id,
...
def handle_action_execution_detail(action_execution_detail):
    ...
    if action_type_id.get('category') == 'Build':
        if action_type_id.get('owner') == 'AWS':
            if action_type_id.get('provider') == 'CodeBuild':
                external_execution_id = action_execution_detail.get('output').get('executionResult').get('externalExecutionId')
                with betterboto_client.ClientContextManager('codebuild',
                  region_name=(config.get_home_region())) as (codebuild):
                    builds = codebuild.batch_get_builds(ids=[external_execution_id]).get('builds')
                    build = builds[0]
                    log_details = build.get('logs')
                    with betterboto_client.ClientContextManager('logs',
                      region_name=(config.get_home_region())) as (logs):
                        with open(f"log-{action_execution_detail.get('input').get('configuration').get('ProjectName')}.log", 'w') as (f):
                            params = {'logGroupName':log_details.get('groupName'),  'logStreamName':log_details.get('streamName'), 
                             'startFromHead':True}
                            has_more_logs = True
                            while has_more_logs:
...
def export_puppet_pipeline_logs(execution_id):
    with betterboto_client.ClientContextManager('codepipeline',
      region_name=(config.get_home_region())) as (codepipeline):
        action_execution_details = codepipeline.list_action_executions(pipelineName=(constants.PIPELINE_NAME),
          filter={'pipelineExecutionId': execution_id}).get('actionExecutionDetails')
        for action_execution_detail in action_execution_details:
            handle_action_execution_detail(action_execution_detail)


def uninstall():
    with betterboto_client.ClientContextManager('cloudformation',
      region_name=(config.get_home_region())) as (cloudformation):
