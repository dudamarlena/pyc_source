...
# [GCC 8.4.0]
# Embedded file name: /codebuild/output/src613133936/src/github.com/awslabs/aws-service-catalog-puppet/servicecatalog_puppet/manifest_utils_for_launches.py
...
def get_configuration_from_launch(manifest, launch_details, launch_name):
    configuration = {'status':launch_details.get('status', constants.PROVISIONED), 
     'launch_name':launch_name, 
     'portfolio':launch_details.get('portfolio'), 
     'product':launch_details.get('product'), 
     'version':launch_details.get('version'), 
     'parameters':[],  'ssm_param_inputs':[],  'launch_parameters':launch_details.get('parameters', {}), 
...
def generate_launch_task_defs_for_launch(launch_name, manifest, puppet_account_id, should_use_sns, should_use_product_plans, include_expanded_from=False, single_account=None, is_dry_run=False):
    ...
    return {'pre_actions':get_actions_from(launch_name, launch_details, 'pre', actions, 'launch'), 
     'post_actions':get_actions_from(launch_name, launch_details, 'post', actions, 'launch'), 
     'task_defs':get_task_defs_from_details(launch_details, accounts, include_expanded_from, launch_name, configuration)}


def generate_launch_tasks(manifest, puppet_account_id, should_use_sns, should_use_product_plans, include_expanded_from=False, single_account=None, is_dry_run=False):
    return [provisioning.LaunchTask(launch_name=launch_name, manifest=manifest, puppet_account_id=puppet_account_id, should_use_sns=should_use_sns, should_use_product_plans=should_use_product_plans, include_expanded_from=include_expanded_from, single_account=single_account, is_dry_run=is_dry_run) for launch_name in manifest.get('launches', {}).keys()]