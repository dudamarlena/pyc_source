...
# [GCC 8.4.0]
# Embedded file name: /codebuild/output/src613133936/src/github.com/awslabs/aws-service-catalog-puppet/servicecatalog_puppet/manifest_utils_for_spoke_local_portfolios.py
...
def get_configuration_from_spoke_local_portfolio(manifest, spoke_local_portfolio_details, spoke_local_portfolio_name):
    configuration = {'spoke_local_portfolio_name':spoke_local_portfolio_name, 
    ...
    configuration.update(get_configuration_overrides(manifest, spoke_local_portfolio_details))
    return configuration


def generate_spoke_local_portfolios_tasks_for_spoke_local_portfolio(spoke_local_portfolio_name, manifest, puppet_account_id, should_use_sns, should_use_product_plans, include_expanded_from=False, single_account=None, is_dry_run=False):
    accounts = manifest.get('accounts', [])
    actions = manifest.get('actions', {})
    spoke_local_portfolio_details = manifest.get('spoke-local-portfolios').get(spoke_local_portfolio_name)
    configuration = get_configuration_from_spoke_local_portfolio(manifest, spoke_local_portfolio_details, spoke_local_portfolio_name)
    ...
    task_defs = get_task_defs_from_details(spoke_local_portfolio_details, accounts, True, spoke_local_portfolio_name, configuration)
    return {'pre_actions':get_actions_from(spoke_local_portfolio_name, spoke_local_portfolio_details, 'pre', actions, 'spoke-local-portfolios'), 
     'post_actions':get_actions_from(spoke_local_portfolio_name, spoke_local_portfolio_details, 'post', actions, 'spoke-local-portfolios'), 
     'task_defs':task_defs}


def generate_spoke_local_portfolios_tasks(manifest, puppet_account_id, should_use_sns, should_use_product_plans, include_expanded_from, single_account, is_dry_run):
    return [provisioning.SpokeLocalPortfolioTask(spoke_local_portfolio_name=spoke_local_portfolio_name, manifest=manifest, puppet_account_id=puppet_account_id, should_use_sns=should_use_sns, should_use_product_plans=should_use_product_plans, include_expanded_from=include_expanded_from, single_account=single_account, is_dry_run=is_dry_run) for spoke_local_portfolio_name in manifest.get('spoke-local-portfolios', {}).keys()]