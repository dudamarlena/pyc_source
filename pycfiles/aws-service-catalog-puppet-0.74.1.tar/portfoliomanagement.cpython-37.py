...
# [GCC 8.4.0]
# Embedded file name: /codebuild/output/src613133936/src/github.com/awslabs/aws-service-catalog-puppet/servicecatalog_puppet/workflow/portfoliomanagement.py
# Compiled at: 2020-05-13 08:59:18
# Size of source mod 2**32: 59080 bytes
import json, os, re, time, luigi
...
class GetVersionIdByVersionName(tasks.PuppetTask):
    ...
    def requires(self):
        product_id = GetProductIdByProductName(self.portfolio, self.product, self.account_id, self.region)
    ...
    def run(self):
        ...
        with betterboto_client.CrossAccountClientContextManager('servicecatalog',
          ...
          region_name=(self.region)) as (cross_account_servicecatalog):
            version_id = aws.get_version_id_for(cross_account_servicecatalog, product_details.get('product_id'), self.version)
            with self.output().open('w') as (f):
                f.write(json.dumps({'version_name':self.version, 
...
class SearchProductsAsAdminTask(tasks.PuppetTask):
    ...
    def requires(self):
        return {'portfolio': GetPortfolioByPortfolioName(self.portfolio, self.account_id, self.region)}
    ...
    def run(self):
        ...
        with betterboto_client.CrossAccountClientContextManager('servicecatalog',
          role, f"sc-{self.account_id}-{self.region}", region_name=(self.region)) as (spoke_service_catalog):
            results = spoke_service_catalog.search_products_as_admin_single_page(PortfolioId=portfolio_id)
...
class GetProductIdByProductName(tasks.PuppetTask):
    ...
    def requires(self):
        return {'portfolio':GetPortfolioByPortfolioName(self.portfolio, self.account_id, self.region), 
         'search_products_as_admin':SearchProductsAsAdminTask(self.portfolio, self.account_id, self.region)}
    ...
    def run(self):
        ...
        self.write_output({'product_name':self.product, 
    ...
    def run(self):
        with betterboto_client.CrossAccountClientContextManager('servicecatalog',
          ...
          region_name=(self.region)) as (cross_account_servicecatalog):
            ...
            with self.output().open('w') as (f):
                f.write(json.dumps({'portfolio_name':self.portfolio, 
...
class ProvisionActionTask(tasks.PuppetTask):
    ...
    def requires(self):
        ...
        for param_name, param_details in self.parameters.items():
            if param_details.get('ssm'):
                ...
                ssm_params[param_name] = tasks.GetSSMParamTask(parameter_name=param_name,
                  name=(param_details.get('ssm').get('name')),
                  region=(param_details.get('ssm').get('region', config.get_home_region())))

        return {'ssm_params': ssm_params}
    ...
    def run(self):
        ...
        environmentVariablesOverride = [{'name':param_name,  'value':param_details,  'type':'PLAINTEXT'} for param_name, param_details in all_params.items()]
        role = f"arn:aws:iam::{self.account_id}:role/servicecatalog-puppet/PuppetRole"
        with betterboto_client.CrossAccountClientContextManager('codebuild',
          role, f"sc-{self.region}-{self.account_id}", region_name=(self.region)) as (codebuild):
            build = codebuild.start_build_and_wait_for_completion(projectName=(self.project_name),
...
class CreateSpokeLocalPortfolioTask(tasks.PuppetTask):
    ...
    def run(self):
        ...
        with betterboto_client.CrossAccountClientContextManager('servicecatalog',
          role, f"sc-{self.account_id}-{self.region}", region_name=(self.region)) as (spoke_service_catalog):
            spoke_portfolio = aws.ensure_portfolio(spoke_service_catalog, self.portfolio, portfolio_details.get('provider_name'), portfolio_details.get('description'))
        with self.output().open('w') as (f):
            f.write(json.dumps(spoke_portfolio,
              indent=4,
              default=str))
...
class CreateAssociationsForPortfolioTask(tasks.PuppetTask):
    ...
    def requires(self):
        return {'create_spoke_local_portfolio_task':CreateSpokeLocalPortfolioTask(account_id=self.account_id,
           region=self.region,
           portfolio=self.portfolio,
    ...
    def run(self):
        ...
        with betterboto_client.CrossAccountClientContextManager('cloudformation',
          role, f"cfn-{self.account_id}-{self.region}", region_name=(self.region)) as (cloudformation):
            template = config.env.get_template('associations.template.yaml.j2').render(portfolio={'DisplayName':self.portfolio, 
             'Associations':self.associations},
              portfolio_id=portfolio_id)
            stack_name = f"associations-for-portfolio-{portfolio_id}"
            self.info(template)
            cloudformation.create_or_update(StackName=stack_name,
              TemplateBody=template,
              NotificationARNs=([f"arn:aws:sns:{self.region}:{self.puppet_account_id}:servicecatalog-puppet-cloudformation-regional-events"] if self.should_use_sns else []))
            result = cloudformation.describe_stacks(StackName=stack_name).get('Stacks')[0]
...
class GetProductsAndProvisioningArtifactsTask(tasks.PuppetTask):
    ...
    def requires(self):
        return {'search_products_as_admin': SearchProductsAsAdminTask(portfolio=(self.portfolio),
                                       region=(self.region),
        ...
        with betterboto_client.ClientContextManager('servicecatalog',
          region_name=(self.region)) as (service_catalog):
            ...
            for product_view_detail in response.get('ProductViewDetails', []):
                ...
                hub_provisioning_artifact_details = service_catalog.list_provisioning_artifacts(ProductId=hub_product_id).get('ProvisioningArtifactDetails', [])
...
class CopyIntoSpokeLocalPortfolioTask(tasks.PuppetTask):
    ...
    def requires(self):
        return {'create_spoke_local_portfolio':CreateSpokeLocalPortfolioTask(account_id=self.account_id,
           region=self.region,
           portfolio=self.portfolio,
           organization=self.organization), 
         'products_and_provisioning_artifacts':GetProductsAndProvisioningArtifactsTask(region=self.region,
           portfolio=self.portfolio,
           puppet_account_id=self.puppet_account_id)}
        ...
        with self.input().get('products_and_provisioning_artifacts').open('r') as (f):
            ...
            for product_view_summary in products_and_provisioning_artifacts:
                ...
                for hub_provisioning_artifact_detail in product_view_summary.get('provisioning_artifact_details', []):
                    if hub_provisioning_artifact_detail.get('Type') == 'CLOUD_FORMATION_TEMPLATE':
                        product_versions_that_should_be_copied[(f"{hub_provisioning_artifact_detail.get('Name')}")] = hub_provisioning_artifact_detail
                        product_versions_that_should_be_updated[(f"{hub_provisioning_artifact_detail.get('Name')}")] = hub_provisioning_artifact_detail
                ...
                copy_args = {'SourceProductArn':hub_product_arn, 
                 'CopyOptions':[
                  'CopyTags']}
                logger.info(f"[{self.portfolio}] {self.account_id}:{self.region} {hub_product_name} :: searching in spoke for product")
                role = f"arn:aws:iam::{self.account_id}:role/servicecatalog-puppet/PuppetRole"
                with betterboto_client.CrossAccountClientContextManager('servicecatalog',
                  role, f"sc-{self.account_id}-{self.region}", region_name=(self.region)) as (spoke_service_catalog):
                    p = None
                    try:
                        p = spoke_service_catalog.search_products_as_admin_single_page(PortfolioId=portfolio_id,
                          Filters={'FullTextSearch': [hub_product_name]})
                    except spoke_service_catalog.exceptions.ResourceNotFoundException as e:
                        try:
                            logger.info(f"[{self.portfolio}] {self.account_id}:{self.region} {hub_product_name} :: swallowing exception: {str(e)}")
                        finally:
                            e = None
                            del e
                    ...
                    if p is not None:
                        for spoke_product_view_details in p.get('ProductViewDetails'):
                            ...
                            if spoke_product_view.get('Name') == hub_product_name:
                                ...
                                spoke_provisioning_artifact_details = spoke_service_catalog.list_provisioning_artifacts(ProductId=spoke_product_id).get('ProvisioningArtifactDetails')
                                for provisioning_artifact_detail in spoke_provisioning_artifact_details:
                                    ...
                                    if product_versions_that_should_be_copied.get(id_to_delete, None) is not None:
                                        logger.info(f"[{self.portfolio}] {self.account_id}:{self.region} {hub_product_name} :: Going to skip {spoke_product_id} {provisioning_artifact_detail.get('Name')}")
                                        del product_versions_that_should_be_copied[id_to_delete]

                    elif len(product_versions_that_should_be_copied.keys()) == 0:
                        logger.info(f"[{self.portfolio}] {self.account_id}:{self.region} {hub_product_name} :: no versions to copy")
                    else:
                        logger.info(f"[{self.portfolio}] {self.account_id}:{self.region} {hub_product_name} :: about to copy product")
                        copy_args['SourceProvisioningArtifactIdentifiers'] = [{'Id': a.get('Id')} for a in product_versions_that_should_be_copied.values()]
                        logger.info(f"[{self.portfolio}] {self.account_id}:{self.region} :: about to copy product withargs: {copy_args}")
                        copy_product_token = (spoke_service_catalog.copy_product)(**copy_args).get('CopyProductToken')
                        while 1:
                            time.sleep(5)
                            r = spoke_service_catalog.describe_copy_product_status(CopyProductToken=copy_product_token)
                            target_product_id = r.get('TargetProductId')
                            logger.info(f"[{self.portfolio}] {self.account_id}:{self.region} :: {hub_product_name} status: {r.get('CopyProductStatus')}")
                            if r.get('CopyProductStatus') == 'FAILED':
                                raise Exception(f"[{self.portfolio}] {self.account_id}:{self.region} :: Copying {hub_product_name} failed: {r.get('StatusDetail')}")
                            elif r.get('CopyProductStatus') == 'SUCCEEDED':
                                break

                        logger.info(f"[{self.portfolio}] {self.account_id}:{self.region} :: adding {target_product_id} to portfolio {portfolio_id}")
                        spoke_service_catalog.associate_product_with_portfolio(ProductId=target_product_id,
                          PortfolioId=portfolio_id)
                        logger.info(f"[{self.portfolio}] {self.account_id}:{self.region} :: waiting for adding of {target_product_id} to portfolio {portfolio_id}")
                        while 1:
                            time.sleep(2)
                            response = spoke_service_catalog.search_products_as_admin_single_page(PortfolioId=portfolio_id)
                            products_ids = [product_view_detail.get('ProductViewSummary').get('ProductId') for product_view_detail in response.get('ProductViewDetails')]
                            logger.info(f"[{self.portfolio}] {self.account_id}:{self.region} :: Looking for {target_product_id} in {products_ids}")
                    ...
                    spoke_provisioning_artifact_details = spoke_service_catalog.list_provisioning_artifacts(ProductId=product_id_in_spoke).get('ProvisioningArtifactDetails', [])
                    for version_name, version_details in product_versions_that_should_be_updated.items():
                        ...
                        for spoke_provisioning_artifact_detail in spoke_provisioning_artifact_details:
                            if spoke_provisioning_artifact_detail.get('Name') == version_name:
                                logging.info(f"Updating active of {version_name}/{spoke_provisioning_artifact_detail.get('Id')} in the spoke to {version_details.get('Active')}")
                                spoke_service_catalog.update_provisioning_artifact(ProductId=product_id_in_spoke,
                                  ProvisioningArtifactId=(spoke_provisioning_artifact_detail.get('Id')),
                                  Active=(version_details.get('Active')))

        with self.output().open('w') as (f):
            f.write(json.dumps({'portfolio':spoke_portfolio, 
...
class ImportIntoSpokeLocalPortfolioTask(tasks.PuppetTask):
    ...
    def requires(self):
        return {'create_spoke_local_portfolio':CreateSpokeLocalPortfolioTask(account_id=self.account_id,
         ...
         'products_and_provisioning_artifacts':GetProductsAndProvisioningArtifactsTask(region=self.region,
           portfolio=self.portfolio,
           puppet_account_id=self.puppet_account_id), 
         'hub_portfolio':GetPortfolioByPortfolioName(portfolio=self.portfolio,
           account_id=self.puppet_account_id,
           region=self.region)}
    ...
    def run(self):
        ...
        with betterboto_client.CrossAccountClientContextManager('servicecatalog',
          role, f"sc-{self.account_id}-{self.region}", region_name=(self.region)) as (spoke_service_catalog):
            while True:
                self.info(f"[{self.portfolio}] {self.account_id}:{self.region} :: Generating product list for portfolio {portfolio_id}")
                response = spoke_service_catalog.search_products_as_admin_single_page(PortfolioId=portfolio_id)
                spoke_portfolio_products = [product_view_detail.get('ProductViewSummary').get('ProductId') for product_view_detail in response.get('ProductViewDetails')]
                ...
                if not target_products:
                    self.info(f"[{self.portfolio}] {self.account_id}:{self.region} :: No more products for import to portfolio {portfolio_id}")
                    break
                self.info(f"[{self.portfolio}] {self.account_id}:{self.region} :: Products {target_products} not yet imported to portfolio {portfolio_id}")
                for product_id in target_products:
                    self.info(f"[{self.portfolio}] {self.account_id}:{self.region} :: Associating {product_id}")
                    spoke_service_catalog.associate_product_with_portfolio(ProductId=product_id,
                      PortfolioId=portfolio_id,
...
class CreateLaunchRoleConstraintsForPortfolio(tasks.PuppetTask):
    ...
    def requires(self):
        if self.product_generation_method == 'import':
            return {'create_spoke_local_portfolio_task': ImportIntoSpokeLocalPortfolioTask(account_id=(self.account_id),
                                                    region=(self.region),
                                                    portfolio=(self.portfolio),
                                                    organization=(self.organization),
                                                    puppet_account_id=(self.puppet_account_id))}
        return {'create_spoke_local_portfolio_task': CopyIntoSpokeLocalPortfolioTask(account_id=(self.account_id),
                                                region=(self.region),
                                                portfolio=(self.portfolio),
                                                organization=(self.organization),
                                                puppet_account_id=(self.puppet_account_id))}
        ...
        with betterboto_client.CrossAccountClientContextManager('cloudformation',
          role, f"cfn-{self.account_id}-{self.region}", region_name=(self.region)) as (cloudformation):
            ...
            for launch_constraint in self.launch_constraints:
                new_launch_constraint = {'products':[],  'roles':launch_constraint.get('roles')}
                if launch_constraint.get('products', None) is not None:
                    if isinstance(launch_constraint.get('products'), tuple):
                        ...
                        if isinstance(launch_constraint.get('products'), str):
                            with betterboto_client.CrossAccountClientContextManager('servicecatalog',
                              role, f"sc-{self.account_id}-{self.region}", region_name=(self.region)) as (service_catalog):
            ...
            template = config.env.get_template('launch_role_constraints.template.yaml.j2').render(portfolio={'DisplayName': self.portfolio},
              portfolio_id=portfolio_id,
            ...
            cloudformation.create_or_update(StackName=stack_name_v2,
              TemplateBody=template,
              NotificationARNs=([f"arn:aws:sns:{self.region}:{self.puppet_account_id}:servicecatalog-puppet-cloudformation-regional-events"] if self.should_use_sns else []))
            result = cloudformation.describe_stacks(StackName=stack_name_v2).get('Stacks')[0]
            with self.output().open('w') as (f):
...
class RequestPolicyTask(tasks.PuppetTask):
    ...
    def run(self):
        ...
        f = open(path, 'w')
        f.write(json.dumps((self.param_kwargs),
          indent=4,
          default=str))
        f.close()
        self.write_output(self.param_kwargs)
...
class SharePortfolioTask(tasks.PuppetTask):
    ...
    def requires(self):
        return {'portfolio': GetPortfolioByPortfolioName(portfolio=(self.portfolio),
                        account_id=(self.puppet_account_id),
                        region=(self.region))}
    ...
    def run(self):
        ...
        with betterboto_client.ClientContextManager('servicecatalog', region_name=(self.region)) as (servicecatalog):
            ...
            if self.account_id in account_ids:
                ...
                servicecatalog.create_portfolio_share(PortfolioId=portfolio_id,
...
class ShareAndAcceptPortfolioTask(tasks.PuppetTask):
    ...
    def requires(self):
        ...
        return {'portfolio':GetPortfolioByPortfolioName(portfolio=self.portfolio,
           account_id=self.puppet_account_id,
           region=self.region), 
         'share':SharePortfolioTask(portfolio=self.portfolio,
           account_id=self.account_id,
           region=self.region,
           puppet_account_id=self.puppet_account_id)}
    ...
    def run(self):
        ...
        with betterboto_client.CrossAccountClientContextManager('servicecatalog',
          ...
          region_name=(self.region)) as (cross_account_servicecatalog):
            was_accepted = False
            accepted_portfolio_shares = cross_account_servicecatalog.list_accepted_portfolio_shares_single_page().get('PortfolioDetails')
            ...
            if not was_accepted:
                ...
                cross_account_servicecatalog.accept_portfolio_share(PortfolioId=portfolio_id)
            principals_for_portfolio = cross_account_servicecatalog.list_principals_for_portfolio_single_page(PortfolioId=portfolio_id).get('Principals')
            ...
            if not principal_was_associated:
                cross_account_servicecatalog.associate_principal_with_portfolio(PortfolioId=portfolio_id,
...
class CreateAssociationsInPythonForPortfolioTask(tasks.PuppetTask):
    ...
    def api_calls_used(self):
        return {f"servicecatalog.associate_principal_with_portfolio_{self.region}": 1}
    ...
    def run(self):
        ...
        with betterboto_client.ClientContextManager('servicecatalog', region_name=(self.region)) as (servicecatalog):
            servicecatalog.associate_principal_with_portfolio(PortfolioId=portfolio_id,
...
class CreateShareForAccountLaunchRegion(tasks.PuppetTask):
    __doc__ = 'for the given account_id and launch and region create the shares'
    puppet_account_id = luigi.Parameter()
    account_id = luigi.Parameter()
    region = luigi.Parameter()
    portfolio = luigi.Parameter()
    ...
    def requires(self):
        ...
        deps = {'topic':RequestPolicyTask(type='topic',
           region=self.region,
           organization=self.organization,
           account_id=self.account_id), 
         'bucket':RequestPolicyTask(type='bucket',
           region=self.region,
           organization=self.organization,
           account_id=self.account_id)}
        if self.account_id == self.puppet_account_id:
            deps['share'] = CreateAssociationsInPythonForPortfolioTask(self.account_id, self.region, self.portfolio)
        else:
            deps['share'] = ShareAndAcceptPortfolioTask(self.account_id, self.region, self.portfolio, self.puppet_account_id)
        return deps
...
class DisassociateProductFromPortfolio(tasks.PuppetTask):
    account_id = luigi.Parameter()
    region = luigi.Parameter()
    ...
    def api_calls_used(self):
        return {f"servicecatalog.disassociate_product_from_portfolio_{self.account_id}_{self.region}_{self.portfolio_id}_{self.product_id}": 1}
    ...
    def run(self):
        ...
        with betterboto_client.CrossAccountClientContextManager('servicecatalog',
          f"arn:aws:iam::{self.account_id}:role/servicecatalog-puppet/PuppetRole",
          f"{self.account_id}-{self.region}-PuppetRole",
          region_name=(self.region)) as (servicecatalog):
            results = servicecatalog.disassociate_product_from_portfolio(PortfolioId=(self.portfolio_id),
              ProductId=(self.product_id))
            self.write_output(results)


class DisassociateProductsFromPortfolio(tasks.PuppetTask):
    account_id = luigi.Parameter()
    region = luigi.Parameter()
    portfolio_id = luigi.Parameter()
    ...
    def api_calls_used(self):
        return {f"servicecatalog.search_products_as_admin_single_page_{self.account_id}_{self.region}_{self.portfolio_id}": 1}
    ...
    def run(self):
        ...
        with betterboto_client.CrossAccountClientContextManager('servicecatalog',
          ...
          f"{self.account_id}-{self.region}-PuppetRole",
          region_name=(self.region)) as (servicecatalog):
            results = servicecatalog.search_products_as_admin_single_page(PortfolioId=(self.portfolio_id))
            for product_view_detail in results.get('ProductViewDetails', []):
                yield DisassociateProductFromPortfolio(account_id=(self.account_id),
                  region=(self.region),
                  portfolio_id=(self.portfolio_id),
                  product_id=(product_view_detail.get('ProductViewSummary').get('ProductId')))

            self.write_output({})


class DeleteLocalPortfolio(tasks.PuppetTask):
    account_id = luigi.Parameter()
    region = luigi.Parameter()
    portfolio_id = luigi.Parameter()
    ...
    def api_calls_used(self):
        return {f"servicecatalog.delete_portfolio_{self.account_id}_{self.region}_{self.portfolio_id}": 1}
...
class DeletePortfolioShare(tasks.PuppetTask):
    account_id = luigi.Parameter()
    region = luigi.Parameter()
    portfolio = luigi.Parameter()
    puppet_account_id = luigi.Parameter()
    ...
    def api_calls_used(self):
        return {f"servicecatalog.list_accepted_portfolio_shares_{self.account_id}_{self.region}_{self.portfolio}": 1, 
    ...
    def run(self):
        ...
        with betterboto_client.CrossAccountClientContextManager('servicecatalog',
          ...
          f"{self.account_id}-{self.region}-PuppetRole",
          region_name=(self.region)) as (servicecatalog):
            self.info(f"About to delete the portfolio share for: {self.portfolio} account: {self.account_id}")
        ...
        if portfolio_id:
            with betterboto_client.CrossAccountClientContextManager('servicecatalog',
              ...
              region_name=(self.region)) as (servicecatalog):
                servicecatalog.delete_portfolio_share(PortfolioId=portfolio_id,
                  AccountId=(self.account_id))
        self.write_output({})


class DeletePortfolio(tasks.PuppetTask):
    account_id = luigi.Parameter()
    region = luigi.Parameter()
    portfolio = luigi.Parameter()
    product_generation_method = luigi.Parameter()
        ...
        with betterboto_client.CrossAccountClientContextManager('servicecatalog',
          ...
          region_name=(self.region)) as (servicecatalog):
            ...
            if result:
                ...
                yield general.DeleteCloudFormationStackTask(account_id=(self.account_id),
                  region=(self.region),
                  stack_name=f"associations-for-portfolio-{portfolio_id}")
                yield general.DeleteCloudFormationStackTask(account_id=(self.account_id),
                  region=(self.region),
                ...
                if not is_puppet_account:
                    yield DisassociateProductsFromPortfolio(account_id=(self.account_id),
                      region=(self.region),
                      portfolio_id=portfolio_id)
                    yield DeleteLocalPortfolio(account_id=(self.account_id),
                      region=(self.region),
                      portfolio_id=portfolio_id)
            if not is_puppet_account:
                yield DeletePortfolioShare(account_id=(self.account_id),
                  region=(self.region),
                  portfolio=(self.portfolio),
                  puppet_account_id=(self.puppet_account_id))
        self.write_output({})