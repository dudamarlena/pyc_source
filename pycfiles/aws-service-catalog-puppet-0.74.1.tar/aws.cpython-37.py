...
# [GCC 8.4.0]
# Embedded file name: /codebuild/output/src613133936/src/github.com/awslabs/aws-service-catalog-puppet/servicecatalog_puppet/aws.py
# Compiled at: 2020-05-13 08:59:18
# Size of source mod 2**32: 23563 bytes
import functools, logging, time, os, click, yaml
...
def terminate_provisioned_product(prefix, service_catalog, provisioned_product_id):
    ...
    record_detail = service_catalog.terminate_provisioned_product(ProvisionedProductId=provisioned_product_id).get('RecordDetail')
...
def terminate_if_status_is_not_available(service_catalog, provisioned_product_name, product_id, account_id, region):
    ...
    for r in response.get('ProvisionedProducts', []):
        if r.get('Name') == provisioned_product_name:
            ...
            if current_status in ('AVAILABLE', 'TAINTED'):
                ...
                while True:
                    status = service_catalog.describe_provisioned_product(Id=(r.get('Id'))).get('ProvisionedProductDetail').get('Status')
                    logger.info(f"{prefix} :: waiting to complete: {status}")
                    time.sleep(5)
                    if status not in ('UNDER_CHANGE', 'PLAN_IN_PROGRESS'):
                        return terminate_if_status_is_not_available(service_catalog, provisioned_product_name, product_id, account_id, region)
...
def provision_product_with_plan(service_catalog, launch_name, account_id, region, product_id, provisioning_artifact_id, puppet_account_id, path_id, params, version, should_use_sns):
    ...
    for provisioned_product_plan in provisioned_product_plans:
        logger.info(f"{uid} :: Found plan for {provisioned_product_plan.get('ProvisionProductName')}")
    ...
    for p in params.keys():
        provisioning_parameters.append({'Key':p, 
         'Value':params.get(p)})

    response = service_catalog.create_provisioned_product_plan(PlanName=stack_name,
    ...
    if plan_status in ('CREATE_SUCCESS', 'EXECUTE_SUCCESS'):
        logger.info(f"{uid} :: Plan created, changes: {yaml.safe_dump(response.get('ResourceChanges'))}")
        if len(response.get('ResourceChanges')) == 0:
            logger.warning(f"{uid} :: There are no resource changes in this plan, running this anyway - your product will be marked as tainted as your CloudFormation changesetwill fail but your product will be the correct version and in tact.")
        ...
        if plan_execute_status in ('CREATE_SUCCESS', 'EXECUTE_SUCCESS'):
            ...
            while True:
                response = service_catalog.describe_provisioned_product(Id=provisioned_product_id)
                logger.info(f"{uid} :: waiting for provision to complete: {response.get('ProvisionedProductDetail').get('Status')}")
...
def provision_product(service_catalog, launch_name, account_id, region, product_id, provisioning_artifact_id, puppet_account_id, path_id, params, version, should_use_sns):
    uid = f"[{launch_name}] {account_id}:{region}]"
    provisioning_parameters = []
    for p in params.keys():
        provisioning_parameters.append({'Key':p, 
    ...
    provisioned_product_id = service_catalog.provision_product(ProductId=product_id,
      ProvisioningArtifactId=provisioning_artifact_id,
      PathId=path_id,
      ProvisionedProductName=launch_name,
      ProvisioningParameters=provisioning_parameters,
     ...
     {'Key':'version', 
      'Value':version}],
      NotificationArns=([f"arn:aws:sns:{region}:{puppet_account_id}:servicecatalog-puppet-cloudformation-regional-events"] if should_use_sns else [])).get('RecordDetail').get('ProvisionedProductId')
...
def update_provisioned_product(service_catalog, launch_name, account_id, region, product_id, provisioning_artifact_id, puppet_account_id, path_id, params, version):
    uid = f"[{launch_name}] {account_id}:{region}]"
    provisioning_parameters = []
    for p in params.keys():
        provisioning_parameters.append({'Key':p, 
    ...
    provisioned_product_id = service_catalog.update_provisioned_product(ProductId=product_id,
      ProvisioningArtifactId=provisioning_artifact_id,
      PathId=path_id,
      ProvisionedProductName=launch_name,
      ProvisioningParameters=provisioning_parameters).get('RecordDetail').get('ProvisionedProductId')
    logger.info(f"{uid}: provisioning started: {provisioned_product_id}")
    while True:
        response = service_catalog.describe_provisioned_product(Id=provisioned_product_id)
        logger.info(f"{uid} :: waiting for provision to complete: {response.get('ProvisionedProductDetail').get('Status')}")
...
def ensure_is_terminated(service_catalog, provisioned_product_name, product_id):
    ...
    r = get_provisioned_product_details(product_id, provisioned_product_name, service_catalog)
    if r is not None:
        provisioned_product_id = r.get('Id')
        provisioning_artifact_id = r.get('ProvisioningArtifactId')
...
def get_provisioning_artifact_id_for(portfolio_name, product_name, version_name, account_id, region):
    logger.info('Getting provisioning artifact id for: {} {} {} in the region: {} of account: {}'.format(portfolio_name, product_name, version_name, region, account_id))
    ...
    with betterboto_client.CrossAccountClientContextManager('servicecatalog',
      role, ('-'.join([account_id, region])), region_name=region) as (cross_account_servicecatalog):
        product_id = None
        version_id = None
        portfolio_id = None
        ...
        response = cross_account_servicecatalog.list_provisioning_artifacts_single_page(ProductId=product_id)
...
def ensure_portfolio(service_catalog, portfolio_name, provider_name, description=None):
    return find_portfolio(service_catalog, portfolio_name) or create_portfolio(service_catalog, portfolio_name, provider_name, description)
...
def create_portfolio(service_catalog, portfolio_name, provider_name, description=None):
    ...
    args = {'DisplayName':portfolio_name, 
...
def run_pipeline(pipeline_name, tail):
    with betterboto_client.ClientContextManager('codepipeline') as (codepipeline):
        ...
        if tail:
            while True:
                pipeline_execution = codepipeline.get_pipeline_execution(pipelineName=pipeline_name,
                  pipelineExecutionId=pipeline_execution_id).get('pipelineExecution')
...
def get_version_id_for(servicecatalog, product_id, version_name):
    version_id = None
    response = servicecatalog.list_provisioning_artifacts_single_page(ProductId=product_id)
    for provisioning_artifact_detail in response.get('ProvisioningArtifactDetails'):
        if provisioning_artifact_detail.get('Name') == version_name:
            version_id = provisioning_artifact_detail.get('Id')
