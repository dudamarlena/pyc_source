...
# [GCC 8.4.0]
# Embedded file name: /codebuild/output/src613133936/src/github.com/awslabs/aws-service-catalog-puppet/servicecatalog_puppet/manifest_utils.py
# Compiled at: 2020-05-13 08:59:18
# Size of source mod 2**32: 13994 bytes
import os, yaml, logging, json
from copy import deepcopy
from servicecatalog_puppet import config
import servicecatalog_puppet.macros as macros
from servicecatalog_puppet import constants
logger = logging.getLogger(__file__)

def load(f):
    manifest = {'schema':'puppet-2019-04-01', 
     'parameters':{},  'accounts':[],  'launches':{},  'spoke-local-portfolios':{}}
    manifest.update(yaml.safe_load(f.read()))
    d = os.path.dirname(os.path.abspath(f.name))
    extendable = [
     'parameters', 'launches', 'spoke-local-portfolios']
    for t in extendable:
...
def expand_manifest(manifest, client):
    ...
    for account in temp_accounts:
        ...
        if not account.get('append'):
            if account.get('overwrite'):
                if account.get('default_region') or account.get('regions_enabled') or account.get('tags'):
                    raise Exception(f"{account_id}: If using append or overwrite you cannot set default_region, regions_enabled or tags")
            if accounts_by_id.get(account_id) is None:
                ...
                if stored_account.get('append'):
                    ...
                    raise Exception(f"Account {account_id} has been seen twice without using append or overwrite")
