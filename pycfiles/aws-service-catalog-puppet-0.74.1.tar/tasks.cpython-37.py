...
# [GCC 8.4.0]
# Embedded file name: /codebuild/output/src613133936/src/github.com/awslabs/aws-service-catalog-puppet/servicecatalog_puppet/workflow/tasks.py
...
class GetSSMParamTask(PuppetTask):
    ...
    def output(self):
        return luigi.LocalTarget(f"output/{self.__class__.__name__}/{self.uid}.json")
    ...
    def run(self):
        with betterboto_client.ClientContextManager('ssm', region_name=(self.region)) as (ssm):
            try:
                ...
                self.write_output({'Name':self.name, 
                 'Region':self.region, 
...
def record_event(event_type, task, extra_event_data=None):
    ...
    with open(Path(constants.RESULTS_DIRECTORY) / event_type / f"{task_type}-{task.task_id}.json", 'w') as (f):
...
def on_task_failure(task, exception):
    exception_details = {'exception_type':type(exception), 
     'exception_stack_trace':traceback.format_exception(etype=type(exception),
       value=exception,
...
def on_task_process_failure(task, error_msg):
    print_stats()
    exception_details = {'exception_type':'PROCESS_FAILURE', 
