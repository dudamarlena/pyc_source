...
# [GCC 8.4.0]
# Embedded file name: /codebuild/output/src613133936/src/github.com/awslabs/aws-service-catalog-puppet/servicecatalog_puppet/workflow/provisioning.py
# Compiled at: 2020-05-13 08:59:18
# Size of source mod 2**32: 42924 bytes
import json
...
class ProvisioningArtifactParametersTask(tasks.PuppetTask):
    ...
    def requires(self):
        return {'details': portfoliomanagement.GetVersionIdByVersionName(self.portfolio, self.product, self.version, self.account_id, self.region)}
        ...
        with self.input().get('details').open('r') as (f):
            ...
            with betterboto_client.CrossAccountClientContextManager('servicecatalog',
              ...
              region_name=(self.region)) as (service_catalog):
                ...
                provisioning_artifact_parameters = service_catalog.describe_provisioning_parameters(ProductId=(details.get('product_id')),
                  ProvisioningArtifactId=(details.get('version_id')),
                  PathId=path_id).get('ProvisioningArtifactParameters', [])
...
class ProvisionProductTask(tasks.PuppetTask):
    ...
    def params_for_results_display(self):
        return {'launch_name':self.launch_name, 
         'account_id':self.account_id, 
         'region':self.region, 
    ...
    def requires(self):
        ...
        version_id = portfoliomanagement.GetVersionIdByVersionName(self.portfolio, self.product, self.version, self.account_id, self.region)
        product_id = portfoliomanagement.GetProductIdByProductName(self.portfolio, self.product, self.account_id, self.region)
        return {'ssm_params':ssm_params, 
         'version':version_id, 
         'product':product_id, 
         'provisioning_artifact_parameters':ProvisioningArtifactParametersTask(self.portfolio, self.product, self.version, self.account_id, self.region)}
    ...
    def run(self):
        ...
        with betterboto_client.CrossAccountClientContextManager('servicecatalog',
          role, f"sc-{self.region}-{self.account_id}", region_name=(self.region)) as (service_catalog):
            ...
            provisioned_product_id, provisioning_artifact_id = aws.terminate_if_status_is_not_available(service_catalog, self.launch_name, product_id, self.account_id, self.region)
            ...
            with betterboto_client.CrossAccountClientContextManager('cloudformation',
              role, f"cfn-{self.region}-{self.account_id}", region_name=(self.region)) as (cloudformation):
                ...
                if provisioning_artifact_id == version_id:
                    ...
                    if provisioned_product_id:
                        ...
                        provisioned_parameters = aws.get_parameters_for_stack(cloudformation, f"SC-{self.account_id}-{provisioned_product_id}")
                ...
                if need_to_provision:
                    ...
                    if provisioned_product_id:
                        stack = aws.get_stack_output_for(cloudformation, f"SC-{self.account_id}-{provisioned_product_id}")
                        stack_status = stack.get('StackStatus')
                        logger.info(f"[{self.uid}] current cfn stack_status is {stack_status}")
                        if stack_status not in ('UPDATE_COMPLETE', 'CREATE_COMPLETE',
                                                'UPDATE_ROLLBACK_COMPLETE'):
                            raise Exception(f"[{self.uid}] current cfn stack_status is {stack_status}")
                        if stack_status == 'UPDATE_ROLLBACK_COMPLETE':
                            logger.warning(f"[{self.uid}] SC-{self.account_id}-{provisioned_product_id} has a status of {stack_status}.  This may need manual resolution.")
                    if provisioned_product_id:
                        if self.should_use_product_plans:
                            provisioned_product_id = aws.provision_product_with_plan(service_catalog, self.launch_name, self.account_id, self.region, product_id, version_id, self.puppet_account_id, path_id, params_to_use, self.version, self.should_use_sns)
                        else:
                            provisioned_product_id = aws.update_provisioned_product(service_catalog, self.launch_name, self.account_id, self.region, product_id, version_id, self.puppet_account_id, path_id, params_to_use, self.version)
                    else:
                        provisioned_product_id = aws.provision_product(service_catalog, self.launch_name, self.account_id, self.region, product_id, version_id, self.puppet_account_id, path_id, params_to_use, self.version, self.should_use_sns)
                with betterboto_client.CrossAccountClientContextManager('cloudformation',
                  role, f"cfn-{self.region}-{self.account_id}", region_name=(self.region)) as (spoke_cloudformation):
                    stack_details = aws.get_stack_output_for(spoke_cloudformation, f"SC-{self.account_id}-{provisioned_product_id}")
                    ...
                    with betterboto_client.ClientContextManager('ssm') as (ssm):
                        ...
                        for output in stack_details.get('Outputs', []):
                            if output.get('OutputKey') == ssm_param_output.get('stack_output'):
                                ...
                                ssm.put_parameter_and_wait(Name=(ssm_param_output.get('param_name')),
                                  Value=(output.get('OutputValue')),
                                  Type=(ssm_param_output.get('param_type', 'String')),
                                  Overwrite=True)

                        if not found_match:
                            raise Exception(f"[{self.uid}] Could not find match for {ssm_param_output.get('stack_output')}")

                with self.output().open('w') as (f):
                    f.write(json.dumps(stack_details,
                      indent=4,
                      default=str))
...
class ProvisionProductDryRunTask(ProvisionProductTask):
    ...
    def run(self):
        ...
        with betterboto_client.CrossAccountClientContextManager('servicecatalog',
          role, f"sc-{self.region}-{self.account_id}", region_name=(self.region)) as (service_catalog):
            logger.info(f"[{self.uid}] looking for previous failures")
            path_id = aws.get_path_for_product(service_catalog, product_id, self.portfolio)
            response = service_catalog.search_provisioned_products(Filters={'SearchQuery': [
              ...
              role, f"cfn-{self.region}-{self.account_id}", region_name=(self.region)) as (cloudformation):
                ...
                if provisioning_artifact_id == version_id:
                    ...
                    if provisioned_product_id:
                        ...
                        if provisioned_parameters == params_to_use:
                            logger.info(f"[{self.uid}] params unchanged")
                            self.write_result(current_version=(self.version),
                    ...
                    if provisioning_artifact_id:
                        ...
                        current_version = ''
                    self.write_result(current_version=current_version,
                      new_version=(self.version),
                      effect=(constants.CHANGE),
    ...
    def get_current_version(self, provisioning_artifact_id, service_catalog):
        with self.input().get('product').open('r') as (f):
            product_id = json.loads(f.read()).get('product_id')
        return service_catalog.describe_provisioning_artifact(ProvisioningArtifactId=provisioning_artifact_id,
          ProductId=product_id).get('ProvisioningArtifactDetail').get('Name')

    def write_result(self, current_version, new_version, effect, notes=''):
        with self.output().open('w') as (f):
            f.write(json.dumps({'current_version':current_version, 
...
class TerminateProductTask(tasks.PuppetTask):
    ...
    def run(self):
        logger.info(f"[{self.launch_name}] {self.account_id}:{self.region} :: starting terminate try {self.try_count} of {self.retry_count}")
        with self.input().get('product').open('r') as (f):
            product_id = json.loads(f.read()).get('product_id')
        role = f"arn:aws:iam::{self.account_id}:role/servicecatalog-puppet/PuppetRole"
        with betterboto_client.CrossAccountClientContextManager('servicecatalog',
          role, f"sc-{self.region}-{self.account_id}", region_name=(self.region)) as (service_catalog):
            ...
            provisioned_product_id, provisioning_artifact_id = aws.ensure_is_terminated(service_catalog, self.launch_name, product_id)
            log_output = self.to_str_params()
            log_output.update({'provisioned_product_id': provisioned_product_id})
            for ssm_param_output in self.ssm_param_outputs:
                param_name = ssm_param_output.get('param_name')
                logger.info(f"[{self.launch_name}] {self.account_id}:{self.region} :: deleting SSM Param: {param_name}")
                with betterboto_client.ClientContextManager('ssm') as (ssm):
                    try:
                        ssm.delete_parameter(Name=param_name)
                        logger.info(f"[{self.launch_name}] {self.account_id}:{self.region} :: deleting SSM Param: {param_name}")
                    except ssm.exceptions.ParameterNotFound:
                        logger.info(f"[{self.launch_name}] {self.account_id}:{self.region} :: SSM Param: {param_name} not found")

            with self.output().open('w') as (f):
                f.write(json.dumps(log_output,
                  indent=4,
                  default=str))
...
class TerminateProductDryRunTask(tasks.PuppetTask):
    ...
    def run(self):
        logger.info(f"[{self.launch_name}] {self.account_id}:{self.region} :: starting dry run terminate try {self.try_count} of {self.retry_count}")
        with self.input().get('product').open('r') as (f):
            product_id = json.loads(f.read()).get('product_id')
        role = f"arn:aws:iam::{self.account_id}:role/servicecatalog-puppet/PuppetRole"
        with betterboto_client.CrossAccountClientContextManager('servicecatalog',
          role, f"sc-{self.region}-{self.account_id}", region_name=(self.region)) as (service_catalog):
            ...
            if r is None:
                ...
                provisioned_product_name = service_catalog.describe_provisioning_artifact(ProvisioningArtifactId=(r.get('ProvisioningArtifactId')),
                  ProductId=product_id).get('ProvisioningArtifactDetail').get('Name')
                if r.get('Status') != 'TERMINATED':
                    self.write_result(provisioned_product_name,
                      '-', (constants.CHANGE), notes='The product would be terminated')
                else:
                    self.write_result('-',
...
class ResetProvisionedProductOwnerTask(tasks.PuppetTask):
    ...
    def run(self):
        ...
        with betterboto_client.CrossAccountClientContextManager('servicecatalog',
          role, f"sc-{self.region}-{self.account_id}", region_name=(self.region)) as (service_catalog):
            logger.info(f"[{logger_prefix} :: Checking if existing provisioned product exists")
            provisioned_product_search_result = service_catalog.search_provisioned_products(AccessLevelFilter={'Key':'Account', 
             'Value':'self'},
              Filters={'SearchQuery': [
                             f"name:{self.launch_name}"]}).get('ProvisionedProducts', [])
            if provisioned_product_search_result:
                ...
                service_catalog.update_provisioned_product_properties(ProvisionedProductId=provisioned_product_id,
                  ProvisionedProductProperties={'OWNER': f"arn:aws:iam::{self.account_id}:role/servicecatalog-puppet/PuppetRole"})
...
class LaunchTask(tasks.PuppetTask):
    ...
    def requires(self):
        ...
        return {'other_launches': [self.__class__(launch_name=dependency, manifest=(self.manifest), puppet_account_id=(self.puppet_account_id), should_use_sns=(self.should_use_sns), should_use_product_plans=(self.should_use_product_plans), include_expanded_from=(self.include_expanded_from), single_account=(self.single_account), is_dry_run=(self.is_dry_run)) for dependency in launch.get('depends_on', [])]}
            ...
            if self.single_account is not None:
                if task_def.get('account_id') != self.single_account:
                    ...
                    if task_status == constants.PROVISIONED:
                        ...
                        if self.is_dry_run:
                            provisions.append(ProvisionProductDryRunTask(**provisioning_parameters))
                        else:
                            provisions.append(ProvisionProductTask(**provisioning_parameters))
                if task_status == constants.TERMINATED:
                    ...
                    if self.is_dry_run:
                        provisions.append(TerminateProductDryRunTask(**terminating_parameters))
                    else:
    ...
    def run(self):
        launch_tasks_def = manifest_utils_for_launches.generate_launch_task_defs_for_launch(self.launch_name, self.manifest, self.puppet_account_id, self.should_use_sns, self.should_use_product_plans, self.include_expanded_from, self.single_account, self.is_dry_run)
...
class SpokeLocalPortfolioTask(tasks.PuppetTask):
    ...
    def params_for_results_display(self):
        return {'spoke_local_portfolio_name': self.spoke_local_portfolio_name}

    def requires(self):
        spoke_local_portfolio = self.manifest.get('spoke-local-portfolios').get(self.spoke_local_portfolio_name)
        return {'other_launches': [LaunchTask(launch_name=dependency, manifest=(self.manifest), puppet_account_id=(self.puppet_account_id), should_use_sns=(self.should_use_sns), should_use_product_plans=(self.should_use_product_plans), include_expanded_from=(self.include_expanded_from), single_account=(self.single_account), is_dry_run=(self.is_dry_run)) for dependency in spoke_local_portfolio.get('depends_on', [])]}
    ...
    def generate_tasks(self, task_defs):
        ...
        tasks = []
        for task_def in task_defs:
            product_generation_method = task_def.get('product_generation_method', 'copy')
            if task_def.get('status') == constants.SPOKE_LOCAL_PORTFOLIO_STATUS_TERMINATED:
                tasks.append(portfoliomanagement.DeletePortfolio(account_id=(task_def.get('account_id')),
                ...
                create_spoke_local_portfolio_task_params = {'account_id':task_def.get('account_id'),  'region':task_def.get('region'), 
                 'portfolio':portfolio, 
                 'organization':task_def.get('organization')}
                create_spoke_local_portfolio_task = (portfoliomanagement.CreateSpokeLocalPortfolioTask)(**create_spoke_local_portfolio_task_params)
                tasks.append(create_spoke_local_portfolio_task)
                create_spoke_local_portfolio_task_as_dependency_params = {'account_id':task_def.get('account_id'), 
                 'region':task_def.get('region'), 
                 'portfolio':portfolio, 
                 'organization':task_def.get('organization')}
                if len(task_def.get('associations', [])) > 0:
                    create_associations_for_portfolio_task = (portfoliomanagement.CreateAssociationsForPortfolioTask)(**create_spoke_local_portfolio_task_as_dependency_params, **{'associations':task_def.get('associations'), 
                     'puppet_account_id':task_def.get('puppet_account_id'), 
                ...
                if product_generation_method == 'import':
                    import_into_spoke_local_portfolio_task = (portfoliomanagement.ImportIntoSpokeLocalPortfolioTask)(**create_spoke_local_portfolio_task_as_dependency_params, **{'puppet_account_id': task_def.get('puppet_account_id')})
                    tasks.append(import_into_spoke_local_portfolio_task)
                else:
                    copy_into_spoke_local_portfolio_task = (portfoliomanagement.CopyIntoSpokeLocalPortfolioTask)(**create_spoke_local_portfolio_task_as_dependency_params, **{'puppet_account_id': task_def.get('puppet_account_id')})
                    tasks.append(copy_into_spoke_local_portfolio_task)
            if len(launch_constraints) > 0:
                create_launch_role_constraints_for_portfolio_task_params = {'launch_constraints':launch_constraints,  'puppet_account_id':task_def.get('puppet_account_id'), 
                 'should_use_sns':task_def.get('should_use_sns')}
                create_launch_role_constraints_for_portfolio = (portfoliomanagement.CreateLaunchRoleConstraintsForPortfolio)(**create_spoke_local_portfolio_task_as_dependency_params, **create_launch_role_constraints_for_portfolio_task_params, **{'product_generation_method': product_generation_method})
    ...
    def run(self):
        self.info('started')
        tasks_defs = manifest_utils_for_spoke_local_portfolios.generate_spoke_local_portfolios_tasks_for_spoke_local_portfolio(self.spoke_local_portfolio_name, self.manifest, self.puppet_account_id, self.should_use_sns, self.should_use_product_plans, self.include_expanded_from, self.single_account, self.is_dry_run)
