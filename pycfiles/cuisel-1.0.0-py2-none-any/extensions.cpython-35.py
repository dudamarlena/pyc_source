...
# Embedded file name: build/bdist.linux-x86_64/egg/cuiows/wsproto/extensions.py
# Compiled at: 2017-01-22 09:55:14
# Size of source mod 2**32: 8200 bytes
...
class PerMessageDeflate(Extension):
    ...
    def __init__(self, client_no_context_takeover=False, client_max_window_bits=15, server_no_context_takeover=False, server_max_window_bits=15):
    ...
    def frame_inbound_header(self, proto, opcode, rsv, payload_length):
        ...
        if rsv[0] and opcode.iscontrol():
            return CloseReason.PROTOCOL_ERROR
        if rsv[0] and opcode is Opcode.CONTINUATION:
            return CloseReason.PROTOCOL_ERROR
    ...
    def frame_inbound_complete(self, proto, fin):
        if not fin or not self._inbound_compressed or not self._inbound_is_compressible:
            return
            try:
