...
# [GCC 8.4.0]
# Embedded file name: /Users/eric/github/local/gitberg/gitenberg/metadata/pandata.py
# Compiled at: 2020-01-02 15:12:42
"""
A wrapper class that makes it easier to work with json or yaml formatted metadata for books
"""
import re, six, yaml, copy, requests, datetime, unicodedata
...
class Pandata(object):
    ...
    def set_edition_id(self):
        if 'edition_identifiers' not in self.metadata:
        ...
        if not base:
            try:
                ...
                base = 'repo:' + unicodestr(self._repo)
    ...
    def _edition(self):
        if self.metadata.get('_edition', ''):
            ...
            if self.get_one_identifier('isbn'):
                return unicodestr(self.get_one_identifier('isbn'))
            if self._repo:
