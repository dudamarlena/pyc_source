...
# [GCC 8.4.0]
# Embedded file name: /Users/eric/github/local/gitberg/gitenberg/util/catalog.py
# Compiled at: 2020-01-05 17:42:23
"""

"""
import csv, errno, json, logging, os, re, shutil, tarfile, time
from six import text_type as unicodestr
import dateutil.parser, pytz, requests
...
class BookMetadata(Pandata):
    ...
    def __init__(self, book, rdf_library='./rdf_library', enrich=True, datafile=None):
        if datafile:
            ...
            self.rdf_path = ('{0}/{1}/pg{1}.rdf').format(book.rdf_library, book.book_id)
            return
        self.book = book
        try:
        ...
        self.rdf_path = ('{0}/{1}/pg{1}.rdf').format(rdf_library, self.book.book_id)
        self.parse_rdf()
        if enrich:
            self.enrich()
...
class Rdfcache(object):
    ...
    def download_rdf(self, force=False):
        ...
        if self.downloading:
            return True
        if not force and os.path.exists(RDF_PATH) and time.time() - os.path.getmtime(RDF_PATH) < RDF_MAX_AGE:
            return False
    ...
    def get_repos_to_upload(self):
        ...
        pg_ids = []
        while more:
            new_rdffile = os.path.join(self.rdf_library_dir, 'cache', 'epub', unicodestr(pg_id), ('pg{}.rdf').format(pg_id))
            if os.path.exists(new_rdffile):
                pg_ids.append(pg_id)
                pg_id += 1
            elif pg_id in missing_pgid:
                pg_id += 1
            else:
                more = False
