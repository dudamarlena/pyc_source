...
# Size of source mod 2**32: 5530 bytes
"""
...
class GithubRepo:
    ...
    def push(self):
        self.book.local_repo.git.remote('origin').push((self.book.local_repo.git.refs.master),
          tags=True)
    ...
    def create_repo(self):
        try:
            self.repo = self.org.create_repository((self.book.repo_name),
    ...
    def update_repo(self):
        try:
            self.repo = self.github.repository(self.org_name, self.book.repo_name)
            self.repo.edit((self.book.repo_name),
              description=(self.format_desc()),
              homepage=(self.org_homepage),
              private=False,
              has_issues=True,
    ...
    def repo_token(self):
        if self._repo_token is not None:
            ...
            token = github3.authorize((config.data['gh_user']),
