...
# [GCC 8.4.0]
# Embedded file name: /Users/eric/github/local/gitberg/gitenberg/metadata/pg_rdf.py
# Compiled at: 2020-01-02 15:06:43
import rdflib, string, yaml, six
...
def get_url(key, val, entities=None):
    if isinstance(val, list):
        return (
         key, [ get_url(key, item, entities=None)[1] for item in val ])
    try:
        return (
         key, unicodestr(val['@id']))
    except KeyError:
        return
...
def get_value(key, val, entities=None):
    if isinstance(val, list):
        return (
         key, [ get_value(key, item, entities=None)[1] for item in val ])
    try:
        return (
         key, val['@value'])
    except KeyError:
...
pandata_map = {'@type': None, '@id': (
         'url', get_id), 
   'cc:license': None, 
   'dcterms:alternative': 'alternative_title', 
   'dcterms:creator': (
                     'creator/author', set_listable_entity), 
   'dcterms:description': 'description', 
   'dcterms:hasFormat': None, 
   'dcterms:issued': (
                    'gutenberg_issued', get_value), 
   'dcterms:language': (
                      'language', get_value), 
   ...
   'dcterms:title': 'title', 
   'dcterms:type': (
                  'gutenberg_type', get_value), 
   'pgterms:alias': (
                   'alias', listable), 
   'pgterms:birthdate': 'birthdate', 
   'pgterms:bookshelf': (
                       'gutenberg_bookshelf', get_value), 
   ...
   'pgterms:marc901': (
                     'covers', get_imagefile), 
   'pgterms:marc902': (
                     'titlepage_image', get_imagefile), 
   'pgterms:marc903': (
                     'back_cover', get_imagefile), 
...
for key in marc_rels.keys():
    pandata_map['marcrel:' + key] = (
     ('{}/{}').format('creator' if key in other_creators else 'contributor', marc_rels[key]), set_listable_entity)

pandata_adders = [identifiers]
postprocessors = [cover_mover]
context = {'cc': 'http://web.resource.org/cc/', 
