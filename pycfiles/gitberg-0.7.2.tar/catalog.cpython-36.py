...
# Compiled at: 2019-05-01 09:10:45
# Size of source mod 2**32: 5992 bytes
"""

"""
import csv, errno, json, logging, os, re, shutil, tarfile, time, dateutil.parser, pytz, requests
...
class Rdfcache(object):
        ...
        if self.downloading:
            ...
            if not force:
                if os.path.exists(RDF_PATH):
                    if time.time() - os.path.getmtime(RDF_PATH) < RDF_MAX_AGE:
                        return False
    ...
    def get_repos_to_upload(self):
        ...
        pg_ids = []
        while more:
            new_rdffile = os.path.join(self.rdf_library_dir, 'cache', 'epub', unicode(pg_id), 'pg{}.rdf'.format(pg_id))
