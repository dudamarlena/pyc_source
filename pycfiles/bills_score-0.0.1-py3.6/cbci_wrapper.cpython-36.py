...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bills_scoring/cbci_wrapper.py
# Compiled at: 2018-02-22 04:04:50
# Size of source mod 2**32: 2499 bytes
from .settings import CBCI_SERVER
import requests, datetime, base64, logging, fire, json
logger = logging.getLogger(__name__)

class Connect:
    __doc__ = ' API Wrapper for CBCI goldmine\n\n       Usage:\n           from cbci_wrapper import Connect\n           cbci_connect = Connect()\n           cbci_connect.get_transactions(\n                        biller_code="MWSIN",\n                        account_no="54537170")\n\n    '

    def __init__(self, url=CBCI_SERVER['URL'], username=CBCI_SERVER['USERNAME'], secret=CBCI_SERVER['SECRET']):
        self.url = url
        self.secret = secret
    ...
    def get_jwt(self):
        ...
        headers = {'Authorization': 'Basic {}'.format(self.authorization.decode())}
    ...
    def get_transactions(self, biller_code, account_no):
        ...
        Parameter:
            ...
            account_no
        """
        logging.info('Grabing Transactions for {}: {}'.format(biller_code, account_no))
        url = '{}/api/getdata'.format(self.url)
        data = {'billerCode':biller_code, 
