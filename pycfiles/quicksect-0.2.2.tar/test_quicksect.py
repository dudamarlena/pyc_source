...
# [GCC 8.4.0]
# Embedded file name: /home/brentp/src/quicksect/tests/test_quicksect.py
# Compiled at: 2018-07-19 11:29:27
...
class LotsaTestCase(unittest.TestCase):
    ...
    def test_find(self):
        ...
        for t in range(250):
            ...
            for feat in results:
                self.assertTrue(feat.end >= start and feat.end <= stop or feat.start <= stop and feat.start >= start)
...
def brute_force_find_left(intervals, f, max_dist, n):
    r = [ x for x in brute_force_find(intervals, 0, f.start) if x.end < f.start and distance(x, f) <= max_dist
        ]
...
def brute_force_find_right(intervals, f, max_dist, n):
    r = [ x for x in brute_force_find(intervals, f.end, 99999999999) if x.start > f.start and distance(x, f) <= max_dist
        ]
