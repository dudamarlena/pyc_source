...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/gui/text.py
# Compiled at: 2019-08-25 05:53:55
# Size of source mod 2**32: 19380 bytes
import tty, sys, curses, datetime, locale
from decimal import Decimal
import getpass, logging, electrum
...
class ElectrumGui:
    ...
    def get_string(self, y, x):
        self.set_cursor(1)
        curses.echo()
        self.stdscr.addstr(y, x, '                    ', curses.A_REVERSE)
    ...
    def edit_str(self, target, c, is_num=False):
        ...
        if c in (8, 127, 263) and target:
            ...
            if not is_num or cc in '0123456789.':
                target += cc
        return target
    ...
    def network_dialog(self):
        ...
        out = self.run_dialog('Network', [
         {'label':'server', 
          'type':'str',  'value':srv},
         {'label':'proxy', 
          'type':'str',  'value':self.config.get('proxy', '')}],
          buttons=1)
        if out:
            if out.get('server'):
