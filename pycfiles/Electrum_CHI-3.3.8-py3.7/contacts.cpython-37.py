...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/contacts.py
# Compiled at: 2019-08-25 05:39:58
    ...
    def resolve(self, k):
        ...
        if out:
            ...
            return {'address':address, 
             ...
             'type':'openalias', 
             'validated':validated}
        raise Exception('Invalid address or alias', k)
    ...
    def resolve_openalias(self, url):
        ...
        for record in records:
            ...
            if string.startswith('oa1:' + prefix):
                address = self.find_regex(string, 'recipient_address=([A-Za-z0-9]+)')
                name = self.find_regex(string, 'recipient_name=([^;]+)')
                if not name:
                    name = address
                if not address:
                    continue
                return (
                 address, name, validated)
