...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/plugins/trezor/trezor.py
# Compiled at: 2019-08-25 06:23:46
# Size of source mod 2**32: 20207 bytes
...
from hw_wallet.plugin import is_any_tx_output_on_change_branch, trezor_validate_op_return_output_and_get_data, LibraryFoundButUnusable, OutdatedHwFirmwareException
_logger = get_logger(__name__)
try:
    import trezorlib, trezorlib.transport
    from trezorlib.transport.bridge import BridgeTransport, call_bridge
    from .clientbase import TrezorClientBase
    from trezorlib.messages import RecoveryDeviceType, HDNodeType, HDNodePathType, InputScriptType, OutputScriptType, MultisigRedeemScriptType, TxInputType, TxOutputType, TxOutputBinType, TransactionType, SignTx
...
class TrezorPlugin(HW_PluginBase):
    ...
    def initialize_device(self, device_id, wizard, handler):
        msg = _('Choose how you want to initialize your {}.\n\nThe first two methods are secure as no secret information is entered into your computer.\n\nFor the last two methods you input secrets on your keyboard and upload them to your {}, and so you should only do those on a computer you know to be trustworthy and free of malware.').format(self.device, self.device)
        choices = [
         (
    ...
    def _initialize_device(self, settings: TrezorInitSettings, method, device_id, wizard, handler):
        if method == TIM_RECOVER:
            if settings.recovery_type == RECOVERY_TYPE_SCRAMBLED_WORDS:
            ...
            if not client:
                ...
                strength_from_word_count = {12:128, 
                 18:192,  24:256}
                client.reset_device(strength=(strength_from_word_count[settings.word_count]),
                ...
                if method == TIM_RECOVER:
                    client.recover_device(recovery_type=(settings.recovery_type),
    ...
    def _make_node_path(self, xpub, address_n):
        ...
        node = HDNodeType(depth=(bip32node.depth),
    ...
    def show_address(self, wallet, address, keystore=None):
        if keystore is None:
        ...
        elif not self.show_address_helper(wallet, address, keystore):
            return
            ...
            if len(xpubs) > 1:
                ...
                multisig = self._make_multisig(wallet.m, [(xpub, deriv_suffix) for _, xpub in sorted_pairs])
        else:
            multisig = None
        client = self.get_client(keystore)
    ...
    def tx_inputs(self, tx, xpub_path, for_sig=False):
        ...
        for txin in tx.inputs():
            ...
            if txin['type'] == 'coinbase':
                prev_hash = b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
            ...
            txinputtype.sequence = txin.get('sequence', 4294967294)
    ...
    def electrum_tx_to_txtype(self, tx, xpub_path):
        ...
        t.bin_outputs = [TxOutputBinType(amount=(vout['value']), script_pubkey=(bfh(vout['scriptPubKey']))) for vout in d['outputs']]
        return t