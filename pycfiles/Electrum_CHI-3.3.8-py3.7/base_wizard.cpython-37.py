...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/base_wizard.py
# Compiled at: 2019-08-25 05:38:38
# Size of source mod 2**32: 29238 bytes
...
from .wallet import Imported_Wallet, Standard_Wallet, Multisig_Wallet, wallet_types, Wallet, Abstract_Wallet
from .storage import WalletStorage, STO_EV_USER_PW, STO_EV_XPUB_PW, get_derivation_used_for_hw_device_encryption
from .i18n import _
...
class BaseWizard(Logger):
    ...
    def choose_keystore(self):
        if not self.wallet_type in ('standard', 'multisig'):
            ...
            if self.wallet_type == 'standard' or i == 0:
                ...
                self.is_kivy or choices.append(('choose_hw_device', _('Use a hardware device')))
            else:
    ...
    def on_import(self, text):
        if keystore.is_address_list(text):
            ...
            if keystore.is_private_key_list(text):
                self.data['addresses'] = {}
    ...
    def choose_hw_device(self, purpose=HWD_SETUP_NEW_WALLET, *, storage=None):
        ...
        try:
            ...
            for splugin in supported_plugins:
                ...
                if not plugin:
                    ...
                    continue
                try:
                    device_infos = devmgr.unpaired_device_infos(None, plugin, devices=scanned_devices, include_failing_clients=True)
        ...
        if not devices:
            ...
            if sys.platform == 'win32':
                msg += _('If your device is not detected on Windows, go to "Settings", "Devices", "Connected devices", and do "Remove device". Then, plug your device again.') + '\n'
        ...
        self.choice_dialog(title=title, message=msg, choices=choices, run_next=(lambda *args: (self.on_device)(*args, purpose=purpose, storage=storage)))
    ...
    def derivation_and_script_type_dialog(self, f):
        ...
        while True:
            try:
                self.choice_and_line_dialog(run_next=f,
    ...
    def on_restore_seed(self, seed, is_bip39, is_ext):
        ...
        if self.seed_type == 'bip39':
            ...
            if self.seed_type in ('standard', 'segwit'):
                f = lambda passphrase: self.run('create_keystore', seed, passphrase)
                self.passphrase_dialog(run_next=f, is_restoring=True) if is_ext else f('')
            else:
                if self.seed_type == 'old':
                    self.run('create_keystore', seed, '')
                else:
                    if mnemonic.is_any_2fa_seed_type(self.seed_type):
                        self.load_2fa()
    ...
    def on_keystore(self, k):
        ...
        if self.wallet_type == 'standard':
            ...
            if self.wallet_type == 'multisig':
                if not has_xpub:
                    raise AssertionError
                elif t1 not in ('standard', 'p2wsh', 'p2wsh-p2sh'):
                    self.show_error(_('Wrong key type') + ' %s' % t1)
                    self.run('choose_keystore')
                    return
    ...
    def create_wallet(self):
        ...
        if self.wallet_type == 'standard' and isinstance(self.keystores[0], keystore.Hardware_KeyStore):
            ...
            self.request_storage_encryption(run_next=(lambda encrypt_storage: self.on_password(password,
              encrypt_storage=encrypt_storage,
              storage_enc_version=STO_EV_XPUB_PW,
              encrypt_keystore=False)))
        else:
            self.reset_stack()
            self.request_password(run_next=(lambda password, encrypt_storage: self.on_password(password,
              encrypt_storage=encrypt_storage,
    ...
    def on_password(self, password, *, encrypt_storage, storage_enc_version=STO_EV_USER_PW, encrypt_keystore):
        ...
        self.pw_args = (
         password, encrypt_storage, storage_enc_version)
        self.terminate()
