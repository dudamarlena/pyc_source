...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/bip32.py
...
def CKD_priv(parent_privkey: bytes, parent_chaincode: bytes, child_index: int) -> Tuple[(bytes, bytes)]:
    ...
    return _CKD_priv(parent_privkey=parent_privkey, parent_chaincode=parent_chaincode,
...
def CKD_pub(parent_pubkey: bytes, parent_chaincode: bytes, child_index: int) -> Tuple[(bytes, bytes)]:
    ...
    return _CKD_pub(parent_pubkey=parent_pubkey, parent_chaincode=parent_chaincode,
      child_index=(bfh(rev_hex(int_to_hex(child_index, 4)))))
...
class BIP32Node(NamedTuple):
    ...
    def from_xkey(cls, xkey: str, *, net=None) -> 'BIP32Node':
        if net is None:
            ...
            if is_private:
                eckey = ecc.ECPrivkey(xkey[46:])
            else:
                eckey = ecc.ECPubkey(xkey[45:])
        return BIP32Node(xtype=xtype, eckey=eckey,
          chaincode=chaincode,
          depth=depth,
    ...
    def from_rootseed(cls, seed: bytes, *, xtype: str) -> 'BIP32Node':
        ...
        master_k = I[0:32]
        master_c = I[32:]
        return BIP32Node(xtype=xtype, eckey=(ecc.ECPrivkey(master_k)),
          chaincode=master_c)

    def to_xprv(self, *, net=None) -> str:
        if not self.is_private():
            raise Exception('cannot serialize as xprv; private key missing')
        payload = xprv_header((self.xtype), net=net) + bytes([self.depth]) + self.fingerprint + self.child_number + self.chaincode + bytes([0]) + self.eckey.get_secret_bytes()
        assert len(payload) == 78, f"unexpected xprv payload len {len(payload)}"
        return EncodeBase58Check(payload)

    def to_xpub(self, *, net=None) -> str:
        payload = xpub_header((self.xtype), net=net) + bytes([self.depth]) + self.fingerprint + self.child_number + self.chaincode + self.eckey.get_public_key_bytes(compressed=True)
    ...
    def subkey_at_private_derivation(self, path: Union[(str, Iterable[int])]) -> 'BIP32Node':
        ...
        return BIP32Node(xtype=(self.xtype), eckey=(ecc.ECPrivkey(privkey)),
          chaincode=chaincode,
          depth=depth,
          fingerprint=fingerprint,
          child_number=child_number)
    ...
    def subkey_at_public_derivation(self, path: Union[(str, Iterable[int])]) -> 'BIP32Node':
        if path is None:
            ...
            if isinstance(path, str):
                path = convert_bip32_path_to_list_of_uint32(path)
            return path or self.convert_to_public()
        depth = self.depth
        chaincode = self.chaincode
        ...
        fingerprint = hash_160(parent_pubkey)[0:4]
        child_number = child_index.to_bytes(length=4, byteorder='big')
        return BIP32Node(xtype=(self.xtype), eckey=(ecc.ECPubkey(pubkey)),
          chaincode=chaincode,
          depth=depth,
          fingerprint=fingerprint,
          child_number=child_number)
...
def convert_bip32_path_to_list_of_uint32(n: str) -> List[int]:
    ...
    for x in n:
        ...
        if not x.endswith("'"):
            ...
            if x.startswith('-'):
                if prime:
                    raise ValueError('bip32 path child index is signalling hardened level in multiple ways')
                prime = BIP32_PRIME
