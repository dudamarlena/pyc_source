...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/wallet.py
# Compiled at: 2019-08-25 06:38:49
# Size of source mod 2**32: 87140 bytes
import os, sys, random, time, json, copy, errno, traceback
from functools import partial
from numbers import Number
from decimal import Decimal
from typing import TYPE_CHECKING, List, Optional, Tuple, Union, NamedTuple, Sequence
from .i18n import _
from .util import NotEnoughFunds, UserCancelled, profiler, format_satoshis, format_fee_satoshis, NoDynamicFeeEstimates, WalletFileException, BitcoinException, InvalidPassword, format_time, timestamp_to_datetime, Satoshis, Fiat, bfh, bh2u, TxMinedInfo, quantize_feerate
...
from .names import get_default_name_tx_label, OP_NAME_REGISTER, OP_NAME_UPDATE
...
def sweep(privkeys, network: 'Network', config: 'SimpleConfig', recipient, fee=None, imax=100, *, locktime=None, tx_version=None):
    inputs, keypairs = sweep_preparations(privkeys, network, imax)
    total = sum((i.get('value') for i in inputs))
    if fee is None:
        outputs = [
         TxOutput(TYPE_ADDRESS, recipient, total)]
...
class InternalAddressCorruption(Exception):
    ...
    def __str__(self):
        return _('Wallet file corruption detected. Please restore your wallet from seed, and compare the addresses in both files')
...
class Abstract_Wallet(AddressSynchronizer):
    ...
    def calc_unused_change_addresses(self):
        with self.lock:
            ...
            self._unused_change_addresses = [addr for addr in addrs if self.get_address_history_len(addr) == 0]
    ...
    def export_private_key(self, address, password):
        if self.is_watching_only():
            ...
            assert self.is_mine(address), _('Address not in wallet.') + f" {address}"
    ...
    def get_tx_info(self, tx) -> TxWalletDetails:
        ...
        if tx.is_complete():
            if self.db.get_transaction(tx_hash):
                ...
                if tx_mined_status.height > 0:
                    ...
                    if tx_mined_status.height in (TX_HEIGHT_UNCONF_PARENT, TX_HEIGHT_UNCONFIRMED):
                        status = _('Unconfirmed')
                        if fee is None:
                            fee = self.db.get_tx_fee(tx_hash)
                        if fee:
                            if self.network:
                                if self.network.config.has_fee_mempool():
        ...
        if is_relevant:
            ...
            amount = None
        return TxWalletDetails(txid=tx_hash,
          status=status,
          label=label,
    ...
    def get_spendable_coins(self, domain, config, *, nonlocal_only=False, include_names=False, only_uno_txids=None, only_uno_identifiers=None):
        ...
        utxos = self.get_utxos(domain, excluded_addresses=(self.frozen_addresses),
          mature_only=True,
    ...
    def get_frozen_balance(self):
        ...
        c2, u2, x2 = self.get_balance(excluded_addresses=(self.frozen_addresses), excluded_coins=(self.frozen_coins))
    ...
    @profiler
    def get_full_history(self, domain=None, from_timestamp=None, to_timestamp=None, fx=None, show_addresses=False, show_fees=False, from_height=None, to_height=None):
        if not from_timestamp is not None:
            ...
            for tx_hash, tx_mined_status, value, balance in h:
                ...
                if from_timestamp:
                    if (timestamp or now) < from_timestamp:
                        continue
                if to_timestamp:
                    if (timestamp or now) >= to_timestamp:
                        continue
                ...
                if to_height is not None:
                    ...
                    if show_addresses:
                        ...
                        item['outputs'] = list(map(lambda x: {'address':x.address,  'value':Satoshis(x.value)}, tx.get_outputs_for_UI()))
                    if value is None:
                        continue
                    if value < 0:
                        expenditures += -value
        ...
        for tx_hash, tx_mined_status, delta, balance in self.get_history():
            if tx_mined_status.conf > 0:
                ...
                if not tx:
                    continue
                txid = tx.txid()
                if any([self.is_mine(o.address) and self.db.get_spent_outpoint(txid, output_idx) for output_idx, o in enumerate(tx.outputs())]):
                    continue
    ...
    def make_unsigned_transaction(self, coins, outputs, config, fixed_fee=None, change_addr=None, is_sweep=False, name_inputs=[]):
        i_max = None
        name_op = None
        ...
        if fixed_fee is None:
            ...
            if fixed_fee is None:
                ...
                if isinstance(fixed_fee, Number):
                    fee_estimator = lambda size: fixed_fee
                else:
                    if callable(fixed_fee):
        ...
        if i_max is None:
            ...
            txi += name_inputs
            tx = coin_chooser.make_tx(coins, txi, outputs[:] + txo, change_addrs, fee_estimator, self.dust_threshold())
        else:
    ...
    def mktx(self, outputs, password, config, fee=None, change_addr=None, domain=None, rbf=False, nonlocal_only=False, *, tx_version=None):
    ...
    def wait_until_synchronized(self, callback=None):
        ...
        def wait_for_wallet():
            ...
            while not self.is_up_to_date():
                if callback:
                    msg = '{}\n{} {}'.format(_('Please wait...'), _('Addresses generated:'), len(self.get_addresses()))
                    callback(msg)
                time.sleep(0.1)
    ...
    def bump_fee(self, *, tx, new_fee_rate, config) -> Transaction:
        ...
        if actual_fee + 1 < target_min_fee:
            raise Exception(f"bump_fee fee target was not met (method: {method_used}). got {actual_fee}, expected >={target_min_fee}. target rate was {new_fee_rate}")
    ...
    def _bump_fee_through_coinchooser(self, *, tx, new_fee_rate, config):
        ...
        coin_chooser = coinchooser.get_coin_chooser(config)
        try:
            return coin_chooser.make_tx(coins, old_inputs, fixed_outputs, change_addrs, fee_estimator, self.dust_threshold())
    ...
    def get_input_tx(self, tx_hash, *, ignore_network_issues=False):
        ...
        if not tx:
            if self.network:
                try:
                    raw_tx = self.network.run_from_another_thread(self.network.get_transaction(tx_hash, timeout=10))
                except NetworkException as e:
                    try:
                        self.logger.info(f"got network error getting input txn. err: {repr(e)}. txid: {tx_hash}. if you are intentionally offline, consider using the --offline flag")
    ...
    def get_unused_addresses(self):
        ...
        return [addr for addr in domain if not self.db.get_addr_history(addr) if addr not in self.receive_requests.keys()]
    ...
    def get_receiving_address(self):
        ...
        choice = domain[0]
        for addr in domain:
            if self.db.get_addr_history(addr) or addr not in self.receive_requests.keys():
                return addr
                choice = addr
...
class Simple_Wallet(Abstract_Wallet):
    ...
    def _update_password_for_keystore(self, old_pw, new_pw):
        if self.keystore:
            if self.keystore.may_have_password():
...
class Imported_Wallet(Simple_Wallet):
    ...
    def import_private_keys(self, keys: List[str], password: Optional[str], *, write_to_disk=True) -> Tuple[(List[str], List[Tuple[(str, str)]])]:
        good_addr = []
        bad_keys = []
        for key in keys:
            try:
...
class Simple_Deterministic_Wallet(Simple_Wallet, Deterministic_Wallet):
    __doc__ = ' Deterministic Wallet with a single pubkey per address '
...
class Multisig_Wallet(Deterministic_Wallet):
    ...
    def add_input_sig_info(self, txin, address):
        ...
        if x_pubkeys_actual:
            if set(x_pubkeys_actual) == set(x_pubkeys_expected):
                return
...
wallet_types = [
 'standard', 'multisig', 'imported']

def register_wallet_type(category):
    wallet_types.append(category)


wallet_constructors = {'standard':Standard_Wallet, 
 'old':Standard_Wallet, 
...
class Wallet(object):
    __doc__ = 'The main wallet "entry point".\n    This class is actually a factory that will return a wallet of the correct\n    type when passed a WalletStorage instance.'
...
def restore_wallet_from_text(text, *, path, network=None, passphrase=None, password=None, encrypt_file=True, gap_limit=None):
    ...
    if keystore.is_address_list(text):
        ...
        if keystore.is_private_key_list(text, allow_spaces_inside_key=False):
            ...
            if storage.file_exists():
                raise AssertionError('file was created too soon! plaintext keys might have been written to disk')
            wallet.update_password(old_pw=None, new_pw=password, encrypt_storage=encrypt_file)
            wallet.synchronize()
            if network:
                ...
                msg = 'This wallet was restored offline. It may contain more addresses than displayed. Start a daemon (not offline) to sync history.'
        wallet.storage.write()
