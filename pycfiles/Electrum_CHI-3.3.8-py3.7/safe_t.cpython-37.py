...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/plugins/safe_t/safe_t.py
# Compiled at: 2019-08-25 06:19:28
# Size of source mod 2**32: 21275 bytes
...
class SafeTPlugin(HW_PluginBase):
    ...
    def enumerate(self):
        ...
        return [Device(path=(d.get_path()), interface_number=(-1), id_=(d.get_path()), product_key='Safe-T mini', usage_page=0, transport_ui_string=(d.get_path())) for d in devices]
    ...
    def _initialize_device(self, settings, method, device_id, wizard, handler):
        ...
        if method == TIM_RECOVER:
            ...
            if not client:
                ...
                u2f_counter = 0
                skip_backup = False
                client.reset_device(True, strength, passphrase_protection, pin_protection, label, language, u2f_counter, skip_backup)
            else:
                if method == TIM_RECOVER:
                    word_count = 6 * (item + 2)
                    client.step = 0
    ...
    def _make_node_path(self, xpub, address_n):
        ...
        node = self.types.HDNodeType(depth=(bip32node.depth),
    ...
    def sign_transaction(self, keystore, tx, prev_tx, xpub_path):
        ...
        signatures = client.sign_tx((self.get_coin_name()), inputs, outputs, lock_time=(tx.locktime),
    ...
    def show_address(self, wallet, address, keystore=None):
        if keystore is None:
            ...
            multisig = self.types.MultisigRedeemScriptType(pubkeys=pubkeys,
              signatures=([
             b''] * wallet.n),
              m=(wallet.m))
            script_type = self.get_safet_input_script_type(wallet.txin_type)
            ...
            if txin['type'] == 'coinbase':
                ...
                if for_sig:
                    ...
                    if len(x_pubkeys) == 1:
                        ...
                        multisig = self.types.MultisigRedeemScriptType(pubkeys=pubkeys,
                          signatures=(list(map(lambda x:                         if x:
bfh(x)[:-1] # Avoid dead code: b'', txin.get('signatures')))),
                          m=(txin.get('num_sig')))
                        script_type = self.get_safet_input_script_type(txin['type'])
                        txinputtype = self.types.TxInputType(script_type=script_type,
                          multisig=multisig)
                        for x_pubkey in x_pubkeys:
    ...
    def tx_outputs(self, derivation, tx):
        ...
        def create_output_by_derivation():
            ...
            if len(xpubs) == 1:
                ...
                txoutputtype = self.types.TxOutputType(amount=amount,
