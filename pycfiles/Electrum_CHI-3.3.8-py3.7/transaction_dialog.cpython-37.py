...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/gui/qt/transaction_dialog.py
# Compiled at: 2019-08-25 05:48:47
# Size of source mod 2**32: 16169 bytes
import sys, copy, datetime, json, traceback
from typing import TYPE_CHECKING
from PyQt5.QtCore import QSize, Qt
from PyQt5.QtGui import QTextCharFormat, QBrush, QFont
from PyQt5.QtWidgets import QDialog, QLabel, QPushButton, QHBoxLayout, QVBoxLayout, QTextEdit, QFrame
import qrcode
...
from .util import MessageBoxMixin, read_QIcon, Buttons, CopyButton, MONOSPACE_FONT, ColorScheme, ButtonsLineEdit
if TYPE_CHECKING:
...
def show_transaction(tx, parent, desc=None, prompt_if_unsaved=False):
    try:
        ...
        try:
            ...
            parent.show_critical(_('Electrum-CHI was unable to deserialize the transaction:') + '\n' + str(e))
        finally:
            e = None
            del e
...
class TxDialog(QDialog, MessageBoxMixin):
    ...
    def closeEvent(self, event):
        if self.prompt_if_unsaved:
            self.saved or self.question((_('This transaction is not saved. Close anyway?')), title=(_('Warning'))) or event.ignore()
        else:
            event.accept()
        try:
            dialogs.remove(self)
    ...
    def update(self):
        ...
        can_sign = not self.tx.is_complete() and (self.wallet.can_sign(self.tx) or bool(self.main_window.tx_external_keypairs))
        ...
        if tx_mined_status.header_hash:
            self.block_hash_label.setText(_('Included in block: {}').format(tx_mined_status.header_hash))
        ...
        def text_format(addr):
            if self.wallet.is_mine(addr):
                if self.wallet.is_change(addr):
                    return chg
                return rec
            if self.wallet.is_billing_address(addr):
                return twofactor
            return ext
