...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/plugins/trustedcoin/trustedcoin.py
# Compiled at: 2019-08-24 06:06:43
# Size of source mod 2**32: 31569 bytes
import asyncio, socket, json, base64, time, hashlib
...
DISCLAIMER = [
 _('Two-factor authentication is a service provided by TrustedCoin.  It uses a multi-signature wallet, where you own 2 of 3 keys.  The third key is stored on a remote server that signs transactions on your behalf.  To use this service, you will need a smartphone with Google Authenticator installed.'),
 _('A small fee will be charged on each transaction that uses the remote server.  You may check and modify your billing preferences once the installation is complete.'),
 _("Note that your coins are not locked in this service.  You may withdraw your funds at any time and at no cost, without the remote server, by using the 'restore wallet' option with your wallet seed."),
 _('The next step will generate the seed of your wallet.  This seed will NOT be saved in your computer, and it must be stored on paper.  To be safe from malware, you may want to do this on an offline computer, and move your wallet later to an online computer.')]
KIVY_DISCLAIMER = [
 _('Two-factor authentication is a service provided by TrustedCoin. To use it, you must have a separate device with Google Authenticator.'),
 _('This service uses a multi-signature wallet, where you own 2 of 3 keys.  The third key is stored on a remote server that signs transactions on your behalf. A small fee will be charged on each transaction that uses the remote server.'),
 _("Note that your coins are not locked in this service.  You may withdraw your funds at any time and at no cost, without the remote server, by using the 'restore wallet' option with your wallet seed.")]
...
class TrustedCoinCosignerClient(Logger):
    ...
    def send_request(self, method, relative_url, data=None, *, timeout=None):
        ...
        try:
            if method == 'get':
                response = Network.send_http_on_proxy(method, url, params=data,
                  headers=headers,
                  on_finish=(self.handle_response),
                  timeout=timeout)
            else:
                if method == 'post':
                    response = Network.send_http_on_proxy(method, url, json=data,
                      headers=headers,
                      on_finish=(self.handle_response),
                      timeout=timeout)
                else:
...
class Wallet_2fa(Multisig_Wallet):
    ...
    def _load_billing_addresses(self):
        billing_addresses = {'legacy':self.storage.get('trustedcoin_billing_addresses', {}), 
    ...
    def make_unsigned_transaction(self, coins, outputs, config, fixed_fee=None, change_addr=None, is_sweep=False):
        mk_tx = lambda o: Multisig_Wallet.make_unsigned_transaction(self, coins, o, config, fixed_fee, change_addr)
...
def make_xpub(xpub, s) -> str:
    ...
    child_pubkey, child_chaincode = bip32._CKD_pub(parent_pubkey=rootnode.eckey.get_public_key_bytes(compressed=True), parent_chaincode=(rootnode.chaincode),
      child_index=s)
    child_node = BIP32Node(xtype=(rootnode.xtype), eckey=(ecc.ECPubkey(child_pubkey)),
...
class TrustedCoinPlugin(BasePlugin):
    ...
    def tc_sign_wrapper(self, wallet, tx, on_success, on_failure):
        if not isinstance(wallet, self.wallet_class):
            ...
            if wallet.can_sign_without_server():
                return
            wallet.keystores['x3/'].get_tx_derivations(tx) or self.logger.info('twofactor: xpub3 not needed')
            return
    ...
    def create_2fa_seed(self, wizard):
        self.create_seed(wizard, '2fa')

    def create_2fa_segwit_seed(self, wizard):
        self.create_seed(wizard, '2fa_segwit')
    ...
    def create_remote_key(self, email, wizard):
        ...
        if r is None:
            ...
            _id = r['id']
            if short_id != _id:
                wizard.show_message('unexpected trustedcoin short_id: expected {}, received {}'.format(short_id, _id))
                return
            if xpub3 != _xpub3:
                wizard.show_message('unexpected trustedcoin xpub3: expected {}, received {}'.format(xpub3, _xpub3))
                return
    ...
    def on_reset_auth(self, wizard, short_id, seed, passphrase, xpub3):
        xprv1, xpub1, xprv2, xpub2 = self.xkeys_from_seed(seed, passphrase)
        if wizard.data['x1/']['xpub'] != xpub1 or wizard.data['x2/']['xpub'] != xpub2:
    ...
    def get_action(self, storage):
        if storage.get('wallet_type') != '2fa':
            ...
            if not storage.get('x1/'):
                ...
                return storage.get('x2/') or (
                 self, 'show_disclaimer')
            return storage.get('x3/') or (
