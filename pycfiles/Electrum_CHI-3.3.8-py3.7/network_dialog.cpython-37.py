...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/gui/qt/network_dialog.py
# Compiled at: 2019-08-25 05:37:01
# Size of source mod 2**32: 21615 bytes
import socket, time
from enum import IntEnum
from typing import Tuple
from PyQt5.QtCore import Qt, pyqtSignal, QThread
from PyQt5.QtWidgets import QTreeWidget, QTreeWidgetItem, QMenu, QGridLayout, QComboBox, QLineEdit, QDialog, QVBoxLayout, QHeaderView, QCheckBox, QTabWidget, QWidget, QLabel
...
class NetworkDialog(QDialog):
    ...
    def __init__(self, network, config, network_updated_signal_obj):
        ...
        self.network_updated_signal_obj.network_updated_signal.connect(self.on_update)
...
class NetworkChoiceLayout(object):
    ...
    def __init__(self, network: Network, config, wizard=False):
        ...
        grid.addWidget(HelpButton(_('Proxy settings apply to all connections: with Electrum-CHI servers, but also with third-party services.')), 2, 4)
    ...
    def update(self):
        ...
        if not self.server_host.hasFocus():
            if not self.server_port.hasFocus():
    ...
    def change_server(self, host, protocol):
        pp = self.servers.get(host, constants.net.DEFAULT_PORTS)
        if protocol:
            if protocol not in protocol_letters:
                protocol = None
        if protocol:
            port = pp.get(protocol)
            if port is None:
                protocol = None
        elif (protocol or 's') in pp.keys():
            protocol = 's'
            port = pp.get(protocol)
        else:
    ...
    def set_server(self):
        net_params = self.network.get_parameters()
        net_params = net_params._replace(host=(str(self.server_host.text())), port=(str(self.server_port.text())),
    ...
    def suggest_proxy(self, found_proxy):
        ...
        if self.proxy_cb.isChecked():
            if self.proxy_mode.currentIndex() == self.proxy_mode.findText('SOCKS5'):
                if self.proxy_host.text() == '127.0.0.1':
                    if self.proxy_port.text() == str(found_proxy[1]):
