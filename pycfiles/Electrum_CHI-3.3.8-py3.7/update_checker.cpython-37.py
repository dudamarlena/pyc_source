...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/gui/qt/update_checker.py
# Compiled at: 2019-08-25 05:51:35
...
class UpdateCheck(QWidget, Logger):
    url = 'https://www.xaya.io/download/electrum-chi/version'
    download_url = 'https://www.xaya.io/download/betas/#electrum-chi'
    VERSION_ANNOUNCEMENT_SIGNING_KEYS = ('N8XRR1apE8bRVe47JkRNWZ3LLx3hD2jRNn', )
    ...
    def update_view(self, latest_version=None):
        if latest_version:
            ...
            if self.is_newer(latest_version):
                ...
                self.detail_label.setText(_('You are already on the latest version of Electrum-CHI.'))
        else:
            self.heading_label.setText('<h2>' + _('Checking for updates...') + '</h2>')
            self.detail_label.setText(_('Please wait while Electrum-CHI checks for available updates.'))
        ...
        async with make_aiohttp_session(proxy=(self.main_window.network.proxy)) as session:
            async with session.get(UpdateCheck.url) as result:
                ...
                for address, sig in sigs.items():
                    ...
                    if ecc.verify_message_with_address(address=address, sig65=sig, message=msg, net=(constants.BitcoinMainnet)):
