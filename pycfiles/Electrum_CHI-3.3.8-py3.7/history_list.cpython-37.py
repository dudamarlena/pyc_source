...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/gui/qt/history_list.py
# Compiled at: 2019-08-25 05:31:34
# Size of source mod 2**32: 31470 bytes
import os, datetime
...
from PyQt5.QtCore import Qt, QPersistentModelIndex, QModelIndex, QAbstractItemModel, QSortFilterProxyModel, QVariant, QItemSelectionModel, QDate, QPoint
from PyQt5.QtWidgets import QMenu, QHeaderView, QLabel, QMessageBox, QPushButton, QComboBox, QVBoxLayout, QCalendarWidget, QGridLayout
...
from .util import read_QIcon, MONOSPACE_FONT, Buttons, CancelButton, OkButton, filename_field, MyTreeView, AcceptFileDragDrop, WindowModalDialog, CloseButton, webopen
if TYPE_CHECKING:
...
class HistorySortModel(QSortFilterProxyModel):
    ...
    def lessThan(self, source_left: QModelIndex, source_right: QModelIndex):
        ...
        v1 = item1.value()
        v2 = item2.value()
        if (v1 is None or isinstance)(v1, Decimal):
            if v1.is_nan():
                v1 = -float('inf')
        if (v2 is None or isinstance)(v2, Decimal):
            if v2.is_nan():
                v2 = -float('inf')
        try:
            return v1 < v2
...
class HistoryModel(QAbstractItemModel, Logger):
        ...
        if not index.isValid():
            ...
            if role == Qt.UserRole:
                d = {HistoryColumns.STATUS_ICON: (
                 ...
                 HistoryColumns.FIAT_VALUE: tx_item['fiat_value'].value if 'fiat_value' in tx_item else None, 
                 
                 HistoryColumns.FIAT_ACQ_PRICE: tx_item['acquisition_price'].value if 'acquisition_price' in tx_item else None, 
                 
                 HistoryColumns.FIAT_CAP_GAINS: tx_item['capital_gain'].value if 'capital_gain' in tx_item else None, 
            ...
            if role not in (Qt.DisplayRole, Qt.EditRole):
                if col == HistoryColumns.STATUS_ICON:
                    if role == Qt.DecorationRole:
                        ...
                        if col > HistoryColumns.DESCRIPTION:
                            if role == Qt.TextAlignmentRole:
                                return QVariant(Qt.AlignRight | Qt.AlignVCenter)
                        if col != HistoryColumns.STATUS_TEXT and role == Qt.FontRole:
                            monospace_font = QFont(MONOSPACE_FONT)
                            return QVariant(monospace_font)
                    if col == HistoryColumns.DESCRIPTION:
                        if role == Qt.DecorationRole:
                            if self.parent.wallet.invoices.paid.get(tx_hash):
                                return QVariant(read_QIcon('seal'))
                elif col in (HistoryColumns.DESCRIPTION, HistoryColumns.COIN_VALUE):
                    if role == Qt.ForegroundRole and tx_item['value'].value < 0:
                        red_brush = QBrush(QColor('#BC1E1E'))
                        return QVariant(red_brush)
                if col == HistoryColumns.FIAT_VALUE:
                    if role == Qt.ForegroundRole and not tx_item.get('fiat_default'):
                        if tx_item.get('fiat_value') is not None:
            ...
            if col == HistoryColumns.COIN_VALUE:
                value = tx_item['value'].value
                v_str = self.parent.format_amount(value, is_diff=True, whitespaces=True)
                return QVariant(v_str)
            if col == HistoryColumns.RUNNING_COIN_BALANCE:
            ...
            if col == HistoryColumns.FIAT_ACQ_PRICE:
                if tx_item['value'].value < 0:
                    if 'acquisition_price' in tx_item:
                        acq = tx_item['acquisition_price'].value
                        return QVariant(self.parent.fx.format_fiat(acq))
            if col == HistoryColumns.FIAT_CAP_GAINS and 'capital_gain' in tx_item:
                cg = tx_item['capital_gain'].value
                return QVariant(self.parent.fx.format_fiat(cg))
        if col == HistoryColumns.TXID:
    ...
    def refresh(self, reason: str):
        ...
        if not self.parent.gui_thread == threading.current_thread():
            raise AssertionError('must be called from GUI thread')
        else:
    ...
    def update_tx_mined_status(self, tx_hash: str, tx_mined_info: TxMinedInfo):
        try:
            ...
            tx_item.update({'confirmations':tx_mined_info.conf, 
    ...
    def headerData(self, section: int, orientation: Qt.Orientation, role: Qt.ItemDataRole):
        ...
        return {HistoryColumns.STATUS_ICON: '', 
         ...
         HistoryColumns.TXID: 'TXID'}[section]
    ...
    def tx_mined_info_from_tx_item(tx_item):
        tx_mined_info = TxMinedInfo(height=(tx_item['height']), conf=(tx_item['confirmations']),
...
class HistoryList(MyTreeView, AcceptFileDragDrop):
    ...
    def format_date(self, d):
        if d:
            return str(datetime.date(d.year, d.month, d.day))
        return _('None')
    ...
    def get_toolbar_buttons(self):
        return (
         self.period_combo, self.start_button, self.end_button)
    ...
    def show_summary(self):
        ...
        grid.addWidget(QLabel(str(h.get('fiat_start_value')) + '/CHI'), 0, 2)
        ...
        grid.addWidget(QLabel(str(h.get('fiat_end_value')) + '/CHI'), 2, 2)
    ...
    def plot_history_dialog(self):
        if plot_history is None:
            self.parent.show_message(_("Can't plot history.") + '\n' + _('Perhaps some dependencies are missing...') + ' (matplotlib?)')
            return
        try:
    ...
    def remove_local_tx(self, delete_tx):
        ...
        if len(to_delete) > 1:
            question = _('Are you sure you want to remove this transaction and {} child transactions?').format(len(to_delete) - 1)
        else:
            return self.parent.question(msg=question, title=(_('Please confirm'))) or None
        for tx in to_delete:
    ...
    def export_history_dialog(self):
        ...
        vbox = QVBoxLayout(d)
        defaultname = os.path.expanduser('~/electrum-chi-history.csv')
        ...
        try:
            ...
            try:
                export_error_label = _('Electrum-CHI was unable to produce a transaction export.')
    ...
    def do_export_history(self, file_name, is_csv):
        hist = self.wallet.get_full_history(domain=(self.hm.get_domain()), from_timestamp=None,
          to_timestamp=None,
          fx=(self.parent.fx),
          show_fees=True)
