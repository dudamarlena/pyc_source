...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/json_db.py
# Compiled at: 2019-08-24 06:06:43
# Size of source mod 2**32: 28878 bytes
import os, ast, json, copy, threading
...
class JsonDB(Logger):
    ...
    def split_accounts(self):
        ...
        if len(d) < 2:
            ...
            if wallet_type == 'old':
                ...
                if wallet_type in ('bip44', 'trezor', 'keepkey', 'ledger', 'btchip',
                                   'digitalbitbox', 'safe_t'):
        ...
        if not self._is_upgrade_method_needed(0, 13):
            ...
            if seed_version == OLD_SEED_VERSION or wallet_type == 'old':
                ...
                if key_type == 'imported':
                    ...
                    if wallet_type in ('xpub', 'standard'):
                        xpub = xpubs['x/']
                        xprv = xprvs.get('x/')
                        d = {'type':'bip32', 
                         'xpub':xpub, 
                        ...
                        if wallet_type in ('bip44', ):
                            ...
                            if wallet_type in ('trezor', 'keepkey', 'ledger', 'digitalbitbox',
                                               'safe_t'):
                                xpub = xpubs["x/0'"]
                                ...
                                if wallet_type == '2fa' or multisig_type(wallet_type):
    ...
    def _raise_unsupported_version(self, seed_version):
        ...
        if seed_version in (5, 7, 8, 9, 10, 14):
            ...
            if self.get('master_public_keys') is None and self.get('master_private_keys') is None and self.get('imported_keys') is None:
                ...
                msg += '\nPlease open this file with Electrum-CHI 1.9.8, and move your coins to a new wallet.'
    ...
    def get_verified_tx(self, txid):
        ...
        return TxMinedInfo(height=height, conf=None,
          timestamp=timestamp,
          txpos=txpos,
    ...
    def _load_transactions(self):
        self.txi = self.get_data_ref('txi')
        self.txo = self.get_data_ref('txo')
        self.transactions = self.get_data_ref('transactions')
        self.spent_outpoints = self.get_data_ref('spent_outpoints')
        self.history = self.get_data_ref('addr_history')
        self.verified_tx = self.get_data_ref('verified_tx3')
        self.tx_fees = self.get_data_ref('tx_fees')
        self.queued_transactions = self.get_data_ref('queued_transactions')
        ...
        for tx_hash in list(self.transactions.keys()):
            if not self.get_txi(tx_hash):
                self.get_txo(tx_hash) or self.logger.info(f"removing unreferenced tx: {tx_hash}")
