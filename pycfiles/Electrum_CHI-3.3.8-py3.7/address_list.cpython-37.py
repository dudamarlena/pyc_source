...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/gui/qt/address_list.py
# Compiled at: 2019-08-24 06:06:43
# Size of source mod 2**32: 10556 bytes
...
class AddressList(MyTreeView):
    ...
    def get_toolbar_buttons(self):
        return (
         QLabel(_('Filter:')), self.change_button, self.used_button)
    ...
    def refresh_headers(self):
        ...
        if fx and fx.get_fiat_address_config():
            ...
            ccy = _('Fiat')
        headers = {self.Columns.TYPE: _('Type'), 
    ...
    def update(self):
        ...
        for address in addr_list:
            ...
            if self.show_used == 1:
                if balance or is_used_and_empty:
                    continue
            if self.show_used == 2:
                if balance == 0:
                    continue
            if self.show_used == 3:
                if not is_used_and_empty:
                    continue
            balance_text = self.parent.format_amount(balance, whitespaces=True)
            if fx:
                if fx.get_fiat_address_config():
                    rate = fx.exchange_rate()
                    fiat_balance = fx.value_str(balance, rate)
                else:
                    fiat_balance = ''
                labels = [
                 '', address, label, balance_text, fiat_balance, '%d' % num]
