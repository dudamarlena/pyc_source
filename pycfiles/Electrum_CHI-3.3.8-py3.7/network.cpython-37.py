...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/network.py
# Compiled at: 2019-08-24 06:06:43
# Size of source mod 2**32: 55893 bytes
...
from . import util
from .util import log_exceptions, ignore_exceptions, bfh, SilentTaskGroup, make_aiohttp_session, send_exception_to_crash_reporter, is_hash256_str, is_non_negative_integer
from .bitcoin import COIN
from . import constants
from . import blockchain
from . import bitcoin
from .blockchain import Blockchain, DISK_HEADER_SIZE
from .interface import Interface, serialize_server, deserialize_server, RequestTimedOut, NetworkTimeout, BUCKET_NAME_OF_ONION_SERVERS, NetworkException
...
class TxBroadcastHashMismatch(TxBroadcastError):
    ...
    def get_message_for_gui(self):
        return '{}\n{}\n\n{}'.format(_('The server returned an unexpected transaction ID when broadcasting the transaction.'), _('Consider trying to connect to a different server, or updating Electrum-CHI.'), str(self))


class TxBroadcastServerReturnedError(TxBroadcastError):

    def get_message_for_gui(self):
        return '{}\n{}\n\n{}'.format(_('The server returned an error when broadcasting the transaction.'), _('Consider trying to connect to a different server, or updating Electrum-CHI.'), str(self))


class TxBroadcastUnknownError(TxBroadcastError):

    def get_message_for_gui(self):
        return '{}\n{}'.format(_('Unknown error when broadcasting the transaction.'), _('Consider trying to connect to a different server, or updating Electrum-CHI.'))
...
class Network(Logger):
    ...
    async def set_parameters(self, net_params: NetworkParameters):
        ...
        try:
            ...
            async with self.restart_lock:
                ...
                if not self.proxy != proxy:
                    if self.protocol != protocol or self.oneserver != net_params.oneserver:
                        await self._stop()
    ...
    def best_effort_reliable(func):
        ...
        async def make_reliable_wrapper(self, *args, **kwargs):
            for i in range(10):
                ...
                if success_fut.done():
                    if success_fut.cancelled() or success_fut.exception():
                        try:
    ...
    async def _start(self):
        ...
        if self.interface or self.interfaces:
            raise AssertionError
        if self.connecting or self.server_queue:
            raise AssertionError
        ...
        async def maintain_healthy_spread_of_connected_servers():
            ...
            for iface in interfaces:
                if not self.check_interface_against_healthy_spread_of_connected_servers(iface):
                    self.logger.info(f"disconnecting from {iface.server}. too many connected servers already in bucket {iface.bucket_based_on_ipaddress()}")
    ...
    @classmethod
    async def _send_http_on_proxy(cls, method: str, url: str, params: str=None, body: bytes=None, json: dict=None, headers=None, on_finish=None, timeout=None):
            ...
            if method == 'get':
                ...
                if method == 'post':
                    if not body is not None:
                        assert json is not None, 'body or json must be supplied if method is post'
                    if body is not None:
