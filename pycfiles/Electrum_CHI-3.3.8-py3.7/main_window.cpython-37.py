...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/gui/qt/main_window.py
# Compiled at: 2019-08-25 05:46:35
# Size of source mod 2**32: 156099 bytes
import sys, time, threading, os, traceback, json, shutil, weakref, csv
...
from PyQt5.QtWidgets import QMessageBox, QComboBox, QSystemTrayIcon, QTabWidget, QSpinBox, QMenuBar, QFileDialog, QCheckBox, QLabel, QVBoxLayout, QGridLayout, QLineEdit, QTreeWidgetItem, QHBoxLayout, QPushButton, QScrollArea, QTextEdit, QShortcut, QMainWindow, QCompleter, QInputDialog, QWidget, QMenu, QSizePolicy, QStatusBar
import electrum
...
from .util import read_QIcon, ColorScheme, text_dialog, icon_path, WaitingDialog, WindowModalDialog, ChoicesLayout, HelpLabel, FromList, Buttons, OkButton, InfoButton, WWLabel, TaskThread, CancelButton, CloseButton, HelpButton, MessageBoxMixin, EnterButton, expiration_values, ButtonsLineEdit, CopyCloseButton, import_meta_gui, export_meta_gui, filename_field, address_field, char_width_in_lineedit, webopen
...
class ElectrumWindow(QMainWindow, MessageBoxMixin, Logger):
    ...
    def __init__(self, gui_object, wallet: Abstract_Wallet):
        ...
        if config.get('check_updates') is None:
            choice = self.question(title=('Electrum-CHI - ' + _('Enable update check')), msg=(_('For security reasons we advise that you always use the latest version of Electrum-CHI.') + ' ' + _('Would you like to be notified when there is a newer version of Electrum-CHI available?')))
    ...
    def show_about(self):
        QMessageBox.about(self, 'Electrum-CHI', _('Version') + ' %s' % ELECTRUM_VERSION + '\n\n' + _("Electrum-CHI's focus is speed, with low resource usage and simplifying Xaya.") + ' ' + _('You do not need to perform regular backups, because your wallet can be recovered from a secret phrase that you can memorize or write on paper.') + ' ' + _('Startup times are instant because it operates in conjunction with high-performance servers that handle the most complicated parts of the Xaya system.') + '\n\n' + _('Uses icons from the Icons8 icon pack (icons8.com).'))
    ...
    def notify_transactions(self):
        if self.tx_notification_queue.qsize() == 0:
            ...
            if len(txns) >= 3:
                ...
                self.notify(_('{} new transactions: Total amount received in the new transactions {}').format(len(txns), self.format_amount_and_units(total_amount)))
        else:
            for tx in txns:
                is_relevant, is_mine, v, fee = self.wallet.get_wallet_delta(tx)
                if not is_relevant:
    ...
    def connect_fields(self, window, btc_e, fiat_e, fee_e):
        ...
        def edit_changed(edit):
            if edit.follows:
                ...
                fiat_e.follows = True
                fiat_e.setText(self.fx.ccy_amount_str(amount * Decimal(rate) / COIN, False))
        ...
        if self.network is None:
            ...
            if self.network.is_connected():
                ...
                if not self.wallet.up_to_date or server_height == 0:
                    ...
                    text = '{} ({}/{})'.format(_('Synchronizing...'), num_answered, num_sent)
    ...
    def update_wallet(self):
        self.update_status()
        if not ((self.wallet.up_to_date or self).network and self.network.is_connected()):
            self.update_tabs()
    ...
    def create_receive_tab(self):
        ...
        if not (self.fx and self.fx.is_enabled()):
    ...
    def new_payment_request(self):
        ...
        if addr is None:
            if not self.wallet.is_deterministic():
                msg = [_('No more addresses in your wallet.'),
                 _('You are using a non-deterministic wallet, which cannot create new addresses.'),
                 _('If you want to create new addresses, use a deterministic wallet instead.')]
                self.show_message(' '.join(msg))
                return
            else:
                return self.question(_('Warning: The next address will not be recovered automatically if you restore your wallet from seed; you may need to add it manually.\n\nThis occurs because you have too many unused addresses in your wallet. To avoid this situation, use the existing addresses first.\n\nCreate anyway?')) or None
    ...
    def update_receive_address_styling(self):
        ...
        if self.wallet.is_used(addr):
            ...
            self.receive_address_e.setToolTip(_('This address has already been used. For better privacy, do not reuse it for new payments.'))
        else:
            self.receive_address_e.setStyleSheet('')
            self.receive_address_e.setToolTip('')

    def set_feerounding_text(self, num_satoshis_added):
        self.feerounding_text = _('Additional {} satoshis are going to be added.').format(num_satoshis_added)
    ...
    def create_send_tab(self):
        ...
        def feerounding_onclick():
            text = self.feerounding_text + '\n\n' + _('To somewhat protect your privacy, Electrum-CHI tries to create change with similar precision to other outputs.') + ' ' + _('At most 100 swartz might be lost due to this rounding.') + ' ' + _("You can disable this setting in '{}'.").format(_('Preferences')) + '\n' + _('Also, dust is not kept as change, but added to the fee.') + '\n' + _('Also, when batching RBF transactions, BIP 125 imposes a lower bound on the fee.')
        ...
        def entry_changed():
            ...
            if self.not_enough_funds:
                ...
                if c + u + x:
                    text += ' ({} {} {})'.format(self.format_amount(c + u + x).strip(), self.base_unit(), _('are frozen'))
            elif self.fee_e.isModified():
                feerate_color = ColorScheme.BLUE
            else:
                if self.feerate_e.isModified():
    ...
    def get_payto_or_dummy(self):
        ...
        if r:
            return r
        return (
         TYPE_ADDRESS, self.wallet.dummy_address())
    ...
    def do_update_fee(self):
        ...
        make_tx = lambda fee_est: self.wallet.make_unsigned_transaction(coins,
          outputs, (self.config), fixed_fee=fee_est,
          is_sweep=is_sweep)
        try:
            ...
            try:
                if not freeze_fee:
                    ...
                    if isinstance(e, NotEnoughFunds):
                        ...
                        if isinstance(e, NoDynamicFeeEstimates):
                            try:
                                tx = make_tx(0)
        ...
        if self.max_button.isChecked():
            ...
            __, x_fee_amount = run_hook('get_tx_extra_fee', self.wallet, tx) or (None,
                                                                                 0)
    ...
    def get_send_fee_estimator(self):
        if self.is_send_fee_frozen():
            ...
            if self.is_send_feerate_frozen():
                amount = self.feerate_e.get_amount()
                amount = 0 if amount is None else amount * 1000
                fee_estimator = partial(simple_config.SimpleConfig.estimate_fee_for_feerate, amount)
            else:
                fee_estimator = None
    ...
    def check_send_tab_outputs_and_show_errors(self, outputs) -> bool:
        ...
        if not pr:
            ...
            if errors:
                ...
                return True
            if self.payto_e.is_alias:
                if self.payto_e.validated is False:
        ...
        for o in outputs:
            if o.address is None:
                ...
                if o.type == TYPE_ADDRESS:
                    if not bitcoin.is_address(o.address):
    ...
    def do_send(self, preview=False):
        if run_hook('abort_send', self):
            ...
            try:
                is_sweep = bool(self.tx_external_keypairs)
                tx = self.wallet.make_unsigned_transaction(coins,
                  outputs, (self.config), fixed_fee=fee_estimator, is_sweep=is_sweep)
            except (NotEnoughFunds, NoDynamicFeeEstimates) as e:
                try:
            ...
            if preview:
                ...
                return
            self.network or self.show_error(_("You can't broadcast a transaction without a live network connection."))
            return
        msg = [
    ...
    def broadcast_transaction(self, tx, tx_desc):
        ...
        WaitingDialog(self, _('Broadcasting transaction...'), broadcast_thread, broadcast_done, self.on_error)
    ...
    msg_sign = _('Signing with an address actually means signing with the corresponding private key, and verifying with the corresponding public key. The address you have entered does not have a unique public key, so these operations cannot be performed.') + '\n\n' + _('The operation is undefined. Not just in Electrum-CHI, but in general.')
    ...
    def do_sign(self, address, message, signature, password):
        ...
        if txin_type not in ('p2pkh', 'p2wpkh', 'p2wpkh-p2sh'):
            self.show_message(_('Cannot sign messages with this type of address:') + ' ' + txin_type + '\n\n' + self.msg_sign)
            return
    ...
    def do_process_from_txid(self):
        ...
        if ok:
            if txid:
                txid = str(txid).strip()
                try:
                    raw_tx = self.network.run_from_another_thread(self.network.get_transaction(txid, timeout=10))
                except Exception as e:
                    try:
    ...
    def settings_dialog(self):
        ...
        if not self.config.is_modifiable('num_zeros'):
            ...
            use_rbf_cb.setToolTip(_('If you check this box, your transactions will be marked as non-final,') + '\n' + _('and you will have the possibility, while they are unconfirmed, to replace them with transactions that pay higher fees.') + '\n' + _('Note that some merchants do not accept non-final transactions until they are confirmed.'))
            ...
            batch_rbf_cb.setToolTip(_('If you check this box, your unconfirmed transactions will be consolidated into a single transaction.') + '\n' + _('This will save fees.'))
        ...
        def on_unit(x, nz):
            ...
            if self.base_unit() == unit_result:
                return
            edits = (
             self.amount_e, self.fee_e, self.receive_amount_e)
        ...
        multiple_cb.setToolTip('\n'.join([
         _('In some cases, use up to 3 change addresses in order to break up large coin amounts and obfuscate the recipient address.'),
        ...
        outrounding_cb.setToolTip(_('Set the value of the change output so that it has similar precision to the other outputs.') + '\n' + _('This might improve your privacy somewhat.') + '\n' + _('If enabled, at most 100 satoshis might be lost due to this, per transaction.'))
        ...
        def on_exchange(idx):
            ...
            if self.fx:
                if self.fx.is_enabled():
                    if exchange:
                        if exchange != self.fx.exchange.name():
    ...
    def plugins_dialog(self):
        ...
        for i, descr in enumerate(plugins.descriptions.values()):
            ...
            try:
                ...
                cb_enabled = not plugin_is_loaded and plugins.is_available(name, self.wallet) or plugin_is_loaded and p.can_user_disable()
    ...
    def cpfp(self, parent_tx, new_tx):
        ...
        if parent_fee is None:
            ...
            vbox = QVBoxLayout(d)
            msg = 'A CPFP is a transaction that sends an unconfirmed output back to yourself, with a high fee. The goal is to have miners confirm the parent transaction in order to get the fee attached to the child transaction.'
            vbox.addWidget(WWLabel(_(msg)))
            msg2 = 'The proposed fee is computed using your fee/kB settings, applied to the total size of both child and parent transactions. After you broadcast a CPFP transaction, it is normal to see a new unconfirmed transaction in your history.'
    ...
    def save_transaction_into_wallet(self, tx):
        ...
        try:
            ...
            self.need_update.set()
            msg = _('Transaction added to wallet history.') + '\n\n' + _('Note: this is an offline transaction, if you want the network to see it, you need to broadcast it.')
