...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/powdata.py
...
def deserialize_base(s: bytes, start_position=0) -> (dict, int):
  ...
  """
    ...
    res['bits'] = int.from_bytes((s[start_position + 1:start_position + 5]), byteorder='little')
    start_position += 5
...
def deserialize(s: bytes, start_position=0) -> (dict, int):
  ...
  """
    ...
    if res['mergemined']:
        res['auxpow'], start_position = auxpow.deserialize_auxpow_header(s,
...
def verify(pow_data: dict, header_hash: str) -> None:
  ...
  """
    algo = pow_data['algo']
    if algo == ALGO_SHA256D and not pow_data['mergemined']:
        ...
        elif pow_data['mergemined']:
            if 'auxpow' not in pow_data:
