...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/gui/qt/installwizard.py
# Compiled at: 2019-08-25 05:34:01
# Size of source mod 2**32: 26982 bytes
...
from PyQt5.QtWidgets import QWidget, QDialog, QLabel, QHBoxLayout, QMessageBox, QVBoxLayout, QLineEdit, QFileDialog, QPushButton, QGridLayout, QSlider, QScrollArea
...
from .util import MessageBoxMixin, Buttons, icon_path, ChoicesLayout, WWLabel, InfoButton, char_width_in_lineedit
...
MSG_PASSPHRASE_WARN_ISSUE4566 = _('Warning') + ': ' + _('You have multiple consecutive whitespaces or leading/trailing whitespaces in your passphrase.') + ' ' + _('This is discouraged.') + ' ' + _('Due to a bug, old versions of Electrum-CHI will NOT be creating the same wallet as newer versions or other software.')
...
class CosignWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        for i in range(self.n):
            alpha = int(5760 * i / self.n)
            alpha2 = int(5760 / self.n)
...
class InstallWizard(QDialog, MessageBoxMixin, BaseWizard):
    ...
    def select_storage(self, path, get_wallet_from_daemon) -> Tuple[(str, Optional[WalletStorage])]:
        ...
        def on_filename(filename):
            ...
            if self.temp_storage:
                if not self.temp_storage.file_exists():
                    ...
                    if not wallet_from_memory:
                        if self.temp_storage.is_encrypted_with_user_pw():
                            msg = _('This file is encrypted with a password.') + '\n' + _('Enter your password or choose another file.')
                            user_needs_to_enter_password = True
                        else:
                            if self.temp_storage.is_encrypted_with_hw_device():
        ...
        while 1:
            if self.loop.exec_() != 2:
                raise UserCancelled
            if self.temp_storage.file_exists():
                if not self.temp_storage.is_encrypted():
                    break
            if not self.temp_storage.file_exists():
                break
            ...
            if self.temp_storage.file_exists() and self.temp_storage.is_encrypted():
                if self.temp_storage.is_encrypted_with_user_pw():
                    ...
                    try:
                        ...
                        try:
                            self.show_message(title=(_('Error')), msg=(_('Failed to decrypt using this hardware device.') + '\n' + _('If you use a passphrase, make sure it is correct.')))
                            self.reset_stack()
        ...
        return (
         self.temp_storage.path, self.temp_storage if self.temp_storage.file_exists() else None)
    ...
    def set_icon(self, filename):
        ...
        self.logo.setPixmap(QPixmap(icon_path(filename)).scaledToWidth(60,
    ...
    def text_input(self, title, message, is_valid, allow_multi=False):
        slayout = KeysLayout(parent=self, header_layout=message, is_valid=is_valid, allow_multi=allow_multi)
    ...
    def pw_layout(self, msg, kind, force_disable_encrypt_cb):
        playout = PasswordLayout(msg=msg, kind=kind, OK_button=(self.next_button), force_disable_encrypt_cb=force_disable_encrypt_cb)
    ...
    @wizard_dialog
    def choice_and_line_dialog(self, title: str, message1: str, choices: List[Tuple[(str, str, str)]], message2: str, test_text: Callable[([str], int)], run_next, default_choice_idx: int=0) -> Tuple[(str, str)]:
        vbox = QVBoxLayout()
        c_values = [x[0] for x in choices]
        c_titles = [x[1] for x in choices]
        ...
        clayout = ChoicesLayout(message1, c_titles, on_choice_click, checked_index=default_choice_idx)
        vbox.addLayout(clayout.layout())
        vbox.addSpacing(50)
    ...
    @wizard_dialog
    def line_dialog(self, run_next, title, message, default, test, warning='', presets=(), warn_issue4566=False):
        vbox = QVBoxLayout()
    ...
    def init_network(self, network):
        message = _('Electrum-CHI communicates with remote servers to get information about your transactions and addresses. The servers all fulfill the same purpose only differing in hardware. In most cases you simply want to let Electrum-CHI pick one at random.  However if you prefer feel free to select a server manually.')
        choices = [
         _('Auto connect'), _('Select server manually')]
