...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/ecc.py
...
def ser_to_point(ser: bytes) -> Tuple[(int, int)]:
    if ser[0] not in (2, 3, 4):
        raise ValueError('Unexpected first byte: {}'.format(ser[0]))
    if ser[0] == 4:
        return (
         string_to_number(ser[1:33]), string_to_number(ser[33:]))
    x = string_to_number(ser[1:])
    return (x, get_y_coord_from_x(x, ser[0] == 3))
...
class InvalidECPointException(Exception):
    __doc__ = 'e.g. not on curve, or infinity'
...
class _MySigningKey(ecdsa.SigningKey):
    __doc__ = 'Enforce low S values in signatures'
...
class ECPubkey(object):
    ...
    def from_point(cls, point):
        _bytes = point_to_ser(point, compressed=False)
    ...
    def __eq__(self, other):
        return self._pubkey.point.x() == other._pubkey.point.x() and self._pubkey.point.y() == other._pubkey.point.y()
...
def msg_magic(message: bytes) -> bytes:
    ...
    return b'\x15Xaya Signed Message:\n' + length + message
...
class ECPrivkey(ECPubkey):
    ...
    def sign(self, data: bytes, sigencode=None, sigdecode=None) -> bytes:
        if sigencode is None:
            ...
            assert public_key.verify_digest(sig, data, sigdecode=sigdecode), 'Sanity check verifying our own signature failed.'
        return sig
