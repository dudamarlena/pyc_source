...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/commands.py
# Compiled at: 2019-08-25 05:39:46
# Size of source mod 2**32: 53975 bytes
import sys, datetime, copy, argparse, json, ast, base64
...
def satoshis(amount):
    if amount not in ('!', None):
        return int(COIN * Decimal(amount))
    return amount
...
def command(s):
    ...
    def decorator(func):
        ...
        def func_wrapper(*args, **kwargs):
            ...
            if c.requires_wallet:
                if wallet is None:
                    raise Exception("wallet not loaded. Use 'electrum-chi daemon load_wallet'")
            if c.requires_password:
                if password is None:
...
class Commands:
    ...
    def __init__(self, config: 'SimpleConfig', wallet: Abstract_Wallet, network: Optional['Network'], callback=None):
        self.config = config
        self.wallet = wallet
    ...
    def create(self, passphrase=None, password=None, encrypt_file=True, seed_type=None):
        ...
        """
        d = create_new_wallet(path=(self.config.get_wallet_path()), passphrase=passphrase,
          password=password,
    ...
    def restore(self, text, passphrase=None, password=None, encrypt_file=True):
        ...
        If you want to be prompted for an argument, type '?' or ':' (concealed)
        """
        d = restore_wallet_from_text(text, path=(self.config.get_wallet_path()),
          passphrase=passphrase,
          password=password,
          encrypt_file=encrypt_file,
          network=(self.network))
        return {'path':d['wallet'].storage.path, 
         'msg':d['msg']}
    ...
    def dumpprivkeys(self):
        """Deprecated."""
        return "This command is deprecated. Use a pipe instead: 'electrum-chi listaddresses | electrum-chi getprivatekeys - '"
    ...
    def _resolver(self, x):
        ...
        if out.get('type') == 'openalias':
            if self.nocheck is False:
                if out.get('validated') is False:
    ...
    @command('w')
    def history(self, year=None, show_addresses=False, show_fiat=False, show_fees=False, from_height=None, to_height=None):
        ...
        for addr in self.wallet.get_addresses():
            if frozen:
                if not self.wallet.is_frozen_address(addr):
                    continue
                elif receiving and self.wallet.is_change(addr):
                    continue
                if change:
    ...
    def queuetransaction(self, tx, trigger_depth, trigger_txid=None, trigger_name=None):
        ...
        if trigger_txid is None:
            ...
            send_when = {'txid':trigger_txid, 
             'name':trigger_name, 
             'name_encoding':'ascii', 
             'confirmations':trigger_depth}
            queue_item = {'tx':tx, 
             'sendWhen':send_when}
            return self.wallet.queue_transaction(txid, queue_item) or False
        self.wallet.storage.write()
        return txid
    ...
    def getfeerate(self, fee_method=None, fee_level=None):
        ...
        if fee_method is None:
            ...
            if fee_method.lower() == 'static':
                dyn, mempool = (False, False)
            else:
                if fee_method.lower() == 'eta':
                    dyn, mempool = (True, False)
                else:
                    if fee_method.lower() == 'mempool':
    ...
    def name_show(self, identifier):
        ...
        if tx_best is None:
            ...
            if header is None:
                ...
                if height != merkle.get('block_height'):
                    raise Exception('requested height {} differs from received height {} for txid {}'.format(height, merkle.get('block_height'), txid))
                pos = merkle.get('pos')
        ...
        for idx, o in enumerate(tx.outputs()):
            if o.name_op is not None:
                ...
                return {'name':o.name_op['name'].decode('utf-8'),  'name_encoding':'utf-8', 
    ...
    def removelocaltx(self, txid):
        ...
        if height != TX_HEIGHT_LOCAL:
            raise Exception(f"Only local transactions can be removed. This tx has height: {height} != {TX_HEIGHT_LOCAL}")
    ...
    def get_tx_status(self, txid):
        ...
        return {'confirmations': self.wallet.get_tx_height(txid).conf}
...
param_descriptions = {'privkey':"Private key. Type '?' to get a prompt.", 
...
command_options = {'password':('-W', 'Password'), 
...
arg_types = {'num':int, 
 ...
 'inputs':json_loads, 
 'outputs':json_loads, 
 'fee':lambda x:  if x is not None:
str(Decimal(x)) # Avoid dead code: None, 
 'amount':lambda x:  if x != '!':
str(Decimal(x)) # Avoid dead code: '!', 
 'locktime':int, 
 'fee_method':str, 
 'fee_level':json_loads, 
 'encrypt_file':eval_bool}
config_variables = {'addrequest':{'requests_dir':'directory where a bip70 file will be written.', 
 ...
 'listrequests':{'url_rewrite': 'Parameters passed to str.replace(), in order to create the r= part of xaya: URIs. Example: "(\'file:///var/www/\',\'https://www.xaya.io/\')"'}}
...
def add_global_options(parser):
    ...
    group.add_argument('-D', '--dir', dest='electrum_path', help='electrum-chi directory')
    group.add_argument('-P', '--portable', action='store_true', dest='portable', default=False, help="Use local 'electrum-chi_data' directory")
...
def get_parser():
    parser = argparse.ArgumentParser(epilog="Run 'electrum-chi help <command>' to see the help for a command")
