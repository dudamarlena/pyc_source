...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/blockchain.py
# Compiled at: 2019-08-24 07:36:23
# Size of source mod 2**32: 31009 bytes
import os, threading
...
def read_blockchains(config: 'SimpleConfig'):
    best_chain = Blockchain(config=config, forkpoint=0,
      parent=None,
    ...
    if best_chain.height() > constants.net.max_checkpoint():
        ...
        header_after_cp and best_chain.can_connect(header_after_cp, check_height=False, skip_auxpow=True) or _logger.info('[blockchain] deleting best chain. cannot connect header after last cp to last cp.')
    ...
    def instantiate_chain(filename):
        ...
        b = Blockchain(config=config, forkpoint=forkpoint,
          parent=parent,
...
class Blockchain(Logger):
    ...
    def __init__(self, config: SimpleConfig, forkpoint: int, parent: Optional['Blockchain'], forkpoint_hash: str, prev_hash: Optional[str]):
        if isinstance(forkpoint_hash, str):
            raise len(forkpoint_hash) == 64 or AssertionError(forkpoint_hash)
        else:
            if not prev_hash is None:
                raise isinstance(prev_hash, str) and len(prev_hash) == 64 or AssertionError(prev_hash)
    ...
    def get_max_child(self) -> Optional[int]:
        ...
        if children:
            return max([x.forkpoint for x in children])
    ...
    def check_hash(self, height: int, header_hash: str) -> bool:
        ...
        is the given hash.
        """
        if not (isinstance(header_hash, str) and len(header_hash) == 64):
            raise AssertionError(header_hash)
        try:
    ...
    def fork(parent, header: dict) -> 'Blockchain':
        ...
        self = Blockchain(config=(parent.config), forkpoint=forkpoint,
          parent=parent,
    ...
    def assert_headers_file_available(self, path):
        if os.path.exists(path):
            return
        if not os.path.exists(util.get_headers_dir(self.config)):
    ...
    def write(self, data: bytes, offset: int, truncate: bool=True) -> None:
        ...
        with open(filename, 'rb+') as (f):
            if truncate:
                if offset != self._size * DISK_HEADER_SIZE:
                    f.seek(offset)
                    f.truncate()
            f.seek(offset)
            f.write(data)
        ...
        while h < 0:
            ...
            if header is not None:
                if header['powdata']['algo'] == algo:
                    return {'height':h,  'timestamp':header['timestamp'], 
    ...
    def bits_to_target(cls, bits: int) -> int:
        ...
        bitsBase = bits & 16777215
        if not 32768 <= bitsBase <= 8388607:
