...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/gui/qt/exception_window.py
# Compiled at: 2019-08-25 05:30:47
...
class Exception_Window(BaseCrashReporter, QWidget, MessageBoxMixin, Logger):
    ...
    def __init__(self, main_window, exctype, value, tb):
        ...
        self.setWindowTitle('Electrum-CHI - ' + _('An Error Occurred'))
        ...
        collapse_info.clicked.connect(lambda : self.msg_box((QMessageBox.NoIcon), self,
          (_('Report contents')), (self.get_report_string()), rich_text=True))
        main_box.addWidget(collapse_info)
        main_box.addWidget(QLabel(BaseCrashReporter.DESCRIBE_ERROR_MESSAGE))
        self.description_textfield = QTextEdit()
        self.description_textfield.setFixedHeight(50)
        self.description_textfield.setPlaceholderText(_('Do not enter sensitive/private information here. The report will be visible on the public issue tracker.'))
    ...
    def send_report(self):
        ...
        def on_success(response):
            self.show_message(parent=self, title=(_('Crash report')),
              msg=response,
              rich_text=True)
            self.close()
        ...
        def on_failure(exc_info):
            ...
            self.show_critical(parent=self, msg=(_('There was a problem with the automatic reporting:') + '<br/>' + repr(e)[:120] + '<br/>' + _('Please report this issue manually') + f' <a href="{constants.GIT_REPO_ISSUES_URL}">on GitHub</a>.'),
              rich_text=True)
