...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/electrum_chi/electrum/plugin.py
# Compiled at: 2019-08-25 06:10:06
# Size of source mod 2**32: 22878 bytes
import os, pkgutil, importlib.util, time, threading, sys
...
class Plugins(DaemonThread):
    ...
    def load_plugins(self):
        for loader, name, ispkg in pkgutil.iter_modules([self.pkgpath]):
            ...
            if d.get('requires_wallet_type') or self.config.get('use_' + name):
                try:
        ...
        for name, (gui_good, details) in self.hw_wallets.items():
            if gui_good:
                try:
                    ...
                    if p.is_enabled():
                        out.append(HardwarePluginToScan(name=name, description=(details[2]),
                          plugin=p,
                          exception=None))
                except Exception as e:
                    try:
        ...
        if client:
            if client.is_pairable():
                ...
                if client_xpub == xpub:
                    ...
                    return client
        raise DeviceUnpairableError(_('Electrum-CHI cannot pair with your {}.\n\nBefore you request CHI to be sent to addresses in this wallet, ensure you can pair with your device, or that you have its seed (and passphrase, if any).  Otherwise all CHI you receive will be unspendable.').format(plugin.device))

    def unpaired_device_infos(self, handler, plugin: 'HW_PluginBase', devices=None, include_failing_clients=False):
        ...
        for device in devices:
            ...
            if not client:
                continue
            infos.append(DeviceInfo(device=device, label=(client.label()),
        ...
        try:
            ...
            for d in hid_list:
                ...
                if product_key in self.recognised_hardware:
                    ...
                    devices.append(Device(path=(d['path']), interface_number=interface_number,
                      id_=id_,
    ...
    def scan_devices(self):
        ...
        for f in self.enumerate_func:
            try:
                ...
                try:
                    self.logger.error('custom device enum failed. func {}, error {}'.format(str(f), repr(e)))
                finally:
                    e = None
                    del e
