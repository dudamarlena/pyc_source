...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/virustotal.py
# Compiled at: 2012-10-29 14:55:31
...
import httplib, mimetypes, urlparse, urllib, urllib2, hashlib, json, time, re, logging, threading
...
class VirusTotal(object):
    ...
    def _fileobj_to_fcontent(cls, anything, filename=None):
        if isinstance(anything, basestring):
            if all(i in '1234567890abcdef' for i in anything.lower()) and len(anything) in (32,
                                                                                            40,
                                                                                            64):
    ...
    def get(self, anything, filename=None):
        ...
        req = urllib2.urlopen(urllib2.Request('http://www.virustotal.com/vtapi/v2/file/report', data)).read()
    ...
    def scan(self, anything, filename=None, reanalyze=None):
        ...
        self._limit_call_handler()
        ret_json = postfile.post_multipart(host='www.virustotal.com', selector='https://www.virustotal.com/vtapi/v2/file/scan', fields={'apikey': self.api_key}.items(), files=[
         o])
...
class Report(object):
    ...
    def state(self):
        return {-2: 'analyzing', 
           1: 'ok', 
           0: 'ko'}.get(self.response_code, 'unknown')
...
def main():
    import sys, optparse, threading, Queue, glob
