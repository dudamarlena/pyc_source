...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dora/isa.py
# Compiled at: 2020-01-16 10:25:08
# Size of source mod 2**32: 26824 bytes
"""
Projeto ISA
"""
import base64, datetime, logging, json, re, os
from time import sleep
import hashlib, sqlparse, boto3
...
class ISAContext:
    ...
    r_user_connections = '^show[\\s]*connections$'
    r_cache_tables = '^show[\\s]*cache$'
    r_version = "'[0-9]{1,}'"
    r_table = '(?:[\\s]+)([\\d\\w\\$_]+\\.[\\d\\w\\$_]+\\.[\\d\\w\\$_]+)(?:(?:[\\s])+|\\Z|;|\\))'
    r_history = '^history[\\s]*' + r_table + '$'
    r_view = '(CREATE.+VIEW[\\s]*([a-zA-Z0-9_.]*)[\\s]+AS)[\\s]+((\\Z|.|\\s)*)'
    r_create = '((CREATE[\\s]*TABLE[\\s]*([a-zA-Z0-9_.]+)[\\s])+AS)[\\s]+((\\Z|.|\\s)*)'
    r_select = '[\\s]?(FROM|JOIN)[\\s]?' + r_table
    r_d_table = 'DROP[\\s]+TABLE(?:[\\s]+IF[\\s]+EXISTS)?[\\s]+(([\\d\\w\\$_]+)|([\\d\\w\\$_]+\\.[\\d\\w\\$_]+))(?:(?:[\\s])+|\\Z|;|\\))'
    r_drop = 'DROP[\\s]?TABLE[\\s]?' + r_table
    r_asof = '(asof+)(?:(?:[\\s])+|\\Z|\\))'
    ...
    def __init__(self, spark, debug=False):
        ...
        try:
            num_of_cores = re.match('^[0-9]+', os.environ.get('DORA_CORES', '1'), re.MULTILINE | re.IGNORECASE)[0]
        ...
        for tbl in table_list:
            if tbl.get('execution') is not None:
                ...
                if execution['status'] == 'RUNNING':
                    ...
                    continue
            input_data = {'username':DORA_USER,  'connection_name':tbl['base'].upper(), 
    ...
    def start_execution(self, input_data):
        ...
        execution = self.steps.start_execution(stateMachineArn=state_machine,
          name=exec_name,
    ...
    def sql(self, query, **kwargs):
        ...
        if re.findall(self.r_cache_tables, query, re.MULTILINE | re.IGNORECASE):
            object_keys = [
             'objectKey', 'lastUpdate', 'cacheDays', 'dora_condition', 'dora_partition']
        ...
        try:
            try:
                ...
                if match_view:
                    ...
                    for glue_keys in ('DatabaseName', 'CreateTime', 'UpdateTime', 'Retention',
                                      'IsRegisteredWithLakeFormation', 'CreatedBy'):
                        if glue_keys in glue_tb:
                            del glue_tb[glue_keys]

                    glue_tb['Parameters']['presto_view'] = 'true'
                    presto_query = json.dumps({'originalSql':glue_tb['ViewExpandedText'], 
                     'schema':USER_DB,  'catalog':AWS_CATALOG,  'columns':[{'name':field['Name'],  'type':field['Type']} for field in glue_tb['StorageDescriptor']['Columns']]})
