...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dora/ml.py
...
import subprocess, shutil, base64, docker, boto3, uuid, os, re
...
class MLContext:
    ...
    def new_model(self, name, code, artefacts, args):
        ...
        self.printd(args)
        if re.search('def[\\s]+score[\\s]*\\(.*?\\):', code) is None:
            ...
            if args is None:
                ...
                if self.name is not None:
                    try:
                        ...
                        try:
                            self.registry_id = re.findall('(?:"|\')(.+?)(?:\'|")', e.response['Error']['Message'])[1]
                            self.repository = '{registry_id}.dkr.ecr.{region_name}.amazonaws.com/{repository_name}'.format(registry_id=(self.registry_id),
                              region_name=(self.region_name),
                              repository_name=('dora-' + str(self.name)))
                        finally:
                            e = None
                            del e
...
class MLMagic:
    ...
    def ml(self, line, code=None):
        ...
        ML 0.2.0
        """
        _code = re.sub('%%ml[\\s]+.+', '', code)
        artefacts = re.findall('%artefacts((?:[\\s]+".+?")+)', _code)
        args = magic_arguments.parse_argstring(self.ml, line)
        if len(artefacts) != 0:
            _code = re.sub('%artefacts((?:[\\s]+".+?")+)', '#%artefacts{}'.format(artefacts[0]), _code)
            artefacts = re.findall('(?:")(.+?)(?:")', artefacts[0])
        else:
