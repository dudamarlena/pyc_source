...
# [GCC 8.4.0]
# Embedded file name: /Users/johnsalvatier/Documents/workspace/multichain_mcmc/multichain_mcmc/amala_examples/model9.py
# Compiled at: 2010-06-16 12:00:14
import numpy
from numpy import *
from pymc import *
N = 12
...
def gen_model():
    mean_hyper_mean = Normal('mean_hyper_mean', mu=0, tau=1.0 / 9)
    mean_hyper_logvar = Normal('mean_hyper_logvar', mu=-0.5, tau=1.0 / 1.0)
    mean = Normal('mean', mu=mean_hyper_mean * ones(N)[newaxis, :], tau=1.0 / exp(mean_hyper_logvar))
    ad_hyper_mean = Normal('ad_hyper_mean', mu=0.2, tau=1.0 / 1.0)
    ad_hyper_logvar = Normal('ad_hyper_logvar', mu=-0.5, tau=1.0 / 1.0)
    ad = Normal('ad', mu=ad_hyper_mean * ones(N)[newaxis, :], tau=1.0 / exp(ad_hyper_logvar))
    f1d_hyper_mean = Normal('f1d_hyper_mean', mu=0.5, tau=1.0 / 4.0)
    f1d_hyper_logvar = Normal('f1d_hyper_logvar', mu=-0.5, tau=1.0 / 1.0)
    f1d = Normal('f1d', mu=f1d_hyper_mean * ones(N)[newaxis, :], tau=1.0 / exp(f1d_hyper_logvar))
    logvar_hyper_mean = Normal('logvar_hyper_mean', mu=-0.5, tau=1.0 / 4.0)
    logvar_hyper_logvar = Normal('logvar_hyper_logvar', mu=-0.5, tau=1.0 / 1.0)
