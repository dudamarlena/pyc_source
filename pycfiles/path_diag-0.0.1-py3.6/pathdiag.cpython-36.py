...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/src/pathdiag.py
# Compiled at: 2018-07-29 13:02:59
...
class PythonPathUtil:
    __doc__ = 'The main class which includes a number of helpers for analyzing and\n    diagnosing problems with environment variables that define a list of\n    paths.'
    RETURN_VALUE = 0
    ...
    def __init__(self, variable, verbosity=None, no_color=False):
        ...
        try:
            ...
            raise Exception('Environment variable `{}` not found'.format(self.envvar))
    ...
    def can_add(self, new_path, priority=0):
        ...
        if not path_properties['exists']:
            raise CommandPathError('Path `{}` does not exist'.format(full_path))
        else:
            if not path_properties['is_directory']:
                raise CommandPathError('Path `{}` is not a directory'.format(full_path))
        if not path_properties['is_readable']:
            raise CommandPathError('Path `{}` is not accessible by the current user'.format(full_path))
    ...
    def _print_error(self, message, path):
        ...
        if self.no_color:
            ...
            message = '{red}×{reset} Path {red}{path}{reset} {msg}'.format(red=(Fore.RED),
    ...
    def _print_good_path(self, path):
        ...
        if self.no_color:
            ...
            message = '{green}✓ {path}{reset}'.format(green=(Fore.GREEN),
              reset=(Style.RESET_ALL),
              path=path)
        logger.info(message)
...
def main(argv=None):
    ...
    parser.add_argument('-v',
      '--verbosity', action='count', help='Increase output verbosity')
    parser.add_argument('--no-color',
      action='store_true', dest='disable_colors')
    parser.add_argument('--var',
      dest='variable',
      help='The environment variable to check',
      default='PATH')
    parser.add_argument('--can-add',
      dest='candidate',
      help='Check if a path can be added to the environment variable without problems')
    ...
    util = PythonPathUtil((args.variable),
      verbosity=(args.verbosity), no_color=(args.disable_colors))
    if args.candidate:
        try:
            ...
            if util.no_color:
                ...
                message = '{red}[× path-diag]{reset} {msg}'.format(red=(Fore.RED),
                  reset=(Style.RESET_ALL),
                  msg=error)
                logger.error(message)
            util.RETURN_VALUE = 1
