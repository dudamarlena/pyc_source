...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ddlgenerator/typehelpers.py
...
import doctest, math, re, sqlalchemy as sa, dateutil.parser
...
_complex_enough_to_be_date = re.compile('[\\-\\. /]')
_digits_only = re.compile('^\\d+$')
...
def coerce_to_specific(datum):
    ...
    if str(datum).strip().lower() in ('0', 'false', 'f', 'n', 'no'):
        return False
    if str(datum).strip().lower() in ('1', 'true', 't', 'y', 'yes'):
        return True
    try:
...
def set_worst(old_worst, new_worst):
    ...
    if isinstance(new_worst, bool):
        return new_worst
    negative = hasattr(old_worst, '__neg__') and old_worst < 0 or hasattr(new_worst, '__neg__') and new_worst < 0
    try:
...
def best_representative(d1, d2):
    ...
    if d2 is None:
        return d1
    preference = (
     datetime.datetime, bool, int, Decimal, float, str)
    worst_pref = 0
    worst = ''
