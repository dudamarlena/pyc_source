...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ddlgenerator/reshape.py
# Compiled at: 2015-02-06 12:20:31
# Size of source mod 2**32: 12550 bytes
import logging
...
_illegal_in_column_name = re.compile('[^a-zA-Z0-9_$#]')
...
def walk_and_clean(data):
    ...
    if hasattr(data, 'items') and hasattr(data, '__setitem__'):
        ...
        if isinstance(data, list) or isinstance(data, tuple) or hasattr(data, '__next__') or hasattr(data, 'next'):
...
_sample_data = [
 {'province': 'Québec',  'capital': {'name': 'Québec City',  'pop': 491140},  'id': 1, 
  'province_id': 1,  'cities': [{'name': 'Montreal',  'pop': 1649519}, {'name': 'Laval',  'pop': 401553}]}, {'province': 'Ontario',  'capital': {'name': 'Toronto',  'pop': 2615060},  'province_id': 2,  'cities': [{'name': 'Ottawa',  'pop': 883391}, {'name': 'Missisauga',  'pop': 713443}]},
 {'province': 'New Brunswick',  'capital': {'name': 'Fredricton',  'pop': 56224},  'id': 3, 
  'province_id': 3,  'cities': [{'name': 'Saint John',  'pop': 70063}, {'name': 'Moncton',  'pop': 69074}]}]
...
def unnest_children(data, parent_name='', pk_name=None, force_pk=False):
    ...
    """
    possible_fk_names = [
     '%s_id' % parent_name, '_%s_id' % parent_name, 'parent_id']
    if pk_name:
    ...
    return (
     parent, parent.pk.name if parent.pk else None, children, child_fk_names)
