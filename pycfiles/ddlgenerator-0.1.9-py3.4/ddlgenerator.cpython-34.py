...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ddlgenerator/ddlgenerator.py
# Compiled at: 2015-02-06 13:43:44
# Size of source mod 2**32: 24228 bytes
"""
...
import doctest, logging, os.path, pprint, re, textwrap, sqlalchemy as sa
from sqlalchemy.schema import CreateTable
import dateutil.parser, yaml
try:
    import pymongo
except ImportError:
    pymongo = None
...
try:
    ...
    import typehelpers as th, reshape
...
for engine_name in ('postgresql', 'sqlite', 'mysql', 'oracle', 'mssql'):
    mock_engines[engine_name] = sa.create_engine('%s://' % engine_name, strategy='mock', executor=_dump)
...
class Table(object):
    ...
    def _find_table_name(self, data):
        if not self.table_name:
            ...
            self.table_name = self.table_name or 'generated_table%s' % Table.table_index
            self.table_name = reshape.clean_key_name(self.table_name)
            Table.table_index += 1

    def __init__(self, data, table_name=None, default_dialect=None, save_metadata_to=None, metadata_source=None, varying_length_text=False, uniques=False, pk_name=None, force_pk=False, data_size_cushion=0, _parent_table=None, _fk_field_name=None, reorder=False, loglevel=logging.WARN, limit=None):
        """
        ...
        if isinstance(data, Source):
            ...
            if hasattr(data, 'lower') or hasattr(data, 'read'):
                ...
                if self.table_name.startswith('generated_table'):
                    if hasattr(self.data, 'table_name'):
                ...
                if hasattr(self.data, 'generator') and hasattr(self.data.generator, 'sqla_columns'):
                    ...
                    self.data, self.pk_name, children, child_fk_names = reshape.unnest_children(data=self.data, parent_name=self.table_name, pk_name=pk_name, force_pk=force_pk)
            ...
            if _parent_table:
                ...
                fk = None
        self.table = sa.Table(self.table_name, metadata, *[sa.Column(cname, col['satype'], fk if fk and _fk_field_name == cname else None, primary_key=cname == self.pk_name, unique=uniques and col['is_unique'], nullable=col['is_nullable'], doc=self.comments.get(cname)) for cname, col in self.columns.items()])
        self.children = {child_name:Table(child_data, table_name=child_name, default_dialect=self.default_dialect, varying_length_text=varying_length_text, uniques=uniques, pk_name=pk_name, force_pk=force_pk, data_size_cushion=data_size_cushion, _parent_table=self, reorder=reorder, _fk_field_name=child_fk_names[child_name], metadata_source=child_metadata_sources.get(child_name), loglevel=loglevel) for child_name, child_data in children.items()}
        if save_metadata_to:
            ...
            logging.info('Pass ``--save-metadata-to %s`` next time to re-use structure' % save_metadata_to)
    ...
    def ddl(self, dialect=None, creates=True, drops=True):
        ...
        comments = '\n\n'.join(self._comment_wrapper.fill('in %s: %s' % (col, self.comments[col])) for col in self.comments)
        result = []
        if drops:
    ...
    table_backref_remover = re.compile(',\\s+table\\s*\\=\\<.*?\\>')
    capitalized_words = re.compile('\\b[A-Z]\\w+')
    ...
    def sqlalchemy(self, is_top=True):
        ...
        for constraint in self.table.constraints:
            if isinstance(constraint, sa.sql.schema.UniqueConstraint):
                col_list = ', '.join("'%s'" % c.name for c in constraint.columns)
    ...
    def django_models(self, metadata_source=None):
        sql = self.sql(dialect='postgresql', inserts=False, creates=True, drops=True, metadata_source=metadata_source)
        u = sql.split(';\n')
        try:
            import django
        except ImportError:
        ...
        if django:
            ...
            if not settings.configured:
                settings.configure(DEBUG='on', SECRET_KEY='1234', ALLOWED_HOSTS='localhost', DATABASES={'default': {'NAME': db_filename,  'ENGINE': 'django.db.backends.sqlite3'}})
                django.setup()
    ...
    def emit_db_sequence_updates(self):
        ...
        if self.source.db_engine:
            if self.source.db_engine.name != 'postgresql':
        ...
        if dialect and dialect.startswith('sqla'):
            if self.data:
                ...
                for row in self.data:
                    yield textwrap.indent('conn.execute(inserter, **{row})'.format(row=str(dict(row))), '    ')
            ...
            for row in self.data:
                ...
                vals = ', '.join(str(self._prep_datum(val, dialect, key, needs_conversion)) for key, val in row.items())
    ...
    def sql(self, dialect=None, inserts=False, creates=True, drops=True, metadata_source=None):
        ...
        """
        result = [
         self.ddl(dialect, creates=creates, drops=drops)]
        if inserts:
    ...
    def _fill_metadata_from_sample(self, col):
        ...
        if isinstance(col['sample_datum'], Decimal):
            ...
            col['satype'] = sa.DECIMAL(precision + self.data_size_cushion * 2, scale + self.data_size_cushion)
        else:
            if isinstance(col['sample_datum'], str):
        ...
        if col['satype'] == sa.Integer:
            if col['sample_datum'] > 2147483647 - self.data_size_cushion * 1000000000 or col['sample_datum'] < -2147483647 + self.data_size_cushion * 1000000000:
                col['satype'] = sa.BigInteger
        return col

    types2sa = {datetime.datetime: sa.DateTime,  int: sa.Integer,  float: sa.Numeric, 
     bool: sa.Boolean,  type(None): sa.Text}
    ...
    def _determine_types(self):
        ...
        if hasattr(self.data, 'generator') and hasattr(self.data.generator, 'sqla_columns'):
            for col in self.data.generator.sqla_columns:
                self.columns[col.name] = {'is_nullable': col.nullable,  'is_unique': col.unique, 
                 'satype': col.type, 
                 'pytype': col.pytype}
        ...
        for row in self.data:
            ...
            for k in keys:
                ...
                if k not in self.columns:
                    self.columns[k] = {'sample_datum': v,  'str_length': len(str(v_raw)),  'is_nullable': not (rowcount == 1 and v is not None and str(v).strip()), 
                     'is_unique': set([v])}
                else:
                    col = self.columns[k]
