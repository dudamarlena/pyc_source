...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ddlgenerator/console.py
# Compiled at: 2015-02-10 14:25:30
...
parser.add_argument('--force-key', help='Force every table to have a primary key', action='store_true')
parser.add_argument('-r', '--reorder', help='Reorder fields alphabetically, ``key`` first', action='store_true')
...
parser.add_argument('--save-metadata-to', type=str, metavar='FILENAME', help='Save table definition in FILENAME for later --use-saved-metadata run')
parser.add_argument('--use-metadata-from', type=str, metavar='FILENAME', help='Use metadata saved in FROM for table definition, do not re-analyze table structure')
...
def generate_one(tbl, args, table_name=None, file=None):
    ...
    """
    table = Table(tbl, table_name=table_name, varying_length_text=args.text, uniques=args.uniques, pk_name=args.key, force_pk=args.force_key, reorder=args.reorder, data_size_cushion=args.cushion, save_metadata_to=args.save_metadata_to, metadata_source=args.use_metadata_from, loglevel=args.log, limit=args.limit)
    if args.dialect.startswith('sqla'):
        ...
        if args.dialect.startswith('dj'):
            ...
            print(table.sql(dialect=args.dialect, inserts=args.inserts, creates=not args.no_creates, drops=args.drops, metadata_source=args.use_metadata_from), file=file)
    return table
