...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/robots/events/events.py
# Compiled at: 2015-01-26 11:58:05
"""
pyRobots' events implementation
"""
import logging
logger = logging.getLogger('robots.events')
...
class EventMonitor:
    ...
    def __init__(self, robot, var, value=None, becomes=None, above=None, below=None, increase=None, decrease=None, oneshot=False, max_firing_freq=10, blocking=True):
        self.cbs = []
        self.robot = robot
        self.valid = False
        if not callable(var):
    ...
    def _monitor(self):
        ...
        while self.monitoring:
            ...
            if self.max_firing_freq > 0:
                self.robot.sleep(1.0 / self.max_firing_freq)
        ...
        if not self.robot.dummy:
            if callable(self.var):
                ...
                if self.var not in self.robot.state:
                    ...
                    while self.var not in self.robot.state:
