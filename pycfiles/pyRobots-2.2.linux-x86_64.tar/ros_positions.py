...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/robots/poses/ros_positions.py
# Compiled at: 2015-01-26 11:58:05
import logging
logger = logging.getLogger('robots.position.ros')
try:
    import rospy, tf
except ImportError:
...
class ROSFrames(FrameProvider):
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.error("Timeout while waiting for the TF transformation with the map! Is someone publishing TF tansforms?\n ROS positions won't be available.")
    ...
    def publish_transform(self, name, pose):
        ...
        self.br.sendTransform((pose['x'], pose['y'], pose['z']), (
         pose['qx'], pose['qy'], pose['qz'], pose['qw']), rospy.Time.now(), name, pose['frame'])
    ...
    def inframe(self, pose, frame):
        ...
        if self.tf.frameExists(frame) and self.tf.frameExists(pose_stamped.header.frame_id):
            ...
            return {'x': newPoseStamped.pose.position.x, 'y': newPoseStamped.pose.position.y, 
