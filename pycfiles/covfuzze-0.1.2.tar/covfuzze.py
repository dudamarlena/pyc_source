...
# [GCC 8.4.0]
# Embedded file name: covfuzze/covfuzze.py
# Compiled at: 2018-07-06 10:46:07
import numpy as np, pysam, pandas as pd, seaborn as sns, matplotlib, re, os
...
def plot_gene(gene, outpref, bams, bedname, gtfname, peakname, labels, nsubplots, normalize):
    ...
    coverage = {}
    bed = BedTool(bedname)
    for label, bamname in zip(labels, bams):
        bam = BedTool(bamname)
    ...
    for i, ax in enumerate(axes):
        ...
        ax.xaxis.set_ticks(np.arange(0, gene_len, max(100 * np.round(gene_len / 500.0, 0), 500)))
