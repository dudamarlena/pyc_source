...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/wsgiprofile/controllers.py
# Compiled at: 2008-09-13 07:12:52
import os
...
class CallsController:
    ...
    def chrome(self, req, stats_info, search=None, rowcount=10):
        ...
        template = Template(filename=os.path.join(dirname, 'base.html'), lookup=lookup)
        url_for = req.environ['wsgiprofile.mapper'].generate
        filters = [rowcount]
        if search:
            filters.insert(0, search)
        stats = stats_info.items(filters=filters)
        context = dict(info=stats_info, stats=stats, pid=stats_info.id, url_for=url_for, search=search)
    ...
    def html(self, req, stats_info, search=None, rowcount=10):
        ...
        dirname = os.path.dirname(__file__)
        lookup = TemplateLookup(directories=[dirname])
        template = Template(filename=os.path.join(dirname, 'rows.html'), lookup=lookup)
        url_for = req.environ['wsgiprofile.mapper'].generate
        filters = [rowcount]
        if search:
            filters.insert(0, search)
        stats = stats_info.items(filters=filters)
        context = dict(stats=stats, pid=stats_info.id, url_for=url_for, search=search)
        return template.render(**context)
...
class StaticController:
    ...
    def index(self, environ, start_response):
        app = urlparser.StaticURLParser(os.path.join(os.path.dirname(__file__), 'public'))
