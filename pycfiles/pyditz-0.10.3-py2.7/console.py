...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ditz/console.py
...
def main(args=sys.argv[1:]):
    ...
    parser.add_argument('-v', '--verbose', action='store_true', help='be verbose about things')
    parser.add_argument('--traceback', action='store_true', help='print traceback on error')
    group = parser.add_argument_group('global arguments')
    group.add_argument('-i', '--issue-dir', type=str, metavar='DIR', help='use the given issue directory')
    group = parser.add_argument_group('configuration arguments')
    group.add_argument('-u', '--user-config', type=str, metavar='FILE', help='use the given user config file')
    group.add_argument('-c', '--config', type=str, action='append', metavar='SECTION.OPTION=VALUE', help='set a config option explicitly')
    group = parser.add_argument_group('feature arguments')
    group.add_argument('-S', '--no-search', action='store_true', help='turn off searching of parent directories')
    group.add_argument('-P', '--no-pager', action='store_true', help='turn off paging of output')
    group.add_argument('-H', '--no-highlight', action='store_true', help='turn off syntax highlighting')
    group.add_argument('-N', '--no-setup', action='store_true', help='turn off interactive setup')
    group.add_argument('-X', '--no-plugins', action='store_true', help='turn off loading of external plugins')
    group.add_argument('-V', '--no-vcs', action='store_true', help='turn off use of version control')
    group = parser.add_argument_group('comment arguments')
    group.add_argument('-m', '--comment', type=str, metavar='STRING', help='specify a comment in commands')
    group.add_argument('-n', '--no-comment', action='store_true', help='skip asking for a comment in commands')
    group = parser.add_argument_group('information arguments')
    group.add_argument('-p', '--path', type=str, choices=[
     'repo', 'issues', 'project'], help='print path information')
