...
# [GCC 8.4.0]
# Embedded file name: /Users/rensike/Workspace/icv/icv/image/vis.py
# Compiled at: 2019-11-12 01:24:28
# Size of source mod 2**32: 11312 bytes
...
import random, numpy as np, matplotlib.pyplot as plt, cv2
TEXT_MARGIN = 2
...
def imshow_bboxes(img, bboxes, labels=None, scores=None, classes=None, score_thresh=0, masks=None, color=(244, 67, 54), thickness=1, use_normalized_coordinates=False, is_show=False, save_path=None):
    if not classes is None:
        if not is_seq(classes):
            assert is_np_array(classes)
        elif not labels is None:
            if not is_seq(labels):
                assert is_np_array(labels)
        elif not scores is None:
            if not is_seq(scores):
                assert is_np_array(scores)
    elif not masks is None:
        if not is_seq(masks):
            assert is_np_array(masks)
    else:
    ...
    if bboxes.shape[0] == 0:
        ...
        if labels is not None:
            ...
            if not labels.shape[0] == bboxes.shape[0]:
                raise AssertionError("param labels's length is not equals to bboxes!")
        if masks is not None:
            if not is_np_array(masks):
                masks = np.array(masks)
            masks = masks[np.where(np.array(scores) >= score_thresh)] if scores is not None else masks
            if not masks.shape[0] == bboxes.shape[0]:
                raise AssertionError("param masks's length is not equals to bboxes!")
        if scores is not None:
            if not is_np_array(scores):
                scores = np.array(scores)
            scores = scores[np.where(scores >= score_thresh)]
            if not scores.shape[0] == bboxes.shape[0]:
                raise AssertionError("param scores's length is not equals to bboxes!")
        colorMap = {}
        ...
        for ix, bbox in enumerate(bboxes):
            ...
            if isinstance(bbox, BBox):
                image = imdraw_bbox(image, bbox.xmin, bbox.ymin, bbox.xmax, bbox.ymax, color, thickness, label, use_normalized_coordinates)
            else:
                image = imdraw_bbox(image, bbox[0], bbox[1], bbox[2], bbox[3], color, thickness, label, use_normalized_coordinates)
...
def imdraw_bbox(image, xmin, ymin, xmax, ymax, color=(244, 67, 54), thickness=1, display_str='', text_color=None, use_normalized_coordinates=False):
    if not xmin <= xmax:
        raise AssertionError("xmin shouldn't be langer than xmax!")
    elif not ymin <= ymax:
        raise AssertionError("ymin shouldn't be langer than ymax!")
    image = imread(image)
    image = image.copy()
...
def imdraw_polygons_with_bbox(image, polygons, label_list=None, is_show=False, save_path=None, alpha=0.45, outline=1, color_map=None, with_bbox=False, bbox_color=None, text_color=None):
    image = imread(image)
    ...
    if len(polygons) <= 0:
        ...
        if not label_list is None:
            if not len(polygons) == len(label_list):
                ...
                if not color_map is None:
                    ...
                    if isinstance(text_color, list):
                        if not len(text_color) == len(polygons):
                            raise AssertionError
            ...
            for ix, (polygon, label) in enumerate(list(zip(polygons, label_list))):
                ...
                if tcolor is None:
                    if isinstance(text_color, list):
                        ...
                        tcolor = text_color
                if label not in tcolor_color_map:
                ...
                if bcolor is None:
                    if isinstance(bbox_color, list):
                        ...
                        bcolor = bbox_color
                if label not in bcolor_color_map:
                ...
                if with_bbox:
                    image = imdraw_bbox(image, left, top, right, bottom, color=bbox_color, thickness=outline, display_str=label, text_color=tcolor)
...
def imdraw_text(image, text='-', x=0, y=0, font_scale=0.6, thickness=1, text_color=(255, 255, 255), with_bg=True, bg_color=(244, 67, 54), bg_alpha=60):
    image = imread(image)
