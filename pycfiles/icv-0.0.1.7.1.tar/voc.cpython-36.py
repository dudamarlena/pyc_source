...
# [GCC 8.4.0]
# Embedded file name: /Users/rensike/Workspace/icv/icv/data/voc.py
# Compiled at: 2019-11-21 06:16:24
# Size of source mod 2**32: 13097 bytes
from .dataset import IcvDataSet
import os, shutil, numpy as np
from ..utils import load_voc_anno, save_voc_anno, make_empty_voc_anno, fcopy, list_from_file, list_to_file, is_file, is_dir, mkdir
...
class Voc(IcvDataSet):
    ...
    def concat(self, voc, output_dir, reset=False, new_split=None):
        if not isinstance(voc, Voc):
            ...
            if new_split is None:
                ...
                anno_path, image_path, imgset_path, imgset_seg_path, seg_class_image_path, seg_object_image_path = Voc.reset_dir(output_dir,
                  reset=reset)
                ...
                if self.mode == voc.mode:
                    ...
                    voc.is_seg_mode or list_to_file(ids1, os.path.join(imgset_path, '%s.txt' % new_split))
            else:
                list_to_file(ids1, os.path.join(imgset_seg_path, '%s.txt' % new_split))
        return Voc(output_dir,
          split=new_split,
    ...
    def reset_dir(dist_dir, reset=False):
        ...
        return (
         anno_path, image_path, imgset_path, imgset_seg_path, seg_class_image_path, seg_object_image_path)
    ...
    def get_sample(self, id):
        if id in self.sample_db:
            ...
            sample_meta = SampleMeta({k:anno_data[k] for k in anno_data if k not in ('object', )})
                ...
                for obj in anno_data['object']:
                    if 'difficult' in obj:
                        ...
                        anno = Anno(bbox=BBox(xmin=xmin,
                          ...
                          meta=(AnnoMeta({k:obj[k] for k in obj if k not in ('name',
                                                                             'bndbox')})))
            ...
            sample = Sample(id, image_file, annos, sample_meta)
    ...
    def _write_sample(self, sample, dist_dir):
        ...
        anno_path, image_path, imgset_path, imgset_seg_path, seg_class_image_path, seg_object_image_path = Voc.reset_dir(dist_dir, reset_dir)
        anno_dist_path = os.path.join(anno_path, '%s.xml' % sample.name)
        anno_data = {'segmented':1 if self.is_seg_mode else 0, 
         'folder':image_path, 
         'filename':os.path.basename(sample.path), 
         'size':{'width':sample.width, 
          'height':sample.height, 
          'depth':sample.dim}, 
         'objects':[]}
        for ix, anno in enumerate(sample.annos):
            bndbox = anno.meta.dict()
            if 'truncated' not in bndbox:
                if 'difficult' not in bndbox:
                    bndbox = {'truncated':1, 
                     'difficult':0}
            bndbox['name'] = anno.label
            bndbox['bndbox'] = {'xmin':anno.bbox.xmin_int, 
            ...
            if anno.seg_mode_mask:
                ...
                if anno.seg_mode_polys:
                    ...
                    anno.polys.to_mask(sample.height, sample.width).save((os.path.join(seg_class_image_path, '%s.png' % sample.name)),
                      id=catid)
                    anno.polys.to_mask(sample.height, sample.width).save((os.path.join(seg_object_image_path, '%s.png' % sample.name)),
                      id=(ix + 1))
