...
# [GCC 8.4.0]
# Embedded file name: /Users/rensike/Workspace/icv/icv/data/converter/labelme_converter.py
# Compiled at: 2019-10-29 08:00:41
...
class LabelMeConverter(object):
    ...
    def to_voc(self, voc_root, split=None, reset=False):
        ...
        setfile = os.path.join(imgset_seg_path, '%s.txt' % split) if self.labelme.is_seg_mode else os.path.join(imgset_path, '%s.txt' % split)
        mkfile(setfile)
        voc = Voc(voc_root,
          split,
          keep_no_anno_image=(self.labelme.keep_no_anno_image),
