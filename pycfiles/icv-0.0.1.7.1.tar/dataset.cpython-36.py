...
# [GCC 8.4.0]
# Embedded file name: /Users/rensike/Workspace/icv/icv/data/dataset.py
# Compiled at: 2019-11-12 01:24:24
# Size of source mod 2**32: 20593 bytes
...
class IcvDataSet(object):
    ...
    def set_colormap(self, color_list_or_map=None):
        if not color_list_or_map is None:
            if not isinstance(color_list_or_map, dict):
                assert is_seq(color_list_or_map)
        else:
            if color_list_or_map is not None:
                if len(self.categories) > 0:
                    if isinstance(color_list_or_map, dict):
                        ...
                        self.color_map.update({c:random_sample(VIS_COLOR) for c in self.categories if c not in self.color_map})
                    else:
                        if is_seq(color_list_or_map):
                            if len(color_list_or_map) > 0:
                                self.color_map = {self.categories[i]:color_list_or_map[(i % len(color_list_or_map))] for i in range(len(self.categories))}
                    return
        self.color_map = {}
        _colors = VIS_COLOR
    ...
    def get_samples(self, ids=None):
        ...
        for id in tqdm(ids):
            ...
            if sample.count == 0:
                if not self.keep_no_anno_image:
                    continue
    ...
    def divide(self, output_dir, splits=None, ratios=None):
        if not (is_seq(splits) and len(splits) > 0):
            raise AssertionError
        elif not (is_seq(ratios) and len(ratios) == len(splits)):
            raise AssertionError
        ids = self.ids[:]
    ...
    def crop_bbox_for_classify(self, output_dir, reset=False, split='train', pad=0, resize=None):
        if not resize is None:
            if not (is_seq(resize) and len(resize) == 2):
                raise AssertionError
        else:
            if reset:
    ...
    def statistic(self, print_log=True, is_plot_show=False, plot_save_path=None):
        ...
        sta['overview'] = {'image_num':self.length, 
        ...
        sta['detail'] = {cat:{'num':0,  'ratio_distribution':OrderedDict()} for cat in self.categories}
        for sample in samples:
            s = sample.statistics()
            for label in s['cats']:
        ...
        sta['overview']['ratio_distribution'] = dict(OrderedDict(sorted((sta['overview']['ratio_distribution'].items()), key=(lambda t: -t[1]))[:RATIO_TOP_K]))
        ratio_kvs_totals = [(r, c) for r, c in sta['detail'][label]['ratio_distribution'].items() for label in sta['detail']]
        ratio_kvs = []
        forget_rks = []
        ...
        for label in sta['detail']:
            sta['detail'][label]['ratio_distribution'] = {k:sta['detail'][label]['ratio_distribution'][k] for k in ratio_keys if k in sta['detail'][label]['ratio_distribution']}
        ...
        if print_log or is_plot_show or plot_save_path is not None:
            ...
            target_statistic_header = [
             'bbox_num', 'small_bbox_num', 'middle_bbox_num', 'large_bbox_num']
            ...
            cats_detail_header = [
             'bbox ratio (h/w) \\ class'] + self.categories
            ...
            if is_plot_show or plot_save_path is not None:
                ...
                b1 = ax1.barh((list(range(len(overview_header_post)))), overview_table_data_post, color='#6699CC', tick_label=overview_header_post)
                for i, rect in enumerate(b1):
                    w = rect.get_width()
                    ax1.text((w + 1), (rect.get_y() + rect.get_height() / 2), ('%d' % int(overview_table_data_post[i])), ha='left',
                      va='center')
                ...
                for xi, rect in enumerate(b2):
                    ax2.text((rect.get_x() + rect.get_width() / 2), (rect.get_height()), ('%d' % int(cats_num_data[xi])), ha='center',
                      va='bottom')

                ax2.set_title('class detail')
                ax3 = fig.add_subplot(3, 2, 3)
                b3 = ax3.barh((list(range(len(target_statistic_header)))), target_statistic_data, color='green', tick_label=target_statistic_header)
                for i, rect in enumerate(b3):
                    w = rect.get_width()
                    ax3.text(w, (rect.get_y() + rect.get_height() / 2), ('%d' % int(target_statistic_data[i])), ha='left',
                      va='center')
                ...
                for xi, rect in enumerate(b4):
                    ax4.text((rect.get_x() + rect.get_width() / 2), (rect.get_height()), ('%d' % int(ratio_distri_data[xi])), ha='center',
                      va='bottom')
