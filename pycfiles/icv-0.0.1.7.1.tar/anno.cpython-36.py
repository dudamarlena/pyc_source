...
# [GCC 8.4.0]
# Embedded file name: /Users/rensike/Workspace/icv/icv/utils/anno.py
# Compiled at: 2019-11-14 22:23:51
...
def save_voc_anno(anno_data, anno_dist_path):
    if not 'folder' in anno_data:
        raise AssertionError
    else:
        if not 'filename' in anno_data:
            raise AssertionError
        else:
            if not 'size' in anno_data:
                raise AssertionError
            else:
                if not 'width' in anno_data['size']:
                    raise AssertionError
                elif not 'height' in anno_data['size']:
                    raise AssertionError
...
def make_empty_coco_anno(**kwargs):
    empty_anno = {'images':[],  'type':'instances', 
...
def make_empty_voc_anno(**kwargs):
    empty_anno = {'folder':'', 
     'filename':'', 
     'source':{'database':'The VOC2012 Database', 
...
def reset_coco_dir(dist_dir):
    ...
    return (
     anno_dir, train_image_dir, test_image_dir, val_image_dir)