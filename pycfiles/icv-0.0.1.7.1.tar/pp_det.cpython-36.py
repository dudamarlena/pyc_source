...
# [GCC 8.4.0]
# Embedded file name: /Users/rensike/Workspace/icv/icv/detector/pp_det.py
# Compiled at: 2019-10-15 23:06:09
...
try:
    ...
    raise Exception('You shoule install PaddleDetection first (https://github.com/PaddlePaddle/models/blob/develop/PaddleCV/PaddleDetection/docs/INSTALL.md). Import PaddleDetection Error!')
...
class PPDetector(Detector):
    ...
    def __init__(self, model_dir, config_file, categories, iou_thr=0.5, score_thr=0, use_gpu=True):
        ...
        if not is_dir(model_dir):
            raise AssertionError('model dir is not exist.')
        else:
    ...
    def init_model(self):
        ...
        if hasattr(self.model, 'is_bbox_normalized') and callable(self.model.is_bbox_normalized):
    ...
    def _infer(self, images, score_thr=-1):
        ...
        for iter_id, data in enumerate(self.reader()):
            timer = Timer()
            outs = self.exe.run((self.infer_prog), feed=(self.feeder.feed(data)),
              fetch_list=values,
              return_numpy=False)
            ...
            if 'mask' in res:
                segms_results = mask2out([res], self.clsid2catid, self.model.mask_head.resolution)
            det_result_list.append(self._build_result(bbox_results, img_ids=img_ids,
