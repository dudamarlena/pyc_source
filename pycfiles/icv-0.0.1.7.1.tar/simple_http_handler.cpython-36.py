...
# [GCC 8.4.0]
# Embedded file name: /Users/rensike/Workspace/icv/icv/core/http/server/simple_http_handler.py
# Compiled at: 2019-08-05 08:28:27
# Size of source mod 2**32: 8590 bytes
...
class IcvHandler(SimpleHTTPRequestHandler):
        ...
        try:
            ...
            if target is None:
                if up.path.rstrip('/') == '':
                    ...
                    self.send_error(HTTPStatus.NOT_FOUND, 'Not Found handler (%r)' % self.path)
                    return
            ms, func, _ = target
            if isinstance(ms, str):
                if self.command != ms:
                    self.send_error(HTTPStatus.METHOD_NOT_ALLOWED, 'method not allowed (%r)' % self.command)
                    return
            if isinstance(ms, (list, tuple)):
                if self.command not in ms:
                    self.send_error(HTTPStatus.METHOD_NOT_ALLOWED, 'method not allowed (%r)' % self.command)
    ...
    def _match(self, rpath):
        for action in routes:
            ...
            if result:
                return (
                 handler_method, handler_func, [x for x in result.groups()])
            ...
            if method is None:
                ...
                if isinstance(handler_method, str):
                    if handler_method.upper() != str(method).upper():
                        ...
                        self.finish()
                        return
                elif str(method).upper() not in handler_method:
                    self.error(501, HTTP_STATUS_CODES[501][0])
                    self.finish()
                    return
            return target
