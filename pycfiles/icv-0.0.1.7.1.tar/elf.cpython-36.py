...
# [GCC 8.4.0]
# Embedded file name: /Users/rensike/Workspace/icv/icv/data/elf.py
# Compiled at: 2019-11-11 01:59:23
# Size of source mod 2**32: 10610 bytes
...
class ElfFiles(object):
    ...
    def __init__(self, image_file, anno_file, attachment_dir=None):
        ...
        elif not is_file(anno_file):
            raise AssertionError
        elif not attachment_dir is None:
            assert is_file(attachment_dir)
...
class Elf(IcvDataSet):
    ...
    def __init__(self, image_anno_attachment_pathlist, split='trainval', keep_no_anno_image=True, categories=None, one_index=False):
        ...
        for f in image_anno_attachment_pathlist:
            ...
            if is_seq(f):
                ...
                if not ('image_file' in f and 'anno_file' in f):
                    raise AssertionError
                else:
    ...
    def _parse_anno_item(self, ix, obj, id, anno_data):
        if 'name' in obj:
            ...
            if 'bndbox' in obj:
                ...
                anno = Anno(bbox=BBox(xmin=xmin, ymin=ymin, xmax=xmax, ymax=ymax, label=label),
                  label=label,
                  color=(self.color_map[label]),
                  meta=(AnnoMeta()))
                return [
                 anno]
            if 'attachments' in anno_data or 'doc' in anno_data and 'attachments' in anno_data['doc']:
    ...
    def _parse_json_annotation(self, id, anno_data):
        annos = []
        if 'labeled' in anno_data:
            if not anno_data['labeled']:
                return annos
        if 'outputs' in anno_data:
            if 'object' in anno_data['outputs']:
