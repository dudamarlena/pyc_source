...
# [GCC 8.4.0]
# Embedded file name: /Users/rensike/Workspace/icv/icv/data/core/polys.py
# Compiled at: 2019-09-14 05:44:14
# Size of source mod 2**32: 11106 bytes
import numpy as np
...
class Polygon(Target):
    ...
    def __init__(self, exterior, label=None, **kwargs):
        if isinstance(exterior, list):
            if not exterior:
                ...
                if isinstance(exterior[0], Keypoint):
                    self.exterior = np.float32([[point.x, point.y] for point in exterior])
                else:
                    if not isinstance(exterior[0], (list, tuple)):
            ...
            if not is_np_array(exterior):
                raise AssertionError('Expected exterior to be a list of tuples (x, y) or an (N, 2) array, got type %s' % (exterior,))
            elif not (exterior.ndim == 2 and exterior.shape[1] == 2):
                raise AssertionError('Expected exterior to be a list of tuples (x, y) or an (N, 2) array, got an array of shape %s' % (exterior.shape,))
            self.exterior = np.float32(exterior)
        if len(self.exterior) >= 2:
            if np.allclose(self.exterior[0, :], self.exterior[-1, :]):
    ...
    def deepcopy(self, exterior=None, label=None, **kwargs):
        ...
        imgaug.Polygon
            Deep copy.
        """
        return Polygon(exterior=np.copy(self.exterior) if exterior is None else exterior, 
         label=self.label if label is None else label, **kwargs)

    def draw_on_image(self, image, color=(244, 67, 54), alpha=0.45, copy=True, raise_if_out_of_image=False):
