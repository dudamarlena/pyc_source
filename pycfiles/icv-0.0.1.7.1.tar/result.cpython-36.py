...
# [GCC 8.4.0]
# Embedded file name: /Users/rensike/Workspace/icv/icv/detector/result.py
# Compiled at: 2019-09-20 09:09:57
...
class DetectionResult(object):
    ...
    def __init__(self, det_bboxes, det_classes, det_scores, det_masks=None, det_time=0, det_image=None, categories=None, score_thr=0):
        if not is_seq(det_bboxes):
            raise AssertionError('param det_bboxes should be a sequence.')
        else:
    ...
    def filter(self, min_score=0, max_score=1):
        ...
        if min_score <= 0:
            if max_score >= 1:
                return result
        filter_ids = np.where(np.logical_and(np.array(result.det_scores) >= min_score, np.array(result.det_scores) <= max_score))
        ...
        if result._det_masks is not None:
            if result._det_masks.shape[0] > 0:
    ...
    def to_dict(self):
        return dict(bboxes=(self.det_bboxes.tolist()),
    ...
    def vis(self, img):
        image_drawed = imshow_bboxes(img, (self.det_bboxes),
    ...
    def deepcopy(self):
        result_copied = DetectionResult(det_bboxes=(self._det_bboxes),
