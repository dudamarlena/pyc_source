...
# [GCC 8.4.0]
# Embedded file name: /Users/rensike/Workspace/icv/icv/detector/service/server.py
# Compiled at: 2019-08-12 07:51:38
...
class DetectorServer(object):
    ...
    def __init__(self, detector, secrect=None, open_web=True, name='', default_params_score_thr=0.5):
    ...
    def _inference_server(self):
        ...
        try:
            if self.secrect is not None:
                if 'secret' in request_headers:
                    if self._decrypt(request_headers['secret']) == self.secrect:
                        pass
            ...
            if request_data is None:
                ...
                try:
                    ...
                    if not isinstance(image_np, np.ndarray):
                        return self._json_response(ERROR_IMAGE_FORMAT, 'parse input image failed, please check input format.')
                ...
                try:
                    ...
                    return self._json_response(ERROR_DETECT_ERROR, 'predict failure, {0}:{1}'.format(e.__class__.__name__, str(e)))

                outputs = DetectionProcessor.post_process(outputs)
                return self._json_response(0, 'success', True, outputs)

    def _inference_page(self):
        return template((self.html_path),
          name=(self.name),
    ...
    def _inference_upload_for_predict(self):
        ...
        if ext not in ('.png', '.jpg', '.jpeg', '.tif', '.bmp'):
            ...
            if isinstance(inference_result, DetectionResult):
                return self._json_response(code=ERROR_DETECT_ERROR,
                  success=True,
                  data={'最大置信度':'%.4f' % inference_result.topk(1)[0][1], 
