...
# [GCC 8.4.0]
# Embedded file name: /Users/rensike/Workspace/icv/icv/detector/tf_obj.py
# Compiled at: 2019-10-15 23:06:12
# Size of source mod 2**32: 8429 bytes
import os
...
class TfObjectDetector(Detector):
    ...
    def __init__(self, model_path, labelmap_path=None, categories=None, iou_thr=0.5, score_thr=0, device=None):
        assert is_file(model_path)
        if not (is_seq(categories) and len(categories) > 0):
            if not is_file(labelmap_path):
                raise AssertionError
        super(TfObjectDetector, self).__init__(categories=categories, labelmap_path=labelmap_path, iou_thr=iou_thr, score_thr=score_thr,
          device=device)
    ...
    def inference(self, image, is_show=False, save_path=None, score_thr=-1):
        ...
        detection_bboxes = [BBox(xmin=(det_box[1] * img_width), ymin=(det_box[0] * img_height), xmax=(det_box[3] * img_width), ymax=(det_box[2] * img_height)) for det_box in detection_boxes]
        score_thr = score_thr if score_thr >= 0 else self.score_thr
        det_result = DetectionResult(det_bboxes=detection_bboxes,
    ...
    def inference_batch(self, image_batch, save_dir=None, resize=None, score_thr=-1):
        ...
        if isinstance(image_batch, np.ndarray):
            ...
            if resize is None:
                ...
                image_np_batch = np.array([imresize(img_np, resize) for img_np in image_np_list])
        timer = Timer()
        output_dict = self.sess.run((self.tensor_dict), feed_dict={self.image_tensor: image_np_batch})
        inference_time = timer.since_start()
        score_thr = score_thr if score_thr >= 0 else self.score_thr
        det_result_list = []
        for ix, image in enumerate(image_np_batch):
            ...
            detection_classes = output_dict['detection_classes'][ix].astype(np.uint8)
