...
# [GCC 8.4.0]
# Embedded file name: /Users/rensike/Workspace/icv/icv/data/coco.py
# Compiled at: 2019-11-20 01:07:54
# Size of source mod 2**32: 10163 bytes
import os, shutil
...
from ..utils import fcopy, is_file, is_empty, is_dir, make_empty_coco_anno, json_encode, encode_to_file, decode_from_file
import numpy as np
from tqdm import tqdm

class Coco(IcvDataSet):

    def __init__(self, image_dir, anno_file, split=None, keep_no_anno_image=True, one_index=True, transform=None, target_transform=None):
        if not os.path.isdir(image_dir):
            raise AssertionError('param image_dir is not a dir!')
        elif not os.path.exists(anno_file):
            raise AssertionError('param anno_file is not exist!')
    ...
    def concat(self, coco, out_dir=None, reset=False, new_split=None):
        ...
        fcopy([os.path.join(self.image_dir, self.coco.loadImgs(id)[0]['file_name']) for id in self.ids], dist_image_path)
        ...
        return Coco(image_dir=dist_image_path,
    ...
    def reset_dir(dist_dir, split, reset=False):
        ...
        return (
         dist_anno_path, dist_image_path)
    ...
    def get_sample(self, id):
        if id in self.sample_db:
            ...
            for ann in self.coco.imgToAnns[id]:
                ...
                annos.append(Anno(bbox=BBox(xmin=xmin, ymin=ymin, xmax=(xmin + width), ymax=(ymin + height), label=cat),
                  polys=(Polygon((ann['segmentation'][0]), label=cat) if 'segmentation' in ann else None),
                  label=cat,
                  color=(self.color_map[cat]),
                  meta=(AnnoMeta({k:ann[k] for k in ann if k not in ('bbox', 'category_id',
                                                                     'segmentation')}))))

            img_info = self.coco.imgs[id]
            sample = Sample(name=(path.rsplit('.', 1)[0]),
              image=image_file,
              annos=annos,
    ...
    def save(self, output_dir, reset_dir=False, split=None, raw_copy=False):
        if raw_copy:
            if is_file(self.anno_file):
                if not is_empty(self.image_dir):
    ...
    def _write_sample(self, dist_dir, split=None):
        ...
        if os.path.isdir(dist_dir):
            for id in self.ids:
                ...
                annotation['images'].append({'id':id, 
                ...
                for anno in sample.annos:
                    anno_dict = {}
                    anno_dict['bbox'] = [
                     anno.bbox.xmin, anno.bbox.ymin, anno.bbox.width, anno.bbox.height]
                    if anno.seg_mode_polys:
                        anno_dict['segmentation'] = [
                         anno.polys.exterior.flatten().tolist()]
                        anno_dict['area'] = anno.polys.area
                        anno_dict['iscrowd'] = 0
                    else:
                        if anno.seg_mode_mask:
                            anno_dict['segmentation'] = [
                             anno.mask.to_ploygons().exterior.flatten().tolist()]
