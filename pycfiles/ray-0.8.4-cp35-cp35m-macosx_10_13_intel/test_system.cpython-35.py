...
# [GCC 8.4.0]
# Embedded file name: /private/var/folders/nz/vv4_9tw56nv9k3tkvyszvwg80000gn/T/pip-unpacked-wheel-_x_fz5x6/psutil/tests/test_system.py
# Compiled at: 2020-04-01 16:57:58
# Size of source mod 2**32: 36162 bytes
"""Tests for system APIS."""
import contextlib, datetime, errno, os, pprint, shutil, signal, socket, sys, tempfile, time, psutil
    ...
    def test_prcess_iter_w_attrs(self):
        ...
        with mock.patch('psutil._psplatform.Process.cpu_times', side_effect=psutil.AccessDenied(0, '')) as (m):
            ...
            for p in psutil.process_iter(attrs=[
             'pid', 'cpu_times'], ad_value=flag):
                self.assertIs(p.info['cpu_times'], flag)
                self.assertGreaterEqual(p.info['pid'], 0)

            assert m.called

    @unittest.skipIf(PYPY and WINDOWS, 'get_test_subprocess() unreliable on PYPY + WINDOWS')
        ...
        if os.name == 'posix':
            ...
            if 'linux' in sys.platform.lower():
                ...
                if 'bsd' in sys.platform.lower():
                    ...
                    if 'sunos' in sys.platform.lower() or 'solaris' in sys.platform.lower():
                        assert psutil.SUNOS
                        names.remove('SUNOS')
...
class TestCpuAPIs(unittest.TestCase):
    ...
    def test_cpu_count_none(self):
        for val in (-1, 0, None):
            with mock.patch('psutil._psplatform.cpu_count_logical', return_value=val) as (m):
                ...
                assert m.called
            with mock.patch('psutil._psplatform.cpu_count_physical', return_value=val) as (m):
    ...
    def test_cpu_stats(self):
        ...
        self.assertEqual(infos._fields, ('ctx_switches', 'interrupts', 'soft_interrupts',
                                         'syscalls'))
...
class TestNetAPIs(unittest.TestCase):
                ...
                if nic_stats[nic].isup:
                    if addr.family == socket.AF_INET:
                        ...
                        for ip in (addr.address, addr.netmask, addr.broadcast,
                         addr.ptp):
                            if ip is not None and addr.family != socket.AF_INET6:
    ...
    def test_net_if_addrs_mac_null_bytes(self):
        if POSIX:
            ret = [
             (
              'em1', psutil.AF_LINK, '06:3d:29', None, None, None)]
        else:
            ret = [
             ('em1', -1, '06-3d-29', None, None, None)]
