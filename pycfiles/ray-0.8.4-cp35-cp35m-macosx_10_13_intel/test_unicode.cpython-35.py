...
# [GCC 8.4.0]
# Embedded file name: /private/var/folders/nz/vv4_9tw56nv9k3tkvyszvwg80000gn/T/pip-unpacked-wheel-_x_fz5x6/psutil/tests/test_unicode.py
# Compiled at: 2020-04-01 16:57:58
# Size of source mod 2**32: 13246 bytes
"""
...
if PY3:
    INVALID_NAME = (TESTFN.encode('utf8') + b'f\xc0\x80').decode('utf8', 'surrogateescape')
else:
    INVALID_NAME = TESTFN + 'fÀ\x80'
...
class _BaseFSAPIsTests(object):
    ...
    @unittest.skipIf(PYPY and WINDOWS, 'copyload_shared_lib() unsupported on PYPY + WINDOWS')
...
@unittest.skipIf(not subprocess_supports_unicode(TESTFN_UNICODE), "subprocess can't deal with unicode")
class TestFSAPIs(_BaseFSAPIsTests, unittest.TestCase):
    __doc__ = 'Test FS APIs with a funky, valid, UTF8 path name.'
...
@unittest.skipIf(not subprocess_supports_unicode(INVALID_NAME), "subprocess can't deal with invalid unicode")
class TestFSAPIsWithInvalidPath(_BaseFSAPIsTests, unittest.TestCase):
    __doc__ = 'Test FS APIs with a funky, invalid path name.'
...
class TestNonFSAPIS(unittest.TestCase):
    __doc__ = 'Unicode tests for non fs-related APIs.'
