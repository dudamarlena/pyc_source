...
# [GCC 8.4.0]
# Embedded file name: /private/var/folders/nz/vv4_9tw56nv9k3tkvyszvwg80000gn/T/pip-unpacked-wheel-_x_fz5x6/psutil/tests/test_connections.py
# Compiled at: 2020-04-01 16:57:58
# Size of source mod 2**32: 25293 bytes
"""Tests for net_connections() and Process.connections() APIs."""
import contextlib, errno, os, socket, textwrap
...
class Base(object):
    ...
    def check_connection_ntuple(self, conn):
        ...
        def check_family(conn):
            ...
            if enum is not None:
                if not isinstance(conn.family, enum.IntEnum):
                    raise AssertionError(conn)
            elif not isinstance(conn.family, int):
                raise AssertionError(conn)
        ...
        def check_type(conn):
            ...
            if enum is not None:
                if not isinstance(conn.type, enum.IntEnum):
                    raise AssertionError(conn)
            elif not isinstance(conn.type, int):
                raise AssertionError(conn)
...
class TestUnconnectedSockets(Base, unittest.TestCase):
    __doc__ = 'Tests sockets which are open but not connected to anything.'
...
class TestConnectedSocket(Base, unittest.TestCase):
    __doc__ = 'Test socket pairs which are are actually connected to\n    each other.\n    '
...
class TestFilters(Base, unittest.TestCase):
    ...
    def test_combos(self):
        ...
        def check_conn(proc, conn, family, type, laddr, raddr, status, kinds):
            all_kinds = ('all', 'inet', 'inet4', 'inet6', 'tcp', 'tcp4', 'tcp6', 'udp',
                         'udp4', 'udp6')
            self.check_connection_ntuple(conn)
        ...
        tcp4_template = Template(tcp_template).substitute(family=int(AF_INET), addr='127.0.0.1', testfn=testfile)
        udp4_template = Template(udp_template).substitute(family=int(AF_INET), addr='127.0.0.1', testfn=testfile)
        tcp6_template = Template(tcp_template).substitute(family=int(AF_INET6), addr='::1', testfn=testfile)
        udp6_template = Template(udp_template).substitute(family=int(AF_INET6), addr='::1', testfn=testfile)
        ...
        for p in thisproc.children():
            ...
            for conn in cons:
                if p.pid == tcp4_proc.pid:
                    check_conn(p, conn, AF_INET, SOCK_STREAM, tcp4_addr, (), psutil.CONN_LISTEN, ('all',
                                                                                                  'inet',
                                                                                                  'inet4',
                                                                                                  'tcp',
                                                                                                  'tcp4'))
                else:
                    if p.pid == udp4_proc.pid:
                        check_conn(p, conn, AF_INET, SOCK_DGRAM, udp4_addr, (), psutil.CONN_NONE, ('all',
                                                                                                   'inet',
                                                                                                   'inet4',
                                                                                                   'udp',
                                                                                                   'udp4'))
                    else:
                        if p.pid == getattr(tcp6_proc, 'pid', None):
                            check_conn(p, conn, AF_INET6, SOCK_STREAM, tcp6_addr, (), psutil.CONN_LISTEN, ('all',
                                                                                                           'inet',
                                                                                                           'inet6',
                                                                                                           'tcp',
                                                                                                           'tcp6'))
                        elif p.pid == getattr(udp6_proc, 'pid', None):
                            check_conn(p, conn, AF_INET6, SOCK_DGRAM, udp6_addr, (), psutil.CONN_NONE, ('all',
                                                                                                        'inet',
                                                                                                        'inet6',
                                                                                                        'udp',
                                                                                                        'udp6'))
...
class TestSystemWideConnections(Base, unittest.TestCase):
    __doc__ = 'Tests for net_connections().'
