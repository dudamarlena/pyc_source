...
# [GCC 8.4.0]
# Embedded file name: /private/var/folders/nz/vv4_9tw56nv9k3tkvyszvwg80000gn/T/pip-unpacked-wheel-_x_fz5x6/psutil/tests/test_misc.py
# Compiled at: 2020-04-01 16:57:58
# Size of source mod 2**32: 38748 bytes
"""
Miscellaneous tests.
"""
import ast, collections, contextlib, errno, json, os, pickle, socket, stat
...
class TestMisc(unittest.TestCase):
    ...
    def test_no_such_process__repr__(self, func=repr):
        self.assertEqual(repr(psutil.NoSuchProcess(321)), 'psutil.NoSuchProcess process no longer exists (pid=321)')
        self.assertEqual(repr(psutil.NoSuchProcess(321, name='foo')), "psutil.NoSuchProcess process no longer exists (pid=321, name='foo')")
        self.assertEqual(repr(psutil.NoSuchProcess(321, msg='foo')), 'psutil.NoSuchProcess foo')

    def test_zombie_process__repr__(self, func=repr):
        self.assertEqual(repr(psutil.ZombieProcess(321)), "psutil.ZombieProcess process still exists but it's a zombie (pid=321)")
        self.assertEqual(repr(psutil.ZombieProcess(321, name='foo')), "psutil.ZombieProcess process still exists but it's a zombie (pid=321, name='foo')")
        self.assertEqual(repr(psutil.ZombieProcess(321, name='foo', ppid=1)), "psutil.ZombieProcess process still exists but it's a zombie (pid=321, name='foo', ppid=1)")
        self.assertEqual(repr(psutil.ZombieProcess(321, msg='foo')), 'psutil.ZombieProcess foo')

    def test_access_denied__repr__(self, func=repr):
        self.assertEqual(repr(psutil.AccessDenied(321)), 'psutil.AccessDenied (pid=321)')
        self.assertEqual(repr(psutil.AccessDenied(321, name='foo')), "psutil.AccessDenied (pid=321, name='foo')")
        self.assertEqual(repr(psutil.AccessDenied(321, msg='foo')), 'psutil.AccessDenied foo')

    def test_timeout_expired__repr__(self, func=repr):
        self.assertEqual(repr(psutil.TimeoutExpired(321)), 'psutil.TimeoutExpired timeout after 321 seconds')
        self.assertEqual(repr(psutil.TimeoutExpired(321, pid=111)), 'psutil.TimeoutExpired timeout after 321 seconds (pid=111)')
        self.assertEqual(repr(psutil.TimeoutExpired(321, pid=111, name='foo')), "psutil.TimeoutExpired timeout after 321 seconds (pid=111, name='foo')")
    ...
    def test__all__(self):
        ...
        for name in dir_psutil:
            if name in ('callable', 'error', 'namedtuple', 'tests', 'long', 'test',
                        'NUM_CPUS', 'BOOT_TIME', 'TOTAL_PHYMEM', 'PermissionError',
                        'ProcessLookupError'):
                pass
                ...
                try:
                    ...
                    if name not in psutil.__all__:
                        ...
                        if fun is None:
                            continue
                        if fun.__doc__ is not None and 'deprecated' not in fun.__doc__.lower():
    ...
    def test_version(self):
        self.assertEqual('.'.join([str(x) for x in psutil.version_info]), psutil.__version__)
    ...
    def test_isfile_strict(self):
        ...
        with mock.patch('psutil._common.os.stat', side_effect=OSError(errno.EACCES, 'foo')):
            self.assertRaises(OSError, isfile_strict, this_file)
        with mock.patch('psutil._common.os.stat', side_effect=OSError(errno.EINVAL, 'foo')):
    ...
    def test_ad_on_process_creation(self):
        with mock.patch.object(psutil.Process, 'create_time', side_effect=psutil.AccessDenied) as (meth):
            psutil.Process()
            assert meth.called
        with mock.patch.object(psutil.Process, 'create_time', side_effect=psutil.ZombieProcess(1)) as (meth):
            psutil.Process()
            assert meth.called
        with mock.patch.object(psutil.Process, 'create_time', side_effect=ValueError) as (meth):
...
class TestWrapNumbers(unittest.TestCase):
    ...
    def test_changing_keys_w_wrap(self):
        ...
        input = {'disk1': nt(50, 50, 50), 
         'disk2': nt(100, 100, 10)}
        self.assertEqual(wrap_numbers(input, 'disk_io'), {'disk1': nt(50, 50, 50), 
    ...
    def test_cache_wrap(self):
        ...
        def assert_():
            cache = wrap_numbers.cache_info()
            self.assertEqual(cache[1], {'disk_io': {('disk1', 0): 0, ('disk1', 1): 0, 
                         ('disk1', 2): 100}})
            self.assertEqual(cache[2], {'disk_io': {'disk1': set([('disk1', 2)])}})
...
@unittest.skipIf(TRAVIS and not os.path.exists(SCRIPTS_DIR), "can't locate scripts directory")
class TestScripts(unittest.TestCase):
    __doc__ = 'Tests for scripts in the "scripts" directory.'
    ...
    def test_coverage(self):
        ...
        for name in os.listdir(SCRIPTS_DIR):
            if name.endswith('.py') and 'test_' + os.path.splitext(name)[0] not in meths:
                self.fail('no test defined for %r script' % os.path.join(SCRIPTS_DIR, name))
