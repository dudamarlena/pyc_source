...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/docstringcoverage/cover.py
# Compiled at: 2015-02-27 08:21:41
"""
...
class DocStringCoverageVisitor(compiler.visitor.ASTVisitor):
    ...
    def visitModule(self, module):
        ...
        if not len(list(module.asList()[1])):
            empty = True
        node = (
         module.doc is not None and module.doc.strip() != '', empty, [])
...
def get_docstring_coverage(filenames, count_magic=True, skip_empty_files=True, verbose_level=0):
    ...
    def log(text, level=1, append=False):
        """Prints to log depending on verbosity level"""
    ...
    for filename in filenames:
        ...
        if file_docs_needed:
            ...
            coverage = 0
        result_dict[filename] = {'missing': file_missing_list, 
    ...
    total_result_dict = {'missing_count': total_docs_needed - total_docs_covered, 
    ...
    if not count_magic:
        postfix += ' (all magic methods omitted!)'
    log('\n', 2)
    if len(filenames) > 1:
...
def main():
    ...
    parser.add_option('-v', '--verbose', dest='verbosity', default='3', metavar='LEVEL', help='verbose level <0-3>, default 3', type='choice', choices=['0', '1', '2', '3'])
    parser.add_option('-m', '--nomagic', action='store_false', dest='magic', default=True, help="don't count docstrings for __magic__ methods")
    parser.add_option('-l', '--followlinks', action='store_true', dest='followlinks', default=False, help='follow symlinks')
