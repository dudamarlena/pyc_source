...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\multiprocessing\examples\mp_distributing.py
# Compiled at: 2009-07-30 09:32:52
__all__ = [
 'Cluster', 'Host', 'get_logger', 'current_process']
import sys, os, tarfile, shutil, subprocess, logging, itertools, Queue
try:
...
class HostManager(managers.SyncManager):
    ...
    def from_address(cls, address, authkey):
        ...
        manager.shutdown = util.Finalize(manager, HostManager._finalize_host, args=(manager._address, manager._authkey, manager._name), exitpriority=-10)
        return manager
...
class DistributedPool(pool.Pool):
    ...
    def __init__(self, cluster, processes=None, initializer=None, initargs=()):
        ...
        pool.Pool.__init__(self, processes or len(cluster), initializer, initargs)
    ...
    def start(self):
        ...
        for host in self._hostlist:
            if host.hostname != 'localhost':
                ...
                other_host.manager = HostManager.from_address(address, self._authkey)
...
class Host(object):
    ...
    def _start_manager(self, index, authkey, address, files):
        if self.hostname != 'localhost':
            ...
            p = subprocess.Popen(['ssh', self.hostname, 'python', '-c', '"import os; os.chdir(%r); from distributing import main; main()"' % tempdir], stdin=subprocess.PIPE)
            data = dict(name='BoostrappingHost', index=index, dist_log_level=_logger.getEffectiveLevel(), dir=tempdir, authkey=str(authkey), parent_address=address)
...
def copy_to_remote_temporary_directory(host, files):
    p = subprocess.Popen(['ssh', host, 'python', '-c', unzip_code], stdout=subprocess.PIPE, stdin=subprocess.PIPE)
