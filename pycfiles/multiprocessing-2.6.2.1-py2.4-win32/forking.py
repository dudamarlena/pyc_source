...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\multiprocessing\forking.py
...
def _reduce_method(m):
    if m.im_self is None:
        return (
         getattr, (m.im_class, m.im_func.func_name))
    else:
        return (
...
if sys.platform != 'win32':
    ...
    import thread, msvcrt, _subprocess, time
    try:
    ...
    def is_forking(argv):
        ...
        """
        if len(argv) >= 2:
            assert argv[1] == '--multiprocessing-fork' and len(argv) == 3
            return True
        else:
            return False
    ...
    def get_command_line():
        ...
        if getattr(sys, 'frozen', False):
            return [
             sys.executable, '--multiprocessing-fork']
        else:
    ...
    def reduce_connection(conn):
        ...
        return (
         type(conn), (Popen.duplicate_for_child(conn.fileno()), conn.readable, conn.writable))
