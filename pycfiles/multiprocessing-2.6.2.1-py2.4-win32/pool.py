...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\multiprocessing\pool.py
# Compiled at: 2009-07-30 09:32:54
__all__ = [
 'Pool']
import threading, Queue, itertools, collections, time
...
class Pool(object):
    ...
    def __init__(self, processes=None, initializer=None, initargs=()):
        ...
        for i in range(processes):
            w = self.Process(target=worker, args=(self._inqueue, self._outqueue, initializer, initargs))
            self._pool.append(w)
        ...
        self._terminate = Finalize(self, self._terminate_pool, args=(self._taskqueue, self._inqueue, self._outqueue, self._pool, self._task_handler, self._result_handler, self._cache), exitpriority=15)
        return
    ...
    @classmethod
    def _terminate_pool(cls, taskqueue, inqueue, outqueue, pool, task_handler, result_handler, cache):
