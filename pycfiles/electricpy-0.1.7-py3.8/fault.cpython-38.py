...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\electricpy\fault.py
# Compiled at: 2019-12-18 21:14:25
# Size of source mod 2**32: 68941 bytes
"""
---------------------
`electricpy.fault.py`
---------------------
...
def indmacheigenvalues(Lr, Ls, Lm, Rr, Rs, wrf=0, freq=60):
    ...
    k1 += complex(0.0, 1.0) * (omega_r / 2 - omega_rf)
    k2 = 1 / (2 * phi * torque_r)
    k2 *= _np.sqrt((1 + alpha) ** 2 - 4 * phi * alpha - (omega_r * phi * torque_r) ** 2 + complex(0.0, 2.0) * (alpha - 1) * omega_r * phi * torque_r)
    lam1 = k1 + k2
    lam2 = k1 - k2
    return (lam1, lam2)
...
def indmachphs3sc(t, Is0, Lr, Ls, Lm, Rr, Rs, wrf=0, freq=60, real=True):
    ...
    pIs0 = -(1 / (phi * torque_s) + complex(0.0, 1.0) * (1 - phi) / phi * omega_r) * Is0
...
def synmach_ifault(t, Ea, alpha, Xd, Xdp, Xdpp, Xqpp, Tdp, Tdpp, Ta, freq=60):
    ...
    if Xqpp != 0:
        ...
        val = 0
    asym = 0.5 * (1 / Xdpp + val) * _np.exp(t / Ta)
    isym = const * (1 / Xd + (1 / Xdp - 1 / Xd) * _np.exp(-t / Tdp) + (1 / Xdpp - 1 / Xdp) * _np.exp(-t / Tdpp)) * _np.sin(we * t + alpha)
