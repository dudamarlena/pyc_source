...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/rest_framework_extras/tests/test_views.py
...
def get_control(model='vanilla', pk=1, another_editable_field_suffix=''):
    return {'url': 'http://testserver/tests-%s/%s/' % (model, pk), 
       'editable_field': 'editable_field', 
       'another_editable_field': 'another_editable_field%s' % another_editable_field_suffix, 
       'many_field': [
                    'http://testserver/tests-bar/1/'], 
...
class ViewsTestCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.vanilla = models.Vanilla.objects.create(editable_field='editable_field', another_editable_field='another_editable_field', foreign_field=cls.foo)
        cls.vanilla.save()
        cls.vanilla.many_field.set([cls.bar])
        cls.with_form = models.WithForm.objects.create(editable_field='editable_field', another_editable_field='another_editable_field', foreign_field=cls.foo)
        cls.with_form.save()
        cls.with_form.many_field.set([cls.bar])
        cls.with_tricky_form = models.WithTrickyForm.objects.create(editable_field='editable_field', another_editable_field='another_editable_field', foreign_field=cls.foo)
    ...
    def test_vanilla_create(self):
        ...
        data = {'editable_field': 'editable_field', 
    ...
    def test_vanilla_patch(self):
        data = {'editable_field': 'editable_field_x'}
        response = self.client.patch('/tests-vanilla/%s/' % self.vanilla.pk, data)
        as_json = response.json()
        self.assertEqual(as_json['editable_field'], 'editable_field_x')
        self.assertEqual(models.Vanilla.objects.get(pk=self.vanilla.pk).editable_field, 'editable_field_x')
    ...
    def test_with_form_patch(self):
        data = {'editable_field': 'editable_field_x'}
        response = self.client.patch('/tests-withform/%s/' % self.vanilla.pk, data)
        as_json = response.json()
        self.assertEqual(as_json['editable_field'], 'editable_field_x')
        self.assertEqual(models.WithForm.objects.get(pk=self.with_form.pk).editable_field, 'editable_field_x')
    ...
    def test_with_tricky_form_create(self):
        ...
        as_json = response.json()
        self.assertEqual(as_json, {'an_integer': ['This field is required.']})