...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/circonus/collectd/df.py
...
def is_mount_dir(metric_name, mount_dir):
    ...
    if len(split_metric_name) > 1:
        return unicode(split_metric_name[1]).translate(PUNCTUATION_TABLE) == unicode(mount_dir).translate(PUNCTUATION_TABLE)
    return False
