...
# [GCC 8.4.0]
# Embedded file name: /Users/clareau/dat/Research/BuenrostroResearch/lareau_dev/bap/bap/cli_bap_bulk_frag.py
# Compiled at: 2019-06-21 12:11:15
...
def main(input, output, name, ncores, reference_genome, cluster, jobs, keep_temp_files, mapq, bedtools_genome, blacklist_file, tss_file, mito_chromosome, r_path, bedtools_path, samtools_path, bgzip_path, tabix_path):
    """
    ...
    if njobs > 0:
        ...
        if reference_genome in ('hg19-mm10', 'hg19_mm10_c'):
            ...
            speciesMix = False
    p = bapBulkFragProject(script_dir, supported_genomes, input, output, name, ncores, reference_genome, cluster, jobs, keep_temp_files, mapq, bedtools_genome, blacklist_file, tss_file, mito_chromosome, r_path, bedtools_path, samtools_path, bgzip_path, tabix_path)
    ...
    line1 = 'python ' + script_dir + '/bin/python/20a_nonames_split_filt.py --input ' + p.bamfile
    line2 = ' --name ' + p.name + ' --output ' + temp_filt_split
    line3 = ' --bedtools-reference-genome ' + p.bedtoolsGenomeFile
    ...
    snakecmd_chr = 'snakemake' + snakeclust + ' --snakefile ' + script_dir + '/bin/snake/Snakefile.bap_bulk_frags --cores ' + ncores + ' --config cfp="' + y_s + '" --stats ' + snake_stats + ' &>' + snake_log
