...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-intel/egg/tests/test_initiator.py
# Compiled at: 2017-06-02 11:17:28
import os, os.path, unittest, shutil
...
class TestInitiator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config_file = os.path.join(BASE_DIR, 'simiki', 'conf_templates', '_config.yml.in')
        self.target_path = os.path.join(BASE_DIR, 'tests', '_build')
        if os.path.exists(self.target_path):
            shutil.rmtree(self.target_path)
        self.files = ['_config.yml',
         'fabfile.py',
         os.path.join(self.default_config['source'], 'intro', 'gettingstarted.md')]
        self.dirs = [
         self.default_config['source'],
         self.default_config['destination'],
         self.default_config['themes_dir'],
         os.path.join(self.default_config['themes_dir'], self.default_config['theme'])]
