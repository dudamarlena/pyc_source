...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-intel/egg/simiki/conf_templates/fabfile.py
...
if rsync_configs:
    ...
    env.hosts = [
     rsync_configs['host']]
    env.local_output = os.path.join(os.path.abspath(os.path.dirname(__file__)), configs['destination'])
...
def deploy_rsync(deploy_configs):
    """for rsync"""
    project.rsync_project(local_dir=env.local_output.rstrip('/') + '/', remote_dir=env.remote_output.rstrip('/') + '/', delete=env.rsync_delete)
...
def deploy_git(deploy_configs):
    ...
    with settings(warn_only=True):
        ...
        if int(res.strip()):
            do_exit('Warning: ghp-import not installed! run: `pip install ghp-import`')
    ...
    local(('ghp-import -p -m "{0}" -r {1} -b {2} {3}').format(_mesg, remote, branch, output_dir))
    ...
    with settings(warn_only=True):
        ...
        if int(res.strip()):
            if False:
                ...
                try:
                    _ans = raw_input(('\n{0}\nAdd these files to index? (y/N) ').format(test_res.strip()))
                ...
                local(('git add {0}').format(add_file))
        res = local('git diff --cached --quiet --exit-code; echo $?', capture=True)
        if int(res.strip()):
            test_res = local('git diff --cached --name-status', capture=True)
            try:
                _ans = raw_input(('\n{0}\nCommit these files? (y/N) ').format(test_res.strip()))
                if _ans.lower() in yes_ans:
