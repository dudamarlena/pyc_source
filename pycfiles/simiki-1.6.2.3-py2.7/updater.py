...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-intel/egg/simiki/updater.py
    ...
    try:
        if os.path.exists(local_dir):
            ...
            if _need_update:
                up_to_date = False
                try:
                    _ans = get_input(('Overwrite {0} {1}? (y/N) ').format(tag, dirname))
                    if _ans.lower() in yes_answer:
...
def update_builtin(**kwargs):
    ...
    _update_file('fabfile.py', os.getcwd(), os.path.join(os.path.dirname(__file__), 'conf_templates'))
    if os.path.exists(os.path.join(os.getcwd(), 'Dockerfile')):
        _update_file('Dockerfile', os.getcwd(), os.path.join(os.path.dirname(__file__), 'conf_templates'))
