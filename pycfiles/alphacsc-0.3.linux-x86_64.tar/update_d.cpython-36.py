...
# [GCC 8.4.0]
# Embedded file name: /home/tom/.local/miniconda/lib/python3.6/site-packages/alphacsc/update_d.py
# Compiled at: 2019-06-04 04:10:26
# Size of source mod 2**32: 9907 bytes
import numpy as np
from scipy import linalg, optimize
from .utils import construct_X, check_consistent_shape

def update_d(X, Z, n_times_atom, lambd0=None, ds_init=None, debug=False, solver_kwargs=dict(), sample_weights=None, verbose=0):
    ...
    factr = solver_kwargs.get('factr', 10000000.0)
    d_hat, lambd_hats = solve_unit_norm_dual(lhs, rhs, lambd0=lambd0, factr=factr,
      debug=debug,
...
def update_d_block(X, Z, n_times_atom, lambd0=None, ds_init=None, projection='dual', n_iter=1, solver_kwargs=dict(), sample_weights=None, verbose=0):
    ...
    for _ in range(n_iter):
        for k in range(n_atoms):
            ...
            if projection == 'primal':
                factr = solver_kwargs.get('factr', 100000000000.0)
                d_hat = solve_unit_norm_primal(lhs, rhs, d_hat0=(ds[k][::-1]), factr=factr,
                  verbose=verbose)
            else:
                if projection == 'dual':
                    factr = solver_kwargs.get('factr', 10000000.0)
                    d_hat, lambd_hat = solve_unit_norm_dual(lhs,
                      rhs, factr=factr, lambd0=(np.array([lambd0[k]])), lhs_is_toeplitz=(sample_weights is None))
...
def solve_unit_norm_dual(lhs, rhs, lambd0, factr=10000000.0, debug=False, lhs_is_toeplitz=False):
    if np.all(rhs == 0):
        return (np.zeros(lhs.shape[0]), 0.0)
    else:
        ...
        if lhs_is_toeplitz:
            ...
            def x_star(lambd):
                lambd += 1e-14
                return linalg.solve(lhs + np.diag(np.repeat(lambd, n_times_atom)), rhs)

        def dual(lambd):
            x_hats = x_star(lambd)
            norms = linalg.norm((x_hats.reshape(-1, n_times_atom)), axis=1)
            return x_hats.T.dot(lhs).dot(x_hats) - 2 * rhs.T.dot(x_hats) + np.dot(lambd, norms ** 2 - 1.0)

        def grad_dual(lambd):
            x_hats = x_star(lambd).reshape(-1, n_times_atom)
            return linalg.norm(x_hats, axis=1) ** 2 - 1.0
        ...
        bounds = [(0.0, None) for idx in range(0, n_atoms)]
        if debug:
            if not optimize.check_grad(func, grad, lambd0) < 1e-05:
                raise AssertionError
        lambd_hats, _, _ = optimize.fmin_l_bfgs_b(func, x0=lambd0, fprime=grad, bounds=bounds,
          factr=factr)
        x_hat = x_star(lambd_hats)
        return (x_hat, lambd_hats)


def solve_unit_norm_primal(lhs, rhs, d_hat0, step_size=0.1, max_iter=1000, factr=100000000000.0, verbose=0):
    ...
    def project(d_hat):
        d_hat /= max(1.0, linalg.norm(d_hat))
        return d_hat
