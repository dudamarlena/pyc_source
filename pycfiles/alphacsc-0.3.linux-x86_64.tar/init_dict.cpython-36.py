...
# [GCC 8.4.0]
# Embedded file name: /home/tom/.local/miniconda/lib/python3.6/site-packages/alphacsc/init_dict.py
# Compiled at: 2019-06-05 05:26:13
# Size of source mod 2**32: 13111 bytes
...
def init_dictionary(X, n_atoms, n_times_atom, uv_constraint='separate', rank1=True, window=False, D_init=None, D_init_params=dict(), random_state=None):
    ...
    if isinstance(D_init, np.ndarray):
        ...
        if D_init is None or D_init == 'random':
            ...
            if D_init == 'kmeans':
                D_hat = kmeans_init(X, n_atoms, n_times_atom, random_state=rng, **D_init_params)
                if not rank1:
                ...
                if D_init == 'ssa':
                    ...
                    if D_init == 'greedy':
                        ...
                        raise NotImplementedError('It is not possible to initialize uv with parameter {}.'.format(D_init))
            if window:
...
def kmeans_init(X, n_atoms, n_times_atom, max_iter=0, random_state=None, non_uniform=True, distances='euclidean', tsne=False):
    ...
    if distances != 'euclidean':
        ...
        X_embed = np.concatenate([_embed(Xi, n_times_atom).T[::step, :] for Xi in X])
        ...
        try:
            ...
            seeding, indices = kmc2.kmc2(X_embed, k=n_atoms, weights=weights, random_state=rng,
              distances=distances)
        except ImportError:
            if max_iter == 0:
                raise ImportError("Could not import alphacsc.other.kmc2. This breaks the logic for the D_init='kmeans'. It should not be used with max_iter=0 in D_init_params.")
        ...
        if max_iter == 0:
            ...
            if distances != 'euclidean':
                ...
                try:
                    ...
                    raise ImportError('Could not import multics.other.k_medoid, make sure to compile it to be able to initialize the dictionary with k-means and a non-euclidean distance.')

                model = KMedoids(n_clusters=n_atoms, init=(np.int_(indices)), max_iter=max_iter,
                ...
                distance_metric = 'euclidean'
                model = MiniBatchKMeans(n_clusters=n_atoms, init=seeding, n_init=1, max_iter=max_iter,
                  random_state=rng).fit(X_embed)
    ...
    if tsne:
        if distances == 'euclidean':
            ...
            if labels is not None:
                labels = labels[::100]
        plot_tsne(X_embed, v_init, labels=labels, metric=distance_metric, random_state=rng)
    if not (distances == 'euclidean' and max_iter > 0):
        ...
        D = np.array([X_original[i, :, t:t + n_times_atom] for i, t in zip(medoid_i, medoid_t)])
        D_init = get_uv(D)
    return D_init


def plot_tsne(X_embed, X_centers, labels=None, metric='euclidean', random_state=None):
    tsne = TSNE(n_components=2, random_state=random_state, perplexity=5, metric=metric,
      verbose=2)
    ...
    if labels is not None:
        ...
        colors = [c for c, l in zip(itertools.cycle(COLORS), np.unique(labels))]
        colors = np.array(colors)[labels]
    else:
        colors = None
    plt.figure('tsne')
    cc = colors[:-n_centers] if colors is not None else None
    plt.scatter((X_tsne[:-n_centers, 0]), (X_tsne[:-n_centers, 1]), c=cc, marker='.')
    cc = colors[-n_centers:] if colors is not None else None
    plt.scatter((X_tsne[-n_centers:, 0]), (X_tsne[-n_centers:, 1]), c=cc, marker='*',
      s=4)
...
def ssa_init(X, n_atoms, n_times_atom, random_state=None):
    ...
    step = 1
    X_embed = np.concatenate([_embed(Xi, n_times_atom).T[::step, :] for Xi in X_strong])
    X_embed = np.atleast_2d(X_embed)
