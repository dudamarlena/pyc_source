...
# [GCC 8.4.0]
# Embedded file name: /home/tom/.local/miniconda/lib/python3.6/site-packages/alphacsc/utils/compute_constants.py
    ...
    for k0 in range(n_atoms):
        for k in range(n_atoms):
            for t in range(n_times_atom):
                if t == 0:
                    DtD[(k0, k, t0)] = np.dot(uv[k0, n_channels:], uv[k, n_channels:])
                else:
                    DtD[(k0, k, t0 + t)] = np.dot(uv[k0, n_channels:-t], uv[k, n_channels + t:])
                    DtD[(k0, k, t0 - t)] = np.dot(uv[k0, n_channels + t:], uv[k, n_channels:-t])
        ...
        for k in range(n_atoms):
            for t in range(n_times_atom):
                if t == 0:
                    ...
                    DtD[(k0, k, t0 + t)] = np.dot(D[k0, :, :-t].ravel(), D[k, :, t:].ravel())
                    DtD[(k0, k, t0 - t)] = np.dot(D[k0, :, t:].ravel(), D[k, :, :-t].ravel())
            ...
            for k in range(n_atoms):
                for t in range(n_times_atom):
                    if t == 0:
                        ...
                        ztz[(k0, k, t0 + t)] += (z[i, k0, :-t] * z[i, k, t:]).sum()
                        ztz[(k0, k, t0 - t)] += (z[i, k0, t:] * z[i, k, :-t]).sum()
