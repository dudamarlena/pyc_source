...
# [GCC 8.4.0]
# Embedded file name: /home/tom/.local/miniconda/lib/python3.6/site-packages/alphacsc/viz/plot_output.py
# Compiled at: 2019-06-04 04:10:26
# Size of source mod 2**32: 8264 bytes
...
def _plot_atom(Dk, info, ax, color, plot='atom'):
    ...
    if plot == 'atom':
        ...
        if plot == 'topo':
            mne.viz.plot_topomap((Dk[:info['n_channels']]), info, axes=ax, show=False)
        else:
            if plot == 'psd':
...
def _plot_activation(z_k, info, ax, color, n_times_atom, t_min=0, plot='density'):
    sfreq = int(info.get('sfreq', 1))
    z_k = z_k.mean(axis=0)
    t = np.arange(len(z_k)) / sfreq + t_min
    if plot == 'density':
        blob = np.blackman(n_times_atom)
        z_k_smooth = np.convolve(z_k, blob, mode='same')
        ax.fill_between(t, z_k_smooth, color=color, alpha=0.5)
    else:
        if plot == 'logratio':
            eps = 0.0001
            baseline = z_k[:sfreq]
            ...
            if plot == 'whiskers':
                ...
                evoked = z_k[((0 < t) * (t < 1))]
                induced = z_k[((1 < t) * (t < 2))]
                ax.boxplot([baseline, evoked, induced], sym='', labels=[
                 'baseline', 'evoked', 'induced'])
            else:
...
def _create_fig(n_cols, n_rows, figname, wrap_col=5, w_plot=5, h_plot=3):
    ...
    fig, axes = plt.subplots(n_rows, n_cols, squeeze=False, sharey=True, sharex=True,
      figsize=figsize,
      num=figname)
    if wrap_subplots:
        axes = [
...
def plot_dictionary(data, info, dirname, name='D_hat', plot='atom'):
    if plot == 'topo':
        if not isinstance(info, mne.Info):
            warnings.warn('topomaps should only be used when `info` parameter is a subclass of `mne.Info`.', UserWarning)
            return
    ...
    for row_ax, (arg, res) in zip(axes, data):
        ...
        row_ax[0].set_ylabel((get_label(info['grid_key'], arg)), fontsize=24,
          labelpad=10,
          rotation='horizontal',
...
def plot_activation(data, info, dirname, name, plot='density'):
    ...
    for row_ax, (arg, res) in zip(axes, data):
        ...
        for color, ax, z_k in zip(color_cycle, row_ax, z.swapaxes(0, 1)):
            _plot_activation(z_k, info, ax, color, n_times_atom=(arg['n_times_atom']),
              t_min=t_min,
              plot=plot)
...
def plot_convergence_curve(data, info, dirname):
    ...
    for (args, res), color in zip(data, color_cycle):
        ...
        plt.loglog(times,
          ((res['pobj'] - best_pobj) / best_pobj + eps), '.-', label=(get_label(info['grid_key'], args)),
          color=color,
          linewidth=2)
...
PLOTS = {'convergence':plot_convergence_curve, 
 ...
 'z_init_whiskers':partial(plot_activation, name='z_init', plot='whiskers'), 
 'z_hat_logratio':partial(plot_activation, name='z_hat', plot='logratio'), 
 'z_init_logratio':partial(plot_activation, name='z_init', plot='logratio'), 
