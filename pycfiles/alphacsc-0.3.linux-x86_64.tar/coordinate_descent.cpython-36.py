...
# [GCC 8.4.0]
# Embedded file name: /home/tom/.local/miniconda/lib/python3.6/site-packages/alphacsc/utils/coordinate_descent.py
# Compiled at: 2019-06-05 05:26:19
# Size of source mod 2**32: 10052 bytes
...
def _coordinate_descent_idx(Xi, D, constants, reg, z0=None, max_iter=1000, tol=0.1, strategy='greedy', n_seg='auto', freeze_support=False, debug=False, timing=False, random_state=None, name='CD', verbose=0):
    ...
    if timing:
        ...
        if timing:
            ...
            t_start = time.time()
        beta, dz_opt, tol = _init_beta(Xi, z_hat, D, constants, reg, norm_Dk, tol,
          use_sparse_dz=False)
        if freeze_support:
            if is_lil(z0):
                mask = z0 != 0
    ...
    for ii in range(int(max_iter)):
        k0, t0, dz = _select_coordinate(strategy, dz_opt, (active_segs[i_seg]), n_atoms,
          n_times_valid, n_times_seg, seg_bounds,
          rng=rng)
        ...
        if abs(dz) > tol:
            z_hat[(k0, t0)] += dz
            beta, dz_opt, accumulator, active_segs = _update_beta(beta, dz_opt, accumulator, active_segs, z_hat, DtD, norm_Dk, dz, k0, t0, reg, tol, seg_bounds, i_seg, n_times_atom, z0, freeze_support, debug)
        else:
            if active_segs[i_seg]:
                accumulator -= 1
                active_segs[i_seg] = False
        if timing:
            if ii % max(100, n_seg // 100) == 0:
...
def _init_beta(Xi, z_hat, D, constants, reg, norm_Dk, tol, use_sparse_dz=False):
    beta = gradient_zi(Xi, z_hat, D=D, reg=None, loss='l2', return_func=False,
      constants=constants)
    if is_lil(z_hat):
    ...
    if is_lil(z_hat):
        ...
        cython_code._assert_cython()
        cython_code.update_dz_opt(z_hat, beta, dz_opt, (norm_Dk[:, 0]), reg, t_start=0,
          t_end=n_times_valid)
    else:
...
def _update_beta(beta, dz_opt, accumulator, active_segs, z_hat, DtD, norm_Dk, dz, k0, t0, reg, tol, seg_bounds, i_seg, n_times_atom, z0, freeze_support, debug):
    ...
    if is_lil(z_hat):
        cython_code._assert_cython()
        cython_code.update_dz_opt(z_hat, beta, dz_opt, norm_Dk[:, 0], reg, t_start_up, t_end_up)
    else:
    ...
    if t_start_up < t_start_seg:
        if not active_segs[(i_seg - 1)]:
            accumulator += 1
            active_segs[i_seg - 1] = True
    if t_end_up > t_end_seg:
        if not active_segs[(i_seg + 1)]:
            accumulator += 1
            active_segs[i_seg + 1] = True
    if freeze_support:
    ...
    return (
     beta, dz_opt, accumulator, active_segs)


def _select_coordinate(strategy, dz_opt, active_seg, n_atoms, n_times_valid, n_times_seg, seg_bounds, rng):
    if strategy == 'random':
        ...
        if strategy == 'cyclic':
            ...
            if strategy == 'greedy':
                ...
                raise ValueError("'The coordinate selection method should be in {'greedy' | 'random' | 'cyclic'}. Got '%s'." % (
                 strategy,))
    return (
     k0, t0, dz)