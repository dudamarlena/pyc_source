...
# [GCC 8.4.0]
# Embedded file name: /home/tom/.local/miniconda/lib/python3.6/site-packages/alphacsc/learn_d_z.py
...
def compute_X_and_objective(X, z_hat, d_hat, reg, sample_weights=None, feasible_evaluation=True):
...
def learn_d_z(X, n_atoms, n_times_atom, func_d=update_d_block, reg=0.1, lmbd_max='fixed', n_iter=60, random_state=None, n_jobs=1, solver_z='l-bfgs', solver_d_kwargs=dict(), solver_z_kwargs=dict(), ds_init=None, ds_init_params=dict(), sample_weights=None, verbose=10, callback=None, stopping_pobj=None):
    ...
    if isinstance(ds_init, np.ndarray):
        ...
        d_hat = init_dictionary((X[:, None, :]), n_atoms, n_times_atom, D_init=ds_init,
          rank1=False,
          D_init_params=ds_init_params,
          random_state=rng)
        d_hat = d_hat[:, 0, :]
        reg0 = reg
        ...
        for ii in range(n_iter):
            ...
            if lmbd_max not in ('fixed', 'scaled'):
                ...
                if lmbd_max == 'shared':
                    reg = reg.max()
                start = time.time()
                z_hat = update_z(X, d_hat, reg, z0=z_hat, parallel=parallel, solver=solver_z,
                  b_hat_0=b_hat_0,
                ...
                if len(z_hat.nonzero()[0]) == 0:
                    ...
                    break
                start = time.time()
                d_hat, lambd0 = func_d(X, z_hat, n_times_atom, lambd0=lambd0, ds_init=d_hat,
                  verbose=verbose,
