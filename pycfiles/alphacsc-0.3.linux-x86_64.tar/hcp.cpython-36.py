...
# [GCC 8.4.0]
# Embedded file name: /home/tom/.local/miniconda/lib/python3.6/site-packages/alphacsc/datasets/hcp.py
# Compiled at: 2019-06-04 04:10:26
# Size of source mod 2**32: 7943 bytes
import re, os, hcp, mne, numpy as np
...
def load_one_record(data_type, subject, run_index, sfreq=300, epoch=None, filter_params=[
 5.0, None], n_jobs=1):
    print(f"subject={subject}, data_type={data_type}, run_index={run_index}, hcp_path={HCP_DIR}")
    raw = hcp.read_raw(subject, data_type=data_type, run_index=run_index, hcp_path=HCP_DIR,
      verbose=0)
    raw.load_data()
...
def load_data(n_trials=10, data_type='rest', sfreq=150, epoch=None, filter_params=[
 5.0, None], equalize='zeropad', n_jobs=1, random_state=None):
    ...
    db = get_all_records()
    records = [(subject, run_index) for subject, runs in db[data_type].items() for run_index in runs]
    X, info = [], []
    records = rng.permutation(records)[:n_trials]
    for i, (subject, run_index) in enumerate(records):
        print(('\rLoading HCP subjects: {:7.2%}'.format(i / n_trials)), end='',
          flush=True)
        X_n, info_n = load_one_record(data_type,
          subject, (int(run_index)), sfreq=sfreq, epoch=epoch, filter_params=filter_params,
          n_jobs=n_jobs)
        X += [X_n]
        info += [info_n]
...
def data_generator(n_trials=10, data_type='rest', sfreq=150, epoch=None, filter_params=[
 5.0, None], equalize='zeropad', n_jobs=1, random_state=None):
...
def make_array(X, equalize='zeropad'):
    ...
    if not np.all(x_shape[..., :-1] == x_shape[0, ..., :-1]):
        ...
        if equalize == 'crop':
            ...
            if equalize == 'zeropad':
                ...
                X = np.array([np.concatenate([x, np.zeros(X_shape + (L - x.shape[(-1)],))], axis=(-1)) for x in X])
            else:
                raise ValueError("The equalize '{}' is not valid. It should be in {'crop', 'zeropad'}".format(equalize))
    return X