...
# [GCC 8.4.0]
# Embedded file name: /home/tom/.local/miniconda/lib/python3.6/site-packages/alphacsc/update_d_multi.py
# Compiled at: 2019-06-05 05:26:19
# Size of source mod 2**32: 14716 bytes
import numpy as np
...
def update_uv(X, z, uv_hat0, constants=None, b_hat_0=None, debug=False, max_iter=300, eps=None, solver_d='alternate', momentum=False, uv_constraint='separate', loss='l2', loss_params=dict(), verbose=0, window=False):
    ...
    if window:
        ...
        def objective(uv):
            ...
            if loss == 'l2':
                ...
                return compute_X_and_objective_multi(X, z, D_hat=uv, loss=loss, loss_params=loss_params,
        ...
        if solver_d in ('joint', 'fista'):
            ...
            def grad(uv):
                if window:
                    uv = uv.copy()
                    uv[:, n_channels:] *= tukey_window_
                grad = gradient_uv(uv=uv, X=X, z=z, constants=constants, loss=loss, loss_params=loss_params)
                if window:
            ...
            def prox(uv, step_size=None):
                if window:
                    uv[:, n_channels:] *= tukey_window_
                uv = prox_uv(uv, uv_constraint=uv_constraint, n_channels=n_channels)
                if window:
                    uv[:, n_channels:] /= tukey_window_
                return uv

            uv_hat, pobj = fista(objective, grad, prox, None, uv_hat0, max_iter, verbose=verbose,
              momentum=momentum,
              eps=eps,
            ...
            if solver_d in ('alternate', 'alternate_adaptive'):
                ...
                def prox_u(u, step_size=None):
                    u /= np.maximum(1.0, np.linalg.norm(u, axis=1, keepdims=True))
                    return u

                def prox_v(v, step_size=None):
                    if window:
                        v *= tukey_window_
                    v /= np.maximum(1.0, np.linalg.norm(v, axis=1, keepdims=True))
                    if window:
                        v /= tukey_window_
                    return v
                ...
                for jj in range(1):
                    ...
                    def grad_u(u):
                        if window:
                            ...
                            uv = np.c_[(u, v_hat)]
                        grad_d = gradient_d(uv, X=X, z=z, constants=constants, loss=loss,
                    ...
                    assert Lu > 0
                    u_hat, pobj_u = fista(obj, grad_u, prox_u, (0.99 / Lu), u_hat, max_iter,
                      momentum=momentum, eps=eps, adaptive_step_size=adaptive_step_size,
                      verbose=verbose,
                      debug=debug,
                      name='Update u')
                    ...
                    assert Lv > 0
                    v_hat, pobj_v = fista(obj, grad_v, prox_v, (0.99 / Lv), v_hat, max_iter,
                      momentum=momentum, eps=eps, adaptive_step_size=adaptive_step_size,
                      verbose=verbose,
                      debug=debug,
                      name='Update v')
...
def update_d(X, z, D_hat0, constants=None, b_hat_0=None, debug=False, max_iter=300, eps=None, solver_d='fista', momentum=False, uv_constraint='joint', loss='l2', loss_params=dict(), verbose=0, window=False):
    """Learn d's in time domain.
    ...
    def objective(D, full=False):
        ...
        if loss == 'l2':
            ...
            return compute_X_and_objective_multi(X, z, D_hat=D, loss=loss, loss_params=loss_params)

    def grad(D):
        if window:
            D = D * tukey_window_
        grad = gradient_d(D=D, X=X, z=z, constants=constants, loss=loss, loss_params=loss_params)
        if window:
            grad *= tukey_window_
        return grad
    ...
    D_hat, pobj = fista(objective, grad, prox, None, D_hat0, max_iter, verbose=verbose,
      momentum=momentum,
      eps=eps,
      adaptive_step_size=True,
      debug=debug,
...
def compute_lipschitz(uv0, constants, variable, b_hat_0=None, random_state=None):
