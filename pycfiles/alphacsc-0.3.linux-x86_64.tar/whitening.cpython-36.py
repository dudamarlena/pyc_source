...
# [GCC 8.4.0]
# Embedded file name: /home/tom/.local/miniconda/lib/python3.6/site-packages/alphacsc/utils/whitening.py
...
def whitening(X, ordar=10, block_length=256, sfreq=1.0, zero_phase=True, plot=False, use_fooof=False):
    ...
    X_white *= signal.tukey(n_times_white, alpha=(3 / float(n_times_white)))[None, None, :]
    if plot:
        ...
        ar_model.periodogram((X_white.reshape(-1, n_times)),
          hold=True, mean_psd=True)
        labels = ['signal', 'model AR', 'signal white']
        if use_fooof:
            labels = [
             'signal', 'FOOOF fit', 'model AR', 'signal white']
        ar_model.plot('periodogram before/after whitening', labels=labels,
          fscale='lin')
...
def apply_whitening(ar_model, X, zero_phase=True, mode='same', reverse_ar=False, n_channels=None):
    if X.ndim == 2:
        msg = 'For rank1 D, n_channels should be provided'
        assert n_channels is not None, msg
        v = X[:, n_channels:]
        v_white = [apply_ar(ar_model, vk, zero_phase=zero_phase, mode=mode) for vk in v]
        return np.c_[(X[:, :n_channels], v_white)]
    if X.ndim == 3:
        return np.array([[apply_ar(ar_model, Xij, zero_phase=zero_phase, mode=mode, reverse_ar=reverse_ar) for Xij in Xi] for Xi in X])
...
def unwhitening(ar_model, X_white, estimate=True, zero_phase=True, plot=False):
    if X_white.ndim == 2:
        ...
        if estimate:
            ar_model.arma2psd(hold=True)
            ar_model.psd[-1] = 1.0 / ar_model.psd[(-1)]
            ar_model.estimate()
        X_unwhite = apply_whitening(ar_model, X_white, zero_phase=zero_phase)
        assert X_unwhite.shape == X_white.shape
        if plot:
            ar_model.periodogram((X_white.reshape(-1, n_times)),
              hold=False, mean_psd=True)
            ar_model.arma2psd(hold=True)
            if zero_phase:
                ar_model.psd[(-1)] **= 2
            ar_model.periodogram((X_unwhite.reshape(-1, n_times)),
              hold=True, mean_psd=True)
            ar_model.plot('periodogram before/after unwhitening', labels=[
             'signal white', 'model AR',
             'signal unwhite'],
              fscale='lin')
            plt.legend(loc='lower left')
    return X_unwhite