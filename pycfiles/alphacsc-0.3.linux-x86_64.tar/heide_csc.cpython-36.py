...
# [GCC 8.4.0]
# Embedded file name: /home/tom/.local/miniconda/lib/python3.6/site-packages/alphacsc/other/heide_csc.py
# Compiled at: 2019-06-04 04:10:26
# Size of source mod 2**32: 16161 bytes
"""
...
def update_d(z_hat, d_hat, size_z, size_x, rho, d, v_D, d_D, lambdas, gammas_D, Mtb, u_D, M, size_k_full, psf_radius, xi_D, xi_D_hat, verbose, max_it_d):
    """D-STEP"""
    zhat_mat, zhat_inv_mat = precompute_D_step(z_hat, size_z, rho, verbose)
    for i_d in range(max_it_d):
        v_D[0] = np.real(ifft(np.einsum('ijk,jk->ik', z_hat, d_hat)))
        v_D[1] = d
        u = v_D[0] - d_D[0]
...
def update_z(z, z_hat, d_hat, u_Z, v_Z, d_Z, lambdas, gammas_Z, Mtb, M, size_x, size_z, xi_Z, xi_z_hat, b, lambda_prior, lambda_residual, psf_radius, verbose, max_it_z):
    """Z-STEP"""
    ...
    for i_z in range(max_it_z):
        v_Z[0] = np.real(ifft(np.einsum('ijk,jk->ik', z_hat, d_hat)))
        v_Z[1] = z
        u = v_Z[0] - d_Z[0]
...
def learn_conv_sparse_coder(b, size_kernel, max_it, tol, lambda_prior=1.0, lambda_residual=1.0, random_state=None, ds_init=None, feasible_evaluation=True, stopping_pobj=None, verbose=1, max_it_d=10, max_it_z=10):
    """
    ...
    M = np.pad(np.ones((b.shape), dtype=real_type), (
     (0, 0), (psf_radius, psf_radius)),
      mode='constant',
      constant_values=0)
    Mtb = np.pad(b, ((0, 0), (psf_radius, psf_radius)), mode='constant',
      constant_values=0)
    lambdas = [
    ...
    z = np.zeros(size_z)
    z_hat = fft(z)
    obj_val = obj_func_2(z, d, b, lambda_prior, psf_radius, feasible_evaluation)
    if verbose > 0:
    ...
    for i in range(max_it):
        start = time.time()
        z, z_hat = update_z(z, z_hat, d_hat, u_Z, v_Z, d_Z, lambdas, gammas_Z, Mtb, M, size_x, size_z, xi_Z, xi_z_hat, b, lambda_prior, lambda_residual, psf_radius, verbose, max_it_z)
        times.append(time.time() - start)
        obj_val = obj_func_2(z, d, b, lambda_prior, psf_radius, feasible_evaluation)
        if verbose > 0:
            print('Iter Z %d/%d, Obj %3.3f' % (i, max_it, obj_val))
        start = time.time()
        d, d_hat = update_d(z_hat, d_hat, size_z, size_x, rho, d, v_D, d_D, lambdas, gammas_D, Mtb, u_D, M, size_k_full, psf_radius, xi_D, xi_D_hat, verbose, max_it_d)
        times.append(time.time() - start)
        obj_val = obj_func_2(z, d, b, lambda_prior, psf_radius, feasible_evaluation)
        if verbose > 0:
    ...
    return (
     d_res, z_res, Dz, np.array(list_obj_val), times)
...
def precompute_D_step(z_hat, size_z, rho, verbose):
    ...
    zhat_inv_mat = 1.0 / rho * (np.eye(k) - np.einsum('knm,kmj->knj', np.einsum('knm,kmj->knj', z_hat_mat_t, inv_rho_z_hat_z_hat_t), zhat_mat))
    return (
     zhat_mat, zhat_inv_mat)
...
def solve_conv_term_D(zhat_mat, zhat_inv_mat, xi_hat, rho, size_z):
    ...
    x = np.einsum('ijk, ikl -> ijl', zhat_inv_mat, np.einsum('ijk, ikl -> ijl', z_hat_mat_t, xi_hat_0_flat) + rho * xi_hat_1_flat).reshape(sx, k)
    d_hat = x.T
    return d_hat
...
def obj_func(z_hat, d_hat, b, lambda_residual, lambda_prior, psf_radius, size_z, size_x):
...
def construct_X(Z, ds):
    ...
    for i in range(Z.shape[1]):
        X[i] = sum([signal.convolve(Z[(k, i)], d) for k, d in enumerate(ds)], 0)
