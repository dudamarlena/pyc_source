...
# [GCC 8.4.0]
# Embedded file name: /home/tom/.local/miniconda/lib/python3.6/site-packages/alphacsc/datasets/somato.py
...
def load_data(dataset='somato', n_splits=10, sfreq=None, epoch=None, filter_params=[
 2.0, None], return_array=True, n_jobs=1):
    ...
    if dataset == 'somato':
        ...
        file_bem = join(subjects_dir, 'somato', 'bem', 'somato-5120-bem-sol.fif')
    else:
        if dataset == 'sample':
            ...
            file_bem = join(subjects_dir, 'sample', 'bem', 'sample-5120-bem-sol.fif')
        else:
    ...
    if dataset == 'somato':
        ...
        epochs_cov = mne.Epochs(raw, events, event_id, tmin=(-4), tmax=0, picks=picks_cov,
          baseline=baseline,
          reject=dict(grad=4e-10, eog=0.00035),
          preload=True)
    ...
    if epoch:
        ...
        epochs = mne.Epochs(raw, events, event_id, t_min, t_max, picks=picks, baseline=baseline,
          reject=dict(grad=4e-10,
          eog=0.00035),
          preload=True)
