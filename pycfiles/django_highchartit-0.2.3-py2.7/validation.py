...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\chartit\validation.py
...
def get_all_field_names(meta):
    ...
    for field in fields:
        if field.is_relation and field.many_to_one and field.related_model is None:
            continue
        if field.model != meta.model and field.model._meta.concrete_model == meta.concrete_model:
            continue
...
def _clean_source(source):
    ...
    if isinstance(source, QuerySet):
        return source
    raise APIInputError("'source' must either be a QuerySet, Model or Manager. Got %s of type %s instead." % (
     source, type(source)))


def _validate_func(func):
    if not isinstance(func, Aggregate):
        raise APIInputError("'func' must an instance of django Aggregate. Got %s of type %s instead" % (
         func, type(func)))
...
def _clean_categories(categories, source):
    if isinstance(categories, six.string_types):
        ...
        if isinstance(categories, (tuple, list)):
            if not categories:
                raise APIInputError("'categories' tuple or list must contain at least one valid model field. Got %s." % categories)
        else:
            raise APIInputError("'categories' must be one of the following types: basestring, tuple or list. Got %s of type %s instead." % (
...
def _clean_legend_by(legend_by, source):
    if isinstance(legend_by, six.string_types):
        ...
        if isinstance(legend_by, (tuple, list)):
            pass
        elif legend_by is None:
            legend_by = ()
        else:
            raise APIInputError("'legend_by' must be one of the following types: basestring, tuple or list. Got %s of type %s instead." % (
    ...
    return (
     legend_by, field_aliases)


def _validate_top_n_per_cat(top_n_per_cat):
    if not isinstance(top_n_per_cat, int):
        raise APIInputError("'top_n_per_cat' must be an int. Got %s of type %s instead." % (
        ...
        if isinstance(terms, dict):
            ...
            for tk, tv in terms.items():
                if isinstance(tv, Aggregate):
                    ...
                    raise APIInputError('Expecting a dict or django Aggregate in place of: %s' % tv)
...
def clean_pdps(series):
    ...
    if isinstance(series, list):
        ...
        for td in series.values():
            ...
            try:
                td['categories'], fa_cat = _clean_categories(td['categories'], td['source'])
            except KeyError:
                raise APIInputError("Missing 'categories': %s" % td)

            try:
                td['legend_by'], fa_lgby = _clean_legend_by(td['legend_by'], td['source'])
            except KeyError:
            ...
            td['field_aliases'] = _clean_field_aliases(fa_actual, fa_cat, fa_lgby)
...
def _convert_dps_to_dict(series_list):
    ...
    for sd in series_list:
        ...
        if isinstance(terms, list):
                ...
                if isinstance(term, six.string_types):
                    ...
                    for tk, tv in term.items():
                        if isinstance(tv, six.string_types):
                            ...
                            raise APIInputError('Expecting a basestring or dict in place of: %s' % tv)
            ...
            raise APIInputError('Expecting a list or dict in place of: %s.' % terms)
                ...
                if isinstance(term, six.string_types):
                    ...
                    for tk, tv in term.items():
                        if not isinstance(tv, dict):
                            raise APIInputError('Expecting a dict in place of: %s' % tv)
                        opts = copy.deepcopy(options)
                        opts.update(tv)
    ...
    if isinstance(series_options, dict):
        for sok, sod in series_options.items():
            if sok not in ds.series.keys():
                raise APIInputError('All the series terms must be present in the series dict of the datasource. Got %s. Allowed values are: %s' % (
                 sok, (', ').join(ds.series.keys())))
            if not isinstance(sod, dict):
                raise APIInputError('All the series options must be of the type dict. Got %s of type %s instead.' % (
                 sod, type(sod)))
        ...
        raise APIInputError('Expecting a dict or list in place of: %s.' % series_options)
                ...
                if isinstance(td, list):
                    for yterm in td:
                        if isinstance(yterm, six.string_types):
                            ...
                            raise APIInputError('Expecting a basestring or dict in place of: %s.' % yterm)

                else:
                    raise APIInputError('Expecting a list instead of: %s' % td)
...
def clean_cso(series_options, ds):
    ...
    if isinstance(series_options, dict):
        for sok, sod in series_options.items():
            if sok not in ds.series.keys():
                raise APIInputError('%s is not one of the keys of the datasource series. Allowed values are: %s' % (
                 sok, (', ').join(ds.series.keys())))
            if not isinstance(sod, dict):
            ...
            if ds.series[sok]['_data'] != ds.series[_x_axis_term]['_data']:
                raise APIInputError('%s and %s do not belong to the same table.' % (
                 sok, _x_axis_term))
        ...
        raise APIInputError("'series_options' must either be a dict or a list. Got %s of type %s instead." % (
...
def clean_sortf_mapf_mts(sortf_mapf_mts):
    ...
    if isinstance(sortf_mapf_mts, tuple):
        if len(sortf_mapf_mts) != 3:
            raise APIInputError('%r must have exactly three elements.' % sortf_mapf_mts)
...
def clean_x_sortf_mapf_mts(x_sortf_mapf_mts):
    ...
    for x_s_m_mts in x_sortf_mapf_mts:
        ...
        if len(x_s_m_mts) != 3:
            raise APIInputError('%r must have exactly three elements.' % x_s_m_mts)
        x_sortf = x_s_m_mts[0]
