...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\chartit\charts.py
...
class Chart(object):
    ...
    def __init__(self, datasource, series_options, chart_options=None, x_sortf_mapf_mts=None):
        ...
        **Raises**:
        ...
        if not isinstance(datasource, DataPool):
            raise APIInputError('%s must be an instance of DataPool.' % datasource)
    ...
    def _set_default_hcoptions(self, chart_options):
        ...
        if max_x_axis >= x_axis_len:
            self.hcoptions['xAxis'].extend([
             HCOptions({})] * (max_x_axis + 1 - x_axis_len))
        ...
        if max_y_axis >= y_axis_len:
            self.hcoptions['yAxis'].extend([
             HCOptions({})] * (max_y_axis + 1 - y_axis_len))
    ...
    def generate_plot(self):
        self.hcoptions['series'] = []
        dss = self.datasource.series
        cht_typ_grp = lambda y_term: 'scatter' if self.series_options[y_term]['type'] in ('scatter', 'pie') else 'line'
        for x_axis_num, vqs_groups in self.x_axis_vqs_groups.items():
            ...
            for ptype, x_y_terms_tuples in ptype_x_y_terms.items():
                ...
                for x_term, y_terms in x_y_terms_tuples:
                    ...
                    y_aliases = [ dss[y_term]['field_alias'] for y_term in y_terms
                                ]
                    y_types = [ self.series_options[y_term].get('type', 'line') for y_term in y_terms
                              ]
                    y_hco_list = [ HCOptions(copy.deepcopy(self.series_options[y_term])) for y_term in y_terms
                                 ]
                    ...
                    if ptype == 'scatter' or ptype == 'line' and len(x_y_terms_tuples) == 1:
                        if x_mts:
                            if x_mapf:
                                data = ((x_mapf(value_dict[x_field]), [ value_dict[y_field] for y_field in y_fields ]) for value_dict in x_vqs)
                                sort_key = (lambda x_y: x_sortf(x_y[0])) if x_sortf is not None else None
                                data = sorted(data, key=sort_key)
                        else:
                            sort_key = (lambda x_y: x_sortf(x_y[1])) if x_sortf is not None else None
                            data = sorted(((value_dict[x_field], [ value_dict[y_field] for y_field in y_fields ]) for value_dict in x_vqs), key=sort_key)
                            if x_mapf:
                                data = [ (x_mapf(x), y) for x, y in data ]
                        if ptype == 'scatter':
                            if self.series_options[y_term]['type'] == 'scatter':
                                for x_value, y_value_tuple in data:
                                    for opts, y_value in zip(y_hco_list, y_value_tuple):
                        ...
                        if ptype == 'line' and len(x_y_terms_tuples) == 1:
                            ...
                            if len(hco_x_axis) - 1 < x_axis_num:
                                hco_x_axis.extend([HCOptions({})] * (x_axis_num - (len(hco_x_axis) - 1)))
                            hco_x_axis[x_axis_num]['categories'] = []
                            for x_value, y_value_tuple in data:
                                hco_x_axis[x_axis_num]['categories'].append(x_value)
                                for opts, y_value in zip(y_hco_list, y_value_tuple):
                                    opts['data'].append(y_value)

                            self.hcoptions['series'].extend(y_hco_list)
                    else:
                        data = ((value_dict[x_field], [ value_dict[y_field] for y_field in y_fields ]) for value_dict in x_vqs)
                ...
                if y_terms_multi:
                    ...
                    if len(hco_x_axis) - 1 < x_axis_num:
                        hco_x_axis.extend([
                         HCOptions({})] * (x_axis_num - (len(hco_x_axis) - 1)))
                    hco_x_axis[x_axis_num]['categories'] = []
                    if x_mts:
                        if x_mapf:
                            data = ((x_mapf(x_value), y_vals) for x_value, y_vals in y_values_multi.iteritems())
                            sort_key = (lambda x_y: x_sortf(x_y[1])) if x_sortf is not None else None
                            data = sorted(data, key=sort_key)
                    else:
...
class PivotChart(object):
    ...
    def __init__(self, datasource, series_options, chart_options=None):
        ...
        **Raises**:
        ...
        if not isinstance(datasource, PivotDataPool):
            raise APIInputError('%s must be an instance of PivotDataPool.' % datasource)
        self.datasource = datasource
    ...
    def set_default_hcoptions(self):
        ...
        lgby_vname_lists = [ [ dss[t]['field_aliases'].get(lgby, lgby) for lgby in lgby_tuple ] for t, lgby_tuple in lgby_dict.items()
                           ]
        lgby_titles = ((':').join(lgby_vname_list).title() for lgby_vname_list in lgby_vname_lists)
        ...
        categories_vnames = [ dss[terms[0]]['field_aliases'][c].title() for c in categories
                            ]
    ...
    def generate_plot(self):
        ...
        for term, options in self.series_options.items():
            ...
            for lv in dss[term]['_lv_set']:
                data = [ dss[term]['_cv_lv_dfv'][cv].get(lv, None) for cv in cv_raw ]
                term_pretty_name = term.replace('_', ' ')
                name = (lv or term_pretty_name.title)() if 1 else ('-').join(lv)
        ...
        self.hcoptions['xAxis']['categories'] = [ (':').join(cv) for cv in self.datasource.cv
                                                ]
        return