...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/contrib/hooks/aws_athena_hook.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 7061 bytes
...
class AWSAthenaHook(AwsHook):
    ...
    def run_query(self, query, query_context, result_configuration, client_request_token=None):
        ...
        :return: str
        """
        response = self.get_conn().start_query_execution(QueryString=query, ClientRequestToken=client_request_token,
        ...
        while True:
            ...
            if query_state is None:
                ...
                if query_state in self.INTERMEDIATE_STATES:
                    self.log.info('Trial {try_number}: Query is still in an intermediate state - {state}'.format(try_number=try_number,
                      state=query_state))
                else:
                    self.log.info('Trial {try_number}: Query execution completed. Final state is {state}'.format(try_number=try_number,
                      state=query_state))
