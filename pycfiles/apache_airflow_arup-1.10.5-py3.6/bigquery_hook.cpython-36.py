...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/contrib/hooks/bigquery_hook.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 91953 bytes
"""
...
class BigQueryHook(GoogleCloudBaseHook, DbApiHook):
    ...
    def __init__(self, bigquery_conn_id='bigquery_default', delegate_to=None, use_legacy_sql=True, location=None):
        super(BigQueryHook, self).__init__(gcp_conn_id=bigquery_conn_id,
    ...
    def get_conn(self):
        ...
        return BigQueryConnection(service=service,
          project_id=project,
    ...
    def get_service(self):
        ...
        return build('bigquery',
          'v2', http=http_authorized, cache_discovery=False)
    ...
    def get_pandas_df(self, sql, parameters=None, dialect=None):
        ...
        return read_gbq(sql, project_id=(self._get_field('project')),
          dialect=dialect,
          verbose=False,
    ...
    def table_exists(self, project_id, dataset_id, table_id):
        ...
        service = self.get_service()
        try:
            service.tables().get(projectId=project_id,
...
class BigQueryBaseCursor(LoggingMixin):
    ...
    def __init__(self, service, project_id, use_legacy_sql=True, api_resource_configs=None, location=None, num_retries=5):
        self.service = service
        self.project_id = project_id
        self.use_legacy_sql = use_legacy_sql
        if api_resource_configs:
            _validate_value('api_resource_configs', api_resource_configs, dict)
        self.api_resource_configs = api_resource_configs if api_resource_configs else {}
        self.running_job_id = None
        self.location = location
        self.num_retries = num_retries

    def create_empty_table(self, project_id, dataset_id, table_id, schema_fields=None, time_partitioning=None, cluster_fields=None, labels=None, view=None, encryption_configuration=None, num_retries=5):
        """
    ...
    def create_external_table(self, external_project_dataset_table, schema_fields, source_uris, source_format='CSV', autodetect=False, compression='NONE', ignore_unknown_values=False, max_bad_records=0, skip_leading_rows=0, field_delimiter=',', quote_character=None, allow_quoted_newlines=False, allow_jagged_rows=False, src_fmt_configs=None, labels=None, encryption_configuration=None):
        """
        ...
        if src_fmt_configs is None:
            ...
            project_id, dataset_id, external_table_id = _split_tablename(table_input=external_project_dataset_table, default_project_id=(self.project_id),
            ...
            table_resource = {'externalDataConfiguration':{'autodetect':autodetect, 
              'sourceFormat':source_format, 
              'sourceUris':source_uris, 
              'compression':compression, 
              'ignoreUnknownValues':ignore_unknown_values}, 
             'tableReference':{'projectId':project_id, 
              'datasetId':dataset_id, 
            ...
            src_fmt_to_param_mapping = {'CSV':'csvOptions', 
             'GOOGLE_SHEETS':'googleSheetsOptions'}
            src_fmt_to_configs_mapping = {'csvOptions':[
    ...
    def patch_table(self, dataset_id, table_id, project_id=None, description=None, expiration_time=None, external_data_configuration=None, friendly_name=None, labels=None, schema=None, time_partitioning=None, view=None, require_partition_filter=None, encryption_configuration=None):
        """
        ...
        try:
            self.service.tables().patch(projectId=project_id,
              datasetId=dataset_id,
              tableId=table_id,
              body=table_resource).execute(num_retries=(self.num_retries))
    ...
    def run_query(self, bql=None, sql=None, destination_dataset_table=None, write_disposition='WRITE_EMPTY', allow_large_results=False, flatten_results=None, udf_config=None, use_legacy_sql=None, maximum_billing_tier=None, maximum_bytes_billed=None, create_disposition='CREATE_IF_NEEDED', query_params=None, labels=None, schema_update_options=None, priority='INTERACTIVE', time_partitioning=None, api_resource_configs=None, cluster_fields=None, location=None, encryption_configuration=None):
        """
        ...
        if bql:
            import warnings
            warnings.warn('Deprecated parameter `bql` used in `BigQueryBaseCursor.run_query` Use `sql` parameter instead to pass the sql to be executed. `bql` parameter is deprecated and will be removed in a future version of Airflow.', category=DeprecationWarning)
        if sql is None:
            if not configuration['query'].get('query', None):
        ...
        if not set(allowed_schema_update_options).issuperset(set(schema_update_options)):
            raise ValueError('{0} contains invalid schema update options. Please only use one or more of the following options: {1}'.format(schema_update_options, allowed_schema_update_options))
        ...
        if destination_dataset_table:
            destination_project, destination_dataset, destination_table = _split_tablename(table_input=destination_dataset_table, default_project_id=(self.project_id))
            destination_dataset_table = {'projectId':destination_project, 
        ...
        for param, param_name, param_default, param_type in query_param_list:
            if param_name not in configuration['query']:
                if param in [None, {}, ()]:
                ...
                if param not in [None, {}, ()]:
                    ...
                    if param_name == 'schemaUpdateOptions':
                        if param:
                    ...
                    if param_name == 'destinationTable':
                        for key in ('projectId', 'datasetId', 'tableId'):
                            if key not in configuration['query']['destinationTable']:
                                raise ValueError("Not correct 'destinationTable' in api_resource_configs. 'destinationTable' must be a dict with {'projectId':'', 'datasetId':'', 'tableId':''}")

                        configuration['query'].update({'allowLargeResults':allow_large_results, 
                         'flattenResults':flatten_results, 
                         'writeDisposition':write_disposition, 
                         'createDisposition':create_disposition})

        if 'useLegacySql' in configuration['query']:
            if configuration['query']['useLegacySql']:
                if 'queryParameters' in configuration['query']:
    ...
    def run_extract(self, source_project_dataset_table, destination_cloud_storage_uris, compression='NONE', export_format='CSV', field_delimiter=',', print_header=True, labels=None):
        """
        ...
        :type labels: dict
        """
        source_project, source_dataset, source_table = _split_tablename(table_input=source_project_dataset_table, default_project_id=(self.project_id),
          var_name='source_project_dataset_table')
        configuration = {'extract': {'sourceTable':{'projectId':source_project, 
    ...
    def run_copy(self, source_project_dataset_tables, destination_project_dataset_table, write_disposition='WRITE_EMPTY', create_disposition='CREATE_IF_NEEDED', labels=None, encryption_configuration=None):
        """
        ...
        for source_project_dataset_table in source_project_dataset_tables:
            source_project, source_dataset, source_table = _split_tablename(table_input=source_project_dataset_table, default_project_id=(self.project_id),
              var_name='source_project_dataset_table')
            source_project_dataset_tables_fixup.append({'projectId':source_project, 
             'datasetId':source_dataset, 
             'tableId':source_table})
        ...
        configuration = {'copy': {'createDisposition':create_disposition, 
                  'writeDisposition':write_disposition, 
                  'sourceTables':source_project_dataset_tables_fixup, 
                  'destinationTable':{'projectId':destination_project, 
                   'datasetId':destination_dataset, 
                   'tableId':destination_table}}}
        if labels:
            configuration['labels'] = labels
    ...
    def run_with_configuration(self, configuration):
        ...
        query_reply = jobs.insert(projectId=(self.project_id),
          body=job_data).execute(num_retries=(self.num_retries))
    ...
    def poll_job_complete(self, job_id):
        ...
        try:
            if self.location:
                job = jobs.get(projectId=(self.project_id), jobId=job_id,
                  location=(self.location)).execute(num_retries=(self.num_retries))
            else:
                job = jobs.get(projectId=(self.project_id), jobId=job_id).execute(num_retries=(self.num_retries))
            if job['status']['state'] == 'DONE':
                return True
        except HttpError as err:
            if err.resp.status in (500, 503):
    ...
    def cancel_query(self):
        ...
        if self.running_job_id:
            if not self.poll_job_complete(self.running_job_id):
                self.log.info('Attempting to cancel job : %s, %s', self.project_id, self.running_job_id)
                if self.location:
                    jobs.cancel(projectId=(self.project_id),
                      jobId=(self.running_job_id),
                      location=(self.location)).execute(num_retries=(self.num_retries))
                else:
                    jobs.cancel(projectId=(self.project_id),
    ...
    def get_schema(self, dataset_id, table_id):
        ...
        """
        tables_resource = self.service.tables().get(projectId=(self.project_id),
          datasetId=dataset_id,
          tableId=table_id).execute(num_retries=(self.num_retries))
        return tables_resource['schema']

    def get_tabledata(self, dataset_id, table_id, max_results=None, selected_fields=None, page_token=None, start_index=None):
        """
        ...
        return (self.service.tabledata().list)(projectId=self.project_id, 
         datasetId=dataset_id, 
         tableId=table_id, **optional_params).execute(num_retries=(self.num_retries))
    ...
    def run_table_delete(self, deletion_dataset_table, ignore_if_missing=False):
        ...
        try:
            self.service.tables().delete(projectId=deletion_project,
              datasetId=deletion_dataset,
              tableId=deletion_table).execute(num_retries=(self.num_retries))
    ...
    def run_table_upsert(self, dataset_id, table_resource, project_id=None):
        ...
        while True:
            ...
            if 'nextPageToken' in tables_list_resp:
                tables_list_resp = self.service.tables().list(projectId=project_id,
                  datasetId=dataset_id,
                  pageToken=(tables_list_resp['nextPageToken'])).execute(num_retries=(self.num_retries))
            else:
    ...
    def run_grant_dataset_view_access(self, source_dataset, view_dataset, view_table, source_project=None, view_project=None):
        ...
        source_dataset_resource = self.service.datasets().get(projectId=source_project,
          datasetId=source_dataset).execute(num_retries=(self.num_retries))
        ...
        if view_access not in access:
            self.log.info('Granting table %s:%s.%s authorized view access to %s:%s dataset.', view_project, view_dataset, view_table, source_project, source_dataset)
            access.append(view_access)
            return self.service.datasets().patch(projectId=source_project,
              datasetId=source_dataset,
              body={'access': access}).execute(num_retries=(self.num_retries))
        else:
            self.log.info('Table %s:%s.%s already has authorized view access to %s:%s dataset.', view_project, view_dataset, view_table, source_project, source_dataset)
    ...
    def create_empty_dataset(self, dataset_id='', project_id='', dataset_reference=None):
        ...
        if not dataset_reference['datasetReference'].get('datasetId'):
            if not dataset_id:
        ...
        try:
            self.service.datasets().insert(projectId=dataset_project_id,
    ...
    def delete_dataset(self, project_id, dataset_id):
        ...
        try:
            self.service.datasets().delete(projectId=project_id,
              datasetId=dataset_id).execute(num_retries=(self.num_retries))
            self.log.info('Dataset deleted successfully: In project %s Dataset %s', project_id, dataset_id)
        except HttpError as err:
            raise AirflowException('BigQuery job failed. Error was: {}'.format(err.content))
    ...
    def get_dataset(self, dataset_id, project_id=None):
        ...
        if not dataset_id or not isinstance(dataset_id, str):
            raise ValueError("dataset_id argument must be provided and has a type 'str'. You provided: {}".format(dataset_id))
        dataset_project_id = project_id if project_id else self.project_id
        try:
            dataset_resource = self.service.datasets().get(datasetId=dataset_id,
    ...
    def insert_all(self, project_id, dataset_id, table_id, rows, ignore_unknown_values=False, skip_invalid_rows=False, fail_on_error=False):
        """
        ...
        **Example or rows**:
        ...
        try:
            ...
            resp = self.service.tabledata().insertAll(projectId=dataset_project_id,
              datasetId=dataset_id,
              tableId=table_id,
            ...
            if 'insertErrors' not in resp:
                ...
                error_msg = '{} insert error(s) occured: {}:{}.{}. Details: {}'.format(len(resp['insertErrors']), dataset_project_id, dataset_id, table_id, resp['insertErrors'])
                if fail_on_error:
...
class BigQueryCursor(BigQueryBaseCursor):
    ...
    def __init__(self, service, project_id, use_legacy_sql=True, location=None, num_retries=5):
        super(BigQueryCursor, self).__init__(service=service,
        ...
        if not self.job_id:
            ...
            if len(self.buffer) == 0:
                if self.all_pages_loaded:
                    return
                query_results = self.service.jobs().getQueryResults(projectId=(self.project_id),
                  jobId=(self.job_id),
...
def _split_tablename(table_input, default_project_id, var_name=None):
    if '.' not in table_input:
        raise ValueError('Expected target table name in the format of <dataset>.<table>. Got: {}'.format(table_input))
    else:
        if not default_project_id:
            ...
            if table_input.count('.') + table_input.count(':') > 3:
                raise Exception('{var}Use either : or . to specify project got {input}'.format(var=(var_print(var_name)),
                  input=table_input))
            ...
            if len(cmpt) == 1:
                ...
                raise Exception('{var}Expect format of (<project:)<dataset>.<table>, got {input}'.format(var=(var_print(var_name)),
                  input=table_input))
            cmpt = rest.split('.')
            if len(cmpt) == 3:
                if project_id:
                ...
                if len(cmpt) == 2:
                    ...
                    raise Exception('{var}Expect format of (<project.|<project:)<dataset>.<table>, got {input}'.format(var=(var_print(var_name)),
                      input=table_input))
        if project_id is None:
            if var_name is not None:
                log = LoggingMixin().log
                log.info('Project not included in %s: %s; using project "%s"', var_name, table_input, default_project_id)
...
def _cleanse_time_partitioning(destination_dataset_table, time_partitioning_in):
    ...
    if destination_dataset_table:
        if '$' in destination_dataset_table:
...
def _api_resource_configs_duplication_check(key, value, config_dict, config_dict_name='api_resource_configs'):
    if key in config_dict:
        if value != config_dict[key]:
            raise ValueError('Values of {param_name} param are duplicated. {dict_name} contained {param_name} param in `query` config and {param_name} was also provided with arg to run_query() method. Please remove duplicates.'.format(param_name=key,
              dict_name=config_dict_name))