...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/contrib/hooks/gcp_mlengine_hook.py
...
class MLEngineHook(GoogleCloudBaseHook):
    ...
    def create_job(self, project_id, job, use_existing_job_fn=None):
        ...
        :rtype: dict
        """
        request = self._mlengine.projects().jobs().create(parent=('projects/{}'.format(project_id)),
          body=job)
        job_id = job['jobId']
        try:
            request.execute()
    ...
    def create_version(self, project_id, model_name, version_spec):
        ...
        create_request = self._mlengine.projects().models().versions().create(parent=parent_name,
          body=version_spec)
        response = create_request.execute()
        get_request = self._mlengine.projects().operations().get(name=(response['name']))
        return _poll_with_exponential_delay(request=get_request,
          max_n=9,
    ...
    def set_default_version(self, project_id, model_name, version_name):
        ...
        request = self._mlengine.projects().models().versions().setDefault(name=full_version_name,
          body={})
        try:
            response = request.execute()
    ...
    def list_versions(self, project_id, model_name):
        ...
        request = self._mlengine.projects().models().versions().list(parent=full_parent_name,
          pageSize=100)
        response = request.execute()
        next_page_token = response.get('nextPageToken', None)
        result.extend(response.get('versions', []))
        while next_page_token is not None:
            next_request = self._mlengine.projects().models().versions().list(parent=full_parent_name,
    ...
    def create_model(self, project_id, model):
        ...
        request = self._mlengine.projects().models().create(parent=project,
          body=model)
        return request.execute()
