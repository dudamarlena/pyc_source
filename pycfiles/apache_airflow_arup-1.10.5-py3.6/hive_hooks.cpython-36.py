...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/hooks/hive_hooks.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 38788 bytes
from __future__ import print_function, unicode_literals
import contextlib, os, re, subprocess, time
from collections import OrderedDict
from tempfile import NamedTemporaryFile
import six, unicodecsv as csv
...
def get_context_from_env_var():
    ...
    """
    return {format_map['default']:os.environ.get(format_map['env_var_format'], '') for format_map in AIRFLOW_VAR_NAME_FORMAT_MAPPING.values()}


class HiveCliHook(BaseHook):
    __doc__ = 'Simple wrapper around the hive CLI.\n\n    It also supports the ``beeline``\n    a lighter CLI that runs JDBC and is replacing the heavier\n    traditional CLI. To enable ``beeline``, set the use_beeline param in the\n    extra field of your connection as in ``{ "use_beeline": true }``\n\n    Note that you can also set default hive CLI parameters using the\n    ``hive_cli_params`` to be used in your connection as in\n    ``{"hive_cli_params": "-hiveconf mapred.job.tracker=some.jobtracker:444"}``\n    Parameters passed here can be overridden by run_cli\'s hive_conf param\n\n    The extra connection parameter ``auth`` gets passed as in the ``jdbc``\n    connection string as is.\n\n    :param mapred_queue: queue used by the Hadoop Scheduler (Capacity or Fair)\n    :type  mapred_queue: str\n    :param mapred_queue_priority: priority within the job queue.\n        Possible settings include: VERY_HIGH, HIGH, NORMAL, LOW, VERY_LOW\n    :type  mapred_queue_priority: str\n    :param mapred_job_name: This name will appear in the jobtracker.\n        This can make monitoring easier.\n    :type  mapred_job_name: str\n    '

    def __init__(self, hive_cli_conn_id='hive_cli_default', run_as=None, mapred_queue=None, mapred_queue_priority=None, mapred_job_name=None):
        ...
        if mapred_queue_priority:
            ...
            if mapred_queue_priority not in HIVE_QUEUE_PRIORITIES:
                raise AirflowException('Invalid Mapred Queue Priority.  Valid values are: {}'.format(', '.join(HIVE_QUEUE_PRIORITIES)))
        self.mapred_queue = mapred_queue or configuration.get('hive', 'default_hive_mapred_queue')
    ...
    def _prepare_cli_cmd(self):
        ...
        if self.use_beeline:
            hive_bin = 'beeline'
            jdbc_url = 'jdbc:hive2://{host}:{port}/{schema}'.format(host=(conn.host),
              port=(conn.port),
              schema=(conn.schema))
            if configuration.conf.get('core', 'security') == 'kerberos':
                ...
                jdbc_url += ';principal={template};{proxy_user}'.format(template=template,
        ...
        return [
         hive_bin] + cmd_extra + hive_params_list
            ...
            with NamedTemporaryFile(dir=tmp_dir) as (f):
                ...
                if self.mapred_queue:
                    hive_conf_params.extend([
                     ...
                     '-hiveconf',
                     'tez.job.queue.name={}'.format(self.mapred_queue)])
                if self.mapred_queue_priority:
                    hive_conf_params.extend([
                     '-hiveconf',
                     'mapreduce.job.priority={}'.format(self.mapred_queue_priority)])
    ...
    def test_hql(self, hql):
        ...
        for query in hql.split(';'):
            ...
            if query.startswith('create table'):
                ...
                if query.startswith(('set ', 'add jar ', 'create temporary function')):
    ...
    def load_df(self, df, table, field_dict=None, delimiter=',', encoding='utf8', pandas_kwargs=None, **kwargs):
        ...
        def _infer_field_types_from_df(df):
            DTYPE_KIND_HIVE_TYPE = {'b':'BOOLEAN', 
             'i':'BIGINT', 
             'u':'BIGINT', 
             'f':'DOUBLE', 
             'c':'STRING', 
        ...
        with TemporaryDirectory(prefix='airflow_hiveop_') as (tmp_dir):
            with NamedTemporaryFile(dir=tmp_dir, mode='w') as (f):
                ...
                (df.to_csv)(path_or_buf=f, sep=delimiter.encode(encoding) if (six.PY2 and isinstance(delimiter, unicode)) else delimiter, 
                 header=False, 
                 index=False, 
                 encoding=encoding, 
                 date_format='%Y-%m-%d %H:%M:%S', **pandas_kwargs)
                f.flush()
                return (self.load_file)(filepath=f.name, table=table, 
                 delimiter=delimiter, 
...
class HiveMetastoreHook(BaseHook):
    __doc__ = ' Wrapper to interact with the Hive Metastore'
    ...
    def get_partitions(self, schema, table_name, filter=None):
        ...
        with self.metastore as (client):
            ...
            if len(table.partitionKeys) == 0:
                ...
                if filter:
                    parts = client.get_partitions_by_filter(db_name=schema,
                      tbl_name=table_name,
                      filter=filter,
                      max_parts=(HiveMetastoreHook.MAX_PART_COUNT))
                else:
                    parts = client.get_partitions(db_name=schema,
                      tbl_name=table_name,
                      max_parts=(HiveMetastoreHook.MAX_PART_COUNT))
    ...
    def _get_max_partition_from_part_specs(part_specs, partition_key, filter_map):
        ...
        if not part_specs:
            ...
            if partition_key not in part_specs[0].keys():
                ...
                if filter_map:
                    if not is_subset:
                        raise AirflowException('Keys in provided filter_map {} are not subset of part_spec keys: {}'.format(', '.join(filter_map.keys()), ', '.join(part_specs[0].keys())))
                candidates = [p_dict[partition_key] for p_dict in part_specs if filter_map is None or all(item in p_dict.items() for item in filter_map.items())]
                if not candidates:
                    return
...
class HiveServer2Hook(BaseHook):
        ...
        with contextlib.closing(self.get_conn(schema)) as (conn):
            with contextlib.closing(conn.cursor()) as (cur):
                ...
                for statement in hql:
                    ...
                    if lowered_statement.startswith('select') or lowered_statement.startswith('with') or lowered_statement.startswith('set') and '=' not in lowered_statement:
    ...
    def get_results(self, hql, schema='default', fetch_size=None, hive_conf=None):
        ...
        results = {'data':list(results_iter), 
         'header':header}
        return results
