...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/www_rbac/api/experimental/endpoints.py
...
def task_info(dag_id, task_id):
    ...
    fields = {k:str(v) for k, v in vars(info).items() if not k.startswith('_') if not k.startswith('_')}
...
@api_experimental.route('/dags/<string:dag_id>/dag_runs/<string:execution_date>',
  methods=[
 'GET'])
@requires_authentication
...
def latest_dag_runs():
    ...
    for dagrun in dagruns:
        if dagrun.execution_date:
            payload.append({'dag_id':dagrun.dag_id, 
             'execution_date':dagrun.execution_date.isoformat(), 
             'start_date':dagrun.start_date or '' and dagrun.start_date.isoformat(), 
             'dag_run_url':url_for('Airflow.graph', dag_id=dagrun.dag_id, execution_date=dagrun.execution_date)})
