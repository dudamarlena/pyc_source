...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/configuration.py
...
class AirflowConfigParser(ConfigParser):
    ...
    deprecated_options = {'celery':{'worker_concurrency':'celeryd_concurrency', 
    ...
    deprecated_values = {'core': {'task_runner': ('BashTaskRunner', 'StandardTaskRunner', '2.0')}}
    ...
    def _validate(self):
        if self.get('core', 'executor') != 'SequentialExecutor':
            if 'sqlite' in self.get('core', 'sql_alchemy_conn'):
                raise AirflowConfigException('error: cannot use sqlite with the {}'.format(self.get('core', 'executor')))
        if self.getboolean('webserver', 'authenticate') and self.get('webserver', 'owner_mode') not in ('user',
                                                                                                        'ldapgroup'):
            raise AirflowConfigException("error: owner_mode option should be either 'user' or 'ldapgroup' when filtering by owner is set")
        else:
            if self.getboolean('webserver', 'authenticate'):
                if self.get('webserver', 'owner_mode').lower() == 'ldapgroup':
                    if self.get('webserver', 'auth_backend') != 'airflow.contrib.auth.backends.ldap_auth':
        ...
        for section, replacement in self.deprecated_values.items():
            for name, info in replacement.items():
                ...
                if self.get(section, name, fallback=None) == old:
                    ...
                    warnings.warn('The {name} setting in [{section}] has the old default value of {old!r}. This value has been changed to {new!r} in the running config, but please update your config before Apache Airflow {version}.'.format(name=name,
                      section=section,
                      old=old,
                      new=new,
                      version=version), FutureWarning)
    ...
    def _get_cmd_option(self, section, key):
        ...
        if (
         section, key) in self.as_command_stdout:
    ...
    def get(self, section, key, **kwargs):
        ...
        if super(AirflowConfigParser, self).has_option(section, key):
            return expand_env_var((super(AirflowConfigParser, self).get)(section, key, **kwargs))
        if deprecated_name:
        ...
        raise AirflowConfigException('section/key [{section}/{key}] not found in config'.format(section=section,
          key=key))
    ...
    def getboolean(self, section, key, **kwargs):
        ...
        if val in ('f', 'false', '0'):
            return False
        raise ValueError('The value for configuration option "{}:{}" is not a boolean (received "{}").'.format(section, key, val))
    ...
    def getsection(self, section):
        ...
        if section not in self._sections:
            if section not in self.airflow_defaults._sections:
                return
    ...
    def as_dict(self, display_source=False, display_sensitive=False, raw=False, include_env=True, include_cmds=True):
        """
        ...
        if include_env:
            for ev in [ev for ev in os.environ if ev.startswith('AIRFLOW__')]:
                ...
                if not display_sensitive:
                    if ev != 'AIRFLOW__CORE__UNIT_TEST_MODE':
                        opt = '< hidden >'
                if raw:
                    opt = opt.replace('%', '%%')
                if display_source:
    ...
    def _warn_deprecate(self, section, key, deprecated_name):
        warnings.warn('The {old} option in [{section}] has been renamed to {new} - the old setting has been used, but please update your config.'.format(old=deprecated_name,
          new=key,
          section=section),
          DeprecationWarning,
          stacklevel=3)
...
def mkdir_p(path):
    try:
        ...
        if exc.errno == errno.EEXIST:
            ...
            raise AirflowConfigException('Error creating {}: {}'.format(path, exc.strerror))
...
_TEST_DAGS_FOLDER = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'tests', 'dags')
if os.path.exists(_TEST_DAGS_FOLDER):
    TEST_DAGS_FOLDER = _TEST_DAGS_FOLDER
else:
    TEST_DAGS_FOLDER = os.path.join(AIRFLOW_HOME, 'dags')
_TEST_PLUGINS_FOLDER = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'tests', 'plugins')
...
if conf.has_option('core', 'AIRFLOW_HOME'):
    msg = 'Specifying both AIRFLOW_HOME environment variable and airflow_home in the config file is deprecated. Please use only the AIRFLOW_HOME environment variable and remove the config file entry.'
    if 'AIRFLOW_HOME' in os.environ:
        warnings.warn(msg, category=DeprecationWarning)
    else:
        if conf.get('core', 'airflow_home') == AIRFLOW_HOME:
            warnings.warn('Specifying airflow_home in the config file is deprecated. As you have left it at the default value you should remove the setting from your airflow.cfg and suffer no change in behaviour.',
              category=DeprecationWarning)
        else:
            AIRFLOW_HOME = conf.get('core', 'airflow_home')
            warnings.warn(msg, category=DeprecationWarning)
_old_config_file = os.path.expanduser('~/airflow/airflow.cfg')
if _old_config_file != AIRFLOW_CONFIG:
    if os.path.isfile(_old_config_file):
        warnings.warn('You have two airflow.cfg files: {old} and {new}. Airflow used to look at ~/airflow/airflow.cfg, even when AIRFLOW_HOME was set to a different value. Airflow will now only read {new}, and you should remove the other file'.format(old=_old_config_file,
          new=AIRFLOW_CONFIG),
          category=DeprecationWarning)
...
for func in [load_test_config, get, getboolean, getfloat, getint, has_option,
 remove_option, as_dict, set]:
    deprecated(func, "Accessing configuration method '{f.__name__}' directly from the configuration module is deprecated. Please access the configuration from the 'configuration.conf' object via 'conf.{f.__name__}'".format(f=func))