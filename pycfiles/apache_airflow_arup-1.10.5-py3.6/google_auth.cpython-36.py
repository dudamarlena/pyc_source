...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/contrib/auth/backends/google_auth.py
...
class GoogleAuthBackend(object):
    ...
    def init_app(self, flask_app):
        ...
        self.google_oauth = OAuth(self.flask_app).remote_app('google',
        ...
        self.flask_app.add_url_rule(get_config_param('oauth_callback_route'), 'google_oauth_callback', self.oauth_callback)

    def login(self, request):
        log.debug('Redirecting user to Google login')
        return self.google_oauth.authorize(callback=url_for('google_oauth_callback',
          _external=True),
