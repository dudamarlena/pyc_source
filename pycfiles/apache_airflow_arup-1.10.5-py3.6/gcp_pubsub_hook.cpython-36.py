...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/contrib/hooks/gcp_pubsub_hook.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 12403 bytes
...
class PubSubHook(GoogleCloudBaseHook):
    ...
    def publish(self, project, topic, messages):
        ...
        request = self.get_conn().projects().topics().publish(topic=full_topic,
          body=body)
        try:
    ...
    def create_topic(self, project, topic, fail_if_exists=False):
        ...
        full_topic = _format_topic(project, topic)
        try:
            service.projects().topics().create(name=full_topic,
    ...
    def create_subscription(self, topic_project, topic, subscription=None, subscription_project=None, ack_deadline_secs=10, fail_if_exists=False):
        ...
        body = {'topic':full_topic, 
         'ackDeadlineSeconds':ack_deadline_secs}
        try:
            service.projects().subscriptions().create(name=full_subscription,
    ...
    def pull(self, project, subscription, max_messages, return_immediately=False):
        ...
        try:
            response = service.projects().subscriptions().pull(subscription=full_subscription,
              body=body).execute(num_retries=(self.num_retries))
    ...
    def acknowledge(self, project, subscription, ack_ids):
        ...
        full_subscription = _format_subscription(project, subscription)
        try:
            service.projects().subscriptions().acknowledge(subscription=full_subscription,
              body={'ackIds': ack_ids}).execute(num_retries=(self.num_retries))
        except HttpError as e:
