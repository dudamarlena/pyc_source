...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/contrib/hooks/qubole_hook.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 9846 bytes
import os, time, datetime, six, re
...
from qds_sdk.commands import Command, HiveCommand, PrestoCommand, HadoopCommand, PigCommand, ShellCommand, SparkCommand, DbTapQueryCommand, DbExportCommand, DbImportCommand, SqlCommand
COMMAND_CLASSES = {'hivecmd':HiveCommand, 
...
def filter_options(options):
    options_to_remove = [
     'help', 'print-logs-live', 'print-logs']
...
class QuboleHook(BaseHook):
    ...
    def execute(self, context):
        ...
        if 'fetch_logs' in self.kwargs:
            if self.kwargs['fetch_logs'] is True:
    ...
    def get_results(self, ti=None, fp=None, inline=True, delim=None, fetch=True):
        ...
        if fp is None:
            ...
            logpath = os.path.expanduser(configuration.conf.get('core', 'BASE_LOG_FOLDER'))
    ...
    def get_extra_links(self, operator, dttm):
        ...
        if conn:
            if conn.host:
                host = re.sub('api$', 'v2/analyze?command_id=', conn.host)
        else:
