...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/contrib/hooks/gcp_sql_hook.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 43905 bytes
import errno, json, os, random, re, shutil, string, socket, platform, subprocess, time, uuid, os.path
...
class CloudSqlHook(GoogleCloudBaseHook):
    ...
    def get_conn(self):
        ...
        if not self._conn:
            ...
            self._conn = build('sqladmin', (self.api_version), http=http_authorized,
    ...
    def get_instance(self, instance, project_id=None):
        ...
        :rtype: dict
        """
        return self.get_conn().instances().get(project=project_id,
    ...
    def create_instance(self, body, project_id=None):
        ...
        :return: None
        """
        response = self.get_conn().instances().insert(project=project_id,
    ...
    def patch_instance(self, body, instance, project_id=None):
        ...
        :return: None
        """
        response = self.get_conn().instances().patch(project=project_id,
          instance=instance,
          body=body).execute(num_retries=(self.num_retries))
        operation_name = response['name']
        self._wait_for_operation_to_complete(project_id=project_id, operation_name=operation_name)
    ...
    def get_database(self, instance, database, project_id=None):
        ...
        :rtype: dict
        """
        return self.get_conn().databases().get(project=project_id,
          instance=instance,
    ...
    def create_database(self, instance, body, project_id=None):
        ...
        :return: None
        """
        response = self.get_conn().databases().insert(project=project_id,
          instance=instance,
          body=body).execute(num_retries=(self.num_retries))
        operation_name = response['name']
        self._wait_for_operation_to_complete(project_id=project_id, operation_name=operation_name)
    ...
    def patch_database(self, instance, database, body, project_id=None):
        ...
        :return: None
        """
        response = self.get_conn().databases().patch(project=project_id,
          instance=instance,
          database=database,
          body=body).execute(num_retries=(self.num_retries))
        operation_name = response['name']
    ...
    def export_instance(self, instance, body, project_id=None):
        ...
        """
        try:
            response = self.get_conn().instances().export(project=project_id,
              instance=instance,
              body=body).execute(num_retries=(self.num_retries))
            operation_name = response['name']
            self._wait_for_operation_to_complete(project_id=project_id, operation_name=operation_name)
    ...
    def import_instance(self, instance, body, project_id=None):
        ...
        """
        try:
            response = self.get_conn().instances().import_(project=project_id,
              instance=instance,
              body=body).execute(num_retries=(self.num_retries))
            operation_name = response['name']
            self._wait_for_operation_to_complete(project_id=project_id, operation_name=operation_name)
    ...
    def _wait_for_operation_to_complete(self, project_id, operation_name):
        ...
        while True:
            operation_response = service.operations().get(project=project_id,
              operation=operation_name).execute(num_retries=(self.num_retries))
...
CLOUD_SQL_PROXY_VERSION_DOWNLOAD_URL = 'https://storage.googleapis.com/cloudsql-proxy/{}/cloud_sql_proxy.{}.{}'
...
class CloudSqlProxyRunner(LoggingMixin):
    ...
    def __init__(self, path_prefix, instance_specification, gcp_conn_id='google_cloud_default', project_id=None, sql_proxy_version=None, sql_proxy_binary_path=None):
        """
        ...
        self.sql_proxy_path = sql_proxy_binary_path if sql_proxy_binary_path else self.path_prefix + '_cloud_sql_proxy'
    ...
    def _download_sql_proxy_if_needed(self):
        if os.path.isfile(self.sql_proxy_path):
            ...
            if not self.sql_proxy_version:
                ...
                download_url = CLOUD_SQL_PROXY_VERSION_DOWNLOAD_URL.format(self.sql_proxy_version, system, processor)
        ...
        if r.status_code != 200:
            raise AirflowException('The cloud-sql-proxy could not be downloaded. Status code = {}. Reason = {}'.format(r.status_code, r.reason))
        ...
        os.chmod(self.sql_proxy_path, 484)
    ...
    def _get_credential_parameters(self, session):
        ...
        if GCP_CREDENTIALS_KEY_PATH in connection.extra_dejson:
            ...
            if GCP_CREDENTIALS_KEYFILE_DICT in connection.extra_dejson:
                ...
                self.log.info('The credentials are not supplied by neither key_path nor keyfile_dict of the gcp connection %s. Falling back to default activated account', self.gcp_conn_id)
                credential_params = []
        if not self.instance_specification:
            ...
            if not project_id:
                raise AirflowException('For forwarding all instances, the project id for GCP should be provided either by project_id extra in the GCP connection or by project_id provided in the operator.')
    ...
    def start_proxy(self):
        ...
        if self.sql_proxy_process:
            ...
            self.sql_proxy_process = Popen(command_to_run, stdin=PIPE,
              stdout=PIPE,
              stderr=PIPE)
...
CONNECTION_URIS = {'postgres':{'proxy':{'tcp':'postgresql://{user}:{password}@127.0.0.1:{proxy_port}/{database}', 
   'socket':'postgresql://{user}:{password}@{socket_path}/{database}'}, 
  'public':{'ssl':'postgresql://{user}:{password}@{public_ip}:{public_port}/{database}?sslmode=verify-ca&sslcert={client_cert_file}&sslkey={client_key_file}&sslrootcert={server_ca_file}', 
   'non-ssl':'postgresql://{user}:{password}@{public_ip}:{public_port}/{database}'}}, 
 'mysql':{'proxy':{'tcp':'mysql://{user}:{password}@127.0.0.1:{proxy_port}/{database}', 
   'socket':'mysql://{user}:{password}@localhost/{database}?unix_socket={socket_path}'}, 
  'public':{'ssl':'mysql://{user}:{password}@{public_ip}:{public_port}/{database}?ssl={ssl_spec}', 
...
class CloudSqlDatabaseHook(BaseHook):
    ...
    def __init__(self, gcp_cloudsql_conn_id='google_cloud_sql_default', gcp_conn_id='google_cloud_default', default_gcp_project_id=None):
    ...
    def _check_ssl_file(file_to_check, name):
        ...
        if not os.path.isfile(file_to_check):
            raise AirflowException('The {file_to_check} must be a readable file'.format(file_to_check=file_to_check))
    ...
    def _validate_inputs(self):
        if self.project_id == '':
            ...
            if self.database_type not in CLOUD_SQL_VALID_DATABASE_TYPES:
                raise AirflowException("Invalid database type '{}'. Must be one of {}".format(self.database_type, CLOUD_SQL_VALID_DATABASE_TYPES))
            if self.use_proxy:
                if self.use_ssl:
                    raise AirflowException('Cloud SQL Proxy does not support SSL connections. SSL is not needed as Cloud SQL Proxy provides encryption on its own')
    ...
    def validate_socket_path_length(self):
        if self.use_proxy:
            if not self.sql_proxy_use_tcp:
                if self.database_type == 'postgres':
                    suffix = '/.s.PGSQL.5432'
                else:
                    suffix = ''
                expected_path = '{}/{}:{}:{}{}'.format(self._generate_unique_path(), self.project_id, self.instance, self.database, suffix)
                if len(expected_path) > UNIX_PATH_MAX:
                    self.log.info('Too long (%s) path: %s', len(expected_path), expected_path)
                    raise AirflowException('The UNIX socket path length cannot exceed {} characters on Linux system. Either use shorter instance/database name or switch to TCP connection. The socket path for Cloud SQL proxy is now:{}'.format(UNIX_PATH_MAX, expected_path))

    @staticmethod
    def _generate_unique_path():
        random.seed()
        while 1:
            candidate = '/tmp/' + ''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(8))
    ...
    def _generate_connection_uri(self):
        if self.use_proxy:
            if self.sql_proxy_use_tcp:
            ...
            elif not self.sql_proxy_unique_path:
                ...
                if self.use_proxy:
                    ...
                    if self.sql_proxy_use_tcp:
                        ...
                        socket_path = '{sql_proxy_socket_path}/{instance_socket_name}'.format(sql_proxy_socket_path=(self.sql_proxy_unique_path),
                          instance_socket_name=(self._get_instance_socket_name()))
                else:
                    public_uris = database_uris['public']
                    if self.use_ssl:
                        format_string = public_uris['ssl']
                        ssl_spec = {'cert':self.sslcert, 
                         'key':self.sslkey, 
                         'ca':self.sslrootcert}
                    else:
            ...
            raise self.database or AirflowException('The database parameter needs to be set in connection')
        connection_uri = format_string.format(user=(quote_plus(self.user) if self.user else ''),
        ...
        self.log.info('DB connection URI %s', connection_uri.replace(quote_plus(self.password) if self.password else 'PASSWORD', 'XXXXXXXXXXXX'))
    ...
    def cleanup_database_hook(self):
        ...
        """
        if self.database_type == 'postgres':
            if hasattr(self.db_hook, 'conn'):
                if self.db_hook.conn:
                    if self.db_hook.conn.notices:
