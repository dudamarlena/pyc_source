...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/www_rbac/views.py
# Compiled at: 2019-09-11 03:47:35
# Size of source mod 2**32: 97715 bytes
import copy, itertools, json, logging, math, os, socket, traceback
from collections import defaultdict
from datetime import timedelta
from six.moves.urllib.parse import quote
import markdown, pendulum, sqlalchemy as sqla
from flask import redirect, request, Markup, Response, render_template, make_response, flash, jsonify, url_for
...
from airflow.www_rbac.forms import DateTimeForm, DateTimeWithNumRunsForm, DateTimeWithNumRunsWithDagRunsForm, DagRunForm, ConnectionForm
...
def get_date_time_num_runs_dag_runs_form_data(request, session, dag):
    ...
    DR = models.DagRun
    drs = session.query(DR).filter(DR.dag_id == dag.dag_id, DR.execution_date <= base_date).order_by(desc(DR.execution_date)).limit(num_runs).all()
    dr_choices = []
    dr_state = None
    for dr in drs:
    ...
    if not dr_state:
        if drs:
            ...
            dr_state = dr.state
    return {'dttm':dttm,  'base_date':base_date, 
     'num_runs':num_runs, 
...
def circles(error):
    return (
     render_template('airflow/circles.html',
       hostname=(socket.getfqdn())), 404)
...
def show_traceback(error):
    ...
    return (
     render_template('airflow/traceback.html',
...
class AirflowBaseView(BaseView):
    route_base = ''
    extra_args = {'macros': airflow.macros}

    def render_template(self, *args, **kwargs):
        return (super(AirflowBaseView, self).render_template)(args, scheduler_job=lazy_object_proxy.Proxy(jobs.SchedulerJob.most_recent_job), **kwargs)
...
class Airflow(AirflowBaseView):
    ...
    def health(self, session=None):
        ...
        payload['scheduler'] = {'status':scheduler_status,  'latest_scheduler_heartbeat':latest_scheduler_heartbeat}
    ...
    def index(self, session=None):
        ...
        if show_paused_arg.strip().lower() == 'false':
            ...
            if show_paused_arg.strip().lower() == 'true':
                hide_paused = False
            else:
        ...
        if arg_search_query:
            query = query.filter(DagModel.dag_id.ilike('%' + arg_search_query + '%') | DagModel.owners.ilike('%' + arg_search_query + '%'))
        import_errors = session.query(errors.ImportError).all()
        for ie in import_errors:
            flash('Broken DAG: [{ie.filename}] {ie.stacktrace}'.format(ie=ie), 'dag_import_error')

        from airflow.plugins_manager import import_errors as plugin_import_errors
        for filename, stacktrace in plugin_import_errors.items():
            flash('Broken plugin: [{filename}] {stacktrace}'.format(stacktrace=stacktrace,
        ...
        return self.render_template('airflow/dags.html',
          dags=dags,
          ...
          paging=wwwutils.generate_pages(current_page, num_of_pages, search=arg_search_query,
            ...
            for dag_id in filter_dag_ids:
                if 'all_dags' in filter_dag_ids or dag_id in filter_dag_ids:
                    ...
                    for state in State.dag_states:
                        ...
                        payload[dag_id].append({'state':state, 
                         'count':count, 
                         'dag_id':dag_id, 
    ...
    def task_stats(self, session=None):
        ...
        if not filter_dag_ids:
            ...
            LastDagRun = session.query(DagRun.dag_id, sqla.func.max(DagRun.execution_date).label('execution_date')).join(Dag, Dag.dag_id == DagRun.dag_id).filter(DagRun.state != State.RUNNING).filter(Dag.is_active == True).group_by(DagRun.dag_id).subquery('last_dag_run')
            RunningDagRun = session.query(DagRun.dag_id, DagRun.execution_date).join(Dag, Dag.dag_id == DagRun.dag_id).filter(DagRun.state == State.RUNNING).filter(Dag.is_active == True).subquery('running_dag_run')
            LastTI = session.query(TI.dag_id.label('dag_id'), TI.state.label('state')).join(LastDagRun, and_(LastDagRun.c.dag_id == TI.dag_id, LastDagRun.c.execution_date == TI.execution_date))
            RunningTI = session.query(TI.dag_id.label('dag_id'), TI.state.label('state')).join(RunningDagRun, and_(RunningDagRun.c.dag_id == TI.dag_id, RunningDagRun.c.execution_date == TI.execution_date))
            UnionTI = union_all(LastTI, RunningTI).alias('union_ti')
            qry = session.query(UnionTI.c.dag_id, UnionTI.c.state, sqla.func.count()).group_by(UnionTI.c.dag_id, UnionTI.c.state)
            data = {}
    ...
    def code(self, session=None):
        ...
        try:
            with wwwutils.open_maybe_zipped(dag.fileloc, 'r') as (f):
                code = f.read()
            html_code = highlight(code, lexers.PythonLexer(), HtmlFormatter(linenos=True))
        except IOError as e:
            html_code = str(e)

        return self.render_template('airflow/dag_code.html',
          html_code=html_code, dag=dag, title=dag_id, root=(request.args.get('root')),
    ...
    def dag_details(self, session=None):
        ...
        TI = models.TaskInstance
        states = session.query(TI.state, sqla.func.count(TI.dag_id)).filter(TI.dag_id == dag_id).group_by(TI.state).all()
        active_runs = models.DagRun.find(dag_id=dag_id,
          state=(State.RUNNING),
          external_trigger=False)
        return self.render_template('airflow/dag_details.html',
          dag=dag,
          title=title,
          root=root,
          states=states,
        ...
        if not filter_dag_ids:
            ...
            for dag in dags:
                if 'all_dags' in filter_dag_ids or dag.dag_id in filter_dag_ids:
                    d[dag.dag_id] = dag.is_subdag or dag.pickle_info()
    ...
    def rendered(self):
        ...
        for template_field in task.__class__.template_fields:
            ...
            if template_field in wwwutils.get_attr_renderer():
                ...
                html_dict[template_field] = '<pre><code>' + str(content) + '</pre></code>'

        return self.render_template('airflow/ti_code.html',
          html_dict=html_dict,
          dag=dag,
          task_id=task_id,
    ...
    def get_logs_with_metadata(self, session=None):
        ...
        try:
            ...
            ti = session.query(models.TaskInstance).filter(models.TaskInstance.dag_id == dag_id, models.TaskInstance.task_id == task_id, models.TaskInstance.execution_date == dttm).first()
            ...
            try:
                ...
                attachment_filename = render_log_filename(ti=ti,
                ...
                return Response((_generate_log_stream(try_number, metadata)), mimetype='text/plain',
                  headers={'Content-Disposition': 'attachment; filename={}'.format(attachment_filename)})
    ...
    def log(self, session=None):
        ...
        logs = [
         ''] * num_logs
        root = request.args.get('root', '')
        return self.render_template('airflow/ti_log.html',
          logs=logs,
          dag=dag,
    ...
    def elasticsearch(self, session=None):
        ...
        log_id = log_id_template.format(dag_id=dag_id,
          task_id=task_id,
          execution_date=execution_date,
        ...
        if not dag or task_id not in dag.task_ids:
            ...
            for attr_name in dir(task):
                if not attr_name.startswith('_'):
                    ...
                    if type(attr) != type(self.task) and attr_name not in wwwutils.get_attr_renderer():
            ...
            no_failed_deps_result = [
             ('Unknown',
              'All dependencies are met but the task instance is not running. In most cases this just means that the task will probably be scheduled soon unless:<br/>\n- The scheduler is down or under heavy load<br/>\n{}\n<br/>\nIf this task instance does not start soon please contact your Airflow administrator for assistance.'.format("- This task instance already ran and had it's state changed manually (e.g. cleared in the UI)<br/>" if ti.state == State.NONE else ''))]
            dep_context = DepContext(SCHEDULER_DEPS)
            failed_dep_reasons = [(dep.dep_name, dep.reason) for dep in ti.get_failed_dep_statuses(dep_context=dep_context)]
            title = 'Task Instance Details'
            return self.render_template('airflow/task.html',
    ...
    def xcom(self, session=None):
        ...
        if not ti:
            ...
            title = 'XCom'
            return self.render_template('airflow/xcom.html',
    ...
    def run(self):
        ...
        if not valid_celery_config and not valid_kubernetes_config:
            ...
            ti.refresh_from_db()
            dep_context = DepContext(deps=QUEUE_DEPS,
              ignore_all_deps=ignore_all_deps,
              ignore_task_deps=ignore_task_deps,
              ignore_ti_state=ignore_ti_state)
            failed_deps = list(ti.get_failed_dep_statuses(dep_context=dep_context))
            if failed_deps:
                failed_deps_str = ', '.join(['{}: {}'.format(dep.dep_name, dep.reason) for dep in failed_deps])
                flash('Could not queue task instance for execution, dependencies not met: {}'.format(failed_deps_str), 'error')
                return redirect(origin)
            executor.start()
    ...
    def _clear_dag_tis(self, dag, start_date, end_date, origin, recursive=False, confirmed=False, only_failed=False):
        if confirmed:
            count = dag.clear(start_date=start_date,
              end_date=end_date,
            ...
            if not tis:
                ...
                response = self.render_template('airflow/confirm.html',
    ...
    def clear(self):
        ...
        dag = dag.sub_dag(task_regex=('^{0}$'.format(task_id)),
          include_downstream=downstream,
          include_upstream=upstream)
        end_date = execution_date if not future else None
        start_date = execution_date if not past else None
        return self._clear_dag_tis(dag, start_date, end_date, origin, recursive=recursive,
          confirmed=confirmed,
          only_failed=only_failed)
    ...
    def dagrun_clear(self):
        ...
        return self._clear_dag_tis(dag, start_date, end_date, origin, recursive=True,
          confirmed=confirmed)
    ...
    def blocked(self, session=None):
        ...
        payload = []
        if filter_dag_ids:
            dags = session.query(DR.dag_id, sqla.func.count(DR.id)).filter(DR.state == State.RUNNING).group_by(DR.dag_id)
            if 'all_dags' not in filter_dag_ids:
    ...
    def _mark_task_instance_state(self, dag_id, task_id, origin, execution_date, confirmed, upstream, downstream, future, past, state):
        dag = dagbag.get_dag(dag_id)
        task = dag.get_task(task_id)
        task.dag = dag
        execution_date = pendulum.parse(execution_date)
        ...
        if not task:
            ...
            if confirmed:
                altered = set_state(tasks=[task], execution_date=execution_date, upstream=upstream,
                ...
                return redirect(origin)
            to_be_altered = set_state(tasks=[task], execution_date=execution_date, upstream=upstream,
              downstream=downstream,
              future=future,
              past=past,
              state=state,
    ...
    def tree(self, session=None):
        ...
        if dag_id not in dagbag.dags:
            ...
            root = request.args.get('root')
            if root:
                dag = dag.sub_dag(task_regex=root,
            ...
            DR = models.DagRun
            dag_runs = session.query(DR).filter(DR.dag_id == dag.dag_id, DR.execution_date <= base_date).order_by(DR.execution_date.desc()).limit(num_runs).all()
            ...
            def recurse_nodes(task, visited):
                visited.add(task)
                node_count[0] += 1
                children = [recurse_nodes(t, visited) for t in task.upstream_list if node_count[0] < node_limit or t not in visited]
                ...
                def set_duration(tid):
                    if isinstance(tid, dict):
                        if tid.get('state') == State.RUNNING:
                ...
                return {'name': task.task_id, 
                 'instances': [set_duration(task_instances.get((task.task_id, d))) or {'execution_date':d.isoformat(),  'task_id':task.task_id} for d in dates], 
            ...
            return self.render_template('airflow/tree.html',
    ...
    def graph(self, session=None):
        ...
        if dag_id not in dagbag.dags:
            ...
            class GraphForm(DateTimeWithNumRunsWithDagRunsForm):
                arrange = SelectField('Layout', choices=(('LR', 'Left->Right'), ('RL', 'Right->Left'),
                                                         ('TB', 'Top->Bottom'), ('BT', 'Bottom->Top')))

            form = GraphForm(data=dt_nr_dr_data)
            form.execution_date.choices = dt_nr_dr_data['dr_choices']
            task_instances = {ti.task_id:alchemy_to_dict(ti) for ti in dag.get_task_instances(dttm, dttm, session=session)}
            tasks = {t.task_id:{'dag_id':t.dag_id,  'task_type':t.task_type,  'extra_links':t.extra_links} for t in dag.tasks}
            if not tasks:
                flash('No tasks found', 'error')
            session.commit()
            doc_md = markdown.markdown(dag.doc_md) if (hasattr(dag, 'doc_md') and dag.doc_md) else ''
            external_logs = conf.get('elasticsearch', 'frontend')
            return self.render_template('airflow/graph.html',
              dag=dag,
              form=form,
    ...
    def duration(self, session=None):
        ...
        if dag is None:
            ...
            chart = nvd3.lineChart(name='lineChart',
              x_is_date=True,
              height=chart_height,
              width='1200')
            cum_chart = nvd3.lineChart(name='cumLineChart',
              x_is_date=True,
              height=chart_height,
              width='1200')
            y = defaultdict(list)
            ...
            tis = dag.get_task_instances(start_date=min_date,
              ...
              session=session)
            TF = TaskFail
            ti_fails = session.query(TF).filter(TF.dag_id == dag.dag_id, TF.execution_date >= min_date, TF.execution_date <= base_date, TF.task_id.in_([t.task_id for t in dag.tasks])).all()
            fails_totals = defaultdict(int)
            for tf in ti_fails:
                dict_key = (
                 tf.dag_id, tf.task_id, tf.execution_date)
                if tf.duration:
            ...
            chart.create_y_axis('yAxis', format='.02f', custom_format=False, label=('Duration ({})'.format(y_unit)))
            chart.axislist['yAxis']['axisLabelDistance'] = '40'
            cum_chart.create_y_axis('yAxis', format='.02f', custom_format=False, label=('Duration ({})'.format(cum_y_unit)))
            ...
            for task in dag.tasks:
                if x[task.task_id]:
                    ...
                    cum_chart.add_serie(name=(task.task_id), x=(x[task.task_id]), y=(scale_time_units(cum_y[task.task_id], cum_y_unit)))
            ...
            cum_chart.htmlcontent = cum_chart.htmlcontent[:s_index] + "$( document ).trigger('chartload')" + cum_chart.htmlcontent[s_index:]
            return self.render_template('airflow/duration_chart.html',
              dag=dag,
              demo_mode=(conf.getboolean('webserver', 'demo_mode')),
              root=root,
              form=form,
    ...
    def tries(self, session=None):
        ...
        form = DateTimeWithNumRunsForm(data={'base_date':max_date,  'num_runs':num_runs})
        chart.buildcontent()
        return self.render_template('airflow/chart.html',
          dag=dag,
          demo_mode=(conf.getboolean('webserver', 'demo_mode')),
          root=root,
          form=form,
    ...
    def landing_times(self, session=None):
        ...
        for task in dag.tasks:
            y[task.task_id] = []
            x[task.task_id] = []
            for ti in task.get_task_instances(start_date=min_date, end_date=base_date, session=session):
    ...
    def gantt(self, session=None):
        ...
        form.execution_date.choices = dt_nr_dr_data['dr_choices']
        tis = [ti for ti in dag.get_task_instances(dttm, dttm, session=session) if ti.start_date if ti.state]
        tis = sorted(tis, key=(lambda ti: ti.start_date))
        TF = TaskFail
        ti_fails = list((itertools.chain)(*[session.query(TF).filter(TF.dag_id == ti.dag_id, TF.task_id == ti.task_id, TF.execution_date == ti.execution_date).all() for ti in tis]))
        gantt_bar_items = []
        for ti in tis:
        ...
        for gantt_bar_item in gantt_bar_items:
            ...
            tasks.append({'startDate':wwwutils.epoch(start_date), 
        ...
        data = {'taskNames':[ti.task_id for ti in tis], 
         'tasks':tasks, 
         'taskStatus':states, 
         'height':len(tis) * 25 + 25}
        session.commit()
        return self.render_template('airflow/gantt.html',
          dag=dag,
          execution_date=(dttm.isoformat()),
          form=form,
          data=data,
    ...
    def extra_links(self):
        ...
        if not dag or task_id not in dag.task_ids:
            response = jsonify({'url':None, 
             'error':"can't find dag {dag} or task_id {task_id}".format(dag=dag,
               task_id=task_id)})
            ...
            if url:
                ...
                return response
            response = jsonify({'url':None, 
             'error':'No URL found for {dest}'.format(dest=link_name)})
            response.status_code = 404
            return response
...
class VersionView(AirflowBaseView):
    ...
    def version(self):
        ...
        title = 'Version Info'
        return self.render_template('airflow/version.html',
          title=title,
    ...
    def conf(self):
        ...
        if conf.getboolean('webserver', 'expose_config'):
            with open(conf.AIRFLOW_CONFIG, 'r') as (f):
                config = f.read()
            table = [(section, key, value, source) for section, parameters in conf.as_dict(True, True).items() for key, (value, source) in parameters.items()]
        else:
            config = '# Your Airflow administrator chose not to expose the configuration, most likely for security reasons.'
            table = None
        if raw:
            ...
            code_html = Markup(highlight(config, lexers.IniLexer(), HtmlFormatter(noclasses=True)))
            return self.render_template('airflow/config.html',
...
class SlaMissModelView(AirflowModelView):
    ...
    formatters_columns = {'task_id':wwwutils.task_instance_link, 
...
class XComModelView(AirflowModelView):
    ...
    base_permissions = [
     'can_add', 'can_list', 'can_edit', 'can_delete']
    search_columns = [
     'key', 'value', 'timestamp', 'execution_date', 'task_id', 'dag_id']
...
class ConnectionModelView(AirflowModelView):
    ...
    def prefill_form(self, form, pk):
        ...
        if not hasattr(d, 'get'):
            logging.warning('extra field for {} is not iterable'.format(form.data.get('conn_id', '<unknown>')))
            return
...
class PoolModelView(AirflowModelView):
    ...
    base_permissions = [
     'can_add', 'can_list', 'can_edit', 'can_delete']
    list_columns = [
     'pool', 'slots', 'used_slots', 'queued_slots']
    ...
    formatters_columns = {'pool':pool_link, 
     'used_slots':fused_slots, 
     'queued_slots':fqueued_slots}
    validators_columns = {'pool':[
...
class VariableModelView(AirflowModelView):
    ...
    base_permissions = [
     'can_add', 'can_list', 'can_edit', 'can_delete', 'can_varimport']
    list_columns = [
     'key', 'val', 'is_encrypted']
    add_columns = ['key', 'val']
    ...
    formatters_columns = {'val': hidden_field_formatter}
...
class JobModelView(AirflowModelView):
    ...
    base_filters = [
     [
      'dag_id', DagFilter, lambda : []]]
    formatters_columns = {'start_date':wwwutils.datetime_f('start_date'), 
...
class DagRunModelView(AirflowModelView):
    ...
    formatters_columns = {'execution_date':wwwutils.datetime_f('execution_date'), 
     'state':wwwutils.state_f, 
    ...
    def action_set_running(self, drs, session=None):
        try:
            ...
            count = 0
            dirty_ids = []
            for dr in session.query(DR).filter(DR.id.in_([dagrun.id for dagrun in drs])).all():
    ...
    @action('set_failed', "Set state to 'failed'", 'All running task instances would also be marked as failed, are you sure?',
      single=False)
    @provide_session
    def action_set_failed(self, drs, session=None):
        try:
            ...
            for dr in session.query(DR).filter(DR.id.in_([dagrun.id for dagrun in drs])).all():
                dirty_ids.append(dr.dag_id)
                count += 1
                altered_tis += set_dag_run_state_to_failed((dagbag.get_dag(dr.dag_id)), (dr.execution_date),
                  commit=True,
                  session=session)

            altered_ti_count = len(altered_tis)
            flash('{count} dag runs and {altered_ti_count} task instances were set to failed'.format(count=count,
              altered_ti_count=altered_ti_count))
        except Exception:
            flash('Failed to set state', 'error')

        return redirect(self.get_default_url())

    @action('set_success', "Set state to 'success'", 'All task instances would also be marked as success, are you sure?',
      single=False)
    @provide_session
    def action_set_success(self, drs, session=None):
        try:
            ...
            for dr in session.query(DR).filter(DR.id.in_([dagrun.id for dagrun in drs])).all():
                dirty_ids.append(dr.dag_id)
                count += 1
                altered_tis += set_dag_run_state_to_success((dagbag.get_dag(dr.dag_id)), (dr.execution_date),
                  commit=True,
                  session=session)

            altered_ti_count = len(altered_tis)
            flash('{count} dag runs and {altered_ti_count} task instances were set to success'.format(count=count,
              altered_ti_count=altered_ti_count))
        except Exception:
            flash('Failed to set state', 'error')
...
class LogModelView(AirflowModelView):
    ...
    base_permissions = [
     'can_list']
    list_columns = [
     'id', 'dttm', 'dag_id', 'task_id', 'event', 'execution_date',
     'owner', 'extra']
    ...
    base_filters = [
     [
      'dag_id', DagFilter, lambda : []]]
    formatters_columns = {'dttm':wwwutils.datetime_f('dttm'), 
     'execution_date':wwwutils.datetime_f('execution_date'), 
     'dag_id':wwwutils.dag_link}
...
class TaskInstanceModelView(AirflowModelView):
    ...
    def log_url_formatter(attr):
        ...
        return Markup('<a href="{log_url}">    <span class="glyphicon glyphicon-book" aria-hidden="true"></span></a>').format(log_url=log_url)
    ...
    formatters_columns = {'log_url':log_url_formatter,  'task_id':wwwutils.task_instance_link, 
     'hostname':wwwutils.nobr_f('hostname'), 
     'state':wwwutils.state_f, 
     'execution_date':wwwutils.datetime_f('execution_date'), 
     'start_date':wwwutils.datetime_f('start_date'), 
    ...
    @provide_session
    @action('clear', (lazy_gettext('Clear')), (lazy_gettext('Are you sure you want to clear the state of the selected task instance(s) and set their dagruns to the running state?')),
      single=False)
    ...
    def set_task_instance_state(self, tis, target_state, session=None):
        try:
            ...
            session.commit()
            flash("{count} task instances were set to '{target_state}'".format(count=count,
...
class DagModelView(AirflowModelView):
    ...
    def get_query(self):
        return super(DagModelView, self).get_query().filter(or_(models.DagModel.is_active, models.DagModel.is_paused)).filter(~models.DagModel.is_subdag)

    def get_count_query(self):
        return super(DagModelView, self).get_count_query().filter(models.DagModel.is_active).filter(~models.DagModel.is_subdag)