...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/contrib/hooks/datadog_hook.py
...
class DatadogHook(BaseHook, LoggingMixin):
    ...
    def query_metric(self, query, from_seconds_ago, to_seconds_ago):
        ...
        """
        now = int(time.time())
        response = api.Metric.query(start=(now - from_seconds_ago),
    ...
    def post_event(self, title, text, aggregation_key=None, alert_type=None, date_happened=None, handle=None, priority=None, related_event_id=None, tags=None, device_name=None):
        ...
        """
        response = api.Event.create(title=title,
          text=text,
