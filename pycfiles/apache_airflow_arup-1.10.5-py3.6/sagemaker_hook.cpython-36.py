...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/contrib/hooks/sagemaker_hook.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 31287 bytes
import tarfile, tempfile, time, os, collections, warnings
...
def argmin(arr, f):
    ...
    for idx, item in enumerate(arr):
        if item is not None and (m is None or f(item) < m):
            m = f(item)
            i = idx
...
class SageMakerHook(AwsHook):
    ...
    def check_s3_url(self, s3url):
        ...
        if key:
            if not self.s3_hook.check_for_key(key=key, bucket_name=bucket):
                if not self.s3_hook.check_for_prefix(prefix=key,
                  bucket_name=bucket,
                  delimiter='/'):
    ...
    def get_log_conn(self):
        ...
        """
        warnings.warn('Method `get_log_conn` has been deprecated. Please use `airflow.contrib.hooks.AwsLogsHook.get_conn` instead.', category=DeprecationWarning,
          stacklevel=2)
    ...
    def log_stream(self, log_group, stream_name, start_time=0, skip=0):
        ...
        """
        warnings.warn('Method `log_stream` has been deprecated. Please use `airflow.contrib.hooks.AwsLogsHook.get_log_events` instead.', category=DeprecationWarning,
          stacklevel=2)
    ...
    def multi_stream_iter(self, log_group, streams, positions=None):
        ...
        event_iters = [self.logs_hook.get_log_events(log_group, s, positions[s].timestamp, positions[s].skip) for s in streams]
        events = []
    ...
    def create_transform_job(self, config, wait_for_completion=True, check_interval=30, max_ingestion_time=None):
        """
    ...
    def create_endpoint(self, config, wait_for_completion=True, check_interval=30, max_ingestion_time=None):
        ...
        if wait_for_completion:
            self.check_status((config['EndpointName']), 'EndpointStatus',
              (self.describe_endpoint),
              check_interval,
              max_ingestion_time, non_terminal_states=(self.endpoint_non_terminal_states))
        return response
    ...
    def describe_training_job_with_log(self, job_name, positions, stream_names, instance_count, state, last_description, last_describe_job_call):
        ...
        if len(stream_names) < instance_count:
            ...
            try:
                streams = logs_conn.describe_log_streams(logGroupName=log_group,
    ...
    def check_status(self, job_name, key, describe_function, check_interval, max_ingestion_time, non_terminal_states=None):
        ...
        while running:
            ...
            try:
                ...
                status = response[key]
                self.log.info('Job still running for %s seconds... current status is %s' % (
                 sec, status))
            except KeyError:
