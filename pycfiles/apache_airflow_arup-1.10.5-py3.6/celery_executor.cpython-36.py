...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/executors/celery_executor.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 11087 bytes
import math, os, subprocess, time, traceback
...
if configuration.conf.has_option('celery', 'celery_config_options'):
    celery_configuration = import_string(configuration.conf.get('celery', 'celery_config_options'))
else:
...
def execute_command(command_to_exec):
    ...
    env = os.environ.copy()
    try:
        subprocess.check_call(command_to_exec, stderr=(subprocess.STDOUT), close_fds=True,
          env=env)
...
def fetch_celery_task_state(celery_task):
    ...
    try:
        with timeout(seconds=2):
            res = (
             celery_task[0], celery_task[1].state)
...
class CeleryExecutor(BaseExecutor):
    ...
    def sync(self):
        ...
        if num_processes == 0:
            ...
            return
        self.log.debug('Inquiring about %s celery task(s) using %s processes', len(self.tasks), num_processes)
