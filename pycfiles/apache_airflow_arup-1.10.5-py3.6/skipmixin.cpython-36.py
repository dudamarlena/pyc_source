...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/models/skipmixin.py
...
class SkipMixin(LoggingMixin):
    ...
    def skip(self, dag_run, execution_date, tasks, session=None):
        ...
        now = timezone.utcnow()
        if dag_run:
            session.query(TaskInstance).filter(TaskInstance.dag_id == dag_run.dag_id, TaskInstance.execution_date == dag_run.execution_date, TaskInstance.task_id.in_(task_ids)).update({TaskInstance.state: State.SKIPPED, 
    ...
    def skip_all_except(self, ti, branch_task_ids):
        ...
        if downstream_tasks:
            ...
            for b in branch_task_ids:
                branch_downstream_task_ids.update(dag.get_task(b).get_flat_relative_ids(upstream=False))

            skip_tasks = [t for t in downstream_tasks if t.task_id not in branch_task_ids if t.task_id not in branch_downstream_task_ids]
