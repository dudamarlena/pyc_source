...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/contrib/hooks/gcp_spanner_hook.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 15870 bytes
...
class CloudSpannerHook(GoogleCloudBaseHook):
    ...
    @GoogleCloudBaseHook.fallback_to_default_project_id
    def create_instance(self, instance_id, configuration_name, node_count, display_name, project_id=None):
        """
    ...
    @GoogleCloudBaseHook.fallback_to_default_project_id
    def update_instance(self, instance_id, configuration_name, node_count, display_name, project_id=None):
        """
    ...
    def update_database(self, instance_id, database_id, ddl_statements, project_id=None, operation_id=None):
        ...
        database = instance.database(database_id=database_id)
        try:
            operation = database.update_ddl(ddl_statements=ddl_statements,
    ...
    def delete_database(self, instance_id, database_id, project_id=None):
        ...
        database = instance.database(database_id=database_id)
        if not database.exists():
            self.log.info('The database {} is already deleted from instance {}. Exiting.'.format(database_id, instance_id))
            return
        try:
    ...
    def execute_dml(self, instance_id, database_id, queries, project_id=None):
        ...
        :type project_id: str
        """
        self._get_client(project_id=project_id).instance(instance_id=instance_id).database(database_id=database_id).run_in_transaction(lambda transaction: self._execute_sql_in_transaction(transaction, queries))
