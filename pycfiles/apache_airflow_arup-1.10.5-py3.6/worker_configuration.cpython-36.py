...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/contrib/kubernetes/worker_configuration.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 15461 bytes
import os, six
...
class WorkerConfiguration(LoggingMixin):
    ...
    def _get_init_containers(self):
        ...
        if self.kube_config.dags_volume_claim or self.kube_config.dags_volume_host or self.kube_config.dags_in_image:
            ...
            init_environment = [
             {'name':'GIT_SYNC_REPO',  'value':self.kube_config.git_repo},
            ...
            if self.kube_config.git_ssh_known_hosts_configmap_name:
                volume_mounts.append({'name':self.git_sync_ssh_known_hosts_volume_name, 
                ...
                init_environment.append({'name':'GIT_KNOWN_HOSTS', 
                 'value':'false'})
            init_containers = [
            ...
            if self.kube_config.git_sync_run_as_user != '':
                init_containers[0]['securityContext'] = {'runAsUser': self.kube_config.git_sync_run_as_user}
    ...
    def _get_environment(self):
        ...
        if not self.kube_config.airflow_configmap:
            if 'AIRFLOW__CORE__SQL_ALCHEMY_CONN' not in self.kube_config.kube_secrets:
    ...
    def _get_security_context(self):
        ...
        if self.kube_config.git_ssh_key_secret_name:
            if security_context.get('fsGroup') is None:
    ...
    def _get_volumes_and_mounts(self):
        ...
        def _construct_volume(name, claim, host):
            ...
            if claim:
                ...
                if host:
                    volume['hostPath'] = {'path':host,  'type':''}
                else:
                    volume['emptyDir'] = {}
            return volume

        volumes = {self.dags_volume_name: _construct_volume(self.dags_volume_name, self.kube_config.dags_volume_claim, self.kube_config.dags_volume_host), 
         
         self.logs_volume_name: _construct_volume(self.logs_volume_name, self.kube_config.logs_volume_claim, self.kube_config.logs_volume_host)}
        volume_mounts = {self.dags_volume_name: {'name':self.dags_volume_name, 
                                 'mountPath':self.generate_dag_volume_mount_path(), 
                                 'readOnly':True}, 
         
         self.logs_volume_name: {'name':self.logs_volume_name, 
        ...
        if self.kube_config.git_ssh_key_secret_name:
            volumes[self.git_sync_ssh_secret_volume_name] = {'name':self.git_sync_ssh_secret_volume_name,  'secret':{'secretName':self.kube_config.git_ssh_key_secret_name, 
              'items':[
               {'key':self.git_ssh_key_secret_key, 
                'path':'ssh', 
                'mode':288}]}}
        if self.kube_config.git_ssh_known_hosts_configmap_name:
            volumes[self.git_sync_ssh_known_hosts_volume_name] = {'name':self.git_sync_ssh_known_hosts_volume_name,  'configMap':{'name': self.kube_config.git_ssh_known_hosts_configmap_name}, 
             'mode':288}
        if self.kube_config.airflow_configmap:
            config_volume_name = 'airflow-config'
            config_path = '{}/airflow.cfg'.format(self.worker_airflow_home)
            volumes[config_volume_name] = {'name':config_volume_name, 
             'configMap':{'name': self.kube_config.airflow_configmap}}
            volume_mounts[config_volume_name] = {'name':config_volume_name, 
    ...
    def make_pod(self, namespace, worker_uuid, pod_id, dag_id, task_id, execution_date, try_number, airflow_command, kube_executor_config):
        volumes_dict, volume_mounts_dict = self._get_volumes_and_mounts()
        worker_init_container_spec = self._get_init_containers()
        resources = Resources(request_memory=(kube_executor_config.request_memory),
        ...
        return Pod(namespace=namespace,
          ...
          image_pull_policy=(kube_executor_config.image_pull_policy or self.kube_config.kube_image_pull_policy),
          cmds=airflow_command,
          labels=(self._get_labels(kube_executor_config.labels, {'airflow-worker':worker_uuid, 
         'dag_id':dag_id, 
         'task_id':task_id, 
         ...
         'try_number':str(try_number)})),
          ...
          node_selectors=(kube_executor_config.node_selectors or self.kube_config.kube_node_selectors),
          affinity=affinity,
