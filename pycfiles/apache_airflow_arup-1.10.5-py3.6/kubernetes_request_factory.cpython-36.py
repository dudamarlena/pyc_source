...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/contrib/kubernetes/kubernetes_request_factory/kubernetes_request_factory.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 9073 bytes
...
class KubernetesRequestFactory:
    ...
    def add_secret_to_env(env, secret):
        env.append({'name':secret.deploy_target, 
         'valueFrom':{'secretKeyRef': {'name':secret.secret, 
                           'key':secret.key}}})

    @staticmethod
    def add_runtime_info_env(env, runtime_info):
        env.append({'name':runtime_info.name, 
         'valueFrom':{'fieldRef': {'fieldPath': runtime_info.field_path}}})
    ...
    def attach_ports(pod, req):
        req['spec']['containers'][0]['ports'] = req['spec']['containers'][0].get('ports', [])
        if len(pod.ports) > 0:
    ...
    def extract_volume_secrets(pod, req):
        ...
        for idx, vol in enumerate(vol_secrets):
            ...
            req['spec']['containers'][0]['volumeMounts'].append({'mountPath':vol.deploy_target, 
             'name':vol_id, 
             'readOnly':True})
            req['spec']['volumes'].append({'name':vol_id, 
             'secret':{'secretName': vol.secret}})

    @staticmethod
    def extract_env_and_secrets(pod, req):
        envs_from_key_secrets = [env for env in pod.secrets if env.deploy_type == 'env' if env.key is not None]
        ...
        if not pod.resources or pod.resources.is_empty_resource_request():
            ...
            if pod.resources.has_limits():
                ...
                if pod.resources.limit_gpu:
                    req['spec']['containers'][0]['resources']['limits']['nvidia.com/gpu'] = pod.resources.limit_gpu
    ...
    def extract_image_pull_secrets(pod, req):
        if pod.image_pull_secrets:
            req['spec']['imagePullSecrets'] = [{'name': pull_secret} for pull_secret in pod.image_pull_secrets.split(',')]
    ...
    def _apply_env_from(pod, req):
        envs_from_secrets = [env for env in pod.secrets if env.deploy_type == 'env' if env.key is None]
        if pod.configmaps or envs_from_secrets:
            req['spec']['containers'][0]['envFrom'] = []
        for secret in envs_from_secrets:
            req['spec']['containers'][0]['envFrom'].append({'secretRef': {'name': secret.secret}})

        for configmap in pod.configmaps:
            req['spec']['containers'][0]['envFrom'].append({'configMapRef': {'name': configmap}})