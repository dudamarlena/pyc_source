...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/jobs/scheduler_job.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 67942 bytes
from __future__ import absolute_import
from __future__ import division
from __future__ import print_function
from __future__ import unicode_literals
import logging, multiprocessing, os, signal, sys, threading, time
...
from airflow.utils.dag_processing import AbstractDagFileProcessor, DagFileProcessorAgent, SimpleDag, SimpleDagBag, SimpleTaskInstance, list_py_file_paths
...
class DagFileProcessor(AbstractDagFileProcessor, LoggingMixin):
    ...
    @staticmethod
    def _run_file_processor(result_channel, file_path, pickle_dags, dag_id_white_list, thread_name):
        """
    ...
    def start(self):
        ...
        self._process = multiprocessing.Process(target=(type(self)._run_file_processor),
          args=(
         _child_channel,
         self.file_path,
         self._pickle_dags,
...
class SchedulerJob(BaseJob):
    ...
    def __init__(self, dag_id=None, dag_ids=None, subdir=settings.DAGS_FOLDER, num_runs=-1, processor_poll_interval=1.0, run_duration=None, do_pickle=False, log=None, *args, **kwargs):
        """
        :param dag_id: if specified, only schedule tasks with this DAG ID
        :type dag_id: unicode
        :param dag_ids: if specified, only schedule tasks with these DAG IDs
    ...
    def is_alive(self, grace_multiplier=None):
        ...
        if grace_multiplier is not None:
            ...
            return self.state == State.RUNNING and (timezone.utcnow() - self.latest_heartbeat).seconds < scheduler_health_check_threshold
    ...
    def manage_slas(self, dag, session=None):
        ...
        if not any([isinstance(ti.sla, timedelta) for ti in dag.tasks]):
            ...
            sq = session.query(TI.task_id, func.max(TI.execution_date).label('max_ti')).with_hint(TI,
              'USE INDEX (PRIMARY)', dialect_name='mysql').filter(TI.dag_id == dag.dag_id).filter(or_(TI.state == State.SUCCESS, TI.state == State.SKIPPED)).filter(TI.task_id.in_(dag.task_ids)).group_by(TI.task_id).subquery('sq')
                ...
                if isinstance(task.sla, timedelta):
                    ...
                    while dttm < timezone.utcnow():
                        ...
                        if following_schedule + task.sla < timezone.utcnow():
                            session.merge(SlaMiss(task_id=(ti.task_id),
                              dag_id=(ti.dag_id),
                              execution_date=dttm,
                              timestamp=ts))
                        dttm = dag.following_schedule(dttm)
    ...
    def update_import_errors(session, dagbag):
        ...
        for filename, stacktrace in six.iteritems(dagbag.import_errors):
            session.add(errors.ImportError(filename=filename,
    ...
    def create_dag_run(self, dag, session=None):
        ...
        if dag.schedule_interval and conf.getboolean('scheduler', 'USE_JOB_SCHEDULE'):
            active_runs = DagRun.find(dag_id=(dag.dag_id),
              state=(State.RUNNING),
              external_trigger=False,
              session=session)
            if len(active_runs) >= dag.max_active_runs:
                if not dag.dagrun_timeout:
                    return
            timedout_runs = 0
                ...
                if last_scheduled_run:
                    ...
                    if not (dag.catchup or dag.schedule_interval == '@once'):
                        ...
                        if dag.start_date:
                            ...
                            dag.start_date = new_start
                next_run_date = None
                task_start_dates = last_scheduled_run or [t.start_date for t in dag.tasks]
                if task_start_dates:
            ...
            if dag.start_date:
                ...
                self.log.debug('Dag start date: %s. Next run date: %s', dag.start_date, next_run_date)
            ...
            if dag.schedule_interval == '@once':
                ...
                if next_run_date:
                    if min_task_end_date:
                        if next_run_date > min_task_end_date:
                            return
                if next_run_date:
                    if period_end:
                        if period_end <= timezone.utcnow():
                            next_run = dag.create_dagrun(run_id=(DagRun.ID_PREFIX + next_run_date.isoformat()),
    ...
    def _process_task_instances(self, dag, task_instances_list, session=None):
        ...
        for run in active_dag_runs:
            ...
            for ti in tis:
                task = dag.get_task(ti.task_id)
                ti.task = task
                if ti.are_dependencies_met(dep_context=DepContext(flag_upstream_failed=True),
                  session=session):
    ...
    def _change_state_for_tis_without_dagrun(self, simple_dag_bag, old_states, new_state, session=None):
        ...
        if self.using_sqlite:
            ...
            subq = query.subquery()
            tis_changed = session.query(models.TaskInstance).filter(and_(models.TaskInstance.dag_id == subq.c.dag_id, models.TaskInstance.task_id == subq.c.task_id, models.TaskInstance.execution_date == subq.c.execution_date)).update({models.TaskInstance.state: new_state},
    ...
    def _find_executable_task_instances(self, simple_dag_bag, states, session=None):
        ...
        if len(task_instances_to_examine) == 0:
            ...
            dag_concurrency_map, task_concurrency_map = self._SchedulerJob__get_concurrency_maps(states=states_to_count_as_running,
              session=session)
            for pool, task_instances in pool_to_task_instances.items():
                ...
                priority_sorted_task_instances = sorted(task_instances,
                  key=(lambda ti: (-ti.priority_weight, ti.execution_date)))
                ...
                Stats.gauge('pool.open_slots.{pool_name}'.format(pool_name=pool_name), pools[pool_name].open_slots())
                Stats.gauge('pool.used_slots.{pool_name}'.format(pool_name=pool_name), pools[pool_name].occupied_slots())
    ...
    def _enqueue_task_instances_with_queued_state(self, simple_dag_bag, simple_task_instances):
        ...
        for simple_task_instance in simple_task_instances:
            ...
            command = TI.generate_command((simple_task_instance.dag_id),
            ...
            self.executor.queue_command(simple_task_instance,
              command,
              priority=priority,
              queue=queue)
    ...
    def _execute_task_instances(self, simple_dag_bag, states, session=None):
        ...
        def query(result, items):
            simple_tis_with_state_changed = self._change_state_for_executable_task_instances(items, states,
              session=session)
            ...
            if not dag:
                ...
                if dag_run:
                    ...
                    if expected_start_date:
                        ...
                        Stats.timing('dagrun.schedule_delay.{dag_id}'.format(dag_id=(dag.dag_id)), schedule_delay)
    ...
    def _process_executor_events(self, simple_dag_bag, session=None):
        ...
        for key, state in list(self.executor.get_event_buffer(simple_dag_bag.dag_ids).items()):
            ...
            if state == State.FAILED or state == State.SUCCESS:
                qry = session.query(TI).filter(TI.dag_id == dag_id, TI.task_id == task_id, TI.execution_date == execution_date)
                ti = qry.first()
                ti or self.log.warning('TaskInstance %s went missing from the database', ti)
            else:
    ...
    def _execute(self):
        ...
        def processor_factory(file_path):
            return DagFileProcessor(file_path, pickle_dags, self.dag_ids)
        ...
        try:
            ...
            if len(dagbag.dags) > 0:
                ...
                if len(self.dag_ids) > 0:
                    dags = [dag for dag in dagbag.dags.values() if dag.dag_id in self.dag_ids if dag.dag_id not in paused_dag_ids]
                else:
                    dags = [dag for dag in dagbag.dags.values() if not dag.parent_dag if dag.dag_id not in paused_dag_ids]
            ti_keys_to_schedule = []
            ...
            for ti_key in ti_keys_to_schedule:
                ...
                if ti.are_dependencies_met(dep_context=dep_context,
                  session=session,
                  verbose=True):
