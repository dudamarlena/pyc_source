...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/geoalchemy/tests/test_mysql.py
# Compiled at: 2012-03-28 03:25:19
from unittest import TestCase
from binascii import b2a_hex
from sqlalchemy import create_engine, MetaData, Column, Integer, Unicode, Numeric, func, Table
from sqlalchemy.orm import sessionmaker, mapper
from sqlalchemy.ext.declarative import declarative_base
from geoalchemy import GeometryColumn, Point, Polygon, LineString, GeometryDDL, WKTSpatialElement, DBSpatialElement, GeometryExtensionColumn, WKBSpatialElement
...
spots_table = Table('spots', metadata, Column('spot_id', Integer, primary_key=True), Column('spot_height', Numeric(precision=10, scale=2)), GeometryExtensionColumn('spot_location', Point(2, srid=4326)))
...
mapper(Spot, spots_table, properties={'spot_location': GeometryColumn(spots_table.c.spot_location, comparator=MySQLComparator)})
...
class TestGeometry(TestCase):
    ...
    def test_envelope(self):
        ...
        eq_(session.scalar(functions.wkt(functions.envelope('POINT(-88.5945861592357 42.9480095987261)'))), 'POLYGON((-88.5945861592357 42.9480095987261,-88.5945861592357 42.9480095987261,-88.5945861592357 42.9480095987261,-88.5945861592357 42.9480095987261,-88.5945861592357 42.9480095987261))')
    ...
    def test_length(self):
        ...
        r = session.query(Road).get(1)
        s = session.query(Spot).get(1)
        assert_almost_equal(session.scalar(r.road_geom.length), 0.85516941641479)
    ...
    def test_area(self):
        ...
        r = session.query(Road).get(1)
        s = session.query(Spot).get(1)
        assert_almost_equal(session.scalar(l.lake_geom.area), 0.0056748625704923)
    ...
    def test_mbr_touches(self):
        ...
        eq_(session.scalar(l1.lake_geom.mbr_touches(r.road_geom)), 0)
        eq_(session.scalar(l2.lake_geom.mbr_touches(r.road_geom)), 0)
