...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/geoalchemy/functions.py
# Compiled at: 2012-07-02 03:26:57
...
def parse_clause(clause, compiler):
    ...
    if isinstance(clause, ClauseElement):
        return clause
    if isinstance(clause, SpatialElement):
        ...
        return clause.desc
    if isinstance(clause, basestring) and WKT_REGEX.match(clause):
...
def _get_function(element, compiler, params, within_column_clause):
    ...
    if isinstance(function_data, list):
        ...
        return function
    if isinstance(function_data, types.FunctionType):
...
def __compile_base_function(element, compiler, **kw):
    ...
    if database_dialect.is_member_function(element.__class__):
        ...
        if database_dialect.is_property(element.__class__):
            geometry = params.pop(0)
            function_name = database_dialect.get_function(element.__class__)
            return '%s.%s' % (
             compiler.process(geometry),
...
def __compile__within_distance(element, compiler, **kw):
    ...
    return compiler.process(function(compiler, parse_clause(arguments.pop(0), compiler), parse_clause(arguments.pop(0), compiler), arguments.pop(0), *arguments))
