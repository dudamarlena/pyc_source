...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/geoalchemy/oracle.py
...
def ST_GeometryFunction(function, returns_geometry=False, relation_function=False, returns_boolean=False, compare_value=1, default_cast=False):
    ...
    """
    assert not (returns_geometry and returns_boolean)
    ...
    def cast_param(params):
        if len(params) > 0:
            ...
            if isinstance(geom, Column) and isinstance(geom.type, GeometryBase):
                params.pop(0)
    ...
    def function_handler(params, within_column_clause):
        if len(params) > 0:
            ...
            if geom_cast is not None:
                return check_comparison(get_function(function, geom_cast, params, returns_geometry, relation_function), within_column_clause, returns_boolean, compare_value)
...
class oracle_functions(functions):
    ...
    def _within_distance(compiler, geom1, geom2, distance, additional_params={}):
        ...
        defining the spatial filter, e.g.::
            ...
            diminfo = text("MDSYS.SDO_DIM_ARRAY("                "MDSYS.SDO_DIM_ELEMENT('LONGITUDE', -180, 180, 0.000000005),"                "MDSYS.SDO_DIM_ELEMENT('LATITUDE', -90, 90, 0.000000005)"                ")")
        ...
        if isinstance(geom1, Column):
            return func.SDO_WITHIN_DISTANCE(geom1, geom2, 'distance=%s %s' % (distance, params)) == 'TRUE'
        else:
            dim1 = additional_params.get('dim1', None)
            dim2 = additional_params.get('dim2', None)
            if dim1 is not None and dim2 is not None:
                return func.SDO_GEOM.WITHIN_DISTANCE(geom1, dim1, distance, geom2, dim2, params) == 'TRUE'
            tol = additional_params.get('tol', None)
            if tol is not None:
                return func.SDO_GEOM.WITHIN_DISTANCE(geom1, distance, geom2, tol, params) == 'TRUE'
            raise Exception('No dimension information ("dim1" and "dim2") or tolerance value ("tol") specified for calling SDO_GEOM.WITHIN_DISTANCE on Oracle, which is required when reprojecting.')
            return


class OracleSpatialDialect(SpatialDialect):
    """Implementation of SpatialDialect for Oracle."""
    __functions = {functions.wkt: [
       ...
       functions.dimension: [
                           'MDSYS.ST_GEOMETRY.ST_DIMENSION', 'MDSYS.ST_GEOMETRY'], 
       functions.srid: [
                      'MDSYS.OGC_SRID', 'MDSYS.ST_GEOMETRY'], 
       functions.geometry_type: [
                               'MDSYS.OGC_GeometryType', 'MDSYS.ST_GEOMETRY'], 
       ...
       oracle_functions.kml: [
                            'TO_CHAR', 'SDO_UTIL.TO_KMLGEOMETRY'], 
       oracle_functions.gml: [
                            'TO_CHAR', 'SDO_UTIL.TO_GMLGEOMETRY'], 
       oracle_functions.gml311: [
    ...
    def get_diminfo_select(column):
        ...
        return select([OracleSpatialDialect.METADATA_TABLE.c.diminfo]).where(and_(OracleSpatialDialect.METADATA_TABLE.c.table_name == column.table.name.upper(), OracleSpatialDialect.METADATA_TABLE.c.column_name == column.name.upper()))
    ...
    def handle_ddl_after_create(self, bind, table, column):
        ...
        if not column.type.kwargs.has_key('diminfo'):
            warnings.warn("No DIMINFO given for '%s.%s', no entry in USER_SDO_GEOM_METADATA will be made and no spatial index will be created." % (
             table.name, column.name), exc.SAWarning, stacklevel=3)
        else:
            diminfo = column.type.kwargs['diminfo']
            bind.execute('INSERT INTO USER_SDO_GEOM_METADATA (table_name, column_name, diminfo, srid) ' + "VALUES ('%s', '%s', %s, %s)" % (
             table.name, column.name, diminfo, column.type.srid))
            if column.type.spatial_index:
                bind.execute('CREATE INDEX %s_%s_sidx ON %s(%s) INDEXTYPE IS MDSYS.SPATIAL_INDEX%s' % (
    ...
    def __get_oracle_gtype(self, type):
        ...
        if isinstance(type, LineString):
            ...
            if isinstance(type, MultiLineString):
                return 'MULTILINE'
            else:
                if isinstance(type, GeometryCollection):
                    return 'COLLECTION'
