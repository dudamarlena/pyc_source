...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/geoalchemy/tests/test_mssql.py
# Compiled at: 2012-03-28 03:25:19
from sqlalchemy import create_engine, MetaData, Column, Integer, String, Numeric, func, Table, and_
...
spots_table = Table('spots', metadata, Column('spot_id', Integer, primary_key=True), Column('spot_height', Numeric(6, 2)), GeometryExtensionColumn('spot_location', Geometry(2)))
...
mapper(Spot, spots_table, properties={'spot_location': GeometryColumn(spots_table.c.spot_location, comparator=MSComparator)})
...
class TestGeometry(TestCase):
    ...
    def test_wkt(self):
        ...
        eq_(session.scalar(p.wkt), 'POINT (-88.3655256496815 43.1402866687898)')
    ...
    def test_wkb(self):
        eq_(session.scalar(WKBSpatialElement(session.scalar(self.r.road_geom.wkb)).wkt), 'LINESTRING (-88.6748409363057 43.1035032292994, -88.6464173694267 42.9981688343949, -88.607961955414 42.9680732929936, -88.5160033566879 42.9363057770701, -88.4390925286624 43.0031847579618)')
        ...
        eq_(session.scalar(WKBSpatialElement(session.scalar(centroid_geom.wkb)).wkt), 'POINT (-88.757840057564835 43.193797540630335)')
    ...
    def test_svg(self):
        ...
        eq_(session.scalar(func.svg('POINT(-88.9055734203822 43.0048567324841)')), 'cx="-88.905573420382197" cy="-43.0048567324841"')
    ...
    def test_kml(self):
        ...
        eq_(session.scalar(s.spot_location.kml), '<Point><coordinates>-88.5945861592357,42.9480095987261</coordinates></Point>')
    ...
    def test_boundary(self):
        r = session.query(Road).filter(Road.road_name == 'Graeme Ave').one()
        eq_(session.scalar(functions.wkt(r.road_geom.boundary)), 'MULTIPOINT ((-88.5912422100082 43.187101952731609), (-88.547770872712135 42.698885396122932))')
        ...
        eq_(session.scalar(functions.wkt(functions.boundary('POLYGON((1 1,0 0, -1 1, 1 1))'))), 'LINESTRING (0 0, 1 1, -1 1, 0 0)')
        return
    ...
    def test_convex_hull(self):
        r = session.query(Road).filter(Road.road_name == 'Graeme Ave').one()
        eq_(session.scalar(functions.wkt(r.road_geom.convex_hull)), 'POLYGON ((-88.547770872712135 42.698885396122932, -88.5912422100082 43.187101952731609, -88.602946031838655 43.088455460965633, -88.609633930027485 42.969745270907879, -88.547770872712135 42.698885396122932))')
        ...
        eq_(session.scalar(functions.wkt(functions.convex_hull('POINT(-88.5945861592357 42.9480095987261)'))), 'POINT (-88.594586159235689 42.948009598726117)')
        return
    ...
    def test_envelope(self):
        ...
        eq_(session.scalar(functions.wkt(functions.envelope('POINT(-88.5945861592357 42.9480095987261)'))), 'POLYGON ((-88.5945871592357 42.948008598726105, -88.5945851592357 42.948008598726105, -88.5945851592357 42.9480105987261, -88.5945871592357 42.9480105987261, -88.5945871592357 42.948008598726105))')
        return
    ...
    def test_constraint_nullable(self):
        spot_null = Spot(spot_height=420.4, spot_location=MS_SPATIAL_NULL)
    ...
    def test_text_zm(self):
        engine.execute('INSERT INTO [spots] VALUES(%f, geometry::STGeomFromText(%s, %i))' % (130.23,
                                                                                             "'POINT (-88.5945861592357 42.9480095987261 130.23 1)'",
                                                                                             4326))
        eq_(session.query(Spot.spot_location.text_zm.label('text_zm')).filter(Spot.spot_height == 130.23).first().text_zm, 'POINT (-88.5945861592357 42.9480095987261 130.23 1)')
        eq_(session.query(Spot.spot_location.text_zm.label('text_zm')).filter(Spot.spot_height == 420.4).first().text_zm, 'POINT (-88.5945861592357 42.9480095987261)')

    def test_buffer_with_tolerance(self):
        r = session.query(Road).filter(Road.road_name == 'Graeme Ave').one()
        assert_almost_equal(session.scalar(functions.area(r.road_geom.buffer_with_tolerance(10.0, 20, 1))), 214.638946687896)
        assert_almost_equal(session.scalar(functions.area(r.road_geom.buffer_with_tolerance(10.0, 20, 0))), 214.638946687896)
        ok_(session.query(Spot).filter(functions.within('POINT(-88.5945861592357 42.9480095987261)', Spot.spot_location.buffer(10))).first() is not None)
    ...
    def test_extended_coords(self):
        ...
        eq_(p.z, 130.23)
        eq_(p.m, 1)
        p = session.query(Spot.spot_location.z.label('z'), Spot.spot_location.m.label('m')).filter(Spot.spot_height == 420.4).first()
        ok_(p.z is None)
        ok_(p.m is None)
        return

    def test_make_valid(self):
        session.add(Shape(shape_name='Invalid Shape', shape_geom=WKTSpatialElement('LINESTRING(0 2, 1 1, 1 0, 1 1, 2 2)')))
        invalid_line = session.query(Shape).filter(Shape.shape_name == 'Invalid Shape').first()
        ...
        valid_line = session.query(Shape).filter(Shape.shape_name == 'Invalid Shape').first()
        eq_(session.scalar(valid_line.shape_geom.is_valid), 1)

    def test_reduce(self):
        r = session.query(Road).first()
        eq_(session.scalar(DBSpatialElement(session.scalar(r.road_geom.reduce(0.5))).wkt), 'LINESTRING (-88.9139332929936 42.5082802993631, -88.3655256496815 43.1402866687898)')
        eq_(session.scalar(DBSpatialElement(session.scalar(r.road_geom.reduce(0.05))).wkt), 'LINESTRING (-88.9139332929936 42.5082802993631, -88.6113059044586 42.9680732929936, -88.3655256496815 43.1402866687898)')
        eq_(session.scalar(DBSpatialElement(session.scalar(r.road_geom.reduce(1e-13))).wkt), session.scalar(r.road_geom.wkt))

    def test_to_string(self):
        engine.execute('INSERT INTO [spots] VALUES(%f, geometry::STGeomFromText(%s, %i))' % (130.23,
                                                                                             "'POINT (-88.5945861592357 42.9480095987261 130.23 1)'",
                                                                                             4326))
        session.add(Lake(lake_name='Vanished lake', lake_geom=MS_SPATIAL_NULL))
        eq_(session.query(Spot.spot_location.text_zm.label('to_string')).filter(Spot.spot_height == 130.23).first().to_string, 'POINT (-88.5945861592357 42.9480095987261 130.23 1)')
        eq_(session.query(Spot.spot_location.text_zm.label('to_string')).filter(Spot.spot_height == 420.4).first().to_string, 'POINT (-88.5945861592357 42.9480095987261)')
        ok_(session.query(Lake.lake_geom.to_string.label('to_string')).filter(Lake.lake_name == 'Vanished lake').first().to_string is None)
        return