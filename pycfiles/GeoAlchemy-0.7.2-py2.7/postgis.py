...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/geoalchemy/postgis.py
# Compiled at: 2013-01-09 17:05:21
from sqlalchemy import select, func, and_
from geoalchemy.base import SpatialComparator, PersistentSpatialElement, WKBSpatialElement, WKTSpatialElement
...
class pg_functions(functions):
    ...
    def _within_distance(compiler, geom1, geom2, distance, *args):
        ...
        """
        return and_(func.ST_Expand(geom2, distance).op('&&')(geom1), func.ST_Expand(geom1, distance).op('&&')(geom2), func.ST_Distance(geom1, geom2) <= distance)


class PGSpatialDialect(SpatialDialect):
    """Implementation of SpatialDialect for PostGIS."""
    __functions = {WKTSpatialElement: 'ST_GeomFromText', 
    ...
    def handle_ddl_after_create(self, bind, table, column):
        bind.execute(select([
         func.AddGeometryColumn(table.schema or 'public', table.name, column.name, column.type.srid, column.type.name, column.type.dimension)]).execution_options(autocommit=True))
