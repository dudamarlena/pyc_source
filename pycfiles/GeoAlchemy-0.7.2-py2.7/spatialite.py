...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/geoalchemy/spatialite.py
# Compiled at: 2012-03-28 10:14:37
from sqlalchemy import select, func
from sqlalchemy.sql import and_, text, column, table
from geoalchemy.base import SpatialComparator, PersistentSpatialElement, WKBSpatialElement
...
class sqlite_functions(mysql_functions):
    ...
    def _within_distance(compiler, geom1, geom2, distance, *args):
        if isinstance(geom1, GeometryExtensionColumn) and geom1.type.spatial_index and SQLiteSpatialDialect.supports_rtree(compiler.dialect):
            return and_(func.Distance(geom1, geom2) <= distance, table(geom1.table.fullname, column('rowid')).c.rowid.in_(select([table('idx_%s_%s' % (geom1.table.fullname, geom1.key), column('pkid')).c.pkid]).where(and_(text('xmin') >= func.MbrMinX(geom2) - distance, and_(text('xmax') <= func.MbrMaxX(geom2) + distance, and_(text('ymin') >= func.MbrMinY(geom2) - distance, text('ymax') <= func.MbrMaxY(geom2) + distance))))))
        else:
...
class SQLiteSpatialDialect(SpatialDialect):
    ...
    __functions = {functions.within_distance: None, 
    ...
    def handle_ddl_after_create(self, bind, table, column):
        bind.execute(select([
         func.AddGeometryColumn(table.name, column.name, column.type.srid, column.type.name, column.type.dimension, 0 if column.nullable else 1)]).execution_options(autocommit=True))
        if column.type.spatial_index and SQLiteSpatialDialect.supports_rtree(bind.dialect):
    ...
    def supports_rtree(dialect):
        return dialect.server_version_info[0] > 3 or dialect.server_version_info[0] == 3 and dialect.server_version_info[1] >= 6