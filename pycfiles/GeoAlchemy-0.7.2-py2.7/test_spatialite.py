...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/geoalchemy/tests/test_spatialite.py
...
from geoalchemy import GeometryColumn, Point, Polygon, LineString, GeometryDDL, WKTSpatialElement, WKBSpatialElement, DBSpatialElement, GeometryExtensionColumn
...
spots_table = Table('spots', metadata, Column('spot_id', Integer, primary_key=True), Column('spot_height', Numeric()), GeometryExtensionColumn('spot_location', Point(2, srid=4326)))
...
mapper(Spot, spots_table, properties={'spot_location': GeometryColumn(spots_table.c.spot_location, comparator=SQLiteComparator)})
...
class TestGeometry(TestCase):
    ...
    def test_wkb(self):
        eq_(session.scalar(functions.wkt(func.GeomFromWKB(self.r.road_geom.wkb, 4326))), 'LINESTRING(-88.674841 43.103503, -88.646417 42.998169, -88.607962 42.968073, -88.516003 42.936306, -88.439093 43.003185)')
        eq_(session.scalar(self.r.road_geom.wkb), self.r.road_geom.geom_wkb)
        centroid_geom = DBSpatialElement(session.scalar(self.r.road_geom.centroid))
        eq_(session.scalar(functions.wkt(func.GeomFromWKB(centroid_geom.wkb, 4326))), 'POINT(-88.576937 42.991563)')
    ...
    def test_envelope(self):
        eq_(session.scalar(functions.wkt(self.r.road_geom.envelope)), 'POLYGON((-88.674841 42.936306, -88.439093 42.936306, -88.439093 43.103503, -88.674841 43.103503, -88.674841 42.936306))')
    ...
    def test_transform(self):
        ...
        eq_(session.scalar(functions.wkt(spot.spot_location.transform(2249))), 'POINT(-3890517.610956 3627658.674651)')
        ok_(session.query(Spot).filter(sqlite_functions.mbr_contains(functions.buffer(Spot.spot_location.transform(2249), 10), WKTSpatialElement('POINT(-3890517.610956 3627658.674651)', 2249))).first() is not None)
        eq_(session.scalar(functions.wkt(functions.transform(WKTSpatialElement('POLYGON((743238 2967416,743238 2967450,743265 2967450,743265.625 2967416,743238 2967416))', 2249), 4326))), 'POLYGON((-71.177685 42.39029, -71.177684 42.390383, -71.177584 42.390383, -71.177583 42.390289, -71.177685 42.39029))')
        return
    ...
    def test_area(self):
        ...
        r = session.query(Road).get(1)
        s = session.query(Spot).get(1)
        assert_almost_equal(session.scalar(l.lake_geom.area), 0.005674862570492767)
    ...
    def test_query_column_name(self):
        ...
        eq_(session.scalar(Select([func.SRID(Spot.spot_location)]).where(Spot.spot_id == 1)), None, 'AsBinary is added and the SRID is not returned')
        eq_(str(session.scalar(Select([func.SRID(Spot.spot_location.RAW)]).where(Spot.spot_id == 1))), '4326', 'AsBinary is not added and the SRID is returned')
