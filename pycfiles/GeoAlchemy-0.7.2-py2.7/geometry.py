...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/geoalchemy/geometry.py
# Compiled at: 2013-04-09 07:19:18
import warnings
...
class GeometryDDL(object):
    ...
    def __init__(self, table):
        if self.use_event:
            ...
            for e in ('before-create', 'after-create', 'before-drop', 'after-drop'):
    ...
    def __call__(self, event, table, bind):
        ...
        if event in ('before-create', 'before-drop'):
            ...
            setattr(table, self.columns_attribute, expression.ColumnCollection(*regular_cols))
    ...
    if isinstance(element.table, (Table, Alias)):
        if kw.has_key('within_columns_clause') and kw['within_columns_clause'] == True:
            if element.type.wkt_internal:
                ...
                warnings.warn('WKT Internal GeometryColumn type not compatible with %s dialect. Defaulting back to WKB' % compiler.dialect.name, exc.SAWarning)
...
def GeometryColumn(*args, **kw):
    ...
    return column_property(column, extension=SpatialAttribute(), comparator_factory=comparator)