...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/socialcommerce/apps/mptt/managers.py
...
class TreeManager(models.Manager):
    ...
    def add_related_count(self, queryset, rel_model, rel_field, count_attr, cumulative=False):
        ...
        if cumulative:
            subquery = CUMULATIVE_COUNT_SUBQUERY % {'rel_table': qn(rel_model._meta.db_table), 
            ...
            subquery = COUNT_SUBQUERY % {'rel_table': qn(rel_model._meta.db_table), 
               'mptt_fk': qn(rel_model._meta.get_field(rel_field).column), 
               'mptt_table': qn(opts.db_table), 
               'mptt_pk': qn(opts.pk.column)}
    ...
    def insert_node(self, node, target, position='last-child', commit=False):
        ...
        if target is None:
            ...
            setattr(node, self.level_attr, 0)
            (space_target, level, left, parent) = self._calculate_inter_tree_move_values(node, target, position)
    ...
    def root_node(self, tree_id):
        ...
        """
        return self.get(**{self.tree_id_attr: tree_id, 
    ...
    def _create_tree_space(self, target_tree_id):
        ...
        cursor.execute('\n        UPDATE %(table)s\n        SET %(tree_id)s = %(tree_id)s + 1\n        WHERE %(tree_id)s > %%s' % {'table': qn(opts.db_table), 
           'tree_id': qn(opts.get_field(self.tree_id_attr).column)}, [
         target_tree_id])
    ...
    def _inter_tree_move_and_close_gap(self, node, level_change, left_right_change, new_tree_id, parent_pk=None):
        """
    ...
    def _make_sibling_of_root_node(self, node, target, position):
        ...
        if node.is_child_node():
            ...
            if position == 'left':
                ...
                raise ValueError(_('An invalid position was given: %s.') % position)
            root_sibling_query = '\n            UPDATE %(table)s\n            SET %(tree_id)s = CASE\n                WHEN %(tree_id)s = %%s\n                    THEN %%s\n                ELSE %(tree_id)s + %%s END\n            WHERE %(tree_id)s >= %%s AND %(tree_id)s <= %%s' % {'table': qn(opts.db_table), 
               'tree_id': qn(opts.get_field(self.tree_id_attr).column)}
            cursor = connection.cursor()
    ...
    def _manage_space(self, size, target, tree_id):
        ...
        """
        opts = self.model._meta
        space_query = '\n        UPDATE %(table)s\n        SET %(left)s = CASE\n                WHEN %(left)s > %%s\n                    THEN %(left)s + %%s\n                ELSE %(left)s END,\n            %(right)s = CASE\n                WHEN %(right)s > %%s\n                    THEN %(right)s + %%s\n                ELSE %(right)s END\n        WHERE %(tree_id)s = %%s\n          AND (%(left)s > %%s OR %(right)s > %%s)' % {'table': qn(opts.db_table), 
           'left': qn(opts.get_field(self.left_attr).column), 
           'right': qn(opts.get_field(self.right_attr).column), 
           'tree_id': qn(opts.get_field(self.tree_id_attr).column)}
        cursor = connection.cursor()
    ...
    def _move_child_node(self, node, target, position):
        ...
        tree_id = getattr(node, self.tree_id_attr)
        target_tree_id = getattr(target, self.tree_id_attr)
        if getattr(node, self.tree_id_attr) == getattr(target, self.tree_id_attr):
    ...
    def _move_child_to_new_tree(self, node, target, position):
        ...
        (space_target, level_change, left_right_change, parent) = self._calculate_inter_tree_move_values(node, target, position)
        tree_width = right - left + 1
        self._create_space(tree_width, space_target, new_tree_id)
        self._inter_tree_move_and_close_gap(node, level_change, left_right_change, new_tree_id, parent.pk)
        setattr(node, self.left_attr, left - left_right_change)
        setattr(node, self.right_attr, right - left_right_change)
