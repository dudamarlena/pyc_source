...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/socialcommerce/apps/cms/templatetags/cms_tags.py
# Compiled at: 2009-10-31 23:19:40
from django import template
from django.core.cache import cache
from cms import settings
from cms.models import Page, Title, CMSPlugin
from cms.utils import get_language_from_request, get_extended_navigation_nodes, find_children, cut_levels, find_selected
...
def show_menu(context, from_level=0, to_level=100, extra_inactive=0, extra_active=100, template='cms/menu.html', next_page=None, root_id=None):
    ...
    if not next_page:
        ...
        if current_page:
            ...
            alist = []
            extenders = Page.objects.published().filter(in_navigation=True, sites__domain=site.domain, level__lte=to_level)
            ...
            filters = {'in_navigation': True, 'sites__domain': site.domain, 
               'level__lte': to_level}
            soft_root_pk = None
            for p in alist:
            ...
            elif current_page and current_page.soft_root:
        ...
        pages = Page.objects.published().filter(**filters).order_by('tree_id', 'parent', 'lft')
        pages = list(pages)
        if root_page:
            pages = [
             root_page] + pages
...
def show_sub_menu(context, levels=100, template='cms/sub_menu.html'):
    ...
    if page:
        root = page.get_root()
        filters = {'in_navigation': True, 'lft__gt': page.lft, 
           'rght__lt': page.rght, 
    ...
    extenders = Page.objects.published().filter(in_navigation=True, sites__domain=site.domain)
    extenders = extenders.exclude(navigation_extenders__isnull=True).exclude(navigation_extenders__exact='')
    children = []
    from_level = 0
    to_level = 0
...
show_sub_menu = register.inclusion_tag('cms/dummy.html', takes_context=True)(show_sub_menu)
...
show_admin_menu = register.inclusion_tag('admin/cms/page/menu.html', takes_context=True)(show_admin_menu)
...
show_breadcrumb = register.inclusion_tag('cms/dummy.html', takes_context=True)(show_breadcrumb)
    ...
    if not url:
        try:
            ...
            if settings.DEBUG:
                ...
                site = Site.objects.get_current()
                send_mail(_('Reverse ID not found on %(domain)s') % {'domain': site.domain}, _("A page_id_url template tag didn't found a page with the reverse_id %(reverse_id)s\nThe url of the page was: http://%(host)s%(path)s") % {'reverse_id': reverse_id, 'host': request.host, 'path': request.path}, settings.DEFAULT_FROM_EMAIL, settings.MANAGERS, fail_silently=True)
        else:
    ...
    if not content:
        try:
            ...
            if settings.DEBUG:
                ...
                site = Site.objects.get_current()
                send_mail(_('Reverse ID not found on %(domain)s') % {'domain': site.domain}, _("A show_placeholder_by_id template tag didn't found a page with the reverse_id %(reverse_id)s\nThe url of the page was: http://%(host)s%(path)s") % {'reverse_id': reverse_id, 'host': request.host, 'path': request.path}, settings.DEFAULT_FROM_EMAIL, settings.MANAGERS, fail_silently=True)
        else:
