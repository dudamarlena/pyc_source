...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/socialcommerce/apps/cms/middleware.py
...
SUB = re.compile('<a([^>]+)href="/(?!(%s|%s|%s))([^"]*)"([^>]*)>' % (('|').join(map(lambda l: l[0] + '/', settings.LANGUAGES)),
 settings.MEDIA_URL[1:],
 settings.ADMIN_MEDIA_PREFIX[1:]))
SUB2 = re.compile('<form([^>]+)action="/(?!(%s|%s|%s))([^"]*)"([^>]*)>' % (('|').join(map(lambda l: l[0] + '/', settings.LANGUAGES)),
 settings.MEDIA_URL[1:],
 settings.ADMIN_MEDIA_PREFIX[1:]))
...
class MultilingualURLMiddleware:
    ...
    def get_language_from_request(self, request):
        ...
        check = re.match('^/(%s)/.*' % langs, request.path_info)
        changed = False
    ...
    def process_response(self, request, response):
        ...
        if response.status_code == 200 and not request.path.startswith(settings.MEDIA_URL) and response._headers['content-type'][1].split(';')[0] == 'text/html':
            response.content = SUB.sub('<a\\1href="/%s/\\3"\\4>' % request.LANGUAGE_CODE, response.content.decode('utf-8'))
            response.content = SUB2.sub('<form\\1action="/%s/\\3"\\4>' % request.LANGUAGE_CODE, response.content.decode('utf-8'))
        return response