...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/socialcommerce/apps/mptt/utils.py
...
__all__ = ('previous_current_next', 'tree_item_iterator', 'drilldown_tree_for_node')
        ...
        if previous:
            ...
            if ancestors:
                if structure['closed_levels']:
                    structure['ancestors'] = structure['ancestors'][:-len(structure['closed_levels'])]
            ...
            if ancestors:
                structure['ancestors'] = []
        if next:
            structure['closed_levels'] = range(current_level, getattr(next, opts.level_attr), -1)
        else:
...
def drilldown_tree_for_node(node, rel_cls=None, rel_field=None, count_attr=None, cumulative=False):
    """
