...
# [GCC 8.4.0]
# Embedded file name: build/bdist.darwin-8.7.0-Power_Macintosh/egg/atomstorage/shelvestorage.py
# Compiled at: 2006-09-25 17:36:36
"""
...
"""
import shelve, datetime, operator, re
...
class EntryManager(object):
    ...
    def get_entries_by_date(self, year, month=None, day=None):
        ...
        if month:
            entries = [ e for e in entries if e['updated'].month == month ]
        if day:
            entries = [ e for e in entries if e['updated'].day == day ]
        return entries
    ...
    def get_entries_by_category(self, category):
        ...
        """
        entries = self.get_entries()
        entries = [ e for e in entries if category.lower() in [ c['term'].lower() for c in e.get('categories', []) ] ]
        return entries
    ...
    def search(self, regexp, flags=0):
        ...
        """
        entries = self.get_entries()
        entries = [ e for e in entries if e.get('content') if re.search(regexp, e['content']['content'], flags) ]
        return entries
