...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/meringue/utils/thumbnails.py
# Compiled at: 2015-08-17 17:37:49
from PIL import Image
import logging, math, os.path, re
try:
...
class Thumbnail(object):
    ...
    def make_thumbnail(self):
        ...
        self.image.save(self.thumbnail_filename, 'PNG', quality=self.quality, optimize=1)
    ...
    def _resize(self):
        if self.resize_method == 'scale':
            res = max(min(self.new_size[0], self.size[0]) / self.size[0], min(self.new_size[1], self.size[1]) / self.size[1])
            new_size = [int(math.floor(self.size[0] * res)),
             int(math.floor(self.size[1] * res))]
        elif self.resize_method == 'inscribe':
            res = min(min(self.new_size[0], self.size[0]) / self.size[0], min(self.new_size[1], self.size[1]) / self.size[1])
            new_size = [int(math.floor(self.size[0] * res)),
             int(math.floor(self.size[1] * res))]
    ...
    def _s(self, size):
        new_size = re.compile('(\\d+)x(\\d+)')
        self.new_size = new_size.search(size).groups()
        self.new_size = [ float(i) for i in self.new_size ]
        self.new_size[0] = int(self.new_size[0] if self.new_size[0] else self.new_size[1] * self.size[0] / self.size[1])
        self.new_size[1] = int(self.new_size[1] if self.new_size[1] else self.new_size[0] * self.size[1] / self.size[0])
