...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/meringue/models.py
...
options.DEFAULT_NAMES = options.DEFAULT_NAMES + ('host_name', 'view', 'reverse_args')
...
class GetAbsoluteUrlMixin(object):
    ...
    def _host_name(self):
        if not hasattr(self._meta, 'host_name'):
            ...
            self._meta.host_name = getattr(module, 'host_name', module_name.lower())
    ...
    def get_absolute_url(self):
        reverse_args = {'viewname': self._view(), 
           'args': self._reverse_args()}
        if 'django_hosts' in settings.INSTALLED_APPS:
            reverse_args.update({'host': self._host_name(), 
...
class PublishModel(GetAbsoluteUrlMixin, models.Model):
    ...
    """
    is_published = models.BooleanField(verbose_name=_('публикация'), help_text=_('Отображать/Скрыть'), default=True, db_index=True)
