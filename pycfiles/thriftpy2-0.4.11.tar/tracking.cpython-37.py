...
# [GCC 8.4.0]
# Embedded file name: /Users/chris/Idea/thriftpy2/tests/compatible/version_2/tracking.py
...
import os.path, time, copy, contextlib, threading, uuid
from thriftpy2.thrift import TClient, TApplicationException, TMessageType, TProcessor, TType
...
class RequestInfo(object):
    ...
    def __init__(self, request_id, api, seq, client, server, status, start, end, annotation, meta):
...
class TTrackedClient(TClient):
    ...
    def _negotiation(self):
        self._oprot.write_message_begin(track_method, TMessageType.CALL, self._seqid)
    ...
    def _req(self, _api, *args, **kwargs):
        ...
        try:
            ...
            header_info = RequestInfo(request_id=(self._header.request_id),
...
class TTrackedProcessor(TProcessor):
    ...
    def _process_in(self, api, iprot):
        if api not in self._service.thrift_services:
            ...
            iprot.read_message_end()
            return (
             TApplicationException(TApplicationException.UNKNOWN_METHOD), None)
        ...
        def call():
            return (getattr(self._handler, api))(*(args.__dict__[k] for k in api_args))
    ...
    def gen_header(self, header):
        ...
        if not hasattr(ctx, 'counter'):
            ...
            if hasattr(ctx, 'header'):
                header.seq = '{prev_seq}.{cur_counter}'.format(prev_seq=(ctx.header.seq),
                  cur_counter=(ctx.counter))
