...
# [GCC 8.4.0]
# Embedded file name: /Users/chris/Idea/thriftpy2/tests/addressbook.py
...
class PhoneNumber(TPayload):
    thrift_spec = {1:(
      TType.I32, 'type', PhoneType, False), 
     2:(
      TType.STRING, 'number', False), 
     3:(
      TType.STRUCT, 'mix_item', MixItem, False)}
    default_spec = [
     ('type', None), ('number', None), ('mix_item', None)]


class Person(TPayload):
    thrift_spec = {1:(
      TType.STRING, 'name', False), 
     2:(
      TType.LIST, 'phones', (TType.STRUCT, PhoneNumber)), 
     4:(
      TType.I32, 'created_at', False)}
    default_spec = [
     ('name', None), ('phones', None), ('created_at', None)]


class AddressBook(TPayload):
    thrift_spec = {1: (TType.MAP, 'people',
         (
...
class AddressBookService(object):
    ...
    class add_args(TPayload):
        thrift_spec = {1: (TType.STRUCT, 'person', Person)}
        default_spec = [
         ('person', None)]

    class add_result(TPayload):
        thrift_spec = {0: (TType.BOOL, 'success')}
        default_spec = [
         ('success', None)]
    ...
    class remove_result(TPayload):
        thrift_spec = {0:(
          TType.BOOL, 'success'), 
         1:(
          TType.STRUCT, 'not_exists', PersonNotExistsError)}
        default_spec = [
         ('success', None), ('not_exists', None)]
    ...
    class book_result(TPayload):
        thrift_spec = {0: (TType.STRUCT, 'success', AddressBook)}
        default_spec = [
         ('success', None)]
    ...
    class get_phonenumbers_result(TPayload):
        thrift_spec = {0: (TType.LIST, 'success', TType.STRUCT)}
        default_spec = [
         ('success', None)]
    ...
    class get_phones_result(TPayload):
        thrift_spec = {0: (TType.MAP, 'success', (TType.I32, TType.STRING))}
        default_spec = [
         ('success', None)]

    class sleep_args(TPayload):
        thrift_spec = {1: (TType.I16, 'ms')}
        default_spec = []
