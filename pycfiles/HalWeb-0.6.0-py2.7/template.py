...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/halicea/baseProject/lib/halicea/template.py
...
"""
import logging, md5, os, conf.settings
...
def _django_setup():
  ...
  """
    ...
    if django_version is not None:
        ...
        if not explicit:
            logging.warn('You are using the default Django version (%s). The default Django version will change in an App Engine release in the near future. Please call use_library() to explicitly select a Django version. For more information see %s', version, 'http://code.google.com/appengine/docs/python/tools/libraries.html#Django')
        try:
        ...
        try:
            django.conf.settings.configure(DEBUG=False, TEMPLATE_DEBUG=False, TEMPLATE_LOADERS=('django.template.loaders.filesystem.load_template_source', ))
...
_config_handle = lib_config.register('webapp', {'django_setup': _django_setup, 'django_version': None})
_config_handle.django_setup()
import django.template, django.template.loader
...
def load(path, debug=False):
  ...
  """
    ...
    if not template:
        ...
        new_settings = {'TEMPLATE_DIRS': (
                           directory, settings.VIEWS_DIR), 
