...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/halicea/baseProject/models/BaseModels.py
...
class Person(polymodel.PolyModel):
    ...
    def __validate__(self):
        ...
        if not self.Email and self.AuthenticationType == 'local':
    ...
    def CreateNew(cls, uname, name, surname, email, password, public=True, notify=True, authType=None, photoUrl=None, _autoSave=False):
        result = cls(UserName=uname, Email=email, Name=name, Surname=surname, Password=password, Public=public, Notify=notify, PhotoUrl=photoUrl)
        if authType:
...
class Role(db.Model):
    ...
    def CreateNew(cls, rolename, roledescription, _isAutoInsert=False):
        result = cls(RoleName=rolename, RoleDescription=roledescription)
        if _isAutoInsert:
            result.put()
        return result
...
class RoleAssociation(db.Model):
    ...
    def CreateNew(cls, rolename, roledescription, role, person, _isAutoInsert=False):
        result = cls(Role=role, Person=person)
        if _isAutoInsert:
            result.put()
        return result
