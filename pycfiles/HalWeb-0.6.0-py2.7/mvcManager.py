...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/halicea/lib/mvcManager.py
...
def setProperties(p, model):
    ...
    if len(t) > 1:
        ...
        if t[1] == 'ref':
            ...
            has_coll_name = reduce(lambda x, y: x == True or x is str and 'collection_name' in x or 'collection_name' in y, t[propStart])
...
operations = [ (x[0], x[1]) for x in settings.DEFAULT_OPERATIONS.iteritems() if x[1].has_key('view') and x[1]['view'] ]
...
def renderController(model=None, baseBlock=None, outputStream=stdout, magicType='magic0', *args):
    ...
    if m:
        ...
        classImport = Template('from ${modelsPath}.${modelModule} import ${modelClass}').substitute(modelsPath=basename(settings.MODELS_DIR), modelModule=m.Package + settings.MODEL_MODULE_SUFIX, modelClass=m.Name)
        formImport = Template('from ${formsPath}.${formModule} import ${formClass}').substitute(formsPath=basename(settings.FORM_MODELS_DIR), formModule=m.Package + settings.MODEL_FORM_MODULE_SUFIX, formClass=m.Name + settings.MODEL_FORM_CLASS_SUFIX)
...
def renderHandlerMap(model=None, baseBlock=None, outputStream=stdout, magicType='magic0', *args):
    ...
    urlEntry = templ.substitute(model=locators.BasePathFromName(m.FullName, '/'), controller=m.Package + settings.CONTROLLER_MODULE_SUFIX + '.' + m.Name + settings.CONTROLLER_CLASS_SUFIX)
...
def makeMvc(args):
    ...
    if not modelList:
        ...
        if 'm' in arg:
            m.InheritsFrom = inherits_from
            i = 0
            print '..............class ' + m.FullName + '(' + m.InheritsFrom + '):'
            p = True
            while p:
