...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/halicea/baseProject/controllers/BaseControllers.py
...
class LoginController(hrh):
    ...
    def JanrainAuth(self, *args):
        ...
        args = {'format': 'json', 
           ...
           'token': token}
        r = urlfetch.fetch(url=url, payload=urllib.urlencode(args), method=urlfetch.POST, headers={'Content-Type': 'application/x-www-form-urlencoded'})
        ...
        if json['stat'] == 'ok':
            ...
            if not person:
                ...
                if len(args) == 1:
                    ...
                    person = Person.CreateNew2(uname=json['profile']['preferredUsername'], name=name, surname=surname, email=email, password='openid', public=True, notify=True, authType=json['profile']['providerName'], photoUrl=photo, _autoSave=True)
...
class AddUserController(hrh):
    ...
    def post(self):
        ...
        try:
            user = Person(UserName=self.g('UserName'), Email=self.g('Email'), Name=self.g('Name'), Surname=self.g('Surname'), Password=self.g('Password'), Public=self.g('Public') == 'on' and True or False, Notify=self.g('Notify') == 'on' and True or False)
