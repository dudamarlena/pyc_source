...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/halicea/lib/baseClasses.py
# Compiled at: 2012-01-05 21:10:30
import os, shutil
...
class Block(object):
    """Utility class that manages Tree structure
        it has basic methods of appending a node or removing a node from the tree
        also methods for listing the code tree
        It needs to be used with some BlockLocator instance that can identify code blocks
    """
    Parrent = None
    Children = []
    Name = ''
    ...
    def append(self, element):
        if isinstance(element, Block):
            ...
            if len(self.Children) > 1:
                ...
                if self.Children[cnt].BlockType == BlockTypes.LINE and self.__Locator.lineIsBlockEnd(str(self.Children[cnt])) and self.__Locator.lineIsBlockBegin(str(self.Children[0])):
