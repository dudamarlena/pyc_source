...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/halicea/baseProject/lib/PyRSS2Gen.py
...
class Cloud:
    ...
    def publish(self, handler):
        _element(handler, 'cloud', None, {'domain': self.domain, 
           'port': str(self.port), 
           'path': self.path, 
...
class Enclosure:
    ...
    def publish(self, handler):
        _element(handler, 'enclosure', None, {'url': self.url, 'length': str(self.length), 
           'type': self.type})
        return
...
class RSS2(WriteXmlMixin):
    ...
    def __init__(self, title, link, description, language=None, copyright=None, managingEditor=None, webMaster=None, pubDate=None, lastBuildDate=None, categories=None, generator=_generator_name, docs='http://blogs.law.harvard.edu/tech/rss', cloud=None, ttl=None, image=None, rating=None, textInput=None, skipHours=None, skipDays=None, items=None):
        self.title = title
        self.link = link
        self.description = description
