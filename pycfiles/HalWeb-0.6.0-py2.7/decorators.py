...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/halicea/baseProject/lib/halicea/decorators.py
# Compiled at: 2012-01-02 14:50:45
"""
...
class ErrorSafe(object):
    ...
    def __init__(self, redirectUrl='/', message=messages.error_happened, Exception=Exception, showStackTrace=False):
...
class Cached(object):
    ...
    def resName(res, request, *args, **kwargs):
        return '__RESOURCE__' + request.__module__ + '.' + request.__name__ + '.' + res.__name__ + '(' + (',').join(set([(',').join([ str(x) for x in args ]), (',').join([ x + '=' + str(kwargs[x]) for x in kwargs.iterkeys() ])])) + ')'
