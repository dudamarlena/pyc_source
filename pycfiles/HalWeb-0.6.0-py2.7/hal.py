...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/halicea/hal.py
...
modelsStructure = {}
commandsDict = {'*': {'new': {'template': {}, 'real': {}}, 'project': {}, 'mvcfh': {}, 'del': {'package': {}, 'model': {}}, 'deploy': {'--no_cookies': {}, '--email=': {}}, 'run': {'--port=': {}, '--clear_datastore=': {}, '--datastore_path=': {}}, 'pack': {'package': {}}, 'unpack': {'package': {}}, 'git': {'status': {}, 'log': {}, 'init': {}, 'push': {'origin': {'master': {}}}, 'commit': {'-a': {}, '-am "': {}}, 'add': {'.': {}}}, 'deploy': {}, 'exit': {}}}
...
def main(args):
    ...
    if True:
        if set(args[0]).issubset(set('mvfch')):
            ...
            if len(args) > 1:
                options = (' ').join(args[1:])
            command = Template('python $appserver $proj $options').substitute(appserver=pjoin(config.APPENGINE_PATH, 'dev_appserver.py'), proj=config.PROJ_LOC, options=options)
            ...
            if len(args) > 1:
                options = (' ').join(args[1:])
            command = Template('python $appcfg update $options $proj').substitute(appcfg=pjoin(config.APPENGINE_PATH, 'appcfg.py'), proj=config.PROJ_LOC, options=options)
