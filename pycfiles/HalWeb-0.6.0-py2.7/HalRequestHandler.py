...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/halicea/baseProject/lib/halicea/HalRequestHandler.py
# Compiled at: 2012-01-04 17:10:43
from os import path
import os, lib.paths as paths, conf.settings as settings
...
import simplejson
templateGroups = {'form': settings.FORM_VIEWS_DIR, 'page': settings.PAGE_VIEWS_DIR, 
...
class HalRequestHandler(webapp.RequestHandler):
    ...
    def initialize(self, request, response):
        ...
        if not self.isAjax:
            self.isAjax = self.params.isAjax == 'true'
        if self.session:
    ...
    def new_factory(cls, *args, **kwargs):
        ...
        new_instance.__name__ = cls.__name__
    ...
    def __route__(self, method, *args, **kwargs):
        ...
        if self.operations.has_key(self.op):
            ...
            elif hasattr(self, self.operations[self.op]['method'].__name__):
        ...
        elif isinstance(self.operations['default']['method'], str):
            outresult = getattr(self, self.operations['default']['method'])(*args, **kwargs)
        elif hasattr(self, self.operations['default']['method'].__name__):
    ...
    def respond(self, item={}, *args):
        if self.responseType == 'html':
            if isinstance(item, str):
                ...
                self.__respond(template.render(self.Template, self.__render_dict(item), debug=settings.TEMPLATE_DEBUG))
            else:
                ...
                if isinstance(item, NewsFeed):
                    self.response.headers['Content-Type'] = 'application/xml; charset=utf-8'
                    self.__respond(template.render(os.path.join(settings.TEMPLATE_DIRS, 'RssTemplate.txt'), {'m': item}, debug=settings.DEBUG))
                else:
    ...
    def __render_dict(self, basedict):
        ...
        result.update(paths.GetFormsDict(path.join(settings.FORM_VIEWS_DIR, self.TemplateType)))
        result.update(paths.getViewsDict(os.path.dirname(self.Template)))
        if mobile_agents.detect_mobile(self.request):
            self.mobile = True
