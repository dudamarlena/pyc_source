...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/halicea/baseProject/models/cmsModels.py
# Compiled at: 2012-01-02 08:54:23
...
class CMSContent(db.Model):
    ...
    def CreateNew(cls, title, content, tags, creator, _isAutoInsert=False):
        result = cls(Title=title, HTMLContent=content, Tags=tags, Creator=creator)
        if _isAutoInsert:
            result.put()
        return result
...
class CMSLink(db.Model):
    ...
    def Url(self):
        if self.ContentTypeNumber not in [ContentType.StaticPage, ContentType.NoContent]:
            ...
            return result
        if self.ContentTypeNumber == ContentType.NoContent:
            return '#'
    ...
    def GetTreeBelow(self, ignoreAddresses=[]):
        ...
        for child in children:
            if child.AddressName not in ignoreAddresses:
    ...
    def CreateNew(cls, addressName, name, parentLink, order, content, contentType, creator, _isAutoInsert=False):
        ...
        if contentType == ContentType.StaticPage:
            ...
            additionalUrl = ''
        result = cls(parent=parentLink, AddressName=addressName, AdditionalUrl=additionalUrl, Name=name, ContentTypeNumber=contentType, ParentLink=parentLink, Depth=depth, Order=order, Content=content, Creator=creator)
        if _isAutoInsert:
            result.put()
        return result
