...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/halicea/baseProject/lib/customFilters.py
# Compiled at: 2011-12-31 11:28:23
...
def call(parser, token):
    ...
    syntax_message = '%(tag_name)s expects a syntax of %(tag_name)s <object>.<method or attribute> [with <*args> <**kwargs>] [as <context_name>]' % dict(tag_name=bits[0])
    temp = bits[1].split('.')
    method = temp[(-1)]
    object = ('.').join(temp[:-1])
    if len(bits) > 2:
