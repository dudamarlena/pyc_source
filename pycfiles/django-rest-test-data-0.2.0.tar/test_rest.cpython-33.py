...
# Compiled at: 2013-11-25 09:06:18
...
class TestSimple(TestBase):
    ...
    def test_create(self):
        ...
        assert_equal(response.content, self.serializer.serialize([Simple.objects.get()]).encode('utf-8'))

    def test_str(self):
        self.client.post(self.url(Simple), {'data': {'str_attr': 'testdata ☃'}})
        obj = Simple.objects.get()
        print(type(obj.str_attr))
        print(type('testdata ☃'))
    ...
    def test_list(self):
        ...
        assert_equal(response.content, self.serializer.serialize(Simple.objects.all()).encode('utf-8'))
    ...
    def test_get_one(self):
        self.client.post(self.url(Simple))
        obj = _load_json_from_response(self.client.post(self.url(Simple)))
        self.client.post(self.url(Simple))
        result = _load_json_from_response(self.client.get(self.url(Simple, pk=obj[0]['pk'])))
    ...
    def test_search(self):
        response = self.client.post(self.url(Simple), {'data': {'int_attr': 1}})
        ...
        assert_equal(Simple.objects.all().count(), 3, 'Objects not created')
        response = self.client.post(self.url(Simple, search=True), {'data': {'int_attr': 1}})
        result = _load_json_from_response(response)
        assert_equal(_pk_sort(result), _pk_sort(q01))
        response = self.client.post(self.url(Simple, search=True), {'data': {'int_attr': 2}})
        result = _load_json_from_response(response)
...
class TestKey(TestBase):
    ...
    def test_create(self):
        ...
        assert_equal(response.status_code, 200)
        assert_equal(response['Content-Type'], 'application/json')
        assert_equal(response.content, self.serializer.serialize([Key.objects.get()]).encode('utf-8'))
