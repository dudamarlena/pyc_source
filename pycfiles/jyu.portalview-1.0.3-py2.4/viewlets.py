...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/jyu/portalview/browser/viewlets.py
...
class MenuBarViewlet(ViewletBase):
    ...
    def parse(ul, deeper=True):
        ...
        for li in ul and ul.findall('li') or []:
            link = li.find('a')
            if link is not None:
                items.append({'text': link.text or None, 'href': link.get('href', None), 'title': link.get('title', None), 'target': link.get('target', None), 'accesskey': link.get('accesskey', None), 'subitems': deeper and MenuBarViewlet.parse(li.find('ul'), False) or None})
            else:
                items.append({'text': None, 'href': None, 'title': None, 'target': None, 'accesskey': None, 'subitems': None})
    ...
    def update(self):
        ...
        """
        if self.context.portal_type == 'Portal View' and self.request.getURL().split('/')[(-1)] in ['view', 'organize', '', self.context.id]:
