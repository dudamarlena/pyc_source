...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/fpys/client.py
# Compiled at: 2008-04-17 01:16:38
import base64, hmac, sha, urllib, urllib2, logging, time
...
class FPSResponse(object):
    ...
    def __init__(self, element=None):
        ...
        for child in element.getchildren():
            ...
            elif child.tag == 'Errors' and child.getchildren()[0].tag == 'Errors':
                self.errors = []
...
class FlexiblePaymentClient(object):
    ...
    def __init__(self, aws_access_key_id, aws_secret_access_key, fps_url='https://fps.sandbox.amazonaws.com', pipeline_url='https://authorize.payments-sandbox.amazon.com/cobranded-ui/actions/start'):
    ...
    def sign_string(self, string):
        ...
        sig = base64.encodestring(hmac.new(self.aws_secret_access_key, string, sha).digest()).strip()
        log.debug(sig)
        return sig
    ...
    def cancelToken(self, token_id, reason=None):
        params = {'Action': 'CancelToken', 'TokenId': token_id}
        if reason is not None:
            params['ReasonText'] = reason
        return self.execute(params)

    def discardResults(self, transaction_ids):
        params = {'Action': 'DiscardResults', 'TransactionIds': transaction_ids}
        return self.execute(params)

    def getAccountActivity(self, start_date, end_date=None, operation=None, payment_method=None, max_batch_size=None, response_group='Detail', sort_order='Descending', role=None, status=None):
        params = {'Action': 'GetAccountActivity', 'StartDate': start_date, 
    ...
    def getDebtBalance(self, instrument_id):
        params = {'Action': 'GetDebtBalance', 'CreditInstrumentId': instrument_id}
        return self.execute(params)

    def getAllCreditInstruments(self, instrument_status='All'):
        params = {'Action': 'GetAllCreditInstruments', 'InstrumentStatus': instrument_status}
        return self.execute(params)

    def getPaymentInstruction(self, token_id):
        params = {'Action': 'GetPaymentInstruction', 'TokenId': token_id}
        return self.execute(params)

    def getPipelineUrl(self, caller_reference, payment_reason, transaction_amount, return_url, pipeline_name='SingleUse'):
        parameters = {'callerReference': caller_reference, 'paymentReason': payment_reason, 
    ...
    def getPrepaidBalance(self, instrument_id):
        params = {'Action': 'GetPrepaidBalance', 'PrepaidInstrumentId': instrument_id}
        return self.execute(params)
    ...
    def getTokenUsage(self, token_id):
        params = {'Action': 'GetTokenUsage', 'TokenId': token_id}
        return self.execute(params)

    def getTransaction(self, transaction_id):
        params = {'Action': 'GetTransaction', 'TransactionId': transaction_id}
        return self.execute(params)

    def installPaymentInstruction(self, payment_instruction, caller_reference, token_type, token_friendly_name=None, payment_reason=None):
        """
        Install a payment instruction that conforms to the GateKeeper
        language specification
        """
        params = {'Action': 'InstallPaymentInstruction', 'PaymentInstruction': payment_instruction, 
    ...
    def pay(self, caller_token, sender_token, recipient_token, amount, caller_reference, date=None, charge_fee_to='Recipient'):
        params = {'Action': 'Pay', 'CallerTokenId': caller_token, 
    ...
    def refund(self, caller_token, refund_sender_token, transaction_id, caller_reference, refund_amount=None, date=None, charge_fee_to='Recipient'):
        params = {'Action': 'Refund', 'CallerTokenId': caller_token, 
    ...
    def reserve(self, caller_token, sender_token, recipient_token, amount, caller_reference, date=None, charge_fee_to='Recipient'):
        params = {'Action': 'Reserve', 'CallerTokenId': caller_token, 
           'SenderTokenId': sender_token, 
           'RecipientTokenId': recipient_token, 
           'TransactionAmount.Amount': amount, 
           'TransactionAmount.CurrencyCode': 'USD', 
    ...
    def retryTransaction(self, transaction_id):
        params = {'Action': 'RetryTransaction', 'OriginalTransactionId': transaction_id}
        return self.execute(params)

    def settle(self, transaction_id, amount, date=None):
        params = {'Action': 'Settle', 'ReserveTransactionId': transaction_id, 
           'TransactionAmount.Amount': amount, 
           'TransactionAmount.CurrencyCode': 'USD'}
        if date is not None:
            params['TransactionDate'] = date
