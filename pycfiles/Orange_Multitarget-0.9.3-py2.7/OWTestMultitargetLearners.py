...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/_multitarget/widgets/OWTestMultitargetLearners.py
...
"""
import Orange, Orange.multitarget.scoring
...
def avg_logloss(res):
    return Orange.multitarget.scoring.mt_average_score(res, Orange.evaluation.scoring.logloss)


def flat_logloss(res):
    return Orange.multitarget.scoring.mt_flattened_score(res, Orange.evaluation.scoring.logloss)


def avg_is(res):
    return Orange.multitarget.scoring.mt_average_score(res, Orange.evaluation.scoring.IS)


def flat_is(res):
    return Orange.multitarget.scoring.mt_flattened_score(res, Orange.evaluation.scoring.IS)


def avg_bs(res):
    return Orange.multitarget.scoring.mt_average_score(res, Orange.evaluation.scoring.Brier_score)


def flat_bs(res):
    return Orange.multitarget.scoring.mt_flattened_score(res, Orange.evaluation.scoring.Brier_score)


def avg_rmse(res):
    return Orange.multitarget.scoring.mt_average_score(res, Orange.evaluation.scoring.RMSE)


def flat_rmse(res):
    return Orange.multitarget.scoring.mt_flattened_score(res, Orange.evaluation.scoring.RMSE)
...
class OWTestMultitargetLearners(OWTestLearners):
    ...
    def __init__(self, parent=None, signalManager=None, title='Test Multitarget Learners'):
        OWTestLearners.__init__(self, parent, signalManager)
        self.setCaption(title)
        self.inputs = [
         (
          'Data', Orange.data.Table, self.setData, Default),
         (
    ...
    def updateScores(self):
        ...
        def needsupdate(learner_id):
            return not (self.learners[learner_id].scores or self.learners[learner_id].results)
    ...
    def score(self, learner_ids):
        ...
        if not self.data:
            ...
            indices = sample.SubsetIndices2(p0=min(n, len(self.data)), stratified=sample.SubsetIndices2.StratifiedIfPossible)
            ...
            for learner_id in learner_ids:
                ...
                try:
                    ...
                    if all(v.varType == c.varType for v, c in zip(predicted, new.domain.class_vars)):
            ...
            if learner_exceptions:
                text = ('\n').join('Learner %r ends with exception: %r' % (learn.name, ex) for learn, ex in learner_exceptions)
                self.warning(0, text)
            if not learners:
                return
            pb = None
            if self.resampling == 0:
                ...
                res = testing.cross_validation(learners, self.data, folds=self.nFolds, strat=sample.SubsetIndices.StratifiedIfPossible, callback=pb.advance, store_examples=True)
                pb.finish()
            else:
                if self.resampling == 1:
                    pb = OWGUI.ProgressBar(self, iterations=len(self.data))
                    res = testing.leave_one_out(learners, self.data, callback=pb.advance, store_examples=True)
                    pb.finish()
                else:
                    if self.resampling == 2:
                        pb = OWGUI.ProgressBar(self, iterations=self.pRepeat)
                        res = testing.proportion_test(learners, self.data, self.pLearning / 100.0, times=self.pRepeat, callback=pb.advance, store_examples=True)
                        pb.finish()
                    else:
                        if self.resampling == 3:
                            pb = OWGUI.ProgressBar(self, iterations=len(learners))
                            res = testing.learn_and_test_on_learn_data(learners, self.data, store_examples=True, callback=pb.advance)
                            pb.finish()
                            ...
                            if not self.testdata:
                                ...
                                return
                            pb = OWGUI.ProgressBar(self, iterations=len(learners))
                            res = testing.learn_and_test_on_test_data(learners, self.data, self.testdata, store_examples=True, callback=pb.advance)
                            pb.finish()
                        if self.preprocessor:
                    ...
                    for i, s in enumerate(self.stat):
                        if s.cmBased:
                            try:
                                ...
                                self.error(i, 'An error occurred while evaluating ' + str(s.f) + 'on %s due to %s' % (
...
if __name__ == '__main__':
    ...
    l1 = Orange.multitarget.binary.BinaryRelevanceLearner(learner=l1, name=l1.name)
    l2 = Orange.multitarget.binary.BinaryRelevanceLearner(learner=l2, name=l2.name)
    ow.setData(data1)
    ow.setLearner(l1, 1)
    ow.setLearner(l2, 2)
    ow.handleNewSignals()
