...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/_multitarget/widgets/OWNeuralNetwork.py
...
class OWNeuralNetwork(OWWidget):
    ...
    def __init__(self, parent=None, signalManager=None, title='Neural Network'):
        ...
        OWGUI.spin(box, self, 'n_mid', 2, 10000, 1, label='Hidden layer neurons', tooltip='Number of neurons in the hidden layer.')
        OWGUI.doubleSpin(box, self, 'reg_fact', 0.1, 10.0, 0.1, label='Regularization factor')
        OWGUI.spin(box, self, 'max_iter', 100, 10000, 1, label='Max iterations', tooltip='Maximal number of optimization iterations.')
    ...
    def apply(self):
        learner = Orange.multitarget.neural.NeuralNetworkLearner(name=self.name, n_mid=self.n_mid, reg_fact=self.reg_fact, max_iter=self.max_iter)
