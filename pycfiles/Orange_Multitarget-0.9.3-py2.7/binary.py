...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/_multitarget/binary.py
...
"""
import Orange.core as orange, Orange, copy
...
class BinaryRelevanceLearner(orange.Learner):
    ...
    def __call__(self, instances, weight=0):
        ...
        """
        if not instances.domain.class_vars:
            raise Exception('No classes defined.')
        m = len(instances.domain.class_vars)
        classifiers = [ None for _ in xrange(m) ]
        domains = [ Orange.data.Domain(instances.domain.attributes, cv) for cv in instances.domain.class_vars
                  ]
        for i in range(m):
...
class BinaryRelevanceClassifier(orange.Classifier):
    ...
    def __call__(self, instance, result_type=orange.GetValue):
        ...
        """
        predictions = [ c(Orange.data.Instance(dom, instance), result_type) for c, dom in zip(self.classifiers, self.domains)
                      ]
        if result_type == Orange.core.GetBoth:
            return zip(*predictions)
        return predictions

    def __reduce__(self):
        return (
         type(self), (self.classifiers, self.domains, self.name), dict(self.__dict__))
