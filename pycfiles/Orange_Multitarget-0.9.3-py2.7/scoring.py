...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/_multitarget/scoring.py
...
from Orange.utils import deprecated_keywords, deprecated_function_name, deprecation_warning, environ
...
def mt_average_score(res, score, weights=None):
    ...
    if res.number_of_learners < 1:
        ...
        return [ sum(w * s for w, s in zip(weights, scores)) / sum(weights) for scores in zip(*single_scores)
               ]
...
def mt_flattened_score(res, score):
    ...
    """
    res2 = Orange.evaluation.testing.ExperimentResults(res.number_of_iterations, res.classifier_names, class_values=res.class_values, weights=res.weights, classifiers=res.classifiers, loaded=res.loaded, test_type=Orange.evaluation.testing.TEST_TYPE_SINGLE, labels=res.labels)
    for te in res.results:
        for i, ac in enumerate(te.actual_class):
            te2 = Orange.evaluation.testing.TestedExample(iteration_number=te.iteration_number, actual_class=ac)
