...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/_multitarget/widgets/OWClusteringRandomForest.py
...
class OWClusteringRandomForest(OWWidget):
    settingsList = [
     'name', 'trees', 'min_instances', 'min_majority',
     'max_depth', 'min_MSE', 'method']

    def __init__(self, parent=None, signalManager=None, title='Clustering Random Forest'):
    ...
    def apply(self):
        ...
        learner = Orange.ensemble.forest.RandomForestLearner(base_learner=learner, trees=self.trees, name=self.name)
