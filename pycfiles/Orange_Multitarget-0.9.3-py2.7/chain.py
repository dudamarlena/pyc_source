...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/_multitarget/chain.py
...
"""
import Orange.core as orange, Orange, random, copy
...
class ClassifierChainLearner(orange.Learner):
    ...
    def __call__(self, instances, weight=0, class_order=None):
        ...
        """
        instances = Orange.data.Table(instances.domain, instances)
        ...
        if self.class_order and not class_order:
            ...
            for i in range(m):
                ...
                instances.change_domain(Orange.data.Domain(instances.domain, False, class_vars=instances.domain.class_vars))
                if self.callback:
...
class ClassifierChain(orange.Classifier):
    ...
    def __call__(self, instance, result_type=orange.GetValue):
        ...
        :param result_type: :class:`Orange.classification.Classifier.GetValue` or               :class:`Orange.classification.Classifier.GetProbabilities` or
...
class EnsembleClassifierChainLearner(orange.Learner):
    ...
    def __call__(self, instances, weight=0):
        ...
        for i in range(self.n_chains):
            ...
            learner = ClassifierChainLearner(learner=self.learner, actual_values=self.actual_values, rand=self.rand)
            classifiers.append(learner(data, weight, copy.copy(class_order)))
            if self.callback:
                self.callback((i + 1.0) / self.n_chains)
    ...
    def __call__(self, instance, result_type=orange.GetValue):
        ...
        for varn in xrange(len(class_vars)):
            ...
            if class_var.var_type == Orange.feature.Discrete.Discrete:
                ...
                elif result_type == orange.GetProbabilities:
        ...
        if result_type == orange.GetValue:
            return tuple(mt_value)
        else:
            if result_type == orange.GetProbabilities:
                return tuple(mt_prob)
            else:
                return [
                 tuple(mt_value), tuple(mt_prob)]

            return

    def __reduce__(self):
        return (
         type(self), (self.classifiers, self.class_order, self.domains, self.name), dict(self.__dict__))
