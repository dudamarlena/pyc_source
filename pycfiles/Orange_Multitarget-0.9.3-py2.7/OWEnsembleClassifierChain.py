...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/_multitarget/widgets/OWEnsembleClassifierChain.py
...
class OWEnsembleClassifierChain(OWWidget):
    settingsList = [
     'name', 'n_chains', 'sample_size', 'actual_values']

    def __init__(self, parent=None, signalManager=None, title='Ensemble Classifier Chain'):
        ...
        self.loadSettings()
        box = OWGUI.widgetBox(self.controlArea, 'Learner/Classifier Name', addSpace=True)
        OWGUI.lineEdit(box, self, 'name')
        box = OWGUI.widgetBox(self.controlArea, 'Settings', addSpace=True)
        OWGUI.spin(box, self, 'n_chains', 5, 1000, label='Num. of chains', tooltip='Number of chains to be constructed.')
        OWGUI.doubleSpin(box, self, 'sample_size', 0.05, 1.0, 0.05, label='Sample size', tooltip='Build each classifier on a fraction of the original data.')
        OWGUI.checkBox(box, self, 'actual_values', label='Use actual values', tooltip='Use actual values insteand of predicted ones when adding classes into features.')
    ...
    def apply(self):
        ...
        if self.base_learner is not None:
            learner = Orange.multitarget.chain.EnsembleClassifierChainLearner(name=self.name, learner=self.base_learner, n_chains=self.n_chains, sample_size=self.sample_size, actual_values=self.actual_values)
        classifier = None
