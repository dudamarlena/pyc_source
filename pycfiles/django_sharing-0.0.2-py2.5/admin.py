...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/sharing/admin.py
# Compiled at: 2010-09-30 06:31:39
...
class ShareAdminMixin(object):
    ...
    def formfield_for_foreignkey(self, db_field, request=None, **kwargs):
        ...
        kwargs['queryset'] = utils.limit_queryset_by_permission(qs=db_field.rel.to.objects.all(), perm=self.opts.app_label + '.view', user=request.user)
    ...
    def queryset(self, request):
        ...
        if request.user.is_superuser:
            ...
            return utils.limit_queryset_by_permission(qs=qs, perm=self.opts.app_label + '.view', user=request.user)
    ...
    def save_model(self, request, obj, form, change):
        ...
        try:
            UserShare.objects.get(user=request.user, content_type=ContentType.objects.get_for_model(obj), object_id=obj.id)
        except UserShare.DoesNotExist:
            UserShare.objects.create(user=request.user, can_view=True, can_change=True, can_delete=True, content_type=ContentType.objects.get_for_model(obj), object_id=obj.id)