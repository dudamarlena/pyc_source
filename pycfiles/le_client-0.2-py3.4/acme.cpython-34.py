...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/le_client/acme.py
# Compiled at: 2016-07-21 15:14:48
# Size of source mod 2**32: 7043 bytes
import urllib.request, urllib.error, base64, json, re, collections, os, time, textwrap
...
class UnexpectedHTTPStatus(RuntimeError):
    ...
    def __init__(self, status):
        super(UnexpectedHTTPStatus, self).__init__('Unexpected HTTP status code: {}'.format(status))
...
class ACMEChallenge(object):
    ...
    def __getattr__(self, name):
        ...
        raise AttributeError("Challenge doesn't have attribute {}".format(name))
...
class ACMEAuthority(object):
    ...
    def register(self):
        res = self.request_signed('/acme/new-reg', {'resource': 'new-reg', 
         'agreement': self.AGREEMENT_URL})
        if res.status not in (201, 409):
            raise UnexpectedHTTPStatus(res.status)
        return (
         res.status == 201, res.headers.get('Location', None))

    def authorize(self, domain):
        res = self.request_signed('/acme/new-authz', {'resource': 'new-authz', 
         'identifier': {'type': 'dns',  'value': domain}})
        if res.status != 201:
            raise UnexpectedHTTPStatus(res.status)
        data = res.data
        challenges = [ACMEChallenge(c) for c in data['challenges']]
        combinations = ACMEChallenges(ACMEChallengeSet([challenges[n] for n in combination]) for combination in data.get('combinations', []))
        return (
         data, combinations)
    ...
    def authorize_http01(self, challenge, path, wait=True):
        ...
        res = self.request_signed(challenge.uri, {'resource': 'challenge', 
    ...
    def request_certificate(self, csr):
        res = self.request_signed('/acme/new-cert', {'resource': 'new-cert', 
         'csr': b64(csr.as_der())}, raw=True)
        if res.status != 201:
            raise UnexpectedHTTPStatus(res.status)
        return '-----BEGIN CERTIFICATE-----\n{}\n-----END CERTIFICATE-----\n'.format('\n'.join(textwrap.wrap(base64.b64encode(res.data).decode('ascii'), 64)))
