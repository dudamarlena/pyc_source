...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/modipy/provisioner_command.py
...
class CommandProvisioner(Provisioner):
    ...
    command_re = re.compile('^(?P<start>.*)["\\\'](?P<quoted>.*)["\\\'](?P<end>.*)$')

    def parse_config_node(self, node):
        Provisioner.parse_config_node(self, node)

    def __init__(self, name, namespace={}, authoritarian=False, autobackout=False, sessionlog=None, command_timeout=300):
        Provisioner.__init__(self, name, namespace, authoritarian=authoritarian, autobackout=autobackout, sessionlog=sessionlog)
...
class MultiConnectingProvisioner(ConnectingProvisioner):
    ...
    """
    required_tags = ConnectingProvisioner.required_tags
    required_tags.remove('connect_command')
    ...
    def run_commands(self, ignored, device, expectset, namespace):
        ...
        for (expr, cmdstring, timeout) in expectset.commands:
            try:
                command = (' ').join([self.command, cmdstring])
                command = str(util.substituteVariables(command, namespace))
                log.debug('commandstring is: %s', command)
                namespace['command.send'] = command
            except KeyError, e:
