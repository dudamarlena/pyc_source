...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/modipy/iterator.py
...
class Iterator:
    ...
    def __init__(self, name, node):
        self.name = name
        self.iterlist = []
        self.delay = int(node.attrib.get('delay', 0))
        log.info('Iteration delay set to: %d', self.delay)

    def __iter__(self):
        log.debug('returning iterlist: %s', self.iterlist)
    ...
    def append(self, item):
        log.debug('appending to my iterlist: %s, %s', item, self.iterlist)
        self.iterlist.append(item)
        log.debug('iterlist is now: %s', self.iterlist)
...
class CSVIterator(Iterator):
    ...
    def __init__(self, name, node):
        Iterator.__init__(self, name, node)
        try:
