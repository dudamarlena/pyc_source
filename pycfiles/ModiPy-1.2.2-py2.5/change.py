...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/modipy/change.py
...
"""
import logging, debug, re, time, datetime
...
class Change(XMLConfigurable):
    ...
    def __init__(self, name, devices=[], pre_impl=None, impl=None, post_impl=None, pre_backout=None, backoutset=None, post_backout=None, pre_requisites=[], serial_mode=True, backout_all=False, on_fail_continue=False, on_fail_retry=False, max_retries=3, retry_delay=5, nopause=False, **kwargs):
        self._state = None
        self.name = name
    ...
    def copy(self):
        ...
        """
        newchange = self.__class__('Copy of %s' % self.name, self.devices, self.pre_impl, self.impl, self.post_impl, self.pre_backout, self.backoutset, self.post_backout, self.pre_requisites, self.serial_mode, self.backout_all, self.on_fail_continue, self.on_fail_retry, self.retry_delay, self.max_retries, self.nopause)
