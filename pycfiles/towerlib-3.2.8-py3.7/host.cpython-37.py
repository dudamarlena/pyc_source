...
# Size of source mod 2**32: 10121 bytes
"""
Main code for host.
...
class Host(Entity):
    __doc__ = 'Models the host entity of ansible tower.'
    ...
    def groups(self):
        ...
        return EntityManager((self._tower), entity_object='Group',
    ...
    def associate_with_groups(self, groups):
        ...
        missing_groups = [group_name for group_name in groups if group_name.lower() not in lower_inventory_group_names]
        if missing_groups:
            raise InvalidGroup(missing_groups)
        lower_group_names = [name.lower() for name in groups]
        final_groups = [group for group in inventory_groups if group.name.lower() in lower_group_names]
    ...
    def disassociate_with_groups(self, groups):
        ...
        if not isinstance(groups, (list, tuple)):
            groups = [
             groups]
        missing_groups = [group_name for group_name in groups if group_name not in self.groups]
        if missing_groups:
            raise InvalidGroup(missing_groups)
        lower_group_names = [name.lower() for name in groups]
        inventory_groups = [group for group in self.inventory.groups if group.name.lower() in lower_group_names]
