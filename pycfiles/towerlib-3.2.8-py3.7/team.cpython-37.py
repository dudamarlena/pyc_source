...
# [GCC 8.4.0]
# Embedded file name: /Users/ctyfoxylos/SBP/python/towerlib/towerlib/entities/team.py
...
"""
import logging
from towerlib.towerlibexceptions import InvalidUser, PermissionNotFound, InvalidProject, InvalidJobTemplate, InvalidInventory, InvalidCredential, InvalidValue, InvalidOrganization
...
class Team(Entity):
    __doc__ = 'Models the team entity of ansible tower.'
    ...
    def _get_permission(role_name, object_roles):
        permission = next((role for role in object_roles if role.name.lower() == role_name.lower()))
        if not permission:
    ...
    def _post_user_with_permission(self, username, role_name, remove=False):
        ...
        url = '{api}/users/{id}/roles/'.format(api=(self._tower.api), id=(user.id))
    ...
    def _post_permission(self, roles, permission_name, remove=False):
        ...
        if remove:
            url = '{api}/roles/{id}/teams/'.format(api=(self._tower.api), id=(permission.id))
            payload = {'id':self.id,  'disassociate':True}
        else:
            url = '{api}/teams/{id}/roles/'.format(api=(self._tower.api), id=(self.id))
            payload = {'id': permission.id}
        response = self._tower.session.post(url, json=payload)
        if not response.ok:
            self._logger.error('Error posting to url "%s", response was "%s"', url, response.text)
