...
# [GCC 8.4.0]
# Embedded file name: /Users/ctyfoxylos/SBP/python/towerlib/towerlib/entities/job.py
# Compiled at: 2019-10-18 06:12:55
# Size of source mod 2**32: 44589 bytes
"""
Main code for jobs.
...
class Job:
    __doc__ = 'Job factory to handle the different jod types returned.'
    ...
    def __new__(cls, tower_instance, data):
        ...
        if entity_type == 'job':
            ...
            if entity_type == 'project_update':
                obj = ProjectUpdateJob(tower_instance, data)
            else:
                if entity_type == 'system_job':
                    obj = SystemJob(tower_instance, data)
                else:
                    if entity_type == 'ad_hoc_command':
...
class JobEvent(Entity):
    __doc__ = 'Models the job event entity of ansible tower.'
    ...
    def job(self):
        ...
        return self._tower._get_object_by_url('Job', url)
...
class JobSummary(Entity):
    __doc__ = 'Models the Job entity of ansible tower.'
...
class JobRun(Entity):
    ...
    def _get_dynamic_value(self, variable):
        ...
        if response.ok:
            return response.json().get(variable)
    ...
    def inventory(self):
        ...
        return self._tower._get_object_by_url('Inventory', url)
    ...
    def unified_job_template(self):
        ...
        return self._tower._get_object_by_url('JobTemplate', url)
    ...
    def job_host_summaries(self):
        ...
        return EntityManager((self._tower), entity_object='JobSummary',
          primary_match_field='name',
          url=url)
    ...
    def job_events(self):
        ...
        return EntityManager((self._tower), entity_object='JobEvent',
          primary_match_field='name',
          url=url)
...
class JobTemplate(Entity):
    __doc__ = 'Models the Job Template entity of ansible tower.'
    ...
    def add_credential(self, credential):
        ...
        url = '{api}/job_templates/{id}/credentials/'.format(api=(self._tower.api), id=(self.id))
    ...
    def launch(self, extra_vars=None, job_tags=None, limit=None, inventory=None, credential=None):
        ...
        """
        payload = {key:value for key, value in locals().items() if value if key != 'self' if key != 'self'}
...
class ProjectUpdateJob(Entity):
    __doc__ = 'Models the project update entity of ansible tower.'
