...
# [GCC 8.4.0]
# Embedded file name: towerlib/entities/organization.py
...
"""
import json, logging
from towerlib.towerlibexceptions import InvalidUserLevel, InvalidUser, InvalidTeam, InvalidVariables, InvalidInventory, InvalidCredential, InvalidProject
...
class Organization(Entity):
    ...
    def create_project(self, name, description, credential, scm_url, scm_branch='master', scm_type='git', scm_clean=True, scm_delete_on_update=False, scm_update_on_launch=True, scm_update_cache_timeout=0):
        ...
        payload = {'name': name, 'description': description, 'scm_type': scm_type, 
    ...
    def create_user(self, first_name, last_name, email, username, password, level='standard'):
        ...
        if level == 'system_auditor':
            payload['user_type'] = {'type': 'system_auditor', 'label': 'System Auditor'}
            payload['is_system_auditor'] = True
        elif level == 'system_administrator':
            payload['user_type'] = {'type': 'system_administrator', 'label': 'System Administrator'}
            payload['is_superuser'] = True
        response = self._tower.session.post(url, data=json.dumps(payload))
        if response.ok:
            return User(self._tower, response.json())
        else:
            return
    ...
    def create_inventory(self, name, description, variables='{}'):
        ...
        payload = {'name': name, 'description': description, 'organization': self.id, 
           'variables': variables}
