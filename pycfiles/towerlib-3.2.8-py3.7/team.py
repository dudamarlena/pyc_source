...
# [GCC 8.4.0]
# Embedded file name: towerlib/entities/team.py
...
"""
import json, logging
from towerlib.towerlibexceptions import InvalidUser, PermissionNotFound, InvalidProject, InvalidJobTemplate, InvalidInventory, InvalidCredential
...
class Team(Entity):
    ...
    def _get_permission(role_name, object_roles):
        permission = next(role for role in object_roles if role.name.lower() == role_name)
        if not permission:
