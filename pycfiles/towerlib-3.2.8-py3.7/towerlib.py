...
"""
import json, logging, sys, math, concurrent.futures
from requests import Session
from cachetools import TTLCache, cached
from .entities import Config, LicenseInfo, LicenseFeatures, Organization, User, Project, Team, Group, Inventory, Host, CredentialType, Credential, JobTemplate, CERTIFICATE_TYPE_KINDS, JOB_TYPES, VERBOSITY_LEVELS, Cluster, ClusterInstance, EntityManager
from .towerlibexceptions import AuthFailed, InvalidOrganization, InvalidInventory, InvalidVariables, InvalidCredentialTypeKind, InvalidUser, InvalidTeam, InvalidCredential, InvalidHost, InvalidProject, InvalidGroup, InvalidCredentialType, InvalidPlaybook, InvalidInstanceGroup, InvalidJobType, InvalidVerbosity, InvalidJobTemplate
...
class Tower(object):
    ...
    def organizations(self):
        ...
        """
        return EntityManager(self, entity_name='organizations', entity_object='Organization', primary_match_field='name')
    ...
    def _get_paginated_response(self, url, params=None):
        ...
        if page_count:
            with concurrent.futures.ThreadPoolExecutor(max_workers=30) as (executor):
                futures = [ executor.submit(self.session.get, ('{url}&page={page_index}').format(url=url, page_index=index)) for index in range(page_count, 1, -1)
                          ]
    ...
    def create_user_in_organization(self, organization, first_name, last_name, email, username, password, level='standard'):
    ...
    def create_project_in_organization(self, organization, name, description, credential, scm_url, scm_branch='master', scm_type='git', scm_clean=True, scm_delete_on_update=False, scm_update_on_launch=True, scm_update_cache_timeout=0):
        ...
        return organization_.create_project(name, description, credential, scm_url, scm_branch, scm_type, scm_clean, scm_delete_on_update, scm_update_on_launch, scm_update_cache_timeout)
    ...
    def create_inventory_in_organization(self, organization, name, description, variables='{}'):
    ...
    def instance_groups(self):
        ...
        """
        return EntityManager(self, entity_name='instance_groups', entity_object='InstanceGroup', primary_match_field='name')
    ...
    def credential_types(self):
        ...
        """
        return EntityManager(self, entity_name='credential_types', entity_object='CredentialType', primary_match_field='name')
    ...
    def tower_credential_types(self):
        ...
        """
        return EntityManager(self, entity_name='credential_types', entity_object='CredentialType', primary_match_field='name').filter({'managed_by_tower': 'true'})
    ...
    def custom_credential_types(self):
        ...
        """
        return EntityManager(self, entity_name='credential_types', entity_object='CredentialType', primary_match_field='name').filter({'managed_by_tower': 'false'})
    ...
    def create_credential_type(self, name, description, kind, inputs_='{}', injectors='{}'):
        ...
        payload = {'name': name, 'description': description, 'kind': kind.lower()}
        variables = {'inputs': inputs_, 'injectors': injectors}
    ...
    def create_credential_in_organization(self, organization, name, description, user, team, credential_type, inputs_='{}'):
        ...
        payload = {'name': name, 'description': description, 'organization': organization_.id, 
           'user': user_.id, 
           'team': team_.id, 
    ...
    def create_job_template(self, name, description, inventory, project, playbook, credential, instance_groups=None, host_config_key=None, job_type='run', vault_credential=None, forks=0, limit=0, verbosity=0, extra_vars='', job_tags='', force_handlers=False, skip_tags='', start_at_task='', timeout=0, use_fact_cache=False, ask_diff_mode_on_launch=False, ask_variables_on_launch=False, ask_limit_on_launch=False, ask_tags_on_launch=False, ask_skip_tags_on_launch=False, ask_job_type_on_launch=False, ask_verbosity_on_launch=False, ask_inventory_on_launch=False, ask_credential_on_launch=False, survey_enabled=False, become_enabled=False, diff_mode=False, allow_simultaneous=False):
        ...
        payload = {'name': name, 'description': description, 'inventory': inventory_.id, 
           'project': project_.id, 
           'playbook': playbook, 
