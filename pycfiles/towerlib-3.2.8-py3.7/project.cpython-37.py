...
# [GCC 8.4.0]
# Embedded file name: /Users/ctyfoxylos/SBP/python/towerlib/towerlib/entities/project.py
# Compiled at: 2019-10-18 06:12:55
# Size of source mod 2**32: 15488 bytes
"""
...
from towerlib.towerlibexceptions import InvalidValue, InvalidCredential, InvalidOrganization
from .core import Entity, EntityManager, validate_max_length, validate_range
...
class Project(Entity):
    __doc__ = 'Models the project entity of ansible tower.'
    ...
    def playbooks(self):
        ...
        if not response.ok:
            self._logger.error('Error getting playbooks for project "%s", response was :"%s"', self.name, response.text)
        if response.ok:
    ...
    def credential(self, value):
        ...
        """
        credential = self.organization.get_credential_by_name_with_type_id(value, self.credential._data.get('credential_type'))
        if not credential:
    ...
    def timeout(self, value):
        ...
        if all(conditions):
            ...
            raise InvalidValue('{value} is invalid, must be between {minimum} and {maximum}'.format(value=value, minimum=minimum,
              maximum=maximum))
