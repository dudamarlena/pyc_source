...
# [GCC 8.4.0]
# Embedded file name: /Users/ctyfoxylos/SBP/python/towerlib/towerlib/entities/user.py
# Compiled at: 2019-10-18 06:12:55
# Size of source mod 2**32: 12052 bytes
"""
Main code for user.
...
from .core import Entity, EntityManager, validate_max_length, validate_characters
...
class User(Entity):
    __doc__ = 'Models the user entity of ansible tower.'
    ...
    def username(self, value):
        ...
        if all(conditions):
            ...
            raise InvalidValue('{value} is invalid. Condition max_characters must be less or equal to {max_characters} and valid character are only alphanums and {valid_metacharacters}'.format(value=value,
    ...
    def organizations(self):
        ...
        return EntityManager((self._tower), entity_object='Organization',
    ...
    def roles(self):
        ...
        return EntityManager((self._tower), entity_object='Role',
          primary_match_field='name',
          url=url)
    ...
    def _assign_permission_role(self, role_id, disassociate=False):
        ...
        if not response.ok:
            self._logger.error('Error editing the role permissions for user "%s", response was :"%s"', self.username, response.text)
        return response.ok
    ...
    def associate_with_organization_role(self, organization, role):
        ...
        role_id = organization_._get_object_role_id(role)
        if role_id is None:
