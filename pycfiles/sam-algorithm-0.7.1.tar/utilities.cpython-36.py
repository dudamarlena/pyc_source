...
# [GCC 8.4.0]
# Embedded file name: /home/alec/Desktop/self-assembling-manifold/sam/utilities.py
# Compiled at: 2020-02-13 03:39:35
# Size of source mod 2**32: 13290 bytes
import numpy as np, scipy as sp, os, errno
...
def sparse_pca(X, npcs, mu=None):
    ...
    XL = sp.sparse.linalg.LinearOperator(matvec=matvec, dtype=(X.dtype), matmat=matmat,
      shape=(X.shape),
      rmatvec=rmatvec,
      rmatmat=rmatmat)
...
if UMAP4:
    ...
    def nearest_neighbors(X, n_neighbors=15, metric='correlation', metric_kwds={}, angular=True, seed=0, low_memory=False):
        ...
        if metric in ('cosine', 'correlation', 'dice', 'jaccard', 'll_dirichlet', 'hellinger'):
            angular = True
        random_state = np.random.RandomState(seed=seed)
        rng_state = random_state.randint(INT32_MIN, INT32_MAX, 3).astype(np.int64)
        rp_forest = make_forest(X, n_neighbors, n_trees, rng_state, angular)
        leaf_array = rptree_leaf_array(rp_forest)
        knn_indices, knn_dists = nn_descent(X,
          n_neighbors,
          rng_state,
          max_candidates=60,
          dist=distance_func,
...
def dist_to_nn(d, K):
    ...
    E[(np.tile(np.arange(E.shape[0]).reshape(E.shape[0], -1), (1, x.shape[1])).flatten(), x.flatten())] = M
    E[E < M] = 0
    E[E > 0] = 1
    return E
...
def gen_sparse_knn(knni, knnd, shape=None):
    if shape is None:
        shape = (
         knni.shape[0], knni.shape[0])
    D1 = sp.sparse.lil_matrix(shape)
    D1[(np.tile(np.arange(knni.shape[0])[:, None], (1, knni.shape[1])).flatten(), knni.flatten())] = knnd.flatten()
    D1 = D1.tocsr()
    return D1