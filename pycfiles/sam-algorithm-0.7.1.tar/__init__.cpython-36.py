...
class SAM(object):
    ...
    def __init__(self, counts=None, inplace=False):
        if isinstance(counts, tuple) or isinstance(counts, list):
            ...
            self.adata_raw = AnnData(X=raw_data,
            ...
            if isinstance(counts, pd.DataFrame):
                ...
                if isinstance(counts, AnnData):
                    all_cell_names = np.array(list(counts.obs_names))
                    all_gene_names = np.array(list(counts.var_names))
                    self.adata_raw = counts
                else:
                    if counts is not None:
                        raise Exception("'counts' must be either a tuple/list of (data,gene IDs,cell IDs) or a Pandas DataFrame ofcells x genes")
        if counts is not None:
    ...
    def preprocess_data(self, div=1, downsample=0, sum_norm=None, norm='log', min_expression=1, thresh=0.01, filter_genes=True):
        ...
        if D.getformat() == 'csc':
            ...
            if sum_norm == 'cell_median':
                ...
                if sum_norm == 'gene_median' and norm != 'multinomial':
                    s = D.sum(0).A.flatten()
                    sum_norm = np.median(s[(s > 0)])
                    s[s == 0] = 1
                    D = D.multiply(1 / s[None, :] * sum_norm).tocsr()
                elif sum_norm is not None:
                    if norm != 'multinomial':
            ...
            if filter_genes:
                ...
                c[a] = ct
                keep = np.where(np.logical_and(c / D.shape[0] > thresh, c / D.shape[0] <= 1 - thresh))[0]
    ...
    def load_data(self, filename, transpose=True, save_sparse_file=None, sep=',', calculate_avg=True, **kwargs):
        ...
        if filename.split('.')[(-1)] == 'p':
            ...
            if filename.split('.')[(-1)] != 'h5ad':
                self.adata_raw = AnnData(X=raw_data, obs={'obs_names': all_cell_names}, var={'var_names': all_gene_names})
                if np.unique(all_gene_names).size != all_gene_names.size:
                    self.adata_raw.var_names_make_unique()
                if np.unique(all_cell_names).size != all_cell_names.size:
                    self.adata_raw.obs_names_make_unique()
        ...
        try:
            ...
            if isinstance(projection, str):
                try:
                    ...
                    print('Please create a projection first using run_umap orrun_tsne')
                ...
                if projection is None:
                    try:
                        ...
                        try:
                            ...
                            print('Please create either a t-SNE or UMAP projectionfirst.')
                            return
                ...
                if axes is None:
                    ...
                    axes = plt.gca()
                if c is None:
                    (axes.scatter)(dt[:, 0], dt[:, 1], s=s, linewidth=linewidth, 
                    ...
                    if (isinstance(c[0], str) or isinstance(c[0], np.str_)) and (isinstance(c, np.ndarray) or isinstance(c, list)):
                        ...
                        cax = (axes.scatter)(dt[:, 0], dt[:, 1], c=i, cmap=cmap, s=s, linewidth=linewidth, 
                         edgecolor=edgecolor, **kwargs)
                        if colorbar:
    ...
    def regress_genes(self, PCs):
        ...
        try:
            y = self.adata.X.toarray() - self.regression_pcs[:, ind].dot(self.regression_pca.components_[ind, :] * self.adata.var['weights'].values)
        except BaseException:
            y = self.adata.X.toarray() - self.regression_pcs[:, ind].dot(self.regression_pca.components_[ind, :])

        self.adata.X = sp.csr_matrix(y)

    def run(self, max_iter=10, verbose=True, projection='umap', stopping_condition=0.005, num_norm_avg=50, k=20, distance='correlation', preprocessing='Normalizer', npcs=None, n_genes=None, weight_PCs=True, sparse_pca=False, proj_kwargs={}):
        ...
        if preprocessing not in ('StandardScaler', 'Normalizer', None, 'None'):
            ...
            self.run_args = {'max_iter':max_iter, 
             'verbose':verbose, 
            ...
            if n_genes == None:
                ...
                if numcells > 3000:
                    if n_genes > 3000:
                        n_genes = 3000
                if numcells > 2000 and n_genes > 4500:
                    n_genes = 4500
            ...
            if npcs is None and numcells > 3000:
                ...
                if npcs is None and numcells > 2000:
                    npcs = 250
        if npcs is None and numcells > 1000:
            npcs = 350
        else:
            if npcs is None:
                npcs = 500
        ...
        RINDS = np.random.randint(0, numcells, (k - 1) * numcells).reshape((numcells, k - 1))
        ...
        if verbose:
            ...
            W = self.dispersion_ranking_NN(edm,
              num_norm_avg=1)
            old = np.zeros(W.size)
            new = W
            i = 0
            ...
            while i < max_iter and err > stopping_condition:
                ...
                i += 1
                old = new
                W, wPCA_data, EDM = self.calculate_nnm(D, W, n_genes, preprocessing, npcs, numcells, nnas, weight_PCs, sparse_pca)
                new = W
                err = ((new - old) ** 2).mean() ** 0.5
    ...
    def calculate_nnm(self, D, W, n_genes, preprocessing, npcs, numcells, num_norm_avg, weight_PCs, sparse_pca):
        ...
        if not sparse_pca:
            if numcells > 500:
                g_weighted, pca = ut.weighted_PCA(D_sub, npcs=(min(npcs, min(D.shape))),
            ...
            g_weighted, components = ut.sparse_pca(D_sub, npcs=(min(npcs, min(D.shape) - 1)))
            if weight_PCs:
                ev = g_weighted.var(0)
                ev = ev / ev.max()
    ...
    def run_diff_umap(self, use_rep='X_pca', metric='euclidean', n_comps=15, method='gauss', **kwargs):
        ...
        k = self.run_args.get('k', 20)
        distance = self.run_args.get('distance', 'correlation')
        sc.pp.neighbors((self.adata), use_rep=use_rep, n_neighbors=k, metric=distance,
          method=method)
        sc.tl.diffmap((self.adata), n_comps=n_comps)
        sc.pp.neighbors((self.adata), use_rep='X_diffmap', n_neighbors=k, metric='euclidean',
          method=method)
    ...
    def run_diff_map(self, use_rep='X_pca', metric='euclidean', n_comps=15, method='gauss', **kwargs):
        import scanpy.api as sc
        k = self.run_args.get('k', 20)
        distance = self.run_args.get('distance', 'correlation')
        sc.pp.neighbors((self.adata), use_rep=use_rep, n_neighbors=k, metric=distance,
    ...
    def density_clustering(self, X=None, eps=1, metric='euclidean', **kwargs):
        ...
        if idx1.size > 0 and idx0.size > 0:
            ...
            nnm[(np.tile(np.arange(knn.shape[0])[:, None], (1, knn.shape[1])).flatten(), knn.flatten())] = 1
