...
# [GCC 8.4.0]
# Embedded file name: /home/alec/Desktop/self-assembling-manifold/sam/gui.py
# Compiled at: 2020-02-14 12:39:54
# Size of source mod 2**32: 61159 bytes
import colorlover as cl, numpy as np, scipy.sparse as sp, ipywidgets as widgets, plotly.graph_objs as go
from . import SAM
...
class SAMGUI(object):
    ...
    def init_from_sam(self, sam):
        ...
        self.out = widgets.Output(layout={'border':'1px solid black',  'height':'600px', 
         'width':'400px'})
    ...
    def load_vars_from_sam(self, sam):
        ...
        try:
            self.marker_genes = [
             np.array(list(sam.adata.var_names))[np.argsort(-sam.adata.var['weights'].values)]]
    ...
    def init_preprocess(self):
        ...
        """
        fgenes = widgets.Checkbox(value=(bool(self.preprocess_args.get('filter_genes', True))),
          description='Filter genes')
        fgenes.observe((self.pp_filtergenes), names='value')
        dfts = widgets.Button(description='Set defaults',
          disabled=False,
          tooltip='Set default options',
          icon='')
        dfts.on_click(self.set_pp_defaults)
        l1 = widgets.Label('Expr threshold:')
        expr_thr = widgets.FloatSlider(value=(float(self.preprocess_args.get('thresh', 0.01))),
          min=0,
          max=0.1,
          step=0.005,
          disabled=False,
        ...
        min_expr = widgets.FloatSlider(value=(float(self.preprocess_args.get('min_expression', 1))),
          min=0,
          max=6.0,
          step=0.02,
          disabled=False,
        ...
        if init is None:
            init = 'None'
        sumnorm = widgets.Dropdown(options=[
         'cell_median', 'gene_median', 'None'],
          value=init,
        ...
        load = widgets.Button(description='Load data',
          tooltip='Enter the path to the desired data file you wish to load. Accepted filetypes are .csv (comma), .txt (tab), .h5ad (AnnData), and .p (pickled SAM dictionary).',
          disabled=False)
        load.on_click(self.load_data)
        load_data = widgets.Text(value='',
          layout={'width': '100%%'})
        loada = widgets.Button(description='Load annotations',
          tooltip='Enter the path to the desired annotations file you wish to load. Accepted filetypes are .csv (comma) or .txt (tab).',
          disabled=False)
        loada.on_click(self.load_ann)
        load_dataa = widgets.Text(value='',
          layout={'width': '100%%'})
        loadv = widgets.Button(description='Load gene annotations',
          tooltip='Enter the path to the desired gene annotations file you wish to load. Accepted filetypes are .csv (comma) or .txt (tab).',
          disabled=False)
        loadv.on_click(self.load_vann)
        load_datav = widgets.Text(value='',
          layout={'width': '100%%'})
        pp = widgets.VBox([
    ...
    def set_pp_defaults(self, event):
        ...
        fgenes = self.pp_box.children[0].children[1]
        norm = self.pp_box.children[1]
        sumnorm = self.pp_box.children[2]
        expr_thr = self.pp_box.children[3].children[1]
        min_expr = self.pp_box.children[4].children[1]
    ...
    def init_run_sam(self):
        runb = widgets.Button(description='Run SAM',
          disabled=False,
        ...
        wpca = widgets.Checkbox(value=(bool(self.run_args.get('weight_PCs', True))),
        ...
        l1 = widgets.Button(description='# genes:',
          disabled=True,
        ...
        if init is None:
            init = 3000
        ngenes = widgets.FloatSlider(value=init,
          min=100,
        ...
        l2 = widgets.Button(description='# PCs:',
          disabled=True,
        ...
        if init is None:
            init = 150
        npcs = widgets.FloatSlider(value=init,
          min=10,
          max=500,
          step=1,
          disabled=False,
        ...
        l3 = widgets.Button(description='# neighbors:',
          disabled=True,
        ...
        if init is None:
            init = 20
        knn = widgets.FloatSlider(value=init,
          min=5,
          max=200,
          step=1,
          disabled=False,
        ...
        l4 = widgets.Button(description='num_norm_avg:',
          disabled=True,
        ...
        if init is None:
            init = 50
        nna = widgets.FloatSlider(value=init,
          min=1,
          max=200,
          step=1,
          disabled=False,
        ...
        if init is None:
            init = 'correlation'
        distance = widgets.Dropdown(options=[
    ...
    def init_cs(self):
        cpm = widgets.Dropdown(options=[
         'UMAP', 't-SNE', 'Diffusion map', 'Diffusion UMAP'],
          value='UMAP',
          description='',
          layout={'width': '60%'})
        cp = widgets.Button(description='Compute projection',
        ...
        if 'X_umap' in initl:
            ...
            init = initl[0]
        dpm = widgets.Dropdown(options=initl,
          value=init,
          description='',
          layout={'width': '60%'})
        dp = widgets.Button(description='Display projection',
          tooltip='Display the 2D projection selected in the dropdown menu to the right.',
          layout={'width': '40%'})
        dp.on_click(self.display_projection)
        clm = widgets.Dropdown(options=[
         'Louvain cluster', 'Density cluster', 'Hdbscan cluster', 'Kmeans cluster', 'Leiden cluster'],
          value='Leiden cluster',
          description='',
          layout={'width': '60%'})
        clm.observe(self.rewire_cluster_slider, 'value')
        cl = widgets.Button(description='Cluster',
        ...
        cslider = widgets.FloatSlider(value=1,
          min=0.1,
          max=10,
          step=0.1,
          disabled=False,
        ...
        initl = [''] + initl
        dam = widgets.Dropdown(options=initl,
          value=(initl[0]),
          description='',
          layout={'width': '60%'})
        da = widgets.Button(description='Disp obs ann',
          tooltip='Overlay the annotations selected in the dropdown menu to the right.',
          layout={'width': '40%'})
        da.on_click(self.display_annotation)
        dv = widgets.Button(description='Disp var ann',
        ...
        initl = [''] + initl
        dvm = widgets.Dropdown(options=initl,
          value=(initl[0]),
          description='',
          layout={'width': '60%'})
        irm = widgets.Button(description='Find markers (RF)',
          tooltip='Identify marker genes of selected cells using a random forest classifier.',
          layout={'width': '34%'})
        irm.on_click(self.irm_genes)
        ism = widgets.Button(description='Find markers (SW)',
        ...
        avg = widgets.Checkbox(indent=False, value=True,
          description='Avg expr',
          layout={'width': '20%'})
        log = widgets.Checkbox(indent=False, value=False,
          description='Log cbar',
          layout={'width': '20%'})
        show_colorbar = widgets.Checkbox(indent=False, value=True,
          description='Show cbar',
          layout={'width': '20%'})
        lann = widgets.Button(description='Annotate',
        ...
        lthr = widgets.Button(description='Threshold expr',
          tooltip='Select genes with greater than the chosen expression threshold.',
          disabled=True,
          layout={'width': '30%'})
        thr = widgets.FloatSlider(value=0,
          min=0,
          max=0,
          step=0.05,
          disabled=False,
        ...
        hotkeys = widgets.Button(description='ReadMe',
          ...
          disabled=True)
        amslider = widgets.Button(description='Opacity',
          tooltip='Changes the opacities of unselected points in the current plot.',
          disabled=True,
          layout={'width': '30%'})
        aslider = widgets.FloatSlider(value=0.05,
          min=0.0,
          max=1,
          step=0.01,
          disabled=False,
        ...
        lmslider = widgets.Button(description='Marker size',
          tooltip='Changes the marker sizes in the current plot.',
          disabled=True,
          layout={'width': '30%'})
        mslider = widgets.FloatSlider(value=5,
          min=0.1,
          max=20,
          step=0.1,
          disabled=False,
    ...
    def save_data(self, path):
        ...
        if path != '':
            if path.split('.')[(-1)] == 'h5ad':
                s = self.sams[self.stab.selected_index]
                s.save_anndata(path)
            elif path.split('.')[(-1)] == 'png' or path.split('.')[(-1)] == 'pdf' or path.split('.')[(-1)] == 'eps' or path.split('.')[(-1)] == 'jpg':
                if len(path.split('/')) > 1:
    ...
    def show_expression(self, text):
        ...
        if gene != '':
            try:
                if gene not in s.adata.var_names:
                ...
                if self.cs_box.children[9].children[2].value:
                    if gene not in s.adata.var_names:
                        with self.out:
                            print('X_knn_avg does not exist. Either run sam.dispersion_ranking_NN() or uncheck `avg` in the control panel.')
                else:
                    if gene not in s.adata.var_names:
                        with self.out:
                            print('Gene not found. Check your spelling and capitalization.')
    ...
    def update_colors_expr(self, a, title):
        ...
        f1.update_traces(marker=dict(color=a, colorscale='spectral', reversescale=True, showscale=showscale,
    ...
    def update_colors_anno(self, labels):
        ...
        if issubclass(labels.dtype.type, np.number):
            ...
            if self.cs_box.children[8].children[(-1)].value:
                ...
                showscale = False
        f1 = self.stab.children[self.stab.selected_index]
        f1.update_traces(marker=dict(color=inv, colorscale=x, colorbar_ticks='outside',
            ...
            if val == 'Louvain cluster':
                ...
                if val == 'Density cluster':
                    ...
                    if val == 'Hdbscan cluster':
                        ...
                        if val == 'Leiden cluster':
                            s.leiden_clustering(res=eps)
        self.cs_box.children[4].children[1].options = [
         ''] + list(s.adata.obs.keys())
    ...
    def compute_projection(self, event):
        ...
        if val == 'UMAP':
            ...
            if val == 't-SNE':
                ...
                if val == 'Diffusion map':
                    s.run_diff_map()
                else:
                    if val == 'Diffusion UMAP':
                        s.run_diff_umap()
        self.update_dropdowns(i)
    ...
    def select(self, trace, points, selector):
        if np.array(points.point_inds).size > 0:
            ...
            self.selected_cells[self.stab.selected_index] = np.array(list(self.sams[self.stab.selected_index].adata.obs_names[self.selected[self.stab.selected_index]]))
    ...
    def pick_cells_dd(self, txt):
        if txt['new'] != '':
            ...
            if ratio >= 0.5:
                ...
                sel[als == al] = True
            self.selected_cells[self.stab.selected_index] = np.array(list(self.sams[self.stab.selected_index].adata.obs_names[sel]))
