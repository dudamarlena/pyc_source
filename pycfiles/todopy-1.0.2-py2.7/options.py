...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\todopy\options.py
# Compiled at: 2011-12-15 09:18:31
...
class Options(object):
        ...
        class Optgroup:
            ...
            def __getattr__(self, attr):
                if getattr(self.options.args, attr) != None:
                    ...
                    if self.group in self.options.config and attr in self.options.config[self.group]:
    ...
    def _make_short_opt_for(self, option):
        ...
        for c in re.sub('r[hC]', '', option + option.upper() + 'abcdefghijklmnopqrstuvxyzABCDEFGHIJKLMNOPQRSTUVXYZ'):
            if c == 'h':
                continue
            opt = ('-{}').format(c)
            if opt not in self.used_optdefs:
    ...
    def add_flag(self, flag, **kwargs):
        ...
        parser.add_argument(self._make_short_opt_for(flag), ('--{}').format(flag), dest=flag, action='store_const', const=not default, help=help)
    ...
    def add_option(self, option, **kwargs):
        ...
        help = kwargs['help'] if 'help' in kwargs else None
        parser = self.subparsers[group] if group in self.subparsers else self.parser
        parser.add_argument(self._make_short_opt_for(option), ('--{}').format(option), dest=option, metavar=option.upper(), type=unicode, help=help)
        if group not in self.defaults:
            self.defaults[group] = {}
        if group not in self.valuators:
            self.valuators[group] = {}
    ...
    def parse(self, **kwargs):
        r"""Parse command line and config file.
        ...
        if 'config_file' in kwargs:
            self.parser.add_argument('-C', '--config_file', dest='config_file', type=unicode, default=kwargs['config_file'])
        self.args = self.parser.parse_args(kwargs['argv']) if 'argv' in kwargs else self.parser.parse_args()
    ...
    def add_subcommand(self, command, **kwargs):
        ...
        help = kwargs['help'] if 'help' in kwargs else None
        if not self.sub:
            self.sub = self.parser.add_subparsers(dest='command', title='Subcommands')
    ...
    def add_argument(self, argument, **kwargs):
        ...
        parser = self.subparsers[group] if group in self.subparsers else self.parser
        parser.add_argument(dest, metavar=argument.upper(), type=type, nargs=count, help=help)
        if group not in self.valuators:
            self.valuators[group] = {}
