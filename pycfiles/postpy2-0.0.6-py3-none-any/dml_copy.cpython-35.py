...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/postpy/dml_copy.py
# Compiled at: 2017-02-15 04:26:41
# Size of source mod 2**32: 4249 bytes
__doc__ = 'Copy specific dml statement generators.'
...
class CopyFromCsvBase(ABC):
    ...
    def __init__(self, table, delimiter=',', encoding='utf8', null_str='', header=True, escape_str='\\', quote_char='"', force_not_null=None, force_null=None):
        self.table = table
        self.copy_table, self.copy_name = self.get_copy_table(self.table)
        self.copy_sql = copy_from_csv_sql(self.copy_name, delimiter, encoding, null_str=null_str, header=header, escape_str=escape_str, quote_char=quote_char, force_not_null=force_not_null, force_null=force_null)
...
class BulkDmlPrimaryKey(CopyFromCsvBase):
    ...
    def generate_temp_table_name(self):
        ...
        temp_table_name = self._TEMP_FORMATTER.format(temp_prefix=self.TEMP_PREFIX, table_name=self.table.name, random=rand_char)
...
def copy_from_csv_sql(qualified_name: str, delimiter=',',
