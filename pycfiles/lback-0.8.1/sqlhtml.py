...
# [GCC 8.4.0]
# Embedded file name: /home/mdipierro/make_web2py/web2py/gluon/sqlhtml.py
...
from gluon.dal import DAL, Field, Table, Row, CALLABLETYPES, smart_query, bar_encode, Reference, REGEX_TABLE_DOT_FIELD, Expression
from gluon.storage import Storage
from gluon.utils import md5_hash
from gluon.validators import IS_EMPTY_OR, IS_NOT_EMPTY, IS_LIST_OF, IS_DATE, IS_DATETIME, IS_INT_IN_RANGE, IS_FLOAT_IN_RANGE, IS_STRONG
import gluon.serializers as serializers, datetime, urllib, re, cStringIO
...
def show_if(cond):
    if not cond:
        ...
        if cond.op.__name__ == 'EQ' and cond.second == True or cond.op.__name__ == 'NE' and cond.second == False:
            return (base, ':checked')
        if cond.op.__name__ == 'EQ' and cond.second == False or cond.op.__name__ == 'NE' and cond.second == True:
...
class FormWidget(object):
    ...
    def _attributes(cls, field, widget_attributes, **attributes):
        ...
        """
        attr = dict(_id='%s_%s' % (field.tablename, field.name), _class=cls._class or widget_class.match(str(field.type)).group(), _name=field.name, requires=field.requires)
...
class StringWidget(FormWidget):
    ...
    def widget(cls, field, value, **attributes):
        ...
        """
        default = dict(_type='text', value=value is not None and str(value) or '')
...
class ListWidget(StringWidget):
    ...
    def widget(cls, field, value, **attributes):
        ...
        if isinstance(value, str):
            value = [value]
        nvalue = value or ['']
        items = [ LI(INPUT(_id=_id, _class=_class, _name=_name, value=v, hideerror=k < len(nvalue) - 1, requires=requires), **attributes) for k, v in enumerate(nvalue)
                ]
    ...
    def widget(cls, field, value, **attributes):
        ...
        for r_index in range(rows):
            ...
            for k, v in options[r_index * cols:(r_index + 1) * cols]:
                ...
                tds.append(inner(INPUT(_type='radio', _id=('%s%s' % (field.name, k)), _name=field.name, requires=attr.get('requires', None), hideerror=True, _value=k, value=value, **checked), LABEL(v, _for='%s%s' % (field.name, k))))
        ...
        for r_index in range(rows):
            ...
            for k, v in options[r_index * cols:(r_index + 1) * cols]:
                if k in values:
                    ...
                    r_value = []
                tds.append(inner(INPUT(_type='checkbox', _id='%s%s' % (field.name, k), _name=field.name, requires=attr.get('requires', None), hideerror=True, _value=k, value=r_value), LABEL(v, _for='%s%s' % (field.name, k))))

            opts.append(child(tds))

        if opts:
            opts.append(INPUT(requires=attr.get('requires', None), _style='display:none;', _disabled='disabled', _name=field.name, hideerror=False))
        return parent(*opts, **attr)
...
class PasswordWidget(FormWidget):
    ...
    def widget(cls, field, value, **attributes):
        ...
        see also: :meth:`FormWidget.widget`
        """
        default = dict(_type='password', _value=value and cls.DEFAULT_PASSWORD_DISPLAY or '')
        attr = cls._attributes(field, default, **attributes)
        requires = field.requires
        if not isinstance(requires, (list, tuple)):
            requires = [
    ...
    def widget(cls, field, value, download_url=None, **attributes):
        ...
        if download_url and value:
            ...
            if requires == [] or isinstance(requires, IS_EMPTY_OR):
                inp = DIV(inp, SPAN('[', A(current.T(UploadWidget.GENERIC_DESCRIPTION), _href=url), '|', INPUT(_type='checkbox', _name=field.name + cls.ID_DELETE_SUFFIX, _id=field.name + cls.ID_DELETE_SUFFIX), LABEL(current.T(cls.DELETE_FILE), _for=field.name + cls.ID_DELETE_SUFFIX, _style='display:inline'), ']', _style='white-space:nowrap'), br, image)
            else:
                inp = DIV(inp, SPAN('[', A(cls.GENERIC_DESCRIPTION, _href=url), ']', _style='white-space:nowrap'), br, image)
        return inp
...
class AutocompleteWidget(object):
    ...
    def __init__(self, request, field, id_field=None, db=None, orderby=None, limitby=(0, 10), distinct=False, keyword='_autocomplete_%(tablename)s_%(fieldname)s', min_length=2, help_fields=None, help_string=None):
        ...
        if self.keyword in self.request.vars:
            ...
            if rows:
                if self.is_reference:
                    ...
                    if self.help_fields:
                        options = [ OPTION(self.help_string % dict([ (h.name, s[h.name]) for h in self.fields[:1] + self.help_fields ]), _value=s[id_field.name], _selected=k == 0) for k, s in enumerate(rows)
                                  ]
                    else:
                        options = [ OPTION(s[field.name], _value=s[id_field.name], _selected=k == 0) for k, s in enumerate(rows) ]
                    raise HTTP(200, SELECT(_id=self.keyword, _class='autocomplete', _size=len(rows), _multiple=(len(rows) == 1), *options).xml())
                else:
                    raise HTTP(200, SELECT(_id=self.keyword, _class='autocomplete', _size=len(rows), _multiple=(len(rows) == 1), *[ OPTION(s[field.name], _selected=k == 0) for k, s in enumerate(rows)
                                                                                                                                  ]).xml())
            else:
                raise HTTP(200, '')
...
def formstyle_inline(form, fields):
    ...
    return CAT(DIV(controls, _style='display:inline'), submit_button)
...
class SQLFORM(FORM):
    ...
    """
    widgets = Storage(dict(string=StringWidget, text=TextWidget, json=JSONWidget, password=PasswordWidget, integer=IntegerWidget, double=DoubleWidget, decimal=DecimalWidget, time=TimeWidget, date=DateWidget, datetime=DatetimeWidget, upload=UploadWidget, boolean=BooleanWidget, blob=None, options=OptionsWidget, multiple=MultipleOptionsWidget, radio=RadioWidget, checkboxes=CheckboxesWidget, autocomplete=AutocompleteWidget, list=ListWidget))
    formstyles = Storage(dict(table3cols=formstyle_table3cols, table2cols=formstyle_table2cols, divs=formstyle_divs, ul=formstyle_ul, bootstrap=formstyle_bootstrap, inline=formstyle_inline))
    ...
    def assert_status(self, status, request_vars):
        if not status and self.record and self.errors:
            for key in self.errors.keys():
                if key in self.table and self.table[key].type == 'upload' and request_vars.get(key, None) in (None,
                                                                                                              '') and self.record[key] and key + UploadWidget.ID_DELETE_SUFFIX not in request_vars:
                    del self.errors[key]
    ...
    def __init__(self, table, record=None, deletable=False, linkto=None, upload=None, fields=None, labels=None, col3={}, submit_button='Submit', delete_label='Check to delete', showid=True, readonly=False, comments=True, keepopts=[], ignore_rw=False, record_id=None, formstyle='table3cols', buttons=[
 'submit'], separator=': ', **attributes):
        """
        SQLFORM(db.table,
               record=None,
               fields=['name'],
        ...
        self.fields = fields
        if self.fields[0] != table.fields[0] and isinstance(table, Table) and not keyed:
        ...
        if keyed:
            self.record_id = dict([ (k, record and str(record[k]) or None) for k in table._primarykey
                                  ])
        self.field_parent = {}
        xfields = []
        self.fields = fields
        ...
        if not keyed:
            ...
            self.id_field_name = table._primarykey[0]
        sep = separator or ''
        for fieldname in self.fields:
            ...
            label = LABEL(label, label and sep, _for=field_id, _id=field_id + SQLFORM.ID_LABEL_SUFFIX)
            ...
            if record:
                ...
                if isinstance(default, CALLABLETYPES):
                    default = default()
            cond = readonly or not ignore_rw and not field.writable and field.readable
        ...
        if record and linkto:
            ...
            for rfld in table._referenced_by:
                ...
                if labels and lname in labels:
                    lname = labels[lname]
                widget = A(lname, _class='reference', _href='%s/%s?query=%s' % (linkto, rfld.tablename, query))
                xfields.append((
        ...
        if record and deletable:
            ...
            widget = INPUT(_type='checkbox', _class=css, _id=self.FIELDKEY_DELETE_RECORD, _name=self.FIELDNAME_REQUEST_DELETE)
            xfields.append((
             self.FIELDKEY_DELETE_RECORD + SQLFORM.ID_ROW_SUFFIX,
             LABEL(T(delete_label), separator, _for=self.FIELDKEY_DELETE_RECORD, _id=self.FIELDKEY_DELETE_RECORD + SQLFORM.ID_LABEL_SUFFIX),
             widget,
        ...
        if not readonly:
            if 'submit' in buttons:
                widget = self.custom.submit = INPUT(_type='submit', _value=T(submit_button))
            elif buttons:
        ...
        if callable(formstyle):
            ...
            if defaults and len(args) - len(defaults) == 4 or len(args) == 4:
                ...
                for id, a, b, c in xfields:
                    ...
                    self.field_parent[id] = getattr(b, 'parent', None) if isinstance(b, XmlComponent) else None
    ...
    def accepts(self, request_vars, session=None, formname='%(tablename)s/%(record_id)s', keepvalues=None, onvalidation=None, dbio=True, hideerror=False, detect_record_change=False, **kwargs):
        """
        ...
        if self.readonly:
            ...
            if self.record:
                if keyed:
                    ...
                    record_id = dict((k, request_vars.get(k, None)) for k in self.table._primarykey)
                else:
                    formname_id, record_id = self.record[self.id_field_name], request_vars.get('id', None)
                keepvalues = True
            else:
                if keyed:
                    formname_id = 'create'
                ...
                if formname:
                    formname = formname % dict(tablename=self.table._tablename, record_id=formname_id)
                for fieldname in self.fields:
                    field = self.table[fieldname]
                    ...
                    if not isinstance(requires, (list, tuple)):
                        requires = [
                         requires]
                    [ item.set_self_id(self.record_id) for item in requires if hasattr(item, 'set_self_id') and self.record_id ]

                fields = {}
                for key in self.vars:
                    fields[key] = self.vars[key]

                ret = FORM.accepts(self, request_vars, session, formname, keepvalues, onvalidation, hideerror=hideerror, **kwargs)
                ...
                if self.record_changed and self.detect_record_change:
                    message_onchange = kwargs.setdefault('message_onchange', current.T('A record change was detected. ' + 'Consecutive update self-submissions ' + 'are not allowed. Try re-submitting or ' + 'refreshing the form page.'))
                ...
                if not ret and not auch:
                    for fieldname in self.fields:
                        field = self.table[fieldname]
                        if not field.widget and field.type.startswith('list:') and not OptionsWidget.has_options(field):
                ...
                if record_id and str(record_id) != str(self.record_id):
                    raise SyntaxError("user is tampering with form's record_id: %s != %s" % (
                ...
                for fieldname in self.fields:
                    ...
                    if not self.ignore_rw and not self.table[fieldname].writable:
                        if self.table[fieldname].type == 'boolean' and self.vars.get(fieldname, True) is None:
                    ...
                    if field.type == 'boolean':
                    ...
                    elif field.type == 'password' and self.record and request_vars.get(fieldname, None) == PasswordWidget.DEFAULT_PASSWORD_DISPLAY:
                        continue
                    elif field.type == 'upload':
                        ...
                        if f == '' or f is None:
                            ...
                            if not f:
                                ...
                                f = os.path.join(current.request.folder, os.path.normpath(f))
                    ...
                    if field.type == 'list:string':
                        ...
                        if value is not None and isinstance(self.table, Table) and not keyed:
                            fields[fieldname] = safe_int(value)
                ...
                for fieldname in self.vars:
                    if fieldname != 'id' and fieldname in self.table.fields and fieldname not in fields and fieldname not in request_vars:
                        fields[fieldname] = self.vars[fieldname]
            ...
            if dbio:
                ...
                for field in self.table:
                    if field.name not in fields and field.writable is False and field.update is None and field.compute is None:
                ...
                if keyed:
                    if reduce(lambda x, y: x and y, record_id.values()):
                        if fields:
    ...
    AUTOTYPES = {type(''): (
       ...
       type(True): (
                  'boolean', None), 
       type(1): (
               'integer', IS_INT_IN_RANGE(-1000000000000.0, +1000000000000.0)), 
       type(1.0): (
                 'double', IS_FLOAT_IN_RANGE()), 
       type([]): (
                'list:string', None), 
    ...
    def dictform(dictionary, **kwargs):
        ...
        for key, value in sorted(dictionary.items()):
            ...
            if t:
                fields.append(Field(key, t, requires=requires, default=value))
    ...
    @staticmethod
    def search_menu(fields, search_options=None, prefix='w2p'):
        T = current.T
        ...
        for field in fields:
            ...
            if options:
                ...
                elif (field.type.startswith('reference ') or field.type.startswith('list:reference ')) and hasattr(field.requires, 'options'):
                    value_input = SELECT(*[ OPTION(v, _value=k) for k, v in field.requires.options()
                                          ], **dict(_id=_id))
                elif field.type == 'integer' or field.type.startswith('reference ') or field.type.startswith('list:integer') or field.type.startswith('list:reference '):
                    value_input = SQLFORM.widgets.integer.widget(field, field.default, _id=_id)
                else:
                    value_input = INPUT(_type='text', _id=_id, _class=field.type)
                new_button = INPUT(_type='button', _value=T('New'), _class='btn', _onclick="%s_build_query('new','%s')" % (prefix, field))
                and_button = INPUT(_type='button', _value=T('And'), _class='btn', _onclick="%s_build_query('and','%s')" % (prefix, field))
                or_button = INPUT(_type='button', _value=T('Or'), _class='btn', _onclick="%s_build_query('or','%s')" % (prefix, field))
                close_button = INPUT(_type='button', _value=T('Close'), _class='btn', _onclick="jQuery('#%s').slideUp()" % panel_id)
                criteria.append(DIV(operators, value_input, new_button, and_button, or_button, close_button, _id='%s_%s' % (field_id, name), _class='w2p_query_row hidden', _style='display:inline'))

        criteria.insert(0, SELECT(_id=fields_id, _onchange=("jQuery('.w2p_query_row').hide();jQuery('#%s_'+jQuery('#%s').val().replace('.','-')).show();" % (field_id, fields_id)), _style='float:left', *selectfields))
        fadd = SCRIPT('\n        jQuery(\'#%(fields_id)s input,#%(fields_id)s select\').css(\n            \'width\',\'auto\');\n        jQuery(function(){web2py_ajax_fields(\'#%(fields_id)s\');});\n        function %(prefix)s_build_query(aggregator,a) {\n          var b=a.replace(\'.\',\'-\');\n          var option = jQuery(\'#%(field_id)s_\'+b+\' select\').val();\n          var value = jQuery(\'#%(value_id)s_\'+b).val().replace(\'"\',\'\\\\"\');\n          var s=a+\' \'+option+\' "\'+value+\'"\';\n          var k=jQuery(\'#%(keywords_id)s\');\n          var v=k.val();\n          if(aggregator==\'new\') k.val(s); else k.val((v?(v+\' \'+ aggregator +\' \'):\'\')+s);\n        }\n        ' % dict(prefix=prefix, fields_id=fields_id, keywords_id=keywords_id, field_id=field_id, value_id=value_id))
        return CAT(DIV(_id=panel_id, _style='display:none;', *criteria), fadd)

    @staticmethod
    def grid(query, fields=None, field_id=None, left=None, headers={}, orderby=None, groupby=None, searchable=True, sortable=True, paginate=20, deletable=True, editable=True, details=True, selectable=None, create=True, csv=True, links=None, links_in_grid=True, upload='<default>', args=[], user_signature=True, maxtextlengths={}, maxtextlength=20, onvalidation=None, onfailure=None, oncreate=None, onupdate=None, ondelete=None, sorter_icons=(
 XML('&#x25B2;'), XML('&#x25BC;')), ui='web2py', showbuttontext=True, _class='web2py_grid', formname='web2py_grid', search_widget='default', ignore_rw=False, formstyle='table3cols', exportclasses=None, formargs={}, createargs={}, editargs={}, viewargs={}, selectable_submit_button='Submit', buttons_placement='right', links_placement='right', noconfirm=False, cache_count=None, client_side_delete=False, ignore_common_filters=None):
        if ui == 'jquery-ui':
            ...
            def fetch_count(dbset):
                if cache_count is None or isinstance(cache_count, tuple):
                    if groupby:
                        c = 'count(*)'
                        nrows = db.executesql('select count(*) from (%s) _tmp;' % dbset._select(c, left=left, cacheable=True, groupby=groupby, cache=cache_count)[:-1])[0][0]
                    elif left:
                        c = 'count(*)'
            ...
            if user_signature:
                if not (('/').join(str(a) for a in args) == ('/').join(request.args) or URL.verify(request, user_signature=user_signature, hash_vars=False) or request.args(len(args)) == 'view' and not logged):
                    session.flash = T('not authorized')
                    redirect(referrer)

            def gridbutton(buttonclass='buttonadd', buttontext=T('Add'), buttonurl=url(args=[]), callback=None, delete=None, trap=True, noconfirm=None):
                if showbuttontext:
                    return A(SPAN(_class=ui.get(buttonclass)), SPAN(T(buttontext), _title=T(buttontext), _class=ui.get('buttontext')), _href=buttonurl, callback=callback, delete=delete, noconfirm=noconfirm, _class=ui.get('button'), cid=request.cid)
                else:
                    return A(SPAN(_class=ui.get(buttonclass)), _href=buttonurl, callback=callback, delete=delete, noconfirm=noconfirm, _title=T(buttontext), _class=ui.get('buttontext'), cid=request.cid)
        ...
        def buttons(edit=False, view=False, record=None):
            ...
            if edit and (not callable(edit) or edit(record)):
                args = [
                 'edit', table._tablename, request.args[(-1)]]
                buttons.append(gridbutton('buttonedit', 'Edit', url(args=args)))
            if view:
                args = [
                 'view', table._tablename, request.args[(-1)]]
        ...
        if create and request.args(-2) == 'new':
            ...
            create_form = SQLFORM(table, ignore_rw=ignore_rw, formstyle=formstyle, _class='web2py_form', **sqlformargs)
            create_form.process(formname=formname, next=referrer, onvalidation=onvalidation, onfailure=onfailure, onsuccess=oncreate)
            ...
            if details and request.args(-3) == 'view':
                table = db[request.args[(-2)]]
                record = table(request.args[(-1)]) or redirect(referrer)
                sqlformargs.update(viewargs)
                view_form = SQLFORM(table, record, upload=upload, ignore_rw=ignore_rw, formstyle=formstyle, readonly=True, _class='web2py_form', **sqlformargs)
                res = DIV(buttons(edit=editable, record=record), view_form, formfooter, _class=_class)
                res.create_form = create_form
                res.update_form = update_form
                res.view_form = view_form
                res.search_form = search_form
                res.rows = None
                return res
            if editable and request.args(-3) == 'edit':
                table = db[request.args[(-2)]]
                record = table(request.args[(-1)]) or redirect(URL('error'))
                sqlformargs.update(editargs)
                deletable_ = deletable(record) if callable(deletable) else deletable
                update_form = SQLFORM(table, record, upload=upload, ignore_rw=ignore_rw, formstyle=formstyle, deletable=deletable_, _class='web2py_form', submit_button=T('Submit'), delete_label=T('Check to delete'), **sqlformargs)
                update_form.process(formname=formname, onvalidation=onvalidation, onfailure=onfailure, onsuccess=onupdate, next=referrer)
                res = DIV(buttons(view=details, record=record), update_form, formfooter, _class=_class)
                res.create_form = create_form
                res.update_form = update_form
                res.view_form = view_form
                res.search_form = search_form
                res.rows = None
                return res
            if deletable and request.args(-3) == 'delete':
                table = db[request.args[(-2)]]
            ...
            exportManager = dict(csv_with_hidden_cols=(
             ExporterCSV, 'CSV (hidden cols)'), csv=(
             ExporterCSV, 'CSV'), xml=(
             ExporterXML, 'XML'), html=(
             ExporterHTML, 'HTML'), json=(
             ExporterJSON, 'JSON'), tsv_with_hidden_cols=(
             ExporterTSV, 'TSV (Excel compatible, hidden cols)'), tsv=(
             ExporterTSV, 'TSV (Excel compatible)'))
            ...
            if export_type:
                ...
                if export_type in exportManager and exportManager[export_type]:
                    if request.vars.keywords:
                        try:
                            dbset = dbset(SQLFORM.build_query(fields, request.vars.get('keywords', '')))
                            rows = dbset.select(left=left, orderby=orderby, cacheable=True, *expcolumns)
                ...
                if request.vars.records and not isinstance(request.vars.records, list):
                    request.vars.records = [
                     request.vars.records]
                else:
                    if not request.vars.records:
                        request.vars.records = []
                    ...
                    error = None
                    if create:
                        add = gridbutton(buttonclass='buttonadd', buttontext=T('Add'), buttonurl=url(args=['new', tablename]))
                        if not searchable:
                            console.append(add)
                    else:
                        add = ''
                    if searchable:
                        sfields = reduce(lambda a, b: a + b, [ [ f for f in t if f.readable ] for t in tables ])
                        ...
                        if search_widget == 'default':
                            ...
                            search_widget = lambda sfield, url: CAT(FORM(INPUT(_name='keywords', _value=request.vars.keywords, _id=skeywords_id, _onfocus="jQuery('#%s').change();jQuery('#%s').slideDown();" % (spanel_id, sfields_id)), INPUT(_type='submit', _value=T('Search'), _class='btn'), INPUT(_type='submit', _value=T('Clear'), _class='btn', _onclick="jQuery('#%s').val('');" % skeywords_id), _method='GET', _action=url), search_menu)
                    ...
                    for field in columns:
                        ...
                        if sortable and not isinstance(field, Field.Virtual):
                            ...
                            if order:
                                ...
                                marker = sorter_icons[1]
                            header = A(header, marker, _href=url(vars=dict(keywords=request.vars.keywords or '', order=key)), cid=request.cid)
                ...
                include_buttons_column = details or editable or deletable or links and links_in_grid and not all([ isinstance(link, dict) for link in links ])
                ...
                if paginate and dbset._db._adapter.dbengine == 'google:datastore':
                    ...
                    limitby = None
                try:
                    table_fields = [ field for field in fields if field.tablename in tablenames and not isinstance(field, Field.Virtual)
                                   ]
                    if dbset._db._adapter.dbengine == 'google:datastore':
                        rows = dbset.select(left=left, orderby=orderby, groupby=groupby, limitby=limitby, reusecursor=cursor, cacheable=True, *table_fields)
                        next_cursor = dbset._db.get('_lastcursor', None)
                    else:
                        rows = dbset.select(left=left, orderby=orderby, groupby=groupby, limitby=limitby, cacheable=True, *table_fields)
                except SyntaxError:
                    rows = None
                    next_cursor = None
            ...
            if paginate and dbset._db._adapter.dbengine == 'google:datastore':
                ...
                if next_cursor:
                    ...
                    paginator.append(LI(A('next', _href=url(vars=d), cid=request.cid)))
            elif paginate and paginate < nrows:
            ...
            if rows:
                ...
                for row in rows:
                    ...
                    id = row[field_id]
                    if selectable:
                        trcols.append(INPUT(_type='checkbox', _name='records', _value=id, value=request.vars.records))
                    for field in columns:
                        if not field.readable:
                            continue
                        if field.type == 'blob':
                        ...
                        if field.represent:
                            try:
                                ...
                                try:
                                    value = field.represent(value, row[field.tablename])
                                except KeyError:
                                    pass
                            ...
                            if value:
                                if callable(upload):
                                    value = A(T('file'), _href=upload(value))
                                elif upload:
                                    value = A(T('file'), _href='%s/%s' % (upload, value))
                            else:
                                value = ''
                        if isinstance(value, str):
                    ...
                    if include_buttons_column:
                        ...
                        if deletable and (not callable(deletable) or deletable(row)):
                            row_buttons.append(gridbutton('buttondelete', 'Delete', url(args=['delete', tablename, id]), callback=url(args=['delete', tablename, id]), noconfirm=noconfirm, delete='tr'))
                        if buttons_placement in ('right', 'both'):
                ...
                htmltable = DIV(htmltable, _class='web2py_htmltable', _style='width:100%;overflow-x:auto;-ms-overflow-x:scroll')
                if selectable:
                    if not callable(selectable):
                        ...
                        inputs = [
                         INPUT(_type='submit', _value=T(selectable_submit_button))]
            ...
            if csv and nrows:
                ...
                for k, v in sorted(exportManager.items()):
                    ...
                    link = url2(vars=dict(order=request.vars.order or '', _export_type=k, keywords=request.vars.keywords or ''))
                    export_links.append(A(T(label), _href=link))

                export_menu = DIV(T('Export:'), _class='w2p_export_menu', *export_links)
            else:
                export_menu = None
            res = DIV(console, DIV(htmltable, _class='web2py_table'), _class='%s %s' % (_class, ui.get('widget')))
            if paginator.components:
                res.append(DIV(paginator, _class='web2py_paginator %(header)s %(cornerbottom)s' % ui))
            if export_menu:
    ...
    @staticmethod
    def smartgrid(table, constraints=None, linked_tables=None, links=None, links_in_grid=True, args=None, user_signature=True, divider='>', breadcrumbs_class='', **kwargs):
        """
        @auth.requires_login()
        def index():
        ...
        *** builds a complete interface to navigate all tables links
        ...
        try:
            ...
            while len(request.args) > nargs:
                ...
                if '.' in key:
                    ...
                    if cond:
                        record = db(db[referee]._id == id)(cond).select().first()
                    else:
                        record = db[referee](id)
                    if previous_id:
                        if record[previous_fieldname] != int(previous_id):
                            raise HTTP(400)
                    previous_tablename, previous_fieldname, previous_id = tablename, fieldname, id
                    name = format(db[referee], record)
                    breadcrumbs.append(LI(A(T(db[referee]._plural), cid=request.cid, _href=url()), SPAN(divider, _class='divider'), _class='w2p_grid_breadcrumb_elem'))
        ...
        for rfield in table._referenced_by:
            check[rfield.tablename] = check.get(rfield.tablename, []) + [rfield.name]
            ...
            for item in linked_tables:
                ...
                if tb:
                    ...
                    for fieldname in linked_fieldnames:
                        t = (multiple_links or T)(tb._plural) if 1 else T(tb._plural + '(' + fieldname + ')')
                        args0 = tablename + '.' + fieldname
                        links.append(lambda row, t=t, nargs=nargs, args0=args0: A(SPAN(t), cid=request.cid, _href=url(args=[
                         args0, row[id_field_name]])))

        grid = SQLFORM.grid(query, args=request.args[:nargs], links=links, links_in_grid=links_in_grid, user_signature=user_signature, **kwargs)
        if isinstance(grid, DIV):
            header = table._plural
            next = grid.create_form or grid.update_form or grid.view_form
            breadcrumbs.append(LI(A(T(header), cid=request.cid, _href=url()), SPAN(divider, _class='divider') if next else '', _class='active w2p_grid_breadcrumb_elem'))
            if grid.create_form:
                header = T('New %(entity)s') % dict(entity=table._singular)
            elif grid.update_form:
                header = T('Edit %(entity)s') % dict(entity=format(grid.update_form.table, grid.update_form.record))
            elif grid.view_form:
                header = T('View %(entity)s') % dict(entity=format(grid.view_form.table, grid.view_form.record))
            if next:
                breadcrumbs.append(LI(A(T(header), cid=request.cid, _href=url()), _class='active w2p_grid_breadcrumb_elem'))
...
`<
    ...
    def __init__(self, sqlrows, linkto=None, upload=None, orderby=None, headers={}, truncate=16, columns=None, th_link='', extracolumns=None, selectid=None, renderstyle=False, cid=None, **attributes):
                ...
                if headers is None:
                    ...
                    for c in columns:
                        if isinstance(headers.get(c, c), dict):
                            ...
                            row.append(TH(A(headers.get(c, c), _href=th_link + '?orderby=' + c, cid=cid)))
                        else:
                ...
                for rc, record in enumerate(sqlrows):
                    ...
                    for colname in columns:
                        ...
                        if not field or isinstance(field, (Field.Virtual, Field.Lazy)):
                            ...
                            key = urllib.urlencode(dict([ tablename in record and isinstance(record, Row) and isinstance(record[tablename], Row) and (k, record[tablename][k]) or (k, record[k]) for k in field._table._primarykey
                                                        ]))
