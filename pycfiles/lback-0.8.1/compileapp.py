...
# [GCC 8.4.0]
# Embedded file name: /home/mdipierro/make_web2py/web2py/gluon/compileapp.py
...
"""
import re, fnmatch, os, copy, random, __builtin__
...
import marshal, shutil, imp, logging
...
def LOAD(c=None, f='index', args=None, vars=None, extension=None, target=None, ajax=False, ajax_trap=False, url=None, user_signature=False, timeout=None, times=1, content='loading...', **attr):
    ...
    if url or ajax:
        url = url or URL(request.application, c, f, r=request, args=args, vars=vars, extension=extension, user_signature=user_signature)
        ...
        other_request.env.path_info = '/' + ('/').join([request.application, c, f] + map(str, other_request.args))
        other_request.env.query_string = vars and URL(vars=vars).split('?')[1] or ''
        ...
        js = None
        if ajax_trap:
            link = URL(request.application, c, f, r=request, args=args, vars=vars, extension=extension, user_signature=user_signature)
...
class LoadFactory(object):
    ...
    def __call__(self, c=None, f='index', args=None, vars=None, extension=None, target=None, ajax=False, ajax_trap=False, url=None, user_signature=False, content='loading...', **attr):
        if args is None:
            args = []
        vars = Storage(vars or {})
        import globals
        ...
        if '.' in f:
            f, extension = f.rsplit('.', 1)
        if url or ajax:
            url = url or html.URL(request.application, c, f, r=request, args=args, vars=vars, extension=extension, user_signature=user_signature)
            script = html.SCRIPT('$.web2py.component("%s","%s")' % (url, target), _type='text/javascript')
            ...
            other_request.env.path_info = '/' + ('/').join([request.application, c, f] + map(str, other_request.args))
            other_request.env.query_string = vars and html.URL(vars=vars).split('?')[1] or ''
            other_request.env.http_web2py_component_location = request.env.path_info
            other_request.cid = target
            other_request.env.http_web2py_component_element = target
            other_response.view = '%s/%s.%s' % (c, f, other_request.extension)
            ...
            js = None
            if ajax_trap:
                link = html.URL(request.application, c, f, r=request, args=args, vars=vars, extension=extension, user_signature=user_signature)
                js = "$.web2py.trap_form('%s','%s');" % (link, target)
...
_base_environment_['SQLDB'] = SQLDB
_base_environment_['SQLField'] = SQLField
...
def run_controller_in(controller, function, environment):
    ...
    if os.path.exists(path):
        ...
        paths = (global_settings.gluon_parent,
         abspath('site-packages', gluon=True), abspath('gluon', gluon=True), '')
...
def run_view_in(environment):
    ...
    if not isinstance(view, str):
        ...
        layer = filename
        if is_gae:
            ccode = getcfs(layer, filename, lambda : compile2(parse_template(view, pjoin(folder, 'views'), context=environment), layer))
        else:
            ccode = parse_template(view, pjoin(folder, 'views'), context=environment)
