...
# [GCC 8.4.0]
# Embedded file name: decimal.pyc
...
class Decimal(object):
    ...
    def __new__(cls, value='0', context=None):
        ...
        if isinstance(value, (int, long)):
            ...
            if isinstance(value, (list, tuple)):
                ...
                raise (isinstance(value[0], (int, long)) and value[0] in (0, 1) or ValueError)('Invalid sign.  The first value in the tuple should be an integer; either 0 for a positive number or 1 for a negative number.')
            self._sign = value[0]
            if value[2] == 'F':
                self._int = '0'
                self._exp = value[2]
                ...
                for digit in value[1]:
                    if isinstance(digit, (int, long)):
                        if 0 <= digit <= 9 and (digits or digit != 0):
                            digits.append(digit)
                    else:
    ...
    def __hash__(self):
        ...
        if self._isinteger():
            ...
            return hash((-1) ** op.sign * op.int * pow(10, op.exp, 18446744073709551615))
    ...
    def _fix(self, context):
        ...
        if not self:
            exp_max = [
             context.Emax, Etop][context._clamp]
    ...
    def normalize(self, context=None):
        ...
        if dup._isinfinity():
            ...
            exp_max = [
             context.Emax, context.Etop()][context._clamp]
            end = len(dup._int)
            exp = dup._exp
...
DefaultContext = Context(prec=28, rounding=ROUND_HALF_EVEN, traps=[
 DivisionByZero, Overflow, InvalidOperation], flags=[], Emax=999999999, Emin=-999999999, capitals=1)
