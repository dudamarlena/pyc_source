...
# [GCC 8.4.0]
# Embedded file name: mysql\connector\fabric\connection.pyc
...
from mysql.connector.errors import Error, InterfaceError, NotSupportedError, MySQLFabricError, InternalError
from mysql.connector.cursor import MySQLCursor, MySQLCursorBuffered, MySQLCursorRaw, MySQLCursorBufferedRaw, MySQLCursorPrepared
...
_CNX_PROPERTIES = {'group': (
           str, 'Name of group of servers', None), 
   'key': (
         (
          str, int), 'Sharding key', None), 
   'tables': (
            (
             tuple, list), 'List of tables in query', None), 
   'mode': (
          int, 'Read-Only, Write-Only or Read-Write', None), 
   'shard': (
           str, 'Identity of the shard for direct connection', None), 
   'mapping': (
             str, '', None), 
   'scope': (
           str, 'GLOBAL for accessing Global Group, or LOCAL', SCOPE_LOCAL), 
   'attempts': (
              int, 'Attempts for getting connection', 3), 
   'attempt_delay': (
                   int, 'Seconds to wait between each attempt', 1)}
...
class Fabric(object):
    ...
    def __init__(self, host, port=MYSQL_FABRIC_PORT, connect_attempts=_CNX_ATTEMPT_MAX, connect_delay=_CNX_ATTEMPT_DELAY):
        """Initialize"""
    ...
    def seed(self, host=None, port=None):
        ...
        if self._version_token == version:
            return
        _LOGGER.info(('Loading Fabric configuration version {version}').format(version=version))
        ...
        for fabric in fabrics:
            ...
            if inst_uuid not in self._fabric_instances:
                ...
                _LOGGER.debug(('Added new Fabric server {host}:{port}').format(host=inst.host, port=inst.port))
    ...
    def reset_cache(self, group=None):
        ...
        """
        if group:
            _LOGGER.debug(("Resetting cache for group '{group}'").format(group=group))
    ...
    def lookup_fabric_servers(self, fabric_cnx=None):
        ...
        try:
            fabric_uuid_str, version, ttl, addr_list = inst.proxy.store.lookup_fabrics()
            ...
            msg = err_msg.format(err='No Fabric server available', host=inst.host, port=inst.port)
            raise InterfaceError(msg)
    ...
    def get_group_servers(self, group, use_cache=True):
        ...
        result = []
        try:
            servers = inst.proxy.store.dump_servers(self._version_token, group)[3]
        except (Fault, socket.error) as exc:
            msg = ('Looking up MySQL servers failed for group {group}: {error}').format(error=str(exc), group=group)
            raise InterfaceError(msg)
    ...
    def get_group_server(self, group, mode=None, status=None):
        ...
        if mode in (MODE_WRITEONLY, MODE_READWRITE) or status == STATUS_PRIMARY:
            if not primary:
                ...
                raise InterfaceError((errmsg + ' {query}={value}').format(query='status' if status else 'mode', group=group, value=status or mode))
            return primary
        if not secondary and primary:
            return primary
        else:
            if group in self._group_balancers:
    ...
    def get_sharding_information(self, tables=None, database=None):
        ...
        for table in tables:
            if not isinstance(table, (list, tuple)) and not database:
                raise ValueError(('No database specified for table {0}').format(table))
        ...
        inst = self.get_instance()
        result = inst.proxy.store.dump_sharding_information(self._version_token, (',').join(patterns))
    ...
    def get_shard_server(self, tables, key, scope=SCOPE_LOCAL, mode=None):
        ...
        for dbobj in tables:
            try:
                ...
                raise ValueError(('tables should be given as <database>.<table>, was {0}').format(dbobj))
            ...
            if entry.shard_type == 'RANGE':
                ...
                raise mysql.connector.InterfaceError(('Unsupported sharding type {0}').format(entry.shard_type))
            groups.append(partition['group'])
            if not all(group == groups[0] for group in groups):
                raise mysql.connector.InterfaceError('Tables are located in different shards.')
...
class FabricConnection(object):
    ...
    def __init__(self, fabric, host, port=MYSQL_FABRIC_PORT, connect_attempts=_CNX_ATTEMPT_MAX, connect_delay=_CNX_ATTEMPT_DELAY):
        ...
        if self.is_connected:
            ...
            while counter != attempts:
                ...
                try:
                    ...
                    if counter == attempts:
                        raise InterfaceError(('Connection to MySQL Fabric failed ({0})').format(exc))
                    _LOGGER.debug(('Retrying {host}:{port}, attempts {counter}').format(host=self.host, port=self.port, counter=counter))
    ...
    def __repr__(self):
        return ('{class_}(host={host}, port={port})').format(class_=self.__class__, host=self._host, port=self._port)
...
class MySQLFabricConnection(object):
    ...
    def __getattr__(self, attr):
        ...
        if attr.startswith('cmd_'):
            raise NotSupportedError(('Calling {attr} is not supported for connections managed by MySQL Fabric.').format(attr=attr))
    ...
    def set_property(self, **properties):
        ...
        for name, value in properties.items():
            if name not in _CNX_PROPERTIES:
                ...
                raise ValueError("'group' property can not be set when 'key' is set.")
            elif name == 'key' and value and props['group']:
                raise ValueError("'key' property can not be set when 'group' is set.")
             ...
             MODE_READWRITE, MODE_READONLY):
                ...
                raise TypeError(('{name} is not valid, expeted a {typename})').format(name=name, typename=_CNX_PROPERTIES[name][0].__name__))
            props[name] = value
        ...
        if isinstance(config, Fabric):
            ...
            for required_key in required_keys:
                if required_key not in config:
                    raise ValueError(("Missing configuration parameter '{parameter}' for fabric").format(paramter=required_key))
    ...
    def store_config(self, **kwargs):
        ...
        try:
            pool = mysql.connector.MySQLConnectionPool(pool_name=str(uuid.uuid4()))
            pool.set_config(**test_config)
        except AttributeError as err:
            raise AttributeError(('Connection configuration not valid: {0}').format(err))
    ...
    def _connect(self):
        ...
        if self.is_connected():
            ...
            if props['group'] and props['tables']:
                raise ValueError("Either 'group' or 'tables' property can be set, not both.")
            if not (props['group'] or props['tables']):
                raise ValueError("Either 'group' or 'tables' property needs to be set.")
            ...
            while counter != attempts:
                ...
                try:
                    ...
                    if props['tables']:
                        if props['scope'] == 'LOCAL' and not props['key']:
                            raise ValueError("Scope 'LOCAL' needs key property to be set")
                        mysqlserver = self._fabric.get_shard_server(props['tables'], props['key'], scope=props['scope'], mode=props['mode'])
                    elif props['group']:
                        group = props['group']
                        mysqlserver = self._fabric.get_group_server(group, mode=props['mode'])
                    else:
                        raise InterfaceError('Missing group or key and tables properties')
                except InterfaceError as err:
                    _LOGGER.debug(('Trying to get MySQL server (attempt {0}; {1})').format(counter, err))
                    if counter == attempts:
                        raise InterfaceError(('Error getting connection: {0}').format(err))
                    if attempt_delay > 0:
                ...
                try:
                    ...
                    if counter == attempts:
                        raise InterfaceError(('Reported faulty server to Fabric ({0})').format(err))
                else:
    ...
    def cursor(self, buffered=None, raw=None, prepared=None, cursor_class=None):
        ...
        self._connect()
        if cursor_class:
            raise NotSupportedError('Custom cursors not supported with MySQL Fabric')
        if prepared:
            raise NotSupportedError('Prepared Statements are not supported with MySQL Fabric')
    ...
    def handle_mysql_error(self, exc):
        ...
        if exc.errno in RESET_CACHE_ON_ERROR:
            self.reset_cache()
            raise MySQLFabricError(('Temporary error ({error}); retry transaction').format(error=str(exc)))
        raise exc
