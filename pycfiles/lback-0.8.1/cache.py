...
# [GCC 8.4.0]
# Embedded file name: /home/mdipierro/make_web2py/web2py/gluon/cache.py
...
"""
import time, portalocker, shelve, thread, os, logging, re, hashlib, datetime
try:
...
class CacheInRam(CacheAbstract):
    ...
    def initialize(self):
        ...
        if app not in self.meta_storage:
            self.storage = self.meta_storage[app] = {CacheAbstract.cache_stats_name: {'hit_total': 0, 'misses': 0}}
        else:
    ...
    def clear(self, regex=None):
        ...
        if CacheAbstract.cache_stats_name not in storage.keys():
    ...
    def __call__(self, key, f, time_expire=DEFAULT_TIME_EXPIRE, destroyer=None):
        ...
        if f is None:
            ...
            if item and (dt is None or item[0] > now - dt):
                return item[1]
            if item and item[0] < now - dt and destroyer:
                destroyer(item[1])
            value = f()
            self.locker.acquire()
    ...
    def _open_shelve_and_lock(self):
        ...
        try:
            ...
            try:
                ...
                logger.error('corrupted cache file %s, will try rebuild it' % self.shelve_name)
                storage = None
...
class CacheAction(object):
    ...
    def __call__(self, *a, **b):
        if not self.key:
            ...
            key2 = self.key.replace('%(name)s', self.__name__).replace('%(args)s', str(a)).replace('%(vars)s', str(b))
...
class Cache(object):
    ...
    def action(self, time_expire=DEFAULT_TIME_EXPIRE, cache_model=None, prefix=None, session=False, vars=True, lang=True, user_agent=False, public=True, valid_statuses=None, quick=None):
        """
        Experimental!
                ...
                if current.request.env.request_method != 'GET':
                    ...
                    if http and isinstance(valid_statuses, list):
                        ...
                        if str(status)[0] in '123':
                            send_headers = True
                    if send_headers:
                        headers = {'Pragma': None, 'Expires': expires, 
