...
# [GCC 8.4.0]
# Embedded file name: /home/mdipierro/make_web2py/web2py/gluon/contrib/pymysql/converters.py
...
def convert_timedelta(connection, field, obj):
    ...
    try:
        ...
        tdelta = datetime.timedelta(hours=int(hours), minutes=int(minutes), seconds=int(seconds), microseconds=microseconds)
        return tdelta
    except ValueError:
        return
...
def convert_time(connection, field, obj):
    ...
    try:
        ...
        if '.' in obj:
            ...
            microseconds = int(tail)
        hours, minutes, seconds = obj.split(':')
        return datetime.time(hour=int(hours), minute=int(minutes), second=int(seconds), microsecond=microseconds)
    except ValueError:
        return
