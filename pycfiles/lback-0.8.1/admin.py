...
# [GCC 8.4.0]
# Embedded file name: /home/mdipierro/make_web2py/web2py/gluon/admin.py
...
===========================================
"""
import os, sys, traceback, zipfile, urllib
...
def upgrade(request, url='http://web2py.com'):
    ...
    if not check:
        ...
        if os.path.exists(os.path.join(gluon_parent, 'web2py.exe')):
            ...
            if gluon_parent.endswith('/Contents/Resources/'):
                version_type = 'osx'
...
def add_path_first(path):
    sys.path = [
     path] + [ p for p in sys.path if not p == path and not p == path + '/'
             ]
    if not global_settings.web2py_runtime_gae:
...
def create_missing_app_folders(request):
    if not global_settings.web2py_runtime_gae:
        if request.folder not in global_settings.app_folders:
            for subfolder in ('models', 'views', 'controllers', 'databases', 'modules',
