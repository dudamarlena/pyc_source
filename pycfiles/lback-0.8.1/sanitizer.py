...
# [GCC 8.4.0]
# Embedded file name: /home/mdipierro/make_web2py/web2py/gluon/sanitizer.py
...
class XssCleaner(HTMLParser):
    ...
    def __init__(self, permitted_tags=[
 'a',
 'b',
 'blockquote',
 'br/',
 ...
 'code',
 'pre',
 'img/'], allowed_attributes={'a': ['href', 'title'], 'img': ['src', 'alt'], 'blockquote': [
                'type']}, fmt=AbstractFormatter, strip_disallowed=False):
    ...
    def handle_starttag(self, tag, method, attrs):
        if tag not in self.permitted_tags:
            ...
            if tag in self.allowed_attributes:
                attrs = dict(attrs)
                self.allowed_attributes_here = [ x for x in self.allowed_attributes[tag] if x in attrs and len(attrs[x]) > 0
                                               ]
    ...
    def url_is_acceptable(self, url):
        ...
        """
        parsed = urlparse(url)
        return parsed[0] in self.allowed_schemes and '.' in parsed[1] or parsed[0] in self.allowed_schemes and '@' in parsed[2] or parsed[0] == '' and parsed[2].startswith('/')
 ...
 'strong', 'span'], allowed_attributes={'a': [
   ...
   'td': [
    ...
    if not isinstance(text, basestring):
        return str(text)
    return XssCleaner(permitted_tags=permitted_tags, allowed_attributes=allowed_attributes).strip(text, escape)