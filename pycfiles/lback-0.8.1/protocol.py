...
# [GCC 8.4.0]
# Embedded file name: mysql\connector\protocol.pyc
...
class MySQLProtocol(object):
    ...
    def _scramble_password(self, passwd, seed):
        ...
        try:
            ...
            xored = [ utils.intread(h1) ^ utils.intread(h3) for h1, h3 in zip(hash1, hash3)
                    ]
            hash4 = struct.pack('20B', *xored)
        except Exception as err:
            raise errors.InterfaceError('Failed scrambling password; %s' % err)
    ...
    def _prepare_auth(self, usr, pwd, dbname, flags, seed):
        ...
        if pwd is not None and len(pwd) > 0:
            if isinstance(pwd, unicode):
                pwd = pwd.encode('utf8')
            _password = utils.int1store(20) + self._scramble_password(pwd, seed)
        else:
            _password = '\x00'
    ...
    def make_auth(self, seed, username=None, password=None, database=None, charset=33, client_flags=0, max_allowed_packet=1073741824):
        """Make a MySQL Authentication packet"""
        if not seed:
            raise errors.ProgrammingError('Seed missing')
        auth = self._prepare_auth(username, password, database, client_flags, seed)
        return utils.int4store(client_flags) + utils.int4store(max_allowed_packet) + utils.int1store(charset) + '\x00' * 23 + auth[0] + auth[1] + auth[2]

    def make_auth_ssl(self, charset=33, client_flags=0, max_allowed_packet=1073741824):
        """Make a SSL authentication packet"""
        return utils.int4store(client_flags) + utils.int4store(max_allowed_packet) + utils.int1store(charset) + '\x00' * 23
    ...
    def parse_handshake(self, packet):
        ...
        packet, res['server_version_original'] = utils.read_string(packet, end='\x00')
        ...
        for pair in pairs:
            ...
            try:
                ...
                try:
                    ...
                    raise errors.InterfaceError('%s (%s:%s).' % (errmsg, lbl, val))
    ...
    def _parse_binary_timestamp(self, packet, field):
        ...
        value = None
        if length == 4:
            value = datetime.date(year=struct.unpack('H', packet[1:3])[0], month=ord(packet[3]), day=ord(packet[4]))
        elif length >= 7:
            mcs = 0
            if length == 11:
                mcs = struct.unpack('I', packet[8:length + 1])[0]
            value = datetime.datetime(year=struct.unpack('H', packet[1:3])[0], month=ord(packet[3]), day=ord(packet[4]), hour=ord(packet[5]), minute=ord(packet[6]), second=ord(packet[7]), microsecond=mcs)
    ...
    def _parse_binary_time(self, packet, field):
        ...
        if int(ord(data[0])) == 1:
            days *= -1
        tmp = datetime.timedelta(days=days, seconds=int(ord(data[7])), microseconds=mcs, minutes=int(ord(data[6])), hours=int(ord(data[5])))
        return (
         packet[length + 1:], tmp)
    ...
    def make_stmt_execute(self, statement_id, data=(), parameters=(), flags=0, long_data_used=None):
        ...
        packet = (utils.int4store(statement_id),
