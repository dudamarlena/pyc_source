...
# [GCC 8.4.0]
# Embedded file name: mysql\connector\network.pyc
...
def _prepare_packets(buf, pktnr):
    ...
    pkts.append(struct.pack('<I', buflen)[0:3] + struct.pack('<B', pktnr) + buf)
    return pkts
...
class BaseMySQLSocket(object):
    ...
    def send_plain(self, buf, packet_number=None):
        ...
        for packet in packets:
            try:
                ...
                raise errors.OperationalError(errno=2055, values=(self.get_address(), _strioerror(err)))
    ...
    def send_compressed(self, buf, packet_number=None):
        ...
        if pllen > maxpktlen:
            ...
            zpkts.append(struct.pack('<I', len(zbuf))[0:3] + struct.pack('<B', seqid) + '\x00@\x00' + zbuf)
            tmpbuf = tmpbuf[16384:]
            pllen = len(tmpbuf)
            seqid = seqid + 1
            while pllen > maxpktlen:
                zbuf = zlib.compress(tmpbuf[:maxpktlen])
                zpkts.append(struct.pack('<I', len(zbuf))[0:3] + struct.pack('<B', seqid) + b'\xff\xff\xff' + zbuf)
            ...
            if tmpbuf:
                ...
                zpkts.append(struct.pack('<I', len(zbuf))[0:3] + struct.pack('<B', seqid) + struct.pack('<I', pllen)[0:3] + zbuf)
            del tmpbuf
        else:
            ...
            if pllen > 50:
                ...
                zpkts.append(struct.pack('<I', len(zbuf))[0:3] + struct.pack('<B', 0) + struct.pack('<I', pllen)[0:3] + zbuf)
            else:
                zpkts.append(struct.pack('<I', pllen)[0:3] + struct.pack('<B', 0) + struct.pack('<I', 0)[0:3] + pkt)
    ...
    def recv_compressed(self):
        ...
        try:
            ...
            while header:
                if len(header) < 7:
                    raise errors.InterfaceError(errno=2013)
                zip_payload_length = struct.unpack('<I', header[0:3] + '\x00')[0]
    ...
    def switch_to_ssl(self, ca, cert, key, verify_cert=False):
        ...
        try:
            self.sock = ssl.wrap_socket(self.sock, keyfile=key, certfile=cert, ca_certs=ca, cert_reqs=cert_reqs, do_handshake_on_connect=False, ssl_version=ssl.PROTOCOL_TLSv1)
            self.sock.do_handshake()
        except NameError:
            raise errors.NotSupportedError('Python installation has no SSL support')
        except (ssl.SSLError, IOError) as err:
            raise errors.InterfaceError(errno=2055, values=(self.get_address(), _strioerror(err)))
...
class MySQLUnixSocket(BaseMySQLSocket):
    ...
    def open_connection(self):
        try:
            ...
            raise errors.InterfaceError(errno=2002, values=(self.get_address(), _strioerror(err)))
...
class MySQLTCPSocket(BaseMySQLSocket):
    ...
    def open_connection(self):
        ...
        addrinfo = None
        try:
            addrinfos = socket.getaddrinfo(self.server_host, self.server_port, 0, socket.SOCK_STREAM)
            ...
            if self.force_ipv6 and not addrinfo:
                raise errors.InterfaceError('No IPv6 address found for %s' % self.server_host)
            if not addrinfo:
                addrinfo = addrinfos[0]
        except IOError as err:
            raise errors.InterfaceError(errno=2003, values=(self.get_address(), _strioerror(err)))
