...
# [GCC 8.4.0]
# Embedded file name: /home/mdipierro/make_web2py/web2py/gluon/contrib/minify/minify.py
...
"""
import cssmin, jsmin, os, hashlib, re
...
def fix_links(css, static_path):
    return re.sub('url\\((["\\\'])\\.\\./', 'url(\\1' + static_path, css)


def minify(files, path_info, folder, optimize_css, optimize_js, ignore_concat=[], ignore_minify=[
 '/jquery.js', '/anytime.js']):
    """
    Input:
    ...
    for k, filename in enumerate(files):
        if not filename.startswith('/') or any(filename.endswith(x) for x in ignore_concat):
            ...
            continue
        abs_filename = os.path.join(folder, 'static', filename[len(static_path) + 1:])
        if filename.lower().endswith('.css'):
            ...
            if concat_js:
                contents = read_binary_file(abs_filename)
                if minify_js and not filename.endswith('.min.js') and not any(filename.endswith(x) for x in ignore_minify):
