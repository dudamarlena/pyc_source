...
# [GCC 8.4.0]
# Embedded file name: /home/mdipierro/make_web2py/web2py/gluon/http.py
...
class HTTP(Exception):
    ...
    def cookies2headers(self, cookies):
        if cookies and len(cookies) > 0:
            self.headers['Set-Cookie'] = [ str(cookie)[11:] for cookie in cookies.values() ]
    ...
    def message(self):
        ...
        if 'web2py_error' in self.headers:
            msg += ' [%(web2py_error)s]'
        return msg % dict(status=self.status, defined_status=defined_status.get(self.status), web2py_error=self.headers.get('web2py_error'))
...
def redirect(location='', how=303, client_side=False):
    if location:
        ...
        if client_side and current.request.ajax:
            ...
            raise HTTP(how, 'You are being redirected <a href="%s">here</a>' % loc, Location=loc)
    else:
        from gluon import current
        if client_side and current.request.ajax:
