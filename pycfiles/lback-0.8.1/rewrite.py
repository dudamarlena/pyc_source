...
# [GCC 8.4.0]
# Embedded file name: /home/mdipierro/make_web2py/web2py/gluon/rewrite.py
...
"""
import os, re, logging, traceback, threading, urllib
...
regex_redirect = re.compile('(\\d+)->(.*)')
regex_full_url = re.compile('^(?P<scheme>http|https|HTTP|HTTPS)\\://(?P<host>[^/]*)(?P<uri>.*)')
regex_version = re.compile('^(_[\\d]+\\.[\\d]+\\.[\\d]+)$')
...
def _router_default():
    ...
    router = Storage(default_application='init', applications='ALL', default_controller='default', controllers='DEFAULT', default_function='index', functions=dict(), default_language=None, languages=None, root_static=[
     'favicon.ico', 'robots.txt'], map_static=None, domains=None, exclusive_domain=False, map_hyphen=False, acfe_match='\\w+$', file_match='([-+=@$%\\w]|(?<=[-+=@$%\\w])[./])*$', args_match='([\\w@ -]|(?<=[\\w@ -])[.=])*$')
    return router
...
params_apps = dict()
params = _params_default(app=None)
...
def fixup_missing_path_info(environ):
    ...
    if not eget('HTTP_HOST'):
        environ['HTTP_HOST'] = '%s:%s' % (eget('SERVER_NAME'), eget('SERVER_PORT'))
...
def url_out(request, environ, application, controller, function, args, other, scheme, host, port):
    """assemble and rewrite outgoing URL"""
    if routers:
        acf = map_url_out(request, environ, application, controller, function, args, other, scheme, host, port)
        url = '%s%s' % (acf, other)
    else:
        ...
        for key, uri in THREAD_LOCAL.routes.routes_onerror:
            if key in keys:
                ...
                query_string += 'code=%s&ticket=%s&requested_uri=%s&request_url=%s' % (
                 status, ticket,
                 urllib.quote_plus(request.env.request_uri), request.url)
                if uri.startswith('http://') or uri.startswith('https://'):
                    url = path_info + '?' + query_string
                    message = 'You are being redirected <a href="%s">here</a>'
                    return (
                     HTTP(303, message % url, Location=url), environ)
                if not environ.get('__ROUTES_ONERROR__', False):
                    environ['__ROUTES_ONERROR__'] = True
...
def load(routes='routes.py', app=None, data=None, rdict=None):
    ...
    if isinstance(rdict, dict):
        ...
        for sym in ('routes_onerror', 'routes_apps_raw', 'error_handler', 'error_message',
                    'error_message_ticket', 'default_application', 'default_controller',
                    'default_function', 'logging'):
            if sym in symbols:
                p[sym] = symbols[sym]
...
def compile_regex(k, v, env=None):
    ...
    return (
     re.compile(k, re.DOTALL), v, env or {})
        ...
        if router.functions:
            if isinstance(router.functions, (set, tuple, list)):
                ...
                if isinstance(router.default_function, str):
                    functions.add(router.default_function)
...
def regex_filter_in(e):
    ...
    if routes.routes_in:
        path = regex_uri(e, routes.routes_in, 'routes_in', e['PATH_INFO'])
...
def regex_url_in(request, environ):
    ...
    if match.group('c') == 'static':
        ...
        static_folder = pjoin(request.env.applications_parent, 'applications', application, 'static')
...
def filter_url(url, method='get', remote='0.0.0.0', out=False, app=False, lang=None, domain=(
 None, None), env=False, scheme=None, host=None, port=None):
    """
    ...
    if app:
        ...
        if out:
            ...
            if items[(-1)] == '':
                items.pop()
            if not len(items) >= 3:
                raise AssertionError('at least /a/c/f is required')
                a = items.pop(0)
                c = items.pop(0)
                f = items.pop(0)
                return routers or regex_filter_out(uri, e)
...
class MapUrlIn(object):
    ...
    def map_prefix(self):
        ...
        if prefix:
            ...
            self.args = List(self.args[prefixlen:])
    ...
    def map_app(self):
        ...
        if not base.exclusive_domain and base.applications and arg0 in base.applications:
         ...
         self.host, self.port) in base.domains:
         ...
         self.host, None) in base.domains:
        ...
        if not base._acfe_match.match(self.application):
            raise HTTP(400, THREAD_LOCAL.routes.error_message % 'invalid request', web2py_error="invalid application: '%s'" % self.application)
        if self.application not in routers and (self.application != THREAD_LOCAL.routes.default_application or self.application == 'welcome'):
            raise HTTP(400, THREAD_LOCAL.routes.error_message % 'invalid request', web2py_error="unknown application: '%s'" % self.application)
        ...
        if self.application not in routers:
            ...
            self.router = routers[self.application]
    ...
    def map_root_static(self):
        ...
        if len(self.args) == 1 and self.arg0 in self.router.root_static:
            ...
            root_static_file = pjoin(self.request.env.applications_parent, 'applications', self.application, self.controller, self.arg0)
            log_rewrite('route: root static=%s' % root_static_file)
            return (
             root_static_file, None)
        else:
            return (None, None)
    ...
    def map_controller(self):
        ...
        if not self.router._acfe_match.match(self.controller):
            raise HTTP(400, THREAD_LOCAL.routes.error_message % 'invalid request', web2py_error='invalid controller')
    ...
    def map_static(self):
        ...
        if self.controller != 'static':
            ...
            if bad_static:
                ...
                raise HTTP(400, THREAD_LOCAL.routes.error_message % 'invalid request', web2py_error='invalid static file')
            if self.language:
                static_file = pjoin(self.request.env.applications_parent, 'applications', self.application, 'static', self.language, file)
            if not self.language or not isfile(static_file):
                static_file = pjoin(self.request.env.applications_parent, 'applications', self.application, 'static', file)
            self.extension = None
    ...
    def map_function(self):
        ...
        if isinstance(self.router.default_function, dict):
            ...
            default_function = self.router.default_function
        ...
        if not self.router._acfe_match.match(self.function):
            raise HTTP(400, THREAD_LOCAL.routes.error_message % 'invalid request', web2py_error='invalid function')
        if self.extension and not self.router._acfe_match.match(self.extension):
            raise HTTP(400, THREAD_LOCAL.routes.error_message % 'invalid request', web2py_error='invalid extension')
        return
    ...
    def validate_args(self):
        ...
        for arg in self.args:
            if not self.router._args_match.match(arg):
                raise HTTP(400, THREAD_LOCAL.routes.error_message % 'invalid request', web2py_error='invalid arg <%s>' % arg)

    def sluggify(self):
        """"""
        self.request.env.update((k.lower().replace('.', '_'), v) for k, v in self.env.iteritems())
...
def map_url_in(request, env, app=False):
    ...
    if app:
        ...
        if root_static_file:
            map.update_request()
            return (
             root_static_file, version, map.env)
