...
# [GCC 8.4.0]
# Embedded file name: /home/mdipierro/make_web2py/web2py/gluon/contrib/ipaddr.py
...
def v6_int_to_packed(address):
    ...
    Returns:
        The binary representation of this address.
    """
    return struct.pack('!QQ', address >> 64, address & 18446744073709551615)
...
def collapse_address_list(addresses):
    ...
    return _collapse_address_list_recursive(sorted(addrs + nets, key=_BaseInterface._get_networks_key))
...
class _BaseAddress(_IPAddrBase):
    ...
    def __init__(self, address):
        if not (_compat_has_real_bytes and isinstance(address, bytes)) and '/' in str(address):
...
class _BaseInterface(_IPAddrBase):
    ...
    def __contains__(self, other):
        if self._version != other._version:
            ...
            if isinstance(other, _BaseInterface):
                return self.network_address <= other.network_address and self.broadcast_address >= other.broadcast_address
    ...
    def hostmask(self):
        ...
        if x is None:
            x = ip_address(int(self.netmask) ^ self._ALL_ONES, version=self._version)
    ...
    def address_exclude(self, other):
        ...
        while s1 != other and s2 != other:
            if other in s1:
                ...
                raise AssertionError('Error performing exclusion: s1: %s s2: %s other: %s' % (str(s1), str(s2), str(other)))
        ...
        if s1 == other:
            ...
            assert True == False, 'Error performing exclusion: s1: %s s2: %s other: %s' % (
    ...
    def iter_subnets(self, prefixlen_diff=1, new_prefix=None):
        ...
        if self._prefixlen == self._max_prefixlen:
            ...
            first = ip_network('%s/%s' % (str(self.network_address),
             str(self._prefixlen + prefixlen_diff)), version=self._version)
            yield first
            current = first
            while True:
                ...
                current = ip_network('%s/%s' % (str(new_addr), str(new_prefixlen)), version=self._version)
                yield current
    ...
    def masked(self):
        ...
        return ip_network('%s/%d' % (self.network_address, self._prefixlen), version=self._version)
    ...
    def supernet(self, prefixlen_diff=1, new_prefix=None):
        ...
        if self._prefixlen == 0:
            ...
            t = ip_interface('%s/%d' % (str(self.network_address),
             self.prefixlen - prefixlen_diff), version=self._version)
            return ip_network('%s/%d' % (str(t.network_address), t.prefixlen), version=t._version)
