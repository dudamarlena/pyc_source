...
# [GCC 8.4.0]
# Embedded file name: /home/mdipierro/make_web2py/web2py/gluon/languages.py
...
Plural subsystem is created by Vladyslav Kozlovskyy (Ukraine)
                               <dbdevelop@gmail.com>
"""
import os, re, sys, pkgutil, logging
...
__all__ = [
 'translator', 'findT', 'update_all_languages']
ostat = os.stat
...
regex_plural_dict = re.compile('^{(?P<w>[^()[\\]][^()[\\]]*?)\\((?P<n>[^()\\[\\]]+)\\)}$')
...
def markmin(s):
    ...
    return render(regex_param.sub(markmin_aux, s), sep='br', autolinks=None, id_prefix='')
...
def read_possible_plural_rules():
    ...
    try:
        ...
        for importer, modname, ispkg in pkgutil.iter_modules(package.__path__):
            if len(modname) == 2:
                module = __import__(package.__name__ + '.' + modname, fromlist=[
                 modname])
                lang = modname
                pname = modname + '.py'
                nplurals = getattr(module, 'nplurals', DEFAULT_NPLURALS)
                get_plural_id = getattr(module, 'get_plural_id', DEFAULT_GET_PLURAL_ID)
                construct_plural_form = getattr(module, 'construct_plural_form', DEFAULT_CONSTRUCT_PLURAL_FORM)
...
def read_possible_languages_aux(langdir):
    ...
    def get_lang_struct(lang, langcode, langname, langfile_mtime):
        if lang == 'default':
            ...
            real_lang = lang
        prules_langcode, nplurals, get_plural_id, construct_plural_form = PLURAL_RULES.get(real_lang[:2], ('default',
         DEFAULT_NPLURALS,
         DEFAULT_GET_PLURAL_ID,
         DEFAULT_CONSTRUCT_PLURAL_FORM))
        if prules_langcode != 'default':
            pluraldict_fname, pluraldict_mtime = plurals.get(real_lang, plurals.get(real_lang[:2], (
    ...
    for fname in flist:
        if regex_langfile.match(fname) or fname == 'default.py':
            ...
            lang = fname[:-3]
            langcode = d.get('!langcode!', lang if lang != 'default' else DEFAULT_LANGUAGE)
    ...
    if 'default' not in langs:
        langs['default'] = get_lang_struct('default', DEFAULT_LANGUAGE, DEFAULT_LANGUAGE_NAME, 0)
...
def read_possible_languages(langpath):
    return getcfs('langs:' + langpath, langpath, lambda : read_possible_languages_aux(langpath))
...
def read_plural_dict(filename):
    return getcfs('plurals:' + filename, filename, lambda : read_plural_dict_aux(filename))
...
class lazyT(object):
    ...
    def __str__(self):
        return str(self.T.apply_filter(self.m, self.s, self.f, self.t) if self.M else self.T.translate(self.m, self.s))
...
class translator(object):
    ...
    def get_possible_languages(self):
        ...
        return list(set(self.current_languages + [ lang for lang in read_possible_languages(self.langpath).iterkeys() if lang != 'default'
                                                 ]))
    ...
    def force(self, *languages):
        ...
        def set_plural(language):
            ...
            if lang_info:
                pname, pmtime, self.plural_language, self.nplurals, self.get_plural_id, self.construct_plural_form = lang_info[3:]
                pdict = {}
                if pname:
        ...
        if languages:
            all_languages = set(lang for lang in pl_info.iterkeys() if lang != 'default') | set(self.current_languages)
    ...
    def __get_otherT__(self, language=None, namespace=None):
        ...
        try:
            ...
            otherT = self.otherTs[index] = translator(self.langpath, self.http_accept_language)
            if language:
    ...
    def apply_filter(self, message, symbols={}, filter=None, ftag=None):
        ...
        message = get_from_cache(self.cache, prefix + message, lambda : get_tr(message, prefix, filter))
        if symbols or symbols == 0 or symbols == '':
            if isinstance(symbols, dict):
                symbols.update((key, xmlescape(value).translate(ttab_in)) for key, value in symbols.iteritems() if not isinstance(value, NUMBERS))
            else:
                if not isinstance(symbols, tuple):
                    symbols = (
                     symbols,)
                symbols = tuple((value if isinstance(value, NUMBERS) else xmlescape(value).translate(ttab_in)) for value in symbols)
            message = self.params_substitution(message, symbols)
        return XML(message.translate(ttab_out))

    def M(self, message, symbols={}, language=None, lazy=None, filter=None, ftag=None, ns=None):
        """
    ...
    def params_substitution(self, message, symbols):
        ...
        def sub_plural(m):
            r"""string in %{} is transformed by this rules:
    ...
    def translate(self, message, symbols):
        ...
        if symbols or symbols == 0 or symbols == '':
            if isinstance(symbols, dict):
                symbols.update((key, str(value).translate(ttab_in)) for key, value in symbols.iteritems() if not isinstance(value, NUMBERS))
            else:
                if not isinstance(symbols, tuple):
                    symbols = (
                     symbols,)
                symbols = tuple((value if isinstance(value, NUMBERS) else str(value).translate(ttab_in)) for value in symbols)
            message = self.params_substitution(message, symbols)
...
def findT(path, language=DEFAULT_LANGUAGE):
    ...
    for filename in listdir(mp, '^.+\\.py$', 0) + listdir(cp, '^.+\\.py$', 0) + listdir(vp, '^.+\\.html$', 0) + listdir(mop, '^.+\\.py$', 0):
    ...
    if '!langname!' not in sentences:
        sentences['!langname!'] = DEFAULT_LANGUAGE_NAME if language in ('default', DEFAULT_LANGUAGE) else sentences['!langcode!']
