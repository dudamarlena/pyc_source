...
# [GCC 8.4.0]
# Embedded file name: /home/mdipierro/make_web2py/web2py/gluon/utf8.py
...
def sort_key(s):
    ...
    try:
        ...
        sort_key = lambda s: unicode_sort_key(unicode(s, 'utf-8') if isinstance(s, str) else s)
    except:
...
class Utf8(str):
    ...
    def __new__(cls, content='', codepage='utf-8'):
        if isinstance(content, unicode):
            ...
            if codepage in ('utf-8', 'utf8') or isinstance(content, cls):
                return str.__new__(cls, content)
            return str.__new__(cls, unicode(content, codepage).encode('utf-8'))

    def __repr__(self):
        ur""" # note that we use raw strings to avoid having to use double back slashes below
    ...
    def __add__(self, other):
        return str.__new__(Utf8, str.__add__(self, unicode.encode(other, 'utf-8') if isinstance(other, unicode) else other))
    ...
    def join(self, iter):
        return str.__new__(Utf8, str.join(self, [ Utf8(c) for c in list(unicode(iter, 'utf-8') if isinstance(iter, str) else iter)
                                                ]))
    ...
    def count(self, sub, start=0, end=None):
        ...
        return unistr.count(unicode(sub, 'utf-8') if isinstance(sub, str) else sub, start, len(unistr) if end is None else end)
    ...
    def find(self, sub, start=None, end=None):
        return unicode(self, 'utf-8').find(unicode(sub, 'utf-8') if isinstance(sub, str) else sub, start, end)

    def ljust(self, width, fillchar=' '):
        return str.__new__(Utf8, unicode(self, 'utf-8').ljust(width, unicode(fillchar, 'utf-8') if isinstance(fillchar, str) else fillchar).encode('utf-8'))
    ...
    def rfind(self, sub, start=None, end=None):
        return unicode(self, 'utf-8').rfind(unicode(sub, 'utf-8') if isinstance(sub, str) else sub, start, end)

    def rindex(self, string):
        return unicode(self, 'utf-8').rindex(string if isinstance(string, unicode) else unicode(string, 'utf-8'))

    def rjust(self, width, fillchar=' '):
        return str.__new__(Utf8, unicode(self, 'utf-8').rjust(width, unicode(fillchar, 'utf-8') if isinstance(fillchar, str) else fillchar).encode('utf-8'))
    ...
    def rsplit(self, sep=None, maxsplit=-1):
        return [ str.__new__(Utf8, part) for part in str.rsplit(self, None if sep is None else Utf8(sep), maxsplit)
               ]

    def split(self, sep=None, maxsplit=-1):
        return [ str.__new__(Utf8, part) for part in str.split(self, None if sep is None else Utf8(sep), maxsplit)
               ]
    ...
    if hasattr(str, 'format'):
        ...
        def format(self, *args, **kwargs):
            ...
            kwargs = dict((unicode(k, 'utf-8') if isinstance(k, str) else k, unicode(v, 'utf-8') if isinstance(v, str) else v) for k, v in kwargs.iteritems())
            return str.__new__(Utf8, unicode(self, 'utf-8').format(*args, **kwargs).encode('utf-8'))

    def __mod__(self, right):
        if isinstance(right, tuple):
            right = tuple((unicode(v, 'utf-8') if isinstance(v, str) else v) for v in right)
        elif isinstance(right, dict):
            right = dict((unicode(k, 'utf-8') if isinstance(k, str) else k, unicode(v, 'utf-8') if isinstance(v, str) else v) for k, v in right.iteritems())
