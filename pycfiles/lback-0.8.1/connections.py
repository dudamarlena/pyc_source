...
# [GCC 8.4.0]
# Embedded file name: /home/mdipierro/make_web2py/web2py/gluon/contrib/pymysql/connections.py
...
import struct, sys, os, ConfigParser
try:
...
def dump_packet(data):
    ...
    for d in dump_data:
        print (' ').join(map(lambda x: '%02X' % byte2int(x), d)) + '   ' * (16 - len(d)) + '  ' + (' ').join(map(lambda x: '%s' % is_ascii(x), d))
...
def unpack_int32(n):
    try:
        return struct.unpack('B', n[0])[0] + (struct.unpack('B', n[1])[0] << 8) + (struct.unpack('B', n[2])[0] << 16) + (struct.unpack('B', n[3])[0] << 24)
    except TypeError:
...
def unpack_int64(n):
    try:
        return struct.unpack('B', n[0])[0] + (struct.unpack('B', n[1])[0] << 8) + (struct.unpack('B', n[2])[0] << 16) + (struct.unpack('B', n[3])[0] << 24) + (struct.unpack('B', n[4])[0] << 32) + (struct.unpack('B', n[5])[0] << 40) + (struct.unpack('B', n[6])[0] << 48) + (struct.unpack('B', n[7])[0] << 56)
    except TypeError:
        return n[0] + (n[1] << 8) + (n[2] << 16) + (n[3] << 24) + (n[4] << 32) + (n[5] << 40) + (n[6] << 48) + (n[7] << 56)
  ...
  for reading/parsing the packet results."""
    ...
    def advance(self, length):
        ...
        if new_position < 0 or new_position > len(self.__data):
            raise Exception('Invalid advance amount (%s) for cursor.  Position=%s' % (
             length, new_position))
    ...
    def is_eof_packet(self):
        return byte2int(self.get_bytes(0)) == 254
...
class MySQLResult(object):
    ...
    def _read_rowdata_packet_unbuffered(self):
        if self.unbuffered_active == False:
            return
