...
# [GCC 8.4.0]
# Embedded file name: mysql\connector\cursor.pyc
# Compiled at: 2014-07-26 22:44:23
"""Cursor classes
"""
import sys, weakref, re, itertools
from mysql.connector import errors
RE_SQL_COMMENT = re.compile('\\/\\*.*\\*\\/')
RE_SQL_ON_DUPLICATE = re.compile('\\s*ON DUPLICATE KEY.*$')
RE_SQL_INSERT_VALUES = re.compile('.*VALUES\\s*(\\(.*\\)).*', re.I | re.M | re.S)
RE_SQL_INSERT_STMT = re.compile('INSERT\\s+INTO', re.I)
RE_SQL_SPLIT_STMTS = re.compile(';(?=(?:[^"\'`]*["\'`][^"\'`]*["\'`])*[^"\'`]*$)')
RE_SQL_FIND_PARAM = re.compile('%s(?=(?:[^"\'`]*["\'`][^"\'`]*["\'`])*[^"\'`]*$)')
...
class MySQLCursor(CursorBase):
    ...
    def _row_to_python(self, rowdata, desc=None):
        ...
        try:
            ...
            raise errors.InterfaceError('Failed converting row to Python types; %s' % err)
        else:
            return tuple(res)
    ...
    def _handle_noresultset(self, res):
        ...
        try:
            ...
            raise errors.ProgrammingError('Failed handling non-resultset; %s' % err)
    ...
    def _execute_iter(self, query_iter):
        ...
        for result, stmt in itertools.izip(query_iter, iter(self._executed_list)):
            self._reset_result()
        ...
        if not operation:
            ...
            if params is not None:
                try:
                    ...
                    raise errors.ProgrammingError('Wrong number of arguments during string formatting')
    ...
    def executemany(self, operation, seq_params):
        ...
        if self._have_unread_result():
            ...
            if len(RE_SQL_SPLIT_STMTS.split(operation)) > 1:
                raise errors.InternalError('executemany() does not support multiple statements')
            if re.match(RE_SQL_INSERT_STMT, operation):
                if not seq_params:
                    self._rowcount = 0
                    return
                tmp = re.sub(RE_SQL_ON_DUPLICATE, '', re.sub(RE_SQL_COMMENT, '', operation))
                ...
                operation = operation.replace(matches.group(1), (',').join(values), 1)
    ...
    def _fetch_warnings(self):
        ...
        try:
            ...
            raise errors.InterfaceError, errors.InterfaceError('Failed getting warnings; %s' % err), sys.exc_info()[2]
    ...
    def _fetch_row(self):
        ...
        if not self._have_unread_result():
            ...
            if self._nextrow == (None, None):
                row, eof = self._connection.get_row(binary=self._binary, columns=self.description)
            else:
                row, eof = self._nextrow
            if row:
                self._nextrow = self._connection.get_row(binary=self._binary, columns=self.description)
                eof = self._nextrow[1]
                if eof is not None:
...
class MySQLCursorPrepared(MySQLCursor):
    ...
    def execute(self, operation, params=(), multi=False):
        ...
        if self._prepared['parameters'] and not params:
            ...
            if len(self._prepared['parameters']) != len(params):
                raise errors.ProgrammingError(errno=1210, msg='Incorrect number of arguments executing prepared statement')
            res = self._connection.cmd_stmt_execute(self._prepared['statement_id'], data=params, parameters=self._prepared['parameters'])
    ...
    def fetchall(self):
        if not self._have_unread_result():
            raise errors.InterfaceError('No result set to fetch from.')
        rows, eof = self._connection.get_rows(binary=self._binary, columns=self.description)
