...
# [GCC 8.4.0]
# Embedded file name: /var/www/dev/lback/dal.py
# Compiled at: 2015-11-19 23:54:29
"""
...
__all__ = [
 'DAL', 'Field']
DEFAULTLENGTH = {'string': 512, 'password': 512, 
   'upload': 512, 
   'text': 2 ** 15, 
   'blob': 2 ** 31}
TIMINGSSIZE = 100
SPATIALLIBS = {'Windows': 'libspatialite', 'Linux': 'libspatialite.so', 
...
class BaseAdapter(ConnectionPool):
    ...
    T_SEP = ' '
    QUOTE_TEMPLATE = '"%s"'
    types = {'boolean': 'CHAR(1)', 'string': 'CHAR(%(length)s)', 
       'text': 'TEXT', 
       'json': 'TEXT', 
        ...
        for key in keys:
            ...
            if key not in sql_fields_old:
                ...
                if self.dbengine in ('postgres', ) and sql_fields[key]['type'].startswith('geometry'):
                    ...
                    query = [
                     'ALTER TABLE %s ADD %s %s;' % (table.sqlsafe, key,
    ...
    def _select(self, query, fields, attributes):
        ...
        if limitby and not groupby and tablenames and orderby_on_limitby and not orderby:
            sql_o += ' ORDER BY %s' % (', ').join([ self.db[t].sqlsafe + '.' + self.db[t][x].sqlsafe_name for t in tablenames for x in hasattr(self.db[t], '_primarykey') and self.db[t]._primarykey or [
             '_id']
                                                  ])
    ...
    def build_parsemap(self):
        self.parsemap = {'id': self.parse_id, 'integer': self.parse_integer, 
...
class OracleAdapter(BaseAdapter):
    drivers = ('cx_Oracle', )
    commit_on_alter_table = False
    types = {'boolean': 'CHAR(1)', 'string': 'VARCHAR2(%(length)s)', 
       'text': 'CLOB', 
       'json': 'CLOB', 
...
class MSSQLAdapter(BaseAdapter):
    ...
    T_SEP = 'T'
    QUOTE_TEMPLATE = '"%s"'
    types = {'boolean': 'BIT', 'string': 'VARCHAR(%(length)s)', 
       'text': 'TEXT', 
       'json': 'TEXT', 
       'password': 'VARCHAR(%(length)s)', 
       'blob': 'IMAGE', 
...
class MSSQL2Adapter(MSSQLAdapter):
    drivers = ('pyodbc', )
    types = {'boolean': 'CHAR(1)', 'string': 'NVARCHAR(%(length)s)', 
       'text': 'NTEXT', 
       'json': 'NTEXT', 
...
class VerticaAdapter(MSSQLAdapter):
    drivers = ('pyodbc', )
    T_SEP = ' '
    types = {'boolean': 'BOOLEAN', 'string': 'VARCHAR(%(length)s)', 
       'text': 'BYTEA', 
...
class SybaseAdapter(MSSQLAdapter):
    drivers = ('Sybase', )
    types = {'boolean': 'BIT', 'string': 'CHAR VARYING(%(length)s)', 
       'text': 'TEXT', 
       'json': 'TEXT', 
...
class TeradataAdapter(BaseAdapter):
    ...
    types = {'boolean': 'CHAR(1)', 'string': 'VARCHAR(%(length)s)', 
       ...
       'big-id': 'BIGINT GENERATED ALWAYS AS IDENTITY', 
...
class CubridAdapter(MySQLAdapter):
    ...
    def __init__(self, db, uri, pool_size=0, folder=None, db_codec='UTF-8', credential_decoder=IDENTITY, driver_args={}, adapter_args={}, do_connect=True, after_connection=None):
        ...
        def connector(host=host, port=port, db=db, user=user, passwd=passwd, driver_args=driver_args):
...
class GoogleDatastoreAdapter(NoSQLAdapter):
    ...
    def __init__(self, db, uri, pool_size=0, folder=None, db_codec='UTF-8', credential_decoder=IDENTITY, driver_args={}, adapter_args={}, do_connect=True, after_connection=None):
        ...
        if self.use_ndb is True:
            self.types.update({'boolean': ndb.BooleanProperty, 'string': lambda **kwargs: ndb.StringProperty(**kwargs), 
    ...
    def NOT(self, first):
        nops = {self.EQ: self.NE, self.NE: self.EQ, 
           self.LT: self.GE, 
           self.GT: self.LE, 
           self.LE: self.GT, 
           self.GE: self.LT}
    ...
    def select_raw(self, query, fields=None, attributes=None, count_only=False):
        ...
        if count_only:
            items = [
             len(items) if isinstance(items, list) else items.count()]
...
class MongoDBAdapter(NoSQLAdapter):
    ...
    def __init__(self, db, uri='mongodb://127.0.0.1:5984/db', pool_size=0, folder=None, db_codec='UTF-8', credential_decoder=IDENTITY, driver_args={}, adapter_args={}, do_connect=True, after_connection=None):
        self.db = db
        self.uri = uri
        if do_connect:
            self.find_driver(adapter_args)
    ...
    def object_id(self, arg=None):
        ...
        if isinstance(arg, basestring):
            ...
            if arg.isdigit() and not rawhex:
                ...
                arg = int('0x%sL' % ('').join([ self.random.choice('0123456789abcdef') for x in range(24)
                                              ]), 0)
            elif arg.isalnum():
    ...
    def parse_reference(self, value, field_type):
        ...
        return super(MongoDBAdapter, self).parse_reference(value, field_type)

    def parse_id(self, value, field_type):
        if isinstance(value, self.ObjectId):
            value = long(str(value), 16)
        return super(MongoDBAdapter, self).parse_id(value, field_type)
        ...
        if fieldtype == 'date':
            ...
            if fieldtype == 'blob':
                ...
                if not isinstance(value, Binary):
                    ...
                    return Binary(value)
                return value
            if isinstance(fieldtype, basestring) and fieldtype.startswith('list:'):
                if fieldtype.startswith('list:reference'):
                    ...
                    return newval
                return value
            if isinstance(fieldtype, basestring) and fieldtype.startswith('reference') or isinstance(fieldtype, Table) or fieldtype == 'id':
    ...
    def count(self, query, distinct=None, snapshot=True):
        ...
        if not isinstance(query, Query):
            raise SyntaxError('Not Supported')
        tablename = self.get_table(query)
        return long(self.select(query, [self.db[tablename]._id], {}, count=True, snapshot=snapshot)['count'])

    def expand(self, expression, field_type=None):
        if isinstance(expression, Query) and isinstance(expression.first, Field):
            if expression.first.type == 'id' or 'reference' in expression.first.type:
                if expression.first.type == 'id':
                    expression.first.name = '_id'
                if isinstance(expression.second, (tuple, list, set)):
                    expression.second = [ self.object_id(item) for item in expression.second ]
                else:
    ...
    def select(self, query, fields, attributes, count=False, snapshot=False):
        ...
        if count:
            ...
            mongo_list_dicts = ctable.find(mongoqry_dict, mongofields_dict, skip=limitby_skip, limit=limitby_limit, sort=mongosort_list, snapshot=snapshot)
            rows = []
            colnames = []
            newnames = []
            for field in fields:
    ...
    def update(self, tablename, query, fields, safe=None):
        ...
        modify = {'$set': dict((k.name, self.represent(v, k.type)) for k, v in fields if k.name not in ('_id',
                                                                              'id'))}
        try:
            result = self.connection[tablename].update(filter, modify, multi=True, safe=safe)
            if safe:
                try:
                    return result['n']
    ...
    def BELONGS(self, first, second):
        ...
        if second == [] or second == () or second == set():
            return {1: 0}
    ...
    def ILIKE(self, first, second):
        val = second if isinstance(second, self.ObjectId) else {'$regex': second.replace('%', ''), 
           '$options': 'i'}
    ...
    def CONTAINS(self, first, second, case_sensitive=False):
        val = second if isinstance(second, self.ObjectId) else {'$regex': '.*' + re.escape(self.expand(second, 'string')) + '.*'}
        return {self.expand(first): val}
...
class IMAPAdapter(NoSQLAdapter):
    ...
    def __init__(self, db, uri, pool_size=0, folder=None, db_codec='UTF-8', credential_decoder=IDENTITY, driver_args={}, adapter_args={}, do_connect=True, after_connection=None):
        ...
        self.imap4 = None
        uri = uri.split('://')[1]
        self.flags = {'deleted': '\\Deleted', 'draft': '\\Draft', 'flagged': '\\Flagged', 
           'recent': '\\Recent', 'seen': '\\Seen', 
           'answered': '\\Answered'}
        self.search_fields = {'id': 'MESSAGE', 'created': 'DATE', 'uid': 'UID', 
           'sender': 'FROM', 'to': 'TO', 
           'cc': 'CC', 'bcc': 'BCC', 
           'content': 'TEXT', 'size': 'SIZE', 
           'deleted': '\\Deleted', 'draft': '\\Draft', 
           'flagged': '\\Flagged', 'recent': '\\Recent', 
           'seen': '\\Seen', 'subject': 'SUBJECT', 
           'answered': '\\Answered', 'mime': None, 
           'email': None, 'attachments': None}
        db['_lastsql'] = ''
        m = self.REGEX_URI.match(uri)
        user = m.group('user')
                ...
                if search_result[0] == 'OK':
                    ...
                    if len(messages_set) > 0:
                        for uid in messages_set:
                            ...
                            if typ == 'OK':
                                fr = {'message': int(data[0][0].split()[0]), 'uid': long(uid), 'email': email.message_from_string(data[0][1]), 
                        ...
                        if '%s.attachments' % tablename in colnames or '%s.content' % tablename in colnames:
                            ...
                            if payload:
                                ...
                                if (filename or 'text' not in maintype) and '%s.attachments' % tablename in colnames:
                                    values.update({'payload': payload, 'filename': filename, 
...
ADAPTERS = {'sqlite': SQLiteAdapter, 'spatialite': SpatiaLiteAdapter, 
...
class DAL(object):
    ...
    def parse_as_rest(self, patterns, args, vars, queries=None, nested_select=True):
        ...
        if ('/').join(args) == 'patterns':
            return Row({'status': 200, 'pattern': 'list', 'error': None, 
               'response': patterns})
        else:
                    ...
                    if re1.match(tag):
                        ...
                        if re2.match(tag) and args[i] == tag[:tag.find('[')]:
                            ...
                            if '.' in ref and otable:
                                ...
                                if nested_select:
                                    try:
                                        ...
                                        return Row({'status': 400, 'pattern': pattern, 'error': 'invalid path', 
                                           'response': None})
                            ...
                            if not db[table][field].readable:
                                return Row({'status': 418, 'pattern': pattern, 'error': "I'm a teapot", 
                                   'response': None})
                            try:
                            ...
                            return Row({'status': 404, 'pattern': pattern, 'error': 'no record found', 
                               'response': None})
                        elif tag != args[i]:
                            break
                        otable = table
        ...
        if _primarykey is not None:
            ...
            if db and db._adapter.uploads_in_blob is True:
                ...
                for field in fields:
                    ...
                    if isinstance(field, Field) and field.type == 'upload' and fn is True and not field.uploadfs:
    ...
    def _build_query(self, key):
        ...
        for k, v in key.iteritems():
            if k in self._primarykey:
                ...
                raise SyntaxError('Field %s is not part of the primary key of %s' % (
                 k, self._tablename))
...
class Field(Expression):
    ...
    def retrieve(self, name, path=None, nameonly=False):
        ...
        if isinstance(self_uploadfield, str):
...
class Query(object):
    ...
    def as_dict(self, flat=False, sanitize=True):
        ...
        """
        SERIALIZABLE_TYPES = (
         tuple, dict, set, list, int, long, float,
...
class Rows(object):
    ...
    def render(self, i=None, fields=None):
        ...
        if i is None:
            ...
            keys = row.keys()
            tables = [ f.tablename for f in fields ] if fields else [ k for k in keys if k != '_extra' ]
            for table in tables:
                repr_fields = [ f.name for f in fields if f.tablename == table ] if fields else [ k for k in row[table].keys() if hasattr(self.db[table], k) and isinstance(self.db[table][k], Field) and self.db[table][k].represent
                                                                                                ]
                for field in repr_fields:
                    row[table][field] = sqlhtml.represent(self.db[table][field], row[table][field], row[table])
    ...
    def xml(self, strict=False, row_name='row', rows_name='rows'):
        ...
        if strict:
            return '<%s>\n%s\n</%s>' % (rows_name,
             ('\n').join(row.as_xml(row_name=row_name, colnames=self.colnames) for row in self), rows_name)
        import sqlhtml
