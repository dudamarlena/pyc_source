...
# [GCC 8.4.0]
# Embedded file name: /home/mdipierro/make_web2py/web2py/gluon/fileutils.py
...
"""
import storage, os, re, tarfile, glob, time, datetime, logging
...
def listdir(path, expression='^.+$', drop=True, add_dirs=False, sort=True):
    """
            ...
            def extractall(self, path='.', members=None):
             ...
             list returned by getmembers().
                ...
                for tarinfo in members:
                    if tarinfo.isdir():
                        try:
                            os.makedirs(os.path.join(path, tarinfo.name), 511)
...
def check_credentials(request, other_application='admin', expiration=3600, gae_login=True):
    """ checks that user is authorized to access other_application"""
    if request.env.web2py_runtime_gae:
        ...
        if users.is_current_user_admin():
            return True
        if gae_login:
            login_html = '<a href="%s">Sign in with your google account</a>.' % users.create_login_url(request.env.path_info)
...
def copystream(src, dest, size, chunk_size=100000):
    """
