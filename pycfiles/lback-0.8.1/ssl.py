...
class SSLSocket(socket):
    ...
    def accept(self):
        ...
        return (
         SSLSocket(newsock, keyfile=self.keyfile, certfile=self.certfile, server_side=True, cert_reqs=self.cert_reqs, ssl_version=self.ssl_version, ca_certs=self.ca_certs, ciphers=self.ciphers, do_handshake_on_connect=self.do_handshake_on_connect, suppress_ragged_eofs=self.suppress_ragged_eofs),
         addr)
...
def get_protocol_name(protocol_code):
    if protocol_code == PROTOCOL_TLSv1:
        ...
        if protocol_code == PROTOCOL_SSLv23:
            return 'SSLv23'
        if protocol_code == PROTOCOL_SSLv2:
            return 'SSLv2'
        if protocol_code == PROTOCOL_SSLv3:
            return 'SSLv3'
        return '<unknown>'
