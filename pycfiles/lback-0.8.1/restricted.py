...
# [GCC 8.4.0]
# Embedded file name: /home/mdipierro/make_web2py/web2py/gluon/restricted.py
...
"""
import sys, cPickle, traceback, types, os, logging
from storage import Storage
from http import HTTP
from html import BEAUTIFY, XML
logger = logging.getLogger('web2py')
__all__ = [
 'RestrictedError', 'restricted', 'TicketStorage', 'compile2']
...
class TicketStorage(Storage):
    ...
    def _store_in_db(self, request, ticket_id, ticket_data):
        ...
        try:
            ...
            id = table.insert(ticket_id=ticket_id, ticket_data=cPickle.dumps(ticket_data), created_datetime=request.now)
            self.db.commit()
    ...
    def _get_table(self, db, tablename, app):
        ...
        if not table:
            table = db.define_table(tablename, db.Field('ticket_id', length=100), db.Field('ticket_data', 'text'), db.Field('created_datetime', 'datetime'))
        return table
...
class RestrictedError(Exception):
    ...
    def __init__(self, layer='', code='', output='', environment=None):
        ...
        if layer:
            ...
            try:
                self.snapshot = snapshot(context=10, code=code, environment=self.environment)
            except:
                self.snapshot = {}
