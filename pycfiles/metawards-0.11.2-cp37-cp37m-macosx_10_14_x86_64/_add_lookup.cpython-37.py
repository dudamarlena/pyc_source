...
# [GCC 8.4.0]
# Embedded file name: /Users/runner/runners/2.169.1/work/MetaWards/MetaWards/tests/../build/lib.macosx-10.14-x86_64-3.7/metawards/utils/_add_lookup.py
...
def add_lookup(network: Network, nthreads: int=1):
    ...
    if lookup_file is None:
        ...
        import csv
        for parts in csv.reader((lines[1:]), quotechar='"', delimiter=',', quoting=(csv.QUOTE_ALL),
        ...
        if len(ward_infos) != network.nnodes + 1:
            print(f"WARNING: Number of wards from {lookup_file} ({len(ward_infos)}) disagrees with the number of wards in the network ({network.nnodes})")
            if len(ward_infos) > network.nnodes + 1:
