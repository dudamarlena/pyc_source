...
# [GCC 8.4.0]
# Embedded file name: /Users/runner/runners/2.169.1/work/MetaWards/MetaWards/tests/../build/lib.macosx-10.14-x86_64-3.7/metawards/iterators/_setup_imports.py
# Compiled at: 2020-05-11 13:26:49
# Size of source mod 2**32: 4204 bytes
...
def setup_seed_specified_ward(network: _Union[(Network, Networks)], infections: Infections, **kwargs):
    ...
    if isinstance(network, Networks):
        raise NotImplementedError('Seeding a specific ward for a multi-demographic Network is not yet supported. If you need this please raise an issue at https://github.com/metawards/MetaWards/issues and we will work as quickly as we can to implement it.')
    wards = network.nodes
    links = network.links
...
def setup_seed_all_wards(network: _Union[(Network, Networks)], population: Population, infections: Infections, **kwargs):
    ...
    if isinstance(network, Networks):
        if network.overall.params.daily_imports == 0:
            return
        raise NotImplementedError('Daily seeding of a multi-demographic network is not yet supported. If you need this, please raise an issue on https://github.com/metawards/MetaWards/issues and we will work as quickly as we can to implement it.')
    wards = network.nodes
    params = network.params
