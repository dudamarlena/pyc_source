...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/qt/core/qtutils.py
# Compiled at: 2020-05-03 00:26:03
# Size of source mod 2**32: 51518 bytes
"""
Utility module that contains useful utilities functions for PySide
"""
from __future__ import print_function, division, absolute_import
import os, re, sys, logging, inspect, subprocess, contextlib
...
def create_qrc_file(src_paths, dst_file):
    ...
    def tree(top='.', filters=None, output_prefix=None, max_level=4, followlinks=False, top_info=False, report=True):
        topdown = True
        total_directories = 0
        total_files = 0
...
def get_signals(class_obj):
    ...
    if class_obj.__base__:
        if class_obj.__base__ != QObject:
...
def change_button_color(button, text_color=200, bg_color=68, hi_color=68, hi_text=255, hi_background=[97, 132, 167], ds_color=[
 255, 128, 128], mode='common', toggle=False, hover=True, destroy=False, ds_width=1):
    ...
    if mode == 'common':
        ...
        if mode == 'button':
            if not destroy:
                button.setStyleSheet('QPushButton{background-color: ' + bg_hex + '; color:  ' + text_hex + '; border-style:solid; border-width: ' + str(ds_width) + 'px; border-color:' + ds_hex + '; border-radius: 0px;}' + 'QPushButton:hover{background-color: ' + hv_hex + '; color:  ' + text_hex + '; border-style:solid; border-width: ' + str(ds_width) + 'px; border-color:' + ds_hex + '; border-radius: 0px;}' + 'QPushButton:pressed{background-color: ' + hi_hex + '; color: ' + text_hex + '; border-style:solid; border-width: ' + str(ds_width) + 'px; border-color:' + ds_hex + '; border-radius: 0px;}')
            else:
...
def change_border_style(btn):
    ...
    button_style += 'QPushButton:pressed{border-style:solid; border-width: 2px; border-color: red ; border-radius: 1px;}'
    btn.setStyleSheet(button_style)


def create_flat_button(icon=None, icon_size=None, name='', text=200, background_color=[
 54, 51, 51], ui_color=68, border_color=180, push_col=120, checkable=True, w_max=None, w_min=None, h_max=None, h_min=None, policy=None, tip=None, flat=True, hover=True, destroy_flag=False, context=None):
    btn = QPushButton()
    btn.setText(name)
    ...
    if icon:
        ...
        btn.setFlat(flat)
        if flat:
            change_button_color(button=btn, text_color=text, bg_color=ui_color, hi_color=background_color, mode='button',
              hover=hover,
              destroy=destroy_flag,
              ds_color=border_color)
            btn.toggled.connect(lambda : change_button_color(button=btn, text_color=text, bg_color=ui_color, hi_color=background_color,
              mode='button',
              toggle=True,
              hover=hover,
              destroy=destroy_flag,
            ...
            change_button_color(button=btn, text_color=text, bg_color=background_color, hi_color=push_col,
              mode='button',
              hover=hover,
              destroy=destroy_flag,
              ds_color=border_color)
...
def recursively_set_menu_actions_visibility(menu, state):
    ...
    if any(action.isVisible() for action in menu.actions()):
        if menu.isVisible() != state:
...
def margins_dpi_scale(left, top, right, bottom):
    ...
    if isinstance(left, tuple):
        margins = left
        return (
         dpi_scale(margins[0]), dpi_scale(margins[1]), dpi_scale(margins[2]), dpi_scale(margins[3]))
    else:
        return (
         dpi_scale(left), dpi_scale(top), dpi_scale(right), dpi_scale(bottom))
...
def find_coordinates_inside_screen(x, y, width, height, padding=0):
    ...
    if not os.name == 'nt':
        ...
        monitor_adjusted = [(x1, y1, x2 - width - padding, y2 - height - padding) for x1, y1, x2, y2 in tuple(m[1] for m in win32.get_active_monitor_area())]
...
def iterate_children(widget, skip=None, qobj_class=None):
    ...
    for child in widget.children():
        yield child
        if skip is not None:
            if child.property(skip) is not None:
                continue
        if qobj_class is not None:
            if not isinstance(child, qobj_class):
                continue
