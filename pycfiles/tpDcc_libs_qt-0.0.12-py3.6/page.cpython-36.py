...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/qt/widgets/page.py
...
class Page(base.BaseWidget, mixin.FieldMixin):
    ...
    def setup_signals(self):
        ...
        self.register_field('display_text', lambda : self._get_page_display_string(self.field('current_page'), self.field('page_size_selected'), self.field('total')))
    ...
    def set_page_config(self, data_list):
        ...
        """
        self.set_field('page_size_list', [{'label':str(data),  'value':data} if isinstance(data, int) else data for data in data_list])
    ...
    def _get_page_display_string(self, current, count_per_page, total):
        ...
        :return: str
        """
        return '{} - {} of {}'.format((current - 1) * count_per_page + 1 if current else 0, min(total, current * count_per_page), total)
