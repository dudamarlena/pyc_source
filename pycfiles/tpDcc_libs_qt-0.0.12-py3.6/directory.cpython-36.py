...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/qt/widgets/directory.py
# Compiled at: 2020-05-03 00:26:03
# Size of source mod 2**32: 19522 bytes
"""
...
class SelectFolderButton(QWidget, object):
    ...
    def __init__(self, text='Browse', directory='', use_app_browser=False, parent=None):
        ...
        self._folder_btn = buttons.IconButton(icon=folder_icon,
          icon_padding=2,
...
class SelectFolder(QWidget, object):
    ...
    def _open_folder_browser_dialog(self):
        ...
        if tp.is_maya():
            ...
            result = maya.cmds.fileDialog2(caption='Select Folder',
              fileMode=3,
              startingDirectory=(self.folder_line.text()))
            if result:
                result = result[0]
            if not result or not os.path.isdir(result):
                return
...
class SelectFile(base.DirectoryWidget, object):
    ...
    def ui(self):
        ...
        if os.path.exists(self._directory):
            ...
            if self._use_icon:
                folder_icon = tp.ResourcesMgr().icon('folder')
                self._file_btn = buttons.IconButton(icon=folder_icon,
                  icon_padding=2,
                  button_style=(buttons.ButtonStyles.FlatStyle))
            else:
    ...
    def _open_file_browser_dialog(self):
        ...
        if tp.is_maya():
            ...
            result = maya.cmds.fileDialog2(caption='Select File',
              fileMode=1,
              fileFilter=(self._filters or ''),
              startingDirectory=file_line)
            if result:
                result = result[0]
