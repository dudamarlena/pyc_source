...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pdf_diff/command_line.py
# Compiled at: 2019-06-20 14:26:57
# Size of source mod 2**32: 20749 bytes
import sys
...
def compute_changes(pdf_fn_1, pdf_fn_2, top_margin=0, bottom_margin=100):
    docs = [
     serialize_pdf(0, pdf_fn_1, top_margin, bottom_margin), serialize_pdf(1, pdf_fn_2, top_margin, bottom_margin)]
...
def serialize_pdf(i, fn, top_margin, bottom_margin):
    ...
    for run in box_generator:
        if run['text'] is None:
            ...
            if normalized_text.endswith('\xad'):
...
def mark_eol_hyphens(boxes):
    ...
    for next_box in boxes:
        if box is not None:
            if box['pdf'] != next_box['pdf'] or box['page'] != next_box['page'] or next_box['y'] >= box['y'] + box['height'] / 2:
                mark_eol_hyphen(box)
            yield box
        box = next_box
...
def mark_eol_hyphen(box):
    if box['text'] is not None and box['text'].endswith('-'):
        box['text'] = box['text'][0:-1] + '\xad'


def perform_diff(doc1text, doc2text):
    import diff_match_patch
    return diff_match_patch.diff(doc1text, doc2text, timelimit=0, checklines=False)
        ...
        for page in list(pages[pdf]):
            ...
            for i, box in enumerate(changes):
                if box != '*':
                    ...
                    try:
                        y1 = max(b['y'] + b['height'] for j, b in enumerate(changes) if j < i and b != '*' and b['pdf']['index'] == pdf and b['page'] == (page, split_index))
                        y2 = min(b['y'] for j, b in enumerate(changes) if j > i and b != '*' and b['pdf']['index'] == pdf and b['page'] == (page, split_index))
                    except ValueError:
                        continue
    ...
    for change in changes:
        if change == '*':
            ...
            if style == 'box':
                draw.rectangle((
                 change['x'], change['y'],
                 change['x'] + change['width'], change['y'] + change['height']), outline='red')
            else:
                if style == 'strike':
                    draw.line((
                     change['x'], change['y'] + change['height'] / 2,
                     change['x'] + change['width'], change['y'] + change['height'] / 2), fill='red')
                elif style == 'underline':
                    draw.line((
                     change['x'], change['y'] + change['height'],
                     change['x'] + change['width'], change['y'] + change['height']), fill='red')
            del draw
...
def zealous_crop(page_groups):
    for idx in (0, 1):
        ...
        if width != None:
            minx = max(0, minx - int(0.02 * width))
            maxx = min(width, maxx + int(0.02 * width))
        for grp in page_groups:
            for pg in grp[idx]:
                im = grp[idx][pg]
...
def simplify_changes(boxes):
    changes = []
    for b in boxes:
        if len(changes) > 0 and changes[(-1)] != '*' and b != '*' and changes[(-1)]['pdf'] == b['pdf'] and changes[(-1)]['page'] == b['page'] and changes[(-1)]['index'] + 1 == b['index'] and changes[(-1)]['y'] == b['y'] and changes[(-1)]['height'] == b['height']:
...
def main():
    import argparse
    description = 'Calculates the differences between two specified files in PDF format (or changes specified on standard input) and outputs to standard output side-by-side images with the differences marked (in PNG format).'
    parser = argparse.ArgumentParser(description=description)
    parser.add_argument('files', nargs='*', help='calculate differences between the two named files')
    parser.add_argument('-c', '--changes', action='store_true', default=False, help='read change description from standard input, ignoring files')
    parser.add_argument('-s', '--style', metavar='box|strike|underline,box|stroke|underline', default='strike,underline', help='how to mark the differences in the two files (default: strike, underline)')
    parser.add_argument('-f', '--format', choices=['png', 'gif', 'jpeg', 'ppm', 'tiff'], default='png', help='output format in which to render (default: png)')
    parser.add_argument('-t', '--top-margin', metavar='margin', default=0.0, type=float, help='top margin (ignored area) end in percent of page height (default 0.0)')
    parser.add_argument('-b', '--bottom-margin', metavar='margin', default=100.0, type=float, help='bottom margin (ignored area) begin in percent of page height (default 100.0)')
    parser.add_argument('-r', '--result-width', default=900, type=int, help='width of the result image (width of image in px)')
