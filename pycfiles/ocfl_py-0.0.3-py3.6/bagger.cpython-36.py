...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/ocfl/bagger.py
# Compiled at: 2020-04-21 14:57:40
...
"""
import bagit, os.path

class BaggerError(Exception):
    __doc__ = 'Exception class for conditions that should abort with message.'
...
def bag_as_source(srcbag, metadata):
    ...
    if not metadata.created:
        if 'Bagging-Date' in bag.info:
            metadata.created = bag.info['Bagging-Date'] + 'T00:00:00Z'
    if not metadata.message:
        if 'External-Description' in bag.info:
...
def bag_extracted_version(dst, metadata):
    ...
    if metadata.address:
        if metadata.address.startswith('mailto:'):
