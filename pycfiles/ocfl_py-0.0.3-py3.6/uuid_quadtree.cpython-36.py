...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/ocfl/uuid_quadtree.py
...
class UUIDQuadtree(Dispositor):
    __doc__ = 'Class to support UUID URIs with quadtree directory names.'
    ...
    def identifier_to_path(self, identifier):
        ...
        match = re.match('([\\da-f]{4})([\\da-f]{4})\\-([\\da-f]{4})\\-([\\da-f]{4})\\-([\\da-f]{4})\\-([\\da-f]{4})([\\da-f]{4})([\\da-f]{4})$', identifier)
        if not match:
            raise Exception('UUIDQuadtree identifier %s not valid' % identifer)
        return os.path.join(match.group(1), match.group(2), match.group(3), match.group(4), match.group(5), match.group(6), match.group(7), match.group(8))
    ...
    def relative_path_to_identifier(self, path):
        ...
        for segment in segments:
            if not re.match('[\\da-f]{4}$', segment):
                raise Exception('Bad path segment %s in UUIDQuadtree path %s' % (segment, path))

        return self.prefix + segments[0] + segments[1] + '-' + segments[2] + '-' + segments[3] + '-' + segments[4] + '-' + segments[5] + segments[6] + segments[7]