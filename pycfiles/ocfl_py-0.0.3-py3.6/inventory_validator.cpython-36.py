...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/ocfl/inventory_validator.py
# Compiled at: 2020-04-26 11:34:15
# Size of source mod 2**32: 14285 bytes
...
class InventoryValidator(object):
    __doc__ = 'Class for OCFL Inventory Validator.'

    def __init__(self, log=None, where='???', lax_digests=False):
    ...
    def validate(self, inventory):
        ...
        elif 'contentDirectory' in inventory:
    ...
    def validate_versions(self, versions, all_versions):
        ...
        for v in all_versions:
            ...
            if 'created' not in version or type(versions[v]['created']) != str:
                self.error('E401', version=v)
            else:
                created = versions[v]['created']
                try:
                    dt = str_to_datetime(created)
                    if not re.search('(Z|[+-]\\d\\d:\\d\\d)$', created):
                        self.error('E049a', version=v)
                    if not re.search('T\\d\\d:\\d\\d:\\d\\d', created):
            ...
            if 'user' not in version:
                ...
                if type(user) != dict:
                    ...
                    if 'address' not in user:
                        ...
                        if type(user['address']) != str:
                            self.error('E406', version=v)
                        else:
                            if not re.match('\\w{3,6}:', user['address']):
    ...
    def is_valid_content_path(self, path):
        ...
        m = re.match('^v\\d+/' + self.content_directory + '/', path)
    ...
    def validate_as_prior_version(self, prior):
        ...
        if not set(prior.all_versions).issubset(set(self.all_versions)):
            ...
            if not set(prior.manifest_files.keys()).issubset(self.manifest_files.keys()):
