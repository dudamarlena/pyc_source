...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/Products/BastionCrypto/BastionCertAuthority.py
...
def using_openssl():
    return _using_openssl


manage_addBastionCertAuthorityForm = PageTemplateFile('zpt/add_ca', globals())

def manage_addBastionCertAuthority(self, id, keyfile='/etc/pki/tls/private/rsa.keydec', cert='/etc/pki/tls/certs/ca.crt', directory='/tmp', ca_directory='/tmp', title='', REQUEST=None):
...
class BastionCertAuthority(ObjectManager, SimpleItem):
    ...
    manage_options = ({'label': 'Sign', 'action': 'manage_sign'}, {'label': 'Configure', 'action': 'manage_config'}, {'label': 'View', 'action': ''}) + SimpleItem.manage_options
    __ac_permissions__ = (
     (
    ...
    def ca_directory(self):
        return self._ca_directory
    ...
    def openssl_mode(self):
        return using_openssl
    ...
    def sign(self, keyfile=None, keytext='', REQUEST={}):
        ...
        if _using_openssl:
            ...
            if os.access('%s.pem' % filename, os.F_OK) and os.stat('%s.pem' % filename)[6] > 0:
                ...
                return 0
            if REQUEST:
                REQUEST.set('manage_tabs_message', structured_text('Error signing certificate - the problem may be highlighted below\n:%s' % rc))
                return self.manage_sign(self, REQUEST)
            raise ValueError, rc
        else:
