...
# [GCC 8.4.0]
# Embedded file name: py-build/2.7/bdist.linux-x86_64/egg/riak/transports/tcp/connection.py
# Compiled at: 2016-11-30 12:10:07
import errno, logging, socket, struct, six, riak.pb.riak_pb2, riak.pb.messages
    ...
    if not USE_STDLIB_SSL:
        ...
        def _ssl_handshake(self):
            ...
            if self._client._credentials:
                try:
                    ssl_ctx = configure_pyopenssl_context(self._client._credentials)
        ...
        def _ssl_handshake(self):
            ...
            if credentials:
                try:
                    ...
                    ssl_socket = ssl.SSLSocket(sock=self._socket, keyfile=credentials.pkey_file, certfile=credentials.cert_file, cert_reqs=ssl.CERT_REQUIRED, ca_certs=credentials.cacert_file, ciphers=credentials.ciphers, server_hostname=host)
