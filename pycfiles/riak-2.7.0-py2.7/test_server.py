...
# [GCC 8.4.0]
# Embedded file name: py-build/2.7/bdist.linux-x86_64/egg/riak/test_server.py
# Compiled at: 2016-10-17 19:06:50
from __future__ import print_function
import os.path, threading, string, re, random, shutil, socket, time, stat
...
class TestServer(object):
    VM_ARGS_DEFAULTS = {'-name': 'riaktest%d@127.0.0.1' % random.randint(0, 100000), 
    ...
    APP_CONFIG_DEFAULTS = {'riak_core': {'web_ip': '127.0.0.1', 
                     'web_port': 9000, 
                     'handoff_port': 9001, 
                     'ring_creation_size': 64}, 
       'riak_kv': {'storage_backend': Atom('riak_kv_test_backend'), 
                   'pb_ip': '127.0.0.1', 
                   'pb_port': 9002, 
                   'js_vm_count': 8, 
                   'js_max_vm_mem': 8, 
    ...
    def __init__(self, tmp_dir='/tmp/riak/test_server', bin_dir=os.path.expanduser('~/.riak/install/riak-0.14.2/bin'), vm_args=None, **options):
    ...
    def wait_for_startup(self):
        ...
        while not listening:
            try:
                socket.create_connection((self._http_ip(), self._http_port()), 1.0)
            except IOError:
                pass
            else:
                listening = True
        ...
        with open(self._riak_script, 'wb') as (temp_bin_file):
            with open(os.path.join(self.bin_dir, 'riak'), 'r') as (riak_file):
                for line in riak_file.readlines():
                    line = re.sub('(RUNNER_SCRIPT_DIR=)(.*)', '\\1%s' % self._temp_bin, line)
                    line = re.sub('(RUNNER_ETC_DIR=)(.*)', '\\1%s' % self._temp_etc, line)
                    line = re.sub('(RUNNER_USER=)(.*)', '\\1', line)
                    line = re.sub('(RUNNER_LOG_DIR=)(.*)', '\\1%s' % self._temp_log, line)
                    line = re.sub('(PIPE_DIR=)(.*)', '\\1%s' % self._temp_pipe, line)
                    line = re.sub('(PLATFORM_DATA_DIR=)(.*)', '\\1%s' % self.temp_dir, line)
                ...
                os.fchmod(temp_bin_file.fileno(), stat.S_IRWXU | stat.S_IRGRP | stat.S_IXGRP | stat.S_IROTH | stat.S_IXOTH)
    ...
    def touch_ssl_distribution_args(self):
        ...
        Popen([script, 'chkconfig'], stdin=PIPE, stdout=PIPE, stderr=PIPE).communicate()
