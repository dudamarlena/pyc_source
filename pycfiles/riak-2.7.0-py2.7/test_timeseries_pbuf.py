...
# [GCC 8.4.0]
# Embedded file name: py-build/2.7/bdist.linux-x86_64/egg/riak/tests/test_timeseries_pbuf.py
# Compiled at: 2016-12-08 20:11:02
import datetime, six, unittest, riak.pb.riak_ts_pb2
...
from riak.util import str_to_bytes, bytes_to_str, unix_time_millis, datetime_from_unix_time_millis, is_timeseries_supported
table_name = 'GeoCheckin'
bd0 = '时间序列'
bd1 = 'временные ряды'
blob0 = '\x00\x01\x02\x03\x04\x05\x06\x07'
...
class TimeseriesPbufUnitTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if cls.ts0ms != ex0ms:
            raise AssertionError(('expected {:d} to equal {:d}').format(cls.ts0ms, ex0ms))
        cls.ts1ms = unix_time_millis(ts1)
        if cls.ts1ms != ex1ms:
            raise AssertionError(('expected {:d} to equal {:d}').format(cls.ts1ms, ex1ms))
        cls.rows = [
         [
...
@unittest.skipUnless(is_timeseries_supported() and RUN_TIMESERIES, 'Timeseries not supported by this Python version or RUN_TIMESERIES is 0')
class TimeseriesPbufTests(IntegrationTestBase, unittest.TestCase):
    client_options = {'transport_options': {'use_ttb': False, 'ts_convert_timestamp': True}}

    @classmethod
    def setUpClass(cls):
        super(TimeseriesPbufTests, cls).setUpClass()
        cls.now = datetime.datetime.utcfromtimestamp(144379690.987)
    ...
    def test_query_that_matches_some_data(self):
        ...
        query = fmt.format(table=table_name, t1=self.tenMinsAgoMsec, t2=self.nowMsec)
        ts_obj = self.client.ts_query(table_name, query)
        self.validate_data(ts_obj)

    def test_query_that_matches_some_data_using_interpolation(self):
        fmt = "\n        select * from {{table}} where\n            time > {t1} and time < {t2} and\n            geohash = 'hash1' and\n            user = 'user2'\n        "
        query = fmt.format(t1=self.tenMinsAgoMsec, t2=self.nowMsec)
        ts_obj = self.client.ts_query(table_name, query)
        self.validate_data(ts_obj)

    def test_query_that_matches_more_data(self):
        fmt = "\n        select * from {table} where\n            time >= {t1} and time <= {t2} and\n            geohash = 'hash1' and\n            user = 'user2'\n        "
        query = fmt.format(table=table_name, t1=self.twentyMinsAgoMsec, t2=self.nowMsec)
        ts_obj = self.client.ts_query(table_name, query)
        j = 0
    ...
    def test_delete_single_value(self):
        key = [
         'hash1', 'user2', self.twentyFiveMinsAgo]
