...
# [GCC 8.4.0]
# Embedded file name: py-build/2.7/bdist.linux-x86_64/egg/riak/util.py
...
def datetime_from_unix_time_millis(ut):
    if isinstance(ut, float):
        raise ValueError('unix timestamp must not be a float, it must be total milliseconds since epoch as an integer')
    utms = ut / 1000.0
...
def deep_merge(a, b):
    ...
    while stack:
        ...
        for key in current_src:
            ...
            elif quacks_like_dict(current_src[key]) and quacks_like_dict(current_dst[key]):
