...
# [GCC 8.4.0]
# Embedded file name: py-build/2.7/bdist.linux-x86_64/egg/riak/codecs/http.py
# Compiled at: 2016-11-30 12:10:07
import re, csv, six
...
class HttpCodec(object):
    ...
    def _parse_body(self, robj, response, expected_statuses):
        ...
        if response is None:
            ...
            if not status:
                m = ('Could not contact Riak Server: http://{0}:{1}!').format(self._node.host, self._node.http_port)
                raise RiakError(m)
            ...
            if status == 201:
                ...
                if ctype == 'multipart/mixed':
                    ...
                    parts = [ message_from_string(p) for p in re.split(boundary, data)[1:-1]
                            ]
                    robj.siblings = [ self._parse_sibling(RiakContent(robj), part.items(), part.get_payload()) for part in parts
                                    ]
                    if robj.resolver is not None:
                        robj.resolver(robj)
                    return robj
                raise Exception(('unexpected sibling response format: {0}').format(ctype))
            robj.siblings = [
             self._parse_sibling(RiakContent(robj), headers.items(), data)]
            return robj
    ...
    def _build_put_headers(self, robj, if_none_match=False):
        ...
        headers = MultiDict({'Content-Type': content_type, 'X-Riak-ClientId': self._client_id})
