...
# [GCC 8.4.0]
# Embedded file name: py-build/2.7/bdist.linux-x86_64/egg/riak/tests/test_datatypes.py
# Compiled at: 2016-10-20 20:34:49
import unittest, riak.datatypes as datatypes
...
class MapUnitTests(DatatypeUnitTestBase, unittest.TestCase):
    ...
    def check_op_output(self, op):
        ...
        self.assertIn(('update', ('d', 'map'), [('update', ('e', 'set'), {'adds': ['deep value']})]), op)
        self.assertNotIn(('update', ('f', 'map'), None), op)
        self.assertNotIn(('update', ('h', 'map'),
         [
          ('update', ('i', 'map'),
           None)]), op)
        return
...
class HllDatatypeIntegrationTests(IntegrationTestBase, unittest.TestCase):
...
class DatatypeIntegrationTests(IntegrationTestBase, unittest.TestCase, Comparison):
    ...
    def test_dt_map(self):
        ...
        self.assertEqual({('e', 'set'): frozenset(['deep value'])}, othermap.maps['d'].value)
    ...
    def test_delete_datatype(self):
        ...
        for t in [counter, set_, map_]:
            ...
            self.client.get(obj)
            self.assertFalse(obj.exists, ('{0} exists after deletion').format(t.type_name))