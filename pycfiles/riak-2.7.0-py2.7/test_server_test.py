...
# [GCC 8.4.0]
# Embedded file name: py-build/2.7/bdist.linux-x86_64/egg/riak/tests/test_server_test.py
...
class TestServerTestCase(unittest.TestCase):
    ...
    def test_options_defaults(self):
        self.assertEqual(self.test_server.app_config['riak_core']['handoff_port'], 9001)
        self.assertEqual(self.test_server.app_config['riak_kv']['pb_ip'], '127.0.0.1')

    def test_merge_riak_core_options(self):
        self.test_server = TestServer(riak_core={'handoff_port': 10000})
        self.assertEqual(self.test_server.app_config['riak_core']['handoff_port'], 10000)

    def test_merge_riak_search_options(self):
        self.test_server = TestServer(riak_search={'search_backend': 'riak_search_backend'})
        self.assertEqual(self.test_server.app_config['riak_search']['search_backend'], 'riak_search_backend')
    ...
    def test_set_ring_state_dir(self):
        self.assertEqual(self.test_server.app_config['riak_core']['ring_state_dir'], '/tmp/riak/test_server/data/ring')
