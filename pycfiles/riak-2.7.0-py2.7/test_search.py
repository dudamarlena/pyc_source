...
# [GCC 8.4.0]
# Embedded file name: py-build/2.7/bdist.linux-x86_64/egg/riak/tests/test_search.py
...
class SolrSearchTests(IntegrationTestBase, unittest.TestCase):
    ...
    def test_add_document_to_index(self):
        self.client.fulltext_add(testrun_search_bucket, [{'id': 'doc', 'username': 'tony'}])
    ...
    def test_add_multiple_documents_to_index(self):
        self.client.fulltext_add(testrun_search_bucket, [{'id': 'dizzy', 'username': 'dizzy'}, {'id': 'russell', 'username': 'russell'}])
        results = self.client.fulltext_search(testrun_search_bucket, 'username:russell OR username:dizzy')
    ...
    def test_delete_documents_from_search_by_query(self):
        self.client.fulltext_add(testrun_search_bucket, [{'id': 'dizzy', 'username': 'dizzy'}, {'id': 'russell', 'username': 'russell'}])
        self.client.fulltext_delete(testrun_search_bucket, queries=[
         'username:dizzy', 'username:russell'])
        results = self.client.fulltext_search(testrun_search_bucket, 'username:russell OR username:dizzy')
        self.assertEqual(0, len(results['docs']))

    def test_delete_documents_from_search_by_query_and_id(self):
        self.client.fulltext_add(testrun_search_bucket, [{'id': 'dizzy', 'username': 'dizzy'}, {'id': 'russell', 'username': 'russell'}])
        self.client.fulltext_delete(testrun_search_bucket, docs=[
         'dizzy'], queries=[
         'username:russell'])
        results = self.client.fulltext_search(testrun_search_bucket, 'username:russell OR username:dizzy')
        self.assertEqual(0, len(results['docs']))
...
class SearchTests(IntegrationTestBase, unittest.TestCase):
    ...
    def test_solr_search_with_params(self):
        bucket = self.client.bucket(testrun_search_bucket)
        bucket.new('user', {'username': 'roidrage'}).store()
        results = self.client.fulltext_search(testrun_search_bucket, 'username:roidrage', wt='xml')
        self.assertEqual(1, len(results['docs']))

    def test_solr_search(self):
        bucket = self.client.bucket(testrun_search_bucket)
        bucket.new('user', {'username': 'roidrage'}).store()
        results = self.client.fulltext_search(testrun_search_bucket, 'username:roidrage')
        self.assertEqual(1, len(results['docs']))
    ...
    def test_search_integration(self):
        ...
        results = self.client.fulltext_search(testrun_search_bucket, 'foo:one OR foo:two')
        if len(results) == 0:
        ...
        query = '(foo:one OR foo:two OR foo:three OR foo:four) AND                 (NOT bar:green)'
