...
# [GCC 8.4.0]
# Embedded file name: py-build/2.7/bdist.linux-x86_64/egg/riak/codecs/pbuf.py
# Compiled at: 2016-12-12 17:42:39
import datetime, six, riak.pb.messages, riak.pb.riak_pb2, riak.pb.riak_dt_pb2, riak.pb.riak_kv_pb2, riak.pb.riak_ts_pb2
...
from riak.util import decode_index_value, str_to_bytes, bytes_to_str, unix_time_millis, datetime_from_unix_time_millis
...
REPL_TO_PY = {riak.pb.riak_pb2.RpbBucketProps.FALSE: False, 
...
QUORUM_TO_PB = {'default': RIAKC_RW_DEFAULT, 'all': RIAKC_RW_ALL, 
   'quorum': RIAKC_RW_QUORUM, 
   'one': RIAKC_RW_ONE}
QUORUM_TO_PY = _invert(QUORUM_TO_PB)
NORMAL_PROPS = [
 'n_val', 'allow_mult', 'last_write_wins', 'old_vclock',
...
MAP_FIELD_TYPES = {riak.pb.riak_dt_pb2.MapField.COUNTER: 'counter', 
...
DT_FETCH_TYPES = {riak.pb.riak_dt_pb2.DtFetchResp.COUNTER: 'counter', 
...
class PbufCodec(Codec):
    ...
    def __init__(self, client_timeouts=False, quorum_controls=False, tombstone_vclocks=False, bucket_types=False):
        if riak.pb is None:
    ...
    def encode_ping(self):
        return Msg(riak.pb.messages.MSG_CODE_PING_REQ, None, riak.pb.messages.MSG_CODE_PING_RESP)
    ...
    def decode_content(self, rpb_content, sibling):
        ...
        if rpb_content.HasField('content_encoding'):
            sibling.content_encoding = bytes_to_str(rpb_content.content_encoding)
        ...
        sibling.usermeta = dict([ (bytes_to_str(usermd.key), bytes_to_str(usermd.value)) for usermd in rpb_content.usermeta
                                ])
        sibling.indexes = set([ (bytes_to_str(index.key), decode_index_value(index.key, index.value)) for index in rpb_content.indexes
                              ])
    ...
    def encode_dt_options(self, req, **kwargs):
        ...
        for o in ['basic_quorum', 'notfound_ok', 'timeout', 'return_body', 'include_context']:
    ...
    def decode_map_value(self, entries):
        ...
        for entry in entries:
            ...
            if dtype == 'counter':
                ...
                raise ValueError(('Map may not contain datatype: {}').format(dtype))
    ...
    def encode_dt_op(self, dtype, req, op):
        if dtype == 'counter':
            ...
            raise TypeError(('Cannot send operation on datatype {!r}').format(dtype))
    ...
    def decode_timeseries(self, resp, tsobj, convert_timestamp=False):
        """
        ...
        if resp.rows is not None:
            for row in resp.rows:
                tsobj.rows.append(self.decode_timeseries_row(row, resp.columns, convert_timestamp))
    ...
    def decode_timeseries_col_type(self, col_type):
        if col_type == TsColumnType.Value('VARCHAR'):
            return 'varchar'
        if col_type == TsColumnType.Value('SINT64'):
            return 'sint64'
        if col_type == TsColumnType.Value('DOUBLE'):
            return 'double'
        if col_type == TsColumnType.Value('TIMESTAMP'):
            return 'timestamp'
        if col_type == TsColumnType.Value('BOOLEAN'):
            return 'boolean'
        if col_type == TsColumnType.Value('BLOB'):
            return 'blob'
    ...
    def decode_timeseries_row(self, tsrow, tscols=None, convert_timestamp=False):
        ...
        for i, cell in enumerate(tsrow.cells):
            ...
            if cell.HasField('varchar_value'):
                if col and not (col.type == TsColumnType.Value('VARCHAR') or col.type == TsColumnType.Value('BLOB')):
    ...
    def decode_preflist(self, item):
        ...
        :rtype dict
        """
        result = {'partition': item.partition, 'node': bytes_to_str(item.node), 
    ...
    def encode_put(self, robj, w=None, dw=None, pw=None, return_body=True, if_none_match=False, timeout=None):
        bucket = robj.bucket
    ...
    def encode_delete(self, robj, rw=None, r=None, w=None, dw=None, pr=None, pw=None, timeout=None):
        ...
        if self._client_timeouts and timeout:
            req.timeout = timeout
        use_vclocks = self._tombstone_vclocks and hasattr(robj, 'vclock') and robj.vclock
        if use_vclocks:
            req.vclock = robj.vclock.encode('binary')
        bucket = robj.bucket
        req.bucket = str_to_bytes(bucket.name)
    ...
    def encode_create_search_index(self, index, schema=None, n_val=None, timeout=None):
    ...
    def encode_get_search_index(self, index):
        req = riak.pb.riak_yokozuna_pb2.RpbYokozunaIndexGetReq(name=str_to_bytes(index))
    ...
    def encode_delete_search_index(self, index):
        req = riak.pb.riak_yokozuna_pb2.RpbYokozunaIndexDeleteReq(name=str_to_bytes(index))
        mc = riak.pb.messages.MSG_CODE_YOKOZUNA_INDEX_DELETE_REQ
        rc = riak.pb.messages.MSG_CODE_DEL_RESP
        return Msg(mc, req.SerializeToString(), rc)

    def encode_create_search_schema(self, schema, content):
        scma = riak.pb.riak_yokozuna_pb2.RpbYokozunaSchema(name=str_to_bytes(schema), content=str_to_bytes(content))
    ...
    def encode_get_search_schema(self, schema):
        req = riak.pb.riak_yokozuna_pb2.RpbYokozunaSchemaGetReq(name=str_to_bytes(schema))
    ...
    def encode_search(self, index, query, **kwargs):
        req = riak.pb.riak_search_pb2.RpbSearchQueryReq(index=str_to_bytes(index), q=str_to_bytes(query))
    ...
    def encode_update_datatype(self, datatype, **kwargs):
        ...
        if not op:
            raise ValueError(('No operation to send on datatype {!r}').format(datatype))
