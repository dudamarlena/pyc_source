...
# [GCC 8.4.0]
# Embedded file name: py-build/2.7/bdist.linux-x86_64/egg/riak/tests/test_timeseries_ttb.py
# Compiled at: 2016-12-08 20:11:02
import datetime, logging, six, unittest
...
from riak.util import str_to_bytes, bytes_to_str, unix_time_millis, is_timeseries_supported
...
class TimeseriesTtbUnitTests(unittest.TestCase):
    ...
    def test_encode_data_for_get(self):
        ...
        req = (
         tsgetreq_a, str_to_bytes(table_name), keylist, udef_a)
        req_test = encode(req)
        test_key = [
         'hash1', 'user2', ts0]
        c = TtbCodec()
    ...
    def test_decode_data_from_get(self):
        colnames = [
         'varchar', 'sint64', 'double', 'timestamp',
        ...
        r1 = (
         bd1, 3, 4.5, unix_time_millis(ts1), False, [], str1, None, blob0)
        rows = [
         r0, r1]
        data_t = (
    ...
    def test_encode_data_for_put(self):
        r0 = (
         bd0, 0, 1.2, unix_time_millis(ts0), True, [])
...
class TimeseriesTtbTests(IntegrationTestBase, unittest.TestCase):
    client_options = {'transport_options': {'use_ttb': True, 'ts_convert_timestamp': True}}
    ...
    def test_store_and_fetch_and_query(self):
        now = datetime.datetime.utcfromtimestamp(144379690.987)
        ...
        query = fmt.format(table=table_name, t1=unix_time_millis(tenMinsAgo), t2=unix_time_millis(now))
        ts_obj = self.client.ts_query(table_name, query)
    ...
    def test_create_error_via_put(self):
        ...
        with self.assertRaises(RiakError) as (cm):
            ts_obj.store()
        logging.debug(('[test_timeseries_ttb] saw exception: {}').format(cm.exception))