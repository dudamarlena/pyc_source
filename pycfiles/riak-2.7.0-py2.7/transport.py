...
# [GCC 8.4.0]
# Embedded file name: py-build/2.7/bdist.linux-x86_64/egg/riak/transports/transport.py
# Compiled at: 2016-12-12 17:42:39
import base64, random, threading, os, json, platform
from six import PY2
...
class Transport(FeatureDetection):
    ...
    client_id = property(_get_client_id, _set_client_id, doc='the client ID for this connection')
    ...
    def make_random_client_id(self):
        ...
        """
        if PY2:
            return 'py_%s' % base64.b64encode(str(random.randint(1, 1073741824)))
        else:
    ...
    def get(self, robj, r=None, pr=None, timeout=None, basic_quorum=None, notfound_ok=None, head_only=False):
        """
        Fetches an object.
        """
        raise NotImplementedError

    def put(self, robj, w=None, dw=None, pw=None, return_body=None, if_none_match=None, timeout=None):
        """
        Stores an object.
        """
        raise NotImplementedError
    ...
    def create_search_index(self, index, schema=None, n_val=None, timeout=None):
        """
    ...
    def stream_index(self, bucket, index, startkey, endkey=None, return_terms=None, max_results=None, continuation=None, timeout=None):
        """
    ...
    def get_counter(self, bucket, key, r=None, pr=None, basic_quorum=None, notfound_ok=None):
        """
    ...
    def _search_mapred_emu(self, index, query):
        ...
        if not self.phaseless_mapred():
            phases.append({'language': 'erlang', 'module': 'riak_kv_mapreduce', 
               ...
               'keep': True})
        mr_result = self.mapred({'module': 'riak_search', 'function': 'mapred_search', 
           'arg': [
                 index, query]}, phases)
        result = {'num_found': len(mr_result), 'max_score': 0.0, 
           'docs': []}
    ...
    def _get_index_mapred_emu(self, bucket, index, startkey, endkey=None):
        ...
        if not self.phaseless_mapred():
            phases.append({'language': 'erlang', 'module': 'riak_kv_mapreduce', 
               ...
               'keep': True})
        if endkey:
            result = self.mapred({'bucket': bucket, 'index': index, 
               'start': startkey, 
               'end': endkey}, phases)
        else:
            result = self.mapred({'bucket': bucket, 'index': index, 
