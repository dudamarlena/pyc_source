...
# [GCC 8.4.0]
# Embedded file name: py-build/2.7/bdist.linux-x86_64/egg/riak/transports/http/resources.py
...
class HttpResources(object):
    ...
    def bucket_properties_path(self, bucket, bucket_type=None, **options):
        if self.riak_kv_wm_bucket_type and bucket_type:
            return mkpath('/types', quote_plus(bucket_type), 'buckets', quote_plus(bucket), 'props', **options)
        else:
            if self.riak_kv_wm_buckets:
                return mkpath('/buckets', quote_plus(bucket), 'props', **options)
            query = options.copy()
    ...
    def bucket_type_properties_path(self, bucket_type, **options):
        return mkpath('/types', quote_plus(bucket_type), 'props', **options)

    def key_list_path(self, bucket, bucket_type=None, **options):
        query = {'keys': True, 'props': False}
        query.update(options)
        if self.riak_kv_wm_bucket_type and bucket_type:
            return mkpath('/types', quote_plus(bucket_type), 'buckets', quote_plus(bucket), 'keys', **query)
        else:
            if self.riak_kv_wm_buckets:
                return mkpath('/buckets', quote_plus(bucket), 'keys', **query)
            return mkpath(self.riak_kv_wm_raw, quote_plus(bucket), **query)
    ...
    def object_path(self, bucket, key=None, bucket_type=None, **options):
        if key:
            key = quote_plus(key)
        if self.riak_kv_wm_bucket_type and bucket_type:
            return mkpath('/types', quote_plus(bucket_type), 'buckets', quote_plus(bucket), 'keys', key, **options)
        else:
            if self.riak_kv_wm_buckets:
                return mkpath('/buckets', quote_plus(bucket), 'keys', key, **options)
            return mkpath(self.riak_kv_wm_raw, quote_plus(bucket), key, **options)

    def index_path(self, bucket, index, start, finish=None, bucket_type=None, **options):
        if not self.riak_kv_wm_buckets:
            raise RiakError('Indexes are unsupported by this Riak node')
        if finish is not None:
            finish = quote_plus(str(finish))
        if self.riak_kv_wm_bucket_type and bucket_type:
            return mkpath('/types', quote_plus(bucket_type), 'buckets', quote_plus(bucket), 'index', quote_plus(index), quote_plus(str(start)), finish, **options)
        else:
            return mkpath('/buckets', quote_plus(bucket), 'index', quote_plus(index), quote_plus(str(start)), finish, **options)
            return
    ...
    def counters_path(self, bucket, key, **options):
        ...
        return mkpath(self.riak_kv_wm_buckets, quote_plus(bucket), 'counters', quote_plus(key), **options)
    ...
    def datatypes_path(self, bucket_type, bucket, key=None, **options):
        ...
        if key:
            key = quote_plus(key)
        return mkpath('/types', quote_plus(bucket_type), 'buckets', quote_plus(bucket), 'datatypes', key, **options)
    ...
    def preflist_path(self, bucket, key, bucket_type=None, **options):
        ...
        if self.riak_kv_wm_bucket_type and bucket_type:
            ...
            return mkpath('/buckets', quote_plus(bucket), 'keys', quote_plus(key), 'preflist', **options)
