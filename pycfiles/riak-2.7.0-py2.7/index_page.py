...
# [GCC 8.4.0]
# Embedded file name: py-build/2.7/bdist.linux-x86_64/egg/riak/client/index_page.py
...
class IndexPage(Sequence, object):
    ...
    def __init__(self, client, bucket, index, startkey, endkey, return_terms, max_results, term_regex):
        self.client = client
        self.bucket = bucket
        self.index = index
    ...
    def next_page(self, timeout=None, stream=None):
        ...
        if stream is not None:
            self.stream = stream
        args = {'bucket': self.bucket, 'index': self.index, 
