...
# [GCC 8.4.0]
# Embedded file name: py-build/2.7/bdist.linux-x86_64/egg/riak/mapreduce.py
...
class RiakMapReduce(object):
    ...
    def search(self, index, query):
        ...
        self._inputs = {'bucket': index, 'index': index, 
           'query': query}
        return self
    ...
    def index(self, bucket, index, startkey, endkey=None, bucket_type=None):
        ...
        self._input_mode = 'query'
        if endkey is None:
            self._inputs = {'bucket': bucket, 'index': index, 'key': startkey}
        else:
            self._inputs = {'bucket': bucket, 'index': index, 
               'start': startkey, 
               'end': endkey}
        if bucket_type is not None:
            self._inputs['bucket'] = [
    ...
    def reduce(self, function, options=None):
        ...
        if isinstance(function, list):
            ...
            language = 'javascript'
        mr = RiakMapReducePhase('reduce', function, options.get('language', language), options.get('keep', False), options.get('arg', None))
        self._phases.append(mr)
        return self
...
class RiakMapReducePhase(object):
    ...
    def to_array(self):
        ...
        elif self._language == 'erlang' and isinstance(self._function, string_types):
            stepdef['source'] = self._function
