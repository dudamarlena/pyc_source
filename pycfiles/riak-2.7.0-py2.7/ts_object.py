...
# [GCC 8.4.0]
# Embedded file name: py-build/2.7/bdist.linux-x86_64/egg/riak/ts_object.py
...
class TsObject(object):
    ...
    def __init__(self, client, table, rows=None, columns=None):
        ...
        if rows is not None and not isinstance(rows, list):
            ...
            self.rows = rows
        if columns is not None and not isinstance(columns, TsColumns):
            raise RiakError('TsObject columns parameter must be a TsColumns instance')
        else:
