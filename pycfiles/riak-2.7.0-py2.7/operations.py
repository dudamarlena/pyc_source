...
# [GCC 8.4.0]
# Embedded file name: py-build/2.7/bdist.linux-x86_64/egg/riak/client/operations.py
# Compiled at: 2016-12-12 17:42:39
import six, riak.client.multi
...
class RiakClientOperations(RiakClientTransport):
    ...
    def get_buckets(self, transport, bucket_type=None, timeout=None):
        ...
        return [ bucketfn(bytes_to_str(name), bucket_type) for name in transport.get_buckets(bucket_type=bucket_type, timeout=timeout)
               ]
    ...
    def stream_buckets(self, bucket_type=None, timeout=None):
        ...
        def make_op(transport):
            return transport.stream_buckets(bucket_type=bucket_type, timeout=timeout)

        for bucket_list in self._stream_with_retry(make_op):
            bucket_list = [ bucketfn(bytes_to_str(name), bucket_type) for name in bucket_list
                          ]
    ...
    @retryable
    def get_index(self, transport, bucket, index, startkey, endkey=None, return_terms=None, max_results=None, continuation=None, timeout=None, term_regex=None):
        """
        ...
        results, continuation = transport.get_index(bucket, index, startkey, endkey, return_terms=return_terms, max_results=max_results, continuation=continuation, timeout=timeout, term_regex=term_regex)
        page.results = results
        page.continuation = continuation
        return page

    def paginate_index(self, bucket, index, startkey, endkey=None, max_results=1000, return_terms=None, continuation=None, timeout=None, term_regex=None):
        """
        ...
        """
        page = self.get_index(bucket, index, startkey, endkey=endkey, max_results=max_results, return_terms=return_terms, continuation=continuation, timeout=timeout, term_regex=term_regex)
        yield page
        while page.has_next_page():
            page = page.next_page()
            yield page

    def stream_index(self, bucket, index, startkey, endkey=None, return_terms=None, max_results=None, continuation=None, timeout=None, term_regex=None):
        """
        ...
        page.results = transport.stream_index(bucket, index, startkey, endkey, return_terms=return_terms, max_results=max_results, continuation=continuation, timeout=timeout, term_regex=term_regex)
    ...
    @retryable
    def get(self, transport, robj, r=None, pr=None, timeout=None, basic_quorum=None, notfound_ok=None, head_only=False):
        """
        ...
        if not isinstance(robj.key, six.string_types):
            raise TypeError(('key must be a string, instead got {0}').format(repr(robj.key)))
    ...
    @retryable
    def create_search_index(self, transport, index, schema=None, n_val=None, timeout=None):
        """
    ...
    @retryable
    def get_counter(self, transport, bucket, key, r=None, pr=None, basic_quorum=None, notfound_ok=None):
    ...
    def update_counter(self, bucket, key, value, w=None, dw=None, pw=None, returnvalue=False):
        """
        ...
        with self._transport() as (transport):
            return transport.update_counter(bucket, key, value, w=w, dw=dw, pw=pw, returnvalue=returnvalue)
    ...
    def fetch_datatype(self, bucket, key, r=None, pr=None, basic_quorum=None, notfound_ok=None, timeout=None, include_context=None):
        ...
        """
        dtype, value, context = self._fetch_datatype(bucket, key, r=r, pr=pr, basic_quorum=basic_quorum, notfound_ok=notfound_ok, timeout=timeout, include_context=include_context)
        return TYPES[dtype](bucket=bucket, key=key, value=value, context=context)
...
def _validate_timeout(timeout, infinity_ok=False):
    ...
    if timeout is None:
        ...
        if timeout == 'infinity':
            if infinity_ok:
                return
            raise ValueError('timeout must be a positive integer ("infinity" is not valid)')
