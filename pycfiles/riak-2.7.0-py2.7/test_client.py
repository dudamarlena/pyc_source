...
# [GCC 8.4.0]
# Embedded file name: py-build/2.7/bdist.linux-x86_64/egg/riak/tests/test_client.py
# Compiled at: 2016-11-08 18:56:48
import unittest
from six import PY2
from threading import Thread
from riak.riak_object import RiakObject
from riak.transports.tcp import TcpTransport
from riak.tests import DUMMY_HTTP_PORT, DUMMY_PB_PORT, RUN_POOL, RUN_CLIENT
...
class ClientTests(IntegrationTestBase, unittest.TestCase):
    ...
    def test_uses_client_id_if_given(self):
        if self.protocol == 'pbc':
            zero_client_id = '\x00\x00\x00\x00'
    ...
    def test_timeout_validation(self):
        ...
        for bad in [0, -1, False, 'foo']:
            ...
            with self.assertRaises(ValueError):
                for i in self.client.stream_index(bucket, 'field1_bin', 'val1', 'val4', timeout=bad):
                    pass
    ...
    def test_multiget_bucket(self):
        ...
        """
        keys = [
         self.key_name, self.randname(), self.randname()]
        for key in keys:
            if PY2:
                self.client.bucket(self.bucket_name).new(key, encoded_data=key, content_type='text/plain').store()
            else:
                self.client.bucket(self.bucket_name).new(key, data=key, content_type='text/plain').store()
    ...
    def test_multiput_errors(self):
        ...
        for failure in results:
            ...
            if PY2:
                self.assertIsInstance(failure[1], StandardError)
            else:
    ...
    def test_multiget_notfounds(self):
        ...
        keys = [
         (
          'default', self.bucket_name, self.key_name),
         (
    ...
    def test_multiget_pool_size(self):
        ...
        for key in keys:
            if PY2:
                client.bucket(self.bucket_name).new(key, encoded_data=key, content_type='text/plain').store()
            else:
                client.bucket(self.bucket_name).new(key, data=key, content_type='text/plain').store()
