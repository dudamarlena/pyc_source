...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\documentor\documentor.py
# Compiled at: 2020-04-06 13:05:53
# Size of source mod 2**32: 6296 bytes
from os.path import dirname, basename, isfile, join
import os, sys, glob, argparse, shaonutil, ast
...
def generateFunctionUsagesString(realcurrentpath):
    ...
    allmodules = [basename(c)[:-3] for c in all_py_files if isfile(c) if not c.endswith('__init__.py')]
    func_usages_string = ''
    for m in allmodules:
        module_heading = '### ' + m + '\n\n'
        module = __import__(m)
