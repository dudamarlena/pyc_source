...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/django_workflow/workflow.py
# Compiled at: 2017-08-29 06:20:40
from django.core import serializers
from django_workflow.models import Workflow, State, CurrentObjectState, Transition, Condition, Function, FunctionParameter, Callback, CallbackParameter, _execute_atomatic_transitions
...
def export_workflow(workflow_name):
    ...
    data = serializers.serialize('json', list(objects) + list(State.objects.filter(workflow__in=objects)) + list(Transition.objects.filter(workflow__in=objects)) + list(Condition.objects.filter(workflow__in=objects)) + list(Function.objects.filter(workflow__in=objects)) + list(FunctionParameter.objects.filter(workflow__in=objects)) + list(Callback.objects.filter(workflow__in=objects)) + list(CallbackParameter.objects.filter(workflow__in=objects)), indent=2, use_natural_foreign_keys=True, use_natural_primary_keys=True)
    return data
