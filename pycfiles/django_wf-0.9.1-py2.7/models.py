...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/django_workflow/models.py
# Compiled at: 2017-08-29 10:35:15
import threading
...
class TransitionManager(models.Manager):
    ...
    def get_by_natural_key(self, name, workflow, initial_state, final_state):
        return self.get(name=name, initial_state__workflow__name=workflow, initial_state__name=initial_state, final_state__name=final_state)
...
class Transition(models.Model):
    ...
    final_state = models.ForeignKey(State, verbose_name=ugettext_lazy('Final State'), related_name='incoming_transitions')
    ...
    def natural_key(self):
        return (
         self.name, self.initial_state.workflow.name, self.initial_state.name, self.final_state.name)
