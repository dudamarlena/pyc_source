...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\prettimer\logutils.py
# Compiled at: 2019-09-12 08:25:20
...
"""
import time, copy, functools, logging
__all__ = [
 'Elapsed', 'timeit', 'logit', 'logruntime']
...
class Elapsed(object):
    ...
    def runtimePrinter(runtime):
        ...
        ret = 'Elapsed Time:'
        if days:
            ret += ' {days:n} [days]'.format(days=days)
        if hours:
            ret += ' {hours:n} [hrs]'.format(hours=hours)
        if minutes:
            ret += ' {minutes:n} [mins]'.format(minutes=minutes)
        if seconds:
            ret += ' {seconds:.3f} [secs]'.format(seconds=seconds)
        return ret
    ...
    def stampFromBreakpoint(self, msg='', reset_last=True, name=''):
        ...
        if name == '':
            ...
            try:
                breakpoint_runtime = self.breakpointRuntime(self.all_breakpoints[name])
            except KeyError:
                breakpoint_runtime = self.last_breakpoint_runtime
...
def logruntime(f_=None, *, logger_attr='params.logger', timer_attr='params.timer'):
    """
