...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/test_corev1_objects.py
# Compiled at: 2019-10-01 18:18:07
# Size of source mod 2**32: 6480 bytes
import os, pytest
...
def test_configmap_is_added_and_emptied(config_map):
    ...
    config_map.data({'some-key1':'some-value1', 
    ...
    config_map.update()
    config_map.reload()
    assert config_map.data() == {'some-key1':'some-value1', 
     'some-key2':'some-value2'}
    config_map.data({'some-key1': None})
    config_map.update()
    config_map.reload()
    assert config_map.data() == {'some-key2': 'some-value2'}
...
def test_secret_is_readed_and_emptied(secret):
    ...
    secret.data({'new-key1':'new-value1', 
     'new-key2':'new-value2'})
    secret.update()
    secret.reload()
    assert secret.data() == {'new-key':'new-value', 
...
def test_secret_values_are_updated(secret):
    secret.data({'new-key0':'new-value0', 
     'new-key1':'new-value1', 
     'new-key2':'new-value2'})
    secret.update()
    secret.reload()
    assert secret.data() == {'new-key0':'new-value0', 
     'new-key1':'new-value1', 
     'new-key2':'new-value2'}
    secret.data({'new-key0':'changed-value0', 
     'new-key1':'changed-value1', 
     'new-key2':'changed-value2'})
    secret.update()
    s = Secret(secret.name, 'default').load()
    assert s.data() == {'new-key0':'changed-value0', 
     'new-key1':'changed-value1', 
     'new-key2':'changed-value2'}
...
def test_secret_values_are_updated_again(secret):
    ...
    secret.update()
    secret.reload()
    assert secret.data() == {'new-key0':'changed-value0', 
     'new-key2':'changed-value2'}
