...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/test_custom_object.py
# Compiled at: 2019-10-01 18:18:07
# Size of source mod 2**32: 8119 bytes
...
def mocked_crd_return_value():
    return SimpleNamespace(spec=SimpleNamespace(group='dummy.com',
      version='v1',
...
def test_raises_if_no_name():
    with mock.patch('kubeobject.kubeobject.open', mock.mock_open(read_data=yaml_data1), create=True) as (_):
        with pytest.raises(ValueError, match='.*needs to be passed as part of the function call.*'):
