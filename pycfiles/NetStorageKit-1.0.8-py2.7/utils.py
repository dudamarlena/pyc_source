...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/netstoragekit/utils.py
# Compiled at: 2015-06-18 22:58:51
...
def format_headers(headers, prefix=''):
    ...
    """
    return ('\n').join([ '%s%s: %s' % (prefix, k, v) for k, v in headers.items()
                       ])
...
def xml_to_data(xml_etree):
    ...
    if children:
        ...
        dd = Data(dd)
        d = Data({xml_etree.tag: Data({k:v[0] if len(v) == 1 else v for k, v in dd.iteritems()})})
    if xml_etree.attrib:
