...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/netstoragekit/api.py
# Compiled at: 2015-07-13 20:07:25
import logging
from hashlib import sha256
import requests, responses
from .exceptions import NetStorageKitError
from .auth import get_data, get_sign
from .utils import escape, format_exception, format_response, get_remote_path, reraise_exception, xml_to_data
try:
...
class Request(object):
    ...
    def __init__(self, key_name, key, cpcode, host, secure=True, timestamp=None, unique_id=None, testing=None, **parameters):
        """Request initializer.
    ...
    def _get_data_header(self):
        ...
        """
        value = get_data(self.key_name, timestamp=self.timestamp, unique_id=self.unique_id)
        ...
        try:
            ...
            if self._testing_mode:
                with responses.RequestsMock() as (r):
                    ...
                    r.add(method, url, status=self.testing.get('status', 200), content_type=self.testing.get('content_type', 'text/xml'), body=self.testing.get('body', ''))
                    response = requests.request(method, url, data=data, headers=headers, hooks=hooks, timeout=timeout)
            else:
                response = requests.request(method, url, data=data, headers=headers, hooks=hooks, timeout=timeout)
