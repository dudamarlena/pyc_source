...
# [GCC 8.4.0]
# Embedded file name: /data/nasif12/home_if12/wachutka/workspace/wBuild/wbuild/scanFiles.py
# Compiled at: 2018-05-23 08:03:04
# Size of source mod 2**32: 8707 bytes
import os, sys, pathlib, re
from snakemake.logging import logger
from wbuild.utils import parseWBInfosFromRFiles, parseMDFiles, getYamlParam, pathsepsToUnderscore, Config
pathsep = '/'
...
def insertPlaceholders(dest, source):
    ...
    if len(path.parts) <= 2 and bool(re.search('{wbP(D_P*)?}', source)):
        print('If using placeholders please make sure you have the right', ' directory structure.')
    if len(path.parts) > 2:
        P = path.parts[(-3)]
...
def writeRule(r, file):
    ...
    if wbInfos.get('type') == 'script':
        ...
        if wbInfos.get('type') == 'noindex':
            ...
            wbInfos['script'] = "'.wBuild/wBRender.R'"
    wbInfos['rule'] = pathsepsToUnderscore(r['file'], True)
    file.write('\n')
...
def writeIndexRule(wbRRows, wbMDrows, file):
    ...
    inputFiles = []
    for r in wbRRows:
        if not getYamlParam(r, 'type') == 'script':
            if getYamlParam(r, 'type') == 'noindex':
                pass
            else:
