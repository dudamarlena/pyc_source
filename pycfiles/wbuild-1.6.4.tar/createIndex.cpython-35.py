...
# [GCC 8.4.0]
# Embedded file name: /data/nasif12/home_if12/wachutka/workspace/wBuild/wbuild/createIndex.py
...
def writeSubMenu(top, wbData, level):
    ...
    for r in wbData:
        if pathlib.PurePath(r['file']).parts[(level - 1)] == top:
            if len(pathlib.PurePath(r['file']).parts) == level + 1 and getYamlParam(r, 'type') != 'script' and getYamlParam(r, 'type') != 'noindex':
...
def writeIndexHTMLMenu():
    ...
    template = Template(template).substitute(menu=menuString, title='title', rf=getRecentMenu())
