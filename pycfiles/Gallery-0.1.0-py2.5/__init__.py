...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gallery/__init__.py
# Compiled at: 2009-07-30 18:17:38
...
def handle_command_gallery(option_sets, command_options, args):
    r"""    usage: ``%(program)s [PROGRAM_OPTIONS] gallery META DEST``
...
def handle_command_thumb(option_sets, command_options, args):
    r"""    usage: ``%(program)s [PROGRAM_OPTIONS] thumb SOURCE DEST``
    ...
    for (dirpath, dirnames, filenames) in os.walk(source):
        ...
        for filename in filenames:
            ...
            if internal_vars.has_key('gallery_meta'):
                if not os.path.exists(os.path.join(dest, 'meta', path[len(source) + 1:])):
                ...
                if filename.split('.')[(-1)].lower() in ('cr2', 'png', 'jpg', 'jpeg'):
                    if internal_vars.has_key('gallery_reduced'):
                        ...
                        if force or not os.path.exists(dst) or not os.stat(dst).st_size or os.stat(source)[8] > os.stat(dst)[8]:
                    ...
                    if internal_vars.has_key('gallery_thumb'):
                        source_1024 = os.path.join(dest, '1024', path[len(source) + 1:])
                        ...
                        if force or not os.path.exists(dst) or not os.stat(dst).st_size or os.stat(source)[8] > os.stat(dst)[8]:
                            ...
                            (out, err) = process.communicate()
                            if retcode != 0:
                                log.warning('Failed to create 150 for %r', source_1024)
                                log.warning('cmd: %s', (' ').join(cmd))
                                log.warning('out: %s', out)
                                log.warning('err: %s', err)
                if internal_vars.has_key('gallery_exif'):
                    if filename.split('.')[(-1)].lower() in ('jpg', 'jpeg'):
                        ...
                        if force or not os.path.exists(dst) or not os.stat(dst).st_size or os.stat(source)[8] > os.stat(dst)[8]:
                            ...
                            retcode = process.wait()
                            if retcode != 0:
                                if 'Warning: JPEG format error, rc = 5' in err or 'No Exif data found in the file' in err:
                                    fp = open(dst, 'w')
                                    fp.write(out)
                                    fp.close()
                                else:
                                ...
                                fp.write(out)
                                fp.close()
                    if filename.split('.')[(-1)].lower() in ('mpeg', 'wmv', '3gp',
                                                             'mov', 'mp4', 'mpg',
                                                             'avi'):
                        if internal_vars.has_key('gallery_still'):
                            dst = os.path.join(dest, 'still', path[len(source) + 1:])
                            if force or not os.path.exists(dst) or not os.stat(dst).st_size or os.stat(source)[8] > os.stat(dst)[8]:
                                log.debug('Converting %r to %r' % (path, dst))
                                cmd = [
                                 'ffmpeg',
                    ...
                    if internal_vars.has_key('gallery_thumb'):
                        source_still = os.path.join(dest, 'still', path[len(source) + 1:])
                        dst = os.path.join(dest, '150', path[len(source) + 1:])
                        if force or not os.path.exists(dst) or not os.stat(dst).st_size or os.stat(source)[8] > os.stat(dst)[8]:
                            os.path.exists(source_still) or log.warning('No extracted still found for %r', path)
                        else:
                            log.debug('Converting %r to %r', source_still, dst)
                            cmd = [
                             'convert',
                             '-auto-orient',
                             '-thumbnail', 'x150',
                            ...
                            (out, err) = process.communicate()
                            if retcode != 0:
                                log.warning('Failed to create thumbnail from still %r', source_still)
                                log.warning('cmd: %s', (' ').join(cmd))
                                log.warning('out: %s', out)
                                log.warning('err: %s', err)
                if internal_vars.has_key('gallery_h264'):
                    ...
                    if force or not os.path.exists(dst) or not os.stat(dst).st_size or os.stat(source)[8] > os.stat(dst)[8]:
                        ...
                        (out, err) = process.communicate()
                        if retcode != 0:
                            log.warning('First pass h264 failed for video %r', path)
                            log.warning('cmd: %s', (' ').join(cmd))
                            log.warning('out: %s', out)
                            log.warning('err: %s', err)
                        else:
                            ...
                            (out, err) = process.communicate()
                            if retcode != 0:
                                log.warning('Second pass h264 failed for video %r', path)
...
def handle_command_metadata(option_sets, command_options, args):
    r"""    usage: ``%(program)s [PROGRAM_OPTIONS] metadata SOURCE DEST [META]``
    ...
    if len(args) == 3:
        ...
        raise internal_vars.has_key('format') or getopt.GetoptError('No FORMAT specified')
    elif internal_vars['format'] not in ('photo', 'gallery'):
        raise getopt.GetoptError('Unrecognized FORMAT %r' % internal_vars['format'])
            ...
            if format == 'gallery':
                for header in ['Path', 'Title', 'Description', 'Index']:
                    if header not in headers:
                        raise getopt.GetoptError('No %s column in CSV file' % header)
    ...
    if internal_vars['format'] == 'photo':
        if not len(args) == 2:
            raise getopt.GetoptError('Got an unexpected argument %r' % args[2])
        for photo in result:
            path = None
            ...
            elif not os.path.exists(os.path.dirname(os.path.join(dest, path))):
                log.debug('Skipping %r, parent directory does not exist.', os.path.join(dest, path))
            else:
                save_record(os.path.join(dest, path), photo)

    elif internal_vars['format'] == 'gallery':
        if not len(args) == 3:
            raise getopt.GetoptError('Expected 3 arguments, including the origin directory')
        ...
        for gallery in result:
            ...
            if not path:
                ...
                if not photo_filenames:
                    ...
                    gallery.append(('photos', ('| ').join(photo_filenames).decode('utf-8')))
    ...
    for name in bases:
        for path in to_make:
            if not os.path.exists(os.path.join(dest, name, path)):
                ...
                raise getopt.GetoptError('The path %r already exists but is not a directory' % os.path.join(dest, name, path))


orientation_types = {1: [], 2: [], 3: [
     '-rotate', '180'], 
   4: [], 5: [], 6: [
     '-rotate', '90'], 
   7: [], 8: [
    ...
    for (dirpath, dirnames, filenames) in os.walk(meta):
        for filename in filenames:
            if filename.startswith('.'):
                log.debug('Ignoring hidden file %r', filename)
                continue
            if filename.split('.')[(-1)].lower() not in ('cr2', 'png', 'jpg', 'jpeg',
                                                         'mpeg', 'wmv', '3gp', 'mov',
                                                         'mp4', 'mpg', 'avi'):
...
option_sets = {'video_overlay_file': [
...
aliases = {'convert': (
   ...
   'autogen': (
             'gc',)}
command_handler_factories = {'convert': makeHandler(handle_command_thumb), 
...
if __name__ == '__main__':
    ...
    try:
        ...
        handle_program(command_handler_factories=command_handler_factories, option_sets=option_sets, aliases=aliases, program_options=program_options, command_options=command_options, command=command, args=args, program_name=program_name, help=help_strings)
