...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gallery/plugin.py
# Compiled at: 2009-05-12 11:36:16
...
"""
import urllib, datetime, logging, os.path, re
...
class GalleryPlugin(Plugin):
    ...
    def parse_config(self, config):
        ...
        if config.has_key('PHOTO_DIRECTORY'):
            result['photo_dir'] = os.path.abspath(os.path.join(self.site_root, config['PHOTO_DIRECTORY'].strip()))
        if config.has_key('GALLERY_DIRECTORY'):
            result['gallery_dir'] = os.path.abspath(os.path.join(self.site_root, config['GALLERY_DIRECTORY'].strip()))
        if config.has_key('GALLERY_TEMPLATE'):
            result['template'] = os.path.abspath(os.path.join(self.site_root, config['GALLERY_TEMPLATE'].strip()))
        return result
    ...
    def on_file(self, path, page=None, template=None):
        ...
        if not self.generated_files:
            ...
            return False
        gen_file = path[:-len('.gallery')] + '.html'
        directory = os.path.split(path)[0]
        add_to_changed = True
        if os.path.exists(gen_file) and os.stat(directory).st_mtime > os.stat(gen_file).st_mtime:
            add_to_changed = False
        if os.path.exists(gen_file) and not os.stat(directory).st_mtime > os.stat(gen_file).st_mtime and not os.stat(path).st_mtime > os.stat(gen_file).st_mtime and not self.force:
        ...
        if galleries > 1:
            log.error('More than one gallery found in %r', os.path.dirname(path))
            return False
        ...
        for (i, filename) in enumerate(photo_filenames):
            ...
            if i > 0:
                prev = (
                 os.path.split(photo_filenames[(i - 1)])[1],)
            next = None
            if i < len(photo_filenames) - 1:
                next = (
                 os.path.split(photo_filenames[(i + 1)])[1],)
            self.make_file(os.path.dirname(path), filename, cur, i + 1, len(photo_filenames), first, last, prev, next)
            links.append('<a href="%s"><img src="%s" alt="Photo or video" /></a>' % (
             cur + '.html',
             relpath(os.path.join(self.site_root, self.config['photo_dir'], '150', filename), os.path.dirname(path))))
        ...
        update(start_path=gen_file, site_root=self.site_root, update_static_items=True)
        return True
    ...
    def build_index_fragment(self, directory):
        ...
        * If there are galleries as sub-directories, use them
        ...
        if os.path.exists(os.path.join(directory, 'index.galleryindex')):
            ...
            galleries = []
            for dir in os.listdir(directory):
                if os.path.isdir(os.path.join(directory, dir)) and not os.path.join(directory, dir).startswith(os.path.join(self.config['gallery_dir'], 'flvplayer')):
        ...
        for gallery in galleries:
            ...
            if not gallery['Index']:
                thumbnail = relpath(os.path.join(self.site_root, self.config['gallery_dir'], 'folder.png'), directory)
            else:
                thumbnail = relpath(os.path.join(self.site_root, self.config['photo_dir'], '150', gallery['Index']), directory)
            index = '<a href="%s"><img src="%s" alt="Photo or video" /></a>' % (
             os.path.join(path, 'index.html'),
             thumbnail)
            content.append('<tr><th valign="top">%s</th><td><b>%s</b><br /><br />%s<br /><br /></td></tr>' % (
             index,
             gallery['Title'],
             gallery['Description']))
    ...
    def build_directory_index(self, directory, title=None):
        ...
        update(start_path=os.path.join(directory, 'index.html'), site_root=self.site_root, update_static_items=True)
    ...
    def make_file(self, directory, filename, link, i, number, first, last, previous, next):
        title = link
        page = DreamweaverTemplateInstance(self.config['template'])
        ...
        if filename.lower().endswith('.jpg') or filename.lower().endswith('.jpeg'):
            page['content'] = '\n                <p style="text-align: center">\n                <img src="%s" alt="%s" /></p>' % (
             relpath(os.path.join(self.site_root, self.config['photo_dir'], '1024', filename), directory),
             'alt')
        else:
            meta_path = os.path.join(self.site_root, self.config['photo_dir'], 'meta', filename)
            ...
            page['content'] = flash_string % {'videowidth': videowidth, 
               'videoheight': videoheight, 
               'width': videowidth + 20, 
               'height': videoheight + 50, 
               'path': relpath(os.path.join(self.site_root, self.config['gallery_dir']), os.path.dirname(os.path.join(directory, link + '.html'))), 
               'url': urllib.quote(relpath(os.path.join(self.site_root, self.config['photo_dir'], 'h264', filename), os.path.join(self.site_root, self.config['gallery_dir'], 'flvplayer'))), 
               'directurl': relpath(os.path.join(self.site_root, self.config['photo_dir'], 'h264', filename), os.path.join(self.site_root, self.config['gallery_dir'], directory)), 
               'still': urllib.quote(relpath(os.path.join(self.site_root, self.config['photo_dir'], 'still', filename), os.path.join(self.site_root, self.config['gallery_dir'], 'flvplayer')))}
        path = os.path.join(self.site_root, self.config['photo_dir'], 'meta', filename)
        if os.path.exists(path):
            data = parse_record(path)
            if data:
                page['content'] += '\n                    <h2>Meta Data</h2>\n                    <table>\n                    %s\n                    </table>\n                    ' % ('').join(('<tr><th>%s</th><td>%s</td></tr>' % (x[0], x[1]) for x in data))
        path = os.path.join(self.site_root, self.config['photo_dir'], 'exif', filename)
        if os.path.exists(path):
            data = parse_record(path)
            if data:
                page['content'] += '\n                    <h2>Exif</h2>\n                    <table>\n                    %s\n                    </table>\n                    ' % ('').join(('<tr><th>%s</th><td>%s</td></tr>' % (x[0], x[1]) for x in data))
        ...
        update(start_path=os.path.join(directory, link + '.html'), site_root=self.site_root, update_static_items=True)
        return True
