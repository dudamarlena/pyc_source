...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/botnee/web/interface/views.py
# Compiled at: 2012-08-16 08:07:05
from django.shortcuts import render_to_response, render
from django import forms
from django.http import HttpResponse
from django.middleware.gzip import GZipMiddleware
from django.template import RequestContext
import datetime, json, StringIO, codecs, os
...
def flush(request):
    if request.method == 'POST':
        ...
        result = [[], [], []]
    c = {'output': ['inserts: ' + unicode(result[0]),
...
class QueryForm(forms.Form):
    ...
    index = forms.IntegerField(min_value=0, max_value=len(engine._meta_dict['guids']) - 1, required=False)
    n_results = forms.IntegerField(min_value=1, max_value=1000, initial=20, required=True)
    title_boost = forms.FloatField(min_value=0, max_value=1, required=True, initial=botnee_config.TITLE_BOOST)
    abstract_boost = forms.FloatField(min_value=0, max_value=1, required=True, initial=botnee_config.ABSTRACT_BOOST)
    date_boost = forms.FloatField(min_value=0, max_value=1, required=True, initial=botnee_config.DATE_BOOST)
    ngram_boost = forms.CharField(max_length=128, initial=[ v['boost'] for v in botnee_config.NGRAMS.values() ], required=False)
    retrieval_type = forms.ChoiceField(choices=RETRIEVAL_CHOICES, initial='TFIDF', required=True)
    feed_title = forms.CharField(max_length=512, required=True, initial='Botnee Feed')
    feed_description = forms.CharField(max_length=1024, required=True, initial='A Sample Feed')
    feed_url = forms.CharField(max_length=1024, required=True, initial='http://www.bmjgroup.com')
    title = forms.CharField(max_length=10000, required=False, widget=forms.Textarea)
    abstract = forms.CharField(max_length=100000, required=False, widget=forms.Textarea)
    body = forms.CharField(max_length=1000000, required=False, widget=forms.Textarea)
    filters = forms.CharField(max_length=1000000, required=False, widget=forms.Textarea)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            with debug.Timer('Received web Query', None, verbose, web_logger):
                ...
                get_related = GetRelated(engine, filters, verbose, title_boost, abstract_boost, date_boost, retrieval_type, ngram_boost)
                scores = None
                idxs = None
                if query_type == 'index':
                ...
                if results:
                    ...
                    if 'query' in request.POST:
                        ...
                        feed = rss_writer.write_feed(summary, feed_title, feed_description, feed_url, c['query'], verbose)
...
def get_matrix_summary(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            verbose = botnee_config.VERBOSE
            with debug.Timer('Received web matrix request', None, verbose, web_logger):
                json_object = form.cleaned_data['filters']
                filters = Filters(json_object, engine._doc_store, True)
                    ...
                    with StringIOStream() as (stream):
                        ...
                        for line in summary.split('\n'):
                            ...
                            if len(items) == 3:
                                ...
                                table_rows.append({'guid0': guid0, 'guid1': guid1, 
                                   'score': score})
...
def meta(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            verbose = botnee_config.VERBOSE
            with debug.Timer('Received web meta data request', None, verbose, web_logger):
                json_object = form.cleaned_data['filters']
                filters = Filters(json_object, engine._doc_store, True)
...
def dump(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            with debug.Timer(None, None, verbose, web_logger):
                std_docs = (doc for (key, doc) in form.cleaned_data.items() if 'input' in key if doc)
                (inserts, updates, deletes) = engine.insert_standard_docs(std_docs, reindex=False, verbose=verbose)
...
def delete(request):
    ...
    if request.method == 'POST':
        ...
        c['form'] = form
        if form.is_valid():
            with debug.Timer('Received web delete request', None, botnee_config.VERBOSE, web_logger):
                guid = form.cleaned_data['guid']
                (inserts, updates, deletes) = engine.delete([
                 guid], reindex=False, verbose=botnee_config.VERBOSE)
                if len(deletes) == 1:
                    c['output'] = [
                     'Successfully deleted: ' + guid, '', '']
                else:
                    c['output'] = [
                     'Unable to delete: ' + guid, '', '']
                c['summary'] = get_engine_summary(request)
...
def force_reindex(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            with debug.Timer('Received dump request', None, botnee_config.VERBOSE, web_logger) as (t):
