...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/scicast/dim_reduction.py
# Compiled at: 2017-03-04 12:54:02
import numpy as np, pandas as pd, os, matplotlib
matplotlib.use('TkAgg')
import matplotlib.pyplot as plt
from sklearn.decomposition import PCA as skPCA
import seaborn as sns, matplotlib.patches as patches
...
def ellip_enclose(points, color, inc=1, lw=2, nst=2):
    ...
    ell = patches.Ellipse(center, width=inc * w, height=inc * h, angle=theta, facecolor=color, alpha=0.2, lw=0)
    edge = patches.Ellipse(center, width=inc * w, height=inc * h, angle=theta, facecolor='none', edgecolor=color, lw=lw)
    return (ell, edge)
...
def plot_kmeans(args, matrix_data, kmeans_range, title=''):
    ...
    for n_clusters in range_n_clusters:
        ...
        for i in range(n_clusters):
            ith_cluster_silhouette_values = sample_silhouette_values_cell[(top_cell_pred == i)]
        ...
        ax2.scatter(top_cell_trans[:, 0], top_cell_trans[:, 1], marker='.', s=30, lw=0, alpha=0.7, c=colors)
        ...
        if args.kmeans_sig_test:
            sns.set(context='poster', font_scale=1.2 - n_clusters * 0.08)
