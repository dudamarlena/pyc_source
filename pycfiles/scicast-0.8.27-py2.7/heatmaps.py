...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/scicast/heatmaps.py
# Compiled at: 2017-01-19 15:15:02
import numpy as np, pandas as pd, os, sys, matplotlib
matplotlib.use('TkAgg')
import matplotlib.pyplot as plt, scipy, json
from scipy.cluster import hierarchy
import seaborn as sns
from functools import reduce
import matplotlib.patches as patches, warnings
...
def clust_heatmap(args, matrix_data, title='', matrix_subset=None, fontsize=18, stablity_plot_num=0, kmeans_color_map={}, plot=True):
    ...
    if longest_side == num_to_plot:
        sns.set(context='poster', font_scale=min(0.4 * (num_to_plot / 100), 3))
        ...
        sns.set(context='poster', font_scale=0.6 * (cell_num / 120))
    ...
    if len(str(args.z_direction)) > 1:
        ...
        try:
            ...
            if matrix_data.gene_label_map and cell_color_map and stablity_plot_num == 0:
                ...
                gene_legend = cg.ax_heatmap.legend(handles=leg_handles_gene, loc=2, bbox_to_anchor=(1.04,
                                                                                                    0.8), title='Gene groups', prop={'size': fontsize})
                plt.setp(gene_legend.get_title(), fontsize=fontsize)
            cell_linkage = cg.dendrogram_col.linkage
            link_mat = pd.DataFrame(cell_linkage, columns=[
             'row label 1', 'row label 2', 'distance', 'no. of items in clust.'], index=[ 'cluster %d' % (i + 1) for i in range(cell_linkage.shape[0]) ])
            if plot:
                if title != '':
                ...
                plt.close('all')
            return (
             cell_linkage, df_by_gene[gene_list], col_order)
