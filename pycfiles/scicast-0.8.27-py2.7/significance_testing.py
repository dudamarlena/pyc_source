...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/scicast/significance_testing.py
# Compiled at: 2017-03-04 12:54:02
import pandas as pd, os, matplotlib
matplotlib.use('TkAgg')
import matplotlib.pyplot as plt, scipy, seaborn as sns, itertools, matplotlib.patches as patches, warnings, numpy as np
    ...
    if len(list(set(group_name_list))) == 1:
        ...
        for gp in group_pairs:
            ...
            if cell1_present != [] and cell2_present != []:
                ...
                if args.verbose:
                    print (
                     gp[1], 'not present in cell matrix')
            elif args.verbose:
                print (
                 gp[0], 'not present in cell matrix')
