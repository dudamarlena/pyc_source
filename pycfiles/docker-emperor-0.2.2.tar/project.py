...
# [GCC 8.4.0]
# Embedded file name: /home/dalou/www/DOCKER/docker-emperor/docker_emperor/nodes/project.py
# Compiled at: 2019-05-29 02:09:38
import os, six
...
class Project(dict):
    FILES = [
     'docker-emperor.yml', 'docker-compose.yml']
    ...
    def name(self):
        ...
        if not hasattr(self, n):
            setattr(self, n, self.pop('name', self.pop('project_name', self['environment'].get('COMPOSE_PROJECT_NAME', os.environ.get('COMPOSE_PROJECT_NAME', os.path.basename(self.root.root_path))))))
        ...
        if name in commands:
            ...
            for line in command.commands:
                if line == name:
                    ...
                    cmd = self.root.bash('docker-emperor', compose=self.compose, is_system=True, *cmd_args)
