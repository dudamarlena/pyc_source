...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/strictdict/tests/test_strict_dict.py
# Compiled at: 2015-05-13 07:06:42
"""
...
def test_create_ok(leg_data, key, value):
    ...
    try:
        ...
        pytest.fail(('{key}={value} raised ValidationError: {exc}').format(key=key, value=value, exc=exc))
...
@pytest.mark.parametrize(('desc', 'data'), [
 (
  'Pure dicts', {'legs': [leg_data()] * 10, 'favorite_leg': leg_data()}),
 (
  'Iterable: objects, non-iterable: dict', {'legs': [Leg(**leg_data())] * 10, 'favorite_leg': leg_data()}),
 (
  'Iterable: empty list, non-iterable: whatever', {'legs': [], 'favorite_leg': leg_data()}),
 (
  'Iterable: objects mixed with dicts, non-iterable: object',
  {'legs': [Leg(**leg_data())] * 5 + [leg_data()] * 3, 'favorite_leg': Leg(**leg_data())})])
...
def test_invalid_nested_dict(leg_data):
    ...
    with pytest.raises(ValidationError):
        Centipede(**{'age': 1, 'legs': [
                  valid_leg] * 10, 
           'favorite_leg': invalid_leg})
    with pytest.raises(ValidationError):
        Centipede(**{'age': 1, 'legs': [
                  valid_leg] * 10 + [invalid_leg], 
           'favorite_leg': valid_leg})
    with pytest.raises(ValidationError):
        Centipede(**{'age': 1, 'favorite_leg': valid_leg})
...
def test_map_field():
    ...
    class Class_v1(StrictDict):
        field1 = api.ref(f.MapField, api.ref(f.String), api.slist(f.Int))
