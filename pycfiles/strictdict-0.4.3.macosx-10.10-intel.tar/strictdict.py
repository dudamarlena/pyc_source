...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/strictdict/strictbase/strictdict.py
# Compiled at: 2015-05-13 07:06:42
import abc, collections, copy, json, msgpack
...
class StrictDictMeta(abc.ABCMeta):
    ...
    def __new__(meta, name, bases, dict_):
        ...
        for attrname, attr in dict_.items():
...
class StrictDict(_StrictDictInterface):
    ...
    def __init__(self, **kwargs):
        if self.is_ignore_unknown_fields:
            kwargs = {k:v for k, v in kwargs.iteritems() if k in self.__fields__.keys()}
        ...
        for key, field in self.__fields__.iteritems():
            ...
            if value_is_empty and field.required:
                exc = ValidationError(('Required field {} is empty or missing!').format(key), path=[], class_=self.__class__, value=full_kwargs)
        ...
        if extra_keys:
            raise ValidationError(('No such fields: {}').format((', ').join(str(x) for x in extra_keys)), class_=self.__class__)
        return
