...
# [GCC 8.4.0]
# Embedded file name: /home/tim/dev/python/piep/test/test_line_ops.py
# Compiled at: 2014-04-26 06:37:46
...
class TestParsing(TestCase):
    ...
    def test_complex_parsing(self):
        self.assertEqual(run('("%s) %s || %s" % ((i + 1) | 0x00, p, p.upper())) | p', ['a', 'b', 'c']), [
         '1) a || A',
         '2) b || B',
         '3) c || C'])
...
class TestJoining(TestCase):
    ...
    def test_default_join(self):
        self.assertEqual(run('p.split(".")', ['a.b.c']), [
         'a b c'])

    def test_custom_join(self):
        self.assertEqual(run('--join=/', 'p.split(".")', ['a.b.c']), [
         'a/b/c'])
    ...
    def test_chaining_of_line_methods(self):
        self.assertEqual(run('p.split("-") | p.join(" ")', ['1-2-3']), [
         '1 2 3'])
...
class TestFilters(TestCase):
    ...
    def test_isdigit(self):
        self.assertEqual(run('p.isdigit()', ['1', '2', 'three']), [
         '1', '2'])

    def test_filter_in_pipeline(self):
        self.assertEqual(run('p.isdigit() | "Digit:", p', ['1', '2', 'three']), [
         'Digit: 1', 'Digit: 2'])

    def test_index(self):
        self.assertEqual(run('--debug', 'i % 2 == 0', ['0', '1', '2', '3', '4']), [
         '0', '2', '4'])
