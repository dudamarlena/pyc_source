...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\optparse_gui\__init__.py
# Compiled at: 2008-03-08 07:12:57
"""
...
class OptparseDialog(wx.Dialog):
    ...
    def __init__(self, option_parser, parent=None, ID=0, title='Optparse Dialog', pos=wx.DefaultPosition, size=wx.DefaultSize, style=wx.DEFAULT_DIALOG_STYLE, name='OptparseDialog'):
        ...
        for option in option_parser.option_list:
            ...
            if 'store' == option.action:
                ...
                if 'choice' == option.type:
                    ...
                    ctrl = wx.ComboBox(self, -1, choices=option.choices, value=option.default, style=wx.CB_DROPDOWN | wx.CB_READONLY | wx.CB_SORT)
                else:
        ...
        self.args_ctrl = wx.TextCtrl(self, -1, '', size=(-1, 100), style=wx.TE_MULTILINE | wx.TE_PROCESS_ENTER)
        self.args_ctrl.SetHelpText('Args can either be separated by a space or a newline\nArgs the contain spaces must be entered like so: "arg with sapce" \n')
        box.Add(label, 0, wx.ALIGN_CENTRE | wx.ALL, 5)
    ...
    def _getArgs(self):
        ...
        args = re.findall('(?:((?:(?:\\w|\\d)+)|".*?"))\\s*', args_buff)
        return args
...
def sample_parse_args():
    ...
    parser.add_option('-f', '--file', dest='filename', default='c:\\1.txt', help='read data from FILENAME')
    parser.add_option('-a', '--action', dest='action', choices=[
     'delete', 'copy', 'move'], help='Which action do you wish to take?!')
    parser.add_option('-n', '--number', dest='number', default=23, type='int', help='Just a number')
