...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/mousedb/timed_mating/tests.py
# Compiled at: 2010-06-20 19:21:58
...
class GroupsModelTests(TestCase):
    ...
    def test_create_plugevent_most_fields(self):
        ...
        new_plugevent = PlugEvents(Breeding=Breeding(Strain=Strain(Strain='Test Strain', Strain_slug='Test_Strain')), PlugDate=datetime.date.today(), PlugMale=Animal(Strain=Strain(Strain='Test Strain', Strain_slug='Test_Strain'), Background='Mixed', Genotype='-/-', Gender='M'), PlugFemale=Animal(Strain=Strain(Strain='Test Strain', Strain_slug='Test_Strain'), Background='Mixed', Genotype='-/-', Gender='M'))
        new_plugevent.save()
        test_plugevent = PlugEvents.objects.get(PlugDate=datetime.date.today())
