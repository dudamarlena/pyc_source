...
# Embedded file name: build/bdist.linux-x86_64/egg/confiture/schema/containers.py
# Compiled at: 2014-05-11 06:19:13
__doc__ = ' Builtin containers of confiture.schema\n'
import sys
try:
    import argparse
except ImportError:
...
class Value(ArgparseContainer):
    ...
    def populate_argparse(self, parser, name):
        ...
        if self._argparse_names:
            ...
            parser.add_argument(action=Action, type=self._type.cast, nargs=nargs, metavar=self._argparse_metavar, help=self._argparse_help, const=const, *self._argparse_names)
            if self._type.is_argparse_flag and self._argparse_names_invert:
                parser.add_argument(action=Action, type=self._type.cast, nargs=0, help=self._argparse_help_invert, const=False, *self._argparse_names_invert)
        return
        ...
        if value is None:
            ...
            if isinstance(value.value, list):
                if len(value.value) == 1:
                    ...
                    raise ValidationError('%r is a list' % value.value, position=value.position)
            try:
...
class Choice(ArgparseContainer):
    ...
    def populate_argparse(self, parser, name):
        ...
        if self._argparse_names:
            parser.add_argument(action=Action, choices=list(self._choices.keys()), metavar=self._argparse_metavar, help=self._argparse_help, *self._argparse_names)
    ...
    def validate(self, value):
        ...
        if value is None:
            ...
            if value.value in self._choices:
                return ConfigValue(value.name, self._choices[value.value], position=value.position)
...
class List(ArgparseContainer):
    ...
    def populate_argparse(self, parser, name):
        ...
        if self._argparse_names:
            nargs = '*'
            parser.add_argument(action=Action, type=self._type.cast, nargs=nargs, metavar=self._argparse_metavar, help=self._argparse_help, *self._argparse_names)
        ...
        if value is None:
            ...
            for i, item in enumerate(values):
                try:
                    ...
                    raise ValidationError('item #%d, %s' % (i, err), position=value.position)
                else:
...
class Array(List):
    r""" An array container used to store a fixed size list of scalar values of
        the specified type.
    ...
    def populate_argparse(self, parser, name):
        ...
        if self._argparse_names:
            parser.add_argument(action=Action, type=self._type.cast, nargs=self._size, metavar=self._argparse_metavar, help=self._argparse_help, *self._argparse_names)
...
class TypedArray(ArgparseContainer):
    ...
    def populate_argparse(self, parser, name):
        ...
        if self._argparse_names:
            nargs = '*'
            parser.add_argument(action=Action, type=str, nargs=nargs, metavar=self._argparse_metavar, help=self._argparse_help, *self._argparse_names)
...
class Section(Container):
    r""" A section container used to store a mapping between name and other
        containers.

        :param \*\*kwargs: parameters used to override meta
    """
    _meta = {'args': None, 'unique': False, 
       'repeat': once, 
       'allow_unknown': False}
    ...
    def validate(self, section):
        ...
        validated_section = ConfigSection(section.name, section.parent, position=section.position)
        if self.meta['args'] is None and section.args is not None:
            raise ValidationError('section %s, this section does not take any argument' % section.name, position=section.position)
        else:
            if self.meta['args'] is not None:
                try:
            ...
            for name, container in self.keys.items():
                if isinstance(container, Section):
                    ...
                    if len(subsections) < rmin:
                        raise ValidationError('section %s, section must be defined at least %d times' % (
                         name, rmin))
                    if rmax is not None and len(subsections) > rmax:
                        raise ValidationError('section %s, section must be defined at max %d times' % (
                         name, rmax))
                    args = set()
                    ...
                    try:
                        ...
                        raise ValidationError('section %s, key %s, %s' % (section.name, name, err), position=err.position)
                    else:
