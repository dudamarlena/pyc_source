...
# Embedded file name: build/bdist.linux-x86_64/egg/confiture/tree.py
# Compiled at: 2014-05-11 06:19:13
__doc__ = ' Classes used to represent the configuration tree.\n'
...
class ConfigSection(object):
    ...
    def iteritems(self, expand_sections=False):
        ...
        """
        if expand_sections:
            return chain(iter(self._values.items()), ((k, v) for k, l in self._subsections.items() for v in l))
        else:
    ...
    def subsection(self, name, default=None):
        ...
        if len(self._subsections.get(name, [])) > 1:
            ...
            if name not in self._subsections or len(self._subsections[name]) == 0:
                return default
