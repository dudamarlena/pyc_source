...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/oosheet/__init__.py
# Compiled at: 2017-11-24 09:18:25
import sys, os
...
class OODoc(object):
    ...
    def dispatch(self, cmd, *args):
        ...
        self.dispatcher.executeDispatch(self.model.getCurrentController(), '.uno:%s' % cmd, '', 0, args)
...
class OOSheet(OODoc):
    ...
    def selector(self):
        ...
        """
        return self._generate_selector(self.start_col, self.end_col, self.start_row, self.end_row)
    ...
    def __getitem__(self, key):
        ...
        if isinstance(stop, str):
            ...
            row_sliced = True
        if not row_sliced:
            return OOSheet(self._generate_selector(self.start_col, self.end_col, self.start_row + start, self.start_row + stop), _row_sliced=True)
        else:
            return OOSheet(self._generate_selector(self.start_col + start, self.start_col + stop, self.start_row, self.end_row))

    def __eq__(self, peer):
        return self.sheet.Name == peer.sheet.Name and self.start_row == peer.start_row and self.start_col == peer.start_col and self.end_row == peer.end_row and self.end_col == peer.end_col
    ...
    def rows(self):
        ...
        """
        for row in range(self.start_row, self.end_row + 1):
            yield OOSheet(self._generate_selector(self.start_col, self.end_col, row, row), _row_sliced=True)
    ...
    def flatten(self):
        ...
        self.copy()
        self.focus()
        self.dispatch('InsertContents', ('Flags', 'SVDNT'), ('FormulaCommand', 0), (
         'SkipEmptyCells', False), (
         'Transpose', False), (
         'AsLink', False), ('MoveMode', 4))
        return self
    ...
    def format_as(self, selector):
        ...
        self.focus()
        self.dispatch('InsertContents', ('Flags', 'T'), ('FormulaCommand', 0), (
         'SkipEmptyCells', False), (
         'Transpose', False), (
         'AsLink', False), ('MoveMode', 4))
...
class OOPacker():
    ...
    def pack(self):
        ...
        subprocess.Popen(['zip', '-0', '-X', document, 'mimetype'], cwd=self.tmp).wait()
        subprocess.Popen(['zip', document, '-r'] + os.listdir(self.tmp) + ['-x', 'mimetype'], cwd=self.tmp).wait()
