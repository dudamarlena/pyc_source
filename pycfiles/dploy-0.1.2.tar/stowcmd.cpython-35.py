...
# [GCC 8.4.0]
# Embedded file name: /evtfs/home/rcarney/Dropbox/projects/dploy/master/dploy/stowcmd.py
# Compiled at: 2017-10-19 14:13:37
# Size of source mod 2**32: 12238 bytes
"""
The logic and workings behind the stow and unstow sub-commands
"""
from collections import Counter
import pathlib, dploy.actions as actions, dploy.utils as utils, dploy.error as error, dploy.main as main
            ...
            if self.ignore.should_ignore(subsources):
                ...
                if does_dest_path_exist:
                    ...
                    if dest_path.is_symlink():
                        ...
                        if not dest_path.parent.exists() and not self.is_unfolding:
...
class CollectFoldingActions:
    __doc__ = '\n    '

    def __init__(self, actions, dest_input):
        self.actions = actions
        self.dest_input = dest_input
        for parent in self.actions.get_unlink_target_parents():
            self.items = utils.get_directory_contents(parent)
            self.other_links_parents = []
            self.other_links = []
            self.source_parent = None
            self.is_normal_files_detected = False
            self.name(parent)
        ...
        for item in self.items:
            if item not in self.actions.get_unlink_targets():
                ...
                if does_item_exist and item.is_symlink():
                    self.source_parent = item.resolve().parent
                    self.other_links_parents.append(item.resolve().parent)
                    self.other_links.append(item)
                else:
                    self.is_normal_files_detected = True
                    break

        if not self.is_normal_files_detected:
            self.other_links_parent_count = len(Counter(self.other_links_parents))
            if self.other_links_parent_count == 1:
                pass
            assert self.source_parent != None
            if utils.is_same_files(utils.get_directory_contents(self.source_parent), self.other_links):
                self._fold(self.source_parent, parent)
        elif self.other_links_parent_count == 0 and not utils.is_same_file(parent, self.dest_input):
...
class UnStow(AbstractBaseStow):
    ...
    def _collect_folding_actions(self):
        ...
        """
        CollectFoldingActions(self.actions, self.dest_input)
