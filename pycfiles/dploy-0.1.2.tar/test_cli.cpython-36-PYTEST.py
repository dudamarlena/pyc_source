...
# [GCC 8.4.0]
# Embedded file name: /Users/arecarn/Dropbox/projects/dploy/master/tests/test_cli.py
# Compiled at: 2017-10-27 00:20:13
...
def test_cli_with_stow_with_simple_senario(source_only_files, dest, capsys):
    ...
    if not @py_assert1:
        @py_format10 = @pytest_ar._call_reprcompare(('==',), (@py_assert1,), ('%(py0)s == %(py9)s\n{%(py9)s = %(py5)s\n{%(py5)s = %(py3)s.format\n}(source=%(py6)s, dest=%(py7)s)\n}',), (out, @py_assert8)) % {'py0':@pytest_ar._saferepr(out) if 'out' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(out) else 'out',  'py3':@pytest_ar._saferepr(@py_assert2),  'py5':@pytest_ar._saferepr(@py_assert4),  'py6':@pytest_ar._saferepr(s) if 's' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(s) else 's',  'py7':@pytest_ar._saferepr(d) if 'd' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(d) else 'd',  'py9':@pytest_ar._saferepr(@py_assert8)}
...
def test_cli_with_version_option(capsys):
    ...
    with pytest.raises(SystemExit):
        ...
        if not @py_assert8:
            @py_format11 = @pytest_ar._call_reprcompare(('is not', ), (@py_assert8,), ('%(py7)s\n{%(py7)s = %(py2)s\n{%(py2)s = %(py0)s.match\n}(%(py4)s, %(py5)s)\n} is not %(py10)s', ), (@py_assert6, @py_assert9)) % {'py0':@pytest_ar._saferepr(re) if 're' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(re) else 're',  'py2':@pytest_ar._saferepr(@py_assert1),  'py4':@pytest_ar._saferepr(@py_assert3),  'py5':@pytest_ar._saferepr(out) if 'out' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(out) else 'out',  'py7':@pytest_ar._saferepr(@py_assert6),  'py10':@pytest_ar._saferepr(@py_assert9)}
