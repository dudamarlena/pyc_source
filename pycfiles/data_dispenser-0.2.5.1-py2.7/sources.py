...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/data_dispenser/sources.py
# Compiled at: 2015-02-06 12:17:00
"""
...
import csv, doctest, glob, itertools, json, logging, os.path, pickle, pprint, re, sys, urllib.parse, xml.etree.ElementTree as et
try:
    import yaml
except ImportError:
...
if yaml:
    ...
    def ordered_yaml_load(stream, Loader=yaml.Loader, object_pairs_hook=OrderedDict, *args, **kwargs):
        """
...
class Source(object):
    ...
    """
    eval_funcs_by_ext = {'.py': [_eval_file_obj], '.json': [
               json_loader], 
       '.yaml': [
               ordered_yaml_load], 
       '.yml': [
       ...
       '.pickle': [
                 pickle_loader]}
    eval_funcs_by_ext['*'] = eval_funcs_by_ext['.pickle'] + [_eval_file_obj] + eval_funcs_by_ext['.html'] + eval_funcs_by_ext['.xml'] + eval_funcs_by_ext['.json'] + eval_funcs_by_ext['.yaml'] + eval_funcs_by_ext['.csv']
    table_count = 0
    ...
    def _deserialize(self, open_file):
        ...
        for deserializer in self.deserializers:
            ...
            try:
                ...
                self.file.seek(0)
                if row_1:
                    if deserializer == ordered_yaml_load and isinstance(row_1, str) and len(row_1) == 1:
    ...
    def _source_is_path(self, src):
        ...
        self.deserializers = self.eval_funcs_by_ext.get(file_extension, self.eval_funcs_by_ext['*'])
    ...
    _actual_ext_finder = re.compile('^(\\.[A-Za-z]*)')
    ...
    def _source_is_excel_worksheet(self, sheet, name):
        ...
        for row_n in range(sheet.nrows):
            ...
            if row_has_data:
                headings = [ heading if heading else default_heading for heading, default_heading in itertools.zip_longest(sheet.row_values(row_n), headings)
                           ]
                row_n += 1
                break

        data = [ OrderedDict(zip(headings, sheet.row_values(r))) for r in range(row_n, sheet.nrows)
               ]
...
sqlalchemy_connection_parser = re.compile('^(\\w+)://')
