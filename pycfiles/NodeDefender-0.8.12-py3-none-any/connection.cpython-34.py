...
# [GCC 8.4.0]
# Embedded file name: /mnt/develop/NodeDefender/NodeDefender/mqtt/connection.py
# Compiled at: 2018-01-14 00:29:40
...
def add(host, port=1883, username=None, password=None):
    ...
    try:
        ...
        NodeDefender.mqtt.logger.error('MQTT {}:{} unable to connect'.format(host, port))
        return False

    NodeDefender.db.mqtt.mark_online(host, port)
    NodeDefender.mqtt.logger.info('MQTT {}:{} initialized'.format(host, port))
    mqtt.loop_start()
    return True
...
def load(mqttlist=None):
    ...
    for m in mqttlist:
        ...
        NodeDefender.mqtt.logger.info('MQTT {}:{} Loaded'.format(m.host, m.port))
...
class MQTT:
    ...
    def on_message(self, client, userdata, msg):
        NodeDefender.mqtt.message.event(msg.topic, msg.payload.decode('utf-8'), self.info)