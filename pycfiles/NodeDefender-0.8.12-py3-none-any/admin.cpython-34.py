...
# [GCC 8.4.0]
# Embedded file name: /mnt/develop/NodeDefender/NodeDefender/frontend/views/admin.py
# Compiled at: 2018-01-09 06:06:44
# Size of source mod 2**32: 5671 bytes
from NodeDefender.frontend.views import admin_view
from NodeDefender.frontend.forms.admin import GeneralForm, CreateUserForm, CreateGroupForm, CreateMQTTForm, UserSettings, UserPassword, UserGroupAdd
...
def admin_server():
    ...
    if request.method == 'GET':
        return render_template('frontend/admin/server.html', MQTTList=MQTTList, MQTTForm=MQTT)
    else:
        if MQTT.Submit.data:
            if MQTT.validate_on_submit():
                try:
...
def admin_groups():
    ...
    if request.method == 'GET':
        return render_template('frontend/admin/groups.html', groups=groups, CreateGroupForm=GroupForm)
    else:
        ...
        try:
            ...
            NodeDefender.db.group.update(group.name, **{'email': GroupForm.Email.data,  'description': GroupForm.description.data})
        except ValueError as e:
            flash('Error: {}'.format(e), 'danger')
            return redirect(url_for('admin_view.admin_groups'))

        flash('Successfully Created Group: {}'.format(group.name), 'success')
        return redirect(url_for('admin_view.admin_group', name=NodeDefender.serializer.dumps(group.name)))
...
def admin_users():
    ...
    if request.method == 'GET':
        ...
        return render_template('frontend/admin/users.html', Users=users, CreateUserForm=UserForm)
    if not UserForm.validate():
        flash('Error adding user', 'danger')
        return redirect(url_for('admin_view.admin_users'))
    try:
        user = NodeDefender.db.user.create(UserForm.Email.data, UserForm.Firstname.data, UserForm.Lastname.data)
    except ValueError as e:
        flash('Error: {}'.format(e), 'danger')
...
def admin_user(email):
    ...
    if request.method == 'GET':
        if user is None:
            ...
            return redirect(url_for('admin_view.admin_groups'))
        return render_template('frontend/admin/user.html', User=user, UserSettings=usersettings, UserPassword=userpassword, UserGroupAdd=usergroupadd)
    if usersettings.Email.data and usersettings.validate():
        NodeDefender.db.user.update(usersettings.Email.data, **{'firstname': usersettings.Firstname.data,  'lastname': usersettings.Lastname.data})
        return redirect(url_for('admin_view.admin_user', email=NodeDefender.serializer.dumps(usersettings.Email.data)))
