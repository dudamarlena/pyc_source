...
# [GCC 8.4.0]
# Embedded file name: /mnt/develop/NodeDefender/NodeDefender/config/database.py
# Compiled at: 2018-04-04 03:57:16
import NodeDefender, flask_migrate, sqlalchemy, os
from flask_sqlalchemy import SQLAlchemy
import alembic, shutil, pip
default_config = {'enabled': False, 'engine': '', 
   'username': '', 
   'password': '', 
   'host': '', 
   'port': '', 
...
def load_config(parser):
    if eval(parser['DATABASE']['ENABLED']):
        ...
        config['enabled'] = True
    NodeDefender.app.config.update(DATABASE=config['enabled'], DATABASE_ENGINE=config['engine'], DATABASE_USERNAME=config['username'], DATABASE_PASSWORD=config['password'], DATABASE_HOST=config['host'], DATABASE_PORT=config['port'], DATABASE_DATABASE=config['database'], DATABASE_FILEPATH=config['filepath'])
    if NodeDefender.app.testing:
        NodeDefender.app.config.update(SQLALCHEMY_DATABASE_URI='sqlite:///:memory:')
    else:
        NodeDefender.app.config.update(SQLALCHEMY_DATABASE_URI=get_uri())
    return config
    ...
    for key, value in kwargs.items():
        ...
        if key == 'engine' and value == 'postgresql':
            if not install_postgresql():
                raise ImportError('Not able to install PostgreSQL                        Please verify that libpq-dev is installed')
