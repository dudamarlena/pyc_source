...
# [GCC 8.4.0]
# Embedded file name: /mnt/develop/NodeDefender/NodeDefender/manage/setup/celery.py
...
def celery():
    print_topic('Celery')
    print_info('Celery is used for concurrent operation. It will spawn multiple               workes on multiple CPU cores and possibly even on multiple               hosts, running as a cluster. Disabling Celery will make               NodeDefender as a single process. Celery requires AMQP or Redis               to communicate between workes')
    enabled = None
    ...
    NodeDefender.config.celery.set(enabled=True, broker=broker, host=host, port=port, database=database, server_name=server_name)
    if NodeDefender.config.celery.write():
        print_info('Celery- config successfully written')
    return True