...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mercury_agent/hardware/erase/os_level_erase.py
...
def write_random_signatures(target, target_size_bytes, number):
    signature_map = [(uuid.uuid4().bytes, random.randint(0, int((target_size_bytes - 16) / 16)) * 16) for _ in range(number)]
...
def ssd_swap_discard(target):
    ...
    def _run(command):
        try:
            ...
            raise MercuryCritical('CLI error during drive erase; target: {}, error: {}'.format(target, e))
...
def erase(target, method='zero', buffer_size_bytes=4194304, verification_blocks=4):
    ...
    log.info('Writing %s verification signatures to %s', verification_blocks, target)
    signature_map = write_random_signatures(target, target_size, verification_blocks)
    log.info('Beginning %s operation on %s [size: %s]', method, target, target_size)
    if method == 'zero':
        ...
        if method == 'ssd_trim':
            ...
            raise MercuryGeneralException(f"Unsupported erase method specified: {method}")
    if not validate_erased(target, signature_map):
        raise MercuryCritical('Could not validate erase procedure, Signatures are intact! {}'.format(signature_map))
    completed_at = time.time()
    time_taken = completed_at - start
    speed_stat = target_size / time_taken / 1048576
    log.info('Wipe operation completed in %s [%s MB/s]', timedelta(seconds=(round(completed_at - start))), speed_stat)
    return {'start_time':start, 
     'completed_at':completed_at,  'target_size':target_size, 
     'target':target,  'speed':f"{speed_stat} MB/s"}