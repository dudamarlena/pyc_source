...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mercury_agent/procedures/megaraid.py
...
@capability('megaraid_add', description='Create and array on a megaraid_sas based controller',
  kwarg_names=[
 'controller', 'array_type', 'drives'],
  serial=True,
  dependency_callback=has_megaraid_driver,
  timeout=60,
  task_id_kwargs=True)
@update_on_change
def megaraid_add(controller, array_type, drives, size=None, pdperarray=None, pdcache=None, dimmerswitch=None, io_mode='direct', write_policy='wb', read_policy='ra', cachevd=False, stripe_size=None, spares=None, cached_bad_bbu=False, after_vd=None):
    """ Add virtual drive
...
discharged
    ...
    log.info('Adding Array: /c{} RAID{} drives: {} size: {}'.format(controller, array_type, drives, size))
    return driver.storcli.add(controller,
      array_type,
      drives,
      size=size,
...
@capability('megaraid_delete', description='Delete megaraid based virtual drive',
  kwarg_names=[
 'controller'],
  serial=True,
  dependency_callback=has_megaraid_driver,
...
def megaraid_delete(controller, virtual_drive='all'):
    ...
    """
    driver = get_megaraid_driver()
    log.info('Deleting virtual drive: {} on controller: {}'.format(controller, virtual_drive))
