...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/unit/hardware/raid/interfaces/megaraid/test_megacli.py
# Compiled at: 2018-10-04 17:41:36
# Size of source mod 2**32: 7898 bytes
import unittest, mock
from subprocess import PIPE
from mercury_agent.hardware.raid.interfaces.megaraid.megacli import LSIRaid, LSIRaidException, clear_lsi
...
class TestLSIRaidInit(unittest.TestCase):
    ...
    def test_refresh(self, mock_init, mock_get_physical_disks, mock_get_virtual_disks, mock_get_enclosure_id, mock_get_adapter_raw, mock_count_virtual_disks):
...
class TestLSIRaid(unittest.TestCase):
    ...
    def setUp(self):
        self.patch_os = mock.patch('mercury_agent.hardware.raid.interfaces.megaraid.megacli.os.path.isfile')
        self.patch_os.start()
        self.patch_refresh = mock.patch.object(LSIRaid, 'refresh')
        self.patch_refresh.start()
        self.patch_time = mock.patch('mercury_agent.hardware.raid.interfaces.megaraid.megacli.time')
    ...
    def test_megacli(self, mock_popen):
        ...
        mock_popen.return_value = mock.Mock(poll=mock.Mock(return_value=1),
          communicate=mock_communicte,
          returncode=0)
        cmd = '-AdpAllInfo -aall'
        out, err, ret = LSIRaid().megacli(cmd)
        expected_cmd = ['/usr/sbin/megacli'] + cmd.split()
        mock_popen.assert_called_once_with((expected_cmd + ['-NoLog']),
          stdout=PIPE, stderr=PIPE, bufsize=1048567)
