...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mercury_agent/procedures/hp_raid.py
...
@capability('hpssa_create_array', description='Create array on an HP SmartArray Controller',
  kwarg_names=[
 'slot', 'selection', 'raid'],
  serial=True,
  ...
  timeout=120)
@update_on_change
def hpssa_create_array(slot, selection, raid, array_letter=None, array_type='ld', size='max', stripe_size='default', write_policy='writeback', sectors=32, caching=True, data_ld=None, parity_init_method='default'):
    """
    Create an array
    ...
    result = hp_raid_driver.handler.hpssa.create(slot,
      selection,
      raid,
    ...
    if result.returncode:
        ...
        raise MemoryError('Failed to create array: stdout={0}\nstderr={1}'.format(result, result.stderr))
    return {'stdout':result,  'stderr':result.stderr,  'returncode':result.returncode}


@capability('hpssa_delete_ld', description='Delete a logical drive on a given controller', kwarg_names=[
 'slot', 'logical_drive'],
  serial=True,
  dependency_callback=has_hp_raid_driver,
  timeout=120)
...
@capability('hpssa_clear_configuration', description='Delete all arrays on a given controller', kwarg_names=[
 'slot'],
  serial=True,
  dependency_callback=has_hp_raid_driver,
  timeout=120)
...
@capability('hpssa_clear_configuration_all_controllers', description='Delete all configurations from all RAID controllers',
  serial=True,
  dependency_callback=has_hp_raid_driver,
  timeout=120)
@update_on_change
...
@capability('hpssa_add_spares', description='Add spare drives to one, many, or all arrays',
  serial=True,
  dependency_callback=has_hp_raid_driver,
  kwarg_names=[
 'slot', 'selection'],
...
def hpssa_add_spares(slot, selection, array_letters=None):
    ...
    :return:
    """
    results = get_hp_raid_driver().handler.hpssa.add_spares(slot, selection, array_letters)
    return [{'stdout':result,  'stderr':result.stderr,  'returncode':result.returncode} for result in results]