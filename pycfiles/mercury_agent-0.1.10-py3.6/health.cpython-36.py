...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mercury_agent/inspector/inspectors/health.py
...
def _qc_hp(_oem_details, log_error):
    ...
    hpasm = hpasmcli.HPASMCLI(get_configuration().agent.hardware.oem.hp.hpasmcli_path)
    ...
    for i, proc in enumerate(_oem_details['server'].get('processors', [])):
        if proc['status'] != 'Ok':
            log_error('Processor {} is not in a nominal state. Reported state: {}'.format(i, proc['status']))

    for dimm in _oem_details.get('dimm', []):
        if dimm['status'] != 'Ok':
            log_error('DIMM module {} on CPU controller {} is not in a nominal state. Reporting state: {}'.format(dimm['module_#'], dimm['processor_#'], dimm['status']))

    for i, power_supply in enumerate(_oem_details.get('power_supplies')):
        if power_supply['condition'] != 'Ok':
            log_error('Power supply {} is not in a nominal state. Condition: {}'.format(i, power_supply['condition']))


@expose_late('system_health')
def system_health_inspector(device_info):
    agent_configuration = get_configuration().agent
    _health = {'corrected_hardware_event_count':mcelog.count_logged_events(), 
    ...
    if _health['corrected_hardware_event_count'] >= agent_configuration.hardware.mce_threshold:
        log_error('Hardware event count is {} which exceeds the threshold of {}'.format(_health['corrected_hardware_event_count'], agent_configuration.hardware.mce_threshold))
    _oem_details = {}
