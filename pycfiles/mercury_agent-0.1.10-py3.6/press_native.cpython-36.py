...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mercury_agent/procedures/press_native.py
# Compiled at: 2018-05-29 11:55:04
...
def entry(run_configuration, mercury_press_configuration):
    ...
    init_plugins(run_configuration, mercury_press_configuration.get('plugins', {}).get('scan_directories'), mercury_press_configuration.get('plugins', {}).get('enabled'))
    return_data = {}
    p = None
    try:
        p = PressOrchestrator(run_configuration, mercury_press_configuration.get('paths', {}).get('parted', 'parted'), mercury_press_configuration.get('deployment_root', '/mnt/press'), mercury_press_configuration.get('staging_directory', '/.press'), mercury_press_configuration.get('layout', {}).get('use_fibre_channel', False), mercury_press_configuration.get('layout', {}).get('loop_only', False), mercury_press_configuration.get('partition_table', {}).get('partition_start', 1048576), mercury_press_configuration.get('partition_table', {}).get('alignment', 1048576), mercury_press_configuration.get('volume_group', {}).get('pe_size', '4MiB'))
    except Exception:
        exec_dict = parse_exception()
        log.error('Error during initialization: {}'.format(fancy_traceback_short(exec_dict)))
    ...
    if p:
        try:
            try:
                ...
                exec_dict = parse_exception()
                log.error('Error during deployment: {}'.format(fancy_traceback_short(exec_dict)))
...
def add_mercury_plugin_data(press_configuration, task_id):
    ...
    press_configuration['mercury'] = {'task_id':task_id, 
     'backend_zurl':get_configuration().agent.remote.backend_url}


@capability('press', description='Native press support in mercury', serial=True, kwarg_names=[
 'configuration'],
  task_id_kwargs=True)
def press_native(**kwargs):
    ...
    backend_client = BackEndClient(mercury_configuration.agent.remote.backend_url)
    log.info('Starting press')
    start = time.time()
    backend_client.update_task({'task_id':task_id,  'action':'Press: Launching'})
    return_data = entry(press_configuration, mercury_configuration.get('press', {}))
