...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/unit/raid_abstraction/test_hp.py
# Compiled at: 2018-03-02 13:43:50
# Size of source mod 2**32: 6045 bytes
import json, os, random, mock
from hpssa.hpssa import Adapter
from mercury_agent.hardware.drivers.hp_raid import RAIDAbstractionException, SmartArrayActions, SmartArrayDriver
...
class MercurySmartArrayDriverTest(MercuryAgentUnitTest):
    ...
    def test_probe(self):
        ...
        if not SmartArrayDriver.probe(pci_data):
            raise AssertionError
        elif not not SmartArrayDriver.probe([]):
            raise AssertionError
...
class MercurySmartArrayActionsTest(MercuryAgentUnitTest):
    ...
    def test_missing_array(self):
        ...
        modified['configuration']['spares'][0]['arrays'] = ['Z']
        dummy_actions = DummySmartArrayActions()
        (self.assertRaises)(RAIDAbstractionException, dummy_actions.transform_configuration, *(modified['configuration'],))
    ...
    def test_create(self):
        if not self.dummy_actions.create_logical_drive(0, '10', [10, 11, 12, 13]):
            raise AssertionError
        else:
    ...
    def test_add_spares(self):
        if not self.dummy_actions.add_spares(0, [10, 11]):
            raise AssertionError
        elif not self.dummy_actions.add_spares(0, [10], [0]):
            raise AssertionError
