...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mercury_agent/procedures/dell_firmware.py
...
def get_return_code_status(ret):
    ret_code = dict(SUCCESS_NO_REBOOT=0,
      FAILURE=1,
    ...
    return_code_status = dict(SUCCESS_NO_REBOOT=dict(error_state=False,
      message='Update was successful and no reboot required',
      reboot_flag=False),
      FAILURE=dict(error_state=True,
      message='Update was unsuccessful (Failure)',
      reboot_flag=False),
      SUCCESS_REBOOT_REQUIRED=dict(error_state=False,
      message='Update was successful and reboot is required',
      reboot_flag=True),
      DEP_SOFT_ERROR=dict(error_state=True,
      message='Soft dependency error',
      reboot_flag=False),
      DEP_HARD_ERROR=dict(error_state=True,
      message='Hard dependency errors',
      reboot_flag=False),
      QUAL_HARD_ERROR=dict(error_state=True,
      message='Qualification error',
      reboot_flag=False),
      REBOOTING_SYSTEM=dict(error_state=False,
      message='Rebooting system',
      reboot_flag=True),
      RPM_VERIFY_FAILED=dict(error_state=True,
      message='RPM verification has failed',
      reboot_flag=False),
      UPDATE_SUCCESSFUL_SOFT_DEPEDENCY_NOT_MET=dict(error_state=False,
      message='Update is successful. Soft Dependencies are not met.',
      reboot_flag=False),
      REBOOT_REQUIRED_SOFT_DEPENDENCY_NOT_MET=dict(error_state=False,
...
def parse_fw_report(existing_firmware):
    ...
    for component_name, installed_versions in existing_firmware.items():
        for ver in installed_versions:
            component_list.append({'name':component_name, 
             'version':ver, 
             'is_installed':any(ver in installer for installer in os.listdir(firmware_path))})
...
def install_updates():
    ...
    if not update_list:
        ...
        if errors:
            raise MercuryFirmwareException('Errors while flashing Dell Firmware: {}'.format(', '.join(errors)))
        return reboot_required


@capability('dell_apply_bios_settings', description='Apply bios settings found in given file',
  kwarg_names=[
 'url'],
  serial=True,
  dependency_callback=vendor_is_dell,
  timeout=600)
...
@capability('dell_update_firmware', description='Installs updates from provided packages',
  kwarg_names=[
 'url', 'dry_run'],
  serial=True,
  dependency_callback=vendor_is_dell,
