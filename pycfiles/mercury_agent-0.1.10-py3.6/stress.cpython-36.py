...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mercury_agent/procedures/stress.py
...
class Stress(object):
    ...
    def __init__(self, stress_path='/usr/bin/stress'):
        ...
        if not os.path.exists(stress_path):
            raise Exception('Stress binary not found at {}.'.format(self.stress_path))
    ...
    def cpu_stress(self, timeout):
        ...
        cmd = '{0} --cpu {1} -t {2}'.format(self.stress_path, cpu_count(), timeout)
    ...
    def memory_stress(self, timeout, amount=None):
        ...
        if not amount:
            ...
            log.info('Stressing memory for %s seconds at %s.' % (
             timeout, amount))
            cmd = '{0} --vm 1 --vm-bytes {1} --vm-hang 5 -t {2}'.format(self.stress_path, amount, timeout)
            log.info('Executing: {}'.format(cmd))
            result = cli.run(cmd)
            if result.returncode:
                self.killall()
...
@capability('memory_stress_test', description='Run stress on memory for specified amount of time',
  num_args=1,
  serial=False,
  timeout=3600)
def memory_stress_test(stress_seconds):
    ...
    :return:
    """
    log.info('Starting stress test on CPU and Memory for {} seconds'.format(stress_seconds))
    stress_tester = Stress()
    stress_tester.memory_stress(stress_seconds)


@capability('cpu_stress_test', description='Run stress on cpu for specified amount of time',
  num_args=1,
  serial=False,
  timeout=3600)
