...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mercury_agent/procedures/raid.py
...
@capability('create_logical_drive', description='Creates a logical drive on the specified controller',
  kwarg_names=[
 'adapter', 'level'],
  serial=True,
...
def abstract_create_logical_drive(adapter, level, drives=None, size=None, array=None):
    ...
    :return:
    """
    log.info('Creating array on adapter {}: level={} drives={} size={} array={}'.format(adapter, level, drives, size, array))
    raid_driver = get_subsystem_drivers('raid')[0]
    return raid_driver.handler.create_logical_drive(adapter, level, drives, size, array)


@capability('delete_logical_drive', description='Delete the specified logical drive',
  kwarg_names=[
...
@capability('clear_configuration', description='Clear all arrays from the adapter',
  kwarg_names=[
 'adapter'],
  serial=True,
  dependency_callback=has_abstraction_handler,
...
@capability('add_spares', description='Assign spare drives to the array',
  kwarg_names=[
 'adapter', 'drives'],
  serial=True,
  dependency_callback=has_abstraction_handler,
