...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mercury_agent/hardware/raid/interfaces/megaraid/megacli.py
# Compiled at: 2018-10-04 17:41:36
# Size of source mod 2**32: 8930 bytes
import os, time, logging
from subprocess import Popen, PIPE
log = logging.getLogger(__name__)

class LSIRaidException(Exception):
    __doc__ = ' Raised exclusively by Megacli classes '
...
class LSIRaid(object):
    ...
    def __init__(self, megacli_bin='/usr/sbin/megacli', adapter=0, use_sudo=False):
        ...
        self.pdisks, self.vdisks, self.enclosure, self.raw_adapter_info, self.clear = (None,
                                                                                       None,
                                                                                       None,
                                                                                       None,
                                                                                       None)
    ...
    def megacli(self, args, bufsize=1048567, timeout=300):
        ...
        raise LSIRaidTimeoutException('Command took too long to execute: {} {}'.format(self.megacli_bin, args))

    def get_adapter_raw(self, adapter='all'):
        out, err, ret = self.megacli('-AdpAllInfo -a{0}'.format(adapter or self.adapter))
        if ret:
            raise LSIRaidException('Could not get adapter info - {0}: {1} {2}'.format(ret, out, err))
        ...
        if not num:
            ...
            def raise_err(key):
                if key == 'Enclosure Device ID':
                    enc_out, enc_err, _ = self.megacli('-EncInfo -a{}'.format(adapter))
                    raise LSIRaidException('Could not read from controller, please make sure it is seated correctly: {} {}'.format(enc_out, enc_err))
                raise LSIRaidException("Error reading raid information for '{}' from megacli".format(key))
    ...
    def get_virtual_disk_members(self, vid, num):
        ...
        for line in out.splitlines()[pos:]:
            if 'Device Id' in line:
                pds.append(int(line.split(':')[1].strip()))
                count += 1
            elif count == num:
                break
    ...
    def get_virtual_disks(self, adapter=0):
        ...
        if not num:
            ...
            for x in range(0, num):
                ...
                phys_ids = self.get_virtual_disk_members(vd.get('id'), vd.get('drive_count'))
