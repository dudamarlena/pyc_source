...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mercury_agent/procedures/erase.py
# Compiled at: 2018-12-17 12:57:51
# Size of source mod 2**32: 7674 bytes
...
def get_raid_drivers():
    ...
    for driver in get_subsystem_drivers('raid'):
        for idx, _ in enumerate(driver.devices):
            if driver.handler.get_adapter_info(idx)['name'].strip() in HARDWARE_ERASE_SUPPORTED:
                hw_erase_drivers.append(driver)
            else:
                os_erase_drivers.append(driver)

    return (
     hw_erase_drivers, os_erase_drivers)


def driver_clear_all(drivers):
    for driver in drivers:
        for idx, _ in enumerate(driver.devices):
            log.info('Clearing configuration [%s adapter: %s]', driver.name, idx)
...
def driver_start_erase(drivers, method):
    for driver in drivers:
        for idx, _ in enumerate(driver.devices):
            log.info('Starting Hardware Erase [%s adapter: %s]', driver.name, idx)
...
def _erase_wrapper(target, method, results_queue):
    try:
        ...
        results_queue.put({'error':True, 
         'data':'An error occurred while erasing device: {} [{}]'.format(target, e)})
        ...
        for idx, _ in enumerate(driver.devices):
            ...
            if res:
                for drive in driver.handler.get_erase_status(idx):
                    log.info('HW drive erase: %s [%s%%]', drive['Drive-ID'], drive['Progress%'])
...
def _check_os_erase_completed(os_erase_tasks):
    incomplete_tasks = [task for task in os_erase_tasks if not task['completed']]
    for incomplete in incomplete_tasks:
        ...
        if not process.is_alive():
            ...
            log.info('Drive wipe operation has completed on %s', incomplete['target'])
...
def loop_until_completed(hw_erase_drivers, os_erase_tasks, interval):
    ...
    while True:
        if not hw_erase_complete:
            ...
            if hw_erase_complete:
                if os_erase_complete:
...
def _check_background_errors(os_erase_tasks):
    ...
    if has_errors:
        raise MercuryCritical('An error has occurred, the system has not been erased')


@capability('erase', 'Erase all data from local storage',
  timeout=0,
  serial=True)
def erase(method='fast'):
    ...
    ssd_results = []
    ssd_devices = [drive['udev']['DEVNAME'] for drive in storage_devices if drive['media_type'] == 'ssd']
    ...
    for drive in [drive['udev']['DEVNAME'] for drive in storage_devices if drive['media_type'] == 'disk']:
        background_erase_tasks.append(_background_erase(drive, os_method))

    loop_until_completed(hw_erase_drivers, background_erase_tasks, check_interval)
    log.info('Clearing all adapter RAID configurations')
    driver_clear_all(hw_erase_drivers + os_erase_drivers)
    _check_background_errors(background_erase_tasks)
    log.info('System erase has completed')
    return {'report': {'ssd':ssd_results, 
