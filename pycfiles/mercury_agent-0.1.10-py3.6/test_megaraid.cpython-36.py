...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/unit/raid_abstraction/test_megaraid.py
# Compiled at: 2018-03-02 13:43:50
# Size of source mod 2**32: 11518 bytes
import json, os, mock
from mercury.common.helpers.cli import CLIResult
from mercury_agent.hardware.drivers.megaraid import MegaRAIDActions, MegaRaidSASDriver
...
def get_storcli_dall_show(c):
    ...
    """
    del c
    with open(os.path.join(os.path.dirname(__file__), '../resources/storcli_dall_show.json')) as (fp):
        data = json.load(fp)
        controller_list = []
...
def get_controllers():
    ...
    :return:
    """
    with open(os.path.join(os.path.dirname(__file__), '../resources/storcli.json')) as (fp):
        return [c['Response Data'] for c in json.load(fp)['Controllers']]


class DummyMegaRAIDActions(MegaRAIDActions):
    __doc__ = ' A dummy megaraid actions driver for easier patching '
...
class TestMegaRAIDActions(MercuryAgentUnitTest):
    __doc__ = ' MegaRAIDActions Test Case '
    ...
    def test_transform_configuration(self):
        ...
        self.assertEqual(configuration['arrays'][1]['physical_drives'][1]['extra']['address'], '32:2')
        self.assertEqual(MegaRAIDActions.get_array_index_by_dg(configuration['arrays'], 1), 1)
        self.assertEqual(MegaRAIDActions.get_array_index_by_dg(configuration['arrays'], 99), -1)
    ...
    def test_with_global_hotspare(self):
        ...
        new_dummy_actions.storcli.get_disk_group.return_value = dg_info
        configuration = new_dummy_actions.transform_configuration(0)
        self.assertEqual(configuration['spares'][0]['extra']['spare_type'], 'global')
    ...
    def test_transform_adapter_info(self):
        ...
        self.assertEqual(MegaRAIDActions.get_controller_id(adapter_info), 0)
        (self.assertRaises)(RAIDAbstractionException, self.dummy_actions.transform_adapter_info, *(100, ))
    ...
    def test_get_vendor_info_static(self):
        ...
        adapter = {'Basics':{},  'Version':{},  'Bus':{},  'Status':{},  'Supported Adapter Operations':{},  'Supported PD Operations':{},  'Supported VD Operations':{}}
    ...
    def test_delete_logical_drive(self):
        ...
        (self.dummy_actions.storcli.delete.assert_called_with)(controller=0, 
         virtual_drive=0)
        (self.assertRaises)(RAIDAbstractionException, self.dummy_actions.delete_logical_drive, *(0,
                                                                                                 0,
                                                                                                 99))
    ...
    def test_add_spares(self):
        ...
        (new_dummy_actions.storcli.add_hotspare.assert_called_with)(controller=0, 
         enclosure=32, 
         slot=3, 
         disk_groups=[])
        ...
        (new_dummy_actions.storcli.add_hotspare.assert_called_with)(controller=0, 
         ...
         disk_groups=[
 0])
        (self.assertRaises)(RAIDAbstractionException, new_dummy_actions.add_spares, *(
         0, [3], [99]))
...
class MercuryMegaRaidDSDriverTest(MercuryAgentUnitTest):
    ...
    def setUp(self):
        ...
        self.pci_data = [
         {'class_id':'0104',  'class_name':'RAID bus controller', 
          'device_id':'005d', 
