...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mercury_agent/procedures/hp_firmware.py
# Compiled at: 2018-03-20 01:49:47
# Size of source mod 2**32: 9742 bytes
import glob, logging, os, lxml, requests
...
def parse_xml_for_components(xml_tree, xml_tag):
    ...
    for i in xml_tree.findall('.//{0}/component'.format(xml_tag)):
        ...
        component_list.append(dict(name=component_name, installed=installed,
          available=available))
...
def firmware_updates_available(component_list):
    ...
    for comp in component_list:
        ...
        if comp['installed'].replace(' ', '') not in comp['available'].replace(' ', ''):
...
def get_return_code_status(ret):
    ret_code = dict(HP_SUCCESS_NO_REBOOT=0,
      HP_SUCCESS_REBOOT=1,
...
def parse_raw_log():
    ...
    with open(raw_log_paths[0]) as (fp):
        ...
        for line in fp.readlines():
            ...
            raw_log_list.append('{} : {}'.format(split_line[0], split_line[(-1)]))

    result['raw_log'] = raw_log_list
    return result


@capability('hp_apply_bios_settings', description='Apply bios settings found in given file',
  kwarg_names=[
 'url'],
  serial=True,
  dependency_callback=vendor_is_hp,
  timeout=60)
...
@capability('hp_update_firmware', description='Installs updates from provided packages',
  kwarg_names=[
 'url'],
  serial=True,
  dependency_callback=vendor_is_hp,
...
def hp_update_firmware(url=None, dry_run=False):
    ...
    try:
        ...
        log.error('There was an error in retrieving the firmware package: {}'.format(err))
        return
