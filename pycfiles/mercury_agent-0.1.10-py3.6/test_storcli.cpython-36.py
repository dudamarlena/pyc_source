...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/unit/raid_abstraction/test_storcli.py
...
class StorcliTest(MercuryAgentUnitTest):
    ...
    def test_add(self, mock_cli):
        ...
        mock_cli.find_in_path.return_value = '/sbin/storcli64'
        s = storcli.Storcli()
        if not s.add(0, 'r0', '32:0-1') == '':
            raise AssertionError
        elif not s.add(0, 'r10', '32:0-3', pdperarray=2) == '':
            raise AssertionError
    ...
    def test_bad_command_status(self):
        (self.assertRaises)(storcli.StorcliException, storcli.Storcli.check_command_status, *(
         {'Command Status': {'Status':'Nope', 
    ...
    def test_get_disk_group(self, mock_cli):
        ...
        s = storcli.Storcli()
        s.run_json = mock.Mock()
        s.run_json.return_value = {'Controllers': [
