...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mercury_agent/hardware/raid/interfaces/megaraid/storcli.py
...
class StorcliException(Exception):
    __doc__ = ' Raised exclusively by Storcli classes '


class Storcli(object):
    __doc__ = ' Thin storcli/percli interface for querying PERC and MegaRAID adapters and creating simple arrays '
    span_arrays = [
     10, 50, 60]
    ...
    def run(self, cmd, ignore_error=False):
        ...
        if not ignore_error:
            if result.returncode:
                raise StorcliException('Command returned: {}, Error: {}'.format(result.returncode, result))
        return result
    ...
    def get_disk_group(self, controller='all', disk_group='all'):
        ...
        :return:
        """
        output = self.run_json('/c{}/d{} show all'.format(controller, disk_group))
        controller_list = []
        for controller in output['Controllers']:
            self.check_command_status(controller)
            controller_list.append(controller['Response Data']['Response Data'])
    ...
    def add(self, controller, array_type, drives, size=None, pdperarray=None, pdcache=None, dimmerswitch=None, io_mode='direct', write_policy='wb', read_policy='ra', cachevd=False, stripe_size=None, spares=None, cached_bad_bbu=False, after_vd=None):
        """ Add a virtual drive
...
value is automatically chosen.(0 to 16)
 ...
 write to maximize power savings.
        ...
        """
        command = ('/c{controller} add vd type={array_type}{size}drives={drives}  {write_policy} {read_policy} {io_mode} {stripe_size}{spares}{cache_bad_bbu}{after_vd}{pdperarray}'.format)(controller=controller, 
    ...
    def add_hotspare(self, controller, enclosure, slot, disk_groups=None):
        ...
        if disk_groups:
            ...
            dgs = None
        self.run('/c{}/e{}/s{} add hotsparedrive{}'.format(controller, enclosure, slot, dgs and ' DGs={}'.format(dgs) or ''))

    def start_erase(self, controller, enclosure, slot, pattern='simple'):
        log.info('Scheduling erase [%s] operation on /c%s/e%s/s%s', pattern, controller, enclosure, slot)
        data = self.run('/c{}/e{}/s{} start erase {} J'.format(controller, enclosure, slot, pattern))['Controllers'][0]
        self.check_command_status(data)

    def stop_erase(self, controller, enclosure, slot):
        log.info('Cancelling erase operation on /c%s/e%s/s%s', controller, enclosure, slot)
        data = self.run('/c{}/e{}/s{} stop erase J'.format(controller, enclosure, slot))['Controllers'][0]
        self.check_command_status(data)
        return data['Response Data']

    def show_erase(self, controller, enclosure, slot):
        data = self.run('/c{}/e{}/s{} show erase J'.format(controller, enclosure, slot))['Controllers'][0]
        self.check_command_status(data)
        return data['Response Data']