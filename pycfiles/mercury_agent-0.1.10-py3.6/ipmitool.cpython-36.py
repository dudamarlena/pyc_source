...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mercury_agent/hardware/obm/ipmitool.py
...
def get_bmc_handler(vendor, **kwargs):
    _SELECTOR = {'Dell Inc.':IPMIToolDrac, 
     'Quanta':DCMITool, 
     'HP':IPMIToolHP}
...
class IPMITool(object):
        ...
        if not self.net_info.get('ip_address') == ip_address:
            ...
            if not self.net_info.get('netmask') == netmask:
                ...
                if 'gateway' in self.net_info:
                    self.net_info['gateway'] == gateway or log.debug('GATEWAY: %s != %s' % (gateway, self.net_info['gateway']))
                    return False
                else:
                    self.net_info.get('source') == source or log.debug('SOURCE: %s != %s' % (source, self.net_info.get('source')))
                    return False
            return True

    def set_static(self, channel=1):
        self.run(self._IPMITool__lan_set(channel, 'ipsrc', 'static'))

    def set_dhcp(self, channel=1):
        self.run(self._IPMITool__lan_set(channel, 'ipsrc', 'dhcp'))

    def configure_static_network(self, ip_address, netmask, gateway, channel=1):
        self.set_static(channel)
        self.run(self._IPMITool__lan_set(channel, 'ipaddr', ip_address))
        self.run(self._IPMITool__lan_set(channel, 'netmask', netmask))
        self.run(self._IPMITool__lan_set(channel, 'defgw', gateway))
        self.parse_network_info()
    ...
    def create_superuser(self, username, password, channels=(1, 2), user_id=2):
        ...
        for channel in channels:
            self.run('channel setaccess %d %d link=on ipmi=on callin=on privilege=4' % (channel,
             user_id))
    ...
    def parse_output_type1(data):
        ...
        for line in data.splitlines():
            ...
            if len(sline) == 1:
                ...
                parsed[key] = sline[1].strip() or list()
            else:
    ...
    def bmc_info(self):
        ...
        if out.returncode:
            log.error('Problem getting bmc info : std {} - err {} - return {}'.format(out, out.stderr, out.returncode))
            return
        else:
...
class IPMIToolDrac(IPMITool):
    ...
    def disable_default_password_prompt(self):
        ...
        if self.drac_version == 'iDRAC7':
            self.run(self._IPMIToolDrac__raw_command(command))
...
class IPMIToolHP(IPMITool):
    ...
    def __init__(self, ipmitool_path=DEFAULT_IPMITOOL_PATH, hponcfg_path='/sbin/hponcfg', list_users_path='hpxml/list_users.xml', users_path='hpxml/users.xml', add_user_path='hpxml/add_user.xml', del_user_path='hpxml/del_user.xml'):
    ...
    def configure_static_network(self, ip_address, netmask, gateway, channel=2):
        ...
        :return:
        """
        return super(IPMIToolHP, self).configure_static_network(ip_address, netmask, gateway, channel)
