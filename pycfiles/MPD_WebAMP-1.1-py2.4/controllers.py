...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/mpd_webamp/controllers.py
...
def formatted_song(song):
    ...
    try:
        if len(mySong) > 0:
            return mySong
        elif song.file.find('/') > 0:
            S = song.file.split('/')
            s = S.pop()
            return "<span class='title'>" + allsafe(s) + '</span>'
        else:
            return "<span class='title'>" + allsafe(song.file) + '</span>'
    except:
        return 'error'
...
def createBrowselist(lsinfo):
    ...
    for item in lsinfo:
        ...
        browse += tmp + "<td id='"
        browse += allsafe(myId)
        browse += "' class='" + myClass
        browse += "'>" + myTxt + '</td></tr><tr height="2px" width="100%"><td></td></tr>'
        i = i + 1

    browse = browse + '</tbody></table>'
    return browse
    ...
    def playlist(self, **kw):
        ...
        if list_start <= PL_count:
            if list_start + list_size >= PL_count:
                ...
                more_PL = True
        if chunk_limit > 0:
            if list_start < chunk_limit - max_size:
    ...
    def playlist_changes(self, version, len_old, changes_from='0', **kw):
        ...
        if changes_from == 0:
            ...
            try:
                ...
                if len_old < 1:
                    raise redirect('playlist')
                e = len_new
                if chunk_end > 0 and chunk_end < len_new:
                    e = chunk_end
                if e > len_old:
                    ua = cherrypy.request.headers.get('User-Agent', None)
                    if ua.find('Konqueror') > 0:
        ...
        if cc == 0:
            ...
            if changes_from == 0:
                ...
                list_size = int(cherrypy.session.get('updatesize', 100))
            if changes_from <= cc:
                ...
                for index in range(changes_from, changes_to):
                    ...
                    p = item.pos
                    li = str(int(p) + 1)
                    row = "<td id='p" + p + "_left' class='icon_PL'>" + li + ".</td><td id='p" + p + "' class='file'>" + formatted_song(item) + '</td>'
                    change[i] = 'p' + p + '_tablerow'
                    changeHTML[i] = row
                    i = i + 1

        if not more_changes:
            cherrypy.session['plchanges'] = ''
