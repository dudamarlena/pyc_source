...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/alpha1p/dance/process.py
# Compiled at: 2020-04-14 13:47:17
# Size of source mod 2**32: 13791 bytes
__doc__ = '\nCreated on Sun March 15 12:35:24 2020\n\n@author: QQ\n'
import numpy as np, pandas as pd, numba, os, sys, pkg_resources
from .conversion import interpolation
names = locals()
__all__ = [
 'norm', 'denorm', 'norm_trans', 'norm_01', 'denorm_01',
...
def segment_csv(csv_path=None, csv_folder=None, np_folder=None, keep_last_actions=True):
    ...
    if csv_path is None:
        ...
        if csv_folder is None:
            ...
            os.path.exists(np_folder) or os.makedirs(np_folder)
        if keep_last_actions:
            end = 0
        else:
            end = 4
...
def kaiser_resample(x, sample_ratio, axis=-1, filter_name='kaiser_best'):
    ...
    if shape[axis] < 1:
        raise ValueError('Input signal length={} is too small to resample'.format(x.shape[axis]))
