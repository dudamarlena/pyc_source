...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/alpha1p/dance/conversion.py
# Compiled at: 2020-04-14 08:04:08
# Size of source mod 2**32: 12422 bytes
__doc__ = '\nCreated on Sun March 15 12:35:24 2020\n\n@author: QQ\n'
import struct, math, numpy as np, pandas as pd
__all__ = [
 'pd2np', 'pd2aesx', 'np2aesx', 'save_bin', 'compression', 'interpolation', 'np2hts']
...
def pd2np(pd_dance):
    ...
    return (
     all_actions, exec_times, total_times)
...
def np2aesx(all_actions, exec_times, total_times, max_group_time=100000, prev_click_num=0):
    ...
    while p_action < len(all_actions) - 10:
        ...
        group_size = 80 + len(group_actions) + 10
        ...
        motion_end = struct.pack('i6s', 6, 'motion')
    ...
    total_size = len(groups) + 69
    file_head = struct.pack('i', total_size)
    arr = ['ubx-alpha', 2, 3, 0, 8, 10]
...
def np2hts(action, run_time, total_time):
    ...
    for i in range(action_amount):
        ...
        if i + 1 == 1:
            ...
            start.append(2)
        start.append(action_amount % 256)
        start.append(action_amount / 256)
        start.append((i + 1) % 256)
        start.append((i + 1) / 256)
        start.extend(action[i])
        start.append(r[i] % 256)
        start.append(t[i] / 256)
        start.append(t[i] % 256)
        check_sum = 0
        for x in start[2:]:
            check_sum += int(x)

        start.append(check_sum % 256)
        start.append(237)
        for j in start:
            if j > 255:
                print(j)
