...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bitcoin/tx/test.py
...
from .. import address
from .. import script
from bitcoin.transaction import signature_form, mk_pubkey_script, SIGHASH_ALL, bin_txhash, der_encode_sig, ecdsa_tx_sign, sign
from bitcoin.main import ecdsa_raw_sign
from ..crypto import ecdsa

def main():
    global private_key
    global signable_bytes
    ...
    tx = SignableTx()
    tx.add_input(TxInput(bytes().fromhex('258fb211724412d6ec6a531973c58233143e6ab355623658adc31' + '64a5c70bd5b'), 0, script.sig.P2PKH()))
    tx.add_output(TxOutput(address.P2PKH.decode(address_to).script, btc=10))
    tx.inputs[0].script.sign(private_key, address.P2PKH.decode(address_to).script)
    print(' - Creating signable transaction: ', end='')
    signable = tx.signable_tx(0, address.P2PKH.decode(address_to).script)
    vbuterin_signable = signature_form(tx.serialize().hex(), 0, mk_pubkey_script(address_to), SIGHASH_ALL)
    our_signable = signable.serialize().hex()
    assert vbuterin_signable == our_signable, 'Our signable transaction does not match Vitalik Buterin signable ' + "transaction: \nVitalik's:\n%s\nOurs:\n%s" % (
    ...
    assert our_signable_bytes == vbuterin_signable_bytes, 'Our signable transaction hash does not match Vitalik Buterin ' + "signable transaction hash: \nVitalik's:\n%s\nOurs:\n%s" % (
    ...
    assert our_signature == vbuterin_signature, 'Our signature does not match Vitalik Buterin signature: ' + "\nVitalik's:\n%s\nOurs:\n%s" % (vbuterin_signature, our_signature)
    print('pass')
    ...
    assert our_der_signature == vbuterin_der_signature, 'Our DER signature does not match Vitalik Buterin DER signature: ' + "\nVitalik's:\n%s\nOurs:\n%s" % (
    ...
    our_signature_full = tx.sign(private_key, 0, address.P2PKH.decode(address_to).script).hex()
    assert our_signature_full == vbuterin_signature_full, 'Our tx signature does not match Vitalik Buterin tx signature: ' + "\nVitalik's:\n%s\nOurs:\n%s" % (
     vbuterin_signature_full, our_signature_full)
    print('pass')
    print(' - Test transaction signed: ', end='')
    vbuterin_tx_signed = sign(tx.serialize().hex(), 0, private_key.hex())
    our_tx_signed = tx.serialize().hex()
    assert our_tx_signed == vbuterin_tx_signed, 'Our tx signed does not match Vitalik Buterin tx signed: ' + "\nVitalik's:\n%s\nOurs:\n%s" % (
