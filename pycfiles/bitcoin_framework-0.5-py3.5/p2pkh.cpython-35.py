...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bitcoin/script/sig/p2pkh.py
# Compiled at: 2017-07-08 11:42:23
# Size of source mod 2**32: 3919 bytes
"""
...
class P2PKH(ScriptSig):
    ...
    def _build(self):
        ...
        """
        assert self._signature is not None and self._public_key is not None, "The P2PKH can't be serialized as it has not been signed"
        self._data = [
    ...
    def sign(self, key, script=None, hashtype=DEFAULT_HASHTYPE):
        ...
        """
        assert self.input is not None and self.input.tx is not None, 'To sign the P2PKH scriptSig, there must be an input related ' + 'to the scriptSig that is related to a transaction'
        assert isinstance(self.input.tx, SignableTx), 'To sign the P2PKH ' + 'scriptSig, the related transaction must be a SignableTx'
        self._public_key = private_to_public(key)
        if script is None:
            script = address.P2PKH(public_key=self._public_key).script
        self._signature = self.input.tx.sign(key, self.input, script, hashtype)
    ...
    def __str__(self, space):
        ...
        """
        return '<%s:%s(%s)>' % (
         '|Not signed yet|' if not self.is_signed else '|Signature & Public key|',
