...
# Embedded file name: build/bdist.linux-x86_64/egg/process_tracing/recording.py
# Compiled at: 2017-01-23 10:53:31
# Size of source mod 2**32: 15248 bytes
...
class TracingRecord(object):
    ...
    def get_result_stats(self):
        ...
        """
        return (
         self._end_time, self._exit_code, self._signal)
...
class LogRecord(object):
    ...
    def get_log_message_items(self):
        ...
        """
        return [
         'log', self.timestamp, self.message]
...
class RuntimeActionRecord(LogRecord):
    ...
    def get_log_message_items(self):
        return [
         'run', self.timestamp, RuntimeActionRecord._get_type_name(self.type), self.message, self.exit_code,
    ...
    def _get_type_name(type):
        ...
        if type == RuntimeActionRecord.TYPE_STARTED:
            ...
            if type == RuntimeActionRecord.TYPE_EXITED:
                return 'exit'
            if type == RuntimeActionRecord.TYPE_SIGNAL_RECEIVED:
                return 'signal'
            if type == RuntimeActionRecord.TYPE_EXEC:
                return 'execve'
            if type == RuntimeActionRecord.TYPE_SPAWN_CHILD:
                return 'spawn'
            return 'unknown'
    ...
    def __repr__(self):
        ...
        if self.type == RuntimeActionRecord.TYPE_STARTED:
            ...
            if self.type == RuntimeActionRecord.TYPE_EXITED:
                message = 'Process terminated: {}'.format(self.message)
            else:
                if self.type == RuntimeActionRecord.TYPE_SIGNAL_RECEIVED:
                    message = 'Process received signal {}'.format(self.message)
                else:
                    if self.type == RuntimeActionRecord.TYPE_EXEC:
...
class FileAccessRecord(LogRecord):
    ...
    def get_log_message_items(self):
        return [
         'file', self.timestamp, self.filename, self.name, self.result, self.is_dir, self.exists]

    def __repr__(self):
        return '[{}] File access to {} by syscall {}, result: {} (exits: {}, is_dir: {})'.format(self.timestamp, self.filename, self.name, self.result, self.exists, self.is_dir)