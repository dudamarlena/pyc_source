...
# Embedded file name: /home/akhavr/src/42-kavyarnya/.env/lib/python2.7/site-packages/pypipedrive/api.py
# Compiled at: 2014-10-30 08:29:42
...
class PipeDrive(object):
    ...
    def get_response(self, base_url, **kwargs):
        ...
        params = dict((k, v) for k, v in kwargs.iteritems() if not k.startswith('_'))
        if 'method' in params:
        ...
        if method not in ('GET', 'POST', 'PUT', 'DELETE'):
            raise BasePipedError('The method is not specified correctly')
        logger.debug(('{method} {url}, params: {params}').format(method=method, url='%s%s' % (self.api_url, base), params=params))
