...
# Embedded file name: build/bdist.macosx-10.8-x86_64/egg/pwmd/multi.py
# Compiled at: 2012-12-18 00:53:02
...
class MultiDatabase(Database):
    ...
    def connect(self):
        with self._conn_lock:
            ...
            self.__local.master = self._connect(self.master, **self.connect_kwargs)
            self.__local.slaves = [ self._connect(slave, **self.connect_kwargs) for slave in self.slaves
                                  ]
    ...
    def execute_sql(self, sql, params=None, require_commit=True):
        if hasattr(self, '_usging'):
            ...
            using = 'slave' if sql.startswith('SELECT') and self.__local.slaves else 'master'
        if using == 'slave':
...
class MultiMySQLDatabase(MultiDatabase, MySQLDatabase):
    ...
    def parse_url(self, url):
        ...
        mysql_config = dict(db=params['db'], user=params['user'], passwd=params['password'], host=params['host'], port=params['port'] or 3306)
