...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/xdjango/forms/validators.py
...
FIELD_NAME_MAPPING = {'g_recaptcha_response': 'g-recaptcha-response'}
...
class ReCaptchaFormValidator(Form):
    ...
    def clean(self):
        ...
        if not validate_captcha(g_recaptcha_response):
            msg = 'Falló la verificación anti robots.'
...
class EmailOrPhoneNumberFormValidator(Form):
    ...
    def clean(self):
        ...
        if not self.errors and not email and not phone_number and not email_or_phone_number and not user_id:
            msg = 'Debes facilitar, al menos, tu correo electrónico o tu número de teléfono.'
