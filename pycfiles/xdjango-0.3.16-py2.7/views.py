...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/xdjango/contrib/auth/views.py
...
from xdjango.contrib.auth.validators import LoginFormValidator, UserQueryValidator, PasswordResetFormValidator, EmailFormValidator
...
def send_reset_password_email(request, user, subject):
    ...
    data = {'url_reset_password': reset_url}
...
class UserEmailViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            message = _('%(model_name)s with this %(field_label)s already exists.') % {'model_name': self.serializer_class.Meta.model.__name__, 'field_label': _('email address')}
    ...
    def post(self, request, format=None):
        ...
        if form.is_valid():
            ...
            try:
                ...
                send_mail(subject='log - reset password', message='Someone tried to reset password from a non-existent account', from_email='some@123.com', recipient_list=[
                 recipient])
