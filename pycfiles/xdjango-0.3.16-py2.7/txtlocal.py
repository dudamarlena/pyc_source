...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/xdjango/core/sms/backends/txtlocal.py
...
class TxtLocalAPI(object):
    ...
    def __init__(self, username=None, hash=None, apiKey=None):
        ...
        if not self.hash and not self.apiKey:
            raise ValueError('At least one of API_SMS_HASH/API_SMS_KEY is required.')
    ...
    def sendSms(self, number, message, sender=None, sched=None, test=False, receiptURL=None, custom=None, optouts=False, simpleReplyService=False):
        if not sender:
            sender = self.sender
