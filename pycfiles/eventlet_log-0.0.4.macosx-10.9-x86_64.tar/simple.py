...
# [GCC 8.4.0]
# Embedded file name: /Users/typhon/.brew/lib/python2.7/site-packages/eventlet_log/simple.py
# Compiled at: 2014-10-24 05:43:05
import eventlet
...
def create_logger(name, format='%(message)s', level=logging.DEBUG):
    ...
    *level*.
    ...
    """
    return create_generic_logger(name, logging.StreamHandler(), format, level)
...
def create_file_logger(name, filename, format='%(message)s', level=logging.DEBUG):
    handler = logging.FileHandler(filename)
    if __KEEP_FILEHANDLERS:
        __filehandlers[(name, filename)] = handler
    return create_generic_logger(name, handler, format, level)


def stop_file_logger(name, filename):
    if (
     name, filename) in __filehandlers:
        logger = logging.getLogger(name)
