...
# Compiled at: 2019-11-29 08:48:00
# Size of source mod 2**32: 10324 bytes
"""
    ...
    if locate_name:
        ...
        if locate_domain:
            ...
            for dns_index in range(len(dns_list), 0, -1):
                dns_name = (b'.').join([label.encode('idna') for label in dns_list[-dns_index:]])
                dns_name_u = '.'.join([label.decode('idna') for label in dns_name.split(b'.')])
