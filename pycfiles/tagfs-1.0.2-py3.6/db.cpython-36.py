...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tagfs/db.py
# Compiled at: 2018-02-19 06:05:48
# Size of source mod 2**32: 12745 bytes
import functools, sqlite3, threading, typing, click
...
class _SelectionsCursor(_BaseCursor):
    ...
    def select(self, name: str):
        ...
        self.execute('SELECT files.name FROM tag_files INNER JOIN files ON files.id = tag_files.file_id INNER JOIN tags ON tags.id = tag_files.tag_id WHERE %s' % self.resolve(name))
        return self.fetch_first()
    ...
    def matches(self, name: str, file: name_id):
        ...
        if isinstance(file, int):
            self.execute('SELECT files.name FROM tag_files INNER JOIN files ON files.id = tag_files.file_id INNER JOIN tags ON tags.id = tag_files.tag_id WHERE files.id = ? AND %s' % self.resolve(name), (file,))
        else:
            self.execute('SELECT files.name FROM tag_files INNER JOIN files ON files.id = tag_files.file_id INNER JOIN tags ON tags.id = tag_files.tag_id WHERE files.name = ? AND %s' % self.resolve(name), (file,))
        c = self.fetchone()
...
class _FilesCursor(_BaseCursor):
    ...
    def get_by_tag(self, tag: name_id):
        ...
        if tag == '__ALL__':
            ...
            self.execute('SELECT files.name FROM files INNER JOIN tag_files ON tag_files.file_id = files.id WHERE tag_files.tag_id = ?', (
             t,))
            return self.fetch_first()
    ...
    def get_tags(self, name: name_id):
        ...
        self.execute('SELECT tags.name FROM tag_files INNER JOIN tags ON tags.id = tag_files.tag_id WHERE tag_files.file_id = ?', (
         name,))
        return self.fetch_first()
