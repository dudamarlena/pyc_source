...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-ppc/egg/megrok/form/constraints.py
...
def isUSPhoneNumber(value, **kwargs):
    expr = re.compile('^\\d{10}$')
    if not kwargs.get('ignore', None):
        kwargs['ignore'] = re.compile('[\\(\\)\\-\\s]')
...
def isZipCode(value, **kwargs):
    expr = re.compile('^(\\d{5}|\\d{9})$')
    errmsg = 'is not a valid zip code.'
    return validate_expression(value, expr, errmsg=errmsg, **kwargs)


def isURL(value, **kwargs):
    protocols = ('http', 'ftp', 'irc', 'news', 'imap', 'gopher', 'jabber', 'webdav',
                 'smb', 'fish', 'ldap', 'pop3', 'smtp', 'sftp', 'ssh')
...
def isEmail(value, **kwargs):
    expr = re.compile("^(\\w&.%#$&'\\*+-/=?^_`{}|~]+!)*[\\w&.%#$&'\\*+-/=?^_`{}|~]+@(([0-9a-z]([0-9a-z-]*[0-9a-z])?\\.)+[a-z]{2,6}|([0-9]{1,3}\\.){3}[0-9]{1,3})$", re.IGNORECASE)
