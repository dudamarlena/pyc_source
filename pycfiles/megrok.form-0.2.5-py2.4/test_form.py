...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-ppc/egg/megrok/form/tests/test_form.py
...
class IPerson(interface.Interface):
    ...
    picture = Image(title='Picture')
    description = HTML(title='Description')
    birthday = schema.Date(title='Birthday')
    resume = File(title='Resume')
    video = BlobFile(title='Favorite Video')
    wallpaper = BlobImage(title='Favorite Wallpaper')
...
class MeGrokFormTests(unittest.TestCase):
    ...
    def test_email_default_constraint(self):
        ...
        email = Email(title='Email')
        self.assertRaises(ConstraintNotSatisfied, email.validate, 'thisisnotanemailaddress')
        email.validate('thisisanemailaddress@email.com')
