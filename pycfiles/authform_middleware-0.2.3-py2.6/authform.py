...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\authform_middleware\authform.py
# Compiled at: 2009-11-11 06:23:00
from paste.httpexceptions import HTTPExceptionHandler
import paste.request
from paste.request import construct_url
from paste.util.import_string import eval_import
import Cookie, md5, socket, struct
...
class AuthFormHandler(object):
    ...
    def __call__(self, environ, start_response):
        ...
        if environ.get('PATH_INFO') == self.process_url:
            ...
            if environ.get('PATH_INFO') == self.logout_url:
        ...
        if authenticated:
            ...
            if 'HTTP_REFERER' in environ:
                if self.loginurl_func is not None:
                    ...
                    loginurl = self.loginurl
                if not environ['HTTP_REFERER'].endswith(self.process_url) and not environ['HTTP_REFERER'].endswith(loginurl):
...
def load_config(options, app_conf, prefix):
    ...
    for (key, value) in options.items():
        if merged.has_key(key):
            warnings.warn('Key %s with value %r set in the config file is being ' + 'replaced with value %r set in the application' % (
             key,
