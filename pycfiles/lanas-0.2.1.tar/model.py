...
# [GCC 8.4.0]
# Embedded file name: kanzen/kanzen/model.py
...
def filter_data_type(data_types):
    ...
    maximum = max(values)
    data_type = [ occurrences[key][1] for key in occurrences if occurrences[key][0] == maximum
                ]
    return data_type[0]
            ...
            if name in attributes:
                ...
                for type_data in assign.data:
                    if type_data in old_assign.data:
                        old_type = old_assign.data[old_assign.data.index(type_data)]
            ...
            if func_name in functions:
                ...
                for type_data in function.return_type:
                    if type_data in old_func.return_type:
                        old_type = old_func.return_type[old_func.return_type.index(type_data)]
                        type_data.data_type = old_type.data_type
...
class Module(Structure):
    ...
    def get_type(self, main_attr, child_attrs='', scope=None):
        ...
        if not scope:
            value = self.imports.get(main_attr, self.attributes.get(main_attr, self.functions.get(main_attr, self.classes.get(main_attr, None))))
            if value is not None and value.__class__ is not Clazz:
                ...
                if child_attrs or isinstance(data_type, basestring) and data_type.endswith(main_attr):
            ...
            scope_name = scope[0]
            structure = self.classes.get(scope_name, self.functions.get(scope_name, None))
            if structure is not None:
                attrs = [
                 main_attr] + canonical_attrs.split('.')
                if len(attrs) > 1 and attrs[1] == '':
                    del attrs[1]
                result = self.recursive_search_type(structure, attrs, scope[1:])
                if not result['found']:
                    value = self.imports.get(main_attr, self.attributes.get(main_attr, self.functions.get(main_attr, None)))
        ...
        if result['type'].__class__ is Clazz:
            if canonical_attrs:
                ...
                result = {'found': False, 'type': result['type'].get_completion_items(), 
                   'object': result['type']}
    ...
    def _search_type(self, structure, attrs):
        ...
        if not attrs:
            ...
            attr = attrs[0]
            value = structure.attributes.get(attr, structure.functions.get(attr, None))
            if value is None:
                return result
            data_type = value.get_data_type()
            ...
            if parent.__class__ is Clazz:
                ...
                for attr in data.get('attributes', []):
                    if attr[:2] == '__' and attr[-2:] == '__':
                        continue
                    assign = Assign(attr)
                    assign.add_data(0, parent_name + attr, '', parent_name + attr)
...
class Function(Structure):
    ...
    def get_data_type(self):
        possible = [ d.data_type for d in self.return_type if d.data_type is not late_resolution and d.data_type is not None
                   ]
        if possible:
...
class Assign(object):
    ...
    def get_data_type(self):
        possible = [ d.data_type for d in self.data if d.data_type is not late_resolution
                   ]
        if possible:
            return filter_data_type(possible)
        else:
