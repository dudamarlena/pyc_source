...
# [GCC 8.4.0]
# Embedded file name: kanzen/kanzen/completion_daemon.py
        ...
        for project in PROJECTS:
            ...
            for root, dirs, files in os.walk(project, followlinks=True):
                if '__init__.py' in files:
                    package = root[len(project) + 1:].replace(os.path.sep, '.')
        ...
        for data in types:
            ...
            if len(sym) != 0:
                ...
                if isinstance(result['type'], basestring) and len(result) < 3:
                    if child_attr and structure.__class__ is not model.Function:
