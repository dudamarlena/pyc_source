...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\soccer\data_connectors\data_connector.py
# Compiled at: 2018-02-27 07:09:35
...
from ..util import DEFAULT_POINT_RULE, DEFAULT_POINT_RULE_WIN_POINTS, DEFAULT_POINT_RULE_DRAW_POINTS, DEFAULT_POINT_RULE_DISPLAY_NEGATIVE_POINTS, DEFAULT_TIE_BREAK_RULES, EMPTY_TEAM_STANDINGS, POINT_RULES, SORT_OPTIONS, TIE_BREAK_RULES, get_current_season

class DataConnector(object):
    __doc__ = '\n    Basic connector that offers the soccer data interface\n    '
    TIME_FRAME_TYPES = ('date', 'season', 'matchday')
    TABLE_STATUS = {'done':'done', 
     'pending':'pending'}
                ...
                if team_ids is None or home and homeId in team_ids and (not head2headOnly or awayId in team_ids):
                    if homeId not in teamStandings:
                        ...
                        if fixture['result']['winsHomeTeam'] == 1:
                            ...
                            if fixture['result']['lossesHomeTeam'] == 1:
            ...
            if team_ids is None or away and awayId in team_ids and (not head2headOnly or homeId in team_ids):
                if awayId not in teamStandings:
                    ...
                    if fixture['result']['winsAwayTeam'] == 1:
                        ...
                        if fixture['result']['lossesAwayTeam'] == 1:
        ...
        for fixture in fixtures:
            ...
            for player, team_name in [(player, fixture['homeTeam']['name']) for player in home_lineup] + [(player, fixture['awayTeam']['name']) for player in away_lineup]:
                if 'player_id' in player:
                    if players is None or player['player_id'] in player_ids:
                        if player['player_id'] is None:
                            ...
                            if player['player_id'] not in player_dict:
                                ...
                                if player_db is None:
                                    ...
                                    player_dict[player['player_id']] = {'goals':0, 
                                     'assists':0, 
                                     'scorers':0, 
                                     'playedGames':1, 
                                     ...
                                     'team':[
                                      team_name]}
                            elif player_dict[player['player_id']]['last_fixture'] != fixture['_id']:
            ...
            for sub, team_name in [(sub, fixture['homeTeam']['name']) for sub in home_subs] + [(sub, fixture['awayTeam']['name']) for sub in away_subs]:
                if sub['in']['player_id'] is None:
                ...
                elif sub['in']['player_id'] not in player_dict:
                    ...
                    if player_db is None:
                        ...
                        player_dict[sub['in']['player_id']] = {'goals':0, 
                         'assists':0, 
                         'scorers':0, 
                         'playedGames':1, 
                        ...
                        if players is None or assist['player_id'] in player_ids:
                            if assist['player_id'] not in player_dict:
                                ...
                                if player_db is None:
                                    ...
                                    player_dict[assist['player_id']] = {'goals':0, 
                                     'assists':1, 
                                     'scorers':1, 
                                     'playedGames':1, 
    ...
    def _get_point_rule_from_timeframe(self, competition, timeframe):
        ...
        if competition is None:
            self.logger.warning('Competition could not be found. Using default point rule.')
            return DEFAULT_POINT_RULE
        else:
            if 'metadata' in competition:
                if 'point_rules' in competition['metadata']:
    ...
    def _get_point_rule_for_season(self, point_rules, season):
        for point_rule in point_rules:
            if 'season_from' in point_rule:
                ...
                if 'season_to' in point_rule:
                    if season <= point_rule['season_to']:
                        return point_rule['rule']
    ...
    def _check_timeframe(self, timeframe=None):
        ...
        if timeframe is None:
            ...
            timeframe = {'type':'season', 
             'season_from':current_season, 
             'season_to':current_season}
            bValid = True
        else:
            if 'type' in timeframe:
                timeframeType = timeframe['type']
                if timeframeType in self.TIME_FRAME_TYPES:
                    if timeframeType == 'date':
                        if 'date_from' in timeframe:
                            if 'date_to' in timeframe:
                                bValid = True
                    else:
                        if timeframeType == 'season':
                            if 'season_from' in timeframe:
                                if 'season_to' in timeframe:
                                    bValid = True
                        elif timeframeType == 'matchday':
                            if 'season_from' in timeframe:
                                if 'season_to' in timeframe:
                                    if 'matchday_from' in timeframe:
                                        if 'matchday_to' in timeframe:
                                            bValid = True
            if bValid == False:
    ...
    def _break_ties(self, standings, start_index, end_index, fixtures, point_rule, tie_break_rules, tie_break_rule_index=0):
        ...
        if upcoming_h2h is False:
            ...
            if compare_index < len(original_standings) - 1:
                if len(tie_break_rules) > tie_break_rule_index + 1:
