...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\soccer\data_connectors\tm_connector.py
# Compiled at: 2018-02-27 09:52:29
# Size of source mod 2**32: 16766 bytes
"""
...
class TMConnector(DataConnector):
    ...
    def __init__(self, mongo_settings=None):
        ...
        if mongo_settings is not None:
            ...
            self.collections = {'teams':self.db['teams'], 
            ...
            self.logger.info('Mongo DB connection initialized')
    ...
    def get_fixtures(self, league_code=None, teams=None, players=None, timeframe=None, count=None, future=None, home=True, away=True):
        if league_code is None:
            ...
            if teams is not None:
                ...
                if len(team_ids) == 1:
                    ...
                    findDict_team_array = [
                     {'homeTeam.team_id': {'$in': team_ids}}] + [{'awayTeam.team_id': {'$in': team_ids}}]
        ...
        if timeframe['type'] == 'date':
            if '$and' in findDict:
                ...
                findDict['$and'] = [
                 {'date': {'$gte': timeframe['date_from']}}, {'date': {'$lte': timeframe['date_to']}}]
            ...
            if '$and' in findDict:
                ...
                findDict['$and'] = [
                 {'season': {'$gte': timeframe['season_from']}}, {'season': {'$lte': timeframe['season_to']}}]
        else:
            if timeframe['type'] == 'matchday':
                if '$and' in findDict:
                    findDict['$and'].extend([
                     {'season': {'$gte': timeframe['season_from']}},
                     {'season': {'$lte': timeframe['season_to']}}])
                ...
                if timeframe['season_from'] != timeframe['season_to']:
                    findMatchday = [{'$and': [{'season': {'$gt': timeframe['season_from']}}, {'season': {'$lt': timeframe['season_to']}}]},
    ...
    def get_table(self, league_code=None, teams=None, timeframe=None):
        ...
        if teams is not None:
            ...
            if len(teams) == 1:
                self.logger.debug('Since only one team was given the complete table is calculated')
                teams = None
        findDict = {'league_code':league_code,  'teams':teams, 
         'timeframe':timeframe}
        self.logger.debug(f"Looking for a table with the following findDict: {findDict}")
        existingTable = self.collections['tables'].find_one({'league_code':league_code, 
         'teams':teams, 
         'timeframe':timeframe})
        if existingTable is None or existingTable['status'] == self.TABLE_STATUS['pending'] and existingTable['next_update'] < datetime.datetime.now():
            table_id = None
            self.logger.debug('Table needs to be created')
            if existingTable is not None:
                if existingTable['status'] == self.TABLE_STATUS['pending']:
                    table_id = existingTable['_id']
            table = self.create_table(league_code, teams, timeframe, table_id)
            return table
        else:
            self.logger.debug('Table found in database')
    ...
    def get_title_table(self, league_code, teams=None, timeframe=None, rank=None):
        if rank is None:
            ...
            if timeframe is None:
                timeframe = {'type':'season',  'season_from':1900, 
                 'season_to':get_current_season()}
            seasons = self._get_seasons_from_timeframe(timeframe)
            for season in seasons:
                temp_timeframe = {'type':'season',  'season_from':season, 
                 'season_to':season}
                ...
                if table is not None and table['status'] == self.TABLE_STATUS['done']:
                    ...
                    if table_entry['teamName'] in title_table:
                        ...
                        title_table_entry = {'teamName':table_entry['teamName'], 
    ...
    def search_player(self, query):
        ...
        return list(self.collections['players.search'].find({'$text': {'$search': search_string}}, {'player_id':True, 
         'name':True, 
    ...
    def search_team(self, query):
        ...
        ngrams = make_ngrams(query)
        search_string = ' '.join([str(x) for x in ngrams])
        return list(self.collections['teams.search'].find({'$text': {'$search': search_string}}, {'team_id':True, 
         'crest_url':True, 
         'name':True, 
         'score':{'$meta': 'textScore'}}).sort([
         (
