...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\soccer\tm\pipelines.py
# Compiled at: 2018-02-09 14:05:39
# Size of source mod 2**32: 9123 bytes
import json, codecs, string, logging, pymongo
...
class MongoDBPipeline(object):
    ...
    def __init__(self):
        ...
        self.collections = {'teams':db['teams'], 
    ...
    def process_item(self, item, spider):
        ...
        if update_fields:
            ...
            self.logger.debug('Item updated successfully')
            if search_fields:
                self.create_ngrams_for_search(item, search_fields, list(find_dict.keys()))
        return item
    ...
    def create_ngrams_for_search(self, item, search_fields, exist_check_fields):
        ...
        self.collections[search_collection].update_one(query, {'$set': search_item},
          upsert=True)
    ...
    def create_search_index(self, collection, name, item):
        ...
        self.collections[collection].create_index(keys,
          name=name,
          weights=weights)