...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\soccer\tm\spiders\tmcom.py
# Compiled at: 2018-02-24 10:31:36
# Size of source mod 2**32: 27827 bytes
import scrapy, re, string, logging, datetime, string
...
class TmcomSpider(scrapy.Spider):
    ...
    start_url_dict = {'/1-bundesliga/startseite/wettbewerb/L1':'BL1', 
    ...
    def parse(self, response):
        ...
        item_competition = CompetitionItem(_id=(ObjectId()),
        ...
        try:
            ...
            if self.settings.get('LEAGUE_CODE') is not None:
                if self.settings.get('LEAGUE_CODE') != item_competition['league_code']:
                    return
                ...
                if self.settings.get('UPDATE_FIXTURES') is not None and self.settings.get('UPDATE_FIXTURES') == 'TRUE':
                    ...
                    update_fixtures = list(db['fixtures'].find({'league_code':item_competition['league_code'], 
                     '$and':[
                    ...
                    for fixture in update_fixtures:
                        yield scrapy.Request((fixture['url']), callback=(self.parseFixture), meta={'item_fixture': FixtureItem(fixture)})
                ...
                if self.settings.get('SEASON') is not None:
                    ...
                    if len(seasons) == 1:
                        ...
                        if len(seasons) == 0:
                            self.logger.warning('Data for this season is not available')
                        else:
                            self.logger.info(f"Scraping {len(seasons)} seasons")
                for season in seasons:
                    season_url = response.url + '/plus/?saison_id=' + season
                    item_competition_season = CompetitionSeasonItem(_id=(ObjectId()),
                      league_code=(item_competition['league_code']),
                      season=(int(season)),
                      url=season_url,
                      collection='competition_season',
                      teams=[])
                    yield scrapy.Request(season_url, callback=(self.parseSeason), meta={'item_competition_season': item_competition_season})
    ...
    def parseSeason(self, response):
        if response.status == 500:
            ...
            for team in teams:
                item_team_season = TeamSeasonItem(_id=(ObjectId()),
                  name=(team.css('::text').extract_first()),
                  team_id=(team.css('::attr(id)').extract_first()),
                  competition={'season':item_competition_season['season'], 
                ...
                item_team = TeamItem(_id=(ObjectId()),
                  team_id=(item_team_season['team_id']),
                  name=(item_team_season['name']),
                  collection='teams',
                  url=team_url,
                  competitions=[
                 item_team_season['competition']])
                yield scrapy.Request((item_team['url']), callback=(self.parseTeam), meta={'item_team': item_team})
                yield scrapy.Request((item_team_season['url']), callback=(self.parseTeamSeason), meta={'item_team_season': item_team_season})
                item_competition_season['teams'].append({'team_id':item_team_season['team_id'], 
                 'name':item_team_season['name']})
            ...
            if all_fixtures is None:
                ...
                yield scrapy.Request(all_fixtures_url, callback=(self.parseAllFixtures), meta={'item_competition_season': item_competition_season})
    ...
    def parseTeamSeason(self, response):
        ...
        for player in players:
            ...
            item_player = PlayerItem(_id=(ObjectId()),
            ...
            yield scrapy.Request((item_player['url']), callback=(self.parsePlayer), meta={'item_player': item_player})
            item_team_season['players'].append({'player_id':item_player['player_id'], 
    ...
    def parseAllFixtures(self, response):
        ...
        for matchday in matchdays:
            ...
            if self.settings.get('MATCHDAY') is not None:
                if self.settings.get('MATCHDAY') != matchday_no:
                    continue
            ...
            for fixture_or_date in fixtures_or_dates:
                ...
                if len(columns.extract()) > 5:
                    ...
                    try:
                        item_fixture = FixtureItem(_id=(ObjectId()),
                          collection='fixtures',
                          date=fixture_date,
                          url=url,
                        ...
                        yield scrapy.Request(url, callback=(self.parseFixture), meta={'item_fixture': item_fixture})
    ...
    def parseFixture(self, response):
        ...
        if referee_name == 'open':
            ...
            if len(lineups) == 2:
                item_fixture['lineups'] = {'home':self.parseLineup(lineups[0]),  'away':self.parseLineup(lineups[1])}
                ...
                item_fixture['lineups'] = {'home':{},  'away':{}}
        ...
        for index, action in enumerate(goals_and_assists):
            ...
            for index_goal, goal_or_assist in enumerate(goal_and_assist):
                if index_goal == 2:
                    ...
                    if 'goal' in newGoal['type']:
                        if 'season' in newGoal['type']:
                ...
                if index_goal == 5:
                    ...
                    try:
                        ...
                        if 'assist' in newAssist['type']:
                            if 'season' in newAssist['type']:
    ...
    def parseCards(self, cardResponse):
        ...
        for index, action in enumerate(card_infos):
            ...
            if 'sb-gelb' in type_of_cards[index]:
                ...
                if 'sb-rot' in type_of_cards[index]:
                    newCard['type'] = 'Red'
                else:
    ...
    def parseLineup(self, lineup):
        ...
        if len(lineup_with_system) == 2:
            ...
            return {'system':self.parseLineupSystem(lineup_with_system[0]), 
            ...
            for player in playerTable:
                ...
                playerDict = {'url':self.base_url + url, 
            ...
            return {'system':system,  'lineup':playerArray, 
             'manager':manager}
    ...
    def parseLineupManager(self, bench):
        ...
        if url is None:
            return {'name': manager.css('::text').extract_first()}
        else:
            return {'url':self.base_url + url,  'name':manager.css('::text').extract_first(), 
             'id':url.split('/')[4]}
    ...
    def parseSubs(self, substitutions):
        ...
        for substitution in substitutions:
            ...
            sub = {'minute':minute, 
             ...
             'in':{'player_id':sub_in.css('a::attr(id)').extract_first(), 
              'name':sub_in.css('a::attr(title)').extract_first()}, 
             'out':{'player_id':sub_out.css('a::attr(id)').extract_first(), 
