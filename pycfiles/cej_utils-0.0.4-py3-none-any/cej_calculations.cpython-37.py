...
# [GCC 8.4.0]
# Embedded file name: /Users/caidan/Projects/cej-utils/cej_utils/cej_calculations.py
# Compiled at: 2020-02-14 17:03:59
# Size of source mod 2**32: 25377 bytes
import imantics, numpy as np
from shapely.geometry import Polygon, LineString, Point, MultiPolygon
from shapely.ops import nearest_points

class CejCalculations:
    __doc__ = '\n    Helper class for calculating CEJ. This class should NOT be used directly.\n    '
    AREA_REMOVAL_PERC = 0.01
    LARGE_DISTANCE = 10000000

    def __init__(self, bone_enamel_boundaries, tooth_mask, is_maxillary, np_image=None, print_flag=False):
        """
        ...
        self.pixel_buffer = int(0.015 * np.sqrt(self.tooth_mask.size))
        self.pixel_buffer_vertical = int(0.01 * self.tooth_mask.shape[0])
        self.pixel_buffer_horizontal = int(0.01 * self.tooth_mask.shape[1])
        self.collinear_threshold = int(0.005 * np.sqrt(self.tooth_mask.size))
        self.snapping_threshold = CejCalculations.compute_snapping_value(self.tooth_mask.size)
    ...
    def is_near_image_border(self, pt, debug_print=False):
        if self.np_image is None:
            ...
            if debug_print:
                print(f"DEBUG sum_top:{sum_top} pt:{pt} pixel_buffer:{self.pixel_buffer_vertical}")
            elif sum_top < self.pixel_buffer_vertical:
                val = True
            else:
                sum_bottom = np.sum(np.squeeze(self.np_image[pt[0]:, pt[1]]).astype(bool))
                if debug_print:
                    print(f"DEBUG sum_bottom:{sum_bottom} pt:{pt} pixel_buffer:{self.pixel_buffer_vertical}")
                elif sum_bottom < self.pixel_buffer_vertical:
                    val = True
                else:
                    sum_left = np.sum(np.squeeze(self.np_image[pt[0], :pt[1]]).astype(bool))
                    if debug_print:
                        print(f"DEBUG sum_sum_left:{sum_left} pt:{pt} pixel_buffer:{self.pixel_buffer_horizontal}")
                    elif sum_left < self.pixel_buffer_horizontal:
                        val = True
                    else:
                        sum_right = np.sum(np.squeeze(self.np_image[pt[0], pt[1]:]).astype(bool))
                        if debug_print:
                            print(f"DEBUG sum_right:{sum_right} pt:{pt} pixel_buffer:{self.pixel_buffer_horizontal}")
                        elif sum_right < self.pixel_buffer_horizontal:
                            val = True
                        else:
                            val = False
        return val
    ...
    def is_interior(self, pt):
        ...
        if pt[0] > border_width:
            if pt[0] < self.tooth_mask.shape[0] - border_width:
                if pt[1] > border_width:
                    if pt[1] < self.tooth_mask.shape[1] - border_width:
                        if not self.is_near_image_border(pt):
                            top_sum = np.sum([1 for i in range(1, self.pixel_buffer) if self.bone_enamel_boundaries['bone_mask'][(
                             max(pt[0] - i, 0), pt[1])]])
                            bottom_sum = np.sum([1 for i in range(1, self.pixel_buffer) if self.bone_enamel_boundaries['bone_mask'][(
                             min(pt[0] + i, self.tooth_mask.shape[0] - 1),
                             pt[1])]])
                            if (self.is_maxillary or top_sum) <= bottom_sum:
                                is_interior_pt = True
                            else:
    ...
    def reject_pt(self, row, col):
        ...
        """
        return col < self.pixel_buffer or col > self.tooth_mask.shape[1] - self.pixel_buffer
    ...
    def cej_alveolor_pts(self, predicted_cejs, debug_print=False):
        ...
        if bone_enamel_boundaries is not None:
            ...
            tooth_mask_points = [ptarray for ptarray in tooth_poly.points if len(ptarray) > 2]
            ...
            for mask_single in tooth_mask_points:
                if len(mask_single) > 2:
                    ...
                    if mask_single_poly.area > max_area:
                        tooth_mask_linear_rings = [LinearRing([(pt[0], pt[1]) for pt in mask_single])]
            ...
            if tooth_mask_multipoly.centroid.coords:
                if list(tooth_mask_multipoly.centroid.coords[0]):
                ...
                if tooth_box:
                    ...
                    W_H = [
                     max(abs(enclosing_coords[0][0] - enclosing_coords[1][0]), abs(enclosing_coords[0][1] - enclosing_coords[1][1])),
                     max(abs(enclosing_coords[1][0] - enclosing_coords[2][0]), abs(enclosing_coords[1][1] - enclosing_coords[2][1]))]
                    W = min(W_H)
                    H = max(W_H)
                else:
                    W = tooth_mask.shape[1]
                    H = tooth_mask.shape[0]
                candidate_bone_pts, *_ = self.find_intersection_pts(bone_enamel_boundaries['bone'], tooth_mask_lines)
                if debug_print:
                ...
                top_border_box = [
                 ...
                 {'x':bone_enamel_boundaries['bone_mask'].shape[1] - self.pixel_buffer_horizontal, 
                  'y':self.pixel_buffer_vertical},
                 {'x':bone_enamel_boundaries['bone_mask'].shape[1] - self.pixel_buffer_horizontal, 
                  'y':int(self.pixel_buffer_vertical / 2)},
                 {'x':self.pixel_buffer, 
                  'y':int(self.pixel_buffer / 2)}]
                bottom_border_box = [
                 {'x':self.pixel_buffer_horizontal, 
                  'y':bone_enamel_boundaries['bone_mask'].shape[0] - self.pixel_buffer_vertical},
                 {'x':bone_enamel_boundaries['bone_mask'].shape[1] - self.pixel_buffer_horizontal, 
                  'y':bone_enamel_boundaries['bone_mask'].shape[0] - self.pixel_buffer_vertical},
                 {'x':bone_enamel_boundaries['bone_mask'].shape[1] - self.pixel_buffer_horizontal, 
                  'y':bone_enamel_boundaries['bone_mask'].shape[0] - int(self.pixel_buffer_vertical / 2)},
                 {'x':self.pixel_buffer_horizontal, 
                  'y':bone_enamel_boundaries['bone_mask'].shape[0] - int(self.pixel_buffer_vertical / 2)}]
                border_box = [
                 top_border_box, bottom_border_box]
                candidate_border_pts, *_ = self.find_intersection_pts(border_box, tooth_mask_lines)
                if debug_print:
            ...
            if dist_values:
                ...
                boundary_bone_enamel_pts['bone'] = sorted([
                 ...
                 {'x':val[1][1], 
                  ...
                  key=(lambda v: v['x']))
        return (
         boundary_bone_enamel_pts, tooth_centroid)
    ...
    def get_two_closest_pts(self, candidate_pts, ref_pts, debug_print=False):
        ...
        if candidate_pts:
            if len(candidate_pts) == 1:
                if ref_pts[0]:
                    dist0 = np.sqrt((candidate_pts[0]['x'] - ref_pts[0]['x']) ** 2 + (candidate_pts[0]['y'] - ref_pts[0]['y']) ** 2)
                else:
                    dist0 = CejCalculations.LARGE_DISTANCE
                if ref_pts[1]:
                    dist1 = np.sqrt((candidate_pts[0]['x'] - ref_pts[1]['x']) ** 2 + (candidate_pts[0]['y'] - ref_pts[1]['y']) ** 2)
                else:
                    dist1 = CejCalculations.LARGE_DISTANCE
                if dist0 < dist1:
                    two_closest_pts = [
                     candidate_pts[0], None]
                else:
                    two_closest_pts = [
                     None, candidate_pts[0]]
            ...
            for ref_pt in ref_pts_list:
                ...
                def dist_fn(pt1):
                    if pt1:
                        if ref_pt:
                            return np.sqrt((pt1['x'] - ref_pt[0]) ** 2 + (pt1['y'] - ref_pt[1]) ** 2)
    ...
    def find_intersection_pts(self, segs, mask_lines, snap=False, swap_xy=True):
        ...
        if not isinstance(mask_lines, MultiLineString):
            if not isinstance(mask_lines, LineString):
                polys_mask = []
                ...
                if len(mask_lines) == 1:
                    mask_lines = Polygon([(value['x'], value['y']) for value in mask_lines[0]])
                    ...
                    if isinstance(mask_lines, Polygon):
                        ...
                        mask_lines = MultiLineString([LinearRing(mask_line.exterior.coords) for mask_line in mask_lines if mask_line if mask_line.exterior])
            else:
                ...
                if snap:
                    if polys_mask:
                        for mask_line in polys_mask:
                            bone_line = CejCalculations.snap_lines(bone_line, mask_line, self.snapping_threshold)
        ...
        if intersection:
            if not (isinstance(intersection, Point) or isinstance(intersection, LineString)):
                for obj in intersection.geoms:
                    if swap_xy:
                        candidate_boundary_pts += [(int(val[1]), int(val[0])) for val in obj.coords]
                    else:
                        candidate_boundary_pts += [(int(val[0]), int(val[1])) for val in obj.coords]

            else:
                if swap_xy:
                    candidate_boundary_pts += [(int(val[1]), int(val[0])) for val in intersection.coords]
                else:
                    candidate_boundary_pts += [(int(val[0]), int(val[1])) for val in intersection.coords]
        return (
         candidate_boundary_pts, intersection, bone_lines, mask_lines)
        ...
        for polygon in polygons:
            ...
            if shapely_poly.area > area_remove_threshold:
                if return_as_shapely_poly:
                    ...
                    boundary.append([{'x':int(coord[0]),  'y':int(coord[1])} for coord in shapely_poly.exterior.coords])
    ...
    def smooth_mask(mask: np.ndarray, print_flag=False) -> np.ndarray:
        ...
        new_image = cv2.threshold(pyrDown, 0.9, 1, cv2.THRESH_BINARY)[1]
        mask = new_image.astype(bool)
        if print_flag:
            print(f"Image smoothed, area reduced from {int(np.sum(vesselImage))} to {int(np.sum(new_image))} pixels")
            import pickle
    ...
    def get_valid(geom):
        if geom.is_valid:
            ...
            geom = geom.is_valid or geom.buffer(0)
        if geom.is_valid:
            return geom
        return False
    ...
    def get_tooth_boundaries(mask):
        ...
        for polygon in polygons:
            if len(polygon) > 2:
                shapely_poly = Polygon(polygon)
                if shapely_poly.area > area_remove_threshold and CejCalculations.get_valid(shapely_poly):
                    multipoly.append(CejCalculations.get_valid(shapely_poly))

        poly_union = unary_union(multipoly)
        if isinstance(poly_union, Polygon):
            boundary = poly_union.is_empty or [[{'x':int(coord[0]),  'y':int(coord[1])} for coord in poly_union.exterior.coords]]
        else:
            if not poly_union.is_empty:
                for poly in poly_union:
                    boundary.append([{'x':int(coord[0]),  'y':int(coord[1])} for coord in poly.exterior.coords])
    ...
    def cleanup_geometry(geometry, return_as_shapely_object=False):
        ...
        if len(geometry) < 3:
            ...
            if return_as_shapely_object:
                return ref_poly
            new_poly = []
            if isinstance(ref_poly, Polygon):
                new_poly = ref_poly.is_empty or [{'x':int(coord[0]),  'y':int(coord[1])} for coord in ref_poly.exterior.coords]
        return new_poly