...
# [GCC 8.4.0]
# Embedded file name: ./ulangel/rnn/nn_block.py
...
from ulangel.rnn.dropouts import ActivationDropout, ConnectionWeightDropout, EmbeddingDropout

class AWD_LSTM(nn.Module):
    __doc__ = 'AWD-LSTM inspired by https://arxiv.org/pdf/1708.02182.pdf.\n    LSTM with embedding dropout, connection weight dropout, activation dropout.\n    '
    initrange = 0.1
    ...
    def __init__(self, vocab_sz, emb_sz, n_hid, n_layers, pad_token, hidden_p=0.2, input_p=0.6, embed_p=0.1, weight_p=0.5):
        ...
        self.rnns = nn.ModuleList([ConnectionWeightDropout(rnn, weight_p) for rnn in self.rnns])
        self.emb.weight.data.uniform_(-self.initrange, self.initrange)
        self.input_dp = ActivationDropout(input_p)
        self.hidden_dps = nn.ModuleList([ActivationDropout(hidden_p) for l in range(n_layers)])
    ...
    def forward(self, input):
        ...
        if n_empty > 0:
            ...
            self.hidden = [(
             h[0][:, :input.size(0)], h[1][:, :input.size(0)]) for h in self.hidden]
...
class LinearDecoder(nn.Module):
    __doc__ = 'The inverse of the embedding layer. Transform embedding vectors back to\n    its corresponding integers.\n    '
...
class SequentialRNN(nn.Sequential):
    __doc__ = 'A sequential module that passes the reset call to its children.'
...
class TextOnlyPoolingLinearClassifier(nn.Module):
    ...
    def forward(self, input):
        ...
        x = torch.cat([
         output[(torch.arange(0, output.size(0)), lengths - 1)], max_pool, avg_pool], 1)
        x = self.layers(x)
        return x
...
class TextPlusPoolingLinearClassifier(nn.Module):
    __doc__ = 'Create a linear classifier with pooling.'
    ...
    def __init__(self, layers1, drops1, layers2, drops2):
        ...
        for n_in, n_out, p, actn in zip(layers1[:-1], layers1[1:], drops1, activs1):
            ...
            mod_layers2 = []
            activs2 = [
             nn.ReLU(inplace=True)] * (len(layers2) - 1)
