...
# Embedded file name: build/bdist.linux-x86_64/egg/pyage/core/locator.py
...
class TorusLocator(Locator):
    ...
    def get_neighbour(self, agent):
        try:
            x, y = self._get_coords(agent)
            self._remove_dead_neighbours(x, y)
            neighbours = [ self._grid[i][j] for i, j in self._get_nieghbour_coords(x, y) if self._grid[i][j] is not None
                         ]
    ...
    def _get_nieghbour_coords(self, x, y):
        return [ (i % self.x, j % self.y) for i in range(x - self.radius, x + self.radius + 1) for j in range(y - self.radius, y + self.radius + 1) if i != x or j != y
               ]
