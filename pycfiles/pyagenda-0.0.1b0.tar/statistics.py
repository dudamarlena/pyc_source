...
# Embedded file name: build/bdist.linux-x86_64/egg/pyage/solutions/forams/statistics.py
...
class PlottingStatistics(Statistics):
    ...
    def _draw_forams(forams, grid):
        plt.scatter([ c[0] for c in forams ], [ c[1] for c in forams ], marker='o', s=[ 200.0 / len(grid) * grid[i][j].foram.energy for i, j in forams ], c=[ 'red' for _ in forams ])
    ...
    def _draw_algae(grid, x, y):
        ...
        plt.scatter(marker='*', s=[ 1000.0 / x * grid[i][j].algae for i in range(x) for j in range(y) ], color='blue', alpha=0.5, *zip(*pts))

    @staticmethod
    def _draw_chambers(grid, i, j):
        plt.annotate(grid[i][j].foram.chambers, xy=(
         i, j), xytext=(-5, 5), textcoords='offset points', ha='right', va='bottom', bbox=dict(boxstyle='round,pad=0.2', fc='yellow', alpha=0.5), arrowprops=dict(arrowstyle='->', connectionstyle='arc3,rad=0'))