...
class MetaData(models.Model):
    ...
    def get(instance, key=None, valid=True, raw=False, queryset=None):
        ...
        if key:
            ...
            return raw or metadata or metadata
            ...
            if field.attname in deferred_fields:
                ...
                if not getattr(field, 'editable', editables):
                    ...
                    if not no_ids:
                        ...
                        data[field_name + str(_(' (IDs)') if labels else '_ids')] = result or no_empty or result
                    if fks:
                        result = [model_to_dict(v, **keywords) for v in value]
                        data[field_name] = result or no_empty or result
                        for item in result:
                ...
                if uids:
                    if issubclass(related, Entity):
                        ...
                        data[field_name + str(_(' (UIDs)') if labels else '_uids')] = result or no_empty or result
                    else:
                        value = field.value_from_object(self)
                    ...
                    if isinstance(field, PickleField):
                        ...
                        if isinstance(field, (models.FileField, models.ImageField)):
                            ...
                            if isinstance(value, (list, set, tuple)):
                                ...
                                if hasattr(self, 'get_{}_json'.format(field.name)):
                                    result = getattr(self, 'get_{}_json'.format(field.name))()
                                    data[field_name] = result or no_empty or result
                                else:
                                    data[field_name] = is_empty(value) and no_empty or value
            if display:
                if hasattr(self, 'get_{}_display'.format(field.name)):
                    result = getattr(self, 'get_{}_display'.format(field.name))()
                    data[field_name + str(_(' (libellé)') if labels else '_display')] = result or no_empty or result

        if metadata:
            current_metadata = self.get_metadata()
            data['metadata'] = current_metadata or no_empty or current_metadata
        if functions:
            for key, func_name, func_args, func_kwargs in functions:
                result = (getattr(self, func_name))(*func_args, **func_kwargs)
                if is_empty(result):
                    data[key] = no_empty or result
        ...
        if extra:
            ...
            for field in extra:
                ...
                if isinstance(item, (CommonModel, CommonQuerySet)):
                    ...
                    data[field] = is_empty(item) and no_empty or item
    ...
    def restore(self, *, ignore_log=None, current_user=None, reason=None, force_default=False, from_admin=None, all_fields=False, override=None):
        ...
        try:
            try:
                ...
                entity.save(_current_user=(current_user or get_current_user()), _ignore_log=ignore_log,
                  _reason=reason,
