...
# [GCC 8.4.0]
# Embedded file name: /home/marc/Git/common-framework/common/api/api_views.py
# Compiled at: 2018-10-19 05:00:48
...
from common.api.input_serializers import ResolveUrlInputSerializer, ResetPasswordSerializer, ConfirmPasswordSerializer
...
def get_urls(request):
    ...
    return Response(OrderedDict(((name, request.build_absolute_uri(url)) for name, url in recursive_get_urls(namespaces=namespaces))))
...
def user_infos(request, user_id=None):
    ...
    user = get_object_or_404((get_user_model().objects.prefetch_related('metadata', 'user_permissions', 'groups__metadata', 'groups__permissions')),
      pk=user_id)
...
def reset_password(request):
    ...
    """
    data = {k:v for k, v in request.validated_data.items() if v if v}
    if not data:
