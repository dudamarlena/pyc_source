...
# [GCC 8.4.0]
# Embedded file name: /home/vivek/Projects/djingles/src/djingles/html/common.py
# Compiled at: 2018-05-24 02:07:50
# Size of source mod 2**32: 6160 bytes
from django.utils.encoding import force_text
import re, json
from jinja2 import Markup
__all__ = [
 'html_json', 'html_attrs', 'Element', 'CssClassList', 'CssStyle', 'add_css_class', 'empty']
...
class CssClassList(object):
    ...
    def append(self, value):
        ...
        elif value not in self.classes:
...
class HtmlAttr(object):
    ...
    def __iter__(self):
        ...
        if self.classes:
            yield (
             'class', str(self.classes))
        if self.styles:
            yield (
             'style', self.styles.render())
...
class Element(object):
    ...
    def convert_to_text(self, el, *args, **kwargs):
        if hasattr(el, 'render'):
            return (el.render)(*args, **kwargs)
        else:
            return force_text(el)
...
empty = Empty('none')
for name in 'html body link meta div span form section article aside main ul li ol dl dd dt p a strong i fieldset legend b em input select button label nav textarea table tbody tfoot thead tr td th figure caption img'.split(' '):
    __all__.append(name)
