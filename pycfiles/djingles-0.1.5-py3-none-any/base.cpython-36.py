...
# [GCC 8.4.0]
# Embedded file name: /home/vivek/Projects/djingles/src/djingles/formatters/base.py
# Compiled at: 2019-10-03 03:40:12
...
__all__ = [
 'Formatter', 'FormattedTable', 'FormattedObject']

class Formatter(object):
    _Formatter__position = 1

    def __init__(self, label=None, attr=None, hidden=False, sortable=True, reverse=False, empty='', icon=None, url=None, editable=False, order_by=None):
        Formatter._Formatter__position += 1
        self._Formatter__position = Formatter._Formatter__position
        self.label = label
        self.icon = icon
        self.url = url
        self.hidden = hidden
    ...
    @property
    def position(self):
        return self._Formatter__position
    ...
    def extract_from(cls, source, include=None, exclude=None):
        result = sorted((inspect.getmembers(source, lambda a: isinstance(a, Formatter))), key=(lambda p: p[1].position))
        if include:
...
class FormattedValue(object):
    ...
    def __init__(self, name, prop, source, attrs=None, owner=None):
        ...
        self.prop = prop
        self.source = source
        self._FormattedValue__attrs = attrs
        self._FormattedValue__owner = owner

    @property
    def attrs(self):
        if self._FormattedValue__attrs:
            return self._FormattedValue__attrs(self)
        else:
            return {}
    ...
    def url(self):
        if not self.prop.url:
            try:
                func = self._FormattedValue__owner.get_cell_url
    ...
    def value(self):
        try:
            return self.prop.extract(self.name, self.source, self._FormattedValue__owner)
    ...
    def wrap_content(self, content):
        prefix = self._FormattedValue__owner.get_prefix(self.name)
        if prefix:
            content = '%s %s' % (prefix, content)
        suffix = self._FormattedValue__owner.get_suffix(self.name)
        if suffix:
...
class FormattedTableColumn(object):
    _FormattedTableColumn__inited = False

    def __init__(self, name, prop, table):
        self.name = name
        self.prop = prop
        self.table = table
        self._FormattedTableColumn__inited = True
    ...
    def __setattr__(self, key, value):
        if self._FormattedTableColumn__inited:
...
class FormattedTableRow(object):
    ...
    def __init__(self, index, source, table, kind=None):
        ...
        for column in self.table.columns:
            self.data[column.name] = FormattedValue((column.name), (column.prop), source, attrs=(self.table.get_cell_attrs),
              owner=table)
