...
# [GCC 8.4.0]
# Embedded file name: /home/vivek/Projects/djingles/src/djingles/views/viewsets.py
# Compiled at: 2020-01-09 01:21:21
# Size of source mod 2**32: 12585 bytes
...
class ViewSetMixin:
    ...
    def as_urls(cls, base_name, url_name=None, **kwargs):
        ...
        for subview in cls.get_action_list():
            ...
            view_class = path(url_regex, (cls.as_view)(base_name=base_name, 
             action=subview.name, 
...
class DetailViewSetMixin(object):
    ...
    def detail(self, request):
        ...
        ctx = {self.context_object_key: self.object}
...
class ListViewSetMixin(object):
    ...
    def list(self, request):
        ...
        if object_list_formatter:
            object_list = object_list_formatter(object_list, sort_key=(self.context_order_key),
...
class CommonModelViewSet(ViewSetMixin, CommonFormView):
    ...
    def get_filter_kwargs(self):
        return {'initial':self.get_filter_initial(), 
    ...
    def get_object_formatter(self):
        if self.object_formatter is None:
            ...
            self.object_formatter = object_formatter_factory(model_class,
    ...
    def get_object_list_formatter(self):
        if self.object_list_formatter is None:
            model_class = self.get_queryset().model
            self.object_list_formatter = table_formatter_factory(model_class,
              fields=(self.object_list_formatter_fields),
              get_cell_url=(lambda me, cell: cell.source.get_absolute_url() if hasattr(cell.source, 'get_absolute_url') else self.reverse('detail', object_id=(cell.source.id))))
    ...
    def paginate_queryset(self, queryset, per_page=None, params_page_key=None):
        ...
        if not per_page:
            ...
            return self.paginator(queryset,
              ...
              allow_empty=False).page(self.request)
        ...
        if method == 'GET':
            ...
            try:
                if self.success_url is None:
                    if self.action in frozenset({'delete', 'create'}):
                        self.success_url = self.reverse('list')
                    else:
