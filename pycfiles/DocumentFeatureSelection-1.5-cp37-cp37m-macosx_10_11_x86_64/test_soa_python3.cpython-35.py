...
class TestSoaPython3(unittest.TestCase):
    ...
    def test_soa_with_term_freq(self):
        data_csr_matrix = data_converter.DataConverter().labeledMultiDocs2TermFreqMatrix(labeled_documents=self.input_dict, n_jobs=5)
        ...
        soa_scores_term_freq = ScoredResultObject(scored_matrix=scored_matrix_term_freq, label2id_dict=label2id_dict, feature2id_dict=vocabulary).ScoreMatrix2ScoreDictionary()
        import pprint
        print('term freq based soa')
        pprint.pprint(soa_scores_term_freq)

    def test_soa_doc_freq(self):
        data_csr_matrix = data_converter.DataConverter().labeledMultiDocs2DocFreqMatrix(labeled_documents=self.input_dict, n_jobs=5)
        assert isinstance(data_csr_matrix, data_converter.DataCsrMatrix)
        label2id_dict = data_csr_matrix.label2id_dict
        csr_matrix_ = data_csr_matrix.csr_matrix_
        n_docs_distribution = data_csr_matrix.n_docs_distribution
        ...
        soa_scores_doc_freq = ScoredResultObject(scored_matrix=scored_matrix_doc_freq, label2id_dict=label2id_dict, feature2id_dict=vocabulary).ScoreMatrix2ScoreDictionary()
        import pprint
