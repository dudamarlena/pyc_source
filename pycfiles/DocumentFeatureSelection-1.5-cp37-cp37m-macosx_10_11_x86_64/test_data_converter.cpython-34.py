...
# [GCC 8.4.0]
# Embedded file name: /Users/kensuke-mi/Desktop/analysis_work/document-feature-selection/tests/test_data_converter.py
...
class TestDataConverter(unittest.TestCase):
    ...
    def test_check_same_csr_matrix(self):
        ...
        """
        n_joblib_tasks = 2
        data_csr_matrix1 = data_converter.DataConverter().labeledMultiDocs2DocFreqMatrix(labeled_documents=self.input_dict, ngram=1, n_jobs=n_joblib_tasks)
        ...
        data_csr_matrix2 = data_converter.DataConverter().labeledMultiDocs2DocFreqMatrix(labeled_documents=self.input_dict, ngram=1, n_jobs=n_joblib_tasks)
        ...
        data_csr_matrix3 = data_converter.DataConverter().labeledMultiDocs2DocFreqMatrix(labeled_documents=self.input_dict, ngram=1, n_jobs=n_joblib_tasks)
    ...
    def test_basic_convert_data(self):
        ...
        :return:
        """
        csr_matrix_information = data_converter.DataConverter().labeledMultiDocs2DocFreqMatrix(labeled_documents=self.input_dict, ngram=1, n_jobs=5)
        ...
        correct_array_numpy = numpy.array([[2, 3, 1, 0, 1, 1, 1, 0], [0, 0, 4, 1, 2, 1, 0, 0], [0, 1, 1, 4, 0, 0, 0, 2]]).astype(numpy.int64)
    ...
    def test_multi_process_convert_data(self):
        ...
        :return:
        """
        data_csr_object = data_converter.DataConverter().labeledMultiDocs2DocFreqMatrix(labeled_documents=self.input_dict, ngram=1, n_jobs=5)
    ...
    def test_n_gram_multi_process_convert_data(self):
        ...
        :return:
        """
        data_csr_object = data_converter.DataConverter().labeledMultiDocs2DocFreqMatrix(labeled_documents=self.input_dict, ngram=3, n_jobs=5)
        assert isinstance(data_csr_object.csr_matrix_, csr_matrix)
        assert isinstance(data_csr_object.label2id_dict, dict)
        assert isinstance(data_csr_object.vocabulary, dict)
    ...
    def test_get_pmi_feature_dictionary(self):
        ...
        pmi_scored_dictionary_objects = data_converter.ScoreMatrix2ScoreDictionary(scored_matrix=pmi_scored_matrix, label2id_dict=data_csr_object.label2id_dict, feature2id_dict=data_csr_object.vocabulary, outformat='dict', sort_desc=True, n_jobs=5)
        assert isinstance(pmi_scored_dictionary_objects, dict)
        logging.debug(pmi_scored_dictionary_objects)
        pmi_scored_dictionary_objects = data_converter.ScoreMatrix2ScoreDictionary(scored_matrix=pmi_scored_matrix, label2id_dict=data_csr_object.label2id_dict, feature2id_dict=data_csr_object.vocabulary, outformat='items', sort_desc=True, n_jobs=5)
