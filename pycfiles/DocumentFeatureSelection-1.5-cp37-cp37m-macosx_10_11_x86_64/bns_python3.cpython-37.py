...
def bns(X: Union[(memmap, csr_matrix)], feature_index: int, sample_index: int, unit_distribution: np.ndarray, true_index: int=0, verbose: bool=False):
    if true_index == 0:
        false_index = 1
    else:
        if true_index == 1:
        ...
        * Params
        ...
        if not isinstance(X, csr_matrix):
            ...
            if 'unit_distribution' not in fit_params:
                ...
                if not isinstance(fit_params['unit_distribution'], (list, ndarray)):
                    raise AssertionError
                else:
