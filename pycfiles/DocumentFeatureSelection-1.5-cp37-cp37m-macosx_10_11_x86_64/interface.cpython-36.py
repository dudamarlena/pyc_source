...
# [GCC 8.4.0]
# Embedded file name: /opt/project/DocumentFeatureSelection/interface.py
...
def run_feature_selection(input_dict: AvailableInputTypes, method: str, use_cython: bool=False, is_use_cache: bool=False, is_use_memmap: bool=False, cache_backend: str='PersistentDict', path_working_dir: str=None, matrix_form=None, n_jobs: int=1) -> ScoredResultObject:
    ...
    * Args
    ...
    if method == 'tf_idf':
        matrix_data_object = data_converter.DataConverter().convert_multi_docs2term_frequency_matrix(labeled_documents=input_dict,
          n_jobs=n_jobs,
        ...
        matrix_data_object = data_converter.DataConverter().convert_multi_docs2document_frequency_matrix(labeled_documents=input_dict,
          n_jobs=n_jobs,
          is_use_cache=is_use_cache,
          is_use_memmap=is_use_memmap,
          path_working_dir=path_working_dir)
        ...
        if method == 'pmi':
            ...
            scored_sparse_matrix = PMI().fit_transform(X=(matrix_data_object.csr_matrix_), n_docs_distribution=(matrix_data_object.n_docs_distribution),
              n_jobs=n_jobs,
            ...
            if method == 'soa':
                backend_strategy = decide_joblib_strategy(matrix_data_object.vocabulary)
                scored_sparse_matrix = SOA().fit_transform(X=(matrix_data_object.csr_matrix_), unit_distribution=(matrix_data_object.n_docs_distribution),
                  n_jobs=n_jobs,
                  joblib_backend=backend_strategy,
                  use_cython=use_cython)
                assert isinstance(scored_sparse_matrix, csr_matrix)
        ...
        if method == 'soa' and matrix_form == 'term_freq':
            ...
            if method == 'bns':
                ...
                backend_strategy = decide_joblib_strategy(matrix_data_object.vocabulary)
                scored_sparse_matrix = BNS().fit_transform(X=(matrix_data_object.csr_matrix_),
