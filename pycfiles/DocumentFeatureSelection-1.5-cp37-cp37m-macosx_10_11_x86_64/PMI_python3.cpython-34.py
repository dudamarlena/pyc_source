...
class PMI(object):
    ...
    def fit_transform(self, X: Union[(csr_matrix, memmap)], n_docs_distribution, n_jobs=1,
                      verbose=False,
        ...
        if use_cython:
            ...
            pmi_score_csr_source = main(X=X, n_docs_distribution=n_docs_distribution, sample_range=sample_range, feature_range=feature_range, n_total_doc=n_total_document, verbose=False)
        else:
            self.pmi = pmi
    ...
    def docId_word_PMI(self, X: Union[(csr_matrix, memmap)], n_docs_distribution: numpy.ndarray, n_total_doc: int, feature_index: int, sample_index: int, verbose=False,
                       use_cython: bool=False):
        ...
        :return:
        """
        pmi_score = self.pmi(X=X, n_docs_distribution=n_docs_distribution, feature_index=feature_index, sample_index=sample_index, n_total_doc=n_total_doc, verbose=verbose)
        return (
