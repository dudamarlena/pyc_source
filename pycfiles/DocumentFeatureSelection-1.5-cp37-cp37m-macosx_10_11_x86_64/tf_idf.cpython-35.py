...
class TFIDF(object):
    ...
    def fit_transform(self, X):
        if isinstance(X, csr_matrix):
            ...
            X = X
        tf_idf_matrix = self.call_sklearn_tfidf(X=X, norm=self.norm_metric, use_idf=self.use_idf_bool, smooth_idf=self.smooth_idf_bool, sublinear_tf=self.sublinear_tf_bool)
    ...
    def call_sklearn_tfidf(self, X, norm='l2', use_idf=True, smooth_idf=True, sublinear_tf=False):
        ...
        tf_idf_generator = TfidfTransformer(norm=norm, use_idf=use_idf, smooth_idf=smooth_idf, sublinear_tf=sublinear_tf)
