...
class TestPmiPython3(unittest.TestCase):
    ...
    def setUp(self):
        input_dict = {'label_a': [
         ...
         'label_c': [
                     ...
                     [
                      'aa', 'xx', 'cc']]}
        data_csr_matrix = data_converter.DataConverter().labeledMultiDocs2DocFreqMatrix(labeled_documents=input_dict, ngram=1, n_jobs=5)
    ...
    def test_output_result_pmi(self):
        ...
        assert isinstance(scored_matrix, csr_matrix)
        pmi_scored_dict = data_converter.ScoreMatrix2ScoreDictionary(scored_matrix=scored_matrix, label2id_dict=self.label2id_dict, feature2id_dict=self.vocabulary, outformat='items', n_jobs=1)
