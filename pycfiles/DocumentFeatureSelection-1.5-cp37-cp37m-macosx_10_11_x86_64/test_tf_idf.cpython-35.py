...
class TestTfIdf(unittest.TestCase):
    ...
    def setUp(self):
        ...
        tf_matrix = numpy.array([
         ...
         [
          0, 1, 1, 7, 0, 0, 0, 3]])
        data_csr_matrix = data_converter.DataConverter().labeledMultiDocs2DocFreqMatrix(labeled_documents=input_dict, n_jobs=-1)
    ...
    def test_output_result_pmi(self):
        ...
        tf_idf_weighted_matrix = tf_idf.TFIDF().fit_transform(X=self.csr_matrix_)
        assert isinstance(tf_idf_weighted_matrix, csr_matrix)
        tf_idf_scored_dict = ScoredResultObject(scored_matrix=tf_idf_weighted_matrix, label2id_dict=self.label2id_dict, feature2id_dict=self.vocabulary).ScoreMatrix2ScoreDictionary(outformat='items')
