...
# [GCC 8.4.0]
# Embedded file name: /opt/project/tests/test_soa_python3.py
# Compiled at: 2018-10-24 08:06:42
...
class TestSoaPython3(unittest.TestCase):
    ...
    def test_soa_with_term_freq(self):
        data_csr_matrix = data_converter.DataConverter().labeledMultiDocs2TermFreqMatrix(labeled_documents=(self.input_dict),
          n_jobs=5)
        ...
        scored_matrix_term_freq = soa_python3.SOA().fit_transform(X=csr_matrix_,
          unit_distribution=n_docs_distribution,
          verbose=True)
        soa_scores_term_freq = ScoredResultObject(scored_matrix=scored_matrix_term_freq,
    ...
    def test_soa_doc_freq(self):
        data_csr_matrix = data_converter.DataConverter().convert_multi_docs2document_frequency_matrix(labeled_documents=(self.input_dict),
          n_jobs=5)
        assert isinstance(data_csr_matrix, data_converter.DataCsrMatrix)
        label2id_dict = data_csr_matrix.label2id_dict
        csr_matrix_ = data_csr_matrix.csr_matrix_
        n_docs_distribution = data_csr_matrix.n_docs_distribution
        vocabulary = data_csr_matrix.vocabulary
        scored_matrix_doc_freq = soa_python3.SOA().fit_transform(X=csr_matrix_,
          unit_distribution=n_docs_distribution,
          verbose=True)
        soa_scores_doc_freq = ScoredResultObject(scored_matrix=scored_matrix_doc_freq,
          label2id_dict=label2id_dict,
          feature2id_dict=vocabulary).convert_score_matrix2score_record()
