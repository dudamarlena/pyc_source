...
# Compiled at: 2018-10-24 08:56:01
...
def run_feature_selection(input_dict: AvailableInputTypes, method: str, use_cython: bool=False,
    ...
    * Args
    ...
    if method == 'tf_idf':
        matrix_data_object = data_converter.DataConverter().convert_multi_docs2term_frequency_matrix(labeled_documents=input_dict, n_jobs=n_jobs, is_use_cache=is_use_cache, is_use_memmap=is_use_memmap, path_working_dir=path_working_dir, cache_backend=cache_backend)
        ...
        if method in ('soa', 'pmi') and matrix_form is None:
            matrix_data_object = data_converter.DataConverter().convert_multi_docs2document_frequency_matrix(labeled_documents=input_dict, n_jobs=n_jobs, is_use_cache=is_use_cache, is_use_memmap=is_use_memmap, path_working_dir=path_working_dir)
            assert isinstance(matrix_data_object, DataCsrMatrix)
            if method == 'pmi':
            ...
            if method == 'soa' and matrix_form == 'term_freq':
                matrix_data_object = data_converter.DataConverter().convert_multi_docs2term_frequency_matrix(labeled_documents=input_dict, n_jobs=n_jobs, is_use_cache=is_use_cache, is_use_memmap=is_use_memmap, path_working_dir=path_working_dir)
                assert isinstance(matrix_data_object, DataCsrMatrix)
                backend_strategy = decide_joblib_strategy(matrix_data_object.vocabulary)
                ...
                if method == 'bns':
                    ...
                    backend_strategy = decide_joblib_strategy(matrix_data_object.vocabulary)
                    scored_sparse_matrix = BNS().fit_transform(X=matrix_data_object.csr_matrix_, unit_distribution=matrix_data_object.n_term_freq_distribution, n_jobs=n_jobs, true_index=true_class_index, joblib_backend=backend_strategy, use_cython=use_cython)
                    if not isinstance(scored_sparse_matrix, csr_matrix):
                        raise AssertionError
                else:
                    raise Exception()
    ...
    return ScoredResultObject(scored_matrix=scored_sparse_matrix, label2id_dict=matrix_data_object.label2id_dict, feature2id_dict=matrix_data_object.vocabulary, method=method, matrix_form=matrix_form, frequency_matrix=matrix_data_object.csr_matrix_)