...
# [GCC 8.4.0]
# Embedded file name: /opt/project/tests/test_tf_idf.py
...
class TestTfIdf(unittest.TestCase):
    ...
    def setUp(self):
        ...
        tf_matrix = numpy.array([
         ...
         [
          0, 1, 1, 7, 0, 0, 0, 3]])
        data_csr_matrix = data_converter.DataConverter().convert_multi_docs2document_frequency_matrix(labeled_documents=input_dict,
          n_jobs=(-1))
    ...
    def test_output_result_pmi(self):
        tf_idf_weighted_matrix = tf_idf.TFIDF().fit_transform(X=(self.csr_matrix_))
        if not isinstance(tf_idf_weighted_matrix, csr_matrix):
            raise AssertionError
        else:
            tf_idf_scored_dict = ScoredResultObject(scored_matrix=tf_idf_weighted_matrix,
              label2id_dict=(self.label2id_dict),
              feature2id_dict=(self.vocabulary)).convert_score_matrix2score_record(outformat='items')
