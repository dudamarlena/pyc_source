...
# [GCC 8.4.0]
# Embedded file name: /codes/tests/test_data_converter.py
...
class TestDataConverter(unittest.TestCase):
    ...
    def setUp(self):
        self.input_dict = {'label_a':[
         ...
         'label_c':[
          ...
          [
           'aa', 'xx', 'cc']]}
        self.input_dict_complex_feature = {'label_a':[
          [
           [
            'a', 'b'], ['b', 'c'], ['a', 'b', 'c']],
          [
    ...
    def test_check_same_csr_matrix(self):
        ...
        """
        n_joblib_tasks = 2
        data_csr_matrix1 = data_converter.DataConverter().labeledMultiDocs2DocFreqMatrix(labeled_documents=(self.input_dict),
          n_jobs=n_joblib_tasks)
        if not isinstance(data_csr_matrix1, DataCsrMatrix):
            ...
            data_csr_matrix2 = data_converter.DataConverter().labeledMultiDocs2DocFreqMatrix(labeled_documents=(self.input_dict),
              n_jobs=n_joblib_tasks)
            ...
            data_csr_matrix3 = data_converter.DataConverter().labeledMultiDocs2DocFreqMatrix(labeled_documents=(self.input_dict),
              n_jobs=n_joblib_tasks)
    ...
    def test_basic_convert_data(self):
        ...
        :return:
        """
        csr_matrix_information = data_converter.DataConverter().labeledMultiDocs2DocFreqMatrix(labeled_documents=(self.input_dict),
          n_jobs=5)
        if not isinstance(csr_matrix_information, DataCsrMatrix):
            raise AssertionError
        else:
            ...
            correct_array_numpy = numpy.array([
             ...
             [
              0, 0, 4, 1, 2, 1, 0, 0],
             [
              0, 1, 1, 4, 0, 0, 0, 2]]).astype(numpy.int64)
    ...
    def test_multi_process_convert_data(self):
        ...
        :return:
        """
        data_csr_object = data_converter.DataConverter().labeledMultiDocs2DocFreqMatrix(labeled_documents=(self.input_dict),
          n_jobs=5)
        if not isinstance(data_csr_object.csr_matrix_, csr_matrix):
            raise AssertionError
        else:
    ...
    def test_complex_feature_convertion(self):
        """"""
        csr_matrix_information = data_converter.DataConverter().labeledMultiDocs2DocFreqMatrix(labeled_documents=(self.input_dict_complex_feature),
          n_jobs=1)
        if not isinstance(csr_matrix_information, DataCsrMatrix):
            raise AssertionError
        else:
            ...
            correct_array_numpy = numpy.array([
             ...
             [
              0.0, 0.0, 1.0, 3.0, 1.0],
             [
              1.0, 1.0, 0.0, 2.0, 1.0]]).astype(numpy.int64)
            assert numpy.array_equal(correct_array_numpy, dense_matrix_constructed_matrix)
