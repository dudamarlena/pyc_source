...
class TestBnsPython3(unittest.TestCase):
    ...
    def test_fit_transform(self):
        data_csr_matrix = data_converter.DataConverter().labeledMultiDocs2DocFreqMatrix(labeled_documents=self.correct_input, ngram=1, n_jobs=5)
        ...
        bns_scores_dict = data_converter.ScoreMatrix2ScoreDictionary(scored_matrix=bns_score_csr_matrix, label2id_dict=label2id_dict, feature2id_dict=vocabulary)
    ...
    def test_check_input_error(self):
        incorrect_input_dict = {'label_a': [
         ...
         'label_c': [
                     ...
                     [
                      'aa', 'xx', 'cc']]}
        data_csr_matrix = data_converter.DataConverter().labeledMultiDocs2DocFreqMatrix(labeled_documents=incorrect_input_dict, ngram=1, n_jobs=5)
        assert isinstance(data_csr_matrix, DataCsrMatrix)
        label2id_dict = data_csr_matrix.label2id_dict
        csr_matrix_ = data_csr_matrix.csr_matrix_
        n_docs_distribution = data_csr_matrix.n_docs_distribution
