...
# Size of source mod 2**32: 10680 bytes
...
class DataConverter(object):
    ...
    def labeledMultiDocs2TermFreqMatrix(self, labeled_documents: AvailableInputTypes, is_use_cache: bool=False,
        ...
        * Params
        ...
        """
        self._DataConverter__check_data_structure(labeled_documents)
        if ngram > 1:
            labeled_documents = ngram_constructor.ngram_constructor(labeled_documents=labeled_documents, ngram=ngram, n_jobs=n_jobs)
        ...
        n_docs_distribution = self.count_document_distribution(labeled_documents=labeled_documents, label2id=set_document_information.label2id)
        term_frequency_distribution = self.count_term_frequency_distribution(labeled_documents=labeled_documents, label2id=set_document_information.label2id)
        return DataCsrMatrix(csr_matrix_=set_document_information.matrix_object, label2id_dict=set_document_information.label2id, vocabulary=set_document_information.feature2id, n_docs_distribution=n_docs_distribution, n_term_freq_distribution=term_frequency_distribution, is_use_cache=is_use_cache, is_use_memmap=is_use_memmap, path_working_dir=path_working_dir)

    def labeledMultiDocs2DocFreqMatrix(self, labeled_documents: AvailableInputTypes, is_use_cache: bool=False,
                                       is_use_memmap: bool=False,
                                       path_working_dir: str=None,
                                       ngram: int=1,
                                       n_jobs: int=1,
        ...
        * Output
        ...
        if ngram > 1:
            labeled_documents = ngram_constructor.ngram_constructor(labeled_documents=labeled_documents, ngram=ngram, n_jobs=n_jobs)
        logger.debug(msg='Now pre-processing before CSR matrix')
        set_document_information = labeledMultiDocs2labeledDocsSet.multiDocs2DocFreqInfo(labeled_documents, n_jobs=n_jobs)
...
def scored_matrix2score_dictionary(scored_matrix: csr_matrix, label2id_dict: Dict[(str, int)], feature2id_dict: Dict[(FeatureType, int)], outformat: str='items',
                                   sort_desc: bool=True,
                                   n_jobs: int=1):
    ...
    >>> [{feature: score}]
    """
    scored_objects = utils.get_feature_dictionary(weighted_matrix=scored_matrix, vocabulary=feature2id_dict, label_group_dict=label2id_dict, n_jobs=n_jobs)
    if sort_desc:
