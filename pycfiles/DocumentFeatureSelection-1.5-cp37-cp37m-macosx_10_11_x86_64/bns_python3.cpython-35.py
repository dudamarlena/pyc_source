...
# [GCC 8.4.0]
# Embedded file name: /Users/kensuke-mi/Desktop/analysis_work/document-feature-selection/DocumentFeatureSelection/bns/bns_python3.py
# Compiled at: 2018-10-24 07:53:19
...
def bns(X: Union[(memmap, csr_matrix)], feature_index: int, sample_index: int, unit_distribution: np.ndarray, true_index: int=0,
        verbose: bool=False):
    if true_index == 0:
        false_index = 1
    else:
    ...
    if verbose:
        ...
        logging.debug('tp:{} fp:{} fn:{} tn:{} pos:{} neg:{} tpr:{} fpr:{}'.format(tp, fp, fn, tn, pos, neg, tpr, fpr))
...
class BNS(TransformerMixin):
    ...
    def fit_transform(self, X: Union[(memmap, csr_matrix)], y=None, **fit_params):
        """* What you can do
        ...
        * Params
        ...
        if is_use_cython:
            ...
            bns_score_csr_source = main(X=X, unit_distribution=unit_distribution, sample_range=sample_range, feature_range=feature_range, true_index=true_index, verbose=verbose)
        else:
            bns_score_csr_source = joblib.Parallel(n_jobs=n_jobs, backend=joblib_backend)(joblib.delayed(self.docId_word_BNS)(X=X, feature_index=feature_index, sample_index=sample_index, true_index=true_index, unit_distribution=unit_distribution, verbose=verbose) for sample_index in sample_range for feature_index in feature_range)
        ...
        bns_featured_csr_matrix = csr_matrix((data_list, (row_list, col_list)), shape=(
         X.shape[0],
         X.shape[1]))
        logging.debug(msg='End calculating BNS')
        return bns_featured_csr_matrix

    def docId_word_BNS(self, X: csr_matrix, feature_index: int, sample_index: int, unit_distribution: np.ndarray, true_index: int, verbose=False):
        assert isinstance(X, csr_matrix)
        assert isinstance(feature_index, int)
        assert isinstance(sample_index, int)
        bns_score = bns(X=X, feature_index=feature_index, sample_index=sample_index, true_index=true_index, unit_distribution=unit_distribution, verbose=verbose)
        return (
         sample_index, feature_index, bns_score)