...
# [GCC 8.4.0]
# Embedded file name: /Users/kensuke-mi/Desktop/analysis_work/document-feature-selection/DocumentFeatureSelection/common/crs_matrix_constructor.py
# Compiled at: 2016-09-26 20:16:08
# Size of source mod 2**32: 4460 bytes
from logging import getLogger, StreamHandler
from collections import namedtuple
import joblib, sys, logging, numpy
...
class PosTuple(object):
    __slots__ = [
     'doc_id', 'word_id', 'document_frequency']
...
def SUB_FUNC_make_value_pairs(doc_id: int, doc_freq_obj: numpy.ndarray, vocabulary: numpy.ndarray) -> numpy.ndarray:
    value_pairs = numpy.array([get_data_col_row_values(doc_id=doc_id, word=key_value_tuple['key'], doc_freq=key_value_tuple['value'], vocaburary=vocabulary) for key_value_tuple in doc_freq_obj])
    return value_pairs
...
def preprocess_csr_matrix(feature_frequency, vocabulary, n_jobs: int, joblib_backend: str='Parallel'):
    ...
    """
    if joblib_backend not in PARAM_JOBLIB_BACKEND:
        ...
        set_value_position_list = joblib.Parallel(n_jobs=n_jobs, backend=joblib_backend)(joblib.delayed(SUB_FUNC_make_value_pairs)(doc_id, doc_freq_obj, vocabulary) for doc_id, doc_freq_obj in enumerate(feature_frequency))
        value_position_list = sorted([l for set in set_value_position_list for l in set], key=lambda pos_tuple: (
         pos_tuple[0], pos_tuple[1], pos_tuple[2]))
