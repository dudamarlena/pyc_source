...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/ore/alchemist/zs2sa.py
...
class ObjectTranslator(object):
    ...
    def __call__(self, field, metadata):
        ...
        return rdb.Column(pk, rdb.Integer, rdb.ForeignKey(field_name), nullable=False)


fieldmap = {'ASCII': StringTranslator(), 
...
def transmute(zopeschema, metadata, tablename='', add_primary=True):
    ...
    if add_primary:
        columns.insert(0, rdb.Column(get_pk_name(tablename), rdb.Integer, primary_key=True))
