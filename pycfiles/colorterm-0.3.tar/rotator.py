...
# Embedded file name: /tmp/0/lib/python2.7/site-packages/colortater/rotator.py
# Compiled at: 2012-06-19 16:56:31
try:
...
import re
from os import path
color_rex = re.compile('(?P<hexcolor>#[0-9a-fA-F]{6}|#[0-9a-fA-F]{3})|(?P<rgbfunc>rgb[(](?P<r>[0-9]+), (?P<g>[0-9]+), (?P<b>[0-9]+)[)])')
HUE_THRESH = 20

def parse_color(match):
    if match.group('hexcolor'):
        return QColor(match.group('hexcolor'))
    if match.group('rgbfunc'):
...
class ColorRotator(object):
    ...
    def color_transform(self, color, groupnum=None):
        groupmatches = groupnum or [ idx for idx, g in enumerate(self.groups) if color in g ]
        if not len(groupmatches) == 1:
            raise AssertionError
            groupnum = groupmatches[0]
        hue_transform = self.group_rotations[groupnum]
        old_hsv = (
         color.hue(), color.hsvSaturation(), color.value())
