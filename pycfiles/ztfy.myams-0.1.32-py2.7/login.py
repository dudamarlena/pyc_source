...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ztfy/myams/login.py
# Compiled at: 2015-11-03 10:51:54
        ...
        if errors:
            ...
            if self.principal is not None:
                if IUnauthorized.providedBy(self.context):
                    ...
                    return writer.write({'status': 'redirect', 'location': absoluteURL(context, self.request)})
                else:
                    came_from = data.get('came_from')
                    if came_from:
                        return writer.write({'status': 'redirect', 'location': came_from})
                    target = queryMultiAdapter((self.context, self.request, Interface), IDefaultView)
                    return writer.write({'status': 'redirect', 'location': '%s/%s' % (absoluteURL(self.context, self.request),
                                  target.viewname if target is not None else '@@index.html')})

            else:
                return writer.write({'status': 'redirect', 'location': '%s/@@login.html?came_from=%s' % (absoluteURL(self.context, self.request),
                              data.get('came_from'))})
            return
