...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ztfy/myams/form.py
# Compiled at: 2017-12-20 05:23:15
from z3c.json.interfaces import IJSONWriter
from ztfy.baseskin.interfaces import IDialog
from ztfy.baseskin.interfaces.form import IGroupsBasedForm, IWidgetsGroup, IAJAXForm, IForm, IInnerSubForm, IInnerTabForm, ICustomUpdateSubForm
from ztfy.myams.interfaces import IModalEditFormButtons, IModalAddFormButtons, IEditFormButtons, IAddFormButtons, IModalDisplayFormButtons, IObjectData
...
class WidgetsGroup(object):
    ...
    def __init__(self, id, widgets=(), legend=None, help=None, css_class='', switch=False, checkbox_switch=False, checkbox_field=None, hide_if_empty=False):
...
def NamedWidgetsGroup(id, widgets, names=(), legend=None, help=None, css_class='', switch=False, checkbox_switch=False, checkbox_field=None, hide_if_empty=False):
    """Create a widgets group based on widgets names"""
    return WidgetsGroup(id, [ widgets.get(name) for name in names ], legend, help, css_class, switch, checkbox_switch, checkbox_field, hide_if_empty)
...
class AjaxForm(ajax.AJAXRequestHandler):
    ...
    def getFormOptions(self):
        if self.form_options:
            ...
            return writer.write(self.form_options)

    def getAjaxErrors(self):
        errors = {'status': 'error', 'error_message': translate(self.status, context=self.request)}
        for error in self.errors:
            ...
            if hasattr(error, 'widget'):
                ...
                if widget is not None:
                    errors.setdefault('widgets', []).append({'name': widget.name, 'label': translate(widget.label, context=self.request), 
...
class BaseForm(GroupsBasedForm, AjaxForm, form.Form):
    ...
    def warnOnChange(self):
        if self.warn_on_change is True:
            ...
            if self.warn_on_change is False:
                return 'false'
            else:
                return
...
class AddForm(BaseForm, form.AddForm):
    ...
    def getSubmitOutput(self, writer, changes):
        return writer.write({'status': 'reload', 'location': self.nextURL()})
...
class EditForm(BaseForm, form.EditForm):
    ...
    def getSubmitOutput(self, writer, changes):
        ...
        return writer.write({'status': 'success', 'message': translate(message, context=self.request)})
