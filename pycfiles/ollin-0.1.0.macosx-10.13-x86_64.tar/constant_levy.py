...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/ollin/movement_models/constant_levy.py
...
class Model(MovementModel):
    ...
    default_parameters = {'movement': {'pareto': 1.8}, 
       'density': {'alpha': 5.139654979615388, 
                   'density_exp': 0.8673231302202313, 
                   'hr_exp': 0.8709250205414867, 
                   'niche_size_exp': 0.8105129468063047}, 
       'home_range': {'alpha': 82.0335925804581, 
                      'exponent': 1.9018678531419373}, 
       'velocity': {'alpha': 0.0005826019701355142, 
    ...
    def generate_movement(self, initial_positions, site, steps, velocity):
        ...
        range_ = site.range
        mov = self._movement(initial_positions, velocity, range_, steps, exponent)
        return mov

    @staticmethod
    @jit(float64[:, :, :](float64[:, :], float64, float64[:], int64, float64), nopython=True)
    def _movement(random_positions, velocity, range_, steps, exponent):
