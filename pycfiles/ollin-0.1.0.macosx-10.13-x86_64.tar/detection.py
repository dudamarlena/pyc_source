...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/ollin/core/detection.py
...
class CameraConfiguration(object):
    ...
    def __init__(self, positions, directions, site, cone_range=None, cone_angle=None):
    ...
    def plot(self, ax=None, figsize=(10, 10), include=None, cone_length=0.4, camera_alpha=0.3, camera_color=None, **kwargs):
        ...
        if 'cameras' in include:
            ...
            for pos, angle in zip(self.positions, self.directions):
                ang = 180 * np.angle(angle[0] + complex(0.0, 1.0) * angle[1]) / np.pi
                wedge = Wedge(pos, cone_length, ang - c_angle, ang + c_angle)
                patches.append(wedge)

            collection = PatchCollection(patches, cmap=plt.cm.hsv, alpha=camera_alpha)
    ...
    @classmethod
    def make_random(cls, num, site, min_distance=None, cone_range=None, cone_angle=None):
        ...
        if min_distance is None:
            ...
            positions = _make_random_camera_positions(num, range, min_distance=min_distance)
        angles = _make_random_directions(num)
        cam = cls(positions, angles, site, cone_range=cone_range, cone_angle=cone_angle)
        return cam

    @classmethod
    def make_grid(cls, distance, site, cone_range=None, cone_angle=None):
        ...
        angles = _make_random_directions(num)
        cam = cls(positions, angles, site, cone_angle=cone_angle, cone_range=cone_range)
        return cam
...
class Detection(object):
    ...
    def estimate_occupancy(self, model='single_species', method='MAP', priors=None):
    ...
    def plot(self, ax=None, figsize=(10, 10), include=None, detection_cmap='Purples', detection_alpha=0.2, **kwargs):
...
class MovementDetection(Detection):
    ...
    def plot(self, ax=None, figsize=(10, 10), include=None, **kwargs):
        ...
        ax = super(MovementDetection, self).plot(ax=ax, include=include, **kwargs)
        return ax
...
def _make_detection_data(movement, camera_config):
    ...
    camera_direction = camera_direction[:, 0] + complex(0.0, 1.0) * camera_direction[:, 1]
    ...
    relative_pos = movement_data[:, :, None, :] - camera_position[None, None, :, :]
    relative_pos = relative_pos[:, :, :, 0] + complex(0.0, 1.0) * relative_pos[:, :, :, 1]
