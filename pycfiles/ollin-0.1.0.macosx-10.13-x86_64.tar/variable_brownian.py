...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/ollin/movement_models/variable_brownian.py
...
class Model(MovementModel):
    ...
    default_parameters = {'movement': {'niche_weight': 0.2}, 
       'density': {'alpha': 4.956627725964207, 
                   'density_exp': 0.8509470680476414, 
                   'hr_exp': 0.8449833387462566, 
                   'niche_size_exp': 0.8752336080881222}, 
       'home_range': {'alpha': 57.61382053222021, 
                      'exponent': 1.9096337585646315}, 
       'velocity': {'alpha': 0.14473388093667755, 
    ...
    def generate_movement(self, initial_positions, site, steps, velocity):
        ...
        range_ = site.range
        mov = self._movement(heatmap, initial_positions, resolution, velocity, range_, steps, niche_weight)
        return mov

    @staticmethod
    @jit(float64[:, :, :](float64[:, :], float64[:, :], float64, float64, float64[:], int64, float64), nopython=True)
    def _movement(heatmap, random_positions, resolution, velocity, range_, steps, niche_weight):
        ...
        directions = np.random.normal(0, sigma, size=(steps, num, 2))
