...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/ollin/movement_analyzers/velocity.py
...
class Analyzer(MovementAnalyzer):
    ...
    def plot(self, ax=None, figsize=(10, 10), num_individual=0, bins=20, width=None, cmap='Reds', alpha=0.8, log=True):
        ...
        if num_individual == 'all':
            ...
            range_ = (0, vdata.max())
            histogram, _ = np.histogram(vdata, bins=bins, range=range_, normed=not log)
            if log:
