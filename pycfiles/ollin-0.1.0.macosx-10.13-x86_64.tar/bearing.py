...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/ollin/movement_analyzers/bearing.py
...
class Analyzer(MovementAnalyzer):
    ...
    def plot(self, ax=None, figsize=(10, 10), num_individual=0, bins=20, width=None, cmap='Reds', alpha=0.8, log=True):
        ...
        if num_individual == 'all':
            ...
            range_ = (-np.pi, np.pi)
            histogram, _ = np.histogram(bdata, bins=bins, range=range_, normed=True)
            if width is None:
