...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/ollin/core/movement.py
...
from .utils import occupancy_to_density, home_range_to_velocity, velocity_modification
from ..movement_models.base import MovementModel
from ..movement_models import get_movement_model
from ..movement_analyzers import get_movement_analyzer, get_movement_analyzer_list
...
class MovementData(object):
    ...
    def sample(self, num):
        ...
        """
        selection = np.random.choice(np.arange(self.num), size=num)
    ...
    def plot(self, ax=None, figsize=(10, 10), include=None, num=10, steps=1000, mov_cmap='Greens', simplify=None, **kwargs):
        ...
        self.site.plot(include=include, ax=ax, **kwargs)
        if 'trajectories' in include:
            cmap = plt.get_cmap(mov_cmap)
            colors = [ cmap(i) for i in np.linspace(0.8, 0.1, 10) ]
...
class Movement(MovementData):
    ...
    def __init__(self, site, movement_data, movement_model, velocity, home_range=None):
        ...
        super(Movement, self).__init__(site, movement_data, times, home_range=home_range)
    ...
    def simulate(cls, site, days=None, num=None, occupancy=None, home_range=None, velocity=None, parameters=None, movement_model='variable_levy'):
        ...
        if not isinstance(movement_model, MovementModel):
            movement_model = get_movement_model(movement_model, parameters=parameters)
        parameters = movement_model.parameters
        if velocity is None:
            if home_range is None:
                msg = 'Arguments velocity or home_range must be provided'
                raise ValueError(msg)
            velocity = home_range_to_velocity(home_range, parameters=parameters['home_range'])
        if num is None:
            ...
            dens = occupancy_to_density(occupancy, home_range_proportion, site.niche_size, parameters=parameters['density'])
            num = int(rangex * rangey * dens)
        if days is None:
            days = GLOBAL_CONSTANTS['days']
        velocity_mod = velocity_modification(site.niche_size, parameters)
        ...
        movement_data = movement_model.generate_movement(initial_positions, site, steps, sim_velocity)
        return cls(site, movement_data, movement_model, velocity, home_range=home_range)
    ...
    def extend(self, days, inplace=True):
        ...
        velocity_mod = velocity_modification(self.site.niche_size, parameters)
        velocity = self.velocity * velocity_mod / steps_per_day
        initial_positions = self.data[:, -1, :]
        new_data = self.movement_model.generate_movement(initial_positions, self.site, steps + 1, velocity)
        data = np.append(self.data, new_data[:, 1:, :], 1)
