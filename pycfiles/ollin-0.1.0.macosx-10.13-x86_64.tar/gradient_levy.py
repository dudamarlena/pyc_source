...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/ollin/movement_models/gradient_levy.py
...
class Model(MovementModel):
    ...
    default_parameters = {'density': {'alpha': 5.5687635749135715, 
                   'density_exp': 0.9094048898627347, 
                   'hr_exp': 0.9442292138564827, 
                   'niche_size_exp': 0.5609397724240494}, 
       'home_range': {'alpha': 102.95793964057235, 
                      'exponent': 1.8355253220927104}, 
       'velocity': {'alpha': -1.2919142788970375, 
                    'beta': 2.3185432601405385}, 
       'movement': {'min_pareto': 1.1, 
                    'max_pareto': 1.9, 
                    'grad_weight': 8.0}}

    def generate_movement(self, initial_positions, site, steps, velocity):
        ...
        mov = self._movement(gradient, heatmap, initial_positions, resolution, velocity, range_, steps, min_exponent, max_exponent, grad_weight)
        return mov

    @staticmethod
    @jit(float64[:, :, :](float64[:, :, :], float64[:, :], float64[:, :], float64, float64, float64[:], int64, float64, float64, float64), nopython=True)
    def _movement(gradient, heatmap, random_positions, resolution, velocity, range_, steps, min_exponent, max_exponent, grad_weight):
        ...
        gradient = gradient[:, :, 0] + complex(0.0, 1.0) * gradient[:, :, 1]
        for k in xrange(steps):
            ...
            for j in xrange(num):
                ...
                magnitude = velocity * (exponent - 1) / (math.pow(1 - magnitude, 1 / exponent) * exponent)
                new_angle = np.angle(grad) + direction / (grad_weight * np.abs(grad) + 1e-10)
                new_direction = (
