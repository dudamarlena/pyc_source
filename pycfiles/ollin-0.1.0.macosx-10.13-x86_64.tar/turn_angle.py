...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/ollin/movement_analyzers/turn_angle.py
...
class Analyzer(MovementAnalyzer):
    ...
    def analyze(self, movement):
        ...
        complex_directions = directions[:, :, 0] + complex(0.0, 1.0) * directions[:, :, 1]
        turn_angles = np.angle(complex_directions[:, 1:] / complex_directions[:, :-1])
        return turn_angles

    def plot(self, ax=None, figsize=(10, 10), num_individual=0, bins=20, width=None, cmap='Reds', alpha=0.8, log=True):
        ...
        if num_individual == 'all':
            ...
            if isinstance(num_individual, int):
                tdata = self.results[num_individual, :]
            else:
                tdata = self.results[num_individual, :].ravel()
            range_ = (-np.pi, np.pi)
            histogram, _ = np.histogram(tdata, bins=bins, range=range_, normed=True)
            if width is None:
            ...
            for value, pbar in zip(histogram, bars):
                pbar.set_facecolor(cmap((value - mins + 0.1) / (0.1 + maxs - mins)))
