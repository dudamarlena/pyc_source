...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/ollin/core/sites.py
...
class BaseSite(object):
    ...
    def plot(self, ax=None, figsize=(10, 10), include=None, niche_cmap='Reds', niche_alpha=1.0, boundary_color='black', **kwargs):
        ...
        if include is None:
            include = [
             'rectangle', 'niche_boundary', 'niche']
        if ax is None:
            fig, ax = plt.subplots(figsize=figsize)
        if 'niche_boundary' in include or 'niche' in include:
            sizex, sizey = self.niche.shape
            rangex, rangey = np.meshgrid(np.linspace(0, self.range[0], sizex), np.linspace(0, self.range[1], sizey))
            if 'niche_boundary' in include:
                zone = self.get_true_niche(self.niche)
                ax.contour(rangex, rangey, zone.T, levels=0.5, colors=boundary_color)
            if 'niche' in include:
                ax.pcolormesh(rangex, rangey, self.niche.T, cmap=niche_cmap, alpha=niche_alpha)
...
class Site(BaseSite):
    ...
    def __init__(self, range, points, resolution=0.1, kde_bandwidth=0.3, max_niche_value=1):
    ...
    def sample(self, num):
        ...
        points = np.maximum(np.minimum(points, self.range), [
         0, 0])
        return points
    ...
    def make_niche_from_kde(kde, range, resolution=1.0):
        ...
        ycoords, xcoords = np.meshgrid(np.linspace(0, range[1], num_sides_y, endpoint=False), np.linspace(0, range[0], num_sides_x, endpoint=False))
        points = np.stack([
         xcoords.ravel() + shift_x, ycoords.ravel() + shift_y], 0)
        niche = kde(points).reshape([num_sides_x, num_sides_y])
        return niche

    def plot(self, ax=None, figsize=(10, 10), include=None, points_color='red', **kwargs):
    ...
    @classmethod
    def make_random(cls, niche_size, resolution=None, range=None, min_clusters=None, max_clusters=None, min_cluster_points=None, max_cluster_points=None, max_niche_value=1):
        """Make random site.
        ...
        if isinstance(range, (int, float)):
            ...
            if len(range) == 1:
                range = [
                 range[0], range[0]]
            range = np.array(range)
        points = _make_random_points(range, min_clusters, max_clusters, min_cluster_points, max_cluster_points)
        bandwidth = _select_bandwidth(range, points, niche_size, resolution)
        site = cls(range, points, resolution=resolution, kde_bandwidth=bandwidth, max_niche_value=max_niche_value)
        return site


def _make_random_points(range, min_clusters, max_clusters, min_cluster_points, max_cluster_points):
    ...
    for k in xrange(n_clusters):
        n_neighbors = np.random.randint(min_cluster_points, max_cluster_points)
        centered_points = np.random.normal(size=[n_neighbors, 2])
        variances = np.random.normal(size=[2, 2])
        sheared_points = np.tensordot(centered_points, variances, (1, 1))
