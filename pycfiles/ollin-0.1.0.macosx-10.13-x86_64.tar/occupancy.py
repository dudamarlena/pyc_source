...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/ollin/core/occupancy.py
...
class Occupancy(object):
    ...
    def plot(self, ax=None, figsize=(10, 10), include=None, occupancy_cmap='Blues', occupancy_level=0.2, occupancy_alpha=0.3, **kwargs):
        ...
        if 'occupancy' in include:
            ...
            h, w = grid.shape
            xcoord, ycoord = np.meshgrid(np.linspace(0, range_[0], h), np.linspace(0, range_[1], w))
            cm = ax.pcolormesh(xcoord, ycoord, grid.T, cmap=occupancy_cmap, alpha=occupancy_alpha, vmax=1.0, vmin=0.0)
    ...
    def plot_occupancy_timeseries(self, ax=None, figsize=(10, 10), color='red', label=''):
