...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/ollin/core/utils.py
...
def occupancy_to_density(occupancy, home_range_proportion, niche_size, parameters=None):
    ...
    density = np.exp((logit(occupancy) - alpha - np.log(home_range_proportion) * hr_exp - np.log(niche_size) * nsz_exp) / den_exp)
    return density


def density_to_occupancy(density, home_range_proportion, niche_size, parameters=None):
    ...
    den_exp = parameters['density_exp']
    nsz_exp = parameters['niche_size_exp']
    occupancy = sigmoid(alpha + np.log(density) * den_exp + np.log(home_range_proportion) * hr_exp + np.log(niche_size) * nsz_exp)
    return occupancy
