...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/ollin/calibration/calibrate.py
...
def calibrate(model, config=None, save_fig=False, save_path=None, plot_style='fivethirtyeight'):
    ...
    if save_fig:
        ...
        with plt.style.context(plot_style):
            ...
            path = os.path.join(save_path, 'occupancy_calibration_density.png')
            ax.get_figure().savefig(path, frameon=True)
            ax = oc_calibrator.plot(x_var='home_range')
            path = os.path.join(save_path, 'occupancy_calibration_home_range.png')
            ax.get_figure().savefig(path, frameon=True)
            ax = oc_calibrator.plot(x_var='niche_sizes')
            path = os.path.join(save_path, 'occupancy_calibration_niche_size.png')
            ax.get_figure().savefig(path, frameon=True)
    return (
