...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/ollin/movement_models/variable_levy.py
...
class Model(MovementModel):
    ...
    default_parameters = {'movement': {'min_pareto': 1.1, 
                    'max_pareto': 1.9}, 
       'density': {'alpha': 5.496771005099745, 
                   'density_exp': 0.9089427578933271, 
                   'hr_exp': 0.9313289770069533, 
                   'niche_size_exp': 0.552259555326986}, 
       'home_range': {'alpha': 84.94331014956782, 
                      'exponent': 1.866467806903153}, 
       'velocity': {'alpha': -3.1454639316707027, 
    ...
    def generate_movement(self, initial_positions, site, steps, velocity):
        ...
        range_ = site.range
        mov = self._movement(heatmap, initial_positions, resolution, velocity, range_, steps, min_exponent, max_exponent)
        return mov

    @staticmethod
    @jit(float64[:, :, :](float64[:, :], float64[:, :], float64, float64, float64[:], int64, float64, float64), nopython=True)
    def _movement(heatmap, random_positions, resolution, velocity, range_, steps, min_exponent, max_exponent):
        ...
        for k in xrange(steps):
            ...
            for j in xrange(num):
                ...
                magnitude = velocity * (exponent - 1) / (math.pow(1 - np.random.rand(), 1 / exponent) * exponent)
