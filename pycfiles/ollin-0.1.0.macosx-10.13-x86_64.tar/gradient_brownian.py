...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/ollin/movement_models/gradient_brownian.py
...
class Model(MovementModel):
    ...
    default_parameters = {'movement': {'grad_weight': 10.0, 
                    'niche_weight': 1.0}, 
       'density': {'alpha': 5.054484212556172, 
                   'density_exp': 0.8546502084649101, 
                   'hr_exp': 0.8556172207468531, 
                   'niche_size_exp': 0.9376571357760055}, 
       'home_range': {'alpha': 37.23282280460764, 
                      'exponent': 1.8750683470837832}, 
       'velocity': {'alpha': 0.6601881286894935, 
    ...
    def generate_movement(self, initial_positions, site, steps, velocity):
        ...
        mov = self._movement(gradient, heatmap, initial_positions, resolution, velocity, range_, steps, grad_weight, niche_weight)
        return mov

    @staticmethod
    @jit(float64[:, :, :](float64[:, :, :], float64[:, :], float64[:, :], float64, float64, float64[:], int64, float64, float64), nopython=True)
    def _movement(gradient, heatmap, random_positions, resolution, velocity, range_, steps, grad_weight, niche_weight):
