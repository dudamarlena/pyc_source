...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/ollin/core/home_range.py
...
class HomeRange(object):
    ...
    def plot(self, ax=None, figsize=(10, 10), include=None, n_individual=0, hr_cmap='Blues', **kwargs):
        """Plot home range.
        ...
        if 'home_range' in include:
            ...
            rangex, rangey = np.meshgrid(np.linspace(0, range_[0], sizex), np.linspace(0, range_[1], sizey))
...
def make_grid(array, range, resolution):
    ...
    nums = np.linspace(0, num, num * steps, endpoint=False).astype(np.int).reshape([-1, 1])
