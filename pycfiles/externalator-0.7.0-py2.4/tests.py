...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/externalator/tests/tests.py
# Compiled at: 2008-02-05 15:23:13
"""
...
"""
commands = [
 'add', 'checkout', 'delete', 'diff', 'freeze', 'get', 'help', 'list', 'merge', 'set', 'update']
...
parser.add_option('--no-clean', dest='clean', default=True, action='store_false', help="don't remove the repositories on success")
...
extcall('-m', 'adding test svn external', 'add', '%s=%s' % (svn_external_name, svn_external), checkout_name)
...
extcall('-m', 'adding test cvs external', 'add', '--cvs=%s' % CVSROOT, '%s=%s' % (cvs_external_name, cvs_external), checkout_name)
tgz_external = 'http://www.dieter.handshake.de/pyprojects/zope/AdvancedQuery.tgz'
extcall('-m', 'adding test tgz external', 'add', '--tgz', tgz_external, checkout_name)
...
oldrevision = re.search('[0-9]+', [ i for i in re.findall('\nr[0-9]+ |', log) if i ][(-1)]).group()
assert oldrevision != newrevision
extcall('-m', 'pegging to an old revision', 'set', '%s=%s' % (svn_external_name, 'r' + oldrevision), checkout_name)
shutil.rmtree(testdir)
extcall('checkout', repo_path, testdir)
...
extcall('-m', 'adding test svn external', 'add', '%s=r%s:%s' % (svn_external_name, oldrevision, svn_external), newdirs[0])
extcall('-m', 'adding test cvs external', 'add', '--cvs=%s' % CVSROOT, '%s=%s' % (cvs_external_name, cvs_external), newdirs[0])
extcall('-m', 'adding test tgz external', 'add', '--tgz', tgz_external, newdirs[0])
new_svn_external = 'https://svn.openplans.org/svn/standalone/contactthem'
new_svn_external_name = new_svn_external.split('/')[(-1)]
extcall('-m', 'adding newer version svn external', 'add', '%s=r%s:%s' % (svn_external_name, newrevision, svn_external), newdirs[1])
extcall('-m', 'a different svn external', 'add', '%s=%s' % (new_svn_external_name, new_svn_external), newdirs[1])
