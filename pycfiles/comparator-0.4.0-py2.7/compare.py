...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/comparator/compare.py
# Compiled at: 2019-03-09 01:02:11
"""
    Base classes for running comparisons between two data sources
"""
import copy, inspect, logging, re, six
            ...
            ...will be automatically formatted  to become...
                ...
                SELECT *
                 ...
                 WHERE uuid IN ('uuid_1', 'uuid_2', 'uuid_3')
                   AND id NOT IN (1, 2, 3)
        """
        qfmt = re.compile('\\{\\{[\\s]?[a-zA-Z0-9\\_]+[\\s]?\\}\\}')
    ...
    def compare(self):
        ...
        if not self._complete:
            for comp in self._comps:
                ...
                result = ComparatorResult(self._name, name, comp(*self._sp.query_results))
...
class ComparatorSet(object):
    ...
    def __init__(self, source_pairs, comps=None, names=None, default_comp=None):
        ...
        self._comparisons = [ Comparator(sp=sp, comps=c, name=n) for sp, c, n in zip(self._source_pairs, self._comps, self._names)
                            ]
    ...
    def _set_source_pairs(self, source_pairs):
        ...
        for sp in source_pairs:
            if not isinstance(sp, SourcePair):
                raise InvalidCompSetException('Each source pair must be a valid SourcePair object. Problem at : %r' % sp)
            ...
            for comp in comp_list:
                if not callable(comp):
                    if COMPS.get(comp, None) is None:
                        raise InvalidCompSetException('Each comp must be a callable. Problem at : %r' % comp)

        if len(self._source_pairs) != len(comps):
            raise InvalidCompSetException('Queries and comparison mapping is mismatched. There are %d source pairs and %d comparisons' % (
    ...
    def _set_names(self, names):
        ...
        if len(self._source_pairs) != len(names):
            raise InvalidCompSetException('Queries and name mapping is mismatched. There are %d source pairs and %d names' % (
    ...
    def from_dict(cls, dict_or_dicts, left=None, right=None, default_comp=None):
        ...
        for d in dict_or_dicts:
            if not isinstance(d, dict) or d.get('lquery') is None and d.get('sp') is None:
                raise InvalidCompSetException('Each value must be a dict with lquery or query pair. Problem with : %r' % d)
