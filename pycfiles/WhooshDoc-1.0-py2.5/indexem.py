...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/whooshdoc/indexem.py
# Compiled at: 2009-02-26 19:57:18
...
indexed_docs = (
 apidoc.ClassDoc, apidoc.ClassMethodDoc, apidoc.ModuleDoc,
...
def traverse(docindex, packages, **filters):
    ...
    while apidoc_queue:
        ...
        if isinstance(the_value, indexed_docs) and in_packages(the_value.canonical_name, packages):
        ...
        apidoc_queue.extend([ v for v in api_doc.apidoc_links(**filters) if v not in val_set and v not in var_set
                            ])

    return (
     accepted_values, variables)
    ...
    if isinstance(value, apidoc.ModuleDoc):
        ...
        for var in variables:
            ...
            if module.is_package is apidoc.UNKNOWN:
                continue
            elif module.is_package and mod_name.dominates(canonical_name) or mod_name[(-1)] == 'api' and mod_name.container().dominates(canonical_name):
...
def as_document(value, variables):
    ...
    if isinstance(value, apidoc.ModuleDoc):
        ...
        kind = 'class'
    elif value.defining_module is apidoc.UNKNOWN:
        raise ValueError('Value for %s does not have a defining module. Skipping.' % value.canonical_name)
    elif value.canonical_name.container() == value.defining_module.canonical_name:
        kind = 'function'
    else:
        kind = 'method'
    if value.summary is not None:
        if value.summary is not apidoc.UNKNOWN:
        ...
        if value.docstring is not None and value.docstring is not apidoc.UNKNOWN:
            ...
            if isinstance(docstring, str):
                ...
                raise isinstance(docstring, unicode) or TypeError('docstring for %s is a %r : %r' % (best, type(docstring), docstring))
    else:
        docstring = ''
    doc = dict(kind=kind, name=name_keywords([best]), _stored_name=unicode(best), aliases=name_keywords([best] + aliases), _stored_aliases=(' ').join(map(unicode, aliases)), docstring=docstring, summary=summary)
    return doc
...
def get_docstrings(roots, clear_cache=False, exclude_regexes=None):
    ...
    with safe_epydoc():
        ...
        if docindex is None:
            raise RuntimeError('epydoc logged an exception. Could not document something in %r' % (
             roots,))
