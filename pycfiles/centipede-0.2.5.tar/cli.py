...
# Embedded file name: build/bdist.linux-x86_64/egg/centinel/vpn/cli.py
# Compiled at: 2015-09-30 11:20:13
import argparse, logging
from random import shuffle
import os, centinel.backend, centinel.client, centinel.config, centinel.vpn.openvpn as openvpn, centinel.vpn.hma as hma
...
def parse_args():
    ...
    parser.add_argument('--create-hma-configs', dest='create_HMA', action='store_true', help='Create the openvpn config files for HMA')
    parser.add_argument('--shuffle', '-s', dest='shuffle_lists', action='store_true', help='Randomize the order of vantage points')
    ...
    group.add_argument('--directory', '-d', dest='directory', help='Directory with experiments, config files, etc.')
    ...
    group.add_argument('--create-config', '-c', help=create_conf_help, dest='create_conf_dir')
...
def scan_vpns(directory, auth_file, crt_file, exclude_list, shuffle_lists=False):
    ...
    for filename in conf_list:
        ...
        try:
            ...
            logging.exception('%s: Failed to geolocate %s: %s' % (
             filename, vpn_address, exp))
        ...
        vpn = openvpn.OpenVPN(timeout=60, auth_file=auth_file, config_file=vpn_config, crt_file=crt_file)
        vpn.start()
        if not vpn.started:
            vpn.stop()
...
def run():
    ...
    if args.create_conf_dir:
        ...
        scan_vpns(args.directory, args.auth_file, args.crt_file, args.exclude_list, args.shuffle_lists)
