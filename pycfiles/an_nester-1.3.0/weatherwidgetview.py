...
# Embedded file name: build/bdist.linux-i686/egg/anz/dashboard/browser/weatherwidgetview.py
# Compiled at: 2010-09-26 21:53:54
import time, socket, pprint, cjson, feedparser
...
_FeedParserMixin.namespaces['http://xml.weather.yahoo.com/ns/rss/1.0'] = 'yweather'
...
elements = [
 'location', 'units', 'wind', 'atmosphere', 'astronomy', 'condition', 'forecast']
for e in elements:
        ...
        try:
            ...
            if feed.has_key('bozo_exception'):
                ...
                if feed.feed.subtitle != 'Yahoo! Weather Error':
                    info = {'astronomy': feed.feed.yweather_astronomy[0], 'atmosphere': feed.feed.yweather_atmosphere[0], 'location': feed.feed.yweather_location[0], 'units': feed.feed.yweather_units[0], 'wind': feed.feed.yweather_wind[0]}
                    if len(feed.entries):
                        entry = feed.entries[0]
                        info['title'] = entry.title
                        info['link'] = entry.link
                        info['updated'] = toLocalizedTime(entry.updated, long_format=1)
                        info['condition'] = entry.yweather_condition[0]
                        info['forecast'] = entry.yweather_forecast
                    ret['success'] = True
                    ret['msg'] = translate(_('Get weather info success.'), context=self.request)
                    ret['info'] = info
                else:
                    ret['success'] = False
