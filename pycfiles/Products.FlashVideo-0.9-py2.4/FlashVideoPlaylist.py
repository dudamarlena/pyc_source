...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\Products\FlashVideo\content\FlashVideoPlaylist.py
...
FlashVideoPlaylistSchema = ATContentTypeSchema.copy() + Schema((ReferenceField('videos', relationship='videos', multiValued=True, required=0, allowed_types=(FLASHVIDEO_PORTALTYPE,), widget=ReferenceBrowserWidget(label='Videos in playlist', description='Select videos to display inside this playlist', label_msgid='label_video_playlist', description_msgid='help_video_playlist', i18n_domain='plone')),), marshall=PrimaryFieldMarshaller())
...
class FlashVideoPlaylist(ATCTContent):
    ...
    actions = updateActions(ATCTContent, ({'id': 'play', 'name': 'Play', 'action': 'string:${object_url}/flashvideoplaylist_play', 'permissions': (View,)},))
