...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\Products\FlashVideo\content\FlashVideo.py
...
FlashVideoSchema = ATContentTypeSchema.copy() + Schema((FileField('file', required=True, primary=True, languageIndependent=True, storage=STORAGE, validators=(('isNonEmptyFile', V_REQUIRED), 'isFLVFile'), widget=FileWidget(label='Movie File', description='Select Flash movie file (*.flv)', label_msgid='label_movie_file', description_msgid='help_movie_file', i18n_domain='plone')), ImageField('screenshot', required=False, storage=AnnotationStorage(migrate=True), sizes={'mini': (200, 200), 'thumb': (128, 128), 'tile': (64, 64), 'icon': (32, 32)}, validators=(('isNonEmptyFile', V_REQUIRED),), widget=ImageWidget(label='Screenshot', description='Image that will be shown when movie is not playing.                                     Shoud have the same resolution as movie.', label_msgid='label_screenshot', description_msgid='help_screenshot', i18n_domain='plone')), StringField('width', searchable=0, widget=IntegerWidget(label='Movie/screenshot width', description='Enter movie width', label_msgid='label_movie_width', description_msgid='help_movie_width', i18n_domain='plone')), StringField('height', searchable=0, widget=IntegerWidget(label='Movie/screenshot height', description='Enter movie height', label_msgid='label_movie_height', description_msgid='help_movie_height', i18n_domain='plone'))))
...
class FlashVideo(ATFile):
    ...
    def getConfigString(self):
        ...
        if self.hasScreenshot():
            image_str = "{url: '%s/screenshot', overlayId: 'play'}, " % self.absolute_url()
        else:
            image_str = ''
        file_str = "{url: '%s', type: 'flv'}" % self.absolute_url()
        config_str = "{playList: [%s%s], autoRewind: true,             initialScale: 'fit', loop: false, autoPlay: false}" % (image_str, file_str)
        return config_str
