...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pywebqq/webqq.py
# Compiled at: 2014-07-25 04:30:57
"""
使用WEBQQ3.0协议
"""
import redis, struct, requests, subprocess, traceback, qqsetting, gevent, greenlet, random, json, time, os, re, logging, logging.config, urllib, cookielib
...
class MessageHandner(object):
    ...
    def __joinmessage(self, message):
        messagebody = ('').join(map(lambda item: ':face' + str(item[1]) + ': ' if isinstance(item, list) else item, message))
    ...
    def on_message(self, message):
        ...
        for msg in mess:
            if isinstance(msg, list):
                ...
                if msgtype == 'offpic':
                    ...
                    self.context.spawn(picpath, str(message['from_uin']), task=self.context.downoffpic)
                elif msgtype == 'cface':
        ...
        notifyer.notify(('').join(messagebody).encode('utf-8'), title=fromwho, timeout=5, icon=faceuri)
        ...
        for msg in mess:
            if isinstance(msg, list):
                ...
                if msgtype == 'cface':
                    ...
                    self.context.spawn(str(gid), str(uin), pichost, hostport, str(fid), filename, vfwebqq, task=self.__downgrppic)
    ...
    def on_file_message(self, message):
        ...
        if message['mode'] == 'recv':
            ...
            self.context.spawn(lcid, to, guid, filename, task=self.context.recvfile, linkok=self.on_end_transfile)
    ...
    def on_push_offfile(self, message):
        ...
        self.context.spawn(downurl, filename, task=self.__downofflinefile)
...
class QQMessage(object):
    ...
    def send(self, context, clientid, psessionid):
        ...
        return context.spawn(self.url, qqrawmsg, task=context.sendpost, linkok=self.sendOk, linkfailed=self.sendFailed)
...
class ImageMessage(QQMessage):
    ...
    def uploadpic(self):
        ...
        formdata = {'skey': self.context.skey, 
        ...
        self.formdata = (' ').join("--form-string '%s=%s'" % (k, str(v)) for k, v in formdata.iteritems())
...
class GroupMessage(QQMessage):
    ...
    def encode(self, clientid, psessionid):
        ...
        r = json.dumps({'group_uin': groupuin, 
           'content': content, 
           'msg_id': MessageIndex.get(), 
           'clientid': clientid, 
           'psessionid': psessionid})
...
class ShakeMessage(QQMessage):
    ...
    def send(self, context, clientid, psessionid):
        return context.spawn('http://d.web2.qq.com/channel/shake2?to_uin=%s&clientid=%s&psessionid=%s&t=%s' % (self.to, clientid, psessionid, ctime()), task=context.sendget, linkfailed=self.sendFailed)
...
class KeepaliveMessage(QQMessage):
    ...
    def send(self, context):
        return context.spawn('http://webqq.qq.com/web2/get_msg_tip?uin=&tp=1&id=0&retype=1&rc=2&lv=3&t=%s' % ctime(), task=context.sendget, linkfailed=self.sendFailed)
...
class LogoutMessage(QQMessage):
    ...
    def send(self, context, clientid, psessionid):
        return context.spawn('http://d.web2.qq.com/channel/logout2?ids=&clientid=%s&psessionid=%s&t=%s' % (clientid, psessionid, ctime()), task=context.sendget)
...
class WebQQ(object):
    ...
    def __init__(self, qqno, qqpwd, handler=None):
        ...
        self.headers = {'User-Agent': 'Mozilla/5.0 (X11; Linux i686; rv:16.0) Gecko/20100101 Firefox/16.0', 
    ...
    def get_password(self):
        ...
        return md5(md5(md5(self.qqpwd).digest() + hexchar2bin(self.vcode2)).hexdigest().upper() + self.vcode.upper()).hexdigest().upper()
    ...
    def build_groupinfo(self):
        response = self.sendpost('http://s.web2.qq.com/api/get_group_name_list_mask2', 'r=' + urllib.quote(json.dumps({'hash': self.get_hash, 'vfwebqq': self.vfwebqq})), {'Referer': 'http://s.web2.qq.com/proxy.html?v=20110412001&callback=1&id=3'})
        ...
        for friendindex, group in enumerate(grouplist):
            ...
            groupinfo = self.sendget('http://s.web2.qq.com/api/get_group_info_ext2?gcode=%s&vfwebqq=%s&t=%s' % (group['code'], self.vfwebqq, ctime()), headers={'Referer': 'http://s.web2.qq.com/proxy.html?v=20110412001&callback=1&id=1'})
            try:
    ...
    def login2(self):
        rdict = json.dumps({'status': 'online', 
        ...
        try:
            with gevent.Timeout(30):
                response = self.sendpost('http://d.web2.qq.com/channel/login2', 'r=%s&clientid=%s&psessionid=null' % (urllib.quote(rdict), self.clientid), headerdict={'Referer': 'http://d.web2.qq.com/proxy.html?v=20110331002&callback=1&id=3'})
                if response['retcode'] != 0:
    ...
    def get_friends(self):
        r = json.dumps({'h': 'hello', 
           'hash': self.get_hash, 
           'vfwebqq': self.vfwebqq})
        self.friends = self.sendpost('http://s.web2.qq.com/api/get_user_friends2', 'r=%s' % urllib.quote(r), headerdict={'Referer': 'http://s.web2.qq.com/proxy.html?v=20110412001&callback=1&id=1'})
        self.build_userinfo()
    ...
    def recvfile(self, lcid, to, guid, filename):
        recvonlineurl = 'http://d.web2.qq.com/channel/get_file2?lcid=' + lcid + '&guid=' + guid + '&to=' + to + '&psessionid=' + self.psessionid + '&count=1&time=1349864752791&clientid=' + self.clientid
        ...
        retcode, wgethandler = processopen(cmd % (
    ...
    def downcface(self, to, guid):
        ...
        getcfaceurl = 'http://d.web2.qq.com/channel/get_cface2?lcid=' + lcid + '&guid=' + guid + '&to=' + to + '&count=5&time=1&clientid=' + self.clientid + '&psessionid=' + self.psessionid

        def sendrequest():
            response = ''
            try:
                response = self.session.get(getcfaceurl, timeout=300).text
                try:
    ...
    def getqqnumber(self, uin):
        ...
        if qqnumber:
            ...
            try:
                response = self.sendget('http://s.web2.qq.com/api/get_friend_uin2?tuin=%s&verifysession=&type=1&code=&vfwebqq=%s&t=%s' % (uin, self.vfwebqq, str(time.time())), headers={'Referer': 'http://s.web2.qq.com/proxy.html?v=20110412001&callback=1&id=1'})
                if response['retcode'] == 0:
    ...
    def downoffpic(self, url, fromuin):
        getoffpicurl = 'http://d.web2.qq.com/channel/get_offpic2?file_path=' + urllib.quote(url) + '&f_uin=' + fromuin + '&clientid=' + self.clientid + '&psessionid=' + self.psessionid
        try:
    ...
    def poll_message(self):
        ...
        rdict = json.dumps({'clientid': self.clientid, 
