...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\chitwanabm\statistics.py
# Compiled at: 2013-01-28 01:41:46
"""
...
from pyabm.statistics import convert_probability_units, get_probability_index, draw_from_prob_dist, calc_prob_from_prob_dist, UnitsError, StatisticsError
...
def choose_spouse(person, eligible_mates):
    ...
    for eligible_mate in eligible_mates:
        ...
        if person.get_sex() == eligible_mate.get_sex() or person.get_ethnicity() != eligible_mate.get_ethnicity() or person.is_sibling(eligible_mate):
            sp_probs.append(0)
        else:
...
def calc_fuelwood_usage_probability(household, time):
    ...
    neighborhood = household.get_parent_agent()
    inner += rcParams['fw_usageprob.coef.elecavail'] * neighborhood._elec_available
    inner += rcParams['fw_usageprob.coef.distnara_km'] * neighborhood._distnara
...
def calc_education_level(person):
    ...
    prob_y_gte_j = np.zeros(len(levels) - 1)
    for n in np.arange(len(prob_y_gte_j)):
        ...
        neighborhood = person.get_parent_agent().get_parent_agent()
        xb_sum += rcParams['education.coef.avg_yrs_services_lt15'] * neighborhood._avg_yrs_services_lt15
        prob_y_gte_j[n] = 1.0 / (1 + np.exp(-(intercept + xb_sum)))

    prob_y_eq_j = np.zeros(4)
