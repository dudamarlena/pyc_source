...
# Compiled at: 2020-04-23 02:42:22
# Size of source mod 2**32: 3616 bytes
import numpy as np
from . import lilcom_extension

def compress(input, tick_power=-8, do_regression=True):
    """
  ...
  """
    ...
    if not (n_dim > 0 and n_dim < 16):
        raise ValueError('Expected number of axes to be in [1,15], got: ', n_dim)
    input = input.astype(np.float32)
    coeffs = regress_array(input, do_regression)
    int_coeffs = [round(x * 256) for x in coeffs]
    meta = [
     tick_power] + int_coeffs
    ans = lilcom_extension.compress_float(input, meta)
    if not isinstance(ans, bytes):
        raise RuntimeError('Something went wrong in compression, return value was ', ans)
    return ans
...
def regress_array(input, regression):
  ...
  """
    ...
    if not regression:
        return coeffs
    tot_sumsq = np.dot(input.reshape(-1), input.reshape(-1)) + 1e-20
    for axis in range(len(input.shape)):
        if input.shape[axis] == 1:
            continue
        else:
            input = np.swapaxes(input, 0, axis)
            coeff = (input[:-1] * input[1:]).sum() / ((input[:-1] ** 2).sum() + 1e-20)
            if abs(coeff) < 0.02:
                coeff = 0.0
            else:
                if coeff < -1.0:
...
def decompress(byte_string):
   ...
   Return:
    ...
    if not isinstance(byte_string, bytes):
        ...
        if shape is None:
            raise ValueError('Could not work out shape of array from input: is not really compressed data?')
        ans = np.empty(shape, dtype=(np.float32))
        ret = lilcom_extension.decompress_float(byte_string, ans)
        if ret is None or ret != 0:
            raise ValueError('Something went wrong in decompression (likely bad data): decompress_float returned {}'.format(ret))
        else:
            return ans