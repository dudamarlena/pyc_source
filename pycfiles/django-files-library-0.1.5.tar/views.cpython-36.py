...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/site-packages/django_files_library/views.py
    ...
    if library.user_can_write(request.user):
        ...
        if form.is_valid():
            ...
            if request.is_ajax():
                ...
                files_list_html = render_library_list({'request':request, 
                 'next_page':next_page}, library)
...
def get_add_form_class():
    ...
    if not app_settings.ADD_FILE_FORM_CLASS:
        ...
        try:
            ...
            raise exceptions.ImproperlyConfigured('%s does not point to a form class' % app_settings.ADD_FILE_FORM_CLASS)

        try:
            mod = import_module(fc_module)
        except ImportError as e:
            raise exceptions.ImproperlyConfigured('Error importing form class %s: "%s"' % (
             fc_module, e))
