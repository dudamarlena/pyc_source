...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/lpo/test/testexpressionset.py
# Compiled at: 2008-07-31 16:05:57
import testbase, unittest, sqlalchemy as sa
...
class ExprSetTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.r1 = es.add_rule(ru.Rule(prems=(ex.expr('esun(e=X1, c=X2)'), ex.expr('es(s=X2, c=X3)')), cons=(ex.expr('esun(e=X1, c=X3)'),)))
        self.r2 = es.add_rule(ru.Rule(prems=(ex.expr('es(s=X1, c=X2)'), ex.expr('es(s=X2, c=X3)')), cons=(ex.expr('es(s=X1, c=X3)'),)))
    ...
    def testExprSetReprRule(self):
        self.assertEquals(self.r1.name, 'es(c=X3,s=X2);esun(c=X2,e=X1)-->esun(c=X3,e=X1)')
    ...
    def testExprSetAskTruth(self):
        self.assertEquals(repr(es.ask('esun(e=juan, c=hombre)')), 'esun(c=hombre, e=juan)')
        self.assertEquals(repr(es.ask('esun(e=julio, c=cosa)')), 'False')
    ...
    def testExprSetAskTruthExtended(self):
        es.extend()
        self.assertEquals(repr(es.ask('esun(e=juan, c=animal)')), 'esun(c=animal, e=juan)')
        self.assertEquals(repr(es.ask('esun(e=juan, c=cosa)')), 'esun(c=cosa, e=juan)')
        self.assertEquals(repr(es.ask('esun(e=julio, c=cosa)')), 'False')
    ...
    def testExprSetDelete(self):
        ...
        self.assertEquals(repr(es.ask('esun(e=juan, c=cosa)')), 'False')
...
class ExprSetArithTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.r1 = es.add_rule(ru.Rule(prems=(ex.expr('esun(e=X1, c=X2)'),), cons=(ex.expr('sobra(q=X1, c=dif(a1=X2, a2=2))'),)))
        self.r1 = es.add_rule(ru.Rule(prems=(ex.expr('esun(e=X1, c=X2)'),), cons=(ex.expr('lt(a1=X2, a2=X1)'),)))
    ...
    def testExprSetBasicSum(self):
        es.extend()
        self.assertEquals(repr(es.ask('sobra(q=8, c=3)')), 'sobra(c=3, q=8)')
