...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/lpo/expression.py
...
class Expression(object):
    ...
    def __init__(self, symbol, **kwargs):
        ...
        if isinstance(symbol, str) and symbol.startswith('~'):
            self.true = False
            symbol = symbol[1:]
        assert isinstance(symbol, str) or isnumber(symbol) and not kwargs
        if isnumber(symbol):
    ...
    def covers(self, expression, substitutions=None):
        ...
        if substitutions is None:
            substitutions = {}
        if self.true != expression.true or self.arg_name != expression.arg_name or self.depth != expression.depth:
            return False
        else:
            if is_variable(self):
                try:
                    ...
                    if subst == expression:
                        return substitutions
                    return False
            if self.symbol != expression.symbol or self.arity != expression.arity:
                return False
...
def solve(expression, ops=preds):
    op = '%s%s%s' % (expression.args['a1'].symbol, ops[expression.symbol], expression.args['a2'].symbol)
    try:
        result = eval(op)
    except NameError:
        return expression
...
orm.mapper(Expression, ta.expressions, allow_column_override=True, properties={'args': orm.relation(Expression, collection_class=ArgList, cascade='all', post_update=True, backref='parent')})
...
def expr(s):
    ...
    if isnumber(s):
        return Expression(s)
    s = re.sub('(?<=[(,=])\\s*(?!False|True|Expr)([a-zA-Z0-9_.~]+)\\s*(?=[),(])', 'Expr("\\1")', s)
    s = re.sub('^\\s*([a-zA-Z0-9_.~]+)', 'Expr("\\1")', s)
...
def is_variable(x):
    ...
    return isinstance(x, Expression) and not x.args and is_var_symbol(x.symbol)
