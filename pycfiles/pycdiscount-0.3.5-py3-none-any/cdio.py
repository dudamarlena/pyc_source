...
# Embedded file name: build/bdist.linux-x86_64/egg/cdio.py
# Compiled at: 2019-08-25 07:37:10
__doc__ = 'The CD Input and Control library (pycdio) encapsulates CD-ROM\nreading and control. Applications wishing to be oblivious of the OS-\nand device-dependent properties of a CD-ROM can use this library.'
import pycdio, _pycdio, sys, types
...
drivers = {'Unknown': pycdio.DRIVER_UNKNOWN, 
...
read_mode2blocksize = {pycdio.READ_MODE_AUDIO: pycdio.CD_FRAMESIZE_RAW, 
...
def have_driver(driver_id):
    ...
    if type(driver_id) in check_types and driver_id in drivers:
...
class Track():
    ...
    def get_preemphasis(self):
        ...
        if rc == pycdio.TRACK_FLAG_FALSE:
            return 'none'
        if rc == pycdio.TRACK_FLAG_TRUE:
            return 'preemphasis'
        if rc == pycdio.TRACK_FLAG_UNKNOWN:
            return 'unknown'
