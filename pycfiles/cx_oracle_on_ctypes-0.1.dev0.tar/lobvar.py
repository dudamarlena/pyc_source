...
# [GCC 8.4.0]
# Embedded file name: /home/lameiro/projects/cx_oracle_on_ctypes/cx_Oracle/lobvar.py
...
class BaseLobVariableType(VariableType):
    ...
    def pre_fetch(self, var):
        ...
        for i in xrange(var.allocelems):
            if typed_data[i]:
                ...
                status = oci.OCILobIsTemporary(var.environment.handle, var.environment.error_handle, typed_data[i], byref(c_is_temporary))
                ...
                if is_temporary and var.connection.handle:
                    status = oci.OCILobFreeTemporary(var.connection.handle, var.environment.error_handle, typed_data[i])
                    var.environment.check_for_error(status, 'LobVar_PreFetch(): free temporary LOB')

    def set_value(self, var, pos, value):
        typed_data = self.get_typed_data(var)
        c_is_temporary = ctypes.c_int()
        status = oci.OCILobIsTemporary(var.environment.handle, var.environment.error_handle, typed_data[pos], byref(c_is_temporary))
        ...
        if not is_temporary:
            ...
            status = oci.OCILobCreateTemporary(var.connection.handle, var.environment.error_handle, typed_data[pos], oci.OCI_DEFAULT, var.type.charset_form, lob_type, 0, oci.OCI_DURATION_SESSION)
            var.environment.check_for_error(status, 'LobVar_SetValue(): create temporary')
        status = oci.OCILobTrim(var.connection.handle, var.environment.error_handle, typed_data[pos], 0)
    ...
    def _write(self, var, pos, data_obj, offset):
        ...
        if var_type == vt_BLOB:
            ...
            if python3_or_better() or var_type == vt_NCLOB:
        ...
        status = oci.OCILobWrite(var.connection.handle, var.environment.error_handle, typed_data[pos], byref(c_amount), offset, buffer.ptr, buffer.size, oci.OCI_ONE_PIECE, None, null_oci_callback, 0, var.type.charset_form)
        var.environment.check_for_error(status, 'LobVar_Write()')
        amount = c_amount.value
        return amount
