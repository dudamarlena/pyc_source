...
# [GCC 8.4.0]
# Embedded file name: /home/lameiro/projects/cx_oracle_on_ctypes/cx_Oracle/timestampvar.py
...
class TimestampType(VariableType):
    ...
    def set_value(self, var, pos, value):
        ...
        status = oci.OCIDateTimeConstruct(var.environment.handle, var.environment.error_handle, typed_data[pos], value.year, value.month, value.day, value.hour, value.minute, value.second, value.microsecond * 1000, None, 0)
        var.environment.check_for_error(status, 'TimestampVar_SetValue(): create structure')
        c_invalid = oci.uword()
        status = oci.OCIDateTimeCheck(var.environment.handle, var.environment.error_handle, typed_data[pos], byref(c_invalid))
