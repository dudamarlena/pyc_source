...
# [GCC 8.4.0]
# Embedded file name: /home/lameiro/projects/cx_oracle_on_ctypes/cx_Oracle/transforms.py
...
def oracle_timestamp_to_python_date(environment, value):
    ...
    second = oci.ub1()
    fsecond = oci.ub4()
    status = oci.OCIDateTimeGetDate(environment.handle, environment.error_handle, value, byref(year), byref(month), byref(day))
    environment.check_for_error(status, 'OracleTimestampToPythonDate(): date portion')
    status = oci.OCIDateTimeGetTime(environment.handle, environment.error_handle, value, byref(hour), byref(minute), byref(second), byref(fsecond))
...
def oracle_interval_to_python_delta(environment, value):
    ...
    seconds = oci.sb4()
    fseconds = oci.sb4()
    status = oci.OCIIntervalGetDaySecond(environment.handle, environment.error_handle, byref(days), byref(hours), byref(minutes), byref(seconds), byref(fseconds), value)
