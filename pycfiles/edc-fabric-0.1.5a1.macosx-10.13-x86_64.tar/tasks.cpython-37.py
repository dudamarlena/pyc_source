...
# [GCC 8.4.0]
# Embedded file name: /Users/erikvw/.venvs/ambition/lib/python3.7/site-packages/edc_fabric/fabfile/virtualenv/tasks.py
# Compiled at: 2017-04-21 13:52:06
# Size of source mod 2**32: 3841 bytes
import os
...
def install_virtualenv(venv_dir=None):
    ...
    lines = [
     'export WORKON_HOME=$HOME/{path}'.format(path=(venv_dir.replace('~/', ''))),
     'export PROJECT_HOME=$HOME/{path}'.format(path=(env.remote_source_root.replace('~/', ''))),
...
def make_virtualenv(venv_name=None, requirements_file=None):
    ...
    run('mkvirtualenv -p python3 --no-setuptools --no-pip --no-wheel {venv_name}'.format(venv_name=(env.venv_name),
...
def create_venv(venv_name=None, requirements_file=None, work_online=None):
    ...
    if exists(os.path.join(env.venv_dir, venv_name)):
        ...
        with cd(env.venv_dir):
            run('python3.6 -m venv --clear --copies {venv_name} {path}'.format(path=(os.path.join(env.venv_dir, venv_name)),
              venv_name=venv_name),
              warn_only=True)
        text = 'workon () {{ source {activate}; }}'.format(activate=(os.path.join(env.venv_dir, '"$@"', 'bin', 'activate')))
