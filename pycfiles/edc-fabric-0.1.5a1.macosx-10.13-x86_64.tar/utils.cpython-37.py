...
# [GCC 8.4.0]
# Embedded file name: /Users/erikvw/.venvs/ambition/lib/python3.7/site-packages/edc_fabric/fabfile/utils.py
# Compiled at: 2018-07-21 06:22:52
# Size of source mod 2**32: 11430 bytes
import configparser, csv, os, re, time
...
def launch_webserver_macosx():
    ...
    sudo("ps auxww | grep gunicorn | awk '{print $2}' | xargs kill -9", warn_only=True)
...
def put_bash_config():
    ...
    local_copy = os.path.expanduser(os.path.join(env.fabric_config_root, 'conf', bash_config))
...
def check_deviceids(app_name=None):
    ...
    remote_path = os.path.join(env.remote_source_root, env.project_appname, app_name)
    for host, device_id in env.device_ids.items():
        with cd(remote_path):
            if not contains('settings.py', 'DEVICE_ID = {}'.format(device_id)):
                abort(red('{} Incorrect device id. Expected {}.'.format(host, device_id)))
...
def get_hosts(path=None, gpg_filename=None):
    ...
    conf_string = local('cd {path}&&gpg2 --decrypt {gpg_filename}'.format(path=path,
      gpg_filename=gpg_filename),
      capture=True)
    conf_string = conf_string.replace('gpg: WARNING: message was not integrity protected', '\n')
        ...
        for index, row in enumerate(csv_reader):
            if index == 0:
                ...
                if row[0] in env.hosts:
                    host = '{user}@{hostname}:22'.format(user=(env.user or 'django'),
                      hostname=(row[0]))
...
def get_device_ids(hostname_pattern=None):
    ...
    for hostname in env.hosts:
        if hostname not in env.roledefs.get('deployment_hosts') and hostname not in env.roledefs.get('servers', []):
            if not re.match(hostname_pattern, hostname):
                warn(f"Invalid hostname. Cannot determine device ID. Ignoring. Got {hostname}".format(hostname=hostname))
            else:
...
def decrypt_to_config(gpg_filename=None, section=None):
    ...
    conf_string = run('gpg2 --decrypt {gpg_filename}'.format(gpg_filename=gpg_filename))
    conf_string = conf_string.replace('gpg: WARNING: message was not integrity protected', '\n')
    conf_string.split(section)[1]
    config = configparser.RawConfigParser()
    config.read_string('{section}\n{conf_string}\n'.format(section=section,
      conf_string=(conf_string.split(section)[1])))
    return config
...
def test_connection(config_path=None, local_fabric_conf=None, bootstrap_branch=None):
    ...
    """
    bootstrap_env(path=(os.path.expanduser(os.path.join(config_path, 'conf'))),
...
def move_media_folder():
    """Moves media folder out of the source repo.
    """
    old_remote_media = os.path.join(env.remote_source_root, env.project_repo_name, 'media')
    if exists(old_remote_media):
        run(f"rsync -pthrvz --remove-source-files {old_remote_media} ~/",
          warn_only=True)