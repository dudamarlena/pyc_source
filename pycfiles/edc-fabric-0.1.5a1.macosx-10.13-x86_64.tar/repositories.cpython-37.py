...
# [GCC 8.4.0]
# Embedded file name: /Users/erikvw/.venvs/ambition/lib/python3.7/site-packages/edc_fabric/fabfile/repositories.py
# Compiled at: 2017-06-12 07:47:57
...
@task
def get_repo(repo_url=None, repo_name=None, remote_root=None, local_root=None, prompt=None):
...
def clone_repo(repo_url=None, remote_root=None, branch=None, prompt=None):
    ...
    pull_repo(repo_url=repo_url, remote_root=remote_root, branch=branch,
      prompt=prompt)
        ...
        with open(os.path.expanduser(os.path.join(deployment_dir, repo_name, 'requirements.txt')), 'r') as (f):
            ...
            for line in lines:
                if 'botswana-harvard' in line or 'erikvw' in line:
                    ...
                    repo_dir = os.path.expanduser(os.path.join(deployment_dir, repo_name))
                    if not os.path.exists(repo_dir):
                        local('git clone {}'.format(line.split('@')[0].replace('git+', '')))
                    else:
                        with lcd(repo_dir):
                            local('git pull')
...
def read_requirements(remote_root=None, project_repo_url=None):
    ...
    remote_root = remote_root or env.remote_source_root
    repo_name = get_repo_name(project_repo_url)
    path = os.path.expanduser(os.path.join(remote_root, repo_name, 'requirements.txt'))
    fd = StringIO()
    get(path, fd)
    content = fd.getvalue()
