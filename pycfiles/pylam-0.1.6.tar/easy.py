...
# Embedded file name: /Library/Python/2.6/site-packages/PyLAF/easy.py
# Compiled at: 2010-07-26 23:37:20
...
class EasyComponent(Component, EasyPortContainer):
    ...
    def gui(self, master, pltcnf={}, cnf={}, **kw):
        frm = Frame(master)
        if not self.PLOTTER == None:
            self.mkplotter(frm, pltcnf).pack()
        o = EasyGrid(frm, self)
        o.pack()
    ...
    def mkplotter(self, master, pltcnf={}):
        v = self.__class__.PLOTTER(master, pltcnf)
        v.ax.plot()
        v.canvas.show()
