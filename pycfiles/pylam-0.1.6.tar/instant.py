...
# Embedded file name: /Library/Python/2.6/site-packages/PyLAF/instant.py
# Compiled at: 2010-08-18 22:15:45
__doc__ = '\nインスタントPyLAFの試験実装\n'
...
class AvailableApps(dict):
    ...
    def check(self, widget):
        for name in widget.children:
            self.check(widget.children[name])
...
class AvailableComponent(dict):
    ...
    def __init__(self, cwd):
        ...
        mods = {}
        for (key, module) in inspect.getmembers(pylaf, lambda x: inspect.ismodule(x)):
            mods[key] = module
...
class PortListbox(Tkinter.Listbox):
    ...
    def __init__(self, master, **key):
        ...
        self.makeScrolledListbox(Tkinter.Frame(frame), **key)
        self.master.pack()
    ...
    def get(self):
        return self.pdic[self.portvar.get()]
...
class PortCollector(dict):
    ...
    def collect(self, component, prefix='.'):
        ...
        for (key, obj) in m:
            if not key == 'master':
                self.collect(obj, prefix + '%s.' % key)


class ClassListbox(Tkinter.Listbox):
    u"""立ち上げ可能コンポーネントクラスの一覧、App(Equipment)でローンチ
    """

    def __init__(self, master, **key):
        frame = Tkinter.Frame(master)
        self.title = Tkinter.Entry(frame)
        self.title.pack()
    ...
    def launch(self, *args):
        try:
            ...
            if title == '':
                title = self.cmps[selection].__name__
...
class Linker(Tkinter.Toplevel):
    u"""起動したクラスのポートメンバの一覧、ローンチされているイクイップメントの一覧（コンボメニュー）、
    ...
    def __init__(self, master=None, cnf={}, **kw):
        ...
        self.src = src = PortListbox(self, apps=apps)
        src.pack(side=Tkinter.LEFT)
        title = apps.keys()[0]
        ...
        self.tgt = tgt = PortListbox(self, apps=apps)
        tgt.pack(side=Tkinter.LEFT)
