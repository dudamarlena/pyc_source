...
# Embedded file name: /Library/Python/2.6/site-packages/PyLAF/kitchen.py
# Compiled at: 2011-04-20 23:43:48
import weakref, platform, random, uuid, inspect, imp, os, Tkinter, tkFileDialog, ttk, core, gui, widgets, mpl
...
def invoke_kitchen(master=None):
    if master.__class__ == Tkinter.Tk:
        Kitchen(master).pack()
    else:
        Kitchen(Tkinter.Toplevel(master)).pack()


def prepare():
    invoke_kitchen(Tkinter.Tk())
...
gui.DefaultMenu.DEFAULT_ITEMS[0].insert(1, ['add_command', kw(label='Kitchen', command=invoke_kitchen)])
...
class SimpleDialog(Tkinter.Toplevel):
    pass
...
class ComponentChooser(ttk.Frame):
    ...
    def parse_selection(self, selection):
        ...
        self.load_modules(cwd=cwd, *names)
...
class ComponentTree(ttk.Treeview):
    ...
    def newsibling(self):
        ...
        if key == None:
            ...
            cc.master.destroy()
            if cc.selection:
                item = cc.selection(parent)
                self.queue.append(item)
                self.master.newtoplevel(item)
            return
...
class WidgetTree(ttk.Treeview):
    ...
    class Menu(Tkinter.Menu):
        ...
        def __init__(self, master=None, cnf={}, **kw):
            Tkinter.Menu.__init__(self, master, cnf, **kw)
            self.add_command(label='New Toplevel as Console', command=core.CallWrapper(master.newtoplevel))
            self.add_command(label='New Toplevel as Console', command=core.CallWrapper(master.newidptoplevel))
    ...
    def newtoplevel(self, component=None):
        if component == None:
    ...
    def newidptoplevel(self):
        ...
        if component == None:
            ...
            parent = self.widgets[key]
            top = Tkinter.Toplevel(parent)
            gui.Console(top, component, sync=False)
            item = self.insert(key, 'end', text=classname(top), values=[repr(id(top))])
            self.widgets[item] = top
            self.populate(item, top)
            return
...
class CanvasItem:
    ...
    def highlight(self):
        if self.state == CanvasItem.STATE_HIGHLIGHT:
            return
    ...
    def normal(self):
        if self.state == CanvasItem.STATE_NORMAL:
            return
        self.state = CanvasItem.STATE_NORMAL
    ...
    def buttonmotion1(self, e):
        self.lift()
        self.canvas().after_cancel(self.aid)
        if self.canvas().is_move():
            self.move(e)
        elif self.canvas().is_wire():
            self.canvas().dragging_wire(e)
...
class CanvasLine:
    ...
    def move(self):
        ...
        if self.bx == bx and self.by == by and self.ex == ex and self.ey == ey:
            return
        (self.bx, self.by, self.ex, self.ey) = (
         bx, by, ex, ey)
        self.delete()
        self.draw()
...
class Linker(Tkinter.Frame):
    ...
    def __init__(self, master, terminals, cnf={}, **kw):
        ...
        pickera = Tkinter.Entry(self)
        pickera.grid(row=1, column=0)
        Linker.PortPicker(self, pickera, a).grid(row=2, column=0)
        cframe = Tkinter.Frame(self, name='cframe')
        cframe.grid(row=1, column=1)
        ...
        linkviewer = LinkViewer(self)
        linkviewer.grid(row=2, column=1)
        ComponentLabel(self, b).grid(row=0, column=2)
        pickerb = Tkinter.Entry(self)
        pickerb.grid(row=1, column=2)
...
class CollaborationMap(Tkinter.Canvas):
    ...
    def components(self):
        return self.master.components()

    def show_links(self, terminals):
        self.master.show_links(terminals)

    def hide_links(self):
        self.master.hide_links()
    ...
    def is_wire(self):
        return self.mode == CollaborationMap.MODE_WIRE

    def is_move(self):
        return self.mode == CollaborationMap.MODE_MOVE
        ...
        for cid in self.items:
            ...
            if item.iscontain(id):
                if cid == self.bitem:
                    ...
                    self.eitem = cid
                    item.highlight()
            elif not cid == self.bitem:
                item.normal()

        return

    def link(self):
        if self.bitem == None or self.eitem == None:
            return
        else:
    ...
    def refresh(self):
        ...
        def pairs(o, clist):
            ...
            for c in tmpclist:
                flag = False
                for item in o:
                    if item[0] == c and item[1] == last or item[0] == last and item[1] == c:
                        flag = True
        ...
        for pair in linksbycid:
            pair = (
             self.items[pair[0]], self.items[pair[1]])
            flag = False
            for line in self.lines:
                if line.eq(pair):
                    flag = True
...
class Kitchen(Tkinter.Frame):
    ...
    def __init__(self, master, cnf={}, **kw):
        ...
        tab = ttk.Notebook(self, name='tabpane')
        ...
        linkviewer = LinkViewer(self, withlabel=True)
        linkviewer.grid(row=1, column=1)
        self.linkviewer = weakref.ref(linkviewer)
    ...
    def newtoplevel(self, component=None):
        self.children['widgettree'].newtoplevel(component)
    ...
    class MyComponent(core.Component):
        ...
        class Control(gui.Panel):
            ...
            def __init__(self, master=None, cnf={}, **kw):
                ...
                builder.add(Tkinter.Label, text='Entry1').grid()
                builder.add(widgets.Entry, name='entry1').grid()

        def __init__(self, master=None, name=None):
            core.Component.__init__(self, master, name)
            self.entry1 = core.Port(0.0)
            self.entry2 = core.Port(0.0)
            self.entry3 = core.Port(0.0)
            self.entry4 = core.Port(0.0)


    tk = Tkinter.Tk()
    basic = gui.Console(tk, MyComponent(core.Root()))
    basic.master.title('basic sample')
    c = basic.comp()
