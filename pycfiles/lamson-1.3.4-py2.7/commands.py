...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/lamson/commands.py
# Compiled at: 2013-02-25 20:20:54
"""
...
import glob, lamson, os, signal, sys, time, mailbox, email

def log_command(port=8825, host='127.0.0.1', chroot=False, chdir='.', uid=False, gid=False, umask=False, pid='./run/log.pid', FORCE=False, debug=False):
    r"""
...
def send_command(port=8825, host='127.0.0.1', username=False, password=False, ssl=False, starttls=False, debug=1, sender=None, to=None, subject=None, body=None, attach=False):
    r"""
    ...
    if password == False:
        password = None
    relay = server.Relay(host, port=port, username=username, password=password, ssl=ssl, starttls=starttls, debug=debug)
...
def start_command(pid='./run/smtp.pid', FORCE=False, chroot=False, chdir='.', boot='config.boot', uid=False, gid=False, umask=False, debug=False):
    r"""
...
def queue_command(pop=False, get=False, keys=False, remove=False, count=False, clear=False, name='run/queue'):
    """
...
def routes_command(TRAILING=[
 'config.testing'], path=os.getcwd(), test=''):
    """
