...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/lamson/routing.py
# Compiled at: 2013-02-25 20:27:46
"""
...
import re, logging, sys, shelve, threading
...
class RoutingBase(object):
    ...
    def _enqueue_undeliverable(self, message):
        if self.UNDELIVERABLE_QUEUE:
            LOG.debug('Message to %r from %r undeliverable, putting in undeliverable queue (# of recipients: %d).', message.route_to, message.route_from, len(message.route_to))
            self.UNDELIVERABLE_QUEUE.push(message)
        else:
            LOG.debug("Message to %r from %r didn't match any handlers. (# recipients: %d)", message.route_to, message.route_from, len(message.route_to))
    ...
    def call_safely(self, func, message, kwargs):
        ...
        try:
            func(message, **kwargs)
            LOG.debug('Message to %s was handled by %s.%s', message.route_to, func.__module__, func.__name__)
        except SMTPError:
            raise
        except:
...
def assert_lamson_settings--- This code section failed: ---

 L. 537         0  LOAD_GLOBAL           0  'has_lamson_settings'
                3  LOAD_FAST             0  'func'
                6  CALL_FUNCTION_1       1  None
                9  POP_JUMP_IF_TRUE     28  'to 28'
               12  LOAD_ASSERT              AssertionError
               15  LOAD_CONST               'Function %s has not be setup with a @route first.'
               18  LOAD_FAST             0  'func'
               21  LOAD_ATTR             2  '__name__'
               24  BINARY_MODULO    
...
def stateless(func):
    ...
    """
    if not (has_lamson_settings(func) and not lamson_setting(func, 'format')):
        raise AssertionError, 'You must use @stateless AFTER @route or @route_like.'
    attach_lamson_settings(func)
