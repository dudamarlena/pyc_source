...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/lamson/queue.py
# Compiled at: 2013-03-07 10:37:34
"""
...
import hashlib, socket, time, os, errno, logging
...
class Queue(object):
        ...
        for key in self.mbox.iterkeys():
            ...
            if over:
                if self.oversize_dir:
                    ...
                    logging.info('Message key %s over size limit %d, DELETING (set oversize_dir).', key, self.pop_limit)
                    os.unlink(over_name)
            else:
                try:
                    try:
    ...
    def oversize(self, key):
        if self.pop_limit:
            ...
            return (
             os.path.getsize(file_name) > self.pop_limit, file_name)
        else:
            return (
             False, None)
            return