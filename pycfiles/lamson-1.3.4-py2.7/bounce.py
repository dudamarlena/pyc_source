...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/lamson/bounce.py
...
BOUNCE_MATCHERS = {'Action': re.compile('(failed|delayed|delivered|relayed|expanded)', re.IGNORECASE | re.DOTALL), 
   'Content-Description': re.compile('(Notification|Undelivered Message|Delivery Report)', re.IGNORECASE | re.DOTALL), 
   'Diagnostic-Code': re.compile('(.+);\\s*([0-9\\-\\.]+)?\\s*(.*)', re.IGNORECASE | re.DOTALL), 
   'Final-Recipient': re.compile('(.+);\\s*(.*)', re.IGNORECASE | re.DOTALL), 
   'Received': re.compile('(.+)', re.IGNORECASE | re.DOTALL), 
   'Remote-Mta': re.compile('(.+);\\s*(.*)', re.IGNORECASE | re.DOTALL), 
   'Reporting-Mta': re.compile('(.+);\\s*(.*)', re.IGNORECASE | re.DOTALL), 
   'Status': re.compile('([0-9]+)\\.([0-9]+)\\.([0-9]+)', re.IGNORECASE | re.DOTALL)}
BOUNCE_MAX = len(BOUNCE_MATCHERS) * 2.0
PRIMARY_STATUS_CODES = {'1': 'Unknown Status Code 1', 
   '2': 'Success', 
   '3': 'Temporary Failure', 
   '4': 'Persistent Transient Failure', 
   '5': 'Permanent Failure'}
SECONDARY_STATUS_CODES = {'0': 'Other or Undefined Status', 
   '1': 'Addressing Status', 
   '2': 'Mailbox Status', 
...
COMBINED_STATUS_CODES = {'00': 'Not Applicable', 
...
def detect(msg):
    ...
    results = {'Content-Description-Parts': originals['Content-Description-Parts']}
    score = 0
...
class bounce_to(object):
 ...
 L. 287         0  LOAD_FAST             1  'soft'
                3  LOAD_FAST             0  'self'
                6  STORE_ATTR            0  'soft'

 L. 288         9  LOAD_FAST             2  'hard'
               12  LOAD_FAST             0  'self'
               15  STORE_ATTR            1  'hard'

 L. 290        18  LOAD_FAST             0  'self'
               21  LOAD_ATTR             0  'soft'
               24  POP_JUMP_IF_FALSE    36  'to 36'
               27  LOAD_FAST             0  'self'
               30  LOAD_ATTR             1  'hard'
             33_0  COME_FROM            24  '24'
               33  POP_JUMP_IF_TRUE     45  'to 45'
               36  LOAD_ASSERT              AssertionError
               39  LOAD_CONST               'You must give at least soft and/or hard'
               42  RAISE_VARARGS_2       2  None

Parse error at or near `LOAD_CONST' instruction at offset 39
