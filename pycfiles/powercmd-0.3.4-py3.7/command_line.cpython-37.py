...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/powercmd/command_line.py
# Compiled at: 2020-03-22 15:31:26
...
class CommandLine:
    ...
    def __eq__(self, other):
        return (
         self.command, self.args) == (other.command, other.args)
    ...
    def named_args(self) -> Mapping[(str, str)]:
        return {arg.name:arg.value for arg in self.args if isinstance(arg, NamedArg) if isinstance(arg, NamedArg)}
    ...
    def assign_args(self, cmd: Command) -> OrderedMapping[(str, Union[(str, MissingArg)])]:
        """
        ...
        for arg in self.args:
            if isinstance(arg, NamedArg):
                if arg.name in assigned_args:
                    raise InvalidInput('cannot assign named argument to %s: argument already present' % (
                     arg.name,))
    ...
    def get_unassigned_params(self, cmd: Command) -> List[Parameter]:
    ...
    def get_current_arg(self, cmd: Command) -> Optional[IncompleteArg]:
        assigned_args = self.assign_args(cmd)
        last_assigned = None
        ...
        if self.has_trailing_whitespace:
            if first_unassigned is not None:
                return IncompleteArg(param=(cmd.parameters[first_unassigned]), value='')
        elif last_assigned is not None:
            return IncompleteArg(param=(cmd.parameters[last_assigned]), value=(assigned_args[last_assigned]))