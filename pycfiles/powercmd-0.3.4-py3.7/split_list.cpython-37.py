...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/powercmd/split_list.py
# Compiled at: 2020-03-22 13:22:29
...
def _split(text: str, is_separator: Callable[([str], bool)], delimiters: Mapping[(str, str)], allow_unmatched: bool=False, collapse_separators: bool=False) -> List[str]:
    """
...
def split_list(text: str, separator: str=',', allow_unmatched: bool=False) -> List[str]:
    """
...
def split_cmdline(text: str, allow_unmatched: bool=False) -> List[str]:
...
def drop_enclosing_quotes(word: str) -> str:
    if len(word) > 1:
        if word[0] in _QUOTES:
            if word[(-1)] == _QUOTES[word[0]]:
                return word[1:-1]
    return word