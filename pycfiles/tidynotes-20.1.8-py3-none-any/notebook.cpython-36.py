...
# [GCC 8.4.0]
# Embedded file name: d:\dropbox\dropbox\code\tidynotes\src\tidynotes\notebook.py
# Compiled at: 2020-01-20 03:59:12
# Size of source mod 2**32: 13190 bytes
import os, jinja2, datetime, markdown, re, json, hashlib, collections, pkg_resources

class Tidybook:
    resource_map = {'config.json':'', 
    ...
    def is_empty(self, file_path: str, threshold: int=0) -> bool:
        ...
        if threshold < 0:
            ...
            non_empty = [x for x in re.findall('(?i)\n[^#][\\w]+', self.read(file_path)) if x.strip()]
