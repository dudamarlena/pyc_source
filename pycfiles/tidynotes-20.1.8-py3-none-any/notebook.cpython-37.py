...
# [GCC 8.4.0]
# Embedded file name: C:\Users\Joe McGrath\Dropbox\code\markdown_notebook\tidynotes\notebook.py
# Compiled at: 2020-01-17 03:04:28
# Size of source mod 2**32: 12562 bytes
import os, jinja2, datetime, markdown, re, json, hashlib, collections, shutil
...
class notebook:
    ...
    def make_note(self, date=datetime.datetime.today(), force=False):
        ...
        if not (force or os.path.exists(dst_path)):
    ...
    def make_note_series(self, n_steps, start=datetime.datetime.today(), step=datetime.timedelta(days=1), force=False):
    ...
    def make_notebook(self, dst_dir):
        ...
        for template in os.listdir(template_src):
            if not os.path.exists(os.path.join(template_dst, template)):
                shutil.copy(os.path.join(template_src, template), os.path.join(template_dst, template))
    ...
    def note_list(self):
        ...
        """
        files = [os.path.join(dp, f) for dp, _, filenames in os.walk(self.note_path) for f in filenames if os.path.splitext(f)[1].lower() == '.md']
    ...
    def _render_markdown(self, markdown_text) -> str:
        ...
        md = markdown.Markdown(extensions=[
         'fenced_code', 'tables', 'sane_lists', 'admonition'])
    ...
    def _render_markdown_to_file(self, markdown_list, output_name):
        ...
        output = [self._render_markdown(self._preprocess_markdown(x)) for x in markdown_list]
