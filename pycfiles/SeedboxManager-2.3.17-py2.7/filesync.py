...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-1.7.35-i686/egg/seedbox/tasks/filesync.py
...
cfg.CONF.import_group('tasks_filesync', 'seedbox.options')
...
class SyncFile(base.BaseTask):
    ...
    def cmd(self):
        ...
        if self._cmd is None:
            ...
            if cfg.CONF.tasks_filesync.identity or cfg.CONF.tasks_filesync.port:
                ssh_cmd = '--rsh=ssh'
                if cfg.CONF.tasks_filesync.port:
                    ssh_cmd = ssh_cmd + ' -p ' + cfg.CONF.tasks_filesync.port
                if cfg.CONF.tasks_filesync.identity:
                    ssh_cmd = ssh_cmd + ' -i ' + cfg.CONF.tasks_filesync.identity
                self._cmd.append(ssh_cmd)
            self._cmd.append(os.path.join(cfg.CONF.tasks.sync_path, self.media_file.filename))
    ...
    def is_actionable(media_file):
        ...
        :rtype: boolean
        """
        return media_file.file_path == cfg.CONF.tasks.sync_path and not media_file.synced
