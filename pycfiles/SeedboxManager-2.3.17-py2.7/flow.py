...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-1.7.35-i686/egg/seedbox/process/flow.py
...
class BaseFlow(xworkflows.WorkflowEnabled):
    ...
    def next_tasks(self):
        ...
        for task in self.tasks:
            ...
            for mf in list(self.dbapi.get_medias_by(self.torrent.torrent_id, missing=False, skipped=False)):
    ...
    def validate(self):
        ...
        LOG.debug('state: %s phase: %s torrent: %s', self.state.name, self.phase, self.torrent)
        result = True
        for mf in list(self.dbapi.get_medias_by(self.torrent.torrent_id, missing=False, skipped=False)):
            if mf.error_msg:
                result = False
...
def get_tasks(phase):
    ...
    """
    mgr = named.NamedExtensionManager('seedbox.tasks', names=cfg.CONF['process'][phase], invoke_on_load=False)
