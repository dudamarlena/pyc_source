...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-1.7.35-i686/egg/seedbox/tests/torrent/test_loader.py
...
class TorrentLoaderTest(test.ConfiguredBaseTestCase):
    ...
    def setUp(self):
        ...
        self.CONF.set_override('torrent_path', torrent_path, group='torrent')
        self.CONF.set_override('media_paths', [
         torrent_path], group='torrent')
        self.CONF.set_override('incomplete_path', torrent_path, group='torrent')
    ...
    def test_torrent_downloading(self):
        mediafiles = [
         ('fake1.mp4', 150000), ('fake2.mp4', 150000)]
        self.CONF.set_override('incomplete_path', self.base_dir, group='torrent')
        result = loader._is_torrent_downloading(mediafiles)
        self.assertFalse(result)
        media1 = tempfile.NamedTemporaryFile(suffix='.mp4', dir=self.base_dir)
        media2 = tempfile.NamedTemporaryFile(suffix='.mp4', dir=self.base_dir)
