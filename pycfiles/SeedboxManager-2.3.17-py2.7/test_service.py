...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-1.7.35-i686/egg/seedbox/tests/test_service.py
# Compiled at: 2015-06-14 13:30:57
import logging, logging.handlers, os, shutil, tempfile, fixtures
...
class ServiceTest(test.ConfiguredBaseTestCase):
    ...
    cfg_data.append('media_paths=$base_client_path/complete, $base_client_path/seedLT\n')
    ...
    log_cfg_data.append('format = %(asctime)s - %(name)s - %(levelname)s - %(message)s\n')
    log_cfg_data.append('\n')
    ...
    def test_setup_logging(self):
        ...
        self.assertEqual(logging.getLogger('sqlalchemy').getEffectiveLevel(), logging.ERROR)
        for hndler in logging.getLogger().handlers:
            self.assertThat(hndler, matchers.MatchesAny(matchers.IsInstance(logging.handlers.RotatingFileHandler), matchers.IsInstance(logging.StreamHandler), matchers.IsInstance(logging.NullHandler)))
    ...
    def test_setup_logging_no_logfile(self):
        ...
        service._setup_logging()
        for hndler in logging.getLogger().handlers:
            self.assertThat(hndler, matchers.MatchesAny(matchers.IsInstance(logging.StreamHandler), matchers.IsInstance(logging.NullHandler)))
    ...
    def test_setup_logging_cron(self):
        ...
        service._setup_logging()
        for hndler in logging.getLogger().handlers:
            self.assertThat(hndler, matchers.MatchesAny(matchers.IsInstance(logging.handlers.RotatingFileHandler), matchers.IsInstance(logging.NullHandler)))
    ...
    def test_setup_logging_no_logging(self):
        ...
        service._setup_logging()
        for hndler in logging.getLogger().handlers:
            self.assertThat(hndler, matchers.MatchesAny(matchers.IsInstance(logging.NullHandler)))

        return

    def test_setup_logging_via_file(self):
        logfile = self.create_tempfiles([
         ('seedbox',
          ('').join(self.log_cfg_data))], '.log')[0]
    ...
    def test_configure_with_venv(self):
        ...
        with fixtures.EnvironmentVariable('VIRTUAL_ENV', vdir):
            ...
            service._configure([])
            self.assertEqual(cfg.CONF.base_client_path, '/home/shad/torrent-test/deluge')
    ...
    def test_configure_without_venv(self):
        ...
        with fixtures.EnvironmentVariable('VIRTUAL_ENV'):
            cfgfile = self.create_tempfiles([
             (
              os.path.join(os.path.expanduser('~'), 'seedbox'),
              ('').join(self.cfg_data))])[0]
            self.addCleanup(shutil.rmtree, os.path.dirname(cfgfile), True)
            service._configure([])
            self.assertEqual(cfg.CONF.base_client_path, '/home/shad/torrent-test/deluge')
