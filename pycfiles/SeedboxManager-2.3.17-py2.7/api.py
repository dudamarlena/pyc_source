...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-1.7.35-i686/egg/seedbox/db/sqlalchemy/api.py
...
class Connection(base.Connection):
    ...
    def __init__(self, conf):
        ...
        self._engine_facade = db_session.EngineFacade.from_config(conf.database.connection, conf)
        self.upgrade()
    ...
    def bulk_update(self, value_map, entity_type, qfilter):
        ...
        session = self._engine_facade.session
        with session.begin():
            transformer = db_model.QueryTransformer(_model, session.query(_model))
    ...
    def delete(self, instance):
        ...
        session = self._engine_facade.session
        with session.begin():
            _row = session.query(_model).get(getattr(instance, instance.PK_NAME))
            if _row:
                session.delete(_row)
