...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-1.7.35-i686/egg/seedbox/torrent/bencode.py
...
def bdecode(x):
    ...
    try:
        ...
        raise BTFailure(('not a valid bencoded string; error details ({0})').format(err))
...
def _encode_string(x, r):
    r.extend((bytes(str(len(x)), 'ascii'), ':', x))
...
def _encode_dict(x, r):
    ...
    ilist.sort()
    for k, v in ilist:
        r.extend((bytes(str(len(k)), 'ascii'), ':', k))
        encode_func[type(v)](v, r)

    r.append('e')


encode_func = {Bencached: _encode_bencached, 
   int: _encode_int, 
   long: _encode_int, 
   str: _encode_string, 
