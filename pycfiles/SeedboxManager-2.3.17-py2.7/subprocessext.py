...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-1.7.35-i686/egg/seedbox/tasks/subprocessext.py
...
def _log(name, filepath, unique_id, data):
    _handler = logging.FileHandler(os.path.join(filepath, ('sync.home.{0}').format(unique_id)))
...
class ProcessLogging(subprocess.Popen):
    ...
    def __init__(self, cmd):
        ...
        """
        self._cmd = cmd
        super(ProcessLogging, self).__init__(self._cmd, shell=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE, bufsize=1, universal_newlines=True)
    ...
    def complete(self):
        ...
        if cfg.CONF.tasks_synclog.stderr_verbose and outerr:
            _log('seedbox.tasks.subproc.stderr', cfg.CONF.tasks_synclog.stderr_dir, _uid, outerr)
        if self.returncode != 0:
            raise subprocess.CalledProcessError(self.returncode, self._cmd)
        if cfg.CONF.tasks_synclog.stdout_verbose and outdata:
            _log('seedbox.tasks.subproc.stdout', cfg.CONF.tasks_synclog.stdout_dir, _uid, outdata)
        return
