...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-1.7.35-i686/egg/seedbox/tests/db/sqlalchemy/test_session.py
...
class EngineFacadeTestCase(test.BaseTestCase):
    ...
    def test_create_engine(self):
        ...
        self.assertEqual(logging.getLogger('sqlalchemy.engine').getEffectiveLevel(), logging.WARNING)
        session.create_engine('sqlite:///:memory:', connection_debug=50)
        self.assertEqual(logging.getLogger('sqlalchemy.engine').getEffectiveLevel(), logging.INFO)
        session.create_engine('sqlite:///:memory:', connection_debug=100)
        self.assertEqual(logging.getLogger('sqlalchemy.engine').getEffectiveLevel(), logging.DEBUG)
    ...
    def test_creation_from_config(self, create_engine, get_maker):
        ...
        create_engine.assert_called_once_with(sql_connection='sqlite:///:memory:', connection_debug=100, idle_timeout=mock.ANY)
