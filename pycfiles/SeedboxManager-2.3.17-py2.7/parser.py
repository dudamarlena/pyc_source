...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-1.7.35-i686/egg/seedbox/torrent/parser.py
# Compiled at: 2015-06-17 00:29:03
"""Parses a torrent file."""
import io, logging, os, six
...
class Bdecode(object):
    ...
    def _parse_str(self):
        ...
        if not str_len:
            raise ParsingError('Empty string length found while parsing at position %d' % self.data.tell())
    ...
    def decode(self):
        ...
        if parsed_char == END_TOKEN:
            ...
            if parsed_char == INT_TOKEN:
                return self._parse_int()
            if parsed_char.isdigit():
                return self._parse_str()
            if parsed_char == DICT_TOKEN:
                return self._parse_dict()
            if parsed_char == LIST_TOKEN:
...
class TorrentParser(object):
    ...
    def load_content(self):
        ...
        try:
            ...
            LOG.info('bencode.bdecode failed: (%s); trying alternate approach', bterr)
    ...
    def get_file_details(self):
        ...
        """
        parsed_files_info = []
        files_info = self.content.get('info')
        if not files_info:
        ...
        if multiple_files_info:
            dir_name = files_info.get('name').decode('utf-8')
            LOG.debug('dirname: %s', dir_name)
            for file_info in multiple_files_info:
                LOG.debug('file_info: %s', file_info)
                parsed_files_info.append((
                 os.path.join(dir_name, os.path.sep.join([ x.decode('utf-8') for x in file_info.get('path')
                                        ])),
                 file_info.get('length')))

        else:
            parsed_files_info.append((
             files_info.get('name').decode('utf-8'),
