...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-1.7.35-i686/egg/seedbox/db/sqlalchemy/session.py
...
class EngineFacade(object):
    ...
    def __init__(self, sql_connection, **kwargs):
        ...
        """
        self._engine = create_engine(sql_connection=sql_connection, idle_timeout=kwargs.get('idle_timeout', 3600), connection_debug=kwargs.get('connection_debug', 0))
    ...
    def from_config(cls, connection_string, conf):
        ...
        """
        LOG.debug('making connection using connect string: %s', connection_string)
        return cls(sql_connection=connection_string, **dict(conf.database.items()))