...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-1.7.35-i686/egg/seedbox/db/sqlalchemy/models.py
# Compiled at: 2015-06-14 13:30:57
"""Provides database model compliant with sqlalchemy."""
import datetime, operator, re, six, sqlalchemy as sa
...
def to_table_name(klass_name):
    ...
    :rtype: string
    """
    return re.sub('[A-Z]+', lambda i: '_' + i.group(0).lower(), klass_name).lstrip('_') + 's'
...
class Base(six.Iterator):
    ...
    def iteritems(self):
        ...
        """
        local = dict(self)
        joined = dict([ (k, v) for k, v in six.iteritems(self.__dict__) if not k[0] == '_'
                      ])
        local.update(joined)
...
class Torrent(Base, HasId, HasTimestamp):
    ...
    media_files = orm.relationship('MediaFile', backref='torrents', cascade='all, delete-orphan')
...
class AppState(Base):
    ...
    def set_value(self, value):
        ...
        if self.dtype and self.dtype != NONE_TYPE and self.dtype != dtype:
            raise TypeError(('Expecting type {0} for {1}: {2}').format(self.dtype, self.name, dtype))
        if dtype in VAL_TYPES:
            ...
            raise TypeError(('Unsupported data type [{0}]; expecting {1}').format(dtype, VAL_TYPES))
        return
...
class QueryTransformer(object):
    ...
    complex_operators = {'or': sa.or_, 'and': sa.and_, 
       'not': sa.not_}
