...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-1.7.35-i686/egg/seedbox/tests/common/test_timeutil.py
...
class TimeutilTest(test.BaseTestCase):
    ...
    def test_nvl_date_default_provided(self):
        now = datetime.datetime.utcnow()
        self.assertIsInstance(timeutil.nvl_date(now, now + datetime.timedelta(1)), datetime.datetime)
        self.assertEqual(timeutil.nvl_date(now, now + datetime.timedelta(1)), now)
        self.assertIsInstance(timeutil.nvl_date(None, now + datetime.timedelta(1)), datetime.datetime)
        self.assertIsNotNone(timeutil.nvl_date(None, now + datetime.timedelta(1)))
        return
    ...
    def test_timed_function_emit(self):
        ...
        handler = logging.handlers.MemoryHandler(capacity=100, flushLevel=logging.DEBUG, target=CaptureOutput())
    ...
    def test_timed_function_no_emit(self):
        ...
        handler = logging.handlers.MemoryHandler(capacity=100, target=CaptureOutput())
        handler.setFormatter(logging.Formatter('%(message)s'))
        LOG.addHandler(handler)
    ...
    def test_advance_time_delta(self):
        ...
        _future = timeutil.advance_time_delta(_cur, datetime.timedelta(seconds=60))
