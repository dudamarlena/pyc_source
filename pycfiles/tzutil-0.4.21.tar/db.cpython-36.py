...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-n8f5s77x/tzutil/tzutil/db/pg/pgwrap/db.py
# Compiled at: 2018-12-04 01:36:04
# Size of source mod 2**32: 19472 bytes
import logging, os, time, urllib.parse
...
class SafeNamedTupleCursor(NamedTupleCursor):
    ...
    def _make_nt(self, namedtuple=namedtuple):
        return namedtuple('Record',
          [d[0] for d in self.description or ()], rename=True)


class connection(object):

    def __init__(self, url=None, hstore=False, log=None, logf=None, min=1, max=5, default_cursor=DictCursor):
        params = urllib.parse.urlparse(url or os.environ.get('DATABASE_URL') or 'postgres://localhost/')
        self.pool = ThreadedConnectionPool(min,
          max,
    ...
    def cursor(self, cursor_factory=None):
        return cursor(self.pool, cursor_factory or self.default_cursor, self.hstore, self.log, self.logf)
...
class cursor(object):
    ...
    def __enter__(self, name=None):
        ...
        self.cursor = self.connection.cursor(name=name,
    ...
    def _build_select(self, table, where, order, columns, limit, offset, update):
        return 'SELECT %s FROM %s' % (sqlop.columns(columns), table) + sqlop.where(where) + sqlop.order(order) + sqlop.limit(limit) + sqlop.offset(offset) + sqlop.for_update(update)
    ...
    def select(self, table, where=None, order=None, columns=None, limit=None, offset=None, update=False):
        """
            ...
            [['aaaaa'], ['bbbbb']]
            >>> db.select('doctest_t1',where={'name__in':('aaaaa','bbbbb')},order=('name__desc',)) ==                     db.query("SELECT * FROM doctest_t1 WHERE name IN ('aaaaa','bbbbb') ORDER BY name DESC")
            True
    ...
    def count(self, table, where=None):
        return self.query(f"SELECT COUNT(1) FROM {table} {sqlop.where(where)}", where)

    def get(self, table, id, column):
        return self.select_one(table, dict(id=id), columns=[column])[column]
    ...
    def _build_join(self, tables, where, on, order, columns, limit, offset):
        ...
        return 'SELECT %s FROM %s ' % (sqlop.columns(columns), tables[0]) + ' '.join(['JOIN %s ON %s' % (tables[i], sqlop.on((tables[0], tables[i]), on[(i - 1)])) for i in range(1, len(tables))]) + sqlop.where(where) + sqlop.order(order) + sqlop.limit(limit) + sqlop.offset(offset)
    ...
    def join(self, tables, where=None, on=None, order=None, columns=None, limit=None, offset=None):
        """
            ...
            ...             order=('name',),limit=2)
            [['aaaaa', 'aa'], ['bbbbb', 'bb']]
            >>> db.join(('doctest_t1','doctest_t2'),on=[('doctest_t1.id','doctest_t2.doctest_t1_id')])                             == db.join(('doctest_t1','doctest_t2'))
            True
        """
...
if __name__ == '__main__':
    ...
    if sys.argv.count('--interact'):
        ...
        try:
            ...
            for i in range(10):
                id = db.insert('doctest_t1',
                  {'name': chr(97 + i) * 5}, returning='id')['id']
                _ = db.insert('doctest_t2', {'value':chr(97 + i) * 2, 
                 'doctest_t1_id':id})
