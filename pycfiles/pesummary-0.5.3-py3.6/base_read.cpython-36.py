...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/pesummary/gw/file/formats/base_read.py
# Compiled at: 2020-04-23 12:33:50
# Size of source mod 2**32: 23115 bytes
import numpy as np
...
class GWRead(Read):
    ...
    def load(self, function, **kwargs):
        ...
        if 'mcmc_samples' in data.keys():
            ...
            parameters, samples = self.translate_parameters(data['parameters'], data['samples'])
            if 'log_likelihood' not in parameters:
                logger.warn("Failed to find 'log_likelihood' in result file. Setting every sample to have log_likelihood 0")
            ...
            if data['injection'] is not None:
                self.injection_parameters = self.convert_injection_parameters(data['injection'])
            else:
                self.injection_parameters = data['injection']
            if isinstance(self.injection_parameters, dict):
                self.injection_parameters = {key.decode('utf-8') if isinstance(key, bytes) else key:val for key, val in self.injection_parameters.items()}
            else:
                if isinstance(self.injection_parameters, list):
                    self.injection_parameters = [{key.decode('utf-8') if isinstance(key, bytes) else key:val for key, val in i.items()} for i in self.injection_parameters]
            if 'prior' in data.keys():
                if data['prior'] != {}:
                    priors = data['prior']
                    default_parameters = list(priors.keys())
                    default_samples = [[priors[parameter][i] for parameter in default_parameters] for i in range(len(priors[default_parameters[0]]))]
                    parameters, samples = self.translate_parameters(default_parameters, default_samples)
                    self.priors = con._Conversion(parameters,
                      samples, extra_kwargs=(self.extra_kwargs))
            if 'weights' in self.data.keys():
                self.weights = self.data['weights']
            else:
                self.weights = self.check_for_weights(self.data['parameters'], self.data['samples'])
    ...
    def convert_injection_parameters(self, data):
        ...
        if all(math.isnan(data[i]) for i in data.keys()):
            ...
            inj_samples = con._Conversion(parameters,
              samples, extra_kwargs=(self.extra_kwargs))
    ...
    def _grab_injection_parameters_from_file(self, injection_file):
        ...
        func_map = {'xml':self._grab_injection_data_from_xml_file,  'hdf5':self._grab_injection_data_from_hdf5_file, 
    ...
    def _grab_injection_data_from_xml_file(self, injection_file):
        ...
        """
        if GLUE:
            xmldoc = ligolw_utils.load_filename(injection_file,
            ...
            injection_values = self._return_all_injection_parameters(self.parameters, table)
        else:
            injection_values = [
             float('nan')] * len(self.parameters)
    ...
    def _return_all_injection_parameters(self, parameters, table):
        ...
        func_map = {'chirp_mass':lambda inj: inj.mchirp, 
         ...
         'symmetric_mass_ratio':lambda inj: con.eta_from_m1_m2(inj.mass1, inj.mass2), 
         'inversed_mass_ratio':lambda inj: con.invq_from_m1_m2(inj.mass1, inj.mass2), 
         'total_mass':lambda inj: inj.mass1 + inj.mass2, 
         'chi_p':lambda inj: con._chi_p(inj.mass1, inj.mass2, inj.spin1x, inj.spin1y, inj.spin2x, inj.spin2y), 
         'chi_eff':lambda inj: con._chi_eff(inj.mass1, inj.mass2, inj.spin1z, inj.spin2z)}
        injection_values = []
        for i in parameters:
            try:
    ...
    def grab_calibration_data(function, path_to_results_file):
        ...
        for key in log_frequencies.keys():
            ...
            data = [interp1d((log_frequencies[key]), samp, kind='cubic', fill_value=0, bounds_error=False)(np.log(fs)) for samp in np.column_stack(amp_params[key])]
            amplitude_upper = 1.0 - np.mean(data, axis=0) + np.std(data, axis=0)
            amplitude_lower = 1.0 - np.mean(data, axis=0) - np.std(data, axis=0)
            amplitude_median = 1 - np.median(data, axis=0)
            data = [interp1d((log_frequencies[key]), samp, kind='cubic', fill_value=0, bounds_error=False)(np.log(fs)) for samp in np.column_stack(phase_params[key])]
    ...
    def _timeseries_from_cache_file(strain_dictionary):
        ...
        if not GLUE:
            raise Exception('lscsoft-glue is required to read from a cached file. Please install this package')
        ...
        try:
            ...
            raise Exception('Failed to read in the cached file because %s' % e)
    ...
    def translate_parameters(parameters, samples):
        ...
        """
        path = 'https://git.ligo.org/lscsoft/pesummary/blob/master/pesummary/gw/file/standard_names.py'
        parameters_not_included = [i for i in parameters if i not in standard_names.keys()]
        if len(parameters_not_included) > 0:
            logger.debug("PESummary does not have a 'standard name' for the following parameters: {}. This means that comparison plots between different codes may not show these parameters. If you want to assign a standard name for these parameters, please add an MR which edits the following file: {}. These parameters will be added to the result pages and meta file as is.".format(', '.join(parameters_not_included), path))
        standard_params = [i for i in parameters if i in standard_names.keys()]
        converted_params = [standard_names[i] if i in standard_params else i for i in parameters]
        return (
         converted_params, samples)
    ...
    def _check_definition_of_inclination(parameters):
        ...
        names = [standard_names[i] for i in parameters if i in standard_names.keys()]
        if all(i in names for i in spin_angles):
            theta_jn = True
        if theta_jn:
            if 'theta_jn' not in names:
                if 'inclination' in parameters:
                    logger.warn("Because the spin angles are in your list of parameters, the angle 'inclination' probably refers to 'theta_jn'. If this is a mistake, please change the definition of 'inclination' to 'iota' in your results file")
    ...
    def generate_all_posterior_samples(self, **kwargs):
        ...
        data = _Conversion(
 self.parameters, self.samples, extra_kwargs=self.extra_kwargs, return_dict=False, **kwargs)
        self.data['parameters'] = data[0]
        self.data['samples'] = data[1]
        if kwargs.get('return_kwargs', False):
            self.extra_kwargs = data[2]

    def to_lalinference(self, outdir='./', label=None, filename=None, overwrite=False, sampler='lalinference_nest', dat=False):
        ...
        write_to_file((self.samples_dict),
          outdir=outdir, label=label, filename=filename, overwrite=overwrite,
          sampler=sampler,
          dat=dat)