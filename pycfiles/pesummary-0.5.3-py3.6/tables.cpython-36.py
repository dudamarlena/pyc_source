...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/pesummary/core/webpage/tables.py
# Compiled at: 2020-04-21 06:57:35
# Size of source mod 2**32: 7459 bytes
from pesummary.core.webpage.base import Base

class table_of_images(Base):

    def __init__(self, content, rows, columns, html_file, code, cli, autoscale=False, unique_id=None, captions=None, extra_div=False):
        """
    ...
    def _insert_image(self, path, width, indent, _id, justify='center'):
        string = "<img src='{}' alt='No image available' ".format(path) + "style='align-items:center; width:{}px;'".format(width)
        string += 'id={} onclick=\'{}("{}", "{}")\'>\n'.format(_id, self.code, _id, self.modal_id)
        ...
        if self.rows == 1:
            ...
            for num, i in enumerate(self.content[1]):
                ...
                if self.captions:
                    ...
                    self.add_content(("<button value='test' class='btn btn-info btn-xs' style='cursor: pointer' data-toggle='popover' data-placement='top' data-content='%s'>Caption</button>" % self.captions[1][num]),
                      indent=8)
                    self.end_div(6)
                    self.end_div(6)
                self.end_div(4)
            ...
            for idx, i in enumerate(self.content):
                if self.autoscale:
                    ...
                    captions_margin_left = str(-1400.0 / (8 * len(i)))
                self.make_div(8, _class='column', _style='padding-left: 1em;')
                for num, j in enumerate(i):
                    _id = j.split('/')[(-1)][:-4]
                    self.make_div(10, _class='container', _style='display: inline-block; width: auto; padding: 0;')
                    self.add_content(("<a href='#%s' data-slide-to='%s'>\n" % (
                     self.demo_id, ind)),
                      indent=6)
                    ...
                    if self.cli:
                        self.add_content(("<button value='test' class='btn btn-info btn-xs' style='cursor: pointer' data-toggle='popover' data-placement='top' data-content='%s'>Command Line</button>" % self.cli[idx][num]),
                          indent=10)
                        self.end_div(10)
                    if self.captions:
                        if self.cli:
                            _style = 'margin-left: {}px;'.format(captions_margin_left)
                        else:
                            _style = None
                        self.make_div(6, _class='col-sm-4', _style=_style)
                        self.add_content(("<button value='test' class='btn btn-info btn-xs' style='cursor: pointer' data-toggle='popover' data-placement='top' data-content='%s'>Caption</button>" % self.captions[idx][num]),
                          indent=10)
                        self.end_div(10)
                    self.end_div(10)
                    if self.extra_div:
