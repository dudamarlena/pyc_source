...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/pesummary/gw/pepredicates.py
# Compiled at: 2020-04-21 06:57:35
# Size of source mod 2**32: 6848 bytes
try:
...
def get_classifications(samples):
    ...
    default_error = 'Failed to generate source classification probabilities because {}'
    try:
        with RedirectLogger('PEPredicates', level='DEBUG') as (redirector):
            parameters = list(samples.keys())
            samples = [[samples[parameter][j] for parameter in parameters] for j in range(len(samples[parameters[0]]))]
            data = PEPredicates.classifications(samples, parameters)
        classifications = {'default':data[0],  'population':data[1]}
    except ImportError:
        logger.warn(default_error.format("'PEPredicates' is not installed"))
...
class PEPredicates(object):
    __doc__ = 'Class to handle the PEPredicates package\n    '

    @staticmethod
    def default_predicates():
        """Set the default possibilities
        """
        default = {'BNS':pep.BNS_p, 
         'NSBH':pep.NSBH_p, 
         'BBH':pep.BBH_p, 
         'MassGap':pep.MG_p}
        return default
    ...
    def check_for_install():
        ...
        """
        if not PEP:
            raise ImportError("Failed to import 'predicates' and therefore unable to calculate astro/terrestrial probabilities")
    ...
    def convert_to_PEPredicated_data_frame(samples, parameters):
        ...
        if not all(i in parameters for i in list(mapping.keys())):
            raise Exception('Failed to generate classification probabilities because not all required parameters have been provided.')
    ...
    def default_classification(samples, parameters):
        ...
        """
        PEPredicates.check_for_install()
        core_samples = PEPredicates.convert_to_PEPredicated_data_frame(samples, parameters)
        ptable = pep.predicate_table(PEPredicates.default_predicates(), core_samples)
    ...
    def population_classification(samples, parameters):
        ...
        ptable = pep.predicate_table(PEPredicates.default_predicates(), psamps_resamples)
        for key, value in ptable.items():
            ptable[key] = np.round(value, 5)
