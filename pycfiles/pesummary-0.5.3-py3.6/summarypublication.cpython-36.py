...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/pesummary/cli/summarypublication.py
# Compiled at: 2020-04-21 06:57:35
# Size of source mod 2**32: 14037 bytes
import pesummary
...
import argparse, matplotlib.pyplot as plt, seaborn, numpy as np
...
def command_line():
    ...
    parser.add_argument('--parameters', dest='parameters', nargs='+', help='parameters of the 2d contour plot you wish to make',
      default=None)
    parser.add_argument('--publication_kwargs', help='Optional kwargs for publication plots',
      action=DictionaryAction,
      nargs='+',
      default={})
    parser.add_argument('--palette', dest='palette', help='Color palette to use to distinguish result files',
      default='colorblind')
    parser.add_argument('--colors', dest='colors', help='Colors you wish to use to distinguish result files',
      nargs='+',
      default=None)
    parser.add_argument('--linestyles', dest='linestyles', help='Linestyles you wish to use to distinguish result files',
      nargs='+',
      default=None)
    return parser
...
def default_violin_plot():
    ...
    """
    violin_plots = [
     'chi_eff', 'chi_p', 'mass_ratio', 'luminosity_distance']
    return violin_plots
...
def read_samples(result_files):
    ...
    for i in result_files:
        try:
            ...
            logger.warn("Failed to read '{}'. Data will not be added to the plots".format(i))
...
def get_colors_and_linestyles(opts):
    ...
    if opts.colors is not None:
        ...
        colors = seaborn.color_palette(palette=(opts.palette),
          n_colors=(len(opts.samples))).as_hex()
    if opts.linestyles is not None:
        ...
        for color in unique_colors:
            ...
            for idx, j in enumerate(indicies):
                linestyles[j] = available_linestyles[np.mod(idx, len(available_linestyles))]
...
def make_2d_contour_plot(opts):
    ...
    for i in default:
        if not all(all(k in j for k in i) for j in parameters):
            idxs = [num for num, j in enumerate(parameters) if not all(k in j for k in i)]
            files = [opts.samples[j] for j in idxs]
            logger.warn('Removing {} from 2d contour plot because the parameters {} are not in the result file'.format(' and '.join(files), ' and '.join(i)))
            ...
            samples1 = [[k[ind1[num]] for k in l] for num, l in enumerate(samples)]
            samples2 = [[k[ind2[num]] for k in l] for num, l in enumerate(samples)]
            twod_samples = [[j, k] for j, k in zip(samples1, samples2)]
            gridsize = opts.publication_kwargs['gridsize'] if 'gridsize' in opts.publication_kwargs.keys() else 100
            fig = pub.twod_contour_plots(i,
              twod_samples, (opts.labels), latex_labels, colors=colors, linestyles=linestyles,
              gridsize=gridsize)
...
def make_violin_plot(opts):
    ...
    for i in default:
        if not all(i in j for j in parameters):
            ...
            files = [opts.samples[j] for j in idxs]
            logger.warn('Removing {} from violin plot because the parameter {} does not exist in the result file'.format(' and '.join(files), i))
            parameters = [j for num, j in enumerate(parameters) if num not in idxs]
            opts.labels = [j for num, j in enumerate(opts.labels) if num not in idxs]
            samples = [j for num, j in enumerate(samples) if num not in idxs]
        try:
            ind = [j.index(i) for j in parameters]
            samples = [[k[ind[num]] for k in l] for num, l in enumerate(samples)]
...
def make_spin_disk_plot(opts):
    ...
    for num, i in enumerate(parameters):
        if not all(j in i for j in required_parameters):
            logger.info('Failed to generate spin disk plots for %s because %s are not in the result file' % (
             opts.labels[num],
            ...
            try:
                ...
                fig = pub.spin_distribution_plots(required_parameters, spin_samples, opts.labels[num], colors[num])
    ...
    if len(opts.samples) > 1:
        ...
        for num, label in enumerate(opts.labels):
            if not all(i in parameters[num] for i in opts.parameters):
                logger.warn("'{}' does not include samples for '{}' and/or '{}'. This analysis will not be added to the plot".format(label, opts.parameters[0], opts.parameters[1]))
            else:
                plotting_data[label] = {}
                if xerr is not None:
                    xerr[label] = {}
                ...
                for param in opts.parameters:
                    ...
                    plotting_data[label][param] = np.median([i[ind] for i in samples[num]])

                if xerr is not None:
                    ind = parameters[num].index(opts.parameters[0])
                    xerr[label][opts.parameters[0]] = [np.abs(plotting_data[label][opts.parameters[0]] - np.percentile([i[ind] for i in samples[num]], j)) for j in (5,
                                                                                                                                                                     95)]
                if yerr is not None:
                    ind = parameters[num].index(opts.parameters[1])
                    yerr[label][opts.parameters[1]] = [np.abs(plotting_data[label][opts.parameters[1]] - np.percentile([i[ind] for i in samples[num]], j)) for j in (5,
                                                                                                                                                                     95)]

        fig = pop.scatter_plot((opts.parameters),
          plotting_data, latex_labels, xerr=xerr, yerr=yerr)
        fig.savefig('{}/event_scatter_plot_{}.png'.format(opts.webdir, '_and_'.join(opts.parameters)))
        plt.close()
...
def main(args=None):
    ...
    func_map = {'2d_contour':make_2d_contour_plot,  'violin':make_violin_plot, 
