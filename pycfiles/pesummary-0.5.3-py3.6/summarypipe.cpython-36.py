...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/pesummary/cli/summarypipe.py
# Compiled at: 2020-04-21 06:57:35
# Size of source mod 2**32: 13229 bytes
from pesummary.utils.utils import logger
from pesummary.utils.decorators import open_config
import argparse, shutil, os, re, ast, glob
...
def command_line():
    ...
    parser.add_argument('-r', '--rundir', dest='rundir', help='run directory of the parameter estimation job',
      default=None,
      required=True)
...
def is_lalinference_rundir(rundir):
    ...
    """
    std_dirs = [
     'ROQdata', 'engine', 'posterior_samples', 'caches', 'log']
...
class Base(object):
    __doc__ = '\n    '
    __arguments__ = {'samples':'samples', 
     'config':'config',  'webdir':'webdir',  'approximant':'approximant', 
     'labels':'label',  '{}_psd':'psd',  '{}_calibration':'calibration', 
     'gwdata':'gwdata'}
    ...
    def __init__(self, rundir, webdir=None, label=None):
        ...
        self.gwdata = None
        self.command_line_arguments = {arg.format(self.label):getattr(self, attribute, None) for arg, attribute in self.__arguments__.items()}
    ...
    def rundir(self, rundir):
        ...
        if not os.path.isdir(rundir):
            raise ValueError("'{}' is not a valid directory.".format(rundir))
    ...
    def config(self, config):
        ...
        self.check_files(files, 'configuration file', self.pattern['config_file'], self.rundir)
    ...
    def samples(self, samples):
        ...
        self.check_files(files, 'result file', self.pattern['posterior_file'], self.rundir)
    ...
    def check_files(files, description, filename, rundir):
        if len(files) == 0:
            raise FileNotFoundError("No file called '{}' found in {}.".format(filename, rundir))
        else:
            if len(files) == 1:
                logger.info('Found the following {}: {}'.format(description, files[0]))
            else:
                logger.info('Multiple {}s found. Using {}'.format(description, files[0]))
...
class LALInference(Base):
    __doc__ = 'Generate a `summarypages` executable for a lalinference run directory\n    '

    def __init__(self, rundir, webdir=None, label=None):
        self.default_label = 'LALInference'
        self.pattern = {'posterior_file':'posterior*.hdf5', 
    ...
    def webdir_fallback(self):
        ...
        logger.info("Using the same webdir as used in '{}': {}".format(self.config, path))
    ...
    def gracedb_fallback(self):
        ...
        try:
            ...
            logger.info("Found the following gracedb ID in '{}': '{}'".format(self.config, gid))
            return gid
        except KeyError:
            logger.info("Unable to find a gracedb entry in '{}'".format(self.config))
            return
    ...
    def approximant_fallback(self):
        ...
        logger.info("Found the following approximant in '{}': '{}'".format(self.config, approx))
        return approx
    ...
    def psd_fallback(self):
        ...
        try:
            ...
            logger.info("Unable to find any PSD information in '{}'. Looking in run directory".format(self.config))
            files = self.glob(self.rundir, '*-PSD.dat')
            if len(files) == 0:
                logger.info("No PSD files found in '{}'".format(self.rundir))
                return
            ifos = {re.split('([A-Z][0-9]+)-PSD.dat', i)[(-2)] for i in files}
            return ' '.join([['{}:{}'.format(ifo, i) for i in files if '{}-PSD.dat'.format(ifo) in i][0] for ifo in ifos])
    ...
    def calibration_fallback(self):
        ...
        config = self.load_config(self.config)
        try:
            return self.grab_psd_calibration_data_from_config(config, '-spcal-envelope')
        except KeyError:
            logger.info("Unable to find any calibration information in '{}'".format(self.config))
            return
    ...
    def gwdata_fallback(self):
        ...
        try:
            ...
            logger.info('Found the following cache files in {}: {}'.format(self.rundir, ', '.join(cache_files)))
            ...
            return ' '.join(['{}:{}'.format(channels[ifo], path) for ifo, path in zip(ifos, cache_files)])
        except ValueError:
            return
    ...
    def grab_psd_calibration_data_from_config(self, config, pattern):
        ...
        data = {ifo.split(pattern)[0].upper():config['engine'][ifo] for ifo in ifos if len(ifos)}
        if not len(ifos):
            raise KeyError
...
def main(args=None):
    ...
    if is_lalinference_rundir(opts.rundir):
        ...
        raise NotImplementedError("'{}' not understood. Currently 'summarypipe' only works with a LALInference rundir.".format(opts.rundir))