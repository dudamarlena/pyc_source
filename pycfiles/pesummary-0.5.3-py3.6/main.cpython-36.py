...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/pesummary/gw/webpage/main.py
# Compiled at: 2020-04-21 06:57:35
# Size of source mod 2**32: 34010 bytes
import os, uuid, numpy as np, pesummary
...
class CommandLineCaption(object):
    __doc__ = 'Class to handle generating the command line used to generate a plot and\n    a caption to describe the plot\n\n    Parameters\n    ----------\n    '
    ...
    def histogram_caption(self):
        ...
        general_cli = '1d histogram showing the posterior distribution for {}.'.format(parameter)
        if parameter == 'chi_p':
            general_cli += ' chi_p is the precession parameter and quantifies how much precession there is in the system. It ranges from 0 to 1 with 0 meaning there is no precession in the system and 1 meaning there is maximal precession in the system.'
        if self.samples is not None:
            general_cli += ' The median of the distribution is {} with 90% confidence interval {}'.format(np.round(self.samples.average(type='median'), 3), [np.round(i, 3) for i in self.samples.confidence_interval()])
        return general_cli

    def skymap_caption(self):
        """Return a caption to describe the skymap plot
        """
        general_cli = 'Skymap showing the possible location of the source of the source of the gravitational waves'


class _WebpageGeneration(_CoreWebpageGeneration):
    __doc__ = '\n    '

    def __init__(self, webdir, samples, labels, publication, user, config, same_parameters, base_url, file_versions, hdf5, colors, custom_plotting, pepredicates_probs, gracedb, approximant, key_data, file_kwargs, existing_labels, existing_config, existing_file_version, existing_injection_data, existing_samples, existing_metafile, add_to_existing, existing_file_kwargs, existing_weights, result_files, notes, disable_comparison, pastro_probs, gwdata, disable_interactive=NoneNoneNoneNoneNoneNoneNoneNoneNoneNoneNoneNoneNoneNoneNoneNoneNoneNoneNoneNoneNoneNoneNoneFalseNoneNoneNoneNoneFalseNoneNoneFalse, publication_kwargs={}, no_ligo_skymap=False, psd=None, priors=None, package_information={'packages': []}):
        ...
        self.psd = psd
        self.priors = priors
        super(_WebpageGeneration, self).__init__(webdir=webdir,
          samples=samples,
          labels=labels,
    ...
    def _make_home_pages(self, pages):
        ...
        """
        title = None if self.gracedb is None else 'Parameter Estimation Summary Pages for {}'.format(self.gracedb)
        banner = 'Summary' if self.gracedb is None else 'Summary for {}'.format(self.gracedb)
        ...
        if self.make_comparison:
            if not all(self.approximant[i] is not None for i in self.labels):
                image_contents = []
                image_contents.append(os.path.join(path, 'compare_time_domain_waveforms.png'))
                image_contents = [
                 image_contents]
        ...
        for i in self.labels:
            ...
            if os.path.isfile(basic_string.format('%s_waveform_time_domain' % i)):
                image_contents.append(relative_path.format('%s_waveform_time_domain' % i))
                captions.append(PlotCaption('time_waveform'))
            if os.path.isfile(basic_string.format('%s_calibration_plot' % i)):
                image_contents.append(relative_path.format('%s_calibration_plot' % i))
            ...
            if self.file_kwargs[i]['sampler'] != {}:
                html_file.make_banner(approximant='Sampler kwargs',
                  key='sampler_kwargs',
            ...
            if self.file_kwargs[i]['meta_data'] != {}:
                html_file.make_banner(approximant='Meta data',
                  key='meta_data',
        ...
        for num, i in enumerate(self.labels):
            html_file = self.setup_page(i,
              (self.navbar['result_page'][i]), i, approximant=i, title=('{} Summary page'.format(i)),
              background_colour=(self.colors[num]))
            html_file.make_banner(approximant=i, key=i)
            images, cli, captions = self.default_images_for_result_page(i)
            unique_id = '{}'.format(uuid.uuid4().hex.upper()[:6])
            html_file.make_table_of_images(contents=images,
              cli=cli,
              unique_id=unique_id,
              captions=captions)
            ...
            if self.custom_plotting:
                ...
                custom_plots = glob('{}/plots/{}_custom_plotting_*'.format(self.webdir, i))
                path = self.image_path['other']
                for num, i in enumerate(custom_plots):
                    custom_plots[num] = path + i.split('/')[(-1)]

                image_contents = [custom_plots[i:4 + i] for i in range(0, len(custom_plots), 4)]
                unique_id = '{}'.format(uuid.uuid4().hex.upper()[:6])
                html_file.make_table_of_images(contents=image_contents,
                  unique_id=unique_id)
                images = [y for x in image_contents for y in x]
            ...
            html_file.make_table(headings=[
             ' ', 'maxL', 'mean', 'median', 'std'],
              contents=contents,
              heading_span=1)
    ...
    def _make_publication_pages(self, pages):
        ...
        if self.publication_kwargs != {}:
            general_cli += '--publication_kwargs %s' % ' '.join(['{}:{}'.format(key, value) for key, value in self.publication_kwargs.items()])
        html_file = self.setup_page('Publication',
          (self.navbar['home']), title='Publication Plots')
        html_file.make_banner(approximant='Publication', key='Publication')
        path = self.image_path['other']
        pub_plots = glob(os.path.join(self.webdir, 'plots', 'publication', '*.png'))
        ...
        cli = []
        cap = []
        posterior_name = --- This code section failed: ---

 L. 397         0  LOAD_FAST                'i'

 L. 398         2  LOAD_GLOBAL              descriptive_names
                4  LOAD_ATTR                keys
                6  CALL_FUNCTION_0       0  '0 positional arguments'
                8  COMPARE_OP               in
               10  POP_JUMP_IF_FALSE    40  'to 40'
               12  LOAD_GLOBAL              descriptive_names
               14  LOAD_FAST                'i'
               16  BINARY_SUBSCR    
               18  LOAD_STR                 ''
               20  COMPARE_OP               !=
               ...
               26  LOAD_ATTR                format
               28  LOAD_FAST                'i'
               30  LOAD_GLOBAL              descriptive_names
               32  LOAD_FAST                'i'
               34  BINARY_SUBSCR    
...
Parse error at or near `COME_FROM' instruction at offset 40_1
        for i in pub_plots:
            ...
            if 'violin_plot' in filename:
                ...
                cli.append(general_cli.format('violin') + ' --parameters %s' % parameter)
                cap.append(PlotCaption('violin').format(posterior_name(parameter)))
            else:
                if 'spin_disk' in filename:
                    ...
                    cli.append(general_cli.format('2d_contour') + ' --parameters %s' % parameters.replace('_and_', ' '))
                    pp = parameters.split('_and_')
                    cap.append(PlotCaption('2d_contour').format(posterior_name(pp[0]), posterior_name(pp[1])))

        image_contents = [pub_plots[i:3 + i] for i in range(0, len(pub_plots), 3)]
        command_lines = [cli[i:3 + i] for i in range(0, len(cli), 3)]
    ...
    def _make_detector_pages(self, pages):
        ...
        """
        from glob import glob
        from pesummary.utils.utils import determine_gps_time_and_window, command_line_dict
        from astropy.time import Time
        executable = self.get_executable('summarydetchar')
        command_line = command_line_dict()
        if isinstance(command_line['gwdata'], dict):
            gwdata_command_line = ['{}:{}'.format(key, val) for key, val in command_line['gwdata'].items()]
        else:
        ...
        maxL_samples = {i:{'geocent_time': self.key_data[i]['geocent_time']['maxL']} for i in self.labels}
        gps_time, window = determine_gps_time_and_window(maxL_samples, self.labels)
        t = Time(gps_time, format='gps')
        t = Time(t, format='datetime')
        link = 'https://ldas-jobs.ligo-wa.caltech.edu/~detchar/summary/day/{}{}{}/'.format(t.value.year, '0{}'.format(t.value.month) if t.value.month < 10 else t.value.month, '0{}'.format(t.value.day) if t.value.day < 10 else t.value.day)
        for det in self.gwdata.keys():
            html_file = self.setup_page(det,
              (self.navbar['home']), title=('{} Detchar'.format(det)))
            ...
            command_lines = [
             [
              ...
              general_cli.format('omegascan', '--gps %s --vmin 0 --vmax 25 --window %s' % (
               gps_time, window))]]
            html_file.make_table_of_images(contents=image_contents,
              cli=command_lines,
              autoscale=True)
    ...
    def _make_classification_pages(self, pages):
        ...
        for num, label in enumerate(self.labels):
            html_file = self.setup_page(('{}_Classification'.format(label)),
              (self.navbar['result_page'][label]),
              label, title=('{} Classification'.format(label)),
              background_colour=(self.colors[num]),
              approximant=label)
            ...
            if self.pepredicates_probs[label] is not None:
                ...
                table_contents = [['{} prior'.format(i)] + [self.pepredicates_probs[label][i][j] for j in keys] for i in ('default',
                                                                                                                          'population')]
                if self.pastro_probs[label] is not None:
                    keys += ['HasNS']
                    keys += ['HasRemnant']
                    table_contents[0].append(self.pastro_probs[label]['default']['HasNS'])
                    table_contents[0].append(self.pastro_probs[label]['default']['HasRemnant'])
                    try:
                        table_contents[1].append(self.pastro_probs[label]['population']['HasNS'])
                        table_contents[1].append(self.pastro_probs[label]['population']['HasRemnant'])
                    except KeyError:
                        table_contents[1].append('-')
                        table_contents[1].append('-')
                        logger.warn("Failed to add 'em_bright' probabilities for population reweighted prior")
                ...
                html_file.make_table(headings=([
                 ' '] + keys),
                  ...
                  heading_span=1,
                  accordian=False)
                html_file.make_cli_button(general_cli.format(self.result_files[num]))
                html_file.export(('classification_{}.csv'.format(label)),
                  margin_top='-1.5em',
                  margin_bottom='0.5em',
                  json=True)
                html_file.end_div(4)
            ...
            base = '%s --webdir %s --labels %s --plot {} --prior {}' % (
    ...
    def _make_entry_in_downloads_table(self, html_file, label, num, base_string):
        table = super(_WebpageGeneration, self)._make_entry_in_downloads_table(html_file, label, num, base_string)
        if not self.no_ligo_skymap:
            table.append([
             base_string.format('Fits file containing skymap for this analysis', self.results_path['other'] + '%s_skymap.fits' % label)])
        if self.psd is not None:
            if self.psd != {}:
                if label in self.psd.keys():
                    for ifo in self.psd[label].keys():
                        if len(self.psd[label][ifo]):
                            table.append([
                             base_string.format('%s psd file used for this analysis' % ifo, os.path.join(self.psd_path['other'], '%s_%s_psd.dat' % (label, ifo)))])

        if 'calibration' in self.priors.keys():
            if label in self.priors['calibration'].keys():
                for ifo in self.priors['calibration'][label].keys():
                    table.append([
                     base_string.format('%s calibration envelope file used for this analysis' % ifo, os.path.join(self.calibration_path['other'], '%s_%s_cal.txt' % (label, ifo)))])
    ...
    def default_images_for_result_page(self, label):
        ...
        general_cli = '%s --webdir %s --samples %s --burnin %s --plot {} {} --labels %s' % (
         executable, os.path.join(self.webdir, 'plots'),
        ...
        cli = [
         ...
         [
          ...
          general_cli.format('1d_histogram', '--parameter luminosity_distance'),
 ...
 L. 731         2  LOAD_GLOBAL              descriptive_names
                4  LOAD_ATTR                keys
                6  CALL_FUNCTION_0       0  '0 positional arguments'
                8  COMPARE_OP               in
               10  POP_JUMP_IF_FALSE    40  'to 40'
...
Parse error at or near `COME_FROM' instruction at offset 40_1
        ...
        return (
         image_contents, cli, captions)

    def default_categories(self):
        """Return the default categories
        """
        categories = self.categories = {'masses':{'accept':[
           'mass'], 
          'reject':[
           'source', 'final']}, 
         'source':{'accept':[
         ...
         'location':{'accept':[
           ...
           'comoving_distance'], 
          'reject':[
           'mass_ratio', 'radiated', 'ram', 'ran', 'rat']}, 
         'timings':{'accept':[
           'time'], 
          'reject':[]}, 
         'SNR':{'accept':[
          ...
          'reject':[]}, 
         'calibration':{'accept':[
           'spcal', 'recalib', 'frequency'], 
          'reject':[
           'minimum']}, 
         'energy':{'accept':[
           'peak_luminosity', 'radiated'], 
          'reject':[]}, 
         'others':{'accept':[
           'phase', 'likelihood', 'prior'], 
          'reject':[
           'spcal', 'recalib']}}
        return categories
