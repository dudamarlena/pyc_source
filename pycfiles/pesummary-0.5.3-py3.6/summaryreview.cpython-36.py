...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/pesummary/cli/summaryreview.py
...
def launch_lalinference(webdir, path_to_results_file, trigger_file=None):
    ...
    if executable_path is None:
        raise Exception("'cbcbayespostproc' is not installed in your environment. failed to generate lalinference pages")
    webdir += '/lalinference'
...
def launch_pesummary(webdir, path_to_results_file, trigger_file=None):
    ...
    executable_path = get_executable_path(executable)
    if executable_path is None:
        raise Exception("'summarypages' is not installed in your environment. failed to generate PESummary pages")
    webdir += '/pesummary'
...
def get_list_of_plots(webdir, lalinference=False, pesummary=False):
    ...
    if lalinference:
        histogram_plots = sorted(glob(webdir + '/lalinference/1Dpdf/*.png'))
        params = [i.split('/')[(-1)].split('.png')[0] for i in histogram_plots]
    else:
        if pesummary:
            histogram_plots = sorted(glob(webdir + '/pesummary/plots/*_1d_posterior*.png'))
            params = [i.split('/')[(-1)].split('_1d_posterior_')[1].split('.png')[0] for i in histogram_plots]
    return (
     histogram_plots, params)
...
def make_basic_comparison_html(webdir):
    ...
    try:
        lalinference_data = np.genfromtxt((os.path.join(webdir, 'lalinference', 'posterior_samples.dat')),
          names=True)
    ...
    lalinference_1d_histograms, lalinference_params = get_list_of_plots(webdir,
      lalinference=True)
    pesummary_1d_histograms, pesummary_params = get_list_of_plots(webdir,
      pesummary=True)
    names = [[standard_names[i], i] for i in lalinference_params if i in list(standard_names.keys())]
    extra = [i for i in lalinference_params if i not in list(standard_names.keys())]
    pages = [
     'home']
    webpage.make_html(web_dir=webdir, pages=pages)
    html_file = webpage.open_html(web_dir=webdir,
      base_url='./',
      html_page='home')
    ...
    html_file.add_content("Here we show the comparison between the plots generated by 'cbcBayesPostProc' and 'PESummary'")
    html_file.end_div()
    ...
    for num, i in enumerate(names):
        if i[0] in pesummary_params:
            ...
            try:
                ...
                html_file.add_content('max difference in %s samples: %s' % (
                 i[0],
                 np.max(np.abs(lalinference_data[i[1]] - pesummary_data[i[0]]))))
                html_file.end_div()
            except Exception:
                pass
    ...
    for num, i in enumerate(extra_pesummary):
        contents = [
         [
          './pesummary/plots/pesummary_1d_posterior_%s.png' % i]]
        html_file.make_container()
        make_table_of_images(html_file, contents)
        html_file.end_container()
...
def main(args=None):
    ...
    logger.info("Different in runtime: 'cbcBayesPostProc' took {}s and 'summarypages' took {}s".format(t1, t2))
