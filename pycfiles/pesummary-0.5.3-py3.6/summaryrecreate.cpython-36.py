...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/pesummary/cli/summaryrecreate.py
# Compiled at: 2020-04-21 06:57:35
# Size of source mod 2**32: 11108 bytes
...
import subprocess, os, argparse, copy
...
def command_line():
    ...
    parser.add_argument('-s',
      '--samples', dest='samples', required=True, help='Path to the PESummary file which stores the analysis you wish to recreate.')
    parser.add_argument('--labels',
      dest='labels', nargs='+', default=None, help='The label/labels of the analysis you wish to recreate')
    parser.add_argument('-r',
      '--rundir', dest='rundir', default='./', help='The run directory of the analysis')
    parser.add_argument('-c',
      '--code', dest='code', default='lalinference', help='The sampling software you wish to run')
    ...
    parser.add_argument('--config_override',
      action=DelimiterSplitAction, nargs='+', dest='config_override',
      help="Changes you wish to make to the configuration file. Must be in the form `key:item` where key is the entry in the config file you wish to modify, ':' is the default delimiter, and item is the string you wish to replace with.",
      default={})
    return parser


class LALInference(object):
    __doc__ = 'Class to create a LALInference analysis\n    '
    ...
    def pipe(rundir, config, **kwargs):
        ...
        kwargs: dict
            ...
            `lalinference_pipe`
        """
        command_line = 'lalinference_pipe {} -r {}'.format(config, os.path.join(rundir, 'outdir'))
    ...
    def map(parameter, delimiter=':'):
        ...
        """
        not_equal_to_length_two_error = "The {} must be in the form '{}:{}' where '%s' is the default delimiter" % delimiter
        p = parameter.split(delimiter)
        if 'psd' in p:
            if len(p) != 2:
                raise ValueError(not_equal_to_length_two_error.format('psd', 'ifo', 'psd'))
            return '{}-{}'.format(p[0].lower(), p[1])
        else:
            if 'calibration' in p:
                if len(p) != 2:
                    raise ValueError(not_equal_to_length_two_error.format('calibration', 'ifo', 'calibration'))
...
class _Input(object):
    ...
    def samples(self, samples):
        if not os.path.isfile(samples):
            raise FileNotFoundError("File '{}' does not exist".format(samples))
        ...
        if labels is not None:
            ...
            for label in labels:
                if label not in self.samples.labels:
                    raise InputError("The label '{}' is not stored in the result file. The list of available labels is {}".format(label, ', '.join(self.samples.labels)))

            logger.info("Recreating the following analyses: '{}'".format(', '.join(labels)))
        else:
            logger.info('No labels provided. Recreating all analyses stored in the result file: {}'.format(', '.join(self.samples.labels)))
    ...
    def rundir(self, rundir):
        ...
        make_dir(rundir)
        logger.info("Setting '{}' to be the run directory of the analysis".format(rundir))
    ...
    def code(self, code):
        ...
        if code.lower() not in allowed_codes:
            raise InputError("The 'summaryrecreate' executable is currently only configured to run with the following codes: {}".format(', '.join(allowed_codes)))
    ...
    def _write_to_file(self, attribute):
        ...
        if len(data):
            if all(data[label] != {} for label in self.labels):
                for label in self.labels:
                    for ifo in data[label].keys():
                        filename = os.path.join(self.rundir, label, '{}_{}.dat'.format(ifo, attribute))
                        data[label][ifo].save_to_file(filename)
                        name = self.code.map((':'.join([ifo, attribute])),
                          delimiter=':')
                        self.settings_to_change[label][name] = filename

        else:
            logger.warn('No {} data found in the file. Using the {}s stored in the configuration file'.format(attribute, attribute))
    ...
    def write_config_file(self):
        ...
        if not len(self.samples.config):
            raise ValueError('No configuration file stored in the result file. Unable to recreate the analysis')
        config_files = {}
        for label in self.labels:
            ...
            if self.settings_to_change is not None:
                for key, item in self.settings_to_change[label].items():
                    try:
                        ...
                        config_data = self.samples.edit_dictionary(config_data, path, item)
                    except ValueError:
                        logger.warn("Unable to change '{}' to '{}' in the config file".format(key, item))

            outdir = os.path.join(self.rundir, label)
            PESummary.save_config_dictionary_to_file(config_data,
              outdir=outdir, filename='config.ini')
