...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/splunklib.py
# Compiled at: 2018-08-07 16:48:04
# Size of source mod 2**32: 13438 bytes
import datetime, json, logging, re, requests, time, traceback, warnings, xml.etree.ElementTree as ET
...
class SplunkQueryObject(object):
    __doc__ = 'Basic query functionality for splunk.'

    def __init__(self, uri=None, username=None, password=None, max_result_count=1000, relative_duration_before='00:00:15', relative_duration_after='00:00:05', query_timeout='00:30:00', network_timeout=30, *args, **kwargs):
    ...
    def query_relative(self, query, event_time=None, relative_duration_before=None, relative_duration_after=None):
        ...
        if not event_time is None:
            ...
            elif not relative_duration_before is None:
                if not isinstance(relative_duration_before, str):
                    raise AssertionError
            elif not relative_duration_after is None:
                assert isinstance(relative_duration_after, str)
        ...
        if not isinstance(query, str):
            ...
            if not isinstance(time_start, datetime.datetime):
                ...
                if time_end is not None:
                    ...
                    query = re.sub('^\\s*search', 'search latest={0}'.format(splunk_time_end), query, 1, re.I)
            splunk_time_start = ''
            if time_start is not None:
                splunk_time_start = time_start.strftime('%m/%d/%Y:%H:%M:%S')
                query = re.sub('^\\s*search', 'search earliest={0}'.format(splunk_time_start), query, 1, re.I)
    ...
    def query_with_index_time(self, query, time_start, time_end):
        if not isinstance(query, str):
            ...
            if not isinstance(time_start, datetime.datetime):
                ...
                if time_end is not None:
                    splunk_time_end = time_end.strftime('%m/%d/%Y:%H:%M:%S')
                    query = re.sub('^\\s*search', 'search _index_latest={0}'.format(splunk_time_end), query, 1, re.I)
            splunk_time_start = ''
            if time_start is not None:
                splunk_time_start = time_start.strftime('%m/%d/%Y:%H:%M:%S')
                query = re.sub('^\\s*search', 'search _index_earliest={0}'.format(splunk_time_start), query, 1, re.I)
        return self.query(query)
    ...
    def authenticate(self):
        try:
            ...
            with warnings.catch_warnings():
                ...
                r = requests.post(('{0}/servicesNS/admin/search/auth/login'.format(self.uri)),
                  data={'username':self.username, 
                 'password':self.password},
                  verify=False,
                  timeout=(self.network_timeout))
            if r.status_code != 200:
                logging.fatal('unable to log into slunk: response code {0} reason {1}'.format(r.status_code, r.reason))
                return False
    ...
    def execute_query(self, query):
        try:
            ...
            with warnings.catch_warnings():
                warnings.simplefilter('ignore')
                r = requests.post(('{0}/servicesNS/admin/search/search/jobs'.format(self.uri)),
                  verify=False,
    ...
    def is_job_completed(self):
        try:
            ...
            with warnings.catch_warnings():
                warnings.simplefilter('ignore')
                r = requests.get(('{0}/servicesNS/admin/search/search/jobs/{1}'.format(self.uri, self.search_id)),
                  headers={'Authorization': 'Splunk {0}'.format(self.session_key)},
                  verify=False,
                  timeout=(self.network_timeout))
            if r.status_code != 200:
                logging.error('unable to get status of search job {0}: response code {1} reason {2}'.format(self.search_id, r.status_code, r.reason))
                return
            m = re.search('<s:key name="isDone">([01])</s:key>', r.text, re.M)
            if not m:
    ...
    def download_search_results(self):
        try:
            ...
            with warnings.catch_warnings():
                warnings.simplefilter('ignore')
                r = requests.get(('{0}/servicesNS/admin/search/search/jobs/{1}/results'.format(self.uri, self.search_id)),
                  headers={'Authorization': 'Splunk {0}'.format(self.session_key)},
                  params={'count':'0', 
                 'output_mode':'json_rows'},
                  ...
                  timeout=(self.network_timeout))
            if r.status_code != 200:
                logging.error('unable to download results for search job {0}: response code {1} reason {2}'.format(self.search_id, r.status_code, r.reason))
                return False
            try:
                self.search_results = json.loads(r.text)
                logging.debug('downloaded {0} rows of results'.format(len(self.search_results['rows'])))
            except Exception as e:
                logging.error('unable to parse the json returned by splunk for search {0}: {1}'.format(self.search_id, str(e)))
                traceback.print_exc()
                return False
