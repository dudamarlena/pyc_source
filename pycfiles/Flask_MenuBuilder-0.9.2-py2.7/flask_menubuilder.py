...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/flask_menubuilder.py
# Compiled at: 2013-01-07 18:23:18
"""
...
class MenuBuilder(object):
    ...
    def add_menu(self, menu_id, id_=None, classes=None, visiblewhen=ANYTIME, activewhen=ANYTIME, **html_opts):
        ...
        menu = Menu(menu_id, id_=id_, classes=classes, visiblewhen=visiblewhen, activewhen=activewhen, **html_opts)
        menu.builder = self.builder
        self.menus[menu_id] = menu
        return menu

    def add_menu_entry(self, menu_id, title, endpoint, priority=0, activewhen=REQUEST_MATCHES_ENDPOINT, visiblewhen=ANYTIME, classes=None, li_classes=None, id_=None, **html_opts):
        if menu_id not in self.menus:
            msg = ('{0!r} menu does not exist yet. Please create it first with Menubuilder.add_menu()').format(menu_id)
            if self.app.debug:
                raise RuntimeError(msg)
            raise RuntimeWarning(msg)
        menu_item = MenuItem(title, endpoint, priority=priority, activewhen=activewhen, visiblewhen=visiblewhen, classes=classes, li_classes=li_classes, id_=id_, **html_opts)
    ...
    def __add_menu_item(self, menu_id, menu_item):
        if not isinstance(menu_item, MenuItem):
            if self.app.debug:
                raise RuntimeError('The menu item being added is not a MenuItem', type(menu_item))
            raise RuntimeWarning('The menu item being added is not a MenuItem', type(menu_item))
        if menu_item.endpoint in self.menus[menu_id].entries:
            msg = ("There's already a menu entry for the endpoint {0} in {1!r}").format(menu_item.endpoint, self.menus[menu_id])
            if self.app.debug:
...
class RenderItem(object):
    ...
    def __init__(self, classes=None, visiblewhen=ANYTIME, activewhen=REQUEST_MATCHES_ENDPOINT, **html_opts):
    ...
    def active_state(self):
        if self.activewhen is NEVER:
            return 'inactive'
        if self.activewhen is ANYTIME:
            return 'active'
        if callable(self.activewhen) and self.activewhen(self):
            return 'active'
        return 'inactive'
...
class Menu(RenderItem):
    ...
    def __init__(self, name, id_=None, classes=None, visiblewhen=ANYTIME, activewhen=ANYTIME, **html_opts):
        super(Menu, self).__init__(classes=classes, activewhen=activewhen, visiblewhen=visiblewhen, **html_opts)
        self.name = name
        self.id_ = id_
        self.entries = {}

    def add_menu_entry(self, title, endpoint, priority=0, activewhen=REQUEST_MATCHES_ENDPOINT, visiblewhen=ANYTIME, classes=None, id_=None, **html_opts):
        menu_item = MenuItem(title, endpoint, priority=priority, activewhen=activewhen, visiblewhen=visiblewhen, classes=classes, id_=id_, **html_opts)
    ...
    def __add_menu_item(self, menu_item):
        ...
        if menu_item.endpoint in self.entries:
            if self.app.debug:
                raise RuntimeError("There's already a menu entry for the endpoint %r" % menu_item.endpoint, self.entries[menu_item.endpoint])
            raise RuntimeWarning("There's already a menu entry for the endpoint %r" % menu_item.endpoint, self.entries[menu_item.endpoint])
...
class MenuItem(RenderItem):
    ...
    def __init__(self, title, endpoint, priority=0, visiblewhen=ANYTIME, activewhen=REQUEST_MATCHES_ENDPOINT, classes=None, id_=None, li_classes=None, **html_opts):
        super(MenuItem, self).__init__(classes=classes, activewhen=activewhen, visiblewhen=visiblewhen, **html_opts)
        self.id_ = id_
        self.title = title
    ...
    def render(self):
        ...
        return self.builder.li(self.builder.a(self.title, **self.render_params), class_=(' ').join(filter(None, [self.li_classes, self.active_state])))
    ...
    def __unicode__(self):
        return '<MenuItem title="%(title)s" endpoint="%(endpoint)s" priority=%(priority)s>' % self.__dict__
...
class MenuItemContent(MenuItem):
    ...
    def __init__(self, content, title=None, endpoint=None, priority=0, activewhen=NEVER, visiblewhen=ANYTIME, classes=None, id_=None, li_classes=None, is_link=True, **html_opts):
        MenuItem.__init__(self, title, endpoint, priority=priority, activewhen=activewhen, visiblewhen=visiblewhen, classes=classes, id_=id_, li_classes=li_classes, **html_opts)
    ...
    def render(self):
        ...
        return self.builder.li(element(content, **render_params), class_=(' ').join(filter(None, [self.li_classes, self.active_state])))