...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pywcs\tests\test_pickle.py
...
def test_dist():
    ...
    s = pickle.dumps(wcs1)
    wcs2 = pickle.loads(s)
    x = np.random.rand(65536, wcs1.wcs.naxis)
...
def test_sip():
    ...
    hdulist = pyfits.open(os.path.join(ROOT_DIR, 'data', 'sip.fits'), ignore_missing_end=True)
...
def test_sip2():
    ...
    hdulist = pyfits.open(os.path.join(ROOT_DIR, 'data', 'sip2.fits'), ignore_missing_end=True)
    wcs1 = pywcs.WCS(hdulist[0].header)
    assert wcs1.sip is not None
    s = pickle.dumps(wcs1)
    wcs2 = pickle.loads(s)
