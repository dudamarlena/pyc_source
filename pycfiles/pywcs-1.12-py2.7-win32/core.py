...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pywcs\core.py
...
if sys.version_info[0] >= 3:
    ...
    import _docutil as __, _pywcs
assert _pywcs._sanity_check(), 'PyWcs did not pass its sanity check for your build on your platform.\nPlease send details about your build and platform to mdroe@stsci.edu'
if sys.version_info[0] >= 3:
    string_types = (
     bytes,)
else:
...
class Wcsprm(_pywcs._Wcsprm):
    pass


for key, val in _pywcs.__dict__.items():
    if key.startswith('WCSSUB') or key.startswith('WCSHDR') or key.startswith('WCSHDO'):
        locals()[key] = val
...
class WCS(WCSBase):
    ...
    def __init__(self, header=None, fobj=None, key=' ', minerr=0.0, relax=False, naxis=None, keysel=None, colsel=None, fix=True):
        """
        ...
        **Exceptions:**
        ...
        if header is None:
            ...
            if isinstance(header, string_types):
                ...
                raise TypeError('header must be a string or a pyfits.Header object')
            try:
                wcsprm = _pywcs._Wcsprm(header=header_string, key=key, relax=relax, keysel=keysel_flags, colsel=colsel)
            except _pywcs.NoWcsKeywordsFoundError:
                if colsel is None:
                    wcsprm = _pywcs._Wcsprm(header=None, key=key, relax=relax, keysel=keysel_flags, colsel=colsel)
                else:
                    raise
    ...
    def __copy__(self):
        ...
        WCSBase.__init__(new_copy, self.sip, (
         self.cpdis1, self.cpdis2), self.wcs, (
    ...
    def __deepcopy__(self, memo):
        ...
        WCSBase.__init__(new_copy, copy.deepcopy(self.sip, memo), (
         ...
         copy.deepcopy(self.cpdis2, memo)), copy.deepcopy(self.wcs, memo), (
    ...
    def _read_det2im_kw(self, header, fobj, err=0.0):
        ...
        if fobj is None:
            ...
            if not HAS_PYFITS:
                raise ImportError('pyfits is required to use Paper IV lookup tables')
                ...
                if distortion in header:
                    ...
                    if dis == 'lookup':
                        if fobj is not None and not HAS_PYFITS:
                            raise ImportError('pyfits is required to use Paper IV lookup tables')
                        assert isinstance(fobj, pyfits.HDUList), 'A pyfits HDUList is required for Lookup table distortion.'
                        dp = (d_kw + str(i)).strip()
    ...
    def _old_style_d2im(self, header, fobj, axiscorr):
        warnings.warn('The use of ``AXISCORR`` for D2IM correction has been deprecated.The new style of this correction is described atPyWCS will read in files with ``AXISCORR`` but to_fits() will writeout files in the new style', DeprecationWarning)
        cpdis = [None, None]
        crpix = [0.0, 0.0]
        crval = [0.0, 0.0]
        cdelt = [1.0, 1.0]
        ...
        if self.det2im1 is None and self.det2im2 is None:
            ...
            def write_d2i(num, det2im):
                if det2im is None:
                    ...
                    hdulist[0].header.update('%s%d' % (dist, num), value='LOOKUP', comment='Detector to image correction type')
                    hdulist[0].header.update('%s%d.EXTVER' % (d_kw, num), value=num, comment='Version number of WCSDVARR extension')
                    hdulist[0].header.update('%s%d.NAXES' % (d_kw, num), value=len(det2im.data.shape), comment='Number of independent variables in d2im function')
                    for i in range(det2im.data.ndim):
                        hdulist[0].header.update('%s%d.AXIS.%d' % (d_kw, num, i + 1), value=i + 1, comment='Axis number of the jth independent variable in a d2im function')
        ...
        if self.cpdis1 is None and self.cpdis2 is None:
            ...
            def write_dist(num, cpdis):
                if cpdis is None:
                    ...
                    hdulist[0].header.update('%s%d' % (dist, num), value='LOOKUP', comment='Prior distortion function type')
                    hdulist[0].header.update('%s%d.EXTVER' % (d_kw, num), value=num, comment='Version number of WCSDVARR extension')
                    hdulist[0].header.update('%s%d.NAXES' % (d_kw, num), value=len(cpdis.data.shape), comment='Number of independent variables in distortion function')
                    for i in range(cpdis.data.ndim):
                        hdulist[0].header.update('%s%d.AXIS.%d' % (d_kw, num, i + 1), value=i + 1, comment='Axis number of the jth independent variable in a distortion function')

                    image = pyfits.ImageHDU(cpdis.data, name='WCSDVARR')
                    header = image.header
                    header.update('CRPIX1', value=cpdis.crpix[0], comment='Coordinate system reference pixel')
                    header.update('CRPIX2', value=cpdis.crpix[1], comment='Coordinate system reference pixel')
                    header.update('CRVAL1', value=cpdis.crval[0], comment='Coordinate system value at reference pixel')
                    header.update('CRVAL2', value=cpdis.crval[1], comment='Coordinate system value at reference pixel')
                    header.update('CDELT1', value=cpdis.cdelt[0], comment='Coordinate increment along axis')
                    header.update('CDELT2', value=cpdis.cdelt[1], comment='Coordinate increment along axis')
                    image.update_ext_version(int(hdulist[0].header[('%s%d.EXTVER' % (d_kw, num))]))
                    hdulist.append(image)
                    return
    ...
    def _read_sip_kw(self, header):
        ...
        if isinstance(header, string_types):
            ...
            if 'A_ORDER' in header and header['A_ORDER'] > 1:
                ...
                if 'B_ORDER' in header and header['B_ORDER'] > 1:
    ...
    def _array_converter(self, func, sky, *args, **kwargs):
        ...
        if len(args) == 2:
            ...
            try:
                ...
                raise TypeError('When providing two arguments, they must be (xy, origin)')
        ...
        if len(args) == 3:
            ...
            try:
                ...
                raise TypeError('When providing three arguments, they must be (x, y, origin)')
    ...
    def to_fits(self, relax=False, wkey=None):
        ...
        """
        if not HAS_PYFITS:
            raise ImportError('pyfits is required to generate a FITS file')
    ...
    def to_header(self, relax=False, wkey=None):
        ...
        if wkey:
            self.wcs.alt = wkey
        if not HAS_PYFITS:
            raise ImportError('pyfits is required to generate a FITS header')
        dosip = False
        if relax == True or relax == WCSHDO_all or relax & WCSHDO_SIP:
            dosip = True
    ...
    def __reduce__(self):
        ...
        """
        if not HAS_PYFITS:
            raise ImportError('pyfits is required for pickling support')
        hdulist = self.to_fits(relax=True)
        buffer = BytesIO()
        hdulist.writeto(buffer)
        return (
         _unpickle_wcs, (self.__class__, self.__dict__, buffer.getvalue()))
...
def find_all_wcs(header, relax=False, keysel=None):
    ...
    if isinstance(header, string_types):
        ...
        if HAS_PYFITS:
            ...
            raise TypeError('header must be a string or pyfits.Header object')
        keysel_flags = _parse_keysel(keysel)
