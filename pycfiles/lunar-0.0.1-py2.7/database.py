...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pumpkin/database.py
# Compiled at: 2014-12-16 03:50:37
""" A lightweight orm framework for sqlite.
    
"""
import copy, sqlite3, sys, threading
...
class ManyToManyField(object):
    ...
    def __init__(self, rel=None, to_table=None):
        if not rel or not to_table:
            raise DatabaseException('Many to many fields must set a rel table.')
        self.rel = rel
        self.to_table = to_table
        self.id = None
        self.self_id = None
    ...
    def _select(self):
        ...
        c = self.db.execute('select "%s" from %s where %s = "%s";' % (
         rel_id, self.rel, self_id, self.id))
        rs = c.fetchall()
        if len(rs) == 0:
            return self.db.__tabledict__[self.to_table].select().where('id=-1')
        return self.db.__tabledict__[self.to_table].select().where((' or ').join([ 'id = "%s"' % c[0] for c in rs ]))
    ...
    def append(self, ins):
        ...
        """
        self_id, rel_id = self._get_rel_names()
        c = self.db.execute('insert into %s(%s, %s) values(%s, %s);' % (
         self.rel, self_id, rel_id, self.id, ins.id), commit=True)
        return c
    ...
    def remove(self, *args):
        ...
        c = self.db.execute('delete from %s where %s = "%s" and %s;' % (
         self.rel, self.self_id, self.id,
         (' and ').join(list(args))), commit=True)
        return c
...
class Sqlite(Database):
    ...
    def create_table(self, model):
        ...
        cursor.execute('create table %s (%s);' % (model.__tablename__, (', ').join(c)))
        if model.__tablename__ not in self.__tabledict__.keys():
    ...
    def add(self, instance):
        ...
        cursor = self.conn.cursor()
        cursor.execute('insert into %s (%s) values (%s);' % (instance.__class__.__tablename__,
         (', ').join(cofk), (', ').join(cofv)))
        return cursor
...
class SelectQuery(BaseQuery):
    ...
    def sql(self):
        if self.like_pattern:
            return self.base_statement % (
             (', ').join([ str(i).strip("'").strip('"') for i in self.query
             ]).strip(','), self.klass.__tablename__, self.like_pattern)
        return self.base_statement % (
         (', ').join([ str(i).strip("'").strip('"') for i in self.query
            ]).strip(','), self.klass.__tablename__)
    ...
    def where(self, *args, **kwargs):
        ...
        if c != ((), ):
            c.extend(list(args))
        self.base_statement = 'select %s from %s where ' + (' and ').join(c) + ';'
        return self

    def _base_function(self, func):
        sql = self.base_statement % (
         func + '("' + (', ').join([ str(i).strip("'").strip('"') for i in self.query ]).strip(',') + '")', self.klass.__tablename__)
...
class UpdateQuery(BaseQuery):
    ...
    def set(self, *args, **kwargs):
        self.update_value = (' and ').join([ '"%s" = "%s"' % (k, v) for k, v in kwargs.items() ])
        return self
