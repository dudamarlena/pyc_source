...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pumpkin/template.py
# Compiled at: 2014-12-15 02:31:04
import sys, re, os
...
class Walker(object):
    ...
    def extends(self):
        ...
        """
        p = re.compile('^{%\\s+(extends\\s*(?P<parent>.*))\\s+%}')
        e = p.match(self.text)
        if not e:
            return None
        else:
    ...
    def parse(self):
        ...
        while not self.walker.empty:
            ...
            if not token:
                ...
                break
            self.writer.write(self.walker.buffer_before_token, indent, in_block_top())
            ...
            if variable:
                ...
                if keyword == 'include':
                    ...
                    self.writer.write_key((' ').join([keyword, suffix]), indent, in_block_top())
                    continue
