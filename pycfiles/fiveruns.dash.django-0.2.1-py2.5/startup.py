...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/fiveruns/dash/django/startup.py
# Compiled at: 2009-03-09 17:50:23
...
def _start():
    if configuration:
        ...
        return fiveruns.dash.start(configuration)
    else:
...
def _builtin_recipes():
    ...
    """
    recipe = fiveruns.dash.recipe('django', 'http://dash.fiveruns.com')
    ...
    recipe.percentage('orm_util', 'django.db ORM Utilization', calculation=Calculation(percentage_fn, 'orm_time', 'response_time'))
    recipe.percentage('db_util', 'django.db Backend Utilization', calculation=Calculation(percentage_fn, 'db_time', 'response_time'))
    yield recipe


def _add_framework_metadata():
    data = configuration.extra_payload_data.get('info', {})
    data.update({'framework_name': 'django', 
