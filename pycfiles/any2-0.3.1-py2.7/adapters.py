...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/any2/adapters.py
# Compiled at: 2015-11-04 17:29:41
import six, logging
...
class Listlike2List(BaseAdapter):
    ...
    def __iter__(self):
        ...
        for row in self.iterator:
            yield [ transformer.apply(item, index=i) for i, item in enumerate(row) ]
...
class List2Dict(object):
    ...
    def __iter__(self):
        ...
        for row in self.iterator:
            yield {attrname:row[index] for index, attrname in mapping.items()}
...
class Obj2List(BaseAdapter):
    ...
    def __iter__(self):
        ...
        attrs = self.attrs
        for row in self.iterator:
            yield [ transformer.apply(item, index=i) for i, item in enumerate([ recursive_getattr(row, name) for name in attrs ])
                  ]
    ...
    def get(self, column_name, default_value):
        ...
        if renderer is not None:
            if callable(renderer):
                ...
                msg = 'Renderer should be callable or string, not %s' % type(renderer)
