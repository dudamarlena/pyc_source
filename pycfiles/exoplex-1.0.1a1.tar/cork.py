...
# [GCC 8.4.0]
# Embedded file name: /home/mows/Scripts/ExoPlex/ExoPlex/burnman/eos/cork.py
...
class CORK(eos.EquationOfState):
    ...
    def volume(self, pressure, temperature, params):
        ...
        V = constants.gas_constant * temperature / pressure + (cork[1] - cork[0] * constants.gas_constant * np.sqrt(temperature) / ((constants.gas_constant * temperature + cork[1] * pressure) * (constants.gas_constant * temperature + 2.0 * cork[1] * pressure)) + cork[2] * np.sqrt(pressure) + cork[3] * pressure)
        return V
    ...
    def gibbs_free_energy(self, pressure, temperature, volume, params):
        ...
        intCpdT = params['Cp'][0] * temperature + 0.5 * params['Cp'][1] * np.power(temperature, 2.0) - params['Cp'][2] / temperature + 2.0 * params['Cp'][3] * np.sqrt(temperature) - (params['Cp'][0] * T_0 + 0.5 * params['Cp'][1] * T_0 * T_0 - params['Cp'][2] / T_0 + 2.0 * params['Cp'][3] * np.sqrt(T_0))
        intCpoverTdT = params['Cp'][0] * np.log(temperature) + params['Cp'][1] * temperature - 0.5 * params['Cp'][2] / np.power(temperature, 2.0) - 2.0 * params['Cp'][3] / np.sqrt(temperature) - (params['Cp'][0] * np.log(T_0) + params['Cp'][1] * T_0 - 0.5 * params['Cp'][2] / (T_0 * T_0) - 2.0 * params['Cp'][3] / np.sqrt(T_0))
        if params['cork_T'] == 0:
            RTlnf = 0.0
        else:
            cork = cork_variables(params['cork_params'], params['cork_P'], params['cork_T'], temperature)
            RTlnf = constants.gas_constant * temperature * np.log(1e-05 * P_relative) + cork[1] * P_relative + cork[0] / (cork[1] * np.sqrt(temperature)) * (np.log(constants.gas_constant * temperature + cork[1] * P_relative) - np.log(constants.gas_constant * temperature + 2.0 * cork[1] * P_relative)) + 2.0 / 3.0 * cork[2] * P_relative * np.sqrt(P_relative) + cork[3] / 2.0 * P_relative * P_relative
    ...
    def validate_parameters(self, params):
        ...
        if params['cork_P'] < 10000.0 or params['cork_P'] > 100000000.0:
