...
# [GCC 8.4.0]
# Embedded file name: /home/mows/Scripts/ExoPlex/ExoPlex/burnman/eos/birch_murnaghan.py
...
def bulk_modulus(volume, params):
    ...
    K = pow(1.0 + 2.0 * f, 5.0 / 2.0) * (params['K_0'] + (3.0 * params['K_0'] * params['Kprime_0'] - 5 * params['K_0']) * f + 27.0 / 2.0 * (params['K_0'] * params['Kprime_0'] - 4.0 * params['K_0']) * f * f)
    return K
...
def birch_murnaghan(x, params):
    ...
    modulus (params['K_0'])
    """
    return 3.0 * params['K_0'] / 2.0 * (pow(x, 7.0 / 3.0) - pow(x, 5.0 / 3.0)) * (1.0 - 0.75 * (4.0 - params['Kprime_0']) * (pow(x, 2.0 / 3.0) - 1.0)) + params['P_0']
...
def shear_modulus_second_order(volume, params):
    ...
    """
    x = params['V_0'] / volume
    G = params['G_0'] * pow(x, 5.0 / 3.0) * (1.0 - 0.5 * (pow(x, 2.0 / 3.0) - 1.0) * (5.0 - 3.0 * params['Gprime_0'] * params['K_0'] / params['G_0']))
    return G
...
def shear_modulus_third_order(volume, params):
    ...
    x = params['V_0'] / volume
    f = 0.5 * (pow(x, 2.0 / 3.0) - 1.0)
    G = pow(1.0 + 2.0 * f, 5.0 / 2.0) * (params['G_0'] + (3.0 * params['K_0'] * params['Gprime_0'] - 5.0 * params['G_0']) * f + (6.0 * params['K_0'] * params['Gprime_0'] - 24.0 * params['K_0'] - 14.0 * params['G_0'] + 9.0 / 2.0 * params['K_0'] * params['Kprime_0']) * f * f)
    return G
