...
# [GCC 8.4.0]
# Embedded file name: /home/mows/Scripts/ExoPlex/ExoPlex/burnman/eos/hp.py
...
class HP_TMT(eos.EquationOfState):
    ...
    def thermal_expansivity(self, pressure, temperature, volume, params):
        ...
        alpha = params['a_0'] * (C_V / C_V0) * 1.0 / ((1.0 + b * psubpth) * (a + (1.0 - a) * np.power(1 + b * psubpth, c)))
        return alpha
    ...
    def heat_capacity_p0(self, temperature, params):
        ...
        """
        Cp = params['Cp'][0] + params['Cp'][1] * temperature + params['Cp'][2] * np.power(temperature, -2.0) + params['Cp'][3] * np.power(temperature, -0.5)
        return Cp
    ...
    def gibbs_free_energy(self, pressure, temperature, volume, params):
        ...
        if pressure != params['P_0']:
            intVdP = (pressure - params['P_0']) * params['V_0'] * (1.0 - a + a * (np.power(1.0 - b * Pth, 1.0 - c) - np.power(1.0 + b * psubpth, 1.0 - c)) / (b * (c - 1.0) * (pressure - params['P_0'])))
        else:
            intVdP = 0.0
    ...
    def entropy(self, pressure, temperature, volume, params):
        ...
        dintVdpdT = params['V_0'] * params['a_0'] * params['K_0'] * a * ksi_over_ksi_0 * (np.power(1.0 + b * (pressure - params['P_0'] - Pth), 0.0 - c) - np.power(1.0 - b * Pth, 0.0 - c))
    ...
    def heat_capacity_p(self, pressure, temperature, volume, params):
        ...
        ksi_over_ksi_0 = einstein.heat_capacity_v(T, T_e, n) / einstein.heat_capacity_v(params['T_0'], T_e, n)
        dintVdpdT = params['V_0'] * params['a_0'] * params['K_0'] * a * ksi_over_ksi_0 * (np.power(1.0 + b * (pressure - params['P_0'] - Pth), 0.0 - c) - np.power(1.0 - b * Pth, 0.0 - c))
        dSdT0 = params['V_0'] * params['K_0'] * np.power(ksi_over_ksi_0 * params['a_0'], 2.0) * (np.power(1.0 + b * (pressure - params['P_0'] - Pth), -1.0 - c) - np.power(1.0 + b * -Pth, -1.0 - c))
        x = T_e / T
        dCv_einstdT = -(einstein.heat_capacity_v(T, T_e, n) * (1 - 2.0 / x + 2.0 / (np.exp(x) - 1.0)) * x / T)
        dSdT1 = -dintVdpdT * dCv_einstdT / einstein.heat_capacity_v(T, T_e, n)
        dSdT = dSdT0 + dSdT1
    ...
    def __relative_thermal_pressure(self, T, params):
        ...
        EQ 12 - 1 of Holland and Powell, 2011
        """
        return self.__thermal_pressure(T, params) - self.__thermal_pressure(params['T_0'], params)
    ...
    def __intCpdT(self, temperature, params):
        ...
        """
        return params['Cp'][0] * temperature + 0.5 * params['Cp'][1] * np.power(temperature, 2.0) - params['Cp'][2] / temperature + 2.0 * params['Cp'][3] * np.sqrt(temperature) - (params['Cp'][0] * params['T_0'] + 0.5 * params['Cp'][1] * params['T_0'] * params['T_0'] - params['Cp'][2] / params['T_0'] + 2.0 * params['Cp'][3] * np.sqrt(params['T_0']))
    ...
    def __intCpoverTdT(self, temperature, params):
        ...
        at ambient pressure [Pa]
        """
        return params['Cp'][0] * np.log(temperature) + params['Cp'][1] * temperature - 0.5 * params['Cp'][2] / np.power(temperature, 2.0) - 2.0 * params['Cp'][3] / np.sqrt(temperature) - (params['Cp'][0] * np.log(params['T_0']) + params['Cp'][1] * params['T_0'] - 0.5 * params['Cp'][2] / (params['T_0'] * params['T_0']) - 2.0 * params['Cp'][3] / np.sqrt(params['T_0']))
    ...
    def validate_parameters(self, params):
        ...
        expected_keys = [
         'H_0', 'S_0', 'V_0', 'Cp', 'a_0', 'n', 'molar_mass']
        ...
        if params['G_0'] is not float('nan') and (params['G_0'] < 0.0 or params['G_0'] > 10000000000000.0):
            warnings.warn('Unusual value for G_0', stacklevel=2)
        if params['Gprime_0'] is not float('nan') and (params['Gprime_0'] < -5.0 or params['Gprime_0'] > 10.0):
        ...
        if self.heat_capacity_p0(params['T_0'], params) < 0.0:
            warnings.warn('Negative heat capacity at T_0', stacklevel=2)
        if self.heat_capacity_p0(2000.0, params) < 0.0:
            warnings.warn('Negative heat capacity at 2000K', stacklevel=2)
        if params['a_0'] < 0.0 or params['a_0'] > 0.001:
