...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-fat64/egg/authorize/base.py
...
class BaseApi(object):
    ...
    def __init__(self, login, key, delimiter=',', encapsulator='', is_test=False, do_raise=False, async=False, uniform=False):
        """
    ...
    def asyncrequest(self, body):
        ...
        return client.getPage('https://' + self.server + self.path, method='POST', postdata=body, headers=self.headers).addCallback(self.parse_response)
    ...
    def parse_response(self, response):
        ...
        """
        return xml.to_dict(response, self.responses, self.do_raise, self.delimiter, self.encapsulator, self.uniform)