...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-fat64/egg/authorize/aim.py
...
class Api(base.BaseApi):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.required_arguments = {'x_login': self.login, 
           'x_tran_key': self.key, 
           'x_delim_char': '|', 
           'x_encap_char': '', 
           'x_delim_data': True, 
    ...
    def transaction(self, **kwargs):
        ...
        for (field, value) in kwargs.iteritems():
            if field == 'items':
                ...
                for item in value:
                    s_item = ('<|>').join(unicode(item))
                    argslist.append((field_name, s_item.encode('utf-8')))

            else:
                if field == 'authentication_indicator' or field == 'cardholder_authentication_value':
    ...
    def parse_response(self, response):
        ...
        if dict_response.code != '1':
            if self.do_raise:
                raise resp.AuthorizeError(dict_response.code, dict_response.reason_text, dict_response)
