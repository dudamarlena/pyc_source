...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/drqa/reader/predictor.py
...
class Predictor(object):
    __doc__ = 'Load a pretrained DocReader model and predict inputs on the fly.'

    def __init__(self, model=None, tokenizer=None, normalize=True, embedding_file=None, num_workers=None):
        """
        Args:
        ...
        if embedding_file:
            ...
            if num_workers is None or num_workers > 0:
                self.workers = ProcessPool(num_workers,
                  initializer=init,
                  initargs=(
    ...
    def predict_batch(self, batch, top_n=1):
        ...
        if candidates:
            candidates = [{'input':d_tokens[i],  'cands':candidates[i]} for i in range(len(candidates))]
