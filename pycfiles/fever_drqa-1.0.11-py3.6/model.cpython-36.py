...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/drqa/reader/model.py
# Compiled at: 2019-08-29 06:03:42
# Size of source mod 2**32: 18317 bytes
"""DrQA Document Reader model"""
import torch, torch.optim as optim, torch.nn.functional as F, numpy as np, logging, copy
...
class DocReader(object):
    __doc__ = 'High level model that handles intializing the underlying network\n    architecture, saving, updating examples, and predicting examples.\n    '
    ...
    def expand_dictionary(self, words):
        ...
        if len(to_add) > 0:
            ...
            self.network.embedding = torch.nn.Embedding((self.args.vocab_size), (self.args.embedding_dim),
              padding_idx=0)
    ...
    def init_optimizer(self, state_dict=None):
        ...
        if self.args.fix_embeddings:
            ...
            if self.args.optimizer == 'sgd':
                self.optimizer = optim.SGD(parameters, (self.args.learning_rate), momentum=(self.args.momentum),
                  weight_decay=(self.args.weight_decay))
            else:
                if self.args.optimizer == 'adamax':
                    self.optimizer = optim.Adamax(parameters, weight_decay=(self.args.weight_decay))
                else:
    ...
    def update(self, ex):
        ...
        if not self.optimizer:
            ...
            self.network.train()
            if self.use_cuda:
                inputs = [e if e is None else Variable(e.cuda(non_blocking=True)) for e in ex[:5]]
    ...
    def predict(self, ex, candidates=None, top_n=1, async_pool=None):
        ...
        self.network.eval()
        if self.use_cuda:
            inputs = [e if e is None else Variable(e.cuda(non_blocking=True), volatile=True) for e in ex[:5]]
        else:
        ...
        if candidates:
            args = (
             score_s, score_e, candidates, top_n, self.args.max_len)
            if async_pool:
            ...
            args = (
             score_s, score_e, top_n, self.args.max_len)
        if async_pool:
    ...
    def decode(score_s, score_e, top_n=1, max_len=None):
        ...
        for i in range(score_s.size(0)):
            ...
            if top_n == 1:
                idx_sort = [
                 np.argmax(scores_flat)]
            else:
                if len(scores_flat) < top_n:
    ...
    def save(self, filename):
        ...
        params = {'state_dict':state_dict,  'word_dict':self.word_dict, 
