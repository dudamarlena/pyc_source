...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/drqa/pipeline/drqa.py
# Compiled at: 2019-08-29 06:03:42
# Size of source mod 2**32: 11851 bytes
"""Full DrQA pipeline."""
import torch, regex, heapq, math, time, logging
...
class DrQA(object):
    ...
    def __init__(self, reader_model=None, embedding_file=None, tokenizer=None, fixed_candidates=None, batch_size=128, cuda=True, data_parallel=False, max_loaders=5, num_workers=None, db_config=None, ranker_config=None):
        ...
        self.processes = ProcessPool(num_workers,
          initializer=init,
          initargs=(
         tok_class, tok_opts, db_class, db_opts, fixed_candidates))
    ...
    def _split_doc(self, doc):
        ...
        for split in regex.split('\\n+', doc):
            ...
            if len(split) == 0:
                ...
                if len(curr) > 0:
                    if curr_len + len(split) > self.GROUP_LENGTH:
                        yield ' '.join(curr)
                        curr = []
                        curr_len = 0
                curr.append(split)
    ...
    def _get_loader(self, data, num_loaders):
        ...
        sampler = SortedBatchSampler((dataset.lengths()),
          (self.batch_size),
          shuffle=False)
    ...
    def process(self, query, candidates=None, top_n=1, n_docs=5, return_context=False):
        ...
        predictions = self.process_batch([
         query], [candidates] if candidates else None, top_n, n_docs, return_context)
    ...
    def process_batch(self, queries, candidates=None, top_n=1, n_docs=5, return_context=False):
        ...
        if len(queries) == 1:
            ranked = [
             self.ranker.closest_docs((queries[0]), k=n_docs)]
        else:
            ranked = self.ranker.batch_closest_docs(queries,
              k=n_docs, num_workers=(self.num_workers))
        ...
        for result, ex_ids, batch_size in result_handles:
            ...
            for i in range(batch_size):
                if len(score[i]) > 0:
                    item = (
                     score[i][0], ex_ids[i], s[i][0], e[i][0])
        ...
        for queue in queues:
            ...
            while len(queue) > 0:
                ...
                prediction = {'doc_id':all_docids[qidx][rel_didx], 
                 'span':s_tokens[sidx].slice(s, e + 1).untokenize(), 
                 'doc_score':float(all_doc_scores[qidx][rel_didx]), 
                 'span_score':float(score)}
                if return_context:
                    prediction['context'] = {'text':s_tokens[sidx].untokenize(),  'start':s_tokens[sidx].offsets()[s][0], 
