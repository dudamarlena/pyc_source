...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/microsoft/store/partnercenterservices/operations/getvversionprofilesmpnmpn_idmpn_id_operations.py
...
class GetvversionprofilesmpnmpnIdmpnIdOperations(object):
    """GetvversionprofilesmpnmpnIdmpnIdOperations operations.
    ...
    def value(self, version, mpn_id=None, verify_advisor=None, ms_correlation_id=None, ms_request_id=None, custom_headers=None, raw=False, **operation_config):
        """Gets the MPN profile.

        :param version: Possible values include: '1.0', '1'
        :type version: str
        :param mpn_id:
        :type mpn_id: str
        :param verify_advisor:
        :type verify_advisor: bool
        ...
        if mpn_id is not None:
            query_parameters['mpnId'] = self._serialize.query('mpn_id', mpn_id, 'str')
        if verify_advisor is not None:
            query_parameters['verifyAdvisor'] = self._serialize.query('verify_advisor', verify_advisor, 'bool')
        header_parameters = {}
        ...
        if response.status_code in (200, 201):
            deserialized = self._deserialize('MicrosoftPartnerSdkContractsV1MpnProfile', response)
        if raw:
    ...
    value.metadata = {'url': '/v{version}/profiles/mpn'}