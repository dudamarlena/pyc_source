...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/dowser/__init__.py
# Compiled at: 2014-01-14 16:41:04
__import__('pkg_resources').declare_namespace(__name__)
import cgi, gc, os, pkg_resources
...
method_types = [
 type(tuple.__le__),
 type([1].__le__),
 type(sys.getcheckinterval),
 type(cgi.FieldStorage.getfirst)]
...
def template(name, **params):
    p = {'maincss': url('/main.css'), 'home': url('/')}
    p.update(params)
...
class Root:
    ...
    def index(self, floor=0):
        ...
        for typename in typenames:
            ...
            if maxhist > int(floor):
                row = '<div class="typecount">%s<br /><img class="chart" src="%s" /><br />Min: %s Cur: %s Max: %s <a href="%s">TRACE</a></div>' % (
                 cgi.escape(typename),
    ...
    def trace(self, typename, objid=None):
        ...
        return template('trace.html', output=('\n').join(rows), typename=cgi.escape(typename), objid=str(objid or ''))
    ...
    def trace_all(self, typename):
        ...
        for obj in gc.get_objects():
            ...
            if objtype.__module__ + '.' + objtype.__name__ == typename:
                rows.append("<p class='obj'>%s</p>" % ReferrerTree(obj).get_repr(obj))

        if not rows:
            rows = [
             '<h3>The type you requested was not found.</h3>']
        return rows
    ...
    def trace_one(self, typename, objid):
        ...
        for obj in all_objs:
            if id(obj) == objid:
                ...
                if objtype.__module__ + '.' + objtype.__name__ != typename:
                    rows = [
                     '<h3>The object you requested is no longer of the correct type.</h3>']
                else:
                    ...
                    rows.append('<p class="desc"><a href="%s">Show the entire tree</a> of reachable objects</p>' % url('/tree/%s/%s' % (typename, objid)))
        ...
        if not rows:
            rows = [
             '<h3>The object you requested was not found.</h3>']
        return rows
    ...
    def tree(self, typename, objid):
        ...
        if not rows:
            rows = [
             '<h3>The object you requested was not found.</h3>']
        params = {'output': ('\n').join(rows), 'typename': cgi.escape(typename), 
           'objid': str(objid)}
...
try:
    ...
    cherrypy.config.update({'/': {'log_debug_info_filter.on': False}, '/main.css': {'static_filter.on': True, 
...
class ReferrerTree(reftree.Tree):
    ...
    def _gen(self, obj, depth=0):
        ...
        for ref in refiter:
            ...
            if id(ref) in self.seen:
                yield (
                 depth, id(ref), 'see %s above' % id(ref))
            else:
    ...
    def get_repr(self, obj, referent=None):
        ...
        return '<a class="objectid" href="%s">%s</a> <span class="typename">%s</span>%s<br /><span class="repr">%s</span>' % (
