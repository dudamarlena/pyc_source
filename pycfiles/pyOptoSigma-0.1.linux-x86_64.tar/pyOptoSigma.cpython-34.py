...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.4/dist-packages/pyOptoSigma.py
# Compiled at: 2016-05-28 04:51:01
# Size of source mod 2**32: 36450 bytes
import sys
...
class Bad_Command_Parameter_or_Timing(Exception):
    __doc__ = "Raised if the command, parameter, or timing is wrong.\n    \n    This exception raises when the controller responds 'NG'.\n    "


class Not_Supported(Exception):
    __doc__ = ' Raised if the command is not supported by the controller.\n    '
...
class Controller_Modes(Enum):
    __doc__ = " Instruction set of controller's operation.\n    "
    SHOT = 0
    SHOT_enhanced = 1
    HIT = 2
...
class Controller:
    ...
    def __init__(self, ctype):
        ...
        if self.ctype is Controllers.SHOT_302GS or self.ctype is Controllers.SHOT_304GS:
            ...
            if ctype is Controllers.SHOT_702:
                self.baudrate = 38400
                self.cmode = Controller_Modes.SHOT
            else:
    ...
    def get_support_baudrates(self):
        if self._Controller__is_30X():
            return (4800, 9600, 19200, 38400)
        if self._Controller__is_70X():
            return (38400, )
        return ()
...
class Session:
    ...
    def connect(self, portname='/dev/ttyUSB0'):
        ...
        """
        self.port = serial.Serial(port=portname, baudrate=self.controller.baudrate, bytesize=self.controller.databit, parity=self.controller.parity, stopbits=self.controller.stopbit, timeout=self.controller.read_timeout, writeTimeout=self.controller.write_timeout, rtscts=self.controller.rtscts)
        self.connected = True
    ...
    def __send(self, command, no_response=True):
        ...
        if self.controller.comm_ack is Comm_ack.MAIN or not no_response:
            response = self.port.readline()[:self._Session__d].decode('utf-8')
            self._Session__print(command + ' >> ' + response, level=2)
            if response == 'NG':
                raise Bad_Command_Parameter_or_Timing(command + ' failed.')
            return response
        self._Session__print('[SUB] ' + command, level=2)
    ...
    def is_busy(self, stage=None, func=lambda x: x):
        ...
        if self.controller.is_SHOT():
            return self._Session__is_busy_shot()
    ...
    def __is_busy_shot(self):
        ...
        return self._Session__send('!:', no_response=False).startswith('B')
    ...
    def is_ready(self, stage=None, func=lambda x: x):
        ...
        """
        if self.controller.is_SHOT():
            return self._Session__is_ready_shot()
        raise NotImplemented()
    ...
    def __is_ready_shot(self):
        ...
        return self._Session__send('!:', no_response=False).startswith('R')

    def __wait_for_ready(self, stage=None, func=lambda x: x):
        self._Session__print('Waiting.')
        while self.is_busy(stage=stage, func=func):
            self._Session__print('     sleep ' + str(self.wait_time), level=2)
            time.sleep(self.wait_time)

    def __load_divisions(self):
        if self.controller.is_SHOT():
            self._Session__load_divisions_shot()
        else:
            raise NotImplemented()

    def __load_divisions_shot(self):
        self._Session__print('Load division settings.')
        if not self.controller.is_support_Qu():
            raise Not_Supported()
        if not self.controller.is_support_QuS():
            raise Not_Supported()
        response = self._Session__send('?:SW', no_response=False)
    ...
    def reset(self, stage=1, all_stages=False, mechanical=False, wait_for_finish=False):
        ...
        """
        if self.controller.is_SHOT():
            self._Session__reset_shot(stage, all_stages, mechanical, wait_for_finish)
    ...
    def __reset_shot(self, stage, all_stages, mechanical, wait_for_finish):
        if mechanical:
            if not self.controller.is_support_H():
                raise Not_Supported()
            if all_stages:
                self._Session__print('Mechanical reset, all stages.')
                self._Session__send('H:W')
            else:
                self._Session__print('Mechanical reset, #' + str(stage))
                self._Session__send('H:' + str(stage))
        else:
            if not (self.controller.is_support_A() and self.controller.is_support_G()):
                raise Not_Supported()
            if all_stages:
                self._Session__print('Electrical reset, ' + str(len(self.stages)) + ' stages')
                com = ['A:W']
                for i in self.stages:
                    com.append(['+P0'])

                self._Session__send(''.join(com))
                self._Session__send('G:')
            else:
                self._Session__print('Electrical reset, #' + str(stage))
                self._Session__send('A:' + str(stage) + '+P0')
                self._Session__send('G:')
        if wait_for_finish:
            self._Session__wait_for_ready()
    ...
    def move(self, stage=1, amount=0, in_pulse=False, absolute=False, wait_for_finish=True):
        ...
        """
        if self.controller.is_SHOT():
            self._Session__move_shot(stage, amount, in_pulse, absolute, wait_for_finish)
        else:
            raise NotImplemented()
        ...
        if isinstance(amount, (list, tuple)):
            ...
            if not in_pulse:
                ...
                if not self.divisions_loaded:
                    self._Session__load_divisions()
                amount = [v * d / get_value_per_pulse(p) for p, v, d in zip(self.stages, amount, self.divisions)]
            com.extend([self._Session__direction(v) + 'P' + str(abs(v)) for v in [int(m) for m in amount]])
            self._Session__print(' '.join(msg))
            self._Session__send(''.join(com))
            self._Session__send('G:')
        else:
            com.append(str(stage))
            com.append(self._Session__direction(amount))
            com.append('P')
            ...
            if not in_pulse:
                ...
                if not self.divisions_loaded:
                    self._Session__load_divisions()
                self._Session__print('Amount: ' + str(amount) + ' Base rate: ' + str(get_value_per_pulse(self.stages[(stage - 1)])) + ' Devision:' + str(self.divisions[(stage - 1)]), level=3)
    ...
    def jog(self, stage=1, directions=1):
        r"""Jog drive. Continue moving before arriving the limit point or system limit.
        ...
        """
        if self.controller.is_SHOT():
            self._Session__jog_shot(stage, directions)
        else:
            raise NotImplemented()
    ...
    def __jog_shot(self, stage, directions):
        ...
        if isinstance(directions, (tuple, list)):
            arg = [self._Session__direction(d) for d in directions]
            com = 'J:W' + ''.join(arg)
            self._Session__print('Jog drive, multi-stages.')
            self._Session__send(com)
            self._Session__send('G:')
        else:
            com = 'J:' + str(stage) + self._Session__direction(directions)
            self._Session__print('Jog drive, single stage.')
            self._Session__send(com)
            self._Session__send('G:')
    ...
    def stop(self, stage=1, all_stages=False, emergency=False):
        ...
        """
        if self.controller.is_SHOT():
            self._Session__stop_shot(stage, all_stages, emergency)
        else:
            raise NotImplemented()

    def __stop_shot(self, stage, all_stages, emergency):
        if emergency:
            self._Session__send('L:E')
            self._Session__print('Emergency stop.')
            return
        if not self.controller.is_support_L():
            raise Not_Supported()
        if all_stages:
            self._Session__print('Stop All stages.')
            self._Session__send('L:W')
        else:
            self._Session__print('Stop stage #' + str(stage))
            self._Session__send('L:' + str(stage))
        self._Session__wait_for_ready()
    ...
    def set_origin(self, stage=1, all_stages=False):
        ...
        """
        if self.controller.is_SHOT():
            self._Session__set_origin_shot(stage, all_stages)
        else:
            raise NotImplemented()
    ...
    def __set_origin_shot(self, stage, all_stages):
        if not self.controller.is_support_R():
            raise Not_Supported()
        if all_stages:
            self._Session__print('Set origin for all stages.')
            self._Session__send('R:W')
        else:
            self._Session__print('Set origin for stage #' + str(stage))
            self._Session__send('R:' + str(stage))
    ...
    def set_speed(self, stage=1, S=1000, F=10000, R=100):
        ...
        """
        if self.controller.is_SHOT():
            self._Session__set_speed_shot(stage, S, F, R)
        else:
            raise NotImplemented()
    ...
    def __set_speed_shot(self, stage, S, F, R):
        ...
        if isinstance(S, (tuple, list)):
            self._Session__print('Set speed of multi-stages.')
            SFRs = [self._Session__check_SFR(s, f, r) for s, f, r in zip(S, F, R)]
            arg = ['S' + str(s) + 'F' + str(f) + 'R' + str(r) for s, f, r in SFRs]
            com = 'D:W' + ''.join(arg)
            self._Session__send(com)
        else:
            self._Session__print('Set speed of stage #' + str(stage))
            s, f, r = self._Session__check_SFR(S, F, R)
    ...
    def set_speed_reset_drive(self, stage=1, S=1000, F=10000, R=100):
        ...
        """
        if self.controller.is_SHOT():
            self._Session__set_speed_reset_drive_shot(stage, S, F, R)
        else:
            raise NotImplemented()
    ...
    def set_excitation_mode(self, stage=1, mode=Excitation.Hold, all_stages=False):
        ...
        """
        if self.controller.is_SHOT():
            self._Session__set_excitation_mode_shot(stage, mode, all_stages)
        else:
            raise NotImplemented()
    ...
    def __set_excitation_mode_shot(self, stage, mode, all_stages):
        if not self.controller.is_support_C():
            raise Not_Supported()
        if all_stages:
            self._Session__print('Set all stages to ' + mode.name)
            self._Session__send('C:W' + str(int(mode)))
        else:
            self._Session__print('Set stage #' + str(stage) + ' to ' + mode.name)
            self._Session__send('C:' + str(stage) + str(int(mode)))
    ...
    def set_division(self, stage=1, division=2):
        ...
        """
        if self.controller.is_SHOT():
            self._Session__set_division_shot(stage, division)
        else:
            raise NotImplemented()
    ...
    def __set_division_shot(self, stage, division):
        if not self.controller.is_support_S():
            raise Not_Supported()
        if division not in self.controller.get_support_devisions():
            raise ValueError('Unsupported division value.')
        self._Session__print('Set division of #' + str(stage) + ' to ' + str(division))
        self._Session__send('S:' + str(stage) + str(division))
        self._Session__load_divisions()
    ...
    def get_status(self):
        ...
        """
        if self.controller.is_SHOT():
            return self._Session__get_status_shot()
        raise NotImplemented()

    def __get_status_shot(self):
        if not self.controller.is_support_Q():
            raise Not_Supported()
        response = self._Session__send('Q:', no_response=False)
        data = response.split(',')
