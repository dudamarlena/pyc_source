...
# [GCC 8.4.0]
# Embedded file name: /home/marcofavorito/.pyenv/versions/3.7.6/lib/python3.7/site-packages/gym_breakout_pygame/breakout_env.py
# Compiled at: 2020-04-04 09:47:59
# Size of source mod 2**32: 23537 bytes
"""
...
class BreakoutConfiguration(object):
    ...
    def __init__(self, brick_rows: int=3, brick_cols: int=3, paddle_width: int=80, paddle_height: int=10, paddle_speed: int=10, brick_width: int=60, brick_height: int=12, brick_xdistance: int=20, brick_reward: float=5.0, step_reward: float=-0.01, game_over_reward: float=-10.0, ball_radius: int=10, resolution_x: int=20, resolution_y: int=10, horizon: Optional[int]=None, fire_enabled: bool=False, ball_enabled: bool=True, complex_bump: bool=False, deterministic: bool=True):
        if not brick_cols >= 3:
            raise AssertionError('The number of columns must be at least three.')
        else:
            assert brick_rows >= 1, 'The number of columns must be at least three.'
            if not fire_enabled:
                if not ball_enabled:
                    raise AssertionError('Either fire or ball must be enabled.')
...
class BrickGrid(PygameDrawable):
    ...
    def __init__(self, brick_cols: int, brick_rows: int, brick_width: int, brick_height: int, brick_xdistance: int):
...
class BreakoutState(object):
    ...
    def __init__(self, breakout_configuration: BreakoutConfiguration):
        ...
        self.brick_grid = BrickGrid(self.config.brick_cols, self.config.brick_rows, self.config.brick_width, self.config.brick_height, self.config.brick_xdistance)
...
class Breakout(gym.Env, ABC):
    __doc__ = 'A generic Breakout env. The feature space must be define in subclasses.'
