...
# Embedded file name: /src/c2cgeoportal/scaffolds/update/CONST_alembic/main/versions/415746eb9f6_changes_for_v2.py
# Compiled at: 2019-04-23 07:29:02
__doc__ = 'Changes to start the implementation of the version 2\n\nRevision ID: 415746eb9f6\nRevises: None\nCreate Date: 2014-10-23 16:00:47.940216\n'
...
def upgrade():
    ...
    if type(engine).__name__ != 'MockConnection' and op.get_context().dialect.has_table(engine, 'interface', schema=schema):
        return
    op.drop_table('user_functionality', schema=schema)
    op.create_table('interface', Column('id', Integer, primary_key=True), Column('name', Unicode), Column('description', Unicode), schema=schema)
    op.create_table('interface_layer', Column('interface_id', Integer, ForeignKey(schema + '.interface.id'), primary_key=True), Column('layer_id', Integer, ForeignKey(schema + '.layer.id'), primary_key=True), schema=schema)
    op.create_table('interface_theme', Column('interface_id', Integer, ForeignKey(schema + '.interface.id'), primary_key=True), Column('theme_id', Integer, ForeignKey(schema + '.theme.id'), primary_key=True), schema=schema)
    op.create_table('layerv1', Column('id', Integer, ForeignKey(schema + '.layer.id'), primary_key=True), Column('is_checked', Boolean, default=True), Column('icon', Unicode), Column('layer_type', Unicode(12)), Column('url', Unicode), Column('image_type', Unicode(10)), Column('style', Unicode), Column('dimensions', Unicode), Column('matrix_set', Unicode), Column('wms_url', Unicode), Column('wms_layers', Unicode), Column('query_layers', Unicode), Column('kml', Unicode), Column('is_single_tile', Boolean), Column('legend', Boolean, default=True), Column('legend_image', Unicode), Column('legend_rule', Unicode), Column('is_legend_expanded', Boolean, default=False), Column('min_resolution', Float), Column('max_resolution', Float), Column('disclaimer', Unicode), Column('identifier_attribute_field', Unicode), Column('exclude_properties', Unicode), Column('time_mode', Unicode(8)), schema=schema)
    op.execute("UPDATE ONLY %(schema)s.treeitem SET type = 'layerv1' WHERE type='layer'" % {'schema': schema})
    op.execute('INSERT INTO %(schema)s.layerv1 (id, is_checked, icon, layer_type, url, image_type, style, dimensions, matrix_set, wms_url, wms_layers, query_layers, kml, is_single_tile, legend, legend_image, legend_rule, is_legend_expanded, min_resolution, max_resolution, disclaimer, identifier_attribute_field, exclude_properties, time_mode) (SELECT id, "isChecked" AS is_checked, icon, "layerType" AS layer_type, url, "imageType" AS image_type, style, dimensions, "matrixSet" AS matrix_set, "wmsUrl" AS wms_url, "wmsLayers" AS wms_layers, "queryLayers" AS query_layers, kml, "isSingleTile" AS is_single_tile, legend, "legendImage" AS legend_image, "legendRule" AS legend_rule, "isLegendExpanded" AS is_legend_expanded, "minResolution" AS min_resolution, "maxResolution" AS max_resolution, disclaimer, "identifierAttributeField" AS identifier_attribute_field, "excludeProperties" AS exclude_properties, "timeMode" AS time_mode FROM %(schema)s.layer)' % {'schema': schema})
    ...
    op.execute('INSERT INTO %(schema)s.interface_layer (layer_id, interface_id) (SELECT l.id AS layer_id, i.id AS interface_id FROM %(schema)s.layer AS l, %(schema)s.interface AS i WHERE i.name in (\'main\', \'edit\', \'routing\') AND l."inDesktopViewer")' % {'schema': schema})
    op.execute('INSERT INTO %(schema)s.interface_layer (layer_id, interface_id) (SELECT l.id AS layer_id, i.id AS interface_id FROM %(schema)s.layer AS l, %(schema)s.interface AS i WHERE i.name = \'mobile\' AND l."inMobileViewer")' % {'schema': schema})
    op.execute('INSERT INTO %(schema)s.interface_theme (theme_id, interface_id) (SELECT l.id AS theme_id, i.id AS interface_id FROM %(schema)s.theme AS l, %(schema)s.interface AS i WHERE i.name in (\'main\', \'edit\', \'routing\') AND l."inDesktopViewer")' % {'schema': schema})
    op.execute('INSERT INTO %(schema)s.interface_theme (theme_id, interface_id) (SELECT l.id AS theme_id, i.id AS interface_id FROM %(schema)s.theme AS l, %(schema)s.interface AS i WHERE i.name = \'mobile\' AND l."inMobileViewer")' % {'schema': schema})
    ...
    op.create_table('layer_internal_wms', Column('id', Integer, ForeignKey(schema + '.layer.id'), primary_key=True), Column('layer', Unicode), Column('image_type', Unicode(10)), Column('style', Unicode), Column('time_mode', Unicode(8)), schema=schema)
    op.create_table('layer_external_wms', Column('id', Integer, ForeignKey(schema + '.layer.id'), primary_key=True), Column('url', Unicode), Column('layer', Unicode), Column('image_type', Unicode(10)), Column('style', Unicode), Column('is_single_tile', Boolean), Column('time_mode', Unicode(8)), schema=schema)
    op.create_table('layer_wmts', Column('id', Integer, ForeignKey(schema + '.layer.id'), primary_key=True), Column('url', Unicode), Column('layer', Unicode), Column('style', Unicode), Column('matrix_set', Unicode), schema=schema)
    op.create_table('ui_metadata', Column('id', Integer, primary_key=True), Column('name', Unicode), Column('value', Unicode), Column('description', Unicode), Column('item_id', Integer, ForeignKey(schema + '.treeitem.id'), nullable=False), schema=schema)
    op.create_table('wmts_dimension', Column('id', Integer, primary_key=True), Column('name', Unicode), Column('value', Unicode), Column('description', Unicode), Column('layer_id', Integer, ForeignKey(schema + '.layer_wmts.id'), nullable=False), schema=schema)
...
def downgrade():
    ...
    op.execute('UPDATE ONLY %(schema)s.theme AS t SET "inMobileViewer" = TRUE FROM %(schema)s.interface AS i, %(schema)s.interface_theme AS it WHERE i.name = \'mobile\' AND i.id = it.interface_id AND it.theme_id = t.id' % {'schema': schema})
    op.execute('UPDATE ONLY %(schema)s.theme AS t SET "inDesktopViewer" = TRUE FROM %(schema)s.interface AS i, %(schema)s.interface_theme AS it WHERE i.name = \'main\' AND i.id = it.interface_id AND it.theme_id = t.id' % {'schema': schema})
    op.execute('UPDATE ONLY %(schema)s.layer AS l SET "inMobileViewer" = TRUE FROM %(schema)s.interface AS i, %(schema)s.interface_layer AS il WHERE i.name = \'mobile\' AND i.id = il.interface_id AND il.layer_id = l.id' % {'schema': schema})
    op.execute('UPDATE ONLY %(schema)s.layer AS l SET "inDesktopViewer" = TRUE FROM %(schema)s.interface AS i, %(schema)s.interface_layer AS il WHERE i.name = \'main\' AND i.id = il.interface_id AND il.layer_id = l.id' % {'schema': schema})
    ...
    op.execute('UPDATE %(schema)s.layer AS l SET (id, "isChecked", icon, "layerType", url, "imageType", style, dimensions, "matrixSet", "wmsUrl", "wmsLayers", "queryLayers", kml, "isSingleTile", legend, "legendImage", "legendRule", "isLegendExpanded", "minResolution", "maxResolution", disclaimer, "identifierAttributeField", "excludeProperties", "timeMode") = (o.id, o.is_checked, o.icon, o.layer_type, o.url, o.image_type, o.style, o.dimensions, o.matrix_set, o.wms_url, o.wms_layers, o.query_layers, o.kml, o.is_single_tile, o.legend, o.legend_image, o.legend_rule, o.is_legend_expanded, o.min_resolution, o.max_resolution, o.disclaimer, o.identifier_attribute_field, o.exclude_properties, o.time_mode ) FROM %(schema)s.layerv1 AS o WHERE o.id = l.id' % {'schema': schema})
