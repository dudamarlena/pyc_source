...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/redturtle/speedupui/pathbar/browser/navigation.py
# Compiled at: 2009-09-12 09:13:49
...
class PhysicalNavigationBreadcrumbs(BasePhysicalNavigationBreadcrumbs):
    ...
    def breadcrumbs(self):
        ...
        if container is None:
            ...
            return ({'id': context.getId(), 'absolute_url': item_url, 'Title': utils.pretty_title_or_id(context, context), 'is_folderish': self.isFolderish(context), 'edit_perm': checkPermission(permissions.ModifyPortalContent, context), 'contents_perm': checkPermission(permissions.ListFolderContents, context), 'add_perm': addPerm, 'addableTypes': addPerm and context.getAllowedTypes() or []},)
        ...
        if not utils.isDefaultPage(context, request) and not rootPath.startswith(itemPath):
            addPerm = checkPermission(permissions.AddPortalContent, context)
            base += ({'id': context.getId(), 'absolute_url': item_url, 'Title': utils.pretty_title_or_id(context, context), 'is_folderish': self.isFolderish(context), 'edit_perm': checkPermission(permissions.ModifyPortalContent, context), 'contents_perm': checkPermission(permissions.ListFolderContents, context), 'add_perm': addPerm, 'addableTypes': addPerm and context.getAllowedTypes() or []},)
        return base