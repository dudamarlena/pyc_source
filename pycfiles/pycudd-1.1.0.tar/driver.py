...
# Embedded file name: build/bdist.linux-armv7l/egg/pycuda/driver.py
...
try:
    ...
    if '_v2' in str(e):
        ...
        warn('Failed to import the CUDA driver interface, with an error message indicating that the version of your CUDA header does not match the version of your CUDA driver.')
    raise
...
def _add_functionality():
    ...
    def function_call_pre_v4(func, *args, **kwargs):
        ...
        if kwargs:
            raise ValueError('extra keyword arguments: %s' % (',').join(six.iterkeys(kwargs)))
        if block is None:
    ...
    def function_prepare_pre_v4(func, arg_types, block=None, shared=None, texrefs=[]):
        ...
        if block is not None:
            warn('setting the block size in Function.prepare is deprecated', DeprecationWarning, stacklevel=2)
            func._set_block_shape(*block)
        if shared is not None:
            warn('setting the shared memory size in Function.prepare is deprecated', DeprecationWarning, stacklevel=2)
            func._set_shared_size(shared)
    ...
    def function_prepared_call_pre_v4(func, grid, block, *args, **kwargs):
        if isinstance(block, tuple):
            ...
            from warnings import warn
            warn('Not passing the block size to prepared_call is deprecated as of version 2011.1.', DeprecationWarning, stacklevel=2)
            args = (block,) + args
        shared_size = kwargs.pop('shared_size', None)
        if shared_size is not None:
            func._set_shared_size(shared_size)
        if kwargs:
            raise TypeError('unknown keyword arguments: ' + (', ').join(six.iterkeys(kwargs)))
        from pycuda._pvt_struct import pack
    ...
    def function_prepared_timed_call_pre_v4(func, grid, block, *args, **kwargs):
        if isinstance(block, tuple):
            ...
            from warnings import warn
            warn('Not passing the block size to prepared_timed_call is deprecated as of version 2011.1.', DeprecationWarning, stacklevel=2)
            args = (block,) + args
        shared_size = kwargs.pop('shared_size', None)
        if shared_size is not None:
            func._set_shared_size(shared_size)
        ...
        def get_call_time():
            ...
            return end.time_since(start) * 0.001
    ...
    def function_prepared_async_call_pre_v4(func, grid, block, stream, *args, **kwargs):
        if isinstance(block, tuple):
            ...
            from warnings import warn
            warn('Not passing the block size to prepared_async_call is deprecated as of version 2011.1.', DeprecationWarning, stacklevel=2)
            args = (stream,) + args
            stream = block
        shared_size = kwargs.pop('shared_size', None)
        if shared_size is not None:
    ...
    def function___getattr__(self, name):
        ...
        if name == 'shared_size_bytes':
            return self._hacky_smem
        if name == 'local_size_bytes':
    ...
    def mark_func_method_deprecated(func):
        ...
        def new_func(*args, **kwargs):
            from warnings import warn
            warn("'%s' has been deprecated in version 2011.1. Please use the stateless launch interface instead." % func.__name__[1:], DeprecationWarning, stacklevel=2)
    ...
    if get_version() >= (4, ):
        ...
        for meth_name in ['set_block_shape', 'set_shared_size',
         ...
         'launch', 'launch_grid', 'launch_grid_async']:
            setattr(Function, meth_name, mark_func_method_deprecated(getattr(Function, '_' + meth_name)))
