...
# Embedded file name: build/bdist.linux-armv7l/egg/pycuda/sparse/packeted.py
...
class PacketedSpMV:
    ...
    def __init__(self, mat, is_symmetric, dtype):
        ...
        self.rows_per_packet = (devdata.shared_memory - 100) // (2 * self.dtype.itemsize)
        ...
        if not is_symmetric:
            ...
            adj_mat = csr_mat
        while True:
            cut_count, dof_to_packet_nr = part_graph(int(self.block_count), xadj=adj_mat.indptr, adjncy=adj_mat.indices)
            packet_nr_to_dofs = {}
            ...
            packet_nr_to_dofs = [ packet_nr_to_dofs.get(i) for i in range(len(packet_nr_to_dofs))
                                ]
            too_big = False
            ...
            if too_big:
                ...
                print('Metis produced a big block at block count %d--retrying with %d' % (
                 old_block_count, self.block_count))
                continue
            break

        assert len(packet_nr_to_dofs) == self.block_count
        new2old_fetch_indices, old2new_fetch_indices, packet_base_rows = self.find_simple_index_stuff(packet_nr_to_dofs)
        local_row_costs, remaining_coo = self.find_local_row_costs_and_remaining_coo(csr_mat, dof_to_packet_nr, old2new_fetch_indices)
        ...
        thread_assignments, thread_costs = self.find_thread_assignment(packet_nr_to_dofs, local_row_costs, thread_count)
        max_thread_costs = np.max(thread_costs)
        from .pkt_build import build_pkt_data_structure
        build_pkt_data_structure(self, packet_nr_to_dofs, max_thread_costs, old2new_fetch_indices, csr_mat, thread_count, thread_assignments, local_row_costs)
    ...
    def find_simple_index_stuff(self, packet_nr_to_dofs):
        ...
        for packet_nr, packet in enumerate(packet_nr_to_dofs):
            ...
            old2new_fetch_indices[pkt_indices] = np.arange(row_start, row_end, dtype=self.index_dtype)
            row_start += len(packet)

        packet_base_rows[self.block_count] = row_start
        return (
         new2old_fetch_indices, old2new_fetch_indices,
         packet_base_rows)

    def find_local_row_costs_and_remaining_coo(self, csr_mat, dof_to_packet_nr, old2new_fetch_indices):
        h, w = self.shape
        local_row_costs = [0] * h
        rem_coo_values = []
        rem_coo_i = []
    ...
    def find_thread_assignment(self, packet_nr_to_dofs, local_row_cost, thread_count):
        ...
        for packet_nr, packet_dofs in enumerate(packet_nr_to_dofs):
            row_costs_and_numbers = sorted([ (local_row_cost[i], i) for i in packet_dofs ], reverse=True)
        ...
        return (
         thread_assignments, thread_costs)

    def build_gpu_data_structure(self, packet_nr_to_dofs, max_thread_costs, old2new_fetch_indices, csr_mat, thread_count, thread_assignments, local_row_costs):
        from .pkt_build import build_pkt_structure
        build_pkt_structure(self, packet_nr_to_dofs, thread_assignments, thread_starts, thread_ends, index_array, data_array)

    @memoize_method
    def get_kernel(self):
        from pycuda.tools import dtype_to_ctype
        mod = SourceModule(PKT_KERNEL_TEMPLATE % {'value_type': dtype_to_ctype(self.dtype), 
           ...
           'rows_per_packet': self.rows_per_packet}, no_extern_c=True)
    ...
    def __call__(self, x, y=None):
        ...
        self.get_kernel().prepared_call((
         self.block_count, 1), (
         self.threads_per_packet, 1, 1), self.packet_base_rows.gpudata, self.thread_starts.gpudata, self.thread_ends.gpudata, self.index_array.gpudata, self.data_array.gpudata, x.gpudata, y.gpudata)
