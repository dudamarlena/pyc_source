...
# Embedded file name: build/bdist.linux-armv7l/egg/pycuda/tools.py
# Compiled at: 2015-06-16 13:16:13
__doc__ = 'Miscallenous helper functionality.'
...
import pycuda._driver as _drv, numpy as np
bitlog2 = _drv.bitlog2
DeviceMemoryPool = _drv.DeviceMemoryPool
PageLockedMemoryPool = _drv.PageLockedMemoryPool
from pycuda.compyte.dtypes import register_dtype, get_or_register_dtype, _fill_dtype_registry, dtype_to_ctype as base_dtype_to_ctype
...
class DebugMemoryPool(DeviceMemoryPool):
    ...
    def allocate(self, size):
        ...
        print('\n  Allocation of size %d occurring (mem: last_free:%d, free: %d, total:%d) (pool: held:%d, active:%d):\n      at: %s' % ((
         size, self.last_free) + cuda.mem_get_info() + (self.held_blocks, self.active_blocks,
         description)), file=self.logfile)
        hist_items = sorted(list(six.iteritems(histogram)))
        for (bsize, descr), count in hist_items:
            print('  %s (%d bytes): %dx' % (descr[len(cpfx):], bsize, count), file=self.logfile)
...
def make_default_context(ctx_maker=None):
    ...
    if ndevices == 0:
        raise RuntimeError('No CUDA enabled device found. Please check your installation.')
    import os
    ...
    if devn is not None:
        try:
            ...
            raise TypeError('CUDA device number (CUDA_DEVICE or ~/.cuda_device) must be an integer')
