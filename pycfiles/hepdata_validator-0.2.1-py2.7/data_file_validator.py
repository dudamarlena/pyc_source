...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hepdata_validator/data_file_validator.py
        ...
        try:
            if type in self.custom_data_schemas:
                ...
                if schema_file_path:
                    ...
                    _schema_file = os.path.join(self.base_path, 'schemas', self.schema_version, ('{0}_schema.json').format(type))
            ...
            raise UnsupportedDataSchemaException(message=("There is no schema defined for the '{0}' data type.").format(type))
    ...
    def validate(self, **kwargs):
        ...
        if data is None:
            try:
                ...
                self.add_validation_message(ValidationMessage(file=file_path, message='There was a problem parsing the file.\n' + e.__str__()))
                return False
...
def check_length_values(data):
    ...
    if len(set(indep_count + dep_count)) > 1:
        raise ValidationError("Inconsistent length of 'values' list: " + 'independent_variables%s, dependent_variables%s' % (str(indep_count), str(dep_count)), instance=data)