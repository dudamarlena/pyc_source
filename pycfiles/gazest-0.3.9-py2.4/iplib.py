...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gazest/lib/iplib.py
# Compiled at: 2007-10-17 15:49:54
"""
iplib module.
...
NM_BITS = 6
NM_WILDCARD = 7
NOTATION_MAP = {IP_DOT: ('dotted decimal', 'dotted', 'quad', 'dot', 'dotted quad'), IP_HEX: ('hexadecimal', 'hex'), IP_BIN: ('binary', 'bin'), IP_OCT: ('octal', 'oct'), IP_DEC: ('decimal', 'dec'), NM_BITS: ('bits', 'bit', 'cidr'), NM_WILDCARD: ('wildcard bits', 'wildcard'), IP_UNKNOWN: ('unknown', 'unk')}
...
def _check_nm(nm, notation):
    ...
    _NM_CHECK_FUNCT = {NM_DOT: _dot_to_dec, NM_HEX: _hex_to_dec, NM_BIN: _bin_to_dec, NM_OCT: _oct_to_dec, NM_DEC: _dec_to_dec_long}
    try:
...
def is_wildcard_nm(nm):
    ...
    try:
        dec = 4294967295 - _dot_to_dec(nm, check=True)
    except ValueError:
        return False
...
def _wildcard_to_dec(nm, check=False):
    ...
    return 4294967295 - _dot_to_dec(nm, check=False)


def _dec_to_wildcard(nm):
    """Decimal to wildcard bits conversion."""
    return _dec_to_dot(4294967295 - nm)


_CHECK_FUNCT = {IP_DOT: (is_dot, is_dot_nm), IP_HEX: (is_hex, is_hex_nm), IP_BIN: (is_bin, is_bin_nm), IP_OCT: (is_oct, is_oct_nm), IP_DEC: (is_dec, is_dec_nm), NM_BITS: (
           lambda : False, is_bits_nm), 
   NM_WILDCARD: (
               lambda : False, is_wildcard_nm)}
...
class _IPv4Base(object):
    ...
    def set(self, ip, notation=IP_UNKNOWN):
        ...
        self._ip_dec = long(_convert(ip, notation=IP_DEC, inotation=notation, _check=True, _isnm=self._isnm))
        self._ip = _convert(self._ip_dec, notation=IP_DOT, inotation=IP_DEC, _check=False, _isnm=self._isnm)
    ...
    def get_hex(self):
        ...
        return _convert(self._ip_dec, notation=IP_HEX, inotation=IP_DEC, _check=False, _isnm=self._isnm)

    def get_bin(self):
        """Return the binary notation of the address/netmask."""
        return _convert(self._ip_dec, notation=IP_BIN, inotation=IP_DEC, _check=False, _isnm=self._isnm)
    ...
    def get_oct(self):
        ...
        return _convert(self._ip_dec, notation=IP_OCT, inotation=IP_DEC, _check=False, _isnm=self._isnm)
...
class IPv4NetMask(_IPv4Base):
    ...
    def get_bits(self):
        ...
        return _convert(self._ip, notation=NM_BITS, inotation=IP_DOT, _check=False, _isnm=self._isnm)

    def get_wildcard(self):
        """Return the wildcard bits notation of the netmask."""
        return _convert(self._ip, notation=NM_WILDCARD, inotation=IP_DOT, _check=False, _isnm=self._isnm)
...
class CIDR(object):
    ...
    def is_valid_ip(self, ip):
        ...
        if isinstance(ip, IPv4Address):
            ...
            if ip._first_ip._ip_dec < compare_to_first or ip._first_ip._ip_dec > compare_to_last:
                return False
        return True
