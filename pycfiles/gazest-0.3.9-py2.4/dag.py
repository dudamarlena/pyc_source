...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gazest/lib/dag.py
# Compiled at: 2007-10-11 10:40:20
...
import gazest.lib.helpers as h, os
ex1 = {76: [74, 75], 75: [67], 74: [73], 73: [71, 72], 72: [59], 71: [68], 68: [67], 67: [61], 61: [59], 59: [58]}
ex1_h = 76
...
class FakeRev:
    ...
    def parents(self):
        return [ FakeRev(self.dag_h, id) for id in self.dag_h.get(self.id, []) ]
...
def layout(root_rev):
    ...
    revs = sorted(dag.nodes(), key=lambda id: nodes_h[id].creat_date, reverse=True)
    vizdag = VizDag()
    levels = []
    prev_level = []
    next_id = [
...
def node_img(top=1, bottom=1):
    url = h.url_for(controller='/dagimage', action='node', top=top, bottom=bottom)
    return '<img width="24" height="24" src="%s" />' % url


def arc_img(top, mid, bottom):
    url = h.url_for(controller='/dagimage', action='arc', top=top, mid=mid, bottom=bottom)
...
def html_layout(root_rev):
    ...
    def new_block():
        return [ [ 0 for i in range(nb_cols) ] for j in range(3) ]
    ...
    for (rev, nodes) in levels:
        ...
        for (i, node) in enumerate(nodes):
            ...
            if node.original_p:
                n_row.append(node_img(root_rev.id == rev.id and '0' or '1', dag.parents(rev.id) and '1' or '0'))
            else:
        ...
        a_row = [ arc_img(arcvals[0][i], arcvals[1][i], arcvals[2][i]) for i in range(nb_cols) ]
...
if __name__ == '__main__':
    ...
    def lca(a, b):
        print 'LCA of %s and %s is %s' % (a, b,
         get_lca(FakeRev(ex1, a), FakeRev(ex1, b)).id)
