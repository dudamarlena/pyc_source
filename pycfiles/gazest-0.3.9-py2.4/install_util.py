...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gazest/lib/install_util.py
# Compiled at: 2007-10-25 20:08:26
...
class MakoInstaller(Installer):
    ...
    def config_content(self, command, vars):
        ...
        return copydir.careful_sub(tmpl.render_unicode(**vars), vars, self.meta_name)
...
OPTIONS_COERCE = [
 (
  asbool, 'staging', 'use_google_search', 'site_readonly'), (int, 'nb_site_proxy'), (parse_timedelta, 'wiki_indexing_delay')]

class GazestInstaller(MakoInstaller):
    __module__ = __name__
    required_vals = [('site_name', 'Another Gazest Site'), 'domain_name',
     ('site_base',
      lambda v: 'http://gazest.%s' % v['domain_name']), ('port', 8082), ('site_uuid', getuuid, False), ('site_changes_feed_uuid', getuuid, False),
     ('webmaster_email',
      lambda v: 'webmaster@%s' % v['domain_name']),
     ('error_email_from',
      lambda v: 'paste@%s' % v['domain_name']),
     ('system_email_from',
      lambda v: 'no-reply@%s' % v['domain_name']), ('smtp_server', 'localhost'), ('copyright_years', date.today().year), 'copyright_owner',
     ('copyright_owner_email',
      lambda v: v['webmaster_email']), ('beaker_session_key', 'gazest'), ('beaker_session_secret', shahex, False), ('authkit_cookie_secret', shahex, False), ('sqlalchemy_url', 'sqlite:///%(here)s/data.db'), ('use_mod_proxy', True, 'site behind mod_proxy or mod_rewrite'),
     ('nb_site_proxy',
      lambda vars: vars['use_mod_proxy'] and 1 or 0, False), ('wiki_indexing_delay', '5d'), ('use_error_mails', False), ('staging', True), ('use_google_search', True)]
