...
# [GCC 8.4.0]
# Embedded file name: /Users/atulvarma/Documents/18f/django-uswds-forms/example/app/tests.py
...
class RenderSourceTests(TestCase):
    ...
    def test_clean_python_source_works(self):
        self.assertEqual(clean_python_source("'''boop\n'''\nhello()\nthere()\n"), 'hello()\nthere()')

    def test_clean_template_source_works(self):
        self.assertEqual(clean_template_source("{% include 'foo' %}" + '\nblah\n'), 'blah')
...
class AddLinksToDocsTests(TestCase):
    ...
    def test_it_hyperlinks_uswds_links(self):
        self.assertEqual(add_links_to_docs('foo uswds_forms.UswdsForm baz'), 'foo <a href="http://docs/reference.html#uswds_forms.UswdsForm"><code>UswdsForm</code></a> baz')

    def test_it_does_not_hyperlink_final_periods(self):
        self.assertEqual(add_links_to_docs('foo uswds_forms.UswdsForm. yup.'), 'foo <a href="http://docs/reference.html#uswds_forms.UswdsForm"><code>UswdsForm</code></a>. yup.')
...
class RadiosExampleTests(ExampleMixin, TestCase):
    url = '/example/radios'
    valid_post = {'president': 'washington'}
...
class EverythingExampleTests(ExampleMixin, TestCase):
    ...
    def test_non_field_errors_are_displayed(self):
        res = self.client.post(self.url, {'trigger_non_field_error': 'on'})
