...
class EverythingForm(uswds_forms.UswdsForm):
    ...
    park = forms.CharField(label='If you could choose the name of the next national park, what would it be?', help_text='Note that "Parky McParkface" is not a valid response.')
    states = uswds_forms.UswdsMultipleChoiceField(label='What states have you visited?', required=False, choices=(('OH', 'Ohio'),
                                                                                                                  ('IL', 'Illinois'),
                                                                                                                  ('CA', 'California')))
    date = uswds_forms.UswdsDateField(label='What is your favorite date?')
    trigger_non_field_error = forms.BooleanField(label='After submitting this form, trigger a non-field error.', required=False)
