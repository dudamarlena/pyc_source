...
class TestVFS:
    ...
    def test_ioplugins_pull_source_data_no_extension(self):
        ...
        self.vfs_plugin.pull_source_data(('vfs://tmp/_test/source/{}'.format(self.filename)), (self.destination_dir),
          'id_0',
        ...
        if not @py_assert6:
            @py_format8 = 'assert %(py7)s\n{%(py7)s = %(py4)s\n{%(py4)s = %(py2)s\n{%(py2)s = %(py0)s.path\n}.isfile\n}(%(py5)s)\n}' % {'py0':@pytest_ar._saferepr(os) if 'os' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(os) else 'os',  'py2':@pytest_ar._saferepr(@py_assert1),  'py4':@pytest_ar._saferepr(@py_assert3),  'py5':@pytest_ar._saferepr(output_file) if 'output_file' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(output_file) else 'output_file',  'py7':@pytest_ar._saferepr(@py_assert6)}
    ...
    def test_ioplugins_pull_source_data_mhd(self):
        ...
        self.vfs_plugin.pull_source_data((self.vfs_plugin.path_to_url(absfilename_mhd)), destination_path,
          'id_mhd_0',
        ...
        if not @py_assert6:
            @py_format8 = 'assert %(py7)s\n{%(py7)s = %(py4)s\n{%(py4)s = %(py2)s\n{%(py2)s = %(py0)s.path\n}.isfile\n}(%(py5)s)\n}' % {'py0':@pytest_ar._saferepr(os) if 'os' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(os) else 'os',  'py2':@pytest_ar._saferepr(@py_assert1),  'py4':@pytest_ar._saferepr(@py_assert3),  'py5':@pytest_ar._saferepr(destination_path_mhd) if 'destination_path_mhd' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(destination_path_mhd) else 'destination_path_mhd',  'py7':@pytest_ar._saferepr(@py_assert6)}
            raise AssertionError(@pytest_ar._format_explanation(@py_format8))
        @py_assert1 = @py_assert3 = @py_assert6 = None
        @py_assert1 = os.path
        @py_assert3 = @py_assert1.isfile
        ...
        if not @py_assert6:
            @py_format8 = 'assert %(py7)s\n{%(py7)s = %(py4)s\n{%(py4)s = %(py2)s\n{%(py2)s = %(py0)s.path\n}.isfile\n}(%(py5)s)\n}' % {'py0':@pytest_ar._saferepr(os) if 'os' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(os) else 'os',  'py2':@pytest_ar._saferepr(@py_assert1),  'py4':@pytest_ar._saferepr(@py_assert3),  'py5':@pytest_ar._saferepr(destination_path_raw) if 'destination_path_raw' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(destination_path_raw) else 'destination_path_raw',  'py7':@pytest_ar._saferepr(@py_assert6)}
            raise AssertionError(@pytest_ar._format_explanation(@py_format8))
        @py_assert1 = @py_assert3 = @py_assert6 = None
