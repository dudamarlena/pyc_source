...
# Size of source mod 2**32: 21140 bytes
...
class PimAPIv2(object):
    ...
    def job_update_loop(self):
        ...
        while self.running or len(self.submitted_job_updates) != 0 or len(self.queued_job_updates) != 0:
            ...
            with self.updates_lock:
                number_of_jobs = min(self.batch_size - len(self.submitted_job_updates), len(self.queued_job_updates))
                for _ in range(number_of_jobs):
                    self.submitted_job_updates.append(self.queued_job_updates.popleft())
    ...
    def pim_serialize_node(self, node, scope, links):
        ...
        node_data = {'name': node.id, 
         ...
         'children': [], 
         'customData': {}, 
         'inPorts': [{'name': output.id, 'title': output.id, 'customData': {'input_group': output.input_group, 'datatype': output.datatype.id, 'dimension_names': [x.name for x in output.dimensions]}} for output in node.inputs.values()], 
    ...
    def pim_register_run(self, network):
        ...
        pim_run_data = {'title': self.run_id, 
         ...
         'links': [], 
         'description': 'Run of {} started at {}'.format(network.id, network.timestamp), 
         
         'customData': {'workflow_engine': 'fastr', 
                        'tmpdir': network.tmpdir}, 
         
         'statusTypes': self.STATUS_TYPES}
        self.pim_serialize_network(network=network, scope='root', parent=pim_run_data['root'], links=pim_run_data['links'])
...
class PimReporter(ReportingPlugin):
    ...
    def configuration_fields(cls):
        return {'pim_host': (str, '', 'The PIM host to report to'), 
