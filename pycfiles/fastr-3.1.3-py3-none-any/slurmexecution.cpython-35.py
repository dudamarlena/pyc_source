...
# Size of source mod 2**32: 10959 bytes
import json, os, re, shellescape, subprocess, sys, threading, time, fastr
...
class SlurmExecution(ExecutionPlugin):
    ...
    def configuration_fields(cls):
        return {'slurm_job_check_interval': (int, 30, 'The interval in which the job checker will startto check for stale jobs'), 
    ...
    def _queue_job(self, job):
        command = [
         ...
         job.commandfile]
        command = ' '.join(shellescape.quote(item) for item in command)
        depends = ''
        if job.hold_jobs:
            ...
            if job.status == JobState.hold:
                ...
                hold = ''
            sbatch_script = SBATCH_SCRIPT_TEMPLATE.format(cores=job.required_cores or 1, memory=required_memory, time=required_time, partition=partition, stdout=job.stdoutfile, stderr=job.stderrfile, command=command, depends=depends, hold=hold)
            if fastr.config.debug:
                fastr.log.debug('USING SBATCH SCRIPT:\n{}'.format(sbatch_script))
            sbatch = subprocess.Popen([
             self.SBATCH], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
