...
import netrc, os, re, tempfile, urlparse
...
class XNATStorage(IOPlugin):
    ...
    def connect(self, server, path='', insecure=False, verify=True):
        if self.server != server:
            ...
            session = xnat.connect(urlparse.urlunparse([schema, server, path, '', '', '']), user=user, password=password, debug=fastr.config.debug, verify=verify)
            self._xnat = (
             server, session)
        return
    ...
    def fetch_url(self, inurl, outpath):
        ...
        insecure = urlparse.parse_qs(url.query).get('insecure', ['0'])[0] in ('true',
                                                                              '1')
        verify = urlparse.parse_qs(url.query).get('verify', ['1'])[0] in ('true', '1')
        use_regex = urlparse.parse_qs(url.query).get('regex', ['0'])[0] in ('true',
                                                                            '1')
        ...
        data_path = os.path.join(workdir, os.listdir(workdir)[0], 'scans', scan_dir, 'resources', resource.label.replace(' ', '_'), 'files', filepath)
    ...
    def put_url(self, inpath, outurl):
        ...
        insecure = urlparse.parse_qs(url.query).get('insecure', ['false'])[0] in ('true',
                                                                                  '1')
        verify = urlparse.parse_qs(url.query).get('verify', ['1'])[0] in ('true', '1')
        use_regex = urlparse.parse_qs(url.query).get('regex', ['0'])[0] in ('true',
                                                                            '1')
        overwrite = urlparse.parse_qs(url.query).get('overwrite', ['0'])[0] in ('true',
                                                                                '1')
        self.connect(url.netloc, path=path_prefix, insecure=insecure, verify=verify)
    ...
    def expand_url(self, url):
        ...
        if parsed_url.path == '/search':
            ...
            search_path = ('/data/archive/projects/{p}/subjects/{s}/experiments/{e}/scans/{sc}/resources/{r}').format(p=query.get('projects', default)[0], s=query.get('subjects', default)[0], e=query.get('experiments', default)[0], sc=query.get('scans', default)[0], r=query.get('resources', default)[0])
