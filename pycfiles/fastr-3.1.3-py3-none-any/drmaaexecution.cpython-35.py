...
class DRMAAExecution(ExecutionPlugin):
    ...
    def configuration_fields(cls):
        return {'drmaa_queue': (str, 'week', 'The default queue to use for jobs send to the scheduler'), 
         'drmaa_max_jobs': (int, 0, 'The maximum jobs that can be send to the scheduler at the same time (0 for no limit)'), 
         'drmaa_engine': (str, 'grid_engine', 'The engine to use (options: grid_engine, torque'), 
         'drmaa_job_check_interval': (int, 900, 'The interval in which the job checker will startto check for stale jobs')}
    ...
    def send_job(self, command, arguments, queue=None, resources=None, job_name=None, joinLogFiles=False, outputLog=None, errorLog=None, hold_job=None, hold=False):
        ...
        if resources.memory:
            ...
            memory = None
        native_spec = self.create_native_spec(queue=queue, walltime=walltime, memory=memory, ncores=resources.cores, outputLog=outputLog, errorLog=errorLog, hold_job=hold_job, hold=hold)
    ...
    def submit_jobs(self):
        while self.running:
            try:
                ...
                cl_job_id = self.send_job(command[0], command[1:], job_name='fastr_{}'.format(job.id), resources=job.resources, outputLog=job.stdoutfile, errorLog=job.stderrfile, hold_job=[self.job_lookup_table[x] for x in job.hold_jobs if x in self.job_lookup_table], hold=job.status == JobState.hold)
