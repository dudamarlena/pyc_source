...
from collections import namedtuple, OrderedDict, Mapping, Iterable
...
class CollectorPlugin(Plugin):
    ...
    def _regexp_path(self, path):
        ...
        if subpaths[0] != os.path.sep and not re.match('[a-zA-Z]:[/\\\\]', subpaths[0]):
...
class FastrInterface(Interface):
    ...
    def __getstate__(self):
        ...
        state = {'id': self.id, 
           ...
           'inputs': [ x.__getstate__() for x in self.input_map.values() ], 'outputs': [ x.__getstate__() for x in self.output_map.values() ]}
        return state
    ...
    def execute(self, target, payload):
        ...
        result = InterfaceResult(result_data={}, log_data=log_data, payload=payload, errors=None)
    ...
    def collect_errors(result):
        ...
        """
        errors = []
        match = re.search('^__FASTR_ERRORS__ = (?P<value>.*)$', result.log_data.get('stdout', ''), re.MULTILINE)
        ...
        match = re.search('^__FASTR_ERRORS__ = (?P<value>.*)$', result.log_data.get('stderr', ''), re.MULTILINE)
        if match is not None:
            value = match.group('value')
            try:
                value = json.loads(value)
    ...
    def get_specials(self, payload, output, cardinality_nr):
        ...
        if output is not None:
            ...
            if issubclass(datatype, URLType):
                if isinstance(datatype.extension, basestring):
                ...
                for member in datatype.members:
                    if isinstance(member.extension, basestring):
        ...
        if len(self.input_map) > 0:
            ...
            inputs_tuple_type = namedtuple('Inputs', input_data.keys())
            inputs = inputs_tuple_type(*input_data.values())
        else:
            inputs = ()
        ...
        if len(output_arguments) > 0:
            ...
            outputs = outputs_tuple_type(*output_data.values())
        else:
            outputs = ()
    ...
    def get_commandline_argument(self, values):
        ...
        if self.action is not None:
            if self.action in self.ACTIONS:
                ...
                fastr.log.warning(('Encountered unknown action {}, only know: {}').format(self.action, self.ACTIONS.keys()))
