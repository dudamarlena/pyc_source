...
if IMPORT_SUCCESS:
    DATATYPE_MAP = {traits.trait_types.Int: 'Int', traits.trait_types.Long: 'Int', 
...
class NipypeInterface(Interface):
    ...
    def __init__(self, id_, nipype_cls=None, document=None):
        ...
        if isinstance(nipype_cls, (str, unicode)):
            if '.' in nipype_cls:
    ...
    def _create_inputs_outputs(self):
        ...
        for id_, input_ in nipype_object.input_spec().items():
            ...
            if default == '':
                default = None
            inputs.append(InputSpec(id_=id_, cardinality=1 if not input_.is_trait_type(traits.trait_types.List) else '1-*', datatype=self.get_type(input_), required=input_.mandatory and default is None, description=input_.desc, default=default, hidden=id_ in ignore_inputs))

        outputs = []
        for id_, output in nipype_object.output_spec().items():
            outputs.append(OutputSpec(id_=id_, cardinality=1 if not output.is_trait_type(traits.trait_types.List) else 'unknown', datatype=self.get_type(output), automatic=True, required=False, description=output.desc, hidden=False))
