...
# Compiled at: 2019-06-26 05:19:41
...
class HiddenFieldMap(Mapping):
    ...
    def __iter__(self):
        for key, value in self._data.items():
            if hasattr(value, 'hidden'):
                value.hidden or (yield key)
...
class FastrInterface(Interface):
    ...
    def get_arguments(self, values):
        ...
        arguments = sorted(arguments, key=(lambda x:         if x.order >= 0:
x.order # Avoid dead code: sys.maxsize - x.order))
        argument_list = []
        for argument in arguments:
            ...
            try:
                ...
                if argument.default is not None:
                    ...
                    raise isinstance(argument, OutputParameterDescription) and argument.automatic or exceptions.FastrValueError('Required argument "{}" has no value!'.format(argument.id))
                else:
                    value = None
...
class ParameterDescription(Serializable):
    ...
    def format_argument_value(self, value, datatype=None):
        if datatype is None:
            ...
            if self._IS_INPUT:
                value.valid or fastr.log.debug('SELF TYPE: {}, ID {}'.format(type(self).__name__, self.id))
    ...
    def __setstate__(self, state):
        if 'location' not in state:
            ...
            if 'method' not in state:
                if not 'automatic' not in state:
                    state['method'] = state['automatic'] or None
                else:
                    state['method'] = 'path'
