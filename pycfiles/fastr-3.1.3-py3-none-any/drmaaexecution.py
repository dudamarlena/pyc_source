...
class DRMAAExecution(ExecutionPlugin):
    ...
    def configuration_fields(cls):
        return {'drmaa_queue': (
           ...
           'drmaa_default_walltime': (
                                    str, '24:00:00', "Walltime used for jobs that don't specify walltime HH:MM:SS, defaults to: 24:00:00 (24 hours) if not set"), 
           'drmaa_default_memory': (
                                  str, '1G', "Memory used for jobs that don't specify memory, defaults to: 1G (1 Gibibyte) if not set"), 
           'drmaa_default_cores': (
                                 int, 1, "Cores used for jobs that don't specify cores, defaults to: 1 (1 core) if not set")}
    ...
    def send_job(self, command, arguments, queue=None, walltime=None, job_name=None, memory=None, ncores=None, joinLogFiles=False, outputLog=None, errorLog=None, hold_job=None, hold=False):
        ...
        native_spec = self.create_native_spec(queue=queue, walltime=walltime, memory=memory, ncores=ncores, outputLog=outputLog, errorLog=errorLog, hold_job=hold_job, hold=hold)
    ...
    def submit_jobs(self):
        while self.running:
            try:
                ...
                if required_memory is None or len(required_memory) == 0:
                    required_memory = fastr.config.drmaa_default_memory
                required_cores = job.required_cores
                if required_cores is None or required_cores <= 0:
                    required_cores = fastr.config.drmaa_default_cores
                required_time = job.required_time
                if required_time is None or len(required_time) == 0:
                    required_time = fastr.config.drmaa_default_walltime
                cl_job_id = self.send_job(command[0], command[1:], job_name=('fastr_{}').format(job.id), memory=required_memory, ncores=required_cores, walltime=required_time, outputLog=job.stdoutfile, errorLog=job.stderrfile, hold_job=[ self.job_lookup_table[x] for x in job.hold_jobs if x in self.job_lookup_table ], hold=job.status == JobState.hold)
