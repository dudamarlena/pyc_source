...
class XNATStorage(IOPlugin):
    ...
    def connect(self, server, path='', insecure=False, verify=True):
        if self.server != server:
            ...
            session = xnat.connect(urllib.parse.urlunparse([schema, server, path, '', '', '']), user=user, password=password, debug=fastr.config.debug, verify=verify)
    ...
    def fetch_url(self, inurl, outpath):
        ...
        for root, dirs, files in os.walk(workdir):
            if root.endswith(target) and file_name in files:
                data_path = os.path.join(root, filepath)
    ...
    def upload(resource, in_path, location, retries=3, overwrite=False):
        ...
        with open(in_path, 'rb') as (in_file_handle):
            while not success and tries > 0:
                ...
                try:
                    ...
                    success = True
                except xnat.exceptions.XNATError as exception:
        ...
        if xnat_size != file_size:
            raise xnat.exceptions.XNATUploadError('Problem with uploading to XNAT (file size differs {} vs {})'.format(xnat_size, file_size))
        else:
