...
class DockerTarget(Target):
    ...
    def run_command(self, command: List) -> TargetResult:
        ...
        container = self._docker_client.containers.run(image=self._docker_image, entrypoint=command[0], command=command[1:], detach=True, volumes=binds, network_disabled=False, user='{}:{}'.format(os.getuid(), os.getgid()), working_dir=os.path.abspath(os.curdir), environment=dict(os.environ))
        self._running_container = True
        sysuse = ProcessUsageCollection()
        monitor_thread = threading.Thread(target=self.monitor_docker, name='DockerMonitor', args=(
         container, sysuse))
        ...
        container.remove()
        return TargetResult(return_code=return_code, stdout=stdout, stderr=stderr, command=command, resource_usage=list(sysuse), time_elapsed=end_time - start_time)
    ...
    def monitor_docker(self, container, resources):
        ...
        try:
            while self._running_container:
                ...
                usage = resources.usage_type(timestamp=timestamp.isoformat(), cpu_percent=stat['cpu_stats']['cpu_usage']['total_usage'], vmem=-1.0, rmem=stat['memory_stats'].get('rss'), read_bytes=0.0, write_bytes=0.0)
