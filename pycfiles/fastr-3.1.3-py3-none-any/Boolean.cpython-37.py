...
class Boolean(ValueType):
    ...
    def __str__(self):
        if self._value is None:
            ...
            if self.format == 'flag' or self.format is None:
                return {True:'__FASTR_FLAG__TRUE___', 
                 False:'__FASTR_FLAG__FALSE__'}[self._value]
            if self.format == 'yn':
                return {True:'y', 
                 False:'n'}[self._value]
            if self.format == 'yes':
                return {True:'yes', 
                 False:'no'}[self._value]
            if self.format == 'Yes':
                return {True:'Yes', 
                 False:'No'}[self._value]
            if self.format == 'YES':
    ...
    def value(self, value):
        ...
        """
        translation_table = {0:False, 
         '0':False,  'false':False,  '__fastr_flag__false__':False,  1:True, 
         '1':True,  'true':True,  '__fastr_flag__true___':True}
