...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/ORCSchlange/__init__.py
# Compiled at: 2017-07-14 03:23:37
...
def main():
    ...
    fetch = subparsers.add_parser('fetch', help='Fetch the information from the ORICID-Public-API.\n                                  Call "fetch -h" for more details.')
    add_fetch(fetch)
    db = subparsers.add_parser('db', help='Manage the SQLite DB that contains the orcids. Call "db -h" for more details.',
      add_help=False)
    add_db(db)
...
def add_fetch(fetch):
    ...
    fetch.add_argument('--dbfile', action='store', dest='dbfile', help='The SQLite DB file that is used.', default='people.db')
    ...
    fetch.add_argument('--path', action='store', dest='path', help='The path where the output is created. (default: %(default)s)',
      default='output/')
    fetch.add_argument('--name', action='store', dest='name', help='The name of the output. (default: %(default)s)', default='index')
    ...
    api = fetch.add_argument_group(title='API-Configuration', description='To interact with the ORCID-API the client-id and client-secret \n                                   need to set or loaded. The default is the sandbox')
    ...
    api.add_argument('--file', action='store', dest='config', help='Load the options out of the file that is given. These need that the file is in a json \n                     format that have a field "client_id" and "client_secret".')
...
def add_db(db):
    ...
    db.add_argument('--dbfile', action='store', dest='dbfile', help='The SQLite DB file that is used.', default='people.db')
    ...
    add_dbs.add_argument('--dbfile', action='store', dest='dbfile', help='The SQLite DB file that is used.', default='people.db')
    add_adddb(add_dbs)
    conf_db = dbsubs.add_parser('addConf', help='Add an new Config to the DB')
    conf_db.add_argument('--dbfile', action='store', dest='dbfile', help='The SQLite DB file that is used.', default='people.db')
    add_conf(conf_db)
    print_db = dbsubs.add_parser('print', help='Print the content of the databank')
    print_db.add_argument('--dbfile', action='store', dest='dbfile', help='The SQLite DB file that is used.', default='people.db')
    print_db.set_defaults(func=(lambda args: DbCommand(args).prints()))
    clean_db = dbsubs.add_parser('clean', help='Reset the databank')
    clean_db.add_argument('--dbfile', action='store', dest='dbfile', help='The SQLite DB file that is used.', default='people.db')
    clean_db.set_defaults(func=(lambda args: DbCommand(args).clean()))
    create_db = dbsubs.add_parser('create', help='Create a new databank')
    create_db.add_argument('--dbfile', action='store', dest='dbfile', help='The SQLite DB file that is used.', default='people.db')
...
def add_conf(conf_db):
    ...
    conf_db.add_argument('auth', action='store', help='The url to authenticate.', nargs='?', default='https://orcid.org/oauth/token')
    conf_db.add_argument('api', action='store', help='The url of the api.', nargs='?', default='https://pub.orcid.org/v2.0/')
