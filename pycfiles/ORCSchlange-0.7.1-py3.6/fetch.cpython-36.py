...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/ORCSchlange/command/fetch.py
# Compiled at: 2017-07-12 09:44:38
# Size of source mod 2**32: 2618 bytes
"""The fetch commands."""
from ORCSchlange.command import BaseCommand
from ORCSchlange.orcid import API
import itertools, pybtex.database
...
class FetchReporeter(BaseCommand):
    __doc__ = 'The class that contains all fetch commands.'
    ...
    def fetch(self):
        ...
        if self.args.jquery:
            ...
            shutil.copyfile(os.path.dirname(__file__) + '/{jname}'.format(jname=jname), '{path}{jname}'.format(path=(self.args.path), jname=jname))