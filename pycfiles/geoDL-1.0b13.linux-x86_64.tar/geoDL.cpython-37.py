...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.7/site-packages/geoDL/geoDL.py
# Compiled at: 2019-05-15 04:28:26
# Size of source mod 2**32: 17002 bytes
"""
...
"""
import sys, os, re, argparse, csv, requests, bs4
...
class SmartFormatter(argparse.HelpFormatter):
    __doc__ = 'Quick hack for formatting helper of argparse with new lines'
...
def get_args():
    ...
    parser.add_argument('mode',
      choices=['geo', 'meta', 'ena', 'prefetch'], help='Which mode the program runs.')
...
def get_metadata(args):
    ...
    if args.mode == 'geo':
        ...
        geo_url = 'http://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc={}'.format(args.inputvalue)
        try:
        ...
        search_url = 'http://www.ebi.ac.uk/ena/data/warehouse/search?query=%22geo_accession=%22{geo}%22%22&result=study&display=xml'.format(geo=(args.inputvalue))
        print(search_url)
        try:
        ...
        urlretrieve('http://www.ebi.ac.uk/ena/data/warehouse/filereport?accession={}&result=read_run&fields=study_accession,secondary_study_accession,sample_accession,secondary_sample_accession,experiment_accession,run_accession,sample_alias,scientific_name,instrument_model,library_layout,read_count,experiment_alias,run_alias,fastq_ftp&download=txt'.format(ena_access), metafile)
        print(Fore.GREEN + ' > Metafile retrieved {}!'.format(metafile) + Fore.RESET)
    else:
        if args.mode == 'ena':
            metafile = 'metadata_{}.xls'.format(args.inputvalue)
            urlretrieve('http://www.ebi.ac.uk/ena/data/warehouse/filereport?accession={}&result=read_run&fields=study_accession,secondary_study_accession,sample_accession,secondary_sample_accession,experiment_accession,run_accession,sample_alias,scientific_name,instrument_model,library_layout,read_count,experiment_alias,run_alias,fastq_ftp&download=txt'.format(args.inputvalue), metafile)
            ...
            if args.mode == 'meta':
                ...
                if args.mode == 'prefetch':
                    ...
                    metafile = 'metadata_{}.xls'.format(args.inputvalue)
                    r = requests.get('https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc={gse}'.format(gse=(args.inputvalue)))
                    geosoup = BeautifulSoup(r.text, 'html.parser')
                    projurl = [url for url in geosoup.find_all('a') if 'PRJ' in url.get_text()][0]
                    payload = {'db':'SRA',  'term':'{}'.format(projurl.get_text()), 
                     'retmax':10000}
                    ...
                    with open(metafile, 'w') as (meta):
                        for i, run in enumerate(runs):
                            ...
                            if n_rep > 1:
                                suffix = run['alias'].split('_')[1]
                                rename = '_'.join([re.sub('\\s+', '_', name), suffix])
                            else:
                                rename = '_'.join(re.sub('\\s+', '_', name))
                            map_dict[run['accession']] = rename
                            if i == 0:
                                header = '\t'.join(list(run.attrs.keys()) + list(run.pool.member.attrs.keys()) + [
                                 'paired', 'rename']) + '\n'
                                meta.write(header)
                            paired = 'SE' if len(run.statistics.find_all('read')) == 1 else 'PE'
                            data = '\t'.join(list(run.attrs.values()) + list(run.pool.member.attrs.values()) + [
                             paired, rename]) + '\n'
                            meta.write(data)
        ...
        for i, row in enumerate(reader):
            if i == 0:
                if args.colname not in row:
                    raiseError('  > ERROR: Column {col} not in the metadata file {meta}'.format(col=(args.colname),
                      meta=metafile))
                ...
                if row[idx] in samplenames:
                    raiseError('  > ERROR: Non uniq sample names in the column {col} of the meta file {meta}\n'.format(col=(args.colname),
                      meta=metafile))
            ...
            for i, line in enumerate(f):
                if i == 0:
                    ...
                    try:
                        ...
                        raiseError('  > ERROR: Fastq urls (ftp) is not in the metadata,                            make sure to add the column and try again.')
                ...
                for r, url in enumerate(data_urls):
                    if pair:
                        print(Fore.GREEN + '\n > Getting {}{}...\n'.format(outname, suffix[r]) + '================================================================================' + Fore.RESET)
                    else:
                        print(Fore.GREEN + '\n > Getting {}...\n'.format(outname, suffix[r]) + '================================================================================' + Fore.RESET)
                    wgetcmd = [
                     'wget',
