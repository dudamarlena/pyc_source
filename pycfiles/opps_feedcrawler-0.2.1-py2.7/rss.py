...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-intel/egg/opps/feedcrawler/processors/rss.py
# Compiled at: 2014-09-18 10:22:37
import feedparser, logging, pytz, json
...
class RSSProcessor(BaseProcessor):
    ...
    def create_entries(self):
        ...
        for i, entry in enumerate(self.parsed.entries):
            ...
            db_entry, created = self.entry_model.objects.get_or_create(site=self.feed.site, slug=slugify(self.feed.slug + '-' + entry_title[:150]), defaults=dict(entry_feed=self.feed, entry_link=entry.link, channel=self.feed.channel, title=entry_title[:140], entry_title=entry_title, user=self.feed.user, published=True, show_on_root_channel=True))
            if created:
                if hasattr(entry, 'links'):
                    links = [ item for item in entry.links if item.get('rel') == 'enclosure'
                            ]
                    for link in links:
