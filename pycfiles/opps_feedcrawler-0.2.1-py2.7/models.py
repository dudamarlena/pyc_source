...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-intel/egg/opps/feedcrawler/models.py
# Compiled at: 2014-09-18 10:20:13
import json
...
class Feed(Publishable, Slugged):
    ...
    group = models.ForeignKey(Group, blank=True, null=True, verbose_name=_('Group or Source'))
    ...
    channel = models.ForeignKey('channels.Channel', null=True, blank=True, on_delete=models.SET_NULL)
    main_image = models.ForeignKey('images.Image', verbose_name=_('Feed Image'), blank=True, null=True, on_delete=models.SET_NULL, related_name='feed_image')
    interval = models.PositiveIntegerField(blank=True, null=True, default=20, help_text=_('interval in minutes (celery only)'))
    ...
    def get_http_absolute_url(self):
        protocol, path = ('http://{0}/{1}').format(self.channel, self.slug).split(self.site.domain)
    ...
    def create_channel(self):
        try:
            ...
            channel = Channel.objects.create(name=self.title, slug=self.slug, published=True, site=self.site, user=self.user)
    ...
    def get_channel(self):
        return self.channel or Channel.objects.get_homepage(site=self.site) or self.create_channel()
    ...
    def save(self, *args, **kwargs):
        ...
        if Feed.objects.filter(**filters).exclude(**exclude).exists():
            self.slug = ('{random}-{o.slug}').format(o=self, random=getrandbits(16))
...
class Entry(Container):
    ...
    entry_category_code = models.CharField(max_length=255, blank=True, null=True)
    entry_original_id = models.PositiveIntegerField(blank=True, null=True)
    post_created = models.BooleanField(_('Post created'), default=False)
    ...
    def define_main_image(self, archive_link, save=False, *args, **kwargs):
        image = Image(title=('{0}-{1}').format(self.title, self.id), slug=('{0}-{1}').format(self.slug, self.id), user=self.user, site=self.site, archive_link=archive_link, **kwargs)
        image.save()
...
class ProcessLog(models.Model):
    ...
    class Meta:
        verbose_name = _('Process Log')
        verbose_name_plural = _('Process Logs')