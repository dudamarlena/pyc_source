...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-intel/egg/opps/feedcrawler/processors/rss_news.py
# Compiled at: 2014-09-18 10:28:34
...
class RSSProcessor(BaseProcessor):
    ...
    def update_feed(self):
        ...
        if hasattr(self.parsed, 'published_parsed'):
            published_time = datetime.fromtimestamp(mktime(self.parsed.feed.published_parsed))
            published_time = published_time - TZ_DELTA
            if self.feed.published_time and self.feed.published_time >= published_time:
                return
        ...
        for i, entry in enumerate(self.parsed.entries):
            ...
            if hasattr(entry, 'published_parsed'):
                ...
                if published_time.date() > now.date():
                    ...
                    self.verbose_print('Entry time is in the past, skipping: %s - %s' % (
            ...
            db_entry, created = self.entry_model.objects.get_or_create(entry_feed=self.feed, entry_link=entry.link, channel=self.feed.get_channel(), title=entry_title[:150].replace('&quot;', '"'), slug=slug[:150], entry_title=entry_title[:150], site=self.feed.site, user=self.feed.user, published=self.feed.publish_entries, show_on_root_channel=False, tags=tags)
            ...
            if created:
                ...
                try:
                    allowed = (
                     str, unicode, dict, list, int, float, long)
                    entry_source = json.dumps({k:v for k, v in entry.iteritems() if isinstance(v, allowed)})
    ...
    def create_post(self, entry):
        ...
        if Post.objects.filter(channel=channel, slug=slug, site=entry.site).exists():
            ...
            return
        post = Post(title=entry.entry_title[:150], slug=slug, content=entry.entry_content or entry.entry_description, channel=channel, site=entry.site, user=entry.user, show_on_root_channel=True, published=True, tags=entry.tags, date_insert=entry.entry_published_time, date_available=entry.entry_published_time)
        if self.feed.group:
