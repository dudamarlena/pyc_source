...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-intel/egg/opps/feedcrawler/processors/ftp.py
# Compiled at: 2014-09-18 10:22:22
import urllib, json, xml.etree.ElementTree as ET
...
class EFEXMLProcessor(BaseProcessor):
    ...
    def process_file(self, s):
        ...
        if self.log_created(s):
            ...
            if self.feed.source_username:
                ...
                url = ('ftp://{0}{1}{2}').format(self.feed.source_url, source_root_folder, s)
    ...
    def parse_xml(self, filename):
        ...
        for item in root.findall('./NewsItem'):
            data = {}
            try:
                data['headline'] = item.find('./NewsComponent/NewsLines/HeadLine').text
                data['subheadline'] = item.find('./NewsComponent/NewsLines/SubHeadLine').text
            except:
                pass

            try:
                tobject_attrib = item.find('./NewsComponent/ContentItem/DataContent/nitf/head/tobject/tobject.subject')
            ...
            try:
                tags_attr = item.find('./NewsComponent/ContentItem/DataContent/nitf/head/docdata/key-list/keyword')
            ...
            try:
                pub_data_attrib = item.find('./NewsComponent/ContentItem/DataContent/nitf/head/pubdata')
            ...
            try:
                data['abstract'] = item.find('./NewsComponent/ContentItem/DataContent/nitf/body/body.head/abstract/').text
            except:
                pass

            try:
                data['owner'] = item.find('./NewsComponent/ContentItem/DataContent/nitf/body/body.head/rights/').text
            except:
                pass

            try:
                data['story_data'] = item.find('./NewsComponent/ContentItem/DataContent/nitf/body/body.head/dateline/story.date').get('norm')
            except:
                pass

            try:
                body = item.find('./NewsComponent/ContentItem/DataContent/nitf/body/body.content')
                data['body'] = ('\n').join(('<p>{0}</p>').format(p.text) for p in body)
            except:
                pass
    ...
    def create_entry(self, data):
        if not data:
            ...
            pub_time = self.parse_dt(data.get('pub_date', data.get('story_data', None)))
            if pub_time:
            ...
            try:
                db_entry, created = self.entry_model.objects.get_or_create(entry_feed=self.feed, channel=self.feed.get_channel(), title=entry_title[:150], slug=slug[:150], entry_title=entry_title[:150], site=self.feed.site, user=self.feed.user, published=self.feed.publish_entries, show_on_root_channel=False, tags=unicode(tags))
...
class EFEXMLProcessorAuto(EFEXMLProcessor):
    ...
    def create_post(self, entry):
        channel_slug = CATEGORY_EFE.get(str(entry.entry_category_code).strip().zfill(8))
        if not channel_slug:
            msg = '{e.id} - {e.entry_category_code} not match category_efe \n'
            open('/tmp/debug_feeds.log', 'a').write(msg.format(e=entry))
        ...
        if Post.objects.filter(channel=channel, slug=slug, site=entry.site).exists():
            self.verbose_print('Post slug exists')
            return
        post = Post(title=entry.entry_title[:150], slug=slug, content=entry.entry_content, channel=channel, site=entry.site, user=entry.user, show_on_root_channel=True, published=True, hat=entry.hat, tags=entry.tags, date_insert=entry.entry_published_time, date_available=entry.entry_published_time)
        if self.feed.group:
        ...
        entry.save()
        self.verbose_print(('Post {p.id}- {p.title} - {p.slug} created').format(p=post))
        return post