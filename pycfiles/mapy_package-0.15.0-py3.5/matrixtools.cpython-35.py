...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\mapy\sympytools\matrixtools.py
...
def dbl_mintegrate(m, a, a1, a2, b, b1, b2, mname, sufix, norm1=False, norm2=False, do_simplify=False, conds='none'):
    m = mintegrate(m, a, a1, a2, mname, sufix, norm=norm1, do_simplify=do_simplify, conds=conds)
    m = mintegrate(m, b, b1, b2, mname, sufix, norm=norm2, do_simplify=do_simplify, conds=conds)
    return m
...
def mprint_as_sparse(m, mname, sufix, numeric=False, use_cse=False, header=None, print_file=True, collect_for=None):
    if use_cse:
    ...
    if use_cse:
        ...
        for i, sub in enumerate(subs[::num]):
            ls.append('cdef double ' + ', '.join(map(str, [j[0] for j in subs[num * i:num * (i + 1)]])))
    ...
    if not numeric:
        ...
        ls.append('# {mname}_{sufix}_num={num}'.format(mname=mname, sufix=sufix, num=num))
        for (i, j), v in np.ndenumerate(m):
            if v:
                ls.append('c += 1')
