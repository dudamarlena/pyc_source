...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\mapy\model\coords.py
# Compiled at: 2017-04-20 23:04:47
...
class Coord(object):
    ...
    def add2model(self, model):
        ...
        if self.idb is not None and self.g1b is not None and self.g2b is not None and self.g3b is not None:
    ...
    def check_to_rebuild(self):
        if self.a1 is not None and self.a2 is not None and self.a3 is not None and self.b1 is not None and self.b2 is not None and self.b3 is not None and self.c1 is not None and self.c2 is not None and self.c3 is not None:
            ...
            if self.ida is not None and self.g1a is not None and self.g2a is not None and self.g3a is not None:
                g1a = self.model.griddict[int(self.g1a)]
                g2a = self.model.griddict[int(self.g2a)]
                g3a = self.model.griddict[int(self.g3a)]
                if not g1a.rebuilt or not g2a.rebuilt or not g3a.rebuilt:
                    return False
                else:
                    return True
            else:
    ...
    def Rmatrix(self, newcr):
        ...
        Rself = np.array([
         ...
         [
          sina * sing + cosa * cosg * sinb, -cosg * sina + cosa * sinb * sing, cosa * cosb]], dtype=FLOAT)
