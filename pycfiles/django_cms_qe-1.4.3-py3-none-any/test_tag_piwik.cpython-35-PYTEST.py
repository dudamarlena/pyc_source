...
# [GCC 8.4.0]
# Embedded file name: /home/tomas/git/euroix/django-cms-qe/cms_qe_analytical/tests/test_tag_piwik.py
...
class PiwikTagTestCase(TagTestCase):
    ...
    def test_tag(self):
        ...
        self.assertTrue('img src="http://example.com/piwik.php?idsite=345"' in r, r)
    ...
    @override_config(PIWIK_DOMAIN_PATH='example.com/piwik', PIWIK_SITE_ID='345')
    def test_domain_path_valid(self):
        r = self.render_tag('piwik', 'piwik')
        self.assertTrue(' ? "https" : "http") + "://example.com/piwik/";' in r, r)

    @override_config(PIWIK_DOMAIN_PATH='example.com:1234', PIWIK_SITE_ID='345')
    def test_domain_port_valid(self):
        r = self.render_tag('piwik', 'piwik')
        self.assertTrue(' ? "https" : "http") + "://example.com:1234/";' in r, r)

    @override_config(PIWIK_DOMAIN_PATH='example.com:1234/piwik', PIWIK_SITE_ID='345')
    def test_domain_port_path_valid(self):
        r = self.render_tag('piwik', 'piwik')
        self.assertTrue(' ? "https" : "http") + "://example.com:1234/piwik/";' in r, r)
    ...
    def test_default_usertrack(self):
        User = get_user_model()
        context = Context({'user': User(username='BDFL', first_name='Guido', last_name='van Rossum')})
        r = PiwikNode().render(context)
