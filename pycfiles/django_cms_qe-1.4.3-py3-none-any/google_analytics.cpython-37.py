...
# [GCC 8.4.0]
# Embedded file name: /home/tomas/git/euroix/django-cms-qe/cms_qe_analytical/templatetags/google_analytics.py
...
import decimal, logging, re
from constance import config as constance_settings
from django.conf import settings
from django.template import Library, Node, TemplateSyntaxError
from cms_qe_analytical.utils import AnalyticalNotSet
from ..utils import AnalyticalException, disable_html, get_domain, get_required_setting, is_internal_ip
...
SCOPE_SESSION = 2
SCOPE_PAGE = 3
PROPERTY_ID_RE = re.compile('^UA-\\d+-\\d+$')
...
CUSTOM_VAR_CODE = "_gaq.push(['_setCustomVar', %(index)s, '%(name)s', '%(value)s', %(scope)s]);"
...
class GoogleAnalyticsNode(Node):
    ...
    def __init__(self):
        self.id_is_set = True
        try:
            self.property_id = get_required_setting('GOOGLE_ANALYTICS_PROPERTY_ID', PROPERTY_ID_RE, "must be a string looking like 'UA-XXXXXX-Y'")
        except AnalyticalNotSet:
            self.id_is_set = False
        except Exception:
            self.id_is_set = False
            logger.warning('Google Analytics has wrong property ID', exc_info=True)

    def render(self, context):
        if not self.id_is_set:
            return ''
        else:
            ...
            if getattr(constance_settings, 'GOOGLE_ANALYTICS_DISPLAY_ADVERTISING', False):
                source = DISPLAY_ADVERTISING_SOURCE
            else:
                source = DEFAULT_SOURCE
        html = SETUP_CODE % {'property_id':self.property_id,  'commands':' '.join(commands), 
    ...
    def _get_domain_commands(self, context):
        ...
        if tracking_type == TRACK_SINGLE_DOMAIN:
            ...
            if domain is None:
                raise AnalyticalException('tracking multiple domains with Google Analytics requires a domain name')
    ...
    def _get_custom_var_commands(self, context):
        ...
        for index, var in params:
            ...
            commands.append(CUSTOM_VAR_CODE % {'index':index, 
             'name':name, 
             'value':value, 
             'scope':scope})
    ...
    def _get_other_commands(self, context):
        commands = []
        if getattr(constance_settings, 'GOOGLE_ANALYTICS_SITE_SPEED', False):
            commands.append(SITE_SPEED_CODE)
        if getattr(constance_settings, 'GOOGLE_ANALYTICS_ANONYMIZE_IP', False):
            commands.append(ANONYMIZE_IP_CODE)
        sampleRate = getattr(constance_settings, 'GOOGLE_ANALYTICS_SAMPLE_RATE', False)
        if sampleRate != '':
        ...
        siteSpeedSampleRate = getattr(constance_settings, 'GOOGLE_ANALYTICS_SITE_SPEED_SAMPLE_RATE', False)
        ...
        sessionCookieTimeout = getattr(constance_settings, 'GOOGLE_ANALYTICS_SESSION_COOKIE_TIMEOUT', False)
        if sessionCookieTimeout != '':
        ...
        visitorCookieTimeout = getattr(constance_settings, 'GOOGLE_ANALYTICS_VISITOR_COOKIE_TIMEOUT', False)
