...
# [GCC 8.4.0]
# Embedded file name: /home/tomas/git/euroix/django-cms-qe/cms_qe_analytical/templatetags/piwik.py
...
from ..utils import is_internal_ip, disable_html, get_required_setting, get_identity, AnalyticalNotSet
logger = logging.getLogger(__file__)
DOMAINPATH_RE = re.compile('^(([^./?#@:]+\\.)*[^./?#@:]+)+(:[0-9]+)?(/[^/?#@:]+)*$')
SITEID_RE = re.compile('^\\d+$')
...
class PiwikNode(Node):
    ...
    def __init__(self):
        try:
            self.domain_path = get_required_setting('PIWIK_DOMAIN_PATH', DOMAINPATH_RE, 'must be a domain name, optionally followed by an URI path, no trailing slash (e.g. piwik.example.com or my.piwik.server/path)')
    ...
    def render(self, context):
        if not self.id_and_domain_is_set:
            ...
            html = TRACKING_CODE % {'url':self.domain_path, 
             'siteid':self.site_id, 
             'variables':'\n  '.join(variables_code)}
