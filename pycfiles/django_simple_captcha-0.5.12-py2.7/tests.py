...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/captcha/tests/tests.py
# Compiled at: 2018-05-18 03:04:51
import unittest
...
import datetime, json, re, six, warnings
...
class CaptchaCase(TestCase):
    ...
    def setUp(self):
        ...
        tested_helpers = [
         'captcha.helpers.math_challenge', 'captcha.helpers.random_char_challenge', 'captcha.helpers.unicode_challenge']
    ...
    def __extract_hash_and_response(self, r):
        hash_ = re.findall('value="([0-9a-f]+)"', str(r.content))[0]
    ...
    def test_image(self):
        for key in [ store.hashkey for store in six.itervalues(self.stores) ]:
            ...
            self.assertEqual(response._headers.get('content-type'), ('Content-Type',
                                                                     'image/png'))
    ...
    def test_repeated_challenge_form_submit(self):
        ...
        for urlname in ('captcha-test', 'captcha-test-model-form'):
            ...
            if re.findall('value="([0-9a-f]+)"', str(r1.content)):
                hash_1 = re.findall('value="([0-9a-f]+)"', str(r1.content))[0]
            else:
                self.fail()
            if re.findall('value="([0-9a-f]+)"', str(r2.content)):
                hash_2 = re.findall('value="([0-9a-f]+)"', str(r2.content))[0]
            else:
                self.fail()
            try:
    ...
    def test_autocomplete_off_django_110(self):
        r = self.client.get(reverse('captcha-test'))
        captcha_input = '<input type="text" name="captcha_1" autocomplete="off" spellcheck="false" autocorrect="off" autocapitalize="off" id="id_captcha_1" />'
    ...
    def test_autocomplete_off(self):
        r = self.client.get(reverse('captcha-test'))
        captcha_input = '<input type="text" name="captcha_1" autocomplete="off" spellcheck="false" autocorrect="off" autocapitalize="off" id="id_captcha_1" required />'
        self.assertContains(r, captcha_input, html=True)
