...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/captcha/fields.py
...
class BaseCaptchaTextInput(MultiWidget):
    ...
    def audio_url(self):
        if settings.CAPTCHA_FLITE_PATH:
            return reverse('captcha-audio', kwargs={'key': self._key})
        else:
            return
...
class CaptchaTextInput(BaseCaptchaTextInput):
    ...
    def format_output(self, rendered_widgets):
        if self.output_format:
            ret = self.output_format % {'image': self.image_and_audio, 
               'hidden_field': self.hidden_field, 
               'text_field': self.text_field}
            return ret
        if self.field_template:
            context = {'image': mark_safe(self.image_and_audio), 'hidden_field': mark_safe(self.hidden_field), 
...
class CaptchaField(MultiValueField):
    ...
    def __init__(self, *args, **kwargs):
        ...
        kwargs['widget'] = kwargs.pop('widget', CaptchaTextInput(output_format=kwargs.pop('output_format', None), id_prefix=kwargs.pop('id_prefix', None), generator=kwargs.pop('generator', None)))
