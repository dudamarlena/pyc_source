...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/helpme/main/workers/tasks.py
# Compiled at: 2019-12-18 16:21:59
...
import json, os, re, requests, shutil, sys, tempfile
...
def post(url, data=None, return_json=True):
    ...
    return call(url,
      headers=headers, func=(requests.post), data=data, return_json=return_json)
...
def get(url, headers=None, token=None, data=None, return_json=True):
    ...
    bot.debug('GET %s' % url)
    return call(url,
      headers=headers, func=(requests.get), data=data, return_json=return_json)
...
def stream(url, headers, stream_to=None, retry=True):
    ...
    response = requests.get(url,
      headers=headers, verify=(not DISABLE_SSL_CHECK), stream=True)
...
def call(url, func, data=None, headers=None, return_json=True, stream=False, retry=True):
