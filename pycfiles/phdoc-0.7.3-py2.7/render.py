...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/phdoc/render.py
# Compiled at: 2013-09-24 11:14:27
import os.path as p
from phdoc.config import Config
import markdown, mdx_mathjax, mdx_fold
...
Config.register_default('document-extensions', frozenset(['.md', '.mdown', '.markdown', '.wiki',
 '.text']))
...
def get_markdown_instance(config, curr_path='/', **extra_config):
    ...
    mdconfig = dict(extensions=config['markdown.extensions'], extension_configs=config['markdown.extension-configs'], safe_mode=config['markdown.safe-mode'], output_format=config['markdown.output-format'])
