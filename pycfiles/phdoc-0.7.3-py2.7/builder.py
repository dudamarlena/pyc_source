...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/phdoc/builder.py
# Compiled at: 2013-09-24 11:14:27
import os, os.path as p, re
...
class Builder(object):
    ...
    def walk(self):
        ...
        def valid_extension(filename):
            return any(filename.endswith(valid_ext) for valid_ext in self.config['document-extensions'])

        for dirpath, subdirs, files in os.walk(self.config.wiki_dir):
            remove_hidden(subdirs)
            subdirs.sort()
            remove_hidden(files)
            files.sort()
...
def get_title(filename, data):
    ...
    match = re.search('<!-- ?title:(.+)-->', data, re.IGNORECASE)
    if match:
        return match.group(1).strip()
    match = re.search('<h1[^>]*>([^<]+)</h1>', data, re.IGNORECASE)
    if match:
