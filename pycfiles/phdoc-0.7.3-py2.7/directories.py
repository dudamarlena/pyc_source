...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/phdoc/directories.py
# Compiled at: 2013-09-24 11:14:27
import os.path as p
from phdoc.config import Config

def html_dir(config):
    return p.abspath(p.join(config['meta.root'], config.get('html-dir', config['hide-prefix'] + 'html')))
...
def temp_dir(config):
    return p.abspath(p.join(config['meta.root'], config.get('temp-dir', config['hide-prefix'] + 'tmp')))


def template_dir(config):
    return p.abspath(p.join(config['meta.root'], config.get('template-dir', config['hide-prefix'] + 'templates')))
