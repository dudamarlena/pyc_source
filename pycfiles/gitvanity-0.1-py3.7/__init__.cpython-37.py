...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/gitvanity/__init__.py
# Compiled at: 2018-10-12 01:47:00
# Size of source mod 2**32: 15584 bytes
...
FIX_WORDS = {
 'fix', 'bug', 'bugfix', 'fixes', 'fixed', 'bug'}

class Vanity:

    def __init__(self, output_type='print', output_dir='./vanity', limit=100):
        if output_type not in {'markdown', 'html', 'print'}:
    ...
    def output_index(self):
        if self.output_type == 'markdown':
            ...
            with open(filepath, 'w') as (f):
                ...
                f.write('Vanity leaderboards for your git repository\n')
                f.write('Last updated at %s\n\n' % datetime.now())
            ...
            if self.output_type == 'html':
                filename = 'index.html'
                filepath = os.path.join(self.output_dir, filename)
                metrics_rich = [{'name':metric[0],  'description':metric[1],  'link':self.format_title(metric[0])} for metric in self.metrics_list]
                self.render_template('index.html', filepath, {'metrics':metrics_rich, 
    ...
    def write_html_table(self, title, description, data, headers):
        ...
        filepath = os.path.join(self.output_dir, filename)
        self.render_template('metric.html', filepath, {'title':title, 
    ...
    def is_fix(self, commit):
        words = re.findall('\\w+|\\W+', commit.message.lower())
        for word in words:
    ...
    def run(self):
        ...
        authors = defaultdict(lambda : {'num_lines_total':0, 
         'num_chars_total':0, 
         'num_commits_total':0, 
         'insertions':0, 
         'deletions':0, 
        ...
        for index, commit in enumerate(repo.iter_commits()):
            ...
            if not is_merge:
                if total_change > author_info['largest_change']:
        ...
        if self.output_type == 'print':
            print('')
        self.output_metric('The Real Product Owner', 'Most Lines In HEAD Ref', authors_list, columns=[
         'name', 'num_lines_total'],
          headers=['Author', 'Lines'])
        self.output_metric('Git Master', 'Most Commits', authors_list, columns=['name', 'num_commits_total'], headers=[
         'Author', 'Commits'])
        self.output_metric('Keyboard Wizard', 'Most Insertions/Deletions', authors_list, columns=[
         'name', 'insertions', 'deletions', 'total_changes'],
          headers=[
         'Author', 'Insertions', 'Deletions', 'Total Lines Changed'])
        self.output_metric('Mythical Man Month', 'Most Commits In A Single Month, One Entry Per Author', authors_list,
          columns=['name', 'best_month', 'best_month_count'], headers=[
         'Author', 'Best Month', 'Commits'])
        self.output_metric("It's A Marathon, Not A Sprint", 'Most Commits In A Single Day, One Entry Per Author', authors_list,
          columns=['name', 'best_day', 'best_day_count'], headers=[
         'Author', 'Best Day', 'Commits'])
        self.output_metric('Too Big To Fail', 'Largest Commit (Non-Merge)', authors_list, columns=[
         'name', 'largest_change_sha', 'largest_change_message', 'largest_change'],
          headers=[
         'Author', 'SHA', 'Message', 'Total Lines Changed'])
        self.output_metric('Branch Cutter', 'Most Merge Commits', authors_list, columns=[
         'name', 'merges'],
          headers=['Author', 'Total Merges'])
        self.output_metric('Exterminator', 'Most Commits With Bug Fixes', authors_list,
          columns=['name', 'fixes'], headers=['Author', 'Total Fixes'])
        self.output_metric('Talk Like A Pirate', 'Most Commit Messages That Contain Profanity', authors_list,
          columns=['name', 'badwords'], headers=['Author', 'Total Profane Commits'])
        self.output_metric('Night Owl', 'Most Commits Late At Night', authors_list, columns=[
         'name', 'nightowl'],
          headers=['Author', 'Total Late Night Commits'])
        self.output_metric("It's Been A Long Time", 'Longest Gap Between Commits', authors_list, columns=[
         'name', 'gap_start', 'gap_end', 'gap_length'],
          headers=[
