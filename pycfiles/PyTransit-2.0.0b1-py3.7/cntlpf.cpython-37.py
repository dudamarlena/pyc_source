...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/pytransit/lpf/cntlpf.py
...
def contaminate(flux, cnt, lcids, pbids):
    ...
    for ipv in range(npv):
        for ipt in range(npt):
            ...
            flux[(ipv, ipt)] = c + (1.0 - c) * flux[(ipv, ipt)]
...
class ContaminatedLPF(BaseLPF):
    ...
    def _init_p_planet(self):
        ps = self.ps
        pk2 = [PParameter('k2', 'area_ratio', 'A_s', UP(0.0001, 0.25), (0.0001, 0.25))]
        pcn = [PParameter('cnt_{}'.format(pb), 'contamination', '', UP(0.0, 1.0), (0.0,
                                                                                   1.0)) for pb in self.passbands]
...
class PhysContLPF(BaseLPF):
    ...
    def _init_p_planet(self):
        ps = self.ps
        pk2 = [PParameter('k2_app', 'apparent_area_ratio', 'A_s', UP(0.0001, 0.0625), (0.0001, 0.0625))]
        pcn = [GParameter('k2_true', 'true_area_ratio', 'As', (UP(0.0001, 0.5625)), bounds=(1e-08, inf)),
    ...
    def create_pv_population(self, npop=50):
        ...
        while npv < npop and i < 10:
            ...
            pvp_trial[:, 5] = pvp_trial[:, 4] / (1.0 - cref)
    ...
    def posterior_samples(self, burn=0, thin=1, derived_parameters=True):
        ...
        if derived_parameters:
            ...
            df['t14'] = d_from_pkaiews(df.p.values, df.k_true.values, df.a.values, df.inc.values, 0.0, 0.0, 1)
