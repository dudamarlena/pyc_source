...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/pytransit/utils/keplerlc.py
# Compiled at: 2020-01-13 18:32:58
# Size of source mod 2**32: 6663 bytes
import numpy as np
from numpy import inf, isfinite, abs, sum, unique, ones, compress, array, mean, round, int, where, float64, full_like, nan
...
class KeplerLC(object):
    ...
    def __init__(self, time, flux, quarter, zero_epoch, period, d_transit, d_baseline, error=None, **kwargs):
        ...
        msk_phase = abs(phase - 0.5) * period < 0.5 * d_baseline
        msk_oot = abs(phase - 0.5) * period > 0.5 * d_transit
        msk_inc = isfinite(time) & isfinite(flux) & msk_phase
        self.time = array(time, float64)
        self.flux = array(flux, float64)
        self.qidarr = array(quarter)
        self.tidarr, nt = orbit, orbit[(-1)]
