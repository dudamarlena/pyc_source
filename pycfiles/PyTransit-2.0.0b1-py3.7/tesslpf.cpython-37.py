...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/pytransit/lpf/tesslpf.py
...
from numpy import zeros, squeeze, ceil, arange, digitize, full, nan, sqrt, percentile, isfinite, floor, argsort
...
class TESSLPF(BaseLPF):
    ...
    def __init__(self, name: str, dfile: Path=None, tic: int=None, zero_epoch: float=None, period: float=None, nsamples: int=2, trdur: float=0.125, bldur: float=0.3, use_pdc=False, split_transits=True, separate_noise=False):
        if tic is not None:
            ...
            BaseLPF.__init__(self, name, ['TESS'], times=times, fluxes=fluxes, pbids=pbids, nsamples=nsamples,
              exptimes=[0.00139],
              wnids=wnids)
    ...
    def plot_basic_posteriors(self):
        ...
        corner((df['tc pr rho b k'.split()]), labels=('Zero epoch, Period, Stellar density, impact parameter, radius ratio'.split(', ')))