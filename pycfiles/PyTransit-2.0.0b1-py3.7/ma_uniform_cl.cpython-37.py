...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/pytransit/models/ma_uniform_cl.py
# Compiled at: 2020-03-22 18:42:45
# Size of source mod 2**32: 10387 bytes
...
class UniformModelCL(TransitModel):
    __doc__ = 'Exoplanet transit over a uniform disk (Mandel & Agol, ApJ 580, L171-L175 2002).\n    '
    ...
    def evaluate(self, k: Union[(float, ndarray)], t0: Union[(float, ndarray)], p: Union[(float, ndarray)], a: Union[(float, ndarray)], i: Union[(float, ndarray)], e: Optional[Union[(float, ndarray)]]=None, w: Optional[Union[(float, ndarray)]]=None, copy: bool=True) -> ndarray:
    ...
    def evaluate_ps(self, k, t0, p, a, i, e=0.0, w=0.0, copy=True):
    ...
    def evaluate_pv(self, pvp, copy=True):
        ...
        self.prg.uniform_eccentric_pop(self.queue, (self.npv, self.nptb), None, self._b_time, self._b_lcids, self._b_pbids, self._b_pv, self._b_nsamples, self._b_etimes, self.spv, self.nlc, self.npb, self._b_flux)
        if copy:
