...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/pytransit/contamination/contamination.py
# Compiled at: 2020-03-20 18:01:12
# Size of source mod 2**32: 14786 bytes
...
import pandas as pd, xarray as xa
...
class _BaseContamination:
    ...
    def c_as_pandas(self, ci, teff1, teff2):
        ...
        return pd.DataFrame((self.contamination(ci, teff1, teff2)), columns=pd.Index((self.instrument.pb_names), name='passband'),
          index=pd.Index((uint32(teff2)), name='teff'))

    def c_as_xarray(self, ci, teff1, teff2):
        """Contamination as an xarray DataArray."""
        from xarray import DataArray
        return DataArray((self.contamination(ci, teff1, teff2)), name='contamination',
          dims=[
         'teff', 'passband'],
          coords=[
         uint32(teff2), self.instrument.pb_names],
          attrs={'TEff_1': teff1})
...
class BBContamination(_BaseContamination):
    ...
    def relative_flux(teff: float, wl: Union[(float, ndarray)], wlref: float) -> ndarray:
        ...
        The black body flux normalized to a given reference wavelength
        """
        return planck(teff, 1e-09 * wl) / planck(teff, 1e-09 * wlref)
    ...
    def absolute_fluxes(self, teff: float) -> ndarray:
        ...
        """
        return array([(self.absolute_flux(teff, g) * f(g)).mean() for f, g in zip(self.instrument.filters, self._wl_grids)])
...
class SMContamination(_BaseContamination):
    __doc__ = 'A class that models flux contamination based on stellar spectrum models.\n    '
    ...
    def __init__(self, instrument, ref_pb=None):
        """
          ...
          """
        super().__init__(instrument, ref_pb)
        self._spectra = DataFrame(pd.read_hdf(resource_filename(__name__, join('data', 'spectra.h5')), 'Z0'))
    ...
    def _integrate_fluxes(self, rdc=None):
        rd = self.reddening(rdc) if rdc is not None else 1.0
        i, sp, wl = self.instrument, self._spectra, self.wl
        tr = self.extinction(wl) if self._apply_extinction else 1.0
