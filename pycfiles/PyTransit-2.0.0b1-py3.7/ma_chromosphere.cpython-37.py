...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/pytransit/models/ma_chromosphere.py
...
class ChromosphereModel(TransitModel):
    ...
    def evaluate(self, k: Union[(float, ndarray)], t0: Union[(float, ndarray)], p: Union[(float, ndarray)], a: Union[(float, ndarray)], i: Union[(float, ndarray)], e: Union[(float, ndarray)]=None, w: Union[(float, ndarray)]=None, copy: bool=True) -> ndarray:
        ...
        if isinstance(t0, float):
            if e is None:
                e, w = (0.0, 0.0)
            flux = chromosphere_model_s(self.time, k, t0, p, a, i, e, w, self.lcids, self.pbids, self.nsamples, self.exptimes, self._es, self._ms, self._tae)
        else:
            npv = t0.size
            if e is None:
            ...
            flux = chromosphere_model_v(self.time, k, t0, p, a, i, e, w, self.lcids, self.pbids, self.nsamples, self.exptimes, self._es, self._ms, self._tae)
        return squeeze(flux)
    ...
    def evaluate_pv(self, pvp: ndarray) -> ndarray:
        ...
        flux = chromosphere_model_pv(self.time, pvp, self.lcids, self.pbids, self.nsamples, self.exptimes, self._es, self._ms, self._tae)
        return squeeze(flux)