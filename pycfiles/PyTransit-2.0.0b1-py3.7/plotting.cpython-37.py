...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/pytransit/contamination/plotting.py
...
def plot_kdist(samples, side: str='right', ax=None, clip: tuple=(0, 1), percentiles: tuple=(16, 84), offset: float=0.02, bw=0.005, gridsize: int=200):
    assert side in ('left', 'right')
    sign = 1 if side == 'right' else -1
    fig, ax = (None, ax) if ax is not None else pl.subplots()
    p = sb.kdeplot(samples, kernel='cos', bw=bw, gridsize=gridsize, cut=0, clip=clip, vertical=True, ax=ax, color='k', legend=False)
...
def plot_two_sided_kde(left, right, clip: tuple=(0, 1), percentiles: tuple=(16, 84), offset: float=0.02, bw=0.005, gridsize: int=200, ax=None):
    fig, ax = (None, ax) if ax is not None else pl.subplots()
...
def _jplot(hte, cte, cnr, imp, rho, fw=10, nb=30, gs=25, simulation=False, **kwargs):
    ...
    gs_ct = GridSpec(2, 5, bottom=0.2, top=1, left=0.37, right=1, hspace=0.05, wspace=0.05, height_ratios=[
     0.15, 0.85],
      width_ratios=[
     1, 1, 1, 1, 0.2])
