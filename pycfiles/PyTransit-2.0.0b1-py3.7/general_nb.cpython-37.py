...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/pytransit/models/numba/general_nb.py
# Compiled at: 2020-03-28 19:16:40
# Size of source mod 2**32: 15618 bytes
...
MODE_NORMAL = 0
MODE_TRSPEC = 1
lanczos_g = 6.02468004077673
lanczos_g_minus_half = 5.52468004077673
...
gamma_integral = array([
 ...
 6402373705728000.0, 1.21645100408832e+17, 2.43290200817664e+18,
...
def jcoeff(alpha, beta, ri):
    res = zeros(4)
    res[0] = 2.0 * ri * (ri + alpha + beta) * (2.0 * ri - 2.0 + alpha + beta)
    res[1] = (2.0 * ri - 1.0 + alpha + beta) * (2.0 * ri + alpha + beta) * (2.0 * ri - 2.0 + alpha + beta)
    res[2] = (2.0 * ri - 1.0 + alpha + beta) * (alpha + beta) * (alpha - beta)
    res[3] = -2.0 * (ri - 1.0 + alpha) * (ri - 1.0 + beta) * (2.0 * ri + alpha + beta)
    return res
...
def general_model_vz(z: ndarray, k: float, u, npol, nldc, npb, anm, avl, ajd, aje):
    ...
    n = arange(nldc + 1)
    for ui in range(npb):
        cn[(ui, 0)] = (1.0 - u[ui].sum()) / (1.0 - sum(u[ui] * n[1:] / (n[1:] + 2.0)))
...
def general_model_v(t: ndarray, k: ndarray, t0: ndarray, p: ndarray, a: ndarray, i: ndarray, e: ndarray, w: ndarray, ldc: ndarray, mode: int, lcids: ndarray, pbids: ndarray, nsamples: ndarray, exptimes: ndarray, npb: int, npol: int, nldc: int, es: ndarray, ms: ndarray, tae: ndarray, anm: ndarray, avl: ndarray, ajd: ndarray, aje: ndarray):
    k = atleast_2d(k)
...
def general_model_s(t: ndarray, k: ndarray, t0: float, p: float, a: float, i: float, e: float, w: float, ldc: ndarray, mode: int, lcids: ndarray, pbids: ndarray, nsamples: ndarray, exptimes: ndarray, npb: int, npol: int, nldc: int, es: ndarray, ms: ndarray, tae: ndarray, anm: ndarray, avl: ndarray, ajd: ndarray, aje: ndarray):
    """
...
@njit(parallel=True, fastmath=False)
def general_model_pv(t: ndarray, pvp: ndarray, ldc: ndarray, mode: int, lcids: ndarray, pbids: ndarray, nsamples: ndarray, exptimes: ndarray, npb: int, npol: int, nldc: int, es: ndarray, ms: ndarray, tae: ndarray, anm: ndarray, avl: ndarray, ajd: ndarray, aje: ndarray):
    pvp = atleast_2d(pvp)
    nk = pvp.shape[1] - 6
    return general_model_v(t, (pvp[:, :nk]), t0=(pvp[:, nk]), p=(pvp[:, nk + 1]), a=(pvp[:, nk + 2]), i=(pvp[:, nk + 3]), e=(pvp[:, nk + 4]), w=(pvp[:, nk + 5]), ldc=ldc,
      mode=mode,
      lcids=lcids,
      pbids=pbids,
      nsamples=nsamples,
...
@njit(parallel=True, fastmath=True)
def general_model_s_slow(t, k, t0, p, a, i, e, w, ldc, lcids, pbids, nsamples, exptimes, npb, es, ms, tae, npol, nldc, anm, avl, ajd, aje):
    ldc = atleast_1d(ldc)
    k = atleast_1d(k)
    npt = t.size
    flux = zeros(npt)
