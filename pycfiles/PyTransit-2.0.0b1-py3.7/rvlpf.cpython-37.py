...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/pytransit/lpf/rvlpf.py
# Compiled at: 2020-03-16 19:34:28
# Size of source mod 2**32: 8803 bytes
from matplotlib.pyplot import subplots
from numba import njit
from numpy import zeros, log, pi, inf, atleast_2d, zeros_like, arange, arctan2, cos, squeeze, median, linspace, percentile, argsort, sum
...
def lnlike_normal(o, m, e):
    ...
    for ipv in range(npv):
        lnl[ipv] = -sum(log(e)) - 0.5 * o.size * log(2.0 * pi) - 0.5 * sum((o - m[ipv]) ** 2 / e ** 2)

    return lnl


class RVModel:
    __doc__ = 'Radial velocity mixin class to allow for customised joint modelling of transits and RVs.\n    '
    ...
    def plot_rv_vs_phase(self, planet: int, method='de', pv=None, nsamples: int=200, ntimes: int=500, axs=None):
        ...
        if pvp is not None:
            axs[0].fill_between((phase_model[msids]), (rv_model_limits[(2, msids)]), (rv_model_limits[(3, msids)]), facecolor='blue',
              alpha=0.15)
            axs[0].fill_between((phase_model[msids]), (rv_model_limits[(0, msids)]), (rv_model_limits[(1, msids)]), facecolor='darkblue',
              alpha=0.25)
