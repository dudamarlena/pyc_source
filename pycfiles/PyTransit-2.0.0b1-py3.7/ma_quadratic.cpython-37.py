...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/pytransit/models/ma_quadratic.py
...
from numba.ma_quadratic_nb import quadratic_model_interpolated_pv, calculate_interpolation_tables, quadratic_model_interpolated_v, quadratic_model_interpolated_s, quadratic_model_direct_v, quadratic_model_direct_s, quadratic_model_direct_pv
from .transitmodel import TransitModel
__all__ = [
 'QuadraticModel']

class QuadraticModel(TransitModel):
    __doc__ = 'Transit model with quadratic limb darkening (Mandel & Agol, ApJ 580, L171-L175 2002).\n    '

    def __init__(self, interpolate=True, klims=(0.005, 0.5), nk=256, nz=256):
        ...
        if interpolate:
            ...
            self.ed, self.le, self.ld, self.kt, self.zt = (None, None, None, None,
                                                           None)
    ...
    def evaluate(self, k: Union[(float, ndarray)], ldc: Union[(ndarray, List)], t0: Union[(float, ndarray)], p: Union[(float, ndarray)], a: Union[(float, ndarray)], i: Union[(float, ndarray)], e: Optional[Union[(float, ndarray)]]=None, w: Optional[Union[(float, ndarray)]]=None, copy: bool=True) -> ndarray:
        ...
        if isinstance(t0, float):
            ...
            if self.interpolate:
                flux = quadratic_model_interpolated_v(self.time, k, t0, p, a, i, e, w, ldc, self.lcids, self.pbids, self.nsamples, self.exptimes, self.npb, self._es, self._ms, self._tae, self.ed, self.ld, self.le, self.kt, self.zt)
            else:
                flux = quadratic_model_direct_v(self.time, k, t0, p, a, i, e, w, ldc, self.lcids, self.pbids, self.nsamples, self.exptimes, self.npb, self._es, self._ms, self._tae)
        return squeeze(flux)
    ...
    def evaluate_ps(self, k: Union[(float, ndarray)], ldc: ndarray, t0: float, p: float, a: float, i: float, e: float=0.0, w: float=0.0, copy: bool=True) -> ndarray:
        ...
        if self.time is None:
            ...
            if self.interpolate:
                flux = quadratic_model_interpolated_s(self.time, k, t0, p, a, i, e, w, ldc, self.lcids, self.pbids, self.nsamples, self.exptimes, self.npb, self._es, self._ms, self._tae, self.ed, self.ld, self.le, self.kt, self.zt)
            else:
                flux = quadratic_model_direct_s(self.time, k, t0, p, a, i, e, w, ldc, self.lcids, self.pbids, self.nsamples, self.exptimes, self.npb, self._es, self._ms, self._tae)
        return squeeze(flux)
    ...
    def evaluate_pv(self, pvp: ndarray, ldc: ndarray, copy: bool=True) -> ndarray:
        ...
        if self.time is None:
            ...
            flux = quadratic_model_interpolated_pv(self.time, pvp, ldc, self.lcids, self.pbids, self.nsamples, self.exptimes, self.npb, self._es, self._ms, self._tae, self.ed, self.ld, self.le, self.kt, self.zt)
        else:
            flux = quadratic_model_direct_pv(self.time, pvp, ldc, self.lcids, self.pbids, self.nsamples, self.exptimes, self.npb, self._es, self._ms, self._tae)
        return squeeze(flux)
