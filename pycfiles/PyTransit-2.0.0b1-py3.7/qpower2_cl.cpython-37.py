...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/pytransit/models/qpower2_cl.py
# Compiled at: 2020-03-25 18:35:11
# Size of source mod 2**32: 11589 bytes
from typing import Optional, Union
import numpy as np, pyopencl as cl
from os.path import dirname, join
import warnings
from pyopencl import CompilerWarning
from numpy import array, uint32, float32, int32, asarray, zeros, ones, unique, atleast_2d, squeeze, ndarray, concatenate, empty
...
class QPower2ModelCL(TransitModel):
    ...
    def __init__(self, cl_ctx=None, cl_queue=None):
        super().__init__()
    ...
    def evaluate(self, k: Union[(float, ndarray)], ldc: ndarray, t0: Union[(float, ndarray)], p: Union[(float, ndarray)], a: Union[(float, ndarray)], i: Union[(float, ndarray)], e: Optional[Union[(float, ndarray)]]=None, w: Optional[Union[(float, ndarray)]]=None, copy: bool=True) -> ndarray:
        ...
        self.prg.qpower2_eccentric_pop(self.queue, (self.npv, self.nptb), None, self._b_time, self._b_lcids, self._b_pbids, self._b_p, self._b_u, self._b_nsamples, self._b_etimes, self.spv, self.nlc, self.npb, self._b_f)
        if copy:
            cl.enqueue_copy(self.queue, self.f, self._b_f)
            return squeeze(self.f)
        return

    def evaluate_ps(self, k, ldc, t0, p, a, i, e=0.0, w=0.0, copy=True) -> ndarray:
