...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/navigation_mdp/world.py
# Compiled at: 2020-04-22 14:40:34
# Size of source mod 2**32: 4957 bytes
import itertools, numpy as np, navigation_mdp as NvMDP

class DiscreteLfDWorld2D:
    __doc__ = ' Discrete World specification class '

    def __init__(self, discrete_state_space, phi_spec, r_spec, slip_prob=0.0, s_lst_lst=None, a_lst_lst=None, class_ids=None):
    ...
    def attach_feature_spec(self, phi_spec):
        ...
        if isinstance(phi_spec, list):
            for _DiscreteLfDWorld2D__phi_spec in phi_spec:
                assert isinstance(_DiscreteLfDWorld2D__phi_spec, NvMDP.features.AbstractStateFeatureSpec), 'Invalid feature spec. Supported type: {}'.format(NvMDP.features.AbstractStateFeatureSpec)
                self.S.attach_feature_spec(_DiscreteLfDWorld2D__phi_spec)

        else:
            assert isinstance(phi_spec, NvMDP.features.AbstractStateFeatureSpec), 'Invalid feature spec. Supported type: {}'.format(NvMDP.features.AbstractStateFeatureSpec)
    ...
    def attach_reward_spec(self, r_spec):
        ...
        if isinstance(r_spec, list):
            for _DiscreteLfDWorld2D__r_spec in r_spec:
                assert isinstance(_DiscreteLfDWorld2D__r_spec, NvMDP.reward.AbstractStateRewardSpec), 'Invalid reward spec. Supported type: {}'.format(NvMDP.reward.AbstractStateRewardSpec)
                self.S.attach_reward_spec(_DiscreteLfDWorld2D__r_spec)

        else:
            assert isinstance(r_spec, NvMDP.reward.AbstractStateRewardSpec), 'Invalid reward spec. Supported type: {}'.format(NvMDP.reward.AbstractStateRewardSpec)
    ...
    def preprocessed_features_lst(self, f_key, r_key):
        return self.preprocess(self.features(numpyize=False, key=f_key), r_key)
