...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/navigation_mdp/dynamics.py
# Compiled at: 2020-04-19 02:28:40
...
class XYDynamics(AbstractDynamics):
    ...
    def __init__(self, state_space, slip_prob=0.0):
    ...
    def _next_state(self, state, action):
        ...
        if action == 'U' and loc[0] - 1 >= 0:
            ...
            if action == 'D' and loc[0] + 1 < self.H:
                loc_prime = (
                 loc[0] + 1, loc[1])
            else:
                if action == 'L' and loc[1] - 1 >= 0:
                    loc_prime = (
                     loc[0], loc[1] - 1)
                else:
                    if action == 'R' and loc[1] + 1 < self.W:
                        loc_prime = (
                         loc[0], loc[1] + 1)
                    else:
                        loc_prime = loc
    ...
    def get_next_states_distribution(self, state, action):
        action_list = [
         action] + self.OOPS_ACTIONS[action]
        p_vals = [1.0 - self.slip_prob] + [self.slip_prob] * len(self.OOPS_ACTIONS[action])
    ...
    def infer_action_by_loc(self, loc1, loc2):
        if self._is_valid_loc(loc1):
            raise self._is_valid_loc(loc2) or Exception('Invalid input: locations out of bound!')
        else:
            if loc1[0] == loc2[0]:
                if loc1[1] == loc2[1]:
