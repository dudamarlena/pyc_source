...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pyBuildWeb\build.py
# Compiled at: 2018-02-16 03:58:40
import os, json, urlparse, requests, _utils.exceptions as custom_expections
...
class Build(object):
    ...
    def __init__(self, id, buildsystem='ob', build_api_url='http://buildapi.eng.vmware.com', **kwargs):
        self.id = id
    ...
    def _get_build_details(self):
        ...
        if response.status_code == requests.codes.ok:
            _ = json.loads(response.content, parse_int=int, parse_float=float, date_format='%Y-%m-%d %H:%M:%S.%f', cls=DateTimeDecoder)
            details = json.loads(json.dumps(_, date_format='%Y-%m-%d %H:%M:%S.%f', cls=DateTimeEncoder), base_url=self.build_api_url, key_contains=['url'], cls=URLDecoder)
        else:
    ...
    def deliverables(self):
        if self._deliverables is None:
            ...
            if deliverables_url is None:
                ...
                if response.status_code == requests.codes.ok:
                    _ = json.loads(response.content, parse_int=int, parse_float=float, date_format='%Y-%m-%d %H:%M:%S.%f', cls=DateTimeDecoder)
                    self._deliverables = json.loads(json.dumps(_, date_format='%Y-%m-%d %H:%M:%S.%f', cls=DateTimeEncoder), base_url=self.build_api_url, key_contains=['url'], cls=URLDecoder)
    ...
    def download_deliverables(self, at_path):
        for deliverable in self.deliverables:
            ...
            if response.status_code == requests.codes.ok:
                ...
                raise custom_expections.BuildDownloadError(('Unable to download the file from url: {0}, provided by BuildAPI.').format(deliverable['_download_url']))
