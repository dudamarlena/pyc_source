...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/goscale/templatetags/goscale_tags.py
# Compiled at: 2013-03-04 08:00:28
...
class GoscaleTemplateInclusionTag(InclusionTag):
    ...
    """
    options = Options(MultiKeywordArgument('params', required=False))
...
class Login(GoscaleTemplateInclusionTag):
    ...
    def get_context(self, context, params):
        ...
        if request.method == 'POST' and not url_required:
            ...
            form = form_class()
        ctx = {'login_form': form, 
           'signup_url': passthrough_login_redirect_url(request, reverse('goscale_account_signup')), 
...
class PluginFilters(AsTag):
    ...
    options = Options(MultiKeywordArgument('params', required=False), 'as', Argument('varname', resolve=False, required=False))
    ...
    def get_value(self, context, params):
        ...
        qs = [
         ('=').join([plugin_filters, ('|').join(filters)])]
...
class PluginPost(AsTag):
    ...
    options = Options(Argument('post'), 'as', Argument('varname', resolve=False, required=False))
...
class GoscalePlaceholder(cms_tags.Placeholder):
    ...
    options = cms_tags.PlaceholderOptions(Argument('name', resolve=False), MultiKeywordArgument('goscale', required=False), MultiValueArgument('extra_bits', required=False, resolve=False), blocks=[
