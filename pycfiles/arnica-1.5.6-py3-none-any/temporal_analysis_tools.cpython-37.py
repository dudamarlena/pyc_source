...
# [GCC 8.4.0]
# Embedded file name: /Users/duranton/Documents/CERFACS/CODES/arnica/src/arnica/utils/temporal_analysis_tools.py
...
def show_power_representative_frequency(time, signal, threshold=0.8):
    """
    ...
    if len(data_shape) > 1:
        ...
        for j in range(data_shape[1]):
            ...
            if distribution == 'lognormal':
                ...
                stat, p_value[j] = stats.kstest(data_1d, 'lognorm', args=(sigma, loc, scale))
...
def duration_for_uncertainty(time, signal, target=10, confidence=0.95, distribution='normal'):
    """
...
def uncertainty_from_duration(dtime, sigma, duration, confidence=0.95, distribution='normal'):
    """
...
def convergence_cartography(time, signal, **kwargs):
    ...
    fig = plt.figure()
    fig = plt.contour(durations, interval_lengths, np.transpose(level), [0.6,
     0.8,
     0.9,
     0.95,
     0.975])
    ...
    plt.clabel(fig, inline=1, fontsize=10, colors='k', fmt={0.6:'60%',  0.8:'80%', 
     0.9:'90%', 
     0.95:'95%', 
     0.975:'97.5%'})
...
def plot_distributions(path='./data.txt'):
    ...
    for j in range(S[1]):
        ...
        ax2.hist((clipped_temperatures[lower:upper]), bins=bin_edges, weights=(weights[lower:upper]))
