...
# [GCC 8.4.0]
# Embedded file name: /Users/duranton/Documents/CERFACS/CODES/arnica/src/arnica/solvers_2d/radiation.py
# Compiled at: 2020-03-30 05:30:51
# Size of source mod 2**32: 19382 bytes
...
def get_p1_eq_field(metric, absorption_coeff, spectral_radiance):
    ...
    alpha = 1.0 / absorption_coeff
    beta = -3.0 * absorption_coeff
    ...
    rhs = -12.0 * np.pi * abs_gas * spec_rad
    return (
     lhs, rhs)
...
def analytical_temperature(abs_coeff, radius, temp_1, temp_2):
    ...
    num = 1.0 + 1.5 * tau2 * np.log(tau2 / tau)
    denum = 1.0 + tau2 / tau1 + 1.5 * tau2 * np.log(tau2 / tau1)
    phi = (temp_1 ** 4.0 - temp_2 ** 4.0) * num / denum + temp_2 ** 4.0
    phi = phi ** 0.25
    return phi


def dump_solution(metric, incident_radiation_sol, s_r_sum, s_r_div, analytic_spectral_radiance, analytic_temperature, q_r_flux):
    """
...
def dump_solution_test_case(metric, q_r_flux, params):
    ...
    out_h5.create_grid(metric.x_coor, metric.y_coor, np.zeros(metric.shp2d))
    adim_flux_x = adim_flux_test_case(q_r_flux[0].reshape(metric.shp2d), params)
    adim_flux_y = adim_flux_test_case(q_r_flux[1].reshape(metric.shp2d), params)
    adim_flux_n = adim_flux_test_case(q_r_flux[2].reshape(metric.shp2d), params)
...
def get_heat_flux_inner(x_coor, y_coor, q_n):
    """get_heat_flux_inner"""
    rad = np.sqrt(x_coor ** 2.0 + y_coor ** 2.0)
    r_min = 1.0
    tol = 1e-07
    mask = np.where(rad < r_min + tol)
    idx = np.argmin(x_coor[mask])
    heat_flux_inner = q_n[mask]
    return float(-1.0 * heat_flux_inner[idx])


def get_heat_source_term(g_solution, absorption_coefficient, spectral_radiance):
    """
    ...
    radiative_source_term = 4.0 * np.pi * spectral_radiance - g_solution
...
def get_heat_fluxes(absorption_coefficient, metric, g_solution):
    ...
    """
    print('\n\t\t>> Compute heat fluxes')
    alpha = 1.0 / absorption_coefficient
    gamma = np.diag(-alpha.ravel() / 3.0)
    ...
    heat_flux_n_bc = metric.n_x.dot(heat_flux_x) + metric.n_y.dot(heat_flux_y)
    return (
     heat_flux_x, heat_flux_y, heat_flux_n_bc)
...
def analytical_test_case(metric, params):
    ...
    radius = np.sqrt(metric.x_coor ** 2.0 + metric.y_coor ** 2.0)
    print('\n\t--> Compute temperature field')
    analytic_temperature = analytical_temperature(params['field']['absorption_coefficient'], radius, t_west, t_east)
    ...
    return (
     analytic_temperature, analytic_spectral_radiance, absorption_field)
...
def get_spectral_radiance(temp):
    ...
    """
    sigma = 5.670373e-08
    return temp ** 4.0 / np.pi * sigma


def postproc_radiation(metric, incident_radiation_sol, absorption_field, analytic_spectral_radiance):
    """
    ...
    """
    heat_fluxes = get_heat_fluxes(absorption_field, metric, incident_radiation_sol)
    radiative_source_term = get_heat_source_term(incident_radiation_sol, absorption_field, analytic_spectral_radiance)
    radiative_source_term_div = get_source_term_div(heat_fluxes, metric)
    return (
     heat_fluxes, radiative_source_term, radiative_source_term_div)
...
def adim_flux_test_case(dim_data, params):
    ...
    def adim_factor(temp1, temp2):
        ...
        """
        sigma = 5.670373e-08
        return 1.0 / (sigma * (temp1 ** 4.0 - temp2 ** 4.0))
...
def compute_robin_values_p1(absorption_coeff, t_wall, emissivity=1):
    ...
    """
    a_value = (2.0 - emissivity) / emissivity * 2.0
    a_value /= 3.0 * absorption_coeff
    b_value = 1.0
    c_value = -4.0 * np.pi * get_spectral_radiance(t_wall)
    return (
    ...
    for _, bc_dict in bc_params.items():
        if bc_dict['type'] == 'Robin':
            ...
            if 'Emissivity' in bc_dict['Values'].keys():
                ...
                bc_val = compute_robin_values_p1(absorption_coeff, t_wall, emissivity=emissivity)
            else:
...
def main_p1_solver(params):
    ...
    metric = Metrics2d(x_coor, y_coor, periodic_ns=(bnd.periodic_ns), periodic_we=(bnd.periodic_we))
    ...
    lhs_csr, rhs_csr = get_p1_eq_field_csr(metric, absorption_field, analytic_spectral_radiance)
    print('\n\tmain ---> Apply BC')
    params['boundaries'] = set_bc_p1(params['boundaries'], np.mean(absorption_field))
    ...
    print('\n\tmain ---> Postproc...')
    q_r_flux, s_r_sum, s_r_div = postproc_radiation(metric, incident_radiation_sol, absorption_field, analytic_spectral_radiance)
    heat_flux_scalar = postproc_analytical_test_case(metric, params, q_r_flux)
    print('\n\tmain ---> Write Solution')
    dump_solution(metric, incident_radiation_sol, s_r_sum, s_r_div, analytic_spectral_radiance, analytic_temperature, q_r_flux)
...
if __name__ == '__main__':
    ...
    BOUNDARY['West']['Values'] = {'Emissivity':1.0,  'Wall_temperature':500.0}
    BOUNDARY['East']['type'] = 'Robin'
    BOUNDARY['East']['Values'] = {'Emissivity':1.0,  'Wall_temperature':1500.0}
    FIELD = {'absorption_coefficient':1.0, 
     'temperature':[
      T_WEST, T_EAST]}
    PARAMS = {}
