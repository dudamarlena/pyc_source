...
# [GCC 8.4.0]
# Embedded file name: /Users/duranton/Documents/CERFACS/CODES/arnica/src/arnica/utils/nparray2xmf.py
# Compiled at: 2020-03-30 05:30:51
# Size of source mod 2**32: 9911 bytes
...
class NpArray2Xmf:
    __doc__ = ' main class for data output in XDMF format '

    def __init__(self, filename, domain_name=None, mesh_name=None, time=None, xmf_only=False):
        """ class startup"""
    ...
    def _type(self, var):
        ...
        if numbertype is None:
            raise RuntimeError('Array of type ' + dtype + '(' + var + ')' + 'not recognized')
        attributetype = None
        ...
        if attributetype is None:
            raise RuntimeError('Var ' + var + ' of shape ' + str(var_shape) + ' not consistent  with grid of shape ' + str(self.shape) + '\n (neither scalar nor 3D vector...)')
    ...
    def xmf_dump(self):
        ...
        xmf_tree['dom'] = etree.SubElement((xmf_tree['root']), 'Domain', Name=(self.mesh['domain']))
        xmf_tree['grd'] = etree.SubElement((xmf_tree['dom']), 'Grid', Name=(self.mesh['mesh']),
          Type='Uniform')
        etree.SubElement((xmf_tree['grd']), 'Time', Type='Single',
          Value=(self.mesh['time']))
        etree.SubElement((xmf_tree['grd']), 'Topology', Name='Topo',
          TopologyType=topology,
          NumberOfElements=dims)
        xmf_tree['geo'] = etree.SubElement((xmf_tree['grd']), 'Geometry', GeometryType='X_Y_Z')
        for var in ('x', 'y', 'z'):
            field = etree.SubElement((xmf_tree['geo']), 'DataItem', Dimensions=dims,
              Format='HDF',
              NumberType='Float',
              Precision='8')
...
def create_time_collection_xmf(collection_filenames, xmf_filename):
    ...
    grid = etree.SubElement(dom, 'Grid', Name=(os.path.split(xmf_filename)[(-1)]),
      GridType='Collection',
      CollectionType='Temporal')
    for filename in collection_filenames:
        etree.SubElement(grid, 'XI_INCLUDE', href=filename, xpointer='xpointer(//Xdmf/Domain/Grid)')

    xmf_ct = etree.tostring(root, pretty_print=True,
      xml_declaration=True,
      doctype='<!DOCTYPE Xdmf SYSTEM "Xdmf.dtd" []>')
...
if __name__ == '__main__':
    ...
    TEST_U = np.sin(TEST_X / SIZE_X * 1 * np.pi) * np.sin(TEST_Y / SIZE_Y * 1 * np.pi) * np.sin(TEST_Z / SIZE_Z * 1 * np.pi)
    ...
    TEST_U = np.sin(TEST_X / SIZE_X * 1 * np.pi) * np.sin(TEST_Y / SIZE_Y * 1 * np.pi) * np.sin(TEST_Z * 0.5 * np.pi)
