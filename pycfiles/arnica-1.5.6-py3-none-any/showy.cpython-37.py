...
# [GCC 8.4.0]
# Embedded file name: /Users/duranton/Documents/CERFACS/CODES/arnica/src/arnica/utils/showy.py
# Compiled at: 2020-03-30 05:30:51
# Size of source mod 2**32: 11040 bytes
r"""
showy.py
...
def showy(layout, data, show=True):
    ...
    for graph_layout in layout['graphs']:
        if not graph_position > display_params['numb_max_of_graphs_per_fig']:
            if figure_number == 0:
                graph_position = 1
                figure_number += 1
                figure = build_new_figure(display_params, figure_number)
            build_new_graph(figure, display_params['fig_struct'], graph_position, graph_layout, dataframe)
            graph_position += 1
...
def initialize_display(layout):
    ...
    if 'figure_structure' in layout:
        ...
        if display_params['numb_of_graphs'] == 1:
            display_params['fig_struct'] = [
             1, 1]
        else:
            display_params['fig_struct'] = [
             2, 3]
    display_params['numb_max_of_graphs_per_fig'] = display_params['fig_struct'][0] * display_params['fig_struct'][1]
    display_params['numb_of_figs'] = ceil(display_params['numb_of_graphs'] / display_params['numb_max_of_graphs_per_fig'])
...
def build_new_figure(display_params, figure_number):
    ...
    if display_params['lay_title']:
        if display_params['numb_of_graphs'] <= display_params['numb_max_of_graphs_per_fig']:
            ...
            figure_name = display_params['lay_title'] + ' (' + str(figure_number) + '/' + str(display_params['numb_of_figs']) + ')'
        figure.canvas.set_window_title(figure_name)
        figure.suptitle(figure_name, fontsize=20, fontweight='bold',
          y=0.99)
    figure.subplots_adjust(left=0.1, bottom=0.1, right=0.9,
      top=0.9,
      wspace=0.3,
      hspace=0.3)
    return figure
...
def build_new_graph(figure, figure_structure, graph_position, graph_layout, dataframe):
    ...
    if 'title' in graph_layout:
        subplot.set_title((graph_layout['title']), fontweight='bold')
    if graph_layout['x_var'] in dataframe.keys():
        for curve_layout in graph_layout['curves']:
            if curve_layout['var'] in dataframe.keys():
                ...
                subplot.plot((dataframe[graph_layout['x_var']]), (dataframe[curve_layout['var']]),
                  label=curve_legend)
