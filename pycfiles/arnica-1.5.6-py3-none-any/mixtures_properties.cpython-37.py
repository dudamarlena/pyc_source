...
# [GCC 8.4.0]
# Embedded file name: /Users/duranton/Documents/CERFACS/CODES/arnica/src/arnica/phys/mixtures_properties.py
# Compiled at: 2020-04-12 11:02:32
# Size of source mod 2**32: 11939 bytes
...
class MixtureAtomic:
    ...
    def _instanciate_species_atomic(self, species_dict):
        """
        *Instanciate SpeciesAtomic object for each species of the mixture*

        :param species_dict: Dict['O2', 'N2', ..] of species mass fraction
        :param fuel: Fuel name
        :type fuel: str

        :returns: List of SpeciesAtomic object
        """
        species_list = []
        for species, mass_fraction in species_dict.items():
            species_list.append(SpeciesAtomic(species, mass_fraction))
    ...
    def _update_streams(self, stream_update):
        ...
        *Update stream data for mixture's species*
        ...
        for species, stream_data in self._stream_dict.items():
            self.species_by_name(species).set_stream_data(stream_data['stream'], stream_data['mass_frac'])
    ...
    def _check_mass_fraction(self):
        ...
        if not np.allclose(mass_fraction, 1.0, atol=0.0001):
            non_unit = np.where(np.abs(mass_fraction - 1.0) > 0.0001)[0]
