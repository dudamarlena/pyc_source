...
# [GCC 8.4.0]
# Embedded file name: /Users/duranton/Documents/CERFACS/CODES/arnica/src/arnica/utils/shell.py
# Compiled at: 2020-04-13 05:42:09
# Size of source mod 2**32: 12108 bytes
"""
...
class Shell:
    ...
    def dump_shell(self, name='shell'):
        ...
        *Dump shell geometrical properties into hdf file*
        ...
        xmdf.create_grid(np.take((self._xyz), 0, axis=(-1)), np.take((self._xyz), 1, axis=(-1)), np.take((self._xyz), 2, axis=(-1)))
    ...
    def set_mask_on_shell(self, point_cloud, tol):
        ...
        *Create a mask on the shell from a point cloud*
        ...
        mask = np.where(dists.reshape(self.shape) > tol, 1.0, 0.0)
        return mask
        ...
        *Create a millefeuille-like shell.
        ...
        for j, n_j in enumerate((self._n_x, self._n_y, self._n_z)):
            for i in range(n_layers):
                cake['xyz'][:, :, i, j] = np.take(self._xyz, j, -1) + (1.0 * i / n_layers * self.width_matrix[width_matrix_label] + shift) * n_j
    ...
    def average_on_shell_over_dirs(self, variable, directions, scale=True):
        ...
        *Performs an integration (average) over one or multiple directions*
        ...
        :param variable: A np.array to be averaged of dim (n_time, n_v, n_u)        :param directions: A list() of directions on which the average process                           is to be performed.                           Contains keywords from ['time','v','u'].
    ...
    def _compute_shell_crest(self, ctrl_pts_1, ctrl_pts_2):
        ...
        *Build the shell crest of shape (2, self.shape[1]) and respecting controle points*
        ...
        tck, _ = interpolate.splprep([ctrl_pts_1,
         ctrl_pts_2],
          s=SPLINE_SMOOTHNESS,
          k=spline_order)
