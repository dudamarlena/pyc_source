...
# [GCC 8.4.0]
# Embedded file name: /Users/duranton/Documents/CERFACS/CODES/arnica/src/arnica/utils/datadict2file.py
# Compiled at: 2020-04-13 05:42:09
# Size of source mod 2**32: 11476 bytes
""" module to data array-like dictionnary to files
    for visulaisation or storage pupopses
"""
import importlib, os, numpy as np
from arnica.utils.nparray2xmf import create_time_collection_xmf, NpArray2Xmf

def dump_dico_2d_nparrays(data_dict, filename, x_coords, y_coords, z_coords, **kw_args):
    """
    ...
    visu_ = NpArray2Xmf(filename, time=time,
      mesh_name=grid_name,
...
def dump_dico_2d_time_nparrays(data_dict, root_path, prefix, x_coords, y_coords, z_coords, **kw_args):
    ...
    if times is None:
        times = np.linspace(0.0, (n_time - 1.0), num=n_time, dtype=float)
    if steps is None:
    ...
    for step in range(n_time):
        ...
        dump_dico_2d_nparrays(step_dict, filename, x_coords, y_coords, z_coords,
          time=(times[step]), domain_name=prefix, grid_name=grid_name)
...
def dump_dico_1d_nparrays(filename, data_dict):
    ...
    if format_ not in ('csv', 'xlsx'):
        msg = (
         'Extension', format_, "is not supported. Supported extensions : 'csv', 'xlsx'. Default extension 'csv' will be used.")
        ...
        if format_ == 'xlsx':
            ...
            for package in ('pandas', 'openpyxl'):
                packages_exists &= bool(importlib.util.find_spec(package))

            if packages_exists:
                import pandas as pd, openpyxl
            else:
                msg = "dump_dico_1d_nparrays cannot find 'pandas' neither/nor 'openpyxl' packages. Import those packages or use '.csv' extension to dump classic csv files instead."
...
def dump_dict2xmdf(filename, grid, data_dict):
    ...
    *Dump 2D matrices into hdf5 file*
    ...
    xmdf.create_grid(np.take(grid, 0, -1), np.take(grid, 1, -1), np.take(grid, 2, -1))
