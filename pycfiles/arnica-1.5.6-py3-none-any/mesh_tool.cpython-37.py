...
# [GCC 8.4.0]
# Embedded file name: /Users/duranton/Documents/CERFACS/CODES/arnica/src/arnica/utils/mesh_tool.py
...
def gen_cart_grid_2d(gridrange, gridpoints):
    ...
    """
    x_vec = np.linspace(0.0, (gridrange[0]), (int(gridpoints[0])), endpoint=True)
    y_vec = np.linspace(0.0, (gridrange[1]), (int(gridpoints[1])), endpoint=True)
    x_coor, y_coor = np.meshgrid(x_vec, y_vec)
    return (
     x_coor, y_coor)


def gen_cyl_grid_2d(r_min, r_max, r_points, theta_min, theta_max, theta_points):
    """
...
def dilate_center(x_coor, y_coor, perturbation=0.1):
    ...
    adim_rad = np.sqrt(((x_coor - x_center) / (0.5 * x_width)) ** 2 + ((y_coor - y_center) / (0.5 * y_width)) ** 2)
    rad_dir_x = (x_coor - x_center) / np.maximum(adim_rad, 1e-16)
    rad_dir_y = (y_coor - y_center) / np.maximum(adim_rad, 1e-16)
    dilate = np.maximum(1.0 - adim_rad, 0)
    ...
    return (
     x_coor + shift_x, y_coor + shift_y, dilate)
...
def get_mesh(params_mesh):
    ...
    if params_mesh['kind'] == 'rect':
        ...
        if params_mesh['kind'] == 'cyl':
            ...
            theta_max -= delta
            x_coor, y_coor = gen_cyl_grid_2d(r_min, r_max, n_r_pts, theta_min, theta_max, int(angle * n_r_pts + 1))
        else:
            raise NotImplementedError('The only mesh kinds implemented are: "rect" and "cyl"')
    return (
     x_coor, y_coor)