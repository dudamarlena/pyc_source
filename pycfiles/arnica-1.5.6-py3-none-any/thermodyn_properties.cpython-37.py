...
# [GCC 8.4.0]
# Embedded file name: /Users/duranton/Documents/CERFACS/CODES/arnica/src/arnica/phys/thermodyn_properties.py
...
def thermal_constants():
    ...
    consts['beta_kader'] = (3.85 * consts['prandtl'] ** 0.3333333333333333 - 1.3) ** 2 + 2.12 * np.log(consts['prandtl'])
    consts['k_kader'] = consts['beta_kader'] - consts['prandtl_turb'] * consts['van_driest'] + consts['prandtl_turb'] / consts['vkarman'] - 2.12 - (consts['prandtl_turb'] / consts['vkarman'] - 2.12) * (200.0 * np.log(200.0) - 100 * np.log(100.0)) / 100.0
    return consts
...
def _estimate_t_tau(u_tau, y_plus, u_2, t_wall, t_2):
    ...
    t_tau_cwm = (t_wall - t_2) / (consts['prandtl'] * y_plus['all'] * np.exp(-kader_g) + (consts['prandtl_turb'] * u_2 / u_tau['cwm'] + consts['k_kader']) * np.exp(-1.0 / kader_g))
    return t_tau_cwm
...
def h_kader(t_wall, rho_wall, y_wall, u_2, t_2, temp_adiab):
    ...
    y_plus['guess'] = 0.157 * (u_2 * y_rho_mu_wall) ** 0.875
    y_plus['guess'] = np.maximum(y_plus['guess'], 0.001)
    u_tau = dict()
    u_tau['lin'] = np.sqrt(u_2 / y_rho_mu_wall)
    u_tau['log'], y_plus['log'] = _estimate_log_region(y_plus['guess'], u_2, y_rho_mu_wall)
...
def fluid_cp(temp, clipping=False):
    ...
    coeffs = [
     513.46, 1.6837, -0.0019275, 1.2773e-06, -4.2773e-10, 5.6735e-14]
    heat_cp = coeffs[0] * np.ones_like(t_clip) + coeffs[1] * t_clip + coeffs[2] * np.power(t_clip, 2) + coeffs[3] * np.power(t_clip, 3) + coeffs[4] * np.power(t_clip, 4) + coeffs[5] * np.power(t_clip, 5)
    return heat_cp