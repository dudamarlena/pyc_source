...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/modify_history/backends/comment.py
...
class CommentHistoryBackend(BasicHistoryBackend):
    ...
    def get_message(self, timeline):
        ...
        if timeline.action == 'create':
            return mark_safe(_("%(user)s has comment to '%(label)s'") % kwargs)
        if timeline.action == 'update':
            return mark_safe(_("%(user)s update a comment of '%(label)s'") % kwargs)
        return mark_safe(_("%(user)s delete a comment of '%(label)s'") % kwargs)
    ...
    def autodiscover(self, instance, action, url=None, label=None, user=None):
        ...
        if cleaned_data:
            ...
            timeline = Timeline.objects.create(content_type=ContentType.objects.get_for_model(instance), object_id=instance.pk, action=action, url=url, label=label, user=user)
            return timeline
        else:
            return
            return