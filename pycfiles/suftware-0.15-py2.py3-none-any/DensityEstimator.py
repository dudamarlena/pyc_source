...
# [GCC 8.4.0]
# Embedded file name: suftware/src/DensityEstimator.py
...
class DensityEstimator:
    ...
    def plot(self, ax=None, save_as=None, resample=True, figsize=(4, 4), fontsize=12, title='', xlabel='', tight_layout=False, show_now=True, show_map=True, map_color='blue', map_linewidth=2, map_alpha=1, num_posterior_samples=None, posterior_color='dodgerblue', posterior_linewidth=1, posterior_alpha=0.2, show_histogram=True, histogram_color='orange', histogram_alpha=1, show_maxent=False, maxent_color='maroon', maxent_linewidth=1, maxent_alpha=1, backend='TkAgg'):
        ...
        if ax is None:
            ...
            tight_layout = True
        if show_histogram:
            ax.bar(self.grid, self.histogram, width=self.grid_spacing, color=histogram_color, alpha=histogram_alpha)
        if show_maxent:
            ax.plot(self.grid, self.maxent, color=maxent_color, linewidth=maxent_linewidth, alpha=maxent_alpha)
        ...
        if num_posterior_samples > 0:
            ...
            ax.plot(self.grid, sample_values[:, :num_posterior_samples], color=posterior_color, linewidth=posterior_linewidth, alpha=posterior_alpha)
        if show_map:
            ax.plot(self.grid, self.values, color=map_color, linewidth=map_linewidth, alpha=map_alpha)
    ...
    def evaluate_samples(self, x, resample=True):
        ...
        values = np.array([ d.evaluate(x_arr) for d in self.sample_density_funcs
                          ]).T
        if resample:
            probs = self.sample_weights / self.sample_weights.sum()
            old_cols = np.array(range(self.num_posterior_samples))
            new_cols = np.random.choice(old_cols, size=self.num_posterior_samples, replace=True, p=probs)
    ...
    def get_stats(self, use_weights=True, show_samples=False):
        ...
        def skewness(Q):
            ...
            x = self.grid
            h = self.grid_spacing
            return np.sum(h * Q * (x - mu) ** 3) / np.sum(h * Q * (x - mu) ** 2) ** (3 / 2)
        ...
        col2func_dict = {'entropy': entropy, 'mean': mean, 
           'variance': variance, 
           'skewness': skewness, 
           'kurtosis': kurtosis}
