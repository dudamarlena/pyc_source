...
# [GCC 8.4.0]
# Embedded file name: suftware/src/deft_core.py
...
def compute_corrector_step(phi, R, Delta, t, N, tollerance, report_num_steps=False):
    ...
    if not isinstance(report_num_steps, bool):
        raise ControlledError('/compute_corrector_step/ report_num_steps must be a boolean: report_num_steps = %s' % type(report_num_steps))
    Q = utils.field_to_prob(phi)
    S = action(phi, R, Delta, t, N)
