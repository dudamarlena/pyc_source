...
# [GCC 8.4.0]
# Embedded file name: /Users/tareen/Desktop/suftware_release_0P13/suftware/src/utils.py
# Compiled at: 2018-04-12 16:18:01
# Size of source mod 2**32: 17084 bytes
    ...
    if not all(np.isreal(P)):
        ...
        if not all(P >= 0):
            ...
            if not any(P > 0):
                ...
                raise all(Q >= 0) or ControlledError('/geo_dist/ Q is not non-negative: Q = %s' % Q)
            raise any(Q > 0) or ControlledError('/geo_dist/ Q is vanishing: Q = %s' % Q)
        P_prob = P / sp.sum(P)
        Q_prob = Q / sp.sum(Q)
        try:
