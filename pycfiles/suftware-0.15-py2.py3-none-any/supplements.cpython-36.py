...
# [GCC 8.4.0]
# Embedded file name: /Users/tareen/Desktop/suftware_release_0P13/suftware/src/supplements.py
# Compiled at: 2018-04-12 13:12:13
# Size of source mod 2**32: 24407 bytes
...
def y_sampling_of_GLap(inputs_array):
    ...
    if similar_enough:
        ...
        for j in range(num_grid):
            Lap_bin_centers_dist[j] = distribution(eig_val, eig_vec, (Lap_bin_centers[j]),
              lambdas, switch=0)
        ...
        for j in range(num_grid):
            GLap_bin_centers_dist[j] = distribution(eig_val, eig_vec, (GLap_bin_centers[j]),
              lambdas,
              switch=1)
    ...
    if not sampling:
        ...
        if similar_enough:
            ...
            for j in range(num_grid):
                bin_centers_dist[j] = distribution(eig_val, eig_vec, (bin_centers[j]), lambdas,
                  switch=1)
