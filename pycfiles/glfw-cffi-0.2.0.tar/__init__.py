...
# [GCC 8.4.0]
# Embedded file name: glfw/__init__.py
...
def _get_function_declaration(line):
    ...
    if all(_ in line for _ in ('(', ')', ';')):
        if '@' not in line:
            if line.lstrip(' ').startswith('typedef') or eng.search(line.strip()):
                found = [ group.groupdict() ][0]
...
def _wrap_func(ffi, func_decl, func):
    ...
    def wrapper(*args, **kwds):
        ...
        if retval in [ffi.NULL, []]:
            ...
            retval = [ (ffi.string(l[0]) if isinstance(l[0], ffi.CData) and 'char' in ffi.typeof(l).cname else l[0]) ]
...
def _camelToSnake(string):
    ...
    patterns = [
     ('(.)([0-9]+)', '\\1_\\2'),
     ('([a-z]+)([A-Z])', '\\1_\\2')]
    engines = [ (pattern, replacement, re.compile(pattern)) ]
    for data in engines:
...
def _initialize_module(ffi):
    ...
    camelCase = {_camelToSnake(d.replace('glfw', '')):getattr(_glfw, d) for d in dir(_glfw) if not (hasattr(_glfw, d) and d.startswith('_')) if hasattr(getattr(_glfw, d), '__call__')}
    ...
    opengl_snake_case = {_camelToSnake(d.replace('gl', '')):getattr(_gl, d) for d in dir(_gl) if not (hasattr(_gl, d) and d.upper() == d) if not d.startswith('GL_') if not d.startswith('GL_')}
...
for func in globs:
    if func not in ('_ffi', '_glfw'):
        if func.startswith('_') and func.startswith('__') and func in ('modname', ):
            globals().pop(func)
