...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/orlo/routes/releases.py
...
def post_releases():
    ...
    **Example curl**:
    ...
    app.logger.info(('Create release {}, references: {}, platforms: {}').format(release.id, release.notes, release.references, release.platforms, release.metadata))
    release.start()
...
def post_packages(release_id):
    ...
    **Example curl**:
    ...
    app.logger.info(('Create package {}, release {}, name {}, version {}').format(package.id, release.id, request.json['name'], request.json['version']))
    db.session.add(package)
    db.session.commit()
    return jsonify(id=package.id)


@app.route('/releases/<release_id>/packages/<package_id>/results', methods=[
 'POST'])
@conditional_auth(token_auth.token_required)
def post_results(release_id, package_id):
    ...
    app.logger.info(('Post results, release {}, package {}').format(release_id, package_id))
...
@app.route('/releases/<release_id>/packages/<package_id>/start', methods=[
 'POST'])
@conditional_auth(token_auth.token_required)
def post_packages_start(release_id, package_id):
    ...
    **Example curl**:
    ...
    app.logger.info(('Package start, release {}, package {}').format(release_id, package_id))
    package.start()
    db.session.add(package)
    db.session.commit()
    return ('', 204)
...
@app.route('/releases/<release_id>/packages/<package_id>/stop', methods=[
 'POST'])
@conditional_auth(token_auth.token_required)
def post_packages_stop(release_id, package_id):
    ...
    **Example curl**:
    ...
    package = fetch_package(release_id, package_id)
    app.logger.info(('Package stop, release {}, package {}, success {}').format(release_id, package_id, success))
...
def get_releases(release_id=None):
    ...
    :query string status: Filter by release status. This field is calculated         from the package status, see special note below.
    ...
    :query string package_status: Filter by package status. Valid statuses are:         "NOT_STARTED", "IN_PROGRESS", "SUCCESSFUL", "FAILED"
