...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/orlo/queries.py
...
def filter_release_status(query, status):
    ...
    if status not in enums:
        raise InvalidUsage(('Invalid package status, {} is not in {}').format(status, str(enums)))
...
def filter_release_rollback(query, rollback):
    ...
    """
    if rollback is True:
        query = query.filter(Release.packages.any(Package.rollback == True))
    elif rollback is False:
        query = query.filter(~Release.packages.any(Package.rollback == True))
    else:
        raise TypeError(("Bad rollback parameter: '{}', type {}. Boolean expected.").format(rollback, type(rollback)))
    return query
...
def build_query(object_type, limit=None, offset=None, asc=None, **kwargs):
    ...
    if object_type is Release:
        ...
        if object_type is Package:
            filter_function = apply_package_filters
        else:
            raise OrloError(('build_query does not support object {}').format(object_type))
        if any(field.startswith('package_') for field in kwargs.keys()) or 'status' in kwargs.keys() and object_type is Release:
            if object_type is not Release:
                raise InvalidUsage("'package_' parameters are only valid for Release queries. (hint: retry without the package_ prefix)")
            else:
        ...
        try:
            ...
            raise InvalidUsage(('An invalid field for table {} was specified: {}').format(object_type.__tablename__, e.args[0]))
...
def user_info(username):
    ...
    :return:
    """
    query = db.session.query(Release.user, db.func.count(Release.id)).filter(Release.user == username).group_by(Release.user)
    return query
...
def team_info(team_name):
    ...
    :return:
    """
    query = db.session.query(Release.user, db.func.count(Release.id)).filter(Release.team == team_name).group_by(Release.team)
    return query
...
def package_info(package_name):
    ...
    :return:
    """
    query = db.session.query(Package.name, db.func.count(Package.id)).filter(Package.name == package_name).group_by(Package.name)
    return query
...
def package_versions(platform=None, by_release=False):
    ...
    :param bool by_release: If true, a package, that is part of a release which
        ...
        Default: False.
    """
    sub_q = db.session.query(Package.name.label('name'), db.func.max(Package.stime).label('max_stime')).filter(Package.status == 'SUCCESSFUL')
    ...
    sub_q = sub_q.group_by(Package.name).subquery()
    q = db.session.query(Package.name, Package.version).join(sub_q, and_(sub_q.c.max_stime == Package.stime, sub_q.c.name == Package.name)).group_by(Package.name, Package.version)
    return q


def count_releases(user=None, package=None, team=None, platform=None, status=None, rollback=None, stime=None, ftime=None):
    """
    ...
    """
    args = {'user': user, 
       'package': package, 'team': team, 'platform': platform, 'status': status, 
...
def platform_summary():
    ...
    """
    query = db.session.query(Platform.name, db.func.count(Platform.id)).join(release_platform).group_by(Platform.name)
    return query
...
def platform_info(platform_name):
    ...
    :return:
    """
    query = db.session.query(Platform.name, db.func.count(Platform.id)).filter(Platform.name == platform_name).group_by(Platform.name)
    return query
...
def platform_list():
    ...
    :return: Query result [('platform'), release_count]
    """
    query = db.session.query(Platform.name.distinct()).join(release_platform)
    return query