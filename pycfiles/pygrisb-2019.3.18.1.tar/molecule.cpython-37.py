...
# [GCC 8.4.0]
# Embedded file name: /home/ykent/GitLab/pygrisb/pygrisb/pygrisb/gutz/molecule.py
# Compiled at: 2019-02-23 22:41:15
# Size of source mod 2**32: 7639 bytes
import sys, numpy
from pymatgen import Molecule
imol = 0

class gmolecule(Molecule):
    __doc__ = 'adding equivalent indices to atoms.\n    '

    def __init__(self, species, coords, equivalent_indices=[], charge=0, fm_direction=None, spin_multiplicity=None, validate_proximity=False, site_properties=None):
        super(Molecule, self).__init__(species, coords, charge=charge, spin_multiplicity=spin_multiplicity,
...
def xtal_get_local_rot(symbols, scaled_positions, cell, iat, dist_cut_, equivalent_indices=[], locrot=None, fm_direction=None, Nmax=4, tol=1e-05, log=sys.stdout):
    """
    Get rotation operations of center atom iat in crystal.
    """
    mol = xtal_extract_mol(symbols, scaled_positions, cell, iat, dist_cut_,
...
def xtal_extract_mol(symbols, scaled_positions, cell, iat, dist_cut_, locrot=None, fm_direction=None, Nmax=10, equivalent_indices=[]):
    """
        ...
        for j in Nmax_list:
            for k in Nmax_list:
                for jat, sp in enumerate(new_scaled_positions):
                    ...
                    n_p = n_sp[0] * numpy.array(cell[0]) + n_sp[1] * numpy.array(cell[1]) + n_sp[2] * numpy.array(cell[2])
    ...
    molecule_positions = [mpos for mpos, pdist in zip(molecule_positions, pair_dist) if pdist < dist_cut]
    molecule_symbols = [msym for msym, pdist in zip(molecule_symbols, pair_dist) if pdist < dist_cut]
    eq_indices = [eind for eind, pdist in zip(eq_indices, pair_dist) if pdist < dist_cut]
    ...
    imol += 1
    return gmolecule(molecule_symbols, molecule_positions, equivalent_indices=eq_indices,
      fm_direction=fm_direction)


def xyz_get_rot_list(symbols, positions, tol=1e-05, log=sys.stdout):
    """
...
def mol_get_rot_list0(mol, tol=1e-05, log=sys.stdout):
    ...
    symmops = [o for o in analyzer.symmops if abs(det(o.rotation_matrix) - 1) < 1e-05]
    symmops = generate_full_symmops(symmops, tol)
    rot_list = [o.rotation_matrix for o in symmops]
    return rot_list


def chk_rot_keep(mol, rot, tol=1e-05):
...
def mol_get_rot_list(mol, tol=1e-05, log=sys.stdout):
...
if __name__ == '__main__':
    ...
    cell = [[0.5, 0.5, 0.0], [0.5, 0.0, 0.5], [0.0, 0.5, 0.5]]
    iat = 0
    dist_cut = -1.0
