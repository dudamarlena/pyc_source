...
# [GCC 8.4.0]
# Embedded file name: /home/ykent/GitLab/pygrisb/pygrisb/pygrisb/math/matrix_util.py
# Compiled at: 2019-03-07 11:40:01
# Size of source mod 2**32: 11385 bytes
"""
...
def get_random_hermitian_matrix(n):
    ...
    """
    x = np.random.rand(n, n) + complex(0.0, 1.0) * np.random.rand(n, n)
...
def get_matrix_bkdim_list(a, rtol=1e-12):
    """
    Get the block dimensions of matrix a.
    """
    l = a.shape[0]
    l_list = []
    lbase = 0
    for i in range(l):
        if (np.abs(a[lbase:i + 1, i + 1:]) < rtol).all():
            if (np.abs(a[i + 1:, lbase:i + 1]) < rtol).all() or i == l - 1:
...
def set_eigen_space(vals):
    ...
    for i in range(1, num_vals):
        if abs((vals[i] - vals[(i - 1)]) / max(vals[(i - 1)], 1e-10)) > 0.0001:
            sec_id.append(i)
...
def sym_array(X, R_list):
    ...
    for i, R in enumerate(R_list):
        if isinstance(R, np.ndarray):
            ...
            raise AssertionError('Unsupported data type of {}!'.format(R.__class__))
...
def get_func_matrix_eigh(A, func, alpha=1.0):
    ...
    if A.shape == (1, 1):
        ...
        if not isinstance(A, csr_matrix):
            ...
            res = np.dot(np.dot(vecs, np.diag(np.exp(alpha * vals))), np.conj(vecs).T)
        else:
            if func == 'log':
                _min = np.min(vals)
                if _min < 1e-20:
                    vals += 1e-20 - _min
                res = np.dot(np.dot(vecs, np.diag(np.log(alpha * vals))), np.conj(vecs).T)
            else:
                if func == 'sqrt':
                    _min = np.min(vals)
                    if _min < 1e-20:
                        vals += 1e-20 - _min
                    res = np.dot(np.dot(vecs, np.diag(np.sqrt(alpha * vals))), np.conj(vecs).T)
                else:
                    if func == 'abs':
                        res = np.dot(np.dot(vecs, np.diag(np.abs(alpha * vals))), np.conj(vecs).T)
                    else:
...
def solve_ab_plus_ba_equal_c(b, c):
    ...
    ap = [[cp[(i, j)] / (w[i] + w[j]) for j in range(b.shape[0])] for i in range(b.shape[0])]
