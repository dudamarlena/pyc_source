...
# [GCC 8.4.0]
# Embedded file name: /home/ykent/GitLab/pygrisb/pygrisb/pygrisb/gutz/gatom.py
# Compiled at: 2019-02-24 00:04:27
# Size of source mod 2**32: 18089 bytes
import itertools as it, numpy, h5py, os
from pygrisb.gutz.atoms import Atoms
from pygrisb.basic.units import Ryd_eV
try:
    import spglib
except:
...
class gAtoms(Atoms):
    ...
    def __init__(self, symbols=None, positions=None, numbers=None, masses=None, magmoms=None, scaled_positions=None, cell=None, pbc=None, wiencase=None, locrot_list=None):
        Atoms.__init__(self, symbols=symbols, positions=positions, numbers=numbers,
          masses=masses,
          magmoms=magmoms,
    ...
    def h5set(self):
        from pygrisb.io.h5io import h5auto_read
        with h5py.File('ginit.h5', 'r') as (f):
            dist_cut = h5auto_read(f, '/usrqa/dist_cut',
              default=(-1.0))
            self.set_sym_dist_cut(dist_cut)
            self.unit = h5auto_read(f, '/usrqa/unit',
              default='rydberg')
            spin_polarization = h5auto_read(f, '/usrqa/spin_polarization',
              default='n')
            self.set_ispin(spin_polarization)
            orbital_polarization = h5auto_read(f, '/usrqa/full_orbital_polarization',
              default='n')
            self.set_orbital_polarization(orbital_polarization)
            spin_orbit_coup = h5auto_read(f, '/usrqa/spin_orbit_coup',
              default='n')
            self.set_iso(spin_orbit_coup)
            crystal_field = h5auto_read(f, '/usrqa/crystal_field',
              default='y')
            self.set_crystal_field(crystal_field)
            lhub = h5auto_read(f, '/usrqa/u_matrix_type',
              default=1)
            self.set_lhub(lhub)
            ldc = h5auto_read(f, '/usrqa/ldc',
              default=1)
            self.set_ldc(ldc)
            idx_equivalent_atoms = h5auto_read(f, '/usrqa/idx_equivalent_atoms')
            idx_equivalent_atoms = idx_equivalent_atoms.tolist()
            self.set_idx_equivalent_atoms(idx_equivalent_atoms)
            unique_corr_symbol_list = h5auto_read(f, '/usrqa/unique_corr_symbol_list')
            self.unique_corr_symbol_list = [s.decode() for s in unique_corr_symbol_list]
            self.unique_df_list = h5auto_read(f, '/usrqa/unique_df_list')
            self.unique_df_list = [df.decode() for df in self.unique_df_list]
            ...
            elif self.lhub in (1, 2):
                self.unique_u_list = numpy.asfarray(h5auto_read(f, '/usrqa/unique_u_list_ev'))
                self.unique_j_list = numpy.asfarray(h5auto_read(f, '/usrqa/unique_j_list_ev'))
                if 'ryd' in self.unit:
                    self.unique_u_list /= Ryd_eV
                    self.unique_j_list /= Ryd_eV
            else:
                if self.lhub == 3:
                    self.unique_f_list = numpy.asfarray(h5auto_read(f, '/usrqa/unique_f_list_ev'))
                self.set_nval_range_list()
                self.set_CorrAtoms()
                self.set_na2_list()
                lnewton = h5auto_read(f, '/usrqa/lnewton',
                  default=0)
                self.set_gimix(lnewton)
                iembeddiag = h5auto_read(f, '/usrqa/iembeddiag',
                  default=(-1))
                self.set_giembeddiag(iembeddiag)
                ferromagnetism = h5auto_read(f, '/usrqa/ferromagnetism',
                  default='n')
                if 'y' == ferromagnetism:
                    self.fm_direction = f['/usrqa/unique_magmom_direction_list'][0]
                else:
    ...
    def get_Rotations(self, iat, Nmax=10):
        ...
        from pygrisb.gutz.molecule import xtal_get_local_rot
        return xtal_get_local_rot(symbols, scaled_positions, cell, iat, (self.sym_dist_cut),
          equivalent_indices=equivalent_indices, locrot=locrot,
          fm_direction=(self.fm_direction),
          Nmax=Nmax)

    def get_EquivalentAtoms(self):
        if self.wiencase is not None:
            from pygrisb.dft.wien import get_equivalent_atom_indices
            idx_equivalent_atoms = get_equivalent_atom_indices(self.wiencase)
        else:
            dataset = spglib.get_symmetry_dataset(self, symprec=1e-05)
    ...
    def set_SL_vector_list(self):
        ...
        ly_list = []
        lz_list = []
        from pygrisb.symm.angular_momentum_1p import get_S_vector, get_L_vector
    ...
    def set_SelfEnergy(self):
        ...
        """
        from pygrisb.gutz.self_energy import get_self_energy
        utrans_list = []
        sigma_list = []
        l_list = self.get_llist()
        rotations_list = []
        ...
        if self.modify_mode:
            ...
            for i, l in enumerate(self.corr_list):
                if self.modify_mode:
                    utrans_list.append(f[('/IMPURITY_' + str(i + 1) + '/DB_TO_SAB')][()].T)
                    sigma_list.append(f[('/IMPURITY_' + str(i + 1) + '/SIGMA_STRUCT')][()].T)
                    if 'y' in self.crystal_field:
                        if 'n' in self.orbital_polarization:
                            rotations_list.append(f[('/IMPURITY_' + str(i + 1) + '/rotations')][()])
                            jgenerator_list.append(f[('/IMPURITY_' + str(i + 1) + '/JGENERATOR')][()])
                            ...
                            if 'y' in self.crystal_field and 'n' in self.orbital_polarization:
                                ...
                                rotations = None
                            jgen, utrans, sigma = get_self_energy((l_list[i]), (self.ispin), (self.orbital_polarization),
                              (self.crystal_field),
                              (self.iso), rotations=rotations)
    ...
    def set_LieParameters(self):
        ...
        """
        import pygrisb.symm.atom_symm as atsym
        Lie_Jeven_list = []
        Lie_Jodd_list = []
        if 'y' in self.crystal_field and 'n' in self.orbital_polarization:
            for i, l in enumerate(self.corr_list):
    ...
    def set_one_particle_rotation_list(self):
        ...
        if self.Lie_Jodd_list is not None:
            sp_rotations_list = []
            for J, lies in zip(self.jgenerator_list, self.Lie_Jodd_list):
                if self.iso == 1:
    ...
    def set_v2e_list(self):
        from pygrisb.mbody.coulomb_matrix import get_v2e_list
        l_list = self.get_llist()
        self.v2e_list, self.u_avg_list, self.j_avg_list = get_v2e_list((self.lhub), l_list, (self.imap_list),
          (self.utrans_list), u_list=(self.u_list),
          j_list=(self.j_list),
          f_list=(self.f_list))
...
def h5calc_save_lrot(gatm):
    ...
    if gatm.giembeddiag != -31:
        return
    from pygrisb.mbody.local_operator_factory import get_lrot_op
    ...
    with h5py.File('GLROT.h5', 'w') as (f):
        for i, imap in enumerate(gatm.imap_list):
            ...
            if i > imap:
                f[prefix] = h5py.SoftLink('/IMPURITY_{}'.format(imap + 1))
                continue
            elif i < imap:
                ...
                lvec = [
                 gatm.lx_list[i], gatm.ly_list[i], gatm.lz_list[i]]
                for ival in range(1, nmax):
                    lrots = get_lrot_op(lvec, (gatm.Lie_Jeven_list[i]), l=orb,
                      ival=ival)
                    for ir, lrot in enumerate(lrots):
                        f[prefix + '/valence_block_{}/ROT_{}'.format(ival, ir + 1)] = lrot.T

                f[prefix + '/dim_rot'] = [
                 len(gatm.Lie_Jeven_list[i])]
