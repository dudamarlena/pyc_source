...
# [GCC 8.4.0]
# Embedded file name: /home/ykent/GitLab/pygrisb/pygrisb/pygrisb/math/matrix_basis.py
...
def hermitian_csc_matrix(i, j, n):
    if i == j:
        return csc_matrix(([1.0], [[i], [j]]), shape=(
         n, n),
          dtype=complex)
    x = complex(0.0, 1.0) / np.sqrt(2.0)
    return csc_matrix(([x, -x], [[i, j], [j, i]]), shape=(
     n, n),
      dtype=complex)
...
def hermitian_csc_matrix_basis(n, istart=0):
    ...
    """
    return [hermitian_csc_matrix(i, j, n) for i in range(istart, n) for j in range(i, n)]