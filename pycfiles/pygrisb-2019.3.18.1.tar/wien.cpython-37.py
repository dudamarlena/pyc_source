...
# [GCC 8.4.0]
# Embedded file name: /home/ykent/GitLab/pygrisb/pygrisb/pygrisb/dft/wien.py
# Compiled at: 2019-02-22 23:25:00
# Size of source mod 2**32: 15390 bytes
from __future__ import print_function
import numpy as np, sys, os, glob, subprocess, shutil, fileinput, h5py
    ...
    with open(str(case_fname), 'r') as (f):
        ...
        for i in range(0, nops * 4, 4):
            ...
            for j in range(3):
                ...
                if abs(float(line[6:17])) > 1e-06:
                    lok = False
                    break

            if lok and abs(np.linalg.det(rot) - 1) < 1e-06:
                rot_list.append(rot)
        ...
        with open(case + '.struct', 'w') as (fd):
            for i, line in enumerate(fs.readlines()):
                if i == 3:
                    ...
                    a_list *= vfrac ** 0.3333333333333333
                    line = ('{:10.6f}{:10.6f}{:10.6f}'.format)(*a_list) + line[30:]
                fd.write(line)
...
def update_case_ref(vfrac_min):
    ...
    cwd = os.getcwd() + '/'
    os.chdir('./template')
    create_struct('../Ref_Vmin/case', (1.0 / vfrac_min), case='case')
    os.chdir(cwd)
...
def create_dir_list(vfrac_min=0.7, vfrac_max=1.3, vfrac_step=0.05, dir_template='./template'):
    ...
    vfrac_list = np.arange(vfrac_min, vfrac_max + 1e-05, vfrac_step)
...
def batch_init_lapw(args=[
 '-vxc', '5', '-rkmax', '8.5', '-numk', '5000']):
...
def h2get_energy_volume_list(path='lapw'):
    ...
    with h5py.File('results.h5', 'a') as (f):
        if 'vol_list' not in f:
            f['/vol_list'] = v_list
        epath = '/' + path + '/etot_list'
        if epath in f:
            del f[epath]
        f[epath] = e_list
    splot.xy_plot(v_list, e_list, xlabel='V ($\\AA^{3}$/primitive cell)', ylabel='E (eV/primitive cell)',
      fsave=('ev_{}.pdf'.format(path)))
...
def compare_ev_plot(path_list, fres='results.h5'):
    ...
    with h5py.File(fres, 'r') as (f):
        ...
        if v_list != []:
            splot.xy2_plot(v_list, e_list, pattern_list, label_list, xlabel='V ($\\AA^{3}$/primitive cell)',
...
def compare_pv_plot(path_list, fres='results.h5'):
    ...
    with h5py.File(fres, 'r') as (f):
        ...
        if v_list != []:
            splot.xy2_plot(v_list, p_list, pattern_list, label_list, xlabel='V ($\\AA^{3}$/primitive cell)',
              ylabel='P (GPa)',
...
def batch_save_lapw(sdir='lapw', args=['-f']):
    ...
    """
    cmd = [
     os.environ['WIENROOT'] + '/save_lapw', '-d'] + [sdir] + args
    cwd = os.getcwd() + '/'
    for dname in get_dirvlist():
        os.chdir(dname + '/case')
...
def run_lapw(args=[
 '-i', '70'], nproc=1):
    ...
    """
    cmd = [
     os.environ['WIENROOT'] + '/run_lapw'] + args
    cwd = os.getcwd() + '/'
    if '-p' in sys.argv:
...
def steps(vfrac_min=0.7, vfrac_max=1.3, vfrac_step=0.05, dir_template='./template'):
    ...
    elif 'Vmin' in sys.argv[1]:
        create_dir1(vfrac_min, 'Ref_Vmin', dir_template=dir_template)
        print('Please goto Ref_Vmin/case and finish manual test.')
    else:
        if 'update_case_ref' == sys.argv[1]:
            update_case_ref(vfrac_min)
        else:
            if 'Vlist' == sys.argv[1]:
                create_dir_list(vfrac_min=vfrac_min, vfrac_max=vfrac_max, vfrac_step=vfrac_step,
                  dir_template=dir_template)
            else:
                if 'batch_init_lapw' == sys.argv[1]:
                    batch_init_lapw()
                else:
                    if 'batch_run_lapw' == sys.argv[1]:
                        run_lapw()
                    else:
                        if 'batch_run_lapwso' == sys.argv[1]:
                            run_lapw(args=['-so', '-i', '70'])
                        else:
                            if 'batch_save' in sys.argv[1]:
                                batch_save_lapw(sdir=(sys.argv[1].split('_')[2]))
                            else:
                                if 'batch_gsave' in sys.argv[1]:
                                    gwien.batch_gsave(sdir=(sys.argv[1].split('_')[2]))
                                else:
                                    if '-cp-dir' == sys.argv[1]:
                                        batch_copy_dir(sys.argv[2], sys.argv[3])
                                    else:
                                        if '-ev' == sys.argv[1]:
                                            h2get_energy_volume_list(path=(sys.argv[2]))
                                        else:
                                            if 'batch_initso_lapw' == sys.argv[1]:
                                                batch_initso_lapw()
                                            else:
                                                if '-pv' == sys.argv[1]:
