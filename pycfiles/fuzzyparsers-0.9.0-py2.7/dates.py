...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/fuzzyparsers/dates.py
...
def locale_format():
    ...
    if has_locale:
        ...
        x = d.strftime('%x')
        dfmt = x.replace('2022', '%Y').replace('22', '%y').replace('09', '%d').replace('07', '%m').replace('9', '%d').replace('7', '%m')
    return dfmt
...
class DateParser:
    ...
    def str_to_date_int(self, s):
        ...
        if m:
            ...
            if m:
                return (
                 int(m.group(2)), str_to_month(m.group(1)), 1)
