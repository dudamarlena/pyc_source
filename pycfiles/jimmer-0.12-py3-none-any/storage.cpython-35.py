...
# [GCC 8.4.0]
# Embedded file name: C:\Users\ealesid\PycharmProjects\geekbrains_python\jimmer\jimmer\server\storage.py
# Compiled at: 2017-11-22 11:33:51
...
class ServerStore(Store):
    ...
    def add_contact(self, client, contact):
        if not self.session.query(ContactsList).filter(ContactsList.clientName == client.clientName, ContactsList.contactName == contact.clientName).first():
            self.session.add(ContactsList(client.clientName, contact.clientName))
            return True

    def del_contact(self, client, contact):
        _query = self.session.query(ContactsList).filter(ContactsList.clientName == client.clientName, ContactsList.contactName == contact.clientName).first()
        if _query:
