...
# [GCC 8.4.0]
# Embedded file name: C:\Users\ealesid\PycharmProjects\geekbrains_python\jimmer\jimmer\client\controller.py
# Compiled at: 2017-12-14 01:09:16
...
class ClientGuiController:
    ...
    def __init__(self, client):
        self.actions_dict = {ACT_CONTACT_LIST: self.add_contact_item, 
    ...
    def get_connected(self):
        try:
            ...
            self.client_view.statusMessage.showMessage('Server {}:{} is unavailable'.format(self.client.server_address, self.client.server_port))
    ...
    def add_contact(self):
        ...
        if _name and not self.client_view.lstContacts.findItems(_name, Qt.MatchExactly) and not is_chat(_name):
            ...
            if _name and not self.client_view.lstContacts.findItems(_name, Qt.MatchExactly) and is_chat(_name):
                self.chat_clicked()
            elif _name:
    ...
    def get_outcoming(self):
        if self.client.outcoming_msg:
            ...
            if _contact:
                self.save_message(_contact, self.client.outcoming_msg.get(FIELD_MESSAGE), self.client.outcoming_msg.get(FIELD_TIME))
    ...
    def chat_clicked(self):
        ...
        if _chat and not is_chat(_chat):
            ...
            if _chat and _chat_action == ACT_JOIN:
                self.client_view.btnChat.setText(ACT_LEAVE.capitalize())
                self.start_chat()
    ...
    def update_chat(self, *args):
        ...
        if _chat:
            ...
            if isinstance(_message, str):
                ...
                _chat.txtChatMessages.insertHtml('<b>{}:</b> {}'.format(_message.get(FIELD_SENDER), _message.get(FIELD_MESSAGE)))
                _chat.txtChatMessages.insertHtml('<br />')
        else:
            self.client_view.lstContacts.findItems(_message.get(FIELD_SENDER), Qt.MatchExactly)[0].setBackground(Qt.cyan)
    ...
    def set_text_format(button):
        ...
        def text_format():
            ...
            if button is chat.bold and chat.txtNewMessage.fontWeight() == QFont.Normal:
                ...
                if button is chat.bold and chat.txtNewMessage.fontWeight() == QFont.Black:
                    chat.txtNewMessage.setFontWeight(QFont.Normal)
                else:
                    if button is chat.italic:
