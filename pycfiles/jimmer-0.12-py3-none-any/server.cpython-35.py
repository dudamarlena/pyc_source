...
# [GCC 8.4.0]
# Embedded file name: C:\Users\ealesid\PycharmProjects\geekbrains_python\jimmer\jimmer\server\server.py
# Compiled at: 2017-12-05 07:10:28
...
class Server:
    ...
    def __init__(self, host=DEFAULT_ADDRESS, port=DEFAULT_PORT, log='server', loop=None):
        ...
        logging.basicConfig(format='%(asctime)s %(levelname)s %(name)s %(message)-s', level=logging.DEBUG, handlers=[
         TimedRotatingFileHandler(self.log, 'midnight')])
        self.log_messages = {self.handle.__name__: {MSG_RECEIVE: {SUCCESSFUL_MSG_KEY: SERVER_CLIENT_RECV_SUCC_MSG, 
                                              UNSUCCESSFUL_MSG_KEY: SERVER_CLIENT_RECV_UNSUCC_MSG}, 
                                
                                MSG_SEND: {SUCCESSFUL_MSG_KEY: SERVER_CLIENT_SEND_SUCC_MSG, 
    ...
    def handle(self, reader, writer):
        ...
        while not reader.at_eof():
            ...
            self.logger.debug(self.log_messages.get(self.handle.__name__).get(MSG_RECEIVE).get(SUCCESSFUL_MSG_KEY).format(self.handle.__name__, *[peer, data]))
            if REQUIRED_JIM_CLIENT_FIELDS.issubset(data.keys()):
                for response in ServerResponse(data, peer, socket).results:
                    ...
                    self.logger.debug(self.log_messages.get(self.handle.__name__).get(MSG_SEND).get(SUCCESSFUL_MSG_KEY).format(self.handle.__name__, *[peer, response.as_dict]))

            else:
                writer.write(json.dumps(JIMResponseMessage({'response': 400, 'error': 'Wrong message format.'}).as_dict))
...
class ServerResponse:
    ...
    def get_contacts(*args):
        ...
        if _contacts:
            _response = [
             JIMResponseMessage({FIELD_RESPONSE: CODE_ACCEPTED, FIELD_QUANTITY: len(_contacts)})]
    ...
    def add_contact(*args):
        ...
        with ServerStore() as (store):
            ...
            if _client and _contact:
                ...
                return [
                 JIMResponseMessage({'response': 400, 'error': 'User not found.'})]
    ...
    action_response = {ACT_PRESENCE: get_presence, 
     ACT_QUIT: quit, 
