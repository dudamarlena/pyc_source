...
# [GCC 8.4.0]
# Embedded file name: C:\Users\ealesid\PycharmProjects\geekbrains_python\jimmer\jimmer\protocol\protocol.py
# Compiled at: 2017-11-21 01:09:47
...
FORMAT_QUANTITY = {FIELD_NAME: FIELD_QUANTITY, FIELD_VALUE: 0, FIELD_TYPE: int, FIELD_LENGTH: 3}
FORMAT_RECEIVER = {FIELD_NAME: FIELD_RECEIVER, FIELD_VALUE: '', FIELD_TYPE: str, FIELD_LENGTH: 25}
FORMAT_RESPONSE = {FIELD_NAME: FIELD_RESPONSE, FIELD_VALUE: 0, FIELD_TYPE: int, FIELD_LENGTH: 3}
...
class ProtocolDescriptor:
    ...
    def __set__(self, instance, value):
        if not isinstance(value, self.type):
            raise TypeError("{}{} should be '{}' type.".format(instance.__class__.__name__, self.name, self.type.__name__))
        try:
        ...
        if value_length > self.max_length:
            raise ValueError('Length of {}{}={} should not exceed {} symbols.'.format(instance.__class__.__name__, self.name, value, self.max_length))
...
class JIMActionMessage:
    ...
    user_fields = [
     FIELD_ACCOUNT_NAME, FIELD_PASSWORD, FIELD_STATUS]
    __slots__ = {
    ...
    def presence(user, msgtype='status'):
        user = {key:value for key, value in user.items() if key in JIMActionMessage.user_fields if key in JIMActionMessage.user_fields}
