...
# Embedded file name: build/bdist.linux-x86_64/egg/aomi/filez.py
# Compiled at: 2017-11-17 12:53:02
# Size of source mod 2**32: 5322 bytes
__doc__ = 'Handle various kinds of import/export of secrets'
from __future__ import print_function
import os, shutil, time, logging, datetime, zipfile
from cryptorito import key_from_keybase, has_gpg_key, import_gpg_key, encrypt, decrypt
from aomi.helpers import subdir_path, ensure_dir, ensure_tmpdir
from aomi.template import get_secretfile
from aomi.validation import gpg_fingerprint as validate_gpg_fingerprint
...
def freeze_archive(tmp_dir, dest_prefix):
    ...
    for root, _dirnames, filenames in os.walk(dest_prefix):
        for filename in filenames:
            ...
            archive.write('%s/%s' % (root, filename), '%s/%s' % (relative_path, filename))
...
def thaw_decrypt(vault_client, src_file, tmp_dir, opt):
    ...
    elif not decrypt(src_file, zip_file):
        raise aomi.exceptions.GPG('Unable to gpg')
    return zip_file
