...
# Embedded file name: build/bdist.linux-x86_64/egg/aomi/model/context.py
# Compiled at: 2017-11-17 12:53:02
# Size of source mod 2**32: 14255 bytes
__doc__ = 'A Context contains a set of Vault resources which may\nor may not end up written to the HCV instance. This\ncontext may be filtered, or pre/post processed.'
...
def py_resources():
    ...
    aomi_mods = [m for m, _v in iteritems(sys.modules) if m.startswith('aomi.model')]
    mod_list = []
    mod_map = []
    for amod in [sys.modules[m] for m in aomi_mods]:
        for _mod_bit, model in inspect.getmembers(amod):
            if str(model) in mod_list:
                ...
                elif inspect.isclass(model) and issubclass(model, Resource) and model.config_key:
                    mod_list.append(str(model))
...
def resource_sort(seed_key):
    ...
    if seed_key == 'mounts' or seed_key == 'userpass' or seed_key == 'audit_logs':
        return False
    return True
...
class Context(object):
    ...
    def load(config, opt):
        ...
        for config_key in config.keys():
            if config_key != 'pgp_keys' and config_key not in seed_keys:
    ...
    def add(self, resource):
        ...
        if isinstance(resource, Resource):
            if isinstance(resource, Secret) and resource.mount != 'cubbyhole':
                ensure_backend(resource, SecretBackend, self._mounts, self.opt, False)
            else:
                if isinstance(resource, Mount):
                    ensure_backend(resource, SecretBackend, self._mounts, self.opt)
                else:
                    if isinstance(resource, Auth):
            ...
            msg = 'Unknown resource %s being added to context' % resource.__class__
    ...
    def sync_policies(self, vault_client):
        ...
        p_resources = [x for x in self.resources() if isinstance(x, Policy)]
        for resource in p_resources:
            resource.sync(vault_client)

        return [x for x in self.resources() if not isinstance(x, Policy)]
    ...
    def sync_auth(self, vault_client, resources):
        ...
        auth_resources = [x for x in resources if isinstance(x, (LDAP, UserPass))]
    ...
    def sync_mounts(self, active_mounts, resources, vault_client):
        ...
        for resource in s_resources:
            active_mounts = self.actually_mount(vault_client, resource, active_mounts)

        for resource in [x for x in resources if isinstance(x, Secret)]:
            n_mounts = self.actually_mount(vault_client, resource, active_mounts)
            if len(n_mounts) != len(active_mounts):
                LOG.warning('Ad-Hoc mount with %s. Please specify explicit mountpoints.', resource)
            active_mounts = n_mounts

        return (
         active_mounts,
         [x for x in resources if not isinstance(x, Mount)])
    ...
    def prune(self, vault_client):
        ...
        for mount_name, _values in existing:
            ...
            if not mount_path.startswith('sys'):
                if mount_path == 'cubbyhole':
                    pass
                else:
    ...
    def fetch(self, vault_client):
        ...
        for rsc in self.resources():
            if issubclass(type(rsc), Secret):
                nc_exists = rsc.mount != 'cubbyhole' and find_backend(rsc.mount, self._mounts).existing
                if nc_exists or rsc.mount == 'cubbyhole':
                    rsc.fetch(vault_client)
                else:
                    if issubclass(type(rsc), Auth):
                        if find_backend(rsc.mount, self._auths).existing:
                            rsc.fetch(vault_client)
                        else:
                            if issubclass(type(rsc), Mount):
                                rsc.existing = find_backend(rsc.mount, self._mounts).existing
                            else:
                                rsc.fetch(vault_client)
