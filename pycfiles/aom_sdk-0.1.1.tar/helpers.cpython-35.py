...
# Embedded file name: build/bdist.linux-x86_64/egg/aomi/helpers.py
# Compiled at: 2017-11-17 12:53:02
# Size of source mod 2**32: 8255 bytes
__doc__ = ' Helpers for aomi that are used throughout the application '
from __future__ import print_function
import sys, os, atexit, tempfile, collections
...
def dict_unicodeize(some_dict):
    ...
    by unicode strings"""
    if isinstance(some_dict, (''.__class__, ''.__class__)):
        if sys.version_info >= (3, 0):
            return some_dict
        return some_dict.decode('utf-8')
    if isinstance(some_dict, collections.Mapping):
        return dict(map(dict_unicodeize, iteritems(some_dict)))
    if isinstance(some_dict, collections.Iterable):
...
def diff_dict(dict1, dict2, ignore_missing=False):
    ...
    if not ignore_missing and len(unidict1) != len(unidict2) or ignore_missing and len(unidict1) >= len(unidict2):
        return True
