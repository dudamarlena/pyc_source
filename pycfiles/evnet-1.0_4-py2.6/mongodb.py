...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/evnet/mongodb.py
# Compiled at: 2011-04-02 10:36:23
import sys, logging, random, struct
logging.basicConfig(level=logging.DEBUG)
from . import PlainClientConnection, loop, unloop, EventGen
from .promise import Promise
import bson
__binzero = '\x00\x00\x00\x00'

def mongo_addhdr(op, data):
    reqid = random.randint(-2147483649, 2147483648)
...
def mongo_gen_query(options, collection, skip, num, query, fields=None):
    return mongo_addhdr(2004, struct.pack('<I', options) + bson._make_c_string(collection) + struct.pack('<ii', skip, num) + bson.BSON.encode(query) + (fields and bson.BSON.encode(fields) or ''))


def mongo_gen_more(collection, num, cursorid):
    return mongo_addhdr(2005, __binzero + bson._make_c_string(collection) + struct.pack('<iq', num, cursorid))


def mongo_gen_insert(collection, docs, check_keys=True):
    return mongo_addhdr(2002, __binzero + bson._make_c_string(collection) + ('').join([ bson.BSON.encode(doc, check_keys) for doc in docs ]))[1]


def mongo_gen_delete(collection, spec):
    return mongo_addhdr(2006, __binzero + bson._make_c_string(collection) + __binzero + bson.BSON.encode(spec))[1]


def mongo_gen_update(collection, spec, doc, upsert=False, multi=False):
    return mongo_addhdr(2001, __binzero + bson._make_c_string(collection) + struct.pack('<i', (upsert and 1 or 0) | (multi and 2 or 0)) + bson.BSON.encode(spec) + bson.BSON.encode(doc))[1]
...
class MongoUnpack(object):
    ...
    def unpack(self):
        ...
        docs = bson.decode_all(str(buffer(self.buf, 36, ml - 16 - 20)))
        del self.buf[:ml]
...
if __name__ == '__main__':
    ...
    def onready():
        ...
        p._except(dbgprint)
        if False:
            a.delete('dashboard.containers', {'content': 'content0'})
            p = a.command('admin', 'getlasterror')
            p._when(dbgprint)
            a.insert('dashboard.containers', [{'content': 'fooinsert'}])
            p = a.command('admin', 'getlasterror')
            p._when(dbgprint)
            a.update('dashboard.containers', {'content': 'fooinsert'}, {'$set': {'title': 'larl'}})
            p = a.command('admin', 'getlasterror')
            p._when(dbgprint)
