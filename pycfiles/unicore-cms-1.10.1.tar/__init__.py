...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/universalcore/unicore-cms/cms/__init__.py
...
LANGUAGE_FALLBACKS = {'swh': 'swa'}
COUNTRY_FALLBACKS = {'UK': 'GB'}
...
def get_locale_with_fallbacks(locale_name):
    if locale_name is None:
        ...
        if lang != language_code:
            log.warning('Invalid language_code used: %s' % language_code, extra={'stack': True})
        if country != country_code:
            log.warning('Invalid country_code used: %s' % country_code, extra={'stack': True})
...
def init_auth(config):
    ...
    def user(request):
        if request.authenticated_userid:
            return User(request.registry.hubclient, request.session[USER_DATA_SESSION_KEY])
        else:
            return
