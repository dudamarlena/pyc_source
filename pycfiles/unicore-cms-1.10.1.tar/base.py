...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/universalcore/unicore-cms/cms/views/base.py
...
class BaseCmsView(object):
    ...
    def __init__(self, request):
        ...
        self.results_per_page = self.parse_setting('results_per_page', 10, int)
        ...
        self.workspace = workspace_init(repo_url, es=self.es_settings, index_prefix=self.settings['es.index_prefix'])
        if is_remote:
    ...
    def get_image_url(self, image_host, image_uuid, width=None, height=None):
        ...
        if not (width or height):
            ...
            image_url = crypto.generate(width=width, height=height, image_url=image_uuid)
        elif width:
            image_url = crypto.generate(width=width, height=0, image_url=image_uuid)
        else:
            image_url = crypto.generate(width=0, height=height, image_url=image_uuid)
