...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/universalcore/unicore-cms/cms/views/forms.py
# Compiled at: 2016-05-07 04:51:23
import time
from datetime import datetime
import pytz, colander
from deform import Form, widget, ValidationFailure
from unicore.content.models import Page
from utils import translation_string_factory as _
COMMENT_MAX_LENGTH = 3000
COMMENT_STALE_AFTER = 7200
COMMENT_CONTENT_TYPES = {Page: 'page'}
...
class CSRFSchema(colander.MappingSchema):
    ...
    """
    csrf = colander.SchemaNode(colander.String(), default=deferred_csrf_default, validator=deferred_csrf_validator, widget=widget.HiddenWidget())


class CommentSchema(CSRFSchema):
    comment = colander.SchemaNode(colander.String(), validator=colander.Length(max=COMMENT_MAX_LENGTH), widget=widget.TextAreaWidget(), title=_('Comment'))
    content_uuid = colander.SchemaNode(colander.String(), default=deferred_content_uuid_default, validator=deferred_content_uuid_validator, widget=widget.HiddenWidget())
    content_type = colander.SchemaNode(colander.String(), default=deferred_content_type_default, validator=deferred_content_type_validator, widget=widget.HiddenWidget())
    timestamp = colander.SchemaNode(colander.Integer(), default=deferred_timestamp_default, validator=validate_comment_timestamp, widget=widget.HiddenWidget())


class CommentForm(Form):

    def __init__(self, request, content_object):
        self.schema = CommentSchema().bind(request=request, content_object=content_object)
    ...
    def validate(self, cstruct):
        ...
        try:
            ...
            security_fields = {'content_type', 'content_uuid', 'timestamp', 'csrf'}
        ...
        data = {'app_uuid': c_client.settings['app_id'], 
           'content_uuid': appstruct['content_uuid'], 
           'user_uuid': user.get('uuid'), 
           'comment': appstruct['comment'], 
           'user_name': user.get('app_data').get('display_name', _('Anonymous')), 
           ...
           'content_url': self.request.route_url('content', uuid=self.content_object.uuid), 
