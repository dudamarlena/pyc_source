...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/fdasrsf/time_warping.py
# Compiled at: 2020-03-04 10:13:44
# Size of source mod 2**32: 44672 bytes
"""
...
import fpls_warp as fpls, cbayesian as bay, collections
...
class fdawarp:
    ...
    def srsf_align(self, method='mean', omethod='DP', smoothdata=False, parallel=False, lam=0.0):
        ...
        if M > 500:
            ...
            if N > 100:
                ...
                mf = f[:, min_ind]
                if parallel:
                    out = Parallel(n_jobs=(-1))((delayed(uf.optimum_reparam)(mq, self.time, q[:, n], omethod, lam, mf[0], f[(0, n)]) for n in range(N)))
                    gam = np.array(out)
                    gam = gam.transpose()
                else:
                    gam = np.zeros((M, N))
                ...
                for r in range(0, MaxItr):
                    ...
                    if r == MaxItr - 1:
                        ...
                        out = Parallel(n_jobs=(-1))((delayed(uf.optimum_reparam)(mq[:, r], self.time, q[:, n, 0], omethod, lam, mf[(0, r)], f[(0, n, 0)]) for n in range(N)))
                        gam = np.array(out)
                        gam = gam.transpose()
                    else:
                        for k in range(0, N):
                            gam[:, k] = uf.optimum_reparam(mq[:, r], self.time, q[:, k, 0], omethod, lam, mf[(0, r)], f[(0, k, 0)])

                    gam_dev = np.zeros((M, N))
                    vtil = np.zeros((M, N))
                    dtil = np.zeros(N)
                    for k in range(0, N):
                        f[:, k, r + 1] = np.interp((self.time[(-1)] - self.time[0]) * gam[:, k] + self.time[0], self.time, f[:, k, 0])
    ...
    def plot(self):
        ...
        fig, ax = plot.f_plot((np.arange(0, M) / float(M - 1)), (self.gam), title='Warping Functions')
        ax.set_aspect('equal')
    ...
    def gauss_model(self, n=1, sort_samples=False):
        ...
        fs = np.zeros((M, n))
        for k in range(0, n):
            fs[:, k] = uf.cumtrapzmid(time, q_s[0:M, k] * np.abs(q_s[0:M, k]), np.sign(q_s[(M, k)]) * q_s[(M, k)] ** 2, mididx)
    ...
    def multiple_align_functions(self, mu, omethod='DP', smoothdata=False, parallel=False, lam=0.0):
        ...
        if M > 500:
            ...
            if N > 100:
                ...
                if parallel:
                    out = Parallel(n_jobs=(-1))((delayed(uf.optimum_reparam)(mq, self.time, q[:, n], omethod, lam, mu[0], f[(0, n)]) for n in range(N)))
                    gam = np.array(out)
                    gam = gam.transpose()
                else:
                    gam = np.zeros((M, N))
            ...
            qn = np.zeros((M, N))
            for k in range(0, N):
                fn[:, k] = np.interp((self.time[(-1)] - self.time[0]) * gam[:, k] + self.time[0], self.time, f[:, k])
...
def pairwise_align_bayes(f1i, f2i, time, mcmcopts=None):
    ...
    if mcmcopts is None:
        ...
        mcmcopts = {'iter':20000,  'burnin':np.minimum(5000, 10000),  'alpha0':0.1,  'beta0':0.1, 
         'zpcn':tmp,  'propvar':1,  'initcoef':np.repeat(0, 20), 
         'npoints':200,  'extrainfo':True}
    ...
    for m in tqdm(range(1, iter)):
        ...
        newscale = 0.5 * SSE_curr + mcmcopts['beta0']
...
def align_fPCA(f, time, num_comp=3, showplot=True, smoothdata=False):
    ...
    if showplot:
        ...
        while itr < MaxItr:
            ...
            if itr == MaxItr:
                ...
                qhat = d1 + tmp
                if parallel:
                    out = Parallel(n_jobs=(-1))((delayed(uf.optimum_reparam)(qhat[:, n], time, qi[:, n, itr], 'DP', lam) for n in range(N)))
                    gam_t = np.array(out)
    ...
    align_fPCAresults = collections.namedtuple('align_fPCA', ['fn', 'qn',
     ...
     'U', 'orig_var', 'amp_var',
     'phase_var', 'cost'])
    out = align_fPCAresults(fn, qn, q0, mqn, gamf, q_pca, f_pca, s, c, U, orig_var, amp_var, phase_var, cost)
    return out
...
def align_fPLS(f, g, time, comps=3, showplot=True, smoothdata=False, delta=0.01, max_itr=100):
    ...
    while itr <= max_itr:
        ...
        gam[:, :, itr + 1] = fpls.fpls_warp(time, gamtmp, qftmp, qgtmp, wqftmp,
          wqgtmp, display=0, delta=delta,
          tol=1e-06,
          max_iter=4000)
        for k in range(0, N):
            gam_k = gam[:, k, itr + 1]
        ...
        print('Iteration: %d - Diff Value: %f - %f' % (itr + 1, wqf_diff[itr],
         cost[itr]))
        if wqf_diff[itr] < 0.1 or abs(cost_diff) < 0.001:
            break
        itr += 1
