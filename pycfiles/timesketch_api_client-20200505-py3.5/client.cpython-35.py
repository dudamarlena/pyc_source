...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/timesketch_api_client/client.py
# Compiled at: 2020-04-20 09:12:39
# Size of source mod 2**32: 16749 bytes
"""Timesketch API client."""
from __future__ import unicode_literals
import os, json, logging, uuid, bs4, requests
from requests.exceptions import ConnectionError
import webbrowser
from google_auth_oauthlib import flow as googleauth_flow
import google.auth.transport.requests, pandas
...
class TimesketchApi(object):
    ...
    def __init__(self, host_uri, username, password='', verify=True, client_id='', client_secret='', auth_mode='timesketch'):
        ...
        self._flow = None
        try:
            self.session = self._create_session(username, password, verify=verify, client_id=client_id, client_secret=client_secret, auth_mode=auth_mode)
        except ConnectionError:
            raise ConnectionError('Timesketch server unreachable')
        except RuntimeError as e:
            raise RuntimeError('Unable to connect to server, with error: {0!s}'.format(e))
    ...
    def _create_oauth_session(self, client_id='', client_secret='', client_secrets_file=None, run_server=True, skip_open=False):
        ...
        if client_secrets_file:
            if not os.path.isfile(client_secrets_file):
                raise RuntimeError('Unable to log in, client secret files does not exist.')
            flow = googleauth_flow.InstalledAppFlow.from_client_secrets_file(client_secrets_file, scopes=self.DEFAULT_OAUTH_SCOPE, autogenerate_code_verifier=True)
        else:
            ...
            flow = googleauth_flow.InstalledAppFlow.from_client_config(client_config, self.DEFAULT_OAUTH_SCOPE, autogenerate_code_verifier=True)
        ...
        if run_server:
            ...
            if skip_open:
                ...
                if open_browser.lower() == 'y' or open_browser.lower() == 'yes':
                    ...
                    print('Need to manually visit URL to authenticate: {0:s}'.format(auth_url))
                code = input('Enter the token code: ')
                _ = flow.fetch_token(code=code)
        session = flow.authorized_session()
        self._flow = flow
        self._credentials = flow.credentials
        login_callback_url = '{0:s}{1:s}'.format(self._host_uri, self.DEFAULT_OAUTH_API_CALLBACK)
        params = {'id_token': session.credentials.id_token}
        response = session.get(login_callback_url, params=params)
        if response.status_code not in definitions.HTTP_STATUS_CODE_20X:
            error.error_message(response, message='Unable to authenticate', error=RuntimeError)
        self._set_csrf_token(session)
        return session

    def _create_session(self, username, password, verify, client_id, client_secret, auth_mode):
        ...
        if auth_mode == 'oauth_local':
            return self._create_oauth_session(client_id=client_id, client_secret=client_secret, run_server=False, skip_open=True)
    ...
    def fetch_resource_data(self, resource_uri):
        ...
        try:
            ...
            logger.error('Error fetching resources: [{0:d}] {1!s} {2!s}, error: {3!s}'.format(response.status_code, response.reason, response.text, e))
    ...
    def get_oauth_token_status(self):
        ...
        return {'expired': self._credentials.expired, 
    ...
    def get_aggregator_info(self, name='', as_pandas=False):
        ...
        for line in response_json:
            line_dict = {'name': line.get('name', 'N/A'), 
             'description': line.get('description', 'N/A')}
            for field_index, field in enumerate(line.get('fields', [])):
                line_dict['field_{0:d}_name'.format(field_index + 1)] = field.get('name')
                line_dict['field_{0:d}_description'.format(field_index + 1)] = field.get('description')
    ...
    def list_sketches(self):
        ...
        for sketch_dict in response['objects']:
            ...
            sketch_obj = sketch.Sketch(sketch_id=sketch_id, api=self, sketch_name=sketch_name)
    ...
    def get_or_create_searchindex(self, searchindex_name, es_index_name=None, public=False):
        ...
        form_data = {'searchindex_name': searchindex_name, 
         'es_index_name': es_index_name, 
         'public': public}
        response = self.session.post(resource_url, json=form_data)
        if response.status_code not in definitions.HTTP_STATUS_CODE_20X:
            error.error_message(response, message='Error creating searchindex', error=RuntimeError)
        response_dict = response.json()
    ...
    def list_searchindices(self):
        ...
        for index_dict in response['objects'][0]:
            ...
            index_obj = index.SearchIndex(searchindex_id=index_id, api=self, searchindex_name=index_name)
