...
class Aggregation(resource.BaseResource):
    ...
    def save(self):
        ...
        if self.id:
            resource_url = '{0:s}/sketches/{1:d}/aggregation/{2:d}/'.format(self.api.api_root, self._sketch.id, self.id)
        else:
            resource_url = '{0:s}/sketches/{1:d}/aggregation/'.format(self.api.api_root, self._sketch.id)
        response = self.api.session.post(resource_url, json=data)
        return response.status_code in definitions.HTTP_STATUS_CODE_20X


class AggregationGroup(resource.BaseResource):
    __doc__ = 'Aggregation Group object.\n\n    Attributes:\n        id: the ID of the group.\n    '
    ...
    def __str__(self):
        ...
        return '[{0:d}] {1:s} - {2:s}'.format(self.id, self._name, self._description)
    ...
    def delete(self):
        ...
        if not self.id:
            return False
        response = self.api.session.delete('{0:s}/{1:s}'.format(self.api.api_root, self.resource_uri))
        return response.status_code in definitions.HTTP_STATUS_CODE_20X
    ...
    def from_dict(self, group_dict):
        ...
        self.id = group_id
        self.resource_uri = 'sketches/{0:d}/aggregation/group/{1:d}/'.format(self._sketch.id, group_id)
    ...
    def save(self):
        ...
        if self.id:
            res_url = '{0:s}/sketches/{1:d}/aggregation/group/{2:d}/'.format(self.api.api_root, self._sketch.id, self.id)
        else:
            res_url = '{0:s}/sketches/{1:d}/aggregation/group/'.format(self.api.api_root, self._sketch.id)
