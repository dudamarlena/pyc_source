...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/timesketch_api_client/sketch.py
# Compiled at: 2020-05-05 06:35:25
# Size of source mod 2**32: 34445 bytes
...
class Sketch(resource.BaseResource):
    ...
    def _build_pandas_dataframe(self, search_response, return_fields=None):
        ...
        if 'datetime' in data_frame:
            ...
            try:
                data_frame['datetime'] = pandas.to_datetime(data_frame.timestamp / 1000000.0, utc=True, unit='s')
            except pandas.errors.OutOfBoundsDatetime:
                pass
    ...
    def create_story(self, title):
        ...
        Returns:
            ...
            created story.
        """
        resource_url = '{0:s}/sketches/{1:d}/stories/'.format(self.api.api_root, self.id)
        data = {'title': title, 
         'content': ''}
    ...
    def add_to_acl(self, user_list=None, group_list=None, make_public=False):
        ...
        if not user_list and not group_list:
            return True
        resource_url = '{0:s}/sketches/{1:d}/collaborators/'.format(self.api.api_root, self.id)
        data = {}
        if group_list:
    ...
    def list_aggregation_groups(self):
        ...
        """
        groups = []
        resource_url = '{0:s}/sketches/{1:d}/aggregation/group/'.format(self.api.api_root, self.id)
        ...
        for group_dict in data.get('objects', []):
            if not group_dict.get('id'):
                ...
                group = aggregation.AggregationGroup(sketch=self, api=self.api)
    ...
    def list_aggregations(self, include_labels=None, exclude_labels=None):
        ...
        if aggregation_groups:
            ...
            groups = [x.get('id', 0) for x in aggregation_groups.get('aggregations', [])]
        else:
            groups = tuple()
    ...
    def get_analyzer_status(self):
        ...
        stats_list = []
        for timeline_obj in self.list_timelines():
            resource_uri = '{0:s}/sketches/{1:d}/timelines/{2:d}/analysis'.format(self.api.api_root, self.id, timeline_obj.id)
    ...
    def list_stories(self):
        ...
        for story_dict in stories:
            story_list.append(story.Story(story_id=story_dict.get('id', -1), sketch=self, api=self.api))
    ...
    def list_views(self):
        ...
        for view in sketch['meta']['views']:
            view_obj = view_lib.View(view_id=view['id'], view_name=view['name'], sketch_id=self.id, api=self.api)
    ...
    def list_timelines(self):
        ...
        for timeline_dict in sketch['objects'][0]['timelines']:
            timeline_obj = timeline.Timeline(timeline_id=timeline_dict['id'], sketch_id=self.id, api=self.api, name=timeline_dict['name'], searchindex=timeline_dict['searchindex']['index_name'])
    ...
    def upload(self, timeline_name, file_path, index=None):
        ...
        """
        logger.warning('This function is about to be deprecated, please use the timesketch_import_client instead')
    ...
    def add_timeline(self, searchindex):
        ...
        Returns:
            Timeline object instance.
        """
        resource_url = '{0:s}/sketches/{1:d}/timelines/'.format(self.api.api_root, self.id)
        form_data = {'timeline': searchindex.id}
        response = self.api.session.post(resource_url, json=form_data)
        if response.status_code not in definitions.HTTP_STATUS_CODE_20X:
            error.error_message(response, message='Failed adding timeline', error=RuntimeError)
        response_dict = response.json()
        timeline_dict = response_dict['objects'][0]
        timeline_obj = timeline.Timeline(timeline_id=timeline_dict['id'], sketch_id=self.id, api=self.api, name=timeline_dict['name'], searchindex=timeline_dict['searchindex']['index_name'])
        return timeline_obj

    def explore(self, query_string=None, query_dsl=None, query_filter=None, view=None, return_fields=None, as_pandas=False, max_entries=None):
        """Explore the sketch.
        ...
        resource_url = '{0:s}/sketches/{1:d}/explore/'.format(self.api.api_root, self.id)
        form_data = {'query': query_string, 
         'filter': query_filter, 
         'dsl': query_dsl, 
        ...
        total_elastic_count = response_json.get('meta', {}).get('es_total_count', 0)
        if total_elastic_count != total_count:
            logger.info('{0:d} results were returned, but {1:d} records matched the search query'.format(total_count, total_elastic_count))
        if as_pandas:
    ...
    def list_available_analyzers(self):
        ...
        resource_url = '{0:s}/sketches/{1:d}/analyzer/'.format(self.api.api_root, self.id)
        response = self.api.session.get(resource_url)
        if response.status_code == 200:
            return response.json()
        return '[{0:d}] {1:s} {2:s}'.format(response.status_code, response.reason, response.text)

    def run_analyzer(self, analyzer_name, analyzer_kwargs=None, timeline_id=None, timeline_name=None):
        ...
        if analyzer_kwargs:
            if not isinstance(analyzer_kwargs, dict):
                return 'Unable to run analyzer, analyzer kwargs needs to be a dict'
        ...
        if timeline_name:
            ...
            if not timelines:
                return 'No timelines with the name: {0:s} were found'.format(timeline_name)
            if len(timelines) != 1:
                return 'There are {0:d} timelines defined in the sketch with this name, please use a unique name or a timeline ID'.format(len(timelines))
        ...
        for name, options in iter(meta.get('aggregators', {}).items()):
            for field in options.get('form_fields', []):
                ...
                if field.get('type') == 'ts-dynamic-form-select-input':
                    ...
                    _, _, entry['type'] = field.get('type').partition('ts-dynamic-form-')
    ...
    def run_aggregator(self, aggregator_name, aggregator_parameters):
        ...
        """
        aggregation_obj = aggregation.Aggregation(sketch=self, api=self.api)
        aggregation_obj.from_aggregator_run(aggregator_name=aggregator_name, aggregator_parameters=aggregator_parameters)
        return aggregation_obj
    ...
    def store_aggregation(self, name, description, aggregator_name, aggregator_parameters, chart_type=''):
        ...
        """
        logger.warning('This function is about to be deprecated, please use the `.save()` function of an aggregation object instead')
    ...
    def comment_event(self, event_id, index, comment_text):
        ...
        form_data = {'annotation': comment_text, 
         ...
         'events': {'_id': event_id, 
                    '_index': index, 
                    '_type': 'generic_event'}}
        resource_url = '{0:s}/sketches/{1:d}/event/annotate/'.format(self.api.api_root, self.id)
        response = self.api.session.post(resource_url, json=form_data)
        return response.json()
    ...
    def label_events(self, events, label_name):
        ...
        """
        form_data = {'annotation': label_name, 
    ...
    def search_by_label(self, label_name, as_pandas=False):
        ...
        """
        query = {'nested': {'path': 'timesketch_label', 
                    'query': {'bool': {'must': [
                                                {'term': {'timesketch_label.name': label_name}},
                                                {'term': {'timesketch_label.sketch_id': self.id}}]}}}}
        return self.explore(query_dsl=json.dumps({'query': query}), as_pandas=as_pandas)
    ...
    def add_event(self, message, timestamp, timestamp_desc):
        ...
        form_data = {'timestamp': timestamp, 
         ...
         'message': message}
        resource_url = '{0:s}/sketches/{1:d}/event/create/'.format(self.api.api_root, self.id)
        response = self.api.session.post(resource_url, json=form_data)
        return response.json()