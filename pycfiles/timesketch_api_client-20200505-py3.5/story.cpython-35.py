...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/timesketch_api_client/story.py
# Compiled at: 2020-04-27 10:54:06
# Size of source mod 2**32: 21862 bytes
...
class ViewBlock(BaseBlock):
    __doc__ = 'Block object for views.'
    TYPE = 'view'
    ...
    def to_dict(self):
        ...
        view_block['componentProps']['view'] = {'id': view_obj.id, 'name': view_obj.name}
        return view_block


class TextBlock(BaseBlock):
    __doc__ = 'Block object for text.'
    TYPE = 'text'
...
class AggregationBlock(BaseBlock):
    __doc__ = 'Block object for aggregation.'
    TYPE = 'aggregation'
    ...
    def to_dict(self):
        ...
        aggregation_block['componentProps']['aggregation'] = {'id': aggregation_obj.id, 
...
class AggregationGroupBlock(BaseBlock):
    __doc__ = 'Block object for aggregation groups.'
    ...
    def to_dict(self):
        ...
        group_block['componentProps']['aggregation_group'] = {'id': group_obj.id, 
         'name': group_obj.name}
        return group_block


class Story(resource.BaseResource):
    __doc__ = 'Story object.\n\n    Attributes:\n        id: Primary key of the story.\n    '
        ...
        if not self._blocks:
            ...
            for content_block in json.loads(content):
                ...
                if content_block.get('content'):
                    ...
                    if name == 'TsViewEventList':
                        block = ViewBlock(self, index)
                        block.from_dict(content_block)
                        view_obj = view.View(block.view_id, block.view_name, self._sketch.id, self._api)
                        block.feed(view_obj)
                    else:
                        if name == 'TsAggregationCompact':
                            ...
                            block.from_dict(content_block)
                            group_obj = aggregation.AggregationGroup(self._sketch, self._api)
    ...
    def commit(self):
        ...
        data = {'title': self.title, 
         'content': content}
        response = self._api.session.post('{0:s}/{1:s}'.format(self._api.api_root, self.resource_uri), json=data)
    ...
    def delete(self):
        ...
        """
        response = self._api.session.delete('{0:s}/{1:s}'.format(self._api.api_root, self.resource_uri))
        return response.status_code in definitions.HTTP_STATUS_CODE_20X
    ...
    def to_export_format(self, export_format):
        ...
        resource_url = '{0:s}/sketches/{1:d}/stories/{2:d}/'.format(self._api.api_root, self._sketch.id, self.id)
        ...
        logger.error('Error exporting story: [{0:d}] {1!s} {2!s}'.format(response.status_code, response.reason, response.text))
        return {}
    ...
    def to_string(self):
        ...
        for block in self.blocks:
            if block.TYPE == 'text':
                ...
                if block.TYPE == 'view':
                    data_frame = self._sketch.explore(view=block.view, as_pandas=True)
