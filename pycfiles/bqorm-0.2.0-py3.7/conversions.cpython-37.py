...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/bqorm/conversions.py
# Compiled at: 2019-03-29 16:28:53
# Size of source mod 2**32: 9954 bytes
import decimal, math, datetime, logging, json, dateutil
NoneType = type(None)
...
def convert(column, field_type='STRING', mode='NULLABLE', infer_required=False):
    ...
    if field_type == 'INTEGER':
        converted_column = [to_integer(value, mode, infer_required) for value in column]
    else:
        if field_type == 'STRING':
            converted_column = [to_string(value, mode, infer_required) for value in column]
        else:
            if field_type == 'NUMERIC':
                converted_column = [to_numeric(value, mode, infer_required) for value in column]
            else:
                if field_type == 'FLOAT':
                    converted_column = [to_float(value, mode, infer_required) for value in column]
                else:
                    if field_type == 'BOOLEAN':
                        converted_column = [to_boolean(value, mode, infer_required) for value in column]
                    else:
                        if field_type == 'BYTES':
                            converted_column = [to_bytes(value, mode, infer_required) for value in column]
                        else:
                            if field_type == 'DATETIME':
                                converted_column = [to_datetime(value, mode, infer_required) for value in column]
                            else:
                                if field_type == 'DATE':
                                    converted_column = [to_date(value, mode, infer_required) for value in column]
                                else:
                                    if field_type == 'TIME':
                                        converted_column = [to_time(value, mode, infer_required) for value in column]
                                    else:
                                        if field_type == 'TIMESTAMP':
                                            converted_column = [to_timestamp(value, mode, infer_required) for value in column]
                                        else:
                                            if field_type in ('STRUCT', 'ARRAY', 'GEOGRAPHY'):
...
def to_timestamp(value, mode='NULLABLE', infer_required=False):
    if isinstance(value, (int, float)):
        ts_value = math.isnan(value) or float(value)
    else:
