...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/yapsy/PluginFileLocator.py
# Compiled at: 2018-09-02 11:52:22
# Size of source mod 2**32: 19577 bytes
"""
Role
====
...
class PluginFileAnalyzerWithInfoFile(IPluginFileAnalyzer):
    ...
    def getPluginNameAndModuleFromStream(self, infoFileObject, candidate_infofile=None):
        ...
        if not config_parser.has_option('Core', 'Name') or not config_parser.has_option('Core', 'Module'):
            ...
            if PLUGIN_NAME_FORBIDEN_STRING in name:
                ...
                return (None, None, None)
            return (
             name, config_parser.get('Core', 'Module'), config_parser)
    ...
    def _extractCorePluginInfo(self, directory, filename):
        ...
        if (
         name, moduleName, config_parser) == (None, None, None):
            return (None, None)
        else:
            infos = {'name':name, 
             'path':os.path.join(directory, moduleName)}
                            ...
                            if not analyzer.isValidPlugin(filename):
                                ...
                                if candidate_infofile in _discovered:
                                    ...
                                    if os.path.isdir(plugin_info.path):
                                        ...
                                        if plugin_info.path.endswith('.py') and os.path.isfile(plugin_info.path) or os.path.isfile(plugin_info.path + '.py'):
