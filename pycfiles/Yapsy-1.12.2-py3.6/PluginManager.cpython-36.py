...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/yapsy/PluginManager.py
# Compiled at: 2019-07-27 12:41:13
# Size of source mod 2**32: 25864 bytes
"""
Role
====
...
class PluginManager(object):
    ...
    def _locatorDecide(self, plugin_info_ext, plugin_locator):
        ...
        if not specific_info_ext:
            if not specific_locator:
                res = PluginFileLocator()
        if not specific_info_ext and specific_locator:
            res = plugin_locator
        elif not specific_locator and specific_info_ext:
            res = PluginFileLocator()
            res.setAnalyzers([PluginFileAnalyzerWithInfoFile('info_ext', plugin_info_ext)])
        else:
            if specific_info_ext:
                if specific_locator:
                    msg = ('Two incompatible arguments (%s) provided:', "'plugin_info_ext' and 'plugin_locator'). Ignoring",
                           "'plugin_info_ext'.")
...
class PluginManagerSingleton(object):
    ...
    _PluginManagerSingleton__instance = None
    _PluginManagerSingleton__decoration_chain = None

    def __init__(self):
        if self._PluginManagerSingleton__instance is not None:
    ...
    def setBehaviour(self, list_of_pmd):
        """
                ...
                value is returned.
                """
        if self._PluginManagerSingleton__decoration_chain is None and self._PluginManagerSingleton__instance is None:
            log.debug('Setting up a specific behaviour for the PluginManagerSingleton')
            self._PluginManagerSingleton__decoration_chain = list_of_pmd
            return True
        else:
    ...
    def get(self):
        """
                ...
                """
        if self._PluginManagerSingleton__instance is None:
            if self._PluginManagerSingleton__decoration_chain is not None:
                pm = self._PluginManagerSingleton__decoration_chain[0]()
                for cls_item in self._PluginManagerSingleton__decoration_chain[1:]:
                    pm = cls_item(decorated_manager=pm)

                self._PluginManagerSingleton__instance = pm
            else:
                self._PluginManagerSingleton__instance = PluginManager()
            log.debug('PluginManagerSingleton initialised')
        return self._PluginManagerSingleton__instance
