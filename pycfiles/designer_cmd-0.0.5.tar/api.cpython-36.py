...
# [GCC 8.4.0]
# Embedded file name: designer_cmd\api.py
# Compiled at: 2020-05-10 05:05:17
# Size of source mod 2**32: 11138 bytes
...
class Connection:
    ...
    def __init__(self, user: str='', password: str='', file_path: str='', server_path: str='', server_base_ref: str='', ib_name: str=''):
        if file_path == '':
            if server_path == '' or server_base_ref == '':
                if ib_name == '':
...
class Designer:
    ...
    def create_base(self):
        ...
        self._Designer__execute_command('CREATEINFOBASE', params, False)
    ...
    def updete_db_config(self, dynamic: bool=False, warnings_as_errors: bool=False):
        ...
        params = ['/UpdateDBCfg']
        self._Designer__execute_command('DESIGNER', params)
    ...
    def dump_config_to_files(self, catalog_path: str) -> None:
        ...
        """
        full_catalog_path = os.path.abspath(catalog_path)
        logger.info(f"Выгружаю конфигурацию в файлы {full_catalog_path} по соединению {self.connection}")
    ...
    def load_config_from_file(self, file_path: str) -> None:
        ...
        """
        full_file_path = os.path.abspath(file_path)
        logger.info(f"Загружаю конфигурацию из файла {full_file_path} в конфигурацию БД по соединению {self.connection}")
    ...
    def dump_config_to_file(self, file_path: str) -> None:
        ...
        """
        full_file_path = os.path.abspath(file_path)
        logger.info(f"Сохраняю конфигурацию в файл {full_file_path} из конфигурации БД по соединению {self.connection}")
    ...
    def compare_config_with_file(self, cf_file_path: str, report_path: str) -> None:
        ...
        logger.info(f"Сравниваю конфигурацию по соединению {self.connection} с файлом {full_cf_file_path} для сохранения отчета по пути {full_report_path} ")
        params = [
         '/CompareCfg',
