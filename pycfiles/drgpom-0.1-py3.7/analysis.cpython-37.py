...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\drgpom\methods\analysis.py
# Compiled at: 2020-02-21 07:53:35
# Size of source mod 2**32: 39433 bytes
"""
...
def fill_in_grouped_sim_data(grouped_sim_data, results, biomarker, sim_types, sim_type_amp_relationship, sim_name_conversions, how_to_fill, filter_biomarkers_for_outliers=False, threshold=None):
    """"
                    ...
                    if how_to_fill == 'median':
                        ...
                        if how_to_fill == 'mean_fillna':
                            ...
                            if how_to_fill == 'mean_freq_fillna':
                                ...
                                if how_to_fill == 'mean_freq_dropna':
                                    assert biomarker == 'ISI'
                                    unaggregated_data = 1000.0 / unaggregated_data
                                    data = unaggregated_data.dropna().mean()
                                else:
                                    if isinstance(how_to_fill, int):
...
def make_grouped_sim_data(pop, biomarker='APFullWidth', agg='mean', filter_outliers=False, scaled_parameters=['GNav18'], threshold=None):
    ...
    grouped_sim_data = make_empty_grouped_sim_data(pop=pop, biomarker=biomarker,
      filter_outliers=filter_outliers,
      scaled_parameters=scaled_parameters,
      sim_types=sim_types,
      sim_type_as_base=sim_type_as_base,
      sim_type_amp_relationship=sim_type_amp_relationship)
    sim_name_conversions = make_sim_name_conversions(pop.get_simulation_names(), sim_types, sim_type_amp_relationship, sim_type_as_base)
    fill_in_grouped_sim_data(grouped_sim_data, (pop.results), biomarker, sim_types,
      sim_type_amp_relationship, sim_name_conversions,
      how_to_fill=agg,
...
def make_empty_grouped_sim_data(pop, biomarker='APFullWidth', filter_outliers=False, scaled_parameters=[
 'GNav18'], sim_types=['step', 'ramp'], sim_type_as_base='step', sim_type_amp_relationship={'step':1,  'ramp':10}):
    """ Aggregate population biomarker results per simulation to analyse at a per simulation level over the ensemble population. """
    arrays = [
     [
      biomarker] * len(sim_types) + ['Amp'] * len(sim_types) + ['Scaling factors'] * len(scaled_parameters), sim_types * 2 + scaled_parameters]
...
def process_firing_pattern_data(firing_pattern_percentages, sim_types=[
 'step', 'ramp'], sim_type_amp_relationship={'step':1, 
 'ramp':10}, scaled_parameters=['GNav18']):
    """
        ...
        for sim_name in sim_names:
            if (sim_name not in sim_types) & (sim_type in sim_name):
                ...
                if short_sim_name in sim_name_conversions.keys():
                    ...
                    assert sim_type == sim_type_as_base, 'Sim type: {}, name:{}, short_sim_name:{} is not the base sim type for the sim type amp relationship.'.format(sim_type, sim_name, short_sim_name)
                    sim_name_conversions[short_sim_name] = {sim_name: sim_type}
                if sim_type == sim_type_as_base:
                    short_sim_names.append(short_sim_name)

    short_sim_names = sorted(short_sim_names)
    formatted_firing_pattern_data = format_firing_pattern_percentages(firing_pattern_percentages, short_sim_names, sim_name_conversions, scaled_parameters, sim_types, sim_type_amp_relationship)
...
def format_firing_pattern_percentages(firing_pattern_percentages, short_sim_names, sim_name_conversions, scaled_parameters, sim_types, sim_type_amp_relationship):
    ...
    assert sim_type_amp_relationship[sim_type_as_base] == 1
    arrays = [
     list(np.repeat(firing_pattern_names, len(sim_types))) + ['Amp', 'Amp', 'Scaling factors'],
     sim_types * (1 + len(firing_pattern_names)) + scaled_parameters]
    columns = pd.MultiIndex.from_arrays(arrays, names=['', ''])
...
def define_region(pop, firing_pattern_thresholds={}, other_thresholds={}, stim_type='step', verbose=False):
    ...
    firing_pattern_percentages = get_firing_pattern_percentages(pop)
    firing_pattern_percentages = process_firing_pattern_data(firing_pattern_percentages)
    region = {}
    simulations = firing_pattern_percentages.index
    count = 0
...
def get_biomarker_values_by_simulation(results, biomarker_names, simulation_names):
    ...
    """
    data = results.copy()
    data = results.swaplevel(0, 1, axis=1)
    data = data.sort_index(level=0, axis=1)
    data = data.loc[:, (biomarker_names, simulation_names)]
    data = data.stack().reset_index().set_index('Model')
    data = data.rename(columns={'': 'Simulation'})
    return data
...
def model_consistency_in_region(model, pop, region, firing_pattern, stim_type, amp_stim_relationship):
    ...
    model_consistency = 100.0 * (num_consistent_simulations / num_simulations)
...
def assign_subpopulation_from_region(pop, region, criteria, verbose=False):
    ...
    for criterion in criteria:
        ...
        pom_consistency = pom_consistency_in_region(pop, region,
          firing_pattern=firing_pattern,
          stim_type='step',
          amp_stim_relationship={'step':1, 
         'ramp':10})
    ...
    if verbose:
        print('{} models out of {} in population of models are in the subpopulation'.format(len(subpopulation.index), len(pop.results.index)))
...
def calculate_percentage_of_firing_patterns(firing_patterns):
    ...
    firing_patterns = firing_patterns.dropna()
...
def pom_consistency_stats(pom_consistency):
    return {'mean':pom_consistency['Consistency'].mean(), 
     'std':pom_consistency['Consistency'].std()}
...
def make_3d_plot(data, x, y, z, cutoff, fillna_value, labels, angle=(20, 300), zticks=None, title=None):
    ...
    df.loc[(df['Z'] >= cutoff, 'Z')] = fillna_value * 3
...
def plot_biomarker_boxplots(pop, biomarker, stim_amps, sim_type, sim_types=[
 'step', 'ramp'], sim_type_amp_relationship={'step':1.0, 
 'ramp':10.0}, base_sim_type='step', save=False, save_path=None, fixed_scaling_factors={}):
...
def plot_biomarker_scatterplots(pop, biomarkers, stim_amps, sim_type, sim_types=['step', 'ramp'], sim_type_amp_relationship={'step':1, 
 'ramp':10}, base_sim_type='step', selected_scaling_factors=None, plot_together=False, plot=True):
...
def get_non_rheobase_sim_names(pop):
    sims = [sim_name for sim_name in pop.results.columns.levels[0] if sim_name not in ('Parameters',
                                                                                       'ramp',
                                                                                       'step')]
    return sims
...
def get_amp_str_from_sim_name(name, amp_units, delimiter):
    ...
    start = name[0:end].rfind(delimiter) + 1
...
def get_all_firing_patterns_in_population(pop):
    ...
    for sim in sims:
        ...
        sim_firing_patterns = df['Firing pattern'].dropna()
...
def short_name_to_full(short_name, stim_type, amp_stim_relationship={'step':1, 
 'ramp':10}, delimiter='_', amp_units='pA'):
    """
    ...
    stim_type_amp_str = str(amp_val * int(amp_stim_relationship[stim_type]))
...
def process_sim_name(name, sim_types, sim_type_amp_relationship, amp_units='pA', delimiter='_'):
    ...
    for sim_type in sim_types:
        if sim_type in name:
            ...
            new_amp_string = str(int(new_amp))
            new_name = new_name.replace('{}{}{}'.format(amp_string, delimiter, amp_units), '{}{}{}'.format(new_amp_string, delimiter, amp_units))
            return (new_name, sim_type)


def get_simulations_with_fixed_stim_amp(pop, stim_amp, sim_type='step', sim_type_amp_relationship={'step':1.0,  'ramp':10}, base_sim_type='step', sim_types=['step', 'ramp'], fixed_scaling_factors={}):
...
def get_default_stim_type_amp_relationship():
    sim_type_amp_relationship = {'step':1, 
     'ramp':10}
