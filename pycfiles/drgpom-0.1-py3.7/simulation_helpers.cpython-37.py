...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\drgpom\methods\simulation_helpers.py
# Compiled at: 2020-04-06 10:09:15
# Size of source mod 2**32: 29093 bytes
"""
Simulation helpers 
...
import numbers, pyDOE, itertools
from neuron import h
import neuron
from . import loadneuron as ln
ln.load_neuron_mechanisms(verbose=True)

def init_model(mechanisms=[], L=30.0, diam=46.0, cm=1.0, Ra=123.0, ko=3.0, ki=135.0, nao=145.0, nai=5.0):
    """
...
def get_init_model_values(name='first'):
    ...
    if name not in allowed_names:
        ...
        if name == 'first':
            model_values['L'] = 30.0
            model_values['diam'] = 46.0
            model_values['cm'] = 1.0
            model_values['Ra'] = 123.0
            model_values['ko'] = 3.0
            model_values['ki'] = 135.0
            model_values['nao'] = 145.0
            model_values['nai'] = 5.0
    return model_values


def build_model(mechanisms={'kdrtf':1.0, 
 'katf':1.0,  'nav18hw':1.0}, conductances=None, mechanism_names=None, mechanism_is_full_parameter_name=False):
    """
        ...
        for current in current_set:
            ...
            if current in get_ionic_current_list():
                ...
                if (current[0:2] == 'na') | ('nav' in current.lower()):
                    ...
                    if (current[0] == 'k') | any((s in current.lower() for s in ('kdr',
                                                                                 'ka',
                                                                                 'km',
                                                                                 'kv'))):
                        i_strs = [
                            ...
                            if 'hcn' in current.lower():
                                if current == 'hcntf':
                                    ...
                                    if current == 'ch_HCNp':
                                        ...
                                        if current == 'hcnkn':
                                            raise ValueError('hcnkn not supported yet')
                                        else:
...
def generate_current_set(mechanisms_to_record, record_ionic_currents=True):
    ...
    currents_to_record = mechanisms_to_record.copy()
...
def build_parameter_set(num_models, parameter_data, minimum=None, maximum=None, filename=None, save=False):
    ...
    """
    parameter_names = [name for name in parameter_data]
    ...
    if isinstance(parameter_data, dict):
        ...
        for parameter_name, range in parameter_data.items():
            ...
            header += '{}: ({} {}) '.format(parameter_name, range[0], range[1])

    else:
        if isinstance(minimum, numbers.Number) & isinstance(maximum, numbers.Number) & isinstance(parameter_data, list):
    ...
    if save:
        with open(filename, 'w') as (f):
            ...
            parameter_sets = parameter_sets.round(6)
...
def build_empty_parameter_set_details():
    ...
    """
    parameter_set_details = {'num_models':None, 
     'parameter_data':None,  'minimum':None,  'maximum':None,  'save':False,  'output_filename':None}
...
def build_empty_sim_protocols():
    ...
    """
    sim_protocols_keys = [
     'delay', 'amp', 'dur', 'interval', 'num_stims', 'stim_func', 't_stop', 'v_init', 'currents_to_record']
    return {protocol:None for protocol in sim_protocols_keys}


def simulation(amp, dur, delay, interval=0, num_stims=1, stim_func=h.IClamp, mechanisms={'kdrtf':1.0,  'katf':3.0,  'nav18hw':1.0}, t_stop=1000.0, make_plot=False, plot_type='default', model=None, ions=['Na', 'K'], mechanisms_to_record=None, record_ionic_currents=True, concs_to_record=None):
    """
...
def simulation_for_ab(amp, dur, delay, interval, num_stims=40, stim_func=h.IClamp, mechanisms={'kdrtf':1.0,  'katf':3.0,  'nav18hw':1.0}, t_stop=1000.0):
    """
...
def divide_into_sections(simulations, n):
    ...
    for i in range(1, n + 1):
        ...
        num_sims_allocated += num_sims_in_section
...
def get_simulation_section(simulations, section, n):
    ...
    simulation_section = simulations[sim_sections[section]]
