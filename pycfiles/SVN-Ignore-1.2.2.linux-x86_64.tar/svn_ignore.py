...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/src/svn_ignore.py
# Compiled at: 2016-08-22 16:12:06
import os, subprocess, logging, glob2
...
class SVNIgnore:
        ...
        for directory, sub_directories, files in os.walk(self.directory, topdown=True):
            ...
            if self.ignore_file in files:
                try:
                    ...
                    self.logger.error('Could not read ignores from %s: %s', os.path.join(directory, self.ignore_file), exception.message)
                    return
            ...
            if self.recursive:
                try:
                    ...
                    self.logger.error('Could not read ignores from parent directory of %s: %s', os.path.join(directory, self.ignore_file), exception.message)
                    return
    ...
    def add_exceptions(self, directory, ignores):
        ...
        for exception in exceptions:
            ...
            for file, version in glob2.iglob(pattern, with_matches=True):
                ...
                process = subprocess.Popen([
                 'svn',
                 'add',
                 file], cwd=directory, stderr=subprocess.PIPE, stdout=subprocess.PIPE).communicate()
        ...
        return list(filter(lambda item: not item.startswith('!'), ignores))
    ...
    def get_existing_ignores(self, directory):
        ...
        process = subprocess.Popen([
         ...
         'propget',
         'svn:ignore',
         directory], cwd=directory, stderr=subprocess.PIPE, stdout=subprocess.PIPE).communicate()
    ...
    def get_ignores_from_file(self, directory, remove_comments=True):
        ...
        f.close()
        if remove_comments:
            new_ignores = list(filter(lambda item: not item.startswith('#'), new_ignores))
    ...
    def set_ignores(self, directory, ignores):
        ...
        process = subprocess.Popen([
         ...
         'svn:ignore',
         ('\n').join(ignores),
         '.'], cwd=directory, stderr=subprocess.PIPE, stdout=subprocess.PIPE).communicate()
        if process[1]:
