...
# [GCC 8.4.0]
# Embedded file name: /home/ajdiaz/env/mico/lib/python2.7/site-packages/mico/lib/aws/ec2/autoscale.py
# Compiled at: 2013-05-13 11:44:52
...
def as_connect(region=None, *args, **kwargs):
    ...
    connection = AutoScaleConnection(os_environ.get('AWS_ACCESS_KEY_ID'), os_environ.get('AWS_SECRET_ACCESS_KEY'), region=region, *args, **kwargs)
    return connection
...
def as_config(name, ami, force=False, *args, **kwargs):
    ...
    if 'security_groups' in kwargs:
        ...
        _sgs = map(lambda x: x.name if isinstance(x, SecurityGroup) else x, _sgs)
    kwargs['security_groups'] = _sgs
    connection = as_connect()
    config = LaunchConfiguration(name=name, image_id=ami, instance_monitoring=True, *args, **kwargs)
    connection.create_launch_configuration(config)
    mico.output.info('create new config %s' % name)
    return config


def as_policy(name, adjustment_type='ChangeInCapacity', scaling_adjustment=2, cooldown=60, *args, **kwargs):
    """
    Scaling Policy
...
def as_ensure(name, zones, instance, balancers=[], events=[], min_size=2, max_size=20, desired_size=None, force=False):
    ...
    ag = AutoScalingGroup(name=ag_name, availability_zones=zones, launch_config=instance, load_balancers=_l, min_size=min_size, max_size=max_size, desired_capacity=desired_size)
    ...
    for group in conn.get_all_groups():
        for arg in args:
            if fnmatch(group.name, arg):
                ...
                if len(_i) > 0:
                    instances = [ i for r in ec2.get_all_instances(_i) for i in r.instances
                                ]
                    for instance in instances:
                        instance.name = instance.tags.get('Name', None)
                        instance.secgroups = (',').join(map(lambda x: x.name, instance.groups))
