...
# [GCC 8.4.0]
# Embedded file name: /home/ajdiaz/env/mico/lib/python2.7/site-packages/mico/__init__.py
# Compiled at: 2013-05-17 09:42:50
import os, sys
config_path = [
 os.curdir,
...
cache_path = os.environ.get('MICO_CACHE_PATH', None) or os.path.join(os.environ.get('HOME', '/'), '.cache/mico')
lib_path = [
 os.path.join(os.environ.get('HOME', '/'), '.local/share/mico'),
 '/usr/lib/mico',
 '/usr/local/lib/mico',
 '/usr/share/mico/usr/local/share/mico']
...
def execute(action, *args, **kwargs):
    ...
    return fabric_execute(mico.hook.task_add_pre_run_hook(mico.hook.run_pre_hook)(mico.hook.task_add_post_run_hook(mico.hook.run_post_hook)(action)), *args, **kwargs)
