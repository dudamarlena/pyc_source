...
# [GCC 8.4.0]
# Embedded file name: /home/ajdiaz/env/mico/lib/python2.7/site-packages/mico/scripts/cmdline.py
# Compiled at: 2013-05-27 07:00:12
import os, sys, cmd, shlex, random, pkgutil, mico, mico.fifo, mico.util, mico.hook, mico.output
...
class MicoCmdline(cmd.Cmd):
    ...
    def complete(self, text, state):
        ...
        options = [ i for i in map(lambda x: x[3:], filter(lambda x: x.startswith('do_'), dir(self))) if i.startswith(text) and i != 'EOF'
                  ]
...
def main():
    ...
    import argparse
    cmdopt = argparse.ArgumentParser(description='mico: %s' % random.choice(mico.output.intros), epilog='©2012  Andrés J. Díaz <ajdiaz@connectical.com>')
    cmdopt.add_argument('-e', '--env', action='append', dest='env', help='set environment variable', type=str, default=[])
    cmdopt.add_argument('-H', '--host', action='append', dest='host', help='set host to target', type=str, default=[])
    cmdopt.add_argument('-R', '--region', action='store', dest='region', help='set EC2 region to work on', type=str, default='us-east-1')
    cmdopt.add_argument('-v', '--verbose', action='store_true', dest='verbose', help='be verbose', default=False)
    cmdopt.add_argument('template', nargs='*', default=None, help='The template:function to execute or an internal command')
