...
# [GCC 8.4.0]
# Embedded file name: /home/ajdiaz/env/mico/lib/python2.7/site-packages/mico/lib/core/service.py
# Compiled at: 2013-04-18 12:01:18
...
def service_is_running(service):
    ...
    with settings(hide('running', 'stdout', 'stderr', 'warnings'), warn_only=True):
        res = mico_sudo(service_string % {'service': service, 
           'action': 'status'}, force=True)
...
def service_start(service):
    ...
    mico.hook.add_post_hook(mico_sudo, (
     service_string % {'service': service, 
        'action': 'start'},))
...
def service_add_boot(service):
    ...
    if env.custom.operating_system == 'ubuntu' or env.custom.operating_system == 'debian':
        _x = mico_sudo('update-rc.d %s defaults' % service)
    if env.custom.operating_system == 'redhat' or env.custom.operating_system == 'centos' or env.custom.operating_system == 'fedora':
