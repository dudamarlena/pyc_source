...
# [GCC 8.4.0]
# Embedded file name: /home/ajdiaz/env/mico/lib/python2.7/site-packages/mico/lib/aws/ec2/elb.py
# Compiled at: 2013-05-09 10:12:59
...
import boto.ec2.elb, mico.output
...
def elb_connect(region=None, *args, **kwargs):
    ...
    connection = ELBConnection(os_environ.get('AWS_ACCESS_KEY_ID'), os_environ.get('AWS_SECRET_ACCESS_KEY'), region=region, *args, **kwargs)
    return connection
...
def elb_check(target, interval=20, healthy_threshold=3, unhealthy_threshold=5):
    ...
    """
    return HealthCheck(target=target, interval=interval, healthy_threshold=healthy_threshold, unhealthy_threshold=unhealthy_threshold)
...
def elb_listener(bind_port, remote_port, protocol, cert=None):
    ...
    if cert is not None:
        if isinstance(cert, str):
            ...
            return (
             bind_port, remote_port, protocol, cert.arn)

    else:
        return (
         bind_port, remote_port, protocol)
    return
...
def elb_ensure(name, listeners, check, zones=None, *args, **kwargs):
    ...
    if _obj:
        ...
        elb = connection.create_load_balancer(name=name, zones=zones, listeners=listeners, *args, **kwargs)
