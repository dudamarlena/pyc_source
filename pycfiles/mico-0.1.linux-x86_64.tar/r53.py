...
# [GCC 8.4.0]
# Embedded file name: /home/ajdiaz/env/mico/lib/python2.7/site-packages/mico/lib/aws/r53.py
# Compiled at: 2013-05-09 09:44:03
...
def r53_connect(region=None, *args, **kwargs):
    ...
    connection = Route53Connection(os_environ.get('AWS_ACCESS_KEY_ID'), os_environ.get('AWS_SECRET_ACCESS_KEY'), *args, **kwargs)
    return connection
...
def r53_zone_remove(domainname):
    ...
    response = filter(lambda x: x['Name'] == domainname, response['ListHostedZonesResponse']['HostedZones'])
    ret = []
    for x in response:
...
def r53_zone_exists(filter_expr='*'):
    ...
    connection = connect()
    response = connection.get_all_hosted_zones()
    return filter(lambda x: fnmatch(x['Name'], filter_expr), response['ListHostedZonesResponse']['HostedZones'])
...
def _add_del(conn, hosted_zone_id, change, name, type, identifier, weight, values, ttl, comment):
    ...
    change = changes.add_change(change, name, type, ttl, identifier=identifier, weight=weight)
    for value in values:
...
def _add_del_alias(conn, hosted_zone_id, change, name, type, identifier, weight, alias_hosted_zone_id, alias_dns_name, comment):
    from boto.route53.record import ResourceRecordSets
    changes = ResourceRecordSets(conn, hosted_zone_id, comment)
    change = changes.add_change(change, name, type, identifier=identifier, weight=weight)
...
def r53_record_ensure(zone, name, type, values, ttl=600, identifier=None, weight=None, comment='', force=False):
    """Add a new record to a zone.  identifier and weight are optional."""
    conn = connect()
    if name[(-1)] != '.':
        response = conn.get_all_hosted_zones()
        domain = filter(lambda x: x['Id'].split('/')[(-1)] == zone, response['ListHostedZonesResponse']['HostedZones'])
        if len(domain) > 0:
    ...
    if not force:
        ...
        if _obj:
            return _obj
    return _add_del(conn, zone, 'CREATE', name, type, identifier, weight, values, ttl, comment)


def r53_record_remove(hosted_zone_id, name, type, values, ttl=600, identifier=None, weight=None, comment=''):
    """Delete a record from a zone: name, type, ttl, identifier, and weight must match."""
    conn = connect()
    return _add_del(conn, hosted_zone_id, 'DELETE', name, type, identifier, weight, values, ttl, comment)


def r53_alias_ensure(hosted_zone_id, name, type, alias_hosted_zone_id, alias_dns_name, identifier=None, weight=None, comment=''):
    """Add a new alias to a zone.  identifier and weight are optional."""
    conn = connect()
    return _add_del_alias(conn, hosted_zone_id, 'CREATE', name, type, identifier, weight, alias_hosted_zone_id, alias_dns_name, comment)


def r53_alias_remove(hosted_zone_id, name, type, alias_hosted_zone_id, alias_dns_name, identifier=None, weight=None, comment=''):
    """Delete an alias from a zone: name, type, alias_hosted_zone_id, alias_dns_name, weight and identifier must match."""
    conn = connect()
    return _add_del_alias(conn, hosted_zone_id, 'DELETE', name, type, identifier, weight, alias_hosted_zone_id, alias_dns_name, comment)


def r53_record_change(hosted_zone_id, name, type, newvalues, ttl=600, identifier=None, weight=None, comment=''):
    ...
    for response in responses:
        ...
        if response.identifier != identifier or response.weight != weight:
            continue
        change1 = changes.add_change('DELETE', name, type, response.ttl, identifier=response.identifier, weight=response.weight)
        for old_value in response.resource_records:
            change1.add_value(old_value)

    change2 = changes.add_change('CREATE', name, type, ttl, identifier=identifier, weight=weight)
...
def r53_alias_change(hosted_zone_id, name, type, new_alias_hosted_zone_id, new_alias_dns_name, identifier=None, weight=None, comment=''):
    ...
    for response in responses:
        ...
        if response.identifier != identifier or response.weight != weight:
            continue
        change1 = changes.add_change('DELETE', name, type, identifier=response.identifier, weight=response.weight)
