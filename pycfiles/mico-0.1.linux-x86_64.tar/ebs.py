...
# [GCC 8.4.0]
# Embedded file name: /home/ajdiaz/env/mico/lib/python2.7/site-packages/mico/lib/aws/ec2/ebs.py
# Compiled at: 2013-04-29 09:16:02
...
def ebs_ensure(size, zone=None, instance=None, device=None, tags={}, force=False, **kwargs):
...
def ebs_exists(tags={}):
    ...
    """
    connection = ec2_connect()
    _x = connection.get_all_volumes(None, dict(map(lambda (x, y): ('tag:%s' % x, y), tags.items())))
    ...
    for x in vol:
        ...
        for arg in args:
            if x.name and fnmatch(x.name, arg):
                ...
                x.instance_id = '%s (%s)' % (ins[x.attach_data.instance_id].tags.get('Name', None), x.attach_data.instance_id) if x.attach_data.id is not None else None
                yield x.__dict__
