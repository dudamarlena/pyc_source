...
# [GCC 8.4.0]
# Embedded file name: /home/ajdiaz/env/mico/lib/python2.7/site-packages/mico/templates/ls.py
# Compiled at: 2013-05-24 11:16:43
import mico.output
...
def instances(*args):
    ...
    """
    fmt = '%(id)-15s%(name)-32s%(_state)-15s%(root_device_type)-15s' + '%(instance_type)-12s%(image_id)-20s%(_placement)-15s%(secgroups)-25s%(ip_address)-20s'
    print fmt % {'id': 'ID', 
       'name': 'Name', 
       '_state': 'State', 
...
def volumes(*args):
    ...
    """
    fmt = '%(id)-15s%(name)-35s%(status)-15s%(size)-8s' + '%(iops)-6s%(zone)-15s%(device)-20s%(instance_id)-20s'
    print fmt % {'id': 'ID', 
       'name': 'Name', 
       'status': 'Status', 
       'size': 'Size', 
...
def autoscale(*args):
    ...
    """
    fmt = '%(id)-15s%(name)-20s%(autoscaling_group)-35s%(_state)-15s%(root_device_type)-18s' + '%(instance_type)-12s%(image_id)-20s%(_placement)-15s%(secgroups)-10s%(ip_address)-8s'
    print fmt % {'id': 'ID', 
       'name': 'Name', 
