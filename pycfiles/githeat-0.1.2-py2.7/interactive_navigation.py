...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/githeat/util/interactive_navigation.py
...
below = lambda csr, n: Cursor(y=min(csr.term.height - 1, csr.y + n), x=csr.x, term=csr.term)
right_of = lambda csr, n: Cursor(y=csr.y, x=min(csr.term.width - 1, csr.x + n), term=csr.term)
left_of = lambda csr, n: Cursor(y=csr.y, x=max(0, csr.x - n), term=csr.term)
...
top = lambda csr: Cursor(y=0, x=csr.x, term=csr.term)
center = lambda csr: Cursor(csr.term.height // 2, csr.term.width // 2, csr.term)
lookup_move = lambda inp_code, csr, term, githeat: {csr.term.KEY_END: below(left_of(csr, 1), 1), 
   ...
   csr.term.KEY_SUP: above(csr, 4)}.get(inp_code, csr)