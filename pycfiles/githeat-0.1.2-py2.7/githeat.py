...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/githeat/githeat.py
...
class Githeat:
    ...
    def __init__(self, git_repo, gtype='block', width='reg', days=[], color='grass', colors=[], stat=False, stat_number=5, separate=True, month_merge=False, legend=False, author=None, grep=None, config=None, logging_level='CRITICAL'):
        global COLORS
    ...
    def toggle_day(self, day_num):
        ...
        """
        if 0 <= day_num <= 6:
            self.days_toggle[day_num] = not self.days_toggle[day_num]
    ...
    def parse_commits(self):
        ...
        if raw_commits and raw_commits[0]:
            for rc in raw_commits:
                abbr_commit_hash, exact_date_and_time, author, author_email, subject = helpers.remove_accents(rc).split(delimiter)
                exact_date_and_time = parse_date(exact_date_and_time)
                if self.days and exact_date_and_time.strftime('%A') not in self.days:
                    continue
                commit = Commit(abbr_commit_hash, exact_date_and_time, author, author_email, subject)
                if exact_date_and_time.date() in self.commits_db:
                    self.commits_db[exact_date_and_time.date()].append(commit)
                else:
                    self.commits_db[exact_date_and_time.date()] = [
                     commit]
        ...
        for commits_on_day in self.commits_db:
            for commit in self.commits_db[commits_on_day]:
                if self.days and commit.date.strftime('%A') not in self.days:
                    continue
                if self.display_months and datetime.date(commit.date.year, commit.date.month, 1) not in self.display_months:
                    continue
                contribution_day = datetime.date(commit.date.year, commit.date.month, commit.date.day)
    ...
    def normalize_daily_contribution_map(self, x1=0, x2=5):
        ...
        self.daily_contribution_map = helpers.normalize_dict(self.daily_contribution_map, x1, x2)
    ...
    def compute_graph_matrix(self):
        ...
        for current_day in sorted_normalized_daily_contribution:
            ...
            try:
                last_week_col.append([current_day,
                 colorize(self.width, ansi=0, ansi_bg=color)])
            except ValueError:
                new_column = self._Column(self.width)
                matrix.append(new_column)
                last_week_col = matrix[(-1)]
    ...
    def print_inline(self):
        ...
        for current_day in sorted_normalized_daily_contribution:
            ...
            print(colorize(self.width, ansi=0, ansi_bg=color), end=(' {}{}').format(current_day.strftime('%b %d, %Y'), '\n'))
    ...
    def print_stats(self):
        ...
        top_n = self.get_top_n_commiters(list(itertools.chain.from_iterable(self.commits_db.values())), n)
        if top_n:
    ...
    def run(self):
        ...
        if self.gtype == 'inline':
            ...
            if matrix_width > int(width):
                print('Your terminal width is smaller than the heatmap. Please consider using the --width {thin, reg, thick},  resizing your terminal, or merging months by including --month-merge')
                return
