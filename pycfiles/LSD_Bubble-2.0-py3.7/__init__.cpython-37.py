...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/LSD/__init__.py
# Compiled at: 2019-02-26 15:23:00
# Size of source mod 2**32: 8954 bytes
"""The functions to handle the main function"""
from argparse import ArgumentParser
from LSD.inout import load
import LSD.detecter.onodera as onodera
import logging
...
def main():
    ...
    parent.add_argument('-f', '--format', action='store', dest='format', help='The format of the input file', default='edgelist',
      choices=['edgelist', 'adjlist', 'gexf', 'gml', 'gpickle', 'graph6',
    ...
    repo = parent.add_argument_group('Reporter', 'The output reporter. Only one can be applied. When nothing is given the print reporter is used.')
    repo.add_argument('--print', action='store_true', dest='print', help='Print on stdout the complete superbubble.')
    repo.add_argument('--sprint', action='store_true', dest='sprint', help='Print on stdout the s and t of a superbubble.')
    ...
    repo.add_argument('--complete', action='store', dest='outpath', help="Give a path where a complete output is written. Use '-' for stdout.")
...
def part_detection(g, rep, args):
    ...
    for c in scc:
        ...
        if not c.source_connected():
            if not c.sink_connected():
                logger.debug('Use sung algorithm')
