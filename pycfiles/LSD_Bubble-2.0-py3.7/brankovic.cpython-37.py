...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/LSD/detecter/brankovic.py
# Compiled at: 2019-01-31 04:50:25
# Size of source mod 2**32: 4361 bytes
...
def super_bubble_brankovice(c, toposort, reporter):
    ...
    while len(candidates) != 0:
        if candidates[(-1)][1]:
            ...
            report_super_bubble(candidates[0][0], candidates[(-1)][0], c, candidates, backcand, toposort, ord_id, previous_entrance, alternative_entrance, out_child_rmq, out_parent_rmq, report)


def report_super_bubble(start_vertex, exit_vertex, c, candidates, backcand, toposort, ord_id, previous_entrance, alternative_entrance, out_child_rmq, out_parent_rmq, report):
    ...
    while s is not None and ord_id(s) >= ord_id(start_vertex):
        valid = validate_super_bubble(s, exit_vertex, c, toposort, ord_id, out_child_rmq, out_parent_rmq, previous_entrance)
        ...
        if valid == s:
            ...
            while candidates[(-1)][0] != s:
                if candidates[(-1)][1]:
                    ...
                    report_super_bubble(next_candidate(s, candidates, backcand), candidates[(-1)][0], c, candidates, backcand, toposort, ord_id, previous_entrance, alternative_entrance, out_child_rmq, out_parent_rmq, report)


def validate_super_bubble(start_vertex, end_vertex, c, toposort, ord_id, out_child_rmq, out_parent_rmq, previous_entrance):
    ...
    if outparent == start:
        return start_vertex
    if is_entrance(outparent, c, toposort):
