...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\cvrptw_optimization\single_depot_column_generation_pulp.py
# Compiled at: 2020-04-20 03:46:31
...
def initiate_single_depot_column_generation(depots, customers, transportation_matrix, vehicles):
    """
    ...
    model_formulation = formulation.ColumnGenerationFormulation(model_inputs.time_variables_dict, model_inputs.assignment_variables_dict, model_inputs.vertices_dict, model_inputs.customers_dict, model_inputs.transit_dict, model_inputs.transit_starting_customers_dict, depot_name)
    return (
     model_inputs, model_formulation)
...
def run_single_depot_column_generation(depots, customers, transportation_matrix, vehicles, capacity, mip_gap=0.001, solver_time_limit_minutes=10, enable_solution_messaging=0, solver_type='PULP_CBC_CMD', max_iteration=50):
    """
    ...
    :return:
    """
    model_inputs, model_formulation = initiate_single_depot_column_generation(depots, customers, transportation_matrix, vehicles)
    ...
    while True:
        ...
        paths_customers_dict = model_inputs.calculate_path_customer_allocation(paths_dict, list(model_inputs.customers_dict['DEMAND'].keys()))
        model_name = str(iteration) + 'MASP'
        price, solution_master_model_objective, solution_master_path = model_formulation.formulate_and_solve_master_problem(paths_dict,
          paths_cost_dict,
        ...
        solution_objective, solution_path, sub_model = model_formulation.formulate_and_solve_subproblem(price, capacity,
          path_name,
          lp_file_name=None,
          bigm=1000000,
          mip_gap=mip_gap,
    ...
    final_price, final_solution_master_model_objective, final_solution_master_path = model_formulation.formulate_and_solve_master_problem(paths_dict,
      paths_cost_dict,
      paths_customers_dict,
      binary_model=True,
      lp_file_name=None,
      ...
      enable_solution_messaging=enable_solution_messaging,
      solver_type=solver_type)
    solution = process_paths(final_solution_master_path, model_inputs.transit_dict, model_inputs.customers_dict, model_inputs.vertices_dict)
    return solution