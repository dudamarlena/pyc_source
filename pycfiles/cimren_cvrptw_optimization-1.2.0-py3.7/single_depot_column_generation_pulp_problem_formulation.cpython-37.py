...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\cvrptw_optimization\src\single_depot_column_generation_pulp_problem_formulation.py
# Compiled at: 2020-04-20 03:16:45
# Size of source mod 2**32: 13593 bytes
"""
...
class ColumnGenerationFormulation:
    ...
    def __init__(self, time_variables_dict, assignment_variables_dict, vertices_dict, customers_dict, transit_dict, transit_starting_customers_dict, depot_name):
    ...
    def formulate_and_solve_master_problem(self, paths_dict, paths_cost_dict, paths_customers_dict, number_of_paths=None, binary_model=False, lp_file_name=None, mip_gap=0.001, solver_time_limit_minutes=10, enable_solution_messaging=1, solver_type='PULP_CBC_CMD'):
        """
        ...
        for customer in self.customers_dict['DEMAND'].keys():
            master_model += (
             pulp.lpSum([paths_customers_dict[(path, customer)] * path_var[path] for path in paths_dict.keys()]) == 1, 'Customer' + str(customer))

        if number_of_paths is not None:
            master_model += (
             pulp.lpSum([path_var[path] for path in paths_dict.keys()]) == number_of_paths, 'No of Vehicles')
        if lp_file_name is not None:
            master_model.writeLP('{}.lp'.format(str(lp_file_name)))
        if solver_type == 'PULP_CBC_CMD':
            master_model.solve(PULP_CBC_CMD(msg=enable_solution_messaging,
        ...
        if master_model.status == 1:
            ...
            for path in path_var.keys():
                if path_var[path].value() > 0:
                    solution_master_path.append({'PATH_NAME':path,  'VALUE':path_var[path].value(), 
            ...
            return (
             price, solution_master_model_objective, solution_master_path)
        raise Exception('No Solution Exists')

    def formulate_and_solve_subproblem(self, price, capacity, path_name, lp_file_name=None, bigm=1000000, mip_gap=0.001, solver_time_limit_minutes=10, enable_solution_messaging=1, solver_type='PULP_CBC_CMD'):
        """
        ...
        sub_model += pulp.lpSum((self.transit_dict['TRANSPORTATION_COST'][(from_loc, to_loc)] * assignment_var[(from_loc, to_loc)] for from_loc, to_loc in self.assignment_variables_dict.keys())) - pulp.lpSum((price[from_loc] * assignment_var[(from_loc, to_loc)] for from_loc, to_loc in objective_keys))
        sub_model += (
         pulp.lpSum([assignment_var[(self.depot_leave, customer)] for customer in self.customers_dict['DEMAND'].keys()]) == 1, 'entryDepotConnection')
        for customer in self.customers_dict['DEMAND'].keys():
            incoming_arcs = []
            outgoing_arcs = []
            for from_loc, to_loc in self.assignment_variables_dict.keys():
            ...
            sub_model += (
             pulp.lpSum([assignment_var[(from_loc, customer)] for from_loc in incoming_arcs]) - pulp.lpSum([assignment_var[(customer, to_loc)] for to_loc in outgoing_arcs]) == 0,
             'forTrip' + str(customer))

        sub_model += (
         pulp.lpSum([assignment_var[(customer, self.depot_enter)] for customer in self.customers_dict['DEMAND'].keys()]) == 1, 'exitDepotConnection')
        sub_model += (
         pulp.lpSum([float(self.customers_dict['DEMAND'][from_loc]) * assignment_var[(from_loc, to_loc)] for from_loc, to_loc in self.transit_starting_customers_dict['DRIVE_MINUTES'].keys()]) <= float(capacity), 'Capacity')
        for from_loc, to_loc in self.assignment_variables_dict.keys():
            stop_time = 0
            if from_loc != self.depot_leave:
                stop_time = self.customers_dict['STOP_TIME'][from_loc]
            sub_model += (
             time_var[to_loc] - time_var[from_loc] >= self.transit_dict['DRIVE_MINUTES'][(from_loc, to_loc)] + stop_time + bigm * assignment_var[(from_loc, to_loc)] - bigm,
             'timewindow' + str(from_loc) + 'p' + str(to_loc))

        for vertex in self.time_variables_dict.keys():
            time_var[vertex].bounds(float(self.vertices_dict['TIME_WINDOW_START'][vertex]), float(self.vertices_dict['TIME_WINDOW_END'][vertex]))

        if lp_file_name is not None:
            sub_model.writeLP('{}.lp'.format(str(lp_file_name)))
        if solver_type == 'PULP_CBC_CMD':
            sub_model.solve(PULP_CBC_CMD(msg=enable_solution_messaging,
              maxSeconds=(60 * solver_time_limit_minutes),
              fracGap=mip_gap))
        if pulp.LpStatus[sub_model.status] in ('Optimal', 'Undefined'):
            ...
            for from_loc, to_loc in self.assignment_variables_dict.keys():
                if assignment_var[(from_loc, to_loc)].value() > 0:
                    solution_assignment.append({'FROM_LOCATION_NAME':from_loc,  'TO_LOCATION_NAME':to_loc, 
            ...
            for loc in time_var.keys():
                if time_var[loc].value() > 0:
                    ...
                    solution_time.append({'LOCATION_NAME':loc,  'START_TIME':time_var[loc].value(), 
            ...
            route.rename(columns={'FROM_LOCATION_NAME':'PREVIOUS_LOCATION_NAME', 
            ...
            return (
             solution_objective, solution_path, sub_model)
