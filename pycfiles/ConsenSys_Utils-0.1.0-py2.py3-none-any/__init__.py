...
class Model(object):
    ...
    def getNeighbors(self, judge, limit=None, threshold=None, neutralScore=0, items=None):
    ...
    def getRecommendations(self, judge, limit=None, neutralScore=0, items=None):
        ...
        return sorted(((i, s) for (i, s) in recommendScores.iteritems()), key=itemgetter(1), reverse=True)[:limit]
...
class DistanceModel(Model):
    ...
    def similarity(self, j1, j2, neutralScore=0, items=None):
        ...
        distance = math.sqrt(sum((pow(self.judgeRatings[j1].get(i, neutralScore) - self.judgeRatings[j2].get(i, neutralScore), 2) for i in items)))
        if not distance:
            return 0
...
class SimpleSimilarityModel(Model):
    ...
    def similarity(self, j1, j2, neutralScore=0, items=None):
        ...
        if items is None:
            items = set(self.judgeRatings[j1]) & set(self.judgeRatings[j2])
        similarityCount = len([ item for item in items if self.judgeRatings[j1][item] > neutralScore if self.judgeRatings[j2][item] > neutralScore ])
...
class SMSimilarityModel(Model):
    ...
    def similarity(self, j1, j2, neutralScore=0, items=None):
        ...
        for item in items:
            top += (self.judgeRatings[j1][item] - neutralScore) * (self.judgeRatings[j2][item] - neutralScore)
...
__all__ = [
 'Model', 'DistanceModel', 'SimpleSimilarityModel', 'SMSimilarityModel']
