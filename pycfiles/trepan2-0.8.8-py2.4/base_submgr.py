    ...
    def help(self, args):
        ...
        if cmd:
            ...
            if doc:
                ...
                self.proc.intf[(-1)].errmsg('Sorry - author mess up. No help registered for subcommand %s of command %s' % (subcmd_name, self.name))
        else:
    ...
    def run(self, args):
        ...
        if subcmd:
            ...
            if nargs < subcmd.min_args:
                self.errmsg(("Subcommand '%s %s' needs at least %d argument(s); " + 'got %d.') % (self.name, subcmd.name, subcmd.min_args, nargs))
                return False
            if subcmd.max_args is not None and nargs > subcmd.max_args:
                self.errmsg(("Subcommand '%s %s' takes at most %d argument(s); " + 'got %d.') % (self.name, subcmd.name, subcmd.max_args, nargs))
                return False
