...
"""
    ...
    def run(self, args):
        ...
        if len(args) == 1:
            try:
                ...
                if frame_num < -i_stack or frame_num > i_stack - 1:
                    self.errmsg(('a frame number number has to be in the range %d to %d.' + ' Got: %d (%s).') % (-i_stack, i_stack - 1, frame_num, args[0]))
                    return False
        ...
        for (name, field) in [('Constants', 'co_consts'), ('Variable names', 'co_varnames'), ('Local Variables', 'co_names')]:
