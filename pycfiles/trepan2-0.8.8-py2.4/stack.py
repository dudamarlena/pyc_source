...
def format_stack_entry(dbg_obj, frame_lineno, lprefix=': ', include_location=True, color='plain'):
    ...
    if '<module>' == funcname and ([], None, None) == (args, varargs, varkw):
        ...
        if is_exec_stmt(frame):
            ...
            s += ' %s(%s)' % (format_token(Mformat.Function, fn_name, highlight=color), source_text)
        else:
...
def print_stack_entry(proc_obj, i_stack, color='plain', opts={}):
    ...
    if frame is proc_obj.curframe:
        ...
        intf.msg_nocr('##')
    intf.msg('%d %s' % (i_stack, format_stack_entry(proc_obj.debugger, frame_lineno, color=color)))
