...
# Compiled at: 2015-06-13 14:48:59
import glob, os, re
from trepan.processor.command import base_cmd as Mbase_cmd
from trepan.processor import cmdproc as Mcmdproc
from trepan.lib import complete as Mcomplete
from trepan import misc as Mmisc
categories = {'breakpoints': 'Making the program stop at certain points', 'data': 'Examining data', 'files': 'Specifying and examining files', 'running': 'Running the program', 'status': 'Status inquiries', 'support': 'Support facilities', 'stack': 'Examining the call stack', 'syntax': 'Debugger command syntax'}
...
"""
        ...
        if len(args) > 1:
            ...
            if command_name:
                ...
                if cmds is None:
                    ...
                    self.msg("Pattern '%s' matches command %s..." % (cmd_name, cmds[0]))
                    args[1] = cmds[0]
                    self.run(args)
                else:
    ...
    def init_syntax_summary_help(self):
        ...
        for name in self.syntax_files():
            path = os.path.join(self.HELP_DIR, '%s%s' % (name, self.RST_EXTENSION))
