...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/trepan/processor/parse/semantics.py
# Compiled at: 2018-10-27 14:00:27
from trepan.processor.parse.parser import parse_bp_location, parse_range, parse_arange
    ...
    def n_arange(self, arange_node):
        ...
        if 1 <= l <= 2:
            ...
            if last_node == 'location':
                ...
                self.result = ListRange(Location(None, None, False, last_node.value[:-2]), None)
                ...
                self.result = ListRange(Location(None, value, is_address, None), None)
            else:
    ...
    def n_range(self, range_node):
        ...
        if 1 <= l <= 2:
            ...
            if last_node == 'location':
                ...
                self.result = ListRange(Location(None, last_node.value, False, None), None)
            else:
                assert last_node == 'DIRECTION'
                self.result = ListRange(None, last_node.value)
            self.prune()
...
def build_bp_expr(string, show_tokens=False, show_ast=False, show_grammar=False):
    parser_debug = {'rules': False, 'transition': False, 'reduce': show_grammar, 'errorstack': None, 'dups': False}
    parsed = parse_bp_location(string, show_tokens=show_tokens, parser_debug=parser_debug)
...
def build_range(string, show_tokens=False, show_ast=False, show_grammar=False):
    parser_debug = {'rules': False, 'transition': False, 'reduce': show_grammar, 'errorstack': None, 'context': False, 'dups': True}
...
def build_arange(string, show_tokens=False, show_ast=False, show_grammar=False):
    parser_debug = {'rules': False, 'transition': False, 'reduce': show_grammar, 'errorstack': None, 'context': True, 'dups': True}
...
if __name__ == '__main__':
    ...
    lines = ('*10', '*10', '/tmp/foo.py:1, *5', '../foo.py:0,  +5', '../foo.py:5,  *30',
             '*0, *10', ', /foo.py:5')
    for line in lines:
        line = line.strip()
        if not line:
