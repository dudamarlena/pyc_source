...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/weresync/utils.py
...
import subprocess, logging, logging.handlers, os, gettext, sys
LOGGER = logging.getLogger(__name__)
LANGUAGES = [
 'en']
DEFAULT_USER_LOG_LOCATION = os.path.expanduser('~/.local/log/weresync/weresync-user.log')

def run_proc(args, target='', error=None, valid_returncodes=[
 0], throw_error=DeviceError):
...
def start_logging_handler(log_loc, stream_level=logging.WARNING, file_level=logging.DEBUG):
...
def check_python_version():
    ...
    try:
        ...
        raise InvalidVersionError('Python version {major}.{minor} not supported. WereSync requires at least Python 3.0\nConsidering installing WereSync with the pip3 command to insure it installs with Python3.'.format(major=(info[0]),
          minor=(info[1])))