...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/weresync/daemon/copier.py
# Compiled at: 2019-06-15 13:43:29
# Size of source mod 2**32: 15438 bytes
...
import random, os, logging, subprocess
...
def mount_loop_device(image_file):
    ...
    if mount_proc.returncode != 0:
        raise DeviceError(image_file, 'Error mounting image on {0}'.format(device_name), str(mount_output, 'utf-8'))
...
def create_new_vg_if_not_exists(lvm, name, target):
    ...
    lvm_part_block = [target.part_mask.format(target.device, x) for x in lvm_partitions]
    lvm_test = subprocess.Popen([
     'vgs', name],
      stdout=(subprocess.PIPE), stderr=(subprocess.STDOUT))
    output, _ = lvm_test.communicate()
    if lvm_test.returncode == 5:
        utils.run_proc(['vgcreate', name] + lvm_part_block, target.device, 'Error creating logical volume group.')
    else:
        if name.startswith('/dev/'):
            name = name[5:]
        result = utils.run_proc([
         'pvs', '-S', 'vg_name=' + name, '--noheadings', '-o', 'pv_name'], name, 'Error finding physical volumes for LVM.')
    ...
    if len(lvm_part_block) > 0:
        utils.run_proc(['vgextend', name] + lvm_part_block, name, 'Error adding PVs to LVM')
...
class DriveCopier(object):
    __doc__ = 'Object which is shared over dbus to allow interfaces to access the\n    daemon.'
    ...
    def CopyDrive(self, source, target, check_if_valid_and_copy=False, source_part_mask='{0}{1}', target_part_mask='{0}{1}', excluded_partitions=[], ignore_copy_failures=True, root_partition=-1, boot_partition=-1, efi_partition=-1, mount_points=('', ''), rsync_args=device.DEFAULT_RSYNC_ARGS, lvm_source='', lvm_target='', bootloader='uuid_copy'):
        ...
        **Note:** if using LVM, any uses of "partition" in the documentation
        ...
        try:
            ...
            if lvm_source is not '':
                create_new_vg_if_not_exists(lvm_source, lvm_source + '-copy', target_manager)
                ...
                if partitions_remade:
                    if check_if_valid_and_copy:
                        copy_partitions(copier, part_callback, lvm=True)
            if mount_points is ('', '') or len(mount_points) < 2 or mount_points[0] == mount_points[1]:
            ...
            try:
                copier.make_bootable(bootloader, mount_points[0], mount_points[1], excluded_partitions, root_partition, boot_partition, efi_partition, boot_callback)
