...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/weresync/daemon/device.py
# Compiled at: 2019-06-14 22:29:48
# Size of source mod 2**32: 77585 bytes
"""This module contains the classes used to modify block devices and copy
data."""
import shlex, os, subprocess, weresync.exception
from weresync.exception import DeviceError, PluginNotFoundError
import math, logging, sys, glob, parse, re, tempfile
MOUNT_POINT = '/mnt'
...
class DeviceManager:
    ...
    def _get_blkid_info(self, partition_num, info_name):
        ...
        if proc.returncode != 0:
            raise DeviceError(self.device, 'Error getting information for partition ' + str(partition_num), str(output, 'utf-8'))
    ...
    def get_partition_table_type(self):
        ...
        for table_type in SUPPORTED_PARTITION_TABLE_TYPES:
            ...
            raise weresync.exception.UnsupportedDeviceError('Partition table type of {0} not supported by WereSync.'.format(self.device))

    def get_drive_size(self):
        """Returns the maximum size of the drive, in sectors."""
        query_proc = subprocess.Popen([
         'blockdev', '--getsz', self.device],
          stdout=(subprocess.PIPE))
    ...
    def get_partition_code(self, partition_num):
        ...
        if table_type == 'gpt':
            ...
            if table_type == 'msdos':
                ...
                result = list(filter(lambda x: x.strip().startswith('Device'), lines))
    ...
    def get_empty_space(self):
        ...
        if table_type == 'gpt':
            ...
            last_part_info = [x for x in result[(-2)].split(' ') if x != '']
            last_sector = int(last_part_info[2])
    ...
    def get_partition_file_system(self, part_num):
        ...
        if result in SUPPORTED_FILESYSTEM_TYPES:
            return result
...
class LVMDeviceManager(DeviceManager):
    ...
    def _get_drive_size_generic(self, units):
        ...
        return int(str(output, 'utf-8').strip()[0:-1])
    ...
    def get_partition_size(self, partition_name):
        ...
        proc = subprocess.Popen([
         'lvdisplay', '-c',
         self.part_mask.format(self.device, partition_name)],
          stdout=(subprocess.PIPE),
          stderr=(subprocess.STDOUT))
        output, error = proc.communicate()
        if proc.returncode != 0:
...
class DeviceCopier:
    ...
    def __init__(self, source, target, lvm_source=None, lvm_target=None):
        ...
        if lvm_source is not None and lvm_target is not None:
            self.lvm_source = lvm_source if isinstance(lvm_source, LVMDeviceManager) else LVMDeviceManager(lvm_source)
            self.lvm_target = lvm_target if isinstance(lvm_target, LVMDeviceManager) else LVMDeviceManager(lvm_target)
        else:
        ...
        if self.uuid_dict is None:
            ...
            if self.lvm_source is not None:
                ...
                for i in self.lvm_source.get_partitions():
                    uuids[lvm_mask.format(source_name, i)] = lvm_mask.format(target_name, i)
    ...
    def _transfer_gpt(self, difference):
        ...
        add_args = []
        type_args = []
        difference -= self.source.get_empty_space() - 34
        for i in reversed(partitions):
            ...
            if space > 0:
                if difference > 0:
                    ...
                    difference += part_size - drive_size
            elif i != partitions[(-1)]:
                add_args = [
                 '-n', '{0}:0:+{1}'.format(i, part_size)] + add_args
            else:
                add_args = [
                 '-n', '{0}:0:0'.format(i)] + add_args
            type_args += [
        ...
        if clear_process.returncode != 0:
            LOGGER.debug('Clear Process error code: {0}'.format(clear_process.returncode))
    ...
    def _transfer_msdos(self, difference, margin=5):
        ...
        for idx, val in enumerate(partition_listings):
            listing = val.split(':')
            pairs = {'part': int(parse.parse(self.source.part_mask.format(self.source.device, '{0}'), listing[0])[0])}
        ...
        final_str = 'unit: sectors\n\n' + ''.join([('{val} : start= {start}, size= {size}, {type_key}= {type}{boot}\n'.format)(val=self.target.part_mask.format(self.target.device, part_line['part']), boot=', bootable' if part_line['bootable'] else '', type_key=id_key, **part_line) for part_line in partition_listings])
    ...
    def _transfer_lvm(self, difference):
        for j in self.lvm_target.get_partitions():
            ...
            if remove_proc.returncode != 0:
                raise weresync.exception.DeviceError(self.lvm_target.device, 'Error removing logical volume from target.', str(output, 'utf-8'))
        ...
        for i in lvs:
            ...
            with tempfile.TemporaryFile() as (tmp):
                copy_proc = subprocess.Popen(command,
                ...
                if copy_proc.returncode != 0:
                    raise weresync.exception.DeviceError(self.lvm_target.device, 'Error creating new logical volume.', str(output, 'utf-8'))
            ...
            try:
                ...
                if callback is not None:
                    ...
                    if callback is not None:
                        ...
                        LOGGER.debug('Callback:\nDrive Size: {0}\nPart Size: {1}\nComplete: {2}'.format(drive_size, part_size, complete))
                        callback(complete)
                else:
                ...
                try:
                    if ignore_errors:
                        logging.getLogger('weresync.device').warning(('Creating filesystem for {0} encountered errors. Partition type: {1}. Skipped.'.format(target_manager.part_mask.format(target_manager.device, i), part_type)),
                          exc_info=(sys.exc_info()))
                        logging.getLogger('weresync.device').debug('Error making file system.',
                          exc_info=(sys.exc_info()))
                    else:
                        raise exe
                finally:
    ...
    def transfer_partition_table(self, resize=True, callback=None):
        ...
        if target_size < source_size:
            raise (resize or weresync.exception.CopyError)('Target device smaller than source device and resize set to false.')
        else:
            if source_type == 'gpt':
                self._transfer_gpt(source_size - target_size)
            else:
                if source_type == 'msdos':
        ...
        progress = lambda prog:         if callback is not None:
callback(0.3 + prog * 0.7) # Avoid dead code: None
    ...
    def partitions_valid(self, lvm=False):
        ...
        for i in source_parts:
            if source_manager.get_partition_file_system(i) != target_manager.get_partition_file_system(i):
                raise weresync.exception.CopyError('File system type for partition {0} does not match. Invalid.'.format(i))
            try:
                if source_manager.get_partition_used(i) > target_manager.get_partition_size(i):
                ...
                try:
                    if 'mount' in str(ex) or 'swapspace' in str(ex):
                        LOGGER.debug(("Partition {0} couldn't be mounted. Bad FS type".format(i)),
                          exc_info=(sys.exc_info))
                    else:
                        raise ex
                finally:
    ...
    def _copy_fstab(self, mnt_source, mnt_target, excluded_partitions=[], lvm=False):
            ...
            try:
                if i not in excluded_partitions:
                    ...
                    if os.path.exists(source_fstab_path):
                        ...
                        with open(source_fstab_path) as (source_fstab):
                            with open(target_loc + ('/' if not target_loc.endswith('/') else '') + 'etc/fstab', 'w') as (target_fstab):
                                    ...
                                    if not stripLine == '':
                                        ...
                                        if words[0].startswith('UUID') or words[0].startswith('LABEL'):
                                            if words[0].startswith('UUID'):
                                                ...
                                                if words[0].startswith('LABEL'):
                                                    blkid_arg = '-L'
                                                identifier = words[0].split('=')[1]
                                                proc = subprocess.Popen([
                                                ...
                                                output, error = proc.communicate()
                                                if proc.returncode == 2:
                                                    raise weresync.exception.DeviceError(self.source.device, 'Could not find block name of device with id: {0}'.format(identifier), str(output, 'utf-8'))
                                                else:
                                                    if proc.returncode != 0:
                                                        raise weresync.exception.DeviceError(source_manager.device, 'Error finding device error for device with id: {0}'.format(identifier), str(output, 'utf-8'))
                                                    out = str(output, 'utf-8').strip()
                                                    if self.lvm_source is not None and self.lvm_source.device in out:
                                                        source = self.lvm_source
                                                        target = self.lvm_target
                                                    else:
                                                        source = self.source
                                                        target = self.target
                                                result = parse.parse(source.part_mask.format(source.device, '{0}'), out)[0]
                                                uuid_proc = subprocess.Popen([
                                                 'blkid',
                                                 source.part_mask.format(target.device, result)],
                                                  stdout=(subprocess.PIPE),
                                                  stderr=(subprocess.STDOUT))
                                                uuid_output, uuid_error = uuid_proc.communicate()
                                                if uuid_proc.returncode == 2:
                                                    raise weresync.exception.DeviceError(target.device, 'Could not find device {0}'.format(target.part_mask.format(target.device, result)), str(uuid_output, 'utf-8'))
                                                else:
                                                    if uuid_proc.returncode != 0:
                                                        raise weresync.exception.DeviceError(target.device, 'Error finding uuid for device {0}'.format(target.part_mask.format(target.device, result)), str(uuid_output, 'utf-8'))
                                            ...
                                            if self.lvm_source is not None:
                                                words[0] = multireplace(words[0], self.get_uuid_dict())
    ...
    def _copy_files(self, mnt_source, mnt_target, excluded_partitions, ignore_failures, rsync_args, callback, lvm=False):
                ...
                try:
                    if i in excluded_partitions:
                        ...
                        if target_loc is None:
                            ...
                            target_mounted = True
                            target_loc = mnt_target
                        LOGGER.info('Starting rsync process for partition {0}.'.format(source_manager.device))
                        command_args = [
                         'rsync'] + shlex.split(rsync_args) + ['--exclude=' + x + '' for x in ('/dev/*',
                                                                                               '/proc/*',
                                                                                               '/sys/*',
                                                                                               '/tmp/*',
                                                                                               '/run/*',
                                                                                               ...
                                                                                               '/lost+found',
                                                                                               '/home/*/.gvfs')] + [
                         source_loc + ('/' if not source_loc.endswith('/') else ''), target_loc]
                        if callback is not None:
                    ...
                    try:
                        if ignore_failures:
                            LOGGER.warning('Error copying data for partition {0} from device {1} to {2}.'.format(i, source_manager.device, target_manager.device))
    ...
    def copy_files(self, mnt_source, mnt_target, excluded_partitions=[], ignore_failures=True, rsync_args=DEFAULT_RSYNC_ARGS, callback=None):
        ...
        self._copy_files(mnt_source, mnt_target, excluded_partitions, ignore_failures,
          rsync_args, callback, lvm=False)
        if self.lvm_source is not None:
            self._copy_files(mnt_source, mnt_target, excluded_partitions, ignore_failures,
              rsync_args, callback, lvm=True)
        ...
        try:
            if plugin_name is not None:
                ...
                if pluginInfo is None:
                    raise PluginNotFoundError('No such plugin {0}'.format(full_name))
            ...
            if self.lvm_source is not None:
                try:
                    ...
                    try:
                        LOGGER.warning('Error copying fstab on LVM. Continuing anyway.')
                        LOGGER.debug('Info: ', exc_info=(sys.exc_info()))
                    finally:
                        ex = None
                        del ex
                ...
                LOGGER.warning('No bootloader plugin specified. Not installingbootloader.')
            if callback is not None:
                callback(True)
        except DeviceError as ex:
            try:
