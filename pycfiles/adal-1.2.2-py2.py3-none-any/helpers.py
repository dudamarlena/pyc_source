...
# Embedded file name: /opt/adagios/adagios/../adagios/misc/helpers.py
...
def change_service_attribute(identifier, new_value):
    ...
    if len(tmp) != 3:
        raise ValueError(_('identifier must be in the form of host_name::service_description::attribute_name (got %s)') % identifier)
...
def check_command(host_name, service_description, name=None, check_command=None, **kwargs):
    ...
    elif service_description in ('None', None, '', ''):
