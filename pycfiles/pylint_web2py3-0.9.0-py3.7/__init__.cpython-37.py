...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/pylint_web2py3/__init__.py
# Compiled at: 2019-05-11 06:20:09
...
def web2py_transform(module):
    ...
    if module.file:
        web2py_match = re.match('(.+?)/applications/(.+?)/(.+?)/', module.file)
        if web2py_match:
...
Add models path too to be able to import it from the fake code
        ...
        if not self.is_pythonpath_modified:
            ...
            app_models_path = join(web2py_path, 'applications', app_name, 'models')
    ...
    def _fill_app_model_names(self, app_models_path):
        ...
        model_files = [model_file for model_file in model_files if re.match('.+?\\.py$', model_file)]
        model_files = sorted(model_files)
        self.app_model_names = [re.match('^(.+?)\\.py$', model_file).group(1) for model_file in model_files]
...
and then use them to remove unused imports.
        ...
        for name in sniffer.unused:
            if name in fake_node.globals and name in module_node.locals:
...
class MessageSniffer(PyLinter):
    __doc__ = 'Special class to mimic PyLinter to intercept messages from checkers. Here we use it to collect info about unused imports and singleton comparsons in db queries'
    ...
    def add_message(self, msg_descr, line=None, node=None, args=None, confidence=UNDEFINED, col_offset=None):
        ...
        if msg_descr == 'unused-wildcard-import':
            ...
            if msg_descr == 'unused-import':
                sym_match = re.match('^(.+?)\\ imported\\ from', args)
                if sym_match:
                    sym_name = sym_match.group(1)
                else:
                    module_match = re.match('^import\\ (.+?)$', args)
                    assert module_match
