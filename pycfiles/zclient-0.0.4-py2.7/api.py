...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/zclient/zenoss/api.py
# Compiled at: 2012-02-10 13:22:17
import json, urllib, urllib2
ROUTERS = {'MessagingRouter': 'messaging', 'EventsRouter': 'evconsole', 
...
class ZenossAPI:
    ...
    def __init__(self, login, debug=False):
        ...
        self.reqCount = 1
        self.login = login
        loginParams = urllib.urlencode(dict(__ac_name=self.login['username'], __ac_password=self.login['password'], submitted='true', came_from=self.login['url'] + '/zport/dmd'))
        self.urlOpener.open(self.login['url'] + '/zport/acl_users/cookieAuthHelper/login', loginParams)

    def _router_request(self, router, method, data=[]):
        if router not in ROUTERS:
            raise Exception('Router "' + router + '" not available.')
        req = urllib2.Request(self.login['url'] + '/zport/dmd/' + ROUTERS[router] + '_router')
        req.add_header('Content-type', 'application/json; charset=utf-8')
        reqData = json.dumps([
         dict(action=router, method=method, data=data, type='rpc', tid=self.reqCount)])
        self.reqCount += 1
        return json.loads(self.urlOpener.open(req, reqData).read())

    def get_devices(self, deviceClass='/zport/dmd/Devices', limit=50):
        return self._router_request('DeviceRouter', 'getDevices', data=[{'uid': deviceClass, 'params': {}}])['result']

    def get_devices_by_name_filter(self, name, deviceClass='/zport/dmd/Devices', limit=50):
        return self._router_request('DeviceRouter', 'getDevices', data=[{'uid': deviceClass, 'params': {'name': name}}])['result']
    ...
    def create_event_on_device(self, event_filter):
        data = dict(device=None, summary='Event created by zclient', severity='Critical', component='', evclasskey='', evclass='')
        for k, v in event_filter.iteritems():
            try:
                data[k] = v
            except KeyError:
        ...
        if data['severity'] not in ('Critical', 'Error', 'Warning', 'Info', 'Debug',
                                    'Clear'):
