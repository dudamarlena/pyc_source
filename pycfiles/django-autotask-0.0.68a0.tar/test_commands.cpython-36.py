...
# [GCC 8.4.0]
# Embedded file name: /home/cameron/Dev/kanban-dev/django-autotask/djautotask/tests/test_commands.py
# Compiled at: 2020-04-16 20:47:33
# Size of source mod 2**32: 18293 bytes
import io
...
def sync_summary(class_name, created_count, updated_count=0):
    return '{} Sync Summary - Created: {}, Updated: {}'.format(class_name, created_count, updated_count)


def full_sync_summary(class_name, deleted_count, updated_count=0):
    return '{} Sync Summary - Created: 0, Updated: {}, Deleted: {}'.format(class_name, updated_count, deleted_count)
...
class AbstractBaseSyncTest(object):
    ...
    def get_return_value(self, at_object, fixture_list):
        return fixture_utils.generate_objects(at_object.title().replace('_', ''), fixture_list)
...
class AbstractPicklistSyncCommandTest(AbstractBaseSyncTest):
    ...
    def get_return_value(self, at_object, fixture_list):
        field_info = fixture_utils.generate_picklist_objects(self.field_name, fixture_list)
        return field_info
...
class TestSyncServiceCallStatusCommand(AbstractPicklistSyncCommandTest, TestCase):
    field_name = 'Status'
    args = (
...
class TestSyncAllCommand(TestCase):
    ...
    def setUp(self):
        ...
        mocks.create_mock_call('djautotask.sync.QueryConditionMixin._get_query_conditions', None)
        mocks.get_field_info_api_calls(fixture_utils.manage_sync_picklist_return_data)
        mocks.wrapper_query_api_calls(fixture_utils.manage_full_sync_return_data)
        sync_test_cases = [
    ...
    def test_partial_sync(self):
        ...
        self.assertEqual(models.Ticket.objects.all().count(), len(fixtures.API_TICKET_LIST))

    def test_full_sync(self):
        """Test the command to run a full sync of all objects."""
        at_object_map = {'status':models.Status, 
        ...
        for fixture, at_object in self.test_args:
            if at_object in ('resource_role_department', 'resource_service_desk_role',
                             'service_call', 'service_call_ticket', 'service_call_task',
                             'service_call_ticket_resource', 'service_call_task_resource'):
                self.assertGreater(pre_full_sync_counts[at_object], 0)
                pre_full_sync_counts[at_object] = 0
            summary = full_sync_summary(slug_to_title(at_object), pre_full_sync_counts[at_object])
            self.assertIn(summary, output.getvalue().strip())
