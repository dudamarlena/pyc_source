...
# [GCC 8.4.0]
# Embedded file name: /home/cameron/Dev/kanban-dev/django-autotask/djautotask/management/commands/atsync.py
...
ERROR_MESSAGE_TEMPLATE = 'Failed to sync {}. Autotask API returned an error(s): {}.'
...
class Command(BaseCommand):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for name, synchronizer, obj_name in synchronizers:
            self.synchronizer_map[name] = (
             synchronizer, obj_name)
    ...
    def sync_by_class(self, sync_class, obj_name, full_option=False):
        ...
        if full_option:
            ...
            fmt_msg = msg.format(obj_name, created_count, updated_count, deleted_count)
    ...
    def handle(self, *args, **options):
        ...
        if autotask_object_arg:
            ...
            if sync_tuple:
                ...
                msg = _('Invalid AT object {}, choose one of the following: \n{}')
        ...
        for sync_class, obj_name in sync_classes:
            ...
            try:
                try:
                    self.sync_by_class(sync_class, obj_name, full_option=full_option)
                except AutotaskProcessException as e:
                    error_msg = ERROR_MESSAGE_TEMPLATE.format(obj_name, api.parse_autotaskprocessexception(e))
                except AutotaskAPIException as e:
                    error_msg = ERROR_MESSAGE_TEMPLATE.format(obj_name, api.parse_autotaskapiexception(e))
                except SAXParseException as e:
                    error_msg = 'Failed to connect to Autotask API. The error was: {}'.format(e)
        ...
        if failed_classes > 0:
            msg = '{} class{} failed to sync.\n'.format(failed_classes, '' if failed_classes == 1 else 'es')
            msg += 'Errors:\n'
            msg += error_messages
            raise CommandError(msg)