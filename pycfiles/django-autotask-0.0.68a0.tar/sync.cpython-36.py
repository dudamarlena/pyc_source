...
# [GCC 8.4.0]
# Embedded file name: /home/cameron/Dev/kanban-dev/django-autotask/djautotask/sync.py
# Compiled at: 2020-04-16 20:47:33
# Size of source mod 2**32: 42036 bytes
import logging
...
class Synchronizer:
    ...
    def _instance_ids(self, filter_params=None):
        if not filter_params:
            ids = self.model_class.objects.all().order_by(self.db_lookup_key).values_list('id',
              flat=True)
        else:
            ids = self.model_class.objects.filter(filter_params).order_by(self.db_lookup_key).values_list('id',
              flat=True)
        return set(ids)
...
class BatchQueryMixin:
    ...
    def _build_fk_batch(self, model_class, object_id_field, sync_job_qset):
        ...
        """
        queries = []
        object_ids = list(model_class.objects.order_by(self.db_lookup_key).values_list('id',
          flat=True))
        while object_ids:
            query = self.build_base_query(sync_job_qset)
            query.open_bracket('AND')
...
class TicketSynchronizer(QueryConditionMixin, Synchronizer, ParentSynchronizer):
    ...
    related_meta = {'Status':(
     ...
     'AssignedResourceID':(
      models.Resource, 'assigned_resource'), 
     'Priority':(
     ...
     'TicketCategory':(
      models.TicketCategory, 'category'), 
     'TicketType':(
      models.TicketType, 'type'), 
     'Source':(
      models.Source, 'source'), 
     'IssueType':(
      models.IssueType, 'issue_type'), 
     'SubIssueType':(
      models.SubIssueType, 'sub_issue_type'), 
     'AssignedResourceRoleID':(
      models.Role, 'assigned_resource_role'), 
     'AllocationCodeID':(
      models.AllocationCode, 'allocation_code'), 
     'ContractID':(
...
class ResourceSynchronizer(Synchronizer):
    ...
    last_updated_field = None
    related_meta = {'LicenseType':(
      models.LicenseType, 'license_type'), 
     'DefaultServiceDeskRoleID':(
      models.Role, 'default_service_desk_role')}
...
class ProjectSynchronizer(FilterProjectStatusMixin, Synchronizer):
    ...
    related_meta = {'ProjectLeadResourceID':(
      models.Resource, 'project_lead_resource'), 
     'AccountID':(
      models.Account, 'account'), 
     'Status':(
...
class PhaseSynchronizer(Synchronizer):
    ...
    last_updated_field = 'LastActivityDateTime'
    related_meta = {'ProjectID':(
      models.Project, 'project'), 
     'ParentPhaseID':(
      models.Phase, 'parent_phase')}
...
class TaskSynchronizer(QueryConditionMixin, FilterProjectStatusMixin, Synchronizer):
    ...
    related_meta = {'AssignedResourceID':(
      models.Resource, 'assigned_resource'), 
     'ProjectID':(
      models.Project, 'project'), 
     'PhaseID':(
    ...
    def get_active_ids(self):
        active_projects = models.Project.objects.exclude(Q(status__is_active=False) | Q(status__label=(models.ProjectStatus.COMPLETE))).values_list('id',
          flat=True).order_by(self.db_lookup_key)
...
class TicketNoteSynchronizer(BatchQueryMixin, NoteSynchronizer, ChildSynchronizer):
    ...
    related_meta = {'NoteType':(
      models.NoteType, 'note_type'), 
     'CreatorResourceID':(
      models.Resource, 'creator_resource'), 
     'TicketID':(
      models.Ticket, 'ticket')}
...
class ServiceCallSynchronizer(Synchronizer):
    ...
    last_updated_field = 'LastModifiedDateTime'
    related_meta = {'AccountID':(
      models.Account, 'account'), 
     'Status':(
      models.ServiceCallStatus, 'status'), 
     'CreatorResourceID':(
      models.Resource, 'creator_resource'), 
     'CanceledByResource':(
      models.Resource, 'canceled_by_resource')}
...
class ServiceCallTicketSynchronizer(BatchQueryMixin, Synchronizer):
    ...
    related_meta = {'ServiceCallID':(
      models.ServiceCall, 'service_call'), 
     'TicketID':(
      models.Ticket, 'ticket')}
