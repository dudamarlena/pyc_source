...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/musixmatch/api.py
# Compiled at: 2011-08-11 05:06:45
...
class ResponseStatusCode(int):
    ...
    """
    __status__ = {200: 'The request was successful.', 
    ...
    *['header']['status_code']* item.
    ...
    def __init__(self, response):
        try:
            ...
            raise ResponseMessageError('Invalid Json response message', e)
...
class Request(object):
    ...
    **query_string** is a :py:class:`dict`, try to cast it into a
    ...
    def __init__(self, api_method, query=(), **keywords):
        self.__api_method = isinstance(api_method, Method) and api_method or Method(api_method)
        self.__query_string = isinstance(query, QueryString) and query or QueryString(query)
    ...
    def response(self):
        ...
        if self.__response is None:
            ...
            ResponseMessageClass = {'json': JsonResponseMessage, 
               'xml': XMLResponseMessage}.get(format, None)
    ...
    def __str__(self):
        return '%(ws_location)s/%(api_method)s?%(query_string)s' % {'ws_location': musixmatch.ws.location, 
