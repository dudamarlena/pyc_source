...
# [GCC 8.4.0]
# Embedded file name: /home/rnix/workspace/touch.server/devsrc/yafowil.widget.array/src/yafowil/widget/array/widget.py
# Compiled at: 2019-04-25 01:14:58
...
factory.register('array_display_proxy', extractors=[
 generic_extractor], edit_renderers=[
 array_display_proxy_renderer], display_renderers=[
 array_display_proxy_renderer])
factory.doc['blueprint']['array_display_proxy'] = UNSET
ICON_CSS = {'add': 'icon-plus-sign', 
...
factory.register('array_actions', edit_renderers=[
 actions_renderer])
...
def array_builder(widget, factory):
    ...
    label = widget.attrs.get('label', ' ')
    if callable(label):
        label = label()
    row['label'] = factory('th', props={'structural': True, 
       'label': label})
...
def array_wrapper_renderer(widget, data):
    ...
    kw = {'id': cssid(widget, 'array'), 'class': cssclasses(widget, data) + add_css}
...
def array_edit_renderer(widget, data):
    if len(widget) == 1 and CONTAINER not in widget:
        raise Exception('Empty array widget defined')
...
def create_array_children(widget, template, value):
    if isinstance(value, dict):
        try:
            ...
            raise Exception('Array value error. %s' % str(e))
        ...
        raise ValueError(("Expected list or dict as value. Got '{}'").format(str(type(value))))
...
def hook_array_template(widget, template):
    ...
    if template.attrs.get('structural'):
        raise Exception('Compound templates for arrays must not be structural.')
...
def duplicate_widget(widget, value=UNSET):
    ...
    leaf = len(widget) == 0
    if leaf and (widget.mode == 'display' or widget.attrs.get('disabled')) and 'array_display_proxy' not in blueprints:
        if not widget.attrs.get('display_proxy'):
            blueprints.insert(0, 'array_display_proxy')
    return factory(blueprints, value=value, props=widget.properties, custom=widget.custom, mode=widget.mode)
...
factory.register('array', extractors=[
 array_extractor, generic_required_extractor], edit_renderers=[
 array_edit_renderer, compound_renderer, array_wrapper_renderer], display_renderers=[
 array_display_renderer, compound_renderer], builders=[
 array_builder])
factory.doc['blueprint']['array'] = 'Add-on widget `yafowil.widget.array\n<http://github.com/bluedynamics/yafowil.widget.array/>`_.\n'
...
factory.doc['props']['array.table_class'] = 'CSS classes rendered on array table.\n'
factory.defaults['array.static'] = False
factory.doc['props']['array.static'] = 'Array is immutable. No Array actions are rendered. If True, ``add``,\n``remove`` and ``sort`` properties are ignored.\n'
factory.defaults['array.add'] = True
factory.doc['props']['array.add'] = 'Render ``add`` action.\n'
factory.defaults['array.remove'] = True
factory.doc['props']['array.remove'] = 'Render ``remove`` action.\n'
factory.defaults['array.sort'] = True
factory.doc['props']['array.sort'] = 'Render ``move up`` and ``move down`` actions.\n'