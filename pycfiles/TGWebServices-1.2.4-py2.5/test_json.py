...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tgwebservices/tests/test_json.py
# Compiled at: 2010-06-21 05:36:20
...
def test_nested_top():
    ...
    testutil.create_request('/ultimatequestion', headers=dict(Accept='application/json'))
    output = cherrypy.response.body[0]
    print output
...
def test_complex_input():
    ...
    testutil.create_request('/tenyearsolder', rfile=StringIO.StringIO(request), method='POST', headers={'Content-Length': str(len(request)), 'Content-Type': 'application/json; charset=utf-8'})
    output = cherrypy.response.body[0]
    print output
...
def test_complex_input_on_get():
    ...
    testutil.create_request('/tenyearsolder?tg_format=json&_json_request=%s' % request)
    output = cherrypy.response.body[0]
    print output
    print cherrypy.root.last_person.age
    assert cherrypy.root.last_person.age == 32
...
def test_complex_params_on_get():
    cherrypy.root = ComplexService('http://foo.bar.baz/')
    person = '{"name":"Fred","age":22}'
    testutil.create_request('/tenyearsolder?tg_format=json&person=%s' % person)
    output = cherrypy.response.body[0]
    print output
    print cherrypy.root.last_person.age
    assert cherrypy.root.last_person.age == 32
...
def test_rwprop():
    ...
    testutil.create_request('/getandsetrwprop', rfile=StringIO.StringIO(request), method='POST', headers={'Content-Length': str(len(request)), 'Content-Type': 'application/json; charset=utf-8', 
...
def test_binary():
    ...
    print request
    testutil.create_request('/reverse', rfile=StringIO.StringIO(request), method='POST', headers={'Content-Length': str(len(request)), 'Content-Type': 'application/json; charset=utf-8', 
       'Accept': 'application/json'})
    output = cherrypy.response.body[0]
    print output
