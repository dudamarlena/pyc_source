...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tgwebservices/soap.py
# Compiled at: 2010-06-21 06:21:42
"""Implements the SOAP protocol. Specifically, this implementation talks in
the wrapped document/literal form."""
import re, sys, traceback, time, logging, datetime
...
from turbogears import controllers, expose as tgexpose, error_handler, exception_handler, validators
from tgwebservices.runtime import primitives, ctvalues, typedproperty, unsigned, binary
log = logging.getLogger('tgwebservices.soap')
namespace_expr = re.compile('^\\{.*\\}')
...
def soap_instance(name, return_type, value):
    ...
    for key in ctvalues(cls):
        elem.append(soap_value(key, getattr(cls, key), getattr(value, key)))
...
def soap_list(name, return_type, value):
    elem = Element(name, **{'xsi:type': soap_type(return_type).replace('types:', '')})
...
type_registry = {basestring: 'xsd:string', 
   str: 'xsd:string', 
   unicode: 'xsd:string', 
   int: 'xsd:int', 
   long: 'xsd:long', 
...
array_registry = {basestring: 'String_Array', 
   str: 'String_Array', 
...
class SoapController(controllers.Controller):
    ...
    @tgexpose('genshi:tgwebservices.templates.soap', fragment=True, content_type='application/soap+xml', format='xml')
    def index(self):
        ...
        try:
            ...
            raise validators.Invalid('Unknown SOAP method: %s' % methodname, methodname, None)

        params = {'_tgws': True}
        params['xml_body'] = body
        assert hasattr(method, 'exposed') and method.exposed == True
        output = method(**params)
        data = dict(baseURL=self.wscontroller._ws_baseURL, tns=self.tns, typenamespace=self.typenamespace, soap_body=soap_body, output=output, methodname=methodname, function_info=self.wscontroller._ws_funcs[methodname]._ws_func_info)
        return data

    @tgexpose('genshi:tgwebservices.templates.wsdl', format='xml', content_type='text/xml')
    def api_wsdl(self, xmlsoap=False):
        ...
        ct.sort(key=lambda item: (item._type_dependents, item.__name__), reverse=True)
        ...
        return dict(service_name=self.wscontroller.__class__.__name__, baseURL=self.wscontroller._ws_baseURL, registry=self.wscontroller._ws_funcs, soap_type=soap_type, funclist=funclist, soap_array=handle_list, complex_types=ct, ctvalues=ctvalues, arrays=self.arrays, soapenc=soapenc, tns=self.tns, typenamespace=self.typenamespace)
