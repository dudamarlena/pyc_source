...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tgwebservices/tests/test_soap.py
# Compiled at: 2010-06-21 06:19:31
import re, httplib, cStringIO as StringIO, mimetools, base64
...
def run_soap(method, params=''):
    ...
    print request
    testutil.create_request('/soap/', rfile=StringIO.StringIO(request), method='POST', headers={'Content-Length': str(len(request)), 'Content-Type': 'application/soap+xml; charset=utf-8'})
...
def test_basic_call():
    ...
    print 'result:', elem
    assert elem.attrib['{http://www.w3.org/2001/XMLSchema-instance}type'] == 'xsd:int', 'Expected result to be labeled an int'
...
def test_wsdl_with_complex():
    ...
    found = False
    for elem in schema_elements:
        if 'name' in elem.attrib and elem.attrib['name'] == 'getfancyResponse':
            found = True
            interior_elem = elem[0][0][0]
            print 'response value: %s (%s)' % (interior_elem,
             interior_elem.attrib)
    ...
    for elem in complex_types:
        if 'name' in elem.attrib and elem.attrib['name'] == 'FancyValue':
            found = True
            ...
            for attr in objattributes:
                if attr.attrib['name'] == 'name':
                    ...
                    assert False, "Found a %s element which shouldn't be there" % attr.attrib['name']
        ...
        elif 'name' in elem.attrib and elem.attrib['name'] == 'FoodItem_Array':
            found_array = True
...
def test_soap_array():
    ...
    print output
    root = et.fromstring(output)
    schema_elements = root.findall('.//{http://www.w3.org/2001/XMLSchema}complexType')
    found = False
    stringarray_found = False
    for elem in schema_elements:
        if 'name' in elem.attrib:
            if elem.attrib['name'] == 'FancyValue_Array':
                found = True
                interior_elem = elem[0]
            else:
                raise interior_elem.tag == '{http://www.w3.org/2001/XMLSchema}sequence' or AssertionError, 'Expected to find a sequence in FancyValue_Array'
            interior_elem = interior_elem[0]
            assert interior_elem.tag == '{http://www.w3.org/2001/XMLSchema}element', 'Expected to find element in FancyValue_Array'
            print interior_elem.attrib
            assert interior_elem.attrib['maxOccurs'] == 'unbounded', 'expected unbounded array'
            assert interior_elem.attrib['nillable'] == 'true', 'Array should be nillable'
            assert interior_elem.attrib['type'] == 'types:FancyValue', 'Should have found array type declaration of FancyValue'
        if 'name' in elem.attrib and elem.attrib['name'] == 'String_Array':
            stringarray_found = True
            interior_elem = elem[0][0]
            assert interior_elem.tag == '{http://www.w3.org/2001/XMLSchema}element', 'Expected to find element in String_Array'
            print interior_elem.attrib
            assert interior_elem.attrib['type'] == 'xsd:string', 'Should have found string type for String_Array'
...
def test_complex_types_stay_in_one_controller():
    ...
    schema_elements = root.findall('.//{http://www.w3.org/2001/XMLSchema}complexType')
    for elem in schema_elements:
        assert not ('name' in elem.attrib and elem.attrib['name'] == 'FoodOrder'), 'Complex type definitions should be limited to controllers that use them'
...
def test_nested_controllers():
    ...
    op_elements = porttype.findall('.//{http://schemas.xmlsoap.org/wsdl/}operation')
...
def test_documentation_strings():
    ...
    docelems = operation.findall('.//{http://schemas.xmlsoap.org/wsdl/}documentation')
...
def test_big_number():
    elem = soap.soap_value('test', int, 3063485326)
    output = str(elem)
        ...
        if 'name' in elem.attrib and elem.attrib['name'] == 'SubFood':
            for child in elem.getchildren()[0].getchildren():
                if child.attrib['name'] == 'person':
                    ...
                    assert child.attrib['type'] == 'types:FancyValue', 'SubFood person should be a FancyValue'
...
def test_explicit_namespace():
    cherrypy.root = NamespacedRoot('https://foo.bar.baz/', 'http://foo.bar/service/URI/')
    output = get_wsdl()
    print output
