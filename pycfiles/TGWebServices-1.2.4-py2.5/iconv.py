...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tgwebservices/iconv.py
# Compiled at: 2010-06-21 07:44:57
...
import decimal, datetime, base64
namespace_expr = re.compile('^\\{.*\\}')
date_re = '(?P<year>-?\\d{4,})-(?P<month>\\d{2})-(?P<day>\\d{2})'
time_re = '(?P<hour>\\d{2}):(?P<min>\\d{2}):(?P<sec>\\d{2})(\\.(?P<sec_frac>\\d+))?'
tz_re = '((?P<tz_sign>[+-])(?P<tz_hour>\\d{2}):(?P<tz_min>\\d{2}))|(?P<tz_z>Z)'
...
def boolean_converter(value):
    ...
    if value in ('true', '1', 'yes', 'on', 'y', 't'):
        return True
    raise validators.Invalid('%s is not a legal boolean value' % value, value, None)
    return
...
def date_converter(value):
    ...
    if m is None:
        raise validators.Invalid('%s is not a legal date value' % value, value, None)
    try:
        return datetime.date(int(m.group('year')), int(m.group('month')), int(m.group('day')))
    except ValueError, e:
        raise validators.Invalid('%s is a out-of-range date' % value, value, None)
...
def time_converter(value):
    ...
    if m is None:
        raise validators.Invalid('%s is not a legal time value' % value, value, None)
    try:
        ms = 0
        if m.group('sec_frac') is not None:
            ...
            ms = int(f[2:])
        return datetime.time(int(m.group('hour')), int(m.group('min')), int(m.group('sec')), ms)
    except ValueError, e:
        raise validators.Invalid('%s is a out-of-range time' % value, value, None)
...
def datetime_converter(value):
    ...
    if m is None:
        raise validators.Invalid('%s is not a legal datetime value' % value, value, None)
    try:
        ms = 0
        if m.group('sec_frac') is not None:
            f = decimal.Decimal('0.' + m.group('sec_frac'))
            ...
            ms = int(str(f)[2:])
        return datetime.datetime(int(m.group('year')), int(m.group('month')), int(m.group('day')), int(m.group('hour')), int(m.group('min')), int(m.group('sec')), ms)
    except ValueError, e:
        raise validators.Invalid('%s is a out-of-range datetime' % value, value, None)
...
def _get_single_value(elem, itype):
    ...
    if elem.get('nil', None) == 'true':
        return
    if elem.get('{http://www.w3.org/2001/XMLSchema-instance}nil', None) == 'true':
        return
    ...
    try:
        ...
        raise validators.Invalid("%s value for the '%s' parameter is not a valid %s" % (
         text, namespace_expr.sub('', elem.tag),
         itype.__name__), text, None)
...
def handle_xml_params(body, input_types):
    ...
    for elem in body.getchildren():
        ...
        try:
            ...
            raise validators.Invalid('%s is not a valid parameter (valid values are: %s)' % (
    ...
    for key in kw:
        if key in input_types:
            try:
                ...
                raise validators.Invalid("%s value for the '%s' parameter is not a valid %s" % (
                 kw[key], key,
                 input_types[key].__name__), kw[key], None)
