...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/webmaster/webmaster/cli.py
# Compiled at: 2016-01-19 21:46:45
"""
Webmaster
...
"""
import os, re, sys, logging, importlib, pkg_resources, utils, __about__, click, yaml, subprocess
CWD = os.getcwd()
...
def copy_resource(src, dest):
    ...
    if pkg_resources.resource_isdir('webmaster', src):
...
def create_project(project_name, skel='basic'):
    ...
    files = [('%s/__init__.py' % APPLICATION_DIR, '# Webmaster'),
     (
...
def create(project, template):
    ...
    click.echo('- Launch local server, run [ webcli local -p %s ]' % app)
    click.echo('')
