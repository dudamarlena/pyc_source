...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/web-portfolio/webmaster/ext.py
# Compiled at: 2016-01-07 02:41:13
"""
Extras
Contains some
"""
...
import utils, humanize, markdown_ext, flask_cloudy, flask_recaptcha, flask_seasurf, flask_kvsession, flask_cache, flask_login, ses_mailer, flask_mail, flask_s3
from webmaster.packages import base
__all__ = [
 'mailer',
 'cache',
 'storage',
...
def _setup(app):
    ...
    sanity_check()
    View.g(APPLICATION_NAME=app.config.get('APPLICATION_NAME'), APPLICATION_VERSION=app.config.get('APPLICATION_VERSION'), APPLICATION_URL=app.config.get('APPLICATION_URL'), APPLICATION_GOOGLE_ANALYTICS_ID=app.config.get('APPLICATION_GOOGLE_ANALYTICS_ID'))
    ...
    def init_app(self, app):
        ...
        if mailer_uri:
            ...
            if 'ses' in scheme:
                ...
                self.mail = ses_mailer.Mail(aws_access_key_id=access_key, aws_secret_access_key=secret_key, region=region, sender=self.config.get('MAILER_SENDER'), reply_to=self.config.get('MAILER_REPLY_TO'), template=self.config.get('MAILER_TEMPLATE'), template_context=self.config.get('MAILER_TEMPLATE_CONTEXT'))
            elif 'smtp' in scheme:
                self.provider = 'SMTP'
