...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/flask_cache_external_assets/lib.py
# Compiled at: 2014-11-05 03:38:06
import os, os.path, hashlib, magic, requests
...
def get_path(url, extension=None):
    ...
    if not extension:
        extension = ''
    filename = ('{}/{}{}').format(cache_dir, filename_hash.hexdigest(), extension)
    get_path.cache[url] = filename
    return get_path.cache[url]


def get_filepath(url):
    return os.path.join(current_app.config['CACHE_EXTERNAL_ASSETS_ROOT'], get_path(url))
...
def get_cached_file_with_extension(url):
    ...
    path = get_path(url)
    for ext in ('.jpg', '.png', ''):
        if os.path.exists(filepath + ext) and os.stat(filepath + ext).st_size > 0:
