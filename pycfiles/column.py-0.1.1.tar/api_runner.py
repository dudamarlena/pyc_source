...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/column/api_runner.py
...
class APIRunner(runner.Runner):
    ...
    def run_playbook(self, playbook_file, inventory_file=None, **kwargs):
        ...
        ansible_inv = inventory.Inventory(loader=loader, variable_manager=variable_manager, host_list=options.inventory)
        ...
        pbex = playbook_executor.PlaybookExecutor(playbooks=playbooks, inventory=ansible_inv, variable_manager=variable_manager, loader=loader, options=options, passwords=passwords)
        self.tqm = pbex._tqm
    ...
    def run_module(self, module_name='ping', module_args=None, hosts='all', inventory_file=None, **kwargs):
        if not module_args:
            check_raw = module_name in ('command', 'win_command', 'shell', 'win_shell',
                                        'script', 'raw')
        ...
        play_obj = play.Play().load(play_ds, variable_manager=variable_manager, loader=loader)
        try:
            tqm = task_queue_manager.TaskQueueManager(inventory=ansible_inv, variable_manager=variable_manager, loader=loader, options=options, passwords=passwords, stdout_callback='minimal', run_additional_callbacks=True)
    ...
    def _build_opt_dict(self, inventory_file, **kwargs):
        args = {'check': None, 
           'listtasks': None, 'listhosts': None, 'listtags': None, 
           'syntax': None, 'module_path': None, 'skip_tags': [], 'ssh_common_args': '', 'sftp_extra_args': '', 
           'scp_extra_args': '', 'ssh_extra_args': '', 
           'become': constants.DEFAULT_BECOME, 'become_user': constants.DEFAULT_BECOME_USER, 
           ...
           'extra_vars': {}, 'subset': constants.DEFAULT_SUBSET, 'tags': [], 'verbosity': 0, 'connection': constants.DEFAULT_TRANSPORT, 
        ...
        if isinstance(args['tags'], str):
            ...
            raise exceptions.InvalidParameter(name=type(args['tags']).__name__, param='tag')
        return Namespace(**args)

    def _play_ds(self, hosts, module_name, module_args):
        return dict(name='Ansible module runner', hosts=hosts, gather_facts='no', tasks=[
         dict(action=dict(module=module_name, args=module_args), async=0, poll=constants.DEFAULT_POLL_INTERVAL)])
    ...
    def _process_stats(stats, failed_results=[]):
        ...
        return {'error_msg': error_msg, 'unreachable_hosts': unreachable_hosts, 'failed_hosts': failed_hosts, 
    ...
    def _process_task_result(task):
        ...
        if isinstance(result, dict) and 'msg' in result:
            ...
            error_msg = (' ').join(msgs)
        return (
         task_obj.get_name(), error_msg, host.get_name())