...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/dataflow/core.py
# Compiled at: 2010-07-22 08:30:58
"""
...
__all__ = [
 'component', 'runner', 'pprint_component']
BYPASS_VALS = [
 Ellipsis]
    ...
    def decor(fun):
        ...
        if 'generator_port' in xtra_args:
            ...
            if generator_port not in out_ports:
                raise ValueError, 'The given generator port %r does not match a port with an OUT port specification' % generator_port
    ...
    for m in mesh:
        ...
        if m[FNAME] in G:
            ...
            if node['component'] != m[FTYPE]:
                raise ValueError, 'Multiple component labels for %r point to different component types. Found %r, excepted %r' % (
...
class runner(object):
    ...
    def input_stream(self, name):
        ...
        def closure():
            if name in self.in_gens:
                ...
                raise ValueError, 'Invalid IN port %r on component %r. Valid Ports: %s' % (
    ...
    def build_arg_list(self):
        ...
        for port in comp.zf_port_order:
            if port in comp.zf_in_ports or port in comp.zf_in_array_ports:
                ...
                raise ValueError, 'Component %r include a port order for port %r, but the port is not registered as an IN port or OUT port' % (
                 comp.zf_name, port)
        ...
        if is_out_array_port:
            if idx != -1:
                if out_port not in self.out_array_lookup:
                    ...
                    if not self.out_array_lookup[out_port][idx]:
                        return True
                    return False
            else:
                if out_port not in self.out_array_lookup:
    ...
    def yield_value(self, out_port, idx):
        ...
        if is_out_array_port:
            ...
            return (
             self.out_buffer[out_port].pop(0), False)
                ...
                while self.buffer_empty(out_port, idx):
                    ...
                    if isinstance(result, tuple) and len(result) == 2 and isinstance(result[0], PortNumber):
                        ...
                        if this_port not in self.out_array_lookup:
                            raise ValueError, 'Encountered OUT array port value before a size has been specified on component %r, port %r' % (
                        ...
                        raise ValueError, 'component %r yielded invalid value %r. Please provide a generator_port or yield (port, value)' % (
                         self.comp.zf_name, result)
