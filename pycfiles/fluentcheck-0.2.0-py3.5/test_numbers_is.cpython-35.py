...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fluentcheck/tests/tests_is/test_numbers_is.py
...
class TestIsNumbersAssertions(unittest.TestCase):
    ...
    def test_is_not_number_fail(self):
        with self.assertRaises(CheckError):
            Is(123).not_number
        with self.assertRaises(CheckError):
            Is(float(123)).not_number
        with self.assertRaises(CheckError):
            Is(complex(33.44, 55.66)).not_number
    ...
    def test_is_negative_pass(self):
        self.assertIsInstance(Is(-0.1).negative, Is)
    ...
    def test_is_at_least_pass(self):
        ...
        Is(obj).at_least(-obj).at_least(obj - 0.5).at_least(obj)
    ...
    def test_is_at_most_pass(self):
        obj = 7
        self.assertIsInstance(Is(obj).at_least(obj), Is)
        Is(obj).at_most(obj).at_most(obj + 0.5)

    def test_is_at_most_fail(self):
        with self.assertRaises(CheckError):
            Is(7).at_most(6.999999)
