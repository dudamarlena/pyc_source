...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/hejtado/quido/business.py
# Compiled at: 2019-10-23 16:48:44
...
class Quido:
    __doc__ = 'Handle communication with Quido box'
    ...
    def get_boiler_temperature(self):
        ...
        boiler_temperature = self._Quido__snmp_get(oid)
        log.debug('get_boiler_temperature received temperature string {}'.format(boiler_temperature))
        boiler_temperature = float(self._Quido__select_return_value(boiler_temperature) / 10)
    ...
    def get_relay_status(self, relay_id):
        ...
        relay_state = self._Quido__snmp_get(oid)
        log.debug('get_relay_status received relay_state {}'.format(relay_state))
        relay_state = self._Quido__select_return_value(relay_state)
        return relay_state
    ...
    def get_relay_name(self, relay_id):
        ...
        relay_name = self._Quido__snmp_get(oid)
        log.debug('get_relay_status received relay_state {}'.format(relay_name))
        relay_name = self._Quido__select_return_value(relay_name)
        return relay_name
    ...
    def get_relay_type(self, relay_id):
        ...
        log.debug('get_relay_name sends oid {}'.format(oid))
        relay_type = self._Quido__snmp_get(oid)
        log.debug('get_relay_type received relay_state {}'.format(relay_type))
        relay_type = int(self._Quido__select_return_value(relay_type))
        if relay_type == 1:
            relay_type = 'static'
        else:
            if relay_type > 1:
    ...
    def set_relay(self, relay_id, desired_state):
        ...
        self._Quido__snmp_set(oid, desired_state_value)
        return True
    ...
    def __snmp_get(self, oid):
        ...
        data = next(getCmd(SnmpEngine(), CommunityData('private', mpModel=0), UdpTransportTarget((self.quido_ip, 161)), ContextData(), ObjectType(ObjectIdentity(oid))))
    ...
    def __snmp_set(self, oid, desired_state_value):
        ...
        """
        data = next(setCmd(SnmpEngine(), CommunityData('private', mpModel=0), UdpTransportTarget((self.quido_ip, 161)), ContextData(), ObjectType(ObjectIdentity(oid), Integer(int(desired_state_value)))))
