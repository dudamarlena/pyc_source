...
# [GCC 8.4.0]
# Embedded file name: /home/thongnguyen/training/asoft/thongnguyen/python-tools/drf-partial-response/drf_partial_response/views.py
# Compiled at: 2020-03-20 04:14:24
...
class OptimizedQuerySetMixin(object):
    ...
    def get_serializer_context(self):
        ...
        if fields_name in self.request.GET:
            if excludes_name in self.request.GET:
                raise exceptions.ParseError(detail=('Cannot provide both "%s" and "%s"' % (fields_name, excludes_name)))
        elif fields_name in self.request.GET:
    ...
    def optimize_queryset(self, queryset):
        ...
        if self.requested_fields or self.excluded_fields:
            return self.apply_requested_data_functions(queryset, self.requested_fields, self.excluded_fields)
    ...
    def apply_requested_data_functions(self, queryset, fields, excludes):
        ...
        for dotted_path, data_function in self._data_predicates.items():
            if should_include_variable(dotted_path,
              requested_structure, is_negated=is_negated):
