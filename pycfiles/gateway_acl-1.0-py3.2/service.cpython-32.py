...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/__plugin__/gateway_acl/service.py
...
"""
from ..gateway.service import updateAssemblyAnonymousGateways, assemblyAnonymousGateways, gatewayMethodMerge, registerMethodOverride
from ..plugin.registry import registerService
from .database import binders
from acl.api.group import IGroupService
from acl.core.impl.processor.gateway.acl_permission import RegisterAclPermissionHandler
from acl.core.impl.processor.gateway.compensate import RegisterCompensatePermissionHandler
...
anonymousGroup = registerPermissionGateway = support.notCreated
...
def updateAssemblyAnonymousGatewaysForAcl():
    assemblyAnonymousGateways().add(anonymousGroup(), rootURI(), registerAclPermission(), registerCompensatePermission(), registerPermissionGateway(), before=gatewayMethodMerge())


@ioc.after(assemblyGroupGateways)
def updateAssemblyGroupGateways():
    assemblyGroupGateways().add(registerAclPermission(), registerCompensatePermission(), rootURI(), registerPermissionGateway(), gatewayMethodMerge(), registerMethodOverride())