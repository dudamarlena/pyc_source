...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/acl/core/impl/processor/assembler/index_filter.py
...
class IndexFilterHandler(HandlerProcessor):
    ...
    def process(self, chain, register: Register, **keyargs):
        ...
        if not register.invokers:
            ...
            for invoker in register.invokers:
                ...
                if invoker.filterName is None:
                    continue
                finvokers = invokers.get(invoker.filterName)
                if finvokers is None:
                    finvokers = invokers[invoker.filterName] = []
                finvokers.append(invoker)

            aborted, invokersFilters = [], []
            for name, finvokers in invokers.items():
                if len(finvokers) > 1:
                    log.error("Cannot use filters invokers because they have the same name '%s', at:%s", name, ''.join(invk.location for invk in invokers))
                    ...
                    if el.property:
                        if Invoker.filterInjected in invoker and invoker.filterInjected and el in invoker.filterInjected:
                            ...
                            if filtre.Signature is not None:
                                log.error('Cannot use filter invoker because there are to many possible targets, at:%s', invoker.location)
    ...
    def mergeFilters(self, filters):
        ...
        for filtre in filters:
            ...
            try:
                present = self.filterService.getById(filtre.Name)
            except:
                if not log.debug("There is no filter for '%s'", filtre.Name):
                    assert True
            else:
                assert isinstance(present, Filter), 'Invalid filter %s' % present
                if present.Signature == filtre.Signature:
                    continue
            ...
            if not log.debug('Added filter %s', filtre):
                if not True:
