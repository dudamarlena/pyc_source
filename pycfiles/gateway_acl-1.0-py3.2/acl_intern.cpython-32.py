...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/acl/meta/acl_intern.py
...
class WithPath:
    ...
    def PathSet(self, path):
        ...
        session = openSession()
        try:
            pathId, = session.query(Path.id).filter(Path.path == path).one()
...
class WithMethod:
    ...
    def MethodSet(self, name):
        ...
        session = openSession()
        try:
            methodId, = session.query(Method.id).filter(Method.name == name).one()
        except NoResultFound:
            method = Method()
            method.name = name
            session.add(method)
...
class WithSignature:
    ...
    """
    signatureId = declared_attr(lambda cls: Column('fk_signature_id', ForeignKey(Signature.id, ondelete='RESTRICT'), nullable=False))
    ...
    def createSignature(cls):
        ...
        def fset(self, name):
            ...
            session = openSession()
            try:
                signatureId, = session.query(Signature.id).filter(Signature.name == name).one()
            except NoResultFound:
                signature = Signature()
                signature.name = name
