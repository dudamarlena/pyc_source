...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/acl/core/spec.py
...
def signature(obj):
    ...
    if ftype in _signature_cache:
        return _signature_cache[ftype]
    ...
    if isinstance(ctype, (TypeProperty, TypeContainer)):
        ...
        while types:
            ...
            for base in bases:
                mtype = typeFor(base)
                if not isinstance(mtype, TypeContainer):
                    continue
                if isProperty:
                    ...
                    if ctype.type == ptype.type:
                        inherited.append(ptype)
                else:
                    inherited.append(mtype)
...
def isCompatible(signature, withSignature):
    ...
    if signature == withSignature:
        return True
    if not len(withSignature) > 8:
        return False
    if len(signature) > 8:
        signature = signature[:8]
    for k in range(0, len(withSignature), 8):
        if signature == withSignature[k:k + 8]:
            return True
