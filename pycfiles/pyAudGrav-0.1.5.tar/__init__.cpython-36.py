...
__doc__ = 'Library for controlling an Apple TV.'
...
class _ServiceListener:
    ...
    def add_hs_service(self, info, address):
        ...
        if self.protocol:
            if self.protocol != PROTOCOL_DMAP:
                return
        name = info.properties['Name'].decode('utf-8')
        hsgid = info.properties['hG'].decode('utf-8')
    ...
    def add_non_hs_service(self, info, address):
        ...
        if self.protocol:
            if self.protocol != PROTOCOL_DMAP:
                return
        name = info.properties['CtlN'].decode('utf-8')
