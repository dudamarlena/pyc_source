...
__doc__ = 'Util functions for extracting and constructing DMAP data.'
import plistlib
...
def uint8_tag(name, value):
    ...
    return name.encode('utf-8') + '\x00\x00\x00\x01' + value.to_bytes(1, byteorder='big')


def uint16_tag(name, value):
    """Create a DMAP tag with uint16 data."""
    return name.encode('utf-8') + '\x00\x00\x00\x02' + value.to_bytes(2, byteorder='big')


def uint32_tag(name, value):
    """Create a DMAP tag with uint32 data."""
    return name.encode('utf-8') + '\x00\x00\x00\x04' + value.to_bytes(4, byteorder='big')


def uint64_tag(name, value):
    """Create a DMAP tag with uint64 data."""
    return name.encode('utf-8') + '\x00\x00\x00\x08' + value.to_bytes(8, byteorder='big')


def bool_tag(name, value):
    """Create a DMAP tag with boolean data."""
    return name.encode('utf-8') + '\x00\x00\x00\x01' + ('\x01' if value else '\x00')
