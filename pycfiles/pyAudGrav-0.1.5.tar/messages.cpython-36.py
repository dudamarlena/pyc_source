...
__doc__ = 'Helper code for dealing with protobuf messages.'
import binascii
...
def send_hid_event(use_page, usage, down):
    ...
    event = message.inner()
    abstime = binascii.unhexlify('438922cf08020000')
    data = use_page.to_bytes(2, byteorder='big')
    data += usage.to_bytes(2, byteorder='big')
    data += (1 if down else 0).to_bytes(2, byteorder='big')
    event.hidEventData = abstime + binascii.unhexlify('00000000000000000100000000000000020' + '00000200000000300000001000000000000') + data + binascii.unhexlify('0000000000000001000000')
    return message
