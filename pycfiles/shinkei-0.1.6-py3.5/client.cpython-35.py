...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/shinkei/client.py
# Compiled at: 2019-12-14 12:45:46
# Size of source mod 2**32: 16461 bytes
import asyncio, logging, traceback, websockets
...
def connect(url, application_id, client_id, auth=None, *, tags=None, reconnect=True, session=None, loop=None, klass=None, handlers=None, proxy_ip=None, **kwargs):
    ...
    A context manager mixin of :class:`Client`
        The client.
    """
    return _ClientMixin(url, application_id, client_id, auth, reconnect=reconnect, session=session, loop=loop, tags=tags, klass=klass, handlers=handlers, proxy_ip=proxy_ip, **kwargs)
...
class Client:
    ...
    @classmethod
    async def _connect(cls, url, application_id, client_id, auth=None, *, reconnect=True, session=None, loop=None, tags=None, handlers=None, proxy_ip=None, **_):
        self = cls()
        ...
        self._rest = await APIClient.create(URL(url).with_scheme(rest_scheme), session=session, auth=auth, loop=self.loop)
