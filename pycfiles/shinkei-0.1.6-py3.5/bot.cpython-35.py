...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/shinkei/ext/bot.py
...
class BotClient(Client):
    ...
    @classmethod
    async def _connect(cls, url, rest_url, application_id, client_id, auth=None, *, reconnect=True, session=None, loop=None, tags=None, **kwargs):
        self = cls()
        try:
    ...
    def add_handler(self, handler):
        ...
        raise NotImplementedError("add_handler() cannot be used with BotClient (events are dispatched through bot.dispatch('shinkei_{event_name}'))")

    def remove_handler(self, handler_name):
        """Events are now handled by bot listeners so this method now raises a :exc:`NotImplementedError`."""
        raise NotImplementedError("remove_handler() cannot be used with BotClient (events are dispatched through bot.dispatch('shinkei_{event_name}'))")
