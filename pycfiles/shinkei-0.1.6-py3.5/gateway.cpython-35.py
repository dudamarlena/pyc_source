...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/shinkei/gateway.py
# Compiled at: 2019-12-14 12:45:46
# Size of source mod 2**32: 7151 bytes
...
class WSClient(websockets.WebSocketClientProtocol):
    ...
    async def poll_event(self):
        try:
            ...
            if exc.code not in {1000, 4010, 4011, 4004}:
    ...
    def _make_metadata_packet(data):
        ...
        try:
            return {key:{'value': value.fmt if isinstance(value, VersionMetadata) else value, 'type': type_mapping[type(value)]} for key, value in data.items()}
        except KeyError:
