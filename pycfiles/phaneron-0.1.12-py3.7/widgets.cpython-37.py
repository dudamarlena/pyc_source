...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/phaneron/widgets.py
# Compiled at: 2019-10-31 16:46:29
# Size of source mod 2**32: 12245 bytes
"""Phaneron widgets"""
...
shading_modes = [
 'none', 'diffuse', 'electron', 'cartoon', 'electron_transparency', 'perlin', 'diffuse_transparency']
...
class Widgets:
    ...
    def display_palette_for_models(self):
        ...
        def set_colormap(model_id, colormap_name, shading_mode):
            ...
            self._circuit_explorer.set_materials(model_ids=[
             model_id],
        ...
        def update_materials_from_shading(v):
            set_colormap(self._brayns.scene.models[model_combobox.index]['id'], palette_combobox.value, shading_combobox.index)
    ...
    def display_model_visibility(self):
        ...
        def update_models(visible):
            for model_id in model_select.index:
                self._brayns.update_model(id=(self._brayns.scene.models[model_id]['id']),
                  visible=visible)
