...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/phaneron/circuit_explorer.py
# Compiled at: 2019-10-19 10:50:37
# Size of source mod 2**32: 28326 bytes
"""Provides a class that wraps the API exposed by the braynsCircuitExplorer plug-in"""

class CircuitExplorer:
    __doc__ = 'Circuit Explorer, a class that wraps the API exposed by the braynsCircuitExplorer plug-in'
    ...
    def load_circuit(self, path, name='Circuit', db_connection_string='', density=1.0, random_seed=0, targets=[], report='', report_type=REPORT_TYPE_NONE, user_data_type=USER_DATATYPE_NONE, synchronous_mode=True, circuit_color_scheme=CIRCUIT_COLOR_SCHEME_NONE, mesh_folder='', mesh_filename_pattern='', mesh_transformation=False, radius_multiplier=1, radius_correction=0, load_soma=True, load_axon=True, load_dendrite=True, load_apical_dendrite=True, use_sdf=False, dampen_branch_thickness_changerate=True, use_metaballs_for_soma=False, metaballs_section_samples=5, metaballs_grid_size=20, metaballs_threshold=1, morphology_color_scheme=MORPHOLOGY_COLOR_SCHEME_NONE, morphology_quality=GEOMETRY_QUALITY_HIGH, max_distance_to_soma=1000000.0, cell_clipping=False, areas_of_interest=0):
        """
    ...
    def set_material(self, model_id, material_id, diffuse_color=[
 1.0, 1.0, 1.0], specular_color=[
 1.0, 1.0, 1.0], specular_exponent=20.0, opacity=1.0, reflection_index=0.0, refraction_index=1.0, simulation_data_cast=True, glossiness=1.0, intensity=1.0, shading_mode=SHADING_MODE_NONE, emission=0.0, clipped=False):
        """
    ...
    def set_materials(self, model_ids, material_ids, diffuse_colors, specular_colors, specular_exponents=list(), opacities=list(), reflection_indices=list(), refraction_indices=list(), simulation_data_casts=list(), glossinesses=list(), shading_modes=list(), emissions=list(), clips=list()):
        """
        ...
        params['clips'] = clips
        return self._client.request('setMaterials', params=params, response_timeout=(self.DEFAULT_RESPONSE_TIMEOUT))

    def set_transfer_function(self, palette, data_range=(0, 255), contribution=1.0, intensity=1.0, emission=(0.0, 0.0, 0.0)):
    ...
    def set_connections_per_value(self, model_id, frame, value, epsilon, radius):
        ...
        return self._client.request('setConnectionsPerValue', params=params, response_timeout=(self.DEFAULT_RESPONSE_TIMEOUT))
    ...
    def set_metaballs_per_simulation_value(self, model_id, frame, value, epsilon, grid_size, threshold):
        ...
        return self._client.request('setMetaballsPerSimulationValue', params=params, response_timeout=(self.DEFAULT_RESPONSE_TIMEOUT))
    ...
    def set_material_extra_attributes(self, model_id):
        ...
        params = dict()
        params['modelId'] = model_id
        return self._client.request('setMaterialExtraAttributes', params=params, response_timeout=(self.DEFAULT_RESPONSE_TIMEOUT))
    ...
    def set_default_transfer_function(self, alpha=1, intensity=1, display_palette=False):
        ...
        points[int((-80.0 - range_min) * delta)] = [
         1.0, 0.0, 0.0, 0.0]
        points[int((-77.181205 - range_min) * delta)] = [0.023529, 0.023529, 0.654902, 0.05]
        ...
        points[int((-50.67785 - range_min) * delta)] = [0.858824, 0.67451, 0.0, 0.4]
        points[int((-31.47 - range_min) * delta)] = [0.964706, 1.0, 0.6313725, 0.8]
    ...
    def load_cells_as_instances(self, connection_string, sql_statement, name, description, morphology_folder, morphology_extension):
        params = dict()
        params['connectionString'] = connection_string
    ...
    def load_somas(self, connection_string, name='', sql_statement='', radius=1.0, show_orientations=False):
        params = dict()
        params['connectionString'] = connection_string
        params['name'] = name
        params['sqlStatement'] = sql_statement
    ...
    def export_frames_to_disk(self, path, animation_frames, camera_definitions, image_format='png', quality=100, samples_per_pixel=1, start_frame=0):
        """
        ...
        return self._client.request('exportFramesToDisk', params, response_timeout=(self.DEFAULT_RESPONSE_TIMEOUT))
