...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/gui/plugins/ImportPDFasImages.py
...
import os, gtk, glib, cairo, poppler
...
from sutekh.base.gui.SutekhDialog import SutekhDialog, do_complaint_buttons, do_complaint_error
...
class ImportPDFImagesPlugin(SutekhPlugin):
    ...
    def run_import_dialog(self, _oMenuWidget):
        ...
        if self._oImageFrame is None:
            ...
            self.oDlg = SutekhDialog('Choose PDF File and Expansion', None, gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT, (
             gtk.STOCK_OK, gtk.RESPONSE_OK,
             gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL))
            self.oDlg.vbox.pack_start(gtk.Label('PDF File'), expand=False)
            self.oFileChooser = SutekhFileWidget(self.parent, gtk.FILE_CHOOSER_ACTION_OPEN)
            ...
            aExpansions = [ x.name for x in Expansion.select() if x.name[:5] != 'Promo'
                          ] + ['Promo']
            ...
            for sName in aExpansions:
                if self._oFirstBut:
                    oBut = gtk.RadioButton(self._oFirstBut, sName, use_underline=False)
                else:
                    self._oFirstBut = gtk.RadioButton(None, sName, use_underline=False)
            ...
            for oBut in self._oFirstBut.get_group():
                if oBut.get_active():
                    ...
                    if sName != 'Promo':
                        ...
                        aExp = [ x for x in Expansion.select() if x.name[:5] == 'Promo'
                               ]
    ...
    def do_import(self, sFile, aExp):
        ...
        oImportDialog = SutekhDialog('Match cards and images', None, gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT, (
        ...
        oOffsetBox = gtk.HBox()
        oXOffAdj = gtk.Adjustment(lower=-CARD_DIM[0] // 2, upper=CARD_DIM[0] // 2, step_incr=1, value=0)
        oYOffAdj = gtk.Adjustment(lower=-CARD_DIM[1] // 2, upper=CARD_DIM[1] // 2, step_incr=1, value=0)
        ...
        oScaleBox = gtk.HBox()
        oXScaleAdj = gtk.Adjustment(lower=10, upper=CARD_DIM[0] + 100, step_incr=1, value=CARD_DIM[0])
        oYScaleAdj = gtk.Adjustment(lower=10, upper=CARD_DIM[1] + 100, step_incr=1, value=CARD_DIM[1])
        ...
        oSaveButton.connect('pressed', self.save_img, oDrawArea, oAbsCardView, aExp)
    ...
    def _render_pdf(self, oContext, iWidth, iHeight):
        ...
        if self._oPageImg:
            ...
            oContext.rectangle(0, 0, self._iScale * CARD_DIM[0], self._iScale * CARD_DIM[1])
            oContext.fill()
    ...
    def _load_page(self):
        ...
        self._oPageImg = cairo.ImageSurface(cairo.FORMAT_ARGB32, iWidth, iHeight)
    ...
    def save_img(self, _oBut, oDrawArea, oAbsView, aExp):
        ...
        if oAbsCard is None:
            ...
            if check_file(sFileName):
                iRes = do_complaint_buttons('File %s exists. Do You want to replace it?' % sFileName, gtk.MESSAGE_QUESTION, (
                 gtk.STOCK_YES, gtk.RESPONSE_YES,
                 gtk.STOCK_NO, gtk.RESPONSE_NO))
                if iRes == gtk.RESPONSE_NO:
                    return
            ensure_dir_exists(os.path.dirname(sFileName))
            oPixbuf = gtk.gdk.Pixbuf(gtk.gdk.COLORSPACE_RGB, True, 8, self._iScale * CARD_DIM[0], self._iScale * CARD_DIM[1])
            oCMap = oDrawArea.window.get_colormap()
            oImgPixmap = gtk.gdk.Pixmap(oDrawArea.window, self._iScale * CARD_DIM[0], self._iScale * CARD_DIM[1])
            oContext = oImgPixmap.cairo_create()
            self._render_pdf(oContext, self._iScale * CARD_DIM[0], self._iScale * CARD_DIM[1])
            oPixbuf.get_from_drawable(oImgPixmap, oCMap, 0, 0, 0, 0, self._iScale * CARD_DIM[0], self._iScale * CARD_DIM[1])
    ...
    def chg_img(self, _oBut, oDrawArea, iChg):
        ...
        if iNewPage != iOldPage:
            self._load_page()
        if self._iCurPageNo == self._iNumPages - 1 and self._iXPos == 2 and self._iYPos == 2:
            self._oNextButton.set_sensitive(False)
        else:
            self._oNextButton.set_sensitive(True)
        if self._iCurPageNo == 0 and self._iXPos == 0 and self._iYPos == 0:
            self._oPrevButton.set_sensitive(False)
        else:
