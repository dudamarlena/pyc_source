...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/base/core/DatabaseVersion.py
...
class DatabaseVersion(object):
    ...
    def populate_cache(cls, oConn):
        ...
        for oVer in VersionTable.select(connection=oConn):
            if oVer.TableName in dCache:
                raise RuntimeError('Multiple version entries for %s found in the database.' % oVer.TableName)
            else:
    ...
    def set_version(self, oTable, iTableVersion, oConn=None):
        ...
        aVer = list(VersionTable.selectBy(TableName=sTableName, connection=oConn))
        iTableCount = len(aVer)
        if iTableCount == 0:
            VersionTable(TableName=sTableName, Version=iTableVersion, connection=oConn)
            ...
            print('Multiple version entries for %s in the database' % sTableName)
    ...
    def get_table_version(self, oTable, oConn=None):
        ...
        if iTableCount < 1:
            ...
            print('Multiple version entries for %s in the database' % oTable.sqlmeta.table)
            print('Giving up. I suggest dumping and reloading everything')
            iTableVersion = -999
        return iTableVersion
