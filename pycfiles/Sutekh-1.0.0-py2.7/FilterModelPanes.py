...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/base/gui/FilterModelPanes.py
# Compiled at: 2019-12-11 16:37:48
"""Handle the panes for the filter editor"""
import gobject, gtk, pango
from ..core.FilterParser import get_filters_for_type
from ..core.FilterBox import FilterBoxItem, FilterBoxModel, BOXTYPE, BOXTYPE_ORDER
...
def add_accel_to_button(oButton, sAccelKey, oAccelGroup, sToolTip=None):
    ...
    if iKeyVal != 0:
        oButton.add_accelerator('clicked', oAccelGroup, iKeyVal, iMod, gtk.ACCEL_VISIBLE)
    if sToolTip:
...
class FilterEditorToolbar(CustomDragIconView):
    ...
    def __init__(self, sFilterType):
        self._oListStore = gtk.ListStore(gobject.TYPE_STRING, gobject.TYPE_STRING)
        ...
        for oFilterType in sorted(get_filters_for_type(self._sFilterType), key=lambda x: x.description):
            aFilters.append((oFilterType.description, oFilterType.keyword))

        self.drag_source_set(gtk.gdk.BUTTON1_MASK | gtk.gdk.BUTTON3_MASK, DRAG_TARGETS, gtk.gdk.ACTION_COPY | gtk.gdk.ACTION_MOVE)
...
class FilterValuesBox(gtk.VBox):
    ...
    def __init__(self, oDialog, sFilterType):
        ...
        self._sFilterType = sFilterType
        self._oEmptyWidget.pack_start(gtk.Label('Select an active\nfilter element'), expand=False)
        self._oNoneWidget.pack_start(gtk.Label('No values for this filter'), expand=False)
    ...
    def _set_drop_for_widget(self, oWidget):
        ...
        oViewWidget.drag_dest_set(gtk.DEST_DEFAULT_ALL, DRAG_TARGETS, gtk.gdk.ACTION_COPY | gtk.gdk.ACTION_MOVE)
    ...
    def _set_drag_for_widget(self, oWidget, fCallback, oFilter):
        ...
        oViewWidget = self._get_view_widget(oWidget)
        oViewWidget.drag_source_set(gtk.gdk.BUTTON1_MASK | gtk.gdk.BUTTON3_MASK, DRAG_TARGETS, gtk.gdk.ACTION_COPY | gtk.gdk.ACTION_MOVE)
    ...
    def _make_filter_group_list(self, oFilter):
        ...
        oFilterTypes.get_selection_object().connect('changed', self.update_box_model, oFilter, oFilterTypes)
        oFilterTypes.set_size_request(100, 150)
        oFilterTypes.view.connect('key-press-event', self.key_press, oFilter, 'Filter Type')
        ...
        oSubFilterList.connect('key-press-event', self.key_press, oFilter, 'Filter')
        self._oWidget.pack_start(AutoScrolledWindow(oSubFilterList), expand=True)
    ...
    def _make_list_from(self, oFilter):
        ...
        self._oWidget.pack_start(AutoScrolledWindow(oSetList, False), expand=True)
        return
    ...
    def _make_filter_values_list(self, oFilter):
        ...
        self._oWidget = ScrolledList('Select Filter Values', bSpecialSelect=True)
        self._oWidget.set_select_multiple()
        self._oWidget.fill_list(oFilter.aValues)
        self._oWidget.view.connect('key-press-event', self.key_press, oFilter, 'Value')
        self._set_drag_for_widget(self._oWidget, self.update_filter_list, oFilter)
    ...
    def set_widget(self, oFilter, oBoxModelEditor):
        ...
        if isinstance(oFilter, FilterBoxModel):
            ...
            if oFilter.iValueType == FilterBoxItem.LIST:
                if oFilter.sFilterName == 'Card_Sets' or oFilter.sFilterName == 'ParentCardSet':
                ...
                self._oWidget.connect('key-press-event', self.key_press, None, 'None')
        else:
            self._oWidget = self._oEmptyWidget
    ...
    def update_filter_list(self, _oBtn, _oContext, oSelectionData, _oInfo, _oTime, oFilter, _oWidget):
    ...
    def update_count_list(self, _oBtn, _oContext, oSelectionData, _oInfo, _oTime, oFilter, oCountList):
        """Update the box model with the new values"""
        sSelect = '%s%s: Count' % (NEW_VALUE, oFilter.sFilterName)
        self._format_selection(sSelect, oCountList.get_selection())
        oSelectionData.set(oSelectionData.target, 8, sSelect)

    def update_set_list(self, _oBtn, _oContext, oSelectionData, _oInfo, _oTime, oFilter, oSetList):
        """Update the box model to the current selection"""
        sSelect = '%s%s' % (NEW_VALUE, oFilter.sFilterName)
        self._format_selection(sSelect, oSetList.get_all_selected_sets())
        oSelectionData.set(oSelectionData.target, 8, sSelect)

    def update_count_set(self, _oBtn, _oContext, oSelectionData, _oInfo, _oTime, oFilter, oSetList):
        """Update the box model to the current selection"""
    ...
    def switch_focus(self, oEvent, sSource):
        ...
        if sSource in set(['Value', 'Entry', 'None', 'CardSet']):
            ...
            if isinstance(self._oLastFilter, FilterBoxModel) or isinstance(self._oLastFilter, FilterBoxItem) and self._oLastFilter.iValueType == FilterBoxItem.LIST_FROM:
            ...
            if oEvent.keyval in RIGHT and sSource in set(['Count', 'Filter Type']):
                oAutoScrolled = self._oWidget.get_children()[2]
                oAutoScrolled.get_child().grab_focus()
            elif oEvent.keyval in LEFT and sSource in set(['Filter', 'Set']):
                self._oWidget.get_children()[0].view.grab_focus()
            else:
                self._oBoxModelEditor.grab_focus()
    ...
    def key_press(self, oWidget, oEvent, oFilter, sSource):
        ...
        if oEvent.keyval in LEFT_RIGHT and oEvent.get_state() & gtk.gdk.MOD1_MASK:
            self.switch_focus(oEvent, sSource)
        if oEvent.keyval not in ENTER_KEYS or not oEvent.get_state() & gtk.gdk.CONTROL_MASK:
            return
        if sSource == 'Filter':
...
class FilterBoxModelStore(gtk.TreeStore):
    ...
    def __init__(self):
        super(FilterBoxModelStore, self).__init__(gobject.TYPE_STRING, gobject.TYPE_PYOBJECT, gtk.gdk.Color)
    ...
    def _do_add_iter(self, oIter, oModel, bDisabled):
        ...
        if hasattr(oModel, 'sFilterDesc'):
            ...
            for sDesc, tInfo in BOXTYPE.iteritems():
                sBoxType, bNegate = tInfo
                if oModel.sBoxType == sBoxType and oModel.bNegate == bNegate:
        ...
        if aValues:
            ...
            for sValue in aValues:
                ...
                if sValue is not None:
                    self.set(oChild, 0, gobject.markup_escape_text(sValue), 1, None, 2, self.BLACK)
                else:
    ...
    def get_drop_iter(self, tRowInfo):
        ...
        if not tRowInfo:
            ...
            if self.iter_depth(oIter) > 0 and iDropPos == gtk.TREE_VIEW_DROP_BEFORE:
...
class FilterBoxModelEditView(CustomDragIconView):
    ...
    def __init__(self, oStore, oValuesWidget, oBoxModel):
        ...
        oTextCell = gtk.CellRendererText()
        oColumn = gtk.TreeViewColumn('Filter', oTextCell, markup=0, foreground_gdk=2)
        self.append_column(oColumn)
        self.drag_dest_set(gtk.DEST_DEFAULT_ALL, DRAG_TARGETS, gtk.gdk.ACTION_COPY | gtk.gdk.ACTION_MOVE)
        self.drag_source_set(gtk.gdk.BUTTON1_MASK | gtk.gdk.BUTTON3_MASK, DRAG_TARGETS, gtk.gdk.ACTION_COPY | gtk.gdk.ACTION_MOVE)
    ...
    def remove_value_at_iter(self, oIter):
        ...
        if oFilter.iValueType == FilterBoxItem.LIST and sValue in oFilter.aCurValues:
            oFilter.aCurValues.remove(sValue)
            self.update_list(self.oCurSelectIter, oFilter)
        elif oFilter.iValueType == FilterBoxItem.LIST_FROM:
            if oFilter.aCurValues[0] and sValue in oFilter.aCurValues[0]:
                oFilter.aCurValues[0].remove(sValue)
            elif oFilter.aCurValues[1] and sValue in oFilter.aCurValues[1]:
    ...
    def update_entry(self, oFilterItem):
        ...
        oChildPath = self._oStore.update_entry(oFilterItem, self.oCurSelectIter)
    ...
    def _update_iter_with_list(self, oIter, aValues):
        ...
        oCurPath = self._oStore.update_iter_with_values(aValues, oIter, oPath)
    ...
    def _update_iter_with_from_list(self, oIter, aCounts, aNames):
        ...
        oPath, _oCol = self.get_cursor()
        oCurPath = self._oStore.update_iter_with_from_list(aCounts, aNames, oIter, oPath)
        self.expand_to_path(oCurPath)
        self.expand_row(oCurPath, True)
    ...
    def drag_drop_handler(self, _oWindow, oDragContext, iXPos, iYPos, oSelectionData, _oInfo, oTime):
        """Handle drops from the filter toolbar"""
        bDragRes = True
        if not oSelectionData and oSelectionData.format != 8:
            bDragRes = False
            ...
            if not sData:
                bDragRes = False
            elif sData.startswith(NEW_FILTER) or sData.startswith(MOVE_FILTER):
    ...
    def _check_drag_value(self, oIter, oFilter, tCurTarget, oAction, oDragContext):
        ...
        if not tCurTarget:
            ...
            if hasattr(oDropObj, 'sFilterName') and oDropObj.sFilterName == oFilter.sFilterName:
    ...
    def drag_motion(self, _oWidget, oDragContext, iXPos, iYPos, _oTime):
        ...
        if oDragContext.get_source_widget() is self:
            if oFilter is None:
                if self._check_drag_value(oIter, oFilter, tCurTarget, gtk.gdk.ACTION_MOVE, oDragContext):
                ...
                bSetRow = True
        elif oFilter is None or not hasattr(oFilter, 'sBoxType'):
            if self._check_drag_value(oIter, oFilter, tCurTarget, gtk.gdk.ACTION_COPY, oDragContext):
                bSetRow = True
                bFixHighlight = True
        else:
    ...
    def _update_values(self, oFilterObj, oIter, sData):
        ...
        if oFilterObj.iValueType == FilterBoxItem.LIST:
            ...
            for sValue in aSelection:
                ...
                elif not oFilterObj.aCurValues[1]:
    ...
    def _do_move_value(self, sData, tRowInfo):
        ...
        oSourceFilter = self._oStore.get_value(self._oStore.iter_parent(oIter), 1)
        sValue = unescape_markup(self._oStore.get_value(oIter, 0))
        ...
        if oFilterObj.iValueType == FilterBoxItem.LIST:
            ...
            if oFilterObj.iValueType == FilterBoxItem.LIST_FROM:
                for iIndex in (0, 1):
                    if oSourceFilter.aCurValues[iIndex] and sValue in oSourceFilter.aCurValues[iIndex]:
    ...
    def _do_drop_filter(self, sData, tRowInfo):
        ...
        if sData.startswith('NewFilter'):
            ...
            oParent = self._oStore.get_value(self._oStore.iter_parent(oSourceIter), 1)
            bDoInsert = False
            if oInsertObj is not oFilterObj:
                if not isinstance(oFilterObj, FilterBoxModel) or not oFilterObj.is_in_model(oInsertObj):
                    bDoInsert = True
            if bDoInsert:
    ...
    def delete(self, _oIgnore):
        ...
        if oCurValue is oFilterObj:
            ...
            self.remove_filter_at_iter(self.oCurSelectIter)
        elif (oFilterObj.iValueType == FilterBoxItem.LIST or oFilterObj.iValueType == FilterBoxItem.ENTRY) and not oFilterObj.aCurValues:
            self.remove_filter_at_iter(self.oCurSelectIter)
        elif oFilterObj.iValueType == FilterBoxItem.LIST_FROM and not oFilterObj.aCurValues[0] and not oFilterObj.aCurValues[1]:
            self.remove_filter_at_iter(self.oCurSelectIter)
        else:
...
class FilterBoxModelEditBox(gtk.VBox):
    ...
    def __init__(self, oValuesWidget):
        ...
        self._oTreeView = FilterBoxModelEditView(oTreeStore, oValuesWidget, None)
