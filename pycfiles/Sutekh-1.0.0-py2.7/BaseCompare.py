...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/base/gui/plugins/BaseCompare.py
...
from ...core.BaseAdapters import IPhysicalCard, IAbstractCard, IPhysicalCardSet, IPrintingName
    ...
    for sCardSetName in aCardSetNames:
        ...
        for oCard in oCardSet:
            ...
            if bIgnoreExpansions:
                oKey = (
                 oAbsCard, oAbsCard.name, UNKNOWN_EXP)
            elif oCard.printing:
                oKey = (
                 oCard, oAbsCard.name,
                ...
                oKey = (
                 oAbsCard, oAbsCard.name, UNKNOWN_EXP)
            dFullCardList.setdefault(oKey, {aCardSetNames[0]: 0, aCardSetNames[1]: 0})
            dFullCardList[oKey][sCardSetName] += 1

    dDifferences = {aCardSetNames[0]: {}, aCardSetNames[1]: {}}
    dCommon = {}
    for tCardInfo in dFullCardList:
        iDiff = dFullCardList[tCardInfo][aCardSetNames[0]] - dFullCardList[tCardInfo][aCardSetNames[1]]
        iCommon = min(dFullCardList[tCardInfo][aCardSetNames[0]], dFullCardList[tCardInfo][aCardSetNames[1]])
        if iDiff > 0:
            dDifferences[aCardSetNames[0]][tCardInfo[0]] = (
             tCardInfo[1],
             tCardInfo[2],
             iDiff)
        elif iDiff < 0:
            dDifferences[aCardSetNames[1]][tCardInfo[0]] = (
             tCardInfo[1],
             tCardInfo[2],
             abs(iDiff))
        if iCommon > 0:
            dCommon[tCardInfo[0]] = (
             tCardInfo[1], tCardInfo[2], iCommon)

    return (
     dDifferences, dCommon)
...
class BaseCompare(BasePlugin):
    ...
    def activate(self, _oWidget):
        ...
        oDlg = SutekhDialog('Choose Card Set to Compare with', self.parent, gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT, (
        ...
        oDlg.connect('response', self.handle_response, oCSView, oIgnoreExpansions)
        oDlg.show_all()
        oDlg.run()
        oDlg.destroy()
        return
    ...
    def handle_response(self, _oWidget, iResponse, oCSView, oIgnoreExpansions):
        ...
        if iResponse == gtk.RESPONSE_OK:
            ...
            if not sSet:
                ...
                self.comp_card_sets(aCardSetNames, oIgnoreExpansions.get_active())
    ...
    def comp_card_sets(self, aCardSetNames, bIgnoreExpansions):
        ...
        def format_list(dCardInfo, sColor):
            ...
            for sGroup, oGroupIter in oGroupedCards:
                ...
                aList.sort()
                sContents += '<span foreground="blue">%s - %d cards</span>\n' % (
                 sGroup, iTot)
                for sCardName, sExpansion, iCount in aList:
                    sContents += '\t%(num)d × <span foreground = "%(color)s"> %(name)s' % {'num': iCount, 
                       'color': sColor, 
                       'name': sCardName}
                    if sExpansion:
                        sContents += ' (%s)</span>\n' % sExpansion
        ...
        def make_page(oList, dCardData):
            ...
            if dCardData:
                ...
                oButton.connect('clicked', self.create_card_set, dCardData)
                oPage.pack_start(oButton, False)
            return oPage

        dDifferences, dCommon = _get_card_set_list(aCardSetNames, bIgnoreExpansions)
        oResultDlg = NotebookDialog('Card Comparison', self.parent, gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT, (
