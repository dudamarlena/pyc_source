...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/base/gui/HTMLTextView.py
# Compiled at: 2019-12-11 16:37:48
"""A gtk.TextView-based renderer for XHTML-IM, as described in:
   http://www.jabber.org/jeps/jep-0071.html .
   """
import re, warnings, HTMLParser
...
if gtk.gdk.screen_get_default() is None:
    ...
    SCREEN_RESOLUTION = 0.3514598 * (gtk.gdk.screen_height() / float(gtk.gdk.screen_height_mm()))
else:
...
class HtmlHandler(HTMLParser.HTMLParser):
    ...
    def _parse_length(self, sValue, bFontRelative, fCallback, *args):
        ...
        if sValue.endswith('%'):
            ...
            if bFontRelative:
                ...
                self.__parse_length_frac_cb(self._oTextView, oAlloc, fFrac, fCallback, args)
                self._oTextView.connect('size-allocate', self.__parse_length_frac_cb, fFrac, fCallback, args)
        elif sValue.endswith('pt'):
            fCallback((float(sValue[:-2]) * SCREEN_RESOLUTION), *args)
        elif sValue.endswith('em'):
            oAttrs = self._get_current_attributes()
            fFontSize = oAttrs.font.get_size() / pango.SCALE
            fCallback((float(sValue[:-2]) * SCREEN_RESOLUTION * fFontSize), *args)
        elif sValue.endswith('ex'):
            oAttrs = self._get_current_attributes()
            fFontSize = oAttrs.font.get_size() / pango.SCALE
            fCallback((float(sValue[:-2]) * SCREEN_RESOLUTION * fFontSize), *args)
        elif sValue.endswith('px'):
    ...
    def _parse_style_font_size(self, oTag, sValue):
        ...
        try:
            iScale = {'xx-small': pango.SCALE_XX_SMALL, 
               'x-small': pango.SCALE_X_SMALL, 
               'small': pango.SCALE_SMALL, 
               'medium': pango.SCALE_MEDIUM, 
               'large': pango.SCALE_LARGE, 
               'x-large': pango.SCALE_X_LARGE, 
               'xx-large': pango.SCALE_XX_LARGE}[sValue]
        except KeyError:
            pass
        else:
            oAttrs = self._get_current_attributes()
    ...
    def _parse_style_font_style(self, oTag, sValue):
        ...
        try:
            iStyle = {'normal': pango.STYLE_NORMAL, 
               'italic': pango.STYLE_ITALIC, 
               'oblique': pango.STYLE_OBLIQUE}[sValue]
        except KeyError:
    ...
    def _parse_style_margin_left(self, oTag, sValue):
        ...
        self._parse_length(sValue, False, self.__frac_length_tag_cb, oTag, 'left-margin')

    def _parse_style_margin_right(self, oTag, sValue):
        """Handle the margin right style attribute."""
        self._parse_length(sValue, False, self.__frac_length_tag_cb, oTag, 'right-margin')

    def _parse_style_font_weight(self, oTag, sValue):
        """Adjust the font to match the font weight specification."""
        try:
            iWeight = {'100': pango.WEIGHT_ULTRALIGHT, 
    ...
    def _parse_style_text_align(self, oTag, sValue):
        ...
        try:
            iAlign = {'left': gtk.JUSTIFY_LEFT, 
               'right': gtk.JUSTIFY_RIGHT, 
               'center': gtk.JUSTIFY_CENTER, 
               'justify': gtk.JUSTIFY_FILL}[sValue]
        except KeyError:
    ...
    for sStyle in ('background-color', 'color', 'font-family', 'font-size', 'font-style',
        ...
        if sStyle is None:
            ...
            for sAttr, sVal in [ item.split(':', 1) for item in sStyle.split(';') ]:
                ...
                try:
                    ...
                    warnings.warn("Style attribute '%s' requested but not yet implemented" % sAttr)
                else:
    ...
    def __parse_length_frac_cb(self, oTextView, oAllocation, fFrac, fCallback, args):
    ...
    def handle_starttag(self, sName, oAttrs):
        ...
        if sName == 'a':
            ...
            if sName == 'em' or sName == 'i':
                ...
                oTag = self._oTextBuf.create_tag()
                dFontSize = {'-2': pango.SCALE_X_SMALL, 
                   '-1': pango.SCALE_SMALL, 
                   '1': pango.SCALE_LARGE, 
                ...
                oTag.set_property('left-margin', 2.0 * iDepth * SCREEN_RESOLUTION * fFontSize)
        ...
        if sName == 'br':
            ...
            if oPixbuf is not None:
                aTags = self._get_style_tags()
                if aTags:
                    oTmpMark = self._oTextBuf.create_mark(None, self._oIter, True)
...
class HTMLTextView(gtk.TextView):
    ...
    def __motion_notify_event(self, oWidget, _oEvent):
        ...
        iXPos, iYPos = oWidget.window_to_buffer_coords(gtk.TEXT_WINDOW_TEXT, iXPos, iYPos)
...
class HTMLViewDialog(SutekhDialog):
    ...
    def __init__(self, oParent, fInput, fLinkLoader):
        """Dialog Window that wraps the HTMLTextView
           ...
           """
        super(HTMLViewDialog, self).__init__('Help', oParent, oButtons=(
         gtk.STOCK_CLOSE,
         gtk.RESPONSE_CLOSE))
        ...
        self.vbox.pack_start(self._oView, True, True)
