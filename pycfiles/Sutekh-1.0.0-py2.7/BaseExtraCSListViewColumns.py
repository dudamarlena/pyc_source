...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/base/gui/plugins/BaseExtraCSListViewColumns.py
...
from ...core.DBSignals import listen_row_destroy, listen_row_update, listen_row_created, listen_changed, disconnect_changed, disconnect_row_destroy, disconnect_row_update, disconnect_row_created
from ..CellRendererIcons import SHOW_TEXT_ONLY
from .BaseExtraColumns import BaseExtraColumns, get_number, format_number
...
class BaseExtraCSListViewColumns(BaseExtraColumns):
    ...
    MODES = {DEFAULT_MODE: SHOW_TEXT_ONLY}
    COLUMNS = {'Total cards': (100, '_render_total', '_get_data_total'), 
       'All Children': (100, '_render_all_children', '_get_data_all_children'), 
       'In-Use Children': (100, '_render_inuse_children', '_get_data_inuse_children'), 
       'Author': (300, '_render_author', '_get_data_author'), 
       'Description': (700, '_render_description', '_get_data_description')}
    CS_KEYS = ('Total Cards', )
    dCardSetListConfig = {}
    ...
    def _get_iter_data(self, oIter):
        ...
        try:
            sCardSetName = pango.parse_markup(self.model.get_value(oIter, 0))[1]
            if sCardSetName not in self._dCache:
                self._dCache[sCardSetName] = {}
                self._dCache[sCardSetName]['Card Set'] = IPhysicalCardSet(sCardSetName)
            return sCardSetName
    ...
    def _get_data_total(self, sCardSet, bGetIcons=True):
        ...
        def query(oCardSet):
            """Query the database"""
            return MapPhysicalCardToPhysicalCardSet.selectBy(physicalCardSetID=oCardSet.id).count()
    ...
    def _get_data_inuse_children(self, sCardSet, bGetIcons=True):
        ...
        def query(oCardSet):
            """Query the database"""
            return PhysicalCardSet.selectBy(parentID=oCardSet.id, inuse=True).count()
