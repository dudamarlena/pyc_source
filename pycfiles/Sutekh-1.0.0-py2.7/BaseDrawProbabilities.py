...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/base/gui/plugins/BaseDrawProbabilities.py
...
def _gen_choice_list(dSelectedCounts):
    ...
    aList = []
    aSelectOrder = sorted(dSelectedCounts.items(), key=lambda x: (x[1], x[0]), reverse=True)
...
def _multi_hyper_prob(aFound, iDraws, aObjects, iTotal):
    ...
    if sum(aFound) < 0 or min(aFound) < 0 or sum(aObjects) < 0 or iTotal <= 0 or iDraws <= 0 or min(aObjects) < 0 or sum(aObjects) > iTotal or iDraws > iTotal:
        raise RuntimeError('Invalid values for multivariate hypergeomtric probability calculation: aFound: %s iDraws: %d aObjects: %s iTotal: %d' % (
         (',').join([ str(x) for x in aFound
           ]),
...
def _hyper_prob_at_least(aFound, iDraws, aObjects, iTotal, iCurCol=0):
    ...
    for iCur in range(aFound[iCurCol], min(iDraws, aObjects[iCurCol]) + 1):
        ...
        if iCurCol < len(aFound) - 1:
            fProb += _hyper_prob_at_least(aThisFound, iDraws, aObjects, iTotal, iCurCol + 1)
        else:
...
class BaseDrawProbPlugin(BasePlugin):
    ...
    def activate(self, _oWidget):
        ...
        if 0 in self.dSelectedCounts.values():
            iRes = do_complaint_warning('Selected cards include cards with a count of 0.\nDo you want to continue?')
    ...
    def _make_dialog(self, oMainTitle):
        ...
        oDialog = SutekhDialog('Card Draw probablities', self.parent, gtk.DIALOG_DESTROY_WITH_PARENT, (
        ...
        oDescLabel = gtk.Label("Columns are number of draws (X)\nrows are the number of cards (Y)\nFor the first row (0 cards), the entry is the possiblity of not drawing any of the selected cards by the X'th draw\nFor the other rows, the entries are the probability of seeing at least the listed cards from the selection\nby the X'th draw with the number in brackets being the probablity of seeing exactly the listed cards\nThe first column represents the total starting draw")
        oTitleLabel = gtk.Label('Draw results :')
        oResultsBox = gtk.VBox(False, 2)
        oResultsBox.pack_start(oTitleLabel, False, False)
        oResultsBox.pack_start(AutoScrolledWindow(self.oResultsTable, True))
    ...
    def _fill_table(self, _oWidget):
        """Fill Results Box with the draw results.
           ...
           'connect' signal.
           """
        iNumCardRows = len([ sum(x) for x in self.aAllChoices if sum(x) <= self.iCardsToDraw
                           ])
        ...
        if iOffset > 0:
            self.oResultsTable.attach(gtk.VSeparator(), 3, 4, 2, iNumRows, xpadding=0, ypadding=0, xoptions=gtk.FILL, yoptions=gtk.FILL)
            oLabel = gtk.Label('Count')
            self.oResultsTable.attach(oLabel, 2, 3, 2, 3)
            self._setup_count_rows()
        self.oResultsTable.attach(self._get_table_draw_title(), 4 + iOffset, 5 + iOffset, 2, 3)
        self.oResultsTable.attach(gtk.HSeparator(), iOffset, iNumCols, 1, 2, xpadding=0, ypadding=0, xoptions=gtk.FILL, yoptions=gtk.FILL)
        self.oResultsTable.attach(gtk.VSeparator(), 1, 2, 1, iNumRows, xpadding=0, ypadding=0, xoptions=gtk.FILL, yoptions=gtk.FILL)
        self.oResultsTable.attach(gtk.VSeparator(), 3 + iOffset, 4 + iOffset, 2, iNumRows, xpadding=0, ypadding=0, xoptions=gtk.FILL, yoptions=gtk.FILL)
        for iCol in range(1, self.iNumSteps):
            oLabel = gtk.Label('+ %d' % (iCol * self.iDrawStep))
            iTableCol = 2 * iCol + 3 + iOffset
            self.oResultsTable.attach(gtk.VSeparator(), iTableCol, iTableCol + 1, 2, iNumRows, xpadding=0, ypadding=0, xoptions=gtk.FILL, yoptions=gtk.FILL)
            self.oResultsTable.attach(oLabel, iTableCol + 1, iTableCol + 2, 2, 3)

        aSelectOrder = sorted(self.dSelectedCounts.items(), key=lambda x: (
         x[1], x[0]), reverse=True)
        for iRow in range(iNumCardRows):
            oLabel = gtk.Label(self._gen_row_label(iRow, aSelectOrder))
            iTableRow = 2 * iRow + 3
            self.oResultsTable.attach(gtk.HSeparator(), 1 + iOffset, iNumCols, iTableRow, iTableRow + 1, xpadding=0, ypadding=0, xoptions=gtk.FILL, yoptions=gtk.FILL)
            self.oResultsTable.attach(oLabel, 2 + iOffset, 3 + iOffset, iTableRow + 1, iTableRow + 2)
    ...
    def _setup_count_rows(self):
        ...
        for iCardRow in range(self.iCardsToDraw + 1):
            self.oResultsTable.attach(gtk.HSeparator(), 1, 3, iBottomRow, iBottomRow + 1, xpadding=0, ypadding=0, xoptions=gtk.FILL, yoptions=gtk.FILL)
            oLabel = gtk.Label('%d' % iCardRow)
            iNumRows = len([ x for x in self.aAllChoices if sum(x) == iCardRow
                           ])
    ...
    def _fill_row(self, iRow, iOffset, bZero, aCardCounts):
        ...
        for iCol in range(self.iNumSteps):
            ...
            if iNumDraws < self.iTotal:
                fProbExact = _multi_hyper_prob(aThisDraw, iNumDraws, aCardCounts, self.iTotal) * 100
                if not bZero:
                    fProbAccum = _hyper_prob_at_least(aThisDraw, iNumDraws, aCardCounts, self.iTotal) * 100
            ...
            self.oResultsTable.attach(oResLabel, iTableCol, iTableCol + 1, iTableRow, iTableRow + 1)
