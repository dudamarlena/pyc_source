...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/gui/plugins/FindLikeCrypt.py
# Compiled at: 2019-12-11 16:37:54
"""Finds crypt cards 'like' the selected card"""
import gtk, gobject, pango
from sutekh.base.core.BaseTables import PhysicalCardSet, PhysicalCard, AbstractCard
from sutekh.base.core.BaseAdapters import IAbstractCard, IPhysicalCard, IPhysicalCardSet
...
from sutekh.core.Filters import MultiGroupFilter, MultiVirtueFilter, MultiDisciplineFilter, MultiDisciplineLevelFilter
...
def _make_superset(oCard, bSuperior, bVampire):
    ...
    if bSuperior:
        return set([ oP.discipline for oP in oCard.discipline if oP.level == 'superior'
                   ])
    if bVampire:
...
class FindLikeVampires(SutekhPlugin):
    ...
    def _group_cards(self, aCards, iNum, aSubSets, bSuperior, bUseCardSet):
        ...
        dResults = {}
        aDistinct.remove(self.oSelCard)
        bVampire = is_vampire(self.oSelCard)
        if bUseCardSet:
            aCardSetCards = set([ IAbstractCard(x) for x in self.model.get_card_iterator(self.model.get_current_filter())
                                ])
        else:
            aCardSetCards = None
        ...
        for oCard in aDistinct:
            ...
            for aSet in aSubSets:
                if aSet.issubset(aFullSet):
                    dResults.setdefault(make_key(aSet, bSuperior), []).append(oCard)
    ...
    def find_vampires_like(self):
        ...
        aFilters = [
         CardTypeFilter('Vampire'), _get_groups(self.oSelCard)]
        oDialog = SutekhDialog('Find Vampires like', self.parent, gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT, (
         gtk.STOCK_OK, gtk.RESPONSE_OK,
         gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL))
        oDialog.vbox.pack_start(gtk.Label('Find Vampires like %s' % self.oSelCard.name), False, False)
        aDisciplines = [ oP.discipline for oP in self.oSelCard.discipline ]
        aSuperior = [ oP.discipline for oP in self.oSelCard.discipline if oP.level == 'superior'
                    ]
        ...
        if aSuperior:
            ...
            oSuperior = gtk.RadioButton(oDisciplines, 'Match Superior Disciplines')
        ...
        if oSuperior:
            ...
            oDisciplines.connect('toggled', self._update_combo_box, oComboBox, aDisciplines, aSuperior)
        ...
        if iRes == gtk.RESPONSE_CANCEL:
            ...
            if bSuperior:
                oDisciplineFilter = MultiDisciplineLevelFilter([ (x.fullname, 'superior') for x in aSuperior ])
                aSets = _gen_subsets(aSuperior, iNum)
            else:
                oDisciplineFilter = MultiDisciplineFilter([ x.fullname for x in aDisciplines ])
            ...
            return self._group_cards(aCandCards, iNum, aSets, bSuperior, bUseCardSet)

    def find_imbued_like(self):
        """Construct a imbued search from the card"""
        aFilters = [
         CardTypeFilter('Imbued'), _get_groups(self.oSelCard)]
        oDialog = SutekhDialog('Find Imbued like', self.parent, gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT, (
         gtk.STOCK_OK, gtk.RESPONSE_OK,
         gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL))
        oDialog.vbox.pack_start(gtk.Label('Find Imbued like %s' % self.oSelCard.name), False, False)
        oUseCardSet = gtk.CheckButton('Only match cards visible in this pane')
        oDialog.vbox.pack_start(oUseCardSet)
        ...
        if iRes == gtk.RESPONSE_CANCEL:
            ...
            if oUseCardSet.get_active():
                bUseCardSet = True
            iNum = int(oComboBox.get_active_text())
            oVirtueFilter = MultiVirtueFilter([ x.fullname for x in self.oSelCard.virtue
                                              ])
            ...
            oDialog.destroy()
            aCandCards = list(oFullFilter.select(AbstractCard))
            return self._group_cards(aCandCards, iNum, aSets, False, bUseCardSet)

    def _update_combo_box(self, oDiscipline, oComboBox, aDisciplines, aSuperior):
    ...
    def display_results(self, dGroups):
        ...
        oResults = NotebookDialog(sTitle, self.parent, gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT, (
         gtk.STOCK_CLOSE, gtk.RESPONSE_CLOSE))
        oResults.set_size_request(700, 600)
        oAllView = LikeCardsView(dGroups['all'], bVampire)
        oResults.add_widget_page(AutoScrolledWindow(oAllView), 'All Matches')
...
class LikeCardsView(gtk.TreeView):
    ...
    VAMP_LABELS = [
     'Name', 'Group', 'Capacity', 'Clan', 'Disciplines']
...
class LikeCardsModel(gtk.ListStore):
    ...
    def __init__(self, aCards, bVampire):
        super(LikeCardsModel, self).__init__(gobject.TYPE_STRING, gobject.TYPE_INT, gobject.TYPE_INT, gobject.TYPE_STRING, gobject.TYPE_STRING)
    ...
    def add_card(self, oCard):
        ...
        if self.bVampire:
            ...
            aDis = [ oP.discipline.name.upper() for oP in oCard.discipline if oP.level == 'superior'
                   ]
            aDis.extend([ oP.discipline.name.lower() for oP in oCard.discipline if oP.level != 'superior'
                        ])
            aDis.sort()
