...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/base/gui/DataFilesDialog.py
...
class DataFilesDialog(SutekhDialog):
    ...
    def __init__(self, oParent, tReaders, bDisplayZip, sZippedUrl, bDisableBackup):
        super(DataFilesDialog, self).__init__('Choose CardList Files', oParent, gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT, (
        ...
        for oReader in tReaders:
            ...
            if oReader.bRequired:
                oFileLabel.set_markup('<b>%s (required)</b>' % oReader.sDescription)
            else:
                oFileLabel.set_markup('<b>%s</b> (optional)' % oReader.sDescription)
            oFileButton = SutekhFileButton(oParent, oReader.sDescription)
            oFileButton.add_filter_with_pattern(*oReader.tPattern)
            if oReader.sUrl:
                oUrlButton = gtk.CheckButton('Use default url (from %s)' % get_domain(oReader.sUrl))
            ...
            self._dFileWidgets[oReader.sName] = WidgetHolder(oFileAlign, oFileButton, oUrlButton, oReader.sUrl)
        ...
        if bDisableBackup:
            ...
            self.oBackupFileLabel.set_sensitive(False)
        self.oBackupFileDialog = SutekhFileDialog(oParent, 'Database Backup file', gtk.FILE_CHOOSER_ACTION_SAVE, (
        ...
        if bDisplayZip:
            ...
            if sZippedUrl:
                oZipUrlCheckBox = gtk.CheckButton('Use default url (from %s)' % get_domain(sZippedUrl))
                oZipAlign = make_alignment(oZipLabel, oZipButton, oZipUrlCheckBox)
                oZipUrlCheckBox.set_active(True)
            else:
                oZipUrlCheckBox = None
                oZipAlign = make_alignment(oZipLabel, oZipButton)
            self._dFileWidgets[COMBINED_ZIP] = WidgetHolder(oZipAlign, oZipButton, oZipUrlCheckBox, sZippedUrl)
        ...
        if iResponse == gtk.RESPONSE_OK:
            ...
            for sName, oWidgets in self._dFileWidgets.items():
                ...
                if oWidgets.oUrlButton and oWidgets.oUrlButton.get_active():
                    self._dChoices[sName] = Result(bIsUrl=True, sName=oWidgets.sUrl)
                else:
                    self._dChoices[sName] = Result(bIsUrl=False, sName=oWidgets.oFileWidget.get_filename())
