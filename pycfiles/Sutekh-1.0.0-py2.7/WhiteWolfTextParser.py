...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/io/WhiteWolfTextParser.py
...
def _find_sect_and_title(aLines):
    ...
    if aLines[0].find('Camarilla') != -1:
        ...
        if sLowerLine.find('sabbat archbishop of') != -1:
            ...
            sTitle = 'Regent'
    elif aLines[0].find('Anarch') != -1 or aLines[0].find('Independent anarch') != -1:
        sSect = 'Anarch'
        try:
            oBaronTitle = re.compile('[aA]narch Baron of')
        ...
        try:
            ...
            if oMatch is not None and not aLines[1].startswith('[MERGED]'):
                ...
                if oMergedMatch is not None and oMergedMatch.groups()[0] == oMatch.groups()[0]:
                    pass
                else:
...
class CardDict(dict):
    ...
    oDisGaps = re.compile('[\\\\\\/{}\\&\\s]+')
    oWhiteSp = re.compile('[{}\\s]+')
    oDispCard = re.compile('\\[[^\\]]+\\]$')
    oArtistSp = re.compile('[&;]')
    oCryptInfoRgx = re.compile('[:\\.] ([+-]\\d) (bleed|strength|stealth|intercept)(?:(?=\\.)|$)')
    dCryptProperties = {'black hand': re.compile('[:.] Black Hand'), 
       ...
       'tremere slave': re.compile('Tremere [Ss]lave[:.]'), 
       'tremere antitribu slave': re.compile('Tremere antitribu [Ss]lave[:.]'), 
       'malkavian antitribu slave': re.compile('Malkavian antitribu slave')}
    dCryptFullTextProperties = {'not for legal play': re.compile('NOT FOR LEGAL PLAY|Added to the V:EKN banned list')}
    dLibProperties = {'not for legal play': re.compile('NOT FOR LEGAL PLAY|Added to the V:EKN banned list')}
    dAllyProperties = {'red list': re.compile('\\. Red List\\.'), 
       'unique': re.compile('Unique [A-Za-z ]+ with \\d life')}
    oLifeRgx = re.compile('(Unique )?\\[?(Gargoyle creature|[A-Za-z]+)\\]? with (\\d) life\\.')
    dEquipmentProperties = {'unique': re.compile('Unique (melee )?weapon|Unique equipment|represents a unique location|Unique Nod|^Unique.|this is a unique location|as equipment (while|when) in play. (Haven. )?Unique.'), 
       'location': re.compile('represents a (unique )?location|this is a (unique )?location'), 
       'melee weapon': re.compile('[mM]elee weapon\\.'), 
       'cold iron': re.compile('weapon\\. Cold iron\\.'), 
       'gun': re.compile('[wW]eapon[:,.] [gG]un\\.'), 
       'weapon': re.compile('[wW]eapon[:,.] [gG]un\\.|[mM]elee weapon\\.|Weapon.|Unique weapon.'), 
       'vehicle': re.compile('Vehicle\\.'), 
       'haven': re.compile('Haven\\.'), 
       'electronic equipment': re.compile('Electronic equipment.|^Electronic\\.')}
    dMasterProperties = {'unique': re.compile('[Uu]nique [mM]aster|Master[:.] unique|Unique\\.|Unique location\\.|Unique contract\\.'), 
       'trifle': re.compile('[tT]rifle\\.'), 
       'discipline': re.compile('Master: Discipline\\.|^Discipline\\.'), 
       'out-of-turn': re.compile('Master: out-of-turn|Out-of-turn\\.'), 
       'location': re.compile('Master[:.] (unique )?[Ll]ocation|Unique location\\.|^Location\\.'), 
       ...
       'archetype': re.compile('Master: archetype|Archetype\\.'), 
       'watchtower': re.compile('Master: watchtower'), 
       'title': re.compile('Title\\.'), 
       'contract': re.compile('Contract\\.|Unique contract\\.|Master: contract\\.')}
    dEventProperties = {'gehenna': re.compile('Gehenna\\.'), 
    ...
    dOtherProperties = {'contract': re.compile('Contract\\.'), 
       'unique': re.compile('Unique\\.'), 
       'boon': re.compile('Boon\\.'), 
       'watchtower': re.compile('Watchtower\\.'), 
       'frenzy': re.compile('Frenzy\\.'), 
       'title': re.compile('Title\\.')}
    dAllyKeywordSpecial = {'Gypsies': [
                 '1 stealth'], 
       'Veneficti': [
                   '1 stealth'], 
       'High Top': [
       ...
       'Ghoul Retainer': [
                        '1 strength']}
    dCryptKeywordSpecial = {'Spider-Killer': {'stealth': 1}, 'Muaziz, Archon of Ulugh Beg': {'stealth': 1}, 'Rebekka, Chantry Elder of Munich': {'stealth': 1}}
    ...
    def _find_crypt_keywords(self, oCard):
        ...
        if self['name'] in self.dCryptKeywordSpecial:
            for sKeyword, iVal in self.dCryptKeywordSpecial[self['name']].iteritems():
    ...
    def _parse_text(self, oCard):
        ...
        if 'cardtype' in self:
            ...
            for sVal in sTypes.split('/'):
                if sVal in ('Vampire', 'Imbued', 'Ally', 'Retainer', 'Equipment', 'Master',
                            'Event'):
                    sType = sVal
    ...
    def _make_aliases(self):
        ...
        for sAlias in self['aka'].split(';'):
            oLookup = self._oMaker.make_lookup_hint('CardNames', sAlias.strip(), self['name'])
            oLookup.syncUpdate()
    ...
    def _add_cost(self, oCard, sCost):
        ...
        if sAmnt.lower() == 'x':
            ...
            iCost = int(sAmnt, 10)
        oCard.cost = iCost
        oCard.costtype = str(sType.lower())
    ...
    def _fix_advanced_name(self):
        ...
        if self['name'].endswith(' (Adv)'):
            self['level'] = 'advanced'
            sAlias = self['name']
            self['name'] = self['name'].replace(' (Adv)', ' (Advanced)')
        ...
        if sAlias:
            for sLookup in [sAlias, sAlias.replace('(Adv)', '(ADV)')]:
                oLookup = self._oMaker.make_lookup_hint('CardNames', sLookup, self['name'])
                oLookup.syncUpdate()
...
class InCardText(LogStateWithInfo):
    ...
    def transition(self, sLine, _dAttr):
        ...
        if sLine.startswith('Artist:') or not sLine.strip():
            self._dInfo['text'] = fix_clarification_markers(self._dInfo['text'].strip())
...
class WhiteWolfTextParser(object):
    ...
    def parse(self, fIn):
        ...
        if hasattr(self._oState, 'flush'):
            ...
            raise IOError('Failed to parse card list - unexpected state at end of file.\nCard list probably truncated.')

    def feed(self, sLine):
        """Feed the line to the current state"""
        sLine = sLine.decode('utf8').lstrip('\ufeff')
