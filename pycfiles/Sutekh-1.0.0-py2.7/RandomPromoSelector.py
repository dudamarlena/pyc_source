...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/gui/plugins/RandomPromoSelector.py
...
class RandomPromoDialog(SutekhDialog):
    ...
    def __init__(self, oParent, aCards):
        super(RandomPromoDialog, self).__init__('Generate random groups of cards', oParent, gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT, (
         gtk.STOCK_OK, gtk.RESPONSE_OK,
         gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL))
        oCardsPerGroupAdj = gtk.Adjustment(value=2, lower=1, upper=100, step_incr=1)
        oNumberOfGroupsAdj = gtk.Adjustment(value=10, lower=1, upper=100, step_incr=1)
        self._aCards = aCards
        ...
        oHbox = gtk.HBox()
        oHbox.pack_start(gtk.Label('Cards per group:'), expand=False, padding=5)
        oHbox.pack_start(self._oCardsPerGroup, expand=False)
        self.vbox.pack_start(oHbox, expand=False)
        oHbox = gtk.HBox()
        oHbox.pack_start(gtk.Label('Number of groups:'), expand=False, padding=5)
...
class RandomPromoSelector(SutekhPlugin):
    ...
    def activate(self, _oWidget):
        ...
        aCards = [ IAbstractCard(oCard) for oCard in self.model.get_card_iterator(oFilter)
                 ]
