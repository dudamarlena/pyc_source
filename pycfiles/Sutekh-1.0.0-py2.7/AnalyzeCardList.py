...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/gui/plugins/AnalyzeCardList.py
...
from sutekh.base.core.BaseAdapters import IAbstractCard, IPhysicalCard, IKeyword
...
RT_BANNED = 'RT:Banned'
ILLEGAL_LOOKUP = {RT_BANNED: 'Cards Banned in Rapid Thoughts', 
...
def _disc_sort_key(tData):
    ...
    return (
     -tData[1][1], -tData[1][2], tData[0].fullname)
...
def _get_card_costs(aCards):
    ...
    return (
     dCosts['blood'], dCosts['pool'], dCosts['conviction'])
...
def _format_multi(sCardType, dMulti, iNum):
    ...
    sText = ''
    for sType, iMulti in sorted(dMulti.items(), key=lambda x: x[1], reverse=True):
        sPer = _percentage(iMulti, iNum, '%s cards' % sCardType)
        sText += '%(num)d %(type)s cards are %(multitype)s cards %(per)s\n' % {'num': iMulti, 'type': sCardType, 'multitype': sType, 
           'per': sPer}
...
def _simple_back_checks(dCards, sType):
    ...
    if 'PDF' in dCards:
        ...
        if len(dCards) == 1 and None not in dCards:
            ...
            return sText
        if None in dCards:
            sText = '%s has cards from unspecified expansions. Ignoring the %s\n' % (
             sType, sType.lower())
            return sText
        return
...
def _percentage_backs(dCards, iSize, fPer, sType):
    ...
    for sGroupName in dCards:
        ...
        sText += '%d %s backs (%2.1f%% of the %s).\n' % (dCards[sGroupName],
         ...
         sType.lower())
        if fThisPer < fPer:
            sText += "Group of cards with the same back that's smaller than %2.1f%% of the %s.\n" % (
             fPer, sType.lower())
            bOK = False
...
def _group_backs(dCards, aCards, iNum, dBacks):
    ...
    for sGrpName in dCards:
        aCardsByExp.append([ oCard for oCard in aCards if dBacks[oCard.printing] == sGrpName
                           ])

    for aExpCards in aCardsByExp:
        aNames = set([ x.abstractCard.name for x in aExpCards ])
        if len(aNames) < iNum:
            sText += 'Group of fewer than %d different cards with the same back.\n' % iNum
            sText += '\t' + (', ').join(aNames) + '\n'
            bOK = False
...
class AnalyzeCardList(SutekhPlugin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._dConstruct = {'Vampire': self._process_vampire, 
    ...
    def activate_heart(self, bRapid=False):
        ...
        if not self._check_cs_size('Analyze Deck', 500):
            ...
            oDlg = NotebookDialog('Analysis of Card List', self.parent, gtk.DIALOG_DESTROY_WITH_PARENT, (
             gtk.STOCK_CLOSE, gtk.RESPONSE_CLOSE))
            oDlg.connect('response', lambda oDlg, resp: oDlg.destroy())
            self.dTypeNumbers = {}
            dCardLists = {}
            aAllPhysCards = [ IPhysicalCard(x) for x in self.model.get_card_iterator(None)
                            ]
            aAllCards = _get_abstract_cards(aAllPhysCards)
            for sCardType in self._dConstruct:
                if sCardType not in SPECIAL:
                    dCardLists[sCardType] = _get_abstract_cards(self.model.get_card_iterator(CardTypeFilter(sCardType)))
                    ...
                    dCardLists[sCardType] = _get_abstract_cards(self.model.get_card_iterator(FilterNot(self.model.oLegalFilter)))
                    self.dTypeNumbers[sCardType] = len(dCardLists[sCardType])
                    if bRapid:
                        try:
                            for sType in [RT_BANNED, RT_WATCHLIST]:
                                dCardLists[sType] = _get_abstract_cards(self.model.get_card_iterator(KeywordFilter(IKeyword(sType))))
                                self.dTypeNumbers[sType] = len(dCardLists[sType])
            ...
            aOrderToList = sorted(CRYPT_TYPES) + [ x for x in sorted(self.dTypeNumbers) if x not in CRYPT_TYPES and x not in SPECIAL and x not in [RT_BANNED, RT_WATCHLIST] ] + sorted(SPECIAL)
            for sCardType in aOrderToList:
                if self.dTypeNumbers[sCardType]:
                    fProcess = self._dConstruct[sCardType]
                    oDlg.add_widget_page(wrap(fProcess(dCardLists[sCardType])), sCardType)

            if bRapid:
                fProcess = self._dConstruct['Not Tournament Legal Cards']
                for sType in [RT_BANNED, RT_WATCHLIST]:
                    if self.dTypeNumbers[sType]:
                        oDlg.add_widget_page(wrap(fProcess(dCardLists[sType], sType)), ILLEGAL_LOOKUP[sType])
    ...
    def get_crypt_stats(self, aVampireCards, aImbuedCards):
        ...
        get_info([ x.group for x in aVampireCards if x.group != -1 ], [ x.group for x in aImbuedCards if x.group != -1 ], 'group')
        get_info([ x.capacity for x in aVampireCards ], [ x.life for x in aImbuedCards ], 'cost')
        aAllCosts = sorted([ x.capacity for x in aVampireCards ] + [ x.life for x in aImbuedCards ])
    ...
    def _prepare_main(self, bRapid):
        ...
        if self.dTypeNumbers['Vampire'] > 0 and self.dTypeNumbers['Imbued'] > 0 or self.iCryptSize == 0:
        ...
        if bRapid:
            if self.iCryptSize < 6:
                sMainText += '<span foreground = "red">Less than 6 Crypt Cards</span>\n'
        ...
        if self.dTypeNumbers['Not Tournament Legal Cards'] > 0:
            sMainText += '<span foreground = "red">Card Set uses cards that are not legal for tournament play</span>\n'
        if bRapid and self.dTypeNumbers[RT_BANNED]:
            sMainText += '<span foreground = "red">Card Set uses cards that are not legal for rapid thought play</span>\n'
        if bRapid and self.dTypeNumbers[RT_WATCHLIST]:
            sMainText += '<span foreground = "yellow">Card Set uses cards that are on the rapid thoughts watchlist</span>\n'
        if self.iCryptSize > 0:
            sMainText += '\nMaximum cost in crypt = %d\n' % self.dCryptStats['max cost']
            sMainText += 'Minimum cost in crypt = %d\n' % self.dCryptStats['min cost']
        fAvg, fAvgDraw = _crypt_avg(self.dCryptStats['total cost'], self.iCryptSize)
        ...
        if self.iLibSize < 40:
            sMainText += '<span foreground = "red">Less than 40 Library Cards</span>\n'
        elif self.iLibSize < 60 and not bRapid:
            sMainText += '<span foreground = "orange">Less than 60 Library Cards - this deck is not legal for standard constructed tournaments</span>'
        elif self.iLibSize > 90:
            sMainText += '<span foreground = "orange">More than 90 Library Cards - this deck is not legal for standard constructed tournaments</span>\n'
        return (
         wrap(sTitleText), oScrolledBox, wrap(sMainText))

    def _process_library(self):
        """Create a notebook for the basic library card overview"""
        oLibNotebook = gtk.Notebook()
        sTypeText = ''
        for sType, iCount in sorted(self.dTypeNumbers.items(), key=lambda x: (
         x[1], x[0]), reverse=True):
            if sType not in CRYPT_TYPES and sType not in SPECIAL and iCount > 0:
                sTypeText += _format_card_line(sType, 'cards', iCount, self.iLibSize)

        if self.dTypeNumbers['Multirole'] > 0:
            sTypeText += '\n' + _format_card_line('Multirole', 'cards', self.dTypeNumbers['Multirole'], self.iLibSize)
        oLibNotebook.append_page(wrap(sTypeText), gtk.Label('Card Types'))
        sDiscText = _format_card_line('Master', 'cards', self.dTypeNumbers['Master'], self.iLibSize)
        sDiscText += _format_card_line('non-master cards with No discipline requirement', '', self.dLibStats['discipline']['No Discipline'], self.iLibSize) + '\n'
        for sDisc, iNum in sorted(self.dLibStats['discipline'].items(), key=lambda x: (
         x[1], x[0]), reverse=True):
            if sDisc != 'No Discipline' and iNum > 0:
                sDiscDesc = 'non-master cards with %s' % sDisc
                sDiscText += _format_card_line(sDiscDesc, '', iNum, self.iLibSize)

        oLibNotebook.append_page(wrap(sDiscText), gtk.Label('Disciplines'))
        sClanText = _format_card_line('cards with No clan requirement', '', self.dLibStats['clan']['No Clan'], self.iLibSize) + '\n'
        for sClan, iNum in sorted(self.dLibStats['clan'].items()):
            if sClan != 'No Clan' and iNum > 0:
                sClanDesc = 'cards requiring %s' % sClan
                sClanText += _format_card_line(sClanDesc, '', iNum, self.iLibSize)

        oLibNotebook.append_page(wrap(sClanText), gtk.Label('Clan Requirements'))
        return oLibNotebook

    def _process_vampire(self, aCards):
        """Process the list of vampires"""
        dDeckDetails = {'vampires': {}, 'titles': {}, 'clans': {}, 'sects': {}, 'votes': 0}
        ...
        sVampText += '%d vampires with titles (%s)\n' % (
         iTotalTitles,
         _percentage(iTotalTitles, self.iCryptSize, 'Crypt'))
        sVampText += '%d votes from titles in the crypt. Average votes per vampire is %2.3f\n' % (
         dDeckDetails['votes'],
         dDeckDetails['votes'] / float(iNum))
        sVampText += '\n<span foreground = "blue">Disciplines</span>\n'
        for oDisc, aInfo in sorted(self.dCryptStats['crypt discipline'].iteritems(), key=_disc_sort_key):
            if aInfo[0] == 'discipline':
                sVampText += '%(infcount)d Vampires with %(disc)s %(iper)s, %(supcount)d at Superior %(sper)s\n' % {'disc': oDisc.fullname, 
                   'infcount': aInfo[1], 
                   'iper': _percentage(aInfo[1], self.iCryptSize, 'Crypt'), 
                   'supcount': aInfo[2], 
                   'sper': _percentage(aInfo[2], self.iCryptSize, 'Crypt')}
    ...
    def _process_imbued(self, aCards):
        ...
        for oVirtue, aInfo in sorted(self.dCryptStats['crypt discipline'].iteritems(), key=_disc_sort_key):
            if aInfo[0] == 'virtue':
                sImbuedText += '%d Imbued with %s %s\n' % (
                 aInfo[1], oVirtue.fullname,
                 _percentage(aInfo[1], self.iCryptSize, 'Crypt'))
    ...
    def _process_master(self, aCards):
        ...
        for sType in ('trifle', 'out-of-turn'):
            try:
                ...
                sText += '<span foreground="red">Keyword <b>%s</b> not in the database -- please re-import the WW cardlist</span>\n' % sType
    ...
    def _default_text(self, aCards, sType):
        ...
        if aConviction[1] > 0:
            sText += _format_cost_numbers(sType, 'conviction', aConviction, iNum)
        if aPool[1] > 0:
    ...
    def _process_multi(self, aCards):
        ...
        dMulti = {}
        sPerCards = _percentage(self.dTypeNumbers['Multirole'], self.iLibSize, 'Library')
        ...
        for sMultiType, iNum in sorted(dMulti.items(), key=lambda x: x[1], reverse=True):
    ...
    def _process_non_legal(self, aCards, sType='Not Tournament Legal Cards'):
        ...
        sText = '\t\t<b>%(type)s :</b>\n\nNumber of cards %(lowertype)s = %(num)d %(per)s\n' % {'type': ILLEGAL_LOOKUP[sType], 
        ...
        for sName, iNum in sorted(dNonLegal.items(), key=lambda x: x[1], reverse=True):
    ...
    def _check_crypt_backs(self, dCrypt, aCrypt):
        ...
        if sAddText:
            sText += sAddText
            return sText
        sAddText, bOK = _percentage_backs(dCrypt, self.iCryptSize, 20.0, 'Crypt')
        if sAddText:
            sText += sAddText
    ...
    def _check_lib_backs(self, dLib, aLib):
        ...
        for aExpCards in aLibByExp:
            ...
            if len(aAllTypes) < 3:
                aNames = set([ x.abstractCard.name for x in aExpCards ])
                sText += 'Group of cards with the same back containing less than 3 different card types.\n'
                sText += '\t' + (', ').join(aNames) + '\n'
                bOK = False
    ...
    def _process_backs(self, aPhysCards):
        ...
        if 'PDF' in dCrypt or 'PDF' in dLib:
            sText += 'Cards from a V:EKN released pdf set in the deck. The V:EKN requires that these cards be sleeved in opaque sleeves with a existing VtES card\n'
        else:
            if None in dCrypt and None in dLib:
                sText += 'Both library and crypt have cards with unspecified expansions.\nIgnoring the mixed backs.\n'
                return sText
            sText += "Mixed card backs in the deck. The V:EKN rules require that a mixed deck be of 'sufficiently mixed card types' if it's to be played without sleeves. This tests some obvious cases, but check with the event judge if playing without sleeves\n\n"
    ...
    def happy_families_init(self, oHFVBox, oDlg):
        ...
        sHappyFamilyText += '\t<small><i>Happy Families Analysis from the Gangrel antitribu newsletter, Jan 2000, by Legbiter</i></small>\n'
        ...
        if self.dTypeNumbers['Event'] > 0:
            sHappyFamilyText += '\n<span foreground = "red">Events are not considered by Happy Families. We treat them as additional masters.</span>'
        if self.iCryptSize == 0:
        ...
        if len(self.dCryptStats['crypt discipline']) < 1:
            sHappyFamilyText += '\n<span foreground = "red">Need disciplines in the crypt to do analysis</span>\n'
            oMainLabel.set_markup(sHappyFamilyText)
            oHFVBox.show_all()
            return
        iHFMasters = int(round(0.2 * self.iLibSize))
        iLibMasters = self.dTypeNumbers['Master'] + self.dTypeNumbers['Event']
        iNonMasters = self.iLibSize - self.dTypeNumbers['Master'] - self.dTypeNumbers['Event']
        sHappyFamilyText += '\n\t<b>Master Cards</b>\n'
        sHappyFamilyText += '%d Masters %s\n' % (
         self.dTypeNumbers['Master'],
         _percentage(self.dTypeNumbers['Master'], self.iLibSize, 'Library'))
        if self.dTypeNumbers['Event'] > 0:
            sHappyFamilyText += '%d Events %s\n' % (
             self.dTypeNumbers['Event'],
             _percentage(self.dTypeNumbers['Event'], self.iLibSize, 'Library'))
            sHappyFamilyText += '%d Total %s\n' % (
             iLibMasters,
             _percentage(iLibMasters, self.iLibSize, 'Library'))
        sHappyFamilyText += 'Happy Families recommends 20%, which would be ' + str(iHFMasters) + '  : '
        sHappyFamilyText += '<span foreground = "blue">Difference = ' + str(abs(iHFMasters - iLibMasters)) + '</span>\n'
        aSortedDiscs = [ x[0].fullname for x in sorted(self.dCryptStats['crypt discipline'].items(), key=_disc_sort_key)
                       ]
        oMainLabel.set_markup(sHappyFamilyText)
        ...
        oButton.connect('clicked', self._redo_happy_family, oDiscSelect, oResLabel)
        oHFVBox.pack_start(oButton, False, False)
        oResLabel.set_markup(self._happy_lib_analysis(aSortedDiscs[:2], iNonMasters))
    ...
    def _happy_lib_analysis(self, aDiscsToUse, iNonMasters):
        ...
        for sDisc in aDiscsToUse:
            oDisc = _lookup_discipline(sDisc, self.dCryptStats['crypt discipline'])
        ...
        if iDiff > 0:
            iHFNoDiscipline += iDiff
        sHappyFamilyText += 'Number of Minion cards requiring  No discipline : %s\n' % (self.dLibStats['discipline']['No Discipline'] - self.dTypeNumbers['Master'] - self.dTypeNumbers['Event'])
        sHappyFamilyText += 'Happy Families recommends %d (%.1f %%): ' % (
         iHFNoDiscipline,
         80 * self.iCryptSize / fDemon)
        sHappyFamilyText += '<span foreground = "blue">Difference = %s</span>\n\n' % abs(iHFNoDiscipline - (self.dLibStats['discipline']['No Discipline'] - self.dTypeNumbers['Master'] - self.dTypeNumbers['Event']))
        for sDisc in aDiscsToUse:
            ...
            if sDisc in self.dLibStats['discipline']:
                ...
                iLibNum = 0
            sHappyFamilyText += 'Number of Minion cards requiring %(disc)s : %(lib)d (%(crypt)d crypt members)\n' % {'disc': sDisc, 
               'lib': iLibNum, 
               'crypt': dCryptDiscs[sDisc]}
            sHappyFamilyText += 'Happy Families recommends %d (%.1f %%): ' % (
             iHFNum, 80 * dCryptDiscs[sDisc] / fDemon)
            sHappyFamilyText += '<span foreground = "blue">Difference = %d </span>\n' % abs(iHFNum - iLibNum)
