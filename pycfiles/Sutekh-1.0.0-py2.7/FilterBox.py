...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/base/core/FilterBox.py
# Compiled at: 2019-12-11 16:37:48
"""FilterBox model of the filters. Mainly used in filter editor"""
from .FilterParser import FilterNode, BinOpNode, NotOpNode, FilterPartNode, escape
...
class FilterBoxModel(list):
    ...
    def __init__(self, oAST, sFilterType, oVarNameMaker=None):
        ...
        elif isinstance(oAST, NotOpNode) and isinstance(oAST.oSubExpression, BinOpNode):
            self.bNegate = True
            self.sBoxType = oAST.oSubExpression.oOp
            self._init_binop(oAST.oSubExpression)
        elif isinstance(oAST, FilterPartNode) or isinstance(oAST, NotOpNode) and isinstance(oAST.oSubExpression, FilterPartNode):
            self.sBoxType = self.AND
            self.append(FilterBoxItem(oAST, self.oVarNameMaker))
        elif oAST is None:
            self.sBoxType = self.AND
        else:
            raise ValueError('FilterBoxModel cannot represent AST %s of type %s' % (
             oAST, type(oAST)))
    ...
    def _init_binop(self, oBinOp):
        ...
        for oChild in (oBinOp.oLeft, oBinOp.oRight):
            ...
            elif isinstance(oChild, NotOpNode) and isinstance(oChild.oSubExpression, BinOpNode):
                self.append(FilterBoxModel(oChild, self.sFilterType, self.oVarNameMaker))
            elif isinstance(oChild, (FilterPartNode, NotOpNode)):
                self.append(FilterBoxItem(oChild, self.oVarNameMaker))
            else:
                raise ValueError('FilterBoxModel encountered unsupported AST node type %s (%s) while examing BinOpNode tree.' % (
                 type(oChild), oChild))
...
class FilterBoxItem(object):
    ...
    def get_current_values(self):
        ...
        if self.aCurValues:
            if self.iValueType == self.LIST:
                dVars[self.sVariableName] = [ '"%s"' % escape(sValue) for sValue in self.aCurValues ]
                ...
                if aFrom:
                    ...
                    if aValues:
                        ...
                        dVars[self.sVariableName] = [
                         aValues, aFrom]
            elif self.iValueType == self.ENTRY:
                dVars[self.sVariableName] = ['"%s"' % escape(self.aCurValues[0])]
        return dVars

    def get_ast(self):
        """Return an AST representation of the filter."""
        if not self.aCurValues and self.iValueType != self.NONE or self.bDisabled:
            return
        oAST = FilterPartNode(self.sFilterName, None, self.sVariableName)
        if self.bNegated:
            return NotOpNode(oAST)
        else:
            return oAST
    ...
    def get_text(self):
        ...
        if self.iValueType == self.NONE:
            ...
            if self.iValueType == self.LIST:
                sValues = (',').join([ '"%s"' % escape(sValue) for sValue in self.aCurValues
                                     ])
            elif self.iValueType == self.LIST_FROM:
                aValues, aFrom = self.aCurValues
                sFromValues = '"-1"'
                sFrom = '""'
                if aValues:
                    sFromValues = (',').join([ '"%s"' % escape(x) for x in aValues
                                             ])
                if aFrom:
...
BOXTYPE = {'All of ...': (
                FilterBoxModel.AND, False), 
   'Any of ...': (
                FilterBoxModel.OR, False), 
   'Not all of ...': (
                    FilterBoxModel.AND, True), 
   'None of ...': (
                 FilterBoxModel.OR, True)}
BOXTYPE_ORDER = [
