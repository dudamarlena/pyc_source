...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/base/gui/plugins/BaseCSVImport.py
# Compiled at: 2019-12-11 16:37:39
"""plugin for managing CSV file imports"""
import csv, gtk, gobject
...
class BaseCSVImport(BasePlugin):
    ...
    def make_dialog(self):
        """Create the dialog.
           ...
           """
        self.oDlg = SutekhDialog('Choose CSV File', None, gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT, (
        ...
        self.oFileChooser.connect('selection-changed', self._selected_file_changed)
    ...
    def _selected_file_changed(self, oFileChooser):
        ...
        if bHasHeader:
            aColumns = [
             (-1, '-')] + list(enumerate(aRow))
        else:
            aColumns = [
             (-1, '-')] + [ (i, str(i)) for i in range(len(aRow)) ]
    ...
    def handle_response(self, _oWidget, oResponse):
        ...
        if oResponse == gtk.RESPONSE_OK:
            ...
            if iCardNameColumn is None or iCountColumn is None:
                ...
                self.oDlg.run()
                return
            oParser = CSVParser(iCardNameColumn, iCountColumn, iExpansionColumn, bHasHeader=True)
