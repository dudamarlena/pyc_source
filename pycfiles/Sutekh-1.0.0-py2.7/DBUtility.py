...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/base/core/DBUtility.py
...
def make_adapter_caches():
    ...
    aCachedAdapters = [ x for x in find_subclasses(Adapter) if hasattr(x, 'make_object_cache')
                      ]
...
def refresh_tables(aTables, oConn, bMakeCache=True):
    ...
    if not oVerHandler.set_version(VersionTable, VersionTable.tableversion, oConn):
        return False
    for cCls in aTables:
...
def get_metadata_date(sKey):
    ...
    try:
        ...
        logging.warn('Failed to find a valid date for %s (err %s) - returning None', sKey, oErr)
