...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/gui/plugins/SecretLibrary.py
# Compiled at: 2019-12-11 16:37:54
"""Plugin for interacting with the Secret Library website."""
import urllib, StringIO, re, logging, gtk, keyring
...
class ImportExportBase(SutekhDialog):
    ...
    def _setup_vbox(self, sTitlePhrase, sSourcePhrase, aDeckWidgets, aInvWidgets, sUsername, sPassword):
        ...
        self.vbox.pack_start(gtk.Label('%s ...' % (sSourcePhrase,)), False, False)
    ...
    def get_auth(self):
        ...
        return (
         self._oUsernameEntry.get_text(), self._oPasswordEntry.get_text())
...
class ImportDialog(ImportExportBase):
    ...
    def __init__(self, oParent, sUsername, sPassword):
        super(ImportDialog, self).__init__('Import from Secret Library', oParent, gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT, (
        ...
        aDeckWidgets = [
         ...
         self._oDeckId]
        self._setup_vbox('Import from', 'Import from', aDeckWidgets, [], sUsername, sPassword)
        self.show_all()
...
class ExportDialog(ImportExportBase):
    ...
    def __init__(self, oParent, sUsername, sPassword):
        super(ExportDialog, self).__init__('Export to Secret Library', oParent, gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT, (
         gtk.STOCK_OK, gtk.RESPONSE_OK,
         gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL))
        ...
        self._setup_vbox('Export to', 'Export as', aDeckWidgets, aInvWidgets, sUsername, sPassword)
        self.show_all()
...
class SecretLibrary(SutekhPlugin):
    ...
    dGlobalConfig = {'username': 'string(default=None)'}
    ...
    def __init__(self, oCardListView, oCardListModel, cModelType):
        ...
        super(SecretLibrary, self).__init__(oCardListView, oCardListModel, cModelType)
        return
    ...
    def get_auth(self):
        ...
        sPassword = None
        if sUsername:
            sPassword = keyring.get_password('www.secretlibrary.info', sUsername)
        return (
         sUsername, sPassword)
    ...
    def set_auth(self, sUsername, sPassword):
        ...
        if sPassword:
            keyring.set_password('www.secretlibrary.info', sUsername, sPassword)
    ...
    def handle_import_response(self, oImportDialog):
        ...
        if iResponse == gtk.RESPONSE_OK:
            ...
            if oImportDialog.get_sl_type() == 'deck':
                dData = {'sl_deck_retrieve': '1', 'sl_user_agent': self.SL_USER_AGENT, 
        ...
        for sLine in sResponse.split('\n'):
            ...
            if oMatch:
                if int(oMatch.group('code')) != 0:
                    do_complaint_error('The Secret Library site returned the error:\n' + sLine.strip())
                    return False
    ...
    def _import_deck(self, sApiUrl, dData):
        ...
        fReq = urlopen_with_timeout(sApiUrl, fErrorHandler=gui_error_handler, sData=sData)
        if not fReq:
            return
        try:
