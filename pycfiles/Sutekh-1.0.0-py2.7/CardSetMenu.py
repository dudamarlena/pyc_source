...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/base/gui/CardSetMenu.py
...
class CardSetMenu(CardListMenu):
    ...
    name = property(fget=lambda self: self._oController.view.sSetName, doc='Associated Card Set Name')
    frame_id = property(fget=lambda self: self._oController.model.frame_id, doc='Frame ID of associated card set (for selecting profiles)')
    cardset_id = property(fget=lambda self: self._oController.model.cardset_id, doc='Cardset ID of associated card set (for selecting profiles)')

    def _create_card_set_menu(self):
        """Create the Actions menu for Card Sets."""
        oMenu = self.create_submenu(self, '_Actions')
        self.create_menu_item('Edit Card Set _Properties', oMenu, self._edit_properties)
        self.create_menu_item('_Save Card Set to File', oMenu, self._do_export)
    ...
    def create_edit_menu(self):
        ...
        self._oEditable = self.create_check_menu_item('Card Set is Editable', oMenu, self.toggle_editable, False, '<Ctrl>e')
        self._oController.view.set_menu(self)
        self.create_menu_item('Copy selection', oMenu, self.copy_selection, '<Ctrl>c')
        self._oPaste = self.create_menu_item('Paste', oMenu, self._paste_selection, '<Ctrl>v')
        self._oDel = self.create_menu_item('Delete selection', oMenu, self._del_selection, 'Delete')
        self._oPaste.set_sensitive(False)
        self._oDel.set_sensitive(False)
        self._oUndo = self.create_menu_item('Undo', oMenu, self._undo_edit, '<Ctrl>z')
        self._oRedo = self.create_menu_item('Redo', oMenu, self._redo_edit, '<Ctrl>y')
        self.set_redo_sensitive(False)
        self.set_undo_sensitive(False)
        self.create_menu_item('Edit _Local Profile', oMenu, self._edit_local_profile)
        self.create_menu_item('Edit _Profiles', oMenu, self._edit_profiles)
        sCardsetProfile = self._oMainWindow.config_file.get_profile(CARDSET, self.cardset_id)
        self._oCardsetProfileMenu = self._create_profile_menu(oMenu, 'Cardset Profile', CARDSET, self._select_cardset_profile, sCardsetProfile)
        sFrameProfiles = self._oMainWindow.config_file.get_profile(FRAME, self.frame_id)
        self._oFrameProfileMenu = self._create_profile_menu(oMenu, 'Pane Profile', FRAME, self._select_frame_profile, sFrameProfiles)
    ...
    def _do_export(self, _oWidget):
        ...
        export_cs(self._oController.model.cardset, self._cPCSWriter, self._oMainWindow, 'xml', [('XML Files', ['*.xml'])])
    ...
    def _edit_local_profile(self, _oWidget):
        ...
        oDlg = LocalProfileEditor(self._oMainWindow, self._oMainWindow.config_file, self.frame_id, self.cardset_id)
        oDlg.run()

    def _edit_profiles(self, _oWidget):
        """Open an options profiles editing dialog."""
        oDlg = FrameProfileEditor(self._oMainWindow, self._oMainWindow.config_file, CARDSET)
        sCurProfile = self._oMainWindow.config_file.get_profile(FRAME, self.frame_id)
        if not sCurProfile:
            sCurProfile = self._oMainWindow.config_file.get_profile(CARDSET, self.cardset_id)
    ...
    def _fix_profile_menu(self):
        ...
        sCardsetProfile = self._oMainWindow.config_file.get_profile(CARDSET, self.cardset_id)
        self._update_profile_group(self._oCardsetProfileMenu, CARDSET, self._select_cardset_profile, sCardsetProfile)
        sFrameProfile = self._oMainWindow.config_file.get_profile(FRAME, self.frame_id)
        self._update_profile_group(self._oFrameProfileMenu, FRAME, self._select_frame_profile, sFrameProfile)
