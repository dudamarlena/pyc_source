...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/gui/MainMenu.py
...
class MainMenu(AppMenu):
    ...
    def _add_download_menu(self, oDownloadMenu):
        ...
        oCheckItem = self.create_menu_item('Check for updated cardlist', oDownloadMenu, self.check_updated_cardlist)
        if is_memory_db():
            oCheckItem.set_sensitive(False)
        self.create_menu_item('Download VTES icons', oDownloadMenu, self.download_icons)

    def _add_prefs_menu(self, oPrefsMenu):
        """Extend the Preferences menu"""
        super(MainMenu, self)._add_prefs_menu(oPrefsMenu)
        oShowErrata = gtk.CheckMenuItem('Show Errata Markers')
    ...
    def _add_help_items(self, oHelpMenu):
        ...
        self.create_menu_item('Sutekh Tutorial', oHelpMenu, self.show_tutorial)
        self.create_menu_item('Sutekh Manual', oHelpMenu, self.show_manual, 'F1')

    def _add_about_dialog(self, oHelpMenu):
        """Add about dialog entry with the correct name"""
        self.create_menu_item('About Sutekh', oHelpMenu, self._oMainWindow.show_about_dialog)
