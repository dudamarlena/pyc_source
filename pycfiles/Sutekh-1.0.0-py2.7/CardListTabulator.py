...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/core/CardListTabulator.py
...
class CardListTabulator(object):
    ...
    def get_default_prop_funcs():
        """Return a dictionary of card property creation functions.
           ...
           """
        dProps = {}
        dProps['group'] = lambda card: card.group is not None and card.group or 0
        dProps['capacity'] = lambda card: card.capacity is not None and card.capacity or 0
        dProps['pool cost'] = lambda card: card.costtype == 'pool' and card.cost or 0
        dProps['blood cost'] = lambda card: card.costtype == 'blood' and card.cost or 0
        dProps['conviction cost'] = lambda card: card.costtype == 'conviction' and card.cost or 0
        dProps['advanced'] = lambda card: card.level == 'advanced' and 1 or 0
        ...
        def make_dis_func(oTmpDis):
            ...
            return lambda card: oTmpDis in [ oPair.discipline for oPair in card.discipline
            ] and 1 or 0
        ...
        def make_rar_func(oTmpRar):
            ...
            return lambda card: oTmpRar in [ oPair.rarity for oPair in card.rarity
            ] and 1 or 0
        ...
        def make_exp_func(oTmpExp):
            ...
            return lambda card: oTmpExp in [ oPair.expansion for oPair in card.rarity
            ] and 1 or 0
