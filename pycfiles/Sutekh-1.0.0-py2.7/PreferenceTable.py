...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/base/gui/PreferenceTable.py
...
class PreferenceTable(gtk.Table):
    ...
    def __init__(self, aOptions, oValidator):
        ...
        for sKey, sConfigSpec, _bInherit in aOptions:
            self._aOptions.append(PreferenceOption(sKey, sConfigSpec, oValidator))

        self._aOptions.sort(key=lambda oOpt: oOpt.sKey != 'name' and oOpt.sKey or '')
        super(PreferenceTable, self).__init__(rows=len(self._aOptions), columns=self.COLUMNS)
        self.set_col_spacings(5)
        self.set_row_spacings(5)
        dAttachOpts = {'xoptions': gtk.FILL, 
           'yoptions': 0}
        for iRow, oOpt in enumerate(self._aOptions):
            self.attach(oOpt.oLabel, self.KEY_COL, (self.KEY_COL + 1), iRow, (iRow + 1), **dAttachOpts)
            self.attach(oOpt.oEntry, self.ENTRY_COL, (self.ENTRY_COL + 1), iRow, (iRow + 1), **dAttachOpts)
            self.attach(oOpt.oInherit, self.INHERIT_COL, (self.INHERIT_COL + 1), iRow, (iRow + 1), **dAttachOpts)
...
class PreferenceOption(object):
    ...
    def set_value(self, oValue, oInheritedValue):
        ...
        if hasattr(self.oInherit, 'set_tooltip_markup'):
            sToolTip = 'Inherited value:\n%s' % self.oSpec.format_value(oInheritedValue)
...
class OptionListParsedSpec(BaseParsedSpec):
    ...
    def get_value(self):
        ...
        return [ oButton.get_label() for oButton in self.oEntry.get_children() if oButton.get_active()
               ]
...
SPEC_TYPE_MAP = {'string': StringParsedSpec, 
...
def parse_spec(sConfigSpec, oValidator):
    ...
    sType, aArgs, dKwargs, sDefault = oValidator._parse_with_caching(sConfigSpec)
