...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/io/IconManager.py
...
def _get_discipline_filename(oDiscipline):
    ...
    if oDiscipline.discipline.name.lower() in ('mal', 'str', 'fli'):
        if oDiscipline.discipline.fullname == 'Flight':
            ...
            sFileName = 'misc/iconmisc%s.gif' % oDiscipline.discipline.fullname.lower()
        else:
            sFileName = 'misc/iconmisc%ssup.gif' % oDiscipline.discipline.fullname.lower()
    elif oDiscipline.level == 'inferior':
        sFileName = 'disciplines/icondis%s.gif' % oDiscipline.discipline.fullname.lower()
    else:
        sFileName = 'disciplines/icondis%ssup.gif' % oDiscipline.discipline.fullname.lower()
    return sFileName
...
class IconManager(BaseIconManager):
    ...
    def get_icon_total(self):
        ...
        return Creed.select().count() + DisciplinePair.select().count() + Clan.select().count() + Virtue.select().count() + CardType.select().count() + 2
