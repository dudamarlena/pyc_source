...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/base/gui/GuiCardLookup.py
# Compiled at: 2019-12-11 16:37:48
"""Lookup AbstractCards for a list of card names, presenting the user with a
   GUI to pick unknown cards from.  """
import re, gtk, pango, gobject
from sqlobject import SQLObjectNotFound
from ..core.BaseTables import AbstractCard, PhysicalCard, Printing
from ..core.BaseAdapters import IAbstractCard, IPhysicalCard, IExpansion, IPrinting, IPrintingName
from ..core.CardLookup import AbstractCardLookup, PhysicalCardLookup, PrintingLookup, LookupFailed
...
class ACLLookupView(PhysicalCardView):
    ...
    def get_selected_card(self):
        ...
        for oPath in aSelection:
            sNewName, _sExpansion, _iCount, _iDepth = oModel.get_all_from_path(oPath)
...
class PCLLookupView(PhysicalCardView):
    ...
    def __init__(self, oDialogWindow, oConfig):
        oController = DummyController('PhysicalCard')
        super(PCLLookupView, self).__init__(oController, oDialogWindow, oConfig)
        self.get_selection().set_mode(gtk.SELECTION_SINGLE)
    ...
    def get_selected_card(self):
        ...
        oModel, aSelection = self.get_selection().get_selected_rows()
        for oPath in aSelection:
            sNewName, sExpansion, _iCount, _iDepth = oModel.get_all_from_path(oPath)
            oAbsCard = oModel.get_abstract_card_from_path(oPath)

        if sExpansion is None:
            sExpansion = ''
        return (
         sNewName, sExpansion, oAbsCard)
...
class ReplacementTreeView(gtk.TreeView):
    ...
    def __init__(self, oCardListView, oFilterToggleButton):
        """Construct a gtk.TreeView object showing the current
           ...
           """
        self._dToolTips = {}
        oModel = gtk.ListStore(gobject.TYPE_INT, gobject.TYPE_STRING, gobject.TYPE_STRING, gobject.TYPE_INT, gobject.TYPE_PYOBJECT)
        ...
        self._create_button_column(gtk.STOCK_OK, 'Set', 'Use the selected card', self._set_to_selection)
        self._create_button_column(gtk.STOCK_REMOVE, 'Ignore', 'Ignore the current card', self._set_ignore)
        self._create_button_column(gtk.STOCK_FIND, 'Filter', 'Filter on best guess', self._set_filter)
    ...
    def run_filter_dialog(self, _oButton):
        ...
        self.oFilterToggleButton.set_active(self.oCardListView.get_model().applyfilter)
        return

    def toggle_apply_filter(self, _oButton):
        """Toggle whether the filter is applied."""
        self.oCardListView.get_model().applyfilter = self.oFilterToggleButton.get_active()
        self.oCardListView.load()
    ...
    NAME_RE = re.compile('^(?P<name>.*?)( \\[(?P<exp>[^]]+)\\])?$')
...
class GuiLookup(AbstractCardLookup, PhysicalCardLookup, PrintingLookup):
        ...
        for sName in aNames:
            if not sName:
                ...
                try:
                    ...
                    sName = sName.decode('ascii', 'replace').encode('ascii', 'replace')
    ...
    def physical_lookup(self, dCardExpansions, dNameCards, dNamePrintings, sInfo):
        ...
        if dUnknownCards:
            if not self._handle_unknown_physical_cards(dUnknownCards, aCards, sInfo):
                raise LookupFailed('Lookup of missing cards aborted by the user.')
        return aCards
    ...
    def printing_lookup(self, aExpPrintNames, sInfo, dCardExpansions):
        ...
        if dUnknownPrintings:
            if not self._handle_unknown_printings(dUnknownPrintings, sInfo, dCardExpansions):
                raise LookupFailed('Lookup of missing printings aborted by the user.')
    ...
    def _handle_unknown_physical_cards(self, dUnknownCards, aPhysCards, sInfo):
        """Handle unknwon physical cards
           ...
           Physical Card List
           """
        oUnknownDialog, oHBox = self._create_dialog(sInfo, 'The following card and expansion combinations could not be found:')
        ...
        for (sName, tExpPrint), iCnt in dUnknownCards.items():
            ...
            oModel.set(oIter, 0, iCnt, 1, sFullName, 2, sBestGuess, 3, pango.WEIGHT_BOLD, 4, None)
        ...
        if iResponse == gtk.RESPONSE_OK:
            ...
            while oIter is not None:
                ...
                sName, sExpPrintName = oReplacementView.parse_card_name(sFullName)
                sNewName, sNewExpPrintName = oReplacementView.parse_card_name(sNewFullName)
                if sNewName != NO_CARD:
                    tExpPrint = (
                     sExpPrintName, None)
                    iCnt = dUnknownCards[(sName, tExpPrint)]
                    oPhys = self._lookup_new_phys_card(oAbsCard, sNewExpPrintName)
    ...
    def _create_dialog(self, sInfo, sMsg):
        ...
        oUnknownDialog = SutekhDialog('Unknown cards found importing %s' % sInfo, None, gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT, (
        ...
        oMesgLabel2 = gtk.Label('OK creates the card set, Cancel aborts the creation of the card set')
        oUnknownDialog.vbox.pack_start(oMesgLabel2)
        return (
         oUnknownDialog, oHBox)
    ...
    def _fill_dialog(self, oHBox, oView):
        ...
        add_accel_to_button(oFilterDialogButton, '<Ctrl>s', oAccelGroup, 'Open the Filter Editing Dialog.\nShortcut :<b>Ctrl-S</b>')
        add_accel_to_button(oFilterApplyButton, '<Ctrl>t', oAccelGroup, 'Toggle the applied state of the filter.\nShortcut : <b>Ctrl-T</b>')
        add_accel_to_button(oLegalButton, '<Ctrl>l', oAccelGroup, 'Toggle the display of cards not legal for tournament play.\nShortcut : <b>Ctrl-L</b>')
        add_accel_to_button(oSearchButton, '<Ctrl>f', oAccelGroup, 'Open the search dailog for the cards.\nShortcut : <b>Ctrl-F</b>')
        oReplacementView = ReplacementTreeView(oView, oFilterApplyButton)
        ...
        oFilterDialogButton.connect('clicked', oReplacementView.run_filter_dialog)
        oFilterApplyButton.connect('toggled', oReplacementView.toggle_apply_filter)
        oLegalButton.connect('toggled', oReplacementView.toggle_show_illegal)
        oSearchButton.connect('clicked', oReplacementView.show_search)
    ...
    def _lookup_new_phys_card(self, oAbsCard, sNewExpName):
        ...
        if sNewExpName is not None:
            ...
            iPrintID = None
        oPhys = PhysicalCard.selectBy(abstractCardID=oAbsCard.id, printingID=iPrintID).getOne()
        return oPhys
    ...
    def _handle_unknown_abstract_cards(self, dUnknownCards, sInfo):
        """Handle the list of unknown abstract cards.
           ...
           Abstract Card List.
           """
        oUnknownDialog, oHBox = self._create_dialog(sInfo, 'The following card names could not be found')
    ...
    def _handle_unknown_printings(self, dUnknownPrintings, sInfo, dCardExpansions):
        """Handle the list of unknown expansion + printings.

           We allow the user to select the correct replacements from the
           expansion / printing combinations listed in the database.
           """
        oUnknownDialog = SutekhDialog('Unknown printings found importing %s' % sInfo, None, gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT, (
         gtk.STOCK_OK, gtk.RESPONSE_OK,
         gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL))
        ...
        oMesgLabel2 = gtk.Label('OK continues the card set creation process, Cancel aborts the creation of the card set')
        oUnknownDialog.vbox.pack_start(oMesgLabel1, False, False)
        oButtonBox = gtk.VBox()
        dReplacement = {}
        for tExpPrint in dUnknownPrintings:
            ...
            oPopupExpList.connect('clicked', self._popup_exp, oSelector, dPrintingCards)
    ...
    def _popup_list(_oButton, sCardText, sPrint):
        ...
        oCardDialog = SutekhDialog('Cards with unknown expansion and print: %s' % sPrint, None, gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT, (
    ...
    def _popup_exp(_oButton, oSelector, dPrintingCards):
        ...
        if sNewName == NO_EXP_AND_PRINT or sNewName is None:
            ...
            oLabel = gtk.Label(('\n').join(sorted(aCards)))
        oCardDialog = SutekhDialog(sTitle, None, gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT, (
         gtk.STOCK_CLOSE, gtk.RESPONSE_CLOSE))
        oCardDialog.vbox.pack_start(AutoScrolledWindow(oLabel, True))
        oCardDialog.set_size_request(300, 400)
        oCardDialog.show_all()
