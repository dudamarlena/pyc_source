...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/io/ARDBXMLInvParser.py
        ...
        if self._iState == self.INCARD:
            if sTag == 'vampire' or sTag == 'card':
                if not self._sCardSet:
                    ...
                    self._sCardSet = unescape_ardb_expansion_name(self._sCardSet)
                ...
                self._oHolder.add(self._iCount, self._sCardName, self._sCardSet, None)
                self._sCardName = None
                self._sCardSet = None
                self._iState = self.NOTAG
                self._sAdvanced = ''
