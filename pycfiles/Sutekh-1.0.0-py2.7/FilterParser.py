...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/base/core/FilterParser.py
...
ENTRY_FILTERS = set([ x.keyword for x in PARSER_FILTERS if hasattr(x, 'istextentry') and x.istextentry
                    ])
WITH_FILTERS = set([ x.keyword for x in PARSER_FILTERS if hasattr(x, 'iswithfilter') and x.iswithfilter
                   ])
FROM_FILTERS = set([ x.keyword for x in PARSER_FILTERS if hasattr(x, 'isfromfilter') and x.isfromfilter
                   ])
LIST_FILTERS = set([ x.keyword for x in PARSER_FILTERS if hasattr(x, 'islistfilter') and x.islistfilter
                   ])
...
def get_filters_for_type(sFilterType):
    ...
    return [ oFilterType for oFilterType in PARSER_FILTERS if sFilterType in oFilterType.types
           ]
...
class ParseFilterDefinitions(object):
    ...
    tokens = ('NOT', 'FILTERTYPE', 'ID', 'STRING', 'OR', 'AND', 'COMMA', 'IN', 'LPAREN',
              'RPAREN', 'VARIABLE', 'WITH', 'FROM')
    t_AND = '\\&\\&'
    t_OR = '\\|\\|'
    t_STRING = '"(\\\\\\\\|\\\\\\\'|\\\\"|[^"\\\\])*?"|\\\'(\\\\\\\\|\\\\\\\'|\\\\"|[^\\\'\\\\])*?\\\''
    t_COMMA = ','
...
class FilterParser(object):
    ...
    def __init__(self):
        ...
        if not self._oGlobalParser:
            ...
            self._oGlobalParser = yacc.yacc(module=self._oGlobalFilterParser, debug=0, write_tables=0)
...
def escape(sData):
    r"""Escape quotes and \'s in the string"""
...
def unescape(sData):
    r"""Unescape quotes and \'s in the string.
...
class AstBaseNode(object):
    ...
    def __str__(self):
        """String representation of the AST
           ...
           """
        sAttrs = '(' + (',').join([ str(oValue) for sKey, oValue in self.__dict__.items() if not sKey.startswith('_') and sKey != 'aChildren' and oValue not in self.aChildren ]) + ')'
...
class FilterPartNode(OperatorNode):
    ...
    def get_values(self):
        ...
        if self.sFilterName in ENTRY_FILTERS:
            aResults = [
             ValueObject(get_filter_type(self.sFilterName).description + ' includes', self)]
        elif self.sFilterName in LIST_FILTERS:
            aResults = [ValueObject(get_filter_type(self.sFilterName).description + ' in', self)]
        else:
            return [
             ValueObject(get_filter_type(self.sFilterName).description, self), ValueObject(None, self)]
    ...
    def get_invalid_values(self):
        ...
        oTemp = get_filter_type(self.sFilterName)([])
        aValidVals = oTemp.get_values()
        if isinstance(aValidVals[0], list) and len(aCurVals) == len(aValidVals):
    ...
    def get_filter(self):
        ...
        if self.sFilterName in ENTRY_FILTERS or self.sFilterName in LIST_FILTERS:
...
class NotOpNode(OperatorNode):
    ...
    def get_values(self):
        ...
        aResults = [
         ValueObject('NOT (', None)] + self.oSubExpression.get_values() + [
         ValueObject(')', None)]
        return aResults
    ...
    def get_values(self):
        ...
        if aLeft is None:
            ...
            if aRight is None:
                return aLeft
            aResults = [
             ValueObject('(', None)] + aLeft + [ValueObject(') ' + self.oOp + ' (', self)] + aRight + [ValueObject(')', None)]
            return aResults
...
class WithNode(OperatorNode):
    ...
    def get_values(self):
        """Get values"""
        return [
         ValueObject(self.oLeft.get_filter()[0] + ' ' + self.oOp + ' ' + self.oRight.get_filter()[0], self)]

    def get_filter(self):
        """Get filter expression"""
        return [
         (
          self.oLeft.get_filter()[0], self.oRight.get_filter()[0])]
...
class FromNode(OperatorNode):
    ...
    def get_values(self):
        """Get values"""
        return [
         self.oLeft.get_values(),
         self.oRight.get_values()]

    def get_filter(self):
        """Get filter expression"""
        return [
         self.oLeft.get_filter(), self.oRight.get_filter()]
