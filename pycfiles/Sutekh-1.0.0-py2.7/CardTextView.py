...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/gui/CardTextView.py
...
from sutekh.base.gui.BaseCardTextView import BaseCardTextBuffer, BaseCardTextView
...
class CardTextView(BaseCardTextView):
    ...
    def print_card_to_buffer(self, oCard):
        ...
        if oCard.capacity is not None:
            self._oBuf.labelled_value('Capacity', str(oCard.capacity), 'capacity')
        ...
        if oCard.level is not None:
            ...
            self._oBuf.labelled_value('Level', str(oCard.level), 'level', oIcon)
        if oCard.cardtype:
        ...
        if oCard.keywords:
            ...
            self._oBuf.labelled_list('Keywords:', aInfo, 'keywords', dIcons)
        if oCard.clan:
            dIcons = self._oIconManager.get_icon_list(oCard.clan)
            self._oBuf.labelled_list('Clan', [ oC.name for oC in oCard.clan ], 'clan', dIcons)
        if oCard.creed:
            dIcons = self._oIconManager.get_icon_list(oCard.creed)
            self._oBuf.labelled_list('Creed', [ oC.name for oC in oCard.creed ], 'creed', dIcons)
        if oCard.sect:
            self._oBuf.labelled_compact_list('Sect', [ oC.name for oC in oCard.sect ], 'sect')
        if oCard.title:
            self._oBuf.labelled_compact_list('Title', [ oC.name for oC in oCard.title ], 'title')
        if oCard.discipline:
            aInfo = []
            aInfo.extend(sorted([ oP.discipline.name for oP in oCard.discipline if oP.level != 'superior'
                                ]))
            aInfo.extend(sorted([ oP.discipline.name.upper() for oP in oCard.discipline if oP.level == 'superior'
                                ]))
            dIcons = self._oIconManager.get_icon_list(oCard.discipline)
            self._oBuf.labelled_list('Disciplines', aInfo, 'discipline', dIcons)
        if oCard.virtue:
            dIcons = self._oIconManager.get_icon_list(oCard.virtue)
            self._oBuf.labelled_list('Virtue', [ oC.name for oC in oCard.virtue ], 'virtue', dIcons)
        self._oBuf.tag_text('\n\n')
        if self._oMainWindow.config_file.get_show_errata_markers():
            self._oBuf.tag_text(format_text(oCard.text), 'card_text')
        else:
            self._oBuf.tag_text(format_text(oCard.search_text), 'card_text')
        if oCard.rulings:
            self._oBuf.tag_text('\n')
        ...
        if oCard.artists:
            self._oBuf.tag_text('\n')
            self._oBuf.labelled_list('Artists', [ oA.name for oA in oCard.artists ], 'artist')
        return