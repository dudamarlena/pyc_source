...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/base/gui/AppMenu.py
...
class AppMenu(SutekhMenu):
    ...
    def __init__(self, oWindow, oConfig):
        ...
        oWindow.add_to_menu_list('Full Card List', self.physical_cl_set_sensitive)
        oWindow.add_to_menu_list('Card Set List', self.pcs_list_pane_set_sensitive)
        oWindow.add_to_menu_list('Card Text', self.add_card_text_set_sensitive)
        oWindow.add_to_menu_list('Log View Frame', self.add_log_pane_set_sensitive)
    ...
    def _create_file_menu(self):
        ...
        self.create_menu_item('Load Saved Card Set from File', oImport, self.do_import_card_set)
        ...
        self.create_menu_item('Manage Profiles', oMenu, self.do_manage_profiles)
        self.create_menu_item('Save Current Pane Set', oMenu, self.do_save_pane_set)
        self.create_menu_item('Restore saved configuration', oMenu, self.do_restore)
        oMenu.add(gtk.SeparatorMenuItem())
        self.create_menu_item('_Quit', oMenu, lambda iItem: self._oMainWindow.action_close(self._oMainWindow, None))

    def _add_prefs_menu(self, oPrefsMenu):
        """Add the File Preferences menu"""
        self._dMenus['File Preferences'] = oPrefsMenu
        oCheckForUpdates = gtk.CheckMenuItem('Check for Cardlist updates on startup')
        ...
        oNameDisplay = gtk.CheckMenuItem('Use "something ..., the" name display')
    ...
    def _add_download_menu(self, oDownloadMenu):
        ...
        oImportItem = self.create_menu_item('Import new Full Card List and rulings', oDownloadMenu, self.do_import_new_card_list)
        if is_memory_db():
    ...
    def _create_pane_menu(self):
        ...
        self.create_menu_item('Equalize _pane sizes', oMenu, self.equalize_panes, '<Ctrl>p')
        self._oAddHorzPane = self.create_menu_item('Split current pane _horizontally (|)', oMenu, self.add_pane_horizontal)
        self._oAddHorzPane.set_sensitive(False)
        self._oAddVertPane = self.create_menu_item('Split current pane _vertically (-)', oMenu, self.add_pane_vertical)
        self._oAddVertPane.set_sensitive(False)
        self._add_add_submenu(oMenu)
        self._add_replace_submenu(oMenu)
        oMenu.add(gtk.SeparatorMenuItem())
        self._oDelPane = self.create_menu_item('_Remove current pane', oMenu, self._oMainWindow.menu_remove_frame, '<Ctrl>w')
    ...
    def _add_add_submenu(self, oMenuWidget):
        ...
        self.create_menu_item('Add New Blank Pane', oAddMenu, self._oMainWindow.add_pane_end)
        self._oAddPCLPane = self.create_menu_item('Add Full Card List', oAddMenu, self._oMainWindow.add_new_physical_card_list)
        self._oAddPCLPane.set_sensitive(True)
        self._oAddCardText = self.create_menu_item('Add Card Text Pane', oAddMenu, self._oMainWindow.add_new_card_text)
        self._oAddCardText.set_sensitive(True)
        self._oAddPCSListPane = self.create_menu_item('Add Card Set List', oAddMenu, self._oMainWindow.add_new_pcs_list)
        self._oAddPCSListPane.set_sensitive(True)
        self._oAddLogPane = self.create_menu_item('Add Log Message Pane', oAddMenu, self._oMainWindow.add_new_log_view_frame)
    ...
    def _add_replace_submenu(self, oMenuWidget):
        ...
        self._oReplacePCLPane = self.create_menu_item('Replace current pane with Full Card List', self._oReplaceMenu, self._oMainWindow.replace_with_physical_card_list)
        self._oReplacePCLPane.set_sensitive(True)
        self._oReplaceCardText = self.create_menu_item('Replace current pane with Card Text Pane', self._oReplaceMenu, self._oMainWindow.replace_with_card_text)
        self._oReplaceCardText.set_sensitive(True)
        self._oReplacePCSListPane = self.create_menu_item('Replace current pane with Card Set List', self._oReplaceMenu, self._oMainWindow.replace_with_pcs_list)
        self._oReplacePCSListPane.set_sensitive(True)
        self._oReplaceLogPane = self.create_menu_item('Replace current pane with Log Message Pane', self._oReplaceMenu, self._oMainWindow.replace_with_log_view_frame)
    ...
    def _create_help_menu(self):
        ...
        self.create_menu_item('Card Filter Help', oMenu, self.show_card_filter_help)
        self.create_menu_item('Card Set Filter Help', oMenu, self.show_card_set_filter_help)
        self.oHelpLast = self.create_menu_item('Last shown help page', oMenu, self._oMainWindow.show_last_help)
    ...
    def do_import_card_set(self, _oWidget):
        ...
        oFileChooser = ImportDialog('Select Card Set(s) to Import', self._oMainWindow)
