...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sutekh/gui/plugins/MergedVampires.py
...
def make_replace_keywords():
    """Cache the various keywords we need to drop when creating the
       merged vampire."""
    dReplaceKeywordMap = {IKeyword('3 bleed'): [
...
class FakeCard(object):
    ...
    def _fix_special_cases(self):
        ...
        if self.name == 'Al-Ashrad, Amr of Alamut (Merged)':
            ...
            self.text = 'Camarilla ' + self.text.replace(' +1 bleed.', ' +2 bleed.')
            ...
            self.keywords.append(IKeyword('1 stealth'))
            self.text = self.text.replace('{NOT FOR LEGAL PLAY} +1 bleed. +1 stealth.', '+1 bleed. +1 stealth. {NOT FOR LEGAL PLAY}')
        elif self.name == 'Appolonius (Merged)':
            self.title = [
             ITitle('Baron')]
            self.text = self.text.replace('Baron of London', 'Anarch Baron of London')
            ...
            self.text = self.text.replace('Independent anarch:', 'Anarch Baron of New York:')
        elif self.name == 'Brunhilde (EC 2013) (Merged)':
            self.title = [
             ITitle('Baron')]
            self.text = self.text.replace('. Anarch Baron of Stockholm', '.')
            self.text = self.text.replace('Anarch:', 'Anarch Baron of Stockholm:')
            self.text = self.text.replace('{NOT FOR LEGAL PLAY} +1 strength.', '+1 strength. {NOT FOR LEGAL PLAY}')
        elif self.name == 'Bulscu (Merged)':
            self.text = self.text.replace(' Prince of Budapest.', '')
            self.title = [ITitle('Prince')]
            self.text = self.text.replace('Camarilla', 'Camarilla Prince of Budapest')
            ...
            self.text = self.text.replace('Independent anarch:', 'Anarch Baron of Berlin:')
        elif self.name == 'Dominique (Merged)':
            self.text = self.text.replace('. Independent Anarch Baron of Paris.', '')
            self.text = self.text.replace('Sabbat:', 'Anarch Baron of Paris:')
            self.title = [ITitle('Baron')]
            self.sect = [ISect('Anarch')]
        elif self.name == 'Duality (Red Sign) (Merged)':
            self.discipline.remove(IDisciplinePair(('Thaumaturgy', 'inferior')))
            self.discipline.append(IDisciplinePair(('Thaumaturgy', 'superior')))
            ...
            self.text = self.text.replace('. Gengis gets +1 level of Auspex [aus] and Celerity [cel].', '')
            ...
            self.discipline.append(IDisciplinePair(('Obtenebration', 'inferior')))
            self.text = self.text.replace(' and gains 1 level of Daimoinon [dai] and Obtenebration [obt]', '')
            ...
            self.text = self.text.replace('Independent anarch:', 'Anarch Baron of Barcelona:')
        elif self.name == 'Jeremy MacNeil (Merged)':
            self.text = self.text.replace(' Anarch Baron of Los Angeles.', '')
            self.text = self.text.replace('Independent:', 'Anarch Baron of Los Angeles.')
            self.title = [ITitle('Baron')]
            self.sect = [ISect('Anarch')]
            ...
            self.title = [
             FakeTitle('Independent with 3 votes')]
            self.text = self.text.replace('Kemintiri has 3 votes (titled). ', '')
            self.text = self.text.replace('Independent.', 'Independent. Kemintiri has 3 votes (titled).')
        elif self.name == 'Lambach (Merged)':
            self.text = self.text.replace(' Lambach has 2 votes (titled).', '')
            self.text = self.text.replace('Independent:', 'Independent. Lambach has 2 votes ((titled).')
            ...
            self.text = self.text.replace(': Louis gets +1 level of Dominate [dom] and Presence [pre].', '')
        elif self.name == 'Lucita (Merged)':
            self.title = [
             ITitle('Archbishop')]
            self.text = self.text.replace(' Archbishop of Aragon.', '')
            self.text = self.text.replace('Sabbat:', 'Sabbat Archbishop of Aragon:')
        elif self.name == 'Maldavis (Merged)':
            self.title = [
             ITitle('Baron')]
            self.text = self.text.replace('. Baron of Chicago.', '')
            self.text = self.text.replace('Independent anarch:', 'Independent Anarch Baron of Chicago:')
            ...
            self.text = self.text.replace('Camarilla primogen', 'Camarilla Prince of Lisbon')
            ...
            self.title = [
             ITitle('Prince')]
            self.text = self.text.replace('Camarilla', 'Camarilla Prince of Boston')
        elif self.name == 'Reverend Adams (Merged)':
            self.text = self.text.replace(' Older vampires do not tap for successfully blocking Reverend Adams.', '', 1)
        elif self.name == 'Rutor (Red Sign) (Merged)':
            self.discipline.remove(IDisciplinePair(('Vicissitude', 'inferior')))
            self.discipline.append(IDisciplinePair(('Vicissitude', 'superior')))
            self.discipline.append(IDisciplinePair(('Fortitude', 'inferior')))
            ...
            self.text = self.text.replace('Sabbat Archbishop of Washington, D.C', 'Sabbat cardinal')
        elif self.name == 'Tariq, The Silent (Merged)':
            self.text = self.text.replace('Independent: ', 'Independent. Black Hand. Red List: ')
            self.text = self.text.replace(" Tariq's capacity is reduced by 4 while he is controlled.", '')
            self.text = self.text.replace("Tariq's capacity is not reduced by his card text.", '')
        elif self.name == 'Tegyrius, Vizier (Merged)':
            self.title = [
             ITitle('Justicar')]
            self.text = self.text.replace(' Assamite Justicar.', '')
            self.text = self.text.replace('Camarilla:', 'Camarilla Assamite Justicar:')
            ...
            self.capacity = 5
            self.text = self.text.replace('Valerius becomes non-infernal and non-Red List as he merges. While merged, his capacity is reduced by 2. Infernal.', '')
...
class MergedVampirePlugin(SutekhPlugin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._oMerged = make_markup_button('<span size="x-small">Show Merged</span>')
        self._oAdv = make_markup_button('<span size="x-small">Show Advanced</span>')
        self._oBase = make_markup_button('<span size="x-small">Show Base</span>')
        self._bMerged = False
    ...
    def post_set_card_text(self, oPhysCard):
        ...
        if self._oAbsCard in self._aExcluded:
            ...
            try:
                ...
                do_complaint_error('Unable to created merged vampire for %s' % self._oAbsCard.name)
                self._aExcluded.add(self._oAbsCard)
                logging.warn('Merged vampire creation failed (%s).', oExc, exc_info=1)
                return
