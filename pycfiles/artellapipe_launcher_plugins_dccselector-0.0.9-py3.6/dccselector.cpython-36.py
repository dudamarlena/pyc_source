...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/launcher/plugins/dccselector/dccselector.py
...
import os, sys, time, random, logging, argparse, importlib
...
class DCCSelector(plugin.ArtellaLauncherPlugin, object):
    ...
    def __init__(self, project, launcher, parent=None):
        ...
        self._config = tpDcc.ConfigsMgr().get_config(config_name='artellapipe-launcher-plugins-dccselector',
    ...
    def load_dccs(self, dccs_dict):
        ...
        for dcc_name, dcc_data in self._dccs.items():
            ...
            try:
                dcc_module = importlib.import_module('artellapipe.launcher.plugins.dccselector.dccs.{}dcc'.format(dcc_name.lower().replace(' ', '')))
            except ImportError:
    ...
    def _get_splash_pixmap(self):
        ...
        if splash_files or not os.path.isfile(splash_path):
            ...
            splash_pixmap = tpDcc.ResourcesMgr().pixmap(splash_name,
              extension=(splash_extension[1:]), key='project')
        else:
    ...
    def _on_dcc_selected(self, selected_dcc, selected_version):
            ...
            if not selected_dcc:
                ...
                if not installation_paths:
                    ...
                    if selected_version not in installation_paths:
                        ...
                        QApplication.instance().processEvents()
                        parser = argparse.ArgumentParser(description=('{} Launcher allows to setup a custom initialization for DCCs. This allows to setup specific paths in an easy way.'.format(self.project.name.title())))
                        ...
                        if id_path in install_path:
                            qtutils.show_warning(None, 'Installation folder is not valid!', 'Folder {} is not a valid installation folder. Install tools in a folder that is not inside Artella Project folder please!'.format(install_path))
                            sys.exit()
                        ...
                        for mod_name in mods_to_register:
                            try:
                                imported_mod = importlib.import_module('{}.{}'.format(self.project.get_clean_name(), mod_name))
                                if imported_mod:
                                    ...
                                    if os.path.isdir(mod_path):
                                        if mod_path not in folders_to_register:
                                            ...
                                            LOGGER.warning('Impossible to register Bootstrap Path for Project "{}" and DCC "{}"'.format(self.project.get_clean_name(), self._selected_dcc))
                            except ImportError:
                                continue
                    ...
                    launch_fn or LOGGER.error('Selected DCC: {} has no launch function!'.format(selected_dcc.name))
                    return
        ...
        if not bootstrap_path or not os.path.isdir(bootstrap_path):
            QMessageBox.warning(None, 'Bootstrap Directory not found!', 'Bootstrap folder for Project "{}" and DCC "{}" not found. Tools will not load. Please contact TD!'.format(self.project.get_clean_name, self._selected_dcc))
