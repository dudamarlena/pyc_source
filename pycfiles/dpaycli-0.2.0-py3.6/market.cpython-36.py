...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/dpaycli/market.py
# Compiled at: 2018-10-15 03:21:44
# Size of source mod 2**32: 34792 bytes
from __future__ import absolute_import
from __future__ import division
from __future__ import print_function
from __future__ import unicode_literals
from builtins import str
import random, pytz, logging
from datetime import datetime, timedelta
from dpaycli.instance import shared_dpay_instance
from .utils import formatTimeFromNow, formatTime, formatTimeString, assets_from_string, parse_time, addTzInfo
...
class Market(dict):
    ...
    def __eq__(self, other):
        if isinstance(other, str):
            ...
            return self['quote']['symbol'] == quote_symbol and self['base']['symbol'] == base_symbol or self['quote']['symbol'] == base_symbol and self['base']['symbol'] == quote_symbol
        if isinstance(other, Market):
            return self['quote']['symbol'] == other['quote']['symbol'] and self['base']['symbol'] == other['base']['symbol']
    ...
    def ticker(self, raw_data=False):
        ...
        if raw_data:
            ...
            data['highest_bid'] = Price((ticker['highest_bid']),
              base=(self['base']),
              quote=(self['quote']),
              dpay_instance=(self.dpay))
            data['latest'] = Price((ticker['latest']),
              quote=(self['quote']),
              base=(self['base']),
              dpay_instance=(self.dpay))
            data['lowest_ask'] = Price((ticker['lowest_ask']),
              base=(self['base']),
              quote=(self['quote']),
              dpay_instance=(self.dpay))
    ...
    def volume24h(self, raw_data=False):
        ...
        if raw_data:
            ...
            return {self['base']['symbol']: Amount((volume['bbd_volume']), dpay_instance=(self.dpay)), 
    ...
    def orderbook(self, limit=25, raw_data=False):
        ...
        if raw_data:
            ...
            asks = list([Order(Amount((x['order_price']['quote']), dpay_instance=(self.dpay)), Amount((x['order_price']['base']), dpay_instance=(self.dpay)), dpay_instance=(self.dpay)) for x in orders['asks']])
            bids = list([Order(Amount((x['order_price']['quote']), dpay_instance=(self.dpay)), Amount((x['order_price']['base']), dpay_instance=(self.dpay)), dpay_instance=(self.dpay)).invert() for x in orders['bids']])
    ...
    def trades(self, limit=100, start=None, stop=None, raw_data=False):
        ...
        if not stop:
            ...
            if self.dpay.rpc.get_use_appbase():
                orders = self.dpay.rpc.get_trade_history({'start':formatTimeString(start),  'end':formatTimeString(stop), 
                 'limit':limit},
                  api='market_history')['trades']
            else:
                orders = self.dpay.rpc.get_trade_history((formatTimeString(start)),
    ...
    def market_history(self, bucket_seconds=300, start_age=3600, end_age=0, raw_data=False):
        ...
        if bucket_seconds < 5:
            ...
            self.dpay.rpc.set_next_node_on_empty_reply(False)
            if self.dpay.rpc.get_use_appbase():
                history = self.dpay.rpc.get_market_history({'bucket_seconds':bucket_seconds,  'start':formatTimeFromNow(-start_age - end_age), 
                 'end':formatTimeFromNow(-end_age)},
                  api='market_history')['buckets']
            else:
                history = self.dpay.rpc.get_market_history(bucket_seconds,
        ...
        if raw_data:
            ...
            for h in history:
                if 'open' in h:
                    if isinstance(h.get('open'), string_types):
    ...
    def accountopenorders(self, account=None, raw_data=False):
        ...
        if not account:
            ...
            r = []
            return self.dpay.is_connected() or None
        else:
            self.dpay.rpc.set_next_node_on_empty_reply(False)
            if self.dpay.rpc.get_use_appbase():
            ...
            for o in orders:
                order = {}
                order['order'] = Order(Amount((o['sell_price']['base']), dpay_instance=(self.dpay)),
    ...
    def buy(self, price, amount, expiration=None, killfill=False, account=None, orderid=None, returnOrderId=False):
        ...
        if not expiration:
            ...
            if not account:
                ...
                if isinstance(amount, Amount):
                    ...
                    assert amount['asset']['symbol'] == self['quote']['symbol'], 'Price: {} does not match amount: {}'.format(str(price), str(amount))
                else:
                    if isinstance(amount, str):
                        amount = Amount(amount, dpay_instance=(self.dpay))
                    else:
                        amount = Amount(amount, (self['quote']['symbol']), dpay_instance=(self.dpay))
            order = (operations.Limit_order_create)(owner=account['name'], 
             orderid=orderid or random.getrandbits(32), 
             amount_to_sell=Amount((float(amount) * float(price)),
  (self['base']['symbol']),
  dpay_instance=(self.dpay)), 
             min_to_receive=Amount((float(amount)),
  (self['quote']['symbol']),
  dpay_instance=(self.dpay)), 
    ...
    def sell(self, price, amount, expiration=None, killfill=False, account=None, orderid=None, returnOrderId=False):
        ...
        if not expiration:
            ...
            order = (operations.Limit_order_create)(owner=account['name'], 
             orderid=orderid or random.getrandbits(32), 
             amount_to_sell=Amount((float(amount)),
  (self['quote']['symbol']),
  dpay_instance=(self.dpay)), 
  ...
  dpay_instance=(self.dpay)), 
    ...
    def cancel(self, orderNumbers, account=None, **kwargs):
        ...
        for order in orderNumbers:
            op.append((operations.Limit_order_cancel)(owner=account['name'], 
             orderid=order, 
             prefix=self.dpay.prefix))
    ...
    def _weighted_average(values, weights):
        ...
        """
        assert len(values) == len(weights) and len(weights) > 0, 'Length of both array must be the same and greater than zero!'
    ...
    def btc_usd_ticker(verbose=False):
        ...
        for r in [x for x in responses if hasattr(x, 'status_code') if x.status_code == 200 if x.json()]:
            try:
                if 'bitfinex' in r.url:
                    data = r.json()
                    prices['bitfinex'] = {'price':float(data['last_price']), 
                     'volume':float(data['volume'])}
                else:
                    if 'gdax' in r.url:
                        data = r.json()
                        prices['gdax'] = {'price':float(data['price']), 
                         'volume':float(data['volume'])}
                    else:
                        if 'kraken' in r.url:
                            data = r.json()['result']['XXBTZUSD']['p']
                            prices['kraken'] = {'price':float(data[0]), 
                             'volume':float(data[1])}
                        else:
                            if 'okcoin' in r.url:
                                data = r.json()['ticker']
                                prices['okcoin'] = {'price':float(data['last']), 
                                 'volume':float(data['vol'])}
                            else:
                                if 'bitstamp' in r.url:
                                    data = r.json()
                                    prices['bitstamp'] = {'price':float(data['last']), 
                                     'volume':float(data['volume'])}
        ...
        return Market._weighted_average([x['price'] for x in prices.values()], [x['volume'] for x in prices.values()])
        ...
        for r in [x for x in responses if hasattr(x, 'status_code') if x.status_code == 200 if x.json()]:
            try:
                if 'poloniex' in r.url:
                    ...
                    prices['poloniex'] = {'price':float(data['last']), 
                    ...
                    if 'bittrex' in r.url:
                        ...
                        if 'binance' in r.url:
                            ...
                            prices['binance'] = {'price':float(data['lastPrice']), 
                             'volume':float(data['quoteVolume'])}
                        else:
                            if 'huobi' in r.url:
                                data = r.json()['data'][(-1)]
                                prices['huobi'] = {'price':float(data['close']), 
                                 'volume':float(data['vol'])}
                            else:
                                if 'upbit' in r.url:
                                    data = r.json()[(-1)]
                                    prices['upbit'] = {'price':float(data['tradePrice']), 
