...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/dpaycliapi/node.py
...
class Nodes(list):
    __doc__ = 'Stores Node URLs and error counts'

    def __init__(self, urls, num_retries, num_retries_call):
        if isinstance(urls, str):
            url_list = re.split(',|;', urls)
            if url_list is None:
                url_list = [
                 urls]
        else:
            if isinstance(urls, Nodes):
