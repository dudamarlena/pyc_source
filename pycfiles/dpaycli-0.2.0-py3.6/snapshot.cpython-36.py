...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/dpaycli/snapshot.py
# Compiled at: 2018-10-15 03:13:48
# Size of source mod 2**32: 23533 bytes
...
from dpaycli.utils import formatTimeString, formatTimedelta, remove_from_dict, reputation_to_score, addTzInfo, parse_time
...
from dpaycli.constants import DPAY_VOTE_REGENERATION_SECONDS, DPAY_1_PERCENT, DPAY_100_PERCENT
log = logging.getLogger(__name__)

class AccountSnapshot(list):
    __doc__ = ' This class allows to easily access Account history\n\n        :param str account_name: Name of the account\n        :param dpaycli.dpay.DPay dpay_instance: DPay\n               instance\n    '
    ...
    def reset(self):
        ...
        self.own_dpay = [Amount(0, (self.dpay.dpay_symbol), dpay_instance=(self.dpay))]
        self.own_bbd = [Amount(0, (self.dpay.bbd_symbol), dpay_instance=(self.dpay))]
                    ...
                    if use_block_num:
                        if stop is not None:
                            if isinstance(stop, int):
                                if op['block'] > stop:
                                    continue
                    if not use_block_num:
                        if stop is not None and isinstance(stop, int):
                            if op['index'] > stop:
                                continue
                elif stop is not None:
                    if isinstance(stop, (datetime, date, time)):
    ...
    def get_data(self, timestamp=None, index=0):
        ...
        return {'timestamp':ts,  'vests':own,  'delegated_vests_in':din,  'delegated_vests_out':dout,  'bp_own':bp_own, 
         'bp_eff':bp_eff,  'dpay':dpay,  'bbd':bbd,  'index':index}
    ...
    def get_account_history(self, start=None, stop=None, use_block_num=True):
        ...
        """
        super(AccountSnapshot, self).__init__([h for h in self.account.history(start=start, stop=stop, use_block_num=use_block_num)])

    def update_rewards(self, timestamp, curation_reward, author_vests, author_dpay, author_bbd):
        self.reward_timestamps.append(timestamp)
        self.curation_rewards.append(curation_reward)
        self.author_rewards.append({'vests':author_vests,  'whitehorse':author_dpay,  'bbd':author_bbd})
    ...
    def update(self, timestamp, own, delegated_in=None, delegated_out=None, dpay=0, bbd=0):
    ...
    def parse_op(self, op, only_ops=[], enable_rewards=False, enable_out_votes=False, enable_in_votes=False):
        ...
        if op['type'] == 'account_create':
            ...
            if op['type'] == 'account_create_with_delegation':
                ...
                if op['new_account_name'] == self.account['name']:
                    ...
                    return
                if op['creator'] == self.account['name']:
                    delegation = {'account':op['new_account_name'], 
                     'amount':Amount(op['delegation'], dpay_instance=self.dpay)}
                ...
                if op['type'] == 'delegate_vesting_shares':
                    ...
                    if op['delegator'] == self.account['name']:
                        delegation = {'account':op['delegatee'], 
                         'amount':vests}
                        self.update(ts, 0, 0, delegation)
                        return
                    if op['delegatee'] == self.account['name']:
                        delegation = {'account':op['delegator'], 
                         'amount':vests}
                    ...
                    if op['type'] == 'transfer':
                        ...
                        if op['from'] == self.account['name']:
                            if amount.symbol == self.dpay.dpay_symbol:
                                self.update(ts, 0, 0, 0, amount * -1, 0)
                            elif amount.symbol == self.dpay.bbd_symbol:
        ...
        if op['type'] == 'fill_order':
            ...
            if op['current_owner'] == self.account['name']:
                if current_pays.symbol == self.dpay.dpay_symbol:
                    self.update(ts, 0, 0, 0, current_pays * -1, open_pays)
                elif current_pays.symbol == self.dpay.bbd_symbol:
        ...
        if op['type'] == 'vote':
            ...
            return
        if op['type'] in ('comment', 'feed_publish', 'shutdown_witness', 'account_witness_vote',
                          'witness_update', 'custom_json', 'limit_order_create',
                          'account_update', 'account_witness_proxy', 'limit_order_cancel',
                          'comment_options', 'delete_comment', 'interest', 'recover_account',
                          'pow', 'fill_convert_request', 'convert', 'request_account_recovery'):
            return
    ...
    def build_bp_arrays(self):
        ...
        self.own_sp = []
        self.eff_sp = []
        for ts, own, din, dout in zip(self.timestamps, self.own_vests, self.delegated_vests_in, self.delegated_vests_out):
            sum_in = sum([din[key].amount for key in din])
            sum_out = sum([dout[key].amount for key in dout])
            bp_in = self.dpay.vests_to_sp(sum_in, timestamp=ts)
            bp_out = self.dpay.vests_to_sp(sum_out, timestamp=ts)
    ...
    def build_vp_arrays(self):
        ...
        for ts, weight in zip(self.out_vote_timestamp, self.out_vote_weight):
            ...
            if self.vp[(-1)] < DPAY_100_PERCENT:
                regenerated_vp = (ts - self.vp_timestamp[(-1)]).total_seconds() * DPAY_100_PERCENT / DPAY_VOTE_REGENERATION_SECONDS
    ...
    def build_curation_arrays(self, end_date=None, sum_days=7):
        ...
        for ts, vests in zip(self.reward_timestamps, self.curation_rewards):
            if vests == 0:
                ...
                bp = self.dpay.vests_to_sp(vests, timestamp=ts)
                data = self.get_data(timestamp=ts, index=index)
                index = data['index']
                if 'bp_eff' in data:
                    if data['bp_eff'] > 0:
                        curation_1k_sp = bp / data['bp_eff'] * 1000 / sum_days * 7
                else:
                    curation_1k_sp = 0
            if ts < end_date:
