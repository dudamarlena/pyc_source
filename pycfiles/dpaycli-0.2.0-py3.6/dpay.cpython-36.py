...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/dpaycli/dpay.py
# Compiled at: 2018-10-15 03:23:17
# Size of source mod 2**32: 82778 bytes
...
import json, logging, re, os, math, ast, time
...
from dpaycliapi.exceptions import NoAccessApi, NoApiWithName
...
from .version import version as dpaycli_version
from .exceptions import AccountExistsException, AccountDoesNotExistsException
...
class DPay(object):
    ...
    def __init__(self, node='', rpcuser=None, rpcpassword=None, debug=False, data_refresh_time_seconds=900, **kwargs):
        """Init dpay
            ...
            :param int num_retries_call: Repeat num_retries_call times a rpc call on node error (default is 5)
                :param int timeout: Timeout setting for https nodes (default is 60)
            :param bool use_dpid: When True, a dpayid object is created. Can be used for broadcast
        ...
        self.dpayconnect = kwargs.get('dpayid', None)
        self.use_dpid = bool(kwargs.get('use_dpid', False))
        self.blocking = kwargs.get('blocking', False)
        self.custom_chains = kwargs.get('custom_chains', {})
        self.config = config
        if not self.offline:
            (self.connect)(node=node, rpcuser=rpcuser, 
             rpcpassword=rpcpassword, **kwargs)
        self.data = {'last_refresh':None,  'last_node':None,  'dynamic_global_properties':None,  'feed_history':None,  'get_feed_history':None, 
         'hardfork_properties':None,  'network':None, 
         'witness_schedule':None,  'reserve_ratio':None,  'config':None, 
         'reward_funds':None}
        self.data_refresh_time_seconds = data_refresh_time_seconds
        self.clear()
        self.wallet = Wallet(dpay_instance=self, **kwargs)
        if self.dpayconnect is not None:
            if not isinstance(self.dpayconnect, DPayID):
                raise ValueError('dpayid musst be DPayID object')
        if self.dpayconnect is None:
            if self.use_dpid:
                self.dpayconnect = DPayID(dpay_instance=self, **kwargs)
        if self.dpayconnect is not None:
            if not self.use_dpid:
                self.use_dpid = True
    ...
    def refresh_data(self, force_refresh=False, data_refresh_time_seconds=None):
        ...
        if self.offline:
            return
        elif data_refresh_time_seconds is not None:
            self.data_refresh_time_seconds = data_refresh_time_seconds
        else:
            if self.data['last_refresh'] is not None:
                if not force_refresh:
                    if self.data['last_node'] == self.rpc.url:
    ...
    def get_reserve_ratio(self, use_stored_data=True):
        ...
        if use_stored_data:
            ...
            reserve_ratio = {'id':0, 
             'average_block_size':props['average_block_size'],  'current_reserve_ratio':props['current_reserve_ratio'], 
    ...
    def get_median_price(self, use_stored_data=True):
        ...
        if median_price is None:
            ...
            a = Price(None,
              ...
              dpay_instance=self)
            return a.as_base(self.bbd_symbol)
    ...
    def get_rc_cost(self, resource_count):
        ...
        rc_regen = int(Amount((dyn_param['total_vesting_shares']), dpay_instance=self)) / (DPAY_RC_REGEN_TIME / config['DPAY_BLOCK_INTERVAL'])
        total_cost = 0
        if rc_regen == 0:
            return total_cost
        else:
    ...
    def rshares_to_bbd(self, rshares, not_broadcasted_vote=False, use_stored_data=True):
        ...
        """
        payout = float(rshares) * self.get_bbd_per_rshares(use_stored_data=use_stored_data, not_broadcasted_vote_rshares=(rshares if not_broadcasted_vote else 0))
        return payout
    ...
    def get_bbd_per_rshares(self, not_broadcasted_vote_rshares=0, use_stored_data=True):
        ...
        if median_price is None:
            ...
            BBD_price = (median_price * Amount(1, (self.dpay_symbol), dpay_instance=self)).amount
    ...
    def get_dpay_per_mvest(self, time_stamp=None, use_stored_data=True):
        ...
        if time_stamp is not None:
            ...
            return Amount((global_properties['total_vesting_fund_dpay']), dpay_instance=self).amount / (Amount((global_properties['total_vesting_shares']), dpay_instance=self).amount / 1000000.0)

    def vests_to_sp(self, vests, timestamp=None, use_stored_data=True):
        """ Converts vests to BP

            :param dpaycli.amount.Amount vests/float vests: Vests to convert
        ...
        if isinstance(vests, Amount):
            vests = vests.amount
        return vests / 1000000.0 * self.get_dpay_per_mvest(timestamp, use_stored_data=use_stored_data)

    def bp_to_vests(self, bp, timestamp=None, use_stored_data=True):
        """ Converts BP to vests

            :param float bp: DPay power to convert
            :param datetime timestamp: (Optional) Can be used to calculate
                the conversion rate from the past
        """
        return bp * 1000000.0 / self.get_dpay_per_mvest(timestamp, use_stored_data=use_stored_data)

    def bp_to_bbd(self, bp, voting_power=DPAY_100_PERCENT, vote_pct=DPAY_100_PERCENT, not_broadcasted_vote=True, use_stored_data=True):
        """ Obtain the resulting BBD vote value from DPay power
            :param number dpay_power: DPay Power
        ...
        """
        vesting_shares = int(self.bp_to_vests(bp, use_stored_data=use_stored_data))
    ...
    def _calc_resulting_vote(self, voting_power=DPAY_100_PERCENT, vote_pct=DPAY_100_PERCENT, use_stored_data=True):
        used_power = int(voting_power * abs(vote_pct) / DPAY_100_PERCENT * 86400)
    ...
    def bp_to_rshares(self, dpay_power, voting_power=DPAY_100_PERCENT, vote_pct=DPAY_100_PERCENT, use_stored_data=True):
        """ Obtain the r-shares from DPay power
    ...
    def vests_to_rshares(self, vests, voting_power=DPAY_100_PERCENT, vote_pct=DPAY_100_PERCENT, subtract_dust_threshold=True, use_stored_data=True):
        ...
        rshares = int(math.copysign(vests * 1000000.0 * used_power / DPAY_100_PERCENT, vote_pct))
    ...
    def bbd_to_rshares(self, bbd, not_broadcasted_vote=False, use_stored_data=True):
        ...
        if isinstance(bbd, Amount):
            ...
            if isinstance(bbd, string_types):
                bbd = Amount(bbd, dpay_instance=self)
            else:
                bbd = Amount(bbd, (self.bbd_symbol), dpay_instance=self)
            if bbd['symbol'] != self.bbd_symbol:
    ...
    def rshares_to_vote_pct(self, rshares, dpay_power=None, vests=None, voting_power=DPAY_100_PERCENT, use_stored_data=True):
        """ Obtain the voting percentage for a desired rshares value
            for a given DPay Power or vesting shares and voting_power
        ...
        if dpay_power is None:
            ...
            if dpay_power is not None:
                vests = int(self.bp_to_vests(dpay_power, use_stored_data=use_stored_data) * 1000000.0)
            if self.hardfork >= 20:
        ...
        vote_pct = used_power * DPAY_100_PERCENT / 86400 / voting_power
    ...
    def bbd_to_vote_pct(self, bbd, dpay_power=None, vests=None, voting_power=DPAY_100_PERCENT, not_broadcasted_vote=True, use_stored_data=True):
        """ Obtain the voting percentage for a desired BBD value
            for a given DPay Power or vesting shares and voting power
            Give either DPay Power or vests, not both.
    ...
    def set_password_storage(self, password_storage):
        """ Set the password storage mode.
            ...
            python -m keyring set dpaycli wallet password
    ...
    def get_default_nodes(self):
        ...
        if 'node' in config:
            ...
            if 'nodes' in config:
                nodes = config['nodes']
            elif 'default_nodes' in config:
                if bool(config['default_nodes']):
    ...
    def finalizeOp(self, ops, account, permission, **kwargs):
        """ This method obtains the required private keys if present in
            ...
            .. note:: This uses ``dpaycli.txbuffer`` as instance of
                :class:`dpaycli.transactionbuilder.TransactionBuilder`.
                You may want to use your own txbuffer
        """
        if self.offline:
            return {}
        else:
            if 'append_to' in kwargs:
                if kwargs['append_to']:
    ...
    def newWallet(self, pwd):
        """ Create a new wallet. This method is basically only calls
            :func:`dpaycli.wallet.create`.

            :param str pwd: Password to use for the new wallet

            :raises dpaycli.exceptions.WalletExists: if there is already a
                wallet created
    ...
    def new_tx(self, *args, **kwargs):
        ...
        """
        builder = TransactionBuilder(args, dpay_instance=self, **kwargs)
    ...
    def claim_account(self, creator, fee=None, **kwargs):
        """"Claim account for claimed account creation.
            ...
            When fee is 0 BEX a subsidized account is claimed and can be created
            later with create_claimed_account.
            The number of subsidized account is limited.

            :param str creator: which account should pay the registration fee (RC or BEX)
                    (defaults to ``default_account``)
            :param str fee: when set to 0 BEX (default), claim account is paid by RC
        """
        fee = fee if fee is not None else '0 %s' % self.dpay_symbol
        if not creator:
            if config['default_account']:
                creator = config['default_account']
        if not creator:
            raise ValueError('Not creator account given. Define it with creator=x, or set the default_account using dpay')
        creator = Account(creator, dpay_instance=self)
        op = {'fee':Amount(fee, dpay_instance=self), 
    ...
    def create_claimed_account(self, account_name, creator=None, owner_key=None, active_key=None, memo_key=None, posting_key=None, password=None, additional_owner_keys=[], additional_active_keys=[], additional_posting_keys=[], additional_owner_accounts=[], additional_active_accounts=[], additional_posting_accounts=[], storekeys=True, store_owner_key=False, json_meta=None, combine_with_claim_account=False, fee=None, **kwargs):
        """ Create new claimed account on DPay

            The brainkey/password can be used to recover all generated keys
            (see `dpaycligraphenebase.account` for more details.
        ...
        if not creator:
            raise ValueError('Not creator account given. Define it with creator=x, or set the default_account using dpay')
        if password:
            if owner_key or active_key or memo_key:
        ...
        if password:
            ...
            if owner_key and active_key and memo_key:
                if posting_key:
                ...
                if combine_with_claim_account:
                    op = {'fee':Amount(fee, dpay_instance=self),  'creator':creator['name'], 
                     'prefix':self.prefix}
                    op = (operations.Claim_account)(**op)
                    ops = [op]
            op = {'creator':creator['name'], 
             ...
             'owner':{'account_auths':owner_accounts_authority, 
              ...
              'address_auths':[],  'weight_threshold':1}, 
    ...
    def witness_set_properties(self, wif, owner, props, use_condenser_api=True):
        ...
        props_list = [
         [
          'key', repr(PrivateKey(wif, prefix=(self.prefix)).pubkey)]]
        for k in props:
    ...
    def witness_update(self, signing_key, url, props, account=None, **kwargs):
        """ Creates/updates a witness
            ...
            Properties:::
                ...
                {
                    "account_creation_fee": "3.000 BEX",
                    "maximum_block_size": 65536,
                    "bbd_interest_rate": 0,
                }
        ...
        if 'account_creation_fee' in props:
            props['account_creation_fee'] = Amount((props['account_creation_fee']), dpay_instance=self)
        op = (operations.Witness_update)(owner=account['name'], 
         url=url, 
         block_signing_key=signing_key, 
         props=props, 
         fee=Amount(0, (self.dpay_symbol), dpay_instance=self), 
         prefix=self.prefix)
    ...
    def custom_json(self, id, json_data, required_auths=[], required_posting_auths=[], **kwargs):
        ...
        op = (operations.Custom_json)(json=json_data, 
    ...
    def post(self, title, body, author=None, permlink=None, reply_identifier=None, json_metadata=None, comment_options=None, community=None, app=None, tags=None, beneficiaries=None, self_vote=False, parse_body=False, **kwargs):
        ...
        :param str app: (Optional) Name of the app which are used for posting
            when not set, dpaycli/<version> is used
        ...
        if app:
            ...
            if 'app' not in json_metadata:
                json_metadata.update({'app': 'dpaycli/%s' % dpaycli_version})
        if not author:
            if config['default_account']:
        ...
        if not permlink:
            permlink = derive_permlink(title)
        post_op = (operations.Comment)(parent_author=parent_author, 
        ...
        if self_vote:
            vote_op = (operations.Vote)(voter=account['name'], 
             author=account['name'], 
             permlink=permlink, 
            ...
            for b in beneficiaries:
                if 'account' not in b:
                    ...
                    if b['weight'] < 1 or b['weight'] > DPAY_100_PERCENT:
                        raise ValueError('beneficiaries error, 1<=weight<=%s!' % DPAY_100_PERCENT)
                weight_sum += b['weight']

            if weight_sum > DPAY_100_PERCENT:
                raise ValueError('beneficiaries exceed total weight limit %s' % DPAY_100_PERCENT)
            options['beneficiaries'] = beneficiaries
        default_max_payout = '1000000.000 %s' % self.bbd_symbol
        comment_op = (operations.Comment_options)(author=author, 
         permlink=permlink, 
         max_accepted_payout=options.get('max_accepted_payout', default_max_payout), 
         percent_dpay_dollars=int(options.get('percent_dpay_dollars', DPAY_100_PERCENT)), 
    ...
    def _get_asset_symbol(self, asset_id):
        """ get the asset symbol from an asset id
            ...
            :@param int asset_id: 0 -> BBD, 1 -> BEX, 2 -> VESTS
    ...
    @property
    def bbd_symbol(self):
        """ get the current chains symbol for BBD (e.g. "TBD" on testnet) """
        try:
    ...
    @property
    def dpay_symbol(self):
        """ get the current chains symbol for BEX (e.g. "TESTS" on testnet) """
