...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/dpaycli/witness.py
# Compiled at: 2018-10-15 03:15:57
# Size of source mod 2**32: 17321 bytes
...
class Witness(BlockchainObject):
    ...
    def __init__(self, owner, full=False, lazy=False, dpay_instance=None):
        self.full = full
        self.lazy = lazy
    ...
    def refresh(self):
        if not self.identifier:
            ...
            raise witness or WitnessDoesNotExistsException(self.identifier)
    ...
    def feed_publish(self, base, quote=None, account=None):
        ...
        if not quote.symbol == self.dpay.dpay_symbol:
            raise AssertionError()
        op = (operations.Feed_publish)(publisher=account['name'], 
    ...
    def printAsTable(self, sort_key='votes', reverse=True, return_str=False, **kwargs):
        ...
        if sort_key == 'base':
            ...
            if sort_key == 'quote':
                ...
                if sort_key == 'last_bbd_exchange_update':
        ...
        for witness in sortedList:
            ...
            t.add_row([witness['owner'],
             str(round(int(witness['votes']) / 1000000000000000.0, 2)),
             disabled,
...
class Witnesses(WitnessesObject):
    ...
    def refresh(self):
        ...
        self.identifier = ''
        super(Witnesses, self).__init__([Witness(x, lazy=(self.lazy), full=(self.full), dpay_instance=(self.dpay)) for x in self.active_witnessess])


class WitnessesVotedByAccount(WitnessesObject):
    __doc__ = ' Obtain a list of witnesses which have been voted by an account\n\n        :param str account: Account name\n        :param dpay dpay_instance: DPay() instance to use when\n            accesing a RPC\n\n        .. code-block:: python\n\n           >>> from dpaycli.witness import WitnessesVotedByAccount\n           >>> WitnessesVotedByAccount("gtg")\n           <WitnessesVotedByAccount gtg>\n\n    '
    ...
    def __init__(self, from_account='', limit=100, lazy=False, full=False, dpay_instance=None):
        ...
        if self.dpay.rpc.get_use_appbase():
            ...
            if last_limit < limit:
                ...
                if self.dpay.rpc.get_use_appbase() and use_condenser:
...
class ListWitnesses(WitnessesObject):
    ...
    def __init__(self, from_account='', limit=100, lazy=False, full=False, dpay_instance=None):
        ...
        if len(witnessess) == 0:
            return
        super(ListWitnesses, self).__init__([Witness(x, lazy=lazy, full=full, dpay_instance=(self.dpay)) for x in witnessess])