...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/dpaycliapi/graphenerpc.py
# Compiled at: 2018-10-15 00:22:13
# Size of source mod 2**32: 19829 bytes
"""graphennewsrpc."""
...
import threading, sys, json, signal, logging, ssl, re, time, warnings, six
from .exceptions import UnauthorizedError, RPCConnection, RPCError, RPCErrorDoRetry, NumRetriesReached, CallRetriesReached, WorkingNodeMissing, TimeoutException
from .rpcutils import is_network_appbase_ready, get_api_name, get_query
from .node import Nodes
from dpaycligraphenebase.version import version as dpaycli_version
...
class SessionInstance(object):
    __doc__ = 'Singelton for the Session Instance'
    instance = None
...
class GrapheneRPC(object):
        ...
        while next_url:
            ...
            if self.url[:3] == 'wss':
                ...
                if self.url[:2] == 'ws':
                    ...
                    self.headers = {'User-Agent':'dpaycli v%s' % dpaycli_version,  'content-type':'application/json'}
            try:
                if self.ws:
                    self.ws.connect(self.url)
                    self.rpclogin(self.user, self.password)
                elif self.disable_chain_detection:
    ...
    def version_string_to_int(self, network_version):
        ...
        return int(int(version_list[0]) * 100000000.0 + int(version_list[1]) * 10000.0 + int(version_list[2]))
            ...
            for k, v in list(self.known_chains.items()):
                if v['chain_id'] == chain_id and self.version_string_to_int(v['min_version']) <= self.version_string_to_int(network_version):
                    if highest_version_chain is None:
                        ...
                        if v['min_version'] == '0.19.5' and self.use_condenser:
                            highest_version_chain = v
                if v['min_version'] == '0.0.0' and self.use_condenser:
                    highest_version_chain = v
                else:
                    if self.version_string_to_int(v['min_version']) > self.version_string_to_int(highest_version_chain['min_version']) and not self.use_condenser:
                        highest_version_chain = v
    ...
    def _check_for_server_error(self, reply):
        ...
        if re.search('Internal Server Error', reply) or re.search('500', reply):
            ...
            if re.search('Not Implemented', reply) or re.search('501', reply):
                raise RPCError('Not Implemented')
            else:
                if re.search('Bad Gateway', reply) or re.search('502', reply):
                    raise RPCErrorDoRetry('Bad Gateway')
                else:
                    if re.search('Service Temporarily Unavailable', reply) or re.search('Service Unavailable', reply) or re.search('503', reply):
                        raise RPCErrorDoRetry('Service Temporarily Unavailable')
                    else:
                        if re.search('Gateway Time-out', reply) or re.search('Gateway Timeout', reply) or re.search('504', reply):
                            raise RPCErrorDoRetry('Gateway Time-out')
                        else:
                            if re.search('HTTP Version not supported', reply) or re.search('505', reply):
                                raise RPCError('HTTP Version not supported')
                            else:
                                if re.search('Variant Also Negotiates', reply) or re.search('506', reply):
                                    raise RPCError('Variant Also Negotiates')
                                else:
                                    if re.search('Insufficient Storage', reply) or re.search('507', reply):
                                        raise RPCError('Insufficient Storage')
                                    else:
                                        if re.search('Loop Detected', reply) or re.search('508', reply):
                                            raise RPCError('Loop Detected')
                                        else:
                                            if re.search('Bandwidth Limit Exceeded', reply) or re.search('509', reply):
                                                raise RPCError('Bandwidth Limit Exceeded')
                                            else:
                                                if re.search('Not Extended', reply) or re.search('510', reply):
                                                    raise RPCError('Not Extended')
                                                else:
                                                    if re.search('Network Authentication Required', reply) or re.search('511', reply):
    ...
    def rpcexec(self, payload):
        ...
        while True:
            ...
            try:
                if self.current_rpc == self.rpc_methods['ws'] or self.current_rpc == self.rpc_methods['wsappbase']:
