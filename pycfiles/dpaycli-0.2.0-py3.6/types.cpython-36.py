...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/dpaycligraphenebase/types.py
# Compiled at: 2018-10-14 09:33:48
# Size of source mod 2**32: 9906 bytes
"""types."""
    ...
    def __bytes__(self):
        ...
        if not self.data:
            ...
            if py23_bytes(self.data):
                return py23_bytes(Bool(1)) + py23_bytes(self.data)
            return py23_bytes(Bool(0))
...
class Enum8(Uint8):
    ...
    def __init__(self, selection):
        if selection not in self.options:
            if not isinstance(selection, int) and len(self.options) < selection:
