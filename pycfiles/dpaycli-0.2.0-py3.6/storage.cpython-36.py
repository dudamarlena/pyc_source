...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/dpaycli/storage.py
# Compiled at: 2018-10-15 03:19:35
# Size of source mod 2**32: 23769 bytes
...
class DataDir(object):
    ...
    def sqlite3_backup(self, backupdir):
        ...
        if not os.path.isdir(backupdir):
            os.mkdir(backupdir)
        backup_file = os.path.join(backupdir, os.path.basename(self.storageDatabase) + datetime.utcnow().strftime('-' + timeformat))
    ...
    def recover_with_latest_backup(self, backupdir='backups'):
        ...
        for filename in os.listdir(backupdir):
            ...
            if os.stat(backup_file).st_ctime > file_date and os.path.isfile(backup_file):
    ...
    def clean_data(self):
        ...
        for filename in os.listdir(self.data_dir):
            ...
            if os.stat(backup_file).st_ctime < time.time() - 6048000 and os.path.isfile(backup_file):
...
class Key(DataDir):
    __doc__ = ' This is the key storage that stores the public key and the\n        (possibly encrypted) private key in the `keys` table in the\n        SQLite3 database.\n    '
    __tablename__ = 'keys'
    ...
    def create_table(self):
        ...
        """
        query = 'CREATE TABLE {0} (id INTEGER PRIMARY KEY AUTOINCREMENT,pub STRING(256),wif STRING(256))'.format(self.__tablename__)
        connection = sqlite3.connect(self.sqlDataBaseFile)
        cursor = connection.cursor()
        cursor.execute(query)
        connection.commit()
    ...
    def getPrivateKeyForPublicKey(self, pub):
        ...
        """
        query = (
         'SELECT wif from {0} WHERE pub=?'.format(self.__tablename__), (pub,))
        connection = sqlite3.connect(self.sqlDataBaseFile)
        cursor = connection.cursor()
        (cursor.execute)(*query)
        key = cursor.fetchone()
    ...
    def updateWif(self, pub, wif):
        ...
        """
        query = (
         'UPDATE {0} SET wif=? WHERE pub=?'.format(self.__tablename__), (wif, pub))
        connection = sqlite3.connect(self.sqlDataBaseFile)
        cursor = connection.cursor()
        (cursor.execute)(*query)
        connection.commit()
    ...
    def add(self, wif, pub):
        ...
        query = (
         'INSERT INTO {0} (pub, wif) VALUES (?, ?)'.format(self.__tablename__), (pub, wif))
        connection = sqlite3.connect(self.sqlDataBaseFile)
        cursor = connection.cursor()
        (cursor.execute)(*query)
        connection.commit()
    ...
    def delete(self, pub):
        ...
        """
        query = (
         'DELETE FROM {0} WHERE pub=?'.format(self.__tablename__), (pub,))
        connection = sqlite3.connect(self.sqlDataBaseFile)
        cursor = connection.cursor()
        (cursor.execute)(*query)
        connection.commit()
...
class Token(DataDir):
    __doc__ = ' This is the token storage that stores the public username and the\n        (possibly encrypted) token in the `token` table in the\n        SQLite3 database.\n    '
    __tablename__ = 'token'
    ...
    def create_table(self):
        """ Create the new table in the SQLite database
        """
        query = 'CREATE TABLE {0} (id INTEGER PRIMARY KEY AUTOINCREMENT,name STRING(256),token STRING(256))'.format(self.__tablename__)
        connection = sqlite3.connect(self.sqlDataBaseFile)
        cursor = connection.cursor()
        cursor.execute(query)
        connection.commit()
    ...
    def getTokenForPublicName(self, name):
        ...
        """
        query = (
         'SELECT token from {0} WHERE name=?'.format(self.__tablename__), (name,))
        connection = sqlite3.connect(self.sqlDataBaseFile)
        cursor = connection.cursor()
        (cursor.execute)(*query)
    ...
    def updateToken(self, name, token):
        ...
        """
        query = (
         'UPDATE {0} SET token=? WHERE name=?'.format(self.__tablename__), (token, name))
        connection = sqlite3.connect(self.sqlDataBaseFile)
        cursor = connection.cursor()
        (cursor.execute)(*query)
        connection.commit()
    ...
    def add(self, name, token):
        ...
        if self.getTokenForPublicName(name):
            raise ValueError('Key already in storage')
        query = (
         'INSERT INTO {0} (name, token) VALUES (?, ?)'.format(self.__tablename__), (name, token))
        connection = sqlite3.connect(self.sqlDataBaseFile)
        cursor = connection.cursor()
        (cursor.execute)(*query)
        connection.commit()
    ...
    def delete(self, name):
        ...
        """
        query = (
         'DELETE FROM {0} WHERE name=?'.format(self.__tablename__), (name,))
        connection = sqlite3.connect(self.sqlDataBaseFile)
        cursor = connection.cursor()
        (cursor.execute)(*query)
        connection.commit()
...
class Configuration(DataDir):
    __doc__ = ' This is the configuration storage that stores key/value\n        pairs in the `config` table of the SQLite3 database.\n    '
    __tablename__ = 'config'
    nodelist = NodeList()
    nodes = nodelist.get_nodes(normal=True, appbase=True, dev=False, testnet=False)
    config_defaults = {'node':nodes, 
     'password_storage':'environment', 
     'rpcpassword':'', 
     'rpcuser':'', 
     'order-expiration':604800, 
     'client_id':'', 
     'hot_sign_redirect_uri':None, 
     'dpid_api_url':'https://go.dpayid.io/api/', 
     'oauth_base_url':'https://go.dpayid.io/oauth2/'}
    ...
    def create_table(self):
        """ Create the new table in the SQLite database
        """
        query = 'CREATE TABLE {0} (id INTEGER PRIMARY KEY AUTOINCREMENT,key STRING(256),value STRING(256))'.format(self.__tablename__)
        connection = sqlite3.connect(self.sqlDataBaseFile)
        cursor = connection.cursor()
        try:
            cursor.execute(query)
    ...
    def checkBackup(self):
        ...
        """
        if 'lastBackup' not in configStorage or configStorage['lastBackup'] == '':
            print('No backup has been created yet!')
            self.refreshBackup()
        try:
            if (datetime.utcnow() - datetime.strptime(configStorage['lastBackup'], timeformat)).days > 7:
    ...
    def _haveKey(self, key):
        ...
        """
        query = (
         'SELECT value FROM {0} WHERE key=?'.format(self.__tablename__), (key,))
        connection = sqlite3.connect(self.sqlDataBaseFile)
        cursor = connection.cursor()
        try:
            (cursor.execute)(*query)
    ...
    def __setitem__(self, key, value):
        if self._haveKey(key):
            query = (
             'UPDATE {0} SET value=? WHERE key=?'.format(self.__tablename__), (value, key))
        else:
            query = (
             'INSERT INTO {0} (key, value) VALUES (?, ?)'.format(self.__tablename__), (key, value))
        connection = sqlite3.connect(self.sqlDataBaseFile)
        cursor = connection.cursor()
        try:
            (cursor.execute)(*query)
    ...
    def delete(self, key):
        ...
        """
        query = (
         'DELETE FROM {0} WHERE key=?'.format(self.__tablename__), (key,))
        connection = sqlite3.connect(self.sqlDataBaseFile)
        cursor = connection.cursor()
        try:
            (cursor.execute)(*query)
...
class MasterPassword(object):
    __doc__ = ' The keys are encrypted with a Masterpassword that is stored in\n        the configurationStore. It has a checksum to verify correctness\n        of the password\n    '
    password = ''
    decrypted_master = ''
    ...
    def newMaster(self):
        ...
        if self.config_key in configStorage:
            if configStorage[self.config_key]:
                return
    ...
    def getEncryptedMaster(self):
        ...
        aes = AESCipher(self.password)
        return '{}${}'.format(self.deriveChecksum(self.decrypted_master), aes.encrypt(self.decrypted_master))
