...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/dpaycli/comment.py
# Compiled at: 2018-10-15 00:29:07
# Size of source mod 2**32: 33087 bytes
from __future__ import absolute_import
from __future__ import division
from __future__ import print_function
from __future__ import unicode_literals
from builtins import str
import json, re, logging, pytz, math
...
class Comment(BlockchainObject):
    ...
    def __init__(self, authorperm, full=True, lazy=False, dpay_instance=None):
        ...
        if isinstance(authorperm, dict):
            if 'author' in authorperm:
                if 'permlink' in authorperm:
        ...
        super(Comment, self).__init__(authorperm,
          id_item='authorperm',
          lazy=lazy,
          full=full,
    ...
    def _parse_json_data(self, comment):
        ...
        bbd_amounts = ['total_payout_value',
         'max_accepted_payout',
    ...
    def json(self):
        ...
        parse_times = ['active', 'cashout_time', 'created', 'last_payout', 'last_update',
         'max_cashout_time']
        for p in parse_times:
            if p in output:
    ...
    def get_curation_penalty(self, vote_time=None):
        ...
        if vote_time is None:
            ...
            if isinstance(vote_time, str):
                ...
                if isinstance(vote_time, (datetime, date)):
    ...
    def get_rewards(self):
        ...
        return {'total_payout':total_payout, 
         'author_payout':author_payout,  'curator_payout':curator_payout}
    ...
    def get_author_rewards(self):
        ...
        if not self.is_pending():
            ...
            bbd_dpay = author_tokens * self['percent_dpay_dollars'] / 20000.0
            vesting_dpay = median_price.as_base(self.dpay.dpay_symbol) * (author_tokens - bbd_dpay)
            return {'pending_rewards':True, 
             'payout_SP':vesting_dpay,  'payout_BBD':bbd_dpay,  'total_payout_BBD':author_tokens}
    ...
    def get_curation_rewards(self, pending_payout_BBD=False, pending_payout_value=None):
        ...
        return {'pending_rewards':pending_rewards, 
         'unclaimed_rewards':unclaimed_rewards,  'active_votes':active_votes}
    ...
    def vote(self, weight, account=None, identifier=None, **kwargs):
        ...
        if not account:
            ...
            if vote_weight < -DPAY_100_PERCENT:
                vote_weight = -DPAY_100_PERCENT
        op = (operations.Vote)(voter=account['name'], 
         author=post_author, 
    ...
    def edit(self, body, meta=None, replace=False):
        ...
        if not newbody:
            ...
            reply_identifier = construct_authorperm(original_post['parent_author'], original_post['parent_permlink'])
            new_meta = {}
            if meta is not None:
                if bool(original_post['json_metadata']):
                    ...
                    new_meta = meta
            return self.dpay.post((original_post['title']),
              newbody,
    ...
    def delete(self, account=None, identifier=None):
        ...
        if not account:
            ...
            if not account:
                raise ValueError('You need to provide an account')
            account = Account(account, dpay_instance=(self.dpay))
            post_author = identifier or self['author']
            post_permlink = self['permlink']
        else:
            post_author, post_permlink = resolve_authorperm(identifier)
        op = (operations.Delete_comment)(author=post_author, 
         permlink=post_permlink)
