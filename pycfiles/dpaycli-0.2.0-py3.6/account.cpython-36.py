...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/dpaycligraphenebase/account.py
# Compiled at: 2018-10-15 03:27:18
# Size of source mod 2**32: 16117 bytes
...
import hashlib, sys, re, os, codecs, ecdsa, ctypes
...
class PasswordKey(object):
    ...
    def __init__(self, account, password, role='active', prefix='DWB'):
...
class Address(object):
    ...
    def derive256address_with_version(self, version=56):
        ...
        if pubkey is None:
            ...
            pkbin = unhexlify(repr(pubkey))
            addressbin = ripemd160(hexlify(hashlib.sha256(pkbin).digest()))
            addr = py23_bytes(bytearray(ctypes.c_uint8(version & 255))) + addressbin
