...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/dpaycli/amount.py
# Compiled at: 2018-10-15 03:20:04
...
class Amount(dict):
    ...
    def __init__(self, amount, asset=None, new_appbase_format=True, dpay_instance=None):
        ...
        if amount and asset is None and isinstance(amount, list) and len(amount) == 3:
            self['amount'] = int(amount[0]) / 10 ** amount[1]
            self['asset'] = Asset((amount[2]), dpay_instance=(self.dpay))
            self['symbol'] = self['asset']['symbol']
        else:
            if amount and asset is None and isinstance(amount, dict) and 'amount' in amount and 'nai' in amount and 'precision' in amount:
                ...
                if asset:
                    if isinstance(asset, string_types):
                        self['amount'] = amount
                        self['asset'] = Asset(asset, dpay_instance=(self.dpay))
                        self['symbol'] = self['asset']['symbol']
            elif isinstance(amount, (integer_types, float)):
                if asset:
                    if isinstance(asset, Asset):
                        self['amount'] = amount
                        self['asset'] = asset
    ...
    def copy(self):
        ...
        """
        return Amount(amount=(self['amount']),
        ...
        if self.dpay.is_connected():
            if self.dpay.rpc.get_use_appbase():
                if self.new_appbase_format:
                    ...
                    return [
                     str(int(self)), self['asset']['precision'], self['asset']['asset']]
        else:
            return str(self)

    def __str__(self):
        return '{:.{prec}f} {}'.format((self['amount']),
          (self['symbol']),
    ...
    def __floordiv__(self, other):
        ...
        if isinstance(other, Amount):
            ...
            return Price(self, other, dpay_instance=(self.dpay))
        else:
            a['amount'] //= other
            return a
