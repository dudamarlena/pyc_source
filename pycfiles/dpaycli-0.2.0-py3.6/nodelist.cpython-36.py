...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/dpaycli/nodelist.py
# Compiled at: 2018-10-15 03:08:05
    ...
    def update_nodes(self, weights=None, dpay_instance=None):
        ...
        if weights is None:
            ...
            for benchmark in benchmarks:
                weights_dict[benchmark] = 1.0 / len(benchmarks)
    ...
    def get_nodes(self, normal=True, appbase=True, dev=False, testnet=False, testnetdev=False, wss=True, https=True, not_working=False):
        ...
        for node in self:
            if node['type'] in node_type_list:
                if node['score'] >= 0 or not_working:
                    if not https:
                        if node['url'][:5] == 'https':
                            continue
                    if not wss:
