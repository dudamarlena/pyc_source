...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/dpaycli/conveyor.py
...
import hashlib, base64, json, random, requests, struct
...
class Conveyor(object):
    ...
    def __init__(self, url='https://api.dsite.io', dpay_instance=None):
        ...
        self.K = hashlib.sha256(py23_bytes('dpay_jsonrpc_auth', self.ENCODING)).digest()
    ...
    def _request(self, account, method, params, key):
        ...
        request = {'jsonrpc':'2.0', 
         'id':self.id, 
         'method':method, 
         'params':{'__signed': {'account':account, 
                       'nonce':nonce_str, 
                       'params':params_enc, 
                       'signatures':[
                        signature_hex], 
    ...
    def get_user_data(self, account, signing_account=None):
        ...
        """
        account = Account(account, dpay_instance=(self.dpay))
        user_data = self._conveyor_method(account, signing_account, 'conveyor.get_user_data', [
         account['name']])
    ...
    def set_user_data(self, account, params, signing_account=None):
        ...
        """
        return self._conveyor_method(account, signing_account, 'conveyor.set_user_data', [
         params])
    ...
    def get_feature_flags(self, account, signing_account=None):
        ...
        """
        account = Account(account, dpay_instance=(self.dpay))
        feature_flags = self._conveyor_method(account, signing_account, 'conveyor.get_feature_flags', [
         account['name']])
    ...
    def get_feature_flag(self, account, flag, signing_account=None):
        ...
        """
        account = Account(account, dpay_instance=(self.dpay))
        return self._conveyor_method(account, signing_account, 'conveyor.get_feature_flag', [
         account['name'], flag])
    ...
    def save_draft(self, account, title, body):
        ...
        return self._conveyor_method(account, None, 'conveyor.save_draft', [
         account['name'], draft])
    ...
    def list_drafts(self, account):
        ...
        """
        account = Account(account, dpay_instance=(self.dpay))
        return self._conveyor_method(account, None, 'conveyor.list_drafts', [
         account['name']])
    ...
    def remove_draft(self, account, uuid):
        ...
        """
        account = Account(account, dpay_instance=(self.dpay))
        return self._conveyor_method(account, None, 'conveyor.remove_draft', [
         account['name'], uuid])
