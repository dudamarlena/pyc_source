...
# [GCC 8.4.0]
# Embedded file name: /Users/erikvw/.venvs/ambition/lib/python3.7/site-packages/ambition_validators/form_validators/patient_history.py
...
class PatientHistoryFormValidator(FormValidator):
    ...
    def clean(self):
        self.m2m_other_specify(HEADACHE,
          m2m_field='symptom', field_other='headache_duration')
        self.m2m_other_specify(VISUAL_LOSS,
          m2m_field='symptom', field_other='visual_loss_duration')
        ...
        self.applicable_if(YES,
          field='taking_arv', field_applicable='first_arv_regimen')
        self.validate_other_specify(field='first_arv_regimen')
        self.validate_other_specify(field='second_arv_regimen')
        self.applicable_if(YES,
          field='taking_arv', field_applicable='first_line_choice')
        if self.cleaned_data.get('first_arv_regimen') == OTHER:
            if self.cleaned_data.get('first_line_choice') != OTHER:
                raise forms.ValidationError({'first_line_choice': 'Invalid. Expected "Other"'})
        if self.cleaned_data.get('first_arv_regimen') != OTHER:
            if self.cleaned_data.get('first_line_choice') == OTHER:
                raise forms.ValidationError({'first_line_choice': 'Invalid. Cannot be "Other"'})
        self.applicable_if(YES,
          field='taking_arv', field_applicable='patient_adherence')
        ...
        self.m2m_other_specify('focal_neurologic_deficit',
