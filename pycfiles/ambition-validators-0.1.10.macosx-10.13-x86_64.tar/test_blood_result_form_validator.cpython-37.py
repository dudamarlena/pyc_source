...
# [GCC 8.4.0]
# Embedded file name: /Users/erikvw/.venvs/ambition/lib/python3.7/site-packages/ambition_validators/tests/test_blood_result_form_validator.py
# Compiled at: 2018-08-20 21:57:29
...
class TestBloodResultFormValidator(TestCase):
    ...
    def setUp(self):
        ...
        self.cleaned_data = {'haemoglobin':15, 
    ...
    def test_haemoglobin_units_invalid_female(self):
        ...
        self.cleaned_data.update(haemoglobin=6.4,
          results_abnormal=YES)
        form_validator = BloodResultFormValidator(cleaned_data=(self.cleaned_data),
    ...
    def test_haemoglobin_units_invalid_male(self):
        self.cleaned_data.update(haemoglobin=6.9,
          results_abnormal=YES)
        form_validator = BloodResultFormValidator(cleaned_data=(self.cleaned_data),
          instance=(BloodResult()))
        self.assertRaises(ValidationError, form_validator.validate)
    ...
    def test_haemoglobin_units_male_valid(self):
        self.cleaned_data.update(haemoglobin=14,
          results_abnormal=NO)
        form_validator = BloodResultFormValidator(cleaned_data=(self.cleaned_data),
          instance=(BloodResult()))
        try:
    ...
    def test_creatinine_mg_invalid(self):
        self.cleaned_data.update(creatinine=2.48,
          creatinine_units=MILLIGRAMS_PER_DECILITER,
          results_abnormal=YES)
        form_validator = BloodResultFormValidator(cleaned_data=(self.cleaned_data),
          instance=(BloodResult()))
    ...
    def test_creatinine_mg(self):
        self.cleaned_data.update(creatinine=1.3,
          creatinine_units=MILLIGRAMS_PER_DECILITER,
          results_abnormal=NO,
          results_reportable=NOT_APPLICABLE)
        form_validator = BloodResultFormValidator(cleaned_data=(self.cleaned_data),
    ...
    def test_creatinine_umol_invalid(self):
        self.cleaned_data.update(creatinine=217,
          creatinine_units=MICROMOLES_PER_LITER,
    ...
    def test_magnesium_invalid(self):
        self.cleaned_data.update(magnesium=0.01,
          are_results_normal=YES)
        form_validator = BloodResultFormValidator(cleaned_data=(self.cleaned_data),
          instance=(BloodResult()))
        self.assertRaises(ValidationError, form_validator.validate)
    ...
    def test_potassium_invalid(self):
        self.cleaned_data.update(potassium=1.0,
          results_abnormal=YES)
        form_validator = BloodResultFormValidator(cleaned_data=(self.cleaned_data),
          instance=(BloodResult()))
        self.assertRaises(ValidationError, form_validator.validate)
    ...
    def test_potassium_high(self):
        self.cleaned_data.update(potassium=6.8,
          results_abnormal=YES,
    ...
    def test_sodium_invalid(self):
        self.cleaned_data.update(sodium=100,
          results_abnormal=YES)
        form_validator = BloodResultFormValidator(cleaned_data=(self.cleaned_data),
          instance=(BloodResult()))
        self.assertRaises(ValidationError, form_validator.validate)
    ...
    def test_sodium_invalid_1(self):
        self.cleaned_data.update(sodium=119,
          results_abnormal=YES,
          results_reportable=NOT_APPLICABLE)
        form_validator = BloodResultFormValidator(cleaned_data=(self.cleaned_data),
          instance=(BloodResult()))
    ...
    def test_alt_invalid(self):
        self.cleaned_data.update(alt=201,
          are_results_normal=YES)
        form_validator = BloodResultFormValidator(cleaned_data=(self.cleaned_data),
          instance=(BloodResult()))
        self.assertRaises(ValidationError, form_validator.validate)
    ...
    def test_alt(self):
        self.cleaned_data.update(alt=10,
          results_abnormal=NO)
        form_validator = BloodResultFormValidator(cleaned_data=(self.cleaned_data),
          instance=(BloodResult()))
        try:
    ...
    def test_platelets_invalid(self):
        self.cleaned_data.update(platelets=50,
          results_abnormal=YES)
        form_validator = BloodResultFormValidator(cleaned_data=(self.cleaned_data),
          instance=(BloodResult()))
        self.assertRaises(ValidationError, form_validator.validate)
    ...
    def test_platelets(self):
        self.cleaned_data.update(platelets=450,
          results_abnormal=NO)
        form_validator = BloodResultFormValidator(cleaned_data=(self.cleaned_data),
          instance=(BloodResult()))
        try:
    ...
    def test_neutrophil(self):
        self.cleaned_data.update(neutrophil=4,
          results_abnormal=NO)
        form_validator = BloodResultFormValidator(cleaned_data=(self.cleaned_data),
          instance=(BloodResult()))
        try:
    ...
    def test_crag_country_botswana_crag_control_result_none(self):
        self.cleaned_data.update(absolute_neutrophil=4,
          are_results_normal=YES,
          bios_crag=YES,
