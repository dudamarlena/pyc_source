...
# [GCC 8.4.0]
# Embedded file name: /home/stanley/IdeaProjects/horizon-python-client/src/mf_horizon_client/client/pipelines/pipeline_interface.py
# Compiled at: 2020-05-09 19:22:50
# Size of source mod 2**32: 37663 bytes
import json
from io import StringIO
from time import sleep
from typing import Any, Dict, List, cast, Union
import numpy as np, pandas as pd
from mf_horizon_client.post_processing.backtests import binary_backtests_returns, calculate_metrics
...
from mf_horizon_client.data_structures.configs.stage_config import ProblemSpecificationConfig, StageConfig
...
from mf_horizon_client.utils.progress_bar_helpers import initialise_progress_bar, update_single_pipeline_status
from mf_horizon_client.utils.string_case_converters import convert_dict_from_camel_to_snake, convert_dict_from_snake_to_camel
...
class PipelineInterface:
    ...
    def update_config(self, pipeline_id: int, stage_id: int, config: StageConfig):
        ...
        if not len(stages_matching_id) == 1:
            raise AssertionError('No stage found with given identifier')
        elif not config.valid_configuration_values:
            raise AssertionError('Invalid numeric configuration specified')
        config_dict = dict(config=(convert_dict_from_snake_to_camel(json.loads(config.as_json()))), preview=False)
        self.client.put((Endpoints.UPDATE_STAGE_CONFIGURATION(pipeline_id, stage_id)),
          json=config_dict)
    ...
    def get_feature_info_for_stage(self, pipeline_id: int, stage_id: int) -> pd.DataFrame:
        ...
        def get_children(feature_list, current_horizon):
            """
            ...
            for feature in feature_list:
                if feature['active']:
                    features.append({'feature_name':feature['name'], 
    ...
    def download_feature_info_for_stage(self, pipeline_id: int, stage_id: int) -> Dict[(str, pd.DataFrame)]:
        ...
        for horizon in tqdm(horizons, desc='Fetching Data'):
            data = self.client.get(Endpoints.FEATURE_DATA_FOR_STAGE(pipeline_id=pipeline_id, stage_id=stage_id, horizon=horizon),
              download=True)
    ...
    def download_backtest_info_for_stage(self, pipeline_id: int, stage_id: int, verbose=True) -> Dict[(str, pd.DataFrame)]:
        ...
        for horizon in horizons:
            if pbar:
                if verbose:
                    pbar.update()
            data = self.client.get(Endpoints.BACKTEST_DATA_FOR_STAGE(pipeline_id=pipeline_id, stage_id=stage_id, horizon=horizon),
              download=True)
    ...
    @catch_errors
    def run_expert_backtest_for_validation_data(self, pipeline_id: int, stage_id: int, horizon: int, dataset_id: int, n_training_rows_for_backtest: int=40, verbose=True) -> pd.DataFrame:
        """
        ...
        if not dataset.summary.name == pipeline.summary.dataset_name:
            raise AssertionError('Dataset id specified is different to the pipeline id')
        else:
            if not int(config.target_feature) in dataset.summary.column_ids:
                raise AssertionError('Dataset id specified is different to the pipeline id')
            else:
        ...
        return self.run_expert_backtest_between_two_rows(horizon=horizon,
    ...
    def run_expert_backtest_between_two_rows(self, horizon: int, start_row: int, end_row: int, n_training_rows_for_backtest: int, pipeline_id: int, stage_id: int, verbose=True):
        """
        ...
        response = self.client.get(Endpoints.EXPERT_BACKTEST_FOR_STAGE_AND_HORIZON(pipeline_id=pipeline_id,
          horizon=horizon,
          first_row=start_row,
          last_row=end_row,
    ...
    def add_stage_to_pipeline(self, pipeline_id: int, parent_stage_id: int, stage_type: StageType) -> Pipeline:
        ...
        """
        body = {'parentStage':parent_stage_id, 
         'stageType':stage_type.name}
    ...
    def _build_pipeline_from_template(self, target_column_name: str, pipeline_template: Pipeline):
        ...
        pipeline = self.create_pipeline(dataset_id=(pipeline_template.dataset.id_),
        ...
        for template_stage in pipeline_template.stages:
            if index == 0:
                ...
                pipeline = self.add_stage_to_pipeline(pipeline_id=(pipeline.summary.id_),
                  parent_stage_id=parent_id,
                  stage_type=(StageType[template_stage.type]))
    ...
    def run_multitarget_forecast(self, pipeline_template: Pipeline, *, column_names: List=-1, column_ids: List=-1, n_training_rows_for_one_point_backtest=None, one_point_backtests=False) -> Dict[(str, pd.DataFrame)]:
        """
        ...
        if not column_names == -1:
            if not column_ids == -1:
                raise AssertionError('Please only specify one of column_ids or column_names')
        terminal_messages.print_expert_message('Please ensure that you have no queued or pending pipelines.')
        if column_names == -1:
            column_names = [column.name for column in pipeline_template.dataset.columns if str(column.id_) in column_ids]
        ...
        for column_name in column_names:
            ...
            backtest = self.run_backtesting_for_multitarget(n_training_rows_for_one_point_backtest, one_point_backtests, pipeline, problem_specification_config)
    ...
    def run_backtesting_for_multitarget(self, n_training_rows_for_one_point_backtest, one_point_backtests, pipeline, problem_specification_config):
        if one_point_backtests:
            backtest = self.run_expert_backtest_for_validation_data(pipeline_id=(pipeline.summary.id_),
            ...
            backtest_dict = self.download_backtest_info_for_stage(pipeline_id=(pipeline.summary.id_),
              stage_id=(pipeline.find_stage_by_type(StageType.backtest)[0].id_),
              verbose=False)
            backtest = backtest_dict[str(problem_specification_config.horizons[0])]
        return backtest

    def run_multitarget_forecast_with_target_specific_feature_set(self, pipeline_template: Pipeline, *, column_names: List=-1, column_ids: List=-1, n_training_rows_for_one_point_backtest=None, one_point_backtests=False) -> Dict[(str, pd.DataFrame)]:
        """
        ...
        if column_names == -1:
            column_names = [column.name for column in pipeline_columns if str(column.id_) in column_ids]
        ...
        data_interface = DataInterface(self.client)
        augmented_dataset = data_interface.upload_data(data=augmented_features_no_duplicates,
          name=f"Features {pipeline_template.summary.name}")
        template_pipeline_regression_only = self.create_pipeline(dataset_id=(augmented_dataset.summary.id_),
        ...
        return self.run_multitarget_forecast(pipeline_template=template_pipeline_regression_only,
    ...
    @catch_errors
    def directional_response_regression(self, *, data: pd.DataFrame, target_column: str, cross_section_column_name: str, date_column_name: str, discrete_value_dates: pd.Series=None, train_up_to_row: int=None, n_training_rows_for_one_point_backtest: Union[(str, int)]='auto', blueprint_type: BlueprintType=BlueprintType.nonlinear, cleanup=True):
        """
        ...
        dataset = data_interface.upload_data_long_format_as_single_data_set(data=data,
          name='Panel',
        ...
        template_pipeline = self.create_pipeline(dataset_id=(dataset.summary.id_),
        ...
        results = self.run_multitarget_forecast(pipeline_template=template_pipeline,
        ...
        for variable in unique_cross_section_variables:
            ...
            average_scores[variable] = pd.Series({'accuracy':classification_metrics['accuracy'], 
            ...
            if discrete_value_dates is not None:
                ...
                discrete_values_average_scores[variable] = pd.Series({'accuracy':discrete_values_full_metrics[variable]['accuracy'], 
        ...
        last_observed_values.index = [['/'.join([target_column, variable])] for variable in last_observed_values[cross_section_column_name]]
