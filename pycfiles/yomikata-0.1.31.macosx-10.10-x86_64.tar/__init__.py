...
# [GCC 8.4.0]
# Embedded file name: /Users/nabetama/.pyenv/versions/2.7.8/lib/python2.7/site-packages/yomikata/__init__.py
# Compiled at: 2015-07-24 13:28:12
...
class Yomikata(object):
    ...
    def get_yomikata(self):
        ...
        if not resp.text.count(self.word):
            print(("The way to read of the word wasn't found.").format(self.word))
            return
        else:
            html = lxml.html.fromstring(resp.text)
            answers = html.cssselect('.spAns')
            for answer in answers:
                print(('{}: ({})').format(*self.text_and_points(answer)))
