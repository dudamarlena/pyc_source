...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/flipy/lp_constraint.py
# Compiled at: 2020-03-05 00:34:20
# Size of source mod 2**32: 8427 bytes
import copy, operator, warnings
...
class LpConstraint:
    __doc__ = ' A class representing a linear constraint '

    def __init__(self, lhs: LpExpression, sense: str, rhs: Optional[LpExpression]=None, name: Optional[str]=None, slack: bool=False, slack_penalty: Numeric=0, copy_expr: bool=False) -> None:
    ...
    def breakdown(self) -> List['LpConstraint']:
        ...
        if self.sense != 'eq':
            return [
             self]
        upper_bound_constraint = LpConstraint(name=(self.name + '_ub'),
    ...
    def slack_variable(self) -> LpVariable:
        ...
        self._slack_variable = self._slack_variable or LpVariable(name=(self.name + '_slack_variable'), var_type=(VarType.Continuous), low_bound=0) if self.slack else None
    ...
    def slack_penalty(self, penalty: Numeric) -> None:
        ...
        self._slack_penalty = penalty

    def check(self) -> bool:
        """ Checks if the constraint is satisfied given variable assignments """
        return {'leq':operator.le, 
         'eq':operator.eq, 
         'geq':operator.ge}[self.sense](self.lhs.evaluate() + (0 if not self.slack else self.slack_variable.evaluate() * (-1 if self.sense == 'leq' else 1)), self.rhs.evaluate())
