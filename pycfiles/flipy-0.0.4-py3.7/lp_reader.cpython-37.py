...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/flipy/lp_reader.py
# Compiled at: 2020-03-11 12:17:21
# Size of source mod 2**32: 21245 bytes
import os, io, re
...
class LpReader:
    __doc__ = ' A class for reading lp files '
    _numeric_regex_pattern = re.compile('^ \\s* [-+]? \\s* (?: (?: \\d* \\. \\d+ ) | (?: \\d+ \\.? ) )(?: [Ee] [+-]? \\d+ ) ?', re.VERBOSE)
    _sense_mapping = {'<=':'leq',  '=<':'leq',  '>=':'geq',  '=>':'geq',  '<':'lt',  '>':'gt',  '=':'eq'}
    _sense_reverse_mapping = {'leq':'geq', 
     'geq':'leq',  'lt':'gt',  'gt':'lt',  'eq':'eq'}
    ...
    def _parse_term(cls, term: str) -> Tuple[(str, Numeric)]:
        ...
        if not '0' < var[0] < '9':
            if var[0] == '.':
                raise NameError(f"variable '{var}' does not have a valid name: A variable name should not begin with a number or a period")
            if ' ' in var:
                raise NameError(f"variable '{var}' does not have a valid name: A variable name should not have whitespaces")
        for char in var:
    ...
    def _mathify_expression(cls, expr_str: str) -> Tuple[(Mapping[(str, Numeric)], Numeric)]:
        ...
        if not expr_str:
            return (
             expr, const)
        tokens = [token.strip() for token in re.split('(-|\\+)', expr_str)]
        sign = 1
        for token in tokens:
            if not token:
                continue
    ...
    def _remove_comments(cls, content: str) -> str:
        r""" Remove the comments from a string
    ...
    def _search_keywords(content: str, keywords: Iterable[str]) -> Tuple[(str, int, int)]:
        ...
        """
        sense_pattern = re.compile('|'.join((f"\\b{re.escape(keyword)}\\b" for keyword in keywords)), re.IGNORECASE)
    ...
    def _split_content_by_sections(cls, content: str) -> Tuple[(bool, Mapping[(str, str)])]:
        ...
        """
        keywords = {'objective':('minimize', 'maximize', 'minimum', 'maximum', 'min', 'max'), 
    ...
    def _find_problem_name(cls, content: str) -> str:
        ...
        """
        pattern = re.compile('(?<=\\\\)\\s* [\\w\\s_]+ \\s*(?=\\\\)', re.DOTALL)
    ...
    @classmethod
    def _parse_variables(cls, lp_variables: Mapping[(str, LpVariable)], bounds: Mapping[(str, Mapping[(str, float)])], generals: List[str], binaries: List[str]):
    ...
    def _parse_constraints(cls, constraints: str) -> List[Mapping[(str, Union[(str, float)])]]:
        ...
        sense_pattern = re.compile('|'.join(cls._sense_mapping.keys()))
        ...
        for i, _ in enumerate(sense_matches):
            ...
            parsed_constraints.append({'name':cons_name, 
             'lhs':lhs, 
             'lhs_const':lhs_const, 
             'sense':sense, 
             'rhs':rhs, 
        ...
        for i, term in enumerate(terms):
            ...
            if term_type == 'free':
                ...
                if previous_term_type == 'variable':
                    ...
                    if previous_term_type == 'constant':
