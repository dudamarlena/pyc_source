...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/flipy/lp_problem.py
# Compiled at: 2020-03-16 22:16:58
...
class LpProblem:
    __doc__ = ' A class representing a linear programming problem '

    def __init__(self, name: str, lp_objective: Optional[LpObjective]=None, lp_constraints: Optional[List[LpConstraint]]=None) -> None:
    ...
    def write_lp(self, buffer):
        ...
        obj_slack_expr = {constraint.slack_variable:constraint.slack_penalty * (1 if self.lp_objective.sense == Minimize else -1) for constraint in self.lp_constraints.values() if constraint.slack}
        terms = [
         f"{objective_name}:"] + self.lp_objective.to_lp_terms(slack=obj_slack_expr)
        ...
        integer_vars = [var for var in sorted_lp_variables if var.var_type is VarType.Integer]
        if integer_vars:
