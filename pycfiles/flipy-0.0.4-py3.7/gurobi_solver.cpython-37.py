...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/flipy/solvers/gurobi_solver.py
# Compiled at: 2020-03-16 22:26:09
# Size of source mod 2**32: 10441 bytes
import os, tempfile
...
class GurobiSolver:
    __doc__ = ' A class for interfacing with gurobi to solve LPs '
    STATUS_MAPPING = {gurobipy.GRB.OPTIMAL: SolutionStatus.Optimal, 
    ...
    def add_slack_variables(self, lp_problem: LpProblem, model: gurobipy.Model) -> None:
        ...
        for _, constraint in lp_problem.lp_constraints.items():
            if constraint.slack:
                self.add_variable(constraint.slack_variable, (-1 if lp_problem.lp_objective.sense == Maximize else 1) * constraint.slack_penalty, model)
    ...
    def retrieve_values(lp_problem: LpProblem, lp_variables: List[LpVariable], model: gurobipy.Model) -> None:
        ...
        """
        try:
            var_name_to_values = dict(zip(model.getAttr(gurobipy.GRB.Attr.VarName, model.getVars()), model.getAttr(gurobipy.GRB.Attr.X, model.getVars())))
            for var in lp_variables:
