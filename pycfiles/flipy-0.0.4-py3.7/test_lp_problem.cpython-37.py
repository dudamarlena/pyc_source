...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/tests/test_lp_problem.py
...
class TestLpProblem(object):
    ...
    def test_init(self):
        ...
        if not (len(problem.lp_constraints) == 0 and isinstance(problem.lp_constraints, dict)):
            raise AssertionError
        if not (len(problem.lp_variables) == 0 and isinstance(problem.lp_variables, dict)):
            raise AssertionError
