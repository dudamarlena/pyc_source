...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.4/site-packages/blackdog/config.py
...
def config_node(node, type=str, section=None):
    ...
    def decorator(fun):
        config_field = '__config__' + re.sub('[^a-zA-Z0-9_]', '_', node)
...
def load(config: ConfigParser, obj, section):
    ...
    for name, method in [(n, m) for n, m in attrs if callable(m) and hasattr(m, 'config_node')]:
        ...
        if val:
            ...
            if issubclass(method.config_type, Enum):
