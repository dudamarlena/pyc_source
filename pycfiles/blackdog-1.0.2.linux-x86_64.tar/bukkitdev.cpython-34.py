...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.4/site-packages/blackdog/bukkitdev.py
...
class BukkitDev(object):
    ...
    def _fill_plugin_meta(self, plugin: Plugin, version=None):
        ...
        try:
            ...
            while True:
                ...
                list = [tr for tr in d('table.listing tbody tr').items() if tr('td.col-filename').text().endswith('.jar')]
                for tr in list:
                    version_str = ''
                    try:
                        version_str = re.sub('.*?([0-9]+(\\.[0-9]+)+).*', '\\1', tr('td.col-file').text())
                        metalink = tr('td.col-file a').attr('href')
                        version_found = version == version_str or version == 'latest'
                        if metalink:
                            if version_str:
                                if not version or version_found:
    ...
    def scan(self, stages=None):
        if not stages:
            stages = [
             PluginStage.release, PluginStage.mature]
