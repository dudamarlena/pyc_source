...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/gpapidl/config.py
...
def getDevicesReadableNames():
    ...
    return [{'codename':s,  'readableName':config.get(s).get('userreadablename')} for s in getDevicesCodenames()]
...
class DeviceBuilder(object):
    ...
    def setLocale(self, locale):
        ...
        if match('[a-z]{2}\\_[A-Z]{2}', locale) is None:
            raise InvalidLocaleError()
        self.locale = locale
    ...
    def getUserAgent(self):
        ...
        return 'Android-Finsky/{versionString} (api=3,versionCode={versionCode},sdk={sdk},device={device},hardware={hardware},product={product},platformVersionRelease={platform_v},model={model},buildId={build_id},isWideScreen=0,supportedAbis={supported_abis})'.format(versionString=version_string,
    ...
    def getAuthHeaders(self, gsfid):
        headers = {'User-Agent': 'GoogleAuth/1.4 ({device} {id})'.format(device=(self.device.get('build.device')),
