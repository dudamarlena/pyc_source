...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/gpapidl/googleplay.py
# Compiled at: 2020-04-21 12:12:34
# Size of source mod 2**32: 28385 bytes
...
class GooglePlayAPI(object):
    ...
    def encryptPassword(self, login, passwd):
        ...
        ciphertext = publicKey.encrypt(to_be_encrypted, padding.OAEP(mgf=padding.MGF1(algorithm=(hashes.SHA1())),
    ...
    def checkin(self, email, ac2dmToken):
        ...
        res = requests.post(CHECKIN_URL, data=stringRequest, headers=headers,
          verify=ssl_verify,
    ...
    def uploadDeviceConfig(self):
        ...
        response = requests.post(UPLOAD_URL, data=stringRequest, headers=headers,
          verify=ssl_verify,
          timeout=60,
          proxies=(self.proxies_config))
    ...
    def login(self, email=None, password=None, gsfId=None, authSubToken=None):
        ...
        if email is not None and password is not None:
            ...
            if 'auth' in params:
                ...
                if 'error' in params:
                    if 'NeedsBrowser' in params['error']:
                        raise SecurityCheckError('Security check is needed, try to visit https://accounts.google.com/b/0/DisplayUnlockCaptcha to unlock, or setup an app-specific password')
            ...
            if gsfId is not None and authSubToken is not None:
                self.gsfId = gsfId
    ...
    def getAuthSubToken(self, email, passwd):
        ...
        response = requests.post(AUTH_URL, data=requestParams,
          verify=ssl_verify,
          headers=headers,
          proxies=(self.proxies_config))
        data = response.text.split()
    ...
    def executeRequestApi2(self, path, post_data=None, content_type=CONTENT_TYPE_URLENC, params=None):
        if self.authSubToken is None:
            ...
            if post_data is not None:
                response = requests.post(path, data=(str(post_data)),
                  headers=headers,
                  params=params,
                  verify=ssl_verify,
                  timeout=60,
    ...
    def bulkDetails(self, packageNames):
        ...
        message = self.executeRequestApi2(BULK_URL, post_data=(data.decode('utf-8')),
    ...
    def _deliver_data(self, url, cookies):
        ...
        chunk_size = 32768
        return {'data':response.iter_content(chunk_size=chunk_size),  'total_size':total_size, 
    ...
    def delivery(self, packageName, versionCode=None, offerType=1, downloadToken=None, expansion_files=False):
        ...
        if versionCode is None:
            ...
            response = requests.get(DELIVERY_URL, headers=headers, params=params,
              verify=ssl_verify,
              timeout=60,
              proxies=(self.proxies_config))
            response = googleplay_pb2.ResponseWrapper.FromString(response.content)
    ...
    def download(self, packageName, versionCode=None, offerType=1, expansion_files=False):
        ...
        if self.authSubToken is None:
            ...
            if versionCode is None:
                ...
                versionCode = appDetails.get('versionCode')
            headers = self.getHeaders()
            params = {'ot':str(offerType),  'doc':packageName, 
             'vc':str(versionCode)}
            self.log(packageName)
            response = requests.post(PURCHASE_URL, headers=headers, params=params,
              verify=ssl_verify,
              timeout=60,
              proxies=(self.proxies_config))
            response = googleplay_pb2.ResponseWrapper.FromString(response.content)
    ...
    def log(self, docid):
        ...
        response = requests.post(LOG_URL, data=string_request,
          headers=(self.getHeaders()),
          verify=ssl_verify,
          timeout=60,
          proxies=(self.proxies_config))
    ...
    def toc(self):
        response = requests.get(TOC_URL, headers=(self.getHeaders()),
          verify=ssl_verify,
          timeout=60,
          proxies=(self.proxies_config))
        data = googleplay_pb2.ResponseWrapper.FromString(response.content)
        tocResponse = data.payload.tocResponse
        if utils.hasTosContent(tocResponse):
            if utils.hasTosToken(tocResponse):
    ...
    def acceptTos(self, tosToken):
        params = {'tost':tosToken, 
         'toscme':'false'}
        response = requests.get(ACCEPT_TOS_URL, headers=(self.getHeaders()),
          params=params,
          verify=ssl_verify,
          timeout=60,
          proxies=(self.proxies_config))
