...
# [GCC 8.4.0]
# Embedded file name: /Users/daniel/workarea/preset/Flask-AppBuilder/flask_appbuilder/tests/base.py
# Compiled at: 2020-03-31 08:10:13
# Size of source mod 2**32: 2698 bytes
import json, unittest
from flask_appbuilder.const import API_SECURITY_PASSWORD_KEY, API_SECURITY_PROVIDER_KEY, API_SECURITY_USERNAME_KEY, API_SECURITY_VERSION
...
class FABTestCase(unittest.TestCase):
    ...
    def auth_client_put(client, token, uri, json):
        return client.put(uri,
          json=json, headers={'Authorization': 'Bearer {}'.format(token)})
    ...
    def _login(client, username, password):
        ...
        """
        return client.post(('api/{}/security/login'.format(API_SECURITY_VERSION)),
          data=(json.dumps({API_SECURITY_USERNAME_KEY: username, 
    ...
    @staticmethod
    def create_user(appbuilder, username, password, role_name, first_name='admin', last_name='user', email='admin@fab.org'):
        role_admin = appbuilder.sm.find_role(role_name)
        appbuilder.sm.add_user(username, first_name, last_name, email, role_admin, password)