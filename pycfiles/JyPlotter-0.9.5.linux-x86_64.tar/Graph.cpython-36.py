...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.6/site-packages/PyPlotter/Graph.py
# Compiled at: 2017-05-19 15:13:22
# Size of source mod 2**32: 46897 bytes
...
try:
    import Gfx, Colors, psGfx
except ImportError:
    from . import Gfx, Colors, psGfx

class CoordinateTransformer(object):
    __doc__ = 'Transforms virtual coordinates to screen (or printer) coordinates\n\n    Attributes (read only!):\n        vx1, vy1, vx2, vy1  -   virtual coordinate range\n        sx1, sy1, sx2, sy2  -   screen coordinate range\n        keepAspect          -   keep aspect ratio (boolean)\n    '

    def __init__(self, x1, y1, x2, y2, vx1=-1.0, vy1=-1.0, vx2=1.0, vy2=1.0, keepAspectRatio=True):
...
class VirtualScreen(object):
    ...
    def __init__(self, gfx, x1, y1, x2, y2, region, keepAspectRatio):
        ...
        self.tr = CoordinateTransformer(scr[0], scr[1], scr[2], scr[3], x1, y1, x2, y2, keepAspectRatio)
...
class HardFramedScreen(VirtualScreen):
    ...
    def __init__(self, gfx, x1, y1, x2, y2, region, top, bottom, left, right, keepAspect, overlap=(1, 1, 0, 0)):
        ...
        VirtualScreen.__init__(self, gfx, x1, y1, x2, y2, region, keepAspect)
    ...
    def innerFrame(self):
        ...
        frame."""
        return (
         self.sx1 + self.left, self.sy1 + self.bottom,
    ...
    def topFrame(self):
        ...
        if self.overlap[2] > self.overlap[0]:
            ...
            sx1 = self.sx1
        return (
         sx1, self.sy2 - self.top + 1, sx2, self.sy2)
    ...
    def bottomFrame(self):
        ...
        if self.overlap[2] > self.overlap[1]:
            ...
            sx1 = self.sx1
        return (
         sx1, self.sy1, sx2, self.sy1 + self.bottom - 1)
    ...
    def leftFrame(self):
        ...
        if self.overlap[1] > self.overlap[2]:
            ...
            sy1 = self.sy1
        return (
         self.sx1, sy1, self.sx1 + self.left - 1, sy2)
    ...
    def rightFrame(self):
        ...
        if self.overlap[1] > self.overlap[3]:
            ...
            sy1 = self.sy1
        return (
         self.sx2 - self.right + 1, sy1, self.sx2, sy2)


AXISES, AXIS_DIVISION, FULL_GRID, LABELS, CAPTION, TITLE, SHUFFLE_DRAW, EVADE_DRAW, LOG_X, LOG_Y, KEEP_ASPECT, AUTO_ADJUST, AUTO_PEN = [2 ** i for i in range(13)]
...
class Cartesian(HardFramedScreen):
    ...
    def __init__(self, gfx, x1, y1, x2, y2, title='Graph', xaxis='X', yaxis='Y', styleFlags=DEFAULT_STYLE, axisPen=Gfx.BLACK_PEN, labelPen=Gfx.BLACK_PEN, titlePen=Gfx.BLACK_PEN, captionPen=Gfx.BLACK_PEN, backgroundPen=Gfx.WHITE_PEN, region=REGION_FULLSCREEN):
        ...
        HardFramedScreen.__init__(self, gfx, self.x1, self.y1, self.x2, self.y2, region, top, bottom, left, right, styleFlags & KEEP_ASPECT)
        self.redraw()
    ...
    def _calcFrame(self):
        ...
        """
        top, bottom, left, right = self._xcalcFrame()
        if top != self.top or bottom != self.bottom or left != self.left or right != self.right:
    ...
    def _scaleX(self, x):
        ...
        if LOG_X & self.styleFlags:
            return self.tr.X(math.log10(x) * (self.x2 - self.x1) / self.stretchX + self.x1)
        else:
            return self.tr.X(x)
    ...
    def _scaleY(self, y):
        ...
        if LOG_Y & self.styleFlags:
            return self.tr.Y(math.log10(y) * (self.y2 - self.y1) / self.stretchY + self.y1)
        else:
            return self.tr.Y(y)
    ...
    def _clearLabels(self):
        ...
        self.gfx.applyPen(self.backgroundPen, Gfx.MASK_FILL)
        self.gfx.fillRect(sx1 - self.labelSize, sy1 + 1, self.labelSize, sy2 - sy1)
        self.gfx.fillRect(sx1 + 1, sy1 - self.labelSize, sx2 - sx1, self.labelSize)
            ...
            for name in nameList:
                if lengthDict[name] > i:
                    ...
                    if self._inside(x1, y1) and self._inside(x2, y2):
                        self.gfx.x_drawLine(self._scaleX(x1), self._scaleY(y1), self._scaleX(x2), self._scaleY(y2))
    ...
    def _assertRangeParameters(self):
        if not self.x2 > self.x1:
            raise AssertionError('x2 must be greater than x1!')
        elif not self.y2 > self.y1:
            raise AssertionError('y2 must be greater than y1!')
        else:
            if not not LOG_X & self.styleFlags:
                if not self.x1 >= 1:
                    raise AssertionError('x1 must be greater or equal 1 when using a logarithmic scale!')
            if not not LOG_Y & self.styleFlags:
                assert self.y1 >= 1, 'y1 must be greater or equal 1 when using a logarithmic scale!'
    ...
    def setStyle(self, styleFlags=None, axisPen=None, labelPen=None, titlePen=None, captionPen=None, backgroundPen=None, redraw=True):
        ...
        if styleFlags != None:
            if MASK_LAYOUT & styleFlags != MASK_LAYOUT & self.styleFlags:
                ...
                if MASK_GRAPHSTYLE & styleFlags != MASK_GRAPHSTYLE & self.styleFlags:
                    redrawFlags |= RD_GRAPH
                else:
                    if (LOG_X | LOG_Y) & styleFlags != (LOG_X | LOG_Y) & self.styleFlags:
        ...
        if oldStyleFlags & AUTO_ADJUST:
            if not self.styleFlags & AUTO_ADJUST:
                x1, y1, x2, y2 = (
                 self.x1, self.y1, self.x2, self.y2)
                ...
                self.adjustRange(x1, y1, x2, y2)
                redrawFlags &= ~RD_GRAPH
        if oldStyleFlags & KEEP_ASPECT:
            if not self.styleFlags & KEEP_ASPECT:
                redrawFlags |= RD_GRAPH
        if updateGeometry:
            self._calcFrame()
        if redraw:
    ...
    def exchangePen(self, name, pen, redraw=True):
        ...
        assert name in self.pens, "Cannot exchange pen '" + name + "' because it did not exist."
        self.pens[name] = pen
        if redraw:
            self.redrawCaption()
            if self.values[name]:
    ...
    def addValue(self, name, x, y):
        ...
        """
        if name not in self.pens:
            if AUTO_PEN & self.styleFlags:
                self.addPen(name)
            else:
                vl = self.values[name]
                vl.append((x, y))
            ...
            if len(vl) > 1 and self._inside(vl[(-2)][0], vl[(-2)][1]):
                x1, y1 = vl[(-2)]
                self.gfx.x_drawLine(self._scaleX(x1), self._scaleY(y1), self._scaleX(x), self._scaleY(y))
            else:
                self.gfx.drawPoint(self._scaleX(x), self._scaleY(y))
...
class OptimizingFilter(object):
    ...
    def graphChanged(self):
        ...
        """
        if OPT_X & self.flags:
            self.steps = self.graph.xaxisSteps(self.graph.x1, self.graph.x2)
        else:
            self.steps = self.graph.yaxisSteps(self.graph.y1, self.graph.y2)
        self.index = {}
