...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.6/site-packages/PyPlotter/Gfx.py
# Compiled at: 2017-09-02 13:54:44
...
class Pen(object):
    __doc__ = 'Record containing the all drawing attributes, namely: color,\n    line width, line style, fill pattern, font type, size and weight.\n    '
    __slots__ = ('color', 'lineWidth', 'linePattern', 'fillPattern', 'fontType', 'fontSize',
                 'fontWeight')

    def __repr__(self):
        return 'Pen(' + repr(self.color) + ',' + repr(self.lineWidth) + ', ' + repr(self.linePattern) + ', ' + repr(self.fillPattern) + ', ' + repr(self.fontType) + ', ' + repr(self.fontSize) + ', ' + repr(self.fontWeight) + ', )'

    def __init__(self, color=(0.0, 0.0, 0.0), lineWidth=THIN, linePattern=CONTINUOUS, fillPattern=SOLID, ftype=SANS, fsize=NORMAL, fweight=PLAIN):
        self.color = color
...
COLOR, LINE_WIDTH, LINE_PATTERN, FILL_PATTERN, FONT = [2 ** i for i in range(5)]
...
class Driver(object):
    ...
    def applyPen(self, pen, mask=MASK_ALL):
        ...
        if COLOR & mask:
            if self.color != pen.color:
                self.setColor(pen.color)
            else:
                if LINE_WIDTH & mask:
                    if self.lineWidth != pen.lineWidth:
                        self.setLineWidth(pen.lineWidth)
                if LINE_PATTERN & mask:
                    if self.linePattern != pen.linePattern:
                        self.setLinePattern(pen.linePattern)
                if FILL_PATTERN & mask:
                    if self.fillPattern != pen.fillPattern:
                        self.setFillPattern(pen.fillPattern)
        else:
            if FONT & mask:
                if self.fontType != pen.fontType or self.fontSize != pen.fontSize or self.fontWeight != pen.fontWeight:
                    self.setFont(pen.fontType, pen.fontSize, pen.fontWeight)

    def savePen(self):
        """Returns the gfx attributes as a pen."""
        return Pen(self.color, self.lineWidth, self.linePattern, self.fillPattern, self.fontType, self.fontSize, self.fontWeight)
    ...
    def clearRegion(self, region, rgbTuple=(1.0, 1.0, 1.0)):
        ...
        self.setFillPattern(SOLID)
        self.fillRect(region[0], region[1], region[2] - region[0] + 1, region[3] - region[1] + 1)
    ...
    def _circle(self, x, y, r, func):
        ...
        'drawPoly' or 'fillPoly'."""
        d = min(self.getSize())
        N = max(r * 100 // d, 8)
        t = 2 * pi / N
        points = [(int(x + r * sin(i * t) + 0.5), int(y + r * cos(i * t) + 0.5)) for i in range(N)]
    ...
    def fillCircle(self, x, y, r):
        ...
        self._circle(x, y, r, self.fillPoly)
    ...
    def refresh(self):
        """Refreshes the graphics output."""
        pass

    def x_dashedLine(self, x1, y1, x2, y2):
    ...
    def x_dottedLine(self, x1, y1, x2, y2):
    ...
    def x_drawLine(self, x1, y1, x2, y2):
        """Draws a line and ensures that it is really dashed or dotted
        if the line pattern requires it.
        """
        if self.linePattern == DASHED:
            self.x_dashedLine(x1, y1, x2, y2)
        else:
            if self.linePattern == DOTTED:
                self.x_dottedLine(x1, y1, x2, y2)
            else:
                self.drawLine(x1, y1, x2, y2)

    def x_drawPoly(self, array):
        ...
        if self.linePattern == CONTINUOUS:
            ...
            if self.linePattern == DASHED:
                ...
                for n in range(1, len(array)):
                    f(array[(n - 1)][0], array[(n - 1)][1], array[n][0], array[n][1])
...
class nilDriver(Driver):
    ...
    def getTextSize(self, text):
        if self.fontSize == SMALL:
            ...
            if self.fontSize == NORMAL:
                return (
                 len(text) * 12, 24)
            if self.fontSize == LARGE:
                return (
                 len(text) * 16, 32)
        raise ValueError('Illegal font size %i' % self.fontSize)
...
class Window(Driver):
    __doc__ = 'Opens a window that can be painted into. '
