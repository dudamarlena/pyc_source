...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.6/site-packages/PyPlotter/wxGfx.py
...
driverName = 'wxGfx'
if not wx.VERSION[0] > 2:
    if not (wx.VERSION[0] == 2 and wx.VERSION[1] >= 4):
        raise AssertionError('wxGfx.py requires wxPython Version 2.4 or higher!')
if wx.VERSION[0] == 2:
    if wx.VERSION[1] >= 6:
        _canRotateText = True
else:
...
class Driver(Gfx.Driver):
    __doc__ = 'A graphics driver for  wxWidgets.\n    For an explanation of the inherited methods see Gfx.py.\n    '

    def __init__(self, dc):
        """Initialize canvas on the device context dc."""
        Gfx.Driver.__init__(self)
        self.font = wx.Font(12, (wx.MODERN), (wx.NORMAL), (wx.NORMAL), encoding=(wx.FONTENCODING_ISO8859_1))
    ...
    def changeDC(self, dc):
        ...
        if dc:
            ...
            dc.SetTextForeground(wx.Colour(int(round(self.color[0] * 255)), int(round(self.color[1] * 255)), int(round(self.color[2] * 255))))
        return oldDC
    ...
    def setColor(self, rgbTuple):
        self.color = rgbTuple
        wxCol = wx.Colour(int(round(rgbTuple[0] * 255)), int(round(rgbTuple[1] * 255)), int(round(rgbTuple[2] * 255)))
        ...
        if ftype == Gfx.SANS:
            ...
            if ftype == Gfx.SERIF:
                ...
                if 'i' in weight:
                    ...
                    fw = wx.BOLD
        self.font = wx.Font(fs, ff, fst, fw, encoding=(wx.FONTENCODING_ISO8859_1))
    ...
    def clear(self, rgbTuple=(1.0, 1.0, 1.0)):
        if isinstance(self.dc, wx.PostScriptDC):
            ...
            self.dc.SetBackground(wx.Brush(wx.Colour(int(round(rgbTuple[0] * 255)), int(round(rgbTuple[1] * 255)), int(round(rgbTuple[2] * 255)))))
            self.dc.Clear()
    ...
    def writeStr(self, x, y, str, rotationAngle=0.0):
        ...
        if rotationAngle == 0.0:
            ...
            dc.DrawText(str, 0, 0)
            if 'ConvertToImage' in _buffer.__dict__:
