...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pdfs/BaseWork.py
...
class Work(metaclass=TypeMapMeta):
    ...
    def toDict(self):
        return dict(meta=(self.meta), citeKey=(self.key()), tags=(self.tags), files=(self.files),
          fileLabels=(self.fileLabels),
    ...
    def _dict_month(self):
        ...
        if len(date) > 1:
            v['month'] = [
             '', 'jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec'][date[1]]
        return v
