...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pdfs/TermOutput.py
    ...
    for frag in splt:
        if frag[0] == '\x1b':
            ...
            if frag[0] == ' ':
                breakIdx = len(words)
        words.append(frag)
        lineSize = sum(len(w) for w in words[:-1] if w if w[0] != '\x1b')
        if frag.strip():
            lineSize += len(frag)
        if lineSize > width:
...
class TermOutput(metaclass=Singleton):
    ...
    def setup(self, *, level='info', term=None):
        ...
        if l == 'debug':
            ...
            if self.isTermE:
                self.icos = [
                 stylize('*', fg('white') + bg('red') + attr('bold')),
