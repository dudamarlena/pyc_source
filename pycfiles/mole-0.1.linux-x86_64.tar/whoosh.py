...
# [GCC 8.4.0]
# Embedded file name: /home/ajdiaz/env/mole/lib/python2.7/site-packages/mole/index/whoosh.py
# Compiled at: 2012-10-17 13:00:19
...
from whoosh.qparser.plugins import GtLtPlugin, FieldsPlugin, RangePlugin, WildcardPlugin
from mole.index import Index, DEFAULT_INDEX_PATH
SCHEMA = Schema(source=TEXT(stored=True), name=TEXT(stored=True), raw=TEXT(stored=True), time=NUMERIC(stored=True), hash=ID(stored=True))
    ...
    def __call__(self, pipeline):
        ...
        try:
            ...
            for event in pipeline:
                self.count += 1
                self._writer.add_document(source=unicode(event['source']), name=unicode(event['index']), raw=unicode(event['_raw']), time=int(event.time), hash=unicode(event.hash))
