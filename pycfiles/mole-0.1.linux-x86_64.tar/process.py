...
# [GCC 8.4.0]
# Embedded file name: /home/ajdiaz/env/mole/lib/python2.7/site-packages/mole/helper/process.py
# Compiled at: 2013-01-23 06:30:44
import time
...
class ProcessPool(object):
    ...
    def apply_async(self, func, name=None, args=tuple(), kwargs={}, callback=None):
        ...
        with self.__pending_lock:
            self.__pending.append({'target': func, 
               'name': name, 
               'args': args, 
               'kwargs': kwargs, 
               'callback': callback})
            ...
            with self.__pending_lock:
                with self.__running_lock:
                    while self.has_pending_processes and not self.is_full:
                        ...
                        q = Queue()
                        p = Process(target=_wrapper(func=next['target'], queue=q), args=next['args'], kwargs=next['kwargs'])
                        if next['name'] is not None:
                            p.name = next['name']
                        self.__running.append({'process': p, 
                           'queue': q, 
