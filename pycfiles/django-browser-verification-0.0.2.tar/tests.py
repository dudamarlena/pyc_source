...
# [GCC 8.4.0]
# Embedded file name: /var/www/paparazziaccessories-com/venv-paparazzi/src/django-browser-verification/browser_verification/tests.py
# Compiled at: 2016-06-22 17:37:33
...
class BrowserVerificationTest(SimpleTestCase):
    ...
    def testBrowserOutdated(self):
        request = self.factory.get('/test/', HTTP_USER_AGENT='Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/25.0.2228.0 Safari/537.36')
        response = TestView.as_view()(request)
        self.assertEqual(response.browser_unsupported, True)
        self.assertEqual(response.browser_unknown, False)

    def testBrowserUnknown(self):
        request = self.factory.get('/test/', HTTP_USER_AGENT='Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Cheeto/41.0.2228.0 Safari/537.36')
        response = TestView.as_view()(request)
        self.assertEqual(response.browser_unsupported, True)

    def testBrowserLockout(self):
        request = self.factory.get('/test/', HTTP_USER_AGENT='Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36')
