...
# Size of source mod 2**32: 14336 bytes
...
class ReflectionMixin(object):
    __doc__ = 'The database reflection base class for `Redshift`.'

    @memoized_property
    def engine(self):
        """A sqlalchemy.engine which wraps `connection`.
        """
        return sqlalchemy.create_engine('redshift+psycopg2://', poolclass=(sqlalchemy.pool.StaticPool),
    ...
    def deep_copy(self, table, schema=None, copy_privileges=True, use_cache=True, cascade=False, distinct=False, analyze_compression=False, analyze=True, deduplicate_partition_by=None, deduplicate_order_by=None, execute=False, **kwargs):
        ...
        batch = ';\n'.join(statements).format(table_name=table_name,
    ...
    def _privilege_statements(self, relation, use_cache):
        ...
        statements = [
         'ALTER {type} {relation_name} OWNER TO {owner}'.format(type=(priv_info.type.upper()),
