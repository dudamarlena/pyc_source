...
# Size of source mod 2**32: 16582 bytes
"""
...
class PostgresMixin(S3Mixin):
    __doc__ = 'The Postgres interaction base class for `Redshift`.'
    ...
    def aws_credentials(self):
        if self.aws_account_id and self.aws_role_name:
            template = 'aws_iam_role=arn:aws:iam::{aws_account_id}:role/{role_name}'
            ...
            secret_key_id = 'aws_secret_access_key={}'.format(self.aws_secret_access_key)
            template = '{key_id}{secret_key_id}'
            if self.security_token:
                template += ';token={security_token}'.format(security_token=(self.security_token))
    ...
    def copy_table_to_s3(self, bucket_name, key_prefix, pg_table_name=None, pg_select_statement=None, temp_file_dir=None, cleanup_s3=True, line_bytes=104857600, canned_acl=None):
        ...
        if not key_prefix.endswith('/'):
            final_key_prefix += '/'
        elif pg_select_statement is None:
            ...
            ValueError('Exactly one of pg_table_name or pg_select_statement must be specified.')
        tmpdir = tempfile.mkdtemp(dir=temp_file_dir)
        copy_statement = 'COPY (SELECT row_to_json(x) FROM ({pg_table_or_select}) AS x) TO PROGRAM $$split - {tmpdir}/chunk_ --line-bytes={line_bytes} --filter=\'sed "s/\\\\\\\\\\\\\\\\/\\\\\\\\/g" | gzip > $FILE.json.gz\'$$'.format(pg_table_or_select=pg_table_or_select,
          tmpdir=tmpdir,
        ...
        try:
            ...
            print('Finished extracting data from Postgres. Waiting on uploads...')
    ...
    def copy_table_to_redshift(self, redshift_table_name, bucket_name, key_prefix, pg_table_name=None, pg_select_statement=None, temp_file_dir=None, cleanup_s3=True, delete_statement=None, manifest_max_keys=None, line_bytes=104857600, canned_acl=None):
        ...
        canned_acl: str
            A canned ACL to apply to objects uploaded to S3
        """
        backfill_timestamp = datetime.datetime.utcnow().strftime('%Y-%m-%d_%H%M%S')
        ...
        manifest_entries = [{'url':'s3://' + bucket.name + s3_path,  'mandatory':True} for s3_path in s3_keys]
        start_idx = 0
        ...
        while start_idx < num_entries:
            ...
            self.write_string_to_s3((json.dumps(manifest)), bucket, manifest_key_path,
              canned_acl=canned_acl)
            complete_manifest_path = ''.join(['s3://', bucket.name,
             manifest_key_path])
            statements = ''
            if delete_statement:
                if end_idx == num_entries:
    ...
    def run(self):
        ...
        *files_still_being_created* is set to False by the main thread.
        ...
        while True:
            ...
            if self._file_creation_complete.is_set():
                if not files:
                    return
            if files:
                if not self._file_creation_complete.is_set():
                    files = files[:-1]
