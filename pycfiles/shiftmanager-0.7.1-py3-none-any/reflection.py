...
# [GCC 8.4.0]
# Embedded file name: /Users/mikenawood/Code/shiftmanager/build/lib/shiftmanager/mixins/reflection.py
...
class ReflectionMixin(object):
    ...
    def engine(self):
        ...
        """
        return sqlalchemy.create_engine('redshift+psycopg2://', poolclass=sqlalchemy.pool.StaticPool, creator=lambda : self.connection)
    ...
    def get_table_names(self, schema=None, **kwargs):
        ...
        """
        return self.engine.dialect.get_table_names(self.engine, schema, **kwargs)
    ...
    def table_definition(self, table, schema=None, copy_privileges=True, use_cache=True, analyze_compression=False):
        """
    ...
    def view_definition(self, view, schema=None, copy_privileges=True, use_cache=True, execute=False, **kwargs):
        ...
        definition = self.engine.dialect.get_view_definition(self.engine, view.name, view.schema, **kwargs)
        create_statement = str(CreateView(view, definition).compile(self.engine))
    ...
    def deep_copy(self, table, schema=None, copy_privileges=True, use_cache=True, cascade=False, distinct=False, analyze_compression=False, analyze=True, deduplicate_partition_by=None, deduplicate_order_by=None, execute=False, **kwargs):
        ...
        table_definition = '\n' + self.table_definition(table, None, copy_privileges, use_cache, analyze_compression)
        insert_statement = '\nINSERT INTO {table_name} \nSELECT '
        identity_cols = self._get_identity_columns(table.name) or {}
        col_str = (',\n\t').join('"%s"' % col.name for col in table.columns if col.name not in identity_cols)
        if distinct:
        ...
        statements = ['LOCK TABLE {table_name}',
        ...
        batch = (';\n').join(statements).format(table_name=table_name, outgoing_name=outgoing_name, outgoing_name_simple=outgoing_name_simple, deduplicate_partition_by=deduplicate_partition_by, deduplicate_order_by=deduplicate_order_by) + ';'
        return self.mogrify(batch, None, execute)
    ...
    def _privilege_statements(self, relation, use_cache):
        ...
        statements = [
         ('ALTER {type} {relation_name} OWNER TO {owner}').format(type=priv_info.type.upper(), relation_name=relation_name, owner=priv_info.owner_name)]
        statements += grants_from_privileges(priv_info.privileges, relation.key)
        return statements
