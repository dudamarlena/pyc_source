...
class S3Mixin(object):
    __doc__ = 'The S3 interaction base class for `Redshift`.'
    ...
    def get_s3_connection(self, ordinary_calling_fmt=False):
        ...
        args = []
        kwargs = {}
        security_token = os.environ.get('AWS_SECURITY_TOKEN') or os.environ.get('AWS_SESSION_TOKEN') or self.security_token
        if security_token:
    ...
    def get_bucket(self, bucket_name):
        ...
        try:
            ...
            dot_msg = "doesn't match either of '*.s3.amazonaws.com', 's3.amazonaws.com'"
            if dot_msg in str(e):
        ...
        try:
            with self.chunked_json_slices(data, slices, local_path, clean_up_local) as (stamp, file_paths):
                ...
                for path in file_paths:
                    ...
                    manifest_entry = {'url':'s3://{}/{}'.format(bukkit.name, data_keypath), 
                     'mandatory':True}
                ...
                jpaths_complete_path = single_dict_write('.jsonpaths', jsonpaths)
            creds = 'aws_access_key_id={};aws_secret_access_key={}'.format(self.aws_access_key_id, self.aws_secret_access_key)
            if self.security_token:
                creds += ';token={}'.format(self.security_token)
            statement = queries.copy_from_s3.format(table=table,
              manifest_key=mfest_complete_path,
              creds=creds,
              jpaths_key=jpaths_complete_path)
    ...
    @check_s3_connection
    def unload_table_to_s3(self, bucket, keypath, table, col_str='*', where=None, to_json=True, options=None):
        """
        ...
        if self.security_token:
            ...
            if to_json:
                ...
                cols = col_str
        select = 'SELECT {col_str} FROM {table} '.format(col_str=cols,
          table=table)
    ...
    def _is_numeric(self, col_type):
        no_quote_types = {
         'bigint', 'double precision', 'integer', 'numeric',
         'real', 'smallint'}
