...
# [GCC 8.4.0]
# Embedded file name: /Users/mikenawood/Code/shiftmanager/build/lib/shiftmanager/redshift.py
...
import os, psycopg2
from shiftmanager.mixins import AdminMixin, ReflectionMixin, PostgresMixin, S3Mixin
...
class Redshift(AdminMixin, ReflectionMixin, PostgresMixin, S3Mixin):
    ...
    def connection(self):
        ...
        """
        print(b'Connecting to %s...' % self.host)
        return psycopg2.connect(user=self.user, host=self.host, port=self.port, database=self.database, password=self.password, **self.pgkwargs)

    def __init__(self, database=None, user=None, password=None, host=None, port=5439, aws_access_key_id=None, aws_secret_access_key=None, security_token=None, **kwargs):
        self.set_aws_credentials(aws_access_key_id, aws_secret_access_key, security_token)
        self.user = user or os.environ.get(b'PGUSER')
        self.host = host or os.environ.get(b'PGHOST')
        self.port = port or os.environ.get(b'PGPORT')
        self.database = database or os.environ.get(b'PGDATABASE')
        self.password = password or os.environ.get(b'PGPASSWORD')
    ...
    def mogrify(self, batch, parameters=None, execute=False):
        ...
        return mogrified.decode(b'utf-8')
    ...
    def table_exists(self, table_name):
        ...
        with self.connection as (conn):
            with conn.cursor() as (cur):
                cur.execute((b"select count (distinct tablename)\n                               from pg_table_def\n                               where tablename = '{}';").format(table_name))
