...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/evla_mcast/mcast_clients.py
# Compiled at: 2018-02-26 13:33:04
...
import os, struct, logging, asyncore, socket, contextlib
...
class McastClient(asyncore.dispatcher):
    __doc__ = 'Generic class to receive the multicast XML docs.'
...
class ObsClient(McastClient):
    ...
    def parse(self):
        ...
        logger.info('Read obs configId={0}, seq={1}'.format(obs.attrib['configId'], obs.attrib['seq']))
        ...
        if self.use_configUrl:
            ...
            try:
                ...
                logger.warn('Error retrieving VCI from {0}. {1}'.format(url, e))
...
class AntClient(McastClient):
    ...
    def parse(self):
        ...
        logger.info('Read ant datasetId={0}'.format(result.attrib['datasetId']))
        if self.controller is not None:
...
if __name__ == '__main__':
    logger.basicConfig(format='%(asctime)-15s %(levelname)8s %(message)s', level=(logger.DEBUG))
