...
# Compiled at: 2020-03-17 09:03:28
# Size of source mod 2**32: 30537 bytes
...
class Sketch(resource.BaseResource):
    ...
    def store_aggregation(self, name, description, aggregator_name, aggregator_parameters, chart_type=''):
        ...
        """
        resource_url = '{0:s}/sketches/{1:d}/aggregation/'.format(self.api.api_root, self.id)
        form_data = {'name': name, 
        ...
        response = self.api.session.post(resource_url, json=form_data)
        if response.status_code not in definitions.HTTP_STATUS_CODE_20X:
            error.error_message(response, message='Error storing the aggregation', error=RuntimeError)
        response_dict = response.json()
