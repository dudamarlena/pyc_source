...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.4-x86_64/egg/hookbox/server.py
# Compiled at: 2012-07-04 04:55:04
import collections, errno, logging, os, socket, sys, urllib, urlparse, eventlet
from paste import urlmap, urlparser
eventlet.monkey_patch(all=False, socket=True, select=True)
from restkit import Resource
import eventlet.wsgi, eventlet.websocket
from csp_eventlet import Listener
import rtjp_eventlet
from errors import ExpectedException
import channel, protocol
...
class HookboxServer(object):
    ...
    def run(self):
        try:
            ...
            eventlet.spawn(eventlet.backdoor.backdoor_server, eventlet.listen(('localhost',
                                                                               3000)), locals={'server': self})
        except Exception:
        ...
        if not self._bound_api_socket:
            ...
            if api_port is None:
                api_port = main_port
            if (
             main_host, main_port) != (api_host, api_port):
    ...
    def serialize(self):
        return {'channels': self.channels.keys(), 
        ...
        if chan.server_presenceful:
            ...
            for user in chan.server_user_presence:
                ...
                if user not in self.user_channel_presence:
                    self.user_channel_presence[user] = [
                     chan]
                else:
