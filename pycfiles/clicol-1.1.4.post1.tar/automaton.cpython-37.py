...
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/onedesk/automaton/automaton.py
# Compiled at: 2019-06-06 17:58:19
# Size of source mod 2**32: 6880 bytes
import logging
logger = logging.getLogger('main')

def search_automata(session, instance, client, category, name):
    params = {'page':0, 
     'size':50,  'sortField':'name',  'sortDirection':'ASC',  'clientId':client['id'],  'taskId':None, 
     'categoryId':None,  'name':name,  'deleted':False}
...
def save_automaton(session, instance, automation):
    headers = {'Content-Type':'application/json;charset=UTF-8', 
     'Accept':'application/json, text/plain, */*', 
     'Accept-Encoding':'gzip, deflate, br'}
    rpc_action = {'action':'automataController',  'method':'saveAutomaton',  'type':'rpc',  'tid':0,  'data':automation.json()}
...
def delete_automata(session, instance, automata):
    logger.debug('!!Deleting automata {}'.format(automata['name']))
...
def export_automata(session, instance, automaton):
    logger.debug('>Exporting automata {} | {}'.format(automaton['id'], automaton['name']))
...
def import_automaton(session, instance, category, automaton_name, automata_dto):
    headers = {'Content-Type':'application/json;charset=UTF-8', 
     'Accept':'application/json, text/plain, */*', 
     'Accept-Encoding':'gzip, deflate, br'}
    import_command = dict(clientId=(category['clientId']), categoryId=(category['id']), exportedAutomatonDto=automata_dto,
      automatonName=automaton_name,
      relinkExisting=True,
      importCategoryStructure=False,
      tags=[],
      linkedImportCommands=[],
      automatonConnectionGroups=[])
    response = session.post(('{}/api/automaton-import-export/import'.format(instance)), headers=headers, json=import_command)
...
def submit_automaton_for_approval(session, instance, version):
    ...
    response = session.put(('{}/api/automaton-approval/versions/{}/actions/submit'.format(instance, version['versionId'])),
      headers=headers,
      params=params)
    logger.debug('Response: %s', response.status_code)
    if response.ok:
...
def approve_automaton(session, instance, version):
    ...
    response = session.put(('{}/api/automaton-approval/versions/{}/actions/approve'.format(instance, version['versionId'])),
      headers=headers,
      params=params)
    logger.debug('Response: %s', response.status_code)
    return response.ok