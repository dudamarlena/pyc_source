...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ai_tools/roc_matrix.py
# Compiled at: 2018-08-24 06:12:34
import os, cv2, xml.etree.ElementTree as ET, shutil, pdb
from zprint import *
...
def hasOverlap(groud_bbox, pre_bbox):
    ...
    inters = iw * ih
    union = (pre_bbox[2] - pre_bbox[0] + 1.0) * (pre_bbox[3] - pre_bbox[1] + 1.0) + (groud_bbox[2] - groud_bbox[0] + 1.0) * (groud_bbox[3] - groud_bbox[1] + 1.0) - inters
...
def get_rect(dir_path, file_name, file_type):
    if file_type == 1:
        ...
        if os.path.isfile(file_path):
            rect_info = parse_rec_xml(file_path)
        else:
            rect_info = []
    else:
        file_path = os.path.join(dir_path, file_name + '.txt')
        if os.path.isfile(file_path):
            rect_info = parse_rec_txt(file_path)
        else:
            rect_info = []
    return rect_info
