...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/segmenttreenode/SegmentTreeNode.py
# Compiled at: 2015-01-03 17:47:40


class SegmentTreeNode(object):
    __slots__ = [
     'start', 'end', 'left', 'right', 'segment_sum']
    ...
    def _query_segment_sum(self, query_start, query_end):
        if query_start <= self.start and self.end <= query_end:
            return self.segment_sum
        if query_end < self.start or self.end < query_start:
            return 0
        return self.left.query_segment_sum(query_start, query_end) + self.right.query_segment_sum(query_start, query_end)
