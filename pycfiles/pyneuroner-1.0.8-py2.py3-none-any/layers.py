...
# Embedded file name: build/bdist.linux-x86_64/egg/pyneurgen/layers.py
...
class Layer(object):
    ...
    def __init__(self, layer_no, layer_type):
        ...
        if layer_type in [LAYER_TYPE_INPUT, LAYER_TYPE_HIDDEN,
         LAYER_TYPE_OUTPUT]:
            ...
            raise ValueError("Layer type must be 'input', 'hidden', or 'output'")
    ...
    def unconnected_nodes(self):
        ...
        """
        return [ node.node_no for node in self.nodes if not node.input_connections
               ]
    ...
    def load_inputs(self, inputs):
        ...
        for i in range(len(inputs)):
            ...
            if node.node_type != LAYER_TYPE_INPUT:
                raise ValueError('Attempting to load an input value into a non-input node')
            if isinstance(inputs[i], float):
                node.set_value(inputs[i])
            else:
                raise ValueError('Invalid value, most be float: %s' % inputs[i])
    ...
    def load_targets(self, targets):
        ...
        if self.layer_type != LAYER_TYPE_OUTPUT:
            raise ValueError('target values are only loaded to the output layer')
        if len(targets) != len(self.nodes):
            raise ValueError('Number of targets: %s, Number of nodes: %s' % (
        ...
        for i in range(self.total_nodes()):
            ...
            if isinstance(targets[i], float):
                ...
                raise ValueError('Invalid value, most be float: %s' % targets[i])
            node = self.nodes[i]
