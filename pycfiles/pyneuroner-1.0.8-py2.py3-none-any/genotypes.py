...
logging.basicConfig(format='%(asctime)s %(message)s', filename=DEFAULT_LOG_FILE, level=DEFAULT_LOG_LEVEL)
...
class Genotype(object):
    ...
    def _place_material(program, item, start_pos, end_pos):
        ...
        elif end_pos == len(program) - 1:
            program = program[:start_pos] + item
        else:
            program = program[:start_pos] + item + program[end_pos:]
        return program
    ...
    def _map_gene(self):
        ...
        """
        self.local_bnf['<fitness>'] = [
         str(self._fitness_fail)]
        try:
            logging.debug('==================================================')
    ...
    def mutate(self, mutation_rate, mutation_type):
        ...
        if mutation_type == MUT_TYPE_S:
            ...
            raise ValueError("The mutation type must be either '%s' or '%s'", MUT_TYPE_S, MUT_TYPE_M)
