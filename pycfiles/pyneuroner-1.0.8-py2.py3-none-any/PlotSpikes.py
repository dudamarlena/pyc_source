...
# Embedded file name: build/bdist.linux-x86_64/egg/pyneuroml/plot/PlotSpikes.py
# Compiled at: 2019-12-12 12:33:22
from pyneuroml import pynml
import argparse, matplotlib.pyplot as plt
from collections import OrderedDict
import numpy as np, re, sys, os
DEFAULTS = {'format': 'id_t', 'rates': False, 
...
def process_args():
    ...
    parser.add_argument('spiketimeFiles', type=str, metavar='<spiketime file>', help='List of text file containing spike times', nargs='+')
    parser.add_argument('-format', type=str, metavar='<format>', default=DEFAULTS['format'], help='How the spiketimes are represented on each line of file:\n' + 'id_t: id of cell, space(s)/tab(s), time of spike (default);\n' + 't_id: time of spike, space(s)/tab(s), id of cell;\n' + 'sonata: SONATA format HDF5 file containing spike times')
    parser.add_argument('-rates', action='store_true', default=DEFAULTS['rates'], help='Show a plot of rates')
    parser.add_argument('-showPlotsAlready', action='store_true', default=DEFAULTS['show_plots_already'], help='Show plots once generated')
    parser.add_argument('-saveSpikePlotTo', type=str, metavar='<spiketime plot filename>', default=DEFAULTS['save_spike_plot_to'], help='Name of file in which to save spiketime plot')
    parser.add_argument('-rateWindow', type=int, metavar='<rate window>', default=DEFAULTS['rate_window'], help='Window for rate calculation in ms')
    parser.add_argument('-rateBins', type=int, metavar='<rate bins>', default=DEFAULTS['rate_bins'], help='Number of bins for rate histogram')
...
def run(a=None, **kwargs):
    ...
    pynml.generate_plot(xs, ys, 'Spike times from: %s' % a.spiketime_files, labels=labels, linestyles=linestyles, markers=markers, xaxis='Time (s)', yaxis='Cell index', xlim=xlim, ylim=ylim, markersizes=markersizes, grid=False, show_plot_already=False, save_figure_to=a.save_spike_plot_to, legend_position='right')
    if a.rates:
        plt.figure()
        bins = a.rate_bins
        for name in times:
