...
# Embedded file name: build/bdist.linux-x86_64/egg/pyneurgen/neuralnet.py
# Compiled at: 2012-05-18 18:02:09
...
class NeuralNet(object):
    ...
    def __init__(self):
        ...
        self._epochs = 1
        self.layers = []
        self._data_range = {'learning': [None, None], 'validation': [
                        None, None], 
           'test': [
                  None, None]}
        self._allinputs = []
    ...
    def get_learn_data(self, random_testing=None):
        ...
        if random_testing:
            for i in self._get_randomized_position(start_position, end_position):
    ...
    def _get_data(self, start_position, end_position):
        ...
        if end_position > len(self._allinputs) - 1:
            raise ValueError('end_position %s is past end of ._allinputs, %s' % (
    ...
    def _get_randomized_position(start_position, end_position):
        ...
        for i in xrange(start_position, end_position):
            order = [ [random(), i] for i in xrange(start_position, end_position)
                    ]
    ...
    def _check_positions(self, start_position, end_position):
        ...
        if start_position is None:
            raise ValueError('Start position is not defined.')
        if end_position is None:
            raise ValueError('End position data is not defined.')
    ...
    def learn(self, epochs=None, show_epoch_results=True, random_testing=False, show_sample_interval=0):
        """
    ...
    def _parse_inputfile_node(config, node_id):
        ...
        if node_type == NODE_BIAS:
            ...
            node._incoming_weight = float(config.get(node_id, 'incoming_weight'))
    ...
    def load(self, filename):
        ...
        if flag == 'True':
            ...
            self.set_random_constraint(float(config.get('net', 'random_constraint')))
            ...
            for layer_no in range(total_layers):
                ...
                for node in layer.nodes:
                    ...
                    if isinstance(node, CopyNode):
                        ...
                        node.set_source_node(self._parse_inputfile_copy(source_str))
    ...
    def output_values(self):
        ...
        output += 'hidden_neurons = %s\n' % (', ').join([ str(layer.total_nodes()) for layer in self.layers[1:-1] ])
        output += 'output_neurons = %s\n' % self.output_layer.total_nodes(NODE_OUTPUT)
        if self.layers[1].total_nodes(NODE_HIDDEN) > 0:
            copy_levels = self.input_layer.total_nodes(NODE_COPY) / self.layers[1].total_nodes(NODE_HIDDEN)
