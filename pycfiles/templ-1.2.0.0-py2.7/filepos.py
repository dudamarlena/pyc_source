...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\templ\filepos.py
        ...
        if self.filename is not None:
            ...
            if self.line is not None:
                ...
                if self.offset is not None:
                    form += ' (col {offset})'
        return form.format(preposition=preposition, filename=self.filename, lineno=self.line, offset=self.offset)
...
class WrapFilepos(Filepos):
    ...
    def wrap(cls, filepos):
        if filepos is None:
            ...
            if not isinstance(filepos, Filepos):
