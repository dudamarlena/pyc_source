...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/turbolucene/__init__.py
# Compiled at: 2007-05-16 21:57:57
...
__all__ = ['start', 'add', 'update', 'remove', 'search', 'Document', 'Field', 'STORE', 'COMPRESS', 'TOKENIZED', 'UN_TOKENIZED']
...
import PyLucene
from PyLucene import PythonThread, IndexModifier, JavaError, Term, IndexSearcher, MultiFieldQueryParser, FSDirectory
...
def _load_language_data():
    ...
    languages_file_encoding = config.get('turbolucene.languages_file_encoding', 'utf-8')
    if languages_file:
        _log.info('Loading custom language data from "%s"' % languages_file)
    else:
        _log.info('Loading default language data')
        languages_file = resource_stream(__name__, 'languages.cfg')
    _language_data = ConfigObj(languages_file, encoding=languages_file_encoding, file_error=True, raise_errors=True)
    return
...
def _schedule_optimization():
    ...
    optimize_time = config.get('turbolucene.optimize_time', (0, 0))
    scheduler.add_weekday_task(_optimize, optimize_days, optimize_time)
    _log.info('Index optimization scheduled on %s at %s' % (unicode(optimize_days), unicode(optimize_time)))
...
def _get_index_path(language):
    ...
    """
    index_base_path = config.get('turbolucene.index_root', 'index')
...
def _read_stopwords(file_path, encoding):
    ...
    if isabs(file_path) or stopwords_base_path:
        ...
        _log.info('Reading custom stopwords file "%s"' % file_path)
        stopwords_file = codecs.open(file_path, 'r', encoding)
    else:
        _log.info('Reading default stopwords file "%s"' % file_path)
        stopwords_file = codecs.getreader(encoding)(resource_stream(__name__, join('stopwords', file_path)))
    stopwords = []
...
def _analyzer_factory(language):
    ...
    args = 'analyzer_class_args' in ldata and ldata['analyzer_class_args'] or []
    if not isinstance(args, list):
        args = [
         args]
    stopwords = []
    if 'stopwords' in ldata and ldata['stopwords']:
        stopwords = [
         ldata.stopwords]
    elif 'stopwords_file' in ldata and 'stopwords_file_encoding' in ldata:
        stopwords = [_read_stopwords(ldata['stopwords_file'], ldata['stopwords_file_encoding'])]
    args += stopwords
    return getattr(PyLucene, ldata['analyzer_class'])(*args)


def _stop():
    """Shutdown search engine threads."""
    global _indexer
    global _searcher_factory
    _searcher_factory.stop()
    _indexer('stop')
    _log.info('Search engine stopped.')
...
def start(make_document, results_formatter=None):
    ...
    _log.info('Search engine started.')
...
class _Indexer(PythonThread):
    ...
    def __init__(self, make_document):
        ...
        self._indexes = {}
        default_language = config.get('turbolucene.default_language', _DEFAULT_LANGUAGE)
        default_force_lock_release = config.get('server.environment', 'development').lower() == 'development' and True or False
        force_lock_release = config.get('turbolucene.force_lock_release', default_force_lock_release)
        ...
        for language in languages:
            ...
            try:
                self._indexes[language] = IndexModifier(index_path, analyzer, create_path)
            except JavaError, error:
                if not error.getJavaException().getClass().getName() == 'java.io.IOException' or not force_lock_release:
                    raise
                _log.warn('Error opening index "%s".  turbolucene.force_lock_release is True, trying to force lock release.' % index_path)
                FSDirectory.getDirectory(index_path, False).makeLock('write.lock').release()
                self._indexes[language] = IndexModifier(index_path, analyzer, create_path)
    ...
    def __call__(self, task, object_=None, language=None):
        ...
        """
        if not language:
            language = config.get('turbolucene.default_language', _DEFAULT_LANGUAGE)
    ...
    def _add(self, object_, language, document=None):
        ...
        _log.info('Adding object "%s" (id %s) to the %s index.' % (unicode(object_), document['id'], language))
    ...
    def _remove(self, object_, language, document=None):
        ...
        if not document:
            document = self._make_document(object_)
        _log.info('Removing object "%s" (id %s) from %s index.' % (unicode(object_), document['id'], language))
    ...
    def _optimize(self):
        ...
        """
        _log.info('Optimizing indexes.')
        for index in self._indexes.values():
            index.optimize()

        _log.info('Indexes optimized.')
...
class _Searcher(PythonThread):
    ...
    def run(self):
        ...
        default_operator = getattr(parser.Operator, config.get('turbolucene.default_operator', 'AND').upper())
...
class _SearcherFactory(PythonThread):
    ...
    def run(self):
        ...
        while True:
            ...
            if request == 'stop':
                break
            self._searcher_queue.put(_Searcher(*self._searcher_args, **self._searcher_kwargs))