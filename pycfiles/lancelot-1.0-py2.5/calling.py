...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/lancelot/calling.py
...
def _format_args(args, kwds):
    ...
    formatted_args.extend([ '%s=%s' % (kwd, repr(value)) for (kwd, value) in kwds.items()
                          ])
...
class MockCall(object):
    ...
    def _verify(self, *args, **kwds):
        """ Check that the collaboration is as specified """
        if self._mock_spec.comparable_args(self._specified_args) != args or self._mock_spec.comparable_kwds(self._specified_kwds) != kwds:
