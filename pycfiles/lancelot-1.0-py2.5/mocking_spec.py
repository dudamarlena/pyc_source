...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/lancelot/specs/mocking_spec.py
# Compiled at: 2009-02-24 05:44:25
...
from lancelot.comparators import ExceptionValue, FloatValue, Type, EqualsEquals, Nothing
...
class MockCallResultOfBehaviour(object):
    ...
    def should_verify_args_specification(self):
        ...
        spec = Spec(mock_call_result)
        msg = "should be collaborating with g.bar(keyword='named argument'), " + "not g.bar(keyword='wrong argument')"
        spec.__call__(keyword='wrong argument').should_raise(UnmetSpecification(msg))
        mock_call = MockSpec(name='h').bar(keyword='named argument')
        mock_call_result = mock_call.result_of('bar')
        spec = Spec(mock_call_result)
        msg = "should be collaborating with h.bar(keyword='named argument'), " + "not h.bar(bad_keyword='named argument')"
        spec.__call__(bad_keyword='named argument').should_raise(UnmetSpecification(msg))
        ...
        mock_call = MockSpec(name='j').bar(keyword='named argument').will_return('monty')
        mock_call_result = mock_call.result_of('bar')
        spec = Spec(mock_call_result)
        spec.__call__(keyword='named argument').should_be('monty')


grouping(MockCallResultOfBehaviour)
...
class StartCollaboratingBehaviour(object):
...
grouping(StartCollaboratingBehaviour)

class MockCallArgsComparatorBehaviour(object):
...
grouping(MockCallArgsComparatorBehaviour)

class MockResultBehaviour(object):
