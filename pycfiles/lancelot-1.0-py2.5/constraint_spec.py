...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/lancelot/specs/constraint_spec.py
# Compiled at: 2009-02-20 10:55:47
""" Specs for core library classes / behaviours """
from lancelot import MockSpec, Spec, grouping, verifiable, verify
from lancelot.constraints import Constraint, CollaborateWith, Not, Raise, EqualsEquals
from lancelot.verification import UnmetSpecification
from lancelot.specs.simple_fns import dont_raise_index_error, number_one, raise_index_error

class BaseConstraintBehaviour(object):
    ...
    def desc_should_use_comparator(self):
        ...
        spec.should_collaborate_with(comparator_description, and_result='should be subtitled')


grouping(BaseConstraintBehaviour)

class RaiseBehaviour(object):
...
class CollaborateWithBehaviour(object):
    ...
    def should_trap_incorrect_args(self):
        ...
        descriptions = [ collaboration.description() for collaboration in collaborations
                       ]
    ...
    def should_trap_incorrect_return(self):
        ...
        mock_spec = MockSpec()
        spec = Spec(CollaborateWith(mock_spec.foo().will_return('baz'), and_result='bar'))
    ...
    def correct_result_should_be_ok(self):
        ...
        the Hungarian gentleman in the examples for a clearer picture... """
        mock_spec = MockSpec()
        spec = Spec(CollaborateWith(mock_spec.foo().will_return('bar'), and_result='bar'))
        spec.verify(lambda : mock_spec.foo())
        spec.should_not_raise(UnmetSpecification)


grouping(CollaborateWithBehaviour)
