...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/lancelot/verification.py
# Compiled at: 2009-02-19 03:15:30
"""
...
class AllVerifiable(object):
    ...
    def include_grouping(self, grouping_class):
        ...
        functions = [ item for item in class_attrs if type(item) == types.FunctionType
                    ]
    ...
    def verify(self, fail_fast=False):
        ...
        outcome = {'total': self.total(), 'verified': verified, 
    ...
    def verify_fn(self, verifiable_fn):
        ...
        try:
            ...
            elif hasattr(verifiable_fn, 'im_func') and verifiable_fn.im_func in self._fn_groups:
                self._fn_groups[verifiable_fn.im_func]()
            else:
                verifiable_fn()
            self._listener.specification_met(verifiable_fn)
            return 1
        except UnmetSpecification, unmet:
