...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/lancelot/specs/comparator_spec.py
# Compiled at: 2009-02-19 03:36:16
""" Specs for core library classes / behaviours """
from lancelot import Spec, grouping, verifiable, verify
from lancelot.comparators import Comparator, EqualsEquals, ExceptionValue, SameAs, LessThan, GreaterThan, Contain, NoneValue, NotNoneValue, FloatValue, StrEquals, ReprEquals, Anything, Nothing, NotComparator, OrComparator, NotContain, Length, Empty, Type, LessThanOrEqual, GreaterThanOrEqual

class BaseComparatorBehaviour(object):
...
grouping(BaseComparatorBehaviour)
...
def type_behaviour():
    ...
    spec.description().should_be("type <type 'list'>")
    spec = Spec(Type([]))
    spec.compares_to([]).should_be(True)
    spec.compares_to({}).should_be(False)
    spec.description().should_be("type <type 'list'>")
