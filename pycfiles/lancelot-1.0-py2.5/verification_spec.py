...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/lancelot/specs/verification_spec.py
...
class SilentListener(ConsoleListener):
    ...
    def __init__(self):
        ...
        super(SilentListener, self).__init__(self, self)
...
class VerifiableDecoratorBehaviour(object):
...
grouping(VerifiableDecoratorBehaviour)

class AllVerifiableTotalBehaviour(object):
    ...
    def should_ignore_duplicates(self):
        ...
        spec = Spec(AllVerifiable, given=silent_listener)
        spec.when(spec.include(raise_index_error), spec.include(raise_index_error))
        spec.then(spec.total()).should_be(1)


grouping(AllVerifiableTotalBehaviour)
...
class AllVerifiableVerifyFnBehaviour(object):
...
grouping(AllVerifiableVerifyFnBehaviour)

class AllVerifiableVerifyBehaviour(object):
    ...
    def should_verify_each_item(self):
        ...
        spec = Spec(AllVerifiable, given=silent_listener)
        spec.when(spec.include(lambda_list_append1), spec.include(lambda_list_append2), spec.verify())
...
grouping(AllVerifiableVerifyBehaviour)

class RelatedVerifiables(object):
...
class GroupingDecoratorBehaviour(object):
...
grouping(GroupingDecoratorBehaviour)
...
def all_verif_failfast_behaviour():
    ...
    spec = Spec(AllVerifiable, given=silent_listener)
    spec.when(spec.include(raise_index_error), spec.include(dont_raise_index_error))
    spec.then(spec.verify(fail_fast=True))
    spec.should_be({'total': 2, 'verified': 0, 'unverified': 2, 'fail_fast': True})
    spec = Spec(AllVerifiable, given=silent_listener)
    spec.when(spec.include(unmet_specification), spec.include(dont_raise_index_error))
    spec.then(spec.verify(fail_fast=True))
    spec.should_be({'total': 2, 'verified': 0, 'unverified': 2, 'fail_fast': True})
...
def notification_behaviour():
    ...
    spec.when(spec.include(string_abc), spec.include(raise_index_error), spec.include(unmet_specification))
    spec.then(spec.verify())
    spec.should_collaborate_with(listener.all_verifiable_starting(all_verifiable_with_mock_listener), listener.verification_started(string_abc), listener.specification_met(string_abc), listener.verification_started(raise_index_error), listener.unexpected_exception(raise_index_error, Type(IndexError)), listener.verification_started(unmet_specification), listener.specification_unmet(unmet_specification, Type(UnmetSpecification)), listener.all_verifiable_ending(all_verifiable_with_mock_listener, results), and_result=results)
