...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/archipelagentvirtualmachinevnc/vnc.py
# Compiled at: 2013-03-20 13:50:16
import libvirt, thread, xmpp
...
class TNArchipelVNC(TNArchipelPlugin):
    ...
    def __init__(self, configuration, entity, entry_point_group):
        ...
        registrar_item = {'commands': ['vnc', 'screen'], 'parameters': [], 'method': self.message_display, 
           'permissions': [
                         'vnc_display'], 
    ...
    def create_novnc_proxy(self, origin=None, user_info=None, parameters=None):
        ...
        if not self.entity.hypervisor.is_hypervisor((archipel.archipelLibvirtEntity.ARCHIPEL_HYPERVISOR_TYPE_QEMU, archipel.archipelLibvirtEntity.ARCHIPEL_HYPERVISOR_TYPE_XEN)):
            ...
            self.novnc_proxy = WebSocketProxy(target_host='127.0.0.1', target_port=current_vnc_port, listen_host='0.0.0.0', listen_port=novnc_proxy_port, cert=cert, ssl_only=onlyssl, wrap_cmd=None, wrap_mode='exit', verbose=self.websocket_verbose, daemon=False)
    ...
    def display(self):
        ...
        try:
            ...
            if supportSSL.lower() in ('none', 'no', 'false'):
                ...
                supportSSL = True
            return {'direct': directport, 'proxy': proxyport, 'onlyssl': self.configuration.getboolean('VNC', 'vnc_only_ssl'), 
    ...
    def iq_display(self, iq):
        ...
        try:
            if not self.entity.domain:
                ...
                ports = self.display()
                payload = ports or xmpp.Node('display', attrs={})
            else:
