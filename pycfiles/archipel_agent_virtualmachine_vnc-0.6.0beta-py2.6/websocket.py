...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/archipelagentvirtualmachinevnc/websocket.py
...
"""
import os, sys, time, errno, signal, socket, traceback, select, array, struct
...
class WebSocketServer(object):
    ...
    """
    buffer_size = 65536
    server_handshake_hixie = 'HTTP/1.1 101 Web Socket Protocol Handshake\r\nUpgrade: WebSocket\r\nConnection: Upgrade\r\n%sWebSocket-Origin: %s\r\n%sWebSocket-Location: %s://%s%s\r\n'
    server_handshake_hybi = 'HTTP/1.1 101 Switching Protocols\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nSec-WebSocket-Accept: %s\r\n'
    ...
    def __init__(self, listen_host='', listen_port=None, source_is_ipv6=False, verbose=False, cert='', key='', ssl_only=None, daemon=False, record='', web='', run_once=False, timeout=0, idle_timeout=0):
    ...
    def socket(host, port=None, connect=False, prefer_ipv6=False, unix_socket=None, use_ssl=False):
        ...
        if connect:
            ...
            addrs = unix_socket or socket.getaddrinfo(host, port, 0, socket.SOCK_STREAM, socket.IPPROTO_TCP, flags)
            if not addrs:
    ...
    def unmask(buf, f):
        ...
        if numpy:
            b = c = s2b('')
            if f['length'] >= 4:
                mask = numpy.frombuffer(buf, dtype=numpy.dtype('<u4'), offset=f['hlen'], count=1)
                data = numpy.frombuffer(buf, dtype=numpy.dtype('<u4'), offset=pstart, count=int(f['length'] / 4))
                b = numpy.bitwise_xor(data, mask).tostring()
            if f['length'] % 4:
                mask = numpy.frombuffer(buf, dtype=numpy.dtype('B'), offset=f['hlen'], count=f['length'] % 4)
                data = numpy.frombuffer(buf, dtype=numpy.dtype('B'), offset=pend - f['length'] % 4, count=f['length'] % 4)
    ...
    def encode_hixie(buf):
        return (s2b('\x00' + b2s(b64encode(buf)) + b'\xff'), 1, 1)

    @staticmethod
    def decode_hixie(buf):
        end = buf.find(s2b(b'\xff'))
        return {'payload': b64decode(buf[1:end]), 'hlen': 1, 
           'length': end - 1, 
           'left': len(buf) - (end + 1)}
    ...
    def send_frames(self, bufs=None):
        ...
        if bufs:
            for buf in bufs:
                ...
                if self.rec:
                    self.rec.write('%s,\n' % repr('{%s{' % tdelta + encbuf[lenhead:-lentail]))
            ...
            while buf:
                if self.version.startswith('hybi'):
                    ...
                    if frame['payload'] == None:
                        ...
                        closed = {'code': frame['close_code'], 'reason': frame['close_reason']}
                        break
                else:
                    if buf[0:2] == s2b(b'\xff\x00'):
                        closed = {'code': 1000, 'reason': 'Client sent orderly close frame'}
                        break
                    elif buf[0:2] == s2b(b'\x00\xff'):
                        buf = buf[2:]
                        continue
                    elif buf.count(s2b(b'\xff')) == 0:
                        self.traffic('}.')
                        self.recv_part = buf
                        break
    ...
    def do_handshake(self, sock, address):
        ...
        if handshake == '':
        ...
        elif handshake[0] in ('\x16', b'\x80', 22, 128):
            if not ssl:
            ...
            retsock = None
            try:
                retsock = ssl.wrap_socket(sock, server_side=True, certfile=self.cert, keyfile=self.key)
            except ssl.SSLError:
                (_, x, _) = sys.exc_info()
            ...
            try:
                try:
                    ...
                    if self.run_once:
                        ...
                        p = multiprocessing.Process(target=self.top_new_client, args=(
                         startsock, address))
                        p.start()
                    else:
...
class WSRequestHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.headers.get('upgrade'):
            if self.headers.get('upgrade').lower() == 'websocket' and (self.headers.get('sec-websocket-key1') or self.headers.get('websocket-key1')):
