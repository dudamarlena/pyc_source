...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/uncompyle6/parsers/parse26.py
...
class Python26Parser(Python2Parser):
    ...
    def reduce_is_invalid(self, rule, ast, tokens, first, last):
        ...
        elif rule == ('list_for', ('expr', 'for_iter', 'store', 'list_iter', 'JUMP_ABSOLUTE', 'come_froms', 'POP_TOP',
 'jb_pop')):
            ja_attr = ast[4].attr
            ...
            if tokens[last] != 'COME_FROM':
                if tokens[(last - 1)] == 'COME_FROM':
                    last -= 1
                return tokens[last] == 'COME_FROM' and tokens[(last - 1)] == 'END_FINALLY' and tokens[(last - 2)] == 'POP_TOP' and tokens[(last - 3)].kind not in frozenset(('JUMP_FORWARD',
                                                                                                                                                                             'RETURN_VALUE')) or tokens[(last - 3)] == 'JUMP_FORWARD' and tokens[(last - 3)].attr != 2
            ...
            if ast[3] == 'except_handler':
                ...
                if tokens[last] == 'COME_FROM' and tokens[(last - 1)] == 'COME_FROM':
                    last -= 1
                return tokens[last] == 'COME_FROM' and (tokens[(last - 1)] == 'END_FINALLY' and tokens[(last - 2)] == 'POP_TOP' and tokens[(last - 3)].kind in frozenset(('JUMP_FORWARD',
                                                                                                                                                                          'RETURN_VALUE')) and tokens[(last - 3)] != 'JUMP_FORWARD' or tokens[(last - 3)].attr == 2)
        return False
