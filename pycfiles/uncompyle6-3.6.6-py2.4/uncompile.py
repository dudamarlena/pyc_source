...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/uncompyle6/bin/uncompile.py
...
def main_bin():
    if sys.version_info[0:2] not in ((2, 4), (2, 5), (2, 6), (2, 7)):
        sys.stderr.write('Error: this branch of %s requires Python 2.4, 2.5, 2.6 or 2.7' % program)
        sys.exit(-1)
    ...
    if numproc <= 1:
        ...
        def process_func():
            try:
                (tot_files, okay_files, failed_files, verify_failed_files) = (0, 0,
                                                                              0,
                                                                              0)
                while 1:
                    f = fqueue.get()
                    if f is None:
                        break
                    (t, o, f, v) = main(src_base, out_base, [f], None, outfile, **options)
                    tot_files += t
                    okay_files += o
                    failed_files += f
