...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/uncompyle6/scanners/scanner30.py
...
class Scanner30(Scanner3):
    ...
    def detect_control_flow(self, offset, targets, inst_index):
        ...
        if op == self.opc.SETUP_LOOP:
            ...
            if jump_back:
                if jump_back != self.prev_op[end] and self.is_jump_forward(jump_forward_offset):
            ...
            if op == opc.JUMP_IF_FALSE:
                ...
                if match:
                    ...
                    if is_jump_forward and pre_rtarget not in self.stmts and self.restrict_to_parent(self.get_target(pre_rtarget), parent) == rtarget:
                        if code[prev_op[pre_rtarget]] == self.opc.JUMP_ABSOLUTE and self.remove_mid_line_ifs([offset]) and target == self.get_target(prev_op[pre_rtarget]) and (prev_op[pre_rtarget] not in self.stmts or self.get_target(prev_op[pre_rtarget]) > prev_op[pre_rtarget]) and 1 == len(self.remove_mid_line_ifs(self.rem_or(start, prev_op[pre_rtarget], JUMP_TF, target))):
                            pass
                        elif code[prev_op[pre_rtarget]] == self.opc.RETURN_VALUE and self.remove_mid_line_ifs([offset]) and 1 == len(set(self.remove_mid_line_ifs(self.rem_or(start, prev_op[pre_rtarget], JUMP_TF, target))) | set(self.remove_mid_line_ifs(self.rem_or(start, prev_op[pre_rtarget], (
                         opc.JUMP_IF_FALSE, opc.JUMP_IF_TRUE, opc.JUMP_ABSOLUTE), pre_rtarget, True)))):
                            pass
                        else:
                            fix = None
                ...
                if prev_op[next] == offset:
                    ...
                    if target == self.get_target(next):
                        if code[prev_op[next]] == opc.JUMP_IF_FALSE and (code[next] == self.opc.JUMP_FORWARD or target != rtarget or code[prev_op[pre_rtarget]] not in (self.opc.JUMP_ABSOLUTE, self.opc.RETURN_VALUE)):
