...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/uncompyle6/scanners/scanner2.py
...
from xdis.bytecode import op_has_argument, instruction_size, _get_const_info
...
class Scanner2(Scanner):
        ...
        for offset in self.op_range(0, codelen):
            if offset in jump_targets:
                ...
                for jump_offset in sorted(jump_targets[offset], reverse=True):
                    ...
                    tokens.append(Token(come_from_name, jump_offset, repr(jump_offset), offset='%s_%d' % (offset, jump_idx), has_arg=True))
                    jump_idx += 1
            ...
            if op in self.varargs_ops:
                ...
                if target <= offset:
                    ...
                    if offset in self.stmts:
                        if self.code[(offset + 3)] not in (self.opc.END_FINALLY, self.opc.POP_BLOCK) and (offset in self.linestarts and self.code[self.prev[offset]] == self.opc.JUMP_ABSOLUTE or self.code[target] == self.opc.FOR_ITER or offset not in self.not_continue):
                            op_name = 'CONTINUE'
    ...
    def detect_control_flow(self, offset, op, extended_arg):
        ...
        if op == self.opc.SETUP_LOOP:
            ...
            if jump_back_offset:
                if jump_back_offset != self.prev[loop_end_offset] and code[jump_forward_offset] in self.jump_forward:
                    if code[self.prev[loop_end_offset]] == self.opc.RETURN_VALUE or code[self.prev[loop_end_offset]] == self.opc.POP_BLOCK and code[return_val_offset1] == self.opc.RETURN_VALUE:
                        jump_back_offset = None
                jump_back_offset = jump_back_offset or self.last_instr(start, loop_end_offset, self.opc.RETURN_VALUE)
                ...
                if match:
                    if code[pre_rtarget] in self.jump_forward and pre_rtarget not in self.stmts and self.restrict_to_parent(self.get_target(pre_rtarget), parent) == rtarget:
                        if code[pre[pre_rtarget]] == self.opc.JUMP_ABSOLUTE and self.remove_mid_line_ifs([offset]) and target == self.get_target(pre[pre_rtarget]) and (pre[pre_rtarget] not in self.stmts or self.get_target(pre[pre_rtarget]) > pre[pre_rtarget]) and 1 == len(self.remove_mid_line_ifs(self.rem_or(start, pre[pre_rtarget], self.pop_jump_if, target))):
                            pass
                        elif code[pre[pre_rtarget]] == self.opc.RETURN_VALUE and self.remove_mid_line_ifs([offset]) and 1 == len(set(self.remove_mid_line_ifs(self.rem_or(start, pre[pre_rtarget], self.pop_jump_if, target))) | set(self.remove_mid_line_ifs(self.rem_or(start, pre[pre_rtarget], (
                        ...
                        if self.version < 2.7 and parent['type'] in ('root', 'for-loop',
                                                                     'if-then', 'else',
                                                                     'try'):
                            self.fixed_jumps[offset] = rtarget
                        else:
                        ...
                        return
            if self.version < 2.7:
                if code[(offset + 3)] == self.opc.POP_TOP:
                ...
                if pre[next] == offset:
                    pass
                if code[next] in self.jump_forward and target == self.get_target(next) and code[pre[next]] == self.opc.PJIF and (code[next] == self.opc.JUMP_FORWARD or target != rtarget or code[pre[pre_rtarget]] not in (self.opc.JUMP_ABSOLUTE, self.opc.RETURN_VALUE)):
        ...
        if self.version == 2.7:
            if code[pre_rtarget] == self.opc.JUMP_ABSOLUTE:
                if pre_rtarget in self.stmts:
                    if pre_rtarget != offset:
                        if pre[pre_rtarget] != offset:
                    ...
                    if code_pre_rtarget in self.jump_forward:
                    ...
                    elif code_pre_rtarget == self.opc.RETURN_VALUE and (self.version == 2.7 or pre_rtarget not in self.ignore_if) and (self.code[self.prev[offset]] != self.opc.COMPARE_OP or self.code[(self.prev[offset] + 1)] != 10):
                        ...
                        if self.version == 2.7 or code[(pre_rtarget + 1)] != self.opc.JUMP_FORWARD:
                            self.fixed_jumps[offset] = rtarget
                            if self.version == 2.7 and self.insts[self.offset2inst_index[pre[pre_rtarget]]].is_jump_target:
                ...
                if label is not None and label != -1:
                    if self.version == 2.7:
                        ...
                        if skip_come_from:
                            ...
                            if skip_come_from or offset not in set(self.ignore_if):
                                if label in self.setup_loops:
                                    source = self.setup_loops[label]
                                else:
                                    source = offset
