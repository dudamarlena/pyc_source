...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/uncompyle6/scanners/scanner37base.py
# Compiled at: 2020-04-20 23:03:27
"""
...
import xdis, xdis.opcodes.opcode_37 as op3
        ...
        for (i, inst) in enumerate(self.insts):
            if inst.opname == 'JUMP_FORWARD':
                ...
                if jump_inst.has_extended_arg and jump_inst.opname.startswith('JUMP'):
                    self.insts[i] = Instruction(jump_inst.opname, jump_inst.opcode, jump_inst.optype, jump_inst.inst_size, jump_inst.arg, jump_inst.argval, jump_inst.argrepr, jump_inst.has_arg, inst.offset, inst.starts_line, inst.is_jump_target, inst.has_extended_arg)
            ...
            if inst.opname == 'EXTENDED_ARG':
                ...
                if inst.offset in jump_targets:
                    ...
                    for jump_offset in sorted(jump_targets[inst.offset], reverse=True):
                        ...
                        j = tokens_append(j, Token(come_from_name, jump_offset, repr(jump_offset), offset='%s_%s' % (inst.offset, jump_idx), has_arg=True, opc=self.opc, has_extended_arg=False))
                        jump_idx += 1

                if inst.offset in self.else_start:
                    end_offset = self.else_start[inst.offset]
                    j = tokens_append(j, Token('ELSE', None, repr(end_offset), offset='%s' % inst.offset, has_arg=True, opc=self.opc, has_extended_arg=inst.has_extended_arg))
                pattr = inst.argrepr
                opname = inst.opname
