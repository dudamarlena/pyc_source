...
# [GCC 8.4.0]
# Embedded file name: /Users/lukasz/develop/workspace/.pythonpath/vcs/tests/test_cli.py
# Compiled at: 2013-04-28 19:09:20
from __future__ import with_statement
import datetime, mock, sys, subprocess, vcs, vcs.cli
...
class TestExecutionManager(unittest.TestCase):
    ...
    def test_get_vcsrc(self):
        with nested(mock.patch('vcs.conf.settings.VCSRC_PATH', 'foobar'), mock.patch('vcs.cli.create_module')) as (VP, m):
    ...
    def test_get_command_class(self):
        with mock.patch.object(vcs.cli, 'registry', {'foo': 'decimal.Decimal', 
           'bar': 'socket.socket'}):
            manager = DummyExecutionManager()
    ...
    def test_get_commands(self):
        with mock.patch.object(vcs.cli, 'registry', {'mock': mock, 
           'foo': 'vcs.cli.BaseCommand', 
           'bar': 'vcs.tests.test_cli.DummyExecutionManager'}):
            manager = DummyExecutionManager()
            self.assertItemsEqual(manager.get_commands(), {'foo': BaseCommand, 
...
class TestBaseCommand(unittest.TestCase):
    ...
    def test_default_stderr(self):
        ...
        with mock.patch.object(sys, 'stderr', stream):
            command = BaseCommand()
            command.stderr.write('foobar')
            self.assertEqual(sys.stderr.getvalue(), 'foobar')
    ...
    def test_get_parser(self):
        ...
        class Command(BaseCommand):
            option_list = (
             make_option('-f', '--foo', action='store', dest='foo', default='bar'),)
