...
# [GCC 8.4.0]
# Embedded file name: /Users/lukasz/develop/workspace/.pythonpath/vcs/cli.py
# Compiled at: 2013-04-28 18:52:59
"""
...
registry = {'cat': 'vcs.commands.cat.CatCommand', 
...
class ExecutionManager(object):
    ...
    def autocomplete(self):
        ...
        if current:
            self.stdout.write(unicode((' ').join([ name for name in cmd_names if name.startswith(current) ])))
        sys.exit(1)
...
class BaseCommand(object):
    ...
    args = ''
    option_list = (
     make_option('--debug', action='store_true', dest='debug', default=False, help='Enter debug mode before raising exception'),
     make_option('--traceback', action='store_true', dest='traceback', default=False, help='Print traceback in case of an error'))
    ...
    def get_parser(self, prog_name, subcommand):
        ...
        """
        parser = OptionParser(prog=prog_name, usage=self.usage(subcommand), version=self.get_version(), option_list=sorted(self.get_option_list()))
        return parser
...
class ChangesetCommand(RepositoryCommand):
    ...
    * ``show_progress_bar``: specifies if bar indicating progress of processed
    ...
    option_list = RepositoryCommand.option_list + (
     make_option('--author', action='store', dest='author', help='Show changes committed by specified author only.'),
     make_option('-r', '--reversed', action='store_true', dest='reversed', default=False, help='Iterates in asceding order.'),
     make_option('-b', '--branch', action='store', dest='branch', help="Narrow changesets to chosen branch. If not given, working directory branch is used. For bare repository default would be SCM's default branch (i.e. master for git)"),
     make_option('--all', action='store_true', dest='all', default=False, help='Show changesets across all branches.'),
     make_option('--since', '--start-date', action='store', dest='start_date', help='Show only changesets not younger than specified start date.'),
     make_option('--until', '--end-date', action='store', dest='end_date', help='Show only changesets not older than specified end date.'),
     make_option('--limit', action='store', dest='limit', default=None, help='Limit number of showed changesets.'))

    def show_changeset(self, changeset, **options):
        author = options.get('author')
        if author:
            if author.startswith('*') and author.endswith('*') and author.strip('*') in changeset.author:
                return True
            if author.startswith('*') and changeset.author.endswith(author.strip('*')):
                return True
            if author.endswith('*') and changeset.author.startswith(author.strip('*')):
                return True
    ...
    def get_changesets(self, repo, **options):
        ...
        * ``limit``: if specified, show no more changesets than this value.
        ...
        changesets = repo.get_changesets(start=options.get('start'), end=options.get('end', options.get('main')), start_date=options.get('start_date'), end_date=options.get('end_date'), branch_name=branch_name, reverse=not options.get('reversed', False))
        try:
...
class SingleChangesetCommand(RepositoryCommand):
    ...
    * ``min_args``: minimal number of arguements to parse. Default is ``1``.
    ...
    min_args = 1
    option_list = RepositoryCommand.option_list + (
     make_option('-c', '--commit', action='store', dest='changeset_id', default=None, help='Use specific commit. By default we use HEAD/tip'),)
