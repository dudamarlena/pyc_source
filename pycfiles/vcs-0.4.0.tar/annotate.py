...
# [GCC 8.4.0]
# Embedded file name: /Users/lukasz/develop/workspace/.pythonpath/vcs/utils/annotate.py
...
def annotate_highlight(filenode, annotate_from_changeset_func=None, order=None, headers=None, **options):
    """
    ...
    formatter = AnnotateHtmlFormatter(filenode=filenode, order=order, headers=headers, annotate_from_changeset_func=annotate_from_changeset_func, **options)
...
class AnnotateHtmlFormatter(HtmlFormatter):
    ...
    def __init__(self, filenode, annotate_from_changeset_func=None, order=None, **options):
        """
...
' %                       (changeset.id, changeset.id)
        ...
        if headers and not ('ls' in headers and 'annotate' in headers and 'code' in headers):
            raise ValueError("If headers option dict is specified it must all 'ls', 'annotate' and 'code' keys")
        self.headers = headers
        if isinstance(filenode, FileNode):
            self.filenode = filenode
        else:
            raise VCSError('This formatter expect FileNode parameter, not %r' % type(filenode))
        return
    ...
    def _wrap_tablelinenos(self, inner):
        ...
        annotate = ('').join(self.annotate_from_changeset(changeset) for changeset in annotate_changesets)
        headers_row = []
        if self.headers:
            headers_row = [
        ...
        for key in self.order:
            if key == 'ls':
                body_row_start.append('<td class="linenos"><div class="linenodiv"><pre>' + ls + '</pre></div></td>')
            elif key == 'annotate':
                body_row_start.append('<td class="annotate"><div class="annotatediv"><pre>' + annotate + '</pre></div></td>')
            elif key == 'code':
                body_row_start.append('<td class="code">')

        yield (
         0,
         '<table class="%stable">' % self.cssclass + ('').join(headers_row) + ('').join(body_row_start))
        yield (
