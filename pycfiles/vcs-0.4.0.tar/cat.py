...
# [GCC 8.4.0]
# Embedded file name: /Users/lukasz/develop/workspace/.pythonpath/vcs/commands/cat.py
...
class CatCommand(SingleChangesetCommand):
    ...
    option_list = SingleChangesetCommand.option_list + (
     make_option('--blame', action='store_true', dest='blame', default=False, help='Annotate output with '),
     make_option('--plain', action='store_true', dest='plain', default=False, help="Simply write output to terminal, don't use any extra formatting/colors (pygments)."),
     make_option('-n', '--line-numbers', action='store_true', dest='linenos', default=False, help='Shows line numbers'))

    def get_option_list(self):
        option_list = super(CatCommand, self).get_option_list()
        try:
            __import__('pygments')
            option = make_option('-f', '--formatter', action='store', dest='formatter_name', default='terminal', help='Pygments specific formatter name.')
            option_list += (option,)
        except ImportError:
            pass
    ...
    def cat(self, node, **options):
        ...
        if options.get('linenos'):
            ...
            text = ('\n').join('%s %s' % (str(lineno + 1).rjust(linenos_width), lines[lineno]) for lineno in xrange(len(lines)))
            text += '\n'
