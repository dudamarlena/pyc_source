...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/examples/ensemble/rf_performance_plot_1.py
# Compiled at: 2014-01-26 00:07:45
import matplotlib
matplotlib.use('Agg')
import os.path, random, re, sys, cPickle as pickle, matplotlib.pyplot as plt, numpy as np
...
def score_trees(rf_classifier, trees, test, DATASET, cluster_indices=None):
    ...
    for trees in models:
        ...
        test_results = evaluation.testing.ExperimentResults(1, classifier_names=[
         'RF'], domain=test.domain, test_type=evaluation.testing.TEST_TYPE_SINGLE, weights=0)
...
def plot_trees_score(DATASET, depth=None, seed=0):
    ...
    if DATASET not in cache:
        ...
        def data_center(table, original=table):
            ...
            model = mm.Model('RF', None, np.mean(np.array([ ex['model'].value.probabilities for ex in table ]), axis=0), onemodel.class_values, [], [
...
if __name__ == '__main__':
    ...
    if len(DO) == 0:
        DO = [
         'breast-cancer-wisconsin', 'voting', 'zoo', 'mushroom', 'adult_sample', 'glass', 'primary-tumor', 'vehicle', 'wdbc', 'dermatology', 'iris', 'marketing']
