...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/orangecontrib/modelmaps/modelmap.py
# Compiled at: 2014-01-29 17:39:49
"""
.. index:: model map
...
"""
import bz2, collections, itertools, math, random, os.path, time, cPickle as pickle, numpy as np
try:
    import scipy.stats
except:
    pass
...
import model
MODEL_LIST = [
 '', 'SCATTERPLOT', 'RADVIZ', 'SPCA', 'POLYVIZ', 'TREE', 'NaiveLearner', 'kNNLearner', 'SVMLearner', 'RF']
...
def distance_manhattan(m1, m2):
    return np.sum(np.abs(m1.probabilities - m2.probabilities)) / 2.0 / len(m1.probabilities)


def distance_euclidean(m1, m2):
    return np.sum(np.sqrt(np.sum((m1.probabilities - m2.probabilities) ** 2, axis=1))) / math.sqrt(2.0) / len(m1.probabilities)
...
def model_network(smx, table=None, knn=1):
    ...
    edge_list = network.GraphLayout().edges_from_distance_matrix(smx, -1.0, -1.0, knn)
...
class BuildModelMap(object):
    ...
    def build_model(self, attributes, learner):
        ...
        return model.Model(type(learner).__name__, learner(_data), np.array(probabilities), {val:i for i, val in enumerate(self.data().domain.class_var.values)}, [ x.name for x in _data.domain.attributes ], np.array(instance_predictions), np.array(instance_classes))
    ...
    def build_projection_model(self, attributes, viz_method):
        ...
        if not table:
            ...
            return model.Model(method, learner(table), np.array(probabilities), {val:i for i, val in enumerate(self.data_d.domain.class_var.values)}, attributes, np.array(instance_predictions), np.array(instance_classes), XAnchors=XAnchors, YAnchors=YAnchors)
    ...
    def build_rf_models(self, trees=50, max_depth=4, min_instances=5):
        ...
        def get_features(cls, domain):
            ...
            def tree_attr(node):
                if not node or node.branch_selector is None:
                    return []
                size = [
                 node.branch_selector.class_var.name]
        ...
        for c in rf_classifier.classifiers:
            ...
            m = model.Model('RF', c, np.array(probabilities), {val:k for k, val in enumerate(test.domain.class_var.values)}, get_features(c, test.domain), np.array(instance_predictions), np.array(instance_classes))
        ...
        for i in range(3 if three_folds else 1):
            ...
            for j in range(2 if three_folds else 1):
                ...
                for c in rf_classifier.classifiers:
                    ...
                    models.append(model.Model('RF', c, np.array(probabilities), {val:k for k, val in enumerate(test.domain.class_var.values)}, get_features(c, test.domain), np.array(instance_predictions), np.array(instance_classes), XAnchors=None, YAnchors=None))
    ...
    def _print_time(self, time_start, iter, numiter):
        ...
        time_total = time_elapsed / iter * (numiter * (numiter - 1) / 2.0)
