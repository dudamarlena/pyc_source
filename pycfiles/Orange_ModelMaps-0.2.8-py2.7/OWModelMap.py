...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/orangecontrib/modelmaps/widgets/OWModelMap.py
# Compiled at: 2014-01-28 13:07:54
"""
<name>Model Map</name>
...
INPUTS = [
 (
  'Community Detection', CommunityDetection, 'set_community'),
 (
...
ICON_PATHS = [
 (
  'TREE', 'Classify/icons/ClassificationTree'),
 (
...
class OWModelMapCanvas(OWNxCanvas):
    ...
    def set_tooltip_attributes(self, attributes):
        if self.graph is None or self.items is None or not isinstance(self.items, data.Table):
            return
        attributes = [
         'Cluster CA', 'label', 'P', 'attributes']
        tooltip_attributes = [ self.items.domain[att] for att in attributes if att in self.items.domain
                             ]
        self.networkCurve.set_node_tooltips(dict((node,
         (', ').join(str(self.items[node][att]) for att in tooltip_attributes)) for node in self.graph))
        return
...
class OWModelMap(OWNxExplorer, OWNxHist):
    ...
    def __init__(self, parent=None, signalManager=None, name='Model Map'):
        ...
        self.inputs = [
         (
          'Community Detection', CommunityDetection, self.set_community),
         (
