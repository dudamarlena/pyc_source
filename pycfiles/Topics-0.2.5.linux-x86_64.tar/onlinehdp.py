...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/Topics/onlinehdp/onlinehdp.py
# Compiled at: 2013-03-01 12:59:26
"""
online hdp with lazy update
part of code is adapted from Matt's online lda code
"""
import random, numpy as np, scipy.special as sp
from Topics.onlinehdp import utils
...
class online_hdp:
    ...
    def new_init(self, c):
        ...
        for doc in docs:
            ...
            for iter in range(5):
                ...
                doc_score = self.doc_e_step(doc, sstats, Elogsticks_1st, word_list, unique_words, var_converge=0.0001, max_iter=5)
    ...
    def process_documents(self, docs, var_converge, unseen_ids=[], update=True, opt_o=True):
        ...
        if adding_noise:
            ...
            self.m_Elogbeta[:, word_list] = sp.psi(self.m_eta + self.m_lambda[:, word_list]) - sp.psi(self.m_W * self.m_eta + self.m_lambda_sum[:, np.newaxis])
            ...
            for i, doc in enumerate(docs):
                doc_score = self.doc_e_step(doc, ss, Elogsticks_1st, word_list, unique_words, var_converge)
                count += doc.total
                score += doc_score
                if i in unseen_ids:
    ...
    def update_lambda(self, sstats, word_list, opt_o):
        ...
        if rhot < rhot_bound:
            rhot = rhot_bound
        self.m_rhot = rhot
        self.m_lambda[:, word_list] = self.m_lambda[:, word_list] * (1 - rhot) + rhot * self.m_D * sstats.m_var_beta_ss / sstats.m_batchsize
        self.m_lambda_sum = (1 - rhot) * self.m_lambda_sum + rhot * self.m_D * np.sum(sstats.m_var_beta_ss, axis=1) / sstats.m_batchsize
        self.m_updatect += 1
        self.m_timestamp[word_list] = self.m_updatect
        self.m_r.append(self.m_r[(-1)] + np.log(1 - rhot))
        self.m_varphi_ss = (1.0 - rhot) * self.m_varphi_ss + rhot * sstats.m_var_sticks_ss * self.m_D / sstats.m_batchsize
        if opt_o:
    ...
    def update_expectations(self):
        ...
        self.m_Elogbeta = sp.psi(self.m_eta + self.m_lambda) - sp.psi(self.m_W * self.m_eta + self.m_lambda_sum[:, np.newaxis])
