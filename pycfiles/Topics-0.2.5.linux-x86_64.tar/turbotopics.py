...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/Topics/visualization/turbotopics.py
# Compiled at: 2013-03-10 06:10:11
from __future__ import division
import sys, random, copy, itertools, regex
from math import pow, log, exp
import codecs
from multiprocessing import Pool
import nltk
_chi_sq_table = {0.1: 2.70554345, 0.01: 6.634897, 
   0.001: 10.82757, 
   0.0001: 15.13671, 
   1e-05: 19.51142, 
   1e-06: 23.92813, 
...
class LikelihoodRatio:
    ...
    def score(self, count, unigram, bigram, total, min_count):
        ...
        for v in bigram.keys():
            ...
            val[v] = 2 * (big * log_pi_vu + (uni - big) * log_pi_vnu - uni * log_pi_v_old + (count - big) * (log_1mp_vu - log_1mp_v))
            assert val[v] == val[v]
...
def write_vocab(v, outfname, incl_stop=False):
    ...
    with codecs.open(outfname, 'w', encoding='utf8') as (f):
        [ f.write('%-25s | %8.2f\n' % (i[0].strip(), i[1])) for i in sorted(v.items(), key=lambda x: -x[1]) if incl_stop or i[0] not in _stop_words ]
...
def words_from_vocab_machine(mach):
    ...
    for v_1, next_mach in mach.iteritems():
        words.append(v_1)
        words.extend([ '%s %s' % (v_1, v_2) for v_2 in words_from_vocab_machine(next_mach)
                     ])
...
def nested_sig_bigrams(iter_generator, update_fun, sig_test, Min):
    ...
    terms = [ item[0] for item in sorted(counts.marg.items(), key=lambda x: -x[1]) if item[1] >= Min
            ]
    while terms:
        new_vocab = {}
        sig_test.reset()
        ...
        terms = [ item[0] for item in sorted(new_vocab.items(), key=lambda x: -x[1]) if item[1] >= Min
                ]
