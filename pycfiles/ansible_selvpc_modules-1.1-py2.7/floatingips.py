...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/ansible/module_utils/selvpc_utils/floatingips.py
...
def delete_useless_ips(client, to_delete, project_id):
    ...
    """
    result = []
    ips = [ ip_obj._info for ip_obj in client.floatingips.list(project_id=project_id)
          ]
...
def add_floatingips(module, client, project_id, project_name, floatingips, force):
    ...
    to_create, to_delete = common.compare_existed_and_needed_objects(actual_ips, parsed_ips, force)
    to_create = [ {'region': region, 'quantity': quantity} for region, quantity in to_create.items() if quantity
                ]
    if to_create:
