...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/ansible/module_utils/selvpc_utils/licenses.py
...
def delete_useless_licenses(client, to_delete, project_id):
    ...
    """
    result = []
    licenses = [ lic_obj._info for lic_obj in client.licenses.list(project_id=project_id)
               ]
...
def add_licenses(module, client, project_id, project_name, licenses, force):
    ...
    to_create, to_delete = common.compare_existed_and_needed_objects(actual_lics, parsed_lics, force)
    to_create = [ {'region': params[0], 'type': params[1], 'quantity': quantity} for params, quantity in to_create.items() if quantity
                ]
    if to_create:
        result = client.licenses.add(project_id, {'licenses': to_create})
