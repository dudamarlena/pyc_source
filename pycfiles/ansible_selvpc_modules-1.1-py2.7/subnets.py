...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/ansible/module_utils/selvpc_utils/subnets.py
...
def delete_useless_subnets(client, to_delete, project_id):
    ...
    """
    result = []
    subnets = [ sub_obj._info for sub_obj in client.subnets.list(project_id=project_id)
              ]
    for key in to_delete:
        subs_to_delete = [ sub for sub in subnets if (sub.get('region'), 'ipv4', int(sub.get('cidr')[-2:])) == key ]
...
def add_subnets(module, client, project_id, project_name, subnets, force):
    ...
    to_create, to_delete = common.compare_existed_and_needed_objects(actual_subs, parsed_subs, force)
    to_create = [ {'region': params[0], 'type': params[1], 'prefix_length': params[2], 'quantity': quantity} for params, quantity in to_create.items() if quantity
                ]
    if to_create:
    ...
    if to_delete:
        result = delete_useless_subnets(client, to_delete, project_id)
        changed = True
        msg.append('some subnets have been deleted')
        jsonifed_result.update({'deleted': result})
    return (
     jsonifed_result, changed, common.generate_result_msg(msg))
