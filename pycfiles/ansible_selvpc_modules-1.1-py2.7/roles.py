...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/ansible/module_utils/selvpc_utils/roles.py
...
def add_role(module, client, project_id, project_name, user_id):
    result, changed, msg = None, False, ('User is {} already in {} project').format(user_id, project_id)
    if not common._check_user_role(client, project_id, user_id):
        client.roles.add_user_role_in_project(project_id, user_id)
        changed, msg = True, ('User {} has been added to {} project').format(user_id, project_id)
    return (
     result, changed, msg)
