...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.7/site-packages/twizzle/attacks_preset.py
# Compiled at: 2019-06-24 20:06:18
# Size of source mod 2**32: 16410 bytes
"""
...
def rotation_cropped(aInputImage, dRotationAngle=25):
    ...
    M = cv2.getRotationMatrix2D(center=(
     (lOriginalX - 1) / 2, (lOriginalY - 1) / 2),
      angle=dRotationAngle,
      scale=1.0)
    lNewX, lNewY = rotatedRectWithMaxArea(lOriginalX, lOriginalY, math.radians(dRotationAngle))
...
def rotation(aInputImage, dRotationAngle=30, bFit=True, tpBorderValue=(0, 0, 0)):
    ...
    if bFit:
        ...
        lNewX, lNewY = abs(np.sin(dAngle) * lNewY) + abs(np.cos(dAngle) * lNewX), abs(np.sin(dAngle) * lNewX) + abs(np.cos(dAngle) * lNewY)
        tx, ty = (lNewX - lOriginalX) / 2, (lNewY - lOriginalY) / 2
        M[(0, 2)] += tx
        M[(1, 2)] += ty
        aRotatedImg = cv2.warpAffine(aInputImage,
          M, dsize=(int(lNewX), int(lNewY)), borderValue=tpBorderValue)
    else:
        aRotatedImg = cv2.warpAffine(aInputImage,
          M, dsize=(lOriginalX, lOriginalY), borderValue=tpBorderValue)
    return aRotatedImg
...
def blend_image(aInputImage, aBGImage, dOpacity=1.0, sBlendMode='mul'):
    ...
    if aInputImage.shape != aBGImage.shape:
        raise Exception('Image and BackgrundImage have to be the same Size and color Model')
    else:
        aInputImageAlpha = __add_alpha_channel(aInputImage)
        aBGImageAlpha = __add_alpha_channel(aBGImage)
        dicBlendModes = {'sl':blend_modes.soft_light, 
    ...
    aBlendedImageAlpha = fnBlendMode(aBGImageAlpha.astype(float), aInputImageAlpha.astype(float), dOpacity)
...
def __resize_pattern(aInputImage, aPatternImage):
    ...
    if lTargetHeight < lPatternHeigh:
        if lTargetWidth < lPatternWidth:
    ...
    aMultipliedPattern = np.tile(aPatternImage, (lPatternHeighMultiplier, lPatternWidth, lPatternColorMultiplier))
...
def salt_and_pepper_noise(aInputImage, dAmount=0.001, dProportion=0.5):
    ...
    image = img_as_float(aInputImage)
    image = util.random_noise(aInputImage,
      's&p', amount=dAmount, salt_vs_pepper=dProportion)
    return img_as_ubyte(image)
...
def position_watermarking(aInputImage, aWatermarkImage, position='top_left', dOpacity=1.0, sBlendMode='mul'):
    ...
    if position == 'top_left':
        ...
        if position == 'bottom_left':
            aBlankImage[lTargetHeight - lWatermarkHeigh:lTargetHeight, 0:lWatermarkWidth] = aWatermarkImage
        else:
            if position == 'bottom_right':
                aBlankImage[lTargetHeight - lWatermarkHeigh:lTargetHeight, lTargetWidth - lWatermarkWidth:lTargetWidth] = aWatermarkImage
            else:
                if position == 'top_right':
                    aBlankImage[0:lWatermarkHeigh, lTargetWidth - lWatermarkWidth:lTargetWidth] = aWatermarkImage
                else:
                    if position == 'middle':
                        if lTargetHeight % 2 == 0 and lTargetWidth % 2 == 0:
                            aBlankImage = cv2.resize(aBlankImage, (lTargetHeight + 1, lTargetWidth + 1))
                        else:
                            if lTargetHeight % 2 == 0:
                                aBlankImage = cv2.resize(aBlankImage, (lTargetHeight + 1, lTargetWidth))
                            else:
                                if lTargetWidth % 2 == 0:
                                    aBlankImage = cv2.resize(aBlankImage, (lTargetHeight, lTargetWidth + 1))
                                if lWatermarkHeigh % 2 == 0 and lWatermarkWidth % 2 == 0:
                                    aWatermarkImage = cv2.resize(aWatermarkImage, (lWatermarkHeigh + 1, lWatermarkWidth + 1))
                                else:
                                    if lWatermarkHeigh % 2 == 0:
                                        aWatermarkImage = cv2.resize(aWatermarkImage, (lWatermarkHeigh + 1, lWatermarkWidth))
                                    else:
                                        if lWatermarkWidth % 2 == 0:
                                            aWatermarkImage = cv2.resize(aWatermarkImage, (lWatermarkHeigh, lWatermarkWidth + 1))
                                lCenterImageX, lCenterImageY = (aBlankImage.shape[0] + 1) / 2, (aBlankImage.shape[1] + 1) / 2
                                lCenterWatermarkX, lCenterWatermarkY = (aWatermarkImage.shape[0] + 1) / 2, (aWatermarkImage.shape[1] + 1) / 2
                                X1 = int(lCenterImageX - lCenterWatermarkX)
                                Y1 = int(lCenterImageY - lCenterWatermarkY)
                                aBlankImage[X1:X1 + aWatermarkImage.shape[0], Y1:Y1 + aWatermarkImage.shape[1]] = aWatermarkImage
