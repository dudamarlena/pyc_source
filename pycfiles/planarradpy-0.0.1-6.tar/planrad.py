...
# [GCC 8.4.0]
# Embedded file name: /home/marrabld/Projects/planarradpy/planarradpy/libplanarradpy/planrad.py
# Compiled at: 2015-06-10 23:47:54
import os, sys, time
...
import logger as log, scipy
from scipy import loadtxt
import libplanarradpy, libplanarradpy.state, csv
...
class RunParameters():
    ...
    def __init__(self, wavelengths):
        ...
        self.a = scipy.zeros_like(self.wavelengths)
        self.a_phi = scipy.zeros_like(self.wavelengths)
        self.a_water = scipy.zeros_like(self.wavelengths)
        self.a_cdom = scipy.zeros_like(self.wavelengths)
        self.bb = scipy.zeros_like(self.wavelengths)
        self.bbp = scipy.zeros_like(self.wavelengths)
        self.b_water = scipy.zeros_like(self.wavelengths)
        self.b = scipy.zeros_like(self.wavelengths)
        self.c = scipy.zeros_like(self.wavelengths)
        self.iop_backscatter_proportion_list = ''
        self.depth = 5
        ...
        self.pure_water_absorption_file = os.path.abspath(os.path.join(os.path.join(self.input_path, 'iop_files'), 'a_water.csv'))
        self.pure_water_scattering_file = os.path.abspath(os.path.join(os.path.join(self.input_path, 'iop_files'), 'b_water.csv'))
        self.phytoplankton_absorption_file = os.path.abspath(os.path.join(os.path.join(self.input_path, 'iop_files'), 'a_phi.csv'))
        ...
        self.water_surface_file = os.path.abspath(os.path.join(os.path.join(self.input_path, 'surface_files'), 'sf_' + self.iface_type + '_w' + str(self.wind_speed) + '_r' + str(self.rays_per_quad) + '_s' + str(self.surface_count) + '_' + self.azimuthally_average + '_mono_' + self.ds_code))
        self.wind_direc = 0
        self.crosswind_vertices = 100
        self.model_eqn = 'clear'
        self.phase_function_file = os.path.abspath(os.path.join(os.path.join(self.input_path, 'phase_files'), 'pf_' + self.iop_type + '_mono_' + self.ds_code))
    ...
    def write_run_parameters_to_file(self):
        ...
        f.write('bs_name = ' + str(len(self.wavelengths)) + ' Bands (' + str(self.wavelengths[0]) + '-' + str(self.wavelengths[(len(self.wavelengths) - 1)]) + ' nm) \n')
        ...
        f.write('\n')
        f.write('Ld_b_image_save_fp = ' + os.path.join(self.output_path, 'image_Ld_b.ppm') + '\n')
        f.write('Ld_b_image_sens_k = ' + str(self.ld_b_image_sens_k) + '\n')
        f.write('\n')
        f.write('Ld_b_save_fp = ' + os.path.join(self.output_path, 'Ld_b_data') + '\n')
        f.write('\n')
...
class BatchRun():
    ...
    def generate_directories(self, overwrite=False):
        ...
        if not os.path.exists(self.batch_output):
            ...
            if os.path.exists(self.batch_output) and overwrite == True:
                try:
                    lg.info('Creating batch project directory')
            ...
            self.bio_params.read_pure_water_absorption_from_file(self.run_params.pure_water_absorption_file)
            self.bio_params.read_pure_water_scattering_from_file(self.run_params.pure_water_scattering_file)
            ...
            for saa in self.saa_list:
                ...
                self.run_params.sky_file = os.path.abspath(os.path.join(os.path.join(self.run_params.input_path, 'sky_files'), 'sky_' + self.run_params.sky_state + '_z' + str(self.run_params.sky_zenith) + '_a' + str(self.run_params.sky_azimuth) + '_' + str(self.run_params.num_bands) + '_' + self.run_params.ds_code))
                                ...
                                for g in self.g_list:
                                    for s in self.s_list:
                                        for z in self.z_list:
                                            file_name = 'SAA' + str(saa) + '_SZA' + str(sza) + '_P' + str(p) + '_X' + str(x) + '_Y' + str(y) + '_G' + str(g) + '_S' + str(s) + '_Z' + str(z)
                                            ...
                                            self.bio_params.build_bbp(x, y)
                                            self.bio_params.build_a_cdom(g, s)
                                            self.bio_params.read_aphi_from_file(self.run_params.phytoplankton_absorption_file)
                                            self.bio_params.scale_aphi(p)
                                            self.bio_params.build_all_iop()
                                            self.run_params.scattering_file = os.path.join(os.path.join(self.run_params.input_path, 'iop_files'), 'b_' + file_name)
                                            self.bio_params.write_b_to_file(self.run_params.scattering_file)
                                            self.run_params.attenuation_file = os.path.join(os.path.join(self.run_params.input_path, 'iop_files'), 'c_' + file_name)
...
class ReportTools():
    ...
    def calc_directional_aop(self, report, parameter, parameter_dir):
        ...
        for i_iter in range(0, int(report['vn'][1])):
            tmp_zenith.append(report['Quad_solid_angle_mean_point_theta'][i_iter][:].split(',')[0])
        ...
        if parameter == 'rrs':
            tmp_rrs = lw / scipy.asarray(report['Ed_w'], dtype=float)[1:]
        elif parameter == 'Rrs':
            tmp_rrs = lw / scipy.asarray(report['Ed_a'], dtype=float)[1:]
