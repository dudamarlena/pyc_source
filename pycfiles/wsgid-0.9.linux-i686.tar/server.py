...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.5/site-packages/wsgid/server.py
# Compiled at: 2009-07-14 19:31:47
import os, sys, signal, simplejson
...
class Server(object):
    ...
    def __init__(self, conf):
        ...
        if conf.ssl_certificate or conf.ssl_private_key:
            if conf.ssl_certificate and conf.ssl_private_key:
                ...
                log.warn('Must provide both ssl_certificate and ssl_private_key for SSL support. Disabled.')
    ...
    def _pretty_display(self):
        ...
        opts.update(dict(app=self.conf.application or self.conf.application_factory or 'Hello World', reloader=not self.conf.no_reloader, ve=self.conf.virtualenv or 'None', ssl=len(self.conf.ssl_certificate and self.conf.ssl_private_key) > 0, h=self.conf.host == '0.0.0.0' and '*' or self.conf.host))
        self.log.info('Listening: %(h)s:%(port)s, app=%(app)r ssl=%(ssl)d, rld=%(reloader)d, dbg=%(debug)d, venv=%(ve)s' % opts)
    ...
    def _create_server(self, app):
        try:
            WSGIServer = import_string('wsgid.servers.%s:WSGIServer' % self.conf.server)
...
class ServerProcess(Server):

    def __init__(self, inpath, outpath):
        (fin, fout) = open_pipes(inpath, outpath)
        data = read_fd(fin)
        conf_data = simplejson.loads(data)
        conf = create_config(conf_data)
        Server.__init__(self, conf)
