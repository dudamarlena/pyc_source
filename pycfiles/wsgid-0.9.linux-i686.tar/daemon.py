...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.5/site-packages/wsgid/daemon.py
# Compiled at: 2009-07-14 19:37:51
"""
wsgid.daemon
~~~~~~~~~~~~
"""
import sys, os, time, atexit, signal, errno, tempfile, subprocess, logging
...
def signal_daemon(pidfile, signum):
    if is_daemon_running(pidfile):
        ...
        raise DaemonNotRunningError('No running daemon at pidfile %r.' % pidfile)
...
log_format = '%(asctime)s:%(levelname)8s:%(name)12s:%(process)d:%(message)s'
...
def create_logger(conf, name='daemon', filename='wsgid.log'):
    ...
    handler = handlers.RotatingFileHandler(os.path.join(conf.logdir, filename), maxBytes=200000, backupCount=20)
...
class WSGIDaemon(object):
    ...
    def _child_waited(self):
        self.log.debug('Server exited with code %s. ' % self.child.returncode)
        self.child = None
        self.child_pid = None
        return
    ...
    def _fork_server(self, server_config):
        ...
        p = subprocess.Popen([sys.executable, runner, '--reload-loop', outpath, inpath], stdout=sys.stdout, stderr=sys.stderr, stdin=subprocess.PIPE, env=os.environ.copy())
