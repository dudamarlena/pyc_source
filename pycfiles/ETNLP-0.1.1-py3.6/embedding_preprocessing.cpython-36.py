...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/api/embedding_preprocessing.py
# Compiled at: 2019-04-04 14:44:57
...
def convert_to_w2v(vocab_file, embedding_file, out_file):
    ...
    emb_model = gensim.models.KeyedVectors.load_word2vec_format(embedding_file, binary=is_binary,
...
def load_and_save_2_word2vec_models(input_embedding_files_str, output_embedding_files_str, embedding_config):
    ...
    if input_embedding_files_str.__contains__(';'):
        ...
        input_model_files = [
         input_embedding_files_str]
    if output_embedding_files_str.__contains__(';'):
        output_model_files = output_embedding_files_str.split(';')
    else:
        output_model_files = [
         output_embedding_files_str]
    assert len(output_model_files) == len(input_model_files), 'Number of input files and output files must be equal. Exiting ...'
    threads = []
