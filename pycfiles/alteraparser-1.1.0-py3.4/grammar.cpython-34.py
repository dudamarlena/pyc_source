...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/alteraparser/bnf/grammar.py
# Compiled at: 2015-12-30 07:08:12
# Size of source mod 2**32: 10368 bytes
...
star = single_char('*')
caret = single_char('^')
ampersand = token(single_char('&'), 'ampersand').transform_ast(lambda ast: AST('no-ws'))
...
cardinality = fork(question_mark, [
 optional(ampersand), plus], [
 optional(ampersand), star]).set_name('cardinality').transform_ast(cardinality_trnsf)
...
esc = fork([single_char('\\'), quote]).set_name('esc').transform_ast(esc_trnsf)
...
terminal = fork([quote,
 ...
 quote]).set_name('term').transform_ast(terminal_trnsf)
...
rule_name = fork([alpha,
 many(fork(alpha_num, fork([underscore, alpha_num])))]).set_name('rule_name').set_unique().transform_ast(rule_name_trnsf)
...
id_name = fork([alpha,
 many(fork(alpha_num, fork([underscore, alpha_num])))]).set_name('id_name').set_unique().transform_ast(id_trnsf)
...
def prod_rule_stmt(self, start, end):
    ...
    global whitespace
    global ws
    start > many(whitespace) > many(annotation_stmt().set_id('annot')) > fork(rule_name, ws) > many(whitespace) > assign.clone() > many(whitespace) > expr_stmt().set_id('rhs') > many(whitespace) > semicolon.clone() > end
...
def annotation_stmt(self, start, end):
    ...
    nl = single_char('\n')
    v = start.clone()
    annotations = fork(keyword('@grammar', name='grammar').set_id('annot'), keyword('@unique', name='unique').set_id('annot'))
    start > many(fork(wspace, nl)) > annotations > many(wspace) > nl.clone() > many(wspace) > v
    v > start
    v > end
...
def expr_stmt(self, start, end):
    global pipe
    start > branch_stmt().set_id('branch') > many(fork([
...
def branch_stmt(self, start, end):
    ...
    v = start.clone()
    start > fork([
     fork(ws, terminal, rule_name, range_stmt(), charset_stmt(), special_char, comp_stmt()),
     optional(fork([hash_char, id_name])),
     optional(cardinality)]).set_id('content') > v
    v > one_to_many(whitespace) > optional(fork([ampersand.set_id('content'),
     one_to_many(whitespace)])) > start
    v > one_to_many(whitespace) > optional(fork([opt_ws.set_id('content'),
     one_to_many(whitespace)])) > start
    v.connect(end)
...
def comp_stmt(self, start, end):
    global par_close
    global par_open
    start > par_open.clone() > optional(whitespace) > expr_stmt().set_id('expr') > optional(whitespace) > par_close.clone() > end
...
def charset_stmt(self, start, end):
    ...
    start > fork([
     ...
     one_to_many(fork(special_char.set_id('special'), no_bracket_close).set_id('char-element')),
     bracket_close]) > end
...
def option_stmt(self, start, end):
    start > keyword('set') > one_to_many(whitespace) > keyword('config.') > fork(keyword('case_sensitive')).set_id('name') > one_to_many(whitespace) > fork(keyword('on'), keyword('off')).set_id('value') > many(whitespace) > semicolon.clone() > end
...
bnf_grammar = grammar('bnf', [
 one_to_many(fork([
  many(whitespace),
  fork(option_stmt().set_id('grammar-element'), comment_stmt().set_id('grammar-element'), prod_rule_stmt().set_id('grammar-element')),
  many(whitespace)]))], bnf_grammar_trnsf)