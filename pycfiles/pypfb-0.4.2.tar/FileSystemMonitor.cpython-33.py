...
# Embedded file name: build/bdist.linux-x86_64/egg/printflow2/FileSystemMonitor.py
# Compiled at: 2014-04-03 10:38:31
# Size of source mod 2**32: 71276 bytes
__doc__ = '\nCreated on Sep 9, 2013\n\n@author: "Colin Manning"\n'
import os.path, uuid, time, traceback, json, logging, smtplib
...
class FileSystemMonitor(pyinotify.ProcessEvent):
    ...
    ignore_files = [
     'New folder', '.DS_Store', 'untitled folder', 'approved', 'failed', '.dropbox']
    created_files = {}
    modified_files = {}
    wdd = None
    wm = None
    ...
    def __init__(self, config_file, workgroup_id, upload_via):
        ...
        if os.path.exists(config_file):
            ...
            self.pdf_toolbox = PdfToolbox(self.config_data['pdftoolbox'], self.config_data['pdftoolbox_cache_path'], self.config_data['activationcode'], self.config_data['pdftoolbox_tmpdir'], self.web_rootdir)
            self.workflow_manager = WorkflowManager(self.logger)
            if 'workflows' in self.workgroup:
                self.workflows = self.workgroup['workflows']
                for workflowName in self.workflows:
                    self.workflow_manager.registerWorkflow(self.workflows[workflowName])
    ...
    def walk(self, f):
        for dirname, dirnames, filenames in os.walk(f):
            for filename in filenames:
                ...
                if not parent_dir == 'approved':
                    if not parent_dir == 'failed':
                        if not parent_dir == 'MisplacedFiles':
                            pass
                if not filename[0] == '.':
                    if not filename[0] == '_':
                        if len(filename_bits) > 1 and filename_bits[(-1)] == 'json':
                            continue
            ...
            try:
                ...
                for index in range(len(path_bits) - 1, 0, -1):
                    ...
                    if path_bits[index] in self.workgroup['clients']:
                        ...
                        if path_bits[index] == 'Projects' and index > 0:
                    ...
                    if client_name is not None:
                        try:
                            ...
                            utils.ensureDirectoryExistsForUser(misplacedFilesPath, self.os_userid, self.os_groupid, 511)
                            ...
                            if 'flowdock_accesstoken' in client:
                                self.inform_flowdock_spurious(client['flowdock_accesstoken'], client_name, file_sub_path, file_name)
                            else:
    ...
    def upload_simple_file_to_cumulus(self, client, file_path, file_name, file_pathname, project, spec, guid, uploaded_by):
        ...
        try:
            try:
                ...
                data = {'name': new_file_name,  'profile': 'Standard',  'fulcrum_PF Original Name': file_object['original_name'], 
                ...
                if response.status_code == 200:
                    ...
                    noosh_file_id = self.post_file_to_noosh(client, self.workgroup['id'], project['noosh_project']['project_id'], spec['noosh_specification']['spec_id'], file_object, uploaded_by)
    ...
    def process_file(self, file_path, file_name, file_pathname):
        if os.path.isdir(file_pathname):
            ...
            if ext is 'pdf' or ext is 'PDF':
                ...
                elif parent_dir == 'approved' or parent_dir == 'failed' or parent_dir == 'MisplacedFiles' or file_name[0] == '.' or file_name[0] == '_' or len(filename_bits) > 1 and filename_bits[(-1)] == 'json':
                    return
                try:
                    ...
                    client = None
                    project_details = project['noosh_project']
                    if 'client_account' in project_details and project_details['client_account'] is not None and project_details['client_account'] in workgroup['clients'] and 'users' in workgroup['clients'][project_details['client_account']]:
                        client = workgroup['clients'][project_details['client_account']]
                    else:
                        if 'identify_filter_key' in workgroup and 'custom_fields' in project_details:
                            ...
                            if len(workgroup['clients']) == 1:
                                client = next(iter(workgroup['clients'].values()))
                            if client is None:
                                print((' - abort processing file', file_pathname, ', no client account available'))
                                return
                            else:
                                is_pdf_file or self.process_simple_file(workgroup, project, specification, client, file_path, file_name, file_pathname)
                                return
                            ...
                            if status:
                                ...
                                db_file, db_file_path = self.upload_file_to_cumulus(client, file_path, file_name, file_pathname, project, specification, '2', info, output, guid, report_file_text, report_file_html, uploaded_by)
                                proj = project['noosh_project']
                                spec = specification['noosh_specification']
                                ...
                                proj = project['noosh_project']
                                spec = specification['noosh_specification']
                                db_file, db_file_path = self.register_failed_file(client, file_path, file_name, file_pathname, project, specification, status, info, output, guid, report_file_text, report_file_html, uploaded_by)
                                ...
                                if 'flowdock_accesstoken' in workgroup['clients'][client['name']]:
                                    self.inform_flowdock_rejected(workgroup['clients'][client['name']]['flowdock_accesstoken'], proj, spec, db_file, client)
    ...
    def inform_flowdock_uploaded(self, accesstoken, proj, spec, db_file, client):
        subject = 'File uploaded for ' + client['name'] + ' - Project ' + proj['project_name'] + ' (' + str(proj['project_id']) + ')'
        message = ' <h3>A file has been uploaded for "%s"</h3> <div id="wrap" style="border-top-style:solid;border-top-width:1px;border-top-color:#666666;border-bottom-style:solid;border-bottom-width:1px;border-bottom-color:#666666"><div style="float:left;margin: 15 15 15 15"> <table> <tr><td>Client:</td><td>%s</td></tr> <tr><td>Project Name:</td><td>%s</td></tr> <tr><td>Project Id:</td><td>%s</td></tr> <tr><td>Specification Name:</td><td>%s</td></tr> <tr><td>Specification Id:</td><td>%s</td></tr> <tr><td>Specification Reference:</td><td>%s</td></tr> <tr><td>Original File Name:</td><td>%s</td></tr> <tr><td>Production Name:</td><td>%s</td></tr> <tr><td>Size:</td><td>%s bytes</td></tr> <tr><td>Upload Time:</td><td>%s</td></tr> <tr><td>Pre-flight Status:</td><td>%s</td></tr> <tr><td>View pre-flight report:</td><td><a href="%s" target="_blank">here</a></td></tr> </table></div> <div style="float: left;margin-bottom:15;margin-top:15;border-style:solid;border-width:1px;border-color:#666666"> <div style="margin:15 15 15 15"><a href="%s"><img src="%s"></a></div></div><br style="clear:both" /></div>' % (
         spec['spec_name'], client['name'], proj['project_name'], str(proj['project_id']),
        ...
        data = {'source': 'PrintFlow 2 Service',  'from_address': 'colin@printoutsource.com',  'subject': subject, 
         'content': message,  'project': str(proj['project_id']), 
         'tags': [
                  '@all', '#project', str(proj['project_id']), str(spec['spec_id']), str(db_file['original_name'])]}
        d = json.dumps(data)
    ...
    def inform_flowdock_rejected(self, accesstoken, proj, spec, db_file, client):
        subject = 'File rejected for ' + client['name'] + ' - Project ' + proj['project_name'] + ' (' + str(proj['project_id']) + ')'
        message = ' <h3>A file has been rejected for "%s"</h3> <div id="wrap" style="border-top-style:solid;border-top-width:1px;border-top-color:#666666;border-bottom-style:solid;border-bottom-width:1px;border-bottom-color:#666666"><div style="float:left;margin: 15 15 15 15"> <table> <tr><td>Client:</td><td>%s</td></tr> <tr><td>Project Name:</td><td>%s</td></tr> <tr><td>Project Id:</td><td>%s</td></tr> <tr><td>Specification Name:</td><td>%s</td></tr> <tr><td>Specification Id:</td><td>%s</td></tr> <tr><td>Specification Reference:</td><td>%s</td></tr> <tr><td>Original File Name:</td><td>%s</td></tr> <tr><td>Production Name:</td><td>%s</td></tr> <tr><td>Size:</td><td>%s bytes</td></tr> <tr><td>Upload Time:</td><td>%s</td></tr> <tr><td>Pre-flight Status:</td><td>%s</td></tr> <tr><td>View HTML Report:</td><td><a href="%s" target="_blank">here</a></td></tr> <tr><td>View Text_Report Report:</td><td><a href="%s" target="_blank">here</a></td></tr> </table> </div> <div style="float: left;margin-bottom:15;margin-top:15;border-style:solid;border-width:1px;border-color:#666666"> <div style="margin:15 15 15 15"><a href="%s"><img src="%s"></a></div> </div><br style="clear:both" /></div>' % (
         spec['spec_name'], client['name'], proj['project_name'], str(proj['project_id']),
         spec['spec_name'], str(spec['spec_id']), str(spec['reference_number']),
         db_file['original_name'], db_file['dam_name'], str(db_file['size']),
         utils.parse_time(db_file['upload_time'], utils.STYLE_DATE_TIME_SHORT_MONTH), db_file['preflight_info'],
        ...
        data = {'source': 'PrintFlow 2 Service',  'from_address': 'colin@printoutsource.com',  'subject': subject, 
         'content': message,  'project': str(proj['project_id']), 
         'tags': [
                  '@all', '#project', str(proj['project_id']), str(spec['spec_id'])]}
        d = json.dumps(data)
        headers = {'Content-Type': 'application/json'}
        response = requests.post(post_url, data=d, headers=headers)
        if response.status_code == 200:
    ...
    def inform_flowdock_spurious(self, accesstoken, client_name, file_path, file_name):
        subject = 'Oops file ' + file_name + ' incorrectly uploaded for ' + client_name
        message = ' <div style="float:left;margin: 15 15 15 15"> <p>File <b>%s</b> for %s was uploaded to the wrong place</p> <p>Files should only be placed in specification folders</p> <p>The file was uploaded to: <b>%s</b></p> <p>The file has been moved to: <b>/%s/MisplacedFiles</b></p> </div>' % (
         file_name, client_name, file_path, client_name)
        post_url = 'https://api.flowdock.com/v1/messages/team_inbox/' + accesstoken
        data = {'source': 'PrintFlow 2 Service',  'from_address': 'printflow@printoutsource.com',  'subject': subject, 
         'content': message,  'tags': [
                  '@all', '#misplaced_file', file_name]}
        d = json.dumps(data)
        headers = {'Content-Type': 'application/json'}
        response = requests.post(post_url, data=d, headers=headers)
        if response.status_code == 200:
    ...
    def get_email_spurious_body(self, client_name, file_path, file_name):
        return '<div><div style="float:left;margin: 15 15 15 15"> <p>File <b>%s</b> for %s was uploaded to the wrong place</p> <p>Files should only be placed in specification folders</p> <p>The file was uploaded to: <b>%s</b></p> <p>The file has been moved to: <b>/%s/MisplacedFiles</b></p> </div><br style="clear:both" /></div>' % (
         file_name, client_name, file_path, client_name)

    def get_email_file_uploaded_body(self, project, specification, db_file, client):
        proj = project['noosh_project']
        spec = specification['noosh_specification']
        return ' <h3>A file has been uploaded for "%s"</h3> <div id="wrap" style="border-top-style:solid;border-top-width:1px;border-top-color:#666666;border-bottom-style:solid;border-bottom-width:1px;border-bottom-color:#666666"><div style="float:left;margin: 15 15 15 15"> <table> <tr><td>Uploaded By:</td><td>%s</td></tr> <tr><td>Client:</td><td>%s</td></tr> <tr><td>Project Name:</td><td>%s</td></tr> <tr><td>Project Id:</td><td>%s</td></tr> <tr><td>Specification Name:</td><td>%s</td></tr> <tr><td>Specification Id:</td><td>%s</td></tr> <tr><td>Specification Reference:</td><td>%s</td></tr> <tr><td>Original File Name:</td><td>%s</td></tr> <tr><td>Production Name:</td><td>%s</td></tr> <tr><td>Size:</td><td>%s bytes</td></tr> <tr><td>Upload Time:</td><td>%s</td></tr> <tr><td>Pre-flight Status:</td><td>%s</td></tr> <tr><td>View pre-flight report:</td><td><a href="%s" target="_blank">here</a></td></tr> </table></div> <div style="float: left;margin-bottom:15;margin-top:15;border-style:solid;border-width:1px;border-color:#666666"> <div style="margin:15 15 15 15"><a href="%s"><img src="%s"></a></div></div><br style="clear:both" /></div>' % (
    ...
    def get_email_file_rejected_body(self, proj, spec, db_file, client):
        return ' <h3>A file has been rejected for "%s"</h3> <div id="wrap" style="border-top-style:solid;border-top-width:1px;border-top-color:#666666;border-bottom-style:solid;border-bottom-width:1px;border-bottom-color:#666666"><div style="float:left;margin: 15 15 15 15"> <table> <tr><td>Uploaded By:</td><td>%s</td></tr> <tr><td>Client:</td><td>%s</td></tr> <tr><td>Project Name:</td><td>%s</td></tr> <tr><td>Project Id:</td><td>%s</td></tr> <tr><td>Specification Name:</td><td>%s</td></tr> <tr><td>Specification Id:</td><td>%s</td></tr> <tr><td>Specification Reference:</td><td>%s</td></tr> <tr><td>Original File Name:</td><td>%s</td></tr> <tr><td>Production Name:</td><td>%s</td></tr> <tr><td>Size:</td><td>%s bytes</td></tr> <tr><td>Upload Time:</td><td>%s</td></tr> <tr><td>Pre-flight Status:</td><td>%s</td></tr> <tr><td>View HTML Report:</td><td><a href="%s" target="_blank">here</a></td></tr> <tr><td>View Text_Report Report:</td><td><a href="%s" target="_blank">here</a></td></tr> </table> </div> <div style="float: left;margin-bottom:15;margin-top:15;border-style:solid;border-width:1px;border-color:#666666"> <div style="margin:15 15 15 15"><a href="%s"><img src="%s"></a></div> </div><br style="clear:both" /></div>' % (
         spec['spec_name'], db_file['uploaded_by'], client['name'], proj['project_name'], str(proj['project_id']),
         spec['spec_name'], str(spec['spec_id']), str(spec['reference_number']),
         db_file['original_name'], db_file['dam_name'], str(db_file['size']),
         utils.parse_time(db_file['upload_time'], utils.STYLE_DATE_TIME_SHORT_MONTH), db_file['preflight_info'],
    ...
    def build_file_web_page(self, proj, spec, db_file, client):
        html = ' <html><head><title>%s File Information</title></head><body> <h3>A file has been uploaded for "%s"</h3> <div id="wrap" style="border-top-style:solid;border-top-width:1px;border-top-color:#666666;border-bottom-style:solid;border-bottom-width:1px;border-bottom-color:#666666"><div style="float:left;margin: 15 15 15 15"> <table> <tr><td>Uploaded By:</td><td>%s</td></tr> <tr><td>Client:</td><td>%s</td></tr> <tr><td>Project Name:</td><td>%s</td></tr> <tr><td>Project Id:</td><td>%s</td></tr> <tr><td>Specification Name:</td><td>%s</td></tr> <tr><td>Specification Id:</td><td>%s</td></tr> <tr><td>Specification Reference:</td><td>%s</td></tr> <tr><td>Original File Name:</td><td>%s</td></tr> <tr><td>Production Name:</td><td>%s</td></tr> <tr><td>Size:</td><td>%s bytes</td></tr> <tr><td>Upload Time:</td><td>%s</td></tr> <tr><td>Pre-flight Status:</td><td>%s</td></tr> <tr><td>View HTML Report:</td><td><a href="%s" target="_blank">here</a></td></tr> <tr><td>View Text_Report Report:</td><td><a href="%s" target="_blank">here</a></td></tr> </table> </div> <div style="float: left;margin-bottom:15;margin-top:15;border-style:solid;border-width:1px;border-color:#666666"> <div style="margin:15 15 15 15"><a href="%s"><img src="%s"></a></div> </div><br style="clear:both" /></div>' % (
    ...
    def upload_file_to_cumulus(self, client, file_path, file_name, file_pathname, project, spec, status, info, output, guid, report_file_text, report_file_html, uploaded_by):
        self.logger.info('Uploading file:' + file_pathname + ' to Cumulus')
        file_object = None
        try:
            try:
                                    ...
                                    for fv in f:
                                        if fv['param_name'] == m_map_bits[1]:
                                            if fv['string_value'] is not None:
                                                ...
                                                if fv['number_value'] is not None:
    ...
    def register_failed_zip_file(self, client, file_path, file_name, file_pathname, project, spec, guid, uploaded_by):
        self.logger.info('Registering failed (zip) file:' + file_pathname)
        file_object = None
        db_file_path = None
        try:
            try:
                ...
                utils.ensureDirectoryExistsForUser(failed_path, self.os_userid, self.os_groupid, 511)
                preview_path = utils.get_guid_path(self.web_rootdir, file_object['guid'])
                utils.ensureDirectoryExistsForUser(preview_path, self.os_userid, self.os_groupid, 511)
                ...
                file_object['thumbnail_url'] = thumbnail_url
                file_object['noosh_file_id'] = 0
                utils.ensureDirectoryExistsForUser(spec['dropbox_path'] + '/failed', self.os_userid, self.os_groupid, 511)
                if file_exists:
                    db_file_path = self.jds.update(self.FILE_CLASS, file_object)
                else:
                    db_file_path = self.jds.create(self.FILE_CLASS, file_object)
    ...
    def register_failed_file(self, client, file_path, file_name, file_pathname, project, spec, status, info, output, guid, report_file_text, report_file_html, uploaded_by):
    ...
    def pdf_preview(self, file_path, file_name, file_pathname, destination):
        ...
        try:
            ...
            status, output, preview_file, preview_file_name = self.pdf_toolbox.run_preview(self.workgroup['serial_number'], file_path, file_name, destination)
    ...
    def preflight_check(self, file_path, file_name, file_pathname, project, spec, guid):
        ...
        try:
            ...
            status, output, report_folder, report_file_text, report_file_html = self.pdf_toolbox.run_job(self.workgroup['serial_number'], file_path, file_name, self.workgroup['preflight_profile'], guid)
            if status == 0:
                ok = True
        ...
        return (
         ok, info, output, report_folder, report_file_text, report_file_html)
