...
# [GCC 8.4.0]
# Embedded file name: /private/tmp/test/lib/python2.7/site-packages/pyrowire/resources/console_scripts/console_scripts.py
...
def parse_args():
    ...
    parser.add_argument('--init', dest='init', default=None, action='store_true', help='initialize stub files for pyrowire development')
    parser.add_argument('--deploy-heroku', dest='deploy_heroku', action='store_true', default=None, help='deploy current pyrowire project to heroku')
    parser.add_argument('--add-heroku-redis', dest='add_heroku_redis', action='store_true', default=None, help='add redis to an existing heroku deployment')
...
def init():
    ...
    copied_files = []
    for f in [ x for x in os.listdir(source_path) if not re.match('^\\.|^_|.*pyc$', x) ]:
    ...
    if install:
        if re.search('darwin', sys.platform):
            ...
            if brew:
                ...
                abort(_red('Attempted to install heroku-toolbelt with brew, but brew was not found. Please install homebrew and try again, or install the heroku-toolbelt manually from https://toolbelt.heroku.com'))
...
def heroku_select_or_create_app(account_name):
    ...
    app = prompt(_white('Type the name of the application you wish to use, or press Enter to create a new one:\n\t%s%s\nSelection: ' % (
...
def heroku_add_redis(app=None, account=None):
    ...
    if 'redistogo' not in redis_exists:
    ...
    if open_redis_details:
        prompted = prompt(_yellow("Once the redis details open, you will be prompted to enter some details. You can type 'q' at any time to cancel updating your application's Redis details.Press Enter to open the Redis details page."))
        if prompted != 'q':
    ...
    if '2' in is_staging:
        profile = 'prod'
    settings_file = [ i for i in os.listdir('./') if re.search('settings', i) and not re.search('.*\\.pyc$', i) ][0]
    settings_path = os.path.join('./', settings_file)
    f = open(settings_path, 'r')
    settings_lines = [ i.replace('<%s.redis_host>' % profile, redis_host).replace('<%s.redis_port>' % profile, str(redis_port)).replace('<%s.redis_password>' % profile, redis_password) for i in f.readlines()
                     ]
...
def heroku_deploy():
    if not heroku_remote_exists():
        ...
        add_redis = prompt(_white("Type 'y' to add Redis To Go to your Heroku application (to use an external service, update your settings file): ")) or None
        ...
        print _green('You are ready to commit any changes and deploy to heroku by typing: %s' % _white('git push heroku <name_of_branch>'))
        return
    else:
