...
class AEDIRWelcomeMailJob(aedir.process.AEProcess):
    ...
    def _welcome_notifications(self, last_run_timestr, current_run_timestr):
        ...
        """
        nopassword_filterstr = FILTERSTR_NO_WELCOME_YET.format(currenttime=current_run_timestr,
          lasttime=last_run_timestr,
          serverid=(self.server_id))
        self.logger.debug('User search filter: %r', nopassword_filterstr)
        ...
        with self.smtp_connection(SMTP_URL,
          local_hostname=SMTP_LOCALHOSTNAME,
          ca_certs=SMTP_TLS_CACERTS,
          debug_level=SMTP_DEBUGLEVEL) as (smtp_conn):
            for ldap_res in ldap_results:
                to_addr = ldap_res.entry_s['mail'][0]
                self.logger.debug('Prepare welcome notification for %r sent to %r', ldap_res.dn_s, to_addr)
                msg_attrs = {'ldap_uri':str(self.ldap_conn.ldap_url_obj.connect_uri()), 
            ...
            try:
                admin_res = self.ldap_conn.read_s(admin_dn,
                  filterstr=FILTERSTR_USER,
                  attrlist=(self.admin_attrs))
            except (ldap0.NO_SUCH_OBJECT, ldap0.INSUFFICIENT_ACCESS) as ldap_err:
                try:
                    self.logger.warning('Error reading admin entry %r referenced by %r: %s', admin_dn, ldap_res.dn_s, ldap_err)
                    admin_entry = {}
                finally:
                    ldap_err = None
                    del ldap_err
                ...
                if admin_res is None:
                    self.logger.warning('Empty result reading admin entry %r referenced by %r', admin_dn, ldap_res.dn_s)
                else:
                    self.logger.debug('Read admin entry %r: %r', admin_dn, admin_res.entry_s)
