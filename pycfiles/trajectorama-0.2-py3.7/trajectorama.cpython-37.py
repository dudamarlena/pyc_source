...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/trajectorama/trajectorama.py
# Compiled at: 2020-01-31 12:39:05
# Size of source mod 2**32: 11792 bytes
import numpy as np, os, random, scipy
...
def transform(X, studies, X_dimred=None, log_transform=LOG_TRANSFORM, corr_method=CORR_METHOD, corr_cutoff=CORR_CUTOFF, cluster_method=CLUSTER_METHOD, min_cluster_samples=MIN_CLUSTER_SAMPLES, seed=SEED, n_jobs=N_JOBS, verbose=VERBOSE):
    """
    ...
    X = _check_X(X)
    _check_params(X, studies, X_dimred, corr_method, corr_cutoff, cluster_method, min_cluster_samples, n_jobs)
    ...
    study_idxs = [np.where(studies == study)[0] for study in uniq_studies]
    if log_transform:
        X = X.log1p()
    if X_dimred is None:
        X_dimred = np.concatenate([reduce_dimensionality(normalize(X[study_idx])) if X.shape[1] < 100 else X[study_idx].toarray() for study_idx in study_idxs])
    if verbose:
        tprint('Performing panresolution clustering...')
    cds = [PanDAG(cluster_method=cluster_method, verbose=verbose).fit(X_dimred[study_idx]) for study_idx in study_idxs]
    if verbose:
    ...
    Xs_coexpr = fill_correlations(X,
      sample_idxs, corr_method=corr_method,
...
def _check_X(X):
    ...
    if issubclass(type(X), np.ndarray):
        ...
        if issubclass(type(X), scipy.sparse.csr.csr_matrix):
            X = X
        else:
            raise TypeError('Dataset must be numpy array or scipy.sparse.csr_matrix, received type {}.\n'.format(type(ds)))
            exit(1)
    return X


def _check_params(X, studies, X_dimred, corr_method, corr_cutoff, cluster_method, min_cluster_samples, n_jobs):
    """
    Basic parameter checking.
    """
    if X.shape[0] != len(studies):
        raise ValueError('Number of samples in X ({}) does not match length of studies ({})'.format(X.shape[0], len(studies)))
    if X_dimred is not None:
        if X_dimred.shape[0] != X.shape[0]:
            raise ValueError('Number of samples in X ({}) does not match number of samples in X_dimred ({})'.format(X.shape[0], X_dimred.shape[0]))
    valid_corr_methods = {
     'pearson', 'spearman'}
    if corr_method not in valid_corr_methods:
        raise ValueError('Invalid corr_method {}, must be one of {}'.format(corr_method, valid_corr_methods))
    if corr_cutoff < -1.0 or corr_cutoff > 1.0:
        raise ValueError('Invalid corr_cutoff {}, must be between -1 and 1'.format(corr_cutoff))
    valid_cluster_methods = {'louvain', 'agg_ward'}
    if cluster_method not in valid_cluster_methods:
        raise ValueError('Invalid cluster_method {}, must be one of {}'.format(cluster_method, valid_cluster_methods))
    if min_cluster_samples < 0:
        raise ValueError('Invalid min_cluster_samples of {}, must be nonnegative'.format(min_cluster_samples))
    if min_cluster_samples < 100:
        warnings.warn('Using less than 100 samples to estimate correlations may be less robust to noise.')
    if n_jobs < 1:
        raise ValueError('Invalid n_jobs of {}, must be positive'.format(n_jobs))
...
def pearson_multi(X):
    ...
    corr = EmpiricalCovariance(store_precision=False,
...
def spearman_multi(X):
    ...
    with warnings.catch_warnings():
        ...
        corr, corr_scores = spearmanr(X,
          nan_policy='omit')
    return corr


def _corr_worker(X, verbose, node_idx, n_nodes, corr_method, cutoff):
    """
    ...
    with warnings.catch_warnings():
        ...
        if corr_method == 'pearson':
            ...
            if corr_method == 'spearman':
                ...
                raise ValueError('Invalid correlation method {}'.format(corr_method))
    corr[np.isnan(corr)] = 0.0
    corr[np.abs(corr) < cutoff] = 0
    if verbose > 1:
        tprint('Filled node {} out of {} with {} samples and {} nonzero correlations'.format(node_idx + 1, n_nodes, X.shape[0], np.count_nonzero(corr)))
    return corr


def fill_correlations(X, sample_idxs, corr_method=CORR_METHOD, corr_cutoff=CORR_CUTOFF, n_jobs=N_JOBS, verbose=VERBOSE):
    """
    ...
    results = Parallel(n_jobs=n_jobs)((delayed(_corr_worker)(X[sample_idx].toarray(), verbose, node_idx, n_nodes, corr_method, corr_cutoff) for node_idx, sample_idx in enumerate(sample_idxs)))
    return results