...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/trajectorama/_louvain.py
# Compiled at: 2020-04-22 11:12:38
...
def louvain(adata: AnnData, resolution: Optional[float]=None, random_state: Optional[Union[(int, RandomState)]]=0, log_fname: str='', restrict_to: Optional[Tuple[(str, Sequence[str])]]=None, key_added: Optional[str]='louvain', adjacency: Optional[spmatrix]=None, flavor: str='vtraag', directed: bool=True, use_weights: bool=False, partition_type: Optional[Type[MutableVertexPartition]]=None, partition_kwargs: Optional[Mapping[(str, Any)]]=None, copy: bool=False) -> Optional[AnnData]:
    ...
    if adjacency is None:
        ...
        if restrict_to is not None:
            ...
            adjacency, restrict_indices = restrict_adjacency(adata, restrict_key, restrict_categories, adjacency)
 ...
 g, partition_type, log_fname=log_fname, **partition_kwargs)
            else:
    ...
    if restrict_to is not None:
        if key_added == 'louvain':
            key_added += '_R'
        groups = rename_groups(adata, key_added, restrict_key, restrict_categories, restrict_indices, groups)
