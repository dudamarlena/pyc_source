...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/easyexplore/data_explorer.py
# Compiled at: 2020-05-10 11:25:16
# Size of source mod 2**32: 67813 bytes
...
class DataExplorer:
    ...
    def __init__(self, df: pd.DataFrame, target: str=None, seed: int=1234, include: List[str]=None, exclude: List[str]=None, ordinal: List[str]=None, date: List[str]=None, date_edges: Tuple[(str, str)]=None, feature_types: Dict[(str, List[str])]=None, include_nan: bool=True, plot: bool=True, plot_type: str=None, file_path: str=None):
        """
        ...
        if df.shape[0] < 25:
            ...
            self.seed = seed if seed > 0 else 1234
            self.cpu_cores = os.cpu_count() - 1 if os.cpu_count() > 1 else 1
            self.max_cats = 500
            self.df = df
            if include is None:
            ...
            self.n_cases = self.df.shape[0]
            self.n_features = self.df.shape[1]
            self.data_types = self.df.dtypes.tolist()
            self.data_index = self.df.index.values.tolist()
            self.date_edges = date_edges
            self.date = [] if date is None else date
            self.ordinal = [] if ordinal is None else ordinal
            if feature_types is None:
                self.feature_types = self._check_data_types()
            else:
        ...
        self.invalid_values = ['nan', 'NaN', 'NaT', 'none', 'None', 'inf', '-inf']
        self.df = self.df.replace(self.invalid_values, np.nan)

    def _check_anomalies(self, meth: List[str], outliers_fraction: float=0.15, contour: bool=False) -> dict:
        """
        ...
        for m in meth:
            if m == 'abod':
                ...
                _classifiers[m] = FeatureBagging(base_estimator=None, n_estimators=10,
                ...
                if m == 'if':
                    _classifiers[m] = IForest(n_estimators=100, max_samples='auto',
                      contamination=outliers_fraction,
                      max_features=1,
                      bootstrap=True,
                      n_jobs=1,
                    ...
                    if m == 'knn':
                        _classifiers[m] = KNN(contamination=outliers_fraction, n_neighbors=5,
                          method='largest',
                          radius=1.0,
                          algorithm='auto',
                          leaf_size=30,
        ...
        _df = self.df[self.feature_types['continuous']].dropna(axis=0)
        _min = [_df[ft].min() for ft in self.feature_types.get('continuous')]
        _max = [_df[ft].max() for ft in self.feature_types.get('continuous')]
        for i, cont in enumerate(self.feature_types.get('continuous')):
            if i == 0:
                _array = _df[cont].values.reshape(-1, 1)
            else:
                _array = np.concatenate((_array, _df[cont].values.reshape(-1, 1)), axis=1)

        for clf in _classifiers.keys():
            _xx, _yy = np.meshgrid(np.linspace(min(_min), max(_max), _array.shape[0]), np.linspace(min(_min), max(_max), _array.shape[0]))
            _classifiers[clf].fit(_array)
            _scores_pred = _classifiers[clf].decision_function(_array) * -1
            _xx, _yy = np.meshgrid(np.linspace(min(_scores_pred), max(_scores_pred), _array.shape[0]), np.linspace(min(_scores_pred), max(_scores_pred), _array.shape[0]))
            _threshold = stats.scoreatpercentile(_scores_pred, 100 * outliers_fraction)
            _pred = _classifiers[clf].predict(_array)
            if contour:
            ...
            _outlier_detector.update({clf: dict(pred=_pred, outliers=_outliers,
                    anomaly_score=_z,
    ...
    def _check_data_types(self) -> Dict[(str, List[str])]:
        ...
        """
        return Utils().get_feature_types(df=(self.df), features=(self.features),
    ...
    def _check_outliers(self, meth: str='iqr', outlier_threshold: float=0.15) -> List[int]:
        ...
        """
        _cases = []
        _o = outlier_threshold if (outlier_threshold > 0 and outlier_threshold < 1) else 0.15
        if meth == 'iqr':
    ...
    def _nan_case_summary(self, nan_case_df: pd.Series) -> pd.DataFrame:
        ...
        _df = pd.DataFrame(data=[_df_freq_bin, _df_freq_rel_bin])
        _df = _df.rename(index={0:'N',  1:'%'}, columns={0:'Cases containing 0 % Missings', 
    ...
    def break_down(self, include_cat: bool=True, plot_type: str='violin') -> dict:
        ...
        _features = self.feature_types.get('categorical') + self.feature_types.get('ordinal')
        ...
        if self.plot:
            DataVisualizer(title='Breakdown Statistics', df=(self.df),
              ...
              file_path=(self.file_path)).run()
        return _break_down_stats

    def cor(self, marginal: bool=True, partial: bool=True, marginal_meth: str='pearson', min_obs: int=1, decimals: int=2) -> dict:
        """
        ...
        """
        _cor = dict(marginal={}, partial={}, diff={})
        _cor_plot = {}
        _min_obs = min_obs if min_obs > 0 else 1
        _decimals = decimals if decimals > 0 else 2
        if partial:
            _ar = self.df[self.feature_types.get('continuous')].dropna().values
            if _ar.shape[0] == 0:
                ...
                _cor_plot.update({'Partial Correlation': dict(data=_partial_cor_matrix, features=(self.feature_types.get('continuous')),
                                          plot_type='heat',
                                          kwargs=dict(layout={}))})
        if marginal:
            if marginal_meth not in ('pearson', 'kendall', 'spearman'):
                _features = self.feature_types.get('continuous')
                _cor_matrix = self.df[_features].corr(method='pearson', min_periods=_min_obs).round(decimals=_decimals)
                ...
                if marginal_meth == 'pearson':
                    _features = self.feature_types.get('continuous')
                    _cor_matrix = self.df[_features].corr(method=marginal_meth, min_periods=_min_obs).round(decimals=_decimals)
                else:
                    _features = self.feature_types.get('ordinal')
                    _cor_matrix = self.df[_features].corr(method=marginal_meth, min_periods=_min_obs).round(decimals=_decimals)
            _cor['marginal'].update(_cor_matrix.to_dict())
            _cor_plot.update({'Marginal Correlation': dict(data=_cor_matrix, features=_features,
                                       plot_type='heat',
                                       kwargs=dict(layout={}))})
        if partial:
            if marginal:
                _diff = _cor_matrix - _partial_cor_matrix
                _cor['diff'].update(_diff.to_dict())
                _cor_plot.update({'Difference: Partial vs. Marginal Correlation': dict(data=_cor_matrix, features=(self.feature_types.get('continuous')),
                                                                   plot_type='heat',
                                                                   kwargs=dict(layout={}))})
        if self.plot:
            DataVisualizer(subplots=_cor_plot, plot_type='heat',
              interactive=True,
              height=500,
              width=500,
              render=(True if self.file_path is None else False),
    ...
    def data_distribution(self, categorical: bool=True, continuous: bool=True, over_time: bool=False, grouping: bool=True) -> dict:
        """
        ...
        _subplots = {}
        _distribution = {}
        _supported_cat_plot_types = [
         'bar', 'pie']
        _supported_conti_plot_types = ['box', 'histo', 'violin']
        if categorical:
            _categorical_features = self.feature_types.get('ordinal') + self.feature_types.get('categorical')
            ...
            _subplots.update({'Categorical Features': dict(data=(self.df), features=_categorical_features,
                                       plot_type='bar',
                                       melt=False)})
        if continuous:
            _desc = self.df[self.feature_types.get('continuous')].describe(percentiles=PERCENTILES).to_dict()
            _norm = StatsUtils(data=(self.df), features=(self.feature_types.get('continuous'))).normality_test(alpha=0.05, meth='shapiro-wilk')
            _skew = StatsUtils(data=(self.df), features=(self.feature_types.get('continuous'))).skewness_test(axis='col')
            _annotations = []
            for ft in _desc.keys():
                _annotations.append(dict(text=('Mean={}<br></br>Median={}<br></br>Std={}<br></br>Normality:{}<br></br>Skewness:{}'.format(self.df[ft].mean(), self.df[ft].median(), self.df[ft].std(), _norm.get(ft), _skew.get(ft))), align='left',
                  showarrow=False,
                  x=0.5,
                  y=0.9,
                  xref='paper',
            ...
            _subplots.update({'Continuous Features': dict(data=(self.df), features=(self.feature_types.get('continuous')),
                                      plot_type='hist',
                                      melt=False)})
        if over_time:
            if len(self.feature_types.get('date')) == 0:
                ...
                _subplots.update({'Distribution over Time': dict(data=(self.df), features=(self.feature_types.get('continuous')),
                                             time_features=(self.feature_types.get('date')),
                                             plot_type='ridgeline')})
        if self.plot:
            DataVisualizer(title='Data Distribution', subplots=_subplots,
              interactive=True,
              height=500,
              width=500,
              render=(True if self.file_path is None else False),
    ...
    def data_health_check(self, sparsity: bool=True, invariant: bool=True, duplicate_cases: bool=False, duplicate_features: bool=True, outliers_univariate: bool=False, nan_heat_map: bool=True, nan_threshold: float=0.95, other_mis: list=None) -> Dict[(str, list)]:
        """
        ...
        _tables = {}
        _subplots = {}
        _nan_threshold = 100 * nan_threshold
        _data_health = {'sparsity':{'cases':[],  'features':[]},  'invariant':[],  'duplicate':{'cases':[],  'features':[]}}
        if not sparsity:
            if not invariant:
                if not duplicate_cases:
                    if not duplicate_features:
                        if not outliers_univariate:
        ...
        if sparsity:
            ...
            _data_health['sparsity']['cases'] = list(Utils().subset_dict(d=(_mis_analysis['cases']['rel']), threshold=_nan_threshold).keys())
            ...
            _info_table['sparsity_cases'] = '{} ({} %)'.format(_nan_cases, str(100 * round(_nan_cases / self.n_cases, 3)))
            _info_table['sparsity_features'] = '{} ({} %)'.format(_nan_features, str(100 * round(_nan_features / self.n_features, 3)))
            ...
            if self.plot:
                ...
                _df_feature_mis = _df_feature_mis.rename(columns={'abs':'N',  'rel':'%'}, index=Utils().replace_dict_keys(d=(_mis_analysis['features']['rel']), new_keys=(self.features)))
                if any(self.df.isnull()):
                    _df_case_mis = pd.DataFrame(data=(_mis_analysis['cases']))
                    _df_all_data = self.df.applymap(lambda x: 1 if x == x else 0)
                    _df_nan_case_sum = self._nan_case_summary(nan_case_df=(_df_case_mis['rel']))
                    _subplots.update({'Sparsity of data set':dict(data=_df_nan_case_sum, features=[], plot_type='pie',
                       kwargs=dict(values=[_df_all_data.sum().sum(), self.n_cases * self.n_features - _df_all_data.sum().sum()], labels=[
                      'Valid Data', 'Missing Data'])), 
                     'Missing Data Distribution Case-wise':dict(data=_df_nan_case_sum, features=[], plot_type='pie',
                       kwargs=dict(values=(_df_nan_case_sum.loc[:, 'N'].values.tolist()), labels=(_df_nan_case_sum.loc[:, 'N'].index.values.tolist()))), 
                     'Missing Data Distribution Features-wise':dict(data=_df_feature_mis, features=[], plot_type='pie',
                       kwargs=dict(values=(_df_feature_mis.loc[:, 'N'].values.tolist()), labels=(_df_feature_mis.loc[:, 'N'].index.values.tolist()))), 
                     'Sparsity of the features':dict(data=_df_feature_mis, features=[], plot_type='table',
                       kwargs=dict(index_title='Features with Missing Data'))})
                    if nan_heat_map:
                        _subplots.update({'Missing Data Heatmap': dict(data=(self.df), features=[], plot_type='heat',
                                                   kwargs=dict(z=(self.df.isnull().astype(int).values), colorbar=dict(title='Value Range', tickvals=[
                                                  '0', '1'],
                                                   ticktext=[
                                                  'Valid',
                                                  'Missing'])))})
        if invariant:
            ...
            if self.plot:
                ...
                _subplots.update({'Invariant Features': dict(data=pd.DataFrame(data=dict(features=(self.features), invariant=_label)), features=[
                                        'invariant'],
                                         plot_type='pie')})
        if duplicate_features:
            ...
            if self.plot:
                ...
                _subplots.update({'Duplicate Cases':dict(data=pd.DataFrame(data=dict(duplicate_cases=_duplicate_cases)), features=[
                  'duplicate_cases'],
                   plot_type='pie',
                   interactive=True), 
                 'Duplicate Features':dict(data=pd.DataFrame(data=dict(features=(self.features), duplicate_features=_duplicate_features)), features=[
                  'duplicate_features'],
                   plot_type='pie',
                   interactive=True)})
        if outliers_univariate:
            ...
            if self.plot:
                _outlier_cases = [
                 'outlier'] * _a + ['inlier'] * (self.n_cases - _a)
                _subplots.update({'Cases containing univariate outliers': dict(data=pd.DataFrame(data=dict(uni_outlier=_outlier_cases)), features=[
                                                          'uni_outlier'],
                                                           plot_type='pie')})
        ...
        if self.plot:
            _results_after_cleaning = pd.DataFrame(columns=(self.features), index=(self.data_index))
            ...
            _summary = pd.DataFrame(data=(_info_table.values()), columns=['N (%)'], index=_index)
            _subplots.update({'Data Structure Describing Data Health':dict(data=_results_after_cleaning, features=[], plot_type='heat',
               kwargs=dict(z=(_results_after_cleaning.values), colorbar=dict(title='Value Range', tickvals=[
              '0', '1', '2', '3'],
               ticktext=[
              'Valid', 'Missing', 'Invariant', 'Duplicate']))), 
             'Data Health Check Summary':dict(data=_summary, features=[], plot_type='table',
               kwargs=dict(index_title='Attributes of Data Health'))})
            DataVisualizer(title='Data Health Check', subplots=_subplots,
              interactive=True,
              width=500,
              height=500,
              render=(True if self.file_path is None else False),
              file_path=(self.file_path)).run()
        return dict(cases=(list(set(_cases))), features=(list(set(_features))))
    ...
    def data_typing(self) -> dict:
        ...
        for i in range(0, len(self.features), 1):
            if any(self.df[self.features[i]].isnull()):
                ...
                if str(self.data_types[i]).find('bool') >= 0:
                    ...
                    if self.features[i] in self.feature_types.get('date'):
                        ...
                        if self.features[i] in self.feature_types.get('ordinal'):
                            ...
                            if self.features[i] in self.feature_types.get('continuous'):
                                if any(self.df[self.features[i]].isnull()):
                                    _table['feature_type'].append('float')
                ...
                if str(self.data_types[i]).find('object') >= 0:
                    if self.features[i] in self.feature_types.get('date'):
        ...
        if self.plot:
            _df = pd.DataFrame(data=_table)
            if _df.shape[0] == 0:
                Log(write=False).log('All feature and data types are correct')
            else:
                _df = _df.rename(columns={'feature_type':'Feature Type',  'data_type':'Data Type',  'rec':'Recommendation'}, index={idx:name for idx, name in zip(range(len(_feature)), _feature)})
                _kwargs = dict(index_title='Features')
                DataVisualizer(title='Data Type Check', df=_df, 
                 plot_type='table', 
                 interactive=True, 
                 height=500, 
                 width=500, 
                 render=True if self.file_path is None else False, 
                 file_path=self.file_path, **_kwargs).run()
        return _typing
    ...
    def geo_stats(self, geo_features: List[str], lat: str=None, lon: str=None, minimum: bool=True, maximum: bool=True, mean: bool=True, std: bool=True, perc: List[float]=None) -> dict:
        """
        ...
        for geo in geo_features:
            ...
            for val in self.df[geo].unique():
                ...
                if val in self.invalid_values:
                    _df = self.df.loc[self.df[geo].isnull(), :]
                else:
                    _df = self.df.loc[self.df[geo] == val, :]
        ...
        if self.plot:
            if lat is None:
                ...
                elif lon not in self.df.keys():
                    raise DataExplorerException('Longitude feature not found in data set')
            DataVisualizer(title='Geo Statistics', subplots={}, interactive=True,
              height=500,
              width=500,
              render=(True if self.file_path is None else False),
              file_path=(self.file_path)).run()
    ...
    def outlier_detector(self, kind: str='uni', multi_meth: List[str]=None, contour: bool=False) -> Dict[(str, List[int])]:
        """
        ...
        if kind is 'uni':
            ...
            _subplots.update({'Univariate Outlier Detection': dict(data=(self.df), features=(self.feature_types.get('continuous')),
                                               plot_type='violin',
                                               melt=True,
            ...
            if kind in ('bi', 'multi'):
                ...
                _multi_meth = ['knn'] if multi_meth is None else multi_meth
                _anomaly_detection = self._check_anomalies(meth=_multi_meth)
                for meth in _multi_meth:
                    ...
                    if contour:
                        _multi = {'contour': dict(x=(_anomaly_detection[meth].get('space')), y=(_anomaly_detection[meth].get('space')),
                        ...
                        _multi = {}
                    if len(self.features) == 2:
                        _multi.update({'scatter_inlier':dict(x=self.df.loc[(self.df['outlier'] == 0, self.features[0])].values, y=self.df.loc[(self.df['outlier'] == 0, self.features[1])].values,
                           mode='markers',
                           name='inlier',
                           marker=dict(color='rgba(255, 255, 255, 1)') if contour else dict(color='rgba(107, 142, 35, 1)'),
                           hoverinfo='text',
                           showlegend=False if contour else True), 
                         'scatter_outlier':dict(x=self.df.loc[(self.df['outlier'] == 1, self.features[0])].values, y=self.df.loc[(self.df['outlier'] == 1, self.features[1])].values,
                           mode='markers',
                           name='outlier',
                           marker=dict(color='rgba(0, 0, 0, 1)') if contour else dict(color='rgba(178, 34, 34, 1)'),
                           hoverinfo='text',
                           showlegend=False if contour else True)})
                        _subplots.update({'Multivariate Outlier Detection': dict(data=(self.df), features=(self.feature_types.get('continuous')),
                                                             plot_type='multi',
                                                             kwargs=dict(multi=_multi, layout=dict(xaxis=dict(title=(self.features[0])),
                                                             yaxis=dict(title=(self.features[1])))))})
                    else:
                        _pairs = Utils().get_pairs(features=(self.features), max_features_each_pair=2)
                        for i, pair in enumerate(_pairs):
                            _multi.update({'scatter_inlier_{}'.format(i): dict(x=(self.df.loc[(self.df['outlier'] == 0, pair[0])].values), y=(self.df.loc[(self.df['outlier'] == 0, pair[1])].values),
                                                              mode='markers',
                                                              name=('inlier ({} | {})'.format(pair[0], pair[1])),
                                                              marker=(dict(color='rgba(255, 255, 255, 1)') if contour else dict(color='rgba(107, 142, 35, 1)')),
                                                              showlegend=True), 
                             
                             'scatter_outlier_{}'.format(i): dict(x=(self.df.loc[(self.df['outlier'] == 1, pair[0])].values), y=(self.df.loc[(self.df['outlier'] == 1, pair[1])].values),
                                                               mode='markers',
                                                               name=('outlier ({} | {})'.format(pair[0], pair[1])),
                                                               marker=(dict(color='rgba(0, 0, 0, 1)') if contour else dict(color='rgba(178, 34, 34, 1)')),
                                                               showlegend=True)})
                            if kind is 'bi':
                                DataVisualizer(title='Bivariate Outlier Detection', df=self.df, 
                                 plot_type='multi', 
                                 interactive=True, 
                                 height=500, 
                                 width=500, 
                                 render=True if self.file_path is None else False, 
                                 file_path=None if self.file_path is None else os.path.join(self.file_path, '{}_{}'.format(pair[0], pair[1])), **dict(multi={'scatter_inlier_{}'.format(i): _multi.get('scatter_inlier_{}'.format(i)), 
                                 'scatter_outlier_{}'.format(i): _multi.get('scatter_outlier_{}'.format(i))},
                                  layout=dict(xaxis=dict(title=(pair[0])), yaxis=dict(title=(pair[1]))))).run()
        ...
        if self.plot:
            if kind is not 'bi':
                DataVisualizer(subplots=_subplots, interactive=True,
                  height=500,
                  width=500,
                  render=(True if self.file_path is None else False),
                  file_path=(self.file_path)).run()
...
class MissingDataAnalysis:
    ...
    def __init__(self, data: np.array, features: List[str], other_mis: list=None):
        """
