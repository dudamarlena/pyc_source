...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.4-x86_64/egg/business_tools/business_id.py
# Compiled at: 2011-07-05 05:12:41
"""
Finnish business id support.
"""
import re
business_id_re = re.compile('^(FI|)(([0-9]{6,8})|([0-9]{6,7}-[0-9]{1}))$', re.VERBOSE)
...
class BusinessID(object):
    ...
    def is_valid(business_id):
        ...
        if not business_id_re.match(unicode(business_id)):
            ...
            if not business_id or len(business_id) == 0 or len(business_id) > 7 or len(checksum_digit) != 1:
                return False
    ...
    def remove_country_code(business_id):
        ...
        """
        if isinstance(business_id, basestring) and not business_id[0:2].isdigit():
