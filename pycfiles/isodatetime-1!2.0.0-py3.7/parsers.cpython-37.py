...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/isodatetime/parsers.py
# Compiled at: 2019-01-30 07:03:09
# Size of source mod 2**32: 25810 bytes
...
class ISO8601SyntaxError(ValueError):
    __doc__ = 'An error denoting invalid input syntax.'
...
class StrptimeConversionError(ValueError):
    __doc__ = 'An error denoting bad conversion from a strftime/strptime format.'
    ...
    def parse(self, expression):
        ...
        for regex in self.RECURRENCE_REGEXES:
            ...
            if 'reps' in result_map:
                if result_map['reps'] is not None:
            ...
            if 'intv' in result_map:
                duration = self.duration_parser.parse(result_map['intv'])
            return data.TimeRecurrence(repetitions=repetitions,
...
class TimePointParser(object):
    ...
    def _generate_regexes(self):
        ...
        for format_type in format_ok_keys:
            ...
            for date_key in date_map[format_type].keys():
                ...
                for date_expr in self.get_expressions(date_map[format_type][date_key]):
                    date_regex = self.parse_date_expression_to_regex(date_expr)
            ...
            for time_key in time_map[format_type].keys():
                ...
                for time_expr in self.get_expressions(time_map[format_type][time_key]):
                    time_regex = self.parse_time_expression_to_regex(time_expr)
                    regex_list.append([re.compile(time_regex), time_expr])

            for time_zone_expr in self.get_expressions(time_zone_map[format_type]):
                time_zone_regex = self.parse_time_zone_expression_to_regex(time_zone_expr)
    ...
    def parse_date_expression_to_regex(self, expression):
        ...
        for expr_regex, substitute, _, _ in parser_spec.get_date_translate_info(self.expanded_year_digits):
    ...
    def parse_time_expression_to_regex(expression):
        ...
        for expr_regex, substitute, _, _ in parser_spec.get_time_translate_info():
            expression = re.sub(expr_regex, substitute, expression)
    ...
    def parse_time_zone_expression_to_regex(expression):
        ...
        for expr_regex, substitute, _, _ in parser_spec.get_time_zone_translate_info():
            expression = re.sub(expr_regex, substitute, expression)
    ...
    def parse(self, timepoint_string, dump_format=None, dump_as_parsed=False):
        ...
        return self._create_timepoint_from_info(date_info,
          time_info, dump_format=dump_format, truncated_dump_format=dump_format)

    def _create_timepoint_from_info(self, date_info, time_info, dump_format=None, truncated_dump_format=None):
        info = {}
        ...
        if date_info.get('truncated'):
            ...
            for property_ in ('year', 'year_of_decade', 'century', 'year_of_century',
                              'expanded_year', 'year_sign'):
        ...
        for key, value in list(time_info.items()):
            ...
            if key == 'time_zone_utc':
                if value == 'Z':
                    time_info.pop(key)
                    time_info.update({'time_zone_hour':0,  'time_zone_minute':0})
                    continue
            time_info[key] = value
    ...
    def strptime(self, strptime_data_string, strptime_format_string, dump_format=None):
        ...
        """
        split_format = parser_spec.REC_SPLIT_STRFTIME_DIRECTIVE.split(strptime_format_string)
        regex = '^'
        ...
        regex += '$'
        return self._parse_from_custom_regex(regex,
          strptime_data_string, dump_format=dump_format,
          source=strptime_format_string)

    def _parse_from_custom_regex(self, regex, data_string, dump_format=None, source=None):
        ...
        date_info_keys = []
        for item in parser_spec.get_date_translate_info(self.expanded_year_digits):
    ...
    def get_info(self, timepoint_string):
        ...
        date_time_time_zone = timepoint_string.split(parser_spec.TIME_DESIGNATOR)
        parsed_expr = ''
        if len(date_time_time_zone) == 1:
            ...
            time_info = {}
            time_zone_info = self.process_time_zone_info({})
            time_info.update(time_zone_info)
        else:
            date, time_time_zone = date_time_time_zone
            if (date or self).allow_truncated:
                keys = (None, 'truncated', '')
                date_info = {'truncated': True}
            else:
                keys, date_info = self.get_date_info(date, bad_types=[
                 'reduced'])
            format_key, type_key, date_expr = keys
            parsed_expr += date_expr
            bad_formats = []
            ...
            if time_time_zone.endswith('Z'):
                ...
                if '+' in time_time_zone:
                    ...
                    if '-' in time_time_zone:
                        ...
                        time_zone = '-' + time_zone
                        try:
                            time_expr, time_info = self.get_time_info(time,
                              bad_formats=bad_formats,
                              bad_types=bad_types)
                            time_zone_expr, time_zone_info = self.get_time_zone_info(time_zone,
                              bad_formats=bad_formats)
            ...
            if time_zone is None:
                ...
                time_zone_info = self.process_time_zone_info(time_zone_info)
            time_expr, time_info = self.get_time_info(time,
              bad_formats=bad_formats, bad_types=bad_types)
            parsed_expr += parser_spec.TIME_DESIGNATOR + (time_expr + time_zone_expr)
            time_info.update(time_zone_info)
        return (
         date_info, time_info, parsed_expr)
    ...
    def process_time_zone_info(self, time_zone_info=None):
        ...
        if time_zone_info is None:
            ...
            if not time_zone_info:
                if self.assumed_time_zone is None:
                    if self.default_to_unknown_time_zone:
                        return {}
                    utc_hour_offset, utc_minute_offset = timezone.get_local_time_zone()
            ...
            if time_zone_info.pop('time_zone_sign', '+') == '-':
                time_zone_info['time_zone_hour'] = -int(time_zone_info['time_zone_hour'])
                if 'time_zone_minute' in time_zone_info:
                    time_zone_info['time_zone_minute'] = -int(time_zone_info['time_zone_minute'])
        return time_zone_info


class DurationParser(object):
    __doc__ = 'Parser for ISO 8601 Durations (durations).'
    DURATION_REGEXES = [
     re.compile('^P(?:(?P<years>\\d+)Y)?\n                   (?:(?P<months>\\d+)M)?\n                   (?:(?P<days>\\d+)D)?$', re.X),
     re.compile('^P(?:(?P<years>\\d+)Y)?\n                   (?:(?P<months>\\d+)M)?\n                   (?:(?P<days>\\d+)D)?\n                   T(?:(?P<hours>\\d.*)H)?\n                   (?:(?P<minutes>\\d.*)M)?\n                   (?:(?P<seconds>\\d.*)S)?$', re.X),
     re.compile('^P(?P<weeks>\\d+)W$', re.X)]
    ...
    def parse(self, expression):
        ...
        if expression.startswith('P'):
            if sign_factor != -1:
                try:
                    timepoint = parse_timepoint_expression((expression[1:]),
                      allow_truncated=False, assumed_time_zone=(0, 0))
                except ISO8601SyntaxError:
