...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/isodatetime/data.py
# Compiled at: 2019-03-06 05:14:39
# Size of source mod 2**32: 95112 bytes
...
class Calendar(object):
    __doc__ = 'Store constants for Gregorian calendar date-time calculation.'
    SECONDS_IN_MINUTE = 60
    MINUTES_IN_HOUR = 60
    HOURS_IN_DAY = 24
    DAYS_IN_WEEK = 7
    DAYS_IN_MONTHS = None
    DAYS_IN_MONTHS_LEAP = None
    ROUGH_DAYS_IN_MONTH = 30
    LEAP_YEAR_FACTOR_TRUTHS = [
     (4, True), (100, False), (400, True)]
    MODE_360 = '360day'
    MODE_365 = '365day'
    MODE_366 = '366day'
    MODE_GREGORIAN = 'gregorian'
    MODES = {MODE_360: (12 * [30], None), 
    ...
    WEEK_DAY_START_REFERENCE = {'calendar':(2000, 1, 3), 
     'ordinal':(2000, 3)}
    UNIX_EPOCH_DATE_TIME_REFERENCE_PROPERTIES = {'year':1970, 
    ...
    def set_mode(self, mode=None):
        ...
        self.INDEXED_DAYS_IN_MONTHS = [(i + 1, days) for i, days in enumerate(self.DAYS_IN_MONTHS)]
        self.INDEXED_DAYS_IN_MONTHS_LEAP = [(i + 1, days) for i, days in enumerate(self.DAYS_IN_MONTHS_LEAP)]
        self.REVERSED_INDEXED_DAYS_IN_MONTHS = reversed(self.INDEXED_DAYS_IN_MONTHS)
        self.REVERSED_INDEXED_DAYS_IN_MONTHS_LEAP = reversed(self.INDEXED_DAYS_IN_MONTHS)
        ...
        self.MINUTES_IN_YEAR_LEAP = self.DAYS_IN_YEAR_LEAP * self.MINUTES_IN_DAY
        self.SECONDS_IN_YEAR_LEAP = self.DAYS_IN_YEAR_LEAP * self.SECONDS_IN_DAY
...
TIMEPOINT_DUMPER_MAP = {0:dumpers.TimePointDumper(num_expanded_year_digits=0), 
 2:dumpers.TimePointDumper(num_expanded_year_digits=2)}

class BadInputError(ValueError):
    __doc__ = 'An error raised when constructor inputs are invalid.'
...
class TimeRecurrence(object):
    __doc__ = 'Represent a recurring duration.'
    __slots__ = ('repetitions', 'start_point', 'duration', 'end_point', 'min_point',
                 'max_point', 'format_number')

    def __init__(self, repetitions=None, start_point=None, duration=None, end_point=None, min_point=None, max_point=None):
        inputs = (
         (
        ...
        if self.duration is None:
            ...
            if self.repetitions == 1:
                ...
                diff_days_float = diff_days / float(self.repetitions - 1)
                diff_seconds_float = diff_seconds / float(self.repetitions - 1)
                diff_days = int(diff_days_float)
                diff_seconds_float += (diff_days_float - diff_days) * CALENDAR.SECONDS_IN_DAY
            ...
            if self.end_point is None:
                ...
                if self.repetitions is not None:
                    self.end_point = self.start_point + self.duration * (self.repetitions - 1)
            elif self.start_point is None:
                self.format_number = 4
                if self.repetitions is not None:
                    self.start_point = self.end_point - self.duration * (self.repetitions - 1)
            else:
                raise BadInputError(BadInputError.RECURRENCE, [i[:2] for i in inputs])
    ...
    def get_next(self, timepoint):
        ...
        if not self.repetitions == 1:
            if timepoint is None:
                return
            next_timepoint = timepoint + self.duration
            if self._get_is_in_bounds(next_timepoint):
                return next_timepoint
        elif self.format_number == 1:
            if next_timepoint > self.end_point:
                diff = next_timepoint - self.end_point
                if 2 * diff < self.duration:
                    if self._get_is_in_bounds(self.end_point):
    ...
    def __getitem__(self, index):
        if not (index < 0 or isinstance(index, int)):
    ...
    def _get_is_in_bounds(self, timepoint):
        ...
        if timepoint is None:
            ...
            if self.start_point is not None:
                if timepoint < self.start_point:
                    return False
            if self.min_point is not None:
                if timepoint < self.min_point:
                    return False
        else:
            if self.max_point is not None:
                if timepoint > self.max_point:
                    return False
    ...
    def __iter__(self):
        if self.start_point is None:
            ...
            in_reverse = False
        if not (self.repetitions == 1 or self.duration):
...
class Duration(object):
    ...
    def __init__(self, years=0, months=0, weeks=0, days=0, hours=0.0, minutes=0.0, seconds=0.0, standardize=False):
        ...
        if not self.years:
            if not (self.months or self.hours or self.minutes or self.seconds):
                if weeks:
                    if not days:
        ...
        if standardize:
            if self.seconds:
                num_minutes, self.seconds = divmod(self.seconds, CALENDAR.SECONDS_IN_MINUTE)
                if self.minutes is None:
                    self.minutes = 0
                self.minutes += num_minutes
            if self.minutes:
                num_hours, self.minutes = divmod(self.minutes, CALENDAR.MINUTES_IN_HOUR)
                if self.hours is None:
                    self.hours = 0
                self.hours += num_hours
            if self.hours:
                num_days, self.hours = divmod(self.hours, CALENDAR.HOURS_IN_DAY)
    ...
    def copy(self):
        ...
        return Duration(years=(self.years),
          months=(self.months),
          weeks=(self.weeks),
          days=(self.days),
          hours=(self.hours),
    ...
    def get_days_and_seconds(self):
        ...
        new = self.copy()
        new.to_days()
        new_days = new.years * CALENDAR.ROUGH_DAYS_IN_YEAR + new.months * CALENDAR.ROUGH_DAYS_IN_MONTH + new.days
        new_seconds = new.hours * CALENDAR.SECONDS_IN_HOUR + new.minutes * CALENDAR.SECONDS_IN_MINUTE + new.seconds
    ...
    def to_days(self):
        ...
        if self.get_is_in_weeks():
            for attribute in ('years', 'months', 'hours', 'minutes', 'seconds'):
    ...
    def __add__(self, other):
        ...
        if isinstance(other, TimePoint):
            return other + new
        raise TypeError('Invalid type for addition: ' + "'%s' should be Duration or TimePoint." % type(other).__name__)
    ...
    def __mul__(self, other):
        if not isinstance(other, int):
            raise TypeError('Invalid type for multiplication: ' + "'%s' should be integer." % type(other).__name__)
        new = self.copy()
    ...
    def __floordiv__(self, other):
        if not isinstance(other, int):
            raise TypeError('Invalid type for division: ' + "'%s' should be integer." % type(other).__name__)
        new = self.copy()
        if self.get_is_in_weeks():
            new.weeks //= other
            return new
    ...
    def __str__(self):
        ...
        for prop_, unit in (('years', 'Y'), ('months', 'M'), ('days', 'D'), ('hours', 'H'),
                            ('minutes', 'M'), ('seconds', 'S')):
...
class TimeZone(Duration):
    ...
    def copy(self):
        """Return an unlinked copy of this instance."""
        return TimeZone(hours=(self.hours), minutes=(self.minutes), unknown=(self.unknown))
...
class TimePoint(object):
    ...
    def __init__(self, expanded_year_digits=0, year=None, month_of_year=None, week_of_year=None, day_of_year=None, day_of_month=None, day_of_week=None, hour_of_day=None, hour_of_day_decimal=None, minute_of_hour=None, minute_of_hour_decimal=None, second_of_minute=None, second_of_minute_decimal=None, time_zone_hour=None, time_zone_minute=None, dump_format=None, truncated=False, truncated_dump_format=None, truncated_property=None, is_empty_instance=False):
        if is_empty_instance:
            return
            if dump_format is not None:
                if not isinstance(dump_format, str):
                    raise BadInputError(BadInputError.TYPE, 'dump_format', repr(dump_format), type(dump_format))
            if truncated_dump_format is not None:
                if not isinstance(truncated_dump_format, str):
                    raise BadInputError(BadInputError.TYPE, 'truncated_dump_format', repr(truncated_dump_format), type(truncated_dump_format))
            if truncated_property is not None:
                if truncated_property not in ('year_of_decade', 'year_of_century'):
                    raise BadInputError(BadInputError.VALUES, 'truncated_property', repr(truncated_property), "'year_of_decade' or 'year_of_century'")
            _type_checker((
            ...
            self.expanded_year_digits = _int_caster(expanded_year_digits, 'expanded_year_digits')
            ...
            self.month_of_year = _int_caster(month_of_year, 'year', allow_none=True)
            self.day_of_year = _int_caster(day_of_year, 'day_of_year', allow_none=True)
            self.day_of_month = _int_caster(day_of_month, 'day_of_month', allow_none=True)
            self.day_of_week = _int_caster(day_of_week, 'day_of_week', allow_none=True)
            self.week_of_year = _int_caster(week_of_year, 'week_of_year', allow_none=True)
            self.hour_of_day = _int_caster(hour_of_day, 'hour_of_day', allow_none=True)
            if hour_of_day_decimal is not None:
                if self.hour_of_day is None:
                    raise BadInputError(BadInputError.MISSING, 'hour_of_day_decimal', 'hour_of_day')
                else:
                    self.hour_of_day += float(hour_of_day_decimal)
                    if minute_of_hour is not None:
                        raise BadInputError(BadInputError.CONFLICT, 'minute_of_hour', 'hour_of_day_decimal')
                    if second_of_minute is not None:
                        raise BadInputError(BadInputError.CONFLICT, 'second_of_minute', 'hour_of_day_decimal')
                    if minute_of_hour_decimal is not None:
                        if minute_of_hour is None:
                            raise BadInputError(BadInputError.MISSING, 'minute_of_hour_decimal', 'minute_of_hour')
                        else:
                            self.minute_of_hour = _int_caster(minute_of_hour, 'minute_of_hour')
                            self.minute_of_hour += float(minute_of_hour_decimal)
                            if second_of_minute is not None:
                                raise BadInputError(BadInputError.CONFLICT, 'second_of_minute', 'minute_of_hour_decimal')
                            else:
                                self.minute_of_hour = _int_caster(minute_of_hour,
                                  'minute_of_hour', allow_none=True)
                        if second_of_minute_decimal is not None:
                            if second_of_minute is None:
                                raise BadInputError(BadInputError.MISSING, 'second_of_minute_decimal', 'second_of_minute')
                            self.second_of_minute = _int_caster(second_of_minute, 'second_of_minute')
                ...
                if not self.truncated:
                    if self.hour_of_day is None:
                        self.hour_of_day = 0
            elif hour_of_day_decimal is None and self.minute_of_hour is None:
                self.minute_of_hour = 0
            if hour_of_day_decimal is None:
                if minute_of_hour_decimal is None:
            ...
            if time_zone_hour is not None:
                ...
                self.time_zone.hours = _int_caster(time_zone_hour, 'time_zone_hour')
            if time_zone_minute is not None:
                has_unknown_tz = False
                self.time_zone.minutes = _int_caster(time_zone_minute, 'time_zone_minute')
        else:
    ...
    def get_calendar_date(self):
        ...
        if self.get_is_ordinal_date():
            return get_calendar_date_from_ordinal_date(self.year, self.day_of_year)
        if self.get_is_week_date():
            return get_calendar_date_from_week_date(self.year, self.week_of_year, self.day_of_week)
    ...
    def get_ordinal_date(self):
        ...
        if self.get_is_calendar_date():
            return get_ordinal_date_from_calendar_date(self.year, self.month_of_year, self.day_of_month)
        if self.get_is_ordinal_date():
            return (
             self.year, self.day_of_year)
        if self.get_is_week_date():
            return get_ordinal_date_from_week_date(self.year, self.week_of_year, self.day_of_week)
    ...
    def get(self, property_name):
        ...
        if property_name == 'minute_of_hour_decimal_string':
            string = '%f' % (float(self.minute_of_hour) - int(self.minute_of_hour))
            string = string.replace('0.', '', 1).rstrip('0')
            if not string:
                return '0'
            return string
        ...
        if property_name == 'second_of_minute_decimal_string':
            string = '%f' % (float(self.second_of_minute) - int(self.second_of_minute))
            string = string.replace('0.', '', 1).rstrip('0')
            if not string:
                return '0'
            return string
        ...
        if property_name == 'time_zone_sign' and not self.time_zone.hours < 0:
            if self.time_zone.minutes < 0:
                return '-'
            return '+'
            if property_name == 'seconds_since_unix_epoch':
                reference_timepoint = TimePoint(**CALENDAR.UNIX_EPOCH_DATE_TIME_REFERENCE_PROPERTIES)
    ...
    def get_week_date(self):
        ...
        if self.get_is_calendar_date():
            return get_week_date_from_calendar_date(self.year, self.month_of_year, self.day_of_month)
        if self.get_is_ordinal_date():
            return get_week_date_from_ordinal_date(self.year, self.day_of_year)
        if self.get_is_week_date():
            return (
             self.year, self.week_of_year, self.day_of_week)

    def apply_time_zone_offset(self, offset):
        """Apply a time zone shift represented by a Duration."""
        if offset.minutes:
            if self.minute_of_hour is None:
                self.hour_of_day += offset.minutes / float(CALENDAR.MINUTES_IN_HOUR)
            else:
    ...
    def get_largest_truncated_property_name(self):
        ...
        for attr in ('year_of_century', 'year_of_decade', 'month_of_year', 'week_of_year',
    ...
    def get_smallest_missing_property_name(self):
        ...
        if not self.truncated:
            return
        prop_dict = self.get_truncated_properties()
        attr_list = (('year_of_century', 'century'), ('year_of_decade', 'decade_of_century'),
                     ('month_of_year', 'year_of_century'), ('week_of_year', 'year_of_century'),
                     ('day_of_year', 'year_of_century'), ('day_of_month', 'month_of_year'),
                     ('day_of_week', 'week_of_year'), ('hour_of_day', 'day_of_month'),
                     ('minute_of_hour', 'hour_of_day'), ('second_of_minute', 'minute_of_hour'))
    ...
    def get_truncated_properties(self):
        ...
        for attr in ('month_of_year', 'week_of_year', 'day_of_year', 'day_of_month',
    ...
    def add_truncated(self, year_of_century=None, year_of_decade=None, month_of_year=None, week_of_year=None, day_of_year=None, day_of_month=None, day_of_week=None, hour_of_day=None, minute_of_hour=None, second_of_minute=None):
        """Combine this TimePoint with truncated time properties."""
        new = self.copy()
        if hour_of_day is not None:
            if minute_of_hour is None:
                minute_of_hour = 0
        if hour_of_day is not None or minute_of_hour is not None:
            if second_of_minute is None:
                second_of_minute = 0
    ...
    def __add__(self, other, no_copy=False):
        if isinstance(other, TimePoint):
            if self.truncated:
                if not other.truncated:
                    new = self.copy()
                    new_other = other.copy()
                    prev_time_zone = new_other.get_time_zone()
                    new_other.set_time_zone(new.get_time_zone())
                    new_other = (new_other.add_truncated)(**new.get_truncated_properties())
                    new_other.set_time_zone(prev_time_zone)
                    return new_other
        elif other.truncated:
            if not self.truncated:
                return other + self
            else:
                if not isinstance(other, Duration):
                    raise TypeError('Invalid addition: can only add Duration or truncated TimePoint to TimePoint.')
                duration = other
            ...
            if duration.seconds:
                if new.second_of_minute is None:
                    if new.minute_of_hour is None:
                        new.hour_of_day += duration.seconds / float(CALENDAR.SECONDS_IN_HOUR)
                    else:
                        new.minute_of_hour += duration.seconds / float(CALENDAR.SECONDS_IN_MINUTE)
                else:
                    new.second_of_minute += duration.seconds
                new.tick_over()
            if duration.minutes:
                if new.minute_of_hour is None:
                    new.hour_of_day += duration.minutes / float(CALENDAR.MINUTES_IN_HOUR)
                else:
            ...
            if duration.years:
                ...
                if new.get_is_calendar_date():
                    ...
                    if get_is_leap_year(new.year):
                        max_day_in_new_month = CALENDAR.DAYS_IN_MONTHS_LEAP[month_index]
        else:
            max_day_in_new_month = CALENDAR.DAYS_IN_MONTHS[month_index]
    ...
    def __eq__(self, other: 'TimePoint') -> bool:
        if other is None:
            ...
            if self.truncated != other.truncated:
                raise TypeError('Cannot compare truncated to non-truncated ' + 'TimePoint: %s, %s' % (self, other))
    ...
    def __sub__(self, other):
        if isinstance(other, TimePoint):
            ...
            return Duration(days=diff_day,
              ...
              seconds=diff_second)
        if not isinstance(other, Duration):
            raise TypeError('Invalid subtraction type ' + "'%s' - should be Duration." % type(other).__name__)
        duration = other
    ...
    def tick_over(self):
        ...
        if self.hour_of_day is not None:
            if self.minute_of_hour is not None:
                ...
                self.minute_of_hour += hours_remainder * CALENDAR.MINUTES_IN_HOUR
        if self.minute_of_hour is not None:
            if self.second_of_minute is not None:
                minutes_remainder = self.minute_of_hour - int(self.minute_of_hour)
                self.minute_of_hour -= minutes_remainder
                self.second_of_minute += minutes_remainder * CALENDAR.SECONDS_IN_MINUTE
        if self.second_of_minute is not None:
            num_minutes, seconds = divmod(self.second_of_minute, CALENDAR.SECONDS_IN_MINUTE)
            self.minute_of_hour += num_minutes
            self.second_of_minute = seconds
        if self.minute_of_hour is not None:
            num_hours, minutes = divmod(self.minute_of_hour, CALENDAR.MINUTES_IN_HOUR)
        ...
        if self.day_of_week is not None:
            num_weeks, days = divmod(self.day_of_week - 1, CALENDAR.DAYS_IN_WEEK)
    ...
    def _tick_over_day_of_month(self):
        if self.day_of_month < 1:
            num_days = 2
            for month, day in iter_months_days((self.year),
    ...
    def __str__(self, override_custom_dump_format=False, strftime_format=None):
        if self.expanded_year_digits not in TIMEPOINT_DUMPER_MAP:
            TIMEPOINT_DUMPER_MAP[self.expanded_year_digits] = dumpers.TimePointDumper(self.expanded_year_digits)
        else:
            ...
            if self.truncated:
                if self.truncated_dump_format:
                    if not override_custom_dump_format:
                        return dumper.dump(self, self.truncated_dump_format)
                return dumper.dump(self, self._get_truncated_dump_format())
            if self.dump_format:
                return override_custom_dump_format or dumper.dump(self, self.dump_format)
    ...
    def _get_dump_format(self):
        ...
        if self.expanded_year_digits:
            ...
            raise OverflowError('Year %s can only be represented in expanded format' % self.year)
        else:
        ...
        if self.get_is_calendar_date():
            ...
            if self.get_is_ordinal_date():
                ...
                if self.get_is_week_date():
                    ...
                    raise RuntimeError('TimePoint has inconsistent state, points must conform to calendar, ordinal or week dates.')
        time_string = 'Thh'
        if self.minute_of_hour is None:
            time_string += ',ii'
        else:
    ...
    def _get_truncated_dump_format(self):
        ...
        if self.truncated_property == 'year_of_decade':
            ...
            if self.truncated_property == 'year_of_century':
                ...
                time_string = ''
                if self.hour_of_day is None:
                    if self.minute_of_hour is not None or self.second_of_minute is not None:
                        time_string = 'T-'
        if self.hour_of_day is not None and int(self.hour_of_day) != self.hour_of_day:
            time_string = 'Thh,ii'
        else:
            if self.hour_of_day is not None:
                time_string = 'Thh'
            if self.minute_of_hour is None and self.second_of_minute is not None:
                ...
                if self.minute_of_hour is not None and int(self.minute_of_hour) != self.minute_of_hour:
                    if self.hour_of_day is not None:
                        time_string += ':'
                    time_string += 'mm,nn'
                else:
...
def _get_days_in_year_range(start_year, end_year, _):
    ...
    for factor, is_leap_factor in CALENDAR.LEAP_YEAR_FACTOR_TRUTHS:
        ...
        if start_year % factor == 0:
            num_corrections += 1
        if end_year != start_year:
            if end_year % factor == 0:
                num_corrections += 1
        factor_start_year = start_year + 1
        while factor_start_year % factor != 0 and factor_start_year < end_year:
            factor_start_year += 1

        if factor_start_year < end_year:
            num_corrections += 1
            num_corrections += (end_year - (factor_start_year + 1)) // factor
        if is_leap_factor:
...
def _get_weeks_in_year(year, _):
    ...
    cal_year_next, cal_ord_days_next = get_ordinal_date_week_date_start(year + 1)
...
def get_calendar_date_from_week_date(year, week_of_year, day_of_week):
    ...
    """
    num_days_week_year = (week_of_year - 1) * CALENDAR.DAYS_IN_WEEK + day_of_week - 1
    ...
    for iter_month, iter_day in iter_months_days(start_year,
      month_of_year=start_month, day_of_month=(start_day + 1)):
        total_iter_days += 1
        if num_days_week_year == total_iter_days:
            return (
             start_year, iter_month, iter_day)
...
def get_ordinal_date_from_week_date(year, week_of_year, day_of_week):
    ...
    """
    cal_year, cal_month, cal_day_of_month = get_calendar_date_from_week_date(year, week_of_year, day_of_week)
    return get_ordinal_date_from_calendar_date(cal_year, cal_month, cal_day_of_month)
...
def get_week_date_from_calendar_date(year, month_of_year, day_of_month):
    ...
    if prev_start <= cal_date < this_start:
        ...
        if this_start <= cal_date < next_start:
            ...
            week_date_start_year = year + 1
    total_iter_days = -1
    for iter_month, iter_day in iter_months_days(start_year, month_of_year=start_month,
      day_of_month=start_day):
        total_iter_days += 1
        if start_year == year and iter_month == month_of_year and iter_day == day_of_month:
    ...
    for iter_start_year in [start_year + 1, start_year + 2]:
        for iter_month, iter_day in iter_months_days(iter_start_year):
            total_iter_days += 1
            if iter_start_year == year and iter_month == month_of_year and iter_day == day_of_month:
                week_of_year = total_iter_days // CALENDAR.DAYS_IN_WEEK + 1
                day_of_week = total_iter_days % CALENDAR.DAYS_IN_WEEK + 1
                return (week_date_start_year, week_of_year, day_of_week)
...
def _get_calendar_date_week_date_start(year, _):
    """Return the calendar date of the start of (week date) year."""
    ref_year, ref_month, ref_day = CALENDAR.WEEK_DAY_START_REFERENCE['calendar']
    ref_year, ref_ordinal_day = CALENDAR.WEEK_DAY_START_REFERENCE['ordinal']
    if year == ref_year:
        return (
...
def get_timepoint_properties_from_seconds_since_unix_epoch(num_seconds):
    ...
    properties = dict(get_timepoint_from_seconds_since_unix_epoch(num_seconds).get_props())
...
def iter_months_days(year, month_of_year=None, day_of_month=None, in_reverse=False):
    ...
    return _iter_months_days(is_leap_year, month_of_year, day_of_month, CALENDAR.mode, in_reverse)


@lru_cache(maxsize=100000)
def _iter_months_days(is_leap_year, month_of_year, day_of_month, _, in_reverse=False):
    if day_of_month is not None:
...
def _int_caster(number, name='number', allow_none=False):
    ...
    try:
        ...
        try:
            raise BadInputError(BadInputError.INT_CAST, name, number, num_exc)
        finally:
            num_exc = None
            del num_exc

    if float(int_number) != float_number:
        raise BadInputError(BadInputError.INT_REMAINDER, name, number)
    return int_number
...
def _type_checker(*objects):
    for type_info in objects:
        ...
        raise BadInputError(BadInputError.TYPE, name, repr(value), values_string)


PARSE_PROPERTY_TRANSLATORS = {'seconds_since_unix_epoch': get_timepoint_properties_from_seconds_since_unix_epoch}