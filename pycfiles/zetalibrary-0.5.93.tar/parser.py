...
# [GCC 8.4.0]
# Embedded file name: /home/klen/Projects/zeta-library/zetalibrary/parser.py
# Compiled at: 2012-08-18 05:50:15
import re, urllib2
...
class CSSParser(Parser):
    import_re = re.compile('^\\s*@import +url\\(\\s*["\\\']?([^\\)\\\'\\"]+)["\\\']?\\s*\\)\\s*;?\\s*$', re.MULTILINE)
    comment_re = re.compile('/\\*(?:[^*]|\\*+[^*/])*\\*+/')
    comment_template = '/* %s */\n'
    link_re = re.compile('url\\(\\s*["\\\']?([^\\)\\\'\\"]+)["\\\']?\\)')
...
class SCSSParser(CSSParser):
    ...
    def parse_src(self, codesrc, path=None):
        ...
        self.parser.children.append(scss.spawn_rule(fileid=path, codestr=codesrc, context=self.parser._scss_vars, options=self.parser._scss_opts, index=self.parser._scss_index))
...
class JSParser(Parser):
    import_re = re.compile('^(?:require|include)\\(\\s*[\\\'\\"]([^\\\'\\"]+)[\\\'\\"]\\s*\\)\\s*;?\\s*$', re.MULTILINE)
    comment_re = re.compile('/\\*(?:[^*]|\\*+[^*/])*\\*+/')
    comment_template = '// %s\n'
