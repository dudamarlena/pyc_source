...
# [GCC 8.4.0]
# Embedded file name: /home/klen/Projects/zeta-library/zetalibrary/utils.py
...
def gen_frameworks():
    for fname in sorted(listdir(LIBDIR)):
        ...
        if not name.startswith('_') and op.isfile(fpath) and ext.strip('.') in ('css',
                                                                                'js',
                                                                                'scss'):
...
class ZetaTrick(Trick):
    ...
    def dispatch(self, event):
        ...
        if not name.startswith(self.args.prefix) and not event.is_directory and ext.lstrip('.').lower() in self.formats:
