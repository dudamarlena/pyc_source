...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/grun/gui/tkgrun.py
# Compiled at: 2009-10-03 10:45:54
try:
    from Tkinter import *
    import tkMessageBox, tkFileDialog
except:
    from tkinter import *
    import tkinter.messagebox as tkMessageBox, tkinter.filedialog as tkFileDialog

TIMER = 2000
import sys, re

def stripTags(data):
    return re.compile('<.*?>').sub('', data)
...
class gBool(tkBase):
    ...
    def __init__(self, name, value, **k):
        ...
        self._e = Checkbutton(self, variable=self.v)
...
class gText(tkBase):
    ...
    def __init__(self, name, value, **k):
        ...
        e = Text(self, height=6, bg='white', wrap=NONE, xscrollcommand=xscrollbar.set, yscrollcommand=yscrollbar.set)
...
class gCombo(tkBase):
    ...
    def __init__(self, name, value, **k):
        ...
        self._e = OptionMenu(self, self.variable, *self.sl)
        self._e.pack({'side': 'left'}, anchor=W)
...
class IconApp(object):
    ...
    def __init__(self, methods, name, img):
        self._name = name
        self._g = GrunApp(methods)

    def run(self):
        self._g.run(self._name, isRunWithOneOnly=True)
...
class GrunApp(object):
    ...
    def run(self, name, doc=None, isRunWithOneOnly=False):
        self._name = name
        if isRunWithOneOnly or len(self._l) > 1:
            ...
            for f in self._l:
                ...
                b = Button(w, text=beautifuler(f.name), command=_cb(f))
                b.pack(fill='x')
