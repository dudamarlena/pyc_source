...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/grun/gui/gtkgrun.py
# Compiled at: 2009-10-03 10:39:48
import pygtk, gtk
pygtk.require('2.0')
import types, os, gobject, time
TIMER = 2000
import string
...
def MessageBox(data, title=None, error=None):
    ...
    stock = gtk.image_new_from_stock(icon, gtk.ICON_SIZE_DIALOG)
...
class gNumber(gString):
    ...
    def __init__(self, name, value, **k):
        gString.__init__(self, name, (value and str(value) or ''), **k)
...
class gFile(GtkBase):
    ...
    def click(self, *a):
        ...
        dialog.set_default_response(gtk.RESPONSE_OK)
        if self.value:
            dialog.set_current_folder(os.path.dirname(self.value))
        r = dialog.run()
...
class gFolder(gFile):
    ...
    def click(self, *a):
        ...
        dialog.set_default_response(gtk.RESPONSE_OK)
        if self.value:
            dialog.set_current_folder(self.value)
        r = dialog.run()
        if r == gtk.RESPONSE_OK:
            self._e.set_text(dialog.get_filename())
        dialog.destroy()
