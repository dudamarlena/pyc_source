...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/damn/main.py
# Compiled at: 2015-10-17 08:25:51
from collections import OrderedDict
import yaml, json, os, glob
join = os.path.join
DAMN_EXTENSIONS = {'damn': lambda d: load_file(d), 
...
def load(path, match=DAMN_FILE_GLOB):
    ...
    if os.path.isdir(path):
        ...
        for full_child in children:
            ...
            if not child or full_child == exists:
                continue
            if '.' in child:
...
class Damn(dict):
    ...
    def __getattribute__(self, name):
        try:
            return super(Damn, self).__getattribute__(name)
        except:
            if name in self:
                return self[name]
            raise
