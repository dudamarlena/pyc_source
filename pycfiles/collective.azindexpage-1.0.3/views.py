...
# Embedded file name: /home/ubuntu/workspace/src/collective/awstats_hitcounter/browser/views.py
# Compiled at: 2015-10-13 16:05:52
from plone import api
from utils import counter
import json, Acquisition
...
class HitcounterView(object):
    ...
    def __call__(self):
        ...
        awstat_pattern = api.portal.get_registry_record('awstats_hitcounter.awstats_url_pattern')
        site = api.portal.get()
        path = self._relative_path()
        downloads = None
        self.request.response.setHeader('Content-type', 'application/json')
        if self.context.portal_type == 'File':
            downloads = counter(('{0}/at_download/file').format(path), awstat_pattern)
        ...
        if views <= 0:
            ...
            data = dict(success='true', creation_date=creation_date, modification_date=modification_date, page_views=views, hits=hits, content_type=content_type)
            if downloads:
...
class PopularContentView(object):
    ...
    def popular_content_items(self):
        ...
        """
        url_of_popular_page = api.portal.get_registry_record('awstats_hitcounter.url_of_popular_page')
        ...
        view_more_item_count = api.portal.get_registry_record('awstats_hitcounter.view_more_item_count')
        popular_urls = get_urls(url_of_popular_page, black_list, limit=view_more_item_count)
        return filter_urls(popular_urls, type_white_list=type_white_list, prevent_direct_downloads=prevent_direct_downloads)