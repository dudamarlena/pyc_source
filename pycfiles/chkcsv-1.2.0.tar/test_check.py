...
# Embedded file name: /Users/kevin/src/os-projects/chkcrontab/tests/test_check.py
# Compiled at: 2017-01-02 13:16:10
__doc__ = 'Test for chkcrontab_lib.'
...
def Flds(field, parsed_fields):
    return [
     field, {'cron_times': parsed_fields}]


def EFlds(field, parsed_fields):
    return [
     field,
     {'parser_error': 'An error', 'cron_times': parsed_fields}]
    ...
    def RunFSMOnFields(self, fields):
        for field, check_parsed_fields in fields:
            ...
            if 'parser_error' in parsed_fields:
                self.assertTrue('parser_error' in check_parsed_fields, 'Unexpected parser error: %s' % parsed_fields['parser_error'])
            else:
                if 'parser_error' in check_parsed_fields:
                    self.assertTrue('parser_error' in parsed_fields, 'Expected parser error: %s' % check_parsed_fields['parser_error'])
                self.assertEquals(len(parsed_fields['cron_times']), len(check_parsed_fields['cron_times']), 'Expected to find %d fields not %d' % (
                 len(check_parsed_fields['cron_times']),
                 len(parsed_fields['cron_times'])))
                for i in range(min(len(parsed_fields['cron_times']), len(check_parsed_fields['cron_times']))):
                    self.assertEquals(parsed_fields['cron_times'][i].Kind, check_parsed_fields['cron_times'][i][0], 'Parser detected "%s" field not "%s"' % (
                     parsed_fields['cron_times'][i].Kind,
                     check_parsed_fields['cron_times'][i][0]))
                    self.assertEquals('%s' % parsed_fields['cron_times'][i], check_parsed_fields['cron_times'][i][1], 'Parser found "%s" not "%s"' % (
...
class CTFUnitTest(unittest.TestCase):
    ...
    def FieldTest(self, field_name, field_count, field_data, field_errors):
        ...
        self.assertEquals(field_count, len(parsed['cron_times']), 'Found %d fields not %d fields.' % (
         len(parsed['cron_times']), field_count))
        actual_field_errors = []
        for field_action in parsed['cron_times']:
            actual_field_errors.extend(field_action.GetDiagnostics(self.cron_time_fields[field_name]))

        self.assertEquals(field_errors, len(actual_field_errors), 'Error list was: %s' % ("\n['" + ("',\n '").join(actual_field_errors) + "']\n"))

    def testMinute(self):
        self.FieldTest('minute', 6, '0,1,30,59,60,61', 2)
        self.FieldTest('minute', 3, '0-59,0-60,0-61', 2)
        self.FieldTest('minute', 4, '0-5/10,0-10/5,0-33/60,0-61/60', 5)
        self.FieldTest('minute', 3, 'mon,mon-fri,mon-fri/3', 5)

    def testHour(self):
        self.FieldTest('hour', 6, '0,1,12,23,24,25', 2)
        self.FieldTest('hour', 7, '0-12,1-12,12-13,12-1,23-24,24-30,25-1', 4)
        self.FieldTest('hour', 6, '0-12/2,1-3/0,12-13/14,12-1/2,23-24/25,24-30/2', 8)

    def testDayOfMonth(self):
        self.FieldTest('day of month', 6, '0,0001,15,31,32,33', 3)
        self.FieldTest('day of month', 7, '0-15,1-15,15-16,15-1,31-32,32-30,33-1', 5)
        self.FieldTest('day of month', 6, '0-14/2,1-3/0,14-16/17,14-1/2,31-32/33,32-30/2', 9)

    def testMonth(self):
        self.FieldTest('month', 6, '0,1,7,12,13,14', 3)
        self.FieldTest('month', 7, '0-7,1-7,7-8,7-1,12-13,13-10,14-1', 5)
        self.FieldTest('month', 6, '0-12/2,1-3/0,4-8/9,14-1/2,12-13/14,13-10/2', 9)

    def testDayOfWeek(self):
        self.FieldTest('day of week', 6, '0,1,4,7,8,9', 2)
        self.FieldTest('day of week', 7, '0-4,1-4,4-5,4-1,7-8,8-7,9-1', 4)
        self.FieldTest('day of week', 6, '0-3/2,1-3/0,3-5/6,3-1/2,7-8/9,8-7/2', 8)
...
class CheckCrontabUnitTest(unittest.TestCase):
    ...
    def CheckACrontab(self, crontab, whitelisted_users=None):
        ...
        self.assertEquals(check.check_crontab(crontab_file, log, whitelisted_users), exp_rc, 'Failed to return %d for crontab errors.' % exp_rc)
