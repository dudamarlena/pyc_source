...
# Embedded file name: build/bdist.linux-i686/egg/proxylet/relocate.py
# Compiled at: 2009-03-17 20:17:45
...
class DAVRelocator(Relocator):
    ...
    _filter_methods = {'OPTIONS': 1, 'PROPFIND': 1, 'REPORT': 1, 'MKACTIVITY': 1, 
       'PROPPATCH': 1, 'CHECKOUT': 1, 'MKCOL': 1, 
       'MOVE': 1, 'COPY': 1, 'LOCK': 1, 'UNLOCK': 1, 
       'MERGE': 1}
...
class DrupalRelocator(Relocator):
    ...
    class RewriteResponse(Relocator.RewriteResponse):
        ...
        def rwBody(self, bodyIn):
            ...
            data = ('').join(data)
            repl = re.compile('<form action="%s([^"]*)"([^>]*)>' % (self.parent.remote.path,))
            data = repl.sub('<form action="%s\\1"\\2>' % (self.parent.local.path,), data)
