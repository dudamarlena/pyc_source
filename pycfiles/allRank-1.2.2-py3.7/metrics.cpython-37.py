...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/allrank/models/metrics.py
...
def dcg(y_pred, y_true, ats=None, gain_function=lambda x: torch.pow(2, x) - 1, padding_indicator=PADDED_Y_VALUE):
    ...
    discounts = (torch.tensor(1) / torch.log2(torch.arange((true_sorted_by_preds.shape[1]), dtype=(torch.float)) + 2.0)).to(device=dev)
