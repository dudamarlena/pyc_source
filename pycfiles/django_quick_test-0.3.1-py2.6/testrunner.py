...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/quick_test/testrunner.py
# Compiled at: 2011-08-01 12:52:56
import os, sys
...
class NoseTestSuiteRunner(DjangoTestSuiteRunner):
    ...
    def run_suite(self, nose_argv):
        ...
        nose.core.TestProgram(argv=nose_argv, exit=False, addplugins=[
         result_plugin])
    ...
    def run_tests(self, test_labels, extra_tests=None):
        ...
        nose_argv = [
         'nosetests', '--verbosity', str(self.verbosity)]
        ...
        nose_argv.extend(OPTION_TRANSLATION.get(opt, opt) for opt in sys.argv[2:] if not any(opt.startswith(d) for d in django_opts))
        if self.verbosity >= 1:
