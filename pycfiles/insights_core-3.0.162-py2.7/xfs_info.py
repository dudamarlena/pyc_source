...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/xfs_info.py
...
class XFSInfo(CommandParser):
    ...
    def parse_content(self, content):
        ...
        """
        info_re = re.compile('^(?P<section>[\\w-]+)?\\s*' + '=(?:(?P<specifier>\\S+)(?:\\s(?P<specval>\\w+))?)?' + '\\s+(?P<keyvaldata>\\w.*\\w)$')
        keyval_re = re.compile('(?P<key>[\\w-]+)=(?P<value>\\d+(?: blks)?)')
        sect_info = None
        for line in content:
