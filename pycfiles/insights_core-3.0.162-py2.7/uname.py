...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/uname.py
...
class Uname(CommandParser):
    ...
    def from_kernel(cls, kernel):
        ...
        content = [
         ('{name} {nodename} {kernel} {kernel_type} {kernel_date} {machine} {processor} {hw_platform} {os}').format(name='Linux', nodename=data['nodename'], kernel=kernel, kernel_type=data['kernel_type'], kernel_date=data['kernel_date'], machine=data['machine'], processor=data['processor'], hw_platform=data['hw_platform'], os=data['os'])]
    ...
    def _best_version(self, other):
        ...
        if self._sv_version and other._sv_version:
            ...
            return (
             self._lv_version, other._lv_version)
    ...
    def __lt__(self, other):
        ...
        if isinstance(other, Uname):
            ...
            other_uname = Uname.from_uname_str(other)
        s_version, o_version = self._best_version(other_uname)
        return s_version < o_version or s_version == o_version and self._lv_release < other_uname._lv_release
...
def pad_release(release_to_pad, num_sections=4):
    ...
    if len(parts) > num_sections:
        raise ValueError(('Too many sections encountered ({found} > {num} in release string {rel}').format(found=len(parts), num=num_sections, rel=release_to_pad))
