...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/ip_netns_exec_namespace_lsof.py
...
class IpNetnsExecNamespaceLsofI(CommandParser):
    ...
    def parse_content(self, content):
        ...
        self.data = parse_fixed_table(content, heading_ignore=[
         'COMMAND', 'PID', 'USER', 'FD', 'TYPE', 'DEVICE', 'SIZE/OFF', 'NODE', 'NAME'], header_substitute=[
         ('COMMAND', 'command'), ('PID', 'pid'), ('USER', 'user'), ('FD', 'fd'),
         ('TYPE', 'type'), ('DEVICE', 'device'), ('SIZE/OFF', 'size_off'),
         ('NODE', 'node'), ('NAME', 'name')])
        if not self.data:
            raise SkipException('Useless data')
        for item in self.data:
            self.fields.append(self.keyvalue(item['command'], item['pid'], item['user'], item['fd'], item['type'], item['device'], item['size_off'], item['node'], item['name']))
