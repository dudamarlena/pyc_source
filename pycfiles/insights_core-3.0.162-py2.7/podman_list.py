...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/podman_list.py
...
class PodmanList(CommandParser):
    ...
    * Some fields can contain nothing, and this is shown as spaces, so we
    ...
    def parse_content(self, content):
        ...
        self.rows = parse_fixed_table(content, heading_ignore=self.heading_ignore, header_substitute=self.substitutions)
        if not self.rows:
