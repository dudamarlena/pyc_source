...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/util/component_graph.py
...
def component_info(component):
    ...
    return {'name': dr.get_name(component), 
...
def main(filename):
    ...
    specs = sorted([ c for c in dr.DELEGATES if is_datasource(c) and dr.get_module_name(c) == 'insights.specs'
                   ], key=dr.get_name)
    with open(filename, 'w') as (fh):
        ...
        for spec in specs:
            ...
            for d in dr.get_dependents(spec):
                info['dependents'].append({'name': dr.get_name(d), 
