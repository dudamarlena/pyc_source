...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/lsblk.py
...
class BlockDevice(object):
    ...
    def __str__(self):
        if 'TYPE' in self.data and 'MOUNTPOINT' in self.data:
            return ('{type}:{name}({mnt})').format(type=self.data['TYPE'], name=self.data['NAME'], mnt=self.data['MOUNTPOINT'])
        else:
...
class LSBlock(BlockDevices):
    ...
    def parse_content(self, content):
        r = re.compile('([\\s\\|\\`\\-]*)(\\S+.*) (\\d+:\\d+)\\s+(\\d)\\s+(\\d+(\\.\\d)?[A-Z])\\s+(\\d)\\s+([a-z]+)(.*)')
        device_list = []
        ...
        for line in content:
            if ' TYPE=' not in line:
                if 'failed to get device path' in line:
                    ...
                    raise ParseException(("TYPE not found in LsBlockPairs line '{l}'").format(l=line))
            d = dict((k, v) for k, v in re.findall('(\\S+)=\\"(.*?)\\"\\s?', line) if len(v) > 0)
