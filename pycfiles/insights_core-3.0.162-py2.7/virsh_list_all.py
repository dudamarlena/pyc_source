...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/virsh_list_all.py
...
class VirshListAll(CommandParser):
    ...
    def parse_content(self, content):
        ...
        if not content:
            return
        self.cols = parse_fixed_table(content, heading_ignore=[
         'Id', 'Name', 'State'], header_substitute=[
         ('Id', 'id'), ('Name', 'name'), ('State', 'state')])[1:]
        self._cleanup()
        for item in self.cols:
            self.fields.append(self.keyvalue(item['name'], item['state'], item['id'], item['name'].lower()))
