...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/mdstat.py
    ...
    def parse_content(self, content):
        ...
        for comp in self.components:
            ...
            if devname not in self.mds:
                self.mds[devname] = {'name': devname, 
                   'active': comp['active'], 'raid': comp['raid'], 
                   'blocks': comp['blocks'], 'devices': []}
                for opt in ['level', 'chunk', 'algorithm']:
                    if opt in comp:
                        self.mds[devname][opt] = comp[opt]

            self.mds[devname]['devices'].append(dict((k, comp[k]) for k in comp if k in ('component_name',
                                                                                         'role',
                                                                                         'up')))

        self.data = {'personalities': self.personalities, 
...
def parse_array_start(md_line):
    ...
    if raid_read_only_token == '(auto-read-only)':
        ...
        for token in tokens:
            ...
            component_row = {'device_name': device_name, 'raid': raid, 'active': active, 
...
def parse_array_status(line, components):
    ...
    * ``algorithm`` - (int) if found, the current algorithm in use.
    ...
    """
    status_line_re = '(?P<blocks>\\d+) blocks' + '(?: super (?P<super>\\S+))?' + '(?: level (?P<level>\\d+),)?' + '(?: (?P<chunk>\\d+k) chunk,)?' + '(?: algorithm (?P<algorithm>\\d+))?'
