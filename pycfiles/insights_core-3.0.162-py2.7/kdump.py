...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/kdump.py
...
class KDumpConf(Parser, LegacyItemAccess):
    ...
    * ``options`` - the ``options`` value in the data(see above).
    ...
    def is_nfs(self):
        ...
        """
        return 'nfs' in self or 'nfs4' in self or 'net' in self and '@' not in self['net']
    ...
    def using_local_disk(self):
        ...
        * Otherwise, the dump is local.
        ...
        """
        return not ('ssh' in self.data or 'net' in self.data or 'nfs' in self.data or 'nfs4' in self.data)
        ...
        for k in keys:
            if k in self.data:
                ...
                if isinstance(v, list):
                    raise ParseException('More than one %s type targets are                                         configured.' % k)
                if target:
