...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/archive/repo.py
...
ARCHIVES = [
 ...
 TestArchive('insights_heartbeat', transforms=[
  Transform('hostname').replace('insights-heartbeat-9cd6f607-6b28-44ef-8481-62b0e7773614')], machine_id=hashlib.sha224('TEST-HEARTBEAT-RHAI2').hexdigest())]
...
def build_integration_test_archive(module_name, i, test_tuple, dest, machine_id=None):
    ...
    try:
        ...
        if console:
            ...
            sys.stdout.flush()
        return {'module': module_name, 'test_func': test_func, 
