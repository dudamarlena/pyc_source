...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/netstat.py
...
NETSTAT_SECTION_ID = {ACTIVE_INTERNET_CONNECTIONS: [
...
class NetstatAGN(CommandParser):
    ...
    def group_by_iface(self):
        ...
        for entry in self.data:
            result[entry['interface']].append(dict((k.lower(), v) for k, v in entry.items() if k in ('refcnt',
                                                                                                     'group')))
...
class NetstatSection(object):
    ...
    def add_data(self, line):
        ...
        self.datalist.append(dict((m, d) for m, d in zip(NETSTAT_SECTION_ID[self.name], [ r[(-1)] for r in self.data ])))
    ...
    * ``Program name`` - the process ID from the 'PID/Program name' field.
    ...
    def running_processes(self):
        ...
        if ACTIVE_INTERNET_CONNECTIONS not in self.data:
            return set()
        return set(pg.split('/', 1)[1].strip() for pg in self.data[ACTIVE_INTERNET_CONNECTIONS]['PID/Program name'] if '/' in pg)
...
class Netstat_I(CommandParser):
    ...
    def parse_content(self, content):
        ...
        for entry in self.data:
            self._group_by_iface[entry['Iface']] = dict((k, v) for k, v in entry.items() if k != 'Iface')
...
class SsTULPN(CommandParser):
    ...
    def get_localport(self, port):
        ...
        for line in self.data:
            ...
            if local_port and ':*' not in local_port and int(port) == int(local_port.split(':')[(-1)]):
