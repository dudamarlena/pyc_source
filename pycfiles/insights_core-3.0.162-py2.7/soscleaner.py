...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/contrib/soscleaner.py
# Compiled at: 2020-03-25 13:10:41
import os, re, errno, sys, uuid, shutil, struct, socket, tempfile, logging, tarfile, six
...
class SOSCleaner:
    ...
    def _start_logging(self, filename):
        ...
        logging.basicConfig(filename=filename, level=eval(loglevel_config), format='%(asctime)s %(name)s %(levelname)s: %(message)s', datefmt='%m-%d %H:%M:%S')
        if not self.quiet:
    ...
    def _prep_environment(self):
        ...
        origin_path = os.path.join(self.report_dir, 'soscleaner-origin-%s' % ran_uuid)
        ...
        return (
         origin_path, dir_path, session, logfile, ran_uuid)
    ...
    def _sub_ip(self, line):
        ...
        """
        try:
            pattern = '(((\\b25[0-5]|\\b2[0-4][0-9]|\\b1[0-9][0-9]|\\b[1-9][0-9]|\\b[1-9]))(\\.(\\b25[0-5]|\\b2[0-4][0-9]|\\b1[0-9][0-9]|\\b[1-9][0-9]|\\b[0-9])){3})'
