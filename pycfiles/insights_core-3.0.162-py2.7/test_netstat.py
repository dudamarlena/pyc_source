...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/tests/test_netstat.py
...
class TestNetstats:
    ...
    def test_netstat_s(self):
        ...
        assert info['ip'] == {'total_packets_received': '3405107', 'forwarded': '0', 
           ...
           'fragments_created': '8'}
        assert info['icmp'] == {'icmp_messages_received': '114', 'input_icmp_message_failed': '0', 
           'icmp_input_histogram': {'destination_unreachable': '107', 
                                    'echo_requests': '4', 
                                    'echo_replies': '3'}, 
           'icmp_messages_sent': '261', 
           'icmp_messages_failed': '0', 
           'icmp_output_histogram': {'destination_unreachable': '254', 
                                     'echo_request': '3', 
                                     'echo_replies': '4'}}
        assert info['icmpmsg'] == {'intype0': '3', 'intype3': '107', 
           'intype8': '4', 
           'outtype0': '4', 
           'outtype3': '254', 
           'outtype8': '3'}
        assert info['tcp'] == {'active_connections_openings': '1648', 'passive_connection_openings': '1525', 
           ...
           'resets_sent': '212'}
        assert info['udp'] == {'packets_received': '4901', 'packets_to_unknown_port_received': '107', 
           'packet_receive_errors': '0', 
           'packets_sent': '1793', 
           'receive_buffer_errors': '0', 
           'send_buffer_errors': '0'}
        assert info['udplite'] == {}
        assert info['tcpext'] == {'tcp_sockets_finished_time_wait_in_fast_timer': '1239', 'delayed_acks_sent': '295934', 
           ...
           'tcpchallengeack': '3', 
           'tcpspuriousrtxhostqueues': '3'}
        assert info['ipext'] == {'innoroutes': '9', 'inmcastpkts': '406', 
           'inbcastpkts': '517437', 
           'inoctets': '865450302', 
           'outoctets': '812810111', 
           'inmcastoctets': '12992', 
    ...
    def test_netstat_s_w(self):
        ...
        assert len(info) == 1
        assert info['ip'] == {'total_packets_received': '6440', 'with_invalid_addresses': '3', 
           'forwarded': '0', 
           'incoming_packets_discarded': '0', 
...
def test_get_netstat_agn():
    ...
    assert result['lo'] == [{'refcnt': '1', 'group': '224.0.0.1'}, {'refcnt': '3', 'group': 'ff02::1'}]
    assert result['eth0'] == [{'refcnt': '1', 'group': '224.0.0.1'}, {'refcnt': '4', 'group': 'ff02::1'}, {'refcnt': '1', 'group': 'ff01::1'}]
...
def test_get_netstat():
    ...
    assert len(nsdl) == 6
    assert nsdl[0] == {'Proto': 'tcp', 
       'Recv-Q': '0', 'Send-Q': '0', 'Local Address': '192.168.0.1:53', 
       'Local IP': '192.168.0.1', 'Port': '53', 
       'Foreign Address': '192.168.0.53:53', 'State': 'ESTABLISHED', 
       'User': '0', 'Inode': '1817', 'PID/Program name': '12/dnsd', 
       'PID': '12', 'Program name': 'dnsd', 'Timer': 'off (0.00/0/0)'}
    assert nsdl[5] == {'Proto': 'tcp', 
       'Recv-Q': '0', 'Send-Q': '0', 'Local Address': '10.24.1.245:80', 
       'Local IP': '10.24.1.245', 'Port': '80', 
       'Foreign Address': '10.24.36.145:32790', 'State': 'SYN_RECV', 
       'User': '0', 'Inode': '0', 'PID/Program name': '-', 
       'Timer': 'on (0.79/0/0)'}
...
def test_get_netstat_keyword_search():
    ...
    assert ns.search(search_list=[
     netstat.ACTIVE_UNIX_DOMAIN_SOCKETS], State__contains='LISTEN') == [
     ns.datalist[netstat.ACTIVE_UNIX_DOMAIN_SOCKETS][1],
     ns.datalist[netstat.ACTIVE_UNIX_DOMAIN_SOCKETS][2]]
...
def test_running_processes():
    ...
    assert 'dnsd' in ns.running_processes
...
def test_get_netstat_i():
    ...
    assert len(result) == 7
    assert result['bond0'] == {'MTU': '1500', 
       'Met': '0', 'RX-OK': '845265', 'RX-ERR': '0', 'RX-DRP': '0', 
       'RX-OVR': '0', 'TX-OK': '1753', 'TX-ERR': '0', 'TX-DRP': '0', 
       'TX-OVR': '0', 'Flg': 'BMmRU'}
    assert result['eth0'] == {'MTU': '1500', 
       'Met': '0', 'RX-OK': '422518', 'RX-ERR': '0', 'RX-DRP': '0', 
       'RX-OVR': '0', 'TX-OK': '1703', 'TX-ERR': '0', 'TX-DRP': '0', 
       'TX-OVR': '0', 'Flg': 'BMsRU'}
...
def test_ss_tulpn_data():
    ...
    assert len(ss) == 13
    assert ss[0] == {'Netid': 'udp', 'Peer-Address-Port': '*:*', 'Send-Q': '0', 'Local-Address-Port': '*:55898', 'State': 'UNCONN', 
       'Recv-Q': '0'}
...
def test_ss_tulpn_get_service():
    ...
    exp = [
     {'Netid': 'udp', 'Process': 'users:(("rpcbind",pid=953,fd=9))', 'Peer-Address-Port': '*:*', 'Send-Q': '0', 'Local-Address-Port': '*:111', 
        'State': 'UNCONN', 'Recv-Q': '0'},
     {'Netid': 'udp', 'Process': 'users:(("rpcbind",pid=953,fd=11))', 'Peer-Address-Port': ':::*', 'Send-Q': '0', 'Local-Address-Port': ':::111', 
        'State': 'UNCONN', 'Recv-Q': '0'}]
...
def test_ss_tulpn_get_port():
    ss = SsTULPN(context_wrap(Ss_TULPN))
    exp01 = [
     {'Netid': 'tcp', 'Process': 'users:(("sshd",pid=1416,fd=4))', 'Peer-Address-Port': ':::*', 'Send-Q': '128', 'Local-Address-Port': ':::2223', 
        'State': 'LISTEN', 'Recv-Q': '0'}]
    assert ss.get_localport('2223') == exp01
    exp02 = [
     {'Netid': 'udp', 'Process': 'users:(("rpc.statd",pid=29559,fd=10))', 'Peer-Address-Port': ':::12345', 'Send-Q': '0', 
        'Local-Address-Port': ':::37968', 'State': 'UNCONN', 'Recv-Q': '0'}]
    assert ss.get_peerport('12345') == exp02
    assert ss.get_port('12345') == exp02
    exp03 = [
     {'Netid': 'tcp', 'Process': 'users:(("sshd",1231,3))', 'Peer-Address-Port': '*:*', 'Send-Q': '128', 'Local-Address-Port': '*:22', 
        'State': 'LISTEN', 'Recv-Q': '0'}]
...
def test_netstat_doc_examples():
    env = {'stats': NetstatS(context_wrap(NETSTAT_S)), 
...
def test_ss_tupna_get_port():
    ...
    exp01 = [
     {'Netid': 'tcp', 'Peer-Address-Port': '192.168.0.105:2049', 'Send-Q': '0', 'Local-Address-Port': '192.168.0.106:739', 
        'State': 'ESTAB', 'Recv-Q': '0'}]
...
def test_ss_tupnla_get_port():
    ...
    exp02 = [
     {'Netid': 'udp', 'Peer-Address-Port': '0.0.0.0:*', 'Send-Q': '0', 'Local-Address-Port': '0.0.0.0:34453', 'State': 'UNCONN', 
        'Recv-Q': '0'}]
