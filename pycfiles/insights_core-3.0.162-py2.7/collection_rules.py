...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/client/collection_rules.py
# Compiled at: 2020-04-23 14:49:03
"""
Rules for data collection
"""
from __future__ import absolute_import
import hashlib, json, logging, six, shlex, os, requests, yaml, stat
...
def correct_format(parsed_data, expected_keys, filename):
    ...
    if invalid_keys:
        return (True,
         'Unknown section(s) in %s: ' % filename + (', ').join(invalid_keys) + '\nValid sections are ' + (', ').join(expected_keys) + '.')
    else:
...
def verify_permissions(f):
    ...
    if not mode == 384:
        raise RuntimeError('Invalid permissions on %s. Expected 0600 got %s' % (
         f, oct(mode)))
...
class InsightsUploadConf(object):
    ...
    def validate_gpg_sig(self, path, sig=None):
        ...
        if sig is None:
            sig = path + '.asc'
        command = '/usr/bin/gpg --no-default-keyring --keyring ' + constants.pub_gpg_path + ' --verify ' + sig + ' ' + path
        if not six.PY3:
            command = command.encode('utf-8', 'ignore')
        args = shlex.split(command)
        logger.debug('Executing: %s', args)
        proc = Popen(args, shell=False, stdout=PIPE, stderr=STDOUT, close_fds=True)
    ...
    def get_collection_rules(self, raw=False):
        ...
        """
        logger.debug('Attemping to download collection rules from %s', self.collection_rules_url)
        logger.log(NETWORK, 'GET %s', self.collection_rules_url)
        try:
            req = self.conn.session.get(self.collection_rules_url, headers={'accept': 'text/plain'})
            ...
            logger.error('ERROR: Could not download dynamic configuration: %s', e)
            return
    ...
    def fetch_gpg(self):
        logger.debug('Attemping to download collection rules GPG signature from %s', self.collection_rules_url + '.asc')
        headers = {'accept': 'text/plain'}
        logger.log(NETWORK, 'GET %s', self.collection_rules_url + '.asc')
        config_sig = self.conn.session.get(self.collection_rules_url + '.asc', headers=headers)
    ...
    def get_rm_conf_old(self):
        ...
        if not self.remove_file:
            ...
            if not os.path.isfile(self.remove_file):
                logger.debug('%s not found. No data files, commands, or patterns will be ignored, and no keyword obfuscation will occur.', self.remove_file)
                return None
            try:
        ...
        if fname not in (self.redaction_file, self.content_redaction_file):
            ...
            if not fname or not os.path.isfile(fname):
                if fname == self.redaction_file:
                    ...
                    logger.debug('%s not found. No patterns will be skipped and no keyword obfuscation will occur.', self.content_redaction_file)
                return None
            try:
    ...
    def create_report(self):
        ...
        return {'obfuscate': self.config.obfuscate, 
