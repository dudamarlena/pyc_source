...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/samba_logs.py
...
def strip_surrounds(s):
    ...
    while s[start] == '[' and s[end] == ']' or s[start] == '[' and s[end] == ',' or s[start] == '(' and s[end] == ')':
        start += 1
        end -= 1
...
class SAMBALog(LogFileOutput):
    ...
    def _parse_line(self, line):
        fields = line.split()
        try:
            parsed_line = {'timestamp': strip_surrounds((' ').join(fields[0:2])), 'pid': strip_surrounds(fields[3]), 
               'function': strip_surrounds(fields[8]), 
               'message': (' ').join(fields[8:]), 
               'raw_message': line}
            parsed_line['datetime'] = datetime.strptime(parsed_line['timestamp'], '%a %b %d %H:%M:%S %Y')
        except:
            pass
