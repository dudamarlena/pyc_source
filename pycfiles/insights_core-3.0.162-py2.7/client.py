...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/client/client.py
# Compiled at: 2020-04-16 14:56:28
from __future__ import print_function
from __future__ import absolute_import
import sys, json, logging, logging.handlers, os, time, six
from .utilities import generate_machine_id, write_to_disk, write_registered_file, write_unregistered_file, delete_registered_file, delete_unregistered_file, delete_cache_files, determine_hostname, read_pidfile, systemd_notify
        ...
        if check['status']:
            ...
            if config.register:
                ...
                if config.display_name is None and config.group is None:
                    ...
                    logger.info('Successfully registered host %s as %s in group %s', hostname, display_name, group)
                if message:
                    logger.info(message)
                write_registered_file()
                return True
            ...
            if check['unreg_date']:
                logger.info('This machine has been unregistered. Use --register if you would like to re-register this machine.')
            else:
                logger.info('This machine has not yet been registered.Use --register to register this machine.')
            return False
...
def update_rules(config, pconn):
    if not pconn:
        raise ValueError('ERROR: Cannot update rules in --offline mode. Disable auto_update in config file.')
...
def _legacy_upload(config, pconn, tar_file, content_type, collection_duration=None):
    ...
    for tries in range(config.retries):
        ...
        if upload.status_code in (200, 201):
            ...
            if account_number:
                logger.info('Successfully uploaded report from %s to account %s.', msg_name, account_number)
            else:
            ...
            logger.error('Upload attempt %d of %d failed! Status Code: %s', tries + 1, config.retries, upload.status_code)
    ...
    for tries in range(config.retries):
        ...
        if upload.status_code in (200, 202):
            ...
            if config.register:
                logger.info('View the Red Hat Insights console at https://cloud.redhat.com/insights/')
            return
        if upload.status_code in (413, 415):
            pconn.handle_fail_rcs(upload)
            raise RuntimeError('Upload failed.')
        else:
            logger.error('Upload attempt %d of %d failed! Status code: %s', tries + 1, config.retries, upload.status_code)
