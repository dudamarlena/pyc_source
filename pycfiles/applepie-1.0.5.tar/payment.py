...
# Embedded file name: build/bdist.linux-x86_64/egg/applepay/payment.py
# Compiled at: 2016-10-04 13:06:41
...
class Payment:
    ...
    def _generate_symmetric_key(self, shared_secred):
        sha = sha256()
        sha.update('\x00\x00\x00')
        sha.update('\x01')
        sha.update(shared_secred)
        sha.update('\rid-aes256-GCMApple' + self._merc_id)
        return sha.digest()
    ...
    def decrypt(self, ephemeral_public_key, cipher_data, transaction_id=None, application_data=''):
        ...
        mode = ciphers.modes.GCM('\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00', cipherdata[-16:], 16)
