...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/kolekto/commands/importer.py
    ...
    with printer.progress(os.path.getsize(source_filename)) as (update):
        with open(source_filename, 'rb') as (fsource):
            ...
            if os.path.exists(dest):
                ...
                source_filename = os.path.relpath(source_filename, os.path.join(tree, '.kolekto', 'movies'))
...
class BaseImport(Command):
    ...
    def prepare(self):
        ...
        self.add_arg('--hardlink', action='store_true', default=False, help='Create an hardlink instead of copying the file')
        self.add_arg('--symlink', action='store_true', default=False, help='Create a symlink instead of copying the file')
        self.add_arg('--auto', '-a', action='store_true', default=False, help='Automatically choose the most revelent movie.')
        self.add_arg('--dont-show', dest='show', action='store_false', default=True, help='Show all informations about imported movie.')
        self.add_arg('--delete', action='store_true', default=False, help='Delete imported file after a successful import')
...
class ImportMovies(BaseImport):
    ...
    def _search(self, mdb, query, filename, year=None, auto=False):
        ...
        for datasource, movie in mdb.search(query, year=year):
            ...
            choices.append(option((datasource, movie), fmt, title=movie['title'], year=movie.get('year', 'Unknown'), directors=directors, datasource=datasource.name))
