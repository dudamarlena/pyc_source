...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/kolekto/tmdb_proxy.py
...
ONE_WEEK = 604800
app = Flask(__name__)
app.config.update(REDIS_HOST=os.environ.get('KOLEKTO_REDIS_HOST', 'localhost'), REDIS_PORT=int(os.environ.get('KOLEKTO_REDIS_PORT', 6379)), REDIS_DB=int(os.environ.get('KOLEKTO_REDIS_DB', 0)), REDIS_RO_HOST=os.environ.get('KOLEKTO_REDIS_RO_HOST', None), REDIS_RO_PORT=int(os.environ.get('KOLEKTO_REDIS_RO_PORT', 6379)), TMDB_API_KEY=os.environ['KOLEKTO_TMDB_API_KEY'], CACHE_TTL=int(os.environ.get('KOLEKTO_CACHE_TTL', ONE_WEEK)), DEBUG=os.environ.get('KOLEKTO_DEBUG') == 'on')
...
if app.config['REDIS_RO_HOST'] is not None:
    redis_ro_conn = redis.StrictRedis(host=app.config['REDIS_RO_HOST'], port=app.config['REDIS_RO_PORT'], db=app.config['REDIS_DB'])
else:
    ...
    if cached:
        ...
        try:
            ...
            for movie in found['results']:
                ...
                movies.append({'title': movie['original_title'], 'directors': [ x['name'] for x in cast['crew'] if x['department'] == 'Directing' ], 'year': year, 
                   '_tmdb_id': movie['id']})
...
def get_movie(tmdb_id):
    ...
    if cached:
        ...
        movie = {'title': details['original_title'], 'score': details['popularity'], 'directors': [ x['name'] for x in cast['crew'] if x['department'] == 'Directing' ], 'writers': [ x['name'] for x in cast['crew'] if x['department'] == 'Writing' ], 'cast': [ x['name'] for x in cast['cast'] ], 'genres': [ x['name'] for x in details['genres'] ], 'countries': [ x['name'] for x in details['production_countries'] ], 'tmdb_votes': int(round(details.get('vote_average', 0) * 0.5)), 
           '_tmdb_id': tmdb_id}
