...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/kolekto/datasources/tmdb.py
...
class TmdbDatasource(Datasource):
    ...
    URL_SEARCH = 'http://api.themoviedb.org/3/search/movie?api_key=%(api_key)s&query=%(query)s'
    ...
    def search(self, title, year=None):
        ...
        for result in results:
            ...
            movie = self.object_class({'title': result['original_title'], 'directors': [ x['name'] for x in cast['crew'] if x['department'] == 'Directing' ], '_datasource': self.name, 
               '_tmdb_id': result['id']})
            if movie_year:
    ...
    def refresh(self, movie):
        ...
        if '_tmdb_id' in movie:
            ...
            refreshed = self.object_class({'title': details['original_title'], 'score': details['popularity'], 
               'directors': [ x['name'] for x in cast['crew'] if x['department'] == 'Directing' ], 'writers': [ x['name'] for x in cast['crew'] if x['department'] == 'Writing' ], 'cast': [ x['name'] for x in cast['cast'] ], 'genres': [ x['name'] for x in details['genres'] ], 'countries': [ x['name'] for x in details['production_countries'] ], 'tmdb_votes': int(round(details.get('vote_average', 0) * 0.5)), 
