...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/kolekto/commands/find_duplicates.py
...
class FindDuplicates(Command):
    ...
    def run(self, args, config):
        ...
        for (title, year), hashs in hash_by_title.iteritems():
            if len(hashs) > 1:
                printer.p('<b>{title}</b> ({year}): {hashs}', title=title, year=year, hashs=(' ').join(hashs))
