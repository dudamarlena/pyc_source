...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/kolekto/commands/link.py
    ...
    try:
        for child in os.listdir(directory):
            ...
            if os.path.islink(fullname):
                ...
                links.update(walk_links(fullname, prefix=os.path.join(prefix, child), linkbase=linkbase))
...
class Link(Command):
    ...
    def prepare(self):
        self.add_arg('--dry-run', '-d', action='store_true', default=False, help='Do not create or delete any link')
    ...
    def run(self, args, config):
        ...
        fs_links = {}
        for view in config.subsections('view'):
            view_links = walk_links(os.path.join(args.tree, view.args), prefix=view.args, linkbase=os.path.join(args.tree, '.kolekto', 'movies'))
        ...
        printer.p('Found {rem} links to delete, {add} links to create', rem=len(links_to_delete), add=len(links_to_create))
        dirs_to_cleanup = set()
