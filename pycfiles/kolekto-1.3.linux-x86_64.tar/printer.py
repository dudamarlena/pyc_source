...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/kolekto/printer.py
# Compiled at: 2014-06-16 16:12:17
import sys, subprocess, tempfile, progressbar
DEFAULT_TAGS = {'b': (
       lambda **attrs: '\x1b[1m',
       lambda : '\x1b[21m'), 
   'inv': (
         lambda **attrs: '\x1b[37;7m',
         lambda : '\x1b[27;39m'), 
   'dim': (
         lambda **attrs: '\x1b[2m',
         lambda : '\x1b[22m'), 
    ...
    def _lexer(self, data):
        while data:
            ...
            if start != -1:
                if start:
                    yield (
                     'text', data[:start], None, None)
                    data = data[start:]
                end = data.find('>')
                recheck = data.find('<', 1)
                if recheck > 0 and recheck < end:
                    yield (
                     'text', data[:recheck], None, None)
                    data = data[recheck:]
                elif end > 0:
                    tag = data[1:end]
                    if tag[0] == '/':
                    ...
                    if tag_components and tag_components[0] in self.tags:
                        ...
                        yield (
                         token_type, data[:end + 1], tag_name, opts)
                    else:
                        yield (
                         'text', data[:end + 1], None, None)
                    data = data[end + 1:]
                else:
                    break
            else:
...
class KolektoPrinter(object):
    ...
    def debug(self, *args, **kwargs):
        if self._debug:
            args = ('[debug]', ) + args
            self._print(err=True, *args, **kwargs)
