...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/kolekto/datasources/rewrite.py
...
class RewriteDatasource(Datasource):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.rewrites = [ (x.args, x.get('value'), x.get('ignore_error')) for x in self.config.subsections('rewrite')
                        ]

    def attach(self, movie_hash, movie):
        for field, rewrite_func, ignore_error in self.rewrites:
            try:
                value = rewrite_func(globals={'movie': movie, 'sub': re.sub, 'split': re.split})
            except:
                if not ignore_error:
                    raise
            else:
