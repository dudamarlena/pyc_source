...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/rabbyt/anims.py
...
def _interpolate(method, start=None, end=None, startt=None, endt=None, dt=None, t=None, extend='constant'):
    try:
        ...
        args = dict(start=start, end=end, startt=startt, endt=endt, dt=dt, t=t, extend=extend, method=method)
...
def chain(*anims):
    ...
    if sequence_count:
        if sequence_count != len(anims):
            raise ValueError('Arguments must be either all Anims or all sequences!  (%d out of %d passed arguments were sequences.)' % (sequence_count, len(anims)))
