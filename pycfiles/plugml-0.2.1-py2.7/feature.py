...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\plugml\feature.py
...
class TextFeature(RawFeature):
    ...
    def __init__(self, data, col):
        self._tokenizer = RegexpTokenizer('\\w\\w+')
    ...
    def _setup(self):
        self._vec = TfidfVectorizer(analyzer=lambda x: x, sublinear_tf=True, smooth_idf=True, norm='l2', min_df=2, max_df=0.9)
        self._vec.fit(self.data)
        self.featNames = self._vec.get_feature_names()
        self.dim = len(self.featNames)
    ...
    def _tokenizeText(self, text):
        ...
        tokens = [ self._prefix + self._stemmer.stem(t) for t in tokens if t not in self._stopwords
                 ]
        return tokens