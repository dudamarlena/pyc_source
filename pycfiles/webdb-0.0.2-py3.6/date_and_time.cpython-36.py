...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/webdb/typehandling/date_and_time.py
# Compiled at: 2018-02-14 05:19:40
...
class DatetimeAdapter(datetime.datetime):
    ...
    def from_dict(dct):
        ...
        if dct['__type__'] != 'datetime':
            raise ConversionException('wrong type: {}'.format(dct['__type__']))
        my_dct = {'year':0,  'month':1,  'day':1,  'hour':0, 
         'minute':0,  'second':0,  'microsecond':0, 
         'utcoffset':0}
        my_dct.update(dct)
        return DatetimeAdapter(my_dct['year'], my_dct['month'], my_dct['day'], my_dct['hour'], my_dct['minute'], my_dct['second'], my_dct['microsecond'], datetime.timezone(datetime.timedelta(minutes=(my_dct['utcoffset']))))
    ...
    def copy(self, datetime):
        """
                ...
                """
        return DatetimeAdapter(datetime.year, datetime.month, datetime.day, datetime.hour, datetime.minute, datetime.second, datetime.microsecond, datetime.tzinfo)
...
class TimeAdapter(datetime.time):
    ...
    def from_dict(dct):
        ...
        my_dct.update(dct)
        return TimeAdapter(my_dct['hour'], my_dct['minute'], my_dct['second'], my_dct['microsecond'], datetime.timezone(datetime.timedelta(minutes=(my_dct['utcoffset']))))
    ...
    def copy(self, time):
        """
                ...
                """
        return TimeAdapter(time.hour, time.minute, time.second, time.microsecond, time.tzinfo)