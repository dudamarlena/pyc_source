...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/webdb/adapter/abstractsql.py
# Compiled at: 2018-02-14 06:57:50
...
class AbstractSQLDB(AbstractDB):
    ...
    def __init__(self):
        self._con = None
        self.handlers = {'INSERT':self.handle_insert,  'UPDATE':self.handle_update, 
    ...
    def handle_request(self, request, inject=None, inject_as=None):
        if request['operation'] not in self.handlers:
            raise NotSupportedError(request['operation'])
        table_names = self.get_table_names()
        if request['table'] not in table_names:
    ...
    def handle_insert(self, request, inject, inject_as):
        ...
        sql_string = 'INSERT INTO {}({}) VALUES({})'.format(request['table'], sql_columns, sql_values)
