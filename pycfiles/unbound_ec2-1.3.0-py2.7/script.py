...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/unbound_ec2/script.py
...
def init(id, cfg):
    ...
    _lookup = lookup.DirectLookup(ec2, conf.main['zone'], conf.lookup_filters, conf.lookup['tag_name_include_domain']) if conf.lookup['type'] == 'direct' else lookup.CacheLookup(ec2, conf.main['zone'], conf.lookup_filters, conf.lookup['tag_name_include_domain'])
    _server = server.Authoritative(conf.main['zone'], conf.main['reverse_zone'], conf.main['ttl'], _lookup, conf.main['ip_order'], conf.main['forwarded_zones']) if conf.server['type'] == 'authoritative' else server.Caching(conf.main['zone'], conf.main['reverse_zone'], conf.main['ttl'], _lookup, conf.main['ip_order'], conf.main['forwarded_zones'])
