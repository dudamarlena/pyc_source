...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/django_mce_pygments/views.py
# Compiled at: 2016-09-12 17:17:35
import os
...
def pygments(request, template_name='pygments/pygments.html'):
    if request.method == 'POST':
        ...
        if linenos == 'true':
            ...
            linenos = False
        highlighted_code = highlight(code, get_lexer_by_name(lang), HtmlFormatter(cssclass='pygments pygments_%s' % lang, full=False, style=style, noclasses=True, linenos=linenos, linenostart=linenostart, hl_lines=highlight_lines))
        if srcattach:
            source_dir = os.path.join(settings.STATIC_ROOT, getattr(settings, 'SOURCE_CODE_FILE_DIR', 'static/source/'))
            file_name, source_file = make_unique_file(source_dir, src_attach_name)
        ...
        mce_popup_location = getattr(settings, 'MCE_POPUP_LOCATION', 'grappelli/tinymce/jscripts/tiny_mce/tiny_mce_popup.js')
        jquery_location = getattr(settings, 'JQUERY_LOCATION', 'mezzanine/js/jquery-1.7.1.min.js')
        context = {'styles': styles, 'langs': langs, 'mce_popup_location': mce_popup_location, 
...
def make_unique_file(source_dir, src_attach_name):
    ...
    while os.path.exists(current_path):
        modified_name = ('_%s' % attempt_count).join(os.path.splitext(src_attach_name))
        attempt_count += 1
        current_path = os.path.join(source_dir, modified_name)

    return (
     modified_name, open(current_path, 'w'))