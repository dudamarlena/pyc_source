...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyowm/weatherapi25/cityidregistry.py
...
class CityIDRegistry:
    MATCHINGS = {'exact': lambda city_name, toponym: city_name == toponym, 
    ...
    def ids_for(self, city_name, country=None, matching='nocase'):
        ...
        if matching not in self.MATCHINGS:
            raise ValueError('Unknown type of matching: allowed values are %s' % ', '.join(self.MATCHINGS))
    ...
    def locations_for(self, city_name, country=None, matching='nocase'):
        ...
        if country is not None:
            if len(country) != 2:
                raise ValueError('Country must be a 2-char string')
        splits = self._filter_matching_lines(city_name, country, matching)
        return [Location(item[0], float(item[3]), float(item[2]), int(item[1]), item[4]) for item in splits]
