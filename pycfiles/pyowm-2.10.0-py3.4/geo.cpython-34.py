...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyowm/utils/geo.py
# Compiled at: 2018-12-21 14:52:45
# Size of source mod 2**32: 13215 bytes
import json, math, geojson
...
def assert_is_lat(val):
    ...
    if not type(val) is float:
        assert type(val) is int, 'Value must be a number'
...
class Point(Geometry):
    ...
    def bounding_square_polygon(self, inscribed_circle_radius_km=10.0):
        """
         ...
         """
        if not isinstance(inscribed_circle_radius_km, int):
            assert isinstance(inscribed_circle_radius_km, float)
            assert inscribed_circle_radius_km > 0.0, 'Radius must be greater than zero'
            rad_distance = float(inscribed_circle_radius_km) / EARTH_RADIUS_KM
            bb_min_lat_deg = self.lat * math.pi / 180.0 - rad_distance
            bb_max_lat_deg = self.lat * math.pi / 180.0 + rad_distance
            if bb_min_lat_deg > math.radians(-90) and bb_max_lat_deg < math.radians(90):
            ...
            bb_min_lat = bb_min_lat_deg * 180.0 / math.pi
            bb_max_lat = bb_max_lat_deg * 180.0 / math.pi
            bb_min_lon = bb_min_lon_deg * 180.0 / math.pi
            bb_max_lon = bb_max_lon_deg * 180.0 / math.pi
            return Polygon([
             [
              [
