...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyowm/agroapi10/agro_manager.py
# Compiled at: 2018-12-21 14:52:45
# Size of source mod 2**32: 14964 bytes
"""
...
class AgroManager(object):
    ...
    def create_polygon(self, geopolygon, name=None):
        ...
        data = dict()
        data['geo_json'] = {'type': 'Feature', 
         'properties': {},  'geometry': geopolygon.as_dict()}
        if name is not None:
            data['name'] = name
        status, payload = self.http_client.post(POLYGONS_URI, params={'appid': self.API_key}, data=data, headers={'Content-Type': 'application/json'})
    ...
    def get_polygons(self):
        ...
        """
        status, data = self.http_client.get_json(POLYGONS_URI, params={'appid': self.API_key}, headers={'Content-Type': 'application/json'})
    ...
    def get_polygon(self, polygon_id):
        ...
        """
        status, data = self.http_client.get_json(NAMED_POLYGON_URI % str(polygon_id), params={'appid': self.API_key}, headers={'Content-Type': 'application/json'})
    ...
    def update_polygon(self, polygon):
        ...
        status, _ = self.http_client.put(NAMED_POLYGON_URI % str(polygon.id), params={'appid': self.API_key}, data=dict(name=polygon.name), headers={'Content-Type': 'application/json'})
    ...
    def soil_data(self, polygon):
        ...
        polyd = polygon.id
        status, data = self.http_client.get_json(SOIL_URI, params={'appid': self.API_key,  'polyid': polyd}, headers={'Content-Type': 'application/json'})
        the_dict = dict()
    ...
    def download_satellite_image(self, metaimage, x=None, y=None, zoom=None, palette=None):
        ...
        if isinstance(metaimage, MetaPNGImage):
            ...
            status, data = self.http_client.get_png(prepared_url, params=params)
        ...
        if isinstance(metaimage, MetaGeoTiffImage):
            prepared_url = metaimage.url
            status, data = self.http_client.get_geotiff(prepared_url, params=params)
            img = Image(data, metaimage.image_type)
            return SatelliteImage(metaimage, img, downloaded_on=timeutils.now(timeformat='unix'), palette=palette)
        if isinstance(metaimage, MetaTile):
    ...
    def stats_for_satellite_image(self, metaimage):
        ...
        :return: dict
        """
        if metaimage.preset != PresetEnum.EVI:
            if metaimage.preset != PresetEnum.NDVI:
