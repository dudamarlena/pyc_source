...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyowm/stationsapi30/stations_manager.py
# Compiled at: 2018-12-21 14:52:45
# Size of source mod 2**32: 11000 bytes
"""
...
class StationsManager(object):
    ...
    def get_stations(self):
        ...
        """
        status, data = self.http_client.get_json(STATIONS_URI, params={'appid': self.API_key}, headers={'Content-Type': 'application/json'})
    ...
    def get_station(self, id):
        ...
        """
        status, data = self.http_client.get_json(NAMED_STATION_URI % str(id), params={'appid': self.API_key}, headers={'Content-Type': 'application/json'})
    ...
    def create_station(self, external_id, name, lat, lon, alt=None):
        ...
        status, payload = self.http_client.post(STATIONS_URI, params={'appid': self.API_key}, data=dict(external_id=external_id, name=name, lat=lat, lon=lon, alt=alt), headers={'Content-Type': 'application/json'})
    ...
    def update_station(self, station):
        ...
        *pyowm.stationsapi30.station.Station* object with all of its fields
        ...
        status, _ = self.http_client.put(NAMED_STATION_URI % str(station.id), params={'appid': self.API_key}, data=dict(external_id=station.external_id, name=station.name, lat=station.lat, lon=station.lon, alt=station.alt), headers={'Content-Type': 'application/json'})
    ...
    def delete_station(self, station):
        ...
        *pyowm.stationsapi30.station.Station*, along with all its related
        ...
        """
        assert station.id is not None
        status, _ = self.http_client.delete(NAMED_STATION_URI % str(station.id), params={'appid': self.API_key}, headers={'Content-Type': 'application/json'})
    ...
    def send_measurement(self, measurement):
        ...
        status, _ = self.http_client.post(MEASUREMENTS_URI, params={'appid': self.API_key}, data=[
         self._structure_dict(measurement)], headers={'Content-Type': 'application/json'})
    ...
    def get_measurements(self, station_id, aggregated_on, from_timestamp, to_timestamp, limit=100):
        ...
        assert limit >= 0
        query = {'appid': self.API_key,  'station_id': station_id, 
         'type': aggregated_on, 
         'from': from_timestamp, 
         'to': to_timestamp, 
         'limit': limit}
        status, data = self.http_client.get_json(MEASUREMENTS_URI, params=query, headers={'Content-Type': 'application/json'})
