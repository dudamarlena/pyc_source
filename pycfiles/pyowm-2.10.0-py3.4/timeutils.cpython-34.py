...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyowm/utils/timeutils.py
...
def tomorrow(hour=None, minute=None):
    ...
    return datetime(tomorrow_date.year, tomorrow_date.month, tomorrow_date.day, hour, minute, 0)
...
def yesterday(hour=None, minute=None):
    ...
    return datetime(yesterday_date.year, yesterday_date.month, yesterday_date.day, hour, minute, 0)
...
def _timedelta_hours(offset, date=None):
    if date is None:
        ...
        assert isinstance(date, datetime), __name__ + ": 'date' must be a datetime.datetime object"
