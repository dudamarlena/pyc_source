...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kiddiepool/pool.py
# Compiled at: 2018-03-12 20:21:56
import random
try:
    import queue
except ImportError:
...
from kiddiepool.exceptions import KiddiePoolEmpty, KiddiePoolMaxAttempts, TidePoolBindError, TidePoolAlreadyBoundError
...
class KiddiePool(object):
    ...
    def __init__(self, hosts, connect_attempts=DEFAULT_CONNECT_ATTEMPTS, connection_factory=None, connection_options=None, max_size=DEFAULT_POOL_MAX, pool_timeout=DEFAULT_POOL_TIMEOUT):
    ...
    def _connect(self, conn):
        ...
        raise KiddiePoolMaxAttempts('Failed to connect to any servers after %d attempts on %r' % (
    ...
    def get(self):
        ...
        try:
            ...
            raise KiddiePoolEmpty('All %d connections checked out' % self.max_size)
...
class TidePool(KiddiePool):
    ...
    def __init__(self, zk_session, znode_parent, deferred_bind=False, lock=threading_lock, **kwargs):
        """
    ...
    def bind(self):
        ...
        if not self._bind_lock.acquire(False):
            raise TidePoolAlreadyBoundError('Cannot bind previously bound TidePool instance.')
        self._data_watcher = self._zk_session.DataWatch(self._znode_parent, func=self._handle_znode_parent_change)
        if self._data_watcher._session_watcher not in self._zk_session.state_listeners:
    ...
    def unbind(self):
        ...
        if self._data_watcher:
            ...
            self._zk_session._data_watchers[self._znode_parent].discard(self._data_watcher._watcher)
            self._data_watcher = None
        if self._child_watcher:
            self._zk_session.remove_listener(self._child_watcher)
            self._zk_session._child_watchers[self._znode_parent].discard(self._child_watcher._watcher)
            self._child_watcher = None
    ...
    def _handle_znode_parent_change(self, data, stats):
        ...
        if data is not None and self._znode_parent not in self._zk_session._child_watchers:
            try:
                self._child_watcher = self._zk_session.ChildrenWatch(self._znode_parent, func=self.set_hosts)
            except self.NoNodeError:
                pass
