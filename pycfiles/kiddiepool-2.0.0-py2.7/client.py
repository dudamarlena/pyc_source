...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kiddiepool/client.py
# Compiled at: 2018-03-12 18:12:15
import socket
from kiddiepool.exceptions import KiddieClientSendFailure, KiddieClientRecvFailure
...
class KiddieClient(object):
    ...
    def _recv(self, size):
        ...
        try:
            ...
            raise self.RecvException('Failed to recv %s bytes. Last exception: %r ' % (size, e))
    ...
    def _sendall(self, request, attempts=None):
        ...
        for attempt in range(attempts):
            ...
            raise self.SendException('Failed to send request (%d bytes) after %d attempts. Last exception: %r' % (
             len(request), attempts, e))
