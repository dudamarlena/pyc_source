...
# [GCC 8.4.0]
# Embedded file name: /home/gianluca/Pubblici/GIThub/MASTER/Videomass/videomass3/vdms_panels/presets_manager.py
# Compiled at: 2020-05-11 07:27:34
# Size of source mod 2**32: 52502 bytes
import wx, os
...
class PrstPan(wx.Panel):
    ...
    def __init__(self, parent, path_srcShare, path_confdir, PWD, OS, iconanalyzes, iconpeaklevel, btn_color, fontBtncolor):
        """
        """
        self.src_prst = os.path.join(path_srcShare, 'presets')
        self.user_prst = os.path.join(path_confdir, 'presets')
        self.PWD = PWD
        self.oS = OS
        self.parent = parent
        self.txtcmdedited = False
        self.normdetails = []
        prst = sorted([os.path.splitext(x)[0] for x in os.listdir(self.user_prst) if os.path.splitext(x)[1] == '.prst'])
        self.btnC = btn_color
        self.fBtnC = fontBtncolor
        wx.Panel.__init__(self, parent, -1)
        sizer_base = wx.BoxSizer(wx.VERTICAL)
        self.list_ctrl = wx.ListCtrl(self, (wx.ID_ANY), style=(wx.LC_REPORT | wx.SUNKEN_BORDER | wx.LC_SINGLE_SEL))
        ...
        lab_prfl = wx.StaticText(nb1_p1, wx.ID_ANY, _('Select a preset from the drop down:'))
        grd_prst.Add(lab_prfl, 0, wx.ALIGN_CENTER_HORIZONTAL | wx.ALIGN_CENTER_VERTICAL, 0)
        self.cmbx_prst = wx.ComboBox(nb1_p1, (wx.ID_ANY), choices=prst,
          size=(200, -1),
        ...
        box_cmd1 = wx.StaticBoxSizer(wx.StaticBox(nb1_p2, wx.ID_ANY, _('1-PASS')), wx.VERTICAL)
        grd_cmd.Add(box_cmd1, 0, wx.ALL | wx.EXPAND | wx.ALIGN_CENTER_HORIZONTAL | wx.ALIGN_CENTER_VERTICAL, 15)
        self.txt_1cmd = wx.TextCtrl(nb1_p2, (wx.ID_ANY), '', style=(wx.TE_MULTILINE | wx.TE_PROCESS_ENTER))
        box_cmd1.Add(self.txt_1cmd, 1, wx.ALL | wx.EXPAND | wx.ALIGN_CENTER_HORIZONTAL | wx.ALIGN_CENTER_VERTICAL, 15)
        box_cmd2 = wx.StaticBoxSizer(wx.StaticBox(nb1_p2, wx.ID_ANY, _('2-PASS')), wx.VERTICAL)
        grd_cmd.Add(box_cmd2, 0, wx.ALL | wx.EXPAND | wx.ALIGN_CENTER_HORIZONTAL | wx.ALIGN_CENTER_VERTICAL, 15)
        self.txt_2cmd = wx.TextCtrl(nb1_p2, (wx.ID_ANY), '', style=(wx.TE_MULTILINE | wx.TE_PROCESS_ENTER))
        box_cmd2.Add(self.txt_2cmd, 1, wx.ALL | wx.EXPAND | wx.ALIGN_CENTER_HORIZONTAL | wx.ALIGN_CENTER_VERTICAL, 15)
        ...
        self.rdbx_norm = wx.RadioBox((self.nb1_p3), (wx.ID_ANY), (_('Audio Normalization')),
          choices=[
         'Off', 'PEAK',
         'RMS', 'EBU R128'],
          majorDimension=1,
        ...
        boxamap = wx.StaticBoxSizer(wx.StaticBox(self.nb1_p3, wx.ID_ANY, _('Audio Streams Mapping')), wx.VERTICAL)
        ...
        self.cmb_A_inMap = wx.ComboBox((self.nb1_p3), (wx.ID_ANY), choices=[
         'Auto', '1', '2', '3',
         '4', '5', '6', '7', '8'],
          size=(160, -1),
          style=(wx.CB_DROPDOWN | wx.CB_READONLY))
        grd_map.Add(self.cmb_A_inMap, 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 10)
        txtAoutmap = wx.StaticText(self.nb1_p3, wx.ID_ANY, _('Output Audio Index'))
        grd_map.Add(txtAoutmap, 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 10)
        self.cmb_A_outMap = wx.ComboBox((self.nb1_p3), (wx.ID_ANY), choices=[
         'Auto', 'All', '1', '2', '3',
         '4', '5', '6', '7', '8'],
          size=(160, -1),
          style=(wx.CB_DROPDOWN | wx.CB_READONLY))
        ...
        self.btn_voldect.SetBaseColours(startcolour=(wx.Colour(AZURE_NEON)), foregroundcolour=(wx.Colour(self.fBtnC)))
        ...
        self.btn_details.SetBaseColours(startcolour=(wx.Colour(AZURE_NEON)), foregroundcolour=(wx.Colour(self.fBtnC)))
        ...
        self.ebupanel = wx.Panel((self.nb1_p3), (wx.ID_ANY), style=(wx.TAB_TRAVERSAL))
        ...
        toolt = _('Gets maximum volume and average volume data in dBFS, then calculates the offset amount for audio normalization.')
        self.btn_voldect.SetToolTip(toolt)
        toolt = _('Limiter for the maximum peak level or the mean level (when switch to RMS) in dBFS. From -99.0 to +0.0; default for PEAK level is -1.0; default for RMS is -20.0')
        self.spin_target.SetToolTip(toolt)
        toolt = _('Integrated Loudness Target in LUFS. From -70.0 to -5.0, default is -24.0')
        self.spin_i.SetToolTip(toolt)
        toolt = _('Maximum True Peak in dBTP. From -9.0 to +0.0, default is -2.0')
        self.spin_tp.SetToolTip(toolt)
        toolt = _('Loudness Range Target in LUFS. From +1.0 to +20.0, default is +7.0')
        self.spin_lra.SetToolTip(toolt)
        toolt = _('Choose from video a specific input audio stream to work out.')
        self.cmb_A_inMap.SetToolTip(toolt)
        toolt = _('Map on the output index. Keep same input map if saving as video; to save as audio select to "all" or "Auto"')
        ...
        self.Bind(wx.EVT_LIST_ITEM_ACTIVATED, self.parent.click_start, self.list_ctrl)
    ...
    def onPopup(self, event):
        ...
        if menuItem.GetLabel() == _('New profile'):
            ...
            if menuItem.GetLabel() == _('Edit selected profile'):
                self.Editprof(self)
            else:
                if menuItem.GetLabel() == _('Delete selected profile'):
                    self.Delprof()
    ...
    def set_listctrl(self):
        ...
        path = os.path.join('%s' % self.user_prst, '%s.prst' % self.cmbx_prst.GetValue())
        ...
        try:
            ...
            try:
                wx.MessageBox(_('ERROR: Typing error on JSON keys: {}\n\nFile: "{}"\nkey malformed ?'.format(err, path)), 'Videomass', wx.ICON_ERROR, self)
                return
            finally:
                err = None
                del err
    ...
    def on_select(self, event):
        ...
        self.txt_1cmd.AppendText('%s' % array[2])
        if array[3]:
            self.txt_2cmd.Enable()
            self.txt_2cmd.AppendText('%s' % array[3])
        else:
    ...
    def New_preset_prst(self):
        ...
        """
        filename = None
        with wx.FileDialog(self, (_('Enter name for new preset')), defaultDir=(self.user_prst),
          ...
          style=(wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT)) as (fileDialog):
            ...
            try:
                ...
                wx.LogError(_("Cannot save current data in file '{}'.").format(filename))
                return

        if filename:
            wx.MessageBox(_("'Successful!\n\nA new preset has been created."), 'Videomass ', wx.ICON_INFORMATION, self)
    ...
    def Del_preset_prst(self):
        ...
        if wx.MessageBox(_('Are you sure you want to remove "{}" preset?\n\n It will be moved to the "Removals" folder in the presets directory').format(filename), _('Videomass: Please confirm'), wx.ICON_QUESTION | wx.YES_NO, self) == wx.NO:
            return
        path = os.path.join('%s' % self.user_prst, '%s.prst' % self.cmbx_prst.GetValue())
        try:
            ...
            try:
                wx.MessageBox(_("{}\n\nSorry, removal failed, I can't continue.").format(err), 'ERROR !', wx.ICON_ERROR, self)
                return
            finally:
                err = None
                del err
    ...
    def Restore(self):
        ...
        dialfile = wx.FileDialog(self, _('Videomass: Choose a videomass preset to restore '), '', '', wildcard, wx.FD_OPEN | wx.FD_FILE_MUST_EXIST)
        if dialfile.ShowModal() == wx.ID_OK:
            dirname = dialfile.GetPath()
            tail = os.path.basename(dirname)
            dialfile.Destroy()
            if wx.MessageBox(_("The following preset:\n\n'{0}'\n\nwill be imported and will overwrite the one in use.\nProceed ?").format(tail), _('Videomass: Please confirm'), wx.ICON_QUESTION | wx.YES_NO, self) == wx.NO:
                return
    ...
    def Default(self):
        ...
        """
        if wx.MessageBox(_('The selected preset will be overwritten with default profiles!\nproceed?'), _('Videomass: Please confirm'), wx.ICON_QUESTION | wx.YES_NO, self) == wx.NO:
            return
        filename = self.cmbx_prst.GetValue()
        copy = copy_restore('%s/%s.prst' % (self.src_prst, filename), '%s/%s.prst' % (self.user_prst, filename))
        if copy:
            wx.MessageBox(_('Sorry, this preset is not part of default Videomass presets.'), 'ERROR !', wx.ICON_ERROR, self)
            return
        self.reset_list()
    ...
    def Default_all(self):
        ...
        """
        if wx.MessageBox(_('WARNING: you are restoring all default presets!\nProceed?'), _('Videomass: Please confirm'), wx.ICON_QUESTION | wx.YES_NO, self) == wx.NO:
            return
    ...
    def Addprof(self):
        ...
        prstdialog = presets_addnew.MemPresets(self, 'newprofile', filename, None, t)
    ...
    def Editprof(self, event):
        ...
        """
        if array == []:
            self.parent.statusbar_msg(_('First select a profile in the list'), YELLOW)
            return
        filename = self.cmbx_prst.GetValue()
        t = _('Edit profile of the "%s" preset: ') % filename
        prstdialog = presets_addnew.MemPresets(self, 'edit', filename, array, t)
        ret = prstdialog.ShowModal()
        if ret == wx.ID_OK:
            self.reset_list()
    ...
    def Delprof(self):
        ...
        if array == []:
            ...
            filename = self.cmbx_prst.GetValue()
            if wx.MessageBox(_('Are you sure you want to delete the selected profile?'), _('Videomass: Please confirm'), wx.ICON_QUESTION | wx.YES_NO, self) == wx.NO:
                return
            path = os.path.join('%s' % self.user_prst, '%s.prst' % self.cmbx_prst.GetValue())
    ...
    def on_start(self):
        ...
        if self.rdbx_norm.GetSelection() in (1, 2):
            if self.btn_voldect.IsEnabled():
                wx.MessageBox(_('Undetected volume values! use the "Volumedetect" control button to analyze data on the audio volume.'), 'Videomass', wx.ICON_INFORMATION)
                return
        else:
            self.time_seq = self.parent.time_seq
            ...
            if array == []:
                self.parent.statusbar_msg(_('First select a profile in the list'), YELLOW)
                return
                if array[2].strip() != self.txt_1cmd.GetValue().strip() or array[3].strip() != self.txt_2cmd.GetValue().strip():
                    if not self.txtcmdedited:
                        msg = _('The selected profile command has been changed manually.\nDo you want to apply it during the conversion process?')
                        dlg = wx.RichMessageDialog(self, msg, _('Videomass: Please confirm'), wx.ICON_QUESTION | wx.YES_NO)
    ...
    def one_Pass(self, filesrc, destdir, cntmax, outext):
        ...
        if ending.ShowModal() == wx.ID_OK:
            self.parent.switch_to_processing('onepass', filesrc, outext, destdir, command, None, '', audnorm, self.logname, cntmax)
    ...
    def two_Pass(self, filesrc, destdir, cntmax, outext):
        ...
        if 'loudnorm=' in pass1:
            if self.rdbx_norm.GetSelection() == 3:
                if wx.MessageBox(_('EBU normalization is enabled twice in automation and command line. Command line has priority.\n\nDo you wish to continue?'), _('Videomass: Please confirm'), wx.ICON_QUESTION | wx.YES_NO, self) == wx.NO:
                    return
        ...
        ending = Formula(self, valupdate[0], valupdate[1], _('Starts'))
        if ending.ShowModal() == wx.ID_OK:
            self.parent.switch_to_processing(typeproc, filesrc, outext, destdir, None, [
             pass1, pass2, loudnorm], cmd_opt['AudioOutMap'], audnorm, self.logname, cntmax)
    ...
    def savepictures(self, dest, file_sources):
        ...
        if len(file_sources) == 1:
            ...
            if not self.parent.filedropselected:
                wx.MessageBox(_('To export as pictures, select one file at a time'), 'Videomass', wx.ICON_INFORMATION, self)
                return
        ...
        if ending.ShowModal() == wx.ID_OK:
            ...
            dir_destin = dest[file_sources.index(clicked)]
            try:
            ...
            command = ' '.join(cmd.split())
            self.parent.switch_to_processing('savepictures', clicked, None, None, command, None, None, None, self.logname, 1, False)
