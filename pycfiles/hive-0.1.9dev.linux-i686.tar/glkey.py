...
# [GCC 8.4.0]
# Embedded file name: /home/git/lib/python2.5/site-packages/hive/lib/glam/glkey.py
...
REGEXP_SET = {'valid_name': '^\\w+@\\w+(?:\\.\\w+)+(?:@\\w+)?$', 
   'valid_public_key': '^(?:ssh-rsa|ssh-dss) \\S+ \\S+$'}

class GLKey:
    """ Gitolite public key manager """

    def __init__(self, name, content=''):
        self.filename = os.path.join(config.GITOLITE_ADMIN_REPO_PATH, 'keydir', name + '.pub')
    ...
    def check_parameters(self):
        ...
        if not (isinstance(self.name, (str, unicode)) and isinstance(self.content, (str, unicode))):
        ...
        if self.content:
            if re.match(REGEXP_SET['valid_public_key'], self.content) == None:
                raise ValueError('Public key is invalid.')
            if self.exists():
                raise os.access(self.filename, os.R_OK) and os.access(self.filename, os.W_OK) or RuntimeError('Permission denied for file %s' % self.filename)
        elif not os.access(os.path.dirname(self.filename), os.W_OK):
            raise RuntimeError('Permission denied for directory %s' % os.path.dirname(self.filename))
        return
    ...
    def index():
        ...
        key_list = filter(lambda x: x.endswith('.pub'), os.listdir(dirpath))
    ...
    def to_dict(self):
        """ Convert to dict """
        return dict(name=self.name, pubkey=self.get_content())
