...
# [GCC 8.4.0]
# Embedded file name: /home/git/lib/python2.5/site-packages/hive/lib/routes/mapper.py
    ...
    def connect(self, *args, **kwargs):
        ...
        for (key, value) in self.kwargs.items():
            if key == 'path_prefix':
                if len(args) > 1:
                    newargs = (
                     args[0], self.kwargs[key] + args[1])
                else:
                    newargs = (
                     self.kwargs[key] + args[0],)
            elif key in kwargs:
...
class Mapper(SubMapperParent):
    ...
    def __str__(self):
        ...
        table = [('Route name', 'Methods', 'Path')] + [ (r.name or '', format_methods(r), r.routepath or '') for r in self.matchlist
                                                      ]
    ...
    def connect(self, *args, **kargs):
        r"""Create and connect a new Route to the Mapper.
    ...
    def generate(self, *args, **kargs):
        ...
        if not actionlist and not args:
            return
        (keylist, sortcache) = actionlist.get(action) or actionlist.get('*', (None, {}))
        ...
        for route in keylist:
            ...
            if path:
                ...
                if environ and environ.get('SCRIPT_NAME', '') != '' and not route.absolute and not external_static:
    ...
    def resource(self, member_name, collection_name, **kwargs):
        ...
        self.connect(('formatted_' + name_prefix + collection_name), conditions={'method': ['GET']}, **options)
