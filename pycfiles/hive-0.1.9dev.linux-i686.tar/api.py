...
# [GCC 8.4.0]
# Embedded file name: /home/git/lib/python2.5/site-packages/hive/lib/glam/api.py
# Compiled at: 2011-07-08 01:47:53
""" Gitolite admin manager
"""
import time, logging, glconfig, glkey, glgroup, glrule, gllock
from etc import config
TRY_TIMES = 10
logging.basicConfig(filename=config.LOG_FILENAME, level=logging.DEBUG, format='%(levelname)s:[func:%(funcName)s]%(message)s')
...
class User:
    ...
    def create(name, pubkey):
        ...
        return _transaction(gllock.OP_CREATE, 'Create User: %s' % name, key, key.create)
    ...
    def update(user_id, newpubkey):
        ...
        if not key.exists():
            raise HTTPNotFound
        return _transaction(gllock.OP_UPDATE, 'Update User: %s' % user_id, key, key.update, newpubkey)
    ...
    def delete(user_id):
        ...
        if not key.exists():
            raise HTTPNotFound
        return _transaction(gllock.OP_DELETE, 'Delete User: %s' % user_id, key, key.delete)
...
class Group:
    ...
    def create(repo_id, name, member_list):
        ...
        return _transaction(gllock.OP_CREATE, 'Create Group: %s' % group_id, conf, bind)
    ...
    def update(repo_id, name, new_member_list):
        ...
        return _transaction(gllock.OP_UPDATE, 'Update Group: %s' % group_id, conf, bind)
    ...
    def delete(repo_id, name):
        ...
        return _transaction(gllock.OP_DELETE, 'Delete Group: %s' % group_id, conf, bind)
...
class Repo:
    ...
    def create(name):
        ...
        if _transaction(gllock.OP_CREATE, 'Create Repo: %s' % name, conf, conf.create):
            return name
    ...
    def delete(name):
        ...
        conf = glconfig.GLConfig(name)
        return _transaction(gllock.OP_DELETE, 'Delete Repo: %s' % name, conf, conf.delete)
...
class Rule:
    ...
    def create(reponame, permission, refex_list, member_list):
        ...
        return _transaction(gllock.OP_CREATE, 'Create Rule: %s' % reponame, conf, bind)
    ...
    def update(reponame, rule_id, new_member_list):
        ...
        return _transaction(gllock.OP_UPDATE, 'Update Rule: %s' % rule_id, conf, bind)
    ...
    def delete(reponame, rule_id):
        ...
        return _transaction(gllock.OP_DELETE, 'Delete Rule: %s' % rule_id, conf, bind)
