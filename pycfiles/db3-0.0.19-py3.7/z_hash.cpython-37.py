...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/ssdb/z_hash.py
...
class ZHash(Base):
    ...
    def zset(self, name, key, score: int) -> int:
        ...
        """
        return self.execute_command('zset', c(name, 1), c(key, 1), score)
    ...
    def zincr(self, name, key, num: int=1, decimals: int=0):
        ...
        """
        v = self.execute_command('zincr', c(name, 1), c(key, 1), int(num))
        if decimals:
            if v < 10 ** (decimals - 1):
    ...
    def zkeys(self, name, key_start=None, score_start=None, score_end=None, limit=None):
        ...
        return bytes_to_str(self.execute_command('zkeys', c(name), key_start, score_start, score_end, check_limit(limit)))
    ...
    def zscan(self, name, key_start=None, score_start=None, score_end=None, limit=None):
        ...
        key_start = c(key_start, False)
        score_start, score_end = deal_start_end(score_start, score_end, 'z')
        return list_to_dict(self.execute_command('zscan', c(name), key_start, score_start, score_end, check_limit(limit)), 'z')
    ...
    def multi_zset(self, name, kvs: dict):
        ...
        if kvs:
            return (self.execute_command)('multi_zset', c(name, 1), *dict_to_list(kvs))
        return 0

    def multi_zget(self, name, keys: list):
        if keys:
            return list_to_dict((self.execute_command)('multi_zget', c(name), *keys), 'z')
        return {}

    def multi_zdel(self, name, keys: list) -> int:
        if keys:
            return (self.execute_command)('multi_zdel', c(name), *keys)
        return 0