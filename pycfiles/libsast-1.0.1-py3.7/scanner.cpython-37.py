...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/libsast/scanner.py
...
class Scanner:
    ...
    def scan(self) -> dict:
        ...
        if self.options.get('match_rules'):
            results['pattern_matcher'] = PatternMatcher(self.options).scan(self.paths)
        if self.options.get('sgrep_rules'):
            results['semantic_grep'] = SemanticGrep(self.options).scan(self.paths)
        return results