...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/libsast/core_matcher/pattern_matcher.py
# Compiled at: 2020-04-14 22:39:36
# Size of source mod 2**32: 4432 bytes
"""Pattern Macher."""
import copy, os
...
class PatternMatcher:
    ...
    def scan(self, paths: list) -> dict:
        ...
        if not self.scan_rules:
            ...
            isinstance(paths, list) or logger.error('A list of path is expected')
            return
        all_files = set()
        for path in paths:
            pobj = Path(path)
    ...
    def validate_file(self, path):
        ...
        if not self.exts:
            ...
            if path.suffix.lower() not in self.exts:
                ...
                return path.exists() and path.is_file() or False
            return True
        ...
        try:
            for rule in self.scan_rules:
                try:
                    ...
                    logger.error("match_rule.py should have the match 'type' defined.")
                    return
                else:
                    try:
                        rule['pattern']
                    except KeyError:
                        logger.error("match_rule.py should have the key 'match' defined with the pattern to match")
                        return
                    else:
