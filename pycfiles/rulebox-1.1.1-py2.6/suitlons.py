...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\rulebox\suitlons.py
...
def templates(params):
    ...
    try:
        ...
        filepath = os.path.abspath(os.path.join(config['pylons.paths']['templates'], filename))
        params['string'] = ''
...
suitrules['[assign]']['var']['var'] = suitrules['[assign]']['var']['var'].copy()
...
suitrules['[try]']['var']['var'] = suitrules['[try]']['var']['var'].copy()
suitrules['[try]']['var']['var']['owner'] = c
del suitrules['[var]']
del suitrules['[var']
pylonsrules = {'[gettext]': {'close': '[/gettext]', 
                 'functions': [
                             templating.walk, gettext]}, 
   '[url': {'close': '/]', 
            'functions': [
                        templating.walk, templating.attribute, url_for], 
            'skip': True, 
            'var': {'equal': templating.default['equal'], 
