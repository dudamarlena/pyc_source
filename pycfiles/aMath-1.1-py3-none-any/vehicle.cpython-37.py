...
# Compiled at: 2020-05-07 22:28:04
# Size of source mod 2**32: 177335 bytes
import numpy as np
...
class Vehicle:
    ...
    def setDragEntryPhaseParams(self, v_switch_kms, lowAlt_km, numPoints_lowAlt, hdot_threshold):
        """
    ...
    def propogateEntryPhaseD(self, timeStep, dt, maxTimeSecs):
        ...
        while self.v_current_kms > self.v_switch_kms:
            ...
            if hdoti > self.hdot_threshold:
                ...
                terminal_apoapsis_km = self.predictApoapsisAltitudeKm_afterJettision(h_km[(-1)], theta_deg[(-1)], phi_deg[(-1)], v_kms[(-1)], gamma_deg[(-1)], psi_deg[(-1)], drange_km[(-1)], heatload[(-1)], maxTimeSecs, dt, 0.0, self.density_mes_int)
            else:
                terminal_apoapsis_km = 0.0
            h_current_km = h_km[(-1)]
            v_current_kms = v_kms[(-1)]
    ...
    def predictApoapsisAltitudeKm_afterJettision(self, h0_km, theta0_deg, phi0_deg, v0_kms, gamma0_deg, psi0_deg, drange0_km, heatLoad0, t_sec, dt, delta_deg, density_mes_int):
        """
                ...
                """
        t_minc, h_kmc, v_kmsc, phi_degc, psi_degc, theta_degc, gamma_degc, drange_kmc, exitflag, acc_net_g, dyn_pres_atm, stag_pres_atm, q_stag_total, heatload, acc_drag_g = self.propogateEntry2D(h0_km, theta0_deg, phi0_deg, v0_kms, gamma0_deg, psi0_deg, drange0_km, heatLoad0, t_sec, dt, delta_deg, density_mes_int)
        terminal_apoapsis_km = self.compute_ApoapsisAltitudeKm(self.planetObj.RP + h_kmc[(-1)] * 1000.0, v_kmsc[(-1)] * 1000.0, gamma_degc[(-1)] * np.pi / 180.0, theta_degc[(-1)] * np.pi / 180.0, phi_degc[(-1)] * np.pi / 180.0, psi_degc[(-1)] * np.pi / 180.0)
        return terminal_apoapsis_km

    def propogateEntry2D(self, h0_km, theta0_deg, phi0_deg, v0_kms, gamma0_deg, psi0_deg, drange0_km, heatLoad0, t_sec, dt, delta_deg, density_mes_int):
        """
                Utility propogator routine for prediction of atmospheric exit
                conditions which is then supplied to the apoapis prediction 
                module.
    ...
    def propogateExitPhaseD(self, timeStep, dt, maxTimeSecs):
        ...
        while h_current_km < h_skip_km:
            ...
            heatload = np.concatenate((heatload, heatload_c), axis=0)
            terminal_apoapsis_km = self.compute_ApoapsisAltitudeKm(self.planetObj.RP + h_km[(-1)] * 1000.0, v_kms[(-1)] * 1000.0, gamma_deg[(-1)] * np.pi / 180.0, theta_deg[(-1)] * np.pi / 180.0, phi_deg[(-1)] * np.pi / 180.0, psi_deg[(-1)] * np.pi / 180.0)
            if hi > self.planetObj.h_skip - 1000.0:
                break
            if hi < 70000.0:
                break
            h_current_km = h_km[(-1)]
    ...
    def setupMonteCarloSimulationD(self, NPOS, NMONTE, atmfiles, heightCol, densLowCol, densAvgCol, densHighCol, densTotalCol, heightInKmFlag, nominalEFPA, EFPA_1sigma_value, nominalbeta1, beta1_1sigma_value, timeStepEntry, timeStepExit, dt, maxTimeSecs):
        """
