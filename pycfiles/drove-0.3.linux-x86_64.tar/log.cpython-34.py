...
# [GCC 8.4.0]
# Embedded file name: /home/ajdiaz/env/drove/lib/python3.4/site-packages/drove/util/log.py
...
class Logger(logging.getLoggerClass()):
    __doc__ = "Create a modular logger for an application. Usually you don't want\n    to use this class directly, but use :func:`getLogger` method instead.\n    "

    def __init__(self, name=__name__, syslog=False, console=False, logfile=False, logfile_size=0, logfile_keep=0, loglevel='info', encoded_priorities={'daemon': 'alert'}):
        super(Logger, self).__init__(name, LOGLEVELS.get(loglevel, logging.INFO))
        if syslog:
            if sys.platform in ('win32', 'cygwin'):
                getDefaultLogger().warning('Current platform does ' + 'not support syslog')
            else:
                if syslog is True:
        ...
        if logfile:
            handler = RotatingFileHandler(logfile, maxBytes=logfile_size, backupCount=logfile_keep)
            handler.setFormatter(logging.Formatter(fmt=LOG_FORMAT))
            self.addHandler(handler)
        if console:
