...
# [GCC 8.4.0]
# Embedded file name: /home/ajdiaz/env/drove/lib/python3.4/site-packages/drove/config.py
...
class ConfigError(Exception):
    __doc__ = 'Models an exception reading config.'
...
class Config(dict):
    ...
    def reloadfd(self, fd, path):
        ...
        if 'include' in self:
            if self['include'][0] != '/':
                include = os.path.join(path, self['include'])
            else:
    ...
    def get(self, key, default=CONFIG_UNSET):
        ...
        if '.' in key:
            ...
            if default != CONFIG_UNSET:
                return default
            raise ConfigError('Required config parameter ' + "'%s' not found" % (key,))
