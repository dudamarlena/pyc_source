...
# [GCC 8.4.0]
# Embedded file name: /home/ajdiaz/env/drove/lib/python3.4/site-packages/drove/package.py
# Compiled at: 2015-02-18 10:36:05
...
package(s).
"""
import os, sys, glob, stat, json, shutil, tarfile, pkgutil, contextlib
...
class PackageError(Exception):
    __doc__ = 'Models an error related with a malformed package'


def find_package(path, pattern='*.*'):
    for dir in path:
        for version in glob.glob(os.path.join(os.path.expanduser(dir), '%s.version' % (pattern,))):
...
class Package(object):
        ...
        for candidate in candidates:
            if os.path.isdir(candidate):
                ...
                for finder, author, ispkg in pkgutil.iter_modules([pdir]):
                    if ispkg:
                        for finder, module, ispkg in pkgutil.iter_modules([os.path.join(pdir, author)]):
                            yield (
                             module, author, pdir)
    ...
    def from_tarballfd(kls, fd, plugin_dir, upgrade=False):
        ...
        with temp.directory() as (tmp_dir):
            ...
            if len(candidates) < 1:
                raise PackageError('Package dot not contains packages ' + 'inside.')
            if len(candidates) > 1:
                raise PackageError('Package contains more than ' + 'one package inside.')
            plugin, author, pdir = candidates[0]
            package = Package(author, plugin, version, pdir, plugin_dir)
    ...
    def __init__(self, author, plugin, version, new_version_path='', current_version_path=''):
        self.author = author
        self.plugin = plugin
        ...
        self.version_file = os.path.join(self.current_version_path, '%s.%s.version' % (self.author, self.plugin))
        ...
        for finder, module, ispkg in pkgutil.iter_modules([
         os.path.join(self.current_version_path, self.author)]):
            if module == self.plugin:
                if ispkg:
                    shutil.rmtree(os.path.join(finder.path, module), ignore_errors=True)
                else:
    ...
    def install_requirements(self):
        ...
        """
        requirements = os.path.abspath(os.path.join(self.new_version_path, '..', 'requirements.txt'))
        if os.path.exists(requirements):
            try:
                from pip import main as pip_main
            except ImportError:
                raise PackageError('This module needs python dependencies. ' + 'You need pip to install dependencies, ' + 'please install pip libraries before.')
    ...
    def install(self, version, install_global=False):
        ...
        vfile = os.path.join(self.current_version_path, '%s.%s.version' % (self.author, self.plugin))
