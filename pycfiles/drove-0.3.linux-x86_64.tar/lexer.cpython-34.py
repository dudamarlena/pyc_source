...
# [GCC 8.4.0]
# Embedded file name: /home/ajdiaz/env/drove/lib/python3.4/site-packages/drove/util/lexer.py
...
class Lexer(object):
    ...
    def parse(self, text, whitespace=' \t\r\n', wordchars='.,_-:;*><=!', commenters='#', quotes='\'"', terminators='.;,'):
        """Parse a text
        ...
        for token in lex:
            ...
            if token[(-1)] in terminators and len(words) != 0:
                if words[0] in self.item_parsers:
                    args = [x for x in words[1:] if x not in self.ignore_words[words[0]]]
