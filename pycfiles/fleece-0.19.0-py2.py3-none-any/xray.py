...
# [GCC 8.4.0]
# Embedded file name: /Users/bruc5529/git/fleece/fleece/xray.py
# Compiled at: 2020-05-01 15:24:33
...
import json, os, socket, string, threading, time, uuid
...
def send_segment_document_to_xray_daemon(segment_document):
    ...
    message = ('{header}\n{document}').format(header=json.dumps(XRAY_DAEMON_HEADER), document=json.dumps(segment_document, ensure_ascii=False, cls=StringJSONEncoder))
    send_data_on_udp(ip_address=xray_daemon.ip_address, port=xray_daemon.port, data=message)


def send_subsegment_to_xray_daemon(subsegment_id, parent_id, start_time, end_time=None, name=None, namespace='remote', extra_data=None):
    ...
    segment_document = {'type': 'subsegment', 
       'id': subsegment_id, 
    ...
    if end_time is None:
        ...
        segment_document.update({'end_time': end_time, 
           'name': name, 
           'namespace': namespace})
        segment_document.update(extra_data)
    LOGGER.debug('Prepared segment document for X-Ray Daemon', segment_document=segment_document)
...
def generic_xray_wrapper(wrapped, instance, args, kwargs, name, namespace, metadata_extractor, error_handling_type=ERROR_HANDLING_GENERIC):
    ...
    if not get_trace_id().sampled:
        ...
        send_subsegment_to_xray_daemon(subsegment_id=subsegment_id, parent_id=parent_id, start_time=start_time)
        try:
            try:
            ...
            end_time = time.time()
            extra_data = metadata_extractor(wrapped=wrapped, instance=instance, args=args, kwargs=kwargs, return_value=return_value)
            extra_data['error'] = error
            if error:
                extra_data['cause'] = cause
            if callable(name):
                name = name(wrapped, instance, args, kwargs)
            send_subsegment_to_xray_daemon(subsegment_id=subsegment_id, parent_id=parent_id, start_time=start_time, end_time=end_time, name=name, namespace=namespace, extra_data=extra_data)
...
def trace_xray_subsegment(skip_args=False):
    ...
    def wrapper(wrapped, instance, args, kwargs):
        metadata_extractor = noop_function_metadata if skip_args else extract_function_metadata
        return generic_xray_wrapper(wrapped, instance, args, kwargs, name=get_function_name, namespace='local', metadata_extractor=metadata_extractor)
...
def extract_aws_metadata(wrapped, instance, args, kwargs, return_value):
    ...
    LOGGER.debug('Extracting AWS metadata', args=args, kwargs=kwargs)
    ...
    metadata = {'aws': {'operation': operation_name, 
               'region': region_name}}
    ...
    if response_metadata is not None:
        metadata['http'] = {'response': {'status': response_metadata['HTTPStatusCode']}}
...
def xray_botocore_api_call(wrapped, instance, args, kwargs):
    ...
    return generic_xray_wrapper(wrapped, instance, args, kwargs, name=get_service_name, namespace='aws', metadata_extractor=extract_aws_metadata, error_handling_type=ERROR_HANDLING_BOTOCORE)


def monkey_patch_botocore_for_xray():
    """Explicit way to monkey-patch botocore to trace AWS API calls."""
    wrapt.wrap_function_wrapper('botocore.client', 'BaseClient._make_api_call', xray_botocore_api_call)
...
def extract_http_metadata(wrapped, instance, args, kwargs, return_value):
    ...
    """
    response = return_value
    LOGGER.debug('Extracting HTTP metadata', args=args, kwargs=kwargs)
    if 'request' in kwargs:
        request = kwargs['request']
    else:
        request = args[0]
    metadata = {'http': {'request': {'method': request.method.upper(), 
                            'url': request.url}}}
    if response is not None:
        metadata['http']['response'] = {'status': response.status_code}
    return metadata
...
def monkey_patch_requests_for_xray():
    ...
    wrapt.wrap_function_wrapper('requests.sessions', 'Session.send', xray_requests_send)
