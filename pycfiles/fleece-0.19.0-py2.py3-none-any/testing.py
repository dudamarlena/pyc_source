...
# [GCC 8.4.0]
# Embedded file name: /Users/bruc5529/git/fleece/fleece/testing.py
# Compiled at: 2019-11-06 12:49:13
import copy, datetime, random, uuid
...
class LambdaContext(object):
    ...
    def __init__(self, function_name='test_function', function_version=1, remaining_time_in_milli=9999, memory_limit_in_mb=256, region='us-east-1', account_id='999999999999', stage='test_stage', aws_request_id=None, client_context=None):
        """
    ...
    def _generate_log_stream_name(self):
        ...
        return stream_format.format(year=datetime.datetime.year, month=datetime.datetime.month, day=datetime.datetime.day, iterator=str(random.randint(1, 999)), random_id=str(uuid.uuid4()).replace('-', ''))
    ...
    def _generate_function_arn(self):
        arn_format = 'arn:aws:lambda:{region}:{account_id}:function:{function_name}:{stage}'
        return arn_format.format(region=self._region, account_id=self._account_id, function_name=self.function_name, stage=self._stage)
...
class LambdaEvent(object):
    ...
    def __init__(self, body=None, gateway=None, header=None, operation=None, path=None, querystring=None, requestor=None):
    ...
    def generate(self, merge_with_default=True, **kwargs):
        ...
        querystring = self._generate_querystring(merge_with_default, kwargs.get('querystring', {}))
        requestor = self._generate_requestor(merge_with_default, kwargs.get('requestor', {}))
...
class LambdaRequestGenerator(object):
    ...
    def generate_request(self, **kwargs):
        request = format_event(event=self.event.generate(**kwargs), context=self.context)
        return request
