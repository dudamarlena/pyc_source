...
# [GCC 8.4.0]
# Embedded file name: /Users/bruc5529/git/fleece/fleece/cli/run/run.py
# Compiled at: 2019-11-06 12:49:13
import argparse, os, subprocess, sys, boto3, requests, yaml
...
FAWS_API_URL = 'https://accounts.api.manage.rackspace.com/v0/awsAccounts/{0}/credentials'
...
def parse_args(args):
    ...
    parser = argparse.ArgumentParser(prog='fleece run', description='Run command in environment with AWS credentials from Rackspace FAWS API')
    parser.add_argument('--username', '-u', type=str, help='Rackspace username. Can also be set via RS_USERNAME environment variable')
    parser.add_argument('--apikey', '-k', type=str, help='Rackspace API key. Can also be set via RS_API_KEY environment variable')
    parser.add_argument('--config', '-c', type=str, default='./environments.yml', help='Path to YAML config file with defined accounts and aliases. Default is ./environments.yml')
    parser.add_argument('--account', '-a', type=str, help='AWS account number. Cannot be used with `--environment`')
    parser.add_argument('--environment', '-e', type=str, help='Environment alias to AWS account defined in config file. Cannot be used with `--account`')
    parser.add_argument('--stage', '-s', type=str, help='Stage name defined in config file which links to an environment. Cannot be used with `--account`')
    parser.add_argument('--role', '-r', type=str, help='Role name to assume after obtaining credentials from FAWS API')
    if run_args is None:
...
def assume_role(credentials, account, role):
    ...
    sts = boto3.client('sts', aws_access_key_id=credentials['accessKeyId'], aws_secret_access_key=credentials['secretAccessKey'], aws_session_token=credentials['sessionToken'])
    resp = sts.assume_role(RoleArn=('arn:aws:sts::{}:role/{}').format(account, role), RoleSessionName='fleece_assumed_role')
    return {'accessKeyId': resp['Credentials']['AccessKeyId'], 
...
def get_account(config, environment, stage=None):
    ...
    for env in config.get('environments', []):
        if env.get('name') == environment:
            ...
            role = env.get('role')
            username = os.environ.get(env.get('rs_username_var')) if env.get('rs_username_var') else None
            apikey = os.environ.get(env.get('rs_apikey_var')) if env.get('rs_apikey_var') else None

    if not account:
        sys.exit(ACCT_NOT_FOUND_ERROR.format(environment))
    return (
     account, role, username, apikey)
...
def get_aws_creds(account, tenant, token):
    ...
    headers = {'X-Auth-Token': token, 
       'X-Tenant-Id': tenant}
    response = requests.post(url, headers=headers, json={'credential': {'duration': '3600'}})
    if not response.ok:
...
def get_rackspace_token(username, apikey):
    ...
    """
    auth_params = {'auth': {'RAX-KSKEY:apiKeyCredentials': {'username': username, 
                                                'apiKey': apikey}}}
...
def run(args):
    ...
    if args.environment or args.stage:
        ...
        account, role, cfg_username, cfg_apikey = get_account(config, args.environment, args.stage)
    else:
