...
# [GCC 8.4.0]
# Embedded file name: /Users/bruc5529/git/fleece/fleece/log.py
# Compiled at: 2020-05-01 14:52:25
...
class RetryHandler(logging.Handler):
    ...
    def __init__(self, handler, max_retries=5, backoff_base=0.1, backoff_cap=1, ignore_errors=True):
...
def _configure_logger(logger_factory=None, wrapper_class=None):
    ...
    structlog.configure(processors=[
     ...
     structlog.processors.JSONRenderer(sort_keys=True)], context_class=WRAPPED_DICT_CLASS, logger_factory=logger_factory, wrapper_class=wrapper_class, cache_logger_on_first_use=True)
...
def get_logger(name=None, level=None, stream=DEFAULT_STREAM, clobber_root_handler=True, logger_factory=None, wrapper_class=None):
...
def initial_trace_and_context_binds(logger, trace_id, lambda_context):
    ...
    return logger.new(trace_id=trace_id, lambda_context={'function_name': lambda_context.function_name, 
