...
# [GCC 8.4.0]
# Embedded file name: /Users/bruc5529/git/fleece/fleece/authpolicy.py
...
class AuthPolicy(object):
    ...
    def __init__(self, aws_account_id, principal=None, context=None, rest_api_id=None, region=None, stage=None):
    ...
    def _add_method(self, effect, verb, resource, conditions):
        ...
        if resource[:1] == '/':
            resource = resource[1:]
        resource_arn = 'arn:aws:execute-api:' + self.region + ':' + self.aws_account_id + ':' + self.rest_api_id + '/' + self.stage + '/' + verb + '/' + resource
        if effect.lower() == 'allow':
            self.allowMethods.append({'resource_arn': resource_arn, 
               'conditions': conditions})
        elif effect.lower() == 'deny':
            self.denyMethods.append({'resource_arn': resource_arn, 
               'conditions': conditions})
    ...
    def build(self):
        ...
        policy['policyDocument']['Statement'].extend(self._get_effect_statement('Allow', self.allowMethods))
        policy['policyDocument']['Statement'].extend(self._get_effect_statement('Deny', self.denyMethods))
        return policy