...
# [GCC 8.4.0]
# Embedded file name: /home/peter/slugs/slugs/tests/unit/test_plugins.py
# Compiled at: 2019-01-17 10:41:13
# Size of source mod 2**32: 8660 bytes
import builtins as @py_builtins, _pytest.assertion.rewrite as @pytest_ar, cherrypy, mock, os, shutil, tempfile, testtools
...
class TestFileMonitoringPlugin(testtools.TestCase):
    ...
    def test_init(self):
        ...
        plugin = plugins.FileMonitoringPlugin(cherrypy.engine, self.temp_file.name, callback)
    ...
    def test_invalid_path(self):
        ...
        args = [
         plugin, 'path', 'invalid']
        self.assertRaisesRegex(ValueError, "Monitored file 'invalid' must be an existing file.", setattr, *args)
    ...
    def test_start(self):
        ...
        plugin.start()
        plugin.bus.log.assert_called_once_with('Starting file monitoring plugin for file: {}'.format(plugin.path))
    ...
    def test_stop(self):
        ...
        plugin.stop()
        plugin.bus.log.assert_called_once_with('Stopping file monitoring plugin for file: {}'.format(plugin.path))
        plugin.bus.unsubscribe.assert_called_once_with('main', plugin.check_path)
    ...
    def test_update_data_with_no_data(self):
        ...
        plugin.update_data()
        plugin.bus.log.assert_called_once_with('Monitored file ({}) updated. Reloading data.'.format(plugin.path))
    ...
    def test_update_data_with_bad_data(self):
        ...
        with open(self.temp_file.name, 'w') as (f):
            ...
            f.write('Jane,Human\n')
        plugin.update_data()
        plugin.bus.log.assert_any_call('Monitored file ({}) updated. Reloading data.'.format(plugin.path))
        plugin.bus.log.assert_any_call('Error parsing monitored file ({}). Halting data reload.'.format(plugin.path))
        callback.assert_not_called()