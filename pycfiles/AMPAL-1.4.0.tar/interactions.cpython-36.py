...
# [GCC 8.4.0]
# Embedded file name: /home/cw12401/code/work/ampal/src/ampal/interactions.py
# Compiled at: 2018-04-11 08:14:19
...
class Interaction(object):
    __doc__ = 'A container for all types of interaction with donor and acceptor.\n\n    Parameters\n    ----------\n    a : ampal.Atom\n        A member of a pairwise interaction.\n    b : ampal.Atom\n        A member of a pairwise interaction.\n    dist : float\n        The distance between `a` and `b`.\n\n    Attributes\n    ----------\n    a : ampal.Atom\n        A member of a pairwise interaction.\n    b : ampal.Atom\n        A member of a pairwise interaction.\n    dist : float\n        The distance between `Atom` `a` and `b`.\n    '
    ...
    def __eq__(self, other):
        return (
         type(self), self._a, self._b) == (type(other), other._a, other._b)
    ...
    def __repr__(self):
        ...
        bm = self._b.parent
        bc = bm.parent
        return '<Interaction between {} {}{} and {} {}{}>'.format(self._a.res_label, am.id, ac.id, self._b.res_label, bm.id, bc.id)


class CovalentBond(Interaction):
    __doc__ = 'Defines a covalent bond.'
    ...
    def __repr__(self):
        ...
        bm = self._b.parent
        bc = bm.parent
        return '<Covalent bond between {}{} {} {} --- {} {} {}{}>'.format(ac.id, am.id, am.mol_code, self._a.res_label, self._b.res_label, bm.mol_code, bc.id, bm.id)


class NonCovalentInteraction(Interaction):
    __doc__ = ' A container for all non-covalent interaction.\n\n    Parameters\n    ----------\n    donor : ampal.Atom\n        The donor `Atom` in the interaction.\n    acceptor : ampal.Atom\n        The acceptor atom in the interaction.\n    dist : float\n        The distance between `Atom` `a` and `b`.\n\n    Attributes\n    ----------\n    donor : ampal.Atom\n        The donor `Atom` in the interaction.\n    acceptor : ampal.Atom\n        The acceptor atom in the interaction.\n    dist : float\n        The distance between `Atom` `a` and `b`.\n    '
    ...
    def __repr__(self):
        return '<Interaction between {} {}{} (donor) and {} {}{} (acceptor)>'.format(self.donor.mol_code, self.donor.id, self.donor.parent.id, self.acceptor.mol_code, self.acceptor.id, self.acceptor.parent.id)


class HydrogenBond(NonCovalentInteraction):
    __doc__ = 'Defines a hydrogen bond in terms of a donor and an acceptor.\n\n    Parameters\n    ----------\n    donor : ampal.Atom\n        The donor `Atom` in the interaction.\n    acceptor : ampal.Atom\n        The acceptor atom in the interaction.\n    dist : float\n        The distance between `Atom` `a` and `b`.\n    ang_a : float\n        Angle between the acceptor and the interaction vector.\n    ang_d : float\n        Angle between the donor and the interaction vector.\n\n    Attributes\n    ----------\n    donor : ampal.Atom\n        The donor `Atom` in the interaction.\n    acceptor : ampal.Atom\n        The acceptor atom in the interaction.\n    dist : float\n        The distance between `Atom` `a` and `b`.\n    ang_a : float\n        Angle between the acceptor and the interaction vector.\n    ang_d : float\n        Angle between the donor and the interaction vector.\n    '
    ...
    def __repr__(self):
        ...
        ac = am.parent
        return '<Hydrogen Bond between ({}{}) {}-{} ||||| {}-{} ({}{})>'.format(dm.id, dc.id, dm.mol_code, self.donor.res_label, self.acceptor.res_label, am.mol_code, am.id, ac.id)
...
def covalent_bonds(atoms, threshold=1.1):
    ...
    bonds = []
    for a, b in atoms:
        bond_distance = (ELEMENT_DATA[a.element.title()]['atomic radius'] + ELEMENT_DATA[b.element.title()]['atomic radius']) / 100
...
def generate_bond_subgraphs_from_break(bond_graph, atom1, atom2):
    ...
    try:
        subgraphs = list(networkx.connected_component_subgraphs(bond_graph,
          copy=False))
    finally:
