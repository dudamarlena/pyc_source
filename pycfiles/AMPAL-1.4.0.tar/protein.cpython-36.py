...
# [GCC 8.4.0]
# Embedded file name: /home/cw12401/code/work/ampal/src/ampal/protein.py
# Compiled at: 2018-05-03 08:12:45
# Size of source mod 2**32: 37346 bytes
...
from ampal.analyse_protein import make_primitive_extrapolate_ends, measure_torsion_angles, residues_per_turn, polymer_to_reference_axis_distances, crick_angles, alpha_angles, sequence_molecular_weight, sequence_molar_extinction_280, sequence_isoelectric_point, measure_sidechain_torsion_angles
from ampal.interactions import generate_covalent_bond_graph, generate_bond_subgraphs_from_break, find_covalent_bonds
from .amino_acids import get_aa_code, get_aa_letter, ideal_backbone_bond_lengths, ideal_backbone_bond_angles
from .geometry import Quaternion, unit_vector, dihedral, find_transformations, distance, angle_between_vectors
...
def align(target, mobile, target_i=0, mobile_i=0):
    ...
    translation, angle, axis, point = find_transformations(s1,
      e1, s2, e2, radians=False)
...
class Polypeptide(Polymer):
    ...
    def __repr__(self):
        if len(self.sequence) > 15:
            ...
            seq = self.sequence
        return '<Polypeptide containing {} {}. Sequence: {}>'.format(len(self._monomers), 'Residue' if len(self._monomers) == 1 else 'Residues', seq)
    ...
    def primitive(self):
        ...
        primitive_coords = make_primitive_extrapolate_ends(cas,
          smoothing_level=(self.sl))
    ...
    def fasta(self):
        ...
        max_line_length = 79
        fasta_str = '>{0}:{1}|PDBID|CHAIN|SEQUENCE\n'.format(self.parent.id.upper(), self.id)
        seq = self.sequence
    ...
    def backbone_bond_lengths(self):
        ...
        """
        bond_lengths = dict(n_ca=[distance(r['N'], r['CA']) for r in self.get_monomers(ligands=False)],
    ...
    def backbone_bond_angles(self):
        ...
        """
        bond_angles = dict(n_ca_c=[angle_between_vectors(r['N'] - r['CA'], r['C'] - r['CA']) for r in self.get_monomers(ligands=False)],
          ca_c_o=[angle_between_vectors(r['CA'] - r['C'], r['O'] - r['C']) for r in self.get_monomers(ligands=False)],
          ca_c_n=[angle_between_vectors(r1['CA'] - r1['C'], r2['N'] - r1['C']) for r1, r2 in [(self[i], self[(i + 1)]) for i in range(len(self) - 1)]],
          c_n_ca=[angle_between_vectors(r1['C'] - r2['N'], r2['CA'] - r2['N']) for r1, r2 in [(self[i], self[(i + 1)]) for i in range(len(self) - 1)]])
        return bond_angles

    def c_join(self, other, psi=-40.76, omega=-178.25, phi=-65.07, o_c_n_angle=None, c_n_ca_angle=None, c_n_length=None, relabel=True):
        ...
        if isinstance(other, Residue):
            ...
            q = Quaternion.angle_and_axis(angle=(psi - measured_psi),
              axis=(r1_c - r1_ca))
            p1 = q.rotate_vector(v=p1, point=r1_c)
            ...
            other.rotate(angle=(c_n_ca_angle - measured_c_n_ca), axis=axis,
              point=(other[0]['N']._vector))
            measured_omega = dihedral(r1['CA'], r1['C'], other[0]['N'], other[0]['CA'])
            other.rotate(angle=(omega - measured_omega), axis=(other[0]['N'] - r1['C']),
              point=(other[0]['N']._vector))
            measured_phi = dihedral(r1['C'], other[0]['N'], other[0]['CA'], other[0]['C'])
            other.rotate(angle=(phi - measured_phi), axis=(other[0]['CA'] - other[0]['N']),
    ...
    def n_join(self, other, psi=-40.76, omega=-178.25, phi=-65.07, o_c_n_angle=None, c_n_ca_angle=None, c_n_length=None, relabel=True):
        ...
        if isinstance(other, Residue):
            ...
            other.rotate(angle=(measured_dihedral - desired_dihedral), axis=axis,
            ...
            other.rotate(angle=(measured_o_c_n - o_c_n_angle), axis=axis,
              point=(other[(-1)]['C']._vector))
            measured_phi = dihedral(other[(-1)]['C'], r1['N'], r1['CA'], r1['C'])
            other.rotate(angle=(phi - measured_phi), axis=(r1_n - r1_ca),
              point=r1_ca)
            measured_omega = dihedral(other[(-1)]['CA'], other[(-1)]['C'], r1['N'], r1['CA'])
            other.rotate(angle=(measured_omega - omega), axis=(r1['N'] - other[(-1)]['C']),
              point=r1_n)
    ...
    def tag_sidechain_dihedrals(self, force=False):
        ...
        if not all(tagged) or force:
            for monomer in self._monomers:
                chi_angles = measure_sidechain_torsion_angles(monomer,
                  verbose=False)
    ...
    def tag_ca_geometry(self, force=False, reference_axis=None, reference_axis_name='ref_axis'):
        ...
        if reference_axis is not None:
            if len(reference_axis) == len(self):
                ref_axis_args = dict(p=self, reference_axis=reference_axis,
                  tag=True,
...
class Residue(Monomer):
    ...
    def __repr__(self):
        return '<Residue containing {} {}. Residue code: {}>'.format(len(self.atoms), 'Atom' if len(self.atoms) == 1 else 'Atoms', self.mol_code)
    ...
    def backbone(self):
        ...
        try:
            ...
            missing_atoms = filter(lambda x: x not in self.atoms.keys(), ('N', 'CA',
                                                                          'C', 'O'))
            raise KeyError('Error in residue {} {} {}, missing ({}) atoms. `atoms` must be an `OrderedDict` with coordinates defined for the backbone (N, CA, C, O) atoms.'.format(self.parent.id, self.mol_code, self.id, ', '.join(missing_atoms)))
    ...
    def unique_id(self):
        ...
        if self.is_hetero:
            ...
            hetero_flag = ' '
        return (
         self.parent.id, (hetero_flag, self.id, self.insertion_code))
