...
# [GCC 8.4.0]
# Embedded file name: /home/cw12401/code/work/ampal/src/ampal/assembly.py
# Compiled at: 2018-04-11 07:53:28
...
class AmpalContainer(object):
    ...
    def __add__(self, other):
        ...
        if isinstance(other, AmpalContainer):
            ...
            raise TypeError('Only AmpalContainer objects may be merged with an AmpalContainer using unary operator "+".')
...
class Assembly(BaseAmpal):
        ...
        if molecules:
            if isinstance(molecules, Polymer):
                ...
                if isinstance(molecules, list):
                    if isinstance(molecules[0], Polymer):
                        self._molecules = list(molecules)
                raise TypeError('Assembly objects can only be initialised empty, using a Polymer or a list of Polymers.')
        else:
            self._molecules = []
    ...
    def __repr__(self):
        ...
        if 'protein' in mol_types:
            repr_strs.append('{} {}'.format(mol_types['protein'], 'Polypeptide' if len(self._molecules) == 1 else 'Polypeptides'))
        if 'nucleic_acid' in mol_types:
            repr_strs.append('{} {}'.format(mol_types['nucleic_acid'], 'Polynucleotide' if len(self._molecules) == 1 else 'Polynucleotides'))
        ligand_count = 0
        if 'ligands' in mol_types:
            repr_strs.append('{} {}'.format(mol_types['ligands'], 'Ligand Group' if len(self._molecules) == 1 else 'Ligand Groups'))
        ...
        if ligand_count:
            repr_strs.append('{} {}'.format(ligand_count, 'Ligand' if ligand_count == 1 else 'Ligands'))
        if 'pseudo_group' in mol_types:
            repr_strs.append('{} {}'.format(mol_types['pseudo_group'], 'Pseudo Group' if len(self._molecules) == 1 else 'Pseudo Groups'))
    ...
    def get_ligands(self, solvent=True):
        ...
        if solvent:
            ...
            ligand_list = [x for x in self.get_monomers() if isinstance(x, Ligand) if not x.is_solvent]
    ...
    def get_atoms(self, ligands=True, pseudo_group=False, inc_alt_states=False):
        ...
        """
        atoms = (itertools.chain)(*(list(m.get_atoms(inc_alt_states=inc_alt_states)) for m in self.get_monomers(ligands=ligands, pseudo_group=pseudo_group)))
        return atoms
    ...
    def relabel_polymers(self, labels=None):
        ...
        if labels:
            if len(self._molecules) == len(labels):
                ...
                raise ValueError('Number of polymers ({}) and number of labels ({}) must be equal.'.format(len(self._molecules), len(labels)))
        else:
    ...
    def tag_ca_geometry(self, force=False, reference_axis=None, reference_axis_name='ref_axis'):
        ...
        for polymer in self._molecules:
            if polymer.molecule_type == 'protein':
                polymer.tag_ca_geometry(force=force,
