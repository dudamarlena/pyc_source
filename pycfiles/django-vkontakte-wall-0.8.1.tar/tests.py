...
# [GCC 8.4.0]
# Embedded file name: /Users/ramusus/workspace/manufacture_old/env/src/django-vkontakte-wall/vkontakte_wall/tests.py
...
class VkontakteWallTest(TestCase):
    ...
    def test_fetch_post_reposts1(self, *args, **kwargs):
        ...
        self.assertEqual(post.reposts_users.count(), 0)
        users = post.fetch_reposts(all=True)
        self.assertGreaterEqual(post.reposts_count, 104)
        self.assertEqual(post.reposts_count, users.count())
        self.assertEqual(post.reposts_count, User.objects.count() - users_initial)
        self.assertEqual(post.reposts_count, post.reposts_users.count())
    ...
    def test_parse_post(self):
        ...
        instance = Post()
        owner = UserFactory(remote_id=201164356)
    ...
    def test_parse_comment(self):
        ...
        self.assertEqual(instance.text, 'Добрый день , кароче такая идея когда опросы создаешь вместо статуса - можно выбрать аудитории опрашиваемых, например только женский или мужской пол могут участвовать (то бишь голосовать в опросе).')
        self.assertEqual(instance.author, author)
        self.assertIsInstance(instance.date, datetime)
    ...
    def test_post_prepare_create_params(self):
        text = 'test text'
        expected_config = {'owner_id': GROUP_ID * -1, 
           'friends_only': 0, 
           'from_group': '', 
           'message': text, 
           'attachments': '', 
    ...
    def test_post_prepare_delete_params(self):
        group = GroupFactory(remote_id=GROUP_ID)
        post = PostFactory(remote_id='%s_17' % GROUP_ID, owner=group)
        expected_params = {'owner_id': GROUP_ID * -1, 
           'post_id': '17'}
    ...
    def test_post_crud_methods(self):
        ...
        post = Post.objects.create(text='Test message created by manager', owner=group, author=user, date=datetime.now(), commit_remote=True)
        self.objects_to_delete += [post]
        self.assertEqual(Post.objects.count(), 2)
        self.assertNotEqual(len(post.remote_id), 0)
        self.assertPostTheSameEverywhere(post)
        post = Post.objects.create(text='Test message', owner=user, author=user, date=datetime.now(), commit_remote=True)
        self.objects_to_delete += [post]
