...
# [GCC 8.4.0]
# Embedded file name: /home/fsc/work/devel/flamingo/flamingo/core/parser.py
...
NO_META_DATA_RE = re.compile('^((\\s+)?\\n){2,}')
DELIMITER_RE = re.compile('((\\s+)?\\n){3,}')
...
class ContentParser:
    ...
    def parse_meta_data(self, file_content, content):
        ...
        def raise_yaml_parsing_error():
            ...
            raise ParsingError('Invalid meta data at line 1 to {}. Meta data has to be valid YAML, defining a key value store'.format(markup_string_end))
        ...
        if match:
            ...
            if not match:
                raise ParsingError('Invalid format. Content files have to start with two blank lines, or a meta data block followed by two blank lines and a markup block')
    ...
    def __init__(self, context):
        ...
        if context.settings.USE_CHARDET:
            try:
                ...
                context.logger.error('USE_CHARDET is set but chardet is not installed.Falling back to plain python read')
