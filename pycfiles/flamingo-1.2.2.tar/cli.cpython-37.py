...
def color(string, color='', background='', style='', reset=True):
    ...
    if not string:
        if reset:
            return reset_string
    if style:
        style = {'bright':'1',  'underlined':'2', 
         'negative':'3'}[style]
    if color:
        color = {'black':'30',  'red':'31', 
         'green':'32', 
         'yellow':'33', 
         'blue':'34', 
         'magenta':'35', 
...
def get_raw_parser(*parser_args, description='', **parser_kwargs):
    ...
    parser = ArgumentParser(parser_args, description=description, formatter_class=RawTextHelpFormatter, **parser_kwargs)
