...
# [GCC 8.4.0]
# Embedded file name: /home/fsc/work/devel/flamingo/flamingo/plugins/rst/base.py
# Compiled at: 2020-05-01 10:30:52
...
SYSTEM_MESSAGE_RE = re.compile('^(?P<name>[^:]+):(?P<line>\\d+)?: \\((?P<level_name>[^/)]+)/(?P<level>\\d+)\\) (?P<short_description>[^\\t\\n]+)(?P<long_description>.*)?$', re.DOTALL)
logger = logging.getLogger('flamingo.plugins.reStructuredText')

class reStructuredTextError(ParsingError):

    def __init__(self, *args, name=None, line=None, level_name=None, level=None, short_description=None, long_description=None, **kwargs):
        (super().__init__)(*args)
        self.name = name
        self.line = line
...
def parse_system_message(raw_message, system_message_re=SYSTEM_MESSAGE_RE):
    ...
    try:
        ...
        logger.error('exception occoured while parsing %s', raw_message,
          exc_info=True)
        raise
...
class WarningStream:
    ...
    def write(self, warning):
        ...
        if level_name == 'DEBUG':
            self.context.logger.debug('%s:%s: %s', path, line, short_description)
        else:
            if level_name == 'INFO':
                self.context.logger.info('%s:%s: %s', path, line, short_description)
            else:
                if level_name == 'WARNING':
                    self.context.logger.warning('%s:%s: %s', path, line, short_description)
...
class FlamingoWriter(Writer):
    ...
    def write(self, document, destination):
        self.flamingo_context.plugins.run_plugin_hook('rst_document_parsed', document)
...
def parse_rst_parts(rst_input, context, system_message_re=SYSTEM_MESSAGE_RE):
    ...
    settings_overrides = {**{'initial_header_level':'2', 
     ...
     'dump_settings':False, 
     'halt_level':3, 
     'report_level':1}, **(context.settings.get('RST_SETTINGS_OVERRIDES', {}))}
    writer = FlamingoWriter(flamingo_context=context)
    try:
        output = publish_parts(settings_overrides=settings_overrides,
          writer=writer,
          source=rst_input)
        return output
        ...
        def remove_system_messages(children, removed):
            for child in children[:]:
                if isinstance(child, system_message):
                    ...
                    if 'classes' in child.attributes:
                        if 'system-messages' in child.attributes['classes']:
                            children.remove(child)
                            removed[0] += 1
                if child.children:
