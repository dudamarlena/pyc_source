...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/internationalization/core/impl/po_file_manager.py
# Compiled at: 2013-10-04 06:08:55
"""
...
from internationalization.support.babel.util_babel import msgId, isMsgTranslated, copyTranslation, fixBabelCatalogAddBug
...
class POFileManager(IPOFileManager):
    ...
    write_po_config = {'no_location': False, 
     'omit_header': False, 
     'sort_output': True, 
     'sort_by_file': True, 
    ...
    def getComponentPOFile(self, component, locale):
        ...
        catalog = self._build(locale, self.messageService.getComponentMessages(component), self._filePath(locale, component=component), self._filePath(locale))
        return self._toPOFile(catalog)
    ...
    def getComponentAsDict(self, component, locale):
        ...
        catalog = self._build(locale, messages, self._filePath(locale, component=component), self._filePath(locale))
    ...
    def getPluginPOFile(self, plugin, locale):
        ...
        catalog = self._build(locale, self.messageService.getPluginMessages(plugin), self._filePath(locale, plugin=plugin), self._filePath(locale))
        return self._toPOFile(catalog)
    ...
    def getPluginAsDict(self, plugin, locale):
        ...
        catalog = self._build(locale, messages, self._filePath(locale, plugin=plugin), self._filePath(locale))
    ...
    def updateGlobalPOFile(self, locale, poFile):
        ...
        return self._update(locale, self.messageService.getMessages(), poFile, self._filePath(locale), self._filePath(locale, format=FORMAT_MO))
    ...
    def updateComponentPOFile(self, component, locale, poFile):
        ...
        assert isinstance(poFile, IInputStream), 'Invalid file object %s' % poFile
        return self._update(locale, self.messageService.getComponentMessages(component), poFile, self._filePath(locale, component=component), self._filePath(locale, component=component, format=FORMAT_MO), False)
    ...
    def updatePluginPOFile(self, plugin, locale, poFile):
        ...
        assert isinstance(poFile, IInputStream), 'Invalid file object %s' % poFile
        return self._update(locale, self.messageService.getPluginMessages(plugin), poFile, self._filePath(locale, plugin=plugin), self._filePath(locale, plugin=plugin, format=FORMAT_MO), False)
    ...
    def _filePath(self, locale, component=None, plugin=None, format=FORMAT_PO):
        ...
        if not component is None:
            assert isinstance(component, str), 'Invalid component %s' % component
        if not plugin is None:
            assert isinstance(plugin, str), 'Invalid plugin %s' % plugin
    ...
    def _processCatalog(self, catalog, messages, fallBack=None):
        ...
        for msg in messages:
            ...
            msgCOrig = copy(msgC)
            catalog.add(id, context=msg.Context if msg.Context != '' else None, locations=(
             (
              src.Path, msg.LineNumber),), user_comments=(
             msg.Comments if msg.Comments else '',))
            if msgC:
                fixBabelCatalogAddBug(msgC, catalog.num_plurals)
            if msg.Plural and msgC and msgCOrig and isinstance(msgCOrig.string, str) and msgCOrig.string != '':
                copyTranslation(msgCOrig, msgC)
                continue

        creationDate = catalog.creation_date
    ...
    def _build(self, locale, messages, path, pathGlobal=None):
        ...
        if not pathGlobal is None:
            assert isinstance(pathGlobal, str), 'Invalid global path %s' % pathGlobal
            if isfile(path):
