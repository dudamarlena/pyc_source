...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/internationalization/impl/po_file.py
# Compiled at: 2013-05-23 04:47:21
"""
...
class POFileService(IPOFileService):
    ...
    def getComponentPOFile(self, component, locale, scheme):
        ...
        try:
            try:
                ...
                mngFileTimestamp = max(self.poFileManager.getGlobalPOTimestamp(locale) or datetime.min, self.poFileManager.getComponentPOTimestamp(component, locale) or datetime.min)
                republish = False if mngFileTimestamp is None else cdmFileTimestamp < mngFileTimestamp
            if republish:
        ...
        if pluginObj.Component:
            ...
            try:
                try:
                    ...
                    mngFileTimestamp = max(self.poFileManager.getGlobalPOTimestamp(locale) or datetime.min, self.poFileManager.getPluginPOTimestamp(plugin, locale) or datetime.min)
                    republish = False if mngFileTimestamp is None else cdmFileTimestamp < mngFileTimestamp
                if republish:
    ...
    def updatePluginPOFile(self, plugin, locale, poFile):
        ...
        assert isinstance(pluginObj, Plugin)
        if pluginObj.Component:
            return self.updateComponentPOFile(pluginObj.Component, locale, poFile)
        poFile = codecs.getreader(poFile.charSet or self.default_charset)(poFile)
        try:
...
InvalidPOFile = lambda poFile: InputError(_('Invalid content for PO file %(file)s') % dict(file=poFile.getName() or C_('Unknown file name', 'unknown')))
