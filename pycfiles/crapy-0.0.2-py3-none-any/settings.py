...
# Embedded file name: /home/jake/CRAPtion/craption/settings.py
# Compiled at: 2018-05-18 03:15:02
...
def write_template():
    conf = ConfigObj(confpath, create_empty=True, indent_type='    ', write_empty_values=True)
    conf['file'] = {'name': 'CRAPtion_{r5}_{u}_{d}', 
       'dir': '~/craptions', 
       'keep': True, 
    ...
    conf['upload'] = {'upload': True, 
       'to': 'imgur', 
       'imgur': {'api-key': 'ecacc93590c0ad5'}, 
       'scp': {'user': 'myuser', 
               'host': 'example.com', 
    ...
    conf['upload'].comments['dropbox'] = [
     ...
     '2: Run craption -d']
    conf['upload']['dropbox'].comments['token'] = [
     'Set by craption -d']
...
def dropbox_login():
    import dropbox
    conf = get_conf()
    if not conf['upload']['dropbox']['app']['key'] or not conf['upload']['dropbox']['app']['secret']:
        conf['upload']['dropbox']['app']['key'] = raw_input('App key? ')
        conf['upload']['dropbox']['app']['secret'] = raw_input('Secret app key? ')
    flow = dropbox.client.DropboxOAuth2FlowNoRedirect(conf['upload']['dropbox']['app']['key'], conf['upload']['dropbox']['app']['secret'])
