...
# [GCC 8.4.0]
# Embedded file name: /plugin/sentry_youtrack/plugin.py
# Compiled at: 2016-09-28 17:53:35
import json
...
from . import VERSION
from .forms import NewIssueForm, AssignIssueForm, DefaultFieldForm, YouTrackConfigurationForm, YouTrackProjectForm, VERIFY_SSL_CERTIFICATE
...
class YouTrackPlugin(IssuePlugin):
    ...
    def get_youtrack_client(self, project):
        settings = {'url': self.get_option('url', project), 
    ...
    def get_project_fields(self, project):
        ...
        def cached_fields(ignore_fields):
            ...
            return list(yt_client.get_project_fields(self.get_option('project', project), ignore_fields))

        return cached_fields(self.get_option('ignore_fields', project))

    def get_initial_form_data(self, request, group, event, **kwargs):
        initial = {'title': self._get_group_title(request, group, event), 
           'description': self._get_group_description(request, group, event), 
           'tags': self.get_option('default_tags', group.project), 
           'default_fields': self.get_option(self.default_fields_key, group.project)}
        return initial
    ...
    def get_new_issue_form(self, request, group, event, **kwargs):
        if request.POST or request.GET.get('form'):
            return self.new_issue_form(project_fields=self.get_project_fields(group.project), data=request.POST or None, initial=self.get_initial_form_data(request, group, event))
        else:
            return forms.Form()
    ...
    def create_issue(self, request, group, form_data, **kwargs):
        ...
        issue_data = {'project': self.get_option('project', group.project), 
    ...
    def actions(self, request, group, action_list, **kwargs):
        action_list = super(YouTrackPlugin, self).actions(request, group, action_list, **kwargs)
    ...
    def project_issues_view(self, request, group):
        ...
        project_issues = yt_client.get_project_issues(project_id, offset=offset, limit=page_limit + 1, query=query)
        data = {'more': len(project_issues) > page_limit, 
