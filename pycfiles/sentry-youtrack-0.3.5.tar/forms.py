...
# [GCC 8.4.0]
# Embedded file name: /plugin/sentry_youtrack/forms.py
# Compiled at: 2016-09-28 17:53:47
...
VERIFY_SSL_CERTIFICATE = getattr(settings, 'YOUTRACK_VERIFY_SSL_CERTIFICATE', True)

class YouTrackProjectForm(forms.Form):
    PROJECT_FIELD_PREFIX = 'field_'
    FIELD_TYPE_MAPPING = {'float': forms.FloatField, 
    ...
    def add_project_fields(self, project_fields):
        ...
        for field in project_fields:
            ...
            if form_field:
                form_field.widget.attrs = {'class': 'project-field', 'data-field': field['name']}
                index = len(fields) + 1
    ...
    def _get_form_field(self, project_field):
        ...
        kwargs = {'label': project_field['name'], 
           'required': False, 
...
class NewIssueForm(YouTrackProjectForm):
    title = forms.CharField(label=_('Title'), widget=forms.TextInput(attrs={'class': 'span9'}))
    description = forms.CharField(label=_('Description'), widget=forms.Textarea(attrs={'class': 'span9'}))
    tags = forms.CharField(label=_('Tags'), help_text=_('Comma-separated list of tags'), widget=forms.TextInput(attrs={'class': 'span6', 
       'placeholder': 'e.g. sentry'}), required=False)
...
class AssignIssueForm(forms.Form):
    issue = forms.CharField(label=_('YouTrack Issue'), widget=forms.TextInput(attrs={'class': 'span6', 'placeholder': _('Choose issue')}))
...
class DefaultFieldForm(forms.Form):
    ...
    def save(self):
        ...
        default_fields = self.plugin.get_option(self.plugin.default_fields_key, self.project) or {}
        default_fields[md5(unidecode(data['field'])).hexdigest()] = data['value']
        self.plugin.set_option(self.plugin.default_fields_key, default_fields, self.project)


class YouTrackConfigurationForm(forms.Form):
    error_message = {'client': _('Unable to connect to YouTrack.'), 
    ...
    url = forms.URLField(label=_('YouTrack Instance URL'), widget=forms.TextInput(attrs={'class': 'span9', 'placeholder': 'e.g. "https://yoursitename.myjetbrains.com/youtrack/"'}), required=True)
    username = forms.CharField(label=_('Username'), help_text=_('User should have admin rights.'), widget=forms.TextInput(attrs={'class': 'span9'}), required=True)
    password = forms.CharField(label=_('Password'), help_text=_('Only enter a password if you want to change it'), widget=forms.PasswordInput(attrs={'class': 'span9'}), required=False)
    project = forms.ChoiceField(label=_('Linked Project'), required=True)
    default_tags = forms.CharField(label=_('Default tags'), help_text=_('Comma-separated list of tags'), widget=forms.TextInput(attrs={'class': 'span6', 'placeholder': 'e.g. sentry'}), required=False)
    ignore_fields = forms.MultipleChoiceField(label=_('Ignore fields'), required=False, help_text=_('These fields will not appear on the form'))
        ...
        if initial:
            ...
            if not client:
                ...
                if initial.get('project'):
                    choices = self.get_ignore_field_choices(client, initial.get('project'))
                ...
                if not any(args) and not initial.get('project'):
                    self.second_step_msg = _('Your credentials are valid but plugin is NOT active yet. Please fill in remaining required fields.')
    ...
    def get_ignore_field_choices(self, client, project):
        try:
            ...
            self.client_errors['project'] = self.error_message['invalid_project'] % (project,)
        else:
    ...
    def get_youtrack_client(self, data, additional_params=None):
        ...
        try:
            ...
            if e.response is not None and e.response.status_code == 403:
                self.client_errors['username'] = self.error_message['invalid_password']
            else:
