...
# [GCC 8.4.0]
# Embedded file name: /plugin/sentry_youtrack/youtrack.py
# Compiled at: 2016-10-05 14:03:29
...
class YouTrackClient(object):
    ...
    def _get_userbundle_values(self, soup):
        ...
        for group in soup.userBundle.findAll('userGroup'):
            users.update(get_user_logins(self._get_users_from_group(group['name'])))
    ...
    def _get_custom_field_values(self, name, value, bundle='enumeration'):
        url = self.url + self.CUSTOM_FIELD_VALUES.replace('<param_name>', name).replace('<param_value>', value)
    ...
    def _get_custom_project_field_details(self, field):
        ...
        values = None
        if field_data.param:
            kwargs = {'name': type_name, 'value': field_data.param['value'], 
               'bundle': bundles.get(type_prefix)}
            values = self._get_custom_field_values(**kwargs)
        field_details = {'name': field_data.projectCustomField['name'], 
    ...
    def request(self, url, data=None, params=None, method='get'):
        ...
        kwargs = {'url': url, 
           ...
           'headers': {'User-Agent': 'sentry-youtrack/%s' % VERSION}}
    ...
    def get_project_issues(self, project_id, query=None, offset=0, limit=15):
        ...
        issues = [ {'id': issue['id'], 'state': issue.find('field', {'name': 'State'}).value.text, 'summary': issue.find('field', {'name': 'summary'}).text} for issue in BeautifulSoup(response.text, 'xml').issues
                 ]
        return issues
