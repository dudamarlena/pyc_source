...
# Embedded file name: build/bdist.linux-x86_64/egg/caver/data.py
...
class TextData:
    ...
    def __init__(self, path='', **kwargs):
        ...
        if os.path.isfile(self.config.word2index):
            if os.path.isfile(self.config.label2index):
                self.load_index()
        else:
    ...
    def load_index(self):
        ...
        print('Load {} words and {} labels.'.format(len(self.word2index), len(self.label2index)))
    ...
    def extract(self):
        ...
        print('Extract {} words and {} labels.'.format(len(words), len(labels)))
        return (
         words, labels)
