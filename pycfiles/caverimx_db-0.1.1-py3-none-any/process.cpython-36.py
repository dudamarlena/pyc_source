...
# Embedded file name: build/bdist.linux-x86_64/egg/caver/process.py
# Compiled at: 2018-12-04 03:40:24
...
class Trainer:
    ...
    def __init__(self, model_name, data_path, **kwargs):
        ...
        self.model = getattr(model, model_name)(vocab_size=(len(self.data.word2index)),
          label_num=(len(self.data.label2index))).to(self.device)
        self.model.apply(init_weight)
        print(model)
        self.loss_func = self.config.loss_func if isinstance(self.config.loss_func, torch.nn.Module) else nn.BCELoss()
    ...
    def init_model_embedding(self):
        ...
        embedding_weight = load_embedding(self.config.embedding_file, self.config.embedding_dim, len(self.index2word), self.index2word)
    ...
    def train_step(self, train_x, train_y, val_x, val_y):
        ...
        for i, (start, end) in enumerate(batches):
            input_data = torch.from_numpy(zero_padding(train_x[start:end], self.config.sentence_length)).type(torch.long).to(self.device)
            input_label = torch.from_numpy(transform2onehot(train_y[start:end], len(self.index2label))).type(torch.float).to(self.device)
            self.model.train()
    ...
    def eval_step(self, x, y):
        ...
        with torch.no_grad():
            for start, end in batches:
                input_data = torch.from_numpy(zero_padding(x[start:end], self.config.sentence_length)).type(torch.long).to(self.device)
                input_label = torch.from_numpy(transform2onehot(y[start:end], len(self.index2label))).type(torch.float).to(self.device)
                self.model.eval()
                logits = self.model(input_data)
        ...
        print('[+] Total loss: {}\nAverage recall@{}: {}'.format(total_loss, self.config.recall_k, total_recall / len(y)))
        print('-' * 80 + '\n')