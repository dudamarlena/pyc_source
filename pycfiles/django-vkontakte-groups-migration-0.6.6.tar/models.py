...
# [GCC 8.4.0]
# Embedded file name: /home/ramusus/workspace/manufacture/env/src/django-vkontakte-groups-migration/vkontakte_groups_migration/models.py
# Compiled at: 2014-10-22 18:01:30
...
class GroupMigrationQueryset(object):
    ...
    @property
    def light(self):
        return self.defer('members_ids', 'members_entered_ids', 'members_left_ids', 'members_deactivated_entered_ids', 'members_deactivated_left_ids', 'members_has_avatar_entered_ids', 'members_has_avatar_left_ids')
...
class GroupMigration(models.Model):
    ...
    class Meta:
        verbose_name = 'Миграция пользователей группы Вконтакте'
        verbose_name_plural = 'Миграции пользователей групп Вконтакте'
        unique_together = ('group', 'time')

    class QuerySet(QuerySet, GroupMigrationQueryset):
        pass

    group = models.ForeignKey(Group, verbose_name='Группа', related_name='migrations')
    time = models.DateTimeField('Дата и время', null=True, db_index=True)
    hidden = models.BooleanField('Скрыть', default=False, db_index=True)
    ...
    def fix_memberships(self):
        ...
        if not self.hidden:
            ...
            if self.next:
                ...
                GroupMembership.objects.filter(group=self.group, time_left=self.time).exclude(user_id__in=self.next.members_ids).update(time_left=self.next.time)
                GroupMembership.objects.filter(group=self.group, time_entered=self.time).exclude(user_id__in=self.next.members_ids).delete()
                GroupMembership.objects.filter(group=self.group, time_entered=self.time).filter(user_id__in=self.next.members_ids).update(time_entered=self.next.time)
            else:
    ...
    def save_final(self):
        ...
        except WrongMembershipsAmmount as e:
            log.warning(e)
...
class GroupMembershipManager(models.Manager):
    ...
    def get_user_ids(self, group, time=None, unique=True):
        if time is None:
            ...
            qs = self.filter(group=group).filter(Q(time_entered=None, time_left=None) | Q(time_entered=None, time_left__gt=time) | Q(time_entered__lte=time, time_left=None) | Q(time_entered__lte=time, time_left__gt=time))
    ...
    def get_user_ids_of_period(self, group, date_from, date_to, field=None, unique=True):
        if field is None:
            kwargs = {'time_entered': None, 'time_left': None} | {'time_entered__lte': date_from, 'time_left': None} | {'time_entered': None, 'time_left__gte': date_to}
...
class GroupMembership(models.Model):
    ...
    class Meta:
        verbose_name = 'Членство пользователя группы Вконтакте'
        verbose_name_plural = 'Членства пользователей групп Вконтакте'

    group = models.ForeignKey(Group, verbose_name='Группа', related_name='memberships')
    user_id = models.PositiveIntegerField('ID пользователя', db_index=True)
    time_entered = models.DateTimeField('Дата и время вступления', null=True, db_index=True)
    time_left = models.DateTimeField('Дата и время выхода', null=True, db_index=True)
