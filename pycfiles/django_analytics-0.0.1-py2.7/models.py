...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/analytics/models.py
# Compiled at: 2011-05-24 10:08:02
...
class Metric(models.Model):
    ...
    "registrations".
    """
    uid = models.CharField(max_length=100, help_text=_('A unique name for this metric, so that it can be addressed from the code.'), unique=True, db_index=True)
    title = models.CharField(max_length=100, help_text=_('A short, descriptive title for this metric'))
    metric_class = models.CharField(max_length=200, help_text=_('The Python class responsible for the calculation of this metric.'))
    active = models.BooleanField(default=True, help_text=_('Whether or not to include this metric in regular calculations.'))

    def __unicode__(self):
        return self.title

    def latest_count(self, frequency=settings.STATISTIC_FREQUENCY_DAILY, count=True, cumulative=True, max_date=None):
        """
        ...
        try:
            if max_date is not None:
                stat = self.statistics.filter(frequency=frequency, date_time__lte=max_date).order_by('-date_time')[0]
            else:
...
class Statistic(models.Model):
    ...
    """
    metric = models.ForeignKey(Metric, db_index=True, related_name='statistics')
    date_time = models.DateTimeField(db_index=True)
    frequency = models.CharField(max_length=1, choices=settings.STATISTIC_FREQUENCY_CHOICES, db_index=True)
    count = models.IntegerField(default=0)
    cumulative_count = models.BigIntegerField(default=0)

    def __unicode__(self):
        return '%s at %s' % (self.metric, self.date_time.strftime('%Y-%m-%d %H:%M:%S'))
    ...
    def update_count(self, count=None):
        ...
        if count is None:
            ...
            self.count = count
        try:
            last_statistic = Statistic.objects.filter(metric=self.metric, frequency=self.frequency, date_time__lt=self.date_time).order_by('-date_time')[0]
            self.cumulative_count = last_statistic.cumulative_count + count
        except IndexError:
            self.cumulative_count = count

        self.save()
        next_statistics = [ s for s in Statistic.objects.filter(metric=self.metric, frequency=self.frequency, date_time__gt=self.date_time).order_by('date_time')
                          ]
