...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/analytics/geckoboard_views.py
# Compiled at: 2011-05-24 10:09:30
...
def get_gecko_params(request, uid=None, days_back=0, cumulative=True, frequency=settings.STATISTIC_FREQUENCY_DAILY, min_val=0, max_val=100, chart_type='standard', percentage='show', sort=False):
    ...
    view request.
    """
    return {'days_back': int(request.GET.get('daysback', days_back)), 
...
def geckoboard_number_widget(request):
    ...
    try:
        ...
        return (latest_stat.count, 0)

    if params['cumulative']:
        return (latest_stat.cumulative_count, prev_stat.cumulative_count)
    return (latest_stat.count, prev_stat.count)
...
def geckoboard_rag_widget(request):
    ...
    results = [ (metric.latest_count(frequency=params['frequency'], count=not params['cumulative'], cumulative=params['cumulative'], max_date=max_date), metric.title) for metric in metrics
              ]
...
def geckoboard_pie_chart(request):
    ...
    metrics = Metric.objects.filter(uid__in=params['uids'])
    results = [ (metric.latest_count(frequency=params['frequency'], count=not params['cumulative'], cumulative=params['cumulative']), metric.title, get_next_colour()) for metric in metrics
              ]
    return tuple(results)
...
def geckoboard_line_chart(request):
    ...
    stats = [ s for s in metric.statistics.filter(frequency=params['frequency'], date_time__gte=start_date).order_by('date_time')
            ]
    if len(stats) == 0:
...
def geckoboard_geckometer(request):
    ...
    metric = Metric.objects.get(uid=params['uid'])
    return (
     metric.latest_count(frequency=params['frequency'], count=not params['cumulative'], cumulative=params['cumulative']), params['min'], params['max'])
