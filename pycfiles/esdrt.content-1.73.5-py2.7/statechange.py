...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/esdrt/content/browser/statechange.py
...
class IFinishObservationReasonForm(Interface):
    comments = schema.Text(title=_('Enter comments if you want'), required=False)
    ...
    def finish_observation(self, action):
        ...
        with api.env.adopt_roles(['Manager']):
            if api.content.get_state(self.context) == 'phase1-conclusions':
                ...
                return self.context.content_status_modify(workflow_action='phase1-request-close')
            if api.content.get_state(self.context) == 'phase2-conclusions':
                self.context.closing_comments_phase2 = comments
                return self.context.content_status_modify(workflow_action='phase2-finish-observation')
...
class IDenyFinishObservationReasonForm(Interface):
    comments = schema.Text(title=_('Enter your reasons to deny the finishing of this observation'), required=False)
    ...
    def finish_observation(self, action):
        ...
        with api.env.adopt_roles(['Manager']):
            if api.content.get_state(self.context) == 'phase1-close-requested':
                ...
                return self.context.content_status_modify(workflow_action='phase1-deny-closure')
            if api.content.get_state(self.context) == 'phase2-close-requested':
                self.context.closing_deny_comments_phase2 = comments
                return self.context.content_status_modify(workflow_action='phase2-deny-finishing-observation')
...
class IAssignAnswererForm(Interface):
    answerers = schema.Choice(title=_('Select the answerers'), vocabulary='plone.app.vocabularies.Users')
    workflow_action = schema.TextLine(title=_('Workflow action'), required=True)
...
class AssignAnswererForm(BrowserView):
    ...
    def revoke_all_roles(self):
        ...
        for userId, username, cp in self.get_counterpart_users():
            if cp:
                revoke_roles(username=userId, obj=target, roles=[
                 'MSExpert'], inherit=False)
    ...
    def __call__(self):
        ...
        if self.request.form.get('send', None):
            ...
            for username in usernames:
                api.user.grant_roles(username=username, roles=[
                 'MSExpert'], obj=target)

            if api.content.get_state(self.context) in ('phase1-pending', 'phase1-pending-answer-drafting'):
                wf_action = 'phase1-assign-answerer'
            elif api.content.get_state(self.context) in ('phase2-pending', 'phase2-pending-answer-drafting'):
            ...
            return self.context.content_status_modify(workflow_action=wf_action)
        else:
            self.revoke_all_roles()
            return self.index()
            return
...
class IAssignCounterPartForm(Interface):
    counterpart = schema.TextLine(title=_('Select the counterpart'))
    workflow_action = schema.TextLine(title=_('Workflow action'), required=True)
...
class AssignCounterPartForm(BrowserView):
    ...
    def get_counterpart_users(self):
        ...
        target = self.assignation_target()
        local_roles = target.get_local_roles()
        counterpart_users = [ u[0] for u in local_roles if 'CounterPart' in u[1] ]
        for groupname in self.target_groupnames():
            try:
                data = [ (u[0], u[1], u[0] in counterpart_users) for u in self.get_users(groupname) if current_id != u
                       ]
                users.extend(data)
            except api.user.GroupNotFoundError:
                from logging import getLogger
...
class IAssignConclusionReviewerForm(Interface):
    reviewers = schema.Choice(title=_('Select the conclusion reviewers'), vocabulary='plone.app.vocabularies.Users')
...
class ReAssignCounterPartForm(AssignCounterPartForm):
    ...
    def __call__(self):
        ...
        if self.request.form.get('send', None):
            ...
            notify(target, _temp, subject, role='CounterPart', notification_name='question_to_counterpart')
            return self.request.response.redirect(url)
        else:
            return self.index()
            return
...
class IRequestRedraftReasonForm(Interface):
    comments = schema.Text(title=_('Enter your reasons to request redraft of this question'), required=False)
    ...
    def request_redraft(self, action):
        ...
        with api.env.adopt_roles(['Manager']):
            ...
            if cur_state.startswith('phase1'):
                ...
                return self.context.content_status_modify(workflow_action='phase1-redraft')
            if cur_state.startswith('phase2'):
                self.context.request_redraft_comments_phase2 = comments
                return self.context.content_status_modify(workflow_action='phase2-redraft')
        return self.response.redirect(self.context.absolute_url())
...
class AssignConclusionReviewerForm(BrowserView):
    ...
    def get_counterpart_users(self):
        ...
        for groupname in self.target_groupnames():
            try:
                data = [ (u[0], u[1], u[0] in counterpart_users) for u in self.get_users(groupname=groupname) if current_id != u
                       ]
                users.extend(data)
            except api.user.GroupNotFoundError:
                from logging import getLogger
    ...
    def __call__(self):
        ...
        if self.request.form.get('send', None):
            ...
            self.revoke_all_roles()
            for username in usernames:
                api.user.grant_roles(username=username, obj=target, roles=[
                 'CounterPart'])
