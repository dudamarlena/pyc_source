...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/esdrt/content/notifications/observation_finalised.py
...
def notification_ms(context, event):
    ...
    if event.action in [_act_ph1, _act_ph2]:
        ...
        if tpl_remarks:
            ...
            tpl = _temp_remarks
        notify(observation, tpl, subject, 'MSAuthority', 'observation_finalised')
...
def notification_rev_ph1(context, event):
    ...
    if event.action in ('phase1-close', ):
        ...
        notify(observation, _temp, subject, 'ReviewerPhase1', 'observation_finalised')
...
def notification_rev_ph2(context, event):
    ...
    if event.action in ('phase2-confirm-finishing-observation', ):
        observation = context
        subject = 'Your observation was finalised'
        notify(observation, _temp, subject, 'ReviewerPhase2', 'observation_finalised')