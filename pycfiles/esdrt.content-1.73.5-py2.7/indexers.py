...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/esdrt/content/indexers.py
# Compiled at: 2020-04-29 12:21:59
...
def SearchableText(context):
    ...
    try:
        questions = context.getFolderContents({'portal_type': 'Question'}, full_objects=True)
        items.extend(to_unicode(context.id))
    except:
        questions = []

    try:
        conclusions = context.getFolderContents({'portal_type': 'Conclusion'}, full_objects=True)
    except:
        conclusions = []

    try:
        conclusionsphase2 = context.getFolderContents({'portal_type': 'ConclusionsPhase2'}, full_objects=True)
    except:
        conclusionsphase2 = []

    for question in questions:
        comments = question.getFolderContents({'portal_type': 'Comment'}, full_objects=True)
        answers = question.getFolderContents({'portal_type': 'CommentAnswer'}, full_objects=True)
    ...
    for conclusion in conclusionsphase2:
        items.extend(index_fields(getFieldsInOrder(IConclusionsPhase2), conclusion))
...
def index_fields(fields, context):
    ...
    for name, field in fields:
        ...
        if IRichTextValue.providedBy(value):
            ...
            value = transforms.convertTo('text/plain', html, mimetype='text/html').getData().strip()
        if value:
...
def has_closing_remarks(context):
    try:
        conclusions = [ c for c in context.values() if c.portal_type in ('Conclusion',
                                                                         'ConclusionsPhase2')
                      ]
