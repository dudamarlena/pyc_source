...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/esdrt/content/observation.py
# Compiled at: 2020-04-29 11:15:49
try:
...
class IObservation(form.Schema, IImageScaleTraversable):
    ...
    """
    text = schema.Text(title='Short description by expert/reviewer', required=True, description='Describe the issue identified. Keep it short, you cannot change this description once you have sent it to the QE/LR. MS can only see the question once it has been approved and sent by the QE/LR. The question to the MS should be asked in the Q&A tab, not here.')
    country = schema.Choice(title='Country', vocabulary='esdrt.content.eea_member_states', required=True)
    crf_code = schema.Choice(title='CRF category codes', vocabulary='esdrt.content.crf_code', required=True)
    year = schema.TextLine(title='Inventory year', description="Inventory year is the year, a range or a list of years or a (e.g. '2012', '2009-2012', '2009, 2012, 2013') when the emissions had occured for which an issue was observed in the review.", required=True)
    form.widget(gas=CheckBoxFieldWidget)
    gas = schema.List(title='Gas', value_type=schema.Choice(vocabulary='esdrt.content.gas'), required=True)
    review_year = schema.Int(title='Review year', description='Review year is the year in which the inventory was submitted and the review was carried out', required=True)
    fuel = schema.Choice(title='Fuel', vocabulary='esdrt.content.fuel', required=False)
    ms_key_catagory = schema.Bool(title='MS key category')
    eu_key_catagory = schema.Bool(title='EU key category')
    form.widget(parameter=CheckBoxFieldWidget)
    parameter = schema.List(title='Parameter', value_type=schema.Choice(vocabulary='esdrt.content.parameter', required=True), required=True)
    form.widget(highlight=CheckBoxFieldWidget)
    highlight = schema.List(title='Description flags', description="Description flags highlight important information that is closely related to the main purpose of 'initial checks' and ESD review", value_type=schema.Choice(vocabulary='esdrt.content.highlight'), required=False, default=[])
    ...
    closing_comments_phase2 = schema.Text(title='Finish request comments for phase 2', required=False)
    form.write_permission(closing_deny_comments_phase2='cmf.ManagePortal')
    closing_deny_comments_phase2 = schema.Text(title='Finish deny comments for phase 2', required=False)
...
def check_country(value):
    ...
    valid = False
    for group in groups:
        if group.startswith('extranet-esd-ghginv-sr-') and group.endswith('-%s' % value):
            valid = True
        if group.startswith('extranet-esd-esdreview-reviewexp-') and group.endswith('-%s' % value):
            valid = True
...
class Observation(dexterity.Container):
    ...
    def country_value(self):
        return self._vocabulary_value('esdrt.content.eea_member_states', self.country)
    ...
    def parameter_value(self):
        parameters = [ self._vocabulary_value('esdrt.content.parameter', p) for p in self.parameter
                     ]
        return (', ').join(parameters)

    def gas_value(self):
        gases = [ self._vocabulary_value('esdrt.content.gas', g) for g in self.gas ]
        return (', ').join(gases)

    def highlight_value(self):
        if self.highlight:
            highlight = [ self._vocabulary_value('esdrt.content.highlight', h) for h in self.highlight
                        ]
            return (', ').join(highlight)
        return ''

    def finish_reason_value(self):
        return self._vocabulary_value('esdrt.content.finishobservationreasons', self.closing_reason)

    def finish_deny_reason_value(self):
        return self._vocabulary_value('esdrt.content.finishobservationdenyreasons', self.closing_deny_reason)
    ...
    def can_draft_conclusions(self):
        ...
        if len(questions) > 0:
            q = questions[0]
            return q.get_state_api() in ('phase1-draft', 'phase1-drafted', 'phase1-recalled-lr',
                                         'phase1-closed', 'phase2-draft', 'phase2-drafted',
                                         'phase2-recalled-lr', 'phase2-closed')
        else:
            return True
    ...
    def wf_location(self):
        ...
        if self.get_status() == 'phase1-closed':
            return 'Quality expert'
        if self.get_status() == 'phase2-closed':
            return 'Lead reviewer'
        if self.get_status() == 'phase1-conclusions':
            return 'Sector expert'
        if self.get_status() == 'phase2-conclusions':
            return 'Review expert'
        if self.get_status() in ('phase1-conclusion-discussion', 'phase2-conclusion-discussion'):
            return 'Counterpart'
        if self.get_status() == 'phase1-close-requested':
            return 'Quality expert'
        if self.get_status() == 'phase2-close-requested':
            return 'Lead reviewer'
        questions = self.get_values_cat('Question')
        if questions:
            ...
            if state in ('phase2-draft', 'phase2-closed'):
                return 'Review expert'
            if state in ('phase1-counterpart-comments', 'phase2-counterpart-comments'):
                return 'Counterparts'
            if state in ('phase1-drafted', 'phase1-recalled-lr'):
                return 'Quality Expert'
            if state in ('phase2-drafted', 'phase2-recalled-lr'):
                return 'Lead reviewer'
            if state in ('phase1-pending', 'phase1-answered', 'phase1-pending-answer-drafting',
                         'phase1-recalled-msa', 'phase2-pending', 'phase2-answered',
                         'phase2-pending-answer-drafting', 'phase2-recalled-msa'):
                return 'Member state coordinator'
            if state in ('phase1-expert-comments', 'phase2-expert-comments'):
    ...
    def wf_status(self):
        ...
        if self.get_status() in ('phase1-closed', 'phase2-closed'):
            return ['Observation finished', 'observationIcon']
        if self.get_status() in ('phase1-close-requested', 'phase2-close-requested'):
            return [
             'Observation finish requested', 'observationIcon']
        if self.get_status() in ('phase1-conclusions', 'phase2-conclusions'):
            return ['Conclusion ongoing', 'conclusionIcon']
        if self.get_status() in ('phase1-conclusion-discussion', 'phase2-conclusion-discussion'):
            return [
             'Counterparts comments requested', 'conclusionIcon']
        questions = self.get_values_cat('Question')
        if questions:
            ...
            if state in ('phase1-counterpart-comments', 'phase2-counterpart-comments'):
                return [
                 "Counterpart's comments requested", 'questionIcon']
            if state in ('phase1-answered', 'phase2-answered'):
                return ['Pending question', 'questionIcon']
            if state in ('phase1-pending', 'phase1-pending-answer-drafting', 'phase1-recalled-msa',
                         'phase2-pending', 'phase2-pending-answer-drafting', 'phase2-recalled-msa'):
                return [
                 'Open question', 'questionIcon']
            if state in ('phase1-drafted', 'phase1-recalled-lr', 'phase2-drafted',
                         'phase2-recalled-lr'):
                return [
                 'Draft question', 'questionIcon']
            if state in ('phase1-expert-comments', 'phase2-expert-comments'):
                return [
                 'MS expert comments requested', 'questionIcon']
    ...
    def observation_status(self):
        ...
        if status in ('phase1-draft', 'phase2-draft', 'phase1-counterpart-comments',
                      'phase2-counterpart-comments', 'observation-phase1-draft',
                      'observation-phase2-draft'):
            return 'SRRE'
        if status in ('phase1-drafted', 'phase2-drafted', 'phase1-recalled-lr', 'phase2-recalled-lr'):
            return 'LRQE'
        if status in ('phase1-pending', 'phase2-pending', 'phase1-pending-answer-drafting',
                      'phase2-pending-answer-drafting', 'phase1-expert-comments',
                      'phase2-expert-comments', 'phase1-recalled-msa', 'phase1-recalled-msa'):
            return 'MSC'
        if status in ('phase1-answered', 'phase2-answered'):
            return 'answered'
        if status in ('phase1-conclusions', 'phase2-conclusions', 'phase1-conclusion-discussion',
                      'phase2-conclusion-discussion'):
            return 'conclusions'
        if status in ('phase1-close-requested', 'phase2-close-requested'):
            return 'close-requested'
        if status in ('phase1-closed', 'phase2-closed'):
...
class ObservationMixin(grok.View):
    ...
    def can_view_redraft_reason(self):
        ...
        return self.context.is_secretariat() or set(self.user_roles).intersection(allowed_roles)
    ...
    def in_conclusions_phase1(self):
        ...
        return state in ('phase1-conclusions', 'phase1-conclusion-discussion', 'phase1-close-requested',
                         'phase1-closed')

    def in_conclusions_phase2(self):
        state = self.context.get_status()
        return state in ('phase2-conclusions', 'phase2-conclusion-discussion', 'phase2-close-requested',
                         'phase2-closed')
...
class ObservationView(ObservationMixin):
    ...
    def can_export_observation(self):
        sm = getSecurityManager()
        return sm.checkPermission('esdrt.content: Export an Observation', self.context)
        ...
        if chats:
            ...
            for chat in chats:
                ...
                if chat.portal_type.lower() == 'comment':
                    p = document.add_paragraph('> %s' % self.strip_special_chars(chat.text))
                    p = document.add_paragraph('From expert/reviewer To Member State \t\t %s' % (sent_info % date.strftime('%d %b %Y, %H:%M CET')))
                if chat.portal_type.lower() == 'commentanswer':
                    p = document.add_paragraph('< %s' % self.strip_special_chars(chat.text))
                    p = document.add_paragraph('From Member State To expert/reviewer \t\t %s' % (sent_info % date.strftime('%d %b %Y, %H:%M CET')))
...
class AddAnswerForm(Form):
    ...
    def add_answer(self, action):
        ...
        if context.has_answers():
            raise ActionExecutionError(Invalid('Question is already answered!'))
        id = str(int(time()))
...
class EditConclusionAndCloseComments(grok.View):
    ...
    def render(self):
        api.content.transition(obj=self.context, transition='phase1-finish-comments')
        conclusions = self.context.get_values_cat('Conclusion')
...
class EditConclusionP2AndCloseComments(grok.View):
    ...
    def render(self):
        api.content.transition(obj=self.context, transition='phase2-finish-comments')
        conclusions = self.context.get_values_cat('ConclusionsPhase2')
        conclusion = conclusions[0]
        url = '%s/edit' % conclusion.absolute_url()
        return self.request.response.redirect(url)
...
class AddConclusions(grok.View):
    ...
    def render(self):
        ...
        transitions_available = [ action['id'] for action in workflow.listActions(object=context) ]
        if context.get_status().startswith('phase1-'):
            current_user_id = api.user.get_current().getId()
            user_roles = api.user.get_roles(username=current_user_id, obj=context)
            if 'ReviewerPhase1' in user_roles:
                ...
                if cs:
                    ...
                    id = context.invokeFactory(id=str(int(time())), type_name='Conclusion', text='')
                    cs = self.context.get_values_cat('Conclusion')
                    conclusion = cs[0]
                    url = conclusion.absolute_url() + '/edit'
            else:
            ...
            if 'phase1-draft-conclusions' in transitions_available:
                api.content.transition(obj=context, transition='phase1-draft-conclusions')
            ...
            if 'ReviewerPhase2' in user_roles:
                ...
                if csp2:
                    ...
                    id = context.invokeFactory(id=str(int(time())), type_name='ConclusionsPhase2', text='')
            ...
            if 'phase2-draft-conclusions' in transitions_available:
                api.content.transition(obj=context, transition='phase2-draft-conclusions')
        else:
            raise ActionExecutionError(Invalid('Invalid context'))
