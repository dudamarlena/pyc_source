...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/esdrt/content/notifications/question_to_ms.py
...
def notification_ms(context, event):
    ...
    if event.action in ('phase1-approve-question', 'phase2-approve-question'):
        ...
        notify(observation, _temp, subject, role='MSAuthority', notification_name='question_to_ms')
...
def notification_rev_ph1(context, event):
    ...
    if event.action in ('phase1-approve-question', ):
        ...
        notify(observation, _temp, subject, role='ReviewerPhase1', notification_name='question_to_ms')
...
def notification_rev_ph2(context, event):
    ...
    if event.action in ('phase2-approve-question', ):
        observation = aq_parent(context)
        subject = 'Your observation was sent to MS'
        notify(observation, _temp, subject, role='ReviewerPhase2', notification_name='question_to_ms')