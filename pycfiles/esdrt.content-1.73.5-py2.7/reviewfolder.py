...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/esdrt/content/reviewfolder.py
# Compiled at: 2020-04-30 12:13:22
...
QUESTION_WORKFLOW_MAP = {'SRRE': 'Sector Reviewer / Review Expert', 
...
LDAP_QUERY_GROUPS = '(|(cn=extranet-esd-ghginv-qualityexpert-*)(cn=extranet-esd-esdreview-leadreview-*)(cn=extranet-esd-ghginv-sr-*)(cn=extranet-esd-esdreview-reviewexp-*))'
...
def filter_for_ms(brains, context):
    ...
    for brain in brains:
        if not any((is_msa, is_mse)):
            result.append(brain)
        elif is_msa and (brain.observation_sent_to_msc or brain.has_closing_remarks and brain.review_state.endswith('-closed')):
            result.append(brain)
...
class IReviewFolder(plone.directives.form.Schema, IImageScaleTraversable):
    ...
    """
    tableau_statistics = Text(title='Tableau statistics embed code', required=False)
    plone.directives.form.widget(tableau_statistics_roles=CheckBoxFieldWidget)
    tableau_statistics_roles = List(title='Roles that can access the statistics', value_type=Choice(vocabulary='esdrt.content.roles'))
...
class ReviewFolderMixin(BrowserView):
    ...
    def get_review_years(self):
        catalog = api.portal.get_tool('portal_catalog')
        return [ c for c in catalog.uniqueValuesFor('review_year') if isinstance(c, basestring)
               ]
    ...
    def get_gases(self):
        vocab_factory = getUtility(IVocabularyFactory, name='esdrt.content.gas')
        vocabulary = vocab_factory(self.context)
        return [ (x.value, x.title) for x in vocabulary ]
...
class ReviewFolderBrowserView(ReviewFolderMixin):
    ...
    def table(self, context, request, sort_on='modified', sort_order='reverse'):
        ...
        view_url = url + '/view'
        table = Table(self.request, url, view_url, self.folderitems(sort_on, sort_order), pagesize=pagesize)
        table.render = ViewPageTemplateFile('browser/templates/reviewfolder_get_table.pt')
        table.is_secretariat = self.is_secretariat
        table.question_workflow_map = QUESTION_WORKFLOW_MAP
        return table

    def update_table(self, pagenumber='1', sort_on='modified', sort_order='reverse', show_all=False):
        self.request.set('sort_on', sort_on)
        self.request.set('pagenumber', pagenumber)
        table = self.table(self.context, self.request, sort_on=sort_on, sort_order=sort_order)
...
class IExportForm(Interface):
    exportFields = List(title='Fields to export', description='Select which fields you want to add into XLS', required=False, value_type=Choice(source=fields_vocabulary_factory))
...
class ExportReviewFolderForm(form.Form, ReviewFolderMixin):
    ...
    def translate_highlights(self, highlights):
        return [ self._vocabulary_value('esdrt.content.highlight', highlight) for highlight in highlights
               ]
    ...
    def extract_data(self, form_data):
        ...
        fields_to_export = [ name for name in form_data.get('exportFields', []) if not user_is_ms or name not in EXCLUDE_FIELDS_FOR_MS
                           ]
        ...
        base_len = 0
        rows = []
        query_ldap = set(fields_to_export).intersection(('get_name_qe', 'get_name_lr',
                                                         'get_name_se', 'get_name_re'))
        if query_ldap:
            self._ldap_group_members = ldap_utils.query_group_members(api.portal.get(), LDAP_QUERY_GROUPS)
        for observation in observations:
            row = [observation.getId]
            for key in fields_to_export:
                if key in ('observation_is_potential_significant_issue', 'observation_is_potential_technical_correction',
                    ...
                    row.append(safe_unicode((', ').join(self.translate_highlights(observation[key] or []))))
                elif key == 'observation_questions_workflow':
                    row_val = (', ').join([ ('. ').join((str(idx), QUESTION_WORKFLOW_MAP.get(val, val))) for idx, val in enumerate(observation[key], start=1)
                                          ])
                    row.append(row_val if row_val else QUESTION_WORKFLOW_MAP.get(observation['observation_status'], 'unknown'))
                elif key == 'last_question_workflow':
                    q_wfs = observation['observation_questions_workflow']
                    last_q_wf = q_wfs[(-1)] if q_wfs else None
                    row_val = QUESTION_WORKFLOW_MAP.get(last_q_wf, last_q_wf) if last_q_wf else None
                    row.append(row_val if row_val else QUESTION_WORKFLOW_MAP.get(observation['observation_status'], 'unknown'))
                    ...
                    row.append(observation.getObject().myHistory()[(-1)]['time'].asdatetime().isoformat())
                else:
    ...
    def extract_qa(self, catalog, observation):
        question_brains = catalog(portal_type='Question', path=observation.getPath())
        questions = tuple([ brain.getObject() for brain in question_brains ])
        comments = tuple(itertools.chain(*[ question.get_questions() for question in questions ]))
        mapping = dict(Comment='Question', CommentAnswer='Answer')
        return tuple([ ('{}: {}').format(mapping[comment.portal_type], safe_unicode(comment.text)) for comment in comments
                     ])
...
def _item_user(fun, self, user, item):
    return (
     user.getId(), item.getId(), item.modified())
...
class RoleMapItem(object):
    ...
    def check_roles(self, rolename):
        if rolename == 'CounterPart':
            return self.isCP
        if rolename == 'MSAuthority':
            return self.isMSA
        if rolename == 'SectorExpert':
            return self.isSE
        if rolename == 'ReviewExpert':
            return self.isRE
        if rolename == 'NotCounterPartPhase1':
            return not self.isCP and self.isSE
        if rolename == 'NotCounterPartPhase2':
            return not self.isCP and self.isRE
        if rolename == 'LeadReviewer':
            return self.isLR
        if rolename == 'QualityExpert':
            return self.isQE
        return False
...
class InboxReviewFolderView(BrowserView):
    ...
    def get_draft_observations(self):
        ...
        """
        phase1 = self.get_observations(rolecheck='SectorExpert', observation_question_status=[
         'observation-phase1-draft'])
        phase2 = self.get_observations(rolecheck='ReviewExpert', observation_question_status=[
    ...
    def get_draft_questions(self):
        ...
        """
        phase1 = self.get_observations(rolecheck='SectorExpert', observation_question_status=[
         'phase1-draft', 'phase1-drafted'])
        phase2 = self.get_observations(rolecheck='ReviewExpert', observation_question_status=[
         'phase2-draft', 'phase2-drafted'])
        no_conclusion_yet = self.get_observations(observation_question_status=[
         'phase1-closed', 'phase2-closed'], observation_finalisation_reason='no-conclusion-yet')
    ...
    def get_counterpart_questions_to_comment(self):
        ...
        """
        return self.get_observations(rolecheck='CounterPart', observation_question_status=[
    ...
    def get_ms_answers_to_review(self):
        """
         ...
         that need review
        """
        answered_phase1 = self.get_observations(rolecheck='SectorExpert', observation_question_status=[
         'phase1-answered'])
        answered_phase2 = self.get_observations(rolecheck='ReviewExpert', observation_question_status=[
         'phase2-answered'])
        pending_phase1 = self.get_observations(rolecheck='SectorExpert', observation_question_status=[
         'phase1-closed'], review_state=[
         'phase1-pending'])
        pending_phase2 = self.get_observations(rolecheck='ReviewExpert', observation_question_status=[
         'phase2-closed'], review_state=[
         'phase2-pending'])
    ...
    def get_approval_questions(self):
        ...
        phase1 = self.get_observations(rolecheck='SectorExpert', observation_question_status=statuses_phase1)
        phase2 = self.get_observations(rolecheck='ReviewExpert', observation_question_status=statuses_phase2)
        return phase1 + phase2
    ...
    def get_waiting_for_comment_from_counterparts_for_question(self):
        """
         Role: Sector expert / Review expert
        """
        phase1 = self.get_observations(rolecheck='NotCounterPartPhase1', observation_question_status=[
         'phase1-counterpart-comments'])
        phase2 = self.get_observations(rolecheck='NotCounterPartPhase2', observation_question_status=[
         'phase2-counterpart-comments'])
        return phase1 + phase2
    ...
    def get_questions_to_be_sent(self):
        ...
        """
        phase1 = self.get_observations(rolecheck='QualityExpert', observation_question_status=[
         'phase1-drafted',
         'phase1-recalled-lr'])
        phase2 = self.get_observations(rolecheck='LeadReviewer', observation_question_status=[
         'phase2-drafted',
         'phase2-recalled-lr'])
        return phase1 + phase2
    ...
    def get_questions_to_be_answered(self):
        ...
        """
        return self.get_observations(rolecheck='MSAuthority', observation_question_status=[
         'phase1-pending',
         'phase2-pending',
         'phase1-recalled-msa',
         'phase2-recalled-msa',
    ...
    def get_questions_with_comments_received_from_mse(self):
        ...
        return self.get_observations(rolecheck='MSAuthority', observation_question_status=[
         'phase1-expert-comments',
         'phase2-expert-comments'], last_answer_has_replies=True)
    ...
    def _get_observations_with_closing_remarks(self, rolecheck):
        ...
        """
        return self.get_observations(rolecheck=rolecheck, observation_question_status=[
         'phase1-closed', 'phase2-closed'], has_closing_remarks=True)
    ...
    def get_answers_requiring_comments_from_mse(self):
        ...
        """
        return self.get_observations(observation_question_status=[
         'phase1-expert-comments',
         'phase2-expert-comments'])
    ...
    def get_answers_sent_to_se_re(self):
        ...
        """
        answered = self.get_observations(observation_question_status=[
         'phase1-answered', 'phase2-answered'])
        ...
        not_closed = self.get_observations(review_state=statuses, observation_already_replied=True)
    ...
    def get_observations_with_my_comments(self):
        ...
        return self.get_observations(observation_question_status=[
         ...
         'phase2-pending-answer-drafting',
         'phase1-recalled-msa',
         'phase2-recalled-msa'], reply_comments_by_mse=[
    ...
    def get_observations_with_my_comments_sent_to_se_re(self):
        ...
        return self.get_observations(observation_question_status=[
         'phase1-answered',
         'phase2-answered'], reply_comments_by_mse=[
         api.user.get_current().getId()])
...
class FinalisedFolderView(BrowserView):
    ...
    def get_no_response_needed_observations(self):
        ...
        """
        return self.get_observations(observation_question_status=[
         'phase1-closed', 'phase2-closed'], observation_finalisation_reason='no-response-needed')
    ...
    def get_resolved_observations(self):
        ...
        """
        return self.get_observations(observation_question_status=[
         'phase1-closed', 'phase2-closed'], observation_finalisation_reason='resolved')
    ...
    def get_unresolved_observations(self):
        ...
        """
        return self.get_observations(observation_question_status=[
         'phase1-closed', 'phase2-closed'], observation_finalisation_reason='unresolved')
    ...
    def get_partly_resolved_observations(self):
        ...
        """
        return self.get_observations(observation_question_status=[
         'phase1-closed', 'phase2-closed'], observation_finalisation_reason='partly-resolved')
    ...
    def get_technical_correction_observations(self):
        ...
        """
        return self.get_observations(observation_question_status=[
         'phase1-closed', 'phase2-closed'], observation_finalisation_reason='technical-correction')
    ...
    def get_revised_estimate_observations(self):
        ...
        """
        return self.get_observations(observation_question_status=[
         'phase1-closed', 'phase2-closed'], observation_finalisation_reason='revised-estimate')
