...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/redi/utils/redcapClient.py
...
class RedcapClient(object):
    ...
    def get_data_from_redcap(self, records_to_fetch=None, events_to_fetch=None, fields_to_fetch=None, forms_to_fetch=None, return_format='xml'):
        ...
        try:
            response = self.project.export_records(records=records_to_fetch, events=events_to_fetch, fields=fields_to_fetch, forms=forms_to_fetch, format=return_format)
        except RedcapError as e:
            logger.debug(e.message)

        return response

    def send_data_to_redcap(self, data, max_retry_count, overwrite=False, retry_count=0):
        ...
        try:
            ...
            if retry_count == max_retry_count:
                message = 'Exiting since network connection timed out after reaching the maximum retry limit for resending data.'
                logger.debug(message)
                sys.exit(message)
            time.sleep(retry_count * 6)
            self.send_data_to_redcap(data, max_retry_count, 'overwrite', retry_count + 1)
        except RedcapError as e:
            logger.debug(e.message)
            raise