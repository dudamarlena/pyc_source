...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/redi/utils/redi_email.py
...
def send_email(host, port, sender, to_addr_list, cc_addr_list, subject, msg_body):
    """
    ...
    try:
        ...
        success = True
        logger.info('Success: Email with subject [' + subject + '] was sent to:' + str(to_addr_list))
    except SMTPException as smtpe:
        logger.error(('Unable to send email with subject [{0}] to {1} due: {2}').format(subject, str(to_addr_list), str(smtpe)))
        logger.info('Please check if the recipient email is valid')
    except Exception as e:
        logger.error(('Unable to send email with subject [{0}] to {1}\n due: {2}').format(subject, str(to_addr_list), str(e)))
