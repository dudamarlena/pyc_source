...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/redi/report.py
# Compiled at: 2018-08-13 08:58:37
import abc, time, pkg_resources
        ...
        try:
            ...
            try:
                try:
                    ...
                    logger.info(('==> You can review the summary report by opening: {} in your browser').format(html_report_path))
                except IOError:
...
class ReportCreator(object):
    ...
    def __init__(self, report_file_path, project_name, redcap_uri, sort_by_lab_id, writer):
        self._report_parameters = {'report_file_path': report_file_path, 
    ...
    def create_report(self, report_data, alert_summary, collection_date_summary_dict, duration_dict):
        ...
        report_xsl = pkg_resources.resource_filename(REDI_PACKAGE_NAME, 'utils/report.xsl')
...
def updateSummaryOfSpecimenTakenTimes(root, collection_date_summary_dict):
    ...
    percentElement.text = str(float(collection_date_summary_dict['blank']) / collection_date_summary_dict['total'] * 100)
