...
# [GCC 8.4.0]
# Embedded file name: /home/user/work/git/autopwn/autopwn/__init__.py
# Compiled at: 2015-07-30 12:18:40
# Size of source mod 2**32: 41111 bytes
import argparse, copy, cmd, collections, operator, os, random, re, readline, shlex, subprocess, sys, threading, time
...
class Arguments:
    ...
    def __init__(self, argslist):
        self.parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter, description=self.argparse_description, epilog=self.argparse_epilog)
        self.parser.add_argument('-t', '--targets', required=True, help='The file containing the targets')
        self.parser.add_argument('-d', '--assessment_directory', help='Specify assessment directory')
        self.parser.add_argument('-s', '--with_screen', action='store_true', help='Run tools in screen session')
        self.parser.add_argument('-p', '--parallel', action='store_true', help='Run tools in parallel regardless of assessment or global parallel option')
...
class Configuration:
    ...
    def load(self, load_type):
        pathname = os.path.abspath(self.find_path(__file__) or self.find_path(sys.argv[0]))
        if load_type == 'tools':
            load_directory = os.path.abspath(pathname) + '/tools/'
            load_string = 'Tools'
        else:
            if load_type == 'assessments':
...
class Search:
    ...
    def search(self, config_item, item_type_string, search_string):
        ...
        for item in config_item:
            if search_string in item['search_name'] or str.lower(search_string) in str.lower(item['description']):
        ...
        for tool in config.tools:
            if tool['name'] == tool_name:
                ...
                for option in tool['options']:
                    default_value = tool['options'][option].get('default_value', None)
    ...
    def show_options(self, config):
        ...
        for tool in config.tools:
            if tool['name'] in config.instance['tool']:
                for option in tool['options']:
                    required = tool['options'][option].get('required', False)
                    ...
                    example_values = str(tool['options'][option].get('example_values', None))
                    option_value = config.instance['config'].get(option, '')
                    if option not in option_displayed:
                        print('        {0:16} {1:16} {2:32} {3}'.format(option, option_value, example_values, required_string))
...
class Process:
    ...
    def __init__(self, config):
        ...
        for instance in config.job_queue:
            ...
            instance['options']['output_dir'] = instance['options']['date_day'] + '_autopwn_' + instance['options']['target_name']
            if config.arguments['screen'] == True:
                if Process.binary_exists('screen') != True:
                    if self.binary_exists('bash') != True:
                        Error(50, '[E] Missing binary - screen or bash')
                instance['screen_name'] = 'autopwn_' + instance['options']['target_name'] + '_' + instance['options']['target'] + '_' + instance['name']
                instance['execute_string'] = 'screen -D -m -S ' + instance['screen_name'] + ' ' + "bash -c '" + instance['execute_string'] + "'"
    ...
    def __init__(self, config, arg):
        ...
        if target_objects.get('targets', None) == None:
            ...
            for target in target_objects['targets']:
                ...
                if target.get('target_list') == None and target.get('target', None) == None:
                    Error(3, '[E] Target entry missing target_name and/or target')
                    continue
...
class Save:
            ...
            for selected_tool in config.instance['tool']:
                if selected_tool == imported_tool['name']:
                    ...
                    for option in imported_tool['options']:
                        ...
                        tool_option = imported_tool['options'][option].get('required', False)
                    ...
                    for option in imported_tool['options']:
                        default_option_value = imported_tool['options'][option].get('default_option_value', None)
                        if default_option_value != None:
                            config.instance['config'][option] = str(config.instance['config'][option][default_option_value])
                            continue
    ...
    def check_required_exists(self, config, option):
        ...
        if option in config.job_queue[(-1)]['options'] or instance_tool['options'][option].get('default_option_value', None) != None:
...
class Execute:
    ...
    def __init__(self, config):
        for instance in config.job_queue:
            ...
            if 'url' in instance:
                log = Log(config, os.getcwd(), False, 'tool_string', '# Executing ' + instance['name'] + ' tool (' + instance['url'] + ') for ' + instance['options']['target_name'] + ':\n' + instance['execute_string'])
            else:
                log = Log(config, os.getcwd(), False, 'tool_string', '# Executing ' + instance['name'] + ' tool:\n# ' + instance['execute_string'])
            time.sleep(0.1)
...
class RunThreads(threading.Thread):
    ...
    def execute_tool(self, config):
        ...
        if exitcode != 0:
            log = Log(self.config, os.getcwd(), False, 'tool_string', '# WARNING: ' + self.instance['name'] + ' for ' + self.instance['options']['target_name'] + ' exited with non-zero return code')

    def run(self):
        print('[+] Launching ' + self.instance['name'] + ' for ' + self.instance['options']['target_name'])
        self.execute_tool(self.config)
        print('[-] ' + self.instance['name'] + ' for ' + self.instance['options']['target_name'] + ' is done..')
        stdout_boolean = self.instance['stdout']
        if stdout_boolean == True:
            log = Log(self.config, os.getcwd() + '/' + self.instance['options']['output_dir'], self.instance['options']['target_name'] + '_' + self.instance['name'], 'tool_output', self.tool_stdout)
        log = Log(self.config, os.getcwd(), False, 'tool_string', '# ' + self.instance['name'] + ' for ' + self.instance['options']['target_name'] + ' has finished')
...
class Log:
    ...
    def __init__(self, config, directory, log_filename, log_type, log_string):
        ...
        if log_type == 'tool_output':
            try:
                log_file = open(directory + '/' + date_time_filename + '_' + log_filename + '_stdout.log', 'a')
            except OSError as e:
    ...
    def __init__(self, view, config, **kwargs):
        ...
        if view == 'use':
            ...
            if config.instance['config']['assessment'] == True:
                for assessment in config.assessments:
                    if assessment['name'] == config.instance['config']['assessment_name']:
...
class Shell(cmd.Cmd):
    ...
    def complete_show(self, text, line, begin, end):
        ...
        if not text:
            ...
            completions = [operation for operation in operations if text in operation]
        return completions
    ...
    def complete_use(self, text, line, begin, end):
        completions = ''
        if not text:
            completions = [assessment['search_name'] for assessment in self.config.assessments]
            completions = completions + [tool['search_name'] for tool in self.config.tools]
        else:
            completions = [assessment['search_name'] for assessment in self.config.assessments if line.split(' ')[1] in assessment['search_name']]
            completions = completions + [tool['search_name'] for tool in self.config.tools if line.split(' ')[1] in tool['search_name']]
        return completions
    ...
    def complete_set(self, text, line, begin, end):
        ...
        if text != None:
            completions = [parameter for parameter in completions if text in parameter]
        return completions
