...
# [GCC 8.4.0]
# Embedded file name: /media/sf_work/git/autopwn/autopwn/__init__.py
# Compiled at: 2015-06-19 09:50:21
import argparse, copy, cmd, operator, os, random, re, readline, shlex, subprocess, sys, threading, time
...
class Save:
    ...
    def check_required_exists(self, config, option):
        parameter_found = False
        print('debug: ' + str(config.job_queue[(-1)]))
        print('debug: option = ' + option)
        print('debug: ' + str(hasattr(config.job_queue[(-1)]['options'], option)))
        for tool in config.tools:
