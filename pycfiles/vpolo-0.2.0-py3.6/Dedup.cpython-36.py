...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vpolo/avs_tools/Dedup.py
...
def collapse(visit_list, umis, pcr_correct):
    ...
    while len(removed_umis) != 0:
        ...
        for umi in visit_list[::-1]:
            umi_freq = umis[umi]
            if one_distance(umi, query_umi) and (not pcr_correct or pcr_correct and query_freq / 2.0 + 1 > umi_freq):
                removed_umis.append(umi)
                num_collapsed += 1
