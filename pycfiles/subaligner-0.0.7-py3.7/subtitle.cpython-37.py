...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/subaligner/subtitle.py
# Compiled at: 2020-02-25 13:05:05
# Size of source mod 2**32: 12609 bytes
import pysrt, tempfile, os, re
...
class Subtitle(object):
    __doc__ = 'Load a subtitle file into internal data structure\n    '
    _Subtitle__secret = object()
    ...
    def __init__(self, secret, subtitle_file_path, subtitle_format):
        ...
        """
        if not secret == Subtitle._Subtitle__secret:
            raise AssertionError('Only factory methods are supported when creating instances')
        else:
            self._Subtitle__subtitle_file_path = subtitle_file_path
            if subtitle_format == 'subrip':
                self._Subtitle__subs = pysrt.open(subtitle_file_path, encoding='utf-8')
            else:
                if subtitle_format == 'ttml':
                    self._Subtitle__subs = self._Subtitle__convert_ttml_to_subs(subtitle_file_path)
                else:
                    if subtitle_format == 'webvtt':
                        self._Subtitle__subs = self._Subtitle__convert_vtt_to_subs(subtitle_file_path)
                    else:
                        raise UnsupportedFormatException('Unknown subtitle format for file: {}'.format(subtitle_file_path))
    ...
    def load_subrip(cls, subtitle_file_path):
        ...
        """
        return cls(cls._Subtitle__secret, subtitle_file_path, 'subrip')
    ...
    def load_ttml(cls, subtitle_file_path):
        ...
        Returns:
            Subtitle -- Subtitle object.
        """
        return cls(cls._Subtitle__secret, subtitle_file_path, 'ttml')
    ...
    def load_webvtt(cls, subtitle_file_path):
        ...
        Returns:
            Subtitle -- Subtitle object.
        """
        return cls(cls._Subtitle__secret, subtitle_file_path, 'webvtt')
    ...
    def load(cls, subtitle_file_path):
        ...
        if file_extension in cls.SUBRIP_EXTENTIONS:
            return cls(cls._Subtitle__secret, subtitle_file_path, 'subrip')
        if file_extension in cls.TTML_EXTENSIONS:
            return cls(cls._Subtitle__secret, subtitle_file_path, 'ttml')
        if file_extension in cls.WEBVTT_EXTENSIONS:
            return cls(cls._Subtitle__secret, subtitle_file_path, 'webvtt')
        return cls(cls._Subtitle__secret, subtitle_file_path, 'unknown')

    @classmethod
    def shift_subtitle(cls, subtitle_file_path, seconds, shifted_subtitle_file_path=None, suffix='_shifted'):
        ...
        if file_extension.lower() in cls.SUBRIP_EXTENTIONS:
            subs = cls(cls._Subtitle__secret, subtitle_file_path, 'subrip').subs
            subs.shift(seconds=seconds)
            if shifted_subtitle_file_path is None:
                shifted_subtitle_file_path = subtitle_file_path.replace(file_extension, '{}{}'.format(suffix, file_extension))
        ...
        if file_extension.lower() in cls.TTML_EXTENSIONS:
            subs = cls(cls._Subtitle__secret, subtitle_file_path, 'ttml').subs
            subs.shift(seconds=seconds)
            tree = ElementTree.parse(subtitle_file_path)
            tt = tree.getroot()
            cues = tt.find('tt:body', Subtitle.TT_NS).find('tt:div', Subtitle.TT_NS).findall('tt:p', Subtitle.TT_NS)
            ...
            return shifted_subtitle_file_path
        if file_extension.lower() in cls.WEBVTT_EXTENSIONS:
            subs = cls(cls._Subtitle__secret, subtitle_file_path, 'webvtt').subs
            subs.shift(seconds=seconds)
            if shifted_subtitle_file_path is None:
                shifted_subtitle_file_path = subtitle_file_path.replace(file_extension, '{}{}'.format(suffix, file_extension))
            Subtitle.export_subtitle(subtitle_file_path, subs, shifted_subtitle_file_path)
    ...
    def export_subtitle(source_file_path, subs, target_file_path):
        ...
        if file_extension in Subtitle.SUBRIP_EXTENTIONS:
            ...
            if file_extension in Subtitle.TTML_EXTENSIONS:
                ...
                with open(target_file_path, 'w') as (target):
                    normalised = ElementTree.tostring(tt, encoding='utf8', method='xml').decode('utf-8').replace("<?xml version='1.0' encoding='utf8'?>", '<?xml version="1.0" encoding="utf8"?>')
                    target.write(normalised)
            else:
                if file_extension in Subtitle.WEBVTT_EXTENSIONS:
                    try:
                    ...
                    raise UnsupportedFormatException('Unknown subtitle format for file: {}'.format(source_file_path))
        ...
        for sub in subs:
            if se_uppercase is not None:
                if se_uppercase:
                    if sub.text.isupper():
                        new_subs.remove(sub)
                if se_uppercase or sub.text.islower():
                    new_subs.remove(sub)
    ...
    def remove_sound_effects_by_affixes(subs, se_prefix, se_suffix=None):
        ...
        for sub in subs:
            if se_suffix is not None:
                match = re.search('^{0}[^{0}{1}]+{1}$'.format(re.escape(se_prefix), re.escape(se_suffix)), sub.text)
            else:
    ...
    def subtitle_file_path(self):
        return self._Subtitle__subtitle_file_path

    @property
    def subs(self):
        return self._Subtitle__subs
