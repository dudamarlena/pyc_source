...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/Garen/Garen.py
# Compiled at: 2017-11-12 22:32:22
import threading, requests, re, urllib, urlparse, json
...
HTML_REGEX = re.compile('((?:src|action|href)=["\\\'])/')
JQUERY_REGEX = re.compile('(\\$\\.(?:get|post)\\(["\\\'])/')
JS_LOCATION_REGEX = re.compile('((?:window|document)\\.location.*=.*["\\\'])/')
CSS_REGEX = re.compile('(url\\(["\\\']?)/')
REGEXES = [
 HTML_REGEX, JQUERY_REGEX, JS_LOCATION_REGEX, CSS_REGEX]
...
class Garen(threading.Thread):
    ...
    def __init__(self, port=8899, browser_name='chrome', config_fpath=None, static_root_path=None, queue=None, pipe_filter=None, proxy_headers=None, proxy_cookies=None):
        """
    ...
    def get_response_headers(self, headers):
        ...
        """
        _ = (',').join(headers.keys())
        response_headers = Headers({'Access-Control-Allow-Origin': '*', 
    ...
    def run(self):
        ...
        def _proxy_request(path):
            ...
            if not request.headers.get('PROXY') and request.headers.get('HOST', ''):
                ...
                if request.method == 'POST' or request.method == 'PUT' or request.method == 'PATCH':
                    ...
                    form_data = None
                new_request_headers = dict(request_headers)
                ...
                for key, value in resp.headers.items():
                    ...
                    if key in ('content-length', 'connection', 'content-type', 'Content-Encoding',
                               'Vary', 'Cache-Control', 'Expires', 'Transfer-Encoding'):
                        continue
                    else:
                ...
                flask_response = Response(response=contents, status=resp.status_code, headers=response_headers, content_type=content_type)
        ...
        return False
        self.app.run(debug=False, use_reloader=False, host=self.listen_ip, port=self.listen_port, threaded=True)
    ...
    def print_proxy_topology(self, sender, request_domain, request_real_ip_host):
        ...
        _ = _.format(sender=sender, request_domain=request_domain, request_real_ip_host=request_real_ip_host).strip()
        for line in _.split('\n'):
            print line.lstrip()
...
def main():
    o = Garen(port=9988, config_fpath='/Volumes/dataDisk/github/Garen/tests/config/config.yml')
    o.daemon = True
    o.start()
    import time
    while True:
