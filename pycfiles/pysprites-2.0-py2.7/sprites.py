...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pysprites/sprites.py
# Compiled at: 2014-08-20 02:09:11
"""
  sprites.py
  ~~~~~~~~~~
...
def gsp_import_psd(gsp, *psds):
    ...
    def f(psd, prefix, buf):
        for r in psd.layers:
            ...
            if isgroup(r):
                f(r, prefix, buf)
            elif islayer(r) and r.bbox.width > 0 and r.bbox.height > 0:
    ...
    def lowest(self):
        ...
        for i in range(len(self._e)):
            if i > 0 and e[(i - 1)] != e[i]:
                ...
                if ls == None or e[i] < e[ls] or e[i] == e[ls] and i - ts > lr - ls:
                    ls = ts
                    lr = i
