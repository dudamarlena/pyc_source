...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hermit/shards/interface.py
# Compiled at: 2019-09-09 13:00:36
...
class ShardWordUserInterface(object):
    ...
    def get_change_password(self, name: str) -> Tuple[(bytes, bytes)]:
        ...
        old_password = prompt('old password> ',
    ...
    def get_name_for_shard(self, share_id, group_index, group_threshold, groups, member_index, member_threshold, shards):
        print_formatted_text('')
    ...
    def choose_shard(self, shards) -> Optional[str]:
        ...
        while True:
            ...
            shard_name = prompt(prompt_msg, completer=shardCompleter, **self.options).strip()
    ...
    def confirm_delete_shard(self, shard_name: str) -> bool:
        return prompt(('Really delete shard {0}? '.format(shard_name)), completer=(self.YesNoCompleter)) == 'yes'

    def confirm_initialize_file(self) -> bool:
        return prompt('Really initialize the shard file? ', completer=(self.YesNoCompleter)) == 'yes'
    ...
    def enter_shard_words(self, name: str) -> str:
        ...
        while 1:
            try:
                line = prompt('', completer=self.ShardWordCompleter, **self.options).strip()
            except EOFError:
                break
            ...
            if set(words).issubset(ShardWords):
                ...
                for word in words:
                    if word not in ShardWords:
                        print('{} is not a valid shard word, ignoring last line'.format(word))
    ...
    def enter_wallet_words(self) -> str:
        ...
        while 1:
            try:
                line = prompt('', completer=self.WalletWordCompleter, **self.options).strip()
            except EOFError:
                break
            ...
            if set(words).issubset(WalletWords):
                ...
                for word in words:
                    if word not in ShardWords:
                        print('{} is not a valid wallet word, ignoring last line'.format(word))
    ...
    def enter_group_information(self) -> Tuple[(int, List[Tuple[(int, int)]])]:
        ...
        group_threshold = int(prompt((HTML('<b>How many groups should be required to unlock the wallet (<i>P</i>)?</b> ')), completer=(self.SmallNumberCompleter)))
        groups = []
        ...
        while True:
            try:
                group_str = prompt((HTML('<b>What shard configuration should be used for <i>Group {}</i>?</b> '.format(len(groups) + 1))),
            ...
            if group_str == '':
                if len(groups) >= group_threshold:
                    break
            match = re.match('^\\s*(\\d+)\\s*of\\s*(\\d+)', group_str)
            if not match:
                print_formatted_text(input_error_message)
            else:
                n, m = match.groups()
                ...
                m = int(m)
                if n > m:
                    print_formatted_text(HTML('The number of required shards (<i>n</i>) must not be larger than the total number of shards (<i>m</i>)'))
                else:
                    groups.append((n, m))
