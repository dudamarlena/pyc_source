...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hermit/shards/shard_set.py
# Compiled at: 2019-11-20 10:59:30
# Size of source mod 2**32: 12639 bytes
import bson, os, textwrap
...
class ShardSet(object):
    ...
    def _ensure_shards(self, shards_expected: bool=True) -> None:
        if not self._shards_loaded:
            ...
            for name, shard_bytes in bdata.items():
                self.shards[name] = Shard(name, shamir_share.mnemonic_from_bytes(shard_bytes))
    ...
    def to_bytes(self):
        data = {name:shard.to_bytes() for name, shard in self.shards.items()}
    ...
    def create_random_share(self):
        ...
        mnemonics = shamir_share.generate_mnemonics_random(group_threshold,
          groups, strength_bits=256)
        self._import_share_mnemonic_groups(mnemonics)
    ...
    def _import_share_mnemonic_groups(self, mnemonic_groups: List[List[str]]) -> None:
        for group in mnemonic_groups:
            for mnemonic in group:
                share_id, _, group_index, group_threshold, groups, member_identifier, member_threshold, _ = shamir_share.decode_mnemonic(mnemonic)
                name = self.interface.get_name_for_shard(share_id, group_index, group_threshold, groups, member_identifier, member_threshold, self.shards)
                password = self.interface.confirm_password()
                shard = Shard(name, shamir_share.encrypt_mnemonic(mnemonic, password), self.interface)
    ...
    def secret_seed(self) -> bytes:
        self._ensure_shards()
        selected = {}
        selected_share_id = None
        selected_shard_ids = set()
        selected_shards = set()
        satisfied = False
        filled_groups = set()
        while not satisfied:
            shards = [shard for name, shard in self.shards.items() if name not in selected and (selected_share_id is None or shard.share_id == selected_share_id) if shard.shard_id not in selected_shard_ids if shard.group_id not in filled_groups]
    ...
    def reveal_shard(self, shard_name: str) -> None:
        ...
        print_formatted_text(HTML('Encrypted SLIP39 phrase for shard <i>{}</i>:\n'.format(shard_name)))
    ...
    def import_shard_qr(self, name: str, shard_data: bytes) -> None:
        ...
        print_formatted_text("Importing shard '{}' from qr code as shard '{}'".format(old_name, name))
    ...
    def copy_shard(self, original: str, copy: str) -> None:
        ...
        if copy in self.shards:
            err_msg = 'Shard {} exists. If you need to replace it, delete it first.'.format(copy)
            raise HermitError(err_msg)
        original_shard = self.shards[original]
        copy_shard = Shard(copy,
          (original_shard.encrypted_mnemonic), interface=(self.interface))
