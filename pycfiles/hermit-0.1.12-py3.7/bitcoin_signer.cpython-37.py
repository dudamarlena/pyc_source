...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hermit/signer/bitcoin_signer.py
# Compiled at: 2019-08-15 14:28:42
# Size of source mod 2**32: 11445 bytes
import binascii
...
from bitcoin.wallet import CBitcoinAddress, CBitcoinAddressError, P2SHBitcoinAddress
import bitcoin, ecdsa
...
class BitcoinSigner(Signer):
    ...
    def display_request(self) -> None:
        ...
        print_formatted_text(HTML('<i>INPUTS:</i>\n{}\n\n<i>OUTPUTS:</i>\n{}\n\n<i>FEE:</i> {} BTC\n'.format(self._formatted_input_groups(), self._formatted_outputs(), self._format_amount(self.fee))))
    ...
    def _formatted_input_groups(self) -> str:
        ...
        for input in self.inputs:
            ...
            bip32_paths[address] = input['bip32_path']

        lines = []
        for address in addresses:
            lines.append('  {}\t{} BTC\tSigning as {}'.format(address, self._format_amount(addresses[address]), bip32_paths[address]))
    ...
    def _format_output(self, output: Dict) -> str:
        return '  {}\t{} BTC'.format(output['address'], self._format_amount(output['amount']))
    ...
    def create_signature(self) -> None:
        ...
        for input_index, input in enumerate(self.inputs):
            ...
            signature_hashes.append(SignatureHash(parsed_redeem_scripts[redeem_script], tx, input_index, SIGHASH_ALL))
            if keys.get(bip32_path) is None:
                keys[bip32_path] = self.generate_child_keys(bip32_path)
                keys[bip32_path]['signing_key'] = ecdsa.SigningKey.from_string((bytes.fromhex(keys[bip32_path]['private_key'])),
        ...
        for input_index, input in enumerate(self.inputs):
            ...
            signatures.append(signing_key.sign_digest_deterministic(signature_hash,
              sha256,
