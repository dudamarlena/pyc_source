...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pybitcointools/main.py
# Compiled at: 2018-12-04 13:59:45
# Size of source mod 2**32: 17049 bytes
from .py2specials import *
from .py3specials import *
import binascii, hashlib, re, sys, os, base64, time, random, hmac
from pybitcointools.ripemd import *
P = 2 ** 256 - 4294967296 - 977
...
def decode_pubkey(pub, formt=None):
    ...
    if formt == 'bin':
        return (
         decode(pub[1:33], 256), decode(pub[33:65], 256))
    if formt == 'bin_compressed':
    ...
    if formt == 'hex':
        return (
         decode(pub[2:66], 16), decode(pub[66:130], 16))
    if formt == 'hex_compressed':
        return decode_pubkey(safe_from_hex(pub), 'bin_compressed')
    if formt == 'bin_electrum':
        return (
         decode(pub[:32], 256), decode(pub[32:64], 256))
    if formt == 'hex_electrum':
        return (
         decode(pub[:64], 16), decode(pub[64:128], 16))
    raise Exception('Invalid format!')
...
def multiply(pubkey, privkey):
    ...
    if not isinf(pubkey):
        if (pubkey[0] ** 3 + B - pubkey[1] * pubkey[1]) % P != 0:
...
def decompress(pubkey):
    ...
    if f == 'hex_compressed' or f == 'decimal':
...
def deterministic_generate_k(msghash, priv):
    v = b'\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01'
    k = b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
...
def ecdsa_raw_recover(msghash, vrs):
    ...
    if not (((xcubedaxb - y * y) % P != 0 or r) % N and s % N):
        return False
    z = hash_to_int(msghash)
...
def ecdsa_recover(msg, sig):
    ...
    if v >= 31:
        return encode_pubkey(Q, 'hex_compressed')
