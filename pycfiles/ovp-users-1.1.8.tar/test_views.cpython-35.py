...
# [GCC 8.4.0]
# Embedded file name: /home/arroyo/ovp/gpa-ovp/django-ovp-users/ovp_users/tests/test_views.py
# Compiled at: 2016-12-16 09:28:46
...
class RecoverPasswordViewSetTestCase(TestCase):
    ...
    def test_can_recover_password(self):
        ...
        data = {'email': 'test_can_recover@password.com', 
         'token': token, 
    ...
    def test_cant_recover_with_empty_password(self):
        ...
        email_content = mail.outbox[1].alternatives[0][0]
        token = re.search('[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}', email_content).group(0)
        data = {'email': 'test_cant_recover_empty@password.com', 
         'token': token, 
         'new_password': ''}
        client = APIClient()
        response = client.post(reverse('recover-password-list'), data, format='json')
    ...
    def test_cant_recover_invalid_token(self):
        ...
        data = {'email': 'test_cant_recover_invalid@password.com', 
         'token': 'invalid_token', 
    ...
    def test_cant_recover_invalid_password(self):
        ...
        email_content = mail.outbox[1].alternatives[0][0]
        token = re.search('[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}', email_content).group(0)
        data = {'email': 'test_cant_recover_invalid_pw@password.com', 
         'token': token, 
         'new_password': 'ab'}
        client = APIClient()
        response = client.post(reverse('recover-password-list'), data, format='json')
