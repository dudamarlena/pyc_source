...
# [GCC 8.4.0]
# Embedded file name: /home/arroyo/ovp/gpa-ovp/django-ovp-users/ovp_users/tests/test_views/profile.py
# Compiled at: 2017-05-15 11:01:22
# Size of source mod 2**32: 6236 bytes
...
class ProfileTestCase(TestCase):
    ...
    def test_skills_validation_on_update(self):
        ...
        user = self.test_can_create_user_with_profile()
        self.client.force_authenticate(User.objects.get(uuid=user.data['uuid']))
        data = {'profile': {'skills': [{'id': 999}, {'id': 998}]}}
    ...
    def test_causes_validation_on_update(self):
        ...
        user = self.test_can_create_user_with_profile()
        self.client.force_authenticate(User.objects.get(uuid=user.data['uuid']))
        data = {'profile': {'causes': [{'id': 999}, {'id': 998}]}}
        response = self.client.patch(reverse('user-current-user'), data, format='json')
        self.assertTrue(response.status_code == 400)
        self.assertTrue(response.data['profile']['causes'][0]['id'] == ["Cause with 'id' 999 does not exist."])
        self.assertTrue(response.data['profile']['causes'][1]['id'] == ["Cause with 'id' 998 does not exist."])
    ...
    def _test_can_update(self):
        data = {'profile': {'full_name': 'New name', 
                     'about': 'New about', 
                     'causes': [{'id': 3}, {'id': 4}], 
                     'skills': [{'id': 3}, {'id': 4}], 
                     'gender': 'female'}}
