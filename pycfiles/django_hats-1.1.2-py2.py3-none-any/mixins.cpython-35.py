...
# [GCC 8.4.0]
# Embedded file name: /Users/mikehearing/GIT/django-hats/django_hats/mixins.py
# Compiled at: 2017-11-19 12:09:04
...
class RoleRequiredMixin(PermissionRequiredMixin):
    ...
    def get_permission_required(self):
        if self.get_role_required() and self.permission_required is None:
            self.permission_required = ()
        return super(RoleRequiredMixin, self).get_permission_required()

    def get_role_required(self):
        if self.role_required is None:
            raise ImproperlyConfigured('{0} is missing `role_required`. Define {0}.role_required, or override {0}.get_role_required().'.format(self.__class__.__name__))
