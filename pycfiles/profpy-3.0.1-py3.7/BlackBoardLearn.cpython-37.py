...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/profpy/apis/BlackBoardLearn.py
# Compiled at: 2020-01-07 15:07:46
# Size of source mod 2**32: 24292 bytes
...
class BlackBoardLearn(Api):
    ...
    USERS = 'v1/users'
    USER = 'v1/users/{0}'
    _BlackBoardLearn__REQUEST_FUNCTIONS = {'GET':requests.get, 
     'POST':requests.post, 
     'PUT':requests.put, 
    ...
    def __init__(self, in_app_key, in_app_id, in_secret_key, in_url):
        super().__init__(in_public_key=in_app_key,
          in_private_key=in_secret_key,
          in_url=in_url)
        self.app_id = in_app_id
        self.token = self._BlackBoardLearn__get_oauth2_token()
        self._set_endpoints()
    ...
    def _set_args_mapping(self):
        ...
        """
        self.endpoint_to_args = {'GET':{self.COURSE: ['fields'], 
          self.COURSES: [
                         'offset',
                         'limit',
                         'courseId',
    ...
    def _hit_endpoint(self, valid_args, endpoint_name, get_one=False, request_type='GET', **kwargs):
        ...
        :return:
        """
        if not all((arg in valid_args for arg in kwargs.keys())):
            if valid_args or self.token.is_expired:
                self.token = self._BlackBoardLearn__get_oauth2_token()
        else:
            ...
            try:
                request_function = self._BlackBoardLearn__REQUEST_FUNCTIONS[r_type]
                if r_type == 'GET':
        ...
        if 300 >= status >= 200:
            ...
            msg = 'Invalid parameter supplied at BlackBoardLearn::_hit_endpoint(). Arguments provided: {0}. Valid arguments: {1}.'.format(bad_args, good_args)
    ...
    def __get_oauth2_token(self):
        ...
        headers = {'Content-Type':'application/x-www-form-urlencoded', 
         'Authorization':auth}
        raw_token = requests.post((self.url + 'v1/oauth2/token'),
    ...
    def get_course(self, course_id, use_blackboard_course_id=False):
        ...
        endpoint = endpoint.format(course_id if use_blackboard_course_id else 'courseId:{0}'.format(course_id))
    ...
    def get_courses(self, **kwargs):
        ...
        return (self._hit_endpoint)(
         (self.endpoint_to_args['GET'][endpoint]), endpoint, **kwargs)['results']
    ...
    def get_course_columns_attempts(self, course_id, column_id, use_blackboard_course_id=False, use_blackboard_column_id=False, **kwargs):
        """
        ...
        valid_args = self.endpoint_to_args['GET'][endpoint]
        course_param = course_id if use_blackboard_course_id else 'courseId:{0}'.format(course_id)
        col_param = column_id if use_blackboard_column_id else 'externalId:{0}'.format(column_id)
        return (self._hit_endpoint)(
         valid_args, (endpoint.format(course_param, col_param)), **kwargs)['results']

    def get_course_members(self, course_id, role=None, use_blackboard_course_id=False, **kwargs):
        """
        ...
        valid_args = self.endpoint_to_args['GET'][endpoint]
        endpoint = endpoint.format(course_id if use_blackboard_course_id else 'externalId:{0}'.format(course_id))
        data = (self._hit_endpoint)(valid_args, endpoint, **kwargs)['results']
        if role:
            return list(filter(lambda x: x['courseRoleId'] == role, data))
        return data

    def get_course_grade_columns_by_user(self, course_id, column_id, user_id, use_blackboard_course_id=False, use_blackboard_column_id=False, use_blackboard_user_id=False, **kwargs):
        """
        ...
        valid_args = self.endpoint_to_args['GET'][endpoint]
        course_param = course_id if use_blackboard_course_id else 'courseId:{0}'.format(course_id)
        column_param = column_id if use_blackboard_column_id else 'externalId:{0}'.format(column_id)
        user_param = user_id if use_blackboard_user_id else 'externalId:{0}'.format(user_id)
        endpoint = endpoint.format(course_param, column_param, user_param)
        return (self._hit_endpoint)(valid_args, endpoint, **kwargs)

    def get_course_group(self, course_id, group_id, use_blackboard_course_id=False, use_blackboard_group_id=False):
        """
        ...
        valid_args = self.endpoint_to_args['GET'][endpoint]
        course_param = course_id if use_blackboard_course_id else 'courseId:{0}'.format(course_id)
        group_param = group_id if use_blackboard_group_id else 'externalId:{0}'.format(group_id)
        return self._hit_endpoint(valid_args, endpoint.format(course_param, group_param))
    ...
    def get_user(self, user_id, use_blackboard_user_id=False):
        ...
        endpoint = self.USER
        valid_args = self.endpoint_to_args['GET'][endpoint]
        endpoint = endpoint.format(user_id if use_blackboard_user_id else 'externalId:{0}'.format(user_id))
        return self._hit_endpoint(valid_args, endpoint)
    ...
    def add_user_to_course(self, user_id, course_id, role='Student', use_blackboard_user_id=False, use_blackboard_course_id=False):
        """
        ...
        :return:
        """
        valid_roles = self._BlackBoardLearn__get_valid_roles()
 ...
 valid_args, endpoint, request_type='PUT', **dict(courseRoleId=role))
    ...
    def update_course_membership(self, user_id, course_id, in_json, use_blackboard_user_id=False, use_blackboard_course_id=False):
        """
 ...
 valid_roles, endpoint, request_type='PATCH', **in_json)
    ...
    def remove_user_from_course(self, user_id, course_id, use_blackboard_user_id=False, use_blackboard_course_id=False):
        """
        ...
        course_param = course_id if use_blackboard_course_id else 'courseId:{0}'.format(course_id)
        user_param = user_id if use_blackboard_user_id else 'externalId:{0}'.format(user_id)
        return self._hit_endpoint(valid_args,
          (endpoint.format(course_param, user_param)), request_type='DELETE')