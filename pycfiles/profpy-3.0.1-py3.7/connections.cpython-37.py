...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/profpy/db/general/connections.py
# Compiled at: 2020-01-07 15:07:46
# Size of source mod 2**32: 12857 bytes
import cx_Oracle, os, re, functools
from sqlalchemy import create_engine, MetaData
from sqlalchemy.orm import scoped_session, sessionmaker
short_form_regex = re.compile('^[a-zA-Z]+[a-zA-Z0-9_]*@[a-zA-Z_]+$')
...
class OracleConnectionHelper(object):
    ...
    def __init__(self, login, password):
        ...
        if re.match(short_form_regex, login):
            ...
            self._OracleConnectionHelper__username = username
            self._OracleConnectionHelper__password = password
            self._OracleConnectionHelper__dsn = dsn
            self._OracleConnectionHelper__engine_string = f"oracle://{username}:{password}@{dsn}"

    def get_cx_oracle_connection(self):
        """
        :return: A cx_Oracle connection object
        """
        return cx_Oracle.connect(user=(self._OracleConnectionHelper__username), password=(self._OracleConnectionHelper__password), dsn=(self._OracleConnectionHelper__dsn))

    def get_sql_alchemy_engine(self):
        """
        :return: A sqlalchemy engine object
        """
        return create_engine(self._OracleConnectionHelper__engine_string)
...
def with_cx_oracle_connection(login=os.environ.get('full_login'), password=os.environ.get('db_password'), auto_commit=False):
    """
...
def with_oracle_connection(login=os.environ.get('full_login'), password=os.environ.get('db_password'), auto_commit=False):
    """
    DEPRECATED
...
def with_sql_alchemy_oracle_session(login=os.environ.get('full_login'), password=os.environ.get('db_password'), scoped=False, auto_commit=False, bind=None):
    """
...
def with_sql_alchemy_oracle_connection(login=os.environ.get('full_login'), password=os.environ.get('db_password'), auto_commit=False, engine=None):
    """
...
def get_sql_alchemy_oracle_session(login=os.environ.get('full_login'), password=os.environ.get('db_password'), scoped=False, bind=None):
    """
