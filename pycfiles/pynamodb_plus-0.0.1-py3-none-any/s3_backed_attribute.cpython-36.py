...
# Embedded file name: /Users/sanhehu/Documents/GitHub/pynamodb_mate-project/pynamodb_mate/s3_backed_attribute.py
# Compiled at: 2020-03-04 16:19:23
# Size of source mod 2**32: 8372 bytes
...
class BaseS3BackedAttribute(object):
    ...
    def _put_binary_data(self, model_obj, data):
        ...
        s3_uri = self.s3_uri_getter_real(model_obj)
        bucket, key = parse_s3_uri(s3_uri)
        res = model_obj.get_s3_client().put_object(Bucket=bucket,
          Key=key,
          Body=data)
        return res
    ...
    def _read_binary_data(self, model_obj):
        ...
        s3_uri = self.s3_uri_getter_real(model_obj)
        bucket, key = parse_s3_uri(s3_uri)
        res = model_obj.get_s3_client().get_object(Bucket=bucket,
          Key=key)
...
class S3BackedMixin(object):
    ...
    def get_s3_client(cls):
        ...
        if cls._s3_client is None:
            ...
            cls._s3_client = pynamodb_connection.session.create_client('s3', pynamodb_connection.region)
        return cls._s3_client

    def atomic_save(self, condition=None, s3_backed_data=None):
        """
        ...
        try:
            ...
            del saved_data_list
            return res
        except Exception as dynamodb_save_error:
            for s3_backed_attr, data in saved_data_list:
                s3_backed_attr.delete_object(self)
    ...
    def atomic_update(self, actions=None, condition=None, s3_backed_data=None):
        """
    ...
    def atomic_delete(self, condition=None):
        """
