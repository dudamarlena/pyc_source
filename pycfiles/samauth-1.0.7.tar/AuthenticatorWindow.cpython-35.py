...
# [GCC 8.4.0]
# Embedded file name: /home/samer/Seafile/Source/Source/Authenticator/SamAuthenticator/AuthenticatorWindow.py
# Compiled at: 2018-04-01 13:07:27
# Size of source mod 2**32: 10305 bytes
from PyQt5.QtWidgets import QMainWindow, QWidget, QGridLayout, QLineEdit, QAction, qApp, QInputDialog, QMessageBox, QFileDialog
from PyQt5.QtCore import QSortFilterProxyModel, pyqtSlot, QSettings
from PyQt5.Qt import Qt, QIcon, QSizePolicy
import SamAuthenticator.KeysDataModel as model, SamAuthenticator.KeyDataView as dataview, SamAuthenticator.Authenticator as auth, SamAuthenticator.TrayIcon as tray, functools, os
...
class AuthenticatorGUI(QMainWindow):
    ...
    def decrypt_data_file(self):
        ...
        if not source_file[1]:
            ...
            if not dest_file[1]:
                return
            password_from_dialog = QInputDialog.getText(self, 'Input encryption password', 'Encryption password:', QLineEdit.Password, '')
            ok_pressed = password_from_dialog[1]
            return ok_pressed or None
        try:
    ...
    def save_data(self, data_file):
        pass_from_dialog_1 = QInputDialog.getText(self, 'Input encryption password', 'New encryption password:', QLineEdit.Password, '')
        ok_pressed_1 = pass_from_dialog_1[1]
        if not ok_pressed_1:
            return
        pass_from_dialog_2 = QInputDialog.getText(self, 'Repeat encryption password', 'Repeat encryption password:', QLineEdit.Password, '')
