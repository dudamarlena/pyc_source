...
# [GCC 8.4.0]
# Embedded file name: graphdash/routes.py
# Compiled at: 2019-04-29 08:38:09
...
import logging
from flask import Flask, render_template, abort, jsonify, request, send_from_directory, redirect, url_for, Response, flash
...
from .load import load_args, load_conf, load_data, load_data_raw, load_tags, load_themes, load_families, post_load, export_conf, export_families, sort_sons, sort_labels, sort_indexes, dump_data, show_conf, show_tags, show_themes, check_theme
...
if CONF['export_conf'] is not None:
    export_conf(CONF, CONF['export_conf'], exclude=('export_conf', 'export_families'))
...
app.template_filter('mdi')(md_iconvert)
...
@app.route('/map')
def get_map():
    return Response(dump_data(DATA, True) + '\n' + dump_data(DATA, False), mimetype='text/plain')
...
def search():
    ...
    results.update({'inner_value': inner_value, 
...
def build_query(value):
    q = {'keywords': {'exclude': set(), 'include': set()}, 'freetext': {'exclude': set(), 'include': set()}}
...
def search_results(query):
    ...
    for family_tuple, node in DATA.iter_all_nodes():
        ...
        family_alias_low = ('/').join(get(p)['alias'] for p in up_paths(family_tuple, include_root=False)).lower()
        for graph_data in node.data['graphs']:
            ...
            t = (
             family_path_low, family_alias_low, graph_title_low, graph_keywords)
            if graph_keywords.issuperset(query.keywords.include) and graph_keywords.isdisjoint(query.keywords.exclude) and all(is_in_any(w, t) for w in query.freetext.include) and all(is_not_in(w, t) for w in query.freetext.exclude):
                nb_matches += 1
                if CONF['headless']:
                    continue
                ...
                matches[family_path].append({'title': md_iconvert(graph_data['title']), 
                   'text': graph_data['text'], 
                   'labels': sort_labels(graph_data['labels']), 
                   'id': graph_data['id']})

    return {'matches': matches, 'families': sorted(matches, key=lambda s: s.lower()), 
       'aliases': aliases, 
