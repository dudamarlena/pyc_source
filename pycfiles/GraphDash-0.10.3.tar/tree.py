...
# [GCC 8.4.0]
# Embedded file name: graphdash/struct/tree.py
...
class Tree(object):
    ...
    def __prettify(self, decorate, with_data, sort_sons, index, name, indent):
        ...
        """
        res = [
         ('{0}({1}) {2}').format(indent, index, decorate((name, self)))]
        increment = '  '
        indent += increment
        if with_data is not None:
            for i, thing in enumerate(with_data(self.data), start=1):
                res.append(('{0}|{1:2d}| {2}').format(indent + increment, i, thing))
    ...
    def prettify(self, decorate=lambda x: '', with_data=None, sort_sons=lambda x: x):
        return self.__prettify(decorate=decorate, with_data=with_data, sort_sons=sort_sons, index=1, name='', indent='')