...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/zope/dependencytool/finddeps.py
...
"""
import errno, getopt, os, re, sys, zope
from zope.dependencytool.dependency import Dependency
from zope.dependencytool import importfinder
ZOPESRC = os.path.dirname(os.path.dirname(os.path.dirname(zope.dependencytool.__file__)))
ZOPESRCPREFIX = os.path.join(ZOPESRC, '')
pythonfile = re.compile('[a-zA-Z_][a-zA-Z0-9_]*\\.py$')
zcmlfile = re.compile('[a-zA-Z][a-zA-Z0-9_]*\\.zcml$')
dottedName = re.compile('"[a-zA-Z\\.][a-zA-Z0-9_\\.]*"')
    ...
    if os.path.isdir(path):
        ...
        for file in os.listdir(path):
            ...
            if pythonfile.match(file):
                ...
                if 'PUBLICATION.cfg' not in filenames and 'SETUP.cfg' not in filenames and 'DEPENDENCIES.cfg' not in filenames and '__init__.py' in filenames:
...
def getAllCleanedDependencies(path, zcml=False, deps=[], paths=[], packages=False):
    """Return a list of all cleaned dependencies in a path."""
    if path.endswith('src/zope/') or path.endswith('src/zope/app/') or path.endswith('src\\zope\\') or path.endswith('src\\zope\\app\\'):
        return deps
...
def parse_args(argv):
    try:
        (opts, args) = getopt.getopt(argv[1:], 'd:m:pahlz', [
    ...
    if options.path is None:
        usage(1, 'The module must be specified either by path, dotted name or ZCML file.')
    return options
...
def main(argv=None):
    try:
        ...
        showDependencies(options.path, options.zcml, options.long, options.all, options.packages)
    except IOError, e:
