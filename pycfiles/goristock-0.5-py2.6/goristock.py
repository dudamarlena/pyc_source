...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/grs/goristock.py
...
import csv, logging, math, random, re, urllib2
...
class goristock(object):
  ...
  """
    ...
    def covstr(self, s):
        u""" convert string to int or float.
        轉換數字為浮動小數格式
    """
        try:
            ret = int(s)
    ...
    def ckinv(self, oo):
        u""" check the value is date or not
        檢查是否為日期格式
    """
    ...
    def high_or_low(self, one, two, rev=0):
        u""" Return ↑↓- for high, low or equal.
        回傳漲跌標示
        rev = 0
          回傳 ↑↓-
        rev = 1
    ...
    """
        ...
        if now >= datetime(now.year, now.month, now.day, 14, 45):
            ...
            addday = 0
        endtime = datetime(now.year, now.month, now.day, 14, 0) + timedelta(days=addday)
    ...
    def MA(self, days):
        u""" Price Moving Average with days.
        return float value.
    ...
    def MAC(self, days, rev=0):
        u""" Comparing yesterday price is high, low or equal.
        return ↑,↓ or -
    ...
    def MAVOL(self, days):
        u""" Volume Moving Average with days.
        return float value.
    ...
    def MACVOL(self, days, rev=0):
        u""" Comparing yesterday volume is high, low or equal.
        return ↑,↓ or -
    ...
    @property
    def VOLMAX3(self):
        u""" Volume is the max in last 3 days.
        三日內最大成交量
    """
    ...
    def MAO(self, day1, day2, rev=0):
        u""" This is MAO(Moving Average Oscillator), not BIAS.
    ...
    """
        ...
        return [
         cum, self.high_or_low(day1MAs[(-1)] - day2MAs[(-1)], day1MAs[(-2)] - day2MAs[(-2)], rev)]
    ...
    def Task_display(self):
        u""" For Task overall stock display
        顯示資訊樣式之一，兩行資訊。
    """
    ...
    def Cmd_display(self):
        ...
        一行顯示資訊，用於終端機顯示樣式。
    """
        re = '%(stock_no)s %(stock_name)s %(stock_date)s %(stock_price)s %(stock_range)s %(stock_range_per).2f%% %(RABC)s %(stock_vol)s' % {'stock_name': unicode(self.stock_name), 
           'stock_no': unicode(self.stock_no), 
           'stock_date': unicode(self.data_date[(-1)]), 
           'stock_price': unicode(self.raw_data[(-1)]), 
           'stock_range': unicode(self.stock_range[(-1)]), 
    ...
    """
        ...
        re = 'http://%(rand)s.chart.apis.google.com/chart?chs=%(sw)sx%(sh)s&cht=lc&chd=t1:0,0,0|0,%(h)s0|0,%(c)s0|0,%(o)s0|0,%(l)s0&chm=F,,1,1:-1,%(candle)s&chxt=y,x&chds=%(min)s,%(max)s&chxr=0,%(min)s,%(max)s,%(range)s&chg=20,%(chg)s&chtt=%(chtt)s&chxl=1:||%(chxl)s' % {'h': sth, 
           'c': stc, 
           'o': sto, 
           'l': stl, 
           'min': stmin, 
...
def Rt_display(stock_no):
    u""" For real time stock display
      即時盤用，顯示目前查詢各股的股價資訊。
  """
    a = twsk(stock_no).real
    if a:
        re = '{%(time)s} %(stock_no)s %(c)s %(range)+.2f(%(pp)+.2f%%) %(value)s' % {'stock_no': stock_no, 
           'time': a['time'], 
           'c': a['c'], 
