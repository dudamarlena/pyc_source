...
# Embedded file name: build/bdist.linux-x86_64/egg/cgecore/alignment.py
# Compiled at: 2020-04-01 11:09:31
# Size of source mod 2**32: 23387 bytes
__doc__ = ' This module contains core functions and classes related to alignment. '
...
def Blaster(inputfile, databases, db_path, out_path='.', min_cov=0.6, threshold=0.9, blast='blastn', cut_off=True):
   ...
   """
    ...
    return (
     results, gene_align_query, gene_align_homo, gene_align_sbjct)
...
def compare_results(save, best_hsp, tmp_results, tmp_gene_split):
    ...
    for hit in list(tmp_results):
        ...
        if new_score > old_score:
            ...
            save = 0
            if hit_id != hit and new_db_hit in tmp_gene_split and hit_id in tmp_gene_split[new_db_hit]:
                ...
                if best_hsp['perc_ident'] > hit_data['perc_ident']:
                    ...
                    if best_hsp['perc_ident'] == hit_data['perc_ident']:
                        ...
                        if new_db_hit in tmp_gene_split and hit_id in tmp_gene_split[new_db_hit]:
                            del tmp_gene_split[new_db_hit][hit_id]
                        break
        if max(old_end_query, new_end_query) - min(old_start_query, new_start_query) <= old_end_query - old_start_query + (new_end_query - new_start_query):
            if new_score > old_score:
                remove_old = 1
                if new_db_hit in tmp_gene_split and hit_id not in tmp_gene_split[new_db_hit]:
                    tmp_gene_split[new_db_hit][hit_id] = 1
