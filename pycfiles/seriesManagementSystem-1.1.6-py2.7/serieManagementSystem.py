...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/SMS/serieManagementSystem.py
# Compiled at: 2015-10-09 08:35:34
import sys, os, getopt, shutil, logging.config
...
class SMS():
    ...
    def __init__(self):
        ...
        self.__log.debug('\x1b[1;33m' + self.__cwd + '\x1b[0m')
        ...
        self.__latex_error_messages = ('Type X to quit or <RETURN> to proceed', '! Undefined control sequence.',
                                       '? ', 'Type  H <return>  for immediate help.',
                                       'Enter file name: ', 'or enter new name. (Default extension: sty)')
        self.__latex_recompile_messages = ('recompile ', 're-run ', 'undefined references.',
                                           'rerun ')
        self.__serie = -1
        self.__exercise = -1
    ...
    def __doCreateSerie(self, _titles, _numbers, _outputDir, filename=None):
        ...
        for number in _numbers:
            serie.write('\\setcounter{section}{' + number + '}\n')
            serie.write('\\addtocounter{section}{-1}' + '\n')
            serie.write('\\renewcommand{\\includepath}{\\compilationpath/' + self.__exoDirName + '/ex' + number + '/latex/ressources}' + '\n')
    ...
    def __doCreateSolution(self, _titles, _numbers, _outputDir, filename=None):
        ...
        for number in _numbers:
            solution.write('\\setcounter{section}{' + number + '}\n')
            solution.write('\\addtocounter{section}{-1}' + '\n')
            solution.write('\\renewcommand{\\includepath}{\\compilationpath/' + self.__exoDirName + '/ex' + number + '/latex/ressources}' + '\n')
    ...
    def __makeWorkBookTitlePage(self, _outputDir):
        ...
        for config in seriesConfigFiles:
            ...
            wbtitle.write("\\textsf{ \\textbf{S{\\'e}rie " + serienumber + '}} \\dotfill' + '\n')
            for number in numbers.split(','):
                wbtitle.write(number + '\n')

            wbtitle.write('\\begin{itemize}\n')
            for title in titles.split(','):
                wbtitle.write('\\item ' + title + '\n')

            wbtitle.write('\\end{itemize}\n')
    ...
    def makeCatalogue(self):
        ...
        os.mkdir('Catalogue')
        catalogue = open(afile, 'w')
        latex = LaTeX(self.__serie)
        latex.createHeader(catalogue, [])
        catalogue.write('\\renewcommand{\\exercice}[1]{\\subsection*{Problem: #1}}' + '\n')
        catalogue.write('\\renewcommand{\\solution}[1]{\\subsection*{Solution: #1}}' + '\n')
        catalogue.write('\\renewcommand{\\question}[1]{\\subsubsection*{#1}}' + '\n')
        catalogue.write('\n')
        catalogue.write('\\makeatletter\n')
        catalogue.write('\\renewcommand{\\section}{\\@startsection{section}{3}{2pt}{12pt}{10pt}{\\center \\huge \\sffamily \\bfseries}}' + '\n')
        catalogue.write('\\renewcommand{\\thesection}{(\\roman{section})}' + '\n')
        catalogue.write('\\renewcommand{\\thesubsection}{(\\roman{subsection})}' + '\n')
        exos = os.listdir(self.__exoDirName)
        exos = Utils.natsort(exos)
        for exo in exos:
            if exo.find('ex') != -1:
                number = exo[2:]
                catalogue.write('\\section*{Exercise ' + number + '}' + '\n')
                catalogue.write('\\renewcommand{\\includepath}{\\compilationpath/' + self.__exoDirName + '/ex' + number + '/latex/resources}' + '\n')
    ...
    def createNewLecture(self, lecturename):
        ...
        copy_tree(resource_filename(__name__, 'data'), lecturename)

    def usage(self):
        print 'Usage:'
        print os.path.basename(sys.argv[0]) + ' <command> [option]'
        print '\x1b[1;33mWhere option is one of:\x1b[0m'
        ...
        print '\x1b[1;33mWhere command is one of:\x1b[0m'
