...
# [GCC 8.4.0]
# Embedded file name: /home/user/feldbauer/PycharmProjects/hubness/skhubness/analysis/tests/test_estimation.py
# Compiled at: 2019-09-12 09:10:53
# Size of source mod 2**32: 10208 bytes
...
def test_all_params_none():
    ...
    hub = Hubness(k=None, return_value=None, hub_size=None, metric=None, store_k_neighbors=None,
      store_k_occurrence=None,
...
def test_shuffle_equal(verbose):
    ...
    skew_shuffle, skew_no_shuffle = [Hubness(metric='precomputed', shuffle_equal=v, verbose=verbose).fit(dist).score() for v in (True,
                                                                                                                                 False)]
...
def test_sparse_equal_dense(verbose, shuffle_equal):
    ...
    hub = Hubness(metric='precomputed', shuffle_equal=shuffle_equal,
      verbose=verbose)
    hub.fit(dist_dense)
...
def test_atkinson():
    ...
    k_occ = hub.score()
    atkinson_0999 = hub._calc_atkinson_index(k_occ, eps=0.999)
...
def test_hubness_return_values_are_self_consistent(n_samples, n_features, k, seed):
    ...
    np.random.seed(seed)
    vectors = 99.0 * (np.random.rand(n_samples, n_features) - 0.5)
    k = 10
...
def test_hubness_against_distance(has_self_distances):
    ...
    verbose = 1
    hub = Hubness(k=10, metric='precomputed', store_k_occurrence=True,
    ...
    hub = Hubness(k=10, metric='euclidean', store_k_neighbors=True,
    ...
    np.testing.assert_allclose(skew_d, skew_v, atol=1e-07)
...
def test_hubness_independent_on_data_set_size(hubness_measure):
    ...
    thousands = 3
    n_objects = thousands * 1000
    rng = np.random.RandomState(1247)
    X = rng.rand(n_objects, 128)
    N_SAMPLES_LIST = np.arange(1, thousands + 1) * 1000
    ...
    for i, n_samples in enumerate(N_SAMPLES_LIST):
        ...
        if hubness_measure == 'k_skewness':
            ...
            if hubness_measure == 'k_skewness_truncnorm':
                value[i] = hub.k_skewness_truncnorm
            else:
                if hubness_measure == 'robinhood':
                    value[i] = hub.robinhood_index
                else:
                    if hubness_measure == 'gini':
                        value[i] = hub.gini_index
                    else:
                        if hubness_measure == 'atkinson':
        ...
        if i > 0:
            if hubness_measure == 'k_skewness':
                ...
                np.testing.assert_allclose((value[i]),
                  (value[(i - 1)]), rtol=0.2, err_msg=f"Hubness measure is too dependent on data set size with S({N_SAMPLES_LIST[i]}) = x and S({N_SAMPLES_LIST[(i - 1)]}) = y.")

    if hubness_measure == 'k_skewness':
        with np.testing.assert_raises(AssertionError):
            np.testing.assert_allclose((value[(-1)]), (value[0]), rtol=0.1)
    else:
        np.testing.assert_allclose((value[(-1)]), (value[0]), rtol=0.2)