...
# [GCC 8.4.0]
# Embedded file name: /home/user/feldbauer/PycharmProjects/hubness/skhubness/reduction/tests/test_hubness_reduction.py
...
def test_neighbors_dexter(hubness_param, metric):
    ...
    graph = kneighbors_graph(X, n_neighbors=10, metric=metric, hubness=hubness,
      hubness_params=param)
    hub = Hubness(k=10, metric='precomputed')
    hub.fit(graph)
    k_skew_hr = hub.score(has_self_distances=True)
    assert k_skew_hr < k_skew_orig * 8 / 10, f"k-occurrence skewness was not reduced by at least 20% for dexter with {hubness}"
