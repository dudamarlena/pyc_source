...
# [GCC 8.4.0]
# Embedded file name: /home/user/feldbauer/PycharmProjects/hubness/skhubness/reduction/tests/test_local_scaling.py
...
def test_fit_sorted(method, verbose):
    ...
    nd_sorted, ni_sorted = ls.fit(neigh_dist, neigh_ind, X, assume_sorted=True).transform(neigh_dist,
      neigh_ind, X, assume_sorted=True)
    nd_unsort, ni_unsort = ls.fit(neigh_dist, neigh_ind, X, assume_sorted=False).transform(neigh_dist,
      neigh_ind, X, assume_sorted=False)
