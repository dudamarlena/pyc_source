...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/py_disc/disc.py
# Compiled at: 2017-02-05 11:53:49
...
def naive_D(eeg_data, subjects, dist):
    ...
    for s in np.unique(subjects):
        ...
        for t in np.arange(num):
            for tt in np.setdiff1d(np.arange(num), [t]):
                p = naive_D_partial(eeg_data, subjects, s, t, tt, dist)
                partials.append(p)
                sra.append(s)
                tra.append(t)
                ttra.append(tt)
    ...
    info = pd.DataFrame(data=np.array([sra, tra, ttra, mra]).T, columns=[
     'Subject',
     'Trial',
     'Trial_Prime',
...
def D(eeg_data, subjects, dist):
    ...
    for s in np.unique(subjects):
        ...
        for t in np.arange(num):
            for tt in np.setdiff1d(np.arange(num), [t]):
                p = D_partial(D, subjects, s, t, tt, dist)
                partials.append(p)
                sra.append(s)
                tra.append(t)
                ttra.append(tt)
