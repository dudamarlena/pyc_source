...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/lbdrabbit-project/lbdrabbit/lbd_func_config/lbd_func_config.py
# Compiled at: 2019-10-06 23:55:31
# Size of source mod 2**32: 50854 bytes
import typing, attr, string
...
class LbdFuncConfig(BaseConfig):
    ...
    param_env_name = attr.ib(default=REQUIRED)
    lbd_func_yes = attr.ib(default=REQUIRED)
    lbd_func_name = attr.ib(default=REQUIRED)
    lbd_func_description = attr.ib(default=NOTHING)
    lbd_func_memory_size = attr.ib(default=NOTHING)
    lbd_func_iam_role = attr.ib(default=REQUIRED)
    lbd_func_timeout = attr.ib(default=NOTHING)
    lbd_func_runtime = attr.ib(default=REQUIRED)
    lbd_func_code = attr.ib(default=REQUIRED)
    lbd_func_layers = attr.ib(default=NOTHING)
    lbd_func_reserved_concurrency = attr.ib(default=NOTHING)
    lbd_func_environment_vars = attr.ib(default=NOTHING)
    lbd_func_kms_key_arn = attr.ib(default=NOTHING)
    lbd_func_vpc_config = attr.ib(default=NOTHING)
    lbd_func_dead_letter_config = attr.ib(default=NOTHING)
    lbd_func_tracing_config = attr.ib(default=NOTHING)
    _lbd_func_aws_object_cache = attr.ib(default=NOTHING)
    lbd_func_metadata = attr.ib(default=NOTHING)
    lbd_func_tags = attr.ib(default=NOTHING)
    apigw_resource_yes = attr.ib(default=NOTHING)
    apigw_restapi = attr.ib(default=NOTHING)
    _apigw_resource_aws_object_cache = attr.ib(default=NOTHING)
    ...
    apigw_method_yes = attr.ib(default=NOTHING)
    apigw_method_int_type = attr.ib(default=NOTHING)
    _apigw_method_aws_object_cache = attr.ib(default=NOTHING)
    _apigw_method_lbd_permission_aws_object_cache = attr.ib(default=NOTHING)
    apigw_method_int_passthrough_behavior = attr.ib(default=NOTHING)
    apigw_method_int_timeout_in_milli = attr.ib(default=NOTHING)
    apigw_method_authorization_type = attr.ib(default=NOTHING)
    apigw_method_authorizer = attr.ib(default=NOTHING)
    apigw_method_enable_cors_yes = attr.ib(default=NOTHING)
    apigw_method_enable_cors_access_control_allow_origin = attr.ib(default=NOTHING)
    apigw_method_enable_cors_access_control_allow_headers = attr.ib(default=NOTHING)
    _apigw_method_options_for_cors_aws_object_cache = attr.ib(default=NOTHING)
    apigw_authorizer_yes = attr.ib(default=NOTHING)
    apigw_authorizer_name = attr.ib(default=NOTHING)
    apigw_authorizer_token_type_header_field = attr.ib(default=NOTHING)
    _apigw_authorizer_aws_object_cache = attr.ib(default=NOTHING)
    _apigw_authorizer_lbd_permission_aws_object_cache = attr.ib(default=NOTHING)
    scheduled_job_yes = attr.ib(default=NOTHING)
    scheduled_job_expression = attr.ib(default=NOTHING)
    _scheduled_job_event_rule_aws_objects_cache = attr.ib(default=NOTHING)
    _scheduled_job_event_lbd_permission_aws_objects_cache = attr.ib(default=NOTHING)
    ...
    s3_event_bucket_basename = attr.ib(default=NOTHING)
    ...
    def s3_event_bucket_name_for_cf(self) -> typing.Union[(str, Sub)]:
        ...
        **中文文档**
        ...
        if self.s3_event_bucket_basename is NOTHING:
            raise TypeError
        if isinstance(self.s3_event_bucket_basename, str):
            return Sub('${AccountId}-${EnvName}-${BucketBasename}', {'AccountId':Ref(AWS_ACCOUNT_ID), 
             'EnvName':Ref(self.param_env_name), 
             'BucketBasename':self.s3_event_bucket_basename})
        raise TypeError

    s3_event_lbd_config_list = attr.ib(default=NOTHING)
    ...
    def s3_notification_configuration_aws_property(self):
        ...
        if self.s3_event_lbd_config_list is not NOTHING:
            if isinstance(self.s3_event_lbd_config_list, list):
                s3_notification_configuration.LambdaConfigurations = [s3.LambdaConfigurations(Event=(s3_event_lbd_config.event), Function=(GetAtt(self.lbd_func_aws_object, 'Arn')), Filter=(s3_event_lbd_config.filter)) for s3_event_lbd_config in self.s3_event_lbd_config_list]
    ...
    def s3_event_bucket_aws_object_pre_check(self):
        if self.s3_event_lbd_config_list is NOTHING:
            raise ValueError('{}.s3_event_lbd_config_list has to be a list of LbdFuncConfig.S3EventLambdaConfig object'.format(self.identifier))
        if isinstance(self.s3_event_lbd_config_list, list):
            if len(self.s3_event_lbd_config_list) == 0:
                raise ValueError("{}.s3_event_lbd_config_list can't be a empty list!".format(self.identifier))

    def s3_event_bucket_aws_object_ready(self) -> bool:
        return self._ready_checker_shortener('s3_event_bucket')

    _s3_event_bucket_aws_object_cache = attr.ib(default=NOTHING)

    @property
    def s3_event_bucket_aws_object(self) -> s3.Bucket:
        if self._s3_event_bucket_aws_object_cache is NOTHING:
            s3_bucket = s3.Bucket((self.s3_event_bucket_logic_id),
    ...
    _s3_event_bucket_lbd_permission_aws_object_cache = attr.ib(default=NOTHING)
    ...
    def s3_event_bucket_lbd_permission_aws_object(self) -> awslambda.Permission:
        if self._s3_event_bucket_lbd_permission_aws_object_cache is NOTHING:
            ...
            s3_event_bucket_lbd_permission = awslambda.Permission(title=s3_event_bucket_lbd_permission_logic_id,
    ...
    _default = dict(lbd_func_yes=True,
      memory_size=128,
      timeout=3,
    ...
    def identifier(self) -> str:
        try:
            return '{}.{}.{}'.format(self._py_module.__name__, self._py_function.__name__, DEFAULT_LBD_FUNC_CONFIG_FIELD)
        except:
            return '{}.{}'.format(self._py_module.__name__, DEFAULT_LBD_FUNC_CONFIG_FIELD)
    ...
    def lbd_func_logic_id(self) -> str:
        return 'LbdFunc{}'.format(camelcase(self.rel_module_name.replace('.', '-')) + camelcase(self._py_function.__name__))

    @property
    def lbd_func_iam_role_arn(self):
        if isinstance(self.lbd_func_iam_role, iam.Role):
            return GetAtt(self.lbd_func_iam_role, 'Arn')
        else:
            if isinstance(self.lbd_func_iam_role, (
             Ref, GetAtt)):
                return self.lbd_func_iam_role
            if isinstance(self.lbd_func_iam_role, Parameter):
                return Ref(self.lbd_func_iam_role)
            if isinstance(self.lbd_func_iam_role, str):
                return self.lbd_func_iam_role
        raise TypeError('{}.lbd_func_iam_role has to be one of troposphere_mate.iam.Role, Ref of a Parameter, GetAtt of a troposphere_mate.iam.Role, a Parameter represent a iam role ARN, a string represent a iam role ARN'.format(self.identifier))
    ...
    def lbd_func_aws_object_pre_check(self):
        if callable(self._py_function):
            ...
            if self.lbd_func_code is NOTHING:
                raise ValueError('{}.lbd_func_code is not valid!'.format(self.identifier))
            if self.lbd_func_runtime is NOTHING:
                raise ValueError('{}.lbd_func_runtime is not valid!'.format(self.identifier))
        else:
            raise TypeError('{}.{} is not a valid function'.format(self._py_module.__name__, self._py_function))
    ...
    def lbd_func_aws_object(self) -> awslambda.Function:
        if self._lbd_func_aws_object_cache is NOTHING:
            lbd_func = awslambda.Function((self.lbd_func_logic_id),
    ...
    def apigw_resource_logic_id(self) -> str:
        ...
        with a Prefix.
        """
        return 'ApigwResource{}'.format(camelcase(self.rel_module_name.replace('.', '-')))

    @property
    def apigw_resource_parent_id(self) -> typing.Union[(Ref, GetAtt)]:
        if '.' not in self.rel_module_name:
            if bool(self.rel_module_name):
    ...
    def apigw_resource_full_path(self) -> str:
        ...
        ``rest/users``
        """
        return '/'.join([slugify(fname) for fname in self.rel_module_name.split('.')])
        ...
        **中文文档**
        ...
        if not self.rel_module_name:
            ...
            if self._py_function is not NOTHING:
                raise ValueError('to create a apigateway.Resource, the config should not bound with a python function!')
            if self.apigw_restapi is NOTHING:
                raise ValueError('to create a apigateway.Resource, LbdFuncConfig.apigw_restapi has to be specified')
    ...
    def apigw_resource_aws_object(self) -> apigateway.Resource:
        if self._apigw_resource_aws_object_cache is NOTHING:
            apigw_resource = apigateway.Resource((self.apigw_resource_logic_id),
    ...
    def apigw_method_logic_id(self) -> str:
        return 'ApigwMethod{}'.format(camelcase(self.rel_module_name.replace('.', '-')) + camelcase(self._py_function.__name__))
    ...
    def check_apigw_method_authorization_type(self):
        ...
        if self.apigw_method_authorization_type.upper() not in allowed_values:
            raise ValueError('{}.apigw_method_authorization_type can only be one of {}'.format(self.identifier, allowed_values))

    def check_apigw_method_int_type(self):
        if self.apigw_method_int_type not in self.ApiMethodIntType.Values():
            raise ValueError('{}.apigw_method_int_type can only be one of {}'.format(self.identifier, self.ApiMethodIntType.Values()))
    ...
    def apigw_method_http_method(self):
        if self.apigw_method_int_type == self.ApiMethodIntType.rest:
            ...
            if self.apigw_method_int_type == self.ApiMethodIntType.rpc:
                return 'POST'
    ...
    def apigw_method_authorizer_id_for_cf(self):
        ...
        if self.apigw_method_authorizer is NOTHING:
            ...
            if isinstance(self.apigw_method_authorizer, (Ref, GetAtt, ImportValue)):
                return self.apigw_method_authorizer
            if isinstance(self.apigw_method_authorizer, str):
                if len(self.apigw_method_authorizer):
                    if len(set(self.apigw_method_authorizer).difference(set(string.ascii_lowercase + string.digits))):
                        return self.apigw_method_authorizer
                return Ref(self.apigw_method_authorizer)
            else:
                raise TypeError
    ...
    def apigw_method_aws_object_pre_check(self):
        ...
        **中文文档**
        ...
        """
        if self._py_function is NOTHING:
            raise ValueError('to create a apigateway.Method, the config must be bound with a python function!')
        if self.apigw_restapi is NOTHING:
            raise ValueError('to create a apigateway.Resource, LbdFuncConfig.apigw_restapi has to be specified')
    ...
    def apigw_method_aws_object(self) -> apigateway.Method:
        if self._apigw_method_aws_object_cache is NOTHING:
            depends_on = [self.apigw_resource_aws_object,
             self.lbd_func_aws_object]
            request_template = {'application/json': "$input.json('$')"}
            if self.apigw_method_int_type == self.ApiMethodIntType.html:
                integration_response_200 = apigateway.IntegrationResponse(StatusCode='200',
                  ResponseParameters={'method.response.header.Access-Control-Allow-Origin':"'*'", 
                 'method.response.header.Content-Type':"'text/html'"},
                  ResponseTemplates={'text/html': "$input.path('$')"})
                method_response_200 = apigateway.MethodResponse(StatusCode='200',
            ...
            integration = apigateway.Integration(Type='AWS',
              IntegrationHttpMethod='POST',
              Uri=(Sub('arn:aws:apigateway:${Region}:lambda:path/2015-03-31/functions/${LambdaArn}/invocations', {'Region':{'Ref': 'AWS::Region'}, 
            ...
            if self.apigw_method_enable_cors_yes is True:
                for integration_response in integration.IntegrationResponses:
                    integration_response.ResponseParameters['method.response.header.Access-Control-Allow-Origin'] = "'*'"
            ...
            apigw_method = apigateway.Method(title=(self.apigw_method_logic_id),
              RestApiId=(Ref(self.apigw_restapi)),
    ...
    def apigw_method_lbd_permission_aws_object(self) -> awslambda.Permission:
        if self._apigw_method_lbd_permission_aws_object_cache is NOTHING:
            ...
            apigw_method_lbd_permission = awslambda.Permission(title=apigw_method_lbd_permission_logic_id,
              Action='lambda:InvokeFunction',
              FunctionName=(GetAtt(self.lbd_func_aws_object, 'Arn')),
              Principal='apigateway.amazonaws.com',
              SourceArn=(Sub('arn:aws:execute-api:${Region}:${AccountId}:${RestApiId}/*/%s/%s' % (
             self.apigw_method_http_method,
             self.apigw_resource_full_path), {'Region':{'Ref': 'AWS::Region'}, 
        ...
        **中文文档**
        ...
        if self.apigw_method_authorizer is not NOTHING:
            if self.apigw_authorizer_token_type_header_field is not NOTHING:
                access_control_allow_headers = access_control_allow_headers + ',{}'.format(self.apigw_authorizer_token_type_header_field)
        response_parameters = {'method.response.header.Access-Control-Allow-Origin':"'*'",  'method.response.header.Access-Control-Allow-Methods':"'OPTIONS,GET,POST'", 
         'method.response.header.Access-Control-Allow-Headers':"'{}'".format(access_control_allow_headers)}
        response_templates = {'application/json': ''}
        integration = apigateway.Integration(Type='MOCK',
          RequestTemplates=request_template,
          IntegrationResponses=[
         apigateway.IntegrationResponse(StatusCode='200',
           ContentHandling='CONVERT_TO_TEXT',
        ...
        if self.apigw_method_int_timeout_in_milli is not NOTHING:
            integration.TimeoutInMillis = self.apigw_method_int_timeout_in_milli
        method_responses = [
         apigateway.MethodResponse(StatusCode='200',
           ResponseModels={'application/json': 'Empty'},
           ResponseParameters={'method.response.header.Access-Control-Allow-Origin':False, 
          'method.response.header.Access-Control-Allow-Methods':False, 
          'method.response.header.Access-Control-Allow-Headers':False})]
        apigw_method = apigateway.Method(title=('ApigwMethod{}Options'.format(camelcase(self.rel_module_name.replace('.', '-')))),
          RestApiId=(Ref(self.apigw_restapi)),
          ResourceId=(Ref(self.apigw_resource_aws_object)),
    ...
    def get_authorizer_id(cls, rel_module_name):
        return 'ApigwAuthorizer{}'.format(camelcase(rel_module_name.replace('.', '-')))
    ...
    def apigw_authorizer_aws_object_pre_check(self):
        ...
        **中文文档**
        ...
        """
        if self._py_function is NOTHING:
            raise ValueError('to create a apigateway.Authorizer, the config must be bound with a python function!')
        if self.apigw_restapi is NOTHING:
            raise ValueError('to create a apigateway.Resource, LbdFuncConfig.apigw_restapi has to be specified')
    ...
    def apigw_authorizer_aws_object(self) -> apigateway.Authorizer:
        if self._apigw_authorizer_aws_object_cache is NOTHING:
            ...
            if len(set(apigw_authorizer_name).difference(set(string.ascii_letters + string.digits))):
                raise ValueError('{}.apigw_authorizer_name can only have letter and digits'.format(self.identifier))
            apigw_authorizer = apigateway.Authorizer(title=(self.apigw_authorizer_logic_id),
              ...
              AuthorizerUri=(Sub('arn:aws:apigateway:${Region}:lambda:path/2015-03-31/functions/${AuthorizerFunctionArn}/invocations', {'Region':{'Ref': 'AWS::Region'}, 
    ...
    def apigw_authorizer_lbd_permission_aws_object(self) -> awslambda.Permission:
        if self._apigw_authorizer_lbd_permission_aws_object_cache is NOTHING:
            ...
            apigw_authorizer_lbd_permission = awslambda.Permission(title=apigw_authorizer_lbd_permission_logic_id,
              Action='lambda:InvokeFunction',
              FunctionName=(GetAtt(self.lbd_func_aws_object, 'Arn')),
              Principal='apigateway.amazonaws.com',
              SourceArn=(Sub('arn:aws:execute-api:${Region}:${AccountId}:${RestApiId}/authorizers/${AuthorizerId}', {'Region':{'Ref': 'AWS::Region'}, 
             'AccountId':{'Ref': 'AWS::AccountId'}, 
             'RestApiId':Ref(self.apigw_restapi), 
    ...
    def scheduled_job_expression_list(self):
        if self.scheduled_job_expression is NOTHING:
            ...
            if isinstance(self.scheduled_job_expression, str):
                return [
                 self.scheduled_job_expression]
            if isinstance(self.scheduled_job_expression, list):
                return self.scheduled_job_expression
        raise TypeError('{}.cron_job_expression'.format(self.identifier))
    ...
    def scheduled_job_event_rule_aws_objects(self) -> typing.Dict[(str, events.Rule)]:
        ...
        if self._scheduled_job_event_rule_aws_objects_cache is NOTHING:
            ...
            for expression in self.scheduled_job_expression_list:
                event_rule_logic_id = 'EventRule{}'.format(fingerprint.of_text(expression + self.lbd_func_name))
                event_rule = events.Rule(title=event_rule_logic_id,
                  State='ENABLED',
                  ScheduleExpression=expression,
                  Targets=[
                 events.Target(Id='EventRuleStartCrawlerGitHubDataTrigger',
    ...
    def scheduled_job_event_lbd_permission_aws_objects(self) -> typing.Dict[(str, awslambda.Permission)]:
        if self._scheduled_job_event_lbd_permission_aws_objects_cache is NOTHING:
            dct = dict()
            for expression in self.scheduled_job_expression_list:
                event_rule_lambda_permission_logic_id = 'LbdPermissionEventRule{}'.format(fingerprint.of_text(expression + self.lbd_func_name))
                event_rule = self.scheduled_job_event_rule_aws_objects[expression]
                event_rule_lambda_permission = awslambda.Permission(title=event_rule_lambda_permission_logic_id,
                  Action='lambda:InvokeFunction',
                  FunctionName=(GetAtt(self.lbd_func_aws_object, 'Arn')),
...
def lbd_func_config_value_handler(module_name: str, config_field: str, config_class: typing.Type[LbdFuncConfig], valid_func_name_list: typing.List[str], default_lbd_handler_name: str):
    root_module_name = module_name
    for py_current_module, py_parent_module, py_handler_func in walk_lbd_handler(module_name, valid_func_name_list):
        current_module_config = getattr(py_current_module, config_field)
        ...
        if py_handler_func is not None:
            py_handler_func_config = getattr(py_handler_func, config_field)
            ...
            if py_handler_func_config.lbd_func_name is REQUIRED:
                py_handler_func_config.lbd_func_name = slugify(py_handler_func_config.rel_module_name.replace('.', '-')) + '-' + slugify(py_handler_func.__name__)


def template_creation_handler(module_name: str, config_field: str, config_class: typing.Type[LbdFuncConfig], valid_func_name_list: typing.List[str], template: Template):
    for py_current_module, py_parent_module, py_handler_func in walk_lbd_handler(module_name, valid_func_name_list):
        current_module_config = getattr(py_current_module, config_field)
