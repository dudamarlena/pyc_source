...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sloancone/cone_search.py
...
class cone_search(object):
    ...
    **Usage**
    ...
    def __init__(self, log, ra, dec, searchRadius, nearest, outputFormat='table', galaxyType=False):
        ...
        squareSearch = sdss_square_search(log=self.log, ra=self.ra, dec=self.dec, searchRadius=self.searchRadius, galaxyType=self.galaxyType)
    ...
    def get(self):
        ...
        **Return**
        ...
        results = sorted(results, key=itemgetter('separation_arcsec'), reverse=True)
        headers = [
        ...
        if not len(filteredResults):
            orderDict = collections.OrderedDict(sorted({}.items()))
            for h in headers:
                if self.galaxyType == 'all' or self.galaxyType == False or self.galaxyType == 'specz' and h not in ('photoz_err',
                                                                                                                    'photoz') or self.galaxyType == 'photoz' and h not in ('specz',
                                                                                                                                                                           'specz_err'):
                    orderDict[h] = ''

            filteredResults = [
             orderDict]
        dataSet = list_of_dictionaries(log=self.log, listOfDictionaries=list(reversed(filteredResults)))
