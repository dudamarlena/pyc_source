...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sloancone/image.py
...
class image(object):
    ...
    **Usage**
    ...
    def __init__(self, log, ra, dec, downloadDirectory='./', filename='sdss_stamp.jpeg', settings=False, grid=True, label=False, photocat=False, speccat=False, invertColors=False, arcminWidth=5, pixelWidth=500):
        self.log = log
        ...
        self.covered = check_coverage(log=log, ra=self.ra, dec=self.dec).get()
        return
    ...
    def _download_sdss_image(self):
        ...
        scale = old_div(self.arcminWidth * 60.0, width)
        ...
        localUrls = multiobject_download(urlList=[
         url], downloadDirectory=self.downloadDirectory, log=self.log, timeStamp=False, timeout=180, concurrentDownloads=10, resetFilename=[
         self.filename], credentials=False, longTime=True, indexFilenames=False)
        print(url)
