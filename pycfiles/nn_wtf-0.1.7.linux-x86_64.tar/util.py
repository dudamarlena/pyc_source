...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/nn_wtf/tests/util.py
...
def train_neural_network(train_data, graph=None):
    ...
    init_graph(graph)
    graph.train(data_sets=data_sets, steps_between_checks=50, max_steps=1000, batch_size=train_data.num_examples, precision=0.99)
    return graph
...
def allow_fail(max_times_fail=1, silent=True):
    ...
    def allow_fail_decorator(func):
        ...
        def run_test_checked(self):
            ...
            """
            if not silent:
                print ('\ntrying {}.{}() max. {} times').format(type(self).__name__, func.__name__, max_times_fail + 1)
