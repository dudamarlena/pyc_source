...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/nn_wtf/tests/neural_network_optimizer_test.py
...
class NeuralNetworkOptimizerTest(unittest.TestCase):
    ...
    def test_brute_force_optimal_network_geometry_runs(self):
        ...
        self.assertEqual(optimal_layers, (self.LAYER_SIZES[0][0], self.LAYER_SIZES[1][0], self.LAYER_SIZES[2][0]))
    ...
    def _create_sa_optimizer(self, layer_sizes=(
 (2, ), (2, 2)), start_size_difference=1):
        return SimulatedAnnealingOptimizer(NeuralNetworkGraph, end_training_precision=0.5 + self.DESIRED_PRECISION / 2, start_training_precision=self.DESIRED_PRECISION, layer_sizes=layer_sizes, start_size_difference=start_size_difference, input_size=2, output_size=2, max_num_before_branching_out=2, learning_rate=0.1, batch_size=self.train_data.num_examples)

    def _create_bf_optimizer(self):
        return BruteForceOptimizer(NeuralNetworkGraph, input_size=self.train_data.input.shape[0], output_size=len(self.train_data.labels), desired_training_precision=self.DESIRED_PRECISION, layer_sizes=self.LAYER_SIZES, batch_size=self.train_data.num_examples, verbose=False)