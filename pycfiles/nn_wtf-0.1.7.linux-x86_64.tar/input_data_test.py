...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/nn_wtf/tests/input_data_test.py
...
class InputDataTest(unittest.TestCase):
    ...
    def test_read_images_from_file_one(self):
        data = read_images_from_file(self.SAMPLE_MNIST_FILE, MNISTGraph.IMAGE_SIZE, MNISTGraph.IMAGE_SIZE, 1)
        self._check_is_one_mnist_image(data)

    def test_read_images_from_file_two(self):
        data = read_images_from_file(self.SAMPLE_MNIST_FILE_WITH_TWO_IMAGES, MNISTGraph.IMAGE_SIZE, MNISTGraph.IMAGE_SIZE, 2)
        self._check_is_n_mnist_images(2, data)

    def test_read_images_from_file_fails_if_file_too_short(self):
        with self.assertRaises(ValueError):
            read_images_from_file(self.SAMPLE_MNIST_FILE_WITH_TWO_IMAGES, MNISTGraph.IMAGE_SIZE, MNISTGraph.IMAGE_SIZE, 3)

    def test_read_images_from_file_two_using_mnist_data_sets(self):
        data = MNISTDataSets.read_images_from_file(self.SAMPLE_MNIST_FILE_WITH_TWO_IMAGES, 2)
        self._check_is_n_mnist_images(2, data)

    def test_read_images_from_file_using_mnist_data_sets_fails_if_file_too_short(self):
        with self.assertRaises(ValueError):
            MNISTDataSets.read_images_from_file(self.SAMPLE_MNIST_FILE_WITH_TWO_IMAGES, 3)

    def test_read_images_from_two_files_using_mnist_data_sets(self):
        data = MNISTDataSets.read_images_from_files(get_project_root_folder() + '/nn_wtf/data/7_from_test_set.raw', get_project_root_folder() + '/nn_wtf/data/2_from_test_set.raw')
        self._check_is_n_mnist_images(2, data)

    def test_read_images_from_three_files_using_mnist_data_sets(self):
        data = MNISTDataSets.read_images_from_files(get_project_root_folder() + '/nn_wtf/data/7_from_test_set.raw', get_project_root_folder() + '/nn_wtf/data/2_from_test_set.raw', get_project_root_folder() + '/nn_wtf/data/1_from_test_set.raw')
    ...
    def test_read_images_from_two_urls_using_mnist_data_sets(self):
        if not self.TEST_READ_URLS:
            self.skipTest('Testing reading from URLs disabled')
        data = MNISTDataSets.read_images_from_urls(self.SAMPLE_MNIST_URL_1, self.SAMPLE_MNIST_URL_2)
        self._check_is_n_mnist_images(2, data)
