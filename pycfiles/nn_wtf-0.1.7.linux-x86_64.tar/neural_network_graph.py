...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/nn_wtf/neural_network_graph.py
# Compiled at: 2016-12-21 16:59:08
...
class NeuralNetworkGraph(NeuralNetworkGraphBase):
    ...
    def _build_neural_network(self):
        ...
        for i in range(1, self.num_hidden_layers + 1):
            self.layers.append(_add_layer('layer%04d' % i, self.layer_sizes[(i - 1)], self.layer_sizes[i], self.layers[(i - 1)], tf.nn.relu))
...
def _initialize_weights(in_units_size, out_units_size):
    ...
    return tf.Variable(tf.truncated_normal([in_units_size, out_units_size], stddev=1.0 / math.sqrt(float(in_units_size))), name='weights')
