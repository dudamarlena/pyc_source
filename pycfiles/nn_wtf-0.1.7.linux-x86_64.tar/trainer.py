...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/nn_wtf/trainer.py
# Compiled at: 2016-12-21 16:55:21
...
class Trainer:
    ...
    def train(self, data_sets, max_steps, precision=None, steps_between_checks=100, run_as_check=None, batch_size=None):
    ...
    def do_eval(self, data_set, batch_size):
        ...
        """
        self.evaluation = Trainer.Evaluation(data_set.num_examples, self._get_num_correctly_predicted(batch_size, data_set))
        return self.evaluation

    def _check_train_parameters_valid(self, batch_size, data_sets, precision):
        assert isinstance(data_sets, DataSets)
        assert precision is None or 0.0 <= precision < 1.0, 'precision, if set, must be between 0 and 1'
        assert data_sets.train.num_examples % batch_size == 0, ('training set size {} not divisible by batch size {}').format(data_sets.train.num_examples, batch_size)
    ...
    def _get_num_correctly_predicted(self, batch_size, data_set):
        ...
        return sum(self.graph.session.run(self.eval_correct_op, feed_dict=self.graph.fill_feed_dict(data_set, batch_size)) for _ in range(data_set.num_examples // batch_size))
    ...
    def _loss(self, logits, labels_placeholder):
        ...
        cross_entropy = tf.nn.softmax_cross_entropy_with_logits(logits, onehot_labels, name='cross_entropy')
