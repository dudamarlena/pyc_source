...
# Embedded file name: build/bdist.linux-x86_64/egg/pyculiarity/detect_vec.py
# Compiled at: 2018-08-28 00:38:22
...
def detect_vec(df, max_anoms=0.1, direction='pos', alpha=0.05, period=None, only_last=False, threshold=None, e_value=False, longterm_period=None, plot=False, y_log=False, xlabel='', ylabel='count', title=None, verbose=False):
    """
    ...
    if only_last:
        ...
        d = {'timestamp': df.timestamp.iloc[-past_obs:-period], 
           'value': df.value.iloc[-past_obs:-period]}
        x_subset_previous = DataFrame(d, index=d['timestamp'])
        all_anoms = all_anoms[(all_anoms.timestamp >= x_subset_single_period.timestamp.iloc[0])]
