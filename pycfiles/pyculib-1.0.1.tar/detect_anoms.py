...
# Embedded file name: build/bdist.linux-x86_64/egg/pyculiarity/detect_anoms.py
...
import numpy as np, pandas as ps, statsmodels.api as sm, sys
...
def detect_anoms(data, k=0.49, alpha=0.05, num_obs_per_period=None, use_decomp=True, one_tail=True, upper_tail=True, verbose=False):
    ...
    if len(list(map(lambda x: x[0], list(groupby(ps.isnull(ps.concat([ps.Series([np.nan]),
     data.value,
     ps.Series([np.nan])]))))))) > 3:
