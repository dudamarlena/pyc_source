...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/yarlp/utils/env_utils.py
...
def wrap_atari(env):
    assert 'NoFrameskip' in env.spec.id, '{} is not an atari env'.format(env)
    env = MonitorEnv(env)
...
class CappedCubicVideoSchedule(object):
    ...
    def __call__(self, count):
        if count < 1000:
            return int(round(count ** 0.3333333333333333)) ** 3 == count
        else:
            return count % 1000 == 0
...
class GymEnv(Env):
    ...
    def __init__(self, env_name, video=False, log_dir=None, force_reset=False, is_atari=False, *args, **kwargs):
...
class NormalizedGymEnv(GymEnv):
    ...
    def __init__(self, env_name, video=False, log_dir=None, force_reset=False, scale_reward=1.0, min_reward_std=0.01, min_obs_std=0.01, norm_obs_clip=5, normalize_obs=False, normalize_rewards=False, scale_continuous_actions=False, is_atari=False, *args, **kwargs):
        ...
        if normalize_obs is True:
            assert is_atari is False, 'normalize_obs must be False if is_atari is True'
            self._obs_rms = RunningMeanStd(shape=self.env.observation_space.shape, min_std=min_obs_std, clip_val=norm_obs_clip)
        if normalize_rewards is True:
            self._reward_rms = RunningMeanStd(shape=1, min_std=min_reward_std)
    ...
    def step(self, action):
        if self._scale_continuous_actions and isinstance(self.env.action_space, Box):
            lb, ub = self.env.action_space.low, self.env.action_space.high
            scaled_action = lb + (action[0] + 1.0) * 0.5 * (ub - lb)
...
class RunningMeanStd(object):
    ...
    def __init__(self, shape, min_std=1e-06, clip_val=None):
    ...
    def update(self):
        ...
        if self._count == 0:
            ...
            M2 = m_a + m_b + delta ** 2 * self._count * X.shape[0] / (self._count + X.shape[0])
...
def get_wrapper_by_name(env, classname):
    ...
    while True:
        ...
        if hasattr(env, 'env'):
            ...
            raise ValueError('Could not find wrapper named {}'.format(classname))


def make_parallel_envs(env_id, num_envs, start_seed, is_atari, **kwargs):
    envs = [NormalizedGymEnv(env_id, is_atari=is_atari, **kwargs) for _ in range(num_envs)]
...
class ParallelEnvs:
    ...
    def __init__(self, env_id, num_envs, start_seed=1, is_atari=True, **kwargs):
        ...
        """
        envs = make_parallel_envs(env_id, num_envs, start_seed, is_atari, **kwargs)
        self.envs = envs
