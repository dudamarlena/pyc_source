...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/yarlp/external/baselines/baselines/deepq/experiments/atari/wang2015_eval.py
# Compiled at: 2018-04-01 14:21:44
# Size of source mod 2**32: 3012 bytes
import argparse, gym, numpy as np, os, baselines.common.tf_util as U
    ...
    for num_noops in range(1, 31):
        ...
        while 1:
            ...
            if len(info['rewards']) > 0:
                ...
                break
            if info['steps'] > 108000:
                episode_rewards.append(sum(env_monitored.rewards))
                break

        print('Num steps in episode {} was {} yielding {} reward'.format(num_noops, eval_episode_steps, episode_rewards[(-1)]), flush=True)
...
def main():
    ...
    with U.make_session(4):
        ...
        act = deepq.build_act(make_obs_ph=lambda name: Uint8Input(env.observation_space.shape, name=name), q_func=dueling_model if args.dueling else model, num_actions=env.action_space.n)
