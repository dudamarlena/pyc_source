...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/yarlp/external/baselines/baselines/gail/gail-eval.py
...
def evaluate_env(env_name, seed, policy_hidden_size, stochastic, reuse, prefix):
    ...
    def policy_fn(name, ob_space, ac_space, reuse=False):
        return mlp_policy.MlpPolicy(name=name, ob_space=ob_space, ac_space=ac_space, reuse=reuse, hid_size=policy_hidden_size, num_hid_layers=2)
    ...
    for i, limit in enumerate(CONFIG['traj_limitation']):
        ...
        avg_len, avg_ret = run_mujoco.runner(env, policy_fn, checkpoint_path, timesteps_per_batch=1024, number_trajs=10, stochastic_policy=stochastic, reuse=i != 0 or reuse)
