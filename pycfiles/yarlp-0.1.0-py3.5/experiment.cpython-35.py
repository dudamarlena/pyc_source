...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/yarlp/experiment/experiment.py
# Compiled at: 2018-04-01 15:02:11
# Size of source mod 2**32: 18619 bytes
import os, gym, sys, json, copy, click, subprocess, pandas as pd, numpy as np
...
class Experiment(object):
    ...
    @classmethod
    def from_json_spec(cls, json_spec_filename, log_dir=None, reload_exp=False, *args, **kwargs):
        """
        ...
        assert os.path.exists(json_spec_filename) and os.path.isfile(json_spec_filename), 'spec-filename does not exist'
        ...
        if not log_dir:
            cls._experiment_dir = cls._create_log_dir(json_spec_filename)
        else:
    ...
    def run_job(self, s):
        ...
        command = "{} -m yarlp.experiment.job --log-dir {} --video {} --spec '{}' --reload-exp '{}'".format(python_path, self._experiment_dir, self.video, json.dumps(s), self.reload_exp)
    ...
    def _add_validated_agent_repeats(self, spec_list):
        ...
        for s in spec_list:
            env_name = s['env']['name']
            assert env_name in env_list, '{} is not an available environment name.'.format(env_name)
            for r in set(s['agent']['seeds']):
                s_copy = copy.deepcopy(s)
                run_name = '{}_{}_run{}'.format(s['env']['name'], s['agent']['type'], r)
    ...
    def _expand_agent_grid(self, spec_list):
        ...
        for scount, s in enumerate(spec_list):
            ...
            singleton_params = {k:v for k, v in params.items() if not isinstance(v, list) or k.endswith('schedule') if not isinstance(v, list) or k.endswith('schedule')}
            grid_params = {k:v for k, v in params.items() if isinstance(v, list) and not k.endswith('schedule') if isinstance(v, list) and not k.endswith('schedule')}
            grid_params = [dict(zip(grid_params.keys(), x)) for x in product(*grid_params.values())]
    ...
    def _validate_agent_names(self, spec):
        ...
        for s in spec:
            ...
            assert agent_name in cls_dict, '{} is not an implemented agent. Select one of {}'.format(agent_name, cls_dict.keys())
            agent_set.add(agent_name)

    @staticmethod
    def _get_experiment_dir(experiment_name):
        home = os.path.expanduser('~')
        experiment_dir = os.path.join(home, 'yarlp_experiments', experiment_name)
    ...
    def _create_log_dir(self, spec_filename):
        ...
        experiment_dir = Experiment._get_experiment_dir(experiment_name)
        return experiment_utils._create_log_directory(experiment_name, experiment_dir)
...
def _merge_stats(experiment_dir):
    ...
    for d in os.listdir(experiment_dir):
        ...
        if not not os.path.isdir(base_path):
            if d == 'stats':
                pass
            else:
...
def generate_plots(yarlp_dir, by_field='env'):
    ...
    ts['run_name'] = ts['run_name'].apply(lambda x: '_'.join(x.split('_')[-2:]))
    for rn in ts['run_name'].unique():
        ts.loc[(ts['run_name'] == rn, 'Iteration')] = np.arange(ts[(ts['run_name'] == rn)].shape[0])

    for f in ts[by_field].unique():
        ts['name'] = 'yarlp'
        yarlp = ts[(ts[by_field] == f)]
        fig = plotting.make_plots(yarlp, f, 'run_name', 'param_run')
        fig.savefig(os.path.join(yarlp_dir, '{}.png'.format(f)))
        fig = plotting.make_plots(yarlp, f, 'run_name', 'run_name')
        fig.savefig(os.path.join(yarlp_dir, '{}_all_runs.png'.format(f)))
...
def generate_plots_benchmark_vs_yarlp(yarlp_dir, benchmark_dir):
    ...
    for env in training_stats.env.unique():
        benchmark = benchmark_stats[(benchmark_stats.env == env)].rename(columns={'EpRewMean': 'Smoothed_total_reward', 
        ...
        for run_name in benchmark.run_name.unique():
            benchmark.loc[(benchmark.run_name == run_name, 'Iteration')] = list(range(benchmark[(benchmark.run_name == run_name)].shape[0]))
        ...
        fig.savefig(os.path.join(yarlp_dir, '{}.png'.format(env)))
...
def run_mujoco1m_benchmark(agent):
    ...
    experiment_dir = Experiment._get_experiment_dir(benchmark_name)
    base_log_path = experiment_utils._create_log_directory(benchmark_name, experiment_dir)
    ...
    for t in benchmark['tasks']:
        d = {'env': {'name': t['env_id'], 
         ...
         'agent': {'type': agent, 
                   ...
                   'training_params': {'max_timesteps': t['num_timesteps']}}}
        j.append(d)

    j = {'runs': j}
    spec_file = os.path.join(base_log_path, 'spec.json')
    json.dump(j, open(spec_file, 'w'))
    e = Experiment.from_json_spec(spec_file, log_dir=base_log_path)
    e.run()
...
def run_atari10m_ddqn_benchmark(n_jobs):
    ...
    spec_file = os.path.join(base_log_path, 'spec.json')
    json.dump(j, open(spec_file, 'w'))
    e = Experiment.from_json_spec(spec_file, log_dir=base_log_path, video=True)
    if n_jobs > 1:
        ...
        d = {'env': {'name': t['env_id'], 
         ...
         'agent': {'type': agent, 
                   ...
                   'params': {'discount_factor': 0.99, 
                              ...
                              'policy_network_params': {'final_dense_weights_initializer': 0.1}, 
                              
                              'value_network_params': {'final_dense_weights_initializer': 1.0}, 
...
@click.command()
@click.option('--spec-file', default='./experiment_configs/reinforce_experiment.json', help='Path to json file spec if continue=False, else path to experiment')
@click.option('--video', default=False, type=bool, help='Whether to record video or not')
@click.option('--parallel', default=True, type=bool, help='Whether to run in parallel or not')
@click.option('--n-jobs', default=1)
@click.option('--reload-exp', default=False, type=bool, help='Whether to restart from an experiment dir')
def run_experiment(spec_file, video, parallel, n_jobs, reload_exp):
    if reload_exp:
        log_dir = os.path.dirname(spec_file)
    else:
        log_dir = None
    e = Experiment.from_json_spec(spec_file, log_dir=log_dir, video=video, reload_exp=reload_exp)
    e.run(parallel=parallel, n_jobs=n_jobs)


@click.command()
@click.option('--upload-dir', help='Path of openai gym session to upload')
