...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/yarlp/external/baselines/baselines/deepq/experiments/atari/train.py
# Compiled at: 2018-04-01 14:21:44
# Size of source mod 2**32: 14048 bytes
import argparse, gym, numpy as np, os, tensorflow as tf, tempfile, time, json, baselines.common.tf_util as U
from baselines import logger
from baselines import deepq
from baselines.deepq.replay_buffer import ReplayBuffer, PrioritizedReplayBuffer
from baselines.common.misc_util import boolean_flag, pickle_load, pretty_eta, relatively_safe_pickle_dump, set_global_seeds, RunningAvg, SimpleMonitor
...
def parse_args():
    ...
    parser.add_argument('--replay-buffer-size', type=int, default=int(1000000.0), help='replay buffer size')
    parser.add_argument('--lr', type=float, default=0.0001, help='learning rate for Adam optimizer')
    parser.add_argument('--num-steps', type=int, default=int(200000000.0), help='total number of steps to run the environment for')
    ...
    parser.add_argument('--prioritized-eps', type=float, default=1e-06, help='eps parameter for prioritized replay buffer')
    parser.add_argument('--save-dir', type=str, default=None, help='directory in which training state and model should be saved.')
    parser.add_argument('--save-azure-container', type=str, default=None, help='It present data will saved/loaded from Azure. Should be in format ACCOUNT_NAME:ACCOUNT_KEY:CONTAINER')
    parser.add_argument('--save-freq', type=int, default=1000000.0, help='save model once every time this many iterations are completed')
...
if __name__ == '__main__':
    ...
    with U.make_session(4) as (sess):
        ...
        act, train, update_target, debug = deepq.build_train(make_obs_ph=lambda name: Uint8Input(env.observation_space.shape, name=name), q_func=model_wrapper, num_actions=env.action_space.n, optimizer=tf.train.AdamOptimizer(learning_rate=args.lr, epsilon=0.0001), gamma=0.99, grad_norm_clipping=10, double_q=args.double_q, param_noise=args.param_noise)
        ...
        exploration = PiecewiseSchedule([
         ...
         (
          approximate_num_iters / 5, 0.01)], outside_value=0.01)
        if args.prioritized:
        ...
        while 1:
            ...
            if not args.param_noise:
                ...
                if args.param_noise_reset_freq > 0 and num_iters_since_reset > args.param_noise_reset_freq:
                    reset = True
                update_eps = 0.01
                update_param_noise_threshold = -np.log(1.0 - exploration.value(num_iters) + exploration.value(num_iters) / float(env.action_space.n))
                kwargs['reset'] = reset
            ...
            if num_iters > max(5 * args.batch_size, args.replay_buffer_size // 20) and num_iters % args.learning_freq == 0:
                ...
                if num_iters > 0 and (num_iters % args.save_freq == 0 or info['steps'] > args.num_steps):
                    maybe_save_model(savedir, container, {'replay_buffer': replay_buffer, 
                     'num_iters': num_iters, 
                ...
                if done:
                    ...
                    fps_estimate = float(steps_per_iter) / (float(iteration_time_est) + 1e-06) if steps_per_iter._value is not None else 'calculating...'
                    logger.dump_tabular()
                    logger.log()
