...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/yarlp/agent/a2c_agent.py
...
class A2CAgent(Agent):
    ...
    def __init__(self, env, policy_network=None, policy_network_params={'final_dense_weights_initializer': 0.01}, policy_learning_rate=0.0005, value_network_params={'final_dense_weights_initializer': 1.0}, entropy_weight=0.01, model_file_path=None, adaptive_std=False, init_std=1.0, min_std=1e-06, n_steps=5, max_timesteps=1000000, grad_norm_clipping=0.5, gae_lambda=0.98, checkpoint_freq=10000, save_freq=50000, policy_learning_rate_schedule=None, *args, **kwargs):
        super().__init__(env, *args, **kwargs)
        assert isinstance(self._env, ParallelEnvs), 'env must be ParallelEnvs class for A2C agent'
        if policy_network is None:
            policy_network = cnn
        pn = get_network(policy_network, policy_network_params)
        self._policy = a2c_model_factory(env, network=pn, policy_network_params=policy_network_params, value_network_params=value_network_params, learning_rate=policy_learning_rate, has_learning_rate_schedule=True, entropy_weight=entropy_weight, min_std=min_std, init_std=init_std, adaptive_std=adaptive_std, grad_norm_clipping=grad_norm_clipping, model_file_path=model_file_path)
        ...
        if policy_learning_rate_schedule is None:
            ...
            lr = PiecewiseSchedule(policy_learning_rate_schedule, outside_value=policy_learning_rate_schedule[(-1)][(-1)])
        self.lr_schedule = lr

    def set_env(self):
        self._env = ParallelEnvs(self.env_id, self.num_envs, self.start_seed, self.is_atari)
    ...
    def train(self):
        ...
        while self.t < self.max_timesteps:
            ...
            for n in range(self.n_steps):
                ...
                values = self._policy.G(self._policy['vf'], {self._policy['state']: obs})
                mb_obs.append(obs)
            ...
            last_values = self._policy.G(self._policy['vf'], {self._policy['state']: obs})
            mb_discounted_rewards = []
            for n in range(self._env.num_envs):
                rollout = {'baseline_preds': mb_values[n].flatten(), 
            ...
            policy_loss = self._policy.update(mb_obs, mb_rewards, mb_actions, mb_rewards - mb_values, lr)
            policy_update_feed_dict = self._policy.build_update_feed_dict(self._policy, mb_obs, mb_rewards, mb_actions, mb_rewards - mb_values, lr)
            policy_entropy = self._policy.G(self._policy['entropy'], policy_update_feed_dict)
            vf_loss = self._policy.G(self._policy['vf_loss'], policy_update_feed_dict)
            ev = explained_variance(mb_rewards, mb_values)
            if self.t > 0 and self.t % self.checkpoint_freq == 0 and len(self._env.get_episode_rewards()) > 0:
                episode_rewards = self._env.get_episode_rewards(int(self.reward_len / self.num_envs))
                ...
                self.logger.add_metric('policy_learning_rate', self.lr_schedule.value(self.t))
                self.logger.log()
            if self.t % self.save_freq == 0 and self.t and self.logger._log_dir is not None and len(self._env.get_episode_rewards()) > 0:
