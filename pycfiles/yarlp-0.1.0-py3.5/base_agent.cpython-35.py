...
class Agent(ABC):
    ...
    def __init__(self, env, discount_factor=0.99, log_dir=None, seed=None, gae_lambda=0, reward_len=100):
        """
    ...
    def set_logger(self, log_dir, reward_len):
        if log_dir is None:
            ...
            self.logger = MetricLogger(log_dir=log_dir, reward_len=reward_len)
    ...
    def get_baseline_pred(self, obs):
        ...
        if self._baseline_model is None:
            return np.zeros(len(obs))
        return self._baseline_model.predict(np.array(obs)).flatten()
    ...
    def get_action(self, state, greedy=False):
        ...
        with self._policy.G._session.as_default():
            a = self._policy.policy.predict(self._policy.get_session(), batch, greedy)[0]
        return a
    ...
    def get_batch_actions(self, states, greedy=False):
        ...
        """
        with self._policy.G._session.as_default():
            return self._policy.policy.predict(self._policy.get_session(), states, greedy)
...
def do_rollout(agent, env, n_steps=None, render=False, render_freq=10, greedy=False):
    """
    ...
    while True:
        if render and t and t % render_freq == 0:
            env.render()
        is_truncated_rollout = n_steps is not None and t > 0 and t % n_steps == 0
        ...
        if is_truncated_rollout or is_completed_rollout:
            ...
            if len(episode_returns) == 0:
                episode_returns = [
                 sum(rewards)]
            rollout = {'observations': np.array(observations), 
...
def add_advantage(rollout, gamma, Lambda=0):
    ...
    for t in reversed(range(T)):
        ...
        gaelam[t] = lastgaelam = delta + gamma * Lambda * nonterminal * lastgaelam
...
class BatchAgent(Agent):
    ...
    def __init__(self, env, baseline_network=mlp, baseline_model_learning_rate=0.001, baseline_train_iters=5, baseline_network_params={'final_weights_initializer': normc_initializer(1.0)}, *args, **kwargs):
        """
        """
        ...
        if isinstance(baseline_network, LinearFeatureBaseline):
            ...
            if baseline_network is None:
                ...
                self._baseline_model = value_function_model_factory(env, baseline_network, learning_rate=baseline_model_learning_rate)
                self.tf_object_attributes.add('_baseline_model')
    ...
    def train(self, num_train_steps=0, num_test_steps=0, n_steps=1024, max_timesteps=0, render=False, whiten_advantages=True, *args, **kwargs):
        """
        Parameters
        ----------
        ...
        None
        """
        rollout_gen = do_rollout(self, self._env, n_steps, render=render, *args, **kwargs)
        train_steps_so_far = 0
        timesteps_so_far = 0
        assert sum([max_timesteps > 0, num_train_steps > 0]) == 1, 'Either max_timesteps > 0 or num_train_steps > 0'
        while 1:
            ...
            if whiten_advantages:
                ...
                adv = (adv - np.mean(adv)) / (np.std(adv) + 1e-08)
                rollout['advantages'] = adv
            self.update(rollout)
            if isinstance(self._baseline_model, LinearFeatureBaseline):
                self._baseline_model.fit(rollout['observations'], rollout['discounted_future_reward'])
            else:
                if hasattr(self._baseline_model, 'G'):
                    data = [
                     rollout['observations'],
                     rollout['discounted_future_reward']]
                    self.logger.add_metric('Baseline_Loss_Before', self._baseline_model.eval_tensor(self._baseline_model['loss'], *data))
                    for _ in range(self.baseline_train_iters):
                        for ob, a in tf_utils.iterbatches([*data]):
                            self._baseline_model.update(ob, a)

                    self.logger.add_metric('Baseline_Loss_After', self._baseline_model.eval_tensor(self._baseline_model['loss'], *data))
            ...
            if num_test_steps > 0:
                test_gen = do_rollout(self, self._env, greedy=True)
                r = []
