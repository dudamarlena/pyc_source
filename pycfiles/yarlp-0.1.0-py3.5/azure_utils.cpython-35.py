...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/yarlp/external/baselines/baselines/common/azure_utils.py
...
class Container(object):
    ...
    def put(self, source_path, blob_name, callback=None):
        ...
        try:
            x_ms_copy_source = 'https://{}.blob.core.windows.net/{}/{}'.format(self._account_name, self._container_name, blob_name)
            self._service.copy_blob(container_name=self._container_name, blob_name=blob_name + '.backup', x_ms_copy_source=x_ms_copy_source)
        ...
        with tempfile.TemporaryDirectory() as (td):
            ...
            self._service.put_block_blob_from_path(container_name=self._container_name, blob_name=blob_name, file_path=arcpath, max_connections=4, progress_callback=progress_callback, max_retries=10)
            upload_done.wait()
    ...
    def get(self, dest_path, blob_name, callback=None):
        ...
        with tempfile.TemporaryDirectory() as (td):
            ...
            for backup_blob_name in [blob_name, blob_name + '.backup']:
                try:
                    properties = self._service.get_blob_properties(blob_name=backup_blob_name, container_name=self._container_name)
                    if hasattr(properties, 'properties'):
                        blob_size = properties.properties.content_length
                    else:
                        blob_size = properties['content-length']
                    if int(blob_size) > 0:
                        self._service.get_blob_to_path(container_name=self._container_name, blob_name=backup_blob_name, file_path=arcpath, max_connections=4, progress_callback=progress_callback)
    ...
    def exists(self, blob_name):
        ...
        try:
            self._service.get_blob_properties(blob_name=blob_name, container_name=self._container_name)
            return True
        except AzureMissingResourceHttpError:
            return False