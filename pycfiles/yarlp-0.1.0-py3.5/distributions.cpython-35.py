...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/yarlp/policy/distributions.py
# Compiled at: 2018-03-18 17:00:49
...
class Categorical(Distribution):
    ...
    def kl(self, old_dist):
        ...
        return tf.reduce_sum(old_probs * (tf.log(old_probs + tf_utils.EPSILON) - tf.log(probs + tf_utils.EPSILON)), axis=-1)

    def entropy(self):
        probs = self.probs
        return -tf.reduce_sum(probs * tf.log(probs + tf_utils.EPSILON), axis=-1)
...
class DiagonalGaussian(Distribution):
    ...
    def kl(self, old_dist):
        ...
        return tf.reduce_sum(old_dist.logstd - self.logstd + numerator / denominator - 0.5, axis=-1)

    def entropy(self):
        return tf.reduce_sum(0.5 * tf.log(2.0 * np.pi * np.e) + self.logstd, axis=-1)

    def log_likelihood(self, x):
        dim = tf.to_float(tf.shape(self.mean)[(-1)])
        log_det_cov = 2.0 * tf.reduce_sum(self.logstd, axis=-1)
        dev = x - self.mean
