...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/yarlp/utils/metric_logger.py
# Compiled at: 2018-04-01 15:02:11
# Size of source mod 2**32: 5701 bytes
import os, re, json, time, logging, numpy as np
from tabulate import tabulate
from collections import deque

class MetricLogger:
    __doc__ = 'Logs metrics to console and to file\n    '
    ...
    def _tabulate(self):
        tabulate_list = list(zip(list(self._metric_dict), self._metric_dict.values()))
...
def explained_variance(y, pred):
    explained_variance = np.nan if np.var(y) == 0 else 1 - np.var(y - pred) / np.var(y)
