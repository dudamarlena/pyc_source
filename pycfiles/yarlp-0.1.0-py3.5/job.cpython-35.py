...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/yarlp/experiment/job.py
# Compiled at: 2018-04-01 15:02:11
# Size of source mod 2**32: 3100 bytes
import os, json, click, traceback, tensorflow as tf
...
class Job(object):
    ...
    def _load(self):
        existing_dirs = [d for d in os.listdir(self._log_dir) if d.startswith(self._spec_dict['run_name'])]
    ...
    def _get_env(self, job_dir):
        ...
        kwargs = {k:v for k, v in self._spec_dict['env'].items() if k != 'name' and k != 'is_parallel' if k != 'name' and k != 'is_parallel'}
        kwargs['video'] = self._video
        if not self._spec_dict['env'].get('is_parallel', False):
            env = NormalizedGymEnv(env_name, log_dir=job_dir, force_reset=True, **kwargs)
        else:
            env = ParallelEnvs(env_name, log_dir=job_dir, force_reset=True, **kwargs)
    ...
    def _create_log_dir(self):
        ...
        job_dir = experiment_utils._create_log_directory(dir_name, self._log_dir)
        experiment_utils._save_spec_to_dir({'runs': self._spec_dict}, job_dir)
        return job_dir
