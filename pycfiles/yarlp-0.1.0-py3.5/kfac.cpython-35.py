...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/yarlp/external/baselines/baselines/acktr/kfac.py
# Compiled at: 2018-04-01 14:21:44
# Size of source mod 2**32: 45523 bytes
...
class KfacOptimizer:
    ...
    def __init__(self, learning_rate=0.01, momentum=0.9, clip_kl=0.01, kfac_update=2, stats_accum_iter=60, full_stats_init=False, cold_iter=100, cold_lr=None, async=False, async_stats=False, epsilon=0.01, stats_decay=0.95, blockdiag_bias=False, channel_fac=False, factored_damping=False, approxT2=False, use_float64=False, weight_decay_dict={}, max_grad_norm=0.5):
    ...
    def getFactors(self, g, varlist):
        ...
        def searchFactors(gradient, graph):
            ...
            if fpropOp.op_def.name in KFAC_OPS:
                ...
                bInputsList = [i for i in bpropOp.inputs[0].op.inputs if 'gradientsSampled' in i.name and 'Shape' not in i.name]
                            ...
                            if not (not self._blockdiag_bias and self.stats[var]['assnWeights']):
                                if bpropFactor not in tmpStatsCache:
                                    slot_bpropFactor_stats = tf.Variable(tf.diag(tf.ones([
                                     bpropFactor.get_shape()[(-1)]])) * self._diag_init_coeff, name='KFAC_STATS/' + bpropFactor.op.name, trainable=False)
                ...
                if opType == 'Conv2D':
                    ...
                    if Oh == 1 and Ow == 1 and self._channel_fac:
                        ...
                        patches = tf.extract_image_patches(fpropFactor, ksizes=[1,
                         convkernel_size[0], convkernel_size[1], 1], strides=strides, rates=[1, 1, 1, 1], padding=padding)
                        if self._approxT2:
                            if KFAC_DEBUG:
                    ...
                    if stats_var_dim == fpropFactor_size + 1 and not self._blockdiag_bias:
                        if opType == 'Conv2D' and not self._approxT2:
                            fpropFactor = tf.concat([fpropFactor,
    ...
    def minimize(self, loss, loss_sampled, var_list=None):
        ...
        update_stats_op = self.compute_and_apply_stats(loss_sampled, var_list=var_list)
