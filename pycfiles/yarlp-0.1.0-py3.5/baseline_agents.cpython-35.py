...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/yarlp/agent/baseline_agents.py
...
class RandomAgent(Agent):
    __doc__ = 'Agent that takes random actions\n    '

    def __init__(self, env, *args, **kwargs):
        super().__init__(env, *args, **kwargs)
        self._policy = self

    def train(self, num_train_steps=1, n_steps=None, num_test_steps=0, max_timesteps=0):
        """
        ...
        rollout_gen = do_rollout(self, self._env, n_steps, greedy=False)
        while True:
    ...
    def get_action(self, states, *args, **kwargs):
        ...
        return np.array([self._env.action_space.sample() for _ in range(states.shape[0])])