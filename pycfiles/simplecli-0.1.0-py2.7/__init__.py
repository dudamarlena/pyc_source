...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/simplecli/__init__.py
# Compiled at: 2013-10-13 17:25:48
...
__version__ = '0.1.0'
__all__ = [
 'Command', 'Shell', 'Manager', 'Group', 'Option',
...
class Manager(object):
        ...
        for arg in args:
            if arg in kwargs:
                ...
                if isinstance(default, bool):
                    options.append(Option('-%s' % arg[0], '--%s' % arg, action='store_true', dest=arg, required=False, default=default))
                else:
                    options.append(Option('-%s' % arg[0], '--%s' % arg, dest=arg, type=unicode_type, required=False, default=default))
            else:
    ...
    def handle(self, name, args=None, prog=None):
        ...
        if getattr(command, 'capture_all_args', False):
            command_namespace, unparsed_args = command_parser.parse_known_args(args)
