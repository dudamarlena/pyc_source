...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/symath/simplify.py
# Compiled at: 2015-08-21 11:58:24
import threading, stdops, core, copy, operator, factor
...
def _strip_identities_pass(exp):
    ...
    if exp.match(a(b, c)):
        ...
        if ridentity != None and exp.match(a(b, ridentity), vals):
            return vals['b'].walk(_strip_identities)
    return exp
...
def _distribute(op1, op2):
    ...
    def _(exp):
        ...
        if exp.match(op1(op2(a, b), c), vals):
            ...
            if exp.match(op1(a, op2(b, c)), vals):
...
def _simplify_mul_div(exp):
    ...
    if exp.match(a / b, vals) and isinstance(vals.b, core.Number):
        return vals.a * (1.0 / vals.b.value())
    if exp.match(a / b, vals) and factor.is_factor(vals.b, vals.a):
...
def _simplify_known_values(exp):
    a, b, c = core.wilds('a b c')
    vals = {}
    if exp.match(a(b, c), vals) and 'numeric' in vals['a'].kargs and isinstance(vals['b'], core._KnownValue) and isinstance(vals['c'], core._KnownValue):
...
def _fold_additions(exp):
    ...
    if exp.match(a + a, vals):
        ...
        if exp.match(a + a * b, vals) or exp.match(a + b * a, vals):
...
def _simplify_bitops(exp):
    ...
    if exp.match(a ^ a):
        ...
        if exp.match(a & a, vals):
            return vals.a
        if exp.match(a << b >> b, vals) or exp.match(a >> b << b, vals):
            return vals.a
        return exp
...
def _simplify_pass(exp):
    exp = exp.walk(_commutative_reorder, _strip_identities, _simplify_mul_div, _strip_identities, _simplify_known_values, _strip_identities, _convert_to_pow, _strip_identities, _remove_subtractions, _strip_identities, _distribute(stdops.BitAnd, stdops.BitOr), _strip_identities, _distribute(stdops.Mul, stdops.Add), _strip_identities, _fold_additions, _strip_identities, _zero_terms, _strip_identities, _commutative_reorder, _strip_identities, _distribute(stdops.BitAnd, stdops.BitOr), _strip_identities, _distribute(stdops.Mul, stdops.Add), _strip_identities, _assoc_reorder, _strip_identities, _simplify_bitops, _strip_identities, _simplify_mul_div, _strip_identities)
