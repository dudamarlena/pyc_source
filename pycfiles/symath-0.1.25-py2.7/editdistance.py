...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/symath/algorithms/editdistance.py
...
rules
"""
import symath, symath.util as util, numpy
...
def _edit_distance(exp1, exp2, k, **subs):
    ...
    if exp1.match(exp2, vals) or exp2.match(exp1, vals):
        ...
        if isinstance(exp1, symath.core.Fn) and isinstance(exp2, symath.core.Fn):
            d, ns = _edit_distance(exp1[0], exp2[0], k, **subs)
            e, ns = _tuple_edit_distance(exp1.args, exp2.args, (k - d), **ns)
            return (
             d + e, ns)
        if isinstance(exp1, symath.core.Fn) and not isinstance(exp2, symath.core.Fn):
            d, ns = _edit_distance(exp1[0], exp2, k, **subs)
            e, ns = _tuple_edit_distance((), exp1.args, (k - d), **ns)
            return (
             d + e, ns)
        if isinstance(exp2, symath.core.Fn) and not isinstance(exp1, symath.core.Fn):
