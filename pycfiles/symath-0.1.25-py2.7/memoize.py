...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/symath/memoize.py
...
class Memoize(object):
    ...
    def MemoizeMembers(obj, *items):
        ...
        for membername in dir(obj):
            value = getattr(obj, membername)
            if (memall and '__' not in membername or membername in items) and not isinstance(value, Memoize) and isinstance(value, (types.FunctionType, types.BuiltinFunctionType)):
                setattr(obj, membername, Memoize(value))
...
class m(object):
    ...
    def __enter__(self):
        import types
        if not isinstance(self.obj, (types.TypeType, types.ModuleType)):
            self.isobj = True
...
if __name__ == '__main__':
    ...
    print identity(5) == 5
    print 5 in identity.results.values()
    print identity(7) == 7
    print 7 in identity.results.values()
    identity.f(10)
    print 10 not in identity.results.values()
    import testm
    for j in range(2):
        with m(testm):
            for i in range(1, 10):
