...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\gastrodon\__init__.py
# Compiled at: 2017-12-08 15:04:24
# Size of source mod 2**32: 33606 bytes
"""
...
_pncb_regex = '_A-Za-zÀ-ÖØ-öø-˿Ͱ-ͽͿ-\u1fff\u200c-\u200d⁰-\u218fⰀ-\u2fef、-\ud7ff豈-\ufdcfﷰ-�𐀀-\U000effff'
...
class Endpoint(metaclass=ABCMeta):
    ...
    def __init__(self, prefixes: Graph=None, base_uri=None):
        ...
        if prefixes != None:
            self._namespaces = set(map(lambda y: y if y[(-1)] in frozenset({'#', '/'}) else y + '/', [str(x[1]) for x in prefixes.namespaces()]))
        ...
        if term == None:
            ...
            if isinstance(term, URIRef):
                if self.prefixes != None:
                    if '/' in term.toPython() or str(term).startswith('urn:'):
                        if self.base_uri:
    ...
    def _to_rdf(self, value, prefixes):
        if not isinstance(value, Identifier):
            if isinstance(value, QName):
                ...
                if isinstance(value, GastrodonURI):
    ...
    def select(self, sparql: str, **kwargs) -> pd.DataFrame:
        ...
        if group_variables:
            if all([x in columnNames for x in group_variables]):
    ...
    def _filter_frame(self, that: FrameType):
        return {'_' + k:v for k, v in that.f_locals.items() if type(v) not in _cannot_substitute if not k.startswith('_') if not k.startswith('_')}
...
class RemoteEndpoint(Endpoint):
    ...
    def __init__(self, url, prefixes=None, user=None, passwd=None, http_auth=None, default_graph=None, base_uri=None):
...
class LocalEndpoint(Endpoint):
    ...
    def __init__(self, graph, prefixes=None):
        """
