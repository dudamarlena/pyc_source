...
# [GCC 8.4.0]
# Embedded file name: /Users/onema/Code/python/evee/build/lib/evee/event_dispatcher.py
# Compiled at: 2016-02-21 02:02:05
...
class EventDispatcher(AbstractEventDispatcher):
    ...
    def __init__(self):
        self._EventDispatcher__listeners = {}
        self._EventDispatcher__sorted = {}
    ...
    def add_listener(self, event_name: str=None, listener: Callable=None, priority: int=0) -> object:
        ...
        """
        if event_name not in self._EventDispatcher__listeners:
            self._EventDispatcher__listeners[event_name] = {}
        else:
            if priority not in self._EventDispatcher__listeners[event_name]:
                self._EventDispatcher__listeners[event_name][priority] = []
            self._EventDispatcher__listeners[event_name][priority].append(listener)
            if event_name in self._EventDispatcher__sorted:
                del self._EventDispatcher__sorted[event_name]
    ...
    def remove_listener(self, event_name: str, listener: Callable):
        ...
        if event_name not in self._EventDispatcher__listeners:
            return
        for priority, listeners in self._EventDispatcher__listeners[event_name].items():
            try:
                key = listeners.index(listener)
                del self._EventDispatcher__listeners[event_name][priority][key]
                if event_name in self._EventDispatcher__sorted:
                    del self._EventDispatcher__sorted[event_name]
            except ValueError:
                pass
    ...
    def get_listeners(self, event_name: str=None) -> List:
        ...
        if event_name:
            if event_name not in self._EventDispatcher__listeners:
                return {}
            if event_name not in self._EventDispatcher__sorted:
                self.sort_listeners(event_name)
            return self._EventDispatcher__sorted[event_name]
        else:
            for event_name, event_listener in self._EventDispatcher__listeners.items():
                if event_name not in self._EventDispatcher__sorted:
                    self.sort_listeners(event_name)

            return dict([(key, value) for key, value in self._EventDispatcher__sorted.items() if value != []])
    ...
    def _do_dispatch(self, listeners: Sequence[Callable[([Event, str, AbstractEventDispatcher], Event)]], event_name: str, event: Event):
        """
    ...
    def sort_listeners(self, event_name: str):
        ...
        :param event_name: the name of the event
        """
        listeners = OrderedDict(sorted((self._EventDispatcher__listeners[event_name].items()), reverse=True))
        ordered_listeners = []
        for priority, listener in listeners.items():
            ordered_listeners += listener

        self._EventDispatcher__sorted[event_name] = ordered_listeners