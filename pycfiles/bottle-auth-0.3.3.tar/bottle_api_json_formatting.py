...
# Embedded file name: /usr/lib/python2.6/site-packages/bottle_api_json_formatting/bottle_api_json_formatting.py
# Compiled at: 2013-11-30 16:16:57
__doc__ = ' Formats output in a json schema. To be used for making json based API \nservers '
...
class JsonFormatting(object):
 ...
 '*/*'], debug=False):
    ...
    def setup(self, app):
        ...
        self.app.default_error_handler = self.function_type(self.custom_error_handler, app, Bottle)
    ...
    def close(self):
        ...
        self.app.default_error_handler = self.function_type(self.function_original, self.app, Bottle)

    def get_response_object(self, status):
        """ Helper for building the json object """
        if status in self.statuses:
            json_response = {'status': self.statuses.get(status), 'status_code': status, 
               'data': None}
            return json_response
        else:
    ...
    def custom_error_handler(self, res, error):
        ...
        if self.in_supported_types(request.headers.get('Accept', '')):
            ...
            response_object['error'] = {'status_code': error.status_code, 
               'status': error.status_line, 
               'message': error.body}
            if self.debug:
                response_object['debug'] = {'exception': repr(error.exception), 'traceback': error.traceback}
            json_response = json_dumps(response_object)
            response.content_type = 'application/json'
            return json_response
        else:
