...
# [GCC 8.4.0]
# Embedded file name: D:\dev\scrapgo\scrapgo\lib\dataframe\orm.py
# Compiled at: 2019-06-09 06:17:12
# Size of source mod 2**32: 7654 bytes
from collections import abc
import sqlite3, pandas as pd, pymysql
from sqlalchemy import create_engine
from scrapgo.utils.fileutils import get_file_extension, read_json, read_conf
from .dfutils import get_difference_from
QUERY_FORMAT = {'select':'SELECT {columns} FROM {table}', 
...
class TableFrame(object):
        ...
        if db_conf_path is not None:
            ...
            if ext in ('.json', ):
                ...
                if ext in ('.cnf', '.conf'):
                    ...
                    raise ValueError(f"{ext} is not DB connection setting file extension(Only support .json, .cnf, .conf)")
    ...
    def select(self, table, columns=None, where=None):
        ...
        if where is None:
            ...
            query = self._get_query('select-where',
 ...
 'count-where', table, where=where, **kwargs)
    ...
    def delete(self, table, where=None, logging=True):
        if where is None:
            ...
            query = self._get_query('delete-where',
