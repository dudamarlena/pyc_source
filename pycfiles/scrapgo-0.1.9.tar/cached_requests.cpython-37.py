...
# [GCC 8.4.0]
# Embedded file name: D:\dev\scrapgo\scrapgo\modules\request\cached_requests.py
...
class CachedRequests(object):
    USER_AGENT_NAME = settings.USER_AGENT_NAME
    HEADERS = None
    ...
    def __init__(self, **kwargs):
        ...
        self.headers = {'User-Agent': getattr(FakeUserAgent(), self.USER_AGENT_NAME)}
        if self.HEADERS:
            self.headers.update(self.HEADERS)
        self.requests = requests_cache.CachedSession(cache_name=(self.CACHE_NAME),
    ...
    def _get_delay(self):
        if isinstance(self.REQUEST_DELAY, (tuple, list)):
            if len(self.REQUEST_DELAY) == 2:
