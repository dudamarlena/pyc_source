...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\supervisor\tests\test_process.py
# Compiled at: 2015-07-18 10:13:57
import errno, os, signal, time, unittest
...
class SubprocessTests(unittest.TestCase):
    ...
    def test_ctor(self):
        ...
        config = DummyPConfig(options, 'cat', 'bin/cat', stdout_logfile='/tmp/temp123.log', stderr_logfile='/tmp/temp456.log')
    ...
    def test_reopenlogs(self):
        ...
        instance = self._makeOne(config)
        instance.dispatchers = {0: DummyDispatcher(readable=True), 1: DummyDispatcher(writable=True)}
        instance.reopenlogs()
    ...
    def test_drain(self):
        options = DummyOptions()
        config = DummyPConfig(options, 'test', '/test', stdout_logfile='/tmp/foo', stderr_logfile='/tmp/bar')
        instance = self._makeOne(config)
        instance.dispatchers = {0: DummyDispatcher(readable=True), 1: DummyDispatcher(writable=True)}
        instance.drain()
    ...
    def test_spawn_fail_make_pipes_emfile(self):
        ...
        result = instance.spawn()
        self.assertEqual(result, None)
        self.assertEqual(instance.spawnerr, "too many open files to spawn 'good'")
        self.assertEqual(options.logger.data[0], "spawnerr: too many open files to spawn 'good'")
        self.assertTrue(instance.delay)
        self.assertTrue(instance.backoff)
        from supervisor.states import ProcessStates
        self.assertEqual(instance.state, ProcessStates.BACKOFF)
    ...
    def test_spawn_fail_make_pipes_other(self):
        ...
        result = instance.spawn()
        self.assertEqual(result, None)
        self.assertEqual(instance.spawnerr, 'unknown error making dispatchers: EPERM')
        self.assertEqual(options.logger.data[0], 'spawnerr: unknown error making dispatchers: EPERM')
        self.assertTrue(instance.delay)
        self.assertTrue(instance.backoff)
        from supervisor.states import ProcessStates
        self.assertEqual(instance.state, ProcessStates.BACKOFF)
    ...
    def test_spawn_fail_make_dispatchers_eisdir(self):
        options = DummyOptions()
        config = DummyPConfig(options, name='cat', command='/bin/cat', stdout_logfile='/a/directory')
        ...
        result = instance.spawn()
        self.assertEqual(result, None)
        self.assertEqual(instance.spawnerr, 'unknown error making dispatchers: EISDIR')
        self.assertEqual(options.logger.data[0], 'spawnerr: unknown error making dispatchers: EISDIR')
        self.assertTrue(instance.delay)
        self.assertTrue(instance.backoff)
        from supervisor.states import ProcessStates
        self.assertEqual(instance.state, ProcessStates.BACKOFF)
    ...
    def test_spawn_fork_fail_eagain(self):
        ...
        result = instance.spawn()
        self.assertEqual(result, None)
        self.assertEqual(instance.spawnerr, "Too many processes in process table to spawn 'good'")
        self.assertEqual(options.logger.data[0], "spawnerr: Too many processes in process table to spawn 'good'")
    ...
    def test_spawn_fork_fail_other(self):
        ...
        result = instance.spawn()
        self.assertEqual(result, None)
        self.assertEqual(instance.spawnerr, 'unknown error during fork: EPERM')
        self.assertEqual(options.logger.data[0], 'spawnerr: unknown error during fork: EPERM')
        self.assertEqual(len(options.parent_pipes_closed), 6)
        self.assertEqual(len(options.child_pipes_closed), 6)
        self.assertTrue(instance.delay)
        self.assertTrue(instance.backoff)
    ...
    def test_spawn_as_child_setuid_ok(self):
        ...
        self.assertEqual(options.written, {2: 'supervisor: child process was not spawned\n'})
        return
    ...
    def test_spawn_as_child_setuid_fail(self):
        ...
        self.assertEqual(len(options.duped), 3)
        self.assertEqual(len(options.fds_closed), options.minfds - 3)
        self.assertEqual(options.written, {2: "supervisor: couldn't setuid to 1: failure reason\nsupervisor: child process was not spawned\n"})
    ...
    def test_spawn_as_child_cwd_ok(self):
        options = DummyOptions()
        options.forkpid = 0
        config = DummyPConfig(options, 'good', '/good/filename', directory='/tmp')
        instance = self._makeOne(config)
        result = instance.spawn()
        self.assertEqual(result, None)
        self.assertEqual(options.parent_pipes_closed, None)
    ...
    def test_spawn_as_child_uses_pconfig_environment(self):
        options = DummyOptions()
        options.forkpid = 0
        config = DummyPConfig(options, 'cat', '/bin/cat', environment={'_TEST_': '1'})
        instance = self._makeOne(config)
        result = instance.spawn()
        self.assertEqual(result, None)
    ...
    def test_spawn_as_child_environment_supervisor_envvars(self):
        ...
        self.assertEqual(result, None)
        self.assertEqual(options.execv_args, ('/bin/cat', ['/bin/cat']))
        self.assertEqual(options.execv_environment['SUPERVISOR_ENABLED'], '1')
        self.assertEqual(options.execv_environment['SUPERVISOR_PROCESS_NAME'], 'cat')
        self.assertEqual(options.execv_environment['SUPERVISOR_GROUP_NAME'], 'dummy')
        self.assertEqual(options.execv_environment['SUPERVISOR_SERVER_URL'], 'http://localhost:9001')
        return
    ...
    def test_spawn_redirect_stderr(self):
        options = DummyOptions()
        options.forkpid = 10
        config = DummyPConfig(options, 'good', '/good/filename', redirect_stderr=True)
        instance = self._makeOne(config)
        result = instance.spawn()
        self.assertEqual(result, 10)
        self.assertEqual(instance.dispatchers[4].__class__, DummyDispatcher)
    ...
    def _dont_test_spawn_and_kill(self):
        ...
        try:
            ...
            data = os.popen('ps').read()
            self.assertEqual(data.find(as_bytes(repr(origpid))), -1)
    ...
    def test_stop(self):
        ...
        self.assertTrue(instance.delay)
        self.assertEqual(options.logger.data[0], 'killing test (pid 11) with signal SIGTERM')
    ...
    def test_stop_not_in_stoppable_state_error(self):
        ...
        try:
            ...
            self.assertEqual(exc.args[0], 'Assertion failed for test: STOPPED not in RUNNING STARTING STOPPING')
    ...
    def test_kill_nopid(self):
        ...
        self.assertEqual(options.logger.data[0], "attempted to kill test with sig SIGTERM but it wasn't running")
        self.assertFalse(instance.killing)
    ...
    def test_kill_error(self):
        ...
        instance.kill(signal.SIGTERM)
        self.assertEqual(options.logger.data[0], 'killing test (pid 11) with signal SIGTERM')
        self.assertTrue(options.logger.data[1].startswith('unknown problem killing test'))
    ...
    def test_kill_from_stopping(self):
        ...
        self.assertEqual(options.logger.data[0], 'killing test (pid 11) with signal SIGKILL')
        self.assertTrue(instance.killing)
    ...
    def test_kill_from_backoff(self):
        ...
        instance.state = ProcessStates.BACKOFF
        instance.kill(signal.SIGKILL)
        self.assertEqual(options.logger.data[0], 'Attempted to kill test, which is in BACKOFF state.')
        self.assertFalse(instance.killing)
        event = L[0]
    ...
    def test_kill_from_stopping_w_killasgroup(self):
        ...
        instance.state = ProcessStates.STOPPING
        instance.kill(signal.SIGKILL)
        self.assertEqual(options.logger.data[0], 'killing test (pid 11) process group with signal SIGKILL')
        self.assertTrue(instance.killing)
        self.assertEqual(options.kills[(-11)], signal.SIGKILL)
        self.assertEqual(L, [])
    ...
    def test_stopasgroup(self):
        ...
        instance.state = ProcessStates.RUNNING
        instance.kill(signal.SIGTERM)
        self.assertEqual(options.logger.data[0], 'killing test (pid 11) process group with signal SIGTERM')
        self.assertTrue(instance.killing)
        self.assertEqual(options.kills[(-11)], signal.SIGTERM)
        self.assertEqual(len(L), 1)
        event = L[0]
    ...
    def test_signal_stopped(self):
        ...
        instance.state = ProcessStates.STOPPED
        instance.signal(signal.SIGWINCH)
        self.assertEqual(options.logger.data[0], "attempted to send test sig SIGWINCH but it wasn't running")
    ...
    def test_signal_error(self):
        ...
        instance.signal(signal.SIGWINCH)
        self.assertEqual(options.logger.data[0], 'sending test (pid 11) sig SIGWINCH')
        self.assertTrue(options.logger.data[1].startswith('unknown problem sending sig test (11)'))
        self.assertTrue('Traceback' in options.logger.data[1])
        self.assertFalse(instance.killing)
        self.assertEqual(len(L), 1)
        event = L[0]
    ...
    def test_finish_stopping_state(self):
        options = DummyOptions()
        config = DummyPConfig(options, 'notthere', '/notthere', stdout_logfile='/tmp/foo')
        instance = self._makeOne(config)
        instance.waitstatus = (123, 1)
        ...
        self.assertEqual(instance.pipes, {})
        self.assertEqual(instance.dispatchers, {})
        self.assertEqual(options.logger.data[0], 'stopped: notthere (terminated by SIGHUP)')
    ...
    def test_finish_running_state_exit_expected(self):
        ...
        self.assertEqual(instance.pipes, {})
        self.assertEqual(instance.dispatchers, {})
        self.assertEqual(options.logger.data[0], 'exited: notthere (terminated by SIGHUP; expected)')
        self.assertEqual(instance.exitstatus, -1)
        self.assertEqual(len(L), 1)
        event = L[0]
    ...
    def test_finish_starting_state_laststart_in_future(self):
        ...
        instance.pipes = pipes
        instance.config.exitcodes = [-1]
        instance.laststart = time.time() + 3600
        from supervisor.states import ProcessStates
        from supervisor import events
        instance.state = ProcessStates.STARTING
        L = []
        ...
        self.assertEqual(instance.pipes, {})
        self.assertEqual(instance.dispatchers, {})
        self.assertEqual(options.logger.data[0], "process 'notthere' (123) laststart time is in the future, don't know how long process was running so assuming it did not exit too quickly")
        self.assertEqual(options.logger.data[1], 'exited: notthere (terminated by SIGHUP; expected)')
        self.assertEqual(instance.exitstatus, -1)
        self.assertEqual(len(L), 2)
        event = L[0]
    ...
    def test_finish_starting_state_exited_too_quickly(self):
        options = DummyOptions()
        config = DummyPConfig(options, 'notthere', '/notthere', stdout_logfile='/tmp/foo', startsecs=10)
        instance = self._makeOne(config)
        instance.config.options.pidhistory[123] = instance
        pipes = {'stdout': '', 'stderr': ''}
        instance.pipes = pipes
        ...
        self.assertEqual(instance.pipes, {})
        self.assertEqual(instance.dispatchers, {})
        self.assertEqual(options.logger.data[0], 'exited: notthere (terminated by SIGHUP; not expected)')
    ...
    def test_finish_with_current_event_sends_rejected(self):
        ...
        self.assertEqual(len(L), 2)
        event1, event2 = L
        self.assertEqual(event1.__class__, events.ProcessStateExitedEvent)
    ...
    def test_cmp_bypriority(self):
        options = DummyOptions()
        config = DummyPConfig(options, 'notthere', '/notthere', stdout_logfile='/tmp/foo', priority=1)
        instance = self._makeOne(config)
        config = DummyPConfig(options, 'notthere1', '/notthere', stdout_logfile='/tmp/foo', priority=2)
        instance1 = self._makeOne(config)
        config = DummyPConfig(options, 'notthere2', '/notthere', stdout_logfile='/tmp/foo', priority=3)
    ...
    def test_transition_starting_to_running(self):
        ...
        self.assertEqual(options.logger.data[0], 'success: process entered RUNNING state, process has stayed up for > than 10 seconds (startsecs)')
        self.assertEqual(len(L), 1)
        event = L[0]
        self.assertEqual(event.__class__, events.ProcessStateRunningEvent)
    ...
    def test_transition_backoff_to_fatal(self):
        ...
        self.assertEqual(process.delay, 0)
        self.assertTrue(process.system_stop)
        self.assertEqual(options.logger.data[0], 'gave up: process entered FATAL state, too many start retries too quickly')
        self.assertEqual(len(L), 1)
        event = L[0]
        self.assertEqual(event.__class__, events.ProcessStateFatalEvent)
    ...
    def test_transition_stops_unkillable(self):
        ...
        self.assertEqual(process.state, ProcessStates.STOPPING)
        self.assertEqual(options.logger.data[0], "killing 'process' (1) with SIGKILL")
...
class FastCGISubprocessTests(unittest.TestCase):
    ...
    def test_prepare_child_fds(self):
        ...
        gconfig = DummyFCGIGroupConfig(options, 'whatever', 999, None, sock_config)
    ...
    def test_finish_override(self):
        ...
        self.assertEqual(sentinel.finish_result, result, 'FastCGISubprocess.finish() did not pass thru result')
        self.assertEqual(1, instance.after_finish.call_count, 'FastCGISubprocess.after_finish() not called once')
        finish_mock = Subprocess.finish
        self.assertEqual(1, finish_mock.call_count, 'Subprocess.finish() not called once')
        pid_arg = finish_mock.call_args[0][1]
        sts_arg = finish_mock.call_args[0][2]
        self.assertEqual(sentinel.pid, pid_arg, 'Subprocess.finish() pid arg was not passed')
        self.assertEqual(sentinel.sts, sts_arg, 'Subprocess.finish() sts arg was not passed')
    ...
    def test_spawn_override_success(self):
        ...
        result = instance.spawn()
        self.assertEqual(sentinel.ppid, result, 'FastCGISubprocess.spawn() did not pass thru result')
        self.assertEqual(1, instance.before_spawn.call_count, 'FastCGISubprocess.before_spawn() not called once')
        spawn_mock = Subprocess.spawn
        self.assertEqual(1, spawn_mock.call_count, 'Subprocess.spawn() not called once')
    ...
    def test_spawn_error(self):
        ...
        result = instance.spawn()
        self.assertEqual(None, result, 'FastCGISubprocess.spawn() did return None on error')
        self.assertEqual(1, instance.before_spawn.call_count, 'FastCGISubprocess.before_spawn() not called once')
        self.assertEqual(None, instance.fcgi_sock, 'FastCGISubprocess.spawn() did not remove sock ref on error')
        return
...
class ProcessGroupBaseTests(unittest.TestCase):
    ...
    def test_stop_all(self):
        ...
        gconfig = DummyPGroupConfig(options, pconfigs=[
         pconfig1, pconfig2, pconfig3, pconfig4])
        group = self._makeOne(gconfig)
        group.processes = {'process1': process1, 'process2': process2, 'process3': process3, 
           'process4': process4}
        group.stop_all()
...
class FastCGIProcessGroupTests(unittest.TestCase):
    ...
    def test___init__with_socket_error(self):
        ...
        self.assertRaises(ValueError, self._makeOne, gconfig, socketManager=DummySocketManager)
        return
...
class EventListenerPoolTests(ProcessGroupBaseTests):
    ...
    def test__eventEnvelope(self):
        ...
        pool = self._makeOne(gconfig)
        from supervisor import events
        result = pool._eventEnvelope(events.EventTypes.PROCESS_COMMUNICATION_STDOUT, 80, 20, 'payload\n')
    ...
    def test_handle_rejected_event_buffer_overflowed(self):
        ...
        self.assertEqual(pool.config.options.logger.data[0], 'pool whatever event buffer overflowed, discarding event a')
    ...
    def test_dispatch_pipe_error(self):
        ...
        self.assertEqual(options.logger.data[0], 'epipe occurred while sending event abc to listener process1, listener state unchanged')
        self.assertEqual(options.logger.data[1], 'rebuffering event abc for pool whatever (bufsize 0)')
