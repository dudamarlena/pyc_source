...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\supervisor\options.py
# Compiled at: 2015-07-18 15:11:49
...
class Options():
    ...
    def default_configfile(self):
        ...
        if config is None and self.require_configfile:
            self.usage('No config file found at default paths (%s); use the -c option to specify a config file at a different path' % (', ').join(self.searchpaths))
        return config
    ...
    def realize(self, args=None, doc=None, progname=None):
        ...
        if self.configfile is None:
            uid = os.getuid()
            if uid == 0 and 'supervisord' in self.progname:
...
class ServerOptions(Options):
    ...
    def __init__(self):
        ...
        self.add('logfile', 'supervisord.logfile', 'l:', 'logfile=', existing_dirpath, default='supervisord.log')
        self.add('logfile_maxbytes', 'supervisord.logfile_maxbytes', 'y:', 'logfile_maxbytes=', byte_size, default=52428800)
        self.add('logfile_backups', 'supervisord.logfile_backups', 'z:', 'logfile_backups=', integer, default=10)
        self.add('loglevel', 'supervisord.loglevel', 'e:', 'loglevel=', logging_level, default='info')
        self.add('pidfile', 'supervisord.pidfile', 'j:', 'pidfile=', existing_dirpath, default='supervisord.pid')
        self.add('identifier', 'supervisord.identifier', 'i:', 'identifier=', str, default='supervisor')
        self.add('childlogdir', 'supervisord.childlogdir', 'q:', 'childlogdir=', existing_directory, default=tempfile.gettempdir())
        ...
        self.add('nocleanup', 'supervisord.nocleanup', 'k', 'nocleanup', flag=1, default=0)
        self.add('strip_ansi', 'supervisord.strip_ansi', 't', 'strip_ansi', flag=1, default=0)
        self.add('profile_options', 'supervisord.profile_options', '', 'profile_options=', profile_options, default=None)
        self.pidhistory = {}
    ...
    def process_groups_from_parser(self, parser):
        ...
        for section in all_sections:
            ...
            groups.append(EventListenerPoolConfig(self, pool_name, priority, processes, buffer_size, pool_events, result_handler))
        ...
        for section in all_sections:
            ...
            try:
                socket_config = self.parse_fcgi_socket(socket, proc_uid, socket_owner, socket_mode)
    ...
    def _processes_from_section(self, parser, section, group_name, klass=None):
        ...
        for process_num in range(numprocs_start, numprocs + numprocs_start):
            ...
            pconfig = klass(self, name=expand(process_name, expansions, 'process_name'), command=command, directory=directory, umask=umask, priority=priority, autostart=autostart, startintervalsecs=startintervalsecs, autorestart=autorestart, startsecs=startsecs, startretries=startretries, uid=uid, stdout_logfile=logfiles['stdout_logfile'], stdout_capture_maxbytes=stdout_cmaxbytes, stdout_events_enabled=stdout_events, stdout_logfile_backups=logfiles['stdout_logfile_backups'], stdout_logfile_maxbytes=logfiles['stdout_logfile_maxbytes'], stdout_syslog=logfiles['stdout_syslog'], stderr_logfile=logfiles['stderr_logfile'], stderr_capture_maxbytes=stderr_cmaxbytes, stderr_events_enabled=stderr_events, stderr_logfile_backups=logfiles['stderr_logfile_backups'], stderr_logfile_maxbytes=logfiles['stderr_logfile_maxbytes'], stderr_syslog=logfiles['stderr_syslog'], stopsignal=stopsignal, stopwaitsecs=stopwaitsecs, stopasgroup=stopasgroup, killasgroup=killasgroup, exitcodes=exitcodes, redirect_stderr=redirect_stderr, environment=environment, serverurl=serverurl)
    ...
    def _parse_username_and_password(self, parser, section):
        ...
        if username is None and password is not None:
            raise ValueError('Must specify username if password is specified in [%s]' % section)
    ...
    def get_autochildlog_name(self, name, identifier, channel):
        ...
        logfile = self.mktempfile(suffix='.log', prefix=prefix, dir=self.childlogdir)
        return logfile

    def clear_autochildlogdir(self):
        childlogdir = self.childlogdir
        fnre = re.compile('.+?---%s-\\S+\\.log\\.{0,1}\\d{0,4}' % self.identifier)
        try:
    ...
    def waitpid(self):
        try:
            ...
            if err not in (errno.ECHILD, errno.EINTR):
                self.logger.critical('waitpid error; a process may not be cleaned up properly')
            if err == errno.EINTR:
    ...
    def set_rlimits(self):
        ...
        if hasattr(resource, 'RLIMIT_NOFILE'):
            limits.append({'msg': 'The minimum number of file descriptors required to run this process is %(min)s as per the "minfds" command-line argument or config file setting. The current environment will only allow you to open %(hard)s file descriptors.  Either raise the number of usable file descriptors in your environment (see README.md) or lower the minfds setting in the config file to allow the process to start.', 
               'min': self.minfds, 
               'resource': resource.RLIMIT_NOFILE, 
               'name': 'RLIMIT_NOFILE'})
        if hasattr(resource, 'RLIMIT_NPROC'):
            limits.append({'msg': 'The minimum number of available processes required to run this program is %(min)s as per the "minprocs" command-line argument or config file setting. The current environment will only allow you to open %(hard)s processes.  Either raise the number of usable processes in your environment (see README.md) or lower the minprocs setting in the config file to allow the program to start.', 
               'min': self.minprocs, 
        ...
        for limit in limits:
            ...
            if soft < min and soft != -1:
                if hard < min and hard != -1:
                    hard = min
                try:
                    resource.setrlimit(res, (min, hard))
                    msgs.append('Increased %(name)s limit to %(min)s' % locals())
    ...
    def make_logger(self, critical_messages, warn_messages, info_messages):
        ...
        loggers.handle_file(self.logger, self.logfile, format, rotating=True, maxbytes=self.logfile_maxbytes, backups=self.logfile_backups)
    ...
    def make_pipes(self, stderr=True):
        ...
        pipes = {'child_stdin': None, 'stdin': None, 
           'stdout': None, 
           'child_stdout': None, 
           'stderr': None, 
           'child_stderr': None}
...
class ClientOptions(Options):
    ...
    def __init__(self):
        ...
        self.plugin_factories = [
         default_factory]
        self.add('interactive', 'supervisorctl.interactive', 'i', 'interactive', flag=1, default=0)
        self.add('prompt', 'supervisorctl.prompt', default='supervisor')
        self.add('serverurl', 'supervisorctl.serverurl', 's:', 'serverurl=', url, default='http://localhost:9001')
    ...
    def read_config(self, fp):
        ...
        self.plugin_factories += self.get_plugins(config, 'supervisor.ctl_factory', 'ctlplugin:')
        return section

    def getServerProxy(self):
        return xmlrpclib.ServerProxy('http://127.0.0.1', transport=xmlrpc.SupervisorTransport(self.username, self.password, self.serverurl))
...
def expand(s, expansions, name):
    try:
        ...
        available.sort()
        raise ValueError('Format string %r for %r contains names (%s) which cannot be expanded. Available names: %s' % (
         s, name, str(ex), (', ').join(available)))
    except Exception as ex:
        raise ValueError('Format string %r for %r is badly formatted: %s' % (
         s, name, str(ex)))
