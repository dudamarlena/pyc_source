...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\supervisor\tests\base.py
# Compiled at: 2015-07-18 11:15:39
_NOW = 1151365354
...
class DummyOptions():
    ...
    def make_logger(self, critical_msgs, warn_msgs, info_msgs):
        self.make_logger_messages = (
         critical_msgs, warn_msgs, info_msgs)
...
class DummyPConfig():
    ...
    def __init__(self, options, name, command, directory=None, umask=None, priority=999, autostart=True, startintervalsecs=None, autorestart=True, startsecs=10, startretries=999, uid=None, stdout_logfile=None, stdout_capture_maxbytes=0, stdout_events_enabled=False, stdout_logfile_backups=0, stdout_logfile_maxbytes=0, stderr_logfile=None, stderr_capture_maxbytes=0, stderr_events_enabled=False, stderr_logfile_backups=0, stderr_logfile_maxbytes=0, redirect_stderr=False, stopsignal=None, stopwaitsecs=10, stopasgroup=False, killasgroup=False, exitcodes=(0, 2), environment=None, serverurl=None):
        self.options = options
        self.name = name
        self.command = command
        self.priority = priority
        self.autostart = autostart
        self.startintervalsecs = startintervalsecs
...
class DummySupervisorRPCNamespace():
    ...
    def startProcessGroup(self, name):
        ...
        if name == 'BAD_NAME':
            raise Fault(xmlrpc.Faults.BAD_NAME, 'BAD_NAME')
        return [
         {'name': 'foo_00', 'group': 'foo', 'status': xmlrpc.Faults.SUCCESS, 
            'description': 'OK'},
         {'name': 'foo_01', 'group': 'foo', 'status': xmlrpc.Faults.SUCCESS, 
            'description': 'OK'}]

    def startAllProcesses(self):
        from supervisor import xmlrpc
        return [
         {'name': 'foo', 'group': 'foo', 'status': xmlrpc.Faults.SUCCESS, 
            'description': 'OK'},
         {'name': 'foo2', 'group': 'foo2', 'status': xmlrpc.Faults.SUCCESS, 
            'description': 'OK'},
         {'name': 'failed', 'group': 'failed_group', 'status': xmlrpc.Faults.SPAWN_ERROR, 
    ...
    def stopAllProcesses(self):
        ...
        return [
         ...
         {'name': 'foo2', 'group': 'foo2', 'status': xmlrpc.Faults.SUCCESS, 
            'description': 'OK'},
         {'name': 'failed', 'group': 'failed_group', 'status': xmlrpc.Faults.BAD_NAME, 
            'description': 'FAILED'}]
