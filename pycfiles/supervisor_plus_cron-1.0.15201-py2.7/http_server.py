...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\supervisor\medusa\http_server.py
# Compiled at: 2015-07-18 10:13:56
RCS_ID = '$Id: http_server.py,v 1.12 2004/04/21 15:11:44 akuchling Exp $'
import re, socket, sys, time, supervisor.medusa.asyncore_25 as asyncore, supervisor.medusa.asynchat_25 as asynchat, supervisor.medusa.http_date as http_date, supervisor.medusa.producers as producers, supervisor.medusa.logger as logger
...
class http_request():
    ...
    def remove_header(self, name, value=None):
        ...
        with that name."""
        found_it = 0
        if name in self.reply_headers and (value is None or self.reply_headers[name] == value):
            del self.reply_headers[name]
            found_it = 1
        removed_headers = []
        if value is not None:
            if (
             name, value) in self.__reply_header_list:
                removed_headers = [
                 (
                  name, value)]
                found_it = 1
...
class http_server(asyncore.dispatcher):
    ...
    def status(self):
        ...
        return producers.composite_producer([
         producers.lines_producer(['<h2>%s</h2>' % self.SERVER_IDENT, '<br>Listening on: <b>Host:</b> %s' % self.server_name, '<b>Port:</b> %d' % self.port, '<p><ul><li>Total <b>Clients:</b> %s' % self.total_clients, '<b>Requests:</b> %s' % self.total_requests, '<b>Requests/Client:</b> %.1f' % ratio, '<li>Total <b>Bytes In:</b> %s' % nice_bytes(self.bytes_in.as_long()), '<b>Bytes Out:</b> %s' % nice_bytes(self.bytes_out.as_long()), '<li>Total <b>Exceptions:</b> %s' % self.exceptions, '</ul><p><b>Extension List</b><ul>'])] + handler_stats + [producers.simple_producer('</ul>')])
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        import supervisor.medusa.monitor as monitor, supervisor.medusa.filesys as filesys, supervisor.medusa.default_handler as default_handler, supervisor.medusa.ftp_server as ftp_server, supervisor.medusa.chat_server as chat_server, supervisor.medusa.resolver as resolver
