...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\supervisor\rpcinterface.py
    ...
    def getAllConfigInfo(self):
        ...
        for gconfig in self.supervisord.options.process_group_configs:
            ...
            for pconfig in gconfig.process_configs:
                configinfo.append({'name': pconfig.name, 'group': gconfig.name, 
                   'inuse': inuse, 
    ...
    def clearAllProcessLogs(self):
        ...
        def clearall():
            ...
            try:
                ...
                results.append({'name': process.config.name, 'group': group.config.name, 
                   'status': e.code, 
                   'description': e.text})
            else:
                results.append({'name': process.config.name, 'group': group.config.name, 
...
def make_allfunc(processes, predicate, func, **extra_kwargs):
    ...
    def allfunc(processes=processes, predicate=predicate, func=func, extra_kwargs=extra_kwargs, callbacks=callbacks, results=results):
        if not callbacks:
