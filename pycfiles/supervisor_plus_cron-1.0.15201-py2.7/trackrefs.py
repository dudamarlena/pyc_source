...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\supervisor\tests\trackrefs.py
...
class TrackRefs(object):
    ...
    def update(self):
        ...
        ct = [ (type_or_class_title(t), type2count[t] - self.type2count.get(t, 0), type2all[t] - self.type2all.get(t, 0)) for t in type2count.keys()
             ]
        ct += [ (type_or_class_title(t), -self.type2count[t], -self.type2all[t]) for t in self.type2count.keys() if t not in type2count
              ]
        ct.sort()
        self.delta = ct
    ...
    def detailed_refcounts(self, rc, prev):
        ...
        print '  sum detail refcount=%-8d sys refcount=%-8d change=%-6d' % (
         self.n, rc, rc - prev)
        self.output()
