...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\supervisor\tests\test_http.py
...
class Test_deferring_http_request(unittest.TestCase):
    ...
    def test_done_http_10_keepalive_no_content_length(self):
        channel = self._makeChannel()
        inst = self._makeOne(channel=channel, version='1.0', header=[
    ...
    def test_done_http_11_connection_close(self):
        channel = self._makeChannel()
        inst = self._makeOne(channel=channel, version='1.1', header=[
         'Connection: close'])
        inst.done()
        self.assertTrue(channel.closed)
    ...
    def test_done_http_11_unknown_transfer_encoding(self):
        channel = self._makeChannel()
        inst = self._makeOne(channel=channel, version='1.1')
    ...
    def test_done_http_09(self):
        channel = self._makeChannel()
        inst = self._makeOne(channel=channel, version=None)
        inst.done()
        self.assertTrue(channel.closed)
        return
...
class TopLevelFunctionTests(unittest.TestCase):
    ...
    def test_make_http_servers_socket_type_error(self):
        config = {'family': 999, 'host': 'localhost', 'port': 17735, 'username': None, 
