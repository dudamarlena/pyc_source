...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\supervisor\tests\test_loggers.py
# Compiled at: 2015-07-18 10:13:57
import errno, sys, unittest, tempfile, shutil, os, syslog
...
class HandlerTests:
    ...
    def _makeLogRecord(self, msg):
        from supervisor import loggers
        record = loggers.LogRecord(level=loggers.LevelsByName.INFO, msg=msg, exc_info=None)
        return record
...
class BareHandlerTests(HandlerTests, unittest.TestCase):
    ...
    def test_flush_stream_flush_raises_IOError_not_EPIPE(self):
        ...
        inst = self._makeOne(stream=stream)
        self.assertRaises(IOError, inst.flush)
...
class RotatingFileHandlerTests(FileHandlerTests):
    ...
    def test_ctor(self):
        handler = self._makeOne(self.filename)
        self.assertEqual(handler.mode, 'a')
        self.assertEqual(handler.maxBytes, 536870912)
    ...
    def test_emit_does_rollover(self):
        ...
        record = self._makeLogRecord('aaaa')
        handler.emit(record)
        ...
        with open(self.filename + '.1', 'r') as (f):
            self.assertEqual(f.read(), 'aaaaaaaaaaaa')
...
class SyslogHandlerTests(HandlerTests, unittest.TestCase):
    ...
    if PY3:
        ...
        def test_emit_unicode_witherror(self):
            ...
            record = self._makeLogRecord(as_string('fií'))
            handler.emit(record)
            self.assertEqual(called, [as_string('fií')])
