...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\supervisor\poller.py
...
class SelectPoller(BasePoller):
    ...
    def poll(self, timeout):
        try:
            r, w, x = self._select.select(self.readables, self.writables, [], timeout)
...
class KQueuePoller(BasePoller):
    ...
    def unregister(self, fd):
        kevent = select.kevent(fd, filter=select.KQ_FILTER_READ | select.KQ_FILTER_WRITE, flags=select.KQ_EV_DELETE)
        self._forget_fd(fd)
        self._kqueue_control(fd, kevent)
