...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\supervisor\tests\test_options.py
# Compiled at: 2015-07-18 10:13:57
"""Test suite for supervisor.options"""
import os, sys, tempfile, socket, unittest, signal, shutil, errno
...
class OptionTests(unittest.TestCase):
    ...
    def _makeOptions(self, read_error=False):
        ...
        options.add(name='anoption', confname='anoption', short='o', long='option', default='default')
        options.add(name='other', confname='other', env='OTHER', short='p:', long='other=', handler=integer)
        return options
    ...
    def test_add_flag_not_None_long_false_short_false(self):
        cls = self._getTargetClass()
        inst = cls()
        self.assertRaises(ValueError, inst.add, flag=True, long=False, short=False)

    def test_add_flag_not_None_short_endswith_colon(self):
        cls = self._getTargetClass()
        inst = cls()
        self.assertRaises(ValueError, inst.add, flag=True, long=False, short=':')

    def test_add_flag_not_None_long_endswith_equal(self):
        cls = self._getTargetClass()
        inst = cls()
        self.assertRaises(ValueError, inst.add, flag=True, long='=', short=False)

    def test_add_inconsistent_short_long_options(self):
        cls = self._getTargetClass()
        inst = cls()
        self.assertRaises(ValueError, inst.add, long='=', short='abc')

    def test_add_short_option_startswith_dash(self):
        cls = self._getTargetClass()
        inst = cls()
        self.assertRaises(ValueError, inst.add, long=False, short='-abc')
    ...
    def test_add_duplicate_short_option_key(self):
        ...
        self.assertRaises(ValueError, inst.add, long=False, short='a')

    def test_add_long_option_startswith_dash(self):
        cls = self._getTargetClass()
        inst = cls()
        self.assertRaises(ValueError, inst.add, long='-abc', short=False)
...
class ServerOptionsTests(unittest.TestCase):
    ...
    def test_realize_getopt_error(self):
        ...
        self.assertEqual(len(recorder), 1)
        self.assertEqual(recorder[0], "GetoptError('option --bad not recognized', 'bad')")
    ...
    def test_options_afunix_password_without_username(self):
        ...
        try:
            ...
            self.assertEqual(exc.args[0], 'Must specify username if password is specified in [unix_http_server]')
    ...
    def test_options_afinet_password_without_username(self):
        ...
        try:
            ...
            self.assertEqual(exc.args[0], 'Must specify username if password is specified in [inet_http_server]')
    ...
    def test_cleanup_afunix_ignores_oserror_enoent(self):
        ...
        try:
            ...
            instance.httpservers = [({'family': socket.AF_UNIX, 'file': notfound}, Server()), ({'family': socket.AF_UNIX, 'file': socketname}, Server())]
            instance.pidfile = ''
            instance.cleanup()
    ...
    def test_processes_from_section(self):
        ...
        self.assertEqual(pconfig.environment, {'KEY1': 'val1', 'KEY2': 'val2', 'KEY3': '0'})
        return
    ...
    def test_processes_from_section_shows_conf_filename_on_valueerror(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w+') as (f):
            try:
                ...
                self.assertEqual(e.args[0], "program section program:foo does not specify a command in section 'program:foo' (file: %s)" % f.name)
            else:
                self.fail('nothing raised')
...
class UnhosedConfigParserTests(unittest.TestCase):
    ...
    def test_saneget_with_default_no_expand(self):
        ...
        parser.expansions = {'pet': 'dog'}
        parser.read_string('[supervisord]\n')
        result = parser.saneget('supervisord', 'foo', default='%(pet)s', do_expand=False)
