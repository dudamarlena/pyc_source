...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\supervisor\tests\test_dispatchers.py
...
class POutputDispatcherTests(unittest.TestCase):
    ...
    def _makeOne(self, process, channel='stdout'):
        ...
        events = {'stdout': events.ProcessCommunicationStdoutEvent, 'stderr': events.ProcessCommunicationStderrEvent}
    ...
    def test_handle_error(self):
        ...
        self.assertTrue(result.startswith('uncaptured python exception, closing channel'), result)

    def test_toggle_capturemode_sends_event(self):
        options = DummyOptions()
        config = DummyPConfig(options, 'process1', '/bin/process1', stdout_logfile='/tmp/foo', stdout_capture_maxbytes=500)
        process = DummyProcess(config)
        process.pid = 4000
        dispatcher = self._makeOne(process)
    ...
    def test_record_output_emits_stdout_event_when_enabled(self):
        options = DummyOptions()
        config = DummyPConfig(options, 'process1', '/bin/process1', stdout_events_enabled=True)
        process = DummyProcess(config)
    ...
    def test_record_output_does_not_emit_stdout_event_when_disabled(self):
        options = DummyOptions()
        config = DummyPConfig(options, 'process1', '/bin/process1', stdout_events_enabled=False)
        process = DummyProcess(config)
        dispatcher = self._makeOne(process, 'stdout')
        dispatcher.output_buffer = 'hello from stdout'
        L = []
    ...
    def test_record_output_emits_stderr_event_when_enabled(self):
        options = DummyOptions()
        config = DummyPConfig(options, 'process1', '/bin/process1', stderr_events_enabled=True)
        process = DummyProcess(config)
    ...
    def test_record_output_does_not_emit_stderr_event_when_disabled(self):
        options = DummyOptions()
        config = DummyPConfig(options, 'process1', '/bin/process1', stderr_events_enabled=False)
        process = DummyProcess(config)
        dispatcher = self._makeOne(process, 'stderr')
        dispatcher.output_buffer = 'hello from stderr'
        L = []
    ...
    def test_record_output_capturemode_string_longer_than_token(self):
        ...
        from supervisor import loggers
        options.loglevel = loggers.LevelsByName.TRAC
        config = DummyPConfig(options, 'process1', '/bin/process1', stdout_logfile='/tmp/foo', stdout_capture_maxbytes=100)
        process = DummyProcess(config)
        dispatcher = self._makeOne(process)
    ...
    def test_stdout_capturemode_single_buffer(self):
        ...
        logfile = '/tmp/log'
        config = DummyPConfig(options, 'process1', '/bin/process1', stdout_logfile=logfile, stdout_capture_maxbytes=1000)
        process = DummyProcess(config)
        dispatcher = self._makeOne(process)
        try:
    ...
    def test_stdout_capturemode_multiple_buffers(self):
        ...
        options.getLogger = getLogger
        logfile = '/tmp/log'
        config = DummyPConfig(options, 'process1', '/bin/process1', stdout_logfile=logfile, stdout_capture_maxbytes=10000)
        process = DummyProcess(config)
        dispatcher = self._makeOne(process)
        try:
    ...
    def test_ctor_capturelog_only(self):
        options = DummyOptions()
        config = DummyPConfig(options, 'process1', '/bin/process1', stdout_capture_maxbytes=300)
        process = DummyProcess(config)
        dispatcher = self._makeOne(process)
        self.assertEqual(dispatcher.process, process)
        self.assertEqual(dispatcher.channel, 'stdout')
    ...
    def test_repr(self):
        ...
        self.assertNotEqual(drepr.find('supervisor.tests.base.DummyProcess'), -1)
...
class PInputDispatcherTests(unittest.TestCase):
    ...
    def test_handle_write_event_epipe_raised(self):
        ...
        dispatcher.handle_write_event()
        self.assertEqual(dispatcher.input_buffer, '')
        self.assertTrue(options.logger.data[0].startswith('fd 0 closed, stopped monitoring'))
...
class PEventListenerDispatcherTests(unittest.TestCase):
    ...
    def test_handle_read_event_nodata(self):
        ...
        self.assertEqual(dispatcher.state_buffer, '')
        from supervisor.dispatchers import EventListenerStates
        self.assertEqual(dispatcher.process.listener_state, EventListenerStates.ACKNOWLEDGED)
        return
    ...
    def test_handle_listener_state_change_acknowledged_gobbles(self):
        ...
        self.assertEqual(dispatcher.state_buffer, '')
        self.assertEqual(options.logger.data[0], 'process1: ACKNOWLEDGED -> READY')
        self.assertEqual(options.logger.data[1], 'process1: READY -> UNKNOWN')
        self.assertEqual(process.listener_state, EventListenerStates.UNKNOWN)
        return
    ...
    def test_handle_listener_state_change_acknowledged_to_insufficient(self):
        ...
        self.assertEqual(options.logger.data, [])
        self.assertEqual(process.listener_state, EventListenerStates.ACKNOWLEDGED)
        return
    ...
    def test_handle_listener_state_change_acknowledged_to_unknown(self):
        ...
        self.assertEqual(options.logger.data[1], 'process1: has entered the UNKNOWN state and will no longer receive events, this usually indicates the process violated the eventlistener protocol')
        self.assertEqual(process.listener_state, EventListenerStates.UNKNOWN)
        return
    ...
    def test_handle_listener_state_change_ready_to_unknown(self):
        ...
        self.assertEqual(dispatcher.handle_listener_state_change(), None)
        self.assertEqual(dispatcher.state_buffer, '')
        self.assertEqual(options.logger.data[0], 'process1: READY -> UNKNOWN')
        self.assertEqual(options.logger.data[1], 'process1: has entered the UNKNOWN state and will no longer receive events, this usually indicates the process violated the eventlistener protocol')
        self.assertEqual(process.listener_state, EventListenerStates.UNKNOWN)
        return
    ...
    def test_handle_listener_state_change_busy_to_acknowledged_procd(self):
        ...
        self.assertEqual(options.logger.data[0], 'process1: event was processed')
        self.assertEqual(options.logger.data[1], 'process1: BUSY -> ACKNOWLEDGED')
        self.assertEqual(process.listener_state, EventListenerStates.ACKNOWLEDGED)
        return
    ...
    def test_handle_listener_state_change_busy_to_acknowledged_rejected(self):
        ...
        self.assertEqual(dispatcher.handle_listener_state_change(), None)
        self.assertEqual(dispatcher.state_buffer, 'abc')
        self.assertEqual(options.logger.data[0], 'process1: event was rejected')
        self.assertEqual(options.logger.data[1], 'process1: BUSY -> ACKNOWLEDGED')
        self.assertEqual(process.listener_state, EventListenerStates.ACKNOWLEDGED)
        return
    ...
    def test_handle_listener_state_change_busy_to_unknown(self):
        ...
        self.assertEqual(dispatcher.handle_listener_state_change(), None)
        self.assertEqual(dispatcher.state_buffer, '')
        self.assertEqual(options.logger.data[0], "process1: bad result line: 'bogus data'")
        self.assertEqual(options.logger.data[1], 'process1: BUSY -> UNKNOWN')
        self.assertEqual(options.logger.data[2], 'process1: has entered the UNKNOWN state and will no longer receive events, this usually indicates the process violated the eventlistener protocol')
        self.assertEqual(process.listener_state, EventListenerStates.UNKNOWN)
    ...
    def test_handle_listener_state_busy_gobbles(self):
        ...
        self.assertEqual(options.logger.data[0], 'process1: event was processed')
        self.assertEqual(options.logger.data[1], 'process1: BUSY -> ACKNOWLEDGED')
        self.assertEqual(options.logger.data[2], 'process1: ACKNOWLEDGED -> UNKNOWN')
        self.assertEqual(options.logger.data[3], 'process1: has entered the UNKNOWN state and will no longer receive events, this usually indicates the process violated the eventlistener protocol')
        self.assertEqual(process.listener_state, EventListenerStates.UNKNOWN)
        return
    ...
    def test_handle_result_exception(self):
        ...
        self.assertEqual(L[0].__class__, events.EventRejectedEvent)
        self.assertEqual(process.listener_state, EventListenerStates.UNKNOWN)
        self.assertEqual(options.logger.data[0], 'process1: event caused an error')
        self.assertEqual(options.logger.data[1], 'process1: BUSY -> UNKNOWN')
        self.assertEqual(options.logger.data[2], 'process1: has entered the UNKNOWN state and will no longer receive events, this usually indicates the process violated the eventlistener protocol')
