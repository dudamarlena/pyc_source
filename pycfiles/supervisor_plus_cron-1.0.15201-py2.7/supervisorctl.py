...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\supervisor\supervisorctl.py
...
actions.
"""
import cmd, sys, getpass, socket, errno, threading
...
class Controller(cmd.Cmd):
    ...
    def upcheck(self):
        try:
            ...
            api = supervisor.getVersion()
            from supervisor import rpcinterface
            if api != rpcinterface.API_VERSION:
                self.output('Sorry, this version of supervisorctl expects to talk to a server with API version %s, but the remote version is %s.' % (
                 rpcinterface.API_VERSION, api))
                return False
        except xmlrpclib.Fault as e:
            if e.faultCode == xmlrpc.Faults.UNKNOWN_METHOD:
                self.output('Sorry, supervisord responded but did not recognize the supervisor namespace commands that supervisorctl uses to control it.  Please check that the [rpcinterface:supervisor] section is enabled in the configuration file (see sample.conf).')
                return False
            raise
    ...
    def complete(self, text, state, line=None):
        ...
        if not phrase.strip():
            ...
            elif action in ('clear', 'fg', 'pid', 'restart', 'signal', 'start', 'status',
                            'stop', 'tail'):
            ...
            if len(args) == 1:
                ...
                try:
                    if channel is 'stdout':
                        output = supervisor.readProcessStdoutLog(name, -bytes, 0)
                    else:
                        output = supervisor.readProcessStderrLog(name, -bytes, 0)
                except xmlrpclib.Fault as e:
                    template = '%s: ERROR (%s)'
    ...
    def do_maintail(self, arg):
        ...
        try:
            ...
            if e.faultCode == xmlrpc.Faults.NO_FILE:
                ...
                self.ctl.output(template % ('supervisord', 'unknown error reading log'))
            else:
                raise
        else:
            self.ctl.output(output)
                ...
                for name in names:
                    ...
                    if process_name is None:
                        ...
                        try:
                            ...
                            error = self._startresult({'status': e.faultCode, 'name': process_name, 
                               'group': group_name, 
                ...
                for name in names:
                    ...
                    if process_name is None:
                        ...
                        try:
                            ...
                            error = self._stopresult({'status': e.faultCode, 'name': process_name, 
                               'group': group_name, 
                               'description': e.faultString})
                            self.ctl.output(error)
                        else:
                ...
                for name in names:
                    ...
                    if process_name is None:
                        ...
                        try:
                            ...
                            error = self._signalresult({'status': e.faultCode, 'name': process_name, 
                               'group': group_name, 
                               'description': e.faultString})
                            self.ctl.output(error)
                        else:
        ...
        if 'all' in names:
            ...
            for name in names:
                ...
                try:
                    ...
                    error = self._clearresult({'status': e.faultCode, 'name': process_name, 
                       'group': group_name, 
                       'description': e.faultString})
                    self.ctl.output(error)
                else:
