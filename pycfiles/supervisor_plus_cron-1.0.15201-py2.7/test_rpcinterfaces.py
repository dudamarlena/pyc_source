...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\supervisor\tests\test_rpcinterfaces.py
...
class SupervisorNamespaceXMLRPCInterfaceTests(TestBase):
    ...
    def test_readLog_unreadable(self):
        ...
        supervisord = DummySupervisor()
        interface = self._makeOne(supervisord)
        self._assertRPCError(xmlrpc.Faults.NO_FILE, interface.readLog, offset=0, length=1)
    ...
    def test_readLog_badargs(self):
        ...
        try:
            ...
            with open(logfile, 'w+') as (f):
                f.write('x' * 2048)
            self._assertRPCError(xmlrpc.Faults.BAD_ARGUMENTS, interface.readLog, offset=-1, length=1)
            self._assertRPCError(xmlrpc.Faults.BAD_ARGUMENTS, interface.readLog, offset=-1, length=-1)
        finally:
            os.remove(logfile)
    ...
    def test_startProcessGroup(self):
        ...
        self.assertEqual(callback(), [
         {'group': 'foo', 'status': Faults.SUCCESS, 
            'description': 'OK', 
            'name': 'process1'},
         {'group': 'foo', 'status': Faults.SUCCESS, 
            'description': 'OK', 
    ...
    def test_stopProcessGroup(self):
        ...
        self.assertEqual(value, [
         {'status': Faults.SUCCESS, 'group': 'foo', 
            'name': 'process1', 
            'description': 'OK'},
         {'status': Faults.SUCCESS, 'group': 'foo', 
            'name': 'process2', 
    ...
    def test_stopProcessGroup_nowait(self):
        ...
        from supervisor.xmlrpc import Faults
        self.assertEqual(callback(), [
         {'name': 'process1', 'description': 'OK', 
            'group': 'foo', 
            'status': Faults.SUCCESS},
         {'name': 'process2', 'description': 'OK', 
            'group': 'foo', 
            'status': Faults.SUCCESS}])
    ...
    def test_getAllConfigInfo(self):
        ...
        self.assertEqual(configs, [
         {'group': 'group1', 'name': 'process1', 
            'inuse': True, 
            'autostart': True, 
            'process_prio': 999, 
            'group_prio': 999},
         {'group': 'group1', 'name': 'process2', 
            'inuse': True, 
            'autostart': True, 
            'process_prio': 999, 
            'group_prio': 999}])
    ...
    def test_getAllProcessInfo(self):
        from supervisor.process import ProcessStates
        options = DummyOptions()
        p1config = DummyPConfig(options, 'process1', '/bin/process1', priority=1, stdout_logfile='/tmp/process1.log')
        p2config = DummyPConfig(options, 'process2', '/bin/process2', priority=2, stdout_logfile='/tmp/process2.log')
    ...
    def test_readProcessStdoutLog_unreadable(self):
        ...
        interface = self._makeOne(supervisord)
        self._assertRPCError(xmlrpc.Faults.NO_FILE, interface.readProcessStdoutLog, 'process1', offset=0, length=1)
    ...
    def test_readProcessStdoutLog_badargs(self):
        ...
        try:
            with open(logfile, 'w+') as (f):
                f.write('x' * 2048)
            self._assertRPCError(xmlrpc.Faults.BAD_ARGUMENTS, interface.readProcessStdoutLog, 'process1', offset=-1, length=1)
            self._assertRPCError(xmlrpc.Faults.BAD_ARGUMENTS, interface.readProcessStdoutLog, 'process1', offset=-1, length=-1)
        finally:
            os.remove(logfile)
    ...
    def test_readProcessStdoutLog_bad_name_no_process(self):
        ...
        supervisord = PopulatedDummySupervisor(options, 'foo', pconfig)
        interface = self._makeOne(supervisord)
        self._assertRPCError(xmlrpc.Faults.BAD_NAME, interface.readProcessStdoutLog, 'foo:*', offset=0, length=1)
    ...
    def test_readProcessStderrLog_unreadable(self):
        ...
        supervisord = PopulatedDummySupervisor(options, 'process1', pconfig)
        interface = self._makeOne(supervisord)
        self._assertRPCError(xmlrpc.Faults.NO_FILE, interface.readProcessStderrLog, 'process1', offset=0, length=1)
    ...
    def test_readProcessStderrLog_badargs(self):
        ...
        try:
            with open(logfile, 'w+') as (f):
                f.write('x' * 2048)
            self._assertRPCError(xmlrpc.Faults.BAD_ARGUMENTS, interface.readProcessStderrLog, 'process1', offset=-1, length=1)
            self._assertRPCError(xmlrpc.Faults.BAD_ARGUMENTS, interface.readProcessStderrLog, 'process1', offset=-1, length=-1)
        finally:
            os.remove(logfile)
    ...
    def test_readProcessStderrLog_bad_name_no_process(self):
        ...
        supervisord = PopulatedDummySupervisor(options, 'foo', pconfig)
        interface = self._makeOne(supervisord)
        self._assertRPCError(xmlrpc.Faults.BAD_NAME, interface.readProcessStderrLog, 'foo:*', offset=0, length=1)
    ...
    def test_tailProcessStdoutLog_all(self):
        ...
        try:
            with open(logfile, 'w+') as (f):
                f.write(letters)
            data, offset, overflow = interface.tailProcessStdoutLog('foo', offset=0, length=len(letters))
    ...
    def test_tailProcessStdoutLog_none(self):
        ...
        try:
            with open(logfile, 'w+') as (f):
                f.write(letters)
            data, offset, overflow = interface.tailProcessStdoutLog('foo', offset=len(letters), length=100)
            self.assertEqual(interface.update_text, 'tailProcessStdoutLog')
            self.assertEqual(overflow, False)
            self.assertEqual(offset, len(letters))
            self.assertEqual(data, '')
            data, offset, overflow = interface.tailProcessStdoutLog('foo', offset=len(letters) + 5, length=100)
            self.assertEqual(interface.update_text, 'tailProcessStdoutLog')
            self.assertEqual(overflow, False)
            self.assertEqual(offset, len(letters))
            self.assertEqual(data, '')
    ...
    def test_tailProcessStdoutLog_overflow(self):
        ...
        try:
            with open(logfile, 'w+') as (f):
                f.write(letters)
            data, offset, overflow = interface.tailProcessStdoutLog('foo', offset=0, length=5)
            self.assertEqual(interface.update_text, 'tailProcessStdoutLog')
    ...
    def test_tailProcessStdoutLog_unreadable(self):
        ...
        supervisord = PopulatedDummySupervisor(options, 'foo', pconfig)
        interface = self._makeOne(supervisord)
        data, offset, overflow = interface.tailProcessStdoutLog('foo', offset=0, length=100)
        self.assertEqual(interface.update_text, 'tailProcessStdoutLog')
        self.assertEqual(overflow, False)
    ...
    def test_tailProcessStderrLog_all(self):
        ...
        try:
            with open(logfile, 'w+') as (f):
                f.write(letters)
            data, offset, overflow = interface.tailProcessStderrLog('foo', offset=0, length=len(letters))
    ...
    def test_tailProcessStderrLog_none(self):
        ...
        try:
            with open(logfile, 'w+') as (f):
                f.write(letters)
            data, offset, overflow = interface.tailProcessStderrLog('foo', offset=len(letters), length=100)
            self.assertEqual(interface.update_text, 'tailProcessStderrLog')
            self.assertEqual(overflow, False)
            self.assertEqual(offset, len(letters))
            self.assertEqual(data, '')
            data, offset, overflow = interface.tailProcessStderrLog('foo', offset=len(letters) + 5, length=100)
            self.assertEqual(interface.update_text, 'tailProcessStderrLog')
            self.assertEqual(overflow, False)
            self.assertEqual(offset, len(letters))
            self.assertEqual(data, '')
    ...
    def test_tailProcessStderrLog_overflow(self):
        ...
        try:
            with open(logfile, 'w+') as (f):
                f.write(letters)
            data, offset, overflow = interface.tailProcessStderrLog('foo', offset=0, length=5)
            self.assertEqual(interface.update_text, 'tailProcessStderrLog')
            self.assertEqual(overflow, True)
            self.assertEqual(offset, len(letters))
            self.assertEqual(data, letters[-5:])
    ...
    def test_tailProcessStderrLog_unreadable(self):
        ...
        supervisord = PopulatedDummySupervisor(options, 'foo', pconfig)
        interface = self._makeOne(supervisord)
        data, offset, overflow = interface.tailProcessStderrLog('foo', offset=0, length=100)
        self.assertEqual(interface.update_text, 'tailProcessStderrLog')
        self.assertEqual(overflow, False)
        self.assertEqual(offset, 0)
        self.assertEqual(data, '')
    ...
    def test_clearAllProcessLogs(self):
        ...
        results = callback()
        from supervisor import xmlrpc
        self.assertEqual(results[0], {'name': 'process1', 'group': 'foo', 
           'status': xmlrpc.Faults.SUCCESS, 
           'description': 'OK'})
        self.assertEqual(results[1], {'name': 'process2', 'group': 'foo', 
           'status': xmlrpc.Faults.SUCCESS, 
           'description': 'OK'})
        process1 = supervisord.process_groups['foo'].processes['process1']
    ...
    def test_sendProcessStdin_unicode_encoded_to_utf8(self):
        ...
        supervisord.set_procattr('process1', 'pid', 42)
        interface = self._makeOne(supervisord)
        interface.sendProcessStdin('process1', as_string('fií'))
        process1 = supervisord.process_groups['process1'].processes['process1']
        self.assertEqual(process1.stdin_buffer, 'fií')
...
class SystemNamespaceXMLRPCInterfaceTests(TestBase):
    ...
    def test_multicall_simplevals(self):
        interface = self._makeOne()
        callback = interface.multicall([{'methodName': 'system.methodHelp', 'params': ['system.methodHelp']}, {'methodName': 'system.listMethods', 'params': []}])
        from supervisor import http
    ...
    def test_multicall_recursion_guard(self):
        from supervisor import xmlrpc
        interface = self._makeOne()
        callback = interface.multicall([{'methodName': 'system.multicall', 'params': []}])
        from supervisor import http
        result = http.NOT_DONE_YET
        while result is http.NOT_DONE_YET:
            result = callback()
    ...
    def test_multicall_nested_callback(self):
        interface = self._makeOne()
        callback = interface.multicall([{'methodName': 'supervisor.stopAllProcesses'}])
        from supervisor import http
        result = http.NOT_DONE_YET
        while result is http.NOT_DONE_YET:
            result = callback()
...
class Test_make_allfunc(unittest.TestCase):
    ...
    def test_rpcerror_nocallbacks(self):
        ...
        result = af()
        self.assertEqual(result, [
         {'description': 'FAILED', 'group': 'process1', 
            'name': 'process1', 
            'status': xmlrpc.Faults.FAILED}])
