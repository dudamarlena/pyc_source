...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\supervisor\web.py
# Compiled at: 2015-07-18 10:13:57
import os, re, time, traceback, datetime, meld3
    ...
    def make_callback(self, namespec, action):
        ...
        if action:
            ...
            if action == 'restartall':
                callback = rpcinterface.system.multicall([{'methodName': 'supervisor.stopAllProcesses'}, {'methodName': 'supervisor.startAllProcesses'}])
            ...
            if namespec:
                ...
                if action == 'restart':
                    callback = rpcinterface.system.multicall([
                     {'methodName': 'supervisor.stopProcess', 'params': [
                                 namespec]},
                     {'methodName': 'supervisor.startProcess', 'params': [
                                 namespec]}])
...
class supervisor_ui_handler():
    ...
    def continue_request(self, data, request):
        ...
        if viewinfo is None:
            ...
            context = ViewContext(template=viewtemplate, request=request, form=form, response=response, supervisord=self.supervisord)
