...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\supervisor\tests\test_poller.py
# Compiled at: 2015-07-18 10:13:57
import sys, unittest, errno, select
...
class SelectPollerTests(unittest.TestCase):
    ...
    def test_poll_returns_readables_and_writables(self):
        _select = DummySelect(result={'readables': [6], 'writables': [
                       8]})
        poller = self._makeOne(DummyOptions())
...
class KQueuePollerTests(KQueuePollerTestsBase):
    ...
    def test_register_readable_and_writable_ignores_ebadf(self):
        ...
        poller.register_readable(6)
        poller.register_writable(7)
        self.assertEqual(options.logger.data[0], 'EBADF encountered in kqueue. Invalid file descriptor 6')
        self.assertEqual(options.logger.data[1], 'EBADF encountered in kqueue. Invalid file descriptor 7')
    ...
    def assertDeletedEvent(self, kevent, fd):
        self.assertKevent(kevent, fd, select.KQ_FILTER_READ | select.KQ_FILTER_WRITE, select.KQ_EV_DELETE)
...
class DummyKQueue(object):
    ...
    def assert_max_events_on_poll(self, max_events):
        assert max_events == KQueuePoller.max_events, '`max_events` parameter of `kqueue.control() should be %d' % KQueuePoller.max_events

    def assert_max_events_on_register(self, max_events):
        assert max_events == 0, '`max_events` parameter of `kqueue.control()` should be 0 on register'
