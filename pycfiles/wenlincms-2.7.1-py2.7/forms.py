...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/wenlincms/generic/forms.py
...
from wenlincms.core.forms import Html5Mixin
from wenlincms.generic.models import Keyword, ThreadedComment
from wenlincms.utils.views import ip_for_request
...
class KeywordsWidget(forms.MultiWidget):
    ...
    class Media:
        js = ('wenlincms/js/admin/keywords_field.js', )
    ...
    def __init__(self, attrs=None):
        ...
        widgets = (
         forms.HiddenInput,
         forms.TextInput(attrs={b'class': b'vTextField'}))
    ...
    def decompress(self, value):
        ...
        """
        if hasattr(value, b'select_related'):
            keywords = [ a.keyword for a in value.select_related(b'keyword') ]
            if keywords:
                keywords = [ (str(k.id), k.title) for k in keywords ]
                self._ids, words = list(zip(*keywords))
                return (
                 (b',').join(self._ids), (b', ').join(words))
        return ('', '')
    ...
    def format_output(self, rendered_widgets):
        ...
        links = b''
        objs = Keyword.objects.all().order_by(b'-assigned_num')[:300]
        for keyword in objs:
            prefix = b'+' if str(keyword.id) not in self._ids else b'-'
            links += b"<a href='#'>%s%s</a>" % (prefix, str(keyword))

        rendered += b'<p style="margin: 5px 0px 2px 130px;color: #888;">下面选取频率最高的前300个标签；也可以直接输入新标签，用，分隔</p>'
        rendered += mark_safe(b"<p class='keywords-field'>%s</p>" % links)
        return rendered
    ...
    def value_from_datadict(self, data, files, name):
        ...
        """
        return data.get(b'%s_0' % name, b'')


class ThreadedCommentForm(CommentForm, Html5Mixin):

    def __init__(self, request, *args, **kwargs):
        kwargs.setdefault(b'initial', {})
    ...
    def save(self, request):
        ...
        comment.replied_to_id = self.data.get(b'replied_to')
        comment.save()
