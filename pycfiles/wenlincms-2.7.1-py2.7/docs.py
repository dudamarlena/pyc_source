...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/wenlincms/utils/docs.py
...
from wenlincms import __version__
from wenlincms.conf import registry
from wenlincms.utils.importing import import_dotted_path, path_for_import
...
def build_settings_docs(docs_path, prefix=None):
    ...
    ``wenlincms.conf.registry`` into RST format for use in docs,
...
def build_deploy_docs(docs_path):
    ...
    project_template_path = path_for_import(b'wenlincms.project_template')
...
def build_changelog(docs_path, package_name=b'wenlincms'):
    """
...
def build_modelgraph(docs_path, package_name=b'wenlincms'):
    """
    Creates a diagram of all the models for wenlincms and the given
    ...
    apps = [ a.rsplit(b'.')[1] for a in settings.INSTALLED_APPS if a.startswith(b'wenlincms.') or a.startswith(package_name + b'.')
           ]
    try:
...
def build_requirements(docs_path, package_name=b'wenlincms'):
    """
    Updates the requirements file with wenlincms's version number.
    """
    mezz_string = b'wenlincms=='
    project_path = os.path.join(docs_path, b'..')
