...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/wenlincms/utils/views.py
...
from wenlincms.utils.sites import has_site_permission
...
def is_spam_akismet(request, form, url):
    ...
    * comment: First field with a ``Textarea`` widget.
    ...
    if not settings.AKISMET_API_KEY:
        ...
        versions = (django.get_version(), wenlincms.__version__)
        headers = {b'User-Agent': b'Django/%s | wenlincms/%s' % versions}
        try:
...
def is_spam(request, form, url):
    ...
    page processor for ``wenlincms.forms``, to check if posted content is
