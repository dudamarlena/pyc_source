...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/wenlincms/generic/fields.py
...
class BaseGenericRelation(GenericRelation):
    ...
    def __init__(self, *args, **kwargs):
        ...
        """
        if kwargs.get(b'frozen_by_south', False):
            raise Exception(b"\n\n    Your project contains migrations that include one of the fields\n    from wenlincms.generic in its Migration.model dict: possibly\n    KeywordsField, CommentsField. These migratons no\n    longer work with the latest versions of Django and South, so you'll\n    need to fix them by hand. This is as simple as commenting out or\n    deleting the field from the Migration.model dict.\n\n    ")
        kwargs.setdefault(b'object_id_field', b'object_pk')
        to = getattr(self, b'related_model', None)
        if to:
            kwargs.setdefault(b'to', to)
    ...
    def contribute_to_class(self, cls, name):
        ...
        for field in cls._meta.many_to_many:
            if isinstance(field, self.__class__):
                e = b'Multiple %s fields are not supported (%s.%s, %s.%s)' % (
        ...
        if not cls._meta.abstract:
            for name_string, field in self.fields.items():
                if b'%s' in name_string:
                    name_string = name_string % name
                if name_string in [ i.name for i, _ in cls._meta.get_fields_with_model()
                                  ]:
                    continue
            ...
            getter_name = b'get_%s_name' % self.__class__.__name__.lower()
    ...
    def _related_items_changed(self, **kwargs):
        ...
        """
        for_model = kwargs[b'instance'].content_type.model_class()
        if issubclass(for_model, self.model):
            instance_id = kwargs[b'instance'].object_pk
            try:
                instance = for_model.objects.get(id=instance_id)
            except self.model.DoesNotExist:
                return

            if hasattr(instance, b'get_content_model'):
...
class CommentsField(BaseGenericRelation):
    ...
    deleted.
    """
    related_model = b'generic.ThreadedComment'
    fields = {b'%s_count': IntegerField(editable=False, default=0)}
...
class KeywordsField(BaseGenericRelation):
    ...
    searching.
    """
    related_model = b'generic.AssignedKeyword'
    fields = {b'%s_string': TextField(editable=False, blank=True)}
    ...
    def formfield(self, **kwargs):
        ...
        """
        from wenlincms.generic.forms import KeywordsWidget
    ...
    def contribute_to_class(self, cls, name):
        ...
        if hasattr(cls, b'search_fields') and name in cls.search_fields:
            try:
    ...
    def related_items_changed(self, instance, related_manager):
        ...
        """
        assigned = related_manager.select_related(b'keyword')
        keywords = (b' ').join([ str(a.keyword) for a in assigned ])
        string_field_name = list(self.fields.keys())[0] % self.related_field_name
