...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/wenlincms/core/templatetags/wenlincms_tags.py
...
from wenlincms.utils.cache import nevercache_token, cache_installed
from wenlincms.utils.html import decode_entities
from wenlincms.utils.importing import import_dotted_path
from wenlincms.utils.urls import admin_url
from wenlincms.utils.views import is_editable
...
if cache_installed():
    ...
    def nevercache(parser, token):
        ...
        in ``wenlincms.core.middleware.UpdateCacheMiddleware``.
...
@register.inclusion_tag(b'includes/editable_loader.html', takes_context=True)
...
def admin_app_list(request):
    ...
    ``wenlincms.conf.ADMIN_MENU_ORDER``. Called from the
...
def dashboard_column(context, token):
    ...
    ``wenlincms.conf.DASHBOARD_TAGS`` to render into the admin
    dashboard.
    """
