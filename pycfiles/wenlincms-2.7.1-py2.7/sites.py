...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/wenlincms/utils/sites.py
...
def has_site_permission(user):
    ...
    """
    mw = b'wenlincms.core.middleware.SitePermissionMiddleware'
    if mw not in settings.MIDDLEWARE_CLASSES:
        from warnings import warn
        warn(mw + b' missing from settings.MIDDLEWARE_CLASSES - per sitepermissions not applied')
        return user.is_staff and user.is_active
    return getattr(user, b'has_site_permission', False)
...
def host_theme_path(request):
    ...
    for host, theme in settings.HOST_THEMES:
        if host.lower() == request.get_host().split(b':')[0].lower():
            try:
                __import__(theme)
...
def templates_for_host(request, templates):
    ...
    if theme_dir:
        for template in templates:
            host_templates.append(b'%s/templates/%s' % (theme_dir, template))
