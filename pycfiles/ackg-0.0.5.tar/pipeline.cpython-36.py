...
# Embedded file name: /anaconda3/lib/python3.6/site-packages/ackeras/pipeline.py
# Compiled at: 2018-09-05 05:08:40
...
import time, pandas as pd, pdb
from itertools import compress
pd.options.mode.chained_assignment = None

def format_list(list_like):
    series_like = pd.Series(list_like)
    series_like = series_like.apply(lambda x: x.lower().replace('-', '_').replace(' ', '_'))
...
class Pipeline:
    ...
    def __init__(self, input_data, categorical_features=None, timecolumn=None, extreme_drop=None, y=None, drop_rest=True, supervised=False, insample=None):
        ...
        for col in input_data.columns:
            if input_data[col].dtype == float:
                input_data[col] = input_data[col].apply(lambda x: int(x) if not pd.isna(x) else x)

        self.insample = insample if insample is not None else int(len(input_data) * 0.9)
        self.categorical_features = categorical_features if len(categorical_features) != 0 else None
        self.timecolumn = format_list(timecolumn[0]) if isinstance(timecolumn, list) else format_string(timecolumn)
    ...
    def clustering(self):
        ...
        self.cluster_class = Clustering((self.data_processed),
          categorical_features=(self.acp.embedded_columns))
        self.clustered_data = self.cluster_class.fit_predict()
        return (
         self.clustered_data, self.cluster_class)

    def regression(self):
        print('Regressing...')
        assert self.acp is not None
        self.regress = Regression((self.data_processed),
          y=(self.reg_class[1]), problem=(self.reg_class[0]))
    ...
    def classification(self):
        ...
        if not self.acp is not None:
            raise AssertionError
        else:
            data = self.data_processed
            insample = self.insample
            assert isinstance(insample, int)
        X_insample, y_insample = data[:insample].drop((self.y),
          axis=1), data[:insample][self.y]
        X_outsample, y_outsample = data[insample:].drop((self.y),
          axis=1), data[insample:][self.y]
        catcols_X = self.acp.categorical_features.copy()
        catcols_X.remove(self.y)
        params = {'categorical_features':catcols_X, 
         'analysis':True, 
         'outputplot':False, 
         'avoid_pca':False}
        ...
        cl = Classification(X_in_pca, y_insample, X_outsample=X_out_pca,
          y_outsample=y_outsample)
    ...
    def process(self):
        try:
            ...
            if self.y is not None:
                ...
                columns = self.acp.label_encoders[self.y].classes_ if self.y in self.acp.label_encoders.keys() else None
                prob_df = pd.DataFrame(joint_prob,
                  index=(self.data_processed[self.insample:].index), columns=columns)
            self.status = 'Done'
            data_processed = pd.DataFrame(data_processed).reset_index().T.to_dict()
            cluster_data = pd.DataFrame(cluster_data[0]).reset_index().T.to_dict()
            coefficients = pd.DataFrame(coefficients).reset_index().T.to_dict()
            prob_df = pd.DataFrame(prob_df).reset_index().T.to_dict()
            outputs = {'acp':data_processed, 
