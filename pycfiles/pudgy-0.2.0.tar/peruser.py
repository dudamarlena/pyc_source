...
# Embedded file name: build/bdist.darwin-8.9.0-Power_Macintosh/egg/pudge/peruser.py
# Compiled at: 2006-03-14 17:35:23
__doc__ = 'Source enhanced Python object inspection.'
import sys, os, os.path as path, inspect, types, pudge.rst as rst, pudge.scanner as scanner
...
def find(name, cache=1):
    ...
    if '.' in name:
        ...
        try:
            rslt = ModulePeruser(name, __import__(name), None, 0)
        except ImportError:
            pass
...
class Peruser(object):
    ...
    """
    __module__ = __name__
    exclude_member_names = set(['__doc__', '__weakref__', '__module__', '__builtins__', '__file__', '__name__', '__version__', '__revision__', '__license__', '__date__', '__author__', '__copyright__', '__metaclass__', '__builtin__'])
        ...
        if self._public_members is None:
            if hasattr(self.obj, '__all__'):
                ...
                for name in self.dir():
                    if name.startswith('_') and not name.endswith('__') or name in self.exclude_member_names:
                        continue
                    members.append(name)
    ...
    def get_attributes(self):
        attrs = {}
        hide = [
         '__doc__', '__all__', '__file__', '__package__', '__name__']
    ...
    def members(self, types=None):
        ...
        for n in names:
            item = self.member(n)
            if item.type_name not in types:
                continue
            m = item.get_actual_module()
            if not m:
                continue
            canonical_root = item.get_canonical_root()
            if canonical_root is None or canonical_root != self.get_canonical_root():
                continue
    ...
    def _load_child(self, name):
        ...
        try:
            ...
            raise MemberError('%s does not contain member: %r' % (self.get_qualified_name(), name))
...
class ModulePeruser(Peruser):
    ...
    def is_package(self):
        if hasattr(self.obj, '__file__'):
            return self.obj.__file__.endswith('__init__.py') or self.obj.__file__[0:-1].endswith('__init__.py')
        ...
        try:
            ...
            for name in all:
                if not isinstance(name, (str, unicode)):
                    yield '%s.__all__ contains a non-string: %r?' % (self.get_qualified_name(), name)
                if not hasattr(self.obj, name):
                    yield '%s.__all__ has %r but hasattr(%s, %r) disagrees.' % ((self.get_qualified_name(), name) * 2)
...
peruser_types = [
 (
  lambda o: isinstance(o, types.ModuleType), ModulePeruser),
 (
  lambda o: isinstance(o, (types.ClassType, types.TypeType)), ClassPeruser),
 (
  lambda o: callable(o), CallablePeruser),
 (
  lambda o: 1, NamePeruser)]
...
__all__ = [
 'find', 'Peruser', 'ModulePeruser', 'ClassPeruser', 'CallablePeruser', 'NamePeruser']
