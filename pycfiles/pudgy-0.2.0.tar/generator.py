...
# Embedded file name: build/bdist.darwin-8.9.0-Power_Macintosh/egg/pudge/generator.py
# Compiled at: 2007-01-07 21:49:07
__doc__ = '\nGenerate Python documentation and other stuff.\n\n\n'
import sys, os, os.path as path, stat, inspect, time, kid, pudge, pudge.browser as browser, pudge.colorizer as colorizer, pudge.rst as rst
...
class Generator(object):
    ...
    fragment = 0
    trac_url = None
    trac_links = [('wiki', 'wiki', 'Collaborative documentation'), ('repository', 'browser', 'Browse subversion repository'), ('roadmap', 'roadmap', 'View the project roadmap'), ('feedback!', 'newticket', 'Report an issue / request an enhancement')]
    ...
    def get_template(self, name):
        ...
        try:
            return Template(generator=self, browser=self.browser)
        except xml.parsers.expat.ExpatError, e:
            raise '%s: File "%s", line %d\n\t%r' % (e, e.filename, e.lineno, e.source)

        return

    def expand_template(self, name, destfile, output=None, extension=None, **kw):
        ...
        if t.settings:
            t.__dict__.update(self.settings)
        basedir = self._ensure_dir(path.dirname(destfile))
        if extension is None:
            extension = self.file_extension
        t.destfile = destfile + extension
        filename = path.join(basedir, os.path.basename(destfile) + extension)
        if output is None:
            if self.xhtml:
                output = 'xhtml-strict'
            else:
                output = 'html-strict'
        t.write(filename, fragment=self.fragment, output=output, encoding='utf-8')
        return

    def load_documents(self):
        docs = []
        self.index_document = {'title': self.title, 'fragment': '', 'basename': ''}
        document_files = [ (path.splitext(path.basename(f))[0], f) for f in self.document_files ]
        if self.license:
            license_file = 'licenses/%s.rst' % (self.license,)
            document_files.append(('doc-license',
             resource_filename(__name__, license_file)))
    ...
    def _document_basename(self, filename):
        if self.doc_base:
            if not filename.startswith(self.doc_base):
                pudge.log.info("Document %s isn't under doc_base %s" % (filename, self.doc_base))
            else:
    ...
    def generate_documents(self):
        for doc in self.documents:
            self.expand_template('document.html', doc['relative_filename'], parts=doc)
    ...
    def generate_docstrings(self, m):
        ...
        if self.needs_regeneration(filename, src_file):
            self.expand_template('module.html', filename, subject=m)
        filename += '-index'
        if self.needs_regeneration(filename, src_file):
            try:
                self.expand_template('module-index.html', filename, subject=m)
            except Exception, e:
                e.args = (
                 'Error when rendering %s: %s' % (m.qualified_name(), e),)
    ...
    def generate_classes(self, m):
        ...
        for c in m.classes():
            ...
            if self.needs_regeneration(filename, src_file):
                self.expand_template('class.html', filename, subject=c)
    ...
    def needs_regeneration(self, dest_file, src_file, name=None):
        ...
        if path.getmtime(dest_file) >= path.getmtime(src_file):
            pudge.log.debug('skipping: %s (fresh)' % (name or dest_file))
            return False
        return True
    ...
    def generate_source(self):
        for m in self.browser.modules(recursive=1):
            module = m.obj
            if not module:
                pudge.log.debug('skipping: %s (no source file)' % m.qualified_name())
                continue
            ...
            if not self.needs_regeneration(dest_file, src_file, name=m.qualified_name()):
                continue
    ...
    def copy_files(self):
        ...
        for (basename, file) in template_files.items():
            ...
            self.expand_template(basename, destfile, output='plain', extension='')
    ...
    def browser(self):
        if not hasattr(self, '_browser'):
            self._browser = browser.Browser(self.modules, self.exclude_modules)
    ...
    def time_func(self, description, func, *args, **kw):
        ...
        if end - start > 0.1:
            self.profile_info.append('%s: %s%4.1f secs' % (description, ' ' * (45 - len(description)), end - start))
        return v
...
class Page(object):
    ...
    def link_to(self, obj):
        ...
        if obj.ismodule():
            ...
            if obj.parent.ismodule():
                ...
                path = 'class-%s.html#%s'
            path = path % (obj.parent.qualified_name(), obj.name)
        return path
    ...
    def link_to_source(self, obj):
        ...
        if line:
            ...
            line = '???'
            hash = params = ''
        return '%s.html%s%s' % (obj.relative_file(), params, hash)
