...
# Embedded file name: build/bdist.darwin-8.9.0-Power_Macintosh/egg/pudge/browser.py
# Compiled at: 2006-09-05 21:55:05
import sys, pydoc, os.path as path, rst, pudge
from inspect import *
import warnings
meta_names = [
 'all', 'author', 'copyright', 'date', 'license', 'revision', 'version']
meta_names = set([ '__%s__' % n for n in meta_names ])
warnings.filterwarnings('ignore', category=DeprecationWarning, module='pydoc')
...
class Name(object):
    ...
    def source_lines(self):
        try:
            ...
            raise IndentationError('In module %s: %s' % (self.qualified_name(), e))
        except Exception, e:
    ...
    def html(self, blurbless=0):
        ...
        warn('Name.html() is deprecated. Use Name.doc(html=1) instead.', DeprecationWarning, stacklevel=2)
    ...
    def modules(self, visible_only=1, recursive=0, real_modules=None):
        if not real_modules:
            real_modules = set()
        for m in self.all(visible_only, predicate=lambda m: m.ismodule(), exclude_names=self.browser.exclude_modules):
            ...
            if real_module not in real_modules:
                if m.qualified_name() == real_module.__name__:
                    ...
                    yield m
                    if recursive:
                        for m in m.modules(visible_only, 1, real_modules=real_modules):
                            real_module = m.real_module
                            if real_module not in real_modules:
                                if m.qualified_name() == real_module.__name__:
                                    real_modules.add(real_module)
    ...
    def all(self, visible_only=1, predicate=None, exclude_names=None):
        for m in self.members.values():
            if exclude_names and m.qualified_name() in exclude_names:
                continue
            if visible_only and not m.isvisible() or predicate and not predicate(m):
                continue
            yield m
...
class Module(Name):
    __module__ = __name__
    exclude_members = Name.exclude_members.union(['__builtins__', '__file__', '__name__', '__path__', '__pudge_all__', '__test__'])
    type_name = 'module'
            ...
            for file in os.listdir(imp_path):
                ...
                if modname:
                    if file.endswith('.py') and not file.startswith('.'):
                        if modname in names:
                            value = names[modname]
                        elif not ismodule(value):
                            pudge.log.warn('%s.%s hides module with same name', qname, modname)
                        continue
...
class Class(Name):
    __module__ = __name__
    type_name = 'class'
    exclude_members = Name.exclude_members.union(['__class__', '__delattr__', '__getattribute__', '__hash__', '__module__', '__new__', '__reduce__', '__reduce_ex__', '__repr__', '__setattr__', '__str__', '__weakref__', '__metaclass__', '__getattr__', '__test__'])
...
class Attribute(Name):
    ...
    def doc(self, strip=1, blurbless=0, html=0):
        if isinstance(self.obj, property) and getattr(self.obj, '__doc__', None):
            return super(Attribute, self).doc(strip=strip, blurbless=blurbless, html=html)
        else:
            return ''
        return