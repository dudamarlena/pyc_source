...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/cctagutils/base32.py
...
upcasetranstable = string.maketrans(string.lowercase, string.uppercase)
digitchars = string.translate(chars, identitytranstable, string.lowercase)
...
def b2a_l_long(os, lengthinbits):
    ...
    if lengthinbits % 8 != 0:
        os[-1] = os[(-1)] >> 8 - lengthinbits % 8
        os[-1] = os[(-1)] << 8 - lengthinbits % 8
    numoctetsneeded = (numquintets * 5 + 7) / 8 + 4
    os.extend([0] * (numoctetsneeded - len(os)))
    quintets = []
    i = 0
    CUTOFF = 2 ** 35
...
def a2b_l_long(cs, lengthinbits):
    ...
    if lengthinbits % 5 != 0:
        qs[-1] = qs[(-1)] >> 5 - lengthinbits % 5
        qs[-1] = qs[(-1)] << 5 - lengthinbits % 5
    numquintetsneeded = (numoctets * 8 + 4) / 5 + 7
    qs.extend([0] * (numquintetsneeded - len(qs)))
    octets = []
    i = 0
    CUTOFF = 2 ** 32
