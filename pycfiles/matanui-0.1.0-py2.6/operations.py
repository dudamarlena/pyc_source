...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/matanui/operations.py
# Compiled at: 2011-01-18 01:52:07
"""Operations to execute from configured service backend."""
__author__ = 'Guy K. Kloss <Guy.Kloss@aut.ac.nz>'
import io, pprint, matanui
...
class StorageOperations(object):
    ...
    def __init__(self, a_request, an_inout):
        ...
        StorageBackend = getattr(module, class_name)
    ...
    def get_content(self):
        ...
        """
        try:
            content = self._storage_backend.get_content(object_id=self.request.object_id, filename=self.request.path)
            self._check_access_allowed(storerinterface.PRIV_READ)
        except exceptions.MataNuiStorerException, err:
    ...
    def post_content(self):
        ...
        object_id = self._storage_backend.put_content(content_object, self.request.query)
    ...
    def delete_file(self):
        ...
        try:
            ...
            object_id = self._storage_backend.delete_file(object_id=self.request.object_id, filename=self.request.path)
        except exceptions.MataNuiStorerException, err:
            status = (404, 'Storage error')
            message = err.message
            raise exceptions.MataNuiServiceException(message, status)
    ...
    def get_metadata(self):
        ...
        """
        try:
            (oid, metadata) = self._storage_backend.get_metadata(object_id=self.request.object_id, filename=self.request.path)
            self._check_access_allowed(storerinterface.PRIV_READ)
        except exceptions.MataNuiStorerException, err:
            status = (404, 'Storage error')
            message = err.message
        ...
        content_type = '%(base)s%(type)s-v%(api)s+%(encoding)s' % {'base': self.request.ACCEPT_HEADER_BASE, 'type': self.request.content_request, 
    ...
    def put_metadata(self):
        ...
        try:
            ...
            if 'filename' in metadata_object.content:
                ...
                self._check_access_allowed(storerinterface.PRIV_WRITE, target=target, dir_mode=True)
            oid = self._storage_backend.set_metadata(metadata_object, object_id=self.request.object_id, filename=self.request.path)
        except exceptions.MataNuiStorerException, err:
    ...
    def _check_access_allowed(self, access_mode, target=None, dir_mode=False):
        ...
        if target is not None:
            filename = target
            object_id = None
        access = self._storage_backend.is_access_allowed(self.request.username, access_mode, filename=filename, object_id=object_id, dir_mode=dir_mode)
        if not access:
            access_types = {storerinterface.PRIV_READ: 'read', storerinterface.PRIV_WRITE: 'write', storerinterface.PRIV_READ_WRITE: 'read/write'}
