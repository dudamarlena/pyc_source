...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/matanui/mongogridfs.py
# Compiled at: 2011-01-18 01:15:33
"""MongoDB GridFS storage back-end."""
__author__ = 'Guy K. Kloss <Guy.Kloss@aut.ac.nz>'
__date__ = '$Date: $'
import mimetypes, pymongo, bson
...
class MongoGridFS(storerinterface.NullStorer):
    ...
    def db(self):
        ...
        if not self._db:
            ...
            if config.GRIDFS_USER:
                self._db.authenticate(config.GRIDFS_USER, config.GRIDFS_PASSWORD)
        return self._db
    ...
    def put_content(self, content_object, query_metadata=None):
        ...
        if 'contentType' not in metadata:
            metadata['contentType'] = content_object.content_type or mimetypes.guess_type(content_object.filename)[0]
    ...
    def delete_file(self, filename=None, object_id=None):
        ...
        try:
            ...
            self._filename = None
            self._gridfs_handle = None
        except gridfs.errors.NoFile, err:
            raise MataNuiStorerException('Error deleting file: %s' % str(err))
    ...
    def list_resources(self, query):
        ...
        @raise MataNuiStorerException: In case of errors from the storage
            sub-system.
        """
        re_query = storerinterface.shell_to_regex_query(query, close_query=False)
        entries = self.files_collection.find({'filename': {'$regex': re_query}}, sort=[
         (
        ...
        unified_entries = storerinterface.filter_results(query, newest_entries.keys())
        results = []
        for item in unified_entries:
            entry = newest_entries.get(item, {'filename': item, 'conentType': 'collection'})
        ...
        if filename is None and object_id is None:
            ...
            try:
                ...
                if dir_mode:
                    ...
                    parent_paths = [ '/' + ('/').join(segments[:i]) for i in range(len(segments), -1, -1)
                                   ]
                    entry = None
                    for path in parent_paths:
                        try:
                            ...
                            entry = {'filename': path, 'accessMode': config.DEFAULT_PERMISSIONS, 
    ...
    def _get_mongo_file_handle_by_name(self, filename):
        ...
        if filename != self._filename or not self._mongo_file_handle:
            entries = self.files_collection.find({'filename': filename}, sort=[
             (
              'uploadDate',
              pymongo.DESCENDING)])
            try:
                ...
                raise MataNuiStorerNotFoundException('Did not find object with name %s' % filename)
            else:
    ...
    def _get_mongo_file_handle_by_oid(self, object_id):
        ...
        if object_id != self._object_id or not self._mongo_file_handle:
            self._object_id = object_id
            self._mongo_file_handle = self.files_collection.find_one({'_id': object_id})
            if not self._mongo_file_handle:
                raise MataNuiStorerNotFoundException('Did not find object with ID %s' % object_id)
            self._filename = self._mongo_file_handle['filename']
        return self._mongo_file_handle
    ...
    def _validate_access_mode(self, metadata, umask=None, default_permissions=None):
        ...
        access_mode = access_mode & storerinterface.PRIV_MAX & (umask ^ storerinterface.PRIV_MASK)
...
class MongoContentObject(storerinterface.ContentObject):
    ...
    def __init__(self, file_handle):
        ...
        self.content_type = str(self.data.content_type or mimetypes.guess_type(self.data.filename)[0])
