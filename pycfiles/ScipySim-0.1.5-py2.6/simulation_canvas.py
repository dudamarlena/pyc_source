...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/scipysim/gui/simulation_canvas.py
...
class CanvasBlock(object):
    ...
    def __init__(self, canvas, codefile, x, y, colour):
        ...
        preview_tags = [
         ...
         'id:%s' % self.id,
         'preview']
        self.canvas.create_rectangle(fill=colour, tags=(preview_tags + ['type:block']), *((x, y) + self.BLOCK_SIZE))
        self.canvas.create_text(x + self.BLOCK_WIDTH / 4, y + self.BLOCK_HEIGHT / 2, font=self.font, text=codefile.name, tags=preview_tags + ['type:text'])
        ...
        if num_inputs == 0:
            ...
            for input in xrange(num_inputs):
                self.canvas.create_polygon(x, y_ + gap, x - gap, y_ + gap * 3 / 2, x, y_ + 2 * gap, tags=preview_tags + ['type:input-%d' % input], fill='#000000')
                y_ = y_ + gap
        ...
        if num_outputs == 0:
            ...
            for output in xrange(num_outputs):
                self.canvas.create_polygon(x_, y_ + gap, x_ + gap, y_ + gap * 3 / 2, x_, y_ + 2 * gap, tags=preview_tags + ['type:output-%d' % output], fill='#000000')
...
class SimulationCanvas(object):
    ...
    def __init__(self, frame):
        self.canvas = Canvas(frame, width=self.width, height=self.height, relief=RIDGE, background=self.colours['background'], borderwidth=1)
    ...
    def preview_actor(self, codefile):
        ...
        block = CanvasBlock(self.canvas, codefile, colour=self.colours['preview'], *self.PREVIEW_LOCATION)
