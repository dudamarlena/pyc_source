...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/scipysim/actors/signal/eventfilter.py
...
class EventFilter(Siso):
    ...
    def __init__(self, input_channel, output_channel, delta=0.5):
        ...
        """
        super(EventFilter, self).__init__(input_channel=input_channel, output_channel=output_channel, child_handles_output=True)
        self.delta = delta
    ...
    def siso_process(self, obj):
        ...
        if quantized_value != self.current_quantum:
            if self.current_quantum is None:
                ...
                signal_gradient = (value - self.last_event['value']) / (tag - self.last_event['tag'])
                event_dt = (quantized_value - self.last_event['value']) / signal_gradient
