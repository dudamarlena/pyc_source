...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/scipysim/actors/logic/testlogic.py
...
class ElsePassThroughTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        [ self.alt_data_in.put({'tag': i, 'value': self.data_alt[i]}) for i in range(100)
        ]
        [ self.data_in.put({'tag': i, 'value': i}) for i in range(100) ]
        self.data_in.put(None)
        self.alt_data_in.put(None)
        self.block = PassThrough(self.bool_in, self.data_in, self.output, else_data_input=self.alt_data_in)
        return
