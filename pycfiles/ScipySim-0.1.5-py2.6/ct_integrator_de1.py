...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/scipysim/actors/math/ct_integrator_de1.py
...
class CTIntegratorDE1(Siso):
    ...
    def __init__(self, xdot, x, init=0.0, delta=0.1, k=10.0):
        ...
        """
        super(CTIntegratorDE1, self).__init__(input_channel=xdot, output_channel=x, child_handles_output=True)
        self.delta = delta
    ...
    def __statestep(self, x, xdot):
        ...
        dist = x - qx if abs(x - qx) > 1e-15 else 0
        dist = dist if abs(self.delta - dist) > 1e-15 else 0
        if xdot > 0:
    ...
    def __external_transition(self, tag, xdot):
        ...
        self.xdot = xdot
        self.last_t += elapsed
        self.next_t = self.last_t + self.__timestep()
        self.x = new_x
...
class CTintegratorTest(unittest.TestCase):
    ...
    def test_simple_integration(self):
        ...
        """
        intags = [
         0.0, 0.1, 0.21111111111111114, 0.33611111111111114, 0.478968253968254, 0.6456349206349207, 0.8456349206349207, 1.0956349206349207, 1.428968253968254, 1.928968253968254, 2.9289682539682538, 12.928968253968254]
        ...
        expected_output_tags = [
         0.0, 0.1, 0.21111111111111114, 0.33611111111111114, 0.478968253968254, 0.6456349206349207, 0.8456349206349207, 1.0956349206349207, 1.428968253968254, 1.928968253968254, 2.9289682539682538, 12.928968253968254, 22.928968253968254]
