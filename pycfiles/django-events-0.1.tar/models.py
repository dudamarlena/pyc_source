...
# [GCC 8.4.0]
# Embedded file name: /home/skylar/pinax/projects/NASA/apps/events/models.py
# Compiled at: 2009-08-18 18:19:43
import datetime
...
class FutureEventsManager(models.Manager):
    ...
    def get_query_set(self):
        return super(FutureEventsManager, self).get_query_set().filter(start__gte=datetime.datetime.now() - datetime.timedelta(hours=1)).order_by('start')
...
class PastEventsManager(models.Manager):
    ...
    def get_query_set(self):
        return super(PastEventsManager, self).get_query_set().filter(start__lte=datetime.datetime.now() - datetime.timedelta(hours=1)).order_by('-start')
...
class Event(models.Model):
    ...
    start = models.DateTimeField(verbose_name='Date and Time', help_text='Checkit')
