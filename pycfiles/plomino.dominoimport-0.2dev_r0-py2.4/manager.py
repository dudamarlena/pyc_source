...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/plomino/dominoimport/browser/manager.py
...
class DominoImportManager(BrowserView):
    ...
    def manageImportDXL(self):
        ...
        if results['forms'] != (0, 0):
            infoMsg = infoMsg + 'Forms imported: ' + str(results['forms'][0]) + ' succeeded, ' + str(results['forms'][1]) + ' failed.' + MSG_SEPARATOR
        if results['views'] != (0, 0):
            infoMsg = infoMsg + 'Views imported: ' + str(results['views'][0]) + ' succeeded, ' + str(results['views'][1]) + ' failed.' + MSG_SEPARATOR
        if results['docs'] != (0, 0):
            infoMsg = infoMsg + 'Documents imported: ' + str(results['docs'][0]) + ' succeeded, ' + str(results['docs'][1]) + ' failed.' + MSG_SEPARATOR
        logger.info(infoMsg)
