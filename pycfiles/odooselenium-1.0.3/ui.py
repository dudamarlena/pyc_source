...
# [GCC 8.4.0]
# Embedded file name: /home/tonio/sources/odooselenium/odooselenium/ui.py
# Compiled at: 2016-12-12 08:06:33
"""Python bindings to Odoo's user interface (UI) driven by Selenium."""
import contextlib, re, time, urlparse
...
class OdooUI(object):
    ...
    def create_button(self):
        return self.webdriver.find_element(By.XPATH, "//button[@data-bt-testing-model_name='stock.move' and @data-bt-testing-name='oe_list_add']")
    ...
    def wait_for_page_load(self, timeout=10):
        ...
        yield
        ui.WebDriverWait(self.webdriver, timeout).until(expected_conditions.staleness_of(initial_body))
        return
    ...
    def login(self, username, password, dbname=None):
        ...
        try:
            db_field = self.webdriver.find_element(By.ID, 'db')
        except NoSuchElementException:
            pass
        else:
        ...
        login_field = self.webdriver.find_element(By.ID, 'login')
        login_field.send_keys(username)
        password_field = self.webdriver.find_element(By.ID, 'password')
        password_field.send_keys(password)
        login_button = self.webdriver.find_element(By.CSS_SELECTOR, '.btn-primary')
        with self.wait_for_page_load():
            login_button.click()

    def url(self, path=''):
        """Return complete URL."""
        return ('{base:s}/{path:s}').format(base=self.base_url, path=path.lstrip('/'))

    def list_modules(self):
        ui.WebDriverWait(self.webdriver, 10).until(expected_conditions.presence_of_element_located((
         By.CSS_SELECTOR,
         '.oe_application .oe_view_manager')))
        modules = self.webdriver.find_elements(By.CSS_SELECTOR, '.navbar-nav .oe_menu_text')
        return modules
    ...
    def go_to_module(self, module_name, timeout=10):
        ...
        tuple_module = (
         module_name, list_module_display)
        assert module_link is not None, ("Couldn't find module menu '{}' in '{}'").format(tuple_module[0], tuple_module[1])
        ui.WebDriverWait(self.webdriver, timeout).until(expected_conditions.presence_of_element_located((
         By.CSS_SELECTOR,
         '.oe_application .oe_view_manager')))
        module_link.click()
        ui.WebDriverWait(self.webdriver, timeout).until(expected_conditions.presence_of_element_located((
    ...
    def go_to_view(self, view_name, timeout=10):
        ...
        secondary_menus = self.webdriver.find_elements_by_css_selector('.oe_secondary_menu')
        menu_parts = view_name.split('/')
        searched_menu = menu_parts.pop(0)
        previous_parts = set()
        view_link = None
        for sec_menu in secondary_menus:
            if sec_menu.is_displayed():
                menus = sec_menu.find_elements(By.CSS_SELECTOR, '.oe_secondary_submenu .oe_menu_text')
                for menu in menus:
                    if menu.text in [searched_menu] + list(previous_parts):
                        ...
                        if menu_parts or menu.text in previous_parts:
                            if 'oe_menu_opened' not in menu_parent.get_attribute('class'):
                                menu.click()
        ...
        tuple_view = (
         view_name, menus)
        assert view_link is not None, ("Couldn't find view menu '{}' in '{}'").format(tuple_view[0], tuple_view[1])
    ...
    def click_form_view_tab(self, tab_name):
        tabs = self.webdriver.find_elements(By.CSS_SELECTOR, '.ui-tabs .ui-corner-top .ui-tabs-anchor')
        requested_tab = None
        for tab in tabs:
        ...
        assert requested_tab is not None, ("Couldn't find form tab '{0}'").format(tab_name)
        with self.wait_for_ajax_load():
            requested_tab.click()
        return

    def click_button(self, button_name, view_name):
        button_divs = self.webdriver.find_elements_by_css_selector(('.oe_view_manager_buttons .oe_{}_buttons').format(view_name))
    ...
    def click_button_by_model(self, model_name, name, timeout=10):
        with self.wait_for_ajax_load():
            button = ui.WebDriverWait(self.webdriver, timeout).until(expected_conditions.presence_of_element_located((
             By.XPATH,
             ("//button[@data-bt-testing-model_name='{}' and @data-bt-testing-name='{}']").format(model_name, name))))
            button.click()
    ...
    def click_ajax_load_button(self, data_bt_testing_name, data_bt_testing_model_name=None, data_class=None, timeout=10):
        if data_bt_testing_model_name:
            if data_class:
                xpath = ('//button[@class="{}" and @data-bt-testing-name="{}" and @data-bt-testing-model_name="{}"]').format(data_class, data_bt_testing_name, data_bt_testing_model_name)
            else:
                xpath = ('//button[@data-bt-testing-name="{}" and @data-bt-testing-model_name="{}"]').format(data_bt_testing_name, data_bt_testing_model_name)
        elif data_class:
            xpath = ('//button[@class={} and @data-bt-testing-name="{}"]').format(data_class, data_bt_testing_name)
        else:
            xpath = ('//button[@data-bt-testing-name="{}"]').format(data_bt_testing_name)
    ...
    def delete_item_from_form_kanban(self, value, timeout=10):
        xpath = ('//a[contains(@class, "oe_kanban_action") and text()="{}"]/ancestor::div[contains(@class, "oe_kanban_record")]//a[contains(@class, "oe_kanban_action") and @data-type="delete"]').format(value)
    ...
    def get_values_from_form_kanban(self):
        ...
        xpath = '//div[@class="oe_form"]//div[contains(@class, "oe_kanban_record")]//table[@class="oe_kanban_table"]//a'
    ...
    def get_rows_from_form_list(self, header=None):
        ...
        if header:
            columns_xpath = ('//div[normalize-space(text())="{}"]//following-sibling::div[1]//table[@class="oe_list_content"]/thead/tr[@class="oe_list_header_columns"]/th').format(header)
        else:
            columns_xpath = '//div[@class="oe_form"]//table[@class="oe_list_content" and not(ancestor::*[@style="display: none;"])]/thead/tr[@class="oe_list_header_columns"]/th'
        headers_xpath = ('{}/div').format(columns_xpath)
        if header:
            xpath = ('//div[normalize-space(text())="{}"]/following-sibling::*[1]//table[@class="oe_list_content"]/tbody/tr/td').format(header)
        else:
            xpath = '//div[@class="oe_form"]//table[@class="oe_list_content" and not(ancestor::*[@style="display: none;"])]/tbody/tr/td'
        return self._get_rows_from_list(columns_xpath, headers_xpath, xpath)

    def delete_item_from_form_list(self, column, value, header=None, timeout=10):
        xpath = ('//table[@class="oe_list_content"]/tbody/tr/td[@data-field="{}" and text()="{}"]/following-sibling::td[@class="oe_list_record_delete"]').format(column, value)
        if header:
            xpath = ('//div[normalize-space(text())="{}"]/following-sibling::*[1]{}').format(header, xpath)
        self._delete_item_from_form(xpath, timeout)
    ...
    def add_item_to_form_table(self, header=None, timeout=10):
        xpath = '//*[@class="oe_form_field_one2many_list_row_add" or @class="oe_form_field_many2many_list_row_add"]/a'
        if header:
            xpath = ('//div[normalize-space(text())="{}"]/following-sibling::*[1]{}').format(header, xpath)
        self._add_item_to_form(xpath, timeout)
    ...
    def select_list_items(self, data_field, column_value):
        ...
        xpath = ('//table[@class="oe_list_content"]/tbody/tr/td[@data-field="{}" and text()="{}"]/../th[@class="oe_list_record_selector"]/input').format(data_field, column_value)
    ...
    def get_rows_from_list(self, data_field=None, column_value=None):
        ...
        columns_xpath = '//table[@class="oe_list_content"]/thead/tr[@class="oe_list_header_columns"]/th[starts-with(@class, "oe_list_header_")]'
        headers_xpath = ('{}/div').format(columns_xpath)
        if data_field and column_value:
            xpath = ('//table[@class="oe_list_content"]/tbody/tr/td[@data-field="{}" and text()="{}"]/../td').format(data_field, column_value)
        else:
            xpath = '//table[@class="oe_list_content"]/tbody/tr/td'
        return self._get_rows_from_list(columns_xpath, headers_xpath, xpath)

    def _get_rows_from_list(self, columns_xpath, headers_xpath, values_xpath):
        columns = [ elem for elem in self.webdriver.find_elements_by_xpath(columns_xpath) if elem.is_displayed()
                  ]
        chunk_size = len(columns)
        header_values = [ elem.text for elem in self.webdriver.find_elements_by_xpath(headers_xpath) if elem.is_displayed()
                        ]
        header_values += [ ('Untitled{}').format(x) for x in xrange(chunk_size - len(header_values))
                         ]
        values = []
        all_values = self.webdriver.find_elements_by_xpath(values_xpath)
        all_values = [ v for v in all_values if v.is_displayed() ]
        lines = [ all_values[i:i + chunk_size] for i in xrange(0, len(all_values), chunk_size)
                ]
        for line in lines:
    ...
    def click_more_item(self, menu_item):
        ...
        items"""
        more_button = self.wait_for_visible_element_by_xpath('//button[normalize-space(text())="More" and not(ancestor::div[@style="display: none;"])]')
        more_button.click()
        item_link = self.wait_for_visible_element_by_xpath(('//ul[@class="oe_dropdown_menu oe_opened"]/li/a[normalize-space(text())="{}"]').format(menu_item))
        item_link.click()
    ...
    def _get_autocomplete_search_items(self):
        menu_items_xpath = '//div[contains(@class, "oe-autocomplete")]/ul/li/span/em'
        menu_items = self.webdriver.find_elements_by_xpath(menu_items_xpath)
        return menu_items

    def search_for(self, search_string, type=None):
        xpath = '//div[@class="oe_searchview_facets"]/div[@class="oe_searchview_input"]'
        input_fields = self.webdriver.find_elements_by_xpath(xpath)
        input_field = next(field for field in input_fields if field.is_displayed())
        if type:
    ...
    def click_list_column(self, data_field, value, click_column=None):
        ...
        while not rows:
            ...
            next_button_xpath = '//div[@class="oe_list_pager"]/ul[@class="oe_pager_group"]/li/a[@data-pager-action="next"]'
            next_buttons = self.webdriver.find_elements_by_xpath(next_button_xpath)
            if rows:
                break
                ...
                pager_status = self.webdriver.find_element_by_xpath(pager_status_xpath)
        ...
        xpath = ('//table[@class="oe_list_content"]/tbody/tr/td[@data-field="{}" and text()="{}"]').format(data_field, value)
        attempts = 2
        elem = None
        counter = 0
        ...
        if not elem:
            raise RuntimeError(('Could not find row with {}').format(value))
        if click_column:
            elem = elem.find_element(by='xpath', value=('following-sibling::td[@data-field="{}"]').format(click_column))
        with self.wait_for_ajax_load():
            elem.click()
        return

    def install_module(self, module_name, column='shortdesc', timeout=60, upgrade=False):
        ...
        btn = self.wait_for_visible_element_by_xpath('//button[@class="oe_button oe_form_button oe_highlight"]')
        with self.wait_for_ajax_load(timeout):
            btn.click()
    ...
    def switch_to_view(self, view_name):
        ...
        xpath = ('//ul[@class="oe_view_manager_switch oe_button_group oe_right"]/li/a[@data-view-type="{}"]').format(view_name)
        button = self.wait_for_visible_element_by_xpath(xpath)
        button.click()

    def _get_bt_testing_element(self, field_name, model_name=None, in_dialog=False, last=False):
        if in_dialog:
            xpath = '//div[@class="modal-content openerp"]'
        else:
            xpath = ''
        if model_name:
            xpath = ('{}//*[@data-bt-testing-name="{}" and @data-bt-testing-model_name="{}"]').format(xpath, field_name, model_name)
        else:
            xpath = ('{}//*[@data-bt-testing-name="{}"]').format(xpath, field_name)
        if last:
            xpath = ('({})[last()]').format(xpath)
        return self.wait_for_visible_element_by_xpath(xpath)

    def write_in_element(self, field_name, model_name, text, clear=True, in_dialog=False):
        ...
        """
        elem = self._get_bt_testing_element(field_name, model_name, in_dialog=in_dialog)
        if clear:
            elem.clear()
        elem.send_keys(text)
    ...
    def open_text_dropdown(self, field_name, model_name, in_dialog):
        ...
        if in_dialog:
            ...
            xpath = ''
        xpath += ('//input[@data-bt-testing-name="{}" and @data-bt-testing-model_name="{}"]/../span[@class="oe_m2o_drop_down_button"]').format(field_name, model_name)
    ...
    def get_edit_field_from_label_text(self, label_text):
        ...
        xpath = ('//tr/td/label[normalize-space(text())="{}"]').format(label_text)
    ...
    def enter_data(self, field, model, data, clear=True, search_column=None, in_dialog=False):
        ...
        """
        input_field = self._get_bt_testing_element(field, model, in_dialog=in_dialog)
        if input_field.tag_name == 'select':
            dropdown_xpath = ('//select[@data-bt-testing-name="{}" and @data-bt-testing-model_name="{}"]/option[normalize-space(text())="{}"]').format(field, model, data)
            ...
            if elem_type in ('text', 'password') and elem_class in ('', 'oe_datepicker_master'):
                self.write_in_element(field, model, data, clear, in_dialog)
            elif elem_type == 'text' and elem_class == 'ui-autocomplete-input':
                if isinstance(data, list):
                    self.create_from_text_dropdown(field, model, in_dialog, data)
                elif data == '':
                    field = self._get_bt_testing_element(field, model)
                    field.clear()
                else:
                    self.search_text_dropdown(field, model, search_column, data, in_dialog)
            elif elem_type == 'checkbox':
                if input_field.is_selected() != data:
                    self.toggle_checkbox(field, model)
            else:
                raise NotImplementedError(("I don't know how to handle {}").format(field))
        elif input_field.tag_name == 'textarea':
            self.write_in_element(field, model, data, clear, in_dialog)

    def wizard_screen(self, config_data, next_button='action_next', timeout=30):
        ...
        for config_item in config_data:
            self.enter_data(config_item['field'], config_item['model'], config_item['value'], config_item.get('clear', True), config_item.get('search_column'), True)
    ...
    def _get_data_id_from_column_title(self, column_title, dialog=False):
        ...
        if dialog:
            ...
            xpath = ''
        xpath += ('//table[@class="oe_list_content"]/thead/tr[@class="oe_list_header_columns"]/th/div[normalize-space(text())="{}"]/..').format(column_title)
    ...
    def _get_autocomplete_dropdown_items(self, field_name, model, in_dialog):
        ...
        menu_items_xpath = '//ul[contains(@class, "ui-autocomplete")]/li[contains(@class, "ui-menu-item")]/a'
        menu_items = self.webdriver.find_elements_by_xpath(menu_items_xpath)
        return menu_items

    def search_text_dropdown(self, field_name, model, column_title, value, in_dialog):
        ...
        More... item."""
        menu_items = self._get_autocomplete_dropdown_items(field_name, model, in_dialog)
        try:
    ...
    def create_from_text_dropdown(self, field_name, model, in_dialog, config_data):
        ...
        with self.wait_for_ajax_load():
            elem.click()
        self.wizard_screen(config_data, next_button='oe_form_button_save_and_close')
        ...
        while tries < attempts and elem is None:
            try:
                condition = expected_conditions.visibility_of_element_located((
                 By.XPATH, xpath))
                elem = ui.WebDriverWait(self.webdriver, timeout).until(condition)
    ...
    def click_translate(self, field_name, model):
        ...
        translate_button = elem.find_element_by_xpath('following-sibling::img[contains(@class, "oe_field_translate")]')
        with self.wait_for_ajax_load():
            translate_button.click()

    def go_to_tab(self, tab_name):
        tab = WebDriverWait(self.webdriver, 10).until(expected_conditions.presence_of_element_located((
         By.XPATH,
         ("//a[@class='ui-tabs-anchor' and @data-bt-testing-original-string='{}']").format(tab_name))))
        tab.click()
        time.sleep(1)
...
class View(object):
    ...
    def get_field(self, field_name, model=None):
        return WebDriverWait(self.ui.webdriver, 10).until(expected_conditions.presence_of_element_located((
         By.XPATH,
         ("//*[@data-bt-testing-model_name='{}' and @data-bt-testing-name='{}']").format(model if model else self.model, field_name))))
    ...
    def fill(self, **kwargs):
        ...
        for key, value in kwargs.iteritems():
            ...
            if relational_field:
                add_link = self.ui.webdriver.find_element_by_css_selector('.oe_form_field_one2many_list_row_add a')
