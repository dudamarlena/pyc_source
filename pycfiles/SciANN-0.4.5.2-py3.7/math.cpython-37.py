...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/sciann/utils/math.py
# Compiled at: 2020-04-14 03:25:33
# Size of source mod 2**32: 26772 bytes
...
def pow(f, a):
    ...
    for l in lmbd:
        if isinstance(a, int):
            ...
            raise ValueError('Expected an `int` or `float` for a in x^a. ')
...
def dot(f, other):
    ...
    for fl, fr in zip(f.outputs, other.outputs):
        assert fl.shape.as_list() == fr.shape.as_list(), 'Expected equal dimensions for output of functionals. '
        l = Lambda(lambda x: K.reshape(tf.math.reduce_sum(x * fr, list(range(1, len(fl.shape)))), [-1, 1]))
...
def diag_part(f):
    ...
    outputs = []
    for o in f.outputs:
        assert len(o.shape) == 3, 'Exptected output dimension to be (None, N, N)'
        dim = o.shape[(-1)]
    ...
    Functional = f.get_class()
    res = Functional(inputs=(f.inputs.copy()),
      outputs=outputs,
      layers=lmbd)
    return res
...
def diag(f):
    ...
    outputs = []
    for o in f.outputs:
        assert len(o.shape) == 2, 'Exptected output dimension to be (None, N)'
        dim = o.shape[(-1)]
...
def _apply_operation(lambda_layer, lhs, rhs=None):
    ...
    if is_functional(rhs):
        ...
        try:
            ...
            print('Unsupported operation with an object of type {}. '.format(type(lhs)))
            outputs = None
...
def _apply_function(x, fname, **kwargs):
    ...
    res = Functional(inputs=(x.inputs.copy()),
...
def _gradients(ys, xs, order=1):
    ...
    if ys.shape[(-1)] == 1:
        ...
        for i in range(order):
            ds = unpack_singleton(tf.gradients(ds,
              xs, unconnected_gradients='zero'))
        ...
        for j, y in enumerate(splitted_ys):
            ds.append(y)
            for i in range(order):
                ds[-1] = unpack_singleton(tf.gradients((ds[(-1)]),
                  xs, unconnected_gradients='zero'))
...
def _diag_gradients(ys, xs, order=1):
    ...
    # Returns
        A list of `D^n y / Dx^n` for each y and x in `ys` and `xs`.
    """
    assert ys.shape.as_list() == xs.shape.as_list(), 'Supported when X and Y has the same dimensions - ' + 'Xs:{}, Ys:{}'.format(xs.shape.as_list(), ys.shape.as_list())
        ...
        if gtype == 'Grad':
            ...
            if gtype == 'dGrad':
                ...
                if gtype == 'Div':
                    ...
                    raise ValueError('Unrecognised gradient type: {} \n'.format(type) + '     Please choose among (Grad, dGrad, Div). ')
...
def _gdiff(gtype, f, *args, **kwargs):
    ...
    # Returns
        A new functional object.
    """
    if not is_functional(f):
        raise AssertionError('Please provide a proper functional object. ')
    elif not len(args) in (1, 2):
        raise AssertionError('Expected (`ys`, `xs`) or `xs` as the input, was provided {:d} inputs'.format(len(args)))
    else:
        if not all([isinstance(v, str) or is_functional(v) for v in args]):
            raise ValueError('Expected a `Layer` name for a `Functional` to perform differentitation.')
        try:
            inputs = f.inputs.copy()
            if len(args) == 0:
                x = unpack_singleton(f.inputs)
                assert is_tensor(x), 'multiple inputs detected - please provide an `x` name. '
            ...
            if len(args) <= 1:
                ...
                assert is_tensor(y), 'multiple outputs detected - please provide a `y` name. '
    ...
    if 'order' in kwargs.keys():
        order = kwargs['order']
    lay, tens = _lambda_gradient(y, x, order, gtype, '{}_{}'.format(y_name, x_name))
    Functional = type(f)
    res = Functional(inputs=(unique_tensors(inputs)),
...
def radial_basis(xs, ci, radii):
    ...
    # Returns
        A new functional object.
    """
    if not len(xs) == len(ci):
        raise AssertionError
    else:
        assert radii > 0.0
        assert all([is_variable(x) for x in xs])
        raise isinstance(xs, list) and isinstance(ci, list) or AssertionError
    for x in xs:
        validate_variable(x)
...
def _warn_for_ndarray(other):
    if isinstance(other, np.ndarray):
        Warning('Expecting `Tensor` objects instead of `ndarray`: Note data should go to the training process and this operation may break batch training. ')