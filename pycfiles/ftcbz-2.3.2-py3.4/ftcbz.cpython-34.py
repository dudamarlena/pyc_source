...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ftcbz/ftcbz.py
# Compiled at: 2016-01-01 06:36:11
# Size of source mod 2**32: 17908 bytes
import os.path, zipfile, argparse, os, sys, shutil, subprocess, abc, tempfile
VERSION = '2.3.2'

class Error(Exception):
    __doc__ = 'Base exception class of this module'
...
class Extractor(metaclass=abc.ABCMeta):
    __doc__ = 'ABC which to extract a input target.'
    id = 'Extractor ID'
    description = 'Extractor description'

    class ExtractError(Error):
        __doc__ = 'Extract operation failed'
...
class RarExtractor(Extractor):
    ...
    def extract(self, input_path, extract_folder):
        ...
        def get_cmds(input_path, extract_folder):
            ...
            yield get_cmd(input_path, extract_folder)
...
class Compressor(metaclass=abc.ABCMeta):
    __doc__ = 'ABC which to compress a folder.'
    id = 'Compressor ID'
        ...
        with zipfile.ZipFile(final_path, 'w') as (zfile):
            for path, dirs, filenames in os.walk(source_folder):
                for filename in filenames:
                    ...
                    rel_filename = os.path.relpath(abs_filename, source_folder)
...
class FolderCompressor(Compressor):
    __doc__ = 'folder compressor, do nothing but move the data.'
    id = 'dir'
...
def get_object_paths(comic_folder):
    ...
    object_paths = [os.path.join(comic_folder, obj) for obj in os.listdir(comic_folder)]
    return sorted(object_paths)


def convert(object_path, extractors=None, compressor=CbzCompressor, new_comic_folder=None, replace=False):
    ...
    if os.path.isfile(object_path):
        ...
        volume_name = baseext
    compress_filepath = compressor.get_final_path(new_comic_folder=new_comic_folder, volume_name=volume_name)
    for extractor in extractors:
        if extractor.fit(object_path):
            ...
            with tempfile.TemporaryDirectory() as (tmp_dirname):
                try:
                    extractor.extract(input_path=object_path, extract_folder=tmp_dirname)
                except Extractor.ExtractError as e:
                    print(e)
                    return ('extract_error', compress_filepath)

                compressor.compress(source_folder=tmp_dirname, new_comic_folder=new_comic_folder, volume_name=volume_name)
                return ('archived', compress_filepath)
        else:
            continue

    return (
     'not_fit', compress_filepath)


def get_used_extractors(args):
    extractors = [eclass(passwords=args.passwords) for eclass in Extractor.__subclasses__() if eclass.id in args.extractors]
    return extractors
...
def get_args():
    ...
    def parse_args():
        parser = argparse.ArgumentParser(formatter_class=argparse.RawTextHelpFormatter, description='Freezing a lot of comic directories & files to .cbz format!')
        parser.add_argument('folders', metavar='COMICDIR', type=str, nargs='*', help='Comic book directories.\nEach COMICDIR contain multiple VOLUME dirs or files.\n(which like volume1.rar or vol_1/).')
        parser.add_argument('-a', '--all', metavar='ALLDIR', dest='all', type=str, nargs='+', help='All directories under ALLDIR directly\nwill become COMICDIRs.')
        parser.add_argument('-o', '--output-alldir', metavar='OUTPUT_ALLDIR', dest='output_alldir', type=str, help='If not assign, the files will be generated\nin their source directories.')
        parser.add_argument('-r', '--replace', dest='replace', action='store_const', const=True, default=False, help='Replace target files if the files are already exists.')
        parser.add_argument('-d', '--delete', dest='delete', action='store_const', const=True, default=False, help='Delete data source when archive complete.')
        parser.add_argument('--reverse', dest='reverse', action='store_const', const=True, default=False, help='Reverse standard archive operation. Equal to:\n"--extractors {zipid} --compressor {dirid}"\nIt will overwrite your manual setting.'.format(zipid=ZipExtractor.id, dirid=FolderCompressor.id))
        e_info, e_ids = get_all_extractors_info()
        parser.add_argument('-e', '--extractors', metavar='EXTRACTOR', dest='extractors', action='store', default=[FolderExtractor.id], nargs='*', choices=e_ids, help='\n'.join([
         'Choice the volume format(s) you want to process.',
         'Available formats:',
         e_info,
         '(default: {})'.format(FolderExtractor.id)]))
        c_info, c_ids = get_all_compressors_info()
        parser.add_argument('-c', '--compressor', metavar='COMPRESSOR', dest='compressor', action='store', default=CbzCompressor.id, choices=c_ids, help='\n'.join([
         'Choice a output format.',
         'Available formats:',
         c_info,
         '(default: %(default)s)']))
        parser.add_argument('-p', '--passwords', metavar='PW', dest='passwords', action='store', default=[], nargs='*', help='One or more unzip passwords for source files.')
...
def main():
    ...
    for ori_comic_folder in get_ori_comic_folders(args):
        ...
        for object_path in get_object_paths(ori_comic_folder):
            status, final_path = convert(object_path=object_path, extractors=extractors, compressor=compressor, new_comic_folder=new_comic_folder, replace=args.replace)
