...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/dust/invite/win32_inet_pton.py
# Compiled at: 2009-11-10 16:41:02
import socket, struct, ctypes, ctypes.wintypes
SOCKET_ERROR = -1
...
def inet_pton(af, src):
    ...
    r = ctypes.windll.ws2_32.WSAStringToAddressA(src.encode('ascii'), af, 0, ctypes.byref(dst), ctypes.byref(dstlen))
    if r == SOCKET_ERROR:
...
def inet_ntop(af, src):
    ...
    if af == socket.AF_INET:
        ...
        sa.sin_port = 0
        ctypes.memmove(ctypes.pointer(sa.sin_addr), src, ctypes.sizeof(sa.sin_addr))
    else:
        ...
        sa.sin6_port = 0
        sa.sin6_flowinfo = 0
        ctypes.memmove(ctypes.pointer(sa.sin6_addr), src, ctypes.sizeof(sa.sin6_addr))
    dst = ctypes.create_string_buffer(40)
    dstlen = ctypes.wintypes.DWORD(len(dst))
    r = ctypes.windll.ws2_32.WSAAddressToStringA(ctypes.byref(ss), srclen, 0, ctypes.byref(dst), ctypes.byref(dstlen))
    if r == SOCKET_ERROR:
        raise ctypes.WinError()
