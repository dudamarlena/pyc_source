...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ztfy/webapp/webapp_base/template.py
# Compiled at: 2011-12-19 03:34:51
from HTMLParser import HTMLParser
from paste.script import templates
from paste.script.templates import var
from urllib2 import urlopen
import pkg_resources, re, os, sys
...
class Webapp(templates.Template):
    ...
    vars = [
     var('python_package', 'Main Python package (without namespace)'),
     var('interpreter', 'Name of custom Python interpreter', default='ztpy'),
     var('release', 'Which release of ZTFY.webapp?\nCheck on %s' % DOWNLOAD_URL),
     var('version', 'Version of your project', default='0.1'),
    ...
    def check_vars(self, vars, cmd):
        ...
        try:
            if 'offline' not in vars:
                ...
                all_versions = sorted(parser.versions, key=lambda v: parse_version(v), reverse=True)
                ...
                print str(latest) + '\n'
                last_change = '1.1.0'
                for line in urlopen(DOWNLOAD_URL + 'ztfy.webapp-%s.cfg' % latest).readlines():
                    if 'LAST_TEMPLATE_CHANGE' in line:
                        last_change = line.split('=')[1].strip()
                        break

                if parse_version(last_change) > parse_version(current):
                    print '**WARNING**: the project template for ZTFY.webapp has changed since version %s.\nYou should upgrade to this version.\n' % last_change
        except IOError:
            print '**WARNING**: error while getting the latest version online'
            print 'Please check that you can access %s\n' % DOWNLOAD_URL
        ...
        for var in self.vars:
            ...
            if var.name == 'python_package':
                ...
                if '.' in vars['project'][1:-1]:
                    var.default = re.sub('[^A-Za-z0-9.]+', '_', vars['project']).lower()

        vars = templates.Template.check_vars(self, vars, cmd)
        vars['package'] = vars['python_package']
        if vars['package'] in ('bluebream', 'bream', 'zope'):
            print
            print 'Error: The chosen project name results in an invalid package name: %s.' % vars['package']
    ...
    def pre(self, command, output_dir, vars):
        ...
        vars['namespace_packages'] = list(reversed([ vars['python_package'].rsplit('.', i)[0] for i in range(1, len(self.ns_split))
                                                   ]))
    ...
    def post(self, command, output_dir, vars):
        ...
        if len(self.ns_split) > 1:
            ...
            target_dir = os.path.join(output_dir, 'src', os.path.join(*self.ns_split[:-1]))
            os.makedirs(target_dir)
            ns_decl = "__import__('pkg_resources').declare_namespace(__name__)"
            for i, namespace_package in enumerate(self.ns_split[:-1]):
                init_file = os.path.join(output_dir, 'src', os.path.join(*self.ns_split[:i + 1]), '__init__.py')
                open(init_file, 'w').write(ns_decl)

            os.rename(tmp_dir, os.path.join(target_dir, os.path.basename(package_dir)))
