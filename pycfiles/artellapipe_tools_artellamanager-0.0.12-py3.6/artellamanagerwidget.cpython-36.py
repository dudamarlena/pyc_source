...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/tools/artellamanager/widgets/artellamanagerwidget.py
# Compiled at: 2020-05-13 18:51:46
# Size of source mod 2**32: 49000 bytes
"""
        ...
        if not status:
            if self._path:
                ...
                if not status:
                    ...
                    return hasattr(status, 'references') or None
...
class ArtellaManagerWidget(base.BaseWidget, object):
    ...
    def _stop_threads(self):
        if self._get_files_thread:
            if self._get_files_worker:
                if self._get_files_thread.isRunning():
                    self._get_files_worker.abort()
            if self._get_dirs_thread:
                if self._get_dirs_worker:
                    if self._get_dirs_thread.isRunning():
                        self._get_dirs_worker.abort()
        else:
            if self._get_folder_status_thread:
                if self._get_status_worker:
                    if self._get_folder_status_thread.isRunning():
    ...
    def _update_selected_folder_files(self, status):
        ...
        if not status:
            ...
            message.PopupMessage.error(text='Impossible to retrieve data from Artella. Maybe Artella is down.',
              duration=5,
              parent=self)
            self.METADATA = artellalib.get_metadata()
        ...
        if isinstance(status, artellaclasses.ArtellaDirectoryMetaData):
            for ref_name, ref_data in status.references.items():
                dir_path = ref_data.path
                if not os.path.isdir(dir_path):
                    if not os.path.splitext(dir_path)[(-1)]:
                        pass
                    else:
    ...
    def _on_refresh_selected_folder(self, selected_indexes=None):
        if self._get_folder_status_thread:
            ...
            selected_indexes or self._files_list.clear()
            ...
            if model:
                ...
                if folder_files_dict:
                    for folder_file_name, folder_file_path in folder_files_dict.items():
                        ...
                        if not list_item.is_directory:
                            if list_item.is_deleted:
                                pass
                            else:
    ...
    def _on_get_dependencies_file(self, item_path):
        ...
        if res == QMessageBox.StandardButton.Yes:
            artellapipe.ToolsMgr().run_tool('artellapipe-tools-dependenciesmanager',
              do_reload=True, debug=False, file_path=item_path)
