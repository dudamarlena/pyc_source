...
def upgrade():
    ...
    with op.batch_alter_table('sql_metrics', naming_convention=conv) as (batch_op):
        batch_op.create_unique_constraint('uq_sql_metrics_metric_name', [
...
def downgrade():
    ...
    with op.batch_alter_table('sql_metrics', naming_convention=conv) as (batch_op):
        batch_op.drop_constraint((generic_find_uq_constraint_name('sql_metrics', {
         'metric_name', 'table_id'}, insp) or 'uq_sql_metrics_table_id'),
          type_='unique')
    with op.batch_alter_table('sql_metrics') as (batch_op):
        batch_op.alter_column('metric_name',
          existing_type=(String(255)),
