...
# [GCC 8.4.0]
# Embedded file name: /Users/mardochee.macxis/Projects/Python/pilot-lib/pilot_lib/views.py
# Compiled at: 2015-04-19 13:45:10
import os
from hashlib import sha1
import time, base64, hmac, urllib, pkg_resources, functools, re, datetime, humanize, jinja2, utils
from flask import abort, redirect, request, url_for, jsonify, session
...
class InitPilot(object):
    ...
    def setup_config(self):
        ...
        if self.app.config.get('PILOT_USER_LOGIN_OAUTH_ENABLE'):
            ...
            if _sl and isinstance(_sl, dict):
                ...
                _pilot['login_oauth_credentials'] = {'client_ids': client_ids, 
                   'buttons': buttons}
        Pilot.__(pilot=_pilot, year=datetime.datetime.now().year)
    ...
    def register_templates(self):
        ...
        env.register('pilot_js', Bundle('pilot/js/s3upload.js', 'pilot/js/hello.js', 'pilot/js/pilot.js', output='pilot.js'))
        env.register('pilot_css', Bundle('pilot/css/pilotlib_style.css', 'pilot/css/bootstrap-social-btns.css', output='pilot.css'))
...
class S3UploadMixin(object):
    ...
    def sign_s3_upload(self):
        ...
        return jsonify({'signed_request': '%s?AWSAccessKeyId=%s&Expires=%d&Signature=%s' % (url, AWS_ACCESS_KEY, expires, signature), 
           'url': url})


def login_view(model, logout_view=None, signin_view=None, login_message='Please Login', allow_signup=True, view_template=None):
    """
    ...
    class Login(S3UploadMixin):
        ...
        def signup_handler(cls):
            ...
            if request.method == 'POST':
                ...
                if not name:
                    ...
                    return User.new(email=email, password=password.strip(), name=name, profile_pic_url=profile_pic_url, signup_method='EMAIL')
            return
        ...
        def change_login_handler(cls, user_context=None, email=None):
            ...
            if not utils.is_valid_email(email):
                raise UserWarning("Invalid email address '%s'" % email)
            elif email != user_context.email and User.get_by_email(email):
        ...
        def reset_password_handler(cls, user_context=None, send_notification=False):
            ...
            if send_notification:
                tmail.send('reset-password', to=user_context.email, new_password=new_password, name=user_context.name, login_url=url_for(login_manager.login_view, _external=True))
            return new_password
        ...
        def login(self):
            ...
            self._can_login()
            logout_user()
            return self.render(login_url_next=request.args.get('next', ''), view_template=template_page % 'login')
        ...
        def oauth_login(self):
            ...
            if not email:
                session[self.SESSION_KEY_SET_EMAIL_DATA] = {'type': 'social_login', 'email': email, 
                   'name': name, 
                   'provider': provider, 
            ...
            user = User.oauth_register(provider=provider, provider_user_id=provider_user_id, email=email, name=name, image_url=image_url, signup_method='SOCIAL:%s' % provider.upper())
            if user:
                login_user(user)
        ...
        def oauth_connect(self):
            ...
            next = request.form.get('next', '')
            try:
                current_user.oauth_connect(provider=provider, provider_user_id=provider_user_id, email=email, name=name, image_url=image_url)
        ...
        def lost_password_post(self):
            ...
            if user:
                self.reset_password_handler(user_context=user, send_notification=True)
        ...
        def set_emailp(self):
            ...
            if email and self.SESSION_KEY_SET_EMAIL_DATA in session:
                ...
                if user:
                    ...
                    User.new(email=email, name=_data['name'], signup_method=_data['signup_method'] if 'signup_method' in _data else '')
                    if 'type' in _data:
                        if _data['type'] == 'social_login':
                            user = User.social_login(provider=_data['provider'], provider_user_id=_data['provider_user_id'], email=email, name=_data['name'], image_url=_data['image_url'])
                            return redirect(request.form.get('next') or url_for(signin_view))
                return redirect(url_for('Login:set_email', next=request.form.get('next', '')))
        ...
        def signup(self):
            ...
            self._can_signup()
            logout_user()
            return self.render(login_url_next=request.args.get('next', ''), view_template=template_page % 'signup')
...
def user_admin_view(model, login_view, view_template=None):
    ...
    class UserAdmin(object):
        ...
        def search_handler(cls, per_page=20):
            ...
            cls.__(user_admin=dict(form=AdminForm(), users=users, search_query={'excluded_deleted': request.args.get('show-deleted'), 
        ...
        def reset_password(self):
            try:
                ...
                password = LoginView.reset_password_handler(user_context=user, send_notification=True)
...
jinja2.filters.FILTERS.update({'to_currency': utils.to_currency, 
   'strip_decimal': strip_decimal, 
   'to_date': to_date, 
   'to_int': int, 
   'to_slug': utils.to_slug, 
   ...
   'to_naturalsize': humanize.naturalsize, 
