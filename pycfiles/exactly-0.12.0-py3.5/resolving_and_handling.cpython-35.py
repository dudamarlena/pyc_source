...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/cli/program_modes/help/request_handling/resolving_and_handling.py
# Compiled at: 2019-12-27 10:07:29
# Size of source mod 2**32: 4981 bytes
...
from exactly_lib.cli.program_modes.help.program_modes.main_program.request_rendering import MainProgramHelpRendererResolver
...
from exactly_lib.cli.program_modes.help.program_modes.test_suite.request_rendering import TestSuiteHelpConstructorResolver
...
def handle_help_request(output: StdOutputFiles, application_help: ApplicationHelp, help_request: HelpRequest):
    handler = _resolve_handler(application_help, help_request)
    handler.handle(output)


def handle_help_request_rr(application_help: ApplicationHelp, help_request: HelpRequest) -> ProcessResultReporter:
    handler = _resolve_handler(application_help, help_request)
...
def _resolve_handler(application_help: ApplicationHelp, request: HelpRequest) -> RequestHandler:
...
def _renderer(application_help: ApplicationHelp, request: HelpRequest) -> SectionContentsConstructor:
...
def _entity_help_request_renderer_resolver_for(application_help: ApplicationHelp, request: EntityHelpRequest) -> EntityHelpRequestRendererResolver:
    try:
    ...
    return EntityHelpRequestRendererResolver(entity_conf.entity_doc_2_article_contents_renderer, entity_conf.cli_list_constructor_getter, entity_conf.entities_help.all_entities)