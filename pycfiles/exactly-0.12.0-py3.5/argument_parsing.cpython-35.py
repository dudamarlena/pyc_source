...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/cli/program_modes/test_suite/argument_parsing.py
# Compiled at: 2019-09-20 02:11:24
# Size of source mod 2**32: 8179 bytes
...
from exactly_lib.cli.program_modes.common.argument_parsing_of_actor import resolve_actor_from_argparse_argument
...
from exactly_lib.util.argument_parsing_utils import ArgumentParsingError, parse_args__raise_exception_instead_of_exiting_on_error, parse_known_args__raise_exception_instead_of_exiting_on_error
from exactly_lib.util.cli_syntax import short_and_long_option_syntax
from .settings import TestSuiteExecutionSettings

def parse(default: TestCaseHandlingSetup, argv: List[str]) -> TestSuiteExecutionSettings:
    """
...
def parse_known_args(default: TestCaseHandlingSetup, argv: List[str]) -> Tuple[(TestSuiteExecutionSettings, List[str])]:
    """
    :raises ArgumentParsingError Invalid usage
    """
...
class _Parser:
    ...
    def __init__(self, default: TestCaseHandlingSetup):
        ...
        self.reporter_name_2_reporter = {reporters.JUNIT_REPORTER.singular_name: JUnitRootSuiteProcessingReporter, 
    ...
    def parse(self, argv: List[str]) -> TestSuiteExecutionSettings:
        ...
        namespace = parse_args__raise_exception_instead_of_exiting_on_error(argument_parser, argv)
    ...
    def parse_known_args(self, argv: List[str]) -> Tuple[(TestSuiteExecutionSettings, List[str])]:
        argument_parser = self._new_argument_parser()
        namespace, remaining_args = parse_known_args__raise_exception_instead_of_exiting_on_error(argument_parser, argv)
    ...
    def _execution_settings(self, namespace: argparse.Namespace) -> TestSuiteExecutionSettings:
        actor = resolve_actor_from_argparse_argument(self.default.actor, namespace.actor)
        suite_file_path = self._resolve_file_path(namespace.file)
        return TestSuiteExecutionSettings(self._resolve_reporter(vars(namespace)), TestCaseHandlingSetup(ActPhaseSetup(actor), self.default.preprocessor), suite_file_path)
    ...
    def _resolve_file_path(self, file_argument: str) -> pathlib.Path:
        ...
        if stat.S_ISDIR(stat_mode):
            ...
            if suite_file_path.is_file():
                return suite_file_path
            raise ArgumentParsingError('{} is a directory, but do not contain the default suite file {}'.format(file_argument, file_names.DEFAULT_SUITE_FILE))
        else:
    ...
    def _new_argument_parser(self) -> argparse.ArgumentParser:
        ret_val = argparse.ArgumentParser(description=misc_texts.SUITE_COMMAND_SINGLE_LINE_DESCRIPTION, prog=program_info.PROGRAM_NAME + ' ' + common_opts.SUITE_COMMAND)
        ret_val.add_argument('file', metavar=opts.TEST_SUITE_FILE_ARGUMENT, type=str, help='The test suite file.')
        ret_val.add_argument(short_and_long_option_syntax.long_syntax(opts.OPTION_FOR_REPORTER__LONG), metavar=opts.REPORTER_OPTION_ARGUMENT, nargs=1, choices=self.reporter_names, help=self._reporter_option_description())
        ret_val.add_argument(short_and_long_option_syntax.long_syntax(common_opts.OPTION_FOR_ACTOR__LONG), metavar=common_opts.ACTOR_OPTION_ARGUMENT, nargs=1, help=_ACTOR_OPTION_DESCRIPTION.format(ARGUMENT=common_opts.ACTOR_OPTION_ARGUMENT, INTERPRETER_ACTOR_TERM=formatting.entity(actors.SOURCE_INTERPRETER_ACTOR.singular_name), ACTOR_CONCEPT=formatting.concept_(concepts.ACTOR_CONCEPT_INFO), shell_syntax_concept=formatting.concept_(concepts.SHELL_SYNTAX_CONCEPT_INFO)))
        return ret_val

    def _reporter_option_description(self) -> str:
        formatted_names = [formatting.cli_argument_option_string(name) for name in self.reporter_names]
        formatted_default_name = formatting.cli_argument_option_string(self.default_reporter_name)
        help_option = ' '.join(help_args.complete_help_for(help_args.concept_single(concepts.SUITE_REPORTER_CONCEPT_INFO.singular_name)))
        return _REPORTER_OPTION_DESCRIPTION_TEMPLATE.format(reporter_names=','.join(formatted_names), default_reporter_name=formatted_default_name, help_option=help_option)


_REPORTER_OPTION_DESCRIPTION_TEMPLATE = 'How to report the result of the suite. Options: {reporter_names} (default {default_reporter_name}).\nUse "{help_option}" for more info.\n'
_ACTOR_OPTION_DESCRIPTION = 'Specifies the {INTERPRETER_ACTOR_TERM} {ACTOR_CONCEPT}, by giving the program that serves as the interpreter.\n\n{ARGUMENT} is an absolute path followed by optional arguments (using {shell_syntax_concept}).\n\nNote that an {ACTOR_CONCEPT} specified in the test suite or individual test cases\nhas precedence over the {ACTOR_CONCEPT} given here.\n'