...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/cli/program_modes/help/util/argument_value_lookup.py
...
def entities_key_value_iter(entities_help: EntityTypeHelp) -> Iterable[Tuple[(str, EntityDocumentation)]]:
    return map(lambda entity_doc: (entity_doc.singular_name(), entity_doc), entities_help.all_entities)


def lookup_argument__dict(object_name: str, argument_pattern: str, elements: Dict[(str, T)]) -> Match[T]:
    return lookup_argument(object_name, argument_pattern, list(elements.items()))


def lookup_argument(object_name: str, argument_pattern: str, elements: Iterable[Tuple[(str, T)]]) -> Match[T]:
    try:
