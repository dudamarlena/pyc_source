...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_case_utils/string_matcher/parse/parts/equality.py
# Compiled at: 2020-01-29 10:35:17
...
def parse(expectation_type: ExpectationType, token_parser: TokenParser) -> StringMatcherSdv:
    return StringMatcherSdv(combinator_sdvs.of_expectation_type(parse__generic(token_parser), expectation_type))
...
def parse__generic(token_parser: TokenParser) -> GenericStringMatcherSdv:
    ...
    expected_contents = parse_here_doc_or_path.parse_from_token_parser(token_parser, EXPECTED_FILE_REL_OPT_ARG_CONFIG, consume_last_here_doc_line=False)
...
class Description(grammar.SimpleExpressionDescription):
    ...
    def __init__(self):
        self._string_or_here_doc_or_file_arg = StringOrHereDocOrFile(_EXPECTED_PATH_NAME, _RELATIVITY_OF_EXPECTED_PATH_NAME, parse_path.all_rel_options_config(_EXPECTED_PATH_NAME), the_path_of('the file that contains the expected contents.'))

    @property
    def argument_usage_list(self) -> Sequence[a.ArgumentUsage]:
        return (
         self._string_or_here_doc_or_file_arg.argument_usage(a.Multiplicity.MANDATORY),)

    @property
    def description_rest(self) -> Sequence[ParagraphItem]:
        tp = TextParser({'HERE_DOCUMENT': formatting.syntax_element_(syntax_elements.HERE_DOCUMENT_SYNTAX_ELEMENT), 
    ...
    def syntax_elements(self) -> Sequence[SyntaxElementDescription]:
        return [
         relative_path_options_documentation.path_element_2(self._string_or_here_doc_or_file_arg.path_argument_configuration)]
