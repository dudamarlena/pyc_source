...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/util/textformat/rendering/text/table/formatter.py
...
from exactly_lib.util.textformat.rendering.text.table.column_max_width import derive_column_max_widths
from exactly_lib.util.textformat.rendering.text.table.width_distribution import distribute_width
from exactly_lib.util.textformat.structure.table import Table, empty_cell

class TableFormatter:
    __doc__ = '\n    Formats a single Table.\n    '
    UNDER_LINE_CHARACTER = '-'

    def __init__(self, cell_formatter_for_given_width: types.FunctionType, available_width: int, table: Table):
        ...
        self.available_width_for_column_contents = available_width - self.num_column_separators * len(table.format.column_separator)
    ...
    def _derive_column_content_widths(self) -> list:
        columns_with_max_line_width = derive_column_max_widths(self.cell_formatter_for_given_width, self.available_width_for_column_contents, self.columns)
    ...
    def _combine_cell_contents_into_lines(self, row_cell_lines: list, column_widths: List[int]) -> List[str]:
