...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_case_utils/regex/parse_regex.py
...
MISSING_STRING_ARGUMENT_FOR_REGEX_ERR_MSG = 'Missing {} argument for {}'.format(syntax_elements.STRING_SYNTAX_ELEMENT.argument.name, syntax_elements.REGEX_SYNTAX_ELEMENT.argument.name)

def regex_parser() -> Parser[RegexSdv]:
    return _PARSER


def parse_regex(parser: TokenParser, must_be_on_same_line: bool=True,
...
def parse_regex2(parser: TokenParser, must_be_on_same_line: bool=True,
    ...
    is_ignore_case = parser.consume_and_handle_optional_option(False, lambda x: True, IGNORE_CASE_OPTION_NAME)
    if must_be_on_same_line:
        parser.require_is_not_at_eol(MISSING_REGEX_ARGUMENT_ERR_MSG)
    source_type, regex_pattern = parse_string_or_here_doc_from_token_parser(parser, consume_last_here_doc_line)
...
class _RegexSdv(RegexSdv):
    ...
    def __init__(self, is_ignore_case: bool, string: StringSdv):
    ...
    def resolve(self, symbols: SymbolTable) -> RegexDdv:
        return _RegexDdv(self._is_ignore_case, self._string.resolve(symbols))


class _ValidatorWhichCreatesRegex(DdvValidator):

    def __init__(self, is_ignore_case: bool, string: StringDdv):
        self._is_ignore_case = is_ignore_case
        self.string = string
        self.pattern = None
    ...
    def _compile_and_set_pattern(self, regex_pattern: str) -> Optional[TextRenderer]:
        try:
            ...
            return text_docs.single_line(strings.FormatPositional("Invalid {}: '{}'", instruction_arguments.REG_EX.name, ex))


class _RegexDdv(RegexDdv):

    def __init__(self, is_ignore_case: bool, string: StringDdv):
        self._describer = custom_details.regex(is_ignore_case, details.String(string_rendering.AsToStringObject(string.describer())))
