...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/util/textformat/section_target_hierarchy/targets.py
# Compiled at: 2018-09-20 06:11:36
...
class TargetInfo(tuple):
    ...
    def __new__(cls, presentation: core.StringText, target: core.CrossReferenceTarget):
    ...
    def anchor_text(self) -> core.Text:
        return anchor_text(self.presentation_text, self.target)


class TargetInfoNode(tuple):

    def __new__(cls, data: TargetInfo, children: list):
        """
...
class TargetInfoFactory:
    __doc__ = 'Build a hierarchical structure of targets to appear in a TOC.'
...
class TargetInfoFactoryWithFixedRoot(TargetInfoFactory):
    ...
    def __init__(self, fixed_root_target: core.CrossReferenceTarget, original: TargetInfoFactory):
        self._fixed_root_target = fixed_root_target
        self._original = original

    def root(self, presentation: core.StringText) -> TargetInfo:
        return TargetInfo(presentation, self._fixed_root_target)
