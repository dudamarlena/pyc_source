...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_case/result/svh.py
...
class SuccessOrValidationErrorOrHardError(tuple):
    ...
    def __new__(cls, is_hard_error: Optional[bool], failure_message: Optional[TextRenderer]):
...
def new_svh_validation_error__str(failure_message: str) -> SuccessOrValidationErrorOrHardError:
    if failure_message is None:
        raise ValueError('A VALIDATION ERROR must have a failure message (that is not None)')
    return SuccessOrValidationErrorOrHardError(False, text_docs.single_pre_formatted_line_object(failure_message))
...
def new_svh_hard_error__str(failure_message: str) -> SuccessOrValidationErrorOrHardError:
    if failure_message is None:
        raise ValueError('A HARD ERROR must have a failure message (that is not None)')
    return SuccessOrValidationErrorOrHardError(True, text_docs.single_pre_formatted_line_object(failure_message))
