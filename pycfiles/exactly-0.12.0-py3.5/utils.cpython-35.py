...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/util/textformat/utils.py
...
def append_section_if_contents_is_non_empty(output_list_of_sections: List[doc.SectionItem], section_title_str_or_text, initial_paragraphs_or_section_contents: InitialParagraphsOrSectionContents):
    ...
    if not section_contents.is_empty:
        output_list_of_sections.append(doc.Section(docs.text_from_unknown(section_title_str_or_text), section_contents))


def append_sections_if_contents_is_non_empty(output_list_of_sections: List[doc.SectionItem], section_name__contents: List[Tuple[(str, InitialParagraphsOrSectionContents)]]):
    """
...
def transform_list_to_table(l: lists.HeaderContentList) -> table.Table:
    ...
    return table.Table(table.TableFormat(), rows)
