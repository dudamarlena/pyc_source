...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_case_utils/parse/parse_string.py
...
class Configuration:
    ...
    def __init__(self, argument_name: str, reference_restrictions: Optional[ReferenceRestrictions]=None):
        self.argument_name = argument_name
        self.reference_restrictions = reference_restrictions


DEFAULT_CONFIGURATION = Configuration(types.STRING_TYPE_INFO.identifier, reference_restrictions=None)

def parse_string_sdv_from_parse_source(source: ParseSource, conf: Configuration=DEFAULT_CONFIGURATION) -> StringSdv:
    """
...
def parse_string_from_token_parser(token_parser: TokenParser, conf: Configuration=DEFAULT_CONFIGURATION) -> StringSdv:
    """
...
def parse_rest_of_line_as_single_string(token_parser: TokenParser, strip_space: bool=False) -> StringSdv:
...
def parse_rest_of_line_as_single_string_and_consume_line(token_parser: TokenParser, strip_space: bool=False) -> StringSdv:
...
def parse_string_sdv(tokens: TokenStream, conf: Configuration=DEFAULT_CONFIGURATION) -> StringSdv:
    """
    :raises SingleInstructionInvalidArgumentException: Invalid arguments
    """
...
def parse_string_sdv_from_token(token: Token, reference_restrictions: ReferenceRestrictions=None) -> StringSdv:
    fragments = parse_fragments_from_token(token)
    return string_sdv_from_fragments(fragments, reference_restrictions)


def parse_fragments_from_tokens(tokens: TokenStream, conf: Configuration=DEFAULT_CONFIGURATION) -> List[symbol_syntax.Fragment]:
    """
...
def string_sdv_from_string(source: str, reference_restrictions: ReferenceRestrictions=None) -> StringSdv:
    fragments = symbol_syntax.split(source)
    return string_sdv_from_fragments(fragments, reference_restrictions)


def fragment_sdv_from_fragment(fragment: symbol_syntax.Fragment, reference_restrictions: ReferenceRestrictions) -> StringFragmentSdv:
...
def string_sdv_from_fragments(fragments: Sequence[symbol_syntax.Fragment], reference_restrictions: Optional[ReferenceRestrictions]=None) -> StringSdv:
    if reference_restrictions is None:
        reference_restrictions = is_any_data_type()
    return StringSdv(tuple([fragment_sdv_from_fragment(f, reference_restrictions) for f in fragments]))