...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/util/process_execution/sub_process_execution.py
...
class Result(tuple):
    ...
    def __new__(cls, error_message: str, exit_code: int, output_dir_path: pathlib.Path):
...
class ResultAndStderr:
    ...
    def __init__(self, result: Result, stderr_contents: str):
        self.result = result
...
class ExecutorThatStoresResultInFilesInDir:
    ...
    def execute(self, storage_dir: pathlib.Path, executable: Executable) -> Result:
        ...
        with open(str(storage_dir / process_output_files.STDOUT_FILE_NAME), 'w') as (f_stdout):
            with open(str(storage_dir / process_output_files.STDERR_FILE_NAME), 'w') as (f_stderr):
                try:
                    exit_code = subprocess.call(executable.arg_list_or_str, stdin=subprocess.DEVNULL, stdout=f_stdout, stderr=f_stderr, env=self.process_execution_settings.environ, timeout=self.process_execution_settings.timeout_in_seconds, shell=executable.is_shell)
                    write_new_text_file(storage_dir / process_output_files.EXIT_CODE_FILE_NAME, str(exit_code))
                    return Result(None, exit_code, storage_dir)
...
def execute_and_read_stderr_if_non_zero_exitcode(executable: Executable, executor: ExecutorThatStoresResultInFilesInDir, storage_dir: pathlib.Path) -> ResultAndStderr:
