...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_case_utils/line_matcher/parse_line_matcher.py
# Compiled at: 2020-01-29 09:08:19
# Size of source mod 2**32: 5512 bytes
...
CONSTANT_TRUE_MATCHER_SDV = LineMatcherSdv(sdv_components.matcher_sdv_from_constant_primitive(constant.MatcherWithConstantResult(True)))
...
def parse_line_matcher_from_token_parser(parser: TokenParser, must_be_on_current_line: bool=True) -> LineMatcherSdv:
    return LineMatcherSdv(parse_line_matcher_from_token_parser__generic(parser, must_be_on_current_line))


def parse_line_matcher_from_token_parser__generic(parser: TokenParser, must_be_on_current_line: bool=True) -> GenericLineMatcherSdv:
    return parse_expression.parse(GRAMMAR, parser, must_be_on_current_line=must_be_on_current_line)


ADDITIONAL_ERROR_MESSAGE_TEMPLATE_FORMATS = {'_REG_EX_': REPLACE_REGEX_ARGUMENT.name, 
 '_STRING_': REPLACE_REPLACEMENT_ARGUMENT.name}
_HELP_TEXT_TEMPLATE_FORMATS = ADDITIONAL_ERROR_MESSAGE_TEMPLATE_FORMATS.copy()
_HELP_TEXT_TEMPLATE_FORMATS.update({'FIRST_LINE_NUMBER': FIRST_LINE_NUMBER, 
...
_LINE_NUMBER_MATCHER_SED_DESCRIPTION = 'Matches {MODEL:s} with a given line number.\n\n\nLine numbers start at {FIRST_LINE_NUMBER}.\n'

class _RegexSyntaxDescription(grammar.SimpleExpressionDescription):

    @property
    def argument_usage_list(self) -> Sequence[a.ArgumentUsage]:
        return [
         a.Single(a.Multiplicity.MANDATORY, REPLACE_REGEX_ARGUMENT)]
...
class _LineNumberSyntaxDescription(grammar.SimpleExpressionDescription):
...
_CONCEPT = grammar.Concept(types.LINE_MATCHER_TYPE_INFO.name, types.LINE_MATCHER_TYPE_INFO.identifier, syntax_elements.LINE_MATCHER_SYNTAX_ELEMENT.argument)
GRAMMAR = standard_expression_grammar.new_grammar(_CONCEPT, model=matcher_model.LINE_MATCHER_MODEL, value_type=ValueType.LINE_MATCHER, simple_expressions=(
 NameAndValue(line_matcher.REGEX_MATCHER_NAME, grammar.SimpleExpression(matches_regex.parse__generic, _RegexSyntaxDescription())),
 NameAndValue(line_matcher.LINE_NUMBER_MATCHER_NAME, grammar.SimpleExpression(line_number.parse_line_number__generic, _LineNumberSyntaxDescription()))))