...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_case_utils/err_msg/path_rendering.py
...
class PathRepresentationsRenderers(ABC):
    __doc__ = 'Gives renderers for each path representation that should be displayed'
...
class PathRepresentationsRenderersForPrimitive(PathRepresentationsRenderers):
    __doc__ = 'Renders DDV, and optional primitive if is rel HDS'
...
class PathDdvLines(SequenceRenderer[LineElement]):
    ...
    def __init__(self, path: PathDescriberForPrimitive, header_line: Optional[Any]=None):
        self._path = path
...
class PathDdvMinorBlock(Renderer[MinorBlock]):
    ...
    def render(self) -> MinorBlock:
        return MinorBlock(PathDdvLines(self._path, self._header_line).render_sequence())
...
def path_renderers(path: PathDescriberForPrimitive) -> List[Renderer[str]]:
    if path.resolving_dependency is DirectoryStructurePartition.HDS:
        return [path.value, path.primitive]
    return [
     path.value]


def path_strings(path: PathDescriberForPrimitive) -> List[str]:
    return [r.render() for r in path_renderers(path)]


class PathMinorBlock(Renderer[MinorBlock]):

    def __init__(self, path: PathRepresentationsRenderers):
        self._path = path

    def render(self) -> MinorBlock:
        return MinorBlock([text_struct.LineElement(text_struct.StringLineObject(line.render())) for line in self._path.renders()], text_struct.ELEMENT_PROPERTIES__INDENTED)


class ExplanationMinorBlock(Renderer[MinorBlock]):

    def __init__(self, explanation: SequenceRenderer[LineElement]):
        self._explanation = explanation

    def render(self) -> MinorBlock:
        return MinorBlock(self._explanation.render_sequence(), text_struct.ELEMENT_PROPERTIES__INDENTED)


class HeaderAndPathMinorBlocks(SequenceRenderer[MinorBlock]):

    def __init__(self, header: Renderer[MinorBlock], path: PathRepresentationsRenderers, explanation: Optional[SequenceRenderer[LineElement]]=None):
        self._header = header
        self._path = path
        self._explanation = explanation
...
class HeaderAndPathMajorBlock(Renderer[MajorBlock]):
    ...
    def render(self) -> MajorBlock:
        minor_blocks_renderer = HeaderAndPathMinorBlocks(self._header, self._path, self._explanation)
...
class HeaderAndPathMajorBlocks(SequenceRenderer[MajorBlock]):
    ...
    def render_sequence(self) -> Sequence[MajorBlock]:
        major_block_renderer = HeaderAndPathMajorBlock(self._header, self._path, self._explanation)
        return [
