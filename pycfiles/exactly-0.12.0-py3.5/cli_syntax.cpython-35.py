...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/help/program_modes/test_suite/contents/cli_syntax.py
...
def root(header: str) -> SectionHierarchyGenerator:
    return h.with_fixed_root_target(PredefinedHelpContentsPartReference(HelpPredefinedContentsPart.TEST_SUITE_CLI), h.leaf(header, ProgramDocumentationSectionContentsConstructor(TestSuiteCliSyntaxDocumentation())))
...
class TestSuiteCliSyntaxDocumentation(CliProgramSyntaxDocumentation):
    ...
    def description(self) -> DescriptionWithSubSections:
        return DescriptionWithSubSections(_TP.text(misc_texts.SUITE_COMMAND_SINGLE_LINE_DESCRIPTION), docs.SectionContents(self.synopsis.paragraphs + _TP.fnap(_DESCRIPTION_PARAGRAPH), []))

    def synopsises(self) -> List[cli_syntax.Synopsis]:
        return [cli_syntax.Synopsis(command_line) for command_line in self.synopsis.command_lines]
    ...
    def outcome(self, environment: ConstructionEnvironment) -> Optional[docs.SectionContents]:
        contents_constructor = SectionContentsConstructorFromHierarchyGenerator(outcome.root('unused'))
        ret_val = contents_constructor.apply(environment)
        return ret_val

    def _actor_argument(self) -> cli_syntax.DescribedArgument:
        extra_format_map = {'interpreter_program': _ACTOR_OPTION.argument}
        return cli_syntax.DescribedArgument(_ACTOR_OPTION, _TP.fnap(_ACTOR_OPTION_DESCRIPTION, extra_format_map), see_also_items=see_also_items_from_cross_refs([
    ...
    def _reporter_argument(self) -> cli_syntax.DescribedArgument:
        return cli_syntax.DescribedArgument(_REPORTER_OPTION, _TP.fnap(_REPORTER_OPTION_DESCRIPTION), see_also_items=see_also_items_from_cross_refs([
         concepts.SUITE_REPORTER_CONCEPT_INFO.cross_reference_target] + reporters.all_suite_reporters_cross_refs()))


def synopsis() -> cli_syntax.Synopsis:
    command_line = arg.CommandLine([
     arg.Single(arg.Multiplicity.MANDATORY, arg.Constant(common_opts.SUITE_COMMAND)),
     arg.Single(arg.Multiplicity.OPTIONAL, _REPORTER_OPTION),
     arg.Single(arg.Multiplicity.OPTIONAL, _ACTOR_OPTION),
     arg.Single(arg.Multiplicity.MANDATORY, _FILE_ARGUMENT)], prefix=program_info.PROGRAM_NAME)
    return cli_syntax.Synopsis(command_line, _TP.text(_DESCRIPTION_PARAGRAPH))


_DESCRIPTION_PARAGRAPH = 'Runs the test suite in file {TEST_SUITE_FILE}.\n'
_ACTOR_OPTION_DESCRIPTION = 'Specifies a default {interpreter_actor} {actor} to use for every test case in the suite.\n\n\n{interpreter_program} {is_a_shell_cmd}\n\n\n{NOTE} An {actor} specified in the test suite or individual test cases\nwill have precedence over the {actor} specified by this option.\n'
_ACTOR_OPTION = arg.short_long_option(long_name=common_cli_options.OPTION_FOR_ACTOR__LONG, argument=common_cli_options.ACTOR_OPTION_ARGUMENT)
_REPORTER_OPTION = arg.short_long_option(long_name=opts.OPTION_FOR_REPORTER__LONG, argument=opts.REPORTER_OPTION_ARGUMENT)
_REPORTER_OPTION_DESCRIPTION = 'Specifies in which format to report the execution of the test suite\n(stdout, stderr, exit code).\n\n\nOptions: {reporter_name_list} (default {default_reporter_name}).\n'
_FILE_ARGUMENT = arg.Named(opts.TEST_SUITE_FILE_ARGUMENT)
DEFAULT_SUITE_FILES_DESCRIPTION = 'If {TEST_SUITE_FILE} is a directory\nthat contains a file "{default_suite_file}",\nthen this file becomes the suite file argument.\n'

def _reporter_name(x: SingularNameAndCrossReferenceId) -> str:
    return formatting.cli_argument_option_string(x.singular_name)


_TP = TextParser({'actor': formatting.concept_(concepts.ACTOR_CONCEPT_INFO), 
