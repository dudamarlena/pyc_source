...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/util/textformat/parse.py
...
_ITEMIZED_LIST_ITEM_RE = re.compile(' +\\* ')
_ORDERED_LIST_ITEM_RE = re.compile(' +1\\. ')

class ListSettings(tuple):

    def __new__(cls, custom_indent_spaces: int, custom_separations: lists.Separations):
...
def parse(normalized_lines: List[str], list_settings: ListSettings=DEFAULT_LIST_SETTINGS) -> List[ParagraphItem]:
...
def normalize_and_parse(text: str, list_settings: ListSettings=DEFAULT_LIST_SETTINGS) -> List[ParagraphItem]:
...
class _Parser:
    ...
    def __init__(self, lines: List[str], list_settings: ListSettings):
    ...
    def _try_parse_list(self) -> Optional[lists.HeaderContentList]:
        ...
        if match:
            return self.parse_list_from_first_item_line(match.group(0), lists.ListType.ITEMIZED_LIST)
        match = _ORDERED_LIST_ITEM_RE.match(first_line)
        if match:
            return self.parse_list_from_first_item_line(match.group(0), lists.ListType.ORDERED_LIST)

    def parse_list_from_first_item_line(self, item_line_prefix: str, list_type: lists.ListType) -> lists.HeaderContentList:
        items = [
         self.parse_list_item_from_item_line(self.consume_current_line(), item_line_prefix)]
        while self.has_more_lines() and self.lines[0].startswith(item_line_prefix):
            next_item = self.parse_list_item_from_item_line(self.consume_current_line(), item_line_prefix)
    ...
    def _list_format(self, list_type: lists.ListType) -> lists.Format:
        return lists.Format(list_type, custom_indent_spaces=self.list_settings.custom_indent_spaces, custom_separations=self.list_settings.custom_separations)

    def parse_list_item_from_item_line(self, first_item_header_line: str, item_line_prefix: str) -> lists.HeaderContentListItem:
    ...
    def parse_list_content_paragraph_items(self, item_contents_indent_len: int) -> List[ParagraphItem]:
        ...
        return _Parser(normalized_item_contents_lines, self.list_settings).apply()
