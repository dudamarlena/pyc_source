...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/type_system/data/string_or_path_ddvs.py
...
class StringOrPath(tuple):
    ...
    def __new__(cls, source_type: SourceType, string_value: Optional[str], file_value: Optional[DescribedPath]):
...
class StringOrPathDdv(MultiDependenciesDdv[StringOrPath]):
    ...
    def __init__(self, source_type: SourceType, string: Optional[StringDdv], path: Optional[PathDdv]):
    ...
    def value_when_no_dir_dependencies(self) -> StringOrPath:
        if self.is_path:
            return StringOrPath(self._source_type, None, self._path.value_when_no_dir_dependencies__d())
        else:
            return StringOrPath(self._source_type, self._string.value_when_no_dir_dependencies(), None)

    def value_of_any_dependency(self, tcds: Tcds) -> StringOrPath:
        if self.is_path:
            return StringOrPath(self._source_type, None, self._path.value_of_any_dependency__d(tcds))
        else:
            return StringOrPath(self._source_type, self._string.value_of_any_dependency(tcds), None)

    def validator__file_must_exist_as(self, file_type: FileType, follow_symlinks: bool=True) -> DdvValidator:
        if not self.is_path:
        ...
        frc = PathCheckDdv(self._path, must_exist_as(file_type, follow_symlinks))
