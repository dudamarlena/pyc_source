...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/util/textformat/constructor/sections.py
# Compiled at: 2019-11-06 08:59:56
# Size of source mod 2**32: 4994 bytes
import itertools
...
from exactly_lib.util.textformat.constructor.section import SectionContentsConstructor, ArticleContentsConstructor, SectionConstructor, ArticleConstructor
...
def contents(initial_paragraphs: ParagraphItemsConstructor=paragraphs.empty(),
             sub_sections: Iterable[SectionConstructor]=()) -> SectionContentsConstructor:
    return _SectionContents([initial_paragraphs], sub_sections)
...
def section(header: Text, section_contents: SectionContentsConstructor) -> SectionConstructor:
    return _SectionConstructorFromSectionContentsConstructor(header, section_contents)


def article(node_target_info: TargetInfo, contents_renderer: ArticleContentsConstructor, tags: Optional[Set[str]]=None) -> ArticleConstructor:
    return _ArticleConstructorFromContentsConstructor(node_target_info, contents_renderer, tags)


class _SectionConstructorFromSectionContentsConstructor(SectionConstructor):

    def __init__(self, header: Text, section_contents_renderer: SectionContentsConstructor):
        self.header = header
        self.section_contents_renderer = section_contents_renderer

    def apply(self, environment: ConstructionEnvironment) -> doc.Section:
        return doc.Section(self.header, self.section_contents_renderer.apply(environment))
...
class _SectionContents(SectionContentsConstructor):
    ...
    def __init__(self, paragraph_items: Iterable[ParagraphItemsConstructor], sub_sections: Iterable[SectionConstructor]):
        self._paragraph_items = paragraph_items
        self._sub_sections = sub_sections

    def apply(self, environment: ConstructionEnvironment) -> doc.SectionContents:
        initial_paragraphs = list(itertools.chain.from_iterable([renderer.apply(environment) for renderer in self._paragraph_items]))
        sub_sections = [ss.apply(environment) for ss in self._sub_sections]
...
class _ArticleConstructorFromContentsConstructor(ArticleConstructor):
    ...
    def __init__(self, node_target_info: TargetInfo, contents_renderer: ArticleContentsConstructor, tags: Optional[Set[str]]=None):
    ...
    def apply(self, environment: ConstructionEnvironment) -> doc.Article:
        ...
        return doc.Article(self._node_target_info.presentation_text, ArticleContents(article_contents.abstract_paragraphs, article_contents.section_contents), target=self._node_target_info.target, tags=self._tags)