...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/cli/program_modes/symbol/request.py
...
class SymbolInspectionRequest:
    ...
    def __init__(self, case_execution_settings: Optional[TestCaseExecutionSettings], suite_execution_settings: Optional[TestSuiteExecutionSettings], variant: RequestVariant):
        self.case_execution_settings = case_execution_settings
        self.suite_execution_settings = suite_execution_settings
        self.variant = variant

    @staticmethod
    def new_for_case(execution_settings: TestCaseExecutionSettings, variant: RequestVariant) -> 'SymbolInspectionRequest':
        return SymbolInspectionRequest(execution_settings, None, variant)

    @staticmethod
    def new_for_suite(execution_settings: TestSuiteExecutionSettings, variant: RequestVariant) -> 'SymbolInspectionRequest':
        return SymbolInspectionRequest(None, execution_settings, variant)
...
class RequestVariantIndividual(RequestVariant):
    ...
    def __init__(self, name: str, list_references: bool):
        self.name = name
...
class RequestVariantVisitor(Generic[RET], ABC):
    ...
    def visit(self, variant: RequestVariant) -> RET:
        ...
        if isinstance(variant, RequestVariantIndividual):
