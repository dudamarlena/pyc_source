...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/instructions/assert_/stderr.py
...
def setup_for_stderr(instruction_name: str) -> SingleInstructionSetup:
    return SingleInstructionSetup(parser(instruction_name), utils.TheInstructionDocumentation(instruction_name, sandbox_directory_structure.RESULT_FILE__STDERR))


def parser(instruction_name: str) -> AssertPhaseInstructionParser:
    return parse_instruction.Parser(instruction_name, utils.Parser(ProcOutputFile.STDERR))