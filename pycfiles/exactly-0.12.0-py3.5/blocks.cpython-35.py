...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/util/simple_textstruct/rendering/blocks.py
# Compiled at: 2019-12-27 10:07:32
# Size of source mod 2**32: 5770 bytes
...
class PrependFirstMinorBlockR(SequenceRenderer[MinorBlock]):
    ...
    def __init__(self, to_prepend: SequenceRenderer[LineElement], to_prepend_to: SequenceRenderer[MinorBlock], properties_if_to_prepend_to_is_empty: ElementProperties=structure.ELEMENT_PROPERTIES__NEUTRAL):
    ...
    def render_sequence(self) -> Sequence[MinorBlock]:
        ...
        if len(to_prepend) == 0:
            ...
            to_prepend_to_list[0] = MinorBlock(line_elements, original_first_block.properties)
            return to_prepend_to_list


class PrependFirstMinorBlockOfFirstMajorBlockR(SequenceRenderer[MajorBlock]):

    def __init__(self, to_prepend: SequenceRenderer[LineElement], to_prepend_to: SequenceRenderer[MajorBlock], properties_if_to_prepend_to_is_empty: ElementProperties=structure.ELEMENT_PROPERTIES__NEUTRAL):
        self.to_prepend = to_prepend
        self.to_prepend_to = to_prepend_to
        self.properties_if_to_prepend_to_is_empty = properties_if_to_prepend_to_is_empty
    ...
    def render_sequence(self) -> Sequence[MajorBlock]:
        ...
        if len(to_prepend_to) == 0:
            ...
            to_prepend_to_list = list(to_prepend_to)
            to_prepend_to_list[0] = MajorBlock(PrependFirstMinorBlockR(self.to_prepend, rend_comb.ConstantSequenceR(to_prepend_to_list[0].parts), self.properties_if_to_prepend_to_is_empty).render_sequence())
            return to_prepend_to_list
...
class MinorBlockOfSingleLineObject(Renderer[MinorBlock]):
    ...
    def render(self) -> MinorBlock:
        renderer = comp_rend.MinorBlockR(rend_comb.SingletonSequenceR(comp_rend.LineElementR(self._line_object_renderer)))
...
class MinorBlocksOfSingleLineObject(SequenceRenderer[MinorBlock]):
    ...
    def render_sequence(self) -> Sequence[MinorBlock]:
        renderer = rend_comb.SingletonSequenceR(MinorBlockOfSingleLineObject(self._line_object_renderer))
...
class MinorBlocksOfLineElements(SequenceRenderer[MinorBlock]):
    ...
    def render_sequence(self) -> Sequence[MinorBlock]:
        renderer = rend_comb.SingletonSequenceR(comp_rend.MinorBlockR(self._line_elements_renderer))
        return renderer.render()
...
class MajorBlockOfSingleLineObject(Renderer[MajorBlock]):
    ...
    def render(self) -> MajorBlock:
        renderer = comp_rend.MajorBlockR(rend_comb.SingletonSequenceR(MinorBlockOfSingleLineObject(self._line_object_renderer)))
        return renderer.render()
...
class MajorBlocksOfSingleLineObject(SequenceRenderer[MajorBlock]):
    ...
    def render_sequence(self) -> Sequence[MajorBlock]:
        renderer = rend_comb.SingletonSequenceR(MajorBlockOfSingleLineObject(self._line_object_renderer))
        return renderer.render_sequence()
...
class MajorBlocksOfLineElements(SequenceRenderer[MajorBlock]):
    ...
    def render_sequence(self) -> Sequence[MajorBlock]:
        renderer = rend_comb.SingletonSequenceR(comp_rend.MajorBlockR(MinorBlocksOfLineElements(self._line_elements_renderer)))
        return renderer.render()