...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/help/entities/syntax_elements/objects/type_program.py
# Compiled at: 2020-02-02 15:10:17
# Size of source mod 2**32: 15409 bytes
from typing import List, Sequence
from exactly_lib import program_info
from exactly_lib.common.help.syntax_contents_structure import InvokationVariant, SyntaxElementDescription, invokation_variant_from_args, cli_argument_syntax_element_description
...
class _Documentation(SyntaxElementDocumentation):
    ...
    def __init__(self):
        super().__init__(TypeCategory.LOGIC, syntax_elements.PROGRAM_SYNTAX_ELEMENT)
    ...
    def invokation_variants(self) -> List[InvokationVariant]:
        return [
         invokation_variant_from_args([
          ...
          a.Single(a.Multiplicity.OPTIONAL, string_transformer.STRING_TRANSFORMATION_ARGUMENT)])]
    ...
    def _transformation_sed(self) -> SyntaxElementDescription:
        return cli_argument_syntax_element_description(string_transformer.STRING_TRANSFORMATION_ARGUMENT, _TEXT_PARSER.fnap(_TRANSFORMATION_DESCRIPTION), [
         invokation_variant_from_args([
...
class _PgmAndArgs(SyntaxElementDescriptionTree):
    ...
    def _pgm_for_arg_list_variant(self) -> InvokationVariant:
        return invokation_variant_from_args([
         ...
         a.Single(a.Multiplicity.ZERO_OR_MORE, self._arg.element)], _TEXT_PARSER.fnap(_PGM_WITH_ARG_LIST_INVOKATION_VARIANT_DESCRIPTION))
...
class _ShellCommandLine(InvokationVariantHelper):
    ...
    def syntax(self) -> Sequence[a.ArgumentUsage]:
        return [
         a.Single(a.Multiplicity.MANDATORY, a.Constant(instruction_names.SHELL_INSTRUCTION_NAME)),
         a.Single(a.Multiplicity.MANDATORY, syntax_elements.SHELL_COMMAND_LINE_SYNTAX_ELEMENT.argument)]
...
class _ProgramWithArgumentList(SyntaxElementDescriptionTree):
    ...
    def _exe_file() -> InvokationVariant:
        return invokation_variant_from_args([
         a.Single(a.Multiplicity.MANDATORY, instruction_arguments.PATH_ARGUMENT)], rel_path_doc.path_element_relativity_paragraphs(parse_path.REL_OPTIONS_CONFIGURATION, _TEXT_PARSER.paras(the_path_of('{executable_file:a}.'))))

    @staticmethod
    def _system_program() -> InvokationVariant:
        return invokation_variant_from_args([
         a.Single(a.Multiplicity.MANDATORY, a.Constant(pgm_syntax_elements.SYSTEM_PROGRAM_TOKEN)),
         a.Single(a.Multiplicity.MANDATORY, syntax_elements.STRING_SYNTAX_ELEMENT.argument)], _TEXT_PARSER.paras(misc_texts.SYSTEM_PROGRAM_DESCRIPTION))

    @staticmethod
    def _sym_ref() -> InvokationVariant:
        return invokation_variant_from_args([
         a.Single(a.Multiplicity.MANDATORY, a.Constant(pgm_syntax_elements.SYMBOL_REF_PROGRAM_TOKEN)),
         a.Single(a.Multiplicity.MANDATORY, syntax_elements.SYMBOL_NAME_SYNTAX_ELEMENT.argument)], _TEXT_PARSER.fnap(_SYM_REF_PROGRAM_DESCRIPTION))

    @staticmethod
    def _python() -> InvokationVariant:
        return invokation_variant_from_args([
         a.Single(a.Multiplicity.MANDATORY, a.Option(pgm_syntax_elements.PYTHON_EXECUTABLE_OPTION_NAME))], _TEXT_PARSER.fnap(_PYTHON_EXECUTABLE_DESCRIPTION))
...
class _ArgumentDoc(SyntaxElementDescriptionTree):
    ...
    def invokation_variants(self) -> List[InvokationVariant]:
        return [
         ...
         self._existing_path(pgm_syntax_elements.EXISTING_FILE_OPTION_NAME, _ARGUMENT__EXISTING_FILE_DESCRIPTION),
         self._existing_path(pgm_syntax_elements.EXISTING_DIR_OPTION_NAME, _ARGUMENT__EXISTING_DIR_DESCRIPTION),
         self._existing_path(pgm_syntax_elements.EXISTING_PATH_OPTION_NAME, _ARGUMENT__EXISTING_PATH_DESCRIPTION)]

    @staticmethod
    def _existing_path(option: a.OptionName, description_rest: str) -> InvokationVariant:
        return invokation_variant_from_args([
         a.Single(a.Multiplicity.MANDATORY, a.Option(option)),
         a.Single(a.Multiplicity.MANDATORY, PATH_OF_EXISTING_FILE_OPT_CONFIG.argument)], _TEXT_PARSER.fnap(description_rest))

    @staticmethod
    def _text_until_end_of_line() -> InvokationVariant:
        return invokation_variant_from_args([
         a.Single(a.Multiplicity.MANDATORY, a.Constant(pgm_syntax_elements.REMAINING_PART_OF_CURRENT_LINE_AS_LITERAL_MARKER)),
         a.Single(a.Multiplicity.MANDATORY, instruction_arguments.TEXT_UNTIL_END_OF_LINE_ARGUMENT)], _TEXT_PARSER.fnap(_ARGUMENT__TEXT_UNTIL_END_OF_LINE_DESCRIPTION))

    @staticmethod
    def _list() -> InvokationVariant:
        return invokation_variant_from_args([
         a.Single(a.Multiplicity.MANDATORY, syntax_elements.LIST_SYNTAX_ELEMENT.argument)], _TEXT_PARSER.fnap(_ARGUMENT__LIST_DESCRIPTION))

    @staticmethod
    def _string() -> InvokationVariant:
        return invokation_variant_from_args([
         a.Single(a.Multiplicity.MANDATORY, syntax_elements.STRING_SYNTAX_ELEMENT.argument)], _TEXT_PARSER.fnap(_ARGUMENT__STRING_DESCRIPTION))

    @property
    def sub_syntax_element_definition_trees(self) -> List[SyntaxElementDescriptionTree]:
        return [
         rel_path_doc.path_element_3(PATH_OF_EXISTING_FILE_OPT_CONFIG, _TEXT_PARSER.paras(the_path_of('an existing file.')))]


PATH_OF_EXISTING_FILE_OPT_CONFIG = arg_config_with_name('PATH-OF-EXISTING', pgm_syntax_elements.REL_OPTION_ARG_CONF)
EXECUTABLE_ARG = a.Named('EXECUTABLE')
_TEXT_PARSER = TextParser({'program_name': formatting.program_name(program_info.PROGRAM_NAME), 
...
_PGM_AND_ARGS = 'A program followed by arguments until end of line.\n'
_PGM_WITH_ARG_LIST_INVOKATION_VARIANT_DESCRIPTION = 'A program with a list of arguments.\n'
_SHELL_COMMAND_INVOKATION_VARIANT_DESCRIPTION = "{shell_command:a/u}.\n\n\n{SHELL_COMMAND_LINE} is the remaining part of the current line.\n\nIt is passed as a single string to the operating system's shell.\n"
PGM_WITH_ARG_LIST_DESCRIPTION = 'An executable program.\n'
_SYM_REF_PROGRAM_DESCRIPTION = 'Reference to a program that has been defined using the {define_symbol} instruction.\n\n\n{SYMBOL_NAME} must have been defined as {program_type:a/q}.\n\n\nArguments and transformations are appended to existing arguments and transformations\nof {SYMBOL_NAME}.\n'
_PYTHON_EXECUTABLE_DESCRIPTION = '{THE_PYTHON_INTERPRETER}\n\n\nSince {program_name} is written in Python,\nthe Python interpreter is guaranteed to be available on the system.\n'
_ARGUMENT_DESCRIPTION = 'An individual argument, or a list of arguments, in case of an unquoted reference\nto {list_type:a/q} {symbol:/q}.\n\n\nAn argument list is an ordinary {list_type:/q} value, with additional features\nfor text-until-end-of-line and references to existing files.\n'
_ARGUMENT__STRING_DESCRIPTION = '{path_type:/q} values are handled as {string_type:/q} values the usual way -\nrendered as an absolute path.\n'
_ARGUMENT__LIST_DESCRIPTION = 'Elements of referenced lists are appended to the preceding arguments -\nargument lists are not nested.\n\n\nTo pass a {list_se} as a single argument to a program,\nconvert it to a {string_se} by surrounding it with quotes.\n(The elements will be separated by a single space.)\n'
_ARGUMENT__TEXT_UNTIL_END_OF_LINE_DESCRIPTION = 'The remaining part of the current line becomes a single argument.\n'
_ARGUMENT__EXISTING_FILE_DESCRIPTION = 'A {path_se}, with additional check for existence.\n\n\nIt is an error - not {FAIL} - if the file does not exist,\nor it is not a regular file.\n\n{SYMBOLIC_LINKS_ARE_FOLLOWED}.\n\n\nValues are rendered as absolute paths.\n'
_ARGUMENT__EXISTING_DIR_DESCRIPTION = 'A {path_se}, with additional check for existence.\n\n\nIt is an error - not {FAIL} - if the file does not exist,\nor it is not a directory.\n\n{SYMBOLIC_LINKS_ARE_FOLLOWED}.\n\n\nValues are rendered as absolute paths.\n'
_ARGUMENT__EXISTING_PATH_DESCRIPTION = 'A {path_se}, with additional check for existence.\n\n\nIt is an error - not {FAIL} - if the path does not exist.\n\n{SYMBOLIC_LINKS_ARE_FOLLOWED}.\n\n\nValues are rendered as absolute paths.\n'
_TRANSFORMATION_DESCRIPTION = 'Transforms the output from the program.\n\n\nDepending on the context, either stdout or stderr is transformed.\n\n\nMust appear on a separate line.\n\nIf not, it will be interpreted as an {ARGUMENT}.\n'