...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/help/entities/concepts/objects/hds.py
# Compiled at: 2020-02-02 12:38:28
# Size of source mod 2**32: 7005 bytes
...
class _DirInfo:
    ...
    def __init__(self, relativity_option_type: RelHdsOptionType, dir_info: tc_fs.TcDirInfo, conf_param: ConfigurationParameterInfo, instruction_name: str):
...
class _HdsConcept(ConceptDocumentation):
    ...
    def __init__(self):
        ...
        self._tp = TextParser({'HDS': concepts.HDS_CONCEPT_INFO.acronym, 
    ...
    def purpose(self) -> DescriptionWithSubSections:
        ...
        sub_sections = [
         self._section(concepts.CONFIGURATION_PARAMETER_CONCEPT_INFO.singular_name.capitalize(), _CONFIGURATION_PARAMETER),
         self._section('Relative paths', _RELATIVITY),
         self._section(self._tp.format(BUILTIN_SYMBOL_ENTITY_TYPE_NAMES.name.plural.capitalize()), _BUILTIN_SYMBOL)]
    ...
    def _section(self, header: str, text_to_format: str) -> docs.Section:
        return docs.section(header, self._tp.fnap(text_to_format))
    ...
    def see_also_targets(self) -> List[SeeAlsoTarget]:
        ...
        ret_val += [dir_info.conf_param.cross_reference_target for dir_info in _ALL_DIRECTORIES]
        ret_val += [phase_infos.CONFIGURATION.instruction_cross_reference_target(dir_info.instruction_name) for dir_info in _ALL_DIRECTORIES]
        return ret_val
    ...
    def _dir_item(self, x: _DirInfo) -> HeaderContentListItem:
        ...
        properties_table = docs.first_column_is_header_table([
         ...
         prop_row(concepts.CONFIGURATION_PARAMETER_CONCEPT_INFO.singular_name.capitalize(), x.conf_param.configuration_parameter_name_text),
         prop_row('Set by instruction', instruction_name_text(x.instruction_name)),
         prop_row(BUILTIN_SYMBOL_ENTITY_TYPE_NAMES.name.singular.capitalize(), REL_HDS_OPTIONS_MAP[x.relativity_option_type].directory_symbol_name_text),
         prop_row('Path relativity option', REL_HDS_OPTIONS_MAP[x.relativity_option_type].option_name_text)])
        paras = [
...
_HDS_CASE_DIR_INFO = _DirInfo(RelHdsOptionType.REL_HDS_CASE, tc_fs.HDS_CASE_INFO, conf_params.HDS_CASE_DIRECTORY_CONF_PARAM_INFO, instruction_names.HDS_CASE_DIRECTORY_INSTRUCTION_NAME)
_HDS_ACT_DIR_INFO = _DirInfo(RelHdsOptionType.REL_HDS_ACT, tc_fs.HDS_ACT_INFO, conf_params.HDS_ACT_DIRECTORY_CONF_PARAM_INFO, instruction_names.HDS_ACT_DIRECTORY_INSTRUCTION_NAME)
_ALL_DIRECTORIES = (
 _HDS_ACT_DIR_INFO,
 _HDS_CASE_DIR_INFO)
_MAIN_DESCRIPTION_REST = 'Typically, the directories in the {hds_concept} ({HDS}) exist before the test case is executed.\nThey contain files read by, but not modified by, the test case.\n\n\nDirectories:\n'
