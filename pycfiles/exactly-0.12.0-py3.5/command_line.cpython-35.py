...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/help/entities/actors/objects/command_line.py
...
from exactly_lib.help.entities.actors.objects.common import SINGLE_LINE_PROGRAM_ACT_PHASE_CONTENTS_SYNTAX_INITIAL_PARAGRAPH, ARGUMENT_SYNTAX_ELEMENT, ActPhaseDocumentationSyntaxBase
...
class CommandLineActorDocumentation(ActorDocumentation):
    ...
    def __init__(self):
        ...
        self._tp = TextParser({'phase': phase_names.PHASE_NAME_DICTIONARY, 
    ...
    def act_phase_contents_syntax(self) -> doc.SectionContents:
        ...
        sub_sections = []
        synopsis_section = doc_utils.synopsis_section(invokation_variants_content(None, documentation.invokation_variants(), documentation.syntax_element_descriptions()))
...
class ActPhaseDocumentationSyntax(ActPhaseDocumentationSyntaxBase):
    ...
    def __init__(self):
        ...
        fm = {'EXECUTABLE': self.executable.name, 
    ...
    def invokation_variants(self) -> List[InvokationVariant]:
        ...
        shell_command_argument = a.Single(a.Multiplicity.MANDATORY, a.Constant(actor.SHELL_COMMAND_MARKER))
        command_argument = a.Single(a.Multiplicity.MANDATORY, self.command)
        return [
         InvokationVariant(self._cl_syntax_for_args([executable_arg,
          optional_arguments_arg]), self._parser.fnap(_PROGRAM_WITH_ARGUMENTS_INVOKATION_VARIANT)),
         InvokationVariant(self._cl_syntax_for_args([shell_command_argument,
          command_argument]), self._parser.fnap(_SHELL_COMMAND_INVOKATION_VARIANT))]

    def syntax_element_descriptions(self) -> List[SyntaxElementDescription]:
        return [
         path_element_2(actor.RELATIVITY_CONFIGURATION, self._parser.paragraph_items(the_path_of('{executable_file:a}.'))),
         SyntaxElementDescription(self.argument.name, self._parser.fnap(ARGUMENT_SYNTAX_ELEMENT)),
         SyntaxElementDescription(self.command.name, self._parser.fnap(_COMMAND_SYNTAX_ELEMENT))]

    def syntax_element_descriptions_for_executable_as_system_command(self) -> List[SyntaxElementDescription]:
        return [
         SyntaxElementDescription(self.executable.name, self._parser.paras(misc_texts.SYSTEM_PROGRAM_DESCRIPTION)),
         SyntaxElementDescription(self.argument.name, self._parser.fnap(ARGUMENT_SYNTAX_ELEMENT)),
         SyntaxElementDescription(self.command.name, self._parser.fnap(_COMMAND_SYNTAX_ELEMENT))]


_ACT_PHASE_CONTENTS = 'A single command line.\n\n\nAny number of empty lines and comment lines are allowed.\n'
_PROGRAM_WITH_ARGUMENTS_INVOKATION_VARIANT = 'Executes {executable_file:a}.\n'
_SHELL_COMMAND_INVOKATION_VARIANT = "Executes {shell_command:a} using the operating system's shell.\n"
_COMMAND_SYNTAX_ELEMENT = "{shell_command_line:a/u}.\n\n\nUses the syntax of the current operating system's shell.\n\nNote that this is not the same as the {shell_syntax_concept} built into {program_name}.\n"