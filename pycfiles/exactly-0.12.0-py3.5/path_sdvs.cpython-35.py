...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/symbol/data/path_sdvs.py
...
def of_rel_option(rel_option: relativity_root.RelOptionType, path_suffix: PathPartDdv=paths.empty_path_part()) -> PathSdv:
    return constant(paths.of_rel_option(rel_option, path_suffix))


def of_rel_option_with_const_file_name(rel_option: relativity_root.RelOptionType, file_name: str) -> PathSdv:
...
def rel_symbol_with_const_file_name(symbol_reference: SymbolReference, file_name: str) -> PathSdv:
    return _with_symbol.PathSdvRelSymbol(path_part_sdvs.from_constant_str(file_name), symbol_reference)