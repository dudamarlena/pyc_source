...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/execution/partial_execution/execution.py
...
def parse_atc_and_validate_symbols(actor: Actor, predefined_symbols: SymbolTable, test_case: TestCase) -> Tuple[(ActionToCheck, SymbolTable)]:
    ...
    """
    return executor.parse_atc_and_validate_symbols(actor, predefined_symbols, test_case)


def execute(test_case: TestCase, full_exe_input_conf: ExecutionConfiguration, conf_phase_values: ConfPhaseValues, initial_setup_settings: SetupSettingsBuilder, is_keep_sandbox: bool) -> PartialExeResult:
    """
    ...
    try:
        with preserved_cwd():
            ret_val = executor.execute(executor.Configuration(full_exe_input_conf, conf_phase_values, initial_setup_settings), test_case)
            return ret_val
    finally:
