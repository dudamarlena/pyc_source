...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/instructions/assert_/utils/instruction_of_matcher.py
...
class Instruction(Generic[T], AssertPhaseInstruction):
    __doc__ = 'Makes an instruction of a :class:`Matcher`'

    def __init__(self, matcher: MatcherSdv[None], err_msg_header_renderer: Renderer[MajorBlock]):
    ...
    def validate_pre_sds(self, environment: i.InstructionEnvironmentForPreSdsStep) -> svh.SuccessOrValidationErrorOrHardError:
    ...
    def main(self, environment: i.InstructionEnvironmentForPostSdsStep, os_services: OsServices) -> pfh.PassOrFailOrHardError:
        try:
