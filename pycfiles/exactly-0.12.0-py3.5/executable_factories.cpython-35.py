...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_case/executable_factories.py
...
class _CommandTranslator(commands.CommandDriverVisitor):
    ...
    def visit_shell(self, driver: commands.CommandDriverForShell) -> Executable:
        return Executable(is_shell=True, arg_list_or_str=driver.shell_command_line_with_args(self.arguments))

    def visit_executable_file(self, driver: commands.CommandDriverForExecutableFile) -> Executable:
        return Executable(is_shell=False, arg_list_or_str=[
         str(driver.executable_file)] + self.arguments)

    def visit_system_program(self, driver: commands.CommandDriverForSystemProgram) -> Executable:
        return Executable(is_shell=False, arg_list_or_str=[
         driver.program] + self.arguments)


_FACTORY_FOR_OPERATING_SYSTEM_MODULE_NAME = {'posix': ExecutableFactoryForPosix(), 
