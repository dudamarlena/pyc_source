...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_case_utils/file_matcher/parse_file_matcher.py
# Compiled at: 2020-01-31 19:29:57
# Size of source mod 2**32: 10918 bytes
from typing import List, Optional, Sequence
import exactly_lib.test_case_utils.file_matcher.file_or_dir_contents_doc, exactly_lib.test_case_utils.files_matcher.config
...
from exactly_lib.test_case_utils.file_matcher.impl import name_regex, name_glob_pattern, regular_file_contents, dir_contents, file_contents_utils
...
REG_EX_ARGUMENT = a.Option(REG_EX_OPTION, syntax_elements.REGEX_SYNTAX_ELEMENT.argument.name)
...
def parse_optional_selection_sdv(parser: TokenParser) -> Optional[FileMatcherSdv]:
    parser = token_stream_parser.token_parser_with_additional_error_message_format_map(parser, ADDITIONAL_ERROR_MESSAGE_TEMPLATE_FORMATS)
    return parser.consume_and_handle_optional_option(None, parse_sdv, exactly_lib.test_case_utils.files_matcher.config.SELECTION_OPTION.name)


def parse_sdv(parser: TokenParser, must_be_on_current_line: bool=True) -> FileMatcherSdv:
    generic_matcher = _parse__generic(parser, must_be_on_current_line)
    return FileMatcherSdv(generic_matcher)


def _parse__generic(parser: TokenParser, must_be_on_current_line: bool) -> GenericFileMatcherSdv:
    parser = token_stream_parser.token_parser_with_additional_error_message_format_map(parser, ADDITIONAL_ERROR_MESSAGE_TEMPLATE_FORMATS)
    return ep.parse(GRAMMAR, parser, must_be_on_current_line)


def _parse_name_matcher(parser: TokenParser) -> GenericFileMatcherSdv:
    return parser.parse_choice_of_optional_option(name_regex.parse__generic, name_glob_pattern.parse__generic, REG_EX_OPTION)


def _parse_type_matcher(parser: TokenParser) -> GenericFileMatcherSdv:
    file_type = parser.consume_mandatory_constant_string_that_must_be_unquoted_and_equal(file_properties.SYNTAX_TOKEN_2_FILE_TYPE, file_properties.SYNTAX_TOKEN_2_FILE_TYPE.get, '{_TYPE_}')
...
def _parse_regular_file_contents(parser: TokenParser) -> GenericFileMatcherSdv:
    string_matcher = parse_string_matcher.parse_string_matcher__generic(parser, must_be_on_current_line=False)
...
def _parse_dir_contents(token_parser: TokenParser) -> GenericFileMatcherSdv:
    ...
    files_matcher = parse_files_matcher.parse_files_matcher__generic(token_parser, False)
    return dir_contents.dir_matches_files_matcher_sdv__generic(model_constructor, files_matcher)
...
ADDITIONAL_ERROR_MESSAGE_TEMPLATE_FORMATS = {'_MATCHER_': types.FILE_MATCHER_TYPE_INFO.name.singular, 
...
def _file_types_table() -> docs.ParagraphItem:
    ...
    return docs.plain_table([row(type_info.type_argument, 'File must be a ' + type_info.description) for type_info in sorted(file_properties.TYPE_INFO.values(), key=lambda ti: ti.type_argument)])
...
class _TypeSyntaxDescription(grammar.SimpleExpressionDescription):
    ...
    def argument_usage_list(self) -> Sequence[a.ArgumentUsage]:
        return [
         a.Single(a.Multiplicity.MANDATORY, TYPE_MATCHER_ARGUMENT)]
...
GRAMMAR = standard_expression_grammar.new_grammar(concept=grammar.Concept(name=types.FILE_MATCHER_TYPE_INFO.name, type_system_type_name=types.FILE_MATCHER_TYPE_INFO.identifier, syntax_element_name=syntax_elements.FILE_MATCHER_SYNTAX_ELEMENT.argument), model=matcher_model.FILE_MATCHER_MODEL, value_type=ValueType.FILE_MATCHER, simple_expressions=(
 NameAndValue(NAME_MATCHER_NAME, grammar.SimpleExpression(_parse_name_matcher, _NameSyntaxDescription())),
 NameAndValue(TYPE_MATCHER_NAME, grammar.SimpleExpression(_parse_type_matcher, _TypeSyntaxDescription())),
 NameAndValue(REGULAR_FILE_CONTENTS, grammar.SimpleExpression(_parse_regular_file_contents, file_contents_utils.FileContentsSyntaxDescription(exactly_lib.test_case_utils.file_matcher.file_or_dir_contents_doc.REGULAR_FILE_DOCUMENTATION_SETUP))),
 NameAndValue(DIR_CONTENTS, grammar.SimpleExpression(_parse_dir_contents, file_contents_utils.FileContentsSyntaxDescription(file_or_dir_contents_doc.DIR_DOCUMENTATION)))))
_ERR_MSG_FORMAT_STRING_FOR_PARSE_NAME = 'Missing {_GLOB_PATTERN_} argument for {_NAME_MATCHER_}'
_DIR_CONTENTS_MODEL_NOT_ON_CURRENT_LINE_ERR_MSG = token_stream_parser.ErrorMessageConfiguration('Missing {_CONTENTS_OPTIONS_} or {_FILES_MATCHER_}', {'_CONTENTS_OPTIONS_': file_or_dir_contents.DIR_FILE_SET_OPTIONS.name, 
