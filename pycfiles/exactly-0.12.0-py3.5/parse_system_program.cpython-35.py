...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_case_utils/program/parse/parse_system_program.py
...
def parse_as_command(parser: TokenParser) -> CommandSdv:
    ...
    return CommandSdv(CommandDriverSdvForSystemProgram(program_name), additional_arguments)
...
_PROGRAM_NAME_STRING_REFERENCES_RESTRICTION = string_made_up_by_just_strings(text_docs.single_pre_formatted_line_object(strings.FormatMap('A program name must be defined in terms of {string_type}.', {'string_type': define_symbol.DATA_TYPE_INFO_DICT[DataValueType.STRING].identifier})))
_PARSE_NAME_CONF = parse_string.Configuration('NAME', _PROGRAM_NAME_STRING_REFERENCES_RESTRICTION)