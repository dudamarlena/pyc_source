...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/cli/program_modes/help/program_modes/test_suite/request_rendering.py
# Compiled at: 2018-09-19 16:40:01
# Size of source mod 2**32: 2586 bytes
from exactly_lib.cli.program_modes.help.program_modes.test_suite.help_request import TestSuiteHelpItem, TestSuiteHelpRequest
...
from exactly_lib.help.program_modes.test_suite.contents_structure.section_documentation import TestSuiteSectionDocumentation
...
class TestSuiteHelpConstructorResolver:
    ...
    def resolve(self, request: TestSuiteHelpRequest) -> SectionContentsConstructor:
        ...
        if item is TestSuiteHelpItem.INSTRUCTION:
            return with_or_without_name(request.do_include_name_in_output, request.name, render_instruction.InstructionDocArticleContentsConstructor(request.data))
        raise ValueError('Invalid %s: %s' % (str(TestSuiteHelpItem), str(item)))

    def render(self, request: TestSuiteHelpRequest, environment: ConstructionEnvironment) -> doc.SectionContents:
