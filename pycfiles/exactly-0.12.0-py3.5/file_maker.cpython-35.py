...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/instructions/utils/file_maker.py
...
from exactly_lib.test_case.phases.common import InstructionEnvironmentForPostSdsStep, InstructionSourceInfo, instruction_log_dir
from exactly_lib.test_case.validation import sdv_validation
from exactly_lib.test_case.validation.sdv_validation import SdvValidator, ConstantSuccessSdvValidator, ValidationStep
...
from exactly_lib.util.process_execution.sub_process_execution import ExecutorThatStoresResultInFilesInDir, execute_and_read_stderr_if_non_zero_exitcode, result_for_non_success_or_non_zero_exit_code
...
class FileMaker:
    ...
    def make(self, environment: InstructionEnvironmentForPostSdsStep, os_services: OsServices, dst_file: DescribedPath) -> Optional[TextRenderer]:
        """
...
class FileMakerForConstantContents(FileMaker):
    ...
    def make(self, environment: InstructionEnvironmentForPostSdsStep, os_services: OsServices, dst_path: DescribedPath) -> Optional[TextRenderer]:
        contents_str = self._contents.resolve_value_of_any_dependency(environment.path_resolving_environment_pre_or_post_sds)
...
class FileMakerForContentsFromProgram(FileMaker):
    ...
    def __init__(self, source_info: InstructionSourceInfo, output_channel: ProcOutputFile, program: ProgramSdv):
    ...
    def make(self, environment: InstructionEnvironmentForPostSdsStep, os_services: OsServices, dst_path: DescribedPath) -> Optional[TextRenderer]:
        ...
        return create_file_from_transformation_of_existing_file__dp(path_of_output, dst_path, program.transformation)

    @property
    def validator(self) -> SdvValidator:
        return sdv_validation.SdvValidatorFromDdvValidator(lambda symbols: self._program.resolve(symbols).validator)
...
class FileMakerForContentsFromExistingFile(FileMaker):
    ...
    def __init__(self, source_info: InstructionSourceInfo, transformer: StringTransformerSdv, src_path: PathSdv):
        self._source_info = source_info
        self._transformer = transformer
        self._src_path = src_path
        self._src_file_validator = path_check.PathCheckValidator(path_check.PathCheck(src_path, file_properties.must_exist_as(file_properties.FileType.REGULAR, follow_symlinks=True)))
        self._validator = sdv_validation.AndSdvValidator([
         sdv_validation.SingleStepSdvValidator(ValidationStep.PRE_SDS, self._src_file_validator),
         sdv_validation.SdvValidatorFromDdvValidator(lambda symbols: self._transformer.resolve(symbols).validator())])
    ...
    def make(self, environment: InstructionEnvironmentForPostSdsStep, os_services: OsServices, dst_path: DescribedPath) -> Optional[TextRenderer]:
        ...
        return create_file_from_transformation_of_existing_file__dp(src_path, dst_path, transformer)


def _create_file(path_to_create: DescribedPath, contents_str: str) -> Optional[TextRenderer]:
    """
    ...
    return file_creation.create_file__dp(path_to_create, write_file)