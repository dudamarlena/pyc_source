...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/processing/parse/act_phase_source_parser.py
...
class ActPhaseParser(SectionElementParser):
    ...
    def parse(self, fs_location_info: FileSystemLocationInfo, source: ParseSource) -> ParsedInstruction:
        ...
        return ParsedInstruction(line_sequence, InstructionInfo(SourceCodeInstruction(line_sequence), None))
...
def _split_space(s: str) -> (str, str):
    ...
    return (
     s[:non_space_char_idx], s[non_space_char_idx:])


class SourceCodeInstruction(ActPhaseInstruction):

    def __init__(self, source_code: LineSequence):
