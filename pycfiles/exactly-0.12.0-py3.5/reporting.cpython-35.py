...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_suite/reporting.py
...
class TestCaseProcessingInfo(tuple):
    ...
    def __new__(cls, result: test_case_processing.Result, duration: datetime.timedelta):
...
class SubSuiteReporter:
    ...
    def __init__(self, suite: structure.TestSuiteHierarchy, listener: SubSuiteProgressReporter):
        self._suite = suite
    ...
    def case_end(self, case: test_case_processing.TestCaseFileReference, execution_info: TestCaseProcessingInfo):
...
class RootSuiteReporter:
    ...
    def new_sub_suite_reporter(self, sub_suite: structure.TestSuiteHierarchy) -> SubSuiteReporter:
        raise NotImplementedError()
...
class RootSuiteProcessingReporter:
    ...
    def execution_reporter(self, root_suite: structure.TestSuiteHierarchy, reporting_environment: Environment, root_suite_file: pathlib.Path) -> RootSuiteReporter:
        raise NotImplementedError()