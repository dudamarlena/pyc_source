...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/symbol/symbol_syntax.py
# Compiled at: 2020-01-20 18:20:39
...
def parse_symbol_reference(token: Token) -> Optional[str]:
    ...
    if token.is_quoted:
        return
    return parse_symbol_reference__from_str(token.string)
    ...
    if token[:len(SYMBOL_REFERENCE_BEGIN)] == SYMBOL_REFERENCE_BEGIN:
        ...
        if token[-len(SYMBOL_REFERENCE_END):] == SYMBOL_REFERENCE_END:
            ...
            if is_symbol_name(token):
                return token
            raise SingleInstructionInvalidArgumentException("Illegal symbol name: `{}'".format(token))


class Fragment:

    def __init__(self, value: str, is_symbol: bool):
        self.value = value
    ...
    def __str__(self):
        ...
        return 'Fragment\\{{type}, value={value}\\}'.format(type=fragment_type, value=repr(self.value))
...
def _extract_fragment(s: str) -> Tuple[(str, List[Fragment])]:
    ...
    return (
     rest, [Fragment(s[:pos], False), Fragment(name, True)])
