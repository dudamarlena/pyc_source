...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/processing/test_case_processing.py
...
class TestCaseFileReference:
    ...
    def __init__(self, file_path: pathlib.Path, path_relativity_root_dir: pathlib.Path):
        self._TestCaseFileReference__path_relativity_root_dir = path_relativity_root_dir
        self._TestCaseFileReference__file_path = file_path

    @property
    def file_path(self) -> pathlib.Path:
        return self._TestCaseFileReference__file_path

    @property
    def path_relativity_root_dir(self) -> pathlib.Path:
        return self._TestCaseFileReference__path_relativity_root_dir


def test_case_reference_of_source_file(source_file: pathlib.Path) -> TestCaseFileReference:
    return TestCaseFileReference(source_file, source_file.parent)


class ErrorInfo(tuple):

    def __new__(cls, description: Optional[ErrorDescription], source_location_path: Optional[SourceLocationPath]=None,
...
class Result(tuple):
    ...
    def __new__(cls, status: Status, error_info: Optional[ErrorInfo]=None,
...
def new_internal_error(error_info: ErrorInfo) -> Result:
    return Result(Status.INTERNAL_ERROR, error_info=error_info)


def new_access_error(error: AccessErrorType, error_info: ErrorInfo) -> Result:
    return Result(Status.ACCESS_ERROR, error_info=error_info, error_type=error)


def new_executed(execution_result: FullExeResult) -> Result:
    return Result(Status.EXECUTED, execution_result=execution_result)


class AccessorError(Exception):

    def __init__(self, error: AccessErrorType, error_info: ErrorInfo):
        self._error = error
...
class Preprocessor:
    ...
    def apply(self, test_case_file_path: pathlib.Path, test_case_source: str) -> str:
        """
        :raises ProcessError:
        """
