...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/util/textformat/rendering/text/section.py
...
from exactly_lib.util.textformat.structure.document import SectionContents, Section, SectionItem, SectionItemVisitor, Article

class Separation(tuple):

    def __new__(cls, between_sections: int=1,
...
class Formatter(SectionItemVisitor):
    ...
    def __init__(self, paragraph_item_formatter: paragraph_item.Formatter, separation: Separation=Separation(),
                 section_content_indent_str: str=''):
        self.paragraph_item_formatter = paragraph_item_formatter
        self.section_content_indent = Indent(section_content_indent_str, section_content_indent_str)
        self.separation = separation
        self.wrapper = paragraph_item_formatter.wrapper

    def format_section_contents(self, section_contents: SectionContents, prepend_separator_for_contents: bool=False) -> list:
        ret_val = []
        ...
        if prepend_separator_for_contents and (init_para_lines or sections_lines):
    ...
    def visit_article(self, article: Article):
        as_section = Section(article.header, article.contents.combined_as_section_contents)
