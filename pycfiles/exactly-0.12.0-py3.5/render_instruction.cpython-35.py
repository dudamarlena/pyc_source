...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/help/program_modes/common/render_instruction.py
...
from exactly_lib.util.textformat.constructor.section import ArticleContentsConstructor, SectionContentsConstructor
...
class InstructionDocArticleContentsConstructor(ArticleContentsConstructor):
    ...
    def apply(self, environment: ConstructionEnvironment) -> doc.ArticleContents:
        ...
        if documentation.invokation_variants():
            sub_sections.append(synopsis_section(invokation_variants_content(documentation.instruction_name(), documentation.invokation_variants(), documentation.syntax_element_descriptions())))
        sub_sections += self._main_description_rest_sections()
        sub_sections += see_also_sections(documentation.see_also_targets(), environment, uppercase_title=True)
        abstract_paragraphs = docs.paras(self.documentation.single_line_description())
        return doc.ArticleContents(abstract_paragraphs, doc.SectionContents([], sub_sections))
...
def instruction_set_list_item(description: InstructionDocumentation, name_2_name_text_fun: Callable[([str], docs.Text)]) -> lists.HeaderContentListItem:
