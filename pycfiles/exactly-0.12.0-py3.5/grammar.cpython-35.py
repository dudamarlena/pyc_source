...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_case_utils/expression/grammar.py
...
class SimpleExpression(Generic[EXPR], ExpressionWithDescription):
    ...
    def __init__(self, parse_arguments: Callable[([TokenParser], EXPR)], syntax: SimpleExpressionDescription):
...
class ComplexExpression(Generic[EXPR], ExpressionWithDescription):
    ...
    def __init__(self, mk_complex: Callable[([Sequence[EXPR]], EXPR)], syntax: OperatorExpressionDescription):
...
class PrefixExpression(Generic[EXPR], ExpressionWithDescription):
    ...
    def __init__(self, mk_expression: Callable[([EXPR], EXPR)], syntax: OperatorExpressionDescription):
...
class Concept:
    ...
    def __init__(self, name: NameWithGenderWithFormatting, type_system_type_name: str, syntax_element_name: a.Named):
...
class Grammar(Generic[EXPR]):
    ...
    def __init__(self, concept: Concept, mk_reference: Callable[([str], EXPR)], simple_expressions: Sequence[NameAndValue[SimpleExpression[EXPR]]], complex_expressions: Sequence[NameAndValue[ComplexExpression[EXPR]]], prefix_expressions: Optional[Sequence[NameAndValue[PrefixExpression[EXPR]]]]=None):
