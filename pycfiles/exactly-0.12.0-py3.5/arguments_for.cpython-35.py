...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/cli/definitions/program_modes/help/arguments_for.py
...
from exactly_lib.definitions.entity.all_entity_types import CONCEPT_ENTITY_TYPE_NAMES, ACTOR_ENTITY_TYPE_NAMES, SUITE_REPORTER_ENTITY_TYPE_NAMES, SYNTAX_ELEMENT_ENTITY_TYPE_NAMES, TYPE_ENTITY_TYPE_NAMES, BUILTIN_SYMBOL_ENTITY_TYPE_NAMES
...
def case_instruction_in_phase(phase_name: str, instruction_name: str) -> List[str]:
    return [
...
def suite_instruction_in_section(section_name: str, instruction_name: str) -> List[str]:
    return [
     clo.TEST_SUITE, section_name, instruction_name]


def entity_single(entity_type_name: str, entity_name: str) -> List[str]:
    return [
     entity_type_name] + entity_name.split()
...
def concept_single(concept_name: str) -> List[str]:
    return entity_help(CONCEPT_ENTITY_TYPE_NAMES, concept_name)
...
def actor_single(actor_name: str) -> List[str]:
    return entity_help(ACTOR_ENTITY_TYPE_NAMES, actor_name)
...
def suite_reporter_single(suite_reporter_name: str) -> List[str]:
    return entity_help(SUITE_REPORTER_ENTITY_TYPE_NAMES, suite_reporter_name)


def entity_help(entity_type_names: EntityTypeNames, entity_name: str='') -> List[str]:
    if entity_name:
        return [entity_type_names.identifier] + entity_name.split()
    else:
        return [
         entity_type_names.identifier]


ARGUMENTS_FOR_PART = {HelpPredefinedContentsPart.TEST_CASE_CLI: case_cli_syntax, 
