...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_case/os_services.py
...
class OsServices:
    ...
    def copy_file_preserve_as_much_as_possible(self, src: str, dst: str) -> sh.SuccessOrHardError:
        return exception_detection.return_success_or_hard_error(self.copy_file_preserve_as_much_as_possible__detect_ex, src, dst)

    def copy_tree_preserve_as_much_as_possible(self, src: str, dst: str) -> sh.SuccessOrHardError:
        return exception_detection.return_success_or_hard_error(self.copy_tree_preserve_as_much_as_possible__detect_ex, src, dst)
...
class _Default(OsServices):
    ...
    def copy_file_preserve_as_much_as_possible__detect_ex(self, src: str, dst: str):
        try:
            ...
            raise exception_detection.DetectedException(FailureDetails.new_message(text_docs.single_line(strings.FormatMap('Failed to copy file {src} -> {dst}', {'src': src, 
             'dst': dst})), ex))
    ...
    def copy_tree_preserve_as_much_as_possible__detect_ex(self, src: str, dst: str):
        try:
            ...
            raise exception_detection.DetectedException(FailureDetails.new_message(text_docs.single_line(strings.FormatMap('Failed to copy tree {src} -> {dst}', {'src': src, 
             'dst': dst})), ex))

    def executable_factory__detect_ex(self) -> ExecutableFactory:
        if self._platform_system_not_supported:
            raise exception_detection.DetectedException(FailureDetails.new_constant_message(self._platform_system_not_supported))
...
class AtcSubProcessExecutor(AtcOsProcessExecutor):
    ...
    def execute(self, command: Command, std_files: StdFiles, process_execution_settings: ProcessExecutionSettings) -> ExitCodeOrHardError:
        executable = self._executable_factory.make(command)
        try:
            exit_code = subprocess.call(executable.arg_list_or_str, timeout=process_execution_settings.timeout_in_seconds, env=process_execution_settings.environ, shell=executable.is_shell, stdin=std_files.stdin, stdout=std_files.output.out, stderr=std_files.output.err)
...
def _raise_fail_to_make_dir_exception(path: pathlib.Path, ex: Exception):
    raise exception_detection.DetectedException(FailureDetails.new_message(text_docs.single_line(strings.FormatMap('Failed to make directory {path}', {'path': path})), ex))