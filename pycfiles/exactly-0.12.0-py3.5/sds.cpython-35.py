...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/help/entities/concepts/objects/sds.py
...
from exactly_lib.definitions.cross_ref.concrete_cross_refs import PredefinedHelpContentsPartReference, HelpPredefinedContentsPart
...
class _SandboxConcept(ConceptDocumentation):
    ...
    def purpose(self) -> DescriptionWithSubSections:
        ...
        return DescriptionWithSubSections(self.single_line_description(), SectionContents(rest_paragraphs, sub_sections))
    ...
    def see_also_targets(self) -> List[SeeAlsoTarget]:
        return [
         ...
         SeeAlsoUrlInfo(_TP.format('Platform dependent location of {SDS}'), _PYTHON_TMP_FILE_LOCATION_URL)]

    def _sandbox_directories_info_sections(self) -> List[docs.SectionItem]:

        def section(directory_name: str, dir_info: tcds.TcDirInfo, paragraph_items: List[ParagraphItem]) -> Section:
            return docs.section(dir_name_text(directory_name), docs.paras(dir_info.single_line_description_str + '.') + paragraph_items)

        return [
         section(sds.SUB_DIRECTORY__ACT, tcds.SDS_ACT_INFO, self._act_dir_description_paragraphs()),
         section(sds.SUB_DIRECTORY__RESULT, tcds.SDS_RESULT_INFO, self._result_dir_description_paragraphs()),
         section(sds.PATH__TMP_USER, tcds.SDS_TMP_INFO, self._tmp_user_dir_description_paragraphs()),
         docs.section(dir_name_text(sds.SUB_DIRECTORY__INTERNAL), self._internal_dir_description_paragraphs())]
    ...
    def _act_dir_description_paragraphs(self) -> List[ParagraphItem]:
        ...
        ret_val += _dir_path_and_rel_options(symbol_name=rel_opt_info.directory_symbol_name_text, rel_option=rel_opt_info.option_name_text)
        return ret_val

    def _result_dir_description_paragraphs(self) -> List[ParagraphItem]:
        ret_val = []
        ret_val += _TP.fnap(_RESULT_DIR_DESCRIPTION)
        ret_val.append(docs.simple_header_only_list(map(file_name_text, sds.RESULT_FILE_ALL), lists.ListType.ITEMIZED_LIST))
    ...
    def _result_dir_symbol_and_rel_option(self) -> List[ParagraphItem]:
        ...
        return [
         _dir_info_items_table(rel_opt_info.directory_symbol_name_text, rel_opt_info.option_name_text, _TP.format(_RESULT_DIR_SYMBOL))]

    def _internal_dir_description_paragraphs(self) -> List[ParagraphItem]:
        return _TP.fnap(_INTERNAL_DIRECTORIES)


_TP = TextParser({'SDS': concepts.SDS_CONCEPT_INFO.acronym, 
...
def _dir_path_and_rel_options(symbol_name: Text, rel_option: Text) -> List[ParagraphItem]:
    return [
     _dir_info_items_table(symbol_name, rel_option, _THE_VAL_OF_THIS_SYMBOL_IS_THE_ABS_PATH_OF_THE_DIRECTORY)]


def _dir_info_items_table(symbol_name: Text, rel_option: Text, symbol_description: str) -> ParagraphItem:
