...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/util/textformat/rendering/text/wrapper.py
...
class Indent(tuple):
    ...
    def __new__(cls, first_line: str, following_lines: str):
...
class Wrapper:
    ...
    def __init__(self, page_width: int=70):
    ...
    def no_word_wrap(self, lines: list) -> List[str]:
        ...
        output_line = self._get_first_output_line_and_store_remaining_part(window_width, line, rest)
    ...
    @staticmethod
    def _get_first_output_line_and_store_remaining_part(window_width: int, line: str, remaining_lines: list) -> str:
        if window_width <= 0:
            return line
        else:
    ...
    def current_indent(self) -> Indent:
        return Indent(self.text_wrapper.initial_indent, self.text_wrapper.subsequent_indent)
    ...
    def push_indent_increase(self, delta: Indent):
        text_wrapper = self.text_wrapper
        indent = Indent(text_wrapper.initial_indent + delta.first_line, text_wrapper.subsequent_indent + delta.following_lines)
