...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_case_utils/program/sdvs/program_symbol_sdv.py
# Compiled at: 2019-12-27 10:07:47
...
class ProgramSdvForSymbolReference(ProgramSdv):
    ...
    def __init__(self, symbol_name: str, accumulated_elements: ProgramElementsSdvAccumulator):
        self._symbol_name = symbol_name
        self._accumulated_elements = accumulated_elements
        self._symbol_reference = SymbolReference(symbol_name, ValueTypeRestriction(ValueType.PROGRAM))

    def new_accumulated(self, additional_stdin: StdinDataSdv, additional_arguments: ArgumentsSdv, additional_transformations: Sequence[StringTransformerSdv]) -> ProgramSdv:
        return ProgramSdvForSymbolReference(self._symbol_name, self._accumulated_elements.new_accumulated(additional_stdin, additional_arguments, additional_transformations))
...
def plain(symbol_name: str, arguments: ArgumentsSdv=arguments_sdvs.empty()) -> ProgramSdvForSymbolReference:
    return ProgramSdvForSymbolReference(symbol_name, accumulator.new_with_arguments(arguments))