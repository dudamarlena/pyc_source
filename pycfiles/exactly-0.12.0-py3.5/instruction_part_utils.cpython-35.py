...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/instructions/multi_phase/utils/instruction_part_utils.py
...
from exactly_lib.instructions.multi_phase.utils.instruction_embryo import MainStepExecutorEmbryo, InstructionEmbryo, InstructionEmbryoParser, T
from exactly_lib.instructions.multi_phase.utils.instruction_parts import MainStepExecutor, InstructionParts, InstructionPartsParser
...
class MainStepResultTranslatorForErrorMessageStringResultAsHardError(MainStepResultTranslator[str]):
    ...
    def translate_for_non_assertion(self, error_message: str) -> sh.SuccessOrHardError:
        ...
        return sh.new_sh_hard_error__str(error_message)

    def translate_for_assertion(self, error_message: str) -> pfh.PassOrFailOrHardError:
        if error_message is None:
            return pfh.new_pfh_pass()
        return pfh.new_pfh_hard_error__str(error_message)
...
class MainStepExecutorFromMainStepExecutorEmbryo(Generic[T], MainStepExecutor):
    ...
    def __init__(self, main_step_embryo: MainStepExecutorEmbryo[T], result_translator: MainStepResultTranslator[T]):
...
def instruction_parts_from_embryo(instruction: InstructionEmbryo, result_translator: MainStepResultTranslator) -> InstructionParts:
    return InstructionParts(instruction.validator, MainStepExecutorFromMainStepExecutorEmbryo(instruction, result_translator), tuple(instruction.symbol_usages))


class PartsParserFromEmbryoParser(InstructionPartsParser):

    def __init__(self, embryo_parser: InstructionEmbryoParser, main_step_result_translator: MainStepResultTranslator):
    ...
    def parse(self, fs_location_info: FileSystemLocationInfo, source: ParseSource) -> InstructionParts:
        ...
        return instruction_parts_from_embryo(instruction_embryo, self.main_step_result_translator)