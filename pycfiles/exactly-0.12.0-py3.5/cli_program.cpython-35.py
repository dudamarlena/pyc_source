...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/help/render/cli_program.py
# Compiled at: 2019-09-20 02:11:24
# Size of source mod 2**32: 5282 bytes
import itertools
...
from exactly_lib.util.cli_syntax.elements.cli_program_syntax import DescribedArgument, Synopsis
from exactly_lib.util.cli_syntax.render.cli_program_syntax import CommandLineSyntaxRenderer, ArgumentInArgumentDescriptionRenderer
...
class _ProgramDocumentationRenderer:
    ...
    def apply(self, program: CliProgramSyntaxDocumentation) -> docs.SectionContents:
        ...
        sections = [
         self._synopsis_section(program)]
    ...
    def _description_sections(program: CliProgramSyntaxDocumentation) -> List[docs.Section]:
        ...
        if description.rest.is_empty:
            return []
        return [
         description_section(description.rest)]
    ...
    def _options_sections(self, argument_descriptions: List[DescribedArgument]) -> List[docs.Section]:
        ...
        return [
         docs.section('OPTIONS', [option_paragraphs])]
    ...
    def _outcome_sections(outcome: Optional[docs.SectionContents]) -> List[docs.Section]:
        if not outcome:
            return []
        return [
         docs.Section(docs.text('OUTCOME'), outcome)]
    ...
    def _files_sections(files: Optional[docs.SectionContents]) -> List[docs.Section]:
        if not files:
            return []
        return [
         docs.Section(docs.text('FILES'), files)]
    ...
    def _list(items: List[lists.HeaderContentListItem]) -> lists.HeaderContentList:
        return lists.HeaderContentList(items, lists.Format(lists.ListType.VARIABLE_LIST, custom_indent_spaces=0, custom_separations=docs.SEPARATION_OF_HEADER_AND_CONTENTS))