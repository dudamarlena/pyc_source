...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_case_utils/parse/parse_relativity.py
...
def reference_restrictions_for_path_symbol(accepted_relativity_variants: PathRelativityVariants) -> ReferenceRestrictions:
    return ReferenceRestrictionsOnDirectAndIndirect(PathRelativityRestriction(accepted_relativity_variants))


def parse_explicit_relativity_info(options: RelOptionsConfiguration, source_file_location: Optional[pathlib.Path], source: TokenStream) -> Optional[Union[(RelOptionType, SymbolReference, pathlib.Path)]]:
    """
    ...
    if info is not None:
        return info
    if source_file_location is not None and _parse_rel_source_file(source):
        return source_file_location
    return _parse_rel_option_type(options, source)


def _raise_invalid_argument_exception_if_symbol_does_not_have_valid_syntax(symbol_name_token: Token, option_str: str):
    symbol_name = symbol_name_token.source_string
    if symbol_name_token.is_quoted:
        msg = 'Symbol name argument for {} must not be quoted: {}'.format(option_str, symbol_name)
        raise SingleInstructionInvalidArgumentException(msg)
    if not symbol_syntax.is_symbol_name(symbol_name_token.source_string):
        msg = 'Invalid name of {symbol_concept}: {invalid_value}'.format(symbol_concept=SYMBOL_CONCEPT_INFO.name.singular, invalid_value=symbol_name_token.source_string)
        raise SingleInstructionInvalidArgumentException(msg)
...
def _try_parse_rel_symbol_option(options: RelOptionsConfiguration, source: TokenStream) -> Optional[SymbolReference]:
    ...
    return SymbolReference(symbol_name, reference_restrictions_for_path_symbol(options.accepted_relativity_variants))


def _parse_rel_option_type(options: RelOptionsConfiguration, source: TokenStream) -> RelOptionType:
    option_str = source.head.string
...
def _valid_options_info_lines(options: RelOptionsConfiguration) -> list:
    ret_val = []
    ret_val.append('  {rel_symbol_option} {symbol_syntax_element_name}'.format(rel_symbol_option=option_parsing.long_option_syntax(REL_SYMBOL_OPTION_NAME.long), symbol_syntax_element_name=instruction_arguments.SYMBOL_SYNTAX_ELEMENT_NAME))
