...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/symbol/data/string_or_path.py
...
class StringOrPathSdv(DirDepValueResolver[StringOrPathDdv]):
    ...
    def __init__(self, source_type: SourceType, string: Optional[StringSdv], path: Optional[PathSdv]):
        self._source_type = source_type
        self._string = string
        self._path = path
        self._references = string.references if string is not None else path.references
    ...
    def resolve(self, symbols: SymbolTable) -> StringOrPathDdv:
        if self.is_path:
            return StringOrPathDdv(self._source_type, None, self._path.resolve(symbols))
        else:
            return StringOrPathDdv(self._source_type, self._string.resolve(symbols), None)
    ...
    def validator__file_must_exist_as(self, file_type: FileType, follow_symlinks: bool=True) -> SdvValidator:
        if not self.is_path:
        ...
        frc = PathCheck(self.path_sdv, must_exist_as(file_type, follow_symlinks))
