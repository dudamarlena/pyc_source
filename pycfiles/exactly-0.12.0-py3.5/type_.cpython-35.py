...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/help/entities/concepts/objects/type_.py
...
class _TypeConcept(ConceptDocumentation):
    ...
    def purpose(self) -> DescriptionWithSubSections:
        ...
        rest_paragraphs += [
         _categories_list(self._list_header('{data} types'), self._list_header('{logic} types'))]
...
def _list_types_in_category(type_category: TypeCategory) -> ParagraphItem:
    ...
    rows = list(map(row, sorted(type_docs, key=lambda type_doc: type_doc.name().singular)))
