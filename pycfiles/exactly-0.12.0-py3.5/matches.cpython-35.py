...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_case_utils/string_matcher/parse/parts/matches.py
...
def parse__generic(token_parser: TokenParser) -> GenericStringMatcherSdv:
    is_full_match = token_parser.consume_and_handle_optional_option(False, lambda parser: True, matcher_options.FULL_MATCH_ARGUMENT_OPTION)
    token_parser.require_has_valid_head_token(syntax_elements.REGEX_SYNTAX_ELEMENT.singular_name)
    source_type, regex_sdv = parse_regex.parse_regex2(token_parser, must_be_on_same_line=False)
...
class Description(grammar.SimpleExpressionDescription):
    ...
    def argument_usage_list(self) -> Sequence[a.ArgumentUsage]:
        return (
         a.Single(a.Multiplicity.OPTIONAL, a.Option(matcher_options.FULL_MATCH_ARGUMENT_OPTION)),
         syntax_elements.REGEX_SYNTAX_ELEMENT.single_mandatory)

    @property
    def description_rest(self) -> Sequence[ParagraphItem]:
        tp = TextParser({'REGEX': syntax_elements.REGEX_SYNTAX_ELEMENT.singular_name, 
