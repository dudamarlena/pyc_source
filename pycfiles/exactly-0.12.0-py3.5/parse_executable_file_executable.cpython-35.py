...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_case_utils/program/parse/parse_executable_file_executable.py
...
def _parse(tokens: TokenStream) -> ExecutableFileWithArgsResolver:
    ...
    """
    if tokens.is_null:
        parse_path.parse_path(tokens, conf=syntax_elements.REL_OPTION_ARG_CONF)
    the_path = _parse_exe_path(tokens)
    return ExecutableFileWithArgsResolver(the_path, list_sdvs.empty())
