...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/help/render/entity_docs.py
...
def entity_doc_list_renderer_as_flat_list_of_single_line_description(entity_doc_list: Iterable[EntityDocumentation]) -> SectionContentsConstructor:
    return EntitiesListConstructor(single_line_description_as_summary_paragraphs, entity_doc_list)
...
class FlatListConstructorWithSingleLineDescriptionGetter(CliListConstructorGetter):
    ...
    def get_constructor(self, all_entity_doc_list: Iterable[EntityDocumentation]) -> SectionContentsConstructor:
        return EntitiesListConstructor(single_line_description_as_summary_paragraphs, all_entity_doc_list)
...
class EntitiesListConstructor(SectionContentsConstructor):
    ...
    def __init__(self, entity_2_summary_paragraphs: Callable[([EntityDocumentation], List[ParagraphItem])], all_entities: Iterable[EntityDocumentation]):
    ...
    def _sorted_entities_list(self, entities: Iterable[EntityDocumentation]) -> lists.HeaderContentList:
        items = [docs.list_item(entity.singular_name(), self.entity_2_summary_paragraphs(entity)) for entity in sorted_entity_list(entities)]
        return lists.HeaderContentList(items, lists.Format(lists.ListType.VARIABLE_LIST, custom_indent_spaces=0, custom_separations=SEPARATION_OF_HEADER_AND_CONTENTS))