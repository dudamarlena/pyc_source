...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_case_utils/err_msg/file_or_dir_contents_headers.py
...
def target_name_of_proc_output_file_from_act_phase(file: ProcOutputFile) -> ToStringObject:
    return strings.FormatPositional('{} from {}', process_output_files.PROC_OUTPUT_FILE_NAMES[file], phase_names.ACT.syntax)


def target_name_of_proc_output_file_from_program(file: ProcOutputFile) -> ToStringObject:
    return strings.FormatPositional('{} from {}', process_output_files.PROC_OUTPUT_FILE_NAMES[file], types.PROGRAM_TYPE_INFO.singular_name)


def unexpected(target: ToStringObject) -> ToStringObject:
    return header_rendering.unexpected_attr_of_obj(file_check_properties.CONTENTS, target)