...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/instructions/utils/documentation/src_dst.py
...
class DocumentationElements:
    ...
    def __init__(self, format_map: Dict[(str, str)], src_rel_opt_conf: rel_opts_configuration.RelOptionArgumentConfiguration, src_description: str, dst_rel_opt_conf: rel_opts_configuration.RelOptionArgumentConfiguration, dst_description: str):
    ...
    def _syntax_element_descriptions_for_src(self) -> List[SyntaxElementDescription]:
        return [
         rel_opts.path_element(instruction_arguments.SOURCE_PATH_ARGUMENT.name, self._src_rel_opt_conf.options, self._tp.fnap(self._src_description))]

    def _syntax_element_descriptions_for_dst(self) -> List[SyntaxElementDescription]:
        return [
         rel_opts.path_element(instruction_arguments.DESTINATION_PATH_ARGUMENT.name, self._dst_rel_opt_conf.options, self._tp.fnap(self._dst_description))]
