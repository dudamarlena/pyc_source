...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/util/simple_textstruct/file_printer_output/printer.py
...
class PrintSettings:
    ...
    def __init__(self, indent: str, color: Optional[ForegroundColor], font_style: Optional[FontStyle]):
        self.indent = indent
        self.color = color
        self.font_style = font_style

    def new_with_increased_indent(self, increase: str) -> 'PrintSettings':
        return PrintSettings(self.indent + increase, self.color, self.font_style)

    def new_with_color(self, color: ForegroundColor) -> 'PrintSettings':
        return PrintSettings(self.indent, color, self.font_style)

    def new_with_font_style(self, style: FontStyle) -> 'PrintSettings':
        return PrintSettings(self.indent, self.color, style)


class Printer:

    def __init__(self, settings: PrintSettings, file_printer: FilePrinter):
        self.settings = settings
        self.file_printer = file_printer

    @staticmethod
    def new(file_printer: FilePrinter) -> 'Printer':
        return Printer(PrintSettings('', None, None), file_printer)

    def new_with_increased_indent(self, increase: str) -> 'Printer':
        """Gives a new printer with modified settings"""
        return Printer(self.settings.new_with_increased_indent(increase), self.file_printer)

    def new_with_color(self, color: ForegroundColor) -> 'Printer':
        """Gives a new printer with modified settings"""
        return Printer(self.settings.new_with_color(color), self.file_printer)

    def new_with_font_style(self, style: FontStyle) -> 'Printer':
        """Gives a new printer with modified settings"""
        return Printer(self.settings.new_with_font_style(style), self.file_printer)
