...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/util/file_utils.py
...
def write_new_text_file(file_path: pathlib.Path, contents: str):
    """
...
def tmp_text_file_containing(contents: str, prefix: str='',
                             suffix: str='',
                             directory=None) -> pathlib.Path:
    fd, absolute_file_path = tempfile.mkstemp(prefix=prefix, suffix=suffix, dir=directory, text=True)
    fo = os.fdopen(fd, 'w+')
    fo.write(contents)
    fo.close()
...
class TmpDirFileSpaceAsDirCreatedOnDemand(TmpDirFileSpace):
    ...
    def _root_dir(self) -> pathlib.Path:
        if self._existing_root_dir_path is None:
            ...
            self._existing_root_dir_path.mkdir(parents=True, exist_ok=True)
