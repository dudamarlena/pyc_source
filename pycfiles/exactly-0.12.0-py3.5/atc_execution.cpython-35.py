...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/execution/partial_execution/impl/atc_execution.py
# Compiled at: 2019-12-27 10:07:40
# Size of source mod 2**32: 7138 bytes
import pathlib, subprocess
from contextlib import contextmanager
from typing import Optional, Callable
from exactly_lib.execution.impl.result import ActionThatRaisesPhaseStepFailureException
from exactly_lib.execution.result import ActionToCheckOutcome, ExecutionFailureStatus, PhaseStepFailure, PhaseStepFailureException
...
class ActionToCheckExecutor:
    ...
    def __init__(self, atc: ActionToCheck, environment_for_validate_post_setup: InstructionEnvironmentForPostSdsStep, environment_for_other_steps: InstructionEnvironmentForPostSdsStep, os_process_executor: AtcOsProcessExecutor, stdin_configuration: StdinConfiguration, exe_atc_and_skip_assertions: Optional[StdOutputFiles]):
        self.atc = atc
    ...
    def validate_post_setup(self, failure_con: PhaseStepFailureConstructorType) -> ActionThatRaisesPhaseStepFailureException:

        def action():
            res = self.atc.validate_post_setup(self.environment_for_validate_post_setup)
            if not res.is_success:
                raise PhaseStepFailureException(failure_con(ExecutionFailureStatus(res.status.value), FailureDetails.new_message(res.failure_message)))
    ...
    def prepare(self, failure_con: PhaseStepFailureConstructorType) -> ActionThatRaisesPhaseStepFailureException:
        ...
        def action():
            res = self.atc.prepare(self.environment_for_other_steps, self.os_process_executor, self.script_output_dir_path)
            if not res.is_success:
                raise PhaseStepFailureException(failure_con(ExecutionFailureStatus.HARD_ERROR, FailureDetails.new_message(res.failure_message)))
    ...
    def execute(self, failure_con: PhaseStepFailureConstructorType) -> ActionThatRaisesPhaseStepFailureException:
        ...
        def action():
            ...
            if not exit_code_or_hard_error.is_exit_code:
                raise PhaseStepFailureException(failure_con(ExecutionFailureStatus.HARD_ERROR, exit_code_or_hard_error.failure_details))
    ...
    def _run_act_program_with_opened_stdin_file(self, file_name: pathlib.Path) -> ExitCodeOrHardError:
        try:
            ...
            return new_eh_hard_error(FailureDetails.new_exception(ex, 'Failure to open stdin file: ' + str(file_name)))
    ...
    def _run_act_program_with_std_files(self, std_files: StdFiles) -> ExitCodeOrHardError:
        exit_code_or_hard_error = self.atc.execute(self.environment_for_other_steps, self.os_process_executor, self.script_output_dir_path, std_files)
