...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_case_utils/condition/integer/integer_sdv.py
...
class IntegerSdv(OperandSdv[int], ObjectWithTypedSymbolReferences):
    ...
    def __init__(self, value_sdv: StringSdv, custom_integer_validator: Optional[CustomIntegerValidator]=None):
        """
    ...
    def resolve(self, symbols: SymbolTable) -> IntegerDdv:
        return IntegerDdv(self._value_sdv.resolve(symbols), self._custom_integer_validator)


class _ValidatorThatReportsViaExceptions(SvhPreSdsValidatorViaExceptions):

    def __init__(self, int_sdv: _IntResolver, custom_integer_validator: Optional[CustomIntegerValidator]):
    ...
    def validate_pre_sds(self, environment: PathResolvingEnvironmentPreSds):
        try:
            ...
            msg = text_docs.single_pre_formatted_line_object(strings.FormatPositional("Argument must be an integer: `{}'{}", ex.value_string, py_ex_str))
