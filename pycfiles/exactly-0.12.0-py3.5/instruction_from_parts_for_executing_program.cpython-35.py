...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/instructions/multi_phase/utils/instruction_from_parts_for_executing_program.py
...
from exactly_lib.instructions.multi_phase.utils.instruction_part_utils import MainStepResultTranslator, PartsParserFromEmbryoParser
...
from exactly_lib.test_case.phases.common import InstructionEnvironmentForPostSdsStep, PhaseLoggingPaths, instruction_log_dir, InstructionSourceInfo
...
class TheInstructionEmbryo(instruction_embryo.InstructionEmbryo[ResultAndStderr]):
    ...
    def __init__(self, source_info: InstructionSourceInfo, program: ProgramSdv):
    ...
    def main(self, environment: InstructionEnvironmentForPostSdsStep, logging_paths: PhaseLoggingPaths, os_services: OsServices) -> ResultAndStderr:
...
class InstructionEmbryoParser(instruction_embryo.InstructionEmbryoParserWoFileSystemLocationInfo):
    ...
    def __init__(self, instruction_name: str, program_parser: Parser[ProgramSdv]):
        self.instruction_name = instruction_name
        self.program_parser = program_parser

    def _parse(self, source: ParseSource) -> TheInstructionEmbryo:
        source_info = InstructionSourceInfo(source.current_line_number, self.instruction_name)
        program = self.program_parser.parse(source)
        return TheInstructionEmbryo(source_info, program)


def parts_parser(instruction_name: str, program_parser: Parser[ProgramSdv]) -> InstructionPartsParser:
    return PartsParserFromEmbryoParser(InstructionEmbryoParser(instruction_name, program_parser), ResultAndStderrTranslator())
