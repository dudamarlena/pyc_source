...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/help/render/see_also.py
# Compiled at: 2019-09-20 02:11:24
# Size of source mod 2**32: 4066 bytes
from typing import List, Sequence
from exactly_lib.common.help.see_also import CrossReferenceIdSeeAlsoItem, TextSeeAlsoItem, SeeAlsoItemVisitor, SeeAlsoSet, SeeAlsoItem
...
from exactly_lib.util.textformat.constructor.section import SectionContentsConstructor, SectionConstructor
...
def see_also_items_paragraph(see_also_items: List[SeeAlsoItem], environment: ConstructionEnvironment) -> ParagraphItem:
    """
    :return: A ParagraphItem that displays a list of `SeeAlsoItem`s.
    """
    renderer = _Renderer(environment.cross_ref_text_constructor)
    return docs.simple_header_only_list([renderer.visit(see_also_item) for see_also_item in see_also_items], lists.ListType.ITEMIZED_LIST)


def see_also_items_paragraphs(see_also_items: List[SeeAlsoItem], environment: ConstructionEnvironment) -> List[ParagraphItem]:
    """
    ...
    if not see_also_items:
        return []
    return [
     see_also_items_paragraph(see_also_items, environment)]


def see_also_sections(see_also_targets: Sequence[SeeAlsoTarget], rendering_environment: ConstructionEnvironment, uppercase_title: bool=False) -> List[docs.SectionItem]:
    """
...
class SeeAlsoSectionConstructor(SectionConstructor):
    ...
    def __init__(self, items: List[SeeAlsoItem], uppercase_title: bool=False):
    ...
    def apply(self, environment: ConstructionEnvironment) -> docs.SectionItem:
        return docs.Section(self._title, self._section_contents_constructor.apply(environment))