...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_case_utils/expression/parser.py
# Compiled at: 2020-01-31 11:01:52
# Size of source mod 2**32: 6456 bytes
...
def parse_from_parse_source(grammar: Grammar[EXPR], source: ParseSource, must_be_on_current_line: bool=True) -> EXPR:
    with token_stream_parser.from_parse_source(source) as (tp):
        return parse(grammar, tp, must_be_on_current_line)


def parse(grammar: Grammar[EXPR], parser: TokenParser, must_be_on_current_line: bool=True) -> EXPR:
    return _Parser(grammar, parser).parse(must_be_on_current_line)


class _Parser(Generic[EXPR]):

    def __init__(self, grammar: Grammar[EXPR], parser: TokenParser):
        self.parser = parser
    ...
    def parse_optional_complex_operator_name(self, must_be_on_current_line: bool=True) -> str:
        return self.parser.consume_optional_constant_string_that_must_be_unquoted_and_equal(self.complex_expressions_keys, must_be_on_current_line)

    def complex_operator_sequence_for_single_operator(self, complex_operator_name: str, first_expression: EXPR) -> EXPR:
        single_accepted_operator = [
         complex_operator_name]
        expressions = [
         first_expression]
    ...
    def parse_mandatory_simple(self, must_be_on_current_line: bool=True) -> EXPR:
        ...
        if self.consume_optional_start_parentheses(must_be_on_current_line):
            ...
            return self.parser.parse_mandatory_string_that_must_be_unquoted(self.grammar.concept.syntax_element.name, self.parse_simple, must_be_on_current_line=must_be_on_current_line)
    ...
    def parse_simple(self, simple_name: str) -> EXPR:
        ...
        if not symbol_syntax.is_symbol_name(simple_name):
            if simple_name.startswith(option_syntax.OPTION_PREFIX_CHARACTER):
                raise SingleInstructionInvalidArgumentException('Invalid option: ' + simple_name)
            else:
    ...
    def consume_optional_prefix_operator(self) -> Optional[Callable[([EXPR], EXPR)]]:
        prefix_operator_name = self.parser.consume_optional_constant_string_that_must_be_unquoted_and_equal(self.prefix_expressions_keys, must_be_on_current_line=False)
    ...
    def consume_mandatory_end_parentheses(self) -> None:
        self.parser.consume_mandatory_constant_string_that_must_be_unquoted_and_equal([')'], lambda x: None, 'Expression inside ( )')