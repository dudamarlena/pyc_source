...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_case_utils/matcher/impls/combinator_matchers.py
...
from exactly_lib.type_system.logic.matcher_base_class import MatcherWTrace, MatchingResult, MatcherWTraceAndNegation, MatcherDdv, MODEL, MatcherAdv, ApplicationEnvironment
from exactly_lib.util.description_tree import renderers

class _CombinatorBase(Generic[MODEL], WithCachedNameAndTreeStructureDescriptionBase, MatcherWTraceAndNegation[MODEL], ABC):
...
class Negation(_CombinatorBase[MODEL]):
    ...
    def new_structure_tree(negated: WithTreeStructureDescription) -> StructureRenderer:
        return renderers.NodeRendererFromParts(Negation.NAME, None, (), (
         negated.structure(),))
    ...
    def matches_w_trace(self, model: MODEL) -> MatchingResult:
        ...
        return TraceBuilder(self.name).append_child(result_to_negate.trace).build_result(not result_to_negate.value)
...
class _SequenceOfOperandsAdv(Generic[MODEL], MatcherAdv[MODEL]):
    ...
    def __init__(self, make_matcher: Callable[([Sequence[MatcherWTraceAndNegation[MODEL]]], MatcherWTraceAndNegation[MODEL])], operands: Sequence[MatcherAdv[MODEL]]):
        self._make_matcher = make_matcher
        self._operands = operands

    @staticmethod
    def of(make_matcher: Callable[([Sequence[MatcherWTraceAndNegation[MODEL]]], MatcherWTraceAndNegation[MODEL])], operands: Sequence[MatcherDdv[MODEL]], tcds: Tcds) -> MatcherAdv[MODEL]:
        return _SequenceOfOperandsAdv(make_matcher, [operand.value_of_any_dependency(tcds) for operand in operands])

    def applier(self, environment: ApplicationEnvironment) -> MatcherWTraceAndNegation[MODEL]:
        return self._make_matcher([operand.applier(environment) for operand in self._operands])
...
class Conjunction(_CombinatorBase[MODEL]):
    ...
    def new_structure_tree(operands: Sequence[WithTreeStructureDescription]) -> StructureRenderer:
        return renderers.NodeRendererFromParts(Conjunction.NAME, None, (), [operand.structure() for operand in operands])
...
class ConjunctionDdv(Generic[MODEL], MatcherDdv[MODEL]):
    ...
    def __init__(self, operands: Sequence[MatcherDdv[MODEL]]):
        self._operands = operands
        self._validator = ddv_validators.all_of([matcher.validator for matcher in operands])
    ...
    def value_of_any_dependency(self, tcds: Tcds) -> MatcherAdv[MODEL]:
        return _SequenceOfOperandsAdv.of(Conjunction, self._operands, tcds)
...
class Disjunction(_CombinatorBase[MODEL]):
    ...
    @staticmethod
    def new_structure_tree(operands: Sequence[WithTreeStructureDescription]) -> StructureRenderer:
        return renderers.NodeRendererFromParts(Disjunction.NAME, None, (), [operand.structure() for operand in operands])
...
class DisjunctionDdv(Generic[MODEL], MatcherDdv[MODEL]):
    ...
    def value_of_any_dependency(self, tcds: Tcds) -> MatcherAdv[MODEL]:
        return _SequenceOfOperandsAdv.of(Disjunction, self._operands, tcds)