...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/util/textformat/section_target_hierarchy/as_section_contents.py
...
class SectionContentsConstructorFromHierarchyGenerator(SectionContentsConstructor):
    ...
    def apply(self, environment: ConstructionEnvironment) -> SectionContents:
        ...
        section_item = self.generator.generate(target_factory).section_item(SectionItemNodeEnvironment(set()), environment)
...
class _TargetsStripper(document.SectionItemVisitor):
    ...
    def visit_section(self, section: document.Section):
        return document.Section(section.header, self._strip_section_contents(section.contents), None, section.tags)

    def visit_article(self, article: document.Article):
        return document.Article(article.header, self._strip_article_contents(article.contents), None, article.tags)

    def _strip_article_contents(self, contents: document.ArticleContents) -> document.ArticleContents:
        return document.ArticleContents(contents.abstract_paragraphs, self._strip_section_contents(contents.section_contents))

    def _strip_section_contents(self, contents: document.SectionContents) -> document.SectionContents:
        return document.SectionContents(contents.initial_paragraphs, list(map(self.visit, contents.sections)))


class _NullTargetInfoFactory(TargetInfoFactory):
    __doc__ = 'A CustomTargetInfoFactory that build empty CrossReferenceTarget:s'
