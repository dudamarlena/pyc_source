...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_suite/processing.py
...
class Processor:
    ...
    def __init__(self, default_case_configuration: case_processing.Configuration, suite_hierarchy_reader: SuiteHierarchyReader, reporter: reporting.RootSuiteProcessingReporter, suite_enumerator: SuiteEnumerator, test_case_processor_constructor: TestCaseProcessorConstructor):
    ...
    def process_reporter(self, suite_root_file_path: pathlib.Path) -> ProcessResultReporter:
        ...
        return _SuiteExecutionReporter(self._default_case_configuration, self._reporter, self._suite_enumerator, self._test_case_processor_constructor, root_suite, suite_root_file_path)


class _SuiteExecutionReporter(ProcessResultReporter):

    def __init__(self, default_case_configuration: case_processing.Configuration, suite_processing_reporter: reporting.RootSuiteProcessingReporter, suite_enumerator: SuiteEnumerator, test_case_processor_constructor: TestCaseProcessorConstructor, root_suite: structure.TestSuiteHierarchy, suite_root_file_path: pathlib.Path):
    ...
    def report(self, environment: Environment) -> int:
        ...
        executor = SuitesExecutor(self._suite_processing_reporter.execution_reporter(self._root_suite, environment, self._suite_root_file_path), self._default_case_configuration, self._test_case_processor_constructor)
...
class SuitesExecutor:
    ...
    def __init__(self, reporter: RootSuiteReporter, default_case_configuration: case_processing.Configuration, test_case_processor_constructor: TestCaseProcessorConstructor):
        self._reporter = reporter
        self._default_case_configuration = default_case_configuration
        self._test_case_processor_constructor = test_case_processor_constructor
    ...
    def _configuration_for_cases_in_suite(self, suite: structure.TestSuiteHierarchy) -> case_processing.Configuration:
        return case_processing.Configuration(self._default_case_configuration.test_case_definition, suite.test_case_handling_setup, self._default_case_configuration.atc_os_process_executor, self._default_case_configuration.is_keep_sandbox, self._default_case_configuration.sandbox_root_dir_resolver)


def _process_and_time(case_processor: test_case_processing.Processor, case: test_case_processing.TestCaseFileReference) -> TestCaseProcessingInfo:
...
def _process_case(case_processor: test_case_processing.Processor, case: test_case_processing.TestCaseFileReference) -> test_case_processing.Result:
    try:
        return case_processor.apply(case)
    except Exception as ex:
        error_info = test_case_processing.ErrorInfo(error_description.of_exception(ex), source_location_path_of(case.file_path, None))
