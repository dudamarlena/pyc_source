...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/common/process_result_reporters.py
...
class ProcessResultReporterWithInitialExitValueOutput(ProcessResultReporter):
    ...
    def __init__(self, exit_value: ExitValue, exit_value_printer: ProcOutputFile, output_rest: Callable[([Environment], None)]):
    ...
    def _print_exit_value(self, environment: Environment):
        ...
        exit_value_printer.write_colored_line(self._exit_value.exit_identifier, self._exit_value.color)
        exit_value_printer.flush()


class ProcessResultReporterWithInitialExitValueOutputBase(ProcessResultReporter, ABC):

    def __init__(self, exit_value_printer: ProcOutputFile):
        self._exit_value_printer = exit_value_printer
...
class ProcessResultReporterOfMajorBlocksBase(ProcessResultReporter, ABC):
    ...
    def __init__(self, exit_code: int, output_printer: ProcOutputFile):
        self._exit_code = exit_code
        self._output_printer = output_printer

    def report(self, environment: Environment) -> int:
        result_reporting.print_major_blocks(self._blocks(), environment.std_file_printers.get(self._output_printer))
...
class ProcessResultReporterOfExitCodeAndMajorBlocksBase(ProcessResultReporter, ABC):
    ...
    def __init__(self, exit_value_file: ProcOutputFile, blocks_file: ProcOutputFile):
    ...
    def report(self, environment: Environment) -> int:
        ...
        result_reporting.print_major_blocks(self._blocks(), environment.std_file_printers.get(self._blocks_file))
        return exit_value.exit_code
