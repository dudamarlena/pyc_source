...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/section_document/parse_source.py
...
class ParseSource:
    ...
    def consume_part_of_current_line(self, num_characters: int):
        ...
        if n > len(self._current_line_text):
            raise ValueError('Line does not contain specified number of characters to consume: ' + str(num_characters))
 ...
 consumed.
        ...
        if number_of_characters > len(self.source_string) - self._column_index:
            raise ValueError('Requested number-of-characters ({}) > length of remaining source ({})'.format(number_of_characters, len(self.source_string) - self._column_index))
        ...
        if num_lines_consumed == 0:
            ...
            index_of_1st_char_on_new_current_line = _index_of_1st_char_on_new_current_line(remaining_source, number_of_characters)
