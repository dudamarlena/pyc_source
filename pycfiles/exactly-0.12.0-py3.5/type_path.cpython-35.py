...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/help/entities/syntax_elements/objects/type_path.py
# Compiled at: 2019-12-27 10:07:40
# Size of source mod 2**32: 11244 bytes
...
from exactly_lib.definitions.cross_ref.name_and_cross_ref import SingularNameAndCrossReferenceId, cross_reference_id_list
...
from exactly_lib.test_case_file_structure.relative_path_options import REL_HDS_OPTIONS_MAP, REL_SDS_OPTIONS_MAP, RelOptionInfo, REL_CWD_INFO
...
class _Documentation(SyntaxElementDocumentation):
    ...
    def __init__(self):
        super().__init__(TypeCategory.DATA, syntax_elements.PATH_SYNTAX_ELEMENT)
        ...
        self._parser = TextParser({'RELATIVITY_OPTION': self._relativity_name.name, 
         ...
         'ref_of_path_symbol': '... ' + str(PurePosixPath(symbol_reference_syntax_for_name(path_type_symbol)) / 'a' / 'path' / 'relative' / 'to' / path_type_symbol)})

    def invokation_variants(self) -> list:
        return [
         InvokationVariant(cl_syntax.cl_syntax_for_args(self._cl_arguments()))]
    ...
    def main_description_rest_sub_sections(self) -> List[SectionItem]:
        return [
         docs.section('Relativity', self._parser.fnap(_MAIN_DESCRIPTION_RELATIVITY), [
          docs.section('NOTE', self._parser.fnap(_MAIN_DESCRIPTION_RELATIVITY_NOTE))]),
    ...
    def _relativity_sed(self) -> SyntaxElementDescription:
        ...
        return SyntaxElementDescription(self._relativity_name.name, description_rest)

    def _string_sed(self) -> SyntaxElementDescription:
        return SyntaxElementDescription(self._string_name.name, self._parser.fnap(_STRING_DESCRIPTION_REST))

    def _symbol_name_sed(self) -> SyntaxElementDescription:
        return SyntaxElementDescription(syntax_elements.SYMBOL_NAME_SYNTAX_ELEMENT.singular_name, self._parser.fnap(_SYMBOL_NAME_DESCRIPTION))

    def _cl_arguments(self) -> List[a.ArgumentUsage]:
        return [
         a.Single(a.Multiplicity.OPTIONAL, self._relativity_name),
         a.Single(a.Multiplicity.MANDATORY, self._string_name)]

    def _relativity_options_paragraph(self) -> ParagraphItem:
        return docs.simple_list_with_space_between_elements_and_content([
         docs.list_item(self._options_for_directories_in_the(concepts.HDS_CONCEPT_INFO), self._options_for_directories_in_the_hds()),
         docs.list_item(self._options_for_directories_in_the(concepts.SDS_CONCEPT_INFO), self._options_for_directories_in_the_sds()),
         docs.list_item(self._parser.format('Option for {cd}'), self._options_for_current_directory()),
         docs.list_item(self._parser.format('Option for a path denoted by a {symbol}'), self._options_for_symbol()),
         docs.list_item(self._parser.format('Option for the location of the current source file'), self._options_for_rel_source_file())], docs.lists.ListType.ITEMIZED_LIST)
    ...
    def _options_for_directories_in_the_hds() -> List[ParagraphItem]:
        return _options_for_directories_in_the_(REL_HDS_OPTIONS_MAP, HDS_DIR_DISPLAY_ORDER)

    @staticmethod
    def _options_for_directories_in_the_sds() -> List[ParagraphItem]:
        return _options_for_directories_in_the_(REL_SDS_OPTIONS_MAP, SDS_DIR_DISPLAY_ORDER)

    def _options_for_current_directory(self) -> List[ParagraphItem]:
        return [
         docs.first_row_is_header_table([
          [
           docs.text_cell(REL_CWD_INFO.option_name_text)]])] + self._parser.fnap(_REL_CD_DESCRIPTION)

    def _options_for_symbol(self) -> List[ParagraphItem]:
        return [
         docs.first_column_is_header_table([
          [
           docs.text_cell(syntax_text(render_argument(REL_SYMBOL_OPTION)))]])] + self._parser.fnap(_REL_SYMBOL_DESCRIPTION)

    def _options_for_rel_source_file(self) -> List[ParagraphItem]:
        return [
         docs.first_column_is_header_table([
          [
           docs.text_cell(syntax_text(REL_source_file_dir_OPTION))]])] + self._parser.fnap(_REL_SOURCE_FILE_DESCRIPTION)


DOCUMENTATION = _Documentation()

def _options_for_directories_in_the_(rel_opt_2_rel_option_info: dict, rel_opts_in_display_order: list) -> List[ParagraphItem]:
    rows = list(map(lambda rel_opt: _mk_dir_info_row(rel_opt_2_rel_option_info[rel_opt]), rel_opts_in_display_order))
    return [
...
_MAIN_DESCRIPTION_RELATIVITY = "If {PATH_STRING} is an absolute path, then {RELATIVITY_OPTION} must not be given.\n\n\nIf {PATH_STRING} is a relative path, then if {RELATIVITY_OPTION} is ...\n\n\n  * given\n  \n    {PATH_STRING} is relative to the directory specified by {RELATIVITY_OPTION}\n\n  * not given\n  \n    {PATH_STRING} is relative to the default relativity root directory,\n\n\nThe default relativity depends on the instruction,\nand also on the argument's role in the instruction.\n"
_MAIN_DESCRIPTION_RELATIVITY_NOTE = 'If {PATH_STRING} begins with a reference to a {path_type} {symbol},\nthen it is an absolute path.\n\n\nFor example:\n\n\n```\n{def_of_path_symbol}\n\n{ref_of_path_symbol}\n```\n'
_STRING_DESCRIPTION_REST = 'A relative or absolute path, using {posix_syntax}.\n\n\nIt is a value of type {string_type}, and thus uses {string_syntax_element} syntax.\n'
_RELATIVITY_DESCRIPTION_REST = "An option that specifies the directory that {PATH_STRING} is relative to.\n\n\nThe available relativities depends on the instruction,\nand also on the argument's role in the instruction. \n\n\nSummary of options:\n"
_REL_CD_DESCRIPTION = 'This options is needed since many instructions\nhave a default relativity other than the {cd}.\n'
_REL_SYMBOL_DESCRIPTION = 'This option is always available.\n'
_REL_SOURCE_FILE_DESCRIPTION = 'This option is only available when defining a path {symbol} using the {define_symbol} instruction.\n'
_SYMBOL_NAME_DESCRIPTION = 'A {symbol} with type {path_type}.\n'