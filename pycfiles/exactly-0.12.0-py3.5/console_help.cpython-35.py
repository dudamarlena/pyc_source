...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/cli/program_modes/help/request_handling/console_help.py
...
class ConsoleHelpRequestHandler(RequestHandler):
    ...
    def __init__(self, application_help: ApplicationHelp, section_contents_constructor: SectionContentsConstructor):
    ...
    def handle(self, output: StdOutputFiles):
        ...
        environment = ConstructionEnvironment(_cross_ref_text_constructor(), construct_simple_header_value_lists_as_tables=True)
...
def _formatter(page_width):
    ...
    return section.Formatter(paragraph_item.Formatter(text_formatter, Wrapper(page_width=page_width), list_formats=list_formats_with(indent_str='  ')), section_content_indent_str='   ')
...
class _HelpCommandLineGetterVisitor(concrete_cross_refs.CrossReferenceIdVisitor):
    ...
    def visit_entity(self, x: concrete_cross_refs.EntityCrossReferenceId):
        return _command_line_display_for_help_arguments(arguments_for.entity_single(x.entity_type_identifier, x.entity_name))
    ...
    def visit_test_case_phase_instruction(self, x: concrete_cross_refs.TestCasePhaseInstructionCrossReference):
        return _command_line_display_for_help_arguments(arguments_for.case_instruction_in_phase(x.phase_name, x.instruction_name))
    ...
    def visit_test_suite_section_instruction(self, x: concrete_cross_refs.TestSuiteSectionInstructionCrossReference):
        return _command_line_display_for_help_arguments(arguments_for.suite_instruction_in_section(x.section_name, x.instruction_name))
