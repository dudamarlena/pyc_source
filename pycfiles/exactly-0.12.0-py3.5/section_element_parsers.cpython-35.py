...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/section_document/element_parsers/section_element_parsers.py
# Compiled at: 2019-09-20 02:11:24
# Size of source mod 2**32: 6831 bytes
from typing import Sequence, Callable, Optional
from exactly_lib.section_document import model
from exactly_lib.section_document import syntax
from exactly_lib.section_document.model import InstructionInfo
from exactly_lib.section_document.parse_source import ParseSource
from exactly_lib.section_document.parsed_section_element import ParsedSectionElement, new_empty_element, new_comment_element, ParsedInstruction, ParsedNonInstructionElement
from exactly_lib.section_document.section_element_parsing import SectionElementParser, UnrecognizedSectionElementSourceError
...
class InstructionAndDescriptionParser(SectionElementParser):
    ...
    def parse(self, fs_location_info: FileSystemLocationInfo, source: ParseSource) -> ParsedInstruction:
        """
...
class InstructionWithoutDescriptionParser(InstructionAndDescriptionParser):
    ...
    def parse(self, fs_location_info: FileSystemLocationInfo, source: ParseSource) -> ParsedInstruction:
        return parse_and_compute_source(self.instruction_parser, fs_location_info, source)
...
class StandardSyntaxCommentAndEmptyLineParser(SectionElementParser):
    ...
    def parse(self, fs_location_info: FileSystemLocationInfo, source: ParseSource) -> Optional[ParsedNonInstructionElement]:
        first_line = source.current_line
        if syntax.is_empty_line(first_line.text):
            return new_empty_element(self._consume_and_return_current_line(source, syntax.is_empty_line))
        else:
            if syntax.is_comment_line(first_line.text):
                return new_comment_element(self._consume_and_return_current_line(source, syntax.is_comment_line))
            return

    @staticmethod
    def _consume_and_return_current_line(source: ParseSource, line_predicate_for_line_to_consume: Callable[([str], bool)]) -> line_source.LineSequence:
...
def parse_and_compute_source(parser: InstructionParser, fs_location_info: FileSystemLocationInfo, source: ParseSource, description: str=None) -> ParsedInstruction:
