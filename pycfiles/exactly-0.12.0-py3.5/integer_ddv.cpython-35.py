...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_case_utils/condition/integer/integer_ddv.py
...
class IntegerDdv(OperandDdv[int]):
    ...
    def __init__(self, int_expression: StringDdv, custom_integer_validator: Optional[CustomIntegerValidator]=None):
        ...
        self._validator = _IntegerDdvValidator(self._primitive_value_computer, custom_integer_validator)
...
class _PrimitiveValueComputer:
    __doc__ = 'Computes the primitive value'
...
class _IntegerDdvValidator(DdvValidator):
    ...
    def __init__(self, value_computer: _PrimitiveValueComputer, custom_validator: Optional[CustomIntegerValidator]):
        self._value_computer = value_computer
        self._custom_validator = custom_validator if custom_validator is not None else (lambda x: None)
