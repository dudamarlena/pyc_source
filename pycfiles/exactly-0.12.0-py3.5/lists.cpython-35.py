...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/util/textformat/structure/lists.py
...
class HeaderContentListItem(tuple):
    ...
    def __new__(cls, header: Text, content_paragraph_items: Iterable[ParagraphItem]=()):
...
class Separations(tuple):
    ...
    def __new__(cls, num_blank_lines_between_elements: int, num_blank_lines_between_header_and_contents: int):
...
class Format(tuple):
    ...
    def __new__(cls, list_type: ListType, custom_indent_spaces: int=None,
...
class HeaderContentList(ParagraphItem):
    ...
    def __init__(self, items: Iterable[HeaderContentListItem], list_format: Format):
        self._items = items
