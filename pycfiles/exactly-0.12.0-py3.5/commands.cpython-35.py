...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/type_system/logic/program/commands.py
# Compiled at: 2019-12-27 10:07:49
...
class CommandDriverDdvForShell(CommandDriverDdv):
    ...
    def structure_for(self, arguments: ListDdv) -> StructureBuilder:
        return commands.CommandDriverForShell.new_structure_builder_for(strings.AsToStringObject(self._command_line.describer()), _arguments_as_to_string_objects(arguments))
...
class CommandDriverDdvForExecutableFile(CommandDriverDdv):
    ...
    def structure_for(self, arguments: ListDdv) -> StructureBuilder:
        return commands.CommandDriverForExecutableFile.new_structure_builder_for(details.String(strings.AsToStringObject(self._exe_file.describer().value)), _arguments_as_to_string_objects(arguments))
...
class CommandDriverDdvForSystemProgram(CommandDriverDdv):
    ...
    def structure_for(self, arguments: ListDdv) -> StructureBuilder:
        return commands.CommandDriverForSystemProgram.new_structure_builder_for(strings.AsToStringObject(self._program.describer()), _arguments_as_to_string_objects(arguments))
...
def _arguments_as_to_string_objects(arguments: ListDdv) -> List[ToStringObject]:
    return [strings.AsToStringObject(argument.describer()) for argument in arguments.string_elements]