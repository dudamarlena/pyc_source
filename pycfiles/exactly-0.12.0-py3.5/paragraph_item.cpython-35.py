...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/util/textformat/rendering/text/paragraph_item.py
# Compiled at: 2019-09-20 02:11:23
# Size of source mod 2**32: 8298 bytes
from typing import List, Iterable
from exactly_lib.util.textformat.rendering.text.lists import ListFormats, ListFormat, list_format_with_indent_str, list_format_with_separations
...
class Formatter:
    ...
    def __init__(self, text_formatter: TextFormatter, wrapper: Wrapper=Wrapper(),
    ...
    def format_header_content_list(self, the_list: HeaderContentList) -> List[str]:
        ...
        return self.format_header_value_list_according_to_format(the_list.items, list_format)

    def format_header_value_list_according_to_format(self, items: Iterable[HeaderContentListItem], list_format: ListFormat) -> List[str]:
    ...
    def format_table(self, table: Table) -> List[str]:
        ...
        if available_width <= 0:
            return []
        formatter = TableFormatter(self._paragraph_items_formatter_for_given_width_for_table_formatter, available_width, table)
    ...
    def _paragraph_items_formatter_for_given_width_for_table_formatter(self, width: int):
        formatter = Formatter(self.text_formatter, Wrapper(page_width=width), num_item_separator_lines=self.num_item_separator_lines, list_formats=self.list_formats)
        return lambda cell: formatter.format_paragraph_items(cell.paragraph_items)


class _ListFormatter:

    def __init__(self, formatter: Formatter, list_format: ListFormat, items: iter):
        ...
        self.blank_lines_between_elements = formatter.wrapper.blank_lines(separations.num_blank_lines_between_elements)
        self.blank_lines_between_header_and_content = formatter.wrapper.blank_lines(separations.num_blank_lines_between_header_and_contents)
        self.ret_val = []
    ...
    def apply(self) -> List[str]:
        ...
        with self.wrapper.indent_increase(identical_indent(self.list_format.indent_str)):
            for item_number, item in enumerate(self.items_list, start=1):
                assert isinstance(item, HeaderContentListItem), 'The list item is not a %s: %s' % (str(HeaderContentListItem),
                 str(item))
                if item_number > 1:
    ...
    def _format_header(self, item, item_number):
        header_string = self.list_format.header_format.header_text(item_number, self.num_items, self.formatter.text_formatter, item.header)
    ...
    def header_indent(self, item_number) -> Indent:
        following_lines_indent = self.list_format.header_format.following_header_lines_indent(item_number, self.num_items)
