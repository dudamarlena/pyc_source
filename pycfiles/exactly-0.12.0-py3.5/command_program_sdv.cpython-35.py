...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_case_utils/program/sdvs/command_program_sdv.py
...
class ProgramSdvForCommand(ProgramSdv):
    ...
    def __init__(self, command: CommandSdv, accumulated_elements: ProgramElementsSdvAccumulator):
        self._command = command
        self._accumulated_elements = accumulated_elements

    def new_accumulated(self, additional_stdin: StdinDataSdv, additional_arguments: ArgumentsSdv, additional_transformations: Sequence[StringTransformerSdv]):
        return ProgramSdvForCommand(self._command, self._accumulated_elements.new_accumulated(additional_stdin, additional_arguments, additional_transformations))
    ...
    def resolve(self, symbols: SymbolTable) -> ProgramDdv:
        ...
        return ProgramDdv(accumulated_command.resolve(symbols), acc.resolve_stdin_data(symbols), acc.resolve_transformations(symbols), has_transformations)


def plain(command: CommandSdv) -> ProgramSdvForCommand:
    return ProgramSdvForCommand(command, accumulator.empty())