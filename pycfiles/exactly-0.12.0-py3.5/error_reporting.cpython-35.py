...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/test_suite/error_reporting.py
...
def report_suite_parse_error(ex: SuiteParseError, stdout_printer: FilePrinter, stderr_printer: FilePrinter) -> int:
    ...
    reporting.print_major_blocks(blocks_renderer, stderr_printer)
...
def _suite_parse_error_renderer(ex: SuiteParseError) -> SequenceRenderer[MajorBlock]:
    blocks_renderers = [
     source_location.location_blocks_renderer(ex.source_location, ex.maybe_section_name, None),
...
class _GetParseErrorErrorMessageLinesRenderer(sec_doc_exceptions.ParseErrorVisitor[TextRenderer]):
    ...
    def visit_file_source_error(self, ex: sec_doc_exceptions.FileSourceError) -> TextRenderer:
        return majors.of_minor(minors.syntax_error_message(line_elements.single_pre_formatted(ex.message)))

    def visit_file_access_error(self, ex: sec_doc_exceptions.FileAccessError) -> TextRenderer:
        return majors.of_minor(minors.file_access_error_message(line_elements.single_pre_formatted(ex.message)))
...
class _GetReadErrorMessageLinesRenderer(SuiteReadErrorVisitor[TextRenderer]):
    ...
    def visit_file_reference_error(self, ex: suite_exception.SuiteFileReferenceError) -> TextRenderer:
        return majors.of_minor(minors.header_and_message(ex.error_message_header, domain_objects.single_path(ex.reference)))