...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/util/cli_syntax/render/cli_program_syntax.py
...
class ArgumentUsageOnCommandLineRenderer(arg.ArgumentUsageVisitor):
    ...
    def visit_single(self, x: arg.Single) -> str:
        ...
        if x.multiplicity is arg.Multiplicity.MANDATORY:
            return arg_str
        if x.multiplicity is arg.Multiplicity.ZERO_OR_MORE:
            return self._and_more(self._optional(arg_str))
        if x.multiplicity is arg.Multiplicity.ONE_OR_MORE:
    ...
    def _multiplicity(arg_str: str, multiplicity: arg.Multiplicity) -> str:
        if multiplicity is arg.Multiplicity.MANDATORY:
            return arg_str
        return '[' + arg_str + ']'
