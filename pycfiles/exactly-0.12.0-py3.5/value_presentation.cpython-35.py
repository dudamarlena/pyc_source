...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/exactly_lib/cli/program_modes/symbol/impl/reports/value_presentation.py
...
class PresentationBlockConstructor:
    ...
    def __init__(self, all_definitions: Sequence[SymbolDefinition], builtin_symbols: SymbolTable):
        self._symbol_table = SymbolTable({definition.name:definition.symbol_container for definition in all_definitions})
...
def _of_single_line_object(line_renderer: LineObjectRenderer) -> ResolvedValuePresentationBlock:
    return _BlockForCustomRenderer(blocks.MajorBlockOfSingleLineObject(line_renderer))
...
class _StringRenderer(MajorBlockRenderer):
    ...
    def render(self) -> MajorBlock:
        s = self._x.render()
        header = _type_of_x_elements_header(len(s), types.STRING_TYPE_INFO, type_system.NUMBER_OF_STRING_CHARACTERS)
        if len(s) == 0:
            return _header_only(header)
        else:
            return _header_and_value(header, [
             text_struct.LineElement(text_struct.PreFormattedStringLineObject(s, False))], text_struct.ELEMENT_PROPERTIES__NEUTRAL)
...
class _ListRenderer(MajorBlockRenderer):
    ...
    def render(self) -> MajorBlock:
        ...
        header = _type_of_x_elements_header(len(the_list), types.LIST_TYPE_INFO, type_system.NUMBER_OF_LIST_ELEMENTS)
        if len(the_list) == 0:
            return _header_only(header)
        num_formatter = self._format_lt_10 if len(the_list) < 10 else self._format_gte_10
        value_list = [text_struct.LineElement(text_struct.StringLineObject('  '.join([num_formatter(n), elem]))) for n, elem in enumerate(the_list, 1)]
        return _header_and_value(header, value_list, text_struct.ELEMENT_PROPERTIES__INDENTED)
    ...
    def render(self) -> MajorBlock:
        return text_struct.MajorBlock([
         text_struct.MinorBlock([
          text_struct.LineElement(text_struct.StringLineObject('program'))])])


def _type_of_x_elements_header(num_elements: int, type_name: TypeNameAndCrossReferenceId, element_name: NumberOfItemsString) -> str:
    return ' '.join([
...
def _header_and_value(header: str, value: Sequence[text_struct.LineElement], value_block_properties: text_struct.ElementProperties) -> text_struct.MajorBlock:
    return text_struct.MajorBlock([
     text_struct.MinorBlock([
      text_struct.LineElement(text_struct.StringLineObject(header))]),
     text_struct.MinorBlock(value, value_block_properties)])
