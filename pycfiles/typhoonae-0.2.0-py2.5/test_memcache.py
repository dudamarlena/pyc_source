...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/typhoonae/memcache/tests/test_memcache.py
# Compiled at: 2010-12-12 04:36:57
"""Unit tests for memcache."""
import google.appengine.api.apiproxy_stub_map, google.appengine.ext.db, os, time, typhoonae.memcache.memcache_stub, unittest
...
class MemcacheTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        google.appengine.api.apiproxy_stub_map.apiproxy.RegisterStub('memcache', typhoonae.memcache.memcache_stub.MemcacheServiceStub())
        self.stub = google.appengine.api.apiproxy_stub_map.apiproxy.GetStub('memcache')
    ...
    def testBehaviors(self):
        ...
        self.assertEqual([
         ...
         ('verify_keys', 0)], self.stub._GetMemcacheBehavior())
    ...
    def testAddingItem(self):
        ...
        assert google.appengine.api.memcache.get('greeting', namespace='me') == greeting
        assert google.appengine.api.memcache.get('greeting', namespace='no') is None
        unicode_data = [
         ('Äquator').decode('utf-8')]
        google.appengine.api.memcache.set('unicode', unicode_data)
        self.assertEqual(unicode_data, google.appengine.api.memcache.get('unicode'))
    ...
    def testGetKey(self):
        ...
        assert typhoonae.memcache.memcache_stub.getKey('b', namespace='a') == 'YS5i'
        os.environ['APPLICATION_ID'] = 'app'
        assert typhoonae.memcache.memcache_stub.getKey('b', namespace='a') == 'YXBwLmEuYg=='
    ...
    def testBatchIncrement(self):
        ...
        self.assertEqual(50, google.appengine.api.memcache.get('high'))
        google.appengine.api.memcache.offset_multi({'max': 5, 'min': -5}, initial_value=10)
    ...
    def testCacheProtobuf(self):
        ...
        os.environ['APPLICATION_ID'] = 'app'
        google.appengine.api.memcache.set('protobuf', google.appengine.ext.db.model_to_protobuf(entity).Encode())
        encoded_entity = google.appengine.api.memcache.get('protobuf')
        cached_entity = google.appengine.ext.db.model_from_protobuf(encoded_entity)
        assert cached_entity.name == 'foobar'

    def testMulti(self):
        """Stores multiple keys' values at once."""
        google.appengine.api.memcache.set_multi({'map_key_one': 1, 'map_key_two': 'some value'})
        values = google.appengine.api.memcache.get_multi([
         'map_key_one', 'map_key_two'])
        assert {'map_key_one': 1, 'map_key_two': 'some value'} == values
        google.appengine.api.memcache.add_multi({'map_key_one': 'one', 'map_key_two': 2, 'three': 'trois'})
        values = google.appengine.api.memcache.get_multi([
         'map_key_two', 'three'])
        assert {'map_key_two': 'some value', 'three': 'trois'} == values

    def testStats(self):
        """Tries to get memcache stats."""
        stats = google.appengine.api.memcache.get_stats()
        self.assertEqual(set(['hits', 'items', 'bytes', 'oldest_item_age', 'misses',
         'byte_hits']), set(stats.keys()))
