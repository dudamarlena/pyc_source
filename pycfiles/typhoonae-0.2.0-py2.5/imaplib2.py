...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/typhoonae/mail/imaplib2.py
...
Commands = {'APPEND': (
   ...
   'LOGOUT': (
            (
             NONAUTH, AUTH, LOGOUT, SELECTED), False), 
   'LSUB': (
          (
           AUTH, SELECTED), True), 
   'MYRIGHTS': (
...
class IMAP4(object):
    ...
    continuation_cre = re.compile('\\+( (?P<data>.*))?')
    literal_cre = re.compile('.*{(?P<size>\\d+)}$')
    mapCRLF_cre = re.compile('\\r\\n|\\r|\\n')
    mustquote_cre = re.compile("[^!#$&'+,./0-9:;<=>?@A-Z\\[^_`a-z|}~-]")
    response_code_cre = re.compile('\\[(?P<type>[A-Z-]+)( (?P<data>[^\\]]*))?\\]')
    untagged_response_cre = re.compile('\\* (?P<type>[A-Z-]+)( (?P<data>.*))?')
    untagged_status_cre = re.compile('\\* (?P<data>\\d+) (?P<type>[A-Z-]+)( (?P<data2>.*))?')
    ...
    def __init__(self, host=None, port=None, debug=None, debug_file=None, identifier=None):
        ...
        self._init_debug(debug, debug_file)
        self.open(host, port)
        if debug:
        ...
        try:
            ...
            if self._get_untagged_response('PREAUTH'):
                self.state = AUTH
                self._log(1, 'state => AUTH')
            elif self._get_untagged_response('OK'):
                self._log(1, 'state => NONAUTH')
            else:
            ...
            self._log(3, 'CAPABILITY: %r' % (self.capabilities,))
  ...
  NB: You must call this to shut down threads before discarding an instance."""
    ...
    def _append_untagged(self, typ, dat):
        ...
        urd.append(dat)
        self.commands_lock.release()
        self._log(5, 'untagged_responses[%s] %s += ["%s"]' % (typ, len(urd) - 1, dat))
        return
    ...
    def _command(self, name, *args, **kw):
        if Commands[name][CMD_VAL_ASYNC]:
            ...
            cmdtyp = 'sync'
        self._log(1, '[%s] %s %s' % (cmdtyp, name, args))
        ...
        if cmdtyp == 'async':
            ...
            self.commands_lock.release()
            if need_event:
                self._log(4, 'sync command %s waiting for empty commands Q' % name)
                self.state_change_free.wait()
                self._log(4, 'sync command %s proceeding' % name)
        ...
        while True:
            ...
            self._log(3, 'continuation => %s, %s' % (ok, data))
            if not ok:
                break
            ...
            if literator is not None:
                crqb = self._request_push(tag='continuation')
            self._log(4, 'write literal size %s' % len(literal))
    ...
    def _end_idle(self):
        ...
        self.ouq.put(irqb)
        self._log(2, 'server IDLE finished')
        return
    ...
    def _get_untagged_response(self, name, leave=False):
        ...
        for (i, (typ, dat)) in enumerate(self.untagged_responses):
            if typ == name:
                ...
                self.commands_lock.release()
                self._log(5, '_get_untagged_response(%s) => %s' % (name, dat))
                return dat
    ...
    def _put_response(self, resp):
        ...
        if self._literal_expected is not None:
            ...
            if self._match(self.literal_cre, dat):
                ...
                self._log(4, 'expecting literal size %s' % self._expecting_data)
                return
            typ = self._literal_expected[0]
            self._literal_expected = None
            self._append_untagged(typ, dat)
            self._log(4, 'literal completed')
        elif self._match(self.tagre, resp):
            tag = self.mo.group('tag')
            typ = self.mo.group('type')
            dat = self.mo.group('data')
            if tag not in self.tagged_commands:
                self._log(1, 'unexpected tagged response: %s' % resp)
            else:
            ...
            if self.mo is None:
                if self._match(self.continuation_cre, resp):
                    if not continuation_expected:
                        self._log(1, "unexpected continuation response: '%s'" % resp)
                        return
                    self._request_pop('continuation', (True, self.mo.group('data')))
                    return
                self._log(1, "unexpected response: '%s'" % resp)
                return
            typ = self.mo.group('type')
            dat = self.mo.group('data')
            if dat is None:
            ...
            if self._match(self.literal_cre, dat):
                self._expecting_data = int(self.mo.group('size'))
                self._log(4, 'read literal size %s' % self._expecting_data)
        ...
        if typ in ('NO', 'BAD', 'BYE'):
            if typ == 'BYE':
                self.Terminate = True
            self._log(1, '%s response: %s' % (typ, dat))
        return
    ...
    def _request_pop(self, name, data):
        self._log(4, '_request_pop(%s, %s)' % (name, data))
        self.commands_lock.acquire()
    ...
    def _request_push(self, tag=None, name=None, **kw):
        ...
        self.commands_lock.release()
        self._log(4, '_request_push(%s, %s, %s)' % (tag, name, `kw`))
        return rqb
    ...
    def _close_threads(self):
        self._log(1, '_close_threads')
        self.ouq.put(None)
        self.wrth.join()
        self._log(1, 'call shutdown')
        self.shutdown()
        self.rdth.join()
        self.inth.join()
        return
        ...
        while not self.Terminate:
            try:
                ...
                if self.idle_timeout > time.time():
                    continue
                self._log(2, 'server IDLE timedout')
                line = IDLE_TIMEOUT_RESPONSE

            if line is None:
                self._log(1, 'inq None - terminating')
                break
    ...
    if hasattr(select_module, 'poll'):
        ...
        def _reader(self):
            ...
            self._log(1, 'starting using poll')

            def poll_error(state):
                PollErrors = {select.POLLERR: 'Error', 
            ...
            while not self.Terminate:
                ...
                try:
                    r = poll.poll(timeout)
                    self._log(5, 'poll => %s' % `r`)
                    if not r:
                        continue
                    (fd, state) = r[0]
                    if state & select.POLLIN:
                        data = self.read(32768)
                        start = 0
                        dlen = len(data)
                        self._log(5, 'rcvd %s' % dlen)
                        if dlen == 0:
                            rxzero += 1
                            if rxzero > 5:
                        ...
                        while True:
                            ...
                            if stop < 0:
                                ...
                                break
                            stop += 1
                            line_part, start, line = '', stop, line_part + data[start:stop]
                            self._log(4, '< %s' % line)
                            self.inq.put(line)
        ...
        def _reader(self):
            threading.currentThread().setName(self.identifier + 'reader')
            self._log(1, 'starting using select')
            line_part = ''
            rxzero = 0
            while not self.Terminate:
                if self.state == LOGOUT:
                ...
                try:
                    ...
                    self._log(5, 'select => %s, %s, %s' % (r, w, e))
                    if not r:
                        continue
                    data = self.read(32768)
                    start = 0
    ...
    def _log(self, lvl, line):
        ...
        if self.debug >= 4:
            ...
            self.debug_lock.release()
            return
        self._cmd_log[self._cmd_log_idx] = (
         line, tn, time.time())
        self._cmd_log_idx += 1
    ...
    def _print_log(self):
        ...
        if n:
            self._mesg('last %d log messages:' % n)
        while n:
            try:
...
class _IdleCont(object):
    ...
    def process(self, data, rqb):
        ...
        self.parent.idle_timeout = self.timeout + time.time()
        self.parent._log(2, 'server IDLE started, timeout in %.2f secs' % self.timeout)
        return


Mon2num = {'Jan': 1, 'Feb': 2, 'Mar': 3, 'Apr': 4, 'May': 5, 'Jun': 6, 'Jul': 7, 
...
Internaldate2tuple = Internaldate2Time
...
FLAGS_cre = re.compile('.*FLAGS \\((?P<flags>[^\\)]*)\\)')
...
if __name__ == '__main__':
    ...
    (debug, port, stream_command, keyfile, certfile) = (
     None,) * 5
    ...
    if not args:
        args = ('',)
    if not port:
        port = keyfile is not None and IMAP4_SSL_PORT or IMAP4_PORT
    host = args[0]
    USER = getpass.getuser()
    data = open(os.path.exists('test.data') and 'test.data' or __file__).read(1000)
    test_mesg = 'From: %(user)s@localhost%(lf)sSubject: IMAP4 test%(lf)s%(lf)s%(data)s' % {'user': USER, 'lf': '\n', 'data': data}
    test_seq1 = [
     (
      'list', ('""', '%')),
     (
    ...
    try:
        ...
        for (cmd, args) in test_seq2:
            if (
             cmd, args) != ('uid', ('SEARCH', 'SUBJECT', 'IMAP4 test')):
                run(cmd, args)
                continue
        ...
        if debug:
            ...
            for (typ, dat) in M.pop_untagged_responses():
                M._mesg('\t%s %s' % (typ, dat))
