...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/typhoonae/taskqueue/taskqueue_celery_stub.py
# Compiled at: 2010-12-12 04:36:57
"""Task queue API proxy stub."""
from celery.task.base import Task
import base64, datetime, google.appengine.api.api_base_pb, google.appengine.api.apiproxy_stub, google.appengine.api.apiproxy_stub_map, google.appengine.api.taskqueue.taskqueue_service_pb, google.appengine.api.taskqueue.taskqueue_stub, google.appengine.api.urlfetch, google.appengine.runtime.apiproxy_errors, logging, os, simplejson, socket, typhoonae.taskqueue, typhoonae.taskqueue.celery_tasks
...
class TaskQueueServiceStub(google.appengine.api.apiproxy_stub.APIProxyStub):
    ...
    def _SetupQueues(self):
        self.celery_tasks_for_queues = typhoonae.taskqueue.celery_tasks.create_task_queues_from_yaml(self.root_path)
    ...
    def _RunAsync(self, publisher=None, **kwargs):
        ...
        if datetime.datetime.utcnow() > eta:
            eta = None
        task.apply_async(kwargs=kwargs, eta=eta, expires=2592000, publisher=publisher)
        return
        ...
        try:
            for add_request in request.add_request_list():
                if not request.add_request(0).has_transaction():
                    ...
                    task_dict = dict(content_type=content_type, eta=add_request.eta_usec() / 1000000, host=self._internal_host, method=add_request.RequestMethod_Name(add_request.method()), name=add_request.task_name(), payload=base64.b64encode(add_request.body()), port=self._internal_port, queue=queue, try_count=0, url=add_request.url())
