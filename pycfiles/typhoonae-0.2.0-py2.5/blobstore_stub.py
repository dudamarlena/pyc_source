...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/typhoonae/blobstore/blobstore_stub.py
...
class BlobstoreServiceStub(apiproxy_stub.APIProxyStub):
    ...
    def __init__(self, blob_storage, time_function=time.time, service_name='blobstore', uploader_path='upload/'):
        """Constructor.
    ...
    def _GetEnviron(self, name):
        ...
        try:
            ...
            raise blobstore_stub.ConfigurationError('%s is not set in environment.' % name)
    ...
    def _Dynamic_CreateUploadURL(self, request, response):
        ...
        """
        session = self._CreateSession(request.success_path(), users.get_current_user())
    ...
    def _Dynamic_FetchData(self, request, response):
        ...
        if fetch_size > blobstore.MAX_BLOB_FETCH_SIZE:
            raise apiproxy_errors.ApplicationError(blobstore_service_pb.BlobstoreServiceError.BLOB_FETCH_SIZE_TOO_LARGE)
