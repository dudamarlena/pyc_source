...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/typhoonae/blobstore/handlers.py
# Compiled at: 2010-12-12 04:36:57
"""TyphoonAE's handler library for Blobstore API."""
import base64, cStringIO, cgi, datetime, google.appengine.api.blobstore, google.appengine.api.datastore, google.appengine.api.datastore_errors, hashlib, httplib, logging, os, re
SALT = 'salt'
BLOB_KEY_HEADER = google.appengine.api.blobstore.BLOB_KEY_HEADER
UPLOAD_INFO_CREATION_HEADER = google.appengine.api.blobstore.UPLOAD_INFO_CREATION_HEADER
...
def EncodeBlobKey(path):
    ...
    """
    blob_key = base64.urlsafe_b64encode('%i.%s' % (int(os.path.basename(path)),
     hashlib.md5(os.environ['APPLICATION_ID'] + SALT).digest()))
    datastore_key = google.appengine.api.datastore.Key.from_path(google.appengine.api.blobstore.BLOB_INFO_KIND, blob_key)
    try:
...
class UploadCGIHandler(object):
    ...
    def __call__(self, fp, environ):
        ...
        """
        match = re.match(UPLOAD_URL_PATTERN % self.upload_url, environ['PATH_INFO'])
        if match == None:
            return fp
        upload_session_key = match.group(1).strip('/')
        try:
            upload_session = google.appengine.api.datastore.Get(upload_session_key)
        except google.appengine.api.datastore_errors.EntityNotFoundError:
        ...
        def splitContentType(content_type):
            ...
            pairs = dict([ (key.lower().strip(), value) for (key, value) in [ p.split('=', 1) for p in parts[1:] ] ])
            return (
             parts[0].strip(), pairs)
        ...
        fields = [ f for f in set([ k.split('.')[0] for k in data.keys() ]) if f + '.content_type' in data
                 ]
        ...
        for field in fields:
            ...
            blob_entity = google.appengine.api.datastore.Entity('__BlobInfo__', name=blobkey)
            ...
            values = dict(blob_key_header=BLOB_KEY_HEADER, blob_key=blobkey, filename=data[(field + '.name')], content_type=data[(field + '.content_type')], content_length=data[(field + '.size')], creation_header=UPLOAD_INFO_CREATION_HEADER, timestamp=timestamp)
        ...
        for field in data:
            message.append('--' + boundary)
            message.append(SIMPLE_FIELD % {'name': field, 'value': data[field]})
...
class CGIResponseRewriter(object):
    ...
    def __call__(self, fp, environ):
        ...
        if BLOB_KEY_HEADER in ('').join(headers):
            ...
            try:
                blob_info = google.appengine.api.datastore.Get(google.appengine.api.datastore.Key.from_path(google.appengine.api.blobstore.BLOB_INFO_KIND, blob_key))
            except google.appengine.api.datastore_errors.EntityNotFoundError:
                return fp
            else:
                ...
                output.write(REDIRECT_HEADER % _URI(DecodeBlobKey(blob_info.key())))
                output.write('\n')
                return output
        return fp