...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/typhoonae/mysql/datastore_mysql_stub.py
...
Datastore stub.
"""
import array, itertools, logging, md5, sys, threading, time
...
try:
    ...
    taskqueue_service_pb = sys.modules.get('google.appengine.api.taskqueue.taskqueue_service_pb')
except ImportError:
...
_MAX_TIMEOUT = 5.0
_OPERATOR_MAP = {datastore_pb.Query_Filter.LESS_THAN: '<', 
   datastore_pb.Query_Filter.LESS_THAN_OR_EQUAL: '<=', 
   datastore_pb.Query_Filter.EQUAL: '=', 
   datastore_pb.Query_Filter.GREATER_THAN: '>', 
   datastore_pb.Query_Filter.GREATER_THAN_OR_EQUAL: '>='}
_ORDER_MAP = {datastore_pb.Query_Order.ASCENDING: 'ASC', 
...
class QueryCursor(object):
    ...
    """
        while self.__cursor and (not self.__next_result[0] or self.__next_result[0] in self.__seen):
    ...
    """
        ...
        if self.__query.order_list():
            ...
            direction = datastore_pb.Query_Order.ASCENDING
        if direction == datastore_pb.Query_Order.ASCENDING:
            if self.__query.has_end_compiled_cursor() and target_position >= self.__query.end_compiled_cursor().position(0).start_key():
                self.__position = (
...
class DatastoreMySQLStub(apiproxy_stub.APIProxyStub):
  ...
  """
    ...
    _INDEX_STATE_TRANSITIONS = {WRITE_ONLY: frozenset((READ_WRITE, DELETED, ERROR)), 
       READ_WRITE: frozenset((DELETED,)), 
       ERROR: frozenset((DELETED,)), 
       DELETED: frozenset((ERROR,))}

    def __init__(self, app_id, database_info_dict, require_indexes=False, verbose=False, service_name='datastore_v3', trusted=False):
        """Constructor.
    ...
    """
        ...
        self.__connection = MySQLdb.connect(host=database_info_dict.get('host', '127.0.0.1'), user=database_info_dict.get('user', 'root'), passwd=database_info_dict.get('passwd', ''), db='mysql')
    ...
    def __Init(self):
        ...
        cursor.execute('CREATE DATABASE IF NOT EXISTS %s' % self.__database_info_dict['db'])
        cursor.close()
    ...
    def Clear(self):
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("SELECT TABLE_NAME FROM information_schema.Tables WHERE TABLE_SCHEMA='%s';" % self.__database_info_dict['db'])
    ...
    """
        ...
        if tx.handle() != self.__current_transaction:
            raise apiproxy_errors.ApplicationError(datastore_pb.Error.BAD_REQUEST, 'Transaction %s not found' % tx)
    ...
    """
        ...
        for elem in key.path().element_list():
            if elem.has_id() == elem.has_name():
                raise datastore_errors.BadRequestError('each key path element should have id or name but not both: %r' % key)
    ...
    """
        ...
        if request_tx != self.__current_transaction:
            raise apiproxy_errors.ApplicationError(datastore_pb.Error.BAD_REQUEST, 'Only one concurrent transaction per thread is permitted.')
    ...
    """
        ...
        for (prefix, group) in itertools.groupby(entities, lambda x: x[0]):
            ...
            cursor.executemany('REPLACE INTO %s_Entities VALUES (%%s, %%s, %%s)' % prefix, group_rows)
    ...
    """
        ...
        def RowGenerator(entities):
            for (unused_prefix, e) in entities:
                for p in e.property_list():
                    p_vals = [
                     self.__GetEntityKind(e), p.name(), self.__EncodeIndexPB(p.value()), self.__EncodeIndexPB(e.key().path())]
        ...
        for (prefix, group) in itertools.groupby(entities, lambda x: x[0]):
            cursor = conn.cursor()
            cursor.executemany('INSERT IGNORE INTO %s_EntitiesByProperty (kind, name, value, __path__, hashed_index)  VALUES (%%s, %%s, %%s, %%s, %%s)' % prefix, RowGenerator(group))
    ...
    """
        ...
        if size is not None:
            ...
            if not block_size:
                ...
                cursor.execute('UPDATE IdSeq SET next_id = next_id + %s WHERE prefix = %s', (
                 block_size, prefix))
            ...
            if max:
                if max >= ret:
                    cursor.execute('UPDATE IdSeq SET next_id = %s WHERE prefix = %s', (
                     max + 1, prefix))
                else:
                    raise int(cursor.rowcount) == 1 or AssertionError
    ...
    def MakeSyncCall(self, service, call, request, response):
        ...
        try:
            super(DatastoreMySQLStub, self).MakeSyncCall(service, call, request, response)
    ...
    def QueryHistory(self):
        ...
        return dict(((pb, times) for (pb, times) in self.__query_history.items() if pb.app() == self.__app_id))
    ...
    def _Dynamic_Put(self, put_request, put_response):
        ...
        try:
            ...
            if put_request.has_transaction():
                if put_request.transaction() in self.__transactions and not self.__inside_tx:
            ...
            for entity in entities:
                ...
                for prop in itertools.chain(entity.property_list(), entity.raw_property_list()):
                ...
                if last_path.id() == 0 and not last_path.has_name():
                    ...
                    assert entity.has_entity_group() and entity.entity_group().element_size() > 0
    ...
    def _Dynamic_Get(self, get_request, get_response):
        ...
        try:
            ...
            if get_request.has_transaction():
                if get_request.transaction() in self.__transactions and not self.__inside_tx:
                    entity_group = self.__ExtractEntityGroupFromKeys(keys)
                    self.__inside_tx = True
            ...
            for key in keys:
                ...
                cursor = conn.cursor()
                cursor.execute('SELECT entity FROM %s_Entities WHERE __path__ = %%s' % prefix, (
    ...
    def _Dynamic_Delete(self, delete_request, delete_response):
        ...
        try:
            ...
            if delete_request.has_transaction():
                if delete_request.transaction() in self.__transactions and not self.__inside_tx:
                    entity_group = self.__ExtractEntityGroupFromKeys(keys)
                    self.__inside_tx = True
    ...
    """
        ...
        ancestor_max = buffer(str(ancestor_min) + b'\xfb\xff\xff\xff\x89')
    ...
    def __KindQuery(self, query, filter_info, order_info):
        ...
        if order_info:
            orders = [
             (
              '__path__', order_info[0][1])]
        else:
            orders = [
             (
              '__path__', datastore_pb.Query_Order.ASCENDING)]
        params = []
        query = 'SELECT Entities.__path__, Entities.entity, %s FROM %s_Entities AS Entities %s %s' % (
    ...
    def __SinglePropertyQuery(self, query, filter_info, order_info):
        ...
        if len([ 1 for (o, _) in filter_ops if o == datastore_pb.Query_Filter.EQUAL
               ]) > 1:
            return
        ...
        orders = [
         (
          'EntitiesByProperty.kind', datastore_pb.Query_Order.ASCENDING),
         (
        ...
        format_args = (
         ...
         self.__CreateFilterString(filters, params),
         self.__CreateOrderString(orders))
        query = 'SELECT Entities.__path__, Entities.entity, %s FROM %s_EntitiesByProperty AS EntitiesByProperty INNER JOIN %s_Entities AS Entities USING (__path__) %s %s' % format_args
        return (query, params)
    ...
    """
        ...
        for (name, filter_ops) in filter_sets:
            ...
            joins.append('INNER JOIN %s_EntitiesByProperty AS %s ON Entities.__path__ = %s.__path__' % (
        ...
        format_args = (
         ...
         self.__CreateFilterString(filters, params),
         self.__CreateOrderString(orders))
        query = 'SELECT Entities.__path__, Entities.entity, %s FROM %s_Entities AS Entities %s %s %s' % format_args
        return (query, params)
    ...
    """
        if self.__require_indexes:
            ...
            if not index:
                raise apiproxy_errors.ApplicationError(datastore_pb.Error.NEED_INDEX, 'This query requires a composite index that is not defined. You must update the index.yaml file in your application root.')
        return self.__StarSchemaQueryPlan(query, filter_info, order_info)
    ...
    """
        (unused_required, kind, ancestor, props, num_eq_filters) = datastore_index.CompositeIndexForQuery(query)
        ...
        for index in indexes:
            ...
            if num_eq_filters > 1 and (kind, ancestor) == index_key[:2]:
                ...
                if eq_filters_set == this_eq_filters_set and remaining_filters == this_remaining_filters:
                    return index
    ...
    """
        ...
        if num_components > _MAX_QUERY_COMPONENTS:
            raise apiproxy_errors.ApplicationError(datastore_pb.Error.BAD_REQUEST, 'query is too large. may not have more than %s filters + sort orders ancestor total' % _MAX_QUERY_COMPONENTS)
        app_id = query.app()
        ...
        for strategy in DatastoreMySQLStub._QUERY_STRATEGIES:
            ...
            raise apiproxy_errors.ApplicationError(datastore_pb.Error.BAD_REQUEST, 'No strategy found to satisfy query.')
        ...
        if self.__verbose:
            logging.debug("Executing statement '%s' with arguments %r", sql_stmt, [ str(x) for x in params ])
    ...
    def _Dynamic_Next(self, next_request, query_result):
        ...
        try:
            ...
            raise apiproxy_errors.ApplicationError(datastore_pb.Error.BAD_REQUEST, 'Cursor %d not found' % next_request.cursor().cursor())
    ...
    def _Dynamic_AddActions(self, request, _):
        if len(self.__tx_actions) + request.add_request_size() > _MAX_ACTIONS_PER_TXN:
            raise apiproxy_errors.ApplicationError(datastore_pb.Error.BAD_REQUEST, 'Too many messages, maximum allowed %s' % _MAX_ACTIONS_PER_TXN)
        new_actions = []
    ...
    def _Dynamic_Commit(self, transaction, _):
        ...
        try:
            ...
            for action in self.__tx_actions:
                try:
                    apiproxy_stub_map.MakeSyncCall('taskqueue', 'Add', action, api_base_pb.VoidProto())
                except apiproxy_errors.ApplicationError, e:
                    logging.warning('Transactional task %s has been dropped, %s', action, e)
            ...
            self.__tx_deletes = set()
            self.__ReleaseConnection(conn, None)
            self.__ReleaseLockForEntityGroup(self.__connection, self.__transactions[transaction] or '')
    ...
    def _Dynamic_GetSchema(self, req, schema):
        ...
        try:
            ...
            params = []
            if req.properties():
                sql_stmt = 'SELECT kind, name, value FROM %s_EntitiesByProperty %s GROUP BY kind, name, substr(value, 1, 1) ORDER BY kind' % (
    ...
    def _Dynamic_AllocateIds(self, allocate_ids_request, allocate_ids_response):
        ...
        if allocate_ids_request.has_size() and allocate_ids_request.has_max():
            raise apiproxy_errors.ApplicationError(datastore_pb.Error.BAD_REQUEST, 'Both size and max cannot be set.')
        if allocate_ids_request.has_size():
            if allocate_ids_request.size() < 1:
                raise apiproxy_errors.ApplicationError(datastore_pb.Error.BAD_REQUEST, 'Size must be greater than 0.')
            first_id = self.__AllocateIds(conn, self.__GetTablePrefix(model_key), size=allocate_ids_request.size())
            ...
            if allocate_ids_request.max() < 0:
                raise apiproxy_errors.ApplicationError(datastore_pb.Error.BAD_REQUEST, 'Max must be greater than or equal to 0.')
            first_id = self.__AllocateIds(conn, self.__GetTablePrefix(model_key), max=allocate_ids_request.max())
            allocate_ids_response.set_start(first_id)
            allocate_ids_response.set_end(max(allocate_ids_request.max(), first_id - 1))
        self.__ReleaseConnection(conn, None)
        return
    ...
    def _Dynamic_CreateIndex(self, index, id_response):
        ...
        try:
            if self.__FindIndex(index):
                raise apiproxy_errors.ApplicationError(datastore_pb.Error.BAD_REQUEST, 'Index already exists.')
            next_id = max([ idx.id() for x in self.__indexes.get(app_id, {}).values() for idx in x
                          ] + [0]) + 1
    ...
    def _Dynamic_UpdateIndex(self, index, _):
        ...
        if not my_index:
            raise apiproxy_errors.ApplicationError(datastore_pb.Error.BAD_REQUEST, "Index doesn't exist.")
        elif index.state() != my_index.state() and index.state() not in self._INDEX_STATE_TRANSITIONS[my_index.state()]:
            raise apiproxy_errors.ApplicationError(datastore_pb.Error.BAD_REQUEST, 'Cannot move index state from %s to %s' % (
