...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/typhoonae/taskqueue/celery_tasks.py
...
import base64, datetime, logging, os, re, urllib2
...
def handle_task(task, **task_args):
    ...
    logger = task.get_logger(task_id=task_args['task_id'], task_name=task_args['task_name'])
    headers = {'Content-Type': task_args['content_type'], 'X-AppEngine-TaskName': task_args['name'], 
       'X-AppEngine-TaskRetryCount': str(task_args['try_count']), 
       'X-AppEngine-QueueName': task_args['queue']}
    req = RequestWithMethod(method=task_args['method'], url='http://%(host)s:%(port)s%(url)s' % task_args, data=base64.b64decode(task_args['payload']), headers=headers)
...
def create_task_queue(queue_name, rate_qps, bucket_size=5):
    ...
    if rate_qps < 999999:
        ...
        rate_limit = None
    return type(task_class_name, (Task,), dict(routing_key=queue_name, name=task_class_name, rate_limit=rate_limit, run=handle_task, ignore_result=True, send_error_emails=False, __module__=create_task_queue.__module__))
...
def create_task_queues_from_yaml(app_root):
    ...
    if queue_info and queue_info.queue:
        for entry in queue_info.queue:
            tasks[entry.name] = create_task_queue(entry.name, queueinfo.ParseRate(entry.rate), entry.bucket_size)
