...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/typhoonae/__init__.py
...
def initURLMapping(conf, options):
    ...
    url_mapping = []
    add_handlers = [ appinfo.URLMap(url=url, script=script) for (url, script) in [
     (
     ...
     ('/_ah/dev/null', '$PYTHON_LIB/typhoonae/handlers/devnull.py')] if url not in [ h.url for h in conf.handlers if h.url ]
                   ]
        ...
        if script != None:
            if script.startswith('$PYTHON_LIB'):
                ...
                try:
                    ...
                    logging.error("Could not initialize script '%s'. %s: %s" % (
...
def setupDatastore(options, conf, datastore_file, history, require_indexes, trusted):
    ...
    if name == 'mongodb':
        ...
        datastore = datastore_mongo_stub.DatastoreMongoStub(conf.application, require_indexes=require_indexes)
    elif name == 'bdbdatastore':
        from notdot.bdbdatastore import socket_apiproxy_stub
        datastore = socket_apiproxy_stub.RecordingSocketApiProxyStub(('localhost',
                                                                      9123))
        end_request_hook = datastore.closeSession
    elif name == 'mysql':
        from typhoonae.mysql import datastore_mysql_stub
        database_info = {'host': options.mysql_host, 'user': options.mysql_user, 
           'passwd': options.mysql_passwd, 
           'db': options.mysql_db}
        datastore = typhoonae.mysql.datastore_mysql_stub.DatastoreMySQLStub(conf.application, database_info, verbose=options.debug_mode)
    else:
...
def setupMail(smtp_host, smtp_port, smtp_user, smtp_password, enable_sendmail=False, show_mail_body=False):
    """Sets up mail."""
    from google.appengine.api import mail_stub
    apiproxy_stub_map.apiproxy.RegisterStub('mail', mail_stub.MailServiceStub(smtp_host, smtp_port, smtp_user, smtp_password, enable_sendmail=enable_sendmail, show_mail_body=show_mail_body))
...
def setupTaskQueue(internal_address, use_celery=False, root_path='.'):
    ...
    apiproxy_stub_map.apiproxy.RegisterStub('taskqueue', taskqueue_stub.TaskQueueServiceStub(internal_address=internal_address, root_path=root_path))
...
def setupUserService(login_url='/_ah/login', logout_url='/_ah/logout'):
    ...
    apiproxy_stub_map.apiproxy.RegisterStub('user', user_service_stub.UserServiceStub(login_url=login_url + '?continue=%s', logout_url=logout_url + '?continue=%s'))
...
def setupXMPP(host):
    ...
    apiproxy_stub_map.apiproxy.RegisterStub('xmpp', xmpp_service_stub.XmppServiceStub(host=host))
...
def setupChannel(internal_addr):
    ...
    apiproxy_stub_map.apiproxy.RegisterStub('channel', channel_service_stub.ChannelServiceStub(internal_addr))
...
def setupBlobstore(blobstore_path, app_id):
    ...
    storage = file_blob_storage.FileBlobStorage(blobstore_path, app_id)
    apiproxy_stub_map.apiproxy.RegisterStub('blobstore', blobstore_stub.BlobstoreServiceStub(storage))


def setupWebSocket():
    """Sets up Web Socket service."""
    from typhoonae.websocket import websocket_stub
    apiproxy_stub_map.apiproxy.RegisterStub('websocket', websocket_stub.WebSocketServiceStub())
...
def setupRemoteDatastore(app_id, email, password):
    ...
    remote_api_stub.ConfigureRemoteApi(app_id, '/remote_api', lambda : (email, password), secure=True, servername=app_id + '.appspot.com', services=['datastore_v3'])
...
def setupStubs(conf, options):
    ...
    if datastore in SUPPORTED_DATASTORES:
        if datastore == 'remote':
            ...
            setupDatastore(options, conf, 'dev_appserver.datastore', 'dev_appserver.datastore.history', False, False)
    setupCapability()
    setupMail(options.smtp_host, options.smtp_port, options.smtp_user, options.smtp_password)
    setupMemcache()
