...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/typhoonae/handlers/json_rpc.py
...
import cgi, logging, sys, traceback
    ...
    def from_json(self, json):
        ...
        try:
            if not isinstance(json, dict):
                raise InvalidRequestError('Invalid JSON-RPC Message. Must be an object.')
...
class JsonRpcHandler(webapp.RequestHandler):
    ...
    def execute_method(self, method, params):
        ...
        if params is None:
            if not len(args) == 0:
                raise InvalidParamsError("Wrong number of parameters. Expected %i but 'params' was omitted from json-rpc message." % len(args))
            return method()
        if isinstance(params, (list, tuple)):
            if not len(args) == len(params):
                raise InvalidParamsError('Wrong number of parameters. Expected %i got %i.' % (
                 len(args), len(params)))
            return method(*params)
        if isinstance(params, dict):
            paramset = set(params)
            if not args == paramset:
                raise InvalidParamsError('Named parameters do not match method. Expected %s.' % str(args))
    ...
    def get_service_method(self, meth_name):
        ...
        if f == None or not hasattr(f, 'IsServiceMethod') or not getattr(f, 'IsServiceMethod') == True:
