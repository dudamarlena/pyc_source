...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/typhoonae/taskqueue/worker.py
# Compiled at: 2010-12-12 04:36:57
"""Task worker implementation."""
from __future__ import with_statement
from amqplib import client_0_8 as amqp
from google.appengine.api.taskqueue import taskqueue_service_pb
import base64, logging, optparse, os, signal, simplejson, socket, sys, time, typhoonae.taskqueue, urllib2
DESCRIPTION = 'AMQP client to perform queued tasks.'
USAGE = 'usage: %prog [options]'
MAX_CONN_RETRIES = 10
MAX_TASK_RETRIES = 10
MAX_TIME_LIMIT = 30
HTTP_METHOD_MAP = {taskqueue_service_pb.TaskQueueAddRequest.GET: 'GET', 
...
def handle_task(msg):
    ...
    if typhoonae.taskqueue.is_deferred_eta(task['eta']):
        return False
    headers = {'Content-Type': task['content_type'], 'X-AppEngine-TaskName': task['name'], 
       'X-AppEngine-TaskRetryCount': str(task['try_count']), 
       'X-AppEngine-QueueName': task['queue']}
    req = RequestWithMethod(url='http://%(host)s:%(port)s%(url)s' % task, data=base64.b64decode(task['payload']), headers=headers, method=HTTP_METHOD_MAP[task.get('method', taskqueue_service_pb.TaskQueueAddRequest.POST)])
    try:
...
def main(queue='tasks', exchange='immediate', routing_key='normal_worker'):
    ...
    while True:
        reason = None
        try:
            conn = amqp.Connection(host='%s:5672' % options.amqp_host, userid='guest', password='guest', virtual_host='/', insist=False)
            if conn:
        ...
        if conn_retry_count > MAX_CONN_RETRIES:
            logging.error('broker not reachable. Giving up. Reason: %s' % err_obj)
            sys.exit(1)
    ...
    def recv_callback(msg):
        if not handle_task(msg):
            ...
            if task_dict['try_count'] < MAX_TASK_RETRIES:
                ...
                task_dict['eta'] = typhoonae.taskqueue.get_new_eta_usec(task_dict['try_count'])
                new_msg = amqp.Message(simplejson.dumps(task_dict))
                new_msg.properties['delivery_mode'] = 2
                new_msg.properties['task_name'] = task['name']
                chan.basic_publish(new_msg, exchange='deferred', routing_key='deferred_worker')
        chan.basic_ack(msg.delivery_tag)

    _consumer_tag = 'consumer.%i' % os.getpid()
    chan.basic_consume(queue=queue, no_ack=False, callback=recv_callback, consumer_tag=_consumer_tag)
    try:
        while True:
            chan.wait()
