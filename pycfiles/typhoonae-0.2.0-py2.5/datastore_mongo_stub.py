...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/typhoonae/mongodb/datastore_mongo_stub.py
...
import logging, pymongo, re, random, sys, threading, types
try:
...
class DatastoreMongoStub(apiproxy_stub.APIProxyStub):
  ...
  """
    ...
    def __init__(self, app_id, datastore_file=None, require_indexes=False, service_name='datastore_v3'):
        """Constructor.
    ...
    """
        ...
        super(DatastoreMongoStub, self).MakeSyncCall(service, call, request, response)
    ...
    def __key_for_id(self, id):
        ...
        return datastore_types.Key.from_path(*[ from_db(a) for a in id.split('\x08') ])

    def __create_mongo_value_for_value(self, value):
        if isinstance(value, datastore_types.Rating):
            return {'class': 'rating', 'rating': int(value)}
        if isinstance(value, datastore_types.Category):
            return {'class': 'category', 'category': str(value)}
        if isinstance(value, datastore_types.Key):
            return {'class': 'key', 'path': self.__id_for_key(value._ToPb())}
        ...
        if isinstance(value, users.User):
            return {'class': 'user', 'email': value.email()}
        if isinstance(value, datastore_types.Text):
            return {'class': 'text', 'string': unicode(value)}
        ...
        if isinstance(value, datastore_types.IM):
            return {'class': 'im', 'protocol': value.protocol, 
               'address': value.address}
        if isinstance(value, datastore_types.GeoPt):
            return {'class': 'geopt', 'lat': value.lat, 
               'lon': value.lon}
        if isinstance(value, datastore_types.Email):
            return {'class': 'email', 'value': value}
        if isinstance(value, datastore_types.BlobKey):
            return {'class': 'blobkey', 'value': str(value)}
        return value
    ...
    """
        ...
        if size is not None:
            ...
            if not block_size:
                ...
                result = self.__db.command('findandmodify', 'datastore', query={'_id': _id}, update={'$inc': {'next_id': next_id + block_size}})
                next_id = int(result['value']['next_id'])
            if size > block_size:
                result = self.__db.command('findandmodify', 'datastore', query={'_id': _id}, update={'$inc': {'next_id': size}})
    ...
    def __filter_binding(self, key, value, operation, prototype):
        ...
        if operation == '<':
            ...
            return (
             key, value)
        raise apiproxy_errors.ApplicationError(datastore_pb.Error.BAD_REQUEST, "Can't handle operation %r." % operation)
    ...
    """
        ...
        (count, query_info_encoded, entity_encoded) = position.start_key().split(_CURSOR_CONCAT_STR)
    ...
    def _Dynamic_RunQuery(self, query, query_result):
        ...
        operators = {datastore_pb.Query_Filter.LESS_THAN: '<', datastore_pb.Query_Filter.LESS_THAN_OR_EQUAL: '<=', 
        ...
        for filt in query.filter_list():
            ...
            filter_val_list = [ datastore_types.FromPropertyPb(filter_prop) for filter_prop in filt.property_list()
                              ]
        ...
        if query.has_compiled_cursor():
            (offset, query_pb, unused_spec, incl) = self._DecodeCompiledCursor(query.compiled_cursor())
    ...
    def _Dynamic_Next(self, next_request, query_result):
        ...
        try:
            ...
            raise apiproxy_errors.ApplicationError(datastore_pb.Error.BAD_REQUEST, 'Cursor %d not found' % cursor)
    ...
    def __collection_and_spec_for_index(self, index):
        ...
        def translate_direction(ae_dir):
            ...
            raise apiproxy_errors.ApplicationError(datastore_pb.Error.BAD_REQUEST, 'Weird direction.')
    ...
    def __has_index(self, index):
        ...
        if self.__gen_index_name(spec) in self.__db[collection].index_information().keys():
            return True
        return False
    ...
    def _Dynamic_CreateIndex(self, index, id_response):
        ...
        if spec:
            ...
            if self.__db.error():
                raise apiproxy_errors.ApplicationError(datastore_pb.Error.BAD_REQUEST, 'Error creating index. Maybe too many indexes?')
        id_response.set_value(1)

    def _Dynamic_GetIndices(self, app_str, composite_indices):
        if app_str.value() != self.__db.name():
            raise apiproxy_errors.ApplicationError(datastore_pb.Error.BAD_REQUEST, 'Getting indexes for a different app unsupported.')
