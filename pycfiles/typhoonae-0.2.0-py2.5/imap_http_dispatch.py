...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/typhoonae/mail/imap_http_dispatch.py
# Compiled at: 2010-12-12 04:36:57
"""Simple IMAP/HTTP dispatcher implementation."""
from imaplib2 import IMAP4, IMAP4_SSL
import datetime, email, logging, optparse, urllib2, time
...
def main():
    ...
    op.add_option('--imap_host', dest='imap_host', metavar='IMAPHOST', help='IMAP Server hostname', default='localhost')
    op.add_option('--imap_port', dest='imap_port', metavar='IMAPPORT', help='IMAP Server port', type='int', default=143)
    op.add_option('--imap_ssl', dest='imap_ssl', metavar='IMAPSSL', action='store_true', help="If it's IMAP SSL", default=False)
    op.add_option('-u', '--imap_user', dest='imap_user', metavar='IMAPUSER', help='IMAP Server username', default='username@example.com')
    op.add_option('-p', '--imap_password', dest='imap_password', metavar='IMAPPASS', help='IMAP Server password', default='secret')
    op.add_option('--imap_mailbox', dest='imap_mailbox', metavar='IMAPMBOX', help='IMAP Server Mailbox', default='INBOX')
