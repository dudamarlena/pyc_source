...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/typhoonae/appcfg/service.py
# Compiled at: 2010-12-12 12:07:45
...
import ConfigParser, Cookie, cStringIO, datetime, logging, mimetools, multifile, optparse, os, shutil, signal, socket, sys, re, threading, time, typhoonae.apptool, typhoonae.fcgiserver
...
class RequestHandler(WSGIRequestHandler):
    ...
    def handle(self):
        ...
        if not self.parse_request():
            return
        handler = WSGIServerHandler(self.rfile, self.wfile, self.get_stderr(), self.get_environ())
...
class AppversionHandler(webapp.RequestHandler):
    ...
    def _extractFileTuples(data):
        tuples = [ tuple(f.split(TUPLE_DELIMITER)) for f in data.split(LIST_DELIMITER)
                 ]
    ...
    def getAppversionDirectory(appversion):
        ...
        """
        app_dir = os.path.join(os.path.abspath(os.environ['APPS_ROOT']), appversion.app_id, appversion.current_version_id)
        return app_dir
    ...
    def createAppversionDirectory(cls, appversion):
        ...
        if os.path.isdir(app_dir):
            raise AppConfigServiceError("Application exists (app_id=u'%s')." % appversion.app_id)
        os.makedirs(app_dir)
        return app_dir
    ...
    def getAppversion(app_id, version, state):
        ...
        """
        query = db.GqlQuery('SELECT * FROM Appversion WHERE app_id = :1 AND version = :2 AND state = :3 ORDER BY updated DESC', app_id, version, state)
        return query.get()

    def _RpcMethod_create(self, app_id, version, path, data):
        if self.getAppversion(app_id, version, STATE_UPDATING):
            raise AppConfigServiceError("Already updating application (app_id=u'%s')." % app_id)
        ...
        appversion.current_version_id = makeCurrentAppversionId(version, now)
    ...
    def _RpcMethod_clonefiles(self, app_id, version, path, data):
        ...
        self.response.out.write(LIST_DELIMITER.join([ filename for (filename, _) in files ]))

    def _RpcMethod_cloneblobs(self, app_id, version, path, data):
        files = self._extractFileTuples(data)
        self.response.out.write(LIST_DELIMITER.join([ filename for (filename, _, mimetype) in files ]))
...
class DeleteHandler(webapp.RequestHandler):
    ...
    def get(self):
        ...
        query = db.GqlQuery('SELECT * FROM Appversion WHERE app_id = :1 AND version = :2', app_id, version)
        ...
        portconfig = PortConfig.get_by_key_name('portconfig_%s' % options.server_name.replace('.', '_'))
...
class AppConfigService(object):
    ...
    def __init__(self, addr, app, apps_root, var, verbose=False):
        ...
        datastore = datastore_sqlite_stub.DatastoreSqliteStub(APPLICATION_ID, datastore_path)
    ...
    def serve_forever(self):
        ...
        try:
            logging.debug('HTTP server listening on %s:%i', self.host, self.port)
            server = make_server(self.host, self.port, self.app, handler_class=RequestHandler)
...
class Options(object):
    ...
    def __init__(self, options, environ):
        ...
        for opt in options:
            val = options[opt]
            for key in environ:
                val = re.sub('\\$%s' % key, environ[key], val)
    ...
    def __getattr__(self, key):
        try:
            ...
            raise AppConfigServiceError("Option key '%s' not found" % key)

        if re.match('^([0-9]+)$', val):
            val = int(val)
        elif re.match('^(False|True)$', val, re.M):
            val = eval(val)
        return val
...
def readDefaultOptions():
    ...
    environ = {'HOSTNAME': os.environ.get('HOSTNAME', socket.getfqdn()), 
       'SERVER_SOFTWARE': os.environ.get('SERVER_SOFTWARE', typhoonae.fcgiserver.SERVER_SOFTWARE)}
...
def configureAppversion(appversion, app_dir, options):
    ...
    def write_httpd_conf(default_version=False):
        ...
        if options.ssl_enabled:
            paths += func(options, conf, app_dir, default_version, secure=True, mode='a')
        paths += func(options, conf, app_dir, default_version, internal=True, mode='a')
        return paths

    conf_paths = set()
    conf_paths.update(write_httpd_conf())
    conf_paths.update(write_httpd_conf(True))
    typhoonae.apptool.make_blobstore_dirs(os.path.abspath(os.path.join(options.blobstore_path, conf.application)))
    conf_paths.update(typhoonae.apptool.write_supervisor_conf(options, conf, app_dir))
...
class DeploymentThread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            raise AppConfigServiceError("Internal error when deploying application (app_id=u'%s')." % self.app_id)

        options = readDefaultOptions()
        portconfig = PortConfig.get_or_insert('portconfig_%s' % options.server_name.replace('.', '_'))
        if self.appversion.fcgi_port and self.appversion.portconfig:
            ...
            if len(portconfig.used_ports) == MAX_USED_PORTS:
                raise AppConfigServiceError('Maximum number of installed applications reached.')
...
def main():
    ...
    op.add_option('-a', '--address', dest='address', metavar='ADDR', help='use this address', default=DEFAULT_ADDRESS)
    op.add_option('--apps_root', dest='apps_root', metavar='PATH', help='the root directory of all application directories', default=typhoonae.apptool.setdir(os.path.abspath(os.path.join('.', 'var'))))
    op.add_option('-c', '--config', dest='config_file', metavar='FILE', help='read configuration from this file')
    op.add_option('-d', '--debug', dest='debug_mode', action='store_true', help='enables debug mode', default=False)
    op.add_option('--var', dest='var', metavar='PATH', help='use this directory for platform independent data', default=typhoonae.apptool.setdir(os.path.abspath(os.path.join('.', 'var'))))
    ...
    if not os.path.isfile(os.environ[CONFIG_FILE_KEY]):
        logging.error('Configuration file "%s" not found', os.environ[CONFIG_FILE_KEY])
        sys.exit(1)
    service = AppConfigService(options.address, app, options.apps_root, options.var, options.debug_mode)
