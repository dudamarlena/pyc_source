...
# [GCC 8.4.0]
# Embedded file name: /Users/david/projects/github_astro/academic_projects_public/pycecream/pycecream/__init__.py
# Compiled at: 2019-02-27 15:54:22
...
class pycecream():
 ...
 'var', 'multiplicative'], name=None, share_previous_lag=False):
        """
    ...
    def set_tophat(self):
        ...
        for i in range(self.count_lightcurves):
            f.write(np.str(self.lightcurve_input_params['tophat centroid'].values[i]) + ' ' + np.str(self.lightcurve_input_params['tophat centroid step'].values[i]) + ' 0.0 -1.0 ' + np.str(self.lightcurve_input_params['tophat width'].values[i]) + ' ' + np.str(self.lightcurve_input_params['tophat width step'].values[i]) + ' 0.0 -1.0\n')
    ...
    def get_light_curve_fits(self, location=None):
        ...
        function_output = {'model': self.output_model, 'merged model': self.output_merged_model, 
