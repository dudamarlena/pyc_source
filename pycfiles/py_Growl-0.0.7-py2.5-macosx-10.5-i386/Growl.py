...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/Growl.py
# Compiled at: 2008-09-01 11:29:39
"""
...
import _growl, types, struct, md5, socket
GROWL_UDP_PORT = 9887
...
class netgrowl:
    ...
    __notAllowed__ = [
     GROWL_APP_ICON, GROWL_NOTIFICATION_ICON, GROWL_NOTIFICATION_APP_ICON]
    ...
    def PostNotification(self, userInfo):
        ...
        if userInfo.has_key(GROWL_NOTIFICATION_STICKY):
            ...
            sticky = False
        data = self.encodeNotify(userInfo[GROWL_APP_NAME], userInfo[GROWL_NOTIFICATION_NAME], userInfo[GROWL_NOTIFICATION_TITLE], userInfo[GROWL_NOTIFICATION_DESCRIPTION], priority, sticky)
        return self.send(data)

    def PostRegistration(self, userInfo):
        data = self.encodeRegistration(userInfo[GROWL_APP_NAME], userInfo[GROWL_NOTIFICATIONS_ALL], userInfo[GROWL_NOTIFICATIONS_DEFAULT])
        return self.send(data)

    def encodeRegistration(self, application, notifications, defaultNotifications):
        data = struct.pack('!BBH', GROWL_PROTOCOL_VERSION, GROWL_TYPE_REGISTRATION, len(application))
        data += struct.pack('BB', len(notifications), len(defaultNotifications))
        data += application
    ...
    def encodeNotify(self, application, notification, title, description, priority=0, sticky=False):
        ...
        if sticky:
            flags = flags | 1
        data = struct.pack('!BBHHHHH', GROWL_PROTOCOL_VERSION, GROWL_TYPE_NOTIFICATION, flags, len(notification), len(title), len(description), len(application))
        data += notification
        data += title
        data += description
        data += application
...
class GrowlNotifier(object):
    ...
    def __init__(self, applicationName=None, notifications=None, defaultNotifications=None, applicationIcon=None, hostname=None, password=None):
        ...
        assert (
         self.applicationName, 'An application name is required.')
        if notifications:
            self.notifications = list(notifications)
        assert (
         self.notifications, 'A sequence of one or more notification names is required.')
    ...
    def register(self):
        if self.applicationIcon is not None:
            self.applicationIcon = self._checkIcon(self.applicationIcon)
        regInfo = {GROWL_APP_NAME: self.applicationName, GROWL_NOTIFICATIONS_ALL: self.notifications, 
    ...
    def notify(self, noteType, title, description, icon=None, sticky=False, priority=None):
        ...
        notifyInfo = {GROWL_NOTIFICATION_NAME: noteType, GROWL_APP_NAME: self.applicationName, 
