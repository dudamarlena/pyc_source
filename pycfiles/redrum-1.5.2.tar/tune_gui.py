...
# [GCC 8.4.0]
# Embedded file name: tune_gui.py
# Compiled at: 2017-04-26 10:02:29
import numpy as np, matplotlib.pyplot as plt
from matplotlib.widgets import Slider, Button, RadioButtons
import redrum, math, argparse, json
parser = argparse.ArgumentParser()
parser.add_argument('ids', metavar='imgur_id', type=str, nargs='+', help='image ID to score from the cache')
args = parser.parse_args()
fig, ax = plt.subplots(figsize=(15, 5))
plt.subplots_adjust(left=0.05, bottom=0.3, right=0.95)
x = np.arange(0, 1, 0.001)

def logistic_function(x, midpoint, k):
    return pow(math.e, 1 / (math.e * (1 + pow(math.e, -k * (x - midpoint)))))
...
plot_ratio, = plt.semilogy(0, 1, lw=2, color='gray')
plt.title('Ratio')
...
final_score_a_text = plot_final_score_axis.text(0.05, 0.85, '', bbox=bbox_a)
final_score_b_text = plot_final_score_axis.text(0.05, 0.75, '', bbox=bbox_b)
final_score_ratio_text = plot_final_score_axis.text(0.05, 0.65, '')
...
def update(val):
    ...
    final_score_a, ratio_score_a, views_score_a, pixel_score_a, ratio_logistic_score_a, views_logistic_score_a, pixel_logistic_score_a = redrum.score_image(image_a, max_views)
    final_score_b, ratio_score_b, views_score_b, pixel_score_b, ratio_logistic_score_b, views_logistic_score_b, pixel_logistic_score_b = redrum.score_image(image_b, max_views)
    ...
    final_score_ratio_text.set_text(('ratio = {:.2e}').format(final_score_a / final_score_b))
