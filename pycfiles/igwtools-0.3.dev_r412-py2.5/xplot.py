...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/igwtools/xplot.py
# Compiled at: 2008-04-02 19:02:42
"""
...
class xyp:
    ...
    def plot(self, **kwargs):
        ...
        (hdr, data) = self.dataset[0]
        A = numpy.array(data).reshape(hdr['ny'], hdr['nx'])
        pylab.imshow(A, aspect='auto', origin='lower', extent=[
         self.header['xmin'], self.header['xmax'],
         self.header['ymin'], self.header['ymax']], **kwargs)
        pylab.xlabel('x')
        pylab.ylabel('y')
        title_str = self.header['title'].replace('_', '\\_')
        pylab.title(title_str)
...
def xlook():
    ...
    parser.add_option('-s', '--save', help='save the image (fileformat determined by extension)')
    parser.add_option('--xlabel', help='x-axis label')
    parser.add_option('--ylabel', help='y-axis label')
...
def xylook():
    ...
    parser = OptionParser(usage=usage, version='%prog (igwtools ' + __version__ + ')')
    parser.add_option('-s', '--save', help='save the image (fileformat determined by extension)')
    parser.add_option('--vmin', type='float', dest='vmin', help='colourbar minimum')
    parser.add_option('--vmax', type='float', dest='vmax', help='colourbar maximum')
    parser.add_option('--cmap', help='select a colour map')
    parser.add_option('--list_cmaps', action='store_true', help='print a list of available colour maps')
    parser.add_option('--mask', help='MASK is an .xyp file  of the same dimensions as the target xypfile which is shown behind in grayscale')
    parser.add_option('-i', '--interpolation', dest='interpolation', default='bicubic', help='interpolation mode: nearest, bicubic [default: %default]')
    parser.add_option('--xlabel', help='x-axis label')
    parser.add_option('--ylabel', help='y-axis label')
    (options, args) = parser.parse_args()
    ...
    if options.mask:
        ...
        pylab.hold(True)
        xyp.plot(interpolation=options.interpolation, vmin=options.vmin, vmax=options.vmax, cmap=pylab.cm.get_cmap(options.cmap), alpha=0.6)
    else:
        xyp.plot(interpolation=options.interpolation, vmin=options.vmin, vmax=options.vmax, cmap=pylab.cm.get_cmap(options.cmap))
...
def xydiff():
    ...
    parser.add_option('--cmap', help='select a colour map')
    parser.add_option('--list_cmaps', action='store_true', help='print a list of available colour maps')
    parser.add_option('-i', '--interpolation', dest='interpolation', default='nearest', help='interpolation mode: nearest, bicubic [default: %default]')
    parser.add_option('--xlabel', help='x-axis label')
    parser.add_option('--ylabel', help='y-axis label')
    (options, args) = parser.parse_args()
    if options.list_cmaps:
    ...
    pylab.subplot(131)
    xyp1.plot(interpolation=options.interpolation, vmin=options.vmin, vmax=options.vmax, cmap=pylab.cm.get_cmap(options.cmap))
    pylab.title(file1)
    pylab.colorbar()
    if options.xlabel != None:
        pylab.xlabel(options.xlabel)
    if options.ylabel != None:
        pylab.ylabel(options.ylabel)
    pylab.subplot(132)
    xyp2.plot(interpolation=options.interpolation, vmin=options.vmin, vmax=options.vmax, cmap=pylab.cm.get_cmap(options.cmap))
    pylab.title(file2)
    pylab.colorbar()
    if options.xlabel != None:
        pylab.xlabel(options.xlabel)
    ...
    vmin = -vmax
    diff = Field(diff, xmin=xyp1.xmin, xmax=xyp1.xmax, zmin=xyp1.zmin, zmax=xyp1.zmax)
    diff.plot(interpolation=options.interpolation, vmin=vmin, vmax=vmax, cmap=pylab.cm.get_cmap(options.cmap))
