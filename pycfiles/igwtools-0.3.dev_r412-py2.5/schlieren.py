...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/igwtools/schlieren.py
...
def checkMinTol(i, j, img, mintol):
    ...
    if j == 0 or j == img.nz - 1:
        return False
    elif (img[(i, j - 1)] - img[(i, j)]) * (img[(i, j)] - img[(i, j + 1)]) < 0.0 or abs(img[(i, j - 1)] - img[(i, j)]) < mintol or abs(img[(i, j)] - img[(i, j + 1)]) < mintol:
        return False
    else:
        return True
...
def getDelZ(i, j, i0, img0, img):
    ...
    if j == 0:
        ...
        deltaZ = (zm - z0) * (img[(i, j)] - img0[(i0, j)]) * (img[(i, j)] - img0[(i0, j + 1)]) / ((img0[(i0, j - 1)] - img0[(i0, j)]) * (img0[(i0, j - 1)] - img0[(i0, j + 1)])) + (zp - z0) * (img[(i, j)] - img0[(i0, j)]) * (img[(i, j)] - img0[(i0, j - 1)]) / ((img0[(i0, j + 1)] - img0[(i0, j)]) * (img0[(i0, j + 1)] - img0[(i0, j - 1)]))
    return deltaZ
...
def schlieren_field(I, mode='qualitative', options=None):
    ...
    if I.nx == 1:
        ...
        data = tools.Field(data, xmin=options.xmin, xmax=options.xmax, zmin=options.zmin, zmax=options.zmax, tmin=options.tmin, tmax=options.tmax)
        ...
        if mode in ('qualitative', 'dz', 'dn2'):
            ...
            if I.nx == 1:
                for n in range(1, I.nt):
                    print n, 'of', I.nt - 1
                    I[n, :] = schlieren(I[n - 1, :] * 1.0, I[n, :], options)
...
def schlieren(reference_image=None, image=None, options=None):
    ...
    output1 = tools.Field(numpy.zeros_like(image), xmin=options.xmin, xmax=options.xmax, zmin=options.zmin, zmax=options.zmax, tmin=options.tmin, tmax=options.tmax)
    output2 = tools.Field(numpy.zeros_like(image), xmin=options.xmin, xmax=options.xmax, zmin=options.zmin, zmax=options.zmax, tmin=options.tmin, tmax=options.tmax)
    if options.verbose:
    ...
    for i in range(options.ndt, image.nt):
        for j in range(image.nz):
            if checkMinTol(i - options.ndt, j, image, options.mintol):
                output1[(i, j)] = getDelZ(i, j, i - options.ndt, image, image)
    ...
    if options.nofill:
        ...
        output2 = tools.Field(output2, xmin=options.xmin, xmax=options.xmax, zmin=options.zmin, zmax=options.zmax, tmin=options.tmin, tmax=options.tmax)
        MinReal = 1e-08
        if options.imagetype == 'xz':
            for i in range(image.nx):
                for j in range(image.nz):
    ...
    if options.mode == 'dn2' or options.mode == 'dn2t':
        intn2 = 0.5 * options.Lt ** 2 + options.Lt * options.nw * (options.Lp / options.np + options.Ls / options.na)
...
def schlieren_entry():
    ...
    parser.add_option('-d', action='store_true', dest='display', help='displays images for debugging purposes')
    parser.add_option('-v', '--verbose', action='store_true', dest='verbose', help='be verbose and output settings used')
    parser.add_option('-T', '--mintol', dest='mintol', type='float', default=10, help='Minimum intensity difference between pixels for computation [default: %default]')
    parser.add_option('--fast', action='store_true', default=False, help="use experimental 'fast' getdelz calculation")
    parser.add_option('-t', '--dt', type='float', help='Time difference between frames')
    parser.add_option('-n', '--ndt', type='int', default=2, help='Take time difference between how many pixels [default : %default]')
    parser.add_option('-s', '--sigma', dest='sigma', type='float', default=0.3, help='Distance over which to fill in uncomputed points [default: %default]')
    parser.add_option('-M', '--mode', default='dn2t', help='Schlieren mode (dz, dzt, dn2, dn2t) [default: %default]')
    parser.add_option('-o', '--outputfile', help='Name of output file (output saved only if supplied)')
    parser.add_option('-f', '--force', action='store_true', help='Force overwriting of anyexisting view')
    parser.add_option('--nofill', action='store_true', help='do not fill-in values that were not computed by interpolation by Gaussian average')
    parser.add_option('--nosmooth', action='store_true', help='do not smooth with uniform average filter')
    parser.add_option('--Ls', type='float', default=24.0, help='Distance from back of tank to screen [default: %default]')
    parser.add_option('--Lt', type='float', default=1.0, help='Width of tank [default: %default]')
    parser.add_option('--Lp', type='float', default=1.0, help='Width of tank walls [default: %default]')
    parser.add_option('--Lc', type='float', default=330.0, help='Distance from fron of tank to camera [default: %default]')
    ...
    parser.add_option('-e', '--experiment', default=default_experiment, help='Experiment name [default: %default]')
    parser.add_option('-L', '--load', help='load input from view')
    parser.add_option('-S', '--save', help='save results as view')
    ...
    if options.mode == 'dzt' or options.mode == 'dn2t':
        if options.imagetype == 'xz':
            if options.dt == None:
                parser.error("mode %s and 'xz' images requires --dt option" % options.mode)
        else:
            options.dt = options.ndt * image.dt
    if options.imagetype == 'xz':
        output = schlieren(reference_image=reference_image, image=image, options=options)
    else:
