...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/igwtools/test.py
# Compiled at: 2008-04-01 17:59:45
"""Tests for igwtools"""
import igwtools
...
class field_tests(TestCase):
    ...
    def testFrame(self):
        """A simple frame"""
        f = Field([[1, 2, 3, 4], [2, 3, 4, 5], [3, 4, 5, 6]], xmin=1, xmax=3, zmin=1, zmax=3)
        self.assert_(True)

    def testVts(self):
        """A simple vts"""
        f = Field([[1, 2, 3, 4], [2, 3, 4, 5], [3, 4, 5, 6]], tmin=1, tmax=3, zmin=1, zmax=3)
        self.assert_(True)

    def testHts(self):
        """A simple hts"""
        f = Field([[1, 2, 3, 4], [2, 3, 4, 5], [3, 4, 5, 6]], xmin=1, xmax=3, tmin=1, tmax=3)
        self.assert_(True)

    def testField(self):
        """xzt field"""
        import pylab
        f = Field(pylab.rand(6, 5, 10), xmin=1, xmax=6, zmin=1, zmax=5, tmin=1, tmax=10)
        self.assert_(True)
