...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/igwtools/grids.py
...
import gtk, sys, os, pylab, Image, numpy
...
class CoordsCollection:
 ...
 (0, 0)], world_coords=[(0, 0)]):
    ...
    def add_coord(self, pixel_coord, world_coord=None):
        if world_coord == None:
            if len(self.pixel_coords) >= 3:
                ...
                world_coord = (
                 round(world_coord[0]), round(world_coord[1]))
            else:
                world_coord = (0.0, 0.0)
                dlg = CoordsDialog(pixel_coord=pixel_coord, world_coord=world_coord)
                response = dlg.run()
    ...
    def del_coord(self, ind):
        self.pixel_coords = [ tup for (i, tup) in enumerate(self.pixel_coords) if i != ind
                            ]
        self.world_coords = [ tup for (i, tup) in enumerate(self.world_coords) if i != ind
                            ]
        self.compute_transform()
...
class CoordsInteractor:
    ...
    def __init__(self, ax, coords):
        ...
        self.line = Line2D(x, y, linestyle='', marker='D', markerfacecolor='r', animated=True)
        self._ind = None
    ...
    def key_press_callback(self, event):
        ...
        if event.key == 't':
            ...
            ind = self.get_ind_under_point(event)
            if ind is not None:
                dlg = CoordsDialog(pixel_coord=self.coords.pixel_coords[ind], world_coord=self.coords.world_coords[ind])
                response = dlg.run()
                if response == gtk.RESPONSE_OK:
...
def coords_editor(imagefilename, coords, **kwargs):
    ...
    ar = numpy.array(im)
    pylab.imshow(ar, aspect='auto', interpolation='nearest', **kwargs)
    pylab.axis('tight')
...
def grid_entry():
    ...
    parser.add_option('--worldgrid', help='name of world grid file')
    parser.add_option('-t', '--timeoffset', help='start time offset')
    ...
    if ext in ('.mov', '.avi', '.dv'):
        ...
        cmd = 'ffmpeg -ss %s ' % options.timeoffset + '-i %s -vcodec png ' % args[0] + '-vframes 1 -an -f rawvideo -y %s' % tempname
        os.system(cmd)
        imagefile = tempname
    else:
        tempname = None
    ...
    if options.worldgrid != None:
        try:
            ...
            coords = CoordsCollection(pixel_coords=pixel_coords, world_coords=world_coords)
        except IOError:
            pass

    coords_editor(imagefile, coords=coords, cmap=pylab.cm.get_cmap(options.cmap))
