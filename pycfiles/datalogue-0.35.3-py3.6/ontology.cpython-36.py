...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/datalogue/models/ontology.py
# Compiled at: 2020-05-13 11:17:34
# Size of source mod 2**32: 10039 bytes
...
class TrainingDataColumn:
    ...
    def __init__(self, datastore_name: str, path: List[str]=[], count: int=0):
        if count is not 0:
    ...
    def __eq__(self, other: 'TrainingDataColumn'):
        if isinstance(self, other.__class__):
            return self.datastore_name == other.datastore_name and self.path == other.path and self.count == other.count
        else:
            return False

    @staticmethod
    def _as_payload(training_data_column: 'TrainingDataColumn') -> Union[(DtlError, dict)]:
        payload = {'datastore_name':str(training_data_column.datastore_name), 
...
class OntologyNode:
    ...
    def __init__(self, name: str, description: Optional[str]=None, children: List['OntologyNode']=[], id: Optional[UUID]=None, total_training_count: int=0, training_data_columns: List[TrainingDataColumn]=[]):
        if not isinstance(name, str):
            ...
            if total_training_count is not 0:
                if not isinstance(total_training_count, int):
    ...
    def __eq__(self, other: 'OntologyNode'):
        if isinstance(self, other.__class__):
            return self.name == other.name and self.description == other.description and self.children == other.children and self.id == other.id and self.total_training_count == other.total_training_count and self.training_data_columns == other.training_data_columns
        else:
            return False
    ...
    def as_payload(ontology_node: 'OntologyNode') -> Union[(DtlError, dict)]:
        payload = {'name':ontology_node.name, 
...
class Ontology:
    ...
    def __init__(self, name: str, description: Optional[str]=None, tree: List[OntologyNode]=[], id: Optional[UUID]=None):
        if not isinstance(name, str):
    ...
    def __eq__(self, other: 'Ontology'):
        if isinstance(self, other.__class__):
            return self.name == other.name and self.description == other.description and self.tree == other.tree and self.id == other.id
        else:
            return False
    ...
    def _create_body_for_sharing(target_id: UUID, target_type: Scope, permission: OntologyPermission) -> Dict:
        if not isinstance(target_type, Scope):
            ...
            if target_type == Scope.User:
                ...
                if target_type == Scope.Group:
                    body['groups'] = {str(target_id): permission.value}
                else:
                    if target_type == Scope.Organization:
