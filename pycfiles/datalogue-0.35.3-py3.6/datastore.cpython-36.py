...
# Size of source mod 2**32: 43450 bytes
...
class Datastore:
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}(id: {self.id}, name: {self.name!r}, alias: {self.alias!r}, credential_id: {self.credential_id}, definition: {self.definition!r}, samples: {self.samples!r}, schema_paths: {self.schema_paths!r}, schema_labels: {self.schema_labels!r}, schema_nodes: {self.schema_nodes!r}, tags: {self.tags})"
...
class JdbcDatastoreDef(DatastoreDef):
    ...
    def __init__(self, root_table: Optional[str]=None, datastore_id: Optional[UUID]=None, view_query: Optional[str]=None, post_processing_query: Optional[str]=None, params: Optional[Dict[(str, str)]]=None):
        """
        ...
        if post_processing_query is not None:
            if root_table is None:
...
def _datastore_from_payload(json: dict) -> Union[(DtlError, Datastore)]:
    ...
    if name is None:
        ...
        if tags is None:
            tags = []
        return Datastore(name, definition, credential_id, alias, datastore_id, samples, schema_paths, schema_classes, schema_labels, schema_nodes, ads, tags)
