...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/datalogue/models/transformations/classify.py
...
class Classifier(object):
    ...
    def __repr__(self):
        return f"Classifier(default_class: {self.default_class_id} classification_methods: \n{self.classification_methods})"
    ...
    def _as_payload(self) -> dict:
        base = {'classificationMethods':list(map(lambda m: m._as_payload(), self.classification_methods)), 
...
class Classify(Transformation):
    ...
    def __init__(self, classifier_id: Union[(str, UUID)], fields_to_target: Optional[List[List[str]]]=None, add_class_fields=False, add_score_fields=False):
        """
    ...
    def __repr__(self):
        return f"Classify(classifier_id: {self.classifier_id}, fields_to_target: {self.fields_to_target}, add_class_fields: {self.add_class_fields}, add_score_fields: {self.add_score_fields})"
