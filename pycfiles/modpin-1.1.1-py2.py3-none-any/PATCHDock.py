...
# [GCC 8.4.0]
# Embedded file name: /home/boliva/sit_sbi/modppi/src/BioLib/Docking/PATCHDock.py
# Compiled at: 2018-11-29 11:31:12
import re, copy, BioLib.Structure.PDB as PDB
    ...
    def __parse(self, PACHDock_file):
        ...
        for decoy in PACHDock_fo:
            if '#' not in decoy and '||' in decoy:
                ...
                self.decoys.append(PATCHDecoy(num, score, pen, area, as12, ace, float(vectors[1]), float(vectors[2]), float(vectors[3]), float(vectors[4]), float(vectors[5]), float(vectors[6])))
