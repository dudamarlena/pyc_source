...
# [GCC 8.4.0]
# Embedded file name: /home/patri/MODPIN/modpin/./src/functions/functions.py
# Compiled at: 2020-04-28 10:16:58
import os, sys, re, collections, gzip, itertools, json, numpy, subprocess, pwd
...
class DSSP(object):
    ...
    def has_residue(self, chain, residue_num):
        self._check_parsing()
        if (
         chain, residue_num) in self._residues:
            return True
        return False
...
def parse_list_file(list_file, gz=False):
    ...
    if os.path.exists(list_file):
        fd = None
        if gz:
            fd = gzip.open(list_file, 'rt')
        else:
            fd = open(list_file, 'rt')
        for line in fd:
            yield line.strip('\n')
...
def get_residue_residue_contacts(chain_A, chain_B):
    ...
    for aminoacid_A in chain_A.aminoacids:
        for aminoacid_B in chain_B.aminoacids:
            ...
            if contact[(-1)] == -1 or contact[(-1)] > 5.5:
                continue
...
def smith_waterman(A, B, emboss_path):
    ...
    try:
        ...
        return (
         ('').join(alignment[0::2]), ('').join(alignment[1::2]), query_positions[0][0], query_positions[(-1)][(-1)], hit_positions[0][0], hit_positions[(-1)][(-1)])
    except:
...
def get_sequence_to_crystal_correlations(pdb, chain, gapped=True):
    ...
    return (
     sequence_to_crystal, crystal_to_sequence)
...
def add_hydrogens(config, path, inp, out, dummy_dir):
    ...
    if fileExist(inp):
        ...
        if relax == 'yes':
            ...
            os.system('%s -s %s -in:file:fullatom -nstruct 1  -packing:repack_only -relax:jump_move false >& relax.log' % (relax_exe, output_hbplus))
