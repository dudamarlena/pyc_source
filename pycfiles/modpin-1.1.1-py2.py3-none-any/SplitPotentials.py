...
# [GCC 8.4.0]
# Embedded file name: /home/boliva/sit_sbi/modppi/src/BioLib/Docking/SplitPotentials.py
# Compiled at: 2018-11-29 11:31:12
import os, sys, random, math, numpy
...
class SplitPotentialsData(object):
    ...
    def __init__(self, ori_ppResidues, ori_combined, ori_ppRE, ori_localppRE, ori_ppEnvironment, ori_ppDist, cof_ppResidues, cof_combined, cof_ppRE, cof_localppRE, cof_ppEnvironment, cof_ppDist, max_ppResidues, max_ppEnvironment, max_ppRE, max_combined, min_ppResidues, min_ppEnvironment, min_ppRE, min_combined, men_ppResidues, men_ppEnvironment, men_ppRE, men_combined):
        self.original = {'D-Score': ori_ppResidues, 'D-Energy': ori_combined, 'D-Eaa3Denv': ori_ppRE, 'D-Elocal': ori_localppRE, 
           'D-E3Denv': ori_ppEnvironment, 'D-E3D': ori_ppDist, 'C-Score': cof_ppResidues, 
           'C-Energy': cof_combined, 'C-Eaa3Denv': cof_ppRE, 'C-Elocal': cof_localppRE, 
           'C-E3Denv': cof_ppEnvironment, 'C-E3D': cof_ppDist, 'M-Score': men_ppResidues, 
           'M-Energy': men_combined, 'M-Eaa3Denv': men_ppRE, 'M-E3Denv': men_ppEnvironment, 
           'U-Score': max_ppResidues, 
           'U-Energy': max_combined, 'U-Eaa3Denv': max_ppRE, 'U-E3Denv': max_ppEnvironment, 
           'L-Score': min_ppResidues, 
           'L-Energy': min_combined, 'L-Eaa3Denv': min_ppRE, 'L-E3Denv': min_ppEnvironment}
        self.normalized = {'D-ZScore': 0, 'D-ZEnergy': 0, 'D-ZEaa3Denv': 0, 'D-ZElocal': 0, 'D-ZE3Denv': 0, 'C-ZScore': 0, 
           'C-ZEnergy': 0, 'C-ZEaa3Denv': 0, 'C-ZElocal': 0, 'C-ZE3Denv': 0, 'M-ZScore': 0, 
           'M-ZEnergy': 0, 'M-ZEaa3Denv': 0, 'M-ZE3Denv': 0, 'U-ZScore': 0, 
           'U-ZEnergy': 0, 'U-ZEaa3Denv': 0, 'U-ZE3Denv': 0, 'L-ZScore': 0, 
           'L-ZEnergy': 0, 'L-ZEaa3Denv': 0, 'L-ZE3Denv': 0}
        self.original_key = [
         'D-Score', 'D-Energy', 'D-Eaa3Denv', 'D-Elocal', 'D-E3Denv', 'D-E3D',
...
class SplitPotentialsMatrix(object):
    ...
    def __init__(self, sp_matrix='ppi3_cbR12.out', cutoff=12):
        ...
        for line in pp_fd:
            if line.startswith('PAIR-POTENTIALS OF RESIDUES AND ENVIRONMENTS'):
                tag = 4
            elif line.startswith('PAIR-POTENTIALS OF ENVIRONMENTS'):
                tag = 2
            elif line.startswith('LOCAL PAIR-POTENTIALS OF RESIDUES AND ENVIRONMENTS'):
                tag = 3
            elif line.startswith('PAIR-POTENTIALS OF RESIDUES'):
                tag = 1
            elif line.startswith('STANDARD PAIR-POTENTIALS OF DISTANCES'):
                tag = 5
    ...
    def calculateSplitPotentials(self, randoms=100):
        ori_ppResidues, cof_ppResidues, max_ppResidues, min_ppResidues, men_ppResidues = (0,
                                                                                          0,
                                                                                          0,
                                                                                          0,
                                                                                          0)
        ori_ppEnvironment, cof_ppEnvironment, max_ppEnvironment, min_ppEnvironment, men_ppEnvironment = (0,
                                                                                                         0,
                                                                                                         0,
                                                                                                         0,
                                                                                                         0)
        ...
        ori_combined, cof_combined, max_combined, min_combined, men_combined = (0,
                                                                                0,
                                                                                0,
                                                                                0,
                                                                                0)
        ...
        result = SplitPotentialsData(ori_ppResidues=ori_ppResidues, ori_combined=ori_combined, ori_ppRE=ori_ppRE, ori_localppRE=ori_localppRE, ori_ppEnvironment=ori_ppEnvironment, ori_ppDist=ori_ppDist, cof_ppResidues=cof_ppResidues, cof_combined=cof_combined, cof_ppRE=cof_ppRE, cof_localppRE=cof_localppRE, cof_ppEnvironment=cof_ppEnvironment, cof_ppDist=cof_ppDist, max_ppResidues=max_ppResidues, max_ppEnvironment=max_ppEnvironment, max_ppRE=max_ppRE, max_combined=max_combined, min_ppResidues=min_ppResidues, min_ppEnvironment=min_ppEnvironment, min_ppRE=min_ppRE, min_combined=min_combined, men_ppResidues=men_ppResidues, men_ppEnvironment=men_ppEnvironment, men_ppRE=men_ppRE, men_combined=men_combined)
        if randoms == 0:
            return result
        ...
        for rad in xrange(randoms):
            rad_str1 = self.randomise_interaface(list=self.str1_contact_pos, range=len(self.realInteraction.get_structure1().get_residues()))
            rad_str2 = self.randomise_interaface(list=self.str2_contact_pos, range=len(self.realInteraction.get_structure2().get_residues()))
    ...
    def get_ppResidues(self, k1, k2, distance):
        ...
        if self.ppResidues.has_key(k1) and self.ppResidues[k1].has_key(k2):
            ...
            if self.ppResidues.has_key(k2) and self.ppResidues[k2].has_key(k1):
    ...
    def get_ppEnvironment(self, k1, k2, distance):
        ...
        if self.ppEnvironment.has_key(k1) and self.ppEnvironment[k1].has_key(k2):
            ...
            if self.ppEnvironment.has_key(k2) and self.ppEnvironment[k2].has_key(k1):
    ...
    def get_ppRE(self, k1, k2, distance):
        ...
        if self.ppRE.has_key(k1) and self.ppRE[k1].has_key(k2):
            ...
            if self.ppRE.has_key(k2) and self.ppRE[k2].has_key(k1):
    ...
    def process_file_line(self, line, tag):
        ...
        if tag == 1 and line.startswith(' PAIR'):
            self.assign_multiple_distances(dic=self.ppResidues, keys=line.split()[1], values=line.split()[2:])
        if tag == 2 and line.startswith(' PAIR'):
            self.assign_multiple_distances(dic=self.ppEnvironment, keys=line.split()[1], values=line.split()[2:])
        if tag == 3 and line.startswith(' AA-ENV'):
            self.assign_single_distance(dic=self.localppRE, key=line.split()[1], value=line.split()[2])
        if tag == 4 and line.startswith(' PAIR'):
            self.assign_multiple_distances(dic=self.ppRE, keys=line.split()[1], values=line.split()[2:])
        if tag == 5 and line.startswith(' Distance'):
            self.assign_single_distance(dic=self.ppDist, key=line.split()[1], value=line.split()[2])
