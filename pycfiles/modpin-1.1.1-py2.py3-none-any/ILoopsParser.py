...
# [GCC 8.4.0]
# Embedded file name: /home/boliva/sit_sbi/modppi/src/BioLib/ILoops/ILoopsParser.py
...
class ILXMLParser(object):
    ...
    0:   +--Interaction......................<interaction/>...............................+ILXMLInteraction (Object)
            ...
            |   +--Random Forest Result......<RF_result/>.......................................|...+ILXMLInteractionRFResult (Sub-Object)
            ...
            |          +--Precision..........<inferred_precision/>..............................|.............+--ILXMLPrecision (Buffer)
            ...
            +--{Interaction Signatures}......<interaction_signatures/>                          +--{signatures}
                +--Interaction Signature.....<interaction_signature/>...............................+ILXMLInteractionSignature (Sub-Object)
    ...
    def reset_active_subObj(self):
        self.active_subObj = None
        return
    ...
    def add_protein(self, proteinObj, store=True):
        ...
        """
        if store is True:
            self.proteins.append(proteinObj)
        else:
            self.proteins.append(proteinObj.get_name())
    ...
    def add_interaction(self, interactionObj, store=True):
        ...
        """
        if store is True:
            self.interactions.append(interactionObj)
        else:
            self.interactions.append((interactionObj.get_i1name(), interactionObj.get_i2name()))
    ...
    def add_feature(self, code, feature_type):
        ...
        if code not in ref_set:
            ref_set.add(code)
            ref_list.append(code)
        return ref_list.index(code)

    def _set_kwds(self, kwds):
        self.kwds = kwds

    def set_last_output_level(self, level):
        self.last_output_level = level

    def set_skip_levels(self, level_no):
        self.skip_levels = level_no
    ...
    def get_active_subObj(self):
        return self.active_subObj

    def get_last_output_level(self):
        return self.last_output_level

    def get_skip_levels(self):
        return self.skip_levels

    def _start_protein(self):
        self.set_active_object(PROTEIN)

    def _start_interaction(self):
        self.set_active_object(INTERACTION)

    def _start_alignment(self, parentProteinID, ali_type, ali_iter):
        self.set_active_subObj(subObj_generator=ILXMLAlignment, parentID=parentProteinID, parentType=PROTEIN, ali_type=ali_type, iter_no=ali_iter)

    def _start_domain_mapping(self, parentProteinID):
        self.set_active_subObj(subObj_generator=ILXMLDomainMapping, parentID=parentProteinID, parentType=PROTEIN)

    def _start_loop_feature(self, parentProteinID):
        self.set_active_subObj(subObj_generator=ILXMLLoop, parentID=parentProteinID, parentType=PROTEIN)

    def _start_domain_feature(self, parentProteinID):
        self.set_active_subObj(subObj_generator=ILXMLDomain, parentID=parentProteinID, parentType=PROTEIN)

    def _start_RF_result(self, parentInteractionID):
        self.set_active_subObj(subObj_generator=ILXMLInteractionRFResult, parentID=parentInteractionID, parentType=INTERACTION)

    def _start_interaction_signature(self, parentInteractionID):
        self.set_active_subObj(subObj_generator=ILXMLInteractionSignature, parentID=parentInteractionID, parentType=INTERACTION)

    def get_proteinID(self, protein_code):
        try:
            return self.get_proteins().index(protein_code)
        except ValueError:
    ...
    def get_featureID(self, feature_code, feature_type=None):
        if feature_type is None:
            if feature_code in self._loops():
                feature_type = LOOP
            elif feature_code in self._domains():
                feature_type = DOMAIN
        if feature_type == LOOP:
            ref_list = self.get_loops()
        elif feature_type == DOMAIN:
    ...
    def signatureCode_2_singatureID(self, feature_codes_list):
        if eval(feature_codes_list)[0] in self._loops:
            feature_type = LOOP
        elif eval(feature_codes_list)[0] in self._domains:
            feature_type = DOMAIN
        else:
            print self._domains
            print self._loops
    ...
    def _check_output_error(self, object_instance_checkClass, object_instance_str, output_level, output_object=None):
        ...
        if output_object is not None and not isinstance(output_object, object_instance_checkClass):
            ...
            raise ValueError(err_msg)
        elif output_object is not None:
            report_object = output_object
        else:
            report_object = check_obj
        return report_object

    def _control_skip_output(self, output_level):
        if self.get_skip_levels() == output_level:
            self.set_skip_levels(output_level - 1)
            self.set_last_output_level(output_level)
            return True
        return False

    def _output_protein_info(self, report_level, protein_object=None, output_alignments=True, output_domain_mappings=True, output_protein_features=True, output_domain_assignations=True):
        if self._control_skip_output(output_level=0) is True:
            return
        report_object = self._check_output_error(object_instance_checkClass=ILXMLProtein, object_instance_str='ILXMLProtein', output_level=0, output_object=protein_object)
        yield self.custom_protein_output(report_object, **self.kwds)
        if report_level < 1:
            if output_alignments is True:
                for alignment in report_object.get_alignments():
                    yield self.custom_alignment_output(alignment, **self.kwds)

            if output_domain_mappings is True:
                for domain_mapping in report_object.get_mappings():
                    yield self.custom_domain_mapping_output(domain_mapping, **self.kwds)

            if output_protein_features is True:
                for domain in report_object.get_domains():
                    yield self.custom_feature_output(domain, DOMAIN, **self.kwds)

                for loop in report_object.get_loops():
                    yield self.custom_feature_output(loop, LOOP, **self.kwds)
                    for domain_assignation in loop.get_mappings():
    ...
    def _output_protein_alignment_info(self, report_level, alignment_object=None):
        if self._control_skip_output(output_level=1) is True:
            return
        report_object = self._check_output_error(object_instance_checkClass=ILXMLAlignment, object_instance_str='ILXMLAlignment', output_level=1, output_object=alignment_object)
    ...
    def _output_protein_domain_mapping_info(self, report_level, domain_mapping_object=None, retrieve_results=False):
        if self._control_skip_output(output_level=1) is True:
            return
        report_object = self._check_output_error(object_instance_checkClass=ILXMLDomainMapping, object_instance_str='ILXMLDomainMapping', output_level=1, output_object=domain_mapping_object)
        if self.get_last_output_level() < 1:
            yield self.custom_protein_output(self.get_active_object(), **self.kwds)
            self.set_skip_levels(0)
    ...
    def _output_protein_features_info(self, report_level, feature_object=None, retrieve_results=False, output_domain_assignations=True):
        if self._control_skip_output(output_level=1) is True:
            return
        report_object = self._check_output_error(object_instance_checkClass=ILXMLFeature, object_instance_str='ILXMLLoop or ILXMLDomain', output_level=1, output_object=feature_object)
        if self.get_last_output_level() < 1:
            yield self.custom_protein_output(self.get_active_object(), **self.kwds)
            self.set_skip_levels(0)
        if isinstance(report_object, ILXMLDomain):
            yield self.custom_domain_output(report_object, **self.kwds)
    ...
    def _output_feature_domain_assignation_info(self, report_level, domain_assignation_object, retrieve_results=False):
        if self._control_skip_output(output_level=2) is True:
            return
        if self.get_last_output_level() < 1:
            yield self.custom_protein_output(self.get_active_object(), **self.kwds)
            self.set_skip_levels(0)
        if self.get_last_output_level() < 2:
            if isinstance(self.get_active_subObj(), ILXMLLoop):
                yield self.custom_loop_output(self.get_active_subObj(), **self.kwds)
            elif isinstance(self.get_active_subOjb(), ILXMLDomain):
                yield self.custom_domain_ouptut(self.get_active_subObj(), **self.kwds)
    ...
    def _output_interaction_info(self, report_level, interaction_object=None, retrieve_results=False, output_interaction_signatures=True, output_RF_results=True, output_RF_precisions=True):
        if self._control_skip_output(output_level=0) is True:
            return
        report_object = self._check_output_error(object_instance_checkClass=ILXMLInteraction, object_instance_str='ILXMLInteraction', output_level=0, output_object=interaction_object)
            ...
            if output_RF_results is True:
                for RFResult in report_object.get_RFResults():
                    ...
                    if output_RF_precisions is True:
                        for precision in RFResult.get_precisions():
                            yield self.custom_RF_precision_output(precision, **self.kwds)
    ...
    def _output_RF_result_info(self, report_level, RF_result_object=None, retrieve_results=False, output_RF_precisions=True):
        if self._control_skip_output(output_level=1) is True:
            return
        report_object = self._check_output_error(object_instance_checkClass=ILXMLInteractionRFResult, object_instance_str='ILXMLInteractionRFResult', output_level=1, output_object=RF_result_object)
        if self.get_last_output_level() < 1:
        ...
        if report_level < 2:
            for precision in report_object.get_precisions():
                yield self.custom_RF_precision_output(precision, **self.kwds)
    ...
    def _output_interaction_signature_info(self, report_level, interaction_signature_object=None, retrieve_results=False):
        if self._control_skip_output(output_level=1) is True:
            return
        report_object = self._check_output_error(object_instance_checkClass=ILXMLInteractionSignature, object_instance_str='ILXMLInteractionSignature', output_level=1, output_object=interaction_signature_object)
        if self.get_last_output_level() < 1:
            yield self.custom_interaction_output(self.get_active_object(), **self.kwds)
            self.set_skip_levels(0)
    ...
    def custom_protein_output(self, protein_object, **kwds):
        return repr(protein_object)

    def custom_alignment_output(self, alignment_subObj, **kwds):
        return repr(alignment_subObj)

    def custom_domain_mapping_output(self, domain_mapping_subObj, **kwds):
        return repr(domain_mapping_subObj)

    def custom_feature_output(self, feature_subObj, feature_type, **kwds):
        return repr(feature_subObj)

    def custom_domain_assignation_output(self, domain_assign_buffer, **kwds):
        return repr(domain_assign_buffer)

    def custom_interaction_output(self, interaction_object, **kwds):
        return repr(interaction_object)

    def custom_interaction_signature_output(self, int_signature_subObj, signature_type, **kwds):
        return repr(int_signature_subObj)

    def custom_RF_score_output(self, RF_score_subObj, **kwds):
        return repr(RF_score_subObj)

    def custom_RF_precision_output(self, RF_precision_buffer, **kwds):
        return repr(RF_precision_buffer)

    def custom_domain_output(self, domain_subObj, **kwds):
        return self.custom_feature_output(domain_subObj, DOMAIN, **kwds)

    def custom_loop_output(self, loop_subObj, **kwds):
        return self.custom_feature_output(loop_subObj, LOOP, **kwds)

    def results_parser(self, xml_file, report_level=0, skip_alignment_type=[], output_proteins=True, output_alignments=True, output_domain_mappings=True, output_protein_features=True, output_domain_assignations=True, output_interactions=True, output_interaction_signatures=True, output_RF_results=True, output_RF_precisions=True, **kwds):
        """
        ...
        for event, element in context:
            if event == 'start':
                if element.tag == 'protein':
                    self._start_protein()
                elif element.tag == 'blastSearch':
                    current_ali_starter = self._start_alignment
                elif element.tag == 'domainMapping':
                    current_ali_starter = self._start_domain_mapping
                elif element.tag == 'align':
                    if current_ali_starter == self._start_alignment:
                        if (element.text or ALL) not in skip_alignment_type:
                            current_ali_starter(parentProteinID=self.get_active_object().get_ID(), ali_type=current_ali_type, ali_iter=current_ali_iter)
                    elif current_ali_starter == self._start_domain_mapping:
                        current_ali_starter(parentProteinID=self.get_active_object().get_ID())
                    ...
                    self._start_RF_result(self.get_active_object().get_ID())
                elif element.tag == 'interaction_signature':
                    self._start_interaction_signature(self.get_active_object().get_ID())
            if event == 'end':
                if element.tag == 'name':
                    self.get_active_object().set_name(element.text)
                elif element.tag == 'MD5':
                    self.get_active_object().set_MD5(element.text)
                elif element.tag == 'sequence':
                    self.get_active_object().set_sequence(element.text)
                elif element.tag == 'ID':
                    current_ali_type = element.text
                elif element.tag == 'search_iteration':
                    current_ali_iter = element.text
                elif element.tag == 'target_id' and (isinstance(self.get_active_subObj(), ILXMLAlignment) or isinstance(self.get_active_subObj(), ILXMLDomainMapping)):
                    self.get_active_subObj().set_targetID(element.text)
                elif element.tag == 'e-value' and (isinstance(self.get_active_subObj(), ILXMLAlignment) or isinstance(self.get_active_subObj(), ILXMLDomainMapping)):
                    self.get_active_subObj().set_eValue(element.text)
                elif element.tag == 'query_start' and (isinstance(self.get_active_subObj(), ILXMLAlignment) or isinstance(self.get_active_subObj(), ILXMLDomainMapping)):
                    self.get_active_subObj().set_QStart(element.text)
                elif element.tag == 'query_end' and (isinstance(self.get_active_subObj(), ILXMLAlignment) or isinstance(self.get_active_subObj(), ILXMLDomainMapping)):
                ...
                elif element.tag == 'hit_start' and (isinstance(self.get_active_subObj(), ILXMLAlignment) or isinstance(self.get_active_subObj(), ILXMLDomainMapping)):
                    self.get_active_subObj().set_HStart(element.text)
                elif element.tag == 'hit_end' and (isinstance(self.get_active_subObj(), ILXMLAlignment) or isinstance(self.get_active_subObj(), ILXMLDomainMapping)):
                    ...
                    if isinstance(self.get_active_object(), ILXMLProtein):
                        if isinstance(self.get_active_subObj(), ILXMLAlignment):
                            if (element.text or ALL) not in skip_alignment_type and store_alignments is True:
                            ...
                            if store_domain_mappings is True:
                                self.get_active_object().add_mapping(self.get_active_subObj())
                ...
                elif element.tag == 'blastSearch' or element.tag == 'domainMapping':
                    current_ali_starter = None
                elif isinstance(self.get_active_object(), ILXMLProtein) and element.tag == 'type':
                    if element.text == 'loop':
                        self._start_loop_feature(parentProteinID=self.get_active_object().get_ID())
                    elif element.text == 'domain':
                        self._start_domain_feature(parentProteinID=self.get_active_object().get_ID())
                elif element.tag == 'loop_subClass':
                    self.get_active_subObj().set_subClass(element.text)
                elif element.tag == 'SCOP_code':
                    self.get_active_subObj().set_SCOP_code(element.text)
                elif element.tag == 'loop_match':
                    self.get_active_subObj().set_loopMatch(element.text)
                elif element.tag == 'SCOP_match':
                    self.get_active_subObj().set_SCOP_match(element.text)
                elif element.tag == 'align_ini':
                    self.get_active_subObj().set_ali_ini(element.text)
                elif element.tag == 'align_end':
                    self.get_active_subObj().set_ali_end(element.text)
                elif element.tag == 'query_feature_ali_seq':
                    self.get_active_subObj().set_QAliSeq(element.text)
                elif element.tag == 'target_feature_ali_seq':
                    self.get_active_subObj().set_HAliSeq(element.text)
                elif element.tag == 'SCOP_id' or element.tag == 'domain_id':
                    current_assignation_id = element.text
                elif element.tag == 'SCOP_ini' or element.tag == 'domain_ini':
                    current_assignation_ini = element.text
                elif element.tag == 'SCOP_end' or element.tag == 'domain_end':
                    current_assignation_end = element.text
                elif element.tag == 'SCOP_assignation' or element.tag == 'domain_type':
                    if element.tag == 'SCOP_assignation':
                        current_assignation_type = 'SCOP'
                    elif element.tag == 'domain_type':
                        current_assignation_type = element.text
                    this_assignation = ILXMLDomainAssignation(current_assignation_type, current_assignation_id, current_assignation_ini, current_assignation_end)
                    if isinstance(self.get_active_subObj(), ILXMLFeature) and isinstance(self.get_active_object(), ILXMLProtein):
                        if store_domain_assignations is True:
                            self.get_active_subObj().add_mapping(this_assignation)
                        if output_domain_assignations is True and report_level >= 2:
                            for x in self._output_feature_domain_assignation_info(report_level=report_level, domain_assignation_object=this_assignation):
                                if x is not None:
                                    yield x
                    ...
                    if isinstance(self.get_active_subObj(), ILXMLFeature) and isinstance(self.get_active_object(), ILXMLProtein):
                        ...
                        if store_protein_features is True:
                            self.get_active_object().add_feature(self.get_active_subObj())
                        if output_protein_features is True and report_level >= 1:
                            for x in self._output_protein_features_info(report_level=report_level, output_domain_assignations=output_domain_assignations):
                                if x is not None:
                                    yield x
                    ...
                    if output_proteins is True and report_level >= 0:
                        for x in self._output_protein_info(report_level=report_level, output_alignments=output_alignments, output_domain_mappings=output_domain_mappings, output_protein_features=output_protein_features, output_domain_assignations=output_domain_assignations):
                            if x is not None:
                                yield x

                    self.reset_active_object()
                elif element.tag == 'P1ID':
                    self.get_active_object().set_interactor1_ID(self.get_proteinID(element.text))
                elif element.tag == 'P2ID':
                    self.get_active_object().set_interactor2_ID(self.get_proteinID(element.text))
                elif element.tag == 'Splus':
                    self.get_active_object().set_Splus(element.text)
                elif element.tag == 'Sminus':
                    self.get_active_object().set_Sminus(element.text)
                elif element.tag == 'LSR':
                    self.get_active_object().set_LSR(element.text)
                elif element.tag == 'LpVR':
                    self.get_active_object().set_LpVR(element.text)
                elif element.tag == 'RF_cost':
                    self.get_active_subObj().set_cost(element.text)
                elif element.tag == 'RF_prediction' and element.text == 'YES':
                    self.get_active_subObj().set_prediction(True)
                elif element.tag == 'RF_prediction' and element.text == 'NO':
                    self.get_active_subObj().set_prediction(False)
                elif element.tag == 'RF_score':
                    self.get_active_subObj().set_RFscore(element.text)
                elif element.tag == 'unbalance_ratio':
                    current_UR = element.text
                elif element.tag == 'precision':
                    current_precision = element.text
                elif element.tag == 'error':
                    current_error = element.text
                    ...
                    if isinstance(self.get_active_object(), ILXMLInteraction):
                        ...
                        if store_RF_precisions is True:
                            self.get_active_subObj().add_precision(this_precision)
                        if output_RF_precisions is True and report_level >= 2:
                            for x in self._output_RF_precision_info(report_level=report_level, RF_precision_object=this_precision):
                                if x is not None:
                                    yield x
                    ...
                    if isinstance(self.get_active_object(), ILXMLInteraction) and isinstance(self.get_active_subObj(), ILXMLInteractionRFResult):
                        if store_RF_results is True:
                            self.get_active_object().add_RFResult(self.get_active_subObj())
                        if output_RF_results is True and report_level >= 1:
                            for x in self._output_RF_result_info(report_level=report_level, output_RF_precisions=output_RF_precisions):
                                if x is not None:
                                    yield x

                    self.reset_active_subObj()
                elif element.tag == 'IS_type':
                    self.get_active_subObj().set_refMatrix(element.text)
                elif element.tag == 'pValue':
                    self.get_active_subObj().set_pValue(element.text)
                ...
                elif element.tag == 'P2IS':
                    self.get_active_subObj().set_P2SignID(tuple(self.signatureCode_2_singatureID(element.text)[1]))
                elif element.tag == 'interaction_signature':
                    if isinstance(self.get_active_object(), ILXMLInteraction) and isinstance(self.get_active_subObj(), ILXMLInteractionSignature):
                        if store_interaction_signatures is True:
                            self.get_active_object().add_signature(self.get_active_subObj())
                    ...
                    if output_interactions is True and report_level >= 0:
                        for x in self._output_interaction_info(report_level=report_level, output_interaction_signatures=output_interaction_signatures, output_RF_results=output_RF_results, output_RF_precisions=output_RF_precisions):
                            if x is not None:
                                yield x
        ...
        if report_level == -1:
            if output_proteins is True:
                for protein in self.get_proteins():
                    self._output_protein_info(report_level=report_level, protein_object=protein, output_alignments=output_alignments, output_domain_mappings=output_domain_mappings, output_protein_features=output_protein_features, output_domain_assignations=output_domain_assignations)

            if output_interactions is True:
                for interaction in self.get_interactions():
                    self._output_interaction_info(report_level=report_level, interaction_object=interaction, output_interaction_signatures=output_interaction_signatures, output_RF_results=output_RF_results, output_RF_precisions=output_RF_precisions)
...
class ILXMLFeature(ILXMLSubObj):
    ...
    def set_ali_ini(self, ali_ini):
        self.ali_ini = ali_ini

    def set_ali_end(self, ali_end):
        self.ali_end = ali_end

    def set_QAliSeq(self, QAliSeq):
        self.QAliSeq = QAliSeq

    def set_HAliSeq(self, HAliSeq):
        self.HAliSeq = HAliSeq

    def get_ali_ini(self):
        return self.ali_ini

    def get_ali_end(self):
        return self.ali_end

    def get_QAliSeq(self):
        return self.QAliSeq

    def get_HAliSeq(self):
        return self.HAliSeq
...
class ILXMLProtein(ILXMLObject):
    ...
    def add_alignment(self, alignmentSubObj):
        if self.get_appendAlignments() is True:
            self.alignments.append(alignmentSubObj)

    def add_mapping(self, mappingSubObj):
        if self.get_appendMappings() is True:
            self.mappings.append(mappingSubObj)

    def add_feature(self, featureSubObj):
        if self.get_appendFeatures() is True:
            self.features.append(featureSubObj)
    ...
    def get_appendAlignments(self):
        return self.appendAlignments

    def get_appendMappings(self):
        return self.appendMappings

    def get_appendFeatures(self):
        return self.appendFeatures
    ...
    def get_loops(self):
        return [ x for x in self.get_features() if isinstance(x, ILXMLLoop) is True ]

    def get_domains(self):
        return [ x for x in self.get_features() if isinstance(x, ILXMLDomain) is True ]
...
class ILXMLAlignment(ILXMLSubObj):
    ...
    def __init__(self, ILXMLParser, parentID, parentType, ali_type, iter_no, target_id=None, eValue=None, q_start=None, q_end=None, q_seq=None, h_start=None, h_end=None, h_seq=None):
        super(ILXMLAlignment, self).__init__(ILXMLParser=ILXMLParser, ID=None, Type=ALIGNMENT, parentID=parentID, parentType=parentType)
    ...
    def set_QSeq(self, QSeq):
        self.q_seq = QSeq

    def set_HStart(self, HStart):
        self.h_start = HStart

    def set_HEnd(self, HEnd):
        self.h_end = HEnd

    def set_HSeq(self, HSeq):
        self.h_seq = HSeq

    def get_aliType(self):
        return self.ali_type
...
class ILXMLDomainMapping(ILXMLSubObj):
    ...
    def __init__(self, ILXMLParser, parentID, parentType, targetID=None, eValue=None, q_start=None, q_end=None, h_start=None, h_end=None):
        super(ILXMLDomainMapping, self).__init__(ILXMLParser=ILXMLParser, ID=None, Type=MAPPING, parentID=parentID, parentType=parentType)
        self.targetID = targetID
        self.eValue = eValue
        self.q_start = q_start
        self.q_end = q_end
...
class ILXMLLoop(ILXMLFeature):
    ...
    def __init__(self, ILXMLParser, parentID, parentType, subClass=None, loopMatch=None, ali_ini=None, ali_end=None, QAliSeq=None, HAliSeq=None):
        super(ILXMLLoop, self).__init__(ILXMLParser=ILXMLParser, ID=None, Type=LOOP, parentID=parentID, parentType=parentType)
    ...
    def set_loopMatch(self, loopMatch):
        self.loopMatch = loopMatch

    def add_mapping(self, mappingObject):
        self.mappings.append(mappingObject)
    ...
    def get_loopMatch(self):
        return self.loopMatch
...
class ILXMLDomain(ILXMLFeature):
    ...
    def __init__(self, ILXMLParser, parentID, parentType, SCOP_code=None, SCOP_match=None, ali_ini=None, ali_end=None, QAliSeq=None, HAliSeq=None):
        super(ILXMLDomain, self).__init__(ILXMLParser=ILXMLParser, ID=None, Type=DOMAIN, parentID=parentID, parentType=parentType)
    ...
    def set_SCOP_code(self, SCOP_code):
        self.SCOP_code = SCOP_code

    def set_SCOP_match(self, SCOP_match):
        self.SCOP_match = SCOP_match

    def get_code(self):
        return self.get_SCOP_match()

    def get_SCOP_code(self):
        return self.SCOP_code

    def get_SCOP_match(self):
        return self.SCOP_match
...
class ILXMLProteinSignature(ILXMLSubObj):
    ...
    def __init__(self, ILXMLParser, parentID, parentType, ID, signature_type, participantsIDs):
        ...
        """
        super(ILXMLProteinSignature, self).__init__(ILXMLParser=ILXMLParser, ID=ID, Type=SIGNATURE, parentID=parentID, parentType=parentType)
        self.signature_type = signature_type
        raise NotImplementedError('Not Implemented yet!\n')

    def get_signatureType(self):
...
class ILXMLInteraction(ILXMLObject):
    ...
    def set_interactor1_ID(self, i1ID):
        self.interactor1_ID = i1ID

    def set_interactor2_ID(self, i2ID):
        self.interactor2_ID = i2ID

    def set_Splus(self, Splus):
        self.Splus = Splus

    def set_Sminus(self, Sminus):
        self.Sminus = Sminus

    def set_LpVR(self, LpVR):
        self.LpVR = LpVR

    def set_LSR(self, LSR):
        self.LSR = LSR

    def add_RFResult(self, RFResultObject):
        self.get_RFResults().append(RFResultObject)

    def add_signature(self, IntSignObject):
        self.get_signatures().append(IntSignObject)

    def reset_RFResults(self):
        self.RFResults = []

    def reset_signatures(self):
        self.signatures = []

    def get_interactor1_ID(self):
        return self.interactor1_ID

    def get_interactor2_ID(self):
        return self.interactor2_ID
    ...
    def get_RFResults(self):
        return self.RFResults
    ...
    def get_i1name(self):
        ...
        if isinstance(i1_protein, ILXMLProtein):
            return i1_protein.get_name()
        else:
            return i1_protein

    def get_i2name(self):
        i2_protein = self.get_parser().get_proteins()[self.get_interactor2_ID()]
        if isinstance(i2_protein, ILXMLProtein):
            return i2_protein.get_name()
        else:
            return i2_protein

    def get_name(self):
        return repr((self.get_i1name(), self.get_i2name()))

    def get_positive_signatures(self):
        return [ x for x in self.get_signatures() if x.get_refMatrix() == POSITIVE ]

    def get_negative_signatures(self):
        return [ x for x in self.get_signatures() if x.get_refMatrix() == NEGATIVE ]
...
class ILXMLInteractionRFResult(ILXMLSubObj):
    ...
    def __init__(self, ILXMLParser, parentID, parentType, cost=None, prediction=None, RF_score=None):
        ...
        """
        super(ILXMLInteractionRFResult, self).__init__(ILXMLParser=ILXMLParser, ID=None, Type=RFRESULT, parentID=parentID, parentType=parentType)
        self.cost = cost
    ...
    def set_RFscore(self, RFscore):
        self.RFscore = RFscore

    def add_precision(self, precisionObject):
        self.get_precisions().append(precisionObject)
    ...
    def __repr__(self):
        if self.get_prediction() is False:
            prediction = 'NO'
        elif self.get_prediction() is True:
            prediction = 'YES'
...
class ILXMLInteractionSignature(ILXMLSubObj):
    ...
    def __init__(self, ILXMLParser, parentID, parentType, refMatrix=None, elementsType=None, P1SignID=None, P2SignID=None, pValue=None):
        super(ILXMLInteractionSignature, self).__init__(ILXMLParser=ILXMLParser, ID=None, Type=SIGNATURE, parentID=parentID, parentType=parentType)
    ...
    def set_refMatrix(self, refMatrix):
        self.refMatrix = refMatrix

    def set_elementsType(self, elementsType):
        self.elementsType = elementsType

    def set_P1SignID(self, P1SignID):
        self.P1SignID = P1SignID

    def set_P2SignID(self, P2SignID):
        self.P2SignID = P2SignID

    def set_pValue(self, pValue):
        self.pValue = pValue

    def get_refMatrix(self):
        return self.refMatrix

    def get_elementsType(self):
        return self.elementsType

    def get_P1SignID(self):
        return self.P1SignID

    def get_P2SignID(self):
        return self.P2SignID
...
class IloopsParser(ILXMLParser):
    ...
    def custom_feature_output(self, feature_subObj, feature_type, **kwds):
        return feature_subObj

    def custom_interaction_signature_output(self, int_signature_subObj, signature_type, **kwds):
        return int_signature_subObj

    def set_xml_file(self, xml_file):
        self.xml_file = xml_file

    def get_protein_loops(self, protein_name, warnings=False, **kwds):
        """
        Get a list of the loops for a certain protein
        """
        loops = []
        for protein in self.results_parser(xml_file=self.xml_file, report_level=0, **kwds):
            if isinstance(protein, ILXMLProtein) and protein.get_name() == protein_name:
                for feature in protein.get_features():
                    loop_subClass = feature.get_subClass()
                    align_ini = int(feature.get_ali_ini())
                    align_end = int(feature.get_ali_end())
    ...
    def get_positive_signatures(self, protein1, protein2, max_protein_signatures=3, warnings=False, **kwds):
        if max_protein_signatures > 3:
            max_protein_signatures = 3
        if max_protein_signatures < 1:
            max_protein_signatures = 1
        positive_signatures = []
        for interaction in self.results_parser(xml_file=self.xml_file, report_level=0, **kwds):
            if isinstance(interaction, ILXMLInteraction) and interaction.get_i1name() == protein1 and interaction.get_i2name() == protein2:
                for signature in interaction.get_positive_signatures():
                    P1IS = list(signature.get_parser().signatureID_2_signatureCode(signature.get_elementsType(), signature.get_P1SignID()))
                    P2IS = list(signature.get_parser().signatureID_2_signatureCode(signature.get_elementsType(), signature.get_P2SignID()))
                    pValue = float(signature.get_pValue())
    ...
    def get_negative_signatures(self, protein1, protein2, max_protein_signatures=3, warnings=False, **kwds):
        if max_protein_signatures > 3:
            max_protein_signatures = 3
        if max_protein_signatures < 1:
            max_protein_signatures = 1
        ...
        for interaction in self.results_parser(xml_file=self.xml_file, report_level=0, **kwds):
            if isinstance(interaction, ILXMLInteraction) and interaction.get_i1name() == protein1 and interaction.get_i2name() == protein2:
                for signature in interaction.get_negative_signatures():
                    P1IS = list(signature.get_parser().signatureID_2_signatureCode(signature.get_elementsType(), signature.get_P1SignID()))
                    P2IS = list(signature.get_parser().signatureID_2_signatureCode(signature.get_elementsType(), signature.get_P2SignID()))
                    pValue = float(signature.get_pValue())
                    if len(P1IS) <= max_protein_signatures and len(P2IS) <= max_protein_signatures:
...
if __name__ == '__main__':
    import time
    parser = IloopsParser()
    parser.set_xml_file('homodimer.iLoops.59.xml')
    start = time.time()
    loops = parser.get_protein_loops('1tef_A')
    end = time.time()
    for i in loops:
        print repr(i)
