...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/schemaconvertor/tests/test_convertor_0_2.py
# Compiled at: 2015-04-01 21:58:56
from unittest import TestCase
from collections import namedtuple
from schemaconvertor.convertor import Schema, SchemaConst, ObjAsDictAdapter, SchemaConvertor
...
class TestObjAsDictAdapter(TestCase):
    ...
    def test_dict_interface(self):
        ...
        self.assertEqual(parent_dict.get('cinsval', child_dict['cinsval']), child_dict.get('cinsval'))
...
class TestSchema(TestCase):
    ...
    def test_field_default_value(self):
        ...
        self.assertEqual(schema.compiled, False)
    ...
    def test_short(self):
        schema1 = Schema({'version': '0.2', 
           'type': 'array', 
           'items': 'string'})
        schema2 = Schema({'version': '0.2', 
           'type': 'array', 
           'items': {'type': 'string'}})
...
class TestSchemaConvertor(TestCase):
    ...
    def test_string_encoding(self):
        convertor = SchemaConvertor({'type': 'string'})
        self.assertEqual(convertor('刘奕聪'), '刘奕聪')
        self.assertEqual(convertor(('刘奕聪').encode('utf-8')), '刘奕聪')
        convertor = SchemaConvertor({'encoding': 'utf-8', 
           'type': 'string'})
        self.assertEqual(convertor('刘奕聪'), '刘奕聪')
        self.assertEqual(convertor(('刘奕聪').encode('utf-8')), '刘奕聪')
        convertor = SchemaConvertor({'encoding': None, 
           'type': 'string'})
        self.assertEqual(convertor('刘奕聪'), '刘奕聪')
        self.assertEqual(convertor(('刘奕聪').encode('utf-8')), ('刘奕聪').encode('utf-8'))
        self.assertEqual(convertor(None), None)
        convertor = SchemaConvertor({'encoding': 'gbk', 
           'type': 'string'})
        self.assertEqual(convertor('刘奕聪'), '刘奕聪')
        self.assertEqual(convertor(('刘奕聪').encode('gbk')), '刘奕聪')
        return
    ...
    def test_string_decoderr(self):
        ...
        convertor = SchemaConvertor({'encoding': 'utf-8', 
           'decoderrors': 'ignore', 
           'type': 'string'})
        self.assertIsInstance(convertor(('刘奕聪').encode('gbk')), unicode)
        self.assertNotEqual(convertor(('刘奕聪').encode('gbk')), '刘奕聪')
        convertor = SchemaConvertor({'encoding': 'utf-8', 
           'decoderrors': 'replace', 
           'type': 'string'})
        self.assertIsInstance(convertor(('刘奕聪').encode('gbk')), unicode)
    ...
    def test_number(self):
        convertor = SchemaConvertor({'type': 'number'})
    ...
    def test_dict(self):
        convertor = SchemaConvertor({'type': 'dict', 
           'properties': {'key': 'string', 
                          'value': 'string'}})
        data = {'key': 'test', 
           'value': 1}
        self.assertDictEqual(convertor(data), {'key': 'test', 
           'value': '1'})
        convertor = SchemaConvertor({'type': 'dict', 
           'patternProperties': {'[a-z]': 'string', 
                                 '[0-9]': 'number'}})
        data = {'a': 0, 
           '1': '2', 
           'b': '3.4', 
           '5': 5.6}
        self.assertDictEqual(convertor(data), {'a': '0', 
           '1': 2, 
           'b': '3.4', 
           '5': 5.6})

    def test_object(self):
        convertor = SchemaConvertor({'type': 'object', 
           'properties': {'key': 'string', 
                          'value': 'string'}})
        data = Pair('test', 1)
        self.assertDictEqual(convertor(data), {'key': 'test', 
    ...
    def test_array(self):
        convertor = SchemaConvertor({'type': 'array', 
           'items': 'string'})
        data = range(3)
        self.assertListEqual(convertor(data), ['0', '1', '2'])

    def test_null(self):
        convertor = SchemaConvertor({'type': 'null'})
    ...
    def test_raw(self):
        convertor = SchemaConvertor({'type': 'raw'})
        data = [
         None, 0, 1.2, '3']
    ...
    def test_typeof(self):
        convertor = SchemaConvertor({'typeOf': {int: 'boolean', 
                      None: 'string', 
                      (float, str): 'integer', 
                      Pair: {'type': 'object', 
                             'properties': {'key': 'string', 
        ...
        self.assertEqual(convertor(None), 'None')
        self.assertEqual(convertor(Pair('test', 1)), {'key': 'test', 
           'value': '1'})
        self.assertEqual(convertor(True), True)
