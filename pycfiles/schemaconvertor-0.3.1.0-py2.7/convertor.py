...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/schemaconvertor/convertor.py
# Compiled at: 2015-04-06 22:45:29
...
class Schema(object):
    ...
    def __init__(self, schema, parent=None):
        ...
        self.parent = parent
        self.version = schema.get(SchemaConst.F_VERSION, parent.version if parent else self.VERSION)
        self.description = str(schema.get(SchemaConst.F_DESCRIPTION, parent.description if parent else self.__class__))
        self.compiled = False
    ...
    def compile(self):
        ...
        if self.compiled:
            ...
            self.properties_schemas = SchemaConst.S_DISABLED if properties is None else {k:self.subschema(s) for k, s in properties.iteritems()}
            typeof_schemas = schema.get(SchemaConst.F_TYPEOF)
            self.typeof_schemas = SchemaConst.S_DISABLED if typeof_schemas is None else {(types.NoneType if t is None else t):self.subschema(s) for t, s in typeof_schemas.iteritems() if isinstance(t, (type, tuple, types.NoneType))}
            self.typeof_default_schema = SchemaConst.S_DISABLED if typeof_schemas is None else self.subschema(typeof_schemas.get(SchemaConst.F_DEFAULT, SchemaConst.T_DEFAULT))
            p_schemas = schema.get(SchemaConst.F_PATTERNPROPERTIES)
            self.pattern_properties_schemas = SchemaConst.S_DISABLED if p_schemas is None else {re.compile(p):self.subschema(s) for p, s in p_schemas.iteritems()}
            self.encoding = schema.get(SchemaConst.F_ENCODING, self.parent.encoding if self.parent else SchemaConst.V_ENCODING)
            self.decoderrors = schema.get(SchemaConst.F_DECODERR, self.parent.decoderrors if self.parent else SchemaConst.V_DECODERR)
            self.compiled = True
            return
    ...
    def properties(self, name, istry=False):
        ...
        if not istry:
            raise FieldMissError('field %s is miss in %s' % (name, SchemaConst.F_PROPERTIES))
    ...
    def pattern_properties(self, name, istry=False):
        ...
        if self.pattern_properties_schemas is SchemaConst.S_DISABLED:
            if istry:
                return SchemaConst.S_UNDEFINED
            raise FieldMissError('field %s is miss' % SchemaConst.F_PATTERNPROPERTIES)
...
class SchemaConvertor(object):
    ...
    CONVERTORS = {SchemaConst.T_STR: _str_convertor, 
