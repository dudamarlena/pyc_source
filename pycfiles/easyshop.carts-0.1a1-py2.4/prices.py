...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/easyshop/carts/adapters/prices.py
...
class CartItemPrices:
    ...
    def getPriceForCustomer(self, with_discount=False):
        ...
        for selected_property in self.context.getProperties():
            price += pm.getPriceForCustomer(selected_property['id'], selected_property['selected_option'])
    ...
    def getPriceGross(self, with_discount=False):
        ...
        pm = IPropertyManagement(product)
        for selected_property in self.context.getProperties():
            price += pm.getPriceGross(selected_property['id'], selected_property['selected_option'])
    ...
    def getPriceNet(self, with_discount=False):
        ...
        pm = IPropertyManagement(product)
        for selected_property in self.context.getProperties():
            price += pm.getPriceNet(selected_property['id'], selected_property['selected_option'])
