...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/easyshop/carts/viewlets/cart.py
# Compiled at: 2008-09-03 11:14:21
...
class CheckoutCartViewlet(ViewletBase):
    ...
    def getCartItems(self):
        ...
        for cart_item in IItemManagement(cart).getItems():
            ...
            result.append({'id': cart_item.getId(), 'product_title': title, 'product_url': product.absolute_url(), 'product_price': product_price, 'price': cm.priceToString(price), 'amount': amount, 'properties': self._getProperties(cart_item), 'total_price': cm.priceToString(total_price), 'discount': discount})
            total_amount += amount

        return {'cart_items': result, 'total_amount': total_amount}
    ...
    def getCountries(self):
        ...
        for country in shop.getCountries():
            result.append({'title': country, 'selected': safe_unicode(country) == customer.selected_country})
    ...
    def getPaymentMethodTypes(self):
        ...
        for payment_method in pm.getPaymentMethods(check_validity=True):
            ...
            result.append({'id': payment_method.getId(), 'title': payment_method.Title(), 'selected': selected})
    ...
    def getPaymentInfo(self):
        ...
        if selected_payment_method is None:
            ...
            return {'price': price, 'title': selected_payment_method.Title(), 'display': len(self.getCartItems()) > 0}
        return
    ...
    def getShippingInfo(self):
        ...
        if method is None:
            ...
            return {'price': price, 'title': method.Title(), 'description': method.Description(), 'display': len(self.getCartItems()) > 0}
        return
    ...
    def refreshCart(self):
        ...
        customer.selected_payment_method = safe_unicode(self.request.get('selected_payment_method'))
        cart = self._getCart()
        if cart is None:
            url = '%s/cart' % self.context.absolute_url()
            self.context.request.response.redirect(url)
        ...
        for (key, value) in self.context.request.items():
            if key.startswith('property_'):
                ...
                selected_properties[cart_item_id].append({'id': property_id, 'selected_option': value})
        ...
        for cart_item in im.getItems():
            ...
            elif selected_properties.has_key(cart_item.getId()):
                cart_item.setProperties(selected_properties[cart_item.getId()])
