...
# Embedded file name: /Users/maxk/Projects/OpenSource/arango-python/arango/tests/tests_document_integration.py
...
class TestsDocument(TestsIntegration):
    ...
    def test_document_deletion(self):
        ...
        assert_equal(c.collection.test.documents.count, count - 1)
        return
    ...
    def test_document_update(self):
        ...
        assert_equal(c.test.documents().first.body['1'], 1)
        assert_equal(c.test.documents().first.body['2'], 2)
        c.test.documents.delete(doc)
        doc1 = c.test.documents.create({'name': 'John'})
        c.test.documents.update(doc1.id, {'position': 'manager'})
        assert_equal(dict([(key, val) for key, val in c.test.documents().first.body.items() if key in ('name',
                                                                                                       'position')]), {'name': 'John', 
         'position': 'manager'})
    ...
    def test_document_body_setter(self):
        ...
        doc.save()
        assert_not_equal(doc, None)
        inter = list(set(c.test.documents().first.body).intersection(set(data)))
        assert_equal(data[inter[0]], c.test.documents().first.body[inter[0]])
        return
    ...
    def test_bulk_insert(self):
        ...
        assert_equal(response, {'created': 3,  'errors': 0,  'empty': 0,  'error': False})
