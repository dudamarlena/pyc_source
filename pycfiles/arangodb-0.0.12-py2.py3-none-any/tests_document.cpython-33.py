...
# Embedded file name: /Users/maxk/Projects/OpenSource/arango-python/arango/tests/tests_document.py
# Compiled at: 2013-09-26 14:12:21
# Size of source mod 2**32: 8047 bytes
from .tests_base import TestsBase
from nose.tools import assert_equal, raises, assert_false, assert_not_equal, assert_true
from arango.document import Document, Documents
from arango.utils import json
from arango.exceptions import DocumentAlreadyCreated, DocumentIncompatibleDataType, DocumentNotFound
__all__ = ('TestDocument', 'TestDocumentBase')

class TestDocumentBase(TestsBase):

    def delete_response_mock(self):
        return self.response_mock(status_code=202, text=json.dumps(dict(_rev='30967598', _id='1/30967598', error=False, code=204)), method='delete')

    def notfound_response_mock(self):
        return self.response_mock(status_code=404, text=json.dumps(dict(error=True, code=404)), method='get')

    def create_response_mock(self, _id=None, body=None):
        body = body if body is not None else {}
        defaults = dict(_rev='30967598', _id='1/30967598', error=False, code=201)
        defaults.update(body)
        patcher = self.response_mock(status_code=201, text=json.dumps(defaults), method='post')
        return patcher
...
class TestDocument(TestDocumentBase):
    ...
    def test_get_document_fields(self):
        body = {'array': [
         ...
         'tree': {'sample1': '1', 
                  'sample2': '2'}}
        doc = self.create_document(body)
        assert_equal(doc.get('array', default=None), [
         1, 2, 3])
        assert_equal(doc.get(), body)
        assert_equal(doc['tree']['sample1'], body['tree']['sample1'])
        return
    ...
    def test_document_update_complex(self):
        doc = self.create_document({'value': {'level1': {'level2': [
                                         1, 2, 3]}}})
        doc.body.get('value').get('level1')['level2'] = [
         3, 4, 5]
        assert_equal(doc.body.get('value').get('level1').get('level2'), [
         3, 4, 5])
    ...
    def test_document_not_found(self):
        ...
        patcher.start()
        doc = Document(collection=self.c, id='123', connection=self.conn)
        doc.body
        patcher.stop()

    def test_save(self):
        doc = self.create_document({})
        patcher = self.response_mock(status_code=201, text=json.dumps(dict(_rev=30967599, _id=1, error=False, code=201)), method='put')
        patcher.start()
        test_data = {'name': 'sample'}
    ...
    def test_delete_notfound(self):
        doc = self.create_document({})
        patcher = self.response_mock(status_code=404, text=json.dumps(dict(error=True, code=404)), method='delete')
        patcher.start()
        assert_equal(doc.delete(), False)
        patcher.stop()
    ...
    def test_repr(self):
        doc = self.create_document({})
        assert_equal(repr(doc), '<ArangoDB Document: Reference {0}, Rev: {1}>'.format(doc._id, doc._rev))