...
# Embedded file name: /Users/maxk/Projects/OpenSource/arango-python/arango/tests/tests_collection.py
# Compiled at: 2013-12-14 12:02:52
...
from arango.exceptions import CollectionIdAlreadyExist, InvalidCollectionId, InvalidCollection

class TestCollectionProxy(TestsBase):

    def test_proxy(self):
        assert_equal(self.conn.collection.__class__, Collections)
        assert_equal(self.conn.collection.test.__class__, Collection)
        assert_equal(self.conn.collection.test.cid, Collection(connection=self.conn, name='test').cid)

    def test_database_property(self):
        assert_equal(self.conn.collection.database.__class__, Database)

    def test_repr(self):
        assert_equal(str(self.conn.collection), '<Collections proxy for {0}>'.format(self.conn))
...
class TestCollection(TestsBase):
    ...
    def test_create(self):
        self.c.create()
        test_data = {'name': 'test', 
         'waitForSync': False,  'type': Collections.COLLECTION_DOCUMENTS}
    ...
    def test_create_edges(self):
        ...
        test_data = {'name': 'test_edges', 
         'waitForSync': False,  'type': Collections.COLLECTION_EDGES}
        test_args = {'data': json.dumps(test_data)}
        assert_true(Client.post.called)
        assert_equal(Client.post.call_args[1], test_args)

    def test_info(self):
        assert_equal(self.c.info(resource='wrong'), self.c.info())
