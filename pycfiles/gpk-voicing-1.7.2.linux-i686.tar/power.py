...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.5/site-packages/gpk_voicing/power.py
# Compiled at: 2009-09-06 10:02:50
import math, numpy
...
def smooth(ph, dt_in, dt_out, extra=0.0, wt=None):
    ...
    return (
     P1.smooth_guts(ph, dt_in, dt_out, w / dt_in, wt)[0], 0.0)
...
def test1():
    ...
    for i in range(20):
        ...
        assert numpy.alltrue(numpy.equal(numpy.less(x, 0.5), numpy.less(xk, 0.5)))
        assert numpy.alltrue(numpy.equal(numpy.less(x, 0.5), numpy.less(xs0, 0.5)))
        assert numpy.alltrue(numpy.equal(numpy.less(x, 0.5), numpy.less(xs1, 0.5)))
        assert numpy.alltrue(numpy.equal(numpy.less(x, 0.5), numpy.less(xs2, 0.5)))
