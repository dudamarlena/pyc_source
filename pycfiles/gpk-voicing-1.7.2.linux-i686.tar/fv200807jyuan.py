...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.5/site-packages/gpk_voicing/fv200807jyuan.py
# Compiled at: 2009-09-06 10:05:37
...
def feature_vec(data, dt, DT, LF=1.0, Nsv=M.NSV, ELF=1.0, do_voicing=1, do_dissonance=True, PlompBouman=False):
    assert do_dissonance
    assert not PlompBouman
    assert do_voicing
    ...
    (all_ectrs, all_ps, t0) = percep_spec.block_percep_spec(data, dt, DT, bmin, bmax, M.DB, do_mod=do_voicing, do_dissonance=do_dissonance, PlompBouman=PlompBouman)
    ...
    for fel in [0.04]:
        ...
        if w not in wset:
            (tmpo, tmpd) = M.fricative_edge(w, ectrs, neural, neural_now, Nsv, do_abs=True)
            o.extend(tmpo)
            descr.extend(tmpd)
            (tmpo, tmpd) = M.fricative_edge(ectrs, neural, neural_now, Nsv, do_abs=False, width=w)
            o.extend(tmpo)
            descr.extend(tmpd)
            wset.add(w)

    wset = set([0])
    for hpl in [0.03, 0.06]:
