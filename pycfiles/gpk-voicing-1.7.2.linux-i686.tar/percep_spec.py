...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.5/site-packages/gpk_voicing/percep_spec.py
...
def tau_lp(fc):
    ...
    Tau200 = 0.4
    Tau10k = 0.13
    tmp = Tau200 + M.log(fc / 200.0) / M.log(10000.0 / 200.0) * (Tau10k - Tau200)
    assert tmp > 0
    return tmp
    ...
    def get(self, Dt):
        if pylab:
            for (i, (y, sh)) in enumerate(zip(self.dbg, self.shifts)):
                ...
                pylab.plot(xpl, [ q / max(ypl) for q in ypl ], linestyle='-', color=(
                 float(i) / float(self.n), 0.5, float(self.n - i) / float(self.n)))
...
def perceptual_spec(data, dt, Dt, bmin=CBmin, bmax=CBmax, db=BBSZ, do_mod=0, do_dissonance=False, PlompBouman=True, do_peakalign=False):
    ...
    if do_mod == 1:
        ...
        bctrs.append({'type': 'haspitch', 'id': 'haspitch1', 'variant': 1, 'Fentropy': -M.log(len(tk))})
        iband += 1
        ...
        bctrs.append({'type': 'haspitch', 'id': 'haspitch_all', 'variant': 'all', 
           'Fentropy': -M.log(len(tk))})
        iband += 1
        neural[iband, :] = (tksum_high / tkn_high - tkmin_high)[:ntau] * 0.15
        bctrs.append({'type': 'haspitch', 'id': 'haspitch_high', 'variant': 'high', 
           'Fentropy': -M.log(len(tk_high))})
        iband += 1
        assert len(bctrs) == iband
    if do_dissonance:
        neural[iband, :] = diss.get(Dt)[:ntau] * 20.0
        bctrs.append({'type': 'roughness', 'id': 'roughness1', 'variant': 1, 
           'Fentropy': -M.log(diss.n)})
        iband += 1
    if do_peakalign:
        neural[iband, :] = palign.get(Dt)[:ntau]
        bctrs.append({'type': 'peakalign', 'id': 'peakalign1', 'variant': 1, 
...
if __name__ == '__main__':
    ...
    while arglist and arglist[0].startswith('-'):
        ...
        if arg == '--':
            ...
            extrahdr = dict([ q.strip().split('=', 1) for q in arglist.pop(0).split(';')
                            ])
        else:
    ...
    try:
        ...
        (bctrs, neural, tshift) = block_percep_spec(signal.column(0), signal.dt(), DT, do_mod=True, do_dissonance=True, do_peakalign=True, blocksize=Blocksize)
