...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.5/site-packages/gpk_voicing/fv_misc.py
# Compiled at: 2009-10-21 14:10:13
import re, math
...
def vowel(width, ectrs, neural, neural_now, Nsv, formant_low=None, formant_high=None):
    VFAC = 0.75
    cs = win(width)
    css = Num.sum(cs)
...
def spectral_entropy(width, ectrs, neural, neural_now, Nsv):
    ...
    assert np > 0
    dtmp = {'type': 'spectral_entropy', 'width': width, 
       'Kentropy': entropy(cs), 
...
def space_time_entropy(width, ectrs, neural, neural_now, Nsv):
    ...
    Num.multiply(ent_sum, -SEF, ent_sum)
    assert np > 0
    dtmp = {'type': 'space_time_entropy', 'width': width, 
       'Kentropy': entropy(cs), 
       'Fentropy': math.log(np), 
...
def pdur(ectrs, neural, neural_now, Dt, Cfac=1.0):
    ...
    descr = [
     {'id': 'pseudoduration:%f' % c, 'Kentropy': NG.N_median(Num.log(out)), 
...
def vowel_edge(width, ectrs, neural, neural_now, Nsv, do_abs=False, FORMANT_LOW=None, FORMANT_HIGH=None):
    VEfac = 0.7
...
def fricative(width, ectrs, neural, neural_now, Nsv):
    ...
    for (rs, re, fac) in [(70.0, 700.0, 0.6), (700.0, 1500.0, 0.6),
     ...
     (4000.0, 6000.0, 1.3)]:
        ...
        o.append(tmp)
        descr.append({'type': 'fricative', 'width': width, 'erbs': [ e['erb'] for e in included ], 'fcs': [ e['fc'] for e in included ], 'Kentropy': entropy(cs), 
...
def prominence(ectrs, neural, neural_now, Nsv):
    ...
    for (rs, re, fac) in [(70.0, 600.0, 0.22), (600.0, 2000.0, 0.25),
     (2000.0, 3000.0, 0.32), (3000.0, 6000.0, 0.35)]:
        ...
        o.append(tmp)
        dtmp = {'type': 'prominence', 'width': WIDTH, 'erbs': [ e['erb'] for e in included ], 'fcs': [ e['fc'] for e in included ], 'Kentropy': entropy(cs), 
           'Fentropy': -math.log(len(included)), 
...
def fricative_edge(width, ectrs, neural, neural_now, Nsv, do_abs=False):
    ...
    for (rs, re, fac) in [(70.0, 700.0, 7), (700.0, 1500.0, 7),
     ...
     (4000.0, 6000.0, 7)]:
        ...
        dtmp = {'width': width, 'erbs': [ e['erb'] for e in included ], 'fcs': [ e['fc'] for e in included ], 'Kentropy': entropy(cs), 
           'Fentropy': -math.log(len(included)), 
           'a_scaling': 1}
        if do_abs:
            o.append(Num.absolute(tmp))
...
def burst(ectrs, neural, neural_now, Nsv):
    ...
    for (rs, re, fac) in [(150.0, 500.0, 1.0), (500.0, 1200.0, 1.0),
     (1200.0, 2800.0, 1.0), (2800.0, 6000.0, 1.0)]:
        ...
        o.append(tss)
        descr.append({'type': 'burst', 'width': WIDTH, 'bkgwidth': BKGWIDTH, 
           'erbs': [ e['erb'] for e in included ], 'fcs': [ e['fc'] for e in included ], 'Kentropy': entropy(cs), 
           'Fentropy': -math.log(len(included)), 
           'id': 'burst:%d:%.1f-%.1f' % (
                WIDTH, elow, ehigh), 
           't_symmetry': 1, 
