...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.5/site-packages/gpk_voicing/fvsimple.py
...
def feature_vec(data, dt, DT, LF=1.0, Nsv=M.NSV, ELF=None, do_voicing=1, do_dissonance=True, PlompBouman=False):
    ...
    (all_ectrs, all_ps, t0) = percep_spec.perceptual_spec(data, dt, DT, bmin, bmax, M.DB, do_mod=do_voicing, do_dissonance=do_dissonance, PlompBouman=PlompBouman)
    ...
    o = []
    descr = []
    (tmpo, tmpd) = M.vowel(1, ectrs, neural, neural_now, Nsv, formant_low=FORMANT_LOW, formant_high=FORMANT_HIGH)
    o.extend(tmpo)
    descr.extend(tmpd)
    assert len(descr) == len(o), 'Descriptor mismatch'
    (tmpo, tmpd) = M.spectral_entropy(1, ectrs, neural, neural_now, Nsv)
    o.extend(tmpo)
    descr.extend(tmpd)
    assert len(descr) == len(o), 'Descriptor mismatch'
    if do_voicing:
