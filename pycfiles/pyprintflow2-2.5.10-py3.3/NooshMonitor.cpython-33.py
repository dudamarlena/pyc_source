...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/printflow2/NooshMonitor.py
# Compiled at: 2014-04-04 12:23:43
...
"""
import time, datetime, os, json, logging, smtplib, traceback
from email.mime.text import MIMEText
from email.header import Header
import requests, pyinotify
from .JDs import JDs
import utils
COMMASPACE = ', '
...
class NooshMonitor(pyinotify.ProcessEvent):
    ...
    accessToken = ''
    pdf_toolbox_path = '/opt/priontflow2/callas/bin/pdfToolboxServer.bin'
    check_interval = 10
    dbDir = ''
    jds = None
        ...
        while self.ready:
            try:
                try:
                    ...
                    ok = self.checkForProjects(self.the_workgroup, last_check_time, 'create_date_from', True)
                    if ok:
                        ok = self.checkForProjects(self.the_workgroup, last_check_time, 'last_spec_update_from', False)
                    else:
    ...
    def createDamSpecificationCategory(self, client, project, spec):
        try:
            ...
            if 'default_dropbox_folder' not in client:
                ...
                utils.ensureDirectoryExistsForUser(spec_folder, self.os_userid, self.os_groupid, 511)
                if 'project_folders' in client:
                    for f in client['project_folders']:
                        fp = os.path.join(spec_folder, f['name'])
                        utils.ensureDirectoryExistsForUser(fp, self.os_userid, self.os_groupid, 511)
