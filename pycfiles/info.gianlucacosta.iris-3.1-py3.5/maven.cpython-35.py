...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/info/gianlucacosta/iris/maven.py
...
class MavenArtifact:
    ...
    def __init__(self, groupId, artifactId=None, version=None, description=None, scope=None):
    ...
    def getFileName(self, suffix=None, extension='jar'):
        ...
        return '{0}-{1}{2}{3}'.format(self._artifactId, self._version.getRawString(), '-' + suffix.lstrip('-') if suffix is not None else '', '.' + extension.lstrip('.') if extension is not None else '')
    ...
    def __str__(self):
        ...
        assert self._artifactId is not None
        assert self._version is not None
        return '{0}:{1}:{2}'.format(self._groupId, self._artifactId, self._version.getRawString())
