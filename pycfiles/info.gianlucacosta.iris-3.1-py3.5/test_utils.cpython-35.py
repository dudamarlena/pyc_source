...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/info/gianlucacosta/iris/tests/io/test_utils.py
# Compiled at: 2017-10-18 21:14:06
...
class PathOperationsTests(AbstractIoTestCase):
    ...
    def testLinearWalkWithoutFilter(self):
        ...
        linearSequence = {os.path.join(item.dirPath, item.baseName) for item in PathOperations.linearWalk(treeDir)}
        self.assertEquals({
         os.path.join(treeDir, 'sigma'),
         os.path.join(treeDir, 'alpha', 'T1'),
         os.path.join(treeDir, 'alpha', 'gamma', 'delta', 'T2'),
         os.path.join(treeDir, 'beta', 'T3')}, linearSequence)
    ...
    def testLinearWalkUsingFilterOnDirNamesOnly(self):
        treeDir = os.path.join(self._ioTestPath, 'tree')
        linearSequence = [os.path.join(item.dirPath, item.baseName) for item in PathOperations.linearWalk(treeDir, self._filterOutAlphaDirsOnly)]
        self.assertEquals([
         os.path.join(treeDir, 'sigma'),
         os.path.join(treeDir, 'beta', 'T3')], linearSequence)
    ...
    def testLinearWalkUsingFilterOnDirNamesAndParentDir(self):
        treeDir = os.path.join(self._ioTestPath, 'tree')
        linearSequence = [os.path.join(item.dirPath, item.baseName) for item in PathOperations.linearWalk(treeDir, self._filterOutAlphaDirsAndTheirParent)]
        self.assertEquals([
         os.path.join(treeDir, 'beta', 'T3')], linearSequence)

    def testLinearWalkFilteringOnFileNames(self):
        treeDir = os.path.join(self._ioTestPath, 'tree')
        linearSequence = [os.path.join(item.dirPath, item.baseName) for item in PathOperations.linearWalk(treeDir, lambda dirPath, dirNames, fileNames: 'T2' in fileNames)]
        self.assertEquals([
         os.path.join(treeDir, 'alpha', 'gamma', 'delta', 'T2')], linearSequence)
