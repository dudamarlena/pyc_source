...
# [GCC 8.4.0]
# Embedded file name: /root/droopescan/dscan/common/update_api.py
# Compiled at: 2019-06-14 01:34:00
# Size of source mod 2**32: 13633 bytes
...
import dscan, dscan.common.functions as functions, dscan.common.versions as v, json, os, os.path, requests, subprocess
GH = 'https://github.com/'
UW = './.update-workspace/'
...
def _newer_tags_get(current_highest, versions):
    ...
    for major in current_highest:
        ...
        for version in versions:
            ...
            if version.startswith(major) and version_gt(version, highest_version):
...
def multipart_parse_json(api_url, data):
    ...
    response_text = requests.post(api_url, data=data, headers=headers).text.encode('ascii', errors='replace')
