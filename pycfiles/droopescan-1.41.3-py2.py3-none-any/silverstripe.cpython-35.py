...
# [GCC 8.4.0]
# Embedded file name: /root/droopescan/dscan/plugins/silverstripe.py
# Compiled at: 2019-06-14 01:39:45
...
import dscan.common.update_api as ua, re, requests, sys
try:
...
class Silverstripe(BasePlugin):
    ...
    def update_version_check(self):
        ...
        """
        return ua.github_tags_newer(self._repo_framework, self.versions_file, update_majors=self.update_majors)

    def update_version(self):
        """
            @return: updated VersionsFile
        """
        fw_gr, versions_file, new_tags = ua.github_repo_new(self._repo_framework, 'silverstripe/framework', self.versions_file, self.update_majors)
        cms_gr, _, _ = ua.github_repo_new(self._repo_cms, 'silverstripe/cms', self.versions_file, self.update_majors)
        hashes = {}
    ...
    def update_plugins(self):
        ...
        return (
         plugins_folder, themes_folder)

    def _get(self, url, package):
        retry = Retrying(wait_exponential_multiplier=2000, wait_exponential_max=120000, retry_on_exception=_retry_msg)
    ...
    def _convert_to_folder(self, packages):
        ...
        with ThreadPoolExecutor(max_workers=12) as (executor):
            ...
            for i, future in enumerate(futures, start=1):
                ...
                if 'installer-name' not in r.text:
                    folder_name = package.split('/')[1]
                else:
                    splat = list(filter(None, re.split('[^a-zA-Z0-9-_.,]', r.text)))
