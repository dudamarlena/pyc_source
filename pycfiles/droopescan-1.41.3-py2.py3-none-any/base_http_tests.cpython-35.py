...
# [GCC 8.4.0]
# Embedded file name: /root/droopescan/dscan/tests/base_http_tests.py
# Compiled at: 2019-06-14 06:54:24
# Size of source mod 2**32: 36260 bytes
...
class BaseHttpTests(BaseTest):
    ...
    def test_plugins_forbidden(self, m):
        ...
        result, empty = self.scanner.enumerate_plugins(self.base_url, self.scanner.plugins_base_url, ScanningMethod.forbidden)
        ...
        assert result == expected_result, "Should have detected the                 'supermodule' module."
    ...
    def test_plugins_ok(self, m):
        self.respond_several(self.base_url + 'sites/all/modules/%s/', {200: ['supermodule'], 
         404: ['nonexistant1', 'nonexistant2']})
        self.scanner.plugins_base_url = '%ssites/all/modules/%s/'
        result, empty = self.scanner.enumerate_plugins(self.base_url, self.scanner.plugins_base_url, ScanningMethod.ok)
        module_name = 'supermodule'
        expected_module_url = self.scanner.plugins_base_url % (self.base_url,
         module_name)
        expected_result = [
         {'url': expected_module_url, 'name': module_name}]
        assert result == expected_result, "Should have detected the                 'supermodule' module."
    ...
    def test_plugins_not_found(self, m):
        ...
        self.respond_several(self.base_url + 'sites/all/modules/%s', {200: [
               'supermodule/%s' % module_file], 
         404: ['nonexistant1', 'nonexistant2',
               'supermodule', 'nonexistant1/%s' % module_file, 'nonexistant2/%s' % module_file]})
        self.scanner.plugins_base_url = '%ssites/all/modules/%s/'
        result, empty = self.scanner.enumerate_plugins(self.base_url, self.scanner.plugins_base_url, ScanningMethod.not_found)
        module_name = 'supermodule'
        expected_module_url = self.scanner.plugins_base_url % (self.base_url,
         module_name)
        expected_result = [{'url': expected_module_url, 'name': module_name}]
    ...
    def test_plugins_multiple_base_url(self, m):
        ...
        self.respond_several(base_2, {200: [
               'supermodule2'], 
         404: ['nonexistant1', 'supermodule', 'amazing_mod']})
        self.respond_several(base_3, {200: [
               'amazing_mod'], 
         404: ['nonexistant1', 'supermodule', 'supermodule2']})
        result, empty = self.scanner.enumerate_plugins(self.base_url, self.scanner.plugins_base_url, ScanningMethod.ok)
    ...
    def test_add_slash_to_urls(self):
        ...
        m = self.mock_controller('drupal', 'determine_scanning_method', side_effect=[
         'forbidden'])
    ...
    def test_determine_detect_false_ok_drupal(self):
        self.add_argv(self.param_plugins)
        responses.add(responses.HEAD, self.base_url + self.scanner.not_found_url, body='A' * 1337)
        responses.add(responses.HEAD, self.base_url + 'misc/drupal.js', body='A' * 15000)
    ...
    def test_determine_with_multiple_ok(self):
        self.scanner.regular_file_url = [
         'misc/drupal_old.js', 'misc/drupal.js']
        self.respond_several(self.base_url + '%s', {200: [Drupal.forbidden_url,
               'misc/drupal.js'], 
         404: ['misc/drupal_old.js',
               self.scanner.not_found_url]})
        scanning_method = self.scanner.determine_scanning_method(self.base_url, 'head')
    ...
    def test_not_cms(self):
        self.add_argv(self.param_plugins)
        url_list = [
         self.scanner.forbidden_url, self.scanner.not_found_url]
    ...
    def test_verb_works(self, m):
        ...
        self.scanner.enumerate_plugins(self.base_url, base_url, verb='get')
    ...
    def test_determine_respects_verb(self):
        ...
        responses.add(responses.GET, self.base_url + self.scanner.not_found_url, status=404)
        m = self.mock_controller('drupal', 'enumerate_plugins')
        self.app.run()
    ...
    def test_finds_interesting_urls(self):
        ...
        self.respond_several(self.base_url + '%s', {200: [self.scanner.interesting_urls[0][0]], 
         404: [self.scanner.interesting_urls[1][0]]})
        found, empty = self.scanner.enumerate_interesting(self.base_url, self.scanner.interesting_urls)
        expected_result = [
         {'url': self.base_url + path, 'description': description}]
        assert not empty
    ...
    def test_interesting_respects_verb(self):
        ...
        self.respond_several(self.base_url + '%s', {200: urls_200}, verb=responses.GET)
        self.app.run()
    ...
    def test_warns_on_500(self, warn, mock):
        ...
        r_500 = ['intermitent/']
        self.respond_several(self.base_url + 'sites/all/modules/%s', {200: r_200, 
         404: r_404, 500: r_500})
        self.scanner.plugins_base_url = '%ssites/all/modules/%s/'
    ...
    def test_redirect_changes_url(self):
        ...
        dr = self.mock_controller('drupal', '_determine_redirect', return_value=self.base_url_https)
        self.mock_controller('drupal', 'determine_scanning_method', return_value=ScanningMethod.not_found)
    ...
    def test_redirect_is_detected(self):
        responses.reset()
        self.respond_several(self.base_url + '%s', {301: ['']}, headers={'location': self.base_url_https})
        result = self.scanner._determine_redirect(self.base_url, Verb.head)
    ...
    def test_redirect_no_relative(self):
        ...
        """
        responses.reset()
        redirect_vals = {'/': self.base_url, 
         '/relative': self.base_url, 
         'relative': self.base_url}
        for i in redirect_vals:
            responses.add(responses.HEAD, self.base_url, status=301, adding_headers={'location': i})
        ...
        responses.add(responses.HEAD, another_base, status=301, adding_headers={'location': 'relative'})
    ...
    def test_redirect_no_same_url(self):
        ...
        """
        responses.reset()
        responses.add(responses.HEAD, self.base_url, status=301, adding_headers={'location': self.base_url + 'install.php'})
        ru = self.scanner._determine_redirect(self.base_url, 'head')
    ...
    def test_respects_timeout_scanning_method(self, mock_head):
        try:
            self.scanner.determine_scanning_method(self.base_url, 'head', timeout=5)
        except RuntimeError:
            pass
    ...
    def test_respects_timeout_enumerate(self, mock_head):
        self.scanner.plugins_base_url = '%ssites/all/modules/%s/'
        result, empty = self.scanner.enumerate_plugins(self.base_url, self.scanner.plugins_base_url, ScanningMethod.forbidden, imu=[
         ('readme', '')], timeout=5)
        self.assert_called_contains_all(mock_head, 'timeout', 5)

    @patch('requests.Session.get')
    def test_respects_timeout_version(self, mock_get):
        version, is_empty = self.scanner.enumerate_version(self.base_url, timeout=5)
        self.assert_called_contains_all(mock_get, 'timeout', 5)

    @patch('requests.Session.head')
    def test_respects_timeout_interesting(self, mock_head):
        found, empty = self.scanner.enumerate_interesting(self.base_url, self.scanner.interesting_urls, timeout=5)
    ...
    def test_progressbar_url_file_hidden_in_enumerate_plugins(self):
        with patch('dscan.plugins.internal.base_plugin_internal.ProgressBar') as (p):
            try:
                self.scanner.enumerate_plugins(self.base_url, self.scanner.plugins_base_url, hide_progressbar=True)
            except:
                pass
    ...
    def test_progressbar_url_file_hidden_in_enumerate_themes(self, p):
        with patch('dscan.plugins.internal.base_plugin_internal.ProgressBar') as (p):
            try:
                self.scanner.enumerate_themes(self.base_url, self.scanner.plugins_base_url, hide_progressbar=True)
            except:
                pass
    ...
    def test_progressbar_url_file_hidden_in_enumerate_interesting(self):
        with patch('dscan.plugins.internal.base_plugin_internal.ProgressBar') as (p):
            try:
                self.scanner.enumerate_interesting(self.base_url, self.scanner.plugins_base_url, hide_progressbar=True)
            except:
                pass
    ...
    def test_progressbar_url_file_hidden_in_enumerate_version(self):
        with patch('dscan.plugins.internal.base_plugin_internal.ProgressBar') as (p):
            try:
                self.scanner.enumerate_version(self.base_url, self.scanner.versions_file, hide_progressbar=True)
            except:
                pass
    ...
    def test_detect_imu(self, m):
        ...
        final_found = self.scanner._enumerate_plugin_if(found, 'head', 4, imu, True, 15, {})
    ...
    def test_respects_host_redirect(self, mock_head):
        try:
            self.scanner._determine_redirect(self.base_url, 'head', headers=self.host_header)
        except RuntimeError:
            pass
    ...
    def test_respects_host_scanning_method(self, mock_head):
        try:
            self.scanner.determine_scanning_method(self.base_url, 'head', headers=self.host_header)
        except RuntimeError:
            pass
    ...
    def test_respects_host_enumerate(self, mock_head):
        self.scanner.plugins_base_url = '%ssites/all/modules/%s/'
        result, empty = self.scanner.enumerate_plugins(self.base_url, self.scanner.plugins_base_url, ScanningMethod.forbidden, imu=[
         ('readme', '')], headers=self.host_header)
        self.assert_called_contains_all(mock_head, 'headers', self.host_header)

    @patch('requests.Session.get')
    def test_respects_host_version(self, mock_get):
        version, is_empty = self.scanner.enumerate_version(self.base_url, headers=self.host_header)
        self.assert_called_contains_all(mock_get, 'headers', self.host_header)

    @patch('requests.Session.head')
    def test_respects_host_interesting(self, mock_head):
        found, empty = self.scanner.enumerate_interesting(self.base_url, self.scanner.interesting_urls, headers=self.host_header)
    ...
    def test_module_fake_200(self, warn, mock):
        ...
        self.respond_several(self.base_url + 'sites/all/modules/%s', {404: r_404, 
         403: r_403, 500: r_500, 200: r_200})
        scanner.plugins_base_url = '%ssites/all/modules/%s/'
        self.mock_controller('drupal', 'enumerate_interesting')
        result, empty = scanner.enumerate_plugins(self.base_url, scanner.plugins_base_url, ScanningMethod.forbidden)
        assert len(result) == 4
        found_500 = False
        found_200 = False
        for res in result:
