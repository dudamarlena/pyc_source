...
# [GCC 8.4.0]
# Embedded file name: /root/droopescan/dscan/plugins/tests.py
...
class Tests(HumanBasePlugin):
    ...
    class Meta:
        ...
        arguments = [
         (
          [
           '-s', '--single-test'],
          dict(action='store', help='Name of test to run', required=False, default=None)),
         (
          [
           '-3', '--just-three'],
          dict(action='store_true', help='Just run python 3 tests.', required=False, default=None)),
         (
          [
           '-2', '--just-two'],
          dict(action='store_true', help='Just run python 3 tests.', required=False, default=None)),
         (
          [
           '-c', '--with-coverage'],
          dict(action='store_true', help='Do test coverage', required=False, default=False))]
    ...
    def default(self):
        ...
        if not single_test:
            call_base = [
             '/usr/local/bin/nosetests', '--exclude=async']
            if with_coverage:
