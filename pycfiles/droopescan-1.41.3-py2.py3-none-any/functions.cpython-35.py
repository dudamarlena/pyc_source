...
# [GCC 8.4.0]
# Embedded file name: /root/droopescan/dscan/common/functions.py
...
try:
    from requests.exceptions import ConnectionError, ReadTimeout, ConnectTimeout, TooManyRedirects
except:
    old_req = 'Running a very old version of requests! Please `pip\n        install -U requests`.'
    print(old_req)

import dscan, hashlib, pystache, re, sys, traceback, xml.etree.ElementTree as ET
...
def scan_http_status(scanning_method):
    ...
    if scanning_method == ScanningMethod.forbidden:
        return 403
    if scanning_method == ScanningMethod.ok:
        return 200
...
def version_gt(version, gt):
    ...
    for i in range(l):
        ...
        v = overcame_shortest or version_split[i]
        g = gt_split[i]
        v_is_rc = '-' in v or '_' in v
        g_is_rc = '-' in g or '_' in g
        if v_is_rc:
            v_split = re.split('[-_]', v)
            v = v_split[0]
            try:
...
def error(msg):
    raise RuntimeError('\x1b[91m%s\x1b[0m' % msg)
    ...
    for cms_name in preferred_order:
        for plugin in plugins:
            ...
            if cms_name == plugin_name:
                instances[plugin_name] = instance_get(plugin, opts, url_file_input, out)
...
def result_anything_found(result):
    ...
    """
    keys = [
     'version', 'themes', 'plugins', 'interesting urls']
    anything_found = False
    for k in keys:
        if k not in result:
            continue
        elif not result[k]['is_empty']:
            anything_found = True
