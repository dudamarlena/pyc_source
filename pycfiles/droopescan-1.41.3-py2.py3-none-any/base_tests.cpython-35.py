...
# [GCC 8.4.0]
# Embedded file name: /root/droopescan/dscan/tests/base_tests.py
# Compiled at: 2019-06-14 01:34:00
# Size of source mod 2**32: 12418 bytes
...
class BaseTests(BaseTest):
    ...
    def test_calls_all(self):
        ...
        for m in all_mocks:
            if not m.called:
                raise AssertionError('module %s' % m)
        ...
        with patch('dscan.common.output.open', m, create=True) as (o):
            with patch(mock_module, autospec=True) as (m):
                ...
                for i, call in enumerate(calls):
                    ...
                    url = args[1]
                    if i == 0:
                        if not url == 'example3.com\n':
                            raise AssertionError
                    elif i == 1:
                        if not url == 'example4.com\n':
                            raise AssertionError
                        else:
                            if not False:
                                raise AssertionError('Only two calls expected.')
    ...
    def test_calls_async(self):
        ...
        self.add_argv(['--url-file', self.valid_file])
        self.add_argv(['--async'])
        with patch('dscan.plugins.internal.scan.identify_url_file', autospec=True) as (iuf):
            self.app.run()
            assert iuf.called