...
# [GCC 8.4.0]
# Embedded file name: /root/droopescan/dscan/plugins/internal/base_plugin_internal.py
# Compiled at: 2019-06-14 01:34:00
# Size of source mod 2**32: 38366 bytes
...
from dscan.common import ScanningMethod, StandardOutput, JsonOutput, VersionsFile, RequestsLogger
...
import dscan, dscan.common.functions as f, hashlib, os, re, requests, sys, traceback
try:
...
class BasePluginInternal(controller.CementBaseController):
    ...
    def _base_kwargs(self, opts):
        kwargs_plugins = {'threads': opts['threads_enumerate'], 
         'verb': opts['verb'], 
    ...
    def _functionality(self, opts):
        ...
        kwargs_plugins = dict_combine(kwargs_base, {'base_url': plugins_base_url, 
        ...
        all = {'plugins': {'func': self.enumerate_plugins, 
                     'template': 'enumerate_plugins.mustache', 
                     'kwargs': kwargs_plugins}, 
         
         'themes': {'func': self.enumerate_themes, 
                    'template': 'enumerate_plugins.mustache', 
                    'kwargs': kwargs_themes}, 
         
         'version': {'func': self.enumerate_version, 
                     'template': 'enumerate_version.mustache', 
                     'kwargs': {'verb': opts['verb'], 
                                'threads': opts['threads_enumerate'], 
                                'timeout': opts['timeout'], 
                                'headers': opts['headers']}}, 
         
         'interesting urls': {'func': self.enumerate_interesting, 
    ...
    def _general_init(self, opts, out=None):
        ...
        return (
         hide_progressbar, functionality, enabled_functionality)
    ...
    def plugin_init(self):
        ...
        if not common.shutdown:
            self.out.echo('\x1b[95m[+] Scan finished (%s elapsed)\x1b[0m' % str(datetime.now() - time_start))
        else:
            sys.exit(130)
    ...
    def process_url(self, opts, functionality, enabled_functionality, hide_progressbar):
        try:
            url = (
             opts['url'], opts['headers']['Host'])
        except:
            url = opts['url']

        if not url:
            self.out.fatal('--url parameter is blank.')
        output = self.url_scan(url, opts, functionality, enabled_functionality, hide_progressbar=hide_progressbar)
        if opts['output'] == 'json':
    ...
    def process_url_iterable(self, iterable, opts, functionality, enabled_functionality):
        ...
        with ThreadPoolExecutor(max_workers=opts['threads_scan']) as (executor):
            ...
            for url in iterable:
                args = [
                 url, opts, functionality, enabled_functionality, True]
                future = executor.submit(self.url_scan, *args)
                url_to_log = str(url).rstrip()
                results.append({'future': future, 
                 'url': url_to_log})
                if i % 1000 == 0 and i != 0:
                    self._process_results_multisite(results, functionality, timeout_host)
                    results = []
                i += 1
    ...
    def process_url_file(self, opts, functionality, enabled_functionality):
        ...
        with open(file_location) as (url_file):
            ...
            self.resume_forward(url_file, opts['resume'], opts['url_file'], opts['error_log'])
    ...
    def url_scan(self, url, opts, functionality, enabled_functionality, hide_progressbar):
        ...
        if need_sm and (self.can_enumerate_plugins or self.can_enumerate_themes):
            ...
            if not scanning_method:
                scanning_method = self.determine_scanning_method(url, opts['verb'], opts['timeout'], self._generate_headers(host_header))
        else:
    ...
    def _determine_fake_200(self, requests_verb, url):
        ...
        return (
         response.status_code == 200, len(response.content))
    ...
    def determine_scanning_method(self, url, verb, timeout=15, headers={}):
        requests_verb = partial(getattr(self.session, verb), timeout=timeout, headers=headers)
        ...
        if folder_300 and ok_200:
            self.out.warn('Server returns redirects for folders. If a module does not have a %s file it will not be detected.' % self.module_common_file)
            return ScanningMethod.not_found
    ...
    def enumerate(self, url, base_url_supplied, scanning_method, iterator_returning_method, iterator_len, max_iterator=500, threads=10, verb='head', timeout=15, hide_progressbar=False, imu=None, headers={}):
        """
        ...
        with ThreadPoolExecutor(max_workers=threads) as (executor):
            for base_url in base_urls:
                ...
                for plugin_name in plugins:
                    ...
                    future = executor.submit(requests_verb, plugin_url, timeout=timeout, headers=headers)
                    if plugin_url.endswith('/'):
                        final_url = plugin_url
                    else:
                        final_url = dirname(plugin_url) + '/'
                    futures.append({'base_url': base_url, 
                     'future': future, 
        ...
        if not common.shutdown and imu != None and not no_results:
            found = self._enumerate_plugin_if(found, verb, threads, imu, hide_progressbar, timeout=timeout, headers=headers)
        return (found, no_results)

    def enumerate_plugins(self, url, base_url, scanning_method='forbidden', max_plugins=500, threads=10, verb='head', timeout=15, hide_progressbar=False, imu=None, headers={}):
        iterator = self.plugins_get
        iterator_len = file_len(self.plugins_file)
        return self.enumerate(url, base_url, scanning_method, iterator, iterator_len, max_plugins, threads, verb, timeout, hide_progressbar, imu, headers)

    def enumerate_themes(self, url, base_url, scanning_method='forbidden', max_plugins=500, threads=10, verb='head', timeout=15, hide_progressbar=False, imu=None, headers={}):
        iterator = self.themes_get
        iterator_len = file_len(self.themes_file)
        return self.enumerate(url, base_url, scanning_method, iterator, iterator_len, max_plugins, threads, verb, timeout, hide_progressbar, imu, headers)

    def enumerate_interesting(self, url, interesting_urls, threads=10, verb='head', timeout=15, hide_progressbar=False, headers={}):
        requests_verb = getattr(self.session, verb)
        if not hide_progressbar:
        ...
        for path, description in interesting_urls:
            if common.shutdown:
                ...
                resp = requests_verb(interesting_url, timeout=timeout, headers=headers)
                if resp.status_code == 200 or resp.status_code == 301:
                    found.append({'url': interesting_url, 
    ...
    def enumerate_version(self, url, threads=10, verb='head', timeout=15, hide_progressbar=False, headers={}):
        """
        ...
        if not hide_progressbar:
            p = ProgressBar(sys.stderr, len(files) + len(changelogs), 'version')
        hashes = {}
        futures = {}
        with ThreadPoolExecutor(max_workers=threads) as (executor):
            for file_url in files:
                futures[file_url] = executor.submit(self.enumerate_file_hash, url, file_url=file_url, timeout=timeout, headers=headers)
    ...
    def _enumerate_plugin_if(self, found_list, verb, threads, imu_list, hide_progressbar, timeout=15, headers={}):
        """
        ...
        """
        if not hide_progressbar:
            p = ProgressBar(sys.stderr, len(found_list) * len(imu_list), name='IMU')
        requests_verb = getattr(self.session, verb)
        with ThreadPoolExecutor(max_workers=threads) as (executor):
            futures = []
            for i, found in enumerate(found_list):
                ...
                for imu in imu_list:
                    ...
                    future = executor.submit(requests_verb, interesting_url, timeout=timeout, headers=headers)
                    futures.append({'url': interesting_url, 
                     'future': future, 
                     'description': imu[1], 
                     'i': i})
            ...
            for f in futures:
                if common.shutdown:
                    ...
                    r = f['future'].result()
                    if r.status_code == 200:
                        found_list[f['i']]['imu'].append({'url': f['url'], 
