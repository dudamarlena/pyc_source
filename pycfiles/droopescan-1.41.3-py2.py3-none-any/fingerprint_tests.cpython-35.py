...
# [GCC 8.4.0]
# Embedded file name: /root/droopescan/dscan/tests/fingerprint_tests.py
# Compiled at: 2019-06-14 01:34:00
# Size of source mod 2**32: 25491 bytes
...
import dscan, hashlib, requests, responses
try:
...
class FingerprintTests(BaseTest):
    ...
    def test_add_to_xml(self):
        add_versions = {'7.31': {'misc/ajax.js': '30d9e08baa11f3836eca00425b550f82', 
    ...
    def test_narrow_down_changelog(self):
        ...
        result = self.scanner.enumerate_version_changelog(self.base_url, mock_versions)
        assert result == ['7.27']
    ...
    def test_narrow_down_ignore_incorrect_changelog(self):
        ...
        result = self.scanner.enumerate_version_changelog(self.base_url, mock_versions, v_changelog)
    ...
    def test_multiple_changelogs_all_fail(self):
        ...
        self.scanner.enumerate_file_hash = self.mock_xml(xml_multi_changelog, '7.1')
        result = self.scanner.enumerate_version_changelog(self.base_url, mock_versions, v_changelog)
        assert result == mock_versions
    ...
    def _mock_cms_multiple(self, cms_ident_side_eff, redir_side_eff=None, url_file_host=False, mock_redir=True):
        ...
        if mock_redir:
            if not redir_side_eff:
                ...
                r_p = patch(self.redir_module, autospec=True, side_effect=redir_side_eff)
            r_p.start()
            self.p_list.append(r_p)
        pui_p = patch(self.pui_module, autospec=True)
        pui = pui_p.start()
        self.p_list.append(pui_p)
        cim_p = patch(self.cms_identify_module, side_effect=cms_ident_side_eff, autospec=True)
        cim = cim_p.start()
    ...
    def test_cms_identify_respected_multiple(self):
        ...
        cim, pui = self._mock_cms_multiple(cms_ident_side_eff=return_value, url_file_host=True)
        self.app.run()
    ...
    def test_version_highest_not_present_multiple_digit(self):
        add_versions = {'3.1.31': {'misc/ajax.js': '30d9e08baa11f3836eca00425b550f82', 
                    'misc/drupal.js': '0bb055ea361b208072be45e8e004117b', 
                    'misc/tabledrag.js': 'caaf444bbba2811b4fa0d5aecfa837e5', 
                    'misc/tableheader.js': 'bd98fa07941364726469e7666b91d14d'}, 
         
         '3.2.12': {'misc/drupal.js': '1904f6fd4a4fe747d6b53ca9fd81f848', 
                    'misc/tabledrag.js': '50ebbc8dc949d7cb8d4cc5e6e0a6c1ca', 
                    'misc/tableheader.js': '570b3f821441cd8f75395224fc43a0ea'}}
        self.v.update(add_versions)
