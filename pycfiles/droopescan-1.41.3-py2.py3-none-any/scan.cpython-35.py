...
# [GCC 8.4.0]
# Embedded file name: /root/droopescan/dscan/plugins/internal/scan.py
# Compiled at: 2019-06-14 01:34:00
# Size of source mod 2**32: 10829 bytes
...
import dscan, dscan.common.functions as f, dscan.common.plugins_util as pu, dscan.common.versions as v, gc
...
class Scan(BasePlugin):
    ...
    class Meta:
        ...
        arguments = [
         ...
         (
          [
           '-U', '--url-file'],
          dict(action='store', help='A file which\n                    contains a list of URLs.')),
         (
          [
           '--enumerate', '-e'],
          dict(action='store', help='R|' + common.template('help_enumerate.mustache'), choices=common.enum_list(common.Enumerate), default='a')),
         (
          [
           '--method'],
          dict(action='store', help='R|' + common.template('help_method.mustache'), choices=common.enum_list(common.ScanningMethod))),
         (
          [
           '--verb'],
          dict(action='store', help="The HTTP verb to use;\n                    the default option is head, except for version enumeration\n                    requests, which are always get because we need to get the hash\n                    from the file's contents", default='head', choices=common.enum_list(common.Verb))),
         (
          [
           '--number', '-n'],
          dict(action='store', help="Number of\n                    words to attempt from the plugin/theme dictionary. Default\n                    is 1000. Use -n 'all' to use all available.", default=BasePluginInternal.NUMBER_DEFAULT)),
         (
          [
           '--plugins-base-url'],
          dict(action='store', help="Location\n                    where the plugins are stored by the CMS. Default is the CMS'\n                    default location. First %%s in string will be replaced with\n                    the url, and the second one will be replaced with the module\n                    name. E.g. '%%ssites/all/modules/%%s/'")),
         (
          [
           '--themes-base-url'],
          dict(action='store', help='Same as\n                    above, but for themes.')),
         (
          [
           '--timeout'],
          dict(action='store', help='How long to wait\n                    for an HTTP response before timing out (in seconds).', default=45, type=int)),
         (
          [
           '--timeout-host'],
          dict(action='store', help='Maximum time\n                    to spend per host (in seconds).', default=1800, type=int)),
         (
          [
           '--no-follow-redirects'],
          dict(action='store_false', help='Prevent\n                    the following of redirects.', dest='follow_redirects', default=True)),
         (
          [
           '--host'],
          dict(action='store', help='Override host header\n                    with this value.', default=None)),
         (
          [
           '--massscan-override'],
          dict(action='store_true', help='Overrides defaults with defaults convenient for\n                    mass-scanning of hosts.', default=False)),
         (
          [
           '--threads', '-t'],
         ...
         (
          [
           '--threads-scan'],
          dict(action='store', help='Threads used\n                    for mass scanning.', default=None, type=int)),
         (
          [
           '--threads-enumerate'],
          dict(action='store', help='Threads\n                    used for plugin enumeration.', default=None, type=int)),
         (
          [
           '--output', '-o'],
          dict(action='store', help='Output format', choices=common.enum_list(common.ValidOutputs), default='standard')),
         (
          [
    ...
    def default(self):
        ...
        if url_file_input:
            ...
            instances = self._instances_get(opts, plugins, url_file_input, self.out)
    ...
    def _process_scan_url_file(self, opts, follow_redirects):
        ...
        with open(file_location) as (url_file):
            ...
            self.resume_forward(url_file, opts['resume'], file_location, opts['error_log'])
            i = 0
            urls = []
            for url in url_file:
                urls.append(url)
                if i % 2500 == 0 and i != 0:
                    ...
                    self._process_generate_futures(urls, executor, opts, instances, follow_redirects)
                    executor.shutdown()
                    gc.collect()
                    urls = []
                i += 1
    ...
    def _process_generate_futures(self, urls, executor, opts, instances, follow_redirects):
        ...
        for url in urls:
            url = url.strip()
            future = executor.submit(self._process_cms_identify, url, opts, instances, follow_redirects)
            future.url = url
    ...
    def _recreate_all(self):
        ...
        return (
         plugins, opts, executor, instances)