...
# [GCC 8.4.0]
# Embedded file name: /root/droopescan/dscan/tests/update_tests.py
# Compiled at: 2019-06-14 01:34:00
# Size of source mod 2**32: 24496 bytes
...
from dscan.common.update_api import github_tags_newer, github_repo, _github_normalize, file_mtime, modules_get, _tag_is_rubbish, _newer_tags_get
...
import codecs, dscan, json, responses
...
class UpdateTests(BaseTest):
    ...
    def setUp(self):
        ...
        for mod in os_mock:
            ...
            if mod_name == 'plugins_base_get':
                ret_val = [
                 self.controller_get('drupal')]
            if ret_val != None:
                self.patchers.append(patch(mod, return_value=ret_val, autospec=True))
            else:
    ...
    def test_files_get_all_chlg(self):
        ...
        assert changelog_file not in files

    def test_updates_changelog(self):
        weird_hash = '13371337133713371337133713371337'
        vf = VersionsFile(self.update_versions_xml)
        hashes = {'6.34': {'misc/ajax.js': 'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA', 
    ...
    def test_plugins_update_check(self):
        drupal = Drupal()
        drupal.update_plugins = up = Mock(spec=self.scanner.update_plugins, return_value=([], []))
    ...
    def test_modules_get(self):
        ...
        per_page = 25
        do_resp = codecs.open(dscan.PWD + 'tests/resources/drupal_org_response.html', encoding='utf-8').read()
        do_resp_last = codecs.open(dscan.PWD + 'tests/resources/drupal_org_response_partial.html', 'r', 'utf-8').read()
        responses.add(responses.GET, 'https://drupal.org/project/project_module?page=0', body=do_resp, match_querystring=True)
        responses.add(responses.GET, 'https://drupal.org/project/project_module?page=1', body=do_resp, match_querystring=True)
        responses.add(responses.GET, 'https://drupal.org/project/project_module?page=2', body=do_resp_last, match_querystring=True)
        plugins = []
    ...
    def test_drupal_plugins_updates_file(self):
        ...
        self.scanner.update_plugins = Mock(spec=self.scanner.update_plugins, return_value=(
         plugins, themes))
        m = mock_open()
        with patch('dscan.plugins.update.open', m, create=True) as (o):
            ...
            for i, call in enumerate(o().write.call_args_list):
                ...
                if not call[0][0].rstrip() == check_against[(i % 3)]:
                    raise AssertionError
    ...
    def _mod_ss_modules_mock(self):
        ...
        responses.add(responses.GET, 'http://addons.silverstripe.org/add-ons?search=&type=module&sort=downloads&start=0', body=do_resp, match_querystring=True)
        responses.add(responses.GET, 'http://addons.silverstripe.org/add-ons?search=&type=module&sort=downloads&start=16', body=do_resp, match_querystring=True)
        responses.add(responses.GET, 'http://addons.silverstripe.org/add-ons?search=&type=module&sort=downloads&start=32', body=do_resp_last, match_querystring=True)
        responses.add(responses.GET, 'http://addons.silverstripe.org/add-ons?search=&type=theme&sort=downloads&start=0', body=do_resp, match_querystring=True)
        responses.add(responses.GET, 'http://addons.silverstripe.org/add-ons?search=&type=theme&sort=downloads&start=16', body=do_resp_last, match_querystring=True)
        ...
        for mod in ss_modules:
            ...
            if mod == 'ajshort/silverstripe-gridfieldextensions':
                responses.add(responses.GET, url, body=packagist_with_installer)
            else:
                responses.add(responses.GET, url, body=packagist_without_installer)
    ...
    def test_moodle_strip_prefix(self):
        versions = [
         'v3.2.2', 'v3.1.5', 'v3.0.9', 'v3.2.1', 'v3.1.4',
         'v3.0.8', 'v3.2.0', 'v3.2.0-rc5']
        current_highest = {'3.2': '3.2rc4', 
         '3.3': '3.2.9999', 
         '3.0': '3.0.7', 
         '3.1': '3.1.3'}
