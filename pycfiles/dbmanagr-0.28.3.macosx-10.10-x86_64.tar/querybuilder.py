...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/dbmanagr/querybuilder.py
...
def add_references(tablename, foreign_keys, joins, comment):
    for _, fk in filter(lambda (k, v): v.a.table.name == tablename and (comment is None or k in comment.display), foreign_keys.iteritems()):
        fktable = fk.b.table
...
def add_join(entity, joins):
    if entity.name not in joins.keys():
        joins[entity.name] = aliased(entity, name=('_{0}').format(entity.name))
    return joins


def replace_filter(f, table, entity, comment, search_fields):
    if isinstance(f, BitOp):
        f.children = map(lambda child: replace_filter(child, table, entity, comment, search_fields), f.children)
    elif f.lhs == '' and search_fields:
        logger.debug('Search fields: %s', search_fields)
        rhs = f.rhs
        if rhs == '' and f.operator == '*':
            rhs = '%'
        ors = OrOp(map(lambda s: QueryFilter(s, f.operator, rhs), filter(lambda s: s in entity.columns, search_fields)))
...
def add_filters(f, filters, table, joins):
    if isinstance(f, BitOp):
        op = {'OrOp': or_, 'AndOp': and_}.get(f.__class__.__name__)
        fs = []
        for c in f.children:
        ...
        logger.debug('Filter: lhs=%s, op=%s, rhs=%s', f.lhs, f.operator, f.rhs)
        if f.lhs != '':
...
def add_filter(f, filters, table, joins):
    if not f.operator:
        ...
        if '.' in f.lhs:
            ...
            return add_filter(QueryFilter(colname, f.operator, f.rhs), filters, t, joins)
        colname = f.lhs
        ...
        if allowed(tentity.columns[col.name], f.operator, f.rhs):
            op = operation(tentity.columns[col.name], f.operator, f.rhs)
            filters.append(op)
            return op
...
def simplify(table, comment, key, d):
    if comment:
        ...
        d['id'] = d[('{0}_{1}').format(comment.aliases[table.name], table.primary_key)]
    else:
        d[key] = create_title(comment, table.columns())[1].format(table.name, **d)
...
class QueryBuilder(object):
    ...
    def __init__(self, connection, table, filter_=None, order=None, limit=None, simplify=True):
    ...
    def build(self):
        ...
        if self.simplify:
            comment = create_comment(self.table, self.connection.comment(self.table.name), self.counter, self.aliases, self.alias)
            ...
            logger.debug('Search fields: %s', search_fields)
            replace_filter(self.filter, self.table, entity, comment, search_fields)
        ...
        joins = dict(filter(lambda (k, v): k != entity.original.name, joins.iteritems()))
        logger.debug('Joins: %s', joins)
