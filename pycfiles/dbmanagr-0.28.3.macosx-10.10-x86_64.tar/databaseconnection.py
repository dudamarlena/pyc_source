...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/dbmanagr/model/databaseconnection.py
...
class DatabaseConnection(BaseItem):
    ...
    def init_tables(self):
        self._tables = dict(map(lambda table: (
         table,
         Table(self.entity(table), self.autocomplete())), self.meta().tables))
    ...
    def init_comments(self):
        self._comments = dict(map(lambda k: (
         k, TableComment('')), self.tables().keys()))
...
class UriDatabaseConnection(DatabaseConnection):
    ...
    def connect(self, database):
        if database:
            try:
                self.connect_to(self.uri.format(user=self.user, password=self.password, host=self.host, database=database))
                self.database = database
            except OperationalError:
                self.connect_to(self.uri.format(user=self.user, password=self.password, host=self.host, database=''))
                database = None
