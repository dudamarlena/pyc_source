...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/dbmanagr/comment.py
...
class Comment(object):
    ...
    def __init__(self, id_, title, subtitle, order, search, display, columns, aliases):
        self.id = id_
        self.title = title
...
def update_aliases(tablename, counter, aliases, foreign_keys):
    for key in filter(lambda k: foreign_keys[k].a.table.name == tablename, foreign_keys.keys()):
        fk = foreign_keys[key]
        fktable = fk.b.table
...
def column_aliases(columns, alias):
    return dict(map(lambda col: (
     col.name, ('{{{0}_{1}}}').format(alias, col.name)), columns))
...
def find_id(comment, caliases, alias, primary_key):
    if comment.id:
        ...
        if primary_key:
            return ('{{{0}_{1}}}').format(alias, primary_key)
        return '-'
...
def create_comment(table, comment, counter, aliases, alias):
    ...
    aliases = update_aliases(table.name, counter, aliases, table.foreign_keys())
    logger.debug('Aliases: %s', aliases)
    caliases = column_aliases(table.columns(), alias)
    for _, v in filter(lambda (k, v): v.a.table.name == table.name, table.foreign_keys().iteritems()):
        caliases.update(filter(lambda (k, v): k not in caliases.keys(), column_aliases(v.b.table.columns(), aliases[v.b.table.name]).iteritems()))
    ...
    if not subtitle:
        if comment.subtitle:
            ...
            d = dict(map(lambda k: (k.name, k.name), table.columns()))
            logger.debug('search.add(subtitle.format(**d)=%s)', subtitle.format(**d))
            search.add(subtitle.format(**d))
            subtitle = ('{0} (id={1})').format(subtitle.format(**caliases), id_)
    if comment.order:
        order = comment.order
    else:
        order = []
    ...
    return Comment(id_, title, subtitle, order, search, display, columns, aliases)