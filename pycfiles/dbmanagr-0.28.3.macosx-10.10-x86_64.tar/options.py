...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/dbmanagr/options.py
...
def restriction(alias, column, operator, value, map_null_operator=True):
    ...
    if column.tablename and alias is not None:
        return ('{0}.{1} {2} {3}').format(alias, escape_keyword(column.name), operator, format_value(column, value))
    else:
        return ('{0} {1} {2}').format(escape_keyword(column.name), operator, format_value(column, value))
...
def format_value(column, value):
    ...
    if type(value) is list:
        return ('({0})').format((', ').join([ format_value(column, v) for v in value ]))
    else:
        ...
        if isinstance(column.type, Boolean) and (type(value) is bool or value in ('true',
                                                                                  'false')) or type(value) is bool:
        ...
        return "'%s'" % escape_statement(value).replace("'", "''")
...
class FileOptionsParser(OptionsParser):
    ...
    def parse(self, source):
        ...
        if driver.uri:
            ...
            driver.show = {1: 'connections', 
               2: 'tables', 
               3: 'columns', 
               4: 'values'}.get(driver.show_code, 'connections')
        return driver
    ...
    def parse(self, source):
        ...
        if driver.uri:
            ...
            driver.show = {1: 'connections', 
               ...
               3: 'tables', 
               4: 'columns', 
               5: 'values'}.get(driver.show_code, 'connections')
