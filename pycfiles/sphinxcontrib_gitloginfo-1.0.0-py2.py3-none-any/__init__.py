...
# [GCC 8.4.0]
# Embedded file name: /home/marble/Repositories/github.com/TYPO3-Documentation/sphinxcontrib-gitloginfo/sphinxcontrib/gitloginfo/__init__.py
# Compiled at: 2020-05-08 03:27:36
"""
...
def _html_page_context(app, pagename, templatename, context, doctree):
    if 'sourcename' in context and 'page_source_suffix' in context:
        ...
        return
    pagename_as_repofile = ospj(wd['project_offset'], wd['t3docdir'], pagename + context['page_source_suffix'])
    ...
    if timestamp is None:
        ...
        last_modified = i18n.format_date(wd['html_last_updated_fmt'], date=last_modified_dt, language=app.config.language)
        commit_url_template = context.get('theme_project_commit_url')
        if not commit_url_template:
            repo_url = context.get('theme_project_repository') or context.get('theme_project_issues')
            if repo_url:
...
def setup(app):
    ...
    if ospe(gitloginfo_jsonfile):
        ...
        wd['project_offset'] = wd['gitloginfo']['abspath_to_project'][len(wd['gitloginfo']['abspath_to_repo']):].strip('/')
        wd['t3docdir'] = wd.get('buildsettings', {}).get('t3docdir', 'Documentation')
    else:
