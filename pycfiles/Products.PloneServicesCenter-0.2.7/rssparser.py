...
if hasattr(socket, 'setdefaulttimeout'):
    ...
    try:
        ...
        def isWellFormed(data):
            if _debug:
                sys.stderr.write('entering isWellFormed\n')
            source = InputSource()
            ...
            parser.parse(source)
            if _debug:
                sys.stderr.write('leaving isWellFormed\n')
    ...
    class FeedParser(sgmllib.SGMLParser):
        namespaces = {'http://backend.userland.com/rss': '', 'http://blogs.law.harvard.edu/tech/rss': '', 
        ...
        can_be_relative_uri = [
         'link', 'id', 'wfw_comment', 'wfw_commentRSS', 'docs', 'url', 'comments']
        ...
        def unknown_starttag(self, tag, attrs):
            ...
            attrs = [ (k.lower(),
             sgmllib.charref.sub(lambda m: chr(int(m.groups()[0])), v).strip())
             for (k, v) in attrs ]
        ...
        def pop(self, element):
            ...
            output = output or ''
            if element in self.explicitly_set_type and self.contentparams.get('type') in self.html_types or element not in self.explicitly_set_type:
        ...
        def _start_content(self, attrs):
            attrsD = dict(attrs)
            self.incontent += 1
            self.contentparams = {'mode': attrsD.get('mode', 'xml'), 'type': attrsD.get('type', 'text/plain'), 
        ...
        def _start_body(self, attrs):
            attrsD = dict(attrs)
            self.incontent += 1
            self.contentparams = {'mode': 'xml', 'type': 'application/xhtml+xml', 
               'language': attrsD.get('xml:lang', None), 
               'base': attrsD.get('xml:base', self.baseuri)}
            self.push('content', 1)
            return
        ...
        def _start_content_encoded(self, attrs):
            attrsD = dict(attrs)
            self.incontent += 1
            self.contentparams = {'mode': 'escaped', 'type': 'text/html', 
               'language': attrsD.get('xml:lang', None), 
               'base': attrsD.get('xml:base', self.baseuri)}
            self.push('content', 1)
            return
    ...
    class BaseHTMLProcessor(sgmllib.SGMLParser):
        ...
        def normalize_attrs(self, attrs):
            attrs = [ (k.lower(),
             sgmllib.charref.sub(lambda m: unichr(int(m.groups()[0])), v).strip())
             for (k, v) in attrs ]
            attrs = [ (k, k in ('rel', 'type') and v.lower() or v) for (k, v) in attrs ]
            return attrs
...
iso8601_tmpl = [
 ...
 '---DD',
 'CC', '']
iso8601_re = [ tmpl.replace('YYYY', '(?P<year>\\d{4})').replace('YY', '(?P<year>\\d\\d)').replace('MM', '(?P<month>[01]\\d)').replace('DD', '(?P<day>[0123]\\d)').replace('OOO', '(?P<ordinal>[0123]\\d\\d)').replace('CC', '(?P<century>\\d\\d$)') + '(T?(?P<hour>\\d{2}):(?P<minute>\\d{2})' + '(:(?P<second>\\d{2}))?' + '(?P<tz>[+-](?P<tzhour>\\d{2})(:(?P<tzmin>\\d{2}))?|Z)?)?' for tmpl in iso8601_tmpl
             ]
...
DATETIME_SUITE = (
 ...
 (
  ...
  time.gmtime()[0:3] + (17, 41, 0) + time.gmtime()[-3:]),
 (
