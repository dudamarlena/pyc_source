...
# Embedded file name: build\bdist.win32\egg\pyivi\ivic\ivifgen.py
# Compiled at: 2013-08-18 06:15:38
from pyivi.ivifactory import register_wrapper
from pyivi.ivic import IviCWrapper
from pyivi.ivic.ivicwrapper import RepeatedCapability, add_repeated_capability
from pyivi.common import add_sc_fields_enum, ShortCut, add_sc_fields
...
class ShortCutFreqGen(ShortCut):
    _fields = [
     ('create_arb_waveform', 'create_arb_waveform'),
...
add_sc_fields(ShortCutFreqGen, ShortCutFreqGen._channel_related_fields, 'sc_active_channel')
...
class IviCFGen(IviCWrapper):
    ...
    def create_arb_waveform(self, waveform):
        ...
        handle = ctypes.c_int()
        self.call('CreateArbWaveform', self.visession, length, c_array, ctypes.byref(handle))
        return handle.value
