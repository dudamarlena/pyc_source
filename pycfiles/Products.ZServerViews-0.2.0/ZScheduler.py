...
# Embedded file name: build/bdist.linux-x86_64/egg/Products/ZScheduler/ZScheduler.py
...
class ZScheduler(PropertyManager, LogSupport):
    ...
    _properties = (
     {'id': 'timezone', 'type': 'selection', 'mode': 'w', 'select_variable': 'timezones'},
     {'id': 'callables', 'type': 'multiple selection', 'mode': 'w', 'select_variable': 'products'},
     {'id': 'timer_type', 'type': 'selection', 'mode': 'w', 'select_variable': 'timers'}, {'id': 'queue_batch_size', 'type': 'int', 'mode': 'w'}) + LogSupport._properties
    __ac_permissions__ = (
     (
      access_contents_information, ('isActive', 'isScheduled', 'nextEvent')), (view_management_screens, ('manage_timer', 'manage_log', 'manage_catalog', 'timezones', 'products', 'timers',
 'logger', 'searchResults', 'events')), (change_configuration, ('manage_delete', 'manage_unschedule', 'manage_reload', 'manage_restartTimer', 'manage_refresh',
 'manage_schedule', 'schedule', 'unschedule'))) + PropertyManager.__ac_permissions__ + LogSupport.__ac_permissions__
    manage_options = (
     {'label': 'Queue', 'action': 'manage_main', 'help': ('ZScheduler', 'queue.stx')}, {'label': 'Catalog', 'action': 'manage_catalog'},
     {'label': 'Timer', 'action': 'manage_timer', 'help': ('ZScheduler', 'timer.stx')},
     {'label': 'Properties', 'action': 'manage_propertiesForm', 'help': ('ZScheduler', 'props.stx')}) + LogSupport.manage_options
    ...
    def products(self):
        ...
        results = filter(lambda x: x not in ('ZScheduleEvent', ), map(lambda x: x['name'], Products.meta_types))
        results.sort()
        return results
    ...
    def isScheduled(self, event):
        ...
        """
        brainz = self.queue.searchResults(getPhysicalPath=('/').join(event.getPhysicalPath()), active=True)
        return len(brainz) == 1
    ...
    def events(self, REQUEST={}, **kw):
        ...
        if domain.find(':') != -1:
            ...
            os.environ['SERVER_URL'] = '%s://%s:%s' % (proto, server_name, server_port)
            os.environ['REQUEST_METHOD'] = 'GET'
            request = HTTPRequest(devnull, os.environ, resp)
            for brainz in self.searchResults(REQUEST, **kw):
                try:
                    events.append(brainz.getObject())
                except:
                    continue
...
def delZScheduleEvent(ob, event):
    ...
    if zt:
        zt.unschedule(ob)
        zt.queue.delete(('/').join(ob.getPhysicalPath()))
    return
