...
# Embedded file name: build/bdist.linux-x86_64/egg/Products/Zpydoc/Zpydoc.py
...
try:
    ...
    class PortalContent(SimpleItem):
        pass
...
class ModuleTree(dict):
    ...
    def keys(self, name='', depth=0):
        if name:
            return filter(lambda x, n=name, d=depth + name.count('.'): x.startswith(n) and x.count('.') == d, dict.keys(self))
        else:
...
class Zpydoc(Folder):
    ...
    _properties = Folder._properties + ({'id': 'description', 'type': 'text', 'mode': 'w'}, {'id': 'show_data', 'type': 'boolean', 'mode': 'w'}, {'id': 'expert_mode', 'type': 'boolean', 'mode': 'w'})
    __ac_permissions__ = Folder.__ac_permissions__ + (
     (
      view,
      ('render', 'module_permitted', 'module_role', 'module_roles', 'getDocumentableNames',
 'isBuiltin', 'isPackage', 'isModule', 'getDocumentables', 'getModule')),
     (
     ...
     (
      manage_zpydoc,
      ('pythonpaths', 'modules', 'module_tree', 'manage_updateModulePermissions')))
    manage_options = ({'label': 'Contents', 'action': 'manage_main', 'help': ('Zpydoc', 'pydoc.stx')}, {'label': 'View', 'action': ''}, {'label': 'Modules', 'action': 'manage_modules'}, {'label': 'Renderers', 'action': 'renderers/manage_workspace'}) + Folder.manage_options[2:-2]
    ...
    def all_meta_types(self):
        ...
        if self.expert_mode:
            return [
             {'action': 'manage_addProduct/Zpydoc/add_documentable', 'permission': manage_zpydoc, 
    ...
    def module_tree(self, **kw):
        ...
        m = modules_tree.keys(name=kw.get('name', ''), depth=kw.get('depth', 0))
        m.sort()
    ...
    def modules(self):
        ...
        global all_modules
        if not all_modules:
            all_modules = map(lambda x: x.__name__, filter(lambda x: hasattr(x, '__path__'), sys.modules.values())) + list(sys.builtin_module_names)
        return all_modules
    ...
    def getDocumentableNames(self):
        ...
        """
        names = filter(lambda x: x.find('.') == -1, self._module_permissions.keys())
    ...
    def getDocumentables(self):
        ...
        """
        return map(lambda x, p=self.isPackage, m=self.isModule: (x, p(x), m(x)), self.getDocumentableNames())
    ...
    def _get_documentable(self, obj, do_permissions=1):
        ...
        while not zpydoc and directory and directory != '/':
            try:
                zpydoc = filter(lambda x, y=directory: x.directory == y, self.objectValues('ZpyDocumentable'))[0]
            except:
