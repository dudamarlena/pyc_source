...
# [GCC 8.4.0]
# Embedded file name: /home/prometeo/cosas/avython/avython/gitautotag/gitautotag.py
# Compiled at: 2017-07-06 16:10:40
# Size of source mod 2**32: 4666 bytes
...
class GitAutotag(object):
    _GitAutotag__revision_list = [
     'mayor', 'minor', 'patch']
    _GitAutotag__list_tags = False
    ...
    def __init__(self, *args, **kwargs):
        ...
        parser.add_argument('-r', '--revision', choices=(self._GitAutotag__revision_list), help=('Revision of release. Choices: {}'.format(str(self._GitAutotag__revision_list))))
    ...
    def parse_commandline(self):
        ...
        self.revision = self.args.revision if self.args.revision else self._GitAutotag__revision_list[2]
        self.force = self.args.force
        self._GitAutotag__push = self.args.push
    ...
    def list_tags(self, cached=True):
        if not self._GitAutotag__list_tags or cached is False:
            self._GitAutotag__list_tags = self.run_parse('for-each-ref', 'refs/tags/' + self.version_pattern, '--sort=taggerdate', '--format=%(tag)')
        return self._GitAutotag__list_tags

    def last_tag(self):
        if self.list_tags():
            return self.list_tags()[(-1)]
        else:
            return False
    ...
    def create_first_version(self):
        return ['0' for _ in range(len(self._GitAutotag__revision_list))]
    ...
    def increase_version_number(self):
        if self.exist_changes() or self.force is True:
            ...
            index_of_revision = self._GitAutotag__revision_list.index(self.revision)
            try:
