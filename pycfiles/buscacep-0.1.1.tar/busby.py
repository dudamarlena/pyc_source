...
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/busby/busby.py
# Compiled at: 2014-07-22 11:33:23
...
def parse_rows(delimiter=',', quotechar='"'):
    filereader = csv.DictReader(sys.stdin, delimiter=delimiter, quotechar=quotechar)
...
def chunk_by_chunk(in_filename, out_filename, endpoint, chunk_size=1000, delimiter=',', quotechar='"'):
    try:
        num_lines = float(sum(1 for line in open(in_filename)))
        filereader = csv.DictReader(open(in_filename, 'r'), delimiter=delimiter, quotechar=quotechar)
            ...
            if i >= chunk_size:
                ...
                if 'result' in response:
                    ...
                    if filewriter is None:
                        filewriter = csv.DictWriter(open(out_filename, 'w'), resp_header)
                        filewriter.writeheader()
                    response = map(lambda vals: dict(zip(resp_header, vals)), zip(*response.values()))
...
def row_by_row(in_filename, out_filename, endpoint, delimiter=',', quotechar='"'):
    perc_done = 0.0
    perc_step = 0.05
    try:
        num_lines = float(sum(1 for line in open(in_filename)))
    ...
    try:
        for i, row in enumerate(filereader):
            ...
            if 'result' in response:
                response = response['result']
                if filewriter is None:
                    filewriter = csv.DictWriter(open(out_filename, 'w'), response.keys())
                    filewriter.writeheader()
