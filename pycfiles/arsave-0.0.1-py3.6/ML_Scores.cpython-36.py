...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/arsave/ML_Scores.py
# Compiled at: 2018-04-21 17:52:00
...
class ML_Scores:
    ...
    def __init__(self, model_name, model, parameters='Default', training_score=None, validation_score=None, description=None, other_metric=None):
        """
        _init_
        ...
        if not type(model_name) == str:
            ...
            if not type(model) == str:
                raise AssertionError
            elif not type(parameters) == str:
                raise AssertionError
            else:
                if training_score != None:
                    if not type(training_score) == float:
                        raise AssertionError
                if validation_score != None:
                    assert type(validation_score) == float
            if description != None:
                assert type(description) == str
                assert other_metric != None and type(other_metric) == dict
                self.new_columns = []
                self.dic_columns = {'Model_Name':[model_name],  'Model':[model],  'Parameters':[parameters],  'Training_Score':[
                  training_score], 
                 'Validation_Score':[validation_score],  'Description':[
                  description]}
    ...
    def add(self, model_name, model, parameters='Default', training_score=None, validation_score=None, description=None, other_metric=None):
        """
        add
        ...
        if model_name in list_models:
            ...
            if description != None:
                assert type(description) == str
                assert other_metric != None and type(other_metric) == dict
                new_row = {'Model_Name':model_name,  'Model':model,  'Parameters':parameters,  'Training_Score':training_score, 
                 'Validation_Score':validation_score,  'Description':description}
                for key, value in other_metric.items():
                    new_row[key] = value

            else:
                new_row = {'Model_Name':model_name, 
                 'Model':model,  'Parameters':parameters,  'Training_Score':training_score, 
                 'Validation_Score':validation_score,  'Description':description}
    ...
    def get_storage(self):
        ...
        """
        columns = [
         'Model', 'Parameters', 'Training_Score', 'Validation_Score', 'Description']
