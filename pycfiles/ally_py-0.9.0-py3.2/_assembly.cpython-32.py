...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ally/container/_impl/_assembly.py
...
class Assembly:
    ...
    def current(cls):
        ...
        """
        if not cls.stack:
            raise SetupError('There is no active assembly to process on')
        return cls.stack[(-1)]
    ...
    def fetchForName(self, name):
        ...
        if not call:
            raise SetupError("No IoC resource for name '%s', possible reasons:\n\t- the setup function you are calling is not registered in the current assembly\n\t- you have added an 'after' or 'before' event to a setup function that is from a different assembly\n\t  and when the event is triggered the setup function assembly is no longer available" % name)
        if not callable(call):
            raise SetupError("Invalid call %s for name '%s'" % (call, name))
        return call
    ...
    def processForName(self, name):
        ...
        try:
            value = self.fetchForName(name)()
        except (SetupError, ConfigError, SystemExit):
            raise
        except:
            raise SetupError("Exception occurred for %r in processing chain '%s'" % (
...
class Context:
    ...
    def assemble(self, assembly):
        ...
        for module in self._modules:
            setups.extend(module.__ally_setups__)
