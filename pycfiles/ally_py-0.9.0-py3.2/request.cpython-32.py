...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/admin/introspection/impl/request.py
...
from ally.core.spec.resources import Node, ConverterPath, Invoker, InvokerInfo, INodeChildListener, INodeInvokerListener, Path
...
class RequestService(IRequestService, INodeChildListener, INodeInvokerListener):
    ...
    def getRequest(self, id):
        ...
        if id not in self._requests:
            raise InputError(Ref(_('Invalid request id'), ref=Request.Id))
    ...
    def getMethod(self, id):
        ...
        """
        self._refresh()
        if id not in self._methods:
            raise InputError(Ref(_('Invalid method id'), ref=Method.Id))
    ...
    def getAllInputs(self, id):
        ...
        if id not in self._patternInputs:
            raise InputError(Ref(_('Invalid request id'), ref=Request.Id))
    ...
    def getAllRequests(self, offset=None, limit=None):
        ...
        """
        self._refresh()
        return IterPart(trimIter(self._requests.values(), len(self._requests), offset, limit), len(self._requests), offset, limit)
    ...
    def _refresh(self):
        ...
        if self._reset:
            ...
            for path in iteratePaths(self.resourcesRoot):
                self._process(path, replacer)
    ...
    def _process(self, path, replacer):
        ...
        if node in self._nodeRequests:
            return
        if not (node.get or node.delete or node.insert or node.update):
            return
        r = Request()
        ...
        self._requestId += 1
        if node.get:
            m = self._toMethod(node.get, r)
            requestMethods.add(m.Id)
            r.Get = m.Id
        if node.delete:
            m = self._toMethod(node.delete, r)
            requestMethods.add(m.Id)
            r.Delete = m.Id
        if node.insert:
            m = self._toMethod(node.insert, r)
            requestMethods.add(m.Id)
            r.Insert = m.Id
        if node.update:
            m = self._toMethod(node.update, r)
            requestMethods.add(m.Id)
            r.Update = m.Id
    ...
    def _toPatternInput(self, match, req):
        ...
        if isinstance(match, MatchProperty):
            ...
            inp.Description = _('The %(type)s of %(model)s %(description)s') % dict(type=_(typ.property), model=_(typ.container.name), description=re.sub('[\\s]+', ' ', getdoc(typ.parent.clazz) or '...'))
        else:
