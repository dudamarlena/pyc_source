...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ally/design/processor/attribute.py
...
"""
from .spec import IAttribute, AttrError, IResolver, Resolvers, ResolverError, ContextMetaClass
...
class Definition:
    ...
    def __get__(self, obj, owner=None):
        ...
        if not owner is None:
            assert owner == self.__objclass__, 'Invalid owner class %s expected %s' % (owner, self.__objclass__)
        return self
...
class Descriptor:
    ...
    def __init__(self, clazz, name, types):
        ...
        for typeClazz in types:
            if not isclass(typeClazz):
                raise AssertionError('Invalid type class %s' % typeClazz)
...
class Attribute(IAttribute):
    ...
    def push(self, name, resolvers):
        ...
        """
        if self._clazz is None:
            raise AttrError('Attribute is not placed, so no definition class is available')
            ...
            if not isinstance(clazz, ContextMetaClass):
                ...
                if other is None:
                    if self.status != REQUIRED:
                        return True
                    return False
                return isinstance(other, Attribute) or False
            assert isinstance(other, Attribute)
            for typ in self.types:
                if typ in other.types:
                    break
            else:
                return False
...
class Resolver(IResolver):
    ...
    """
    __slots__ = ('status', 'types', 'nameContext', 'nameAttribute', 'doc', 'defined',
                 'usedIn')
    ...
    def __init__(self, nameContext, nameAttribute, status, types, doc, defined=None):
        ...
        if defined is None:
            if status & DEFINED:
                ...
                defined = ()
        elif not isinstance(defined, Iterable):
            raise AssertionError('Invalid defined classes %s' % defined)
    ...
    def merge(self, other, isFirst=True):
        ...
        if self is other:
            ...
            if self.status == REQUIRED:
                ...
                if not types:
                    raise AttrError('Incompatible required types of %s, with required types of %s' % (self, other))
            else:
                if self.status == OPTIONAL:
                    ...
                    if other.status & DEFINED:
                        ...
                        if self.status & OPTIONAL and other.status & OPTIONAL:
                            status |= OPTIONAL
                    else:
    ...
    def solve(self, other):
        ...
        if self is other:
            return self
        if self.status & DEFINED:
            resolver = self.merge(other, True)
        else:
            resolver = other.merge(self, True)
        return resolver
    ...
    def create(self, attributes):
        ...
        if self.status == REQUIRED:
            raise AttrError('Resolver %s\n, cannot generate attribute' % self)
        if self.status & OPTIONAL:
            return
        if self.nameAttribute in attributes:
            return
