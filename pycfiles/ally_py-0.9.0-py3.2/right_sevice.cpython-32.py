...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/acl/right_sevice.py
...
class Alternate:
    ...
    def add(self, forRef, theRef):
        ...
        key = (
         forService, forService.service.calls[forName])
        alternates = self._alternates.get(key)
        if alternates is None:
            alternates = self._alternates[key] = set()
...
class StructureRight:
    ...
    def obtainMethod(self, call):
        ...
        if not structMethod:
            structMethod = self.methods[call.method] = StructMethod()
        return structMethod
...
class StructMethod:
    ...
    def obtainService(self, serviceType):
        ...
        if not structService:
            structService = self.services[serviceType] = StructService()
        return structService
...
class StructService:
    ...
    def obtainCall(self, serviceType, call):
        ...
        if not structCall:
            structCall = self.calls[call.name] = StructCall(serviceType, call)
        return structCall
...
class StructCall:
    ...
    def pushFilter(self, filter):
        ...
        if filterOld:
            ...
            if filter.priority > filterOld.priority:
                self.filters[filter.resource] = filter
        else:
            self.filters[filter.resource] = filter