...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ally/xml/digester.py
...
class Digester(ContentHandler):
    ...
    def parseInputSource(self, source):
        ...
        @return: object
            The object obtained from parsing.
        """
        assert isinstance(source, InputSource), 'Invalid input source %s' % source
        del self.errors[:]
    ...
    def _processEnd(self, nodes, name):
        ...
        if nodes:
            for node in nodes:
                assert isinstance(node, Node)
                for rule in reversed(node.rules):
                    assert isinstance(rule, Rule)
                    rule.end(node, self)
                    processed |= True
...
class DigesterXMLUpdate(Digester, XMLGenerator):
    ...
    def __init__(self, root, out, encoding='UTF-8', acceptAttributes=True, acceptUnknownTags=True, shortEmptyElements=True):
        """
