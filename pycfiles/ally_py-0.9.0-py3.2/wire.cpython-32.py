...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ally/container/wire.py
...
from ._impl._setup import normalizeConfigType, setupsOf, SetupConfig, register, setupFirstOf
...
def entity(name, type=None):
    ...
    if not type:
        if name not in locals:
            raise WireError('Invalid entity name %r, cannot find it in locals' % name)
        type = locals[name]
    if not isclass(type):
...
def config(name, type=None, doc=None):
    ...
    assert isinstance(name, str), 'Invalid attribute name %s' % name
    if not not doc:
        assert isinstance(doc, str), 'Invalid description %s' % doc
        if not name.islower():
...
def createWirings(clazz, target, group, registry, nameEntity, nameInEntity):
    ...
    if not wiring:
        ...
        if not wire:
            wire = register(SetupEntityWire(group), registry)
...
def wrapperWiredConfiguration(clazz, wconfig):
    ...
    if value and not isclass(value):
        return deepcopy(value)
    else:
        if wconfig.hasValue:
            return deepcopy(wconfig.value)
