...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ally/support/sqlalchemy/util_service.py
...
"""
from ally.api.criteria import AsLike, AsOrdered, AsBoolean, AsEqual, AsDate, AsTime, AsDateTime, AsRange
        ...
        for criteria in exclude:
            if isinstance(criteria, str):
                ...
                if not column is not None:
                    raise AssertionError("Invalid exclude criteria name '%s' for query class %s" % (criteria, clazz))
            else:
                typ = typeFor(criteria)
                assert isinstance(typ, TypeCriteriaEntry), 'Invalid exclude criteria %s' % criteria
                column = columns.pop(typ.name, None)
                if not column is not None:
                    raise AssertionError("Invalid exclude criteria '%s' for query class %s" % (criteria, clazz))
