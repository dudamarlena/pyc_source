...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ally/gateway/http/impl/processor/captcha_validator.py
...
from ally.http.spec.server import HTTP, RequestHTTP, RequestContentHTTP, ResponseHTTP, ResponseContentHTTP, HTTP_POST, IDecoderHeader
...
class GatewayCaptchaValidationHandler(HandlerBranchingProceed):
    ...
    def __init__(self):
        ...
        super().__init__(Using(self.assembly, request=RequestHTTP, requestCnt=RequestContentHTTP, response=ResponseHTTP, responseCnt=ResponseContentHTTP))

    def process(self, processing, request: Request, response: Response, responseCnt: ResponseContent, Gateway: Gateway, Match: Match, **keyargs):
        """
    ...
    def checkCaptcha(self, processing, clientIP, challenge, resolve):
        ...
        chain.process(request=request, requestCnt=requestCnt, response=processing.ctx.response(), responseCnt=processing.ctx.responseCnt()).doAll()
