...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ally/http/impl/processor/forward.py
    ...
    def process(self, request: Request, requestCnt: RequestContent, response: Response, responseCnt: ResponseContent, **keyargs):
        ...
        if requestCnt.source is not None:
            if isinstance(requestCnt.source, Iterable):
                source = BytesIO()
                for bytes in requestCnt.source:
                    source.write(bytes)
        ...
        if request.parameters:
            parameters = urlencode(request.parameters)
        else:
            parameters = None
