...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ally/http/impl/processor/headers/content_length.py
...
class ContentLengthDecodeHandler(HandlerProcessorProceed):
    ...
    def process(self, request: RequestDecode, requestCnt: RequestContentDecode, response: ResponseDecode, **keyargs):
        ...
        if value:
            try:
                requestCnt.length = int(value)
            except ValueError:
                if response.isSuccess is False:
                    return
                else:
                    response.code, response.status, response.isSuccess = CONTENT_LENGHT_ERROR
                    response.text = "Invalid value '%s' for header '%s', expected an integer value" % (
                     value, self.nameContentLength)
                    return
            else:
