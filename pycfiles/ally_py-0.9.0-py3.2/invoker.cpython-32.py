...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ally/core/impl/invoker.py
...
class InvokerFunction(Invoker):
    ...
    def __init__(self, method, function, output, inputs, hints, name=None, infoIMPL=None):
        ...
        if infoIMPL is None:
            infoIMPL = InvokerInfo(name, function.__code__.co_filename, function.__code__.co_firstlineno, getdoc(function))
        elif not isinstance(infoIMPL, InvokerInfo):
            raise AssertionError('Invalid invoker information %s' % infoIMPL)
...
class InvokerRestructuring(Invoker):
    ...
    def __init__(self, invoker, inputs, indexes, indexesSetValue={}):
        ...
        for index in indexes:
            ...
            if not (index >= 0 and index < len(inputs)):
                raise AssertionError('Index out of inputs range %s' % index)
        ...
        for index, toSet in indexesSetValue.items():
            ...
            for prop, fromIndex in toSet.items():
                ...
                if not (fromIndex >= 0 and fromIndex < len(inputs)):
                    raise AssertionError('Index out of inputs range %s' % fromIndex)

        self.invoker = invoker
        self.indexes = indexes
        self.indexesSetValue = indexesSetValue
        super().__init__(invoker.name, invoker.method, invoker.output, inputs, invoker.hints, invoker.infoIMPL, invoker.infoAPI)
    ...
    def invoke(self, *args):
        ...
        for index in self.indexes:
            if index < lenArgs:
                ...
                if not inp.hasDefault:
                    raise DevelError("No value available for '%s' for %s" % (inp.name, self))
                value = inp.default
            wargs.append(value)
        ...
        for index, toSet in self.indexesSetValue.items():
            ...
            for prop, fromIndex in toSet.items():
                ...
                if val is None:
                    setattr(obj, prop, arg)
                elif val != arg:
                    raise DevelError("Cannot set value %s for '%s', expected value %s" % (val, prop, arg))
                    continue
