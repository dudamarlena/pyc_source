...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/__unit_test__/ally/support/sqlalchemy/mapper.py
...
from ally.support.sqlalchemy.mapper import mapperSimple, validate, DeclarativeMetaModel
...
tableParent = Table('user_parent', meta, Column('id', INTEGER(unsigned=True), primary_key=True, key='Id'), Column('name', String(20), nullable=False, unique=True, key='Name'))
tableUserParent = Table('user_with_parent', meta, Column('fk_user_id', INTEGER(unsigned=True), ForeignKey(UserMapped.userId), primary_key=True, key='Id'), Column('fk_parent_id', INTEGER(unsigned=True), ForeignKey(tableParent.c.Id), key='Parent'))
