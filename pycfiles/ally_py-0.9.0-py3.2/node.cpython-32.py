...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ally/core/impl/node.py
...
class MatchProperty(Match):
    ...
    def toPath(self, converterPath, isFirst, isLast):
        ...
        """
        assert isinstance(converterPath, ConverterPath)
        assert self.value is not None, 'Cannot represent the path element for %s because there is no value' % self.type
...
class NodePath(Node):
    ...
    def correspondentIn(self, nodes):
        ...
        """
        for node in nodes:
            if isinstance(node, NodePath) and self.name == node.name:
                return True
...
class NodeProperty(Node):
    ...
    def __init__(self, parent, inp):
        ...
        if not inp.type.isOf(int):
            assert inp.type.isOf(str), 'Invalid input type %s' % inp.type
        self.inputs = set((inp,))
    ...
    def correspondentIn(self, nodes):
        ...
        """
        for node in nodes:
            if isinstance(node, self.__class__) and self.type == node.type:
                return True
    ...
    def isFor(self, inp):
        ...
        """
        assert isinstance(inp, Input), 'Invalid input %s' % inp
        if not isinstance(inp.type, TypeModelProperty):
            return False
        if not (inp.type.isOf(int) or inp.type.isOf(str)):
            return False
