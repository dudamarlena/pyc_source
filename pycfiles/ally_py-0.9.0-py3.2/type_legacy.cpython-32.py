...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ally/type_legacy.py
...
try:
    ...
    class Iterator(Iterable):
        ...
        @classmethod
        def __subclasshook__(cls, C):
            if cls is Iterator and any('__next__' in B.__dict__ for B in C.__mro__):
                if any('__iter__' in B.__dict__ for B in C.__mro__):
                    return True
            return NotImplemented
...
if not isinstance(0, Number):
    try:
        import __builtin__ as builtins
    except ImportError:
        import builtins

    unicode = getattr(builtins, 'unicode', False)
    isinstanceOrig = isinstance

    def isinstance(obj, clas):
        if not isinstanceOrig(obj, clas):
            checkHook = getattr(clas, '__subclasshook__', None)
            clazz = getattr(obj, '__class__', None)
            if checkHook and clazz:
                return checkHook(clazz)
            if unicode and str is clas:
                return isinstanceOrig(obj, unicode)
            return False
        else:
            return True
