...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ally/support/util_io.py
...
class ReplaceInFile:
    ...
    """
    __slots__ = [
     '_fileObj', '_replacements', '_maxKey', '_leftOver']
...
def synchronizeURIToDir(path, dirPath):
    ...
    if not isdir(path):
        ...
        for zipInfo in zipFile.filelist:
            ...
            if zipInfo.filename.startswith(inDirPath):
                if zipInfo.filename[0] == '/':
                    dest = zipInfo.filename[1:]
                else:
                    dest = zipInfo.filename
                dest = normpath(join(dirPath, dest[lenPath:]))
                if exists(dest) and zipTime <= datetime.fromtimestamp(stat(dest).st_mtime):
                    continue
    ...
    for root, _dirs, files in os.walk(path):
        for file in files:
            ...
            if exists(dest) and datetime.fromtimestamp(stat(src).st_mtime) <= datetime.fromtimestamp(stat(dest).st_mtime):
                continue
            destDir = dirname(dest)
            if not exists(destDir):
                makedirs(destDir)
