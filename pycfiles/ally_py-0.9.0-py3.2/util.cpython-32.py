...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ally/support/util.py
...
class Singletone:
    ...
    def __new__(cls):
        ...
        try:
            ...
            cls._ally_singletone = super().__new__(cls)
...
class Referencer:
    ...
    def __getattr__(self, name):
        ...
        if not isfunction(function):
            raise AttributeError("Invalid function name '%s'" % name)
        return (
         self._ally_referencer_class, name)
...
def ref(clazz):
    ...
    if references is None:
        references = globals()['_ally_referenceres'] = WeakKeyDictionary()
    referencer = references.get(clazz)
    if referencer is None:
        referencer = references[clazz] = Referencer(clazz)
    return referencer
...
def iterRefClass(refsClass):
    ...
    if not classes.isdisjoint(indexed):
        for clazz, ref in indexed.items():
            if clazz in classes:
                raise ValueError('Cannot have also reference and also a simple class for %s, please either remove reference %s or the class' % (
                 clazz, ref))
                continue
...
class immut(dict):
    ...
    def __new__(cls, *args, **keyargs):
        if not (args or keyargs):
            try:
                ...
                cls.__empty__ = dict.__new__(cls)
    ...
    def __hash__(self):
        ...
        """
        try:
            return self._immut__hash__value
        except AttributeError:
            self._immut__hash__value = hash(tuple(p for p in self.items()))
