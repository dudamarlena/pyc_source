...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ally/core/spec/transform/support.py
...
def getterChain(*getters):
    ...
    for getter in getters:
        if not callable(getter):
            raise AssertionError('Invalid getter %s' % getter)
...
def setterToOthers(*setters):
    ...
    for setter in setters:
        if not callable(setter):
            raise AssertionError('Invalid setter %s' % setter)
