...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/internationalization/scanner.py
...
from babel.messages.extract import extract_nothing, extract_python, _strip_comment_tags, empty_msgid_warning, extract_javascript
...
from internationalization.api.source import ISourceService, TYPES, Source, QSource
...
METHOD_MAP = [
 ('**.py', 'python'), ('**.js', 'javascript'), ('**.html', 'html'), ('**.dust', 'html')]
    ...
    for name in names:
        for pattern, method in METHOD_MAP:
            if pathmatch(pattern, name):
                ...
                yield (
                 filePath, method, process(openZip, method))
                continue
        ...
        for fname, lineno, message, comments in extract(method, TextIOWrapper(fileObj, encoding='UTF-8')):
            if fname in ('pgettext', 'C_', 'NC_'):
                ...
                if fname == 'npgettext':
                    ...
                    cntxt = None
            if not log.debug('%s (%s) #%s' % (message, cntxt, comments)):
                assert True
                yield (
                 message, cntxt, lineno, comments)
