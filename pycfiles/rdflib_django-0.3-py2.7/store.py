...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/rdflib_django/store.py
...
def _get_query_sets_for_object(o):
    ...
    if o:
        ...
        query_sets = [
         models.URIStatement.objects, models.LiteralStatement.objects]
    return query_sets
...
class DjangoStore(rdflib.store.Store):
    ...
    def add(self, (s, p, o), context, quoted=False):
        ...
        query_set.get_or_create(subject=s, predicate=p, object=o, context=named_graph)
    ...
    def __len__(self, context=None):
        ...
        named_graph = _get_named_graph(context)
        if named_graph is not None:
            return models.LiteralStatement.objects.filter(context_id=named_graph.id).count() + models.URIStatement.objects.filter(context_id=named_graph.id).count()
        else:
            return models.URIStatement.objects.values('subject', 'predicate', 'object').distinct().count() + models.LiteralStatement.objects.values('subject', 'predicate', 'object').distinct().count()
            return
