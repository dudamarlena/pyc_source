...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/bta/ntds2db.py
# Compiled at: 2015-10-20 16:27:01
import libesedb, bta.tools.progressbar, bta.tools.RPNedit, bta.tools.ask, bta.backend.mongo, bta.postprocessing, bta.dblog, logging
...
class ESETable(object):
    ...
    def parse_file(self, dbtable):
        ...
        pbar = self.options.progress_bar(total, desc='Importing [%s.%s]' % (dbtable.db.name, self._tablename_), step=100, obj='rec')
        next(pbar)
        try:
...
class Datatable(ESETable):
    ...
    _indexes_ = [
     'rightsGuid']
    ATTRIBUTE_ID = 131102
    ATTRIBUTE_SYNTAX = 131104
    LDAP_DISPLAY_NAME = 131532
    MSDS_INTID = 591540
    attsyntax2type = {524289: 'DN', 
       524290: 'OID', 
       524291: 'CaseExactString', 
       524292: 'CaseIgnoreString', 
       524293: 'IA5String', 
       524294: 'NumericString', 
       524295: 'DNWithBinary', 
       524296: 'Boolean', 
       524297: 'Enumeration', 
       524298: 'OctetString', 
       524299: 'GeneralizedTime', 
       524300: 'DirectoryString', 
       524301: 'PresentationAddress', 
       524302: 'DNWithString', 
       524303: 'NTSecurityDescriptor', 
       524304: 'Integer8', 
       524305: 'Sid'}
    type2type = {'DN': (
            'Text', False), 
    ...
    def identify_columns(self):
        ...
        att2ldn = {}
        att2asy = {}
        cols = {int(c.name[4:]):c for c in self.esetable if c.name.startswith('ATT') if c.name.startswith('ATT')}
        nbcols = len(cols)
        ...
        for c in self.esetable.columns:
            if c.name in self.attname2col:
                ...
                esecol = ESEColumn(dbsanecolname(att2ldn.get(c.name, c.name)), c.name, synt, idx)
            columns.append(esecol)
...
def import_file((options, fname, connection)):
    ...
    except libesedb.libesedb.ESEDB_Error as e:
        log.error(e)
...
def main():
    ...
    cnxparser.add_argument('--connection', '-C', help="Backend connection string. Ex: 'dbname=test user=john' for PostgreSQL " + "or '[ip]:[port]:dbname' for mongo", metavar='CNX')
    cnxparser.add_argument('--C-list', help='Comma seaparated list of backend connection strings, ' + 'one for each file to import')
    cnxparser.add_argument('--C-from-filename', nargs=2, help='RPN program to infer connection name from filename. ' + 'Ex: -C-from "::%%s" "basename rmext -  replace upper"', metavar='CNXFMT RPNPROG')
    parser.add_argument('--backend-class', '-B', default='mongo', help='database backend', choices=bta.backend.Backend.backends.keys())
    parser.add_argument('--only', default='', help='Restrict import to TABLENAME', metavar='TABLENAME')
    parser.add_argument('--append', action='store_true', help='Append ESE tables to existing data in db')
    parser.add_argument('--overwrite', action='store_true', help='Delete tables that already exist in db')
    parser.add_argument('--ignore-version-mismatch', action='store_true', help="Ignore mismatch between stored data and this program's format versions")
    parser.add_argument('--ignore-import-errors', '-f', action='store_true', help='Ignore ESE tables reading errors and import as much as possible')
    parser.add_argument('--no-post-processing', dest='no_post_proc', action='store_true', help="Don't post-process imported data")
    parser.add_argument('--only-post-processing', dest='only_post_proc', action='store_true', help='Do not import any tables, only post-process data')
    parser.add_argument('--multi', action='store_true', help='Spawn many workers')
    parser.add_argument('--proc-number', default=None, help='Number of workers. Default: as much as processors')
    parser.add_argument('--yes', '-y', dest='yes', action='store_true', help='Do not ask for validations')
    parser.add_argument('--verbose', '-v', action='count', default=3, help='be more verbose (can be used many times)')
    parser.add_argument('--quiet', '-q', action='count', default=0, help='be more quiet (can be used many times)')
    parser.add_argument('ntds', nargs='+', help='paths to ntds.dit to import')
    ...
    if not options.yes and len(options.connections) > 1:
        if bta.tools.ask.ask('Can I carry on?', 'yn') == 'n':
            ...
            raise SystemExit
    jobs = [ (options, fname, cnx) for fname, cnx in zip(options.ntds, options.connections) ]
    if options.multi:
