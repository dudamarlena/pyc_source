...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/bta/miners/search_for_right.py
...
class Search4Rights(Miner):
    ...
    _rights_ = {'User-Force-Change-Password': ['ADSRightDSControlAccess'], 'Send-As': [
       ...
       'GP-Options': [
                    'GenericWrite', 'ADSRightDSWriteProp', 'GenericAll'], 
       'Can-Get-Ownership': [
                           'GenericAll', 'WriteOwner'], 
       'Can-Modify-Groups': [
                           'ADSRightDSWriteProp', 'GenericWrite', 'GenericAll']}
    _types_ = [
     'User', 'Computer', 'Group']
    ...
    def ACEAllowRight(self, searchedRight, flags, searchedType):
        ...
        req_filter = {'sd_value.DACL.ACEList.SID': 1, 'sd_value.DACL.ACEList.AccessMask.flags': 1, 
           'sd_id': 1, 
        ...
        for sd in self.sd_table.find(req, req_filter):
            ...
            for ace in sd['sd_value']['DACL']['ACEList']:
                if 'InheritedObjectType' not in ace.keys():
                    ...
                    continue
                if (
                 ace['SID'], self._magic_word_) in denied_ace or (ace['SID'], searchedRight) in denied_ace:
                    continue
                who = ace['ObjectType'] if 'ObjectType' in ace.keys() else self._magic_word_
                ...
                if any([ ace['AccessMask']['flags'].get(flag, False) for flag in flags ]) and string_who in [searchedRight, self._magic_word_]:
                    if '%s' % sd['sd_id'] not in result.keys():
                        result['%s' % sd['sd_id']] = list()
                    result[('%s' % sd['sd_id'])].append('----------%s have the right %s on %s' % (SID2StringFull(ace['SID'], self.guid),
                     string_who,
                     SID2StringFull(ace['InheritedObjectType'], self.guid)))
