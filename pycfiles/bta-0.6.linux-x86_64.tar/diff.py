...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/bta/diff.py
# Compiled at: 2015-10-20 16:27:01
import bta.backend
DEFAULT_IGNORE_LIST = {
 'whenChanged', 'replPropertyMetaData', 'dSCorePropagationData',
    ...
    def run(self):
        ...
        while True:
            ...
            if icB is None or icA is not None and icA < icB:
                print ('A, %i: [%s]' % (icA, rA.get('name', ''))).encode('utf-8')
                icA = None
                old += 1
            elif icA is None or icA > icB:
                print ('B, %i: [%s]' % (icB, rB.get('name', ''))).encode('utf-8')
                icB = None
                new += 1
            else:
                ...
                if AnotB or BnotA or ABdiff:
                    ...
                    print ('AB, %i: [%s] %s' % (icA, name, (', ').join(descr))).encode('utf-8')
                    diff += 1
                icA = icB = None
...
def main():
    ...
    parser.add_argument('--CA', dest='connectionA', help="Backend A connection string. Ex: 'dbname=test user=john' for PostgreSQL or '[ip]:[port]:dbname' for mongo)", metavar='CNX')
    parser.add_argument('--CB', dest='connectionB', help="Backend B connection string. Ex: 'dbname=test user=john' for PostgreSQL or '[ip]:[port]:dbname' for mongo)", metavar='CNX')
    parser.add_argument('--BA', dest='backend_classA', default='mongo', help='database A backend', choices=bta.backend.Backend.backends.keys())
    parser.add_argument('--BB', dest='backend_classB', default='mongo', help='database B backend', choices=bta.backend.Backend.backends.keys())
    parser.add_argument('--only', dest='only', default='', help='Diff only TABLENAME', metavar='TABLENAME')
    parser.add_argument('-X', '--ignore-field', dest='ignore_list', action='append', default=[], help='Add a field name to be ignored', metavar='FIELD')
    parser.add_argument('-A', '--consider-field', dest='consider_list', action='append', default=[], help='Add a field name to be considered even if present in default ignore list', metavar='FIELD')
    parser.add_argument('--ignore-version-mismatch', dest='ignore_version_mismatch', action='store_true', help="Ignore mismatch between stored data and this program's format versions")
    parser.add_argument('--ignore-defaults', dest='ignore_defaults', action='store_true', help='Add %s to list of ignored fields' % (', ').join(DEFAULT_IGNORE_LIST))
