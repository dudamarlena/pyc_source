...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/bta/miners/can_create.py
...
class CanCreate(Miner):
    ...
    def ACECreationRight(self, searchedType, flag):
        ...
        req_filter = {'sd_value.DACL.ACEList.SID': 1, 'sd_value.DACL.ACEList.AccessMask.flags.%s' % flag: 1, 
           'sd_id': 1, 
            ...
            for ace in sd['sd_value']['DACL']['ACEList']:
                ...
                if (ace['SID'], searchedType) not in denied_ace and (ace['SID'], magic_word) not in denied_ace:
                    ...
                    if ace['AccessMask']['flags'][flag] and string_who in [searchedType, magic_word]:
                        result[('%s' % sd['sd_id'])].append('----------%s have the right on %s' % (SID2StringFull(ace['SID'], self.guid), string_who))
