...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dbmigrator/utils.py
...
import datetime, difflib, functools, glob, logging, os, pkg_resources, re
from select import select
import sys, subprocess, psycopg2
...
def super_user():
    ...
    with db_connect(settings['db_connection_string'], user=super_user) as (db_conn):
    ...
    while 1:
        try:
            ...
            if state == POLL_OK:
                ...
                raise conn.OperationalError(('bad state from poll: {}').format(state))
...
def get_settings_from_entry_points(settings, contexts):
    ...
    for context in contexts:
        entry_points = pkg_resources.get_entry_map(context, __package__).values()
        for entry_point in entry_points:
            ...
            if old_value and old_value != value or isinstance(old_value, list) and value not in old_value:
                if not isinstance(old_value, list):
                    context_settings[setting_name] = [
                     old_value]
...
def get_migrations(migration_directories, import_modules=False, reverse=False):
    ...
    python_files = functools.reduce(lambda a, b: a + b, [ glob.glob(path) for path in paths ], [])
    for path in sorted(python_files, key=lambda path: os.path.basename(path), reverse=reverse):
        ...
        if m:
            ...
            if import_modules:
                yield (
                 version, migration_name, import_migration(path))
            else:
                yield (
                 version, migration_name)


def get_schema_versions(cursor, versions_only=True, raise_error=True, include_deferred=True, order_by='version'):
    try:
        cursor.execute(('SELECT * FROM schema_migrations ORDER BY {}').format(order_by))
        ...
        logger.warning('You may need to create the schema_migrations table: dbmigrator init --help')

    return


def get_pending_migrations(migration_directories, cursor, import_modules=False, up_to_version=None, include_defers=False):
    migrated_versions = {i[0]:i[1] for i in get_schema_versions(cursor, versions_only=False)}
    migrations = list(get_migrations(migration_directories, import_modules=True))
    ...
    for migration in migrations:
        ...
        if not deferred or include_defers:
            if hasattr(mod, 'should_run'):
                yield migration
            elif deferred and include_defers or not deferred and not migrated_versions.get(version):
                yield migration
...
def compare_schema(db_connection_string, callback, *args, **kwargs):
    ...
    new_schema = subprocess.check_output([
     'pg_dump', '-s', db_connection_string]).decode('utf-8')
    logger.info(('').join(list(difflib.unified_diff(old_schema.splitlines(True), new_schema.splitlines(True), n=10))).encode('utf-8'))


def run_migration(cursor, version, migration_name, migration, run_deferred=False):
    if not run_deferred:
        migrated_versions = dict(list(get_schema_versions(cursor, versions_only=False, raise_error=False)))
        if is_deferred(version, migration, migrated_versions):
            logger.info(('Skipping deferred migration {} {}').format(version, migration_name))
            return
    try:
        cursor.execute('SAVEPOINT pre_should_run')
        if not migration.should_run(cursor):
            logger.info(('Skipping migration {} {}: should_run is false').format(version, migration_name))
...
def mark_migration(cursor, version, completed):
    if completed == 'deferred':
        cursor.execute('INSERT INTO schema_migrations (version, applied) VALUES (%s, NULL)', (
         version,))
    elif completed:
...
def is_deferred(version, migration, migrated_versions):
    ...
    if timestamp == '':
        ...
        if timestamp is None:
            return True
        if hasattr(migration, 'should_run') and hasattr(migration.up, 'dbmigrator_deferred'):
            return True
        return False