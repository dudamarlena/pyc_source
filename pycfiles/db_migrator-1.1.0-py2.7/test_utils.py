...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dbmigrator/tests/test_utils.py
# Compiled at: 2017-11-22 15:30:22
...
class UtilsTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        dbmigrator.utils.set_settings({'db_connection_string': testing.db_connection_string})
    ...
    def test_super_user(self):
        ...
        def cleanup():
            with db_connect(testing.db_connection_string, user='postgres') as (db_conn):
                with db_conn.cursor() as (cursor):
        ...
        with db_connect(testing.db_connection_string) as (db_conn):
            with db_conn.cursor() as (cursor):
                ...
                with super_user() as (super_cursor):
                    ...
                    super_cursor.execute('GRANT ALL PRIVILEGES ON TABLE super_table TO travis')
                    super_cursor.execute("INSERT INTO super_table VALUES ('62')")
                cursor.execute('SELECT current_user')
                self.assertEqual('travis', cursor.fetchone()[0])
    ...
    def test_get_settings_from_entry_points(self):
        ...
        settings = {'db_connection_string': testing.db_connection_string}
        get_settings_from_entry_points(settings, contexts)
        self.assertEqual(settings, {'migrations_directory': testing.test_migrations_directories, 'db_connection_string': testing.db_connection_string})

    def test_get_settings_from_config(self):
        from ..utils import get_settings_from_config
        tmp_dir = tempfile.gettempdir()
        settings = {'migrations_directory': tmp_dir}
        get_settings_from_config(testing.test_config_path, [
         'db-connection-string', 'migrations-directory', 'does-not-exist'], settings)
        self.assertEqual(settings, {'db_connection_string': testing.db_connection_string, 'migrations_directory': tmp_dir})
    ...
    def test_with_cursor(self):
        ...
        @with_cursor
        def func(cursor, arg_1, kwarg_1='kwarg_1', kwarg_2='kwarg_2', db_connection_string=None):
            ...
            self.assertEqual(db_connection_string, testing.db_connection_string)
            self.called = True

        func('arg_1', kwarg_1='called', db_connection_string=testing.db_connection_string)
    ...
    def test_import_migration(self):
        ...
        migration_path = os.path.join(testing.test_data_path, 'package-a', 'package_a', 'migrations', '20160228202637_add_table.py')
        ...
        with db_connect(testing.db_connection_string) as (db_conn):
            with db_conn.cursor() as (cursor):
                try:
                    ...
                    self.fail('a_table should not be in the database before the migration is run')
        ...
        with db_connect(testing.db_connection_string) as (db_conn):
            with db_conn.cursor() as (cursor):
                try:
                    cursor.execute('SELECT * FROM a_table')
                    self.fail('a_table should not be in the database after the migration is rolled back')
                except psycopg2.ProgrammingError as e:
                    self.assertIn('relation "a_table" does not exist', str(e))
    ...
    def test_get_pending_migrations(self):
        ...
        with db_connect(testing.db_connection_string) as (db_conn):
            with db_conn.cursor() as (cursor):
                migrations = list(get_pending_migrations(testing.test_migrations_directories, cursor))
    ...
    def test_run_migration(self):
        ...
        with db_connect(testing.db_connection_string) as (db_conn):
            with db_conn.cursor() as (cursor):
                migrations = list(get_pending_migrations(testing.test_migrations_directories, cursor, import_modules=True))
                version, name, migration = list(migrations)[0]
                run_migration(cursor, version, name, migration)
                after_migrations = list(get_pending_migrations(testing.test_migrations_directories, cursor))
        logger.info.assert_called_with(('Running migration {} {}').format(version, name))
        self.assertNotIn((version, name), after_migrations)

    def test_rollback_migration(self):
        from ..utils import rollback_migration, run_migration, get_pending_migrations
        with db_connect(testing.db_connection_string) as (db_conn):
            with db_conn.cursor() as (cursor):
                migrations = list(get_pending_migrations(testing.test_migrations_directories, cursor, import_modules=True))
                version, name, migration = list(migrations)[0]
                ...
                after_migrations = list(get_pending_migrations(testing.test_migrations_directories, cursor))
        logger.info.assert_called_with(('Rolling back migration {} {}').format(version, name))
