...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/holmium/core/pageobject.py
# Compiled at: 2016-08-11 20:06:15
"""
implementation of page objects, element(s) and sections
"""
import inspect, weakref, types, threading, contextlib, collections
...
class ElementList(list):
    ...
    def __getitem__(self, index):
        return list.__getitem__(self, index).__get__(self.instance(), self.instance().__class__)
...
class ElementDict(dict):
    ...
    def __getitem__(self, key):
        return dict.__getitem__(self, key).__get__(self.instance(), self.instance().__class__)
...
class ElementGetter(object):
    ...
    def __init__(self, locator_type, query_string, base_element=None, timeout=0, value=lambda el: el, only_if=lambda el: el is not None, facet=False, filter_by=lambda el: el is not None):
        """
    ...
    def _get_element(self, method=None):
        ...
        if self.base_element:
            if isinstance(self.base_element, types.LambdaType):
                ...
                _meth = getattr(self.base_element.__get__(self, self.__class__), method.__name__)
            ...
            _meth = method
        log.debug('looking up locator:%s, query_string:%s, timeout:%d' % (
        ...
        if self.timeout:
            try:
                ...
                def callback(_):
                    ...
                    """
                    return _meth(self.locator_type, self.query_string) and (BaseCondition.get_current() or self.only_if)(_meth(self.locator_type, self.query_string))

                WebDriverWait(self.root, self.timeout, ignored_exceptions=[
                 StaleElementReferenceException]).until(callback)
            except TimeoutException:
                log.debug('unable to find element %s after waiting for %d seconds' % (
...
class Element(ElementGetter):
    ...
    def __get__(self, instance, owner):
        if not instance:
            ...
            try:
                if self.root:
                    return self.value_mapper(self.enhance(self._get_element(self.root.find_element)))
                else:
                    return
...
class Elements(ElementGetter):
    ...
    def __init__(self, locator_type, query_string=None, base_element=None, timeout=0, value=lambda el: el, only_if=lambda els: len(els) > 0, facet=False, filter_by=lambda el: el is not None):
        super(Elements, self).__init__(locator_type, query_string, base_element=base_element, timeout=timeout, facet=facet, value=value, only_if=only_if, filter_by=filter_by)
    ...
    def __get__(self, instance, owner):
        ...
        try:
            if self.root:
                return [ self.value_mapper(self.enhance(el)) for el in self._get_element(self.root.find_elements) ]
            else:
                return []
...
class ElementMap(Elements):
    ...
    def __init__(self, locator_type, query_string=None, base_element=None, timeout=0, key=lambda el: el.text, value=lambda el: el, only_if=lambda els: len(els) > 0, facet=False, filter_by=lambda el: el is not None):
        super(ElementMap, self).__init__(locator_type, query_string, base_element, timeout, facet=facet, only_if=only_if, filter_by=filter_by)
        self.key_mapper = key
        self.value_mapper = value
    ...
    def __get__(self, instance, owner):
        ...
        try:
            if self.root:
                return OrderedDict((self.key_mapper(el), self.value_mapper(self.enhance(el))) for el in self._get_element(self.root.find_elements))
            else:
                return {}
...
class Section(Faceted):
    ...
    def __getattribute__(self, item):
        ...
        attr_setter = lambda key, value: super(Section, self).__setattr__(key, value)
    ...
    def root(self):
        ...
        try:
            if not self.__root_val:
                WebDriverWait(Page.get_driver(), self.timeout).until(lambda _: Page.get_driver().find_element(self.locator_type, self.query_string))
...
class Sections(Section, collections.Sequence):
    ...
    def __getelements__(self):
        if self.timeout:
            try:
                WebDriverWait(Page.get_driver(), self.timeout).until(lambda _: Page.get_driver().find_elements(self.locator_type, self.query_string))
            except TimeoutException:
                log.debug('unable to find element %s after waiting for %d seconds' % (
                 self.query_string, self.timeout))
