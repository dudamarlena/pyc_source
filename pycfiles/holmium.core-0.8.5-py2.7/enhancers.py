...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/holmium/core/enhancers.py
...
class ElementEnhancer(object):
    ...
    def __getattribute__(self, key):
        ...
        try:
            ...
            raise AttributeError('neither %s, nor %s object has an attribute %s' % (
...
def register_enhancer(enhancer):
    ...
    if not issubclass(enhancer, ElementEnhancer):
        raise TypeError('Only subclasses of holmium.core.ElementEnhancer can be registered')
    if not hasattr(enhancer, '__TAG__') or not enhancer.__TAG__:
        raise AttributeError('ElementEnhancer implementations must declare a __TAG__property to match against')
