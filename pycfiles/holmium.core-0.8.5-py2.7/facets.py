...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/holmium/core/facets.py
...
class Title(Facet):
    ...
    def evaluate(self, driver):
        assert_true(re.compile(self.arguments['title']).match(driver.title), 'title did not match %s' % self.arguments['title'])
...
class Cookie(Facet):
    ...
    def evaluate(self, driver):
        ...
        if cookie_value and self.options['value']:
            ...
            assert_true(cookie_value is not None, 'cookie %s does not exist' % self.arguments['name'])
        return
...
class ElementFacet(Facet):
    ...
    def evaluate(self, driver):
        assert_true(self.element.__get__(self.parent_class, self.parent_class), 'No such element')
