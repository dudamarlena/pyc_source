...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/holmium/core/config.py
# Compiled at: 2016-08-11 20:06:15
"""
configuration objects for holmium
"""
import sys, os, inspect, jinja2
...
BROWSER_MAPPING = {'firefox': webdriver.Firefox, 'chrome': webdriver.Chrome, 
...
CAPABILITIES = {'firefox': webdriver.DesiredCapabilities.FIREFOX, 'chrome': webdriver.DesiredCapabilities.CHROME, 
...
class HolmiumConfig(dict):
    ...
    def __init__(self, browser, remote, capabilities, user_agent, environment, ignore_ssl, fresh_instance):
        data = {}
...
class PhantomConfig(DriverConfig):
    ...
    def __call__(self, config, args):
        if config.ignore_ssl:
            args.setdefault('service_args', []).append('--ignore-ssl-errors=true --ssl-protocol=tlsv1')
...
CONFIGURATOR_MAPPER = {'firefox': FirefoxConfig(), 
   'chrome': ChromeConfig(), 
