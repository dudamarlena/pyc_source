...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/holmium/core/cucumber.py
...
def word_to_index(word):
    ...
    anomolies = {1: 'first', 2: 'second', 3: 'third', 4: 'fourth', 5: 'fifth', 6: 'sixth', 
       7: 'seventh', 8: 'eighth', 9: 'ninth', 10: 'tenth', 11: 'eleventh', 
       12: 'twelvth', 13: 'thirteenth', 14: 'fourteenth', 
...
def transform_element(name):
    ...
    if not hasattr(scc.page, name):
        raise AttributeError('page object %s does not contain an element named %s' % (
         scc.page.__class__.__name__, name))
    element = getattr(scc.page, name)
    return element


@NamedTransform('{item_in_elements}', '((.*?)\\s+item\\s+in\\s+(\\w+))', '(.*?)\\s+item\\s+in\\s+(\\w+)')
