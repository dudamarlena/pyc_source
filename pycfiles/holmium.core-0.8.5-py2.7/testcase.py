...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/holmium/core/testcase.py
# Compiled at: 2016-09-06 10:14:21
"""
...
class TestCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.holmium_config = HolmiumConfig(browser, remote, {}, user_agent, environment, ignore_ssl, fresh_instance)
        config = None
        ...
        if remote:
            ...
            if cls.holmium_config.browser not in BROWSER_MAPPING:
                raise SkipTest('Unknown browser (%s) specified in HO_BROWSER' % cls.holmium_config.browser)
    ...
    def assertConditionWithWait(self, driver, condition, timeout=0, msg=None, ignored_exceptions=None):
        ...
        try:
            ...
            _msg = self._formatMessage(msg() if callable(msg) else msg, 'Timeout waiting on condition %s' % condition)
