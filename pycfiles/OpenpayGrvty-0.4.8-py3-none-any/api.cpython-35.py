...
# [GCC 8.4.0]
# Embedded file name: /home/daniel/Documents/VentasMedicas/openpay_python/build/lib/openpay/api.py
# Compiled at: 2018-02-27 01:49:10
# Size of source mod 2**32: 6179 bytes
import datetime, calendar, time, platform, json, openpay
...
class APIClient(object):
    ...
    def __init__(self, key=None, client=None, test_mode=False):
        ...
        self._client = client or http_client.new_default_http_client(verify_ssl_certs=verify_ssl_certs)
    ...
    def handle_api_error(self, rbody, rcode, resp):
        ...
        if rcode in (400, 404):
            ...
            if rcode == 401:
                raise error.AuthenticationError(err.get('description'), rbody, rcode, resp)
            else:
                if rcode == 402:
                    raise error.CardError(err.get('description'), err.get('request_id'), err.get('error_code'), rbody, rcode, resp)
                else:
    ...
    def request_raw(self, method, url, params=None):
        ...
        if my_api_key is None:
            raise error.AuthenticationError('No API key provided. (HINT: set your API key using "openpay.api_key = <API-KEY>"). You can generate API keys from the Openpay Dashboard.  See http://docs.openpay.mx for details, or email soporte@openpay.mx if you have any questions.')
        ...
        if method == 'get' or method == 'delete':
            ...
            if method == 'post' or method == 'put':
                ...
                raise error.APIConnectionError('Unrecognized HTTP method %r.  This may indicate a bug in the Openpay bindings.  Please contact soportesu@openpay.mx for assistance.' % (
                 method,))
        ua = {'bindings_version': version.VERSION, 
         'lang': 'python', 
         'publisher': 'openpay', 
        ...
        headers = {'X-Openpay-Client-User-Agent': json.dumps(ua), 
         'User-Agent': 'Openpay/v1 PythonBindings/%s' % (version.VERSION,), 
         'content-type': 'application/json'}
        if api_version is not None:
            headers['Openpay-Version'] = api_version
        rbody, rcode = self._client.request(method, abs_url, headers, post_data, user=my_api_key)
