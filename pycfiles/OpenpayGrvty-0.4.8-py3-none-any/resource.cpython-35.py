...
# [GCC 8.4.0]
# Embedded file name: /home/daniel/Documents/VentasMedicas/openpay_python/build/lib/openpay/resource.py
# Compiled at: 2018-02-27 01:49:10
# Size of source mod 2**32: 19839 bytes
try:
    import json
except ImportError:
...
def convert_to_openpay_object(resp, api_key, item_type=None):
    ...
    if isinstance(resp, list):
        ...
        if isinstance(resp, dict) and not isinstance(resp, BaseObject):
            resp = resp.copy()
...
class BaseObject(dict):
    ...
    def __getitem__(self, k):
        try:
            ...
            if k in self._transient_values:
                raise KeyError("%r.  HINT: The %r attribute was set in the past.It was then wiped when refreshing the object with the result returned by Openpay's API, probably as a result of a save().  The attributes currently available on this object are: %s" % (
                 k, k, ', '.join(list(self.keys()))))
            else:
                raise err

    def __delitem__(self, k):
        raise TypeError('You cannot delete attributes on a BaseObject. To unset a property, set it to None.')
    ...
    def refresh_from(self, values, api_key=None, partial=False):
        ...
        for k, v in list(values.items()):
            super(BaseObject, self).__setitem__(k, convert_to_openpay_object(v, api_key))
    ...
    def request(self, method, url, params=None):
        ...
        if isinstance(response, list):
            ...
            response = data
        if isinstance(response, dict) and self.get('item_type') and 'object' not in list(response.keys()):
...
class APIResource(BaseObject):
    ...
    def class_name(cls):
        if cls == APIResource:
            raise NotImplementedError('APIResource is an abstract class. You should perform action on its subclasses (e.g. Charge, Customer)')
...
class UpdateableAPIResource(APIResource):
    ...
    def save(self):
        ...
        if updated_params:
            ...
            if 'balance' in updated_params and 'status' in updated_params:
                updated_params.update({'status': None, 'balance': None})
            else:
                updated_params.update({'status': None})
            self.refresh_from(self.request('put', self.instance_url(), updated_params))
        else:
    ...
    def serialize_metadata(self):
        if 'metadata' in self._unsaved_values:
            ...
            keys_to_unset = set(self._previous_metadata.keys()) - set(self.metadata.keys())
...
class Charge(CreateableAPIResource, ListableAPIResource, UpdateableAPIResource):
    ...
    def class_url(cls, params=None):
        ...
        if params and 'customer' in params:
            return '/v1/{0}/customers/{1}/{2}s'.format(merchant_id, params['customer'], cls_name)
        else:
            return '/v1/%s/%ss' % (merchant_id, cls_name)
...
class Payout(CreateableAPIResource, ListableAPIResource, DeletableAPIResource):
