...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_zabbix/executors.py
...
class HostCreateExecutor(executors.CreateExecutor):
    ...
    def get_task_signature(cls, host, serialized_host, **kwargs):
        return tasks.BackendMethodTask().si(serialized_host, 'create_host', state_transition='begin_creating')
...
class HostUpdateExecutor(executors.UpdateExecutor):
    ...
    @classmethod
    def get_task_signature(cls, host, serialized_host, **kwargs):
        return tasks.BackendMethodTask().si(serialized_host, 'update_host', state_transition='begin_updating')
...
class HostDeleteExecutor(executors.DeleteExecutor):
    ...
    def get_task_signature(cls, host, serialized_host, **kwargs):
        if host.backend_id:
            return tasks.BackendMethodTask().si(serialized_host, 'delete_host', state_transition='begin_deleting')
        else:
...
class ITServiceCreateExecutor(executors.CreateExecutor):
    ...
    def get_task_signature(cls, itservice, serialized_itservice, **kwargs):
        return tasks.BackendMethodTask().si(serialized_itservice, 'create_itservice', state_transition='begin_creating')
...
class ITServiceDeleteExecutor(executors.DeleteExecutor):
    ...
    def get_task_signature(cls, itservice, serialized_itservice, **kwargs):
        if itservice.backend_id:
            return tasks.BackendMethodTask().si(serialized_itservice, 'delete_itservice', state_transition='begin_deleting')
        else:
...
class UserCreateExecutor(executors.CreateExecutor):
    ...
    def get_task_signature(cls, user, serialized_user, **kwargs):
        creation_tasks = [
         tasks.BackendMethodTask().si(serialized_user, 'create_user', state_transition='begin_creating')]
        if user.phone:
...
class UserDeleteExecutor(executors.DeleteExecutor):
    ...
    def get_task_signature(cls, user, serialized_user, **kwargs):
        if user.backend_id:
            return tasks.BackendMethodTask().si(serialized_user, 'delete_user', state_transition='begin_deleting')
        else:
