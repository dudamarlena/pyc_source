...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_zabbix/tasks.py
...
def update_itservice_sla(itservice_pk, period, start_time, end_time):
    logger.debug('Updating SLAs for IT Service with PK %s. Period: %s, start_time: %s, end_time: %s', itservice_pk, period, start_time, end_time)
    try:
    ...
    try:
        ...
        if itservice.host and itservice.host.scope and itservice.is_main:
            ResourceSla.objects.update_or_create(object_id=itservice.host.scope.id, content_type=ContentType.objects.get_for_model(itservice.host.scope), period=period, defaults={'value': current_sla, 
        ...
        if itservice.backend_trigger_id:
            ...
            for event in events:
                ...
                entry.events.get_or_create(timestamp=int(event['timestamp']), state=event_state)
                if itservice.host and itservice.host.scope and itservice.is_main:
                    ResourceSlaStateTransition.objects.get_or_create(scope=itservice.host.scope, period=period, timestamp=int(event['timestamp']), state=int(event['value']) == 0)

    except ZabbixBackendError as e:
        logger.warning('Unable to update SLA for IT Service %s (ID: %s). Reason: %s', itservice.name, itservice.backend_id, e)
...
def update_monitoring_items():
    ...
    for host in hosts:
        for config in host.MONITORING_ITEMS_CONFIGS:
            update_host_scope_monitoring_items.delay(host.uuid.hex, zabbix_item_key=config['zabbix_item_key'], monitoring_item_name=config['monitoring_item_name'])
...
def update_host_scope_monitoring_items(host_uuid, zabbix_item_key, monitoring_item_name):
    ...
    if host.scope is None:
        ...
        if Item.objects.filter(template__hosts=host, key=zabbix_item_key).exists():
            ...
            ResourceItem.objects.update_or_create(object_id=host.scope.id, content_type=ContentType.objects.get_for_model(host.scope), name=monitoring_item_name, defaults={'value': value})
            logger.debug('Successfully updated monitoring item %s for host %s (%s). Current value: %s.', monitoring_item_name, host.visible_name, host.uuid.hex, value)
        else:
            logger.debug('Host %s (UUID: %s) does not have monitoring item %s.', host.visible_name, host.uuid.hex, monitoring_item_name)
        return value
...
def after_creation_monitoring_item_update(host_uuid, config):
    item_value = update_host_scope_monitoring_items(host_uuid, config['zabbix_item_key'], config['monitoring_item_name'])
...
class SMSTask(core_tasks.Task):
    ...
    def execute(self, settings, message, phone):
        ...
        if sender and recipient and '{phone}' in recipient:
            ...
            logger.warning('SMS was not sent, because `sms_email_from` and `sms_email_rcpt` were not configured properly.')


@shared_task(name='nodeconductor.zabbix.pull_hosts')
def pull_hosts():
    pullable_hosts = Host.objects.exclude(backend_id='')
    for host in pullable_hosts.filter(state=Host.States.ERRED):
        serialized_host = core_utils.serialize_instance(host)
        core_tasks.BackendMethodTask().apply_async(args=(
         serialized_host, 'pull_host'), link=core_tasks.RecoverTask().si(serialized_host), link_error=core_tasks.ErrorMessageTask().s(serialized_host))

    for host in Host.objects.filter(state=Host.States.OK):
        serialized_host = core_utils.serialize_instance(host)
        core_tasks.BackendMethodTask().apply_async(args=(
         serialized_host, 'pull_host'), link_error=core_tasks.ErrorStateTransitionTask().s(serialized_host))
