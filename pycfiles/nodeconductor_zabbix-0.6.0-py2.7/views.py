...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_zabbix/views.py
...
class HostViewSet(six.with_metaclass(structure_views.ResourceViewMetaclass, structure_views.ResourceViewMixin, StateExecutorViewSet)):
    ...
    def items_aggregated_values(self, request):
        ...
        for host in hosts:
            ...
            host_aggregated_values = backend.get_items_aggregated_values(host, items, filter_data['start'], filter_data['end'], filter_data['method'])
    ...
    def _get_stats(self, request, hosts):
        ...
        if non_numeric_items:
            raise exceptions.ValidationError('Cannot show historical data for non-numeric items: %s' % (', ').join(non_numeric_items))
        points = self._get_points(request)
        stats = []
        for item in items:
            values = self._sum_rows([ host.get_backend().get_item_stats(host.backend_id, item, points) for host in hosts
                                    ])
            for point, value in zip(points, values):
                stats.append({'point': point, 
                   'item': item.key, 
                   'item_name': item.name, 
                   'value': value})
    ...
    def _get_points(self, request):
        mapped = {'start': request.query_params.get('start'), 
           'end': request.query_params.get('end'), 
           'points_count': request.query_params.get('points_count'), 
           'point_list': request.query_params.getlist('point')}
        serializer = HistorySerializer(data={k:v for k, v in mapped.items() if v if v})
        serializer.is_valid(raise_exception=True)
...
class ITServiceViewSet(six.with_metaclass(structure_views.ResourceViewMetaclass, structure_views.ResourceViewMixin, StateExecutorViewSet)):
...
class UserViewSet(structure_views.BaseServicePropertyViewSet, StateExecutorViewSet):
    ...
    def password(self, request, uuid):
        ...
        return response.Response({'detail': 'password update was scheduled successfully', 'password': user.password}, status=status.HTTP_200_OK)