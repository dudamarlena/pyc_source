...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_zabbix/backend.py
# Compiled at: 2016-09-21 16:06:28
import sys, logging, pyzabbix, requests, warnings
    ...
    def request(self, *args, **kwargs):
        if not kwargs.get('verify', self.verify):
            with warnings.catch_warnings():
                if hasattr(exceptions, 'InsecurePlatformWarning'):
...
class ZabbixBackend(ServiceBackend):
    DEFAULTS = {'host_group_name': 'nodeconductor', 
       'templates_names': [], 'database_parameters': {'host': 'localhost', 
                               'port': '3306', 
                               'name': 'zabbix', 
                               'user': 'admin', 
                               'password': ''}, 
       'interface_parameters': {'dns': '', 
                                'ip': '0.0.0.0', 
                                'main': 1, 
                                'port': '10050', 
                                'type': 1, 
    ...
    TREND_DELAY_SECONDS = 3600
    HISTORY_DELAY_SECONDS = 900
    ...
    def api(self):
        if not hasattr(self, '_api'):
            self._api = self._get_api(self.settings.backend_url, self.settings.username, self.settings.password)
        return self._api
    ...
    def create_host(self, host):
        ...
        zabbix_host_id, created = self._get_or_create_host_id(host_name=host.name, visible_name=host.visible_name, group_id=group_id, templates_ids=templates_ids, interface_parameters=interface_parameters, status=host.status)
        if not created:
    ...
    def update_host(self, host):
        try:
            ...
            self.api.host.update({'hostid': host.backend_id, 
               'host': host.name, 
               'name': host.visible_name, 
               'group_id': group_id, 
               'templates': [ {'templateid': t.backend_id} for t in host.templates.all() ], 'status': host.status})
    ...
    def pull_templates(self):
        ...
        try:
            zabbix_templates = self.api.template.get(output=[
             'name', 'templateid'], selectTriggers=[
             'description', 'triggerid'], selectItems=[
             'itemid', 'name', 'key_', 'value_type', 'units', 'history', 'delay'], selectTemplates=[
             'templateid'])
        except (pyzabbix.ZabbixAPIException, RequestException) as e:
        ...
        for zabbix_template in zabbix_templates:
            nc_template, created = models.Template.objects.get_or_create(backend_id=zabbix_template['templateid'], settings=self.settings, defaults={'name': zabbix_template['name']})
            ...
            for zabbix_trigger in zabbix_template['triggers']:
                nc_template.triggers.update_or_create(backend_id=zabbix_trigger['triggerid'], settings=nc_template.settings, defaults={'name': zabbix_trigger['description']})
            ...
            for zabbix_item in zabbix_template['items']:
                defaults = {'name': zabbix_item['name'], 
                ...
                nc_item, created = nc_template.items.get_or_create(backend_id=zabbix_item['itemid'], defaults=defaults)
                if not created:
                    update_fields = []
    ...
    def pull_itservices(self):
        ...
        for zabbix_service in zabbix_services:
            ...
            defaults = {'name': zabbix_service['name'], 
            ...
            nc_service, created = models.ITService.objects.get_or_create(settings=self.settings, backend_id=zabbix_service['serviceid'], defaults=defaults)
            if not created:
                update_fields = []
                for name, value in defaults.items():
    ...
    def pull_user_groups(self):
        ...
        for zabbix_user_group in zabbix_user_groups:
            models.UserGroup.objects.update_or_create(backend_id=zabbix_user_group['usrgrpid'], settings=self.settings, defaults={'name': zabbix_user_group['name']})
    ...
    def pull_users(self):
        ...
        for zabbix_user in zabbix_users:
            ...
            for zabbix_user_group in zabbix_user['usrgrps']:
                group, _ = models.UserGroup.objects.get_or_create(backend_id=zabbix_user_group['usrgrpid'], settings=self.settings, defaults={'name': zabbix_user_group['name']})
                groups.append(group)

            user, _ = models.User.objects.update_or_create(backend_id=zabbix_user['userid'], settings=self.settings, defaults={'name': zabbix_user['name'], 
    ...
    def create_user(self, user):
        try:
            zabbix_user = self.api.user.create(name=user.name, surname=user.surname, alias=user.alias, type=user.type, passwd=user.password, usrgrps=[{'usrgrpid':group.backend_id for group in user.groups.all()}])
        except (pyzabbix.ZabbixAPIException, RequestException) as e:
            six.reraise(ZabbixBackendError, e)
    ...
    def update_user(self, user):
        try:
            self.api.user.update(userid=user.backend_id, name=user.name, surname=user.surname, alias=user.alias, type=user.type, passwd=user.password, usrgrps=[{'usrgrpid':group.backend_id for group in user.groups.all()}])
        except (pyzabbix.ZabbixAPIException, RequestException) as e:
            six.reraise(ZabbixBackendError, e)
    ...
    def add_permission_to_user_group(self, user_group, host_group_name, permission_id):
        try:
            ...
            self.api.usergroup.update(usrgrpid=user_group.backend_id, rights=[{'id': host_group_id, 'permission': permission_id}])
        except (pyzabbix.ZabbixAPIException, RequestException) as e:
            six.reraise(ZabbixBackendError, e)
    ...
    def _get_triggers_map(self, zabbix_services):
        ...
        nc_triggers = models.Trigger.objects.filter(settings=self.settings, backend_id__in=template_ids).only('pk')
        nc_triggers_map = {trigger.backend_id:trigger.pk for trigger in list(nc_triggers)}
        return {trigger['triggerid']:nc_triggers_map.get(trigger['templateid']) for trigger in zabbix_triggers}
    ...
    def _get_or_create_host_id(self, host_name, visible_name, group_id, templates_ids, interface_parameters, status):
        ...
        host_id = self._create_host(host_name, visible_name, group_id, templates_ids, interface_parameters, status)
        return (host_id, True)
    ...
    def _create_host(self, host_name, visible_name, group_id, templates_ids, interface_parameters, status):
        host_parameters = {'host': host_name, 
           'name': visible_name, 
           'interfaces': [
                        interface_parameters], 
           'groups': [{'groupid': group_id}], 'templates': [ {'templateid': template_id} for template_id in templates_ids ], 'status': status}
        try:
            host = self.api.host.create(host_parameters)
            return host['hostids'][0]
        except (pyzabbix.ZabbixAPIException, RequestException) as e:
            logger.error('Cannot create host with parameters: %s. Exception: %s', host_parameters, six.text_type(e))
            six.reraise(ZabbixBackendError, e)
    ...
    def _get_showsla(self, algorithm):
        if algorithm == models.ITService.Algorithm.ANY:
            ...
            if algorithm == models.ITService.Algorithm.ALL:
                return 1
            return 0
    ...
    def _get_trigger_id(self, host_id, description):
        ...
        """
        try:
            data = self.api.trigger.get(filter={'description': description}, hostids=host_id, output=[
             'triggerid'])
    ...
    def get_sla(self, service_id, start_time, end_time):
        try:
            data = self.api.service.getsla(filter={'serviceids': service_id}, intervals={'from': start_time, 'to': end_time})
    ...
    def get_itservice(self, service_id):
        try:
            ...
            raise ZabbixBackendError('Can not get status of Zabbix IT service with ID %s. Exception: %s', service_id, e)

        if len(response) != 1:
            raise ZabbixBackendError('Zabbix IT service with ID %s is not found. Response is %s', service_id, response)
        return response[0]
    ...
    def get_trigger_events(self, trigger_id, start_time, end_time):
        try:
            event_data = self.api.event.get(output='extend', objectids=trigger_id, time_from=start_time, time_till=end_time, sortfield=[
             'clock'], sortorder='ASC')
        except (pyzabbix.ZabbixAPIException, RequestException) as e:
    ...
    def get_item_stats(self, hostid, item, points):
        if item.value_type == models.Item.ValueTypes.FLOAT:
            ...
            if item.value_type == models.Item.ValueTypes.INTEGER:
            ...
            points = points[::-1]
            history_cursor = self._get_history(item.key, hostid, history_table, points[(-1)] - history_delay_seconds, points[0])
            trends_cursor = self._get_history(item.key, hostid, trend_table, points[(-1)] - trend_delay_seconds, points[0])
            values = []
    ...
    def get_items_aggregated_values(self, host, items, start_timestamp, end_timestamp, method='MAX'):
        ...
        db_data = tuple()
        default_kwargs = {'hostid': host.backend_id, 
           'start_timestamp': start_timestamp, 
           'end_timestamp': end_timestamp, 
           'method': method}
        if int_items:
            cursor = self._get_aggregated_values(item_keys=[ item.key for item in int_items ], table='history_uint', **default_kwargs)
            db_data += cursor.fetchall()
        if float_items:
            cursor = self._get_aggregated_values(item_keys=[ item.key for item in float_items ], table='history', **default_kwargs)
            db_data += cursor.fetchall()
    ...
    def _get_history(self, item_key, hostid, table, start_timestamp, end_timestamp):
        ...
        """
        query = 'SELECT clock time, %(value_path)s value FROM %(table)s history, items WHERE history.itemid = items.itemid AND items.key_ = "%(item_key)s" AND items.hostid = %(hostid)s AND clock > %(start_timestamp)s AND clock < %(end_timestamp)s ORDER BY clock DESC'
    ...
    def _get_aggregated_values(self, hostid, item_keys, start_timestamp, end_timestamp, table, method='MAX'):
        ...
        """
        query = 'SELECT items.key_, %(method)s(value) FROM hosts, items, %(table)s history WHERE items.hostid = %(hostid)s AND hosts.hostid = %(hostid)s AND history.itemid = items.itemid AND items.key_ IN (%(item_keys)s) AND clock >= %(start_timestamp)s AND clock <= %(end_timestamp)s GROUP BY items.key_'
    ...
    def _get_db_connection(self, force=False):
        ...
        if key not in connections.databases or force:
            connections.databases[key] = {'ENGINE': 'django.db.backends.mysql', 'NAME': name, 
               'HOST': host, 
               'PORT': port, 
               'USER': user, 
               'PASSWORD': password}
    ...
    def import_host(self, host_backend_id, service_project_link=None, save=True):
        ...
        try:
            backend_host = self.api.host.get(filter={'hostid': host_backend_id}, selectGroups=True, output=[
             'host', 'name', 'description', 'error', 'status', 'groups'])[0]
        except IndexError:
    ...
    def get_host_templates(self, host):
        ...
        return models.Template.objects.filter(backend_id__in=[ t['templateid'] for t in backend_templates ], settings=host.service_project_link.service.settings)
    ...
    def pull_host(self, host):
        ...
        if host.modified < import_time:
            update_fields = ('name', 'visible_name', 'description', 'error', 'status',
                             'host_group_name')
