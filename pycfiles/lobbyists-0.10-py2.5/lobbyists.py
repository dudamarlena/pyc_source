...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/lobbyists/lobbyists.py
...
def _period(x):
    periods = {'1st Quarter (Jan 1 - Mar 31)': 'Q1', '2nd Quarter (Apr 1 - June 30)': 'Q2', 
...
def _lobbyist_indicator(x):
    indicator = {0: 'not covered', 1: 'covered', 
       2: 'undetermined'}
...
def _parse_attrs(elt, attrs):
    ...
    for (name, id, parse) in attrs:
        yield (
         id, parse(_attr_of(elt, name)))


def _parse_element(elt, id, attrs):
    return (
     id, dict(_parse_attrs(elt, attrs)))
...
_client_attrs = [
 (
  'ClientCountry', 'country', _optional),
 (
...
_foreign_entity_attrs = [
 (
  'ForeignEntityContribution', 'contribution', _amount),
 (
...
_org_attrs = [
 (
  'AffiliatedOrgCountry', 'country', _optional),
 (
...
_subelt_parsers = {'Registrant': _parse_registrant, 'Client': _parse_client, 
...
_where_stmt = {'client': 'client WHERE country=:country AND name=:name AND ppb_country=:ppb_country AND state=:state AND ppb_state=:ppb_state AND state_or_local_gov=:state_or_local_gov', 
   'registrant': 'registrant WHERE country=:country AND senate_id=:senate_id AND name=:name AND ppb_country=:ppb_country', 
   'lobbyist': 'lobbyist WHERE name=:name AND indicator=:indicator AND official_position=:official_position', 
   'affiliated_org': 'affiliated_org WHERE name=:name AND country=:country AND ppb_country=:ppb_country', 
   'foreign_entity': 'foreign_entity WHERE name=:name AND country=:country AND ppb_country=:ppb_country'}
...
def _import_client(client, filing, cur):
    ...
    if db_key is None:
        ...
        cur.execute('INSERT INTO client VALUES(NULL, :country, :name, :ppb_country, :state, :ppb_state, :state_or_local_gov)', client)
...
def _import_registrant(reg, filing, cur):
    ...
    if db_key is None:
        ...
        cur.execute('INSERT INTO org VALUES(?)', [
         reg['name']])
        cur.execute('INSERT INTO registrant VALUES(NULL, :country, :senate_id, :name, :ppb_country)', reg)
        db_key = cur.lastrowid
...
def _import_lobbyist(lobbyist, filing, cur):
    ...
    if db_key is None:
        ...
        cur.execute('INSERT INTO lobbyist VALUES(NULL, :name, :indicator, :official_position)', lobbyist)
        db_key = cur.lastrowid
...
def _import_issue(issue, filing, cur):
    ...
    cur.execute('INSERT INTO issue VALUES(NULL, :code, :specific_issue)', issue)
    db_key = cur.lastrowid
...
def _import_affiliated_org(org, filing, cur):
    ...
    if db_key is None:
        ...
        cur.execute('INSERT INTO affiliated_org VALUES(NULL, :name, :country, :ppb_country)', org)
        db_key = cur.lastrowid
...
def _import_foreign_entity(entity, filing, cur):
    ...
    if db_key is None:
        ...
        cur.execute('INSERT INTO foreign_entity VALUES(NULL, :name, :country, :ppb_country)', entity)
        db_key = cur.lastrowid
...
def _import_filing(filing, cur):
    ...
    """
    cur.execute('INSERT INTO filing VALUES(:id, :type, :year, :period, :filing_date, :amount)', filing)


_list_importers = {'lobbyist': _import_lobbyist, 'govt_entity': _import_govt_entity, 
   'issue': _import_issue, 
