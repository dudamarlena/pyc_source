...
# Embedded file name: build/bdist.linux-x86_64/egg/prompt_tool_kit/key_binding/bindings/named_commands.py
# Compiled at: 2019-08-15 23:53:38
# Size of source mod 2**32: 15929 bytes
...
from prompt_tool_kit.selection import PasteMode
...
def unix_line_discard(event):
    ...
    if buff.document.cursor_position_col == 0:
        if buff.document.cursor_position > 0:
...
def yank(event):
    ...
    event.current_buffer.paste_clipboard_data((event.cli.clipboard.get_data()),
      count=(event.arg), paste_mode=(PasteMode.EMACS))
...
def yank_pop(event):
    ...
    if doc_before_paste is not None:
        ...
        clipboard.rotate()
        buff.paste_clipboard_data((clipboard.get_data()),
...
def menu_complete(event):
    ...
    way of completing input in prompt_tool_kit.)
    """
