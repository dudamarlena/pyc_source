...
# Embedded file name: build/bdist.linux-x86_64/egg/prompt_tool_kit/key_binding/bindings/vi.py
# Compiled at: 2019-08-15 23:53:38
# Size of source mod 2**32: 67820 bytes
from __future__ import unicode_literals
from prompt_tool_kit.buffer import ClipboardData, indent, unindent, reshape_text
from prompt_tool_kit.document import Document
from prompt_tool_kit.enums import IncrementalSearchDirection, SEARCH_BUFFER, SYSTEM_BUFFER
from prompt_tool_kit.filters import Filter, Condition, HasArg, Always, IsReadOnly
from prompt_tool_kit.filters.cli import ViNavigationMode, ViInsertMode, ViInsertMultipleMode, ViReplaceMode, ViSelectionMode, ViWaitingForTextObjectMode, ViDigraphMode, ViMode
from prompt_tool_kit.key_binding.digraphs import DIGRAPHS
from prompt_tool_kit.key_binding.vi_state import CharacterFind, InputMode
from prompt_tool_kit.keys import Keys
from prompt_tool_kit.layout.utils import find_window_for_buffer_name
from prompt_tool_kit.selection import SelectionType, SelectionState, PasteMode
...
import prompt_tool_kit.filters as filters
from six.moves import range
import codecs, six, string
try:
...
__all__ = ('load_vi_bindings', 'load_vi_search_bindings', 'load_vi_system_bindings',
...
class TextObject(object):
    ...
    def operator_range(self, document):
        ...
        if self.type == TextObjectType.EXCLUSIVE:
            if doc.translate_index_to_position(end + doc.cursor_position)[1] == 0:
                end -= 1
    ...
    def cut(self, buffer):
        ...
        to += buffer.cursor_position
        to -= 1
        document = Document(buffer.text, to, SelectionState(original_cursor_position=from_,
          type=(self.selection_type)))
        new_document, clipboard_data = document.cut_selection()
        return (
         new_document, clipboard_data)
...
def create_text_object_decorator(registry):
    ...
    def text_object_decorator(*keys, **kw):
        ...
        def decorator(text_object_func):
            if not callable(text_object_func):
                ...
                @no_selection_handler or (registry.add_binding)(*keys, filter=~operator_given & filter & selection_mode, eager=eager)
                def _(event):
                    text_object = text_object_func(event)
    ...
    def operator_decorator(*keys, **kw):
        ...
        def decorator(operator_func):
            ...
            def _(event):
                ...
                text_object = TextObject((selection_state.original_cursor_position - buff.cursor_position),
                  type=text_obj_type)
...
def load_vi_bindings(get_search_state=None):
    ...
    def _(event):
        ...
        if before:
            if after:
                before = before + '\n'
        buffer.document = Document(text=(before + after),
        ...
        if c in vi_register_names:
            ...
            if data:
                event.current_buffer.paste_clipboard_data(data,
                  count=(event.arg), paste_mode=(PasteMode.VI_AFTER))
    ...
    def _(event):
        ...
        if c in vi_register_names:
            ...
            if data:
                event.current_buffer.paste_clipboard_data(data,
                  count=(event.arg), paste_mode=(PasteMode.VI_BEFORE))
    ...
    def _(event):
        ...
        match = event.current_buffer.document.find((event.data),
          in_current_line=True, count=(event.arg))
        if match:
    ...
    def _(event):
        ...
        return TextObject(event.current_buffer.document.find_backwards((event.data),
          in_current_line=True, count=(event.arg)) or 0)
    ...
    def _(event):
        ...
        """
        event.cli.vi_state.last_character_find = CharacterFind(event.data, True)
        match = event.current_buffer.document.find_backwards((event.data),
          in_current_line=True, count=(event.arg))
    ...
    def _(event):
        buff = event.current_buffer
        cursor_position = buff.get_search_position((get_search_state(event.cli)),
    ...
    def _(event):
        event.current_buffer.apply_search((get_search_state(event.cli)),
          include_current_position=False, count=(event.arg))
    ...
    def _(event):
        ...
        'g_', 'cg_', 'yg_', etc..
        """
        return TextObject((event.current_buffer.document.last_non_blank_of_current_line_position()),
          type=(TextObjectType.INCLUSIVE))
    ...
    def _(event):
        ...
        return TextObject((buf.document.translate_row_col_to_index(buf.document.line_count - 1, 0) - buf.cursor_position),
...
def load_vi_search_bindings(get_search_state=None, search_buffer_name=SEARCH_BUFFER):
    ...
    def incremental_search(cli, direction, count=1):
        ...
        if not direction_changed:
            ...
            input_buffer.apply_search(search_state, include_current_position=False,
              count=count)
