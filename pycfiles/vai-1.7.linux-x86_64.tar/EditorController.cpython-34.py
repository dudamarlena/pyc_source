...
# [GCC 8.4.0]
# Embedded file name: /home/sbo/lib/python3.4/site-packages/vai/controllers/EditorController.py
# Compiled at: 2015-05-02 14:07:56
# Size of source mod 2**32: 9510 bytes
import os, shlex, hashlib
...
class EditorController:
    ...
    def forceQuit(self):
        for b in self._buffer_list.buffers:
            if b.document.documentMetaInfo('Filename').data() is None:
                continue
            models.EditorState.instance().setCursorPosForPath(os.path.abspath(b.document.documentMetaInfo('Filename').data()), b.cursor.pos)
    ...
    def tryQuit(self):
        if any([b.isModified() for b in self._buffer_list.buffers]):
            self._editor.status_bar.setMessage('Document has been modified. ' + 'Use :q! to quit without saving ' + 'or :qw to save and quit.', 3000)
        else:
            self.forceQuit()
