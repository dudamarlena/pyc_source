...
# [GCC 8.4.0]
# Embedded file name: /home/sbo/lib/python3.4/site-packages/vai/models/TextDocumentCursor.py
...
class TextDocumentCursor(core.VObject):
    ...
    def toLineNext(self):
        ...
        self._pos = (next_line_number,
         min(self._text_document.lineLength(next_line_number), max(self._pos[1], self._optimistic_column)))
        self.positionChanged.emit(self._pos)
        return True
    ...
    def toLinePrev(self):
        ...
        self._pos = (prev_line_number,
         min(self._text_document.lineLength(prev_line_number), max(self._pos[1], self._optimistic_column)))
        self.positionChanged.emit(self._pos)
        return True
    ...
    def toCharFirstNonBlank(self):
        ...
        if len(lstrip_text) == 0:
            ...
            self._pos = (
             self._pos[0], 1 + len(text) - len(lstrip_text))
        self._optimistic_column = self._pos[1]
        self.positionChanged.emit(self._pos)
        return True
    ...
    def toCharFirstNonBlankForLine(self, line_number):
        ...
        if len(lstrip_text) == 0:
            ...
            self._pos = (
             line_number, 1 + len(text) - len(lstrip_text))
        self._optimistic_column = self._pos[1]
        self.positionChanged.emit(self._pos)
        return True
    ...
    def toLastLine(self):
        self._pos = (
         self._text_document.numLines(), 1)
        self._optimistic_column = 0
        self.positionChanged.emit(self._pos)
        return True