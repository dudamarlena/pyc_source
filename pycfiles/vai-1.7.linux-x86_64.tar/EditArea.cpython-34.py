...
# [GCC 8.4.0]
# Embedded file name: /home/sbo/lib/python3.4/site-packages/vai/EditArea.py
...
class EditArea(gui.VWidget):
    ...
    def __init__(self, global_state, editor_controller, parent):
        ...
        self._color_schema = models.SyntaxColors(models.Configuration.get('colors.syntax_schema'), gui.VApplication.vApp.screen().numColors()).colorMap()
        ...
        self._icons = models.Icons.getCollection(models.Configuration.get('icons.collection'))
    ...
    def paintEvent(self, event):
        ...
        if current_word_pos is not None:
            word_entries = Search.findAll(document, current_word, line_interval=visible_line_interval, word=True)
        for visual_line_num, doc_line_num in enumerate(range(*visible_line_interval)):
            ...
            for i in range(5, indent_spaces, 4):
                colors[i - 1] = (
                 gui.VGlobalColor.term_303030, None, None)

            selection = buffer.selection
            if selection.isValid():
                if selection.low_line <= doc_line_num <= selection.high_line:
            ...
            for word_start in word_entries_for_line:
                for pos in range(word_start - 1, word_start - 1 + len(current_word)):
                    if colors[pos] == (None, None, None) and self._highlight_current_identifier:
                        colors[pos] = (
                         gui.VGlobalColor.lightred, None, None)
                        continue
