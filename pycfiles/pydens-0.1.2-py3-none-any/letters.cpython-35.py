...
class GradArray(np.ndarray):
    __doc__ = ' Stores arrays of differentials. Allows tensor operations on n-dimensional\n    derivatives and simple np.arrays (for instance, multiplication of a matrix on\n    gradient-vector)\n    '
...
class Letters(ABC):
    __doc__ = ' Abstract class for custom letters. Defines which letters should be implemented. '
...
class TFLetters(Letters):
    __doc__ = ' TensorFlow implementations of custom letters. '
...
class NPLetters(Letters):
    __doc__ = ' NumPy implementations of custom letters. '
    try:
        _ = (grad, autonp)
    except NameError:
        _ = np
...
class TorchLetters(Letters):
    __doc__ = ' PyTorch implementations of custom letters. '
