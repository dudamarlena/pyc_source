...
# [GCC 8.4.0]
# Embedded file name: /Users/tonra/ohm2/clients/ohm2/ohm2-dev-light/backend/webapp/backend/apps/ohm2_accounts_light/api/v1/views.py
# Compiled at: 2018-05-07 12:53:00
# Size of source mod 2**32: 10853 bytes
...
def set_password_reset(request):
    """
        ...
        """
    keys = (('username', 'username', ''), ('password', 'password', ''), ('identity', 'identity', ''),
            ('code', 'code', ''))
...
def set_password_reset_and_get_token(request):
    ...
    if error:
        return JsonResponse({'error': error.regroup()})
    if res.get('error') or not res.get('ret'):
...
def update_user_information(request):
    """
        ...
        """
    keys = (('first_name', 'firstname', ''), ('last_name', 'lastname', ''), ('email', 'email', ''))
