...
# [GCC 8.4.0]
# Embedded file name: /Users/jsick/.virtualenvs/paperweight/lib/python2.7/site-packages/preprint/pack.py
# Compiled at: 2015-01-11 21:10:10
"""
Command for packaging the manuscript for submission.
"""
import logging, os, shutil, codecs, re, subprocess
...
class Package(Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument('name', help='Name of packaged manuscript (saved to build/name).')
        parser.add_argument('--style', default='aastex', choices=[
         'aastex', 'arxiv'], help='Build style (aastex, arxiv).')
        parser.add_argument('--exts', nargs='*', default=self.app.confs.config('exts'), help='Figure extensions to use in order of priority')
        parser.add_argument('--jpeg', action='store_true', default=False, help='Make JPEG versions of figures if too large (for arxiv)')
        parser.add_argument('--maxsize', default=2.0, type=float, help='Max figure size (MB) before converting to JPEG (for arxiv)')
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        if self._build_style == 'aastex':
            ...
            output_tex_path = os.path.join(dirname, os.path.basename(self.app.options.master))
    ...
    def _process_figures(self, tex, dirname):
        ...
        tex = install_figs(tex, figs, dirname, naming=self._build_style, format_priority=self._ext_priority, max_size=maxsize)
        return tex
...
def discover_figures(tex, ext_priority):
    ...
    figs : dict
        ...
        figure environment.
    """
    figs_pattern = re.compile('\\\\includegraphics(.*?){(.*?)}', re.UNICODE)
    ...
    for i, match in enumerate(matches):
        ...
        for ext in exts:
            ...
            sizes.append(os.path.getsize(p) / 1000000.0)

        figs[basename] = {'path': path, 'exts': exts, 'size_mb': sizes, 
           'options': opts, 
           'env': '\\\\includegraphics', 
           'num': i + 1}
...
def install_figs(tex, figs, install_dir, naming=None, format_priority=('pdf', 'eps', 'ps', 'png', 'jpg', 'tif'), max_size=None):
    ...
    for figname, fig in figs.iteritems():
        ...
        if naming == 'aastex':
            install_path = os.path.join(install_dir, ('f{0:d}.{1}').format(fig['num'], ext))
        elif naming == 'arxiv':
            install_path = os.path.join(install_dir, ('figure{0:d}.{1}').format(fig['num'], ext))
        else:
        ...
        old_fig_cmd = ('{env}{opts}{{{path}}}').format(env=fig['env'].replace('\\\\', '\\'), opts=fig['options'], path=fig['path'])
        new_fig_cmd = ('{env}{opts}{{{path}}}').format(env=fig['env'].replace('\\\\', '\\'), opts=fig['options'], path=os.path.basename(os.path.splitext(install_path)[0]))
...
def rasterize_figure(original_path):
    ...
    subprocess.call(('convert -density 300 -trim -quality 80 {path} {jpgpath}').format(path=original_path, jpgpath=jpg_path), shell=True)
