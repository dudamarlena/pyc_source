...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/contrib/hooks/gcp_compute_hook.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 15179 bytes
...
class GceHook(GoogleCloudBaseHook):
    ...
    def get_conn(self):
        ...
        if not self._conn:
            ...
            self._conn = build('compute', (self.api_version), http=http_authorized,
    ...
    def start_instance(self, zone, resource_id, project_id=None):
        ...
        :return: None
        """
        response = self.get_conn().instances().start(project=project_id,
          zone=zone,
          instance=resource_id).execute(num_retries=(self.num_retries))
        try:
    ...
    def stop_instance(self, zone, resource_id, project_id=None):
        ...
        :return: None
        """
        response = self.get_conn().instances().stop(project=project_id,
          zone=zone,
          instance=resource_id).execute(num_retries=(self.num_retries))
        try:
            operation_name = response['name']
    ...
    def _execute_set_machine_type(self, zone, resource_id, body, project_id):
        return self.get_conn().instances().setMachineType(project=project_id,
          zone=zone,
    ...
    def get_instance_template(self, resource_id, project_id=None):
        ...
        :rtype: dict
        """
        response = self.get_conn().instanceTemplates().get(project=project_id,
          instanceTemplate=resource_id).execute(num_retries=(self.num_retries))
        return response
    ...
    def insert_instance_template(self, body, request_id=None, project_id=None):
        ...
        :return: None
        """
        response = self.get_conn().instanceTemplates().insert(project=project_id,
          body=body,
          requestId=request_id).execute(num_retries=(self.num_retries))
        try:
            operation_name = response['name']
        except KeyError:
            raise AirflowException("Wrong response '{}' returned - it should contain 'name' field".format(response))
    ...
    def get_instance_group_manager(self, zone, resource_id, project_id=None):
        ...
        :rtype: dict
        """
        response = self.get_conn().instanceGroupManagers().get(project=project_id,
          zone=zone,
          instanceGroupManager=resource_id).execute(num_retries=(self.num_retries))
        return response
    ...
    def patch_instance_group_manager(self, zone, resource_id, body, request_id=None, project_id=None):
        ...
        :return: None
        """
        response = self.get_conn().instanceGroupManagers().patch(project=project_id,
          zone=zone,
    ...
    def _wait_for_operation_to_complete(self, project_id, operation_name, zone=None):
        ...
        while True:
            if zone is None:
                operation_response = self._check_global_operation_status(service=service,
    ...
    def _check_zone_operation_status(service, operation_name, project_id, zone, num_retries):
        return service.zoneOperations().get(project=project_id,
          zone=zone,
          operation=operation_name).execute(num_retries=num_retries)

    @staticmethod
    def _check_global_operation_status(service, operation_name, project_id, num_retries):
        return service.globalOperations().get(project=project_id,
          operation=operation_name).execute(num_retries=num_retries)