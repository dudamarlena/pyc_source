...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/contrib/auth/backends/github_enterprise_auth.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 7898 bytes
import flask_login
...
class GHEAuthBackend(object):
    ...
    def ghe_api_route(self, leaf):
        if not self.api_url:
            self.api_url = 'https://api.github.com' if self.ghe_host == 'github.com' else '/'.join(['https:/',
             self.ghe_host,
             'api',
    ...
    def init_app(self, flask_app):
        ...
        self.ghe_oauth = OAuth(self.flask_app).remote_app('ghe',
        ...
        self.flask_app.add_url_rule(get_config_param('oauth_callback_route'), 'ghe_oauth_callback', self.oauth_callback)
    ...
    def get_ghe_user_profile_info(self, ghe_token):
        resp = self.ghe_oauth.get((self.ghe_api_route('/user')), token=(
         ghe_token, ''))
        if not resp or resp.status != 200:
            raise AuthenticationError('Failed to fetch user profile, status ({0})'.format(resp.status if resp else 'None'))
        return (
         resp.data['login'], resp.data['email'])

    def ghe_team_check(self, username, ghe_token):
        try:
            try:
                allowed_teams = [int(team.strip()) for team in get_config_param('allowed_teams').split(',')]
            except ValueError:
                raise ValueError('it appears that you are using the string name for a team, please use the id number instead')

        except AirflowConfigException:
            return True
        else:
            resp = self.ghe_oauth.get((self.ghe_api_route('/user/teams')), token=(
             ghe_token, ''))
            if not resp or resp.status != 200:
                raise AuthenticationError('Bad response from GHE ({0})'.format(resp.status if resp else 'None'))
            for team in resp.data:
                if team['id'] in allowed_teams:
                    return True

            log.debug('Denying access for user "%s", not a member of "%s"', username, str(allowed_teams))
            return False
    ...
    def oauth_callback(self, session=None):
        ...
        try:
            if resp is None:
                raise AuthenticationError('Null response from GHE, denying access.')
