...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/contrib/hooks/ssh_hook.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 10456 bytes
import getpass, os, warnings, paramiko
...
class SSHHook(BaseHook):
        ...
        if self.ssh_conn_id is not None:
            ...
            if conn.extra is not None:
                ...
                if 'compress' in extra_options:
                    if str(extra_options['compress']).lower() == 'false':
                        self.compress = False
                if 'no_host_key_check' in extra_options:
                    if str(extra_options['no_host_key_check']).lower() == 'false':
                        self.no_host_key_check = False
                if 'allow_host_key_change' in extra_options:
                    if str(extra_options['allow_host_key_change']).lower() == 'true':
    ...
    def get_conn(self):
        ...
        if not self.allow_host_key_change:
            ...
            if self.password and self.password.strip():
                client.connect(hostname=(self.remote_host), username=(self.username),
    ...
    def __enter__(self):
        warnings.warn('The contextmanager of SSHHook is deprecated.Please use get_conn() as a contextmanager instead.This method will be removed in Airflow 2.0', category=DeprecationWarning)
        return self
    ...
    def get_tunnel(self, remote_port, remote_host='localhost', local_port=None):
        ...
        if self.password:
            if self.password.strip():
                client = SSHTunnelForwarder((self.remote_host), ssh_port=(self.port),
    ...
    def create_tunnel(self, local_port, remote_port=None, remote_host='localhost'):
        warnings.warn('SSHHook.create_tunnel is deprecated, Pleaseuse get_tunnel() instead. But please note that theorder of the parameters have changedThis method will be removed in Airflow 2.0', category=DeprecationWarning)
