...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/contrib/hooks/gcs_hook.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 23367 bytes
import gzip as gz, os, shutil, warnings
...
class GoogleCloudStorageHook(GoogleCloudBaseHook):
    ...
    def get_conn(self):
        ...
        """
        if not self._conn:
            self._conn = storage.Client(credentials=(self._get_credentials()), project=(self.project_id))
        return self._conn
    ...
    def copy(self, source_bucket, source_object, destination_bucket=None, destination_object=None):
        ...
        if source_bucket == destination_bucket:
            if source_object == destination_object:
                raise ValueError('Either source/destination bucket or source/destination object must be different, not both the same: bucket=%s, object=%s' % (
        ...
        destination_object = source_bucket.copy_blob(blob=source_object,
    ...
    def rewrite(self, source_bucket, source_object, destination_bucket, destination_object=None):
        ...
        destination_bucket = client.bucket(destination_bucket)
        token, bytes_rewritten, total_bytes = destination_bucket.blob(blob_name=destination_object).rewrite(source=source_object)
        ...
        self.log.info('Object %s in bucket %s copied to object %s in bucket %s', source_object.name, source_bucket.name, destination_object, destination_bucket.name)
    ...
    def upload(self, bucket, object, filename, mime_type='application/octet-stream', gzip=False, multipart=None, num_retries=None):
        """
        ...
        if multipart is not None:
            warnings.warn("'multipart' parameter is deprecated. It is handled automatically by the Storage client", DeprecationWarning)
        else:
            if num_retries is not None:
                warnings.warn("'num_retries' parameter is deprecated. It is handled automatically by the Storage client", DeprecationWarning)
            if gzip:
    ...
    def list(self, bucket, versions=None, maxResults=None, prefix=None, delimiter=None):
        ...
        pageToken = None
        while 1:
            blobs = bucket.list_blobs(max_results=maxResults,
    ...
    def get_crc32c(self, bucket, object):
        ...
        :type object: str
        """
        self.log.info('Retrieving the crc32c checksum of object: %s in bucket: %s', object, bucket)
        client = self.get_conn()
        bucket = client.bucket(bucket)
        blob = bucket.get_blob(blob_name=object)
    ...
    def insert_object_acl(self, bucket, object_name, entity, role, generation=None, user_project=None):
        """
    ...
    def compose(self, bucket, source_objects, destination_object, num_retries=None):
        ...
        """
        if num_retries is not None:
            warnings.warn("'num_retries' parameter is Deprecated. Retries are now handled automatically", DeprecationWarning)
        else:
        ...
        self.log.info('Composing %s to %s in the bucket %s', source_objects, destination_object, bucket)
        client = self.get_conn()
        bucket = client.bucket(bucket)
        destination_blob = bucket.blob(destination_object)
        destination_blob.compose(sources=[bucket.blob(blob_name=source_object) for source_object in source_objects])
