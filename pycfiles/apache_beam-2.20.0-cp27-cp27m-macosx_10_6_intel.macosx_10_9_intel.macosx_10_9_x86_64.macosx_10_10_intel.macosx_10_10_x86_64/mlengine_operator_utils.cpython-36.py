...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/contrib/utils/mlengine_operator_utils.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 10924 bytes
import base64, json, os, re, dill
...
def create_evaluate_ops(task_prefix, data_format, input_paths, prediction_path, metric_fn_and_keys, validate_fn, batch_prediction_job_id=None, project_id=None, region=None, dataflow_options=None, model_uri=None, model_name=None, version_name=None, dag=None):
    ...
    if not re.match('^[a-zA-Z][-A-Za-z0-9]*$', task_prefix):
        ...
        if dag is not None:
            if dag.default_args is not None:
    ...
    evaluate_prediction = MLEngineBatchPredictionOperator(task_id=(task_prefix + '-prediction'),
    ...
    evaluate_summary = DataFlowPythonOperator(task_id=(task_prefix + '-summary'),
      py_options=[
     '-m'],
      py_file='airflow.contrib.utils.mlengine_prediction_summary',
      dataflow_default_options=dataflow_options,
      options={'prediction_path':prediction_path, 
    ...
    evaluate_validation = PythonOperator(task_id=(task_prefix + '-validation'),
    ...
    return (
     evaluate_prediction, evaluate_summary, evaluate_validation)