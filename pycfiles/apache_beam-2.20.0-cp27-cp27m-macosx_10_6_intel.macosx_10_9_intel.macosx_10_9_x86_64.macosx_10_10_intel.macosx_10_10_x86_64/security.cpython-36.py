...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/www_rbac/security.py
# Compiled at: 2019-09-11 03:47:35
# Size of source mod 2**32: 16460 bytes
from flask import g
...
class AirflowSecurityManager(SecurityManager, LoggingMixin):
    ...
    def init_role(self, role_name, role_vms, role_perms):
        ...
        pvms = [p for p in pvms if p.permission if p.view_menu]
    ...
    def get_all_permissions_views(self):
        ...
        for role in self.get_user_roles():
            perms_views.update({(perm_view.permission.name, perm_view.view_menu.name) for perm_view in role.permissions})
    ...
    def _has_perm(self, permission_name, view_menu_name):
        ...
        if hasattr(self, 'perms'):
            if (
             permission_name, view_menu_name) in self.perms:
                return True
    ...
    def has_all_dags_access(self):
        ...
        """
        return self._has_role(['Admin', 'Viewer', 'Op', 'User']) or self._has_perm('can_dag_read', 'all_dags') or self._has_perm('can_dag_edit', 'all_dags')
    ...
    def clean_perms(self):
        ...
        pvms = sesh.query(sqla_models.PermissionView).filter(or_(sqla_models.PermissionView.permission == None, sqla_models.PermissionView.view_menu == None))
        deleted_count = 0
        for pvm in pvms:
            sesh.delete(pvm)
            deleted_count += 1
    ...
    def _merge_perm(self, permission_name, view_menu_name):
        ...
        if permission:
            if view_menu:
                pv = self.get_session.query(self.permissionview_model).filter_by(permission=permission,
    ...
    def create_custom_dag_permission_view(self, session=None):
        ...
        def merge_pv(perm, view_menu):
            if view_menu:
                if perm:
                    if (view_menu, perm) not in all_pvs:
        ...
        all_dags_models = session.query(models.DagModel).filter(or_(models.DagModel.is_active, models.DagModel.is_paused)).filter(~models.DagModel.is_subdag).all()
        ...
        all_perm_view_by_user = session.query(ab_perm_view_role).join(perm_view, perm_view.id == ab_perm_view_role.columns.permission_view_id).filter(ab_perm_view_role.columns.role_id == user_role.id).join(view_menu).filter(perm_view.view_menu_id != dag_vm.id)
        ...
        for role in dag_role:
            ...
            existing_perms_views = set([pv.permission_view_id for pv in existing_perm_view_by_user])
