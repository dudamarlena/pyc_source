...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/contrib/hooks/databricks_hook.py
...
class DatabricksHook(BaseHook):
    ...
    def __init__(self, databricks_conn_id='databricks_default', timeout_seconds=180, retry_limit=3, retry_delay=1.0):
        """
    ...
    def _do_api_call(self, endpoint_info, json):
        ...
        url = 'https://{host}/{endpoint}'.format(host=(self._parse_host(self.databricks_conn.host)),
          endpoint=endpoint)
...
def _retryable_error(exception):
    return isinstance(exception, requests_exceptions.ConnectionError) or isinstance(exception, requests_exceptions.Timeout) or exception.response is not None and exception.response.status_code >= 500
...
class RunState:
    ...
    def __eq__(self, other):
        return self.life_cycle_state == other.life_cycle_state and self.result_state == other.result_state and self.state_message == other.state_message
