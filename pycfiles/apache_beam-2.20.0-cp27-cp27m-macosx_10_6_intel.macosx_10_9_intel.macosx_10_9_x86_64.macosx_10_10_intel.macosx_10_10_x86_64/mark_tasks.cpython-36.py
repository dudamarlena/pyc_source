...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/api/common/experimental/mark_tasks.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 14337 bytes
__doc__ = 'Marks tasks APIs.'
import datetime
...
def _create_dagruns(dag, execution_dates, state, run_id_template):
    ...
    for date in dates_to_create:
        dag_run = dag.create_dagrun(run_id=(run_id_template.format(date.isoformat())),
          execution_date=date,
...
@provide_session
def set_state(tasks, execution_date, upstream=False, downstream=False, future=False, past=False, state=State.SUCCESS, commit=False, session=None):
    """
...
def all_subdag_tasks_query(sub_dag_run_ids, session, state, confirmed_dates):
    ...
    qry_sub_dag = session.query(TaskInstance).filter(TaskInstance.dag_id.in_(sub_dag_run_ids), TaskInstance.execution_date.in_(confirmed_dates)).filter(or_(TaskInstance.state.is_(None), TaskInstance.state != state))
    return qry_sub_dag


def get_all_dag_task_query(dag, session, state, task_ids, confirmed_dates):
    """Get all tasks of the main dag that will be affected by a state change"""
    qry_dag = session.query(TaskInstance).filter(TaskInstance.dag_id == dag.dag_id, TaskInstance.execution_date.in_(confirmed_dates), TaskInstance.task_id.in_(task_ids)).filter(or_(TaskInstance.state.is_(None), TaskInstance.state != state))
    return qry_dag
        ...
        for task_id in task_ids:
            if not current_dag.has_task(task_id):
                ...
                if isinstance(current_task, SubDagOperator):
                    dag_runs = _create_dagruns((current_task.subdag), execution_dates=confirmed_dates,
                      state=(State.RUNNING),
...
def _set_dag_run_state(dag_id, execution_date, state, session=None):
    ...
    :param session: database session
    """
    dag_run = session.query(DagRun).filter(DagRun.dag_id == dag_id, DagRun.execution_date == execution_date).one()
    dag_run.state = state
...
def set_dag_run_state_to_success(dag, execution_date, commit=False, session=None):
    ...
    if not dag or not execution_date:
        ...
        return set_state(tasks=(dag.tasks), execution_date=execution_date, state=(State.SUCCESS),
          commit=commit,
          session=session)
...
def set_dag_run_state_to_failed(dag, execution_date, commit=False, session=None):
    ...
    if not dag or not execution_date:
        ...
        tis = session.query(TaskInstance).filter(TaskInstance.dag_id == dag.dag_id, TaskInstance.execution_date == execution_date, TaskInstance.task_id.in_(task_ids)).filter(TaskInstance.state == State.RUNNING)
        ...
        return set_state(tasks=tasks, execution_date=execution_date, state=(State.FAILED),
          commit=commit,
          session=session)
