...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/migrations/versions/e3a246e0dc1_current_schema.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 12374 bytes
__doc__ = 'current schema\n\nRevision ID: e3a246e0dc1\nRevises:\nCreate Date: 2015-08-18 16:35:00.883495\n\n'
...
def upgrade():
    ...
    if 'known_event_type' not in tables:
        op.create_table('known_event_type', sa.Column('id', (sa.Integer()), nullable=False), sa.Column('know_event_type', sa.String(length=200), nullable=True), sa.PrimaryKeyConstraint('id'))
    ...
    if 'user' not in tables:
        op.create_table('user', sa.Column('id', (sa.Integer()), nullable=False), sa.Column('username', sa.String(length=250), nullable=True), sa.Column('email', sa.String(length=500), nullable=True), sa.PrimaryKeyConstraint('id'), sa.UniqueConstraint('username'))
    if 'variable' not in tables:
        op.create_table('variable', sa.Column('id', (sa.Integer()), nullable=False), sa.Column('key', sa.String(length=250), nullable=True), sa.Column('val', (sa.Text()), nullable=True), sa.PrimaryKeyConstraint('id'), sa.UniqueConstraint('key'))
    ...
    if 'known_event' not in tables:
        op.create_table('known_event', sa.Column('id', (sa.Integer()), nullable=False), sa.Column('label', sa.String(length=200), nullable=True), sa.Column('start_date', (sa.DateTime()), nullable=True), sa.Column('end_date', (sa.DateTime()), nullable=True), sa.Column('user_id', (sa.Integer()), nullable=True), sa.Column('known_event_type_id', (sa.Integer()), nullable=True), sa.Column('description', (sa.Text()), nullable=True), sa.ForeignKeyConstraint(['known_event_type_id'], [
         'known_event_type.id']), sa.ForeignKeyConstraint(['user_id'], ['user.id']), sa.PrimaryKeyConstraint('id'))
    if 'xcom' not in tables:
        op.create_table('xcom', sa.Column('id', (sa.Integer()), nullable=False), sa.Column('key', sa.String(length=512), nullable=True), sa.Column('value', (sa.PickleType()), nullable=True), sa.Column('timestamp',
          (sa.DateTime()),
          default=(func.now()),
          nullable=False), sa.Column('execution_date', (sa.DateTime()), nullable=False), sa.Column('task_id', sa.String(length=250), nullable=False), sa.Column('dag_id', sa.String(length=250), nullable=False), sa.PrimaryKeyConstraint('id'))
