...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/models/taskinstance.py
# Compiled at: 2019-09-11 03:47:34
# Size of source mod 2**32: 54098 bytes
import copy, functools, getpass, hashlib, logging, math, os, signal, time
from datetime import timedelta
from typing import Optional
from urllib.parse import quote
import dill, lazy_object_proxy, pendulum
from six.moves.urllib.parse import quote_plus
from sqlalchemy import Column, String, Float, Integer, PickleType, Index, func
from sqlalchemy.orm import reconstructor
from sqlalchemy.orm.session import Session
from airflow import configuration, settings
from airflow.exceptions import AirflowException, AirflowTaskTimeout, AirflowSkipException, AirflowRescheduleException
...
class TaskInstance(Base, LoggingMixin):
    ...
    def command(self, mark_success=False, ignore_all_deps=False, ignore_depends_on_past=False, ignore_task_deps=False, ignore_ti_state=False, local=False, pickle_id=None, raw=False, job_id=None, pool=None, cfg_path=None):
        """
        Returns a command that can be executed anywhere where airflow is
        installed. This command is part of the message sent to executors by
        the orchestrator.
        """
        return ' '.join(self.command_as_list(mark_success=mark_success,
    ...
    def command_as_list(self, mark_success=False, ignore_all_deps=False, ignore_task_deps=False, ignore_depends_on_past=False, ignore_ti_state=False, local=False, pickle_id=None, raw=False, job_id=None, pool=None, cfg_path=None):
        ...
        elif should_pass_filepath:
            if dag.full_filepath:
                path = dag.full_filepath
        else:
            path = None
        return TaskInstance.generate_command((self.dag_id),
          (self.task_id),
          (self.execution_date),
    ...
    def log_filepath(self):
        ...
        return '{log}/{dag_id}/{task_id}/{iso}.log'.format(log=log,
          dag_id=(self.dag_id),
    ...
    def log_url(self):
        ...
        relative_url = '/log?execution_date={iso}&task_id={task_id}&dag_id={dag_id}'.format(iso=(quote_plus(iso)),
          task_id=(quote_plus(self.task_id)),
    ...
    def mark_success_url(self):
        ...
        base_url = configuration.conf.get('webserver', 'BASE_URL')
        return base_url + '/success?task_id={task_id}&dag_id={dag_id}&execution_date={iso}&upstream=false&downstream=false'.format(task_id=(self.task_id),
          dag_id=(self.dag_id),
          iso=iso)
    ...
    @property
    def previous_start_date_success(self):
    ...
    def _check_and_change_state_before_execution(self, verbose=True, ignore_all_deps=False, ignore_depends_on_past=False, ignore_task_deps=False, ignore_ti_state=False, mark_success=False, test_mode=False, job_id=None, pool=None, session=None):
        ...
        if not ignore_all_deps:
            if not ignore_ti_state:
                if self.state == State.SUCCESS:
                    Stats.incr('previously_succeeded', 1, 1)
        queue_dep_context = DepContext(deps=QUEUE_DEPS,
          ignore_all_deps=ignore_all_deps,
          ignore_ti_state=ignore_ti_state,
          ignore_depends_on_past=ignore_depends_on_past,
          ignore_task_deps=ignore_task_deps)
        if not self.are_dependencies_met(dep_context=queue_dep_context,
          session=session,
          verbose=True):
            session.commit()
            return False
        ...
        dep_context = DepContext(deps=(RUN_DEPS - QUEUE_DEPS),
          ignore_all_deps=ignore_all_deps,
          ignore_depends_on_past=ignore_depends_on_past,
          ignore_task_deps=ignore_task_deps,
          ignore_ti_state=ignore_ti_state)
        runnable = self.are_dependencies_met(dep_context=dep_context,
          session=session,
          verbose=True)
        ...
        try:
            if not mark_success:
                ...
                Stats.timing('dag.{dag_id}.{task_id}.duration'.format(dag_id=(task_copy.dag_id),
                  task_id=(task_copy.task_id)), duration)
    ...
    @provide_session
    def run(self, verbose=True, ignore_all_deps=False, ignore_depends_on_past=False, ignore_task_deps=False, ignore_ti_state=False, mark_success=False, test_mode=False, job_id=None, pool=None, session=None):
        res = self._check_and_change_state_before_execution(verbose=verbose,
          ignore_all_deps=ignore_all_deps,
          ignore_depends_on_past=ignore_depends_on_past,
          ignore_task_deps=ignore_task_deps,
          ignore_ti_state=ignore_ti_state,
    ...
    @provide_session
    def _handle_reschedule(self, actual_start_date, reschedule_exception, test_mode=False, context=None, session=None):
        if test_mode:
            return
        self.end_date = timezone.utcnow()
        self.set_duration()
        session.add(TaskReschedule(self.task, self.execution_date, self._try_number, actual_start_date, self.end_date, reschedule_exception.reschedule_date))
    ...
    def get_template_context(self, session=None):
        ...
        if hasattr(task, 'dag'):
            ...
            from airflow.models.dagrun import DagRun
            dag_run = session.query(DagRun).filter_by(dag_id=(task.dag.dag_id),
              execution_date=(self.execution_date)).first()
        ...
        ti_key_str = '{dag_id}__{task_id}__{ds_nodash}'.format(dag_id=(task.dag_id),
        ...
        class VariableAccessor:
            """TaskInstance.get_template_context.<locals>.VariableAccessor"""
        ...
        class VariableJsonAccessor:
            """TaskInstance.get_template_context.<locals>.VariableJsonAccessor"""
        ...
        return {'dag':task.dag, 
         ...
         'prev_execution_date_success':lazy_object_proxy.Proxy(lambda : self.previous_execution_date_success), 
    ...
    def email_alert(self, exception):
        ...
        jinja_context.update(dict(exception=exception,
          exception_html=exception_html,
          try_number=(self.try_number - 1),
          max_tries=(self.max_tries)))
        jinja_env = self.task.get_template_env()
        default_subject = 'Airflow alert: {{ti}}'
        default_html_content = 'Try {{try_number}} out of {{max_tries + 1}}<br>Exception:<br>{{exception_html}}<br>Log: <a href="{{ti.log_url}}">Link</a><br>Host: {{ti.hostname}}<br>Log file: {{ti.log_filepath}}<br>Mark success: <a href="{{ti.mark_success_url}}">Link</a><br>'
    ...
    def xcom_pull(self, task_ids=None, dag_id=None, key=XCOM_RETURN_KEY, include_prior_dates=False):
        ...
        if dag_id is None:
            dag_id = self.dag_id
        pull_fn = functools.partial((XCom.get_one),
    ...
    def get_num_running_task_instances(self, session):
        TI = TaskInstance
        return session.query(TI).filter(TI.dag_id == self.dag_id, TI.task_id == self.task_id, TI.state == State.RUNNING).count()
