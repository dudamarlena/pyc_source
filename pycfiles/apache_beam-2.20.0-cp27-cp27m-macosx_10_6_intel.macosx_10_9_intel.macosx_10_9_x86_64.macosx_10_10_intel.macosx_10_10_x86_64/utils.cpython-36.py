...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/airflow/www_rbac/utils.py
# Compiled at: 2019-09-11 03:47:35
# Size of source mod 2**32: 15321 bytes
from future import standard_library
standard_library.install_aliases()
import inspect, json, time, markdown, re, zipfile, os, io
...
import flask_appbuilder.models.sqla.filters as fab_sqlafilters, sqlalchemy as sqla
...
DEFAULT_SENSITIVE_VARIABLE_FIELDS = ('password', 'secret', 'passwd', 'authorization',
                                     'api_key', 'apikey', 'access_token')

def should_hide_value_for_key(key_name):
    if key_name:
        config_set = configuration.conf.getboolean('admin', 'hide_sensitive_variable_fields')
...
def generate_pages(current_page, num_of_pages, search=None, showPaused=None, window=7):
    """
    ...
    output = [
     Markup('<ul class="pagination" style="margin-top:0px;">')]
    is_disabled = 'disabled' if current_page <= 0 else ''
    output.append(first_node.format(href_link=('?{}'.format(get_params(page=0, search=search,
      showPaused=showPaused))),
      disabled=is_disabled))
    page_link = void_link
    if current_page > 0:
        page_link = '?{}'.format(get_params(page=(current_page - 1), search=search,
          showPaused=showPaused))
    else:
        ...
        if current_page <= mid or num_of_pages < window:
            ...
            if mid < current_page < last_page - mid:
    ...
    for page in pages:
        vals = {'is_active':'active' if is_current(current_page, page) else '', 
         'href_link':void_link if is_current(current_page, page) else '?{}'.format(get_params(page=page, search=search,
           showPaused=showPaused)), 
         'page_num':page + 1}
        output.append((page_node.format)(**vals))

    is_disabled = 'disabled' if current_page >= num_of_pages - 1 else ''
    page_link = void_link if current_page >= num_of_pages - 1 else '?{}'.format(get_params(page=(current_page + 1), search=search,
      showPaused=showPaused))
    output.append(next_node.format(href_link=page_link, disabled=is_disabled))
    output.append(last_node.format(href_link=('?{}'.format(get_params(page=last_page, search=search,
      showPaused=showPaused))),
      disabled=is_disabled))
...
def epoch(dttm):
    ...
    return (
     int(time.mktime(dttm.timetuple())) * 1000,)
...
def json_response(obj):
    ...
    return Response(response=json.dumps(obj,
      indent=4, cls=AirflowJsonEncoder),
      status=200,
      mimetype='application/json')


ZIP_REGEX = re.compile('((.*\\.zip){})?(.*)'.format(re.escape(os.sep)))
...
def task_instance_link(attr):
    ...
    url_root = url_for('Airflow.graph',
      dag_id=dag_id,
      root=task_id,
      execution_date=(execution_date.isoformat()))
...
def state_token(state):
    ...
    return Markup('<span class="label" style="background-color:{color};">{state}</span>').format(color=color,
      state=state)
...
def get_attr_renderer():
    attr_renderer = {'bash_command':lambda x: render(x, lexers.BashLexer), 
     ...
     'python_callable':lambda x: render(inspect.getsource(x), lexers.PythonLexer)}
    ...
    conversion_table = (
     (
      ...
      [UtcAwareFilterEqual,
       ...
       UtcAwareFilterNotEqual]),) + fab_sqlafilters.SQLAFilterConverter.conversion_table
