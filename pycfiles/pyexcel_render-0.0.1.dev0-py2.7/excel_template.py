...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/pyexcel_render/excel_template.py
# Compiled at: 2016-11-22 01:24:21
"""
...
partter = '{{([^\\s]+)}}'
for_partter = '{%Foreach.([^\\s]+)%}'
for_end = '{%Forend.([^\\s]+)%}'
fix_start = '{%FixStart.([^\\s]+)%}'
fix_end = '{%FixEnd.([^\\s]+)%}'
table_partter = '{%Table.([^\\s]+)%}'
