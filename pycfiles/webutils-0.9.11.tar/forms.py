...
# [GCC 8.4.0]
# Embedded file name: /Users/pjs/python-modules/webutils/baseacct/forms.py
...
class ResetForm(forms.Form):
    email = forms.EmailField(max_length=125, label=_('Email Address'))

    def clean_email(self):
        if 'email' in self.cleaned_data:
            email = self.cleaned_data['email']
            if not User.objects.filter(email__iexact=email).count() > 0:
                raise forms.ValidationError('There is no account registered to this email address.')
            return email.lower()
