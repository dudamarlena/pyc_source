...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/niflow_manager/cli/build.py
# Compiled at: 2020-03-13 12:49:48
# Size of source mod 2**32: 6631 bytes
import subprocess as sp, tempfile, os, json
from pathlib import Path
from copy import deepcopy
import yaml, click, neurodocker as ndr
...
def neurodocker_dict(workflow_path):
    ...
    elif 'required_env' not in params['build']:
        ...
        for key, spec in params_env.items():
            if key == 'base':
                ...
                if key in VALID_DOCKER_KEYS:
                    ...
                    raise Exception(f"{key} is not a valid key, must be from the list {VALID_DOCKER_KEYS}")
            instructions.append(this_instruction)

        instructions.insert(1, ('install', ['git']))
        post_build = params.get('post_build', {})
        post_build['copy'] = post_build or [
         '.', '/nfm']
        post_build['miniconda'] = {'pip_install': ['https://github.com/niflows/niflow-manager/tarball/master']}
        post_build['run_bash'] = '/opt/miniconda-latest/envs/niflows/bin/nfm install /nfm'
        if params_entry:
            post_build['entrypoint'] = params_entry
        else:
            post_build['entrypoint'] = f"/opt/miniconda-latest/envs/niflows/bin/{workflow_path.name}"
    ...
    return {'pkg_manager':pkg_manager, 
...
def docker_image(workflow_path, working_dir=None):
    ...
    if working_dir:
        ...
        working_dir = Path(tempfile.mkdtemp(prefix=f"nfm-{workflow_path.name}")).absolute()
    dockerfile = working_dir / 'Dockerfile'
    if dockerfile.exists():
        dockerfile.unlink()
    jsonfile = working_dir / 'nrd_spec.json'
    if jsonfile.exists():
        jsonfile.unlink()
    ...
    build_image(dockerfile=dockerfile,
