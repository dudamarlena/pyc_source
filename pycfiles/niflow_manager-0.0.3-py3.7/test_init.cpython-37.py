...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/niflow_manager/cli/tests/test_init.py
...
@pytest.mark.parametrize('directory, organization, workflow, override, target, abort', [
...
def test_init_python(gitconfig):
    runner = CliRunner()
    args = [
     'init', 'niflow-org-wf', '--language', 'python']
    with runner.isolated_filesystem():
        inputs = None
    ...
    if gitconfig == os.devnull:
        ...
        if not 'Enter package author email: ' in result.stdout:
            raise AssertionError
    else:
...
def test_init_python_bidsapp():
    runner = CliRunner()
    args = [
     'init', 'niflow-org-bidsapp', '--language', 'python', '--bids', '1.0']
    with runner.isolated_filesystem():
...
def test_init_bidsapp(language):
    runner = CliRunner()
    args = [
     'init', 'niflow-org-bidsapp', '--bids', '1.0']
    ...
    if not result.exit_code != 0:
        raise AssertionError
    elif language is None and not 'language-specific' in result.exception.args[0]:
        raise AssertionError
    else:
