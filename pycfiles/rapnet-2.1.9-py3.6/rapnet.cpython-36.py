...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/rapnet.py
# Compiled at: 2018-06-26 01:14:47
# Size of source mod 2**32: 10720 bytes
...
class RapNetAPI:
    __doc__ = 'API Client Library class for RapNet'
    SHAPES = ['round', 'pear', '']
    ...
    def _get_token(self):
        """Get token Smartly."""
        if self.token is None or self.timestamp is None or datetime.datetime.utcnow() - self.timestamp > datetime.timedelta(minutes=58):
            try:
    ...
    def _get_data(self, url, body={}, mode='BASIC', header='self', raw=False, data={}, extras={}):
        """Base function for flexible request."""
        if header == 'self':
            header = self.FORM_HEADER
        else:
            if mode == 'BASIC':
                json_body, params = {'request': {'header':self._auth(), 
                             'body':body}}, {}
            else:
                if mode == 'TOKEN':
                    json_body, params = {'request': {'body': body}}, self._auth('TOKEN')
                params.update(data)
                response = (requests.post)(
 url, json=json_body, data=params, 
                 headers=header, **extras).text
                if raw:
                    return response
                data = json.loads(response.replace('\n', '').replace('\r', '').replace('\t', ''))['response']
                if data['header']['error_code'] == 0:
                    return data['body']
            if data['header']['error_code'] == 4001:
                return {}
        raise RuntimeWarning('{}: {}'.format(str(data['header']['error_code']), data['header']['error_message']))
    ...
    def get_price_changes(self, shape='round'):
        ...
        if shape in self.SHAPES:
            return self._get_data((self.BASE_URL + self.PRICE_CHANGES_URL), body={'shape': shape})
        raise RuntimeWarning('Invalid Shape Choose from {}.'.format(', '.join(self.SHAPES)))
 ...
 'clarity':'VS2'}):
        ...
        if 'shape' not in search_params or search_params['shape'] not in self.SHAPES:
            search_params['shape'] = 'round'
        if 'color' not in search_params or search_params['color'] not in self.COLORS:
            search_params['color'] = 'E'
        if 'clarity' not in search_params or search_params['clarity'] not in self.CLARITIES:
            search_params['clarity'] = 'VS2'
        if 'size' not in search_params:
            search_params['size'] = 2.1
        try:
    ...
    def get_diamonds_only(self, page, search_type):
        return self.get_diamonds_list(params={'page_number':page, 
         'page_size':50, 
         'search_type':search_type})['diamonds']

    def get_all_diamonds(self, search_type='White', datafile=None, verbose=False, multiprocessing=False, pool=0):
        ...
        if verbose is True:
            ...
            if multiprocessing is False:
                for page in range(2, total_pages + 1):
                    data.append(self.get_diamonds_list(params={'page_number':page, 
                     'page_size':50, 
                     'search_type':search_type})['diamonds'])
                ...
                if pool == 0:
                    ...
                    page_pool = Pool(pool)
                data = data + page_pool.map(lambda x: self.get_diamonds_only(x, search_type), list(range(2, total_pages + 1)))
        if datafile is None:
            return data
    ...
    def upload_string(self, datastring):
        ...
        """
        return self._get_data((self.BASE_URL + self.UPLOAD_STRING_URL), mode='TOKEN',
          data={'UploadCSVString':datastring, 
         'ReplaceAll':'false'},
          raw=True)
    ...
    def upload_csv(self, uploadfile):
        ...
        https://technet.rapaport.com/Info/LotUpload/Upload_HTTP.aspx
        """
        return self._get_data((self.BASE_URL + self.UPLOAD_File_URL), mode='TOKEN',
          data={'ReplaceAll': 'false'},
          raw=True,
          extras={'files': {'file': open(uploadfile, 'rb')}})