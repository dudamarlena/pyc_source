...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xicam\core\execution\workflow.py
# Compiled at: 2018-07-06 18:13:02
# Size of source mod 2**32: 12874 bytes
...
class Workflow(object):
            ...
            for input in inputprocess.inputs.values():
                ...
                for outputprocess in reversed(self.processes[:self.processes.index(inputprocess)]):
                    for output in outputprocess.outputs.values():
                        if output.name == input.name and matchness < 1:
                            bestmatch = output
                            matchness = 1
                            if output.type == input.type and matchness < 2:
                                bestmatch = output
                                matchness = 2

                if bestmatch:
                    bestmatch.connect(input)
                    msg.logMessage(f"connected {bestmatch.parent.__class__.__name__}:{bestmatch.name} to {input.parent.__class__.__name__}:{input.name}",
                      level=(msg.DEBUG))
    ...
    def execute(self, connection, callback_slot=None, finished_slot=None, except_slot=None, default_exhandle=True, lock=None, fill_kwargs=True, threadkey=None, **kwargs):
        """
        ...
        future = QThreadFuture((execution.executor.execute), self, callback_slot=callback_slot,
    ...
    def execute_all(self, connection, callback_slot=None, finished_slot=None, except_slot=None, default_exhandle=True, lock=None, fill_kwargs=True, threadkey=None, **kwargs):
        """
        Execute this workflow on the specified host. Connection will be a Connection object (WIP) keeping a connection
        to a compute resource, include connection.hostname, connection.username...
        ...
        future = QThreadFutureIterator(executeiterator, self, callback_slot=callback_slot,
          finished_slot=finished_slot,
          default_exhandle=default_exhandle,
          lock=lock,
          threadkey=threadkey)
