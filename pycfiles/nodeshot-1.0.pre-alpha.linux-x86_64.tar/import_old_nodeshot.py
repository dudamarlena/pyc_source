...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/nodeshot/interop/oldimporter/management/commands/import_old_nodeshot.py
# Compiled at: 2015-04-12 14:33:49
...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
     make_option('--noinput', action='store_true', dest='noinput', default=False, help='Do not prompt for user intervention and use default settings'),
     make_option('--nodelete', action='store_true', dest='nodelete', default=False, help='Do not delete imported data if any uncaught exception occurs'))
    ...
    def extract_users(self):
        ...
        for node in self.old_nodes:
            ...
            if node.email not in users_dict:
                users_dict[node.email] = {'owner': node.owner}
    ...
    def import_nodes(self):
        ...
        for old_node in self.old_nodes:
            ...
            if len(intersecting_layers) > 1:
                ...
                if 2 > len(intersecting_layers) > 0:
                    node.layer = intersecting_layers[0]
                elif self.default_layer is False:
    ...
    def import_devices(self):
        ...
        for old_device in self.old_devices:
            ...
            device.data = {'model': old_device.type, 
    ...
    def import_interfaces(self):
        ...
        for old_interface in self.old_interfaces:
            interface_dict = {'id': old_interface.id, 
            ...
            if old_interface.type == 'eth':
                ...
                if old_interface.type == 'wifi':
                    ...
                    if old_interface.essid or old_interface.bssid:
                        vap = Vap(**{'interface_id': old_interface.id, 
                           'essid': old_interface.essid, 
                           'bssid': old_interface.bssid})
                        try:
                            v = Vap.objects.get(Q(interface_id=old_interface.id) & (Q(essid=old_interface.essid) | Q(bssid=old_interface.bssid)))
                            vap.id = v.id
                ...
                if old_interface.ipv4_address:
                    ...
                    ipv4 = Ip(**{'interface_id': old_interface.id, 
                ...
                if old_interface.ipv6_address:
                    ...
                    ipv6 = Ip(**{'interface_id': old_interface.id, 
    ...
    def import_contacts(self):
        ...
        for old_contact in self.old_contacts:
            ...
            contact = Inward(**{'id': old_contact.id, 
