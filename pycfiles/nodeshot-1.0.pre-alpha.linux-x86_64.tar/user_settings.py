...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/nodeshot/community/notifications/models/user_settings.py
...
def add_notifications(myclass):
    ...
    for key, value in TEXTS.items():
        ...
        if field_type == 'boolean':
            ...
            field = models.IntegerField(_(key), default=DEFAULT_DISTANCE, help_text=_('-1 (less than 0): disabled; 0: enabled for all;                            1 (less than 0): enabled for those in the specified distance range (km)'))
...
class UserWebNotificationSettings(models.Model):
    ...
    """
    user = models.OneToOneField(settings.AUTH_USER_MODEL, verbose_name=_('user'), related_name='web_notification_settings')
...
class UserEmailNotificationSettings(models.Model):
    ...
    """
    user = models.OneToOneField(settings.AUTH_USER_MODEL, verbose_name=_('user'), related_name='email_notification_settings')
