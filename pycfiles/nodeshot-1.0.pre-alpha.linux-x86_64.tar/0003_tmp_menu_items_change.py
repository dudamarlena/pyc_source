...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/nodeshot/core/cms/migrations/0003_tmp_menu_items_change.py
...
class Migration(DataMigration):
    ...
    NEW_STRING = "javascript:$.createModal({message:'not implemented yet'})"

    def forwards(self, orm):
        """Write your forwards methods here."""
        for menuitem in orm['cms.menuitem'].objects.filter(url=self.OLD_STRING):
            menuitem.url = self.NEW_STRING
            menuitem.save()

    def backwards(self, orm):
        """Write your backwards methods here."""
        for menuitem in orm['cms.menuitem'].objects.filter(url=self.NEW_STRING):
