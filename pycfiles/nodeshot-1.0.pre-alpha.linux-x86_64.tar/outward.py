...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/nodeshot/community/mailing/models/outward.py
# Compiled at: 2015-03-25 13:08:56
import socket, time
...
class Outward(BaseDate):
    ...
    is_scheduled = models.SmallIntegerField(_('schedule sending'), choices=SCHEDULE_CHOICES, default=0)
    scheduled_date = models.DateField(_('scheduled date'), blank=True, null=True)
    scheduled_time = models.CharField(_('scheduled time'), max_length=20, choices=OUTWARD_SCHEDULING, default=OUTWARD_SCHEDULING[0][0], blank=True)
    ...
    def get_recipients(self):
        ...
        if not self.is_filtered:
            ...
            elif FILTERS.get('groups') in self.filters and FILTERS.get('layers') not in self.filters:
    ...
    def clean(self, *args, **kwargs):
        ...
        if self.is_filtered is 1 and (len(self.filters) < 1 or self.filters == [''] or self.filters == [''] or self.filters == '' or self.filters is None):
            raise ValidationError(_('If "recipient filtering" is active one of the filtering options should be selected'))
        if self.is_filtered is 1 and FILTERS.get('groups') in self.filters and (len(self.groups) < 1 or self.groups == [''] or self.groups == [''] or self.groups == '' or self.groups is None):
