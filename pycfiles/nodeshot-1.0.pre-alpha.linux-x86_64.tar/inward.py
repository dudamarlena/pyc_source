...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/nodeshot/community/mailing/models/inward.py
...
class Inward(BaseDate):
    ...
    def __unicode__(self):
        return _('Message from %(from)s to %(to)s') % {'from': self.from_name, 'to': self.content_type}
    ...
    def send(self):
        ...
        if self.content_type.name == 'node':
            ...
            if self.content_type.name == 'layer':
                to = [
                 self.to.email]
                ...
                to = [
                 self.to.email]
            context = {'sender_name': self.from_name, 
            ...
            email = EmailMessage(_('Contact request from %(sender_name)s - %(site)s') % context, message, settings.DEFAULT_FROM_EMAIL, to, headers={'Reply-To': self.from_email})
            import socket
            try:
                email.send()
                self.status = 1
