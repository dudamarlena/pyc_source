...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/nodeshot/core/layers/models/layer.py
...
class Layer(BaseDate):
    ...
    description = models.CharField(_('description'), max_length=250, blank=True, null=True, help_text=_('short description of this layer'))
    text = models.TextField(_('extended text'), blank=True, null=True, help_text=_('extended description, specific instructions, links, ecc.'))
    is_published = models.BooleanField(_('published'), default=True)
    is_external = models.BooleanField(_('is it external?'), default=False)
    area = models.GeometryField(_('area'), help_text=_('If a polygon is used nodes of this layer will have to be contained in it.                                                        If a point is used nodes of this layer can be located anywhere. Lines are not allowed.'))
    organization = models.CharField(_('organization'), max_length=255, blank=True, help_text=_('Organization which is responsible to manage this layer'))
    website = models.URLField(_('Website'), blank=True, null=True)
    email = models.EmailField(_('email'), blank=True, help_text=_('possibly an email address that delivers messages to all the active participants;\n                                          if you don\'t have such an email you can add specific users in the "mantainers" field'))
    mantainers = models.ManyToManyField(settings.AUTH_USER_MODEL, verbose_name=_('mantainers'), blank=True, help_text=_('you can specify the users who are mantaining this layer so they will receive emails from the system'))
    nodes_minimum_distance = models.IntegerField(default=NODES_MINIMUM_DISTANCE, help_text=_('minimum distance between nodes in meters, 0 means there is no minimum distance'))
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk and self.is_published != self._current_is_published:
            layer_is_published_changed.send(sender=self.__class__, instance=self, old_is_published=self._current_is_published, new_is_published=self.is_published)
