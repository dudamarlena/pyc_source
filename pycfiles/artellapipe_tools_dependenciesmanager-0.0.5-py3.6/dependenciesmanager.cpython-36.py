...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/tools/dependenciesmanager/widgets/dependenciesmanager.py
# Compiled at: 2020-04-15 10:43:07
# Size of source mod 2**32: 19397 bytes
"""
...
import os, traceback, logging.config
...
class DependenciesManager(artellapipe.ToolWidget, object):
    ...
    def _init(self):
        ...
        if self._init_file_path:
            if os.path.isfile(self._init_file_path):
    ...
    def _refresh_files(self):
        ...
        if invalid_deps:
            ...
            result = qtutils.show_question(None, 'Invalid dependencies found', 'Invalid dependencies paths found in current file. If you continue, dependencies manager will try to fix them. Are you sure you want to continue?')
            ...
            if invalid_deps:
                ...
                deps, invalid_deps = artellapipe.DepsMgr().get_dependencies(root_path)
                if invalid_deps:
                    qtutils.show_warning(None, 'Invalid dependencies found', 'Invalid dependencies paths found in current file.\n\n' + '\n'.join(invalid_deps) + 'Please fix paths manually!')
                    return
                invalid_deps_fixed = True
            else:
                invalid_deps_fixed = True
        if invalid_deps_found and invalid_deps_fixed:
            result = qtutils.show_question(None, 'File paths updated in file', 'File Paths fixed in the following file: "{}". Do you want to upload new version of the file to Artella server?'.format(root_path))
            if result == QMessageBox.Yes:
                artellapipe.FilesMgr().upload_working_version(root_path,
                  skip_saving=True, notify=False, comment='DependenciesManager: Fixed paths')
        if deps:
    ...
    def _on_browse(self, export_path=None):
        ...
        if not export_path or not os.path.isfile(export_path):
            export_path = tp.Dcc.select_file_dialog(title='Select Root Path',
    ...
    def _on_sync(self):
        ...
        if not checked_items:
            ...
            return
        open_file = True
        result = qtutils.show_question(None, 'Updating Dependencies', 'Do you want to open file after files are synced?')
        if result == QMessageBox.No:
            open_file = False
        working_folder = self._project.get_working_folder()
        locked = False
            ...
            try:
                ...
                for item in checked_items:
                    if not item.latest_path:
                        LOGGER.warning('Impossible to update dependency "{}". Latest path was not found ...'.format(item.path))
                    else:
                        if working_folder in item.path:
                        ...
                        if not os.path.exists(folder_to_sync):
                            LOGGER.warning('Impossible to update dependency "{}". Was impossible to sync latest path: "{}". You will need to update the dependency manually'.format(item.path, item.latest_path))
                        else:
