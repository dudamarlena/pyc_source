...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nilsimsa/test.py
...
def test_compatability():
    ...
    for fname in fnames:
        ...
        text = f.read()
        f.close()
        assert Nilsimsa(text).hexdigest() == orig_Nilsimsa(text).hexdigest() or False
