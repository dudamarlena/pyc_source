...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/flappy/events/eventdispatcher.py
...
class EventDispatcher(object):
    ...
    def addEventListener(self, etype, listener, use_capture=False, priority=0, use_weak_reference=False, once=False):
        if etype not in self.eventMap:
            self.eventMap[etype] = []
        lref = None
        if use_weak_reference:
            ...
            lref = listener
        self.eventMap[etype].append(Listener(lref, use_capture, priority, once))
        return
    ...
    def dispatchEvent(self, e):
        if self.suspended:
            ...
            if not self.eventMap or e.type not in self.eventMap:
                return False
            if e.target == None:
