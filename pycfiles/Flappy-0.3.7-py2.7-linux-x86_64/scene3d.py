...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/flappy/display3d/scene3d.py
...
class Scene3D(DirectRenderer):
    ...
    def __init__(self, antialias=0, enable_depth_and_stencil=False, name='Scene3D'):
    ...
    def clear(self, red=1.0, green=0.0, blue=0.0, alpha=1.0, depth=1.0, stencil=0, mask=Scene3DClearMask.ALL):
    ...
    def setVertexBufferAt(self, locname, vbuffer, offset=0, format=VertexBuffer3DFormat.FLOAT_1):
        ...
        dimension = 4
        try:
            dimension = {VertexBuffer3DFormat.BYTES_4: 4, VertexBuffer3DFormat.FLOAT_1: 1, 
               VertexBuffer3DFormat.FLOAT_2: 2, 
               VertexBuffer3DFormat.FLOAT_3: 3, 
               VertexBuffer3DFormat.FLOAT_4: 4}[format]
        except KeyError:
            raise ValueError('Buffer format "%s" is not supported' % str(format))

        self._do(gl.bindBuffer, gl.ARRAY_BUFFER, vbuffer.gl_buffer)
        self._do(gl.enableVertexAttribArray, loc)
        self._do(gl.vertexAttribPointer, loc, dimension, tp, False, vbuffer.data32_per_vertex * numbytes, offset * numbytes)
    ...
    def setTextureAt(self, locname, texture, texture_index=0):
        loc = gl.getUniformLocation(self._current_prog.gl_program, locname)
        assert 0 <= texture_index < 8, 'Texture index "%s" is not supported' % str(texture_index)
        self._do(gl.activeTexture, gl.TEXTURE0 + texture_index)
        self._bind_texture(texture)
        self._do(gl.uniform1i, loc, texture_index)
        self._do(gl.texParameter, gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE)
        self._do(gl.texParameter, gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE)
        self._do(gl.texParameter, gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR)
        self._do(gl.texParameter, gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR)
    ...
    def drawTriangles(self, ibuffer, first_index=0, num_triangles=-1):
        ...
        self._do(gl.drawElements, gl.TRIANGLES, num_indeces, gl.UNSIGNED_SHORT, first_index)

    def setRenderToTexture(self, texture, enable_depth_and_stencil=False, antialias=0, surface_selector=0):
        ...
        self._do(gl.texParameter, gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.REPEAT)
        self._do(gl.texParameter, gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.REPEAT)
        self._do(gl.texParameter, gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR)
        self._do(gl.texParameter, gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR)
        self._do(gl.bindFramebuffer, gl.FRAMEBUFFER, self._frame_buffer)
        self._set_depth_and_stencil(enable_depth_and_stencil)
        self._do(gl.framebufferTexture2D, gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, texture.gl_texture, 0)
    ...
    def _bind_texture(self, texture):
        if isinstance(texture, BitmapData):
            ...
            raise ValueError('Unsupported texture class "%s"' % texture.__class__.__name__)
    ...
    def _render(self, rect):
        ...
        if not self._render_to_texture and self._depth_n_stencil or self._render_to_texture and self._fb_depth_n_stencil:
