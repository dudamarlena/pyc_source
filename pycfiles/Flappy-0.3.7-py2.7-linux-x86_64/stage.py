...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/flappy/display/stage.py
...
class Stage(DisplayObjectContainer, _core._Stage):
    ...
    _EF_ALT_DOWN = 8
    _EF_COMMAND_DOWN = 16
    _EF_LOCATION_RIGHT = 16384
    _EF_NO_NATIVE_CLICK = 65536
    _MOUSE_CHANGES = (
    ...
    def __init__(self):
        ...
        self._event_map = {_core.etKeyDown: lambda event: self._on_key(event, KeyboardEvent.KEY_DOWN), 
           _core.etKeyUp: lambda event: self._on_key(event, KeyboardEvent.KEY_UP), 
           _core.etMouseMove: lambda event: self._on_mouse(event, MouseEvent.MOUSE_MOVE, True), 
           _core.etMouseDown: lambda event: self._on_mouse(event, MouseEvent.MOUSE_DOWN, True), 
           _core.etMouseUp: lambda event: self._on_mouse(event, MouseEvent.MOUSE_UP, True), 
           _core.etMouseClick: lambda event: self._on_mouse(event, MouseEvent.CLICK, True), 
           _core.etResize: lambda event: self._on_resize(), 
           _core.etPoll: lambda event: self._poll_timers(), 
           _core.etQuit: lambda event: self._on_quit(), 
           _core.etFocus: self._on_focus, 
           _core.etShouldRotate: self._should_rotate, 
           _core.etDestroyHandler: lambda event: None, 
           _core.etRedraw: lambda event: self._render(True), 
           _core.etTouchBegin: self._on_touch_begin, 
           _core.etTouchMove: self._on_touch_move, 
           _core.etTouchEnd: self._on_touch_end, 
           _core.etChange: self._on_change, 
           _core.etActivate: lambda event: self._set_active(True), 
           _core.etDeactivate: lambda event: self._set_active(False), 
           _core.etGotInputFocus: lambda event: self._on_got_input_focus(), 
           _core.etLostInputFocus: lambda event: self._on_lost_input_focus(), 
           _core.etJoyAxisMove: lambda event: self._on_joystick(event, 0), 
           _core.etJoyBallMove: lambda event: self._on_joystick(event, 0), 
           _core.etJoyHatMove: lambda event: self._on_joystick(event, 0), 
           _core.etJoyButtonDown: lambda event: self._on_joystick(event, 0), 
           _core.etJoyButtonUp: lambda event: self._on_joystick(event, 0)}
        return
    ...
    def _on_key(self, event, etype):
        ...
        if stack:
            ...
            evt = KeyboardEvent(etype, bubbles=True, cancelable=True, charCodeValue=event.code, keyCodeValue=value, keyLocationValue=key_location, ctrlKeyValue=ctrl_down, altKeyValue=alt_down, shiftKeyValue=shift_down)
            obj._fire_event(evt)
        return
    ...
    def _on_focus(self, event):
        ...
        if stack and (event.value == 1 or event.value == 2):
            ...
            evt = FocusEvent(etype, bubbles=True, cancelable=True, relatedObject=relobj, shiftKey=event.flags > 0, keyCode=event.code)
            obj._fire_event(evt)
            if evt.isCancelled:
                event.result = 1
        stack.reverse()
    ...
    def _checkFocusInOuts(self, event, stack):
        ...
        if new_obj != old_obj:
            common = 0
            while common < new_n and common < old_n and stack[common] == self._focus_over_objects[common]:
                common += 1

            fout = FocusEvent(FocusEvent.FOCUS_OUT, bubbles=False, cancelable=False, relatedObject=new_obj, shiftKey=event.flags > 0, keyCode=event.code)
            i = old_n - 1
            while i >= common:
                self._focus_over_objects[i]._dispatch_event(fout)
                i -= 1

            fin = FocusEvent(FocusEvent.FOCUS_IN, bubbles=False, cancelable=False, relatedObject=old_obj, shiftKey=event.flags > 0, keyCode=event.code)
            i = new_n - 1
            while i >= common:
    ...
    def _on_mouse(self, event, event_type, from_mouse):
        ...
        if event_type == MouseEvent.MOUSE_DOWN and button < 3:
            ...
            if click_obj == self._last_down[button]:
                evt = MouseEvent._create(Stage._S_CLICK_EVENTS[button], event, local, click_obj)
                ...
                if button == 0 and click_obj.doubleClickEnabled:
                    ...
                    if diff <= self.DOUBLE_CLICK_INTERVAL:
                        evt = MouseEvent._create(MouseEvent.DOUBLE_CLICK, event, local, click_obj)
                        click_obj._fire_event(evt)
    ...
    def _check_in_outs(self, event, stack, touch_info=None):
        ...
        if new_obj != old_obj:
            ...
            common = 0
            while common < new_n and common < old_n and stack[common] == prev[common]:
                common += 1
    ...
    def _on_touch(self, event, etype, touch_info):
        ...
        if stack:
            ...
            stack.reverse()
            local = obj.globalToLocal(Point(event.x, event.y))
            evt = TouchEvent._create(etype, event, local, obj, event.sx, event.sy)
            evt.touchPointID = event.value
            evt.isPrimaryTouchPoint = bool(event.flags & TouchEvent.efPrimaryTouch)
            self._check_in_outs(evt, stack, touch_info)
            obj._fire_event(evt)
            if evt.isPrimaryTouchPoint and etype == TouchEvent.TOUCH_MOVE:
                if self._drag_object:
                    self._drag(Point(event.x, event.y))
                evt = MouseEvent._create(MouseEvent.MOUSE_MOVE, event, local, obj)
                obj._fire_event(evt)
        else:
            evt = TouchEvent._create(etype, event, Point(event.x, event.y), None, event.sx, event.sy)
            evt.touchPointID = event.value
            evt.isPrimaryTouchPoint = bool(event.flags & TouchEvent.efPrimaryTouch)
            self._check_in_outs(evt, stack, touch_info)
        return
