...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/unittests/test_kill_hogs.py
# Compiled at: 2019-09-03 09:39:33
# Size of source mod 2**32: 8152 bytes
from unittest import mock
from kill_hogs import kill_hogs
import mailtest, random, unittest, yaml
...
class PostToSlackTestcase(unittest.TestCase):
    ...
    def test_post_to_slack(self, mock_get):
        ...
        right parameters.
        """
        kill_hogs.post_to_slack('Hello world',
          slack_url='https://hooks.slack.com/services/some/random/string')
        self.assertIn(mock.call('https://hooks.slack.com/services/some/random/string',
          data=b'{"channel": "#peregrine-alerts", "username": "kill-hogs", "text": "Hello world", "icon_emoji": ":scales:"}',
          headers={'Content-Type': 'application/json'}), mock_get.call_args_list)
...
class KillhogsTestCase(unittest.TestCase):
    ...
    def mocked_psutil_process_iter(*args, **kwargs):
        ...
        class MockedProc:
            ...
            def __init__(self, uid, name='emacs', username=None, cpu_percent=10.0):
                self.uid = uid
                self.pid = 123456
                self._name = name
                self._cpu_percent = cpu_percent
                if username is None:
                    self._username = random.choice(['p', 's', 'f']) + ''.join([str(random.choice(range(9))) for _ in range(6)])
                else:
        ...
        yield MockedProc(1,
          name='root_stuff', username='root', cpu_percent=700.0)
        yield MockedProc(14584,
          name='dontkillme', username='duckling', cpu_percent=1800.0)
    ...
    def test_send_message_to_terminals(self, mock_run):
        ...
        self.assertIn(mock.call('echo "hello, user" | write p945314 pts/49', shell=True), mock_run.call_args_list)
    ...
    def test_no_innocents_are_killed(self, mock_run, mock_terminate, mock_process_iter):
        kill_hogs.kill_hogs(config=(self.config_dict),
          memory_threshold=10,
          cpu_threshold=600)
    ...
    @mock.patch('kill_hogs.kill_hogs.terminate', side_effect=mocked_terminate)
    @mock.patch('psutil.process_iter', side_effect=mocked_psutil_process_iter)
    def test_violators_are_shot(self, mock_run, mock_terminate, mock_process_iter):
        kill_hogs.kill_hogs(config=(self.config_dict),
          memory_threshold=10,
          cpu_threshold=9.5)
    ...
    def test_find_email(self, mock_run):
        self.assertEqual(kill_hogs.find_email('p458749'), 'E.R.T.scrooge@rug.nl')

    def test_send_mail(self):
        with mailtest.Server() as (mt):
            kill_hogs.send_mail('root@exacluster',
              'loue@institution.nl', 'abcde', port=1025)
