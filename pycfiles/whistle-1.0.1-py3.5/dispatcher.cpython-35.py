...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/whistle/dispatcher.py
...
class EventDispatcher(object):
    ...
    def remove_listener(self, event_id, listener):
        ...
        for priority, listeners in self._listeners[event_id].items():
            if listener in self._listeners[event_id][priority]:
                self._listeners[event_id][priority] = list(filter(lambda l: l != listener, self._listeners[event_id][priority]))
                if event_id in self._sorted:
                    del self._sorted[event_id]
    ...
    def sort_listeners(self, event_id):
        ...
        if event_id in self._listeners:
            self._sorted[event_id] = [listener for listeners in sorted(self._listeners[event_id].items(), key=operator.itemgetter(0)) for listener in listeners[1]]