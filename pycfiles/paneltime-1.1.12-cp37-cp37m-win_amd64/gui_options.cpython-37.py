...
# [GCC 8.4.0]
# Embedded file name: b:\forskning\papers\paneltime\paneltime\paneltime\gui\gui_options.py
# Compiled at: 2020-03-27 08:22:04
...
class options_item(ttk.Treeview):
    ...
    def get_script(self):
        ...
        for i in d:
            if hasattr(d[i], 'value') and d[i].value != self.default_options.__dict__[i].value:
                scripts.append(f"options.{i}.set({d[i].value})")
                search_patterns.append(f"options.{i}.set\\(([\\s\\S]*?)\\)")

        return (
         scripts, search_patterns)
...
class managed_text(tk.Entry):
    ...
    def register_validation(self):
        vcmd = (
         self.master.register(self.onValidate), '%d', '%P')
