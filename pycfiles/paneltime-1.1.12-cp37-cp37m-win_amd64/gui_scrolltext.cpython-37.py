...
# Compiled at: 2020-03-11 06:32:09
...
class ScrollText(tk.Frame):
    ...
    def __init__(self, master, readonly=False, text=None, format_text=True, name=None, window=None):
        ...
        self.master = master
        self.text_box = CustomText(self, wrap=(tk.NONE), xscrollcommand=(xscrollbar.set), yscrollcommand=(yscrollbar.set),
          undo=True,
...
class CustomText(tk.Text):
    ...
    def key_released(self, event=None):
        if not self.format_text:
            ...
            if event is not None:
                if event.keysym == 'Return':
                    if hasattr(self.master.master, 'tab'):
                        self.master.master.tab.edit_data_set()
                if self.released_ignore_key == True:
                    if event.keysym not in ignore_press_keys:
                        self.released_ignore_key = False
                        if event.keysym != 'v':
                            return
        ...
        for i in self.builtins:
            self.highlight_pattern('\\m(%s)\\M' % (i,), 'builtins')
    ...
    def highlight_pattern(self, pattern, tag, start='1.0', end='end', regexp=True, tag2=None, addstart=0, subtractend=0):
        ...
        for index1, index2 in indicies:
            len(self.tag_names(index1)) or self.tag_add(tag, index1, index2)
            if tag2 is not None:
...
ignore_press_keys = [
 'Up', 'Down', 'Right', 'Left', 'Prior', 'Next', 'Tab', 'Control_L', 'Control_R', 'Alt_R', 'Alt_L']
