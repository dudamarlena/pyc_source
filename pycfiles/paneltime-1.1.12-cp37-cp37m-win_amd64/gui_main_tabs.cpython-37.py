...
# Compiled at: 2020-04-20 04:41:51
# Size of source mod 2**32: 13062 bytes
import tkinter as tk
from tkinter import ttk
from gui import gui_scrolltext
import functions as fu, os
from tkinter import filedialog
import types, numpy as np
from multiprocessing import pool
import paneltime as pt, traceback
...
class main_tabs(ttk.Notebook):
    ...
    def recreate_tabs(self):
        ...
        for i in self.win.data.get('editor_data'):
            try:
                ...
                if not name == ADD_EDITOR_NAME:
                    name[:6] == 'script' and text == '' or self.add_editor(name, text, top_text=top_text, top_color=top_color, attached_to=attached_to, path=path).frame.focus()
                    n += 1
            except:
    ...
    def current_editor(self, return_obj=1):
        ...
        if return_obj == 1:
            return str(self._tabs[selection].frame)
        if return_obj == 2:
            return self._tabs[selection].widget
    ...
    def gen_name(self, core):
        if core is None:
            core = 'script'
        if core not in self._tabs.names:
            return core
            i = 0
            n = list(core.split('.'))
            if len(n) == 1:
                while True:
                    i += 1
                    name = f"{core} {i}"
                    if name not in self._tabs.names:
                        return name
    ...
    def remove_redundant_script(self, name):
        ...
        for i in self._tabs:
            try:
                if self._tabs[i].name[:6] == 'script':
                    if self._tabs[i].widget.get_all().replace('\n', '') == '':
                        poptabs.append(i)
            except:
                pass
...
class tab:
    ...
    def add_buttons(self, name, top_color, attached_to, top_text):
        ...
        self.button_save = tk.Button((self.button_frame_L), image=(self.button_img['save']), command=(self.save), highlightthickness=0,
          bd=0,
          height=size,
          anchor=(tk.E),
        ...
        self.button_run = tk.Button((self.button_frame_L), image=(self.button_img['run']), command=(self.run), highlightthickness=0,
          bd=0,
          height=size,
          anchor=(tk.E),
          background=top_color)
        self.button_img['stop'] = tk.PhotoImage(file=(fu.join(os.path.dirname(__file__), ['img', 'stop.png'])), master=(self.button_frame))
        self.button_stop = tk.Button((self.button_frame_L), image=(self.button_img['stop']), command=(self.stop), highlightthickness=0,
          bd=0,
          height=size,
          anchor=(tk.E),
          background=top_color)
        ...
        self.button_img['delete'] = tk.PhotoImage(file=(fu.join(os.path.dirname(__file__), ['img', 'delete.png'])), master=(self.button_frame))
        self.button_delete = tk.Button((self.button_frame_R), image=(self.button_img['delete']), command=(self.delete), highlightthickness=0,
          bd=0,
          height=15,
          anchor=(tk.E),
          background=top_color)
    ...
    def attachment_change(self, event):
        ...
        if name == '':
            ...
            if self.top_text.get() == 'Data editor':
                ...
                if self.top_text.get() == 'Import script':
                    self.dataset.script_editor = str(self.frame)
                else:
                    if self.top_text.get() == 'Model execution editor':
    ...
    def save(self):
        ...
        filename = filedialog.asksaveasfilename(initialdir=p, title='Save', filetypes=(('text', '*.txt'),
    ...
    def save_all_in_temp(self):
        ...
        for i in self:
            ...
            if not t.name == ADD_EDITOR_NAME:
                win.data['editor_data'][i] = (
                 t.name, text, t.top_text.get(), t.top_color, t.attached_to.get(), t.path)
