...
# [GCC 8.4.0]
# Embedded file name: b:\forskning\papers\paneltime\paneltime\paneltime\gui\gui_data_objects.py
# Compiled at: 2020-04-20 04:42:14
# Size of source mod 2**32: 19672 bytes
import tkinter as tk
from tkinter import ttk
from gui import gui_charts
import functions as fu, time
...
import json, os, tempstore, time, paneltime as pt, numpy as np
NON_NUMERIC_TAG = '|~|'
font = 'Arial 9 '
tags = dict()
...
class data_objects(ttk.Treeview):
    ...
    def add_button(self, master, img, command, tooltip, bgcolor, size=22):
        ...
        btn = tk.Button(master, image=(self.button_img[img]), command=command, highlightthickness=0,
          bd=0,
          height=size,
          width=size,
          compound='left',
    ...
    def add(self):
        if self.import_buttons is None:
            ...
            return
        elif self.import_buttons.state() == 'normal':
    ...
    def open_json(self):
        ...
        filename = filedialog.askopenfilename(initialdir=p, title='Open data file', filetypes=(('JSON', '*.json'), ))
        if not filename:
            return
    ...
    def open_file(self):
        ...
        filename = filedialog.askopenfilename(initialdir=p, title='Open data file', filetypes=(('CSV', '*.csv'),
                                                                                               ('text', '*.txt'),
                                                                                               ('All files', '*.*')))
        if not filename:
            return
    ...
    def save(self):
        p = self.win.data['current path']
        filename = filedialog.asksaveasfilename(initialdir=p, title='Save', filetypes=(('CSV', '*.csv'), ),
        ...
        X = []
        for i in dataset:
            if type(dataset[i]) == np.ndarray and len(dataset[i].shape) == 2:
    ...
    def selected_item(self):
        ...
        if len(item) != 1:
            item = [
             self.win.data.get('selected data', item[0])]
        item = item[0]
        return item
    ...
    def recreate_editors(self, datastore, main_tabs):
        ...
        for i in datastore:
            ...
            if i == self.exe_editor:
                exe_editor = main_tabs.add_editor(name, text=text, top_text='Model execution editor', top_color='#fcdbd9',
                  dataset=self,
                  attached_to=attached_to,
                  path=path)
                pop_editors.append(i)
            if i == self.edit_editor:
                edit_editor = main_tabs.add_editor(name, text=text, top_text='Data editor', top_color='#ddfcd9',
                  dataset=self,
                  attached_to=attached_to,
                  path=path)
                pop_editors.append(i)
            if i == self.script_editor:
                script_editor = main_tabs.add_editor(name, text=text, top_text='Import script', top_color='#6bff9c',
                  dataset=self,
                  attached_to=attached_to,
                  path=path)
                pop_editors.append(i)
        ...
        if exe_editor is not None:
            self.exe_editor = str(exe_editor.frame)
        if edit_editor is not None:
...
class datasets(dict):
    ...
    def delete(self, item, tree, optionset):
        ...
        for i in tree.main_tabs._tabs:
            ...
            if hasattr(tab, 'attached_to') and tab.attached_to is not None and tab.attached_to.get() == item:
                tab.attached_to.set('')
                tab.dataset = None
