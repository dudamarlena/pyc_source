...
# Compiled at: 2009-07-09 19:28:00
...
def validate_config(config):
    ...
    for apiname in apis:
        ...
        if 'int' != type['name'] or not type.has_key('min') or type['min'] < 0:
...
def generate_files(output_dir, tmpl_dir, config):
    ...
    for (root, dirs, files) in os.walk(tmpl_dir):
        for file in files:
            ...
            print '  %s ... ' % file,
            tmpl = Template(filename=file, input_encoding='utf-8')
