...
# [GCC 8.4.0]
# Embedded file name: /private/var/folders/_d/md744_dn7ggffch8955xb93h0000gs/T/pip-install-xY4LYQ/uniseg/uniseg/wrap.py
            ...
            for field in re.split(b'(\\t)', para):
                ...
                for boundary in iter_boundaries(field):
                    ...
                    if wrap_width is not None and cur + w > wrap_width:
                        ...
                        line_extents = partial_extents(field_extents, breakpoint, prev_boundary)
...
class TTFormatter(object):
    ...
    def __init__(self, wrap_width, tab_width=8, tab_char=b' ', ambiguous_as_wide=False):
        self._lines = [
         b'']
    ...
    def tab_char(self, value):
        if east_asian_width(value) not in ('N', 'Na', 'H'):
            raise ValueError(b'only a narrow code point is available for \n                             tab_char')
...
def tt_width(s, index=0, ambiguous_as_wide=False):
    ...
    if eaw in ('W', 'F') or eaw == b'A' and ambiguous_as_wide:
        return 2
    return 1
...
def tt_wrap(s, wrap_width, tab_width=8, tab_char=b' ', ambiguous_as_wide=False, cur=0, offset=0, char_wrap=False):
    ...
    """
    formatter = TTFormatter(wrap_width, tab_width, tab_char, ambiguous_as_wide)
