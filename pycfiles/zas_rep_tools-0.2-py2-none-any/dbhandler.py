...
# [GCC 8.4.0]
# Embedded file name: /Users/egoruni/Desktop/BA/Code/zas-rep-tools/zas_rep_tools/src/classes/dbhandler.py
# Compiled at: 2018-10-19 05:53:04
from __future__ import absolute_import
import os, copy, sys, time, logging, json, traceback, threading, gc, inspect, threading, subprocess
...
class DBHandler(BaseContent, BaseDB):
    ...
    templates = {'twitter': db_helper.default_tables['corpus']['documents']['twitter'], 
    ...
    mapped_states = {'synchronous': {'0': 'off', 
                       '1': 'normal', 
                       '2': 'full', 
                       '3': 'extra'}, 
       'temp_store': {'0': 'default', 
                      '1': 'file', 
                      '2': 'memory'}}
    non_mapped_states = {'journal_mode': [
    ...
    def _get_arguments_for_conn(self):
        ...
        if not self._isolation_level == False:
    ...
    def init(self, typ, prjFolder, DBname, language, visibility, platform_name=False, encryption_key=False, fileName=False, source=False, license=False, template_name=False, version=False, cols_and_types_in_doc=False, corpus_id=False, stats_id=False, retrival_template_automat=True, thread_name='Thread0', db_frozen=False, context_lenght=None):
    ...
    def init_corpus(self, prjFolder, DBname, language, visibility, platform_name, encryption_key=False, fileName=False, source=False, license=False, template_name=False, version=False, thread_name='Thread0', cols_and_types_in_doc=False, corpus_id=False, retrival_template_automat=True):
        cols_and_types_in_doc = copy.deepcopy(cols_and_types_in_doc)
        self._encryption_key = encryption_key
        if not source:
            source = 'NULL' if 1 else source
            ...
            typ = 'corpus'
            corpus_id = corpus_id or db_helper.create_id(DBname, language, typ, visibility)
        fileName, path_to_db = db_helper.get_file_name(prjFolder, corpus_id, DBname, language, visibility, typ, fileName, platform_name, encrypted=True if encryption_key else False, rewrite=self._rewrite, stop_if_db_already_exist=self._stop_if_db_already_exist)
    ...
    def init_stats(self, prjFolder, DBname, language, visibility, corpus_id, thread_name='Thread0', encryption_key=False, fileName=False, version=False, stats_id=False, db_frozen=False, context_lenght=None):
        self._encryption_key = encryption_key
        if not version:
            version = 'NULL' if 1 else version
            typ = 'stats'
            stats_id = stats_id or db_helper.create_id(DBname, language, typ, visibility)
        if not stats_id:
            ...
            fileName, path_to_db = db_helper.get_file_name(prjFolder, corpus_id, DBname, language, visibility, typ, fileName, second_id=stats_id, encrypted=True if encryption_key else False, rewrite=self._rewrite, stop_if_db_already_exist=self._stop_if_db_already_exist)
            if path_to_db is None:
            ...
            if os.path.isdir(prjFolder):
                ...
                attributs_list = db_helper.default_tables[typ]['info']
                values = [
                 stats_id, corpus_id, DBname, version, created_at, visibility, typ, db_frozen, context_lenght]
                status = self._init_info_table(attributs_list)
                if not status['status']:
    ...
    def initempty(self, prjFolder, DBname, encryption_key=False, thread_name='Thread0'):
        self._encryption_key = encryption_key
        fileName, path_to_db = db_helper.get_file_name_for_empty_DB(prjFolder, DBname, encrypted=True if encryption_key else False, rewrite=self._rewrite, stop_if_db_already_exist=self._stop_if_db_already_exist)
        if path_to_db is None:
    ...
    def _execution(self, query, values=False, dbname='main', many=False, thread_name='Thread0', new_cursor=False):
        try:
            ...
            if 'UNIQUE constraint failed:' in str(exception):
                ...
                return Status(status=False, track_id=track_id, desc='Redundant row was get.', level=level_name, action='outsorted', inp_obj=(
                 query, values, dbname), error_name=('{} (UniquenessERROR)').format(exception.__class__.__name__), exception=exception)
            if 'SQL logic error' in str(exception):
                ...
                self.logger.error_insertion(msg)
                self.error_insertion_counter.incr()
                return Status(status=False, track_id=track_id, desc='SQL logic error. ', level=level_name, action='ThreadsCrash', inp_obj=(
                 query, values, dbname), error_name=('{} (ErrorBindingParameter)').format(exception.__class__.__name__), exception=exception)
            ...
            self.error_insertion_counter.incr()
            return Status(status=False, track_id=track_id, desc="It is not possible to insert all got columns into CorpDB. Possible Explanation: 1. Current DB was initialized with wrong and not full number of columns, please reinitialize current CorpusDB with right column names and types. Or use also option  precomputed corp types. For this use option  'template_name' on the Corpus Level ( while Initialization) (ex:template_name='twitter',or template_name='blogger'  ) or also on the Reader Level 'reader_formatter_name='twitter'.", level='error_insertion', action='stop_execution', inp_obj=(
             query, values, dbname), func_name=function_name(-3), error_name=exception.__class__.__name__, exception=repr(exception))
            ...
            if 'Error binding parameter ' in str(exception):
                ...
                self.logger.error_insertion(msg)
                self.error_insertion_counter.incr()
                return Status(status=False, track_id=track_id, desc='Error binding parameter', level=level_name, action='ThreadsCrash', inp_obj=(
                 query, values, dbname), error_name=('{} (ErrorBindingParameter)').format(exception.__class__.__name__), exception=exception)
            ...
            self.error_insertion_counter.incr()
            return Status(status=False, track_id=track_id, desc=repr(exception), level='error_insertion', action='ignored', inp_obj=(
             query, values, dbname), func_name=function_name(-3), error_name=exception.__class__.__name__, exception=exception)
        except Exception as exception:
            track_id = self._error_track_id.incr()
            print_exc_plus() if self._ext_tb else ''
            ...
            if 'has no column named' in str(exception):
                ...
                self.error_insertion_counter.incr()
                return Status(status=False, track_id=track_id, desc='One of the columns is not in the Table.', inp_obj=(
                 query, values, dbname), func_name=function_name(-3), level='error_insertion', action='ignored', error_name=exception.__class__.__name__, exception=exception)
            l_query = query if self._log_content else '!!LogContentDisable!!'
            l_values = values if self._log_content else '!!LogContentDisable!!'
            ...
            self.error_insertion_counter.incr()
            return Status(status=False, track_id=track_id, desc='Something happens', inp_obj=(
             query, values, dbname), func_name=function_name(-3), level='error_insertion', action='ignored', error_name=exception.__class__.__name__, exception=exception)
    ...
    def _intern_getter(self, tableName, columns=False, select=False, dbname='main', where=False, connector_where='AND', limit=-1, offset=-1, thread_name='Thread0', case_sensitiv=True, distinct=False):
        s = self._check_db_should_exist()
        if not s['status']:
            return s
        else:
            ...
            if columns:
                ...
                if where:
                    ...
                    query = ('SELECT {} {} FROM  {}.{} LIMIT {} OFFSET {}').format(distinct_tag, select_conditions, dbname, tableName, limit, offset)
                ...
                try:
                    ...
                    self.logger.error(("Exception was throw:  '{}' for following query: '{}'").format(repr(exception), q), exc_info=self._logger_traceback)
                    return Status(status=False, track_id=self._error_track_id.incr(), func_name=function_name(-2))
    ...
    def _dict_to_cash(self, table_name, inp_obj, dbname, commit_number, thread_name='Thread0'):
        if len(inp_obj) < 1:
            ...
            return Status(status=False, track_id=track_id, desc=msg, func_name=function_name(-2), level='outsorted_corpus', action='outsorted')
        else:
            try:
                new_val_list = []
                ...
                if len(set(length)) > 1:
                    ...
                    self.error_insertion_counter.incr()
                    return Status(status=False, track_id=track_id, desc=msg, func_name=function_name(-2), level='error_insertion', action='ignored')
            except TypeError:
                try:
                    new_val_list = []
                    for item in inp_obj.values():
                ...
                self.error_insertion_counter.incr()
                return Status(status=False, desc=str(exception), level='error_insertion', action='ignored', error_name=exception.__class__.__name__, exception=exception)
        ...
        try:
            ...
            if isinstance(inp_obj, dict):
                ...
                if isinstance(inp_obj, list):
                    ...
                    self.error_insertion_counter.incr()
                    return Status(status=False, track_id=track_id, desc='Not Supported type of lazy_writer.', inp_obj=inp_obj, func_name=function_name(-2), level='error', action='ignored')
                if not status['status']:
                    return status
            ...
            self.error_insertion_counter.incr()
            return Status(status=False, track_id=track_id, desc=str(exception), inp_obj=(
             table_name, inp_obj), func_name=function_name(-2), level='error_insertion', action='ignored', error_name=exception.__class__.__name__, exception=exception)
    ...
    def insertdict(self, table_name, inp_dict, dbname='main', thread_name='Thread0'):
        if not isinstance(inp_dict, dict):
            ...
            self.error_insertion_counter.incr()
            return Status(status=False, track_id=track_id, desc='InsertDictError: Given object in not an dict!', inp_obj=inp_dict, func_name=function_name(-2), level='error', action='ignored')
        try:
            ...
            self.error_insertion_counter.incr()
            return Status(status=False, track_id=track_id, desc=str(exception), inp_obj=inp_dict, func_name=function_name(-2), level='error_insertion', action='ignored', error_name=exception.__class__.__name__, exception=exception)
    ...
    def insertlist(self, table_name, inp_list, dbname='main', thread_name='Thread0'):
        if not isinstance(inp_list, list):
            ...
            self.error_insertion_counter.incr()
            return Status(status=False, track_id=track_id, desc='InsertDictError: Given object in not an list!', inp_obj=inp_list, func_name=function_name(-2), level='error', action='ignored')
        try:
            if self._make_backup:
                if dbname in self.not_initialized_dbs:
                    self._backup(dbname)
            ...
            self.error_insertion_counter.incr()
            return Status(status=False, track_id=track_id, desc=str(exception), inp_obj=inp_list, func_name=function_name(-2), level='error_insertion', action='ignored', error_name=exception.__class__.__name__, exception=exception)
    ...
    def _insertdict_with_many_rows(self, table_name, inp_dict, dbname='main', thread_name='Thread0'):
        ...
        if not s['status']:
            ...
            if not isinstance(inp_dict, dict):
                ...
                self.error_insertion_counter.incr()
                return Status(status=False, track_id=track_id, desc=("Table ('{}') wasn't found or not exist.").format(table_name), inp_obj=inp_dict, level='error_insertion', action='ignored')
    ...
    def _insertlist_with_many_rows(self, table_name, inp_list, dbname='main', thread_name='Thread0'):
        ...
        if not s['status']:
            ...
            if dbname not in self.dbnames:
                ...
                self.error_insertion_counter.incr()
                return Status(status=False, track_id=track_id, desc=("Table ('{}') wasn't found or not exist.").format(table_name), inp_obj=inp_list, level='error_insertion', action='ignored')
    ...
    def _check_should_be_str_or_unicode(self, giv_obj):
        if not isinstance(giv_obj, (str, unicode)):
            ...
            return Status(status=False, desc='Given Object is not from following type: (str, unicode).', func_name=function_name(-3))
        else:
            return Status(status=True)
        ...
        for column in columns:
            if column not in columns_from_db:
                if 'json_extract' not in column:
                    ...
                    self.logger.error(msg, exc_info=self._logger_traceback)
                    return Status(status=False, desc=msg, func_name=function_name(-3))
    ...
    def _check_if_table_exist(self, tableName, dbname='main'):
        if tableName not in self.tables(dbname=dbname):
            ...
            return Status(status=False, desc=("Given Table '{}' is not exist (dbname='{}')) ").format(tableName, dbname), func_name=function_name(-3))
        else:
            return Status(status=True)
    ...
    def _check_db_should_exist(self):
        if not self._db:
            ...
            return Status(status=False, desc='No active DB was found.', func_name=function_name(-3))
        else:
            return Status(status=True)
    ...
    def _check_db_should_not_exist(self):
        if self._db:
            ...
            self.logger.error(msg, exc_info=self._logger_traceback)
            s = Status(status=False, desc=msg, func_name=function_name(-3))
            return s
        else:
            return Status(status=True)
    ...
    def _db_should_be_a_corpus(self):
        ...
        if db_typ != 'corpus':
            ...
            return Status(status=False, track_id=track_id, desc=("Active DB is from typ '{}'. But it should be from typ 'corpus'. ").format(db_typ), func_name=function_name(-3))
        return Status(status=True)
    ...
    def _db_should_be_stats(self):
        ...
        if db_typ != 'stats':
            ...
            return Status(status=False, track_id=track_id, desc=("Active DB is from typ '{}'. But it should be from typ 'stats'. ").format(db_typ), func_name=function_name(-3))
        return Status(status=True)
    ...
    def _check_file_existens(self, path_to_file):
        if not os.path.isfile(path_to_file):
            ...
            return Status(status=False, track_id=track_id, desc=("DB-File wasn't found: ('{}').").format(path_to_file), func_name=function_name(-3))
        else:
            return Status(status=True)
        ...
        while True:
            ...
            try:
                ...
                if os.path.isfile(DBHandler.path_to_json1 + '.c'):
                    ...
                    args2 = [
                     'gcc', '-g', '-fPIC', '-shared', DBHandler.path_to_json1 + '.c', '-o', DBHandler.path_to_json1]
    ...
    def _init_default_tables(self, typ, template=False, cols_and_types_in_doc=False):
        ...
        if typ == 'corpus':
            ...
            for table_name, columns in db_helper.default_tables[typ].iteritems():
                if table_name == 'info':
                    continue
    ...
    def _get_all_attr_from_db(self, dbname='main', thread_name='Thread0'):
        ...
        if not s['status']:
            ...
            if number_of_rows_info_table == 0:
    ...
    def _optimize(self, thread_name='Thread0', dbname='main'):
        ...
        optimizer_names = {'p': 'page_size', 
           'c': 'cache_size', 
           'l': 'locking_mode', 
           's': 'synchronous', 
           'j': 'journal_mode', 
           't': 'temp_store'}
        statement_setting = {'p': self._optimizer_page_size, 
    ...
    def _init_instance_variables(self):
        ...
        self.bool_conv = {'true': True, 'null': None, 'false': False}
