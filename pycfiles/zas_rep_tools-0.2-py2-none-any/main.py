...
# [GCC 8.4.0]
# Embedded file name: /Users/egoruni/Desktop/BA/Code/zas-rep-tools/zas_rep_tools/cli/main.py
# Compiled at: 2018-10-25 11:06:53
import click, shutil, os, inspect, sys, logging, ast, json
from blessings import Terminal
import enlighten, time, itertools
...
supported_commands = {'configer': {'prjdir': [
                         'clean', 'set', 'reset', 'print'], 
                'error_track': [
                              'set', 'reset', 'respeak', 'print'], 
                'twitter': [
                          'set', 'reset', 'respeak', 'print'], 
                'email': [
                        'set', 'reset', 'respeak', 'print'], 
                'user_data': [
                            'clean', 'location', 'print']}, 
   'corpora': [
             'add', 'del', 'names', 'meta', 'basic_stats', 'update_attr', 'export', 'used_tools', 'clean_dir', 'cols', 'doc', 'ids'], 
   'stats': [
           'compute', 'del', 'names', 'meta', 'basic_stats', 'update_attr', 'export', 'clean_dir', 'recompute', 'optimize', 'recreate_indexes'], 
   'streamer': []}
...
@click.option('--logdir', '-ld', default='logs', help='Choose the name of the Directory for log data.')
def corpora(command1, status_bar, use_end_file_marker, tok_split_camel_case, make_backup, lazyness_border, rewrite, use_cash, optimizer, optimizer_page_size, optimizer_cache_size, optimizer_locking_mode, optimizer_synchronous, optimizer_journal_mode, optimizer_temp_store, gready, corp_fname, language, visibility, platform_name, encryption_key, corp_intern_dbname, source, license, template_name, version, cols_and_types_in_doc, corpus_id_to_init, tokenizer, pos_tagger, sentiment_analyzer, sent_splitter, preprocession, lang_classification, del_url, del_punkt, del_num, del_mention, del_hashtag, del_html, case_sensitiv, emojis_normalization, text_field_name, id_field_name, heal_me_if_possible, path_to_read, file_format_to_read, reader_regex_template, reader_regex_for_fname, read_from_zip, formatter_name, reader_ignore_retweets, min_files_pro_stream, csvdelimiter, encoding, stream_number, value, attr_name, type_to_export, export_dir, export_name, doc_id, rows_limit_in_file, mode, logdir):
    ...
    if command1 == 'add':
        ...
        reader = Reader(path_to_read, file_format_to_read, regex_template=reader_regex_template, regex_for_fname=reader_regex_for_fname, read_from_zip=read_from_zip, end_file_marker=end_file_marker, send_end_file_marker=use_end_file_marker, stop_process_if_possible=stop_process_if_possible, formatter_name=formatter_name, text_field_name=text_field_name, id_field_name=id_field_name, ignore_retweets=reader_ignore_retweets, mode=mode)
        ...
        corp = Corpus(mode=mode, error_tracking=answer_error_tracking, status_bar=status_bar, end_file_marker=end_file_marker, use_end_file_marker=use_end_file_marker, tok_split_camel_case=tok_split_camel_case, make_backup=make_backup, lazyness_border=lazyness_border, thread_safe=True, rewrite=rewrite, stop_if_db_already_exist=stop_if_db_already_exist, use_cash=use_cash, optimizer=optimizer, optimizer_page_size=optimizer_page_size, optimizer_cache_size=optimizer_cache_size, optimizer_locking_mode=optimizer_locking_mode, optimizer_synchronous=optimizer_synchronous, optimizer_journal_mode=optimizer_journal_mode, optimizer_temp_store=optimizer_temp_store, stop_process_if_possible=stop_process_if_possible, heal_me_if_possible=heal_me_if_possible)
        corp.init(main_folders['corp'], corp_intern_dbname, language, visibility, platform_name, encryption_key=encryption_key, fileName=corp_fname, source=source, license=license, template_name=template_name, version=version, cols_and_types_in_doc=cols_and_types_in_doc, corpus_id=corpus_id_to_init, text_field_name=text_field_name, id_field_name=id_field_name, tokenizer=tokenizer, pos_tagger=pos_tagger, sentiment_analyzer=sentiment_analyzer, sent_splitter=sent_splitter, preprocession=preprocession, lang_classification=lang_classification, del_url=del_url, del_punkt=del_punkt, del_num=del_num, del_mention=del_mention, del_hashtag=del_hashtag, del_html=del_html, case_sensitiv=case_sensitiv, emojis_normalization=emojis_normalization)
        ...
        if not corp_fname or not attr_name or not value:
            ...
            if corp_fname in files:
                ...
                if corp_fname in [ os.path.splitext(fname)[0] for fname in files ]:
...
@click.option('--mode', '-m', default='prod', help='Set one of the Tool Modus', type=click.Choice(helpers.modi))
@click.option('--logdir', '-ld', default='logs', help='Choose the name of the Directory for log data.')
def stats(command1, status_bar, use_end_file_marker, make_backup, lazyness_border, rewrite, use_cash, optimizer, optimizer_page_size, optimizer_cache_size, optimizer_locking_mode, optimizer_synchronous, optimizer_journal_mode, optimizer_temp_store, gready, min_files_pro_stream, baseline_delimiter, corp_fname, stream_number, create_indexes, freeze_db, optimized_for_long_syntagmas, stats_fname, stats_intern_dbname, visibility, encryption_key, version, stats_id, context_lenght, full_repetativ_syntagma, repl_up, ignore_hashtag, case_sensitiv, ignore_url, ignore_mention, ignore_punkt, ignore_num, recompute_flag, value, attr_name, baseline_insertion_border, export_dir, syntagma_for_export, exp_repl, exp_redu, exp_sentiment, export_name, export_file_type, rows_limit_in_file, encryption_key_corp, output_table_type, additional_doc_cols, max_scope, stemmed_search, context_len_left, context_len_right, separator_syn, word_examples_sum_table, exp_syntagma_typ, ignore_symbol, mode, logdir):
    logger = get_cli_logger(mode, logdir)
    func_name = 'stats'
    ...
    if command1 not in supported_commands[func_name]:
        ...
        if command1 == 'compute':
            ...
            stop_if_db_already_exist = False if rewrite else True
            stop_process_if_possible = False if gready else True
            stats = Stats(mode=mode, error_tracking=answer_error_tracking, status_bar=status_bar, make_backup=strtobool(make_backup), lazyness_border=lazyness_border, thread_safe=True, rewrite=strtobool(rewrite), stop_if_db_already_exist=stop_if_db_already_exist, use_cash=strtobool(use_cash), optimizer=strtobool(optimizer), optimizer_page_size=optimizer_page_size, optimizer_cache_size=optimizer_cache_size, optimizer_locking_mode=optimizer_locking_mode, optimizer_synchronous=optimizer_synchronous, optimizer_journal_mode=optimizer_journal_mode, optimizer_temp_store=optimizer_temp_store, stop_process_if_possible=stop_process_if_possible)
            stats.init(main_folders['stats'], stats_intern_dbname, language, visibility, corpus_id=corpus_id, encryption_key=encryption_key, fileName=stats_fname, version=version, stats_id=stats_id, context_lenght=context_lenght, full_repetativ_syntagma=strtobool(full_repetativ_syntagma), min_scope_for_indexes=2, repl_up=repl_up, ignore_hashtag=strtobool(ignore_hashtag), force_cleaning=False, case_sensitiv=strtobool(case_sensitiv), ignore_url=strtobool(ignore_url), ignore_mention=strtobool(ignore_mention), ignore_punkt=strtobool(ignore_punkt), ignore_num=strtobool(ignore_num), baseline_delimiter=baseline_delimiter)
            stats.compute(corp, stream_number=stream_number, datatyp='dict', adjust_to_cpu=True, min_files_pro_stream=min_files_pro_stream, cpu_percent_to_get=50, create_indexes=create_indexes, freeze_db=freeze_db, baseline_insertion_border=baseline_insertion_border, drop_indexes=True, optimized_for_long_syntagmas=optimized_for_long_syntagmas)
            ...
            if stats_fname in files:
                ...
                if stats_fname in [ os.path.splitext(fname)[0] for fname in files ]:
            ...
            if stats.statsdb._db:
                ...
                stats.export(export_dir, syntagma=syntagma_for_export, repl=exp_repl, redu=exp_redu, baseline=True, syntagma_type=exp_syntagma_typ, sentiment=exp_sentiment, fname=export_name, export_file_type=export_file_type, rows_limit_in_file=rows_limit_in_file, encryption_key_corp=encryption_key_corp, output_table_type=output_table_type, additional_doc_cols=additional_doc_cols, path_to_corpdb=path_to_corpdb, max_scope=max_scope, stemmed_search=stemmed_search, rewrite=False, context_len_left=context_len_left, context_len_right=context_len_right, separator_syn=separator_syn, word_examples_sum_table=word_examples_sum_table, ignore_num=ignore_num, ignore_symbol=ignore_symbol)
            else:
                if strtobool(encryption_key):
...
def streamTwitter(path_to_save, language, stop_words, terms, encoding, ignore_rt, filter_strategie, save_used_terms, logdir, mode):
    ...
    stream = Streamer(consumer_key, consumer_secret, access_token, access_token_secret, path_to_save, platfrom='twitter', language=language, email_addresse=email, stop_words=stop_words, terms=terms, encoding=encoding, ignore_rt=ignore_rt, save_used_terms=save_used_terms, filterStrat=filter_strategie, mode=mode)
...
def streamerInfo(command, logdir, mode):
    ...
    func_name = sys._getframe().f_code.co_name
    possible_commands = [
     'enc', 'lang', 'nltk_lang', 'twitter_lang', 'classiefier_lang', 'stop_words', 'platforms']
