...
# [GCC 8.4.0]
# Embedded file name: /Users/egoruni/Desktop/BA/Code/zas-rep-tools/zas_rep_tools/src/utils/zaslogger.py
# Compiled at: 2018-10-19 07:11:57
import logging, datetime, time, os, sys
from collections import defaultdict
import codecs, traceback, inspect, gc
...
class ZASLogger(object):
    ...
    def __init__(self, logger_name, level=logging.INFO, folder_for_log='logs', logger_usage=True, save_logs=False):
        ...
        self._formatter = logging.Formatter('%(asctime)7s [%(name)s] %(threadName)11s:{%(sthread)s} [%(funcName)s()]  %(levelname)5s \n %(message)s \n', '%H:%M:%S')
        self._logger = None
    ...
    def _add_multihandler(self, sealed_off=True):
        ...
        elif ZASLogger._handlers['MultiHandler'].level > self._level:
    ...
    def _add_level(self, log_name, custom_log_module=None, log_num=None, log_call=None, lower_than=None, higher_than=None, same_as=None, verbose=True):
        """
        ...
        if custom_log_module is None:
            import imp
            custom_log_module = imp.load_module('custom_log_module', *imp.find_module('logging'))
        ...
        available_level_nums = [ key for key in custom_log_module._levelNames if isinstance(key, int) ]
        available_levels = {key:custom_log_module._levelNames[key] for key in custom_log_module._levelNames if isinstance(key, str)}
        if log_num is None:
            try:
                if lower_than is not None:
                    ...
                    raise Exception('Infomation about the ' + 'log_num should be provided')
            except KeyError:
        ...
        exec 'custom_log_module.Logger.' + eval('log_call') + ' = cust_log'
    ...
    def emit(self, record):
        try:
            if 'sthread' not in record.__dict__:
                record.sthread = str(record.thread)[10:] if len(str(record.thread)) > 10 else str(record.thread)
