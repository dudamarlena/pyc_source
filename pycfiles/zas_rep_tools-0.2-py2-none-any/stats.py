...
# [GCC 8.4.0]
# Embedded file name: /Users/egoruni/Desktop/BA/Code/zas-rep-tools/zas_rep_tools/src/classes/stats.py
# Compiled at: 2018-10-25 08:45:54
import os, sys
from raven import Client
import types, Queue, enlighten, json
...
class Stats(BaseContent, BaseDB):
    phenomena_table_map = {'repl': 'replications', 
       'redu': 'reduplications', 
       'baseline': 'baseline'}
    ...
    output_tables_col_names = {'baseline': {'all': 'occur_syntagma_all', 
                    'repl': {'uniq': 'occur_repl_uniq', 
                             'exhausted': 'occur_repl_exhausted'}, 
                    'redu': {'uniq': 'occur_redu_uniq', 
                             'exhausted': 'occur_redu_exhausted'}}}
    min_col = {'repl': (
              'id', 'doc_id', 'redufree_len', 'index_in_corpus', 'index_in_redufree', 'normalized_word', 'stemmed', 'in_redu'), 
       'redu': (
              'id', 'doc_id', 'redufree_len', 'index_in_corpus', 'index_in_redufree', 'redu_length', 'normalized_word', 'stemmed'), 
       'baseline': [
                  'syntagma', 'occur_syntagma_all', 'scope', 'stemmed']}
    ...
    def _init_compution_variables(self):
        ...
        self._cleaned_tags = {'number': ':number:', 
           'URL': ':URL:', 
           'symbol': ':symbol:', 
           'mention': ':mention:', 
           'hashtag': ':hashtag:'}
    ...
    def _init_column_index_variables(self):
        self.col_index_orig = {'repl': {colname:index for index, colname in enumerate(self.statsdb.col('replications'))}, 'redu': {colname:index for index, colname in enumerate(self.statsdb.col('reduplications'))}, 'baseline': {colname:index for index, colname in enumerate(self.statsdb.col('baseline'))}}
        self.col_index_min = {'repl': {colname:index for index, colname in enumerate(Stats.min_col['repl'])}, 'redu': {colname:index for index, colname in enumerate(Stats.min_col['redu'])}, 'baseline': {colname:index for index, colname in enumerate(Stats.min_col['baseline'])}}

    def additional_attr(self, repl_up, ignore_hashtag, ignore_url, ignore_mention, ignore_punkt, ignore_num, force_cleaning, case_sensitiv, full_repetativ_syntagma, min_scope_for_indexes, baseline_delimiter):
        additional_attributes = {'repl_up': repl_up, 
    ...
    def init(self, prjFolder, DBname, language, visibility, corpus_id=None, encryption_key=False, fileName=False, version=False, stats_id=False, context_lenght=5, full_repetativ_syntagma=False, min_scope_for_indexes=2, repl_up=3, ignore_hashtag=False, force_cleaning=False, baseline_delimiter='|+|', case_sensitiv=False, ignore_url=False, ignore_mention=False, ignore_punkt=False, ignore_num=False):
        if self.statsdb:
            ...
            was_initialized = self.statsdb.init('stats', prjFolder, DBname, language, visibility, corpus_id=corpus_id, encryption_key=encryption_key, fileName=fileName, version=version, stats_id=stats_id, db_frozen=False, context_lenght=context_lenght)
            ...
            if self.statsdb.exist():
                ...
                additional_attributes = self.additional_attr(repl_up, ignore_hashtag, ignore_url, ignore_mention, ignore_punkt, ignore_num, force_cleaning, case_sensitiv, full_repetativ_syntagma, min_scope_for_indexes, baseline_delimiter)
    ...
    def _init_attributesfor_dbhandler(self):
        init_attributes_db_handler = {'stop_if_db_already_exist': self._stop_if_db_already_exist, 
    ...
    def get_streams_from_corpus(self, inp_corp, stream_number, datatyp='dict', size_to_fetch=1000):
        ...
        def intern_gen(limit, offset):
            query = ('SELECT {}, {}  FROM  main.documents  LIMIT {} OFFSET {};').format(self._id_field_name, self._text_field_name, limit, offset)
    ...
    def export(self, path_to_export_dir, syntagma='*', repl=False, redu=False, baseline=True, syntagma_type='lexem', sentiment=False, fname=False, export_file_type='csv', rows_limit_in_file=1000000, encryption_key_corp=False, output_table_type='exhausted', additional_doc_cols=False, encryption_key_for_exported_db=False, path_to_corpdb=False, max_scope=False, stemmed_search=False, rewrite=False, context_len_left=True, context_len_right=True, separator_syn=' || ', word_examples_sum_table=True, ignore_num=False, ignore_symbol=False):
                    ...
                    try:
                        ...
                        for word in syntagma:
                            if word not in Stats._non_pos_tags:
                                self.logger.error(("POSGetterError: Additional POS-Tag was found in Syntagma. ('{}')  Current CorpusDB contain just default meta tags. ('{}') If you want to search in additional POS, than recompute CorpusDB with POS-Tagger.").format(word, Stats._non_pos_tags))
                                return False
        ...
        if not header:
            return False
        rows_generator = self._export_generator(header, inp_syntagma=syntagma, reptype_sum_table=reptype_sum_table, syntagma_type=syntagma_type, sentiment=sentiment, separator_syn=separator_syn, output_table_type=output_table_type, max_scope=max_scope, ignore_num=ignore_num, ignore_symbol=ignore_symbol, word_examples_sum_table=word_examples_sum_table, stemmed_search=stemmed_search)
    ...
    def _export_generator(self, header, inp_syntagma='*', syntagma_type='lexem', sentiment=False, output_table_type='exhausted', reptype_sum_table=False, separator_syn=' || ', thread_name='Thread0', ignore_num=False, ignore_symbol=False, word_examples_sum_table=True, max_scope=False, stemmed_search=False):
        if not separator_syn:
            ...
            if output_table_type == 'sum':
                ...
                data = self.compute_rep_sum(inp_syntagma, reptype_sum_table, syntagma_type=syntagma_type, sentiment=sentiment, stemmed_search=stemmed_search, thread_name=thread_name, ignore_num=ignore_num, ignore_symbol=ignore_symbol, word_examples_sum_table=word_examples_sum_table)
                ...
                data = self.get_data(inp_syntagma=inp_syntagma, repl=repl, redu=redu, baseline=baseline, syntagma_type=syntagma_type, sentiment=sentiment, thread_name=thread_name, max_scope=max_scope, stemmed_search=stemmed_search, send_empty_marker=True, minimum_columns=False, order_output_by_syntagma_order=False, return_full_tuple=False, delete_duplicates=True, get_columns_repl=False, get_columns_redu=False, get_columns_baseline=False, if_type_pos_return_lexem_syn=True)
                if not data:
                    ...
                    if repl:
                        ...
                        if redu:
                            ...
                            for singl_redu in vals_redu:
                                ...
                                redu_ids[singl_redu[ix_redu_id]] = (
                                 singl_redu[ix_doc_id_redu], singl_redu[ix_redu_in_redufree])
                ...
                if self._status_bar:
                    if status_bar_current_all.total != status_bar_current_all.count:
                        status_bar_current_all.count = status_bar_current_all.total
                        status_bar_current_all.refresh()
    ...
    def _convert_cols_to_indexes(self, get_columns_repl, get_columns_redu, get_columns_baseline, indexes):
        ...
        return (
         indexes_to_get_repl, indexes_to_get_redu, indexes_to_get_baseline)
    ...
    def compute_rep_sum(self, syntagma_to_search, reptype, syntagma_type='lexem', sentiment=False, stemmed_search=False, thread_name='Thread0', ignore_num=False, ignore_symbol=False, word_examples_sum_table=True):
        max_scope = 1
        if reptype == 'repl':
            repl = True
            redu = False
        ...
        if reptype == 'repl':
            ...
            get_columns_repl = ('doc_id', 'index_in_corpus', 'repl_letter', 'repl_length',
                                'rle_word', 'pos')
            i = 0
            for item in self.get_data(syntagma_to_search, repl=True, redu=False, baseline=False, get_columns_repl=get_columns_repl, max_scope=max_scope, sentiment=sentiment, syntagma_type=syntagma_type, stemmed_search=stemmed_search):
                if self._status_bar:
                    status_bar_current.update(incr=1)
                i += 1
            ...
            return summery
        get_columns_redu = (
         db_helper.tag_normalized_word, 'redu_length', 'pos')
        collected_redus_from_corp = defaultdict(lambda : defaultdict(lambda : 0))
        i = 0
        for item in self.get_data(syntagma_to_search, redu=True, repl=False, baseline=False, get_columns_redu=get_columns_redu, max_scope=max_scope, sentiment=sentiment, syntagma_type=syntagma_type, stemmed_search=stemmed_search):
            i += 1
            if self._status_bar:
                status_bar_current.update(incr=1)
    ...
    def _get_row_num_in_baseline_with_rep(self, redu=False, repl=False, max_scope=False):
        if repl or redu:
            ...
            if max_scope:
                where_str += (' AND scope<={} ').format(max_scope)
    ...
    def get_data(self, inp_syntagma='*', repl=False, redu=False, baseline=False, syntagma_type='lexem', sentiment=False, thread_name='Thread0', max_scope=False, stemmed_search=False, send_empty_marker=False, minimum_columns=False, order_output_by_syntagma_order=False, return_full_tuple=False, delete_duplicates=True, get_columns_repl=False, get_columns_redu=False, get_columns_baseline=False, if_type_pos_return_lexem_syn=False):
        if inp_syntagma == '*':
            return self._get_data(inp_syntagma=inp_syntagma, repl=repl, redu=redu, baseline=baseline, syntagma_type=syntagma_type, sentiment=sentiment, thread_name=thread_name, max_scope=max_scope, stemmed_search=stemmed_search, minimum_columns=minimum_columns, order_output_by_syntagma_order=order_output_by_syntagma_order, send_empty_marker=send_empty_marker, return_full_tuple=return_full_tuple, delete_duplicates=delete_duplicates, if_type_pos_return_lexem_syn=if_type_pos_return_lexem_syn, get_columns_repl=get_columns_repl, get_columns_redu=get_columns_redu, get_columns_baseline=get_columns_baseline)
        else:
            if thread_name not in self.preprocessors:
                if not self._init_preprocessors(thread_name=thread_name):
                    ...
                    return False
            try:
                inp_syntagma[0].decode
                extract_type = 1
            except AttributeError:
                try:
                    inp_syntagma[0][0].decode
                    extract_type = 2
            ...
            if extract_type == 1:
                gen = self._get_data(inp_syntagma=inp_syntagma, repl=repl, redu=redu, baseline=baseline, syntagma_type=syntagma_type, sentiment=sentiment, thread_name=thread_name, max_scope=max_scope, stemmed_search=stemmed_search, minimum_columns=minimum_columns, order_output_by_syntagma_order=order_output_by_syntagma_order, send_empty_marker=send_empty_marker, return_full_tuple=return_full_tuple, delete_duplicates=delete_duplicates, if_type_pos_return_lexem_syn=if_type_pos_return_lexem_syn, get_columns_repl=get_columns_repl, get_columns_redu=get_columns_redu, get_columns_baseline=get_columns_baseline)
                if not gen:
            ...
            for counter, inp_syn in enumerate(inp_syntagma):
                gen = self._get_data(inp_syntagma=inp_syn, repl=repl, redu=redu, baseline=baseline, syntagma_type=syntagma_type, sentiment=sentiment, thread_name=thread_name, max_scope=max_scope, stemmed_search=stemmed_search, send_empty_marker=send_empty_marker, minimum_columns=minimum_columns, order_output_by_syntagma_order=order_output_by_syntagma_order, return_full_tuple=return_full_tuple, delete_duplicates=delete_duplicates, if_type_pos_return_lexem_syn=if_type_pos_return_lexem_syn, get_columns_repl=get_columns_repl, get_columns_redu=get_columns_redu, get_columns_baseline=get_columns_baseline)
                if not gen:
                    not_init_gens += 1
                else:
    ...
    def _lexem_syn_extractor_from_pos(self, inp_syntagma, inpdata, repl=False, redu=False, baseline=False, sentiment=False, minimum_columns=False, order_output_by_syntagma_order=False, return_full_tuple=False, delete_duplicates=True, get_columns_repl=False, get_columns_redu=False, get_columns_baseline=False):
        max_scope = False
        ...
        def intern_gen():
            ...
            for b in exctracted_baseline:
                lexem_syn = b[0]
                data = self._get_data_for_one_syntagma(lexem_syn, repl=repl, redu=redu, baseline=baseline, syntagma_type='lexem', additional_pos_where=inp_syntagma, sentiment=sentiment, max_scope=False, for_optimization=False, stemmed_search=False, get_also_non_full_repetativ_result=False, order_output_by_syntagma_order=order_output_by_syntagma_order, return_full_tuple=return_full_tuple, minimum_columns=minimum_columns, delete_duplicates=delete_duplicates)
    ...
    def _get_data(self, inp_syntagma='*', repl=False, redu=False, baseline=False, syntagma_type='lexem', sentiment=False, thread_name='Thread0', max_scope=False, stemmed_search=False, send_empty_marker=False, minimum_columns=False, order_output_by_syntagma_order=False, return_full_tuple=False, delete_duplicates=True, get_columns_repl=False, get_columns_redu=False, get_columns_baseline=False, if_type_pos_return_lexem_syn=False):
        if not self._check_stats_db_should_exist():
            return False
        ...
        if inp_syntagma == '*':
            ...
            def intern_gen_all():
                for baseline_container in self._baseline('*', max_scope=max_scope):
                    data = self._get_data_for_one_syntagma(baseline_container[0], repl=repl, redu=redu, baseline=False, syntagma_type=syntagma_type, sentiment=sentiment, thread_name=thread_name, stemmed_search=False, max_scope=max_scope, order_output_by_syntagma_order=order_output_by_syntagma_order, return_full_tuple=return_full_tuple, delete_duplicates=delete_duplicates, minimum_columns=minimum_columns, indexes=indexes)
                    if data:
                        if baseline:
                            data['baseline'] = (
                             baseline_container,)
        ...
        if stemmed_search:
            ...
            def intern_gen_2():
                ...
                where = tuple(self._get_where_statement(inp_syntagma, scope=scope, thread_name=thread_name, with_context=False, syntagma_type='lexem', sentiment=sentiment, stemmed_search=True))
                if not where:
                    yield False
                    return
                else:
                    for baseline_container in self._baseline(inp_syntagma, where=where, minimum_columns=minimum_columns, max_scope=max_scope, split_syntagma=True):
                        data = self._get_data_for_one_syntagma(baseline_container[0], repl=repl, redu=redu, baseline=False, syntagma_type=syntagma_type, sentiment=sentiment, thread_name=thread_name, max_scope=False, stemmed_search=False, order_output_by_syntagma_order=order_output_by_syntagma_order, minimum_columns=minimum_columns, return_full_tuple=return_full_tuple, delete_duplicates=delete_duplicates, indexes=indexes)
                        if data:
                            data['baseline'] = (
                             baseline_container,)
                            ...
                            if get_columns_repl or get_columns_redu or get_columns_baseline:
                                data = self._extract_certain_columns(data, indexes_to_get_repl, indexes_to_get_redu, indexes_to_get_baseline)
                            yield data
                        elif send_empty_marker:
                            yield

                    return

            if if_type_pos_return_lexem_syn and syntagma_type == 'pos':
                return self._lexem_syn_extractor_from_pos(inp_syntagma, intern_gen_all(), repl=repl, redu=redu, baseline=baseline, sentiment=sentiment, minimum_columns=minimum_columns, order_output_by_syntagma_order=order_output_by_syntagma_order, return_full_tuple=return_full_tuple, delete_duplicates=delete_duplicates, get_columns_repl=get_columns_repl, get_columns_redu=get_columns_redu, get_columns_baseline=get_columns_baseline)
            return LenGen(intern_gen_2(), num)
        else:

            def inter_gen_3():
                data = self._get_data_for_one_syntagma(inp_syntagma, repl=repl, redu=redu, baseline=baseline, syntagma_type=syntagma_type, sentiment=sentiment, thread_name=thread_name, max_scope=max_scope, stemmed_search=False, order_output_by_syntagma_order=order_output_by_syntagma_order, minimum_columns=minimum_columns, return_full_tuple=return_full_tuple, delete_duplicates=delete_duplicates, indexes=indexes)
                if data:
                    if get_columns_repl or get_columns_redu or get_columns_baseline:
                        data = self._extract_certain_columns(data, indexes_to_get_repl, indexes_to_get_redu, indexes_to_get_baseline)
                    yield data
            ...
            if if_type_pos_return_lexem_syn and syntagma_type == 'pos':
                return self._lexem_syn_extractor_from_pos(inp_syntagma, inter_gen_3(), repl=repl, redu=redu, baseline=baseline, sentiment=sentiment, minimum_columns=minimum_columns, order_output_by_syntagma_order=order_output_by_syntagma_order, return_full_tuple=return_full_tuple, delete_duplicates=delete_duplicates, get_columns_repl=get_columns_repl, get_columns_redu=get_columns_redu, get_columns_baseline=get_columns_baseline)
            return LenGen(inter_gen_3(), 1)

    def _get_data_for_one_syntagma(self, inp_syntagma_splitted, inp_syntagma_unsplitted=False, repl=False, redu=False, baseline=False, syntagma_type='lexem', additional_pos_where=False, sentiment=False, thread_name='Thread0', max_scope=False, for_optimization=False, stemmed_search=False, get_also_non_full_repetativ_result=False, order_output_by_syntagma_order=False, return_full_tuple=False, output_type='list', minimum_columns=False, delete_duplicates=True, indexes=False):
        ...
        if not self._is_syntagma_scope_right(scope):
            ...
            if repl:
                if not where1:
                    where1 = tuple(self._get_where_statement(inp_syntagma_splitted, scope=scope, thread_name=thread_name, with_context=True, syntagma_type=syntagma_type, sentiment=sentiment, inp_syntagma_unsplitted=inp_syntagma_unsplitted, stemmed_search=stemmed_search, additional_pos_where=additional_pos_where))
                    if not where1:
                        return False
                _repl = self.get_reps('repl', inp_syntagma_splitted, scope, where1, indexes, thread_name=thread_name, minimum_columns=minimum_columns, order_output_by_syntagma_order=order_output_by_syntagma_order, return_full_tuple=return_full_tuple, stemmed_search=False, output_type=output_type, delete_duplicates=delete_duplicates, syntagma_type=syntagma_type, for_optimization=for_optimization, get_also_non_full_repetativ_result=get_also_non_full_repetativ_result)
            if redu:
                if not where1:
                    where1 = tuple(self._get_where_statement(inp_syntagma_splitted, scope=scope, thread_name=thread_name, with_context=True, syntagma_type=syntagma_type, sentiment=sentiment, inp_syntagma_unsplitted=inp_syntagma_unsplitted, stemmed_search=stemmed_search, additional_pos_where=additional_pos_where))
                    if not where1:
                        return False
                _redu = self.get_reps('redu', inp_syntagma_splitted, scope, where1, indexes, thread_name=thread_name, minimum_columns=minimum_columns, order_output_by_syntagma_order=order_output_by_syntagma_order, return_full_tuple=return_full_tuple, stemmed_search=False, output_type=output_type, delete_duplicates=delete_duplicates, syntagma_type=syntagma_type, for_optimization=for_optimization, get_also_non_full_repetativ_result=get_also_non_full_repetativ_result)
            if baseline:
                if syntagma_type == 'lexem':
                    where2 = tuple(self._get_where_statement(inp_syntagma_splitted, scope=scope, thread_name=thread_name, with_context=False, syntagma_type=syntagma_type, sentiment=sentiment, inp_syntagma_unsplitted=inp_syntagma_unsplitted, stemmed_search=stemmed_search, additional_pos_where=additional_pos_where))
                    if not where2:
                        return False
                    ...
                    for temp_syntagma in set(all_syntagmas):
                        where2 = tuple(self._get_where_statement(temp_syntagma, scope=scope, thread_name=thread_name, with_context=False, syntagma_type='lexem', sentiment=sentiment, inp_syntagma_unsplitted=inp_syntagma_unsplitted, stemmed_search=stemmed_search, additional_pos_where=False))
                        if not where2:
                            return False
    ...
    def get_reps(self, rep_type, inp_syntagma_splitted, scope, where, indexes, thread_name='Thread0', order_output_by_syntagma_order=False, return_full_tuple=False, stemmed_search=False, output_type='list', minimum_columns=False, delete_duplicates=True, syntagma_type='lexem', for_optimization=False, get_also_non_full_repetativ_result=False):
        _rep = []
        ...
        if order_output_by_syntagma_order:
            ...
            if for_optimization:
                return False
        if _rep:
            if get_also_non_full_repetativ_result:
                return _rep
    ...
    def _exctract_full_syntagmas(self, reconstr_tree, scope, redu_free_elem_length, inp_syntagma_splitted, syntagma_type='lexem'):
        try:
            ...
            for doc_id, doc_data in dict(sorted(reconstr_tree.items())).iteritems():
                redu_free_length = [ l - 1 for l in redu_free_elem_length[doc_id] ]
                current_syn_ixs = ()
                syn_start_word = None
                start_tok = None
                temp_tok = None
                    ...
                    for current_tok in sorted(sents_data.keys()):
                        ...
                        if not start_tok:
                            ...
                            if tok_to_use[1] + counter_full_syn == current_tok:
                                counter_full_syn += 1
                                incr = True
                            elif current_tok == 0 and current_sent - last_sent == 1 and last_token == redu_free_length[last_sent]:
    ...
    def _delete_dublicats_in_reps(self, reps, order_output_by_syntagma_order, id_ix):
        ...
        if order_output_by_syntagma_order:
            for word, reps in reps[::-1]:
                ...
                elif self._full_repetativ_syntagma:
                    new_reps = ()
                    break
                else:
                    new_reps.append((word, temp_reps))
    ...
    def _rep_getter_from_db(self, rep_type, inp_syntagma='*', scope=False, where=False, output_type='list', size_to_get=1000, columns=False, thread_name='Thread0', for_optimization=False):
        if inp_syntagma != '*':
            if not where:
    ...
    def _get_where_statement(self, inp_syntagma_splitted, inp_syntagma_unsplitted=False, scope=False, syntagma_type='lexem', sentiment=False, thread_name='Thread0', with_context=True, stemmed_search=False, additional_pos_where=False):
        status = True
        convert = False
        if syntagma_type != 'pos':
            try:
                ...
                if with_context:
                    ...
                    for token_index in xrange(scope):
                        ...
                        for i, token in zip(range(scope), inp_syntagma_splitted):
                            if i < token_index:
                                col_name = ('{}{}').format(context_tag_name_l, token_index - i)
                                search_pattern = ("{}='{}'").format(col_name, token) if syntagma_type == 'lexem' and not stemmed_search else ('json_extract("{}", "$[{}]")  = "{}"').format(col_name, word_index, token)
                                where.append(search_pattern)
                                if additional_pos_where and syntagma_type != 'pos':
                                    col_name = ('{}{}').format('context_infoL', token_index - i)
                                    search_pattern = ('json_extract("{}", "$[0]")  = "{}"').format(col_name, additional_pos_where[i])
                                    where.append(search_pattern)
                            elif i == token_index:
                                where.append(("{}='{}' ").format(normalized_word_tag_name, token))
                                if additional_pos_where and syntagma_type != 'pos':
                                    where.append((" pos = '{}' ").format(additional_pos_where[i]))
                            elif i > token_index:
                                col_name = ('{}{}').format(context_tag_name_r, i - token_index)
                                search_pattern = ("{}='{}'").format(col_name, token) if syntagma_type == 'lexem' and not stemmed_search else ('json_extract("{}", "$[{}]")  = "{}"').format(col_name, word_index, token)
                                where.append(search_pattern)
                                if additional_pos_where and syntagma_type != 'pos':
                                    col_name = ('{}{}').format('context_infoR', i - token_index)
                                    search_pattern = ('json_extract("{}", "$[0]")  = "{}"').format(col_name, additional_pos_where[i])
                                    where.append(search_pattern)

                        if sentiment:
                            where.append(("polarity LIKE '%{}%'").format(sentiment))
                        yield where
                    ...
                    syntagma_qeary = ("{}= '{}'").format(syntagma_tag, inp_syntagma_unsplitted)
    ...
    def compute(self, inp_corp, stream_number=1, datatyp='dict', adjust_to_cpu=True, min_files_pro_stream=1000, cpu_percent_to_get=50, thread_name='Thread0', create_indexes=True, freeze_db=False, drop_indexes=True, optimized_for_long_syntagmas=True, baseline_insertion_border=1000000):
        if not self._check_stats_db_should_exist():
            return False
        else:
            if not self._check_db_should_be_an_stats():
            ...
            try:
                if not isinstance(inp_corp, Corpus):
                    ...
                    elif self._language != self._corp_info['language']:
    ...
    def _compute(self, inp_data, datatyp='dict', thread_name='Thread0', baseline_insertion_border=1000000, add_also_repeted_redu_to_baseline=True):
        try:
            if not self._check_corp_should_exist():
                ...
                if self._status_bar:
                    if not status_bar_insertion_in_the_current_thread:
                        return False
    ...
    def _get_occur(self, counted_rep, scope=1, splitted_syntagma=False):
        if scope > 1:
            ...
            if splitted_syntagma:
                ...
                return (
                 occur_uniq_output, occur_e_output)
            ...
            return (
             occur_uniq, occur_rep_exhausted)
    ...
    def _compute_baseline_sum(self, insertion_border=10000, thread_name='Thread0', size_to_fetch=10000):
        if not self._check_stats_db_should_exist():
            ...
            for i, baseline_container in enumerate(self.statsdb.lazyget('baseline', thread_name='baseline_sum')):
                ...
                where = tuple(self._get_where_statement(splitted_syntagma, scope=scope, thread_name=thread_name, with_context=True, syntagma_type='lexem'))
                if not where:
                    return False
                repls_container = self.get_reps('repl', splitted_syntagma, scope, where, indexes, thread_name=thread_name, return_full_tuple=True, delete_duplicates=False, syntagma_type=syntagma_type, minimum_columns=minimum_columns)
                redus_container = self.get_reps('redu', splitted_syntagma, scope, where, indexes, thread_name=thread_name, return_full_tuple=True, delete_duplicates=False, syntagma_type=syntagma_type, minimum_columns=minimum_columns)
                ...
                if scope == 1:
                    ...
                    if repls_container:
                        ...
                        for repl in repls:
                            counted_repls[repl[ix_word_repl]][repl[ix_doc_id_repl]][repl[ix_token_repl]] += 1
    ...
    def extract_reduplications(self, repl_free_text_container, rle_for_repl_in_text_container, thread_name='Thread0'):
        ...
        for sent in repl_free_text_container:
            ...
            for rep in extracted_reps:
                ...
                rep_free_sent[i_redu_free] = (
                 rep_free_sent[i_redu_free], dict(counts))
        ...
        return (
         extracted_redu_in_text_container, redu_free_text_container, mapping_redu)
    ...
    def extract_replications(self, text_elem, thread_name='Thread0'):
        ...
        return (
         extracted_repl_in_text_container, repl_free_text_container, rle_for_repl_in_text_container)
                    ...
                    for repl_container in repls_for_current_token:
                        if repl_container:
                            try:
                                ...
                                input_dict = {'doc_id': doc_elem[0], 
                                   'redufree_len': redufree_len, 
                                   'index_in_corpus': (
                                                     sent_index, token_index), 
                                   'index_in_redufree': (
                                                       sent_index, token_index_in_redu_free), 
            ...
            for redu in redu_in_sent:
                try:
                    ...
                    input_dict = {'doc_id': doc_elem[0], 
                       'redufree_len': redufree_len, 
                       'index_in_corpus': (
                                         sent_index, redu['start_index_in_orig']), 
                       'index_in_redufree': (
                                           sent_index, index_in_redu_free), 
    ...
    def _get_context_left_for_repl(self, input_dict, text_elem, token_index_in_redu_free, mapping_redu, redu_free_text_container, sent_index, stemmed_text_container):
        for context_number in range(1, self._context_lenght + 1):
            ...
            if temp_index >= 0:
                ...
                leftover_contextnumber = context_number - token_index_in_redu_free
                if not context_number:
    ...
    def _get_context_right_for_repl(self, input_dict, text_elem, token_index_in_redu_free, mapping_redu, redu_free_text_container, sent_index, stemmed_text_container):
        for context_number in range(1, self._context_lenght + 1):
            ...
            if temp_index < len(redu_free_text_container[sent_index]):
                ...
                leftover_contextnumber = context_number - (len(redu_free_text_container[sent_index]) - (token_index_in_redu_free + 1))
                if not leftover_contextnumber:
    ...
    def _get_context_right_for_redu(self, input_dict, text_elem, mapping_redu, redu_free_text_container, sent_index, redu, stemmed_text_container):
        for context_number in range(1, self._context_lenght + 1):
            ...
            if temp_index < len(redu_free_text_container[sent_index]):
                ...
                leftover_contextnumber = context_number - (len(redu_free_text_container[sent_index]) - (redu['index_in_redu_free'] + 1))
                if not leftover_contextnumber:
                    raise Exception, '1. WrongContextNumber. It should be >0'
                number_of_loops = 0
                while True:
    ...
    def _get_context_left_for_redu(self, input_dict, text_elem, mapping_redu, redu_free_text_container, sent_index, redu, stemmed_text_container):
        for context_number in range(1, self._context_lenght + 1):
            ...
            if temp_index >= 0:
                ...
                leftover_contextnumber = context_number - redu['index_in_redu_free']
                if not context_number:
                    raise Exception, 'WrongContextNumber. It should be >0'
                number_of_loops = 0
                while True:
        ...
        try:
            if not self._check_stats_db_should_exist():
                ...
                for baseline_container in gen:
                    ...
                    scope = len(inp_syntagma_splitted)
                    syntagma_type = 'lexem'
                    where = self._get_where_statement(inp_syntagma_splitted, scope=scope, thread_name=thread_name, with_context=True, syntagma_type=syntagma_type)
                    collected_w = ()
                    for w in where:
                        if w:
                            current_reps = self.statsdb._threads_cursors[thread_name].execute(('SELECT id FROM replications WHERE {} {};').format((' AND ').join(w), case)).fetchone()
                            if current_reps:
                                was_found = True
                                break
                            collected_w += (w,)
                    ...
                    for w in collected_w:
                        if w:
                            current_reps = self.statsdb._threads_cursors[thread_name].execute(('SELECT id FROM reduplications WHERE {} {};').format((' AND ').join(w), case)).fetchone()
                            if current_reps:
                                was_found = True
                                break
                        else:
        ...
        try:
            ...
            for r in self.statsdb.getall('replications', columns=normalized_word_tag, distinct=True):
                ...
                if r[0] not in baseline:
                    ...
                    self.logger.debug(("StatsDB is inconsistence. There Exist NO-Baseline-Entry  for '{}'-word ").format(word))
    ...
    def _get_created_indexes(self):
        all_indexes = self.statsdb._get_indexes_from_db()
        pattern = re.compile('create.+index(.+)on\\s.*\\((.+)\\)', re.IGNORECASE)
        pattern_index_columns = re.compile('\\((.+)\\)')
    ...
    def _drop_created_indexes(self, table_name='*'):
        ...
        elif table_name not in self.statsdb.tables():
                ...
                for token_index in xrange(step):
                    ...
                    for i in xrange(step):
                        if i < token_index:
                            ...
                            col_name = ('{}').format(normalized_word_tag_name)
                            temp_columns.append(col_name)
                        elif i > token_index:
                            col_name = ('{}{}').format(context_tag_name_r, i - token_index)
                            temp_columns.append(col_name)
    ...
    def _get_biggest_column_pairs_for_indexes(self, raw_columns_to_index):
        for syntagma_type, column_pairs in raw_columns_to_index.iteritems():
            ...
            for column_pair in column_pairs:
                ...
                elif len(temp_pairs_for_current_syntagma_type[column_pair[0]]) < len(column_pair):
                    temp_pairs_for_current_syntagma_type[column_pair[0]] = column_pair
    ...
    def _isrighttype(self, inp_data):
        check = (
         isinstance(inp_data, list), isinstance(inp_data, LenGen))
