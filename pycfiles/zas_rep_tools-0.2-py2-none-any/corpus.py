...
# [GCC 8.4.0]
# Embedded file name: /Users/egoruni/Desktop/BA/Code/zas-rep-tools/zas_rep_tools/src/classes/corpus.py
# Compiled at: 2018-10-25 08:52:40
from __future__ import absolute_import
import os, copy, sys, logging, threading, time, Queue, json, traceback
...
import enlighten, multiprocessing as mp
...
if platform.uname()[0].lower() != 'windows':
    ...
    import colorama
    os.system('color 09252d')

class Corpus(BaseContent, BaseDB, CorpusData):

    def __init__(self, use_test_pos_tagger=False, tok_split_camel_case=True, end_file_marker=-1, use_end_file_marker=False, status_bar=True, heal_me_if_possible=False, **kwargs):
    ...
    def additional_attr(self, language, tokenizer, pos_tagger, sentiment_analyzer, sent_splitter, preprocession, lang_classification, del_url, del_punkt, del_num, del_mention, del_hashtag, del_html, case_sensitiv, emojis_normalization, text_field_name, id_field_name):
        additional_attributes = {'language': language, 
           'tokenizer': tokenizer, 
    ...
    def init(self, prjFolder, DBname, language, visibility, platform_name, encryption_key=False, fileName=False, source=False, license=False, template_name=False, version=False, cols_and_types_in_doc=False, corpus_id=False, tokenizer=True, pos_tagger=False, sentiment_analyzer=False, sent_splitter=False, preprocession=True, lang_classification=False, del_url=False, del_punkt=False, del_num=False, del_mention=False, del_hashtag=False, del_html=False, case_sensitiv=False, emojis_normalization=True, text_field_name='text', id_field_name='id'):
        if self.corpdb:
            ...
            was_initialized = self.corpdb.init('corpus', prjFolder, DBname, language, visibility, platform_name=platform_name, encryption_key=encryption_key, fileName=fileName, source=source, license=license, template_name=template_name, version=version, cols_and_types_in_doc=cols_and_types_in_doc, corpus_id=corpus_id)
            if not was_initialized:
                ...
                return False
            self.corpdb.update_attrs(self.additional_attr(language, tokenizer, pos_tagger, sentiment_analyzer, sent_splitter, preprocession, lang_classification, del_url, del_punkt, del_num, del_mention, del_hashtag, del_html, case_sensitiv, emojis_normalization, text_field_name, id_field_name))
        ...
        try:
            if self._preprocession:
                if not self._terminated:
                    p_list = [
                     self._sent_splitter, self._pos_tagger, self._lang_classification, self._tokenizer, self._sentiment_analyzer]
    ...
    def _valid_input(self, language, preprocession, tokenizer, sent_splitter, pos_tagger, sentiment_analyzer):
        ...
        if preprocession:
            ...
            if sentiment_analyzer:
                ...
                elif sentiment_analyzer not in Corpus.supported_sentiment_analyzer:
            ...
            if token_container[1] in pattern:
                if prev_pos:
                    if prev_pos == token_container[1]:
                        new_text_elem = ('{}{}').format(collected_elem[0], token_container[0])
    ...
    def _error_correction_after_somajo_tokenization(self, output):
        ...
        output = [ (token_cont[0], 'symbol') if text_is_punkt(token_cont[0]) and token_cont[1] != 'symbol' else token_cont for token_cont in output ]
    ...
    def _preprocessing(self, inp_str, thread_name='Thread0', log_ignored=True, row=False):
        ...
        if self._del_html:
            ...
            output = remove_html_codded_chars(output)
        if self._lang_classification:
            output_with_deleted_repetitions = self.preprocessors[thread_name]['rle'].del_rep(output)
            lang = langid.classify(output_with_deleted_repetitions)[0]
            if lang != self._language:
                try:
                    if log_ignored:
                        row = row if row else ':ORIG_ROW_WASNT_GIVEN:'
                        self.logger.outsorted_corpus(("LangClassification: Following row was out-sorted and ignored. Reason: Given language not equal to recognized language ('{}'!='{}');  TextElement: '{}'; FullRow: '{}'. ").format(self._language, lang, output, row))
                except Exception as e:
            ...
            output = self._case_sensitiv or self._lower_case_sents(output)
    ...
    def _rebuild_non_regular_tokens(self, non_regular_tokens, output):
        for backup_data in non_regular_tokens:
            output[backup_data[0]][backup_data[1]] = (
             output[backup_data[0]][backup_data[1]][0], backup_data[2])
