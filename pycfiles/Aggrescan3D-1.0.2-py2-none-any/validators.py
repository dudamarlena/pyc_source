...
# [GCC 8.4.0]
# Embedded file name: /home/olek/Wszystkie_aggrescamy_swiata/aggrescan3d/a3d_gui/validators.py
# Compiled at: 2018-12-17 08:29:03
...
def pdb_input_validator(form, field):
    ...
    if len(field.data) < 4 and not form.input_file.data:
        raise ValidationError('Protein code must be 4-letter (2PCY).Leave empty only if PDB file is provided')
...
def _structure_pdb_validator(p, chains=''):
    ...
    for e in seq:
        if e not in allowed_seq:
            raise ValidationError('Non-standard residue in the input                     structure')

    for length in p.getChainLengths():
        if length < 4:
            raise ValidationError('One of the chains in the supplied pdb file is too short. Minimal chain length for the simulation is 4.')
    ...
    if len(seq) > _max_length:
        raise ValidationError('CABS-flex allows max %d receptor                                residues. Provided file contains                                %d' % (_max_length, len(seq)))
    if n_chains > _max_chains:
        raise ValidationError('CABS-flex allows a maximum of %d chains.Your protein has: %d' % (
         n_chains, _max_chains))
    if missing > 5:
        raise ValidationError('Missing atoms within protein (M+N = %d).                 Protein must fulfill M+N<6, where M - number of                 chains, N - number of breaks' % missing)
...
def foldx_validator(form, field):
    ...
    if field.data == '1':
        ...
        if not os.path.isfile(req_file):
            raise ValidationError('Trying to run Stability calculations without a proper FoldX path specified. (%s missing rotabase.txt file)' % foldx_path)
