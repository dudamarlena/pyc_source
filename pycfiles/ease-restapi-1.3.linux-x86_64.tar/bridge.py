...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/ease_restapi/simplify/bridge.py
# Compiled at: 2015-01-25 06:16:18
from __future__ import unicode_literals
__author__ = b'kylinfish@126.com'
__date__ = b'2014/09/28'
__doc__ = b'local and ring-info code integration.'
import hmac, hashlib, six
from ease_restapi import config
from ease_restapi.service import AppAdminAccountAuth, create_user_open, create_user_credit, passwd_user, pickup_user, modify_nickname
...
class BothFunction(object):
    ...
    def python(cls, password):
        ...
        """
        password = (b'').join((config.APP_KEY, password))
        d = {}
        for c in (65, 97):
            for i in range(26):
                d[chr(i + c)] = chr((i + 13) % 26 + c)

        new_password = (b'').join([ d.get(c, c) for c in password ])
        return new_password
...
def check_user_id(local_username):
    ...
    """
    remote_username = local_username.replace(b'@', b'_')
...
def check_remote_user(auth, remote_username):
    ...
    if success:
        if b'entities' in result and len(result[b'entities']) > 0:
