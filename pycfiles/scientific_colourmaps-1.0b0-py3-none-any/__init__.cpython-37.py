...
# [GCC 8.4.0]
# Embedded file name: C:\Users\GMWERB\Software\scientific-colormaps\python\scientific_colourmaps\__init__.py
# Compiled at: 2018-10-30 20:22:04
...
def list_cmaps(cmap_path=CMAP_DEFAULT_PATH):
    with ZipFile(cmap_path, 'r') as (archive):
        return {p.parent.name for p in (Path(zi.filename) for zi in archive.infolist()) if p.suffix == '.txt' if p.suffix == '.txt'}