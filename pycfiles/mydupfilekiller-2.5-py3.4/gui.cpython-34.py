...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\mydupfilekiller\gui.py
# Compiled at: 2014-07-06 09:20:23
...
try:
    ...
    class App(wx.App):
        ...
        def OnInit(self):
            ...
            self.res.Load(pkg_resources.ResourceManager().resource_filename('mydupfilekiller', 'wx_gui.xrc'))
            self.init_frame()
            return True
        ...
        def OnAdd(self, event):
            ...
            if not os.path.exists(path) or not os.path.isdir(path):
                wx.MessageBox('Wrong path!', 'My Duplicate File Killer %s' % __version__, wx.OK | wx.ICON_ERROR)
                return
            if path in self.paths:
                wx.MessageBox('Path already exists!', 'My Duplicate File Killer %s' % __version__, wx.OK | wx.ICON_ERROR)
                return
        ...
        def working_function(self):
            find_and_delete(self.paths, self.choose_callback, False, self.skip_empty_files.GetValue(), self.follow_links.GetValue())
            self.start.Enable()
...
def require_wx():
    showerror('My Duplicate File Killer %s' % __version__, 'Neither wxPython-Phoenix nor PyGI installed. Please type pip install --upgrade --pre -f http://wxpython.org/Phoenix/snapshot-builds wxpython-phoenix.')
