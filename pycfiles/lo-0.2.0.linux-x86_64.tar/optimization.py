...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/lo/iterative/optimization.py
# Compiled at: 2010-10-24 15:20:11
"""
...
class QuadraticModel(Model):
    ...
    def __init__(self, M, b, Ds=[], hypers=[], **kargs):
        ...
        norms = n * (norm2,)
        dnorms = n * (dnorm2,)
        Model.__init__(self, M, b, Ds=Ds, hypers=hypers, norms=norms, dnorms=dnorms, **kargs)
...
class QuadraticOptimalStep:
    ...
    def __call__(self, origin, function, state, **kwargs):
        ...
        g = state['gradient']
        a = -0.5 * np.dot(d.T, g)
        a /= norm2(self.M * d) + sum([ h * norm2(D * d) for h, D in zip(self.hypers, self.Ds)
                                     ])
        state['alpha_step'] = a
        return origin + a * d
...
class BacktrackingFromOptimal:
    ...
    def __init__(self, M, b, Ds=[], hypers=[], rho=0.1, alpha_step=1.0, alpha_factor=0.5, **kwargs):
...
def quadratic_optimization(M, b, Ds=[], hypers=[], maxiter=None, tol=1e-06, min_alpha_step=1e-10, **kwargs):
    """
    ...
    myoptimizer = VerboseStandardOptimizer(function=model, step=mystep, line_search=mylinesearch, criterion=mycriterion, x0=np.zeros(M.shape[1]))
    return myoptimizer.optimize()


def huber_optimization(M, b, Ds=[], hypers=[], maxiter=None, tol=1e-06, min_alpha_step=1e-10, **kwargs):
    """
    Use scikits.optimization to perform least-square inversion.
    """
    from scikits.optimization import step, line_search, criterion, optimizer
