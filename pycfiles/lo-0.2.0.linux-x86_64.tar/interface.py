...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/lo/interface.py
# Compiled at: 2011-01-19 02:00:47
import numpy as np
...
class LinearOperator():
    ...
    def __init__(self, shape, matvec, rmatvec=None, matmat=None, rmatmat=None, dtypein=None, dtypeout=None, dtype=None):
        shape = tuple(shape)
        self.shape = shape
        self._matvec = matvec
        if rmatvec is None:
    ...
    def __mul__(self, x):
        if np.isscalar(x):
            ...
            return LinearOperator(self.shape, matvec, rmatvec=rmatvec, matmat=matmat, rmatmat=rmatmat)
        else:
            if isinstance(x, LinearOperator):
                ...
                if self._rmatmat is not None and x._matmat is not None:
                    ...
                    rmatmat = None
                return LinearOperator(shape, matvec, rmatvec=rmatvec, matmat=matmat, rmatmat=rmatmat, dtypein=x.dtypein, dtypeout=self.dtypeout)
            x = np.asarray(x)
    ...
    def __add__(self, A):
        if isinstance(A, LinearOperator):
            ...
            if self._matmat is not None and A._matmat is not None:
                ...
                rmatmat = None
            return LinearOperator(self.shape, matvec, rmatvec=rmatvec, matmat=matmat, rmatmat=rmatmat, dtype=self.dtype, dtypein=self.dtypein, dtypeout=self.dtypeout)
        else:
            if np.isscalar(A):
                return self.__add__(A * aslinearoperator(np.eye(self.shape[0], self.shape[1], dtype=self.dtype)))
    ...
    def T(self):
        ...
        return LinearOperator(self.shape[::-1], matvec, rmatvec=rmatvec, matmat=matmat, rmatmat=rmatmat, dtype=dtype, dtypein=dtypeout, dtypeout=dtypein)
...
def aslinearoperator(A):
    ...
    if isinstance(A, LinearOperator):
        ...
        if isinstance(A, np.ndarray) or isinstance(A, np.matrix):
            ...
            return LinearOperator(A.shape, matvec, rmatvec=rmatvec, matmat=matmat, rmatmat=rmatmat, dtype=A.dtype)
        if hasattr(A, 'shape') and hasattr(A, 'matvec'):
            rmatvec = getattr(A, 'rmatvec', None)
            matmat = getattr(A, 'matmat', None)
            rmatmat = getattr(A, 'rmatmat', None)
            dtype = getattr(A, 'dtype', None)
            return LinearOperator(A.shape, A.matvec, rmatvec=rmatvec, matmat=matmat, rmatmat=rmatmat, dtype=dtype)
...
def block_diagonal(As):
    ...
    dtype = As[0].dtype
    shape = (
     np.sum([ A.shape[0] for A in As ]), np.sum([ A.shape[1] for A in As ]))
