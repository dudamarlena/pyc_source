...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/process_sat/function_unittests.py
# Compiled at: 2012-08-02 11:10:38
"""
Run unit tests on the parse, geo, out, and map functions
"""
import unittest, os, sys, tempfile
from itertools import izip, product
import pdb, numpy, netCDF4, tables, parse_geo, grid_geo, map_geo, out_geo, utils
...
class TestKnmiOmiL2Parser(unittest.TestCase):
    ...
    def avKern_6_5(self):
        return numpy.array([0, 0.335, 0.728, 0.741, 0.807, 0.838, 0.863, 0.898, 0.937,
         0.986, 1.038, 1.077, 1.092, 1.083, 1.073, 1.068,
         1.068, 1.066, 1.056, 1.047, 1.039, 1.031, 1.026,
         1.018, 1.009, 1.005, 0.993, 0.98, 0.957, 0.933, 0.875,
         0.824, 0.819, 0.925])
    ...
    def test_get_retrieves_right_type(self):
        ...
        for key, type in izip(self.checkKeys, types):
            self.assertEqual(self.parser.get(key).dtype, type, msg='Type did not match for field %s.' % key)
    ...
    def test_get_cm_retrieves_right_type(self):
        ...
        with self.parser as (p):
            for key, type in izip(self.checkKeys, types):
                self.assertEqual(p.get_cm(key).dtype, type, msg='Mismatched type in field %s.' % key)
    ...
    def test_get_retrieves_valid_at_6_5(self):
        ...
        vals = [numpy.array([3.9495022]),
         ...
         self.TM4presLev,
         numpy.array([0]),
         numpy.array([5.801014241989858 * pow(10, 14)]),
         numpy.array([-1]),
         numpy.array([73.72836, 73.68924, 73.12749, 73.089355]),
         numpy.array([5.782786651906409 * pow(10, 8)])]
        for key, val in izip(self.checkKeys, vals):
            numpy.testing.assert_allclose(self.parser.get(key, (6, 5)), val, rtol=1e-05)
    ...
    def test_get_cm_retrieves_valid_at_6_5(self):
        ...
        with self.parser as (p):
            for key, val in izip(self.checkKeys, vals):
                numpy.testing.assert_allclose(p.get_cm(key, (6, 5)), val, rtol=1e-05)

    def test_get_zero_order_array_from_single_point(self):
        self.assertEqual(numpy.rank(self.parser.get('CloudFraction', (6, 5))), 0)
        self.assertEqual(numpy.rank(self.parser.get('SolarZenithAngle', (6, 5))), 0)
        self.assertEqual(numpy.rank(self.parser.get('CloudFraction', (0, 0))), 0)
        self.assertEqual(numpy.rank(self.parser.get('SolarZenithAngle', (6, 5))), 0)

    def test_get_cm_zero_order_array_from_single_point(self):
        with self.parser as (p):
            self.assertEqual(numpy.rank(p.get_cm('CloudFraction', (6, 5))), 0)
            self.assertEqual(numpy.rank(p.get_cm('SolarZenithAngle', (6, 5))), 0)
            self.assertEqual(numpy.rank(p.get_cm('CloudFraction', (0, 0))), 0)
            self.assertEqual(numpy.rank(p.get_cm('SolarZenithAngle', (6, 5))), 0)
...
class TestKnmiOmiL2GetGeoCorners(TestKnmiOmiL2Parser):
    ...
    def test_geocorners_size(self):
        self.assertEqual(self.geoarray.size, 97320)
    ...
    def test_geocorners_lat_first_element(self):
        ...
        for i in range(aFlat.shape[0]):
            if numpy.allclose(aFlat[i], firstElLat):
                break

        numpy.testing.assert_array_equal(self.geoarray['ind'].reshape((-1, 2))[i], (0,
                                                                                    0))
    ...
    def test_geocorners_lon_first_element(self):
        ...
        for i in range(aFlat.shape[0]):
            if numpy.allclose(aFlat[i], firstElLon):
                break
    ...
    def test_geocorners_lat_last_element(self):
        ...
        aFlat = self.geoarray['lat'].reshape((-1, 4))
        for i in range(aFlat.shape[0]):
            if numpy.allclose(aFlat[i], finalElLat):
                break

        numpy.testing.assert_array_equal(self.geoarray['ind'].reshape((-1, 2))[i], (1621,
                                                                                    59))
    ...
    def test_geocorners_lon_last_element(self):
        ...
        aFlat = self.geoarray['lon'].reshape((-1, 4))
        for i in range(aFlat.shape[0]):
            if numpy.allclose(aFlat[i], finalElLon):
                break
    ...
    def test_geocorners_lat_random_element(self):
        ...
        aFlat = self.geoarray['lat'].reshape((-1, 4))
        for i in range(aFlat.shape[0]):
            if numpy.allclose(aFlat[i], randElLat):
                break

        numpy.testing.assert_array_equal(self.geoarray['ind'].reshape((-1, 2))[i], (189,
                                                                                    2))
    ...
    def test_geocorners_lon_random_element(self):
        ...
        aFlat = self.geoarray['lon'].reshape((-1, 4))
        for i in range(aFlat.shape[0]):
            if numpy.allclose(aFlat[i], randElLon):
                break
...
class TestKnmiOmiL2GetGeoCenters(TestKnmiOmiL2Parser):
    ...
    def test_geocenters_lat_first_element(self):
        ...
        for i in range(almostFlat.shape[0]):
            if (almostFlat[i] == firstElInd).all():
                break
...
class TestNASAOmiL2Parser(TestParser):
    ...
    def setUp(self):
        ...
        self.parser = parse_geo.HDFnasaomil2_File(self.fname, subtype='knmiomil2', cornerDir=sample_dir, cornerFileList=[
        ...
        self.validKeys = ['ColumnAmountNO2', 'ColumnAmountNO2Std', 'ColumnAmountNO2Initial',
         ...
         'GroundPixelQualityFlags']
        self.validSizes = [98640, 591840, 98640, 1644, 98640,
         1, 1644, 98640, 98640]
        ...
        self.firstElValues = numpy.array(self.bldFlatList([Helpers.genNan(1), Helpers.genNan(6), -32.767,
         2, 2104, 255, 5.78275952674192 * 100000000,
         -79.71093, 94.31383]))
        self.nanElInd = (1545, 39)
        self.nanElValues = numpy.array(self.bldFlatList([Helpers.genNan(1), Helpers.genNan(6), -32.767, 2,
         2100, 255, 5.782790432626631 * 100000000, 71.152504, 88.35141]))
        self.lastElInd = (1643, 59)
        self.lastElValues = numpy.array(self.bldFlatList([Helpers.genNan(1), Helpers.genNan(6), -32.767,
         2, 2104, 255, 5.782792393000011 * 100000000, 61.2573, 102.92529]))
        self.valElInd = (520, 52)
        self.valElValues = numpy.array(self.bldFlatList([4.80269451 * 100000000000000,
         [6.431637 * 100000000000000000000, -9.0338365 * 10000000000000000000,
          8.2326397 * 10000000000000000, 7.3165924 * 1000000000000000000,
          8.2205615 * 1000000000000000000, -5.8147095 * 100000000000000000],
         0.125, 0, 8192, 255, 5.78276992872236 * 100000000, -15.731218, 46.450844]))
    ...
    def test_override_extension(self):
        dir = os.path.dirname(__file__)
        obj = parse_geo.HDFnasaomil2_File(self.fname, extension='foo', cornerDir=os.path.join(dir, 'sample_data'), cornerFileList=[
         'ominasacornersample.hdf'])
        self.assertEqual(obj.ext, 'foo')
    ...
    def test_get_retrives_valid_element(self):
        ...
        numpy.testing.assert_allclose(chkVals.flat, self.valElValues.flat, rtol=1e-05)
...
class TestNASAOmiL2GetGeoCorners(unittest.TestCase):
    ...
    def setUp(self):
        ...
        parser = parse_geo.HDFnasaomil2_File(self.fname, cornerDir=self.sample_dir, cornerFileList=[
         'ominasacornersample.hdf'])
    ...
    def test_corners_size(self):
        self.assertEqual(self.geoarray.size, 98640)
    ...
    def test_corners_lat_first_element(self):
        knownLat = [
         -79.5652, -80.0409, -79.8142, -79.3238]
    ...
    def test_corners_lon_first_element(self):
        knownLon = [
         172.91501, -179.07101, -178.61101, 173.598]
    ...
    def test_corners_lat_last_element(self):
        knownLat = [
         61.3584, 61.3896, 61.126198, 61.1233]
    ...
    def test_corners_lon_last_element(self):
        knownLon = [
         0.96595997, -1.84995, -1.81073, 0.97933]
    ...
    def test_corners_lat_rand_element(self):
        knownLat = [
         20.7784, 20.8055, 20.934, 20.906399]
    ...
    def test_corners_lon_rand_element(self):
        knownLon = [
         -165.727, -165.49, -165.51901, -165.75601]
    ...
    def test_raises_IO_if_no_corner_file_valid_dir(self):
        dir = os.path.dirname(__file__)
        newParser = parse_geo.HDFnasaomil2_File(self.fname, cornerDir=dir, cornerFileList=[
         ''])
...
class TestMOPPITl2Parser(TestParser):
    ...
    def setUp(self):
        ...
        self.validSizes = [200032, 9, 400064, 3600576, 200032]
        ...
        fElProf = self.bldFlatList([
         [82.91398, 80.92176, 75.33437, 73.38161, 74.29397,
        ...
        fElVals = [3.95020813831 * 100000000, presGrid, [84.239365, 29.985426], fElProf, 0]
        self.firstElVals = self.bldFlatArray(fElVals)
        self.firstElInd = (0, )
        lElProf = self.bldFlatList([
         [58.33968, 58.10826, 58.364857, 58.7423, 59.134647,
        ...
        lElVals = [3.95107203890999 * 100000000, presGrid, [58.138416, 20.491892], lElProf, 0]
        self.lastElVals = self.bldFlatArray(lElVals)
        self.lastElInd = (200031, )
        nElProf = self.bldFlatList([
         [numpy.nan, numpy.nan, numpy.nan, 61.714626, 62.36158,
        ...
        nElVals = [3.9510279793499965 * 100000000, presGrid, [62.32344, 20.00358], nElProf, 1]
        self.nanElVals = self.bldFlatArray(nElVals)
        self.nanElInd = (191000, )
        vElProf = self.bldFlatList([
         [79.70813, 78.30978, 74.31502, 73.186646, 74.27929,
        ...
        vElVals = [3.950208313689997 * 100000000, presGrid, [72.061165, 25.169048], vElProf, 0]
...
class TestOverallGridGeo(unittest.TestCase):
    ...
    def setUp(self):
        self.fakeParms = {'stdPar1': 45, 'stdPar2': 50, 
           'refLat': 47.5, 
           'refLon': -90, 
           'earthRadius': 6370000, 
           'xOrig': -10000, 
...
class Testlcc2par_GridDef(TestOverallGridGeo):
    ...
    def test_geo2Proj_vs_matlab(self):
        for (lat, lon), (x, y) in izip(self.knownGeoCoords, self.knownProjCoords):
            ...
            if y != 0:
                self.assertAlmostEqual(y, yCalc, delta=abs(y * 1e-06))
            else:
                self.assertAlmostEqual(y, yCalc, places=6)
            if x != 0:
                self.assertAlmostEqual(x, xCalc, delta=abs(x * 1e-06))
            else:
    ...
    def test_proj2Geo_vs_matlab(self):
        for (x, y), (lat, lon) in izip(self.knownProjCoords, self.knownGeoCoords):
            ...
            self.assertAlmostEqual(lat, latCalc, delta=abs(lat * 1e-06))
            self.assertAlmostEqual(lon, lonCalc, delta=abs(lon * 1e-06))
    ...
    def test_griddedToGeo_vs_matlab(self):
        for (row, col), (lat, lon) in izip(self.knownGridCoords, self.knownGeoCoords):
            ...
            lonCalc = utils.wrap_lon_0_360(lonCalc)
            lon = utils.wrap_lon_0_360(lon)
            self.assertAlmostEqual(lat, latCalc, delta=abs(lat * 1e-05))
            self.assertAlmostEqual(lon, lonCalc, delta=abs(lon * 1e-05))

    def test_geoToProj_vs_JPsnyder_example(self):
        JPparms = {'stdPar1': 33, 'stdPar2': 45, 
           'refLat': 23, 
           'refLon': -96, 
           'earthRadius': 1.0, 
           'xOrig': 0, 
        ...
        self.assertAlmostEqual(y, 0.2462112, places=7)
        self.assertAlmostEqual(x, 0.2966785, places=7)
...
class TestMapGeo(unittest.TestCase):
    ...
    def test_ValidMaps_are_valid(self):
        ...
        for name in maps:
            assert hasattr(map_geo, name + '_map_geo')
...
class Test_regional_intersect(TestMapGeo):
    ...
    def test_poly_olap_top_edge(self):
        ...
        lon = numpy.array([[0.25, 0.25, 0.75, 0.75]])
        ind = numpy.array([ [i] for i in range(lat.shape[0]) ])
        self.parser.prime_corners(lat, lon, ind)
        mapDict = self.mapper(self.parser, self.grid)
    ...
    def test_poly_on_corner_case_vertices_outside(self):
        lat = numpy.array([[0.25, 0.5, -0.25, -0.5]])
        lon = numpy.array([[-0.5, -0.25, 0.5, 0.25]])
        ind = numpy.array([ [i] for i in range(lat.shape[0]) ])
        self.parser.prime_corners(lat, lon, ind)
        mapDict = self.mapper(self.parser, self.grid)
        self.testMapDict[(0, 0)] = [((0,), None)]
...
class TestUtils(unittest.TestCase):
    ...
    def test_wrap_lon_0_360_known_inputs(self):
        knownIn = [
         0, 360, 180, 47.5, -47.5, -80, -180, -365, -725, 410]
    ...
    def test_timestr_to_nsecs_known_input_defaults(self):
        inputTstr = [
         '00:00:00 01-01-1970', '01:00:00 01-02-1970',
    ...
    def test_timestr_to_nsecs_accepts_format(self):
        formats = [
         '%H:%M:%S %m/%d/%Y', '%m %d %y %H %M %S ', '%I:%M:%S %p %b %d, %Y']
    ...
    def test_nsecs_to_timestr_accepts_epoch(self):
        epochs = [
         '00:00:00 08-15-2011', '00:00:00 08-15-2010', '12:15:15 08-15-2011']
    ...
    def test_nsecs_and_tstr_reversible(self):
        ...
        numpy.testing.assert_allclose(inputNsecs, outputNsecs, atol=0.5)

    def test_UTCoffset_from_lon_known_input(self):
        inLons = [
         0, -4.5, -10.5, 17.5, 95, -110]
        knownOut = [0, 0, -1, 1, 6, -7]
        knownOut = [ 3600 * out for out in knownOut ]
...
class TestOutGeo(unittest.TestCase):
    ...
    def test_ValidOutfuncs_are_valid(self):
        ...
        for func in funcs:
            assert hasattr(out_geo, func + '_out_func'), 'Function %s_out_func not in module' % func
...
class Test_OMNO2e_wght_avg_out_func(TestOutGeo):
    ...
    def setUp(self):
        ...
        gridParms = {'xOrig': 0, 'yOrig': 0, 'xCell': 0, 
           'yCell': 0, 'nRows': 1, 
           'nCols': 1}
        self.one_el_grid = grid_geo.latlon_GridDef(gridParms)
        self.mapDict = {'parser': self.parser}
        fieldnames = {'toAvg': 'toAvg', 'overallQualFlag': 'qualFlag', 
           'cloudFrac': 'cFrac', 
           'solarZenithAngle': 'solZenAng'}
        outfuncParms = {'cloudFractUpperCutoff': 0.25, 'solarZenAngUpperCutoff': 80, 
           'pixIndXtrackAxis': 1, 
           'fillVal': -99999}
    ...
    def test_zero_weight_if_mult_problems(self):
        self.cfrac[0, 28:33] = [0.4, 0.5, 0.1, 0.5, 0.1]
    ...
    def test_multi_element_pix_array(self):
        self.cfrac[0:2, 28:34] = [
         [
          0.11, 0.12, 0.13, 0.14, 0.15, 0.16],
         [
          0.21, 0.22, 23, 0.24, 0.25, 0.26]]
...
class Test_OMNO2e_netCDF_avg_out_func(TestOutGeo):
    ...
    def setUp(self):
        ...
        self.one_el_gridParms = {'xOrig': 0, 'yOrig': 0, 'xCell': 0, 
           'yCell': 0, 'nRows': 1, 
           'nCols': 1}
        self.one_el_grid = grid_geo.latlon_GridDef(self.one_el_gridParms)
        self.six_el_gridParms = {'xOrig': 0, 'yOrig': 0, 'xCell': 0, 
           'yCell': 0, 'nRows': 2, 
           'nCols': 3}
        self.six_el_grid = grid_geo.latlon_GridDef(self.six_el_gridParms)
        self.mapDict = {'parser': self.parser}
        self.fnames = {'overallQualFlag': 'qualFlag', 'cloudFrac': 'cfrac', 
           'solarZenithAngle': 'solZenAng', 
           'time': 'time', 
           'longitude': 'lon'}
        ...
        self.defParms = {'inFieldNames': ['test2D', 'test3D'], 'outFieldNames': [
                           'outTest2D', 'outTest3D'], 
           'outUnits': [
                      'Jigawatts', 'fathoms'], 
           'extraDimLabel': [
    ...
    def test_parser_still_in_empty_map_at_end(self):
        self.mapDict[(0, 0)] = []
        unused_result = self.defOutFunc(self.mapDict, self.one_el_grid, self.outFname, verbose=False, version=self.version)
        self.assertIs(self.mapDict['parser'], self.parser)
    ...
    def test_result_right_shape_emptyMap_2D(self):
        self.mapDict[(0, 0)] = []
        resDict = self.defOutFunc(self.mapDict, self.six_el_grid, self.outFname, verbose=False, version=self.version)
    ...
    def test_single_valid_pixel_2D(self):
        ...
        resDict = self.defOutFunc(self.mapDict, self.one_el_grid, self.outFname, verbose=False, version=self.version)
    ...
    def test_correct_pixel_count_single_valid_pixel(self):
        ...
        self.test2D[(0, 30)] = data
        self.mapDict[(0, 0)] = [((0, 30), None)]
        resDict = newOutFunc(self.mapDict, self.one_el_grid, self.outFname, verbose=False, version=self.version)
    ...
    def test_2_valid_pix_accurate_weight_2D(self):
        ...
        self.assertAlmostEqual(resDict['outTest2D'][(0, 0)], expected, delta=1e-07)
        return
    ...
    def test_zero_weight_if_mult_problems_2D(self):
        self.cfrac[0, 28:37] = [0.4, 0.5, 0.1, 0.5, 0.4, 0.5, 0.1, 0.5, 0.1]
    ...
    def test_multi_element_pix_array_correct_counts(self):
        self.defParms['includePixelCount'] = True
        newOutFunc = out_geo.OMNO2e_netCDF_avg_out_func(self.defParms)
        self.cfrac[0:2, 28:34] = [[0.11, 0.12, 0.13, 0.14, 0.15, 0.16],
         [
          0.21, 0.22, 23, 0.24, 0.25, 0.26]]
        self.qualFlag[0:2, 28:34] = [[0, 0, 0, 0, 0, 0],
         [
        ...
        resDict = newOutFunc(self.mapDict, self.six_el_grid, self.outFname, verbose=False, version=self.version)
    ...
    def test_multiple_dictionaries_yield_correct_count(self):
        ...
        secondMapDict[(0, 0)] = [((0, 30), None)]
        dictList = [self.mapDict, secondMapDict]
        resDict = newOutFunc(dictList, self.one_el_grid, self.outFname, verbose=False, version=self.version)
        self.assertEqual(resDict['ValidPixelCount'][(0, 0)], 1)
        return
    ...
    def test_multidict_one_zero_weight_2D(self):
        ...
        secondMapDict[(0, 0)] = [((0, 30), None)]
        dictList = [self.mapDict, secondMapDict]
        resDict = self.defOutFunc(dictList, self.one_el_grid, self.outFname, verbose=False, version=self.version)
        self.assertAlmostEqual(resDict['outTest2D'][(0, 0)], data[0])
        return
    ...
    def test_nonzero_weight_pix_in_multiple_cells_2D(self):
        self.cfrac[0, 29:32] = [0.5, 0.12, 1.0]
        self.qualFlag[0, 29:32] = [0, 0, 0]
    ...
    def test_output_file_is_netcdf(self):
        ...
        unused_result = self.defOutFunc(self.mapDict, self.six_el_grid, self.outFname, verbose=False, version=self.version)
    ...
    def test_output_file_opens_if_variables_share_extra_dim(self):
        ...
        unused_result = newOutFunc(self.mapDict, self.six_el_grid, self.outFname, verbose=False, version=self.version)
        self.fid = netCDF4.Dataset(self.outFname, 'r')
    ...
    def test_output_file_contains_input_file_list(self):
        ...
        unused_result = self.defOutFunc(dictList, self.one_el_grid, self.outFname, verbose=False, version=self.version)
        self.fid = netCDF4.Dataset(self.outFname, 'r')
    ...
    def test_output_file_contains_gridParms(self):
        ...
        unused_result = self.defOutFunc(self.mapDict, self.six_el_grid, self.outFname, verbose=False, version=self.version)
        self.fid = netCDF4.Dataset(self.outFname, 'r')
        outDict = {'xOrig': self.fid.xOrig, 'yOrig': self.fid.yOrig, 'xCell': self.fid.xCell, 
           'yCell': self.fid.yCell, 'nRows': self.fid.nRows, 
           'nCols': self.fid.nCols}
...
class Test_unweighted_filtered_MOPITT_avg_netCDF_out_func(TestOutGeo):
    ...
    def setUp(self):
        TestOutGeo.setUp(self)
        self.version = 'TEST VERSION'
        self.six_el_gridParms = {'xOrig': 0, 'yOrig': 0, 'xCell': 1, 'yCell': 1, 
           'nRows': 2, 'nCols': 3}
        self.sixElGr = grid_geo.latlon_GridDef(self.six_el_gridParms)
        self.mapDict = {'parser': self.parser, (0, 0): [], (1, 0): [], (0, 1): [], (1, 1): [], (0, 2): [], (1, 2): []}
        self.pDict = {'time': 'time', 'longitude': 'lon', 
           'inFieldNames': [
                          'twoDnorm', 'threeDnorm', 'twoDlog', 'threeDlog', 'fourDcol'], 
           'outFieldNames': [
                           'twoDnorm', 'threeDnorm', 'twoDlog', 'threeDlog', 'fourDcol'], 
           'outUnits': [
                      'foo', 'bar', 'baz', 'qux', 'spam'], 
           'logNormal': [
                       'False', 'False', 'True', 'True', 'False'], 
           'dimLabels': [[], ['layer'], [], ['layer'], ['layer', 'value']], 'dimSizes': [[], ['3'], [], ['3'], ['3', '2']], 'timeStart': '00:00:00_01-04-2012', 
        ...
        outFid, self.outFname = tempfile.mkstemp()
        os.close(outFid)
        self.defaultOutFunc = lambda : self.defaultOutClass(self.mapDict, self.sixElGr, self.outFname, verbose=False, version=self.version)
        ...
        fNames = [
         'time', 'lon', 'SZA', 'sType', 'twoDnorm', 'twoDlog', 'threeDnorm', 'threeDlog', 'fourDcol']
        for fn in fNames:
    ...
    def test_single_valid_pixel_4D_norm(self):
        self.mapDict[(1, 2)] = [((2, 3), None)]
        expected = self.fourDcol[(2, 3, Ellipsis)]
        resDict = self.defaultOutFunc()
        numpy.testing.assert_array_almost_equal(resDict['fourDcol'][(1, 2, Ellipsis)], expected)
        return
    ...
    def test_two_valid_pixel_4D_norm(self):
        self.mapDict[(1, 2)] = [((2, 3), None), ((2, 4), None)]
        self.fourDcol[(2, 3, Ellipsis)] = numpy.array([[2.2, 3.2], [4.4, 5.4], [7.6, 8.6]])
        self.fourDcol[(2, 4, Ellipsis)] = numpy.array([[2.4, 3.4], [4.6, 5.6], [7.8, 8.8]])
    ...
    def test_two_valid_pixel_4D_log(self):
        ...
        self.mapDict[(1, 2)] = [((2, 3), None), ((2, 4), None)]
        self.fourDcol[(2, 3, Ellipsis)] = numpy.array([[2, 4], [6, 8], [10, 12]])
        self.fourDcol[(2, 4, Ellipsis)] = numpy.array([[0.5, 1], [1.5, 2], [2.5, 3]])
        expected = [[1, 2], [3, 4], [5, 6]]
        resDict = newOutClass(self.mapDict, self.sixElGr, self.outFname, verbose=False, version=self.version)
        numpy.testing.assert_array_almost_equal(resDict['fourDcol'][(1, 2, Ellipsis)], expected)
        return
    ...
    def test_screen_properly_on_exactly_threshold_2D(self):
        ...
        self.twoDnorm[(3, 1)] = expected * 0.5
    ...
    def test_screen_out_UTC_times_earlier_than_start_4D(self):
        self.mapDict[(1, 2)] = [((2, 2), None), ((2, 3), None)]
        self.time[(2, 3)] = self.toTAI93('12:00:00 01-03-2012')
        expected = self.fourDcol[(2, 2, Ellipsis)]
        resDict = self.defaultOutFunc()
        numpy.testing.assert_array_almost_equal(resDict['fourDcol'][(1, 2, Ellipsis)], expected)
        return
    ...
    def test_time_and_filter_still_zero_out_pixel_4D(self):
        ...
        self.time[(0, 4)] = self.toTAI93('12:00:00 01-05-2012')
        expected = numpy.random.rand(3, 2)
        self.fourDcol[(0, 0, Ellipsis)] = expected + 5
        self.fourDcol[(0, 1, Ellipsis)] = expected - 2
        self.fourDcol[(0, 2, Ellipsis)] = expected - 2
        self.fourDcol[(0, 3, Ellipsis)] = expected - 1
        resDict = self.defaultOutFunc()
        numpy.testing.assert_array_almost_equal(resDict['fourDcol'][1, 2, :, :], expected)
        return
    ...
    def test_both_filter_still_zero_out_pixel_4D(self):
        ...
        self.fourDcol[(0, 4, 2, 0)] = numpy.NaN
        expected = numpy.random.rand(3, 2)
        self.fourDcol[(0, 0, Ellipsis)] = expected
        self.fourDcol[(0, 1, Ellipsis)] = expected - 3
        self.fourDcol[(0, 2, Ellipsis)] = expected + 2
        self.fourDcol[(0, 3, Ellipsis)] = expected + 1
        resDict = self.defaultOutFunc()
        numpy.testing.assert_array_almost_equal(resDict['fourDcol'][(1, 2, Ellipsis)], expected)
    ...
    def test_multidict_raises_appropriate_exception(self):
        ...
        self.assertRaises(NotImplementedError, self.defaultOutClass, dictList, self.sixElGr, self.outFname, verbose=False)
    ...
    def test_output_file_has_correct_variables(self):
        ...
        newOutClass = out_geo.unweighted_filtered_MOPITT_avg_netCDF_out_func(self.pDict)
        unused_result = newOutClass(self.mapDict, self.sixElGr, self.outFname, verbose=False, version=self.version)
        self.fid = netCDF4.Dataset(self.outFname, 'r')
    ...
    def test_output_correctly_writes_valid_values_4D(self):
        self.mapDict[(1, 2)] = [((2, 2), None), ((2, 3), None)]
        expected = numpy.random.rand(3, 2)
        self.fourDcol[(2, 2, Ellipsis)] = expected + 1
        self.fourDcol[(2, 3, Ellipsis)] = expected - 1
        unused_result = self.defaultOutFunc()
        self.fid = netCDF4.Dataset(self.outFname, 'r')
        numpy.testing.assert_array_almost_equal(self.fid.variables['fourDcol'][(1,
                                                                                2,
                                                                                Ellipsis)], expected)
        return
