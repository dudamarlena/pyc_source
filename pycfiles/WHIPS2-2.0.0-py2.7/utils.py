...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/process_sat/utils.py
# Compiled at: 2012-08-02 16:23:57
"""
...
import numpy, netCDF4, filetypes
...
def timestr_to_nsecs(timestr, epoch='00:00:00 01-01-1970', format='%H:%M:%S %m-%d-%Y'):
    """
...
def nsecs_to_timestr(nSecsSinceEpoch, epoch='00:00:00 01-01-1970', format='%H:%M:%S %m-%d-%Y'):
    """
...
def write_grid_to_netcdf(griddef, outFname):
    ...
    cols, rows = numpy.meshgrid(numpy.arange(minCol, maxCol + 1), numpy.arange(minRow, maxRow + 1))
    cols = cols.astype(numpy.float32)
...
def parse_fromFile_input_file(inFileName, dryRun):
            ...
            if s == 'BEGIN\n':
                while s != '':
                    ...
                    if len(s) > 1 and dryRun:
                        print 'Warning: Found line containing stray quotation marks... stripping.'
                    words = ('').join(s).split()
                    if words == []:
                        continue
                    elif words[1] != '=':
                        ...
                        attrs += [
                         ('"{0}:{1}"').format(words[0], (' ').join(words[2:]))]
                    else:
                        attrs += [
                         ('{0}:{1}').format(words[0], (' ').join(words[2:]))]
    ...
    if s != '':
        raise SyntaxError(("Invalid input file.  File must be formatted correctly.\nCheck line '{0}' and try again").format(s))
    raise SyntaxError('Invalid input file.  Check that the file matches the format described in the documentation and try again')
...
def parse_filetype(namespace):
    ...
    for attr in dir(filetype):
        if attr == 'parser':
            ...
            try:
                wng += ('Warning: Value {0} supplied for attribute {1} has been ignored. Filetype {2} does not support custom values for this parameter.\n').format(getattr(namespace, attr), attr, printfiletype)
