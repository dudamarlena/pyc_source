...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/process_sat/out_geo.py
# Compiled at: 2014-11-11 08:57:45
"""
...
import datetime, warnings, pdb, numpy, netCDF4, utils
...
tai93conv = lambda timestring: utils.timestr_to_nsecs(timestring, '00:00:00_01-01-1993', '%H:%M:%S_%m-%d-%Y')
...
class OMNO2e_wght_avg_BORKED(out_func):
    ...
    @staticmethod
    def required_parms():
        return {'toAvg': ('The name of the field to be averaged', None), 'overallQualFlag': ('The name of the field containing the overall quality flag for the pixels.  This flag should be true (1) for invalid pixels and false (0) for valid pixels.\n{ OMI KNMI - TroposphericColumnFlag\n  OMI NASA - vcdQualityFlags }',
 None), 
           'cloudFrac': ('The name of the field containing the cloud fractions.\n{ OMI KNMI - CloudFraction\n  OMI NASA - CloudFraction }',
 None), 
           'solarZenithAngle': ('The name of the field containing the solar zenith angles in degrees.\n{ OMI KNMI - SolarZenithAngle\n  OMI NASA - SolarZenithAngle }',
 None), 
 ...
 'decimal'), 
           'pixIndXtrackAxis': ('The dimension order (0 based) of the of the "cross-track" dimension (whichever dimension has size 60).  For all currently known cases should be 1  (may change in future versions of OMI products).',
 'int'), 
 ...
 'decimal')}
    ...
    def __call__(self, maps, griddef, outfilename, verbose, version):
        ...
        sys.exit(BORKED)
        castDict = {'toAvg': str, 'overallQualFlag': str, 'cloudFrac': str, 
           'solarZenithAngle': str, 'cloudFractUpperCutoff': float, 
           'solarZenAngUpperCutoff': float, 
           'pixIndXtrackAxis': int, 
           'fillVal': float}
        ...
        for map in maps:
            with map.pop('parser') as (p):
                if verbose:
                    print ('Processing {0} for output at {1}.').format(p.name, str(datetime.datetime.now()))
                for k, v in map.iteritems():
                    sumFlag = numpy.array([ p.get_cm(self.parmDict['overallQualFlag'], pxind) for pxind, unused_weight in v
                                          ])
                    sumFlag = numpy.mod(sumFlag, 2)
                    cFrac = numpy.array([ p.get_cm(self.parmDict['cloudFrac'], pxind) for pxind, unused_weight in v
                                        ])
                    cFracFlag = cFrac > self.parmDict['cloudFractUpperCutoff']
                    solZen = numpy.array([ p.get_cm(self.parmDict['solarZenithAngle'], pxind) for pxind, unused_weight in v
                                         ])
                    ...
                    fov = numpy.array([ pxind[self.parmDict['pixIndXtrackAxis']] for pxind, unused_weight in v
                                      ])
                    toAvg = numpy.array([ p.get_cm(self.parmDict['toAvg'], pxind) for pxind, unused_weight in v
                                        ])
                    weights = 0
        ...
        avgs = numpy.where(sum_weights != 0, numpy.divide(sum_weighted_vals, sum_weights), self.parmDict['fillVal'])
...
class OMNO2e_netCDF_avg_out_func(out_func):
    ...
    @staticmethod
    def required_parms():
        return {'overallQualFlag': ('The name of the field containing the overall quality flag for the pixels.  This flag should be true (1) for invalid pixels and false (0) for valid pixels.\n{ OMI KNMI - TroposphericColumnFlag\n  OMI NASA - vcdQualityFlags }',
 None), 
           'cloudFrac': ('The name of the field containing the cloud fractions\n{ OMI KNMI - CloudFraction\n  OMI NASA - CloudFraction }',
 None), 
           'solarZenithAngle': ('The name of the field containing the solar zenith angles in degrees.\n{ OMI KNMI - SolarZenithAngle\n  OMI NASA - SolarZenithAngle }',
 None), 
           'time': ('The name of the field containing the timestamps.  Timestamps are assumed to be the in TAI-93 format.\n{ OMI KNMI - Time\n  OMI NASA - TIME }',
 None), 
           'longitude': ('The name of the field containing longitudes at cell centers.  Longitudes should be in degrees east.\n{ OMI KNMI - Longitude\n  OMI NASA - Longitude }',
 None), 
           'inFieldNames': ('The names of the fields desired to be output.  Input as comma-delimited list ', 'list'), 
 ...
 'list'), 
           'extraDimLabel': ('The label for the extra dimension (should the variable have an extra dimension).  Ignored in the case of a 2D variable.  Should be a comma-delimited list co-indexed to inFieldNames',
 'list'), 
 ...
 None), 
           'timeStart': ('The earliest time for which data should be recorded into the output file.  All times in input files before this time will be filtered out.  Must be in the format hh:mm:ss_MM-DD-YYYY',
 'time'), 
           'timeStop': ('The latest time for which data should be recorded into the output files.  All times in input files after this time will be filtered out.  Must be in the format hh:mm:ss_MM-DD-YYYY',
 'time'), 
           'cloudFractUpperCutoff': ('The maximum cloud fraction to allow before excluding pixel from average.  Suggested value from NASA is 0.3',
 'decimal'), 
           'solarZenAngUpperCutoff': ('The maximum solar zenith angle to allow before excluding pixel from average, in degrees.  Suggested value from NASA is 85.',
 'int'), 
 ...
 'int'), 
           'fillVal': ('The value to use as a fill value in the output netCDF file.  This value will replace any missing or invalid output values',
 'decimal'), 
           'includePixelCount': ('If set to true, the output will include a field "ValidPixelCount" that contains the number of valid pixels in each grid cell.  Only pixels with nonzero weight  are considered valid.',
 'bool')}
    ...
    def __call__(self, maps, griddef, outfilename, verbose, version):
        ...
        for i in range(len(dimsizes)):
            try:
                ...
                print ('Warning: {0} is not a valid extraDimSize value.  Using 0 instead').format(dimsizes[i])
                dimsizes[i] = 0
                continue

        self.parmDict['extraDimSize'] = dimsizes
        castDict = {'overallQualFlag': str, 'cloudFrac': str, 'solarZenithAngle': str, 
           'time': str, 'longitude': str, 
           'inFieldNames': list, 'outFieldNames': list, 
           'outUnits': list, 'extraDimLabel': list, 
           'extraDimSize': list, 'timeComparison': str, 
           'timeStart': tai93conv, 'timeStop': tai93conv, 
           'cloudFractUpperCutoff': float, 'solarZenAngUpperCutoff': int, 
        ...
        if self.parmDict['timeStart'] > self.parmDict['timeStop']:
            ...
            raise IOError(msg)
        if len(self.parmDict['inFieldNames']) != len(self.parmDict['outFieldNames']) or len(self.parmDict['inFieldNames']) != len(self.parmDict['outUnits']) or len(self.parmDict['inFieldNames']) != len(self.parmDict['extraDimLabel']):
            msg = 'All field/unit inputs ' + 'should have the same number of elements.'
            raise IOError(msg)
        minRow, maxRow, minCol, maxCol = griddef.indLims()
        nRows = maxRow - minRow + 1
        nCols = maxCol - minCol + 1
        nValidPixels = numpy.zeros((nRows, nCols))
        sumWght = numpy.zeros((nRows, nCols, 1))
        sumVars = dict()
        ...
        if not isinstance(maps, list):
            maps = [
             maps]
        for map in maps:
            with map.pop('parser') as (parser):
                if verbose:
                    print ('Processing {0} for output at {1}.').format(parser.name, str(datetime.datetime.now()))
                    ...
                    for pxInd, unused_weight in pixTup:
                        ...
                        for field in self.parmDict['inFieldNames']:
                            ...
                            if weightVals.size > 1:
                                sumVars[field][gridInd] = numpy.nansum([sumVars[field][gridInd],
                                 weightVals], axis=0)
                            else:
                                sumVars[field][gridInd] = numpy.nansum([sumVars[field][gridInd][0],
                                 weightVals])
        ...
        if self.parmDict['includePixelCount']:
            varDims = ('row', 'col')
            varHandle = outFid.createVariable('ValidPixelCount', 'i', varDims, fill_value=self.parmDict['fillVal'])
...
class wght_avg_netCDF(out_func):
    ...
    def __init__(self, parmDict=None):
        ...
        lists = [
         'outFieldNames', 'outUnits', 'dimLabels', 'dimSizes', 'logNormal']
        ...
        if not all(isCorrectLength):
            ...
            msg = 'All lists must be the same length.  The following list lengths do not match the length of inFieldNames: ' + (' ').join(wrongLength)
        ...
        if not all(tupsMatch):
            ...
            msg = 'All tuple-like strings must correspond to tuples of corresponding size. The following sets do not correspond: \n' + ('\n').join(misMatch)
            raise ValueError(msg)
        try:
            ...
            msg = 'The value %s in the dimSizes argument was not castable to an integer.' % uncastable
            raise ValueError(msg)
    ...
    def __call__(self, maps, griddef, outfilename, verbose, version):
        ...
        for field in self.parmDict['inFieldNames']:
            dims = [
             nRows, nCols] + self.parmDict['dimSizes'][field]
        ...
        if not isinstance(maps, list):
            maps = [
             maps]
        if len(maps) != 1:
            msg = 'Though in general output functions are designed to handle multiple input files, this function currently can only process individual input files.  Please only provide one input file or use a different output function.  This limitation may be fixed if there is a convincing reason to rewrite the function to accomodate more inputs'
        ...
        for map in maps:
            with map.pop('parser') as (p):
                ...
                for cellInd, pixTups in map.iteritems():
                    wghts = [ wghtDict.setdefault(ind, wghtFunc(p, ind, wgt)) for ind, wgt in pixTups
                            ]
                    tArray = numpy.array([ p.get_cm(self.parmDict['time'], ind) for ind, wgt in pixTups
                                         ]).squeeze()
                    if self.parmDict['timeComparison'] == 'local':
                        offsets = numpy.array([ utils.UTCoffset_from_lon(p.get_cm(self.parmDict['longitude'], ind)) for ind, wgt in pixTups
                                              ]).squeeze()
                        tArray += offsets
                    ...
                    for field in self.parmDict['inFieldNames']:
                        vals = numpy.array([ p.get_cm(field, ind) for ind, wgt in pixTups
                                           ]).squeeze()
                        ...
                        wghtSlice = [
                         Ellipsis] + [numpy.newaxis] * nExtraDims
                        if vals.size == 0:
                        ...
                        wghtValAvg = numpy.where(numpy.isnan(wghtValAvg), self.parmDict['fillVal'], wghtValAvg)
        ...
        outFid.createDimension('col', nCols)
        setattr(outFid, 'Version', vsnmsg(version))
        setattr(outFid, 'File_start_time', utils.nsecs_to_timestr(self.parmDict['timeStart'], epoch='00:00:00 01-01-1993', format='%H:%M:%S %m-%d-%Y'))
        setattr(outFid, 'File_stop_time', utils.nsecs_to_timestr(self.parmDict['timeStop'], epoch='00:00:00 01-01-1993', format='%H:%M:%S %m-%d-%Y'))
        setattr(outFid, 'Time_comparison_scheme', self.parmDict['timeComparison'])
...
class unweighted_filtered_MOPITT_avg_netCDF_out_func(wght_avg_netCDF):
    ...
    @staticmethod
    def required_parms():
        return {'time': ('The name of the field containing timestamps.  Timestamps are assumed to be in the TAI-93 format.\n{ MOPITT - TIME }',
 None), 
           'longitude': ('The name of the field containing longitudes at cell centers.  Longitudes should be in degrees east.\n{ MOPITT - Longitude }',
 None), 
           'inFieldNames': ('The names of the fields desired to be output.  Input as comma-delimited list.', 'list'), 
 ...
 'list'), 
           'outUnits': ('The units of the variables to be written out.  Should be a comma-delimited list co-indexed to inFieldNames',
 'list'), 
           'logNormal': ('List of boolean strings that specify how to take the averages of the corresponding fields.  If the string is "True" that field is averaged assuming a lognormal distribution.  If the string is "False" that field is averaged assuming a normal distribution.  Should be a comma-delimited list co-indexed to inFieldNames',
 'list'), 
           'dimLabels': ('List of names of the extra dimensions in the output file.  Must be a semicolon-delimited list of comma-delimited strings. Fields with noextra dimensions may be left blank.  For example, if there are four inFields, the first and third of which have no extra dimensions, the second of which has one ("foo"), and the fourth has two ("foo" and "bar"), the dimLabels entry should look like this: ;foo;;foo,bar  The outer (semicolon-delimited) list must be co-indexed to inFieldNames',
 'listoflists'), 
           'dimSizes': ('List of the sizes of the extra dimensions in the output file.  Must be a semicolon-delimited list of comma-delimited lists of integers.  Fieldswith no extra dimensions may be left blank.  For example, if there are four inFields, the first and third of which have no extra dimensions, the second of which has one (which has length four), and the fourth has two (which have lengths four and five, respectively), the dimSizes entry should look like this: ;4;;4,5 The outer (semicolon-delimited) list must be co-indexed to inFieldNames and all sub-lists should be the same size as the corresponding sublist in dimLabels.',
 'listoflists'), 
           'timeStart': ('The earliest time for which data should be recorded into the output file.  All times before this time in the input file(s) will be filtered out.  Must be in the format:  hh:mm:ss_MM-DD-YYYY',
 'time'), 
           'timeStop': ('The latest time for which data should be recorded into the output file.  All times after this time in the input file(s) will be filtered out.  Must be in the format: hh:mm:ss_MM-DD-YYYY',
 'time'), 
           'timeComparison': ('Must be set to either "local" or "UTC".  Determines how the file timestamps are compared to the start/stop time.  If set to "local", then the file timestamps are converted to local time on a pixel-by-pixel basis (using longitude to estimate time zone) before being compared to time boundaries.  If set to "UTC" the file timestamps (which are assumed to be in UTC) are compared against the start/stop time directly.',
 None), 
           'fillVal': ('The value to use as a fill value in the output netCDF file.  This value will replace any missing or invalid output values',
 'decimal'), 
           'solZenAngCutoff': ('The solar zenith angle that defines the day to night transition (we use the SZA to separate day and night pixels, which should not be averaged together), in degrees.  The geometric value here would be 90.  Recommended value is 85.',
 'decimal'), 
 ...
 'bool'), 
           'surfTypeField': ('The name of the field containing the surface type index.\n{ MOPITT - Surface Index }',
 None), 
 ...
 None)}
    ...
    def __init__(self, pDict):
        ...
        parmDict = dict(pDict)
        castDict = {'time': str, 'longitude': str, 'inFieldNames': list, 
           'outFieldNames': list, 'outUnits': list, 
           'logNormal': list, 'dimLabels': list, 
           'dimSizes': list, 'timeStart': tai93conv, 
           'timeStop': tai93conv, 'timeComparison': str, 
           'fillVal': float, 'solZenAngCutoff': float, 
           'solZenAng': str, 'dayTime': bool, 
           'surfTypeField': str, 'colMeasField': str}
        for k, func in castDict.items():
            try:
...
class MODIS_simp_avg_netCDF_out_func(wght_avg_netCDF):
    ...
    def parm_list():
        return [
         'timeStart', 'timeStop', 'timeComparison', 'time', 'fillVal', 'inFieldNames', 'outUnits', 'outFieldNames', 'extraDimSize', 'extraDimLabels']

    @staticmethod
    def required_parms():
        return {'timeStart': ('The earliest time for which data should be recorded into the output file.  All times before this time in the input file(s) will be filtered out.  Must be in the format: hh:mm:ss_MM-DD-YYYY',
 'time'), 
           'timeStop': ('The latest time for which data should be recorded into the output file.  All times after this time in the input file(s) will be filtered out.   Must be in the format: hh:mm:ss_MM-DD-YYYY',
 'time'), 
           'timeComparison': ('Must be set to either "local" or "UTC".  Determines how the file timestamps are compared to the start/stop time.  If set to "local", then the file timestamps are converted to local time on a pixel-by-pixel basis (using longitude to estimate time zone) before being compared to time boundaries.  If set to "UTC" the file timestamps (which are assumed to be in UTC) are compared against the start/stop time directly.',
 ...
 'listoflists'), 
           'extraDimLabels': ('List of names of the extra dimensions in the output file.  Must be a comma-delimited list of strings.  Fields with no extra dimensions should be left blank.  For example, if there are four inFields, the first and third of which have no extra dimensions, the second of which has one ("foo"), and the fourth has one ("bar"), the extraDimLabels entry should look like this: ,foo,,bar The list must be co-indexed to inFieldNames',
 'listoflists'), 
           'includePixelCount': ('If set to true, the output will include a field "ValidPixelCount" that contains the number of valid pixels in each grid cell.  Only pixels with nonzero weight  are considered valid.',
 'bool')}
    ...
    def __init__(self, pDict):
        ...
        castDict = {'time': str, 'inFieldNames': list, 'outUnits': list, 
           'outFieldNames': list, 'extraDimLabels': list, 
           'extraDimSize': list, 'timeStart': tai93conv, 
           'timeStop': tai93conv, 'timeComparison': str, 
    ...
    def __call__(self, maps, griddef, outfilename, verbose, version):
        ...
        if self.parmDict['timeStart'] > self.parmDict['timeStop']:
            msg = 'Input start time must come before stop time.'
            raise IOError(msg)
        if len(self.parmDict['inFieldNames']) != len(self.parmDict['outFieldNames']) or len(self.parmDict['inFieldNames']) != len(self.parmDict['outUnits']) or len(self.parmDict['inFieldNames']) != len(self.parmDict['extraDimLabels']):
            msg = 'All field/unit inputs ' + 'should have the same number of elements.'
            raise IOError(msg)
        minRow, maxRow, minCol, maxCol = griddef.indLims()
        nRows = maxRow - minRow + 1
        ...
        for map in maps:
            ...
            with map.pop('parser') as (parser):
                if verbose:
                    print ('\nProcessing {0} for output at {1}.').format(parser.name, str(datetime.datetime.now()))
                ...
                for gridCell, pixTup in map.iteritems():
                    ...
                    for pxInd, unused_weight in pixTup:
                        ...
                        try:
                            ...
                            if all(numpy.isnan(el).all() for el in rawDataDict.values()):
                                raise invalidPixCeption
                        except invalidPixCeption:
                        ...
                        pixSum = pixSum + 1
                        if verbose:
                            sys.stdout.write(('\rData added from {0} pixels. NaNs found for {1} pixels.').format(pixSum, totofNaN))
                            sys.stdout.flush()
                        sumWght[gridInd] += 1
                        nValidPixels[gridInd] += 1
                        for field in self.parmDict['inFieldNames']:
        ...
        numpy.seterr(divide=oldSettings['divide'])
        if verbose:
            print ('\nWriting final output to file at {0}.').format(str(datetime.datetime.now()))
        outFnames = dict(izip(self.parmDict['inFieldNames'], self.parmDict['outFieldNames']))
        units = dict(izip(self.parmDict['inFieldNames'], self.parmDict['outUnits']))
        ...
        if self.parmDict['includePixelCount']:
            ...
            varHandle = outFid.createVariable('ValidPixelCount', 'i', varDims, fill_value=self.parmDict['fillVal'])
            varHandle[:] = nValidPixels
            varHandle = outFid.createVariable('NaNPixelCount', 'i', varDims, fill_value=self.parmDict['fillVal'])
...
class OMIHCHO_netCDF_avg_out_func(out_func):
    ...
    def required_parms():
        return {'overallQualFlag': ('The name of the field containing the overall quality flag for the pixels.  This flag should be true (1) for invalid pixels and false (0) for valid pixels.\n{ OMI KNMI - TroposphericColumnFlag\n  OMI NASA - vcdQualityFlags }',
 None), 
           'XtrackQualityFlags': ('The name of field containing the rowanomaly Flags', None), 
           'cloudFrac': ('The name of the field containing the cloud fractions\n{ OMI KNMI - CloudFraction\n  OMI NASA - CloudFraction }',
 None), 
           'solarZenithAngle': ('The name of the field containing the solar zenith angles in degrees.\n{ OMI KNMI - SolarZenithAngle\n  OMI NASA - SolarZenithAngle }',
 None), 
 ...
 'bool')}
    ...
    def __call__(self, maps, griddef, outfilename, verbose, version):
        ...
        self.parmDict['extraDimSize'] = dimsizes
        castDict = {'overallQualFlag': str, 'XtrackQualityFlags': str, 'cloudFrac': str, 'solarZenithAngle': str, 
           'time': str, 'longitude': str, 
           'inFieldNames': list, 'outFieldNames': list, 
           'outUnits': list, 'extraDimLabel': list, 
           'extraDimSize': list, 'timeComparison': str, 
        ...
        for map in maps:
            with map.pop('parser') as (parser):
                ...
                for gridCell, pixTup in map.iteritems():
                    ...
                    for pxInd, unused_weight in pixTup:
                        sumFlag = parser.get_cm(self.parmDict['overallQualFlag'], pxInd)
                        xtrackFlag = parser.get_cm(self.parmDict['XtrackQualityFlags'], pxInd)
                        cFrac = parser.get_cm(self.parmDict['cloudFrac'], pxInd)
                        if sumFlag != 0:
                            continue
                        if xtrackFlag != 0:
...
class OMNO2e_netCDF_avg_v3_out_func(out_func):
    """
    Weighted average for a given set of filtered values
    based on OMNO2e algorithm.
    ...
    @staticmethod
    def parm_list():
        return ['overallQualFlag', 'cloudFrac', 'XTrackQualityFlags', 'solarZenithAngle',
         'time', 'longitude', 'inFieldNames', 'outFieldNames',
         'outUnits', 'extraDimLabel', 'extraDimSize',
         'timeComparison', 'timeStart', 'timeStop',
         'cloudFractUpperCutoff', 'solarZenAngUpperCutoff',
