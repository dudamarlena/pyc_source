...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/process_sat/grid_geo.py
# Compiled at: 2012-02-29 11:03:16
"""
...
class latlon_GridDef(GridDef):
    ...
    def requiredParms():
        ...
        return {'xOrig': ('The longitude of the lower-left corner of the domain', 'decimal'), 
           'yOrig': ('The latitude of the lower-left corner of the domain', 'decimal'), 
           'xCell': ('The size of a gridcell in the x (longitude) direction in degrees', 'posdecimal'), 
           'yCell': ('The size of a gridcell in the y (latitude) direction in degrees', 'posdecimal'), 
    ...
    def __init__(self, parmDict):
        ...
        castDict = {'xOrig': float, 'yOrig': float, 'xCell': float, 
           'yCell': float, 'nRows': int, 
           'nCols': int}
...
class lcc2par_GridDef(GridDef):
    ...
    def requiredParms():
        ...
        """
        return {'stdPar1': ('One of the 2 standard parallels (latitudes) used to define the Lambert Conic Conformal projection, in degrees',
 'decimal'), 
           'stdPar2': ('The second of the standard parallels (latitudes) used to define the Lambert Conic Conformal projection, in degrees.  Set this equal to stdPar1 if the single-parallel form of the projection is being used',
 'decimal'), 
           'refLat': ('The reference latitude upon which the projection is centered, in degrees.  This is the YCENT value in the GRIDDESC file',
 'decimal'), 
           'refLon': ('The reference longitude upon which the projection is centered, in degrees.  This is BOTH the XCENT and PROJ_GAMMA values in the GRIDDESC file.  If these values are not identical, do not use this function',
 'decimal'), 
           'xOrig': ('The location of the origin in projected x coordinates, in the same units as earthRadius.  This is the XORIG value in the GRIDDESC file',
 'decimal'), 
           'yOrig': ('The location of the origin in projected y coordinates, in the same units as earthRadius.  This is the YORIG value in the GRIDDESC file',
 'decimal'), 
 ...
 'posdecimal')}
    ...
    def __init__(self, parms):
        ...
        castDict = {'stdPar1': float, 'stdPar2': float, 'refLat': float, 
           'refLon': float, 'xOrig': float, 
           'yOrig': float, 'xCell': float, 
           'yCell': float, 'nRows': int, 
           'nCols': int}
        for k, func in castDict.items():
        ...
        self.parms = parms
        pyprojParms = {'proj': 'lcc', 'a': parms['earthRadius'], 
           'lat_1': parms['stdPar1'], 
           'lat_2': parms['stdPar2'], 
           'lat_0': parms['refLat'], 
           'lon_0': parms['refLon'], 
