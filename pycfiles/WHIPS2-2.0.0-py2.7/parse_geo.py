...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/process_sat/parse_geo.py
# Compiled at: 2014-11-13 15:34:40
"""
...
"""
import os, sys, string, pdb, tables, numpy, pyhdf.HDF, pyhdf.V, pyhdf.VS, pyhdf.SD, filetypes
    ...
    def get_cm(self, key, indices=None, missingValue=None):
        ...
        if key not in self._open_vars.keys():
            ...
            if missingValue and self._open_vars[key].dtype in ('float32', 'float64'):
                self._open_vars[key] = numpy.where(self._open_vars[key] == missingValue, numpy.NaN, self._open_vars[key])
        if indices is not None:
            indFunc = self._indexMap.get(key, self._indexMap['default'])
...
class HDFFile(GeoFile):
    ...
    def __init__(self, filename, subtype='', extension=None):
        GeoFile.__init__(self, filename, subtype=subtype, extension=extension)
        if tables.isHDF5File(self.name):
            pass
        else:
    ...
    def get_cm(self, key, indices=None):
        ...
        if self._scales[key] != 1 or self._offsets[key] != 0:
            return indFunc(self._open_vars[key], indices) * self._scales[key] + self._offsets[key]
        else:
            return indFunc(self._open_vars[key], indices)
            return
...
class HDFknmiomil2_File(HDFFile):
    ...
    _nameExpMap = {'AirMassFactor': '/HDFEOS/SWATHS/DominoNO2/Data Fields/AirMassFactor', 'AirMassFactorGeometric': '/HDFEOS/SWATHS/DominoNO2/Data Fields/AirMassFactorGeometric', 
...
class HDFnasaomil2_File(HDFFile):
    ...
    framework.
    """
    OMIAURANO2_FILE_NAME = 'OMI/Aura Nitrogen Dioxide (NO2) Total & Troposph.Column 1-Orbit L2 Swath 13x24km'
    OMIAURANO2_CORNER_FILE_NAME = 'OMI/Aura Global Ground Pixel Corners 1-Orbit L2 Swath 13x24km'

    def __init__(self, filename, subtype='', extension=None, cornerDir=None, cornerFileList=None):
        ...
        if os.path.isdir(cornerDir):
            ...
            if self.pixCorners == None:
                ...
                for f in allPossible:
                    try:
                        if tables.isHDF5File(f) and getLongName(f) == HDFnasaomil2_File.OMIAURANO2_CORNER_FILE_NAME and getOrbitNumber(f) == forbitnumber:
                            self.pixCorners = f
                            break
                    except:
                        pass
    ...
    _nameExpMap = {'AMFInitial': __dataPath + 'AMFInitial', 'AMFInitialClear': __dataPath + 'AMFInitialClear', 
...
class HDFmopittl2_File(HDF4File):
    ...
    _nameExpMap = {'Time': __GeoPath + 'Time', 'Latitude': __GeoPath + 'Latitude', 
       'Longitude': __GeoPath + 'Longitude', 
    ...
    _indexMap = {'default': lambda var, ind: var[(ind[0], ...)], 'Pressure Grid': lambda var, ind: var[:]}
...
class HDFmodisl2_File(HDF4File):
    ...
    _nameExpMap = {'Latitude': __GeoPath + 'Latitude', 'Longitude': __GeoPath + 'Longitude', 
       ...
       'Optical_Depth_Land_And_Ocean': __DataPath + 'Optical_Depth_Land_And_Ocean', 
       'Image_Optical_Depth_Land_And_Ocean': __DataPath + 'Image_Optical_Depth_Land_And_Ocean', 
       'Optical_Depth_Ratio_Small_Land_And_Ocean': __DataPath + 'Optical_Depth_Ratio_Small_Land_And_Ocean', 
       'Mean_Reflectance_Land_All': __DataPath + 'Mean_Reflectance_Land_All', 
       'Standard_Deviation_Reflectance_Land_All': __DataPath + 'Standard_Deviation_Reflectance_Land_All', 
       'Path_Radiance_Land': __DataPath + 'Path_Radiance_Land', 
       'Error_Path_Radiance_Land': __DataPath + 'Error_Path_Radiance_Land', 
       'Critical_Reflectance_Land': __DataPath + 'Critical_Reflectance_Land', 
       'Error_Critical_Reflectance_Land': __DataPath + 'Error_Critical_Reflectance_Land', 
       'QualityWeight_Path_Radiance_Land': __DataPath + 'QualityWeight_Path_Radiance_Land', 
       'QualityWeight_Critical_Reflectance_Land': __DataPath + 'QualityWeight_Critical_Reflectance_Land', 
       'Aerosol_Type_Land': __DataPath + 'Aerosol_Type_Land', 
       'Fitting_Error_Land': __DataPath + 'Fitting_Error_Land', 
       'Surface_Reflectance_Land': __DataPath + 'Surface_Reflectance_Land', 
       'Corrected_Optical_Depth_Land': __DataPath + 'Corrected_Optical_Depth_Land', 
       'Corrected_Optical_Depth_Land_wav2p1': __DataPath + 'Corrected_Optical_Depth_Land_wav2p1', 
       'Optical_Depth_Small_Land': __DataPath + 'Optical_Depth_Small_Land', 
       'Optical_Depth_Ratio_Small_Land': __DataPath + 'Optical_Depth_Ratio_Small_Land', 
       'Number_Pixels_Used_Land': __DataPath + 'Number_Pixels_Used_Land', 
       'Mean_Reflectance_Land': __DataPath + 'Mean_Reflectance_Land', 
       'STD_Reflectance_Land': __DataPath + 'STD_Reflectance_Land', 
       'Mass_Concentration_Land': __DataPath + 'Mass_Concentration_Land', 
       'Angstrom_Exponent_Land': __DataPath + 'Anstrom_Exponent_Land', 
       'Cloud_Fraction_Land': __DataPath + 'Cloud_Fraction_Land', 
       'Quality_Assurance_Land': __DataPath + 'Quality_Assurance_Land', 
       'Quality_Assurance_Crit_Ref_Land': __DataPath + 'Quality_Assurance_Crit_Ref_Land', 
       'Aerosol_Cldmask_Byproducts_Land': __DataPath + 'Aerosol_Cldmask_Byproducts_Land', 
       'Solution_Index_Ocean_Small': __DataPath + 'Solution_Index_Ocean_Small', 
       'Solution_Index_Ocean_Large': __DataPath + 'Solution_Index_Ocean_Large', 
       'Effective_Optical_Depth_Best_Ocean': __DataPath + 'Effective_Optical_Depth_Best_Ocean', 
       'Effective_Optical_Depth_Average_Ocean': __DataPath + 'Effective_Optical_Depth_Average_Ocean', 
       'Optical_Depth_Small_Best_Ocean': __DataPath + 'Optical_Depth_Small_Best_Ocean', 
       'Optical_Depth_Small_Average_Ocean': __DataPath + 'Optical_Depth_Small_Average_Ocean', 
       'Optical_Depth_Large_Best_Ocean': __DataPath + 'Optical_Depth_Large_Best_Ocean', 
       'Optical_Depth_Large_Average_Ocean': __DataPath + 'Optical_Depth_Large_Average_Ocean', 
       'Mass_Concentration_Ocean': __DataPath + 'Mass_Concentration_Ocean', 
       'Effective_Radius_Ocean': __DataPath + 'Effective_Radius_Ocean', 
       'Cloud_Condensation_Nuclei_Ocean': __DataPath + 'Cloud_Condensation_Nuclei_Ocean', 
       'Asymmetry_Factor_Best_Ocean': __DataPath + 'Asymmetry_Factor_Best_Ocean', 
       'Asymmetry_Factor_Average_Ocean': __DataPath + 'Assymetry_Factor_Average_Ocean', 
       'Backscattering_Ratio_Best_Ocean': __DataPath + 'Backscattering_Ratio_Best_Ocean', 
       'Backscattering_Ratio_Average_Ocean': __DataPath + 'Backscattering_Ratio_Average_Ocean', 
       'Angstrom_Exponent_1_Ocean': __DataPath + 'Angstrom_Exponent_1_Ocean', 
       'Angstrom_Exponent_2_Ocean': __DataPath + 'Angstrom_Exponent_2_Ocean', 
       'Least_Squares_Error_Ocean': __DataPath + 'Least_Squares_Error_Ocean', 
       'Optical_Depth_Ratio_Small_Ocean_0.55micron': __DataPath + 'Optical_Depth_Ratio_Small_Ocean_0.55micron', 
       'Optical_Depth_by_models_ocean': __DataPath + 'Optical_Depth_by_models_ocean', 
       'Cloud_Fraction_Ocean': __DataPath + 'Cloud_Fraction_Ocean', 
       'Number_Pixels_Used_Ocean': __DataPath + 'Number_Pixels_Used_Ocean', 
       'Mean_Reflectance_Ocean': __DataPath + 'Mean_Reflectance_Ocean', 
       'STD_Reflectance_Ocean': __DataPath + 'STD_Reflectance_Ocean', 
       'Quality_Assurance_Ocean': __DataPath + 'Quality_Assurance_Ocean', 
       'Aerosol_Cldmask_Byproducts_Ocean': __DataPath + 'Aerosol_Cldmask_Byproducts_Ocean', 
       'Deep_Blue_Aerosol_Optical_Depth_550_Land': __DataPath + 'Deep_Blue_Aerosol_Optical_Depth_550_Land', 
       'Deep_Blue_Aerosol_Optical_Depth_Land': __DataPath + 'Deep_Blue_Optical_Depth_Land', 
       'Deep_Blue_Angstrom_Exponent_Land': __DataPath + 'Deep_Blue_Angstrom_Exponent_Land', 
       'Deep_Blue_Single_Scattering_Albedo_Land': __DataPath + 'Deep_blue_Single_Scattering_Albedo_Land', 
       'Deep_Blue_Surface_Reflectance_Land': __DataPath + 'Deep_Blue_Surface_Reflectance_Land', 
       'Deep_Blue_Mean_Reflectance_Land': __DataPath + 'Deep_Blue_Mean_Reflectance_Land', 
       'Deep_Blue_Number_Pixels_Used_Land': __DataPath + 'Deep_Blue_Number_Pixels_Used_Land', 
       'Deep_Blue_Aerosol_Optical_Depth_550_Land_STD': __DataPath + 'Deep_Blue_Aerosol_Optical_Depth_550_Land_STD', 
       'Deep_Blue_Aerosol_Optical_Depth_Land_STD': __DataPath + 'Deep_Blue_Aerosol_Optical_Depth_Land_Std'}
    _indexMap = {'default': lambda var, ind: var[(..., ind[0], ind[1])], 
       'Quality_Assurance_Land': lambda var, ind: var[(..., ind[0], ind[1], 0)], 
       'Quality_Assurance_Ocean': lambda var, ind: var[(..., ind[0], ind[1], 0)]}
    _FVMap = {'Latitude': -999.0, 'Longitude': -999.0, 
    ...
    _ScaleOffsetMap = {'Latitude': None, 'Longitude': None, 
       ...
       'Mean_Reflectance_Land_All': (9.999999747378752e-05, 0.0), 
       'Standard_Deviation_Reflectance_Land_All': (9.999999747378752e-05, 0.0), 
       'Path_Radiance_Land': (9.999999747378752e-05, 0.0), 
       'Error_Path_Radiance_Land': (0.0, 9.999999747378752e-05), 
       'Critical_Reflectance_Land': (9.999999747378752e-05, 0.0), 
       'Error_Critical_Reflectance_Land': (9.999999747378752e-05, 0.0), 
       ...
       'Mean_Reflectance_Land': (9.999999747378752e-05, 0.0), 
       'STD_Reflectance_Land': (9.999999747378752e-05, 0.0), 
       ...
       'Mean_Reflectance_Ocean': (9.999999747378752e-05, 0.0), 
       'STD_Reflectance_Ocean': (9.999999747378752e-05, 0.0), 
       ...
       'Deep_Blue_Mean_Reflectance_Land': (9.999999747378752e-05, 0.0), 
    ...
    def get_geo_centers(self):
        ...
        ind = numpy.transpose(numpy.array([tem / tem[0].size, tem % tem[0].size]), (1,
                                                                                    2,
                                                                                    0))
    ...
    def get_geo_corners(self):
        ...
        raise NotImplementedError('The regional_intersect and global_intersect\n                     mapping functions cannot be used with MODIS data,\n                     as no corner information is provided by this filetype.\n                     Please use the point_in_cell mapping function instead.')
...
class HDFnasaomihchol2_File(HDFFile):
    ...
    framework.
    """
    OMIAURAHCHO_FILE_NAME = 'OMI/Aura Formaldehyde (HCHO) Total Column 1-Orbit L2 Swath 13x24km'
    OMIAURAHCHO_CORNER_FILE_NAME = 'OMI/Aura Global Ground Pixel Corners 1-Orbit L2 Swath 13x24km'
    ...
    def __init__(self, filename, subtype='', extension=None, cornerDir=None, cornerFileList=None):
        ...
        if os.path.isdir(cornerDir):
            ...
            if self.pixCorners == None:
                ...
                for f in allPossible:
                    try:
                        if tables.isHDF5File(f) and getLongName(f) == HDFnasaomihchol2_File.OMIAURAHCHO_CORNER_FILE_NAME and getOrbitNumber(f) == forbitnumber:
                            self.pixCorners = f
                            break
                    except:
                        pass
    ...
    _nameExpMap = {'GroundPixelQualityFlags': __dataPath + 'GroundPixelQualityFlags', 'ColumnAmount': __dataPath + 'ColumnAmount', 
