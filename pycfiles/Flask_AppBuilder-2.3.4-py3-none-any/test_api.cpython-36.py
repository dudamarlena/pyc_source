...
# [GCC 8.4.0]
# Embedded file name: /Users/daniel/workarea/preset/Flask-AppBuilder/flask_appbuilder/tests/test_api.py
# Compiled at: 2020-04-29 00:55:00
# Size of source mod 2**32: 97280 bytes
import json, logging, os
from flask_appbuilder import ModelRestApi, SQLA
from flask_appbuilder.const import API_ADD_COLUMNS_RES_KEY, API_ADD_COLUMNS_RIS_KEY, API_ADD_TITLE_RIS_KEY, API_DESCRIPTION_COLUMNS_RES_KEY, API_DESCRIPTION_COLUMNS_RIS_KEY, API_EDIT_COLUMNS_RES_KEY, API_EDIT_COLUMNS_RIS_KEY, API_EDIT_TITLE_RIS_KEY, API_FILTERS_RIS_KEY, API_LABEL_COLUMNS_RES_KEY, API_LABEL_COLUMNS_RIS_KEY, API_LIST_COLUMNS_RES_KEY, API_LIST_COLUMNS_RIS_KEY, API_LIST_TITLE_RIS_KEY, API_ORDER_COLUMNS_RIS_KEY, API_PERMISSIONS_RES_KEY, API_PERMISSIONS_RIS_KEY, API_RESULT_RES_KEY, API_SECURITY_ACCESS_TOKEN_KEY, API_SELECT_COLUMNS_RIS_KEY, API_SELECT_KEYS_RIS_KEY, API_SHOW_COLUMNS_RIS_KEY, API_SHOW_TITLE_RIS_KEY, API_URI_RIS_KEY
from flask_appbuilder.models.sqla.filters import FilterGreater, FilterSmaller
from flask_appbuilder.models.sqla.interface import SQLAInterface
import prison
from sqlalchemy.sql.expression import func
from .base import FABTestCase
from .const import MAX_PAGE_SIZE, MODEL1_DATA_SIZE, MODEL2_DATA_SIZE, PASSWORD_ADMIN, PASSWORD_READONLY, USERNAME_ADMIN, USERNAME_READONLY
from .sqla.models import insert_model1, insert_model2, Model1, Model2, Model4, ModelMMChild, ModelMMParent, ModelMMParentRequired, ModelOMChild, ModelOMParent, ModelWithEnums, ModelWithProperty, TmpEnum, validate_name
...
class APICSRFTestCase(FABTestCase):
    ...
    def test_auth_login(self):
        ...
        assert json.loads(rv.data.decode('utf-8')).get(API_SECURITY_ACCESS_TOKEN_KEY, False)
...
class APITestCase(FABTestCase):
    ...
    def setUp(self):
        ...
        from flask_appbuilder.models.sqla.interface import SQLAInterface
        from flask_appbuilder.api import BaseApi, ModelRestApi, protect, expose, rison, safe
        self.app = Flask(__name__)
        ...
        class Model1Api(ModelRestApi):
            ...
            list_columns = [
             ...
             'field_string',
             'field_date']
            description_columns = {'field_integer':'Field Integer', 
        ...
        class CustomFilter(BaseFilter):
            ...
            def apply(self, query, value):
                return query.filter(~Model1.field_string.like(value + '%'), Model1.field_integer == 1)
        ...
        class Model1PermOverride(ModelRestApi):
            ...
            method_permission_name = {'get_list':'access', 
             'get':'access', 
             'put':'access', 
             'post':'access', 
             'delete':'access', 
    ...
    def test_base_rison_schema(self):
        ...
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        arguments = {'number': 1}
        uri = 'api/v1/base1api/test1?{}={}'.format(API_URI_RIS_KEY, prison.dumps(arguments))
        rv = self.auth_client_get(client, token, uri)
        self.assertEqual(rv.status_code, 200)
        data = json.loads(rv.data.decode('utf-8'))
    ...
    def test_exclude_route_methods(self):
        ...
        rv = self.auth_client_delete(client, token, uri)
        self.assertEqual(rv.status_code, 405)
        pvm = self.appbuilder.sm.find_permission_view_menu('can_info', 'Model1ApiExcludeRoutes')
        self.assertIsNone(pvm)
        pvm = self.appbuilder.sm.find_permission_view_menu('can_delete', 'Model1ApiExcludeRoutes')
        self.assertIsNone(pvm)
    ...
    def test_include_route_methods(self):
        ...
        rv = self.auth_client_get(client, token, uri)
        self.assertEqual(rv.status_code, 200)
        pvm = self.appbuilder.sm.find_permission_view_menu('can_info', 'Model1ApiIncludeRoutes')
        self.assertIsNone(pvm)
        pvm = self.appbuilder.sm.find_permission_view_menu('can_put', 'Model1ApiIncludeRoutes')
        self.assertIsNone(pvm)
        pvm = self.appbuilder.sm.find_permission_view_menu('can_post', 'Model1ApiIncludeRoutes')
        self.assertIsNone(pvm)
        pvm = self.appbuilder.sm.find_permission_view_menu('can_delete', 'Model1ApiIncludeRoutes')
        self.assertIsNone(pvm)
    ...
    def assert_get_item(self, rv, data, value):
        self.assertEqual(data[API_RESULT_RES_KEY], {'field_date':None, 
        ...
        self.assertEqual(data[API_LABEL_COLUMNS_RES_KEY], {'field_date':'Field Date', 
         'field_float':'Field Float', 
    ...
    def test_get_item_select_cols(self):
        ...
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        for i in range(1, MODEL1_DATA_SIZE):
            uri = 'api/v1/model1api/{}?q=({}:!(field_integer))'.format(i, API_SELECT_COLUMNS_RIS_KEY)
            rv = self.auth_client_get(client, token, uri)
            data = json.loads(rv.data.decode('utf-8'))
            self.assertEqual(data[API_RESULT_RES_KEY], {'field_integer': i - 1})
            self.assertEqual(data[API_DESCRIPTION_COLUMNS_RES_KEY], {'field_integer': 'Field Integer'})
            self.assertEqual(data[API_LABEL_COLUMNS_RES_KEY], {'field_integer': 'Field Integer'})
            self.assertEqual(rv.status_code, 200)
    ...
    def test_get_item_select_meta_data(self):
        ...
        for selectable_key in selectable_keys:
            ...
            uri = 'api/v1/model1api/1?{}={}'.format(API_URI_RIS_KEY, prison.dumps(argument))
            rv = self.auth_client_get(client, token, uri)
            data = json.loads(rv.data.decode('utf-8'))
    ...
    def test_get_item_excluded_cols(self):
        ...
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        pk = 1
        rv = self.auth_client_get(client, token, 'api/v1/model1apiexcludecols/{}'.format(pk))
        data = json.loads(rv.data.decode('utf-8'))
        self.assertEqual(data[API_RESULT_RES_KEY], {'field_string': 'test0'})
        self.assertEqual(rv.status_code, 200)
    ...
    def test_get_item_base_filters(self):
        ...
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        pk = 1
        rv = self.auth_client_get(client, token, 'api/v1/model1apifiltered/{}'.format(pk))
        self.assertEqual(rv.status_code, 404)
        pk = 4
    ...
    def test_get_item_1m_field(self):
        ...
        data = json.loads(rv.data.decode('utf-8'))
        self.assertEqual(rv.status_code, 200)
        expected_rel_field = {'group': {'field_date':None, 
                   'field_float':0.0, 
                   'field_integer':0, 
                   'field_string':'test0', 
                   'id':1}}
    ...
    def test_get_item_mm_field(self):
        ...
        expected_rel_field = [
         {'field_string':'1', 
          'field_integer':1,  'id':1},
         {'field_string':'2', 
          'field_integer':2,  'id':2},
         {'field_string':'3', 
          'field_integer':3,  'id':3}]
    ...
    def test_get_item_dotted_mm_field(self):
        ...
        data = json.loads(rv.data.decode('utf-8'))
        self.assertEqual(rv.status_code, 200)
        expected_result = {'field_string':'0', 
         'children':[
          {'field_integer': 1},
          {'field_integer': 2},
          {'field_integer': 3}]}
    ...
    def test_get_item_om_field(self):
        ...
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        pk = 1
        rv = self.auth_client_get(client, token, 'api/v1/modelomparentapi/{}'.format(pk))
        data = json.loads(rv.data.decode('utf-8'))
        self.assertEqual(rv.status_code, 200)
        expected_rel_field = [{'field_string':f"text0.{i}",  'id':i,  'parent':1} for i in range(1, 4)]
        self.assertEqual(data[API_RESULT_RES_KEY]['children'], expected_rel_field)
    ...
    def test_get_list_dotted_notation(self):
        ...
        client = self.app.test_client()
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        arguments = {'order_column':'field_string', 
         'order_direction':'asc'}
        uri = 'api/v1/model2dottednotationapi/?{}={}'.format(API_URI_RIS_KEY, prison.dumps(arguments))
        rv = self.auth_client_get(client, token, uri)
        data = json.loads(rv.data.decode('utf-8'))
        self.assertEqual(data['count'], MODEL1_DATA_SIZE)
        self.assertEqual(len(data[API_RESULT_RES_KEY]), self.model1api.page_size)
        i = 0
        self.assertEqual(data[API_RESULT_RES_KEY][i], {'field_string':'test0', 
         'group':{'field_string': 'test0'}})
    ...
    def test_get_list_dotted_order(self):
        ...
        client = self.app.test_client()
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        arguments = {'order_column':'group.field_string', 
        ...
        self.assertEqual(len(data[API_RESULT_RES_KEY]), self.model1api.page_size)
        i = 0
        self.assertEqual(data[API_RESULT_RES_KEY][i], {'field_string':'test9', 
         'group':{'field_string': 'test9'}})
    ...
    def test_get_list_multiple_dotted_order(self):
        ...
        client = self.app.test_client()
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        arguments = {'order_column':'model1_1.field_string', 
         'order_direction':'desc'}
        ...
        arguments = {'order_column':'model1_2.field_string', 
         'order_direction':'asc'}
        uri = 'api/v1/model4api/?{}={}'.format(API_URI_RIS_KEY, prison.dumps(arguments))
        rv = self.auth_client_get(client, token, uri)
        data = json.loads(rv.data.decode('utf-8'))
    ...
    def test_get_list_order(self):
        ...
        client = self.app.test_client()
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        arguments = {'order_column':'field_integer', 
         'order_direction':'asc'}
        uri = 'api/v1/model1api/?{}={}'.format(API_URI_RIS_KEY, prison.dumps(arguments))
        rv = self.auth_client_get(client, token, uri)
        data = json.loads(rv.data.decode('utf-8'))
        self.assertEqual(data[API_RESULT_RES_KEY][0], {'field_date':None, 
         'field_float':0.0, 
         'field_integer':0, 
         'field_string':'test0'})
        self.assertEqual(rv.status_code, 200)
    ...
    def test_get_list_filters(self):
        ...
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        filter_value = 5
        arguments = {API_FILTERS_RIS_KEY: [
                               {'col':'field_integer', 
                                'opr':'gt',  'value':filter_value}], 
         
         'order_column': 'field_integer', 
         'order_direction': 'asc'}
        uri = f"api/v1/model1api/?{API_URI_RIS_KEY}={prison.dumps(arguments)}"
        expected_result = {'field_date':None, 
    ...
    def test_get_list_filters_m_m(self):
        ...
        client = self.app.test_client()
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        arguments = {API_FILTERS_RIS_KEY: [{'col':'children',  'opr':'rel_m_m',  'value':[4]}]}
        uri = f"api/v1/modelmmapi/?{API_URI_RIS_KEY}={prison.dumps(arguments)}"
        rv = self.auth_client_get(client, token, uri)
        data = json.loads(rv.data.decode('utf-8'))
        self.assertEqual(data['count'], 1)
        self.assertEqual(data['result'][0]['children'][0]['field_string'], 'test_child_tmp')
        arguments = {API_FILTERS_RIS_KEY: [
                               {'col':'children', 
                                'opr':'rel_m_m',  'value':[1, 2]}]}
        uri = f"api/v1/modelmmapi/?{API_URI_RIS_KEY}={prison.dumps(arguments)}"
        rv = self.auth_client_get(client, token, uri)
        data = json.loads(rv.data.decode('utf-8'))
        self.assertEqual(data['count'], MODEL1_DATA_SIZE)
        parent_ = session.query(ModelMMParent).filter_by(field_string='test_tmp').one_or_none()
        child_ = session.query(ModelMMChild).filter_by(field_string='test_child_tmp').one_or_none()
    ...
    def test_get_list_filters_wrong_order(self):
        ...
        client = self.app.test_client()
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        arguments = {'order_column':'wrong_column', 
         'order_direction':'asc'}
        uri = f"api/v1/model1api/?{API_URI_RIS_KEY}={prison.dumps(arguments)}"
        rv = self.auth_client_get(client, token, uri)
        self.assertEqual(rv.status_code, 400)
    ...
    def test_get_list_custom_search_filters(self):
        ...
        arguments = {API_FILTERS_RIS_KEY: [
                               {'col':'field_string', 
                                'opr':'custom_filter',  'value':filter_value}]}
        rison_args = prison.dumps(arguments)
        uri = f"api/v1/model1apisearchfilters/?{API_URI_RIS_KEY}={rison_args}"
        client = self.app.test_client()
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
    ...
    def test_get_info_custom_search_filters(self):
        ...
        self.assertIn({'name':'Custom Filter', 
         'operator':'custom_filter'}, field_string_filters)
    ...
    def test_get_list_select_cols(self):
        ...
        client = self.app.test_client()
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        argument = {API_SELECT_COLUMNS_RIS_KEY: ['field_integer'], 
         'order_column': 'field_integer', 
         'order_direction': 'asc'}
    ...
    def test_info_fields(self):
        ...
        expect_add_fields = [
         ...
         {'description':'Field String', 
          ...
          'type':'String', 
          'validate':[
           '<Length(min=None, max=50, equal=None, error=None)>']},
         {'description':'', 
          'label':'Field Date', 
          'name':'field_date', 
          'required':False, 
    ...
    def test_info_fields_rel_field(self):
        ...
        rv = self.auth_client_get(client, token, uri)
        data = json.loads(rv.data.decode('utf-8'))
        expected_rel_add_field = {'count':MODEL2_DATA_SIZE, 
         'description':'', 
         'label':'Group', 
         'name':'group', 
         'required':True, 
        ...
        for i in range(self.model2api.page_size):
            expected_rel_add_field['values'].append({'id':i + 1, 
             'value':'test{}'.format(i)})
    ...
    def test_info_fields_rel_filtered_field(self):
        ...
        rv = self.auth_client_get(client, token, uri)
        data = json.loads(rv.data.decode('utf-8'))
        expected_rel_add_field = {'description':'', 
         'label':'Group', 
         'name':'group', 
         'required':True, 
         'unique':False, 
    ...
    def test_delete_item(self):
        ...
        client = self.app.test_client()
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        model = self.appbuilder.get_session.query(Model2).filter_by(field_string='test2').one_or_none()
        pk = model.id
    ...
    def test_delete_item_integrity(self):
        ...
        client = self.app.test_client()
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        model = self.appbuilder.get_session.query(Model1).filter_by(field_string='test0').one_or_none()
        pk = model.id
    ...
    def test_delete_item_base_filters(self):
        ...
        client = self.app.test_client()
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        model = self.appbuilder.get_session.query(Model1).filter_by(field_integer=2).one_or_none()
        pk = model.id
    ...
    def test_update_item(self):
        ...
        client = self.app.test_client()
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        model1 = self.appbuilder.get_session.query(Model1).filter_by(field_string='test2').one_or_none()
        pk = model1.id
    ...
    def test_update_item_base_filters(self):
        ...
        client = self.app.test_client()
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        model1 = self.appbuilder.get_session.query(Model1).filter_by(field_integer=3).one_or_none()
        pk = model1.id
        ...
        insert_model1((self.appbuilder.get_session), i=(pk - 1))
        model1 = self.appbuilder.get_session.query(Model1).filter_by(field_integer=1).one_or_none()
        pk = model1.id
        uri = f"api/v1/model1apifiltered/{pk}"
        rv = self.auth_client_put(client, token, uri, item)
        self.assertEqual(rv.status_code, 404)
    ...
    def test_update_val_size(self):
        ...
        client = self.app.test_client()
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        model1 = self.appbuilder.get_session.query(Model1).filter_by(field_string='test0').one_or_none()
        pk = model1.id
        field_string = 'a' * 51
        ...
        self.assertEqual(rv.status_code, 422)
        data = json.loads(rv.data.decode('utf-8'))
        self.assertEqual(data['message']['field_string'][0], 'Longer than maximum length 50.')
    ...
    def test_update_mm_field(self):
        ...
        session.add(child)
        session.commit()
        child_id = session.query(ModelMMChild).filter_by(field_string='update_m,m').one_or_none().id
        ...
        self.assertEqual(rv.status_code, 200)
        data = json.loads(rv.data.decode('utf-8'))
        self.assertEqual(data[API_RESULT_RES_KEY], {'children':[child_id],  'field_string':'0'})
        original_model = session.query(ModelMMParent).filter_by(id=pk).one_or_none()
    ...
    def test_create_item(self):
        ...
        model = self.db.session.query(Model1).filter_by(field_string=('test{}'.format(MODEL1_DATA_SIZE + 1))).first()
    ...
    def test_create_item_bad_request(self):
        ...
        uri = 'api/v1/model1api/'
        rv = client.post(uri,
          data=item, headers={'Authorization': 'Bearer {}'.format(token)})
        data = json.loads(rv.data.decode('utf-8'))
        self.assertEqual(rv.status_code, 400)
    ...
    def test_create_item_custom_validation(self):
        ...
        data = json.loads(rv.data.decode('utf-8'))
        self.assertEqual(rv.status_code, 422)
        self.assertEqual(data, {'message': {'field_string': ['Name must start with an A']}})
        item = dict(field_string=f"A{MODEL1_DATA_SIZE + 1}",
          field_integer=(MODEL1_DATA_SIZE + 1),
          field_float=(float(MODEL1_DATA_SIZE + 1)),
          field_date=None)
        rv = self.auth_client_post(client, token, uri, item)
        data = json.loads(rv.data.decode('utf-8'))
        self.assertEqual(rv.status_code, 201)
        self.appbuilder.get_session.query(Model1).filter_by(field_string=f"A{MODEL1_DATA_SIZE + 1}").delete()
        self.appbuilder.get_session.commit()
    ...
    def test_create_item_val_size(self):
        ...
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        field_string = 'a' * 51
        item = dict(field_string=field_string,
          field_integer=(MODEL1_DATA_SIZE + 1),
          field_float=(float(MODEL1_DATA_SIZE + 1)))
        uri = 'api/v1/model1api/'
        rv = self.auth_client_post(client, token, uri, item)
    ...
    def test_create_item_val_type(self):
        ...
        client = self.app.test_client()
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        item = dict(field_string=f"test{MODEL1_DATA_SIZE}",
        ...
        data = json.loads(rv.data.decode('utf-8'))
        self.assertEqual(data['message']['field_integer'][0], 'Not a valid integer.')
        item = dict(field_string=MODEL1_DATA_SIZE,
    ...
    def test_create_item_excluded_cols(self):
        ...
        rv = self.auth_client_post(client, token, uri, item)
        self.assertEqual(rv.status_code, 201)
        item = dict(field_string=('test{}'.format(MODEL1_DATA_SIZE + 2)),
          field_integer=(MODEL1_DATA_SIZE + 2))
        rv = self.auth_client_post(client, token, uri, item)
        self.assertEqual(rv.status_code, 201)
        model = self.db.session.query(Model1).filter_by(field_string=f"test{MODEL1_DATA_SIZE + 1}").first()
        self.assertEqual(model.field_integer, None)
        self.assertEqual(model.field_float, None)
        self.assertEqual(model.field_date, None)
        self.appbuilder.get_session.query(Model1).filter_by(field_string=f"test{MODEL1_DATA_SIZE + 1}").delete()
        self.appbuilder.get_session.query(Model1).filter_by(field_string=f"test{MODEL1_DATA_SIZE + 2}").delete()
        self.appbuilder.get_session.commit()
    ...
    def test_create_item_mm_field(self):
        ...
        self.assertEqual(rv.status_code, 201)
        data = json.loads(rv.data.decode('utf-8'))
        self.assertEqual(data[API_RESULT_RES_KEY], {'children':[1, 2],  'field_string':'new1'})
        item = dict(field_string='new2')
        uri = 'api/v1/modelmmapi/'
        rv = self.auth_client_post(client, token, uri, item)
        self.assertEqual(rv.status_code, 201)
        data = json.loads(rv.data.decode('utf-8'))
        self.assertEqual(data[API_RESULT_RES_KEY], {'children':[],  'field_string':'new2'})
        item = dict(field_string='new1')
        uri = 'api/v1/modelmmrequiredapi/'
        rv = self.auth_client_post(client, token, uri, item)
        self.assertEqual(rv.status_code, 422)
        data = json.loads(rv.data.decode('utf-8'))
        self.assertEqual(data, {'message': {'children': ['Missing data for required field.']}})
        model1 = self.appbuilder.get_session.query(ModelMMParent).filter_by(field_string='new1').one_or_none()
        model2 = self.appbuilder.get_session.query(ModelMMParent).filter_by(field_string='new2').one_or_none()
    ...
    def test_create_item_om_field(self):
        ...
        self.assertEqual(rv.status_code, 201)
        data = json.loads(rv.data.decode('utf-8'))
        self.assertEqual(data[API_RESULT_RES_KEY], {'children':[
          child1.id, child2.id], 
         'field_string':'new1'})
        model1 = self.appbuilder.get_session.query(ModelOMParent).filter_by(field_string='new1').one_or_none()
        self.appbuilder.get_session.delete(model1)
        self.appbuilder.get_session.commit()
    ...
    def test_get_list_col_function(self):
        ...
        for i in range(1, self.model1api.page_size):
            ...
            self.assertEqual(item['full_concat'], f"test{str(i - 1)}.{i - 1}.{float(i - 1)}.{None}")
    ...
    def test_method_permission_override(self):
        ...
        role = self.appbuilder.sm.add_role('Test')
        pvm = self.appbuilder.sm.find_permission_view_menu('can_read', 'Model2PermOverride2')
        self.appbuilder.sm.add_permission_role(role, pvm)
        self.appbuilder.sm.add_user('test', 'test', 'user', 'test@fab.org', role, 'test')
        client = self.app.test_client()
        token = self.login(client, 'test', 'test')
    ...
    def test_base_permission_override(self):
        ...
        pvm = self.appbuilder.sm.find_permission_view_menu('can_write', 'Model2PermOverride3')
        self.assertEqual(pvm.permission.name, 'can_write')
        pvm = self.appbuilder.sm.find_permission_view_menu('can_read', 'Model2PermOverride3')
    ...
    def test_permission_converge_compress(self):
        ...
        class Model1PermConverge(ModelRestApi):
            ...
            method_permission_name = {'get_list':'access2', 
             'get':'access2', 
             'put':'access2', 
             'post':'access2', 
             'delete':'access2', 
        ...
        self.appbuilder.baseviews.pop(i)
        target_state_transitions = {'add':{('Model1Api', 'can_get'): {('api2', 'can_access2')}, 
    ...
    def test_permission_converge_expand(self):
        ...
        class Model1PermConverge(ModelRestApi):
            ...
            method_permission_name = {'get_list':'get', 
             'get':'get', 
             'put':'put', 
             'post':'post', 
             'delete':'delete', 
             'info':'info'}
            previous_method_permission_name = {'get_list':'access', 
             'get':'access', 
             'put':'access', 
             'post':'access', 
             'delete':'access', 
        ...
        self.appbuilder.baseviews.pop(i)
        target_state_transitions = {'add':{('api', 'can_access'): {
