...
# [GCC 8.4.0]
# Embedded file name: /Users/daniel/workarea/preset/Flask-AppBuilder/flask_appbuilder/tests/test_mvc.py
# Compiled at: 2020-03-31 08:10:13
# Size of source mod 2**32: 57835 bytes
import datetime, json, logging
from typing import Set
from flask import Flask, redirect, request, session
from flask_appbuilder import AppBuilder, SQLA
from flask_appbuilder.charts.views import ChartView, DirectByChartView, DirectChartView, GroupByChartView, TimeChartView
...
from .sqla.models import insert_model1, insert_model2, insert_model3, insert_model_with_enums, Model1, Model2, Model3, ModelWithEnums, TmpEnum
...
class MVCBabelTestCase(FABTestCase):
    ...
    def test_babel_languages(self):
        ...
        app = Flask(__name__)
        app.config.from_object('flask_appbuilder.tests.config_api')
        app.config['LANGUAGES'] = {'en':{'flag':'gb', 
          'name':'English'}, 
         'pt':{'flag':'pt', 
          'name':'Portuguese'}}
        db = SQLA(app)
...
class BaseMVCTestCase(FABTestCase):
    ...
    def get_registered_view_endpoints(self, view_name) -> Set:
        return {item.endpoint for item in self.app.url_map.iter_rules() if item.endpoint.split('.')[0] == view_name}
...
class MVCCSRFTestCase(BaseMVCTestCase):
    ...
    def test_a_csrf_delete_not_allowed(self):
        ...
        self.browser_login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        model = self.appbuilder.get_session.query(Model2).filter_by(field_string='test0').one_or_none()
        pk = model.id
...
class MVCSwitchRouteMethodsTestCase(BaseMVCTestCase):
    ...
    def setUp(self):
        ...
        self.appbuilder.add_view_no_menu(Model2IncludeExcludeView, 'Model2IncludeExcludeView')
    ...
    def test_include_route_methods(self):
        ...
        """
        expected_endpoints = {
         'Model2IncludeView.list', 'Model2IncludeView.show'}
        self.assertEqual(expected_endpoints, self.get_registered_view_endpoints('Model2IncludeView'))
    ...
    def test_exclude_route_methods(self):
        ...
        self.assertEqual(expected_endpoints, self.get_registered_view_endpoints('Model2ExcludeView'))
    ...
    def test_include_exclude_route_methods(self):
        ...
        self.assertEqual(expected_endpoints, self.get_registered_view_endpoints('Model2IncludeExcludeView'))
        unexpected_permissions = [
    ...
    def test_disable_mvc_api_methods(self):
        ...
        self.assertEqual(expected_endpoints, self.get_registered_view_endpoints('Model2DisableMVCApiView'))
...
class MVCTestCase(BaseMVCTestCase):
    ...
    def setUp(self):
        ...
        self.appbuilder.add_view(Model1ViewWithRedirects,
          'Model1ViewWithRedirects', category='Model1')
        ...
        self.appbuilder.add_view(Model1MasterChartView,
          'Model1MasterChart', category='Model1')
        self.appbuilder.add_view(Model1Filtered1View,
          'Model1Filtered1', category='Model1')
        self.appbuilder.add_view(Model1Filtered2View,
          'Model1Filtered2', category='Model1')
        self.appbuilder.add_view(Model1FormattedView,
          'Model1FormattedView', category='Model1FormattedView')
        ...
        self.appbuilder.sm.add_user(USERNAME_READONLY, 'readonly', 'readonly', 'readonly@fab.org', role_read_only, PASSWORD_READONLY)
    ...
    def test_model_crud_add(self):
        ...
        rv = client.post('/model1view/add',
          data=dict(field_string=field_string,
          field_integer=(f"{MODEL1_DATA_SIZE}"),
          field_float=(f"{float(MODEL1_DATA_SIZE)}"),
          field_date='2014-01-01'),
          follow_redirects=True)
        self.assertEqual(rv.status_code, 200)
        model = self.db.session.query(Model1).filter_by(field_string=field_string).one_or_none()
    ...
    def test_model_crud_edit(self):
        ...
        model = self.appbuilder.get_session.query(Model1).filter_by(field_string='test0').one_or_none()
        pk = model.id
        rv = client.post(f"/model1view/edit/{pk}",
    ...
    def test_model_delete_integrity(self):
        ...
        client = self.app.test_client()
        self.browser_login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        model1 = self.appbuilder.get_session.query(Model1).filter_by(field_string='test1').one_or_none()
        pk = model1.id
    ...
    def test_model_crud_composite_pk(self):
        ...
        rv = client.post(('/model3view/edit/' + quote(pk)),
          ...
          follow_redirects=True)
        self.assertEqual(rv.status_code, 200)
        model = self.appbuilder.get_session.query(Model3).filter_by(pk1='2',
          pk2='2017-02-02 00:00:00').one_or_none()
    ...
    def test_model_crud_edit_with_enum(self):
        ...
        data = {'enum1':'e3', 
         'enum2':'e3'}
        pk = 1
        rv = client.post(f"/modelwithenumsview/edit/{pk}",
    ...
    def test_modelview_add_redirects(self):
        ...
        rv = client.post('/model1viewwithredirects/add',
          data=dict(field_string='test_redirect'))
        self.assertEqual(rv.status_code, 302)
        self.assertEqual('http://localhost/', rv.headers['Location'])
        model1 = self.appbuilder.get_session.query(Model1).filter_by(field_string='test_redirect').one_or_none()
    ...
    def test_modelview_edit_redirects(self):
        ...
        client = self.app.test_client()
        self.browser_login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        model_id = self.db.session.query(Model1).filter_by(field_string='test0').one_or_none().id
        rv = client.post(f"/model1viewwithredirects/edit/{model_id}",
    ...
    def test_modelview_delete_redirects(self):
        ...
        client = self.app.test_client()
        rv = self.browser_login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        model_id = self.db.session.query(Model1).filter_by(field_string='test0').first().id
    ...
    def test_model_list_order(self):
        ...
        client = self.app.test_client()
        self.browser_login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        rv = client.get('/model1view/list?_oc_Model1View=field_string&_od_Model1View=asc',
          follow_redirects=True)
        self.assertEqual(rv.status_code, 200)
        data = rv.data.decode('utf-8')
        self.assertIn('test0', data)
        rv = client.get('/model1view/list?_oc_Model1View=field_string&_od_Model1View=desc',
          follow_redirects=True)
        self.assertEqual(rv.status_code, 200)
        data = rv.data.decode('utf-8')
    ...
    def test_api_create(self):
        ...
        client = self.app.test_client()
        self.browser_login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        rv = client.post('/model1view/api/create',
    ...
    def test_api_update(self):
        ...
        rv = client.put('/model1view/api/update/1',
          data=dict(field_string='zzz'),
          follow_redirects=True)
        self.assertEqual(rv.status_code, 200)
        item = self.db.session.query(Model1).filter_by(id=1).one()
    ...
    def test_class_method_permission_override(self):
        ...
        self.assertEqual(rv.status_code, 200)
        rv = client.post('/model1permoverride/add',
          data=dict(field_string='test1',
          field_integer='1',
          field_float='0.12',
          field_date='2014-01-01'),
          follow_redirects=True)
        self.assertEqual(rv.status_code, 200)
        model = self.db.session.query(Model1).filter_by(field_string='test1').one_or_none()
    ...
    def test_method_permission_override(self):
        ...
        class Model1PermOverride(ModelView):
            datamodel = SQLAInterface(Model1)
            method_permission_name = {'list':'read', 
             'show':'read', 
             'edit':'write', 
             'add':'write', 
             'delete':'write', 
        ...
        self.appbuilder.add_view_no_menu(Model1PermOverride)
        role = self.appbuilder.sm.add_role('Test')
        pvm_read = self.appbuilder.sm.find_permission_view_menu('can_read', 'Model1PermOverride')
        pvm_write = self.appbuilder.sm.find_permission_view_menu('can_write', 'Model1PermOverride')
        ...
        self.browser_login(client, 'test', 'test')
        rv = client.post('/model1permoverride/add',
          data=dict(field_string=f"test{MODEL1_DATA_SIZE + 1}",
          field_integer='1',
          field_float='0.12',
          field_date='2014-01-01'),
          follow_redirects=True)
        self.assertEqual(rv.status_code, 200)
        model1 = self.appbuilder.get_session.query(Model1).filter_by(field_string=f"test{MODEL1_DATA_SIZE + 1}").one_or_none()
        self.assertIsNotNone(model1)
        self.appbuilder.get_session.delete(model1)
        self.appbuilder.get_session.commit()
        rv = client.get('/model1permoverride/list/')
    ...
    def test_permission_converge_compress(self):
        ...
        target_state_transitions = {'add':{('Model1View', 'can_edit'): {('view2', 'can_access')}, 
