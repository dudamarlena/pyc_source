...
# [GCC 8.4.0]
# Embedded file name: /Users/daniel/workarea/preset/Flask-AppBuilder/flask_appbuilder/tests/sqla/models.py
# Compiled at: 2020-04-29 00:55:00
# Size of source mod 2**32: 9291 bytes
import datetime, enum
from flask_appbuilder import Model
from marshmallow import fields, post_load, Schema, ValidationError
from sqlalchemy import Column, Date, DateTime, Enum, Float, ForeignKey, Integer, String, Table, UniqueConstraint
...
class Model1(Model):
    ...
    def full_concat(self):
        return '{}.{}.{}.{}'.format(self.field_string, self.field_integer, self.field_float, self.field_date)
...
assoc_parent_child = Table('parent_child', Model.metadata, Column('id', Integer, primary_key=True), Column('parent_id', Integer, ForeignKey('parent.id')), Column('child_id', Integer, ForeignKey('child.id')), UniqueConstraint('parent_id', 'child_id'))
...
assoc_parent_child_required = Table('parent_child_required', Model.metadata, Column('id', Integer, primary_key=True), Column('parent_id', Integer, ForeignKey('parent_required.id')), Column('child_id', Integer, ForeignKey('child_required.id')), UniqueConstraint('parent_id', 'child_id'))

class ModelMMParentRequired(Model):
    __tablename__ = 'parent_required'
    id = Column(Integer, primary_key=True)
    field_string = Column((String(50)), unique=True, nullable=False)
    children = relationship('ModelMMChildRequired',
...
class ModelOMChild(Model):
    ...
    parent = relationship('ModelOMParent',
