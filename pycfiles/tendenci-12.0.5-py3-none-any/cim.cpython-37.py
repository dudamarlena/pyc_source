...
# [GCC 8.4.0]
# Embedded file name: /Users/jennyq/.pyenv/versions/venv_t12/lib/python3.7/site-packages/tendenci/apps/recurring_payments/authnet/cim.py
# Compiled at: 2020-03-30 17:48:04
# Size of source mod 2**32: 34320 bytes
import re
...
TRANSACTION_EXTRA_OPTIONS = {'x_delim_data':'TRUE',  'x_delim_char':'||', 
 'x_encap_char':''}
...
class CIMBase(object):
    ...
    def create_payment_profile_node(self, node_name, billing_info, credit_card_info, **kwargs):
        ...
        if billing_info:
            if type(billing_info) is dict:
                bill_to_node = ET.SubElement(payment_profiles_node, 'billTo')
                bill_to_node = self.build_node_from_dict(bill_to_node, billing_info, BILL_TO_FIELDS)
        payment_node = ET.SubElement(payment_profiles_node, 'payment')
        credit_card_info = to_camel_case(credit_card_info)
        if credit_card_info:
            if type(credit_card_info) is dict:
                credit_card_node = ET.SubElement(payment_node, 'creditCard')
                credit_card_node = self.build_node_from_dict(credit_card_node, credit_card_info, CREDIT_CARD_FIELDS)
    ...
    def process_request(self, xml_root):
        ...
        request = Request(self.cim_url, bytes(request_xml_str, encoding='UTF-8'), {'Content-Type':'text/xml', 
         'encoding':'utf-8'})
...
class CIMCustomerProfile(CIMBase):
    ...
    def create_profile_node(self, **kwargs):
        ...
        if mode == 'create' and not customer_id or all([email, description]):
            ...
            if not customer_profile_id:
                ...
                if mode == 'create':
                    ...
                    if billing_info or credit_card_info:
                        payment_profiles_node = self.create_payment_profile_node('paymentProfiles', billing_info, credit_card_info)
...
class CIMCustomerPaymentProfile(CIMBase):
    ...
    def create(self, **kwargs):
        ...
        credit_card_info = kwargs.get('credit_card_info', '')
        if billing_info or credit_card_info:
            payment_profiles_node = self.create_payment_profile_node('paymentProfile', billing_info, credit_card_info)
    ...
    def delete(self, **kwargs):
       ...
       Output fields:
        ...
        """
        if not (self.customer_profile_id and self.customer_payment_profile_id):
...
class CIMCustomerProfileTransaction(CIMBase):
    ...
    def create(self, **kwargs):
        ...
        """
        if not self.customer_profile_id:
            msg_string = '%s Missing customer_profile_id in input.' % 'createCustomerProfileTransactionRequest'
    ...
    def create_transaction_node(self, **kwargs):
        ...
        if tax:
            if type(tax) is dict:
                ...
                tax_node = self.build_node_from_dict(tax_node, tax, TAX_FIELDS)
        if shipping:
            if type(shipping) is dict:
                shipping = to_camel_case(shipping)
                shipping_node = ET.SubElement(trans_auth_capture_node, 'shipping')
                shipping_node = self.build_node_from_dict(shipping_node, shipping, SHIPPING_FIELDS)
        if line_items_list:
            if type(line_items_list) is list:
                for line_items in line_items_list:
                    if line_items and type(line_items) is dict:
                        ...
                        line_items_node = self.build_node_from_dict(line_items_node, line_items, LINE_ITEMS_FIELDS)
        ...
        if order:
            if type(order) is dict:
                ...
                order_node = self.build_node_from_dict(order_node, order, ORDER_FIELDS)
        if tax_exempt:
...
class CIMHostedProfilePage(CIMBase):
    ...
    def get(self, **kwargs):
        ...
        """
        if not self.customer_profile_id:
            msg_string = '%s Missing customer_profile_id in input.' % 'getHostedProfilePageRequest'
            raise AttributeError(_(msg_string))
        ...
        if hosted_profile_settings:
            if type(hosted_profile_settings) is dict:
                ...
                for key in hosted_profile_settings:
                    ...
                    setting_node = self.build_node_from_dict(setting_node, hosted_profile_setting, ('settingName', ))
