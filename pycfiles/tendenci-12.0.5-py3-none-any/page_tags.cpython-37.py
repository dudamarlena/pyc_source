...
# [GCC 8.4.0]
# Embedded file name: /Users/jennyq/.pyenv/versions/venv_t12/lib/python3.7/site-packages/tendenci/apps/pages/templatetags/page_tags.py
...
def page_options(context, user, page):
    context.update({'opt_object':page, 
     'user':user})
    return context
...
def page_nav(context, user, page=None):
    context.update({'nav_object':page, 
     'user':user})
    return context
...
def page_current_app(context, user, page=None):
    context.update({'app_object':page, 
     'user':user})
    return context
