...
# [GCC 8.4.0]
# Embedded file name: /Users/jennyq/.pyenv/versions/venv_t12/lib/python3.7/site-packages/tendenci/apps/corporate_memberships/templatetags/corporate_memberships_tags.py
# Compiled at: 2020-03-16 17:04:48
...
def render_corpmembership_field(request, field_obj, corpprofile_form, corpmembership_form):
    if field_obj.field_type == 'section_break':
        field = None
    else:
        field_name = field_obj.field_name
        if field_name in corpprofile_form.field_names:
            field = field_obj.display_only or corpprofile_form[field_name]
        else:
            if field_name in corpmembership_form.field_names:
                field = field_obj.display_only or corpmembership_form[field_name]
            else:
                field = None
...
def corpmemb_nav(context, user, corp_memb=None):
    context.update({'nav_object':corp_memb, 
     'user':user})
    return context
...
def corpmemb_current_app(context, user, corp_memb=None):
    context.update({'app_object':corp_memb, 
     'user':user})
    return context
...
def corpmemb_options(context, user, corp_memb):
    context.update({'opt_object':corp_memb, 
     'user':user})
    return context
...
class ListCorpMembershipNode(Node):
    ...
    def render(self, context):
        ...
        randomize = False
        renewed_only = False
        allow_anonymous_search = get_setting('module', 'corporate_memberships', 'anonymoussearchcorporatemembers')
        allow_member_search = get_setting('module', 'corporate_memberships', 'membersearchcorporatemembers')
