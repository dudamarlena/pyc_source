...
# [GCC 8.4.0]
# Embedded file name: /Users/jennyq/.pyenv/versions/venv_t12/lib/python3.7/site-packages/tendenci/apps/stories/templatetags/story_tags.py
...
def stories_options(context, user, story):
    context.update({'opt_object':story, 
     'user':user})
    return context
...
def stories_nav(context, user, story=None):
    context.update({'nav_object':story, 
     'user':user})
    return context
...
def story_current_app(context, user, story=None):
    context.update({'app_object':story, 
     'user':user})
    return context
