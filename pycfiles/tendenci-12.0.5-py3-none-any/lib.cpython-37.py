...
# [GCC 8.4.0]
# Embedded file name: /Users/jennyq/.pyenv/versions/venv_t12/lib/python3.7/site-packages/tendenci/apps/helpdesk/lib.py
...
def send_templated_mail(template_name, context, recipients, sender=None, bcc=None, fail_silently=False, files=None):
    ...
    if not locale:
        ...
        if not sender:
            ...
            text_part = get_template_from_string("%s{%% include '%s' %%}" % (t.plain_text, footer_file)).render(context=context)
            ...
            html_part = get_template_from_string("{%% extends '%s' %%}{%% block title %%}%s{%% endblock %%}{%% block content %%}%s{%% endblock %%}" % (email_html_base_file, t.heading, t.html)).render(context=context)
            subject_part = get_template_from_string(HELPDESK_EMAIL_SUBJECT_TEMPLATE % {'subject': t.subject}).render(context=context)
...
def safe_template_context(ticket):
    ...
    for field in ('title', 'created', 'modified', 'submitter_email', 'status', 'get_status_display',
                  'on_hold', 'description', 'resolution', 'priority', 'get_priority_display',
                  'last_escalation', 'ticket', 'ticket_for_url', 'get_status', 'ticket_url',
                  'staff_url', '_get_assigned_to'):
...
def text_is_spam(text, request):
    ...
    from django.conf import settings
    try:
        from tendenci.apps.helpdesk.akismet import Akismet
    except:
        return False
        try:
        ...
        ak = Akismet(blog_url=('http://%s/' % site.domain),
        ...
        if hasattr(settings, 'TYPEPAD_ANTISPAM_API_KEY'):
            ...
            if hasattr(settings, 'AKISMET_API_KEY'):
                ...
                return False
        if ak.verify_key():
            ak_data = {'user_ip':request.META.get('REMOTE_ADDR', '127.0.0.1'),  'user_agent':request.META.get('HTTP_USER_AGENT', ''), 
