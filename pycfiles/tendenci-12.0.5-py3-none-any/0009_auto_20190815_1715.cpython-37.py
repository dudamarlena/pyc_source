...
# [GCC 8.4.0]
# Embedded file name: /Users/jennyq/.pyenv/versions/venv_t12/lib/python3.7/site-packages/tendenci/apps/news/migrations/0009_auto_20190815_1715.py
...
def remove_google_profile_from_news_view(apps, schema_editor):
    ...
    if os.path.isfile(file_path):
        with open(file_path, 'r') as (f):
            content = f.read()
            p = '{0}([\\d\\D\\s\\S\\w\\W]*?){1}([\\s\\S]*?){2}'.format(re.escape('{% if news.google_profile %}'), re.escape('{% endif %}'), re.escape('{% endif %}'))
