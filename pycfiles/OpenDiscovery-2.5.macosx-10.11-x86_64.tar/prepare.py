...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/OpenDiscovery/md/prepare.py
# Compiled at: 2016-03-23 20:28:36
from . import *
from .. import *
import sh, logging, os, shutil, glob
logging.basicConfig(level=logging.DEBUG)
kSYSTEM = enum(RECEPTOR=0, CONF=1, LIGAND=2)

class MDPreparation(object):
    """docstring for MDPreparation"""

    def __init__(self, directory, after_screening=True):
    ...
    def prepareFilesForMDPrep(self):
        ...
        for s in self.systems['systems']:
            _systemPath = _prepPath + s[kSYSTEM.RECEPTOR] + '.' + s[kSYSTEM.CONF] + '.' + s[kSYSTEM.LIGAND] + '/'
            makeFolder(_systemPath)
            self.log.debug(('Creating folder: {0}').format(_systemPath))
            shutil.copy2(_dockingPath + 'receptors/' + s[kSYSTEM.RECEPTOR] + '.pdb', _systemPath + s[kSYSTEM.RECEPTOR] + '.pdb')
            self.log.debug(('Copying {0} to {1}').format(_dockingPath + 'receptors/' + s[kSYSTEM.RECEPTOR] + '.pdb', _systemPath + s[kSYSTEM.RECEPTOR] + '.pdb'))
            shutil.copy2(_dockingPath + 'results-' + s[kSYSTEM.RECEPTOR] + '/' + s[kSYSTEM.CONF] + '/' + s[kSYSTEM.LIGAND] + '/' + s[kSYSTEM.LIGAND] + '_mode_1.pdb', _systemPath + s[kSYSTEM.LIGAND] + '.pdb')
            self.log.debug(('Copying {0} to {1}').format(_dockingPath + 'results-' + s[kSYSTEM.RECEPTOR] + '/' + s[kSYSTEM.CONF] + '/' + s[kSYSTEM.LIGAND] + '/' + s[kSYSTEM.LIGAND] + '_mode_1.pdb', _systemPath + s[kSYSTEM.LIGAND] + '.pdb'))

    def cleanReceptorPDBs(self):
        for s in self.systems['systems']:
            self.log.debug(('Cleaning Receptor with path: {0}').format(self.getReceptorPathFromSystem(s)))
    ...
    def prepareLigands(self):
        for s in self.systems['systems']:
            for l in self.systems['ligands']:
                ...
                _systemPath = self.md_prep_path + s[kSYSTEM.RECEPTOR] + '.' + s[kSYSTEM.CONF] + '.' + s[kSYSTEM.LIGAND] + '/'
                lig_path = os.path.join(_systemPath, lig_full_name)
                sh.antechamber(('-i {0} -fi pdb -o {1}.ante.mol2 -fo mol2 -c bcc').format(lig_path, os.path.join(_systemPath, lig_name)))
    ...
    def getSystemList(self):
        _receptors = self.getFilesFromDockingFolderWithPath('receptors/*.pdbqt')
