...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/epcy/utils/readers.py
# Compiled at: 2020-03-30 18:11:48
# Size of source mod 2**32: 12195 bytes
import os, re, sys, time, gzip, h5py, pandas as pd, numpy as np
...
GTF_HEADER = [
 ...
 'strand', 'frame']
R_SEMICOLON = re.compile('\\s*;\\s*')
R_COMMA = re.compile('\\s*,\\s*')
R_KEYVALUE = re.compile('(\\s+|\\s*=\\s*)')
...
def counts2cpm(counts):
    ...
    counts = counts / total_mass * 1000000.0
    return counts
...
def counts2tpm(counts, transcripts_len):
    ...
    tpm_value = counts / total_mass * 1000000.0
    return tpm_value


def read_kall_project(file_h5, num_features, transcripts_len, args, ids_genes):
...
def get_design(args):
    ...
    design = design.sort_values(by=[args.SUBGROUP, 'sample'], ascending=[
     False, True])
    return design
...
def create_kal_mat(args, design, design_bootstrapped, df_anno):
    ...
    for index, row in design.iterrows():
        ...
        sample_data = read_kall_project(file_h5, num_features, transcripts_len, args, ids_genes)
        if args.BS != 0:
