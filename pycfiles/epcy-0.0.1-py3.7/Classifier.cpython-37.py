...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/epcy/utils/Classifier.py
# Compiled at: 2020-03-30 17:59:09
# Size of source mod 2**32: 24831 bytes
...
class Classifier:
    ...
    def __pred(self):
        ...
        if hasattr(self.args, 'BS'):
            if self.args.BS is not None:
                num_bs = self.args.BS
        self._Classifier__create_empty_res()
        cpt_id = 0
        for select_id in self.list_ids:
            ...
            ids_na = None
            if self.with_na > 0:
                row_data, num_query, ids_na = self.rm_missing(row_data, num_query)
            ...
            self.bw_query[cpt_id] = Classifier.bw_nrd((row_data[:num_query]), num_bs=num_bs)
            self.bw_ref[cpt_id] = Classifier.bw_nrd((row_data[num_query:]), num_bs=num_bs)
            if np.unique(row_data).size != 1 and sum_row >= self.args.EXP and abs(self.l2fc[cpt_id]) >= self.args.L2FC:
                if self.args.AUC:
                ...
                ct_by_bagging_and_pred_by_sample = self.pred_fill_cont_table_kernel(row_data,
                  num_query, (self.args.MIN_BW), n_bagging=(self.args.N_BAGGING),
                  num_bs=num_bs,
                ...
                if self.args.FULL:
                    if ids_na is not None:
                        if len(ids_na) > 0:
                            pred_by_sample = np.insert(pred_by_sample, ids_na, np.nan)
                    self.kernel_pred[cpt_id, :] = pred_by_sample
                if self.args.NORMAL:
                    ct_by_bagging_and_pred_by_sample = self.pred_fill_cont_table_normal(row_data,
                      num_query, n_bagging=(self.args.N_BAGGING), num_draw=(self.args.N_DRAW),
                      random_state=(self.random_state),
                      num_bs=num_bs)
                    mcc, pred_by_sample = self.get_mcc_ped_by_bagging(ct_by_bagging_and_pred_by_sample)
    ...
    def auc_u_test(row_data, num_query, num_ref):
        u_value, p_value = mannwhitneyu((row_data[:num_query]), (row_data[num_query:]),
    ...
    def t_test_welch(row_data, num_query):
        t_value, p_value = ttest_ind((row_data[:num_query]), (row_data[num_query:]),
          equal_var=False)
        return p_value
    ...
    def pred_fill_cont_table_kernel(row_data, num_query, min_bw, bw=None, n_bagging=1, num_bs=0, num_draw=100, random_state=np.random.RandomState()):
        N = len(row_data)
        return Classifier.get_ct_using_fx_kernel(row_data,
          num_query, N, min_bw, bw=bw, n_bagging=n_bagging, num_bs=num_bs,
          num_draw=num_draw,
          random_state=random_state)

    @staticmethod
    def pred_fill_cont_table_normal(row_data, num_query, n_bagging=1, num_draw=100, num_bs=0, random_state=np.random.RandomState()):
        N = len(row_data)
        return Classifier.get_ct_using_fx_normal(row_data,
          num_query, N, n_bagging=n_bagging, num_draw=num_draw, random_state=random_state,
          num_bs=num_bs)
    ...
    @staticmethod
    def compute_kernel_fx_and_ct(k_bw_nq_gen, num_query, N, num_bs=0, num_draw=100, random_state=np.random.RandomState()):
        fx_by_sample = [Classifier.compute_kernel_fx(k_bw_nq, i, num_bs=num_bs) for i, k_bw_nq in enumerate(k_bw_nq_gen)]
        return Classifier.fx_to_tables(fx_by_sample, num_query, N, num_draw=num_draw,
          random_state=random_state)
    ...
    def get_ct_using_fx_kernel(row_data, num_query, N, min_bw, bw=None, n_bagging=1, num_bs=0, num_draw=100, random_state=np.random.RandomState()):
        ...
        k_bw_gen_by_fold = [Classifier.get_k_gaussian_kernel_for_all_x(row_data, x_ids, num_query, min_bw, bw=bw, n_bagging=n_bagging, num_bs=num_bs, random_state=random_state) for x_ids in n_folds]
        k_bw_gen_by_bag = np.transpose(np.asarray(k_bw_gen_by_fold), (2, 0, 1, 3))
        k_bw_gen_by_bag = np.reshape(k_bw_gen_by_bag, (n_bagging, N, 2))
        ct_by_bag = np.asarray([Classifier.compute_kernel_fx_and_ct(k_bw_nq_gen, num_query, N, num_bs, num_draw=num_draw, random_state=random_state) for k_bw_nq_gen in k_bw_gen_by_bag])
        return ct_by_bag
    ...
    def get_ct_using_fx_normal(row_data, num_query, N, n_bagging=1, num_bs=0, num_draw=100, random_state=np.random.RandomState()):
        ...
        if num_bs > 0:
            ...
            n_folds = np.array_split(np.arange(N), N)
        fx_by_fold = [Classifier.compute_normal_fx_for_all_x(row_data, x_ids, num_query, n_bagging=n_bagging, num_bs=num_bs, random_state=random_state) for x_ids in n_folds]
        fx_by_fold = np.asarray(fx_by_fold)
        fx_by_bag = np.transpose(fx_by_fold, (2, 0, 1))
        fx_by_bag = np.reshape(fx_by_bag, (n_bagging, N))
        ct_by_bag = (Classifier.fx_to_tables(fx_by_sample, num_query, N, num_draw=num_draw, random_state=random_state) for fx_by_sample in fx_by_bag)
        return ct_by_bag
    ...
    def fx_to_tables(fx_by_sample, num_query, N, num_draw=100, random_state=np.random.RandomState()):
        ...
        cont_tables = []
        pclass_by_sample = np.zeros(shape=(len(fx_by_sample)),
          dtype=(np.float16))
    ...
    def get_bagging_other(other, num_query, random_state=np.random.RandomState()):
        ...
        return (
         other[ids], bag_num_query)

    @staticmethod
    def get_k_gaussian_kernel_for_all_x(row_data, x_ids, num_query, min_bw, bw=None, n_bagging=1, num_bs=0, random_state=np.random.RandomState()):
        x_values = row_data[x_ids]
        other = np.delete(row_data, x_ids)
        o_num_query = num_query - np.sum(x_ids < num_query)
        return [Classifier.get_k_gaussian_kernel(x, other, o_num_query, min_bw, bw, n_bagging, random_state, num_bs=num_bs) for x in x_values]

    @staticmethod
    def get_k_gaussian_kernel(x, other, num_query, min_bw, bw, n_bagging, random_state, num_bs=0):
        if n_bagging > 1:
            bag_others = [Classifier.get_bagging_other(other, num_query, random_state=random_state) for j in range(n_bagging)]
            return [Classifier.k_gaussian_kernel(x, other, min_bw, b_num_query, bw, num_bs=num_bs) for other, b_num_query in bag_others]
        return [Classifier.k_gaussian_kernel(x, other, min_bw, num_query, bw, num_bs=num_bs) for j in range(1)]
    ...
    def k_gaussian_kernel(x, other, min_bw, ids_split, bw, num_bs=0):
        ...
        return (
         np.concatenate((res_query, res_ref)), ids_split)

    @staticmethod
    def compute_normal_fx_for_all_x(row_data, x_ids, num_query, epsilon=0.001, n_bagging=1, num_bs=0, random_state=np.random.RandomState()):
        x_values = row_data[x_ids]
        other = np.delete(row_data, x_ids)
        o_num_query = num_query - np.sum(x_ids < num_query)
        return [Classifier.compute_normal_fx(x, other, o_num_query, epsilon, n_bagging, num_bs=num_bs, random_state=random_state) for x in x_values]

    @staticmethod
    def compute_normal_fx(x, other, num_query, epsilon, n_bagging, random_state, num_bs=0):
        if n_bagging > 1:
            bag_others = (Classifier.get_bagging_other(other, num_query, random_state=random_state) for j in range(n_bagging))
            return [Classifier.fx_normal(x, other, b_num_query, epsilon=epsilon, num_bs=num_bs) for other, b_num_query in bag_others]
        return [Classifier.fx_normal(x, other, num_query, epsilon=epsilon, num_bs=num_bs) for j in range(1)]
    ...
    def get_mcc_ped_by_bagging(ct_by_bagging):
        ...
        return (
         [
          all_mcc[first_quantile], mean_mcc,
