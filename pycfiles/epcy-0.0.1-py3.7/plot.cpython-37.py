...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/epcy/utils/plot.py
# Compiled at: 2020-03-19 17:34:47
# Size of source mod 2**32: 7092 bytes
...
def plot_explore_heatmap(df_heatmap, df_pred, args):
    ...
    sns_plot = sns.clustermap(df_heatmap,
      linewidths=0, metric='euclidean', xticklabels=True,
      yticklabels=True,
      vmin=(-1),
      vmax=1,
      row_cluster=False,
    ...
    sns_plot.ax_heatmap.set_xticklabels((sns_plot.ax_heatmap.get_xmajorticklabels()),
      fontsize=4)
    sns_plot.ax_heatmap.set_yticklabels((sns_plot.ax_heatmap.get_ymajorticklabels()),
      fontsize=4)
...
def plot_qc_histo(df_pred, quantiles, legend_quantile, mcc_bins, args):
    if not os.path.exists(args.PATH_OUT):
        ...
        if args.L2FC:
            ...
            plt.title('QC histogram of kernel_mcc colored by max of mean                    expression',
              fontsize=22)
    plt.xlabel(x_var)
    plt.ylabel('# features')
    ...
    if args.YLOG:
        ...
        x_var = 'kernel_mcc'
        plt.figure(figsize=(16, 9), dpi=150)
        n, bins, patches = plt.hist([df_pred['bw_query'], df_pred['bw_ref']], bins=100,
          color=['r', 'b'],
          label=[
         'Query', 'Ref'])
...
def plot_profile(id, query_exp, ref_exp, bw_query, bw_ref, args):
    df_swarn = pd.DataFrame(data={'x':np.append(query_exp, ref_exp), 
     'subgroup':np.append(np.repeat(args.QUERY, len(query_exp)), np.repeat('Other', len(ref_exp)))})
    ...
    sns_plot = sns.kdeplot(query_exp, shade=True, bw=bw_query, color=(col_pal[0]),
      label=(args.QUERY),
      ax=ax_kde)
    sns_plot = sns.kdeplot(ref_exp, shade=True, bw=bw_ref, color=(col_pal[1]), label='Other',
      ax=ax_kde)
    sns_plot.set_title(str(id) + ' ' + args.QUERY + '\nbw_query=' + str(bw_query) + '\nbw_ref=' + str(bw_ref))
    sns_plot = sns.swarmplot(x='x',
      y='subgroup',
      data=df_swarn,
      ax=ax_swarm,
