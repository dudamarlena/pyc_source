...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/python/reloader.py
...
class ImportRemapData(object):
    ...
    def __init__(self, dest_module_name, dest_attr_name, source_module_name, source_attr_name, attr_type):
...
def do_reload(module, remapping=False):
    ...
    for attrName in attr_names:
        ...
        if not attr_module:
            debug_log(dbg_phase_title,
              '"', attrName, '"(attr): cannot get module for attribute. Skipped.', verbosity=2)
        else:
            debug_log(dbg_phase_title,
              '"', attrName, '"(attr): attribute belongs to module "', (attr_module.__name__),
              '"', verbosity=2)
            ...
            elif not hasattr(attr_module, '__file__'):
                debug_log(dbg_phase_title,
                  '"', (attr_module.__name__), '"(module): no "__file__" attribute. Skipped.', verbosity=2)
            elif attr_module == sys.modules[__name__]:
                debug_log(dbg_phase_title,
                  '"', (attr_module.__name__), '"(module): ', SCRIPT_NAME, ' itself. Skipped.',
                  verbosity=2)
            elif attr_module in _processed_modules:
                debug_log(dbg_phase_title,
                  '"', (attr_module.__name__), '"(module): already processed. Skipped.', verbosity=2)
            elif need_to_be_ignored(attr_module.__file__.lower()):
                debug_log(dbg_phase_title, '"', (attr_module.__name__), '"(module): in ignore list. Skipped.', verbosity=2)
            else:
                detected_modules.add(attr_module)
                debug_log(dbg_phase_title,
                  '"', (attr_module.__name__), '"(module): added to list of detected modules.', verbosity=2)
                if remapping:
                    if not hasattr(attr_val, '__name__'):
                        ...
                        if attr_module != module:
                            debug_log((' '.join((dbg_phase_title + 'preparing mapping data:', module.__name__, attrName, '<-',
                             attr_module.__name__, orig_name, str(type(attr_val))))),
                              verbosity=2)
                            remap_data = ImportRemapData(module.__name__, attrName, attr_module.__name__, orig_name, type(attr_val))
    ...
    if _verbosity > 0:
        ...
        for detectedModule in detected_modules:
            if detectedModule in _reloaded_modules:
                debug_log(dbg_phase_title,
                  ('module "' + detectedModule.__name__ + '" was already reloaded. Skipping recursion.'),
                  verbosity=1)
            else:
                debug_log(dbg_phase_title,
                  ('module "' + detectedModule.__name__ + '" is not analyzed. Entering recursion.'),
                  verbosity=1)
