...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/python/python.py
# Compiled at: 2020-04-11 22:12:39
# Size of source mod 2**32: 21715 bytes
"""
Modules that contains utility functions related with Python
"""
from __future__ import print_function, division, absolute_import
import os, sys, imp, ast, uuid, time, types, inspect, traceback, collections
...
def get_version():
    ...
    current_version = version.SemanticVersion(major=(py_version.major),
...
def force_sequence(var, sequence_type=list):
    ...
    if type(var) == tuple:
        if sequence_type == list:
            var = list(var)
        return type(var) == sequence_type or sequence_type(var)
    else:
        return var
...
def from_list_to_nested_dict(input_arg, separator='/'):
    ...
    if not isinstance(input_arg, (list, tuple, set)):
        raise TypeError('Input argument "input_arg" should be a list or tuple or set, but get {}'.format(type(input_arg)))
    ...
    for item in input_arg:
        ...
        for i, comp in enumerate(components):
            ...
            if action_comp is None:
                action_comp = {'value':comp, 
                 'label':comp,  'children':list()}
