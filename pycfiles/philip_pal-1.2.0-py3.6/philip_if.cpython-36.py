...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/philip_pal/philip_if.py
# Compiled at: 2020-03-26 05:07:50
# Size of source mod 2**32: 24464 bytes
"""Interfaces for PHiLIP in philip_pal
This handles the basic protocol interface to Philip and extended helpers for
Philip.
"""
import logging, errno, os, json, csv, time
...
class PhilipBaseIf:
    __doc__ = 'Handles basic functions and commands for memory map interface\n\n    Args:\n        (*args, **kwargs) -> See base_device for documentation of args\n    '
    ...
    def read_bytes(self, index, size=1, to_byte_array=False, timeout=None):
        ...
        """
        return self.send_and_parse_cmd('rr {} {}'.format(index, size), to_byte_array, timeout)
    ...
    def read_bits(self, index, offset, bit_amount, timeout=None):
        ...
        if cmd_info['result'] == self.RESULT_SUCCESS:
            cmd_info['cmd'] = ['read_bits({},{},{})'.format(index, offset, bit_amount),
             cmd_info['cmd']]
        ...
        if isinstance(data, list):
            for data_byte in data:
                for i in range(0, size):
                    ret_str += ' {}'.format(int(data_byte) >> i * 8 & 255)

        else:
            for i in range(0, size):
                ret_str += ' {}'.format(int(data) >> i * 8 & 255)
    ...
    def write_bytes(self, index, data, size=1, timeout=None):
        ...
        Returns:
            see send_and_parse_cmd()
        """
        cmd = 'wr {}{}'.format(index, self._write_byte_arg_to_string(data, size))
    ...
    def write_bits(self, index, offset, bit_amount, data, timeout=None):
        ...
        """
        cmds = [
         'write_bits({},{},{},{})'.format(index, offset, bit_amount, data)]
        bit_amount = int(bit_amount)
        offset = int(offset)
        index = int(index)
        bytes_to_read = int((bit_amount - 1 + offset) / 8 + 1)
        ...
        if cmd_info['result'] != self.RESULT_SUCCESS:
            ...
            cmds.append(cmd_info['cmd'])
            if cmd_info['result'] == self.RESULT_SUCCESS:
                cmd_info['cmd'] = 'write_bits {} {} {} {}'.format(index, offset, bit_amount, data)
            cmd_info['cmd'] = cmds
            return cmd_info
...
class PhilipExtIf(PhilipBaseIf):
    ...
    def scan_dev_map(self):
        ...
        """
        prim_types = {'uint8_t':1, 
         'int8_t':1,  'uint16_t':2,  'int16_t':2,  'uint32_t':4, 
         'int32_t':4,  'uint64_t':8,  'int64_t':8,  'char':1, 
         'float':4,  'double':8}
    ...
    def _parse_array(data, type_size, prim_type):
        ...
        try:
            ...
            for i in range(0, elements):
                num = int.from_bytes((data[i * type_size:(i + 1) * type_size]), byteorder='little')
    ...
    def read_reg(self, cmd_name, offset=0, size=None, to_byte_array=False, timeout=None):
        ...
        if cmd['bits'] != '':
            response = self.read_bits((cmd['offset']), (cmd['bit_offset']),
              (cmd['bits']),
              timeout=timeout)
        else:
            if cmd['total_size'] != '':
                ...
                response['data'] = self._parse_array(response['data'], cmd['type_size'], cmd['type'])
            else:
                response = self.read_bytes(cmd['offset'], size, to_byte_array)
        if 'data' in response:
            response['data'] = self._c_cast(response['data'], cmd['type'])
        response['cmd'] = [
         'read_reg({},{},{})'.format(cmd_name, offset, size),
         response['cmd']]
        return response
    ...
    def write_reg(self, cmd_name, data, offset=0, timeout=None):
        ...
        if cmd['bits'] != '':
            ...
            if cmd['total_size'] != '':
                ...
                offset *= int(cmd['type_size'])
                offset += int(cmd['offset'])
                response = self.write_bytes(offset, data, (int(cmd['type_size'])),
                  timeout=timeout)
            else:
                response = self.write_bytes((cmd['offset']), data, (int(cmd['type_size'])),
                  timeout=timeout)
        response['cmd'] = [
         'write_reg({},{},{})'.format(cmd_name, data, offset),
         response['cmd']]
        return response
    ...
    def dut_reset(self, reset_time=0.3, timeout=None):
        ...
        response = []
        response.extend(self.write_and_execute('sys.mode.dut_rst', 1, timeout=timeout))
        time.sleep(reset_time)
        response.extend(self.write_and_execute('sys.mode.dut_rst', 0, timeout=timeout))
        return response
    ...
    def _get_trace_events(self, trace, index, chunk_size, to_ns):
        logging.debug('_get_trace_events(trace=?, index=%r, chunk_size=%r, to_ns=%r)', index, chunk_size, to_ns)
        trace_tick_divs = self.read_reg('trace.tick_div', index, chunk_size)['data']
        trace_sources = self.read_reg('trace.source', index, chunk_size)['data']
        trace_ticks = self.read_reg('trace.tick', index, chunk_size)['data']
        trace_values = self.read_reg('trace.value', index, chunk_size)['data']
