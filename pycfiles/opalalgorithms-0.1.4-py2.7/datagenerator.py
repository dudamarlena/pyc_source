...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/opalalgorithms/utils/datagenerator.py
...
class OPALDataGenerator(object):
    ...
    def __init__(self, num_antennas, num_antennas_per_user, num_records_per_user, bandicoot_extended=True):
    ...
    def generate_data(self):
        ...
        antennas = [ str(random.randint(0, self.num_antennas - 1)) for i in range(self.num_antennas_per_user)
                   ]
        latitude = [ str(round(random.random() * 90, 6)) for i in antennas
                   ]
        longitude = [ str(round(random.random() * 180, 6)) for i in antennas
                    ]
        location = [ self.__levels[random.randint(0, 15)] for i in antennas
                   ]
        users = [ random.choice(string.ascii_letters) + random.choice(string.ascii_letters) for j in range(20)
                ]
        lines = []
        if self.bandicoot_extended:
            lines.append('interaction,direction,correspondent_id,datetime,call_duration,antenna_id,latitude, longitude,location_level_1,location_level_2')
        else:
            lines.append('interaction,direction,correspondent_id,datetime,call_duration,antenna_id')
        date_props = [ random.random() for i in range(self.num_records_per_user)
                     ]
        date_props.sort()
        for k in range(self.num_records_per_user):
            lines.append(self.__generate_single_line(antennas, users, date_props[k], latitude, longitude, location))

        return ('\n').join(lines) + '\n'

    def __generate_single_line(self, antennas, users, date_prop, latitude, longitude, location):
        ...
        date = self.__random_date('2016-01-01 00:00:01', '2016-12-31 23:59:59', date_prop)
        call_length = ''
        if interaction == 'call':
            call_length = str(random.randint(0, self.num_antennas - 1))
