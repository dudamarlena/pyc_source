...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/opalalgorithms/utils/algorithmrunner.py
# Compiled at: 2018-04-24 11:50:48
"""Given an algorithm object, run the algorithm."""
from __future__ import division, print_function
import multiprocessing as mp, random, time, os, math, six, codejail
from codejail.safe_exec import not_safe_exec
from codejail.limits import set_limit
import textwrap, signal, sys
__all__ = [
 'AlgorithmRunner']
...
def check_environ():
    ...
    req_environ_vars = [
     'OPALALGO_SANDBOX_VENV', 'OPALALGO_SANDBOX_USER']
...
def get_jail():
    ...
    codejail.configure('python', os.path.join(sandbox_env, 'bin', 'python'), user=sandbox_user)
    codejail.configure('python3', os.path.join(sandbox_env, 'bin', 'python'), user=sandbox_user)
...
def process_user_csv(params, user_csv_file, algorithm, dev_mode, sandboxing, jail):
...
def process(params, users_csv_files, algorithm, dev_mode, sandboxing):
    ...
    for user_csv_file in users_csv_files:
        yield process_user_csv(params, user_csv_file, algorithm, dev_mode, sandboxing, jail)


def mapper(writing_queue, params, users_csv_files, algorithm, dev_mode=False, sandboxing=True):
    ...
    Args:
        ...
        sandboxing (bool): Should sandboxing be used or not.
    """
    for result in process(params, users_csv_files, algorithm, dev_mode, sandboxing):
        if result:
...
class AlgorithmRunner(object):
    ...
    def __init__(self, algorithm, dev_mode=False, multiprocess=True, sandboxing=True):
        """Initialize class."""
    ...
    def __call__(self, params, data_dir, num_threads):
        ...
        csv_files = [ os.path.join(os.path.abspath(data_dir), f) for f in os.listdir(data_dir) if f.endswith('.csv')
                    ]
    ...
    def _multiprocess(self, params, num_threads, csv_files):
        ...
        chunks_list = [ csv_files[j:min(j + step_size, len(csv_files))] for j in range(0, len(csv_files), step_size)
                      ]
        manager = mp.Manager()
    ...
    def _singleprocess(self, params, csv_files):
        for result in process(params, csv_files, self.algorithm, self.dev_mode, self.sandboxing):
