...
# Embedded file name: build/bdist.macosx-10.8-x86_64/egg/pypi_rankings/webserver.py
# Compiled at: 2015-06-10 04:48:07
...
def get_downloads_data(download_key='downloads_total'):
    res = conn.execute(('SELECT name, summary, {0}, python3 FROM packages WHERE {0} IS NOT NULL ORDER BY {0} DESC').format(download_key))
    for rank, (package, summary, download_count, py3) in enumerate(res, 1):
        yield (
         rank, package, summary, download_count, py3)


def get_dependency_data():
    res = conn.execute('SELECT dependency, COUNT(name) FROM dependencies WHERE dependency IS NOT NULL GROUP BY dependency ORDER BY COUNT(name) DESC')
...
def get_author_data():
    res = conn.execute("SELECT author, COUNT(author) FROM packages WHERE author NOT IN ('', 'UNKNOWN', 'AUTHOR') GROUP BY author ORDER BY COUNT(author) DESC")
...
def format_data_for_table(data, data_type='package'):
    ...
    format_package = lambda key, desc: ('<a href="/{0}/{1}">{1}</a>').format(data_type, key) + ('<br><div class="package-description">{0}</div>').format(desc)
    format_py3 = lambda py3: '<div class="three noselect">3</div>' if py3 else ''
    format_value = lambda dc: ('{:,d}').format(dc)
    filtered_data = [ (rank, format_package(package, summary), format_value(value), format_py3(py3)) for rank, package, summary, value, py3 in data if search.lower() in package.lower()
                    ]
...
def more_stats():
    ...
    return render_template('more_stats.html', scm=scm, len_py3=len_py3, total_len=total_len, no_sdist=no_sdist, no_releases=no_releases, no_downloads=no_downloads, dependency_data=dependency_data, len_scm=len_scm, authors_len=authors_len)
...
def package(package_name):
    ...
    versions = list(conn.execute('SELECT * FROM versions WHERE package=? ORDER BY upload_time DESC, version DESC', (
     package_name,)))
    ...
    dependencies = list(conn.execute('SELECT dependencies.dependency, packages.summary FROM dependencies JOIN packages ON packages.name=dependencies.dependency WHERE dependencies.dependency IS NOT NULL AND dependencies.name=?', (
     package_name,)))
    has_dependency_data = len(list(conn.execute('SELECT raw_dependencies FROM dependencies WHERE name=? AND raw_dependencies IS NOT NULL', (
     package_name,)))) > 0
    show_all_packages = bool(request.args.get('show_all'))
    scm = list(conn.execute('SELECT * FROM scm WHERE name=?', (package_name,)))

    def dependent_packages():
        res = list(conn.execute('SELECT dependencies.name, packages.summary FROM dependencies JOIN packages ON packages.name=dependencies.name WHERE dependencies.dependency=?', (
         package_name,)))
        if len(res) > 10 and not show_all_packages:
            res[10] = (
             ('... {} more').format(len(res) - 10), None)
            res = res[:11]
        return res
    ...
    return render_template('package.html', last_update=last_update, rank=rank, format_time=format_time, has_dependency_data=has_dependency_data, dependencies=dependencies, dependent_packages=dependent_packages(), len=len, release_history=versions, scm=scm, get_days_since=get_days_since, **package_data)
...
@app.route('/')
def index():
    kwargs = dict(search=request.args.get('search'), length=request.args.get('length'), start=request.args.get('start'), source=request.args.get('source', 'downloads_total'))
