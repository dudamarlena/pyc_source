...
# Embedded file name: build/bdist.macosx-10.8-x86_64/egg/pypi_rankings/crawlers/pypi.py
# Compiled at: 2016-01-29 09:04:05
...
def per_package(package):
    try:
        ...
        return dict(downloads=downloads, python3=python3, author=d['info']['author'], crawl_time=time.time(), release_history=release_history, summary=d['info']['summary'], homepage=d['info']['home_page'], latest_sdist=latest_sdist)
    except ValueError:
        return
...
def save_package_data(conn, package, package_data):
    ...
    conn.execute(('REPLACE INTO packages ({}) VALUES ({})').format((', ').join(data_to_insert.keys()), ('?,' * len(data_to_insert))[:-1]), data_to_insert.values())
    conn.commit()
    return
