...
# Embedded file name: build/bdist.linux-i686/egg/collective/portlet/googleapps/portlets/googledocsportlet.py
# Compiled at: 2009-08-13 00:05:53
...
class IGoogleDocsPortlet(IPortletDataProvider):
    ...
    """
    __module__ = __name__
    header = schema.TextLine(title=_('Portlet header'), description=_('Title of the rendered portlet'), required=True, default=_('My Google Docs'))
    limit = schema.Int(title=_('Limit'), description=_('Specify the maximum number of items to show in the portlet. '), required=True, default=5)
...
class Assignment(base.Assignment):
    ...
    def __init__(self, header='My Google Docs', limit=0, hosted_domain='', secure_connection=True, portlet_height='160', portlet_width='100%'):
        self.header = header
        self.limit = limit
...
class EditForm(base.EditForm):
    ...
    __module__ = __name__
    form_fields = form.Fields(IGoogleDocsPortlet)
    label = _('Edit Google Docs Portlet')
    description = _("A portlet which displays a listing of a user's Google Documents.")