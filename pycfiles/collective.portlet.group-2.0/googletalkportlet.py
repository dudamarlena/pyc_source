...
# Embedded file name: build/bdist.linux-i686/egg/collective/portlet/googleapps/portlets/googletalkportlet.py
# Compiled at: 2009-08-12 23:13:36
...
class IGoogleTalkPortlet(IPortletDataProvider):
    """A portlet
    ...
    """
    __module__ = __name__
    header = schema.TextLine(title=_('Portlet header'), description=_('Title of the rendered portlet'), required=True, default=_('My Google Talk'))
...
class Assignment(base.Assignment):
    ...
    """
    __module__ = __name__
    implements(IGoogleTalkPortlet)
    header = 'My Google Talk'
    hosted_domain = ''
    secure_connection = True
    portlet_height = '160'
    portlet_width = '100%'

    def __init__(self, header='My Google Talk', hosted_domain='', secure_connection=True, portlet_height='160', portlet_width='100%'):
        self.header = header
...
class Renderer(base.Renderer):
    ...
    """
    __module__ = __name__
    render = ViewPageTemplateFile('googletalkportlet.pt')

    def __init__(self, *args):
        base.Renderer.__init__(self, *args)

    @memoize
    def google_talk_iframe_url(self):
        url = 'http' + (self.data.secure_connection and 's' or '') + '://' + (self.data.hosted_domain and 'hosted' or '') + 'talkgadget.google.com'
        if self.data.hosted_domain:
            url += '/a/' + self.data.hosted_domain
        url += '/talkgadget/client'
        return url
...
class AddForm(base.AddForm):
    ...
    """
    __module__ = __name__
    form_fields = form.Fields(IGoogleTalkPortlet)
...
class EditForm(base.EditForm):
    ...
    __module__ = __name__
    form_fields = form.Fields(IGoogleTalkPortlet)
    label = _('Edit Google Talk Portlet')
    description = _('A portlet which offers access to Google Talk (Jabber/XMPP)')