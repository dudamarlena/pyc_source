...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/go_http/contacts.py
...
class ContactsApiClient(object):
    ...
    def _api_request(self, method, api_collection, api_path, data=None, params=None):
    ...
    def contacts(self, start_cursor=None):
        ...
        """
        if start_cursor:
            page = self._api_request('GET', 'contacts', '?cursor=%s' % start_cursor)
        else:
        ...
        while True:
            ...
            if page['cursor'] is None:
                break
            try:
                page = self._api_request('GET', 'contacts', '?cursor=%s' % page['cursor'])
    ...
    def _contact_by_field(self, field, value):
        contact = self._api_request('GET', 'contacts', '', params={'query': '%s=%s' % (field, value)})
    ...
    def get_contact(self, *args, **kw):
        ...
        raise ValueError('get_contact may either be called as .get_contact(contact_key) or .get_contact(field=value)')
    ...
    def group_contacts(self, group_key, start_cursor=None):
        ...
        """
        if start_cursor:
            page = self._api_request('GET', 'groups/%s' % group_key, 'contacts?cursor=%s' % start_cursor)
        else:
            page = self._api_request('GET', 'groups/%s' % group_key, 'contacts')
        while True:
            for contact in page['data']:
                yield contact
            ...
            if page['cursor'] is None:
                break
            try:
                page = self._api_request('GET', 'groups/%s' % group_key, 'contacts?cursor=%s' % page['cursor'])
            except Exception as err:
                raise PagedException(page['cursor'], err)
