...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/go_http/send.py
# Compiled at: 2017-02-17 10:13:07
""" Simple utilities for sending messages via Vumi Go' HTTP API.
"""
import json, logging, pprint, uuid, requests
...
class HttpApiSender(object):
    ...
    def __init__(self, account_key, conversation_key, conversation_token, api_url=None, session=None):
    ...
    def _raw_send(self, data):
        try:
            ...
            if e.response.status_code != 400 or 'opted out' not in response.get('reason', '') or response.get('success'):
                raise e
            raise UserOptedOutException(data.get('to_addr'), data.get('content'), response.get('reason'))
    ...
    def send_voice(self, to_addr, content, speech_url=None, wait_for=None, session_event=None):
...
class LoggingSender(HttpApiSender):
    ...
    def _handle_metrics(self, data):
        for metric, value, agg in data:
            ...
            self._logger.log(self._level, 'Metric: %r [%s] -> %g' % (
