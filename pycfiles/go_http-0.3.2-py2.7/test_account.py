...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/go_http/tests/test_account.py
...
class FakeAccountApi(object):
    ...
    def jsonrpc_error_response(self, fault, fault_code, fault_string):
        return Resp(json.dumps({'error': {'fault': fault, 
                     'faultCode': fault_code, 'faultString': fault_string}}), 200, headers={})

    def jsonrpc_success_response(self, result):
        return Resp(json.dumps({'error': None, 
           'result': result}), 200, headers={})
    ...
    def handle_request(self, request):
        if request.headers['Authorization'] != 'Bearer %s' % self.auth_token:
            ...
            if request.method != 'POST':
                return self.jsonrpc_error_response('Fault', 8000, 'Only POST method supported')
...
class TestAccountApiClient(TestCase):
    ...
    def make_client(self, auth_token=AUTH_TOKEN):
        return AccountApiClient(auth_token, api_url=self.API_URL, session=self.session)
    ...
    def test_jsonrpc_error_handling(self):
        ...
        self.account_backend.add_error_response('campaigns', [], fault='Fault', fault_code=8002, fault_string='Meep')
    ...
    def test_campaigns(self):
        client = self.make_client()
        self.account_backend.add_success_response('campaigns', [], fixtures.campaigns)
        self.assertEqual(client.campaigns(), fixtures.campaigns)

    def test_conversations(self):
        client = self.make_client()
        self.account_backend.add_success_response('conversations', ['campaign-1'], fixtures.conversations)
        self.assertEqual(client.conversations('campaign-1'), fixtures.conversations)

    def test_channels(self):
        client = self.make_client()
        self.account_backend.add_success_response('channels', ['campaign-1'], fixtures.channels)
        self.assertEqual(client.channels('campaign-1'), fixtures.channels)

    def test_routers(self):
        client = self.make_client()
        self.account_backend.add_success_response('routers', ['campaign-1'], fixtures.routers)
        self.assertEqual(client.routers('campaign-1'), fixtures.routers)

    def test_routing_entries(self):
        client = self.make_client()
        self.account_backend.add_success_response('routing_entries', ['campaign-1'], fixtures.routing_entries)
        self.assertEqual(client.routing_entries('campaign-1'), fixtures.routing_entries)

    def test_routing_table(self):
        client = self.make_client()
        self.account_backend.add_success_response('routing_table', ['campaign-1'], fixtures.routing_table)
        self.assertEqual(client.routing_table('campaign-1'), fixtures.routing_table)

    def test_update_routing_tabel(self):
        client = self.make_client()
        self.account_backend.add_success_response('update_routing_table', ['campaign-1', fixtures.routing_table], None)
        self.assertEqual(client.update_routing_table('campaign-1', fixtures.routing_table), None)
        return