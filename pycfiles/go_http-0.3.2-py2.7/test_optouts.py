...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/go_http/tests/test_optouts.py
...
class TestOptOutsApiClient(TestCase):
    ...
    def setUp(self):
        ...
        self.client = OptOutsApiClient(auth_token='auth-token', api_url='http://example.com/api/v1/go', session=self.session)

    def response_ok(self, data):
        data.update({'status': {'reason': 'OK', 
                      'code': 200}})
        return data

    def test_default_session(self):
        import requests
        client = OptOutsApiClient(auth_token='auth-token')
    ...
    def test_error_response(self):
        ...
        self.session.mount('http://example.com/api/v1/go/optouts/msisdn/%2b1234', adapter)
        self.assertRaises(HTTPError, self.client.get_optout, 'msisdn', '+1234')
        self.check_request(adapter.request, 'GET', headers={'Authorization': 'Bearer auth-token'})
    ...
    def test_get_optout(self):
        opt_out = {'created_at': '2015-11-10 20:33:03.742409', 
           'message': None, 
           'user_account': 'fxxxeee'}
        response = self.response_ok({'opt_out': opt_out})
    ...
    def test_get_optout_not_found(self):
        response = {'status': {'reason': 'Opt out not found', 
                      'code': 404}}
    ...
    def test_set_optout(self):
        ...
        self.assertEqual(result, opt_out)
        self.check_request(adapter.request, 'PUT', headers={'Authorization': 'Bearer auth-token'})
        return
    ...
    def test_delete_optout(self):
        ...
        self.assertEqual(result, opt_out)
        self.check_request(adapter.request, 'DELETE', headers={'Authorization': 'Bearer auth-token'})
        return
    ...
    def test_count(self):
        response = self.response_ok({'opt_out_count': 2})
        adapter = RecordingAdapter(json.dumps(response))
        self.session.mount('http://example.com/api/v1/go/optouts/count', adapter)
