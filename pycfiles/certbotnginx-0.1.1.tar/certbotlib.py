...
# Embedded file name: build/bdist.macosx-10.12-intel/egg/certbotlib/certbotlib.py
# Compiled at: 2017-10-06 09:15:09
...
class Certbot(object):
    ...
    def __init__(self, email, logs_dir, work_dir, config_dir, domain, region, profile):
        """
    ...
    def _parse(self):
        ...
        process = subprocess.Popen(self.cmd, shell=False, bufsize=0, stdin=subprocess.PIPE, stderr=subprocess.PIPE, stdout=subprocess.PIPE)
    ...
    def _parse_after_input(self, result):
        ...
        for line in result[0].split('\n'):
            ...
            if self._domain in line:
                ...
                self.logger.info(('Certificates created.Path: {}').format(self.certs_dir))
            else:
    ...
    def create_aws_dns(self):
        ...
        change_batch = {'Changes': [
                     {'Action': 'UPSERT', 'ResourceRecordSet': {'Name': self.acme, 'Type': 'TXT', 
                                              'TTL': 300, 
                                              'ResourceRecords': [{'Value': ('"{txt_value}"').format(txt_value=self.key)}]}}]}
        response = self.route.change_domain_by_zone(zone=self.__get_public_zone()[0].zone_id, domain_details=change_batch)
    ...
    def __get_public_zone(self):
        ...
        """
        zone = [ zone for zone in self.route.get_zones() if not zone.is_private and zone.name[:-1] in self._domain
               ]
        return zone
    ...
    def upload_certificates(self, cert, key, chain):
        ...
        response = self.acm.import_certificate(certificate=cert.read(), private_key=key.read(), certificate_chain=chain.read(), **kwargs)
        self.logger.debug(response)
