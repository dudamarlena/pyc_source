...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sparts/vtask.py
...
class VTask(_SpartsObject):
    ...
    def initTask(self):
        ...
        if not self.LOOPLESS:
            for i in xrange(self.workers):
                ...
                self.threads.append(threading.Thread(target=self._run, name=name))
    ...
    def _run(self):
        try:
            ...
            self.logger.debug('Thread %s exited', threading.currentThread().name)
    ...
    def getTaskOption(self, opt, default=None):
        return getattr(self.service.options, self._optName(opt), default)
...
class Tasks(object):
    ...
    def init(self):
        ...
        if len(exceptions):
            raise Exception('Unable to start service (%d task start errors)' % len(exceptions))
