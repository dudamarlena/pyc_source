...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sparts/counters.py
# Compiled at: 2015-02-07 04:24:12
...
_SampleMethod = {SampleType.COUNT: Count, 
   SampleType.SUM: Sum, 
...
class Samples(_Nameable, _Bindable, ProvidesCounters):
    ...
    def _bind(self, obj):
        return self.__class__(types=self.types, windows=self.windows, name=self.name)

    def _genCounterCallbacks(self):
        """Yield all the child counters."""
        for subcounter in self.iterkeys():
            yield (
             subcounter, partial(self.getCounter, subcounter))
    ...
    def getCounters(self):
        ...
        def _saveCounterValues(window):
            ...
            if self.name is not None:
                prefix = self.name + '.'
            for op in ops:
                result[prefix + op.suffix + '.' + str(window)] = op.getvalue()

            try:
                return (
                 next(genwindows), False)
