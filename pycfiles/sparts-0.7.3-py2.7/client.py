...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sparts/thrift/client.py
# Compiled at: 2015-08-18 20:22:56
...
class ThriftClient(object):
    ...
    def __init__(self, host=None, port=None, module=None, lazy=True, connect_timeout=None, transport_class=None, protocol_class=None, path=None, multiplex_service=None):
        ...
        self._initAttribute('connect_timeout', connect_timeout, self.CONNECT_TIMEOUT)
        self._initAttribute('multiplex_service', multiplex_service, self.MULTIPLEX_SERVICE)
        if self.path is None:
            fallback_transport = TFramedTransport
        else:
            fallback_transport = TBufferedTransport
        self._initAttribute('transport_class', transport_class, self.TRANSPORT_CLASS, fallback_transport)
        self._initAttribute('protocol_class', protocol_class, self.PROTOCOL_CLASS)
        self.lazy = lazy
        assert self.module is not None, 'You must define a thrift module!'
        assert hasattr(self.module, 'Client'), "Module %s requires 'Client' class." % self.module
        if self.lazy:
            self._client = None
        else:
            self._connect()
    ...
    def _connect(self):
        ...
        if self.multiplex_service is not None:
            self._protocol = TMultiplexedProtocol(self._protocol, self.multiplex_service)
    ...
    def _makeConnectURI(self):
        ...
        host = self.host
        if ':' in host:
            assert sys.version >= '2.7', 'Native v6 IP URLs not parseable on Python < 2.7'
            host = ('[{host}]').format(host=host)
        return ('http://{host}:{port}{path}').format(host=host, port=self.port, path=self.path)
