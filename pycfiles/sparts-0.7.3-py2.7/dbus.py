...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sparts/tasks/dbus.py
# Compiled at: 2015-04-03 22:57:34
...
import dbus, dbus.service, gobject, glib, time
...
class VServiceDBusObject(dbus.service.Object):
    ...
    @dbus.service.method(dbus_interface='org.sparts.Service', in_signature='s', out_signature='v')
    def getOption(self, name):
        return self.service.getOption(name)

    @dbus.service.method(dbus_interface='org.sparts.Service', in_signature='sv', out_signature='')
    ...
    @dbus.service.method(dbus_interface='org.sparts.Service', in_signature='', out_signature='a{sv}')
    def getOptions(self):
        result = {}
    ...
    @dbus.service.method(dbus_interface='org.sparts.Service', in_signature='', out_signature='as')
    def listOptions(self):
        return self.service.getOptions().keys()

    @dbus.service.method(dbus_interface='org.sparts.Service', in_signature='', out_signature='')
    def shutdown(self):
    ...
    @dbus.service.method(dbus_interface='org.sparts.Service', in_signature='', out_signature='x')
    def uptime(self):
...
class DBusServiceTask(DBusTask):
    ...
    bus_name = option(default=lambda cls: cls.BUS_NAME, metavar='NAME', help='Bus Name.  Should be something like "com.sparts.AwesomeService"')
    replace = option(action='store_true', type=bool, default=False, help='Replace, and enable replacing of this service')
    queue = option(action='store_true', type=bool, default=False, help='If not --{task}-replace, will wait to take this bus name')
    system_bus = option(action='store_true', type=bool, default=lambda cls: cls.USE_SYSTEM_BUS, help='Use system bus')
    dbus_service = None

    def initTask(self):
        super(DBusServiceTask, self).initTask()
        assert self.bus_name is not None, 'You must pass a --{task}-bus-name'
        return
    ...
    def _asyncStartCb(self):
        ...
        self.dbus_service = dbus.service.BusName(self.bus_name, self.bus, self.replace, self.replace, self.queue)
        return True
    ...
    def addHandlers(self):
        ...
        if HAVE_FB303:
            ...
            if task is not None:
                self.fb303_dbus = FB303DbusService(self.dbus_service, task, self.service.name)
        return
