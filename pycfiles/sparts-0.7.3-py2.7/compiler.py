...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sparts/thrift/compiler.py
# Compiled at: 2015-08-19 13:10:35
"""Tools for dynamically compiling and loading thrift code."""
import distutils.spawn, imp, os.path, tempfile
...
def _require_executable(name, fallback):
    ...
    assert path is not None, 'Unable to find %s or %s in PATH' % (name, fallback)
    return path
...
class CompileContext(object):
    ...
    def getThriftOptions(self, new_style=True, twisted=False, tornado=False, utf8strings=False, slots=True, dynamic=False, dynbase=None, dynexc=None, dynimport=None):
        param = 'py'
        options = []
        if new_style:
    ...
    def importThrift(self, path, **kwargs):
        ...
        check_output(args)
        args = [
         self.thrift_bin] + self.makeIncludeArgs(srcdir) + [
            ...
            for dirpath, dirnames, filenames in os.walk(outdir.join('gen-py')):
                ...
                with ctx.add_path(dirpath):
                    ...
                    if 'constants.py' in filenames:
                        result = self.mergeModules(self.importPython(dirpath + '/constants.py'), result)
