...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sparts/tasks/select.py
# Compiled at: 2015-04-11 03:31:00
...
import logging, os, select, six, sys
...
class SelectTask(VTask):
    ...
    def _select(self):
        return select.select(self._rcallbacks.keys(), self._wcallbacks.keys(), self._xcallbacks.keys())
...
class ProcessStreamHandler(object):
    ...
    def __init__(self, popen, select_task, on_stdout=None, on_stderr=None, on_exit=None, encoding=None):
...
class ProcessCommunicateHandler(object):
    ...
    def __init__(self, popen, select_task):
        ...
        self.future = Future()
        ProcessStreamHandler(popen, select_task, on_stdout=self._stdout_data.append, on_stderr=self._stderr_data.append, on_exit=self._on_exit)
    ...
    def _on_exit(self, returncode):
        ...
        if returncode == 0:
            self.future.set_result(ProcessResult(('').join(self._stdout_data), ('').join(self._stderr_data), returncode))
        else:
            self.future.set_exception(ProcessFailed(('').join(self._stdout_data), ('').join(self._stderr_data), returncode))