...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sparts/tasks/queue.py
# Compiled at: 2015-08-18 19:33:25
...
class QueueTask(VTask):
    ...
    MAX_ITEMS = 0
    WORKERS = 1
    max_items = option(type=int, default=lambda cls: cls.MAX_ITEMS, help='Set a bounded queue length.  This may cause unexpected deadlocks. [%(default)s]')
    workers = option(type=int, default=lambda cls: cls.WORKERS, help='Number of threads to spawn to work on items from its queue. [%(default)s]')
    ...
    def initTask(self):
        ...
        self.counters['queue_depth'] = CallbackCounter(lambda : self.queue.qsize())
