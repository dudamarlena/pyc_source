...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.4-intel/egg/tumbledore/admin.py
# Compiled at: 2012-09-18 19:24:54
...
class TumblelogAdmin(admin.ModelAdmin):
    list_display = ('__unicode__', 'mount_on', 'theme', 'is_active', 'sort_posts_by',
                    'created_at')
...
class TumblelogPostAdmin(admin.ModelAdmin):
    list_display = ('title', 'author', 'tumblelog', 'is_published', 'is_sticky', 'created_at',
                    'published_at')
    ...
    fieldsets = (
     (
      None,
      {'fields': ('title', 'slug', 'tumblelog', 'author', 'content', 'is_published', 'is_sticky', 'has_permalink',
 'published_at')}),
     (
      'Advanced options',
      {'classes': ('collapse', ), 
