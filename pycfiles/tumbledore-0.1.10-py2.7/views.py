...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.4-intel/egg/tumbledore/views.py
...
def index(request, mount_point):
    ...
    return render(request, '%s/index.html' % tumblelog.theme, {'tumblelog': tumblelog, 
...
def post(request, mount_point, slug):
    ...
    return render(request, '%s/post.html' % tumblelog.theme, {'tumblelog': tumblelog, 
       'post': post, 
       'widgets': _get_widgets_for(tumblelog)})
