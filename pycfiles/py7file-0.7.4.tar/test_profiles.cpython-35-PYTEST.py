...
class AtmosProfileTests(unittest.TestCase):
    ...
    def test_from_lat_and_date(self):
        ...
        if not @py_assert1:
            @py_format10 = @pytest_ar._call_reprcompare(('==', ), (@py_assert1,), ('%(py0)s == %(py9)s\n{%(py9)s = %(py4)s\n{%(py4)s = %(py2)s.PredefinedType\n}(%(py7)s\n{%(py7)s = %(py5)s.SubarcticSummer\n})\n}', ), (ap, @py_assert8)) % {'py7': @pytest_ar._saferepr(@py_assert6), 'py2': @pytest_ar._saferepr(AtmosProfile) if 'AtmosProfile' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(AtmosProfile) else 'AtmosProfile', 'py4': @pytest_ar._saferepr(@py_assert3), 'py0': @pytest_ar._saferepr(ap) if 'ap' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(ap) else 'ap', 'py5': @pytest_ar._saferepr(AtmosProfile) if 'AtmosProfile' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(AtmosProfile) else 'AtmosProfile', 'py9': @pytest_ar._saferepr(@py_assert8)}
