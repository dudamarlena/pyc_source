...
# Embedded file name: build\bdist.win32\egg\pyojo\data.py
# Compiled at: 2013-06-09 09:40:11
__doc__ = ' Classes for data manipulation.\n'
import StringIO, xml.etree.ElementTree as etree
...
class XML(object):
    ...
    def new_node(self, tag, prop={}, text='', parent=None):
        if not type(prop) == type({}):
            raise DataError(self, 'Node properties must be a dictionary, not %s' % type(prop).__name__)
        if parent is None:
            parent = self.root
...
ENVIRON_TEST = {'PATH_INFO': '/', 'REQUEST_METHOD': 'GET', 
   'QUERY_STRING': '', 
   'HTTP_ACCEPT': '*/*'}