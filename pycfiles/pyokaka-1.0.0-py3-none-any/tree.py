...
# Embedded file name: build\bdist.win32\egg\pyojo\js\lib\tree.py
# Compiled at: 2013-06-09 07:34:02
__doc__ = ' Javascript Tree code blocks.\n\n'
import os, urllib2
...
class TreeJSON(js.Block):
    ...
    """
    _z = js.Function('fullObject', 'return fullObject.children;')
    getChildren = js.Function('object', 'return this.get(object.id).then(%s);' % _z)
    getRoot = js.Function('onItem', "this.store.get('root').then(onItem);")
    mayHaveChildren = js.Function('object', "return 'children' in object;")

    def __init__(self, data_source, dnd=False):
        super(TreeJSON, self).__init__('TreeJSON')
        store = JsonRest(target=data_source, getChildren=self.getChildren)
        self.tree_data = js.Var('tree_data', store)
        model = ObjectStoreModel(store=self.tree_data.var, getRoot=self.getRoot, mayHaveChildren=self.mayHaveChildren)
        self.tree_model = js.Var('tree_model', model)
        self.tree = dijit.Tree('TREE_Tree', model=self.tree_model.var, target='TREE')
        if dnd:
        ...
        if value is None:
            ...
            if child:
                ...
                if len(more['children']) == 0:
                    more['children'] = False
            elif key in self.parent.values():
                more['children'] = True
            value.update(more)
            return value
