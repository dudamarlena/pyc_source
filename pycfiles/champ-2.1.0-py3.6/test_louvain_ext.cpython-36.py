...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/champ/test_louvain_ext.py
# Compiled at: 2019-05-20 15:27:10
# Size of source mod 2**32: 3541 bytes
import logging, os, re, sys, champ, pandas as pd, pickle, gzip, igraph as ig, numpy as np
DESCRIPTION = ''
...
def main():
    ...
    np.random.seed(0)
    test_graph = ig.Graph.Erdos_Renyi(n=200, p=0.1)
...
def test_multilayer_on_lawyer():
    ...
    inter_elist = [(i, i + n) for i in range(n)] + [(i + n, 2 * n + i) for i in range(n)] + [(2 * n + i, i) for i in range(n)]
    C = np.zeros((3 * n, 3 * n))
    ...
    mult_part_ens = champ.parallel_multilayer_louvain_from_adj(intralayer_adj=super_adj, interlayer_adj=C,
      layer_vec=layer_vec,
      progress=True,
      numprocesses=2,
      inter_directed=False,
