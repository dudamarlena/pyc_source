...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/champ/louvain_ml_extension.py
# Compiled at: 2018-06-13 09:25:12
# Size of source mod 2**32: 51412 bytes
from __future__ import absolute_import
from __future__ import print_function
from __future__ import division
from future.utils import iteritems, iterkeys
from future.utils import lmap
import gzip, sys, os, re, tempfile
...
import matplotlib.lines as mlines, matplotlib.pyplot as plt, matplotlib.patheffects as path_effects
from matplotlib import rc
import igraph as ig, louvain, numpy as np, h5py, sklearn.metrics as skm
from time import time
import logging
logging.basicConfig(format=':%(asctime)s:%(levelname)s:%(message)s', level=(logging.DEBUG))

class MultiLayerPartitionEnsemble:
    __doc__ = 'Group of partitions of a multilayer graph stored in membership vector format\n\n\tThe attribute for each partition is stored in an array and can be indexed\n\n\t:cvar graph: The graph associated with this PartitionEnsemble.  Each ensemble \tcan only have a single graph and the nodes on the graph must be orded the same as \teach of the membership vectors.\n\t:type graph: igraph.Graph\n\n\t:cvar partitions:  of membership vectors for each partition.  If h5py is set this is a dummy \tvariable that allows access to the file, but never actually hold the array of parititons.\n\t:type partitions:  np.array\n\t:cvar int_edges:  Number of edges internal to the communities\n\t:type int_edges:  list\n\n\t:cvar exp_edges:  Number of expected edges (based on configuration model)\n\t:type exp_edges:  list\n\n\t:cvar resoltions:  If partitions were idenitfied with Louvain, what resolution \twere they identified at (otherwise None)\n\t:type resolutions: list\n\n\t:cvar orig_mods:  Modularity of partition at the resolution it was identified at \tif Louvain was used (otherwise None).\n\t:type orig_mods: list\n\n\t:cvar numparts: number of partitions\n\t:type numparts: int\n\t:cvar ind2doms: Maps index of dominant partitions to boundary points of their dominant \tdomains\n\t:type ind2doms: dict\n\t:cvar ncoms: List with number of communities for each partition\n\t:type numcoms: list\n\t:cvar min_com_size: How many nodes must be in a community for it to count towards the number \tof communities.  This eliminates very small or unstable communities.  Default is 5\n\t:type min_com_size: int\n\t:cvar unique_partition_indices: The indices of the paritions that represent unique coefficients.  This will be a \tsubset of all the partitions.\n\t:cvar hdf5_file: Current hdf5_file.  If not None, this serves as the default location for loading and writing \tpartitions to, as well as the default location for saving.\n\t:type hdf5_file: str\n\t:type unique_partition_indices: np.array\n\t:cvar twin_partitions:  We define twin partitions as those that have the same coefficients, but are actually \tdifferent partitions.  This and the unique_partition_indices are only calculated on demand which can take some time.\n\t:type twin_partitions: list of np.arrays\n\t'

    def __init__(self, intra_graph=None, inter_graph=None, layer_vec=None, listofparts=None, name='unnamed_graph', maxpt=None, min_com_size=5):
        ...
        if intra:
            if 'intra_adj' not in self.__dict__:
                if 'weight' in self.intra_graph.edge_attributes():
                    self.intra_adj = self.intra_graph.get_adjacency(type='GET_ADJACENCY_BOTH', attribute='weight')
                else:
            ...
            if 'inter_adj' not in self.__dict__:
                if 'weight' in self.inter_graph.edge_attributes():
                    self.intra_adj = self.inter_graph.get_adjacency(type='GET_ADJACENCY_BOTH', attribute='weight')
                else:
                ...
                if openfile['_partitions'].shape[0] == len(self.int_edges):
                    if openfile['_partitions'].shape[0] == len(self.resolutions):
                        if openfile['_partitions'].shape[0] == len(self.exp_edges):
                            if openfile['_partitions'].shape[0] == len(self.couplings):
                                return True
                return False
        if self.partitions.shape[0] == len(self.int_edges):
            if self.partitions.shape[0] == len(self.int_inter_edges):
                if self.partitions.shape[0] == len(self.resolutions):
                    if self.partitions.shape[0] == len(self.exp_edges):
                        return True
        return False
    ...
    def _combine_partitions_hdf5_files(self, otherfile):
        ...
        with h5py.File(self._hdf5_file, 'a') as (myfile):
            with h5py.File(otherfile, 'r') as (file_2_add):
                for attribute in ('_partitions', 'resolutions', 'couplings', 'orig_mods',
                                  'int_edges', 'int_inter_edges', 'exp_edges'):
    ...
    def _append_partitions_hdf5_file(self, partitions):
        ...
        with h5py.File(self._hdf5_file, 'a') as (openfile):
            ...
            for i, part in enumerate(partitions):
                ...
                elif self.resolutions[(-1)] is not None:
                ...
                for part in partitions:
                    ...
                    if 'orig_mod' in part:
                        ...
                        if self.resolutions[(-1)] is not None:
                            ...
                            self.orig_mods = np.append(self.orig_mods, None)
                    self.numcoms = np.append(self.numcoms, get_number_of_communities((part['partition']), min_com_size=(self.min_com_size)))
                    assert self._check_lengths()
    ...
    def get_broadedst_domains(self, n=4):
        r"""
                Return the starting $\gamma$ for the top n domains by the length of the domain          (i.e. $\gamma_{i+1}-\gamma_{i}$)
    ...
    def _write_graph_to_hd5f_file(self, file, compress=4):
        ...
        grph.create_dataset('edge_list', data=(np.array([e.tuple for e in self.graph.es])),
          compression='gzip',
          compression_opts=compress)
        edge_atts = grph.create_group('edge_attributes')
        for attrib in self.graph.edge_attributes():
            edge_atts.create_dataset(attrib, data=(np.array(self.graph.es[attrib])),
              compression='gzip',
              compression_opts=compress)

        node_atts = grph.create_group('node_attributes')
        for attrib in self.graph.vertex_attributes():
            node_atts.create_dataset(attrib, data=(np.array(self.graph.vs[attrib])),
              compression='gzip',
              compression_opts=compress)
    ...
    def save_graph(self, filename=None, dir='.'):
        """
                Save a copy of the graph with each of the optimal partitions stored as vertex attributes                in graphml compressed format.  Each partition is attribute names part_gamma where gamma is              the beginning of the partitions domain of dominance
    ...
    def plot_modularity_mapping(self, ax=None, downsample=2000, champ_only=False, legend=True, no_tex=False):
        r"""
        ...
        if ax == None:
            ...
            if not champ_only:
                if self.orig_mods[0] is not None:
            ...
            sct2 = a2.scatter(allgams, allcoms, marker='^', color='#91AEC1', alpha=1,
              label=('\\# communities ($\\ge %d$ nodes)' % self.min_com_size),
              zorder=1)
            mk3 = a2.scatter([], [], marker='^', color='#91AEC1', alpha=1, label=('\\# communities ($\\ge %d$)' % self.min_com_size),
              zorder=1,
              s=20)
            ...
            if legend:
                l = ax.legend([mk1, mk3, mk2, mk4, mk5], [
                 'modularity', '\\# communities ($\\ge %d $ nodes)' % self.min_com_size, 'transitions,$\\gamma$',
                 '\\# communities ($\\ge %d$ nodes) optimal' % self.min_com_size, 'convex hull of $Q(\\gamma)$'],
                  bbox_to_anchor=[
                 0.5, 0.87],
                  loc='center',
                  frameon=True,
...
def _create_all_layer_igraphs_multi(intralayer_edges, layer_vec, directed=False):
    ...
    for i, layer_elist in enumerate(layer_elists):
        if not directed:
            layer_elist = list(set(layer_elist))
        else:
            cnts[i]
            ...
            cgraph.vs['nid'] = range(tot, tot + cnts[i])
            if len(layer_weights[i]) > 0:
...
def _label_nodes_by_identity(intralayer_graphs, interlayer_edges, layer_vec):
    """Go through each of the nodes and determine which ones are shared across multiple slices.        We create an attribute on each of the graphs to indicate the shared identity    of that node.  This is done through tracking the predecessors of the node vi the interlayer     connections
...
def create_multilayer_igraph_from_edgelist(intralayer_edges, interlayer_edges, layer_vec, directed=False):
    """
           We create an igraph representation used by the louvain package to represents multi-slice graphs.        For this method only two graphs are created :
           intralayer_graph : all edges withis this graph are treated equally though the null model is adjusted            based on each slice's degree distribution
           interlayer_graph:  sinlge graph that contains only interlayer connections between all nodes

        :param intralayer_edges: edges representing intralayer connections.  Note each node should be assigned a unique index.
        :param interlayer_edges: connection across layers.
        :param layer_vec: indication of which layer each node is in.  This important in computing the modulary modularity       null model.
...
def call_slices_to_layers_from_edge_list(intralayer_edges, interlayer_edges, layer_vec, directed=False):
    """
           We create an igraph representation used by the louvain package to represents multi-slice graphs.  This returns          three values:
                layers : list of igraphs each one representing a single slice in the network (all nodes across all layers               are present but only the edges in that slice)
                interslice_layer: igraph representing interlayer connectiosn
                G_full : igraph with connections for both inter and intra slice connections across all nodes ( differentiated)          by igraph.es attribute.
...
def create_multilayer_igraph_from_adjacency(A, C, layer_vec, directed=False):
    """
        Create the multilayer igraph representation necessary to call igraph-louvain    in the multilayer context.  Edge list are formed and champ_fucntions.create_multilayer_igraph_from_edgelist     is called.  Each edge list includes the weight of the edge      as indicated in the appropriate adjacency matrix.
    ...
    return create_multilayer_igraph_from_edgelist(intralayer_edges=intra_edgelist, interlayer_edges=inter_edgelist,
      layer_vec=layer_vec,
      directed=directed)
...
def _save_ml_graph(slice_layers, interslice_layer):
    ...
    filehandles = []
    filenames = []
    for layer in slice_layers + [interslice_layer]:
...
def get_expected_edges_ml(part_obj, layer_vec, weight='weight'):
    """
        Multilayer calculation of expected edges.  Breaks up partition object   by layer and calculated expected edges for each layer-subgraph seperately       thus getting the relative weights correct
...
def run_louvain_multilayer(intralayer_graph, interlayer_graph, layer_vec, weight='weight', resolution=1.0, omega=1.0, nruns=1):
    ...
    for run in range(nruns):
        ...
        for i, layer in enumerate(rlayers):
            ...
            cpart = louvain.RBConfigurationVertexPartitionWeightedLayers(layer, layer_vec=layer_vec, weights=weight,
              resolution_parameter=resolution)
            layer_partition_objs.append(cpart)

        coupling_partition = louvain.RBConfigurationVertexPartition(interslice_layer_rand, weights='weight',
        ...
        outparts.append({'partition':np.array(finalpartition),  'resolution':resolution, 
         'coupling':omega, 
         'orig_mod':0.5 / mu * _get_modularity_from_partobj_list(all_layer_partobjs), 
         'int_edges':A, 
         'exp_edges':P, 
         'int_inter_edges':C})
...
def _parallel_run_louvain_multimodularity(files_layervec_gamma_omega):
    ...
    np.random.seed()
    louvain.set_rng_seed(np.random.randint(2147483647))
...
def parallel_multilayer_louvain(intralayer_edges, interlayer_edges, layer_vec, gamma_range, ngamma, omega_range, nomega, maxpt=None, numprocesses=2):
    """"""
    logging.debug('creating graphs from edges')
    t = time()
    intralayer_graph, interlayer_graph = create_multilayer_igraph_from_edgelist(intralayer_edges=intralayer_edges, interlayer_edges=interlayer_edges,
      layer_vec=layer_vec)
    logging.debug('time {:.4f}'.format(time() - t))
    gammas = np.linspace((gamma_range[0]), (gamma_range[1]), num=ngamma)
    omegas = np.linspace((omega_range[0]), (omega_range[1]), num=nomega)
    args = itertools.product([intralayer_graph], [interlayer_graph], [layer_vec], gammas, omegas)
...
def parallel_multilayer_louvain_from_adj(intralayer_adj, interlayer_adj, layer_vec, gamma_range, omega_range):
    ...
    return parallel_multilayer_louvain(intralayer_edges=intralayer_edges, interlayer_edges=interlayer_edges, layer_vec=layer_vec,
