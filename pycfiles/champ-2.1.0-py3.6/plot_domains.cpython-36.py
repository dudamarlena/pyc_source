...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/champ/plot_domains.py
# Compiled at: 2018-08-27 09:11:17
# Size of source mod 2**32: 9709 bytes
import matplotlib.cm as cm, matplotlib.colors as mcolors, matplotlib.patches as patch, matplotlib.pyplot as plt, numpy as np
...
def plot_2d_domains(ind_2_domains, ax=None, col=None, close=False, widths=None, label=False):
    ...
    for i, ind_pts in enumerate(iteritems(ind_2_domains)):
        ...
        polypatch = patch.PathPatch(polypath, facecolor=c, lw=2, alpha=0.75)
...
def plot_single_layer_modularity_domains(ind_2_domains, ax=None, colors=None, labels=None):
    ...
    if ax == None:
        ...
        for i, ind_pts in enumerate(iteritems(ind_2_domains)):
            ...
            ax.plot((coords[0]), (coords[1]), color=c, lw=2, alpha=0.75)
...
def plot_similarity_heatmap_single_layer(partitions, index_2_domain, partitions_other=None, index_2_dom_other=None, sim_mat=None, ax=None, cmap=None, title=None):
    """
    ...
    if cmap is None:
        ...
        if ax is None:
            ...
            gamma_transitions.append(index_2_domain[ind_vals[(-1)][0]][(-1)][0])
            ...
            if sim_mat is None:
                ...
                for i in range(len(ind_vals)):
                    for j in range(len(ind_vals2)):
                        ...
                        AMI_mat[i][j] = adjusted_mutual_info_score(partition1, partition2)
