...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/bytecodehacks/code_editor.py
# Compiled at: 2000-04-01 09:14:35
...
class CodeString(Fleshable):
    ...
    def compute_stack(self):
        ...
        for op in self.opcodes:
            op.stack_manipulate(push=tracker.push, pop=tracker.pop, block_pop=tracker.block_pop, block_push=tracker.block_push, tracker=tracker)
...
class EditableCode(Flesher, Fleshable):
    ...
    def init_tuple(self, tup):
        self.co_argcount, ignored, ignored, self.co_flags, self.co_code, co_consts, co_names, co_varnames, self.co_filename, self.co_name, self.co_firstlineno, self.co_lnotab = tup
...
class Function(Fleshable, Flesher):
    ...
    def init_tuple(self, tup):
        self.__name, self.__doc, func_code, self.func_defaults, self.func_globals = tup
    ...
    def make_function(self):
        newfunc = new.function(self.func_code.make_code(), self.func_globals, self.__name)
    ...
    def as_tuple(self):
        if not self.func_defaults:
            ...
            defs = tuple(self.func_defaults)
        return (
         self.__name, self.__doc, self.func_code.as_tuple(), defs, {})
...
class InstanceMethod(Flesher):
    ...
    def make_instance_method(self):
        return new.instancemethod(self.im_func.make_function(), self.im_self, self.im_class)
