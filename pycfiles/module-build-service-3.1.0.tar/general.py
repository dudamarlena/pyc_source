...
# [GCC 8.4.0]
# Embedded file name: /home/breilly/git/fm-orchestrator/module_build_service/utils/general.py
# Compiled at: 2019-12-12 15:53:56
import os, functools, inspect, hashlib, time, locale, contextlib
...
def load_mmd(yaml, is_file=False):
    ...
    try:
        ...
        if mmd.get_mdversion() < target_mmd_version:
            ...
            raise UnprocessableEntity(('The modulemd version cannot be greater than {}').format(target_mmd_version))
        ...
        if 'modulemd-error-quark: ' in str(e):
            error = ("{} The error was '{}'.").format(error, str(e).split('modulemd-error-quark: ')[(-1)])
        elif 'Unknown ModuleStream version' in str(e):
            error = ("{}. The modulemd version can't be greater than {}.").format(error, target_mmd_version)
...
def scm_url_schemes(terse=False):
    ...
    :param terse=False: Whether to return terse list of unique URL schemes
                        even without the "://".
    """
    scm_types = {'git': ('git://', 'git+http://', 'git+https://', 'git+rsync://', 'http://', 'https://', 'file://')}
    if not terse:
        return scm_types
    else:
        scheme_list = []
        ...
        def inner(*args, **kwargs):
            ...
            while True:
                try:
                    ...
                    log.warning('Exception %r raised from %r.  Retry in %rs' % (e, function, interval))
                    time.sleep(interval)
...
def generate_koji_tag(name, stream, version, context, max_length=256, scratch=False, scratch_id=0):
    ...
    if scratch:
        ...
        suffix = ''
    nsvc_list = [
     name, stream, str(version), context]
...
def validate_koji_tag(tag_arg_names, pre='', post='-', dict_key='name'):
    ...
    def validation_decorator(function):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                if call_args['self'].module_str in conf.allowed_privileged_module_names + conf.base_module_names:
                    return function(*args, **kwargs)
            ...
            for tag_arg_name in tag_arg_names:
                ...
                if tag_arg_name not in call_args:
                    raise ProgrammingError(('{} Inspected argument {} is not within function args. The function was: {}.').format(err_subject, tag_arg_name, function.__name__))
                tag_arg_val = call_args[tag_arg_name]
                if not tag_arg_val:
                    raise ValidationError(('{} Can not validate {}. No value provided.').format(err_subject, tag_arg_name))
                if isinstance(tag_arg_val, dict):
                    if dict_key not in tag_arg_val:
                        raise ProgrammingError(('{} Inspected dict arg {} does not contain {} key. The function was: {}.').format(err_subject, tag_arg_name, dict_key, function.__name__))
                    tag_list = [tag_arg_val[dict_key]]
                else:
                    if isinstance(tag_arg_val, list):
                    ...
                    raise ValidationError(('Koji tag validation: {} does not satisfy any of allowed prefixes: {}').format(tag_list, [ pre + p + post for p in conf.koji_tag_prefixes ]))
...
def get_rpm_release(db_session, module_build):
    ...
    try:
        ...
        log.warning(('Module build {0} does not have buildrequires in its xmd').format(module_build.id))
        buildrequires = None
    ...
    if buildrequires:
        for module in conf.allowed_privileged_module_names + conf.base_module_names:
            ...
            if not module_in_xmd:
                continue
            module_obj = models.ModuleBuild.get_build_from_nsvc(db_session, module, module_in_xmd['stream'], module_in_xmd['version'], module_in_xmd['context'])
            if not module_obj:
                continue
            try:
            ...
            log.warning(('Module build {0} does not buildrequire a base module ({1})').format(module_build.id, (' or ').join(conf.base_module_names)))

    prefix = 'scrmod+' if module_build.scratch else conf.default_dist_tag_prefix
    return ('{prefix}{base_module_marking}{index}+{dist_hash}').format(prefix=prefix, base_module_marking=br_module_marking, index=index, dist_hash=dist_hash)
...
def import_mmd(db_session, mmd, check_buildrequires=True):
    ...
    if virtual_streams and (not isinstance(virtual_streams, list) or any(not isinstance(vs, string_types) for vs in virtual_streams)):
    ...
    if check_buildrequires:
        ...
        if len(deps) > 1:
            raise UnprocessableEntity("The imported module's dependencies list should contain just one element")
        ...
        if len(deps) > 0:
            ...
            if brs - xmd_brs:
                raise UnprocessableEntity('The imported module buildrequires other modules, but the metadata in the xmd["mbs"]["buildrequires"] dictionary is missing entries')
...
def get_build_arches(mmd, config):
    ...
    try:
        ...
        log.warning(('Module {0} does not have buildrequires in its xmd').format(mmd.get_nsvc()))
        buildrequires = None
    ...
    if buildrequires:
        for module in conf.allowed_privileged_module_names + conf.base_module_names:
            ...
            if not module_obj:
                continue
            arches = module_build_service.builder.GenericBuilder.get_module_build_arches(module_obj)
            if arches:
                log.info('Setting build arches of %s to %r based on the buildrequired module %r.' % (
                 nsvc, arches, module_obj))
                return arches
...
def deps_to_dict(deps, deps_type):
    ...
    return {module:streams_func(module) for module in names_func()}
