...
# [GCC 8.4.0]
# Embedded file name: /home/breilly/git/fm-orchestrator/module_build_service/messaging.py
...
class BaseMessage(object):
    ...
    def __repr__(self):
        ...
        args_strs = ((('{}={!r}').format(name, getattr(self, name)) if param.default != param.empty else repr(getattr(self, name))) for name, param in init_sig.parameters.items())
...
class FedmsgMessageParser(MessageParser):
    ...
    def parse(self, msg):
        ...
        regex_pattern = re.compile('(?P<category>' + categories_re + ')(?:(?:\\.)(?P<object>build|repo|module|decision))?(?:(?:\\.)(?P<subobject>state|build))?(?:\\.)(?P<event>change|done|end|tag|update)$')
        ...
        if regex_results:
            ...
            if not msg_inner_msg:
                log.debug(('Skipping message without any content with the topic "{0}"').format(topic))
                return
            msg_obj = None
            if category == 'buildsys':
                ...
                if instance != 'primary':
                    ...
                    return
            if category == 'buildsys' and object == 'build' and subobject == 'state' and event == 'change':
                ...
                msg_obj = KojiBuildChange(msg_id, build_id, task_id, build_new_state, build_name, build_version, build_release)
            else:
                if category == 'buildsys' and object == 'repo' and subobject is None and event == 'done':
                ...
                elif category == 'mbs' and object == 'module' and subobject == 'state' and event == 'change':
                    msg_obj = MBSModule(msg_id, msg_inner_msg.get('id'), msg_inner_msg.get('state'))
                elif category == 'greenwave' and object == 'decision' and subobject is None and event == 'update':
                    msg_obj = GreenwaveDecisionUpdate(msg_id=msg_id, decision_context=msg_inner_msg.get('decision_context'), policies_satisfied=msg_inner_msg.get('policies_satisfied'), subject_identifier=msg_inner_msg.get('subject_identifier'))
                if msg_obj:
                    return msg_obj
        return
...
class KojiBuildChange(BaseMessage):
    ...
    def __init__(self, msg_id, build_id, task_id, build_new_state, build_name, build_version, build_release, module_build_id=None, state_reason=None):
        if task_id is None:
...
def _in_memory_publish(topic, msg, conf, service):
    ...
    _in_memory_msg_id += 1
    wrapped_msg = FedmsgMessageParser().parse({'msg_id': str(_in_memory_msg_id), 
       'topic': service + '.' + topic, 
       'msg': msg})
...
_fedmsg_backend = {'publish': _fedmsg_publish, 
   'services': [
              'buildsys', 'mbs', 'greenwave'], 
   'parser': FedmsgMessageParser(), 
   'topic_suffix': '.'}
_in_memory_backend = {'publish': _in_memory_publish, 
   'services': [], 'parser': FedmsgMessageParser(), 
   'topic_suffix': '.'}
_messaging_backends = {}
