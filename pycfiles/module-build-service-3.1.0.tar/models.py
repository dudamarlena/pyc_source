...
# [GCC 8.4.0]
# Embedded file name: /home/breilly/git/fm-orchestrator/module_build_service/models.py
# Compiled at: 2019-12-12 15:53:56
""" SQLAlchemy Database models for the Flask app
"""
import hashlib, json, koji, re
from collections import OrderedDict, namedtuple
from datetime import datetime
import sqlalchemy, kobo.rpmlib
...
FAILED_STATES = (
 BUILD_STATES['failed'], BUILD_STATES['garbage'])
STATE_TRANSITION_FAILURE_TYPES = [
 'unspec', 'user', 'infra']
Contexts = namedtuple('Contexts', 'build_context runtime_context context build_context_no_bms')
...
module_builds_to_module_buildrequires = db.Table('module_builds_to_module_buildrequires', db.Column('module_id', db.Integer, db.ForeignKey('module_builds.id'), nullable=False), db.Column('module_buildrequire_id', db.Integer, db.ForeignKey('module_builds.id'), nullable=False), db.UniqueConstraint('module_id', 'module_buildrequire_id', name='unique_buildrequires'))
module_builds_to_virtual_streams = db.Table('module_builds_to_virtual_streams', db.Column('module_build_id', db.Integer, db.ForeignKey('module_builds.id'), nullable=False), db.Column('virtual_stream_id', db.Integer, db.ForeignKey('virtual_streams.id'), nullable=False), db.UniqueConstraint('module_build_id', 'virtual_stream_id', name='unique_module_to_virtual_stream'))
module_builds_to_arches = db.Table('module_builds_to_arches', db.Column('module_build_id', db.Integer, db.ForeignKey('module_builds.id'), nullable=False), db.Column('module_arch_id', db.Integer, db.ForeignKey('module_arches.id'), nullable=False), db.UniqueConstraint('module_build_id', 'module_arch_id', name='unique_module_to_arch'))
...
class ModuleBuild(MBSBase):
    ...
    koji_tag = db.Column(db.String)
    cg_build_koji_tag = db.Column(db.String)
    ...
    virtual_streams = db.relationship('VirtualStream', secondary=module_builds_to_virtual_streams, back_populates='module_builds')
    ...
    arches = db.relationship('ModuleArch', secondary=module_builds_to_arches, back_populates='module_builds', order_by='ModuleArch.name')
    batch = db.Column(db.Integer, default=0)
    stream_version = db.Column(db.Float)
    buildrequires = db.relationship('ModuleBuild', secondary=module_builds_to_module_buildrequires, primaryjoin=module_builds_to_module_buildrequires.c.module_id == id, secondaryjoin=module_builds_to_module_buildrequires.c.module_buildrequire_id == id, backref='buildrequire_for')
    rebuild_strategies = {'all': 'All components will be rebuilt', 
       'changed-and-after': 'All components that have changed and those in subsequent batches will be rebuilt', 
    ...
    def current_batch(self, state=None):
        ...
        if state is not None:
            return [ component for component in self.component_builds if component.batch == self.batch and component.state == state
                   ]
        else:
            return [ component for component in self.component_builds if component.batch == self.batch
                   ]
            return
    ...
    def get_last_build_in_all_streams(db_session, name):
        ...
        """
        subq = db_session.query(func.max(ModuleBuild.id).label('maxid'), func.max(sqlalchemy.cast(ModuleBuild.version, db.BigInteger))).group_by(ModuleBuild.stream).filter_by(name=name, state=BUILD_STATES['ready']).subquery('t2')
        query = db_session.query(ModuleBuild).join(subq, and_(ModuleBuild.id == subq.c.maxid))
        return query.all()

    @staticmethod
    def _get_last_builds_in_stream_query(db_session, name, stream, **kwargs):
        subq = db_session.query(func.max(sqlalchemy.cast(ModuleBuild.version, db.BigInteger)).label('maxversion')).filter_by(name=name, state=BUILD_STATES['ready'], stream=stream, **kwargs).subquery('t2')
        query = db_session.query(ModuleBuild).join(subq, and_(ModuleBuild.name == name, ModuleBuild.stream == stream, sqlalchemy.cast(ModuleBuild.version, db.BigInteger) == subq.c.maxversion))
        return query
    ...
    def get_last_build_in_stream(db_session, name, stream, **kwargs):
        ...
        :param dict kwargs: Key/value pairs passed to SQLAlchmey filter_by method
            allowing to set additional filter for results.
        """
        return ModuleBuild._get_last_builds_in_stream_query(db_session, name, stream, **kwargs).first()
    ...
    def get_build_from_nsvc(db_session, name, stream, version, context, **kwargs):
        ...
        filter_by method.
        """
        return db_session.query(ModuleBuild).filter_by(name=name, stream=stream, version=str(version), context=context, **kwargs).first()
    ...
    def get_scratch_builds_from_nsvc(db_session, name, stream, version, context, **kwargs):
        ...
        """
        return db_session.query(ModuleBuild).filter_by(name=name, stream=stream, version=str(version), scratch=True, **kwargs).filter(ModuleBuild.context.like(context + '%')).all()
    ...
    def _add_stream_version_lte_filter(db_session, query, stream_version):
        ...
        return query.filter(ModuleBuild.stream_version <= stream_version).filter(ModuleBuild.stream_version >= min_stream_version)
    ...
    def _add_virtual_streams_filter(db_session, query, virtual_streams):
        ...
        if not virtual_streams:
            return query
        modules_with_virtual_streams = db_session.query(ModuleBuild).join(VirtualStream, ModuleBuild.virtual_streams).filter(VirtualStream.name.in_(virtual_streams)).order_by(ModuleBuild.id).distinct(ModuleBuild.id).subquery()
        return query.join(modules_with_virtual_streams, ModuleBuild.id == modules_with_virtual_streams.c.id)

    @staticmethod
    def get_last_builds_in_stream_version_lte(db_session, name, stream_version=None, virtual_streams=None, states=None):
        """
        ...
        query = db_session.query(ModuleBuild).filter(ModuleBuild.name == name).filter(ModuleBuild.state.in_(states)).order_by(sqlalchemy.cast(ModuleBuild.version, db.BigInteger).desc())
    ...
    def validate_rebuild_strategy(self, key, rebuild_strategy):
        if rebuild_strategy not in self.rebuild_strategies.keys():
            ...
            raise ValueError(('The rebuild_strategy of "{0}" is invalid. Choose from: {1}').format(rebuild_strategy, choices))
    ...
    def contexts_from_mmd(cls, mmd_str):
        ...
        return Contexts(build_context, runtime_context, cls.calculate_module_context(build_context, runtime_context), build_context_no_bms)
    ...
    def calculate_build_context(mbs_xmd_buildrequires, filter_base_modules=False):
        ...
        mmd_formatted_buildrequires = {dep:info['stream'] for dep, info in mbs_xmd_buildrequires.items() if dep not in deps_to_filter}
    ...
    def siblings(self, db_session):
        query = db_session.query(ModuleBuild).filter(ModuleBuild.name == self.name, ModuleBuild.stream == self.stream, ModuleBuild.version == self.version, ModuleBuild.scratch == self.scratch, ModuleBuild.id != self.id).options(load_only('id'))
    ...
    def nvr(self):
        return {'name': self.name, 
           'version': self.stream.replace('-', '_'), 
           'release': ('{0}.{1}').format(self.version, self.context)}
    ...
    @classmethod
    def create(cls, db_session, conf, name, stream, version, modulemd, scmurl, username, context=None, rebuild_strategy=None, scratch=False, srpms=None, publish_msg=True, **kwargs):
        now = datetime.utcnow()
        module = cls(name=name, stream=stream, version=version, context=context, state='init', modulemd=modulemd, scmurl=scmurl, owner=username, time_submitted=now, time_modified=now, rebuild_strategy=(rebuild_strategy or conf.rebuild_strategy), scratch=scratch, srpms=json.dumps(srpms or []), **kwargs)
        ...
        db_session.commit()
        if publish_msg:
            module_build_service.messaging.publish(service='mbs', topic='module.state.change', msg=module.json(db_session, show_tasks=False), conf=conf)
        return module
    ...
    def transition(self, db_session, conf, state, state_reason=None, failure_type='unspec'):
        ...
        log.info('State transition: %r -> %r, %r', INVERSE_BUILD_STATES[old_state], new_state_name, self)
        if old_state != self.state:
            module_build_service.messaging.publish(service='mbs', topic='module.state.change', msg=self.json(db_session, show_tasks=False), conf=conf)
    ...
    def local_modules(cls, db_session, name=None, stream=None):
        ...
        if not local_modules:
            return []
        local_modules = [ m for m in local_modules if m.koji_tag and m.koji_tag.startswith(conf.mock_resultsdir) ]
    ...
    def from_repo_done_event(cls, db_session, event):
        ...
        if event.repo_tag.endswith('-build'):
            ...
            tag = event.repo_tag
        query = db_session.query(cls).filter(cls.koji_tag == tag).filter(cls.state == BUILD_STATES['build'])
        count = query.count()
        if count > 1:
    ...
    def json(self, db_session, show_tasks=True):
        ...
        rv = self.short_json()
        rv.update({'component_builds': [ build.id for build in self.component_builds ], 'koji_tag': self.koji_tag, 
           'owner': self.owner, 
    ...
    def extended_json(self, db_session, show_state_url=False, api_version=1):
        ...
        rv.update({'base_module_buildrequires': [ br.short_json(True, False) for br in self.buildrequires ], 'build_context': self.build_context, 
           'modulemd': self.modulemd, 
           'reused_module_id': self.reused_module_id, 
           'runtime_context': self.runtime_context, 
           'state_trace': [ {'time': _utc_datetime_to_iso(record.state_time), 'state': record.state, 'state_name': INVERSE_BUILD_STATES[record.state], 'reason': record.state_reason} for record in self.state_trace(db_session, self.id)
                        ], 
           'state_url': state_url, 
           'stream_version': self.stream_version, 
           'virtual_streams': [ virtual_stream.name for virtual_stream in self.virtual_streams ], 'arches': [ arch.name for arch in self.arches ]})
        return rv

    def log_message(self, session, message):
        log.info(message)
        log_msg = LogMessage(component_build_id=None, module_build_id=self.id, message=message, time_created=datetime.utcnow())
        session.add(log_msg)
        session.commit()
        return
    ...
    def tasks(self, db_session):
        ...
        if self.id and self.state != 'init':
            for build in db_session.query(ComponentBuild).filter_by(module_id=self.id).options(lazyload('module_build')).all():
                tasks[build.format] = tasks.get(build.format, {})
                tasks[build.format][build.package] = dict(task_id=build.task_id, state=build.state, state_reason=build.state_reason, nvr=build.nvr)

        return tasks

    def state_trace(self, db_session, module_id):
        return db_session.query(ModuleBuildTrace).filter_by(module_id=module_id).order_by(ModuleBuildTrace.state_time).all()
    ...
    def get_buildrequired_base_modules(self, db_session):
        ...
        for bm in conf.base_module_names:
            ...
            if not bm_dict:
                continue
            base_module = self.get_build_from_nsvc(db_session, bm, bm_dict['stream'], bm_dict['version'], bm_dict['context'])
            if not base_module:
                log.error(('Module #{} buildrequires "{}" but it wasn\'t found in the database').format(self.id, repr(bm_dict)))
                continue
    ...
    def __repr__(self):
        return '<ModuleBuild %s, id=%d, stream=%s, version=%s, scratch=%r, state %r, batch %r, state_reason %r>' % (
         self.name,
         self.id,
         self.stream,
         self.version,
    ...
    def update_virtual_streams(self, db_session, virtual_streams):
        ...
        for stream_name in dropped_virtual_streams:
            virtual_stream = VirtualStream.get_by_name(db_session, stream_name)
            only_associated_with_self = len(virtual_stream.module_builds) == 1 and virtual_stream.module_builds[0].id == self.id
...
class VirtualStream(MBSBase):
    ...
    module_builds = db.relationship('ModuleBuild', secondary=module_builds_to_virtual_streams, back_populates='virtual_streams')
...
class ModuleArch(MBSBase):
    ...
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String, nullable=False, unique=True)
    module_builds = db.relationship('ModuleBuild', secondary=module_builds_to_arches, back_populates='arches')
...
class ComponentBuild(MBSBase):
    ...
    def from_component_event(cls, db_session, event):
        if isinstance(event, module_build_service.messaging.KojiBuildChange):
            if event.module_build_id:
                return db_session.query(cls).filter_by(task_id=event.task_id, module_id=event.module_build_id).one()
            else:
    ...
    def state_trace(self, db_session):
        return db_session.query(ComponentBuildTrace).filter_by(component_id=self.id).order_by(ComponentBuildTrace.state_time).all()
    ...
    def extended_json(self, db_session, show_state_url=False, api_version=1):
        ...
        json.update({'batch': self.batch, 
           'state_trace': [ {'time': _utc_datetime_to_iso(record.state_time), 'state': record.state, 'state_name': INVERSE_BUILD_STATES.get(record.state), 'reason': record.state_reason} for record in self.state_trace(db_session)
                        ], 
           'state_url': state_url})
        return json
    ...
    def log_message(self, session, message):
        log.info(message)
        log_msg = LogMessage(component_build_id=self.id, module_build_id=self.module_id, message=message, time_created=datetime.utcnow())
        session.add(log_msg)
        session.commit()
    ...
    def is_unsuccessful(self):
        return self.state == koji.BUILD_STATES['FAILED'] or self.state == koji.BUILD_STATES['CANCELED']
...
class LogMessage(MBSBase):
    ...
    def __repr__(self):
        return '<LogMessage %s, component_build_id: %s, module_build_id: %r, message: %s, time_created: %s>' % (
         self.id,
...
def session_before_commit_handlers(session):
    for item in set(session.new) | set(session.dirty):
        if isinstance(item, ComponentBuild):
            cbt = ComponentBuildTrace(state_time=datetime.utcnow(), state=item.state, state_reason=item.state_reason, task_id=item.task_id)
