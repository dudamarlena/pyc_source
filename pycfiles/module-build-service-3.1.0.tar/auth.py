...
# [GCC 8.4.0]
# Embedded file name: /home/breilly/git/fm-orchestrator/module_build_service/auth.py
...
class Ldap(object):
    ...
    def __init__(self):
        ...
        if conf.ldap_uri.startswith('ldaps://'):
            tls = ldap3.Tls(ca_certs_file='/etc/pki/tls/certs/ca-bundle.crt', validate=ssl.CERT_REQUIRED)
        ...
        try:
            ...
            log.error(('The connection to "{0}" failed. The following error was raised: {1}').format(conf.ldap_uri, str(error)))
            raise Forbidden("The connection to the LDAP server failed. Group membership couldn't be obtained.")
    ...
    def get_user_membership(self, uid):
        ...
        self.connection.search(self.base_dn, ldap_filter, search_scope=ldap3.LEVEL, attributes=['cn'])
        groups = self.connection.response
        try:
            return [ group['attributes']['cn'][0] for group in groups ]
        except KeyError:
            log.exception(('The LDAP groups could not be determined based on the search results of "{0}"').format(str(groups)))
            return []
