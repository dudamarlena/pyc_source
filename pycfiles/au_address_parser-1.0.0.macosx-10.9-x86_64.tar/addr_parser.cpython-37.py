...
# [GCC 8.4.0]
# Embedded file name: /Library/Frameworks/Python.framework/Versions/3.7/lib/python3.7/site-packages/au_address_parser/addr_parser.py
# Compiled at: 2020-02-04 06:52:25
# Size of source mod 2**32: 17656 bytes
import re
...
class AbAddressUtility(object):
    ...
    def __init__(self, addr_string):
        ...
        if len(split_addr) == 2:
            ...
            if len(split_addr) == 3:
                if re.match('(?P<state>(NSW|ACT|QLD|VIC|TAS|SA|NT|WA)){1}\\s+(?P<post>(\\d{1,4}){1})', split_addr[(-1)].strip()):
                    street_part, locality_part = split_addr[0].strip(), ' '.join([i.strip() for i in split_addr[-2:]])
                else:
                    prop_name, street_part, locality_part = [i.strip() for i in split_addr]
            else:
                if len(split_addr) == 1:
                    guess = self._addr_string.split()
                    if len(guess) == 6 or len(guess) == 7:
                        street_part, locality_part = ' '.join(guess[:3]), ' '.join(guess[3:])
                    else:
                        raise Exception('Not Valid Address Foramt')
                else:
                    raise Exception('Not Valid Address Foramt')
        street_part_patterns = ['(UNIT|LOT|SHOP|SUITE|U|ROOM)\\s*(?P<flat_number>[A-Z]*\\d+[A-Z]*)\\s+(?P<number>[A-Z]*\\d+[A-Z]*(-[A-Z]*\\d*[A-Z]*)*\\b)\\s+(?P<street_name>[^,]*?)$',
         '(?P<flat_number>\\b[A-Z]*\\d+[A-Z]*)/(?P<number>[A-Z]*\\d+[A-Z]*(-\\d*[A-Z]*)*\\b)\\s+(?P<street_name>[^,]*?)$',
         '(?P<number>[A-Z]*\\d+[A-Z]*(-[A-Z]*\\d*[A-Z]*)*\\b)\\s+(?P<street_name>[^,]*?)$']
        street_part_dict = {}
        ...
        locality_part_patterns = [
         '((?P<locality>^[A-Z]+((\\s|-)*[A-Z]*)*)\\s+(?P<state>(NSW|ACT|QLD|VIC|TAS|SA|NT|WA)){1}\\s+(?P<post>(\\d{1,4}){1}))',
         '((?P<locality>^[A-Z]+((\\s|-)*[A-Z]*)*)\\s+(?P<state>(NSW|ACT|QLD|VIC|TAS|SA|NT|WA)){1})',
         '((?P<locality>^[A-Z]+((\\s|-)*[A-Z]*)*)\\s+(?P<post>(\\d{1,4}){1}))',
         '(?P<locality>^[A-Z]+((\\s|-)*[A-Z]*)*)']
        locality_part_dict = {}
        ...
        if flat_number_part:
            flat_number_dict = re.search('(?P<flat_number_prefix>\\b[A-Z]*)(?P<flat_number>\\d+)(?P<flat_number_suffix>[A-Z]*)', flat_number_part).groupdict()
        else:
            flat_number_dict = {'flat_number_prefix':'', 
             'flat_number':'', 
             'flat_number_suffix':''}
        number = street_part_dict.get('number', None)
        number_dict = re.search('((?P<number_first_prefix>\\b[A-Z]+))*(?P<number_first>\\d+)((?P<number_first_suffix>[A-Z]+))*(-((?P<number_last_prefix>\\b[A-Z]+))*(?P<number_last>\\d+)((?P<number_last_suffix>[A-Z]+))*)*', number).groupdict()
        for k, v in number_dict.items():
            if v is None:
                number_dict[k] = ''

        street_name_part = street_part_dict.get('street_name', None)
        street_name_dict = {'street_name':'', 
         'street_type_abbr':'', 
         'street_type':'', 
         'street_suffix':'', 
         'street_suffix_abbr':''}
    ...
    def _clean_address(address):
        address = re.sub('"[A-Z\\s]*"\\s', '', address)
        address = re.sub("'[A-Z\\s]*'\\s", '', address)
    ...
    def from_gnaf_dict(cls, **kwags):
        ...
        """
        a = {'flat_number_prefix':kwags.get('flat_number_prefix', None), 
    ...
    def from_elk_search(cls, **kwags):
        ...
        """
        a = {'flat_part':kwags.get('flat_part', None), 
         'number_first':kwags.get('number_first', None), 
         'number_last':kwags.get('number_last', None), 
    ...
    def _deal_street_suffix(self, street_name_list, street_name_dict):
        street_suffix_dict = {'WEST':'W', 
         'EAST':'E',  'NORTH':'N',  'SOUTH':'S',  'NORTHEAST':'NE',  'SOUTHEAST':'SE', 
        ...
        if street_name_list[(-1)] in street_suffix_dict.keys():
            ...
            if street_name_list[(-1)] in street_suffix_dict.values():
                ...
                street_name_list = street_name_list[:-1]
        return (
         street_name_list, street_name_dict)

    def _deal_street_abbr(self, street_name_list, street_name_dict):
        self.rd_abbr_dict = {'ACCESS':'ACCS', 
         'ALLEY':'ALLY',  'ALLEYWAY':'ALWY',  'AMBLE':'AMBL',  'APPROACH':'APP',  'ARCADE':'ARC', 
         'ARTERY':'ART',  'ARTERIAL':'ARTL',  'AVENUE':'AVE',  'BANAN':'BA',  'BROADWAY':'BDWY', 
         'BEND':'BEND',  'BRAE':'BRAE',  'BRACE':'BRCE',  'BREAK':'BRK',  'BROW':'BROW', 
         'BOULEVARD':'BVD',  'BOARDWALK':'BWLK',  'BYPASS':'BYPA',  'BYWAY':'BYWY',  'CAUSEWAY':'CAUS', 
         'CIRCUIT':'CCT',  'CUL':'CDS',  'CHASE':'CH',  'CIRCLE':'CIR',  'CLOSE':'CL', 
         'CIRCLET':'CLT',  'COMMON':'CMMN',  'CORNER':'CNR',  'CONCOURSE':'CON',  'COVE':'COVE', 
         'COPSE':'CPS',  'CIRCUS':'CRCS',  'CRESCENT':'CRES',  'CROSSING':'CRSG',  'CROSS':'CRSS', 
         'CREST':'CRST',  'CUL-DE-SAC':'CSAC',  'COURT':'CT',  'CENTRE':'CTR',  'COURTYARD':'CTYD', 
         'CUTTING':'CUTT',  'DALE':'DALE',  'DEVIATION':'DEVN',  'DIP':'DIP',  'DRIVE':'DR', 
         'DRIVEWAY':'DRWY',  'DISTRIBUTOR':'DSTR',  'EDGE':'EDGE',  'ELBOW':'ELB',  'END':'END', 
         'ENTRANCE':'ENT',  'ESPLANADE':'ESP',  'EXPRESSWAY':'EXP',  'EXTENSION':'EXTN',  'FAIRWAY':'FAWY', 
         'FIRETRAIL':'FITR',  'FOLLOW':'FOLW',  'FORMATION':'FORM',  'FRONTAGE':'FRTG', 
         'FIRETRACK':'FTRK',  'FOOTWAY':'FTWY',  'FREEWAY':'FWY',  'GAP':'GAP',  'GATE':'GATE', 
         'GARDEN':'GDN',  'GARDENS':'GDNS',  'GLADE':'GLD',  'GLEN':'GLEN',  'GROVE':'GR', 
         'GRANGE':'GRA',  'GREEN':'GRN',  'HILL':'HILL',  'HIGHROAD':'HRD',  'HEIGHTS':'HTS', 
         'HIGHWAY':'HWY',  'INTERCHANGE':'INTG',  'JUNCTION':'JNC',  'KEY':'KEY',  'LANE':'LANE', 
         'LINE':'LINE',  'LINK':'LINK',  'LOOKOUT':'LKT',  'LANEWAY':'LNWY',  'LOOP':'LOOP', 
         'MALL':'MALL',  'MEWS':'MEWS',  'MEANDER':'MNDR',  'MOTORWAY':'MWY',  'NOOK':'NOOK', 
         'OUTLOOK':'OTLK',  'PASS':'PASS',  'PATH':'PATH',  'PARADE':'PDE',  'PATHWAY':'PHWY', 
         'PIAZZA':'PIAZ',  'POCKET':'PKT',  'PARKWAY':'PKWY',  'PLACE':'PL',  'PLAZA':'PLZA', 
         'POINT':'PNT',  'PORT':'PORT',  'PROMENADE':'PROM',  'PASSAGE':'PSGE',  'QUADRANT':'QDRT', 
         'QUAY':'QY',  'QUAYS':'QYS',  'RAMP':'RAMP',  'ROAD':'RD',  'RIDGE':'RDGE', 
         'ROADS':'RDS',  'REST':'REST',  'RING':'RING',  'RISE':'RISE',  'RAMBLE':'RMBL', 
         'ROW':'ROW',  'ROWE':'ROWE',  'ROUTE':'RTE',  'RETREAT':'RTT',  'ROTARY':'RTY', 
         'RUE':'RUE',  'SUBWAY':'SBWY',  'SHUNT':'SHUN',  'SPUR':'SPUR',  'SQUARE':'SQ', 
         'STREET':'ST',  'STEPS':'STPS',  'STRIP':'STRP',  'STAIRS':'STRS',  'SERVICEWAY':'SVWY', 
         'TARN':'TARN',  'TERRACE':'TCE',  'THOROUGHFARE':'THOR',  'TOLLWAY':'TLWY', 
         'TOP':'TOP',  'TOR':'TOR',  'TRACK':'TRK',  'TRAIL':'TRL',  'TURN':'TURN',  'UNDERPASS':'UPAS', 
         'VALE':'VALE',  'VIADUCT':'VIAD',  'VIEW':'VIEW',  'VISTA':'VSTA',  'WALK':'WALK', 
         'WAY':'WAY',  'WHARF':'WHRF',  'WALKWAY':'WKWY',  'WYND':'WYND',  'OVAL':'OVAL'}
        if street_name_list[(-1)] in self.rd_abbr_dict.keys():
            ...
            if street_name_list[(-1)] in self.rd_abbr_dict.values():
                street_name_dict['street_name'] = ' '.join(street_name_list[:-1])
                street_name_dict['street_type_abbr'] = street_name_list[(-1)]
