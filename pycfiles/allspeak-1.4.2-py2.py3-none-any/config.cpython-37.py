...
# Embedded file name: build/bdist.linux-x86_64/egg/allrank/config.py
    ...
    def from_dict(cls, config):
        ...
        if config['model'].transformer:
            ...
            if config['model'].transformer.positional_encoding:
                config['model'].transformer.positional_encoding = PositionalEncoding(**(config['model'].transformer).positional_encoding)
    ...
    def _parse_metrics(metrics):
        ...
        for metric_string in metrics:
            try:
                ...
                raise MetricConfigError(metric_string, 'Wrong formatting of metric in config. Expected format: <name>@<at> where name is valid metric name and at is and int')
