...
# Compiled at: 2016-03-06 13:59:33
...
"""
import asyncio, aiohttp, collections, gc, logging, pytest, re, socket, sys, warnings
    ...
    def __exit__(self, exc_type, exc_value, tb):
        ...
        if exc_type is not None:
            ...
            if first_matching is not None:
                ...
                assert 0, '"{}" does not match "{}"'.format(self.expected_regex.pattern, str(first_matching))
                if self.obj_name:
                    __tracebackhide__ = True
                    if not 0:
                        raise AssertionError('{} not triggered by {}'.format(exc_name, self.obj_name))
        else:
            __tracebackhide__ = True
...
class _AssertLogsContext:
    ...
    def __exit__(self, exc_type, exc_value, tb):
        ...
        if len(self.watcher.records) == 0:
            __tracebackhide__ = True
            assert 0, 'no logs of level {} or higher triggered on {}'.format(logging.getLevelName(self.level), self.logger.name)
...
def pytest_ignore_collect(path, config):
    if 'test_py35' in str(path) and sys.version_info < (3, 5, 0):
        return True