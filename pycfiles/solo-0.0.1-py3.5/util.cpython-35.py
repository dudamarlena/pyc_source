...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/solo/configurator/util.py
# Compiled at: 2016-03-06 13:36:59
# Size of source mod 2**32: 6502 bytes
import inspect, functools
from .compat import is_nonstr_iter
from .exceptions import ConfigurationError, CyclicDependencyError

class Sentinel(object):
    __doc__ = '  This class is a copy of ``pyramid.util.Sentinel``.\n    '
...
class TopologicalSorter(object):
    ...
    def add(self, name, val, after=None, before=None):
        ...
        special sentinel values :attr:`solo.configurator.util.FIRST`` or
        :attr:`solo.configurator.util.LAST` representing the first or last positions
    ...
    def sorted(self):
        ...
        if not self.req_before.issubset(has_before):
            raise ConfigurationError('Unsatisfied before dependencies: %s' % ', '.join(sorted(self.req_before - has_before)))
        if not self.req_after.issubset(has_after):
            raise ConfigurationError('Unsatisfied after dependencies: %s' % ', '.join(sorted(self.req_after - has_after)))
        sorted_names = []
        while roots:
            root = roots.pop(0)
