...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/cachesweeper/utils.py
# Compiled at: 2011-01-26 07:35:14
...
def update_cache_token_for_record_with_attribute(instance, token_attr):
    ...
    cache.set(cache_key, token_value_hash, 0)
...
def generate_fragment_cache_key_for_record(record, *cache_keys):
    unique_fragment_key = (':').join(map(lambda key: urlquote(str(key)), cache_keys))
...
class ModelSweeper(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        post_save.connect(self._sweep_signal_handler, sender=self.__class__, dispatch_uid='%s-ModelSweeper' % self.__class__.__name__)
