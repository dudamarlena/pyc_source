...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/cachesweeper/templatetags/cachesweeper_tags.py
# Compiled at: 2011-01-21 09:03:18
...
class CacheSweeperNode(Node):
    ...
    def render(self, context):
        ...
        unique_fragment_cache_keys = [ var.resolve(context) for var in self.cache_key_vars
                                     ]
...
def cachesweeper(parser, token):
    ...
    if len(tokens) < 4:
        raise TemplateSyntaxError("'%s' tag requires at least 3 arguments" % tokens[0])
