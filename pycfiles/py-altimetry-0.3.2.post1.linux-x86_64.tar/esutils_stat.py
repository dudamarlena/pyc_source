...
# [GCC 8.4.0]
# Embedded file name: /home/rdussurget/.virtualenvs/compile.octant.UBU64/lib/python2.7/site-packages/altimetry/externals/esutils_stat.py
...
def histogram(data, bins=None, binsize=1.0, min=None, max=None, rev=False, use_weave=True, verbose=0):
    """
...
def _weave_dohist(data, s, binsize, hist, rev, dorev=False, verbose=0):
    ...
    scipy.weave.inline(code, ['data', 's', 'binsize', 'hist', 'rev', 'dorev'], type_converters=scipy.weave.converters.blitz, verbose=verbose)
...
def histogram2d(x, y, nx=None, ny=None, xbin=None, ybin=None, xmin=None, xmax=None, ymin=None, ymax=None, rev=False):
    if x.size != y.size:
    ...
    ind = xind + nx * yind
    hist = histogram(ind, min=0, max=nx * ny - 1, rev=rev)
    if rev:
        hist, revind = hist
