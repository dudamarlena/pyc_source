...
# [GCC 8.4.0]
# Embedded file name: /home/rdussurget/.virtualenvs/compile.octant.UBU64/lib/python2.7/site-packages/altimetry/tools/map_tools.py
...
class plot_map(Basemap):
    ...
    def __init__(self, *args, **kwargs):
        ...
        lat = np.array(lat)
        if limit is None:
            limit = np.array([lat.min() - 0.5, lon.min() - 0.5, lat.max() + 0.5, lon.max() + 0.5])
        else:
        ...
        self.clon = clon
        self.clat = clat
        self.mcenter = self(self.clon, self.clat)
        cparallel = np.array(self(self.limit[[1, 3]], [self.clat, self.clat]))
    ...
    def pcolormesh(self, *args, **kwargs):
        if len(args) == 1:
            raise Exception('Lon/Lat arguments must be provided')
        lon = np.array(args[0])
        lat = np.array(args[1])
        if lon.ndim != 2:
        ...
        image = (isinstance(args[2], np.ma.masked_array) or np.ma.masked_array)(args[2]) if 1 else args[2]
    ...
    def contour(self, *args, **kwargs):
        ...
        mlon, mlat = self(lon, lat)
        if len(args) == 3:
            return Basemap.contour(self, mlon, mlat, args[2], **kwargs)
        else:
            return Basemap.contour(self, mlon, mlat, args[2], args[3], **kwargs)
    ...
    def contourf(self, *args, **kwargs):
        ...
        mlon, mlat = self(lon, lat)
        if len(args) == 3:
            return Basemap.contourf(self, mlon, mlat, args[2], **kwargs)
        else:
            return Basemap.contourf(self, mlon, mlat, args[2], args[3], **kwargs)

    def drawmapscale(self, lon, lat, lon0, lat0, length, barstyle='simple', units='km', fontsize=9, yoffset=None, labelstyle='simple', fontcolor='k', fillcolor1='w', fillcolor2='k', ax=None, format='%d', zorder=None):
    ...
    def arrow(self, *args, **kwargs):
        ...
        if rg.max() > 0:
            for i in rg:
                ax.arrow(lon[i], lat[i], dx[i], dy[i], **kwargs)

        else:
            ax.arrow(lon, lat, dx, dy, **kwargs)

    def across_track_arrow(self, lon, lat, z, scale=None, ref=None, angle=0.0, **kwargs):
        if scale is None:
            scale = calcul_distance(self.clat, self.clon, self.clat, self.clon + 1) * 1000.0
        else:
            scale *= calcul_distance(self.clat, self.clon, self.clat, self.clon + 1) * 1000.0
        if ref is None:
            sense, orient = track_orient(lon, lat, orient=True)
        else:
            sense = True
            orient = np.pi / 2.0
        across_track = orient + angle - np.pi / 2.0 if sense else orient + angle + np.pi / 2.0
        if isinstance(z, np.ma.masked_array):
            z = z.data
        dx = scale * z * np.cos(across_track)
        dy = scale * z * np.sin(across_track)
        if ref is None:
            self.arrow(lon, lat, dx, dy, **kwargs)
        else:
            reflon = (self.lonmax - self.lonmin) * 0.8 + self.lonmin
            ...
            if type(ref) == bool:
                self.text(reflon, reflat, ('{0:.1f} m/s').format(z[0]))
            else:
                self.text(reflon, reflat, ('{0:.1f} m/s').format(ref))
        return
    ...
    def quiver(self, *args, **kwargs):
        ...
        u = (isinstance(args[2], np.ma.masked_array) or np.ma.masked_array)(args[2]) if 1 else args[2]
        v = (isinstance(args[3], np.ma.masked_array) or np.ma.masked_array)(args[3]) if 1 else args[3]
    ...
    def get_cursor_id(self, x, y):
        ...
        for enum in enumerate(zip(*(lonpt, latpt))):
            ...
            if enum[0] == 0:
                i = [np.argmin(dst).tolist()]
            else:
                i.append(np.argmin(dst))
    ...
    def setup_map(self, londel=1.0, latdel=1.0, *args, **kwargs):
        """
    ...
    def drawbathy(self, fname=None, V=[-250, -2000], Nlevels=None, step=None, colors='k', linestyles='dotted', linewidths=2, **kwargs):
        ...
        if os.path.exists(bathy.path):
            ...
            return self.contour(np.array(self.limit[[1, 3]]), np.array(self.limit[[0, 2]]), np.ones((2,
                                                                                                     2), dtype=float), np.array([1.0, 1.0]), colors=colors, linestyles=linestyles, linewidths=linewidths, **kwargs)
            return
...
def load_bathy(bathy='MENOR', **kwargs):
    if not bathy.set:
        Warning('Bathymetry not set : check your altimetry.config module')
    if bathy.ext == '.nc':
        ncf = nc.nc()
        attr = ncf.attributes(bathy.path)
        convetion = attr[[ k for k in attr if k.upper().startswith('CONVENTION') ][0]].upper()
        if len(convetion) == 0:
            raise Exception('Unknown NetCDF format')
        if convetion == 'COARDS':
            if kwargs.has_key('lon'):
                kwargs['x'] = recale(kwargs.pop('lon'), degrees=True)
            if kwargs.has_key('lat'):
                kwargs['y'] = kwargs.pop('lat')
            ...
            return str
        raise Exception(('Unknown NetCDF convetion {0}').format(convetion))
    else:
        if bathy.ext == '.mat':
            ...
            del str['H0']
            return str
        raise Exception(('Unknown bathymetry file extension ({0})').bathy.ext)