...
# [GCC 8.4.0]
# Embedded file name: /home/michal/workspace/code/newtbase/blastplus/forms.py
# Compiled at: 2018-09-04 11:31:32
# Size of source mod 2**32: 8959 bytes
"""
...
class BlastForm(forms.Form):
    __doc__ = 'Form used for blastn search following validation input sequence.  '
    ...
    word_size_in_form = forms.CharField(widget=(forms.TextInput(blast_settings.BLAST_FORM_INPUTTEXT_ATTRS)), initial=(blast_settings.BLASTN_SETS.default_word_size),
      label='word size')
...
class TBlastnForm(forms.Form):
    __doc__ = 'Form used for tblastn search following validation input sequence.  '
    sequence_in_form = forms.CharField(widget=(forms.Textarea(blast_settings.BLAST_FORM_ATTRS)), label='sequence')
    evalue_in_form = forms.ChoiceField(widget=(forms.Select), choices=(blast_settings.EVALUE_CHOICE_LIST), label='e-value')
    matrix_in_form = forms.ChoiceField(widget=(forms.Select), choices=(blast_settings.MATRIX_CHOICE_LIST), initial=(blast_settings.MATRIX_DEFAULT),
      label='matrix')
    word_size_in_form = forms.CharField(widget=(forms.TextInput(blast_settings.BLAST_FORM_INPUTTEXT_ATTRS)), initial=(blast_settings.TBLASTN_SETS.default_word_size),
      label='word size')
    search_sensitivity_in_form = forms.ChoiceField(widget=(forms.Select), choices=(blast_settings.PROTEIN_SEARCH_SENSITIVE_CHOICE),
      label='sensitivity')
    blast_db_in_form = forms.ChoiceField(widget=(forms.Select), choices=(blast_settings.BLAST_DB_NUCL_CHOICE), label='Database')
...
class BlastpForm(forms.Form):
    __doc__ = 'Form used for blastp search following validation input sequence.  '
    sequence_in_form = forms.CharField(widget=(forms.Textarea(blast_settings.BLAST_FORM_ATTRS)), label='sequence')
    evalue_in_form = forms.ChoiceField(widget=(forms.Select), choices=(blast_settings.EVALUE_CHOICE_LIST), label='e-value')
    matrix_in_form = forms.ChoiceField(widget=(forms.Select), choices=(blast_settings.MATRIX_CHOICE_LIST), initial=(blast_settings.MATRIX_DEFAULT),
      label='matrix')
    word_size_in_form = forms.CharField(widget=(forms.TextInput(blast_settings.BLAST_FORM_INPUTTEXT_ATTRS)), initial=(blast_settings.BLASTP_SETS.default_word_size),
      label='word size')
    search_sensitivity_in_form = forms.ChoiceField(widget=(forms.Select), choices=(blast_settings.PROTEIN_SEARCH_SENSITIVE_CHOICE),
      label='sensitivity')
...
class BlastxForm(forms.Form):
    __doc__ = 'Form used for blastx search following validation input sequence.  '
    sequence_in_form = forms.CharField(widget=(forms.Textarea(blast_settings.BLAST_FORM_ATTRS)), label='sequence')
    evalue_in_form = forms.ChoiceField(widget=(forms.Select), choices=(blast_settings.EVALUE_CHOICE_LIST), label='e-value')
    matrix_in_form = forms.ChoiceField(widget=(forms.Select), choices=(blast_settings.MATRIX_CHOICE_LIST), initial=(blast_settings.MATRIX_DEFAULT),
      label='matrix')
    word_size_in_form = forms.CharField(widget=(forms.TextInput(blast_settings.BLAST_FORM_INPUTTEXT_ATTRS)), initial=(blast_settings.BLASTX_SETS.default_word_size),
      label='word size')
    search_sensitivity_in_form = forms.ChoiceField(widget=(forms.Select), choices=(blast_settings.PROTEIN_SEARCH_SENSITIVE_CHOICE),
      label='sensitivity')
    blast_db_in_form = forms.ChoiceField(widget=(forms.Select), choices=(blast_settings.BLAST_DB_PROT_CHOICE), label='Database')
...
def validate_sequence(sequence: str, sequence_is_as_nucleotide=True):
    ...
    if str(sequence).strip()[0] != '>':
        tmp_seq.write('>seq1\n'.encode())
    tmp_seq.write(str(sequence.encode()))
    tmp_seq.close()
