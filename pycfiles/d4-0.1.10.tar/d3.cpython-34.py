...
# [GCC 8.4.0]
# Embedded file name: /root/d3/d3/d3.py
# Compiled at: 2018-06-17 07:25:04
# Size of source mod 2**32: 7021 bytes
import shutil, d3, argparse, sys, yaml, os, subprocess
VERSION = '0.0.1'
PKG_PATH = d3.__path__[0]
...
def build():
    ...
    build_cmd = 'docker build . -t %s %s' % (
     image_name, build_args_str)
...
def test():
    ...
    image_name = common_params['registry_server'] + '/' + common_params['image_name']
    test_cmd = 'container-structure-test test --image %s %s' % (
     image_name, test_files_str)
...
def login(registry_user, registry_pass):
    ...
    login_cmd = 'docker login %s %s' % (
     login_option, common_params['registry_server'])
...
def push():
    ...
    check_params('common.yaml', check_list, common_params)
    image_name = common_params['registry_server'] + '/' + common_params['image_name']
    push_cmd = 'docker push %s' % image_name
...
def main():
    parser = argparse.ArgumentParser(prog='d3', description='D3: Dock to Develop Dockerfile')
