...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/django_includes/jinja2.py
...
def get_markup(view, response, via='render'):
    ...
    if hasattr(response, 'render'):
        response.render()
    if DEBUG_INCLUDES:
        return Markup('<div style="outline: 1px solid #B9B280; padding: 1em; position: relative;">  <em style="color: white; position: absolute; right: -1px; top: -1px; padding: 0.15em 0.5em; font-size: 10px; background-color: #B9B280">    {} ({}){}  </em>  {} </div>'.format(html.escape(get_view_class(view).__name__), via, headers, response.render().content.decode(encoding='utf-8')))
...
def get_url(route, request, name, args, kwargs):
    return request.build_absolute_uri(reverse(route, kwargs={'token': jwt.encode({'v': name, 'a': args, 'k': kwargs}, settings.SECRET_KEY).decode('utf-8')}))
