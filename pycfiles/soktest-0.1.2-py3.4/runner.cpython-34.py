...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soktest/runner.py
# Compiled at: 2014-08-09 06:18:51
...
class TestRunner(object):
    ...
    class _PrepereSpecyficTestCases(object):
        ...
        def get_test_case(self, name):
            try:
                ...
                raise RuntimeError('Bad test name: %s. Use one of:\n\t%s' % (name, names))

        def validate_test_case(self, test_case, name):
            if test_case is None:
                filter_names = lambda test_name: test_name.endswith(name) and test_name != name
                names = '\n\t'.join(filter(filter_names, TestCase._alltests_dict.keys()))
                raise RuntimeError('Test name is ambiguous. Please provide full package name:\n\t%s' % (
                 names,))
        ...
        def __call__(self):
            ...
            for name in self.tests_names:
                ...
                test_cases.append(self.prepere_suite(test_case, method_name))
    ...
    def prepere_all_test_cases(self, suite):
        ...
        for test_case in TestCase._alltests:
            test_cases.append(suite.loadTestsFromTestCase(test_case))
    ...
    def start_logging(self):
        ...
        logging.basicConfig(level=logging.INFO, format='%(asctime)-15s:%(message)s', filename=filename)
