...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/pluzz/qtpluzz.py
# Compiled at: 2014-05-18 10:38:48
# Size of source mod 2**32: 13374 bytes
try:
...
class QVideoView(QtWebKitWidgets.QWebView):
    ...
    def setup_topbar(self):
        with self.pluzz as (s):
            try:
                cat_l = [hb.LI(hb.CLASS('category') if c != self.category_selected else hb.CLASS('category selected'), hb.A(c, href='javascript:void(0)')) for c in s.get_categories()]
                chn_l = [hb.LI(hb.CLASS('channel') if c != self.channel_selected else hb.CLASS('channel selected'), hb.A(c, href='javascript:void(0)')) for c in s.get_channels()]
    ...
    def build_show_list(self, **args):
        ...
        with self.pluzz as (s):
            try:
                shw_l = [hb.DIV(hb.CLASS('image'), hb.A(hb.CLASS('show'), hb.IMG(src=md['image'], alt=md['title']), hb.DIV(hb.CLASS('caption'), hb.P(hb.CLASS('caption_content'), md['title'])), id=md['id'], href='javascript:void(0)')) for md in s.list(**args)]
            except Exception as err:
                shw_l = [
                 str(err)]
                if self.verbose:
        ...
        doc = hb.HTML(hb.HEAD(self.css_head), hb.BODY(hb.DIV(hb.CLASS('header'), self.topbar_channels, self.topbar_categories), hb.DIV(hb.CLASS('content'), hb.UL(*shw_l))))
    ...
    def build_show_data(self, i):
        with PluzzMovie(i) as (m):
            try:
                ...
                show_md = [
                 ...
                 hb.DIV(hb.SPAN(hb.CLASS('md_key'), _('Crew'), ':'), hb.UL(hb.CLASS('crew_list'), *[hb.LI(hb.UL(hb.SPAN(hb.CLASS('md_key'), ', '.join(p['fonctions']), ':'), hb.SPAN(hb.CLASS('md_val'), ' ', p['prenom'], ' ', p['nom']))) for p in m['personnes']])),
        ...
        doc = hb.HTML(hb.HEAD(self.css_head), hb.BODY(hb.DIV(hb.A('×', href='javascript:void(0)'), id='back'), hb.DIV(hb.CLASS('header')), hb.DIV(hb.CLASS('content'), hb.UL(*show_md))))
        self.setHtml(html_dumps(doc).decode('utf8'))
        self.settings().setUserStyleSheetUrl(QtCore.QUrl(os.path.join(os.path.dirname(__file__), 'resources', 'pypluzz.css')))
        self.page().mainFrame().evaluateJavaScript(TOPBAR_JS)
    ...
    def goto_list(self):
        ...
        self.build_show_list(channel=self.channel_selected, category=self.category_selected, limit=100, sort='alpha')
...
class QPluzz(QtWidgets.QWidget):
    ...
    def on_update_search(self):
        if self.url_txt.text().lstrip().startswith('http://'):
            ...
            if self.re_match_id.match(self.url_txt.text().lstrip()):
                self.web_view.select_show(self.url_txt.text())
            else:
                self.web_view.build_show_list(channel=self.web_view.channel_selected, category=self.web_view.category_selected, query=self.url_txt.text(), limit=100, sort='relevance')
