...
# [GCC 8.4.0]
# Embedded file name: /Users/dauptain/GITLAB/tiny_3d_engine/src/tiny_3d_engine/scene3d.py
# Compiled at: 2020-02-25 17:39:36
...
__all__ = [
 'Scene3D', 'comp_min_max', 'load_geo_as_scene']
...
class Scene3D:
    __doc__ = 'Gather part3D objects to be shown in a renderer'

    def __init__(self, title='Tiny 3D engine', axes=True):
        """Startup."""
        self._data = dict()
        self.title = title
        if axes:
    ...
    def add_or_update_part(self, name, points, conn, color=None):
        ...
        if not color.startswith('#'):
            raise ValueError('part color must be in hexadecimal format: #ffffff for white:' + color)
        self._data[name]['color'] = color

    def add_axes(self, size=1):
        """add axes to the scene"""
        ax_x = Part3D()
        ax_x.add_line([0, 0, 0], [size, 0, 0], 10)
        self.add_or_update_part('axis-x', (ax_x.points), (ax_x.conn), color='#ff0000')
        ax_y = Part3D()
        ax_y.add_line([0, 0, 0], [0, size, 0], 10)
        self.add_or_update_part('axis-y', (ax_y.points), (ax_y.conn), color='#00ff00')
        ax_z = Part3D()
        ax_z.add_line([0, 0, 0], [0, 0, size], 10)
        self.add_or_update_part('axis-z', (ax_z.points), (ax_z.conn), color='#0000ff')
    ...
    def conn(self, partname=None):
        ...
        if self.is_void():
            ...
            for part in list_data[1:]:
                out = np.concatenate((
                 out,
                 ax1_padded(self._data[part]['conn'] + shift, max_nvertex)),
                  axis=0)
    ...
    def dump(self, fname='dummy'):
        ...
        for i, part in enumerate(self.parts()):
            ...
            for m in range(n_elmts):
                cnt.append(' '.join([str(item + 1) for item in self._data[part]['conn'][m, :].tolist()]))
