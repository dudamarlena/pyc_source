...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellalauncher/core/launcher.py
# Compiled at: 2019-08-27 13:12:05
# Size of source mod 2**32: 23347 bytes
"""
...
import os, sys, time, json, random, argparse, importlib, traceback
...
from artellalauncher.core import defines, config, updater, dccselector
from artellalauncher.widgets import console
...
class ArtellaLauncher(QObject, object):
    ...
    LAUNCHER_CONFIG_PATH = artellalauncher.get_launcher_config_path()
    ...
    def create_logger(self):
        ...
        if not os.path.exists(log_path):
            ...
            logger = log.logger
            if '{}_DEV'.format(self.get_clean_name().upper()) in os.environ and os.environ.get('{}_DEV'.format(self.get_clean_name().upper())) in ('True',
                                                                                                                                                   'true'):
    ...
    def init_config(self):
        ...
        if not self.LAUNCHER_CONFIG_PATH or not os.path.isfile(self.LAUNCHER_CONFIG_PATH):
            artellalauncher.logger.error('Launcher Configuration File for Artella Launcher not found! {}'.format(self.LAUNCHER_CONFIG_PATH))
            return
        else:
            with open(self.LAUNCHER_CONFIG_PATH, 'r') as (f):
                launcher_config_data = json.load(f)
            if not launcher_config_data:
                artellalauncher.logger.error('Launcher Configuration File for Artella Project is empty! {}'.format(self.LAUNCHER_CONFIG_PATH))
                return
            else:
                ...
                for dcc_name, dcc_data in dccs.items():
                    ...
                    self._dccs[dcc_name] = DccData(name=dcc_name,
                      icon=dcc_icon,
                      enabled=dcc_enabled,
                ...
                self._dccs or artellalauncher.logger.warning('No DCCs enabled!')
                return
        for dcc_name, dcc_data in self._dccs.items():
            if dcc_data.enabled:
                if not dcc_data.supported_versions:
                    artellalauncher.logger.warning('{0} DCC enabled but no supported versions found in launcher settings. {0} DCC has been disabled!'.format(dcc_name.title()))
            else:
                try:
                    dcc_module = importlib.import_module('artellalauncher.artelladccs.{}dcc'.format(dcc_name.lower().replace(' ', '')))
                except ImportError:
                    artellalauncher.logger.warning('DCC Python module {}dcc not found!'.format(dcc_name.lower().replace(' ', '')))
                    continue
                ...
                if not dcc_data.enabled:
                    ...
                    if not hasattr(dcc_module, fn_name):
                        ...
                        if hasattr(dcc_module, fn_launch):
                            ...
                            artellalauncher.logger.warning('DCC {} has not launch function implemented. Disabling it ...'.format(dcc_data.name))
    ...
    def _get_splash_pixmap(self):
        ...
        if not os.path.isfile(splash_path):
            ...
            splash_files = [f for f in os.listdir(splash_dir) if f.startswith('splash') if os.path.isfile(os.path.join(splash_dir, f))]
            if splash_files:
                ...
                splash_pixmap = artellalauncher.resource.pixmap('splash')
        else:
    ...
    def _setup_splash(self, dcc_name):
        ...
        artella_icon = artellalauncher.resource.icon('artella')
        artella_lbl = QLabel()
        ...
        dcc_icon = artellalauncher.resource.icon(dcc_name.lower())
        dcc_lbl = QLabel()
        ...
        console_icon = artellalauncher.resource.icon('console')
    ...
    def _on_dcc_selected(self, selected_dcc, selected_version):
        ...
        try:
            if not selected_dcc:
                ...
                QApplication.instance().processEvents()
                self._config = cfg
                parser = argparse.ArgumentParser(description=('{} Launcher allows to setup a custom initialization for DCCs. This allows to setup specific paths in an easy way.'.format(self.name)))
                ...
                if args.edit:
                    ...
                    if folders_to_register:
                        if os.environ.get('PYTHONPATH'):
                            ...
                            for p in folders_to_register:
                                ...
                                artellalauncher.logger.debug('Adding path to PYTHONPATH: {}'.format(p))
                    ...
                    try:
                        ...
                        launch_fn or self._console.write_error('Selected DCC: {} has no launch function!'.format(selected_dcc.name))
                        QApplication.instance().processEvents()
                    return
        except Exception as e:
            artellalauncher.logger.error('Error while running {} launcher ...'.format(self.project.name.title()))
            artellalauncher.logger.error('{} | {}'.format(e, traceback.format_exc()))
