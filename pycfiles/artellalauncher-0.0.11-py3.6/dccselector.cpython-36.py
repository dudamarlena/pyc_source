...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellalauncher/core/dccselector.py
...
class DCCButton(base.BaseWidget, object):
    ...
    def __init__(self, dcc, resource, parent=None):
        self._dcc = dcc
    ...
    def ui(self):
        ...
        if len(icon_split) == 1:
            ...
            if len(icon_split) > 1:
                ...
                theme = 'color'
                icon_name = dcc_name
            icon_path = self._resource.get('icons', theme, '{}.png'.format(icon_name))
            if not os.path.isfile(icon_path):
                icon_path = artellalauncher.resource.get('icons', theme, '{}.png'.format(icon_name))
                if not os.path.isfile(icon_path):
                    dcc_icon = artellalauncher.resource.icon('artella')
                else:
                    dcc_icon = artellalauncher.resource.icon(icon_name, theme=theme)
            else:
                dcc_icon = self._resource.icon(icon_name, theme=theme)
...
class DCCSelector(window.ArtellaWindow, object):
    ...
    def __init__(self, launcher, parent=None):
        ...
        super(DCCSelector, self).__init__(name='DCCSelectorWindow',
          title='DCC Selector',
          size=(310, 600),
          fixed_size=False,
          parent=parent)
    ...
    def ui(self):
        ...
        artellalauncher.logger.debug('Using Launcher: {}'.format(self.launcher))
        artellalauncher.logger.debug('DCCs found: {}'.format(self.launcher.dccs))
        if self.launcher.dccs:
            for dcc_name, dcc_data in self.launcher.dccs.items():
                artellalauncher.logger.debug('DCC: {} | {}'.format(dcc_name, dcc_data))
                if not dcc_data.enabled:
                    continue
                if not dcc_data.installation_paths:
                    artellalauncher.logger.warning('No installed versions found for DCC: {}'.format(dcc_name))
                else:
                    dcc_departments = [
                     'All']
                    ...
                    for department in dcc_departments:
                        ...
                        dcc_btn = DCCButton(dcc=dcc_data, resource=(self._launcher.resource))
                        dcc_btn.clicked.connect(self._on_dcc_selected)
                        self.add_dcc_to_department(department, dcc_btn)

        search_folder_icon = artellalauncher.resource.icon('search_folder')
        uninstall_icon = artellalauncher.resource.icon('uninstall')
    ...
    def _on_uninstall(self):
        ...
        if install_path and os.path.isdir(install_path):
            dirs_to_remove = [
             os.path.join(install_path, self.launcher.project.get_clean_name())]
