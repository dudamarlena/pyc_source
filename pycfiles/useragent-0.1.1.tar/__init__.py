...
# [GCC 8.4.0]
# Embedded file name: /Users/don/Development/python/useragent/useragent/__init__.py
# Compiled at: 2012-10-19 16:40:16
import re, json
...
class Parser(object):
    ...
    def parse(self, ua_string):
        ...
        if not matches:
            ...
            opts = self.options
            family, major, minor, patch, patch_minor = (result.groups() + (None, None,
                                                                           None,
                                                                           None,
                                                                           None))[:5]
            ...
            return (matches,
             Result(family=family, major_version=major, minor_version=minor, patch_version=patch, patch_minor_version=patch_minor, version=version))


REGEXES = json.load(open(join(dirname(__file__), 'resources', 'user_agent_data.json')))
PARSERS = {'browser': map(Parser, REGEXES['user_agent_parsers']), 
