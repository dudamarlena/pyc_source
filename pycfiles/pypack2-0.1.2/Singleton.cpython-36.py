...
# Compiled at: 2018-01-13 12:13:09
...
class Singleton:
    ...
    def __call__(self, *arg, **kwarg):
        if self._cls not in self._Singleton__instances:
            ...
            if '__call__' in dir(self._Singleton__instances[self._cls]):
    ...
    def get_instance(self):
        if isinstance(self, Singleton):
            if self._cls in list(self._Singleton__instances.keys()):
                return self._Singleton__instances[self._cls]
    ...
    @classmethod
    def get(cls, g):
        if isinstance(g, Singleton):
            if g._cls in list(Singleton._Singleton__instances.keys()):
