...
# Compiled at: 2020-04-19 00:12:01
# Size of source mod 2**32: 18617 bytes
...
class Pipeline(object):
    ...
    def __init__(self, _input=None, pipeline=None, _output='terminal', _format='json', use_db=None, tool_base='stanza', boost='pathos'):
    ...
    def annotate(self):
        if self._use_db != None:
            ...
            self.documents = PluginManager().call_plugin_db(plugin_name=(self._use_db),
    ...
    def run(self, _tool_name):
        ...
        for document in tqdm((self.documents), desc='Processing document(s)'):
            ...
            if self._use_db != None:
                ...
                return self.output(data_formating, self._id_dataset, JSONEncoder().encode(document['name']).replace('"', ''), plugin_name)
    ...
    def ssplit(self, document):
        ...
        if self._tool_base == 'stanza':
            self.documents = PluginManager().call_plugin_nlp(plugin_name='stanza',
              _id_pool=(self._id_pool),
              lang=(self._custom_pipeline['lang']),
              document=document,
              pipeline=['tokenize'])
        ...
        if self._tool_base == 'stanfordcorenlp':
            self.documents = PluginManager().call_plugin_nlp(plugin_name='stanfordcorenlp',
              _id_pool=(self._id_pool),
              lang=(self._custom_pipeline['lang']),
              document=document,
              pipeline=['ssplit'])
