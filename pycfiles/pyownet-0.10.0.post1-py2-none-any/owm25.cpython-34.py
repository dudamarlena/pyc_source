...
# Embedded file name: build/bdist.linux-x86_64/egg/pyowm/weatherapi25/owm25.py
# Compiled at: 2018-12-21 14:52:45
# Size of source mod 2**32: 63707 bytes
__doc__ = '\nModule containing the PyOWM library main entry point\n'
from time import time
from pyowm import constants
from pyowm.weatherapi25.configuration25 import OBSERVATION_URL, GROUP_OBSERVATIONS_URL, FIND_OBSERVATIONS_URL, THREE_HOURS_FORECAST_URL, DAILY_FORECAST_URL, CITY_WEATHER_HISTORY_URL, STATION_WEATHER_HISTORY_URL, FIND_STATION_URL, STATION_URL, BBOX_STATION_URL, BBOX_CITY_URL
...
class OWM25(owm.OWM):
    ...
    def __init__(self, parsers, API_key=None, cache=nullcache.NullCache(), language='en', subscription_type='free', use_ssl=False):
    ...
    def is_API_online(self):
        ...
        """
        params = {'q': 'London,GB'}
        uri = http_client.HttpClient.to_url(OBSERVATION_URL, self._API_key, self._subscription_type)
        try:
    ...
    def weather_at_place(self, name):
        ...
        uri = http_client.HttpClient.to_url(OBSERVATION_URL, self._API_key, self._subscription_type, self._use_ssl)
    ...
    def weather_at_ids(self, ids_list):
        ...
        uri = http_client.HttpClient.to_url(GROUP_OBSERVATIONS_URL, self._API_key, self._subscription_type, self._use_ssl)
        _, json_data = self._wapi.cacheable_get_json(uri, params=params)
    ...
    def weather_at_places(self, pattern, searchtype, limit=None):
        ...
        *'like'* (matches names that are similar to the supplied pattern).
        ...
        if limit is not None:
            params['cnt'] = limit - 1
        uri = http_client.HttpClient.to_url(FIND_OBSERVATIONS_URL, self._API_key, self._subscription_type, self._use_ssl)
        _, json_data = self._wapi.cacheable_get_json(uri, params=params)
        return self._parsers['observation_list'].parse_JSON(json_data)
    ...
    def weather_at_station(self, station_id):
        ...
        uri = http_client.HttpClient.to_url(STATION_URL, self._API_key, self._subscription_type, self._use_ssl)
        _, json_data = self._wapi.cacheable_get_json(uri, params=params)
        return self._parsers['observation'].parse_JSON(json_data)
    ...
    def weather_at_stations_in_bbox(self, lat_top_left, lon_top_left, lat_bottom_right, lon_bottom_right, cluster=False, limit=None):
        ...
        assert type(limit) in (int, type(None)), "'limit' must be an int or None"
        ...
        if limit is not None and limit < 1:
            raise ValueError("'limit' must be None or greater than zero")
        params = {'bbox': ','.join([str(lon_top_left), str(lat_top_left),
                  str(lon_bottom_right),
                  str(lat_bottom_right)]), 
         'cluster': 'yes' if cluster else 'no'}
        if limit is not None:
            params['cnt'] = limit
        uri = http_client.HttpClient.to_url(BBOX_STATION_URL, self._API_key, self._subscription_type, self._use_ssl)
        _, json_data = self._wapi.cacheable_get_json(uri, params=params)
        return self._parsers['observation_list'].parse_JSON(json_data)
    ...
    def weather_at_places_in_bbox(self, lon_left, lat_bottom, lon_right, lat_top, zoom=10, cluster=False):
        ...
        params = {'bbox': ','.join([str(lon_left),
                  ...
                  str(zoom)]), 
         'cluster': 'yes' if cluster else 'no'}
        uri = http_client.HttpClient.to_url(BBOX_CITY_URL, self._API_key, self._subscription_type, self._use_ssl)
        _, json_data = self._wapi.cacheable_get_json(uri, params=params)
        return self._parsers['observation_list'].parse_JSON(json_data)
    ...
    def daily_forecast(self, name, limit=None):
        ...
        if limit is not None:
            ...
            if limit is not None:
                params['cnt'] = limit
            uri = http_client.HttpClient.to_url(DAILY_FORECAST_URL, self._API_key, self._subscription_type, self._use_ssl)
            _, json_data = self._wapi.cacheable_get_json(uri, params=params)
            forecast = self._parsers['forecast'].parse_JSON(json_data)
            if forecast is not None:
                pass
        ...
        if start is None and end is None:
            ...
            if start is not None and end is not None:
                ...
                if unix_start >= unix_end:
                    raise ValueError('Error: the start time boundary must precede the end time!')
                current_time = time()
                if unix_start > current_time:
                    raise ValueError('Error: the start time boundary must precede the current time!')
    ...
    def station_at_coords(self, lat, lon, limit=None):
        ...
        if limit is not None:
            params['cnt'] = limit
        uri = http_client.HttpClient.to_url(FIND_STATION_URL, self._API_key, self._subscription_type, self._use_ssl)
        _, json_data = self._wapi.cacheable_get_json(uri, params=params)
    ...
    def _retrieve_station_history(self, station_ID, limit, interval):
        ...
        if limit is not None:
            params['cnt'] = limit
        uri = http_client.HttpClient.to_url(STATION_WEATHER_HISTORY_URL, self._API_key, self._subscription_type, self._use_ssl)
        _, json_data = self._wapi.cacheable_get_json(uri, params=params)
        station_history = self._parsers['station_history'].parse_JSON(json_data)
        if station_history is not None:
    ...
    def __repr__(self):
        return '<%s.%s - API key=%s, OWM Weather API version=%s, subscription type=%s, PyOWM version=%s, language=%s>' % (
         __name__,
