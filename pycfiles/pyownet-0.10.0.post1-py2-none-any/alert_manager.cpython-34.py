...
# Embedded file name: build/bdist.linux-x86_64/egg/pyowm/alertapi30/alert_manager.py
...
class AlertManager:
    ...
    def create_trigger(self, start, end, conditions, area, alert_channels=None):
        ...
        the_time_period = {'start': {'expression': 'after', 
                   'amount': delta_millis_start}, 
         'end': {'expression': 'after', 
        ...
        status, payload = self.http_client.post(TRIGGERS_URI, params={'appid': self.API_key}, data=dict(time_period=the_time_period, conditions=the_conditions, area=the_area), headers={'Content-Type': 'application/json'})
    ...
    def get_triggers(self):
        ...
        """
        status, data = self.http_client.get_json(TRIGGERS_URI, params={'appid': self.API_key}, headers={'Content-Type': 'application/json'})
    ...
    def get_trigger(self, trigger_id):
        ...
        status, data = self.http_client.get_json(NAMED_TRIGGER_URI % trigger_id, params={'appid': self.API_key}, headers={'Content-Type': 'application/json'})
    ...
    def update_trigger(self, trigger):
        ...
        status, _ = self.http_client.put(NAMED_TRIGGER_URI % trigger.id, params={'appid': self.API_key}, data=dict(time_period=the_time_period, conditions=the_conditions, area=the_area), headers={'Content-Type': 'application/json'})
    ...
    def get_alert(self, alert_id, trigger):
        ...
        assert isinstance(trigger.id, str), 'Value must be a string'
        status, data = self.http_client.get_json(NAMED_ALERT_URI % (trigger.id, alert_id), params={'appid': self.API_key}, headers={'Content-Type': 'application/json'})
    ...
    def delete_all_alerts_for(self, trigger):
        ...
        assert trigger is not None
        assert isinstance(trigger.id, str), 'Value must be a string'
        status, _ = self.http_client.delete(ALERTS_URI % trigger.id, params={'appid': self.API_key}, headers={'Content-Type': 'application/json'})
    ...
    def delete_alert(self, alert):
        ...
        status, _ = self.http_client.delete(NAMED_ALERT_URI % (alert.trigger_id, alert.id), params={'appid': self.API_key}, headers={'Content-Type': 'application/json'})