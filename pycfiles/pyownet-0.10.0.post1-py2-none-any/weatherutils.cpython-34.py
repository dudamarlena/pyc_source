...
# Embedded file name: build/bdist.linux-x86_64/egg/pyowm/utils/weatherutils.py
...
def status_is(weather, status, weather_code_registry):
    ...
    *WeatherCodeRegistry* object.
    ...
    """
    weather_status = weather_code_registry.status_for(weather.get_weather_code()).lower()
...
def find_closest_weather(weathers_list, unixtime):
    ...
    if not is_in_coverage(unixtime, weathers_list):
        raise api_response_error.NotFoundError('Error: the specified time is ' + 'not included in the weather coverage range')
