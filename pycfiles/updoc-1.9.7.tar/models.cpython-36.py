...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/Updoc/updoc/models.py
# Compiled at: 2017-11-19 12:35:41
# Size of source mod 2**32: 9026 bytes
import ipaddress, os, plistlib, shutil
...
class Keyword(models.Model):
    ...
    @classmethod
    def get(cls, name):
        if cls._Keyword__cache is None:
            cls._Keyword__cache = ObjectCache(lambda key: cls.objects.get_or_create(value=key)[0])
        return cls._Keyword__cache.get(name)
...
class UploadDoc(models.Model):
    ...
    class Meta:
        __doc__ = 'Meta informations on this model'
    ...
    def docset_feed(self):
        return settings.SERVER_BASE_URL[:-1] + reverse('updoc:docset_feed', kwargs={'doc_id':self.id, 
         'doc_name':self.name})
        ...
        if os.path.isfile(path):
            ...
            if os.path.isdir(path):
                ...
                if os.path.isdir(documents_dir):
                    if os.path.isfile(plist_path):
                        ...
                        if 'dashIndexFilePath' in content:
                            searched_files = [
                             content['dashIndexFilePath']] + searched_files
                index_path = ''
                for index in searched_files:
                    ...
                    if len(ldir) == 1:
                        if os.path.isfile(os.path.join(path, ldir[0])):
                            index_path = ldir[0]
...
class ProxyfiedHost(models.Model):
    host = models.CharField((_('URL to proxify')), db_index=True, max_length=255, blank=True, default='', help_text='Can be a regexp on URL (like http://*.example.com:*/*) or a subnet (like 192.168.0.0/24). Leave it blank to use as default value.')
    proxy = models.CharField((_('Proxy to use')), db_index=True, max_length=255, blank=True, default='', help_text=(_('e.g. proxy.example.com:8080. Leave it empty if direct connection. Several values can be given, separated by semi-colons (;).')))
    priority = models.IntegerField((_('Priority')), db_index=True, default=0, blank=True, help_text=(_('Low priorities are written first in proxy.pac')))
