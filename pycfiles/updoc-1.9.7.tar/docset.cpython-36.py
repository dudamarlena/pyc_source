...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/Updoc/updoc/docset.py
# Compiled at: 2017-11-07 13:56:03
...
"""
import os, plistlib, re, sqlite3, tarfile, tempfile, shutil
    ...
    def prepare(self):
        ...
        with tempfile.TemporaryDirectory(dir=(settings.FILE_UPLOAD_TEMP_DIR)) as (dirname):
            ...
            with open(os.path.join(root, 'Info.plist'), 'wb') as (fd):
                info = {'CFBundleIdentifier':slugify(self.doc.name), 
                 'CFBundleName':self.doc.name,  'DocSetPlatformFamily':slugify(self.doc.name), 
                 'isDashDocset':True,  'isJavaScriptEnabled':True}
                if index_path:
