...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/Updoc/updoc/process.py
# Compiled at: 2017-06-18 15:46:24
...
def clean_archive(root_path: str):
    ...
    for root, dirnames, filenames in os.walk(root_path):
        ...
        while index < len(dirnames):
            ...
            if dirname in frozenset({'__pycache__', '.hg', '__MACOSX', '.git', '.svn'}):
                try:
...
def process_uploaded_file(doc: UploadDoc, temp_file, original_filename: str):
    ...
    * Index content
    ...
    """
    if not isinstance(doc, UploadDoc):
        ...
        if original_filename[-7:] in ('.tar.gz', '.tar.xz') or original_filename[-8:] == '.tar.bz2' or original_filename[-4:] in ('.tar',
                                                                                                                                  '.tbz',
                                                                                                                                  '.tgz',
                                                                                                                                  '.txz'):
            ...
            for member in tar_file.getmembers():
                ...
                if member.isdir():
                    os.makedirs(dst_path, mode=511, exist_ok=True)
                else:
                    if member.issym():
