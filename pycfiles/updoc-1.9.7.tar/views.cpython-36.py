...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/Updoc/updoc/views.py
# Compiled at: 2017-11-29 13:26:03
# Size of source mod 2**32: 20588 bytes
import datetime, mimetypes, os, re, stat, tarfile, tempfile, uuid, zipfile, markdown
...
def send_file_replace_url(request, filename, allow_replace=False):
    ...
    if not was_modified_since(request.META.get('HTTP_IF_MODIFIED_SINCE'), statobj[stat.ST_MTIME], statobj[stat.ST_SIZE]):
        ...
        if allow_replace and extension in frozenset({'html', 'css', 'txt', 'js', 'md'}) and os.path.getsize(filename) < 102400:
...
def index(request):
    if request.user.is_authenticated:
        ...
        if request.user.is_anonymous:
            ...
            keywords_with_counts = Keyword.objects.all().annotate(count=(Count('uploaddoc'))).filter(count__gt=0).order_by('-count')[0:15]
        ...
        if request.user.is_anonymous:
            if not settings.PUBLIC_BOOKMARKS:
                rss_roots = []
        rss_roots = RssRoot.objects.all().order_by('name')
    template_values = {'recent_checked':recent_checked, 
     'title':_('Updoc'),  'rss_roots':rss_roots,  'recent_uploads':recent_uploads, 
     'keywords':keywords_with_counts,  'show_all_link':True, 
...
def show_favorite(request, root_id=None):
    if request.user.is_authenticated:
        set_websocket_topics(request)
    elif request.user.is_anonymous and not settings.PUBLIC_BOOKMARKS:
        roots = []
        favorites = []
    ...
    template_values = {'roots':roots, 
     'values':favorites,  'current_root_name':current_root_name,  'current_id':int(root_id)}
...
def show_proxies(request):
    ...
    if request.user.is_anonymous:
        if not settings.PUBLIC_PROXIES:
            proxies = []
            defaults = ''
    if not defaults:
        defaults = 'DIRECT'
    template_values = {'proxies':proxies, 
     'model':ProxyfiedHost,  'defaults':defaults}
...
def my_docs(request):
    ...
    template_values = {'uploads':uploads,  'title':_('My documents'),  'rw_urls':rw_urls,  'rw_form':form, 
     'editable':True,  'has_search_results':False}
...
def delete_url(request, url_id):
    if request.method != 'POST':
        ...
        messages.info(request, _('The replacement of %(src)s by %(dst)s has been removed') % {'src':url.src, 
         'dst':url.dst})
        return HttpResponseRedirect(reverse('updoc:my_docs'))
...
def upload(request):
    ...
    template_values = {'form':form, 
     'title':_('Upload a new file'),  'root_host':settings.SERVER_BASE_URL[:-1]}
...
def upload_doc_progress(request):
    ...
    doc.save()
    scall(request, 'updoc.process_file', to=[SERVER], doc_id=(doc.id), filename=(tmp_file.name), original_filename=(uploaded_file.name))
...
def upload_doc_api(request):
    ...
    if not form.is_valid():
        ...
        scall(request, 'updoc.process_file', to=[SERVER], doc_id=(doc.id), filename=(tmp_file.name), original_filename=(os.path.basename(form.cleaned_data['filename'])))
...
def show_doc(request, doc_id, path=''):
    ...
    if not user:
        ...
        directory = os.path.isfile(full_path) or list_directory(root_path, path, view, view_arg='path', view_kwargs={'doc_id': doc.id},
          dir_view_name=view,
          dir_view_arg='path',
          dir_view_kwargs={'doc_id': doc.id},
          show_files=True,
          show_dirs=True,
          show_parent=(bool(path)),
          show_hidden=False)
        template_values = {'directory':directory,  'doc':doc,  'editable':editable,  'title':str(doc),  'keywords':' '.join([keyword.value for keyword in doc.keywords.all()]), 
         'doc_id':doc_id}
        ...
        if full_path.endswith('.md'):
            ...
            path_components = [
             (
              reverse(view_name, kwargs={'doc_id':doc_id,  'path':''}), _('root'))]
            ...
            template_values = {'doc':doc, 
             'editable':editable,  'title':str(doc),  'path':path,  'paths':path_components,  'keywords':' '.join([keyword.value for keyword in doc.keywords.all()]), 
             'doc_id':doc_id}
            try:
...
def show_search_results(request):
    ...
    if request.user.is_authenticated:
        ...
        if request.user.is_anonymous:
            ...
            docs = doc_id or UploadDoc.objects.all()
            if len(pattern) > 3:
            ...
            docs = None
    template_values = {'uploads':docs, 
     'title':_('Search results'),  'rw_form':None,  'editable':False, 
     'es_data':es_data,  'doc_id':doc_id}
    return TemplateResponse(request, 'updoc/my_docs.html', template_values)
...
def show_all_docs(request):
    ...
    template_values = {'recent_checked':recent_checked, 
     'title':_('Updoc'),  'rss_roots':rss_roots,  'recent_uploads':recent_uploads, 
     'search':search,  'keywords':[],  'show_all_link':False, 
