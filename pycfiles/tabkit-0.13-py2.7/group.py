...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/tabkit/awk/group.py
# Compiled at: 2016-11-01 20:53:54
import ast, sys
from itertools import chain
from collections import OrderedDict
from .map import _join_exprs, Statement, Assignment, OmittedAssignment, Expression, SimpleExpression, AwkNodeVisitor, OutputAwkGenerator
...
class GrpProgram(object):
    ...
    def __init__(self, init_aggr=None, grp_keys=None, grp_exprs=None, grp_output=None, aggr_exprs=None, aggr_output=None):
    ...
    def __add__(self, other):
        return GrpProgram(self.init_aggr + other.init_aggr, self.grp_keys + other.grp_keys, self.grp_exprs + other.grp_exprs, self.grp_output + other.grp_output, self.aggr_exprs + other.aggr_exprs, self.aggr_output + other.aggr_output)
...
class AggregateExpression(Expression):
    ...
    def __init__(self, code, type, children=None, aggregators=None):
        ...
        if children:
            combined_aggregators.append(chain.from_iterable(child.aggregators for child in children if isinstance(child, AggregateExpression)))
        if aggregators:
...
class AggregateFunction(object):
    ...
    def __init__(self, var_name, *args, **kwargs):
        ...
        self._set_code_attr('code', var_name=var_name, *(arg.code for arg in args), **kwargs)
...
class AggregateAwkNodeVisitor(AwkNodeVisitor):
    aggregate_funcs = {'cumsum': CumulativeSumFunction, 
       'sum': SumFunction, 
...
class AggregateAwkGenerator(AggregateAwkNodeVisitor, OutputAwkGenerator):
    ...
    def visit(self, node):
        ...
        if isinstance(expr, Statement) and not isinstance(expr, AggregateExpression) and expr.children and all(isinstance(child, AggregateExpression) for child in expr.children):
    ...
    def visit_AggregateFunction(self, node):
        ...
        return SimpleAggregateExpressions(code=var_name, type=func.type, children=args, aggregators=[
         func])

    def visit_Num(self, node):
        return AggregateExpression.from_expression(super(AggregateAwkGenerator, self).visit_Num(node))

    def visit_Str(self, node):
        return AggregateExpression.from_expression(super(AggregateAwkGenerator, self).visit_Str(node))

    def visit_Name(self, node):
        if node.id in self.group_context:
            expr = self.group_context[node.id]
            return AggregateExpression(code=expr.code, type=expr.type)
