...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/tabkit/awk/map.py
# Compiled at: 2016-11-01 18:25:35
import ast, sys
...
class MapProgram(object):
    ...
    def __add__(self, other):
        return MapProgram(self.row_exprs + other.row_exprs, self.output_cond + other.output_cond, self.output + other.output)
...
class AwkNodeVisitor(ast.NodeVisitor):
    ...
    def visit_Compare(self, node):
        ...
        if op not in self.compareops:
            raise TabkitException("Syntax error: compare operation '%s' is not suported" % op.__name__)
        return [
    ...
    def visit_BoolOp(self, node):
        ...
        if op not in self.boolops:
            raise TabkitException("Syntax error: boolean operation '%s' is not supported" % op.__name__)
    ...
    def visit_BinOp(self, node):
        ...
        if op not in self.binops:
            raise TabkitException("Syntax error: binary operation '%s' is not supported" % op.__name__)
        return [
         self.visit(node.left), self.visit(node.right)]

    funcs = {'int': Function('int(%s)', TabkitTypes.int), 
...
class AwkGenerator(AwkNodeVisitor):
    ...
    def visit_Compare(self, node):
        ...
        return Expression(code='%s%s%s' % (left_expr.code, op, right_expr.code), type=infer_type(op, left_expr.type, right_expr.type), children=[
         left_expr, right_expr])

    def visit_BoolOp(self, node):
        exprs = super(AwkGenerator, self).visit_BoolOp(node)
        op = self.boolops[type(node.op)]
        return Expression(code='(%s)' % op.join(expr.code for expr in exprs), type=infer_type(op, *(expr.type for expr in exprs)), children=exprs)
    ...
    def visit_Num(self, node):
        return Expression(code=str(node.n), type=type(node.n))

    def visit_Str(self, node):
        return Expression(code='"%s"' % node.s.replace('"', '\\"'), type=str)

    def visit_Function(self, node):
        func = self.funcs[node.func.id]
        args = super(AwkGenerator, self).visit_Function(node)
        return Expression(code=func.code(args), type=func.type, children=args)

    def visit_Name(self, node):
        if node.id in self.data_desc:
            field_index = self.data_desc.index(node.id)
            return SimpleExpression(code='$%d' % (field_index + 1), type=self.data_desc.fields[field_index].type)
...
class OutputAwkGenerator(AwkGenerator):
    ...
    def visit_Assign(self, node):
        ...
        assign_expr = Expression(code=target_var_name, type=value.type, children=[
         value])
        self.context[target_name] = assign_expr
        return Assignment(code='%s=%s' % (assign_expr.code, value.code), value=value)
