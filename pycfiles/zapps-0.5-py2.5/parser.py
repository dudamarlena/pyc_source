...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/zapps/parser.py
# Compiled at: 2008-04-25 10:15:33
from zapps.support import *
...
class ParserDescription(Parser):
    ...
    def ClauseC(self, tokens):
        ...
        if _token_ == 'PLUS':
            ...
            return Question(ClauseD)
        elif _token_ not in ('"ignore"', '"token"', '"option"', '":"', '"parser"',
                             'ATTR', 'COLON', 'PACK'):
            return ClauseD
        else:
...
yapps_options = [
 ('context-insensitive-scanner', 'context-insensitive-scanner', 'Scan all tokens (see docs)')]
...
def generate(inputfilename, outputfilename='', dump=0, **flags):
    ...
    DIVIDER = '\n%%\n'
    (preparser, postparser) = (None, None)
