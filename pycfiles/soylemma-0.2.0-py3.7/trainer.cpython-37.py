...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soylemma/trainer.py
# Compiled at: 2019-09-30 14:54:21
# Size of source mod 2**32: 9867 bytes
        ...
        if decompose(surface[0])[0] != decompose(lw[(-1)])[0]:
            ...
            if len(canon[0]) == 1:
                if len(canon[1]) == 1:
                    ...
                    if not surf_cho_r == canon_cho_r:
                        if not (canon_jung_r == ' ' and surf_jong_r == canon_cho_r):
                            if canon_cho_r != 'ㅇ':
                                raise surf_cho_r == 'ㅊ' and canon_cho_r == 'ㅈ' or ValueError('어미 첫글자의 초성이 다른 경우 eojeol={}, {}/{} + {}/{}'.format(eojeol, lw, lt, rw, rt))
        return (
         surface, canon)
...
def train_model_using_sejong_corpus_cleaner(local_repository_path, table_path, show_exception=False):
    ...
    return (
     adjectives, verbs, eomis, rules, exceptions, lemmatizing_count)