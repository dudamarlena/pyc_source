...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dessia_common/core.py
# Compiled at: 2020-04-10 11:33:20
# Size of source mod 2**32: 25380 bytes
"""

"""
import warnings, math, random, copy
...
try:
    ...
    from dessia_common.core_protected import inspect_arguments, recursive_instantiation, recursive_type, JSONSCHEMA_HEADER, jsonschema_from_annotation, prettyname, set_default_value, TYPING_EQUIVALENCES, deserialize_argument
...
class DessiaObject(protected_module.DessiaObject if not _open_source else object):
    ...
    def __init__(self, name: str='', **kwargs):
        implements_eq = hasattr(self, '__eq__') and hasattr(self, '__hash__') and self.__class__.__eq__ is not object.__eq__ and self.__class__.__hash__ is not object.__hash__
        if self._standalone_in_db:
            if not self._generic_eq:
                if not implements_eq:
    ...
    def __eq__(self, other_object):
        ...
        if full_classname(self) != full_classname(other_object) or self.__dict__.keys() != other_object.__dict__.keys():
            return False
        dict_ = {k:v for k, v in self.__dict__.items() if k not in self._non_eq_attributes}
        other_dict = {k:v for k, v in other_object.__dict__.items() if k not in self._non_eq_attributes}
    ...
    def __getstate__(self):
        dict_ = {k:v for k, v in self.__dict__.items() if k not in self._non_serializable_attributes}
        return dict_
    ...
    def volmdlr_volume_model(self, frame=None):
        if hasattr(self, 'volmdlr_primitives'):
            ...
            if hasattr(self, 'volmdlr_primitives_step_frames'):
                return vm.MovingVolumeModel(self.volmdlr_primitives(), self.volmdlr_primitives_step_frames())
            if frame is None:
                frame = vm.OXYZ
            try:
    ...
    def cad_export(self, fcstd_filepath=None, istep=0, python_path='python', freecad_lib_path='/usr/lib/freecad/lib', export_types=[
 'fcstd']):
        """
        ...
        if fcstd_filepath is None:
            ...
            model.FreeCADExport(fcstd_filepath, python_path=python_path, freecad_lib_path=freecad_lib_path,
    ...
    def _display_angular(self):
        ...
        if hasattr(self, 'babylon_data'):
            display.append({'angular_component':'cad_viewer',  'data':self.babylon_data()})
        else:
            if hasattr(self, 'volmdlr_primitives') or self.__class__.volmdlr_volume_model is not DessiaObject.volmdlr_volume_model:
                model = self.volmdlr_volume_model()
                display.append({'angular_component':'cad_viewer',  'data':model.babylon_data()})
        if hasattr(self, 'plot_data'):
            display.append({'angular_component':'plot_data',  'data':self.plot_data()})
        return display
...
class Parameter(DessiaObject):
    ...
    def __init__(self, lower_bound, upper_bound, periodicity=None, name=''):
        DessiaObject.__init__(self, name=name, lower_bound=lower_bound,
    ...
    def are_values_equal(self, value1, value2, tol=0.01):
...
class Evolution(DessiaObject):
    ...
    def _display_angular(self):
        displays = [
         {'angular_component':'app-evolution1d', 
          'table_show':False, 
          'evolution':[
           self.evolution], 
          'label_y':[
...
class CombinationEvolution(DessiaObject):
    ...
    def __init__(self, evolution1: List[Evolution], evolution2: List[Evolution], title1: str='x', title2: str='y', name: str=''):
...
def number2factor(number):
    ...
    if number:
        factors = list(set(reduce(list.__add__, ([i, number // i] for i in factor_range if number % i == 0))))
        grids = [(factor_x, int(number / factor_x)) for factor_x in factors if (number / factor_x).is_integer()]
    else:
        grids = []
    return grids
...
def number3factor(number, complete=True):
    ...
    if number:
        ...
        if not complete:
            ...
            grids = [(factor_x, factor_y, int(number / (factor_x * factor_y))) for factor_x in factors if (number / (factor_x * factor_y)).is_integer() for factor_y in factors]
        return grids
    return []
...
def dict_to_object(dict_, class_=None):
    ...
    if class_ is None:
        if 'object_class' in working_dict:
            class_ = get_python_class_from_class_name(working_dict['object_class'])
    else:
        if class_ is not None:
            if hasattr(class_, 'dict_to_object'):
                if class_.dict_to_object.__func__ is not DessiaObject.dict_to_object.__func__:
...
def serialize_typing(typing_):
    if hasattr(typing_, '_name'):
        if typing_._name == 'List':
...
def deserialize(serialized_element):
    if isinstance(serialized_element, dict):
        ...
        if isinstance(serialized_element, (list, tuple)):
