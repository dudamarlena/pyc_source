...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dessia_common/workflow.py
# Compiled at: 2020-04-10 11:33:20
# Size of source mod 2**32: 58983 bytes
"""

"""
import inspect, time, tempfile, json
from importlib import import_module
import webbrowser, networkx as nx
...
class Block(dc.DessiaObject):
     ...
     'properties':{'inputs':{'type':'array', 
       ...
       'items':{'type':'object', 
        'classes':[
         ...
         'dessia_common.workflow.TypedVariabledessia_common.workflow.VariableWithDefaultValue',
    ...
    def _method_jsonschemas(self):
        ...
        for i, input_ in enumerate(self.inputs):
            ...
            annotation = (
             str(i), input_.type_)
            annotation_jsonschema = dc.jsonschema_from_annotation(annotation=annotation, jsonschema_element=current_dict,
              order=i,
            ...
            if not isinstance(input_, (VariableWithDefaultValue, TypedVariableWithDefaultValue)):
                ...
                current_dict.update(dc.set_default_value(current_dict, str(i), input_.default_value))
...
class InstanciateModel(Block):
    _jsonschema = dc.dict_merge(Block._jsonschema, {'title':'Instantiate model Base Schema', 
     'required':[
      'object_class'], 
     'properties':{'object_class': {'type':'string', 
                       'editable':True, 
                       'examples':[
                        'Nom']}}})
    ...
    def __init__(self, model_class, name=''):
        ...
        outputs = [
         TypedVariable(self.model_class, 'Instanciated object')]
    ...
    def to_dict(self):
        dict_ = dc.DessiaObject.base_dict(self)
        dict_.update({'model_class':self.model_class.__name__,  'model_class_module':self.model_class.__module__})
        return dict_

    @classmethod
    @set_block_variable_names_from_dict
    def dict_to_object(cls, dict_):
        class_ = getattr(import_module(dict_['model_class_module']), dict_['model_class'])
...
class ClassMethod(Block):
    ...
    def __init__(self, class_, method_name, name=''):
        ...
        outputs = [
         TypedVariable(dc.type_from_annotation(method.__annotations__['return'], method.__module__), 'method result of {}'.format(self.method_name))]
        Block.__init__(self, inputs, outputs, name=name)
    ...
    def equivalent(self, other_block):
        if not Block.equivalent(self, other_block):
            return False
        return self.class_.__name__ == other_block.class_.__name__ and self.method_name == other_block.method_name

    def to_dict(self):
        dict_ = dc.DessiaObject.base_dict(self)
        dict_.update({'method_name':self.method_name,  'class_':self.class_.__name__, 
    ...
    @set_block_variable_names_from_dict
    def dict_to_object(cls, dict_):
        class_ = getattr(import_module(dict_['class_module']), dict_['class_'])
        return cls(class_=class_, method_name=(dict_['method_name']),
          name=(dict_['name']))
...
class ModelMethod(Block):
    _jsonschema = dc.dict_merge(Block._jsonschema, {'title':'Model method Base Schema', 
     'required':[
      'model_class', 'method_name'], 
     'properties':{'model_class':{'type':'string', 
       'editable':True, 
       'examples':[
        'Nom']}, 
    ...
    def __init__(self, model_class, method_name, name=''):
        ...
        if 'return' in method.__annotations__:
            return_output = TypedVariable(dc.type_from_annotation(method.__annotations__['return'], method.__module__), 'method result of {}'.format(self.method_name))
        else:
            return_output = Variable('method result of {}'.format(self.method_name))
        outputs = [return_output,
         TypedVariable(model_class, 'model at output {}'.format(self.method_name))]
        if name == '':
    ...
    def equivalent(self, other_block):
        if not Block.equivalent(self, other_block):
            return False
        return self.model_class.__name__ == other_block.model_class.__name__ and self.method_name == other_block.method_name

    def to_dict(self):
        dict_ = dc.DessiaObject.base_dict(self)
        dict_.update({'method_name':self.method_name,  'model_class':self.model_class.__name__, 
    ...
    def dict_to_object(cls, dict_):
        class_ = getattr(import_module(dict_['model_class_module']), dict_['model_class'])
        return cls(model_class=class_, method_name=(dict_['method_name']),
          name=(dict_['name']))
...
class Function(Block):
    ...
    def __init__(self, function, name=''):
        ...
        for arg_name in inspect.signature(function).parameters.keys():
            inputs.append(TypedVariable(dc.type_from_annotation(function.__annotations__[arg_name]), arg_name))

        outputs = [TypedVariable(dc.type_from_annotation(function.__annotations__['return']), 'Output function')]
        Block.__init__(self, inputs, outputs, name=name)

    def equivalent_hash(self):
        return int(hash(self.function.__name__) % 1000000.0)
...
class Sequence(Block):
    ...
    def evaluate(self, values):
        return [
         [values[var] for var in self.inputs]]
...
class ForEach(Block):
    ...
    def equivalent_hash(self):
        return int(hash(self.workflow) % 1000000.0)

    def equivalent(self, other_block):
        if not Block.equivalent(self, other_block):
            return False
        return self.workflow == other_block.workflow and self.workflow.variable_indices(self.workflow_iterable_input) == other_block.workflow.variable_indices(other_block.workflow_iterable_input)
    ...
    def evaluate(self, values):
        values_workflow = {var2:values[var1] for var1, var2 in zip(self.inputs, self.workflow.inputs)}
        output_values = []
...
class Filter(Block):
    ...
    def equivalent_hash(self):
        return int(sum([hash(v) for f in self.filters for v in f.values()]) % 1000000.0)
        ...
        for object_ in objects:
            ...
            for filter_ in self.filters:
                ...
                if operator == 'gte':
                    if attribute < bound:
                        valid = False
                    elif operator == 'lt' and attribute >= bound:
                        valid = False
                    if operator == 'gt':
                        if attribute <= bound:
                            valid = False
...
class ModelAttribute(Block):
    ...
    def evaluate(self, values):
        return [
         getattr(values[self.inputs[0]], self.attribute_name)]
...
class Substraction(Block):
    ...
    def evaluate(self, values):
        return [
         values[self.inputs[0]] - values[self.inputs[1]]]
...
class Pipe(dc.DessiaObject):
    _jsonschema = {'definitions':{},  '$schema':'http://json-schema.org/draft-07/schema#', 
     ...
     'properties':{'input_variable':{'type':'object', 
       'editable':True, 
       'classes':[
        'dessia_common.workflow.Variable',
        'dessia_common.workflow.TypedVariabledessia_common.workflow.VariableWithDefaultValue',
        'dessia_common.workflow.TypedVariableWithDefaultValue']}, 
      'output_variable':{'type':'object', 
       'editable':True, 
       'classes':[
        'dessia_common.workflow.Variable',
        'dessia_common.workflow.TypedVariabledessia_common.workflow.VariableWithDefaultValue',
...
class Workflow(Block):
    ...
    _jsonschema = {'definitions':{},  '$schema':'http://json-schema.org/draft-07/schema#', 
     ...
     'properties':{'blocks':{'type':'array', 
       ...
       'items':{'type':'object', 
        'classes':[
         'dessia_common.workflow.InstanciateModel',
    ...
    def __init__(self, blocks, pipes, output, *, imposed_variable_values=None, name=''):
        ...
        for variable in self.variables:
            if variable not in self.imposed_variable_values and len(nx.ancestors(self.graph, variable)) == 0:
    ...
    def __hash__(self):
        ...
        block_hash = int(sum([b.equivalent_hash() for b in self.blocks]) % 1000000.0)
    ...
    def __eq__(self, other_workflow):
        if hash(self) != hash(other_workflow):
            ...
            return Block.equivalent(self, other_workflow) or False
    ...
    def __deepcopy__(self, memo=None):
        ...
        nonblock_variables = [v._Workflow__deepcopy(memo=memo) for v in self.nonblock_variables]
        pipes = []
        for pipe in self.pipes:
        ...
        output = self.variable_from_index(self.variable_indices(self.output), blocks, nonblock_variables)
        ...
        copied_workflow = Workflow(blocks, pipes, output, imposed_variable_values=imposed_variable_values,
          name=(self.name))
    ...
    def _display_angular(self):
        ...
        displays.extend([
         {'angular_component':'workflow',  'blocks':data['blocks'], 
    ...
    def dict_to_arguments(self, dict_, method):
        arguments_values = {}
        for i, input_ in enumerate(self.inputs):
            if isinstance(input_, (VariableWithDefaultValue, TypedVariableWithDefaultValue)):
                if not isinstance(input_, (VariableWithDefaultValue, TypedVariableWithDefaultValue)) or str(i) in dict_:
                    value = dict_[str(i)]
    ...
    def layout(self, min_horizontal_spacing=300, min_vertical_spacing=200, max_height=800, max_length=1500):
        ...
        for i, distance in enumerate(sorted(elements_by_distance.keys())[::-1]):
            ...
            for j, element in enumerate(elements_by_distance[distance]):
                coordinates[element] = (
                 i * horizontal_spacing, (j + 0.5) * vertical_spacing)

        return coordinates

    def plot_graph(self):
        pos = nx.kamada_kawai_layout(self.graph)
        nx.draw_networkx_nodes((self.graph), pos, (self.blocks), node_shape='s',
          node_color='grey')
    ...
    def run(self, input_variables_values, verbose=False, progress_callback=None):
        log = ''
        ...
        log_line = 'Starting workflow run at {}'.format(time.strftime('%d/%m/%Y %H:%M:%S UTC', time.gmtime(start_time)))
        log += log_line + '\n'
        if verbose:
            print(log_line)
        progress = 0
        while something_activated:
            ...
            for pipe in self.pipes:
                if activated_items[pipe] or activated_items[pipe.input_variable]:
            ...
            for block in self.blocks:
                all_inputs_activated = activated_items[block] or True
                ...
                if all_inputs_activated:
                    if verbose:
                        ...
                        if verbose:
                            print(log_line)
                    output_values = block.evaluate({i:values[i] for i in block.inputs})
    ...
    def mxgraph_data(self):
        nodes = []
        for block in self.blocks:
            nodes.append({'name':block.__class__.__name__,  'inputs':[{'name':i.name,  'workflow_input':i in self.inputs,  'has_default':hasattr(block, 'default')} for i in block.inputs], 
             'outputs':[{'name':o.name,  'workflow_output':o in self.outputs} for o in block.outputs]})
    ...
    def jointjs_data(self):
        ...
        for block in self.blocks:
            ...
            block_data.update({'inputs':[{'name':i.name,  'is_workflow_input':i in self.inputs,  'has_default_value':hasattr(i, 'default_value')} for i in block.inputs], 
        ...
        nonblock_variables = []
        for variable in self.nonblock_variables:
            nonblock_variables.append({'name':variable.name,  'is_workflow_input':variable in self.inputs, 
        ...
        data.update({'blocks':blocks,  'nonblock_variables':nonblock_variables, 
         'edges':edges})
        return data

    def plot_jointjs(self):
        env = Environment(loader=(PackageLoader('dessia_common', 'templates')), autoescape=(select_autoescape(['html', 'xml'])))
        template = env.get_template('workflow_jointjs.html')
        data = self.jointjs_data()
        options = {}
        rendered_template = template.render(blocks=(json.dumps(data['blocks'])), nonblock_variables=(json.dumps(data['nonblock_variables'])),
          edges=(data['edges']),
          options=options)
            ...
            if hasattr(pipe.input_variable, 'type_') and hasattr(pipe.output_variable, 'type_'):
                ...
                if type1 != type2:
                    try:
                        ...
                        consistent = True
                        if not consistent:
                            raise TypeError('inconsistent pipe type from pipe input {} to pipe output {}: {} incompatible with {}'.format(pipe.input_variable.name, pipe.output_variable.name, pipe.input_variable.type_, pipe.output_variable.type_))
...
class WorkflowBlock(Block):
    ...
    @set_block_variable_names_from_dict
    def dict_to_object(cls, dict_):
        b = cls((Workflow.dict_to_object(dict_['workflow'])), name=(dict_['name']))
        return b
...
class WorkflowRun(dc.DessiaObject):
    ...
    _jsonschema = {'definitions':{},  '$schema':'http://json-schema.org/draft-07/schema#', 
     ...
     'required':[],  'properties':{'workflow':{'type':'object', 
       'title':'Workflow', 
       'classes':[
        'dessia_common.workflow.Workflow'], 
       'order':0, 
       'editable':False, 
       'description':'Workflow'}, 
      'output_value':{'type':'array', 
       'items':{'type':'object', 
        'classes':'*'}, 
       'title':'Values', 
    ...
    def __eq__(self, other_workflow_run):
        if hasattr(self.output_value, '__iter__'):
            equal_output = hasattr(self.output_value, '__iter__') and all([v == other_v for v, other_v in zip(self.output_value, other_workflow_run.output_value)])
        else:
    ...
    def __hash__(self):
        if hasattr(self.output_value, '__iter__'):
            hash_output = int(sum([hash(v) for v in self.output_value]) % 1000000.0)
        else:
            hash_output = hash(self.output_value)
        return hash(self.workflow) + int(hash_output % 1000000.0)
    ...
    def _display_angular(self):
        ...
        for block in self.workflow.blocks:
            if isinstance(block, Filter):
                ...
                values = [{f['attribute']:dc.getdeepattr(v, f['attribute']) for f in filter_display[0]['filters']} for v in self.output_value]
                filter_display[0]['values'] = values
                filter_display[0]['datasets'] = [
                 {'label':'Results',  'values':list(range(len(values))), 
                  'color':'#99b4d6'}]
    ...
    def to_dict(self):
        dict_ = dc.DessiaObject.base_dict(self)
        dict_.update({'workflow':self.workflow.to_dict(),  'start_time':self.start_time, 
         'end_time':self.end_time, 
         'execution_time':self.execution_time, 
         'log':self.log})
        if self.output_value is not None:
            dict_.update({'output_value':dc.serialize_sequence(self.output_value),  'output_value_type':dc.recursive_type(self.output_value)})
        return dict_
    ...
    def dict_to_object(cls, dict_):
        ...
        if 'output_value' in dict_ and 'output_value_type' in dict_:
            ...
            output_value = None
        return cls(workflow=workflow, output_value=output_value, start_time=(dict_['start_time']),
    ...
    for iargument, argument in enumerate(args_specs.args[1:]):
        if argument not in ('self', 'progress_callback'):
            ...
            if iargument >= nargs - ndefault_args:
                ...
                inputs.append(TypedVariableWithDefaultValue(type_, default_value=default_value,
                  name=argument))
            else:
