...
# [GCC 8.4.0]
# Embedded file name: /pmc/Work/kolotev/0git/.github/django-enumfield-named-choices/django_enumfield_named_choices/tests/test_enum.py
# Compiled at: 2019-08-20 19:03:24
# Size of source mod 2**32: 9835 bytes
...
from django_enumfield_named_choices.enum import Enum
from django_enumfield_named_choices.exceptions import InvalidStatusOperationError
from django_enumfield_named_choices.tests.models import Person, PersonStatus, Lamp, LampState, Beer, BeerStyle, BeerState, LabelBeer, NamedPersonStatus
from parameterized import parameterized
test_class_params = [
 [
  'PersonStatus', PersonStatus],
 [
  'NamedPersonStatus', NamedPersonStatus]]

class EnumFieldTest(TestCase):

    @parameterized.expand(test_class_params)
    ...
    @parameterized.expand(test_class_params)
    def test_enum_field_save(self, test_name, PersonStatus):
    ...
    @parameterized.expand(test_class_params)
    def test_enum_field_modelform_create(self, test_name, PersonStatus):
    ...
    @parameterized.expand(test_class_params)
    def test_enum_field_modelform(self, test_name, PersonStatus):
        person = Person.objects.create()
...
class EnumTest(TestCase):
    ...
    @parameterized.expand(test_class_params)
    def test_label(self, test_name, PersonStatus):
    ...
    def test_choices_int(self):
        self.assertEqual(tuple(dict(list(PersonStatus.choices())).keys()), tuple(dict(list(PersonStatus.items())).values()))

    def test_choices_str(self):
        self.assertEqual(tuple(dict(list(NamedPersonStatus.choices())).keys()), tuple((k.lower() for k in dict(list(NamedPersonStatus.items())).keys())))
    ...
    def test_interface(self):
        self.assertEqual(PersonStatus.interface, int)
        self.assertEqual(NamedPersonStatus.interface, str)