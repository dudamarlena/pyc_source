...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fever/reader/fever_predictor.py
...
class FEVERPredictor(Predictor):
    ...
    def _json_to_instance(self, json_dict: JsonDict) -> Instance:
        claim_id = json_dict['id'] if 'id' in json_dict else None
        claim = json_dict['claim']
        label = json_dict['label'] if 'label' in json_dict else None
        evidence = json_dict['predicted_sentences']
        evidence = [[(None, item[0], item[1]) for item in evidence]]
    ...
    def predict_instance(self, instance: Instance) -> JsonDict:
        if len(instance.fields['premise'].tokens) == 0:
            out_dict = {'label_logits':[0, 0, 0],  'label_probs':[
              0, 0, 0]}
            out_dict['label_logits'][self._model.vocab.get_token_index('NOT ENOUGH INFO', 'labels')] = 1
            out_dict['label_probs'][self._model.vocab.get_token_index('NOT ENOUGH INFO', 'labels')] = 1
            return out_dict
...
class FEVEROraclePredictor(FEVERPredictor):
    ...
    def _json_to_instance(self, json_dict: JsonDict) -> Instance:
        ...
        claim = json_dict['claim']
        label = json_dict['label'] if 'label' in json_dict else None
        all_evidence_groups = json_dict['evidence']
        evidence = [[(None, item[2], item[3]) for item in evidence] for evidence in all_evidence_groups]
        generated = self._dataset_reader._instance_generator.generate_instances(self._dataset_reader, evidence, claim)[0]
        return self._dataset_reader.text_to_instance(claim_id, None, generated['evidence'], claim, label)