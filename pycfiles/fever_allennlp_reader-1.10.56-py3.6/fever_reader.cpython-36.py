...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fever/reader/fever_reader.py
...
class FEVERDatasetReader(DatasetReader):
    ...
    def __init__(self, database='data/fever/fever.db', wiki_tokenizer=None, claim_tokenizer=None, token_indexers=None, instance_generator=None):
        super().__init__()
    ...
    def generate_instances(self, claim_id: int, evidence: List[List[Tuple[(int, str, int)]]], claim: str, label: str=None) -> Iterable[Instance]:
    ...
    def _read(self, file_path: str):
        with open(file_path, 'r') as (f):
            for line in f:
                ...
                evidence = [[(item[1], item[2], item[3]) for item in group] for group in evidence]
