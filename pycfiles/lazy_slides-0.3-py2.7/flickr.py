...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/lazy_slides/flickr/flickr.py
...
API = '/services/rest'
API_KEY = 'aadfb4777800783f088b2e38471518ca'
API_SECRET = '8e2bb36ce7d70d81'
email = None
password = None
AUTH = False
debug = False
...
class Photo(object):
    ...
    __readonly = [
     'id', 'secret', 'server', 'farm', 'isfavorite', 'license', 'rotation',
    ...
    def __init__(self, id, owner=None, dateuploaded=None, title=None, description=None, ispublic=None, isfriend=None, isfamily=None, cancomment=None, canaddmeta=None, comments=None, tags=None, secret=None, isfavorite=None, server=None, farm=None, license=None, rotation=None, url=None):
    ...
    def _load_properties(self):
        ...
        owner = photo.owner
        self.__owner = User(owner.nsid, username=owner.username, realname=owner.realname, location=owner.location)
        ...
        if hasattr(photo.tags, 'tag'):
            if isinstance(photo.tags.tag, list):
                self.__tags = [ Tag(tag.id, User(tag.author), tag.raw, tag.text) for tag in photo.tags.tag ]
            else:
                tag = photo.tags.tag
    ...
    def setMeta(self, title=None, description=None):
        ...
        _dopost(method, auth=True, title=title, description=description, photo_id=self.id)
        self.__title = title
        self.__description = description
        return
    ...
    def getGalleryList(self, per_page='', page=''):
        ...
        try:
            data = _doget(method, photo_id=self.id, per_page=per_page, page=page)
        except FlickrError:
            return
...
class Photoset(object):
    ...
    def __init__(self, id, title, primary, photos=0, description='', secret='', server=''):
        self.__id = id
        self.__title = title
    ...
    def editPhotos(self, photos, primary=None):
        ...
        _dopost(method, auth=True, photoset_id=self.id, primary_photo_id=primary.id, photo_ids=ids)
    ...
    def editMeta(self, title=None, description=None):
        ...
        if description is None:
            description = self.description
        _dopost(method, auth=True, title=title, description=description, photoset_id=self.id)
        self.__title = title
        self.__description = description
        return True
    ...
    def create(cls, photo, title, description=''):
        ...
        data = _dopost(method, auth=True, title=title, description=description, primary_photo_id=photo.id)
        set = Photoset(data.rsp.photoset.id, title, Photo(photo.id), photos=1, description=description)
        return set
...
class User(object):
    ...
    def __init__(self, id, username=None, isadmin=None, ispro=None, realname=None, location=None, firstdate=None, count=None):
    ...
    photos_firstdate = property(lambda self: self._general_getattr('photos_firstdate'))
    photos_firstdatetaken = property(lambda self: self._general_getattr('photos_firstdatetaken'))
    photos_count = property(lambda self: self._general_getattr('photos_count'))
    ...
    def _general_getattr(self, var):
        ...
        if getattr(self, '_%s__%s' % (self.__class__.__name__, var)) is None and not self.__loaded:
            self._load_properties()
        ...
        if not getattr(data.rsp.photosets, 'photoset', None):
            ...
            if isinstance(data.rsp.photosets.photoset, list):
                for photoset in data.rsp.photosets.photoset:
                    sets.append(Photoset(photoset.id, photoset.title.text, Photo(photoset.primary), secret=photoset.secret, server=photoset.server, description=photoset.description.text, photos=photoset.photos))
    ...
    def getPublicFavorites(self, per_page='', page=''):
        return favorites_getPublicList(user_id=self.id, per_page=per_page, page=page)

    def getFavorites(self, per_page='', page=''):
        return favorites_getList(user_id=self.id, per_page=per_page, page=page)

    def getGalleries(self, per_page='', page=''):
        return galleries_getList(user_id=self.id, per_page=per_page, page=page)


class Group(object):
    """Flickr Group Pool"""

    def __init__(self, id, name=None, members=None, online=None, privacy=None, chatid=None, chatcount=None):
        self.__loaded = False
        self.__id = id
        self.__name = name
...
class Gallery(object):
    ...
    def __init__(self, id, owner=None, title=None, description=None, date_create=None, date_update=None, count_photos=None, count_videos=None, primary_photo_id=None, primary_photo_server=None, primary_photo_farm=None, primary_photo_secret=None):
        self.__loaded = False
        self.__url = None
        self.__id = id
        self.__owner = owner
    ...
    def addPhoto(self, photo, comment=''):
        ...
        _dopost(method, auth=True, photo_id=photo.id, gallery_id=self.id, comment=comment)
        return True
    ...
    def editMeta(self, title='', description=''):
        ...
        if title == '':
            title = self.title
        _dopost(method, auth=True, gallery_id=self.id, title=title, description=description)
        return True

    def editPhoto(self, photo, comment):
        """Change the comment for the given Photo."""
        method = 'flickr.galleries.editPhoto'
        _dopost(method, auth=True, gallery_id=self.id, photo_id=photo.id, comment=comment)
        return True
    ...
    def editPhotos(self, primary_photo, *photos):
        ...
        _dopost(method, auth=True, gallery_id=self.id, primary_photo_id=primary_photo.id, photo_ids=photo_ids)
        return True
    ...
    def getPhotos(self, per_page='', page='', **extras):
        ...
        *extras (optional): A comma-delimited list of extra information
        ...
        data = _doget(method, gallery_id=self.id, per_page=per_page, page=page, extras=extras)
        photos = {}
        for photo in data.rsp.photos.photo:
...
def photos_search(user_id='', auth=False, tags='', tag_mode='', text='', min_upload_date='', max_upload_date='', min_taken_date='', max_taken_date='', license='', per_page='', page='', sort='', safe_search='', content_type=''):
    """Returns a list of Photo objects.

    If auth=True then will auth the user.  Can see private etc
    """
    method = 'flickr.photos.search'
    data = _doget(method, auth=auth, user_id=user_id, tags=tags, text=text, min_upload_date=min_upload_date, max_upload_date=max_upload_date, min_taken_date=min_taken_date, max_taken_date=max_taken_date, license=license, per_page=per_page, page=page, sort=sort, safe_search=safe_search, content_type=content_type, tag_mode=tag_mode)
    photos = []
    if 'photo' in data.rsp.photos.__dict__:
        if isinstance(data.rsp.photos.photo, list):
            ...
            photos = [
             _parse_photo(data.rsp.photos.photo)]
    return photos


def photos_search_pages(user_id='', auth=False, tags='', tag_mode='', text='', min_upload_date='', max_upload_date='', min_taken_date='', max_taken_date='', license='', per_page='', page='', sort=''):
    """Returns the number of pages for the previous function (photos_search())
    """
    method = 'flickr.photos.search'
    data = _doget(method, auth=auth, user_id=user_id, tags=tags, text=text, min_upload_date=min_upload_date, max_upload_date=max_upload_date, min_taken_date=min_taken_date, max_taken_date=max_taken_date, license=license, per_page=per_page, page=page, sort=sort)
...
def people_getPublicPhotos(user_id, per_page='', page=''):
    ...
    photos = []
    if hasattr(data.rsp.photos, 'photo'):
        if isinstance(data.rsp.photos.photo, list):
            for photo in data.rsp.photos.photo:
                photos.append(_parse_photo(photo))
...
def favorites_getList(user_id='', per_page='', page=''):
    ...
    data = _doget(method, auth=True, user_id=user_id, per_page=per_page, page=page)
    photos = []
    if isinstance(data.rsp.photos.photo, list):
        for photo in data.rsp.photos.photo:
            photos.append(_parse_photo(photo))
...
def favorites_getPublicList(user_id, per_page='', page=''):
    ...
    data = _doget(method, auth=False, user_id=user_id, per_page=per_page, page=page)
    photos = []
    if isinstance(data.rsp.photos.photo, list):
        for photo in data.rsp.photos.photo:
            photos.append(_parse_photo(photo))
...
def groups_pools_getGroups():
    ...
    if isinstance(data.rsp.groups.group, list):
        for group in data.rsp.groups.group:
            groups.append(Group(group.id, name=group.name, privacy=group.privacy))
...
def contacts_getPublicList(user_id):
    ...
    try:
        if isinstance(data.rsp.contacts.contact, list):
            return [ User(user.nsid, username=user.username) for user in data.rsp.contacts.contact
                   ]
    except AttributeError:
...
def galleries_create(title, description, primary_photo_id=None):
    ...
    if primary_photo_id is None:
        _dopost(method, auth=True, title=title, description=description, primary_photo_id=primary_photo_id)
...
def galleries_getList(user_id='', per_page='', page=''):
    ...
    if isinstance(data.rsp.galleries.gallery, list):
        ...
        galleries = [
         _parse_gallery(data.rsp.galleries.gallery)]
    return galleries
...
def _parse_photo(photo):
    ...
    secret = photo.secret
    server = photo.server
    p = Photo(photo.id, owner=owner, title=title, ispublic=ispublic, isfriend=isfriend, isfamily=isfamily, secret=secret, server=server)
    return p
...
def unmarshal(element):
    ...
    if childElements:
        ...
        text = ('').join([ e.data for e in element.childNodes if isinstance(e, minidom.Text)
                         ])
...
class Auth():
    ...
    def getToken(self, frob):
        ...
        signature_hash = hashlib.md5(sig_str).hexdigest()
        data = _doget(method, auth=False, api_sig=signature_hash, api_key=API_KEY, frob=frob)
