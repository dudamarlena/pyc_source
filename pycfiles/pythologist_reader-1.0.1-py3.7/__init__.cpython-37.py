...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/pythologist_reader/__init__.py
# Compiled at: 2019-03-30 16:23:27
# Size of source mod 2**32: 38484 bytes
import pandas as pd, numpy as np, h5py, os, json, sys, shutil
...
class CellFrameGeneric(object):
    ...
    def __init__(self):
        ...
        self.data_tables = {'cells':{'index':'cell_index', 
         ...
         'cell_tags':{'index':'db_id', 
          'columns':[
           'tag_index', 'cell_index']}, 
         'cell_measurements':{'index':'measurement_index', 
          'columns':[
           'cell_index', 'statistic_index', 'feature_index', 'channel_index', 'value']}, 
         'measurement_features':{'index':'feature_index', 
          'columns':[
           'feature_label']}, 
         'measurement_channels':{'index':'channel_index', 
          'columns':[
           'channel_label', 'channel_abbreviation', 'image_id']}, 
         ...
         'segmentation_images':{'index':'db_id', 
          'columns':[
           'segmentation_label', 'image_id']}, 
         'regions':{'index':'region_index', 
          'columns':[
           'region_label', 'region_size', 'image_id']}, 
         'cell_interactions':{'index':'db_id', 
          'columns':[
           'cell_index', 'neighbor_cell_index', 'pixel_count', 'touch_distance']}, 
    ...
    def set_data(self, table_name, table):
        ...
        if table_name not in self.data_tables:
            raise ValueError("Error table name doesn't exist in defined formats")
        if set(list(table.columns)) != set(self.data_tables[table_name]['columns']):
            raise ValueError("Error column names don't match defined format\n" + str(list(table.columns)) + '\n' + str(self.data_tables[table_name]['columns']))
        if table.index.name != self.data_tables[table_name]['index']:
            raise ValueError("Error index name doesn't match defined format")
        self._data[table_name] = table.loc[:, self.data_tables[table_name]['columns']].copy()
    ...
    def set_regions(self, regions, use_processed_region=True, unset_label='undefined', verbose=False):
        ...
        if use_processed_region:
            remainder = self.processed_image
        for i, label in enumerate(labels):
            my_image = regions[label]
            if use_processed_region:
                my_image = my_image & self.processed_image
        ...
        regions2 = pd.DataFrame({'region_label':labels,  'region_size':sizes, 
         'image_id':ids})
        ...
        if verbose:
            sys.stderr.write(str(recode.loc[recode['new_region_label'].isna()].shape[0]) + ' cells with centroids beyond the processed region are being dropped\n')
        recode = recode.loc[(~recode['new_region_label'].isna())].copy()
        recode = recode.drop(columns='region_index').reset_index().merge((regions2[['region_label']].reset_index()),
          left_on='new_region_label',
          right_on='region_label').drop(columns=[
         'region_label', 'new_region_label']).set_index('cell_index')
    ...
    def to_hdf(self, h5file, location='', mode='w'):
        ...
        for table_name in self.data_tables.keys():
            ...
            data_table.to_hdf(h5file, (location + '/data/' + table_name),
              mode='a',
              format='table',
              complib='zlib',
              complevel=9)
    ...
    def cell_map(self):
        ...
        """
        if 'cell_map' not in list(self.get_data('segmentation_images')['segmentation_label']):
            return
        cmid = self.get_data('segmentation_images').set_index('segmentation_label').loc[('cell_map',
                                                                                         'image_id')]
    ...
    def edge_map(self):
        ...
        """
        if 'edge_map' not in list(self.get_data('segmentation_images')['segmentation_label']):
            return
        cmid = self.get_data('segmentation_images').set_index('segmentation_label').loc[('edge_map',
                                                                                         'image_id')]
        return map_image_ids(self.get_image(cmid)).rename(columns={'id': 'cell_index'})
    ...
    def segmentation_info(self):
        ...
        """
        return self.edge_map().reset_index().groupby(['cell_index']).count()[['x']].rename(columns={'x': 'edge_pixels'}).merge(self.cell_map().reset_index().groupby(['cell_index']).count()[['x']].rename(columns={'x': 'area_pixels'}),
          left_index=True,
    ...
    def set_interaction_map(self, touch_distance=1):
        ...
        full = self.cell_map()
        edge = self.edge_map()
        if full is None or edge is None:
            return
        d1 = edge.reset_index()
        d1['key'] = 1
        ...
        im = full.reset_index().merge(d3, on=['x', 'y']).query('cell_index!=neighbor_cell_index').drop_duplicates().groupby([
         'cell_index', 'neighbor_cell_index']).count()[['key']].reset_index().rename(columns={'key': 'pixel_count'})
    ...
    def get_channels(self, all=False):
        ...
        """
        if all:
            return self.get_data('measurement_channels')
    ...
    def get_raw(self, feature_label, statistic_label, all=False, channel_abbreviation=True):
        ...
        if not all:
            channels = channels.loc[(~channels['channel_label'].isin(self.excluded_channels))]
        else:
            ...
            return channel_abbreviation or measure
        temp = dict(zip(self.get_data('measurement_channels')['channel_label'], self.get_data('measurement_channels')['channel_abbreviation']))
    ...
    def cdf(self):
        ...
        temp1 = self.get_data('cells').drop(columns='phenotype_index').merge((self.get_data('regions')),
        ...
        if temp2 is not None:
            temp2 = temp2.apply(lambda x: dict(zip(list(x.index), list(x))), 1).reset_index().rename(columns={0: 'scored_calls'}).set_index('cell_index')
            temp1 = temp1.merge(temp2, left_index=True, right_index=True)
        else:
            temp1['scored_calls'] = np.nan
        temp3 = self.phenotype_calls().apply(lambda x: dict(zip(list(x.index), list(x))), 1).reset_index().rename(columns={0: 'phenotype_calls'}).set_index('cell_index')
        temp1 = temp1.merge(temp3, left_index=True, right_index=True)
        temp4 = self.default_raw()
        if temp4 is not None:
            temp4 = temp4.apply(lambda x: dict(zip(list(x.index), list(x))), 1).reset_index().rename(columns={0: 'channel_values'}).set_index('cell_index')
            temp1 = temp1.merge(temp4, left_index=True, right_index=True)
        else:
            temp1['channel_values'] = np.nan
        neighbors = self.interaction_map().groupby('cell_index').apply(lambda x: dict(zip(x['neighbor_cell_index'].astype(int), x['pixel_count'].astype(int)))).reset_index().rename(columns={0: 'neighbors'}).set_index('cell_index')
        edge_length = self.edge_map().reset_index().groupby('cell_index').count()[['x']].rename(columns={'x': 'edge_length'})
        edge_length['edge_length'] = edge_length['edge_length'].astype(int)
        cell_area = self.cell_map().reset_index().groupby('cell_index').count()[['x']].rename(columns={'x': 'cell_area'})
        ...
        temp5.loc[(temp5['neighbors'].isna(), 'neighbors')] = temp5.loc[(temp5['neighbors'].isna(), 'neighbors')].apply(lambda x: {})
        temp1 = temp1.merge(temp5, left_index=True, right_index=True, how='left')
        temp1.loc[(temp1['neighbors'].isna(), 'neighbors')] = np.nan
        ...
        temp1['phenotype_label'] = temp1.apply(lambda x: _get_phenotype(x['phenotype_calls']), 1)
        temp1['frame_shape'] = temp1.apply(lambda x: self.shape, 1)
        return CellDataFrame(temp1)

    def binary_df(self):
        temp1 = self.phenotype_calls().stack().reset_index().rename(columns={'level_1':'binary_phenotype', 
         0:'score'})
    ...
    def cell_df(self):
        celldf = self.get_data('cells').merge(self.get_data('regions').rename(columns={'image_id': 'region_image_id'}),
          left_on='region_index',
          right_index=True).merge((self.get_data('phenotypes')),
          left_on='phenotype_index', right_index=True).merge((self.segmentation_info()),
...
class CellSampleGeneric(object):
    ...
    def to_hdf(self, h5file, location='', mode='w'):
        ...
        for frame_id in self.frame_ids:
            ...
            frame.to_hdf(h5file, (location + '/frames/' + frame_id),
              mode='a')
    ...
    def interaction_map(self):
        ...
        frames = pd.concat(frames).reset_index(drop=True)
        return frames.merge(fc, on=['frame_id', 'cell_index']).merge(fc.rename(columns={'sample_cell_index':'neighbor_sample_cell_index',  'cell_index':'neighbor_cell_index'}),
          on=[
         'frame_id', 'neighbor_cell_index'])
...
class CellProjectGeneric(object):
    ...
    def __init__(self, h5path, mode='r'):
        ...
        if mode == 'r':
            if not os.path.exists(h5path):
                raise ValueError('Cannot read a file that does not exist')
        if mode == 'w' or mode == 'r+':
            f = h5py.File(self.h5path, mode)
            if '/samples' not in f.keys():
                f.create_group('/samples')
            elif '/meta' not in f.keys():
    ...
    def concat(self, path, array_like, overwrite=False, verbose=False):
        ...
        if verbose:
            sys.stderr.write('Copy the first element\n')
        cpi = arr[0].copy(path, output_mode='r+', overwrite=overwrite)
        if len(arr) == 1:
            return
        for project in array_like[1:]:
            if verbose:
                sys.stderr.write('Add project ' + str(project.id) + ' ' + str(project.project_name) + '\n')
            for s in project.sample_iter():
                if verbose:
                    sys.stderr.write('   Add sample ' + str(s.id) + ' ' + str(s.sample_name) + '\n')
                cpi.append_sample(s)
    ...
    def append_sample(self, sample):
        ...
        """
        if self.mode == 'r':
            raise ValueError('Error: cannot write to a path in read-only mode.')
        else:
            sample.to_hdf((self.h5path), location=('samples/' + sample.id), mode='a')
            current = self.key
            if current is None:
                current = pd.DataFrame([
                 {'sample_id':sample.id,  'sample_name':sample.sample_name}])
                current.index.name = 'db_id'
            else:
                iteration = max(current.index) + 1
            addition = pd.DataFrame([
             {'db_id':iteration,  'sample_id':sample.id, 
    ...
    def project_name(self, name):
        if self.mode == 'r':
            raise ValueError('cannot write if read only')
        f = h5py.File(self.h5path, 'r+')
    ...
    def interaction_map(self):
        ...
        samples = pd.concat(samples).reset_index(drop=True)
        return samples.merge(fc, on=['sample_id', 'frame_id', 'cell_index']).merge(fc.rename(columns={'project_cell_index':'neighbor_project_cell_index',  'cell_index':'neighbor_cell_index'}),
          on=[
         'sample_id', 'frame_id', 'neighbor_cell_index'])
    ...
    def key(self):
        ...
        if 'info' in [x for x in f]:
            ...
            f.close()
            return val or None
        return pd.read_hdf(self.h5path, 'info')
