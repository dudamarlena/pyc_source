...
# [GCC 8.4.0]
# Embedded file name: /Users/abenkevich/defender/alertlogic-cli/alertlogic/services.py
# Compiled at: 2019-09-23 06:34:40
...
class Otis(Service):
    ...
    def set_subnet(self, account_id, provider_id, provider_type, vpc_id, subnet_id):
        option_scope = {'provider_id': provider_id, 
...
class Sources(Service):
    ...
    def set_mode(self, account_id, deployment_id, mode):
        source = {'source': {'config': {'deployment_mode': mode}}}
    ...
    def set_deployment_scope(self, account_id, source_id, include, exclude):
        source = {'source': {'config': {'aws': {'scope': {'include': include, 
                                                   'exclude': exclude}}}}}
    ...
    def create_deployment(self, account_id, aws_account_id, credential_id, name, mode, scan=True):
        source = {'source': {'config': {'collection_method': 'api', 
                                 'collection_type': 'aws', 
...
class Saturn(Service):
    ...
    def terminate_instance(self, account_id, deployment_id, region, instance_id):
        j = {'region': region, 'instance_id': instance_id}
        return self.post([account_id, 'deployments', deployment_id, 'terminate_instance'], None, j)
...
class Themis(Service):
    ...
    def validate_credentials(self, account_id, platform_type, role_type, version, arn):
        role = {'platform_type': platform_type, 
           'role_type': role_type, 
...
class AssetsQuery(Service):
    ...
    def get_assets_in_account(self, account_id, params=None):
        return self.get([account_id, 'assets'], query_params=params)
...
class AssetsWrite(Service):
    ...
    def create_network(self, account_id, deployment_id, name, cidr_ranges, public_cidr_ranges):
        network = {'operation': 'create_network', 
           'scope': 'datacenter', 
           'properties': {'cidr_ranges': cidr_ranges, 
...
class Deployments(Service):
    ...
    def list_accounts(self):
        return self.get(['accounts'], query_params={'platform.type': 'aws'})
...
class Album(Service):
    ...
    def put_shares(self, vendor_id, type='aws'):
        return self.put(['shares', type, vendor_id])


class Config(Service):

    def __init__(self, session):
        Service.__init__(self, 'config', 'v1', session)

    def list_scan_accounts(self):
        return self.get(['features', 'ci_scan_deployment', 'accounts'])


class Scanmon(Service):

    def __init__(self, session):
        Service.__init__(self, 'scanmon', 'v1', session)

    def list_metric(self, metric):
        return self.get(['stats', 'global', 'details', metric])


class CloudExplorer(Service):

    def __init__(self, session):
        Service.__init__(self, 'cloud_explorer', 'v1', session)

    def discover(self, account_id, deployment_id):
        return self.post([account_id, 'environment', deployment_id, 'discover'], query_params={'sync': 'true'})