...
# Embedded file name: build/bdist.linux-x86_64/egg/cary/carycommand.py
# Compiled at: 2015-08-04 05:55:57
...
class CaryAction(metaclass=ABCMeta):
    ...
    def make_working_directories(self):
        self.working_dir = tempfile.mkdtemp(dir=os.path.join(self.config['WORKSPACE_DIR'], 'transactions'))
    ...
    def write_response(self, body, html_body=None):
        ...
        """
        outer = MIMEMultipart() if html_body is None else MIMEMultipart('alternative')
    ...
    def attach_files(outer, filenames):
        for fnam in filenames:
            ...
            part.add_header('Content-Disposition', 'attachment; filename="{0}"'.format(os.path.split(fnam)[1]))
            outer.attach(part)
