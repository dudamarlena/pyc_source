...
# [GCC 8.4.0]
# Embedded file name: /home/imposeren/kava/django-currency/.env/lib/python2.7/site-packages/currency/models.py
# Compiled at: 2013-11-18 06:09:45
import datetime
...
class Currency(models.Model):
    ...
    short_name = models.CharField(_('Symbol or short name'), max_length=8, blank=True, default='', help_text=_('e.g. $, руб.'))
    full_name = models.CharField(_('Full name'), max_length=64, blank=True, default='', help_text=_('e.g. holland dollar, ukrainian hryvnya'))
    money_format = models.CharField(_('money format string'), max_length=64, blank=True, default='%(short_name)s%(value)s', help_text=_('e.g. %(value)s%(short_name)s'))
    ...
    def format(self, value):
        ...
        """
        result = self.money_format % {'code': self.code, 
    ...
    def get_default_currency(cls):
        currency, _ = cls.objects.get_or_create(code='USD', defaults={'short_name': '$', 'money_format': '%(short_name)s%(value)s'})
        return currency
    ...
    def get_rate_object(self, other_currency, ignore_conflict=False):
        ...
        with localcontext(Context(prec=ExchangeRate.PRECISION + 10)):
            try:
                direct_rate = self.rates.filter(foreign_currency=other_currency).latest()
            except ExchangeRate.DoesNotExist:
                direct_rate = None

            try:
                reverse_rate = other_currency.rates.filter(foreign_currency=self).latest()
            except ExchangeRate.DoesNotExist:
                reverse_rate = None
            ...
            try:
                ...
                if default_currency.code != self.code:
                    rate_to_self = default_currency.rates.filter(foreign_currency=self).latest()
                    rate_to_other = default_currency.rates.filter(foreign_currency=other_currency).latest()
                    new_rate = rate_to_self.rate / rate_to_other.rate
                    new_date = max(rate_to_self.date, rate_to_other.date)
                    indirect_rate = ExchangeRate(base_currency=self, foreign_currency=other_currency, rate=new_rate, date=new_date)
            except ExchangeRate.DoesNotExist:
                pass
            ...
            if not indirect_rate:
                if direct_rate:
                    ...
                    rate = reverse_rate
                    is_reverse = True
            elif not (direct_rate or reverse_rate):
                rate = indirect_rate
            else:
                if direct_rate:
                    rate = direct_rate
                ...
                if rate.date < indirect_rate.date:
                    if not ignore_conflict:
                        raise ValueError('direct rate `%s` is older then indirect rate `%s`. Please investigate' % (
...
class ExchangeRate(models.Model):
    ...
    rate = models.DecimalField(_('Exchange rate'), max_digits=9, decimal_places=PRECISION, default='1', validators=[
     validate_positive])
    ...
    class Meta:
        ...
        unique_together = (('base_currency', 'foreign_currency', 'date'), )
    ...
    def clean(self):
        try:
            reverse_rate = ExchangeRate.objects.get(base_currency=self.foreign_currency, foreign_currency=self.base_currency, date=self.date)
        except ExchangeRate.DoesNotExist:
            reverse_rate = None

        if reverse_rate:
            raise ValidationError('Reverse rate with pk=%s for same date already exists.' % reverse_rate.pk)
...
def get_currency(currency):
    ...
    if isinstance(currency, Currency):
        ...
        raise TypeError('currency argument should be of type string or Currency. Got `%s` instead' % currency)
    return currency
...
class Money(object):
    ...
    def same_currencies(one, other):
        if one.currency != other.currency:
            raise ValueError('Currencies of %s and %s differ. Please convert them to same currencies' % (
             one, other))
        return True
