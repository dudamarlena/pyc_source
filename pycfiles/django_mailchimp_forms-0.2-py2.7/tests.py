...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/django_mailchimp_forms/tests.py
...
class ModelApiTest(TestCase):
    fixtures = [
     'auth.user.json', 'mailing_list.json']
    ...
    def test_add_user_to_mailing_list(self):
        self.assertRaises(MailingList.DoesNotExist, lambda : self.guest.mailinglist)
    ...
    def test_remove_user_from_mailing_list(self):
        ...
        self.guest2 = User.objects.get(username='guest2')
        self.assertRaises(MailingList.DoesNotExist, lambda : self.guest2.mailinglist)
    ...
    def test_default_form(self):
        form = ChimpyForm({'username': 'guest3', 'password1': 'guest3', 'password2': 'guest3', 
    ...
    def test_class_wrapper(self):
        ...
        form = form_class({'username': 'guest3', 
           'password': 'guest3', 'email': TEST_EMAIL, 
           'newsletter': True})
        assert form.is_valid()
        guest3 = form.save()
        assert guest3.id
    ...
    def test_instance_wrapper(self):
        ...
        post_data = {'username': 'guest3', 'password': 'guest3', 'email': TEST_EMAIL, 
           'newsletter': True}
...
class UserExperienceTest(TestCase):
    ...
    def test_user_sign_up_with_mailinglist(self):
        self.client.post('/accounts/register/', {'username': 'guest3', 
           'password1': 'guest3', 'password2': 'guest3', 
           'email': TEST_EMAIL, 'mailing_list': True})
    ...
    def test_user_sign_up_without_mailinglist(self):
        self.client.post('/accounts/register/', {'username': 'guest3', 
           'password1': 'guest3', 'password2': 'guest3', 
           'email': TEST_EMAIL, 'mailing_list': False})
        guest3 = User.objects.get(username='guest3')
        self.assertRaises(MailingList.DoesNotExist, lambda : guest3.mailinglist)
