...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/ucloudclient/tests/test_shell.py
# Compiled at: 2015-11-11 08:06:57
__author__ = 'hyphen'
import re, sys, mock
...
FAKE_ENV = {'UCLOUD_REGION': 'cn-north-03', 'UCLOUD_URL': 'https://api.ucloud.cn', 
   'UCLOUD_PUBKEY': 'asdf', 
   'UCLOUD_PRIKEY': 'asdf'}
FAKE_ENV2 = {'UCLOUD_REGION': 'cn-north-03', 'UCLOUD_URL': 'https://api.ucloud.cn', 
   'UCLOUD_PUBKEY': 'asdf', 
   'UCLOUD_PRIKEY': 'asdf'}
...
class ShellTest(TestCase):
    _msg_no_region = 'You must provide region name via --ucloud_region or env[UCLOUD_REGION].'
    _msg_no_url = 'You must provide url via --ucloud_url or env[UCLOUD_URL].'
    _msg_no_public_key = 'You must provide public key via --ucloud_pubkey or env[UCLOUD_PUBKEY].'
    _msg_no_private_key = 'You must provide private key via --ucloud_prikey or env[UCLOUD_PRIKEY].'
    _msg_help = [
     '.*?^usage: ucloud',
    ...
    def setUp(self):
        ...
        self.useFixture(fixtures.MonkeyPatch('ucloudclient.client.get_client_class', mock.MagicMock))
