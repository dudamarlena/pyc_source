...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/ucloudclient/utils/base.py
# Compiled at: 2015-11-11 06:54:58
import json, httplib, urlparse, urllib, hashlib, os, glob, errno, time
...
class CompletionCache(object):
    ...
    def _make_directory_name(self, username, auth_url):
        ...
        of the current user.
        """
        uniqifier = hashlib.md5(username.encode('utf-8') + auth_url.encode('utf-8')).hexdigest()
        base_dir = os.environ.get('UCLOUDCLIENT_UUID_CACHE_DIR', default='~/.ucloudclient')
        return os.path.expanduser(os.path.join(base_dir, uniqifier))

    def _prepare_directory(self):
        try:
            os.makedirs(self.directory, 493)
        except OSError:
            pass
