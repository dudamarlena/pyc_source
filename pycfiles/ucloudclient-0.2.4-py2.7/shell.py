...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/ucloudclient/shell.py
# Compiled at: 2015-11-11 06:54:58
"""
Command line interface for ucloud
"""
from __future__ import print_function
import argparse, logging, os, sys, six
from api import shell_action
from utils import encodeutils
from utils import shell_utils
import client, uexceptions
...
class UcloudClientArgumentParser(argparse.ArgumentParser):
    ...
    def error(self, message):
        ...
        self.exit(2, "error: %(errmsg)s\nTry '%(mainp)s help %(subp)s' for more information.\n" % {'errmsg': message.split(choose_from)[0], 'mainp': progparts[0], 
           'subp': progparts[2]})
    ...
    def _get_option_tuples(self, option_string):
        ...
        """
        option_tuples = super(UcloudClientArgumentParser, self)._get_option_tuples(option_string)
...
class UcloudHelpFormatter(argparse.HelpFormatter):
    ...
    def __init__(self, prog, indent_increment=2, max_help_position=32, width=None):
        super(UcloudHelpFormatter, self).__init__(prog, indent_increment, max_help_position, width)
...
class UcloudShell(object):
    ...
    def get_base_parser(self):
        ...
        :return:parser
        """
        parser = UcloudClientArgumentParser(prog='ucloud', description=__doc__.strip(), epilog='See "ucloud help COMMAND" for help on a specific command.', add_help=False, formatter_class=UcloudHelpFormatter)
        parser.add_argument('-h', '--help', action='store_true', help=argparse.SUPPRESS)
        parser.add_argument('--debug', default=False, action='store_true', help='Print debugging output')
        parser.add_argument('--timing', default=False, action='store_true', help='Print call timing info')
        parser.add_argument('--ucloud_region', help=argparse.SUPPRESS)
        parser.add_argument('--ucloud_url', help=argparse.SUPPRESS)
        parser.add_argument('--ucloud_pubkey', help=argparse.SUPPRESS)
        parser.add_argument('--ucloud_prikey', help=argparse.SUPPRESS)
    ...
    @shell_utils.arg('command', metavar='<subcommand>', nargs='?', help='Display help for <subcommand>')
    def do_help(self, args):
        ...
        if args.command:
            if args.command in self.subcommands:
                ...
                raise uexceptions.CommandError("'%s' is not a valid subcommand" % args.command)
        else:
    ...
    def _find_actions(self, subparsers, actions_module):
        for attr in (a for a in dir(actions_module) if a.startswith('do_')):
            ...
            subparser = subparsers.add_parser(command, help=action_help, description=desc, add_help=False, formatter_class=UcloudHelpFormatter)
    ...
    def _add_bash_completion_subparser(self, subparsers):
        subparser = subparsers.add_parser('bash_completion', add_help=False, formatter_class=UcloudHelpFormatter)
    ...
    def main(self, argv):
        ...
        if not ucloud_region:
            raise uexceptions.CommandError('You must provide region name via --ucloud_region or env[UCLOUD_REGION].')
        if not ucloud_url:
            raise uexceptions.CommandError('You must provide url via --ucloud_url or env[UCLOUD_URL].')
        if not ucloud_pubkey:
            raise uexceptions.CommandError('You must provide public key via --ucloud_pubkey or env[UCLOUD_PUBKEY].')
        if not ucloud_prikey:
            raise uexceptions.CommandError('You must provide private key via --ucloud_prikey or env[UCLOUD_PRIKEY].')
...
def main():
    try:
        ...
        details = {'name': encodeutils.safe_encode(e.__class__.__name__), 'msg': six.text_type(e)}
