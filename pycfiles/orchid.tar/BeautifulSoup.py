...
# [GCC 8.4.0]
# Embedded file name: /home/pesachzon/workspace/crawler/ugrah/BeautifulSoup.py
# Compiled at: 2005-12-04 22:04:02
"""Beautiful Soup
Elixir and Tonic
...
class PageElement:
    ...
    def fetchPreviousSiblings(self, name=None, attrs={}, text=None, limit=None):
        """Returns the siblings of this Tag that match the given
        criteria and appear before this Tag in the document."""
        return self._fetch(name, attrs, text, limit, self.previousSiblingGenerator)
...
class Tag(PageElement):
    ...
    def fetch(self, name=None, attrs={}, recursive=True, text=None, limit=None):
...
class BeautifulStoneSoup(Tag, SGMLParser):
    ...
    RESET_NESTING_TAGS = {}
    QUOTE_TAGS = {}
    MS_CHARS = {b'\x80': '&euro;', b'\x81': ' ', b'\x82': '&sbquo;', b'\x83': '&fnof;', b'\x84': '&bdquo;', b'\x85': '&hellip;', b'\x86': '&dagger;', b'\x87': '&Dagger;', b'\x88': '&caret;', b'\x89': '%', b'\x8a': '&Scaron;', b'\x8b': '&lt;', b'\x8c': '&OElig;', b'\x8d': '?', b'\x8e': 'Z', b'\x8f': '?', b'\x90': '?', b'\x91': '&lsquo;', b'\x92': '&rsquo;', b'\x93': '&ldquo;', b'\x94': '&rdquo;', b'\x95': '&bull;', b'\x96': '&ndash;', b'\x97': '&mdash;', b'\x98': '&tilde;', b'\x99': '&trade;', b'\x9a': '&scaron;', b'\x9b': '&gt;', b'\x9c': '&oelig;', b'\x9d': '?', b'\x9e': 'z', b'\x9f': '&Yuml;'}
    PARSER_MASSAGE = [
     (
      re.compile('(<[^<>]*)/>'), lambda (x): x.group(1) + ' />'), (re.compile('<!\\s+([^<>]*)>'), lambda (x): '<!' + x.group(1) + '>'), (re.compile(b'([\x80-\x9f])'), lambda (x): BeautifulStoneSoup.MS_CHARS.get(x.group(1)))]
    ROOT_TAG_NAME = '[document]'

    def __init__(self, text=None, avoidParserProblems=True, initialTextIsEverything=True):
        ...
        if avoidParserProblems and not isList(avoidParserProblems):
    ...
    def popTag(self):
        ...
        if len(self.currentTag.contents) == 1 and isinstance(self.currentTag.contents[0], NavigableText):
    ...
    def unknown_starttag(self, name, attrs, selfClosing=0):
        if self.quoteStack:
            ...
            return
        self.endData()
        if name not in self.SELF_CLOSING_TAGS and not selfClosing:
            self._smartPop(name)
...
class BeautifulSoup(BeautifulStoneSoup):
    ...
    * Tag nesting rules:
    ...
    NESTABLE_TABLE_TAGS = {'table': [], 'tr': ['table', 'tbody', 'tfoot', 'thead'], 'td': ['tr'], 'th': ['tr']}
...
class ICantBelieveItsBeautifulSoup(BeautifulSoup):
    ...
    I_CANT_BELIEVE_THEYRE_NESTABLE_INLINE_TAGS = [
     'em', 'big', 'i', 'small', 'tt', 'abbr', 'acronym', 'strong', 'cite', 'code', 'dfn', 'kbd', 'samp', 'strong', 'var', 'b', 'big']
...
class BeautifulSOAP(BeautifulStoneSoup):
    ...
    def popTag(self):
        if len(self.tagStack) > 1:
            ...
            parent._getAttrMap()
            if isinstance(tag, Tag) and len(tag.contents) == 1 and isinstance(tag.contents[0], NavigableText) and not parent.attrMap.has_key(tag.name):
