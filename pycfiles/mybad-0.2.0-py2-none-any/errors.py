...
# [GCC 8.4.0]
# Embedded file name: /Users/grimen/Dev/Private/python-mybad/mybad/errors.py
# Compiled at: 2019-05-10 15:45:03
import re, sys, traceback, inspect, inspecta, json
...
class BaseError(Exception):
    ...
    def __init__(self, error=None, id=None, key=None, code=None, message=None, details=None):
        message = message or error and str(error) or 'Unknown'
        message = message and str(message)
        details = details or {}
        if not isinstance(details, dict):
            raise TypeError(('Expected argument {klass}(details = <details>) to be a `{expected_type}`, but was `{actual_type}`.').format(klass=self.__class__.__name__, expected_type=type({}), actual_type=type(details)))
    ...
    def stackobjects(self):
        ...
        for stackframe in self.stackframes:
            frame, filename, line_number, function_name, lines, index = stackframe
            file = path.abspath(filename)
            function = function_name
            line = line_number
            column = None
            source = ('\n').join(list(lines))
            stackobject = dict(file=file, function=function, line=line, column=column, source=source)
    ...
    @property
    def data(self):
        return AttributeDict({'type': self.__class__.__name__, 
           'id': self.id, 
           'code': self.code, 
           'key': self.key, 
    ...
    def json(self, indent=None):
        ...
        return json.dumps(AttributeDict.dict(self.data), default=repr, indent=indent)

    def inspect(self, colors=None, verbose=None):
        if colors is None:
            colors = DEFAULT_ERROR_COLORS
        if verbose is None:
            verbose = DEFAULT_ERROR_VERBOSE
        return inspecta.inspect(self.data, colors=colors)

    def format(self, colors=None, verbose=None):
        if colors is None:
            colors = DEFAULT_ERROR_COLORS
        if verbose is None:
            verbose = DEFAULT_ERROR_VERBOSE
        ...
        if verbose:
            _details = inspecta.inspect(self.details, colors=colors)
        else:
            _details = None
    ...
    def __str__(self):
        ...
        if verbose is None:
            verbose = DEFAULT_ERROR_VERBOSE
        verbose = re.search('^true|1$', str(verbose), flags=re.IGNORECASE)
        string = self.format(colors=colors, verbose=verbose)
        string = string.strip()
        return string
    ...
    def object(error, **attrs):
        ...
        return AttributeDict({'type': error.__class__.__name__, 
           ...
           'stack': extended_error.stackobjects}, **attrs)
