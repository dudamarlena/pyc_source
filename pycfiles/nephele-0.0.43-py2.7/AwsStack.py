...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/nephele/AwsStack.py
# Compiled at: 2017-05-24 08:33:08
import os, pyperclip
...
class AwsStack(AwsProcessor):
    ...
    def wrapStack(self, stack):
        ...
        for resource in stack.resource_summaries.all():
            if resource.resource_type not in resourcesByType:
            ...
            if (None == include or resourceType in include) and len(resources):
                ...
                for index, resource in resources.items():
                    if fnmatches(resource.logical_id.lower(), filters):
                        print frm.format(index, resource.logical_id, elipsifyMiddle(repr(resource.resource_status), 50), elipsifyMiddle(repr(defaultify(resource.resource_status_reason, '')), 150))
