...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/nephele/AwsProcessor.py
# Compiled at: 2017-08-01 17:50:08
...
from stdplus import *
import cmd, json, os, re, signal, sys, traceback, Config
...
def getPlugins(pluginPath, mainModule):
    ...
    for i in possibleplugins:
        ...
        if not os.path.isdir(location) or mainModule + '.py' not in os.listdir(location):
            continue
...
class AwsProcessor(cmd.Cmd):
    backgroundTasks = []
    resourceTypeAliases = {'AWS::AutoScaling::AutoScalingGroup': 'asg', 'AWS::CloudFormation::Stack': 'stack', 'AWS::EC2::NetworkInterface': 'eni', 
    ...
    def do_mfa(self, args):
        ...
        credentials_command = [
         'aws', '--profile', awsProfile, '--output', 'json', 'sts', 'get-session-token', '--serial-number', arn, '--token-code', token]
        output = run_cmd(credentials_command)
    ...
    def do_config(self, args):
        ...
        * config save - save the configuration to disk
        ...
        {'print': AwsProcessor.sub_configPrint, 
           ...
           'reload': AwsProcessor.sub_configReload}[args['command']](self, args)
