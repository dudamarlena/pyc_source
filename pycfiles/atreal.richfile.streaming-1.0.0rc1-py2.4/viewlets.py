...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/atreal/richfile/streaming/browser/viewlets.py
...
base_videoplayer = "flowplayer('player','flowplayer-3.1.1.swf',{clip: {autoPlay: %(AUTOPLAY)s,url: '%(URL)s'}})"
base_audioplayer = "flowplayer('player','flowplayer-3.1.1.swf',{plugins: {controls: {fullscreen: %(FULLSCREEN)s,height: %(HEIGHT)s}},clip: {autoPlay: %(AUTOPLAY)s,url: '%(URL)s'}})"
...
class FlowPlayerViewlet(RichfileViewlet):
    ...
    def videoPlayer(self):
        """ """
        parameters = dict(AUTOPLAY=self.autoplay() and 'true' or 'false', URL='%s/rfstreaming/streaming.flv' % self.context.absolute_url())
        return base_videoplayer % parameters

    def audioPlayer(self):
        """ """
        parameters = dict(AUTOPLAY=self.autoplay() and 'true' or 'false', URL='%s/rfstreaming/streaming.mp3' % self.context.absolute_url(), FULLSCREEN='false', HEIGHT='25')
