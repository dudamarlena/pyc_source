...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/eventsource/client.py
...
class EventSourceClient(object):
    ...
    def __init__(self, url, action, target, callback=None, retry=0, keep_alive=False, ssl=False, validate_cert=False, user=None, password=None):
        ...
        self.http_request = HTTPRequest(url=self._url, method='GET', headers={'content-type': 'text/event-stream'}, request_timeout=0, validate_cert=validate_cert, streaming_callback=self.handle_stream, auth_username=user, auth_password=password)
        if callback is None:
    ...
    def _get_request(self):
        ...
        """
        return HTTPRequest(url=self._url, method='GET', headers=self._get_headers(), request_timeout=0, streaming_callback=self.handle_stream, auth_username=self._user, auth_password=self._password)
...
def start():
    ...
    parser = argparse.ArgumentParser(prog=sys.argv[0], description='Event Source Client')
    parser.add_argument('-H', '--host', dest='host', default='127.0.0.1', help='Host to connect to')
    parser.add_argument('-P', '--port', dest='port', help='Port to be used connection')
    parser.add_argument('-S', '--ssl', dest='ssl', action='store_true', help='enables HTTPS scheme support')
    parser.add_argument('-V', '--validate-cert', dest='validate_cert', action='store_true', help='Forces HTTPS certificate validation')
    parser.add_argument('-d', '--debug', dest='debug', action='store_true', help='enables debug output')
    parser.add_argument('-r', '--retry', dest='retry', default='0', help='Reconnection delay (in microseconds)')
    parser.add_argument('-k', '--keep-reconnecting', dest='keep_alive', action='store_true', help='Keep trying to reconnect on disconnection')
    parser.add_argument('-a', '--action', dest='action', default='poll', help='The listening action to connect to')
    parser.add_argument('-u', '--user', dest='user', help='Username for basic authentication')
    parser.add_argument('-p', '--password', dest='password', help='Password for basic authentication')
    parser.add_argument(dest='token', help='Token to be used for connection')
    ...
    if not args.port:
        ...
        port = args.port
    EventSourceClient(url='%s:%s' % (args.host, port), action=args.action, target=args.token, retry=args.retry, keep_alive=args.keep_alive, ssl=args.ssl, validate_cert=args.validate_cert, user=args.user, password=args.password).poll()
