...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/eventsource/listener.py
...
import tornado.web, tornado.gen, tornado.ioloop, tornado.httpserver, toro
...
class EventSourceHandler(tornado.web.RequestHandler):
    ...
    def buffer_event(self, target, action, value=None):
        ...
        self._lock[target]._ready = False
    ...
    def write_error(self, status_code, **kwargs):
        ...
        if self.settings.get('debug') and 'exc_info' in kwargs:
            ...
            self.finish('<html><title>%(code)d: %(message)s</title><body>%(code)d: %(mesg)s</body></html>\n' % {'code': status_code, 
               'message': httplib.responses[status_code], 
               'mesg': kwargs['mesg']})
        else:
            self.finish('<html><title>%(code)d: %(message)s</title><body>%(code)d: %(message)s</body></html>\n' % {'code': status_code, 
               'message': httplib.responses[status_code]})
...
def start():
    ...
    parser = argparse.ArgumentParser(prog=sys.argv[0], description='Event Source Listener')
    parser.add_argument('-H', '--host', dest='host', default='0.0.0.0', help='Host to bind on')
    parser.add_argument('-P', '--port', dest='port', default='8888', help='Port to bind on')
    parser.add_argument('-K', '--keyfile', dest='ssl_keyfile', default='', help='Path to Key file\nif specified with --certfile, SSL is enabled')
    parser.add_argument('-C', '--certfile', dest='ssl_certfile', default='', help='Path to CA Cert file\nif specified with --keyfile, SSL is enabled')
    parser.add_argument('-d', '--debug', dest='debug', action='store_true', help='enables debug output')
    parser.add_argument('-j', '--json', dest='json', action='store_true', help='to enable JSON Event')
    parser.add_argument('-k', '--keepalive', dest='keepalive', default='0', help='Keepalive timeout')
    parser.add_argument('-i', '--id', dest='id', action='store_true', help='to generate identifiers')
    ...
    try:
        application = tornado.web.Application([
         (
          '/(.*)/(.*)', EventSourceHandler, dict(event_class=chosen_event, keepalive=args.keepalive))])
        if args.ssl_certfile != '' or args.ssl_keyfile != '':
            if os.path.exists(args.ssl_certfile) and os.path.exists(args.ssl_keyfile):
                application = tornado.httpserver.HTTPServer(application, ssl_options={'certfile': args.ssl_certfile, 
