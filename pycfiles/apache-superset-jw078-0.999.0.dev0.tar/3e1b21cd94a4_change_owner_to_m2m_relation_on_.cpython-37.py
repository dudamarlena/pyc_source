...
# Embedded file name: /Users/bogdankyryliuk/src/oss/incubator-superset/superset/migrations/versions/3e1b21cd94a4_change_owner_to_m2m_relation_on_.py
# Compiled at: 2020-01-16 13:27:41
# Size of source mod 2**32: 4604 bytes
__doc__ = 'change_owner_to_m2m_relation_on_datasources.py\n\nRevision ID: 3e1b21cd94a4\nRevises: 4ce8df208545\nCreate Date: 2018-12-15 12:34:47.228756\n\n'
...
sqlatable_user = sa.Table('sqlatable_user', sa.MetaData(), sa.Column('id', (sa.Integer), primary_key=True), sa.Column('user_id', sa.Integer, sa.ForeignKey('ab_user.id')), sa.Column('table_id', sa.Integer, sa.ForeignKey('tables.id')))
SqlaTable = sa.Table('tables', sa.MetaData(), sa.Column('id', (sa.Integer), primary_key=True), sa.Column('user_id', sa.Integer, sa.ForeignKey('ab_user.id')))
druiddatasource_user = sa.Table('druiddatasource_user', sa.MetaData(), sa.Column('id', (sa.Integer), primary_key=True), sa.Column('user_id', sa.Integer, sa.ForeignKey('ab_user.id')), sa.Column('datasource_id', sa.Integer, sa.ForeignKey('datasources.id')))
DruidDatasource = sa.Table('datasources', sa.MetaData(), sa.Column('id', (sa.Integer), primary_key=True), sa.Column('user_id', sa.Integer, sa.ForeignKey('ab_user.id')))
...
def upgrade():
    ...
    for druiddatasource in druiddatasources:
        if druiddatasource.user_id is not None:
            session.execute(druiddatasource_user.insert().values(user_id=(druiddatasource.user_id),
    ...
    with op.batch_alter_table('datasources') as (batch_op):
        batch_op.drop_constraint((generic_find_fk_constraint_name('datasources', {'id'}, 'ab_user', insp)),
          type_='foreignkey')
        batch_op.drop_column('user_id')
