...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hammr/commands/scan/scan.py
# Compiled at: 2016-12-16 11:09:34
import urllib, os.path, shutil, paramiko, getpass, shlex, sys, time
...
class Scan(Cmd, CoreGlobal):
    ...
    def arg_run(self):
        ...
        optional.add_argument('--scan-port', dest='port', required=False, help='the ssh port of the running system')
    ...
    def do_run(self, args):
        try:
            ...
            running = True
            while running:
                myScannedInstances = self.api.Users(self.login).Scannedinstances.Getall(Includescans='true', Name=doArgs.name)
                myScannedInstances = myScannedInstances.scannedInstances.scannedInstance
                if myScannedInstances is None or len(myScannedInstances) == 0:
                    time.sleep(5)
                else:
    ...
    def do_build(self, args):
        try:
            ...
            if generics_utils.query_yes_no('Do you want to cancel the job ?'):
                if 'myRScannedInstance' in locals() and 'myScan' in locals() and 'rImage' in locals() and hasattr(myRScannedInstance, 'dbId') and hasattr(myScan, 'dbId') and hasattr(rImage, 'dbId'):
        ...
        try:
            ...
            if myScan is not None and myScan.status.complete and not myScan.status.error and not myScan.status.cancelled:
                ...
                while not (status.complete or status.error or status.cancelled):
                    statusWidget.status = status
                    progress.update(status.percentage)
                    status = self.api.Users(self.login).Scannedinstances(myRScannedInstance.dbId).Scans(myScan.dbId).Imports().Status.Get(I=rScanImport.uri).statuses.status[0]
                    time.sleep(2)
            ...
            printer.out('\n')
            if generics_utils.query_yes_no('Do you want to cancel the job ?'):
                if 'myRScannedInstance' in locals() and 'myScan' in locals() and 'rScanImport' in locals() and hasattr(myRScannedInstance, 'dbId') and hasattr(myScan, 'dbId') and hasattr(rScanImport, 'dbId'):
