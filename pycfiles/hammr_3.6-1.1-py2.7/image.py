...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hammr/commands/image/image.py
...
class Image(Cmd, CoreGlobal):
            ...
            try:
                if doArgs.id:
                    ...
                    if images is None or len(images) == 0:
                        ...
                        if builder is None:
                            printer.out('No builder part found for image with format type: ' + str(template['type']), printer.ERROR)
                            return 2
    ...
    def publish_builder(self, builder, template, appliance, rInstallProfile, i, comliantImage):
        try:
            ...
            rpImage = self.api.Users(self.login).Appliances(appliance.dbId).Images(comliantImage.dbId).Pimages().Publish(body=mypImage, element_name='ns1:publishImage')
            ...
            if generics_utils.query_yes_no('Do you want to cancel the job ?'):
                if 'appliance' in locals() and 'comliantImage' in locals() and 'rpImage' in locals() and hasattr(appliance, 'dbId') and hasattr(comliantImage, 'dbId') and hasattr(rpImage, 'dbId'):
