...
# Embedded file name: \.\cx_Freeze\windist.py
# Compiled at: 2020-01-04 18:05:46
# Size of source mod 2**32: 20806 bytes
import distutils.command.bdist_msi, distutils.errors, distutils.util, msilib, importlib, os
__all__ = [
 'bdist_msi']
...
class bdist_msi(distutils.command.bdist_msi.bdist_msi):
    ...
    def add_config(self, fullname):
        if self.add_to_path:
            msilib.add_data(self.db, 'Environment', [
             ('E_PATH', '=-*Path', '[~];[TARGETDIR]', 'TARGETDIR')])
        if self.directories:
        ...
        msilib.add_data(self.db, 'CustomAction', [
         (
          'A_SET_TARGET_DIR', 307, 'TARGETDIR',
    ...
    def add_cancel_dialog(self):
        dialog = msilib.Dialog(self.db, 'CancelDlg', 50, 10, 260, 85, 3, self.title, 'No', 'No', 'No')
        dialog.text('Text', 48, 15, 194, 30, 3, 'Are you sure you want to cancel [ProductName] installation?')
    ...
    def add_error_dialog(self):
        dialog = msilib.Dialog(self.db, 'ErrorDlg', 50, 10, 330, 101, 65543, self.title, 'ErrorText', None, None)
    ...
    def add_exit_dialog(self):
        dialog = distutils.command.bdist_msi.PyDialog(self.db, 'ExitDialog', self.x, self.y, self.width, self.height, self.modal, self.title, 'Finish', 'Finish', 'Finish')
        ...
        dialog.text('Description', 15, 235, 320, 20, 196611, 'Click the Finish button to exit the installer.')
        button = dialog.next('Finish', 'Cancel', name='Finish')
        button.event('EndDialog', 'Return')

    def add_fatal_error_dialog(self):
        dialog = distutils.command.bdist_msi.PyDialog(self.db, 'FatalError', self.x, self.y, self.width, self.height, self.modal, self.title, 'Finish', 'Finish', 'Finish')
        dialog.title('[ProductName] installer ended prematurely')
        dialog.back('< Back', 'Finish', active=False)
        dialog.cancel('Cancel', 'Back', active=False)
        dialog.text('Description1', 15, 70, 320, 80, 196611, '[ProductName] setup ended prematurely because of an error. Your system has not been modified. To install this program at a later time, please run the installation again.')
        dialog.text('Description2', 15, 155, 320, 20, 196611, 'Click the Finish button to exit the installer.')
        button = dialog.next('Finish', 'Cancel', name='Finish')
        button.event('EndDialog', 'Exit')
        ...
        while todo:
            ...
            for file in os.listdir(dir.absolute):
                if os.path.isdir(os.path.join(dir.absolute, file)):
                    newDir = msilib.Directory(db, cab, dir, file, file, '%s|%s' % (dir.make_short(file), file))
                    todo.append(newDir)
                else:
                    dir.add_file(file)
    ...
    def add_files_in_use_dialog(self):
        dialog = distutils.command.bdist_msi.PyDialog(self.db, 'FilesInUse', self.x, self.y, self.width, self.height, 19, self.title, 'Retry', 'Retry', 'Retry', bitmap=False)
        dialog.text('Title', 15, 6, 200, 15, 196611, '{\\DlgFontBold8}Files in Use')
        dialog.text('Description', 20, 23, 280, 20, 196611, 'Some files that need to be updated are currently in use.')
        dialog.text('Text', 20, 55, 330, 50, 3, 'The following applications are using files that need to be updated by this setup. Close these applications and then click Retry to continue the installation or Cancel to exit it.')
        dialog.control('List', 'ListBox', 20, 107, 330, 130, 7, 'FileInUseProcess', None, None, None)
    ...
    def add_maintenance_type_dialog(self):
        dialog = distutils.command.bdist_msi.PyDialog(self.db, 'MaintenanceTypeDlg', self.x, self.y, self.width, self.height, self.modal, self.title, 'Next', 'Next', 'Cancel')
        dialog.title('Welcome to the [ProductName] Setup Wizard')
        dialog.text('BodyText', 15, 63, 330, 42, 3, 'Select whether you want to repair or remove [ProductName].')
        group = dialog.radiogroup('RepairRadioGroup', 15, 108, 330, 60, 3, 'MaintenanceForm_Action', '', 'Next')
        ...
        button.event('[REINSTALL]', 'ALL', 'MaintenanceForm_Action="Repair"', 5)
        button.event('[Progress1]', 'Repairing', 'MaintenanceForm_Action="Repair"', 6)
        button.event('[Progress2]', 'repairs', 'MaintenanceForm_Action="Repair"', 7)
        button.event('Reinstall', 'ALL', 'MaintenanceForm_Action="Repair"', 8)
        button.event('[REMOVE]', 'ALL', 'MaintenanceForm_Action="Remove"', 11)
        button.event('[Progress1]', 'Removing', 'MaintenanceForm_Action="Remove"', 12)
        button.event('[Progress2]', 'removes', 'MaintenanceForm_Action="Remove"', 13)
        button.event('Remove', 'ALL', 'MaintenanceForm_Action="Remove"', 14)
        button.event('EndDialog', 'Return', 'MaintenanceForm_Action<>"Change"', 20)
        button = dialog.cancel('Cancel', 'RepairRadioGroup')
        button.event('SpawnDialog', 'CancelDlg')

    def add_prepare_dialog(self):
        dialog = distutils.command.bdist_msi.PyDialog(self.db, 'PrepareDlg', self.x, self.y, self.width, self.height, self.modeless, self.title, 'Cancel', 'Cancel', 'Cancel')
        dialog.text('Description', 15, 70, 320, 40, 196611, 'Please wait while the installer prepares to guide you through the installation.')
        dialog.title('Welcome to the [ProductName] installer')
        text = dialog.text('ActionText', 15, 110, 320, 20, 196611, 'Pondering...')
        text.mapping('ActionText', 'Text')
        text = dialog.text('ActionData', 15, 135, 320, 30, 196611, None)
    ...
    def add_progress_dialog(self):
        dialog = distutils.command.bdist_msi.PyDialog(self.db, 'ProgressDlg', self.x, self.y, self.width, self.height, self.modeless, self.title, 'Cancel', 'Cancel', 'Cancel', bitmap=False)
        dialog.text('Title', 20, 15, 200, 15, 196611, '{\\DlgFontBold8}[Progress1] [ProductName]')
        dialog.text('Text', 35, 65, 300, 30, 3, 'Please wait while the installer [Progress2] [ProductName].')
        dialog.text('StatusLabel', 35, 100, 35, 20, 3, 'Status:')
        text = dialog.text('ActionText', 70, 100, self.width - 70, 20, 3, 'Pondering...')
        text.mapping('ActionText', 'Text')
        control = dialog.control('ProgressBar', 'ProgressBar', 35, 120, 300, 10, 65537, None, 'Progress done', None, None)
    ...
    def add_select_directory_dialog(self):
        dialog = distutils.command.bdist_msi.PyDialog(self.db, 'SelectDirectoryDlg', self.x, self.y, self.width, self.height, self.modal, self.title, 'Next', 'Next', 'Cancel')
        ...
        button.event('SpawnDialog', 'CancelDlg')
        dialog.control('DirectoryCombo', 'DirectoryCombo', 15, 70, 272, 80, 393219, 'TARGETDIR', None, 'DirectoryList', None)
        dialog.control('DirectoryList', 'DirectoryList', 15, 90, 308, 136, 3, 'TARGETDIR', None, 'PathEdit', None)
        dialog.control('PathEdit', 'PathEdit', 15, 230, 306, 16, 3, 'TARGETDIR', None, 'Next', None)
    ...
    def add_user_exit_dialog(self):
        dialog = distutils.command.bdist_msi.PyDialog(self.db, 'UserExit', self.x, self.y, self.width, self.height, self.modal, self.title, 'Finish', 'Finish', 'Finish')
        dialog.title('[ProductName] installer was interrupted')
        dialog.back('< Back', 'Finish', active=False)
        dialog.cancel('Cancel', 'Back', active=False)
        dialog.text('Description1', 15, 70, 320, 80, 196611, '[ProductName] setup was interrupted. Your system has not been modified. To install this program at a later time, please run the installation again.')
        dialog.text('Description2', 15, 155, 320, 20, 196611, 'Click the Finish button to exit the installer.')
        button = dialog.next('Finish', 'Cancel', name='Finish')
        button.event('EndDialog', 'Exit')

    def add_wait_for_costing_dialog(self):
        dialog = msilib.Dialog(self.db, 'WaitForCostingDlg', 50, 10, 260, 85, self.modal, self.title, 'Return', 'Return', 'Return')
        dialog.text('Text', 48, 15, 194, 30, 3, 'Please wait while the installer finishes determining your disk space requirements.')
    ...
    def finalize_options(self):
        ...
        if self.initial_target_dir is None:
            ...
            self.initial_target_dir = '[%s]\\%s' % (programFilesFolder, name)
    ...
    def run(self):
        ...
        self.db = msilib.init_database(self.target_name, msilib.schema, self.distribution.metadata.name, self.product_code, sversion, author)
