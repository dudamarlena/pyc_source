...
# Embedded file name: \.\cx_Freeze\hooks.py
# Compiled at: 2020-01-04 18:05:46
# Size of source mod 2**32: 33967 bytes
import glob, os, sys
...
def load_email(finder, module):
    ...
    for name in ('Charset', 'Encoders', 'Errors', 'FeedParser', 'Generator', 'Header',
                 'Iterators', 'Message', 'Parser', 'Utils', 'base64MIME', 'quopriMIME'):
...
def load_pythoncom(finder, module):
    ...
    import pythoncom
    finder.IncludeFiles(pythoncom.__file__, os.path.join('lib', os.path.basename(pythoncom.__file__)), copyDependentFiles=False)
...
def load_pywintypes(finder, module):
    """the pywintypes module is actually contained in a DLL but since those
       ...
       target directory."""
    import pywintypes
    finder.IncludeFiles(pywintypes.__file__, os.path.join('lib', os.path.basename(pywintypes.__file__)), copyDependentFiles=False)
...
def _qt_implementation(module):
    ...
    try:
        ...
        print('WARNING: Tried to load multiple incompatible Qt wrappers. Some incorrect files may be copied.')
...
def load_PyQt4_Qt(finder, module):
    ...
    for mod in ('_qt', 'QtSvg', 'Qsci', 'QtAssistant', 'QtNetwork', 'QtOpenGL', 'QtScript',
    ...
    if sys.platform == 'win32':
        ...
        for env_name, ver_var, mod_name in zipped:
            try:
                ...
                lib_texts = os.path.join(sys.base_prefix, 'tcl', mod_name + str(ver_var))
...
def load_zmq(finder, module):
    ...
    if sys.platform == 'win32':
        try:
            ...
            finder.IncludeFiles(os.path.join(module.path[0], srcFileName), srcFileName)
        except ImportError:
            pass
