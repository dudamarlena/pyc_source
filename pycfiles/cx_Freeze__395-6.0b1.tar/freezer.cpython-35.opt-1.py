...
# Embedded file name: \.\cx_Freeze\freezer.py
# Compiled at: 2020-01-04 18:05:46
# Size of source mod 2**32: 33229 bytes
__doc__ = '\nBase class for freezing scripts into executables.\n'
from __future__ import print_function
import datetime, distutils.sysconfig
from importlib.util import MAGIC_NUMBER
import marshal, os, shutil, socket, stat, struct, sys, time, zipfile, cx_Freeze
__all__ = [
 'ConfigError', 'ConstantsModule', 'Executable', 'Freezer']
...
class Freezer(object):
    ...
    def __init__(self, executables, constantsModule, includes=[], excludes=[], packages=[], replacePaths=[], compress=True, optimizeFlag=0, path=None, targetDir=None, binIncludes=[], binExcludes=[], binPathIncludes=[], binPathExcludes=[], includeFiles=[], zipIncludes=[], silent=False, namespacePackages=[], metadata=None, includeMSVCR=False, zipIncludePackages=[], zipExcludePackages=['*']):
        ...
        self.binIncludes = [os.path.normcase(n) for n in self._GetDefaultBinIncludes() + binIncludes]
        self.binExcludes = [os.path.normcase(n) for n in self._GetDefaultBinExcludes() + binExcludes]
        self.binPathIncludes = [os.path.normcase(n) for n in binPathIncludes]
        self.binPathExcludes = [os.path.normcase(n) for n in self._GetDefaultBinPathExcludes() + binPathExcludes]
    ...
    def _AddVersionResource(self, exe):
        ...
        versionInfo = VersionInfo(self.metadata.version, comments=self.metadata.long_description, description=self.metadata.description, company=self.metadata.author, product=self.metadata.name, copyright=exe.copyright, trademarks=exe.trademarks)
        stamp(fileName, versionInfo)

    def _CopyFile(self, source, target, copyDependentFiles, includeMode=False, relativeSource=False):
        ...
        if copyDependentFiles and source not in self.finder.exclude_dependent_files:
            if sys.platform == 'darwin':
                targetDir = self.targetDir
            sourceDir = os.path.dirname(source)
            for source in self._GetDependentFiles(source):
                if relativeSource and os.path.isabs(source) and os.path.commonpath((source, sourceDir)) == sourceDir:
                ...
                self._CopyFile(source, target, copyDependentFiles, relativeSource=relativeSource)
    ...
    def _FreezeExecutable(self, exe):
        ...
        startupModule = get_resource_file_path('initscripts', '__startup__', '.py')
        finder.IncludeFile(startupModule)
        if sys.platform == 'linux':
            self._CopyFile(exe.base, exe.targetName, copyDependentFiles=False, includeMode=True)
            targetDir = os.path.join(os.path.dirname(exe.targetName), 'lib')
            dependentFiles = self._GetDependentFiles(exe.base) or self._GetDependentFiles(sys.executable)
        ...
        if exe.icon is not None:
            ...
            targetName = os.path.join(os.path.dirname(exe.targetName), os.path.basename(exe.icon))
    ...
    def _GetDefaultBinPathExcludes(self):
        ...
        if sys.platform == 'win32':
            ...
            return [
             '/lib', '/lib32', '/lib64', '/usr/lib', '/usr/lib32',
             '/usr/lib64']
        ...
        if dependentFiles is None:
            if sys.platform == 'win32':
                if path.endswith(('.exe', '.dll', '.pyd')):
                    ...
                    os.environ['PATH'] = origPath + os.pathsep + os.pathsep.join(sys.path)
                    try:
            ...
            for line in os.popen(command):
                ...
                if len(parts) != 2:
                    ...
                    if dependentFile == os.path.basename(path):
                        pass
                    elif dependentFile in ('not found', '(file not found)'):
                        fileName = parts[0]
            ...
            if sys.platform == 'darwin':
                dependentFiles = [p.replace('@loader_path', dirname) for p in dependentFiles]
                dependentFiles = [p.replace('@rpath', sys.prefix + '/lib') for p in dependentFiles]
            if sys.platform == 'darwin':
                dependentFiles = [self._CheckDependentFile(f, dirname) for f in dependentFiles if self._ShouldCopyFile(f)]
            else:
    ...
    def _CheckDependentFile(self, dependentFile, dirname):
        ...
        if os.path.isfile(joined):
            return joined
        raise FileNotFoundError('otool returned a dependent file that could not be found: ' + dependentFile)

    def _GetModuleFinder(self, argsSource=None):
        if argsSource is None:
            argsSource = self
        finder = cx_Freeze.ModuleFinder(self.includeFiles, self.excludes, self.path, self.replacePaths, self.zipIncludeAllPackages, self.zipExcludePackages, self.zipIncludePackages, self.constantsModule, self.zipIncludes)
            ...
            if name.startswith('msvcr') and name.endswith('.dll'):
                for otherName in [name.replace('r', c) for c in 'mp']:
                    ...
                    if not os.path.exists(sourceName):
                        ...
                        self._CopyFile(sourceName, targetName, copyDependentFiles=False)
    ...
    def _VerifyConfiguration(self):
        ...
        for sourceFileName, targetFileName in self.includeFiles + self.zipIncludes:
            if not os.path.exists(sourceFileName):
                raise ConfigError('cannot find file/directory named %s', sourceFileName)
        ...
        if self.zipExcludeAllPackages and self.zipIncludeAllPackages:
            raise ConfigError('all packages cannot be included and excluded from the zip file at the same time')
        for name in self.zipIncludePackages:
            if name in self.zipExcludePackages:
                raise ConfigError('package %s cannot be both included and excluded from zip file', name)
    ...
    def _WriteModules(self, fileName, finder):
        ...
        modules = [m for m in finder.modules if m.name not in self.excludeModules]
        ...
        filesToCopy = []
        ignorePatterns = shutil.ignore_patterns('*.py', '*.pyc', '*.pyo', '__pycache__')
        for module in modules:
            ...
            if module.path is not None and includeInFileSystem:
                ...
                if not os.path.exists(targetPackageDir):
                    ...
                    shutil.copytree(sourcePackageDir, targetPackageDir, ignore=ignorePatterns)
                if module.code is None and module.file is not None and not includeInFileSystem:
                ...
                if includeInFileSystem:
                    ...
                    if module.code is None:
                        ...
                        self._CopyFile(module.file, targetName, copyDependentFiles=True)
                    else:
            ...
            if os.path.isdir(sourceFileName):
                for dirPath, _, fileNames in os.walk(sourceFileName):
                    ...
                    for name in fileNames:
                        outFile.write(os.path.join(dirPath, name), targetPath + '/' + name)
        ...
        for module, target in filesToCopy:
            try:
                ...
                self._CopyFile(module.file, target, copyDependentFiles=True, relativeSource=sys.platform == 'linux')
            finally:
                os.environ['PATH'] = origPath
        ...
        for sourceFileName, targetFileName in self.finder.include_files:
            if os.path.isdir(sourceFileName):
                for path, dirNames, fileNames in os.walk(sourceFileName):
                    ...
                    if 'CVS' in dirNames:
                        dirNames.remove('CVS')
                    fullTargetDir = os.path.join(targetDir, targetFileName, shortPath)
                    ...
                    for fileName in fileNames:
                        ...
                        self._CopyFile(fullSourceName, fullTargetName, copyDependentFiles=True)

            else:
                fullName = os.path.join(targetDir, targetFileName)
                self._CopyFile(sourceFileName, fullName, copyDependentFiles=True)
...
class Executable(object):
    ...
    def __init__(self, script, initScript=None, base=None, targetName=None, icon=None, shortcutName=None, shortcutDir=None, copyright=None, trademarks=None):
        self.script = script
...
class ConstantsModule(object):
    ...
    def __init__(self, releaseString=None, copyright=None, moduleName='BUILD_CONSTANTS', timeFormat='%B %d, %Y %H:%M:%S', constants=[]):
    ...
    def Create(self, finder):
            ...
            if module.file is None:
                ...
                if module.source_is_zip_file:
                    ...
                    if not os.path.exists(module.file):
                        raise ConfigError('no file named %s (for module %s)', module.file, module.name)
        ...
        self.values['SOURCE_TIMESTAMP'] = sourceTimestamp.strftime(self.timeFormat)
...
class VersionInfo(object):
    ...
    def __init__(self, version, internalName=None, originalFileName=None, comments=None, company=None, description=None, copyright=None, trademarks=None, product=None, dll=False, debug=False, verbose=True):
        parts = version.split('.')
        while len(parts) < 4:
            parts.append('0')
