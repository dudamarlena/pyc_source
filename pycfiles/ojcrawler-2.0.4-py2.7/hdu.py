...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ojcrawler/crawlers/hdu.py
# Compiled at: 2018-12-28 09:43:01
...
class HDU(OJ):
    ...
    def login(self):
        data = dict(username=self.handle, userpass=self.password, login='Sign In')
        ...
        if ret:
            ...
            return (
             False, '登陆：http方法错误，请检查网络后重试')

    def is_login(self):
        ret = self.get(self.url_home)
        if ret:
            html = ret.read().decode('gbk')
            if html.find('<img alt="Author"') > 0:
                return True
            return False
        return False
    ...
    def submit_code(self, source, lang, pid):
        if not self.is_login():
            ...
            if not success:
                return (False, info)
        data = dict(problemid=pid, language=self.get_languages()[lang.upper()], usercode=source, check='0')
        ...
        if ret:
            if ret.url == self.url_status[:-1]:
                ...
                if ret.url == self.url_submit:
                    html = ret.read()
                    soup = BeautifulSoup(html, 'html5lib')
                    return (
                     False, soup.find('div', {'style': 'color:red; font-size:12px'}).text)
                return (False, '提交代码：未知错误')

        else:
            return (
             False, '提交代码：http方法错误，请检查网络后重试')
    ...
    def _get_result(self, url_result):
        ...
        if ret:
            ...
            if len(trs) <= 1:
                return (False, '没有结果')
            data = {'rid': trs[1].contents[0].text.strip(), 
    ...
    def get_compile_error_info(self, rid):
        ...
        if ret:
            ...
            return (
             False, '获取编译错误信息：http方法错误，请检查网络后重试')