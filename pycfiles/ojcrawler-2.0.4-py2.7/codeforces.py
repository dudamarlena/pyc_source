...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ojcrawler/crawlers/codeforces.py
# Compiled at: 2018-12-29 12:53:00
...
class Codeforces(OJ):
    ...
    def http_headers(self):
        return {'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Ubuntu Chromium/52.0.2743.116 Chrome/52.0.2743.116 Safari/537.36', 
    ...
    def login(self):
        try:
            ...
            if self.browser.url == self.url_home:
                return (True, '')
            if self.browser.url == self.url_login:
                soup = BeautifulSoup(self.browser.response.content, 'html5lib')
                return (
                 False, soup.find('span', {'class': 'error for__password'}).text)
            return (False, '登陆：未知错误')
        except (HTTPError, URLError) as error:
            return (
             False, '登陆：HTTPError/URLError - ' + str(error))
        except timeout:
            return (
             False, '登陆：http方法超时')
    ...
    def get_problem(self, cpid):
        ...
        if ret:
            ...
            problem_type = 'interactive' if interactive and interactive.text == 'This is an interactive problem.' else 'special judge'
            origin = self.url_problem(cid, pid)
            limits = {'default': (
                         int(float(header.find('div', {'class': 'time-limit'}).contents[1].split(' ')[0]) * 1000),
            ...
            for i in range(n):
                ...
                samples[i + 1] = (
                 input_html.text, output_html.text)
    ...
    def submit_code(self, source, lang, cpid):
        ...
        if ret:
            ...
            if self.browser.url == self.url_status[:-1]:
                ...
                if ok:
                    return (True, info['rid'])
                return (False, '提交代码（获取提交id）：' + info)
            if self.url_submit in self.browser.url:
                soup = BeautifulSoup(self.browser.response.content, 'html5lib')
                info = []
    ...
    def get_result(self):
        ...
        if ret:
            ...
            data = {'rid': trs[1].contents[1].text.strip(), 
