...
# [GCC 8.4.0]
# Embedded file name: /home/jim/workspace/python_module_helm_values/reckoner_values/reckoner_file_creator.py
# Compiled at: 2019-07-22 07:08:25
...
class ReckonerFileCreator:
    ...
    def __init__(self, namespace, region, download_path, repositories, envname, colour, output_file, use_latest_chart_version=False):
        ...
        self.reckoner_data = OrderedDict({'namespace':self.namespace, 
         'repositories':{'stable': 'https://kubernetes-charts.storage.googleapis.com/'}, 
         'minimum_versions':{'helm':'2.10.0', 
          'autohelm':'0.6.5'}, 
         'charts':OrderedDict()})
    ...
    def create(self):
        for release in self.get_releases():
            self.reckoner_data['charts'][release.name] = {'chart':release.chart.metadata.name, 
    ...
    def get_chart_values(self, release):
        return {'fullnameOverride': self.get_full_name_override(release)}
    ...
    def get_value_files_for_release(self, release):
        value_files = []
        gv = GitValues(namespace=(self.namespace),
    ...
    def get_tiller_pod(self):
        ...
        try:
            api_instance = kubernetes.client.CoreV1Api(kubernetes.client.ApiClient())
            api_response = api_instance.list_namespaced_pod(namespace='kube-system',
