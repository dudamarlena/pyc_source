...
# [GCC 8.4.0]
# Embedded file name: /home/ali/work/my_projects/flexrest/flexrest/api/rest_api.py
# Compiled at: 2016-01-10 04:25:54
...
class RestApiHandler(object):
    ...
    def __init__(self):
        if self.resource_name is None:
            self.resource_name = self.resource_class and self.resource_class.__name__
        if self.resource_field is None:
            self.resource_field = self.resource_name and self.resource_name.lower()
        if self.resources_field is None:
            self.resources_field = self.resource_field and self.resource_field + 's'
        return
    ...
    def get_db_session(self):
        assert current_app.flexrest_manager.db_session_callback is not None, 'db_session_callback should not be None'
        return current_app.flexrest_manager.db_session_callback()

    def get_db_base(self):
        assert current_app.flexrest_manager.db_base is not None, 'db_base should not be None'
    ...
    def make_add_result(self, new_entity, dbs=None):
        return {self.resource_field: self.serialize(new_entity), 
           'id': new_entity.id}
    ...
    def make_get_result(self, entity, dbs=None):
        return {self.resource_field: self.serialize(entity, self.get_entity_fields_get())}
    ...
    def make_list_result(self, query, args, order, offset=0, limit=0, dbs=None):
        ...
        if self.list_pagination:
            result['pagination'] = {'offset': offset, 'limit': len(entities), 'total': total}
        return result
    ...
    def validate_ordering(self, value, valid_fields):
        ...
        for v in value.split(','):
            if v not in valid_fields:
                raise InvalidOrderField('Invalid order [%s]! valid fields are: %s' % (
                 v, valid_fields))
            v = ('.').join(v.split('.')[:2])
    ...
    def get_foreign_keys(self):
        if not self.resource_class:
            ...
            if self.__fks is None:
                self.__fks = {fk.parent.name:self.get_base_class_by_table_name(fk._column_tokens[1]) for fk in self.resource_class.__table__.foreign_keys}
            return self.__fks

    def validate_foreign_key(self, fk_col_name, ref_class, fk_value, dbs, abort=True):
        fk_obj = self.get_query(dbs, resource_class=ref_class).filter_by(id=fk_value).first()
        if not fk_obj:
    ...
    def on_invalid_arguments(self, error=None, data=None):
        ...
        self.abort_jsonify(400, 'Invalid arguments', reason=errors)
    ...
    def add(self):
        ...
        if isinstance(result, ResponseBase):
            return result
        return self.jsonify_status(message=('%s successfully created' % self.resource_name), **result)
    ...
    def update(self, resource_id):
        ...
        if isinstance(result, ResponseBase):
            ...
            return self.jsonify_status(message=('%s updated' % self.resource_name), **result)
    ...
    def delete(self, resource_id):
        ...
        if isinstance(result, ResponseBase):
            ...
            return self.jsonify_status(message=('%s deleted' % self.resource_name), **result)
    ...
    def get(self, resource_id):
        ...
        if isinstance(result, ResponseBase):
            ...
            return self.jsonify_status(message=('%s details loaded' % self.resource_name), **result)
    ...
    def get_pagination_params(self, order_fields, default_order):
        try:
            ...
            if default_limit is None:
                default_limit = current_app.config.get('FLEXREST_DEFAULT_PAGING_LIMIT', 25)
            if max_limit is None:
                max_limit = current_app.config.get('FLEXREST_MAX_PAGING_LIMIT', 100)
            limit = int(request.args.get('limit', default_limit))
            if max_limit != 0 and (limit == 0 or limit > max_limit):
                limit = max_limit
            order_param = request.args.get('order', default_order).strip()
            order = self.validate_ordering(order_param, valid_fields=order_fields)
        except InvalidOrderField as e:
            self.abort_jsonify(400, 'Invalid order argument', reason=e.message)
        except ValueError as e:
            self.abort_jsonify(400, 'Invalid offset/limit arguments', reason=str(e))
    ...
    def list(self):
        ...
        order_param = request.args.get('order', self.list_default_order).strip()
        offset, limit, order = self.get_pagination_params(order_fields, self.list_default_order)
        dbs = self.get_db_session()
        ...
        if self.need_to_join_with_related_tables_for_order:
            ...
            for table in joined_tables:
                if isinstance(table, tuple):
                    query = query.outerjoin(table[0], getattr(self.resource_class, table[1]) == table[0].columns['id'])
                else:
                    query = query.outerjoin(table)

        result = self.make_list_result(query, request.args, order, offset, limit, dbs)
        if isinstance(result, ResponseBase):
            return result
        return self.jsonify_status(message=('%ss details loaded' % self.resource_name), **result)
...
class RestApiResource(object):
    ...
    def __init__(self, name, route, app, handler, actions=None, decorators=None, identifier='resource_id', extra_handlers=None, needs_id=None, use_common_decorators=True):
        """
        :name:
    ...
    def add_url_rule(self, app, action):
        ...
        app.add_url_rule(self._get_route_for(action, prefix), '%s_%s' % (self._name, action), method, methods=[
         http_method])
        return