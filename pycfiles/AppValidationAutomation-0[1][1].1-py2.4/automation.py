...
# Embedded file name: build/bdist.linux-x86_64/egg/App/Validation/automation.py
# Compiled at: 2012-05-03 10:28:25
import string
...
class AppValidationAutomation(Unix, Web, Alarming, Purging):
    ...
    def validate_urls(self, urls):
        ...
        for url in urls:
            if self.validate_url(url) is False:
                if re.search('Pas.+?Exp', self.data['WEB_MSG']):
                    self.__handle_pwd_expiry(url)
                elif re.search('Auth.+?Fai', self.data['WEB_MSG']):
                    msg = self.data['WEB_MSG'] + 'Incorrect credentials...Exiting!'
                    logger.error(msg)
                    sys.exit(1)
                elif re.search('Missdirected', self.data['WEB_MSG']):
                    msg = url + ' : Inaccessible\nERROR: ' + self.data['WEB_MSG']
                    logger.error(msg)
    ...
    def __handle_pwd_expiry(self, url):
        ...
        if self.change_web_pwd(url) is True:
            logger.info('msg : OK')
        else:
            logger.error(msg + self.data['WEB_MSG'])
            sys.exit(1)
        msg = 'About to change at Unix level :'
        if self.change_unix_pwd(url) is True:
            logger.info('msg : OK')
        else:
            logger.error(msg + self.data['UNIX_MSG'])
            sys.exit(1)
        return True
    ...
    def validate_processes_mountpoints(self, validation_map=None):
        ...
        for remote_host in validation_map.keys():
            ...
            if self.connect_to(remote_host, user, passwd) is True:
                ...
                proc_tmpls = validation_map[remote_host]['PROCESSES'].split(',')
                for proc_tmpl in proc_tmpls:
                    if self.validate_process(proc_tmpl, remote_pcmd_tmpl) is True:
                        ...
                        logger.error(remote_host + ':' + proc_tmpl + ':NOT OK:' + self.data['UNIX_MSG'])
                        faulty_processes.append(proc_tmpl)

                logger.info('Validating Mountpoints...')
                filesystems = validation_map[remote_host]['FILE_SYS'].split(',')
                for fs in filesystems:
                    if self.validate_mountpoint(fs, remote_mcmd_tmpl) is True:
                        logger.info(remote_host + ':' + fs + ':OK')
                    else:
                        logger.error(remote_host + ':' + fs + ':NOT OK:' + self.data['UNIX_MSG'])
