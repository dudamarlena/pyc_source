...
# Embedded file name: build/bdist.macosx-10.5-i386/egg/checkpoint/manager.py
# Compiled at: 2009-01-07 23:58:31
__doc__ = 'Checkpoint application program interface (API)'
import logging
from checkpoint.repository import Repository, Mirror, SUPPORTED_REPOSITORY_FORMAT
from checkpoint.error import VersionError, CheckpointError, UninitializedMirrorError, UninitializedRepositoryError
__all__ = [
 'MirrorManager', 'RepositoryManager']
    ...
    def uses_mirror(method):
        ...
        def wrapper(self, *args, **kwargs):
            if Repository.contains_repository(self.destination_directory):
                raise CheckpointError("Can't run a Mirror command inside %r: this directory contains a Repository." % self.destination_directory)
...
class RepositoryManager(Common):
    ...
    COMMANDS = [
     'watch', 'status', 'commit', 'revert', 'forget', 'recover']
    ...
    def uses_repository(method):
        ...
        def wrapper(self, *args, **kwargs):
            if Mirror.contains_mirror(self.directory):
                raise CheckpointError("Can't run a Repository command inside %r: this directory contains a Mirror." % self.directory)
