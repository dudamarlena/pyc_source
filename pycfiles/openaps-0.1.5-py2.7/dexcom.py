...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/openaps/vendors/dexcom.py
# Compiled at: 2016-05-29 02:42:04
"""
...
class battery(scan):
    ...
    def main(self, args, app):
        battery = dict(level=self.dexcom.ReadBatteryLevel(), status=self.dexcom.ReadBatteryState())
        return battery
...
class DescribeClocks(scan):
    ...
    def main(self, args, app):
        system = dict(offset=self.dexcom.ReadSystemTimeOffset().total_seconds(), utc=self.dexcom.ReadSystemTime())
        display = dict(offset=self.dexcom.ReadDisplayTimeOffset().total_seconds(), clock=self.dexcom.ReadDisplayTime())
...
* iter_glucose 2 - read last 2 records
  """
    ...
    def configure_app(self, app, parser):
        parser.add_argument('count', type=int, nargs='?', default=100, help='Number of glucose records to read.')
...
class oref0_glucose(glucose):
  ...
  """
    ...
    def configure_app(self, app, parser):
        parser.add_argument('--hours', type=float, nargs='?', default=1, help='Number of hours of glucose records to read.')
        parser.add_argument('--threshold', type=int, default=100, help='Merge EGV and Sensor records occuring within THRESHOLD seconds of each other.')
        parser.add_argument('--no-raw', action='store_true', default=False, help='Skip raw data.')
        parser.add_argument('--glucose', default=None, help='File to read glucose from instead of device.')
        parser.add_argument('--sensor', default=None, help='File to read sensor (raw) from instead of device.')
        return
...
* iter_sensor_insertions 2 - read last 2 records
  """
    ...
    def configure_app(self, app, parser):
        parser.add_argument('count', type=int, nargs='?', default=10, help='Number of sensor insertion, removal, and expiration records to read.')
...
* iter_sensor_insertions_hours 4.3 - read last 4.3 hours of sensor insertion, removal, and expiration records
  """
    ...
    def configure_app(self, app, parser):
        parser.add_argument('hours', type=float, nargs='?', default=1, help='Number of hours of sensor insertion, removal, and expiration records to read.')
...
* iter_calibrations 2 - read last 2 calibration records
  """
    ...
    def configure_app(self, app, parser):
        parser.add_argument('count', type=int, nargs='?', default=10, help='Number of calibration records to read.')
