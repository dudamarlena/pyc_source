...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/glucose/test_convert.py
...
class ConvertTestCase(TestCase):
    ...
    def test_mmol_l_to_mg_dl(self):
        results = [ Convert.mmol_l_to_mg_dl(mmol) for mmol in self.MMOL_LIST
                  ]
        self.assertListEqual(results, self.MGDL_LIST)

    def test_mg_dl_to_mmol_l(self):
        results = [ Convert.mg_dl_to_mmol_l(mg_dl) for mg_dl in self.MGDL_LIST
                  ]
        self.assertListEqual(results, self.MMOL_LIST)