...
# [GCC 8.4.0]
# Embedded file name: /home/hvelarde/collective/sc.photogallery/src/sc/photogallery/tests/test_views.py
# Compiled at: 2018-01-29 16:25:00
...
class ViewTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with api.env.adopt_roles(['Manager']):
            self.gallery = api.content.create(container=self.portal, type='Photo Gallery', title='My Photo Gallery')
        for i, name in enumerate(IMAGES):
            obj = api.content.create(container=self.gallery, type='Image', title=('My Image {0}').format(i + 1))
    ...
    def test_zip_url(self):
        ...
        self.assertEqual(self.view.zip_url(), url.format(self.view.last_modified))

    @unittest.skipUnless(HAS_ZIPEXPORT and not HAS_DEXTERITY, 'requires ftw.zipexport')
...
class ZipViewTestCase(unittest.TestCase):
    ...
    def test_filename(self):
        ...
        self.assertEqual(self.view.filename, filename.format(self.view.last_modified))

    @unittest.skipUnless(HAS_ZIPEXPORT and not HAS_DEXTERITY, 'requires ftw.zipexport')
    def test_zip_selected(self):
        self.view.zip_selected([self.gallery])
        response = self.request.response
        disposition = 'inline; filename="http://nohost/plone/my-photo-gallery/@@zip/{0}/my-photo-gallery.zip"'
        self.assertEqual(response.getHeader('Content-Disposition'), disposition.format(self.view.last_modified))
