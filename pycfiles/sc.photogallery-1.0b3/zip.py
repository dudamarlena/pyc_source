...
# [GCC 8.4.0]
# Embedded file name: /home/hvelarde/collective/sc.photogallery/src/sc/photogallery/browser/zip.py
...
class ZipView(BrowserPage):
    ...
    def filename(self):
        ...
        if not (len(subpath) > 1 and str(last_modified) == subpath[0]):
            url = ('{0}/@@zip/{1}/{2}.zip').format(base_url, str(last_modified), self.context.getId())
        ...
        if not HAS_ZIPEXPORT:
            ...
            with ZipGenerator() as (generator):
                ...
                if generator.is_empty:
                    message = _('Zip export is not supported on the selected content.')
                    api.portal.show_message(message, self.request, type='error')
                    self.request.response.redirect(self.context.absolute_url())
                    return
                zip_file = generator.generate()
                response.setHeader('Content-Disposition', ('inline; filename="{0}"').format(self.filename))
