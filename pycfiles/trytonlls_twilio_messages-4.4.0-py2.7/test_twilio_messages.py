...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/trytond/modules/twilio_messages/tests/test_twilio_messages.py
# Compiled at: 2018-05-09 02:27:39
import unittest
...
class TwilioMessagesTestCase(ModuleTestCase):
    ...
    def test_MessageDataManager_multiple_join(self, get_twilio_client):
        ...
        self.assertIs(transaction.join(twilio_messages.MessageDataManager()), datamanager)
    ...
    def test_Message_values(self):
        ...
        Message = pool.get('twilio.message')
        User = pool.get('res.user')
        message = Message(body='Message', to='+12345', from_='+67890', resource=User(1))
        message.save()
        self.assertDictEqual(message.values, {'body': 'Message', 'to': '+12345', 'from_': '+67890', 'status_callback': message.callback_url})


def suite():
    suite = test_suite()
    suite.addTests(unittest.TestLoader().loadTestsFromTestCase(TwilioMessagesTestCase))
    return suite