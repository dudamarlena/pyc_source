...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/trytond/modules/twilio_messages/twilio_messages.py
# Compiled at: 2018-05-12 00:05:37
import logging, urllib, uuid
...
__all__ = [
 'Message', 'sendmessage', 'SendMessage', 'SendMessageStart']
...
CALLBACK_URL = config.get('twilio', 'callback_url', default='http%s://%s/' % (
...
class Message(ResourceMixin):
    ...
    error_code = fields.Char('Error Code', readonly=True, states={'invisible': ~Eval('status').in_(['failed', 'undelivered'])}, depends=[
     'status'])
    error_message = fields.Char('Error Message', readonly=True, states={'invisible': ~Eval('status').in_(['failed', 'undelivered'])}, depends=[
     'status'])
    ...
    def callback_url(self):
        url = urllib.quote('%(database)s/twiliomessages/%(uuid)s' % {'database': Transaction().database.name, 
           'uuid': self.uuid})
...
class SendMessage(Wizard):
    ...
    start_state = 'start'
    start = StateView('twilio.send_message.start', 'twilio_messages.send_message_form', [
    ...
    def transition_send(self):
        ...
        message = Message(from_=self.start.from_, to=self.start.to, body=self.start.body, resource=self.start.resource)
        message.save()
        message.send()
        return 'end'
