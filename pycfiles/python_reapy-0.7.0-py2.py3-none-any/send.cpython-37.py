...
# [GCC 8.4.0]
# Embedded file name: C:\Users\despres\Desktop\reaper\scripts\reapy\reapy\core\track\send.py
...
class Send(ReapyObject):
    ...
    def get_info(self, param_name):
        ...
        """
        value = RPR.GetTrackSendInfo_Value(self.track_id, self._get_int_type(), self.index, param_name)
        return value
    ...
    def get_sws_info(self, param_name):
        ...
        float
        """
        value = RPR.BR_GetSetTrackSendInfo(self.track_id, self._get_int_type(), self.index, param_name, False, 0.0)
        return value
    ...
    def _midi_flags_unpacked(self):
        ...
        ch_flags = flags % 1024
        bus_flags = flags >> 14
        src_bus = bus_flags % 32
        dst_bus = bus_flags >> 8
        src_ch = ch_flags % 32
        dst_ch = ch_flags >> 5
        return (
         (
          src_bus, src_ch), (dst_bus, dst_ch))
    ...
    def set_info(self, param_name, value):
        RPR.SetTrackSendInfo_Value(self.track_id, self._get_int_type(), self.index, param_name, value)

    @depends_on_sws
    def set_sws_info(self, param_name, value):
        RPR.BR_GetSetTrackSendInfo(self.track_id, self._get_int_type(), self.index, param_name, True, value)
