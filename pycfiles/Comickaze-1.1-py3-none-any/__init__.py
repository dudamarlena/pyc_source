...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/comicgen/__init__.py
# Compiled at: 2014-03-02 21:12:47
...
def generate(params):
    with TempDir('comicgen_') as (temp_dir):
        ...
        template_params = dict(indexes=indexes, uuid=uuid, title=params.title, author=params.author, publisher=params.publisher, publication=params.publication)
        ...
        subprocess.call("%(kindlegen)s '%(opf_path)s'" % dict(kindlegen=params.kindlegen, opf_path=temp_dir.path_to('%s.opf' % params.title)), shell=True)
