...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/tweethandler/tweethandler.py
# Compiled at: 2010-05-04 12:41:05
...
class TweetHandler(logging.Handler):
    ...
    def __init__(self, ckey, csecret, akey, asecret, dm_threshold=logging.ERROR, dm_to=[], compact_log=True, ignore_twerror=True, max_retries=5, async=False):
        """
        ...
        self._pool = None
        if async:
            self._pool = _APIPool(ckey, csecret, akey, asecret, max_retries=max_retries)
            self._pool.start()
        return
    ...
    def emit(self, record):
        ...
        try:
            ...
            self._api_call('update_status', self.format(record))
        except TweepError, e:
    ...
    def _send_dm(self, record):
        ...
        if type(self._dm_to) is list:
            for d in self._dm_to:
                self._api_call('send_direct_message', d, text=msg)

        else:
            self._api_call('send_direct_message', self._dm_to, text=msg)
...
class _APIPool:
    ...
    def __init__(self, ckey, csecret, akey, asecret, max_retries=5, numthreads=10):
        self._ckey = ckey
    ...
    def start(self):
        ...
        for i in range(self._numthreads):
            ...
            newth = threading.Thread(target=self._worker, name=name)
