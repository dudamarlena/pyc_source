...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/echoip/sources.py
# Compiled at: 2015-05-21 02:49:13
"""
...
import copy, random, sys, zope.interface
...
class JSONIPSource(SimpleIPSource):
    ...
    def fetch(self):
        ...
        try:
            ...
            raise InvalidJSONSourceIPKey(('Key {} not in response dictionary for URL {}').format(self._ip_key, self._ip_url))
        except IndexError:
...
class IPSourceFactory(object):
    ...
    """
    _builtin_sources = {'ip-api.com': (JSONIPSource, 'http://ip-api.com/json', 'query'), 'ipinfo.io': (
                   JSONIPSource, 'http://ipinfo.io/json', 'ip'), 
       'httpbin.org': (
                     JSONIPSource, 'http://httpbin.org/get', 'origin'), 
       'wtfismyip.com': (
                       JSONIPSource, 'http://wtfismyip.com/json',
                       'YourFuckingIPAddress'), 
       'eth0.me': (
                 SimpleIPSource, 'http://eth0.me/'), 
       'l2.io': (
               SimpleIPSource, 'http://l2.io/ip'), 
       'curlmyip.com': (
                      SimpleIPSource, 'http://curlmyip.com/')}
