    ...
    def thread_stopped(self):
        ...
        if tracker is not None:
            try:
                ...
                elif self.exception_event().is_set() is False and self.track_termination() is True:
...
class WSimpleTrackerStorage(WCriticalResource, WThreadTrackerInfoStorageProto):
    ...
    def __store_record(self, record):
        ...
        if limit is not None and len(self._WSimpleTrackerStorage__registry) >= limit:
    ...
    def last_record(self, task_uid, *requested_events):
        ...
        for record in self:
            if isinstance(record.thread_task, WScheduleTask) is False:
                pass
            elif record.thread_task.uid() == task_uid and (len(requested_events) == 0 or record.record_type in requested_events):
                return record
