...
# [GCC 8.4.0]
# Embedded file name: /home/ale/progr/github/wasp-general/wasp_general/network/upload.py
# Compiled at: 2018-01-21 05:59:45
# Size of source mod 2**32: 6639 bytes
...
class WBasicUploader(WUploaderProto):
    ...
    def __init__(self, scheme, supported_options, required_options):
        ...
        self._WBasicUploader__scheme = scheme
        self._WBasicUploader__supported_options = supported_options
        self._WBasicUploader__required_options = required_options

    def scheme(self):
        return self._WBasicUploader__scheme

    def supported_options(self):
        return self._WBasicUploader__supported_options

    def required_options(self):
        return self._WBasicUploader__required_options
    ...
    def upload(self, file_obj, options):
        if self.is_capable(self.scheme(), options) is False:
            raise RuntimeError('Invalid options specified (required options may be missing, or unsupported options was specified)')
...
class WFTPUploader(WBasicUploader):
    ...
    def __init__(self):
        WBasicUploader.__init__(self, 'ftp', (
    ...
    def _upload(self, file_obj, options):
        try:
            ...
            self._WFTPUploader__change_dir(ftp_client, dir_name)
...
class WUploaderCollection(WUploaderProto):
    __urlparse_options__ = {WUploaderProto.URLOption.hostname: 'hostname', 
    ...
    def __init__(self, *uploaders):
        WUploaderProto.__init__(self)
        self._WUploaderCollection__uploaders = list(uploaders)

    def uploaders(self):
        return tuple(self._WUploaderCollection__uploaders)
