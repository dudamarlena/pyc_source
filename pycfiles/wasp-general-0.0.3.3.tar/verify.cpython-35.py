    ...
    def check(self, type_spec, arg_name, decorated_function):
        ...
        if isinstance(type_spec, (tuple, list, set)):
            ...
            if None in type_spec:
                ...
                return lambda x: None if x is None or isinstance(x, tuple(type_spec)) is True else raise_exception(str(type(x)))
            else:
                return lambda x: None if isinstance(x, tuple(type_spec)) is True else raise_exception(str(type(x)))
        else:
            if isclass(type_spec):
                return lambda x: None if isinstance(x, type_spec) is True else raise_exception(str(type(x)))
            raise RuntimeError('Invalid specification. Must be type or tuple/list/set of types')
...
class SubclassVerifier(Verifier):
    ...
    def check(self, type_spec, arg_name, decorated_function):
        ...
        if isinstance(type_spec, (tuple, list, set)):
            ...
            if None in type_spec:
                type_spec = tuple(filter(lambda x: x is not None, type_spec))
                return lambda x: None if x is None or isclass(x) is True and issubclass(x, type_spec) is True else raise_exception(str(x))
            else:
                return lambda x: None if isclass(x) is True and issubclass(x, type_spec) is True else raise_exception(str(x))
        else:
            if isclass(type_spec):
                return lambda x: None if isclass(x) is True and issubclass(x, type_spec) is True else raise_exception(str(x))
            raise RuntimeError('Invalid specification. Must be type or tuple/list/set of types')
