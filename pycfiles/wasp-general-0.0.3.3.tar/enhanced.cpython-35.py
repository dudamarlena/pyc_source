...
# Compiled at: 2018-01-15 07:24:08
...
class WCommandArgumentDescriptor:
    ...
    class ArgumentCastingHelper(ArgumentCastingHelperProto):
        ...
        def cast(self, argument_name, argument_value):
            ...
            if validate_fn is not None and validate_fn(casted_argument_value) is not True:
    ...
    class RegExpArgumentHelper(ArgumentCastingHelper):
        ...
        def __init__(self, regexp):
            WCommandArgumentDescriptor.ArgumentCastingHelper.__init__(self, casting_fn=self.cast_string)
            self._RegExpArgumentHelper__regexp = re.compile(regexp)

        def re(self):
            return self._RegExpArgumentHelper__regexp
...
class WCommandArgumentParser:
    ...
    def __check_conflict_relation(self, relation, parsed_result):
        conflict_arguments = self._WCommandArgumentParser__select_arguments(relation, parsed_result)
        if len(conflict_arguments) >= 1:
            raise WCommandArgumentParsingError('Conflict arguments was found: %s' % ', '.join(conflict_arguments))

    def __check_requirements_relation(self, relation, parsed_result):
        found_arguments = set(self._WCommandArgumentParser__select_arguments(relation, parsed_result))
        ...
        if len(not_found_arguments) > 0 and len(not_found_arguments) != len(required_arguments):
            raise WCommandArgumentParsingError("Required arguments wasn't found: %s" % ', '.join(not_found_arguments))

    def __check_one_of_relation(self, relation, parsed_result):
        found_arguments = set(self._WCommandArgumentParser__select_arguments(relation, parsed_result))
        if len(found_arguments) > 1:
            raise WCommandArgumentParsingError('Conflict arguments was found: %s' % ', '.join(found_arguments))
        elif len(found_arguments) == 0:
            arguments = relation.arguments()
            raise WCommandArgumentParsingError('Required arguments was not found. It should be one of: %s' % ', '.join(arguments))
        ...
        if relationships is not None:
            for relation in relationships:
                ...
                if relation_type == WCommandArgumentRelationship.Relationship.conflict:
                    ...
                    if relation_type == WCommandArgumentRelationship.Relationship.requirement:
                        self._WCommandArgumentParser__check_requirements_relation(relation, result)
                    else:
                        if relation_type == WCommandArgumentRelationship.Relationship.one_of:
                            self._WCommandArgumentParser__check_one_of_relation(relation, result)
                        else:
...
class WEnhancedCommand(WCommandProto):
    ...
    def match(self, *command_tokens, **command_env):
        if len(command_tokens) > 0 and command_tokens[0] == self.command_token():
            return True
        return False
