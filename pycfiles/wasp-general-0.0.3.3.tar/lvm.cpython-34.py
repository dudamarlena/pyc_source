...
# [GCC 8.4.0]
# Embedded file name: /home/ale/progr/github/wasp-general/wasp_general/os/linux/lvm.py
# Compiled at: 2017-09-27 08:05:04
# Size of source mod 2**32: 15583 bytes
...
class WLVMInfoCommand:
    ...
    def __init__(self, command, fields_count, cmd_timeout=None, sudo=False):
        """ Create new command
                ...
                """
        self._WLVMInfoCommand__command = command
        self._WLVMInfoCommand__fields_count = fields_count
        self._WLVMInfoCommand__cmd_timeout = cmd_timeout if cmd_timeout is not None else self.__lvm_cmd_default_timeout__
        self._WLVMInfoCommand__sudo = sudo
    ...
    def command(self):
        """ Return target program
                ...
                :return: str
                """
        return self._WLVMInfoCommand__command
    ...
    def fields_count(self):
        """ Return number of fields in a program output
                ...
                :return: int
                """
        return self._WLVMInfoCommand__fields_count
    ...
    def cmd_timeout(self):
        """ Timeout for a program to complete
                ...
                :return: int, float
                """
        return self._WLVMInfoCommand__cmd_timeout
    ...
    def sudo(self):
        """ Return 'sudo' flag (whether to run this program with sudo or not)
                ...
                :return: bool
                """
        return self._WLVMInfoCommand__sudo
...
class WLVMInfo:
    __doc__ = ' Basic class for actual LVM information. This class creates :class:`.WLVMInfoCommand` object\n\twhich may be called on an object creation (it depends on constructor parameters)\n\t'
    ...
    @verify_value(lvm_entity=--- This code section failed: ---

 L. 133         0  LOAD_GLOBAL              isinstance
                3  LOAD_FAST                'x'
                6  LOAD_GLOBAL              str
...
Parse error at or near `None' instruction at offset -1
    ...
    def __init__(self, command, fields_count, lvm_entity, sudo=False):
        """ Create new info-object
                ...
                :param lvm_entity: if this is a list/tuple/set - then it is a collection of fields (collection length           must be the same as 'fields_count'). If it is a string, then command is executed to get corresponding           fields
                :param sudo: same as sudo in :meth:`.WLVMInfoCommand.__init__`
                """
        self._WLVMInfo__lvm_command = WLVMInfoCommand(command, fields_count, cmd_timeout=self.__class__.__lvm_info_cmd_timeout__, sudo=sudo)
        if isinstance(lvm_entity, (tuple, list, set)) is True:
            if len(lvm_entity) != fields_count:
                raise ValueError('Invalid lvm entity fields count: %i (expected: %i)' % (
                 len(lvm_entity), fields_count))
            self._WLVMInfo__lvm_entity = tuple(lvm_entity)
        else:
            self._WLVMInfo__lvm_entity = self.lvm_command().lvm_info(lvm_entity)[0]
    ...
    def lvm_command(self):
        """ Return LVM-command object
                ...
                """
        return self._WLVMInfo__lvm_command
    ...
    def lvm_entity(self):
        """ Return object fields
                ...
                :return: tuple of str (fields)
                """
        return self._WLVMInfo__lvm_entity


class WPhysicalVolume(WLVMInfo):
    __doc__ = ' Class represent a physical volume\n\t'

    @verify_type('paranoid', physical_volume=(str, tuple, list, set), sudo=bool)
    @verify_value('paranoid', physical_volume=--- This code section failed: ---

 L. 177         0  LOAD_GLOBAL              isinstance
                3  LOAD_FAST                'x'
                6  LOAD_GLOBAL              str
                9  CALL_FUNCTION_2       2  '2 positional, 0 named'
               12  POP_JUMP_IF_FALSE    31  'to 31'
...
class WVolumeGroup(WLVMInfo):
    __doc__ = ' Class represent a volume group\n\t'

    @verify_type('paranoid', volume_group=(str, tuple, list, set), sudo=bool)
    @verify_value('paranoid', volume_group=--- This code section failed: ---

 L. 256         0  LOAD_GLOBAL              isinstance
                3  LOAD_FAST                'x'
                6  LOAD_GLOBAL              str
                9  CALL_FUNCTION_2       2  '2 positional, 0 named'
               12  POP_JUMP_IF_FALSE    31  'to 31'
...
class WLogicalVolume(WLVMInfo):
    __doc__ = ' Class represent a logical volume\n\t'
    ...
    @verify_value('paranoid', logical_volume=--- This code section failed: ---

 L. 392         0  LOAD_GLOBAL              isinstance
                3  LOAD_FAST                'x'
                6  LOAD_GLOBAL              str
                9  CALL_FUNCTION_2       2  '2 positional, 0 named'
               12  POP_JUMP_IF_FALSE    31  'to 31'
...
Parse error at or near `None' instruction at offset -1
    ...
    def device_number(self):
        """ Return tuple of major and minor device number of logical volume
                ...
                """
        return (
         int(self.lvm_entity()[11]), int(self.lvm_entity()[12]))
    ...
    def create_snapshot(self, snapshot_size, snapshot_suffix):
        """ Create snapshot for this logical volume.
                ...
                :param snapshot_size: size of newly created snapshot volume. This size is a fraction of the source              logical volume space (of this logical volume)
                :param snapshot_suffix: suffix for logical volume name (base part is the same as the original volume            name)
    ...
    def snapshot_allocation(self):
        """ Return allocated size (fraction of total snapshot volume space). If this is not a snapshot volume,
                ...
                """
        check_cmd = [
         'lvs', self.volume_path(), '-o', 'snap_percent', '--noheadings']
