...
# [GCC 8.4.0]
# Embedded file name: /home/ale/progr/github/wasp-general/wasp_general/cli/cli.py
# Compiled at: 2017-10-28 15:30:14
# Size of source mod 2**32: 16960 bytes
...
class WConsoleHistory:
    __doc__ = ' Simple console history implementation\n\t'

    def __init__(self):
        self._WConsoleHistory__history = []
        self._WConsoleHistory__history_position = None
    ...
    def size(self):
        """ Returns history entries count
                ...
                :return: int
                """
        return len(self._WConsoleHistory__history)
    ...
    def position(self, pos=None):
        """ Get current and/or set history cursor position
                ...
                """
        if pos is not None:
            if pos >= len(self._WConsoleHistory__history):
                raise IndexError('History position is out of bound')
            self._WConsoleHistory__history_position = pos
        return self._WConsoleHistory__history_position
    ...
    def add(self, value):
        """ Add new record to history. Record will be added to the end
                ...
                """
        index = len(self._WConsoleHistory__history)
        self._WConsoleHistory__history.append(value)
        return index
    ...
    def entry(self, position):
        """ Get record from history by record position
                ...
                :return: str
                """
        return self._WConsoleHistory__history[position]
    ...
    def update(self, value, position):
        """ Change record in this history
                ...
                :return: None
                """
        self._WConsoleHistory__history[position] = value


class WConsoleProto(metaclass=ABCMeta):
    __doc__ = ' Basic class for console implementation. It has non-changeable and changeable history\n\t(:class:`.WConsoleHistory`). One stores previous entered rows, other one helps to entered new row by editing\n\tprevious one.\n\t'

    def __init__(self):
        self._WConsoleProto__history_mode = False
        self._WConsoleProto__history = WConsoleHistory()
        self._WConsoleProto__editable_history = None
        self._WConsoleProto__current_row = None
        self._WConsoleProto__prompt_show = None
    ...
    def history(self):
        """ Return changeable history
                ...
                """
        return self._WConsoleProto__editable_history
    ...
    def history_mode(self, mode_value=None):
        ...
        if mode_value is not None:
            self._WConsoleProto__history_mode = mode_value
        return self._WConsoleProto__history_mode
    ...
    def start_session(self):
        """ Start new session and prepare environment for new row editing process
                ...
                :return: None
                """
        self._WConsoleProto__current_row = ''
        self._WConsoleProto__history_mode = False
        self._WConsoleProto__editable_history = deepcopy(self._WConsoleProto__history)
        self._WConsoleProto__prompt_show = True
        self.refresh_window()
    ...
    def fin_session(self):
        """ Finalize current session
                ...
                :return: None
                """
        self._WConsoleProto__prompt_show = False
        self._WConsoleProto__history.add(self.row())
        self.exec(self.row())
    ...
    def update_row(self, value):
        """ Change row
                ...
                :return: None
                """
        if not self._WConsoleProto__history_mode:
            self._WConsoleProto__current_row = value
        else:
    ...
    def row(self):
        """ Get row
                ...
                """
        if not self._WConsoleProto__history_mode:
            return self._WConsoleProto__current_row
        else:
    ...
    def prompt_show(self):
        """ Return flag, that shows, whether to display prompt and current command at the window end, or not
                ...
                :return: bool
                """
        return self._WConsoleProto__prompt_show
...
class WConsoleWindowProto(metaclass=ABCMeta):
    __doc__ = ' Basic class for console window implementation\n\t'
    ...
    def __init__(self, console):
        """ Create new console window
                ...
                """
        self._WConsoleWindowProto__console = console
        self._WConsoleWindowProto__previous_data = ''
        self._WConsoleWindowProto__cursor_position = 0
        if self.width() < 2:
    ...
    def data(self, previous_data=False, prompt=False, console_row=False, console_row_to_cursor=False, console_row_from_cursor=False):
        """ Return output data. Flags specifies what data to append. If no flags was specified
                ...
                :param prompt: If True, then console prompt appends. If console_row or console_row_to_cursor is True,           then this value is omitted
                :param console_row: If True, then console prompt and current input appends.
                :param console_row_to_cursor: If True, then console prompt and current input till cursor appends.               If console_row is True, then this value is omitted
                :param console_row_from_cursor: If True, then current input from cursor appends.                If console_row is True, then this value is omitted
                :return: str
                """
        result = ''
        if previous_data:
            result += self._WConsoleWindowProto__previous_data
    ...
    def console(self):
        """ Return linked console
                ...
                """
        return self._WConsoleWindowProto__console
    ...
    def cursor(self, pos=None):
        """ Set and/or get relative cursor position. Defines cursor position in current input row.
                ...
                :param pos: if value is not None, then current cursor position is set to this value and the same value          is returned
                :return: int
                """
        if pos is not None:
            self._WConsoleWindowProto__cursor_position = pos
        return self._WConsoleWindowProto__cursor_position
    ...
    def commit(self):
        """ Store current input row. Keep current input row as previous output
                ...
                :return: None
                """
        self._WConsoleWindowProto__previous_data += self.data(console_row=True) + '\n'
    ...
    def write_feedback(self, feedback, cr=True):
        """ Store feedback. Keep specified feedback as previous output
                ...
                :return: None
                """
        self._WConsoleWindowProto__previous_data += feedback
        if cr is True:
            self._WConsoleWindowProto__previous_data += '\n'
    ...
    def truncate_feedback(self, length):
        """ Remove data from feedback (removes text from previous output)
                ...
                :return: None
                """
        self._WConsoleWindowProto__previous_data = self._WConsoleWindowProto__previous_data[:-length]


class WConsoleDrawerProto(metaclass=ABCMeta):
    __doc__ = ' Basic class that helps displaying console content\n\t'
...
class WConsoleWindowBase(WConsoleWindowProto, metaclass=ABCMeta):
    ...
    def __init__(self, console, *drawers):
        ...
        self._WConsoleWindowBase__drawers = []
        self._WConsoleWindowBase__drawers.extend(drawers)
    ...
    def refresh(self, prompt_show=True):
        """ Refresh current window. Clear current window and redraw it with one of drawers
                ...
                """
        self.clear()
        for drawer in self._WConsoleWindowBase__drawers:
...
class WConsoleBase(WConsoleProto):
    ...
    def __init__(self, command_set=None):
        ...
        self._WConsoleBase__command_set = command_set if command_set is not None else WCommandSet()

    def command_set(self):
        return self._WConsoleBase__command_set
