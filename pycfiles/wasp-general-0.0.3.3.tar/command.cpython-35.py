...
# [GCC 8.4.0]
# Embedded file name: /home/ale/progr/github/wasp-general/wasp_general/command/command.py
# Compiled at: 2017-12-19 08:04:42
# Size of source mod 2**32: 13234 bytes
...
class WCommand(WCommandProto):
    __doc__ = ' Basic WCommandProto implementation\n\t'
    ...
    def __init__(self, *command_tokens):
        ...
        self._WCommand__command = tuple(command_tokens)
    ...
    def command(self):
        """ Return command tokens
                ...
                :return: tuple of str
                """
        return self._WCommand__command
...
class WCommandSelector:
    __doc__ = ' This class store command and selects suitable command for the given tokens.\n\t'

    def __init__(self):
        """ Create new storage/selector
                """
        self._WCommandSelector__commands = []
    ...
    def add(self, command_obj):
        """ Add command to selector
                ...
                :return: None
                """
        self._WCommandSelector__commands.append(command_obj)
    ...
    def __iter__(self):
        """ Iterate over internal storage and yield next command
                """
        for command in self._WCommandSelector__commands:
            yield command
    ...
    def __len__(self):
        """ Return command count
                ...
                :return: int
                """
        return len(self._WCommandSelector__commands)


class WCommandPrioritizedSelector(WCommandSelector):
    __doc__ = ' This class has priority for every stored commands. Command with lower priority value will be selected first.\n\t'

    @verify_type(default_priority=int)
    def __init__(self, default_priority=30):
        """ Create new selector

                :param default_priority: priority for commands, that were added via             :meth:`.WCommandPrioritizedSelector.add` method
                """
        WCommandSelector.__init__(self)
        self._WCommandPrioritizedSelector__default_priority = default_priority
        self._WCommandPrioritizedSelector__priorities = {}

    @verify_type(command_obj=WCommandProto)
    def add(self, command_obj):
        """ :meth:`.WCommandSelector.add` redefinition (sets default priority for the given command)
                """
        self.add_prioritized(command_obj, self._WCommandPrioritizedSelector__default_priority)
    ...
    def add_prioritized(self, command_obj, priority):
        """ Add command with the specified priority
                ...
                :return: None
                """
        if priority not in self._WCommandPrioritizedSelector__priorities.keys():
            self._WCommandPrioritizedSelector__priorities[priority] = []
        self._WCommandPrioritizedSelector__priorities[priority].append(command_obj)

    def __iter__(self):
        """ Iterate over internal storage and yield next command. Commands with lower priority will be yielded
                first
                """
        priorities = list(self._WCommandPrioritizedSelector__priorities.keys())
        priorities.sort()
        for priority in priorities:
            for command in self._WCommandPrioritizedSelector__priorities[priority]:
                yield command
    ...
    def __len__(self):
        """ Return command count
                ...
                """
        result = 0
        for commands in self._WCommandPrioritizedSelector__priorities.values():
            result += len(commands)
...
class WCommandSet:
    __doc__ = ' Class wraps routine of execution command from a command group. This class is able to keep command\n\tenvironment variables from previous commands results to use them in a future commands calls. Only\n\tthose variables whose names were specified in a constructor will be kept.\n\t'

    class NoCommandFound(Exception):
        __doc__ = ' Exception that is raised when no suitable command was found during :meth:`.WCommandSet.exec` method\n\t\t'
    ...
    def __init__(self, command_selector=None, tracked_vars=None):
        """ Create new set
                ...
                :param tracked_vars: if it is specified - tuple/list/set of variables names, that must be kept          between commands calls
                """
        self._WCommandSet__commands = command_selector if command_selector is not None else WCommandSelector()
        self._WCommandSet__tracked_vars = tuple(tracked_vars) if tracked_vars is not None else tuple()
        self._WCommandSet__vars = {}
    ...
    def commands(self):
        """ Return used command selector
                ...
                """
        return self._WCommandSet__commands
    ...
    def tracked_vars(self):
        """ Return variables names that are kept (tracked) by this command set
                ...
                :return: tuple of str
                """
        return self._WCommandSet__tracked_vars
    ...
    def has_var(self, var_name):
        """ Return True - if a environment variable with a specified name is kept by this command set.
                ...
                :return: bool
                """
        return var_name in self._WCommandSet__vars.keys()
    ...
    def var_value(self, var_name):
        """ Return value of environment variable that is kept by this command set.
                ...
                :return: anything
                """
        return self._WCommandSet__vars[var_name]
    ...
    def exec(self, command_str, **command_env):
        """ Execute the given command (command will be split into tokens, every space that is a part of a token
                ...
                :return: WCommandResultProto
                """
        env = self._WCommandSet__vars.copy()
        ...
        self._WCommandSet__track_vars(result)
        return result
    ...
    def __track_vars(self, command_result):
        ...
        for var_name in self.tracked_vars():
            if var_name in command_env.keys():
                self._WCommandSet__vars[var_name] = command_env[var_name]


class WCommandAlias(WCommandProto):
    __doc__ = " Prototype for command, that doesn't do anything useful itself, but it helps to run a modified command\n\tand to return its result. For getting any useful result a :class:`.WCommandSelector` object is used.\n\t"
    ...
    def __init__(self, selector):
        """ Create new command alias
                ...
                """
        WCommandProto.__init__(self)
        self._WCommandAlias__selector = selector
    ...
    def selector(self):
        """ Return original command selector
                ...
                :return: WCommandSelector
                """
        return self._WCommandAlias__selector
...
class WReduceCommand(WCommandAlias):
    ...
    def __init__(self, selector, *reduce_tokens):
        ...
        self._WReduceCommand__reduce_tokens = reduce_tokens
    ...
    def reduce_tokens(self):
        """ Return section names (aliases)
                ...
                :return: tuple of str
                """
        return self._WReduceCommand__reduce_tokens

    def mutate_command_tokens(self, *command_tokens):
        """ :meth:`.WCommandAlias.mutate_command_tokens` implementation
                """
        if len(command_tokens) > 0 and command_tokens[0] in self.reduce_tokens():
