...
# [GCC 8.4.0]
# Embedded file name: /home/ale/progr/github/wasp-general/wasp_general/task/dependency.py
# Compiled at: 2017-10-04 13:39:05
# Size of source mod 2**32: 13228 bytes
...
class WTaskDependencyRegistryStorage(WTaskRegistryStorage):
    __doc__ = ' Storage that is used to store :class:`.WDependentTask` task.\n\t'
    ...
    def __init__(self):
        ...
        self._WTaskDependencyRegistryStorage__started = []
        ...
        def check(check_task_cls, global_dependencies):
            ...
            for dependency in check_task_cls.__dependency__:
                ...
                if dependent_task is None and skip_unresolved is False:
                    raise RuntimeError("Task '%s' dependency unresolved (%s)" % (
    ...
    def started_tasks(self, task_registry_id=None, task_cls=None):
        """ Return tasks that was started. Result way be filtered by the given arguments.
                ...
                :param task_registry_id: if it is specified, then try to return single task which id is the same as             this value.
        ...
        if task_registry_id is not None:
            task = None
            for registered_task in self._WTaskDependencyRegistryStorage__started:
            ...
            if task_cls is not None and task is not None:
                ...
                return
            return task
        result = filter(lambda x: x is not None, self._WTaskDependencyRegistryStorage__started)
    ...
    def start_task(self, task_tag, skip_unresolved=False):
        ...
        def start_dependency(start_task_cls):
            ...
            self._WTaskDependencyRegistryStorage__started.append(start_task_cls.start_dependent_task())
    ...
    def stop_task(self, task_tag, stop_dependent=True, stop_requirements=False):
        """ Stop task with the given task tag. If task already stopped, then nothing happens.
                ...
                :param stop_dependent: if True, then every task, that require the given task as dependency, will be             stopped before.
                :param stop_requirements: if True, then every task, that is required as dependency for the given task,          will be stopped after.
                :return: None
                """
        ...
        def stop(task_to_stop):
            if task_to_stop in self._WTaskDependencyRegistryStorage__started:
                if isinstance(task_to_stop, WStoppableTask) is True:
                    task_to_stop.stop()
                self._WTaskDependencyRegistryStorage__started.remove(task_to_stop)

        def stop_dependency(task_to_stop):
            deeper_dependencies = []
            for dependent_task in self._WTaskDependencyRegistryStorage__started:
        ...
        def calculate_priorities(task_to_stop, *extra_tasks, current_result=None, requirements_left=None):
            ...
            return calculate_priorities(*list(nested_requirements), requirements_left=all_requirements)
