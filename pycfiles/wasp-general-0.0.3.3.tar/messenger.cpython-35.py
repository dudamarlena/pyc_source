...
# [GCC 8.4.0]
# Embedded file name: /home/ale/progr/github/wasp-general/wasp_general/network/beacon/messenger.py
# Compiled at: 2017-06-28 14:06:31
# Size of source mod 2**32: 18947 bytes
...
class WBeaconMessengerBase(metaclass=ABCMeta):
    __doc__ = ' This is interface for classes, that implement communication (messaging) logic for beacons\n\n\tsee also: :class:`.WNetworkBeacon`\n\t'
    message_maxsize = 512
...
class WBeaconMessenger(WBeaconMessengerBase):
    __doc__ = ' Simple. Demo/debug messenger. Server side just return original hello message to the sender.\n\t'
...
class WBeaconGouverneurMessenger(WBeaconMessengerBase):
    ...
    def __init__(self, hello_message, invert_hello=False):
        """ Construct new messenger
                ...
                :param invert_hello: this flag defines whether response will have the original header           ('hello_message' value) or it will have reversed value. For example, when this flag is set to True              and 'hello_message' is b'sample', then response will have b'elpmas' header.
                """
        WBeaconMessengerBase.__init__(self)
        self._WBeaconGouverneurMessenger__gouverneur_message = hello_message
        self._WBeaconGouverneurMessenger__invert_hello = invert_hello
    ...
    def invert_hello(self):
        """ Return whether this messenger was constructed with 'invert_hello' or not.
                ...
                :return: bool
                """
        return self._WBeaconGouverneurMessenger__invert_hello

    @verify_type(invert_hello=bool)
    def hello_message(self, invert_hello=False):
        """ Return message header.

                :param invert_hello: whether to return the original header (in case of False value) or reversed                 one (in case of True value).
                :return: bytes
                """
        if invert_hello is False:
            return self._WBeaconGouverneurMessenger__gouverneur_message
        hello_message = []
        for i in range(len(self._WBeaconGouverneurMessenger__gouverneur_message) - 1, -1, -1):
            hello_message.append(self._WBeaconGouverneurMessenger__gouverneur_message[i])
    ...
    def _message(self, beacon_config, invert_hello=False):
        """ Generate request/response message.
                ...
                :param beacon_config: server or client configuration. Client configuration is used for request and              server configuration for response
    ...
    def has_response(self, beacon_config, request, client_address):
        """ :meth:`.WBeaconMessengerBase.has_response` method implementation. This method compares request
                ...
                """
        try:
            self._message_address_parse(request, invert_hello=self._WBeaconGouverneurMessenger__invert_hello)
            return True
        except ValueError:
            pass
    ...
    def response(self, beacon_config, request, client_address):
        """ :meth:`.WBeaconMessengerBase.request` method implementation.
                ...
                see :class:`.WBeaconGouverneurMessenger`
                """
        return self._message(beacon_config, invert_hello=self._WBeaconGouverneurMessenger__invert_hello)
    ...
    def response_address(self, beacon_config, request, client_address):
        """ :meth:`.WBeaconMessengerBase.request` method implementation.
                ...
                see :class:`.WBeaconGouverneurMessenger`
                """
        si = self._message_address_parse(request, invert_hello=self._WBeaconGouverneurMessenger__invert_hello)
        address = si.address()
        port = si.port()
        return WIPV4SocketInfo(address if address is not None else client_address.address(), port if port is not None else client_address.port())
    ...
    def valid_response(self, beacon_config, response, server_address):
        """ :meth:`.WBeaconMessengerBase.valid_response` method implementation. Response when it has correct
                ...
                """
        try:
            self._message_address_parse(response, invert_hello=self._WBeaconGouverneurMessenger__invert_hello)
            return True
        except ValueError:
            pass
...
class WHostgroupBeaconMessenger(WBeaconGouverneurMessenger):
    ...
    def __init__(self, hello_message, *hostgroup_names, invert_hello=False):
        ...
        self._WHostgroupBeaconMessenger__hostgroups = []
        self._WHostgroupBeaconMessenger__hostgroups.extend([x.encode() for x in hostgroup_names])
    ...
    def hostgroups(self):
        """ Return list of host group names
                ...
                :return: list of str
                """
        return [x.decode() for x in self._WHostgroupBeaconMessenger__hostgroups]
    ...
    def _message_hostgroup_generate(self):
        """ Encode messenger host group names
                :return: bytes
                """
        return (b',').join(self._WHostgroupBeaconMessenger__hostgroups)
    ...
    def has_response(self, beacon_config, request, client_address):
        ...
        try:
            ...
            if len(self._WHostgroupBeaconMessenger__hostgroups) == 0 or len(groups) == 0:
                return True
            else:
                for group_name in groups:
                    if group_name in self._WHostgroupBeaconMessenger__hostgroups:
                        return True
