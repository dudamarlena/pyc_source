...
# [GCC 8.4.0]
# Embedded file name: /home/ale/progr/github/wasp-general/wasp_general/cache.py
# Compiled at: 2017-06-30 09:18:29
...
class WCacheStorage(metaclass=ABCMeta):
    __doc__ = ' Abstract class for cache storage\n\t'

    class CacheMissedException(Exception):
        __doc__ = " Exception is raised in :meth:`.WCacheStorage.get_result` and derived methods as an error for\n\t\taccessing cache record that doesn't exist\n\t\t"

    class CacheEntry:
        __doc__ = ' Cache request result, is used in :meth:`.WCacheStorage.get_cache` to determine if there is a\n\t\tcached value and what that value is.\n\t\t'
...
class WGlobalSingletonCacheStorage(WCacheStorage):
    __doc__ = " Simple storage that acts as global singleton. Result (singleton) is saved on the very first call. It doesn't\n\tmatter with which parameters function was called, result will be the same for all the rest calls.\n\t"
...
class WInstanceSingletonCacheStorage(WCacheStorage):
    ...
    class InstanceCacheRecord:
        ...
        def __init__(self, result, decorated_function):
            """ Create new cache record
                        ...
                        """
            self._InstanceCacheRecord__decorated_function = decorated_function
            self._InstanceCacheRecord__result = result
        ...
        def decorated_function(self):
            """ Return original method
                        ...
                        """
            return self._InstanceCacheRecord__decorated_function
        ...
        def cache_entry(self, *args, **kwargs):
            """ Return cache entry for the specified arguments
                        ...
                        :return: WCacheStorage.CacheEntry
                        """
            return WCacheStorage.CacheEntry(has_value=True, cached_value=self._InstanceCacheRecord__result)
    ...
    def __init__(self, cache_record_cls=None, statistic=False):
        ...
        self._WInstanceSingletonCacheStorage__statistic = statistic
        self._WInstanceSingletonCacheStorage__cache_missed = 0 if self._WInstanceSingletonCacheStorage__statistic is True else None
        self._WInstanceSingletonCacheStorage__cache_hit = 0 if self._WInstanceSingletonCacheStorage__statistic is True else None
    ...
    def put(self, result, decorated_function, *args, **kwargs):
        """ :meth:`WCacheStorage.put` method implementation
                """
        self._WInstanceSingletonCacheStorage__check(decorated_function, *args, **kwargs)
        ...
        if decorated_function in self._storage:
            for i in self._storage[decorated_function]:
                if i['instance']() == args[0]:
                    ...
                    if self._WInstanceSingletonCacheStorage__statistic is True:
                        if result.has_value is True:
                            self._WInstanceSingletonCacheStorage__cache_hit += 1
                        else:
                            self._WInstanceSingletonCacheStorage__cache_missed += 1
                    return result
    ...
    def clear(self, decorated_function=None):
        ...
        if decorated_function is not None and decorated_function in self._storage:
            ...
            self._storage.clear()
        if self._WInstanceSingletonCacheStorage__statistic is True:
            self._WInstanceSingletonCacheStorage__cache_missed = 0
            self._WInstanceSingletonCacheStorage__cache_hit = 0
    ...
    def cache_missed(self):
        """ Return cache misses (return None if class was constructed without 'statistic' flag)
                ...
                :return: int or None
                """
        return self._WInstanceSingletonCacheStorage__cache_missed
    ...
    def cache_hit(self):
        """ Return cache hits (return None if class was constructed without 'statistic' flag)
                ...
                :return: int or None
                """
        return self._WInstanceSingletonCacheStorage__cache_hit
...
def cache_control(validator=None, storage=None):
    r""" Decorator that is used for caching result.

        :param validator: function, that has following signature (decorated_function, \*args, \*\*kwargs), where        decorated_function - original function, args - function arguments, kwargs - function keyword arguments.         This function must return True if cache is valid (old result must be use if it there is one), or False - to     generate and to store new result. So function that always return True can be used as singleton. And function    that always return False won't cache anything at all. By default (if no validator is specified), it presumes    that cache is always valid.
