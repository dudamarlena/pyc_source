...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pybitbucket/consumer.py
...
class ConsumerPayload(PayloadBuilder):
    ...
    def __init__(self, payload=None, consumer_id=None):
    ...
    def add_consumer_id(self, consumer_id):
        ...
        new['id'] = consumer_id
        return ConsumerPayload(payload=new,
...
class Consumer(BitbucketBase):
    ...
    @staticmethod
    def is_type(data):
        return data.get('id') is not None and data.get('name') is not None and data.get('secret') is not None and data.get('key') is not None
    ...
    def create(cls, payload, client=None):
        ...
        api_url = expand(templates['create'], {'bitbucket_url':client.get_bitbucket_url(), 
         'username':owner})
    ...
    def find_consumers(client=Client()):
        ...
        """
        url = expand(Consumer.get_link_template('consumers'), {'bitbucket_url':client.get_bitbucket_url(), 
         'username':client.get_username()})
    ...
    def find_consumer_by_id(consumer_id, client=Client()):
        ...
        """
        url = expand(Consumer.get_link_template('self'), {'bitbucket_url':client.get_bitbucket_url(), 
         'username':client.get_username(), 
         'consumer_id':consumer_id})
