...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pybitbucket/build.py
...
class BuildStatusPayload(PayloadBuilder):
    ...
    def __init__(self, payload=None, owner=None, repository_name=None, revision=None):
    ...
    def add_owner(self, owner):
        return BuildStatusPayload(payload=(self._payload.copy()),
          owner=owner,
    ...
    def add_name(self, name):
        ...
        new['name'] = name
        return BuildStatusPayload(payload=new,
          owner=(self.owner),
          repository_name=(self.repository_name),
          revision=(self.revision))
...
class BuildStatus(BitbucketBase):
    __doc__ = 'Represents a build status.'
    id_attribute = 'key'
    resource_type = 'build'
    templates = {'create': '{+bitbucket_url}' + '/2.0/repositories{/owner,repository_name}' + '/commit{/revision}/statuses/build'}
    ...
    @classmethod
    def create(cls, payload, revision=None, repository_name=None, owner=None, client=None):
        ...
        if not (owner and repository_name and revision):
            raise ValueError('owner, repository_name, and revision are required')
    ...
    @staticmethod
    def find_buildstatus_for_repository_commit_by_key(repository_name, revision, key, owner=None, client=None):
        """
        ...
        return next(Bitbucket(client=client).repositoryCommitBuildStatusByKey(owner=owner,
          repository_name=repository_name,
          revision=revision,
          key=key))

    @staticmethod
    def find_buildstatuses_for_repository_commit(repository_name, revision, owner=None, client=None):
        """
        ...
        client = client or Client()
        owner = owner or client.get_username()
        return Bitbucket(client=client).repositoryCommitBuildStatuses(owner=owner,
          repository_name=repository_name,
          revision=revision)
