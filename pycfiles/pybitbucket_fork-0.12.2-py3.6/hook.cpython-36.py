...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pybitbucket/hook.py
...
class HookPayload(PayloadBuilder):
    ...
    schema = Schema({Required('description'): str, 
     Required('url'): str, 
    ...
    def add_owner(self, owner):
        return HookPayload(payload=(self._payload.copy()),
          owner=owner,
    ...
    def add_description(self, description):
        ...
        return HookPayload(payload=new,
          owner=(self.owner),
...
class Hook(BitbucketBase):
    id_attribute = 'uuid'
    resource_type = 'hooks'
    templates = {'create': '{+bitbucket_url}' + '/2.0/repositories' + '{/owner,repository_name}' + '/hooks'}
    ...
    @staticmethod
    def find_hook_by_uuid_in_repository(uuid, repository_name, owner=None, client=None):
        """
        ...
        return next(Bitbucket(client=client).repositoryHookById(owner=owner,
          repository_name=repository_name,
          uuid=uuid))

    @staticmethod
    def find_hooks_for_repository(repository_name, owner=None, client=None):
        """
        ...
        client = client or Client()
        owner = owner or client.get_username()
        return Bitbucket(client=client).repositoryHooks(owner=owner,
          repository_name=repository_name)
