...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pybitbucket/pullrequest.py
# Compiled at: 2016-12-28 18:40:02
# Size of source mod 2**32: 14514 bytes
from __future__ import unicode_literals
from functools import partial
from uritemplate import expand
from voluptuous import Schema, Required, Optional
from pybitbucket.bitbucket import Bitbucket, BitbucketBase, Client, PayloadBuilder, Enum
...
class PullRequestPayload(PayloadBuilder):
    ...
    schema = Schema({Required('title'): str, 
     Optional('description'): str, 
     Optional('close_source_branch'): bool, 
     Optional('reviewers'): [{Required('username'): str}], 
     Required('destination'): {Required('branch'): {Required('name'): str}, 
                               
                               Optional('commit'): {Required('hash'): str}}, 
     
     Required('source'): {Required('branch'): {Required('name'): str}, 
                          
                          Required('repository'): {Required('full_name'): str}, 
                          
                          Optional('commit'): {Required('hash'): str}}})

    def __init__(self, payload=None, destination_repository_owner=None, destination_repository_name=None):
    ...
    def add_title(self, title):
        ...
        new['title'] = title
        return PullRequestPayload(payload=new,
    ...
    def add_destination_repository_owner(self, owner):
        return PullRequestPayload(payload=(self._payload.copy()),
    ...
    def add_destination_branch(self, branch):
        ...
        return PullRequestPayload(payload=(self._payload.copy()),
          destination_repository_owner=owner,
          destination_repository_name=name).add_destination_branch_name(branch.name)
    ...
    def add_destination_commit(self, commit):
        ...
        return PullRequestPayload(payload=(self._payload.copy()),
          destination_repository_owner=owner,
          destination_repository_name=name).add_destination_commit_by_hash(commit.hash)
    ...
    def add_source_branch(self, branch):
        return self.add_source_branch_name(branch.name).add_source_repository_full_name(branch.repository.full_name)
    ...
    def add_source_commit(self, commit):
        return self.add_source_commit_by_hash(commit.hash).add_source_repository_full_name(commit.repository.full_name)


class PullRequest(BitbucketBase):
    id_attribute = 'id'
    resource_type = 'pullrequests'
    templates = {'create': '{+bitbucket_url}' + '/2.0/repositories' + '{/owner,repository_name}' + '/pullrequests'}
    ...
    def attr_from_subchild(self, target_attribute, child, child_object):
        if self.data.get(child, {}).get(child_object, {}):
            setattr(self, target_attribute, self.client.convert_to_object(self.data[child][child_object]))

    def __init__(self, data, client=Client()):
        super(PullRequest, self).__init__(data, client=client)
        self.attr_from_subchild('source_commit', 'source', 'commit')
        self.attr_from_subchild('source_repository', 'source', 'repository')
        self.attr_from_subchild('destination_commit', 'destination', 'commit')
        self.attr_from_subchild('destination_repository', 'destination', 'repository')
        if data.get('links', {}).get('approve', {}).get('href', {}):
            url = data['links']['approve']['href']
            setattr(self, 'approve', partial((self.post_approval),
              template=url))
            setattr(self, 'unapprove', partial((self.delete_approval),
              template=url))
        if data.get('links', {}).get('decline', {}).get('href', {}):
            url = data['links']['decline']['href']
            setattr(self, 'decline', partial((self.post),
              client=client, url=url, json=None))
        if data.get('links', {}).get('merge', {}).get('href', {}):
            url = data['links']['merge']['href']
            setattr(self, 'merge', partial((self.post),
              client=client, url=url, json=None))
        if data.get('links', {}).get('diff', {}).get('href', {}):
            url = data['links']['diff']['href']
            setattr(self, 'diff', partial((self.content),
              url=url))
    ...
    def create(cls, payload, repository_name=None, owner=None, client=None):
        ...
        owner = owner or payload.destination_repository_owner
        repository_name = repository_name or payload.destination_repository_name
    ...
    @staticmethod
    def find_pullrequest_by_id_in_repository(pullrequest_id, repository_name, owner=None, client=None):
        """
        ...
        return next(Bitbucket(client=client).repositoryPullRequestByPullRequestId(owner=owner,
          repository_name=repository_name,
          pullrequest_id=pullrequest_id))

    @staticmethod
    def find_pullrequests_for_repository_by_state(repository_name, owner=None, state=None, client=None):
        """
        ...
        return Bitbucket(client=client).repositoryPullRequestsInState(owner=owner,
          repository_name=repository_name,
          state=state)
