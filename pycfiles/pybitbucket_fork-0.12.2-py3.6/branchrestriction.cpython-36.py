...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pybitbucket/branchrestriction.py
# Compiled at: 2016-12-28 18:40:02
# Size of source mod 2**32: 7577 bytes
...
class BranchRestrictionPayload(PayloadBuilder):
    ...
    schema = Schema({Required('kind'): In(BranchRestrictionKind), 
     Optional('pattern'): str, 
     Optional('groups'): [
                          {Required('owner'): {Required('username'): str}, 
                           
                           Required('slug'): str}], 
     
     Optional('users'): [{Required('username'): str}]})

    def __init__(self, payload=None, owner=None, repository_name=None):
    ...
    def add_owner(self, owner):
        return BranchRestrictionPayload(payload=(self._payload.copy()),
          owner=owner,
    ...
    def add_kind(self, kind):
        ...
        new['kind'] = kind
        return BranchRestrictionPayload(payload=new,
          owner=(self.owner),
          repository_name=(self.repository_name))
    ...
    def add_group(self, group):
        return self.add_group_by_username_and_groupname(group.owner.username, group.name)

    def add_group_by_username_and_groupname(self, username, groupname):
        new = self._payload.copy()
        groups = self._payload.get('groups', [])
        groups.append({'owner':{'username': username}, 
         'slug':groupname})
        new['groups'] = groups
        return BranchRestrictionPayload(payload=new,
          owner=(self.owner),
...
class BranchRestriction(BitbucketBase):
    ...
    templates = {'create': '{+bitbucket_url}' + '/2.0/repositories' + '{/owner,repository_name}' + '/branch-restrictions'}
    ...
    @staticmethod
    def find_branchrestrictions_for_repository(repository_name, owner=None, client=None):
        """
        ...
        return Bitbucket(client=client).repositoryBranchRestrictions(owner=owner,
          repository_name=repository_name)

    @staticmethod
    def find_branchrestriction_for_repository_by_id(repository_name, restriction_id, owner=None, client=None):
        """
        ...
        client = client or Client()
        owner = owner or client.get_username()
        return next(Bitbucket(client=client).repositoryBranchRestrictionByRestrictionId(owner=owner,
          repository_name=repository_name,
