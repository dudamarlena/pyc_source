...
# [GCC 8.4.0]
# Embedded file name: /home/kiv/projects/python-build-system/python_build_system/modules/platform/mcu.py
# Compiled at: 2016-10-24 12:34:44
...
def convert_to_hex(ctx, source):
    ...
    if not os.path.isfile(objcopy):
        objcopy += '.exe'
        return os.path.isfile(objcopy) or None
...
def display_elf_size(ctx, source):
    ...
    if not os.path.isfile(size):
        size += '.exe'
        return os.path.isfile(size) or None
...
def load_to_context(ctx, config):
    ...
    ctx.add_rule('.*\\.elf$', convert_to_hex)
    ctx.add_rule('.*\\.elf$', convert_to_bin)
    ctx.add_rule('.*\\.elf$', display_elf_size)