...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/flask_captcha/views.py
# Compiled at: 2014-06-13 14:58:08
...
import time, random, re, tempfile, os, subprocess
try:
...
def set_serializable():
    if 'postgres' in current_app.config.get('SQLALCHEMY_DATABASE_URI', ''):
        if current_app.config.get('CAPTCHA_SERIALIZE_TRANSACTIONS', False):
            import psycopg2
            db.session.commit()
...
def serializable_retry(func, max_num_retries=None):
    ...
    def wrap(max_num_retries, *args, **kwargs):
        if max_num_retries is None:
            max_num_retries = current_app.config.get('MAX_NUM_SERIALIZED_RETRIES', 1)
        retries = 1
...
def make_image(text):
    ...
    image = Image.new('RGB', size, current_app.config['CAPTCHA_BACKGROUND_COLOR'])
    try:
