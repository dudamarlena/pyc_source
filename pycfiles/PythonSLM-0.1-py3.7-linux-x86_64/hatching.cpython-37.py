...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyslm/hatching/hatching.py
# Compiled at: 2020-05-02 13:22:01
# Size of source mod 2**32: 29772 bytes
import pyclipper, numpy as np, abc
...
class BaseHatcher(abc.ABC):
    ...
    def error(cls) -> float:
        ...
        """
        return 1.0 / cls.PYCLIPPER_SCALEFACTOR
                ...
                if plotArrows:
                    if not plot3D:
                        for hatch in hatches:
                            ...
                            plt.annotate('', xytext=(midPoint - delta * 0.0001),
                              xy=midPoint,
        ...
        if plotContours:
            for contourGeom in layer.contours:
                ...
                if plotArrows:
                    ...
                    if plot3D:
                        ax.plot((contourGeom.coords[:, 0]), (contourGeom.coords[:, 1]), zs=zPos, color=lineColor, linewidth=lineWidth)
                else:
                    ax.plot((contourGeom.coords[:, 0]), (contourGeom.coords[:, 1]), color=lineColor, linewidth=lineWidth)
    ...
    def offsetBoundary(self, paths, offset: float):
        ...
        for path in paths:
            pc.AddPath(self.scaleToClipper(path), pyclipper.JT_ROUND, pyclipper.ET_CLOSEDPOLYGON)
    ...
    def _getChildPaths(self, poly):
        offsetPolys = []
        paths = [path.Contour for path in poly.Childs]
        paths.append(poly.Contour)
        for path in paths:
            path.append(path[0])
        ...
        for i in range(self._numOuterContours):
            ...
            for poly in offsetBoundary:
                for path in poly:
                    ...
                    layer.contours.append(contourGeometry)
...
class StripeHatcher(Hatcher):
    ...
    def generateHatching(self, paths, hatchSpacing: float, hatchAngle: float=90.0):
        ...
        for i in np.arange(0, numStripes):
            ...
            y = np.tile(np.arange((-bboxRadius + np.mod(i, 2) * self._stripeOffset * hatchSpacing), (bboxRadius + np.mod(i, 2) * self._stripeOffset * hatchSpacing),
              hatchSpacing, dtype=(np.float32)).reshape(-1, 1), 2).flatten()
    ...
    def generateHatching(self, paths, hatchSpacing: float, hatchAngle: float=90.0):
        ...
        for i in np.arange(0, numIslands):
            for j in np.arange(0, numIslands):
                ...
                if np.mod(i + j, 2):
                    y = np.tile(np.arange((startY + np.mod(i + j, 2) * self._islandOffset * hatchSpacing), (endY + np.mod(i + j, 2) * self._islandOffset * hatchSpacing),
                      hatchSpacing, dtype=(np.float32)).reshape(-1, 1), 2).flatten()
                    x = np.array([startX, endX])
                    x = np.resize(x, y.shape)
                    z = np.arange(hatchOrder, hatchOrder + y.shape[0] / 2, 0.5).astype(np.int64)
                else:
                    x = np.tile(np.arange((startX + np.mod(i + j, 2) * self._islandOffset * hatchSpacing), (endX + np.mod(i + j, 2) * self._islandOffset * hatchSpacing),
                      hatchSpacing, dtype=(np.float32)).reshape(-1, 1), 2).flatten()
