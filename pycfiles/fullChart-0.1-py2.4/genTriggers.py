...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/charts/genTriggers.py
# Compiled at: 2006-04-13 14:06:08
import os, sys, traceback, re
from pysqlite2 import dbapi2 as sqlite
TESTDIR = './test/'
TESTLOG = os.path.join(TESTDIR, 'genTriggers.log')

class GenTriggersError(Exception):
    __module__ = __name__
...
class constraint:
    ...
    def __init__(self, child_table_name, child_col_name, child_constraint_name, parent_table_name, parent_col_name, on_delete_cascade):
    ...
    def __repr__(self):
        return '%s %s %s %s %s' % (self.child_table_name, self.child_col_name, self.child_constraint_name, self.parent_table_name, self.parent_col_name)


def _readsql(sqlfile):
    import pkg_resources
    if not pkg_resources.resource_exists(__name__, sqlfile):
        raise GenTriggersError('readsql file does not exist: %s' % sqlfile)
...
def _filterConstraint(sql):
    ...
    TBL = '--['
    TB2 = ']'
    if not (sql.find(INT.lower()) >= 0 and sql.find(CON.lower()) >= 0 and sql.find(REF.lower()) >= 0 and sql.find(TBL.lower()) >= 0 and sql.find(TB2.lower()) >= 0):
        return
    ...
    c = temp()
    if sql.find(OND.lower()) > 0:
        c.on_delete_cascade = True
    else:
        c.on_delete_cascade = False
    ...
    const = constraint(c.child_table_name, c.child_col_name, c.child_constraint_name, c.parent_table_name, c.parent_col_name, c.on_delete_cascade)
    return const
...
def _genTriggers(sql, c):
    ...
    if c.on_delete_cascade == True:
        retval.append(_genCascadeDeleteTrigger(c))
    else:
        retval.append(_genDeleteTrigger(c))
    return ('\n').join(retval)
...
def _fileToText(f):
    import pkg_resources
    if not pkg_resources.resource_exists(__name__, f):
        raise GenTriggersError('fileToText file does not exist: %s' % f)
...
def _execSql(db, sql, script=False):
    ...
    for line in sql.splitlines():
        if len(line) > 0:
            if not line.endswith(';') and script == False:
                line = '%s;' % line
            f.write('%s\n' % line)
