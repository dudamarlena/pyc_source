...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/charts/gdchart2A.py
...
"""
import types, _gdchartc

class GDChartError(Exception):
    __module__ = __name__
...
def _uniformLength(*lst):
    """
        ...
        The lists are assumed to be of the form:
                '''[['a','b','c'],['d','e','f']],'''
        Note the trailing comma, introduced when the list is transformed into
        a tuple (see the *lst above).
        Returns a 0 if lengths differ, 1 if not.
        This is an internal PyGDChart utility function.
    """
    baselist = lst[0]
    baselen = len(baselist[0])
    for l in baselist:
        if len(l) != baselen:
            return -1
...
class ChartBase(object):
    ...
    """
    __module__ = __name__
    _ImageTypes = {'GIF': _gdchartc.GDC_GIF, 'JPEG': _gdchartc.GDC_JPEG, 'PNG': _gdchartc.GDC_PNG, 'WBMP': _gdchartc.GDC_WBMP}
    _FontSizes = {'TINY': _gdchartc.GDC_TINY, 'SMALL': _gdchartc.GDC_SMALL, 'MEDBOLD': _gdchartc.GDC_MEDBOLD, 'LARGE': _gdchartc.GDC_LARGE, 'GIANT': _gdchartc.GDC_GIANT}
    _lookupOptions = {}
    _maskOptions = {}
            ...
            if self._lookupOptions.has_key(option):
                ...
                for i in myval:
                    ...
                    if bits is not None:
                        ...
                        raise GDChartError('%s must be a list chosen from the following strings: %s' % (option, values))
                ...
                elif type in (_gdchartc.OPT_FONTSIZE, _gdchartc.OPT_INT, _gdchartc.OPT_LONG, _gdchartc.OPT_SHORT, _gdchartc.OPT_USHORT, _gdchartc.OPT_UCHAR, _gdchartc.OPT_CHAR):
                    try:
                        val = int(myval)
                    except ValueError:
    ...
    def setLabels(self, labels):
        ...
        if self._datalen:
            if len(labels) != self._datalen:
                raise GDChartError, 'List of labels must have same length as data:self._datalen:len(labels):%s:%s' % (self._datalen, len(labels))
        self._labels = labels
...
class PieBase(ChartBase):
    ...
    _ChartTypes = {'3D': _gdchartc.GDC_3DPIE, '2D': _gdchartc.GDC_2DPIE}
    _PercentPlacement = {'NONE': _gdchartc.GDCPIE_PCT_NONE, 'ABOVE': _gdchartc.GDCPIE_PCT_ABOVE, 'BELOW': _gdchartc.GDCPIE_PCT_BELOW, 'RIGHT': _gdchartc.GDCPIE_PCT_RIGHT, 'LEFT': _gdchartc.GDCPIE_PCT_LEFT}
    _lookupOptions = {'title_size': ChartBase._FontSizes, 'label_size': ChartBase._FontSizes, 'image_type': ChartBase._ImageTypes, 'percent_labels': _PercentPlacement}
    ...
    def draw(self, filedef):
        ...
        try:
            _gdchartc.out_pie(self._width, self._height, filedef, self._type, self._datalen, self._labels, self._data)
...
class Scatter(object):
    ...
    """
    __module__ = __name__
    _ScatterTypes = {'TRIANGLE_DOWN': _gdchartc.GDC_SCATTER_TRIANGLE_DOWN, 'TRIANGLE_UP': _gdchartc.GDC_SCATTER_TRIANGLE_UP, 'CIRCLE': _gdchartc.GDC_SCATTER_CIRCLE}
...
class GraphBase(ChartBase):
    ...
    _StackTypes = {'DEPTH': _gdchartc.GDC_STACK_DEPTH, 'SUM': _gdchartc.GDC_STACK_SUM, 'BESIDE': _gdchartc.GDC_STACK_BESIDE, 'LAYER': _gdchartc.GDC_STACK_LAYER}
    _TickTypes = {'LABELS': _gdchartc.GDC_TICK_LABELS, 'POINTS': _gdchartc.GDC_TICK_POINTS, 'NONE': _gdchartc.GDC_TICK_NONE}
    _BorderTypes = {'NONE': _gdchartc.GDC_BORDER_NONE, 'ALL': _gdchartc.GDC_BORDER_ALL, 'X': _gdchartc.GDC_BORDER_X, 'Y': _gdchartc.GDC_BORDER_Y, 'Y2': _gdchartc.GDC_BORDER_Y2, 'TOP': _gdchartc.GDC_BORDER_TOP}
    _HilocloseTypes = {'DIAMOND': _gdchartc.GDC_HLC_DIAMOND, 'CLOSE_CONNECTED': _gdchartc.GDC_HLC_CLOSE_CONNECTED, 'CONNECTING': _gdchartc.GDC_HLC_CONNECTING, 'I_CAP': _gdchartc.GDC_HLC_I_CAP}
    ...
    _maskOptions = {'hlc_style': _HilocloseTypes, 'border': _BorderTypes}
    ...
    def draw(self, filedef):
        ...
        try:
            _gdchartc.out_graph(self._width, self._height, filedef, self._type, self._datalen, self._labels, self._numsets, self._data, self._combodata)
        except _gdchartc.PGError, val:
            raise GDChartError, val
