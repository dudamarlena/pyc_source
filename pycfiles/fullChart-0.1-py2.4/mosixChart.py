...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/charts/mosixChart.py
# Compiled at: 2006-04-13 13:59:58
import sys, os
from pysqlite2 import dbapi2
import pkg_resources, gdchart2A, mosixDataStore

class MosixChartError(Exception):
    __module__ = __name__


FILTERLIST = [
 'USER', 'LOGINS', 'tty', 'pts', 'load average', 'PST']
PREFIX = 'mos'
...
def generateCharts(MOSIX_LOG_FILE, CHARTOUTPUT_DIR, MOSIX_DB_FILE):
    ...
    data = [
     data]
    chart.makeMosixChartBar3D('Mosix Cluster', data, labels, 'Nodes', '%CPU UTILIZATION (simple)', '%s/mosix-cpu-simple.png' % CHARTOUTPUT_DIR)
    (data, labels) = chart.getNode_TotalMem()
    (labels, data) = _sortbymachinenoTwoCol(labels, data[0], PREFIX)
    data = [data]
    chart.makeMosixChartBar3D('Mosix Cluster', data, labels, 'Nodes', '%MEM UTILIZATION', '%s/mosix-mem.png' % CHARTOUTPUT_DIR)
    (data, labels) = chart.getNode_TotalCPU()
    (labels, data) = _sortbymachinenoTwoCol(labels, data[0], PREFIX)
    data = [data]
    chart.makeMosixChartBar3D('Mosix Cluster', data, labels, 'Nodes', "%CPU UTILIZATION\n(not normalized by # cpu's", '%s/mosix-cpu.png' % CHARTOUTPUT_DIR)
...
if __name__ == '__main__':
    ...
    l = len(sys.argv)
    if l >= 2:
        MOSIX_LOG_FILE = sys.argv[1]
    else:
        MOSIX_LOG_FILE = './mosix.log'
    if l >= 3:
        CHARTOUTPUT_DIR = sys.argv[2]
    else:
        CHARTOUTPUT_DIR = '.'
    if l >= 4:
        MOSIX_DB_FILE = sys.argv[3]
    else:
        MOSIX_DB_FILE = './mosix.db'
    if l < 2 or l >= 5:
        print USAGE
        sys.exit(-1)
