...
# [GCC 8.4.0]
# Embedded file name: /Users/grimen/Dev/Private/python-connection-uri/connection_uri/uri.py
# Compiled at: 2019-02-04 22:41:53
import rootpath
rootpath.append()
import mybad, inspecta, re, yaml
try:
    ...
    def serialize(self, data):
        try:
            if data is None:
                ...
                result = ('{protocol}://{auth}@{endpoints}{path}?{query}').format(protocol=protocol, auth=auth, endpoints=endpoints, path=path, query=urlencode(query))
            ...
            raise URIError(error, details={'data': data})
        ...
        try:
            if data is None:
                ...
                credentials = {'username': None, 
                   'password': None}
                if auth:
                    auth = dict(enumerate(auth.split('://'))).get(1, auth)
                    if auth == ':':
                        auth = None
                ...
                for endpoint_index, endpoint in enumerate(endpoints):
                    endpoint_result = {'protocol': protocol, 
                       'auth': auth, 
                       'endpoint': endpoint, 
                       'endpoints': [
                                   endpoint], 
                       'host': host, 
                       'hosts': [
                               hosts[endpoint_index]], 
                       'port': port, 
                       'ports': [
                               ports[endpoint_index]], 
                       'path': path, 
                       'query': query, 
                       'credentials': credentials, 
                       'key': key}
