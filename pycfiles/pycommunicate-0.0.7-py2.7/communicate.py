...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pycommunicate\server\app\communicate.py
# Compiled at: 2016-06-12 13:05:30
...
class CommunicateApp:
    ...
    def __init__(self, web_port=8080, host='localhost', debug=False, maximum_handler_threads=10000, template_directory='templates'):
    ...
    def _do_js_lib(self):
        ...
        if user is None:
            user = self.user_tracker.connect_user()
            session['shared_id'] = user.id_
        return user.active_controller.templater.render('__pycommunicate_js/pycommunicate.js', request_id=user.request_id)
    ...
    def add_controller(self, route, controller):
        if route.startswith('/__pycommunicate'):
            raise ValueError('Routes cannot start with /__pycommunicate, as this is reserved for pycommunicate itself')
