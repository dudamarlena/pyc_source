...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/ssh_keydb/plugins/user.py
...
class UserController(Controller):
    ...
    list.usage = {'shortdesc': 'Manage users', 
       'usage': [
               '%(exec)s user list [--user=<user>] [--location=<location>] [--long]'], 
    ...
    set.usage = {'shortdesc': 'Manage users', 
       'usage': [
               '%(exec)s user set <user> <location> [--keyfile=<keyfile>] [--keystring=<string>]'], 
       'options': {'help': 'displays the current help', 
                   'dbpath=': 'database path (~/.ssh-keydb.db by default)', 
                   'keyfile=': 'set/filter by key from file', 
                   'keystring=': 'set/filter by key from string'}, 
       'shortopts': {'help': 'h', 'dbpath': 'd:'}}
    remove.usage = {'shortdesc': 'Manage users', 
       'usage': [
               '%(exec)s user remove [--user=<user>] [--key=<key>]'], 
       'options': {'help': 'displays the current help', 
                   'dbpath=': 'database path (~/.ssh-keydb.db by default)', 
