...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/ssh_keydb/plugins/server.py
# Compiled at: 2012-10-29 03:35:38
import sys
from model import *
...
class ServerController(Controller):
    ...
    set.usage = {'shortdesc': 'Manage servers', 
       'usage': [
               '%(exec)s server set <server> [<fqdn> [<servergroup>] ]'], 
       'options': {'help': 'displays the current help', 
                   'dbpath=': 'database path (~/.ssh-keydb.db by default)'}, 
       'shortopts': {'help': 'h', 'dbpath': 'd:'}}
    key.usage = {'shortdesc': 'Manage server keys', 
       'usage': [
       ...
       'options': {'help': 'displays the current help', 
                   'dbpath=': 'database path (~/.ssh-keydb.db by default)'}, 
       'shortopts': {'help': 'h', 'dbpath': 'd:'}}
    remove.usage = {'shortdesc': 'Manage servers', 
       'usage': [
               '%(exec)s server remove [--server=<server>]'], 
       'options': {'help': 'displays the current help', 
                   'dbpath=': 'database path (~/.ssh-keydb.db by default)', 
                   'server=': 'filter by server name'}, 
       'shortopts': {'help': 'h', 'dbpath': 'd:'}}
    list.usage = {'shortdesc': 'Manage servers', 
       'usage': [
               '%(exec)s server list [--server=<server>]'], 
       'options': {'help': 'displays the current help', 
                   'dbpath=': 'database path (~/.ssh-keydb.db by default)', 
                   'server=': 'filter by server name'}, 
       'shortopts': {'help': 'h', 'dbpath': 'd:'}}
