...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/ssh_keydb/plugins/generator.py
# Compiled at: 2013-02-18 12:34:41
                ...
                for perm in self._permlist:
                    ...
                    for memb in self._memblist:
                        ...
                        if memb.role == perm.role and memb.user.location == perm.location:
                            if (
                             login, perm.server.server) not in outputsrvlst:
                                outputsrvlst.append((login, perm.server.server))
                            output.process(perm, memb)

            return outputsrvlst

    openssh.usage = {'shortdesc': 'Generate authorization file', 
       'usage': [
               '%(exec)s generate openssh [--server=<server>] [--group=<group>] [--role=<role>] [--login=<login>] [--output=<pattern>]'], 
