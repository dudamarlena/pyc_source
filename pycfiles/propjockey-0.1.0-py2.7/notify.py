...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/propjockey/notify.py
...
def notify():
    ...
    for r in requests_pending:
        ...
        if entry:
            vcoll.update_one({'_id': r['_id']}, {'$set': vconf['filter_completed']})
    ...
    for r in requests_needing_notification:
        time.sleep(5)
        eid = r[vconf['entry_id']]
        response = mailer.send({'to': r[vconf['requesters']], 
           'subject': nconf['user_subject'].format(eid), 
           'text': nconf['user_text'].format(eid, econf['url_for_prop'].format(e_id=eid)), 
           'from': nconf['from'], 
           'use_bcc': True, 
           'to_for_bcc': nconf['to_for_bcc']})
        if hasattr(response, 'status_code') and response.status_code == 200:
            print ('Sent notification about {} to {} requesters.').format(eid, len(r[vconf['requesters']]))
            vcoll.update_one({'_id': r['_id']}, {'$set': {vconf['requesters_notified']: True}})
            requests_with_notification_sent.append(r)
        responses.append(response)

    body_staff = ('\n').join([ nconf['staff_text'].format(eid=r[vconf['entry_id']], n=r[vconf['nvotes']], s='s' if r[vconf['nvotes']] > 1 else '', url_for_prop=econf['url_for_prop'].format(e_id=r[vconf['entry_id']])) for r in requests_with_notification_sent
                             ])
    n_entries = len(requests_with_notification_sent)
    n_users = len({u for r in requests_with_notification_sent for u in r[vconf['requesters']]})
    if n_entries:
        response = mailer.send({'to': nconf['staff_to'], 
           'subject': nconf['staff_subject'].format(n_entries, n_users), 
           'text': body_staff, 
           'from': nconf['from'], 
           'use_bcc': False})
        if hasattr(response, 'status_code') and response.status_code == 200:
        ...
        print ('No notifications required for votes collection {}').format(vcoll)
    return responses
