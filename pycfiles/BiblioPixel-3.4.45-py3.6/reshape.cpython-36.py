...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bibliopixel/util/image/reshape.py
...
def resize(image, x, y, stretch=False, top=None, left=None, mode='RGB', resample=None):
    ...
    if stretch:
        ...
        if ratios[0] < ratios[1]:
            new_size = (
             size[0], int(image.size[1] * ratios[0]))
        else:
            new_size = (
             int(image.size[0] * ratios[1]), size[1])
