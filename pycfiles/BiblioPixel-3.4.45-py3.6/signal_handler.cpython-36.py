...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bibliopixel/util/signal_handler.py
# Compiled at: 2019-08-11 12:22:47
# Size of source mod 2**32: 2465 bytes
import contextlib, os, signal
from . import log, pid_context
SIGNAL_NUMBERS = {k:v for k, v in signal.__dict__.items() if k.startswith('SIG') if '_' not in k if '_' not in k}
...
def run(filename, stopper, stop_signals=STOP_SIGNALS, restart_signals=RESTART_SIGNALS):
...
def make_command(default_signal, help=''):
    ...
    def add_arguments(parser):
        parser.add_argument('signal',
          nargs='?', default=default_signal, choices=(sorted(SIGNAL_NUMBERS)),
