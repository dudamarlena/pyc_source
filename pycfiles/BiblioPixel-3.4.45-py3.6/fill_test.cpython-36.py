...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/test/bibliopixel/project/fill_test.py
...
BASE = {'animation':{'run':{},  'datatype':Off, 
  'name':'Off'}, 
 'controls':[],  'drivers':[
  {'typename': 'simpixel'}], 
 'layout':{},  'maker':{'typename': 'bibliopixel.project.data_maker.Maker'}, 
 'path':'', 
 'typename':'bibliopixel.project.project.Project'}

def fill_before(desc):
    desc = merge.merge(merge.DEFAULT_PROJECT, {'animation': {'typename': 'off'}}, desc)
...
class FillTest(unittest.TestCase):
    ...
    def test_layout(self):
        source = {'animation': {'datatype':Matrix, 
                       'width':23, 
                       'height':32, 
                       'wombat':7}}
        actual = fill_after(copy.deepcopy(source))
        expected = {'aliases':{},  'shape':(),  'driver':{},  'drivers':[],  'layout':{'width':23, 
          'height':32, 
