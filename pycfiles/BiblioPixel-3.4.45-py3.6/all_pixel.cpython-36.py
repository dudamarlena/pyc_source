...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bibliopixel/commands/all_pixel.py
...
def run(args):
    ...
    try:
        ...
        d = ''
        if not devs:
            get_input('No devices found! Please connect one and press any key...')
            raise ValueError()
        if len(devs) == 1:
            d = devs[0][1]
        else:
...
def add_arguments(parser):
    ...
    parser.add_argument('--hardware-id', default='1D50:60AB', help='USB Vendor ID : Product ID of device. Defaults to VID:PID for AllPixel')
