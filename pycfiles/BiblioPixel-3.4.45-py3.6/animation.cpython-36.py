...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bibliopixel/animation/animation.py
...
class Animation(object):
    ...
    def __init__(self, layout, *, preclear=True, fail_on_exception=None, **kwds):
        ...
        """
        self.palette = (legacy_palette.pop_legacy_palette)(kwds, *self.COLOR_DEFAULTS)
    ...
    def _run_one_frame(self):
        if self.top_level:
            ...
            self.cur_step += 1
            if self.state == runner.STATE.complete:
                if self.runner.max_cycles > 0:
    ...
    def _set_runner(self, run):
        ...
        self.threading = animation_threading.AnimationThreading(self.runner, self.run_all_frames)
...
def _report_framerate(timestamps):
    ...
    log.frame('%dms/%dfps / Frame: %dms / Update: %dms', 1000 * total_time, fps, 1000 * (timestamps[1] - timestamps[0]), 1000 * (timestamps[2] - timestamps[1]))