...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bibliopixel/drivers/serial/devices.py
...
def Devices(hardware_id, baudrate):
    ...
    if devices:
        if devices.baudrate != baudrate:
            log.warning('Expected baudrate %s but got %s', baudrate, devices.baudrate)
    else:
...
class DevicesImpl(object):
        ...
        if self.devices is not None:
            ...
            for ports in serial.tools.list_ports.grep(hardware_id):
                ...
                try:
                    ...
                    log.debug('Error getting device_id for %s, %s', port, self.baudrate)
                    raise
                    continue

                if getattr(ports, '__len__', lambda : 0)():
                    log.debug('Multi-port device %s:%s:%s with %s ports found', self.hardware_id, id, ver, len(ports))
                if id < 0:
                    log.debug('Serial device %s:%s:%s with id %s < 0', self.hardware_id, id, ver)
                else:
                    self.devices[id] = (
                     port, ver)
    ...
    def get_device(self, id=None):
        ...
        log.info('Using COM Port: %s, Device ID: %s, Device Ver: %s', device, id, version)
    ...
    def set_device_id(self, dev, id):
        ...
        com, code, ok = io.send_packet(CMDTYPE.SETID, 1, dev, self.baudrate, 5, id)
        if not ok:
            raise_error(code)
    ...
    def _get_device_version(self, dev):
        com, code, ok = io.send_packet(CMDTYPE.GETVER, 0, dev, self.baudrate, 0.5)
