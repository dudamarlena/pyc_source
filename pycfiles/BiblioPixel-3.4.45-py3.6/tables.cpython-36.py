...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bibliopixel/colors/tables.py
...
def _classic_pairs():
    find = re.compile('[A-Z][a-z0-9]+').finditer
    for name in dir(classic):
        if name == 'Green_HTML':
            yield (
             'green html', classic.Green_HTML)
        else:
...
_TO_NAME = {v:k for k, v in COLOR_DICT.items() if k not in _SECONDARY_NAMES if k not in _SECONDARY_NAMES}
