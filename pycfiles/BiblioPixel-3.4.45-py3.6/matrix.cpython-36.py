...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bibliopixel/layout/matrix.py
# Compiled at: 2019-08-11 12:22:47
# Size of source mod 2**32: 12772 bytes
...
class Matrix(MultiLayout):
    CLONE_ATTRS = MultiLayout.CLONE_ATTRS + ('width', 'height', 'rotation', 'vert_flip',
                                             'y_flip', 'serpentine', 'pixelSize')

    def __init__(self, drivers, width=0, height=0, rotation=0, vert_flip=False, y_flip=False, serpentine=True, threadedUpdate=False, brightness=255, pixelSize=(1, 1), **kwargs):
        ...
        if self.width * self.height > self.numLEDs:
            raise ValueError('width * height cannot exceed total pixel count! %s * %s > %s' % (
             self.width, self.height, self.numLEDs))
        if not self.coord_map:
            if len(self.drivers) == 1:
                log.debug('Auto generating coordinate map. Use make_matrix_coord_map directly if more control needed.')
                y_flip = y_flip or vert_flip
                self.coord_map = make_matrix_coord_map((self.width),
                  (self.height), serpentine=serpentine,
                  rotation=rotation,
                  y_flip=vert_flip)
            elif self.drivers:
        ...
        if self.width % pw != 0 or self.height % ph != 0:
            ...
            self._set = self._Matrix__setNormal
        else:
            self._set = self._Matrix__setScaled
    ...
    def __setScaled(self, x, y, color):
        ...
        for xs in range(sx, sx + self.pixelSize[0]):
            for ys in range(sy, sy + self.pixelSize[1]):
                self._Matrix__setNormal(xs, ys, color)
    ...
    def drawChar(self, x, y, c, color, bg, aa=False, font=font.default_font, font_scale=1):
        ...
        """
        md.draw_char(self.fonts, self.set, self.width, self.height, x, y, c, color, bg, aa, font, font_scale)

    def drawText(self, text, x=0, y=0, color=None, bg=colors.COLORS.Off, aa=False, font=font.default_font, font_scale=1):
        """
        ...
        :param aa: if True, use Bresenham's algorithm for line drawing;
            otherwise use Xiaolin Wu's algorithm
        """
        md.draw_text(self.fonts, self.set, text, self.width, self.height, x, y, color, bg, aa, font, font_scale)
