...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bibliopixel/project/project_runner.py
...
def _get_projects(args):
    ...
    if movie != '':
        ...
        if filename:
            if not any(filename.endswith(s) for s in MOVIE_SUFFIXES):
                log.warning("--movie argument %s didn't end with %s", movie, ' or '.join(MOVIE_SUFFIXES))
                movie += '.gif'
            if len(args.name) > 1:
 ...
 args, *descs, **{'root_file': root_file})
    ...
    if not failed:
        return projects
    log.error(FAILURE_ERROR.format(count=(len(failed)),
      s=('' if len(failed) == 1 else 's')))
...
def _might_be_classname(s):
    return s and set(s) <= _ALL_CHARS and s[0] in _START_CHARS and s[(-1)] in _END_CHARS