...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/scxml/pyscxml.py
# Compiled at: 2012-01-07 09:36:53
""" 
...
import logging, os, eventlet, errno, re
...
class StateMachine(object):
    ...
    def _open_document(self, uri):
        if hasattr(uri, 'read'):
            ...
            if isinstance(uri, basestring) and re.search('<(.+:)?scxml', uri):
...
class MultiSession(object):
    ...
    def set_processors(self, sm):
        sm.datamodel['_ioprocessors'] = {'scxml': {'location': '#_scxml_' + sm.datamodel['_sessionid']}, 'basichttp': {'location': '#_scxml_' + sm.datamodel['_sessionid']}}
    ...
    def on_sm_exit(self, sender):
        if sender.sessionid in self:
            ...
            self.logger.error("The session '%s' reported exit but it can't be found in the mapping." % sender.sessionid)
...
__all__ = [
 'StateMachine', 'MultiSession', 'custom_executable', 'preprocessor', 'expr_evaluator', 'expr_exec']
