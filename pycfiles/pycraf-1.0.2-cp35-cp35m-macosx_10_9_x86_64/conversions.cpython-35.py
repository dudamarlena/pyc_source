...
# [GCC 8.4.0]
# Embedded file name: /Users/runner/runners/2.166.2/work/1/s/build/lib.macosx-10.9-x86_64-3.5/pycraf/conversions/conversions.py
# Compiled at: 2020-04-16 04:29:51
# Size of source mod 2**32: 25096 bytes
...
__all__ = [
 ...
 'free_space_loss',
 'Erx_unit', 'R0', 'efield_equivalency'] + UNITS
...
R0 = (1.0 * (con.mu0 / con.eps0) ** 0.5).to(apu.Ohm)
Erx_unit = ((1 * apu.W / 4.0 / np.pi * R0) ** 0.5 / (1 * apu.km)).to(apu.uV / apu.m)
...
@utils.ranged_quantity_input(freq=(
 0, None, apu.Hz), strip_input_units=True, output_unit=apu.m ** 2)
...
@utils.ranged_quantity_input(geom_area=(
 0, None, apu.m ** 2), eta_a=(
 0, 100, apu.percent), strip_input_units=True, output_unit=apu.m ** 2)
def eff_from_geom_area(geom_area, eta_a):
    ...
    `~astropy.units.Quantity`), unit conversion is automatically performed.
    """
    return geom_area * eta_a / 100.0


@utils.ranged_quantity_input(eff_area=(
 0, None, apu.m ** 2), eta_a=(
 0, 100, apu.percent), strip_input_units=True, output_unit=apu.m ** 2)
def geom_from_eff_area(eff_area, eta_a):
    ...
    `~astropy.units.Quantity`), unit conversion is automatically performed.
    """
    return eff_area / eta_a * 100.0


@utils.ranged_quantity_input(geom_area=(
 0, None, apu.m ** 2), eff_area=(
 0, None, apu.m ** 2), strip_input_units=True, output_unit=apu.percent)
def eta_a_from_areas(geom_area, eff_area):
    ...
    `~astropy.units.Quantity`), unit conversion is automatically performed.
    """
    return eff_area / geom_area * 100.0


@utils.ranged_quantity_input(eff_area=(
 0, None, apu.m ** 2), strip_input_units=True, output_unit=apu.K / apu.Jy)
def gamma_from_eff_area(eff_area):
    ...
    `~astropy.units.Quantity`), unit conversion is automatically performed.
    """
    return eff_area / 2 / KB_VALUE * 1e-26


@utils.ranged_quantity_input(gamma=(
 0, None, apu.K / apu.Jy), strip_input_units=True, output_unit=apu.m ** 2)
def eff_area_from_gamma(gamma):
    ...
    `~astropy.units.Quantity`), unit conversion is automatically performed.
    """
    return gamma * 2 * KB_VALUE * 1e+26


@utils.ranged_quantity_input(eff_area=(
 0, None, apu.m ** 2), freq=(
 0, None, apu.Hz), strip_input_units=True, output_unit=dBi)
def gain_from_eff_area(eff_area, freq):
    ...
    `~astropy.units.Quantity`), unit conversion is automatically performed.
    """
    return 10 * np.log10(4.0 * np.pi * eff_area * (freq / C_VALUE) ** 2)


@utils.ranged_quantity_input(gain=(
 1e-30, None, dimless), freq=(
 0, None, apu.Hz), strip_input_units=True, output_unit=apu.m ** 2)
def eff_area_from_gain(gain, freq):
    ...
    `~astropy.units.Quantity`), unit conversion is automatically performed.
    """
    return gain * (C_VALUE / freq) ** 2 / 4.0 / np.pi


@utils.ranged_quantity_input(gain=(
 1e-30, None, dimless), freq=(
 0, None, apu.Hz), zi=(
 0, None, apu.Ohm), strip_input_units=True, output_unit=dB_1_m)
def antfactor_from_gain(gain, freq, zi):
    ...
    `~astropy.units.Quantity`), unit conversion is automatically performed.
    """
    return 10 * np.log10(np.sqrt(4.0 * np.pi / gain * (freq / C_VALUE) ** 2 * R0_VALUE / zi))


@utils.ranged_quantity_input(antfactor=(
 1e-30, None, 1.0 / apu.m), freq=(
 0, None, apu.Hz), zi=(
 0, None, apu.Ohm), strip_input_units=True, output_unit=dBi)
def gain_from_antfactor(antfactor, freq, zi):
    ...
    `~astropy.units.Quantity`), unit conversion is automatically performed.
    """
    return 10 * np.log10(4.0 * np.pi / antfactor ** 2 * (freq / C_VALUE) ** 2 * R0_VALUE / zi)


@utils.ranged_quantity_input(efield=(
 1e-30, None, apu.V / apu.meter), strip_input_units=True, output_unit=apu.W / apu.m ** 2)
...
@utils.ranged_quantity_input(powerflux=(
 None, None, apu.W / apu.m ** 2), strip_input_units=True, output_unit=apu.uV / apu.meter)
def efield_from_powerflux(powerflux):
    ...
    `~astropy.units.Quantity`), unit conversion is automatically performed.
    """
    return np.sqrt(powerflux * R0_VALUE) * 1000000.0


@utils.ranged_quantity_input(efield=(
 1e-30, None, apu.V / apu.meter), dist=(
 1e-30, None, apu.m), gtx=(
 1e-30, None, dimless), strip_input_units=True, output_unit=apu.W)
def ptx_from_efield(efield, dist, gtx):
    ...
    `~astropy.units.Quantity`), unit conversion is automatically performed.
    """
    return 4.0 * np.pi * dist ** 2 / gtx * efield ** 2 / R0_VALUE


@utils.ranged_quantity_input(ptx=(
 1e-30, None, apu.W), dist=(
 1e-30, None, apu.m), gtx=(
 1e-30, None, dimless), strip_input_units=True, output_unit=apu.uV / apu.meter)
def efield_from_ptx(ptx, dist, gtx):
    ...
    `~astropy.units.Quantity`), unit conversion is automatically performed.
    """
    return (ptx * gtx / 4.0 / np.pi * R0_VALUE) ** 0.5 / dist * 1000000.0


@utils.ranged_quantity_input(ptx=(
 1e-30, None, apu.W), dist=(
 1e-30, None, apu.m), gtx=(
 1e-30, None, dimless), strip_input_units=True, output_unit=apu.W / apu.m ** 2)
def powerflux_from_ptx(ptx, dist, gtx):
    ...
    `~astropy.units.Quantity`), unit conversion is automatically performed.
    """
    return gtx * ptx / 4.0 / np.pi / dist ** 2


@utils.ranged_quantity_input(powerflux=(
 1e-30, None, apu.W / apu.m ** 2), dist=(
 1e-30, None, apu.m), gtx=(
 1e-30, None, dimless), strip_input_units=True, output_unit=apu.W)
def ptx_from_powerflux(powerflux, dist, gtx):
    ...
    `~astropy.units.Quantity`), unit conversion is automatically performed.
    """
    return powerflux * 4.0 * np.pi * dist ** 2 / gtx


@utils.ranged_quantity_input(prx=(
 1e-30, None, apu.W), freq=(
 1e-30, None, apu.Hz), grx=(
 1e-30, None, dimless), strip_input_units=True, output_unit=apu.W / apu.m ** 2)
def powerflux_from_prx(prx, freq, grx):
    ...
    `~astropy.units.Quantity`), unit conversion is automatically performed.
    """
    return prx / grx * (4.0 * np.pi * freq ** 2 / C_VALUE ** 2)


@utils.ranged_quantity_input(powerflux=(
 1e-30, None, apu.W / apu.m ** 2), freq=(
 1e-30, None, apu.Hz), grx=(
 1e-30, None, dimless), strip_input_units=True, output_unit=apu.W)
def prx_from_powerflux(powerflux, freq, grx):
    ...
    `~astropy.units.Quantity`), unit conversion is automatically performed.
    """
    return powerflux * grx * (C_VALUE ** 2 / 4.0 / np.pi / freq ** 2)


@utils.ranged_quantity_input(prx_nu=(
 1e-30, None, apu.W / apu.Hz), strip_input_units=True, output_unit=apu.K)
...
@utils.ranged_quantity_input(T_A=(
 1e-30, None, apu.K), strip_input_units=True, output_unit=apu.W / apu.Hz)
...
@utils.ranged_quantity_input(powerflux_nu=(
 1e-30, None, apu.Jy), eff_area=(
 0, None, apu.m ** 2), strip_input_units=True, output_unit=apu.K)
...
@utils.ranged_quantity_input(T_A=(
 1e-30, None, apu.K), eff_area=(
 0, None, apu.m ** 2), strip_input_units=True, output_unit=apu.Jy)
def powerflux_nu_from_t_a(T_A, eff_area):
    ...
    `~astropy.units.Quantity`), unit conversion is automatically performed.
    """
    return 1e+26 * T_A / eff_area * 2 * KB_VALUE


def _free_space_loss(d, f):
    return (C_VALUE / 4.0 / np.pi / f / d) ** 2


@utils.ranged_quantity_input(dist=(
 1e-30, None, apu.m), freq=(
 1e-30, None, apu.Hz), strip_input_units=True, output_unit=dB)
def free_space_loss(dist, freq):
    ...
    `~astropy.units.Quantity`), unit conversion is automatically performed.
    """
    return 10.0 * np.log10(_free_space_loss(freq, dist))


@utils.ranged_quantity_input(ptx=(
 1e-30, None, apu.W), gtx=(
 1e-30, None, dimless), grx=(
 1e-30, None, dimless), dist=(
 1e-30, None, apu.m), freq=(
 1e-30, None, apu.Hz), strip_input_units=True, output_unit=apu.W)
