...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pytextgrid/PraatTextGrid.py
# Compiled at: 2018-12-19 13:27:22
# Size of source mod 2**32: 12480 bytes
import codecs
...
class Point:
    ...
    def __init__(self, fd=None, time=None, mark=''):
        ...
        self._time = time
        if fd:
            assert re.match('\\s*points\\s*\\[\\d+\\]:\\s*', fd.readline())
...
class Interval:
    ...
    def loadfromfd(cls, fd):
        fdline = fd.readline()
        assert re.match('\\s*intervals\\s*\\[\\d+\\]:?\\s*', fdline)
...
class Tier:
    ...
    def __init__(self, xmin=0.0, xmax=0.0, tierclass='IntervalTier', name=None):
        self._intervals = []
        self._xmin = xmin
        self._xmax = xmax
        self._name = name
    ...
    def loadfromfd(cls, fd):
        ...
        """
        assert re.match('\\s*item\\s*\\[\\d+\\]:\\s*', fd.readline())
    ...
    @classmethod
    def initfromlist(cls, l, xmin=0.0, xmax=0.0, tierclass='IntervalTier', name=''):
        l.sort()
...
class PraatTextGrid:
    ...
    def loadfromfile(cls, fname):
        ...
        assert f.readline().strip() == 'tiers? <exists>'
        nbtiers = getintval(f.readline(), 'size')
        assert re.match('item \\[\\]:\\s*', f.readline())
