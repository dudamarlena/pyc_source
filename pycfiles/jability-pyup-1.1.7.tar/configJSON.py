...
# [GCC 8.4.0]
# Embedded file name: /Users/fabrom/workspace/jability-python-package/jabilitypyup/configJSON.py
...
class ConfigJSON:
    ...
    def __init__(self, filepath='foo.conf'):
        ...
        self.confdict = None
        log.debug('init configJSON for file %s' % self.filepath)
        return

    def exists(self):
        u""" Teste l'existence du fichier de configuration.
        ...
        if not self.filepath == None:
            if os.path.exists(self.filepath) and os.path.isfile(self.filepath):
                return True
        log.debug('%s not found' % self.filepath)
        return False
    ...
    def load(self):
        ...
        """
        if self.exists():
            log.debug('loading existing configuration file %s...' % self.filepath)
            fp = open(self.filepath, 'r')
            self.confdict = json.load(fp)
            fp.close()
            log.debug('configuration file %s loaded.' % self.filepath)
            ...
            if not self.confdict['__configJSON__'].has_key('__VERSION__'):
                ...
                self.confdict['__configJSON__']['this']['__VERSION__'] = '0.1'
            self.version = self.confdict['__configJSON__']['this']['__VERSION__']
        else:
            log.error('%s not found' % self.filepath)
            self.confdict = None
            self.version = None
    ...
    def update(self, updjsonfile='confupd.json'):
        u"""Mise à jour de la configuration courante à partir du fichier de
        ...
        if os.path.exists(fullupdfile):
            try:
                ...
                fp.close()
                log.debug('update configuration file %s loaded.' % updjsonfile)
            except Exception:
                log.error('Cannot read update configuration file %s' % fullupdfile)
                log.exception('')
                return
            ...
            try:
                ...
                log.info('configuration updated (%s -> %s)' % (self.version, version))
                self.version = version
            except Exception:
                log.error('Error during configuration update')
                log.exception('')

    def save(self, confdict=None, filepath=None):
        u""" Sauvegarde de la configuration dans un fichier.
        ...
        log.debug('saving configuration file %s...' % self.filepath)
        fp = open(filepath, 'w+')
        json.dump(confdict, fp, indent=4, sort_keys=True)
        fp.close()
        log.debug('configuration file %s saved.' % self.filepath)
        return