...
# [GCC 8.4.0]
# Embedded file name: /Users/fabrom/workspace/jability-python-package/jabilitypyup/amqp/middleware.py
...
class AMQP_Connection:
    ...
    def open(self):
        try:
            self.conn = amqp.Connection(host=self.cfg['host']['name'], userid=self.cfg['host']['user'], password=self.cfg['host']['password'], virtual_host=self.cfg['host']['virtual_host'], insist=self.cfg['host']['insist'])
            self.chan = self.conn.channel()
            self.chan.exchange_declare(exchange=self.cfg['exchange']['name'], type=self.cfg['exchange']['type'], durable=self.cfg['exchange']['durable'], auto_delete=self.cfg['exchange']['auto_delete'])
            for qname in self.cfg['queue']['name']:
                self.chan.queue_declare(qname, durable=self.cfg['queue']['durable'], auto_delete=self.cfg['queue']['auto_delete'], exclusive=self.cfg['queue']['exclusive'])
                self.chan.queue_bind(queue=qname, exchange=self.cfg['exchange']['name'])

        except Exception as e:
            logging.error('AMQP_Connection : Error during open() : %s' % e)

        logging.debug("AMQP_Connection : open exchange '%s'" % self.cfg['exchange']['name'])
    ...
    def close(self):
        try:
            ...
            logging.error('AMQP_Connection : Error during close() : %s' % e)

        logging.debug("AMQP_Connection : close exchange '%s'" % self.cfg['exchange']['name'])
...
class AMQP_Publisher(AMQP_Connection):
    ...
    def send_message(self, message):
        try:
            ...
            self.chan.basic_publish(msg, exchange=self.cfg['exchange']['name'])
        except Exception as e:
            logging.error('AMQP_Publisher : Error during send_message() : %s' % e)

        logging.debug("AMQP_Publisher : send message on exchange '%s'" % self.cfg['exchange']['name'])
...
class AMQP_Consumer(AMQP_Connection):
    ...
    def callback(self, msg):
        if self.callback_function:
            ...
            logging.error('AMQP_Consumer : message receive but no call back declared !')

    def consume_forever(self):
        self.chan.basic_consume(self.cfg['consumer']['queue'], callback=self.callback, no_ack=self.cfg['consumer']['no_ack'], nowait=self.cfg['consumer']['nowait'], consumer_tag=self.cfg['consumer']['tag'])
...
class AMQP_LogHandler(logging.Handler):
    ...
    def emit(self, record):
        message = {'name': record.name, 'machine': self.machine, 
    ...
    @staticmethod
    def getStreamAndBroadcastLogHandler(namespace, amqp_config, level=logging.DEBUG):
        """Factory function for a logging.StreamHandler instance connected to your namespace."""
        logging.basicConfig(datefmt='%Y%m%d %H%M%S', format='%(asctime)s - %(levelname)s - %(name)s - %(filename)s %(funcName)s %(lineno)d - %(message)s')
...
if __name__ == '__main__':
    amqp_config = {'exchange': {'name': 'notify.log.add', 
                    'type': 'fanout', 
                    'durable': True, 
                    'auto_delete': False, 
                    'delivery_mode': 2}, 
