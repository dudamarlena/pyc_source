...
# [GCC 8.4.0]
# Embedded file name: /Users/fabrom/workspace/jability-python-package/jabilitypyup/filescollector.py
# Compiled at: 2015-04-21 06:23:02
...
class FilesCollector:
    ...
    def setFileFilter(self, filefilter='^.*$'):
        u"""initialisation du filtre de sélection des fichiers
    ...
    def setMinimumFileLastChangeAge(self, mins=0):
        u"""Si l'âge d'un fichier est inférieur au délai exprimé en minutes,
            il est ignoré.
        """
        self.minimumFileAge = mins * 60

    def run(self, returnAbspath=False):
        u"""Collecte des fichiers à traiter.
        ...
        try:
            ...
            if not os.access(normpath, os.F_OK | os.R_OK):
                log.error("'%s' path not exists or not readable !" % normpath)
                return self.files
            if not os.path.exists(normpath):
                log.error("'%s' path not exists !" % normpath)
                return self.files
            if not os.path.isdir(normpath):
                log.error("'%s' is not a directory !" % normpath)
                return self.files
            flist = os.listdir(normpath)
            for tfile in flist:
                absfile = os.path.join(normpath, tfile)
                if not os.access(absfile, os.F_OK | os.R_OK):
                    log.error("'%s' file not exists or not readable !" % absfile)
                    continue
                if not os.path.isfile(absfile):
                    continue
                if self.reFileFilter.match(tfile) is None:
                    continue
            ...
            log.error("unknow error during collect from path '%s'" % self.sourcedir)
            log.exception('')
