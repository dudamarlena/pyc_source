...
# Embedded file name: build/bdist.linux-x86_64/egg/powercmd/match_string.py
# Compiled at: 2020-03-15 13:15:50
...
class TextMatchStrategy:
    ...
    def __init__(self, name: str, matcher: Callable[([str, str], bool)]):
        self.name = name
        self.matcher = matcher

    def __call__(self, text: str, pattern: str) -> bool:
    ...
    @staticmethod
    def _matches_words(text: str, words: Sequence[str]) -> bool:
        """
        ...
        for word in words:
            ...
            if not common_prefix:
                ...
                for prefix in TextMatchStrategy._prefixes_of(common_prefix):
                    if TextMatchStrategy._matches_words(text[len(prefix):], words[1:]):
                        return True

        return False

    @staticmethod
    def snake_case_matches(short: str, full: str) -> bool:
        """
    ...
    @staticmethod
    def fuzzy_matches(short: str, full: str) -> bool:
        """
...
TextMatchStrategy.Exact = TextMatchStrategy('exact', lambda a, b: a == b)
TextMatchStrategy.Prefix = TextMatchStrategy('prefix', lambda short, full: full.startswith(short))
TextMatchStrategy.SnakeCase = TextMatchStrategy('snake case', TextMatchStrategy.snake_case_matches)
TextMatchStrategy.Fuzzy = TextMatchStrategy('fuzzy', TextMatchStrategy.fuzzy_matches)

def _match_string(text: str, possible: List[str], match_strategies: Sequence[Tuple[(str, Callable[([str, str], bool)])]], verbose: bool) -> List[str]:
    """
