...
# [GCC 8.4.0]
# Embedded file name: /home/cameron/Dev/kanban-dev/django-connectwise/djconnectwise/sync.py
# Compiled at: 2020-04-17 19:39:02
# Size of source mod 2**32: 83337 bytes
...
from djconnectwise.utils import get_hash, get_filename_extension, generate_thumbnail, generate_filename, remove_thumbnail
...
class SyncResults:
    __doc__ = 'Track results of a sync job.'
...
class Synchronizer:
    ...
    def set_relations(self, instance, json_data):
        for json_field, value in self.related_meta.items():
            ...
            self._assign_relation(instance, json_data, json_field, model_class, field_name)
    ...
    def _assign_null_relation(instance, model_field):
        ...
        try:
            ...
            raise InvalidObjectException("Unable to set field {} on {} to null, as it's required.".format(model_field, instance))

    def _assign_relation(self, instance, json_data, json_field, model_class, model_field):
        """
        ...
        try:
            ...
            logger.warning('Failed to find {} {} for {} {}.'.format(json_field, uid, type(instance), instance.id))
            self._assign_null_relation(instance, model_field)

    def _instance_ids(self, filter_params=None):
        if not filter_params:
            ids = self.model_class.objects.all().order_by(self.lookup_key).values_list((self.lookup_key),
              flat=True)
        else:
            ids = self.model_class.objects.filter(filter_params).order_by(self.lookup_key).values_list((self.lookup_key),
              flat=True)
        return set(ids)
    ...
    def get(self, results, conditions=None):
        ...
        page = 1
        while 1:
            logger.info('Fetching {} records, batch {}'.format(self.model_class, page))
            page_conditions = conditions or self.api_conditions
            page_records = self.get_page(page=page,
    ...
    def fetch_delete_by_id(self, instance_id, pre_delete_callback=None, pre_delete_args=None, post_delete_callback=None):
        try:
            self.get_single(instance_id)
            logger.warning('ConnectWise API returned {} {} even though it was expected to be deleted.'.format(self.model_class.__name__, instance_id))
            ...
            logger.info('Deleted {} {} (if it existed).'.format(self.model_class.__name__, instance_id))
    ...
    def update_or_create_instance(self, api_instance):
        ...
        try:
            ...
            msg = 'IntegrityError while attempting to create {}. Error: {}'.format(self.model_class, e)
            logger.error(msg)
            raise InvalidObjectException(msg)

        logger.info('{}: {} {}'.format('Created' if created else 'Updated', self.model_class.__name__, instance))
        return (
         instance, created)
    ...
    def prune_stale_records(self, initial_ids, synced_ids):
        ...
        if stale_ids:
            ...
            if self.pre_delete_callback:
                pre_delete_result = (self.pre_delete_callback)(*self.pre_delete_args)
            logger.info('Removing {} stale records for model: {}'.format(len(stale_ids), self.model_class))
            delete_qset.delete()
    ...
    def get_sync_job_qset(self):
        return models.SyncJob.objects.filter(entity_name=(self.model_class.__name__))
    ...
    def sync(self):
        ...
        if sync_job_qset.exists():
            if not self.full:
                ...
                self.api_conditions.append('lastUpdated>[{0}]'.format(last_sync_job_time))
        results = SyncResults()
        results = self.get(results)
        if self.full:
            initial_ids = self._instance_ids()
            results.deleted_count = self.prune_stale_records(initial_ids, results.synced_ids)
        return (
    ...
    def sync_children(self, *args):
        for synchronizer, filter_params in args:
            created_count, updated_count, deleted_count = synchronizer.callback_sync(filter_params)
            msg = '{} Child Sync - Created: {}, Updated: {}, Deleted: {}'.format(synchronizer.model_class.__name__, created_count, updated_count, deleted_count)
            logger.info(msg)
...
class ServiceNoteSynchronizer(CallbackPartialSyncMixin, Synchronizer):
    ...
    related_meta = {'member': (models.Member, 'member')}

    def _assign_field_data(self, instance, json_data):
        instance.id = json_data['id']
        instance.text = json_data.get('text')
        instance.detail_description_flag = json_data.get('detailDescriptionFlag')
        instance.internal_analysis_flag = json_data.get('internalAnalysisFlag')
        instance.resolution_flag = json_data.get('resolutionFlag')
        ...
        try:
            ...
            raise InvalidObjectException('Service note {} has no ticketId key to find its target- skipping.'.format(instance.id))
        except ObjectDoesNotExist as e:
            raise InvalidObjectException('Service note {} has a ticketId that does not exist. ObjectDoesNotExist Exception: '.format(instance.id, e))
    ...
    def get_page(self, *args, **kwargs):
        ...
        if conditions:
            try:
                ...
                kwargs['conditions'] = [conditions[1]] if len(conditions) > 1 else []
    ...
    def create_new_note(self, target, **kwargs):
        ...
        if isinstance(target, models.Ticket):
            ...
            raise ValueError('Invalid target type for note creation: {}.'.format(str(target.__class__)))
...
class OpportunityNoteSynchronizer(Synchronizer):
    ...
    def _assign_field_data(self, instance, json_data):
        ...
        try:
            ...
            raise InvalidObjectException('Opportunity note {} has a opportunityId that does not exist. ObjectDoesNotExist Exception: {}'.format(instance.id, e))
    ...
    def get_page(self, *args, **kwargs):
        records = []
        opportunity_qs = models.Opportunity.objects.all().order_by(self.lookup_key)
...
class BoardSynchronizer(Synchronizer):
    ...
    related_meta = {'workRole':(
      models.WorkRole, 'work_role'), 
     'workType':(
...
class BoardChildSynchronizer(Synchronizer):
    ...
    def get_page(self, *args, **kwargs):
        records = []
        board_qs = models.ConnectWiseBoard.objects.all().order_by(self.lookup_key)
...
class BoardStatusSynchronizer(BoardChildSynchronizer):
    ...
    def _assign_field_data(self, instance, json_data):
        instance = super(BoardStatusSynchronizer, self)._assign_field_data(instance, json_data)
...
class TeamSynchronizer(BoardChildSynchronizer):
    ...
    def _assign_field_data(self, instance, json_data):
        instance = super(TeamSynchronizer, self)._assign_field_data(instance, json_data)
        members = []
        if json_data.get('members'):
            members = list(models.Member.objects.filter(id__in=(json_data['members'])))
        instance.save()
...
class CompanySynchronizer(Synchronizer):
    ...
    def _assign_field_data(self, company, company_json):
        ...
        if status_json:
            try:
                ...
                logger.warning('Failed to find CompanyStatus: {}'.format(status_json['id']))
            ...
            if calendar_id:
                try:
                    ...
                    logger.warning('Failed to find Calendar: {}'.format(calendar_id))
            ...
            if type_json:
                try:
                    company_type = models.CompanyType.objects.get(pk=(type_json['id']))
                    company.company_type = company_type
                except models.CompanyType.DoesNotExist:
                    logger.warning('Failed to find CompanyType: {}'.format(type_json['id']))

            territory_id = company_json.get('territoryId')
            if territory_id:
                try:
                    company.territory = models.Territory.objects.get(pk=territory_id)
                except models.Territory.DoesNotExist:
                    logger.warning('Failed to find Territory: {}'.format(territory_id))

            else:
                logger.warning('No Territory ID recieved in request for Company: {}'.format(company.id))
        company.save()
        return company
    ...
    def fetch_delete_by_id(self, company_id, pre_delete_callback=None, pre_delete_args=None, post_delete_callback=None):
        pre_delete_result = None
        try:
            if pre_delete_callback:
                pre_delete_result = pre_delete_callback(*pre_delete_args)
...
class CompanyStatusSynchronizer(Synchronizer):
    ...
    def _assign_field_data(self, instance, json_data):
        ...
        instance.cancel_open_tracks_flag = json_data.get('cancelOpenTracksFlag')
...
class MyCompanyOtherSynchronizer(Synchronizer):
    ...
    related_meta = {'defaultCalendar': (models.Calendar, 'default_calendar')}
...
class ActivityStatusSynchronizer(Synchronizer):
    ...
    def _assign_field_data(self, instance, json_data):
        ...
        instance.spawn_followup_flag = json_data.get('spawnFollowupFlag', False)
...
class ActivitySynchronizer(Synchronizer):
    ...
    related_meta = {'opportunity':(
      models.Opportunity, 'opportunity'), 
     'ticket':(
      models.Ticket, 'ticket'), 
     'assignTo':(
      models.Member, 'assign_to'), 
     'status':(
      models.ActivityStatus, 'status'), 
     'type':(
    ...
    def __init__(self, *args, **kwargs):
        (super().__init__)(*args, **kwargs)
        self.api_conditions = [
         'status/id in ({})'.format(','.join([str(i.id) for i in models.ActivityStatus.objects.filter(closed_flag=False)]))]
...
class ScheduleEntriesSynchronizer(BatchConditionMixin, Synchronizer):
    ...
    related_meta = {'where':(
      models.Location, 'where'), 
     'status':(
      models.ScheduleStatus, 'status'), 
     'type':(
      models.ScheduleType, 'schedule_type'), 
     'member':(
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.api_conditions = [
         ...
         'doneFlag=false']
        ticket_ids = set(models.Ticket.objects.order_by(self.lookup_key).values_list('id', flat=True))
        opportunity_ids = set(models.Opportunity.objects.order_by(self.lookup_key).values_list('id', flat=True))
    ...
    def get_optimal_size(self, condition_list):
        ...
        if object_id_size:
            ...
            return super().get_optimal_size(condition_list)

    def get_batch_condition(self, conditions):
        return 'objectId in ({})'.format(','.join([str(i) for i in conditions]))
    ...
    def _assign_field_data(self, instance, json_data):
        ...
        if 'member' not in json_data:
            raise InvalidObjectException('Schedule entry {} has no member field- skipping.'.format(instance.id))
        try:
            member_id = json_data['member']['id']
            models.Member.objects.get(pk=member_id)
        except ObjectDoesNotExist:
            raise InvalidObjectException('Schedule entry {} can not find member: {}- skipping.'.format(instance.id, member_id))
        ...
        if date_start:
            ...
            if instance.date_start.time() == datetime.time(0, 0):
                if json_data.get('hours') == 0.0:
                    instance.date_start = parse(date_start) - timezone.localtime().utcoffset()
                    if date_end:
                        instance.date_end = parse(date_end) - timezone.localtime().utcoffset()
            if date_end:
        ...
        try:
            ...
            raise InvalidObjectException('Schedule entry {} has no objectId key to find its target- skipping.'.format(instance.id))
        ...
        if json_data['type']['identifier'] == 'S':
            try:
                ...
                logger.warning('Ticket not found for {}.'.format(instance.id) + ' ObjectDoesNotExist Exception: {}.'.format(e))

        else:
            if json_data['type']['identifier'] == 'C':
                try:
                    related_activity = activity_class.objects.get(pk=uid)
                    setattr(instance, 'activity_object', related_activity)
                except ObjectDoesNotExist as e:
                    logger.warning('Activity not found for {}.'.format(instance.id) + ' ObjectDoesNotExist Exception: {}.'.format(e))

            else:
                raise InvalidObjectException('Invalid ScheduleEntry type for schedule entry {}- skipping.'.format(instance.id))
            return instance
    ...
    def create_new_entry(self, target, **kwargs):
        ...
        schedule_type = models.ScheduleType.objects.get(identifier=(target.SCHEDULE_ENTRY_TYPE))
...
class TimeEntrySynchronizer(BatchConditionMixin, CallbackPartialSyncMixin, Synchronizer):
    client_class = api.TimeAPIClient
    model_class = models.TimeEntry
    batch_condition_list = []
    related_meta = {'company':(
      models.Company, 'company'), 
     'member':(
      models.Member, 'member')}
    ...
    def get_batch_condition(self, conditions):
        return 'chargeToId in ({})'.format(','.join([str(i) for i in conditions]))
    ...
    def _assign_field_data(self, instance, json_data):
        ...
        try:
            ...
            raise InvalidObjectException('Time Entry {} has no chargeToId key to find its target- skipping.'.format(instance.id))
    ...
    def create_new_entry(self, target, **kwargs):
        ...
        if isinstance(target, models.Ticket):
            ...
            raise InvalidObjectException('Invalid target type for TimeEntry creation: ' + str(target.__class__) + '.')
...
class ProjectPhaseSynchronizer(Synchronizer):
    ...
    related_meta = {'board': (models.ConnectWiseBoard, 'board')}
    ...
    def _assign_field_data(self, instance, json_data):
        ...
        try:
            ...
            raise InvalidObjectException('Project phase {} has no projectId key to find its target- skipping.'.format(instance.id))
        except ObjectDoesNotExist as e:
            raise InvalidObjectException('Project phase {} has a projectId that does not exist. ObjectDoesNotExist Exception: '.format(instance.id, e))
...
class ProjectSynchronizer(Synchronizer):
    ...
    related_meta = {'status':(
     ...
     'manager':(
      models.Member, 'manager'), 
     'company':(
      models.Company, 'company'), 
     'type':(
      models.ProjectType, 'type')}

    def __init__(self, *args, **kwargs):
        (super().__init__)(*args, **kwargs)
        self.api_conditions = [
         'status/id in ({})'.format(','.join(str(i.id) for i in models.ProjectStatus.objects.filter(closed_flag=False)))]
...
class MemberSynchronizer(Synchronizer):
    ...
    def _assign_field_data(self, instance, json_data):
        ...
        if instance.office_email is None:
            raise InvalidObjectException('Office email of user {} is null- skipping.'.format(instance.identifier))
    ...
    def _save_avatar(self, member, avatar, attachment_filename):
        ...
        if process_thumbnails or not default_storage.exists(current_avatar_filename):
            member.avatar = filename
            avatar_file = None
            for size in thumbnail_size:
                current_filename = generate_filename(thumbnail_size[size], current_avatar, extension)
                filename = '{}.{}'.format(get_hash(avatar), extension)
                try:
                    avatar_file, filename = generate_thumbnail(avatar, thumbnail_size[size], extension, filename)
                except Exception as e:
                    logger.warning('Error saving member avatar. {}'.format(e))

                if avatar_file and filename:
                    default_storage.delete(current_filename)
                    logger.info("Saving member '{}' avatar to {}.".format(member.identifier, filename))
    ...
    def update_or_create_instance(self, api_instance):
        try:
            ...
            raise InvalidObjectException('Failed to update member: {}'.format(e))
            ...
            if instance.avatar:
                if not photo_id:
                    try:
                        ...
                        msg = 'Error when removing thumbnail for {}. NoCredentialsError Exception: {}.'.format(instance.username, e)
                        logger.warning(msg)

                    instance.avatar = None
                    instance.save()
            if photo_id:
                if self.full or not self.last_sync_job_time or not bool(instance.avatar) or member_stale or created:
                    logger.info('Fetching avatar for member {}.'.format(username))
                    attachment_filename, avatar = self.client.get_member_image_by_photo_id(photo_id, username)
                    if attachment_filename:
                        if avatar:
                            try:
                                self._save_avatar(instance, avatar, attachment_filename)
                            except NoCredentialsError as e:
                                msg = 'Error when saving avatar for {}. NoCredentialsError Exception: {}.'.format(username, e)
                                logger.warning(msg)
...
class TicketSynchronizerMixin:
    ...
    related_meta = {'team':(
      models.Team, 'team'), 
     'board':(
      models.ConnectWiseBoard, 'board'), 
     'company':(
      models.Company, 'company'), 
     'priority':(
      models.TicketPriority, 'priority'), 
     'project':(
      models.Project, 'project'), 
     'phase':(
      models.ProjectPhase, 'phase'), 
     'serviceLocation':(
      models.Location, 'location'), 
     'status':(
      models.BoardStatus, 'status'), 
     'owner':(
      models.Member, 'owner'), 
     'sla':(
      models.Sla, 'sla'), 
     'type':(
      models.Type, 'type'), 
     'subType':(
      models.SubType, 'sub_type'), 
     'item':(
      models.Item, 'sub_type_item'), 
     'agreement':(
      models.Agreement, 'agreement')}
    ...
    def __init__(self, *args, **kwargs):
        ...
        filtered_statuses = models.BoardStatus.available_objects.filter(closed_status=False).order_by(self.lookup_key)
        if board_names:
            boards = [board.strip() for board in board_names.split(',')]
            boards_exist = models.ConnectWiseBoard.available_objects.filter(name__in=boards).exists()
            if boards_exist:
                filtered_statuses = filtered_statuses.filter(board__name__in=boards)
        if filtered_statuses:
            self.batch_condition_list = list(filtered_statuses.values_list('id', flat=True))

    def get_batch_condition(self, conditions):
        return 'status/id in ({})'.format(','.join([str(i) for i in conditions]))
    ...
    def _instance_ids(self, filter_params=None):
        ...
        if not filter_params:
            ...
            ids = tickets_qset.filter(filter_params).values_list((self.lookup_key),
              flat=True)
        return set(ids)
...
class ServiceTicketSynchronizer(TicketSynchronizerMixin, BatchConditionMixin, Synchronizer):
    client_class = api.ServiceAPIClient
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not sync_child_tickets:
            if sync_child_tickets is not None:
    ...
    def _assign_field_data(self, instance, json_data):
        ...
        instance.automatic_email_cc_flag = json_data.get('automaticEmailCcFlag', False)
        instance.automatic_email_contact_flag = json_data.get('automaticEmailContactFlag', False)
        instance.automatic_email_resource_flag = json_data.get('automaticEmailResourceFlag', False)
        instance.automatic_email_cc = json_data.get('automaticEmailCc')
    ...
    def filter_by_record_type(self):
        return self.model_class.objects.filter(record_type=(models.Ticket.SERVICE_TICKET))


class ProjectTicketSynchronizer(TicketSynchronizerMixin, BatchConditionMixin, Synchronizer):
    client_class = api.ProjectAPIClient
    ...
    def filter_by_record_type(self):
        ...
        return self.model_class.objects.filter(record_type__in=project_record_types)
...
class CalendarSynchronizer(Synchronizer):
    ...
    def _assign_field_data(self, instance, json_data):
        ...
        self._assign_relation(instance, json_data, 'holidayList', models.HolidayList, 'holiday_list')
        return instance
...
class SLASynchronizer(Synchronizer):
    ...
    related_meta = {'customCalendar': (models.Calendar, 'calendar')}
...
class OpportunitySynchronizer(Synchronizer):
    ...
    related_meta = {'type':(
      models.OpportunityType, 'opportunity_type'), 
     'stage':(
      models.OpportunityStage, 'stage'), 
     'status':(
      models.OpportunityStatus, 'status'), 
     'probability':(
      models.SalesProbability, 'probability'), 
     'primarySalesRep':(
      models.Member, 'primary_sales_rep'), 
     'secondarySalesRep':(
      models.Member, 'secondary_sales_rep'), 
     'company':(
      models.Company, 'company'), 
     'closedBy':(
      models.Member, 'closed_by')}

    def __init__(self, *args, **kwargs):
        (super().__init__)(*args, **kwargs)
        open_statuses = list(models.OpportunityStatus.objects.filter(closed_flag=False).order_by(self.lookup_key).values_list('id',
          flat=True))
        if open_statuses:
            self.api_conditions.append('status/id in ({})'.format(','.join([str(i) for i in open_statuses])))

    def _update_or_create_child(self, model_class, json_data):
        child_name = json_data['name']
        child, created = model_class.objects.get_or_create(id=(json_data['id']),
    ...
    def _assign_field_data(self, instance, json_data):
        ...
        if priority:
            instance.priority = self._update_or_create_child(models.OpportunityPriority, priority)
        self.set_relations(instance, json_data)
        return instance
