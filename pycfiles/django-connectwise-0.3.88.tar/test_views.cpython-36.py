...
# [GCC 8.4.0]
# Embedded file name: /home/cameron/Dev/kanban-dev/django-connectwise/djconnectwise/tests/test_views.py
...
class BaseTestCallBackView(TestCase):
    ...
    def _test_added(self, callback_type, entity):
        response = self.post_data(callback_type, views.CALLBACK_ADDED, entity['id'], entity)
    ...
    def _test_update(self, callback_type, entity):
        response = self.post_data(callback_type, views.CALLBACK_UPDATED, entity['id'], entity)
        instances = list(self.MODEL_CLASS.objects.all())
        instance = instances[0]
        self.assert_fields(instance, entity)
        self.assertEqual(instance.id, entity['id'])
    ...
    def _test_delete(self, callback_type, entity_id, manager='objects'):
        response = self.post_data(callback_type, views.CALLBACK_DELETED, entity_id)
...
class TestTicketCallBackView(BaseTestCallBackView):
    ...
    def test_delete(self):
        ...
        self._test_delete('ticket', fixtures.API_SERVICE_TICKET['id'])
...
class TestProjectCallBackView(BaseTestCallBackView):
    ...
    def test_delete(self):
        ...
        fixture_utils.init_projects()
        self.assertEqual(Project.objects.count(), 1)
        mocks.project_api_get_project_call(None,
          raised=(api.ConnectWiseRecordNotFoundError))
        self._test_delete('project', fixtures.API_PROJECT['id'])
