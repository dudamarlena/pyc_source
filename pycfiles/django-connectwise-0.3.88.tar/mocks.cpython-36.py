...
# [GCC 8.4.0]
# Embedded file name: /home/cameron/Dev/kanban-dev/django-connectwise/djconnectwise/tests/mocks.py
# Compiled at: 2020-02-20 12:46:44
# Size of source mod 2**32: 13981 bytes
import os
...
def _project_api_tickets_call(page=1, page_size=25, conditions=[]):
    ...
    conditions.append('lastUpdated>' + timezone.localtime(value=test_datetime).isoformat())
    if page == 1:
        return_value = [
         fixtures.API_PROJECT_TICKET]
    return return_value


def project_api_tickets_call():
    method_name = 'djconnectwise.api.TicketAPIMixin.get_tickets'
    mock_call, _patch = create_mock_call(method_name,
      None,
...
def system_api_get_member_image_by_photo_id_call(return_value):
    method_name = 'djconnectwise.api.SystemAPIClient.get_member_image_by_photo_id'
    return create_mock_call(method_name, return_value)
...
def get_raw(url, data, content_type='application/octet-stream', headers=None, status=200):
...
def get_member_avatar():
    ...
    cw_member_image_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), CW_MEMBER_IMAGE_FILENAME)
