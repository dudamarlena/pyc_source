...
# [GCC 8.4.0]
# Embedded file name: /home/cameron/Dev/kanban-dev/django-connectwise/djconnectwise/tests/test_sync.py
# Compiled at: 2020-03-09 17:36:53
# Size of source mod 2**32: 73987 bytes
...
class TestCompanySynchronizer(TestCase, SynchronizerTestMixin):
    ...
    def setUp(self):
        fixture_utils.init_territories()
        mocks.company_api_get_company_statuses_call(fixtures.API_COMPANY_STATUS_LIST)
...
class TestCompanyStatusSynchronizer(TestCase, SynchronizerTestMixin):
    ...
    def _assert_fields(self, instance, json_data):
        ...
        self.assertEqual(instance.dissalow_saving_flag, json_data['disallowSavingFlag'])
        self.assertEqual(instance.notification_message, json_data['notificationMessage'])
        self.assertEqual(instance.custom_note_flag, json_data['customNoteFlag'])
        self.assertEqual(instance.cancel_open_tracks_flag, json_data['cancelOpenTracksFlag'])
...
class TestTimeEntrySynchronizer(TestCase, SynchronizerTestMixin):
    ...
    def test_sync_update(self):
        ...
        self._sync(new_json_list)
        changed = self.model_class.objects.get(id=instance_id)
        self.assertNotEqual(original.time_start, start)
        self._assert_fields(changed, new_json)
...
class TestScheduleEntriesSynchronizer(TestCase, SynchronizerTestMixin):
    ...
    def _assert_fields(self, instance, json_data):
        ...
        self.assertEqual(instance.date_start, parse(json_data['dateStart']))
        self.assertEqual(instance.date_end, parse(json_data['dateEnd']))
...
class TestProjectPhaseSynchronizer(TestCase, SynchronizerTestMixin):
    ...
    def _assert_fields(self, instance, json_data):
        ...
        self.assertEqual(instance.scheduled_start, parse(json_data['scheduledStart']).date())
        self.assertEqual(instance.scheduled_end, parse(json_data['scheduledEnd']).date())
        self.assertEqual(instance.actual_start, parse(json_data['actualStart']).date())
        self.assertEqual(instance.actual_end, parse(json_data['actualEnd']).date())
...
class TestProjectSynchronizer(TestCase, SynchronizerTestMixin):
    ...
    def _assert_fields(self, instance, json_data):
        ...
        self.assertAlmostEqual(float(instance.actual_hours), json_data['actualHours'])
        self.assertAlmostEqual(float(instance.budget_hours), json_data['budgetHours'])
        self.assertAlmostEqual(float(instance.scheduled_hours), json_data['scheduledHours'])
        self.assertEqual(instance.actual_start, parse(json_data['actualStart']).date())
        self.assertEqual(instance.actual_end, parse(json_data['actualEnd']).date())
        self.assertEqual(instance.estimated_start, parse(json_data['estimatedStart']).date())
        self.assertEqual(instance.estimated_end, parse(json_data['estimatedEnd']).date())
...
class TestPrioritySynchronizer(TestCase, SynchronizerTestMixin):
    ...
    def _assert_fields(self, priority, api_priority):
        if not priority.name == api_priority['name']:
            raise AssertionError
        elif not priority.id == api_priority['id']:
            raise AssertionError
        else:
            ...
            elif not priority.sort is None:
                raise AssertionError

    def setUp(self):
        self.synchronizer = sync.PrioritySynchronizer()
        self.valid_prio_colors = list(TicketPriority.DEFAULT_COLORS.values()) + [
...
class TestBoardSynchronizer(TestCase, SynchronizerTestMixin):
    ...
    def _assert_fields(self, instance, json_data):
        ...
        self.assertEqual(instance.work_role.name, json_data['workRole']['name'])
        self.assertEqual(instance.work_type.name, json_data['workType']['name'])
...
class TestBoardStatusSynchronizer(TestCase, SynchronizerTestMixin):
    ...
    def _assert_fields(self, instance, json_data):
        ...
        self.assertEqual(instance.display_on_board, json_data['displayOnBoard'])
...
class TestServiceNoteSynchronizer(TestCase, SynchronizerTestMixin):
    ...
    def _assert_fields(self, instance, json_data):
        ...
        self.assertEqual(instance.detail_description_flag, json_data['detailDescriptionFlag'])
        self.assertEqual(instance.internal_analysis_flag, json_data['internalAnalysisFlag'])
        self.assertEqual(instance.resolution_flag, json_data['resolutionFlag'])
        self.assertEqual(instance.member.identifier, json_data['member']['identifier'])
        self.assertEqual(instance.date_created, parse(json_data['dateCreated']))
    ...
    def test_sync_update(self):
        ...
        self._sync(new_json_list)
        changed = self.model_class.objects.get(id=instance_id)
        self.assertNotEqual(original.detail_description_flag, flag)
        self._assert_fields(changed, new_json)
...
class TestOpportunityNoteSynchronizer(TestCase, SynchronizerTestMixin):
    ...
    def _assert_fields(self, instance, json_data):
        self.assertEqual(instance.id, json_data['id'])
        self.assertEqual(instance.text, json_data['text'])
        self.assertEqual(instance.opportunity.id, json_data['opportunityId'])
...
class TestMemberSynchronization(TransactionTestCase):
    ...
    @override_settings(DEFAULT_FILE_STORAGE='storages.backends.s3boto3.S3Boto3Storage', AWS_STORAGE_BUCKET_NAME='somebucket')
...
class TestOpportunitySynchronizer(TestCase, SynchronizerTestMixin):
    ...
    def setUp(self):
        super().setUp()
        self.synchronizer = self.synchronizer_class()
        mocks.sales_api_get_opportunity_types_call(fixtures.API_SALES_OPPORTUNITY_TYPES)
        fixture_utils.init_activities()
        fixture_utils.init_opportunity_statuses()
        fixture_utils.init_opportunity_types()
    ...
    def _assert_fields(self, instance, json_data):
        self.assertEqual(instance.id, json_data['id'])
        self.assertEqual(instance.name, json_data['name'])
        self.assertEqual(instance.expected_close_date, parse(json_data['expectedCloseDate']).date())
        self.assertEqual(instance.pipeline_change_date, parse(json_data['pipelineChangeDate']))
        self.assertEqual(instance.date_became_lead, parse(json_data['dateBecameLead']))
        self.assertEqual(instance.closed_date, parse(json_data['closedDate']))
        self.assertEqual(instance.notes, json_data['notes'])
        self.assertEqual(instance.source, json_data['source'])
        self.assertEqual(instance.location_id, json_data['locationId'])
        self.assertEqual(instance.business_unit_id, json_data['businessUnitId'])
        self.assertEqual(instance.customer_po, json_data['customerPO'])
        self.assertEqual(instance.priority_id, json_data['priority']['id'])
        self.assertEqual(instance.stage_id, json_data['stage']['id'])
        self.assertEqual(instance.opportunity_type_id, json_data['type']['id'])
        self.assertEqual(instance.status_id, json_data['status']['id'])
        self.assertEqual(instance.primary_sales_rep_id, json_data['primarySalesRep']['id'])
        self.assertEqual(instance.secondary_sales_rep_id, json_data['secondarySalesRep']['id'])
        self.assertEqual(instance.company_id, json_data['company']['id'])
        self.assertEqual(instance.closed_by_id, json_data['closedBy']['id'])
...
class TestHolidaySynchronizer(TestCase, SynchronizerTestMixin):
    ...
    def _assert_fields(self, instance, json_data):
        ...
        self.assertEqual(instance.start_time, parse(json_data['timeStart']).time())
        self.assertEqual(instance.end_time, parse(json_data['timeEnd']).time())
        self.assertEqual(instance.holiday_list.id, json_data['holidayList']['id'])
...
class TestCalendarSynchronizer(TestCase, SynchronizerTestMixin):
    ...
    def _assert_fields(self, instance, json_data):
        self.assertEqual(instance.id, json_data['id'])
        self.assertEqual(instance.name, json_data['name'])
        self.assertEqual(instance.monday_start_time, parse(json_data['mondayStartTime']).time())
        self.assertEqual(instance.monday_end_time, parse(json_data['mondayEndTime']).time())
        self.assertEqual(instance.tuesday_start_time, parse(json_data['tuesdayStartTime']).time())
        self.assertEqual(instance.tuesday_end_time, parse(json_data['tuesdayEndTime']).time())
        self.assertEqual(instance.wednesday_start_time, parse(json_data['wednesdayStartTime']).time())
        self.assertEqual(instance.wednesday_end_time, parse(json_data['wednesdayEndTime']).time())
        self.assertEqual(instance.thursday_start_time, parse(json_data['thursdayStartTime']).time())
        self.assertEqual(instance.thursday_end_time, parse(json_data['thursdayEndTime']).time())
        self.assertEqual(instance.friday_start_time, parse(json_data['fridayStartTime']).time())
        self.assertEqual(instance.friday_end_time, parse(json_data['fridayEndTime']).time())
        self.assertEqual(instance.saturday_start_time, json_data['saturdayStartTime'])
        self.assertEqual(instance.saturday_end_time, json_data['saturdayEndTime'])
        self.assertEqual(instance.sunday_start_time, json_data['sundayStartTime'])
        self.assertEqual(instance.sunday_end_time, json_data['sundayEndTime'])
...
class TestMyCompanyOtherSynchronizer(TestCase, SynchronizerTestMixin):
    ...
    def test_sync_update(self):
        ...
        self._sync(new_json_list)
        changed = self.model_class.objects.get(id=instance_id)
        self.assertNotEqual(original.default_calendar, changed.default_calendar)
...
class TestSLASynchronizer(TestCase, SynchronizerTestMixin):
    ...
    def _assert_fields(self, instance, json_data):
        ...
        self.assertEqual(instance.resolution_hours, json_data['resolutionHours'])
...
class TestTicketSynchronizerMixin:
    ...
    def test_sync_child_tickets(self):
        ...
        mock_call, _patch = mocks.create_mock_call(method_name, self.ticket_fixture)
        ...
        self.assertEqual(datetime.datetime(2005,
          5, 16, 15, 0, tzinfo=(datetime.timezone.utc)), updated_time.time_end)
    ...
    def test_callback_sync_time_entry(self):
        ...
        self.assertGreater(SyncJob.objects.filter(entity_name='TimeEntry').count(), 0)
...
class TestServiceTicketSynchronizer(TestTicketSynchronizerMixin, TestCase):
    ...
    def _assert_sync(self, instance, json_data):
        ...
        self.assertEqual(instance.entered_date_utc, parse(json_data.get('_info').get('dateEntered')))
        self.assertEqual(instance.last_updated_utc, parse(json_data.get('_info').get('lastUpdated')))
        self.assertEqual(instance.required_date_utc, parse(json_data.get('requiredDate')))
        ...
        self.assertEqual(instance.parent_ticket_id, json_data.get('parentTicketId'))
        self.assertEqual(instance.has_child_ticket, json_data.get('hasChildTicket'))
        self.assertEqual(instance.has_child_ticket, json_data.get('hasChildTicket'))
        self.assertEqual(instance.team_id, json_data['team']['id'])
        self.assertEqual(instance.board_id, json_data['board']['id'])
        self.assertEqual(instance.company_id, json_data['company']['id'])
        self.assertEqual(instance.priority_id, json_data['priority']['id'])
        self.assertEqual(instance.location_id, json_data['serviceLocation']['id'])
        self.assertEqual(instance.status_id, json_data['status']['id'])
        ...
        self.assertEqual(instance.bill_time, json_data['billTime'])
        self.assertEqual(instance.automatic_email_cc_flag, json_data['automaticEmailCcFlag'])
        self.assertEqual(instance.automatic_email_contact_flag, json_data['automaticEmailContactFlag'])
        self.assertEqual(instance.automatic_email_resource_flag, json_data['automaticEmailResourceFlag'])
        self.assertEqual(instance.automatic_email_cc, json_data['automaticEmailCc'])
    ...
    def test_project_tickets_not_deleted_during_sync(self):
        ...
        project_ticket = Ticket.objects.create(summary='Project ticket',
        ...
        synchronizer = self.sync_class(full=True)
        synchronizer.sync()
        self.assertEqual(Ticket.objects.get(id=(project_ticket.id)), project_ticket)
        self.assertFalse(Ticket.objects.filter(id=(self.ticket_fixture['id'])).exists())

    def test_callback_sync_service_note(self):
        mocks.service_api_get_notes_call(fixtures.API_SERVICE_NOTE_LIST)
        service_note_sync = sync.ServiceNoteSynchronizer()
        service_note_sync.sync()
        self.assertGreater(SyncJob.objects.filter(entity_name='ServiceNote').count(), 0)
        mocks.time_api_get_time_entries_call(fixtures.API_TIME_ENTRY_LIST)
        mocks.sales_api_get_activities_call(fixtures.API_SALES_ACTIVITIES)
        method_name = 'djconnectwise.sync.TicketSynchronizerMixin.get_single'
        mock_call, _patch = mocks.create_mock_call(method_name, self.ticket_fixture)
        ...
        synchronizer = self.sync_class()
        synchronizer.fetch_sync_by_id(ticket_id)
        last_sync_job = SyncJob.objects.filter(entity_name='ServiceNote').last()
        self.assertEqual(last_sync_job.deleted, 0)
        self.assertEqual(last_sync_job.updated, 0)
        self.assertEqual(last_sync_job.added, 1)
        self.assertEqual(last_sync_job.sync_type, 'partial')
...
class TestProjectTicketSynchronizer(TestTicketSynchronizerMixin, TestCase):
    ...
    def _assert_sync(self, instance, json_data):
        ...
        self.assertEqual(instance.priority_id, json_data['priority']['id'])
        self.assertEqual(instance.location_id, json_data['serviceLocation']['id'])
        self.assertEqual(instance.project_id, json_data['project']['id'])
        self.assertEqual(instance.status_id, json_data['status']['id'])
        self.assertEqual(instance.type_id, json_data['type']['id'])
        self.assertEqual(instance.sub_type_id, json_data['subType']['id'])
        self.assertEqual(instance.sub_type_item_id, json_data['item']['id'])
    ...
    def test_service_tickets_not_deleted_during_sync(self):
        ...
        synchronizer.sync()
        self.assertTrue(Ticket.objects.get(id=(self.ticket_fixture['id'])))
        service_ticket = Ticket.objects.create(summary='Service ticket',
        ...
        synchronizer = self.sync_class(full=True)
        synchronizer.sync()
        self.assertEqual(Ticket.objects.get(id=(service_ticket.id)), service_ticket)
        self.assertFalse(Ticket.objects.filter(id=(self.ticket_fixture['id'])).exists())
...
class TestActivityStatusSynchronizer(TestCase, SynchronizerTestMixin):
    ...
    def _assert_fields(self, instance, json_data):
        ...
        self.assertEqual(instance.default_flag, json_data['defaultFlag'])
        self.assertEqual(instance.inactive_flag, json_data['inactiveFlag'])
        self.assertEqual(instance.spawn_followup_flag, json_data.get('spawnFollowupFlag', False))
        self.assertEqual(instance.closed_flag, json_data['closedFlag'])
...
class TestActivitySynchronizer(TestCase, SynchronizerTestMixin):
    ...
    def _assert_fields(self, activity, api_activity):
        ...
        self.assertEqual(activity.date_start, self._get_datetime(api_activity, 'dateStart'))
        self.assertEqual(activity.date_end, self._get_datetime(api_activity, 'dateEnd'))
        self.assertEqual(activity.assign_to_id, api_activity['assignTo']['id'])
        self.assertEqual(activity.opportunity_id, api_activity['opportunity']['id'])
        if api_activity['ticket'] is not None:
            self.assertEqual(activity.ticket_id, api_activity['ticket']['id'])
        self.assertEqual(activity.status_id, api_activity['status']['id'])
        self.assertEqual(activity.type_id, api_activity['type']['id'])
        self.assertEqual(activity.company_id, api_activity['company']['id'])
        self.assertEqual(activity.agreement_id, api_activity['agreement']['id'])
    ...
    def test_sync_null_member_activity(self):
        ...
        null_member_activity['assignTo'] = {'id': 99999}
        activity_list = [
         null_member_activity]
        method_name = 'djconnectwise.api.SalesAPIClient.get_activities'
...
class TestSyncSettings(TestCase):
    ...
    def test_dynamic_batch_size(self):
        ...
        request_settings = {'batch_size':10, 
         'timeout':10.0}
        _, _patch = mocks.create_mock_call(method_name, request_settings)
        synchronizer = sync.BoardSynchronizer()
        self.assertEqual(synchronizer.batch_size, request_settings['batch_size'])
        _patch.stop()
...
class TestSyncJob(TestCase):
    ...
    def assert_sync_job(self, created, updated, deleted, message, success):
        ...
        self.assertEqual(self.synchronizer.model_class.__name__, sync_job.entity_name)
