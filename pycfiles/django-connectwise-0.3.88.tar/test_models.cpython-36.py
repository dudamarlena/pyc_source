...
# [GCC 8.4.0]
# Embedded file name: /home/cameron/Dev/kanban-dev/django-connectwise/djconnectwise/tests/test_models.py
# Compiled at: 2019-05-14 12:25:52
# Size of source mod 2**32: 21909 bytes
...
class ModelTestCase(TestCase):
    ...
    def setUp(self):
        self.members = mommy.make_recipe('djconnectwise.tests.member',
          _quantity=3)
        self.projects = mommy.make_recipe('djconnectwise.tests.project',
          _quantity=4)
        self.companies = mommy.make_recipe('djconnectwise.tests.company',
          _quantity=2)
        self.ticket_priorities = mommy.make_recipe('djconnectwise.tests.ticket_priority',
          _quantity=3)
        self.connectwise_boards = mommy.make_recipe('djconnectwise.tests.connectwise_board',
          _quantity=3)
        Sla.objects.create(name='Standard SLA',
          default_flag=True,
          respond_hours=2,
          plan_within=8,
          resolution_hours=96,
          based_on='MyCalendar')
        holiday_list = HolidayList.objects.create(name='Test List')
        Holiday.objects.create(name='Test Holiday',
          all_day_flag=True,
          date=datetime.datetime(year=2017, day=12, month=12),
          holiday_list=holiday_list)
        calendar = Calendar.objects.create(name='Standard Office Hours',
          ...
          holiday_list=holiday_list)
        MyCompanyOther.objects.create(default_calendar=calendar)
        Calendar.objects.create()
        for i, s in enumerate(ticket_statuses_names):
            BoardStatus.objects.create(name=s,
              sort_order=i,
...
class TestBoard(ModelTestCase):
    ...
    def test_get_closed_status_prefers_closed(self):
        board = self.connectwise_boards[0]
        self.assertEqual(board.get_closed_status().name, 'Closed')
    ...
    def test_get_closed_status_any_closed(self):
        ...
        self.assertNotEqual(closed_status.name, 'Closed')
...
class TestCalendar(ModelTestCase):
    ...
    def test_get_first_day(self):
        ...
        day, days = calendar.get_first_day(datetime.datetime(year=2018, day=29, month=9))
        self.assertEqual(day, 0)
        self.assertEqual(days, 2)
        day, days = calendar.get_first_day(datetime.datetime(year=2018, day=19, month=9))
        self.assertEqual(day, 2)
        self.assertEqual(days, 0)
        day, days = calendar.get_first_day(datetime.datetime(year=2018, day=23, month=9))
        self.assertEqual(day, 0)
...
class TestTicket(ModelTestCase):
    ...
    def test_str(self):
        ...
        self.assertEqual('{}'.format(t), '1-Únicôde wôrks!')

    def test_save_checks_status(self):
        ticket = Ticket.objects.create(summary='test',
    ...
    def test_save_calls_update_cw_when_kwarg_passed(self):
        ...
        ticket = Ticket.objects.create(summary='test',
          ...
          board=board)
        with patch('djconnectwise.api.ServiceAPIClient') as (mock_serviceapiclient):
    ...
    def test_calculate_sla_expiry(self):
        ...
        self.assertEqual(str(ticket.sla_expire_date.astimezone(tz=None)), '2018-08-23 12:24:34-07:00')
    ...
    def test_calculate_next_state_sla_expiry(self):
        ...
        ticket.calculate_sla_expiry()
        self.assertEqual(str(ticket.sla_expire_date.astimezone(tz=None)), '2018-08-24 09:24:34-07:00')
    ...
    def test_calculate_sla_expiry_several_days(self):
        ...
        ticket = Ticket.objects.create(summary='test',
          ...
          entered_date_utc=(timezone.now()))
        ticket.calculate_sla_expiry()
        self.assertEqual(str(ticket.sla_expire_date.astimezone(tz=None)), '2018-09-06 16:24:34-07:00')
    ...
    def test_calculate_sla_expiry_out_of_hours(self):
        ...
        ticket = Ticket.objects.create(summary='test',
          ...
          entered_date_utc=(timezone.now()))
        ticket.calculate_sla_expiry()
        self.assertEqual(str(ticket.sla_expire_date.astimezone(tz=None)), '2018-08-27 10:00:34-07:00')
    ...
    def test_sla_enter_waiting(self):
        ...
        self.assertEqual(timezone.now(), ticket.do_not_escalate_date)
