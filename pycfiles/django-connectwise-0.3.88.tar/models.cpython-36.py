...
# [GCC 8.4.0]
# Embedded file name: /home/cameron/Dev/kanban-dev/django-connectwise/djconnectwise/models.py
# Compiled at: 2020-04-16 16:15:52
# Size of source mod 2**32: 63759 bytes
import re, logging, urllib, datetime
...
BILL_TYPES = (('Billable', 'Billable'), ('DoNotBill', 'Do Not Bill'), ('NoCharge', 'No Charge'))
...
class SyncJob(models.Model):
    ...
    synchronizer_class = models.CharField(max_length=100, blank=True, null=True)
...
class AvailableConnectWiseBoardManager(models.Manager):
    __doc__ = 'Return only active ConnectWise boards.'
...
class ConnectWiseBoard(TimeStampedModel):
    ...
    work_role = models.ForeignKey('WorkRole',
      blank=True,
      null=True,
    ...
    def get_closed_status(self):
        ...
        """
        try:
            closed_status = self.board_statuses.get(name='Closed',
              closed_status=True)
        except BoardStatus.DoesNotExist:
            closed_status = self.board_statuses.filter(closed_status=True).first()
...
class BoardStatus(TimeStampedModel):
    ...
    CLOSED = 'Closed'
    ESCALATION_STATUSES = (('NotResponded', 'Not Responded'), ('Responded', 'Responded'),
                           ('ResolutionPlan', 'Resolution Plan'), ('Resolved', 'Resolved'),
                           ('NoEscalation', 'No Escalation'))
    ESCALATION_RANK = dict(zip([type[0] for type in ESCALATION_STATUSES], [i for i in range(5)]))
    ...
    escalation_status = models.CharField(max_length=20,
...
class RegularMemberManager(models.Manager):
    __doc__ = "Return members that aren't API members."
...
class Member(TimeStampedModel):
    LICENSE_CLASSES = (('F', 'Full license'), ('A', 'API license'))
    ...
    license_class = models.CharField(blank=True,
      null=True,
      max_length=20,
...
class AvailableCompanyManager(models.Manager):
    __doc__ = "Return only companies whose deleted_flag isn't true."
...
class Company(TimeStampedModel):
    ...
    calendar = models.ForeignKey('Calendar',
      ...
      null=True,
      on_delete=(models.SET_NULL))
    status = models.ForeignKey('CompanyStatus',
      blank=True,
      null=True,
      on_delete=(models.SET_NULL))
...
class MyCompanyOther(models.Model):
    default_calendar = models.ForeignKey('Calendar',
      blank=True,
      null=True,
      on_delete=(models.SET_NULL))
...
class Calendar(models.Model):
    ...
    holiday_list = models.ForeignKey('HolidayList',
      on_delete=(models.SET_NULL), blank=True, null=True)
    monday_start_time = models.TimeField(auto_now=False, auto_now_add=False, blank=True,
      null=True)
    monday_end_time = models.TimeField(auto_now=False, auto_now_add=False, blank=True,
      null=True)
    tuesday_start_time = models.TimeField(auto_now=False, auto_now_add=False, blank=True,
      null=True)
    tuesday_end_time = models.TimeField(auto_now=False, auto_now_add=False, blank=True,
      null=True)
    wednesday_start_time = models.TimeField(auto_now=False, auto_now_add=False, blank=True,
      null=True)
    wednesday_end_time = models.TimeField(auto_now=False, auto_now_add=False, blank=True,
      null=True)
    thursday_start_time = models.TimeField(auto_now=False, auto_now_add=False, blank=True,
      null=True)
    thursday_end_time = models.TimeField(auto_now=False, auto_now_add=False, blank=True,
      null=True)
    friday_start_time = models.TimeField(auto_now=False, auto_now_add=False, blank=True,
      null=True)
    friday_end_time = models.TimeField(auto_now=False, auto_now_add=False, blank=True,
      null=True)
    saturday_start_time = models.TimeField(auto_now=False, auto_now_add=False, blank=True,
      null=True)
    saturday_end_time = models.TimeField(auto_now=False, auto_now_add=False, blank=True,
      null=True)
    sunday_start_time = models.TimeField(auto_now=False, auto_now_add=False, blank=True,
      null=True)
    sunday_end_time = models.TimeField(auto_now=False, auto_now_add=False, blank=True,
      null=True)
    ...
    def get_first_day(self, start):
        ...
        while 1:
            ...
            if day:
                if not self.is_holiday(datetime.timedelta(days=days) + start):
                    return (
                     start_day, days)
                else:
    ...
    def get_sla_time(self, start, end):
        ...
        start_time = datetime.timedelta(hours=(start.hour), minutes=(start.minute))
        start_day_end_time = self.get_day_hours(False, start.weekday())
        if start_day_end_time:
            if not self.is_holiday(timezone.now().astimezone(tz=None)):
                end_of_day = datetime.timedelta(hours=(start_day_end_time.hour), minutes=(start_day_end_time.minute))
        else:
            end_of_day = None
        if start.date() == end.date():
            if end_of_day:
                end_time = min(start_day_end_time, end.time())
                end_time_delta = datetime.timedelta(hours=(end_time.hour),
            ...
            if end_of_day:
                ...
                if end_of_day:
                    end_of_day = datetime.timedelta(hours=(end_of_day.hour),
                      minutes=(end_of_day.minute))
                    end_time = datetime.timedelta(hours=(end.hour), minutes=(end.minute)) if self.get_day_hours(False, end.weekday()) > end.time() else end_of_day
                    start_of_day = self.get_day_hours(True, end.weekday())
                    start_of_day = datetime.timedelta(hours=(start_of_day.hour), minutes=(start_of_day.minute))
    ...
    def next_phase_expiry(self, sla_hours, ticket):
        ...
        if day_of_week is None:
            if days is None:
                ...
                return
        sla_minutes, days, minutes, sla_start = self.get_sla_start(sla_hours, ticket.minutes_waiting, start, day_of_week, days)
        while sla_minutes >= 0:
            day_of_week = (day_of_week + 1) % 7
            days += 1
            start_of_day = self.get_day_hours(True, day_of_week)
            if start_of_day:
                if not self.is_holiday(datetime.timedelta(days=days) + start):
                    start_of_day = datetime.timedelta(hours=(start_of_day.hour), minutes=(start_of_day.minute))
                    end_of_day = self.get_day_hours(False, day_of_week)
                    end_of_day = datetime.timedelta(hours=(end_of_day.hour), minutes=(end_of_day.minute))
                else:
                    continue
                minutes = (end_of_day - start_of_day).total_seconds() / 60
                todays_minutes = minutes if minutes >= 0 else 1440
    ...
    def get_sla_start(self, sla_hours, waiting_min, start, day_of_week, days):
        if days > 0:
            ...
            start_of_day = self.get_day_hours(True, day_of_week)
            start = start.replace(hour=(start_of_day.hour),
              minute=(start_of_day.minute))
            days = 0
        start_date = datetime.timedelta(hours=(start.hour), minutes=(start.minute))
        ...
        sla_minutes = sla_minutes - minutes
        return (
         sla_minutes, days, minutes, start)
    ...
    def set_sla_end(self, start, days, day_of_week, sla_minutes, ticket):
        ...
        if days == 0:
            ...
            expiry_date = expiry_date.replace(hour=(sla_expire_time.hour),
              minute=(sla_expire_time.minute))
        else:
            sla_expire_time = self.get_day_hours(True, day_of_week)
            expiry_date = expiry_date.replace(hour=(sla_expire_time.hour),
              minute=(sla_expire_time.minute)) + datetime.timedelta(minutes=sla_minutes)
...
class ScheduleEntry(models.Model):
    ...
    ticket_object = models.ForeignKey('Ticket',
      blank=True,
      null=True,
      on_delete=(models.CASCADE))
    activity_object = models.ForeignKey('Activity',
      blank=True,
      null=True,
      on_delete=(models.CASCADE))
...
class TimeEntry(models.Model):
    CHARGE_TYPES = (('ServiceTicket', 'Service Ticket'), ('ProjectTicket', 'Project Ticket'),
                    ('ChargeCode', 'Charge Code'), ('Activity', 'Activity'))
    ...
    actual_hours = models.DecimalField(blank=True,
      null=True,
      decimal_places=2,
      max_digits=9)
    ...
    hours_deduct = models.DecimalField(blank=True,
      null=True,
      decimal_places=2,
      max_digits=9)
    ...
    charge_to_id = models.ForeignKey('Ticket',
    ...
    def get_entered_time(self):
        if self.time_end:
            ...
            if self.actual_hours:
                ...
                entered_time = self.time_start + timezone.timedelta(minutes=minutes)
            else:
...
class AvailableBoardTeamManager(models.Manager):
    __doc__ = 'Return only teams whose ConnectWise board is active.'
...
class TicketPriority(TimeStampedModel):
    ...
    _color = models.CharField(max_length=50,
      null=True,
      blank=True,
      db_column='color')
    DEFAULT_COLORS = {'1':'red', 
     '2':'orange', 
     '3':'yellow', 
     '4':'white', 
     '5':'darkmagenta'}
    ...
    def color(self):
        ...
        if self._color == 'Custom':
            ...
            return TicketPriority.DEFAULT_COLORS.get(prio_number, TicketPriority.DEFAULT_COLOR)
...
class ProjectPhase(TimeStampedModel):
    ...
    scheduled_hours = models.DecimalField(blank=True,
      null=True,
      decimal_places=2,
      max_digits=9)
    actual_hours = models.DecimalField(blank=True,
      ...
      decimal_places=2,
      max_digits=9)
    budget_hours = models.DecimalField(blank=True,
      null=True,
      decimal_places=2,
      max_digits=9)
...
class AvailableProjectManager(models.Manager):
    ...
    def get_queryset(self):
        return super().get_queryset().filter(status__closed_flag=False)
...
class AvailableOpportunityStatusManager(models.Manager):
    ...
    def get_queryset(self):
        return super().get_queryset().filter(inactive_flag=False)
...
class Opportunity(TimeStampedModel):
    ...
    probability = models.ForeignKey('SalesProbability', blank=True,
      null=True,
      related_name='sales_probability',
      on_delete=(models.SET_NULL))
    source = models.CharField(max_length=100, blank=True, null=True)
    closed_by = models.ForeignKey('Member', blank=True,
      null=True,
      related_name='opportunity_closed_by',
      on_delete=(models.SET_NULL))
    company = models.ForeignKey('Company', blank=True, null=True, related_name='company_opportunities',
      on_delete=(models.SET_NULL))
    primary_sales_rep = models.ForeignKey('Member', blank=True,
      null=True,
      related_name='opportunity_primary',
      on_delete=(models.SET_NULL))
    priority = models.ForeignKey('OpportunityPriority', on_delete=(models.SET_NULL),
      null=True)
    stage = models.ForeignKey('OpportunityStage', blank=True, null=True, on_delete=(models.SET_NULL))
    status = models.ForeignKey('OpportunityStatus', blank=True, null=True, on_delete=(models.SET_NULL))
    secondary_sales_rep = models.ForeignKey('Member',
      blank=True,
      null=True,
      related_name='opportunity_secondary',
      on_delete=(models.SET_NULL))
    opportunity_type = models.ForeignKey('OpportunityType', blank=True,
      null=True,
      on_delete=(models.SET_NULL))
    ...
    def get_connectwise_url(self):
        params = dict(recordType='OpportunityFv',
          recid=(self.id),
          companyName=(settings.CONNECTWISE_CREDENTIALS['company_id']))
        return '{}/{}?{}'.format(settings.CONNECTWISE_SERVER_URL, settings.CONNECTWISE_TICKET_PATH, urllib.parse.urlencode(params))
    ...
    def update_cw(self):
        ...
        return sales_client.update_opportunity_stage(self.id, self.stage)
...
class Ticket(TimeStampedModel):
    ...
    BILL_TIME_TYPES = (('Billable', 'Billable'), ('DoNotBill', 'Do Not Bill'), ('NoCharge', 'No Charge'),
    ...
    SLA_STAGE = (
     ...
     (
      WAITING, 'Waiting'))
    STAGE_RANK = dict(zip([i for i in range(5)], [type[0] for type in SLA_STAGE]))
    actual_hours = models.DecimalField(blank=True,
      null=True,
      decimal_places=2,
      max_digits=9)
    ...
    sla_stage = models.CharField(blank=True, null=True, max_length=250,
      choices=SLA_STAGE,
      db_index=True)
    minutes_waiting = models.PositiveIntegerField(default=0)
    bill_time = models.CharField(blank=True, null=True, max_length=20,
      choices=BILL_TIME_TYPES)
    automatic_email_cc_flag = models.BooleanField(default=False)
    automatic_email_cc = models.CharField(blank=True, null=True,
      max_length=1000)
    ...
    board = models.ForeignKey('ConnectwiseBoard',
      blank=True, null=True, on_delete=(models.CASCADE))
    company = models.ForeignKey('Company',
      blank=True, null=True, related_name='company_tickets', on_delete=(models.SET_NULL))
    location = models.ForeignKey('Location',
      blank=True, null=True, related_name='location_tickets', on_delete=(models.SET_NULL))
    members = models.ManyToManyField('Member',
      through='ScheduleEntry', related_name='member_tickets')
    owner = models.ForeignKey('Member',
      blank=True, null=True, on_delete=(models.SET_NULL))
    priority = models.ForeignKey('TicketPriority',
      blank=True, null=True, on_delete=(models.SET_NULL))
    project = models.ForeignKey('Project',
      blank=True, null=True, related_name='project_tickets', on_delete=(models.CASCADE))
    phase = models.ForeignKey('ProjectPhase',
      blank=True, null=True, related_name='phase_tickets', on_delete=(models.SET_NULL))
    status = models.ForeignKey('BoardStatus',
      blank=True, null=True, related_name='status_tickets', on_delete=(models.SET_NULL))
    team = models.ForeignKey('Team',
      blank=True, null=True, related_name='team_tickets', on_delete=(models.SET_NULL))
    sla = models.ForeignKey('Sla',
      blank=True, null=True, related_name='tickets', on_delete=(models.SET_NULL))
    type = models.ForeignKey('Type',
      blank=True, null=True, related_name='type_tickets', on_delete=(models.SET_NULL))
    sub_type = models.ForeignKey('Subtype',
      blank=True, null=True, related_name='subtype_tickets', on_delete=(models.SET_NULL))
    sub_type_item = models.ForeignKey('Item',
      blank=True, null=True, related_name='item_tickets', on_delete=(models.SET_NULL))
    agreement = models.ForeignKey('Agreement',
      blank=True, null=True, related_name='agreement_tickets', on_delete=(models.SET_NULL))
        ...
        if self.status:
            if self.board:
                if self.status.board != self.board:
                    logger.warning("For ticket {}, {} (ID {}) is not a valid status for the ticket's ConnectWise board ({}, ID {}).".format(self.id, self.status.name, self.status.id, self.board, self.board.id))
    ...
    def update_cw(self):
        ...
        return api_client.update_ticket(self.id, self.closed_flag, self.priority, self.status)
    ...
    def close(self, *args, **kwargs):
        ...
        if closed_status is None:
            raise InvalidStatusError("There are no closed statuses on this ticket's ConnectWise board ({}). Its status has not been changed.".format(self.board))
    ...
    def calculate_sla_expiry(self):
        if not self.entered_date_utc:
            ...
            if not self.status:
                logger.error('Ticket {}-{}, does not have a status set. Skipping SLA calculation.'.format(self.id, self.summary))
                return
            else:
                if not self.sla:
                    return
                else:
                    sla_priority = SlaPriority.objects.filter(sla=(self.sla),
                      priority=(self.priority)).first()
                    if sla_priority:
                        sla = sla_priority
                    else:
                        sla = self.sla
                new_stage = self.STAGE_RANK.get(self.status.get_status_rank())
                calendar = self.sla.get_calendar(self.company)
                calendar or logger.info('No calendar found for SLA {} on ticket {}'.format(sla.id, self.id))
                return
        self.change_sla_state(new_stage, calendar, sla)

    def change_sla_state(self, new_stage, calendar, sla):
        valid_stage = self.sla_state.get_valid_next_state(new_stage) if self.sla_state else new_stage
        if valid_stage:
...
class Sla(TimeStampedModel, SlaGoalsMixin):
    BASED_ON = (('MyCalendar', 'My Company Calendar'), ('Customer', "Customer's Calendar"),
                ('AllHours', '24 Hours'), ('Custom', 'Custom Calendar'))
    name = models.TextField(max_length=250)
    default_flag = models.BooleanField(default=False)
    respond_hours = models.BigIntegerField()
    plan_within = models.BigIntegerField()
    resolution_hours = models.BigIntegerField()
    based_on = models.CharField(max_length=50, choices=BASED_ON, default='MyCalendar')
    calendar = models.ForeignKey('Calendar',
      blank=True,
      null=True,
      on_delete=(models.SET_NULL))
    ...
    def get_calendar(self, company):
        try:
            if self.calendar:
                ...
                if self.based_on == 'Customer':
                    if company:
                        return Company.objects.get(id=(company.id)).calendar
                if self.based_on == 'MyCalendar':
...
class SlaPriority(TimeStampedModel, SlaGoalsMixin):
    ...
    def __str__(self):
        return 'priority: {}, on SLA:{}'.format(str(self.priority), self.sla.id)
...
class OpportunityNote(TimeStampedModel):
    ...
    def __str__(self):
        return 'id: {}, on Opportunity id:{}'.format(str(self.id), self.opportunity.id)
...
class Activity(TimeStampedModel):
    ...
    def update_cw(self):
        ...
        """
        sales_client = api.SalesAPIClient()
        return sales_client.update_activity_status(self.id, self.status)

    def get_connectwise_url(self):
        params = dict(recordType='ActivityFv',
          recid=(self.id),
          companyName=(settings.CONNECTWISE_CREDENTIALS['company_id']))
        return '{}/{}?{}'.format(settings.CONNECTWISE_SERVER_URL, settings.CONNECTWISE_TICKET_PATH, urllib.parse.urlencode(params))
...
class Resolve(EscalateState):
    ...
    def __init__(self, ticket):
        super().__init__(ticket)
        self.valid_next_states = self.valid_next_states.union({Ticket.RESOLVED})
...
class Waiting(SLAMachineState):
    ...
    def leave(self, calendar, sla):
        if self.ticket.do_not_escalate_date:
            self.ticket.minutes_waiting += calendar.get_sla_time(self.ticket.do_not_escalate_date.astimezone(tz=None), timezone.now().astimezone(tz=None))
    ...
    def _lowest_possible_stage(self, stage):
        if stage == Ticket.RESOLVED or stage == Ticket.RESOLVE:
            ...
            if stage == Ticket.PLAN:
                if self.ticket.date_resplan_utc:
                    return Ticket.RESOLVE
                else:
                    return stage
            else:
                if stage == Ticket.RESPOND:
                    if self.ticket.date_resplan_utc:
                        return Ticket.RESOLVE
                    else:
                        if self.ticket.date_responded_utc:
                            return Ticket.PLAN
                        return stage
                else:
...
class StateMachineManager(object):
    SLA_STATE = {Ticket.RESPOND: Respond, 
     Ticket.PLAN: Plan, 
