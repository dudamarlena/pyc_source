...
# [GCC 8.4.0]
# Embedded file name: /home/cameron/Dev/kanban-dev/django-connectwise/djconnectwise/tests/test_api.py
# Compiled at: 2019-12-04 18:58:20
# Size of source mod 2**32: 30559 bytes
...
class TestConnectWiseAPIClient(TestCase):
    ...
    def test_prepare_conditions_single(self):
        conditions = [
         'closedFlag=False']
        self.assertEqual(self.client.prepare_conditions(conditions), '(closedFlag=False)')

    def test_cw_cloud_url(self):
        client = api.ServiceAPIClient()
        self.assertEqual(client.server_url, 'https://localhost')
        client = api.ServiceAPIClient(server_url='https://na.myconnectwise.net')
        self.assertEqual(client.server_url, 'https://api-na.myconnectwise.net')
        client = api.ServiceAPIClient(server_url='https://api-na.myconnectwise.net')
        self.assertEqual(client.server_url, 'https://api-na.myconnectwise.net')
    ...
    def test_prepare_conditions_multiple(self):
        ...
        conditions = [
         'closedFlag=False',
         'status/id in (1,2,3)',
         'lastUpdated>' + timezone.localtime(value=test_datetime).isoformat()]
        self.assertEqual(self.client.prepare_conditions(conditions), '(closedFlag=False and status/id in (1,2,3) and lastUpdated>1948-05-14T12:00:00-05:51)')
    ...
    def test_request_failure(self):
        ...
        responses.add((responses.GET),
          endpoint, body=(requests.RequestException()))
...
class TestServiceAPIClient(BaseAPITestCase):
    ...
    def test_get_statuses(self):
        endpoint_url = 'boards/{}/statuses'.format(fixtures.API_BOARD['id'])
    ...
    def test_get_notes(self):
        endpoint_url = 'tickets/{}/notes'.format(fixtures.API_SERVICE_TICKET['id'])
        endpoint_url = self.client._endpoint(endpoint_url)
    ...
    def test_get_subtypes(self):
        board_id = fixtures.API_BOARD_LIST[0]['id']
        endpoint = '{}/{}/subtypes/'.format(self.client.ENDPOINT_BOARDS, board_id)
        endpoint_url = self.client._endpoint(endpoint)
...
class TestHostedAPIClient(BaseAPITestCase):
    ...
    def test_post_hosted_tab(self):
        body = {'screenId':fixtures.API_HOSTED_SCREENS['row_values'][3], 
         'description':'A new hosted tab', 
         'url':'https://example.com', 
         'type':'Tab'}
        method_name = 'djconnectwise.api.ConnectWiseAPIClient.request'
        mock_call, _patch = mk.create_mock_call(method_name, body)
        self.client.post_hosted_tab(screen_id=(fixtures.API_HOSTED_SCREENS['row_values'][3]),
          description='A new hosted tab',
    ...
    def test_get_hosted_tabs(self):
        endpoint_url = self.client._endpoint(self.client.ENDPOINT_HOSTED_SETUPS)
    ...
    def test_delete_hosted_tab(self):
        ...
        mock_call, _patch = mk.create_mock_call(method_name, '201 Created')
...
class TestHostedReportAPIClient(BaseAPITestCase):
    ...
    def test_get_hosted_screen_ids(self):
        endpoint_url = self.client._endpoint(self.client.ENDPOINT_HOSTED_REPORT)
...
class TestSystemAPIClient(BaseAPITestCase):
    ...
    def test_get_connectwise_version(self):
        ...
        mk.get(endpoint, fixtures.API_CW_VERSION)
    ...
    def test_get_members(self):
        ...
        mk.get(endpoint, fixtures.API_MEMBER_COUNT)
        endpoint = self.client._endpoint(self.client.ENDPOINT_MEMBERS)
        mk.get(endpoint, fixtures.API_MEMBER_LIST)
    ...
    def test_get_member_image_by_photo_id(self):
        ...
        endpoint = self.client._endpoint(self.client.ENDPOINT_MEMBERS_IMAGE.format(member['photo']['id'], member['identifier']))
        mk.get_raw(endpoint,
          avatar,
          headers={'content-disposition': 'attachment; filename={}'.format(avatar_filename)})
        result_filename, result_avatar = self.client.get_member_image_by_photo_id(member['photo']['id'], member['identifier'])
    ...
    def test_attachment_filename_returns_filename(self):
        self.assertEqual(self.client._attachment_filename('attachment; filename=somefile.jpg'), 'somefile.jpg')
        self.assertEqual(self.client._attachment_filename('attachment; filename=somefile'), 'somefile')
        self.assertEqual(self.client._attachment_filename('attachment; filename="somefile and a space.jpg"'), 'somefile and a space.jpg')
        filename = 'attachment; filename=Ƨōmefile.jpg'
        self.assertEqual(self.client._attachment_filename(filename), 'Ƨōmefile.jpg')

    def test_attachment_filename_returns_none_on_invalid(self):
        self.assertEqual(self.client._attachment_filename(''), None)
...
class TestCompanyAPIClient(BaseAPITestCase):
    ...
    def test_by_id(self):
        ...
        mk.get(endpoint_url, fixtures.API_COMPANY)
    ...
    @responses.activate
    def test_get(self):
        mk.get(self.endpoint, fixtures.API_COMPANY_LIST)
    ...
    def test_get_no_results(self):
        data = {}
        mk.get(self.endpoint, data)
        result = self.client.get_companies()
        self.assertEqual(result, data)

    @responses.activate
    def test_get_company_statuses(self):
        endpoint = self.client._endpoint(self.client.ENDPOINT_COMPANY_STATUSES)
    ...
    def test_get_company_types(self):
        endpoint = self.client._endpoint(self.client.ENDPOINT_COMPANY_TYPES)
...
class TestScheduleAPIClient(BaseAPITestCase):
    ...
    def test_post_schedule_entry(self):
        ...
        ticket_object.id = 4
        method_name = 'djconnectwise.api.ConnectWiseAPIClient.request'
        mock_call, _patch = mk.create_mock_call(method_name, fixtures.API_SCHEDULE_ENTRY_FOR_TICKET)
        self.client = api.ScheduleAPIClient()
        self.client.post_schedule_entry(ticket_object, schedule_type)
        self.assertEqual(mock_call.call_count, 1)
    ...
    def test_delete_schedule_entry(self):
        object_id = 69
        method_name = 'requests.request'
        mock_call, _patch = mk.create_mock_call(method_name, '200 OK')
        self.client = api.ScheduleAPIClient()
...
class TestTimeAPIClient(BaseAPITestCase):
    ...
    def test_post_time_entry(self):
        method_name = 'djconnectwise.api.ConnectWiseAPIClient.request'
        mock_call, _patch = mk.create_mock_call(method_name, fixtures.API_TIME_ENTRY)
        self.client = api.TimeAPIClient()
        target_data = {'id':1, 
         'type':'ServiceTicket'}
        self.client.post_time_entry(target_data,
...
class TestFetchAPICodebase(TestCase):
    ...
    def _companyinfo_endpoint(self):
        return api.CompanyInfoManager.COMPANYINFO_ENDPOINT.format(self.HOST, self.COMPANY_ID)

    def test_fetch_api_codebase_bad_url(self):
        api_codebase, updated = self.manager.fetch_api_codebase('', 'bad-company-id')
        self.assertEqual(api_codebase, api.DEFAULT_CW_API_CODEBASE)
    ...
    def test_fetch_api_company_info_endpoint_unavailable(self):
        ...
        cache.clear()
        with self.assertRaises(ConnectWiseAPIError):
            api_codebase, updated = self.manager.fetch_api_codebase(self.HOST, self.COMPANY_ID)
    ...
    def test_fetch_api_codebase_from_warm_cache(self):
        ...
        mock_get_call, _patch = mk.company_info_get_company_info_call(fixtures.API_COMPANY_INFO)
        api_codebase, updated = self.manager.fetch_api_codebase(self.HOST, self.COMPANY_ID)
        self.assertEqual(api_codebase, cache.get('api_codebase'))
        _patch.stop()
...
class TestSalesAPIClient(BaseAPITestCase):
    ...
    def setUp(self):
        ...
        self.endpoint = self.client._endpoint(self.client.ENDPOINT_OPPORTUNITY_STATUSES)

    @responses.activate
    def test_by_id(self):
        endpoint = self.client._endpoint(self.client.ENDPOINT_OPPORTUNITIES)
    ...
    def test_get_opportunity_statuses(self):
        endpoint = self.client._endpoint(self.client.ENDPOINT_OPPORTUNITY_STATUSES)
    ...
    def test_get_opportunity_types(self):
        endpoint = self.client._endpoint(self.client.ENDPOINT_OPPORTUNITY_TYPES)
    ...
    def test_get_sales_probabilities(self):
        endpoint = self.client._endpoint(self.client.ENDPOINT_PROBABILITIES)
    ...
    def test_get_activities(self):
        endpoint = self.client._endpoint(self.client.ENDPOINT_ACTIVITIES)
    ...
    def test_get_activity_statuses(self):
        endpoint = self.client._endpoint(self.client.ENDPOINT_ACTIVITY_STATUSES)
    ...
    def test_get_activity_types(self):
        endpoint = self.client._endpoint(self.client.ENDPOINT_ACTIVITY_TYPES)
    ...
    def test_get_opportunity_notes(self):
        ...
        endpoint_url = 'opportunities/{}/notes'.format(opportunity_id)
        endpoint_url = self.client._endpoint(endpoint_url)
...
class TestAPISettings(TestCase):
    ...
    def test_dynamic_batch_size(self):
        ...
        client = api.ServiceAPIClient()
        self.assertEqual(client.timeout, request_settings['timeout'])
        _patch.stop()
    ...
    def test_retry_attempts(self):
        with self.assertRaises(ConnectWiseAPIError):
            ...
            client = api.ServiceAPIClient()
            client.fetch_resource('localhost/some-bad-url', retry_counter=retry_counter)
            self.assertEqual(retry_counter['count'], client.request_settings['max_attempts'])
    ...
    def test_no_retry_attempts_in_400_range(self):
        ...
        for status_code in http_400_range:
            ...
            try:
                ...
                client.fetch_resource((api.ServiceAPIClient.ENDPOINT_TICKETS),
    ...
    def test_retry_attempts_cloud_domain_cold_cache(self):
        ...
        with self.assertRaises(ConnectWiseRecordNotFoundError):
            client.fetch_resource((api.ServiceAPIClient.ENDPOINT_TICKETS),
              retry_counter=retry_counter)
        self.assertEqual(retry_counter['count'], api.ServiceAPIClient.MAX_404_ATTEMPTS + 1)
