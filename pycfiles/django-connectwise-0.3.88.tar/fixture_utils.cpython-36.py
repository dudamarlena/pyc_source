...
# [GCC 8.4.0]
# Embedded file name: /home/cameron/Dev/kanban-dev/django-connectwise/djconnectwise/tests/fixture_utils.py
...
def init_opportunity_notes():
    mocks.sales_api_get_opportunity_notes_call(fixtures.API_SALES_OPPORTUNITY_NOTE_LIST)
...
def init_slapriorities():
    mocks.service_api_get_sla_priorities_call(fixtures.API_SERVICE_SLA_PRIORITY_LIST)
...
def init_holidays():
    mocks.schedule_api_get_holidays_call(fixtures.API_SCHEDULE_HOLIDAY_MODEL_LIST)
    synchronizer = sync.HolidaySynchronizer()
    return synchronizer.sync()


def init_holiday_lists():
    mocks.schedule_api_get_holiday_lists_call(fixtures.API_SCHEDULE_HOLIDAY_LIST_LIST)
...
def init_project_statuses():
    mocks.projects_api_get_project_statuses_call(fixtures.API_PROJECT_STATUSES)
    synchronizer = sync.ProjectStatusSynchronizer()
    return synchronizer.sync()


def init_project_types():
    mocks.projects_api_get_project_types_call(fixtures.API_PROJECT_TYPES)
    synchronizer = sync.ProjectTypeSynchronizer()
    return synchronizer.sync()


def init_project_phases():
    mocks.projects_api_get_project_phases_call(fixtures.API_PROJECT_PHASE_LIST)
...
def init_opportunity_statuses():
    mocks.sales_api_get_opportunity_statuses_call(fixtures.API_SALES_OPPORTUNITY_STATUSES)
    synchronizer = sync.OpportunityStatusSynchronizer()
    return synchronizer.sync()


def init_sales_probabilities():
    mocks.sales_api_get_sales_probabilities_call(fixtures.API_SALES_PROBABILITY_LIST)
...
def init_opportunity_stages():
    mocks.sales_api_get_opportunity_stages_call(fixtures.API_SALES_OPPORTUNITY_STAGES)
    synchronizer = sync.OpportunityStageSynchronizer()
    return synchronizer.sync()


def init_opportunities():
    mocks.sales_api_get_opportunities_call(fixtures.API_SALES_OPPORTUNITIES)
    synchronizer = sync.OpportunitySynchronizer()
    return synchronizer.sync()


def init_schedule_types():
    mocks.schedule_api_get_schedule_types_call(fixtures.API_SCHEDULE_TYPE_LIST)
    synchronizer = sync.ScheduleTypeSynchronizer()
    return synchronizer.sync()


def init_schedule_statuses():
    mocks.schedule_api_get_schedule_statuses_call(fixtures.API_SCHEDULE_STATUS_LIST)
    synchronizer = sync.ScheduleStatusSynchronizer()
    return synchronizer.sync()


def init_schedule_entries():
    mocks.schedule_api_get_schedule_entries_call(fixtures.API_SCHEDULE_ENTRIES)
    synchronizer = sync.ScheduleEntriesSynchronizer()
    return synchronizer.sync()


def init_activity_statuses():
    mocks.sales_api_get_activities_statuses_call(fixtures.API_SALES_ACTIVITY_STATUSES)
    synchronizer = sync.ActivityStatusSynchronizer()
    return synchronizer.sync()


def init_activity_types():
    mocks.sales_api_get_activities_types_call(fixtures.API_SALES_ACTIVITY_TYPES)
...
def init_types():
    mocks.service_api_get_types_call(fixtures.API_TYPE_LIST)
    synchronizer = sync.TypeSynchronizer()
    return synchronizer.sync()


def init_subtypes():
    mocks.service_api_get_subtypes_call(fixtures.API_SUBTYPE_LIST)
    synchronizer = sync.SubTypeSynchronizer()
    return synchronizer.sync()


def init_items():
    mocks.service_api_get_items_call(fixtures.API_ITEM_LIST)
    synchronizer = sync.ItemSynchronizer()
    return synchronizer.sync()


def init_work_types():
    mocks.time_api_get_work_types_call(fixtures.API_WORK_TYPE_LIST)
    synchronizer = sync.WorkTypeSynchronizer()
    return synchronizer.sync()


def init_work_roles():
    mocks.time_api_get_work_roles_call(fixtures.API_WORK_ROLE_LIST)
    synchronizer = sync.WorkRoleSynchronizer()
    return synchronizer.sync()


def init_agreements():
    mocks.finance_api_get_agreements_call(fixtures.API_AGREEMENT_LIST)
