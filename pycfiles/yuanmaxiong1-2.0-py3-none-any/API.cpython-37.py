...
# [GCC 8.4.0]
# Embedded file name: C:\Users\17630\Desktop\瓦力课程\试听课\代码\dianyi\API.py
# Compiled at: 2019-10-24 22:43:05
# Size of source mod 2**32: 25239 bytes
...
def random_pos():
    return (
     random.randint(-global_var.WIDTH // 2, global_var.WIDTH // 2), random.randint(-global_var.HEIGHT // 2, global_var.HEIGHT // 2))
...
class NewGameObject:
    ...
    def collide(self, other):
        if not self._gameObject.sprite and self._gameObject.sprite.visible or isinstance(other, tuple) or isinstance(other, vec):
            if self._gameObject.body:
                ...
                offset = self._gameObject.pos - other + vec(self._gameObject.sprite.width // 2, self._gameObject.sprite.height // 2)
                try:
                ...
                if self._gameObject.body:
                    if other._gameObject.body:
                        ...
                        if other._gameObject.tiledmap_bodies:
                        ...
                        offset = self._gameObject.pos - other._gameObject.pos + vec(self._gameObject.sprite.width // 2, self._gameObject.sprite.height // 2) - vec(other._gameObject.sprite.width // 2, other._gameObject.sprite.height // 2)
                        return bool(mask1.overlap(mask2, [int(i) for i in offset]))
                else:
                    pass
                if not self._gameObject.sprite:
                    if isinstance(other, tuple) or isinstance(other, vec):
                        if self._gameObject.body:
                            return self._gameObject.body.point_query(other)
                        if self._gameObject.tiledmap_bodies:
                            return self._gameObject.tiledmap_bodies.point_query(other)
        elif self._gameObject.body:
            return self._gameObject.body.collide(other._gameObject.body)
        if self._gameObject.tiledmap_bodies:
            if other._gameObject.body:
                return self._gameObject.tiledmap_bodies.collide(other._gameObject.body)
