...
# [GCC 8.4.0]
# Embedded file name: c:\Users\asus\Desktop\PythonGameEngine\Walimaker\sprite.py
# Compiled at: 2019-09-05 05:59:33
...
class ListSpriteStrategy:
    ...
    def update(self):
        ...
        if self.playing:
            if time.time() - self._last_update_time > self.dt:
...
class AnimatorStrategy:
    ...
    def __init__(self, imgs_dict):
        ...
        for state in imgs_dict:
            self._imgs_dict[state] = {'animation':[],  'dt':0.05, 
             'next_state':state, 
             'start_func':None, 
             'end_func':None}
    ...
    def update(self):
        ...
        if self.playing:
            if time.time() - self._last_update_time > self._imgs_dict[self._state]['dt']:
                self._last_update_time = time.time()
                self._frame += 1
