...
# Embedded file name: /Users/erikvw/.venvs/ambition/lib/python3.7/site-packages/ambition_validators/tests/test_medical_expenses_form_validator.py
...
class TestMedicalExpensesFormValidator(TestCase):
    ...
    def test_total_money_spent_error(self):
        ...
        cleaned_data = {'subject_spent_last_4wks':10, 
    ...
    def test_transport_fare_invalid_bicycle(self):
        cleaned_data = {'form_of_transport':'bicycle', 
         'transport_fare':10.0}
        form_validator = MedicalExpensesFormValidator(cleaned_data=cleaned_data)
        self.assertRaises(ValidationError, form_validator.validate)
    ...
    def test_transport_fare_invalid_ambulance(self):
        cleaned_data = {'form_of_transport':'ambulance', 
         'transport_fare':10.0}
        form_validator = MedicalExpensesFormValidator(cleaned_data=cleaned_data)
        self.assertRaises(ValidationError, form_validator.validate)
        self.assertIn('transport_fare', form_validator._errors)
    ...
    def test_transport_fare_invalid_foot(self):
        cleaned_data = {'form_of_transport':'foot', 
         'transport_fare':100.0}
        form_validator = MedicalExpensesFormValidator(cleaned_data=cleaned_data)
        self.assertRaises(ValidationError, form_validator.validate)
        self.assertIn('transport_fare', form_validator._errors)

    def test_transport_fare_valid(self):
        cleaned_data = {'form_of_transport':'private_taxi', 
         'transport_fare':10.0}
        form_validator = MedicalExpensesFormValidator(cleaned_data=cleaned_data)
        try:
            form_validator.validate()
