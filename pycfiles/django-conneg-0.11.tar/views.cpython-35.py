...
# [GCC 8.4.0]
# Embedded file name: /home/kebl2765/Projects/django-conneg/django_conneg/views.py
# Compiled at: 2017-02-08 05:59:23
# Size of source mod 2**32: 18528 bytes
from __future__ import unicode_literals
import datetime
try:
    import httplib as http_client, urlparse as urllib_parse
    from urllib import urlencode
    str_types = (unicode, str)
except ImportError:
    import http.client as http_client, urllib.parse as urllib_parse
    from urllib.parse import urlencode
    str_types = (
     str,)
    unicode = str

import inspect, itertools, logging, sys, time, urllib, warnings
...
class BaseContentNegotiatedView(View):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if format_url_parameter:
            self.format_override = [
             format_url_parameter]
        else:
            if request.GET.get(self._format_override_parameter):
                self.format_override = request.GET[self._format_override_parameter].split(',')
            else:
                if request.POST.get(self._format_override_parameter):
    ...
    def set_renderers(self, request=None, context=None, template_name=None, early=False):
        ...
        args = (
         self.conneg, context, template_name,
        ...
        if getattr(request, 'renderers_for_args', None) != args:
            ...
            if not isinstance(fallback_formats, (list, tuple)):
                fallback_formats = (
                 fallback_formats,)
            request.renderers = self.conneg.get_renderers(request=request, context=context, template_name=template_name, accept_header=request.META.get('HTTP_ACCEPT'), formats=self.format_override, default_format=self._default_format, fallback_formats=fallback_formats, early=early)
    ...
    def get_render_params(self, request, context, template_name):
        if not template_name:
            ...
            return (
             request or self.request, context or self.context, template_name)
...
class ContentNegotiatedView(BaseContentNegotiatedView):
    ...
    def error(self, request, exception, args, kwargs, status_code):
        ...
        template_names = self.error_template_names.get(status_code, self.error_template_names['default'])
    ...
    def error_406(self, request, exception, *args, **kwargs):
        ...
        context = {'error': {'status_code': http_client.NOT_ACCEPTABLE, 
                   ...
                   'format_parameter': request.GET.get(self._format_override_parameter) or request.POST.get(self._format_override_parameter), 
                   
                   'format_parameter_parsed': (request.GET.get(self._format_override_parameter, '') or request.POST.get(self._format_override_parameter, '')).split(','), 
                   
                   'accept_header': request.META.get('HTTP_ACCEPT'), 
                   'accept_header_parsed': accept_header_parsed}}
        return self.error_view(request, context, self.error_template_names[http_client.NOT_ACCEPTABLE])
...
if 'json' in locals():
    ...
    class JSONView(ContentNegotiatedView):
        ...
        def render_json(self, request, context, template_name):
            ...
            return http.HttpResponse(json.dumps(self.simplify_for_json(context), indent=self._json_indent), content_type='application/json')
    ...
    class JSONPView(JSONView):
        ...
        def render_js(self, request, context, template_name):
            context = self.preprocess_context_for_json(context)
            callback_name = request.GET.get(self._default_jsonp_callback_parameter, self._default_jsonp_callback)
            return http.HttpResponse('%s(%s);' % (callback_name, json.dumps(self.simplify_for_json(context), indent=self._json_indent)), content_type='application/javascript')
