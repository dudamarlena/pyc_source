...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/npsql/hashjoin.py
# Compiled at: 2020-03-13 15:13:35
# Size of source mod 2**32: 9852 bytes
"""
.. module:: npsql.hashjoin
...
def arg_left_join(tbl_l, index1, tbl_r, index2):
    ...
    idx = np.array([(s, i) for i, r in enumerate(zip(*tbl_l[:, index1].data)) for s in hash_r.get(r, [-1])])
    return idx
...
def add_empty_row(tbl):
    ...
    for c in tbl.dtype.names:
        ...
        try:
            ...
            raise ValueError('Outer join cannot be fullfilled when there are other ', 'columns than float, string or integer because Nptab ', "doesn't know what to fill it up with.")
...
class HashJoinMixin(object):
    __doc__ = 'Mixin to add to the Nptab class, providing join and group_by methods\n    '
    ...
    def join(self, tbl_r, key, key_r=None, jointype='inner', suffixes=('_l', '_r')):
        ...
        and the provided npsql 'tbl_r' on the columns listed in 'key'.

        arguments:
            tbl_r (Nptab) :
                The right npsql to be joined.
            key (string or list) :
                Name of the column(s) to be used as the key(s).
            key_r (list) :
                A list of columnnames of the right npsql matching the left
                npsql. Defaults to the list provided in `ind`.
            jointype (str) :
                One of: `inner`, `left`, `right`, `outer`. If `inner`, returns
                the elements common to both tabls. If `outer`, returns the
                common elements as well as the elements of the left npsql not in
                the right npsql and the elements of the right npsql not in the
                left npsql. If `left`, returns the common elements and the
                elements of the left npsql not in the right npsql. If `right`,
                returns the common elements and the elements of the right npsql
                not in the left npsql.
            suffixes (tuple) :
                Strings to be added to the left and right npsql column names.
        ...
        Examples:
            ...
            >>> tbl = Nptab({"a":list(range(4)), "b": ['a', 'b'] *2})
            >>> tbl_b = Nptab({"a":list(range(4)), "c": ['d', 'e'] *2})
            >>> tbl.join(tbl_b, "a")
               a | b_l   | c_r
            -----+-------+-------
               0 | a     | d
    ...
    def group_by(self, key, aggregate_fie_col=None):
        ...
        Examples:
            ...
            >>> tbl = Nptab({'a':[10, 20, 30, 40]*3, 'b':["100", "200"]*6, 'c':[100, 200]*6})
            >>> from npsql import first
...
def first(array):
    """
    Get the first element when doing a :mod:`npsql.Nptab.group_by`.
