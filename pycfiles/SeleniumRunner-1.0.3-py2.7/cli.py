...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/sr/cli.py
# Compiled at: 2019-03-23 11:01:29
import argparse, time, yaml, sys, getopt, unittest
...
def main_run():
    parser = argparse.ArgumentParser(description='Selenium Testing')
    parser.add_argument('--testcase_path', help='测试集路径')
    parser.add_argument('--driver_path', help='chrome driver路径')
    args = parser.parse_args()
    testcase_path = str(args.testcase_path)
    driver_path = str(args.driver_path)
    print ('\x1b[0;32m{0}\x1b[0m').format(testcase_path)
    print ('\x1b[0;32m{0}\x1b[0m').format(driver_path)
    suite = unittest.TestSuite()
    param = {}
