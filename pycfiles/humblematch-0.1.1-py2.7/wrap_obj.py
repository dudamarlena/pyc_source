...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\humblematch\wrap_obj.py
# Compiled at: 2015-06-20 09:55:02
from __future__ import print_function
from enum import Enum
from collections import Mapping, Iterable
from warnings import warn
import logging, types, numbers
__all__ = [
 'Any', 'OR', 'WrapObj', 'w', 'WrapMultiObj', 'inf', 'StringTypes', 'NumberType', 'FunctionType', 'ClassType']
...
class WrapObj(object):
    ...
    def as_obj(self):
        ...
        try:
            ...
            raise TypeError(('data={data} should be of type Mapping (e.g. dict)').format(data=self.data))
    ...
    def match_list(self, other):
        ...
        if check_dir == CHECK_DIR.BACKWARDS:
            for ele_index in range(-1, -(len(self.data) + 1), -1):
                ele_data = self.data[ele_index]
                if isinstance(ele_data, WrapMultiObj):
    ...
    def match_dict_or_obj(self, other, dict_or_obj=dict):
        ...
        for data_key, data_value in self.data.iteritems():
            if isinstance(data_key, type):
                warn(('{data_key} is a class used as a key, but it wont match keys which are instances of this type').format(data_key=data_key))
            try:
            ...
            if not bool(match_dict_or_True) is True:
    ...
    def __eq__(self, other):
        ...
        if self.DO_TYPECHECK and type(self.data) != type(other):
            ...
            if isinstance(self.data, WrapObj):
                return self.data == other
            if isinstance(self.data, WrapMultiObj):
                return self.data == [other]
            if isinstance(self.data, types.StringTypes):
                return self.data == other
            if isinstance(self.data, Iterable) and not isinstance(self.data, Mapping) and isinstance(other, Iterable) and not isinstance(other, Mapping):
                return self.match_list(other)
            if not self.treat_as_object and isinstance(self.data, Mapping) and isinstance(other, Mapping):
                return self.match_dict_or_obj(other, dict)
            if self.treat_as_object:
                try:
                    assert isinstance(self.data, Mapping)
                except AssertionError:
                    raise TypeError(('data={data} should be of type Mapping (e.g. dict)').format(data=self.data))
            ...
            if check_as_value_and_type(other, self.data):
...
class WrapMultiObj(object):
    ...
    def __init__(self, data, range_low, range_high):
        ...
        if WrapObj([INT_OR_INF, INT_OR_INF]) == [range_low, range_high]:
            ...
            raise TypeError(('range_low={range_low} and range_high={range_high} must be of appropiate type').format(range_low=range_low, range_high=range_high))
        logging.debug('OK')
        self.repeat_allowed_range = [
        ...
        if self.DO_TYPECHECK and type(self.data) != type(other):
            ...
            while True:
                ...
                if repeat_count > self.repeat_allowed_range[1]:
                    logging.debug(('Checking not yet complete, but max_allowed_repeated={max_repeat} crossed. So, returning False.').format(max_repeat=self.repeat_allowed_range[1]))
                    return False
                        ...
                        if match_dict_or_True and isinstance(match_dict_or_True, dict):
                            for match_key in match_dict_or_True:
                                ...
                                if isinstance(prev_match_val, list):
                                    ...
                                    self.dict_saved_values[match_key] = [
                                     match_val]
