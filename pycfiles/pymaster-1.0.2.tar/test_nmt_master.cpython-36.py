...
# [GCC 8.4.0]
# Embedded file name: /home/alonso/Science/Codes/ReformCodes/NaMaster/test/test_nmt_master.py
# Compiled at: 2020-03-08 06:44:17
# Size of source mod 2**32: 34251 bytes
...
class TestWorkspaceCAR(unittest.TestCase):
    ...
    def mastest(self):
        f0 = nmt.NmtField((self.msk), [self.mps[0]], wcs=(self.wcs),
          n_iter=0)
        f2 = nmt.NmtField((self.msk), [self.mps[1], self.mps[2]], wcs=(self.wcs),
          n_iter=0)
        f = [f0, f2]
        for ip1 in range(2):
    ...
    def test_workspace_car_methods(self):
        ...
        self.assertEqual(c.shape, (1, self.b.bin.n_bands))
        with self.assertRaises(ValueError):
            w.decouple_cell((self.n_good), cl_bias=(self.n_good), cl_noise=(self.n_bad))
        with self.assertRaises(ValueError):
            w.decouple_cell((self.n_good), cl_bias=(self.n_good), cl_noise=(self.n_half))
        with self.assertRaises(ValueError):
            w.decouple_cell((self.n_good), cl_bias=(self.n_bad), cl_noise=(self.n_good))
        with self.assertRaises(ValueError):
            w.decouple_cell((self.n_good), cl_bias=(self.n_half), cl_noise=(self.n_good))
...
class TestWorkspaceHPX(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mps = np.array(hp.read_map('test/benchmarks/mps.fits', verbose=False,
          field=[
         0, 1, 2]))
        self.tmp = np.array(hp.read_map('test/benchmarks/tmp.fits', verbose=False,
          field=[
         0, 1, 2]))
        ...
        self.b_half = nmt.NmtBin.from_nside_linear(self.nside // 2, self.nlb)
        self.b_doub = nmt.NmtBin.from_nside_linear(2 * self.nside, self.nlb)
    ...
    def mastest(self, wtemp, wpure, do_teb=False):
        ...
        if wtemp:
            prefix += '_yc'
            f0 = nmt.NmtField((self.msk), [self.mps[0]], templates=[
             [
              self.tmp[0]]])
            f2 = nmt.NmtField((self.msk), [self.mps[1], self.mps[2]], templates=[
             [
        ...
        for ip1 in range(2):
            for ip2 in range(ip1, 2):
                ...
                if wtemp:
                    ...
                    self.assertTrue((np.fabs(dlb - tlb) <= np.fmin(np.fabs(dlb), np.fabs(tlb)) * 1e-05).all())
                    clb += dlb
                cl = w.decouple_cell((nmt.compute_coupled_cell(f[ip1], f[ip2])),
                  cl_bias=clb)
                tl = np.loadtxt((prefix + '_c%d%d.txt' % (2 * ip1, 2 * ip2)), unpack=True)[1:, :]
                self.assertTrue((np.fabs(cl - tl) <= np.fmin(np.fabs(cl), np.fabs(tl)) * 1e-05).all())
    ...
    def test_workspace_shorten(self):
        w = nmt.NmtWorkspace()
        w.read_from('test/benchmarks/bm_yc_yp_w02.fits')
        lmax = w.wsp.lmax
        larr = np.arange(lmax + 1)
        larr_long = np.arange(2 * lmax + 1)
        cls = 100.0 / (larr + 10.0)
        cls_long = 100.0 / (larr_long + 10.0)
    ...
    def test_workspace_rebeam(self):
        ...
        b = np.ones(lmax + 1) * 2.0
        w.update_beams(b, b)
        b2 = np.ones(lmax // 2 + 1) * 2.0
        with self.assertRaises(ValueError):
            w.update_beams(b, b2)
        b2 = 1.0
        with self.assertRaises(ValueError):
    ...
    def test_workspace_full_master(self):
        ...
        w.compute_coupling_matrix(self.f0, self.f0, self.b)
        c = nmt.compute_full_master((self.f0), (self.f0), (self.b), workspace=w)
        self.assertEqual(c.shape, (1, self.b.bin.n_bands))
        with self.assertRaises(RuntimeError):
            nmt.compute_full_master((self.f0_half), (self.f0_half), (self.b_half),
              workspace=w)
        with self.assertRaises(ValueError):
            nmt.compute_full_master((self.f0), (self.f0), (self.b), cl_noise=(self.n_bad))
        with self.assertRaises(ValueError):
            nmt.compute_full_master((self.f0), (self.f0), (self.b), cl_guess=(self.n_bad))
        with self.assertRaises(RuntimeError):
            nmt.compute_full_master((self.f0), (self.f0), (self.b), cl_noise=(self.n_half))
        with self.assertRaises(RuntimeError):
            nmt.compute_full_master((self.f0), (self.f0), (self.b), cl_guess=(self.n_half))
...
class TestWorkspaceFsk(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.f0 = nmt.NmtFieldFlat(self.lx, self.ly, self.msk, [
         self.mps[0]])
        self.f0_half = nmt.NmtFieldFlat(self.lx, self.ly, self.msk[:self.ny // 2,
         :self.nx // 2], [
         self.mps[0,
          :self.ny // 2,
          :self.nx // 2]])
    ...
    def mastest(self, wtemp, wpure, do_teb=False):
        ...
        for ip1 in range(2):
            for ip2 in range(ip1, 2):
                ...
                if wtemp:
                    dlb = nmt.deprojection_bias_flat(f[ip1], f[ip2], self.b, self.ll, clth + nlth)
                    tlb = np.loadtxt((prefix + '_cb%d%d.txt' % (2 * ip1, 2 * ip2)), unpack=True)[1:, :]
                    self.assertTrue((np.fabs(dlb - tlb) <= np.fmin(np.fabs(dlb), np.fabs(tlb)) * 1e-05).all())
                    clb += dlb
                cl = w.decouple_cell((nmt.compute_coupled_cell_flat(f[ip1], f[ip2], self.b)),
                  cl_bias=clb)
                tl = np.loadtxt((prefix + '_c%d%d.txt' % (2 * ip1, 2 * ip2)), unpack=True)[1:, :]
                self.assertTrue((np.fabs(cl - tl) <= np.fmin(np.fabs(cl), np.fabs(tl)) * 1e-05).all())
    ...
    def test_workspace_flat_io(self):
        ...
        with self.assertRaises(RuntimeError):
            w.write_to('test/wspc.fits')
        w.read_from('test/benchmarks/bm_f_yc_yp_w02.fits')
    ...
    def test_workspace_flat_full_master(self):
        ...
        c = nmt.compute_full_master_flat((self.f0), (self.f0), (self.b), workspace=w)
        self.assertEqual(c.shape, (1, self.b.bin.n_bands))
        with self.assertRaises(RuntimeError):
            nmt.compute_full_master_flat((self.f0), (self.f0), (self.b_half), workspace=w)
        with self.assertRaises(RuntimeError):
            nmt.compute_full_master_flat((self.f0), (self.f0), (self.b), workspace=w_half)
        with self.assertRaises(ValueError):
            nmt.compute_full_master_flat((self.f0), (self.f0), (self.b), cl_noise=(self.nb_bad),
              workspace=w)
        with self.assertRaises(ValueError):
            nmt.compute_full_master_flat((self.f0), (self.f0), (self.b), cl_noise=(self.n_good),
              workspace=w)
        with self.assertRaises(ValueError):
            nmt.compute_full_master_flat((self.f0), (self.f0), (self.b), cl_noise=(self.nb_good),
              cl_guess=(self.n_good),
              workspace=w)
        with self.assertRaises(ValueError):
            nmt.compute_full_master_flat((self.f0), (self.f0), (self.b), cl_noise=(self.nb_good),
              cl_guess=(self.n_bad),
    ...
    def test_workspace_flat_deprojection_bias(self):
        c = nmt.deprojection_bias_flat(self.f0, self.f0, self.b, self.ll, self.n_good)
        self.assertEqual(c.shape, (1, self.b.bin.n_bands))
        with self.assertRaises(ValueError):
            nmt.deprojection_bias_flat(self.f0, self.f0, self.b, self.ll, self.n_bad)
        with self.assertRaises(ValueError):
            nmt.deprojection_bias_flat(self.f0, self.f0, self.b, self.ll, self.nb_good)
        with self.assertRaises(RuntimeError):
            nmt.deprojection_bias_flat(self.f0, self.f0_half, self.b, self.ll, self.n_good)
