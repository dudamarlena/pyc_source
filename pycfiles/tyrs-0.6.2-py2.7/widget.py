...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/tyrs/widget.py
# Compiled at: 2011-12-19 10:43:03
import re, tyrs, time, urwid
...
class HeaderWidget(urwid.WidgetWrap):
    ...
    def set_activities(self):
        buffers = ('home', 'mentions', 'direct', 'search', 'user', 'favorite', 'thread',
                   'user_retweet')
        display = {'home': 'H', 
           'mentions': 'M', 'direct': 'D', 'search': 'S', 
           'user': 'U', 'favorite': 'F', 'thread': 'T', 
           'user_retweet': 'R'}
        buff_widget = []
        for b in buffers:
            if b == self.buffer:
...
class StatusWidget(urwid.WidgetWrap):
    ...
    def __init__(self, id, status):
        ...
        self.id = id
        self.status = status
        status_content = urwid.Padding(urwid.AttrWrap(urwid.Text(self.get_text(status)), 'body'), left=1, right=1)
    ...
    def get_text(self, status):
        ...
        if status.rt:
            text = text.split(':')[1:]
            text = (':').join(text)
        if hasattr(status, 'retweeted_status') and hasattr(status.retweeted_status, 'text'):
    ...
    def get_header(self, status):
        ...
        header = unicode(header_template).format(time=timer, nick=nick, reply=reply, retweeted=retweeted, source=source, retweet_count=retweet_count, retweeter=retweeter)
...
class TitleLineBox(urwid.WidgetDecoration, urwid.WidgetWrap):
    ...
    def __init__(self, original_widget, title=''):
        ...
        middle = urwid.Columns([('fixed', 1, lline),
         original_widget, ('fixed', 1, rline)], box_columns=[0, 2], focus_column=1)
