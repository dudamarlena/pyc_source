...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/tyrs/tweets.py
# Compiled at: 2011-11-25 04:17:21
import os, sys, tyrs, urwid, logging, urllib2, oauth2 as oauth
...
class Tweets(object):
    ...
    def authentication(self):
        ...
        self.api = ApiPatch(self.conf.token[self.conf.service]['consumer_key'], self.conf.token[self.conf.service]['consumer_secret'], self.conf.oauth_token, self.conf.oauth_token_secret, base_url=url, proxy=proxify)
        self.set_myself()
    ...
    def get_proxy(self):
        ...
        if proxy:
            return {'http': ('http://{0}').format(proxy), 
    ...
    def load_user_public_timeline(self, page=None):
        if self.search_user:
            return self.api.GetUserTimeline(self.search_user, include_rts=True, page=page)
        else:
            return []
...
class ApiPatch(Api):
    ...
    def __init__(self, consumer_key=None, consumer_secret=None, access_token_key=None, access_token_secret=None, input_encoding=None, request_headers=None, cache=DEFAULT_CACHE, shortner=None, base_url=None, use_gzip_compression=False, debugHTTP=False, proxy={}):
        self.SetCache(cache)
