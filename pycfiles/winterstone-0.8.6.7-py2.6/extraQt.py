...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/winterstone/extraQt.py
...
class WinterLine(QLineEdit):
    ...
    def _newchar(self):
        ...
        if self.checkLine():
            ...
            self.decor = 'none'
            self.dlock = True
        self.setStyleSheet('QWidget { font: bold; color: %s; text-decoration: %s;}' % (self.color.name(), self.decor))
        self.onChange()
...
class WinterEditor(QWidget, WinterObject):
    ...
    def __init__(self, parent, filename='', lexer='Python'):
        ...
        if QSCI_SUPPORT:
            ...
            self.lexers = {'Bash': QsciLexerBash, 'Batch': QsciLexerBatch, 'CMake': QsciLexerCMake, 'CPP': QsciLexerCPP, 'CSS': QsciLexerCSS, 
               'D': QsciLexerD, 'Diff': QsciLexerDiff, 'Fortran': QsciLexerFortran77, 'HTML': QsciLexerHTML, 
               'Lua': QsciLexerLua, 'Make': QsciLexerMakefile, 'Pascal': QsciLexerPascal, 
               'Perl': QsciLexerPerl, 
               'PostScript': QsciLexerPostScript, 'POV': QsciLexerPOV, 'Properties': QsciLexerProperties, 
               'Python': QsciLexerPython, 'Ruby': QsciLexerRuby, 'SQL': QsciLexerSQL, 
               'TCL': QsciLexerTCL, 'TeX': QsciLexerTeX, 'VHDL': QsciLexerVHDL, 
               'YAML': QsciLexerYAML, 'Plain': QsciLexerPython}
