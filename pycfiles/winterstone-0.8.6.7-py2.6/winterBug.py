...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/winterstone/winterBug.py
...
class WinterQtDebug(QDockWidget):
    ...
    def info(self, msg):
        ...
        """
        self.debugList.addItem(self.makeMessage(msg, self.config.options.info_bg_color, 'ok', timestamp=True, fgcolor=self.config.options.info_fg_color))
    ...
    def error(self, msg, obj=''):
        ...
        if not obj:
            item = self.makeMessage(vmsg, self.config.options.error_bg_color, 'error', timestamp=True, fgcolor=self.config.options.error_fg_color)
            item2 = self.makeMessage(msg, self.config.options.error_bg_color, 'error', timestamp=True, fgcolor=self.config.options.error_fg_color)
        else:
            item = self.makeMessage('%s::%s' % (obj, vmsg), self.config.options.erorr_bg_color, 'error', timestamp=True, fgcolor=self.config.options.error_fg_color)
            item2 = self.makeMessage('%s::%s' % (obj, msg), self.config.options.error_bg_color, 'error', timestamp=True, fgcolor=self.config.options.error_fg_color)
        item.e = exc
    ...
    def debug(self, msg):
        ...
        """
        self.debugList.addItem(self.makeMessage(msg, self.config.options.debug_bg_color, 'warning', timestamp=True, fgcolor=self.config.options.debug_fg_color))