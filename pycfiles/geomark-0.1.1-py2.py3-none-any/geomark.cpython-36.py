...
# [GCC 8.4.0]
# Embedded file name: /mnt/NTFS_D/Documents/PyCharmProjects/python-geomark/geomark/geomark.py
# Compiled at: 2017-12-24 08:56:37
# Size of source mod 2**32: 8354 bytes
import os, requests
...
class Geomark:
    ...
    def __init__(self, geomarkId=None, geomarkUrl=None, config=_config):
        ...
        if not geomarkId:
            ...
            if geomarkId:
                self.geomarkUrl = self.config.GEOMARK_ID_BASE_URL.format(protocol=(config.PROTOCOL),
                  geomarkId=geomarkId)
                self.geomarkId = geomarkId
            else:
                self.geomarkId, self.geomarkUrl, self.config.PROTOCOL = self._parse_geomark_url(geomarkUrl)
        self.logger.info('Initiated Geomark object with the following parameters: geomarkId={geomarkId}; geomarkUrl={geomarkUrl}; protocol={protocol}; custom_config: {custom_config}'.format(geomarkId=geomarkId,
    ...
    def boundingBox(self, fileFormatExtension='json', srid=None):
        url = self.geomarkUrl + '/boundingBox.{fileFormatExtension}'.format(fileFormatExtension=fileFormatExtension)
    ...
    def feature(self, fileFormatExtension='json', srid=None):
        url = self.geomarkUrl + '/feature.{fileFormatExtension}'.format(fileFormatExtension=fileFormatExtension)
        return self._handle_get(requests.get(url, params=({'srid': srid} if srid else None)))

    def info(self, fileFormatExtension='json', srid=None):
        url = self.geomarkUrl + '.{fileFormatExtension}?'.format(fileFormatExtension=fileFormatExtension)
        return self._handle_get(requests.get(url, params=({'srid': srid} if srid else None)))

    def parts(self, fileFormatExtension='json', srid=None):
        url = self.geomarkUrl + '/parts.{fileFormatExtension}'.format(fileFormatExtension=fileFormatExtension)
        return self._handle_get(requests.get(url, params=({'srid': srid} if srid else None)))

    def point(self, fileFormatExtension='json', srid=None):
        url = self.geomarkUrl + '/point.{fileFormatExtension}'.format(fileFormatExtension=fileFormatExtension)
        return self._handle_get(requests.get(url, params=({'srid': srid} if srid else None)))

    def copy(self, geomarkUrl=None, resultFormat='json', allowOverlap=False, bufferMetres=None, bufferJoin=None, bufferCap=None, bufferMitreLimit=None, bufferSegments=None):
        """
    ...
    @staticmethod
    def create(format=None, srid=4326, resultFormat='json', multiple=False, allowOverlap=False, bufferMetres=None, bufferJoin=None, bufferCap=None, bufferMitreLimit=None, bufferSegments=None, body=None, extra_kwargs={}):
        """
