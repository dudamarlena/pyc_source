...
# Embedded file name: /usr/lib/python2.7/site-packages/agn_periodics/utils.py
# Compiled at: 2014-10-08 08:51:05
__author__ = 'yarnaid'
import json, logging, os, numpy as np, pyfftw, copy, mlpy.wavelet as wave
...
def get_scalogram_final(data_row):
    ...
    errors = np.abs(np.random.normal(scalogr.mean(), scalogr.std(), scalogr.shape)) * conf['scalogram_noise_level']
    errors[errors < 1] = 1
...
def make_residual_bank(data_row, window_len=None):
    ...
    if window_len is None:
        window_len = min(conf['sf_models_smooth_window'], len(data_storage[0]) / 2.0)
