...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/WS2801_RPI.py
...
import spidev as __spidev, logging as __logging, time
...
__gamma = [
 255] * 256
__string_type_error_1 = 'Attribute pixels must be an int addressing one of the available LEDs or a list of ints'
__string_type_error_2 = 'Attribute rgb_values must be an list containing 3 ints between 0 and 255 or a list of lists with rgb values'
...
__string_runtime_error_3 = 'SPI Bus must be driven with clock speed > 1500HZ for WS2801'
__string_runtime_error_4 = 'Mode must be between 0 and 3.'
__string_warning_1 = 'more leds addressed than rgb values given: assume last rgb for remaining leds'
__string_warning_3 = 'Your dict contains floats as keys. These are not taken into account when adressing LEDs.'
__string_warning_2 = 'too many rgb values supplied: skipping'
try:
...
def set_spidev_bus_device(bus=0, device=0):
    ...
    global __BUS
    global __DEVICE
    if type(bus) is not int or type(device) is not int or bus < 0 or bus > 1 or device < 0 or device > 1:
...
def hello():
    ...
    for j in range(3):
        set_led_colors_buffer_list_multi_call([ i + 1 for i in range(__NUMBER_LEDS) ], [
         [
...
def get_led_colors_buffer_dict():
    ...
    for index in range(0, __NUMBER_LEDS * 3, 3):
        rgb_dict = {'red': __rgb_leds[index], 'green': __rgb_leds[(index + 1)], 
...
def set_led_colors_buffer_dict_multi_call(leds_dict):
    ...
    for led, rgb_dict in leds_dict.items():
        ...
        if type(led) is int:
            ...
            if type(rgb_dict) is not dict:
                raise TypeError(__string_type_error_4)
            for color, rgb_value in rgb_dict.items():
                if color == 'red':
                    if type(rgb_value) is not int or rgb_value < 0 or rgb_value > 255:
...
def get_led_colors_buffer_list():
    ...
    return [ [__rgb_leds[i], __rgb_leds[(i + 1)], __rgb_leds[(i + 2)]] for i in range(0, 3 * __NUMBER_LEDS, 3)
           ]


def set_led_colors_buffer_list_multi_call(pixels, rgb_values=[
 255, 255, 255]):
