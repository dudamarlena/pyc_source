...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/indra/ipc/siesta_test.py
...
class TestBase:
    ...
    def test_basic_get(self):
        ...
        self.assertEquals(req.get_response(self.server).body, llsd.format_xml(None))
        return

    def test_bad_method(self):
        req = siesta.Request.blank('/')
        req.environ['REQUEST_METHOD'] = 'UNDERPANTS'
        self.assertEquals(req.get_response(self.server).status_int, exc.HTTPMethodNotAllowed.code)

    json_safe = {'none': None, 'bool_true': True, 'bool_false': False, 'int_zero': 0, 'int_max': 2147483647, 'int_min': -2147483648, 'long_zero': 0, 'long_max': 2147483647, 'long_min': -2147483648, 'float_zero': 0, 'float': math.pi, 'float_huge': 3.1415926535897933e+299, 'str_empty': '', 'str': 'foo', 'unicṑde_empty': '', 'unicṑde': 'Ṏxx၈0'}
    json_safe['array'] = json_safe.values()
    json_safe['tuple'] = tuple(json_safe.values())
    json_safe['dict'] = json_safe.copy()
    json_unsafe = {'uuid_empty': lluuid.UUID(), 'uuid_full': lluuid.UUID('dc61ab0530200d7554d23510559102c1a98aab1b'), 'binary_empty': llsd.binary(), 'binary': llsd.binary(b'f\x00\xff'), 'uri_empty': llsd.uri(), 'uri': llsd.uri('http://www.secondlife.com/'), 'datetime_empty': datetime.datetime(1970, 1, 1), 'datetime': datetime.datetime(1999, 9, 9, 9, 9, 9)}
    ...
    def test_client_content_type_llsd_xml_good(self):
        self._test_client_content_type_good('application/llsd+xml', self.json_unsafe)

    def test_client_content_type_llsd_notation_good(self):
        self._test_client_content_type_good('application/llsd+notation', self.json_unsafe)

    def test_client_content_type_llsd_binary_good(self):
        self._test_client_content_type_good('application/llsd+binary', self.json_unsafe)

    def test_client_content_type_xml_good(self):
        self._test_client_content_type_good('application/xml', self.json_unsafe)

    def _test_client_content_type_bad(self, content_type):
        req = siesta.Request.blank('/')
        req.environ['REQUEST_METHOD'] = 'POST'
        req.body = '\x00invalid nonsense under all encodings'
        req.content_type = content_type
        self.assertEquals(req.get_response(self.server).status_int, exc.HTTPBadRequest.code)
    ...
    def test_client_content_type_bad(self):
        ...
        self.assertEquals(req.get_response(self.server).status_int, exc.HTTPUnsupportedMediaType.code)
    ...
    def test_request_default_accept(self):
        ...
        self.assertEquals(str(req.accept).replace(' ', ''), req.default_accept.replace(' ', ''))

    def test_request_llsd_auto_body(self):
        req = siesta.Request.blank('/')
        req.llsd = {'a': 2}
        self.assertEquals(req.body, '<?xml version="1.0" ?><llsd><map><key>a</key><integer>2</integer></map></llsd>')
