...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/indra/ipc/siesta.py
# Compiled at: 2008-07-28 17:15:44
from indra.base import llsd
from webob import exc
import webob, re, socket
try:
...
llsd_parsers = {'application/json': json_decode, 'application/llsd+binary': llsd.parse_binary, 'application/llsd+notation': llsd.parse_notation, 'application/llsd+xml': llsd.parse_xml, 'application/xml': llsd.parse_xml}
...
class BodyLLSD(object):
        ...
        try:
            ...
            if not self.body:
                ...
                try:
                    ...
                    raise exc.HTTPUnsupportedMediaType('Content type %s not supported' % mtype).exception

                try:
                    self._llsd = parser(self.body)
                except (llsd.LLSDParseError, JsonDecodeError, TypeError), err:
                    raise exc.HTTPBadRequest('Could not parse body: %r' % err.args).exception
    ...
    def _llsd__set(self, val):
        ...
        if req is not None:
            ...
            (formatter, ctype) = formatter_for_mime_type(mime_type(self.content_type))
...
class Response(webob.Response, BodyLLSD):
    ...
    body = property(webob.Response._body__get, _body__set, webob.Response._body__del, webob.Response._body__get.__doc__)
...
class Request(webob.Request, BodyLLSD):
    ...
    __module__ = __name__
    default_content_type = 'application/llsd+xml'
    default_accept = 'application/llsd+xml; q=0.5, application/llsd+notation; q=0.3, application/llsd+binary; q=0.2, application/xml; q=0.1, application/json; q=0.0'
    ...
    body = property(webob.Request._body__get, _body__set, webob.Request._body__del, webob.Request._body__get.__doc__)

    def create_response(self, llsd=None, status='200 OK', conditional_response=webob.NoDefault):
        resp = self.ResponseClass(status=status, request=self, conditional_response=conditional_response)
        resp.llsd = llsd
        return resp
    ...
    def curl(self):
        ...
        if self.headers:
            c.setopt(pycurl.HTTPHEADER, [ '%s: %s' % (k, self.headers[k]) for k in self.headers ])
...
llsd_formatters = {'application/json': json_encode, 'application/llsd+binary': llsd.format_binary, 'application/llsd+notation': llsd.format_notation, 'application/llsd+xml': llsd.format_xml, 'application/xml': llsd.format_xml}
...
def formatter_for_mime_type(mime_type):
    ...
    try:
        return (
         llsd_formatters[mime_type], mime_type)
    except KeyError:
        raise exc.HTTPInternalServerError('Could not use MIME type %r to format response' % mime_type).exception
    ...
    try:
        ...
        if not isinstance(resp, webob.Response):
            try:
                ...
                resp = exc.HTTPInternalServerError(detail='Could not format response')
...
def llsd_class(cls):
    ...
    def foo(req, **kwargs):
        ...
        try:
            ...
            allowed = [ m for m in http11_methods if hasattr(instance, 'handle_' + m.lower()) ]
            raise exc.HTTPMethodNotAllowed(headers={'Allowed': (', ').join(allowed)}).exception
...
predefined_regexps = {'uuid': '[a-f0-9][a-f0-9-]{31,35}', 'int': '\\d+'}
...
class Router(object):
    ...
    def add(self, route, app, methods=None):
        ...
        self.routes.append((re.compile(compile_route(route)), app, methods and dict.fromkeys(methods)))
    ...
    def __call__(self, environ, start_response):
        ...
        if allowed:
            ...
            allowed.sort()
            resp = exc.HTTPMethodNotAllowed(headers={'Allowed': (', ').join(allowed)})
        else:
