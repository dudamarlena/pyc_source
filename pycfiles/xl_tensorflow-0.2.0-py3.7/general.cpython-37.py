...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xl_tensorflow\metrics\general.py
# Compiled at: 2020-03-26 04:53:41
...
def recall_precision_f1(y_predict, y_true, class_num, class2id=None):
    ...
    result = pd.DataFrame(result) if not id2class else pd.DataFrame(result, index=[id2class[i] for i in range(class_num)])
    return result
...
def topk_caculate(predict_p, true_labels, k=1, number_cat=None, epsilon=1e-19):
    ...
    prev_tp_sum = 0
    prev_cat_tp = [0 for i in range(number_cat)]
    real_cat_positive = [sum(real == i) for i in range(number_cat)]
    for i in range(k):
        pred = arg_sort[:, -1 * (i + 1)]
        topk, cat_precition, prev_cat_tp, prev_tp_sum = acc_caculate(pred, real, prev_tp_sum, prev_cat_tp, real_cat_positive, number_cat, total)
