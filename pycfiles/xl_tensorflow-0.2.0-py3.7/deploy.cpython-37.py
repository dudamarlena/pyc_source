...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xl_tensorflow\utils\deploy.py
# Compiled at: 2020-04-17 01:39:54
...
def tf_saved_model_to_lite(model_path, save_lite_file, input_shape=None, quantize_method=None, allow_custom_ops=False):
    ...
    if quantize_method:
        converter = quantize_model(converter, quantize_method, (*(100, ), *input_shape[1:]) if input_shape else (100,
                                                                                                                 224,
                                                                                                                 224,
                                                                                                                 3))
    if allow_custom_ops:
...
def serving_model_export(model, path, version=1, auto_incre_version=True):
    ...
    if auto_incre_version is True:
        old_version = [int(i) for i in os.listdir(path) if i[0] in '0123456789' if os.path.isdir(path + '/' + i)]
        if old_version:
...
def b64_image_model_wrapper(model, target_size, method=tf.image.ResizeMethod.BILINEAR, mean=0.0, std=255.0, input_name='b64_image', outs_signature='b64_output_tensor'):
    ...
    ouput_tensor = tf.keras.layers.Lambda(lambda img: tf.map_fn((lambda im: preprocess_and_decode(im[0])), img, dtype='float32'))(input64)
...
def tflite_evaluate(lite_model_path, keras_data_generater, mul_thread=None):
    ...
    if mul_thread:
        ...
        for i in range(mul_thread):
            ...
            threads[i] = MyThread(target=lite_interpreter, args=(
             keras_data_generater, start, end))
