...
# Embedded file name: build\bdist.win32\egg\IDE\CCodeEditor_FC.py
# Compiled at: 2020-01-15 23:36:16
import wx, wx.xrc, wx.stc as stc
from CLogText import LogText

class Panel_Code_FC(wx.Panel):
    ...
    def __init__(self, parent, hanndel):
        ...
        self.m_textCtrl_Description = wx.TextCtrl(self, wx.ID_ANY, wx.EmptyString, wx.DefaultPosition, wx.DefaultSize, 0)
        bzfoo.Add(self.m_textCtrl_Description, 1, wx.ALL, 5)
        bz.Add(bzfoo, 0, wx.EXPAND, 5)
        bzReturn = wx.BoxSizer(wx.HORIZONTAL)
        m_staticText21 = wx.StaticText(self, wx.ID_ANY, 'return type', wx.DefaultPosition, wx.DefaultSize, 0)
        m_staticText21.Wrap(-1)
        bzReturn.Add(m_staticText21, 0, wx.ALL, 5)
        self.m_textCtrl_ReturnType = wx.TextCtrl(self, wx.ID_ANY, wx.EmptyString, wx.DefaultPosition, wx.DefaultSize, 0)
        bzReturn.Add(self.m_textCtrl_ReturnType, 0, wx.ALL, 5)
        m_staticText5 = wx.StaticText(self, wx.ID_ANY, 'cmmt', wx.DefaultPosition, wx.DefaultSize, 0)
        m_staticText5.Wrap(-1)
        bzReturn.Add(m_staticText5, 0, wx.ALL, 5)
        self.m_textCtrl_ReturnCmmt = wx.TextCtrl(self, wx.ID_ANY, wx.EmptyString, wx.DefaultPosition, wx.DefaultSize, 0)
        bzReturn.Add(self.m_textCtrl_ReturnCmmt, 1, wx.ALL, 5)
        bz.Add(bzReturn, 0, wx.EXPAND, 5)
    ...
    def savedata(self):
        ...
        lt.append('RETURN')
        lt.append('')
        if self.m_textCtrl_ReturnType == '':
            lt.append('void')
        else:
            lt.append(self.m_textCtrl_ReturnType.GetValue())
        lt.append('')
        lt.append(self.m_textCtrl_ReturnCmmt.GetValue())
        data['Declare'].append(lt)
    ...
    def renderdata(self, data):
        ...
        for i in xrange(len(data['Declare'])):
            if data['Declare'][i][0] != 'RETURN':
                index = self.list.InsertItem(self.list.GetItemCount(), data['Declare'][i][0])
                for j in range(1, min(len(data['Declare'][i]), 5)):
        ...
        if return_idx < 0:
            self.m_textCtrl_ReturnType.SetValue('')
            self.m_textCtrl_ReturnCmmt.SetValue('')
        else:
            self.m_textCtrl_ReturnType.SetValue(data['Declare'][return_idx][2])
            self.m_textCtrl_ReturnCmmt.SetValue(data['Declare'][return_idx][4])
