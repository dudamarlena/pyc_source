...
# [GCC 8.4.0]
# Embedded file name: /home/forest/Development/gnome-workcycle-applet/workcycle/__init__.py
# Compiled at: 2010-11-11 12:10:41
import gtk, gnomeapplet, gobject, datetime, time, pynotify, gconf
...
class WorkcycleApplet(gnomeapplet.Applet):
    ...
    def update_state(self):
        if not (self.start_time is None and self.stop_time is None):
            ...
            if current_time < self.stop_time:
                if self.is_worktime:
                    total = datetime.timedelta(minutes=self.worktime)
                else:
                    total = datetime.timedelta(minutes=self.funtime)
    ...
    def _create_menu(self):
        ...
        verbs = [
         (
          'About', self.show_about), ('Preferences', self.show_preferences), ('Stop', self.stop_timer)]
    ...
    def start_timer(self):
        ...
        if self.is_worktime:
            ...
            self.stop_time = self.start_time + datetime.timedelta(minutes=self.worktime)
            self.wt_notify.show()
        else:
            self.display_icon.set_from_stock(gtk.STOCK_MEDIA_PAUSE, gtk.ICON_SIZE_SMALL_TOOLBAR)
            self.display_label.set_text('Funtime:')
            self.stop_time = self.start_time + datetime.timedelta(minutes=self.funtime)
            self.ft_notify.show()
        return
