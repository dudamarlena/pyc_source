...
# [GCC 8.4.0]
# Embedded file name: C:\Users\victim\Desktop\Responder3\responder3\servers\SOCKS5.py
    ...
    async def run(self):
        while not self.shutdown_evt.is_set():
            ...
            if isinstance(result[0], R3ConnectionClosed):
                ...
                if self.session.current_state == SOCKS5ServerState.NOT_AUTHENTICATED:
            ...
            if self.session.current_state == SOCKS5ServerState.REQUEST:
                ...
                if msg.CMD == SOCKS5Command.CONNECT:
                    if self.session.proxymode == SOCKS5ServerMode.OFF:
                        return
                    if self.session.proxymode == SOCKS5ServerMode.NORMAL:
