...
# [GCC 8.4.0]
# Embedded file name: C:\Users\victim\Desktop\Responder3\responder3\core\responder3.py
# Compiled at: 2019-04-29 07:33:30
# Size of source mod 2**32: 15957 bytes
import os, sys, copy, json, logging, argparse, datetime, asyncio, itertools, importlib.machinery, importlib.util, warnings
...
class Responder3:
        ...
        if args.list_interfaces == True:
            ...
            if args.config is not None:
                ...
                if args.python_config is not None:
                    ...
                    if args.environ_config is not None:
                        responder.config = Responder3Config.from_os_env()
                    else:
                        raise Exception('No suitable configuration method was supplied!Use either -e or -c or -p')
        return responder
    ...
    async def start_server_task(self, serverconfig):
        ...
        ste.task = Responder3ServerTask(log_queue=(self.log_queue),
    ...
    async def create_server(self, cmd):
        print('create_server')
        server_id = await self.start_server_task(cmd.server_config)
    ...
    async def start_manager(self):
        ...
        if not self.config.manager_settings:
            return
        elif self.config.manager_settings['mode'] == 'CLIENT':
            ...
            asyncio.ensure_future(self.handle_remote_commands())
            self.manager_task = Responder3ManagerClient(server_url, config, self.log_queue, self.manager_log_queue, self.manager_cmd_queue_in, self.manager_cmd_queue_out, self.manager_shutdown_evt, ssl_ctx)
            asyncio.ensure_future(self.manager_task.run())
        else:
            if self.config.manager_settings['mode'] == 'SERVER':
                self.manager_log_queue = None
                self.manager_task = None
    ...
    def get_serverconfigs(self):
        for serverentry in self.config.server_settings:
            if self.override_interfaces is None:
                ...
                portspecs = serverentry.get('bind_port', defaultports[serverentry['handler']] if serverentry['handler'] in defaultports else None)
                if portspecs is None:
                    raise Exception('For protocol %s the port must be supplied!' % (serverentry['handler'],))
                portspecs = isinstance(portspecs, list) or [
                 portspecs]
            for element in itertools.product(ifaces, portspecs):
                socket_configs = interfaces.get_socketconfig((element[0]),
                  (element[1][0]), (element[1][1]), ipversion=bind_family)
            ...
            if self.config.startup is not None:
                if 'mode' in self.config.startup:
                    if self.config.startup['mode'] == 'STANDARD':
                        ...
                        if self.config.startup['mode'] == 'TEST':
                            os.environ['PYTHONASYNCIODEBUG'] = '1'
                            os.environ['R3DEEPDEBUG'] = '1'
                            ...
                            if 'output_queue' not in self.config.log_settings['TEST']:
                                ...
                                if self.config.startup['mode'] == 'SERVICE':
...
class Responder3Config:
    ...
    def from_os_env():
        ...
        if config_file is None:
            raise Exception('Could not find configuration file path in os environment variables!Name to be set: %s' % Responder3Config.CONFIG_OS_KEY)
