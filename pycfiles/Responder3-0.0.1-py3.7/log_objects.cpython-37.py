...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\responder3\core\logging\log_objects.py
# Compiled at: 2019-08-15 18:27:11
# Size of source mod 2**32: 17200 bytes
import json, enum, datetime, time
...
class ProxyData:
    ...
    def to_dict(self):
        ...
        t = {}
        t['src_addr'] = [
         str(self.src_addr[0]), int(self.src_addr[1])]
...
class TrafficLog:
    ...
    def to_dict(self):
        ...
        for date in self.data_recv:
            t['data_recv'][int(date.timestamp() * 1000)] = self.data_recv[date].hex()

        for date in self.data_sent:
            t['data_sent'][int(date.timestamp() * 1000)] = self.data_sent[date].hex()
    ...
    def from_dict(d):
        ...
        for date in d['data_recv']:
            t.data_recv[datetime.datetime.fromtimestamp(int(date) / 1000)] = bytes.fromhex(d['data_recv'][date])

        for date in d['data_sent']:
            t.data_sent[datetime.datetime.fromtimestamp(int(date) / 1000)] = bytes.fromhex(d['data_sent'][date])
...
logobj2type = {LogObjectType.LOGENTRY: LogEntry, 
...
class UnifiedLog:
    ...
    def construct(log_obj):
        ...
        if isinstance(log_obj, ProxyData):
            ...
            if isinstance(log_obj, Credential):
                ul.credential = log_obj
            else:
                if isinstance(log_obj, PoisonResult):
                    ul.poisonresult = log_obj
                else:
                    if isinstance(log_obj, EmailEntry):
                        ul.email = log_obj
                    else:
                        if isinstance(log_obj, ConnectionOpened):
                            ul.connection_opened = log_obj
                        else:
                            if isinstance(log_obj, ConnectionClosed):
                                ul.connection_closed = log_obj
                            else:
                                if isinstance(log_obj, TrafficLog):
                                    ul.traffic = log_obj
                                return ul
