...
# [GCC 8.4.0]
# Embedded file name: C:\Users\victim\Desktop\Responder3\responder3\servers\FTP.py
...
class FTP(ResponderServer):
    ...
    async def run(self):
        await asyncio.wait_for((self.send_data(FTPReply(220, self.session.banner).to_bytes())),
          timeout=1)
                ...
                if self.session.current_state == FTPState.AUTHORIZATION:
                    if cmd.command in [FTPCommand.USER, FTPCommand.PASS, FTPCommand.QUIT]:
                        if cmd.command == FTPCommand.QUIT:
                            await asyncio.wait_for((self.send_data(FTPReply(200).to_bytes())),
                              timeout=1)
                            return
                            ...
                            if res == FTPAuthStatus.MORE_DATA_NEEDED:
                                await asyncio.wait_for((self.send_data(FTPReply(331).to_bytes())),
                                  timeout=1)
                                continue
                        else:
                            if res == FTPAuthStatus.NO:
                                await self.logger.credential(cred)
                                await asyncio.wait_for((self.send_data(FTPReply(530).to_bytes())),
                                  timeout=1)
                                return
                            if res == FTPAuthStatus.OK:
                                await asyncio.wait_for((self.send_data(FTPReply(200).to_bytes())),
                                  timeout=1)
                                self.session.current_state = FTPState.AUTHENTICATED
                                continue
                    else:
                        await asyncio.wait_for((self.send_data(FTPReply(503).to_bytes())),
                          timeout=1)
                        return
            else:
                if self.session.current_state == FTPState.AUTHENTICATED:
                    return
