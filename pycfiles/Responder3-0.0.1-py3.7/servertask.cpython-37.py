...
# [GCC 8.4.0]
# Embedded file name: C:\Users\victim\Desktop\Responder3\responder3\core\servertask.py
# Compiled at: 2019-04-29 07:33:30
# Size of source mod 2**32: 8219 bytes
import sys, socket, copy, importlib, importlib.util, asyncio, datetime
...
class Responder3ServerTask:
    ...
    async def accept_client(self, client_reader, client_writer):
        ...
        ct.handler = self.server_handler((self.server_name),
    ...
    async def create_server(self, server_task_config):
        ...
        if 'settings' in server_task_config:
            ...
            if 'handler' in server_task_config and server_task_config['handler'] is not None:
                server_module = Responder3ServerTask.import_template(server_task_config['handler'])
            ...
            if 'bind_sslctx' in server_task_config:
                ...
                if self.listener_socket_config.bind_protocol == socket.SOCK_STREAM:
                    sock = None
                    if getattr(self.server_handler, 'custom_socket', None) is not None and callable(getattr(self.server_handler, 'custom_socket', None)):
                    ...
                    if self.listener_socket_config.bind_protocol == socket.SOCK_DGRAM:
                        ...
                        sock = None
                        if getattr(self.server_handler, 'custom_socket', None) is not None:
                            if callable(getattr(self.server_handler, 'custom_socket', None)):
                                sock = self.server_handler.custom_socket(self.listener_socket_config)
