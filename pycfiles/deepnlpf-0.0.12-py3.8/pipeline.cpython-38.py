...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/deepnlpf/pipeline.py
# Compiled at: 2020-04-26 19:51:36
# Size of source mod 2**32: 18760 bytes
...
class Pipeline(object):
    ...
    def __init__(self, _input=None, pipeline=None, _output='terminal', _format='json', use_db=None, tool_base='stanza', boost='pathos', memory=None, cpus=None, gpus=None):
        ...
        self._memory = memory
        if cpus is None:
            self._cpus_count = psutil.cpu_count()
        else:
    ...
    def annotate(self):
        if self._use_db != None:
            ...
            self._documents = PluginManager().call_plugin_db(plugin_name=(self._use_db),
    ...
    def run(self, tool):
        ...
        for document in tqdm((self._documents), desc='Processing document(s)'):
            ...
            self.list_temp.append(self.output(data_formating, self._id_dataset, JSONEncoder().encode(document['name']).replace('"', ''), plugin_name))
        else:
    ...
    def ssplit(self, document):
        ...
        if self._tool_base == 'stanza':
            self._documents = PluginManager().call_plugin_nlp(plugin_name='stanza',
              _id_pool=(self._id_pool),
              lang=(self._custom_pipeline['lang']),
              document=document,
              pipeline=[
        ...
        if self._tool_base == 'stanfordcorenlp':
            self._documents = PluginManager().call_plugin_nlp(plugin_name='stanfordcorenlp',
              _id_pool=(self._id_pool),
              lang=(self._custom_pipeline['lang']),
              document=document,
              pipeline=['ssplit'])
        ...
        if self._use_db != None:
            dataset_document = {'name':names.get_first_name(), 
             'data_time':OutputFormat.data_time(self)}
            _id_dataset = PluginManager().call_plugin_db(plugin_name=(self._use_db),
              operation='insert',
              collection='dataset',
              document=dataset_document)
            document = {'_id_dataset':_id_dataset, 
        ...
        name = names.get_first_name()
        return (
         _id_dataset, [{'_id':_id,  '_id_dataset':_id_dataset,  'name':name,  'sentences':sentences}])
    ...
    def processing_path_dataset(self, path_dataset, ssplit=False):
        ...
        if os.path.isdir(path_dataset):
            if os.listdir(path_dataset) == []:
                ...
                if self._use_db != None:
                    dataset_document = {'name':dataset_name,  'data_time':datetime.datetime.now()}
                    _id_dataset = PluginManager().call_plugin_db(plugin_name=(self._use_db),
                      operation='insert',
                      collection='dataset',
                      document=dataset_document)
            ...
            if os.path.isfile(os.path.join(os.path.abspath(path_dataset), filename)):
            ...
            if subfolders:
                ...
                if files:
                    ...
                    for file_name in tqdm((os.listdir(path_dataset)), desc='Document(s) save'):
                        ...
                        if ssplit:
                        ...
                        elif self._use_db != None:
                            document_document = {'_id_dataset':_id_dataset,  'name':file_name, 
                             'sentences':text_raw}
                            PluginManager().call_plugin_db(plugin_name=(self._use_db),
                              operation='insert',
                              collection='document',
                            ...
                            document_document = {'_id':RandomObjectId.gen_random_object_id(self),  '_id_dataset':_id_dataset, 
                             'name':file_name, 
                             'sentences':text_raw}
                        ...
                        if self._use_db != None:
                            log_document = {'_id_dataset':_id_dataset,  'info':'Save Dataset.', 
                             'data':data, 
                             'data_time':datetime.datetime.now()}
