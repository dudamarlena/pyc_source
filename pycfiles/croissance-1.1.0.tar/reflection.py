...
# Embedded file name: /home/dcollins/.env/deleteme/lib/python2.7/site-packages/croi/reflection.py
# Compiled at: 2015-03-14 16:02:40
...
def _public_attrs(obj, pred=lambda attr: True):
    ...
    """
    return sorted([ attr for attr in dir(obj) if not attr.startswith('_') and pred(attr)
                  ])


def _is_instance_method(obj, attr):
    attribute = getattr(obj, attr)
    return isinstance(attribute, types.MethodType) and attribute.im_class is obj.__class__


def _is_instance_field(obj, attr):
    attribute = getattr(obj, attr, None)
    return not isinstance(attribute, types.MethodType) and getattr(obj.__class__, attr, None) is None


def _is_instance_property(obj, attr):
    attribute = getattr(obj, attr, None)
    return isinstance(getattr(obj.__class__, attr, None), property)
...
def class_attrs(obj):
    ...
    def pred(attr):
        return (_is_instance_field(obj.__class__, attr) or _is_instance_method(obj.__class__, attr)) and not isinstance(getattr(obj.__class__, attr, None), property)
...
def class_fields(obj):
    ...
    def pred(attr):
        return _is_instance_field(obj.__class__, attr) and not isinstance(getattr(obj.__class__, attr, None), property)
...
def class_methods(obj):
    ...
    def pred(attr):
        return _is_instance_method(obj.__class__, attr) and not isinstance(getattr(obj.__class__, attr, None), property)
...
def instance_attrs(obj):
    ...
    def pred(attr):
        return _is_instance_field(obj, attr) or _is_instance_method(obj, attr) or _is_instance_property(obj, attr)
...
def instance_members(obj):
    ...
    def pred(attr):
        return _is_instance_field(obj, attr) or _is_instance_property(obj, attr)
