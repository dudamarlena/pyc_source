...
# [GCC 8.4.0]
# Embedded file name: /Users/cameronlowe/Development/django-zencoder/zencoder/views.py
# Compiled at: 2017-01-17 17:26:08
# Size of source mod 2**32: 7362 bytes
import json, base64, hmac, hashlib, datetime, os, re
...
def video(request, video_id=None):
    ...
    video.save()
    policy_dict = {'expiration': expiration.strftime('%Y-%m-%dT%H:%M:%S.000Z'), 
     'conditions': [{'bucket': settings.VIDEO_ENCODING_BUCKET}, {'acl': 'private'}, {'success_action_status': '201'}, {'key': key},
                    [
                     'content-length-range', 0, settings.VIDEO_ENCODING_MAX_SIZE]]}
    policy_document = json.dumps(policy_dict).encode('utf-8')
    policy = base64.b64encode(policy_document)
    signature = hmac.new(settings.AWS_SECRET_ACCESS_KEY.encode('utf-8'), policy, hashlib.sha1).digest()
...
class VideoEmbedView(DetailView):
    ...
    def get_template_names(self):
        names = [
         'zencoder/embed/video.html', 'zencoder/embed/default.html']
        ...
        elif self.object.sources.count() == 0:
            names.insert(0, 'zencoder/embed/failed.html')
        return names
...
class VideoJsonDetailView(views.APIView):
    ...
    def get(self, request, video_id):
        ...
        """
        video = get_object_or_404(Video, pk=video_id)
        response = ZencoderVideoSerializer().to_representation(video)
