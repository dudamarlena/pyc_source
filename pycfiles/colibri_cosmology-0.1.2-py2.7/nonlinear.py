...
class nonlinear_pk(cc.cosmo):
    ...
    def __init__(self, z=0.0, k=np.logspace(-4.0, 2.0, 1001), code='camb', kwargs_code={}, BAO_smearing=True, cosmology=cc.cosmo()):
        ...
        if BAO_smearing:
            ...
            sv2 = [ 1.0 / (6.0 * np.pi ** 2.0) * np.trapz(k_tmp * pk_cbcb[i], x=np.log(self.k_tmp)) for i in range(self.nz) ]
            pk_dw = [ (pk_cbcb[i] - pk_nw[i]) * np.exp(-k ** 2.0 * sv2[i]) + pk_nw[i] for i in range(self.nz) ]
        else:
            pk_nw = pk_cbcb
            pk_dw = pk_cbcb
