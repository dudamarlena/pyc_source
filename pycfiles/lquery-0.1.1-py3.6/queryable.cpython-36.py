...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\lquery\queryable.py
# Compiled at: 2018-08-03 15:11:09
# Size of source mod 2**32: 17481 bytes
...
from .func import to_memory, select, select_with_index, select_with_correspondence, select_many, select_many_with_index, select_many_with_correspondence, group_by, where, of_type, take, take_while, skip, skip_while, distinct, order_by, order_by_descending, reverse, default_if_empty, first, first_or_default, last, last_or_default, single, single_or_default, element_at, count, concat, difference, intersect, union, join, group_join, zip, min, max, sum, average, any, all, contains, sequence_equal
    ...
    def print(self):
        if self._mode == self.MODE_NORMAL:
            if all(d.type == self.TYPE_MEMORY for d in self.details):
                ...
                if all(d.type == self.TYPE_SQL for d in self.details):
...
class Queryable(IQueryable):
    ...
    def select_with_index(self, selector: Callable=IndexedElement, transform: Callable=identity):
        call_expr = Make.call(select_with_index, Make.parameter('self'), Make.ref(selector), Make.ref(transform))
        return self._then_query(call_expr)

    @typechecked
    def select_with_correspondence(self, selector: Callable, result_selector: Callable=KeyedElement):
        call_expr = Make.call(select_with_correspondence, Make.parameter('self'), Make.ref(selector), Make.ref(result_selector))
        return self._then_query(call_expr)

    @typechecked
    def select_many(self, collection_selector: Callable=identity, result_selector: Callable=identity):
        call_expr = Make.call(select_many, Make.parameter('self'), Make.ref(collection_selector), Make.ref(result_selector))
        return self._then_query(call_expr)

    @typechecked
    def select_many_with_index(self, collection_selector: Callable=IndexedElement, result_selector: Callable=lambda source_element, collection_element: collection_element):
        call_expr = Make.call(select_many_with_index, Make.parameter('self'), Make.ref(collection_selector), Make.ref(result_selector))
        return self._then_query(call_expr)

    @typechecked
    def select_many_with_correspondence(self, collection_selector: Callable=identity, result_selector: Callable=KeyedElement):
        call_expr = Make.call(select_many_with_correspondence, Make.parameter('self'), Make.ref(collection_selector), Make.ref(result_selector))
        return self._then_query(call_expr)

    @typechecked
    def group_by(self, key_selector: Callable=identity, element_selector: Callable=identity, result_selector: Callable=lambda key, grouping: grouping):
        call_expr = Make.call(group_by, Make.parameter('self'), Make.ref(key_selector), Make.ref(element_selector), Make.ref(result_selector))
        return self._then_query(call_expr)
    ...
    @typechecked
    def join(self, inner_iterable: Iterable, outer_key_selector: Callable=identity, inner_key_selector: Callable=identity, result_selector: Callable=lambda outer, inner: (
 outer, inner)):
        call_expr = Make.call(join, Make.parameter('self'), Make.ref(inner_iterable), Make.ref(outer_key_selector), Make.ref(inner_key_selector), Make.ref(result_selector))
        return self._then_query(call_expr)

    @typechecked
    def group_join(self, inner_iterable: Iterable, outer_key_selector: Callable=identity, inner_key_selector: Callable=identity, result_selector: Callable=lambda outer, grouping: grouping):
        call_expr = Make.call(group_join, Make.parameter('self'), Make.ref(inner_iterable), Make.ref(outer_key_selector), Make.ref(inner_key_selector), Make.ref(result_selector))
        return self._then_query(call_expr)
    ...
    def aggregate(self, reducer: Callable, seed, result_selector: Callable=identity):
        call_expr = Make.call(aggregate, Make.parameter('self'), Make.ref(reducer), Make.ref(seed), Make.ref(result_selector))
        return self._then_query(call_expr)
    ...
    def contains(self, value, equality_comparer: Callable=operator.eq):
        call_expr = Make.call(contains, Make.parameter('self'), Make.ref(value), Make.ref(equality_comparer))
        return self._then_query(call_expr)

    @typechecked
    def sequence_equal(self, other, equality_comparer: Callable=operator.eq) -> bool:
        call_expr = Make.call(sequence_equal, Make.parameter('self'), Make.ref(other), Make.ref(equality_comparer))
        return self._then_query(call_expr)
...
class QueryProvider(IQueryProvider):
    ...
    def get_reduce_info(self, queryable: IQueryable) -> ReduceInfo:
        ...
        """
        if queryable.src:
            info = queryable.src.get_reduce_info()
