...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\SQLAlchemy_wrap\sql_factory\sqlalchemy_factory.py
# Compiled at: 2019-08-20 12:44:42
...
class SqlalchemyFactory(IDbFactory):
            ...
            if self._db_dir is None:
                ...
                for attr_name in dir(m):
                    if table_name == attr_name:
                        if type(getattr(m, attr_name)) == type:
                            if issubclass(getattr(m, attr_name), BaseTable):
    ...
    def get_base_table_instance(self, table_name: str, db_name: str) -> BaseTable:
        msg = '{}.{} not exists table operator class `{}`, return BaseTable()'.format(self._db_dir, db_name, table_name)
        print(msg)
