...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/archipelagentiphonenotification/__init__.py
...
def make_archipel_plugin(configuration, entity, group):
    ...
    """
    return [
     {'info': appnotificator.AppNotificator.plugin_info(), 'plugin': appnotificator.AppNotificator(configuration, entity, group)}]
...
def version():
    ...
    return (
     __name__, pkg_resources.get_distribution('archipel-agent-iphone-notification').version, [appnotificator.AppNotificator.plugin_info()])