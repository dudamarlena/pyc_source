...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\li_rn_networks\li_rn_network.py
# Compiled at: 2020-02-12 22:01:39
# Size of source mod 2**32: 14718 bytes
...
class li_rnn_networks:
    ...
    def output_W(self):
        return (
         self.W01, self.W11, self.W12, self.b1, self.b2)
            ...
            if i % 100 == 0:
                if i != 0:
                    ...
                    if self.loss_container / 0.25 > self.min_loss:
                        New_lr = self.lr * (self.loss_container / 0.25) ** self.lr_index
            if i % 500 == 0:
                if i != 0:
                    print('loss:', dout, 'epoch:', i)
                    print('_________')
    ...
    def gene_li_out(self):
        ...
        for i in range(self.test_data.shape[0]):
            self.def_parameter(self.Tau_H, self.Tau_O, self.dt, self.random_del, self.Step_length)
            self.network_reset()
