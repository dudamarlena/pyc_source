...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\li_rn_networks\pi_gen_imput_data.py
# Compiled at: 2020-02-12 22:00:49
# Size of source mod 2**32: 16222 bytes
import numpy as np, test_rnn_realtime as net, time
high_value = 1.0
low_value = 0.0
category_num = 2
steps_num = 3
    ...
    for r in range(category_num ** (chunk - 1) - 2):
        for i in range(category_num):
            for j in range(1, category_num):
                non_much_num = (i + j) % category_num * category_num ** 3 + i * category_num ** 2 + i * category_num + i
...
def iadd_traning_3data(input_data, target_data, category_num, steps_num):
    ...
    for r in range(1):
        for i in range(category_num):
            for j in range(1, category_num):
                non_much_num = (i + j) % category_num * category_num ** 2 + i * category_num + i
                print('non_much_num:', non_much_num)
                print(input_data[non_much_num])
                iinput_data = np.vstack((
                 iinput_data, np.array([input_data[non_much_num]])))
