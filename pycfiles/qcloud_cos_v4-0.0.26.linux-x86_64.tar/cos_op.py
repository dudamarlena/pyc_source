...
# [GCC 8.4.0]
# Embedded file name: /home/travis/virtualenv/python2.7.14/lib/python2.7/site-packages/qcloud_cos/cos_op.py
# Compiled at: 2018-03-19 03:53:29
import os, time, json, hashlib, urllib
...
class BaseOp(object):
    ...
    def _check_params(self, request):
        u"""检查用户输入参数, 检查通过返回None, 否则返回一个代表错误原因的dict
    ...
    def del_base(self, request):
        u"""删除文件或者目录, is_file_op为True表示是文件操作
...
class FileOp(BaseOp):
    ...
    def upload_file(self, request):
        u"""上传文件, 根据用户的文件大小,选择单文件上传和分片上传策略
    ...
    def _upload_slice_file(self, request):
        ...
        if check_params_ret is not None:
            ...
            if control_ret['code'] != 0:
                return control_ret
            if 'access_url' in control_ret['data']:
                return control_ret
            local_path = request.get_local_path()
            file_size = os.path.getsize(local_path)
            if 'slice_size' in control_ret['data']:
                slice_size = control_ret['data']['slice_size']
            offset = 0
            session = control_ret['data']['session']
            if request._max_con <= 1 or 'serial_upload' in control_ret['data'] and control_ret['data']['serial_upload'] == 1:
                ...
                with open(local_path, 'rb') as (local_file):
                    while offset < file_size:
                        ...
                        if data_ret['code'] == 0:
                            if 'access_url' in data_ret['data']:
                                return data_ret
                        else:
                            return data_ret
                        offset += slice_size
                ...
                if not result['success_all']:
                    return {'code': 1, 'message': str(result)}
    ...
    def download_file(self, request):
        ...
        try:
            ...
            return {'code': 0, 'message': 'download successfully'}
        except Exception as e:
            return {'code': 1, 'message': 'download failed, exception: ' + str(e)}
