...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/snmpfwd/daemon.py
    ...
    class PrivilegesOf(object):
        ...
        def __enter__(self):
            ...
            try:
                ...
                raise error.SnmpfwdError('getpwnam()/getgrnam() failed for %s/%s: %s' % (self._uname, self._gname, sys.exc_info()[1]))
            ...
            try:
                ...
                raise error.SnmpfwdError('%s failed for %s/%s: %s' % (self._final and 'setgid()/setuid()' or 'setegid()/seteuid()', runningGid, runningUid, sys.exc_info()[1]))
        ...
        def __exit__(self, *args):
            ...
            try:
                ...
                raise error.SnmpfwdError('setegid()/seteuid() failed for %s/%s: %s' % (self._oldgid, self._olduid, sys.exc_info()[1]))
