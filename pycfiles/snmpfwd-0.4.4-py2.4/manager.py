...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/snmpfwd/trunking/manager.py
...
class TrunkingManager(object):
    ...
    def setupTrunks(self, timeNow):
        for trunkId in self.__clients:
            if trunkId in self.__runningClientsTrunkMap and not self.__runningClientsTrunkMap[trunkId].isUp:
            ...
            if trunkId not in self.__runningClientsTrunkMap:
                self.__runningClientsTrunkMap[trunkId] = client.TrunkingClient(*self.__clients[trunkId])
    ...
    def monitorTrunks(self, timeNow):
        self.__monitorTrunks(timeNow, self.__runningClientsTrunkMap, self.__runningClientsConnMap)
        self.__monitorTrunks(timeNow, self.__runningServersTrunkMap, self.__runningServersConnMap)
    ...
    def addClient(self, trunkId, localEndpoint, remoteEndpoint, pingPeriod, secret):
        ...
        self.__clients[trunkId] = (
         localEndpoint, remoteEndpoint, secret, self.__proxyDataCbFun)
    ...
    def __ctlCbFun(self, connection, msg, cbCtx):
        if msg:
            ...
            pingPeriod = cbCtx
            if trunkId in self.__runningServersTrunkMap or trunkId in self.__runningClientsTrunkMap:
    ...
    def addServer(self, localEndpoint, pingPeriod, secret):
        server.TrunkingSuperServer(localEndpoint, secret, self.__proxyDataCbFun, self.__ctlCbFun, pingPeriod)