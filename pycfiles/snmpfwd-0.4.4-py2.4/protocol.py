...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/snmpfwd/trunking/protocol.py
...
class Message(univ.Sequence):
    __module__ = __name__
    componentType = namedtype.NamedTypes(namedtype.NamedType('version', univ.Integer()), namedtype.NamedType('msg-id', univ.Integer()), namedtype.NamedType('content-id', univ.Integer(namedValues=namedval.NamedValues(('request', MSG_TYPE_REQUEST), ('response', MSG_TYPE_RESPONSE), ('announcement', MSG_TYPE_ANNOUNCEMENT), ('ping', MSG_TYPE_PING), ('pong', MSG_TYPE_PONG)))), namedtype.NamedType('payload', univ.OctetString()))


class Request(univ.Sequence):
    __module__ = __name__
    componentType = namedtype.NamedTypes(namedtype.NamedType('callflow-id', univ.OctetString()), namedtype.NamedType('snmp-engine-id', univ.OctetString()), namedtype.NamedType('snmp-transport-domain', univ.ObjectIdentifier()), namedtype.NamedType('snmp-peer-address', univ.OctetString()), namedtype.NamedType('snmp-peer-port', univ.Integer()), namedtype.NamedType('snmp-bind-address', univ.OctetString()), namedtype.NamedType('snmp-bind-port', univ.Integer()), namedtype.NamedType('snmp-security-model', univ.Integer()), namedtype.NamedType('snmp-security-level', univ.Integer()), namedtype.NamedType('snmp-security-name', univ.OctetString()), namedtype.NamedType('snmp-security-engine-id', univ.OctetString()), namedtype.NamedType('snmp-context-engine-id', univ.OctetString()), namedtype.NamedType('snmp-context-name', univ.OctetString()), namedtype.NamedType('snmp-pdu', univ.OctetString()), namedtype.NamedType('snmp-credentials-id', univ.OctetString('')), namedtype.NamedType('snmp-context-id', univ.OctetString('')), namedtype.NamedType('snmp-content-id', univ.OctetString('')), namedtype.NamedType('snmp-peer-id', univ.OctetString('')))


class Response(univ.Sequence):
    __module__ = __name__
    componentType = namedtype.NamedTypes(namedtype.NamedType('error-indication', univ.OctetString('')), namedtype.NamedType('snmp-pdu', univ.OctetString('')))


class Announcement(univ.Sequence):
    __module__ = __name__
    componentType = namedtype.NamedTypes(namedtype.NamedType('trunk-id', univ.OctetString()))


class Ping(univ.Sequence):
    __module__ = __name__
    componentType = namedtype.NamedTypes(namedtype.NamedType('serial', univ.Integer()))
...
pduMap = {MSG_TYPE_REQUEST: Request(), MSG_TYPE_RESPONSE: Response(), MSG_TYPE_ANNOUNCEMENT: Announcement(), MSG_TYPE_PING: Ping(), MSG_TYPE_PONG: Pong()}

def prepareRequestData(msgId, req, secret):
    r = Request()
    r['callflow-id'] = req['callflow-id']
    for k in ('snmp-engine-id', 'snmp-transport-domain', 'snmp-peer-address', 'snmp-peer-port',
              'snmp-bind-address', 'snmp-bind-port', 'snmp-security-model', 'snmp-security-level',
              'snmp-security-name', 'snmp-security-engine-id', 'snmp-context-engine-id',
              'snmp-context-name', 'snmp-credentials-id', 'snmp-context-id', 'snmp-content-id',
              'snmp-peer-id'):
        r[k] = req[k]
...
def prepareDataElements(octets, secret):
    try:
        ...
        return (
         None, None, None, octets)

    if msg['version'] != PROTOCOL_VERSION:
        raise SnmpfwdError('Unsupported protocol versions %s/%s' % (msg['version'], PROTOCOL_VERSION))
    (r, _) = decoder.decode(secret and crypto.decrypt(secret, msg['payload'].asOctets()) or msg['payload'].asOctets(), asn1Spec=pduMap.get(msg['content-id']))
    rsp = {}
