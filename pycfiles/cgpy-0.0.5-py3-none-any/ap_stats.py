...
# Embedded file name: build\bdist.win32\egg\cgp\virtexp\elphys\ap_stats.py
# Compiled at: 2013-01-14 06:47:43
__doc__ = 'Action potential statistics.'
import numpy as np

def apd(time, voltage, p_repol=(0.25, 0.5, 0.75, 0.9), interpolate=True, decay_p=None):
    """
    ...
    * peak : peak voltage
    ...
    if interpolate:
        with np.errstate(all='ignore'):
            t_repol = time[repol_i] + (threshold - voltage[repol_i]) * (time[(repol_i + 1)] - time[repol_i]) / (voltage[(repol_i + 1)] - voltage[repol_i])
    else:
    ...
    result = dict(i=np.r_[(peak_i, repol_i)], base=base_v, ttp=peak_t, peak=peak_v, amp=peak_v - base_v, p_repol=p_repol, t_repol=t_repol)
