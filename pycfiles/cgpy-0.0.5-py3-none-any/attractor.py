...
# Embedded file name: build\bdist.win32\egg\cgp\phenotyping\attractor.py
# Compiled at: 2013-01-14 06:47:43
__doc__ = 'Find steady state or stable limit cycle of dynamic system.'
...
class AttractorMixin(object):
    ...
    def eq(self, tmax=None, tol=0.0001, last_only=True):
        """
        Find steady state.
        ...
        if gout < 0:
            ...
            if tmax is None:
                tmax = self.t[(-1)]
            t, y, flag = self.integrate(t=tmax, nrtfn=1, g_rtfn=g_rtfn, assert_flag=cvode.CV_ROOT_RETURN)
        y = y.squeeze()
        if last_only:
            return (t[(-1)], y[(-1)])
        else:
    ...
    def next_extremum(self, tmax=None, index=0):
        ...
        """
        t, y, _flag = self.integrate(t=tmax, nrtfn=1, g_rtfn=self.ydoti(index), assert_flag=cvode.CV_ROOT_RETURN)
        return (
         t, y.squeeze(), (t[(-1)], y[(-1)]))

    def cycle(self, index=0, tmax=None, tol=0.0001, n=None):
        """
        ...
        while True:
            ...
            for lag, (t_, y_, (te_, ye_)) in enumerate(extrema):
                ...
                if self.weighted_rms(diff) < tol:
                    ...
                    t, y, _ = catrec(globalize_time=False, *L)
                    period = te - te_
                    return (
                     t, y.squeeze(), period)
