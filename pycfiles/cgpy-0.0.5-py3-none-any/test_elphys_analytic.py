...
# Embedded file name: build\bdist.win32\egg\cgp\test\test_elphys_analytic.py
# Compiled at: 2013-01-14 06:47:43
__doc__ = 'Mockup model of action potential: :class:`Test_cell`.'
import numpy as np
...
class Test_cell(Namedcvodeint, Paceable, Clampable):
    ...
    ..  plot::
        ...
        for t, y, stats in c.aps(n=2):
            plt.plot(t, y.V, '.-')
    """
    pr = dict2rec(m=np.log(20), stim_duration=1.0, stim_amplitude=10.0, stim_period=2.0).view(np.recarray)

    def __init__(self):
        super(Test_cell, self).__init__(self.f_ode, t=(0, 2), y=dict2rec(V=1.0), p=self.pr, reltol=1e-10)
    ...
    def test_ap_solution_end(self):
        ...
        with self.autorestore():
            t, y, _stats = self.ap()
        ys = self.solution(t, y)
        np.testing.assert_allclose(y.V.squeeze()[(-1)], ys[(-1)], rtol=1e-06)
