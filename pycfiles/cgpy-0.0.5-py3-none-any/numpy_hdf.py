...
# Embedded file name: build\bdist.win32\egg\cgp\utils\numpy_hdf.py
# Compiled at: 2013-01-14 06:47:43
__doc__ = 'Convert HDF5 file with groups to/from directory tree of Numpy recarrays.'
import os, logging, ast, numpy as np, tables as pt
            ...
            for file_ in files:
                ...
                if extension == ext:
                    ...
                    if a.dtype == object and a.shape == ():
                        ...
                        f.createTable(group, name, np.atleast_1d(a), createparents=True, expectedrows=len(np.atleast_1d(a)))
                    elif a.dtype == object:
    ...
    for k, v in d.items():
        ...
        if hasattr(v, 'items'):
            ...
            try:
                ...
                if v.dtype.names:
                    f.createTable(description=np.atleast_1d(v), expectedrows=len(np.atleast_1d(v)), **kw)
                else:
