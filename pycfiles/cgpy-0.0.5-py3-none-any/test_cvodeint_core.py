...
# Embedded file name: build\bdist.win32\egg\cgp\test\test_cvodeint_core.py
# Compiled at: 2013-01-14 06:47:43
__doc__ = 'Tests for :mod:`..cvodeint.core`.'
...
def test_integrate_adaptive_steps():
    ...
    """
    c = Cvodeint(example_ode.logistic_growth, t=[0, 2], y=[0.1], reltol=0.001)
    c.integrate()
...
def test_integrate():
    ...
    np.testing.assert_allclose(y.squeeze(), ys, rtol=1e-06)
...
def test_repr():
    ...
    wants = [
     'Cvodeint(f_ode=exp_growth, t=array([0, 2]), y=[0.1...], ' + 'abstol=c_double(1e-08))',
     "<class 'cvodeint.Cvodeint'> <function ode at 0x..." + 'getargspec(__init__) is not available when running under Cython']
...
def test_y_dtype_rec():
    ...
    Cvodeint(example_ode.vdp, t=[0, 2], y=np.rec.fromrecords([(1.0, 2.0)], dtype=[
     (
      'u', float), ('v', float)]))
...
def test_pickling():
    ...
    old = Cvodeint(example_ode.logistic_growth, t=[0, 2], y=[0.1], reltol=0.001)
    s = pickle.dumps(old)
    new = pickle.loads(s)
