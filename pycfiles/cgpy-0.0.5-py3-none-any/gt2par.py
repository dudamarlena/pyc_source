...
# Embedded file name: build\bdist.win32\egg\cgp\gt\gt2par.py
# Compiled at: 2013-01-14 06:47:43
__doc__ = 'Simple genotype-to-parameter maps.'
...
def prepare_geno2par_additive(genes, parnames, origpar, relchange, nloci=None):
    ...
    tup = tuple([ (np.mean(ch, axis=1) - np.min(ch, axis=1)) / np.mean(ch, axis=1) for ch in flatten(relchange)
                ])
