...
# [GCC 8.4.0]
# Embedded file name: /Users/sage/fatedoc/FATE/fate_flow/utils/job_utils.py
# Compiled at: 2020-05-06 03:08:10
# Size of source mod 2**32: 23971 bytes
import datetime, functools, errno, json, operator, os, subprocess, sys, threading, typing, uuid, psutil
...
def generate_task_input_data_namespace(task_id, role, party_id):
    return 'input_data_{}'.format(generate_session_id(task_id=task_id, role=role,
      party_id=party_id))
...
def check_config(config: typing.Dict, required_parameters: typing.List):
    for parameter in required_parameters:
        if parameter not in config:
            return (
             False, 'configuration no {} parameter'.format(parameter))
    else:
        return (True, 'ok')
...
def runtime_conf_basic(if_local=False):
    job_runtime_conf = {'initiator':{},  'job_parameters':{'work_mode': WORK_MODE}, 
...
def get_job_conf_path(job_id):
    ...
    return {'job_dsl_path':job_dsl_path,  'job_runtime_conf_path':job_runtime_conf_path, 
...
def get_job_dsl_parser_by_job_id(job_id):
    with DB.connection_context():
        ...
        if jobs:
            job = jobs[0]
            job_dsl_parser = get_job_dsl_parser(dsl=(json_loads(job.f_dsl)), runtime_conf=(json_loads(job.f_runtime_conf)), train_runtime_conf=(json_loads(job.f_train_runtime_conf)))
...
def get_job_dsl_parser(dsl=None, runtime_conf=None, pipeline_dsl=None, train_runtime_conf=None):
    ...
    dsl_parser.run(dsl=dsl, runtime_conf=runtime_conf,
...
def get_job_configuration(job_id, role, party_id, tasks=None):
    with DB.connection_context():
        ...
        jobs = Job.select(Job.f_dsl, Job.f_runtime_conf, Job.f_train_runtime_conf).where(Job.f_job_id == job_id, Job.f_role == role, Job.f_party_id == party_id)
        if jobs:
            job = jobs[0]
...
def run_subprocess(config_dir, process_cmd, log_dir=None):
    ...
    if log_dir:
        ...
        if os.name == 'nt':
            ...
            startupinfo = None
    p = subprocess.Popen(process_cmd, stdout=std_log,
      stderr=std_log,
...
def kill_task_executor_process(task: Task, only_child=False):
    try:
        ...
        if not pid:
            return False
        schedule_logger(task.f_job_id).info('try to stop job {} task {} {} {} process pid:{}'.format(task.f_job_id, task.f_task_id, task.f_role, task.f_party_id, pid))
        ...
        if not only_child:
            if check_job_process(p.pid):
                if is_task_executor_process(task=task, process=p):
                    p.kill()
        return True
    except Exception as e:
        try:
...
def start_clean_job(**kwargs):
    ...
    if tasks:
        for task in tasks:
            ...
            try:
                stat_logger.info('start {} {} {} {} session stop'.format(task.f_job_id, task.f_role, task.f_party_id, task.f_component_name))
                start_session_stop(task)
                stat_logger.info('stop {} {} {} {} session success'.format(task.f_job_id, task.f_role, task.f_party_id, task.f_component_name))
            except Exception as e:
                try:
                    pass
                finally:
            ...
            try:
                stat_logger.info('start delete {} {} {} {} data table'.format(task.f_job_id, task.f_role, task.f_party_id, task.f_component_name))
                data_views = query_data_view(**task_info)
                if data_views:
                    delete_table(data_views)
                    stat_logger.info('delete {} {} {} {} data table success'.format(task.f_job_id, task.f_role, task.f_party_id, task.f_component_name))
            except Exception as e:
                try:
                    stat_logger.info('delete {} {} {} {} data table failed'.format(task.f_job_id, task.f_role, task.f_party_id, task.f_component_name))
            ...
            try:
                stat_logger.info('start delete {} {} {} {} metric data'.format(task.f_job_id, task.f_role, task.f_party_id, task.f_component_name))
                delete_metric_data(task_info)
                stat_logger.info('delete {} {} {} {} metric data success'.format(task.f_job_id, task.f_role, task.f_party_id, task.f_component_name))
            except Exception as e:
                try:
                    stat_logger.info('delete {} {} {} {} metric data failed'.format(task.f_job_id, task.f_role, task.f_party_id, task.f_component_name))
                    stat_logger.exception(e)
                finally:
                    e = None
                    del e
...
def start_session_stop(task):
    ...
    schedule_logger(task.f_job_id).info('start run subprocess to stop component {} session'.format(task.f_component_name))
    task_dir = os.path.join(get_job_directory(job_id=(task.f_job_id)), task.f_role, task.f_party_id, task.f_component_name, 'session_stop')
...
def gen_all_party_key(all_party):
    ...
    if not all_party:
        ...
        if isinstance(all_party, dict):
            ...
            all_party_key = '#'.join(['%s-%s' % (role_name,
...
def get_timeout(job_id, timeout, runtime_conf, dsl):
    try:
        ...
        schedule_logger(job_id).info('setting job {} timeout {} not a positive number, using the default timeout {}'.format(job_id, timeout, default_timeout))
        return default_timeout
    except:
        default_timeout = job_default_timeout(runtime_conf, dsl)
        schedule_logger(job_id).info('setting job {} timeout {} is incorrect, using the default timeout {}'.format(job_id, timeout, default_timeout))
        return default_timeout
