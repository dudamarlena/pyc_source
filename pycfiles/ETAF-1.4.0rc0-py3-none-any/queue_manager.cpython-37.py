...
# [GCC 8.4.0]
# Embedded file name: /Users/sage/fatedoc/FATE/fate_flow/manager/queue_manager.py
# Compiled at: 2020-04-28 09:19:05
# Size of source mod 2**32: 14145 bytes
import json, operator, queue, threading
...
class MysqlQueue(BaseQueue):
    ...
    def get(self, block=True, timeout=None):
        with self.not_empty:
            if not (block or self.query_events()):
                raise Exception
            else:
                if timeout is None:
...
class ListQueue(BaseQueue):
    ...
    def put(self, item, block=True, timeout=None):
        with self.not_full:
            if self.maxsize > 0:
                if (block or self.qsize()) >= self.maxsize:
                    raise Exception
                else:
                    if timeout is None:
