...
# [GCC 8.4.0]
# Embedded file name: /Users/sage/fatedoc/FATE/fate_flow/driver/job_controller.py
...
class JobController(object):
    ...
    def submit_job(job_data, job_id=None):
        if not job_id:
            ...
            if job_type != 'predict':
                ...
                job_tracker = Tracking(job_id=job_id, role=(job_initiator['role']), party_id=(job_initiator['party_id']), model_id=(job_parameters['model_id']),
            ...
            path_dict = save_job_conf(job_id=job_id, job_dsl=job_dsl,
            ...
            get_job_dsl_parser(dsl=job_dsl, runtime_conf=job_runtime_conf,
              train_runtime_conf=train_runtime_conf)
        ...
        schedule_logger(job_id).info('submit job successfully, job id is {}, model id is {}'.format(job.f_job_id, job_parameters['model_id']))
        ...
        return (job_id, path_dict['job_dsl_path'], path_dict['job_runtime_conf_path'], logs_directory,
         {'model_id':job_parameters['model_id'], 
          'model_version':job_parameters['model_version']}, board_url)
    ...
    def kill_job(job_id, role, party_id, job_initiator, timeout=False, component_name=''):
        ...
        for task in tasks:
            ...
            try:
                ...
                schedule_logger(job_id).info('job {} component {} on {} {} process {} kill {}'.format(job_id, task.f_component_name, task.f_role, task.f_party_id, task.f_run_pid, 'success' if kill_status else 'failed'))

            status = TaskStatus.FAILED if not timeout else TaskStatus.TIMEOUT
            if task.f_status != TaskStatus.COMPLETE:
                task.f_status = status
            try:
                TaskExecutor.sync_task_status(job_id=job_id, component_name=(task.f_component_name), task_id=(task.f_task_id), role=role,
                  party_id=party_id,
    ...
    def update_task_status(job_id, component_name, task_id, role, party_id, task_info):
        ...
        schedule_logger(job_id).info('job {} component {} {} {} status {}'.format(job_id, component_name, role, party_id, task_info.get('f_status', '')))
    ...
    def update_job_status(job_id, role, party_id, job_info, create=False):
        ...
        if create:
            ...
            if USE_AUTHENTICATION:
                authentication_check(src_role=(job_info.get('src_role', None)), src_party_id=(job_info.get('src_party_id', None)), dsl=dsl,
                  runtime_conf=runtime_conf,
                  role=role,
                  party_id=party_id)
            save_job_conf(job_id=job_id, job_dsl=dsl,
              job_runtime_conf=runtime_conf,
              train_runtime_conf=train_runtime_conf,
              pipeline_dsl=None)
            job_parameters = runtime_conf['job_parameters']
            job_tracker = Tracking(job_id=job_id, role=role, party_id=party_id, model_id=(job_parameters['model_id']),
              model_version=(job_parameters['model_version']))
            ...
            dag = get_job_dsl_parser(dsl=dsl, runtime_conf=runtime_conf,
              train_runtime_conf=train_runtime_conf)
    ...
    def save_pipeline(job_id, role, party_id, model_id, model_version):
        ...
        job_dsl, job_runtime_conf, train_runtime_conf = job_utils.get_job_configuration(job_id=job_id, role=role, party_id=party_id)
    ...
    def clean_job(job_id, role, party_id, roles, party_ids):
        ...
        for task in tasks:
            try:
                ...
                schedule_logger(job_id).info('job {} component {} on {} {} clean done'.format(job_id, task.f_component_name, role, party_id))
            except Exception as e:
                try:
                    schedule_logger(job_id).info('job {} component {} on {} {} clean failed'.format(job_id, task.f_component_name, role, party_id))
                    schedule_logger(job_id).exception(e)
                finally:
                    e = None
                    del e
    ...
    def cancel_job(job_id, role, party_id, job_initiator):
        ...
        if jobs:
            raise Exception('role {} party id {} cancel waiting job {} failed, not is initiator'.format(role, party_id, job_id))
