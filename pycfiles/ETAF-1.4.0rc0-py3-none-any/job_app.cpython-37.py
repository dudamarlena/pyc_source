...
# [GCC 8.4.0]
# Embedded file name: /Users/sage/fatedoc/FATE/fate_flow/apps/job_app.py
...
def submit_job():
    ...
    if work_mode == RuntimeConfig.WORK_MODE:
        ...
        return get_json_result(retcode=0, retmsg='success', job_id=job_id,
...
def update_job():
    ...
    if not jobs:
        return get_json_result(retcode=101, retmsg='find job failed')
    job = jobs[0]
    TaskScheduler.sync_job_status(job_id=(job.f_job_id), roles={job.f_role: [job.f_party_id]}, work_mode=(job.f_work_mode),
      initiator_party_id=(job.f_party_id),
      initiator_role=(job.f_role),
      job_info={'f_description': job_info.get('notes', '')})
...
def job_config():
    ...
    response_data['model_info'] = {'model_id':response_data['runtime_conf']['job_parameters']['model_id'],  'model_version':response_data['runtime_conf']['job_parameters']['model_version']}
