...
# [GCC 8.4.0]
# Embedded file name: /Users/sage/fatedoc/FATE/fate_flow/driver/dsl_parser.py
# Compiled at: 2020-04-28 09:16:53
# Size of source mod 2**32: 31868 bytes
import copy, json, os
...
class DSLParser(object):
    ...
    def _init_component_setting(self, setting_conf_prefix, runtime_conf, default_runtime_conf_prefix):
        ...
        for i in range(len(self.topo_rank)):
            ...
            if self.train_input_model.get(name, None) is None:
                ...
                role_parameters = parameter_util.ParameterUtil.override_parameter(default_runtime_conf_prefix, setting_conf_prefix, runtime_conf, module, name)
    ...
    def _check_component_valid_names(self):
        ...
        for component in self.components:
            ...
            if len(name.split('_', -1)) == 1:
                raise ValueError("component's name should be format of name_index, index is start from 0 and be consecutive for same module")
            ...
            if name_module_mapping.get(name_prefix) != module:
                raise ValueError("component's name should be format of name_index, index is start from 0 and same modules should have the same name")
    ...
    def _find_dependencies(self, pipeline_dsl=None, mode='train'):
                        ...
                        for model_key in input_model_keyword:
                            if model_key in upstream_input:
                                ...
                                for model in model_list:
                                    ...
                                    elif module_name == self.pipeline_module_alias:
                                        raise ValueError('Pipeline Model can not be used')
                                    if name not in self.pipeline_modules:
                                        if module_name in self.pipeline_modules:
                                            raise ValueError('Pipeline Model can not be used')
        ...
        for i in range(len(self.components)):
            ...
            if self.component_upstream[i]:
                ...
                self.in_degree[self.component_name_index.get(self.components[i].get_name())] = len(self.component_upstream[i])
    ...
    def _get_end_of_pipeline_dsl(self, pipeline_dsl):
        ...
        for component in component_details:
            if has_down[component] or component_details[component].get('output', None) and 'data' in component_details[component]['output']:
                return component
    ...
    def prepare_graph_dependency_info(self):
        ...
        for component in self.components:
            ...
            if upstream:
                ...
                for up_component in upstream:
                    if (
                     name, up_component) in self.component_upstream_data_relation_set:
                        dependence_dict[name].append({'component_name':up_component,  'type':'data'})
                    if (name, up_component) in self.component_upstream_model_relation_set:
                        dependence_dict[name].append({'component_name':up_component,  'type':'model'})
        ...
        base_dependency = {'component_list':component_list,  'dependencies':dependence_dict, 
        ...
        for role in runtime_conf['role']:
            ...
            for rank in range(len(self.topo_rank)):
                ...
                elif self.train_input_model.get(name, None) is None:
                    param_class = parameter_util.ParameterUtil.get_param_class_name(self.setting_conf_prefix, module)
                    for i in range(len(dependency_list)):
                        if parameters[role][i].get(param_class) is None or parameters[role][i][param_class].get('need_run') is False:
                            dependency_list[i]['component_need_run'][name] = False
                        else:
                ...
                for i in range(len(dependency_list)):
                    dependency_list[i]['component_need_run'][name] = dependency_list[i]['component_need_run'][input_model_name]
    ...
    def _auto_deduction(self, setting_conf_prefix):
        ...
        for i in range(len(self.predict_components)):
            ...
            if self.get_need_deploy_parameter(name=name, setting_conf_prefix=setting_conf_prefix):
                need_predict = True
            ...
            elif 'input' in self.dsl['components'][name]:
                if 'input' not in self.predict_dsl['components'][name]:
                    self.predict_dsl['components'][name]['input'] = {}
                    ...
                    if 'data' in self.dsl['components'][name]['input']['data']:
                        ...
                        if input_data.split('.')[0] == 'args':
                            ...
                            if self.get_need_deploy_parameter(name=pre_name, setting_conf_prefix=setting_conf_prefix):
                                ...
                                self.predict_dsl['components'][name]['input']['data']['eval_data'] = output_data_maps[pre_name][data_suffix]
                        ...
                        if input_data.split('.')[0] == 'args':
                            ...
                            if self.get_need_deploy_parameter(name=pre_name, setting_conf_prefix=setting_conf_prefix):
                                ...
                                self.predict_dsl['components'][name]['input']['data']['eval_data'] = output_data_maps[pre_name].get(data_suffix)
                    else:
                        ...
                        if 'output' in self.dsl['components'][name]:
                            if 'data' in self.dsl['components'][name]['output']:
                            ...
                            if up_input_data_component_name == 'args' or self.get_need_deploy_parameter(name=up_input_data_component_name,
                              setting_conf_prefix=setting_conf_prefix):
                                output_data_maps[name][output_data_str] = [
                                 up_input_data]
                        else:
    ...
    def run(self, pipeline_dsl=None, pipeline_runtime_conf=None, dsl=None, runtime_conf=None, default_runtime_conf_prefix=None, setting_conf_prefix=None, mode='train'):
