...
# [GCC 8.4.0]
# Embedded file name: /Users/sage/fatedoc/FATE/fate_flow/utils/parameter_util.py
# Compiled at: 2020-04-28 09:16:53
# Size of source mod 2**32: 7495 bytes
import builtins, importlib, json, os, copy

class ParameterUtil(object):

    @staticmethod
    def override_parameter(default_runtime_conf_prefix=None, setting_conf_prefix=None, submit_dict=None, module=None, module_alias=None):
    ...
    def merge_parameters(runtime_dict, role_parameters, param_obj, idx=-1):
        ...
        for key, val_list in role_parameters.items():
            ...
            if not type(attr).__name__ in dir(builtins):
                if attr or idx != -1:
                    if len(val_list) <= idx:
                        continue
