...
# [GCC 8.4.0]
# Embedded file name: /Users/sage/fatedoc/FATE/arch/standalone/core.py
...
class EggrollSession(object):
    ...
    def clean_duplicated_table(self, eggroll):
        for item in list(self._gc_table.collect()):
            ...
            if isinstance(eggroll, Standalone):
                ...
                table.gc_enable or eggroll.destroy(table)
    ...
    def to_protobuf(self):
        return SessionInfo(sessionId=(self._session_id), computingEngineConf=(self._computing_engine_conf),
          namingPolicy=(self._naming_policy.name),
          tag=(self._tag))

    @staticmethod
    def from_protobuf(session):
        return EggrollSession(session_id=(session.get_session_id()), computing_engine_conf=(session.get_computing_engine_conf()),
    ...
    def __str__(self):
        return '<EggrollSession: session_id: {}, computing_engine_conf: {}, naming_policy: {}, tag: {}, runtime: {}>'.format(self._session_id, self.computing_engine_conf(), self._naming_policy.name, self._tag, self._runtime)