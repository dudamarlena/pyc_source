...
# [GCC 8.4.0]
# Embedded file name: /Users/sage/fatedoc/FATE/fate_flow/utils/session_utils.py
...
def init_session_for_flow_server():
    session.init(job_id=('session_used_by_fate_flow_server_{}'.format(fate_uuid())), mode=(RuntimeConfig.WORK_MODE),
    ...
    stat_logger.info('init detect table {} {} for session {}'.format(detect_table.get_namespace(), detect_table.get_name(), session.get_session_id()))
    ...
    def _out_wrapper(func):
        ...
        def _wrapper(*args, **kwargs):
            if RuntimeConfig.PROCESS_ROLE in [ProcessRole.SERVER]:
                for i in range(3):
                    try:
                        stat_logger.info('detect session {} by table {} {}'.format(session.get_session_id(), DETECT_TABLE[0], DETECT_TABLE[1]))
