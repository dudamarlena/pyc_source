...
# [GCC 8.4.0]
# Embedded file name: /Users/sage/fatedoc/FATE/fate_flow/driver/task_scheduler.py
# Compiled at: 2020-05-06 02:27:06
# Size of source mod 2**32: 29119 bytes
...
class TaskScheduler(object):
    ...
    def distribute_job(job, roles, job_initiator):
        for role, partys in roles.items():
            ...
            for party_id in partys:
                ...
                if role == job_initiator['role']:
                    if party_id == job_initiator['party_id']:
                        job.f_is_initiator = 1
                    else:
                        job.f_is_initiator = 0
                    response_json = federated_api(job_id=(job.f_job_id), method='POST',
                      endpoint=('/{}/schedule/{}/{}/{}/create'.format(API_VERSION, job.f_job_id, role, party_id)),
                    ...
                    if response_json['retcode']:
                        ...
                        TaskScheduler.sync_job_status(job_id=(job.f_job_id), roles=roles, work_mode=(job.f_work_mode),
    ...
    def run_job(job_id, initiator_role, initiator_party_id):
        job_dsl, job_runtime_conf, train_runtime_conf = job_utils.get_job_configuration(job_id=job_id, role=initiator_role,
          party_id=initiator_party_id)
        job_parameters = job_runtime_conf.get('job_parameters', {})
        job_initiator = job_runtime_conf.get('initiator', {})
        dag = get_job_dsl_parser(dsl=job_dsl, runtime_conf=job_runtime_conf,
        ...
        if not job_initiator:
            ...
            TaskScheduler.sync_job_status(job_id=job_id, roles=(job_runtime_conf['role']), work_mode=(job_parameters['work_mode']),
              initiator_party_id=(job_initiator['party_id']),
              initiator_role=(job_initiator['role']),
              job_info=(job.to_json()))
            top_level_task_status = set()
            components = dag.get_next_components(None)
            schedule_logger(job_id).info('job {} root components is {}'.format(job.f_job_id, [component.get_name() for component in components], None))
            for component in components:
                try:
                    run_status = TaskScheduler.run_component(job_id, job_runtime_conf, job_parameters, job_initiator, job_args, dag, component)
            ...
            if len(top_level_task_status) == 2:
                ...
                if True in top_level_task_status:
    ...
    def run_component(job_id, job_runtime_conf, job_parameters, job_initiator, job_args, dag, component):
        ...
        for role, partys_parameters in parameters.items():
            for party_index in range(len(partys_parameters)):
                ...
                response = federated_api(job_id=job_id, method='POST',
                  endpoint=('/{}/schedule/{}/{}/{}/{}/{}/run'.format(API_VERSION, job_id, component_name, task_id, role, dest_party_id)),
                  src_party_id=(job_initiator['party_id']),
                 ...
                 'job_server':{'ip':get_lan_ip(), 
                  'http_port':RuntimeConfig.HTTP_PORT}},
                  work_mode=(job_parameters['work_mode']))
                if response['retcode'] and 'not authorized' in response['retmsg']:
        ...
        if component_task_status:
            ...
            schedule_logger(job_id).info('job {} component {} run {}'.format(job_id, component_name, 'success' if task_success else 'failed'))
            TaskScheduler.sync_job_status(job_id=job_id, roles=(job_runtime_conf['role']), work_mode=(job_parameters['work_mode']),
              initiator_party_id=(job_initiator['party_id']),
              initiator_role=(job_initiator['role']),
              job_info=(job_utils.update_job_progress(job_id=job_id, dag=dag, current_task_id=task_id).to_json()))
            TaskScheduler.stop(job_id=job_id, component_name=component_name)
            if task_success:
                next_components = dag.get_next_components(component_name)
                schedule_logger(job_id).info('job {} component {} next components is {}'.format(job_id, component_name, [next_component.get_name() for next_component in next_components]))
                for next_component in next_components:
                    try:
                        schedule_logger(job_id).info('job {} check component {} dependencies status'.format(job_id, next_component.get_name()))
                        dependencies_status = TaskScheduler.check_dependencies(job_id=job_id, dag=dag, component=next_component)
                        job_status = TaskScheduler.check_job_status(job_id)
                        schedule_logger(job_id).info('job {} component {} dependencies status is {}, job status is {}'.format(job_id, next_component.get_name(), dependencies_status, job_status))
                        if dependencies_status and job_status:
                            run_status = TaskScheduler.run_component(job_id, job_runtime_conf, job_parameters, job_initiator, job_args, dag, next_component)
                        else:
                            run_status = False
                    except Exception as e:
                        try:
    ...
    def check_dependencies(job_id, dag, component):
        ...
        schedule_logger(job_id).info('job {} component {} all dependent component: {}'.format(job_id, component.get_name(), dependent_component_names))
        for dependent_component in dependent_component_names:
            ...
            schedule_logger(job_id).info('job {} component {} dependency {} status is {}'.format(job_id, component.get_name(), dependent_component_name, dependent_component_task_status))
            ...
            try:
                ...
                for _role, _partys_parameters in parameters.items():
                    for _party_parameters in _partys_parameters:
                        ...
                        schedule_logger(job_id).info('job {} component {} run on {} {} status is {}'.format(job_id, component.get_name(), _role, _party_id, task_status))
                ...
                if 'timeout' in status_collect:
                    return
                if len(status_collect) == 1:
                    if TaskStatus.COMPLETE in status_collect:
                        return True
                time.sleep(interval)
            except Exception as e:
                try:
    ...
    def run_task(job_id, component_name, task_id, role, party_id, task_config):
        schedule_logger(job_id).info('job {} {} {} {} task subprocess is ready'.format(job_id, component_name, role, party_id, task_config))
        ...
        try:
            try:
                ...
                if backend.is_eggroll():
                    process_cmd = ['python3', sys.modules[TaskExecutor.__module__].__file__,
                     '-j', job_id,
                     '-n', component_name,
                     '-t', task_id,
                     '-r', role,
                ...
                schedule_logger(job_id).info('job {} {} {} {} task subprocess start'.format(job_id, component_name, role, party_id, task_config))
            ...
            schedule_logger(job_id).info('job {} component {} on {} {} start task subprocess {}'.format(job_id, component_name, role, party_id, 'success' if task_process_start_status else 'failed'))
    ...
    def sync_job_status(job_id, roles, work_mode, initiator_party_id, initiator_role, job_info):
        for role, partys in roles.items():
            ...
            for party_id in partys:
                ...
                federated_api(job_id=job_id, method='POST',
                  endpoint=('/{}/schedule/{}/{}/{}/status'.format(API_VERSION, job_id, role, party_id)),
    ...
    def finish_job(job_id, job_runtime_conf, stop=False):
        ...
        for role, partys in job_runtime_conf['role'].items():
            for party_id in partys:
                if not stop:
                    federated_api(job_id=job_id, method='POST',
                      endpoint=('/{}/schedule/{}/{}/{}/{}/{}/save/pipeline'.format(API_VERSION, job_id, role, party_id, model_id_base64, model_version_base64)),
                      src_party_id=(job_initiator['party_id']),
                      dest_party_id=party_id,
                      src_role=(job_initiator['role']),
                      json_body={},
                      work_mode=(job_parameters['work_mode']))
                federated_api(job_id=job_id, method='POST',
                  endpoint=('/{}/schedule/{}/{}/{}/{}/{}/clean'.format(API_VERSION, job_id, role, party_id, roles, party_ids)),
                  src_party_id=(job_initiator['party_id']),
                  dest_party_id=party_id,
                  src_role=(job_initiator['role']),
                  json_body={},
    ...
    def stop(job_id, end_status=JobStatus.FAILED, component_name=''):
        ...
        if jobs:
            ...
            if not component_name:
                TaskScheduler.sync_job_status(job_id=job_id, roles=roles, initiator_party_id=initiator_party_id, initiator_role=(initiator_job.f_role),
                  work_mode=job_work_mode,
                  job_info=job_info)
            for role, partys in roles.items():
                for party_id in partys:
                    response = federated_api(job_id=job_id, method='POST',
                      endpoint=('/{}/schedule/{}/{}/{}/{}'.format(API_VERSION, job_id, role, party_id, 'cancel' if is_cancel else 'kill')),
                      src_party_id=initiator_party_id,
                      dest_party_id=party_id,
                    ...
                    if response['retcode'] == 0:
                        cancel_success = True
                        schedule_logger(job_id).info('send {} {} {} job {} {} command successfully'.format(role, party_id, 'cancel' if is_cancel else 'kill', job_id, component_name))
                        if is_cancel:
                            break
                    else:
                        schedule_logger(job_id).info('send {} {} {} job {} {} command failed: {}'.format(role, party_id, 'cancel' if is_cancel else 'kill', job_id, component_name, response['retmsg']))
    ...
    def clean_queue():
        ...
        if jobs:
            for job in jobs:
                schedule_logger(job.f_job_id).info('start send {} job {} command success'.format(JobStatus.CANCELED, job.f_job_id))
                ...
                TaskScheduler.sync_job_status(job_id=(job.f_job_id), roles=roles, initiator_party_id=initiator_party_id, initiator_role=(job.f_role),
                  work_mode=job_work_mode,
                  job_info=job_info)
                job_runtime_conf = json_loads(job.f_runtime_conf)
                event = job_utils.job_event(job.f_job_id, job_runtime_conf['initiator']['role'], job_runtime_conf['initiator']['party_id'])
                try:
                    RuntimeConfig.JOB_QUEUE.del_event(event)
                    schedule_logger(job.f_job_id).info('send {} job {} command success'.format(JobStatus.CANCELED, job.f_job_id))
                except Exception as e:
                    try:
