...
# [GCC 8.4.0]
# Embedded file name: /Users/sage/fatedoc/FATE/fate_flow/apps/tracking_app.py
# Compiled at: 2020-04-28 09:19:05
# Size of source mod 2**32: 18036 bytes
    ...
    if job_view_data:
        ...
        for metric_namespace, namespace_metrics in job_metric_list.items():
            ...
            for metric_name in namespace_metrics:
                job_view_data['model_summary'][metric_namespace][metric_name] = job_view_data['model_summary'][metric_namespace].get(metric_name, {})
                for metric_data in job_tracker.get_job_metric_data(metric_namespace=metric_namespace, metric_name=metric_name):
...
def component_metric_all():
    request_data = request.json
    check_request_parameters(request_data)
    tracker = Tracking(job_id=(request_data['job_id']), component_name=(request_data['component_name']), role=(request_data['role']),
    ...
    if metrics:
        for metric_namespace, metric_names in metrics.items():
            ...
            for metric_name in metric_names:
                ...
                metric_data, metric_meta = get_metric_all_data(tracker=tracker, metric_namespace=metric_namespace, metric_name=metric_name)
...
def component_metric_data():
    ...
    tracker = Tracking(job_id=(request_data['job_id']), component_name=(request_data['component_name']), role=(request_data['role']),
      party_id=(request_data['party_id']))
    metric_data, metric_meta = get_metric_all_data(tracker=tracker, metric_namespace=(request_data['metric_namespace']), metric_name=(request_data['metric_name']))
    if metric_data or metric_meta:
        return get_json_result(retcode=0, retmsg='success', data=metric_data, meta=metric_meta)
    return get_json_result(retcode=0, retmsg='no data', data=[], meta={})


def get_metric_all_data(tracker, metric_namespace, metric_name):
    metric_data = tracker.get_metric_data(metric_namespace=metric_namespace, metric_name=metric_name)
    metric_meta = tracker.get_metric_meta(metric_namespace=metric_namespace, metric_name=metric_name)
    if metric_data or metric_meta:
    ...
    if job_dsl_parser:
        ...
        for role, partys_parameters in parameters.items():
            for party_parameters in partys_parameters:
                if party_parameters.get('local', {}).get('role', '') == request_data['role'] and party_parameters.get('local', {}).get('party_id', '') == int(request_data['party_id']):
                    output_parameters = {}
...
def component_output_model():
    request_data = request.json
    check_request_parameters(request_data)
    job_dsl, job_runtime_conf, train_runtime_conf = job_utils.get_job_configuration(job_id=(request_data['job_id']), role=(request_data['role']),
      party_id=(request_data['party_id']))
    ...
    dag = job_utils.get_job_dsl_parser(dsl=job_dsl, runtime_conf=job_runtime_conf, train_runtime_conf=train_runtime_conf)
    ...
    if output_model_json:
        ...
        for buffer_name, buffer_object in output_model.items():
            if buffer_name.endswith('Meta'):
                this_component_model_meta['meta_data'] = json_format.MessageToDict(buffer_object, including_default_value_fields=True)
...
def component_output_data_download():
    ...
    if output_data_count:
        ...
        tar_file_name = 'job_{}_{}_{}_{}_output_data.tar.gz'.format(request_data['job_id'], request_data['component_name'], request_data['role'], request_data['party_id'])
...
def component_output_data_table():
    ...
    if data_views:
        return get_json_result(retcode=0, retmsg='success', data={'table_name':data_views[0].f_table_name,  'table_namespace':data_views[0].f_table_namespace})
...
def save_metric_data(job_id, component_name, task_id, role, party_id):
    ...
    tracker.save_metric_data(metric_namespace=(request_data['metric_namespace']), metric_name=(request_data['metric_name']), metrics=metrics,
...
def save_metric_meta(job_id, component_name, task_id, role, party_id):
    ...
    tracker.save_metric_meta(metric_namespace=(request_data['metric_namespace']), metric_name=(request_data['metric_name']), metric_meta=metric_meta,
      job_level=(request_data['job_level']))
    return get_json_result()
...
def get_component_output_data_table(task_data):
    ...
    tracker = Tracking(job_id=(task_data['job_id']), component_name=(task_data['component_name']), role=(task_data['role']),
      party_id=(task_data['party_id']))
...
def check_request_parameters(request_data):
    with DB.connection_context():
        if 'role' not in request_data:
            if 'party_id' not in request_data:
