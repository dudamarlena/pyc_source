...
# [GCC 8.4.0]
# Embedded file name: /Users/sage/fatedoc/FATE/arch/api/session.py
# Compiled at: 2020-05-06 02:27:06
# Size of source mod 2**32: 14031 bytes
import os, typing, uuid
...
def init(job_id=None, mode: typing.Union[(int, WorkMode)]=WorkMode.STANDALONE, backend: typing.Union[(int, Backend)]=Backend.EGGROLL, persistent_engine: str=StoreTypes.ROLLPAIR_LMDB, eggroll_version=None, set_log_dir=True, options: dict=None):
    """
    ...
    if job_id is None:
        ...
        if set_log_dir:
        ...
        elif eggroll_version is None:
            eggroll_version = _EGGROLL_VERSION
        elif backend.is_eggroll():
            if eggroll_version < 2:
                ...
                builder = build.Builder(session_id=job_id, work_mode=mode, persistent_engine=persistent_engine, options=options)
        else:
            if backend.is_spark():
                if eggroll_version < 2:
...
@log_elapsed
def table(name, namespace=None, partition=1, persistent=True, create_if_missing=True, error_if_exist=False, in_place_computing=False, **kwargs) -> Table:
    """
    ...
    return (RuntimeInstance.SESSION.table)(name=name, namespace=namespace, 
     partition=partition, 
     persistent=persistent, 
     in_place_computing=in_place_computing, 
     create_if_missing=create_if_missing, 
     error_if_exist=error_if_exist, **kwargs)


@log_elapsed
def parallelize(data: Iterable, include_key=False, name=None, partition=None, namespace=None, persistent=False, create_if_missing=True, error_if_exist=False, chunk_size=100000, in_place_computing=False) -> Table:
    """
    ...
    return RuntimeInstance.SESSION.parallelize(data=data, include_key=include_key, name=name, partition=partition, namespace=namespace,
      persistent=persistent,
...
def save_data_table_meta(kv, data_table_name, data_table_namespace):
    ...
    """
    return RuntimeInstance.SESSION.save_data_table_meta(kv=kv, data_table_name=data_table_name,
...
def get_data_table_meta(key, data_table_name, data_table_namespace):
    ...
    """
    return RuntimeInstance.SESSION.get_data_table_meta(key=key, data_table_name=data_table_name,
      data_table_namespace=data_table_namespace)
...
def get_data_table_metas(data_table_name, data_table_namespace):
    ...
    """
    return RuntimeInstance.SESSION.get_data_table_metas(data_table_name=data_table_name, data_table_namespace=data_table_namespace)
...
def save_data(kv_data: Iterable, name, namespace, partition=1, persistent: bool=True, create_if_missing=True, error_if_exist=False, in_version: bool=False, version_log=None):
    """
    ...
    """
    return RuntimeInstance.SESSION.save_data(kv_data=kv_data, name=name,
      namespace=namespace,
      partition=partition,
      persistent=persistent,
      create_if_missing=create_if_missing,
