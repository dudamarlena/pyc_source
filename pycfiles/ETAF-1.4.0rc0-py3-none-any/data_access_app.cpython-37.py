...
# [GCC 8.4.0]
# Embedded file name: /Users/sage/fatedoc/FATE/fate_flow/apps/data_access_app.py
...
def download_upload(access_module):
    ...
    if access_module == 'upload':
        if USE_LOCAL_DATA:
            file = request.json and request.json.get('module') or request.files.get('file')
        ...
        required_arguments = [
         'work_mode', 'namespace', 'table_name']
        if access_module == 'upload':
    ...
    if access_module == 'upload':
        ...
        if count:
            if int(request_config.get('drop', 2)) == 2:
                return get_json_result(retcode=100, retmsg=('The data table already exists, table data count:{}.If you still want to continue uploading, please add the parameter -drop. 0 means not to delete and continue uploading, 1 means to upload again after deleting the table'.format(count)))
        if count:
            if int(request_config.get('drop', 2)) == 1:
                data_table.destroy()
    job_dsl, job_runtime_conf = gen_data_access_job_config(request_config, access_module)
    job_id, job_dsl_path, job_runtime_conf_path, logs_directory, model_info, board_url = JobController.submit_job({'job_dsl':job_dsl, 
     'job_runtime_conf':job_runtime_conf},
      job_id=job_id)
    data.update({'job_dsl_path':job_dsl_path,  'job_runtime_conf_path':job_runtime_conf_path,  'board_url':board_url, 
...
def get_upload_info(jobs_run_conf):
    ...
    for job_id, job_run_conf in jobs_run_conf.items():
        ...
        info['upload_info'] = {'table_name':table_name, 
         'namespace':namespace, 
         'partition':partition, 
...
def gen_data_access_job_config(config_data, access_module):
    job_runtime_conf = {'initiator':{},  'job_parameters':{},  'role':{},  'role_parameters':{}}
    ...
    job_dsl = {'components': {}}
    if access_module == 'upload':
        job_runtime_conf['role_parameters'][initiator_role] = {'upload_0': {'work_mode':[
                       int(config_data['work_mode'])], 
                      'head':[
                       int(config_data['head'])], 
                      'partition':[
                       int(config_data['partition'])], 
                      'file':[
                       config_data['file']], 
                      'namespace':[
                      ...
                      'in_version':[
                       config_data.get('in_version')]}}
        job_dsl['components']['upload_0'] = {'module': 'Upload'}
    if access_module == 'download':
        job_runtime_conf['role_parameters'][initiator_role] = {'download_0': {'work_mode':[
                         config_data['work_mode']], 
                        'delimitor':[
                         config_data.get('delimitor', ',')], 
                        'output_path':[
                        ...
                        'table_name':[
                         config_data['table_name']]}}
        job_dsl['components']['download_0'] = {'module': 'Download'}
    return (
