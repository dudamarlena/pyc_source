...
# [GCC 8.4.0]
# Embedded file name: /Users/sage/fatedoc/FATE/fate_flow/utils/authentication_utils.py
# Compiled at: 2020-04-28 09:16:53
# Size of source mod 2**32: 15354 bytes
import functools, json, os, re
from flask import request
from arch.api.utils import file_utils
from fate_flow.settings import USE_AUTHENTICATION, PRIVILEGE_COMMAND_WHITELIST, stat_logger, CLUSTER_STANDALONE_JOB_SERVER_PORT

class PrivilegeAuth(object):
    privilege_cache = {}
    local_storage_file = ''
    USE_LOCAL_STORAGE = True
    ALL_PERMISSION = {'privilege_role':['guest', 'host', 'arbiter'],  'privilege_command':[],  'privilege_component':[]}
    ...
    def authentication_privilege(src_party_id, src_role, request_path, func_name):
        if request.url_root.split(':')[(-1)].split('/')[0] == str(CLUSTER_STANDALONE_JOB_SERVER_PORT):
            return
        if not int(PrivilegeAuth.get_dest_party_id(request_path, func_name)) or src_party_id == PrivilegeAuth.get_dest_party_id(request_path, func_name):
            return
        if src_role != 'guest':
        ...
        for privilege_type, value in privilege_dic.items():
            if value in PrivilegeAuth.command_whitelist:
                continue
            if value and value not in PrivilegeAuth.privilege_cache.get(src_party_id, {}).get(src_role, {}).get(privilege_type, []) and value not in PrivilegeAuth.get_permission_config(src_party_id, src_role).get(privilege_type, []):
    ...
    def get_new_permission_config(src_party_id, src_role, privilege_role, privilege_command, privilege_component, delete):
        with open(PrivilegeAuth.local_storage_file) as (f):
            stat_logger.info('add permissions: src_party_id {} src_role {} privilege_role {} privilege_command {} privilege_component {}'.format(src_party_id, src_role, privilege_role, privilege_command, privilege_component))
                ...
                if values:
                    for value in values.split(','):
                        ...
                        if value not in PrivilegeAuth.ALL_PERMISSION.get(privilege_type):
                            if value != PrivilegeAuth.ALL_PERMISSION.get(privilege_type):
                                ...
                                if local_storage.get(src_party_id, {}):
                                    if local_storage.get(src_party_id).get(src_role, {}):
                                        if local_storage.get(src_party_id).get(src_role).get(privilege_type):
                                            local_storage[src_party_id][src_role][privilege_type].append(value) if isinstance(value, str) else local_storage[src_party_id][src_role][privilege_type].extend(value)
                                        else:
                                            local_storage[src_party_id][src_role][privilege_type] = [value] if isinstance(value, str) else value
                                else:
                                    local_storage[src_party_id][src_role] = {privilege_type: [value]} if isinstance(value, str) else {privilege_type: value}
                            else:
                                local_storage[src_party_id] = {src_role: {privilege_type: [value]}} if isinstance(value, str) else {src_role: {privilege_type: value}}
                            local_storage[src_party_id][src_role][privilege_type] = list(set(local_storage[src_party_id][src_role][privilege_type]))
                        else:
                            try:
                                ...
                                stat_logger.info('{} {} is not authorized ,it cannot be deleted'.format(privilege_type.split('_')[1], value) if isinstance(value, str) else 'No permission to delete')
                                raise Exception('{} {} is not authorized ,it cannot be deleted'.format(privilege_type.split('_')[1], value) if isinstance(value, str) else 'No permission to delete')
    ...
    def get_dest_party_id(request_path, func_name):
        if 'task' not in func_name:
            return request_path.split('/')[3]
        return request_path.split('/')[5]

    @staticmethod
    def get_privilege_dic(privilege_role, privilege_command, privilege_component):
        return {'privilege_role':privilege_role,  'privilege_command':privilege_command, 
    ...
    def init():
        if USE_AUTHENTICATION:
            ...
            component_path = os.path.join(file_utils.get_project_base_directory(), 'federatedml', 'conf', 'setting_conf')
            command_file_path = os.path.join(file_utils.get_project_base_directory(), 'fate_flow', 'apps', 'schedule_app.py')
...
def modify_permission(permission_info, delete=False):
    if PrivilegeAuth.USE_LOCAL_STORAGE:
        new_json = PrivilegeAuth.get_new_permission_config(src_party_id=(permission_info.get('src_party_id')), src_role=(permission_info.get('src_role')),
          privilege_role=(permission_info.get('privilege_role', None)),
          privilege_command=(permission_info.get('privilege_command', None)),
          privilege_component=(permission_info.get('privilege_component', None)),
          delete=delete)
...
def request_authority_certification(func):
    ...
    def _wrapper(*args, **kwargs):
        if USE_AUTHENTICATION:
            PrivilegeAuth.authentication_privilege(src_party_id=(str(request.json.get('src_party_id'))), src_role=(request.json.get('src_role')),
...
def authentication_check(src_role, src_party_id, dsl, runtime_conf, role, party_id):
    ...
    if need_run_commond != PrivilegeAuth.privilege_cache.get(src_party_id, {}).get(src_role, {}).get('privilege_command', []):
        if need_run_commond != PrivilegeAuth.get_permission_config(src_party_id, src_role).get('privilege_command', []):
            stat_logger.info('src_role {} src_party_id {} commond authentication that needs to be run failed:{}'.format(src_role, src_party_id, set(need_run_commond) - set(PrivilegeAuth.privilege_cache.get(src_party_id, {}).get(src_role, {}).get('privilege_command', []))))
            raise Exception('src_role {} src_party_id {} commond authentication that needs to be run failed:{}'.format(src_role, src_party_id, set(need_run_commond) - set(PrivilegeAuth.privilege_cache.get(src_party_id, {}).get(src_role, {}).get('privilege_command', []))))
    if not set(components).issubset(PrivilegeAuth.privilege_cache.get(src_party_id, {}).get(src_role, {}).get('privilege_component', [])):
        if not set(components).issubset(PrivilegeAuth.get_permission_config(src_party_id, src_role).get('privilege_component', [])):
            stat_logger.info('src_role {} src_party_id {} component authentication that needs to be run failed:{}'.format(src_role, src_party_id, components))
            raise Exception('src_role {} src_party_id {} component authentication that needs to be run failed:{}'.format(src_role, src_party_id, set(components) - set(PrivilegeAuth.privilege_cache.get(src_party_id, {}).get(src_role, {}).get('privilege_component', []))))
