...
# [GCC 8.4.0]
# Embedded file name: /Users/sage/fatedoc/FATE/fate_flow/driver/task_executor.py
# Compiled at: 2020-05-06 03:08:10
# Size of source mod 2**32: 16911 bytes
import argparse, importlib, os, traceback
...
class TaskExecutor(object):
    ...
    def run_task():
        ...
        try:
            try:
                ...
                TaskExecutor.sync_task_status(job_id=job_id, component_name=component_name, task_id=task_id, role=role, party_id=party_id,
                  initiator_party_id=(job_initiator.get('party_id', None)),
                  initiator_role=(job_initiator.get('role', None)),
                  task_info=(task.to_json()))
                RuntimeConfig.init_config(WORK_MODE=(job_parameters['work_mode']), BACKEND=(job_parameters.get('backend', 0)))
                if args.processors_per_node and args.processors_per_node > 0 and RuntimeConfig.BACKEND == Backend.EGGROLL:
                    ...
                    session_options = {}
                session.init(job_id=(job_utils.generate_session_id(task_id, role, party_id)), mode=(RuntimeConfig.WORK_MODE),
                ...
                task_run_args = TaskExecutor.get_task_run_args(job_id=job_id, role=role, party_id=party_id, task_id=task_id,
        ...
        schedule_logger().info('finish {} {} {} {} {} {} task'.format(job_id, component_name, task_id, role, party_id, task.f_status if sync_success else TaskStatus.FAILED))
            ...
            if input_type == 'data':
                ...
                for data_type, data_list in input_detail.items():
                    for data_key in data_list:
                        ...
                        if search_component_name == 'args':
                            if job_args.get('data', {}).get(search_data_name).get('namespace', ''):
                                if job_args.get('data', {}).get(search_data_name).get('name', ''):
                                    data_table = session.table(namespace=(job_args['data'][search_data_name]['namespace']),
                                      name=(job_args['data'][search_data_name]['name']))
                                else:
                                    data_table = None
                            else:
                                data_table = Tracking(job_id=job_id, role=role, party_id=party_id, component_name=search_component_name).get_output_data_table(data_name=search_data_name)
                        else:
                            ...
                            if SAVE_AS_TASK_INPUT_DATA_SWITCH:
                                if data_table:
                                    schedule_logger().info('start save as task {} input data table {} {}'.format(task_id, data_table.get_namespace(), data_table.get_name()))
                                    ...
                                    data_table = data_table.save_as(namespace=job_utils.generate_session_id(task_id=task_id, role=role,
                                      party_id=party_id),
                                    ...
                                    schedule_logger().info('save as task {} input data table to {} {} done'.format(task_id, data_table.get_namespace(), data_table.get_name()))
                                else:
                ...
                for dsl_model_key in input_detail:
                    ...
                    if len(dsl_model_key_items) == 2:
                        ...
                        if len(dsl_model_key_items) == 3 and dsl_model_key_items[0] == 'pipeline':
                    ...
                    models = Tracking(job_id=job_id, role=role, party_id=party_id, component_name=search_component_name, model_id=(job_parameters['model_id']),
                      model_version=(job_parameters['model_version'])).get_output_model(model_alias=search_model_alias)
    ...
    def get_parameters(job_id, component_name, role, party_id):
        ...
        job_dsl_parser = job_utils.get_job_dsl_parser(dsl=(job_conf_dict['job_dsl_path']), runtime_conf=(job_conf_dict['job_runtime_conf_path']),
    ...
    def sync_task_status(job_id, component_name, task_id, role, party_id, initiator_party_id, initiator_role, task_info, update=False):
        ...
        for dest_party_id in {party_id, initiator_party_id}:
            if party_id != initiator_party_id:
                if dest_party_id == initiator_party_id:
                    task_info['f_run_ip'] = ''
            response = federated_api(job_id=job_id, method='POST',
              endpoint=('/{}/schedule/{}/{}/{}/{}/{}/status'.format(API_VERSION, job_id, component_name, task_id, role, party_id)),
        ...
        if not sync_success:
            if not update:
                ...
                TaskExecutor.sync_task_status(job_id, component_name, task_id, role, party_id, initiator_party_id, initiator_role,
                  task_info, update=True)
        if update:
