...
# [GCC 8.4.0]
# Embedded file name: /Users/sage/fatedoc/FATE/federatedml/param/intersect_param.py
# Compiled at: 2020-04-28 09:16:53
# Size of source mod 2**32: 7476 bytes
import copy
...
class EncodeParam(BaseParam):
    ...
    def check(self):
        if type(self.salt).__name__ != 'str':
            raise ValueError("encode param's salt {} not supported, should be str type".format(self.salt))
        ...
        if type(self.base64).__name__ != 'bool':
            raise ValueError("encode param's base64 {} not supported, should be bool type".format(self.base64))
...
class IntersectCache(BaseParam):
    ...
    def check(self):
        if type(self.use_cache).__name__ != 'bool':
            raise ValueError("encode param's salt {} not supported, should be bool type".format(self.use_cache))
        descr = "intersect cache param's "
        self.check_and_change_lower(self.id_type, [
         consts.PHONE, consts.IMEI], descr)
        self.check_and_change_lower(self.encrypt_type, [
         consts.MD5, consts.SHA256], descr)
...
class IntersectParam(BaseParam):
    ...
    def __init__(self, intersect_method=consts.RAW, random_bit=128, sync_intersect_ids=True, join_role='guest', with_encode=False, only_output_key=False, encode_params=EncodeParam(), intersect_cache_param=IntersectCache(), repeated_id_process=False, repeated_id_owner='guest'):
        super().__init__()
    ...
    def check(self):
        ...
        self.intersect_method = self.check_and_change_lower(self.intersect_method, [
         consts.RSA, consts.RAW], descr)
        if type(self.random_bit).__name__ not in ('int', ):
            raise ValueError("intersect param's random_bit {} not supported, should be positive integer".format(self.random_bit))
        if type(self.sync_intersect_ids).__name__ != 'bool':
            raise ValueError("intersect param's sync_intersect_ids {} not supported, should be bool type".format(self.sync_intersect_ids))
        self.join_role = self.check_and_change_lower(self.join_role, [
         consts.GUEST, consts.HOST], descr)
        if type(self.with_encode).__name__ != 'bool':
            raise ValueError("intersect param's with_encode {} not supported, should be bool type".format(self.with_encode))
        if type(self.only_output_key).__name__ != 'bool':
            raise ValueError("intersect param's only_output_key {} not supported, should be bool type".format(self.only_output_key))
        if type(self.repeated_id_process).__name__ != 'bool':
            raise ValueError("intersect param's repeated_id_process {} not supported, should be bool type".format(self.repeated_id_process))
