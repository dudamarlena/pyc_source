...
# [GCC 8.4.0]
# Embedded file name: /Users/sage/fatedoc/FATE/fate_flow/manager/model_manager/pipelined_model.py
# Compiled at: 2020-04-28 09:19:05
# Size of source mod 2**32: 11776 bytes
import importlib, inspect, os, shutil, base64
...
class PipelinedModel(object):
    ...
    def create_pipelined_model(self):
        if os.path.exists(self.model_path):
            raise Exception('Model creation failed because it has already been created, model cache path is {}'.format(self.model_path))
        else:
    ...
    def save_component_model(self, component_name, component_module_name, model_alias, model_buffers):
        ...
        self.update_component_meta(component_name=component_name, component_module_name=component_module_name,
    ...
    def read_component_model(self, component_name, model_alias):
        ...
        for model_name, buffer_name in model_proto_index.items():
            with open(os.path.join(component_model_storage_path, model_name), 'rb') as (fr):
                ...
                model_buffers[model_name] = self.parse_proto_object(buffer_name=buffer_name, buffer_object_serialized_string=buffer_object_serialized_string)
    ...
    def packaging_model(self):
        ...
        stat_logger.info('Make model {} {} archive on {} successfully'.format(self.model_id, self.model_version, archive_file_path))
