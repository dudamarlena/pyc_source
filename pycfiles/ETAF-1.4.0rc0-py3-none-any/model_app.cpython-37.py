...
# [GCC 8.4.0]
# Embedded file name: /Users/sage/fatedoc/FATE/fate_flow/apps/model_app.py
# Compiled at: 2020-04-28 09:19:05
# Size of source mod 2**32: 9065 bytes
import os, shutil
...
from fate_flow.settings import stat_logger, API_VERSION, SERVING_PATH, SERVINGS_ZK_PATH, USE_CONFIGURATION_CENTER, ZOOKEEPER_HOSTS, SERVER_CONF_PATH, DEFAULT_MODEL_STORE_ADDRESS, TEMP_DIRECTORY
...
def load_model():
    ...
    for role_name, role_partys in request_config.get('role').items():
        ...
        for _party_id in role_partys:
            request_config['local'] = {'role':role_name, 
             'party_id':_party_id}
            try:
                response = federated_api(job_id=_job_id, method='POST',
...
def do_load_model():
    ...
    request_data['servings'] = CenterConfig.get_settings(path=SERVING_PATH, servings_zk_path=SERVINGS_ZK_PATH, use_zk=USE_CONFIGURATION_CENTER,
...
def bind_model_service():
    ...
    if not request_config.get('servings'):
        request_config['servings'] = CenterConfig.get_settings(path=SERVING_PATH, servings_zk_path=SERVINGS_ZK_PATH, use_zk=USE_CONFIGURATION_CENTER,
          hosts=ZOOKEEPER_HOSTS,
          server_conf_path=SERVER_CONF_PATH)
    else:
        return request_config.get('service_id') or get_json_result(retcode=101, retmsg='no service id')
...
def save_table_version():
    save_version(name=(request.json.get('name')), namespace=(request.json.get('namespace')),
...
def gen_model_operation_job_config(config_data: dict, model_operation: ModelOperation):
    ...
    if model_operation in [ModelOperation.STORE, ModelOperation.RESTORE]:
        ...
        job_dsl['components'][component_name] = {'module': 'Model{}'.format(model_operation.capitalize())}
    else:
