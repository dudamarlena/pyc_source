...
# [GCC 8.4.0]
# Embedded file name: /Users/sage/fatedoc/FATE/fate_flow/utils/grpc_utils.py
...
import grpc
from fate_flow.settings import ROLE, IP, GRPC_PORT, PROXY_HOST, PROXY_PORT, HEADERS, DEFAULT_GRPC_OVERALL_TIMEOUT, audit_logger
...
class UnaryServicer(proxy_pb2_grpc.DataTransferServiceServicer):
    ...
    def unaryCall(self, _request, context):
        ...
        try:
            nodes_check(param_dict.get('src_party_id'), param_dict.get('src_role'), param_dict.get('appKey'), param_dict.get('appSecret'))
        except Exception as e:
            try:
                resp_json = {'retcode':100, 
                 'retmsg':str(e)}
