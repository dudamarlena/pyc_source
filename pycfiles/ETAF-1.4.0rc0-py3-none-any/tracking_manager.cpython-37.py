...
# [GCC 8.4.0]
# Embedded file name: /Users/sage/fatedoc/FATE/fate_flow/manager/tracking_manager.py
# Compiled at: 2020-05-06 03:08:10
# Size of source mod 2**32: 28082 bytes
...
class Tracking(object):
    ...
    def __init__(self, job_id: str, role: str, party_id: int, model_id: str=None, model_version: str=None, component_name: str=None, component_module_name: str=None, task_id: str=None):
        self.job_id = job_id
        self.role = role
        ...
        self.task_id = task_id if task_id else job_utils.generate_task_id(job_id=(self.job_id), component_name=(self.component_name))
        ...
        if self.party_model_id:
            if self.model_version:
                self.pipelined_model = pipelined_model.PipelinedModel(model_id=(self.party_model_id), model_version=(self.model_version))

    def log_job_metric_data(self, metric_namespace: str, metric_name: str, metrics: List[Metric]):
        self.save_metric_data_remote(metric_namespace=metric_namespace, metric_name=metric_name, metrics=metrics, job_level=True)

    def log_metric_data(self, metric_namespace: str, metric_name: str, metrics: List[Metric]):
        self.save_metric_data_remote(metric_namespace=metric_namespace, metric_name=metric_name, metrics=metrics, job_level=False)

    def save_metric_data_remote(self, metric_namespace: str, metric_name: str, metrics: List[Metric], job_level=False):
        schedule_logger(self.job_id).info('request save job {} component {} on {} {} {} {} metric data'.format(self.job_id, self.component_name, self.role, self.party_id, metric_namespace, metric_name))
        request_body = dict()
        ...
        response = api_utils.local_api(method='POST', endpoint=('/{}/tracking/{}/{}/{}/{}/{}/metric_data/save'.format(API_VERSION, self.job_id, self.component_name, self.task_id, self.role, self.party_id)),
          json_body=request_body)
        return response['retcode'] == 0

    def save_metric_data(self, metric_namespace: str, metric_name: str, metrics: List[Metric], job_level=False):
        schedule_logger(self.job_id).info('save job {} component {} on {} {} {} {} metric data'.format(self.job_id, self.component_name, self.role, self.party_id, metric_namespace, metric_name))
        kv = []
    ...
    def set_metric_meta(self, metric_namespace: str, metric_name: str, metric_meta: MetricMeta, job_level: bool=False):
        self.save_metric_meta_remote(metric_namespace=metric_namespace, metric_name=metric_name, metric_meta=metric_meta,
          job_level=job_level)

    def save_metric_meta_remote(self, metric_namespace: str, metric_name: str, metric_meta: MetricMeta, job_level: bool=False):
        schedule_logger(self.job_id).info('request save job {} component {} on {} {} {} {} metric meta'.format(self.job_id, self.component_name, self.role, self.party_id, metric_namespace, metric_name))
        request_body = dict()
        request_body['metric_namespace'] = metric_namespace
        request_body['metric_name'] = metric_name
        request_body['metric_meta'] = serialize_b64(metric_meta, to_str=True)
        request_body['job_level'] = job_level
        response = api_utils.local_api(method='POST', endpoint=('/{}/tracking/{}/{}/{}/{}/{}/metric_meta/save'.format(API_VERSION, self.job_id, self.component_name, self.task_id, self.role, self.party_id)),
          json_body=request_body)
        return response['retcode'] == 0

    def save_metric_meta(self, metric_namespace: str, metric_name: str, metric_meta: MetricMeta, job_level: bool=False):
        schedule_logger(self.job_id).info('save job {} component {} on {} {} {} {} metric meta'.format(self.job_id, self.component_name, self.role, self.party_id, metric_namespace, metric_name))
    ...
    def get_metric_list(self, job_level: bool=False):
        with DB.connection_context():
            metrics = dict()
            query_sql = 'select distinct f_metric_namespace, f_metric_name from t_tracking_metric_{} where f_job_id = "{}" and f_component_name = "{}" and f_role = "{}" and f_party_id = "{}" and f_task_id = "{}"'.format(self.get_table_index(), self.job_id, self.component_name if not job_level else 'dag', self.role, self.party_id, self.task_id)
    ...
    def save_output_data_table(self, data_table: Table, data_name: str='component'):
        ...
        if data_table:
            ...
            schedule_logger(self.job_id).info('persisting the component output temporary table: {} {} to {} {}'.format(data_table.get_namespace(), data_table.get_name(), persistent_table_namespace, persistent_table_name))
            persistent_table = data_table.save_as(namespace=persistent_table_namespace,
              name=persistent_table_name)
            session.save_data_table_meta({'schema':data_table.schema, 
            ...
            data_table_info = {data_name: {'name':persistent_table.get_name(),  'namespace':persistent_table.get_namespace()}}
        else:
            data_table_info = {}
        session.save_data((data_table_info.items()),
          ...
          partition=48)
        self.save_data_view((self.role), (self.party_id), data_info={'f_table_name':persistent_table._name if data_table else '', 
    ...
    def get_output_data_table(self, data_name: str='component'):
        ...
        :return:
        """
        output_data_info_table = session.table(name=(Tracking.output_table_name('data')), namespace=(self.table_namespace))
        data_table_info = output_data_info_table.get(data_name)
        if data_table_info:
            data_table = session.table(name=(data_table_info.get('name', '')), namespace=(data_table_info.get('namespace', '')))
    ...
    def save_output_model(self, model_buffers: dict, model_alias: str):
        if model_buffers:
            self.pipelined_model.save_component_model(component_name=(self.component_name), component_module_name=(self.module_name),
              model_alias=model_alias,
              model_buffers=model_buffers)
            self.save_data_view((self.role), (self.party_id), data_info={'f_party_model_id':self.party_model_id, 
             'f_model_version':self.model_version})

    def get_output_model(self, model_alias):
        model_buffers = self.pipelined_model.read_component_model(component_name=(self.component_name), model_alias=model_alias)
    ...
    def insert_data_to_db(self, metric_namespace: str, metric_name: str, data_type: int, kv, job_level=False):
        with DB.connection_context():
            try:
                ...
                self.bulk_insert_model_data(TrackingMetric.model(table_index=(self.get_table_index())), tracking_metric_data_source)
    ...
    def read_data_from_db(self, metric_namespace: str, metric_name: str, data_type, job_level=False):
        with DB.connection_context():
            metrics = []
            try:
                query_sql = 'select f_key, f_value from t_tracking_metric_{} where f_job_id = "{}" and f_component_name = "{}" and f_role = "{}" and f_party_id = "{}"and f_task_id = "{}" and f_metric_namespace = "{}" and f_metric_name= "{}" and f_type="{}" order by f_id'.format(self.get_table_index(), self.job_id, self.component_name if not job_level else 'dag', self.role, self.party_id, self.task_id, metric_namespace, metric_name, data_type)
                cursor = DB.execute_sql(query_sql)
                for row in cursor.fetchall():
                    yield (
                     deserialize_b64(row[0]), deserialize_b64(row[1]))
    ...
    def save_task(self, role, party_id, task_info):
        with DB.connection_context():
            tasks = Task.select().where(Task.f_job_id == self.job_id, Task.f_component_name == self.component_name, Task.f_task_id == self.task_id, Task.f_role == role, Task.f_party_id == party_id)
            is_insert = True
            if tasks:
                task = tasks[0]
                is_insert = False
            ...
            if not 'f_status' in task_info or task.f_status in [TaskStatus.COMPLETE, TaskStatus.FAILED]:
                for k, v in task_info.items():
                    try:
                        if k in ('f_job_id', 'f_component_name', 'f_task_id', 'f_role',
                                 'f_party_id') or v == getattr(Task, k).default:
                            continue
                    except:
                        pass
    ...
    def save_data_view(self, role, party_id, data_info, mark=False):
        with DB.connection_context():
            data_views = DataView.select().where(DataView.f_job_id == self.job_id, DataView.f_component_name == self.component_name, DataView.f_task_id == self.task_id, DataView.f_role == role, DataView.f_party_id == party_id)
            is_insert = True
            if mark:
                if self.component_name == 'upload_0':
                    return
            else:
                if data_views:
                ...
                for k, v in data_info.items():
                    if not k in ('f_job_id', 'f_component_name', 'f_task_id', 'f_role',
                                 'f_party_id'):
                        if v == getattr(DataView, k).default:
                            continue
    ...
    def clean_task(self, roles, party_ids):
        schedule_logger(self.job_id).info('clean task {} on {} {}'.format(self.task_id, self.role, self.party_id))
        try:
            for role in roles.split(','):
                for party_id in party_ids.split(','):
                    namespace_clean = job_utils.generate_session_id(task_id=(self.task_id), role=role,
                      party_id=party_id)
                    schedule_logger(self.job_id).info('clean table by namespace {} on {} {}'.format(namespace_clean, self.role, self.party_id))
                    session.clean_tables(namespace=namespace_clean, regex_string='*')
                    schedule_logger(self.job_id).info('clean table by namespace {} on {} {} done'.format(namespace_clean, self.role, self.party_id))
        ...
        schedule_logger(self.job_id).info('clean task {} on {} {} done'.format(self.task_id, self.role, self.party_id))
    ...
    def get_table_namespace(self, job_level: bool=False):
        if not job_level:
            return self.table_namespace
        return self.job_table_namespace
