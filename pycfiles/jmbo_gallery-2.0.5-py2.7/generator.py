...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gallery/generator.py
...
def generate():
    ...
    for i in range(1, GALLERY_COUNT + 1):
        objects.append({'model': 'gallery.Gallery', 
           'fields': {'title': 'Gallery %s Title' % i, 
                      'state': 'published', 
    ...
    for i in range(1, GALLERY_COUNT * 3):
        objects.append({'model': 'gallery.GalleryImage', 
           'fields': {'title': 'Gallery Image %s Title' % i, 
                      'state': 'published', 
                      'gallery': {'model': 'gallery.Gallery', 
                                  'fields': {'title': 'Gallery %s Title' % (i / 3 + 1)}}, 
                      'image': random.sample(IMAGES, 1)[0], 
                      'sites': {'model': 'sites.Site', 
                                'fields': {'name': 'example.com'}}}})
        objects.append({'model': 'gallery.VideoEmbed', 
           'fields': {'title': 'Video Embed %s Title' % i, 
                      ...
                      'sites': {'model': 'sites.Site', 
                                'fields': {'name': 'example.com'}}}})
        objects.append({'model': 'gallery.VideoFile', 
           'fields': {'title': 'Video File %s Title' % i, 
                      'state': 'published', 
