...
# [GCC 8.4.0]
# Embedded file name: /cygdrive/c/Users/Nad/haufe.sharepoint-0.1.9/haufe/sharepoint/connector.py
# Compiled at: 2014-12-12 20:15:00
import re, logging, sys, os, base64, urllib2
...
field_regex = re.compile('^_[a-zA-Z0-9]')
_marker = object
CLIENT = dict()
MODEL = dict()
CONNECTED = False
...
class Factory(object):
    ...
    def generate(self):
        global VERBOSE
        return Connector(CLIENT['url'], CLIENT['username'], CLIENT['password'], CLIENT['list_id'], CLIENT['NTLM'], VERBOSE, CLIENT['timeout'])
...
def Connector(url, username, password, list_id, NTLM=True, verbose=False, timeout=65):
    global VERBOSE
    if 'connected' not in CLIENT.keys():
...
class ParsedSoapResult(object):
    ...
    def __init__(self, results):
        ...
        for item_result in results:
            d = dict(code=item_result.ErrorCode, success=item_result.ErrorCode == '0x00000000')
...
class ListEndpoint(object):
    ...
    def __init__(self, client, list_id, copy=False):
        ...
        if not CONNECTED:
            ...
            CONNECTED = True
        elif 'fields' not in MODEL.keys() and not copy:
            self.model = MODEL['fields'] = self._getFields()
            self.required_fields = MODEL['required_fields'] = self._required_fields()
            self.all_fields = MODEL['keys'] = self.model.keys()
    ...
    def _find_primary_key(self):
        ...
        for k, field_d in self.model.items():
            if field_d.get('PrimaryKey') == 'TRUE':
                return k
    ...
    def getItem(self, item_id, viewName=None):
        ...
        stdoff()
        result = Factory().generate().client.service.GetListItems(self.list_id, viewName=viewName or self.viewName, query=query0, viewFields=viewfields, queryOptions=queryOptions, rowLimit=1)
        stdon()
    ...
    def query(self, mode='exact', viewName=None, **kw):
        ...
        """
        if mode not in ('exact', 'contains', 'beginswith'):
        ...
        queryOptions = Element('queryOptions')
        stdoff()
        result = Factory().generate().client.service.GetListItems(self.list_id, viewName=viewName or self.viewName, query=query0, viewFields=viewfields, queryOptions=queryOptions)
        stdon()
    ...
    def addAttachment(self, data):
        if 'raw' not in data.keys():
            data['raw'] = False
        if 'overwrite' not in data.keys():
        ...
        if data['overwrite']:
            try:
                self.deleteAttachment(dict(listItemID=data['listItemID'], url=('{0}Attachments/{1}/{2}').format(self.lbase, data['listItemID'], data['fileName'])))
            except:
                pass
    ...
    def upload(self, data):
        ...
        cli = Factory().generate()
        result = cli.client.service.CopyIntoItems(SourceUrl=data['sourceUrl'], DestinationUrls=du, Fields=fit_m, Stream=f)
        stdon()
        return result
    ...
    def getUpload(self, data):
        ...
        result = cli.client.service.GetItem(self.list_id, Url=data['url'])
        stdoff()
        return result
