...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xl_tensorflow\preprocessing\annotation.py
# Compiled at: 2020-04-05 02:37:00
# Size of source mod 2**32: 7370 bytes
...
def voc2txt_annotation(xml_files, train_txt, classes, image_path=None, seperator=' ', encoding='utf-8'):
    ...
    for xml_file in xml_files:
        ...
        if not root.find('object'):
            continue
        train_fp.write(xml_file.replace('xml', 'jpg') if not image_path else os.path.join(image_path, os.path.basename(root.find('filename').text)))
        for obj in root.iter('object'):
            difficult = obj.find('difficult').text
            cls = obj.find('name').text
            if not cls not in classes:
                if int(difficult) == 1:
                    continue
...
def voc2voc_dataset(data_path, target_path, validation_split=None, cat_val=True, subset='train'):
    ...
    if not xml_files:
        return
    xml_files, image_files = zip(*[(xml_file, xml_file.replace('xml', 'jpg')) for xml_file in xml_files if os.path.exists(xml_file.replace('xml', 'jpg'))])
    logging.info(f"Find labeled data: {len(xml_files)}")
    ImageSets, JPEGImages, Annotations = f"{target_path}/ImageSets/Main", f"{target_path}/JPEGImages", f"{target_path}/Annotations"
    os.makedirs(ImageSets, exist_ok=True)
    os.makedirs(JPEGImages, exist_ok=True)
    os.makedirs(Annotations, exist_ok=True)
    for file in xml_files:
        shutil.copy(file, f"{Annotations}")

    for file in image_files:
        shutil.copy(file, f"{JPEGImages}")
    ...
    if not validation_split:
        ...
        if cat_val:
            ...
            files = [list(map(name_map_func, [k for k in file_scanning(f"{data_path}/{d}", 'xml', sub_scan=True) if os.path.exists(k.replace('xml', 'jpg'))])) for d in cats]
...
def voc_merge(voc_07, voc_12, target_path):
    ...
    train = read_txt(f"{voc_07}/ImageSets/Main/train.txt", return_list=True) + read_txt(f"{voc_12}/ImageSets/Main/train.txt",
      return_list=True)
...
def main():
    classes = os.listdir('E:\\Programming\\Python\\8_Ganlanz\\food_recognition\\dataset\\自建数据集\\7_增强图片\\single_pyramid')
    train_text = 'E:\\Programming\\Python\\5_CV\\学习案例\\xl_tf2_yolov3\\model_data\\train.txt'
    path = 'E:\\Programming\\Python\\8_Ganlanz\\food_recognition\\dataset\\自建数据集\\7_增强图片\\single_pyramid'
    xml_files = [i for i in file_scanning(path, sub_scan=True, full_path=True, file_format='xml') if os.path.exists(i.replace('xml', 'jpg'))]
    voc2txt_annotation(xml_files, train_text, classes, seperator='\t')


if __name__ == '__main__':
    voc2voc_dataset('E:\\Programming\\Python\\8_Ganlanz\\food_recognition\\dataset\\自建数据集\\1_真实场景\\0_已标框', 'E:\\Programming\\Python\\8_Ganlanz\\food_recognition\\dataset\\自建数据集\\1_真实场景\\voc',
