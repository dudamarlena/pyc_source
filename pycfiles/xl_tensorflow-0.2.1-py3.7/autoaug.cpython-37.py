...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xl_tensorflow\models\vision\classification\utils\autoaug.py
# Compiled at: 2020-04-26 08:15:28
# Size of source mod 2**32: 38041 bytes
...
def _convert_translation_to_transform(translations: tf.Tensor) -> tf.Tensor:
    ...
    if translations.get_shape().ndims is None:
        ...
        if len(translations.get_shape()) == 1:
            ...
            if len(translations.get_shape()) != 2:
...
def transform(image: tf.Tensor, transforms) -> tf.Tensor:
    ...
    image = to_4d(image)
    image = image_ops.transform(images=image,
...
def rotate(image: tf.Tensor, degrees: float) -> tf.Tensor:
    ...
    transforms = _convert_angles_to_transform(angles=radians, image_width=image_width,
...
def shear_x(image: tf.Tensor, level: float, replace: int) -> tf.Tensor:
    ...
    image = transform(image=(wrap(image)), transforms=[
...
def sharpness(image: tf.Tensor, factor: float) -> tf.Tensor:
    ...
    degenerate = tf.nn.depthwise_conv2d(image,
      kernel, strides, padding='VALID', dilations=[1, 1])
    degenerate = tf.clip_by_value(degenerate, 0.0, 255.0)
...
class ImageAugment(object):
    __doc__ = 'Image augmentation class for applying image distortions.'
...
class AutoAugment(ImageAugment):
    __doc__ = 'Applies the AutoAugment policy to images(input mast be with 0-255).\n\n      AutoAugment is from the paper: https://arxiv.org/abs/1805.09501.\n    '

    def __init__(self, augmentation_name='v0', policies=None, cutout_const=100, translate_const=250):
        """Applies the AutoAugment policy to images.

        Args:
          augmentation_name: The name of the AutoAugment policy to use,valid value:
            v0, test, simple. The available options are `v0` and `test`. `v0` is the policy used for all
          ...
          policies: list of lists of tuples in the form `(func, prob, level)`,
            ...
            argument for `func`.
          cutout_const: multiplier for applying cutout.  recommand 0.05 * target size
          translate_const: multiplier for applying translation. recommand 0.2 * target size
        """
        super(AutoAugment, self).__init__()
        if policies is None:
            self.available_policies = {'v0':self.policy_v0(),  'test':self.policy_test(), 
...
class RandAugment(ImageAugment):
    __doc__ = 'Applies the RandAugment policy to images.\n\n    RandAugment is from the paper https://arxiv.org/abs/1909.13719,\n    '

    def __init__(self, num_layers=2, magnitude=10.0, cutout_const=40.0, translate_const=100.0):
    ...
    def distort(self, image: tf.Tensor) -> tf.Tensor:
        ...
        for _ in range(self.num_layers):
            ...
            for i, op_name in enumerate(self.available_ops):
                prob = tf.random.uniform([], minval=min_prob,
                  maxval=max_prob,
                  dtype=(tf.float32))
