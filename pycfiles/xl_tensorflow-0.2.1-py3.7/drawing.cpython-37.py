...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xl_tensorflow\models\vision\detection\utils\drawing.py
# Compiled at: 2020-04-18 01:58:04
...
def draw_boxes_cv(image, boxes, scores, labels, colors, classid2label):
    ...
    for b, l, s in zip(boxes, labels, scores):
        ...
        cv2.putText(image, label, (xmin, ymax - baseline), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,
                                                                                           0,
                                                                                           0), 1)
...
def draw_boxes_pil(image, out_boxes, out_scores, out_classes, classid2label):
    ...
    hsv_tuples = [(x / len(classid2label), 1.0, 1.0) for x in range(len(classid2label))]
