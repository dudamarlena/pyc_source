...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xl_tensorflow\models\vision\classification\utils\preprocessing.py
# Compiled at: 2020-04-26 08:59:59
...
def mean_image_subtraction(image_bytes: tf.Tensor, means: Tuple[(float, ...)], num_channels: int=3, dtype: tf.dtypes.DType=tf.float32) -> tf.Tensor:
...
def standardize_image(image_bytes: tf.Tensor, stddev: Tuple[(float, ...)], num_channels: int=3, dtype: tf.dtypes.DType=tf.float32) -> tf.Tensor:
...
def normalize_images(features: tf.Tensor, mean_rgb: Tuple[(float, ...)]=MEAN_RGB, stddev_rgb: Tuple[(float, ...)]=STDDEV_RGB, num_channels: int=3, dtype: tf.dtypes.DType=tf.float32, data_format: Text='channels_last') -> tf.Tensor:
...
def decode_and_center_crop(image_bytes: tf.Tensor, image_size: int=IMAGE_SIZE, crop_padding: int=CROP_PADDING, no_crop_padding=True) -> tf.Tensor:
    ...
    shape = tf.shape(image_bytes) if decoded else tf.image.extract_jpeg_shape(image_bytes)
    image_height = shape[0]
    image_width = shape[1]
    if not no_crop_padding:
        ...
        if decoded:
            image = tf.image.crop_to_bounding_box(image_bytes,
    ...
    image = resize_image(image_bytes=image, height=image_size,
      width=image_size)
    return image


def decode_crop_and_flip(image_bytes: tf.Tensor, min_object_covered: float=0.7, aspect_ratio_range: tuple=(0.4, 2.5), area_range: tuple=(0.7, 1.0), max_attempts: int=10) -> tf.Tensor:
    ...
    if decoded:
        cropped = tf.image.crop_to_bounding_box(image_bytes,
          offset_height=offset_height,
          offset_width=offset_width,
...
def resize_image(image_bytes: tf.Tensor, height: int=IMAGE_SIZE, width: int=IMAGE_SIZE) -> tf.Tensor:
    ...
    return tf.compat.v1.image.resize(image_bytes,
      [height, width], method=(tf.image.ResizeMethod.BILINEAR), align_corners=False)


def preprocess_for_eval(image_bytes: tf.Tensor, image_size: int=IMAGE_SIZE, num_channels: int=3, mean_subtract: bool=False, standardize: bool=False, dtype: tf.dtypes.DType=tf.float32, no_crop_padding: bool=True) -> tf.Tensor:
    ...
    """
    images = decode_and_center_crop(image_bytes, image_size, no_crop_padding=no_crop_padding)
...
def build_eval_dataset(filenames: List[Text], labels: List[int]=None, image_size: int=IMAGE_SIZE, batch_size: int=1) -> tf.Tensor:
    ...
    dataset = dataset.map(lambda filename, label: (
     load_eval_image(filename, image_size), label))
    dataset = dataset.batch(batch_size)
    return dataset


def preprocess_for_train(image_bytes: tf.Tensor, image_size: int=IMAGE_SIZE, augmenter: Optional[ImageAugment]=None, mean_subtract: bool=False, standardize: bool=False, dtype: tf.dtypes.DType=tf.float32, min_object_covered: float=0.7, aspect_ratio_range: tuple=(0.4, 2.5), area_range: tuple=(0.7, 1.0), max_attempts: int=10) -> tf.Tensor:
    ...
    Returns:
      A preprocessed and normalized image `Tensor`.
    """
    images = decode_crop_and_flip(image_bytes=image_bytes, min_object_covered=min_object_covered, aspect_ratio_range=(list(aspect_ratio_range)),
      area_range=(list(area_range)),
      max_attempts=max_attempts)
