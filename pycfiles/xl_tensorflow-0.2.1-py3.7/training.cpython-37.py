...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xl_tensorflow\models\yolov3\training.py
# Compiled at: 2020-04-17 22:48:58
...
from .model import yolo_body, tiny_yolo_body, yolo_loss, yolo_efficientnetb3_body, yolo_efficientnetliteb4_body, yolo_efficientnetliteb1_body, yolo_efficientnetb0_body
...
def _main(train_annotation_path, val_annotation_path, classes_path, anchors_path, weights_path):
    ...
    model.fit_generator((data_generator_wrapper(train_lines, batch_size, input_shape, anchors, num_classes)), steps_per_epoch=(max(1, num_train // batch_size)),
    ...
    model.fit_generator((data_generator_wrapper(train_lines, batch_size, input_shape, anchors, num_classes)), steps_per_epoch=(max(1, num_train // batch_size)),
      ...
      initial_epoch=50,
      callbacks=[
     log, checkpoint, reduce_lr, early_stopping])
...
def create_model(input_shape, anchors, num_classes, load_pretrained=True, freeze_body=2, weights_path='model_data/yolo_weights.h5'):
    ...
    model_loss = Lambda(yolo_loss, output_shape=(1, ), name='yolo_loss', arguments={'anchors':anchors, 
     'num_classes':num_classes,  'ignore_thresh':0.5})([
...
def create_tiny_model(input_shape, anchors, num_classes, load_pretrained=True, freeze_body=2, weights_path='model_data/tiny_yolo_weights.h5'):
    ...
    if load_pretrained:
        ...
        if freeze_body in (1, 2):
            ...
            print('Freeze the first {} layers of total {} layers.'.format(num, len(model_body.layers)))
    model_loss = Lambda(yolo_loss, output_shape=(1, ), name='yolo_loss', arguments={'anchors':anchors, 
     'num_classes':num_classes,  'ignore_thresh':0.7})([
     *model_body.output, *y_true])
    model = Model([model_body.input, *y_true], model_loss)
    return model


def create_datagenerator(train_annotation_path, val_annotation_path, batch_size, input_shape, anchors, num_classes, seed=100):
    with open(train_annotation_path, encoding='utf-8') as (f):
        train_lines = f.readlines()
    with open(val_annotation_path, encoding='utf-8') as (f):
        val_lines = f.readlines()
    ...
    val = data_generator_wrapper(val_lines, batch_size, input_shape, anchors, num_classes)
...
def mul_gpu_training(train_annotation_path, val_annotation_path, classes_path, batch_size=8, input_shape=(416, 416), body='darknet', suffix='voc', pre_weights=None, giou_loss=False, mul_gpu=False):
    """
    Todo 加速训练
    Args:
        number_classes:
    ...
    callback = create_callback(f"./log/yolo_{body}_{suffix}", f"./mdoel/yolo_{body}_{suffix}_weights.h5", early_stopping_patience=20)
    train_dataset, val_dataset, num_train, num_val = create_datagenerator(train_annotation_path, val_annotation_path, batch_size, input_shape, YoloLoss.defalt_anchors, num_classes)
    with mirrored_strategy.scope():
        for i in range(185):
