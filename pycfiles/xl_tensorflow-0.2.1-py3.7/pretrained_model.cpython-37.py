...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xl_tensorflow\utils\pretrained_model.py
# Compiled at: 2020-04-04 22:37:29
# Size of source mod 2**32: 6825 bytes
import os, numpy as np, sys
from tensorflow.keras.applications import ResNet50, ResNet50V2, InceptionV3, DenseNet169, InceptionResNetV2, MobileNet, ResNet101, NASNetMobile, VGG16, MobileNetV2
...
from xl_tensorflow.models.efficientnet import EfficientNetB0, EfficientNetB1, EfficientNetB2, EfficientNetB3, EfficientNetB4, EfficientNetB5, EfficientNetB6, EfficientNetB7, EfficientNetLiteB1, EfficientNetLiteB2, EfficientNetLiteB3, EfficientNetLiteB4
from xl_tensorflow.models import MobileNetV3Small, MobileNetV3Large
eff_input_dict = {'efficientnetb0':224, 
 'efficientnetb1':240,  'efficientnetb2':260, 
 'efficientnetb3':300, 
 'efficientnetb4':380, 
 'efficientnetb5':456, 
 'efficientnetb6':528, 
...
def my_call_backs(model_name, log_path=None, model_path=None, monitor='val_loss', patience=5, reducelr=3):
    ...
    if 'win' in sys.platform:
        log_dir = os.path.join(os.getcwd(), '\\logs\\{}'.format(model_name)) if not log_path else os.path.join(log_path, model_name)
    else:
        log_dir = './logs/{}'.format(model_name) if not model_path else os.path.join(model_path, model_name)
    model_path = './model'
    ...
    early_stop = EarlyStopping(monitor=monitor, min_delta=1e-07, patience=patience, verbose=0, mode='auto', baseline=None)
    model_check_point = ModelCheckpoint(('./model/{}_weight.h5'.format(model_name)), verbose=1, save_best_only=True, save_weights_only=True)
...
class ImageFineModel:
    ...
    weight = {'resnet50':'resnet50_weights_tf_dim_ordering_tf_kernels_notop.h5', 
    ...
    @classmethod
    def create_fine_model(cls, pre_model_name, cat_num, suffix='', prefix='my_', weights='imagenet', non_flatten_trainable=False, loss='categorical_crossentropy', using_se_global_pooling=False, metrics=('accuracy',), learning_rate=0.0005, input_shape=None, dropout=None):
        if pre_model_name in ('mobilenetv3large', 'mobilenetv3small'):
            model = cls.model_dict[pre_model_name](include_top=True, weights=None, classes=cat_num, input_shape=input_shape) if input_shape else cls.model_dict[pre_model_name](include_top=True, weights=None, classes=cat_num)
            ...
            if pre_model_name in ['efficientnetb{}'.format(i) for i in range(8)]:
                pre_model = cls.model_dict[pre_model_name](include_top=False, weights=weights, using_se_global_pooling=using_se_global_pooling,
                  input_shape=(
                 eff_input_dict[pre_model_name],
            ...
            model = Model(inputs=(pre_model.input), outputs=[x], name=(prefix + pre_model_name + suffix))
