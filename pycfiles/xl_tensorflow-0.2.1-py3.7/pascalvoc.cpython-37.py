...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xl_tensorflow\metrics\rafaelpadilla\pascalvoc.py
# Compiled at: 2020-04-06 02:40:28
...
def ValidatePaths(arg, nameArg, errors):
    if arg is None:
        ...
        if os.path.isdir(arg) is False and os.path.isdir(os.path.join(currentPath, arg)) is False:
...
parser.add_argument('-gt',
  '--gtfolder',
  dest='gtFolder',
  default=(os.path.join(currentPath, '/test/groundtruths')),
  metavar='',
  help='folder containing your ground truth bounding boxes')
parser.add_argument('-det',
  '--detfolder',
  dest='detFolder',
  default=(os.path.join(currentPath, '/test/detections')),
  metavar='',
...
parser.add_argument('-gtformat',
  ...
  metavar='',
  default='xywh',
  help="format of the coordinates of the ground truth bounding boxes: ('xywh': <left> <top> <width> <height>) or ('xyrb': <left> <top> <right> <bottom>)")
parser.add_argument('-detformat',
  dest='detFormat',
  metavar='',
  default='xywh',
  help="format of the coordinates of the detected bounding boxes ('xywh': <left> <top> <width> <height>) or ('xyrb': <left> <top> <right> <bottom>)")
parser.add_argument('-gtcoords',
  dest='gtCoordinates',
  default='abs',
  metavar='',
  help="reference of the ground truth bounding box coordinates: absolute values ('abs') or relative to its image size ('rel')")
...
parser.add_argument('-sp',
  '--savepath', dest='savePath', metavar='', help='folder where the plots are saved')
...
if ValidateMandatoryArgs(args.gtFolder, '-gt/--gtfolder', errors):
    ...
    imgSize = (0, 0)
    if gtCoordType == CoordinatesType.Relative:
        imgSize = ValidateImageSize(args.imgSize, '-imgsize', '-gtCoordinates', errors)
    if detCoordType == CoordinatesType.Relative:
...
if ValidateMandatoryArgs(args.detFolder, '-det/--detfolder', errors):
    ...
    elif args.savePath is not None:
    ...
    allBoundingBoxes, allClasses = getBoundingBoxes(gtFolder,
      True, gtFormat, gtCoordType, imgSize=imgSize)
    allBoundingBoxes, allClasses = getBoundingBoxes(detFolder,
      False, detFormat, detCoordType, allBoundingBoxes, allClasses, imgSize=imgSize)
    allClasses.sort()
    evaluator = Evaluator()
    acc_AP = 0
    validClasses = 0
